...
# [GCC 8.4.0]
# Embedded file name: /Users/duranton/Documents/CERFACS/CODES/arnica/src/arnica/solvers_2d/core_fd.py
# Compiled at: 2020-03-30 05:30:51
# Size of source mod 2**32: 14869 bytes
"""
...
class Metrics2d:
    ...
    def _compute_metric(self):
        ...
        inv_det_j = np.reciprocal(diff_x_ns * diff_y_we - diff_x_we * diff_y_ns)
    ...
    def _compute_matrices_csr(self):
        ...
        grad_y = scp.csr_matrix((
         array_y, (np.repeat(ijc_array, 4), column_y)),
          shape=(
         size, size))
        self.lapl = grad_x.dot(grad_x) + grad_y.dot(grad_y)
    ...
    def _get_bc_iterators(self):
        ...
        """
        i_we = self.size_we - 1
        north = np.flip(np.array([j_idx + self.size_ns * i_we for j_idx in range(self.size_ns)]))
        j_ns = 0
        west = np.flip(np.array([j_ns + self.size_ns * i_idx for i_idx in range(self.size_we)]))
        i_we = 0
        south = np.array([j_idx + self.size_ns * i_we for j_idx in range(self.size_ns)])
        j_ns = self.size_ns - 1
        east = np.array([j_ns + self.size_ns * i_idx for i_idx in range(self.size_we)])
        self.bnd_nodes = {}
    ...
    def diff_ns_2(self, array_):
        ...
        size_j = out.shape[1]
        array_2 = 0.5 * (array_[:, range(2, size_j)] - array_[:, range(0, size_j - 2)])
    ...
    def diff_we_2(self, array_):
        ...
        out = np.zeros_like(array_)
        size_i = out.shape[0]
        array_2 = 0.5 * (array_[range(2, size_i), :] - array_[range(0, size_i - 2), :])
...
def set_row_csr(csr, row_idx, new_row):
    ...
    matrix A
    """
    if not scp.isspmatrix_csr(csr):
        raise AssertionError('A shall be a csr_matrix')
    else:
        assert row_idx < csr.shape[0], 'The row index ({0}) shall be smaller than the number of rows in A ({1})'.format(row_idx, csr.shape[0])
        try:
    ...
    csr.indptr = np.r_[(csr.indptr[:row_idx + 1],
     csr.indptr[row_idx + 1:] + additional_nnz)]