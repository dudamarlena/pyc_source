...
# [GCC 8.4.0]
# Embedded file name: /Users/duranton/Documents/CERFACS/CODES/arnica/src/arnica/utils/cloud2cloud.py
...
def _build_ravel_component(in_xyz, limitsize=None):
    ...
    if limitsize is not None:
        ...
        if skipper > 1:
            print('Warning, source data too big ({0}/{1}),\n sub sampling every {2}th element.'.format(input_size, limitsize, skipper))
...
def _interpolate_var(in_val, index, inv_dist):
    ...
    estimate = np.sum((inv_dist * np.reshape(in_val[np.ravel(index)], np.shape(inv_dist))),
      axis=1)
...
def _compute_dists(source_xyz, target_xyz, limitsource, stencil, power):
    sce_ravel, skipper = _build_ravel_component(source_xyz, limitsize=limitsource)
...
def cloud2cloud(source_xyz, source_val, target_xyz, stencil=3, **kwargs):
    ...
    """
    opts = {'limitsource':None, 
     'power':1.0,  'tol':None}
    for keyword in opts:
        if keyword in kwargs:
            opts[keyword] = kwargs[keyword]

    index, dists, inv_dist, skipper = _compute_dists(source_xyz, target_xyz, opts['limitsource'], stencil, opts['power'])
    target_val = {}
    for key in source_val:
        if stencil > 1:
            estimate = _interpolate_var(np.ravel(source_val[key])[::skipper], index, inv_dist)
        else:
            estimate = np.ravel(source_val[key])[::skipper][index]
        if opts['tol'] is not None:
            estimate = np.where(dists[:, 0] > opts['tol'], 0, estimate)
        target_val[key] = np.reshape(estimate, np.take(target_xyz, 0, axis=(-1)).shape)
