...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/tincan.py
# Compiled at: 2019-07-15 16:10:15
# Size of source mod 2**32: 34862 bytes
import os, sys, ast, binascii
from base64 import b16encode, b16decode
from chameleon import PageTemplate, PageTemplateFile, TemplateError
import email.utils, functools, importlib
from inspect import isclass
import io, logging, mimetypes, py_compile
from stat import S_ISDIR, S_ISREG
from string import whitespace
from threading import Lock
import time, urllib, bottle
...
class TemplateFile(object):
    ...
    def _header(self, line):
        ...
        if line.startswith(self._END):
            if len(line) == self._LEND or line[self._LEND] in self._WS:
                self._state = self._body
...
class ChameleonTemplate(bottle.BaseTemplate):
    ...
    def prepare(self, **options):
        if self.source:
            ...
            self.tpl = PageTemplateFile(self.filename, encoding=self.encoding, search_path=self.lookup, **options)
...
class _LoadedFile(object):
    ...
    def __init__(self, raw):
        if raw.startswith('<'):
            ...
            raise _casef(self.fname).endswith(_IEXTEN) or ValueError('file does not end in {0}'.format(_IEXTEN))
...
class _TinCanBaseRoute(object):
    ...
    def __init__(self, launcher, name, subdir):
        global _get_template_cache
        global _get_template_nocache
...
class _TinCanRoute(_TinCanBaseRoute):
    ...
    def launch(self):
        ...
        while True:
            ...
            if oheader is None:
                oheader = self._header
            else:
                if (oheader.errors is None) != (self._header.errors is None):
    ...
    def _mkerror(self, rerrors):
        ...
        route = _TinCanErrorRoute(ChameleonTemplate(source=(self._template.body), encoding=(self._encoding)), self._loads, self._class, self.lock, self.logger)
        for error in errors:
            if error < _ERRMIN or error > _ERRMAX:
                raise TinCanError('{0}: bad #errors code'.format(self._urlpath))
            self._app.error_handler[error] = route
