...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ddmonitoring/utils/view.py
# Compiled at: 2019-12-01 08:38:12
# Size of source mod 2**32: 8662 bytes
import os, time
from collections import defaultdict
from asciimatics.widgets import Frame, Layout, MultiColumnListBox, Label, TextBox, Divider
from asciimatics.screen import Screen
from ddmonitoring.utils.constants import DATA_INTERVAL_SHORT, DATA_INTERVAL_LONG, REFRESH_LONG, REFRESH_SHORT

class UIInfo:
    __doc__ = 'Class to hold UI data'

    def __init__(self, last_frames, last_refreshes, headers, interval, displayed_codes, base_titles, columns):
...
class MonitorFrame(Frame):
    __doc__ = "This class details the 'window' to be drawn into the terminal"
    ...
    def __init__(self, screen, model):
        ...
        now = time.time()
        self._last_frame = 0
        self._ui_info = UIInfo(last_frames=(defaultdict(int)),
          last_refreshes=(defaultdict(lambda : now)),
          headers={'LONG':'Past hour - ', 
        ...
        self._metrics_short = MultiColumnListBox(height=(screen.height // 3 - 3),
          columns=(self._ui_info.columns),
          options=[],
          titles=(self._ui_info.titles),
          name='Metrics Short')
        self._metrics_long = MultiColumnListBox(height=(screen.height // 3 - 3),
          columns=(self._ui_info.columns),
          options=[],
          titles=(self._ui_info.titles),
          name='Metrics Long')
        self._alerts = MultiColumnListBox(height=(screen.height // 3 - 3),
          columns=[
         100],
          options=[],
          titles=[
        ...
        layout.add_widget(Divider())
        layout.add_widget(Label('Press `Ctrl+{0}` to exit, `Tab` to switch focus,            Arrow Keys to scroll.'.format('Break' if os.name == 'nt' else 'C')))
        self.fix()
    ...
    def _update(self, frame_no):
        if frame_no - self._last_frame >= self.frame_update_count or self._last_frame == 0:
            ...
            self.make_refresh_header(base_header=(self._ui_info.headers['SHORT']),
              header=(self._metrics_h_short),
              refresh=REFRESH_SHORT,
              last_refresh=(self._ui_info.last_refreshes['SHORT']),
              now=now)
            self.make_refresh_header(base_header=(self._ui_info.headers['LONG']),
    ...
    def make_refresh_header(base_header, header, refresh, last_refresh, now):
        ...
        header.value = base_header + 'refreshing in: ' + str(int(round(last_refresh + refresh - now)))