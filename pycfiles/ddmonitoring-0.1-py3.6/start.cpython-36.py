...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ddmonitoring/utils/start.py
# Compiled at: 2019-12-01 08:58:26
...
def setup_background_jobs(scheduler, model):
    ...
    def configure_metrics_job(run_interval, data_interval):
        scheduler.add_job((model.make_global_metrics),
          trigger=None,
          args=[
         data_interval])
        scheduler.add_job((model.make_global_metrics),
    ...
    for website in check_data:
        ...
        for check_interval in intervals:
            scheduler.add_job((model.check),
              trigger=None,
              args=[
             website, const.DATA_INTERVAL_SHORT])
            scheduler.add_job((model.check),
              trigger='interval',
...
async def populate_metrics_data(model, user_data):
    ...
    futures = [loop.run_in_executor(None, model.check, website, const.DATA_INTERVAL_SHORT) for website in user_data]
...
def validate_input(user_data, logger):
    ...
    url_mask = "^(?:http(s)?:\\/\\/)?[\\w.-]+(?:\\.[\\w\\.-]+)+[\\w\\-\\._~:/?#[\\]@!\\$&'\\(\\)\\*\\+,;=.]+$"
    success = False
    input_file_schema = {'type':'object', 
     'patternProperties':{url_mask: {'type':'array', 
                 'items':{'type':'number', 
    ...
    try:
        ...
        logger.warning("The input file format is wrong. Reason: %s.             Reminder: the configuration JSON file must be a single JSON object,            with each key being the website's URL you want to check, and each value being an array             of check intervals in seconds, greater than 2 seconds.", valid_err.message)
    else:
