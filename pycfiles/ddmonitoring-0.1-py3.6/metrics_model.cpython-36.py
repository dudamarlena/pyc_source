...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ddmonitoring/utils/metrics_model.py
# Compiled at: 2019-12-01 06:39:56
# Size of source mod 2**32: 7024 bytes
...
class MetricsModel(object):
    ...
    def make_metrics_for(self, website, data_interval):
        ...
        data_considered = [data for data in self.res_data[website] if (now - data['call_time']).total_seconds() < data_interval]
    ...
    def _get(self, website, timeout):
        ...
        load_reducer_coefficient = 0.6
        self.logger.debug('requesting ' + website)
        try:
            headers = {'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.102 Safari/537.36'}
            res = requests.get(website, timeout=(timeout * load_reducer_coefficient), headers=headers)
        except Exception as e:
            res = str(e.__class__)
        else:
            success = True
        return (
         res, req_time, success)
    ...
    def _handle_alerts_for(self, website):
        ...
        if metrics_data['prev_av'] >= 80:
            if metrics_data['availability'] < 80:
                self.alerts.append(self._make_alert_for(website, 'DOWN', metrics_data['availability']))
        if metrics_data['prev_av'] < 80:
            if metrics_data['availability'] >= 80:
                if metrics_data['prev_av'] != -1:
