...
# [GCC 8.4.0]
# Embedded file name: /Volumes/Ozgur/Dropbox/Sites/django-manifest/manifest/utils.py
# Compiled at: 2019-10-15 15:40:16
# Size of source mod 2**32: 4144 bytes
""" Manifest Utilities
"""
import datetime, hashlib, random, urllib
...
def get_image_path(instance, filename):
    ...
    key = generate_sha1('_'.join([str(datetime.datetime.now()), str(instance.id)]))
    return '%(path)s/%(key)s.%(extension)s' % {'path':getattr(defaults, 'MANIFEST_PICTURE_PATH', '%s/%s' % (
...
def validate_picture(file, handler):
    if file:
        ...
        if content_type:
            ...
            raise main == 'image' and sub in defaults.MANIFEST_PICTURE_FORMATS or handler.ValidationError(_('%s only.' % defaults.MANIFEST_PICTURE_FORMATS))
...
def get_gravatar(email):
    ...
    gravatar = '%(url)s%(id)s?' % {'url':url, 
     'id':hashlib.md5(email.lower().encode('utf-8')).hexdigest()}
    gravatar += urllib.parse.urlencode({'s':str(defaults.MANIFEST_AVATAR_SIZE), 
...
def get_login_redirect(redirect=None):
    ...
    if redirect:
        return redirect
    return defaults.MANIFEST_LOGIN_REDIRECT_URL
...
def generate_sha1(string, salt=None):
    ...
    key = hashlib.sha1((salt + str(string)).encode('utf-8')).hexdigest()
    return (
     salt, key)
