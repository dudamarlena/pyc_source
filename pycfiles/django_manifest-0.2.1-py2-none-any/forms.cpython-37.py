...
# [GCC 8.4.0]
# Embedded file name: /Volumes/Ozgur/Dropbox/Sites/django-manifest/manifest/forms.py
# Compiled at: 2019-10-15 15:40:16
# Size of source mod 2**32: 8779 bytes
""" Manifest Forms
"""
...
class LoginForm(forms.Form):
    ...
    identification = forms.CharField(label=(_('Username or Email')),
      widget=forms.TextInput(attrs=ATTRS_DICT),
      max_length=75,
      error_messages={'required': _('Please enter your username or email address.')})
        ...
        if identification:
            if password:
                ...
                if self.user_cache is None:
                    raise forms.ValidationError(_('Please check your identification and password.'))
...
class RegisterForm(UserCreationForm):
    ...
    username = forms.RegexField(label=(_('Username')),
      regex='^\\w+$',
      max_length=30,
      widget=forms.TextInput(attrs=dict(ATTRS_DICT, placeholder=(_('Pick a username')))),
    ...
    password1 = forms.CharField(label=(_('Password')),
      widget=forms.PasswordInput(attrs=dict(ATTRS_DICT, placeholder=(_('Enter a password')))))
    password2 = forms.CharField(label=(_('Comfirm pasword')),
      widget=forms.PasswordInput(attrs=dict(ATTRS_DICT, placeholder=(_('Enter password again')))))
    ...
    def clean_username(self):
        ...
        if self.cleaned_data['username'].lower() in defaults.MANIFEST_FORBIDDEN_USERNAMES:
    ...
    def clean_email(self):
        ...
        """
        if get_user_model().objects.filter(Q(email__iexact=(self.cleaned_data['email'])) | Q(email_unconfirmed__iexact=(self.cleaned_data['email']))):
    ...
    def save(self, commit=True):
        ...
        """
        user = get_user_model().objects.create_user(self.cleaned_data['username'], self.cleaned_data['email'], self.cleaned_data['password1'], not defaults.MANIFEST_ACTIVATION_REQUIRED)
        return user
...
class ProfileUpdateForm(forms.ModelForm):
    ...
    birth_date = forms.DateField(required=True,
      widget=forms.widgets.SelectDateWidget(years=(range(datetime.today().year - 10, datetime.today().year - 99, -1))))
