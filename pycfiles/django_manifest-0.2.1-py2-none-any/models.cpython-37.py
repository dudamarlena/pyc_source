...
# [GCC 8.4.0]
# Embedded file name: /Volumes/Ozgur/Dropbox/Sites/django-manifest/manifest/models.py
...
class AccountActivationMixin(models.Model):
    ...
    def activation_key_expired(self):
        ...
        """
        expiration_days = datetime.timedelta(days=(defaults.MANIFEST_ACTIVATION_DAYS))
...
class UserProfileMixin(models.Model):
    ...
    picture = models.ImageField((_('Picture')),
      blank=True, null=True, upload_to=get_image_path)
    mugshot = ImageSpecField(source='picture',
      processors=[
     ResizeToFill(defaults.MANIFEST_AVATAR_SIZE, defaults.MANIFEST_AVATAR_SIZE)],
      format='JPEG',
      options={'quality': 80})
    ...
    def get_absolute_url(self):
        return (
         'user_detail', None, {'username': self.username})
    ...
    def get_full_name_or_username(self):
        ...
        if self.first_name or self.last_name:
            name = _('%(first_name)s %(last_name)s') % {'first_name':self.first_name, 
    ...
    def get_short_name_or_username(self):
        ...
        """
        if self.first_name or self.last_name:
            name = '%(first_name)s %(last_name)s.' % {'first_name':self.first_name, 
...
class BaseUser(AccountActivationMixin, EmailConfirmationMixin, UserProfileMixin, UserLocaleMixin):
