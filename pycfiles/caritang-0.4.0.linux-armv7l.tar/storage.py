...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.5/site-packages/caritang/core/storage.py
...
@author: thierry
"""
import logging, os.path, mimetypes, gdata.docs, gdata.docs.service, gdata.photos.service, gdata.media, gdata.geo
from ..common import version
version.getInstance().submitRevision('$Revision: 175 $')
...
class AggregatorStorage(DocStorage):
    ...
    def isConnected(self):
        return False not in map(lambda storage: storage.isConnected(), self.storages)
...
class Picassa(DocStorage):
    ...
    def canStore(self, doc):
        ...
        if type == 'image/jpeg' or type == 'video/mp4':
            return True
        return False
    ...
    def store(self, docItem):
        ...
        try:
            if self.album is None:
                photo = self.gd_client.InsertPhotoSimple('/data/feed/api/user/default/albumid/default', photoName, 'Uploaded with Caritang', doc, content_type=type)
            else:
                album_url = '/data/feed/api/user/%s/albumid/%s' % (self.login, self.album_entry.gphoto_id.text)
                photo = self.gd_client.InsertPhotoSimple(album_url, photoName, 'Uploaded with Caritang', doc, content_type=type)
...
class GoogleDocs(DocStorage):
    ...
    def canStore(self, doc):
        extension = doc.getName().rpartition('.')[2].upper()
        return not extension == 'PDF' and extension in gdata.docs.service.SUPPORTED_FILETYPES.keys()
    ...
    def store(self, docItem):
        ...
        """
        doc = docItem.getFile()
        extension = docItem.getName().rpartition('.')[2].upper()
        type = gdata.docs.service.SUPPORTED_FILETYPES[extension]
        ...
        try:
            ...
            logging.exception('GData client failed to upload the file')
            raise CaritangStorageException()
