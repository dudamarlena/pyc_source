...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/improb/decision/tree.py
# Compiled at: 2011-06-09 03:27:23
"""Decision trees."""
from abc import ABCMeta, abstractmethod, abstractproperty
import cdd, collections, itertools, numbers
...
class Tree(collections.MutableMapping):
    ...
    def __str__(self):
        ...
        children = [ key.name if isinstance(key, Event) else str(key) for key in self
                   ]
        subtrees = [ str(subtree).split('\n') for subtree in self.itervalues()
                   ]
        width = max(len(child) for child in children) + 2
        children = [ child.ljust(width, '-') + subtree[0] for (child, subtree) in itertools.izip(children, subtrees)
                   ]
        children = [ ('\n').join([child] + [ '|' + ' ' * (width - 1) + line for line in subtree[1:] ] + ['|']) for (child, subtree) in itertools.izip(children[:-1], subtrees[:-1])
                   ] + [ ('\n').join([child] + [ ' ' * width + line for line in subtree[1:] ]) for (child, subtree) in itertools.izip(children[-1:], subtrees[-1:])
                       ]
        return ('\n').join(children)
    ...
    def get_norm_opt(self, opt=None, event=True):
        ...
        if opt is None:
            ...
            opt_gambles = set(opt((gamble for (gamble, tree) in normal_form), event))
    ...
    def get_norm_back_opt(self, opt=None, event=True):
        ...
        if opt is None:
            ...
            opt_gambles = set(opt((gamble for (gamble, tree) in _norm_back_opt), event))
...
class Reward(Tree, cdd.NumberTypeable):
    ...
    def __repr__(self):
        return ("Reward({0}, number_type='{1}')").format(self.number_repr(self.reward), self.number_type)

    def __add__(self, value):
        return Reward(self.reward + self.make_number(value), number_type=self.number_type)
...
class Decision(Tree):
    ...
    def _get_norm_back_opt(self, opt=None, event=True):
        for (decision, subtree) in self.iteritems():
            for (gamble, normal_subtree) in subtree.get_norm_back_opt(opt, event):
                yield (
                 gamble, Decision(data={decision: normal_subtree}))
    ...
    def __repr__(self):
        return 'Decision({' + (', ').join(('{0}: {1}').format(repr(key), repr(value)) for (key, value) in self.iteritems()) + '})'

    def __add__(self, value):
        return Decision(OrderedDict((decision, subtree + value) for (decision, subtree) in self.iteritems()))
...
class Chance(Tree):
    ...
    def _get_norm_back_opt(self, opt=None, event=True):
        ...
        all_normal_forms = itertools.product(*[ tuple(subtree.get_norm_back_opt(opt, event)) for subtree in self.itervalues()
                                              ])
        for normal_forms in all_normal_forms:
            data = {}
            tree = OrderedDict()
            for (event, (gamble, normal_subtree)) in itertools.izip(self.iterkeys(), normal_forms):
                for omega in event:
                    if isinstance(gamble, numbers.Real):
                        ...
                        raise RuntimeError('expected int, long, float, or Gamble')

                tree[event] = normal_subtree

            yield (
             Gamble(pspace=self.pspace, data=data, number_type=number_type),
    ...
    def __repr__(self):
        return 'Chance(' + ('pspace={0}').format(repr(self.pspace)) + ', data={' + (', ').join(('{0}: {1}').format(repr(key), repr(value)) for (key, value) in self.iteritems()) + '})'

    def __add__(self, value):
        return Chance(self.pspace, OrderedDict((event, subtree + value) for (event, subtree) in self.iteritems()))
