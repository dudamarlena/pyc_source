...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/improb/lowprev/lowpoly.py
# Compiled at: 2011-06-09 03:27:23
"""Polyhedral lower previsions."""
from __future__ import division, absolute_import, print_function
import cdd, collections
...
class LowPoly(LowPrev):
    ...
    def __init__(self, pspace=None, mapping=None, lprev=None, uprev=None, prev=None, lprob=None, uprob=None, prob=None, bba=None, credalset=None, number_type=None):
        ...
        def iter_items(obj):
            ...
            if isinstance(obj, collections.Sequence):
                ...
                return (((omega,), value) for (omega, value) in itertools.izip(self.pspace, obj))
            raise TypeError('expected collections.Mapping or collections.Sequence')

        def get_number_type(xprevs, xprobs):
            """Determine number type from arguments."""
            if all(xprev is None for xprev in xprevs) and all(xprob is None for xprob in xprobs):
                return 'float'
            else:
                for xprev in xprevs:
                    if xprev is None:
                    ...
                    for (key, value) in xprev.iteritems():
                        ...
                        if isinstance(value, collections.Sequence):
                            if cdd.get_number_type_from_sequences(value) == 'float':
                                return 'float'
                        elif cdd.get_number_type_from_value(value) == 'float':
                            return 'float'
        ...
        if number_type is None:
            number_type = get_number_type([
             ...
             lprob, uprob, prob] + (credalset if credalset else []))
        ...
        if bba:
            setfunc = SetFunction(pspace=self.pspace, data=bba, number_type=self.number_type)
    ...
    def __str__(self):
        ...
        if self:
            maxlen_value = max(max(len(self.number_str(gamble[omega])) for omega in self.pspace) for (gamble, event) in self)
            maxlen_prev = max(max(len(self.number_str(prev)) for prev in prevs if prev is not None) for prevs in self.itervalues())
        else:
            maxlen_value = 0
            maxlen_prev = 0
        maxlen = max(maxlen_pspace, maxlen_value)
        result = (' ').join(('{0: ^{1}}').format(omega, maxlen) for omega in self.pspace) + '\n'
        result += ('\n').join((' ').join(('{0:{1}}').format(self.number_str(value), maxlen) for value in gamble.values()) + ' | ' + (' ').join(('{0:{1}}').format(omega if omega in event else '', maxlen_pspace) for omega in self.pspace) + ' : [' + (('{0:{1}}').format(self.number_str(lprev), maxlen_prev) if lprev is not None else ' ' * maxlen_prev) + ', ' + (('{0:{1}}').format(self.number_str(uprev), maxlen_prev) if uprev is not None else ' ' * maxlen_prev) + ']' for ((gamble, event), (lprev, uprev)) in sorted(self.iteritems(), key=lambda val: tuple(-x for x in val[0][1].indicator(number_type='fraction').values()) + tuple(x for x in val[0][0].values())))
        return result
    ...
    def get_matrix(self, gamble=None, event=True):
        ...
        matrix.obj_func = [0] + [ gamble[omega] if omega in event else 0 for omega in self.pspace
                                ]
        return matrix
    ...
    def _get_relevant_items(self, event=True, items=None):
        ...
        num_evs = len(evs)
        low_items = [ ((ga, ev), (lprev, uprev)) for ((ga, ev), (lprev, uprev)) in items if lprev is not None
                    ]
        upp_items = [ ((ga, ev), (lprev, uprev)) for ((ga, ev), (lprev, uprev)) in items if uprev is not None
                    ]
        num_items = len(low_items + upp_items)
        matrix = cdd.Matrix([ [0] + [ 1 if i == j else 0 for i in xrange(num_evs) ] + [ 0 for i in xrange(num_items) ] for j in xrange(num_evs) ] + [ [1] + [ -1 if i == j else 0 for i in xrange(num_evs) ] + [ 0 for i in xrange(num_items) ] for j in xrange(num_evs) ] + [ [0] + [ 0 for i in xrange(num_evs) ] + [ 1 if i == j else 0 for i in xrange(num_items) ] for j in xrange(num_items) ] + [ [0] + [ -1 if omega in ev else 0 for ev in evs ] + [ lprev - ga[omega] if omega in ev else 0 for ((ga, ev), (lprev, uprev)) in low_items ] + [ ga[omega] - uprev if omega in ev else 0 for ((ga, ev), (lprev, uprev)) in upp_items ] for omega in compl_event
                                                                                                                                                                                                                                                                                                                                                                                                       ], number_type=self.number_type)
        matrix.rep_type = cdd.RepType.INEQUALITY
        matrix.obj_type = cdd.LPObjType.MAX
        matrix.obj_func = [
         0] + [1] * num_evs + [0] * num_items
        ...
        new_items = set(((ga, ev), (lprev, uprev)) for ((ga, ev), (lprev, uprev)) in items if ev in new_evs)
        if items == new_items:
            return items
        else:
    ...
    def _get_matrix(self, event=True):
        ...
        add_constraint([
         +1] + [ -1 if omega in event else 0 for omega in self.pspace ], linear=True)
        ...
        for ((ga, ev), (lprev, uprev)) in self.get_relevant_items(event=event):
            if lprev is None and uprev is None:
                ...
                add_constraint([
                 0] + [ value - lprev if omega in ev else 0 for (omega, value) in ga.iteritems()
                      ], linear=True)
            else:
                ...
                if uprev is not None:
                    add_constraint([
                     0] + [ uprev - value if omega in ev else 0 for (omega, value) in ga.iteritems()
                          ])
    ...
    def set_lower(self, gamble, lprev, event=True):
        ...
        if lprev is None:
            ...
            try:
                ...
                lprev = max(lprev, old_lprev) if old_lprev is not None else lprev
    ...
    def set_upper(self, gamble, uprev, event=True):
        ...
        if uprev is None:
            ...
            try:
                ...
                uprev = min(uprev, old_uprev) if old_uprev is not None else uprev
    ...
    def set_precise(self, gamble, prev, event=True):
        ...
        if prev is None:
            ...
            try:
                ...
                lprev = max(prev, old_lprev) if old_lprev is not None else prev
                uprev = min(prev, old_uprev) if old_uprev is not None else prev
    ...
    def get_credal_set(self, event=True):
        ...
        for vert in poly.get_generators():
            if event.is_true():
                ...
                vert = tuple((vert[(i + 1)] if omega in event else 0) for (i, omega) in enumerate(self.pspace))
    ...
    def get_coherent(self, algorithm='linprog'):
        ...
        for ((gamble, event), (lprev, uprev)) in list(mapping.iteritems()):
            ...
            mapping[(gamble, event)] = (
             lprev, uprev)

        return self.__class__(pspace=self.pspace, mapping=mapping, number_type=self.number_type)
    ...
    def is_coherent(self, algorithm='linprog'):
        if not self.is_avoiding_sure_loss(algorithm):
            ...
            for (gamble, event) in self:
                (lprev, uprev) = self[(gamble, event)]
                if lprev is not None and self.number_cmp(self.get_lower(gamble, event, algorithm), lprev) == 1:
                    return False
                if uprev is not None and self.number_cmp(self.get_upper(gamble, event, algorithm), uprev) == -1:
                    return False
    ...
    def is_linear(self, algorithm='linprog'):
        if not self.is_avoiding_sure_loss(algorithm):
            return False
        for (gamble, event) in self:
            if self.number_cmp(self.get_lower(gamble, event, algorithm), self.get_upper(gamble, event, algorithm)) != 0:
                return False

        return True

    def get_extend_domain(self):
        raise ValueError('cannot extend to full domain: specify keys')