...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/improb/lowprev/linvac.py
...
class LinVac(BelFunc):
    ...
    def get_lower(self, gamble, event=True, algorithm='linvac'):
        ...
        if algorithm != 'linvac':
            ...
            return (sum(self[({omega: 1}, True)][0] * gamble[omega] for omega in event) + epsilon * min(gamble[omega] for omega in event)) / (sum(self[({omega: 1}, True)][0] for omega in event) + epsilon)
