...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/improb/__init__.py
# Compiled at: 2011-06-13 11:09:05
"""improb is a Python module for working with imprecise probabilities."""
from __future__ import division, absolute_import, print_function
__version__ = '0.1.1'
__release__ = __version__
from abc import ABCMeta, abstractmethod, abstractproperty
import cdd, collections, fractions, itertools, numbers
...
def _str_keys_values(keys, values):
    ...
    return ('\n').join(('{0: <{1}} : {2}').format(key, maxlen_keys, value) for (key, value) in itertools.izip(keys, values))
...
class PSpace(collections.Set, collections.Hashable):
        ...
        * As a special case, you can also specify just a single integer. This
        ...
        if not args:
            ...
            if isinstance(arg, int):
                ...
                added = set()
                self._data = tuple(elem for elem in arg if elem not in added if not added.add(elem))
            else:
                raise TypeError('specify possibility space as iterable or integer')
        else:
            self._data = tuple(itertools.product(*[ PSpace(arg) for arg in args ]))
    ...
    def make_event(self, *args, **kwargs):
        ...
        if not args:
            ...
            if event is False:
                return Event(self, event, name=name)
            if isinstance(event, Gamble):
                if self != event.pspace:
                    raise ValueError('possibility space mismatch')
                if not set(event.itervalues()) <= set([0, 1]):
                    raise ValueError('not an indicator gamble')
                return Event(self, (omega for (omega, value) in event.iteritems() if value == 1), name=name)
            return Event(self, event, name=name)
        else:
    ...
    def subsets(self, event=True, empty=True, full=True, size=None, contains=False):
        ...
        if not contains <= event:
            ...
            if size is None:
                size_range = xrange(0 if empty else 1, len(event) + (1 if full else 0))
...
class Gamble(collections.Mapping, collections.Hashable, cdd.NumberTypeable):
    ...
    def __init__(self, pspace, data, number_type=None):
        ...
        if isinstance(data, collections.Mapping):
            if number_type is None:
                cdd.NumberTypeable.__init__(self, cdd.get_number_type_from_sequences(data.itervalues()))
            else:
                cdd.NumberTypeable.__init__(self, number_type)
            self._data = dict((omega, self.make_number(data.get(omega, 0))) for omega in self.pspace)
        elif isinstance(data, collections.Sequence):
            if len(data) < len(self.pspace):
                raise ValueError('data sequence too short')
            if number_type is None:
                cdd.NumberTypeable.__init__(self, cdd.get_number_type_from_sequences(data))
            else:
                cdd.NumberTypeable.__init__(self, number_type)
            self._data = dict((omega, self.make_number(value)) for (omega, value) in itertools.izip(self.pspace, data))
        elif isinstance(data, numbers.Real):
            if number_type is None:
                cdd.NumberTypeable.__init__(self, cdd.get_number_type_from_value(data))
            else:
                cdd.NumberTypeable.__init__(self, number_type)
            self._data = dict((omega, self.make_number(data)) for omega in self.pspace)
        else:
    ...
    def __repr__(self):
        ...
        Gamble(pspace=PSpace([2, 3, 4]),
               mapping={2: '1/3',
                        ...
                        4: 8})
        """
        return ('Gamble(pspace={0}, mapping={{{1}}})').format(repr(self._pspace), (', ').join(('{0}: {1}').format(repr(omega), self.number_repr(value)) for (omega, value) in self.iteritems()))
    ...
    def __str__(self):
        ...
        clouds : 20.0
        """
        return _str_keys_values(self.pspace, (self.number_str(value) for value in self.itervalues()))
    ...
    def _scalar(self, other, oper):
        ...
        return Gamble(self.pspace, [ oper(value, other) for value in self.itervalues() ], number_type=self.number_type)
    ...
    def _pointwise(self, other, oper):
        ...
        if isinstance(other, Gamble):
            ...
            return Gamble(self.pspace, [ oper(value, other_value) for (value, other_value) in itertools.izip(self.itervalues(), other.itervalues())
                                       ], number_type=self.number_type)
        else:
            if isinstance(other, Event):
                return self._pointwise(other.indicator(number_type=self.number_type), oper)
    ...
    def __neg__(self):
        return Gamble(self.pspace, [ -value for value in self.itervalues() ], number_type=self.number_type)
...
class Event(collections.Set, collections.Hashable):
    ...
    def __init__(self, pspace, data=False, name=None):
        ...
        def validated(omega):
            if omega in pspace:
                return omega
            raise ValueError(('event has element ({0}) not in possibility space').format(omega))
    ...
    def __str__(self):
        ...
        clouds : 1
        """
        return _str_keys_values(self.pspace, ((1 if omega in self else 0) for omega in self.pspace))
    ...
    def indicator(self, number_type=None):
        ...
        if number_type is None:
            number_type = 'float'
        return Gamble(self.pspace, dict((omega, 1 if omega in self else 0) for omega in self.pspace), number_type=number_type)
