...
# Embedded file name: build\bdist.win-amd64\egg\PyAndor\_AndorCamera.py
# Compiled at: 2018-01-18 05:28:16
import numpy as np, logging, ctypes
from ctypes import byref, c_int, c_float, c_char_p, c_long, c_ulong, POINTER
from ._AndorUtils import ANDOR_STATUS, ANDOR_CODES
...
class AndorCamera(object):
    READ_MODE_CODES = {'FVB': 0, 'Image': 4}
    TRIGGER_MODE_CODES = {'internal': 0, 'external': 1, 
       'softwareTrigger': 10}
    ACQUISITION_MODE_CODES = {'single': 1, 'accumulate': 2}
    OUTPUT_AMPLIFIERS = {'EMCCD': 0, 'CCD': 1}
    FAN_MODES = {'full': 0, 'low': 1, 
       'off': 2}
    ...
    def __init__(self):
        ...
        self.dll.SetExposureTime.argtypes = [
         c_float]
        ...
        self.dll.GetStatus.argtypes = [
         POINTER(c_int)]
        self.dll.GetMostRecentImage.argtypes = [POINTER(c_long), c_ulong]
        self.dll.GetBaselineClamp.argtypes = [
         POINTER(c_int)]
        ...
        self.dll.GetFastestRecommendedVSSpeed.argtypes = [
        ...
        self.dll.GetNumberADChannels.argtypes = [
         POINTER(c_int)]
    ...
    def Initialize(self, acquisitionMode='accumulate', triggerMode='internal', temperature=-85):
        self.logger.debug('Initialize')
        with self.lock:
            self._chk(self.dll.Initialize('.'))
    ...
    def GetAcquisitionTimings(self):
        with self.lock:
            ...
            self._chk(self.dll.GetAcquisitionTimings(byref(exposure), byref(accumulate), byref(kinetic)))
    ...
    def GetInfo(self):
        ...
        methods = [(self.IsCoolerOn, []),
         ...
         (
          self.GetNumberHSSpeeds, [(0, 'EMCCD'), (0, 'CCD')]),
         (
          self.GetHSSpeed, [(0, 'EMCCD', 0), (0, 'CCD', 0), (0, 'EMCCD', 1), (0, 'CCD', 1), (0, 'EMCCD', 2), (0, 'CCD', 2)]),
         (
          self.GetEMCCDGain, [])]
