...
# Embedded file name: build/bdist.linux-x86_64/egg/pyqart/art/source.py
# Compiled at: 2017-05-03 02:35:53
...
class QArtSourceImage(object):
    ...
    def _calc_contrast(self, img, y, x, dy, dx, rand):
        if 0 <= y < img.height:
            assert 0 <= x < img.width, 'Point out of image.'
    ...
    def to_image(self, args, dither, dy, dx):
        if not dx is None:
            assert dx > 0, 'dx must >= 0.'
            if not dy is None:
                assert dy > 0, 'dy must >= 0.'
