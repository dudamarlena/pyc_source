...
# Embedded file name: build/bdist.linux-x86_64/egg/pyqart/common/bits.py
# Compiled at: 2016-08-02 22:17:06
...
class Bits(object):
    ...
    def extend(self, other, start=0, count=None):
        ...
        if isinstance(other, (bytes, bytearray)):
            end = len(other) * _BIT_PER_BYTE if count is None else start + count
            end = min(end, len(other) * _BIT_PER_BYTE)
            for i in range(start, end):
                self.append_bit(bit_at(other[(i // _BIT_PER_BYTE)], _BIT_PER_BYTE, i % _BIT_PER_BYTE))
    ...
    def _expand_capacity(self, target):
        ...
        while target > self.capacity:
            self._raw += '\x00' * self.capacity_by_byte
    ...
    def __repr__(self):
        return 'Bits at {id}: [{self}]'.format(id=id(self), self=self)
    ...
    def __getitem__(self, index):
        if index >= self.length:
            raise IndexError()
        return bit_at(self._raw[(index // _BIT_PER_BYTE)], _BIT_PER_BYTE, index % _BIT_PER_BYTE)
