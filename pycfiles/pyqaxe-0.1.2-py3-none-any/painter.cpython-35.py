...
# Embedded file name: build/bdist.linux-x86_64/egg/pyqart/qr/painter/painter.py
# Compiled at: 2016-08-03 11:06:07
...
class QrPainter(object):
    ...
    def _get_and_test_params(self):
        ...
        if self._version is not None and self._version < min_version:
            raise QrPainterException('The {} version QrCode does not have enough space to encode the data your provided, minimum version is {}.'.format(self._version, min_version))
        return (
         min_version, available, used)
    ...
    def get_params(self):
        ...
        if self._version is None:
            ...
            encode_list = [cls(data, args.cci_length_of(cls)) for cls, data in self._data.data_set]
            used = sum([x.needed_space for x in encode_list])
            available = args.dcwc * BIT_PER_CW - used
        return (
         QrArgs(version, self._data.ec_level, self._mask, self._rotation), available, used)
