...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pdftables/display.py
# Compiled at: 2016-07-21 11:30:28
...
def to_string(table):
    ...
    hbar = '    {}\n'.format('-' * table_width)
    result.write('      {}\n'.format(' '.join([str(col_index).rjust(width, ' ') for col_index, width in enumerate(col_widths)])))
    result.write(hbar)
    for row_index, row in enumerate(table):
        cells = [cell.rjust(width, ' ') for cell, width in zip(row, col_widths)]
