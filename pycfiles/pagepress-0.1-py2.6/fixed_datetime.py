...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/pagepress/fixed_datetime.py
...
import time as _time, pytz as _pytz, math as _math, re as _re
...
def _parse_iso(timestamp):
    ...
    if m.group('tzempty') == None and m.group('tzh') == None:
        raise ValueError('Not a proper ISO 8601 timestamp: ' + 'missing timezone (Z or +hh[:mm])!')
    if m.group('frac'):
        frac = m.group('frac')
        power = len(frac)
...
class datetime(_datetime):
    ...
    def __new__(cls, year, month, day, hour=0, minute=0, second=0, microsecond=0, tzinfo=None, is_dst=False):
        ...
        return _datetime.__new__(cls, dt.year, dt.month, dt.day, dt.hour, dt.minute, dt.second, dt.microsecond, dt.tzinfo)
