...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cantools/scripts/builder.py
# Compiled at: 2019-09-15 17:01:19
...
def encodestrings(text):
    ...
    while start != 0:
        ...
        if end == -1:
            error('parse', 'unclosed quote: character %s' % (start - 1,), 'this quote: %s' % (text[start:start + config.parse_error_segment_length],), text)
...
def processhtml(html):
    ...
    if start == end:
        return (html, '')
    return (
     html[:firststart] + '{jsspot}' + html[end + config.js.endoffset:], js)
...
def bfiles(dirname, fnames):
    return [ fname for fname in fnames if os.path.isfile(os.path.join(dirname, fname)) and fname != '.svn' and not fname.endswith('~') and '_old.' not in fname ]
...
def processjs(path, jspaths=[], inits=set(), admin_ct_path=None):
    p = path.split('#')[0]
    if admin_ct_path:
        if path.startswith(config.js.path):
    ...
    for line in [ bit for bit in block.split('\n') if not bit.startswith('//') ]:
        for flag in ['CT.require(', 'CT.scriptImport(']:
            if flag in line and line[(line.index(flag) - 1)] != "'":
                ...
                if rline[0] != '\\' and rline[1] not in '.,':
                    block = require('%s%s' % (flag, rline), jspaths, block, inits, admin_ct_path)
