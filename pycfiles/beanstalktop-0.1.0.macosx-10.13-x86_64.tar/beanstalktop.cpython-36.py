...
# [GCC 8.4.0]
# Embedded file name: /usr/local/Cellar/asdf/0.4.3/installs/python/3.6.4/lib/python3.6/site-packages/beanstalktop.py
# Compiled at: 2018-09-26 05:34:54
# Size of source mod 2**32: 9125 bytes
import curses, optparse, select, sys, errno, beanstalkc
...
class BeanstalkTopUI(object):
    ...
    def __init__(self, win, options):
        ...
        self.default_overview = dict((i, '-') for i in ('pid', 'total-jobs', 'current-connections',
                                                        'current-producers', 'current-workers',
                                                        'current-tubes', 'current-jobs-ready',
                                                        'current-jobs-urgent', 'current-jobs-buried',
                                                        'current-jobs-reserved'))
        self.default_row = dict((i, '-') for i in ('current-jobs-buried', 'current-jobs-delayed',
 ...
 L.  53         0  LOAD_GLOBAL              beanstalkc
                2  LOAD_ATTR                Connection
                4  LOAD_FAST                'self'
                6  LOAD_ATTR                options
                8  LOAD_ATTR                host
               10  LOAD_GLOBAL              int
               12  LOAD_FAST                'self'
               14  LOAD_ATTR                options
               16  LOAD_ATTR                port
               18  CALL_FUNCTION_1       1  '1 positional argument'
               20  LOAD_CONST               ('host', 'port')
               22  CALL_FUNCTION_KW_2     2  '2 total positional and keyword args'
               24  RETURN_VALUE     

 L.  57        26  DUP_TOP          
               28  LOAD_GLOBAL              beanstalkc
               30  LOAD_ATTR                SocketError
 ...
 L.  59        52  LOAD_GLOBAL              SystemExit
               54  LOAD_STR                 'Host {0} not contactable on port {1}'
               56  LOAD_ATTR                format

 L.  60        58  LOAD_FAST                'self'
               60  LOAD_ATTR                options
               62  LOAD_ATTR                host

 L.  61        64  LOAD_FAST                'self'
 ...
 L.  63        82  LOAD_FAST                'self'
               84  LOAD_ATTR                _connection
               86  RETURN_VALUE     
               -1  RETURN_LAST      
...
Parse error at or near `DUP_TOP' instruction at offset 26
    ...
    def _format_uptime(self, seconds):
        hours, remainder = divmodseconds3600
        minutes, seconds = divmodremainder60
        return '{0}h {1}m {2}s'.format(hours, minutes, seconds)

    def run(self):
        poll = select.poll()
        poll.registersys.stdin.fileno()select.POLLIN | select.POLLPRI
        while 1:
            self.resize()
    ...
    def handle_key(self, key):
        key_bindings = {ord('q'): lambda : sys.exit(0), 
         ord('Q'): lambda : sys.exit(0)}
        action = key_bindings.getkeylambda : None
        action()
    ...
    def refresh_display(self):
        self.win.erase()
        titles = ('TUBE', 'READY', 'URGENT', 'RESRVD', 'DELAYD', 'BURIED')
        ...
        summary_items = [(item.format)(**overview) for item in ('PID: {pid}', 'Uptime: {uptime}',
        ...
        for i in range(len(titles)):
            ...
            titlelen += len(title)
            self.win.addstrtitleattr

        self.win.addstr' ' * (self.width - titlelen)curses.A_REVERSE
        columns = ('name', 'current-jobs-ready', 'current-jobs-urgent', 'current-jobs-reserved',
    ...
    def get_data(self):
        ...
        try:
            return (
             self.connection.stats(), [self.connection.stats_tube(tube) for tube in self.connection.tubes()])
        except (TypeError, beanstalkc.SocketError, beanstalkc.CommandFailed):
            return (
             self.default_overview, [self.default_row])


def run_beanstalktop_window(win, options):
    ui = BeanstalkTopUIwinoptions
    ui.run()


def run_beanstalktop(options):
    return curses.wrapperrun_beanstalktop_windowoptions
...
def main():
    ...
    parser.add_option('-d', '--delay', dest='delay_seconds',
      default=1,
      metavar='NUM',
