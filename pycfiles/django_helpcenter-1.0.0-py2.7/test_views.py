...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/helpcenter/tests/test_views.py
# Compiled at: 2016-09-30 00:56:02
from django.core.urlresolvers import reverse
from django.test import TestCase, override_settings
from helpcenter import models
from helpcenter.testing_utils import AuthTestMixin, create_article, create_category, instance_to_queryset_string
...
class TestArticleCreateView(AuthTestMixin, TestCase):
    ...
    @override_settings(HELPCENTER_ARTICLE_CREATE_FORM='helpcenter.tests.dummy_classes.BlankForm')
...
class TestArticleDetailView(TestCase):
    ...
    def test_invalid_pk(self):
        ...
        should 404.
        """
        url = reverse('helpcenter:article-detail', kwargs={'article_pk': 1, 
           'article_slug': 'foo'})
        response = self.client.get(url)
...
class TestArticleUpdateView(AuthTestMixin, TestCase):
    ...
    @override_settings(HELPCENTER_ARTICLE_UPDATE_FORM='helpcenter.tests.dummy_classes.BlankForm')
    def test_override_form(self):
        """Test overriding the form for the view.
...
class TestCategoryCreateView(AuthTestMixin, TestCase):
    ...
    @override_settings(HELPCENTER_CATEGORY_CREATE_FORM='helpcenter.tests.dummy_classes.BlankForm')
    def test_override_form(self):
        """Test overriding the form for the view.
...
class TestCategoryDetailView(AuthTestMixin, TestCase):
    ...
    def test_context_data(self):
        ...
        response = self.client.get(url)
        self.assertEqual(200, response.status_code)
        self.assertQuerysetEqual(response.context['categories'], map(instance_to_queryset_string, category.category_set.all()))
        self.assertQuerysetEqual(response.context['articles'], map(instance_to_queryset_string, category.article_list))
    ...
    def test_draft_as_editor(self):
        ...
        self.login()
        category = create_category()
        create_article(category=category, title='Normal Article')
        create_article(category=category, title='Draft Article', draft=True)
        url = category.get_absolute_url()
        response = self.client.get(url)
        self.assertEqual(200, response.status_code)
        self.assertQuerysetEqual(response.context['articles'], map(instance_to_queryset_string, category.article_list), ordered=False)
    ...
    def test_draft_as_normal_user(self):
        ...
        response = self.client.get(url)
        self.assertEqual(200, response.status_code)
        self.assertQuerysetEqual(response.context['articles'], map(instance_to_queryset_string, category.article_list.exclude(draft=True)))
    ...
    def test_invalid_pk(self):
        ...
        """
        url = reverse('helpcenter:category-detail', kwargs={'category_pk': 1, 
           'category_slug': 'foo'})
        response = self.client.get(url)
        self.assertEqual(404, response.status_code)
...
class TestCategoryUpdateView(AuthTestMixin, TestCase):
    ...
    def test_initial_get(self):
        ...
        self.add_permission('change_category')
        self.login()
        category = create_category(title='Cool Category', parent=create_category())
        url = category.get_update_url()
        response = self.client.get(url)
        form = response.context['form']
        self.assertEqual(200, response.status_code)
    ...
    @override_settings(HELPCENTER_CATEGORY_UPDATE_FORM='helpcenter.tests.dummy_classes.BlankForm')
    def test_override_form(self):
        """Test overriding the form for the view.
