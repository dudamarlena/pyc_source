...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/helpcenter/models.py
...
class Article(models.Model):
    ...
    category = models.ForeignKey('Category', on_delete=models.SET_NULL, null=True, blank=True, help_text='The parent category for the article.', verbose_name='Article Category')
    draft = models.BooleanField(default=False, help_text='Marking an article as a draft will hide it from users.', verbose_name='article is a draft')
    slug = models.SlugField(verbose_name='Article URL Slug')
    time_edited = models.DateTimeField(auto_now=True, verbose_name='time last modifed')
    time_published = models.DateTimeField(default=timezone.now, help_text=('Please use the following format: ',
                                                                           '<em>YYYY-MM-DD HH:MM[:ss[.uuuuuu]][TZ]</em>'), verbose_name='Time Published')
    title = models.CharField(max_length=200, db_index=True, help_text='An article title is restricted to 200 characters.', verbose_name='Article Title')
...
class Category(models.Model):
    ...
    title = models.CharField(max_length=200, db_index=True, help_text="A category's title is restricted to 200 characters.", verbose_name='Category Title')
    parent = models.ForeignKey('Category', on_delete=models.SET_NULL, null=True, blank=True, help_text='Categories can be nested as deep as you would like.', verbose_name='Parent Category')
    slug = models.SlugField(verbose_name='Category URL Slug')
