...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/helpcenter/api/tests/test_serializers.py
...
class TestArticleSerializer(TestCase):
    ...
    def test_serialize(self):
        ...
        serializer = serializers.ArticleSerializer(article, context={'request': self.request})
        expected_dict = {'body': article.body, 
           'category': article.category, 
           'category_id': None, 
           'url': full_url('help:api:article-detail', kwargs={'pk': article.pk}), 
           'id': article.id, 
    ...
    def test_serialize_with_category(self):
        ...
        serializer = serializers.ArticleSerializer(article, context={'request': self.request})
        expected_dict = {'body': article.body, 
           'category': full_url('helpcenter:helpcenter-api:category-detail', kwargs={'pk': category.pk}), 
    ...
    def test_update(self):
        ...
        """
        article = create_article()
        data = {'title': 'New Awesome Title'}
        serializer = serializers.ArticleSerializer(article, data=data, partial=True)
        self.assertTrue(serializer.is_valid())
...
class TestCategorySerializer(TestCase):
    ...
    def test_serialize(self):
        ...
        serializer = serializers.CategorySerializer(category, context={'request': self.request})
        expected_dict = {'id': category.id, 
           'parent': full_url('helpcenter:helpcenter-api:category-detail', kwargs={'pk': parent.pk}), 
           'parent_id': parent.id, 
           'title': category.title, 
           'url': full_url('help:api:category-detail', kwargs={'pk': category.pk})}
        expected = json.dumps(expected_dict)
        self.assertJSONEqual(expected, serializer.data, msg=('\nExpected: {}\n  Actual: {}').format(expected, serializer.data))
