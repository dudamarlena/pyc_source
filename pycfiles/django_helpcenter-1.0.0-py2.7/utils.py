...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/helpcenter/utils.py
...
def string_to_class(class_string):
    ...
    if '.' not in class_string:
        logger.error(("'{}' is not a fully qualifed class name").format(class_string))
        raise ValueError("'class_string' must be a fully qualifed name.")
    module_name, class_name = class_string.rsplit('.', 1)
    try:
        module = importlib.import_module(module_name)
    except ImportError:
        logger.error(("Could not import '{}'").format(module_name), exc_info=True)
        raise

    try:
        class_obj = getattr(module, class_name)
    except AttributeError:
        error_msg = ("Could not import '{}' from '{}'.").format(class_name, module_name)
        logger.error(error_msg, exc_info=True)
        raise ImportError(error_msg)

    logger.debug(("Succesfully imported '{}' from '{}'.").format(class_name, module_name))
    return class_obj