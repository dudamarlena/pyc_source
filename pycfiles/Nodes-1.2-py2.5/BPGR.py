...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/BPGR.py
# Compiled at: 2009-09-27 01:19:53
...
evidences = [
 ...
 'engine',
 'chassis']
objects = {'bird': (1, 1, 1, 1, -1, -1), 
   'plane': (1, 1, -1, -1, 1, 1), 
   'glider': (1, -1, -1, -1, -1, -1), 
   'rocket': (-1, 0, -1, -1, 1, -1)}
pmatrix = {('bird', 'plane'): ('wings', 'tail', 'tail'), 
...
def create_glob():
    decider = Decider([ (evid, InputMemory) for evid in evidences ] + [ (obj, DecisionNode) for obj in objects.keys() ] + [
     (
      '__decider__', SelectBestNode),
     (
      '__result__', OutputMemory)], [ (evid, obj, -1, -1, 1, objects[obj][i]) for (i, evid) in enumerate(evidences) for obj in objects.keys() ] + [ (obj, '__decider__', 0, -1) for obj in objects.keys() ] + [
     ('__decider__', '__result__', -1, -1)], evidences, ['__result__'], heart='__decider__', name='Expert')
    return decider


def create_glob2():
    decider = Decider([ (evid, InputMemory) for evid in evidences ] + [ (obj, DecisionNode) for obj in objects.keys() ] + [ (x + '_or_' + y, DecisionNode) for (x, y) in pmatrix.keys() ] + [
     (
      '__decider__', SelectBestNode),
     (
      '__result__', OutputMemory)], [ (evid, x + '_or_' + y, -1, -1, 1, objects[obj][evidences.index(evid)]) for (x, y) in pmatrix.keys() for evid in pmatrix[(x, y)] ] + [ (x + '_or_' + y, obj, -1, -1) for (x, y) in pmatrix.keys() for obj in (x, y) ] + [ (obj, '__decider__', -1, -1) for obj in objects.keys() ] + [
     ('__decider__', '__result__', -1, -1)], evidences, ['__result__'], heart='__decider__', name='Expert')
    return decider
    ...
    if x >= 0:
        ...
        if len(obj) > 0:
            ...
            for k in objects.keys():
                if k == obj:
                    decider.nodes[k].teach_feed(+0.5)
                else:
                    decider.nodes[k].teach_feed(-0.5)
