...
# Embedded file name: build/bdist.macosx-10.13-intel/egg/tests/0000_setup_corevpn.py
...
def test_enable_extension():
    if subprocess.call(['grep', 'corevpn.views.api', '/etc/corecluster/config.py']) != 0:
        subprocess.call(['sed', '-i',
         's/LOAD_API = \\[/LOAD_API = \\["corevpn.views.api", /g',
         '/etc/corecluster/config.py'])
    if subprocess.call(['grep', 'corevpn.models.corevpn', '/etc/corecluster/config.py']) != 0:
        subprocess.call(['sed', '-i.bak',
         's/LOAD_MODELS = \\[/LOAD_MODELS = \\["corevpn.models.corevpn", /g',
         '/etc/corecluster/config.py'])
