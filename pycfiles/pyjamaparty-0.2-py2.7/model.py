...
# Embedded file name: /usr/lib/python2.5/site-packages/pyjama/core/model.py
# Compiled at: 2012-01-13 14:55:19
__doc__ = '\nCreated on Jan 6, 2012\n\n@author: maemo\n'
import os, shutil, logging, string, gtk
...
class Project:
    ...
    def __init__(self, type='default', name='pyjama', author=os.environ['LOGNAME'], summary='pyjama generated project', description='This a project generated by Pyjama with default layout', inception=None, logo_uri=None, version='0.1.0'):
        self.type = type
        self.name = name
        self.author = author
        ...
        self.file_encoding = '-*- encoding: UTF-8 -*-'
        self.copyright = None
        self.header = None
    ...
    def _generate_copyright(self):
        copyleft = Copyright()
        SourceTransformer().process(copyleft, self, None)
        self.copyright = copyleft.get_content()
        return

    def _generate_header(self):
        header = Header()
        SourceTransformer().process(header, self, None)
...
class Copyright(TextMemoryTemplate, IVisitableTemplate):
    ...
    def __init__(self):
        MemoryTemplate.__init__(self, 'Copyright', value='Copyright (C) $INCEPTION  $AUTHOR')
...
class Header(TextMemoryTemplate, IVisitableTemplate):
    ...
    def __init__(self):
        MemoryTemplate.__init__(self, 'Header', value='$FILE_ENCODING\n\n$DESCRIPTION\n\n$COPYRIGHT\n\n$LICENCE\n')
...
class Licence(TextMemoryTemplate, IVisitableTemplate):
    ...
    def __init__(self):
        MemoryTemplate.__init__(self, 'GNU Licence', value=GPL_LICENCE)
...
class FilenameTransformer(Transformer):
    ...
    def visit_imageFileTemplate(self, template):
        if template.get_relative_input_file().find('media/pyjama.png') > -1 or template.get_relative_input_file().find('hildon/icons/48x48/pyjama.png') > -1 or template.get_relative_input_file().find('hildon/icons/64x64/pyjama.png') > -1 or template.get_relative_input_file().find('src/pyjama-logo.png') > -1:
            template.set_relative_input_file(template.get_relative_input_file().replace('pyjama', self.project.name))
            return [
             template]
        return self._return_no_template()
...
TRANSFORMERS_PIPELINE = [
 SourceTransformer(), FilenameTransformer(), LogoTransformer(), CopyTransformer()]
