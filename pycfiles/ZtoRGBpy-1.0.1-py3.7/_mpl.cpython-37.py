...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\ZtoRGBpy\_mpl.py
    ...
    **kwargs : `matplotlib.colorbar.make_axes` parameters
    ...
    if mappable is None:
        ...
        if scale is None or profile is None:
            raise RuntimeError('No scale and profile was found to use for colorbar creation. Either define them as parameters, oruse a mappable created by `ZtoRBGpy.imshow`.')
        else:
            try:
    ...
    z_cb = cax.imshow(remap((magnitude * np.exp(complex(0.0, 1.0) * phase)), profile=profile), aspect='auto',
      extent=extent)
...
def colorwheel(ax=None, scale=None, profile=None, rotation=0, grid=False):
    r"""Renders a colorwheel, showing the colorspace, with optional grid
    ...
    zmesh = xmesh + complex(0.0, 1.0) * ymesh
    zmesh *= np.exp(complex(0.0, 1.0) * rotation)
    rgba = np.ones(zmesh.shape + (4, ))
    rgba[(Ellipsis, 3)] = abs(zmesh) <= 1
    rgba[..., 0:3], scale, profile = remap(zmesh, scale=scale, profile=profile, return_metadata=True)
    current_ax = plt.gca()
    if ax is None:
        ax = current_ax
    else:
...
def imshow(z, ax=None, scale=None, profile=None, **kwargs):
    r"""Displays a complex image on the ax or the current axes.
    ...
    **kwargs : `matplotlib.axes.Axes.imshow` parameters
    ...
    if ax is None:
        ax = current_ax
    rgb, scale, profile = remap(z, scale=scale, profile=profile, return_metadata=True)
    z_im = (ax.imshow)(rgb, **kwargs)
    z_im._ZtoRGBpy__meta = {'scale':scale, 
     'profile':profile}
    plt.sca(ax)
    plt.sci(z_im)
    return z_im