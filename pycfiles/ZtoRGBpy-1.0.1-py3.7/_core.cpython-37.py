...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\ZtoRGBpy\_core.py
# Compiled at: 2019-05-19 01:57:06
# Size of source mod 2**32: 22787 bytes
"""
ZtoRGB core module
        ...
        for subclass in type(self).__mro__:
            ...
            if subclass.__init__ != subclass.__mro__[1].__init__:
                ...
                if num_values + num_defaults < num_args:
                    raise RuntimeError('{0:s} dose not define __args for non-default arguments'.format(subclass.__name__))
                ...
                for kwname in spec.kwonlyargs:
                    if kwname not in kwarg_names:
                        if kwname in subclass_kwargs:
                            ...
                            if spec.kwonlydefaults is not None and kwname in spec.kwonlydefaults:
                                kwargs[kwname] = spec.kwonlydefaults[kwname]
                            else:
                                raise RuntimeError('{0:s} dose not define __kwargs for non-default keyword arguments'.format(subclass.__name__))
...
class LinearScale(Scale):
    ...
    def ticks(self):
        r"""Returns a list of tick marks suitable for a colorbar
        ...
        dfactor = 1.0
        while True:
            maxsteps = np.ceil(self.mag / (LinearScale._divisors[divisor] * dfactor))
            if maxsteps < 4:
                if divisor == 0:
                    dfactor /= 10
...
class LogScale(Scale):
    ...
    def __call__(self, value):
        ...
        avalue = abs(value)
        return self.lightness_buf + value * (np.log10(avalue) - self.logmin) / avalue * self.factor
...
class RGBColorProfile(object):
    ...
    def get_ratios(self):
        r"""Returns the relative ratios for red and blue
        ...
        """
        red_ratio = self.weights[0] / (self.weights[0] + self.weights[1] + self.weights[2])
        blue_ratio = self.weights[2] / (self.weights[0] + self.weights[1] + self.weights[2])
        return (red_ratio, blue_ratio)

    def remove_gamma(self, RGB):
        r"""Removes gamma correction from color
    ...
    def apply_gamma(self, rgb):
        r"""Applies gamma correction to color
    ...
    def get_limits(self):
        r"""Returns :math:`U_{max}` and :math:`V_{max}`
        ...
        blue_ratio_cuberoot = blue_ratio ** 0.3333333333333333
        red_ratio_cuberoot = red_ratio ** 0.3333333333333333
    ...
    def get_transform(self):
        r"""Returns the UV to RGB transformation matrix :math:`\mathbf{K_Q}`.
        ...
        trans_matrix[(1, 0)] = blue_ratio * (1 - blue_ratio) / (umax * (blue_ratio + red_ratio - 1))
        trans_matrix[(1, 1)] = red_ratio * (1 - red_ratio) / (vmax * (blue_ratio + red_ratio - 1))
        trans_matrix[(2, 0)] = 1 - blue_ratio / umax
        return trans_matrix

    def get_chroma_limit(self):
        r"""Return the limiting chroma
        ...
        chroma_limit.append(-((trans_matrix[(1, 0)] + np.sqrt(kg2)) * kg2 / (kg2 + trans_matrix[(1,
                                                                                                 0)] * np.sqrt(kg2))))
        chroma_limit.append(-((trans_matrix[(1, 0)] - np.sqrt(kg2)) * kg2 / (kg2 - trans_matrix[(1,
                                                                                                 0)] * np.sqrt(kg2))))
    ...
    def __repr__(self):
        return '{0:s}.{1:s}({2!r:s}, {3:g})'.format(type(self).__module__, type(self).__name__, self.weights, self.gamma)
...
def remap(data, scale=None, profile=None, return_int=False, return_metadata=False, **kwargs):
    ...
    **kwargs :
    ...
    if not (profile is None or isinstance)(profile, str) or profile.lower() == 'srgb':
        profile = sRGB
    else:
        if isinstance(profile, str) and profile.lower() == 'srgb_high':
            profile = sRGB_HIGH
        else:
            if isinstance(profile, str):
                if profile.lower() == 'srgb_low':
                    profile = sRGB_LOW
    if not isinstance(profile, RGBColorProfile):
        raise ValueError("profile can't be converted to an instance of RGBColorProfile.")
    if not (scale is None or isinstance)(scale, str) or scale.lower() == 'linear':
        scale = LinearScale
    else:
        if isinstance(scale, str):
            if scale.lower() == 'log':
                scale = LogScale
            elif isinstance(scale, type):
                if issubclass(scale, Scale):
            ...
            lightness_cutoff = 0.7937005259840997
