...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\evmcodegen\codegen.py
# Compiled at: 2018-10-28 13:43:32
# Size of source mod 2**32: 15315 bytes
"""
...
class Rnd:
    ...
    def uni_integer(min=None, max=None):
        min = min or 0
        max = max or 18446744073709551615
        assert min <= max
        return min + random.randint(min, max) % (max - min)
    ...
    @staticmethod
    def percent():
        return Rnd.uni_integer(0, 100)

    @staticmethod
    def small_uni_integer():
        return Rnd.uni_integer(1, 10485760)
    ...
    @staticmethod
    def memory_length():
        return Rnd.uni_integer(0, 10485760)

    @staticmethod
    def opcode():
        return Rnd.uni_integer(0, 255)


VALUEMAP = {evmdasm.argtypes.Address: lambda : Rnd.byte_sequence(20), 
...
class CodeGen(object):
                        ...
                        for arg in reversed(instr.args):
                            ...
                            if f:
                                ...
                                if arg.__class__ == evmdasm.argtypes.Address:
                                    self._addresses_seen.add(binascii.hexlify(v).decode('utf-8'))
    ...
    def generate(self, generator, length=None, min_gas=0):
        ...
        while 1:
            if generator.type == generator.TYPE_OPCODE_ONLY:
                ...
                if generator.type == generator.TYPE_OPCODE_WITH_OPERAND:
                    instructions += evmdasm.EvmBytecode(generator.generate(length * 3)).disassemble()[:length]
                else:
    ...
    def get_stats(self):
        self.reassemble()
        stats = {'instruction_total': len(self.instructions), 
         'instruction_histogram': Counter([instr.opcode for instr in self.instructions]), 
         'sequental_gas_required': self.instructions.get_gas_required(), 
         'time_total': sum(self._timeit_duration.values()), 
         'time_details': self._timeit_duration, 
         'stack_balance': self.instructions.get_stack_balance(), 
         'generated_addresses': self._addresses_seen}
        return stats
    ...
    def print_stats(stats):
        out = []
        out.append('========stats==========')
        out.append('instructions: %s' % stats['instruction_total'])
        out.append('gas (sequentially, all instructions): %s' % stats['sequental_gas_required'])
        out.append('time taken (total): %0.4fs' % stats['time_total'])
        out.append('time detail: %r' % stats['time_details'])
        out.append('stack balance: %d' % stats['stack_balance'])
        out.append('number of unique autogenerated addresses (for args): %d' % len(stats['generated_addresses']))
        stats_instructions = stats['instruction_histogram']
...
def main():
    ...
    parser.add_option('-l', '--length', default=-1, type=int, help='instructions per generated code')
    parser.add_option('-s', '--stats', default=False, action='store_true', help='show statistics after generating code')
    parser.add_option('-S', '--stats-overall', default=False, action='store_true', help='show statistics for all generation runs')
    ...
    while options.count > 0:
        evmcode = CodeGen().generate(generator=rnd_codegen, length=options.length, min_gas=options.min_gas)
        if not options.no_fixups:
            evmcode.fix_stack_arguments(valuemap=VALUEMAP).fix_jumps()
        if options.balance:
        ...
        if options.stats:
            print(CodeGen.print_stats(evmcode.get_stats()))
        if options.stats_overall:
            all_stats.append(evmcode.get_stats())
        options.count -= 1

    if options.stats_overall:
        totals = {'instruction_total': 0, 
         'instruction_histogram': Counter(), 
         'sequental_gas_required': 0, 
         'time_total': 0, 
         'time_details': {}, 
         'stack_balance': 0, 
         'generated_addresses': set([])}
        num_stats = float(len(all_stats))
        for stats in all_stats:
            totals['instruction_total'] += stats['instruction_total']
            totals['sequental_gas_required'] += stats['sequental_gas_required']
            totals['time_total'] += stats['time_total']
            totals['stack_balance'] += stats['stack_balance']
            totals['generated_addresses'].update(stats['generated_addresses'])
            totals['instruction_histogram'] += stats['instruction_histogram']

        print('====== OVERALL STATS (Averages) ======')
        print('total number of instructions: %s ' % totals['instruction_total'])
        print('number of runs: %d' % len(all_stats))
        totals['instruction_total'] /= num_stats
        totals['sequental_gas_required'] /= num_stats
        totals['time_total'] /= num_stats
        totals['stack_balance'] /= num_stats
        print(CodeGen.print_stats(totals))
