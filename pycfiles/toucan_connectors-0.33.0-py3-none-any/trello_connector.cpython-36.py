...
class TrelloConnector(ToucanConnector):
    ...
    def _retrieve_data(self, data_source: TrelloDataSource) -> pd.DataFrame:
        ...
        if 'lists' in data_source.fields_list:
            ...
            lists_ids_mapping = {x['id']:x['name'] for x in self.get_board(f"{data_source.board_id}/lists", fields='name')}
        if 'labels' in data_source.fields_list:
            fields_for_request += ['labels']
            labels_id_mapping = {x['id']:x['name'] for x in self.get_board(f"{data_source.board_id}/labels", fields='name')}
        if 'members' in data_source.fields_list:
            fields_for_request += ['idMembers']
            members_id_mapping = {x['id']:x['fullName'] for x in self.get_board(f"{data_source.board_id}/members", fields='fullName')}
        if data_source.custom_fields:
            custom_fields_id_mapping = {x['id']:x for x in self.get_board(f"{data_source.board_id}/customFields", fields='name')}
