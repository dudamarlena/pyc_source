...
def handle(context):
    ...
    log_entry = {v:k for k, v in context.items() if k in frozenset({'cs_user_info', 'cs_path_info', 'cs_timestamp'}) if k in frozenset({'cs_user_info', 'cs_path_info', 'cs_timestamp'})}
    m = None
    status = HTTPStatus.OK
    if 'view' not in perms:
        if 'view_all' not in perms or :
