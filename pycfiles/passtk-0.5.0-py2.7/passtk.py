...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/passtk/passtk.py
# Compiled at: 2018-10-01 09:57:13
"""
TODO:
...
import os, sys, datetime, argparse, string, random, struct, base64, getpass
...
def main():
    parser = argparse.ArgumentParser(description='A tool to generate random password.')
    parser.add_argument('-l', '--level', dest='level', type=int, default=DEFAULT_LEVEL, help='The level(1-3, default is %s) of password, higher is complex' % DEFAULT_LEVEL)
    parser.add_argument('-n', '--length', dest='length', type=int, default=DEFAULT_LENGTH, help='The length of password(at least 4, default is %s)' % DEFAULT_LENGTH)
    parser.add_argument('-m', '--comment', dest='comment', help='Add comment for password')
    parser.add_argument('-u', '--unsave', dest='unsave', action='store_true', help='Disable storing password into ~/.passtk')
    parser.add_argument('-p', dest='preview', action='store_true', help='Show password entries in ~/.passtk')
    parser.add_argument('-d', dest='delete', type=int, help='Delete password entries by id in ~/.passtk')
    parser.add_argument('-a', dest='add', help='Add password manually into ~/.passtk')
