...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\ramldocgen\generator.py
# Compiled at: 2019-04-11 10:35:22
# Size of source mod 2**32: 31581 bytes
...
def urlify(text):
    return re.sub('http(?:s?)://\\S+', '<a href="\\g<0>" target="_blank">\\g<0></a>', text)
...
def classify(name):
    return re.sub('/+|\\{|\\}', '_', re.sub('[^a-zA-Z0-9/{}]', '', name.lstrip('/')))
...
class Generator(object):
    ...
    def generate(self):
        ...
        for group in self.raml.resources:
            ...
            for ep in collect(groupc, group):
                ...
                for method in sorted(ep.methods):
                    ...
                    if methodc.securedBy is not None:
                        if len(methodc.securedBy) > 0:
                    ...
                    if methodc.securedBy is not None:
                        ...
                        if methodc.headers is not None and len(methodc.headers) > 0 or methodc.queryParameters is not None and len(methodc.queryParameters) > 0 or methodc.body is not None and len(methodc.body) > 0:
                            ...
                            if methodc.headers is not None:
                                if len(methodc.headers) > 0:
                            ...
                            if methodc.queryParameters is not None:
                                if len(methodc.queryParameters) > 0:
                            ...
                            if methodc.body is not None:
                                if len(methodc.body) > 0:
                        ...
                        if methodc.responses is not None:
                            if len(methodc.responses) > 0:
                                if first_tab:
        ...
        if self.raml.securitySchemes is not None:
            if len(self.raml.securitySchemes) > 0:
                ...
                for scheme in self.raml.securitySchemes:
                    ...
                    if schemec.describedBy is not None:
                        ...
                        if desc.headers is not None and len(desc.headers) > 0 or desc.queryParameters is not None and len(desc.queryParameters) > 0:
                            ...
                            if desc.queryParameters is not None:
                                if len(desc.queryParameters) > 0:
                                    contents.append(HTMLNode('h3')).append('Query Parameters')
                                    param_list = contents.append(HTMLNode('ul'))
                        ...
                        if desc.responses is not None:
                            if len(desc.responses) > 0:
                                if first_tab:
                                    response_tab = tabs.append(HTMLNode('li', {'class': 'active'}))
