...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\bzETL\util\elasticsearch.py
# Compiled at: 2013-12-18 14:05:11
from datetime import datetime
import re, sha, time, requests
...
class ElasticSearch(object):
    ...
    def __init__(self, settings):
        if not settings.host:
            ...
            self.metadata = None
            settings.port = settings.port or 9200
    ...
    def create_index(settings, schema):
        ...
        ElasticSearch.post(settings.host + ':' + unicode(settings.port) + '/' + settings.index, data=CNV.object2JSON(schema), headers={'Content-Type': 'application/json'})
        time.sleep(2)
    ...
    def delete_index(settings, index=None):
        ...
        ElasticSearch.delete(settings.host + ':' + unicode(settings.port) + '/' + index)
    ...
    def add_alias(self, alias):
        self.metadata = None
        requests.post(self.settings.host + ':' + unicode(self.settings.port) + '/_aliases', CNV.object2JSON({'actions': [{'add': {'index': self.settings.index, 'alias': alias}}]}))
        return
    ...
    def get_proto(self, alias):
        ...
        """
        output = Q.sort([ a.index for a in self.get_aliases() if re.match(re.escape(alias) + '\\d{8}_\\d{6}', a.index) and not a.alias
                        ])
        return output
    ...
    def get_index(self, alias):
        ...
        """
        output = Q.sort([ a.index for a in self.get_aliases() if a.alias == alias
                        ])
        if len(output) > 1:
    ...
    def delete_record(self, query):
        if isinstance(query, dict):
            ElasticSearch.delete(self.path + '/_query', data=CNV.object2JSON(query))
        else:
            ElasticSearch.delete(self.path + '/' + query)
    ...
    def extend(self, records):
        ...
        if not lines:
            ...
            response = ElasticSearch.post(self.path + '/_bulk', data=('\n').join(lines).encode('utf8') + '\n', headers={'Content-Type': 'text'})
            items = response['items']
            for i, item in enumerate(items):
                if not item.index.ok:
                    Log.error('{{error}} while loading line:\n{{line}}', {'error': item.index.error, 
                       'line': lines[(i * 2 + 1)]})
    ...
    def set_refresh_interval(self, seconds):
        ...
        response = ElasticSearch.put(self.settings.host + ':' + unicode(self.settings.port) + '/' + self.settings.index + '/_settings', data='{"index.refresh_interval":"' + interval + '"}')
        if response.content != '{"ok":true}':
            Log.error('Can not set refresh interval ({{error}})', {'error': response.content})
