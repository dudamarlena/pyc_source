...
# [GCC 8.4.0]
# Embedded file name: /home/fabian/devs/novix/python/pyplan-core/pyplan_core/classes/XHelpers.py
# Compiled at: 2020-04-29 16:08:41
# Size of source mod 2**32: 46714 bytes
import inspect, numbers
from sys import exc_info, getsizeof, platform
import numpy as np, numpydoc, pandas as pd, xarray as xr
...
class XHelpers(object):
                ...
                if isinstance(coords[0], XIndex):
                    ...
                    if values is None:
                        if dtype is not None:
                            ...
                            if self.kindToString(np.dtype(dtype).kind) == 'string':
                        ...
                        if isinstance(values, list) or isinstance(values, np.ndarray):
                            ...
                            return xr.DataArray(values, _realCoords)
                        if isinstance(values, numbers.Number) and values == self.random:
    ...
    def excelConnection(self, filepath, useOpenpyxl=False, dataOnly=True, readOnly=True):
        ...
        if not my_file.is_file():
            ...
            my_file = Path(fullFilename)
            if my_file.is_file() and not useOpenpyxl:
                if self.isLinux():
    ...
    def indexFromExcel(self, excel, sheetName=None, namedRange=None, cellRange=None, columnName=None, removeEmpty=True):
        ...
        raise ValueError('excel can be pp.excel object or a str path to the filename')
    ...
    def cubeFromExcel(self, excel, sheetName=None, namedRange=None, cellRange=None, indexes=None, valueColumns=None, indexColumnHeaders=None, replaceByIndex=None):
        ...
        """
        dataframe = self.pandasFromExcel(excel, sheetName, namedRange, cellRange)
        import pandas as pd
        valueIndex = None
            ...
            if isinstance(param2, XIndex):
                ...
                for row in param1.values:
                    for col in param2.values:
                        _res.loc[{param1.name: slice(row, row), param2.name: slice(col, col)}] = _fn(col, row)
    ...
    def splitText(self, param1, separator, part=None):
        ...
        for _part in range(_max_q_separators + 1):
            _dataarray = self.apply(param1, lambda x: x.split(separator)[_part])
    ...
    def copyAsValues(self, source, targetId):
        ...
        if isinstance(source, str):
            ...
            if not isinstance(source, xr.DataArray):
                if not isinstance(source, XIndex):
                    if not isinstance(source, float):
                        if not isinstance(source, int):
                            raise ValueError("The 'source' parameter must be a DataArray, Index, float or int")
            if not isinstance(targetId, str):
                raise ValueError("The 'targetId' parameter must be a string (identifier of node)")
            assert self.node.model.existNode(targetId), "Node '" + targetId + "' not found"
        newDef = ''
        if isinstance(source, float) or isinstance(source, int):
            ...
            if isinstance(source, xr.DataArray):
                ...
                data = np.array2string((source.values), separator=',', precision=20, formatter={'float_kind': lambda x:                                if np.isnan(x):
'np.nan' # Avoid dead code: repr(x)}).replace('\n', '')
                newDef = 'result = pp.dataArray(' + indexes + ',' + data + ')'
            else:
                if isinstance(source, XIndex):
                    np.set_printoptions(threshold=(np.prod(source.values.shape)))
                    data = np.array2string((source.values), separator=',', precision=20, formatter={'float_kind': lambda x:                                    if np.isnan(x):
'np.nan' # Avoid dead code: repr(x)}).replace('\n', '')
    ...
    def sel(self, dataArray, filterList, compareMode=1, defaultValue=None):
        ...
        for filterItem in filterList:
            ...
            if filterItem.ndim == 2:
                ...
                res = self.changeIndex(res, oldIndex, newIndex, compareMode, defaultValue)
        ...
        if coorSize != valueSize:
            if coorSize > valueSize:
                if values.dtype.kind in {'S', 'U'}:
                    ...
                    values = np.append(values.reshape(values.size), np.zeros(coorSize - values.size)).reshape(shape)
            else:
    ...
    def fillInf(self, dataArray, value=0):
        ...
        """
        return self.apply(dataArray, lambda x:         if np.isinf(x):
...
class XIndex(object):
    __doc__ = ' Class for quickly create a xarray coord \n    '
