...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nslocalizer/xcodeproj/pbProj/PBXItem.py
...
def resolvePathTypeFromSource(source):
    result = None
    lookup = {'<absolute>':'resolveAbsolutePath', 
...
class PBX_Base_Reference(PBXItem):
    ...
    def findParent(self, project):
        parent = None
        results = [pbx_object for pbx_object in project.pbx_objects if isinstance(pbx_object, PBX_Base_Reference) if PBX_Constants.kPBX_REFERENCE_children in list(pbx_object.keys())]
        for item in results:
