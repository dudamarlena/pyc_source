...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gitdata/__init__.py
# Compiled at: 2019-03-15 07:56:09
import yaml, logging, urlparse, os, shutil, exectools
from pushd import Dir
SCHEMES = [
 'ssh', 'ssh+git', 'http', 'https']
...
class GitData(object):
    ...
    def __init__(self, data_path=None, clone_dir='./', branch='master', sub_dir=None, exts=['yaml', 'yml', 'json'], logger=None):
        """
    ...
    def clone_data(self, data_path):
        ...
        if data_url.scheme in SCHEMES or data_url.scheme == '' and ':' in data_url.path:
            ...
            if os.path.isdir(data_destination):
                ...
                with Dir(data_destination):
                    ...
                    if branch != self.branch:
                        if not synced:
                            msg = ('Local branch is `{}`, but requested `{}` and you have uncommitted/pushed changes\nYou must either clear your local data or manually checkout the correct branch.').format(branch, self.branch)
            ...
            raise ValueError(('Invalid data_path: {} - invalid scheme: {}').format(self.data_path, data_url.scheme))
        if self.sub_dir:
