...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/gitversionbuilder/versioninfo.py
# Compiled at: 2015-09-21 10:21:45
...
class TagInterpretation(EqualityMixin):
    ...
    def __init__(self, version_components, version_tag, is_dev_version):
        ...
        self.is_stable = not is_dev_version and self.version_tag in ('', 'stable',
                                                                     'final')
...
class VersionInfo(EqualityMixin):
    ...
    def interpret_tag_name(self):
        matched = re.match('^v?([0-9]+(?:\\.[0-9]+)*)(?:-?((alpha|beta|rc|pre|m)[0-9]?|stable|final))?$', self.git_tag_name, re.IGNORECASE)
        if matched:
