...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/scripy/shell.py
# Compiled at: 2010-02-16 04:34:09
"""
...
"""
__all__ = [
 'ReturnCode', 'Run', 'expand']
import collections, glob, re, shlex, subprocess, sys, yamlog
from . import _path
...
class Run(object):
    ...
    background : boolean, optional
        ...
        Default is *False*.
    bin : class with variables as str, optional
        The class with added commands absolute path.
    ...
    Attributes
    ----------
    MAIN_PATH_BIN : class with variables as str
        The main class with variables that indicate the commands
        absolute path.
    PIPE : str
    ...
    the instance `bin`. Examples:
        ...
        Without command expansion::
            ...
            run("{sudo} {ls} /dev/null".format(
                sudo=_path.Bin.sudo, ls=_path.Bin.ls))

        VS::
            run("!sudo !ls /dev/null")

    """
    MAIN_PATH_BIN = _path.Bin
    PIPE = '|'
    SIGN_EXPANSION = '!'
    ...
    def __init__(self, bin=None, background=False, cmd_expansion=True, debug=False):
        ...
        if bin:
            ...
            class Bin(self.MAIN_PATH_BIN, bin):
                pass

        else:

            class Bin(self.MAIN_PATH_BIN):
                pass
    ...
    def __call__(self, command):
        ...
        if command.rstrip().endswith(self.PIPE):
            ...
            try:
                for (num, cmd) in enumerate(l_command):
                    ...
                    if self.cmd_expansion:
                        expand_cmd = []
                        if self._QUOTE1 in cmd:
                            cmd_ = cmd.replace(self._QUOTE1, self._TAG1)
                        elif self._QUOTE2 in cmd:
                        ...
                        for i in expand_cmd:
                            if not i:
                                return self.throw(ExpansionError, cmd, 'the expansion sign has to be together to the command')
                            try:
                                path_cmd = getattr(self.bin, i)
                            except AttributeError, err:
                                return self.throw(CommandError, i, 'command not found in the class variables', self.bin.__module__)
                            else:
                                cmd_ = re.sub(('{sign}{command}').format(sign=self.SIGN_EXPANSION, command=i), ('{path_cmd}').format(path_cmd=path_cmd), cmd_)
                    ...
                    if num_ is 1:
                        process[num_] = subprocess.Popen(cmd_split, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=env)
                    else:
                        process[num_] = subprocess.Popen(cmd_split, stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=process[(num_ - 1)].stdout, env=env)
            ...
            return self.Output(returncode=process[num_].returncode, stdout=stdout.rstrip())
    ...
    def sudo(self):
        ...
        """
        self.__call__('!sudo !ls /dev/null')
