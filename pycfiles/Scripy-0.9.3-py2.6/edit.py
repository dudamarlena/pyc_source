# uncompyle6 version 3.7.4
# Python bytecode 2.6 (62161)
# Decompiled from: Python 3.6.9 (default, Apr 18 2020, 01:56:04) 
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/scripy/edit.py
# Compiled at: 2010-02-16 11:57:09
"""
Edit - tools to edit text files.
"""
__all__ = [
 'Edit']
import fnmatch, os, pwd, tempfile, yamlog
from . import shell
from ._config import run
_log = yamlog.logger(__name__)

class Edit(object):
    """Edit an existing file.

    Use shell commands running through *sudo* so it's possible to edit
    files owned by whatever user.

    ...

    Parameters
    ----------
    filename : str
        The file name to edit.
    background : boolean, optional
        If *True*, the exceptions are not raised else that they are
        logged. Useful for daemons or other background processes.
        Default is *False*.
    do_backup : boolean, optional
        If *True*, do a backup of `filename`.
        Default is *True*.

    Attributes
    ----------
    background
    do_backup
    char : str, optional
        The character used for comments.
        Default is *#*.
    file_path : str
        The canonical path of the specified `filename`.
    dirname : str
        The directory component of `file_path`.
    basename : str
        The final component of `file_path`.
    backup_path : str, optional
        The path of the backup for the `filename`.
    file_user : str
        The user name of owner for `file_path`.
    file_group : str
        The group name of owner for `file_path`.
    throw : class `yamlog.Throw()`
        Instance to throw exceptions which are logged.
    user_name : str
        The user name that is running the script.

    Raises
    ------
    PathError
        If the path of `file_path` does not exists.
    FileError
        If the file given on `file_path` does not exists.

    """

    def __init__(self, filename, char='#', background=False, do_backup=True):
        self.throw = yamlog.Throw(_log, background)
        self.file_path = run(('!sudo !readlink -f {file}').format(file=filename)).stdout
        if not self.file_path:
            self.throw(PathError, os.path.dirname(filename), 'directory does not exist')
        self.dirname = os.path.dirname(self.file_path)
        self.basename = os.path.basename(self.file_path)
        self.backup_path = None
        self.do_backup = do_backup
        if not run(('!sudo !find {dir} -type f -name {file}').format(dir=self.dirname, file=self.basename)).stdout:
            self.throw(FileError, self.file_path, "doesn't exist or isn't a file")
        stat_cmd = ('!sudo !stat {file}').format(file=self.file_path)
        self.file_user = run(('{0} -c %U').format(stat_cmd)).stdout
        self.file_group = run(('{0} -c %G').format(stat_cmd)).stdout
        self.user_name = pwd.getpwuid(os.getuid()).pw_name
        self.char = char
        return

    def _backup(self):
        """Create a backup of `filename`.

        The schema used to the new names is: *`filename`+`number`~*

            filename : The original file name.
            + : Character used to separate the file name from rest.
            number : Each time that it is made a backup then it is added
                a new number, beginning by '1'.
            ~ : Last character to indicate that it is a backup, as it's
                used in Unix/Linux systems.

        """
        if run(('!sudo !find {dir} -type f -name {file} -empty').format(dir=self.dirname, file=self.basename)).stdout:
            return
        listdir = run(('!sudo !ls {dir}').format(dir=self.dirname)).stdout.splitlines()
        all_backup = fnmatch.filter(listdir, ('{file}+[0-9]*~').format(file=self.basename))
        if all_backup:
            num_backup = [ int(x.split('+')[(-1)].rstrip('~')) for x in all_backup
                         ]
            num_backup = ('{0}').format(str(max(num_backup) + 1))
        else:
            num_backup = '1'
        backup_base = ('{file}+{number}~').format(file=self.basename, number=num_backup)
        self.backup_path = os.path.join(self.dirname, backup_base)
        run(('!sudo !cp -p {source} {backup}').format(source=self.file_path, backup=self.backup_path))
        self._setback_chown(self.backup_path)

    def _is_same(self, file1, file2):
        """Check if `file1` and `file2` are the same one.

        ...

        Returns
        -------
        boolean
            *True* if the files seem equal, *False* otherwise.

        """
        diff_cmd = run(('!sudo !diff {0} {1}').format(file1, file2))
        if not diff_cmd.stdout:
            return True
        else:
            return False

    def _setup_tmp(self):
        """Create a temporary file.

        If the user of the file is different then change the file owner
        so it's possible to copy it from python run by whatever user.

        """
        if self.user_name != self.file_user:
            run(('!sudo !chown {user} {file}').format(user=self.user_name, file=self.file_path))
            self.is_different_owner = True
        else:
            self.is_different_owner = False
        self.tmp_file = tempfile.NamedTemporaryFile(mode='w')

    def _setback_chown(self, filename):
        """Set to the original owner of the file."""
        if self.is_different_owner:
            run(('!sudo !chown {user}:{group} {file}').format(user=self.file_user, group=self.file_group, file=filename))

    def _setdown_tmp(self):
        """Remove the temporary file.

        Also set to the original owner of the file.

        """
        self._setback_chown(self.file_path)
        self.tmp_file.close()

    def _setwrite_tmp(self):
        """Write the temporary data to disk."""
        self.tmp_file.flush()
        os.fsync(self.tmp_file)

    def append(self, text, overwrite=False):
        """Write the string on `text` to file given on `filename`.

        Use a temporary file to copy the text and then that file is
        copied to the main file, so avoid to have escape the text.

        ...

        Parameters
        ----------
        text : str
            The text string to write into the file.
        overwrite : boolean, optional
            If *True*, truncate `filename` to zero length.
            Default is *False*.

        Returns
        -------
        ReturnCode.OK : int
            Exit status code indicating that it was correctly run.

        """
        self._setup_tmp()
        if self.do_backup:
            self._backup()
        if not overwrite:
            run(('!sudo !cp -p {source} {temp}').format(source=self.file_path, temp=self.tmp_file.name))
            self.tmp_file.seek(0, os.SEEK_END)
        self.tmp_file.write(text + '\n')
        self._setwrite_tmp()
        run(('!sudo !cp -p {temp} {source}').format(temp=self.tmp_file.name, source=self.file_path))
        self._setdown_tmp()
        return shell.ReturnCode.OK

    def comment(self, search):
        """Comment all lines in `filename` matching `search`.

        Prepend the comment character at the beginning of each line
        matched.

        ...

        Parameters
        ----------
        search : str or list of str
            The pattern to search.

        Returns
        -------
        ReturnCode.OK : int
            Exit status code indicating that it was correctly run.

        See Also
        --------
        comment_out

        """
        if not isinstance(search, list) and not isinstance(search, tuple):
            search = [
             search]
        expr = ''
        for regex in search:
            expr += (" -e '/{regex}/ s/^/{character}/'").format(regex=regex, character=self.char)

        self.sed(expr)
        return shell.ReturnCode.OK

    def comment_out(self, search, lstrip=False):
        """Comment out all lines in `filename` matching `search`.

        Remove the comment character of the beginning of each line
        matched.

        ...

        Parameters
        ----------
        search : str or list of str
            The pattern to search.
        lstrip : boolean, optional
            If *True*, strip the blank characters at the beginning.
            Default is *False*.

        Returns
        -------
        ReturnCode.OK : int
            Exit status code indicating that it was correctly run.

        See Also
        --------
        comment

        """
        if not isinstance(search, list) and not isinstance(search, tuple):
            search = [
             search]
        expr_search = ''
        if lstrip:
            expr_search += '[[:space:]]*'
        expr_search += ('{character}[[:space:]]*').format(character=self.char)
        expr = ''
        for regex in search:
            expr += (" -e '/^[[:space:]]*{char}.*{regex}/ s/{search}//'").format(char=self.char, regex=regex, search=expr_search)

        self.sed(expr)
        return shell.ReturnCode.OK

    def sed(self, args):
        """Wrapper to *sed* command.

        If there were any change at editing the file then does a backup
        of `filename`.

        ...

        Parameters
        ----------
        args : str
            The arguments passed to *sed*.

        Returns
        -------
        ReturnCode.OK : int
            Exit status code indicating that it was correctly run.

        """
        self._setup_tmp()
        run(('!sudo !cp -p {source} {temp}').format(source=self.file_path, temp=self.tmp_file.name))
        sed_cmd = run(('!sed -i {arguments} {file}').format(arguments=args, file=self.tmp_file.name)).stdout
        self._setwrite_tmp()
        if self.do_backup and not self._is_same(self.file_path, self.tmp_file.name):
            self._backup()
            run(('!sudo !cp -p {temp} {source}').format(temp=self.tmp_file.name, source=self.file_path))
        self._setdown_tmp()
        return shell.ReturnCode.OK


class FileError(Exception):
    pass


class PathError(Exception):
    pass