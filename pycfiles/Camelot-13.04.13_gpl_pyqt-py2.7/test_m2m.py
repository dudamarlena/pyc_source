...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/test/test_orm/test_m2m.py
...
from camelot.core.orm import Field, ManyToMany, ManyToOne, using_options, has_field, has_many, belongs_to, options, has_and_belongs_to_many, options
...
class TestManyToMany(TestMetaData):
    ...
    def test_alternate_column_formatter(self):
        options.M2MCOL_NAMEFORMAT = options.ALTERNATE_M2MCOL_NAMEFORMAT
    ...
    def test_multi(self):
        ...
        with self.session.begin():
            b1 = B(name='b1')
            a1 = A(name='a1', rel1=[B(name='b2'), b1], rel2=[
             B(name='b3'), B(name='b4'), b1])
        self.session.expire_all()
        a1 = A.query.one()
        b1 = B.get_by(name='b1')
        b2 = B.get_by(name='b2')
    ...
    def test_local_and_remote_colnames(self):
        ...
        class A(self.Entity):
            ...
            key1 = Field(Integer, primary_key=True, autoincrement=False)
            key2 = Field(String(40), primary_key=True)
            bs_ = ManyToMany('B', local_colname=['foo', 'bar'], remote_colname='baz')

        class B(self.Entity):
            using_options(shortnames=True)
            name = Field(String(60))
            as_ = ManyToMany('A', remote_colname=['foo', 'bar'], local_colname='baz')
    ...
    def test_manual_table_auto_joins(self):
        ...
        a_b = schema.Table('a_b', self.metadata, schema.Column('a_key1', Integer()), schema.Column('a_key2', String(40)), schema.Column('b_id', Integer(), schema.ForeignKey('b.id')), schema.ForeignKeyConstraint(['a_key1', 'a_key2'], [
         'a.key1', 'a.key2']))
    ...
    def test_manual_table_manual_joins(self):
        ...
        a_b = schema.Table('a_b', self.metadata, schema.Column('a_key1', Integer), schema.Column('a_key2', String(40)), schema.Column('b_id', String(60)))

        class A(self.Entity):
            using_options(shortnames=True)
            key1 = Field(Integer, primary_key=True, autoincrement=False)
            key2 = Field(String(40), primary_key=True)
            bs_ = ManyToMany('B', table=a_b, primaryjoin=lambda : and_(A.key1 == a_b.c.a_key1, A.key2 == a_b.c.a_key2), secondaryjoin=lambda : B.id == a_b.c.b_id, foreign_keys=[
             a_b.c.a_key1, a_b.c.a_key2,
             a_b.c.b_id])
