...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/camelot/view/storage.py
...
def save_stored_file(parent, stored_file):
    ...
    path = QtGui.QFileDialog.getSaveFileName(parent, _('Save as'), proposal)
    if path:
        progress = OpenFileProgressDialog()
...
def create_stored_file(parent, storage, on_finish, filter='All files (*)', remove_original=False, filename=None):
    ...
    if filename == None:
        filename = QtGui.QFileDialog.getOpenFileName(parent, 'Open file', dir, filter)
    if filename:
        filename = unicode(filename)
        remove = False
        if remove_original:
            reply = QtGui.QMessageBox(QtGui.QMessageBox.Warning, _('The file will be stored.'), _('Do you want to remove the original file?'), QtGui.QMessageBox.No | QtGui.QMessageBox.Yes)
