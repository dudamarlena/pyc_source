...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/camelot/view/action_steps/change_object.py
...
class ChangeObjectDialog(StandaloneWizardPage):
    ...
    def __init__(self, obj, admin, title=_('Please complete'), subtitle=_('Complete the form and press the OK button'), icon=Icon('tango/22x22/categories/preferences-system.png'), parent=None, flags=QtCore.Qt.Dialog):
    ...
    def update_complete(self, row=0):
        ...
        if note != None and ok != None:
            ...
            if complete:
                ...
                note.set_value(_('Please correct the data above before proceeding with the import.<br/>Incorrect cells have a pink background.'))
        return
...
class ChangeFieldDialog(StandaloneWizardPage):
    ...
    def field_changed(self, index):
        ...
        if selected_field not in (None, ValueLoading):
            ...
            delegate = field_attributes['delegate'](parent=self, **static_field_attributes)
...
class ChangeField(ActionStep):
    ...
    def render(self):
        """create the dialog. this method is used to unit test
        the action step."""
        return ChangeFieldDialog(self.admin, self.field_attributes)
