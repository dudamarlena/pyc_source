...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/camelot/admin/entity_admin.py
...
.. attribute:: expanded_list_search
    ...
    def __init__(self, app_admin, entity):
        ...
        try:
            ...
            logger.error('%s is not a mapped class, configured mappers include %s' % (self.entity, (',').join(mapped_entities)), exc_info=exception)
            raise exception
    ...
    def get_verbose_identifier(self, obj):
        if obj:
            ...
            if None not in primary_key:
                primary_key_representation = (',').join([ unicode(v) for v in primary_key ])
    ...
    def get_field_attributes(self, field_name):
        ...
        try:
            ...
            attributes = dict(python_type=str, to_string=to_string, field_name=field_name, getter=create_default_getter(field_name), length=None, tooltip=None, background_color=None, editable=False, nullable=True, widget='str', blank=True, delegate=delegates.PlainTextDelegate, validator_list=[], name=ugettext_lazy(field_name.replace('_', ' ').capitalize()))
            forced_attributes = {}
            try:
            ...
            try:
                ...
                if isinstance(property, orm.properties.ColumnProperty):
                    ...
                    target = forced_attributes.get('target', property.mapper.class_)
                    ...
                    if property.direction == orm.interfaces.ONETOMANY:
                        attributes.update(python_type=list, editable=True, nullable=True, delegate=delegates.One2ManyDelegate, target=target, create_inline=False, direction='onetomany', admin=get_entity_admin(target))
                    elif property.direction == orm.interfaces.MANYTOONE:
                        attributes.update(python_type=str, editable=True, delegate=delegates.Many2OneDelegate, target=target, nullable=foreign_keys[0].nullable, direction='manytoone', admin=get_entity_admin(target))
                    elif property.direction == orm.interfaces.MANYTOMANY:
                        attributes.update(python_type=list, editable=True, target=target, nullable=True, create_inline=False, direction='manytomany', delegate=delegates.One2ManyDelegate, admin=get_entity_admin(target))
                    else:
    ...
    def create_select_view(admin, query=None, search_text=None, parent=None):
        ...
        class SelectView(admin.TableView):
            ...
            def __init__(self, admin, parent):
                ...
                super(SelectView, self).__init__(gui_context, admin, search_text=search_text, parent=parent)
            ...
            if entity_instance in session.new:
                ...
                if None not in primary_key:
                    ...
                    if memento != None:
                        ...
                        change = memento_change(model=unicode(self.entity.__name__), memento_type='before_delete', primary_key=primary_key, previous_attributes=modifications)
            ...
            for obj_to_flush in objects_to_flush:
                if obj_to_flush in session.dirty:
                    ...
                    if modifications and None not in primary_key:
                        change = memento_change(model=unicode(self.entity.__name__), memento_type='before_update', primary_key=primary_key, previous_attributes=modifications)
    ...
    def get_expanded_search_fields(self):
        ...
        return [ (field, self.get_field_attributes(field)) for field in field_list ]
        ...
        for property in self.mapper.iterate_properties:
            if isinstance(property, orm.properties.PropertyLoader):
                if property.direction == orm.interfaces.MANYTOONE:
                    setattr(new_obj, property.key, getattr(obj, property.key))
