...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/test/test_orm/test_o2m.py
...
from camelot.core.orm import Field, OneToMany, ManyToOne, using_options, has_field, has_many, belongs_to
...
class TestOneToMany(TestMetaData):
    ...
    def test_viewonly(self):
        ...
        class User(self.Entity):
            ...
            boston_addresses = OneToMany('Address', primaryjoin=lambda : and_(Address.user_id == User.id, Address.city == 'Boston'), viewonly=True)
        ...
        self.create_all()
        with self.session.begin():
            user = User(name='u1', addresses=[
             Address(street='Queen Astrid Avenue, 32', city='Brussels'),
             Address(street='Cambridge Street, 5', city='Boston')])
        self.session.expunge_all()
        user = User.get(1)
    ...
    def test_filter_func(self):
        ...
        class User(self.Entity):
            name = Field(String(50))
            boston_addresses = OneToMany('Address', filter=lambda c: c.city == 'Boston')
            addresses = OneToMany('Address')
