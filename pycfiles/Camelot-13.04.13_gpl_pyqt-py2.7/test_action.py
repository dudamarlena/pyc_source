...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/test/test_action.py
# Compiled at: 2013-04-11 17:47:52
import datetime, logging, os, StringIO
from PyQt4 import QtGui, QtCore
from PyQt4.QtCore import Qt
from sqlalchemy import orm
from camelot.admin.action import Action, GuiContext, ActionStep
from camelot.admin.action import list_action, application_action, document_action, form_action
...
class ActionWidgetsCase(ModelThreadTestCase):
    ...
    def test_action_label(self):
        ...
        widget = ActionLabel(self.action, self.application_gui_context, self.parent)
    ...
    def test_action_push_botton(self):
        ...
        widget = ActionPushButton(self.action, self.application_gui_context, self.parent)
        self.grab_widget_states(widget, 'application')
...
class ActionStepsCase(ModelThreadTestCase):
    ...
    def test_open_file(self):
        ...
        context = {'columns': ['width', 'height'], 'table': [
                   [
                    1, 2], [3, 4]]}
    ...
    def test_orm(self):
        ...
        person = party.Person(first_name='Living', last_name='Stone', social_security_number='2003030212345')
        party.PartyContactMechanism(party=person, contact_mechanism=contact)
        ...
        class UpdatePerson(Action):
            ...
            def model_run(self, model_context):
                for person in model_context.get_selection():
                    ...
                    if soc_number:
                        person.birth_date = datetime.date(int(soc_number[0:4]), int(soc_number[4:6]), int(soc_number[6:8]))
                    ...
                    m = ('email',
                     '%s.%s@example.com' % (person.first_name,
                      person.last_name))
                    cm = party.ContactMechanism(mechanism=m)
                    pcm = party.PartyContactMechanism(party=person, contact_mechanism=cm)
                    ...
                    if soc_number:
                        ...
                        m = ('email',
                         '%s.%s@example.com' % (person.first_name,
                          person.last_name))
                        cm = party.ContactMechanism(mechanism=m)
                        party.PartyContactMechanism(party=person, contact_mechanism=cm)
...
class ListActionsCase(test_model.ExampleModelCase):
    ...
    def test_gui_context(self):
        self.assertTrue(isinstance(self.gui_context.copy(), list_action.ListActionGuiContext))
        model_context = self.gui_context.create_model_context()
        self.assertTrue(isinstance(model_context, list_action.ListActionModelContext))
    ...
    def test_sqlalchemy_command(self):
        ...
        bt.flush()
        model_context.session.query(BatchJobType).update(values={'name': 'accounting audit'}, synchronize_session='evaluate')
    ...
    def test_import_from_file(self, filename='import_example.csv'):
        ...
        self.context = MockModelContext()
        self.context.obj = Person.query.first()
    ...
    def test_drag_and_drop(self):
        ...
        proxy.dropMimeData(mime_data, Qt.MoveAction, -1, -1, QtCore.QModelIndex())
...
class FormActionsCase(test_model.ExampleModelCase):
    ...
    def test_gui_context(self):
        self.assertTrue(isinstance(self.gui_context.copy(), form_action.FormActionGuiContext))
        self.assertTrue(isinstance(self.gui_context.create_model_context(), form_action.FormActionModelContext))
...
class DocumentActionsCase(ModelThreadTestCase):
    ...
    def test_gui_context(self):
        self.assertTrue(isinstance(self.gui_context.copy(), document_action.DocumentActionGuiContext))
        self.assertTrue(isinstance(self.gui_context.create_model_context(), document_action.DocumentActionModelContext))
