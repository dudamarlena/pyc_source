...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/camelot/core/orm/entity.py
...
from sqlalchemy.ext.declarative.api import _declarative_constructor, DeclarativeMeta
    ...
    def create_pk_cols(self):
        ...
        if self._pk_col_done:
            ...
            if not self.has_pk and base_descriptor == None:
                ...
                self.add_column(colname, schema.Column(colname, options.DEFAULT_AUTO_PRIMARYKEY_TYPE, primary_key=True))
...
def entity_to_dict(entity, deep={}, exclude=[]):
    ...
    mapper = orm.object_mapper(entity)
    col_prop_names = [ p.key for p in mapper.iterate_properties if isinstance(p, orm.properties.ColumnProperty)
                     ]
    data = dict([ (name, getattr(entity, name)) for name in col_prop_names if name not in exclude
                ])
