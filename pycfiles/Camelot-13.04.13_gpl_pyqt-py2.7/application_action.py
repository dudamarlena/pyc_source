...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/camelot/admin/action/application_action.py
...
class OpenTableView(EntityAction):
    ...
    """
    modes = [
     Mode('new_tab', _('Open in New Tab'))]
...
class ShowAbout(Action):
    ...
    def gui_run(self, gui_context):
        ...
        QtGui.QMessageBox.about(gui_context.workspace, ugettext('About'), unicode(abtmsg))
...
class Refresh(Action):
    ...
    def model_run(self, model_context):
        ...
        for i, (_key, obj) in enumerate(session.identity_map.items()):
            ...
            if i % 10 == 0:
                yield action_steps.UpdateProgress(i, session_items, progress_db_message)
    ...
    def model_run(self, model_context):
        ...
        class Options(object):
            ...
            class Admin(ObjectAdmin):
                list_display = [
                 'level']
                field_attributes = {'level': {'delegate': delegates.ComboBoxDelegate, 'editable': True, 
...
class RuntimeInfo(Action):
    ...
    def model_run(self, model_context):
        from camelot.view import action_steps
        import sys, sqlalchemy, chardet, jinja2, xlrd, xlwt
...
class SegmentationFault(Action):
    ...
    def model_run(self, model_context):
        from camelot.view import action_steps
        ok = yield action_steps.MessageBox(text='Are you sure you want to segfault the application', standard_buttons=QtGui.QMessageBox.No | QtGui.QMessageBox.Yes)
