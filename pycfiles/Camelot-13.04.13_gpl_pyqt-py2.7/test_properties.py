...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/test/test_orm/test_properties.py
...
from camelot.core.orm import Field, GenericProperty, ColumnProperty, ManyToOne, OneToMany, has_field, has_property
...
class TestSpecialProperties(TestMetaData):
    ...
    def test_generic_property(self):
        ...
        class Tag(self.Entity):
            score1 = Field(Float)
            score2 = Field(Float)
            score = GenericProperty(lambda c: column_property((c.score1 * c.score2).label('score')))
    ...
    def test_column_property_eagerload_and_reuse(self):
        ...
        class User(self.Entity):
            ...
            score = ColumnProperty(lambda c: select([func.sum(Tag.score)], Tag.user_id == c.id).as_scalar())

        class Category(self.Entity):
            name = Field(String(16))
            users = OneToMany('User', lazy=False)
            score = ColumnProperty(lambda c: select([func.avg(User.score)], User.category_id == c.id).as_scalar())
    ...
    def test_has_property(self):
        ...
        class Tag(self.Entity):
            ...
            has_property('score', lambda c: column_property((c.score1 * c.score2).label('score')))
