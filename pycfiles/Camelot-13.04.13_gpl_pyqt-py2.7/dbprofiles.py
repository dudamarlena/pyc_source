...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/camelot/core/dbprofiles.py
...
def selected_profile_info():
    ...
    try:
        ...
        logger.error('no profile named %s, available profiles are ' % profilename)
...
def last_used_profile():
    ...
    return unicode(settings.value('last_used_database_profile', QtCore.QVariant('')).toString(), 'utf-8')
