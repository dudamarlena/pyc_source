...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/camelot/view/action_steps/gui.py
...
class OpenFormView(ActionStep):
    ...
    def gui_run(self, gui_context):
        ...
        model = CollectionProxy(self.admin, create_collection_getter(self.objects), self.admin.get_fields, max_number_of_rows=10)
        title = ''
...
class ShowChart(ActionStep):
    ...
    def gui_run(self, gui_context):
        ...
        ChartEditor.show_fullscreen_chart(self._chart, gui_context.workspace)
...
class MessageBox(ActionStep):
    ...
    def __init__(self, text, icon=QtGui.QMessageBox.Information, title=_('Message'), standard_buttons=default_buttons):
        self.icon = icon
    ...
    def render(self):
        ...
        the action step."""
        return QtGui.QMessageBox(self.icon, self.title, self.text, self.standard_buttons)
