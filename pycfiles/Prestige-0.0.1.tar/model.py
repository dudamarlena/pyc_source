...
# Compiled at: 2015-06-03 02:38:40
import datetime
...
def post_model(UserModel):
    ...
    class PostTag(SlugNameMixin, db.Model):
        ...
        @property
        def total_posts(self):
            return PostPostTag.all().filter(PostPostTag.tag_id == self.id).count()
    ...
    class Post(db.Model):
        ...
        def get_published_by_tag_slug(cls, slug):
            ...
            :return SQLA :
            """
            return cls.all().join(PostTag).filter(PostTag.slug.in_(slug)).filter(cls.is_published == True)
        ...
        def update_tags(self, tags_list):
            ...
            """
            tags = PostPostTag.all().filter(PostPostTag.post_id == self.id)
    ...
    return utils.to_struct(Post=Post, Category=PostCategory, Type=PostType, PostCategory=PostPostCategory, Tag=PostTag, PostTag=PostPostTag, UploadObject=PostUploadObject)