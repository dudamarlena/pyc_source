...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/tasty/controllers.py
# Compiled at: 2007-05-08 14:19:01
...
class ObjectsDescriptor(object):
    ...
    def __get__(self, obj, type=None):
        try:
            ...
            value = {'user': [], 'item': [], 'tag': [], '-user': [], '-item': [], '-tag': []}
...
class Root(controllers.Root):
    ...
    def to_event_str(self):
        users = 'users=' + (',').join([ u.username for u in self.obj['user'] ]) + (',').join([ '-' + u.username for u in self.obj['-user'] ])
        items = 'items=' + (',').join([ u.name for u in self.obj['item'] ]) + (',').join([ '-' + u.name for u in self.obj['-item'] ])
        tags = 'tags=' + (',').join([ u.name for u in self.obj['tag'] ]) + (',').join([ '-' + u.name for u in self.obj['-tag'] ])
