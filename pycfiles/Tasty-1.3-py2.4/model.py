...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/tasty/model.py
# Compiled at: 2007-05-08 14:18:56
...
class Service(SQLObject):
    ...
    def all_rels(self, minus):
        return [ (uid.user, uid.item, uid.tag) for uid in UserItemTag.select(AND(UserItemTag.q.userID == User.q.id, User.q.serviceID == self.id, UserItemTag.q.itemID == Item.q.id, Item.q.serviceID == self.id, UserItemTag.q.tagID == Tag.q.id, Tag.q.serviceID == self.id)) if uid.user not in minus if uid.item not in minus if uid.tag not in minus
               ]
...
def add_uit(user, tag, item):
    r = list(UserItemTag.select(AND(UserItemTag.q.userID == user.id, UserItemTag.q.tagID == tag.id, UserItemTag.q.itemID == item.id)))
    if len(r) == 0:
...
def del_user_item(user, item):
    r = UserItemTag.select(AND(UserItemTag.q.userID == user.id, UserItemTag.q.itemID == item.id))
    for uit in r:
        uit.destroySelf()
...
def del_user_tag(user, tag):
    r = UserItemTag.select(AND(UserItemTag.q.userID == user.id, UserItemTag.q.tagID == tag.id))
    for uit in r:
        uit.destroySelf()
...
def del_item_tag(item, tag):
    r = UserItemTag.select(AND(UserItemTag.q.itemID == item.id, UserItemTag.q.tagID == tag.id))
    for uit in r:
        uit.destroySelf()
    ...
    for u in users:
        for i in items:
            for t in tags:
                ...
                r = UserItemTag.select(AND(UserItemTag.q.userID == u.id, UserItemTag.q.itemID == i.id, UserItemTag.q.tagID == t.id))
                for uit in r:
                    uit.destroySelf()
...
def filter_query(service, users, items, tags, nusers, nitems, ntags):
    ...
    res = {}
    if zeros == 1:
        res = filter_one_variable(users, items, tags, build_minus(nusers, nitems, ntags))
    if zeros == 2:
        res = filter_two_variables(users, items, tags, build_minus(nusers, nitems, ntags))
    if zeros == 3:
        res = filter_three_variables(service, users, items, tags, build_minus(nusers, nitems, ntags))
    return res
...
def construct_query(vars, fixed):
    ...
    return 'SELECT ' + s + ' FROM user_item_tag WHERE (' + where + ') GROUP BY ' + s + ' HAVING count(id) = ' + str(fixed_len(fixed))
...
def filter_two_variables(users, items, tags, minus):
    ...
    res = UserItemTag._connection.queryAll(q)
    rdict = {vars[0].name_s + '_' + vars[1].name_s + 's': [ (a, b) for (a, b) in [ (vars[0].get(i[0]), vars[1].get(i[1])) for i in res ] if a not in minus if b not in minus ]}
    for v in vars:
...
def filter_three_variables(service, users, items, tags, minus):
    ...
    res = {'users': users, 'items': items, 'tags': tags, 'user_item_tags': service.all_rels(minus)}
    return res
...
def cloud_one(users, items, tags):
    ...
    q = 'select u.%s, count(*) as cnt from user_item_tag uit, %s u\n    where uit.%s = u.id AND uit.%s in (%s) and uit.%s in (%s) group by uit.%s,u.%s order by upper(u.%s) ASC' % (var.fname_s, var.sqlmeta.table, var.field_s, fixed[0][0].field_s, (',').join([ str(i.id) for i in fixed[0][1] ]), fixed[1][0].field_s, (',').join([ str(i.id) for i in fixed[1][1] ]), var.field_s, var.fname_s, var.fname_s)
...
def cloud_two(users, items, tags):
    ...
    r = {}
    for t in var:
        q = 'select u.%s, count(*) as cnt from user_item_tag uit, %s u\n            where uit.%s = u.id and uit.%s in (%s) group by uit.%s,u.%s order by upper(u.%s) ASC' % (t.fname_s, t.sqlmeta.table, t.field_s, fixed[0].field_s, (',').join([ str(i.id) for i in fixed[1] ]), t.field_s, t.fname_s, t.fname_s)
...
def cloud_three(service):
    ...
    for t in [User, Item, Tag]:
        q = 'select u.%s, count(*) as cnt from user_item_tag uit, %s u\n        where uit.%s = u.id and u.service_id = %d group by uit.%s,u.%s order by upper(u.%s) ASC' % (t.fname_s, t.sqlmeta.table, t.field_s, service.id, t.field_s, t.fname_s, t.fname_s)
        r[t.name_s + 's'] = [ {t.name_s: uit[0], 'count': uit[1]} for uit in UserItemTag._connection.queryAll(q) ]
