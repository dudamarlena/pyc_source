...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/tasty/dimension.py
# Compiled at: 2007-05-08 14:18:58
...
class Dimension(RESTResource):
    ...
    __module__ = __name__
    list = list
    REST_content_types = {'application/xml': 'to_xml', 'text/html': 'to_html', 'text/plain': 'to_json'}
    ...
    def delete(self, obj):
        delete_rels(self.main.s, self.main.obj['user'], self.main.obj['item'], self.main.obj['tag'], self.main.obj['-user'], self.main.obj['-item'], self.main.obj['-tag'])
        return 'ok'

    delete.expose_resource = True

    def index(self, obj, **params):
        results = filter_query(self.main.s, self.main.obj['user'], self.main.obj['item'], self.main.obj['tag'], self.main.obj['-user'], self.main.obj['-item'], self.main.obj['-tag'])
        return results

    index.expose_resource = True

    def related(self, obj, limit=10, **params):
        results = related(self.main.s, self.main.obj['user'], self.main.obj['item'], self.main.obj['tag'], self.main.obj['-user'], self.main.obj['-item'], self.main.obj['-tag'], limit=limit)
        return results
    ...
    def REST_instantiate(self, id, **kwargs):
        ...
        try:
            if id.startswith('-'):
                id = id[1:]
                o = self.rclass.select(AND(self.r_id_q == id.encode('utf8'), self.rclass.q.serviceID == service.id))[0]
