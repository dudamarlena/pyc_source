...
# [GCC 8.4.0]
# Embedded file name: troy/utils/time_it.py
# Compiled at: 2014-02-24 20:40:00
import time, troy, pymongo, weakref, datetime, radical.utils as ru
...
class Timed(object):
    ...
    def timed_store(self, url):
        ...
        for ct in self.timed_components:
            components.append({'type': ct, 'ids': self.timed_components[ct].keys()})

        collection.save({'_id': self.timed_id, 'components': components, 
        ...
        for component_type in self.timed_components:
            for component_id in self.timed_components[component_type]:
                ...
                for ct in component.timed_components:
                    components.append({'type': ct, 'ids': component.timed_components[ct].keys()})

                collection.save({'_id': component.timed_id, 'type': component.timed_type, 
    ...
    def timed_component(self, component, component_type, component_id):
        if component_type not in self.timed_components:
            self.timed_components[component_type] = dict()
        if component_id not in self.timed_components[component_type]:
    ...
    def timed_duration_start(self, event, tags):
        if not isinstance(tags, list):
            tags = [
             tags]
        if event in self._timed_current and self._timed_current[event]:
            raise RuntimeError('cannot recursively time %s' % event)
        start = datetime.datetime.utcnow()
        self._timed_current[event] = {'start': start, 
           'event': event, 
           'tags': tags}
    ...
    def timed_duration_stop(self, event, tags):
        if not isinstance(tags, list):
            tags = [
             tags]
        if event not in self._timed_current or not self._timed_current[event]:
    ...
    def timed_event(self, event, tags, timer=None):
        ...
        if event_sig in self.timed_events_known:
            ...
            self.timed_events.append({'time': timer, 'event': event, 
               'tags': tags})
