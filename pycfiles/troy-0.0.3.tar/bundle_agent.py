...
# [GCC 8.4.0]
# Embedded file name: troy/bundle_wrapper/../external/bundle/src/bundle/impl/bundle_agent.py
# Compiled at: 2014-02-24 20:40:00
"""
Handle link-down case
"""
import sys, getopt, Queue, threading, logging, socket, datetime, time, os, xml.etree.ElementTree as ET, re, traceback
try:
    import paramiko
except ImportError:
...
class MoabAgent(threading.Thread):
    ...
    def __init__(self, credential, verbose=0, db_dir=None, use_built_in=False):
        ...
        try:
            self.ssh.connect(hostname=self.hostname, port=credential.get('port', 22), username=credential.get('username'), password=credential.get('password'), key_filename=credential.get('key_filename'))
    ...
    def workload(self):
        ...
        if self.h_flag is True:
            return {'alive_nodes': self.alive_nodes, 
            ...
            return {'alive_nodes': self.alive_nodes, 'alive_procs': self.alive_procs, 
               'busy_nodes': self.busy_nodes, 
               'busy_procs': self.busy_procs, 
               'free_nodes': self.free_nodes, 
               'free_procs': self.free_procs}
    ...
    def exec_showq2(self):
        ...
        if exit_status != 0:
            ...
            if self.state is AgentState.Fault:
                self.state = AgentState.Up
                self.state_reason_code = ''
            root = ET.fromstring(stdout)
            cluster = root.find('cluster')
            ...
            for queue in root.iter('queue'):
                if queue.attrib['option'] == 'active':
                    for job in queue.findall('job'):
                        ...
                        running_list.append({'JobID': job.attrib['JobID'], 
                    ...
                    for job in queue.findall('job'):
                        ...
                        if 'ReqNodes' in job.attrib:
                            ...
                            ReqNodes = int(job.attrib['ReqProcs']) / 8 + (1 if int(job.attrib['ReqProcs']) % 8 else 0)
                        queueing_list.append({'JobID': job.attrib['JobID'], 
                           'Class': job.attrib['Class'], 
                           'Group': job.attrib['Group'], 
                           'User': job.attrib['User'], 
                           'JobName': job.attrib['JobName'], 
        ...
        return (
         cur_time, cur_alive_nodes, cur_busy_nodes, cur_avail_nodes, running_list, queueing_list)
                    ...
                    for job in queue.findall('job'):
                        ...
                        if _JobID not in self.rtime_tbl:
                            ...
                            if _job_signature in self.rtime_lkup_tbl:
                                ...
                                self.rtime_lkup_tbl[_job_signature] = [
                                 _JobID, -1]
    ...
    def get_nodes_stat(self):
        ...
        for node in root.findall('Node'):
            ...
            if state == 'free' or state == 'job-exclusive':
                alive_node_list[node.find('name').text] = [
                 state, int(node.find('np').text), node.find('properties').text]
            ...
            for k, v in nl.items():
                ...
                if properties in self.queue_features.values():
                    if properties not in properties_nodes:
                        properties_nodes[properties] = {'np': np, 'alive_nodes': 0, 
                           'alive_procs': 0, 
                           'busy_nodes': 0, 
                           'busy_procs': 0, 
                           'free_nodes': 0, 
            ...
            if line_str.find('Queue') == 0:
                ...
                if queue_attr_k == 'acl_user_enable':
                    ...
                    if queue_attr_v.count(':') == 2:
                        ...
                        _max_walltime = seconds = 86400 * d + 3600 * h + 60 * m + s
    ...
    def show_start(self, p_procs, wall_time):
        exit_status, stdout, stderr = self.run_cmd(('showstart --xml {0}@{1}').format(str(p_procs), wall_time))
    ...
    def resource_query1(self):
        ...
        return (
         cur_avail_nodes * 8, cur_avail_nodes, self.max_walltime)
    ...
    def resource_query5(self, est_runtime, p_procs=None, n_nodes=None):
        ...
        if cur_avail_nodes >= n_nodes and len(queueing_list) == 0:
            ...
            return (
             est_queue_time, est_start_time)
        cur_time_save = cur_time
        monalisasmile = ';-)'
        fake_job = {'JobID': monalisasmile, 'Class': monalisasmile, 'Group': monalisasmile, 'User': monalisasmile, 'JobName': monalisasmile, 'ReqNodes': n_nodes, 
           'ReqProcs': p_procs, 'WallClock': est_runtime, 'Priority': monalisasmile}
        ...
        while len(queueing_list):
            ...
            while cur_time < shadow_time:
                ...
                for idx in range(1, len(queueing_list)):
                    if queueing_list[idx]['ReqNodes'] <= cur_avail_nodes and cur_time + queueing_list[idx]['WallClock'] <= shadow_time:
                        pass
                    else:
                        if queueing_list[idx]['ReqNodes'] <= min(cur_avail_nodes, extra_nodes):
                ...
                if hostname == self.hostname:
                    ...
                    for line in file_trace:
                        ...
                        if f_job['JobID'] not in self.rtime_tbl:
                            self.rtime_tbl[f_job['JobID']] = [
                             int(f_job['AWDuration']), int(f_job['CompletionTime'])]
                            _counting1 += 1
                            if 'ReqNodes' in f_job:
                                _job_signature = (
                                 f_job['Class'], f_job['Group'], f_job['User'],
                            ...
                            if _job_signature in self.rtime_lkup_tbl:
                                ...
                                elif self.rtime_lkup_tbl[_job_signature][1] != -1 and int(f_job['CompletionTime']) > self.rtime_tbl[self.rtime_lkup_tbl[_job_signature][1]][1]:
            ...
            while cur_time < shadow_time:
                ...
                while found_bf_job:
                    ...
                    for idx in range(left_pos, len(queueing_list)):
                        if queueing_list[idx]['ReqNodes'] <= cur_avail_nodes and cur_time + self.run_time(queueing_list[idx]) <= shadow_time:
                            found_bf_job = True
...
supported_cluster_types = {'moab': MoabAgent}

def get(cluster_attributes, db_dir=None):
    try:
        if cluster_attributes['cluster_type'].lower() == 'moab':
            return MoabAgent(cluster_attributes, 0, db_dir, use_built_in=True)
