...
# [GCC 8.4.0]
# Embedded file name: /home/merzky/saga/troy/troy/workload/workload_manager.py
...
class WorkloadManager(tu.Timed):
    ...
    def __init__(self, session, translator=AUTOMATIC, scheduler=AUTOMATIC, dispatcher=AUTOMATIC):
        """
    ...
    def _init_plugins(self):
        ...
        if not self._dispatcher:
            raise RuntimeError('Could not load dispatcher plugin')
    ...
    def unit_id_to_native_id(cls, unit_id, native_id=None):
        if native_id:
            ...
            if unit_id not in cls._unit_id_map:
    ...
    def parse_workload(self, workload_description):
        ...
        try:
            ...
            troy._logger.warn("cannot read WL description at '%s'" % workload_description)
        ...
        if not task_descriptions and not relation_descriptions:
            raise ValueError('Could not parse workload description\n----\n%s\n----\n' % workload_description)
        workload = troy.Workload(self.session, task_descriptions, relation_descriptions)
        return workload.id

    def create_workload(self, task_descriptions=None, relation_descriptions=None):
        """
        Notes
    ...
    def translate_workload(self, workload_id, overlay_id=None):
        ...
        self._init_plugins()
        workload.timed_method('translate', [], self._translator.translate, [workload, overlay])
    ...
    def bind_workload(self, workload_id, overlay_id, bind_mode=None):
        ...
        if bind_mode == EARLY:
            if overlay.state not in [TRANSLATED, SCHEDULED]:
                raise ValueError("overlay '%s' not in TRANSLATED or SCHEDULED state, cannot do early binding" % str(overlay.id))
        elif bind_mode == LATE:
            if overlay.state not in [SCHEDULED, PROVISIONED]:
                raise ValueError("overlay '%s' not in SCHEDULED or PROVISIONED state, cannot do late binding" % str(overlay.id))
        self._init_plugins()
        workload.timed_method('schedule', [overlay.id], self._scheduler.schedule, [workload, overlay])
    ...
    def dispatch_workload(self, workload_id, overlay_id):
        ...
        workload.timed_method('dispatch', [overlay.id], self._dispatcher.dispatch, [workload, overlay])
    ...
    def stage_in_workload(self, workload_id):
        ...
        workload = self.get_workload(workload_id)
        workload.timed_method('stage_in_workload', [], self._stager.stage_in_workload, [workload])
    ...
    def stage_out_workload(self, workload_id):
        ...
        if not self._stager:
            self._stager = troy.DataStager(self.session)
        workload = self.get_workload(workload_id)
        workload.timed_method('stage_out_workload', [], self._stager.stage_out_workload, [workload])