...
# [GCC 8.4.0]
# Embedded file name: /home/merzky/saga/troy/troy/plugins/overlay_provisioner/plugin_overlay_provisioner_local.py
# Compiled at: 2014-02-27 11:31:04
import os, time, subprocess, radical.utils as ru
...
class _Unit(object):
    ...
    def __init__(self, descr):
        ...
        self.proc = None
        self.retval = None
        if 'executable' not in self.descr:
            self.descr['executable'] = '/bin/true'
        if 'arguments' not in self.descr:
            self.descr['arguments'] = list()
        if 'environment' not in self.descr:
    ...
    def run(self):
        ...
        self._proc = subprocess.Popen([command], env=env, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        self.state = _RUNNING
    ...
    **Configuration Options:**  None
    ...
    def provision(self, overlay):
        ...
        for pid in overlay.pilots.keys():
            ...
            if resource.scheme != 'fork' or resource.host != 'localhost':
                raise ValueError('Can only provision to fork://localhost (%s)' % resource)
            p = _Pilot(pilot)
            p.run()
            self.pilots[p.id] = p
            pilot._set_instance(instance_type='default', provisioner=self, instance=p, native_id=p.id)
    ...
    def pilot_get_info(self, pilot):
        ...
        for unit_id in p.list_units():
            unit = troy.ComputeUnit(pilot.session, _native_id=unit_id, _pilot_id=pilot.id)
            info['units'][unit.id] = unit

        info['state'] = {_NEW: DESCRIBED, _RUNNING: PROVISIONED, 
           _FAILED: FAILED, 
           _CANCELED: CANCELED, 
