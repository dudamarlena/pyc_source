...
# [GCC 8.4.0]
# Embedded file name: troy/planner/planner.py
# Compiled at: 2014-02-24 20:40:00
__author__ = 'TROY Development Team'
__copyright__ = 'Copyright 2013, RADICAL'
__license__ = 'MIT'
import threading, radical.utils as ru, troy.utils as tu
...
class Planner(tu.Timed):
    ...
    def derive_overlay(self, workload_id):
        ...
        if workload.state not in [PLANNED, DESCRIBED]:
            raise ValueError("workload '%s' not in DESCRIBED or PLANNED state" % workload.id)
        elif workload.state is DESCRIBED and workload.parametrized:
            raise ValueError("Parametrized workload '%s' not PLANNED yet." % workload.id)
        self._init_plugins(workload)
        overlay_descr = workload.timed_method('derive_overlay', [], self._derive.derive_overlay, [workload])
    ...
    def expand_workload(self, workload_id):
        ...
        self._init_plugins(workload)
        workload.timed_method('expand', [], self._expand.expand_workload, [workload])
