...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/presence.py
# Compiled at: 2017-04-03 02:32:49
# Size of source mod 2**32: 16627 bytes
"""Reports presence information into Zookeeper."""
import errno, os, time, logging, subprocess, sys, kazoo, yaml
from . import exc
from . import cgroups
...
def _create_ephemeral_with_retry(zkclient, path, data):
    ...
    for _ in range(0, 5):
        try:
            return zkutils.create(zkclient, path, data, acl=[_SERVERS_ACL], ephemeral=True)
        except kazoo.client.NodeExistsError:
            prev_data, metadata = zkutils.get_default(zkclient, path, need_metadata=True)
            _LOGGER.warn('Node exists, will retry: %s, data: %r, metadata: %r', path, prev_data, metadata)
...
class EndpointPresence(object):
    __doc__ = 'Manages application endpoint registration in Zookeeper.'
    ...
    def register_running(self):
        ...
        self.log.logger.info('registering container as running: %s', self.appname)
        _create_ephemeral_with_retry(self.zkclient, z.path.running(self.appname), self.hostname)

    def unregister_running(self):
        """Safely deletes the "running" node for the container."""
        self.log.logger.info('un-registering container as running: %s', self.appname)
    ...
    def unregister_endpoints(self):
        ...
        for endpoint in endpoints:
            ...
            ep_proto = endpoint.get('proto', 'tcp')
            if not ep_name:
                self.log.critical('Logic error, no endpoint info: %s', self.manifest)
                return
            path = z.path.endpoint(self.appname, ep_proto, ep_name)
    ...
    def register_identity(self):
        ...
        self.log.logger.info('Register identity: %s, %s', identity_group, identity)
        _create_ephemeral_with_retry(self.zkclient, z.path.identity_group(identity_group, str(identity)), {'host': self.hostname,  'app': self.appname})
...
def kill_node(zkclient, node):
    ...
    for app in apps:
        ...
        try:
            ...
            app_presence = EndpointPresence(zkclient, manifest, hostname=node, appname=app)
...
class ServicePresence(object):
    __doc__ = 'Manages service presence and lifecycle events.'

    def __init__(self, manifest, container_dir, appevents_dir, hostname=None):
        self.manifest = manifest
    ...
    def ensure_supervisors_running(self):
        ...
        for service in self.services:
            while not supervisor.is_supervisor_running(self.services_dir, service):
                self.log.logger.info('%s/%s not yet supervised.', self.services_dir, service)
                time.sleep(0.5)
    ...
    def start_service(self, service_name):
        ...
        try:
            ...
            restart_data = {'limit': _MAX_RESTART_RATE, 
             'interval': _RESTART_RATE_INTERVAL}

        svc_dir = os.path.join(self.services_dir, service_name)
        restart_rate_exceeded, actual_restarts = self._actual_restarts(service_name, restart_data)
        self.log.logger.info('starting %s, retries %s/%s in %s', service_name, actual_restarts, restart_data['limit'], restart_data['interval'])
    ...
    def wait_for_exit(self, container_svc_dir):
        ...
        watched_dirs = [os.path.join(self.services_dir, svc) for svc in self.services]
    ...
    def update_exit_status(self, service_name):
        ...
        if prev_count == count:
            self.log.logger.info('Exit status already reported, count: %d', count)
            return
        self.log.logger.info('exit (rc, signal): (%s, %s)', exitinfo['rc'], exitinfo['sig'])
        appevents.post(self.appevents_dir, traceevents.ServiceExitedTraceEvent(instanceid=self.appname, uniqueid=self.uniqueid, service=service_name, rc=exitinfo['rc'], signal=exitinfo['sig']))
    ...
    def report_running(self, service_name):
        ...
        appevents.post(self.appevents_dir, traceevents.ServiceRunningTraceEvent(instanceid=self.appname, uniqueid=self.uniqueid, service=service_name))
