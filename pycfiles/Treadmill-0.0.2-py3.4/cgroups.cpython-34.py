...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/cgroups.py
...
_BLKIO_THROTTLE_TYPES = {'bps': 'blkio.throttle.io_service_bytes', 
...
def available_subsystems():
    ...
    with open(PROCCGROUPS) as (cgroups):
        for cgroup in cgroups:
            try:
                subsys_name, _hierarchy, _num_cgroups, enabled = cgroup.split()
                if subsys_name[0] != '#':
                    if enabled == '1':
...
def mounted_subsystems():
    ...
    with open(PROCMOUNTS) as (mounts):
        ...
        for mountline in mounts:
            try:
                _fs_spec, fs_file, fs_vfstype, fs_mntops, _fs_freq, _fs_passno = mountline.split()
