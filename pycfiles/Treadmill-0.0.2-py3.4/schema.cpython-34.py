...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/schema.py
# Compiled at: 2017-04-03 02:32:49
# Size of source mod 2**32: 1827 bytes
"""Helper tools for jsonschema."""
import os, decorator, jsonschema
_SCHEMA_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), 'schemas'))
_TEST_MODE = False

def schema(*schemas, **kwschemas):
    """Schema decorator."""
    resolver = jsonschema.RefResolver('file://' + _SCHEMA_DIR + '/', None)
    validators = [jsonschema.Draft4Validator(s, resolver=resolver) for s in schemas]
    kwvalidator = jsonschema.Draft4Validator({'type': 'object', 
     'additionalProperties': False, 
     'properties': kwschemas}, resolver=resolver)
    ...
    def decorated(func, *args):
        ...
        kwargs = {}
        for kw_name, kw_value, kw_default in zip(reversed(argspec.args), reversed(args), defaults):
