...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/master.py
# Compiled at: 2017-04-03 02:32:49
# Size of source mod 2**32: 44455 bytes
"""Treadmill master process."""
import collections, logging, fnmatch, os, time, threading, re, kazoo
...
def resources(data):
    ...
    parsers = {'memory': utils.megabytes, 
...
class Master(object):
    __doc__ = 'Treadmill master scheduler.'
    ...
    def load_partition(self, partition):
        ...
        try:
            ...
            self.cell.partitions[partition] = scheduler.Partition(max_server_uptime=data.get('server_uptime'), max_lease=data.get('max_lease'), threshold=data.get('threshold'))
    ...
    def load_bucket(self, bucketname):
        ...
        data = zkutils.get_default(self.zkclient, z.path.bucket(bucketname), default={})
    ...
    def load_server(self, servername, readonly=False):
        ...
        try:
            ...
            if not data:
                _LOGGER.info('No capacity detected: %s', z.path.server(servername))
                return
            ...
            server = scheduler.Server(servername, resources(data), valid_until=partition.valid_until(up_since), label=label, traits=data.get('traits', 0))
            parent = self.buckets.get(parentname)
            if not parent:
                _LOGGER.warn('Server parent does not exist: %s/%s', servername, parentname)
                return
            self.buckets[parentname].add_node(server)
            self.servers[servername] = server
            assert server.parent == self.buckets[parentname]
            if not readonly:
                zkutils.ensure_exists(self.zkclient, z.path.placement(servername), acl=[
                 _SERVERS_ACL])
    ...
    def reload_server(self, servername):
        ...
        try:
            ...
            if current_server.is_same(server) and current_server.parent == parent:
    ...
    def adjust_server_state(self, servername, readonly=False):
        ...
        elif server.state is not scheduler.State.frozen:
            server.state = scheduler.State.up
        state, since = server.get_state()
        if not readonly:
            zkutils.put(self.zkclient, placement_node, {'state': state.value,  'since': since})
    ...
    def load_app(self, appname, readonly=False):
        ...
        if app:
            ...
            app = scheduler.Application(appname, priority, demand, affinity=affinity, affinity_limits=affinity_limits, identity_group=identity_group, schedule_once=schedule_once, data_retention_timeout=data_retention, lease=lease)
    ...
    def load_identity_groups(self):
        ...
        for name in names:
            ident = zkutils.get_default(self.zkclient, z.path.identity_group(name))
            if ident:
    ...
    def restore_placements(self):
            ...
            for appname in placed_apps:
                ...
                if appname not in self.cell.apps:
                    ...
                    if server.put(app):
                        ...
                        _LOGGER.info('Failed to restore placement: %s => %s', appname, servername)
                        zkutils.ensure_deleted(self.zkclient, appnode)
                        if app.schedule_once:
                            _LOGGER.info('Removing scheduled once app: %s', appname)
                            zkutils.ensure_deleted(self.zkclient, z.path.scheduled(appname))
                            self.cell.remove_app(appname)
                            continue
        ...
        for appname, servers in integrity.items():
            if len(servers) > 1:
                ...
                for servername in servers:
                    zkutils.ensure_deleted(self.zkclient, z.path.placement(servername, appname))
    ...
    def load_placement_data(self):
        ...
        for appname, app in self.cell.apps.items():
            if app.server:
                placement_data = zkutils.get_default(self.zkclient, z.path.placement(app.server, appname))
    ...
    def adjust_presence(self, servers):
        ...
        down_servers = set([servername for servername in self.servers if self.servers[servername].state is scheduler.State.down])
        ...
        for server in servers:
            ...
            for app in apps:
                if app not in app2server:
                    ...
                    continue
                _LOGGER.critical('Duplicate placement: %s: (%s, %s)', app, app2server[app], server)
                ...
                if server != correct_placement:
                    _LOGGER.critical('Removing incorrect placement: %s/%s', server, app)
                    zkutils.ensure_deleted(self.zkclient, z.path.placement(server, app))
                if app2server[app] != correct_placement:
                    _LOGGER.critical('Removing incorrect placement: %s/%s', app2server[app], app)
                    zkutils.ensure_deleted(self.zkclient, z.path.placement(app2server[app], app))
                    continue
            ...
            if app.server:
                if appname not in app2server:
                    ...
                    if app.server != app2server[appname]:
                        _LOGGER.critical('corrupted placement %s: expected: %s, actual: %s', appname, app.server, app2server[appname])
                        success = False
                    else:
                        continue
    ...
    def process(self, event):
        ...
        callbacks = {z.SERVER_PRESENCE: self.process_server_presence, 
    ...
    def process_scheduled(self, scheduled):
        ...
        for appname in current - target:
            app = self.cell.apps[appname]
            if app.server:
                zkutils.ensure_deleted(self.zkclient, z.path.placement(app.server, appname))
            if self.events_dir:
                appevents.post(self.events_dir, traceevents.DeletedTraceEvent(instanceid=appname))
            self.cell.remove_app(appname)
    ...
    def process_events(self, events):
        ...
        ordered = sorted([tuple([event.split('-')[i] for i in [0, 2, 1]]) for event in events if re.match('\\d+\\-\\w+\\-\\d+$', event)])
        for prio, seq, resource in ordered:
            ...
            if resource == 'allocations':
                ...
                apps = zkutils.get_default(self.zkclient, z.path.event(node_name), default=[])
                for app in apps:
                    self.load_app(app)
                ...
                servers = zkutils.get_default(self.zkclient, z.path.event(node_name), default=[])
                if not servers:
                    servers = set(self.servers.keys()) ^ set(self.zkclient.get_children(z.SERVERS))
    ...
    def _schedule_reboot(self, servername):
        ...
        zkutils.ensure_exists(self.zkclient, z.path.reboot(servername), acl=[
         _SERVERS_ACL_DEL])
    ...
    def check_reboot(self):
        ...
        for name, server in self.servers.items():
            if now > server.valid_until:
                _LOGGER.info('Expired: %s at %s', name, server.valid_until)
            ...
            if now + scheduler.DEFAULT_APP_LEASE > server.valid_until:
                ...
                if now > latest_app_expiry:
                    _LOGGER.info('All app expired: %s at %s', name, server.valid_until)
                    self._schedule_reboot(name)
                    continue
                else:
                    continue
    ...
    def _placement_data(self, app):
        ...
        return {'identity': self.cell.apps[app].identity, 
    ...
    def load_schedule(self):
        ...
        for servername, server in self.cell.members().items():
            placement_node = z.path.placement(servername)
            zkutils.ensure_exists(self.zkclient, placement_node, acl=[
             _SERVERS_ACL])
            ...
            for app in current - correct:
                ...
                zkutils.ensure_deleted(self.zkclient, os.path.join(placement_node, app))

            for app in correct - current:
                _LOGGER.info('Scheduling: %s - %s,%s', servername, app, self.cell.apps[app].identity)
                placement_data = self._placement_data(app)
                zkutils.put(self.zkclient, os.path.join(placement_node, app), placement_data, acl=[
                 _SERVERS_ACL])
    ...
    def reschedule(self):
        ...
        placement = self.cell.schedule()
        changed_placement = [(app, before, exp_before, after, exp_after) for app, before, exp_before, after, exp_after in placement if before != after or exp_before != exp_after]
        for app, before, exp_before, after, exp_after in changed_placement:
            if before and before != after:
                _LOGGER.info('Unscheduling: %s - %s', before, app)
                zkutils.ensure_deleted(self.zkclient, z.path.placement(before, app))
                continue

        for app, before, exp_before, after, exp_after in changed_placement:
            placement_data = self._placement_data(app)
            if after:
                _LOGGER.info('Scheduling: %s - %s,%s, expires at: %s', after, app, self.cell.apps[app].identity, exp_after)
                zkutils.put(self.zkclient, z.path.placement(after, app), placement_data, acl=[
                 _SERVERS_ACL])
    ...
    def _unschedule_evicted(self):
        ...
        for appname, app in self.cell.apps.items():
            if app.schedule_once and app.evicted:
                _LOGGER.info('Removing schedule_once/evicted app: %s', appname)
                zkutils.ensure_deleted(self.zkclient, z.path.scheduled(appname))
                continue

    def _create_task(self, appname):
        """Ensures that task is created for an app."""
        zkutils.ensure_exists(self.zkclient, z.path.task(appname), acl=[
         _SERVERS_ACL])
    ...
    def _update_task(self, appname, server):
        ...
        if self.events_dir:
            if server:
                appevents.post(self.events_dir, traceevents.ScheduledTraceEvent(instanceid=appname, where=server))
            else:
                appevents.post(self.events_dir, traceevents.PendingTraceEvent(instanceid=appname))

    def _abort_task(self, appname, exception):
        """Set task into aborted state in case of scheduling error."""
        if self.events_dir:
            appevents.post(self.events_dir, traceevents.AbortedTraceEvent(instanceid=appname, why=type(exception).__name__))


def create_event(zkclient, priority, event, payload):
    """Places event on the event queue."""
    assert 0 <= priority <= 100
    node_path = z.path.event('%(priority)03d-%(event)s-' % {'priority': priority,  'event': event})
    return os.path.basename(zkutils.put(zkclient, node_path, payload, acl=[_SERVERS_ACL], sequence=True))
...
def create_apps(zkclient, app_id, app, count):
    ...
    for _idx in range(0, count):
        node_path = zkutils.put(zkclient, _app_node(app_id, existing=False), app, sequence=True, acl=[
         acl])
...
def create_server(zkclient, server_id, parent_id):
    ...
    if parent_id:
        if not data:
            ...
            data['parent'] = parent_id
    _LOGGER.info('Creating server node %s with data %r and ACL %r', server_node, data, server_acl)
    if zkutils.put(zkclient, server_node, data, acl=[
     server_acl], check_content=True):
...
def update_server_capacity(zkclient, server_id, memory=None, cpu=None, disk=None):
...
def update_identity_group(zkclient, ident_group_id, count):
    ...
    data = {'count': count}
    if zkutils.put(zkclient, node, data, check_content=True, acl=[
     _SERVERS_ACL]):
