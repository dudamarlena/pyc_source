...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/zkutils.py
# Compiled at: 2017-04-04 01:45:57
# Size of source mod 2**32: 18216 bytes
"""Treadmill ZooKeeper helper functions."""
import sys, os, fnmatch, importlib, logging, pickle, threading, types, kazoo, kazoo.client, kazoo.exceptions, kazoo.security
...
def make_anonymous_acl(perm):
    ...
    if not perm:
        perm = 'r'
    assert _is_valid_perm(perm)
    return kazoo.security.make_acl('world', 'anyone', read='r' in perm, write='w' in perm, create='c' in perm, delete='d' in perm, admin='a' in perm)
...
def make_safe_create(zkclient):
    ...
    def safe_create(self_unused, path, value=b'', acl=None, ephemeral=False, sequence=False, makepath=False):
        """Safe wrapper around kazoo.client.create"""
        return _create(path, value=value, acl=make_default_acl(acl), ephemeral=ephemeral, sequence=sequence, makepath=makepath)
...
def connect(zkurl, idpath=None, listener=None, max_tries=30, timeout=ZK_MAX_CONNECTION_START_TIMEOUT, chroot=None):
    ...
    zkclient = connect_native(zkurl, client_id=client_id, listener=listener, timeout=timeout, max_tries=max_tries, chroot=chroot)
    if idpath:
    ...
    zkclient.ensure_path = types.MethodType(make_safe_ensure_path(zkclient), zkclient)
    zkclient.set_acls = types.MethodType(make_safe_set_acls(zkclient), zkclient)
    return zkclient


def connect_native(zkurl, client_id=None, listener=None, max_tries=30, timeout=ZK_MAX_CONNECTION_START_TIMEOUT, chroot=None):
    """Establish connection with Zk and return KazooClient."""
    ...
    zk_retry = {'delay': 0.2, 
     ...
     'max_tries': max_tries, 
     'ignore_expire': False}
    connargs = {'client_id': client_id, 
     'auth_data': [],  'connection_retry': zk_retry, 
...
class SequenceNodeWatch(object):
    __doc__ = 'Sequential nodes watcher which keeps track of last node seen.'
    ...
    def nodes(self, children):
        ...
        if self.pattern:
            children = [node for node in children if node.startswith(self.pattern)]
        seq_children = [(node[node.rfind(self.delim) + 1:], node) for node in children if node.rfind(self.delim) > 0]
...
def watch_sequence(zkclient, path, func, delim='-', pattern=None, include_data=False):
...
def create(zkclient, path, data=None, acl=None, sequence=False, default_acl=True, ephemeral=False):
    ...
    if default_acl:
        ...
        realacl = acl
    return zkclient.create(path, payload, makepath=True, acl=realacl, sequence=sequence, ephemeral=ephemeral)


def put(zkclient, path, data=None, acl=None, sequence=False, default_acl=True, ephemeral=False, check_content=False):
    ...
    if default_acl:
        realacl = make_default_acl(acl)
        _LOGGER.debug('put (default_acl=%s): %s acl=%s seq=%s', default_acl, path, realacl, sequence)
    else:
        realacl = acl
...
def get_default(zkclient, path, watcher=None, strict=True, need_metadata=False, default=None):
    """Read content of Zookeeper node, return default value if does not exist.
    """
    try:
        return get(zkclient, path, watcher=watcher, strict=strict, need_metadata=need_metadata)
...
def ensure_exists(zkclient, path, acl=None, sequence=False, data=None):
    ...
    try:
        ...
        return zkclient.create(path, newdata, makepath=True, acl=realacl, sequence=sequence)
    except kazoo.client.NodeExistsError:
        if data is not None:
            newdata = _payload(data)
