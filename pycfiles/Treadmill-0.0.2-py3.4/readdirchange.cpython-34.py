...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/syscall/readdirchange.py
# Compiled at: 2017-04-03 02:32:49
# Size of source mod 2**32: 11953 bytes
"""Windows directory watcher API wrapper module
"""
import ctypes.wintypes, os, queue, select, _thread, threading
...
class OVERLAPPED(ctypes.Structure):
    __doc__ = 'Structure for async IO.'
    _fields_ = [
     (
...
class FILE_NOTIFY_INFORMATION(ctypes.Structure):
    __doc__ = 'Info for file notification.'
    _fields_ = [
     (
...
class InotifyEvent(object):
    __doc__ = 'Event class for file change.'
...
class ReadDirChange(object):
    __doc__ = 'ReadDirectoryChangesW system interface.'
    ...
    def add_watch(self, path):
        ...
        self._handle = CreateFileW(self._path, FILE_LIST_DIRECTORY, WATCHDOG_FILE_SHARE_FLAGS, None, OPEN_EXISTING, WATCHDOG_FILE_FLAGS, None)
    ...
    def _read_events_tread_proc(self, *args):
    ...
    def _get_directory_change_events(self):
        ...
        ReadDirectoryChangesW(self._handle, ctypes.byref(event_buffer), len(event_buffer), True, WATCHDOG_FILE_NOTIFY_FLAGS, ctypes.byref(nbytes), None, None)
        events = self._create_events_from_byte_array(event_buffer.raw, int(nbytes.value))
        return [InotifyEvent(action, os.path.join(self._path, path)) for action, path in events]
