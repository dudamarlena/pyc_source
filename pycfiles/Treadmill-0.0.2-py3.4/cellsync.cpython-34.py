...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/sproc/cellsync.py
# Compiled at: 2017-04-03 02:32:49
# Size of source mod 2**32: 3695 bytes
"""Syncronizes cell Zookeeper with LDAP data."""
import importlib, logging, os, time, click
    ...
    for name, entity in zip(names, entities):
        ...
        if match:
            ...
            if not zkname:
                ...
                continue
            if zkutils.put(zkclient, z.join_zookeeper_path(zkpath, zkname), entity, check_content=True):
...
def _run_sync():
    ...
    while True:
        ...
        _sync_collection(context.GLOBAL.zk.conn, app_groups, z.path.appgroup(), match_appgroup)
        ...
        try:
            servers_plugin = importlib.import_module('treadmill.plugins.sproc.servers')
            servers_plugin.init()
        except ImportError as err:
            _LOGGER.warn('Unable to load treadmill.plugins.sproc.servers: %s', err)
