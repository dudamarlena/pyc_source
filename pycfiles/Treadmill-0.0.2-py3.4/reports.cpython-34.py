...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/reports.py
# Compiled at: 2017-04-03 02:32:49
# Size of source mod 2**32: 5262 bytes
"""Handles reports over scheduler data."""
import time, datetime, itertools, logging, numpy as np, pandas as pd
...
def servers(cell):
    ...
    frame = pd.DataFrame.from_dict([_server_row(server) for server in cell.members().values()])
    if frame.empty:
...
def allocations(cell):
    ...
    for label, partition in cell.partitions.items():
        ...
        alloc_df = pd.DataFrame.from_dict([_alloc_row(label, name, alloc) for name, alloc in leaf_allocs]).set_index([
         'label', 'name'])
...
def utilization(prev_utilization, apps_df):
    ...
    if row.empty:
        ...
        row = row.groupby('name').agg({'cpu': np.sum,  'memory': np.sum, 
         'disk': np.sum, 
         'count': np.sum, 
         'util': np.max})
        row = row.stack()
