...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/services/_base_service.py
# Compiled at: 2017-04-03 02:32:49
# Size of source mod 2**32: 27928 bytes
"""Base for all node resource services."""
import abc, collections, contextlib, errno, functools, glob, importlib, logging, os, select, shutil, socket, struct, tempfile, time, yaml
from .. import exc
from .. import fs
...
class ResourceServiceError(exc.TreadmillError):
    __doc__ = 'Base Resource Service error.\n    '
    __slots__ = ()
...
class ResourceServiceRequestError(ResourceServiceError):
    __doc__ = 'Resource Service Request error.\n    '
    __slots__ = 'request'
...
class ResourceServiceTimeoutError(ResourceServiceError, socket.timeout):
    __doc__ = 'Resource Service timeout.\n    '
    __slots__ = ()
...
class ResourceServiceClient(object):
    __doc__ = 'Client class for all Treadmill services.\n\n    /apps/<container>/rsrc/req-<svc_name>/\n        request.yml\n        reply.yml\n        svc_req_id\n    '
    _REQ_UID_FILE = 'svc_req_id'
    __slots__ = ('_serviceinst', '_clientdir')
    ...
    def create(self, rsrc_id, rsrc_data):
        ...
        with open(os.path.join(req_dir, _REQ_FILE), 'w') as (f):
            os.fchmod(f.fileno(), 420)
            yaml.dump(rsrc_data, explicit_start=True, explicit_end=True, default_flow_style=False, stream=f)
    ...
    def wait(self, rsrc_id, timeout=None):
        ...
        if not _wait_for_file(rep_file, timeout):
            raise ResourceServiceTimeoutError('Resource %r not available in time' % rsrc_id)
        try:
        ...
        if isinstance(reply, dict):
            if '_error' in reply:
                raise ResourceServiceRequestError(reply['_error']['why'], reply['_error']['input'])
        return reply
    ...
    def _req_dirname(self, rsrc_id):
        ...
        :param `str` rsrc_id:
            Unique identifier for the requested resource.
        """
        req_dir_name = 'req-{name}-{rsrc_id}'.format(name=self._serviceinst.name, rsrc_id=rsrc_id)
    ...
    def _bck_dirname(self, req_uuid):
        ...
        """
        bck_dir_name = 'bck{ts}-{name}-{req_uuid}'.format(name=self._serviceinst.name, req_uuid=req_uuid, ts=int(time.time()))
        bck_dir = os.path.join(self._clientdir, bck_dir_name)
        return bck_dir


class ResourceService(object):
    __doc__ = 'Server class for all Treadmill services.\n\n    /service_dir/resources/<containerid>-<uid>/ ->\n        /apps/<containerid>/rsrc/req-<svc_name>/\n\n    /apps/<container>/rsrc/<svc_name>/\n        request.yml\n        reply.yml\n        svc_req_id\n\n    '
    __slots__ = ('_is_dead', '_dir', '_rsrc_dir', '_service_impl', '_service_class',
                 '_service_name', '_io_eventfd')
    ...
    def status(self, timeout=30):
        ...
        while backoff <= timeout / 2:
            with contextlib.closing(socket.socket(socket.AF_UNIX, type=socket.SOCK_STREAM, proto=0)) as (status_socket):
                try:
            ...
            if status is not None:
                break
            _LOGGER.info('Waiting for service %r to become available', self.name)
            backoff += backoff or 1
            time.sleep(backoff)
        else:
            raise ResourceServiceTimeoutError('Service %r timed out' % self.name)
    ...
    def run(self, watchdogs_dir, *impl_args, **impl_kwargs):
        ...
        watchdog_lease = watchdogs.create(name='svc-{svc_name}'.format(svc_name=self.name), timeout='{hb:d}s'.format(hb=impl.WATCHDOG_HEARTBEAT_SEC), content='Service %r failed' % self.name)
        ...
        base_event_handlers = [
         (
          self._io_eventfd,
          select.POLLIN,
          functools.partial(self._handle_queued_io_events, watcher=watcher, impl=impl)),
         (
          watcher.inotify,
          select.POLLIN,
          functools.partial(self._handle_io_events, watcher=watcher, impl=impl)),
         (
          ss,
          select.POLLIN,
          functools.partial(self._publish_status, status_socket=ss, status_info=status_info))]
        impl_event_handlers = impl.event_handlers()
        self._update_poll_registration(loop_poll, loop_callbacks, base_event_handlers + impl_event_handlers)
        loop_timeout = impl.WATCHDOG_HEARTBEAT_SEC / 2
        while not self._is_dead:
            updated = self._run_events(loop_poll, loop_timeout, loop_callbacks)
            if updated:
                status_info.clear()
                status_info.update(impl.report_status())
                impl_event_handlers = impl.event_handlers()
    ...
    def _publish_status(self, status_socket, status_info):
        ...
        with contextlib.closing(status_socket.accept()[0]) as (clt):
            ...
            try:
                yaml.dump(status_info, explicit_start=True, explicit_end=True, default_flow_style=False, stream=clt_stream)
                clt_stream.flush()
            except socket.error as err:
    ...
    def _run_events(loop_poll, loop_timeout, loop_callbacks):
        ...
        try:
            for fd, _event in loop_poll.poll(loop_timeout * 1000):
                ...
                pending_callbacks.append(fd_data['callback'])

        except select.error as err:
            if err[0] != errno.EINTR:
                raise

        results = [callback() for callback in pending_callbacks]
        return any(results)
    ...
    def _update_poll_registration(poll, poll_callbacks, handlers):
        ...
        handlers = [(_normalize_fd(fd), events, callback) for fd, events, callback in handlers]
    ...
    def _load_impl(self):
        ...
        assert issubclass(impl_class, BaseResourceServiceImpl), 'Invalid implementation %r' % impl_class
        return impl_class
    ...
    def clt_new_request(self, req_id, req_data_dir):
        ...
        _LOGGER.info('Registering %r: %r -> %r', req_id, svc_req_lnk, req_data_dir)
        tmpsymlink = tempfile.mktemp(dir=self._rsrc_dir, prefix='.tmp' + req_id)
    ...
    def _create_status_socket(self):
        ...
        status_socket = socket.socket(family=socket.AF_UNIX, type=socket.SOCK_STREAM, proto=0)
        status_socket.bind(self.status_sock)
        os.chmod(self.status_sock, 438)
    ...
    def _handle_io_events(self, watcher, impl, resume=False):
        ...
        :returns ``bool``:
            ``True`` if any of the event handlers returns ``True``.
        """
        io_res = watcher.process_events(max_events=impl.MAX_REQUEST_PER_CYCLE, resume=resume)
        if io_res:
            if io_res[(-1)][0] == idirwatch.DirWatcherEvent.MORE_PENDING:
                _LOGGER.debug('More requests events pending')
                os.write(self._io_eventfd, self._IO_EVENT_PENDING)
        return any([callback_res for _, _, callback_res in io_res])
    ...
    def _on_created(self, impl, filepath):
        ...
        try:
            ...
            if err.errno == errno.ENOENT or err.errno == errno.ENOTDIR:
        ...
        try:
            ...
            _LOGGER.exception('Unable to process request: %r %r:', req_id, req_data)
            res = {'_error': {'input': req_data,  'why': str(err)}}

        if res is None:
            return False
        _LOGGER.debug('created %r', req_id)
        with tempfile.NamedTemporaryFile(dir=filepath, delete=False, mode='w') as (f):
            os.fchmod(f.fileno(), 420)
            yaml.dump(res, explicit_start=True, explicit_end=True, default_flow_style=False, stream=f)
...
class BaseResourceServiceImpl(object, metaclass=abc.ABCMeta):
    __doc__ = 'Base interface of Resource Service implementations.\n    '
    __slots__ = ('_service_dir', '_service_rsrc_dir')
