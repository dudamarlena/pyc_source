...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/rest/api/allocation.py
# Compiled at: 2017-04-03 02:32:49
# Size of source mod 2**32: 7378 bytes
"""
...
def init(api, cors, impl):
    ...
    namespace = api.namespace('allocation', description='Allocation REST operations')
    assignment = api.model('Assignment', {'priority': fields.Integer(description='Priority'), 
     'pattern': fields.String(description='App pattern')})
    reservation = api.model('Reservation', {'_id': fields.String(description='Name'), 
    ...
    allocation_model = {'_id': fields.String(description='Name'), 
     'environment': fields.String(description='App environment', enum=[
                     'dev', 'qa', 'uat', 'prod'], required=True), 
     'reservations': fields.List(fields.Nested(reservation))}
    request_model = api.model('ReqAllocation', allocation_model)
    response_model = api.model('RespAllocation', allocation_model)

    @namespace.route('/')
    class _AllocationsList(restplus.Resource):
        __doc__ = 'Treadmill Allocation resource'
    ...
    class _AllocationResource(restplus.Resource):
        ...
        def post(self, tenant_id, alloc_id):
            ...
            return impl.create(_alloc_id(tenant_id, alloc_id), flask.request.json)
    ...
    class _ReservationResource(restplus.Resource):
        __doc__ = 'Treadmill cell allocation resource'
        ...
        def post(self, tenant_id, alloc_id, cell):
            ...
            return impl.reservation.create('/'.join([tenant_id, alloc_id, cell]), flask.request.json)
        ...
        def put(self, tenant_id, alloc_id, cell):
            ...
            return impl.reservation.update('/'.join([tenant_id, alloc_id, cell]), flask.request.json)
    ...
    class _AssignmentResource(restplus.Resource):
        ...
        def get(self, tenant_id, alloc_id, cell, pattern):
            """Returns the details of the reservation."""
            return impl.assignment.get('/'.join([tenant_id, alloc_id, cell, pattern]))

        @webutils.post_api(api, cors, req_model=request_model, resp_model=response_model)
        def post(self, tenant_id, alloc_id, cell, pattern):
            """Creates a Treadmill reservation in the cell."""
            return impl.assignment.create('/'.join([tenant_id, alloc_id, cell, pattern]), flask.request.json)

        @webutils.put_api(api, cors, req_model=request_model, resp_model=response_model)
        def put(self, tenant_id, alloc_id, cell, pattern):
            """Updates Treadmill allocation assignment."""
            return impl.assignment.update('/'.join([tenant_id, alloc_id, cell, pattern]), flask.request.json)

        @webutils.delete_api(api, cors)
        def delete(self, tenant_id, alloc_id, cell, pattern):
            """Deletes Treadmill allocation assignment."""
            return impl.assignment.delete('/'.join([tenant_id, alloc_id, cell, pattern]))