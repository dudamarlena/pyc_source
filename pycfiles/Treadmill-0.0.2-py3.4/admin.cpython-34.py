...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/admin.py
# Compiled at: 2017-04-11 07:05:24
# Size of source mod 2**32: 46610 bytes
"""Low level admin API to manipulate global cell topology."""
import sys, collections, copy, hashlib, itertools, logging, shlex
from distutils import util
import ldap3, jinja2, treadmill.ldap3kerberos
sys.modules['ldap3.protocol.sasl.kerberos'] = treadmill.ldap3kerberos
_LOGGER = logging.getLogger(__name__)
_SYNTAX_2_TYPE = {'1.3.6.1.4.1.1466.115.121.1.7': 'bool', 
...
_TYPE_2_EQUALITY = {('str', False): 'caseExactMatch', 
 ('str', True): 'caseIgnoreMatch', 
 ('dn', False): 'distinguishedNameMatch', 
 ('int', False): 'integerMatch'}
_TYPE_2_ORDERING = {('int', None): 'integerOrderingMatch'}
_TYPE_2_SUBSTR = {('str', False): 'caseExactSubstringsMatch', 
...
def _entry_2_dict(entry, schema):
    ...
    return {k:v for k, v in obj.items() if v is not None if v is not None}
        ...
        if value is None:
            ...
            if elem_type == str:
                elem_type = (
                 str, str)
            if value:
                filtered = [str(v) for v in value if isinstance(v, elem_type)]
                if len(filtered) < len(value):
                    _LOGGER.critical('Exptected %r, got %r', field_type, value)
...
def _abstract_2_attrtype(name, abstract):
    ...
    type_case = (
     abstract['type'], bool(abstract.get('ignore_case')))
...
def _attrtype_2_str(attr):
    ...
    template = "( {{ oid_pfx }}{{ item.idx }} NAME '{{ item.name }}' DESC '{{ item.desc }}' SYNTAX {{ item.syntax }} {% if item.equality -%} EQUALITY {{ item.equality }} {% endif -%} {% if item.substr -%} SUBSTR {{ item.substr }} {% endif -%} {% if item.ordering -%} ORDERING {{ item.ordering }} {% endif -%} {% if item.single_value -%} SINGLE-VALUE {% endif -%} )"
    return str(jinja2.Template(template).render(item=attr, oid_pfx=_TREADMILL_ATTR_OID_PREFIX))


def _objcls_2_abstract(obj_cls):
    """Converts object class to abstract."""
    name = obj_cls['name']
    abstract = {'must': obj_cls.get('must', []), 
     'may': obj_cls.get('may', []), 
...
def _objcls_2_str(name, obj_cls):
    ...
    template = "( {{ oid_pfx }}{{ item.idx }} NAME '{{ name }}' DESC '{{ item.desc }}' SUP top STRUCTURAL MUST ( {{ item.must | join(' $ ') }} ) {% if item.may -%} MAY  ( {{ item.may | join(' $ ') }} ) {% endif -%} )"
    return str(jinja2.Template(template).render(name=name, item=obj_cls, oid_pfx=_TREADMILL_OBJCLS_OID_PREFIX))


def _group_entry_by_opt(entry):
    """Group by attr;option."""
    attrs_with_opt = [tuple(k.split(';') + [entry[k]]) for k in entry.keys() if k.find(';') > 0]
    attrs_with_opt.sort(key=lambda x: x[1])
    return {key:list(group)[0::1] for key, group in itertools.groupby(attrs_with_opt, lambda x: x[1])}
...
def _grouped_to_list_of_dict(grouped, prefix, schema):
    ...
    filtered = {k:v for k, v in grouped.items() if k.startswith(prefix)}
...
class Admin(object):
    __doc__ = 'Manages Treadmill objects in ldap.'
    ...
    def connect(self):
        ...
        for uri in self.uri:
            try:
                ...
                self.ldap = ldap3.Connection(server, authentication=ldap3.SASL, sasl_mechanism='GSSAPI', client_strategy=ldap3.STRATEGY_SYNC_RESTARTABLE, auto_bind=True)
    ...
    def search(self, search_base, search_filter, search_scope=ldap3.SUBTREE, attributes=None):
        """Call ldap search and return a list of dn, entry tuples."""
        self.ldap.search(search_base=search_base, search_filter=search_filter, search_scope=search_scope, attributes=attributes, dereference_aliases=ldap3.DEREF_NEVER)
        self._test_raise_exceptions()
        if not self.ldap.response:
            return
        for entry in self.ldap.response:
            yield (
             str(entry['dn']), _dict_normalize(entry['raw_attributes']))
    ...
    def _test_raise_exceptions(self):
        ...
        if exception_type:
            raise exception_type(result=self.ldap.result['result'], description=self.ldap.result['description'], dn=self.ldap.result['dn'], message=self.ldap.result['message'], response_type=self.ldap.result['type'])
    ...
    def schema(self, abstract=True):
        """Get schema."""
        result = self.search(search_base='cn=schema,cn=config', search_filter='(cn={*}treadmill)', search_scope=ldap3.LEVEL, attributes=[
         'olcAttributeTypes',
         'olcObjectClasses'])
        if not result:
            return
        ...
        for attr_type_s in entry.get('olcAttributeTypes', []):
            ...
            while attr_type_l:
                ...
                if token == 'SYNTAX':
                    ...
                    attr['obsolete'] = True
                elif not 'Unsupported token: %s: %s' % (attr_type_s, token):
                        ...
                        if token == 'SUP':
                            ...
                            if token == 'STRUCTURAL':
                                ...
                                if token == 'MUST':
                                    ...
                                    elif not 'Invalid token: %s, %r' % (obj_cls_s, token):
                                        raise AssertionError
        ...
        return {'dn': schema_dn,  'attributeTypes': attr_types, 
    ...
    def _schema_attrtype_diff(old_attr_types, new_attr_types):
        ...
        for name in common:
            ...
            for key in ['type', 'ignore_case']:
                if new_attr.get(key) != old_attr.get(key):
                    if not 'Type modified in place: %s' % name:
                        raise AssertionError
                    continue
        ...
        if not old_attr_types:
            ...
            next_oid = max([attr['idx'] for attr in old_attr_types.values()]) + 1
        for name in added:
    ...
    def _schema_objcls_diff(old_ocs, new_ocs):
        ...
        if not old_ocs:
            ...
            next_oid = max([item['idx'] for item in old_ocs.values()]) + 1
        for name in added:
            objcls = new_ocs[name]
            objcls['idx'] = next_oid
            next_oid += 1
    ...
    def update_schema(self, new_schema):
        ...
        to_del, to_add = self._schema_attrtype_diff(old_attr_types, new_attr_types)
        if to_del:
            values = [_attrtype_2_str(_abstract_2_attrtype(name, attr)) for name, attr in to_del.items()]
            _LOGGER.debug('del: %s - olcAttributeTypes: %r', schema_dn, values)
            changes['olcAttributeTypes'].extend([
             (
              ldap3.MODIFY_DELETE, values)])
        if to_add:
            values = [_attrtype_2_str(_abstract_2_attrtype(name, attr)) for name, attr in to_add.items()]
        ...
        to_del, to_add = self._schema_objcls_diff(old_obj_classes, new_obj_classes)
        if to_del:
            values = [_objcls_2_str(name, item) for name, item in to_del.items()]
            _LOGGER.debug('del: %s - olcObjectClasses: %r', schema_dn, values)
            changes['olcObjectClasses'].extend([(ldap3.MODIFY_DELETE, values)])
        if to_add:
            values = [_objcls_2_str(name, item) for name, item in to_add.items()]
    ...
    def init(self, domain):
        ...
        def _build_ou(ou, name=None):
            ...
            return (
             'ou=' + ou + ',' + treadmill_ou,
             [
              'organizationalUnit'], {'ou': name or ou})
    ...
    def get(self, dn, query, attrs):
        ...
        result = self.search(search_base=dn, search_filter=str(query), search_scope=ldap3.BASE, attributes=attrs)
...
class LdapObject(object):
    __doc__ = 'Ldap object base class.'
    ...
    def get(self, ident):
        ...
        entry = self.admin.get(self.dn(ident), self._query(), self.attrs())
        if entry:
    ...
    def create(self, ident, attrs):
        ...
        if isinstance(ident, list):
            ...
            ident_attr = [
             str(ident)]
        entry.update({'objectClass': [self.oc()],  self.entity(): ident_attr})
    ...
    def list(self, attrs):
        ...
        result = self.admin.search(search_base=self.dn(), search_filter=query.to_str(), search_scope=ldap3.SUBTREE, attributes=self.attrs())
    ...
    def children(self, ident, clazz):
        ...
        search = self.admin.search(search_base=dn.decode(), search_filter='(objectclass=%s)' % clazz.oc(), attributes=attrs)
        return [children_admin.from_entry(entry, _dn) for _dn, entry in search]


class Server(LdapObject):
    __doc__ = 'Server object.'
    _schema = [
     (
      'server', '_id', str),
     (
...
class AppGroup(LdapObject):
    ...
    def get(self, ident, group_type=None):
        ...
        if not entries:
            raise ldap3.LDAPNoSuchObjectResult('No entries for {0} and group-type {1}'.format(ident, group_type))
        return entries[0]
...
class Application(LdapObject):
    ...
    def schema():
        ...
        name_only = lambda schema_rec: (
         schema_rec[0], None, None)
        return sum([list(map(name_only, Application._svc_schema)),
         ...
         list(map(name_only, Application._affinity_schema))], Application._schema)
    ...
    def from_entry(self, entry, dn=None):
        ...
        services = _grouped_to_list_of_dict(grouped, 'tm-service-', Application._svc_schema)
        service_restarts = _grouped_to_list_of_dict(grouped, 'tm-service-', Application._svc_restart_schema)
        endpoints = _grouped_to_list_of_dict(grouped, 'tm-endpoint-', Application._endpoint_schema)
        environ = _grouped_to_list_of_dict(grouped, 'tm-envvar-', Application._environ_schema)
        affinity_limits = _grouped_to_list_of_dict(grouped, 'tm-affinity-', Application._affinity_schema)
        for service in services:
            for service_restart in service_restarts:
                if service_restart['name'] == service['name']:
                    service['restart'] = {'limit': service_restart['limit'],  'interval': service_restart['interval']}
                    continue

        affinity_limits = {affinity['level']:affinity['limit'] for affinity in affinity_limits}
        obj.update({'services': services, 
         'endpoints': endpoints, 
         'environ': environ, 
    ...
    def to_entry(self, obj):
        ...
        for service in obj.get('services', []):
            service_entry = _dict_2_entry(service, Application._svc_schema, 'tm-service', service['name'])
            service_restart = self._default_svc_restart.copy()
            service_restart.update(service.get('restart', {}))
            service_entry.update(_dict_2_entry(service_restart, Application._svc_restart_schema, 'tm-service', service['name']))
            entry.update(service_entry)

        for endpoint in obj.get('endpoints', []):
            endpoint_entry = _dict_2_entry(endpoint, Application._endpoint_schema, 'tm-endpoint', endpoint['name'])
            entry.update(endpoint_entry)

        for envvar in obj.get('environ', []):
            environ_entry = _dict_2_entry(envvar, Application._environ_schema, 'tm-envvar', envvar['name'])
            entry.update(environ_entry)

        aff_lim = [{'level': aff,  'limit': obj['affinity_limits'][aff]} for aff in obj.get('affinity_limits', {})]
        for limit in aff_lim:
            aff_lim_entry = _dict_2_entry(limit, Application._affinity_schema, 'tm-affinity', limit['level'])
...
class Cell(LdapObject):
    ...
    def schema():
        ...
        name_only = lambda schema_rec: (
         schema_rec[0], None, None)
        return Cell._schema + list(map(name_only, Cell._master_host_schema))

    def from_entry(self, entry, dn=None):
        """Converts LDAP app object to dict."""
        obj = super(Cell, self).from_entry(entry, dn)
        grouped = _group_entry_by_opt(entry)
        masters = _grouped_to_list_of_dict(grouped, 'tm-master-', Cell._master_host_schema)
        obj.update({'masters': masters})
        return obj
    ...
    def to_entry(self, obj):
        ...
        for master in obj.get('masters', []):
            master_entry = _dict_2_entry(master, Cell._master_host_schema, 'tm-master', master['idx'])
...
class Tenant(LdapObject):
    ...
    def dn(self, ident=None):
        ...
        parts = ['%s=%s' % (self.entity(), part) for part in reversed(ident.split(':'))]
...
def _allocation_dn_parts(ident):
    ...
    parts.extend(['%s=%s' % (Tenant.entity(), part) for part in reversed(tenant_id.split(':'))])
...
def _dn2cellalloc_id(dn):
    ...
    tenants = ':'.join(reversed([part.split('=')[1] for part in parts if part.startswith('tenant=')]))
    return '%s/%s/%s' % (tenants, allocation, cell)


class CellAllocation(LdapObject):
    __doc__ = 'Models allocation reservation in a given cell.'
    _schema = [
     (
      'cell', 'cell', str),
     (
    ...
    def schema():
        ...
        name_only = lambda schema_rec: (
         schema_rec[0], None, None)
        return CellAllocation._schema + list(map(name_only, CellAllocation._assign_schema))
    ...
    def from_entry(self, entry, dn=None):
        ...
        if ident:
            obj['_id'] = ident
        grouped = _group_entry_by_opt(entry)
        assignments = _grouped_to_list_of_dict(grouped, 'tm-alloc-assignment-', CellAllocation._assign_schema)
        obj.update({'assignments': assignments})
        return obj
    ...
    def to_entry(self, obj):
        ...
        for assignment in obj.get('assignments', []):
            assign_entry = _dict_2_entry(assignment, CellAllocation._assign_schema, 'tm-alloc-assignment', assignment['pattern'])
...
class Allocation(LdapObject):
    ...
    def delete(self, ident):
        """Deletes LDAP record."""
        dn = self.dn(ident)
        cell_allocs_search = self.admin.search(search_base=dn, search_filter='(objectclass=tmCellAllocation)', attributes=[])
