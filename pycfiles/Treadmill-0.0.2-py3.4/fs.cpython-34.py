...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/fs.py
# Compiled at: 2017-04-03 02:32:49
# Size of source mod 2**32: 13902 bytes
"""Implements functions for setting up container chroot and file system."""
import errno, importlib, logging, os, stat, tarfile, tempfile, io
from . import exc
from . import utils
from . import subproc
if os.name != 'nt':
...
def make_rootfs(newroot, proid):
    ...
    for directory in stickydirs:
        os.chmod(newroot_norm + directory, 511 | stat.S_ISVTX)
...
def archive_filesystem(block_dev, rootdir, archive, files):
    ...
    if not os.path.exists(rootdir):
        ...
        return False
    archive_cmd = '{tm_root}/sbin/archive_container.sh'.format(tm_root=utils.rootdir())
    arguments = [
     'unshare',
     '--mount',
     archive_cmd,
...
def tar(target, sources, compression=None):
    ...
    if not compression is None:
        assert compression in ('gzip', 'bzip2')
        if not isinstance(target, str):
            assert isinstance(target, io.IOBase)
            if compression == 'gzip':
                mode = 'gz'
                ext = '.gz'
            else:
