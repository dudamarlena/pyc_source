...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/tickets.py
# Compiled at: 2017-04-03 02:32:49
# Size of source mod 2**32: 11763 bytes
"""Handles ticket forwarding from the ticket master to the node."""
import pwd, base64, hashlib, logging, os, random, shutil, stat, subprocess, tempfile
...
class Ticket(object):
    ...
    def write(self, path=None):
        ...
        if path is None:
            path = self.tkt_path
        try:
            with tempfile.NamedTemporaryFile(dir=os.path.dirname(path), prefix='.tmp' + self.princ, delete=False, mode='w') as (tkt_file):
    ...
    def copy(self, dst, src=None):
        ...
        try:
            with open(src, 'rb') as (tkt_src_file):
                with tempfile.NamedTemporaryFile(dir=dst_dir, prefix='.tmp' + self.princ, delete=False, mode='w') as (tkt_dst_file):
                    ...
                    os.fchmod(tkt_dst_file.fileno(), stat.S_IMODE(src_stat.st_mode))
                    tkt_dst_file.flush()
    ...
    def renew(self):
        ...
        try:
            ...
            subproc.check_call([
             'kinit', '-R'], environ={'KRB5CCNAME': 'FILE:' + self.tkt_path}, runas=ticket_owner)
            _LOGGER.info('Tickets renewed successfully.')
        except subprocess.CalledProcessError as err:
            _LOGGER.info('Tickets not renewable, kinit rc: %s', err.returncode)
...
class TicketLocker(object):
    __doc__ = 'Manages ticket exchange between ticket locker and the container.'
...
def run_server(locker):
        ...
        def got_line(self, line):
            ...
            if tkts:
                for princ, encoded in tkts.items():
                    if encoded:
                        _LOGGER.info('Sending ticket: %s:%s', princ, hashlib.sha1(encoded).hexdigest())
    ...
    class TicketLockerServerFactory(protocol.Factory):
        __doc__ = 'TicketLockerServer factory.'
            ...
            try:
                if client.connect():
                    ...
                    while True:
                        ...
                        if encoded:
                            _LOGGER.info('got ticket %s:%s', princ, hashlib.sha1(encoded.encode()).hexdigest())
                            ticket = Ticket(princ, base64.urlsafe_b64decode(encoded))
                    ...
                    _LOGGER.warn('Cannot connect to %s:%s, %s', host, port, service)
            except Exception:
...
def store_tickets(reply, tkt_spool_dir):
    ...
    for tkt in reply:
        ...
        if realm_sep != -1:
            _LOGGER.info('Creating link: %s => %s', tkt_spool_link, os.path.basename(tkt_spool_path))
