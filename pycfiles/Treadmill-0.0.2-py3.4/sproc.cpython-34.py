...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/cli/sproc.py
# Compiled at: 2017-04-03 02:32:49
# Size of source mod 2**32: 3060 bytes
"""Implementation of treadmill-admin CLI plugin."""
import logging, logging.config, os, tempfile, traceback, click, yaml, treadmill
...
def init():
    ...
    @click.option('--zookeeper', required=False, envvar='TREADMILL_ZOOKEEPER', callback=cli.handle_context_opt, expose_value=False)
    @click.pass_context
    def run(ctx, cgroup):
        """Run system processes"""
        log_conf_file = os.path.join(treadmill.TREADMILL, 'etc', 'logging', 'daemon.yml')
        try:
