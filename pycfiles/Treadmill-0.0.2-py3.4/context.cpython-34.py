...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/context.py
# Compiled at: 2017-04-03 02:32:49
# Size of source mod 2**32: 8045 bytes
"""Treadmill context."""
import importlib, logging, ldap3
...
class ContextError(Exception):
    __doc__ = 'Raised when unable to connect to LDAP or Zookeeper.'


class AdminContext(object):
    __doc__ = 'Ldap context.'
    __slots__ = ('search_base', '_url', '_conn', '_resolve')
    ...
    def conn(self):
        ...
        if self._conn is None:
            ...
            if self.url is None:
                ...
                _LOGGER.debug('Connecting to LDAP %s, %s', self.url, self.search_base)
...
class ZkContext(object):
    __doc__ = 'Zookeeper context.'
    __slots__ = ('url', 'proid', '_conn', '_resolve')
...
class Context(object):
    __doc__ = 'Global connection context.'
    __slots__ = ('cell', 'ldap', 'zk', 'dns_domain', 'admin_api_scope', 'ctx_plugin',
                 'resolvers')
    ...
    def __init__(self):
        ...
        try:
            self.ctx_plugin = importlib.import_module('treadmill.plugins.context')
    ...
    def _resolve_srv(self, srv_rec):
        ...
        if result:
            return [dnsutils.srv_target_to_url(srv_rec, target) for target in result]
    ...
    def _resolve_cell_from_ldap(self, cellname):
        ...
        try:
            ...
            zk_hostports = ['%s:%s' % (master['hostname'], master['zk-client-port']) for master in cell['masters']]
