...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/netdev.py
# Compiled at: 2017-04-03 02:32:49
# Size of source mod 2**32: 10746 bytes
"""Network device management."""
import errno, os, logging, enum
from . import subproc
...
def dev_conf_route_localnet_set(eth, enabled):
    ...
    """
    _proc_sys_write(_PROC_CONF_ROUTE_LOCALNET.format(dev=eth), int(enabled))
...
def dev_conf_proxy_arp_set(eth, enabled):
    ...
    :param ``bool`` enabled:
        Enable or disable the feature.
    """
    _proc_sys_write(_PROC_CONF_PROXY_ARP.format(dev=eth), int(enabled))
...
def dev_conf_forwarding_set(eth, enabled):
    ...
    :param ``bool`` enabled:
        Enable or disable the feature.
    """
    _proc_sys_write(_PROC_CONF_FORWARDING.format(dev=eth), int(enabled))
...
def dev_conf_arp_ignore_set(eth, value):
    ...
    """
    _proc_sys_write(_PROC_CONF_ARP_IGNORE.format(dev=eth), int(value))
