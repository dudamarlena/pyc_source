...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/metrics.py
...
_SYSPROCS = [
 's6-*', 'treadmill_disc*', 'pid1', 'app_tickets', 'app_presence',
 'app_endpoint*']
...
def read_blkio_stats(cgrp, major_minor):
    ...
    blk_bps_info = raw_blk_bps_info.get(major_minor, {'Read': 0,  'Write': 0})
    blk_iops_info = raw_blk_iops_info.get(major_minor, {'Read': 0,  'Write': 0})
    return {'read_iops': blk_iops_info['Read'], 
...
def read_cpu_stats(cgrp):
    ...
    usage_ratio = cpu_usage * total_bogomips / (delta * cpu_shares) / cpu_count
    usage = cpu_usage * total_bogomips / (delta * sysinfo.BMIPS_PER_CPU) / cpu_count * 100
    return (
