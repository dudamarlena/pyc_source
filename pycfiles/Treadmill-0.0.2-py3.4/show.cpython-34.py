...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/cli/show.py
# Compiled at: 2017-04-03 02:32:49
# Size of source mod 2**32: 4615 bytes
"""Manage Treadmill app manifest."""
import logging, urllib.request, urllib.parse, urllib.error, click
from .. import cli
...
def _show_list(apis, match, states):
    ...
    if match:
        url += '?' + urllib.parse.urlencode([('match', match)])
    response = restclient.get(apis, url)
    names = [item['name'] for item in response.json() if item['state'] in states]
    for name in names:
        print(name)
...
def _show_endpoints(apis, pattern, endpoint, proto):
    ...
    if endpoint:
        ...
        url += '/' + endpoint
    response = restclient.get(apis, url)
    endpoints = [{'name': end['name'],  'proto': end['proto'],  'endpoint': end['endpoint'],  'hostport': '{0}:{1}'.format(end['host'], end['port'])} for end in response.json()]
