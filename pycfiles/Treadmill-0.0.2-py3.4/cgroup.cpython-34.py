...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/sproc/cgroup.py
# Compiled at: 2017-04-03 02:32:49
# Size of source mod 2**32: 7770 bytes
"""Manage core level cgroups."""
import errno, logging, os, subprocess, click
...
def init():
    ...
    def cginit(mem, mem_core, cpu):
        ...
        _LOGGER.info('Configuring CPU limits: total: %d, treadmill: %d, system: %d, treadmill core: %d, treadmill apps: %d', total_cpu_shares, tm_cpu_shares, system_cpu_shares, tm_core_cpu_shares, tm_apps_cpu_shares)
        ...
        if tm_mem <= 0:
            ...
            real_tm_mem = tm_mem
        _LOGGER.info('Configuring memory limits: total: %s, treadmill total: %s, treadmill core: %s', utils.bytes_to_readable(total_physical_mem, 'B'), utils.bytes_to_readable(real_tm_mem, 'B'), utils.bytes_to_readable(tm_core_mem, 'B'))
        cgutils.create_treadmill_cgroups(system_cpu_shares, tm_cpu_shares, tm_core_cpu_shares, tm_apps_cpu_shares, real_tm_mem, tm_core_mem)
    ...
    @click.option('-d', '--delete', is_flag=True, default=False, help='Delete directories.')
    @click.option('-a', '--apps', is_flag=True, default=False, help='Clean apps group.')
    @click.option('-c', '--core', is_flag=True, default=False, help='Clean core group.')
    def cgcleanup(delete, apps, core):
        """Kill stale apps in cgroups."""
        subsystems = [
         'cpu', 'cpuacct', 'memory', 'blkio']
        cgrps = []
        if core:
    ...
    def cgmount():
        ...
        wanted_cgroups = {'memory': '/cgroup/memory', 
         'cpu': '/cgroup/cpu', 
        ...
        for cgname, cgmounts in mounted_cgroups.items():
            ...
            if cgname in unwanted_cgroups:
                ...
                _LOGGER.warning('Cgroup %r has multiple mount points: %r', cgname, cgmounts)
                continue
