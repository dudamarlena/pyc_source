...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/versionmgr.py
# Compiled at: 2017-04-03 02:32:49
# Size of source mod 2**32: 4971 bytes
"""Helper tools to manage Treadmill versions."""
import hashlib, logging, os, kazoo
from . import zkutils
    ...
    for root, dirs, files in os.walk(path, topdown=True, followlinks=False):
        ...
        for filename in sorted(files):
            ...
            if os.path.islink(fullpath):
                checksum.update('{src} -> {dst}'.format(src=fullpath[path_len:], dst=os.readlink(fullpath)))
            else:
                stat = os.stat(fullpath)
                checksum.update('{name!r} {mode!r} {size!r} {mtime!r} {ctime!r}'.format(name=fullpath[path_len:], mode=stat.st_mode, size=stat.st_size, mtime=stat.st_mtime, ctime=stat.st_ctime))
...
def upgrade(zkclient, expected, servers, batch_size, timeout, stop_on_error=False, force_upgrade=False):
    ...
    def _upgrade_ok(server):
        ...
        version_path = z.path.version(server)
        exists = zkutils.exists(zkclient, version_path, timeout)
        if exists:
            version_info = zkutils.get(zkclient, version_path)
            version_ok = version_info.get('digest') == expected
            _LOGGER.info('Version check: %s - %s', server, 'ok.' if version_ok else 'fail.')
            return version_ok
        else:
