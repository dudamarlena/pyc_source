...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/cli/admin/ok.py
# Compiled at: 2017-03-22 02:19:40
# Size of source mod 2**32: 4833 bytes
"""Zookeeper status interface."""
import socket, logging, click, kazoo, websocket as ws_client
...
def _check_api(apis):
    ...
    success = True
    if len(apis) < 2:
        _LOGGER.error('API is under capacity: expected 2, running: %s', len(apis))
        success = False
    for api in apis:
        try:
...
def check_capacity():
    ...
    _LOGGER.info('Server capacity - total: %s, blacked-out: %s, up: %s', configured, blacked_out, present)
    check_blackouts()
