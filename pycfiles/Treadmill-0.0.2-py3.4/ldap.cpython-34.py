...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/cli/admin/ldap.py
# Compiled at: 2017-04-03 02:32:49
# Size of source mod 2**32: 25924 bytes
"""Implementation of treadmill admin ldap CLI plugin."""
import logging, click, ldap3, yaml
...
def server_group(parent):
    ...
    @click.option('-t', '--traits', help='List of server traits', multiple=True, default=[])
    ...
    def _list(cell, traits, label):
        """List servers"""
        admin_srv = admin.Server(context.GLOBAL.ldap.conn)
        servers = admin_srv.list({'cell': cell,  'traits': cli.combine(traits), 
         'label': label})
...
def dns_group(parent):
    ...
    @click.option('--server', help='Server name', required=False, type=cli.LIST)
    @click.option('-m', '--manifest', help='Load DNS from manifest file', type=click.File('rb'), required=True)
    @cli.admin.ON_EXCEPTIONS
    ...
    @click.option('--server', help='List servers matching this name', required=False)
    @cli.admin.ON_EXCEPTIONS
...
def app_groups_group(parent):
    ...
    @app_group.command()
    @click.argument('name', nargs=1, required=True)
    @click.option('--group-type', help='App group type', required=False)
    @click.option('--cell', help='Cell app pattern could be in; comma separated list of cells', type=cli.LIST)
    @click.option('--pattern', help='App pattern')
    @click.option('--endpoints', help='App group endpoints, comma separated list.', type=cli.LIST)
    @click.option('--data', help='App group specific data as key=value comma separated list', type=cli.LIST)
...
def app_group(parent):
    ...
    @app.command()
    @click.option('-m', '--manifest', help='Application manifest.', type=click.File('rb'))
    @click.argument('app')
    @cli.admin.ON_EXCEPTIONS
...
def schema_group(parent):
    ...
    @parent.command()
    @click.option('-l', '--load', help='Schema (YAML) file.', type=click.File('rb'))
    @cli.admin.ON_EXCEPTIONS
    def schema(load):
        ...
        def dict_to_namevalue_list(item):
            ...
            return [pair[1].update({'name': pair[0]}) or pair[1] for pair in sorted(item.items())]

        schema_obj['attributeTypes'] = dict_to_namevalue_list(schema_obj['attributeTypes'])
        schema_obj['objectClasses'] = dict_to_namevalue_list(schema_obj['objectClasses'])
...
def direct_group(parent):
    ...
    @click.option('-a', '--attrs', help='Addition attributes', type=cli.LIST)
    ...
    def get(rec_dn, cls, attrs):
        ...
        entry = context.GLOBAL.ldap.conn.get(rec_dn, '(objectClass=*)', list(set(attrs)))
...
def cell_group(parent):
    ...
    @click.argument('cell')
    @cli.admin.ON_EXCEPTIONS
    def configure(cell, version, root, location, username, archive_server, archive_username, ssq_namespace):
    ...
    @cell.command()
    @click.option('--idx', help='Master index.', type=click.Choice(['1', '2', '3']), required=True)
    @click.option('--hostname', help='Master hostname.', required=True)
    @click.option('--client-port', help='Zookeeper client port.', type=int, required=True)
    @click.option('--kafka-client-port', help='Kafka client port.', type=int, required=False)
    @click.option('--jmx-port', help='Zookeeper jmx port.', type=int, required=True)
    @click.option('--followers-port', help='Zookeeper followers port.', type=int, required=True)
    @click.option('--election-port', help='Zookeeper election port.', type=int, required=True)
    @click.argument('cell')
    @cli.admin.ON_EXCEPTIONS
    def insert(cell, idx, hostname, client_port, jmx_port, followers_port, election_port, kafka_client_port):
...
def ldap_tenant_group(parent):
    ...
    @tenant.command()
    @click.option('-s', '--system', help='System eon id', type=int, multiple=True, default=[])
...
def ldap_allocations_group(parent):
    ...
    @allocation.command()
    @click.option('-e', '--environment', help='Environment', type=click.Choice(['dev', 'qa', 'uat', 'prod']))
    ...
    @allocation.command()
    @click.option('-m', '--memory', help='Memory.', callback=cli.validate_memory)
    @click.option('-c', '--cpu', help='CPU.', callback=cli.validate_cpu)
    @click.option('-d', '--disk', help='Disk.', callback=cli.validate_disk)
    @click.option('-r', '--rank', help='Rank.', type=int, default=100)
    @click.option('-u', '--max-utilization', help='Max utilization.', type=float)
    ...
    @click.argument('allocation')
    @cli.admin.ON_EXCEPTIONS
    def reserve(allocation, cell, memory, cpu, disk, rank, max_utilization, traits, label):
    ...
    @allocation.command()
    @click.option('--pattern', help='Application name pattern.', required=True)
    @click.option('--priority', help='Assigned priority.', type=int, required=True)
    @click.option('--cell', help='Cell.', required=True)
    @click.option('--delete', help='Delete assignment.', is_flag=True, default=False)
    @click.argument('allocation')
    @cli.admin.ON_EXCEPTIONS
