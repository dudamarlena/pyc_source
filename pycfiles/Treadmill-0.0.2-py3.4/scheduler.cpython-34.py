...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/scheduler.py
# Compiled at: 2017-04-03 02:32:49
# Size of source mod 2**32: 45183 bytes
"""Treadmill hierarchical scheduler."""
import abc, collections, heapq, logging, operator, itertools, time, enum, numpy as np
...
def eps_capacity():
    ...
    assert DIMENSION_COUNT is not None, 'Dimension count not set.'
    return np.array([np.finfo(float).eps for _x in range(0, DIMENSION_COUNT)])
...
class IdentityGroup(object):
    __doc__ = 'Identity group.'
    __slots__ = ('available', 'count')
...
class State(enum.Enum):
    __doc__ = 'Enumeration of node/server states.'
    up = 'up'
    down = 'down'
    frozen = 'frozen'
...
class Affinity(object):
    __doc__ = 'Model affinity and affinity limits.'
    __slots__ = ('name', 'limits')
...
class Application(object):
    ...
    __slots__ = ('global_order', 'name', 'demand', 'affinity', 'priority', 'allocation',
                 'data_retention_timeout', 'server', 'lease', 'identity', 'identity_group',
                 'identity_group_ref', 'schedule_once', 'evicted', 'placement_expiry',
                 'renew')
    ...
    def acquire_identity(self):
        ...
        if self.identity is None:
            ...
            _LOGGER.info('Acquired identity: %s: %s - %s', self.name, self.identity_group, self.identity)
        return self.identity is not None

    def release_identity(self):
        """Release app identity."""
        if self.identity_group_ref:
...
class Strategy(object, metaclass=abc.ABCMeta):
    __doc__ = 'Base class for all placement strategies.'
...
class SpreadStrategy(Strategy):
    __doc__ = 'Spread strategy will suggest new node for each subsequent placement.'
    __slots__ = ('current_idx', 'node')
...
class PackStrategy(Strategy):
    __doc__ = 'Pack strategy will suggest same node until it is full.'
    __slots__ = ('current_idx', 'node')
...
class TraitSet(object):
    __doc__ = 'Hierarchical set of traits.'
    __slots__ = ('self_traits', 'children_traits', 'traits')

    def __init__(self, traits=0):
        if not traits:
            traits = 0
        if not isinstance(traits, int):
            assert isinstance(traits, int)
...
class AffinityCounter(object):
    __doc__ = 'Manages affinity count'
    __slots__ = ('affinity_counter', )
...
class Node(object):
    __doc__ = 'Abstract placement node.'
    __slots__ = ('name', 'level', 'free_capacity', 'parent', 'children', 'children_by_name',
                 'traits', 'labels', 'affinity_counters', 'valid_until', '_state',
                 '_state_since')
    ...
    def set_state(self, state, since):
        ...
        self._state = state
        _LOGGER.debug('state: %s - (%s, %s)', self.name, self._state, self._state_since)
    ...
    def adjust_valid_until(self, child_valid_until):
        ...
        if child_valid_until:
            ...
            if self.empty():
                ...
                self.valid_until = max([node.valid_until for node in self.children_iter()])
        if self.parent:
    ...
    def check_app_constraints(self, app):
        ...
        if app.traits != 0 and not self.traits.has(app.traits):
            ...
            return False
        if self.affinity_counters[app.affinity.name] >= app.affinity.limits[self.level]:
            return False
    ...
    def size(self, label):
        ...
        if self.empty() or label not in self.labels:
            return eps_capacity()
        return np.sum([n.size(label) for n in self.children_iter()], 0)
...
class Bucket(Node):
    __doc__ = 'Collection of nodes/buckets.'
    __slots__ = ('affinity_strategies', 'traits')
    ...
    def adjust_capacity_down(self, prev_capacity=None):
        ...
        if self.empty():
            ...
            if prev_capacity is not None and _all_lt(prev_capacity, self.free_capacity):
                return
            free_capacity = zero_capacity()
            for child_node in self.children_iter():
                if child_node.state is not State.up:
                    continue
                free_capacity = np.maximum(free_capacity, child_node.free_capacity)
...
class Server(Node):
    __doc__ = 'Server object, final app placement.'
    __slots__ = ('init_capacity', 'apps')

    def __init__(self, name, capacity, valid_until, traits=0, label=None):
        super(Server, self).__init__(name, traits=traits, level='server', valid_until=valid_until)
    ...
    def is_same(self, other):
        ...
        """
        return self.labels == other.labels and _all_eq(self.init_capacity, other.init_capacity) and self.traits.is_same(other.traits)
    ...
    def set_state(self, state, since):
        ...
        if state == State.up:
            ...
            if state == State.down or state == State.frozen:
                if self.parent:
...
class Allocation(object):
            ...
            if isinstance(reserved, int):
                ...
                if isinstance(reserved, float):
                    ...
                    if isinstance(reserved, list):
                        ...
                        if isinstance(reserved, np.ndarray):
                            self.reserved = reserved
                        elif not 'Unsupported type: %r' % type(reserved):
    ...
    def priv_utilization_queue(self):
        ...
        def app_key(app):
            ...
            return (
             -app.priority, 0 if app.server else 1,
        ...
        for app in prio_queue:
            ...
            if util <= self.max_utilization:
                yield (
                 self.rank, util, pending, app.global_order, app)
            else:
                break
    ...
    def utilization_queue(self, free_capacity):
        ...
        queues = [alloc.utilization_queue(0) for alloc in self.sub_allocations.values()]
        ...
        for item in heapq.merge(*queues):
            ...
            if app.priority == 0:
                util = _MAX_UTILIZATION
            yield (
             rank, util, pending, order, app)

    def total_reserved(self):
        """Total reserved capacity including sub-allocs."""
        return reduce(lambda acc, alloc: acc + alloc.total_reserved(), self.sub_allocations.values(), self.reserved)
...
class Partition(object):
    __doc__ = 'Cell partition.'
    __slots__ = ('allocation', 'max_server_uptime', 'max_lease', 'threshold')
    ...
    def valid_until(self, up_since):
        ...
        expires_local_tm = time.localtime(up_since + self.max_server_uptime)
...
class Cell(Bucket):
    __doc__ = 'Top level node.'
    __slots__ = ('partitions', 'next_event_at', 'apps', 'identity_groups')
        ...
        for app in queue:
            if app.identity is not None and app.identity_group_ref is not None:
                if app.identity >= app.identity_group_ref.count:
                    _LOGGER.info('Identity exceeds limit: %s - %s, limit %s', app.name, app.identity, app.identity_group_ref.count)
                    app.identity = None
                    if app.server:
                        servers[app.server].remove(app.name)
                else:
            ...
            if state == State.down:
                ...
                for name, app in server.apps.items():
                    ...
                    if expires_at <= time.time():
                        ...
                        _LOGGER.debug('Keep placement: %s until %s', name, expires_at)
                        self.next_event_at = min(expires_at, self.next_event_at)
        ...
        for app in queue:
            ...
            if app.renew:
                ...
                if not server.renew(app):
                    _LOGGER.debug('Cannot renew app %s on server %s', app.name, app.server)
            ...
            if app.server:
                assert app.server in servers
                if not app.has_identity():
                    raise AssertionError
                    continue
                assert app.server is None
                if not app.acquire_identity():
                    _LOGGER.info('Unable to acquire identity: %s, %s', app.name, app.identity_group)
                    continue
                if app in evicted:
                    pass
            assert app.has_identity()
    ...
    def schedule_alloc(self, allocation):
        ...
        before = [(app.name, app.server, app.placement_expiry) for app in queue]
        ...
        after = [(app.server, app.placement_expiry) for app in queue]
        _LOGGER.info('Scheduled %d apps in %r', len(queue), time.time() - begin)
        placement = [tuple(itertools.chain(b, a)) for b, a in zip(before, after)]
        for appname, s_before, exp_before, s_after, exp_after in placement:
            if s_before != s_after:
                _LOGGER.info('New placement: %s - %s => %s', appname, s_before, s_after)
            elif exp_before != exp_after:
                _LOGGER.info('Renewed: %s [%s] - %s => %s', appname, s_before, exp_before, exp_after)
                continue
