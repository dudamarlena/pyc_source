...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/webutils.py
# Compiled at: 2017-04-03 02:32:49
# Size of source mod 2**32: 9230 bytes
"""Flask method decorators and other web utilities."""
import datetime, functools, logging, json, re, flask
try:
    import tornado
...
def cors_domain_match(base_domain):
    ...
    if base_domain == '*':
        ...
        origin_re = re.compile('https?://%s' % base_domain)
        key = 'HTTP_ORIGIN'
...
def cors(origin=None, methods=None, headers=None, max_age=21600, attach_to_all=True, automatic_options=True, credentials=False, content_type=None):
...
def raw_get_api(api, cors_handler, marshal=None, resp_model=None, parser=None):
    ...
    """
    return get_api(api, cors_handler, marshal=marshal, resp_model=resp_model, parser=parser, json_resp=False)


def get_api(api, cors_handler, marshal=None, resp_model=None, parser=None, json_resp=True):
    """Returns default API decorator for GET request.
    ...
    funcs.append(api.doc(responses={403: 'Not Authorized', 
...
def _common_api(api, cors_handler, marshal=None, req_model=None, resp_model=None, parser=None):
...
def post_api(api, cors_handler, marshal=None, req_model=None, resp_model=None, parser=None):
    """Returns default API decorator for POST request."""
    return _common_api(api, cors_handler, marshal=marshal, req_model=req_model, resp_model=resp_model, parser=parser)


def put_api(api, cors_handler, req_model=None, resp_model=None):
    """Returns default API decorator for PUT request."""
    return _common_api(api, cors_handler, req_model=req_model, resp_model=resp_model)
...
def namespace(api, name, description):
    ...
    return api.namespace(name.split('.')[(-1)].replace('_', '-'), description=description)