...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/cli/admin/blackout.py
# Compiled at: 2017-04-03 02:32:49
# Size of source mod 2**32: 4303 bytes
"""Kills all connections from a given treadmill server."""
import logging, re, click, kazoo
...
def _list_server_blackouts(zkclient, fmt):
    ...
    try:
        ...
        for server in blacked_out_nodes:
            ...
            data, metadata = zkutils.get(zkclient, node_path, need_metadata=True)
...
def _blackout_server(zkclient, server, reason):
    ...
    path = z.path.blackedout_server(server)
    zkutils.ensure_exists(zkclient, path, acl=[
     zkutils.make_host_acl(server, 'rwcda')], data=reason)
...
def init():
    ...
    @click.option('--fmt', help='Format of the blackout output.', default='[%t] %h %r')
    @click.option('--clear', is_flag=True, default=False, help='Clear blackout.')
    @_ON_EXCEPTIONS
