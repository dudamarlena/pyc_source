...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/supervisor.py
# Compiled at: 2017-04-03 02:32:49
# Size of source mod 2**32: 8324 bytes
...
"""
import glob, logging, os, stat, subprocess, time, jinja2
from . import fs
from . import utils
from . import subproc
...
def create_service(app_root, user, home, shell, service, runcmd, env=None, down=True, envdirs=None, as_root=True, template=None):
    ...
    if envdirs is None:
        envdirs = []
    utils.create_script(os.path.join(real_svc_dir, 'run'), template, service=service, user=user, home=home, shell=shell, cmd=cmd, env=env, envdirs=envdirs, as_root=as_root)
    utils.create_script(os.path.join(real_svc_dir, 'finish'), 'supervisor.finish', service=service, user=user, cmds=[])
    if down:
...
def is_supervisor_running(app_root, service):
    ...
    try:
        subproc.check_call(['s6-svok', os.path.join(app_root, service)], stderr=subprocess.STDOUT)
        return True
...
def wait_all_up(svcroot, timeout=0, subset=None):
    ...
    _service_wait(svcroot, up_opt='-u', any_all_opt='-a', timeout=timeout, subset=subset)


def wait_all_down(svcroot, timeout=0, subset=None):
    """Waits for services to be up."""
    _service_wait(svcroot, up_opt='-d', any_all_opt='-a', timeout=timeout, subset=subset)


def wait_any_up(svcroot, timeout=0, subset=None):
    """Waits for services to be up."""
    _service_wait(svcroot, up_opt='-u', any_all_opt='-o', timeout=timeout, subset=subset)


def wait_any_down(svcroot, timeout=0, subset=None):
    """Waits for services to be up."""
    _service_wait(svcroot, up_opt='-d', any_all_opt='-o', timeout=timeout, subset=subset)
