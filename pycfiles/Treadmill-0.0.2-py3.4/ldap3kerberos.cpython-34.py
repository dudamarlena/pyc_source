...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/ldap3kerberos.py
# Compiled at: 2017-04-03 02:32:49
# Size of source mod 2**32: 3068 bytes
"""
Replaces the use of python-gssapi with pykerberos in ldap3.
"""
import socket, base64, kerberos
...
def sasl_gssapi(connection, controls):
    ...
    try:
        in_token = ''
        while 1:
            status = kerberos.authGSSClientStep(ctx, base64.b64encode(in_token))
            out_token = kerberos.authGSSClientResponse(ctx) or ''
            result = send_sasl_negotiation(connection, controls, base64.b64decode(out_token))
        ...
        unwrapped_token = base64.b64decode(kerberos.authGSSClientResponse(ctx) or '')
        ...
        kerberos.authGSSClientWrap(ctx, base64.b64encode(bytes(client_security_layers)))
        out_token = kerberos.authGSSClientResponse(ctx) or ''
