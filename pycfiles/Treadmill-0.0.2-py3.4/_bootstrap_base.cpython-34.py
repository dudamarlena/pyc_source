...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/osmodules/_bootstrap_base.py
# Compiled at: 2017-04-11 07:05:33
# Size of source mod 2**32: 5611 bytes
"""An interface for bootstrapping treadmill."""
import errno, logging, os, tempfile, jinja2, treadmill
from .. import admin
from .. import context
from .. import fs
...
class BootstrapBase(object):
    __doc__ = 'Base interface for bootstrapping.'
    ...
    def _interpolate_dict(self, value, params):
        ...
        while counter < 100:
            counter += 1
            result = {k:self._interpolate(v, params) for k, v in target.items()}
            if result == target:
                break
            target = dict(result)
        else:
    ...
    def _update(self, filename, content):
        ...
        with tempfile.NamedTemporaryFile(dir=os.path.dirname(filename), prefix='.tmp', delete=False) as (tmp_file):
            ...
            for filename in files:
                ...
                if os.path.islink(src_file):
                    ...
                    if not os.path.exists(tgt_file):
                        _LOGGER.critical('Broken symlink: %s -> %s, %r', src_file, tgt_file, params)
                        raise Exception('Broken symlink, aborting install.')
                else:
                    with open(src_file) as (src):
                        self._update(tgt_file, self._render(src.read(), params))
