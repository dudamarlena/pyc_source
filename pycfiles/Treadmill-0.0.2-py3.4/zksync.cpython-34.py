...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/zksync.py
# Compiled at: 2017-04-06 03:53:49
# Size of source mod 2**32: 5340 bytes
"""
Syncronizes Zookeeper to file system.
"""
import logging, glob, os, tempfile, time
...
class Zk2Fs(object):
    __doc__ = 'Syncronize Zookeeper with file system.'
    ...
    def _write_data(self, fpath, data, stat):
        ...
        """
        with tempfile.NamedTemporaryFile(dir=os.path.dirname(fpath), delete=False, prefix='.tmp', mode='w') as (temp):
            temp.write(data)
            os.fchmod(temp.fileno(), 420)
    ...
    def _children_watch(self, zkpath, children, watch_data, on_add, on_del):
        ...
        fpath = self.fpath(zkpath)
        filenames = set(map(os.path.basename, glob.glob(os.path.join(fpath, '*'))))
    ...
    def sync_children(self, zkpath, watch_data=False, on_add=None, on_del=None):
        """Sync children of zkpath to fpath."""
        _LOGGER.info('sync children: zk = %s, watch_data: %s', zkpath, watch_data)
        fpath = self.fpath(zkpath)
        fs.mkdir_safe(fpath)
        if not on_del:
        ...
        def _children_watch(children):
            """Callback invoked on children watch."""
            renew = self._children_watch(zkpath, children, watch_data, on_add, on_del)
            self._update_last()
            return renew