...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/appmgr/manifest.py
...
def load(tm_env, event):
    ...
    manifest['services'] = [{'name': service['name'],  'command': service['command'],  'restart': {'limit': int(service['restart']['limit']),  'interval': int(service['restart']['interval'])},  'root': service.get('root', False)} for service in manifest.get('services', [])]
    manifest['endpoints'] = [{'name': endpoint['name'],  'port': int(endpoint['port']),  'type': endpoint.get('type', None),  'proto': endpoint.get('proto', 'tcp')} for endpoint in manifest.get('endpoints', [])]
...
def _add_system_services(manifest):
    ...
    sshd_svc = {'name': 'sshd', 
     ...
     'restart': {'limit': 5, 
                 'interval': 60}, 
     'command': '%s -D -f /etc/ssh/sshd_config -p $TREADMILL_ENDPOINT_SSH' % subproc.resolve('sshd')}
    manifest['system_services'].append(sshd_svc)
    ssh_endpoint = {'name': 'ssh', 
     'port': 0, 
     'type': 'infra'}
...
def _add_features(manifest):
    ...
    for feature in manifest.get('features', []):
        try:
            feature_mod = importlib.import_module('treadmill.appmgr.features.' + feature)
