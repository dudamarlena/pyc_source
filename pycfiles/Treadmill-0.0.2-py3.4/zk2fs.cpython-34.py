...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/sproc/zk2fs.py
# Compiled at: 2017-04-03 02:32:49
# Size of source mod 2**32: 4248 bytes
"""Syncronize Zookeeper with file system."""
import fnmatch, logging, os, re, shutil, time, click
from .. import zksync
from .. import fs
from .. import context
...
def _on_add_app(zk2fs_sync, zkpath, reobj):
    ...
    if reobj.match(app_name):
        ...
        zk2fs_sync.sync_children(zkpath, on_add=lambda p: _on_add_instance(zk2fs_sync, p), on_del=lambda p: _on_del_instance(zk2fs_sync, p))
...
def init():
    ...
    @click.option('--root', help='Output directory.', required=True)
    @click.option('--endpoints', help='Sync endpoints.', is_flag=True, default=False)
    @click.option('--appgroups', help='Sync appgroups.', is_flag=True, default=False)
    @click.option('--running', help='Sync running.', is_flag=True, default=False)
    @click.option('--scheduled', help='Sync scheduled.', is_flag=True, default=False)
    @click.option('--servers', help='Sync servers.', is_flag=True, default=False)
    @click.option('--placement', help='Sync placement.', is_flag=True, default=False)
    @click.option('--tasks', help='Sync trace with app pattern.')
    @click.option('--once', help='Sync once and exit.', is_flag=True, default=False)
    def zk2fs_cmd(root, endpoints, appgroups, running, scheduled, servers, placement, tasks, once):
        ...
        if endpoints:
            zk2fs_sync.sync_children(z.ENDPOINTS, on_add=lambda p: _on_add_proid(zk2fs_sync, p), on_del=lambda p: _on_del_proid(zk2fs_sync, p))
        if scheduled:
        ...
        if tasks:
            ...
            zk2fs_sync.sync_children(z.TASKS, on_add=lambda p: _on_add_app(zk2fs_sync, p, reobj), on_del=lambda p: _on_del_app(zk2fs_sync, p, reobj))
