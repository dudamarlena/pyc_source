...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/sproc/kafka.py
# Compiled at: 2017-04-03 02:32:49
# Size of source mod 2**32: 6791 bytes
"""Treadmill Kafka system process"""
import sys, logging, os, pwd, click, kazoo, jinja2, treadmill
...
DEFAULT_KAFKA_PROPERTIES = os.path.join(treadmill.TREADMILL, 'etc', 'kafka', BROKER_PROP_FILE_NAME)
HOSTNAME = sysinfo.hostname()
CURRENT_USER = pwd.getpwuid(os.getuid()).pw_name
_LOGGER = logging.getLogger(__name__)

def setup_kafka_env(zkurl, template_dir, broker_port, broker_id, zkroot, krb_realm):
    ...
    with open(broker_props, 'w') as (prop_fh):
        variables = {'broker_port': str(broker_port),  'broker_id': str(broker_id), 
    ...
    with open(jaas_conf, 'w') as (jaas_fh):
        variables = {'user_id': CURRENT_USER,  'hostname': HOSTNAME, 
         'krb_realm': krb_realm}
...
def make_sasl_acl(perm, krb_realm):
    ...
    acls = [
     kazoo.security.make_acl(scheme, host_principal, read='r' in perm, write='w' in perm, create='c' in perm, delete='d' in perm, admin='a' in perm)]
    admin_cell = admin.Cell(context.GLOBAL.ldap.conn)
    cell = admin_cell.get(context.GLOBAL.cell)
    for master in cell['masters']:
        host_principal = principal.format(CURRENT_USER, master['hostname'], krb_realm)
        acls.append(kazoo.security.make_acl(scheme, host_principal, read='r' in perm, write='w' in perm, create='c' in perm, delete='d' in perm, admin='a' in perm))
...
def init():
    ...
    @click.command()
    @click.option('--kafka-dir', help='The Kafka dir to instal in', default=tkafka.DEFAULT_KAFKA_DIR)
    @click.option('--broker-port', help='The Kafka broker port', type=int, envvar='TREADMILL_ENDPOINT_CLIENT', required=True)
    @click.option('--broker-id', help='The Kafka broker ID', type=int, envvar='TREADMILL_IDENTITY', required=True)
    @click.option('--launcher', help='Kafka launcher name, see etc/linux.exe.config', default=DEFAULT_KAFKA_LAUNCHER)
    @click.option('--template-dir', help='Kafka template dir', default=DEFAULT_KAFKA_TEMPLATE_DIR)
    @click.option('--java-home', help='Value of JAVA_HOME', envvar='JAVA_HOME')
    @click.option('--zkroot', help='The ZK root to use', default=tkafka.KAFKA_ZK_ROOT)
    @click.option('--env-vars', help='Environment vars to pass to Kafka', type=cli.LIST, default='')
    @click.option('--krb-realm', help='Kerberos domain', required=True)
    @click.argument('opts', nargs=-1)
    def kafka(kafka_dir, broker_port, broker_id, launcher, template_dir, java_home, zkroot, env_vars, krb_realm, opts):
        """Run Treadmill Kafka"""
        ...
        kafka_env = tkafka.setup_env(kafka_dir=kafka_dir, with_data_dir=True, server=True)
        ...
        broker_props, jaas_conf = setup_kafka_env(zkurl, template_dir, broker_port, broker_id, zkroot, krb_realm)
        kafka_opts = '-Djava.security.auth.login.config={} -Dzookeeper.sasl.client.username={}'.format(jaas_conf, CURRENT_USER)
