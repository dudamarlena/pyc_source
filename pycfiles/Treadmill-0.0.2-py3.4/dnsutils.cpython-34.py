...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treadmill/dnsutils.py
# Compiled at: 2017-04-03 02:32:49
# Size of source mod 2**32: 5145 bytes
"""
DNS resolution methods
"""
import collections, logging, socket, dns.exception, dns.rdatatype, dns.resolver
...
def _build_resource_to_address_map(answer):
    ...
    for resource in answer.response.additional:
        ...
        mapping[target].extend(record.address for record in resource.items if record.rdtype == dns.rdatatype.A)
...
def _build_result_set(answer):
    ...
    for resource in answer:
        ...
        if target in resource_map:
            result_set.extend((address, resource.port, resource.priority, resource.weight) for address in resource_map[target])
        else:
...
def txt(label, resolver=None):
    ...
    """
    return [str(rec).strip('"') for rec in query(label, dns.rdatatype.TXT, resolver)]
