...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/bibstract/cli.py
...
class ExtractorCli(object):
    ...
    def _assert_master_bib(self):
        if self.ex.master_bib is None:
            sys.stderr.write('missing the master BibTex -m option\n')
            sys.exit(1)

    def _assert_texpath(self):
        if self.ex.texpath is None:
            sys.stderr.write('missing the (La)Tex path -t option\n')
            sys.exit(1)
...
class ConfAppCommandLine(OneConfPerActionOptionsCliEnv):
    ...
    def __init__(self):
        ...
        super(ConfAppCommandLine, self).__init__(cnf,
          config_env_name='bibstractrc', pkg_dist='zensols.bibstract', config_type=AppConfig)
