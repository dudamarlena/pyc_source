...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/muntjac/data/util/filter/simple_string_filter.py
...
class SimpleStringFilter(IFilter):
    ...
    def __eq__(self, obj):
        if not isinstance(obj, SimpleStringFilter):
            ...
            if self.propertyId != obj.propertyId and obj.propertyId is not None and obj.propertyId != self.propertyId:
                return False
            if self.filterString != obj.filterString and obj.filterString is not None and obj.filterString != self.filterString:
                return False
