...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/muntjac/demo/SimpleAddressBook.py
...
from muntjac.api import Application, Form, TextField, Table, HorizontalLayout, HorizontalSplitPanel, Window, VerticalLayout, Button

class SimpleAddressBook(Application):
    _fields = [
     'First Name', 'Last Name', 'Company', 'Mobile Phone',
    ...
    def initAddressList(self):
        ...
        self._contactList.addCallback(onContactChange, field.ValueChangeEvent, self)
    ...
    def initFilteringControls(self):
        for pn in self._visibleCols:
            ...
            sf.addCallback(onFilterChange, property.ValueChangeEvent, pn, sf, self)
...
def onFilterChange(event, name, text, book):
    ...
    book.getMainWindow().showNotification('%d matches found' % book._addressBookData.size())
