...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/muntjac/demo/sampler/features/upload/UploadWithProgressMonitoringExample.py
# Compiled at: 2013-04-04 15:36:38
import time
from StringIO import StringIO
from muntjac.api import VerticalLayout, Label, ProgressIndicator, Upload, CheckBox, Button, Panel, FormLayout, HorizontalLayout
...
class ProgressListener(upload.IProgressListener):
    ...
    def updateProgress(self, readBytes, contentLength):
        ...
        self._c._textualProgress.setValue('Processed ' + readBytes + ' bytes of ' + contentLength)
        self._c._result.setValue(self._c._counter.getLineBreakCount() + ' (counting...)')
...
class SucceededListener(upload.ISucceededListener):
    ...
    def uploadSucceeded(self, event):
        self._c._result.setValue(self._c._counter.getLineBreakCount() + ' (total)')
...
class FailedListener(upload.IFailedListener):
    ...
    def uploadFailed(self, event):
        self._c._result.setValue(self._c._counter.getLineBreakCount() + ' (counting interrupted at ' + round(100 * self._c._pi.getValue()) + '%)')
