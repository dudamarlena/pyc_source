...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/muntjac/demo/sampler/features/dragndrop/DragDropRearrangeComponentsExample.py
    ...
    def drop(self, dropEvent):
        ...
        if isinstance(sourceComponent, WrappedComponent):
            ...
            if nxt is None or nxt != target:
                return
            if dropTargetData.getData('horizontalLocation') == str(HorizontalDropLocation.CENTER):
                if sourceWasAfterTarget:
                    index -= 1
            elif dropTargetData.getData('horizontalLocation') == str(HorizontalDropLocation.LEFT):
                index -= 1
                if index < 0:
                    index = 0
