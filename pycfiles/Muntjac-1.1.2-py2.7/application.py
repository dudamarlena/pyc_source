...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/muntjac/application.py
...
class Application(IUriHandler, ITerminal, IErrorListener):
    ...
    def getRelativeLocation(self, resource):
        ...
        """
        warn('this method is intended to be used by the terminal only. It may be removed or moved in the future.', DeprecationWarning)
        key = self._resourceKeyMap.get(resource)
        if key is None:
            return
        else:
    ...
    def handleURI(self, context, relativeUri):
        ...
        """
        warn('this method is called be the terminal implementation only and might be removed or moved in the future. Instead of overriding this method, add your L{IUriHandler} to a top level L{Window} (eg. getMainWindow().addUriHanler(handler) instead.', DeprecationWarning)
    ...
    def addListener(self, listener, iface=None):
        ...
        with L{removeWindow}.
        """
        if isinstance(listener, IUserChangeListener) and (iface is None or issubclass(iface, IUserChangeListener)):
            self._userChangeListeners.append(listener)
        if isinstance(listener, IWindowAttachListener) and (iface is None or issubclass(iface, IWindowAttachListener)):
            self._windowAttachListeners.append(listener)
        if isinstance(listener, IWindowDetachListener) and (iface is None or issubclass(iface, IWindowDetachListener)):
    ...
    def removeListener(self, listener, iface=None):
        ...
        """
        if isinstance(listener, IUserChangeListener) and (iface is None or iface == IUserChangeListener):
    ...
    def terminalError(self, event):
        ...
        if isinstance(t, IOError):
            logger.exception('SocketException in CommunicationManager. Most likely client (browser) closed socket.')
            return
        else:
            owner = None
            ...
            traceback.print_exception(exc_type, t, exc_traceback, file=sys.stdout)
            return
