...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/muntjac/ui/login_form.py
...
class LoginForm(CustomComponent):
    ...
    def addListener(self, listener, iface=None):
        ...
        """
        if isinstance(listener, ILoginListener) and (iface is None or issubclass(iface, ILoginListener)):
...
class LoginPage(IApplicationResource):
    ...
    def getStream(self):
        return DownloadStream(StringIO(self._form.getLoginHTML()), self.getMIMEType(), self.getFilename())
...
class UriHandler(IUriHandler):
    ...
    def __init__(self, form):
        self._form = form
        self._responce = '<html><body>Login form handeled.' + "<script type='text/javascript'>top.vaadin.forceSync();" + '</script></body></html>'
    ...
    def handleURI(self, context, relativeUri):
        if relativeUri is not None and 'loginHandler' in relativeUri:
            ...
            downloadStream = DownloadStream(StringIO(self._responce), 'text/html', 'loginSuccesfull')
