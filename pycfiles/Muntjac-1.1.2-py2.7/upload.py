...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/muntjac/ui/upload.py
...
from muntjac.terminal.stream_variable import IStreamVariable, IStreamingEvent
...
class Upload(AbstractComponent, IFocusable):
    ...
    def addListener(self, listener, iface=None):
        ...
        """
        if isinstance(listener, IFailedListener) and (iface is None or issubclass(iface, IFailedListener)):
            self.registerListener(FailedEvent, listener, _UPLOAD_FAILED_METHOD)
        if isinstance(listener, IFinishedListener) and (iface is None or issubclass(iface, IFinishedListener)):
            self.registerListener(FinishedEvent, listener, _UPLOAD_FINISHED_METHOD)
        if isinstance(listener, IProgressListener) and (iface is None or issubclass(iface, IProgressListener)):
            self._progressListeners.add(listener)
        if isinstance(listener, IStartedListener) and (iface is None or issubclass(iface, IStartedListener)):
            self.registerListener(StartedEvent, listener, _UPLOAD_STARTED_METHOD)
        if isinstance(listener, ISucceededListener) and (iface is None or issubclass(iface, ISucceededListener)):
            self.registerListener(SucceededEvent, listener, _UPLOAD_SUCCEEDED_METHOD)
    ...
    def addCallback(self, callback, eventType=None, *args):
        ...
        elif issubclass(eventType, IProgressListener):
    ...
    def removeListener(self, listener, iface=None):
        ...
        """
        if isinstance(listener, IFailedListener) and (iface is None or issubclass(iface, IFailedListener)):
            self.withdrawListener(FailedEvent, listener, _UPLOAD_FAILED_METHOD)
        if isinstance(listener, IFinishedListener) and (iface is None or issubclass(iface, IFinishedListener)):
            self.withdrawListener(FinishedEvent, listener, _UPLOAD_FINISHED_METHOD)
        if isinstance(listener, IProgressListener) and (iface is None or issubclass(iface, IProgressListener)):
            if listener in self._progressListeners:
                self._progressListeners.remove(listener)
        if isinstance(listener, IStartedListener) and (iface is None or issubclass(iface, IStartedListener)):
            self.withdrawListener(StartedEvent, listener, _UPLOAD_STARTED_METHOD)
        if isinstance(listener, ISucceededListener) and (iface is None or issubclass(iface, ISucceededListener)):
            self.withdrawListener(SucceededEvent, listener, _UPLOAD_SUCCEEDED_METHOD)
...
class InnerStreamVariable(IStreamVariable):
    ...
    def getOutputStream(self):
        receiveUpload = self._upload.receiver.receiveUpload(self._lastStartedEvent.getFileName(), self._lastStartedEvent.getMimeType())
        self._lastStartedEvent = None
    ...
    def streamingFinished(self, event):
        self._upload.fireUploadSuccess(event.getFileName(), event.getMimeType(), event.getContentLength())
    ...
    def streamingFailed(self, event):
        ...
        if isinstance(exception, NoInputStreamException):
            self._upload.fireNoInputStream(event.getFileName(), event.getMimeType(), 0)
        elif isinstance(exception, NoOutputStreamException):
            self._upload.fireNoOutputStream(event.getFileName(), event.getMimeType(), 0)
        else:
            self._upload.fireUploadInterrupted(event.getFileName(), event.getMimeType(), 0, exception)
        self._upload.endUpload()
