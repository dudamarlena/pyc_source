...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/muntjac/ui/grid_layout.py
...
class GridLayout(AbstractLayout, IAlignmentHandler, ISpacingHandler, ILayoutClickNotifier):
    ...
    def addComponent(self, *args):
        ...
        if nargs == 1:
            ...
            while not done:
                try:
                    area = Area(component, self._cursorX, self._cursorY, self._cursorX, self._cursorY)
                    self.checkExistingOverlaps(area)
                    done = True
                except OverlapsException as e:
                    self.space()
            ...
            if column1 < 0 or row1 < 0 or column2 >= self._cols or row2 >= self._rows:
            ...
            while not done and index < len(self._areas):
                ...
                if existingArea._row1 >= row1 and existingArea._column1 > column1 or existingArea._row1 > row1:
            ...
            if self._cursorX >= column1 and self._cursorX <= column2 and self._cursorY >= row1 and self._cursorY <= row2:
    ...
    def paintContent(self, target):
        ...
        if colSum == 0:
            ...
            for i in range(self._cols):
                myRatio = int(round(self.getColumnExpandRatio(i) / colSum * 1000))
                columnExpandRatioArray[i] = myRatio
            ...
            for cury in range(self._rows):
                ...
                if not equallyDividedRows:
                    myRatio = int(round(self.getRowExpandRatio(cury) / rowSum * 1000))
                    rowExpandRatioArray[cury] = myRatio
                    realRowExpandRatioSum += myRatio
                for curx in range(self._cols):
                    if area is not None and area._row1 == cury and area._column1 == curx:
                        if emptyCells > 0:
                            target.startTag('gc')
    ...
    def setColumns(self, columns):
        ...
        """
        if columns < 1:
            raise ValueError, 'The number of columns and rows in the grid must be at least 1'
    ...
    def setComponentAlignment(self, *args):
        ...
        if nargs == 2:
            ...
            self._componentToAlignment[childComponent] = Alignment(horizontalAlignment + verticalAlignment)
            self.requestRepaint()
        else:
            raise ValueError, 'invalid number of arguments'
    ...
    def insertRow(self, row):
        ...
        """
        if row > self._rows:
            raise ValueError, 'Cannot insert row at ' + row + ' in a gridlayout with height ' + self._rows
        for existingArea in self._areas:
    ...
    def removeRow(self, row):
        ...
        """
        if row >= self._rows:
            raise ValueError, 'Cannot delete row ' + row + ' from a gridlayout with height ' + self._rows
    ...
    def getComponent(self, x, y):
        ...
        """
        for area in self._areas:
            if area.getColumn1() <= x and x <= area.getColumn2() and area.getRow1() <= y and y <= area.getRow2():
