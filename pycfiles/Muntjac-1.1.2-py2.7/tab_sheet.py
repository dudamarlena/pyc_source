...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/muntjac/ui/tab_sheet.py
# Compiled at: 2013-04-04 15:36:35
"""Defines a tab sheet component."""
from warnings import warn
from muntjac.terminal.key_mapper import KeyMapper
from muntjac.terminal.paintable import IRepaintRequestListener, RepaintRequestEvent
...
_SELECTED_TAB_CHANGE_METHOD = getattr(ISelectedTabChangeListener, 'selectedTabChange')
...
class TabSheet(AbstractComponentContainer):
    ...
    def updateSelection(self):
        ...
        while True:
            try:
                ...
                if self._selected is None or selectedTabInfo is None or not selectedTabInfo.isVisible() or not selectedTabInfo.isEnabled():
    ...
    def addListener(self, listener, iface=None):
        ...
        """
        if isinstance(listener, ISelectedTabChangeListener) and (iface is None or issubclass(iface, ISelectedTabChangeListener)):
            self.registerListener(SelectedTabChangeEvent, listener, _SELECTED_TAB_CHANGE_METHOD)
    ...
    def addCallback(self, callback, eventType=None, *args):
        ...
        if issubclass(eventType, SelectedTabChangeEvent):
            self.registerCallback(SelectedTabChangeEvent, callback, None, *args)
        else:
    ...
    def removeListener(self, listener, iface=None):
        ...
        if isinstance(listener, ISelectedTabChangeListener) and (iface is None or issubclass(iface, ISelectedTabChangeListener)):
            self.withdrawListener(SelectedTabChangeEvent, listener, _SELECTED_TAB_CHANGE_METHOD)
        super(TabSheet, self).removeListener(listener, iface)
        return
