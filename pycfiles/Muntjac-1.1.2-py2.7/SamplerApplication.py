...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/muntjac/demo/sampler/SamplerApplication.py
...
from muntjac.api import UriFragmentUtility, VerticalLayout, HorizontalLayout, Table, Panel, Window, Alignment, HorizontalSplitPanel, CssLayout, ComboBox, PopupView, NativeButton, Tree, CustomComponent, Label, Embedded
...
class SamplerApplication(Application):
    ...
    def __init__(self):
        ...
        self._currentApplicationTheme = self._SAMPLER_THEME_NAME + '-' + self._THEMES[0]
    ...
    def getThemeBase(cls):
        ...
        """
        uri = cls._APP_URL + '../VAADIN/themes/' + cls._SAMPLER_THEME_NAME + '/'
        return uri
...
class SamplerWindow(Window):
    ...
    def __init__(self, app):
        ...
        self._uriFragmentUtility.addListener(UriListener(self), IFragmentChangedListener)
        ...
        self.addListener(WindowCloseListener(self, self._app), window.ICloseListener)
    ...
    def createSearch(self):
        ...
        self._search.addListener(SearchListener(self), prop.IValueChangeListener)
    ...
    def createThemeSelect(self):
        ...
        for themeName in self._app._THEMES:
            ...
            self.theme.setItemCaption(idd, themeName[:1].upper() + themeName[1:] + ' theme')
        ...
        self.theme.addListener(ThemeChangeListener(self, self._app), prop.IValueChangeListener)
    ...
    def createMenuTree(self):
        ...
        self._currentFeature.addListener(FeatureChangeListener(self, tree), prop.IValueChangeListener)
...
class FeatureTable(Table, IFeatureList):
    ...
    def __init__(self, app):
        ...
        self.addGeneratedColumn(Feature.PROPERTY_ICON, IconColumnGenerator(self._app))
        self.addGeneratedColumn('', TableColumnGenerator(self._app))
        self.addListener(FeatureTableClickListener(self._app), IItemClickListener)
            ...
            if isinstance(f, FeatureSet):
                if c.isRoot(f):
                    if rootTitle is not None:
                        rootTitle.setValue('<em>' + str(sampleCount) + ' samples</em>' + rootTitle.getValue())
                        sampleCount = 0
                    ...
                    rootTitle = Label('<h2>' + f.getName() + '</h2><span>' + desc[:idx + 1] + '</span>', Label.CONTENT_XHTML)
                    rootTitle.setSizeUndefined()
                    if f.getRelatedFeatures() is not None:
                        rootTitle.setValue('<em>' + len(f.getRelatedFeatures()) + ' samples</em>' + rootTitle.getValue())
                    rootSet = CssLayout()
                ...
                if rootSet.getParent() is None:
                    if rootTitle is None:
                        ...
                        rootTitle = Label('<h2>' + parent.getName() + '</h2>', Label.CONTENT_XHTML)
                        rootTitle.setStyleName('root-section highlights-title')
                        rootTitle.setSizeUndefined()
                        self._grid.addComponent(rootTitle)
                        if parent.getDescription() is not None:
                            desc = Label(parent.getDescription(), Label.CONTENT_XHTML)
...
class SourceWindow(Window):
    ...
    def __init__(self, app):
        ...
        self.addListener(SourceWindowCloseListener(app), window.ICloseListener)
...
class TreeClickListener(button.IClickListener):
    ...
    def buttonClick(self, event):
        if self.b.getStyleName().contains('down'):
            ...
            self._window._mainSplit.setSplitPosition(250, ISizeable.UNITS_PIXELS)
...
class FeatureTableClickListener(IItemClickListener):
    ...
    def itemClick(self, event):
        f = event.getItemId()
        if event.getButton() == ItemClickEvent.BUTTON_MIDDLE or event.isCtrlKey() or event.isShiftKey():
            er = ExternalResource(self.getURL() + '#' + f.getFragmentName())
...
if __name__ == '__main__':
    ...
    muntjac(SamplerApplication, nogui=True, forever=True, debug=True, widgetset='com.vaadin.demo.sampler.gwt.SamplerWidgetSet')