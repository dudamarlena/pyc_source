...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/muntjac/ui/abstract_select.py
...
from muntjac.event.dd.acceptcriteria.contains_data_flavor import ContainsDataFlavor
...
class AbstractSelect(AbstractField, container.IContainer, container.IViewer, container.IPropertySetChangeListener, container.IPropertySetChangeNotifier, container.IItemSetChangeNotifier, container.IItemSetChangeListener):
    ...
    def paintContent(self, target):
        ...
        if self.isMultiSelect():
            selectedKeys = [
             None] * len(self.getValue())
        else:
            if self.getValue() is None and self.getNullSelectionItemId() is None:
                selectedKeys = [
                 None] * 0
            else:
                selectedKeys = [
            ...
            keyIndex = 0
            ids = self.getItemIds()
            if self.isNullSelectionAllowed() and self.getNullSelectionItemId() is not None and self.getNullSelectionItemId() not in ids:
            ...
            i = self.getItemIds()
            for idd in i:
                if not self.isNullSelectionAllowed() and idd is not None and idd == self.getNullSelectionItemId():
                    continue
    ...
    def changeVariables(self, source, variables):
        ...
        if 'selected' in variables:
            ...
            if self.isMultiSelect():
                ...
                if not self.isNullSelectionAllowed() and (len(ka) == 0 or ka[0] is None or ka[0] == self.getNullSelectionItemId()):
                    self.requestRepaint()
                    return
                if len(ka) == 0:
    ...
    def addContainerProperty(self, propertyId, typ, defaultValue):
        ...
        if retval and not isinstance(self.items, container.IPropertySetChangeNotifier):
    ...
    def addItem(self, itemId=None):
        ...
        if itemId is None:
            ...
            if retval is not None and not isinstance(self.items, container.IItemSetChangeNotifier):
                self.fireItemSetChange()
            return retval
    ...
    def removeItem(self, itemId):
        ...
        if retval and not isinstance(self.items, container.IItemSetChangeNotifier):
            self.fireItemSetChange()
        return retval
    ...
    def setContainerDataSource(self, newDataSource):
        ...
        if self.items != newDataSource:
            if self.items is not None:
                if isinstance(self.items, container.IItemSetChangeNotifier):
                    self.items.removeListener(self, container.IItemSetChangeListener)
                if isinstance(self.items, container.IPropertySetChangeNotifier):
                    self.items.removeListener(self, container.IPropertySetChangeListener)
            self.items = newDataSource
            self.itemIdMapper.removeAll()
            if self.items is not None:
                if isinstance(self.items, container.IItemSetChangeNotifier):
                    self.items.addListener(self, container.IItemSetChangeListener)
                if isinstance(self.items, container.IPropertySetChangeNotifier):
                    self.items.addListener(self, container.IPropertySetChangeListener)
            self.setValue(None)
            self.requestRepaint()
        return
    ...
    def getItemCaption(self, itemId):
        ...
        if itemId is None:
            ...
            if test == self.ITEM_CAPTION_MODE_ID:
                ...
                if test == self.ITEM_CAPTION_MODE_INDEX:
    ...
    def addListener(self, listener, iface=None):
        ...
        """
        if isinstance(listener, container.IItemSetChangeListener) and (iface is None or issubclass(iface, container.IItemSetChangeListener)):
    ...
    def removeListener(self, listener, iface=None):
        ...
        """
        if isinstance(listener, container.IItemSetChangeListener) and (iface is None or iface == container.IItemSetChangeListener):
            if listener in self._itemSetEventListeners:
                self._itemSetEventListeners.remove(listener)
        if isinstance(listener, container.IPropertySetChangeListener) and (iface is None or iface == container.IPropertySetChangeListener):
    ...
    def isEmpty(self):
        ...
        if not self._multiSelect:
            ...
            value = self.getValue()
            return super(AbstractSelect, self).isEmpty() or isinstance(value, list) and len(value) == 0
    ...
    def addNewItem(self, newItemCaption):
        ...
        if self._select.addItem(newItemCaption) is not None:
            if self._select.getItemCaptionPropertyId() is not None:
                try:
                    prop = self._select.getContainerProperty(newItemCaption, self._select.getItemCaptionPropertyId())
...
class CaptionChangeListener(item.IPropertySetChangeListener, prop.IValueChangeListener):
    ...
    def addNotifierForItem(self, itemId):
        ...
        if test == self._select.ITEM_CAPTION_MODE_ITEM:
            ...
            if isinstance(i, item.IPropertySetChangeNotifier):
                i.addListener(self._select.getCaptionChangeListener(), item.IPropertySetChangeListener)
            ...
            if pids is not None:
                for pid in pids:
                    ...
                    if p is not None and isinstance(p, prop.IValueChangeNotifier):
                        p.addListener(self._select.getCaptionChangeListener(), prop.IValueChangeListener)
                        self._captionChangeNotifiers.add(p)

        elif test == self._select.ITEM_CAPTION_MODE_PROPERTY:
            p = self._select.getContainerProperty(itemId, self._select.getItemCaptionPropertyId())
            if p is not None and isinstance(p, prop.IValueChangeNotifier):
                p.addListener(self._select.getCaptionChangeListener(), prop.IValueChangeListener)
                self._captionChangeNotifiers.add(p)
        return
    ...
    def clear(self):
        for notifier in self._captionChangeNotifiers:
            if isinstance(notifier, item.IPropertySetChangeNotifier):
                notifier.removeListener(self._select.getCaptionChangeListener(), item.IPropertySetChangeListener)
            else:
                notifier.removeListener(self._select.getCaptionChangeListener(), prop.IValueChangeListener)
