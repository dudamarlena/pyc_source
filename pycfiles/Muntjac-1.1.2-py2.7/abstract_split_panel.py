...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/muntjac/ui/abstract_split_panel.py
...
class AbstractSplitPanel(AbstractLayout):
    ...
    def setSplitPosition(self, *args):
        ...
        if nargs == 1:
            ...
            if unit != self.UNITS_PERCENTAGE and unit != self.UNITS_PIXELS:
                raise ValueError, 'Only percentage and pixel units are allowed'
            self._pos = pos
            self._posUnit = unit
    ...
    def addListener(self, listener, iface=None):
        if isinstance(listener, ISplitterClickListener) and (iface is None or issubclass(iface, ISplitterClickListener)):
            self.registerListener(self._SPLITTER_CLICK_EVENT, SplitterClickEvent, listener, ISplitterClickListener.clickMethod)
    ...
    def addCallback(self, callback, eventType=None, *args):
        ...
        if issubclass(eventType, SplitterClickEvent):
            ...
            super(AbstractSplitPanel, self).addCallback(callback, eventType, *args)
        return

    def removeListener(self, listener, iface=None):
        if isinstance(listener, ISplitterClickListener) and (iface is None or issubclass(iface, ISplitterClickListener)):
            self.withdrawListener(self._SPLITTER_CLICK_EVENT, SplitterClickEvent, listener)
    ...
    def removeCallback(self, callback, eventType=None):
        if eventType is None:
            eventType = callback._eventType
        if issubclass(eventType, SplitterClickEvent):
            self.withdrawCallback(SplitterClickEvent, callback, self._SPLITTER_CLICK_EVENT)
        else:
