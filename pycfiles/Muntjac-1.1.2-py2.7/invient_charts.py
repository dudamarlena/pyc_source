...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/muntjac/addon/invient/invient_charts.py
...
from muntjac.addon.invient.invient_charts_config import BaseLineConfig, PointConfig, SeriesConfig
from muntjac.addon.invient.invient_charts_util import writeTitleConfig, writeSubtitleConfig, writeCreditConfig, writeLegendConfig, writeTooltipConfig, writeGeneralChartConfig, writeSeriesConfigPerSeriesType, writeXAxes, writeYAxes, writeChartLabelConfig, writeSeries, writeChartDataUpdates, getDate
...
class InvientCharts(AbstractComponent):
    ...
    def __init__(self, chartConfig):
        ...
        if chartConfig is None:
            raise ValueError('The chart cannot be created without chartConfig argument.')
    ...
    def paintContent(self, target):
        ...
        if self._chartConfig is not None:
            ...
            writeGeneralChartConfig(target, self._chartConfig.getGeneralChartConfig())
            writeSeriesConfigPerSeriesType(target, self._chartConfig.getSeriesConfig())
        ...
        writeSeries(target, self._chartConfig.getGeneralChartConfig().getType(), self._chartSeries, self._chartConfig.getXAxes(), self._chartConfig.getYAxes())
    ...
    def paintChartEvents(self, target):
        ...
        if self._chartAddSeriesListener is not None and len(self._chartAddSeriesListener) > 0:
    ...
    def paintSeriesEvents(self, target, seriesType):
        ...
        if seriesType in self._seriesClickListeners and len(self._seriesClickListeners[seriesType]) > 0:
            target.addAttribute('click', True)
        if seriesType in self._seriesHideListeners and len(self._seriesHideListeners[seriesType]) > 0:
            target.addAttribute('hide', True)
        if seriesType in self._seriesShowListeners and len(self._seriesShowListeners[seriesType]) > 0:
            target.addAttribute('show', True)
        if seriesType in self._seriesLegendItemClickListeners and len(self._seriesLegendItemClickListeners[seriesType]) > 0:
    ...
    def paintPointEvents(self, target, seriesType):
        ...
        if seriesType in self._pointClickListeners and len(self._pointClickListeners[seriesType]) > 0:
            target.addAttribute('click', True)
        if seriesType in self._pointRemoveListeners and len(self._pointRemoveListeners[seriesType]) > 0:
            target.addAttribute('remove', True)
        if seriesType in self._pointSelectListeners and len(self._pointSelectListeners[seriesType]) > 0:
            target.addAttribute('select', True)
        if seriesType in self._pointUnselectListeners and len(self._pointUnselectListeners[seriesType]) > 0:
            target.addAttribute('unselect', True)
        if SeriesType.PIE == seriesType and len(self._pieChartLegendItemClickListener) > 0:
            target.addAttribute('legendItemClick', True)
    ...
    def changeVariables(self, source, variables):
        if 'event' in variables:
            ...
            if eventName.lower() == ('addSeries').lower():
                ...
                self.fireChartClick(DecimalPoint(self, xAxisPos, yAxisPos), mousePosition)
                ...
                self.fireChartZoom(ChartArea(xAxisMin, xAxisMax, yAxisMin, yAxisMax))
                ...
                mousePosition = self.getClickPosition(eventData)
                self.fireSeriesClick(self.getSeriesFromEventData(pointEventData.getSeriesName()), self.getPointFromEventData(pointEventData), mousePosition)
                ...
                mousePosition = self.getClickPosition(eventData)
                pointEventData = self.getPointEventData(eventData)
                self.firePointClick(pointEventData.getCategory(), self.getPointFromEventData(pointEventData), mousePosition)
            elif eventName.lower() == ('pointSelect').lower():
                pointEventData = self.getPointEventData(eventData)
                self.firePointSelect(pointEventData.getCategory(), self.getPointFromEventData(pointEventData))
            elif eventName.lower() == ('pointUnselect').lower():
                pointEventData = self.getPointEventData(eventData)
                self.firePointUnselect(pointEventData.getCategory(), self.getPointFromEventData(pointEventData))
            elif eventName.lower() == ('pointRemove').lower():
                pointEventData = self.getPointEventData(eventData)
                self.firePointRemove(pointEventData.getCategory(), self.getPointFromEventData(pointEventData))
        ...
        if series is not None:
            if isinstance(series, XYSeries):
                for point in series.getPoints():
                    if point.getY() is not None and cmp(point.getY(), eventData.getPointY()) == 0 and point.getX() is not None and cmp(point.getX(), eventData.getPointX()) == 0:
                        return point

            else:
                for point in series.getPoints():
                    if point.getY() is not None and cmp(point.getY(), eventData.getPointY()) == 0 and point.getX() is not None and getDate(point.getX(), series.isIncludeTime()) == eventData.getPointX():
                        return point
    ...
    def addListener(self, listener, seriesTypes=None):
        ...
        if seriesTypes is None:
            if isinstance(listener, ChartAddSeriesListener):
                ...
                self.registerListener(ChartAddSeriesEvent, listener, _CHART_ADD_SERIES_METHOD)
            elif isinstance(listener, ChartClickListener):
                self._chartClickListener.add(listener)
                self.registerListener(ChartClickEvent, listener, _CHART_CLICK_METHOD)
            elif isinstance(listener, ChartResetZoomListener):
                self._chartResetZoomListener.add(listener)
                self.registerListener(ChartResetZoomEvent, listener, _CHART_RESET_ZOOM_METHOD)
            elif isinstance(listener, ChartSVGAvailableListener):
                if self._svgAvailableListener is not None and self._svgAvailableListener != listener:
                    self.removeListener(self._svgAvailableListener)
                self._svgAvailableListener = listener
                self.registerListener(ChartSVGAvailableEvent, self._svgAvailableListener, _CHART_SVG_AVAILABLE_METHOD)
                self._isRetrieveSVG = True
                self.requestRepaint()
            elif isinstance(listener, ChartZoomListener):
                self._chartZoomListener.add(listener)
                self.registerListener(ChartZoomEvent, listener, _CHART_ZOOM_METHOD)
            elif isinstance(listener, PieChartLegendItemClickListener):
                self._pieChartLegendItemClickListener.add(listener)
                self.registerListener(PieChartLegendItemClickEvent, listener, _LEGENDITEM_CLICK_METHOD)
            else:
                super(InvientCharts, self).addListener(listener, seriesTypes)
        elif isinstance(listener, PointClickListener):
            if len(seriesTypes) == 0:
                seriesTypes = [
                 SeriesType.COMMONSERIES]
            ...
            self.registerListener(PointClickEvent, listener, _POINT_CLICK_METHOD)
            ...
            self.registerListener(PointRemoveEvent, listener, _POINT_REMOVE_METHOD)
            ...
            self.registerListener(PointSelectEvent, listener, _POINT_SELECT_METHOD)
            ...
            self.registerListener(PointUnselectEvent, listener, _POINT_UNSELECT_METHOD)
            ...
            self.registerListener(SeriesClickEvent, listener, _SERIES_CLICK_METHOD)
            ...
            self.registerListener(SeriesHideEvent, listener, _SERIES_HIDE_METHOD)
            ...
            self.registerListener(SeriesLegendItemClickEvent, listener, _SERIES_LEGENDITEM_CLICK_METHOD)
            ...
            self.registerListener(SeriesShowEvent, listener, _SERIES_SHOW_METHOD)
        else:
            iface = seriesTypes
    ...
    def removeListener(self, listener, seriesTypes=None):
        ...
        if seriesTypes is None:
            if isinstance(listener, ChartAddSeriesListener):
                ...
                self.withdrawListener(ChartAddSeriesEvent, listener, _CHART_ADD_SERIES_METHOD)
            elif isinstance(listener, ChartClickListener):
                self._chartClickListener.remove(listener)
                self.withdrawListener(ChartClickEvent, listener, _CHART_CLICK_METHOD)
            elif isinstance(listener, ChartResetZoomListener):
                self._chartResetZoomListener.remove(listener)
                self.withdrawListener(ChartResetZoomEvent, listener, _CHART_RESET_ZOOM_METHOD)
            elif isinstance(listener, ChartSVGAvailableListener):
                if self._svgAvailableListener == listener:
                    self.withdrawListener(ChartSVGAvailableEvent, listener, _CHART_SVG_AVAILABLE_METHOD)
                    self._isRetrieveSVG = False
                    self._svgAvailableListener = None
            elif isinstance(listener, ChartZoomListener):
                self._chartZoomListener.remove(listener)
                self.withdrawListener(ChartZoomEvent, listener, _CHART_ZOOM_METHOD)
            elif isinstance(listener, PieChartLegendItemClickListener):
                self._pieChartLegendItemClickListener.remove(listener)
                self.withdrawListener(PieChartLegendItemClickEvent, listener, _LEGENDITEM_CLICK_METHOD)
            else:
            ...
            self.withdrawListener(PointClickEvent, listener, _POINT_CLICK_METHOD)
        elif isinstance(listener, PointRemoveListener):
            if len(seriesTypes) == 0:
                seriesTypes = [
                 SeriesType.COMMONSERIES]
            ...
            self.withdrawListener(PointRemoveEvent, listener, _POINT_REMOVE_METHOD)
        elif isinstance(listener, PointSelectListener):
            if len(seriesTypes) == 0:
                seriesTypes = [
                 SeriesType.COMMONSERIES]
            ...
            self.withdrawListener(PointSelectEvent, listener, _POINT_SELECT_METHOD)
        elif isinstance(listener, PointUnselectListener):
            if len(seriesTypes) == 0:
                seriesTypes = [
                 SeriesType.COMMONSERIES]
            ...
            self.withdrawListener(PointUnselectEvent, listener, _POINT_UNSELECT_METHOD)
        elif isinstance(listener, SeriesClickListerner):
            if len(seriesTypes) == 0:
                seriesTypes = [
                 SeriesType.COMMONSERIES]
            ...
            self.withdrawListener(SeriesClickEvent, listener, _SERIES_CLICK_METHOD)
        elif isinstance(listener, SeriesHideListerner):
            if len(seriesTypes) == 0:
                seriesTypes = [
                 SeriesType.COMMONSERIES]
            ...
            self.withdrawListener(SeriesHideEvent, listener, _SERIES_HIDE_METHOD)
        elif isinstance(listener, SeriesLegendItemClickListerner):
            if len(seriesTypes) == 0:
                seriesTypes = [
                 SeriesType.COMMONSERIES]
            ...
            self.withdrawListener(SeriesLegendItemClickEvent, listener, _SERIES_LEGENDITEM_CLICK_METHOD)
        elif isinstance(listener, SeriesShowListerner):
            if len(seriesTypes) == 0:
                seriesTypes = [
                 SeriesType.COMMONSERIES]
            ...
            self.withdrawListener(SeriesShowEvent, listener, _SERIES_SHOW_METHOD)
        else:
            iface = seriesTypes
    ...
    def setAxisInSeriesIfNotSetAlready(self, series):
        if self.getConfig() is not None:
            if series.getXAxis() is None and self.getConfig().getXAxes() is not None and len(self.getConfig().getXAxes()) > 0:
                series.setXAxis(iter(self.getConfig().getXAxes()).next())
            if series.getYAxis() is None and self.getConfig().getYAxes() is not None and len(self.getConfig().getYAxes()) > 0:
                series.setYAxis(iter(self.getConfig().getYAxes()).next())
        return
            ...
            if seriesCURSet is None or len(seriesCURSet) == 0:
                ...
                for seriesCUR in seriesCURItr:
                    if seriesCUR.getName() == newSeriesCUR.getName():
                        if SeriesCURType.REMOVE == newSeriesCUR.getType() and SeriesCURType.ADD == seriesCUR.getType():
                            seriesCURSet.remove(seriesCUR)
                            return False
                        if SeriesCURType.UPDATE == newSeriesCUR.getType() and SeriesCURType.ADD == seriesCUR.getType():
                            return False
                        if SeriesCURType.REMOVE == newSeriesCUR.getType() and SeriesCURType.UPDATE == seriesCUR.getType():
                            seriesCURSet.remove(seriesCUR)
                            break
    ...
    def addSeriesPointAddedOperation(self, seriesName, point):
        ...
        if seriesCURSet is None or len(seriesCURSet) == 0:
            ...
            if lastSeriesCur.getType() == SeriesCURType.UPDATE and not lastSeriesCur.isReloadPoints():
...
class MousePosition(object):
    ...
    def __str__(self):
        return 'MousePosition [mouseX=' + str(self._mouseX) + ', mouseY=' + str(self._mouseY) + ']'
...
class PointEventData(object):
    ...
    def __str__(self):
        return 'PointEventData [seriesName=' + self._seriesName + ', category=' + self._category + ', pointX=' + str(self._pointX) + ', pointY=' + str(self._pointY) + ']'
...
_SERIES_LEGENDITEM_CLICK_METHOD = getattr(SeriesLegendItemClickListerner, 'seriesLegendItemClick')
...
_LEGENDITEM_CLICK_METHOD = getattr(PieChartLegendItemClickListener, 'legendItemClick')
...
class ChartClickEvent(ComponentEvent):
    ...
    def __str__(self):
        return 'ChartClickEvent [point=' + str(self._point) + ', mousePosition=' + str(self._mousePosition) + ']'
...
class ChartArea(object):
    ...
    def __str__(self):
        return 'ChartSelectedArea [xAxisMin=' + str(self._xAxisMin) + ', xAxisMax=' + str(self._xAxisMax) + ', yAxisMin=' + str(self._yAxisMin) + ', yAxisMax=' + str(self._yAxisMax) + ']'
...
class Point(object):
    ...
    def __str__(self):
        return 'Point [id=' + self._id + ', name=' + self._name + ', series=' + self._series.getName() + ', config=' + str(self._config) + ']'
...
class DecimalPoint(Point):
    ...
    def __str__(self):
        return 'DecimalPoint [x=' + self._x + ', y=' + self._y + ', id=' + self.getId() + ', name=' + self.getName() + ', seriesName=' + (self._series.getName() if self._series is not None else '') + ']'
...
class DateTimePoint(Point):
    ...
    def __str__(self):
        return 'DateTimePoint [x=' + getDate(self._x, self._series.isIncludeTime() if self._series is not None else False) + ', y=' + str(self._y) + ', id=' + self.getId() + ', name=' + self.getName() + ', seriesName=' + (self._series.getName() if self._series is not None else '') + ']'
    ...
    def __eq__(self, obj):
        if self is obj:
            ...
            if self._x is None or other._x is None:
                return False
            pointIncludeTime = self.getSeries().isIncludeTime() if isinstance(self.getSeries(), DateTimeSeries) else False
            pointOtherIncludeTime = other.getSeries().isIncludeTime() if isinstance(other.getSeries(), DateTimeSeries) else False
...
class Series(object):
    ...
    def __hash__(self):
        prime = 31
        result = 1
        result = prime * result + (0 if self._name is None else hash(self._name))
        return result
    ...
    def __str__(self):
        return 'Series [points=' + self._points + ', name=' + self._name + ', type=' + str(self._type) + ', stack=' + self._stack + ', xAxis=' + str(self._xAxis) + ', yAxis=' + str(self._yAxis) + ', config=' + str(self._config) + ']'
...
class DateTimeSeries(Series):
    ...
    def updatePointXValuesIfNotPresent(self):
        ...
        if isinstance(super(DateTimeSeries, self).getConfig(), BaseLineConfig):
            ...
            if config.getPointInterval() is not None:
                pointInterval = config.getPointInterval()
        prevDate = datetime.fromtimestamp(pointStart / 1000.0)
        count = 0
        for point in self.getPoints():
            if point.getX() is None or point.getX() is not None and point.isAutosetX():
                point.setAutosetX(True)
    ...
    def __str__(self):
        return 'DateTimeSeries [includeTime=' + self._includeTime + ', getConfig()=' + str(self._invientCharts.getConfig()) + ', getName()=' + self.getName() + ', getType()=' + str(self.getType()) + ', getStack()=' + self.getStack() + ', getXAxis()=' + str(self.getXAxis()) + ', getYAxis()=' + str(self.getYAxis()) + ']'
...
class SeriesCUR(object):
    ...
    def __str__(self):
        return 'SeriesCUR [type=' + str(self._type) + ', name=' + self._name + ', reloadPoints=' + str(self._reloadPoints) + ', pointsAdded=' + str(self._pointsAdded) + ', pointsRemoved=' + str(self._pointsRemoved) + ']'
