...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/muntjac/demo/sampler/FeatureView.py
...
from muntjac.api import HorizontalLayout, Label, VerticalLayout, Panel, NativeButton, Link, Window
...
class FeatureView(HorizontalLayout):
    ...
    def setFeature(self, feature):
        ...
        if feature != self._currentFeature:
            ...
            if desc is not None and desc != '':
                l = Label('<div class="outer-deco"><div class="deco"><span class="deco"></span>' + desc + '</div></div>', Label.CONTENT_XHTML)
            ...
            er = ExternalResource(self.getApplication().getURL() + 'src/' + self._currentFeature.getFragmentName())
            self._showSrc.setResource(er)
            resources = feature.getRelatedResources()
            if resources is not None:
                res = VerticalLayout()
                self.caption = Label('<span>Additional Resources</span>', Label.CONTENT_XHTML)
            ...
            if apis is not None:
                api = VerticalLayout()
                self.caption = Label('<span>API Documentation</span>', Label.CONTENT_XHTML)
                self.caption.setStyleName('section')
                self.caption.setWidth('100%')
            ...
            if features is not None:
                rel = VerticalLayout()
                self.caption = Label('<span>Related Samples</span>', Label.CONTENT_XHTML)
                self.caption.setStyleName('section')
                self.caption.setWidth('100%')
                ...
                for c in features:
                    ...
                    if f is not None:
                        er = ExternalResource(self.getApplication().getURL() + '#' + f.getFragmentName())
                        ...
                        al.setIcon(ic)
                        al.addListener(LinkListener(self, f), ILinkActivatedListener)
                        rel.addComponent(al)
...
class ShowSrcListener(ILinkActivatedListener):
    ...
    def linkActivated(self, event):
        if not event.isLinkOpened():
            self._view.showSource(self._view._currentFeature.getSource())
...
class LinkListener(ILinkActivatedListener):
    ...
    def linkActivated(self, event):
        if event.isLinkOpened():
            self._view.getWindow().showNotification(self._feature.getName() + ' opened if new window/tab')
        else:
