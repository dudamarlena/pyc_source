...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/muntjac/ui/tree.py
...
from muntjac.terminal.gwt.client.ui.v_tree import VTree
...
class Tree(AbstractSelect, container.IHierarchical, action.IContainer, IItemClickSource, IItemClickNotifier, IDragSource, IDropTarget):
    ...
    def changeVariables(self, source, variables):
        ...
        if 'selected' in variables and self.isMultiSelect() and self._multiSelectMode == MultiSelectMode.DEFAULT:
    ...
    def paintContent(self, target):
        ...
        if self._partialUpdate:
            ...
            if self._dragMode != TreeDragMode.NONE:
                target.addAttribute('dragMode', TreeDragMode.ordinal(self._dragMode))
            ...
            while len(iteratorStack) > 0:
                ...
                try:
                    ...
                    if self._itemDescriptionGenerator is not None:
                        description = self._itemDescriptionGenerator.generateDescription(self, itemId, None)
                    ...
                    if self.isExpanded(itemId) and self.hasChildren(itemId) and self.areChildrenAllowed(itemId):
    ...
    def addListener(self, listener, iface):
        ...
        """
        if isinstance(listener, ICollapseListener) and (iface is None or issubclass(iface, ICollapseListener)):
            self.registerListener(CollapseEvent, listener, COLLAPSE_METHOD)
        if isinstance(listener, IExpandListener) and (iface is None or issubclass(iface, IExpandListener)):
            self.registerListener(ExpandEvent, listener, EXPAND_METHOD)
        if isinstance(listener, IItemClickListener) and (iface is None or issubclass(iface, IItemClickListener)):
            self.registerListener(VTree.ITEM_CLICK_EVENT_ID, ItemClickEvent, listener, ITEM_CLICK_METHOD)
    ...
    def addCallback(self, callback, eventType=None, *args):
        ...
        if issubclass(eventType, CollapseEvent):
            ...
            self.registerCallback(ItemClickEvent, callback, VTree.ITEM_CLICK_EVENT_ID, *args)
        else:
    ...
    def removeListener(self, listener, iface=None):
        ...
        """
        if isinstance(listener, ICollapseListener) and (iface is None or issubclass(iface, ICollapseListener)):
            self.withdrawListener(CollapseEvent, listener, COLLAPSE_METHOD)
        if isinstance(listener, IExpandListener) and (iface is None or issubclass(iface, IExpandListener)):
            self.withdrawListener(ExpandEvent, listener, EXPAND_METHOD)
        if isinstance(listener, IItemClickListener) and (iface is None or issubclass(iface, IItemClickListener)):
            self.withdrawListener(VTree.ITEM_CLICK_EVENT_ID, ItemClickEvent, listener)
    ...
    def removeCallback(self, callback, eventType=None):
        ...
        if issubclass(eventType, CollapseEvent):
            ...
            self.withdrawCallback(ItemClickEvent, callback, VTree.ITEM_CLICK_EVENT_ID)
        else:
...
class TreeTargetDetails(AbstractSelectTargetDetails):
    ...
    def getItemIdInto(self):
        ...
        if self.areChildrenAllowed(itemIdOver) and self.getDropLocation() == VerticalDropLocation.MIDDLE:
            return itemIdOver
    ...
    def accept(self, dragEvent):
        try:
            ...
            if eventDetails.getItemIdOver() is not None:
                ...
                i = 0
                while itemId is not None and self._depthToCheck == -1 or i <= self._depthToCheck:
