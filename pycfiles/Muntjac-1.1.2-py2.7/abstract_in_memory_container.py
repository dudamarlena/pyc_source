...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/muntjac/data/util/abstract_in_memory_container.py
...
from muntjac.data.container import IIndexed, IItemSetChangeNotifier, ISortable
from muntjac.data.util.abstract_container import AbstractContainer

class AbstractInMemoryContainer(AbstractContainer, IItemSetChangeNotifier, IIndexed):
    ...
    def addItemAt(self, index, newItemId=None):
        raise NotImplementedError('Adding items not supported. Override the relevant addItem*() methods if required as specified in AbstractInMemoryContainer docstring.')
    ...
    def removeItem(self, itemId):
        raise NotImplementedError('Removing items not supported. Override the removeItem() method if required as specified in AbstractInMemoryContainer docstring.')

    def removeAllItems(self):
        raise NotImplementedError('Removing items not supported. Override the removeAllItems() method if required as specified in AbstractInMemoryContainer docstring.')
    ...
    def addCallback(self, callback, eventType=None, *args):
        super(AbstractInMemoryContainer, self).addCallback(callback, eventType, *args)

    def removeListener(self, listener, iface=None):
        super(AbstractInMemoryContainer, self).removeListener(listener, iface)

    def removeCallback(self, callback, eventType=None):
        super(AbstractInMemoryContainer, self).removeCallback(callback, eventType)
    ...
    def doFilterContainer(self, hasFilters):
        ...
        if not hasFilters:
            ...
            try:
                ...
                return wasUnfiltered and len(self.getAllItemIds()) > 0 or not equal
    ...
    def getSortablePropertyIds(self):
        ...
        for propertyId in self.getContainerPropertyIds():
            ...
            if hasattr(propertyType, '__eq__') or isinstance(propertyType, (int, long, float, bool)):
    ...
    def internalAddAt(self, position, itemId, item):
        ...
        """
        if position < 0 or position > len(self.getAllItemIds()) or itemId is None or item is None:
            return
    ...
    def internalAddItemAt(self, index, newItemId, item, fltr):
        ...
        if index == 0:
            ...
            return self.internalAddItemAfter(self.getIdByIndex(index - 1), newItemId, item, fltr)
            return
