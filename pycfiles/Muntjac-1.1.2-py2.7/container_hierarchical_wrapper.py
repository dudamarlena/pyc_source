...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/muntjac/data/util/container_hierarchical_wrapper.py
...
from muntjac.data.container import IContainer, IHierarchical, IItemSetChangeListener, IItemSetChangeNotifier, IPropertySetChangeListener, IPropertySetChangeNotifier, IItemSetChangeEvent, IPropertySetChangeEvent
from muntjac.data.util.hierarchical_container import HierarchicalContainer

class ContainerHierarchicalWrapper(IHierarchical, IContainer, IItemSetChangeNotifier, IPropertySetChangeNotifier):
    """A wrapper class for adding external hierarchy to containers not
    ...
    def updateHierarchicalWrapper(self):
        ...
        """
        if not self._hierarchical:
            if self._noChildrenAllowed is None or self._parent is None or self._children is None or self._roots is None:
                ...
                basedOnOrderFromWrappedContainer = ListedItemsFirstComparator(itemIds)
                s = set()
    ...
    def addCallback(self, callback, eventType=None, *args):
        ...
        if issubclass(eventType, IItemSetChangeEvent):
            ...
            super(ContainerHierarchicalWrapper, self).addCallback(callback, eventType, *args)
        return
    ...
    def removeCallback(self, callback, eventType=None):
        ...
        if issubclass(eventType, IItemSetChangeEvent):
            ...
            super(ContainerHierarchicalWrapper, self).removeCallback(callback, eventType)
        return


class PiggybackListener(IContainer, IPropertySetChangeListener, IItemSetChangeListener):
