...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/muntjac/data/util/propertyset_item.py
# Compiled at: 2013-04-04 15:36:37
from muntjac.data.item import IItem, IPropertySetChangeEvent, IPropertySetChangeNotifier, IPropertySetChangeListener
from muntjac.util import EventObject

class PropertysetItem(IItem, IPropertySetChangeNotifier):
    ...
    def addCallback(self, callback, eventType=None, *args):
        ...
        if issubclass(eventType, IPropertySetChangeEvent):
            ...
            super(PropertysetItem, self).addCallback(callback, eventType, *args)
        return
    ...
    def __eq__(self, obj):
        ...
        if other._propertySetChangeListeners != self._propertySetChangeListeners:
            thisEmpty = self._propertySetChangeListeners is None or len(self._propertySetChangeListeners) == 0
            otherEmpty = other.propertySetChangeListeners is None or len(other.propertySetChangeListeners) == 0
    ...
    def __hash__(self):
        return (0 if self._list is None else hash(self._list)) ^ (0 if self._map is None else hash(self._map)) ^ (0 if self._propertySetChangeListeners is None or len(self._propertySetChangeListeners) == 0 else hash(self._propertySetChangeListeners))
