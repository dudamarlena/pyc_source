...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/muntjac/ui/select.py
...
from muntjac.event.field_events import FocusEvent, BlurEvent, IBlurListener, IFocusListener

class Select(abstract_select.AbstractSelect, abstract_select.IFiltering, field_events.IBlurNotifier, field_events.IFocusNotifier):
        ...
        if self.isMultiSelect():
            ...
            if self.isMultiSelect():
                ...
                if self._currentPage < 0:
                    ...
                    self._currentPage = 0
                    self._filterstring = ''
                nullFilteredOut = self._filterstring is not None and self._filterstring != '' and self._filteringMode != self.FILTERINGMODE_OFF
                ...
                while True:
                    try:
                        idd = i.next()
                        if not self.isNullSelectionAllowed() and idd is not None and idd == self.getNullSelectionItemId() and not self.isSelected(idd):
                            continue
    ...
    def getOptionsWithFilter(self, needNullSelectOption):
        ...
        if self.pageLength == 0:
            ...
            if not isinstance(container, IFilterable) or not isinstance(container, IIndexed) or self.getItemCaptionMode() != self.ITEM_CAPTION_MODE_PROPERTY:
                return
            ...
            if self.isScrollToSelectedItem() and not self._optionRequest and not self.isMultiSelect() and selection is not None:
    ...
    def getFilteredOptions(self):
        ...
        """
        if self._filterstring is None or self._filterstring == '' or self.FILTERINGMODE_OFF == self._filteringMode:
            self._prevfilterstring = None
            ...
            if self._filterstring == self._prevfilterstring:
                return self._filteredOptions
            if self._prevfilterstring is not None and self._filterstring.startswith(self._prevfilterstring):
            ...
            for itemId in items:
                ...
                if test == self.FILTERINGMODE_CONTAINS:
                    ...
                    pass
                elif caption.startswith(self._filterstring):
                    self._filteredOptions.append(itemId)
