...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/muntjac/demo/sampler/features/dragndrop/DragDropHtml5FromDesktopExample.py
...
from muntjac.api import VerticalLayout, Label, CssLayout, Panel, ProgressIndicator, Window
...
class ImageDropBox(DragAndDropWrapper, IDropHandler):
        ...
        if files is not None:
            for html5File in files:
                ...
                if html5File.getFileSize() > self._FILE_SIZE_LIMIT:
                    self._component.getWindow().showNotification('File rejected. Max 2Mb files are accepted by Sampler', Notification.TYPE_WARNING_MESSAGE)
                else:
                    bas = StringIO()
    ...
    def showFile(self, name, typ, bas):
        ...
        resource = StreamResource(streamSource, name, self._component.getApplication())
...
class FileStreamVariable(IStreamVariable):
    ...
    def streamingFinished(self, event):
        ...
        self.showFile(self._fileName, self._html5File.getType(), self._bas)
