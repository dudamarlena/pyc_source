...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/muntjac/test/server/data/util/abstract_container_test.py
...
class AbstractContainerTest(TestCase):
    ...
    def validateContainer(self, container, expectedFirstItemId, expectedLastItemId, itemIdInSet, itemIdNotInSet, checkGetItemNull, expectedSize):
        ...
        for propId in container.getContainerPropertyIds():
            if checkGetItemNull:
                self.assertIsNone(container.getContainerProperty(itemIdNotInSet, propId))
            self.assertIsNotNone(container.getContainerProperty(itemIdInSet, propId))
        ...
        if indexed is not None:
            ...
            self.assertEquals(itemIdList[(len(itemIdList) - 2)], indexed.prevItemId(last))
    ...
    def _testBasicContainerOperations(self, container):
        ...
        self.validateContainer(container, self.sampleData[0], self.sampleData[(len(self.sampleData) - 1)], self.sampleData[10], 'abc', True, len(self.sampleData))
    ...
    def _testContainerIndexed(self, container, itemId, itemPosition, testAddEmptyItemAt, newItemId, testAddItemAtWithId):
        self.initializeContainer(container)
        ...
        if testAddEmptyItemAt:
            ...
            self.assertEquals(len(container) - 1, container.indexOfId(newLastId))
            self.assertEquals(itemPosition + 2, container.indexOfId(itemId))
            self.assertEquals(newLastId, container.lastItemId())
            self.assertEquals(newLastId, container.getIdByIndex(len(container) - 1))
            self.assertEquals(itemId, container.getIdByIndex(itemPosition + 2))
            ...
            self.assertFalse(container.removeItem(addedId), 'Removing non-existing item should indicate failure')
        if testAddItemAtWithId:
            ...
            self.assertEquals(len(container) - 1, container.indexOfId(newItemId))
            self.assertEquals(itemPosition, container.indexOfId(itemId))
            self.assertEquals(newItemId, container.lastItemId())
            self.assertEquals(newItemId, container.getIdByIndex(len(container) - 1))
            self.assertEquals(itemId, container.getIdByIndex(itemPosition))
            self.assertTrue(container.removeItem(newItemId))
            self.assertFalse(container.containsId(newItemId))
    ...
    def _testContainerFiltering(self, container):
        ...
        self.validateContainer(container, 'com.vaadin.data.BufferedValidatable', 'com.vaadin.ui.TabSheet', 'com.vaadin.terminal.gwt.client.Focusable', 'com.vaadin.data.Buffered', self.isFilteredOutItemNull(), 20)
        container.removeAllContainerFilters()
        f = SimpleStringFilter(self.REVERSE_FULLY_QUALIFIED_NAME, 'ad', False, False)
        container.addContainerFilter(f)
        self.validateContainer(container, 'com.vaadin.data.Buffered', 'com.vaadin.terminal.gwt.server.ComponentSizeValidator', 'com.vaadin.data.util.IndexedContainer', 'com.vaadin.terminal.gwt.client.ui.VUriFragmentUtility', self.isFilteredOutItemNull(), 37)
    ...
    def _testContainerSortingAndFiltering(self, sortable):
        ...
        self.validateContainer(sortable, 'com.vaadin.data.BufferedValidatable', 'com.vaadin.ui.TableFieldFactory', 'com.vaadin.ui.TableFieldFactory', 'com.vaadin.data.util.BeanItem', self.isFilteredOutItemNull(), 20)
    ...
    def _testContainerSorting(self, container):
        ...
        sortable.sort([self.FULLY_QUALIFIED_NAME], [True])
        self.validateContainer(container, 'com.vaadin.Application', 'org.vaadin.test.LastClass', 'com.vaadin.terminal.ApplicationResource', 'blah', True, len(self.sampleData))
        sortable.sort([self.REVERSE_FULLY_QUALIFIED_NAME], [True])
        self.validateContainer(container, 'com.vaadin.terminal.gwt.server.ApplicationPortlet2', 'com.vaadin.data.util.ObjectProperty', 'com.vaadin.ui.BaseFieldFactory', 'blah', True, len(self.sampleData))
