...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/muntjac/ui/option_group.py
...
class OptionGroup(AbstractSelect, IBlurNotifier, IFocusNotifier):
        ...
        if repaintIsNotNeeded is not None and repaintIsNotNeeded is True:
            if self.isMultiSelect():
                ...
                for itemId in currentValueSet:
                    if not self.isItemEnabled(itemId) and itemId not in newValueSet:
                        self.requestRepaint()
                        return

                for itemId in newValueSet:
                    if not self.isItemEnabled(itemId) and itemId not in currentValueSet:
                        self.requestRepaint()
                        return
    ...
    def isItemEnabled(self, itemId):
        ...
        """
        if itemId is not None:
            return itemId not in self._disabledItemIds
        else:
            return True
