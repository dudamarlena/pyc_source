...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/muntjac/ui/form.py
...
class Form(AbstractField, IEditor, IBuffered, IItem, IValidatable, INotifier):
    ...
    def getErrorMessage(self):
        ...
        if self.getComponentError() is None and validationError is None and self._currentBufferedSourceException is None:
            return
        else:
    ...
    def registerField(self, propertyId, field):
        ...
        field.addListener(self._fieldValueChangeListener, IValueChangeListener)
    ...
    def removeItemProperty(self, idd):
        ...
        if field is not None:
            ...
            field.removeListener(self._fieldValueChangeListener, IValueChangeListener)
            return True
        else:
            return False
    ...
    def setItemDataSource(self, newDataSource, propertyIds=None):
        ...
        if propertyIds is None:
            if newDataSource is not None:
                self.setItemDataSource(newDataSource, newDataSource.getItemPropertyIds())
            else:
    ...
    def replaceWithSelect(self, propertyId, values, descriptions):
        ...
        if oldField is None:
            raise ValueError, "IField with given propertyid '" + str(propertyId) + "' can not be found."
        ...
        if value is not None and not found:
            if isinstance(value, (list, set)):
                for val in value:
                    ...
                    if not found:
                        raise ValueError, "Currently selected value '" + val + "' of property '" + str(propertyId) + "' was not found"

                isMultiselect = True
            else:
                raise ValueError, "Current value '" + value + "' of property '" + str(propertyId) + "' was not found"
        newField = Select()
        if isMultiselect:
        ...
        newField.addListener(self._fieldValueChangeListener, prop.IValueChangeListener)
        oldField.removeListener(self._fieldValueChangeListener, prop.IValueChangeListener)
        return newField
