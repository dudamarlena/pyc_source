...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/muntjac/ui/window.py
...
from muntjac.event.field_events import IFocusNotifier, IBlurNotifier, FocusEvent, BlurEvent, IBlurListener, IFocusListener
...
class Window(Panel, IUriHandler, IParameterHandler, IFocusNotifier, IBlurNotifier):
    ...
    def addComponent(self, c):
        if isinstance(c, Window):
            raise ValueError, 'Window cannot be added to another via addComponent. Use addWindow(Window) instead.'
    ...
    def paintContent(self, target):
        ...
        if self._scrollIntoView is not None:
            ...
            self._scrollIntoView = None
        if self.getApplication() is not None and self == self.getApplication().getMainWindow():
        ...
        if self._pendingFocus is not None:
            if self._pendingFocus.getWindow() == self or self._pendingFocus.getWindow() is not None and self._pendingFocus.getWindow().getParent() == self:
    ...
    def open(self, resource, windowName=None, width=-1, height=-1, border=None):
    ...
    def setName(self, name):
        ...
        if self.getApplication() is not None:
            raise ValueError, 'Window name can not be changed while the window is in application'
        self._name = name
        return
    ...
    def changeVariables(self, source, variables):
        ...
        if 'height' in variables and (self.getHeightUnits() != self.UNITS_PIXELS or variables.get('height') != self.getHeight()):
            sizeHasChanged = True
        if 'width' in variables and (self.getWidthUnits() != self.UNITS_PIXELS or variables['width'] != self.getWidth()):
            sizeHasChanged = True
    ...
    def addListener(self, listener, iface=None):
        ...
        if isinstance(listener, ICloseListener) and (iface is None or issubclass(iface, ICloseListener)):
            self.registerListener(CloseEvent, listener, _WINDOW_CLOSE_METHOD)
        ...
        if isinstance(listener, IResizeListener) and (iface is None or issubclass(iface, IResizeListener)):
    ...
    def removeListener(self, listener, iface=None):
        ...
        if isinstance(listener, ICloseListener) and (iface is None or issubclass(iface, ICloseListener)):
            self.withdrawListener(CloseEvent, listener, _WINDOW_CLOSE_METHOD)
        if isinstance(listener, IFocusListener) and (iface is None or issubclass(iface, IFocusListener)):
    ...
    def addWindow(self, window):
        ...
        if window.getApplication() is not None:
            raise ValueError, 'Window was already added to application - it can not be added to another window also.'
...
class Notification(object):
    ...
    def __init__(self, *args):
        ...
        if nargs == 1:
            caption, = args
            Notification.__init__(self, caption, None, self.TYPE_HUMANIZED_MESSAGE)
        elif nargs == 2:
            if isinstance(args[1], int):
                caption, typ = args
                ...
                caption, description = args
                Notification.__init__(self, caption, description, self.TYPE_HUMANIZED_MESSAGE)
        elif nargs == 3:
            caption, description, typ = args
