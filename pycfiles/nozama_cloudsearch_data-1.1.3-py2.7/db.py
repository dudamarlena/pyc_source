...
# [GCC 8.4.0]
# Embedded file name: /home/vagrant/src/nozama-cloudsearch/nozama-cloudsearch-data/nozama/cloudsearch/data/db.py
...
class ElasticSearchHelper(object):
    ...
    def __init__(self, config={}):
        ...
        self.log.info(("Using ElasticSearch Endpoint '{0}'").format(self.base_uri))
        ...
        self.log.info(("document_uri is '{0}' search_uri is '{0}'").format(self.document_uri, self.search_uri))
    ...
    def hard_reset(self):
        ...
        self.log.warn(('hard_reset: removing all content from {0}').format(url))
        self.conn.delete_all_indexes()
        self.log.warn(('hard_reset: all content removed from {0} OK.').format(url))
...
def get_es():
    ...
    """
    if not __es:
        raise ValueError('No ElasticSearchHelper instance configured! Call init() first.')
    return __es