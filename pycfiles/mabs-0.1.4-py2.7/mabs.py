...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mabs/mabs.py
# Compiled at: 2015-04-30 22:17:52
import paramiko, sys, threading, os
from multiprocessing.dummy import Pool as ThreadPool
from time import sleep, ctime
from optparse import OptionParser
import collections, logging, sh
...
class Tran_Base(object):
    ...
    def logger_log(self):
        ...
        """
        logging.basicConfig(filename=os.path.join('/var/log', 'mabslog.txt'), level=logging.INFO, format='%(asctime)s - %(levelname)s: %(message)s')
        return logging
    ...
    def notify(self, color, msg):
        ...
        """
        c, w, b, g = ('\x1b[1;40;31m', '\x1b[1;40;33m', '\x1b[1;40;34m', '\x1b[1;40;38m')
        endc = '\x1b[0m'
        if color == 'c':
    ...
    def ssh_conn(self):
        ...
        try:
            ...
            self.ssh.connect(self.hostname, port=self.port, username=self.username, password=self.pwd, timeout=self.timeout)
        except:
    ...
    def send_file_support_shell(self, machine):
        ...
        """
        find_comm1 = re.compile('`(.*)`')
        find_comm2 = re.compile('\\$\\((.*)\\)')
...
class Tran_Command(Tran_Base):
    ...
    def FileTran(self, tran_type):
        ...
        local, remote = self.send_file_support_shell(local), self.send_file_support_shell(remote)
...
class TranTune(Tran_Command):
    ...
    def command_log_export(self, head_log='', stdoutlog='', stderrlog='', access_log=''):
        ...
        self.logqueue.append(self.notify('w', head_log))
        self.logqueue.append(self.notify('b', '*' * 50))
        self.logqueue.append(self.notify('b', 'exec command:%s' % self.form_log(self.commandline)))
        if stdoutlog.strip():
...
def MyMul(args):
    ...
    for conf in confs.split('\n'):
        if not conf.startswith('#') and len(conf) >= 1:
            ...
            if file_comms:
                [ config.append([ip, port, username, pwd, i]) for i in file_comms ]
            else:
    ...
    if fail_list:
        print '\x1b[1;40;31m'
        print '  failed events:' + str(len(fail_list)) + ' host: ' + (',').join(i[1] for i in fail_list)
        print '\x1b[0m'
    if success_list:
        print '\x1b[1;40;38m'
        if fail_list:
            print '  succeed events: ' + str(len(success_list)) + ' host: ' + (',').join(i[1] for i in success_list)
        else:
            print '  all succeed, ' + ' host: ' + (',').join(i[1] for i in success_list)
        print '\x1b[0m'
    print '\x1b[1;40;33m'
    print '  ' + '*' * 50
    print '  ' + '*' * 50
...
if __name__ == '__main__':
    parser = OptionParser()
    parser.add_option('-H', '--host', dest='host', help='define the groups or host,like -h web1 ,-h group-name1')
    parser.add_option('-C', '--command', dest='command', help='execute simple command')
    parser.add_option('', '--hostfile', dest='hostfile', help='define the host file, like --hostfile hostfile.txt')
    parser.add_option('', '--commfile', dest='commfile', help='define the command file, like --commfile commfile.txt')
    parser.add_option('-s', '--show', action='store_true', dest='show_clients', help='display all the clients, like -s')
    parser.add_option('-u', '--user', dest='user', help='define the use to execute command, like -u root')
    options, args = parser.parse_args()
    verify = Verify_bool()
    host_bool = verify.common_verify(options.host, options.hostfile)
    comm_bool = verify.common_verify(options.command, options.commfile, 'comm')
    ...
    if options.user:
        try:
            options.host = (' ').join([ i if i != 'root' else options.user for i in options.host.split() ])
        except:
            pass

    paradict = {'host': options.host, 'hostfile': options.hostfile, 
