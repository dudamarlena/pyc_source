...
# Embedded file name: build/bdist.linux-x86_64/egg/capture0_data/online_handles.py
# Compiled at: 2016-09-25 19:56:10
# Size of source mod 2**32: 1889 bytes
import csv, functools, logging, pprint
...
class IndexCompany(object):
    ...
    def __init__(self, company: str, display_name: Optional[str], official_name: Optional[str], handles: Optional[Set[str]]):
    ...
    def split_keywords(keyword_string: str) -> Set[str]:
        ...
        return {k for k in keywords if k if k}

    @classmethod
    def build_from_row(cls, row):
        handle_columns = {v for k, v in row.items() if v and 'twitter' in k.lower() if v and 'twitter' in k.lower()}
        handles = {h.strip().lstrip('@') for hc in handle_columns for h in hc.split(' ')}
        return cls(company=row['company'], official_name=row['official_name'], display_name=row['display_name'], handles=handles)
