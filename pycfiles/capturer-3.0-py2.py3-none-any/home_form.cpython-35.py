...
# Embedded file name: build/bdist.linux-x86_64/egg/capture0/forms/home_form.py
# Compiled at: 2016-10-03 14:50:36
...
def home_form_instance_factory(companies: typing.List[IndexCompany]) -> BaseHomeForm:
    ...
    new_form_elements = OrderedDict({c.company:RadioField(c.company, description=form_empathy_question(c.display_name), validators=[DataRequired()], choices=EMPATHY_CHOICES) for c in companies})
    new_form = type('frm0', (BaseHomeForm,), new_form_elements)
    new_form.most_empathic = StringField('most_empathic', description=messages['question_most_empathic'], validators=[
     DataRequired()])
    new_form.least_empathic = StringField('least_empathic', description=messages['question_least_empathic'], validators=[
     DataRequired()])
    new_form.question_missing = StringField('question_missing', description=messages['question_missing'], validators=[
     DataRequired()])
