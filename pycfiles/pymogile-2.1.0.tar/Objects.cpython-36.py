...
# Embedded file name: build\bdist.win-amd64\egg\Pymoe\Mal\Objects.py
# Compiled at: 2017-08-03 07:38:56
# Size of source mod 2**32: 15557 bytes
import xml.etree.ElementTree as ET
from .Abstractions import NT_DATE_OBJ, NT_DATES, NT_EPISODES, NT_FLAGS, NT_REWATCHED, NT_SCORES, NT_STATUS, NT_STORAGE, NT_TYPEDATA
...
class Anime:
    ...
    def __init__(self, aid, **kwargs):
        ...
        self.dates = NT_DATES(series=[kwargs.get('anime_start'), kwargs.get('anime_end')], user=[
         kwargs.get('date_start'), kwargs.get('date_finish')])
    ...
    def to_xml(self):
        ...
        for x in self.xml_tags:
            if getattr(self, x):
                if x in ('episodes', 'scores', 'status', 'dates', 'storage', 'rewatched',
                         'flags', 'tags'):
                    if x == 'episodes':
...
class Manga:
    ...
    def __init__(self, mid, **kwargs):
        ...
        self.dates = NT_DATES(series=[kwargs.get('manga_start'), kwargs.get('manga_end')], user=[
         kwargs.get('date_start'), kwargs.get('date_finish')])
        self.synopsis = kwargs.get('synopsis')
        self.image = kwargs.get('image')
        self.storage = NT_STORAGE(type=(kwargs.get('storage_type')), value=(kwargs.get('storage_value')))
    ...
    def to_xml(self):
        ...
        for x in self.xml_tags:
            if getattr(self, x):
                if x in ('chapters', 'volumes', 'scores', 'status', 'dates', 'storage',
                         'reread', 'flags', 'tags'):
                    if x == 'chapters':
