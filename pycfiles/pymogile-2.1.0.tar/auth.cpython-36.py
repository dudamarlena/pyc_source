...
# Embedded file name: build\bdist.win-amd64\egg\Pymoe\Kitsu\auth.py
# Compiled at: 2017-08-03 07:38:56
# Size of source mod 2**32: 3441 bytes
...
class KitsuAuth:
    ...
    def authenticate(self, username, password):
        ...
        """
        r = requests.post((self.apiurl + '/token'), params={'grant_type':'password', 
         'username':username,  'password':password,  'client_id':self.cid, 
         'client_secret':self.csecret})
        if r.status_code != 200:
            raise ServerError
        jsd = r.json()
        if self.remember:
            self.token_storage[username] = {'token':jsd['access_token'], 
             'refresh':jsd['refresh_token'],  'expiration':int(jsd['created_at']) + int(jsd['expires_in'])}
        return (
         jsd['access_token'], int(jsd['expires_in']) + int(jsd['created_at']), jsd['refresh_token'])
    ...
    def refresh(self, refresh_token):
        ...
        """
        r = requests.post((self.apiurl + '/token'), params={'grant_type':'refresh_token',  'client_id':self.cid,  'client_secret':self.csecret, 
         'refresh_token':refresh_token})
        if r.status_code != 200:
            raise ServerError
        jsd = r.json()
        return (
         jsd['access_token'], int(jsd['expires_in']) + int(jsd['created_at']))
