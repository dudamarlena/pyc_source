...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\i18n\format.py
...
try:
    filterwarnings('ignore', message='Not importing directory', category=ImportWarning, module='turbogears.i18n')
except NameError:
    pass
...
def format_number(value, locale=None):
    ...
    thou = re.compile('([0-9])([0-9][0-9][0-9]([%s]|$))' % gf).search
    v = str(value)
    mo = thou(v)
...
def format_decimal(value, num_places, locale=None):
    ...
    return format_number(num, locale) + unicode(get_decimal_format(locale) + decimals)
...
def format_date(dt, format='medium', locale=None, time_format='', date_format=''):
    ...
    return date_str.decode('utf8') % dict(monthname=get_month_names(locale)[month], abbrmonthname=get_abbr_month_names(locale)[month], dayname=get_weekday_names(locale)[weekday], abbrdayname=get_abbr_weekday_names(locale)[weekday])