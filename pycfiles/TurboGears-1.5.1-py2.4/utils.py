...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\i18n\utils.py
# Compiled at: 2011-06-10 12:10:30
...
def google_translate(from_lang, to_lang, text):
    ...
    if isinstance(text, unicode):
        has_nbsp = '\xa0' in text
        text = text.encode('utf-8')
    else:
        has_nbsp = False
    params = urllib.urlencode(dict(v='1.0', langpair='%s|%s' % (from_lang, to_lang), q=text))
    try:
        result = TGURLopener().open('http://ajax.googleapis.com/ajax/services/language/translate', params).read()
    except IOError:
        text = None
    else:
        try:
