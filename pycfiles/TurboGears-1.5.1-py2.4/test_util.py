...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\tests\test_util.py
# Compiled at: 2011-07-07 06:36:10
import sys
...
def test_parse_http_accept_header():
    ...
    assert t('text/plain; q=0.5, text/html,\n        text/x-dvi; q=0.8, text/x-c') == ['text/html', 'text/x-c', 'text/x-dvi', 'text/plain']
    assert t('text/*;q=0.3, text/html;q=0.7, text/html;level=1,\n        text/html;level=2;q=0.4, */*;q=0.5') == ['text/html;level=1', 'text/html', '*/*', 'text/html;level=2', 'text/*']
...
def test_quote_cookie():
    ...
    assert quote(b'K\xe4se') == '"K\\344se"'
    assert quote(b'Hello, W\xf6rld!') in ('"Hello, W\\366rld!"', '"Hello\\054 W\\366rld!"')
    assert quote('$1;\tinsert coin!') in ('"$1;\\011insert coin!"', '"$1\\073\\011insert coin!"')
    assert quote('Hello, "W\\orld"!') in ('"Hello, \\"W\\\\orld\\"!"', '"Hello\\054 \\"W\\\\orld\\"!"')


def test_unquote_cookie():
    unquote = util.unquote_cookie
    assert unquote('Hello!') == 'Hello!'
    assert unquote('"K\\344se"') == b'K\xe4se'
    assert unquote('"Hello, W\\366rld!"') == b'Hello, W\xf6rld!'
    assert unquote('"$1;\\011insert coin!"') == '$1;\tinsert coin!'
    assert unquote('"Hello, \\"W\\\\orld\\"!"') == 'Hello, "W\\orld"!'
...
def test_get_mime_type_for_format():
    ...
    config.update({'global': {'tg.format_mime_types': {'xhtml': 'application/xhtml+xml', 'foo': 'text/bar'}}})
...
def test_copy_if_mutable():
    test_values = (
     (
      None, False), ('foo', False), (42, False), ((1, 2), False), ([1, 2], True), ({1: 2}, True))
...
def test_fixentities():
    assert util.fixentities('Chip&nbsp;&amp;&nbsp;Chap') == 'Chip&#160;&amp;&#160;Chap'
...
def test_match_ip():
    ...
    assert m('2001:0db8:85a3:08d3:1319:8a2e:0370:7334', '2001:db8:85a3:8d3:1319:8a2e:3.112.115.52')
    assert m('2001:db8:85a3:8d3:1300::/72', '2001:0db8:85a3:08d3:1319:8a2e:0370:7334')
    assert m('2001:db8:85a3:8d3:1300::/72', '2001:0db8:85a3:08d3:1399:8a2e:0370:7334')
    assert not m('2001:db8:85a3:8d3:1300::/72', '2001:0db8:85a3:08d3:1219:8a2e:0370:7334')