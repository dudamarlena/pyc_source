...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\identity\tests\test_identity_sa.py
# Compiled at: 2011-07-08 11:39:23
"""SQLAlchemy based Identity tests."""
import unittest, urllib, base64
...
class TestIdentity(testutil.TGTest):
    ...
    def setUp(self):
        test_config = {'visit.on': True, 'visit.manager': 'sqlalchemy', 'identity.on': True, 'identity.provider': 'sqlalchemy', 'identity.failure_url': '/identity_failed', 'identity.force_external_redirect': False, 'identity.source': 'form,http_auth,visit', 'identity.http_basic_auth': True, 'identity.http_auth_realm': 'test realm', 'identity.form.user_name': 'user_name', 'identity.form.password': 'password', 'identity.form.submit': 'login', 'identity.saprovider.encryption_algorithm': None, 'identity.custom_encryption': None, 'tg.strict_parameters': True, 'tg.allow_json': False}
        if not self.config:
    ...
    def init_model(self):
        if not session.query(TG_User).first():
            user = TG_User()
            user.user_name = 'samIam'
            user.email_address = 'samiam@example.com'
            user.display_name = 'Samuel Amicus'
            user.password = 'secret'
            session.add(user)
            admin_group = TG_Group()
            admin_group.group_name = 'admin'
            admin_group.display_name = 'Administrator'
            session.add(admin_group)
            peon_group = TG_Group()
            peon_group.group_name = 'peon'
            peon_group.display_name = 'Regular Peon'
            session.add(peon_group)
            other_group = TG_Group()
            other_group.group_name = 'other'
            other_group.display_name = 'Another Group'
            session.add(other_group)
            chopper_perm = TG_Permission()
            chopper_perm.permission_name = 'chops_wood'
            chopper_perm.description = 'Wood Chopper'
            session.add(chopper_perm)
            boss_perm = TG_Permission()
            boss_perm.permission_name = 'bosses_people'
            boss_perm.description = 'Benevolent Dictator'
            session.add(boss_perm)
            peon_group.permissions.append(chopper_perm)
    ...
    def current_in_peon_group(self, current):
        ...
        group_ids = set((TG_Group.by_group_name('peon').group_id, TG_Group.by_group_name('other').group_id))
    ...
    def test_user_password_unicode(self):
        ...
        config.update({'identity.saprovider.encryption_algorithm': None})
        self.app.get('/')
        user = TG_User()
        user.password = 'garçon'
        assert user.password == 'garçon'
        return

    def test_user_password_hashed_sha(self):
        """Test if a sha hashed password is stored in the database."""
        config.update({'identity.saprovider.encryption_algorithm': 'sha1'})
        self.app.get('/')
        user = TG_User()
        user.password = 'password'
    ...
    def test_user_password_hashed_md5(self):
        ...
        config.update({'identity.saprovider.encryption_algorithm': 'md5'})
        self.app.get('/')
        user = TG_User()
        user.password = 'password'
    ...
    def test_user_password_hashed_md5_utf8string(self):
        ...
        self.app.get('/')
        user = TG_User()
        user.password = ('garçon').encode('UTF-8')
        assert user.password == 'c295c4bb2672ca8c432effc53b40bb1e'
    ...
    def test_user_password_raw_unicode(self):
        ...
        self.app.get('/')
        user = TG_User()
        user.set_password_raw('garçon')
        assert user.password == 'garçon'

    def test_user_password_hashed_custom(self):
        """Test if a custom hashed password is stored in the database."""
        config.update({'identity.saprovider.encryption_algorithm': 'custom', 'identity.custom_encryption': 'identity.tests.test_identity.mycustomencrypt'})
        self.app.get('/')
        user = TG_User()
        user.password = 'password'
    ...
    def test_deny_anonymous_viewable(self):
        ...
        response = self.app.get('/logged_in_only?user_name=samIam&password=secret&login=Login')
        assert 'logged_in_only' in response, response

    def test_external_redirect(self):
        """Test that external redirection at identity failures works."""
        config.update({'identity.failure_url': 'test_login', 'identity.force_external_redirect': True})
    ...
    def test_external_redirect_with_funky_params(self):
        ...
        response = self.app.get('/logged_in_only?a-0.x=1&a-0.y=2&a-1.x=3&a-1.y=4', status=302)
        assert 'identity_failed_answer' not in response, response
        (path, params) = response.headers['Location'].split('?', 1)
        assert path.endswith('/identity_failed')
        params = ('&').join(sorted(params.split('&')))
        assert params == 'a-0.x=1&a-0.y=2&a-1.x=3&a-1.y=4&forward_url=%2Flogged_in_only'
    ...
    def test_basic_authentication(self):
        ...
        response = self.app.get('/logged_in_only', headers={'Authorization': 'Basic %s' % credentials}, status=401)
        assert 'identity_failed_answer' in response, response
        credentials = base64.encodestring('samIam:secret:appendix')[:-1]
        response = self.app.get('/logged_in_only', headers={'Authorization': 'Basic %s' % credentials}, status=401)
        assert 'identity_failed_answer' in response, response
        response = self.app.get('/logged_in_only', headers={'Authorization': 'Basic samIam:secret'}, status=401)
        assert 'identity_failed_answer' in response, response
        credentials = base64.encodestring('samIam:secret')[:-1]
        response = self.app.get('/logged_in_only', headers={'Authorization': 'Basic %s' % credentials})
        assert 'logged_in_only' in response, response

    def test_logout(self):
        """Test that logout works and session id gets invalid afterwards."""
        response = self.app.get('/in_peon_group?user_name=samIam&password=secret&login=Login')
    ...
    def test_user_not_in_right_group(self):
        ...
        response = self.app.get('/in_admin_group?user_name=samIam&password=secret&login=Login', status=403)
        assert 'identity_failed_answer' in response, response
    ...
    def test_require_permission_viewable(self):
        ...
        response = self.app.get('/has_chopper_permission?user_name=samIam&password=secret&login=Login')
    ...
    def test_user_lacks_permission(self):
        ...
        permission.
        """
        response = self.app.get('/has_boss_permission?user_name=samIam&password=secret&login=Login', status=403)
        assert 'identity_failed_answer' in response, response

    def test_user_info_available(self):
        """Test that we can see user information inside our controller."""
        response = self.app.get('/user_email?user_name=samIam&password=secret&login=Login')
        assert 'samiam@example.com' in response, response

    def test_bad_login(self):
        """Test that we are denied access if we provide a bad login."""
        response = self.app.get('/logged_in_only?user_name=samIam&password=wrong&login=Login', status=403)
        assert 'identity_failed_answer' in response, response
    ...
    def test_restricted_subdirectory_viewable(self):
        ...
        response = self.app.get('/peon_area/index?user_name=samIam&password=secret&login=Login')
    ...
    def test_decoratator_in_restricted_subdirectory(self):
        ...
        response = self.app.get('/peon_area/in_other_group?user_name=samIam&password=secret&login=Login')
    ...
    def test_decoratator_failure_in_restricted_subdirectory(self):
        ...
        response = self.app.get('/peon_area/in_admin_group?user_name=samIam&password=secret&login=Login', status=403)
        assert 'identity_failed_answer' in response, response
    ...
    def test_explicit_checks_in_restricted_subdirectory(self):
        ...
        response = self.app.get('/peon_area/in_other_group_explicit_check?user_name=samIam&password=secret&login=Login')
        assert 'in_other_group' in response, response
    ...
    def test_throwing_identity_exception_in_restricted_subdirectory(self):
        ...
        directory is handled as expected"""
        response = self.app.get('/peon_area/in_admin_group_explicit_check?user_name=samIam&password=secret&login=Login', status=403)
    ...
    def test_user_unicode(self):
        ...
        user = TG_User()
        user.user_name = 'säm'
        user.display_name = 'Sämüel Käsfuß'
        user.email_address = 'samkas@example.com'
        user.password = 'geheim'
        session.add(user)
        assert user.user_name == 'säm'
        response = self.app.get('/logged_in_only?user_name=säm&password=geheim&login=Login')
        assert 'logged_in_only' in response, response
        self.app.reset()
    ...
    def test_remote_ip(self):
        ...
        r = self.app.get('/remote_ip', headers={'Remote-Addr': '127.0.0.1'}, status=200)
        assert r.raw['remote_ip'] == '127.0.0.1'
        r = self.app.get('/remote_ip', headers={'X-Forwarded-For': '192.168.1.100, 224.50.214.12'}, status=200)
        assert r.raw['remote_ip'] == '224.50.214.12'

    def test_from_localhost(self):
        """Test we can connect from 127.0.0.1."""
        r = self.app.get('/from_localhost', headers={'Remote-Addr': '192.168.4.100'}, status=403)
        assert 'identity_failed_answer' in r
        r = self.app.get('/from_localhost', headers={'Remote-Addr': '127.0.0.1'}, status=200)
        assert 'localhost_only' in r

    def test_from_any_host(self):
        """Test we can connect from any host in an IP list."""
        r = self.app.get('/from_any_host', headers={'Remote-Addr': '192.168.4.100'}, status=403)
        assert 'identity_failed_answer' in r
        r = self.app.get('/from_any_host', headers={'Remote-Addr': '127.0.0.1'}, status=200)
        assert 'hosts_on_list_only' in r
        r = self.app.get('/from_any_host', headers={'Remote-Addr': '127.0.0.2'}, status=200)
        assert 'hosts_on_list_only' in r

    def test_nested_login(self):
        """Check that we can login using a nested form."""
        config.update({'identity.form.user_name': 'credentials.user', 'identity.form.password': 'credentials.pass', 'identity.form.submit': 'log.me.in'})
        testutil.stop_server(tg_only=False)
        self.app = testutil.make_app(self.root)
        testutil.start_server()
        response = self.app.get('/logged_in_only?credentials.user=samIam&credentials.pass=secret&log.me.in=Enter')
        assert 'logged_in_only' in response, response
...
class TestTGUser(testutil.DBTest):
    ...
    def setUp(self):
        ...
        config.update({'identity.on': False, 'identity.provider': 'sqlalchemy'})
        super(TestTGUser, self).setUp()

    def tearDown(self):
        testutil.stop_server()
        super(TestTGUser, self).tearDown()
        config.update({'identity.on': self._identity_on, 'identity.provider': self._identity_provider})
...
class TestIdentityVisitPlugin(unittest.TestCase):
    ...
    def setUp(self):
        test_config = {'visit.on': True, 'visit.manager': 'sqlalchemy', 'identity.on': True, 'identity.http_basic_auth': True, 'identity.provider': 'sqlalchemy', 'identity.source': 'form, http_auth, visit, '}
        original_config = dict()
        for key in test_config:
            original_config[key] = config.get(key)
    ...
    def test_inactive_identity_source(self):
        ...
        config.update({'identity.source': 'http_auth,visit', 'identity.http_basic_auth': False, 'visit.on': False})
        self.assertRaises(IdentityConfigurationException, IdentityVisitPlugin)
