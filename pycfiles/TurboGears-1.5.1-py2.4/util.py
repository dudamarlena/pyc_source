...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\util.py
# Compiled at: 2011-11-15 15:14:45
"""The TurboGears utility module."""
_all__ = [
 'Bunch', 'DictObj', 'DictWrapper', 'Enum', 'setlike', 'get_package_name', 'get_model', 'load_project_config', 'ensure_sequence', 'has_arg', 'to_kw', 'from_kw', 'adapt_call', 'call_on_stack', 'remove_keys', 'arg_index', 'inject_arg', 'inject_args', 'add_tg_args', 'bind_args', 'recursive_update', 'combine_contexts', 'request_available', 'flatten_sequence', 'load_class', 'parse_http_accept_header', 'simplify_http_accept_header', 'to_unicode', 'to_utf8', 'quote_cookie', 'unquote_cookie', 'get_template_encoding_default', 'get_mime_type_for_format', 'mime_type_has_charset', 'find_precision', 'copy_if_mutable', 'match_ip', 'deprecated']
import os, sys, re, logging, warnings, htmlentitydefs, socket, struct
...
def deprecated(message=None):
    ...
    def decorate(func):
        if not decorate.message:
            decorate.message = 'Call to deprecated function %s.' % func.__name__

        def new_func(*args, **kwargs):
            if not decorate.warned:
                warnings.warn(decorate.message, category=DeprecationWarning, stacklevel=2)
...
def Enum(*names):
    ...
    class EnumValue(object):
        ...
        def __cmp__(self, other):
            if not (isinstance(other, EnumValue) and self.EnumType is other.EnumType):
...
def to_kw(func, args, kw, start=0):
    ...
    return (
     args[len(argnames) - len(defaults) - start:], kw)
...
def from_kw(func, args, kw, start=0):
    ...
    (argnames, defaults) = getargspec(func)[::3]
    defaults = ensure_sequence(defaults)
    newargs = [ kw.pop(name) for name in islice(argnames, start, len(argnames) - len(defaults)) if name in kw
              ]
    newargs.extend(args)
    return (newargs, kw)
...
def inject_arg(func, argname, argval, args, kw, start=0):
    ...
    if pos is None or pos > len(argnames) - len(defaults) - 1:
        ...
        pos -= start
        args = tuple(chain(islice(args, pos), (argval,), islice(args, pos, None)))
    return (
     args, kw)
...
def bind_args(**add):
    ...
    def make_decorator(func):
        ...
        defaults = [ d for d in defaults if argnames[(-len(defaults) + defaults.index(d))] not in add ]
...
def load_class(dottedpath):
    ...
    try:
        try:
            ...
            if not modulename:
                module = __import__(__name__.split('.', 1)[0], globals(), locals(), [classname])
...
def get_template_encoding_default(engine_name=None):
    ...
    return config.get('%s.encoding' % engine_name, config.get('%s.default_encoding' % engine_name, 'utf-8'))


_format_mime_types = dict(plain='text/plain', text='text/plain', html='text/html', xhtml='text/html', xml='text/xml', json='application/json')
...
def inet_aton(addr):
    ...
    if ':' in addr:
        ...
        return struct.pack('!QL', 0, 65535) + socket.inet_aton(addr)
