...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\widgets\datagrid.py
# Compiled at: 2011-07-03 10:38:34
"""Generic widget to present and manipulate data in a grid (tabular) form."""
__all__ = [
 'DataGrid', 'PaginateDataGrid']
    ...
    class attrwrapper:
        ...
        def __init__(self, name):
            if not isinstance(name, (int, str)):
                raise ValueError('attribute name must be an integer index or a string attribute')
            self.name = name
...
class DataGridDesc(CoreWD):
    ...
    for_widget = DataGrid(fields=[('Name', lambda row: row[1]),
     ('Country',
      lambda row: row[2]),
     ('Age',
      lambda row: row[0])], default=[(33, 'Anton Bykov', 'Bulgaria'), (23, 'Joe Doe', 'Great Britain'), (44, 'Pablo Martelli', 'Brazil')])
