...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\config.py
# Compiled at: 2011-03-26 09:20:22
"""TurboGears configuration"""
__all__ = [
 'update_config', 'get', 'update']
import os, glob, re, cherrypy
from cherrypy import request, config as cp_config, log as cp_log
from configobj import ConfigObj
import pkg_resources, logging.handlers
server = cp_config
    ...
    for (key, handler) in handlers.items():
        ...
        if formatter:
            try:
                ...
                raise ConfigError('Handler %s references unknown formatter %s' % (key, formatter))
            else:
        ...
        if cfghandlers:
            ...
            for handler in cfghandlers:
                try:
                    ...
                    raise ConfigError('Logger %s references unknown handler %s' % (key, handler))
...
def config_defaults():
    ...
    return dict(current_dir_uri=os.path.abspath(os.getcwd()))
...
def config_obj(configfile=None, modulename=None):
    ...
    if modulename:
        ...
        top_level_dir = os.path.normpath(pkg_resources.resource_filename(top_level_package, ''))
        package_dir = os.path.normpath(pkg_resources.resource_filename(packagename, ''))
...
def _check_name(name):
    ...
    if basename in _tools_names:
        ...
        raise KeyError('The config setting %s is obsolete. Check the CherryPy 3 docs for the new setting.' % name)
