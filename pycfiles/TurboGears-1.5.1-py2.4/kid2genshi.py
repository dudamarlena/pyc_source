...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\command\kid2genshi.py
...
class Kid2Genshi(object):
    ...
    def __init__(self, version):
        parser = optparse.OptionParser(usage='%prog kid2genshi [options]', version='%prog ' + version)
        parser.add_option('-f', '--force', help='overwrite existing templates', action='store_true', dest='force')
        parser.add_option('-k', '--keep', help='keep the original templates', action='store_true', dest='keep')
        parser.add_option('-r', '--reverse', help='convert Genshi to Kid', action='store_true', dest='reverse')
        self.parser = parser
    ...
    def change_config(self, filename):
        ...
        new_defaultview = '\\1%s\\2' % self.engines[self.to_index].lower()
    ...
    def run(self):
        ...
        convert = [('<([a-z]+)\\b([^<]*\\bxmlns:py\\s*=\\s*[\\\'"])%s([\\\'"][^<]*>.*</\\1)>' % xmlns[from_index], '<\\1\\2%s\\3>' % self.xmlns[to_index])]
        if reverse:
            convert.append(('<([a-z]+)\\b([^<]*)\\bxmlns:xi(\\s*=\\s*")http://www.w3.org/2001/XInclude("[^<]*>)\\s*<xi:include\\s+href=[\\\'"]([^\\\'"]*)\\.html[\\\'"]/>(.*</\\1)>',
                            "<\\1\\2py:extends\\3'\\5.kid'\\4\\6>"))
        else:
            convert.append(('<([a-z]+)\\b([^<]*)\\bpy:extends(\\s*=\\s*")\\\'([^\\\'"]*)\\.kid\\\'("[^<]*>)(.*</\\1)>',
                            '<\\1\\2xmlns:xi\\3http://www.w3.org/2001/XInclude\\5<xi:include href="\\4.html"/>\\6>'))
        self.re_convert = [ (re.compile(r[0], re.S), r[1]) for r in convert ]
        self.re_defaultview = re.compile('^(\\s*tg.defaultview\\s*=\\s*[\\\'"])(?:%s)([\\\'"]\\s*(?:#.*)?)$' % engines[from_index].lower(), re.M)
        print 'Converting all %s to %s templates in package %s' % (engines[from_index], engines[to_index], package_name)
        print '(the %s templates will be %s) ...' % (engines[from_index], keep and 'kept' or 'removed')
        n = 0
        ...
        if n:
            if n > 1:
                print '%d %s templates have been converted to %s.' % (n, engines[from_index], engines[to_index])
            print 'Changing the configuration of package %s ...' % package_name
            if not self.change_config(os.path.join(package_name, 'config', 'app.cfg')):
                print 'Application config with defaultview setting not found.'
        elif force:
            print 'No %s templates were found in the current project.' % engines[from_index]
        else:
            print 'No %s templates needed to be converted in this project.' % engines[from_index]