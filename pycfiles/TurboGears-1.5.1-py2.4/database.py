...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\database.py
# Compiled at: 2011-11-15 15:13:40
"""Convenient access to an SQLObject or SQLAlchemy managed database."""
__all__ = [
 'AutoConnectHub', 'bind_metadata', 'create_session', 'create_session_mapper', 'commit_all', 'end_all', 'DatabaseError', 'DatabaseConfigurationError', 'EndTransactions', 'get_engine', 'get_metadata', 'mapper', 'metadata', 'PackageHub', 'rollback_all', 'session', 'session_mapper', 'set_db_uri', 'so_columns', 'so_joins', 'so_to_dict']
import sys, time, logging, cherrypy
...
if sqlalchemy:
    ...
    def bind_metadata():
        ...
        try:
            ...
            raise DatabaseConfigurationError('No sqlalchemy database configuration found!')
        ...
        for (k, v) in config.items():
            if '.dburi' in k and 'sqlalchemy.' not in k:
                get_metadata(k.split('.', 1)[0]).bind = sqlalchemy.create_engine(v, **alch_args)
    ...
    def create_session_mapper(scoped_session=session):
        ...
        def mapper(cls, *args, **kw):
            ...
            if set_kwargs_on_init and (getattr(cls, '__init__', object.__init__) is object.__init__ or getattr(cls.__init__, '_session_mapper', False)):
...
if sqlobject:
        ...
        def getConnection(self):
            try:
                ...
                if uri:
                    ...
                    elif uri.startswith('mysql') and config.get('turbogears.enable_mysql41_timestamp_workaround', False):
                        self._enable_timestamp_workaround(conn)
                    self.threadingLocal.connection = conn
                    return self.begin(conn)
                raise AttributeError('No connection has been defined for this thread or process')
    ...
    class PackageHub(object):
        ...
        def set_hub(self):
            ...
            if not dburi:
                raise DatabaseConfigurationError('No sqlobject database configuration found!')
...
def dispatch_exception(exception, args, kw):
    ...
    try:
        output = dispatch_error(controller, real_func, None, exception, *args, **kw)
...
def so_columns(sqlclass, columns=None):
    ...
    if columns is None:
        columns = {}
    columns.update(filter(lambda i: i[0] != 'childName', sqlclass.sqlmeta.columns.items()))
