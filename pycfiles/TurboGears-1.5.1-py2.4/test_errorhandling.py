...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\tests\test_errorhandling.py
# Compiled at: 2011-07-14 08:27:59
import unittest
from turbogears.controllers import error_handler, exception_handler, expose, validate, RootController, Controller
...
class MyRoot(RootController):
    ...
    def defaulterrorhandler(self, tg_source, tg_errors, tg_exceptions, *args, **kw):
        return dict(title='Default error handler', errors=_errors_to_str(tg_errors), args=args, kw=kw)

    def specialisederrorhandler(self, tg_source, tg_errors, *args, **kw):
        return dict(title='Specialised error handler', errors=_errors_to_str(tg_errors), args=args, kw=kw)
    ...
    @expose()
    @validate(validators={'bar': validators.Int(not_empty=True), 'baz': validators.Email()})
    ...
    def impliciterror(self, tg_errors=None, bar=''):
        if tg_errors:
            return dict(title='Implicit error handler', tg_errors=str(tg_errors))
        else:
    ...
    @expose()
    @validate(validators={'bar': validators.StringBoolean(), 'second': validators.Int(not_empty=True)})
    @error_handler(defaulterrorhandler)
    def positionalargs(self, first, second, *args, **kw):
        return dict(title='Positional arguments', first=first, second=second, third=args[0], args=args, bar=kw['bar'])
    ...
    @expose()
    @validate(validators={'bar': validators.Int(not_empty=True), 'baz': validators.Int(not_empty=True)})
    def failsafenone(self, tg_errors=None, bar='', baz=''):
        return dict(title='No failsafe', bar=bar, baz=baz)

    @expose()
    @validate(validators={'bar': validators.Int(not_empty=True), 'baz': validators.Int(not_empty=True)}, failsafe_schema=FailsafeSchema.values, failsafe_values={'bar': 1, 'baz': 2})
    def failsafevaluesdict(self, tg_errors=None, bar='', baz=''):
        return dict(title='Failsafe values-dict', bar=bar, baz=baz)

    @expose()
    @validate(validators={'bar': validators.Int(not_empty=True), 'baz': validators.Int(not_empty=True)}, failsafe_schema=FailsafeSchema.values, failsafe_values=13)
    def failsafevaluesatom(self, tg_errors=None, bar='', baz=''):
        return dict(title='Failsafe values-atom', bar=bar, baz=baz)

    @expose()
    @validate(validators={'bar': validators.Int(not_empty=True), 'baz': validators.Int(not_empty=True)}, failsafe_schema=FailsafeSchema.map_errors)
    def failsafemaperrors(self, tg_errors=None, bar='', baz=''):
        return dict(title='Failsafe map errors', bar=str(bar), baz=str(baz))

    @expose()
    @validate(validators={'bar': validators.Int(if_invalid=1), 'baz': validators.Int(if_invalid=2)})
    def failsafeformencode(self, tg_errors=None, bar='', baz=''):
        return dict(title='Formencode if_invalid', bar=bar, baz=baz)

    @expose()
    @validate(validators={'bar': validators.Int(not_empty=True), 'baz': validators.Int(not_empty=True)}, failsafe_schema=FailsafeSchema.defaults)
