...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\identity\saprovider.py
# Compiled at: 2011-05-26 13:40:06
import logging
...
class SqlAlchemyIdentity(object):
    ...
    def visit_link(self):
        ...
        if self.visit_key is None:
            return
        return session.query(visit_class).filter_by(visit_key=self.visit_key).first()
...
class SqlAlchemyIdentityProvider(object):
    ...
    def __init__(self):
        ...
        for classname in ('user', 'group', 'permission', 'visit'):
            default_classname = '.TG_' + (classname == 'visit' and 'VisitIdentity' or classname.capitalize())
            class_path = config.get('identity.saprovider.model.%s' % classname, __name__ + default_classname)
            ...
            if class_:
                ...
                log.error('Could not load class "%s". Check identity.saprovider.model.%s setting', class_path, classname)
...
class TG_User(object):
    ...
    def __repr__(self):
        return '<User: name="%s", email="%s", display name="%s">' % (self.user_name, self.email_address, self.display_name)
    ...
    def _set_password(self, cleartext_password):
        ...
        try:
            ...
            if hash == cleartext_password:
                log.info('Identity provider not enabled, and no encryption algorithm specified in config. Setting password as plaintext.')
    ...
    @classmethod
    def _map(cls):
        cls._table = Table('tg_user', metadata, Column('user_id', Integer, primary_key=True), Column('user_name', Unicode(16), unique=True, nullable=False), Column('email_address', Unicode(255), unique=True), Column('display_name', Unicode(255)), Column('password', Unicode(40)), Column('created', DateTime, default=datetime.now))
        cls._mapper = mapper(cls, cls._table, properties=dict(_password=cls._table.c.password))
...
class TG_Group(object):
    ...
    def __repr__(self):
        return '<Group: name="%s", display_name="%s">' % (self.group_name, self.display_name)
    ...
    @classmethod
    def _map(cls):
        cls._table = Table('tg_group', metadata, Column('group_id', Integer, primary_key=True), Column('group_name', Unicode(16), unique=True, nullable=False), Column('display_name', Unicode(255)), Column('created', DateTime, default=datetime.now))
        cls._user_association_table = Table('user_group', metadata, Column('user_id', Integer, ForeignKey('tg_user.user_id', onupdate='CASCADE', ondelete='CASCADE'), primary_key=True), Column('group_id', Integer, ForeignKey('tg_group.group_id', onupdate='CASCADE', ondelete='CASCADE'), primary_key=True))
        cls._mapper = mapper(cls, cls._table, properties=dict(users=relation(TG_User, secondary=cls._user_association_table, backref='groups')))
...
class TG_Permission(object):
    ...
    @classmethod
    def _map(cls):
        cls._table = Table('permission', metadata, Column('permission_id', Integer, primary_key=True), Column('permission_name', Unicode(16), unique=True, nullable=False), Column('description', Unicode(255)))
        cls._group_association_table = Table('group_permission', metadata, Column('group_id', Integer, ForeignKey('tg_group.group_id', onupdate='CASCADE', ondelete='CASCADE'), primary_key=True), Column('permission_id', Integer, ForeignKey('permission.permission_id', onupdate='CASCADE', ondelete='CASCADE'), primary_key=True))
        cls._mapper = mapper(cls, cls._table, properties=dict(groups=relation(TG_Group, secondary=cls._group_association_table, backref='permissions')))
...
class TG_VisitIdentity(object):
    ...
    @classmethod
    def _map(cls):
        cls._table = Table('visit_identity', metadata, Column('visit_key', String(40), primary_key=True), Column('user_id', Integer, ForeignKey('tg_user.user_id'), index=True))
        cls._mapper = mapper(cls, cls._table, properties=dict(user=relation(TG_User, backref='visit_identity')))
