...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\identity\tests\test_identity_so.py
# Compiled at: 2011-07-08 11:41:41
...
class TestIdentity(testutil.TGTest):
    ...
    def setUp(self):
        test_config = {'visit.on': True, 'visit.manager': 'sqlobject', 'identity.on': True, 'identity.provider': 'sqlobject', 'identity.failure_url': '/identity_failed', 'identity.force_external_redirect': False, 'identity.source': 'form,http_auth,visit', 'identity.http_basic_auth': True, 'identity.http_auth_realm': 'test realm', 'identity.form.user_name': 'user_name', 'identity.form.password': 'password', 'identity.form.submit': 'login', 'identity.soprovider.encryption_algorithm': None, 'identity.custom_encryption': None, 'tg.strict_parameters': True, 'tg.allow_json': False}
        if not self.config:
    ...
    def init_model(self):
        if TG_User.select().count() == 0:
            user = TG_User(user_name='samIam', email_address='samiam@example.com', display_name='Samuel Amicus', password='secret')
    ...
    def current_in_peon_group(self, current):
        ...
        group_ids = set((TG_Group.by_group_name('peon').id, TG_Group.by_group_name('other').id))
    ...
    def test_user_password_hashed_custom(self):
        ...
        config.update({'identity.soprovider.encryption_algorithm': 'custom', 'identity.custom_encryption': 'identity.tests.test_identity.mycustomencrypt'})
        self.app.get('/')
        hub.begin()
        user = TG_User.by_user_name('samIam')
        user.password = 'password'
    ...
    def test_user_unicode(self):
        ...
        user = TG_User(user_name='säm', display_name='Sämüel Käsfuß', email_address='samkas@example.com', password='geheim')
...
class TestTGUser(testutil.DBTest):
    ...
    def setUp(self):
        ...
        config.update({'identity.on': False, 'identity.provider': 'sqlobject'})
    ...
    def test_create_user(self):
        ...
        user = TG_User(user_name='testcase', email_address='testcase@example.com', display_name='Test Me', password='test')
...
class TestIdentityVisitPlugin(unittest.TestCase):
    ...
    def setUp(self):
        test_config = {'visit.on': True, 'visit.manager': 'sqlobject', 'identity.on': True, 'identity.http_basic_auth': True, 'identity.provider': 'sqlobject', 'identity.source': 'form, http_auth, visit, '}
        original_config = dict()
        for key in test_config:
            original_config[key] = config.get(key)
