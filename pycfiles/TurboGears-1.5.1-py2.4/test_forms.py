...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\widgets\tests\test_forms.py
# Compiled at: 2011-03-26 09:20:16
from cherrypy import request
from turbogears import config, controllers, expose, testutil, validate, validators, widgets
...
def test_rendering():
    ...
    for Form in (widgets.TableForm, widgets.ListForm):
        ...
        assert (Form is widgets.TableForm) == ('</table>' in output and '</td>' in output)
        assert (Form is widgets.ListForm) == ('</ul>' in output and '</li>' in output)
        ...
        form = Form(name='myform', use_name=True, fields=[widgets.TextField('myfield')])
        output = form.render(format='xhtml')
...
def test_input_conversion():
    ...
    form = widgets.TableForm(fields=[widgets.TextField('name'), widgets.TextField('age', validator=validators.Int())], submit_text='Submit')
...
def test_input_errors():
    ...
    form = widgets.TableForm(fields=[widgets.TextField('name'), widgets.TextField('age', validator=validators.Int())])
...
class w1(widgets.FormField):
    __module__ = __name__
    javascript = [
     widgets.JSLink(__module__, 'foo.js'), widgets.JSSource("alert('foo');"),
...
def test_class_attributes_form():
    ...
    form = TestForm(fields=TestForm.fields + [widgets.TextField('phone')], submit_text='your number too')
    output = form.render()
...
def test_help_text():
    ...
    class TestForm(widgets.ListForm):
        __module__ = __name__
        fields = [widgets.TextField('name', help_text='Enter your name here'), widgets.TextField('age', help_text='Enter your age here')]
...
def test_callable_options_for_selection_field():
    ...
    assert cc.counter == 1
    cc = CallableCounter()
    w = widgets.CheckBoxList('collections', label='Collections', options=cc, validator=validators.Int())
    assert cc.counter == 0
...
nestedform = widgets.TableForm(fields=[widgets.FieldSet('foo', fields=[widgets.TextField('name'), widgets.TextField('age')])])
...
class NestedController(controllers.Controller):
    ...
    def repeating(self, abc=None):
        ...
        return widgets.RepeatingFieldSet('my_fieldset', legend=legend, repetitions=5, fields=[widgets.TextField(name='lower', label='Lower'), widgets.TextField(name='upper', label='Upper')]).render()


def test_nested_variables():
    url = ('/checkform?foo.name=Kevin&foo.age=some%20Numero').encode('utf-8')
    ...
    assert request.raw['age'] == 'some Numero'
