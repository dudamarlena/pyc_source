...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\identity\soprovider.py
# Compiled at: 2011-07-07 06:36:09
import logging
...
from sqlobject import SQLObject, SQLObjectNotFound, RelatedJoin, DateTimeCol, IntCol, StringCol, UnicodeCol
...
class SqlObjectIdentityProvider(object):
    ...
    def __init__(self):
        ...
        for classname in ('user', 'group', 'permission', 'visit'):
            ...
            class_path = config.get('identity.soprovider.model.%s' % classname, __name__ + default_classname)
            class_ = load_class(class_path)
            if class_:
                log.info('Successfully loaded "%s".', class_path)
                glob_ns['%s_class' % classname] = class_
            else:
                log.error('Could not load class "%s". Check identity.soprovider.model.%s setting', class_path, classname)

        try:
            encoding = glob_ns['user_class'].sqlmeta.columns['user_name'].dbEncoding
...
class TG_User(SQLObject):
    ...
    groups = RelatedJoin('TG_Group', intermediateTable='user_group', joinColumn='user_id', otherColumn='group_id')
...
class TG_Group(SQLObject):
    ...
    display_name = UnicodeCol(length=255)
    created = DateTimeCol(default=datetime.now)
    users = RelatedJoin('TG_User', intermediateTable='user_group', joinColumn='group_id', otherColumn='user_id')
    permissions = RelatedJoin('TG_Permission', joinColumn='group_id', intermediateTable='group_permission', otherColumn='permission_id')
...
class TG_Permission(SQLObject):
    ...
    groups = RelatedJoin('TG_Group', intermediateTable='group_permission', joinColumn='permission_id', otherColumn='group_id')
