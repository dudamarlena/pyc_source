...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\widgets\tests\test_mixed_templates.py
...
class GenshiWrapper(widgets.Widget):
    ...
    __module__ = __name__
    template = '<div id="${name}" class="genshi" xmlns:py="http://genshi.edgewall.org/" py:content="value()"/>'
...
class KidWrapper(widgets.Widget):
    ...
    __module__ = __name__
    template = '<div id="${name}" class="kid" xmlns:py="http://purl.org/kid/ns#" py:content="value()"/>'
...
def test_external():
    ...
    w = widgets.Widget('baz', 'turbogears.widgets.tests.simple', engine_name='kid')
    assert w.engine_name == 'kid'
    output = w.render('foe')
...
def test_nesting_same():
    ...
    output = baz.render()
    assert output == '<div id="baz" class="genshi"><div id="bar" class="genshi"><div id="foo" class="genshi">genshi</div></div></div>'
    foo = KidWrapper('foo')
    ...
    output = baz.render()
    assert output == '<div id="baz" class="kid"><div id="bar" class="kid"><div id="foo" class="kid">kid</div></div></div>'
...
def test_nesting_mixed():
    ...
    assert baz.engine_name == 'genshi'
    output = baz.render()
    assert output == '<div id="baz" class="genshi"><div id="bar" class="kid"><div id="foo" class="genshi">genshi</div></div></div>'
    foo = KidWrapper('foo')
    bar = GenshiWrapper('bar', default=lambda : foo)
    baz = KidWrapper('baz', default=lambda : bar)
    assert foo.engine_name == 'kid'
    ...
    assert baz.engine_name == 'kid'
    output = baz.render()
    assert output == '<div id="baz" class="kid"><div id="bar" class="genshi"><div id="foo" class="kid">kid</div></div></div>'
