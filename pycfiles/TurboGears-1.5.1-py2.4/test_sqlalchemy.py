...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\tests\test_sqlalchemy.py
# Compiled at: 2011-07-14 07:57:05
...
from turbogears.testutil import make_app, sqlalchemy_cleanup, start_server, stop_server

class User(object):
    __module__ = __name__

    def __repr__(self):
        return "<User: '%s', name: '%s', password: '%s'>" % (self.user_id, self.user_name, self.password)
...
class Person(object):
    ...
    def __repr__(self):
        return "<Person: '%s', name: '%s', addresses: '%s'>" % (self.id, self.name, self.addresses)
...
class Address(object):
    ...
    def __repr__(self):
        return "<Address: '%s', where: '%s', city: '%s', person: '%s'>" % (self.id, self.address, self.city, self.person_id)
...
def setup_module():
    global fresh_multi_table
    global multi_table
    global test_table
    global users_table
    config.update({'sqlalchemy.dburi': 'sqlite:///:memory:', 'fresh.dburi': 'sqlite:///freshtest.db'})
    ...
    persons_table = Table('persons', metadata, Column('id', Integer, primary_key=True), Column('name', String(40)))
    addresses_table = Table('addresses', metadata, Column('id', Integer, primary_key=True), Column('address', String(40)), Column('city', String(40)), Column('person_id', Integer, ForeignKey(persons_table.c.id)))
    multi_table = Table('multi', metadata, Column('id', Integer, primary_key=True))
    metadata.create_all()
    test_table = Table('test', fresh_metadata, Column('id', Integer, primary_key=True), Column('val', String(40)))
    fresh_multi_table = Table('multi', fresh_metadata, Column('id', Integer, primary_key=True))
    fresh_metadata.create_all()
    mapper(User, users_table)
    mapper(Person, persons_table)
    mapper(Address, addresses_table, properties=dict(person=relation(Person, backref='addresses')))
...
def test_implicit_trans_no_error():
    ...
    assert 'someconfirmhandler' in response, 'The no error should have redirected to someconfirmhandler'
...
def test_raise_sa_exception():
    ...
    assert 'KARL25' not in response, 'Exception should NOT have been handled by our handler'
    assert 'IntegrityError' in response, 'The page should have displayed a database integrity error'
...
def test_user_exception():
    ...
    assert 'KARL25' in response, 'The exception handler should have answered us'
    p = session.query(Person).get(19)
    assert p is None, 'This Person should have been rolled back: %s' % p
    return
...
def test_user_redirect():
    ...
    assert session.query(Person).get(22) is not None, 'The controller only redirected, the Person should have been saved'
    return
...
def test_cntrl_commit():
    ...
    assert session.query(Person).get(23) is not None, 'The Person 23 should have been saved during commit inside controller'
    return
...
def test_cntrl_commit2():
    ...
    assert 'InvalidRequestError' not in response
    assert session.query(Person).get(24) is not None, 'The Person 24 should have been saved during commit inside controller and not rolled back'
    return
