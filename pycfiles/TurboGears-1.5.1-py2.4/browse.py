...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\toolbox\catwalk\browse.py
# Compiled at: 2011-07-14 07:29:14
import cherrypy
try:
    import sqlobject
except ImportError:
...
class Browse(controllers.Controller):
    ...
    @expose('turbogears.toolbox.catwalk.browse_grid', allow_json=True)
    def index(self, object_name, start=0, page_size=10, context='', filters=''):
        try:
            start, page_size = int(start), int(page_size)
        except ValueError:
            (start, page_size) = (0, 10)

        if not context:
            context = object_name
        headers = [ header for header in self.column_headers(object_name, context) if header['visible'] ]
        (total, rows) = self.rows_for_model(object_name, start, page_size, filters)
        return dict(object_name=object_name, start=start, page_size=page_size, total=total, headers=headers, rows=rows)

    @expose('turbogears.toolbox.catwalk.columns', allow_json=True)
    def columns(self, object_name, context=''):
        if not context:
            context = object_name
        return dict(object_name=object_name, context=context, columns=self.extended_column_headers(object_name, context))
    ...
    def relation_values(self, object_name, rows):
        ...
        for column in obj.sqlmeta.joins:
            ...
            if coltype in ('SOMultipleJoin', 'SOSQLMultipleJoin'):
                query = conn.sqlrepr(Select([column.soClass.q.id, func.Count(column.otherClass.q.id)], where=AND(column.soClass.q.id == self.join_foreign_key(column), IN(column.soClass.q.id, ids)), groupBy=column.soClass.q.id))
            elif coltype in ('SORelatedJoin', 'SOSQLRelatedJoin'):
                d = (
                 column.intermediateTable, column.joinColumn, column.intermediateTable, column.otherColumn, column.intermediateTable, column.intermediateTable, column.joinColumn, (',').join([ '%s' % x for x in ids ]), column.intermediateTable, column.joinColumn)
                ...
                query = conn.sqlrepr(Select([column.soClass.q.id, getattr(column.otherClass.q, alias)], where=AND(column.soClass.q.id == self.join_foreign_key(column), IN(column.soClass.q.id, ids))))
            if not query:
                continue
    ...
    def foreign_key_query(self, column, alias, ids):
        ...
        query = conn.sqlrepr(Select([sql_object.q.id, getattr(sql_object.q, alias)], where=IN(sql_object.q.id, ids)))
        fk_values = {}
    ...
    def join_foreign_key(self, column):
        try:
            foreign_key = '%sID' % column.joinColumn.rsplit('_', 1)[0]
    ...
    def column_labels(self, object_name, extended=False):
        ...
        for column_name in sql_object.sqlmeta.columns:
            ...
            if extended:
                ...
                if isinstance(column, sqlobject.col.SOForeignKey):
                    col['column_label'] = self.load_label_column_for_object(column.foreignKey)
