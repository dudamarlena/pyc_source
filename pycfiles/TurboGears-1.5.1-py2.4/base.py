...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\widgets\base.py
# Compiled at: 2011-07-14 09:27:26
"""Base TurboGears widget implementation"""
__all__ = [
 'load_widgets', 'all_widgets', 'Widget', 'CompoundWidget', 'WidgetsList', 'register_static_directory', 'static', 'Resource', 'Link', 'CSSLink', 'JSLink', 'Source', 'CSSSource', 'JSSource', 'js_location', 'mochikit', 'jsi18nwidget', 'WidgetDescription', 'set_with_self']
import os, itertools, warnings, pkg_resources
from cherrypy import request
import tgmochikit
from turbogears import config, startup, view
from turbogears.util import setlike, to_unicode, copy_if_mutable, get_package_name, request_available
...
class Widget(object):
    ...
    def __init__(self, name=None, template=None, engine_name=None, default=None, **params):
        ...
        **params:    Keyword arguments specific to your widget or to any of its
        ...
        if template:
            (self.template_c, self.template, self.engine_name) = load_template(template, self.engine_name)
        ...
        for param in self.__class__.params:
            if param in params:
                ...
                (value, mutable) = copy_if_mutable(getattr(self.__class__, param), True)
                if mutable:
                    setattr(self, param, value)

        for unused in params.iterkeys():
            warnings.warn('keyword argument "%s" is unused at %r instance' % (unused, self.__class__.__name__))
    ...
    def display(self, value=None, **params):
        ...
        **params: Extra parameters specific to the widget. All keyword params
        ...
        if not getattr(self, 'template_c', False):
            warnings.warn(engine_name in view.engines and 'Widget instance %r has no template defined' % self or 'Trying to render a widget, but the %s templating engine is not installed or not yet loaded.' % engine_name)
            return
        self._prepare_params(value, params)
        try:
            transform = view.engines[engine_name].transform
        except (KeyError, AttributeError):
            raise RuntimeError('Trying to render a widget, but the %s templating engine is not installed or not yet loaded.' % engine_name)
        ...
        try:
            ...
            if engine_names:
                adapter = {'kid-genshi': view.kid_xml, 'genshi-kid': view.genshi_et, 'genshi-genshi': list}.get(('-').join(engine_names[-2:]))
                if adapter:
    ...
    def render(self, value=None, format='html', **params):
        ...
        engine_names.append(engine_name)
        try:
            output = render(params, format=format, fragment=True, template=self.template_c)
        finally:
            if engine_names:
                engine_names.pop()
        return output
    ...
    def __repr__(self):
        return '%s(%s)' % (self.__class__.__name__, (', ').join([ '%s=%r' % (var, getattr(self, var)) for var in ['name'] + self.__class__.params ]))
...
class WidgetsList(list):
    ...
    def retrieve_javascript(self):
        return itertools.chain(*(w.retrieve_javascript() for w in self if callable(getattr(w, 'retrieve_javascript', None))))

    def retrieve_css(self):
        return itertools.chain(*(w.retrieve_css() for w in self if callable(getattr(w, 'retrieve_css', None))))
...
def register_static_directory(modulename, directory):
    ...
    config.update({'/tg_widgets/%s' % modulename: {'tools.staticdir.on': True, 'tools.staticdir.dir': directory}})


static = 'turbogears.widgets'
register_static_directory(static, pkg_resources.resource_filename(__name__, 'static'))
register_static_directory('turbogears', pkg_resources.resource_filename('turbogears', 'static'))
...
class Link(Resource):
    ...
    def update_params(self, d):
        ...
        d['link'] = '%s/tg_widgets/%s/%s' % (startup.webpath, self.mod, self.name)

    def __hash__(self):
        return hash(self.mod + self.name)

    def __eq__(self, other):
        return self.mod == getattr(other, 'mod', None) and self.name == getattr(other, 'name', None)
...
class JSLink(Link):
    ...
    params = [
     'charset', 'defer']
    params_doc = {'charset': 'The character encoding of the linked script', 'defer': 'If true, browser may defer execution of the script'}
    charset = None
    defer = False
    ...
    def retrieve_javascript(self):
        ...
        if config.get('tg.mochikit_suppress', False):
            if 'turbogears.mochikit' in config.get('tg.include_widgets', []):
                warnings.warn("tg.mochikit_suppress == True, but 'turbogears.mochikit' is to be included via\n'tg.include_widgets'. This is a contradiction, and mochikit_suppress overrides\nthe inclusion to prevent subtle errors due to version mixing of MochiKit.")
            return []
        javascripts = [ JSLink('tgmochikit', path) for path in tgmochikit.get_paths() ]
        return javascripts
...
class JSI18NWidget(Widget):
    ...
    def register_package_provider(self, package=None, directory=None, domain=None):
        parent = self
        ...
        class PackageProvider(object):
            ...
            def __call__(self, locale):
                ...
                for loc in (locale, locale[:2]):
                    link = JSLink(self._package_name, '%s-%s.js' % (self._domain, loc))
...
class CSSSource(Source):
    ...
    params = [
     'media']
    params_doc = {'src': 'The CSS source for the link', 'media': 'Specify the media the css source link is for'}
    media = 'all'
    retrieve_css = set_with_self
