...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\paginate.py
# Compiled at: 2011-07-08 14:15:28
import types
...
def paginate(var_name, default_order='', limit=10, max_limit=0, max_pages=5, max_sort=1000, dynamic_limit=None):
    ...
    def entangle(func):
        ...
        def decorated(func, *args, **kw):
            ...
            if page is None:
                ...
                log.debug('paginate params: page=%s, limit=%s, order=%s', page, limit_, order)
                output = func(*args, **kw)
                ...
                try:
                    ...
                    raise KeyError('paginate: var_name (%s) not found in output dict' % var_name)
                ...
                if dynamic_limit:
                    try:
                        ...
                        raise KeyError('paginate: dynamic_limit (%s) not found in output dict' % dynamic_limit)
                    else:
                        limit_ = dyn_limit
                if ordering:
                ...
                if order:
                    order = str(order)
                    log.debug('paginate: ordering was %s, sort is %s', ordering, order)
                ...
                if ordering:
                    var_data = sort_data(var_data, ordering, max_sort is None or 0 < row_count <= max_sort)
                if not limit_:
                ...
                paginate_instance = Paginate(current_page=page, limit=limit_, pages=pages_to_show, page_count=page_count, input_values=input_values, order=order, ordering=ordering, row_count=row_count, var_name=var_name)
                ...
                log.debug('paginate: slicing data between %d and %d', offset, endpoint)
                if _simulate_offset is None:
                    ...
                    if _simulate_offset is None:
                        ...
                        if so_db in _so_no_offset or sa_db in _sa_no_offset:
                            ...
                            log.warning('paginate: simulating OFFSET, paginate may be slow (disable with paginate.simulate_offset=False)')
                if _simulate_offset:
                    ...
                    try:
                        ...
                        output[var_name] = [ var_data.next() for i in xrange(offset, endpoint) ]

            return output

        if not get('tg.strict_parameters', False):
            add_tg_args(func, (var_name + '_tgp_' + arg for arg in ('no', 'limit',
                                                                    'order', 'ordering')))
        return decorated
...
class Paginate:
    ...
    def __init__(self, current_page, pages, page_count, input_values, limit, order, ordering, row_count, var_name):
        ...
        if current_page < page_count:
            self.input_values.update({var_name + '_tgp_no': current_page + 1, var_name + '_tgp_limit': limit})
            self.href_next = turbogears.url(cherrypy.request.path_info, self.input_values)
            self.input_values.update({var_name + '_tgp_no': 'last', var_name + '_tgp_limit': limit})
            self.href_last = turbogears.url(cherrypy.request.path_info, self.input_values)
        else:
            self.href_next = None
            self.href_last = None
        if current_page > 1:
            self.input_values.update({var_name + '_tgp_no': current_page - 1, var_name + '_tgp_limit': limit})
            self.href_prev = turbogears.url(cherrypy.request.path_info, self.input_values)
            self.input_values.update({var_name + '_tgp_no': 1, var_name + '_tgp_limit': limit})
            self.href_first = turbogears.url(cherrypy.request.path_info, self.input_values)
        else:
            self.href_prev = None
