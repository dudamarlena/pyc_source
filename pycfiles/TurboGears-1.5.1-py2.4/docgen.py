...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\docgen.py
# Compiled at: 2011-07-14 09:34:33
import os, time
...
class GenSite(Command):
    ...
    def render_template(self, src, dest, depth):
        if not self.force:
            if not self.check_if_newer(src, dest):
                return
            self.dry_run or log.info('rendering %s' % dest)
        else:
        ...
        output = template.serialize(encoding=self.encoding, root=toroot, updated=updated, destfile=destfile, eggs=self.eggs)
        ...
        for (root, dirs, files) in os.walk(srcdir):
            ...
            for file in files:
                ...
                if not processTemplates or ext != '.html':
                    self.copy_if_newer(os.path.join(root, file), dest)
                else:
                    self.render_template(os.path.join(root, file), dest, depth)
    ...
    def run(self):
        ...
        for (dest, src) in self.copydirs.items():
            if os.path.isdir(src):
                self.update_site_files(src, processTemplates=False, destroot=os.path.join(self.destdir, dest))
            else:
