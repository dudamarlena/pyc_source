...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\tests\test_paginate.py
# Compiled at: 2011-07-14 09:18:02
"""Tests for paginate"""
...
def test_sort_in_memory():
    ...
    for access in range(2):
        ...
        if access:
            assert not isinstance(ab[0], dict) and getattr(ab[0], 'a', 'b') == 'a'
        else:
            assert isinstance(ab[0], dict) and ab[0].get('a', 'b') == ['a'] and getattr(ab[0], 'a', 'b') == 'b'
...
class Spy(object):
    ...
    def __str__(self):
        ...
        return 'ok: [paginate %s ]' % (', ').join([ '%s=%r' % (k, v) for (k, v) in paginate.__dict__.items() ])
...
class TestBasicPagination(TestPagination):
    ...
    class MyRoot(RootController):
        ...
        def basic(self):
            spy = Spy(var_name='data', pages=xrange(1, 4), limit=4, page_count=3, order=None, ordering=[], row_count=10)
            data = range(10)
            return dict(data=data, spy=spy)
        ...
        def default_max_pages(self):
            spy = Spy(var_name='data', limit=10, page_count=10, order=None, ordering=[], row_count=100)
            data = range(100)
            return dict(data=data, spy=spy)
        ...
        def dynamic(self):
            spy = Spy(var_name='data', pages=xrange(1, 3), limit=7, page_count=2, order=None, ordering=[], row_count=10)
            data = range(10)
        ...
        def multiple(self):
            spy_foo = Spy(name='foo', var_name='foo', pages=xrange(1, 6), limit=4, page_count=5, order=None, ordering=[], row_count=20)
            spy_bar = Spy(name='bar', var_name='bar', pages=xrange(1, 4), limit=10, page_count=3, order=None, ordering=[], row_count=26)
            foo = range(20)
            bar = list('ABCDEFGHIJKLMNOPQRSTUVWXYZ')
            return dict(foo=foo, bar=bar, spy_foo=spy_foo, spy_bar=spy_bar)

        @expose()
        @paginate('data', limit=4, max_limit=None)
        def empty(self):
            spy = Spy(var_name='data', pages=xrange(1, 1), page_count=0, order=None, ordering=[], row_count=0, current_page=1, first_item=0, last_item=0)
            data = []
            return dict(data=data, spy=spy)
    ...
    def test_strict_parameters(self):
        ...
        try:
            ...
            assert "basic() got an unexpected keyword argument 'data_tgp_rubbish'" in self.body
            self.request('/basic?tg_paginate_no=2', status=500)
            assert "basic() got an unexpected keyword argument 'tg_paginate_no'" in self.body
        finally:
    ...
    def test_href(self):
        self.request('/basic?data_tgp_no=0')
        Spy.assert_ok(self.body, 'current_page', 1)
        Spy.assert_ok(self.body, 'href_first', None)
        Spy.assert_ok(self.body, 'href_prev', None)
        Spy.assert_ok(self.body, 'href_next', "'/basic?data_tgp_no=2&data_tgp_limit=4'", raw=True)
        Spy.assert_ok(self.body, 'href_last', "'/basic?data_tgp_no=last&data_tgp_limit=4'", raw=True)
        self.request('/basic?data_tgp_no=1')
        Spy.assert_ok(self.body, 'current_page', 1)
        Spy.assert_ok(self.body, 'href_first', None)
        Spy.assert_ok(self.body, 'href_prev', None)
        ...
        self.request('/basic?data_tgp_no=2')
        Spy.assert_ok(self.body, 'current_page', 2)
        Spy.assert_ok(self.body, 'href_first', "'/basic?data_tgp_no=1&data_tgp_limit=4'", raw=True)
        Spy.assert_ok(self.body, 'href_prev', "'/basic?data_tgp_no=1&data_tgp_limit=4'", raw=True)
        Spy.assert_ok(self.body, 'href_next', "'/basic?data_tgp_no=3&data_tgp_limit=4'", raw=True)
        Spy.assert_ok(self.body, 'href_last', "'/basic?data_tgp_no=last&data_tgp_limit=4'", raw=True)
        self.request('/basic?data_tgp_no=3')
        Spy.assert_ok(self.body, 'current_page', 3)
        Spy.assert_ok(self.body, 'href_first', "'/basic?data_tgp_no=1&data_tgp_limit=4'", raw=True)
        Spy.assert_ok(self.body, 'href_prev', "'/basic?data_tgp_no=2&data_tgp_limit=4'", raw=True)
        Spy.assert_ok(self.body, 'href_next', None)
        Spy.assert_ok(self.body, 'href_last', None)
        self.request('/basic?data_tgp_no=4')
        Spy.assert_ok(self.body, 'current_page', 3)
        Spy.assert_ok(self.body, 'href_first', "'/basic?data_tgp_no=1&data_tgp_limit=4'", raw=True)
        Spy.assert_ok(self.body, 'href_prev', "'/basic?data_tgp_no=2&data_tgp_limit=4'", raw=True)
        Spy.assert_ok(self.body, 'href_next', None)
    ...
    def test_with_webpath(self):
        ...
        try:
            self.request('/basic?data_tgp_no=2')
            Spy.assert_ok(self.body, 'current_page', 2)
            Spy.assert_ok(self.body, 'href_first', "'/web/path/basic?data_tgp_no=1&data_tgp_limit=4'", raw=True)
            Spy.assert_ok(self.body, 'href_prev', "'/web/path/basic?data_tgp_no=1&data_tgp_limit=4'", raw=True)
            Spy.assert_ok(self.body, 'href_next', "'/web/path/basic?data_tgp_no=3&data_tgp_limit=4'", raw=True)
            Spy.assert_ok(self.body, 'href_last', "'/web/path/basic?data_tgp_no=last&data_tgp_limit=4'", raw=True)
        finally:
...
def create_tables():
    ...
    occupations_table = Table('occupations', metadata, Column('id', Integer, primary_key=True), Column('name', String(20)))
    users_table = Table('users', metadata, Column('id', Integer, primary_key=True), Column('name', String(20)), Column('occupation_id', Integer, ForeignKey('occupations.id')))
    addresses_table = Table('addresses', metadata, Column('id', Integer, primary_key=True), Column('user_id', Integer, ForeignKey('users.id')), Column('street', String(50)), Column('city', String(40)))
    mapper(Occupation, occupations_table)
    mapper(User, users_table, properties={'occupation': relation(Occupation, lazy=False), 'addresses': relation(Address, backref='user', lazy=False)})
    mapper(Address, addresses_table)
    metadata.create_all()
    occupations = ({'id': 1, 'name': 'Doctor'}, {'id': 2, 'name': 'Actor'}, {'id': 3, 'name': 'Programmer'})
    users = ({'id': 1, 'name': 'Smith', 'occupation_id': 1}, {'id': 2, 'name': 'John', 'occupation_id': 2}, {'id': 3, 'name': 'Fred', 'occupation_id': 2}, {'id': 4, 'name': 'Albert', 'occupation_id': 3}, {'id': 5, 'name': 'Nicole', 'occupation_id': None}, {'id': 6, 'name': 'Sarah', 'occupation_id': None}, {'id': 7, 'name': 'Walter', 'occupation_id': 1}, {'id': 8, 'name': 'Bush', 'occupation_id': None})
    addresses = ({'id': 1, 'user_id': None, 'street': 'street P', 'city': 'city X'}, {'id': 2, 'user_id': 2, 'street': 'street B', 'city': 'city Z'}, {'id': 3, 'user_id': 4, 'street': 'street C', 'city': 'city X'}, {'id': 4, 'user_id': 1, 'street': 'street D', 'city': 'city Y'}, {'id': 5, 'user_id': None, 'street': 'street E', 'city': 'city Z'}, {'id': 6, 'user_id': 1, 'street': 'street F', 'city': 'city Z'}, {'id': 7, 'user_id': 3, 'street': 'street G', 'city': 'city Y'}, {'id': 8, 'user_id': 7, 'street': 'street H', 'city': 'city Y'}, {'id': 9, 'user_id': 1, 'street': 'street I', 'city': 'city X'}, {'id': 10, 'user_id': 6, 'street': 'street J', 'city': 'city X'}, {'id': 11, 'user_id': 3, 'street': 'street K', 'city': 'city Z'}, {'id': 12, 'user_id': 2, 'street': 'street L', 'city': 'city X'}, {'id': 13, 'user_id': 8, 'street': 'street M', 'city': 'city X'}, {'id': 14, 'user_id': 5, 'street': 'street N', 'city': 'city Y'}, {'id': 15, 'user_id': 1, 'street': 'street O', 'city': 'city Y'}, {'id': 16, 'user_id': 3, 'street': 'street A', 'city': 'city X'})
...
class TestDatabasePagination(TestPagination):
    ...
    class MyRoot(RootController):
        ...
        def __common(self, method=None):
            ...
            spy = Spy(var_name='data', pages=xrange(1, 3), limit=10, page_count=2, order=None, row_count=16)
            return dict(data=data, spy=spy)
        ...
        def zero_limit(self, method=None):
            ...
            spy = Spy(var_name='data', pages=xrange(1, 2), limit=16, page_count=1, order=None, row_count=16)
            return dict(data=data, spy=spy)
        ...
        def empty_with_groupby(self):
            ...
            spy = Spy(var_name='data', pages=xrange(1, 1), limit=10, page_count=0, order=None, row_count=0)
            return dict(data=data, spy=spy)
    ...
    def test_ordering(self):
        for test in ('basic1', 'basic2'):
            for method in query_methods:
                self.request('/%s?method=%s&data_tgp_ordering=%s' % (test, method, 'street'))
                self.assert_order(16, 2, 3, 4, 5, 6, 7, 8, 9, 10)
                Spy.assert_ok(self.body, 'current_page', 1)
                Spy.assert_ok(self.body, 'first_item', 1)
                Spy.assert_ok(self.body, 'last_item', 10)
                Spy.assert_ok(self.body, 'ordering', ['street'])
                self.request('/%s?method=%s&data_tgp_no=2&data_tgp_ordering=%s' % (test, method, 'street'))
    ...
    def test_strict_parameters(self):
        config.update({'tg.strict_parameters': True})
        try:
            self.request('/basic1?method=Q&data_tgp_ordering=street', status=200)
            self.assert_order(16, 2, 3, 4, 5, 6, 7, 8, 9, 10)
            Spy.assert_ok(self.body, 'ordering', ['street'])
            self.request('/basic1?method=Q&tg_paginate_ordering=street', status=500)
            assert "got an unexpected keyword argument 'tg_paginate_ordering'" in self.body
        finally:
            config.update({'tg.strict_parameters': False})
    ...
    def test_compound_ordering(self):
        for test in ('basic1', 'basic2'):
            for method in query_methods:
                self.request('/%s?method=%s&data_tgp_ordering=%s' % (test, method, quote('city,street')))
                self.assert_order(16, 3, 9, 10, 12, 13, 1, 4, 7, 8)
                Spy.assert_ok(self.body, 'ordering', ['city', 'street'])
                self.request('/%s?method=%s&data_tgp_no=2&data_tgp_ordering=%s' % (test, method, quote('city,street')))
    ...
    def test_default_compound_ordering_1(self):
        for method in query_methods:
            ...
            self.assert_order(16, 3, 9, 10, 12, 13, 1, 4, 7, 8)
            Spy.assert_ok(self.body, 'ordering', ['city', 'street'])
            self.request('/default_compound_ordering1?method=%s&data_tgp_no=2' % method)
            self.assert_order(14, 15, 2, 5, 6, 11)
            Spy.assert_ok(self.body, 'ordering', ['city', 'street'])
    ...
    def test_default_compound_ordering_2(self):
        for method in query_methods:
            ...
            self.assert_order(1, 13, 12, 10, 9, 3, 16, 15, 14, 8)
            Spy.assert_ok(self.body, 'ordering', ['city', '-street'])
            self.request('/default_compound_ordering2?method=%s&data_tgp_no=2' % method)
            self.assert_order(7, 4, 11, 6, 5, 2)
            Spy.assert_ok(self.body, 'ordering', ['city', '-street'])
    ...
    def test_default_compound_ordering_3(self):
        for method in query_methods:
            ...
            self.assert_order(2, 5, 6, 11, 4, 7, 8, 14, 15, 16)
            Spy.assert_ok(self.body, 'ordering', ['-city', 'street'])
            self.request('/default_compound_ordering3?method=%s&data_tgp_no=2' % method)
            self.assert_order(3, 9, 10, 12, 13, 1)
            Spy.assert_ok(self.body, 'ordering', ['-city', 'street'])
    ...
    def test_default_compound_ordering_4(self):
        for method in query_methods:
            ...
            self.assert_order(11, 6, 5, 2, 15, 14, 8, 7, 4, 1)
            Spy.assert_ok(self.body, 'ordering', ['-city', '-street'])
            self.request('/default_compound_ordering4?method=%s&data_tgp_no=2' % method)
            self.assert_order(13, 12, 10, 9, 3, 16)
            Spy.assert_ok(self.body, 'ordering', ['-city', '-street'])
    ...
    def test_related_objects_ordering_level_1(self):
        ...
        self.request('/related?data_tgp_no=2&data_tgp_ordering=%s' % quote('user.name,id'))
    ...
    def test_related_objects_ordering_level_2(self):
        self.request('/related?data_tgp_ordering=%s' % quote('user.occupation.name,id'))
        self.assert_order(1, 5, 10, 13, 14, 2, 7, 11, 12, 16)
        Spy.assert_ok(self.body, 'ordering', ['user.occupation.name', 'id'])
        self.request('/related?data_tgp_no=2&data_tgp_ordering=%s' % quote('user.occupation.name,id'))
    ...
    def test_related_objects_with_explicit_join(self):
        self.request('/related?join=%s&data_tgp_ordering=%s' % (quote('user,occupation'), quote('user.occupation.name,id')))
        self.assert_order(1, 5, 10, 13, 14, 2, 7, 11, 12, 16)
        Spy.assert_ok(self.body, 'ordering', ['user.occupation.name', 'id'])
        self.request('/related?join=%s&data_tgp_ordering=%s' % (quote('user,occupation'), quote('-user.occupation.name,-id')))
        self.assert_order(3, 15, 9, 8, 6, 4, 16, 12, 11, 7)
