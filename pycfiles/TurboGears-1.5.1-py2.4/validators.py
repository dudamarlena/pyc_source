...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\validators.py
# Compiled at: 2011-07-14 07:12:10
...
def StateProvince(*kw, **kwargs):
    warnings.warn('Please use national.USStateProvince', DeprecationWarning, 2)
    return national.USStateProvince(*kw, **kwargs)


def PhoneNumber(*kw, **kwargs):
    warnings.warn('Please use national.USPhoneNumber', DeprecationWarning, 2)
    return national.USPhoneNumber(*kw, **kwargs)


def PostalCode(*kw, **kwargs):
    warnings.warn('Please use national.USPostalCode', DeprecationWarning, 2)
    return national.USPostalCode(*kw, **kwargs)


def IPhoneNumberValidator(*kw, **kwargs):
    warnings.warn('Please use national.InternationalPhoneNumber', DeprecationWarning, 2)
...
class Money(TgFancyValidator):
    ...
    __module__ = __name__
    messages = {'badFormat': _('Invalid number format'), 'empty': _('Empty values not allowed')}
...
class DateTimeConverter(TgFancyValidator):
    ...
    __module__ = __name__
    format = '%Y/%m/%d %H:%M'
    messages = {'badFormat': _('Invalid datetime format'), 'empty': _('Empty values not allowed')}
    ...
    def _to_python(self, value, state):
        ...
        if value and isinstance(value, datetime):
            ...
            return datetime(year=tpl.tm_year, month=tpl.tm_mon, day=tpl.tm_mday, hour=tpl.tm_hour, minute=tpl.tm_min, second=tpl.tm_sec)
...
class FieldStorageUploadConverter(TgFancyValidator):
    __module__ = __name__
    messages = {'notEmpty': _('Filename must not be empty')}
