...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\identity\tests\test_identity.py
# Compiled at: 2011-07-14 07:31:19
"""ORM independent Identity tests and test controllers."""
import unittest, formencode, cherrypy
from turbogears import identity, expose
from turbogears.controllers import Controller, RootController
from turbogears.identity import SecureObject, SecureResource, in_group, in_all_groups, in_any_group, not_anonymous, has_permission, has_all_permissions, has_any_permission, from_host, from_any_host, Any, All, NotAny
...
class IdentityRoot(RootController):
    ...
    def test_params(self, **kwargs):
        ...
        expected_params = to_datastruct(dict([ p.split('=') for p in params.split('&') ]))
        params_ok = True
        if not expected_params['b'].decode('utf-8') == cherrypy.request.params['b']:
            params_ok = False
        if not expected_params['d']['a'].decode('utf-8') == cherrypy.request.params['d']['a']:
            params_ok = False
        if params_ok:
            return 'params ok'
        else:
