...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\tests\test_controllers.py
# Compiled at: 2011-11-26 15:42:10
import unittest, pkg_resources, formencode, cherrypy
try:
    import json
except ImportError:
    import simplejson as json

from turbogears import absolute_url, config, controllers, database, error_handler, exception_handler, expose, flash, redirect, testutil, url, util, validate, validators
...
class MyRoot(controllers.RootController):
    ...
    def validation_error_handler(self, tg_source, tg_errors, *args, **kw):
        ...
        return dict(msg='Error Message', values=kw, errors=errors, functionname=tg_source.__name__)
    ...
    @expose()
    def servefile(self):
        return cherrypy.lib.static.serve_file(pkg_resources.resource_filename('turbogears.tests', 'test_controllers.py'))
    ...
    def returnedtemplate(self):
        return dict(title='Foobar', mybool=False, someval='foo', tg_template='kid:turbogears.tests.simple')
    ...
    @expose()
    @validate(validators={'firstname': validators.String(min=2, not_empty=True), 'lastname': validators.String()})
    @error_handler(validation_error_handler)
    def save(self, submit, firstname, lastname='Miller'):
        fullname = '%s %s' % (firstname, lastname)
        return dict(firstname=firstname, lastname=lastname, fullname=fullname, submit=submit)
    ...
    def flash_data_structure(self):
        flash(dict(uni='ünicode', testing=[1, 2, 3]))
        return dict(title='Foobar', mybool=False, someval='niggles')

    @expose('kid:turbogears.tests.simple', allow_json=True)
    def flash_redirect(self):
        flash('redirect ünicode')
...
class TestRoot(testutil.TGTest):
    ...
    def test_js_files(self):
        ...
        assert response.headers['Content-Type'] in ('text/javascript', 'text/x-js',
    ...
    def test_strict_parameters(self):
        ...
        response = self.app.get('/save?submit=save&firstname=Foo&lastname=Bar&badparam=1', status=500)
    ...
    def test_validation_list_post(self):
        ...
        response = self.app.post('/valuelist', 'value=', {'Content-Type': 'application/x-www-form-urlencoded'})
        assert response.body == ''
        response = self.app.post('/valuelist', 'value=42', {'Content-Type': 'application/x-www-form-urlencoded'})
        assert response.body == '42'
        response = self.app.post('/valuelist', 'value=4&value=2', {'Content-Type': 'application/x-www-form-urlencoded'})
        assert response.body == '4,2', 'values not properly passed as list'
        response = self.app.post('/valuelist', '--AaB03x\r\nContent-Disposition: form-data; name="value"\r\n\r\n\r\n--AaB03x--', {'Content-Type': 'multipart/form-data; boundary=AaB03x'})
        assert response.body == ''
        response = self.app.post('/valuelist', '--AaB03x\r\nContent-Disposition: form-data; name="value"\r\n\r\n42\r\n--AaB03x--', {'Content-Type': 'multipart/form-data; boundary=AaB03x'})
        assert response.body == '42'
        response = self.app.post('/valuelist', '--AaB03x\r\nContent-Disposition: form-data; name="value"\r\n\r\n4\r\n--AaB03x\r\nContent-Disposition: form-data; name="value"\r\n\r\n2\r\n--AaB03x--', {'Content-Type': 'multipart/form-data; boundary=AaB03x'})
    ...
    def test_flash_unicode(self):
        ...
        values = json.loads(response.body)
        assert values['tg_flash'] == 'ünicode'
        assert 'tg_flash' not in response.headers
    ...
    def test_flash_on_redirect(self):
        ...
        response = self.app.get(response.location, headers=dict(Cookie=response.headers['Set-Cookie']))
        values = json.loads(response.body)
        assert values['tg_flash'] == 'redirect ünicode'
    ...
    def test_flash_redirect_with_trouble_chars(self):
        ...
        response = self.app.get(response.location, headers=dict(Cookie=response.headers['Set-Cookie']))
        values = json.loads(response.body)
        assert values['tg_flash'] == '$foo, käse;\tbar!'

    def test_double_flash(self):
        """latest set flash should have precedence"""
        response = self.app.get('/flash_plain?tg_format=json', headers=dict(Cookie='tg_flash="old flash"; Path=/;'))
        values = json.loads(response.body)
        assert values['tg_flash'] == 'plain'
        assert 'tg_flash' in response.cookies_set, 'Cookie clearing request should be present'
    ...
    def test_internal_redirect_nested_variables(self):
        """regression test for #1022, #1407 and #1598"""
        response = self.app.get('/internal_redirect?a=1&a-1.b=2&a-2.c=3&a-2.c-1=4')
        assert 'redirected OK' in response
...
class TestURLs(testutil.TGTest):
    ...
    def test_multi_values(self):
        self.app.get('/')
        assert url('/foo', bar=[1, 2]) in ['/foo?bar=1&bar=2', '/foo?bar=2&bar=1']
        assert url('/foo', bar=('asdf', 'qwer')) in ['/foo?bar=qwer&bar=asdf', '/foo?bar=asdf&bar=qwer']

    def test_unicode(self):
        """url() can handle unicode parameters"""
        self.app.get('/')
        assert url('/', x='àèìòù') == '/?x=%C3%A0%C3%A8%C3%AC%C3%B2%C3%B9'

    def test_list(self):
        """url() can handle list parameters, with unicode too"""
        self.app.get('/')
        assert url('/', foo=['bar', 'à']) == '/?foo=bar&foo=%C3%A0'
...
class TestAbsoluteURLs(testutil.TGTest):
    ...
    def test_scheme_from_config(self):
        ...
        hostname = 'foo.org'
        scheme = 'https'
        self._config_update({'base_url_filter.on': False, 'base_url_filter.use_x_forwarded_host': True, 'tg.url_scheme': scheme})
        url = self.app.get('/absolute_url/?url=%2Ffoo', headers={'X-Forwarded-Host': hostname}).body
        self._config_reset()
    ...
    def test_scheme_from_header(self):
        ...
        hostname = 'foo.org'
        scheme = 'https'
        self._config_update({'base_url_filter.on': False, 'base_url_filter.use_x_forwarded_host': True})
        url = self.app.get('/absolute_url/?url=%2Ffoo', headers={'X-Forwarded-Host': hostname, 'X-Use-SSL': scheme}).body
        self._config_reset()
        assert url == '%s://%s/foo' % (scheme, hostname)
    ...
    def test_use_base_url(self):
        ...
        hostname = 'foo.org'
        port = 1234
        self._config_update({'base_url_filter.on': True, 'base_url_filter.use_x_forwarded_host': False, 'base_url_filter.base_url': 'http://%s:%s' % (hostname, port)})
    ...
    def test_use_config(self):
        ...
        hostname = 'foo.org'
        port = 1234
        self._config_update({'base_url_filter.on': False, 'base_url_filter.use_x_forwarded_host': False, 'tg.url_domain': '%s:%s' % (hostname, port)})
        url = self.app.get('/absolute_url/?url=%2Ffoo').body
        self._config_reset()
        print url
        assert url == 'http://%s:%s/foo' % (hostname, port)
    ...
    def test_use_host(self):
        ...
        self._config_update({'base_url_filter.on': False, 'base_url_filter.use_x_forwarded_host': False})
        hostname = 'foo.org'
        url = self.app.get('/absolute_url/?url=%2Ffoo', headers={'Host': hostname}).body
        self._config_reset()
        print url
    ...
    def test_use_server_config(self):
        ...
        hostname = 'foo.org'
        port = 1234
        self._config_update({'base_url_filter.on': False, 'base_url_filter.use_x_forwarded_host': False, 'server.socket_host': hostname, 'server.socket_port': port})
        url = self.app.get('/absolute_url/?url=%2Ffoo', headers={'Host': ''}).body
        self._config_reset()
        print url
        assert url == 'http://%s:%s/foo' % (hostname, port)
