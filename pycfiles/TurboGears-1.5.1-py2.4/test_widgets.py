...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\widgets\tests\test_widgets.py
# Compiled at: 2011-07-14 09:26:59
import os
from turbogears import config, controllers, expose, testutil, validators, view, widgets
...
def test_unicode_input():
    ...
    output = tf.render('Pete Ĝ', format='xhtml')
    assert 'value="Pete Ĝ"' in output
    try:
        tf.render(b'Pete \xfe\xcd')
    except ValueError:
        pass
    else:
...
def test_widget_js():
    ...
    expected = ('<script ', ' src="/tg_widgets/turbogears.widgets/foo.js"', ' type="text/javascript"',
                '></script>')
    ...
    js3 = widgets.JSLink(mod=widgets.static, name='foo.js', defer=False, charset=None)
    assert js3 == js
...
def test_declared_widgets_stay_as_attrs():
    ...
    assert hasattr(w1, 'foo'), 'Declared widgets should be kept accessible as attribute'
    ...
    assert not hasattr(w3, 'foo'), 'Only declared widgets should be accessible as attribute'
...
def test_selection_field():
    ...
    assert '<option selected="selected" value="2">' in output or '<option value="2" selected="selected">' in output
    assert '<option value="3">' in output
    w = widgets.SingleSelectField(options=options, default=3)
    output = w.render(format='xhtml')
    assert '<option value="1">' in output
    assert '<option value="2">' in output
    assert '<option selected="selected" value="3">' in output or '<option value="3" selected="selected">' in output
    output = w.render(options=options + [(4, 'cobol'), (5, 'ruby')], format='xhtml')
    assert 'python' in output
    assert 'java' in output
    assert 'pascal' in output
    assert 'cobol' in output
    assert 'ruby' in output
    output = w.render(options=options + [(4, 'cobol'), (5, 'ruby')], value=5, format='xhtml')
    assert '<option value="1">' in output
    assert '<option value="2">' in output
    assert '<option value="3">' in output
    assert '<option value="4">' in output
    assert '<option selected="selected" value="5">' in output or '<option value="5" selected="selected">' in output
    w = widgets.MultipleSelectField(options=options, default=[1, 3])
    output = w.render(format='xhtml')
    assert '<option selected="selected" value="1">' in output or '<option value="1" selected="selected">' in output
    assert '<option value="2">' in output
    assert '<option selected="selected" value="3">' in output or '<option value="3" selected="selected">' in output
    output = w.render(options=options + [(4, 'cobol'), (5, 'ruby')], value=[2, 4, 5], format='xhtml')
    assert '<option value="1">' in output
    assert '<option selected="selected" value="2">' in output or '<option value="2" selected="selected">' in output
    assert '<option value="3">' in output
    assert '<option selected="selected" value="4">' in output or '<option value="4" selected="selected">' in output
    assert '<option selected="selected" value="5">' in output or '<option value="5" selected="selected">' in output
...
def test_selection_field_converted_value():
    ...
    single = widgets.SingleSelectField('testval', options=[(TestValue(1), 'one'), (TestValue(2), 'two'), (TestValue(3), 'three'), (TestValue(4), 'four')], validator=TestValidator())
    output = single.render(format='xhtml')
    assert '<option value="1">one</option><option value="2">two</option><option value="3">three</option><option value="4">four</option>' in output, output
    ...
    assert '<option value="1">one</option><option value="2">two</option><option * value="3">three</option><option value="4">four</option>' in selected, output
    assert single.render('3', format='xhtml') == output
    multiple = widgets.MultipleSelectField('testval', options=[(TestValue(1), 'one'), (TestValue(2), 'two'), (TestValue(3), 'three'), (TestValue(4), 'four')], validator=validators.ForEach(TestValidator()))
    ...
    assert multiple.render('3', format='xhtml') == output
    ...
    assert multiple.render(['1', '3'], format='xhtml') == output
    assert multiple.render(set(['1', '3']), format='xhtml') == output
    return


def test_empty_selection_field():
    """Check that the section field value can be empty."""
    single = widgets.SingleSelectField('testval', options=[(None, 'undefined'), (1, 'one'), (2, 'two'), (3, 'three')], validator=validators.Int())
    output = single.render(format='xhtml')
    ...
    assert '<option value="">undefined</option>' in output
    assert '<option * value="2">two</option>' in output
    multiple = widgets.MultipleSelectField('testval', options=[(None, 'undefined'), (1, 'one'), (2, 'two'), (3, 'three')], validator=validators.Int())
    output = multiple.render(format='xhtml')
    assert '<option value="">undefined</option>' in output
    assert '<option value="2">two</option>' in output
...
def test_simple_widget_attrs():
    ...
    output = w.render(attrs=dict(onchange='java', size='50', alt=None), format='xhtml')
...
def test_render_field_for():
    ...
    output = tf.render_field_for('name', attrs={'onclick': 'hello'}, format='xhtml')
    ...
    output = tf.render_field_for('age', rows='1000', cols='2000', format='xhtml')
...
def test_css_classes():
    ...
    w = widgets.TextField(name='foo', css_classes=['bar', 'bye'], attrs={'class': 'again'})
    output = w.render(format='xhtml')
    assert 'class="again"' in output
    output = w.render(css_classes=['coke', 'pepsi'], format='xhtml')
    assert 'class="again"' in output
    output = w.render(css_classes=['coke', 'pepsi'], attrs={'class': 'funny'}, format='xhtml')
...
class TestSchemaValidation:
    ...
    def test_goodvalues(self):
        values = dict(name='JosÃ©', age='99', passwd='fado', passwd2='fado')
...
class TestSchemaValidationWithChildWidgetsValidators:
    ...
    def test_widget_validator_failure(self):
        values = dict(name='JosÃ©', age='ninetynine', passwd='fado', passwd2='fado')
        (values, errors) = catch_validation_errors(self.form, values)
...
def test_selectfield_with_with_non_iterable_option_elements():
    ...
    assert '<option selected="selected" value="python">' in output or '<option value="python" selected="selected">' in output
    assert '<option value="java">' in output
    assert '<option value="pascal">' in output
...
def test_calendardatepicker():
    ...
    """
    w = widgets.CalendarDatePicker(name='test_cdp', field_class='my_cdp', label='Test', format='%m/%d/%Y', validator=validators.DateConverter(format='mm/dd/yyyy'))
    output = w.render(format='xhtml')
...
def test_calendardatetimepicker():
    ...
    """
    w = widgets.CalendarDateTimePicker(name='test_cdp', field_class='cdp', label='Test', format='%m/%d/%Y', validator=validators.DateConverter(format='mm/dd/yyyy'))
    output = w.render(format='xhtml')
...
def test_autocompletefield():
    ...
    """
    w = widgets.AutoCompleteField(name='my_acf', field_class='acf', search_controller='/search', search_param='test', result_name='test')
    output = w.render(format='xhtml')
...
def test_autocompletetextfield():
    ...
    """
    w = widgets.AutoCompleteTextField(name='my_actf', field_class='actf', search_controller='/search', search_param='test', result_name='test')
    output = w.render(format='xhtml')
...
def test_jsi18nwidget():
    ...
    os.path.exists = lambda path: os.path.split(path)[1] == 'messages-ru.js' or exists(path)
    try:
