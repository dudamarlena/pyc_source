...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\widgets\tests\test_request_related_features.py
# Compiled at: 2011-07-14 07:59:12
import re
from cherrypy import request
from turbogears import controllers, expose, error_handler, testutil, validate, validators, widgets
...
def test_required_fields():
    ...
    assert re.search(('.*').join([class_p, name_p]), output) or re.search(('.*').join([name_p, class_p]), output)
    name_p = 'name="name"'
...
def test_two_form_in_the_same_page():
    ...
    class MyRoot(controllers.RootController):
        ...
        def test(self, age, email):
            ...
            return dict(form1=form1, form2=form2, validated_form=validated_form, form1_valid=form1_valid, form2_valid=form2_valid)
        ...
        def test2(self):
            ...
            form1_valid = form1.is_validated
            form2_valid = form2.is_validated
            return dict(form=form2, validated_form=validated_form, form1_valid=form1_valid, form2_valid=form2_valid)
        ...
        def test3(self):
            ...
            form1_valid = form1.is_validated
            form2_valid = form2.is_validated
            return dict(form=form1, validated_form=validated_form, form1_valid=form1_valid, form2_valid=form2_valid)
    ...
    for engine in ('genshi', 'kid'):
        ...
        value_p = 'value="foo"'
        id_p = 'id="form1_age"'
        assert re.search(('.*').join([value_p, id_p]), output) or re.search(('.*').join([id_p, value_p]), output)
        value_p = 'value="foo"'
        id_p = 'id="form2_age"'
        assert not (re.compile(('.*').join([value_p, id_p])).search(output) or re.compile(('.*').join([id_p, value_p])).search(output))
...
def test_repeating_fields():
    ...
    form = widgets.TableForm(name='form', fields=[widgets.RepeatingFieldSet(name='repeat', fields=MyFields(), repetitions=repetitions)])
    ...
    class MyRoot(controllers.RootController):
        ...
        @expose('turbogears.widgets.tests.form')
        def test_value(self):
            value = dict(repeat=[{'name': 'foo', 'comment': 'hello'}, None, None, {'name': 'bar', 'comment': 'byebye'}])
    ...
    value_p = 'value="foo"'
    assert re.search(('.*').join([value_p, name_p]), output) or re.search(('.*').join([name_p, value_p]), output)
    ...
    assert re.search(('.*').join([value_p, name_p]), output) or re.search(('.*').join([name_p, value_p]), output)
    app = testutil.make_app(MyRoot)
    response = app.get('/test_validation?repeat-0.name=foo&repeat-1.name=&repeat-2.name=bar&repeat-3.name=&repeat-4.name=')
    output = response.body.lower()
...
def test_widgets_on_genshi_page():
    ...
    output = response.body.lower()
    assert 'content="genshi"' in output and 'content="kid"' not in output
    assert '<div class="form2"><div class="form1"><form ' in output and '</form></div></div>' in output
    assert 'id="form1"' in output and 'class="tableform' in output
    assert '<table ' in output and '</table>' in output
    assert '<input ' in output and ' type="text"' in output
