...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\command\quickstart.py
# Compiled at: 2011-11-15 15:07:01
...
"""
import optparse, os, re, shutil, stat, sys, pkg_resources, turbogears
from paste.script import templates, create_distro
beginning_letter = re.compile('^[^a-z]*')
valid_only = re.compile('[^a-z0-9_]')
...
class TGTemplate(templates.Template):
    ...
    def run(self, command, output_dirs, vars):
        ...
        vars.setdefault('turbogearsversion', pkg_resources.get_distribution('turbogears').version)
...
class BaseTemplate(TGTemplate):
    ...
    _template_dir = pkg_resources.resource_filename('turbogears.qstemplates', 'qsbase')
...
class TurbogearsTemplate(TGTemplate):
    ...
    _template_dir = pkg_resources.resource_filename('turbogears.qstemplates', 'quickstart')
...
class TGBig(TGTemplate):
    ...
    _template_dir = pkg_resources.resource_filename('turbogears.qstemplates', 'quickstartbig')
    ...
    def post(self, command, output_dir, vars):
        ...
        if os.path.exists(controllersdir) and os.path.exists(controllersfile):
            ...
            if not command.simulate and (controllerstext == roottext or query_interactive(controllersfile, rootfile, controllerstext, roottext, False)):
                shutil.copyfile(controllersfile, rootfile)
                try:
                    if not os.path.exists(os.path.join(os.path.dirname(os.path.abspath(controllersfile)), '.svn')):
                        raise OSError
...
class TGWidgetTemplate(TGTemplate):
    __module__ = __name__
    required_templates = ['tgbase']
    _template_dir = pkg_resources.resource_filename('turbogears.qstemplates', 'widget')
...
def get_requirement(name, pkg=None):
    ...
    raise ValueError('Did not find matching %s requirement in the TurboGears setup.py:extras_require.' % name)
...
class quickstart:
    ...
    def __init__(self, version):
        parser = optparse.OptionParser(usage='%prog quickstart [options] [project name]', version='%prog ' + version)
        parser.add_option('-s', '--sqlalchemy', help='use SQLAlchemy instead of SQLObject (the default)', action='store_true', dest='sqlalchemy', default=False)
        parser.add_option('-e', '--elixir', help='use SQLAlchemy Elixir instead of SQLObject', action='store_true', dest='elixir', default=False)
        parser.add_option('-o', '--sqlobject', help='use SQLObject instead of SQLAlchemy', action='store_true', dest='sqlobject', default=False)
        parser.add_option('-i', '--identity', help='provide Identity support', action='store_true', dest='identity', default=False)
        parser.add_option('', '--no-identity', help="don't prompt for Identity support (ignored with -i)", action='store_false', dest='prompt_identity', default=True)
        parser.add_option('', '--table-prefix', help='add the specified prefix to all generated tables', dest='table_prefix')
        parser.add_option('-p', '--package', help='package name for the code', dest='package')
        parser.add_option('-t', '--templates', help='user specific templates', dest='templates', default=self.templates)
        parser.add_option('-r', '--svn-repository', metavar='REPOS', help='create project in given SVN repository', dest='svn_repository', default=self.svn_repository)
        parser.add_option('--dry-run', help="dry run (don't actually do anything)", action='store_true', dest='dry_run')
        (options, args) = parser.parse_args()
        self.__dict__.update(options.__dict__)
        if True not in (self.elixir, self.sqlalchemy, self.sqlobject):
...
class update:
    ...
    def __init__(self, version):
        self.parser = parser = optparse.OptionParser(usage='%prog update [options]', version='%prog ' + version)
        parser.add_option('-s', '--sqlalchemy', help='use SQLAlchemy instead of SQLObject', action='store_true', dest='sqlalchemy', default=False)
        parser.add_option('-e', '--elixir', help='use SQLAlchemy Elixir instead of SQLObject', action='store_true', dest='elixir', default=False)
        parser.add_option('-o', '--sqlobject', help='use SQLObject instead of SQLAlchemy', action='store_true', dest='sqlobject', default=False)
        parser.add_option('-i', '--identity', help='provide Identity support', action='store_true', dest='identity', default=False)
        parser.add_option('-t', '--templates', help='user specific templates', dest='templates', default=self.templates)
    ...
    def run(self):
        ...
        try:
            ...
            project_requires = [ r.project_name.lower() for r in project_dist.requires() ]
            self.parser.set_defaults(sqlobject='sqlobject' in project_requires, sqlalchemy='sqlalchemy' in project_requires, elixir='elixir' in project_requires)
        except pkg_resources.DistributionNotFound:
            print 'Could not determine requirements for your application.'
            print "Please run 'python setup.py develop' to make your " + 'application known to setuptools.'
        ...
        if not currentdir == self.name:
            print 'It looks like your project directory "%s" is named incorrectly.' % currentdir
