...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\errorhandling.py
# Compiled at: 2011-07-14 07:10:29
"""Error handling functions."""
__all__ = [
 'dispatch_error', 'dispatch_error_adaptor', 'try_call', 'run_with_errors', 'default', 'register_handler', 'FailsafeSchema', 'dispatch_failsafe', 'error_handler', 'exception_handler']
import sys
from itertools import izip, islice
from inspect import getargspec
import cherrypy
from peak.rules import abstract, when, around, NoApplicableMethods
from turbogears.util import inject_args, adapt_call, call_on_stack, has_arg, remove_keys, Enum
from turbogears.decorator import func_id

@abstract()
def dispatch_error(controller, tg_source, tg_errors, tg_exceptions, *args, **kw):
    """Dispatch error.
...
def _register_implicit_errh(controller, tg_source, tg_errors, tg_exceptions, *args, **kw):
    ...
    return dispatch_error(controller, tg_source, tg_errors, tg_exceptions, *args, **kw)


@when(dispatch_error, "tg_exceptions and has_arg(tg_source, 'tg_exceptions')")
def _register_implicit_exch(controller, tg_source, tg_errors, tg_exceptions, *args, **kw):
...
def dispatch_error_adaptor(func):
    ...
    def adaptor(controller, tg_source, tg_errors, tg_exceptions, *args, **kw):
        ...
        (args, kw) = inject_args(func, {'tg_source': tg_source, 'tg_errors': tg_errors, 'tg_exceptions': tg_exceptions}, args, kw, 1)
...
def try_call(func, self, *args, **kw):
    ...
    try:
        ...
        if isinstance(e, cherrypy.HTTPRedirect) or call_on_stack('dispatch_error', {'tg_source': func, 'tg_exception': e}, 4):
            raise
        else:
...
def run_with_errors(errors, func, self, *args, **kw):
    ...
    if errors:
        if hasattr(self, 'validation_error'):
            import warnings
            warnings.warn('Use decorator error_handler() on per-method base rather than defining a validation_error() method.', DeprecationWarning, 2)
            ...
            try:
                ...
                raise NotImplementedError('Method %s.%s() has no applicable error handler.' % (self.__class__.__name__, func.__name__))
...
def register_handler(handler=None, rules=None):
    ...
    def register(func):
        ...
        around(dispatch_error, new_rules)(dispatch_error_adaptor(handler or func))
        return func
...
@when(dispatch_failsafe, 'schema is FailsafeSchema.values and isinstance(values, dict) and isinstance(errors, dict)')
...
@when(dispatch_failsafe, 'schema is FailsafeSchema.values and isinstance(errors, dict)')
...
@when(dispatch_failsafe, 'schema is FailsafeSchema.map_errors and isinstance(errors, dict)')
...
@when(dispatch_failsafe, 'schema is FailsafeSchema.defaults and isinstance(errors, dict)')
def _failsafe_defaults(schema, values, errors, source, kw):
    ...
    defaults = dict(izip(islice(argnames, len(argnames) - len(defaultvals), None), defaultvals))
    for key in errors:
        if key in defaults:
            kw[key] = defaults[key]
