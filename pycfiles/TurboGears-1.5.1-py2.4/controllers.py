...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\controllers.py
# Compiled at: 2011-10-08 07:05:19
"""Classes and methods for TurboGears controllers."""
__all__ = [
 'Controller', 'absolute_url', 'error_handler', 'exception_handler', 'expose', 'get_server_name', 'flash', 'redirect', 'Root', 'RootController', 'url', 'validate']
import logging, urllib, urlparse, types
...
if config.get('tools.sessions.on', False):
    if config.get('tools.sessions.storage_type') == 'PostgreSQL':
        import psycopg2
        config.update({'tools.sessions.get_db': psycopg2.connect(config.get('sessions.postgres.dsn'))})

def _process_output(output, template, format, content_type, fragment=False, **options):
    ...
    if isinstance(output, dict):
        ...
        output = view.render(output, template=template, format=format, headers=headers, fragment=fragment, **options)
...
def validate(form=None, validators=None, failsafe_schema=errorhandling.FailsafeSchema.none, failsafe_values=None, state_factory=None):
    """Validate input.
    ...
    def entangle(func):
        ...
        def validate(func, *args, **kw):
            ...
            if validators:
                if isinstance(validators, dict):
                    for (field, validator) in validators.iteritems():
                        try:
                            kw[field] = validator.to_python(kw.get(field, None), state)
            ...
            if errors:
                kw = errorhandling.dispatch_failsafe(failsafe_schema, failsafe_values, errors, func, kw)
...
def _add_rule(_expose, found_default, as_format, accept_format, template, rulefunc):
    if as_format == 'default':
        if found_default:
            ...
            found_default = True
    ruleparts = [
     "kw.get('tg_format', 'default') == '%s'" % as_format]
    if accept_format:
        ruleparts.append("(accept == '%s' and kw.get('tg_format', 'default') == 'default')" % accept_format)
...
def _build_rules(func):
    ...
    if func._allow_json:
        rule = "allow_json and (kw.get('tg_format') == 'json' or accept in ('application/json', 'text/javascript'))"
        log.debug('Adding allow_json rule for %s: %s', func, rule)
        first(_expose, rule)(lambda _func, accept, allow_json, *args, **kw: _execute_func(_func, 'json', 'json', 'application/json', False, {}, args, kw))
    found_default = False
...
def expose(template=None, allow_json=None, format=None, content_type=None, fragment=False, as_format='default', accept_format=None, **options):
    ...
    if not template:
        template = format
    if format == 'json' or format is None and template is None and (allow_json is None or allow_json):
        template = 'json'
        allow_json = True
    ...
    def entangle(func):
        ...
        log.debug('template: %s, format: %s, allow_json: %s, content-type: %s', template, format, allow_json, content_type)
            ...
            def expose(func, *args, **kw):
                ...
                try:
                    if hasattr(request, 'in_transaction'):
                        output = func._expose(func, accept, func._allow_json, *args, **kw)
                    else:
                        request.in_transaction = True
                        output = database.run_with_transaction(func._expose, func, accept, func._allow_json, *args, **kw)
                except NoApplicableMethods, e:
                    args = e.args
                    if args and args[0] and isinstance(args[0], tuple) and args[0][0] is func:
        ...
        func._ruleinfo.insert(0, dict(as_format=as_format, accept_format=accept_format, template=template, rulefunc=lambda _func, accept, allow_json, *args, **kw: _execute_func(_func, template, format, content_type, fragment, options, args, kw)))
        if allow_json:
...
def _execute_func(func, template, format, content_type, fragment, options, args, kw):
    """Call controller method and process its output."""
    if config.get('tg.strict_parameters', False):
        tg_util.remove_keys(kw, ['tg_random', 'tg_format'] + config.get('tg.ignore_parameters', []))
    else:
        try:
    ...
    if str(getattr(response, 'status', '')).startswith('204'):
        ...
        assert isinstance(output, (
         basestring, dict, list, types.GeneratorType)) or hasattr(output, '__iter__') and hasattr(output, 'next'), 'Method %s.%s() returned unexpected output. Output should be of type basestring, dict, list or generator.' % (args[0].__class__.__name__, func.__name__)
        if isinstance(output, dict):
        ...
        return _process_output(output, template, format, content_type, fragment, **options)
...
class ExposedDescriptor(object):
    ...
    def __get__(self, obj, cls=None):
        ...
        if callable(method) and getattr(method, 'exposed', False):
            return True
        raise cherrypy.HTTPError(405, '%s not allowed on %s' % (cp_methodname, cherrypy.request.browser_url))
        return
...
def get_server_name():
    ...
    if not host:
        host = '%s:%s' % (get('server.socket_host', 'localhost'), get('server.socket_port', 8080))
    return host
...
def redirect(redirect_path, redirect_params=None, **kw):
    ...
    if not (redirect_path.startswith('/') or redirect_path.startswith('http://') or redirect_path.startswith('https://')):
