...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\tests\test_expose.py
# Compiled at: 2011-07-14 09:42:26
try:
...
def test_gettingjsonviaaccept():
    app = make_app(ExposeRoot)
    response = app.get('/with_json_via_accept', headers=dict(Accept='application/json'))
    assert '"title": "Foobar"' in response
...
def test_getting_plaintext():
    app = make_app(ExposeRoot)
    response = app.get('/with_json_via_accept', headers=dict(Accept='text/plain'))
...
def test_expose_pass_options():
    ...
    assert info['fruit'] == 'apple' and info['tree'] == 'Rowan', 'Data dict returned from exposed method not included in output.'
    assert options['cache'] is True, 'Options passed to template engine constructor not used.'
    assert options['renderer'] == 'fast', 'Options passed as kwargs to expose() not passed to render().'
    assert options['strict'] is not True, 'Options passed to expose() do not override options passed to __init__().'
    response = app.get('/with_testplugin_using_mapping')
    (info, options) = loads(response.body)
    assert options['mapping'] == dict(deprecated=True), 'Options passed in mapping arg to expose() not passed to render().'