...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\widgets\tests\test_datagrid.py
# Compiled at: 2011-07-03 10:54:49
...
class TestDataGrid:
    ...
    def test_column_declaration(self):
        ...
        grid = DataGrid(name='grid', fields=[DataGrid.Column('name', options=dict(foobar=123)), ('Subtype', 'subtype'), DataGrid.Column('text', 'excerpt', 'TEXT')])
        d = dict(value='value')
    ...
    def test_alternative_column_declaration(self):
        ...
        g = lambda : None
        grid = DataGrid(fields=[g, ('t', g), ('t', g, dict(p=0)), DataGrid.Column('n', g, 't', dict(p=0))])
        for i in range(4):
    ...
    def test_column_alignment(self):
        ...
        grid = DataGrid(fields=[
         ('left col',
          lambda d: d[0]),
         ('center col',
          lambda d: d[1], dict(align='center')),
         ('right col',
          lambda d: d[2], dict(align='right'))])
        output = grid.render([('left data', 'center data', 'right data')])
        assert '<td>left data</td><td align="center">center data</td><td align="right">right data</td>' in output

    def test_template_overridal(self):
        """Test overriding the template."""
        grid = DataGrid(fields=[('Name', 'name'), ('Subtype', 'subtype'), ('TEXT', Foo.excerpt.fget)], template='turbogears.fastdata.templates.datagrid')
        d = dict(value='value')
        grid.update_params(d)

    def test_wiki_samples(self):
        """Test that sample code on DataGridWidget wiki page actually works."""
        grid = DataGrid(fields=[('ID', 'userId'), ('Name', 'displayName'), ('E-mail', 'emailAddress')])
        users = [
         User(1, 'john', 'john@foo.net'), User(2, 'fred', 'fred@foo.net')]
        ...
        grid = DataGrid(fields=[
         ...
         ('Age',
          lambda row: row[0])])
        data = [(33, 'Anton Bykov', 'Bulgaria'), (23, 'Joe Doe', 'Great Britain'), (44, 'Pablo Martelli', 'Brazil')]
    ...
    def test_attrwrapper_with_dict(self):
        ...
        grid = DataGrid(fields=[('Field 1', 'key1'), ('Field 2', 'key2'), ('Field 3', 'key3')])
    ...
    def test_attrwrapper_with_dict_and_dotted_key(self):
        ...
        grid = DataGrid(fields=[('Field 1', 'key1.sub'), ('Field 2', 'key2.sub'), ('Field 3', 'key3.sub')])
    ...
    def test_attrwrapper_with_obj(self):
        ...
        grid = DataGrid(fields=[('Field 1', 'col1'), ('Field 2', 'col2'), ('Field 3', 'col3')])
    ...
    def test_attrwrapper_with_nested_obj(self):
        ...
        grid = DataGrid(fields=[('Field 1', 'sub1.col'), ('Field 2', 'sub1.sub2.col'), ('Field 3', 'sub1.empty'), ('Field 4', 'sub1.empty.col'), ('Field 5', 'sub1.foobar')])
