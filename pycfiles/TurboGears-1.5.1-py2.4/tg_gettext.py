...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\i18n\tg_gettext.py
...
def is_locale_supported(locale, domain=None):
    ...
    return localedir and os.path.exists(os.path.join(localedir, locale, 'LC_MESSAGES', '%s.mo' % domain))
...
def get_catalog(locale, domain=None):
    ...
    try:
        ...
        try:
            messages = translation(domain=domain, localedir=localedir, languages=languages)
...
def plain_gettext(key, locale=None, domain=None):
    ...
    gettext_func = dict(tg_gettext=tg_gettext, so_gettext=so_gettext, sa_gettext=sa_gettext).get(func_name)
    if gettext_func:
        return gettext_func(key, locale, domain)
    else:
        log.error('Translation disabled - i18n.gettext function %s not available', gettext_func)
        return key
...
def gettext(key, locale=None, domain=None):
    ...
    """
    return (request_available() and plain_gettext or lazy_gettext)(key, locale, domain)
...
def ngettext(key1, key2, num, locale=None):
    ...
    """
    return (request_available() and plain_ngettext or lazy_ngettext)(key1, key2, num, locale)
