...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\identity\tests\test_visit_so.py
# Compiled at: 2011-07-14 06:30:05
...
from turbogears.identity.tests.test_visit import SimpleVisitPlugin, VisitRoot, cookie_header
...
class TestVisit(TestCase):
    ...
    def setUp(self):
        ...
        config.update({'visit.on': True, 'visit.manager': 'sqlobject', 'visit.timeout': 50})
        self.app = testutil.make_app(VisitRoot)
        testutil.start_server(tg_only=True)

    def tearDown(self):
        testutil.stop_server(tg_only=True)
        config.update({'visit.timeout': self._visit_timeout, 'visit.on': self._visit_on, 'visit.manager': self._visit_manager})
    ...
    def test_visit_from_form(self):
        ...
        try:
            ...
            self.app.cookies = {}
            response = self.app.get('/', params={self._visit_key_param: first_key})
        finally:
    ...
    def test_cookie_expires(self):
        ...
        try:
            ...
            self.app = _app
        assert response.cookies_set[self.cookie_name] != morsel, 'cookie values should not match'
        assert response.raw['new'], 'this should be a new visit, as the cookie has expired'
    ...
    def test_cookie_permanent(self):
        ...
        expires = time.mktime(time.strptime(morsel['expires'], '%a, %d-%b-%Y %H:%M:%S GMT')[:8] + (0, ))
...
class TestSetVisitManager(TestCase):
    ...
    def tearDown(self):
        testutil.stop_server(tg_only=True)
        config.update({'visit.on': self._visit_on, 'visit.manager': self._visit_manager, 'visit.timeout': self._visit_timeout})

    def test_visit_manager_from_class(self):
        """Visit manager class is loaded correctly from dotted-path notation."""
        config.update({'visit.manager': 'turbogears.identity.tests.test_visit_so.MyVisitManager'})
        testutil.start_server()
