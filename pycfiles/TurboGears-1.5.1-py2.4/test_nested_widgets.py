...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\turbogears\widgets\tests\test_nested_widgets.py
...
class TestNestedWidgets:
    ...
    def test_display(self):
        ...
        assert re.compile(('.*').join([value_p, name_p])).search(output) or re.compile(('.*').join([name_p, value_p])).search(output)
        ...
        assert re.compile(('.*').join([value_p, name_p])).search(output) or re.compile(('.*').join([name_p, value_p])).search(output)
        assert re.compile(('.*').join([value_p, id_p])).search(output) or re.compile(('.*').join([id_p, value_p])).search(output)
...
class TestNestedSchemaValidators:
    ...
    def test_nested_schemas_good_values(self):
        values = dict(age='21', number='22', sub=dict(age='27', number='28', sub2=dict(age='33', number='34')))
        (values, errors) = catch_validation_errors(self.form, values)
        assert not errors
