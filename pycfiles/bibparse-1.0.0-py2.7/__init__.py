...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bibparse/__init__.py
# Compiled at: 2019-07-18 09:21:01
...
import re
version = '1.0.0'
bibkeys = ('key', 'author', 'title', 'subtitle', 'origtitle', 'translator', 'crossref',
           'editor', 'booktitle', 'booksubtitle', 'origbooktitle', 'chapter', 'journal',
           'series', 'volume', 'number', 'edition', 'organization', 'institution',
           'school', 'publisher', 'address', 'pubstate', 'howpublished', 'url', 'urldate',
           'doi', 'month', 'year', 'pubyear', 'pages', 'isbn', 'issn', 'annote',
           'eprint', 'type', 'note', 'keywords')
...
class BibItem(dict):
    ...
    def __repr__(self):
        ...
        if self.bibtype == 'preamble':
            ...
            buff = [('@{}{{{}').format(self.bibtype, self.bibid)] + [ ('    {} = {{{}}}').format(key, to_bibtex(key, self[key])) for key in bibkeys if key in self ] + [ ('    {} = {{{}}}').format(key, val) for key, val in self.items() if key not in bibkeys
                                                                                                                                                                       ]
            ret = (',\n').join(buff) + '\n}'
        return ret
    ...
    def update(self, fields, overwrite=True):
        ...
        if not isinstance(fields, dict):
            raise TypeError
        item = fields if overwrite else {k:v for k, v in fields.items() if k.lower() not in self}
            ...
            if state == 'skip':
                ...
                if c == '}' and itemdata.count('{') == itemdata.count('}'):
                    if itemtype.lower() == 'preamble':
                        ...
                        self['@preamble'] = BibItem(bibtype='preamble', data=itemdata)
                    else:
    ...
    def write(self, filename=None, unordered=False):
        ...
        with open(filename, 'w') as (f):
            if unordered:
                ...
                f.write(repr(sorted(self.by_type(['preamble', 'collection'], complement=True))))
