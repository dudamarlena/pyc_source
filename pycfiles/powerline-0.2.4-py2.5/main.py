...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/powerline/main.py
# Compiled at: 2008-04-11 18:06:44
import cherrypy
...
class root(web.controller):
    ...
    def index(self):
        ...
        yield (
         'reservation_entry_prefill', settings['reservation_entry_prefill'])
    ...
    def sessions(self):
        ...
        for (i, result) in enumerate(results):
            ...
            if result.line_position != 0:
                result.line_position = output.ordinalize(result.line_position)
            result.timestamp = time.mktime(result.timestamp.timetuple())
            if result.end_time:
                result.end_time = time.mktime(result.end_time.timetuple())
            if result.start_time:
                result.start_time = time.mktime(result.start_time.timetuple())
            del result.user
