...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/powerline/manager.py
# Compiled at: 2008-04-11 16:54:10
import cherrypy
...
class hours_controller(web.controller):
    ...
    def modify(self, **kwargs):
        ...
        for i in xrange(7):
            ...
            if sent_properties != {}:
                current_hours[i] = (
                 datetime.strptime(sent_properties['opentime'], '%I:%M %p').time(), datetime.strptime(sent_properties['closetime'], '%I:%M %p').time())
...
class interface(web.controller):
    ...
    def login_setup(self):
        ...
        settings = settings_model(database)
        yield (
         'username', settings['manager_users'][0].split(':')[0])
    ...
    def statistics(self):
        ...
        session = session_model(database)
        user = user_model(database)
        return {'result': {'sessions_for_day': session.rows('date(timestamp) = current_date').select_value('count(end_time)'), 
