...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\vcode_api\vcode.py
# Compiled at: 2018-03-13 03:43:11
...
def generate(number=4, bgSize=None, font=None, colorAll=None, pool=None, drawLine=True, foggy=True, saveDirPath=None, showImg=True, returnText=True):
    """
    :param number: int,
    :param bgSize: tuple,backgroundSize(width,height). The `height` affects `fontSize` & `lineWidth`.
    :param font: str,Default font 'arial'. Windows :file:`C:\x0contPath\x07rial.ttf` directory.
    ...
    if drawLine:
        lineInd = [
         (
          textInd[0], randint(int(textInd[1] + 1), bgSize[1] - int(textInd[1] - 1))),
         (
