...
# Embedded file name: build/bdist.linux-x86_64/egg/pyfva/clientes/validator.py
# Compiled at: 2017-07-25 00:08:00
# Size of source mod 2**32: 5043 bytes
__doc__ = '\nCreated on 19 jul. 2017\n\n@author: luis\n'
import warnings
from pyfva.soap.validador_certificado import ValideElServicio as ValideServicioCertificado, ValidadorDeCertificadoSoapServiceStub, SoliciteLaValidacionDeCetificadoDeAutenticacion
from pyfva.soap.validador_documento import ValideElServicio as ValideServicioDocumento, ValidadorDeDocumentoSoapServiceStub, ValideElDocumentoXmlEnvelopedCoFirma
from pyfva.soap import settings

class ClienteValidador(object):
    DEFAULT_CERTIFICATE_ERROR = {'cod_error':2, 
     'exitosa':False, 
     'certificado':None}
    DEFAULT_DOCUMENT_ERROR = {'exitosa':False, 
     'advertencias':None, 
    ...
    def _validar_documento_xml(self, documento):
        ...
        try:
            ...
            dev = self._extract_documento_xml(status.soap_body.ValideElDocumentoXmlEnvelopedCoFirmaResult)
    ...
    def _extract_certificado_autenticacion(self, result):
        dev = {'cod_error':result.CodigoDeError, 
         'exitosa':result.FueExitosa, 
         'certificado':None}
        if result.FueExitosa:
            cert = result.InformacionDelCertificado
            dev['certificado'] = {'identificacion':cert.Identificacion or 'N/D', 
