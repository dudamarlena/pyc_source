...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pytest_gui_status\status_gui\gui_backend.py
# Compiled at: 2016-01-13 15:25:21
import htmlPy
from .utils_gui import render_template
import os, redis, dateutil.parser
from datetime import datetime
import humanfriendly, os.path
from ..utils import s
from ..utils import REDIS_PORT

class Controller(htmlPy.Object):
    dict_state_desc = {'start': 'Starting Tests', 
    ...
    def redraw(self):
        ...
        try:
            ...
            self.app_gui.stop()
            raise redis.exceptions.ConnectionError('Cant connect to redis, something else seems to be running on this socket. Is redis running?\nStopping pytest_status_gui')
        ...
        if hash_dir_name is None:
            self.app_gui.stop()
            raise redis.exceptions.ConnectionError(('dir_name = {dir_name} not found in redis db').format(dir_name=dir_name))
        dict_state = {}
        ...
        last_updated_rel = (datetime.now() - dict_state['last_updated_obj']).seconds
