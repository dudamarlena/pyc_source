...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/oebakery/sync.py
...
def do_mirror_put(argv):
    ...
    parser.add_option('-c', '--check-md5', action='store_true', dest='check_md5', default=False, help='verify MD5 sums')
    parser.add_option('-i', '--interactive', action='store_true', dest='interactive', default=False, help='interactive mode')
    parser.add_option('-n', '--dry-run', action='store_true', dest='dry_run', default=False, help='perform a trial run with no changes made')
    ...
    call('scp%s %s:%s' % (local_files_str,
     ...
     config.get('mirror', 'put_path')), dry_run=options.dry_run)
    ...
    for file in local_files:
        ...
        if not os.path.exists(md5_file):
            ...
            if options.interactive:
                while True:
                    answer = raw_input('(s)kip file, (r)emove file, (g)enerate md5: ')
                    if answer == 's':
                        break
                    elif answer == 'r':
                        os.remove(file)
    ...
    if options.check_md5:
        ...
        for file in local_files:
            ...
            if local_md5_gen == local_md5:
                ...
                if options.interactive:
                    while True:
                        answer = raw_input('(s)kip file, (r)emove file, (g)enerate new md5 file: ')
                        if answer == 's':
                            break
                        elif answer == 'r':
                            os.remove(file)
...
def ls_mirror_files(config):
    pipe = subprocess.Popen([
     ...
     'ls', config.get('mirror', 'put_path')], stdout=subprocess.PIPE)
    mirror_files = []
...
def mirror_files_filter(config, options, local_files):
    ...
    (mirror_files, local_files) = filter_mirror_files_md5_sanity(mirror_files, local_files, options)
    if options.check_md5:
        local_files = filter_compare_md5(local_files, mirror_files, config, options)
    else:
...
def filter_mirror_files_md5_sanity(mirror_files, local_files, options):
    ...
    filtered_local_files = []
    for file in local_files:
        if file in mirror_files and file + '.md5' not in mirror_files:
            print '\nWARNING: mirror missing MD5 sum for', file
            if options.interactive:
                while True:
                    answer = raw_input('(s)kip file, (o)verwrite mirror copy: ')
                    if answer == 's':
                        break
                    elif answer == 'o':
                        filtered_local_files.append(file)
    ...
    return (
     filtered_mirror_files, filtered_local_files)
...
def filter_compare_md5(local_files, mirror_files, config, options):
    ...
    for file in local_files:
        md5_file = file + '.md5'
        if md5_file not in mirror_files:
            filtered_local_files.append(file)
        else:
            mirror_md5 = subprocess.Popen([
             'ssh', config.get('mirror', 'put_hostname'),
             'cat',
             os.path.join(config.get('mirror', 'put_path'), md5_file)], stdout=subprocess.PIPE).communicate()[0]
            with open(md5_file, 'r') as (local_md5_file):
                local_md5 = local_md5_file.readline()
            if local_md5 == mirror_md5:
                ...
                if options.interactive:
                    while True:
                        answer = raw_input('(s)kip file, (o)verwrite mirror copy, (g)et mirror copy: ')
                        if answer == 's':
                            break
                        elif answer == 'o':
                            filtered_local_files.append(file)
