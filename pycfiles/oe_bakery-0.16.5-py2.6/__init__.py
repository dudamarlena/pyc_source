...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/oebakery/__init__.py
# Compiled at: 2010-01-25 09:03:53
...
def set_topdir(dir):
    global TOPDIR
    if not (os.path.exists(os.path.join(dir, 'bakery.ini')) or os.path.exists(os.path.join('.bakery'))):
...
def locate_topdir():
    global TOPDIR
    if os.path.exists('%s/bakery.ini' % os.getcwd()) or os.path.exists('%s/.bakery' % os.getcwd()):
        TOPDIR = os.getcwd()
    else:
...
def call(cmd, dir=None, quiet=False, success_returncode=0):
    ...
    retval = None
    if quiet:
        process = subprocess.Popen(cmd, shell=True, stdin=sys.stdin, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
