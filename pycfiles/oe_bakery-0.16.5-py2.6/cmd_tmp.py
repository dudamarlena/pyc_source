...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/oebakery/cmd_tmp.py
# Compiled at: 2009-11-16 07:06:54
...
class TmpCommand:
    ...
    def __init__(self, config, argv=[]):
        ...
        parser.add_option('-c', '--create', action='store_true', dest='create', default=False, help='create new tmp area')
        parser.add_option('-d', '--delete', action='store_true', dest='delete', default=False, help='delete tmp area')
        (self.options, self.args) = parser.parse_args(argv)
        if self.options.create and self.options.delete or self.options.create and len(self.args) != 1 or self.options.delete and len(self.args) < 1:
        ...
        if self.config.has_option('tmp', 'basedir'):
            self.basedir = os.path.normpath(os.path.join(oebakery.get_topdir(), self.config.get('tmp', 'basedir')))
        else:
            self.basedir = None
        self.tmpdir = os.path.normpath(os.path.join(oebakery.get_topdir(), self.config.get('tmp', 'tmpdir')))
        return
    ...
    def show_tmp_areas(self):
        ...
        for areadir in sorted(os.listdir(self.basedir)):
            real_areadir = os.path.realpath(os.path.join(self.basedir, areadir))
