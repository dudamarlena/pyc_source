...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mercury/backend/active_asyncio.py
# Compiled at: 2018-11-30 15:05:10
# Size of source mod 2**32: 4735 bytes
import asyncio, logging, time, msgpack, zmq, zmq.asyncio
...
def add_active_record(record):
    ...
    active_state.update({record['mercury_id']: {'mercury_id':record['mercury_id'], 
...
async def ping(record, ctx, timeout, retries, backoff, inventory_client, rpc_client):
    ...
    """
    failures = 0
    zurl = 'tcp://{}:{}'.format(record['rpc_address6'] or record['rpc_address'], record['ping_port'])
    while failures < retries:
        ...
        if await socket.poll(current_timeout):
            ...
            return
        log.debug('ping timeout failures/backoff: {}/{}  : {} : {}'.format(failures, backoff, record['mercury_id'], zurl))
        failures += 1
        socket.close()

    log.info('{} timed out, removing active record'.format(record['mercury_id']))
    await inventory_client.update_one(record['mercury_id'], {'active': None})
    ...
    if task_count > 0:
        ...
        for task in tasks:
            await rpc_client.complete_task({'job_id':task['job_id'], 
             'task_id':task['task_id'], 
             'status':'ERROR', 
             'message':'AAWOL - Agent went AWOL while handling task.'})


async def ping_loop(ctx, ping_interval, cycle_time, initial_ping_timeout, ping_retries, backoff, loop, inventory_router_url, rpc_client):
    """
    ...
    while True:
        ...
        for mercury_id, data in list(active_state.items()):
            if now - data['last_ping'] > ping_interval:
                data['pinging'] or log.debug('Scheduling ping for {}'.format(mercury_id))
                active_state[mercury_id]['pinging'] = True
                asyncio.ensure_future((ping(data, ctx, initial_ping_timeout, ping_retries, backoff, inventory_client, rpc_client)),
                  loop=loop)
