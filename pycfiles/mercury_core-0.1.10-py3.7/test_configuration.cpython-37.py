...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/common/unit/test_configuration.py
# Compiled at: 2019-02-07 12:47:28
# Size of source mod 2**32: 8970 bytes
"""Module to unit test mercury.common.configuration."""
import os, mock, pytest, sys, six
...
class MercuryConfigurationUnitTests(MercuryCommonUnitTest):
    __doc__ = 'Unit tests for mercury.common.configuration.'
    ...
    def test_find_config(self, isfile_mock):
        ...
        isfile_return_values = [
         False] * len(config.DEFAULT_SEARCH_DIRS)
    ...
    def test_mercury_configuration_init():
        ...
        sys.argv = [
         'test']
        mc.add_option('test', '--test', 'TESTVAR', 'test', default='HELLO')
    ...
    def test_mercury_configuration_scan_options():
        ...
        with mock.patch(patch_path) as (mock_load):
            mock_load.return_value = {'test_server':{'host':'test.com', 
              'port':31137}, 
             'nest1':{'nest2': {'nest3': 'the cake is a lie'}}}
            new_mc = config.MercuryConfiguration('test', 'test.yaml')
            new_mc.add_option('host', '--host', 'HOST',
              'test_server.host',
              default='localhost')
    ...
    def test_mercury_configuration_type_conversions():
        sys.argv = [
         'test']
        new_mc = config.MercuryConfiguration('test', 'test.yaml')
        new_mc.add_option('array', '--array', 'ARRAY',
          'none.none',
          special_type=list,
          default=[
         1, 2, 3])
        ...
        new_mc.add_option('bool', '--bool', 'BOOL',
          'none.none',
          special_type=bool,
          default=True)
        namespace = new_mc.scan_options()
        ...
        new_mc.add_option('int', '--int', 'INT', 'none.none', special_type=int)
        new_mc.add_option('float', '--float', 'FLOAT', 'none.none', special_type=float)
    ...
    def test_mercury_configuration_auto_format():
        ...
        with mock.patch(patch_path) as (mock_load):
            ...
            os.environ['NEST1_NEST2_NEST3'] = 'Is it ok to drink beer while writing tests?'
            namespace = new_mc.scan_options()
            assert namespace.nest1.nest2.nest3 == 'Is it ok to drink beer while writing tests?'
    ...
    def test_proc_cmdline():
        ...
        option = {'name':'test.option',  'proc_cmdline_argument':'TEST_OPTION', 
         'special_type':None}
