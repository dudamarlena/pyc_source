...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mercury/rpc/jobs/jobs.py
# Compiled at: 2018-01-29 12:10:54
# Size of source mod 2**32: 5006 bytes
...
class Job(object):
    ...
    def __init__(self, instruction, targets, jobs_collection, tasks_collection, job_id=None):
        """
    ...
    def create_task(self, target):
        try:
            ...
            backend = format_zurl(target['origin']['address'], target['origin']['port'])
        except KeyError:
            raise MercuryCritical('Encountered malformed target, the database is corrupted')
        ...
        if method:
            self.primitive = True
            call_data = {'method':self.instruction['method'], 
    ...
    async def insert(self):
        ...
        await self.tasks_collection.insert_many([task.to_dict() for task in list(self.tasks.values())])

    def enqueue_tasks(self):
        """ Uses backend manager processes to enqueue tasks """
        backend_index = util.build_index_l([task.to_dict() for task in self.tasks.values()], 'backend')
