...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mercury/common/helpers/cli.py
...
class CLIResult(str):
    ...
    def __new__(cls, *args, **kwargs):
        return str.__new__(cls, object=(cls._CLIResult__decode(args[0])))
    ...
    def __str__(self):
        return self._CLIResult__decode(self._stdout)
    ...
    @property
    def stderr(self):
        return self._CLIResult__decode(self._stderr)
...
class CLIException(Exception):
    __doc__ = 'Exception used by cli.run function.'


def run(command, bufsize=1048567, dry_run=False, raise_exception=False, ignore_error=False, quiet=False, env=None, _input=None, raw=False):
    ...
    if not quiet:
        ...
        if not dry_run:
            ...
            out, err, ret = ('', '', 0)
        quiet or log.debug('Return Code: {}'.format(ret))
        if out:
            log.debug('stdout: \n{}'.format(out.strip()))
        if ret:
            if not ignore_error:
