...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mercury/rpc/backend_clients.py
...
def set_queue_client_opts(linger, response_timeout):
    ...
    receive data.
    """
    que_client_opts.update({'linger':linger, 
...
class BackendQueueManager(object):
    ...
    def queue_runner(self):
        ...
        while not self._kill:
            ...
            log.debug(('Dispatching task to queue service, job_id: {job_id} task_id: {task_id} backend: {backend}'.format)(**task))
