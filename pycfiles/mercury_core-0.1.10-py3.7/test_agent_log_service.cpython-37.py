...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/log_service/unit/test_agent_log_service.py
# Compiled at: 2018-01-29 12:10:54
# Size of source mod 2**32: 2100 bytes
import mock, pytest, unittest
from mercury.common.exceptions import MercuryClientException
import mercury.log_service as log_server
...
class AgentLogServiceTestCase(unittest.TestCase):
    ...
    def setUp(self):
        with mock.patch('mercury.log_service.service.SimpleRouterReqService.__init__',
          new=mock_init):
            log_collection = mock.Mock()
            self.agent_log_service = log_server.AgentLogService('invalid_bind_url', log_collection)

    def test_process(self):
        with pytest.raises(MercuryClientException):
            self.agent_log_service.process({})
        message = {'name':'mercury.agent.agent', 
