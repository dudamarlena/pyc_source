...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/behave/api/features/client.py
# Compiled at: 2018-11-30 15:02:35
# Size of source mod 2**32: 5754 bytes
import copy, json, requests, datetime
...
class APIClient(object):
    ...
    def __init__(self, base_url, request_kwargs=None, client_kwargs=None):
        ...
        if request_kwargs:
            ...
            ssl_certificate_verify = request_kwargs.get('ssl_certificate_verify', ssl_certificate_verify)
    ...
    def get(self, resource_id=None, params=None, url=None, url_suffix=None, headers=None):
        ...
        if self.verbose:
            ...
            s = '{}-{}-{} {}:{}:{}'.format(d.year, d.month, d.day, d.hour, d.minute, d.second)
            print('{0}GET REQUEST{1}'.format('********************', '************************'))
    ...
    def delete(self, url_suffix=None, headers=None, resource_id=None):
        ...
        if self.verbose:
            d = datetime.datetime.now()
            s = '{}-{}-{} {}:{}:{}'.format(d.year, d.month, d.day, d.hour, d.minute, d.second)
            print('{0}DELETE REQUEST{1}'.format('********************', '************************'))
            print('{0}{1}{2}'.format('******************', s, '*********************'))
            print(request_kwargs)
            print('{0}RESPONSE{1}'.format('********************', '***************************'))
            print(resp.content)
    ...
    def get_identity_token(self):
        ...
        internal_identity_username = self.cfg.INTERNAL_IDENTITY.internal_identity_username
        internal_identity_password = self.cfg.INTERNAL_IDENTITY.internal_identity_password
        ...
        identity_data = {'auth': {'passwordCredentials': {'username':internal_identity_username, 
                                          'password':internal_identity_password}, 
                  
                  domain: {'name': domain_name}}}
        token_resp = requests.post(internal_idenity_url,
