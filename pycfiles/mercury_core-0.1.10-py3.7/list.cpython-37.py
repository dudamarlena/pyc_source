...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/behave/api/features/steps/list.py
...
def step_i_get_the_offset_list_of_service_with_params_in_filename(context, service_name, filename):
    ...
    service_client = context.services[service_name]['client']
    context.services[service_name]['offset_resp'] = service_client.get(url_suffix=suffix)
...
@step('the response contains an offset list of {service_name} that have been offset by the offset_id')
def step_the_response_contains_a_list_of_service_ids(context, service_name):
    """
    :type context: behave.runner.Context
    :type service_name: str
...
def step_the_entity_is_exists_in_the_service_list(context, existence, service_name):
    """
    :type context: behave.runner.Context
    ...
    if existence == 'not found':
        ...
        if existence == 'found':
            ...
            raise Exception("{} is not a valid existence name!  Please use 'found' or 'not found'")