...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mercury/rpc/jobs/tasks.py
# Compiled at: 2018-01-22 15:39:21
# Size of source mod 2**32: 5709 bytes
import datetime, logging, time, uuid
log = logging.getLogger(__name__)
COMPLETED_STATUSES = [
 'SUCCESS', 'ERROR', 'EXCEPTION', 'TIMEOUT']
...
async def update_task(update_data, tasks_collection):
    ...
    log.info('Task updated: task_id: {}, status: {}, action: {}, progress: {}'.format(task_id, update_data.get('status', 'UPDATED'), update_data.get('action', ''), update_data.get('progress', 0), update_data.get('traceback')))
    await tasks_collection.update_one(filter={'task_id': task_id},
      update={'$set': task_update})
...
async def complete_task(response_data, jobs_collection, tasks_collection):
    ...
    log.info(('Task completed: task_id: {task_id} job: {job_id} message: {message}'.format)(**response_data))
    job_id = response_data.pop('job_id')
    task_update = {'task_id':response_data['task_id'], 
    ...
    if await is_completed(tasks_collection, job_id):
        ...
        await jobs_collection.update_one({'job_id': job_id}, {'$set': {'time_completed':now, 
                  'ttl_time_completed':ttl_time}})
...
async def is_completed(tasks_collection, job_id):
    ...
    :return:
    """
    return await tasks_collection.count({'job_id':job_id, 
     'status':{'$nin': COMPLETED_STATUSES}}) == 0
...
class Task(object):
    ...
    def __init__(self, job_id, mercury_id, host, port, method, backend, args=None, kwargs=None):
        """
    ...
    def __repr__(self):
        return ('Task: {task_id} <{host}:{port}> [{backend}][method: {method} args: {args}, kwargs: {kwargs}]'.format)(**self.to_dict())
