...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mercury/common/transport.py
...
class SimpleRouterReqClient(object):
    ...
    def raise_reply_error(self, reply):
        ...
        """
        raise MercuryClientException('Problem talking to {} backend: message = {}, tb = {}'.format(self.service_name, reply.get('message'), '\n'.join(reply.get('tb', []))))
...
def parse_multipart_message(message):
    ...
    return {'address':address_segment, 
...
class SimpleRouterReqService(object):
    __doc__ = 'Base class for a message router backend.'
    ...
    def receive(self):
        ...
        try:
            ...
            try:
                ...
                self.send_error(parsed_message['address'], 'Client error, message is not packed')
                raise MercuryClientException('Message is malformed')
            finally:
                type_error = None
                del type_error
         ...
         msgpack.ExtraData) as msgpack_exception:
            try:
                ...
                self.send_error(parsed_message['address'], 'Client error, message is malformed')
                raise MercuryClientException('Message is malformed')
            finally:
                msgpack_exception = None
                del msgpack_exception

        return (
         parsed_message['address'], message)
    ...
    def message_handler(self, address, message):
        if isinstance(message, dict) and message.get('_protocol_message') == 'keep_alive':
            ...
            try:
                ...
                try:
                    return self.send_error(address, 'Encountered client error: {}'.format(mce))
                finally:
                    mce = None
                    del mce
                ...
                return self.send_error(address, 'Encountered server error, sorry')
