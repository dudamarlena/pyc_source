...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/idapload/test/test_runners.py
# Compiled at: 2020-04-13 02:37:12
# Size of source mod 2**32: 39578 bytes
...
from locust.runners import LocustRunner, LocalLocustRunner, MasterLocustRunner, SlaveNode, SlaveLocustRunner, STATE_INIT, STATE_HATCHING, STATE_RUNNING, STATE_MISSING
...
class TestLocustRunner(LocustTestCase):
    ...
    def assert_locust_class_distribution(self, expected_distribution, classes):
        ...
        for locust_class in classes:
            if locust_class not in distribution:
        ...
        self.assertEqual(expected_distribution, distribution, 'Expected a locust class distribution of %s but found %s' % (
         expected_str,
         actual_str))
    ...
    def test_reset_stats(self):
        ...
        class User(Locust):
            ...
            class task_set(TaskSet):
                ...
                @task
                def my_task(self):
                    self.locust.environment.events.request_success.fire(request_type='GET',
                      name='/test',
                      response_time=666,
...
class TestMasterRunner(LocustTestCase):
    ...
    def test_master_total_stats(self):
        with mock.patch('locust.rpc.rpc.Server', mocked_rpc()) as (server):
            ...
            server.mocked_send(Message('stats', {'stats':stats.serialize_stats(), 
             'stats_total':stats.total.serialize(), 
             'errors':stats.serialize_errors(), 
             'user_count':1}, 'fake_client'))
            server.mocked_send(Message('stats', {'stats':stats2.serialize_stats(), 
             'stats_total':stats2.total.serialize(), 
             'errors':stats2.serialize_errors(), 
             'user_count':2}, 'fake_client'))
    ...
    def test_master_total_stats_with_none_response_times(self):
        with mock.patch('locust.rpc.rpc.Server', mocked_rpc()) as (server):
            ...
            server.mocked_send(Message('stats', {'stats':stats2.serialize_stats(), 
             ...
             'errors':stats2.serialize_errors(), 
             'user_count':2}, 'fake_client'))
            server.mocked_send(Message('stats', {'stats':stats3.serialize_stats(), 
...
class TestSlaveLocustRunner(LocustTestCase):
    ...
    def test_slave_stop_timeout(self):
        ...
        with mock.patch('locust.rpc.rpc.Client', mocked_rpc()) as (client):
            ...
            client.mocked_send(Message('hatch', {'hatch_rate':1, 
             'num_clients':1, 
             'host':'', 
             'stop_timeout':1}, 'dummy_client_id'))
    ...
    def test_slave_without_stop_timeout(self):
        ...
        with mock.patch('locust.rpc.rpc.Client', mocked_rpc()) as (client):
            ...
            client.mocked_send(Message('hatch', {'hatch_rate':1, 
             'num_clients':1, 
             'host':'', 
             'stop_timeout':None}, 'dummy_client_id'))
            self.assertIn('hatching', [m.type for m in client.outbox])
            slave.hatching_greenlet.join()
            self.assertEqual(1, len(slave.locusts))
            gevent.sleep(0.01)
    ...
    def test_change_user_count_during_hatching(self):
        ...
        with mock.patch('locust.rpc.rpc.Client', mocked_rpc()) as (client):
            ...
            client.mocked_send(Message('hatch', {'hatch_rate':5, 
             'num_clients':10, 
             'host':'', 
             'stop_timeout':None}, 'dummy_client_id'))
            sleep(0.6)
    ...
    def test_stop_timeout(self):
        ...
        class MyTaskSet(TaskSet):
            ...
            def my_task(self):
                ...
                MyTaskSet.state = 'second'
                gevent.sleep(short_time)
                MyTaskSet.state = 'third'
        ...
        environment.stop_timeout = short_time / 2
        runner = LocalLocustRunner(environment, [MyTestLocust])
        runner.start(1, 1)
        gevent.sleep(short_time)
        runner.quit()
        self.assertEqual('second', MyTaskSet.state)
        environment.stop_timeout = short_time * 3
        runner = LocalLocustRunner(environment, [MyTestLocust])
        runner.start(1, 1)
        gevent.sleep(short_time)
