...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/idapload/test/test_web.py
...
class TestWebUI(LocustTestCase):
    ...
    def test_stats(self):
        ...
        self.assertEqual(2, len(data['stats']))
    ...
    def test_stats_cache(self):
        ...
        self.web_ui.app.view_functions['request_stats'].clear_cache()
        data = json.loads(requests.get('http://127.0.0.1:%i/stats/requests' % self.web_port).text)
        self.assertEqual(3, len(data['stats']))
    ...
    def test_swarm_host_value_specified(self):
        ...
        response = requests.post(('http://127.0.0.1:%i/swarm' % self.web_port),
          data={'locust_count':5, 
         'hatch_rate':5,  'host':'https://localhost'})
        self.assertEqual(200, response.status_code)
    ...
    def test_swarm_in_step_load_mode(self):
        ...
        response = requests.post(('http://127.0.0.1:%i/swarm' % self.web_port),
          data={'locust_count':5, 
         'hatch_rate':2,  'step_locust_count':2,  'step_duration':'2m'})
        self.assertEqual(200, response.status_code)
