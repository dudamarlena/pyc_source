...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/idapload/argument_parser.py
# Compiled at: 2020-04-15 05:32:05
# Size of source mod 2**32: 11381 bytes
import os, sys, configargparse, idapload
version = idapload.__version__
DEFAULT_CONFIG_FILES = [
 '~/.idapload.conf', 'idapload.conf']
...
def find_idaploadfile(idaploadfile):
    """
    Attempt to locate a idaploadfile, either explicitly or by searching parent dirs.
    """
    names = [
     idaploadfile]
    ...
    if os.path.dirname(names[0]):
        for name in names:
            ...
            if not os.path.exists(expanded) or name.endswith('.py') or _is_package(expanded):
        ...
        while True:
            for name in names:
                ...
                if not os.path.exists(joined) or name.endswith('.py') or _is_package(joined):
...
def get_empty_argument_parser(add_help=True, default_config_files=DEFAULT_CONFIG_FILES):
    ...
    parser.add_argument('-f',
      '--idaploadfile', default='idaploadfile',
      help="Python module file to import, e.g. '../other.py'. Default: idaploadfile")
    return parser


def parse_idaploadfile_option(args=None):
    """
    ...
    idaploadfile = find_idaploadfile(options.idaploadfile)
    if not idaploadfile:
        if options.help:
            parse_options(args=args)
        sys.stderr.write("Could not find any idaploadfile! Ensure file ends in '.py' and see --help for available options.\n")
        sys.exit(1)
    if idaploadfile == 'idapload.py':
        sys.stderr.write('The idaploadfile must not be named `idapload.py`. Please rename the file and try again.\n')
        sys.exit(1)
    return idaploadfile
...
def setup_parser_arguments(parser):
    ...
    """
    parser.add_argument('-H',
      '--host', help='Host to load test in the following format: http://10.21.32.33')
    ...
    parser.add_argument('--csv',
      '--csv-base-name', dest='csvfilebase',
      help='Store current request stats to files in CSV format.')
    parser.add_argument('--csv-full-history',
      action='store_true',
      default=False,
    ...
    parser.add_argument('--master',
      action='store_true',
      help='Set idapload to run in distributed mode with this process as master')
    parser.add_argument('--slave',
      action='store_true',
      help='Set idapload to run in distributed mode with this process as slave')
    parser.add_argument('--master-host',
      default='127.0.0.1',
      help='Host or IP address of idapload master for distributed load testing. Only used when running with --slave. Defaults to 127.0.0.1.')
    parser.add_argument('--master-port',
      type=int,
      default=5557,
      help='The port to connect to that is used by the idapload master for distributed load testing. Only used when running with --slave. Defaults to 5557.')
    parser.add_argument('--master-bind-host',
      default='*',
      help='Interfaces (hostname, ip) that idapload master should bind to. Only used when running with --master. Defaults to * (all available interfaces).')
    parser.add_argument('--master-bind-port',
      type=int,
      default=5557,
      help='Port that idapload master should bind to. Only used when running with --master. Defaults to 5557.')
    parser.add_argument('--expect-slaves',
      type=int,
      default=1,
    ...
    parser.add_argument('--only-summary',
      action='store_true',
      help='Only print the summary stats')
    parser.add_argument('--no-reset-stats',
      action='store_true',
    ...
    parser.add_argument('-l',
      ...
      dest='list_commands',
      help='Show list of possible idapload classes and exit')
    parser.add_argument('--show-task-ratio',
      action='store_true',
      help="print table of the idapload classes' task execution ratio")
    parser.add_argument('--show-task-ratio-json',
      action='store_true',
      help="print json data of the idapload classes' task execution ratio")
    parser.add_argument('-V',
      '--version', action='version',
      version=('%(prog)s {}'.format(version)))
    parser.add_argument('--exit-code-on-error',
      type=int,
      default=1,
      help='sets the exit code to post on error')
    parser.add_argument('-s',
      '--stop-timeout', action='store',
      type=int,
      dest='stop_timeout',
      default=None,
    ...
    parser.add_argument('idapload_classes',
      nargs='*',
      metavar='LocustClass')
...
def get_parser(default_config_files=DEFAULT_CONFIG_FILES):
    ...
    idapload.events.init_command_line_parser.fire(parser=parser)
    return parser
