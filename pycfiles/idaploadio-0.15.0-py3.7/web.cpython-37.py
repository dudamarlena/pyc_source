...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/idapload/web.py
# Compiled at: 2020-04-15 05:34:55
# Size of source mod 2**32: 9695 bytes
import csv, json, logging, os.path
...
from idapload import __version__ as version
...
class WebUI:
    ...
    def __init__(self, environment):
        ...
        def index():
            if not environment.runner:
                ...
                if environment.host:
                    ...
                    if environment.runner.idapload_classes:
                        all_hosts = set([l.host for l in environment.runner.idapload_classes])
                        if len(all_hosts) == 1:
                        ...
                        host = None
            return render_template('index.html', state=(environment.runner.state),
        ...
        def swarm():
            ...
            idapload_count = int(request.form['idapload_count'])
            hatch_rate = float(request.form['hatch_rate'])
            if request.form.get('host'):
                environment.host = str(request.form['host'])
            if environment.step_load:
                step_idapload_count = int(request.form['step_idapload_count'])
                step_duration = parse_timespan(str(request.form['step_duration']))
                environment.runner.start_stepload(idapload_count, hatch_rate, step_idapload_count, step_duration)
                return jsonify({'success':True,  'message':'Swarming started in Step Load Mode',  'host':environment.host})
            environment.runner.start(idapload_count, hatch_rate)
        ...
        def exceptions():
            return jsonify({'exceptions': [{'count':row['count'],  'msg':row['msg'],  'traceback':row['traceback'],  'nodes':', '.join(row['nodes'])} for row in environment.runner.exceptions.values()]})
