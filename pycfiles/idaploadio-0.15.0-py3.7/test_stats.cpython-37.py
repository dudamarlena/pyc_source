...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/idapload/test/test_stats.py
# Compiled at: 2020-04-13 02:37:12
# Size of source mod 2**32: 22050 bytes
import time, unittest, re, os, gevent, locust
...
class TestRequestStats(unittest.TestCase):
    ...
    def test_total_rps(self):
        ...
        self.assertEqual(self.s.total_rps, 1.8)
        self.assertAlmostEqual(s2.total_rps, 0.2)
        self.assertEqual(self.stats.total.total_rps, 2.0)
    ...
    def test_aggregation_with_decimal_rounding(self):
        ...
        self.assertEqual(s1.avg_response_time, 2.063333333333333)
...
class TestStatsEntryResponseTimesCache(unittest.TestCase):
    ...
    def test_response_times_cached(self):
        ...
        self.assertEqual(CachedResponseTimes(response_times={11: 1},
          num_requests=1), s.response_times_cache[(int(s.last_request_timestamp) - 1)])
    ...
    def test_latest_total_response_times_pruned(self):
        ...
        self.assertEqual(CachedResponseTimes(response_times={17: 1}, num_requests=1), s.response_times_cache.popitem(last=True)[1])

    def test_get_current_response_time_percentile(self):
        s = StatsEntry((self.stats), '/', 'GET', use_response_times_cache=True)
        t = int(time.time())
        s.response_times_cache[t - 10] = CachedResponseTimes(response_times={i:1 for i in range(100)},
          num_requests=200)
    ...
    def test_diff_response_times_dicts(self):
        self.assertEqual({1:5,  6:8}, diff_response_time_dicts({1:6, 
         6:16,  2:2}, {1:1, 
         6:8,  2:2}))
