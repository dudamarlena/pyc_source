...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/idapload/runners.py
# Compiled at: 2020-04-15 05:34:19
# Size of source mod 2**32: 26690 bytes
import logging, random, socket, traceback, warnings
from uuid import uuid4
from time import time
import gevent, psutil
...
class LocustRunner(object):
    ...
    def __init__(self, environment, idapload_classes):
        environment.runner = self
        self.environment = environment
        self.idapload_classes = idapload_classes
        self.idaploads = Group()
        self.greenlet = Group()
    ...
    @property
    def user_count(self):
        return len(self.idaploads)

    def cpu_log_warning(self):
        """Called at the end of the test to repeat the warning & return the status"""
        if self.cpu_warning_emitted:
            logger.warning('Loadgen CPU usage was too high at some point during the test! See https://docs.idapload.io/en/stable/running-idapload-distributed.html for how to distribute the load over multiple CPU cores or machines')
            return True
        return False

    def weight_idaploads(self, amount):
        """
        Distributes the amount of idaploads for each WebLocust-class according to it's weight
        returns a list "bucket" with the weighted idaploads
        """
        bucket = []
        weight_sum = sum((idapload.weight for idapload in self.idapload_classes if idapload.task_set))
        residuals = {}
        for idapload in self.idapload_classes:
            if not idapload.task_set:
                warnings.warn('Notice: Found Locust class (%s) got no task_set. Skipping...' % idapload.__name__)
                continue
            if self.environment.host is not None:
                idapload.host = self.environment.host
            percent = idapload.weight / float(weight_sum)
            num_idaploads = int(round(amount * percent))
            bucket.extend([idapload for x in range(num_idaploads)])
            residuals[idapload] = amount * percent - round(amount * percent)

        if len(bucket) < amount:
            for idapload in [l for l, r in sorted((residuals.items()), key=(lambda x: x[1]),
              reverse=True)][:amount - len(bucket)]:
                bucket.append(idapload)

        else:
            if len(bucket) > amount:
                for idapload in [l for l, r in sorted((residuals.items()), key=(lambda x: x[1]))][:len(bucket) - amount]:
                    bucket.remove(idapload)

        return bucket

    def spawn_idaploads(self, spawn_count, hatch_rate, wait=False):
        bucket = self.weight_idaploads(spawn_count)
        spawn_count = len(bucket)
        if self.state == STATE_INIT or self.state == STATE_STOPPED:
            self.state = STATE_HATCHING
        existing_count = len(self.idaploads)
        ...
        occurrence_count = dict([(l.__name__, 0) for l in self.idapload_classes])

        def hatch():
            sleep_time = 1.0 / hatch_rate
            while 1:
                if not bucket:
                    logger.info('All idaploads hatched: %s (%i already running)' % (
                     ', '.join(['%s: %d' % (name, count) for name, count in occurrence_count.items()]),
                     existing_count))
                    self.environment.events.hatch_complete.fire(user_count=(len(self.idaploads)))
                    return
                    idapload = bucket.pop(random.randint(0, len(bucket) - 1))
                    occurrence_count[idapload.__name__] += 1
                    new_idapload = idapload(self.environment)

                    def start_idapload(_):
                        try:
                            new_idapload.run(runner=self)
                        except GreenletExit:
                            pass

                    self.idaploads.spawn(start_idapload, new_idapload)
                    if len(self.idaploads) % 10 == 0:
                        logger.debug('%i idaploads hatched' % len(self.idaploads))
                    if bucket:
        ...
        if wait:
            self.idaploads.join()
            logger.info('All idaploads dead\n')

    def kill_idaploads(self, kill_count):
        """
        Kill a kill_count of weighted idaploads from the Group() object in self.idaploads
        """
        bucket = self.weight_idaploads(kill_count)
        kill_count = len(bucket)
        logger.info('Killing %i idaploads' % kill_count)
        dying = []
        for g in self.idaploads:
            for l in bucket:
                if l == type(g.args[0]):
                    dying.append(g)
                    bucket.remove(l)
        ...
        self.kill_idapload_greenlets(dying)
        self.environment.events.hatch_complete.fire(user_count=(self.user_count))

    def kill_idapload_greenlets(self, greenlets):
        """
        Kill running idapload greenlets. If environment.stop_timeout is set, we try to stop the 
        ...
        if self.environment.stop_timeout:
            ...
            for g in greenlets:
                idapload = g.args[0]
                if idapload._state == LOCUST_STATE_WAITING:
                    self.idaploads.killone(g)
                else:
                    idapload._state = LOCUST_STATE_STOPPING
                    dying.add(g)

            if not dying.join(timeout=(self.environment.stop_timeout)):
                logger.info('Not all idaploads finished their tasks & terminated in %s seconds. Killing them...' % self.environment.stop_timeout)
        ...
        while True:
            ...
            if self.current_cpu_usage > 90:
                if not self.cpu_warning_emitted:
                    logging.warning('Loadgen CPU usage above 90%! This may constrain your throughput and may even give inconsistent response time measurements! See https://docs.idapload.io/en/stable/running-idapload-distributed.html for how to distribute the load over multiple CPU cores or machines')
                    self.cpu_warning_emitted = True
            gevent.sleep(CPU_MONITOR_INTERVAL)

    def start(self, idapload_count, hatch_rate, wait=False):
        if self.state != STATE_RUNNING:
            if self.state != STATE_HATCHING:
                ...
                self.environment.events.idapload_start_hatching.fire()
        if self.state != STATE_INIT and self.state != STATE_STOPPED:
            self.state = STATE_HATCHING
            if self.user_count > idapload_count:
                kill_count = self.user_count - idapload_count
                self.kill_idaploads(kill_count)
            elif self.user_count < idapload_count:
                spawn_count = idapload_count - self.user_count
                self.spawn_idaploads(spawn_count=spawn_count, hatch_rate=hatch_rate)
            else:
                self.environment.events.hatch_complete.fire(user_count=(self.user_count))
        else:
            self.hatch_rate = hatch_rate
            self.spawn_idaploads(idapload_count, hatch_rate=hatch_rate, wait=wait)

    def start_stepload(self, idapload_count, hatch_rate, step_idapload_count, step_duration):
        if idapload_count < step_idapload_count:
            logger.error('Invalid parameters: total idapload count of %d is smaller than step idapload count of %d' % (idapload_count, step_idapload_count))
            return
        self.total_clients = idapload_count
        ...
        logger.info('Start a new swarming in Step Load mode: total idapload count of %d, hatch rate of %d, step idapload count of %d, step duration of %d ' % (idapload_count, hatch_rate, step_idapload_count, step_duration))
        self.state = STATE_INIT
        self.stepload_greenlet = self.greenlet.spawn(self.stepload_worker, hatch_rate, step_idapload_count, step_duration)
        self.stepload_greenlet.link_exception(callback=(self.noop))

    def stepload_worker--- This code section failed: ---

 L. 248         0  LOAD_CONST               0
                2  STORE_FAST               'current_num_clients'

 L. 249         4  SETUP_LOOP          110  'to 110'
                6  LOAD_FAST                'self'
                8  LOAD_ATTR                state
               10  LOAD_GLOBAL              STATE_INIT
               12  COMPARE_OP               ==
               14  POP_JUMP_IF_TRUE     36  'to 36'
               16  LOAD_FAST                'self'
               18  LOAD_ATTR                state
               20  LOAD_GLOBAL              STATE_HATCHING
               22  COMPARE_OP               ==
               24  POP_JUMP_IF_TRUE     36  'to 36'
               26  LOAD_FAST                'self'
               28  LOAD_ATTR                state
               30  LOAD_GLOBAL              STATE_RUNNING
               32  COMPARE_OP               ==
               34  POP_JUMP_IF_FALSE   108  'to 108'
             36_0  COME_FROM            24  '24'
             36_1  COME_FROM            14  '14'

 L. 250        36  LOAD_FAST                'current_num_clients'
               38  LOAD_FAST                'step_clients_growth'
               40  INPLACE_ADD      
               42  STORE_FAST               'current_num_clients'

 L. 251        44  LOAD_FAST                'current_num_clients'
               46  LOAD_GLOBAL              int
               48  LOAD_FAST                'self'
               50  LOAD_ATTR                total_clients
               52  CALL_FUNCTION_1       1  '1 positional argument'
               54  COMPARE_OP               >
               56  POP_JUMP_IF_FALSE    70  'to 70'

 L. 252        58  LOAD_GLOBAL              logger
               60  LOAD_METHOD              info
               62  LOAD_STR                 'Step Load is finished.'
 ...
 L. 254        70  LOAD_FAST                'self'
               72  LOAD_METHOD              start
               74  LOAD_FAST                'current_num_clients'
               76  LOAD_FAST                'hatch_rate'
               78  CALL_METHOD_2         2  '2 positional arguments'
               80  POP_TOP          

 L. 255        82  LOAD_GLOBAL              logger
               84  LOAD_METHOD              info
               86  LOAD_STR                 'Step loading: start hatch job of %d idapload.'
               88  LOAD_FAST                'current_num_clients'
               90  BINARY_MODULO    
               92  CALL_METHOD_1         1  '1 positional argument'
               94  POP_TOP          

 L. 256        96  LOAD_GLOBAL              gevent
               98  LOAD_METHOD              sleep
              100  LOAD_FAST                'step_duration'
              102  CALL_METHOD_1         1  '1 positional argument'
              104  POP_TOP          
              106  JUMP_BACK             6  'to 6'
            108_0  COME_FROM            34  '34'
              108  POP_BLOCK        
            110_0  COME_FROM_LOOP        4  '4'
...
Parse error at or near `POP_BLOCK' instruction at offset 108
    ...
    def stop(self):
        ...
        self.kill_idapload_greenlets([g for g in self.idaploads])
        self.state = STATE_STOPPED
        self.cpu_log_warning()
        self.environment.events.idapload_stop_hatching.fire()
...
class LocalLocustRunner(LocustRunner):
    ...
    def __init__(self, environment, idapload_classes):
        super(LocalLocustRunner, self).__init__(environment, idapload_classes)

        def on_idapload_error(idapload_instance, exception, tb):
            formatted_tb = ''.join(traceback.format_tb(tb))
            self.log_exception('local', str(exception), formatted_tb)

        self.environment.events.idapload_error.add_listener(on_idapload_error)
    ...
    def start(self, idapload_count, hatch_rate, wait=False):
        ...
        if self.hatching_greenlet:
            self.hatching_greenlet.kill(block=True)
        self.hatching_greenlet = self.greenlet.spawn(lambda : super(LocalLocustRunner, self).start(idapload_count, hatch_rate, wait=wait))
...
class MasterLocustRunner(DistributedLocustRunner):
    ...
    def start(self, idapload_count, hatch_rate):
        self.target_user_count = idapload_count
        ...
        if not num_slaves:
            ...
            return
        self.hatch_rate = hatch_rate
        slave_num_clients = idapload_count // (num_slaves or 1)
        slave_hatch_rate = float(hatch_rate) / (num_slaves or 1)
        remaining = idapload_count % num_slaves
        logger.info('Sending hatch jobs of %d idaploads and %.2f hatch rate to %d ready clients' % (slave_num_clients, slave_hatch_rate, num_slaves))
        ...
        for client in self.clients.ready + self.clients.running + self.clients.hatching:
            data = {'hatch_rate':slave_hatch_rate, 
    ...
    def client_listener(self):
        while 1:
            ...
            if msg.type == 'client_stopped':
                ...
                if msg.type == 'heartbeat':
                    if msg.node_id in self.clients:
                        ...
                        if not c.cpu_warning_emitted:
                            if c.cpu_usage > 90:
                                self.slave_cpu_warning_emitted = True
                                c.cpu_warning_emitted = True
                                logger.warning('Slave %s exceeded cpu threshold (will only log this once per slave)' % msg.node_id)
                elif msg.type == 'stats':
                    self.environment.events.slave_report.fire(client_id=(msg.node_id), data=(msg.data))
                else:
            ...
            if self.state == STATE_INIT or all(map(lambda x: x.state != STATE_RUNNING and x.state != STATE_HATCHING, self.clients.all)):
                self.state = STATE_STOPPED
    ...
    def worker(self):
        while 1:
            ...
            if msg.type == 'hatch':
                ...
                if self.hatching_greenlet:
                    self.hatching_greenlet.kill(block=True)
                self.hatching_greenlet = self.greenlet.spawn(lambda : self.start(idapload_count=(job['num_clients']), hatch_rate=(job['hatch_rate'])))
            elif msg.type == 'stop':
                self.stop()
