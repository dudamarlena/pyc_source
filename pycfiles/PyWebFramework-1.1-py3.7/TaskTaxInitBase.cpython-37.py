...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pyWebFramework\tax_init\TaskTaxInitBase.py
# Compiled at: 2020-02-07 01:04:10
# Size of source mod 2**32: 37855 bytes
import os, random, shutil
from datetime import date, datetime, timedelta
import calendar, pdfplumber
...
import pyWebFramework.core.settings as settings
...
class TaskTaxInitBase(TaskBase):
    ...
    def __init__(self):
        ...
        self.tables = []
        self._TaskTaxInitBase__current_table_index = 0
        self.samples = []
        self.collected_data = []
    ...
    def InitTask(self):
        if not super(TaskTaxInitBase, self).InitTask():
            ...
            if not settings.TAX_INIT_SAMPLE_ROOT:
                ...
                return self.LoadSamples(settings.TAX_INIT_SAMPLE_ROOT) or False
            for table_id in self.GetTables():
                table = {'id':table_id,  'name':self.GetTableAttrById(table_id, 'name'), 
    ...
    def LoadSamples(self, sample_root):
        for file in os.listdir(sample_root):
            ...
            self.samples.append({'table_id':table_id, 
             'name':file_sep[0], 
             'sbqq':sbqq, 
             'sbqz':sbqz, 
    ...
    def GetNextTable(self):
        if self._TaskTaxInitBase__current_table_index >= len(self.tables):
            return
        table = self.tables[self._TaskTaxInitBase__current_table_index]
        self._TaskTaxInitBase__current_table_index += 1
        return table
    ...
    def LoadSampleCells(self, sample_file):
        ...
        for sheet_name in workbook.get_sheet_names():
            ...
            sheet_data = {'cells':[],  'labels':[],  'floats':{},  'verifies':[],  'sheet':[],  'combine_sheet':[]}
            for row in sheet.iter_rows():
                for cell in row:
                    if not cell.value:
                        continue
                    ...
                    cell_data = {'coordinate':cell.coordinate, 
                     'row':cell.row, 
                     'column':cell.col_idx, 
                    ...
                    if len(sep) < 2:
                        continue
                    if sep[0].lower() not in keywords:
                        if sep[1].lower() in keywords:
                            comment = sep[0]
                            sep = sep[1:]
    ...
    def Collect(self, target, table):
        if isinstance(target, PageTaxInitFormBase):
            ...
            if type(target) == str and target.lower().endswith('.pdf'):
                pdf_file = target
                ...
                if type(target) == list and target and target[0].lower().endswith('.pdf'):
                    xlsx_files = []
                    target_xlsx = ''
                    ...
                    if type(target) == str and target.lower().endswith('.xlsx'):
                        target = load_workbook(target)
                        handler = self.CollectExcel
                    else:
                        if type(target) == list:
                            if target and target[0].lower().endswith('.xlsx'):
                            ...
                            assert sample_file, '未找到样本文件 table_id=%s ssqq=%s ssqz=%s' % (table['id'], table['ssqs'], table['ssqz'])
    ...
    def CollectExcel(self, workbook, cache_sheet, new_cache, table, table_name, cells):
        if not 'sheet' not in cells:
            if 'cells' not in cells['sheet']:
            ...
            if 'combine_sheet' in cells:
                if 'cells' in cells['combine_sheet']:
                    if cells['combine_sheet']['cells']:
            ...
            for label in cells['labels']:
                ...
                if new_cache:
                    ...
                    cache_cell.fill = changed_fill
                    verify_pass = False
                elif cache_cell.value.strip() != value.strip():
                    cache_cell.value = value
                    cache_cell.fill = changed_fill
                    verify_pass = False

            return verify_pass or False
        for cell in cells['cells']:
            coordinate = cell['coordinate']
            ...
            if value and value[0].isdigit() and value[(-1)].isdigit():
                ...
                if len(value) >= 2 and value[0] == '-' and value[1].isdigit():
                    if value[(-1)].isdigit():
                        value = value.replace(',', '')
                elif value:
                    if value[(-1)] == '%' and value[:-1].replace('.', '').isdigit():
                        val = value[:-1]
                        if val.count('.') == 1:
                            val_f = val.split('.')[1]
                        ...
                        fmt = '%%.%df' % float_len
                        value = fmt % float_val
                item_data = {'id':cell['id'],  'value':value}
    ...
    def SaveTableData(self, tableset_id, tableset_type, tax_type, ssqs, ssqz, ssq_type, table_name, params):
        for exist_data in self.collected_data:
            if exist_data['tableset_id'] == tableset_id and exist_data['tableset_type'] == tableset_type and exist_data['tax_type'] == tax_type and exist_data['ssqs'] == ssqs and exist_data['ssqz'] == ssqz and exist_data['ssq_type'] == ssq_type:
                tableset_data = exist_data
                break
        else:
            tableset_data = {'tableset_id':tableset_id, 
    ...
    def clean_template_files(self):
        ...
        if not dir:
            ...
            return os.path.exists(dir) or None
...
if __name__ == '__main__':
    pdf_to_xlsx('Y:\\Downloads\\YBNSRZZS.pdf', 'Y:\\Downloads\\YBNSRZZS.xlsx')
    task = TaskTaxInitBase()
    task.LoadSamples('Y:\\Documents\\Works\\PyProjects\\FuJian\\任务\\税务初始化\\样本文件')
    currentTable = {'name':'增值税纳税申报表（一般纳税人适用）', 
     'id':'010100', 
     'ssqType':'', 
     'sbqs':'2019-06-01', 
     'sbqz':'2019-06-30', 
     ...
     'ssqz':'2019-05-31', 
     'type':'2'}
    task.Collect('Y:\\Downloads\\forms.xlsx', currentTable)
    task.Success()