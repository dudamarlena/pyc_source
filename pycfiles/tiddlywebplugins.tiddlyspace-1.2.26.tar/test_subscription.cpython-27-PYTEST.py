...
# [GCC 8.4.0]
# Embedded file name: /Users/cdent/src/tiddlyspace/test/test_subscription.py
# Compiled at: 2013-08-20 13:22:51
import __builtin__ as @py_builtins, _pytest.assertion.rewrite as @pytest_ar, simplejson, httplib2, wsgi_intercept
...
def remove_subscription(subscribed, subscriber, cookie=None):
    return add_subscription(subscribed, subscriber, cookie=cookie, unsubscribe=True)
...
def add_subscription(subscribed, subscriber, cookie=None, unsubscribe=False):
    ...
    return http.request('http://0.0.0.0:8080/spaces/%s' % subscriber, method='POST', headers={'Content-Type': 'application/json', 
...
def test_subscription():
    ...
    @py_assert2 = len(recipe)
    @py_assert5 = 8
    @py_assert4 = @py_assert2 == @py_assert5
    if not @py_assert4:
        @py_format7 = @pytest_ar._call_reprcompare(('==', ), (@py_assert4,), ('%(py3)s\n{%(py3)s = %(py0)s(%(py1)s)\n} == %(py6)s', ), (@py_assert2, @py_assert5)) % {'py0': @pytest_ar._saferepr(len) if 'len' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(len) else 'len', 'py1': @pytest_ar._saferepr(recipe) if 'recipe' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(recipe) else 'recipe', 'py3': @pytest_ar._saferepr(@py_assert2), 'py6': @pytest_ar._saferepr(@py_assert5)}
...
def test_unsubscribe():
    ...
    if not @py_assert2:
        ...
        raise AssertionError(@pytest_ar._format_explanation(@py_format7))
    @py_assert0 = @py_assert2 = @py_assert3 = None
    @py_assert0 = 'Invalid content for unsubscription'
    ...
    if not @py_assert2:
        ...
        raise AssertionError(@pytest_ar._format_explanation(@py_format7))
    @py_assert0 = @py_assert2 = @py_assert3 = None
    @py_assert0 = 'Attempt to unsubscribe self'
    @py_assert2 = @py_assert0 in content
    if not @py_assert2:
        @py_format4 = @pytest_ar._call_reprcompare(('in', ), (@py_assert2,), ('%(py1)s in %(py3)s', ), (@py_assert0, content)) % {'py1': @pytest_ar._saferepr(@py_assert0), 'py3': @pytest_ar._saferepr(content) if 'content' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(content) else 'content'}
        @py_format6 = 'assert %(py5)s' % {'py5': @py_format4}
