...
# [GCC 8.4.0]
# Embedded file name: /Users/dr/Documents/code/Python/Projects/ControllProxy/seed/mrpackage/services.py
# Compiled at: 2018-06-25 06:53:11
# Size of source mod 2**32: 21315 bytes
from socketserver import BaseRequestHandler, UDPServer
from socket import socket, AF_INET, SOCK_DGRAM
from socket import timeout
import time, base64, json, sys, os, pickle
from socketserver import BaseRequestHandler, UDPServer
import rsa
                                ...
                                if op == b'script':
                                    ...
                                    if op == b'reset':
                                        ...
                                        if op == b'check':
                                            o = Os()
                                            msgr = []
                                            for i in ('python3', 'pip3', 'tor', 'proxychains',
                                                      'sslocal'):
                                                service = {}
                                                service['name'] = i
                                                if o.check(i):
                                            ...
                                            if op == b'log':
                                                ...
                                                if op == b'reinstall':
                                                    ...
                                                    if hasattr(APIS, op.decode('utf8')):
        ...
        def _rpc(*args, **kwargs):
            if self.op == 'rpc':
                if '.' in func_name:
                    ...
                    lib = func_name
                    fun = func_name
                msg = {'args':pickle.dumps(args),  'kwargs':pickle.dumps(kwargs), 
                 'op':func_name, 
                 'lib':lib, 
                 'fun':fun}
                if self.async:
    ...
    def _async_read_patch(self, callback):
        ...
        def __async_read_patch(rec):
            if rec:
                res = deFormat(rec)
                if 'result' in res:
                    if 'pickle' in res['result']:
                        if res['result']['pickle']:
        ...
        return _Bitter__async_read_patch
