...
# [GCC 8.4.0]
# Embedded file name: /home/stepank/projects/pyws/examples/functions.py
...
@register('add_string_dicts', return_type=ABStringDict, args=(
 (
  ABStringDict, ab_string_dict_none),
 (
  ABStringDict, ab_string_dict_none)))
@register('add_integer_dicts', return_type=ABIntegerDict, args=(
 (
...
@register('add_string_lists', return_type=StringList, args=(
...
@register('add_integer_lists', return_type=IntegerList, args=(
...
def sum_tree(p):
    return p and p['value'] + (p['left'] and sum_tree(p['left']) or 0) + (p['right'] and sum_tree(p['right']) or 0) or 0
...
def get_tree(p):
    if not p:
        ...
        return {'value': 3, 'left': {'value': 4, 'left': None, 'right': None}, 'right': {'value': 5, 'left': None, 'right': None}}


class HelloError(Exception):

    def __init__(self, name):
        super(HelloError, self).__init__('%s error' % name)
