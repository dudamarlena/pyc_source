...
# [GCC 8.4.0]
# Embedded file name: /Users/brentpayne/anaconda/lib/python2.7/site-packages/phrase/noun_phrase_dictionary.py
...
class NounPhraseDictionary(PhraseDictionary):
    ...
    def convert_noun_phrases(self, id_run, pos_run):
        ...
        while i < len(id_run):
            ...
            if phrase_id:
                if pos_run[i] in ('JJ', 'JJR', 'JJS', 'NN', 'NNS', 'NNP', 'NNPS', 'SYM',
                                  'CD', 'VBG', 'FW', 'NP'):
...
EXCLUDE_SET1 = (')', '(', ',', "'", '"')
EXCLUDE_SET2 = (':', ')', '(', ',', "'", '"', 'a', 'on', 'the', '!', '?', 'of', "n't",
                "'re", 'to', '.')

def exclude_ngram_filter(w1, w2):
    if type(w1) in (tuple, list):
        rv = w2[1] not in ('NNP', 'NN', 'VBG', 'NNS', 'NNPS', 'FW', 'CD', 'NP') or w2[0] in EXCLUDE_SET2 or w1[0] in EXCLUDE_SET1
        return rv
    else:
        return False