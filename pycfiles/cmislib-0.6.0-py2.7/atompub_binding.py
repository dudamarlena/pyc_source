...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/cmislib/atompub_binding.py
# Compiled at: 2014-04-15 23:31:58
"""
...
from exceptions import CmisException, ObjectNotFoundException, InvalidArgumentException, NotSupportedException
...
import datetime, StringIO, logging
...
class AtomPubBinding(Binding):
    ...
    def get(self, url, username, password, **kwargs):
        ...
        resp, content = Rest().get(url, username=username, password=password, **kwargs)
        if resp['status'] != '200':
    ...
    def delete(self, url, username, password, **kwargs):
        ...
        if len(self.extArgs) > 0:
            kwargs.update(self.extArgs)
        resp, content = Rest().delete(url, username=username, password=password, **kwargs)
    ...
    def post(self, url, username, password, payload, contentType, **kwargs):
        ...
        if len(self.extArgs) > 0:
            kwargs.update(self.extArgs)
        resp, content = Rest().post(url, payload, contentType, username=username, password=password, **kwargs)
        if resp['status'] == '200':
            try:
                return minidom.parseString(content)
            except ExpatError:
    ...
    def put(self, url, username, password, payload, contentType, **kwargs):
        ...
        if len(self.extArgs) > 0:
            kwargs.update(self.extArgs)
        resp, content = Rest().put(url, payload, contentType, username=username, password=password, **kwargs)
...
class RepositoryService(RepositoryServiceIfc):
    ...
    def getRepositories(self, client):
        ...
        for node in [ e for e in workspaceElements if e.nodeType == e.ELEMENT_NODE ]:
            ...
            repositories.append({'repositoryId': repository.getRepositoryId(), 'repositoryName': repository.getRepositoryInfo()['repositoryName']})
...
class AtomPubCmisObject(CmisObject):
    ...
    def reload(self, **kwargs):
        ...
        self.xmlDoc = self._cmisClient.binding.get(byObjectIdUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, **addOptions)
        self._initData()
    ...
    def getObjectParents(self, **kwargs):
        ...
        result = self._cmisClient.binding.get(parentUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, **kwargs)
    ...
    def getProperties(self):
        ...
        if self._properties == {}:
            ...
            for node in [ e for e in propertiesElement.childNodes if e.nodeType == e.ELEMENT_NODE and e.namespaceURI == CMIS_NS ]:
                ...
                if node.childNodes and node.getElementsByTagNameNS(CMIS_NS, 'value')[0] and node.getElementsByTagNameNS(CMIS_NS, 'value')[0].childNodes:
                    valNodeList = node.getElementsByTagNameNS(CMIS_NS, 'value')
                    if len(valNodeList) == 1:
                        propertyValue = parsePropValue(valNodeList[0].childNodes[0].data, node.localName)
                    else:
                        propertyValue = []
                        for valNode in valNodeList:
                            propertyValue.append(parsePropValue(valNode.childNodes[0].data, node.localName))
    ...
    def updateProperties(self, properties):
        ...
        updatedXmlDoc = self._cmisClient.binding.put(selfUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, xmlEntryDoc.toxml(encoding='utf-8'), ATOM_XML_TYPE, **args)
    ...
    def move(self, sourceFolder, targetFolder):
        ...
        self._cmisClient.binding.post(postUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, self.xmlDoc.toxml(encoding='utf-8'), ATOM_XML_ENTRY_TYPE, **args)
    ...
    def delete(self, **kwargs):
        ...
        self._cmisClient.binding.delete(url.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, **kwargs)
    ...
    def createRelationship(self, targetObj, relTypeId):
        ...
        result = self._cmisClient.binding.post(url.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, xmlDoc.toxml(encoding='utf-8'), ATOM_XML_TYPE)
    ...
    def getRelationships(self, **kwargs):
        ...
        url = self._getLink(RELATIONSHIPS_REL)
        assert url is not None, 'Could not determine relationships URL'
        result = self._cmisClient.binding.get(url.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, **kwargs)
        return AtomPubResultSet(self._cmisClient, self._repository, result)
    ...
    def getACL(self):
        ...
        if self._repository.getCapabilities()['ACL']:
            ...
            result = self._cmisClient.binding.get(aclUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password)
    ...
    def applyACL(self, acl):
        ...
        if self._repository.getCapabilities()['ACL'] == 'manage':
            raise (isinstance(acl, ACL) or CmisException)('The ACL to apply must be an instance of the ACL class.')
        aclUrl = self._getLink(ACL_REL)
        if not aclUrl:
            raise AssertionError("Could not determine the object's ACL URL.")
            result = self._cmisClient.binding.put(aclUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, acl.getXmlDoc().toxml(encoding='utf-8'), CMIS_ACL_TYPE)
            return AtomPubACL(xmlDoc=result)
        raise NotSupportedException
    ...
    def _getLink(self, rel, ltype=None):
        ...
        for linkElement in linkElements:
            ...
            elif linkElement.attributes.has_key('rel'):
                relAttr = linkElement.attributes['rel'].value
                if relAttr == rel:
                    return linkElement.attributes['href'].value
...
class AtomPubRepository(object):
    ...
    def getSupportedPermissions(self):
        ...
        if self._permissions or self.xmlDoc is None:
            self.reload()
        suppEls = self.xmlDoc.getElementsByTagNameNS(CMIS_NS, 'supportedPermissions')
        if not len(suppEls) == 1:
            raise AssertionError('Expected the repository service document to have one element named supportedPermissions')
    ...
    def getPropagation(self):
        ...
        if not self.getCapabilities()['ACL']:
            raise NotSupportedException(messages.NO_ACL_SUPPORT)
        if self._propagation or self.xmlDoc is None:
            self.reload()
        propEls = self.xmlDoc.getElementsByTagNameNS(CMIS_NS, 'propagation')
        if not len(propEls) == 1:
            raise AssertionError('Expected the repository service document to have one element named propagation')
    ...
    def getTypeChildren(self, typeId=None):
        """
        ...
        if typeId:
            ...
            for entryElement in entryElements:
                objectType = AtomPubObjectType(self._cmisClient, self, xmlDoc=entryElement)
    ...
    def getTypeDescendants(self, typeId=None, **kwargs):
        ...
        typesXmlDoc = self._cmisClient.binding.get(descendUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, **kwargs)
        entryElements = typesXmlDoc.getElementsByTagNameNS(ATOM_NS, 'entry')
        types = []
        for entryElement in entryElements:
            objectType = AtomPubObjectType(self._cmisClient, self, xmlDoc=entryElement)
    ...
    def getTypeDefinitions(self, **kwargs):
        ...
        typesXmlDoc = self._cmisClient.binding.get(typesUrl, self._cmisClient.username, self._cmisClient.password, **kwargs)
        entryElements = typesXmlDoc.getElementsByTagNameNS(ATOM_NS, 'entry')
        types = []
        for entryElement in entryElements:
            objectType = AtomPubObjectType(self._cmisClient, self, xmlDoc=entryElement)
    ...
    def getObject(self, objectId, **kwargs):
        """
    ...
    def getObjectByPath(self, path, **kwargs):
        ...
        result = self._cmisClient.binding.get(byObjectPathUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, **addOptions)
        entryElements = result.getElementsByTagNameNS(ATOM_NS, 'entry')
    ...
    def query(self, statement, **kwargs):
        ...
        result = self._cmisClient.binding.post(queryUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, xmlDoc.toxml(encoding='utf-8'), CMIS_QUERY_TYPE)
    ...
    def getContentChanges(self, **kwargs):
        ...
        result = self._cmisClient.binding.get(changesUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, **kwargs)
        return AtomPubChangeEntryResultSet(self._cmisClient, self, result)

    def createDocumentFromString(self, name, properties={}, parentFolder=None, contentString=None, contentType=None, contentEncoding=None):
        """
        ...
        return parentFolder.createDocument(name, properties, StringIO.StringIO(contentString), contentType, contentEncoding)

    def createDocument(self, name, properties={}, parentFolder=None, contentFile=None, contentType=None, contentEncoding=None):
        """
        ...
        xmlDoc = getEntryXmlDoc(self, None, properties, contentFile, contentType, contentEncoding)
        result = self._cmisClient.binding.post(postUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, xmlDoc.toxml(encoding='utf-8'), ATOM_XML_ENTRY_TYPE)
        return AtomPubDocument(self._cmisClient, self, xmlDoc=result)

    def createDocumentFromSource(self, sourceId, properties={}, parentFolder=None):
        """
        This is not yet implemented.
    ...
    def createFolder(self, parentFolder, name, properties={}):
        """
    ...
    def getUriTemplates(self):
        ...
        if self._uriTemplates == {}:
            ...
            for uriTemplateElement in uriTemplateElements:
                ...
                self._uriTemplates[templType] = UriTemplate(template, templType, mediatype)
    ...
    def getCollection(self, collectionType, **kwargs):
        ...
        result = self._cmisClient.binding.get(self.getCollectionLink(collectionType).encode('utf-8'), self._cmisClient.username, self._cmisClient.password, **kwargs)
        return AtomPubResultSet(self._cmisClient, self, result)
...
class AtomPubResultSet(ResultSet):
    ...
    def _getPageResults(self, rel):
        ...
        if link:
            result = self._cmisClient.binding.get(link.encode('utf-8'), self._cmisClient.username, self._cmisClient.password)
            self._xmlDoc = result
            self._results = []
    ...
    def getResults(self):
        ...
        if self._xmlDoc:
            ...
            entries = []
            for entryElement in entryElements:
                cmisObject = getSpecializedObject(AtomPubCmisObject(self._cmisClient, self._repository, xmlDoc=entryElement))
...
class AtomPubDocument(AtomPubCmisObject):
    ...
    def checkout(self):
        ...
        result = self._cmisClient.binding.post(checkoutUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, entryXmlDoc.toxml(encoding='utf-8'), ATOM_XML_ENTRY_TYPE)
        self.reload()
    ...
    def checkin(self, checkinComment=None, **kwargs):
        ...
        url = self._getSelfLink()
        result = self._cmisClient.binding.put(url.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, entryXmlDoc.toxml(encoding='utf-8'), ATOM_XML_TYPE, **kwargs)
        return AtomPubDocument(self._cmisClient, self._repository, xmlDoc=result)
    ...
    def getAllVersions(self, **kwargs):
        ...
        result = self._cmisClient.binding.get(versionsUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, **kwargs)
        return AtomPubResultSet(self._cmisClient, self._repository, result)
    ...
    def getContentStream(self):
        ...
        if contentElements[0].attributes.has_key('src'):
            ...
            result, content = Rest().get(srcUrl.encode('utf-8'), username=self._cmisClient.username, password=self._cmisClient.password, **self._cmisClient.extArgs)
    ...
    def setContentStream(self, contentFile, contentType=None):
        ...
        if contentElements[0].attributes.has_key('src'):
            srcUrl = contentElements[0].attributes['src'].value
        if not srcUrl:
            raise AssertionError('Unable to determine content stream URL.')
        ...
        if self.properties.has_key('cmis:changeToken') and self.properties['cmis:changeToken'] is not None:
            self.logger.debug('Change token present, adding it to args')
            args = {'changeToken': self.properties['cmis:changeToken']}
        result = self._cmisClient.binding.put(srcUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, contentFile.read(), mimetype, **args)
        return AtomPubDocument(self._cmisClient, self._repository, xmlDoc=result)
    ...
    def deleteContentStream(self):
        ...
        if self.properties.has_key('cmis:changeToken') and self.properties['cmis:changeToken'] is not None:
            self.logger.debug('Change token present, adding it to args')
            args = {'changeToken': self.properties['cmis:changeToken']}
        self._cmisClient.binding.delete(srcUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, **args)
        return
    ...
    def getPaths(self):
        ...
        if parentUrl is None:
            raise NotSupportedException('Root folder does not support getObjectParents')
        result = self._cmisClient.binding.get(parentUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, filter='cmis:path', includeRelativePathSegment=True)
        paths = []
...
class AtomPubFolder(AtomPubCmisObject):
    ...
    def createFolder(self, name, properties={}):
        ...
        result = self._cmisClient.binding.post(postUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, entryXml.toxml(encoding='utf-8'), ATOM_XML_ENTRY_TYPE)
        return AtomPubFolder(self._cmisClient, self._repository, xmlDoc=result)

    def createDocumentFromString(self, name, properties={}, contentString=None, contentType=None, contentEncoding=None):
        """
        Creates a new document setting the content to the string provided. If
        the repository supports unfiled objects, you do not have to pass in
        a parent :class:`Folder` otherwise it is required.
        ...
        """
        return self._repository.createDocumentFromString(name, properties, self, contentString, contentType, contentEncoding)

    def createDocument(self, name, properties={}, contentFile=None, contentType=None, contentEncoding=None):
        """
        ...
        The following optional arguments are not yet supported:
         ...
         - removeACEs
        """
        return self._repository.createDocument(name, properties, self, contentFile, contentType, contentEncoding)
    ...
    def getChildren(self, **kwargs):
        ...
        result = self._cmisClient.binding.get(childrenUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, **kwargs)
        return AtomPubResultSet(self._cmisClient, self._repository, result)
    ...
    def getDescendants(self, **kwargs):
        ...
        result = self._cmisClient.binding.get(descendantsUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, **kwargs)
        return AtomPubResultSet(self._cmisClient, self._repository, result)
    ...
    def getParent(self):
        ...
        """
        parentUrl = self._getLink(UP_REL)
        result = self._cmisClient.binding.get(parentUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password)
        return AtomPubFolder(self._cmisClient, self._repository, xmlDoc=result)
    ...
    def deleteTree(self, **kwargs):
        ...
        url = self._getLink(DOWN_REL, CMIS_TREE_TYPE_P)
        result = self._cmisClient.binding.delete(url.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, **kwargs)
    ...
    def addObject(self, cmisObject, **kwargs):
        ...
        postUrl = self.getChildrenLink()
        self._cmisClient.binding.post(postUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, cmisObject.xmlDoc.toxml(encoding='utf-8'), ATOM_XML_ENTRY_TYPE, **kwargs)
    ...
    def removeObject(self, cmisObject):
        ...
        self._cmisClient.binding.post(postUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, cmisObject.xmlDoc.toxml(encoding='utf-8'), ATOM_XML_ENTRY_TYPE, **args)
...
class AtomPubObjectType(ObjectType):
    ...
    def reload(self, **kwargs):
        ...
        result = self._cmisClient.binding.get(byTypeIdUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, **kwargs)
        entryElements = result.getElementsByTagNameNS(ATOM_NS, 'entry')
    ...
    def getProperties(self):
        ...
        if self._properties == {}:
            ...
            for node in [ e for e in propertiesElement.childNodes if e.nodeType == e.ELEMENT_NODE ]:
                propertyName = node.attributes['propertyDefinitionId'].value
                if node.childNodes and node.getElementsByTagNameNS(CMIS_NS, 'value')[0] and node.getElementsByTagNameNS(CMIS_NS, 'value')[0].childNodes:
                    propertyValue = parsePropValue(node.getElementsByTagNameNS(CMIS_NS, 'value')[0].childNodes[0].data, node.localName)
                else:
                    propertyValue = None
                self._properties[propertyName] = propertyValue
...
def getEntryXmlDoc(repo=None, objectTypeId=None, properties=None, contentFile=None, contentType=None, contentEncoding=None):
    """
