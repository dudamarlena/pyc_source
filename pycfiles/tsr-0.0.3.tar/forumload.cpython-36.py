...
# [GCC 8.4.0]
# Embedded file name: /home/marx/venv3/lib/python3.6/site-packages/tsr/modules/forumload.py
# Compiled at: 2018-05-13 10:05:54
...
def load_forum_database(forumtype, f_or_u_id, **kwargs):
    ...
    if forumtype == 'forum':
            ...
            try:
                ...
                if 'nPosts_claimed' in thread_dict:
                    if thread_dict['nPosts'] == thread_dict['nPosts_claimed']:
                        ...
                        toprint += general.myjoin(cl.green, threadid, '', successes, cl.yellow, warnings, cl.brown, errors, cl.red, failures, cl.end, '\r')
                    else:
                        errors += 1
                        ls_error.append(int(threadid))
                        if load_threads:
                            ls_threads.append(thread_dict)
                        toprint += general.myjoin(cl.brown, threadid, cl.green, successes, cl.yellow, warnings, cl.brown, errors, cl.red, failures, cl.red, 'nPosts:', thread_dict['nPosts'], 'nPosts_claimed:', thread_dict['nPosts_claimed'], cl.end)
                else:
                    if len(thread_dict['posts']) == 0:
                        ...
                        if load_threads:
                            ls_threads.append(thread_dict)
                        toprint += general.myjoin(cl.yellow, threadid, cl.green, successes, cl.yellow, warnings, cl.brown, errors, cl.red, failures, cl.yellow, 'No nPosts_claimed', cl.end)
            except KeyError:
                try:
                    failures += 1
                    ls_failure.append(int(threadid))
                    toprint += general.myjoin(cl.red, threadid, cl.green, successes, cl.yellow, warnings, cl.brown, errors, cl.red, failures, cl.end)
                except:
                    pass
        ...
        return (
         ls_success, ls_warning, ls_error, ls_failure, ls_threads, usernames_to_id)