...
# [GCC 8.4.0]
# Embedded file name: /home/marx/venv3/lib/python3.6/site-packages/tsr/modules/regex.py
# Compiled at: 2018-07-05 14:49:13
...
def parse_custom_colours(string):
    for tpl in colours_and_regex:
        ...
        regex = tpl[1]
        groups = tpl[2]
        string = re.sub(regex, ('\\{0}{1}\\{2}{3}\\{4}'.format(groups[0], colour_string, groups[1], '@@@useTwoLeft', groups[2])), string, flags=(re.MULTILINE))

    return string


url = 'https?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\\(\\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+'
url2 = '(https?:\\/\\/(www\\.)?[-a-zA-Z0-9@:%._\\+~#=]{2,256}\\.[a-z]{2,6}\\b([-a-zA-Z0-9@:%_\\+.,~#?&//=]*))'
image = '([a-zA-Z0-9_-]*\\.(jpg|jpeg|png|gif))'
between_words = '[\\ \\n\\.\\,\\;\\:\\/\\-\\+\\=\\?\\!\\"\\&\\(\\)\\[\\]\\@\\£\\$\\%\\’]'
between_words_title = '[\\ \\n\\.\\,\\;\\:\\/\\-\\+\\=\\?\\!\\"\\&\\(\\)\\[\\]\\@\\£\\$\\%]'
sentence_ending = ' ?[.\\!\\?$]*'
...
def domain_from_url(url):
    url = re.sub('(https?://((www|images?|img|[a-z])\\.)?)', '', url)
    url = url.split('/')[0]
    try:
        return re.search('([a-z][a-z]*\\.(com|(co|org|gov|police|ac)\\.(uk|nz|au)|org|gov|net|tech|ac|edu|io|ie|de|fr|ca|cz|ru|se|gr|nz|nl|gl|eu|tv|va|pl))', url).group(0)
    except:
        return url
...
def findall_domains(search_string, **kwargs):
    ...
    for u in urls:
        if kwargs['only_image_hosts']:
            add_domain = False
            if u.split('.')[(-1)].lower() in ('jpg', 'png', 'jpeg', 'gif', 'webm',
                                              'mp4'):
                add_domain = True
            else:
                add_domain = True
