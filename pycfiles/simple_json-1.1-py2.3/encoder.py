...
# [GCC 8.4.0]
# Embedded file name: build/bdist.darwin-8.3.0-Power_Macintosh/egg/simple_json/encoder.py
# Compiled at: 2005-12-30 15:03:38
import re, math
INFCHARS = re.compile('[infINF]')
ESCAPE = re.compile('[\\x00-\\x19\\\\"\\b\\f\\n\\r\\t]')
...
class JSONEncoder(object):
    ...
    def iterencode_list(self, lst, markers=None):
        if not lst:
            yield '[]'
            return
    ...
    def iterencode_dict(self, dct, markers=None):
        if not dct:
            yield '{}'
            return
        if markers is not None:
    ...
    def _iterencode(self, o, markers=None):
        if isinstance(o, basestring):
            ...
            for chunk in self.iterencode_list(o, markers):
                yield chunk

        elif isinstance(o, dict):
            for chunk in self.iterencode_dict(o, markers):
                yield chunk
            ...
            if markers is not None:
                ...
                if markerid in markers:
                    raise ValueError('Circular reference detected')
                markers[markerid] = o
            for chunk in self.iterencode_default(o, markers):
                yield chunk
    ...
    def iterencode_default(self, o, markers=None):
