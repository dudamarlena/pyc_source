...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/sy/net/remote.py
# Compiled at: 2010-11-23 16:34:20
"""
:synopsis: Utilities for working with servers on the network.

"""
import sy, urllib, os, pwd, platform, os.path, urllib2, smtplib, mimetypes
try:
...
def sendmail(to, subject, message, sender=None, attachments=(), mailhost='localhost', mailport=25, timeout=60.0):
    """ Send an email
    ...
    for attachment in attachments:
        ...
        if maintype == 'text':
            ...
            msg = MIMEImage(sy.path.slurp(attachment, binary=True), _subtype=subtype)
        elif maintype == 'audio':
            msg = MIMEAudio(sy.path.slurp(attachment, binary=True), _subtype=subtype)
        else:
