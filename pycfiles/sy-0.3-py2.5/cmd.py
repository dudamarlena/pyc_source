...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/sy/cmd.py
# Compiled at: 2011-02-07 06:47:57
"""
...
"""
import time, os, select, signal, re, sy.log, sy.util
...
def format_cmd(command, args):
    ...
    if command.count('{}') != len(args):
        raise TypeError('Number of arguments do not match the format string %r: %r' % (
         command, args))
...
def do(command, *args, **kwargs):
    """Spawn a command and returns stdout and stderr. If the command
...
def run(command, *args, **kwargs):
    ...
    if process.read(timeout):
        ...
        log.error(errormsg)
        raise CommandTimeoutError(errormsg, out=process.outdata, err=process.errdata, cmd=escapedcmd, timeout=timeout)
    exitstatus = os.WEXITSTATUS(process.cleanup())
    out = process.outdata
    err = process.errdata
    del process
    log.debug('Command result, stdout:{}, stderr:{}, exitstatus:{}', out.strip(), err.strip(), exitstatus)
