...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/sy/path.py
# Compiled at: 2011-02-05 16:35:33
"""
...
"""
import os, re, grp, pwd, codecs, fcntl, tempfile, os.path, stat, sys, shutil, tarfile, zipfile, sy.log, sy.cmd
...
def dump(path, content, append=False, binary=False, encoding=None, newline=os.linesep):
    r""" Dump content to a file.
...
def extract(archive, dir):
    ...
    if tarfile.is_tarfile(archive):
        ...
        decompressor_table = {'gzcat': ('gz', 'tgz'), 
...
def lines(path, encoding=None, newline=True):
    r""" Return list of lines from file. 
...
def replace(path, match, replacement, encoding=None):
    r""" Replace all occurances of ``match`` with ``replacement``. 
...
')
    ...
    (_, tmppath) = tempfile.mkstemp(dir=os.path.dirname(path), prefix=os.path.basename(path), suffix='.replacement')
    try:
