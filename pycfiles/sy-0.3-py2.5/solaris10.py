...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/sy/net/intf/solaris10.py
# Compiled at: 2010-11-23 16:34:20
"""
...
class Interface(object):
    ...
    def __init__(self, **kwargs):
        ...
        if bad_keys:
            raise AttributeError('Bad arguments passed to init: %s' % (', ').join(bad_keys))
        return
...
def check(ifname, ipaddress=None, netmask=None, group=None, min_interface_speed=MIN_INTERFACE_SPEED):
    ...
    if intf.speed < min_interface_speed:
        raise sy.Error('Interface %s speed is to low: %d' % (ifname, intf.speed))
...
def _unconfigure_physical(intf):
    ...
    try:
        sy.cmd.do('ifconfig {} unplumb', ifname, prefix='Unable to unconfigure interface %s' % ifname)
    finally:
        refresh()
...
def _unconfigure_logical(intf):
    ...
    cmd = [
     'ifconfig', physical_ifname, 'removeif', intf.ipaddress]
    try:
        sy.cmd.do('ifconfig {} removeif {}', physical_ifname, intf.ipaddress, prefix='Unable to unconfigure logical interface %s' % intf.name)
    finally:
        refresh()
...
def configure(ifname, ipaddress=None, netmask=None, group=None, standby=False, hostname=None, permanent=True):
...
def addif(ifname, ipaddress, netmask, hostname=None, permanent=True):
    ...
    _add_network(ipaddress, netmask)
    ifconfig = [
     'addif', ipaddress, 'netmask', netmask, 'up']
