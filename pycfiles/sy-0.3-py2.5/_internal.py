...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/sy/_internal.py
...
def platform_select(options, test_platform=()):
    ...
    if test_platform:
        ...
        (system, release, version) = this_platform = platform.system_alias(platform.system(), platform.release(), platform.version())
    for option in options:
        (match, select) = option
        match_padding = [
         None] * (3 - len(match))
        (match_system, match_release, match_version) = match + match_padding
...
def dynload_module(modulename, modules, objects):
    ...
    new_module.__dict__.update({'__file__': old_module.__file__, 
