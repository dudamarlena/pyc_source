...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/sy/prompt.py
...
def confirm(question, default=''):
    ...
    if default is True:
        ...
        default = 'n'
    return ask(question, default=default, type=bool)
...
def choose(question, choices, multichoice=False, default=''):
    ...
    def to_index_list(answer):
        try:
            ints = [ int(i) - 1 for i in re.split('\\s*,\\s*|\\s+', answer) ]
            for i in ints:
                if i < 0 or i >= len(choices):
                    return (
                     None, '%d is not a valid option' % (i + 1))
    ...
    while True:
        selected = ask(out(choice_q, to_s=True), type=to_index_list, default=default)
        if selected:
            if not multichoice:
                if len(selected) > 1:
