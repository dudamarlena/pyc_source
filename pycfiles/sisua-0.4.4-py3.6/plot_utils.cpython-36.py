...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/sisua/utils/plot_utils.py
# Compiled at: 2019-05-28 07:36:04
# Size of source mod 2**32: 5135 bytes
from matplotlib import pyplot as plt
import seaborn, pandas as pd, numpy as np
...
def _mean(W_or_V):
  ...
  """
    W_or_V = W_or_V[0] if isinstance(W_or_V, (tuple, list)) or W_or_V.ndim == 3 else W_or_V
...
def plot_countsum_series(original, imputed, p=None, reduce_axis=0, title=None, ax=None):
    """
  x: [n_samples, n_genes]
    original count
  ...
  """
    if ax is None:
        ...
        if isinstance(imputed, (tuple, list)):
        ...
        ax, handles, indices = plot_series_statistics(count_sum_observed,
...
def plot_countsum_comparison(original, reconstructed, imputed, title, comparing_axis=0, ax=None):
    """
  ...
  """
    ...
    ax.axhline(xmin=0, xmax=max_val, y=(np.median(original)), color=(colors[0]),
      linestyle='--',
      linewidth=1.5,
      label='Corrupted Median')
    ax.axhline(xmin=0, xmax=max_val, y=(np.median(imputed)), color=(colors[1]),
      linestyle='--',
      linewidth=1.5,
      label='Imputed Median')
    ax.axhline(xmin=0, xmax=max_val, y=(np.median(reconstructed)), color=(colors[2]),
      linestyle='--',
      linewidth=1.5,
