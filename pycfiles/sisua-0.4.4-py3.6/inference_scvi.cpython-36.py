...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/sisua/inference/inference_scvi.py
# Compiled at: 2019-06-27 08:22:47
# Size of source mod 2**32: 15035 bytes
...
def _create_gene_dataset(X, local_means=None, local_vars=None):
    ...
    gene_dataset = GeneExpressionDataset(X=X,
...
def _create_data_iter(X, log_variational):
    import torch
    for start, end in batching(batch_size=(PREDICTION_BATCH_SIZE // 3), n=(X.shape[0])):
...
class InferenceSCVI(Inference):
    __doc__ = ' InferenceSCVI '

    def __init__(self, gene_dim, dispersion='gene-cell', xnorm='log', tnorm='raw', ynorm='prob', xclip=0, yclip=0, xdist='zinb', ydist='bernoulli', zdist='normal', xdrop=0.3, edrop=0, zdrop=0, ddrop=0, hdim=128, zdim=32, nlayer=2, batchnorm=True, analytic=True, kl_weight=1.0, warmup=400, y_weight=10.0, extra_module_path=None, no_library_size=False, **kwargs):
        try:
            import scvi
        except ImportError as e:
            raise RuntimeError('scVI package at https://github.com/YosefLab/scVI is required')
        ...
        if xnorm not in ('log', 'raw'):
            raise ValueError('Only support log variational or raw count variational for scVI')
        if tnorm not in ('raw', ):
            raise ValueError('Only support raw count target value for scVI')
    ...
    def _initialize_model(self, weights=None):
        ...
        self._vae = VAE(n_input=(self.gene_dim), n_batch=0, n_labels=0, n_hidden=(self.hdim),
          n_latent=(self.zdim),
          n_layers=(self.nlayer),
    ...
    def fit(self, X, y=None, supervised_percent=0.8, validation_percent=0.1, n_mcmc_samples=1, corruption_rate=0.25, corruption_dist='binomial', batch_size=64, n_epoch=120, learning_rate=0.0001, monitoring=False, fig_dir=None, detail_logging=False):
        ...
        if self._corruption_rate is None:
            ...
            elif not n_mcmc_samples == 1:
                raise AssertionError('Only support 1 MCMC sample during training for scVI')
            ...
            assert X.shape[1] == self.gene_dim, 'Number of gene expression mismatch: %d != %d' % (self.gene_dim, X.shape[1])
        if validation_percent == 0:
            raise RuntimeError("scVI doesn't support 0% validation percent!")
        from scvi.inference import UnsupervisedTrainer
        gene_dataset = _create_gene_dataset(X)
        if 'dataset' in self.configs:
            if self.configs['dataset'] == 'cortex':
                batch_size = batch_size - 1
        trainer = UnsupervisedTrainer((self._vae), gene_dataset, use_cuda=(_use_cuda()),
    ...
    """
        ...
        pos = Posterior(model=(self._vae), gene_dataset=(_create_gene_dataset(X)), shuffle=False,
          use_cuda=(_use_cuda()),
    ...
    def predict_y(self, X, y, n_mcmc_samples=100):
        ...
        assert self.is_semi_supervised and y is not None, 'Only semi-supervised model has this prediction, and `y` must not be None'
    ...
    def predict_W(self, X, n_mcmc_samples=100):
        ...
        for x in _create_data_iter(X, log_variational=False):
            px_scale, px_r, px_rate, px_dropout, qz_m, qz_v, z, ql_m, ql_v, library = self._vae.inference(x,
              batch_index=None, y=None, n_samples=n_mcmc_samples)
    ...
    """
        ...
        for x in _create_data_iter(X, log_variational=False):
            n = x.shape[0]
            out = self._vae.inference(x,
              batch_index=None, y=None, n_samples=n_mcmc_samples)[1]
    ...
    """
        ...
        for x in _create_data_iter(X, log_variational=False):
            out = self._vae.inference(x,
              batch_index=None, y=None, n_samples=n_mcmc_samples)[3]
            out = op(out.mean(0))
            out = _to_array(out)
            PI.append(out)
    ...
    def sample_Z(self, X, n_mcmc_samples=1):
        ...
        pos = Posterior(model=(self._vae), gene_dataset=(_create_gene_dataset(X)), shuffle=False,
          use_cuda=(_use_cuda()),
          data_loader_kwargs={'batch_size': 64})
        return np.concatenate([np.expand_dims((pos.get_latent(sample=True)[0]), axis=0) for i in range(n_mcmc_samples)],
          axis=0)
