...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/sisua/data/experimental_data/pbmc_8k_ecc_ly.py
# Compiled at: 2019-09-16 06:40:59
...
def read_PBMCcross_ecc_8k(subset, return_ecc, filtered_genes=False, override=False, verbose=False):
  ...
  """
    preprocessed_path = os.path.join(PREPROCESSED_BASE_DIR, 'PBMCcross_%s_%s_preprocessed' % ('ecc' if return_ecc else '8k',
    ...
    if not os.path.exists(os.path.join(preprocessed_path, 'X')):
        pbmc8k_full = read_PBMC8k(subset=subset, override=override,
          filtered_genes=False,
          verbose=verbose)
        pbmcecc_full = read_PBMCeec(subset=subset, override=override,
          filtered_genes=False,
          verbose=verbose)
        all_genes = set(pbmc8k_full['X_col']) & set(pbmcecc_full['X_col'])
        if filtered_genes:
            pbmc8k = read_PBMC8k(subset=subset, override=override,
              filtered_genes=True,
              verbose=verbose)
            pbmcecc = read_PBMCeec(subset=subset, override=override,
              filtered_genes=True,
              verbose=verbose)
...
def read_PBMCcross_remove_protein(subset, return_ecc, filtered_genes=False, override=False, verbose=False, remove_protein=[
 'CD4', 'CD8']):
    remove_protein = sorted([i.lower() for i in as_tuple(remove_protein, t=string_types)])
    preprocessed_path = os.path.join(PREPROCESSED_BASE_DIR, 'PBMCcross_%s_%s_no%s_preprocessed' % (
    ...
    if not os.path.exists(os.path.join(preprocessed_path, 'X')):
        ...
        y = ds['y']
        y_col = ds['y_col']
        remove_ids = [i for i, j in enumerate(y_col) if standardize_protein_name(j).lower() in remove_protein]
