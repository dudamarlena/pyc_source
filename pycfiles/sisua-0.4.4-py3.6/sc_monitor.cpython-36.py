...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/sisua/analysis/sc_monitor.py
# Compiled at: 2019-08-28 08:30:23
# Size of source mod 2**32: 6910 bytes
...
import numpy as np, seaborn as sns, tensorflow as tf
...
from sisua.analysis.latent_benchmarks import plot_distance_heatmap, plot_latents_binary
from sisua.analysis.sc_metrics import SingleCellMetric, _preprocess_output_distribution, _to_binary
...
__all__ = [
 'SingleCellMonitor', 'LearningCurves', 'ScatterPlot', 'HeatmapPlot']
...
class SingleCellMonitor(SingleCellMetric, Visualizer):
    ...
    def call(self, y_true: List[SingleCellOMIC], y_crpt: List[SingleCellOMIC], y_pred: List[Distribution], latents: List[Distribution], extras):
        self.plot(y_true, y_crpt, y_pred, latents, self.model.history.history, extras)
        if len(self.get_figures()) > 0:
            base, ext = os.path.splitext(self.path)
            path = base + '.%d' % (self._last_epoch + 1) + ext
            self.save_figures(path=path, dpi=(self.dpi),
              separate_files=False,
              clear_figures=True)

    @abstractmethod
    def plot(self, y_true: List[SingleCellOMIC], y_crpt: List[SingleCellOMIC], y_pred: List[Distribution], latents: List[Distribution], history, extras):
        raise NotImplementedError


class LearningCurves(SingleCellMonitor):
    __doc__ = " Additional key in the loss dictionary could be given by `extras`,\n  otherwise, plotting 'loss' and 'val_loss' by default.\n  "
...
class ScatterPlot(SingleCellMonitor):
    ...
    def __init__(self, path, data='latent', use_pca=True, dpi=80, **kwargs):
        ...
        data = str(data.lower())
        assert data in ('latent', 'imputation'), 'Only support three data types: latent, imputation and corrupt'
        self.data = data

    def _preprocess(self, y_true, y_pred, latents, extras):
        y_true = y_true[0]
        y_pred = [_preprocess_output_distribution(i) for i in y_pred]
        if not isinstance(extras, SingleCellOMIC):
            raise AssertionError('protein data must be provided as extras in form of SingleCellOMIC')
        else:
            ...
            if self.data == 'latent':
                ...
                if self.data == 'imputation':
                    data = y_pred
                else:
                    raise NotImplementedError()
        return (
         y_true, data, labels, labels_name)

    def plot(self, y_true: List[SingleCellOMIC], y_crpt: List[SingleCellOMIC], y_pred: List[Distribution], latents: List[Distribution], history, extras):
        y_true, data, labels, labels_name = self._preprocess(y_true, y_pred, latents, extras)
        n = len(data)
        for idx, x in enumerate(data):
            distribution = x.__class__.__name__ if not isinstance(x, tfd.Independent) else x.distribution.__class__.__name__
            x = x.mean().numpy()
            if x.ndim == 3:
                x = np.squeeze(x, axis=0)
            fig = plt.figure(figsize=(8, 8))
            plot_latents_binary(Z=x, y=labels,
    ...
    def plot(self, y_true: List[SingleCellOMIC], y_crpt: List[SingleCellOMIC], y_pred: List[Distribution], latents: List[Distribution], history, extras):
        ...
        for idx, x in enumerate(data):
            ...
            if x.ndim == 3:
                x = np.squeeze(x, axis=0)
            fig = plt.figure(figsize=(8, 8))
            plot_distance_heatmap(X=x, labels=labels,
              labels_name=labels_name,
              lognorm=True,
              colormap='hot',
              ax=None,
