...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/sisua/inference/inference_base.py
# Compiled at: 2019-08-26 08:53:27
# Size of source mod 2**32: 52401 bytes
from __future__ import print_function, division, absolute_import
import os, re, io, copy, types, pickle, inspect, warnings
...
from tensorflow.python.keras.layers import Lambda as KerasLambda, Add as KerasAdd, Concatenate as KerasConcatenate
from tensorflow.python.util.nest import flatten
from odin.fuel import Dataset
from odin.ml import fast_pca, fast_tsne
from odin.stats import train_valid_test_split, describe
from odin.utils import get_script_path, ctext, batching, one_hot, uuid, as_list, get_module_from_path, stdio, get_formatted_datetime
from odin import nnet as N, backend as K, training
from odin.backend.keras_helpers import has_keras_meta, to_keras_objective, copy_keras_metadata
import sisua
...
possible_outputs = ('Z', 'Z_sample', 'loss', 'metr', 'L', 'L_sample', 'L_stddev', 'W',
                    'W_sample', 'W_stddev', 'V', 'V_sample', 'V_stddev', 'y', 'y_sample',
                    'pi')
...
class Inference(BaseEstimator):
        ...
        if not int(warmup) > 0:
            ...
            if isinstance(model, string_types):
                ...
                for p in primary_path + extra_module_path:
                    m, e = get_module_from_path(identifier=model,
                      path=p,
                      prefix='models_',
                      return_error=True)
            ...
            self._training_log = ''
            self._history = {'train':defaultdict(list),  'valid':defaultdict(list)}
            ...
            if hasattr(self._model, '__call__'):
                sign = inspect.getargspec(self._model.__call__ if self.is_keras_model else self._model._func._func)
                func_args = sign.args
                if 'self' == func_args[0]:
                ...
                raise ValueError('Design model must be callable! Found object: %s' % self._model)
        self._reset()
    ...
    def set_states(self, states):
        self._name, self._configs, self._history, self._training_log, self._gmm_threshold, self._gene_dim, self._prot_dim, self._is_fitted, self._ps, self._trained_n_epoch, self._batch_size, self._learning_rate, self._n_mcmc_train, self._corruption_rate, self._corruption_dist = states

    def __getstate__(self):
        if not self.is_keras_model:
            model = N.serialize(nnops=(self._model), path=None, save_variables=True,
              binary_output=True)
        else:
            model = self._model.get_weights()
        return (
         self.get_states(), model)
    ...
    def __setstate__(self, states):
        ...
        if not self.is_keras_model:
            ...
            for p in self.configs['module_path']:
                m, e = get_module_from_path(identifier=(self.configs['module_name']),
                  path=p,
                  prefix='models_',
                  return_error=True)
                found += m
    ...
    def _initialize_placeholders_and_outputs(self):
        if self.input_plhs is not None:
            ...
            with tf.variable_scope(self.name + '_placeholders'):
                ...
                self.n_mcmc_samples_var = K.variable(1, dtype='int32', name='n_mcmc_samples',
                  initialize=True)
                self.n_epochs_var = K.variable((self._trained_n_epoch), dtype='float32', name='n_epochs',
                  initialize=True)
            ...
            input_dicts = {'X':X_plh, 
             'T':T_plh,  'L':L_plh, 
             'L_mean':L_mean_plh,  'L_var':L_var_plh,  'mask':mask_plh, 
             'y':y_plh,  'nsample':self.n_mcmc_samples_var, 
            ...
            assert sorted(list(outputs_train.keys())) == sorted(list(outputs_test.keys())), 'Output for training and testing must be the same set of Tensor'
            assert 'Z' in outputs_train and 'Z' in outputs_test, 'Latent space must be in the outputs'
            assert 'loss' in outputs_train and 'loss' in outputs_test, 'Loss must be given in the outputs for training'
            if self.is_semi_supervised:
                assert 'y' in outputs_test, 'Require output `y` if model is semi-supervised'
        for output_idx, (output_type, outputs) in enumerate(zip(('train', 'test'), (outputs_train, outputs_test))):
            ...
            assert Z.get_shape().as_list()[(-1)] == int(self.zdim), 'Expect %d latent dimension but return Z with shape: %s' % (
             int(self.zdim), Z.get_shape().as_list())
            loss = copy_keras_metadata(outputs['loss'], tf.identity((outputs['loss']), name='loss'))
            ...
            for m in metr:
                if m.shape.ndims != 0:
                    raise RuntimeError('Returned metrics must be scalar, but given: %s' % str(m))
            ...
            self._outputs[output_type] = {'z':Z,  'z_sample':Z_sample,  'pi':pi, 
             'y':y, 
             'y_sample':y_sample,  'l':L_expected, 
             'l_sample':L_sample,  'l_stddev':L_stddev,  'w':W_expected, 
             'w_sample':W_sample,  'w_stddev':W_stddev,  'v':V_expected, 
             'v_sample':V_sample,  'v_stddev':V_stddev,  'loss':loss, 
             'metr':metr}
    ...
    def _initialize_predict_functions(self):
        if len(self._pred_functions) > 0:
            ...
            metrics = sorted((set(as_list(self._outputs['test']['loss']) + as_list(self._outputs['test']['metr']))),
              key=(lambda x: x.name))
            self._metrics_name = [_normalize_tensor_name(i.name) for i in metrics]

            def output_iter():
                for name, tensor in chain(self._outputs['test'].items(), self._other_outputs['test'].items()):
            ...
            if self.is_keras_model:
                ...
                for name, tensor in output_iter():
                    m = Model(inputs=inputs, outputs=tensor, name=('predict_%s' % name))
                    if '_sample' in name:
                ...
                self._pred_functions['metrics'] = partial((Model(inputs=[i for i in self.input_plhs if i is not None], outputs=metrics,
                  name='predict_metrics').predict),
                  batch_size=PREDICTION_BATCH_SIZE,
                  verbose=0)
                if self.is_semi_supervised:
                    m = Model(inputs=inputs, outputs=(self._outputs['test']['y']),
                      name='predict_y')
                    self._pred_functions['y'] = partial((m.predict),
            ...
            def create_func(ins_, out_, is_scoring=False):
                if isinstance(out_, (tuple, list)):
                    ...
                    if len(out_) == 0:
                        out_ = None
                if out_ is not None:
                    fn = K.function(inputs=ins_,
                      outputs=out_,
                      training=False)

                    def _batch_prediction(X):
                        if not len(set(x.shape[0] for x in X)) == 1:
                            raise AssertionError
                        else:
                            n = X[0].shape[0]
                            all_y = []
                            for start, end in batching(batch_size=PREDICTION_BATCH_SIZE, n=n,
                              seed=None):
                            ...
                            if not is_scoring:
                                if isinstance(y, (tuple, list)):
                                    return [np.concatenate([j[i] for j in all_y], axis=0) for i in range(len(y))]
                                else:
            ...
            self._pred_functions['metrics'] = create_func(ins_=[i for i in self.input_plhs if i is not None],
              out_=metrics,
              is_scoring=True)
            if self.is_semi_supervised:
                self._pred_functions['y'] = create_func(ins_=inputs,
    ...
    def normalize(self, x, method, data_name):
        ...
        if method == 'raw':
            ...
            if method == 'log':
                ...
                if method == 'bin' or method == 'prob':
                    is_binary_classes = sorted(np.unique(x.astype('float32'))) == [0.0, 1.0]
    ...
    """
        if not X.shape[1] == self.gene_dim:
            raise AssertionError('This model require %d gene expression, but given X.shape=%s' % (
             self.gene_dim, str(X.shape)))
        elif self.is_semi_supervised:
            if y is not None:
                assert y.shape[1] == self.prot_dim, 'This model require %d protein markers, but given y.shape=%s' % (
    ...
    def fit(self, X, y=None, supervised_percent=0.8, validation_percent=0.1, n_mcmc_samples=1, corruption_rate=0.25, corruption_dist='binomial', batch_size=64, n_epoch=120, learning_rate=0.0001, monitoring=False, fig_dir=None, detail_logging=False):
        """
    ...
    """
        ...
        if self._corruption_rate is None:
            ...
            if not (self._corruption_rate == corruption_rate and self._corruption_dist == corruption_dist):
                raise AssertionError('Model have been fitted with following corruption configurations: %s, %s;but given: %s, %s' % (
                 self._corruption_rate, self._corruption_dist,
                ...
                X = apply_artificial_corruption(X,
                  dropout=corruption_rate, distribution=corruption_dist)
                if self.is_semi_supervised:
                    if y is None:
                        raise ValueError('`y` must be not None for fitting semi-supervised model')
                X, T, L, L_mean, L_var, y = self._preprocess_inputs(X, y)
                X_train, T_train, L_train, Lmean_train, Lvar_train, y_train = (None,
                                                                               None,
                                                                               None,
                                                                               None,
                                                                               None,
                                                                               None)
                X_valid, T_valid, L_valid, Lmean_valid, Lvar_valid, y_valid = (None,
                                                                               None,
                                                                               None,
                                                                               None,
                                                                               None,
                ...
                if validation_percent > 0:
                    train_ids, valid_ids = train_valid_test_split(x=np.arange(n_samples, dtype='int32'),
                ...
                supervised_indices = rand.choice((np.arange(0, n_train)),
                ...
                inputs = [i for i in self.input_plhs if i is not None]
                ...
                assert int(n_mcmc_samples) >= 1, 'Number of MCMC samples must be >= 1'
            K.set_value(self.n_mcmc_samples_var, int(n_mcmc_samples))
            if self.is_keras_model:
                from tensorflow.python.keras import Model
                from tensorflow.python.keras.optimizers import Adam
                if self.is_semi_supervised:
                    model = Model(inputs=inputs, outputs=(KerasConcatenate(axis=(-1))([self._outputs['train']['w'],
                     self._outputs['train']['y']])))
                else:
                    model = Model(inputs=inputs, outputs=(self._outputs['train']['w']))
                ...
                model.compile(optz, loss=(to_keras_objective(self._outputs['train']['loss'])),
                  metrics=[to_keras_objective(i) for i in self._outputs['train']['metr']])
                hist = model.fit(x=train_data, y=None, batch_size=batch_size,
                  epochs=n_epoch,
                  verbose=(1 if detail_logging else 0),
                  callbacks=[
                 LambdaCallback(on_epoch_begin=(lambda *args, **kwargs: K.set_value(self.n_epochs_var, self.n_epochs_var + 1)))],
                  validation_data=(
                 valid_data, None),
                  shuffle=True)
                ...
                optz = K.optimizers.Adam(lr=(float(learning_rate)), clipnorm=None, name=(self.name))
                updates = optz.minimize((self._outputs['train']['loss']), verbose=(is_verbose()))
                global_norm = optz.norm
                K.initialize_all_variables()
                tmp = [
                 self._outputs['train']['loss'], global_norm]
                for i in self._outputs['train']['metr']:
                    if i not in tmp:
                        tmp.append(i)

                f_train = K.function(inputs=inputs, outputs=tmp, training=True, updates=updates)
                tmp = [
                 self._outputs['train']['loss']]
                for i in self._outputs['train']['metr']:
                    if i not in tmp:
                        tmp.append(i)

                f_score = K.function(inputs=inputs, outputs=tmp, training=False, batch_size=None)
                overlap_metrics = sorted((set([self._outputs['train']['loss']] + self._outputs['train']['metr'])),
                  key=(lambda x: x.name))
                if is_verbose():
                ...
                trainer = training.MainLoop(batch_size=(int(batch_size)), seed=UNIVERSAL_RANDOM_SEED,
                  shuffle_level=2,
                ...
                all_callbacks = [
                 training.LambdaCallback(fn=(lambda : K.set_value(self.n_epochs_var, self.n_epochs_var + 1)),
                   task_name='train',
                   signal=(training.TaskSignal.EpochStart)),
                 training.NaNDetector(),
                 training.CheckpointGeneralization('valid', output_name=(self._outputs['train']['loss'])),
                 None if fig_dir is None else LearningCurves(save_path=fig_dir),
                 training.LambdaCallback(fn=(self.monitoring_epoch), task_name='train', signal=(training.TaskSignal.EpochEnd)) if bool(monitoring) else None]
    ...
    def plot_tsne(self, X, y=None, labels=None, labels_name=None, n_samples=1000, n_mcmc_samples=100, output_type='Z', show_pca=False, ax=None, title=None, seed=5218):
        """
    Parameters
    ----------
    output_type : string
    ...
    """
        ...
        if labels is None:
            ...
            if np.max(labels) > 1:
                if labels.shape[1] == 1:
                    labels = one_hot(y=(labels.ravel()), nb_classes=(np.max(labels) + 1 if labels_name is None else len(labels_name)))
            assert len(labels) == X.shape[0], 'Number of samples mismatch between X and labels'
        ...
        if isinstance(Z, (tuple, list)):
            ...
            if Z is None:
                raise RuntimeError("Model has no support for output type: '%s'" % output_type)
            is_single_label = np.allclose(np.max(labels, axis=1), 1)
            Z_pca = fast_pca(Z, n_components=(min(512, Z.shape[1])), algo='pca', random_state=(rand.randint(1000000000.0)))
            if not bool(show_pca):
                Z_tsne = fast_tsne(Z_pca, n_components=2, random_state=(rand.randint(1000000000.0)))
            else:
                Z_tsne = None
        from odin.visual import plot_scatter, plot_figure, plot_scatter_heatmap
        if is_single_label:
            plot_scatter(x=(Z_pca[:, :2] if bool(show_pca) else Z_tsne), color=[labels_name[i] for i in np.argmax(labels, axis=(-1))],
              size=6,
              ax=ax,
              grid=False,
              legend_enable=True,
            ...
            for i, name in enumerate(labels_name):
                ...
                plot_scatter_heatmap(x=(Z_pca[:, :2] if bool(show_pca) else Z_tsne),
                  val=(val / np.sum(val)),
                  ax=(
                 nrow, ncol, i + 1),
                  colormap=colormap,
    ...
    """
        if not self.is_semi_supervised:
            y = None
        scores = self._make_prediction('metrics',
          X=X, y=y, n_mcmc_samples=n_mcmc_samples)
        if self.is_keras_model:
            scores = [np.mean(i) for i in scores]
        return {_normalize_tensor_name(name):s for name, s in zip(self._metrics_name, scores)}

    def _sample(self, name, X, n_mcmc_samples):
        output = []
        for s, e in batching(batch_size=PREDICTION_BATCH_SIZE, n=(X.shape[0])):
            x = X[s:e]
            output.append(self._make_prediction(pred_type=name, X=x, n_mcmc_samples=n_mcmc_samples))
    ...
    def sample_y(self, X, n_mcmc_samples=1):
        assert self.is_semi_supervised, 'Only semi-supervised model has this prediction!'
    ...
    def predict_y(self, X, n_mcmc_samples=100):
        ...
        assert self.is_semi_supervised, 'Only semi-supervised model has this prediction!'
        return self._make_prediction(pred_type='y',
          X=X,
          y=None,
    ...
    def predict_L(self, X, n_mcmc_samples=100):
        ...
        try:
            ...
            return np.sum(self._make_prediction('V', X, n_mcmc_samples=n_mcmc_samples), axis=(-1),
              keepdims=True)
    ...
    def predict_Lstddev(self, X, n_mcmc_samples=100):
        ...
        try:
            ...
            return np.sum(self._make_prediction('V_stddev', X, n_mcmc_samples=n_mcmc_samples), axis=(-1),
              keepdims=True)
    ...
    def id(self):
        ...
        name = self.configs['module_name'] if 'module_name' in self.configs else self.name
...
class InferenceSCVAE(Inference):
    ...
    def __init__(self, gene_dim, dispersion='gene-cell', xnorm='log', tnorm='raw', ynorm='prob', xclip=0, yclip=0, xdist='zinb', ydist='bernoulli', zdist='normal', xdrop=0.3, edrop=0, zdrop=0, ddrop=0, hdim=128, zdim=32, nlayer=2, batchnorm=True, analytic=True, kl_weight=1.0, warmup=400, y_weight=10.0, extra_module_path=None, **kwargs):
        (super(InferenceSCVAE, self).__init__)(gene_dim=gene_dim, prot_dim=None, model='vae', 
         dispersion=dispersion, xnorm=xnorm, 
         tnorm=tnorm, ynorm=ynorm, xclip=xclip, 
         yclip=yclip, xdist=xdist, 
         ydist=ydist, zdist=zdist, xdrop=xdrop, 
         edrop=edrop, zdrop=zdrop, ddrop=ddrop, hdim=hdim, 
         zdim=zdim, nlayer=nlayer, batchnorm=batchnorm, 
         analytic=analytic, kl_weight=kl_weight, 
         warmup=warmup, y_weight=y_weight, extra_module_path=extra_module_path, **kwargs)


class InferenceSISUA(Inference):

    def __init__(self, gene_dim, prot_dim, dispersion='gene-cell', xnorm='log', tnorm='raw', ynorm='prob', xclip=0, yclip=0, xdist='zinb', ydist='bernoulli', zdist='normal', xdrop=0.3, edrop=0, zdrop=0, ddrop=0, hdim=128, zdim=32, nlayer=2, batchnorm=True, analytic=True, kl_weight=1.0, warmup=400, y_weight=10.0, extra_module_path=None, **kwargs):
        (super(InferenceSISUA, self).__init__)(gene_dim=gene_dim, prot_dim=prot_dim, model='movae', 
         dispersion=dispersion, xnorm=xnorm, 
         tnorm=tnorm, ynorm=ynorm, xclip=xclip, 
         yclip=yclip, xdist=xdist, 
         ydist=ydist, zdist=zdist, xdrop=xdrop, 
