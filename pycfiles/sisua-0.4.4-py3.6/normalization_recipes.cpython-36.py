...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/sisua/data/normalization_recipes.py
# Compiled at: 2019-09-23 09:39:23
# Size of source mod 2**32: 11983 bytes
...
class Methods(Flag):
    ...
    def apply(self, X, min_counts_per_gene=1, min_cells_per_gene=None, min_counts_per_cell=1, min_genes_per_cell=None, min_disp=0.5, flavor='seurat', n_top_genes=None, target_sum=None, max_value=None, gene_subset=None, trained_embedding=None):
        assert isinstance(X, SingleCellOMIC), 'X must be instance of sisua.data.SingleCellOMIC'
        is_logarithmized = False
        for method in self:
            if method == Methods.CELL_COUNT:
                X.filter_cells(min_counts=min_counts_per_cell, min_genes=min_genes_per_cell,
                  inplace=True)
            else:
                if gene_subset is None:
                    if method == Methods.GENE_COUNT:
                        X.filter_genes(min_counts=min_counts_per_gene, min_cells=min_cells_per_gene,
                          inplace=True)
                    ...
                    if method == Methods.GENE_DISP:
                        ...
                        X.filter_highly_variable_genes(min_disp=min_disp, n_top_genes=n_top_genes,
                          flavor=flavor,
                          inplace=True)
                else:
                    if method == Methods.TOTAL_COUNTS:
            ...
            if method == Methods.EXP:
                ...
                if method == Methods.LOG:
                    if not is_logarithmized:
                        X.normalize(log1p=True, inplace=True)
                    else:
                        if method == Methods.SCALE:
...
class NormalizationRecipe(object):
    __doc__ = ' Predefined normalization recipe\n\n  Note\n  ----\n  It is recommended to enable corruption within this normalization to\n  simulate actual conditions\n  '

    def __init__(self, methods=Methods.RAW, corruption_rate=0.25, corruption_dist='binomial', target_sum=None, max_value=None, min_counts_per_gene=1, min_cells_per_gene=None, min_counts_per_cell=1, min_genes_per_cell=None, min_disp=0.5, flavor='seurat', n_top_genes=None):
        ...
        assert isinstance(self.methods, Methods), 'methods must be instance of sisua.data.normalization_recipes.Methods'
        ...
        if isinstance(X, SingleCellOMIC):
            ...
            if training:
                if self.corruption_rate > 0:
                    X.corrupt(corruption_rate=(self.corruption_rate), corruption_dist=(self.corruption_dist),
                      inplace=True)
            X = self.methods.apply(X, min_counts_per_gene=(self.min_counts_per_gene),
...
class Seurat(NormalizationRecipe):
    __doc__ = 'Normalization and filtering as of Seurat [Satija15]_.\n  This uses a particular preprocessing.\n  Expects non-logarithmized data.\n\n  Note\n  ----\n  It is recommended to enable corruption within this normalization to\n  simulate actual conditions\n  '

    def __init__(self, corruption_rate=0.25, corruption_dist='binomial', min_cells=3, min_genes=200, log=True):
        super(Seurat, self).__init__(methods=(Methods.CELL_COUNT | Methods.GENE_COUNT | Methods.TOTAL_COUNTS | Methods.GENE_DISP | (Methods.LOG if log else Methods.EXP) | Methods.SCALE),
          target_sum=10000.0,
          max_value=10,
...
class CellRanger(NormalizationRecipe):
    __doc__ = 'Normalization and filtering as of [Zheng17]_.\n  Reproduces the preprocessing of [Zheng17]_ - the Cell Ranger R Kit of 10x\n  Genomics.\n  Expects non-logarithmized data.\n\n  The procedure is as follow\n\n  References\n  ----------\n  pass\n\n  Note\n  ----\n  It is recommended to enable corruption within this normalization to\n  simulate actual conditions\n  '

    def __init__(self, corruption_rate=0.25, corruption_dist='binomial', min_counts=1, n_top_genes=1000, log=True):
        super(CellRanger, self).__init__(methods=(Methods.GENE_COUNT | Methods.TOTAL_COUNTS | Methods.GENE_DISP),
          target_sum=None,
          max_value=None,
...
class Sisua(NormalizationRecipe):
    ...
    def __init__(self, corruption_rate=0.25, corruption_dist='binomial', min_counts=1, total_counts=True, n_top_genes=1000):
        methods = Methods.CELL_COUNT | Methods.GENE_COUNT | Methods.TOTAL_COUNTS | (Methods.LOG if n_top_genes is None else Methods.GENE_DISP | Methods.LOG) | Methods.SCALE
        if not total_counts:
            methods = methods ^ Methods.TOTAL_COUNTS
        super(Sisua, self).__init__(methods=methods, target_sum=10000.0,
          max_value=None,
          min_counts_per_gene=min_counts,
          min_cells_per_gene=None,
