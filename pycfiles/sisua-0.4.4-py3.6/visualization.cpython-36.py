...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/sisua/utils/visualization.py
# Compiled at: 2019-09-23 05:02:07
# Size of source mod 2**32: 31416 bytes
from __future__ import absolute_import, division, print_function
import os
from collections import OrderedDict
import numpy as np, pandas as pd, seaborn as sbn, tensorflow as tf
from sklearn.metrics import confusion_matrix, explained_variance_score, f1_score, mean_absolute_error, mean_squared_error, r2_score
from odin import backend as K
from odin.ml import fast_pca, fast_tsne
from odin.utils import async_mpi, catch_warnings_ignore, ctext, one_hot
from odin.visual import generate_random_colors, generate_random_marker, plot_confusion_matrix, plot_figure, plot_gridSpec, plot_gridSubplot, plot_histogram, plot_save, plot_scatter, plot_scatter_heatmap, plot_scatter_layers, plot_series_statistics, subplot
...
def downsample_data(*X):
    ...
    if not len(_) == 1:
        raise AssertionError('Inconsistent shape[0] for X and y')
    else:
        num_samples = _[0]
        _RAND = np.random.RandomState(seed=87654321)
        if num_samples > 8000:
            print('[Warning] Given: %s; downsample to 8000 samples' % ctext(', '.join([str(x.shape) for x in X if x is not None]), 'cyan'))
...
def show_image(x, is_probability=False):
    ...
    x = anything2image(x)
    if x.shape[0] > 32:
        x = tf.nn.max_pool((np.reshape(x, (1, x.shape[0], x.shape[1], 1))), ksize=(1,
                                                                                   4,
                                                                                   4,
                                                                                   1),
          strides=(1, 4, 4, 1),
    ...
    plt.imshow(x, interpolation='nearest',
      cmap=(plt.cm.Greys_r),
      vmin=(0.0 if is_probability else None),
      vmax=(1.0 if is_probability else None))
    plt.xticks(())
    ax.set_xticklabels([])
    plt.yticks(())
    ax.set_yticklabels([])
...
def fast_scatter(x, y, labels, title, azim=None, elev=None, ax=None, enable_legend=False, size=18, fontsize=12):
    y = np.squeeze(y)
    if y.ndim == 1:
        pass
    else:
...
def save_figures(figures, path, dpi=None, separate_files=True, clear_figures=True):
    ...
    elif not isinstance(figures, dict):
        raise AssertionError('figures must be dictionary mapping from figure name to matplotlib.Figure')
...
def plot_evaluate_reconstruction(X, W, y_raw, y_prob, X_row, X_col, labels, title, pi=None, enable_image=True, enable_tsne=True, enable_sparsity=True):
    """
  ...
  """
    ...
    if isinstance(W, (tuple, list)):
        ...
        assert X.shape[0] == W.shape[0] == y_raw.shape[0] and X.shape == W.shape and y_raw.shape == y_prob.shape
    ...
    if enable_image:
        ...
        count = 1
        all_ids = _RAND.choice((np.arange(0, X.shape[0])), size=(n_img * n_img_row),
          replace=False)
    ...
    if enable_tsne:
        ...
        X_cell_pca, X_cell_tsne, W_cell_pca, W_cell_tsne = pca_and_tsne(X_log, W_log)
        X_gene_pca, X_gene_tsne, W_gene_pca, W_gene_tsne = pca_and_tsne(X_log.T, W_log.T)
        n_plot = 5
        if pi is not None:
            n_plot += 2
        plot_figure(nrow=(n_plot * 5), ncol=18)
        fast_scatter(x=X_cell_pca, y=y_argmax,
          labels=labels,
          title='[PCA]Original Cell Data',
          ax=(
         n_plot, 2, 1),
          enable_legend=False)
        fast_scatter(x=W_cell_pca, y=y_argmax,
          labels=labels,
          title='[PCA]Reconstructed Cell Data',
          ax=(
         n_plot, 2, 2),
          enable_legend=False)
        fast_scatter(x=X_cell_tsne, y=y_argmax,
          labels=labels,
          title='[t-SNE]Original Cell Data',
          ax=(
         n_plot, 2, 3),
          enable_legend=True)
        fast_scatter(x=W_cell_tsne, y=y_argmax,
          labels=labels,
        ...
        plot_scatter_heatmap(x=X_cell_tsne,
    ...
    if enable_sparsity:
        ...
        plot_confusion_matrix(ax=None, cm=confusion_matrix(y_true=z, y_pred=z_res,
          labels=(0, 1)),
...
def plot_evaluate_classifier(y_pred, y_true, labels, title, show_plot=True, return_figure=False, figsize=None):
    """
  Parameters
  ----------
  ...
  """
    ...
    for i, (name, pred, true) in enumerate(zip(labels, y_pred.T, y_true.T)):
        ...
        if show_plot:
            plot_confusion_matrix(confusion_matrix(y_true=true, y_pred=pred), labels=[
             0, 1],
              fontsize=fontsize,
              ax=(
             nrow, ncol, i + 1),
    ...
    results = dict(F1micro=(f1_micro * 100),
      F1macro=(f1_macro * 100),
      F1weight=(f1_weight * 100))
...
def plot_evaluate_regressor(y_pred, y_true, labels, title):
    ...
    def plot_hist(hist, ax, name):
        count, bins = plot_histogram(true, bins=nbins,
          ax=ax,
          title=name,
          fontsize=fontsize)
        plt.xlim((np.min(bins), np.max(bins)))
        plt.xticks(np.linspace(start=(np.min(bins)), stop=(np.max(bins)),
          num=8,
          dtype='int32'),
          fontsize=6)
...
@async_mpi
def plot_monitoring_epoch(X, X_drop, y, Z, Z_drop, W_outputs, W_drop_outputs, pi, pi_drop, row_name, dropout_percentage, curr_epoch, ds_name, labels, save_dir):
    from matplotlib import pyplot as plt
    if y.ndim == 2:
        y = np.argmax(y, axis=(-1))
    ...
    if W_outputs is not None:
        X_pca, X_pca_drop, W_pca, W_pca_drop = fast_pca(X, X_drop,
          (W_outputs[0]),
          (W_drop_outputs[0]),
          n_components=2,
          random_state=5218)
    ...
    if n_test_samples > 8000:
        ...
        if W_outputs is None:
            ...
            fast_scatter(x=X_pca_drop, y=y, 
             ...
             ax=(2, 3, 4), **config)
            fast_scatter(x=W_pca_drop, y=y, 
             title='Reconstructed', 
             ax=(2, 3, 5), 
             enable_legend=True, **config)
            fast_scatter(x=Z_pca_drop, y=y, 
             title='Latent space', 
             ax=(2, 3, 6), **config)
    plot_save((os.path.join(save_dir, 'latent_epoch%d.png') % curr_epoch), dpi=180,
      clear_all=True,
      log=True)
    if W_outputs is not None:
        ...
        plot_figure(nrow=23, ncol=18)
        with plot_gridSpec(nrow=(3 * (2 if pi is None else 3) + 12 + 1), ncol=6,
          wspace=1.0,
          hspace=0.8) as (grid):
            kws = dict(colorbar=True, fontsize=10, size=10, marker=y, n_samples=1200)
            ax = subplot(grid[:3, 0:3])
            plot_scatter_heatmap(x=X_pca, val=X_countsum, 
             ax=ax, 
             legend_enable=False, 
             title='Original data (Count-sum)', **kws)
            ax = subplot(grid[:3, 3:6])
            plot_scatter_heatmap(x=W_pca, val=W_countsum, 
             ax=ax, 
             legend_enable=False, 
             title='Reconstruction (Count-sum)', **kws)
            ax = subplot(grid[3:6, 0:3])
            plot_scatter_heatmap(x=X_pca_drop, val=X_drop_countsum, 
             ax=ax, 
             legend_enable=True if pi is None else False, 
             legend_ncol=len(labels), 
             title='[Dropped:%s]Original data (Count-sum)' % dropout_percentage_text, **kws)
            ax = subplot(grid[3:6, 3:6])
            plot_scatter_heatmap(x=W_pca_drop, val=W_drop_countsum, 
             ax=ax, 
             legend_enable=False, 
             title='[Dropped:%s]Reconstruction (Count-sum)' % dropout_percentage_text, **kws)
            row_start = 6
            if pi is not None:
                ax = subplot(grid[6:9, 0:3])
                plot_scatter_heatmap(x=X_pca, val=pi_sum, 
                 ax=ax, 
                 legend_enable=True, 
                 legend_ncol=len(labels), 
                 title='Zero-inflated probabilities', **kws)
                ax = subplot(grid[6:9, 3:6])
                plot_scatter_heatmap(x=X_pca, val=pi_drop_sum, 
                 ax=ax, 
                 legend_enable=False, 
                 title='[Dropped:%s]Zero-inflated probabilities' % dropout_percentage_text, **kws)
                row_start += 3
 ...
 count_sum_observed,
 count_sum_expected, explained_stdev=count_sum_stdev_explained, 
    ...
    if W_outputs is not None:
        if len(W_outputs) == 3:
            ...
            col_width = 5
            with plot_gridSpec(nrow=(3 * n_visual_samples), ncol=(4 * col_width),
              wspace=5.0,
              hspace=1.0) as (grid):
                curr_grid_index = 0
                for i in rand.permutation(len(X))[:n_visual_samples]:
                    ...
                    for j, kws in enumerate(series_config):
                        ax = subplot(grid[
                         curr_grid_index:curr_grid_index + 3, j * col_width:j * col_width + col_width])
                        ax, handles, indices = plot_series_statistics(
 observed,
 expected, explained_stdev=stdev_explained, 
                         total_stdev=stdev_total, 
                         fontsize=8, 
                         legend_enable=False, 
                         despine=True if pi is None else False, 
                         title="'%s' X_original - W_original" % row_name[i] if j == 0 else None, 
                         return_handles=True, 
                         return_indices=True, **kws)
                        if pi is not None:
                    ...
                    for j, kws in enumerate(series_config):
                        ...
                        ax = subplot(grid[
                         curr_grid_index:curr_grid_index + 3,
                         col_start + j * col_width:col_start + j * col_width + col_width])
                        ax, handles, indices = plot_series_statistics(
 observed,
 expected_drop, explained_stdev=stdev_explained_drop, 
                         total_stdev=stdev_total_drop, 
                         fontsize=8, 
                         legend_enable=False, 
                         despine=True if pi is None else False, 
                         title="[Dropped:%s]'%s' X_original - W_dropout" % (dropout_percentage_text, row_name[i]) if j == 0 else None, 
                         return_handles=True, 
                         return_indices=True, **kws)
                        if pi is not None:
            ...
            plot_save((os.path.join(save_dir, 'samples_epoch%d.png') % curr_epoch), dpi=180,
              clear_all=True,
              log=True)
    if 'mnist' in ds_name:
        if W_outputs is not None:
            ...
            n_images = 32
            ids = rand.choice(np.arange((X.shape[0]), dtype='int32'), size=n_images,
              replace=False)
            ...
            plot_save((os.path.join(save_dir, 'image_epoch%d.png') % curr_epoch), dpi=180,
              clear_all=True,
              log=True)