...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/sisua/inference/inference_dca.py
# Compiled at: 2019-07-19 04:40:47
# Size of source mod 2**32: 12521 bytes
...
def dca_normalize(X, log=True):
    ...
    with catch_warnings_error(Warning):
        adata = normalize((AnnData(X)), filter_min_counts=False,
          size_factors=True,
          normalize_input=True,
          logtrans_input=log1p)
    return (
     adata.X, adata.obs.size_factors.values)


def dca_train(X, network, log, learning_rate, epochs=300, reduce_lr=10, early_stop=15, batch_size=32, validation_split=0.1, verbose=True):
    ...
    model = network.model
    loss = network.loss
    optimizer = opt.__dict__['adam'](lr=learning_rate, clipvalue=5.0)
    ...
    output = adata.raw.X
    loss = model.fit(inputs, output, epochs=epochs,
...
class InferenceDCA(Inference):
    ...
    def _initialize_model(self, weights=None):
        ...
        self._ae = Autoencoder(input_size=(self.gene_dim),
          output_size=None,
          hidden_size=((
         self.hdim,) * self.nlayer + (self.zdim,) + (self.hdim,) * self.nlayer),
    ...
    def fit(self, X, y=None, supervised_percent=0.8, validation_percent=0.1, n_mcmc_samples=1, corruption_rate=0.25, corruption_dist='binomial', batch_size=64, n_epoch=120, learning_rate=0.0001, monitoring=False, fig_dir=None, detail_logging=False):
        ...
        if self._corruption_rate is None:
            ...
            assert self._corruption_rate == corruption_rate and self._corruption_dist == corruption_dist, 'Model have been fitted with following corruption configurations: %s, %s;but given: %s, %s' % (
             self._corruption_rate, self._corruption_dist,
             corruption_rate, corruption_dist)
            assert n_mcmc_samples == 1, 'Only support 1 MCMC sample during training for scVI'
    ...
    """
        ...
        loss = self._ae.model.evaluate({'count':X, 
         'size_factors':L},
          y=X, batch_size=PREDICTION_BATCH_SIZE, verbose=0)
        return {'loss': loss}
    ...
    (i.e. mean of Normal distribution) """
        X, L = dca_normalize(X, log=(self.xnorm == 'log'))
        return self._ae.encoder.predict({'count':X, 
         'size_factors':L},
    ...
    """
        ...
        if 'pi' not in self._ae.extra_models:
            ...
            pi = self.predict_PI(X, n_mcmc_samples)
            w_mean = (1 - pi) * v
            return np.sqrt((1 - pi) * (v_stddev + v ** 2) - w_mean ** 2)

    def predict_V(self, X, n_mcmc_samples=100):
        """ Return mean of the denoised value """
        X, L = dca_normalize(X, log=(self.xnorm == 'log'))
        return self._ae.model.predict({'count':X, 
         'size_factors':L},
          batch_size=PREDICTION_BATCH_SIZE)
    ...
    """
        X, L = dca_normalize(X, log=(self.xnorm == 'log'))
        return self._ae.extra_models['dispersion'].predict({'count':X, 
         'size_factors':L},
          batch_size=PREDICTION_BATCH_SIZE)
    ...
    """
        ...
        X, L = dca_normalize(X, log=(self.xnorm == 'log'))
        return self._ae.extra_models['pi'].predict({'count':X, 
         'size_factors':L},
          batch_size=PREDICTION_BATCH_SIZE)
