...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/sisua/analysis/results_sheet.py
# Compiled at: 2019-08-15 10:12:53
# Size of source mod 2**32: 25701 bytes
...
from odin.utils import as_tuple, cache_memory, catch_warnings_ignore, ctext, flatten_list, md5_checksum
from odin.visual import plot_aspect, plot_confusion_matrix, plot_figure, plot_frame, plot_save, plot_scatter, plot_scatter_heatmap, to_axis2D
...
from sisua.analysis.latent_benchmarks import clustering_scores, plot_distance_heatmap, plot_latents_binary, plot_latents_multiclasses
...
class ResultsSheet(object):
    ...
    def __init__(self, *posteriors, verbose=True):
        ...
        if not len(posteriors) > 0:
            raise AssertionError('No posteriors for analysis!')
        elif not all(isinstance(i, Posterior) for i in posteriors):
    ...
    def save_scores(self, outpath, scores=[
 'imputation', 'classifier', 'cluster',
 'spearman', 'pearson']):
        ...
        if ext == '':
            ...
            if ext != '.html':
                raise ValueError("Only support excel with extension: html; but given '%s'" % ext)
            scores = as_tuple(scores, t=string_types)
            assert all(i in ResultsSheet.ALL_SCORE_TYPES for i in scores), 'Support score type %s but given %s' % (
    ...
    def plot_comparison_f1(self, test=True, model_id=lambda m: m.name, fig_width=12):
        ...
        fig, subplots = plt.subplots(nrows=1, ncols=n_system, sharey=True,
          squeeze=True,
          figsize=(
         int(fig_width), 2))
        for idx, (name, f1) in enumerate(scores):
            ...
            ax.plot((np.arange(n_labels)), [f1_weight for i in range(n_labels)], linewidth=1.2,
              linestyle=':',
              color='black',
              label=('$\\overline{F1}$:%.1f' % f1_weight))
        ...
        plt.axis('off')
        plt.legend(ncol=(int(np.ceil(len(scores_name) / 2))), scatterpoints=1,
    ...
    def _bar_box_line(self, title, ylabel, get_score, model_id, ax, ignore=[], using_bar=True):
        start_time = time.time()
        if not callable(model_id):
            raise AssertionError('model_id must be callable')
        else:
    ...
    def boxplot_cluster(self, model_id=lambda m: m.name, ax=None):
        return self._bar_box_line(title='boxplot_cluster', ylabel='Pooled Cluster Metrics', get_score=(lambda x: x.scores_clustering),
          model_id=model_id,
          ax=ax,
          using_bar=False)

    def boxplot_f1(self, model_id=lambda m: m.name, ax=None):
        return self._bar_box_line(title='boxplot_classifier', ylabel='F1 score', get_score=(lambda x: x.scores_classifier),
          model_id=model_id,
          ax=ax,
          using_bar=False,
          ignore=[
         'F1weight', 'F1micro', 'F1macro'])

    def boxplot_pearson(self, model_id=lambda m: m.name, ax=None):
        return self._bar_box_line(title='boxplot_pearson', ylabel='Pearson correlation', get_score=(lambda x: x.scores_pearson),
          model_id=model_id,
          ax=ax,
          using_bar=False)

    def boxplot_spearman(self, model_id=lambda m: m.name, ax=None):
        return self._bar_box_line(title='boxplot_spearman', ylabel='Spearman correlation', get_score=(lambda x: x.scores_spearman),
          model_id=model_id,
          ax=ax,
          using_bar=False)

    def barplot_cluster(self, model_id=lambda m: m.name, ax=None):
        return self._bar_box_line(title='barplot_cluster', ylabel='Pooled Cluster Metrics', get_score=(lambda x: x.scores_clustering),
          model_id=model_id,
          ax=ax,
          using_bar=False)

    def barplot_f1(self, model_id=lambda m: m.name, ax=None):
        return self._bar_box_line(title='barplot_classifier', ylabel='F1 score',
    ...
    def barplot_pearson(self, model_id=lambda m: m.name, ax=None):
        return self._bar_box_line(title='barplot_pearson', ylabel='Pearson correlation',
          get_score=(lambda x: x.scores_pearson),
          model_id=model_id,
          ax=ax)

    def barplot_spearman(self, model_id=lambda m: m.name, ax=None):
        return self._bar_box_line(title='barplot_spearman', ylabel='Spearman correlation',
    ...
    def plot_correlation_marker_pairs(self, test=True, fontsize=8):
        ...
        for pos in self:
            ...
            if original_series is None:
                original_series = correlation_scores(X=x,
                  ...
                  return_series=True)
            imputed_series.append(correlation_scores(X=v,
              y=y,
              gene_name=(pos.gene_name),
              protein_name=(pos.labels),
              return_series=True))
        ...
        for row_idx, prot_gene in enumerate(original_series.keys()):
            ...
            for col_idx, (name, g) in enumerate(zip(system_name, gene)):
                ax = fig.add_subplot(grids[row_idx, width * col_idx:width * col_idx + width])
    ...
    def plot_imputation_scatter(self, test=True, pca=False, color_by_library=True):
        ...
        allV = [X_org, X_crr] + [pos.V_test if test else pos.V_train for pos in self.posteriors]
        if not (X_org.shape == X_crr.shape and all(v.shape == X_org.shape for v in allV)):
            raise AssertionError
        else:
            all_names = [
             '[%s]Original' % data_type,
             '[%s]Corrupted' % data_type] + [i.short_id_lines for i in self.posteriors]
            if len(X_org) > 5000:
                np.random.seed(5218)
        ...
        for idx, (name, v) in enumerate(zip(all_names, allV)):
            ...
            with catch_warnings_ignore(Warning):
                if color_by_library:
                    plot_scatter_heatmap(x=v, val=n, ax=ax, size=8, legend_enable=False,
                      grid=False,
                      title=name)
                else:
                    plot_scatter(x=v, color=[labels[i] for i in y],
        ...
        with catch_warnings_ignore(Warning):
            plt.tight_layout()
        self.add_figure('imputation_scatter_%s_%s' % ('lib' if color_by_library else 'cell',
         data_type), fig)
    ...
    def plot_latents_binary_scatter(self, test=True, pca=False):
        ...
        for idx, pos in enumerate(self):
            ax = plt.subplot(nrow, ncol, idx + 1)
            with catch_warnings_ignore(Warning):
                pos.plot_latents_binary_scatter(test=test, ax=ax, legend=(True if idx == 0 else False),
                  pca=pca)
    ...
    def plot_learning_curves(self):
        ...
        for row_idx, pos in enumerate(self):
            ...
            for col_idx, (name, i, j) in enumerate(zip(['loss', 'LLK_x', 'LLK_y', 'KLqp_x', 'KLqp_y'], train, valid)):
                col_idx += 1
                ...
                if i is None or j is None:
                    ...
                    plt.plot(i, linewidth=2.5, label=('train:%.2f' % (np.max(i) if 'LLK' in name else np.min(i))), linestyle='-')
                    plt.plot(j, linewidth=2.5, label=('valid:%.2f' % (np.max(j) if 'LLK' in name else np.min(j))), linestyle='--',
                      alpha=0.8)
                    plt.legend()

        plt.tight_layout()
        self.add_figure('learning_curves', fig)
        return self._log('plot_learning_curves %s(s)' % ctext(time.time() - start_time, 'lightyellow'))
    ...
    """
        ...
        fmt = '%.4f' if score_type in ('imputation', 'cluster') else '%.2f'
          ...
          'train', train_scores), ('test', test_scores))):
            ...
            for sc_idx, sc_name in enumerate(scores_name):
                ...
                for x, y in zip(xids, sc):
                    plt.text(x, y, (fmt % y), fontsize=score_text_size,
                      horizontalalignment='center')

            if idx == 0:
                plt.xticks(xticks, scores_name, rotation=0,
                  fontsize=main_text_size)
            else:
                plt.xticks(())
            plt.title(('[%s]Comparison chart for %s scores' % (name, score_type)), fontsize=(main_text_size + 2))
            if idx == 1 and handlers is not None:
                ax.legend(handlers, [i.short_id for i in self.posteriors], loc='upper center',
                  bbox_to_anchor=(0.5, -0.01),
                  ncol=1,
                  columnspacing=0.4,
                  labelspacing=0.0,
    ...
    def save_plots(self, path, dpi=None, separate_files=True):
        save_figures((self.figures), path, dpi, separate_files, clear_figures=True)
        return self
    ...
    def figures(self):
        ...
        elif not all(isinstance(k, string_types) and isinstance(v, plt.Figure) for k, v in self._figures.items()):
            raise AssertionError('Invalid stored Figures')
