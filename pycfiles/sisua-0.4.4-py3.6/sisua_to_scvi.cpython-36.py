...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/sisua/data/sisua_to_scvi.py
...
class FacsDataset(GeneExpressionDataset):
    ...
    def __init__(self, n_proteins=7):
        ...
        (super().__init__)(*GeneExpressionDataset.get_attributes_from_matrix(expression_data))

    def preprocess(self):
        ds, gene_ds, prot_ds = get_dataset(dataset_name=('facs_%d' % self.n_proteins),
          override=False)
...
class PbmcCiteseqDataset(GeneExpressionDataset):
    ...
    def preprocess(self):
        ds, gene_ds, prot_ds = get_dataset(dataset_name='pbmc_citeseq', override=False)
        expression_data = gene_ds.X
        gene_symbols = gene_ds.X_col
        self.gene_symbols = gene_symbols
        self.cell_names = gene_ds.X_row
