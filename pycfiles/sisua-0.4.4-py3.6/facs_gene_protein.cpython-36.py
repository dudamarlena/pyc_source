...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/sisua/data/data_loader/facs_gene_protein.py
# Compiled at: 2019-09-16 06:47:50
# Size of source mod 2**32: 8272 bytes
from __future__ import absolute_import, division, print_function
import base64, gzip, os, pickle, shutil
...
def read_FACS(n_protein, override=False, verbose=False):
    ...
    if not os.path.exists(os.path.join(preprocessed_path, 'X')):
        ...
        for name, data in crypto.unzip_aes(zip_path, password=_PASSWORD,
          verbose=False):
            ...
            if '.npz' in name:
                ...
                if '.csv' in name:
                    data = np.loadtxt((StringIO(str(data, 'utf-8'))), dtype=str,
                      delimiter=',')
                else:
        ...
        X, X_col = remove_allzeros_columns(matrix=X, colname=X_col,
          print_log=verbose)
        save_to_dataset(path=preprocessed_path, X=X,
          X_col=X_col,
          y=y,
          y_col=y_col,
          rowname=X_row,
    ...
    if not os.path.exists(os.path.join(preprocessed_path, 'X')):
        ...
        for name, _ in file_url:
            ...
            with gzip.open(zip_path, 'rb') as (f):
                data_map[name.split('.')[0]] = np.array([str(line, 'utf-8').strip().split(',') for line in f]).T
        ...
        y_row = prot[1:, 0]
        y_col = np.array([i.replace('"', '').split('_')[(-1)].upper() for i in prot[0, 1:]])
