...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/sisua/data/single_cell_dataset.py
# Compiled at: 2019-09-17 05:22:39
# Size of source mod 2**32: 46122 bytes
from __future__ import absolute_import, division, print_function
import os, warnings
from copy import deepcopy
from typing import Optional, Tuple
import numpy as np, scanpy as sc, scipy as sp, tensorflow as tf
...
from odin.utils import as_tuple, batching, cache_memory, catch_warnings_ignore, ctext
...
def apply_artificial_corruption(x, dropout, distribution, copy=False, seed=8):
  ...
  """
    ...
    assert 0 <= dropout < 1, 'dropout value must be >= 0 and < 1, given: %f' % dropout
    ...
    if dropout == 0:
        ...
        if distribution == 'uniform':
            i, j = np.nonzero(x)
            ix = rand.choice((range(len(i))), size=(int(np.floor(dropout * len(i)))),
              replace=False)
            i, j = i[ix], j[ix]
            corrupted = np.multiply(x[(i, j)], rand.binomial(n=np.ones((len(ix)), dtype=(np.int32)), p=0.9))
        else:
            if distribution == 'binomial':
                ...
                raise ValueError("Only support 2 corruption distribution: 'uniform' and 'binomial', but given: '%s'" % distribution)
...
def get_library_size(X, return_log_count=False):
  ...
  """
    if not X.ndim == 2:
        raise AssertionError('Only support 2-D matrix')
    else:
        total_counts = X.sum(axis=1)
        assert np.all(total_counts >= 0), 'Some cell contains negative-count!'
    log_counts = np.log(total_counts + 1e-08)
...
class SingleCellOMIC(sc.AnnData, Visualizer):
    ...
    def assert_matching_cells(self, sco) -> 'SingleCellOMIC':
        if not isinstance(sco, SingleCellOMIC):
            raise AssertionError('sco must be instance of SingleCellOMIC')
        else:
            assert sco.shape[0] == self.shape[0], 'Number of cell mismatch %d and %d' % (self.shape[0], sco.shape[0])
            if 'cellid' in sco.obs and 'cellid' in self.obs:
                assert np.all(sco.obs['cellid'] == self.obs['cellid'])
            elif not np.all(sco.obs.iloc[:, 0] == self.obs.iloc[:, 0]):
    ...
    def _calculate_library_info(self):
        ...
        log_counts, local_mean, local_var = get_library_size((self.X), return_log_count=True)
    ...
    def as_obsm(self, obsm_name):
        if not obsm_name in self.obsm:
            raise AssertionError
        else:
            omics = SingleCellOMIC(self, oidx=(slice(None, None)),
    ...
    """
        ...
        if not issubclass(itype, (np.bool, np.bool_, np.integer)):
            ...
            if observation:
                ...
                self._obsm = AxisArrays(self,
                  0, vals={i:j[indices] for i, j in self._obsm.items()})
                ...
                self._varm = AxisArrays(self,
                  1, vals={i:j[indices] for i, j in self._varm.items()})
        return self
    ...
    """
        ...
        if not self.isbacked:
            omics = SingleCellOMIC((X if X is not None else self._X.copy() if self._X is not None else None),
              (self._obs.copy()),
              (self._var.copy()),
            ...
            if filename is None:
                raise ValueError("To copy an SingleCellOMIC object in backed mode, pass a filename: `.copy(filename='myfilename.h5ad')`.")
            else:
                if self.isview:
                    self.write(filename)
                else:
            ...
            omics = SingleCellOMIC(filename=filename, name=(self.name if name is None else name))
    ...
    def is_binary(self):
        ...
        return sorted(np.unique(self.X.astype('float32'))) == [0.0, 1.0]
    ...
    modeling in log-space """
        return (
         self._local_mean, self._local_var)
    ...
    def as_tensorflow_dataset(self, obs=(), obsm=(), include_x=True) -> tf.data.Dataset:
        ...
        ds = tf.data.Dataset.from_tensor_slices(data[0] if len(data) == 1 else tuple(data))
        return ds

    def split(self, seed=8, train_percent=0.8) -> Tuple[('SingleCellOMIC', 'SingleCellOMIC')]:
    ...
    """
        ...
        ids = np.random.RandomState(seed=seed).permutation(self.n_obs).astype('int32')
        ntrain = int(train_percent * self.n_obs)
        train_ids = ids[:ntrain]
        train = SingleCellOMIC(X=(self.X[train_ids]),
        ...
        train._indices = train_ids
        test_ids = ids[ntrain:]
        test = SingleCellOMIC(X=(self.X[test_ids]),
        ...
        test._indices = test_ids
        return (train, test)

    def corrupt(self, corruption_rate=0.25, corruption_dist='binomial', inplace=True, seed=8):
        """
    Parameters
    ----------
    ...
    """
        if corruption_rate <= 0:
            ...
            data = apply_artificial_corruption((self.X), corruption_rate,
              corruption_dist,
              copy=(not inplace),
              seed=seed)
    ...
    def filter_highly_variable_genes(self, min_disp=0.5, max_disp=np.inf, min_mean=0.0125, max_mean=3, n_top_genes=1000, n_bins=20, flavor='cell_ranger', inplace=True):
    ...
    **dispersions_norm**
    ...
    def filter_genes(self, min_counts=None, max_counts=None, min_cells=None, max_cells=None, inplace=True):
    ...
    """
        omics = self if inplace else self.copy()
        sc.pp.filter_genes(omics, min_counts=min_counts,
    ...
    def filter_cells(self, min_counts=None, max_counts=None, min_genes=None, max_genes=None, inplace=True):
    ...
    Proxy method to Scanpy preprocessing
    """
        cells_subset, number_per_cell = sc.pp.filter_cells(self, min_counts=min_counts,
          max_counts=max_counts,
          min_genes=min_genes,
          max_genes=max_genes,
          inplace=False)
    ...
    def probabilistic_embedding(self, n_components_per_class=2, positive_component=1, log_norm=False, clip_quartile=0.0, remove_zeros=True, ci_threshold=-0.68, seed=8, pbe: Optional[ProbabilisticEmbedding]=None):
    ...
    """
        ...
        if pbe is None:
            if 'pbe' not in self.uns:
                pbe = ProbabilisticEmbedding(n_components_per_class=n_components_per_class,
            ...
            assert isinstance(pbe, ProbabilisticEmbedding), 'pbe, if given, must be instance of sisua.ProbabilisticEmbedding'
    ...
    def expm1(self, inplace=True):
        omics = self if inplace else self.copy()
        _expm1 = lambda x: np.expm1((x.data), out=(x.data)) if issparse(x) else np.expm1(x, out=x)
    ...
    def normalize(self, total_counts=False, log1p=False, scale=False, target_sum=None, exclude_highly_expressed=False, max_fraction=0.05, max_value=None, inplace=True):
        """
    ...
    """
        ...
        if log1p:
            sc.pp.log1p(omics, chunked=True,
    ...
    def neighbors(self, n_neighbors=15, n_pcs=None, use_rep=None, knn=True, method='umap', metric='euclidean', seed=8):
    ...
    **distances** : sparse matrix (`.uns['neighbors']`, dtype `float32`)
        ...
        neighbors.
    """
        sc.pp.neighbors(self, n_neighbors=n_neighbors,
          knn=knn,
          method=method,
          metric=metric,
          n_pcs=n_pcs,
    ...
    def louvain(self, resolution=None, restrict_to=None, key_added='louvain', adjacency=None, flavor='vtraag', directed=True, use_weights=False, partition_type=None, partition_kwargs=None, seed=8):
    ...
    def calculate_qc_metrics(self, expr_type='counts', var_type='genes', qc_vars=(), percent_top=(50, 100, 200, 500), layer=None, use_raw=False, parallel=None):
    ...
    pct_dropout_by_{expr_type}
      ...
      appear in.
    """
        sc.pp.calculate_qc_metrics(self, expr_type=expr_type,
          var_type=var_type,
          qc_vars=qc_vars,
    ...
    def plot_percentile_histogram(self, n_hist, title=None, outlier=0.001, non_zeros=False, fig=None):
    ...
    """
        ...
        for i, (p_min, p_max) in enumerate(zip(percentile, percentile[1:])):
            ...
            a = arr[mask]
            _, bins = vs.plot_histogram(a,
              bins=120,
              ax=(
             n_row, n_col, i + 1),
              fontsize=8,
              color=('red' if len(a) / n_samples < outlier else 'blue'),
              title=(('[%s]' % title if i == 0 else '') + '%d(samples)  Range:[%g, %g]' % (len(a), p_min, p_max)))
    ...
    def save_to_mmaparray(self, path, dtype=None):
        ...
        with MmapArrayWriter(path=path, shape=(self.shape),
    ...
    def _get_str(self):
        ...
        text += '\n    Nonzeros: %s' % describe(all_nonzeros,
          shorten=True, float_precision=2)
        text += '\n    Cell    : %s' % describe((self.counts_per_cell),
          shorten=True, float_precision=2)
        text += '\n    Gene    : %s' % describe((self.counts_per_gene),
          shorten=True, float_precision=2)
        text += '\n    TotalCount: %s' % describe((self.total_counts),
          shorten=True, float_precision=2)
        text += '\n    LogCount  : %s' % describe((self.log_counts),
          shorten=True, float_precision=2)
        text += '\n    LocalMean : %s' % describe((self.local_mean),
          shorten=True, float_precision=2)
        text += '\n    LocalVar  : %s' % describe((self.local_var),
          shorten=True, float_precision=2)
        return text
