...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/sisua/analysis/latent_benchmarks.py
# Compiled at: 2019-09-23 05:06:52
# Size of source mod 2**32: 19765 bytes
...
from odin.visual import plot_colorbar, plot_figure, plot_scatter_heatmap, to_axis
from sisua.data.const import MARKER_GENES, PROTEIN_PAIR_COMPARISON, UNIVERSAL_RANDOM_SEED
from sisua.data.utils import standardize_protein_name
from sisua.label_threshold import ProbabilisticEmbedding
from sisua.utils.visualization import downsample_data, fast_pca, fast_scatter, fast_tsne, plot_evaluate_classifier

def multi_label_adj_Rindex(label_bin_true, label_pred):
    if not label_bin_true.ndim == 2:
        raise AssertionError
    elif not label_bin_true.shape[1] == len(np.unique(label_pred)):
        raise AssertionError
    ...
    with catch_warnings_ignore(Warning):
        ...
        for y in label_bin_true.T:
            y = y.astype('int32')
            s = max(adjusted_rand_score(labels_true=y, labels_pred=((label_pred == i).astype('int32'))) for i in range(n_classes))
            scores.append(s)
...
def clustering_scores(latent, labels, n_labels, prediction_algorithm='both'):
  ...
  """
    ...
    if prediction_algorithm == 'knn':
        ...
        if prediction_algorithm == 'gmm':
            gmm = GaussianMixture(n_labels, random_state=5218)
            gmm.fit(latent)
            labels_pred = gmm.predict(latent)
        else:
            if prediction_algorithm == 'both':
                score1 = clustering_scores(latent, labels,
                  n_labels=n_labels,
                  prediction_algorithm='knn')
                score2 = clustering_scores(latent, labels,
                  n_labels=n_labels,
                  prediction_algorithm='gmm')
                return {k:(v + score2[k]) / 2 for k, v in score1.items()}
            raise ValueError("Not support for prediction_algorithm: '%s'" % prediction_algorithm)
...
def streamline_classifier(Z_train, y_train, Z_test, y_test, labels_name, mode='ovr', title='', plot_train_results=False, show_plot=True, return_figure=False, figsize=None):
    """
  Parameters
  ----------
  ...
  """
    ...
    assert mode in ('ovr', 'ovo'), 'Only support ovr - one vs rest, ovo - one vs one; mode for streamline classifier'
    ...
    with catch_warnings_ignore(FutureWarning):
        with catch_warnings_ignore(RuntimeWarning):
            ...
            is_binary_classes = sorted(np.unique(y_train.astype('float32'))) == [0.0, 1.0]
            ...
            with catch_warnings_ignore(UndefinedMetricWarning):
                results_train = plot_evaluate_classifier(y_pred=(classifier.predict(Z_train)),
                  y_true=y_train,
                  labels=labels_name,
                  title=('[train]' + title),
                  ...
                  return_figure=True,
                  figsize=figsize)
                results_test = plot_evaluate_classifier(y_pred=(classifier.predict(Z_test)),
                  y_true=y_test,
                  labels=labels_name,
                  title=('[test]' + title),
                  show_plot=show_plot,
            ...
            results_train = OrderedDict(sorted((results_train.items()), key=(lambda x: x[0])))
            results_test = OrderedDict(sorted((results_test.items()), key=(lambda x: x[0])))
            results = (results_train, results_test)
            if show_plot:
                if return_figure:
                    return (
                     results, (fig_train, fig_test))
            return results


def plot_distance_heatmap(X, labels, labels_name=None, lognorm=True, colormap='hot', ax=None, legend_enable=True, legend_loc='upper center', legend_ncol=3, legend_colspace=0.2, fontsize=10, show_colorbar=True, title=None):
    """
  Parameters
  ----------
  ...
  """
    ...
    if labels.ndim == 2:
        ...
        if not np.all(labels_int == labels) or labels_name is not None and len(labels_name) == 2 or len(np.unique(labels)) == 2:
        ...
        order_X = np.vstack([x for _, x in sorted((zip(labels, X)), key=(lambda pair: pair[0]))])
        order_label = np.vstack([y for y, x in sorted((zip(labels, X)), key=(lambda pair: pair[0]))])
        ...
        assert np.sum(final_img[:width, :width]) == 0, 'Something wrong with my spacial coordination when writing this code!'
        ax.imshow(final_img)
        ax.axis('off')
        if labels_name is not None:
            if bool(legend_enable):
                ...
                legend_elements = [Line2D([0], [0], marker='o', color=color, label=name, linewidth=0, linestyle=None, lw=0, markerfacecolor=color, markersize=(fontsize // 2)) for color, name in zip(all_colors, labels_name)]
                ax.legend(handles=legend_elements, markerscale=1.0,
                  scatterpoints=1,
                  scatteryoffsets=[
                 0.375, 0.5, 0.3125],
                  loc=legend_loc,
    ...
    if show_colorbar:
        plot_colorbar(colormap, vmin=(np.min(distance)),
...
def plot_latents_multiclasses(Z, y, labels_name, title=None, elev=None, azim=None, use_PCA=False, show_colorbar=False, figsize=None):
    ...
    if n_pairs == 0:
        ...
        for idx, labels_name in enumerate(pairs):
            ...
            plot_scatter_heatmap(x=(Z[:, 0]),
              ...
              colorbar_ticks=[
             labels_name[0], 'Others', labels_name[1]],
...
def plot_latents_binary(Z, y, labels_name, title=None, elev=None, azim=None, use_PCA=False, ax=None, show_legend=True, size=12, fontsize=12, show_scores=True, enable_argmax=True, enable_separated=False):
    from matplotlib import pyplot as plt
    if title is None:
        title = ''
    else:
    ...
    if enable_separated:
        ...
        for i, lab in enumerate(labels_name):
            ...
            plot_scatter_heatmap(x=(Z[:, 0]), y=(Z[:, 1]),
              val=(val / np.sum(val)),
              ax=(
             nrow, ncol, i + 1),
              colormap=colormap,
