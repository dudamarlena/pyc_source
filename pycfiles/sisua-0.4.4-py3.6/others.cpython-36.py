...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/sisua/utils/others.py
# Compiled at: 2019-07-19 10:57:32
# Size of source mod 2**32: 5369 bytes
...
def filtering_experiment_path(ds_name, incl_keywords, excl_keywords, fn_filter=None, return_dataset=False, print_log=False, exp_path=''):
    """
  ...
  """
    ...
    if fn_filter is None:
    ...
    elif ds_name is None or return_dataset:
        ...
        if isinstance(incl_keywords, string_types):
            ...
            if isinstance(incl_keywords, (tuple, list)):
        ...
        if isinstance(excl_keywords, string_types):
            ...
            if isinstance(excl_keywords, (tuple, list)):
                excl_keywords = as_tuple(excl_keywords, t=str)
            else:
                raise ValueError('No support for excl_keywords type: %s' % str(type(excl_keywords)))
    all_exp = [i for i in all_exp if all(any(j in keyword for keyword in os.path.basename(i).split('_')) for j in incl_keywords)]
    all_exp = [i for i in all_exp if all(all(j not in keyword for keyword in os.path.basename(i).split('_')) for j in excl_keywords)]
