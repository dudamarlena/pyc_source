...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/sisua/label_threshold.py
# Compiled at: 2019-07-26 11:18:00
# Size of source mod 2**32: 17430 bytes
from __future__ import print_function, division, absolute_import
from matplotlib import pyplot as plt
import matplotlib.mlab as mlab, os
...
from odin.visual import print_dist, merge_text_graph, plot_confusion_matrix, plot_figure, plot_histogram_layers, plot_save, generate_random_colors, plot_histogram, Visualizer
from odin.utils import unique_labels, ctext, auto_logging, batching, UnitTimer, ArgController, get_script_path, mpi, Progbar, catch_warnings_ignore, is_number
...
def _draw_hist(x, ax, title, n_bins, show_yticks=True):
    ...
    plt.xticks(np.linspace(start=(np.min(x)), stop=(np.max(x)), num=5,
      dtype='float32'),
      fontsize=6)
    if show_yticks:
        plt.yticks(np.linspace(start=(np.min(count)), stop=(np.max(count)), num=5,
          dtype='int32'),
          fontsize=5)
    else:
        plt.yticks([], [])
...
class ProbabilisticEmbedding(BaseEstimator, DensityMixin, Visualizer):
    ...
    def __init__(self, n_components_per_class=2, positive_component=1, log_norm=True, clip_quartile=0.0, remove_zeros=True, ci_threshold=-0.68, random_state=8, verbose=False):
        ...
        if not self.positive_component > 0:
    ...
    """
        return np.hstack([gmm.means_.ravel()[order][:, np.newaxis] for order, gmm in self._models])
    ...
    """
        return np.hstack([gmm.precisions_.ravel()[order][:, np.newaxis] for order, gmm in self._models])
    ...
    def normalize(self, x, test_mode=False):
        if x.ndim > 1:
            ...
            if not np.all(x >= 0):
                raise AssertionError('Only support non-negative values')
            else:
    ...
    def fit(self, X):
        ...
        for i in it:
            ...
            gmm = GaussianMixture(n_components=(self.n_components_per_class), covariance_type='diag',
              init_params='kmeans',
              n_init=8,
              max_iter=120,
    ...
    def _predict(self, X, threshold):
        ...
        for i, (order, gmm) in enumerate(self._models):
            ...
            if threshold is not None:
                ci = stats.norm.interval((np.abs(threshold)), loc=(gmm.means_[order[self.positive_component]]),
                ...
                probas = gmm.predict_proba(x_test[:, np.newaxis]).T[order][self.positive_component:]
    ...
    def _check_input(self, X, labels):
        if not X.ndim == 2:
            raise AssertionError('Only support input matrix but given: %s' % str(X.shape))
        else:
            n_classes = X.shape[1]
            assert n_classes == self.n_classes, 'Fitted with %d classes but give %d classes' % (self.n_classes, n_classes)
            if labels is None:
                labels = ['#%d' % i for i in range(n_classes)]
            assert len(labels) == n_classes, 'Number of classes and labels mismatch'
        return (
         X, labels, n_classes)
    ...
    def plot_diagnosis(self, X, labels=None, n_bins=200):
        ...
        for i, (name, (order, gmm)) in enumerate(zip(labels, self._models)):
            ...
            count, bins = _draw_hist(x,
              ax=ax, title=('[%s] LLK:%.2f BIC:%.2f AIC:%.2f' % (
            ...
            ax.plot(bins, y_, color='red', linestyle='-',
              linewidth=1.5,
              alpha=0.6)
            ci = stats.norm.interval((np.abs(self.ci_threshold)), loc=(gmm.means_[order[self.positive_component]]),
              scale=(np.sqrt(1 / gmm.precisions_[order[self.positive_component]])))
            threshold = ci[0] if self.ci_threshold < 0 else ci[1]
            ids = np.where(bins >= threshold, True, False)
            ax.fill_between((bins[ids]), y1=0, y2=(np.max(count)), facecolor=(colors[(-2)]),
              alpha=0.3)
            ...
            for c, m, p in zip(colors, means_, precision_):
                ...
                ax.scatter((_x[np.argmax(j)]), (np.max(j)), s=66,
                  alpha=0.8,
                  linewidth=0,
                  color=c)
    ...
    def boxplot(self, X, labels=None):
        ...
        for i, (x, name) in enumerate(zip(X.T, labels)):
            ...
            ax.boxplot(self.normalize(x, test_mode=False), whis=1.5,
              labels=['Normalized'],
              flierprops={'marker':'.', 
             'markersize':8},
              showmeans=True,
...
def get_arguments():
    args = ArgController().add('input', 'Name of the dataset or path to csv file').add('-n', 'number of GMM components', 2).add('-idx', 'index of the positive component', 1).add('-norm', 'method for normalizing: raw, log', 'log', ('log',
                                                                                                                                                                                                                                       'raw')).add('-outpath', 'y_bin and y_prob will be saved to this path', '').add('-figpath', 'path for saving analysis figure', '/tmp/tmp.pdf').add('--verbose', 'Enable verbose and saving diagnosis', False).parse()
    inp = str(args.input)
    if os.path.exists(inp):
        if not os.path.isfile(inp):
            raise AssertionError('%s must be path to a file' % inp)
        else:
            data = []
    ...
    return {'y_prot':y_prot, 
     'y_prot_names':y_prot_names,  'n_components':int(args.n), 
     'index':int(args.idx),  'log_norm':True if args.norm == 'log' else False, 
     'outpath':outpath if len(outpath) > 0 else None, 
     'figpath':args.figpath if len(args.figpath) > 0 else None, 
     'verbose':bool(args.verbose)}


def main(y_prot, y_prot_names, n_components=2, index=1, log_norm=True, clip_quartile=0.0, remove_zeros=True, ci_threshold=-0.68, outpath=None, figpath=None, verbose=False):
    if outpath is not None:
        ...
        if figpath is not None:
            pb.boxplot(y_prot, y_prot_names).plot_diagnosis(y_prot, y_prot_names).plot_distribution(y_prot, y_prot_names).save_figures(path=figpath,
              verbose=verbose)
