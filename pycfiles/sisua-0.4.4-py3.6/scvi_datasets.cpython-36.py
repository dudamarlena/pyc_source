...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/sisua/data/data_loader/scvi_datasets.py
# Compiled at: 2019-09-20 05:50:25
...
def _save_data_to_path(preprocessed_path, X, y, gene_names, label_names, cell_names, verbose):
    if verbose:
...
def _read_scvi_dataset(name, clazz_name, override, verbose):
    preprocessed_path = select_path((os.path.join(PREPROCESSED_BASE_DIR, '%s_preprocessed' % name)),
      create_new=True)
    if override:
    ...
    if not os.path.exists(os.path.join(preprocessed_path, 'X')):
        ...
        if hasattr(gene_dataset, 'de_metadata'):
            ...
            gene_names = np.array([pbmc8kconverter[i] if i in pbmc8kconverter else converter[i] for i in gene_names])
        ...
        _save_data_to_path(preprocessed_path, X, y, gene_names, label_names, cell_names, verbose)
...
def read_Hemato(override=False, verbose=False):
    preprocessed_path = select_path((os.path.join(PREPROCESSED_BASE_DIR, 'HEMATO_preprocessed')),
      create_new=True)
    if override:
        shutil.rmtree(preprocessed_path)
        os.mkdir(preprocessed_path)
    if not os.path.exists(os.path.join(preprocessed_path, 'X')):
        ...
        y_bin = np.argmax((np.hstack((
         gene_dataset.meta.iloc[:, 1].values[:, None],
         gene_dataset.meta.iloc[:, 2].values[:, None]))),
          axis=(-1))
