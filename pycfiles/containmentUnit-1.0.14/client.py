...
# Compiled at: 2015-11-03 14:11:03
import requests
try:
...
class Client(object):
    ...
    def proxy_ports(self):
        ...
        """
        r = requests.get('%s/proxy:' + self.port % self.host).json()
    ...
    def blacklist(self, regexp, status_code):
        ...
        """
        r = requests.put('%s/proxy/%s/blacklist' % (self.host, self.port), {'regex': regexp, 'status': status_code})
        return r.status_code
    ...
    def whitelist(self, regexp, status_code):
        ...
        """
        r = requests.put('%s/proxy/%s/whitelist' % (self.host, self.port), {'regex': regexp, 'status': status_code})
        return r.status_code
    ...
    def basic_authentication(self, domain, username, password):
        ...
        """
        r = requests.post(url='%s/proxy/%s/auth/basic/%s' % (self.host, self.port, domain), data=json.dumps({'username': username, 'password': password}), headers={'content-type': 'application/json'})
        return r.status_code
    ...
    def headers(self, headers):
        ...
        r = requests.post(url='%s/proxy/%s/headers' % (self.host, self.port), data=json.dumps(headers), headers={'content-type': 'application/json'})
        return r.status_code
    ...
    def response_interceptor(self, js):
        ...
        """
        r = requests.post(url='%s/proxy/%s/interceptor/response' % (self.host, self.port), data=js, headers={'content-type': 'x-www-form-urlencoded'})
        return r.status_code
    ...
    def request_interceptor(self, js):
        ...
        :param js: the javascript to execute
        """
        r = requests.post(url='%s/proxy/%s/interceptor/request' % (self.host, self.port), data=js, headers={'content-type': 'x-www-form-urlencoded'})
        return r.status_code

    LIMITS = {'upstream_kbps': 'upstreamKbps', 
    ...
    TIMEOUTS = {'request': 'requestTimeout', 
       'read': 'readTimeout', 
    ...
    def remap_hosts(self, address, ip_address):
        ...
        r = requests.post('%s/proxy/%s/hosts' % (self.host, self.port), json.dumps({address: ip_address}), headers={'content-type': 'application/json'})
        return r.status_code
    ...
    def wait_for_traffic_to_stop(self, quiet_period, timeout):
        ...
        """
        r = requests.put('%s/proxy/%s/wait' % (self.host, self.port), {'quietPeriodInMs': quiet_period, 'timeoutInMs': timeout})
        return r.status_code
    ...
    def retry(self, retry_count):
        ...
        """
        r = requests.put('%s/proxy/%s/retry' % (self.host, self.port), {'retrycount': retry_count})
        return r.status_code