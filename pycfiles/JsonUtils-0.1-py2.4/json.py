...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/jsonutils/json.py
...
class JsonReader(object):
    ...
    def _readString(self):
        result = ''
        quotechar = self._next()
        assert quotechar == '"' or quotechar == "'"
        try:
            while self._peek() != quotechar:
                ch = self._next()
                if ch == '\\':
                    ch = self._next()
                    if ch in 'brnft':
