...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/sphinxcontrib/swf/__init__.py
# Compiled at: 2013-08-25 00:30:12
"""
    sphinxcontrib.swf
    ~~~~~~~~~~~~~~~~~
...
def bool_option(argument):
    return directives.choice(argument, ('yes', 'no', 'true', 'false', '0', '1'))


def quality_option(argument):
    return directives.choice(argument, ('low', 'autolow', 'autohigh', 'medium', 'high',
                                        'best'))


def scale_option(argument):
    return directives.choice(argument, ('default', 'noborder', 'exactfit', 'noscale'))


def align_option(argument):
    return directives.choice(argument, ('l', 'r', 't'))


def salign_option(argument):
    return directives.choice(argument, ('l', 'r', 't', 'tl', 'tr'))


def wmode_option(argument):
    return directives.choice(argument, ('window', 'direct', 'opaque', 'transparent',
                                        'gpu'))
...
class swf(nodes.General, nodes.Inline, nodes.Element):
    pass


FLASH_PARAMS = {'width': directives.nonnegative_int, 
...
class ShockWaveFlash(Directive):
    ...
    option_spec = dict({'class': directives.class_option, 
       'zoom-to-fit': bool_option}, **FLASH_PARAMS)
    has_content = False
    ...
    def run(self):
        ...
        if 'allowfullscreen' not in self.options and 'allowFullScreen' not in self.options:
        ...
        if 'fullscreenaspectratio' in self.options:
            self.options['fullScreenAspecRatio'] = self.options['fullscreenaspecratio']
            del self.options['fullscreenaspecratio']
        reference = directives.uri(self.arguments[0])
        self.options['uri'] = reference
        env.config.swf_flash_files.append(reference)
        return [
         swf(rawsource=self.block_text, **self.options)]
...
def html_visit_swf(self, node):
    ...
    result += '<object%s classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="%s" height="%s">\n' % (
     attrs, width, height)
...
def on_builder_inited(app):
    app.config.html_static_path.append(os.path.relpath(os.path.join(os.path.dirname(__file__), 'javascript'), app.confdir))
