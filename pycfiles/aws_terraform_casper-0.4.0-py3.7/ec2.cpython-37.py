...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/casper/services/ec2.py
...
class EC2Service(BaseService):
    ...
    def _get_live_aws_elb(self):
        ...
        while 'NextMarker' in elb.keys():
            ...
            lbs.update({lb['LoadBalancerName']:lb for lb in elb['LoadBalancerDescriptions']})
    ...
    def _get_live_aws_instance(self):
        ...
        ec2s = {instance['InstanceId']:instance for instance_group in instances for instance in instance_group if instance['State']['Code'] == 16}
        asg_client = self.session.client('autoscaling')
        asgs = asg_client.describe_auto_scaling_instances()
        dynamic_instances = [instance['InstanceId'] for instance in asgs['AutoScalingInstances']]
        while 'NextToken' in asgs.keys():
            asgs = asg_client.describe_auto_scaling_instances(NextToken=(asgs['NextToken']))
            dynamic_instances.extend((instance['InstanceId'] for instance in asgs['AutoScalingInstances']))

        static_instances = {k:ec2s[k] for k in set(ec2s.keys()).difference(set(dynamic_instances))}
    ...
    def _get_live_aws_autoscaling_group(self):
        ...
        while 'NextToken' in asgs_group.keys():
            asgs_group = asg_client.describe_auto_scaling_groups(NextToken=(asgs_group['NextToken']))
            asgs.update({a['AutoScalingGroupName']:a for a in asgs_group['AutoScalingGroups']})
    ...
    def _get_live_aws_security_group(self):
        ...
        while 'NextToken' in sgs_group.keys():
            sgs_group = ec2_client.describe_security_groups(NextToken=(sgs_group['NextToken']))
    ...
    def scan_service(self, ghosts):
        ...
        if 'aws_autoscaling_group' in ghosts:
            if len(ghosts['aws_autoscaling_group']['ids']) > 0:
                ...
                for defaulting_asgs_batch in batch(defaulting_asgs, 50):
                    asgs = asg_client.describe_auto_scaling_groups(AutoScalingGroupNames=defaulting_asgs_batch)
                    asg_instances.update({instance['InstanceId']:instance for sublist in (afull['Instances'] for afull in asgs['AutoScalingGroups']) for instance in sublist})
                    while 'NextToken' in asgs.keys():
                        asgs = asg_client.describe_auto_scaling_groups(AutoScalingGroupNames=defaulting_asgs_batch,
                ...
                if 'resources' in ghosts['aws_instance']:
                    asg_instances.update({r['InstanceId']:r for r in ghosts['aws_instance']['resources']})
                    resources = [asg_instances.get(k) for k in ghosts['aws_instance']['ids']]
