...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/dnaStreaming/listener.py
# Compiled at: 2020-05-12 03:20:35
# Size of source mod 2**32: 6403 bytes
...
class Listener(object):
        ...
        while True:
            ...
            try:
                if r.json()['data']['attributes']['job_status'] == 'DOC_COUNT_EXCEEDED':
                    ...
                    logger.error("OOPS! Looks like you've exceeded the maximum number of documents received for your account ({}). As such, no new documents will be added to your stream's queue. However, you won't lose access to any documents that have already been added to the queue. These will continue to be streamed to you. Contact your account administrator with any questions or to upgrade " + 'your account limits.'.format(limit_msg))
            except KeyError:
                raise Exception('Unable to request data from your stream subscription id')
    ...
    def listen(self, on_message_callback, maximum_messages=DEFAULT_UNLIMITED_MESSAGES, subscription_id='', batch_size=10):
        ...
        if not subscription_id:
            raise Exception('No subscription specified. You must specify the subscription ID either through an environment variable, a config file or by passing the value to the method.')
