...
# [GCC 8.4.0]
# Embedded file name: scorer/__init__.py
# Compiled at: 2018-06-12 04:58:22
...
class RankingScorer(_BaseScorer):
    ...
    def __init__(self, score_func, sign=1):
        ...
        if not score_func.__module__ == 'xgboostextension.scorer.metrics':
            raise ValueError('Only score functions included with this package are supported')
        super(RankingScorer, self).__init__(_make_grouped_metric(score_func), sign, {})
    ...
    def __repr__(self):
        if hasattr(self._ungrouped_score_func, '__name__'):
            return ('RankingScorer({0})').format(self._ungrouped_score_func.__name__)
        else:
            if hasattr(self._ungrouped_score_func, '__class__'):
                return ('RankingScorer({0})').format(self._ungrouped_score_func.__class__.__name__)
