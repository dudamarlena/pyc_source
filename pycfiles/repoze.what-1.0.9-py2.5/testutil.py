...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/repoze/what/adapters/testutil.py
# Compiled at: 2010-04-06 05:16:33
"""Utilities to test source adapters."""
from repoze.what.adapters import SourceError, ExistingSectionError, NonExistingSectionError, ItemPresentError, ItemNotPresentError
__all__ = [
 'GroupsAdapterTester', 'PermissionsAdapterTester',
...
class _ReadOnlyBaseAdapterTester(object):
    ...
    def test_getting_section_items(self):
        for (section_name, items) in self.all_sections.items():
            self.assertEqual(self.adapter._get_section_items(section_name), items)

    def test_checking_existing_section(self):
        for section_name in self.all_sections.keys():
            assert self.adapter._section_exists(section_name), 'Section "%s" does NOT exist' % section_name

    def test_checking_non_existing_section(self):
        section_name = 'i_dont_exist'
        assert not self.adapter._section_exists(section_name), 'Section "%s" DOES exist' % section_name

    def test_checking_item_inclusion(self):
        for (section_name, items) in self.all_sections.items():
            for item in self.adapter._get_section_items(section_name):
                assert self.adapter._item_is_included(section_name, item), 'Item "%s" must be included in section "%s"' % (
                 item, section_name)

    def test_checking_excluded_item_inclusion(self):
        excluded_item = self.new_items.pop()
        for (section_name, items) in self.all_sections.items():
            assert not self.adapter._item_is_included(section_name, excluded_item), 'Item "%s" must not included in section "%s"' % (
             item, section_name)

    def test_checking_section_existence(self):
        for section_name in self.all_sections.keys():
            assert self.adapter._section_exists(section_name), 'Section "%s" must exist' % section_name

    def test_checking_non_existing_section_existence(self):
        invalid_section = 'designers'
        assert not self.adapter._section_exists(invalid_section), 'Section "%s" must not exist' % invalid_section

    def test_sets_if_it_is_writable(self):
        assert hasattr(self.adapter, 'is_writable'), "The adapter doesn't have the 'is_writable' attribute; please call its parent's constructor too"
...
class _BaseAdapterTester(_ReadOnlyBaseAdapterTester):
    ...
    def test_adding_many_items_to_section(self):
        for (section_name, items) in self.all_sections.items():
            ...
            assert self.adapter._get_section_items(section_name) == final_items, '"%s" does not include %s' % (section_name, self.new_items)

    def test_creating_section(self):
        section = 'cool-section'
        self.adapter._create_section(section)
        assert section in self.adapter._get_all_sections().keys(), 'Section "%s" could not be added' % section
    ...
    def test_editing_section(self):
        ...
        assert new_section in self.adapter._get_all_sections().keys() and old_section not in self.adapter._get_all_sections().keys(), 'Section "%s" was not renamed to "%s"' % (old_section,
         new_section)

    def test_deleting_section(self):
        section = self.all_sections.keys()[0]
        self.adapter._delete_section(section)
        assert section not in self.adapter._get_all_sections().keys(), 'Section "%s" was not deleted' % section
...
class ReadOnlyGroupsAdapterTester(_ReadOnlyBaseAdapterTester):
    ...
    * php
    ...
    def setUp(self):
        self.all_sections = {'admins': set(('rms', )), 
    ...
    def test_finding_groups_of_authenticated_user(self):
        for userid in self._get_all_items():
            ...
            self.assertEqual(self.adapter._find_sections(credentials), self._get_item_sections(userid))
...
class GroupsAdapterTester(ReadOnlyGroupsAdapterTester, _BaseAdapterTester):
    ...
    def test_removing_many_users_from_group(self):
        ...
        assert self.adapter._get_section_items(group) == set(), '"%s" still includes %s' % (group, users)
...
class PermissionsAdapterTester(ReadOnlyPermissionsAdapterTester, _BaseAdapterTester):
    ...
    def test_deying_permisssion_to_many_groups(self):
        ...
        assert self.adapter._get_section_items(permission) == set(), '"%s" still includes %s' % (permission, groups)