    ...
    def grow_cluster(self, key=None, value=None):
        ...
        if key:
            while nodes_to_visit:
                ...
                nodes_to_visit = nodes_to_visit.union(set([neigh for neigh in node.neighbours if neigh not in visited if neigh.labels[key] == value]))
                add(node)
    ...
    def torture_py(self):
        ...
        while uc:
            ...
            while node_stack:
                ...
                if node.labels['UC_index'] == uc_index:
                    if node.index != index:
