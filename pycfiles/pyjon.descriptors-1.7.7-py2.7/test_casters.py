...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyjon/descriptors/tests/test_casters.py
# Compiled at: 2014-06-30 06:47:44
from pyjon.descriptors.casters import Caster
import decimal, datetime, unittest

class TestDescriptors(unittest.TestCase):

    def create_type_dict(self, ftype, format=None, thousanddelimiter=None, decimaldelimiter=None, decimalplaces=None):
        type_dict = dict(name='%s' % ftype, items={})
        if format:
    ...
    def test_caster_thousand_coma_with_decimal_dot(self):
        ...
        """
        caster = Caster()
        typenode = self.create_type_dict('decimal', thousanddelimiter=',', decimaldelimiter='.')
