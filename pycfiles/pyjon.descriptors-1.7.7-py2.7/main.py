...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyjon/descriptors/main.py
...
class Descriptor(object):
    ...
    def __init__(self, payload_tree, encoding, buffersize=16384):
        ...
        self.conditional_descriptors = self.__get_conditional_descriptors(encoding, buffersize)
    ...
    def __get_conditional_descriptors(self, encoding, buffersize):
        ...
        for field_node in field_nodes:
            if field_node.find('type') is None:
                raise ValueError('One of the fields in your descriptor misses a type node')
                ...
                for descriptor_node in descriptors_nodes:
                    ...
                    desc['descriptor'] = Descriptor(descriptor_node, encoding, buffersize=buffersize)
            ...
            def apply_condition(condition_set, flow):
                for item in flow:
                    if eval(condition_set.get('condition', 'True')):
                        value = list(condition_set.get('descriptor').read([
                         getattr(item, condition_set.get('field_name'))]))[0]
                        setattr(item, condition_set.get('field_name'), value)
                    yield item

            iterators = tee(main_flow, len(self.conditional_descriptors))
            iterators = [ apply_condition(self.conditional_descriptors[it], citer) for it, citer in enumerate(list(iterators))
                        ]
