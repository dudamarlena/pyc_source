...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/techrec/forge.py
...
def get_timefile_exact(time):
    ...
    """
    return os.path.join(get_config()['AUDIO_INPUT'], time.strftime(get_config()['AUDIO_INPUT_FORMAT']))
...
def create_mp3(start, end, outfile, options={}, **kwargs):
    intervals = [ (get_timefile(begin), start_cut, end_cut) for begin, start_cut, end_cut in get_files_and_intervals(start, end)
                ]
    ...
    p = Popen(mp3_join(intervals) + metadata_list + get_config()['FFMPEG_OPTIONS'] + [outfile])
