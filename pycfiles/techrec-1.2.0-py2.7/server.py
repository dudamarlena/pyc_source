...
class RecAPI(Bottle):
    ...
    def help(self):
        return '<h1>help</h1><hr/>        <h2>/get, /get/, /get/<id> </h2>        <h3>Get Info about rec identified by ID </h3>                <h2>/search, /search/, /search/<key>/<value></h2>        <h3>Search rec that match key/value (or get all)</h3>                <h2>/delete/<id> </h2>        <h3>Delete rec identified by ID </h3>        <h2>/update </h2>        <h3>Not implemented.</h3>'
...
class RecServer:
    ...
    def _route(self):
        self._app.route('/output/<filepath:path>', callback=lambda filepath: static_file(filepath, root=get_config()['AUDIO_OUTPUT'], download=True))
        self._app.route('/static/<filepath:path>', callback=lambda filepath: static_file(filepath, root=get_config()['STATIC_FILES']))
