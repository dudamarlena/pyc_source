...
# Embedded file name: /usr/lib/python2.7/site-packages/braincore/__init__.py
# Compiled at: 2014-11-26 02:12:34
__doc__ = 'This code implements the core objects needed for most brainfuck-like languages.\n\nThe number of cells for this implementation is infinite, the max value\nof cells defaults to 255.\n'
...
class IP:
    ...
    def next(self):
        ...
        self.x += vx
        self.y += vy
        if self.x > self.max_x or self.y > self.max_y or self.y < 0 or self.x < 0:
            raise StopIteration
...
class Cell:
    ...
    def inc_or_dec(self, val):
        self.vals[self.pointer] = (self.vals.get(self.pointer, 0) + val) % self.max_val
