...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/s100py/s111.py
# Compiled at: 2020-01-27 10:45:33
# Size of source mod 2**32: 48752 bytes
...
"""
import contextlib, datetime, os, numpy, warnings, shutil
...
H5T_CLASS_T = {h5py.h5t.NO_CLASS: 'H5T_NO_CLASS', 
...
class S111File:
    ...
    def add_feature_instance_group_data(self, datetime_value, speed, direction, cycletime, target_depth):
        ...
        self.feature_instance_groups = [obj for obj in feature_instance_objs if isinstance(self.feature_instance[obj], h5py.Group)]
        ...
        if len(self.feature_instance_groups) == 0:
            ...
            if feature_instance_date < 199406:
                ...
                if feature_instance_date < 199706:
                    epoch = 'G730'
                else:
                    if feature_instance_date < 200201:
                        epoch = 'G873'
                    else:
                        if feature_instance_date < 201202:
                            epoch = 'G1150'
                        else:
                            if feature_instance_date < 201310:
                                epoch = 'G1674'
                            else:
                                if feature_instance_date >= 201310:
                                    epoch = 'G1762'
    ...
    def add_positioning(self, longitude, latitude):
        ...
        geometry_dset = feature_positioning.create_dataset('geometryValues', (dim,), dtype=geometry_dtype, chunks=True,
          compression='gzip',
          compression_opts=9)
    ...
    def add_model_metadata(self):
        ...
        self.feature.attrs.create('interpolationType', (self.input_metadata.INTERPOLATION_TYPE['discrete']), dtype=h5py.special_dtype(enum=(numpy.uint8, self.input_metadata.INTERPOLATION_TYPE)))
        ...
        if self.h5_file.__contains__('/SurfaceCurrent/SurfaceCurrent.01/Group_002'):
            first_time = datetime.datetime.strptime(self.h5_file['/SurfaceCurrent/SurfaceCurrent.01/Group_001'].attrs['timePoint'], '%Y%m%dT%H%M%SZ')
            second_time = datetime.datetime.strptime(self.h5_file['/SurfaceCurrent/SurfaceCurrent.01/Group_002'].attrs['timePoint'], '%Y%m%dT%H%M%SZ')
...
class S111Metadata:
    ...
    DATA_CODING_FORMAT = {'Time series at fixed stations':1,  'Regularly-gridded arrays':2, 
     'Ungeorectified gridded arrays':3, 
     'Moving platform':4}
    DEPTH_TYPE_INDEX = {'Layer average':1, 
     'Sea surface':2,  'Vertical datum':3,  'Sea Bottom':4}
    INTERPOLATION_TYPE = {'nearestneighbor':1,  'linear':2, 
     'quadratic':3, 
     'cubic':4, 
     'bilinear':5, 
     'biquadratic':6, 
    ...
    COMMON_POINT_RULE = {'average':1, 
     'low':2,  'high':3,  'all':4}
    TYPE_OF_CURRENT_DATA = {'Historical observation (O)':1,  'Real-time observation (R)':2, 
    ...
    VERTICAL_DATUM = {'meanLowWaterSprings':1,  'meanLowerLowWaterSprings':2, 
     'meanSeaLevel':3, 
     'lowestLowWater':4, 
     'meanLowWater':5, 
...
class S111TimeSeries:
    __doc__ = 'Contains prediction time series data to pass to S111File. '
...
def model_to_s111(model_index_file, model_files, s111_path_prefix, cycletime, input_metadata, data_coding_format, target_depth):
    ...
    if os.path.isdir(s111_path_prefix):
        ...
        s111_path_prefix += 'S111{}_{}_{}_TYP{}'.format(input_metadata.producer_code, file_issuance, str.upper(input_metadata.model_system), data_coding_format)
    else:
            ...
            try:
                ...
                if model_index_file.dim_subgrid is not None and model_index_file.var_subgrid_id is not None:
                    ...
                    for i in range(model_index_file.dim_subgrid.size):
                        ...
                        s111_file = S111File(filename, input_metadata, data_coding_format, model_index_file,
                          subgrid_index=i, clobber=True)
                        s111_file_paths.append(s111_file.path)
                        stack.enter_context(s111_file)
                        s111_files.append(s111_file)

                else:
                    s111_file = S111File(('{}.h5'.format(s111_path_prefix)), input_metadata, data_coding_format, model_index_file,
                      clobber=True)
                    s111_file_paths.append(s111_file.path)
                for model_file in model_files:
                    try:
                        ...
                        for time_index in range(len(model_file.datetime_values)):
                            ...
                            elif model_index_file.dim_subgrid is not None and model_index_file.var_subgrid_id is not None:
            ...
            with S111File(('{}.h5'.format(s111_path_prefix)), input_metadata, data_coding_format, clobber=True) as (s111_file):
                ...
                for model_file in model_files:
                    try:
                        model_file.open()
                        for time_index in range(len(model_file.datetime_values)):
                            u_compressed, v_compressed, lat_compressed, lon_compressed = model_file.output_native_grid(time_index, target_depth)
...
def time_series_to_s111(input_data, s111_path_prefix, input_metadata, data_coding_format, current_depth):
    ...
    if os.path.isdir(s111_path_prefix):
        ...
        s111_path_prefix += 'S111{}_{}_{}_TYP{}'.format(input_metadata.producer_code, file_issuance, input_metadata.region, data_coding_format)
        stations_longitude = []
        stations_latitude = []
        with S111File(('{}.h5'.format(s111_path_prefix)), input_metadata, data_coding_format, clobber=True) as (s111_file):
            if data_coding_format == 1:
                for station in input_data:
                    s111_file.add_feature_instance_group_data(station.datetime_values[0], station.speed, station.direction, timestamp, current_depth)
    ...
    try:
        ...
        for idx, path in enumerate(h5_files[1:], 2):
            ...
            try:
                ...
                if idx == 2:
                    first_time = datetime.datetime.strptime(output_file['/SurfaceCurrent/SurfaceCurrent.01/Group_001'].attrs['timePoint'], '%Y%m%dT%H%M%SZ')
                    second_time = datetime.datetime.strptime(input_file['/SurfaceCurrent/SurfaceCurrent.01/Group_001'].attrs['timePoint'], '%Y%m%dT%H%M%SZ')
                    time_interval_secs = (second_time - first_time).total_seconds()
