...
# [GCC 8.4.0]
# Embedded file name: /home/j/git/lab.gsi/social/orchestrator/soneti/__init__.py
# Compiled at: 2019-02-08 13:16:23
# Size of source mod 2**32: 6356 bytes
name = 'soneti_tasks'
import json, luigi, requests, time, logging
        ...
        with self.input().open('r') as (fobj):
            with self.output().open('w') as (outfile):
                ...
                for i in fobj:
                    ...
                    b['_id'] = i['@id']
                    b['@type'] = i['@type']
                    r = requests.post((self.host), data={'input':i[self.fieldName], 
                     'apiKey':self.apiKey, 
                     'algo':self.algorithm, 
                     'lang':self.lang})
    ...
    def run(self):
        ...
        with self.input().open('r') as (infile):
            with self.output().open('w') as (outfile):
                ...
                r = requests.put('http://{fuseki}:{port}/{dataset}/data'.format(fuseki=(self.host), port=(self.port),
...
class GSICrawler(luigi.Task):
    ...
    taskoutput = luigi.ChoiceParameter(description='return the results in json format, or into an elasticsearch index', choices=[
     'json', 'elasticsearch'],
      default='json')
