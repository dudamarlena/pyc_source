...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/elasticluster/cluster.py
# Compiled at: 2014-10-22 16:00:16
__author__ = 'Nicolas Baer <nicolas.baer@uzh.ch>, Antonio Messina <antonio.s.messina@gmail.com>'
import operator, os, re, signal, socket, sys, time
from multiprocessing.dummy import Pool
import paramiko
from binascii import hexlify
from elasticluster import log
from elasticluster.exceptions import TimeoutError, NodeNotFound, InstanceError, ClusterError
...
class Cluster(object):
   ...
   """
    ...
    def __init__(self, name, cloud_provider, setup_provider, user_key_name, user_key_public, user_key_private, repository=None, **extra):
        self.name = name
    ...
    def add_node(self, kind, image_id, image_user, flavor, security_group, image_userdata='', name=None, **extra):
        ...
        if not re.match('^[a-zA-Z0-9-]+$', kind):
            raise ValueError('Invalid name `%s`. The `kind` argument may only contains characters in [a-z0-9-] range, as it is going to be used as hostname' % kind)
        if kind not in self.nodes:
            self.nodes[kind] = []
        if not name:
            name = '%s%03d' % (kind, len(self.nodes[kind]) + 1)
        node = Node(name, self.name, kind, self._cloud_provider, self._user_key_public, self.user_key_private, self._user_key_name, image_user, security_group, image_id, flavor, image_userdata=image_userdata, **extra)
    ...
    def add_nodes(self, kind, num, image_id, image_user, flavor, security_group, image_userdata='', **extra):
        ...
        """
        for i in range(num):
            self.add_node(kind, image_id, image_user, flavor, security_group, image_userdata=image_userdata, **extra)
    ...
    def remove_node(self, node):
        ...
        if node.kind not in self.nodes:
            log.error('Unable to remove node %s: invalid node type `%s`.', node.name, node.kind)
        else:
    ...
    def _start_node(node):
        ...
        if node.is_alive():
            log.info('Not starting node %s which is already up&running.', node.name)
            return True
        else:
            try:
                node.start()
                ...
                log.error('could not start node `%s` for reason `%s`' % (
                 node.name, e))
                return
    ...
    def start(self, min_nodes=None):
        ...
        if log.DO_NOT_FORK:
            ...
            try:
                ...
                log.error('Not all nodes were started correctly within the given timeout `%s`' % Cluster.startup_timeout)
                for node in starting_nodes:
                    log.error('Stopping node `%s`, since it could not start within the given timeout' % node.name)
                    node.stop()
            ...
            try:
                while pending_nodes:
                    for node in pending_nodes[:]:
                        if node.connect():
                            log.info('Connection to node %s (%s) successful.', node.name, node.connection_ip())
                ...
                log.error('Could not connect to all the nodes of the cluster within the given timeout `%s`.' % Cluster.startup_timeout)
                for node in pending_nodes:
                    log.error('Stopping node `%s`, since we could not connect to it within the timeout.' % node.name)
                    node.stop()
                    self.remove_node(node)
            ...
            self.repository.save_or_update(self)
            if not min_nodes:
                min_nodes = dict((key, len(self.nodes[key])) for key in self.nodes.iterkeys())
            else:
    ...
    def _check_cluster_size(self, min_nodes):
        ...
        if len(self.get_all_nodes()) < minimum_nodes:
            raise ClusterError('The cluster does not provide the minimum amount of nodes specified in the configuration. The nodes are still running, but will not be setup yet. Please change the minimum amount of nodes in the configuration or try to start a new cluster after checking the cloud provider settings.')
        unsatisfied_groups = []
        for group, size in min_nodes.iteritems():
        ...
        if unsatisfied_groups:
            raise ClusterError('Could not find an optimal solution to distribute the started nodes into the node groups to satisfy the minimum amount of nodes. Please change the minimum amount of nodes in the configuration or try to start a new clouster after checking the cloud provider settings')
    ...
    def stop(self, force=False):
        ...
        for node in self.get_all_nodes():
            if node.instance_id:
                try:
                    ...
                    log.error('could not stop instance `%s`, it might already be down.', node.instance_id)

            else:
                log.debug('Not stopping node with no instance id. It seems like node `%s` did not start correctly.' % node.name)
                self.nodes[node.kind].remove(node)
        ...
        if not self.get_all_nodes():
            ...
            log.warning('Not all instances have been terminated. Please rerun the `elasticluster stop %s`', self.name)
            self.repository.save_or_update(self)
        else:
            log.warning('Not all instances have been terminated. However, as requested, the cluster has been force-removed.')
            self._setup_provider.cleanup(self)
            self.repository.delete(self)
    ...
    def setup(self):
        ...
        try:
            ...
            log.error('the setup provider was not able to setup the cluster, but the cluster is running by now. Setup provider error message: `%s`', str(e))
            ret = False

        if not ret:
            log.warning('Cluster `%s` not yet configured. Please, re-run `elasticluster setup %s` and/or check your configuration', self.name, self.name)
        return ret
...
class Node(object):
    ...
    def __init__(self, name, cluster_name, kind, cloud_provider, user_key_public, user_key_private, user_key_name, image_user, security_group, image, flavor, image_userdata=None, **extra):
        self.name = name
    ...
    def start(self):
        ...
        self.instance_id = self._cloud_provider.start_instance(self.user_key_name, self.user_key_public, self.user_key_private, self.security_group, self.flavor, self.image, self.image_userdata, username=self.image_user, node_name=('%s-%s' % (self.cluster_name, self.name)), **self.extra_args)
    ...
    def is_alive(self):
        ...
        if running:
            log.debug('node `%s` (instance id %s) is up and running', self.name, self.instance_id)
            self.update_ips()
        else:
            log.debug('node `%s` (instance id `%s`) still building...', self.name, self.instance_id)
        return running
    ...
    def connect(self):
        ...
        for ip in [self.preferred_ip] + ips:
            ...
            try:
                ...
                ssh.connect(ip, username=self.image_user, allow_agent=True, key_filename=self.user_key_private, timeout=Node.connection_timeout)
    ...
    def __str__(self):
        return 'name=`%s`, id=`%s`, ips=%s, connection_ip=`%s`' % (
         self.name, self.instance_id, str.join(', ', self.ips),
         self.preferred_ip)
