...
class BotoCloudProvider(AbstractCloudProvider):
    ...
    def __init__(self, ec2_url, ec2_region, ec2_access_key, ec2_secret_key, storage_path=None, request_floating_ip=False):
        self._url = ec2_url
    ...
    def _connect(self):
        ...
        try:
            ...
            self._connection = boto.connect_ec2(aws_access_key_id=self._access_key, aws_secret_access_key=self._secret_key, is_secure=self._secure, host=self._ec2host, port=self._ec2port, path=self._ec2path, region=region)
            log.debug('Connection has been successful.')
        except Exception as e:
            log.error('connection to cloud could not be established: message=`%s`', str(e))
            raise
    ...
    def _allocate_address(self, instance):
        ...
        if not free_addresses:
            try:
                ...
                log.error('Unable to allocate a public IP address to instance `%s`', instance.id)
                return
        ...
        try:
            ...
            log.error('Unable to associate IP address %s to instance `%s`', address, instance.id)
            return
    ...
    def _check_keypair(self, name, public_key_path, private_key_path):
        ...
        if name not in keypairs:
            log.warning('Keypair `%s` not found on resource `%s`, Creating a new one', name, self._url)
            with open(os.path.expanduser(public_key_path)) as (f):
                key_material = f.read()
                try:
                    if 'amazon' in self._ec2host and is_dsa_key:
                        log.error('Apparently, amazon does not support DSA keys. Please specify a valid RSA key.')
                        raise KeypairError('Apparently, amazon does not support DSA keys.Please specify a valid RSA key.')
                    connection.import_key_pair(name, key_material)
                except Exception as ex:
                    log.error('Could not import key `%s` with name `%s` to `%s`', name, public_key_path, self._url)
            ...
            if pkey:
                ...
                if fingerprint != cloud_keypair.fingerprint:
                    if 'amazon' in self._ec2host:
                        log.error('Apparently, Amazon does not compute the RSA key fingerprint as we do! We cannot check if the uploaded keypair is correct!')
                    else:
