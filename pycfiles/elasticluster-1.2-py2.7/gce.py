...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/elasticluster/providers/gce.py
...
"""
__author__ = 'Riccardo Murri <riccardo.murri@uzh.ch>, Nicolas Baer <nicolas.baer@gmail.com>, Antonio Messina <antonio.s.messina@gmail.com>'
import copy, httplib2, os, random, threading, time, uuid
...
class GoogleCloudProvider(AbstractCloudProvider):
    ...
    def __init__(self, gce_client_id, gce_client_secret, gce_project_id, zone=GCE_DEFAULT_ZONE, network='default', email=GCE_DEFAULT_SERVICE_EMAIL, storage_path=None):
    ...
    def start_instance(self, key_name, public_key_path, private_key_path, security_group, flavor, image_id, image_userdata, username=None, instance_name=None, **kwargs):
        ...
        instance = {'name': instance_name, 
           ...
           'disks': [
                   {'autoDelete': 'true', 
                      'boot': 'true', 
                      'type': 'PERSISTENT', 
                      'initializeParams': {'diskName': '%s-disk' % instance_name, 
           ...
           'networkInterfaces': [
                               {'accessConfigs': [
                                  ...
                                  'network': network_url}], 
           'serviceAccounts': [
                             {'email': self._email, 'scopes': GCE_DEFAULT_SCOPES}], 
    ...
    def list_instances(self, filter=None):
        ...
        try:
            ...
            raise InstanceError('could not retrieve all instances on the cloud: ``' % e)
    ...
    def get_ips(self, instance_id):
        ...
        try:
            request = instances.get(instance=instance_id, project=self._project_id, zone=self._zone)
            response = self._execute_request(request)
            ip_private = None
            ip_public = None
            ...
            raise InstanceError('could not retrieve the ip address for node `%s`, please check the node through the cloud provider interface' % instance_id)
        except (HttpError, CloudProviderError) as e:
            raise InstanceError('could not retrieve the ip address of `%s`: `%s`' % (
             instance_id, e))
    ...
    def _check_response(self, response):
        ...
        if 'error' in response:
            ...
            raise CloudProviderError('The following error occurred while interacting with the cloud provider `%s`' % error)
