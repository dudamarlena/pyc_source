...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/elasticluster/conf.py
...
class Configurator(object):
   ...
   :raises MultipleInvalid: configuration validation
    ...
    default_storage_dir = os.path.expanduser('~/.elasticluster/storage')
    ...
    def create_cluster(self, template, name=None):
        ...
        if template not in self.cluster_conf:
            raise ConfigurationError('Invalid configuration for cluster `%s`: %s' % (
             template, name))
        ...
        cluster = Cluster(name, self.create_cloud_provider(template), self.create_setup_provider(template, name=name), conf_login['user_key_name'], conf_login['user_key_public'], conf_login['user_key_private'], repository=self.create_repository(), **extra)
        ...
        for kind, num in nodes.iteritems():
            ...
            cluster.add_nodes(kind, num, conf_kind['image_id'], conf_login['image_user'], conf_kind['flavor'], conf_kind['security_group'], image_userdata=userdata, **extra)
    ...
    def create_setup_provider(self, cluster_template, name=None):
        ...
        return provider(groups, playbook_path=playbook_path, environment_vars=environment, storage_path=storage_path, sudo_user=conf_login['image_user_sudo'], sudo=conf_login['image_sudo'], **conf)
...
class ConfigValidator(object):
        ...
        * interpolating configuraiton options
        ...
        for cluster, props in self.config.iteritems():
            if 'setup' in props and 'playbook_path' in props['setup']:
                if props['setup']['playbook_path'].startswith('%(ansible_pb_dir)s'):
                    pbpath = props['setup']['playbook_path']
                    pbpath = pbpath.replace('%(ansible_pb_dir)s', str(ansible_pb_dir))
    ...
    def validate(self):
        ...
        schema = {'cluster': {'cloud': All(str, Length(min=1)), 'setup_provider': All(str, Length(min=1)), 
                       'login': All(str, Length(min=1))}, 
           'setup': {'provider': All(str, Length(min=1)), Optional('playbook_path'): check_file()}, 
           'login': {'image_user': All(str, Length(min=1)), 'image_user_sudo': All(str, Length(min=1)), 
                     'image_sudo': Boolean(str), 
                     'user_key_name': All(str, Length(min=1)), 
                     'user_key_private': check_file(), 
                     'user_key_public': check_file()}}
        cloud_schema_ec2 = {'provider': 'ec2_boto', 'ec2_url': Url(str), 
        ...
        cloud_schema_gce = {'provider': 'google', 'gce_client_id': All(str, Length(min=1)), 
           'gce_client_secret': All(str, Length(min=1)), 
           'gce_project_id': All(str, Length(min=1))}
        cloud_schema_openstack = {'provider': 'openstack', 'auth_url': All(str, Length(min=1)), 
        ...
        node_schema = {'flavor': All(str, Length(min=1)), 
        ...
        for cluster, properties in self.config.iteritems():
            ...
            if properties['cloud']['provider'] == 'ec2_boto':
                ...
                for node, props in properties['nodes'].iteritems():
                    match = re.search('^[a-zA-Z0-9-]*$', node)
                    if not match:
                        raise Invalid('Invalid name `%s` for node group. A valid node group can only consist of letters, digits or the hyphens character (`-`)' % node)
...
class ConfigReader(object):
    ...
    def __init__(self, configfile):
        ...
        self.schemas = {'cloud': Schema({'provider': Any('ec2_boto', 'google', 'openstack'), 'ec2_url': Url(str), 
                     'ec2_access_key': All(str, Length(min=1)), 
                     'ec2_secret_key': All(str, Length(min=1)), 
                     'ec2_region': All(str, Length(min=1)), 
           ...
           'cluster': Schema({'cloud': All(str, Length(min=1)), 'setup_provider': All(str, Length(min=1)), 
                       'login': All(str, Length(min=1))}, required=True, extra=True), 
           'setup': Schema({'provider': All(str, Length(min=1))}, required=True, extra=True), 
           'login': Schema({'image_user': All(str, Length(min=1)), 'image_user_sudo': All(str, Length(min=1)), 
                     'image_sudo': Boolean(str), 
                     'user_key_name': All(str, Length(min=1)), 
                     'user_key_private': check_file(), 
    ...
    def read_config(self):
        ...
        """
        clusters = dict((key, value) for key, value in self.conf.iteritems() if re.search(ConfigReader.cluster_section + '/(.*)', key) and key.count('/') == 1)
        conf_values = dict()
        ...
        for cluster in clusters:
            ...
            try:
                nodes = dict((key, value) for key, value in values['cluster'].items() if key.endswith('_nodes'))
                values['nodes'] = dict()
                for node in nodes.iterkeys():
                    ...
                    if property_name in self.conf:
                        node_values = dict((key, value.strip("'").strip('"')) for key, value in self.conf[property_name].iteritems())
                        node_values = dict(values['cluster'].items() + node_values.items())
