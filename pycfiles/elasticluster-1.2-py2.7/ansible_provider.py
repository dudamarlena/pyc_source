...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/elasticluster/providers/ansible_provider.py
# Compiled at: 2014-10-22 16:00:16
__author__ = 'Nicolas Baer <nicolas.baer@uzh.ch>, Antonio Messina <antonio.s.messina@gmail.com>'
import logging, os, tempfile, shutil, sys, ansible.utils, ansible.callbacks as anscb, ansible.constants as ansible_constants
...
class ElasticlusterPbCallbacks(anscb.PlaybookCallbacks):
    ...
    def on_import_for_host(self, host, imported_file):
        anscb.call_callback_module('playbook_on_import_for_host', host, imported_file)

    def on_not_import_for_host(self, host, missing_file):
        anscb.call_callback_module('playbook_on_not_import_for_host', host, missing_file)
...
class AnsibleSetupProvider(AbstractSetupProvider):
    ...
    def __init__(self, groups, playbook_path=None, environment_vars=dict(), storage_path=None, sudo=True, sudo_user='root', ansible_module_dir=None, **extra_conf):
        self.groups = groups
        ...
        if not self._playbook_path:
            self._playbook_path = os.path.join(sys.prefix, 'share/elasticluster/providers/ansible-playbooks', 'site.yml')
        else:
    ...
    def setup_cluster(self, cluster):
        ...
        if not os.path.exists(inventory_path):
            raise AnsibleError('inventory file `%s` could not be found' % inventory_path)
        if not os.path.exists(self._playbook_path):
            raise AnsibleError('playbook `%s` could not be found' % self._playbook_path)
        if not os.path.isfile(self._playbook_path):
            raise AnsibleError('the playbook `%s` is not a file' % self._playbook_path)
        ...
        pb = PlayBook(playbook=self._playbook_path, host_list=inventory_path, callbacks=playbook_cb, runner_callbacks=runner_cb, forks=10, stats=stats, sudo=self._sudo, sudo_user=self._sudo_user, private_key_file=private_key_file)
        try:
            status = pb.run()
        ...
        for host, hoststatus in status.items():
            if hoststatus['unreachable']:
                elasticluster.log.error('Host `%s` is unreachable, please re-run elasticluster setup', host)
                cluster_failures = True
            if hoststatus['failures']:
                elasticluster.log.error('Host `%s` had %d failures: please re-run elasticluster setup or check the Ansible playbook `%s`' % (
    ...
    def _build_inventory(self, cluster):
        ...
        for node in cluster.get_all_nodes():
            if node.kind in self.groups:
                extra_vars = [
                 'ansible_ssh_user=%s' % node.image_user]
                if node.kind in self.environment:
                    extra_vars.extend('%s=%s' % (k, v) for k, v in self.environment[node.kind].items())
