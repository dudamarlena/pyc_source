...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/elasticluster/subcommands.py
...
from elasticluster.exceptions import ClusterNotFound, ConfigurationError, ImageError, SecurityGroupError, NodeNotFound, ClusterError
...
def cluster_summary(cluster):
    try:
        ...
        frontend = 'unknown'
        log.error('Unable to get information on the frontend node: %s', str(ex))
...
class Start(AbstractCommand):
    ...
    def setup(self, subparsers):
        parser = subparsers.add_parser('start', help='Create a cluster using the supplied configuration.', description=self.__doc__)
        ...
        parser.add_argument('--nodes', metavar='N1:GROUP[,N2:GROUP2,...]', help='Override the values in of the configuration file and starts `N1` nodes of group `GROUP`,N2 of GROUP2 etc...')
        parser.add_argument('--no-setup', action='store_true', default=False, help='Only start the cluster, do not configure it')
    ...
    def execute(self):
        ...
        configurator = Configurator.fromConfig(self.params.config, storage_path=self.params.storage)
        ...
        try:
            ...
            try:
                cluster = configurator.create_cluster(cluster_template, cluster_name)
        ...
        try:
            ...
            min_nodes = dict((k[:-10], int(v)) for k, v in conf['cluster'].iteritems() if k.endswith('_nodes_min'))
...
class Stop(AbstractCommand):
    ...
    def setup(self, subparsers):
        ...
        """
        parser = subparsers.add_parser('stop', help='Stop a cluster and all associated VM instances.', description=self.__doc__)
        parser.set_defaults(func=self)
        parser.add_argument('cluster', help='name of the cluster')
        parser.add_argument('-v', '--verbose', action='count', default=0, help='Increase verbosity.')
        parser.add_argument('--force', action='store_true', default=False, help='Remove the cluster even if not all the nodes have been terminated properly.')
...
class ResizeCluster(AbstractCommand):
    ...
    def setup(self, subparsers):
        parser = subparsers.add_parser('resize', help='Resize a cluster by adding or removing compute nodes.', description=self.__doc__)
        parser.set_defaults(func=self)
        parser.add_argument('cluster', help='name of the cluster')
        parser.add_argument('-a', '--add', metavar='N1:GROUP1[,N2:GROUP2]', help='Add N1 nodes of group GROUP1, N2 of group GROUP2 etc...')
        parser.add_argument('-r', '--remove', metavar='N1:GROUP1[,N2:GROUP2]', help='Remove N1 nodes of group GROUP1, N2 of group GROUP2 etc...')
        parser.add_argument('-t', '--template', help='name of the template of this cluster')
        parser.add_argument('-v', '--verbose', action='count', default=0, help='Increase verbosity.')
        parser.add_argument('--no-setup', action='store_true', default=False, help='Only start the cluster, do not configure it')
        parser.add_argument('--yes', action='store_true', default=False, help='Assume `yes` to all queries and do not prompt.')
    ...
    def execute(self):
        ...
        for grp in self.params.nodes_to_add:
            ...
            if (grp not in cluster.nodes or not cluster.nodes[grp]) and not template:
                print 'Elasticluster can not infer which template to use for the new node(s). Please provide the template with the `-t` or `--template` option'
                return
            if not template:
                sample_node = cluster.nodes[grp][0]
                for i in range(self.params.nodes_to_add[grp]):
                    cluster.add_node(grp, sample_node.image, sample_node.image_user, sample_node.flavor, sample_node.security_group, image_userdata=sample_node.image_userdata)
                ...
                for i in range(self.params.nodes_to_add[grp]):
                    ...
                    cluster.add_node(grp, conf_kind['image_id'], image_user, conf_kind['flavor'], conf_kind['security_group'], image_userdata=userdata)
        ...
        for grp in self.params.nodes_to_remove:
            ...
            if not self.params.yes:
                yesno = raw_input('Do you really want to remove them? [yN] ')
                if yesno.lower() not in ('yes', 'y'):
                    print 'Aborting as per user request.'
                    sys.exit(0)
...
class ListClusters(AbstractCommand):
    ...
    def setup(self, subparsers):
        parser = subparsers.add_parser('list', help='List all started clusters.', description=self.__doc__)
        parser.set_defaults(func=self)
        parser.add_argument('-v', '--verbose', action='count', default=0, help='Increase verbosity.')
...
class ListTemplates(AbstractCommand):
    ...
    def setup(self, subparsers):
        parser = subparsers.add_parser('list-templates', description=self.__doc__, help='Show the templates defined in the configuration file.')
        parser.set_defaults(func=self)
        parser.add_argument('-v', '--verbose', action='count', default=0, help='Increase verbosity.')
...
class ListNodes(AbstractCommand):
    ...
    def setup(self, subparsers):
        parser = subparsers.add_parser('list-nodes', help='Show information about the nodes in the cluster', description=self.__doc__)
        parser.set_defaults(func=self)
        parser.add_argument('cluster', help='name of the cluster')
        parser.add_argument('-v', '--verbose', action='count', default=0, help='Increase verbosity.')
        parser.add_argument('-u', '--update', action='store_true', default=False, help='By default `elasticluster list-nodes` will not contact the EC2 provider to get up-to-date information, unless `-u` option is given.')
...
class SetupCluster(AbstractCommand):
    ...
    def setup(self, subparsers):
        parser = subparsers.add_parser('setup', help='Configure the cluster.', description=self.__doc__)
        parser.set_defaults(func=self)
        parser.add_argument('cluster', help='name of the cluster')
        parser.add_argument('-v', '--verbose', action='count', default=0, help='Increase verbosity.')
...
class SshFrontend(AbstractCommand):
    ...
    def setup(self, subparsers):
        parser = subparsers.add_parser('ssh', help='Connect to the frontend of the cluster using the `ssh` command', description=self.__doc__)
        parser.set_defaults(func=self)
        parser.add_argument('cluster', help='name of the cluster')
        parser.add_argument('-v', '--verbose', action='count', default=0, help='Increase verbosity.')
        parser.add_argument('ssh_args', metavar='args', nargs='*', help='Execute the following command on the remote machine instead of opening an interactive shell.')
...
class SftpFrontend(AbstractCommand):
    ...
    def setup(self, subparsers):
        parser = subparsers.add_parser('sftp', help='Open an SFTP session to the cluster frontend host.', description=self.__doc__)
        parser.set_defaults(func=self)
        parser.add_argument('cluster', help='name of the cluster')
        parser.add_argument('-v', '--verbose', action='count', default=0, help='Increase verbosity.')
        parser.add_argument('sftp_args', metavar='args', nargs='*', help='Arguments to pass to ftp, instead of opening an interactive shell.')
