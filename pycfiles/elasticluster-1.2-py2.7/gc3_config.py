...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/elasticluster/gc3_config.py
# Compiled at: 2014-04-28 06:26:44
...
slurm_sinfo_regexp = re.compile('^(?P<hostnames>[^ \t]+)\\s+(?P<num>[0-9]+)\\s+(?P<partition>[^ \t]+)\\s+(?P<state>[^ \t]+)\\s+(?P<cpus>[0-9]+)\\s+(?P<S>[0-9]+):(?P<C>[0-9]+):(?P<T>[0-9]+)\\s+(?P<memory>[0-9]+)\\s+(?P<tmp_disk>[0-9]+)\\s+(?P<weight>[0-9]+)\\s+(?P<features>[^ ]+)\\s+(?P<reason>[^ \t]+)')
slurm_scontrol_regexp = re.compile('PartitionName=(?P<PartitionName>[^ \t]+)\\s+AllocNodes=(?P<AllocNodes>[^ \t]+)\\s+AllowGroups=(?P<AllowGroups>[^ \t]+)\\s+Default=(?P<Default>[^ \t]+)\\s+DefaultTime=(?P<DefaultTime>[^ \t]+)\\s+DisableRootJobs=(?P<DisableRootJobs>[^ \t]+)\\s+GraceTime=(?P<GraceTime>[^ \t]+)\\s+Hidden=(?P<Hidden>[^ \t]+)\\s+MaxNodes=(?P<MaxNodes>[^ \t]+)\\s+MaxTime=(?P<MaxTime>[^ \t]+)\\s+MinNodes=(?P<MinNodes>[^ \t]+)\\s+Nodes=(?P<Nodes>[^ \t]+)\\s+Priority=(?P<Priority>[^ \t]+)\\s+RootOnly=(?P<RootOnly>[^ \t]+)\\s+Shared=(?P<Shared>[^ \t]+)\\s+PreemptMode=(?P<PreemptMode>[^ \t]+)\\s+State=(?P<State>[^ \t]+)\\s+TotalCPUs=(?P<TotalCPUs>[^ \t]+)\\s+TotalNodes=(?P<TotalNodes>[^ \t]+)\\s+DefMemPerNode=(?P<DefMemPerNode>[^ \t]+)\\s+MaxMemPerNode=(?P<MaxMemPerNode>[^ \t]+)')
