...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/test_cluster.py
...
class TestCluster(unittest.TestCase):
    ...
    def get_cluster(self, cloud_provider=None, config=None, nodes=None):
        ...
        cluster = Cluster('mycluster', cloud_provider, setup, repository, conf_login['user_key_name'], conf_login['user_key_public'], conf_login['user_key_private'])
        if not nodes:
            nodes = {'compute': 2, 'frontend': 1}
        for kind, num in nodes.iteritems():
            conf_kind = configurator.cluster_conf['mycluster']['nodes'][kind]
            cluster.add_nodes(kind, num, conf_kind['image_id'], conf_login['image_user'], conf_kind['flavor'], conf_kind['security_group'])
    ...
    def test_add_node(self):
        ...
        cluster.add_node('compute', 'image_id', 'image_user', 'flavor', 'security_group')
        ...
        name = 'test-node'
        size = len(cluster.nodes['compute'])
        cluster.add_node('compute', 'image_id', 'image_user', 'flavor', 'security_group', image_userdata='', name=name)
        self.assertEqual(size + 1, len(cluster.nodes['compute']))
    ...
    def test_start(self):
        ...
        cloud_provider.start_instance.return_value = 'test-id'
    ...
    def test_check_cluster_size(self):
        ...
        cluster = self.get_cluster(nodes=nodes)
        self.failUnlessRaises(ClusterError, cluster._check_cluster_size, min_nodes=nodes_min)
    ...
    def test_stop(self):
        ...
        cluster.repository = MagicMock()
        cluster.stop()
        cloud_provider.stop_instance.assert_called_with('test-id')
...
class TestNode(unittest.TestCase):
    ...
    def get_node(self):
        cloud_provider = MagicMock()
        node = Node(self.name, self.cluster_name, self.node_kind, cloud_provider, self.user_key_public, self.user_key_private, self.user_key_name, self.image_user, self.security_group, self.image, self.flavor, self.image_userdata)
        return node
    ...
    def test_start(self):
        ...
        cloud_provider.start_instance.assert_called_once_with(self.user_key_name, self.user_key_public, self.user_key_private, self.security_group, self.flavor, self.image, self.image_userdata, username=self.image_user, node_name=node_name)
