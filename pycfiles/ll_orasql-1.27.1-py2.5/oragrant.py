...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/ll/orasql/scripts/oragrant.py
...
def main(args=None):
    ...
    for (i, obj) in enumerate(connection.iterprivileges(schema='user')):
        ...
        if options.verbose:
            if options.execute:
                msg = astyle.style_default('oragrant.py: ', cs1, ' -> ', cs2, ': fetching/granting #%d' % (i + 1))
            else:
                msg = astyle.style_default('oragrant.py: ', cs1, ' fetching #%d' % (i + 1))
                ...
                if options.execute:
                    try:
                        ...
                        if not options.ignore or 'ORA-01013' in str(exc):
                            raise
                        stderr.writeln('oragrant.py: ', s4error('%s: %s' % (exc.__class__.__name__, str(exc).strip())))
