...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/ll/orasql/scripts/oramerge.py
# Compiled at: 2009-01-21 07:14:09
...
def main(args=None):
    ...
    for (i, obj) in enumerate(allobjects):
        ...
        if in1 != in2:
            if not in1 and in2 and not in3:
                if options.verbose:
                    stderr.writeln(' => ', s4action('new (create it)'))
                countcreate += 1
                action = 'create'
            elif not in1 and in2 and in3:
            ...
            elif in1 and not in2 and not in3:
                if options.verbose:
                    stderr.writeln(' => removed (not needed)')
            elif in1 and not in2 and in3:
                if options.verbose:
                    ...
                    for field in fields1 | fields2 | fields3:
                        ...
                        if in1 != in2:
                            if not in1 and in2 and not in3:
                                ...
                                stdout.writeln(conflictmarker('<<<<<<<', 'added in ', cs(connection2), ' and ', cs(connection3), ' with different content'))
                            elif in1 and not in2 and not in3:
                                pass
                            elif in1 and not in2 and in3:
                                fieldcountdrop += 1
