...
# [GCC 8.4.0]
# Embedded file name: /marrow/mongo/query/query.py
...
class Q(object):
    ...
    def __getitem__(self, name):
        ...
        if self._combining:
            raise KeyError()
        if self._field.__foreign__ != 'array':
    ...
    def __unicode__(self):
        ...
        if self._combining:
            return {__and__: '$and', __or__: '$or', __xor__: '$$xor'}[self._combining]
        else:
            return self._name
    ...
    def _op(self, operation, other, *allowed):
        ...
        if self._combining:
            ...
            if __debug__:
                if _complex_safety_check(f, {operation} | set(allowed)):
    ...
    def _iop(self, operation, other, *allowed):
        ...
        f = self._field
        if self._combining:
            return reduce(self._combining, ((q._iop)(operation, other, *allowed) for q in f))
        else:
            if __debug__:
                if _complex_safety_check(f, {operation} | set(allowed)):
                    raise NotImplementedError('{self!r} does not allow {op} comparison.'.format(self=self,
                      op=operation))
    ...
    def __eq__(self, other):
        ...
        if self._combining:
            ...
            if __debug__:
                if _simple_safety_check(self._field, '$eq'):
    ...
    def _combine(self, other, operation):
        ...
        if self._combining and self._combining is operation:
            if other._combining:
                if other._combining is operation:
            ...
            if other._combining:
                if other._combining is operation:
                    return self.__class__(self._document, [
                     self] + other._field, None, operation)
    ...
    def re(self, *parts):
        r"""Matches string values against a regular expression compiled of individual parts.
    ...
    def match(self, q):
        ...
        if self._combining:
            ...
            if __debug__:
                if _complex_safety_check(self._field, {'$elemMatch', '#document'}):
    ...
    def size(self, value):
        ...
        if self._combining:
            ...
            if __debug__:
                if _complex_safety_check(self._field, {'$size', '#array'}):
