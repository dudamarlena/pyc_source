...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/tracbmacros/wiki.py
# Compiled at: 2010-07-17 13:53:21
"""Display information about wiki pages
...
class WikiHistoryMacro(WikiMacroBase):
    """Embed information about wiki changelog in wiki pages.
    ...
    """
    table_class = Option('wiki', 'chgtable_class', 'listing', doc=_('CSS class to render wiki history table'))
    labels = ListOption('wiki', 'chgtable_titles', 'Version, Date, Author, Comment', keep_empty=True, doc=_('Comma separated list of column labels for version, time, author, and comment (in that order)'))
    ...
    def expand_macro(self, formatter, name, content):
        ...
        ATTR_LABELS = len(ATTR_LABELS) == len(ALL_ATTRS) and ATTR_LABELS or [_('Version'), _('Date'), _('Author'), _('Comment')]

        def format_verinfo(value, attridx):
            return {1: lambda x: tag.i(format_datetime(x, '%d/%m/%Y %H:%M:%S', tzinfo=localtz)), 
               0: lambda x: tag.b(to_unicode(x, 'utf-8')), 
               3: lambda x: to_unicode(x or _('(No comments)'), 'utf-8'), 
               2: lambda x: tag.i(to_unicode(x, 'utf-8'))}.get(attridx, lambda x: tag.i(_('Atributo desconocido')))(value)
        ...
        if attr is None:
            ...
            return tag.table(tag.thead([tag.tr([ tag.th(tag.i(h or '')) for h in (ATTR_LABELS[c] for c in cols) ])]), tag.tbody([ tag.tr([ tag.td(format_verinfo(x[c], c)) for c in cols ]) for x in page.get_history()
