...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/tracbmacros/config.py
# Compiled at: 2010-07-18 00:43:13
...
class ConfigOptionMacro(WikiMacroBase):
    ...
    """
    OPTION_MAP = {Option: 'text', BoolOption: 'bool', 
       IntOption: 'int', 
       ListOption: 'list', 
       PathOption: 'path', 
    ...
    def expand_macro(self, formatter, name, content):
        ...
        try:
            ...
            if 'TRAC_ADMIN' not in req.perm and all((p != '*' and p not in req.perm for _o in perm_options for p in getlist('config-perm', _o, keep_empty=False))):
                self.log.warning('Preventing user `%s` from reading option `%s` in `%s`', req.authname, o, s)
                raise TracError(_('Insufficient privileges to perform this operation.') + _(' Check %s, %s, %s in trac.ini') % perm_options)
            opt_type = kw.get('type', 'auto')
            self.log.debug('Rendering config option %s in %s using %s', o, s, opt_type)
    ...
    def _render_auto(self, s, o, opts):
        ...
        if opt is not None:
            ...
            return getattr(self, '_render_' + opt_type, self._do_render_unknown)(s, o, opts)
        return
    ...
    def _render_bool(self, s, o, opts):
        ...
        return tag.input(type='radio', checked=value in _TRUE_VALUES and 'true' or None, disabled='true')
    ...
    def _render_list(self, s, o, opts):
        value = self.config.getlist(s, o, None, opts.get('sep', ','), opts.get('keep_empty', 'true').lower() in _TRUE_VALUES)
        if value:
    ...
    def _render_extension(self, s, o, opts):
        try:
            interface = opts.get('interface') or Option.registry.get((s, o)).xtnpt.interface
        except:
        ...
        try:
            ...
            if value is not None:
                return tag.span(tag.strike(value), tag.span(_('missing ?'), style='color: red;'))
            else:
    ...
    def _render_extension_list(self, s, o, opts):
        ...
        if components:
            return tag.ol(([tag.li(self._render_extension(None, c, opts))] for c in components))
        else:
