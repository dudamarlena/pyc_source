...
# [GCC 8.4.0]
# Embedded file name: /Users/mengliu/Dropbox/Research/graph_lib/graph_lib/wrappers/python/ppr_path.py
# Compiled at: 2017-09-07 23:37:09
...
def ppr_path(n, ai, aj, alpha, eps, rho, seedids, nseedids, xlength):
    ...
    class rank_info(Structure):
        _fields_ = [
         (
          'starts', POINTER(c_int64 * maxstep)),
         (
    ...
    rank_stats = rank_info(pointer(starts), pointer(ends), pointer(nodes), pointer(deg_of_pushed), pointer(size_of_solvec), pointer(size_of_r), pointer(val_of_push), pointer(global_bcond), pointer(nrank_changes), pointer(nrank_inserts), pointer(nsteps), pointer(size_for_best_cond))
    eps_stats = path_info(pointer(num_eps), pointer(epsilon), pointer(conds), pointer(cuts), pointer(vols), pointer(setsizes), pointer(stepnums))
    ...
    return (
     actual_length, actual_xids, ret_eps_stats, ret_rank_stats)