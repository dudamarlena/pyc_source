...
# [GCC 8.4.0]
# Embedded file name: /home/thejoeejoee/projects/VUT-FIT-IFJ-2017-tests/ifj2017/test/loader.py
...
class TestLoader(object):
    ...
    def load_section_dirs(self):
        return sorted(path.join(self._tests_dir, dir_) for dir_ in os.listdir(self._tests_dir) if path.isdir(path.join(self._tests_dir, dir_)) and self._allow_wildcard(dir_))
    ...
    def load_tests(self, section_dir):
        ...
        return sorted(tests, key=lambda test: (
         len(test.name), test.name))

    def _load_json_file(self, section_dir):
        data = self.load_file(path.join(section_dir, 'tests.json'), allow_fail=True)
        if not data:
            return {}
        try:
            return json.loads(data)
        except (json.JSONDecodeError, TypeError) as e:
            TestLogger.log_warning('File {} is not valid json to load ({}).'.format(path.join(section_dir, 'tests.json'), e))
            return {}
    ...
    def _load_compact_tests(self, section_dir, data):
        ...
        try:
            for i, test_case in enumerate(data.get('tests', ())):
                ...
                if name and code:
                    TestLogger.log_warning('Redefined test {} in file {}, skipping.'.format(name, path.join(section_dir, 'tests.json')))
                    continue
                    if not name:
                        name = '{:03}'.format(i + 1)
                    ...
                    if not code and not test_case.get('allow_empty'):
                        ...
                        continue
                        cases.append(TestInfo(name, code, test_case.get('stdin') or self._load_test_file(section_dir, name, 'stdin'), test_case.get('stdout') or self._load_test_file(section_dir, name, 'stdout'), int(test_case.get('compiler_exit_code') or self._load_test_file(section_dir, name, 'cexitcode') or 0), int(test_case.get('interpreter_exit_code') or self._load_test_file(section_dir, name, 'iexitcode') or 0), test_case.get('info') or self._load_test_file(section_dir, name, 'info') or self._get_code_info(code), section_dir, set(tuple(test_case.get('extensions', ())) + extensions), test_case.get('timeout') or self._default_timeout))
            ...
            if name in already_loaded:
                ...
                if not self._allow_wildcard(basename(section_dir), name):
                    ...
                    try:
                        ...
                        info = TestInfo(name, code, self._load_test_file(section_dir, name, 'stdin'), self._load_test_file(section_dir, name, 'stdout'), int(self._load_test_file(section_dir, name, 'cexitcode') or 0), int(self._load_test_file(section_dir, name, 'iexitcode') or 0), self._load_test_file(section_dir, name, 'info') or self._get_code_info(code) or '', section_dir, set(extensions), self._default_timeout)
    ...
    def _allow_wildcard(self, section, name=None):
        if not self._tests_wildcards:
            return True
        if name:
            return any(section_wc in section and (name_wc is None or name == name_wc) for section_wc, name_wc in self._tests_wildcards)
        return any(section_wc in section for section_wc, name_wc in self._tests_wildcards)
    ...
    def _load_test_file(self, section_dir, test_name, type_):
        return (self.load_file(path.join(section_dir, '.'.join((test_name, type_))), allow_fail=True) or '').replace('\r\n', '\n').replace('\r', '\n') or ''

    @staticmethod
    def load_file(file, allow_fail=False):
        if not allow_fail:
            assert path.isfile(file) and os.access(file, os.R_OK)
            try:
