...
# [GCC 8.4.0]
# Embedded file name: /home/thejoeejoee/projects/VUT-FIT-IFJ-2017-tests/ifj2017/interpreter/instruction.py
# Compiled at: 2017-12-06 12:07:35
...
def odd_round(v: int) -> int:
    return int((
     ...
     math.ceil,
     math.floor,
     lambda v: math.floor(v) if v % 2.0 == 1.5 else math.ceil(v))[int(v % 2 // 0.5)](v))
...
class Instruction(object):
    ...
    _commands = {'MOVE': State.set_value, 
     ...
     'READ': State.read, 
     'TYPE': lambda state, op0, op1: state.set_value(op0, type(state.get_value(op1)).__name__ if state.get_value(op1) is not None else ''), 
     ...
     'FLOAT2R2EINT': lambda state, op0, op1: state.set_value(op0, even_round(state.get_value(op1))), 
     
     'FLOAT2R2OINT': lambda state, op0, op1: state.set_value(op0, odd_round(state.get_value(op1))), 
     
     'INT2CHAR': lambda state, to, what: state.set_value(to, chr(state.get_value(what))), 
     'STRI2INT': lambda state, to, what, index: state.set_value(to, ord(state.get_value(what)[state.get_value(index)])), 
     ...
     'FLOAT2R2EINTS': lambda state: state.push_stack(even_round(state.pop_stack())), 
     
     'FLOAT2R2OINTS': lambda state: state.push_stack(odd_round(state.pop_stack())), 
