...
# Size of source mod 2**32: 16602 bytes
import difflib, os, os.path as path, platform, shutil
...
_STDOUT_TWICE = 'EXPECTED INTERPRETER STDOUT:\n\n{}\n\n# CURRENT INTERPRETER STDOUT:\n\n{}\n'
_STDOUT_DIFF = 'STDOUT DIFF:\n# {}\n'

class TestRunner(object):
    INTERPRETERS = {'Linux': path.join(__PROJECT_ROOT__, 'ifj2017/bin/linux/ic17int'), 
    ...
    def __init__(self, args):
        ...
        if not args.no_interpreter:
            ...
            self._loader = TestLoader(args.tests_dir, args.command_timeout, args.tests)
            ...
            self._uploader = BenchmarkUploader(args.benchmark_url_target, token_file)
    ...
    def run(self):
        ...
        if self._uploader.version != ifj2017.version:
            TestLogger.log_warning('Outdated package version, actual version on PyPI is {}.'.format(self._uploader.version))
        return result
    ...
    def _run_test(self, test_info):
        ...
        report.skipped = None
        TestLogger.log_test(test_info.name, ' ({})'.format(test_info.info.strip().rstrip('.').lower()) if test_info.info else None)
        if test_info.extensions - self._extensions:
            TestLogger.log(TestLogger.GREEN, ' skipping, required extension(s) {} is not activated.'.format(', '.join(test_info.extensions - self._extensions)), end=False)
            report.success = None
            report.skipped = True
        ...
        if test_info.compiler_exit_code is not None and report.compiler_exit_code != test_info.compiler_exit_code:
        ...
        if test_info.interpreter_exit_code is not None and report.interpreter_exit_code != test_info.interpreter_exit_code:
        ...
        if report.interpreter_exit_code != 0:
            ...
            return
        TestLogger.log_test_ok()
        if test_info.stdout is not None and report.interpreter_stdout != test_info.stdout:
    ...
    def _compile(self, test_info):
        ...
        return (
         out.decode('raw_unicode_escape'), err.decode('raw_unicode_escape'), process.returncode)
    ...
    def _interpret(self, report: TestReport, test_info: TestInfo) -> None:
        ...
        try:
            try:
                out, err = process.communicate(input=bytes(test_info.stdin, encoding='utf-8'), timeout=test_info.timeout)
            except (TimeoutError, TimeoutExpired):
                process.kill()
                raise
    ...
    def _save_report(self, test_info, report):
        ...
        with open(path.join(self._log_dir, self._actual_section, '.'.join((test_info.name, 'IFJcode17'))), 'wb') as (f):
            write = lambda v: f.write(bytes(v, encoding='utf-8'))
            write(TEST_LOG_HEADER.format(datetime.now(), basename(test_info.section_dir), test_info.name, test_info.info, ', '.join(test_info.extensions), ', '.join(self._extensions), 'SUCCESS' if report.success else 'FAIL', replace(report.compiler_stderr), replace(report.interpreter_stderr), self._stdout_log(test_info.stdout, report.interpreter_stdout), replace(test_info.stdin), test_info.compiler_exit_code, report.compiler_exit_code, test_info.interpreter_exit_code, report.interpreter_exit_code, '{} ({}+{})'.format(report.state.price, report.state.instruction_price, report.state.operand_price) if report.state else '---', report.groot_price if report.groot_price is not None else '---'))
            write('\n'.join('# {: 3}: {}'.format(i, line) for i, line in enumerate(test_info.code.splitlines(), start=1)))
            write('\n\n' + '#' * 40 + '\n\n')
            lines = (report.compiler_stdout or '').splitlines()
            count = len(lines)
            write('\n'.join('{:80}# {:5}/{}'.format(line, i, count) for i, line in enumerate(lines, start=1) if line) or '# ---')
            write('\n\n')
    ...
    def _welcome_message(self):
        TestLogger.log(TestLogger.BLUE, TestLogger.BOLD, 'Welcome to automatic test runner for IFJ17 compiler {} (https://github.com/thejoeejoee/VUT-FIT-IFJ-2017-tests).'.format(ifj2017.__version__))
        if self._extensions:
            TestLogger.log(TestLogger.GREEN, TestLogger.BOLD, 'Activated extensions: {}{}.'.format(', '.join(sorted(self._extensions)), ' from {}'.format(self._extensions_auto_loaded_from) if self._extensions_auto_loaded_from else ''))
        if self._no_interpreter:
            TestLogger.log(TestLogger.WARNING, TestLogger.UNDERLINE, TestLogger.BOLD, 'WARNING: Running without interpreter - without STDOUT check.')
    ...
    def _try_load_extensions(self, extensions_file, compiler_path):
        if not extensions_file:
            alt_paths = tuple(path_ for path_ in (
             ...
             abspath(path.join(path.dirname(compiler_path), '..', self.EXTENSION_FILE_NAME)),
             abspath(path.join(path.dirname(compiler_path), '../..', self.EXTENSION_FILE_NAME))) if isfile(path_) and os.access(path_, os.R_OK))
            if not alt_paths:
                return set()
            extensions_file = alt_paths[0]
            self._extensions_auto_loaded_from = extensions_file
        if not path.isfile(extensions_file):
            TestLogger.log_warning("File '{}' is not file or is not readable, assuming none of extensions.".format(extensions_file))
            return set()
    ...
    def _stdout_log(self, stdout, interpreter_stdout):
        if self._no_stdout_diff or not stdout or not interpreter_stdout:
            return _STDOUT_TWICE.format(''.join('# {}\n'.format(line) for line in (stdout or '').splitlines()), ''.join('# {}\n'.format(line) for line in (interpreter_stdout or '').splitlines()))
        diff = tuple(difflib.unified_diff(stdout.splitlines(True), interpreter_stdout.splitlines(True), fromfile='expected', tofile='actual'))
