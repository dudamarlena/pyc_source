...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/spl/commands/disable.py
# Compiled at: 2016-12-14 12:12:54
...
def run(spiget, args):
    with State.load(spiget) as (state):
        try:
            ...
            if resource_state == ResourceState.INSTALLED_DISABLED:
                print('{} is already disabled.'.format(resource.name))
                return ExitCode.ALREADY_DISABLED
            if resource_state == ResourceState.INSTALLED_ENABLED:
