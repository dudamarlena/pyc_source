...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/alpha1p/robot/bthandler.py
# Compiled at: 2020-04-14 08:04:08
# Size of source mod 2**32: 39850 bytes
import serial
from math import ceil
from struct import unpack, pack
__all__ = [
 'packageCommand', 'usb_reformat_response', 'parsing_all_response', 'parsing_gen_resp',
...
def packageCommand(parameters, cmdType):
    ...
    """
    type2Number = {'handShake':1, 
     'get_action_list':2,  'execute_action_table':3,  'stop_play':5,  'sound_switch':6, 
     'play_switch':7,  'heart_beat':8,  'robot_status':10,  'set_sound':11,  'power_down':12, 
     'light_switch':13,  'set_time':14,  'read_alarm':15,  'set_alarm':16,  'soft_version':17, 
     'power_info':24,  'hard_version':32,  'single_control':34,  'multi_Control':35,  'read_back_single':36, 
     'read_back_all':37,  'set_single_offset':38,  'set_all_offset':39,  'read_single_offset':40, 
     'read_all_offset':41,  'single_geer_version':42,  'all_geer_version':43,  'play_and_charge':50, 
     'get_sn':51,  'get_udid':52}
    cmd_list = [251, 191]
    cmd_list.append(len(parameters) + 5)
    cmd_list.append(type2Number[cmdType])
    for x in parameters:
        cmd_list.append(x)
...
def usb_reformat_response(raw_response):
    ...
    while p_end < len(raw_response) - 1:
        ...
        if response != b'' and response[(-1)] != 237:
            ...
            print('命令解析错误，命令结尾不是í，错误命令为：%s' % raw_response[p_start:p_end + 1])
            return 0
...
def parsing_all_response(response):
    ...
    cmd：空
    """
    type2function = {1:parsing_handshake, 
     2:parsing_get_action_list,  3:parsing_execute_action_table,  5:parsing_gen_resp, 
     6:parsing_gen_resp,  7:parsing_gen_resp,  8:parsing_gen_resp,  10:parsing_get_robot_status, 
     11:parsing_gen_resp,  12:parsing_gen_resp,  13:parsing_gen_resp, 
     14:parsing_gen_resp,  15:parsing_read_alarm,  16:parsing_gen_resp, 
     17:parsing_soft_version,  24:parsing_power_info,  25:parsing_low_voltage_alarm, 
     32:parsing_hard_version,  34:parsing_single_control,  35:parsing_multi_control, 
     36:parsing_rb_single,  37:parsing_rb_all,  38:parsing_set_single_offsset, 
     39:parsing_set_all_offsset,  40:parsing_read_single_offsset, 
     41:parsing_read_all_offsset,  42:parsing_single_geer_version, 
     43:parsing_all_geer_version,  49:parsing_play_end,  50:parsing_gen_resp, 
     51:parsing_sn,  52:parsing_udid}
    cmd_type = response[3]
...
def parsing_gen_resp(response, tips=True):
    ...
    if tips:
        s = {5:'停止运行', 
         6:'声音开关',  7:'运动开关',  8:'心跳包',  10:'状态控制相关的',  11:'设置音量',  12:'所有舵机掉电',  13:'灯光控制', 
         14:'设置时间',  16:'设置闹钟',  50:'边玩边充电'}.get(cmd_type)
        print('已收到%s命令！' % s)
...
def parsing_get_action_list(response, tips=False):
    ...
    if cmd_type != 129 or response[(poffset + 4)] != 0:
...
def parsing_get_robot_status(response, tips=False):
    ...
    return (
     is_mute, play_status, sound_size, light_status, tf_card)
...
def parsing_read_alarm(response, tips=False):
    ...
    if tips:
        ...
        times = '{}:{}:{}'.format(str(alarm['hour']).rjust(2, '0'), str(alarm['min']).rjust(2, '0'), str(alarm['sec']).rjust(2, '0'))
        print('======机器人闹钟======\n闹钟开关: %s\n每日重复: %s\n响铃时间: %s\n闹钟舞蹈: %s\n' % (
...
def get_set_alarm_cmd(alarm):
    ...
    """
    parameters = [
     int(alarm['switch']), int(alarm['every_day']), alarm['hour'], alarm['min'],
     alarm['sec']]
...
def get_single_control_cmd(cmd):
    ...
    parameters_list = []
    parameters_list.append(joint_id)
    parameters_list.append(angle)
    parameters_list.append(runtime % 256)
    parameters_list.append(nextTime // 256)
    parameters_list.append(nextTime % 256)
