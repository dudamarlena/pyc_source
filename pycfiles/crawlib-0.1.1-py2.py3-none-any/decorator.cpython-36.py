...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/crawlib-project/crawlib/decorator.py
# Compiled at: 2019-12-26 18:53:20
...
def validate_func_def(func, response_arg='response', html_arg='html', soup_arg='soup', rhtml_arg='rhtml'):
    """
    ...
    for arg in [response_arg, html_arg]:
        if arg not in arg_spec.args:
            raise NotImplementedError('`{func}` has to take the keyword syntax input, example: {func}({response_arg}=None, {html_arg}=None, ..., **kwargs)'.format(func=(get_func_fullname(func)),
              response_arg=response_arg,
              html_arg=html_arg))
        else:
            if args_with_defaults_mapper.get(arg) is not None:
                raise NotImplementedError('`{func}` default value for arg `{arg}` has to be None!'.format(func=(get_func_fullname(func)),
                  arg=arg))

    if (soup_arg in arg_spec.args) + (rhtml_arg in arg_spec.args) > 1:
        raise NotImplementedError('`{func}` can only at none or only one of `{soup_arg}`, `{rhtml_arg}` arguments!'.format(func=(get_func_fullname(func)),
          soup_arg=soup_arg,
          rhtml_arg=rhtml_arg))
...
def resolve_arg(response_arg='response', html_arg='html', soup_arg='soup', rhtml_arg='rhtml', encoding='utf-8', encoding_error_handler=decoder.ErrorsHandle.ignore):
    """
    ...
    def deco(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if html is None:
                ...
                try:
                    html = decoder.decode(binary=binary,
                      url=(response.url),
                      encoding=encoding,
