...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/crawlib-project/crawlib/entity/sql/entity.py
# Compiled at: 2019-12-30 22:29:08
# Size of source mod 2**32: 17348 bytes
...
class SqlEntity(ExtendedBase, Entity):
    ...
    @classmethod
    def _pre_process_only_fields_arg(cls, only_fields: List[Union[(sa.Column, str)]]) -> Tuple[(
 bool, List[str], List[sa.Column])]:
    ...
    @classmethod
    def _build_query_to_get_unfinished_or_finished(cls, is_finished: bool, session: Session, filters: Union[(List[BinaryExpression], Tuple[BinaryExpression])]=None, only_fields: Union[(List[sa.Column], Tuple[sa.Column])]=None, order_by: List=None, limit: int=None, **kwargs) -> Tuple[(bool, Query)]:
        """
        Build sqlalchemy ORM query object from arguments.
        """
        is_partial_load, only_column_names, only_column_objects = cls._pre_process_only_fields_arg(only_fields)
        if is_partial_load:
            query = (session.query)(*only_column_objects)
        else:
            query = session.query(cls)
        if is_finished:
            final_filters = [getattr(cls, cls.CONF_STATUS_KEY) >= FINISHED_STATUS_CODE,
             getattr(cls, cls.CONF_EDIT_AT_KEY) >= x_seconds_before_now(cls.CONF_UPDATE_INTERVAL)]
        else:
            final_filters = [
             sa.or_(getattr(cls, cls.CONF_STATUS_KEY) < FINISHED_STATUS_CODE, getattr(cls, cls.CONF_EDIT_AT_KEY) < x_seconds_before_now(cls.CONF_UPDATE_INTERVAL))]
        ...
        return (
         is_partial_load, query)

    @classmethod
    def _get_unfinished(cls, session: Session, filters: Union[(List[BinaryExpression], Tuple[BinaryExpression])]=None, only_fields: Union[(List[sa.Column], Tuple[sa.Column])]=None, limit: int=None, **kwargs) -> List['SqlEntity']:
        return (cls.get_unfinished)(session=session, 
         filters=filters, 
         only_fields=only_fields, 
         limit=limit, **kwargs)

    @classmethod
    def get_unfinished(cls, session: Session, filters: Union[(List[BinaryExpression], Tuple[BinaryExpression])]=None, only_fields: Union[(List[sa.Column], List[str])]=None, order_by: List=None, limit: int=None, **kwargs) -> List['SqlEntity']:
        """
        ...
        """
        is_partial_load, query = (cls._build_query_to_get_unfinished_or_finished)(is_finished=False, 
         session=session, 
         filters=filters, 
         only_fields=only_fields, 
         order_by=order_by, 
         limit=limit, **kwargs)
        if is_partial_load:
            return [cls(**row._asdict()) for row in query]
        else:
            return list(query)

    @classmethod
    def count_unfinished(cls, session: Session, filters: Union[(List[BinaryExpression], Tuple[BinaryExpression])]=None, limit: int=None, **kwargs) -> int:
        return (cls._build_query_to_get_unfinished_or_finished)(is_finished=False, 
         session=session, 
         filters=filters, 
         only_fields=None, 
         limit=limit, **kwargs)[1].count()

    @classmethod
    def get_finished(cls, session: Session, filters: Union[(List[BinaryExpression], Tuple[BinaryExpression])]=None, only_fields: Union[(List[sa.Column], List[str])]=None, order_by: List=None, limit: int=None, **kwargs) -> List['SqlEntity']:
        """
        ...
        :return: an iterable object of ORM entities
        """
        is_partial_load, query = (cls._build_query_to_get_unfinished_or_finished)(is_finished=True, 
         session=session, 
         filters=filters, 
         only_fields=only_fields, 
         order_by=order_by, 
    ...
    @classmethod
    def count_finished(cls, session: Session, filters: Union[(List[BinaryExpression], Tuple[BinaryExpression])]=None, limit: int=None, **kwargs) -> int:
        return (cls._build_query_to_get_unfinished_or_finished)(is_finished=True, 
         session=session, 
         filters=filters, 
         only_fields=None, 
         limit=limit, **kwargs)[1].count()
    ...
    def _validate_orm_related(cls):
        ...
        """
        try:
            status_column = getattr(cls, cls.CONF_STATUS_KEY)
            if not isinstance(status_column.type, sa.Integer):
                raise NotImplementedError('`{}.{}` column has to be a `sqlalchemy.Column(sqlalchemy.Integer, ...)`!'.format(cls.__name__, status_column))
        except:
            raise NotImplementedError('status column (a sqlalchemy.Column) not found!')

        try:
            edit_at_column = getattr(cls, cls.CONF_EDIT_AT_KEY)
            if not isinstance(edit_at_column.type, sa.DateTime):
                raise NotImplementedError('`{}.{}` column has to be a `sqlalchemy.Column(sqlalchemy.DateTime, ...)`!'.format(cls.__name__, edit_at_column))
        except:
            raise NotImplementedError('edit at column (a sqlalchemy.Column) not found!')
        ...
        for klass in cls.CONF_RELATIONSHIP.mapping:
            if cls.CONF_RELATIONSHIP.get_relationship(klass) == Relationship.Option.many:
                ...
                n_child_column = getattr(cls, n_child_key)
                if not isinstance(n_child_column.type, sa.Integer):
                    raise NotImplementedError('`{}` column has to be a `sqlalchemy.Column(sqlalchemy.Integer, ...)`!'.format(n_child_key))

    def set_db_values(self, data: dict, engine: sa.engine.Engine, upsert: bool=False, **kwargs) -> None:
        """
        ...
        **中文文档**
        ...
        upd = (table.update().where(getattr(table.c, id_field_name) == id_field_value).values)(**data)
    ...
    def process_pr(self, pres: ParseResult, engine, **kwargs):
        """
    ...
    def statistics(cls, seconds=3600, **kwargs):
        ...
        for status in status_list:
            counts = session.query(cls).filter(getattr(cls, cls.CONF_STATUS_KEY) == status.id).filter(getattr(cls, cls.CONF_EDIT_AT_KEY) >= x_seconds_before_now).count()
            row = (
             cls.__name__, status.id, status.description, counts)
            data.append(row)
