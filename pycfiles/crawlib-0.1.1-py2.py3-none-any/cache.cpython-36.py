...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/crawlib-project/crawlib/cache.py
# Compiled at: 2019-12-26 16:17:01
# Size of source mod 2**32: 3738 bytes
"""
A disk cache layer to store url and its html.
"""
from __future__ import print_function
import os, zlib, diskcache
...
class CompressedDisk(diskcache.Disk):
    ...
    def __init__(self, directory, compress_level=6, value_type_is_binary=False, **kwargs):
        ...
        if value_type_is_binary is True:
            ...
            if value_type_is_binary is False:
    ...
    def fetch(self, mode, filename, value, read):
        data = super(CompressedDisk, self).fetch(mode, filename, value, read)
        if not read:
...
def create_cache(directory, compress_level=6, value_type_is_binary=False, **kwargs):
 ...
 directory, disk=CompressedDisk, 
     disk_compress_level=compress_level, 
     disk_value_type_is_binary=value_type_is_binary, **kwargs)
    return cache


def create_cache_here(this_file: str, compress_level: int=6, value_type_is_binary: bool=False, **kwargs) -> diskcache.Cache:
    """
    ...
    """
    return create_cache(directory=os.path.join(os.path.dirname(this_file), '.cache'), 
     compress_level=compress_level, 
     value_type_is_binary=value_type_is_binary, **kwargs)