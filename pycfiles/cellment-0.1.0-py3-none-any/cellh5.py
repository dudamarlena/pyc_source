...
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/cellh5/cellh5.py
# Compiled at: 2015-11-23 16:10:35
...
import sys, os, numpy, h5py, pandas, zlib, matplotlib.pyplot as plt, base64, warnings, unittest, datetime, functools, collections
from itertools import chain, izip
from collections import OrderedDict
from contextlib import contextmanager
from matplotlib.colors import hex2color
from hmm_wrapper import HMMConstraint, HMMAgnosticEstimator, normalize
version_num = (
 1, 3, 0)
...
def repack_cellh5(cellh5_folder, output_file=None, check_reg='^[A-Z]\\d{2}_\\d{2}', new_plate_name=None):
...
class CH5Position(object):
    ...
    def get_prediction_probabilities(self, indices=None, object_='primary__primary'):
    ...
    def get_crack_contour(self, index, object_='primary__primary', bb_corrected=True, size=None):
        if size is None:
            size = GALLERY_SIZE
        ...
        for ind in index:
            ...
            crack = numpy.asarray(zlib.decompress(base64.b64decode(crack_str)).split(','), dtype=numpy.float32).reshape(-1, 2)
            if bb_corrected:
    ...
    def get_object_feature_by_name(self, name, object_='primary__primary'):
        if len(self['feature'][object_][name]) > 0:
            return self['feature'][object_][name].value
        else:
            return []
    ...
    def get_gallery_image_matrix_with_classification(self, index, shape, object_='primary__primary'):
        ...
        for i in range(shape[0]):
            for j in range(shape[1]):
                ...
                for c in range(3):
                    image[a:a + 10, b:b + 10, c] = col_rgb[c]
        ...
        if len(evtable) == 0:
            ...
            for event_id in event_ids:
                ...
                if occurence == 1:
                    ...
                    raise RuntimeError('Split events with more than 2 childs are not suppored. How did it get there anyway?')
...
class CH5CachedPosition(CH5Position):
    ...
    def get_prediction_probabilities(self, *args, **kwargs):
        return super(CH5CachedPosition, self).get_prediction_probabilities(*args, **kwargs)
...
class CH5File(object):
    ...
    def __init__(self, filename, mode='a', cached=True):
        ...
        for well, positions in self.positions.iteritems():
            for pos in positions:
                ...
                self._position_group[(well, pos)] = self._open_position(self.plate, well, pos)
    ...
    def _open_position(self, plate, well, position):
        ...
        try:
            ...
            warnings.warn('Warning: cellh5 - well, position (%s, %s)could not be loaded ' % (
             well, position))
    ...
    def has_position(self, well, pos):
        return (
         well, str(pos)) in self._position_group
...
class CH5MappedFileCollection(object):
    ...
    def __init__(self, name='CH5MappedFileCollection', mapping_files=None, cellh5_files=None, sites=None, rows=None, cols=None, locations=None, init=True):
        self.name = name
    ...
    def get_treatment(self, plate, well, site):
        return list(self.mapping[((self.mapping['Plate'] == plate) & (self.mapping['Well'] == well) & (self.mapping['Site'] == site))][[
...
class CH5Analysis(CH5MappedFileCollection):
    ...
    def __init__(self, name='CH5Analysis', mapping_files=None, cellh5_files=None, sites=None, rows=None, cols=None, locations=None, output_dir=None, init=True):
        CH5MappedFileCollection.__init__(self, name=name, mapping_files=mapping_files, cellh5_files=cellh5_files, sites=sites, rows=rows, cols=cols, locations=locations, init=True)
    ...
    def get_treatment(self, plate, w, p):
        return list(self.ch5map.mapping[((self.mapping['Plate'] == plate) & (self.mapping['Well'] == w) & (self.mapping['Site'] == p))][[
         'siRNA ID', 'Gene Symbol']].iloc[0])
    ...
    def read_feature(self, object_='primary__primary', time_frames=None, remove_feature=(), use_features=None, read_classification=True, idx_selector_functor=None):
    ...
    def get_column_as_matrix(self, column_name, get_index=False):
        ...
        if get_index:
            ...
            return (
             res, numpy.array(list(chain.from_iterable(index))))
        return res
    ...
    def get_data_sampled(self, in_group, in_classes, n_sample=10000, in_class_type='Object classification label', type_='Object features'):
        assert sum(in_classes.values()) - 1 < 1e-06
...
class TestCH5Basic(CH5TestBase):
    ...
    def testTrackFirst(self):
        self.assertListEqual(self.pos.track_first(42), self.pos.track_all(42)[0])

    def testTrackLast(self):
        self.assertListEqual(self.pos.track_last(1111), self.pos.track_all(1111)[(-1)])
...
class TestCH5Examples(CH5TestBase):
    ...
    def testPrintTrackingTrace(self):
        ...
        for idx1, idx2 in zip(tracking['obj_idx1'], tracking['obj_idx2']):
    ...
    def testComputeTheMitoticIndex(self):
        ...
        mitotic_index = numpy.array(map(lambda x: [ len(numpy.nonzero(x == class_idx)[0]) for class_idx in range(n_classes) ], [ predictions[(nucleus['time_idx'] == time_idx)]['label_idx'] for time_idx in range(time_max) ]))
        fig = plt.figure()
