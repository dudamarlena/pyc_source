...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.5/site-packages/airsign/__main__.py
# Compiled at: 2016-05-11 05:11:50
# Size of source mod 2**32: 9979 bytes
...
def executeOps(ops, wifs=None):
    ...
    tx = transactions.Signed_Transaction(ref_block_num=ref_block_num, ref_block_prefix=ref_block_prefix, expiration=expiration, operations=ops)
    tx = tx.sign(wifs, 'BTS')
...
def main():
    ...
    parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter, description='Command line tool to interact with the BitShares network')
    parser.add_argument('--node', type=str, default=config['node'], help='Websocket URL for public BitShares API (default: "wss://bitshares.openledger.info/ws")')
    parser.add_argument('--rpcuser', type=str, default=config['rpcuser'], help='Websocket user if authentication is required')
    parser.add_argument('--rpcpassword', type=str, default=config['rpcpassword'], help='Websocket password if authentication is required')
    parser.add_argument('--nobroadcast', action='store_true', help='Do not broadcast anything')
    subparsers = parser.add_subparsers(help='sub-command help')
    parser.set_defaults(command=None)
    setconfig = subparsers.add_parser('set', help='Set configuration')
    setconfig.add_argument('key', type=str, choices=[
     'node',
     'rpcuser',
     'rpcpassword',
     'account'], help='Configuration key')
    ...
    addkey.add_argument('wifkeys', nargs='*', type=str, help='the private key in wallet import format (wif)')
    ...
    getbalance.add_argument('account', type=str, nargs='*', default=[
     config['account']], help='Accounts for which to retrieve the balance')
    ...
    transfer.add_argument('--from', type=str, help='Transfer from this account', default=config['account'])
    transfer.add_argument('--to', type=str, help='Transfer to this account', required=True)
    transfer.add_argument('--amount', type=str, help='Transfer this amount (format: "amount SYMBOL")', action='append', required=True)
    transfer.add_argument('--memo', default='', type=str, help='Memo')
    ...
    approve.add_argument('--account', type=str, help='Approve with this account', default=config['account'])
    approve.add_argument('proposal', type=str, help='Proposal to approve')
                        ...
                        if args.command == 'transfer':
                            ...
                            for amountStr in args.amount:
                                ...
                                if args.memo:
                                    ...
                                    encrypted_memo = memo.encode_memo(PrivateKey(memo_key), PublicKey(to_account['options']['memo_key']), nonce, args.memo)
