...
# [GCC 8.4.0]
# Embedded file name: /media/martin/windows/Desarrollo/Python/django-endless-pagination-vue/bin/django-endless-pagination-vue/tests/endless_pagination/templatetags/endless.py
# Compiled at: 2015-07-13 19:15:37
# Size of source mod 2**32: 22691 bytes
"""Django Endless Pagination template tags."""
from __future__ import unicode_literals
import re
from django import template
from django.utils.encoding import iri_to_uri
from endless_pagination import models, settings, utils
from endless_pagination.paginators import DefaultPaginator, EmptyPage, LazyPaginator
...
def paginate(parser, token, paginator_class=None):
    ...
    if '.' in objects and kwargs['var_name'] is None:
        msg = '%(tag)r tag requires a variable name `as` argumnent if the queryset is provided as a nested context variable (%(objects)s). You must either pass a direct queryset (e.g. taking advantage of the `with` template tag) or provide a new variable name to store the resulting queryset (e.g. `%(tag)s %(objects)s as objects`).' % {'tag': tag_name, 'objects': objects}
        raise template.TemplateSyntaxError(msg)
...
class PaginateNode(template.Node):
    __doc__ = "Add to context the objects of the current page.\n\n    Also add the Django paginator's *page* object.\n    "
        ...
        if number is None:
            ...
            if key is None:
                ...
                if override_path is None:
                    ...
                    if override_path[0] in ('"', "'") and override_path[(-1)] == override_path[0]:
...
def show_more(context, label=None, loading=settings.LOADING):
    ...
    if page.has_next():
        ...
        querystring = utils.get_querystring_for_page(request, page_number, querystring_key, default_number=data['default_number'])
        return {'label': label, 
         'loading': loading, 
...
class GetPagesNode(template.Node):
    __doc__ = 'Add the page list to context.'
    ...
    def render(self, context):
        data = utils.get_data_from_context(context)
        context[self.var_name] = models.PageList(context['request'], data['page'], data['querystring_key'], default_number=data['default_number'], override_path=data['override_path'])
        return ''
...
class ShowPagesNode(template.Node):
    __doc__ = 'Show the pagination.'

    def render(self, context):
        data = utils.get_data_from_context(context)
        pages = models.PageList(context['request'], data['page'], data['querystring_key'], default_number=data['default_number'], override_path=data['override_path'])
...
class ShowCurrentNumberNode(template.Node):
    __doc__ = 'Show the page number taken from context.'
