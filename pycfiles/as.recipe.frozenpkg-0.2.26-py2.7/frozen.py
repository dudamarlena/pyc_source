...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/as/recipe/frozenpkg/frozen.py
# Compiled at: 2013-05-06 04:49:00
import os, sys, shutil, logging, glob, re, tempfile, pkg_resources, subprocess
...
class Frozen(object):
    ...
    def install(self):
        ...
        self.dereference = self.options.get('dereference', 'no').strip().lower() in ('yes',
                                                                                     'true',
                                                                                     '1')
    ...
    def _create_venv(self):
        ...
        job = subprocess.Popen(virtualenv, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    ...
    def _copy_eggs(self):
        ...
        ws = zc.buildout.easy_install.working_set(distributions, [
