...
# [GCC 8.4.0]
# Embedded file name: /home/mass/PythonProjects/oideas/oideas_backend/accounts/tests/tests_views.py
# Compiled at: 2015-08-17 02:49:41
...
class SignupViewTests(TestCase):
    ...
    def test_can_create_new_user_with_email(self):
        email = 'a@b.com'
        self.client.post(reverse('signup'), data={'email': email, 
           'password': 'Secret!'})
    ...
    def test_cannot_create_duplicated_user(self):
        ...
        response = self.client.post(reverse('signup'), data={'email': email, 
           'password': password})
...
class LoginViewTests(TestCase):
    ...
    def test_can_login_with_email(self):
        ...
        response = self.client.post(reverse('login'), data={'email': email, 
           'password': password})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['token'], str(valid_user._auth_token_cache))

    def test_return_error_when_try_to_login_with_wrong_credentials(self):
        response = self.client.post(reverse('login'), data={'username': 'Wrong user!', 
...
class LogoutViewTests(TestCase):
    ...
    def test_can_login_with_user_name(self):
        ...
        token = self.user._auth_token_cache
        client = APIClient()
