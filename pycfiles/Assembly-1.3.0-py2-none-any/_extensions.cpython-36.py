...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/Assembly/assembly/_extensions.py
# Compiled at: 2019-12-14 14:55:13
# Size of source mod 2**32: 20993 bytes
"""
Assembly: extensions
"""
import os, re, six, copy, logging, markdown, flask_s3, flask_mail, ses_mailer, flask_login, flask_kvsession
...
def session(app):
    ...
    if uri:
        ...
        if 'redis' in scheme:
            ...
            if 's3' in scheme or 'google_storage' in scheme:
    ...
    def init_app(self, app):
        ...
        if mailer_uri:
            ...
            if 'ses' in scheme.lower():
                ...
                self.mail = ses_mailer.Mail(aws_access_key_id=access_key, aws_secret_access_key=secret_key,
                  region=region,
                ...
                if 'smtp' in scheme.lower():
                    ...
                    _ses_mailer = ses_mailer.Mail(template=templates_sources, template_context=(self.config.get('MAIL_TEMPLATE_CONTEXT')))
...
class _AssetsDelivery(flask_s3.FlaskS3):
    ...
    def init_app(self, app):
        ...
        if delivery_method:
            if delivery_method.upper() in ('S3', 'CDN'):
                is_secure = False
...
gl_tag_re = re.compile('(?:<(/?)([a-zA-Z0-9_-]+)\\s*|(>\\s*))(?s)')
gl_ws_normalize_re = re.compile('[ \\t\\r\\n]+')
...
class StreamProcessContext(object):
    ...
    def fail(self, message):
        raise TemplateSyntaxError(message, self.token.lineno, self.stream.name, self.stream.filename)
...
class HTMLCompress(Extension):
    ...
    def is_breaking(self, tag, other_tag):
        ...
        return breaking and (tag in breaking or '#block' in breaking and tag in self.block_elements)
...
class SelectiveHTMLCompress(HTMLCompress):
    ...
    def filter_stream(self, stream):
        ctx = StreamProcessContext(stream)
        strip_depth = 0
        while stream.current.type == 'block_begin':
            if stream.look().test('name:strip') or stream.look().test('name:endstrip'):
                stream.skip()
                if stream.current.value == 'strip':
                    ...
                    stream.skip()
            elif strip_depth > 0 and stream.current.type == 'data':
...
class PlaceholderAddTo(Extension):
    ...
    def parse(self, parser):
        ...
        args = [name]
        return CallBlock(self.call_method('_render_tag', args), [], [], body).set_lineno(lineno)
...
class PlaceholderRender(Extension):
    ...
    def parse(self, parser):
        ...
        name = parser.parse_expression()
        args = [name]
        return CallBlock(self.call_method('_render_tag', args), [], [], []).set_lineno(lineno)