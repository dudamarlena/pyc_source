...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/Assembly/assembly/asm.py
# Compiled at: 2019-12-13 22:03:20
# Size of source mod 2**32: 12699 bytes
"""
Assembly: asm
Set of helpers and functions. These functions are dependents of some config and setup
"""
import re, six, copy, blinker, logging, inspect, datetime, functools, itsdangerous, flask_cloudy
...
def __crypt_init(app):
    ...
    CONFIG
        ...
        BCRYPT_IDENT = '2b'
    """
    __CRYPT.update({'secret_key':config.get('SECRET_KEY'), 
...
def hash_string(string):
    ...
    :returns: string
    """
    conf = {'rounds': __CRYPT.get('rounds')}
...
def encode_jwt(data, expires_in=1, **kw):
    ...
    """
    expires_in *= 60
    s = (itsdangerous.TimedJSONWebSignatureSerializer)(secret_key=__CRYPT.get('secret_key'), expires_in=expires_in, 
     salt=__CRYPT.get('salt'), **kw)
    return s.dumps(data)
...
def sign_data(data, expires_in=None, **kw):
    ...
    if expires_in:
        expires_in *= 60
        s = URLSafeTimedSerializer2(secret_key=__CRYPT.get('secret_key'), expires_in=expires_in, 
         salt=__CRYPT.get('salt'), **kw)
    else:
