...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/Assembly/assembly/assembly.py
# Compiled at: 2019-12-25 00:16:20
# Size of source mod 2**32: 31160 bytes
"""
Assembly: core

This is the Assembly core
"""
import re, os, sys, six, jinja2, inspect, logging, werkzeug, functools, arrow as date, pkg_resources, logging.config
...
class Assembly(object):
    ...
    assets = None
    _ext = set()
    _Assembly__special_methods = ['get', 'put', 'patch', 'post', 'delete', 'index']
    _app = None
    _init_apps = set()
    _template_fsl = {}
    _static_paths = set()
    ...
    @classmethod
    def init(cls, import_name, views_list, app_name='default', app_env='Development'):
        """
        Initialize Assembly
        ...
        if cls._template_fsl:
            loader = [
             app.jinja_loader, _JinjaPrefixLoader(cls._template_fsl)]
            app.jinja_loader = jinja2.ChoiceLoader(loader)
        cls.assets.load_path = [
         app.static_folder] + list(cls._static_paths)
    ...
    def render(cls, data={}, __template__=None, **kwargs):
        ...
        """
        vars = dict(__NAME__=(about.__title__),
    ...
    @classmethod
    def _register__(cls, app, base_route=None, subdomain=None, route_prefix=None, trailing_slash=True):
            ...
            for name, value in _get_interesting_members(Assembly, cls):
                ...
                try:
                    if hasattr(value, '_rule_cache') and name in value._rule_cache:
                        ...
                        if name in cls._Assembly__special_methods:
    ...
    def _error_handler__(cls, fn, e):
        ...
        if isinstance(resp, Response) or isinstance(resp, BaseResponse):
            ...
            if isinstance(resp, dict) or isinstance(resp, tuple) or resp is None:
                ...
                return (
                 (cls.render)(**resp), e.code, headers)
            return resp
...
def _get_interesting_members(base_class, cls):
    ...
    return (member for member in all_members if member[0] not in base_members if (hasattr(member[1], '__self__') and member[1].__self__ not in inspect.getmro(cls) if six.PY2 else True) if not member[0].startswith('_') if not member[0].startswith('before_') if not member[0].startswith('after_'))
...
def _get_interesting_members_http_error(base_class, cls):
    ...
    predicate = inspect.ismethod if six.PY2 else inspect.isfunction
    all_members = inspect.getmembers(cls, predicate=predicate)
    return (member for member in all_members if member[0] not in base_members if (hasattr(member[1], '__self__') and member[1].__self__ not in inspect.getmro(cls) if six.PY2 else True) if member[0].startswith('_') if _match_http_error(member[0]) if not member[0].startswith('before_') if not member[0].startswith('after_'))
...
def _match_http_error(val):
    ...
    """
    return re.match('^_error_(\\d+|handler)$', val)
