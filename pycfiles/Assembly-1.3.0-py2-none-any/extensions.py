...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/Flasik/flasik/extensions.py
# Compiled at: 2019-09-07 12:28:17
import re, six, logging, copy, itsdangerous
from flask import request, current_app, send_file, session
import flask_kvsession, ses_mailer, flask_mail, flask_s3
    ...
    def send(self, to, subject=None, body=None, reply_to=None, template=None, **kwargs):
        ...
        if not isinstance(to, list):
            ...
            kwargs.update({'subject': subject, 
               'body': body, 
               'reply_to': reply_to})
            raise (self.validated or Error)('Mail configuration error')
...
class Crypt(object):
    ...
    def jwt_encode(self, data, expires_in=1, **kw):
        ...
        """
        expires_in *= 60
        s = itsdangerous.TimedJSONWebSignatureSerializer(secret_key=self.secret_key, expires_in=expires_in, salt=self.salt, **kw)
        return s.dumps(data)
    ...
    def url_safe_encode(self, data, expires_in=None, **kw):
        ...
        if expires_in:
            expires_in *= 60
            s = URLSafeTimedSerializer2(secret_key=self.secret_key, expires_in=expires_in, salt=self.salt, **kw)
        else:
    ...
    def url_safe_decode(self, token, **kw):
        ...
        if len(token.split('.')) == 3:
            ...
            if timestamp > now:
                return value
            raise itsdangerous.SignatureExpired('Signature age %s < %s ' % (timestamp, now), payload=value, date_signed=timestamp)
        else:
            s = itsdangerous.URLSafeSerializer(secret_key=self.secret_key, salt=self.salt, **kw)
            return s.loads(token)
        return
