...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/size/size.py
# Compiled at: 2018-05-04 15:24:22
"""
...
class Size(object):
    ...
    def _convert(self, value):
        ...
        if not isinstance(value, string_types):
            raise SizeObjectValError(('Value is not in a format I can understand : {} - {}').format(type(value), value))
        if value.isdigit():
            return int(value)
        ...
        if not suffix_index:
            raise SizeObjectValError(('Value is not in a format I can understand. Invalid Suffix. {}').format(value))
        val, suffix = value[:suffix_index].strip(), value[suffix_index:].strip()
        if suffix not in valid_suffices:
            raise SizeObjectValError('Value is not in a format I can understand. Invalid Suffix.')
        try:
            val = Decimal(val)
        except InvalidOperation:
            raise SizeObjectValError('Value is not in a format I can understand. Could not convert value to int')
