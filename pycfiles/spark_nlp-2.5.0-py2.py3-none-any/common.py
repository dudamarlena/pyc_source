...
# [GCC 8.4.0]
# Embedded file name: /Users/maziyar/Desktop/Work/JohnSnowLabs/spark-nlp/python/build/lib/sparknlp/common.py
# Compiled at: 2020-03-20 15:04:10
...
class AnnotatorProperties(Params):
    inputCols = Param(Params._dummy(), 'inputCols', 'previous annotations columns, if renamed', typeConverter=TypeConverters.toListString)
    outputCol = Param(Params._dummy(), 'outputCol', 'output annotation column. can be left default.', typeConverter=TypeConverters.toString)
    lazyAnnotator = Param(Params._dummy(), 'lazyAnnotator', 'Whether this AnnotatorModel acts as lazy in RecursivePipelines', typeConverter=TypeConverters.toBoolean)
...
class HasEmbeddingsProperties(Params):
    dimension = Param(Params._dummy(), 'dimension', 'Number of embedding dimensions', typeConverter=TypeConverters.toInt)
...
class HasStorageRef:
    storageRef = Param(Params._dummy(), 'storageRef', 'unique reference name for identification', TypeConverters.toString)
...
class HasCaseSensitiveProperties:
    caseSensitive = Param(Params._dummy(), 'caseSensitive', 'whether to ignore case in tokens for embeddings matching', typeConverter=TypeConverters.toBoolean)
...
class HasExcludableStorage:
    includeStorage = Param(Params._dummy(), 'includeStorage', 'whether to include indexed storage in trained model', typeConverter=TypeConverters.toBoolean)
...
class HasStorage(HasStorageRef, HasCaseSensitiveProperties, HasExcludableStorage):
    storagePath = Param(Params._dummy(), 'storagePath', 'path to file', typeConverter=TypeConverters.identity)
...
class AnnotatorApproach(JavaEstimator, JavaMLWritable, _internal.AnnotatorJavaMLReadable, AnnotatorProperties, _internal.ParamsGettersSetters):
...
class RecursiveAnnotatorApproach(_internal.RecursiveEstimator, JavaMLWritable, _internal.AnnotatorJavaMLReadable, AnnotatorProperties, _internal.ParamsGettersSetters):
