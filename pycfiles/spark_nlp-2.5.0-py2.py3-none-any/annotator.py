...
# [GCC 8.4.0]
# Embedded file name: /Users/maziyar/Desktop/Work/JohnSnowLabs/spark-nlp/python/build/lib/sparknlp/annotator.py
# Compiled at: 2020-05-06 10:55:16
import sys
...
class RecursiveTokenizer(AnnotatorApproach):
    ...
    prefixes = Param(Params._dummy(), 'prefixes', 'strings to be considered independent tokens when found at the beginning of a word', typeConverter=TypeConverters.toListString)
    suffixes = Param(Params._dummy(), 'suffixes', 'strings to be considered independent tokens when found at the end of a word', typeConverter=TypeConverters.toListString)
    infixes = Param(Params._dummy(), 'infixes', 'strings to be considered independent tokens when found in the middle of a word', typeConverter=TypeConverters.toListString)
    whitelist = Param(Params._dummy(), 'whitelist', 'strings to be considered as single tokens', typeConverter=TypeConverters.toListString)
...
class RecursiveTokenizerModel(AnnotatorModel):
    ...
    def __init__(self, classname='com.johnsnowlabs.nlp.annotators.RecursiveTokenizerModel', java_model=None):
        super(RecursiveTokenizerModel, self).__init__(classname=classname, java_model=java_model)


class Tokenizer(AnnotatorApproach):
    targetPattern = Param(Params._dummy(), 'targetPattern', 'pattern to grab from text as token candidates. Defaults \\S+', typeConverter=TypeConverters.toString)
    prefixPattern = Param(Params._dummy(), 'prefixPattern', 'regex with groups and begins with \\A to match target prefix. Defaults to \\A([^\\s\\w\\$\\.]*)', typeConverter=TypeConverters.toString)
    suffixPattern = Param(Params._dummy(), 'suffixPattern', 'regex with groups and ends with \\z to match target suffix. Defaults to ([^\\s\\w]?)([^\\s\\w]*)\\z', typeConverter=TypeConverters.toString)
    infixPatterns = Param(Params._dummy(), 'infixPatterns', 'regex patterns that match tokens within a single target. groups identify different sub-tokens. multiple defaults', typeConverter=TypeConverters.toListString)
    exceptions = Param(Params._dummy(), 'exceptions', "Words that won't be affected by tokenization rules", typeConverter=TypeConverters.toListString)
    exceptionsPath = Param(Params._dummy(), 'exceptionsPath', 'path to file containing list of exceptions', typeConverter=TypeConverters.toString)
    caseSensitiveExceptions = Param(Params._dummy(), 'caseSensitiveExceptions', 'Whether to care for case sensitiveness in exceptions', typeConverter=TypeConverters.toBoolean)
    contextChars = Param(Params._dummy(), 'contextChars', 'character list used to separate from token boundaries', typeConverter=TypeConverters.toListString)
    splitPattern = Param(Params._dummy(), 'splitPattern', 'character list used to separate from the inside of tokens', typeConverter=TypeConverters.toString)
    splitChars = Param(Params._dummy(), 'splitChars', 'character list used to separate from the inside of tokens', typeConverter=TypeConverters.toListString)
    minLength = Param(Params._dummy(), 'minLength', 'Set the minimum allowed legth for each token', typeConverter=TypeConverters.toInt)
    maxLength = Param(Params._dummy(), 'maxLength', 'Set the maximum allowed legth for each token', typeConverter=TypeConverters.toInt)
    name = 'Tokenizer'

    @keyword_only
    def __init__(self):
        super(Tokenizer, self).__init__(classname='com.johnsnowlabs.nlp.annotators.Tokenizer')
        self._setDefault(targetPattern='\\S+', contextChars=[
         '.', ',', ';', ':', '!', '?', '*', '-', '(', ')', '"', "'"], caseSensitiveExceptions=True, minLength=0, maxLength=99999)
...
class TokenizerModel(AnnotatorModel):
    ...
    caseSensitiveExceptions = Param(Params._dummy(), 'caseSensitiveExceptions', 'Whether to care for case sensitiveness in exceptions', typeConverter=TypeConverters.toBoolean)
    targetPattern = Param(Params._dummy(), 'targetPattern', 'pattern to grab from text as token candidates. Defaults \\S+', typeConverter=TypeConverters.toString)
    rules = Param(Params._dummy(), 'rules', 'Rules structure factory containing pre processed regex rules', typeConverter=TypeConverters.identity)
    splitPattern = Param(Params._dummy(), 'splitPattern', 'character list used to separate from the inside of tokens', typeConverter=TypeConverters.toString)
    splitChars = Param(Params._dummy(), 'splitChars', 'character list used to separate from the inside of tokens', typeConverter=TypeConverters.toListString)
...
class Chunker(AnnotatorModel):
    regexParsers = Param(Params._dummy(), 'regexParsers', 'an array of grammar based chunk parsers', typeConverter=TypeConverters.toListString)
    name = 'Chunker'
...
class Normalizer(AnnotatorApproach):
    cleanupPatterns = Param(Params._dummy(), 'cleanupPatterns', 'normalization regex patterns which match will be removed from token', typeConverter=TypeConverters.toListString)
    lowercase = Param(Params._dummy(), 'lowercase', 'whether to convert strings to lowercase')
    slangMatchCase = Param(Params._dummy(), 'slangMatchCase', 'whether or not to be case sensitive to match slangs. Defaults to false.')
    slangDictionary = Param(Params._dummy(), 'slangDictionary', "slang dictionary is a delimited text. needs 'delimiter' in options", typeConverter=TypeConverters.identity)

    @keyword_only
    def __init__(self):
        super(Normalizer, self).__init__(classname='com.johnsnowlabs.nlp.annotators.Normalizer')
        self._setDefault(cleanupPatterns=[
         '[^\\pL+]'], lowercase=False, slangMatchCase=False)
...
class NormalizerModel(AnnotatorModel):
    ...
    def __init__(self, classname='com.johnsnowlabs.nlp.annotators.NormalizerModel', java_model=None):
        super(NormalizerModel, self).__init__(classname=classname, java_model=java_model)

    name = 'NormalizerModel'


class RegexMatcher(AnnotatorApproach):
    strategy = Param(Params._dummy(), 'strategy', 'MATCH_FIRST|MATCH_ALL|MATCH_COMPLETE', typeConverter=TypeConverters.toString)
    externalRules = Param(Params._dummy(), 'externalRules', "external resource to rules, needs 'delimiter' in options", typeConverter=TypeConverters.identity)
...
class RegexMatcherModel(AnnotatorModel):
    ...
    def __init__(self, classname='com.johnsnowlabs.nlp.annotators.RegexMatcherModel', java_model=None):
        super(RegexMatcherModel, self).__init__(classname=classname, java_model=java_model)

    name = 'RegexMatcherModel'


class Lemmatizer(AnnotatorApproach):
    dictionary = Param(Params._dummy(), 'dictionary', 'lemmatizer external dictionary.' + " needs 'keyDelimiter' and 'valueDelimiter' in options for parsing target text", typeConverter=TypeConverters.identity)
    ...
    def setDictionary(self, path, key_delimiter, value_delimiter, read_as=ReadAs.TEXT, options={'format': 'text'}):
        opts = options.copy()
...
class LemmatizerModel(AnnotatorModel):
    ...
    def __init__(self, classname='com.johnsnowlabs.nlp.annotators.LemmatizerModel', java_model=None):
        super(LemmatizerModel, self).__init__(classname=classname, java_model=java_model)
...
class DateMatcherUtils(Params):
    dateFormat = Param(Params._dummy(), 'dateFormat', 'desired format for dates extracted', typeConverter=TypeConverters.toString)
    readMonthFirst = Param(Params._dummy(), 'readMonthFirst', 'Whether to parse july 07/05/2015 or as 05/07/2015', typeConverter=TypeConverters.toBoolean)
    defaultDayWhenMissing = Param(Params._dummy(), 'defaultDayWhenMissing', 'which day to set when it is missing from parsed input', typeConverter=TypeConverters.toInt)
...
class DateMatcher(AnnotatorModel, DateMatcherUtils):
    ...
    def __init__(self):
        ...
        self._setDefault(dateFormat='yyyy/MM/dd', readMonthFirst=True, defaultDayWhenMissing=1)
...
class TextMatcher(AnnotatorApproach):
    entities = Param(Params._dummy(), 'entities', 'ExternalResource for entities', typeConverter=TypeConverters.identity)
    caseSensitive = Param(Params._dummy(), 'caseSensitive', 'whether to match regardless of case. Defaults true', typeConverter=TypeConverters.toBoolean)
    mergeOverlapping = Param(Params._dummy(), 'mergeOverlapping', 'whether to merge overlapping matched chunks. Defaults false', typeConverter=TypeConverters.toBoolean)
...
class TextMatcherModel(AnnotatorModel):
    ...
    mergeOverlapping = Param(Params._dummy(), 'mergeOverlapping', 'whether to merge overlapping matched chunks. Defaults false', typeConverter=TypeConverters.toBoolean)
    searchTrie = Param(Params._dummy(), 'searchTrie', 'searchTrie', typeConverter=TypeConverters.identity)

    def __init__(self, classname='com.johnsnowlabs.nlp.annotators.TextMatcherModel', java_model=None):
        super(TextMatcherModel, self).__init__(classname=classname, java_model=java_model)
...
class BigTextMatcher(AnnotatorApproach, HasStorage):
    entities = Param(Params._dummy(), 'entities', 'ExternalResource for entities', typeConverter=TypeConverters.identity)
    caseSensitive = Param(Params._dummy(), 'caseSensitive', 'whether to ignore case in index lookups', typeConverter=TypeConverters.toBoolean)
    mergeOverlapping = Param(Params._dummy(), 'mergeOverlapping', 'whether to merge overlapping matched chunks. Defaults false', typeConverter=TypeConverters.toBoolean)
    tokenizer = Param(Params._dummy(), 'tokenizer', 'TokenizerModel to use to tokenize input file for building a Trie', typeConverter=TypeConverters.identity)
...
class BigTextMatcherModel(AnnotatorModel, HasStorageModel):
    ...
    def __init__(self, classname='com.johnsnowlabs.nlp.annotators.btm.TextMatcherModel', java_model=None):
        super(BigTextMatcherModel, self).__init__(classname=classname, java_model=java_model)
...
class PerceptronApproach(AnnotatorApproach):
    posCol = Param(Params._dummy(), 'posCol', 'column of Array of POS tags that match tokens', typeConverter=TypeConverters.toString)
    nIterations = Param(Params._dummy(), 'nIterations', 'Number of iterations in training, converges to better accuracy', typeConverter=TypeConverters.toInt)
...
class PerceptronModel(AnnotatorModel):
    ...
    def __init__(self, classname='com.johnsnowlabs.nlp.annotators.pos.perceptron.PerceptronModel', java_model=None):
        super(PerceptronModel, self).__init__(classname=classname, java_model=java_model)
...
class SentenceDetectorParams:
    useAbbreviations = Param(Params._dummy(), 'useAbbreviations', 'whether to apply abbreviations at sentence detection', typeConverter=TypeConverters.toBoolean)
    customBounds = Param(Params._dummy(), 'customBounds', 'characters used to explicitly mark sentence bounds', typeConverter=TypeConverters.toListString)
    useCustomBoundsOnly = Param(Params._dummy(), 'useCustomBoundsOnly', 'Only utilize custom bounds in sentence detection', typeConverter=TypeConverters.toBoolean)
    explodeSentences = Param(Params._dummy(), 'explodeSentences', 'whether to explode each sentence into a different row, for better parallelization. Defaults to false.', typeConverter=TypeConverters.toBoolean)
    splitLength = Param(Params._dummy(), 'splitLength', 'length at which sentences will be forcibly split.', typeConverter=TypeConverters.toInt)
    minLength = Param(Params._dummy(), 'minLength', 'Set the minimum allowed length for each sentence.', typeConverter=TypeConverters.toInt)
    maxLength = Param(Params._dummy(), 'maxLength', 'Set the maximum allowed length for each sentence', typeConverter=TypeConverters.toInt)
...
class SentenceDetector(AnnotatorModel, SentenceDetectorParams):
    ...
    def __init__(self):
        ...
        self._setDefault(useAbbreviations=True, useCustomBoundsOnly=False, customBounds=[], explodeSentences=False, minLength=0, maxLength=99999)


class DeepSentenceDetector(AnnotatorModel, SentenceDetectorParams):
    includesPragmaticSegmenter = Param(Params._dummy(), 'includesPragmaticSegmenter', 'Whether to include rule-based sentence detector as first filter', typeConverter=TypeConverters.toBoolean)
    endPunctuation = Param(Params._dummy(), 'endPunctuation', 'An array of symbols that deep sentence detector will consider as end of sentence punctuation', typeConverter=TypeConverters.toListString)
    ...
    @keyword_only
    def __init__(self):
        super(DeepSentenceDetector, self).__init__(classname='com.johnsnowlabs.nlp.annotators.sbd.deep.DeepSentenceDetector')
        self._setDefault(includesPragmaticSegmenter=False, endPunctuation=['.', '!', '?'], explodeSentences=False)


class SentimentDetector(AnnotatorApproach):
    dictionary = Param(Params._dummy(), 'dictionary', 'path for dictionary to sentiment analysis', typeConverter=TypeConverters.identity)
    positiveMultiplier = Param(Params._dummy(), 'positiveMultiplier', 'multiplier for positive sentiments. Defaults 1.0', typeConverter=TypeConverters.toFloat)
    negativeMultiplier = Param(Params._dummy(), 'negativeMultiplier', 'multiplier for negative sentiments. Defaults -1.0', typeConverter=TypeConverters.toFloat)
    incrementMultiplier = Param(Params._dummy(), 'incrementMultiplier', 'multiplier for increment sentiments. Defaults 2.0', typeConverter=TypeConverters.toFloat)
    decrementMultiplier = Param(Params._dummy(), 'decrementMultiplier', 'multiplier for decrement sentiments. Defaults -2.0', typeConverter=TypeConverters.toFloat)
    reverseMultiplier = Param(Params._dummy(), 'reverseMultiplier', 'multiplier for revert sentiments. Defaults -1.0', typeConverter=TypeConverters.toFloat)
    enableScore = Param(Params._dummy(), 'enableScore', 'if true, score will show as the double value, else will output string "positive" or "negative". Defaults false', typeConverter=TypeConverters.toBoolean)

    def __init__(self):
        super(SentimentDetector, self).__init__(classname='com.johnsnowlabs.nlp.annotators.sda.pragmatic.SentimentDetector')
        self._setDefault(positiveMultiplier=1.0, negativeMultiplier=-1.0, incrementMultiplier=2.0, decrementMultiplier=-2.0, reverseMultiplier=-1.0, enableScore=False)
...
class SentimentDetectorModel(AnnotatorModel):
    ...
    def __init__(self, classname='com.johnsnowlabs.nlp.annotators.sda.pragmatic.SentimentDetectorModel', java_model=None):
        super(SentimentDetectorModel, self).__init__(classname=classname, java_model=java_model)


class ViveknSentimentApproach(AnnotatorApproach):
    sentimentCol = Param(Params._dummy(), 'sentimentCol', "column with the sentiment result of every row. Must be 'positive' or 'negative'", typeConverter=TypeConverters.toString)
    pruneCorpus = Param(Params._dummy(), 'pruneCorpus', 'Removes unfrequent scenarios from scope. The higher the better performance. Defaults 1', typeConverter=TypeConverters.toInt)
    importantFeatureRatio = Param(Params._dummy(), 'importantFeatureRatio', 'proportion of feature content to be considered relevant. Defaults to 0.5', typeConverter=TypeConverters.toFloat)
    unimportantFeatureStep = Param(Params._dummy(), 'unimportantFeatureStep', 'proportion to lookahead in unimportant features. Defaults to 0.025', typeConverter=TypeConverters.toFloat)
    featureLimit = Param(Params._dummy(), 'featureLimit', 'content feature limit, to boost performance in very dirt text. Default disabled with -1', typeConverter=TypeConverters.toInt)
...
class ViveknSentimentModel(AnnotatorModel):
    ...
    def __init__(self, classname='com.johnsnowlabs.nlp.annotators.sda.vivekn.ViveknSentimentModel', java_model=None):
        super(ViveknSentimentModel, self).__init__(classname=classname, java_model=java_model)
...
class NorvigSweetingApproach(AnnotatorApproach):
    dictionary = Param(Params._dummy(), 'dictionary', "dictionary needs 'tokenPattern' regex in dictionary for separating words", typeConverter=TypeConverters.identity)
    caseSensitive = Param(Params._dummy(), 'caseSensitive', 'whether to ignore case sensitivty', typeConverter=TypeConverters.toBoolean)
    doubleVariants = Param(Params._dummy(), 'doubleVariants', 'whether to use more expensive spell checker', typeConverter=TypeConverters.toBoolean)
    shortCircuit = Param(Params._dummy(), 'shortCircuit', 'whether to use faster mode', typeConverter=TypeConverters.toBoolean)
    frequencyPriority = Param(Params._dummy(), 'frequencyPriority', 'applies frequency over hamming in intersections. When false hamming takes priority', typeConverter=TypeConverters.toBoolean)
    wordSizeIgnore = Param(Params._dummy(), 'wordSizeIgnore', 'minimum size of word before ignoring. Defaults to 3', typeConverter=TypeConverters.toInt)
    dupsLimit = Param(Params._dummy(), 'dupsLimit', 'maximum duplicate of characters in a word to consider. Defaults to 2', typeConverter=TypeConverters.toInt)
    reductLimit = Param(Params._dummy(), 'reductLimit', 'word reductions limit. Defaults to 3', typeConverter=TypeConverters.toInt)
    intersections = Param(Params._dummy(), 'intersections', 'hamming intersections to attempt. Defaults to 10', typeConverter=TypeConverters.toInt)
    vowelSwapLimit = Param(Params._dummy(), 'vowelSwapLimit', 'vowel swap attempts. Defaults to 6', typeConverter=TypeConverters.toInt)

    @keyword_only
    def __init__(self):
        super(NorvigSweetingApproach, self).__init__(classname='com.johnsnowlabs.nlp.annotators.spell.norvig.NorvigSweetingApproach')
        self._setDefault(caseSensitive=False, doubleVariants=False, shortCircuit=False, wordSizeIgnore=3, dupsLimit=2, reductLimit=3, intersections=10, vowelSwapLimit=6, frequencyPriority=True)
...
class NorvigSweetingModel(AnnotatorModel):
    ...
    def __init__(self, classname='com.johnsnowlabs.nlp.annotators.spell.norvig.NorvigSweetingModel', java_model=None):
        super(NorvigSweetingModel, self).__init__(classname=classname, java_model=java_model)
...
class SymmetricDeleteApproach(AnnotatorApproach):
    corpus = Param(Params._dummy(), 'corpus', 'folder or file with text that teaches about the language', typeConverter=TypeConverters.identity)
    dictionary = Param(Params._dummy(), 'dictionary', 'folder or file with text that teaches about the language', typeConverter=TypeConverters.identity)
    maxEditDistance = Param(Params._dummy(), 'maxEditDistance', 'max edit distance characters to derive strings from a word', typeConverter=TypeConverters.toInt)
    frequencyThreshold = Param(Params._dummy(), 'frequencyThreshold', 'minimum frequency of words to be considered from training. ' + 'Increase if training set is LARGE. Defaults to 0', typeConverter=TypeConverters.toInt)
    deletesThreshold = Param(Params._dummy(), 'deletesThreshold', 'minimum frequency of corrections a word needs to have to be considered from training.' + 'Increase if training set is LARGE. Defaults to 0', typeConverter=TypeConverters.toInt)
    dupsLimit = Param(Params._dummy(), 'dupsLimit', 'maximum duplicate of characters in a word to consider. Defaults to 2', typeConverter=TypeConverters.toInt)
...
class SymmetricDeleteModel(AnnotatorModel):
    ...
    def __init__(self, classname='com.johnsnowlabs.nlp.annotators.spell.symmetric.SymmetricDeleteModel', java_model=None):
        super(SymmetricDeleteModel, self).__init__(classname=classname, java_model=java_model)
...
class NerApproach(Params):
    labelColumn = Param(Params._dummy(), 'labelColumn', 'Column with label per each token', typeConverter=TypeConverters.toString)
...
class NerCrfApproach(AnnotatorApproach, NerApproach):
    ...
    includeConfidence = Param(Params._dummy(), 'includeConfidence', "external features is a delimited text. needs 'delimiter' in options", TypeConverters.toBoolean)
    ...
    def __init__(self):
        ...
        self._setDefault(minEpochs=0, maxEpochs=1000, l2=float(1), c0=2250000, lossEps=float(0.001), verbose=4, includeConfidence=False)
...
class NerDLApproach(AnnotatorApproach, NerApproach):
    ...
    validationSplit = Param(Params._dummy(), 'validationSplit', 'Choose the proportion of training dataset to be validated against the model on each Epoch. The value should be between 0.0 and 1.0 and by default it is 0.0 and off.', TypeConverters.toFloat)
    evaluationLogExtended = Param(Params._dummy(), 'evaluationLogExtended', 'Choose the proportion of training dataset to be validated against the model on each Epoch. The value should be between 0.0 and 1.0 and by default it is 0.0 and off.', TypeConverters.toBoolean)
    ...
    includeConfidence = Param(Params._dummy(), 'includeConfidence', 'whether to include confidence scores in annotation metadata', TypeConverters.toBoolean)
    enableOutputLogs = Param(Params._dummy(), 'enableOutputLogs', 'Whether to use stdout in addition to Spark logs.', TypeConverters.toBoolean)
    ...
    def __init__(self):
        ...
        self._setDefault(minEpochs=0, maxEpochs=50, lr=float(0.001), po=float(0.005), batchSize=8, dropout=float(0.5), verbose=2, useContrib=uc, validationSplit=float(0.0), evaluationLogExtended=False, includeConfidence=False, enableOutputLogs=False)
...
class NerConverter(AnnotatorModel):
    name = 'NerConverter'
    whiteList = Param(Params._dummy(), 'whiteList', 'If defined, list of entities to process. The rest will be ignored. Do not include IOB prefix on labels', typeConverter=TypeConverters.toListString)
...
class DependencyParserApproach(AnnotatorApproach):
    dependencyTreeBank = Param(Params._dummy(), 'dependencyTreeBank', 'Dependency treebank source files', typeConverter=TypeConverters.identity)
    conllU = Param(Params._dummy(), 'conllU', 'Universal Dependencies source files', typeConverter=TypeConverters.identity)
    numberOfIterations = Param(Params._dummy(), 'numberOfIterations', 'Number of iterations in training, converges to better accuracy', typeConverter=TypeConverters.toInt)
...
class DependencyParserModel(AnnotatorModel):
    ...
    perceptron = Param(Params._dummy(), 'perceptron', 'Dependency parsing perceptron features', typeConverter=TypeConverters.identity)

    def __init__(self, classname='com.johnsnowlabs.nlp.annotators.parser.dep.DependencyParserModel', java_model=None):
        super(DependencyParserModel, self).__init__(classname=classname, java_model=java_model)
...
class TypedDependencyParserApproach(AnnotatorApproach):
    conll2009 = Param(Params._dummy(), 'conll2009', 'Path to file with CoNLL 2009 format', typeConverter=TypeConverters.identity)
    conllU = Param(Params._dummy(), 'conllU', 'Universal Dependencies source files', typeConverter=TypeConverters.identity)
    numberOfIterations = Param(Params._dummy(), 'numberOfIterations', 'Number of iterations in training, converges to better accuracy', typeConverter=TypeConverters.toInt)

    @keyword_only
    def __init__(self):
        super(TypedDependencyParserApproach, self).__init__(classname='com.johnsnowlabs.nlp.annotators.parser.typdep.TypedDependencyParserApproach')
...
class TypedDependencyParserModel(AnnotatorModel):
    ...
    trainOptions = Param(Params._dummy(), 'trainOptions', 'Training Options', typeConverter=TypeConverters.identity)
    trainParameters = Param(Params._dummy(), 'trainParameters', 'Training Parameters', typeConverter=TypeConverters.identity)
    trainDependencyPipe = Param(Params._dummy(), 'trainDependencyPipe', 'Training dependency pipe', typeConverter=TypeConverters.identity)
    conllFormat = Param(Params._dummy(), 'conllFormat', 'CoNLL Format', typeConverter=TypeConverters.toString)

    def __init__(self, classname='com.johnsnowlabs.nlp.annotators.parser.typdep.TypedDependencyParserModel', java_model=None):
        super(TypedDependencyParserModel, self).__init__(classname=classname, java_model=java_model)
...
class WordEmbeddings(AnnotatorApproach, HasEmbeddingsProperties, HasStorage):
    name = 'WordEmbeddings'
    writeBufferSize = Param(Params._dummy(), 'writeBufferSize', 'buffer size limit before dumping to disk storage while writing', typeConverter=TypeConverters.toInt)
    readCacheSize = Param(Params._dummy(), 'readCacheSize', 'cache size for items retrieved from storage. Increase for performance but higher memory consumption', typeConverter=TypeConverters.toInt)
    ...
    def __init__(self):
        ...
        self._setDefault(caseSensitive=False, writeBufferSize=10000, storageRef=self.uid)
...
class WordEmbeddingsModel(AnnotatorModel, HasEmbeddingsProperties, HasStorageModel):
    ...
    def __init__(self, classname='com.johnsnowlabs.nlp.embeddings.WordEmbeddingsModel', java_model=None):
        super(WordEmbeddingsModel, self).__init__(classname=classname, java_model=java_model)
...
class BertEmbeddings(AnnotatorModel, HasEmbeddingsProperties, HasCaseSensitiveProperties, HasStorageRef):
    name = 'BertEmbeddings'
    maxSentenceLength = Param(Params._dummy(), 'maxSentenceLength', 'Max sentence length to process', typeConverter=TypeConverters.toInt)
    ...
    configProtoBytes = Param(Params._dummy(), 'configProtoBytes', 'ConfigProto from tensorflow, serialized into byte array. Get with config_proto.SerializeToString()', TypeConverters.toListString)
    poolingLayer = Param(Params._dummy(), 'poolingLayer', 'Set BERT pooling layer to: -1 for last hidden layer, -2 for second-to-last hidden layer, and 0 for first layer which is called embeddings', typeConverter=TypeConverters.toInt)
    ...
    def __init__(self, classname='com.johnsnowlabs.nlp.embeddings.BertEmbeddings', java_model=None):
        ...
        self._setDefault(dimension=768, batchSize=32, maxSentenceLength=128, caseSensitive=True, poolingLayer=0)
...
class SentenceEmbeddings(AnnotatorModel, HasEmbeddingsProperties, HasStorageRef):
    ...
    poolingStrategy = Param(Params._dummy(), 'poolingStrategy', 'Choose how you would like to aggregate Word Embeddings to Sentence Embeddings: AVERAGE or SUM', typeConverter=TypeConverters.toString)
...
class StopWordsCleaner(AnnotatorModel):
    ...
    def __init__(self):
        ...
        self._setDefault(stopWords=StopWordsCleaner.loadDefaultStopWords('english'), caseSensitive=False, locale=self._java_obj.getLocale())

    stopWords = Param(Params._dummy(), 'stopWords', 'The words to be filtered out', typeConverter=TypeConverters.toListString)
    caseSensitive = Param(Params._dummy(), 'caseSensitive', 'whether to do a case sensitive ' + 'comparison over the stop words', typeConverter=TypeConverters.toBoolean)
    locale = Param(Params._dummy(), 'locale', 'locale of the input. ignored when case sensitive ' + 'is true', typeConverter=TypeConverters.toString)
...
class NGramGenerator(AnnotatorModel):
    ...
    enableCumulative = Param(Params._dummy(), 'enableCumulative', 'whether to calculate just the actual n-grams ' + 'or all n-grams from 1 through n', typeConverter=TypeConverters.toBoolean)
...
class ChunkEmbeddings(AnnotatorModel):
    ...
    poolingStrategy = Param(Params._dummy(), 'poolingStrategy', 'Choose how you would like to aggregate Word Embeddings to Chunk Embeddings:' + 'AVERAGE or SUM', typeConverter=TypeConverters.toString)
...
class NerOverwriter(AnnotatorModel):
    ...
    stopWords = Param(Params._dummy(), 'stopWords', 'The words to be overwritten', typeConverter=TypeConverters.toListString)
    newResult = Param(Params._dummy(), 'newResult', 'new NER class to apply to those stopwords', typeConverter=TypeConverters.toString)
...
class UniversalSentenceEncoder(AnnotatorModel, HasEmbeddingsProperties, HasStorageRef):
    ...
    def __init__(self, classname='com.johnsnowlabs.nlp.embeddings.UniversalSentenceEncoder', java_model=None):
        super(UniversalSentenceEncoder, self).__init__(classname=classname, java_model=java_model)
...
class ElmoEmbeddings(AnnotatorModel, HasEmbeddingsProperties, HasCaseSensitiveProperties, HasStorageRef):
    ...
    batchSize = Param(Params._dummy(), 'batchSize', 'Batch size. Large values allows faster processing but requires more memory.', typeConverter=TypeConverters.toInt)
    configProtoBytes = Param(Params._dummy(), 'configProtoBytes', 'ConfigProto from tensorflow, serialized into byte array. Get with config_proto.SerializeToString()', TypeConverters.toListString)
    poolingLayer = Param(Params._dummy(), 'poolingLayer', 'Set ELMO pooling layer to: word_emb, lstm_outputs1, lstm_outputs2, or elmo', typeConverter=TypeConverters.toString)
    ...
    def setPoolingLayer(self, layer):
        if layer == 'word_emb':
            ...
            if layer == 'lstm_outputs1':
                return self._set(poolingLayer=layer)
            if layer == 'lstm_outputs2':
                return self._set(poolingLayer=layer)
            if layer == 'elmo':
                return self._set(poolingLayer=layer)
...
class ClassifierDLApproach(AnnotatorApproach):
    ...
    def __init__(self):
        ...
        self._setDefault(maxEpochs=30, lr=float(0.005), batchSize=64, dropout=float(0.5), enableOutputLogs=False)
...
class ClassifierDLModel(AnnotatorModel, HasStorageRef):
    ...
    def __init__(self, classname='com.johnsnowlabs.nlp.annotators.classifier.dl.ClassifierDLModel', java_model=None):
        super(ClassifierDLModel, self).__init__(classname=classname, java_model=java_model)
...
class AlbertEmbeddings(AnnotatorModel, HasEmbeddingsProperties, HasCaseSensitiveProperties, HasStorageRef):
    ...
    def __init__(self, classname='com.johnsnowlabs.nlp.embeddings.AlbertEmbeddings', java_model=None):
        super(AlbertEmbeddings, self).__init__(classname=classname, java_model=java_model)
        self._setDefault(batchSize=32, dimension=768, maxSentenceLength=128)
...
class XlnetEmbeddings(AnnotatorModel, HasEmbeddingsProperties, HasCaseSensitiveProperties, HasStorageRef):
    ...
    def __init__(self, classname='com.johnsnowlabs.nlp.embeddings.XlnetEmbeddings', java_model=None):
        super(XlnetEmbeddings, self).__init__(classname=classname, java_model=java_model)
        self._setDefault(batchSize=32, dimension=768, maxSentenceLength=128)
...
class ContextSpellCheckerApproach(AnnotatorApproach):
    ...
    languageModelClasses = Param(Params._dummy(), 'languageModelClasses', 'Number of classes to use during factorization of the softmax output in the LM.', typeConverter=TypeConverters.toInt)
    wordMaxDistance = Param(Params._dummy(), 'wordMaxDistance', 'Maximum distance for the generated candidates for every word.', typeConverter=TypeConverters.toInt)
    maxCandidates = Param(Params._dummy(), 'maxCandidates', 'Maximum number of candidates for every word.', typeConverter=TypeConverters.toInt)
    caseStrategy = Param(Params._dummy(), 'caseStrategy', 'What case combinations to try when generating candidates.', typeConverter=TypeConverters.toInt)
    errorThreshold = Param(Params._dummy(), 'errorThreshold', 'Threshold perplexity for a word to be considered as an error.', typeConverter=TypeConverters.toFloat)
    epochs = Param(Params._dummy(), 'epochs', 'Number of epochs to train the language model.', typeConverter=TypeConverters.toInt)
    batchSize = Param(Params._dummy(), 'batchSize', 'Batch size for the training in NLM.', typeConverter=TypeConverters.toInt)
    initialRate = Param(Params._dummy(), 'initialRate', 'Initial learning rate for the LM.', typeConverter=TypeConverters.toFloat)
    finalRate = Param(Params._dummy(), 'finalRate', 'Final learning rate for the LM.', typeConverter=TypeConverters.toFloat)
    validationFraction = Param(Params._dummy(), 'validationFraction', 'Percentage of datapoints to use for validation.', typeConverter=TypeConverters.toFloat)
    minCount = Param(Params._dummy(), 'minCount', 'Min number of times a token should appear to be included in vocab.', typeConverter=TypeConverters.toInt)
    compoundCount = Param(Params._dummy(), 'compoundCount', 'Min number of times a compound word should appear to be included in vocab.', typeConverter=TypeConverters.toInt)
    classCount = Param(Params._dummy(), 'classCount', 'Min number of times the word need to appear in corpus to not be considered of a special class.', typeConverter=TypeConverters.toInt)
    tradeoff = Param(Params._dummy(), 'tradeoff', 'Tradeoff between the cost of a word error and a transition in the language model.', typeConverter=TypeConverters.toFloat)
    weightedDistPath = Param(Params._dummy(), 'weightedDistPath', 'The path to the file containing the weights for the levenshtein distance.', typeConverter=TypeConverters.toString)
    maxWindowLen = Param(Params._dummy(), 'maxWindowLen', 'Maximum size for the window used to remember history prior to every correction.', typeConverter=TypeConverters.toInt)
    configProtoBytes = Param(Params._dummy(), 'configProtoBytes', 'ConfigProto from tensorflow, serialized into byte array. Get with config_proto.SerializeToString()', TypeConverters.toListString)
    ...
    @keyword_only
    def __init__(self):
        super(ContextSpellCheckerApproach, self).__init__(classname='com.johnsnowlabs.nlp.annotators.spell.context.ContextSpellCheckerApproach')
...
class ContextSpellCheckerModel(AnnotatorModel):
    ...
    weightedDistPath = Param(Params._dummy(), 'weightedDistPath', 'The path to the file containing the weights for the levenshtein distance.', typeConverter=TypeConverters.toString)
    maxWindowLen = Param(Params._dummy(), 'maxWindowLen', 'Maximum size for the window used to remember history prior to every correction.', typeConverter=TypeConverters.toInt)
    gamma = Param(Params._dummy(), 'gamma', 'Controls the influence of individual word frequency in the decision.', typeConverter=TypeConverters.toFloat)
    configProtoBytes = Param(Params._dummy(), 'configProtoBytes', 'ConfigProto from tensorflow, serialized into byte array. Get with config_proto.SerializeToString()', TypeConverters.toListString)
    ...
    def __init__(self, classname='com.johnsnowlabs.nlp.annotators.spell.context.ContextSpellCheckerModel', java_model=None):
        super(ContextSpellCheckerModel, self).__init__(classname=classname, java_model=java_model)
...
class SentimentDLApproach(AnnotatorApproach):
    ...
    def __init__(self):
        ...
        self._setDefault(maxEpochs=30, lr=float(0.005), batchSize=64, dropout=float(0.5), enableOutputLogs=False, threshold=0.6, thresholdLabel='neutral')
...
class SentimentDLModel(AnnotatorModel, HasStorageRef):
    ...
    def __init__(self, classname='com.johnsnowlabs.nlp.annotators.classifier.dl.SentimentDLModel', java_model=None):
        super(SentimentDLModel, self).__init__(classname=classname, java_model=java_model)
        self._setDefault(threshold=0.6, thresholdLabel='neutral')
