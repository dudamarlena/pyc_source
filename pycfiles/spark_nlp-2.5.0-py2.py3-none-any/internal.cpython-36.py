...
# [GCC 8.4.0]
# Embedded file name: /Users/maziyar/Desktop/Work/JohnSnowLabs/spark-nlp/python/build/lib/sparknlp/internal.py
# Compiled at: 2020-05-06 10:55:08
# Size of source mod 2**32: 10972 bytes
from abc import ABC
...
class ParamsGettersSetters(Params):
    ...
    def __init__(self):
        ...
        for param in self.params:
            ...
            fg_attr = 'get' + re.sub('(?:^|_)(.)', lambda m: m.group(1).upper(), param_name)
            fs_attr = 'set' + re.sub('(?:^|_)(.)', lambda m: m.group(1).upper(), param_name)
            try:
...
class AnnotatorJavaMLReader(JavaMLReader):
    ...
    def _java_loader_class(cls, clazz):
        if hasattr(clazz, '_java_class_name'):
            if clazz._java_class_name is not None:
    ...
    def fit(self, dataset, params=None, pipeline=None):
        if params is None:
            ...
            if isinstance(params, dict):
                ...
                raise ValueError('Params must be either a param map or a list/tuple of param maps, but got %s.' % type(params))
...
class _GetResourceSize(ExtendedJavaWrapper):
    ...
    def __init__(self, name, language, remote_loc):
        super(_GetResourceSize, self).__init__('com.johnsnowlabs.nlp.pretrained.PythonResourceDownloader.getDownloadSize', name, language, remote_loc)


class _ShowUnCategorizedResources(ExtendedJavaWrapper):

    def __init__(self):
        super(_ShowUnCategorizedResources, self).__init__('com.johnsnowlabs.nlp.pretrained.PythonResourceDownloader.showUnCategorizedResources')


class _ShowPublicPipelines(ExtendedJavaWrapper):

    def __init__(self):
        super(_ShowPublicPipelines, self).__init__('com.johnsnowlabs.nlp.pretrained.PythonResourceDownloader.showPublicPipelines')


class _ShowPublicModels(ExtendedJavaWrapper):

    def __init__(self):
        super(_ShowPublicModels, self).__init__('com.johnsnowlabs.nlp.pretrained.PythonResourceDownloader.showPublicModels')
