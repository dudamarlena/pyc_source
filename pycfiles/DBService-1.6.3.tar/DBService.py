...
# [GCC 8.4.0]
# Embedded file name: /Users/zhanghang/Desktop/workspace/molbase/molbase_2017_7_3/release/DBService/DBService.py
# Compiled at: 2019-04-12 05:56:22
"""DB tools
...
class MongoService(DBService):
    ...
    def find(self, collection, query_sql, limit_count, skip_count=0, fetch_rate=5000, extract_fields={}, sort_fields={}, include_id=False):
        ...
        while rate < result_count:
            ...
            cmd = 'self.db["%(collection)s"].find(%(query_sql)s, projection=%(fields)s, limit=%(current_fetch_rate)s, skip=%(current_rate)s, sort=%(sort_fields)s)' % {'collection': collection, 
               'query_sql': query_sql, 
               'fields': fields, 
