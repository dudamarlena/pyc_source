...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/football/views.py
# Compiled at: 2012-11-15 09:46:37
import datetime
...
def _fixtures_batching(request, queryset, type='fixtures', window=1):
    ...
    this_week_start = this_week_start - datetime.timedelta(days=this_week_start.weekday())
    initial = False
    week_start = None
    ...
    fixtures = queryset.filter(datetime__gte=week_start, datetime__lte=week_end, completed=type == 'results' and True or False).order_by('datetime')
    previous_week_start = week_start - datetime.timedelta(days=window * 7)
    past_fixtures = queryset.filter(datetime__lt=week_start, completed=type == 'results' and True or False).order_by('-datetime')[:1]
    future_fixtures = queryset.filter(datetime__gte=week_end, completed=type == 'results' and True or False).order_by('datetime')[:1]
    ...
    if past_fixtures:
        ...
        previous_week_start = previous_week_start - datetime.timedelta(days=previous_week_start.weekday())
    if future_fixtures:
        next_week_start = future_fixtures[0].datetime.date()
        next_week_start = next_week_start - datetime.timedelta(days=next_week_start.weekday())
...
def league_logs(request, league_slug=None, template_name=None):
    ...
    if league_slug:
        ...
        league = leagues[0]
    extra = dict(league=league, leagues=leagues)
...
def league_fixtures(request, league_slug=None, template_name=None):
    ...
    (week_start, fixtures, previous_week_start, next_week_start) = _fixtures_batching(request, qs, window=1)
    extra = dict(league=league, leagues=leagues, week_start=week_start, fixtures=fixtures, previous_week_start=previous_week_start, next_week_start=next_week_start, has_pagination=previous_week_start or next_week_start)
...
def league_results(request, league_slug=None, template_name=None):
    ...
    (week_start, fixtures, previous_week_start, next_week_start) = _fixtures_batching(request, qs, type='results', window=1)
    extra = dict(league=league, leagues=leagues, week_start=week_start, fixtures=fixtures, previous_week_start=previous_week_start, next_week_start=next_week_start, has_pagination=previous_week_start or next_week_start)
...
def league_dashboard_basic(request, slug):
    ...
    extra = dict(object=league, related_object_list_top=related_items[:1], related_object_list=related_items[1:4])
...
def league_dashboard_web(request, slug):
    ...
    extra = dict(object=league, related_object_list=related_object_list, logs=league_logs_widget(request, league.slug).content, fixtures=league_fixtures_widget(request, league.slug).content, results=league_results_widget(request, league.slug).content)
    return render_to_response('football/league_dashboard.html', extra, context_instance=RequestContext(request))
...
def team_fixtures_widget(request, team_slug):
    ...
    (week_start, fixtures, previous_week_start, next_week_start) = _fixtures_batching(request, qs, window=4)
    extra = dict(team=team, week_start=week_start, fixtures=fixtures, previous_week_start=previous_week_start, next_week_start=next_week_start)
...
def team_results_widget(request, team_slug):
    team = get_object_or_404(Team, slug=team_slug)
    qs = Fixture.objects.filter(Q(home_team=team) | Q(away_team=team))
    (week_start, fixtures, previous_week_start, next_week_start) = _fixtures_batching(request, qs, type='results', window=4)
    extra = dict(team=team, week_start=week_start, fixtures=fixtures, previous_week_start=previous_week_start, next_week_start=next_week_start)
...
def team_players_widget(request, team_slug):
    team = get_object_or_404(Team, slug=team_slug)
    extra = dict(team=team, players=team.player_set.all().order_by('title'))
...
def team_dashboard_web(request, slug):
    team = get_object_or_404(Team, slug=slug)
    extra = dict(object=team, related_object_list=team.get_related_items('post_teams', 'reverse')[:4], fixtures=team_fixtures_widget(request, team.slug).content, results=team_results_widget(request, team.slug).content, players=team_players_widget(request, team.slug).content)
    return render_to_response('football/team_dashboard.html', extra, context_instance=RequestContext(request))
