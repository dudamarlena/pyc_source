...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/EMS/exaManagementSystem.py
# Compiled at: 2015-10-08 16:01:16
import sys, logging, logging.config
if float(sys.version[:3]) < 3.0:
    import ConfigParser
else:
    import configparser as ConfigParser
import shutil, getopt, os, subprocess
...
class ExaManagementSystem():
    ...
    def __init__(self):
        ...
        self.__examStructure = [
         'code', 'code/donne', 'code/solution', 'latex', 'latex/ressources', 'latex/ressources/figures', 'latex/ressources/code']
        self.__outputDir = './'
    ...
    def __doCreateExam(self, titles, numbers, outputDir, date, percentages, language, doPreview, filename=None):
        ...
        for number in numbers:
            serie.write('\\newpage\n')
            serie.write('\\renewcommand{\\includepath}{\\compilationpath/' + self.__exerciseDir + '/ex' + number + '/latex/ressources}' + '\n')
            exo = open(self.__exerciseDir + '/' + 'ex' + number + '/latex/exo-' + language + '.tex', 'r')
            for line in exo:
                serie.write(line)

            serie.write('\\clearpage\n')
    ...
    def __doCreateSolution(self, titles, numbers, outputDir, date, percentages, filename=None):
        ...
        for number in numbers:
            solution.write('\\newpage\n')
            solution.write('\\renewcommand{\\includepath}{\\compilationpath/' + self.__exerciseDir + '/ex' + number + '/latex/ressources}' + '\n')
    ...
    def __makeWorkBookTitlePage(self, _outputDir):
        ...
        for config in examConfigFiles:
            ...
            wbtitle.write('\\textsf{ \\textbf{Exam ' + examnumber + '}} \\dotfill' + '\n')
    ...
    def doMakeCatalogue(self):
        ...
        catalog.write('\\renewcommand{\\exercice}[1]{\\subsection*{Problem: #1}}' + '\n')
        catalog.write('\\renewcommand{\\solution}[1]{\\subsection*{Solution: #1}}' + '\n')
        catalog.write('\\renewcommand{\\question}[1]{\\subsubsection*{#1}}' + '\n')
        catalog.write('\n')
        catalog.write('\\makeatletter\n')
        catalog.write('\\renewcommand{\\section}{\\@startsection{section}{3}{2pt}{12pt}{10pt}{\\center \\huge \\sffamily \\bfseries}}' + '\n')
        catalog.write('\\renewcommand{\\thesection}{(\\roman{section})}' + '\n')
        catalog.write('\\renewcommand{\\thesubsection}{(\\roman{subsection})}' + '\n')
        exos = os.listdir(self.__exerciseDir)
        exos = Utils.natsort(exos)
        for exo in exos:
            if exo.find('ex') != -1:
                number = exo[2:]
                catalog.write('\\section*{Exercise ' + number + '}' + '\n')
                catalog.write('\\renewcommand{\\includepath}{\\compilationpath/' + self.__exerciseDir + '/ex' + number + '/latex/ressources}' + '\n')
                for lang in self.__languages:
                    catalog.write('\\renewcommand{\\exercice}[1]{\\subsection*{Problem ' + lang + ': #1}}' + '\n')
                ...
                catalog.write('\\renewcommand{\\includepath}{\\compilationpath/' + self.__exerciseDir + '/ex' + number + '/latex/ressources}' + '\n')
                catalog.write('\\renewcommand{\\exercice}[1]{\\subsection*{Solution: #1}}' + '\n')
