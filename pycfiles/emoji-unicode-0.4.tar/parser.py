...
# [GCC 8.4.0]
# Embedded file name: /home/esteban/PycharmProjects/emoji-awesome/emoji_unicode/parser.py
...
def replace(txt, callback):
    ...
    def func(m):
        return callback(Emoji(unicode=m.group(b'emoji')))

    return re.sub(PATTERN, func, txt)


def normalize(code_points, separator=b'-'):
    """
    ...
    :rtype: str
    """
    return separator.join(c.lstrip(b'0').lower() for c in code_points.split(separator) if code_point_to_unicode(c) not in JOINER_CHARS)