...
# [GCC 8.4.0]
# Embedded file name: /home/esteban/PycharmProjects/emoji-unicode/emoji_unicode/models.py
# Compiled at: 2015-11-19 09:50:57
...
class Emoji(object):
    ...
    def code_points(self):
        ...
        self._code_points = '-'.join(unicode_to_code_point(u) for u in self.unicode if u not in JOINER_CHARS)
        return self._code_points

    def as_map(self):
        """
        A map containing the individual unicode chars and code points.        The code points are normalized as by :py:func:`.normalize`
        ...
        if self._map is not None:
            return self._map
        self._map = [(u, unicode_to_code_point(u)) for u in self.unicode if u not in JOINER_CHARS]
        return self._map