...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/QUBEKit/run.py
# Compiled at: 2019-09-30 10:00:18
# Size of source mod 2**32: 47606 bytes
...
from QUBEKit.utils.helpers import mol_data_from_csv, generate_bulk_csv, append_to_log, pretty_progress, pretty_print, Configure, unpickle, make_and_change_into, COLOURS, display_molecule_objects
import argparse
...
import subprocess as sp, sys, numpy as np
...
class ArgsAndConfigs:
    ...
    def __init__(self):
        ...
        if not os.path.exists(config_folder):
            ...
            elif self.args.bulk_run is not None:
                self.handle_bulk()
            else:
    ...
    def parse_commands():
        ...
        class SetupAction(argparse.Action):
            __doc__ = 'The setup action class that is called when setup is found in the command line.'
            ...
            def __call__(self, pars, namespace, values, option_string=None):
                ...
                if choice == 1:
                    ...
                    if choice == 3:
                        ...
                        sys.exit('Cancelling setup; no changes made. If you accidentally entered the wrong key, restart with QUBEKit -setup')
                sys.exit()

        class CSVAction(argparse.Action):
            __doc__ = 'The csv creation class run when the csv option is used.'
        ...
        class ProgressAction(argparse.Action):
            __doc__ = 'Run the pretty progress function to get the progress of all running jobs.'
        ...
        class DisplayMolAction(argparse.Action):
            __doc__ = 'Display the molecule objects requested'
        ...
        class TorsionMakerAction(argparse.Action):
            __doc__ = 'Help the user make a torsion scan file.'
        ...
        intro = 'Welcome to QUBEKit! For a list of possible commands, use the help command: -h.Alternatively, take a look through our github page for commands, recipes and common problems:https://github.com/qubekit/QUBEKit'
        parser = argparse.ArgumentParser(prog='QUBEKit', formatter_class=(argparse.RawDescriptionHelpFormatter), description=intro)
        parser.add_argument('-c', '--charge', type=int, help='Enter the charge of the molecule, default 0.')
        parser.add_argument('-m', '--multiplicity', type=int, help='Enter the multiplicity of the molecule, default 1.')
        parser.add_argument('-threads', '--threads', type=int, help='Number of threads used in various stages of analysis, especially for engines like PSI4, Gaussian09, etc. Value is given as an int.')
        parser.add_argument('-memory', '--memory', type=int, help='Amount of memory used in various stages of analysis, especially for engines like PSI4, Gaussian09, etc. Value is given as an int, e.g. 6GB is simply 6.')
        parser.add_argument('-ddec', '--ddec_version', choices=[3, 6], type=int, help='Enter the ddec version for charge partitioning, does not effect ONETEP partitioning.')
        parser.add_argument('-geo', '--geometric', choices=[True, False], type=string_to_bool, help='Turn on geometric to use this during the qm optimisations, recommended.')
        parser.add_argument('-bonds', '--bonds_engine', choices=['psi4', 'g09', 'g16'], help='Choose the QM code to calculate the bonded terms.')
        parser.add_argument('-charges', '--charges_engine', choices=['onetep', 'chargemol'], help='Choose the method to do the charge partitioning.')
        parser.add_argument('-density', '--density_engine', choices=['onetep', 'g09', 'g16', 'psi4'], help='Enter the name of the QM code to calculate the electron density of the molecule.')
        parser.add_argument('-solvent', '--solvent', choices=[True, False], type=string_to_bool, help='Enter whether or not you would like to use a solvent.')
        parser.add_argument('-convergence', '--convergence', choices=['GAU', 'GAU_TIGHT', 'GAU_VERYTIGHT'], type=(str.upper),
          help='Enter the convergence criteria for the optimisation.')
        parser.add_argument('-param', '--parameter_engine', choices=['xml', 'antechamber', 'openff', 'none'], help='Enter the method of where we should get the initial molecule parameters from, if xml make sure the xml has the same name as the pdb file.')
        parser.add_argument('-mm', '--mm_opt_method', choices=['openmm', 'rdkit_mff', 'rdkit_uff', 'none'], help='Enter the mm optimisation method for pre qm optimisation.')
        parser.add_argument('-config', '--config_file', default='default_config', choices=(Configure().show_ini()), help='Enter the name of the configuration file you wish to use for this run from the list available, defaults to master.')
        parser.add_argument('-theory', '--theory', help='Enter the name of the qm theory you would like to use.')
        parser.add_argument('-basis', '--basis', help='Enter the basis set you would like to use.')
        ...
        parser.add_argument('-progress', '--progress', nargs='?', const=True, help='Get the current progress of a QUBEKit single or bulk job.',
        ...
        parser.add_argument('-tor_test', '--torsion_test', action='store_true', help='Enter True if you would like to run a torsion test on the chosen torsions.')
        parser.add_argument('-tor_make', '--torsion_maker', action=TorsionMakerAction, help='Allow QUBEKit to help you make a torsion input file for the given molecule')
        parser.add_argument('-log', '--log', type=str, help='Enter a name to tag working directories with. Can be any alphanumeric string.This helps differentiate (by more than just date) different analyses of the same molecule.')
        parser.add_argument('-vib', '--vib_scaling', type=float, help='Enter the vibrational scaling to be used with the basis set.')
        parser.add_argument('-iters', '--iterations', type=int, help='Max number of iterations for QM scan.')
        parser.add_argument('-constraints', '--constraints_file', type=str, help='The name of the geometric constraints file.')
        parser.add_argument('-dihedrals', '--dihedral_file', type=str, help='The name of the qubekit/tdrive torsion file.')
        parser.add_argument('-v', '--verbose', choices=[True, False], type=string_to_bool, default=True, help='Decide whether the log file should contain all the input/output information')
        parser.add_argument('-display', '--display', type=str, nargs='+', action=DisplayMolAction, help='Get the molecule object with this name in the cwd')
        groups = parser.add_mutually_exclusive_group()
        groups.add_argument('-setup', '--setup_config', nargs='?', const=True, help='Setup a new configuration or edit an existing one.',
          action=SetupAction)
        groups.add_argument('-sm', '--smiles', nargs='+', help='Enter the smiles string of a molecule as a starting point.')
        groups.add_argument('-bulk', '--bulk_run', help='Enter the name of the csv file to run as bulk, bulk will use smiles unless it finds a molecule file with the same name.')
        groups.add_argument('-csv', '--csv_filename', action=CSVAction, nargs='*', help='Enter the name of the csv file you would like to create for bulk runs.Optionally, you may also add the maximum number of molecules per file.')
...
class Execute:
    ...
    def __init__(self, molecule):
        ...
        self.engine_dict = {'psi4':PSI4, 
         'g09':Gaussian,  'g16':Gaussian,  'onetep':ONETEP}
    ...
    def create_log(self):
        ...
        copy_files = [
         f"{self.molecule.name}.xml", self.molecule.filename]
    ...
    def run(self, torsion_options=None):
        ...
        stage_dict = {'parametrise':[
         ...
         'hessian':[
          'Calculating Hessian matrix', 'Hessian matrix calculated and confirmed to be symmetric'], 
         'mod_sem':[
          'Calculating bonds and angles with modified Seminario method', 'Bonds and angles calculated'], 
         'density':[
          f"Performing density calculation with {self.molecule.density_engine}",
          'Density calculation complete'], 
         'charges':[
          f"Chargemol calculating charges using DDEC{self.molecule.ddec_version}", 'Charges calculated'], 
         'lennard_jones':[
          'Performing Lennard-Jones calculation', 'Lennard-Jones parameters calculated'], 
         'torsion_scan':[
          'Performing QM-constrained optimisation with Torsiondrive',
          'Torsiondrive finished and QM results saved'], 
         'torsion_optimise':[
          'Performing torsion optimisation', 'Torsion optimisation complete'], 
         'finalise':[
          'Finalising analysis', 'Molecule analysis complete!'], 
         'pause':[
          'Pausing analysis', 'Analysis paused!'], 
         'skip':[
          'Skipping section', 'Section skipped'], 
         'torsion_test':[
          'Testing torsion single point energies', 'Torsion testing complete']}
    ...
    def parametrise(molecule):
        ...
        param_dict = {'antechamber':AnteChamber, 
         'xml':XML,  'openff':OpenFF}
            ...
            if molecule.mm_opt_method == 'openmm':
                if molecule.parameter_engine != 'none':
                    ...
                    with open('log.txt', 'w+') as (log):
                        sp.run(f"geometric-optimize --reset --epsilon 0.0 --maxiter {molecule.iterations} --pdb {molecule.name}.pdb --openmm {molecule.name}.xml {molecule.constraints_file if molecule.constraints_file is not None else ''}", shell=True,
                          stdout=log,
                          stderr=log)
                    molecule.read_file(f"{molecule.name}_optim.xyz", input_type='traj')
                    molecule.coords['mm'] = molecule.coords['traj'][(-1)]
                else:
                    raise OptimisationFailed('You can not optimise a molecule with OpenMM and no initial parameters; consider parametrising or using UFF/MFF in RDKit')
            else:
                rdkit_ff = {'rdkit_mff':'MFF', 
                 'rdkit_uff':'UFF'}
    ...
    def qm_optimise(self, molecule):
        ...
        if molecule.geometric and molecule.bonds_engine == 'psi4':
            ...
            result = qceng.call_qcengine(engine='geometric', driver='gradient', input_type=(f"{'mm' if list(molecule.coords['mm']) else 'input'}"))
            restart_count = 0
            while not result['success']:
                if restart_count < max_restarts:
                    append_to_log(f"{molecule.bonds_engine} optimisation failed with error {result['error']}; restarting", msg_type='minor')
                    try:
            ...
            result = qm_engine.generate_input(input_type=(f"{'mm' if list(molecule.coords['mm']) else 'input'}"), optimise=True,
            ...
            while not result['success']:
                if restart_count < max_restarts:
                    ...
                    if result['error'] == 'FileIO':
                        ...
                        if result['error'] == 'Max iterations':
            ...
            if not result['success']:
                raise OptimisationFailed(f"{molecule.bonds_engine} optimisation did not converge after 3 restarts; last error {result['error']}")
    ...
    def hessian(molecule):
        ...
        if molecule.bonds_engine in ('g09', 'g16'):
            ...
            try:
                ...
                append_to_log('qm_optimise checkpoint not found, optimising first to refine atomic coordinates', msg_type='minor')
    ...
    def density(self, molecule):
        ...
        if molecule.density_engine == 'onetep':
            ...
            qm_engine.generate_input(input_type=('qm' if list(molecule.coords['qm']) else 'input'), density=True,
    ...
    def pause():
        ...
        """
        printf('QUBEKit stopping at ONETEP step!\n To continue please move the ddec.onetep file and xyz file to the density folder and use QUBEKit -restart lennard_jones.')
    ...
    def store_torsions(molecule, torsions_list=None):
        ...
        if torsions_list is not None:
            ...
            for torsion in torsions_list:
                ...
                core = (
                 molecule.get_atom_with_name(tor[1]).atom_index, molecule.get_atom_with_name(tor[2]).atom_index)
