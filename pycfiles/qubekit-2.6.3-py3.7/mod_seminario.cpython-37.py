...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/QUBEKit/mod_seminario.py
# Compiled at: 2019-09-16 10:20:01
# Size of source mod 2**32: 14203 bytes
"""
...
class ModSemMaths:
    __doc__ = 'Static methods for various mathematical functions relevant to the modified Seminario method.'
    ...
    def force_constant_angle(angle, bond_lens, eigenvals, eigenvecs, coords, scalings):
        ...
        u_n = ModSemMaths.unit_vector_n(u_cb, u_ab)
        if not abs(sum(u_cb - u_ab)) < 0.01:
            if 1.99 < abs(sum(u_cb - u_ab)) < 2.01:
                k_theta, theta_0 = ModSemMaths.f_c_a_special_case(u_ab, u_cb, [bond_len_ab, bond_len_bc], [eigenvals_ab, eigenvals_cb], [eigenvecs_ab, eigenvecs_cb])
        else:
    ...
    def calculate_angles(self, eigenvals, eigenvecs):
        ...
        for i in range(len(central_atoms_angles)):
            ...
            for j in range(len(central_atoms_angles[i])):
                angle = (
                 central_atoms_angles[i][j][0], i, central_atoms_angles[i][j][1])
        ...
        with open('Modified_Seminario_Angles.txt', (f"{'w' if self.molecule.restart else 'a+'}")) as (angle_file):
            for i, angle in enumerate(self.molecule.angles):
                ...
                self.molecule.HarmonicAngleForce[angle] = [
                 theta_0[i] * constants.DEG_TO_RAD, k_theta[i] * conversion]
        ...
        with open('Modified_Seminario_Bonds.txt', (f"{'w' if self.molecule.restart else 'a+'}")) as (bond_file):
            for pos, bond in enumerate(bonds):
                ...
                self.molecule.HarmonicBondForce[bond] = [
                 bond_len_list[pos] / 10, conversion * k_b[pos]]
