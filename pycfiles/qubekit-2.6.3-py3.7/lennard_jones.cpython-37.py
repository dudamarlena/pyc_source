...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/QUBEKit/lennard_jones.py
...
class LennardJones:
    ...
    def extract_params_chargemol(self):
        ...
        if not os.path.exists(net_charge_file_name):
            raise FileNotFoundError('\nCannot find the DDEC output file.\nThis could be indicative of several issues.\nPlease check Chargemol is installed in the correct location and that the configs point to that location.')
    ...
    def extract_params_onetep(self):
        ...
        for atom in self.molecule.atoms:
            self.ddec_data.append([atom.atom_index + 1, atom.atomic_symbol] + [self.molecule.coords['input'][atom.atom_index][i] for i in range(3)])
    ...
    def calculate_sig_eps(self):
        ...
        for pos, atom in enumerate(self.ddec_data):
            if atom[(-1)]:
                sigma = (atom[(-1)] / atom[(-2)]) ** 0.16666666666666666
    ...
    def apply_symmetrisation(self):
        ...
        for sym_set in atom_types.values():
            ...
            if len(sym_set) > 1:
                ...
                for atom in sym_set:
                    self.non_bonded_force[atom] = [
                     round(charge, 6), round(sigma, 6), round(epsilon, 6)]
    ...
    def extract_extra_sites(self):
        ...
        """
        w1o, w2o, w3o = (1.0, 0.0, 0.0)
        w1x, w2x, w3x = (-1.0, 1.0, 0.0)
        w1y, w2y, w3y = (-1.0, 0.0, 1.0)
            ...
            if element != 'X':
                for pos_site in lines[i + 3:]:
                    if str(pos_site.split()[0]) != 'X':
                        ...
                        ab = w1x * parent_pos + w2x * close_a + w3x * close_b
                        ac = w1y * parent_pos + w2y * close_a + w3y * close_b
                        ...
                        sites[sites_no] = [
                         (
                          parent, closest_atoms[0], closest_atoms[1]), (p1 / 10, p2 / 10, p3 / 10), charge]
                        sites_no += 1
