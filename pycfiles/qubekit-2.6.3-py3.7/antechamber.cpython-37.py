...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/QUBEKit/parametrisation/antechamber.py
    ...
    def antechamber_cmd(self):
        ...
        with TemporaryDirectory() as (temp):
            ...
            with open('Antechamber.log', 'a') as (log):
                sp.run(f"parmchk2 -i out.mol2 -f mol2 -o out.frcmod -s {self.fftype}", shell=True,
                  stdout=log,
                  stderr=log)
        ...
        with TemporaryDirectory() as (temp):
            ...
            if not (os.path.exists('out.prmtop') and os.path.exists('out.inpcrd')):
