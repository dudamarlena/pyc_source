...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/QUBEKit/utils/helpers.py
...
import math, operator, os, pickle, numpy as np
...
class Configure:
    ...
    excited = {'excited_state':'False', 
     'excited_theory':'TDA', 
     'nstates':'3', 
     'excited_root':'1', 
     'use_pseudo':'False', 
     'pseudo_potential_block':''}
    descriptions = {'chargemol':'/home/<QUBEKit_user>/chargemol_09_26_2017', 
     'log':'999'}
    help = {'theory':';Theory to use in freq and dihedral scans recommended wB97XD or B3LYP, for example', 
     'basis':';Basis set', 
    ...
    def load_config(self, config_file='default_config'):
        ...
        clean_ints = [
         'threads', 'memory', 'iterations', 'ddec_version', 'dih_start',
...
def append_to_log(message, msg_type='major'):
    ...
    while 'QUBEKit_log.txt' not in os.listdir(search_dir):
        ...
        assert search_dir, 'Cannot locate QUBEKit log file.'
...
def pretty_progress():
    ...
    header_string = '{:15}{:>10}{:>10}{:>10}{:>10}{:>10}{:>10}{:>10}{:>10}{:>10}{:>10}'
    print(header_string.format('Name', 'Param', 'MM Opt', 'QM Opt', 'Hessian', 'Mod-Sem', 'Density', 'Charges', 'L-J', 'Tor Scan', 'Tor Opt'))
...
def populate_progress_dict(file_name):
    ...
    """
    search_terms = ('PARAMETRISATION', 'MM_OPT', 'QM_OPT', 'HESSIAN', 'MOD_SEM', 'DENSITY',
                    'CHARGE', 'LENNARD', 'TORSION_S', 'TORSION_O')
...
def unpickle(location=None):
    ...
    if location is not None:
        ...
        while pickle_file not in os.listdir(search_dir):
            search_dir = os.path.split(search_dir)[0]
            assert search_dir, 'Pickle file not found; have you deleted it?'
...
def check_symmetry(matrix, error=1e-05):
    """Check matrix is symmetric to within some error."""
    with assert_wrapper(ValueError):
        assert np.allclose(matrix, (matrix.T), atol=error), 'Matrix is not symmetric.'
    print(f"{COLOURS.purple}Symmetry check successful. The matrix is symmetric within an error of {error}.{COLOURS.end}")
    return True


def check_net_charge(charges, ideal_net=0, error=1e-05):
    ...
    print(f"{COLOURS.purple}Charge check successful. Net charge is within {error} of the desired net charge of {ideal_net}.{COLOURS.end}")
    return True
...
def missing_import(name, fail_msg=''):
    ...
    def init(self, *args, **kwargs):
        raise ImportError(f"The class {name} you tried to call is not importable; this is likely due to it not doing installed.\n\n{Fail Message: {fail_msg} if fail_msg else ''}")
