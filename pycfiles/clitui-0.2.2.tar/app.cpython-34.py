...
# Embedded file name: build/bdist.linux-x86_64/egg/clitube/app.py
# Compiled at: 2015-10-13 09:40:35
# Size of source mod 2**32: 9573 bytes
import requests, html, re, os, subprocess, curses, itertools, clitube.model as model
URL_SEARCH = 'https://www.youtube.com/results?filters=video&search_query={}&page={}'
...
def youtube_search(search):
    for page in itertools.count(start=1, step=1):
        ...
        if r.status_code == 200:
            yield zip(re.findall(PATTERN_ID, r.text), map(html.unescape, re.findall(PATTERN_NAME, r.text)), re.findall(PATTERN_TIME, r.text))
        else:
...
def download(uid, path):
    ...
    return subprocess.Popen(['youtube-dl', url,
     '-o', path], stdout=FNULL, stderr=FNULL)
...
def play(uid):
    ...
    player = subprocess.Popen(['mplayer',
     ...
     PIPE_STREAM], stdout=FNULL, stderr=FNULL)
    return (
     dl, player)
    ...
    while 1:
        if not playlist.is_over():
            ...
            if redraw_clitube:
                ...
                search_scr.clear()
                search_scr.box()
                search_scr.addstr(0, int(search_width / 2) - 5, ' CLItube ', curses.A_BOLD)
            ...
            if redraw_playlist:
                playlist_scr = stdscr.subwin(playlist_height, playlist_width, 0, search_width)
                playlist_scr.clear()
                playlist_scr.box()
        ...
        if cmdMode:
            if c == '\n':
                ...
                if type(c) == str and ord(c) == 27:
                    cmd = ''
                    cmdMode = False
                    stdscr.deleteln()
                else:
                    if c == curses.KEY_BACKSPACE and len(cmd) > 1:
                        cmd = cmd[:-1]
                        redraw_cmd = True
                    elif c != -1:
                        try:
