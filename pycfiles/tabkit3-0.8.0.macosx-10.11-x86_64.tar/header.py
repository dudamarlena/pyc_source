...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/tabkit/header.py
# Compiled at: 2016-06-08 10:26:24
import re
...
def _field_list(iterable, class_):
    return [ (isinstance(field, class_) or class_)(*field) if 1 else field for field in iterable ]
...
def parse_order(string):
    ...
    for field in re.findall('[^,\\s]+', string):
        ...
        try:
            ...
            yield (
             order_name, order_type, order_desc)
        except ValueError:
...
def make_header(desc):
    ...
    if desc.order:
        header += '\t# ORDER: ' + (', ').join('%s%s%s' % (o.name, ':' + o.type if o.type != 'str' else '', ':desc' if o.desc else '') for o in desc.order)
    return '# ' + header
