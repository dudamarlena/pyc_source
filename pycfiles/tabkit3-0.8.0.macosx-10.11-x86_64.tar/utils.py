...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/tabkit/utils.py
# Compiled at: 2016-06-08 10:22:42
...
class Files(object):
    ...
    def call(self, args):
        cmd = 'LC_ALL=C ' + args.pop(0) + ' ' + (' ').join(quote(arg) for arg in args) + ' ' + (' ').join(self.descriptors())
        ...
        def parse():
            ...
            try:
                for lineno, line in enumerate(stream):
                    ...
                    try:
                        values = [ f.type(v) for v, f in zip(raw, self.data_desc) ]
                    except ValueError as e:
                        raise TabkitException(str(e).capitalize())

                    if len(values) != rowlen:
                        if strict:
                            raise TabkitException('Found %d columns, whereas %d columns expected' % (
                             len(values), rowlen))
                        if len(values) > rowlen:
        ...
        for field in self.data_desc:
            ...
            if value is not None:
                ...
                try:
                    ...
                    raise TabkitException('Value convertable to type %s expected in field %r, but got %r' % (
...
class LooseWriter(WriterBase):
    ...
    def __call__(self, **kwargs):
        self.fh.write('%s\n' % ('\t').join(_str(kwargs.get(name)) for name in self.data_desc.field_names))
