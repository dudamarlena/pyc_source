...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\splashclient\core.py
# Compiled at: 2018-11-08 09:10:43
# Size of source mod 2**32: 4160 bytes
"""
基于requests来操作splash
@author: yubang
创建于2018年11月8日
...
class SplashClient:
    ...
    def __init__(self, splash_url, default_time=30, default_proxy=None, default_header=None, default_wait=1):
        ...
        self.default_header = default_header if default_header else {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3497.81 Safari/537.36'}
    ...
    def request_splash(self, url, http_method, timeout, wait, need_png=False):
        ...
        data = dumps(source)
        try:
            result = requests.post((self.splash_url), data=data, headers={'Content-Type': 'application/json'}, timeout=timeout)
        except:
