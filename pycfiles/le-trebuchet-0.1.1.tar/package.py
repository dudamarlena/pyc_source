...
# [GCC 8.4.0]
# Embedded file name: /code/trebuchet/trebuchet/lib/package.py
# Compiled at: 2013-12-04 10:13:33
from fabric.api import local, settings, lcd, prefix
import os
from datetime import datetime
import fnmatch, shutil, copy
from .utils import prepare_folder, prepare_virtual_env, local_sed, get_temp_path
from .my_yaml import flatten_dict
from .custom_file import get_custom_file, DEBIANCustomFile

def get_packages(application_path, config=None, architecture=None, options=None, version_options=None):
    ...
    if config_environment:
        if config_environment['type'] == 'python':
            venv = PythonEnvironmentPackage(config_environment['name'] + name_suffix, application_path, architecture=architecture, version=version_options.get('env'))
            venv.prepare(binary=config_environment.get('binary', ''), requirements=config_environment.get('requirements', []), debian_scripts=config_environment.get('debian_scripts', {'postinst': [], 'preinst': [], 'prerm': []}), post_environment_steps=config_environment.get('post_environment', []), pip_options=options.get('pip_options', ''))
        else:
            raise NotImplementedError('environment type: %s' % config_environment['type'])
    if config.get('type') == 'application':
        pkg = ApplicationPackage(config['name'] + name_suffix, application_path, environment=venv, version=version_options.get('app'))
    else:
        if config.get('type') == 'configuration':
            pkg = ConfigurationPackage(config['name'] + name_suffix, application_path, environment=venv, version=version_options.get('app'), maiden_name=config['name'])
        else:
            raise NotImplementedError('package type: %s' % config.get('type'))
        pkg.prepare(exclude_folders=config.pop('exclude_folders', []), build_assets_steps=config.pop('build_assets', []), debian_scripts=config.pop('debian_scripts', {'postinst': [], 'preinst': [], 'prerm': []}), configuration=config, config_applications=config_applications)
        extra_files_list = {}
        for file_ in config_extra_files:
            file_bin = get_custom_file(file_['type'], file_['name'] + name_suffix, file_['template'], file_.get('target_path', None), file_.get('target_extension', None), file_.get('target_is_executable', None), file_.get('target_filename', None), file_['name'], options=config)
        ...
        for service in config_services:
            srv = ServicePackage(service['name'] + name_suffix, pkg, version=version_options.get('service'))
            srv.prepare(binary=service['binary_name'] + name_suffix, binary_file=extra_files_list[service['binary_name']], debian_scripts=service.get('debian_scripts', {'postinst': [], 'preinst': [], 'prerm': []}), env_var=service.get('env_var', {}))
            yield srv
        ...
        for staticfileconf in config_static:
            static = StaticPackage(staticfileconf['name'] + name_suffix, pkg, version=version_options.get('static_files'))
            static.prepare(folders=staticfileconf.pop('folders'), debian_scripts=staticfileconf.pop('debian_scripts', {'postinst': [], 'preinst': [], 'prerm': []}))
            yield static
...
class Package(object):
    ...
    def create_deb(self, build_path, extra_description=None):
        ...
        options = {'full_package_name': self.full_package_name, 
        ...
        for file_custom in local('ls %s' % os.path.join(my_cwd, '..', 'templates', 'DEBIAN'), capture=True).split():
            file_ = DEBIANCustomFile(file_custom, os.path.join('DEBIAN', file_custom))
            file_.build(build_path, options=options, extra_template_dir=self.path)
    ...
    def get_extra_file_final_path(self, file_name):
        ...
        return os.path.join('/', self.extra_files[file_name].relative_filepath)
    ...
    def build_extra_files(self, build_path):
        ...
        for key, binary in self.extra_files.iteritems():
            binary.build(build_path, options, extra_template_dir=self.path)
...
class ApplicationPackage(Package):
    ...
    def __init__(self, name, application_path, environment=None, version=None):
    ...
    def prepare(self, exclude_folders=None, build_assets_steps=None, debian_scripts=None, configuration=None, config_applications=None):
    ...
    @property
    def extra_config(self):
        return {'base_template': 'base_shell.sh', 
...
class PythonEnvironmentPackage(Package):
    ...
    def prepare(self, binary=None, requirements=None, post_environment_steps=None, pip_options='', debian_scripts=None):
        self.binary = binary
    ...
    def pre_build(self, build_path):
        ...
        with lcd(venv_bin_path):
            for script in local('ls', capture=True).split():
                local_sed(script, self.working_path, self.target_venv)
...
class ServicePackage(Package):
    ...
    def pre_build(self, build_path):
        ...
        options = {'executable': binary_path, 'upstart_worker': self.final_service, 
           'package': self.name, 
           'name': self.binary, 
        ...
        upstart_worker = get_custom_file('upstart', self.final_service, self.binary_file['template'])
        upstart_worker.build(build_path, options, extra_template_dir=self.path)
        upstart_ctl = get_custom_file('upstart', self.controller_service, 'upstart/controller.conf')
        upstart_ctl.build(build_path, options, extra_template_dir=self.path)
...
class ConfigurationPackage(ApplicationPackage):
    ...
    def __init__(self, name, application_path, environment=None, version=None, maiden_name=None):
    ...
    def pre_build(self, build_path):
        file_bin = get_custom_file('product', self.name, 'config.ini', maiden_name=self.maiden_name, options=self.config)
            ...
            def exclude_and_include(path, names):
                ...
                if 'includes' in folder:
                    for include in folder['includes']:
                        names.difference_update(set(fnmatch.filter(list(names), include)))
