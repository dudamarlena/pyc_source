...
# [GCC 8.4.0]
# Embedded file name: /code/trebuchet/trebuchet/run.py
...
def main():
    ...
    if arguments['build']:
        ...
        if arguments.get('--dry-run'):
            ...
            sys.exit(0)
        build_app(project_config, prepare_config, package_config, version_options=versions)
    elif arguments['develop']:
        prepare_config = parse_prepare(arguments)
        versions = parse_version(arguments)
        develop_app(project_config, prepare_config, version_options=versions)
    elif arguments['lint']:
