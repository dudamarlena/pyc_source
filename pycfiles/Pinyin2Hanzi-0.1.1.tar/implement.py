...
# [GCC 8.4.0]
# Embedded file name: ../Pinyin2Hanzi/implement.py
# Compiled at: 2016-02-07 09:29:59
...
class DefaultHmmParams(AbstractHmmParams):
    ...
    def __init__(self):
        ...
        self.py2hz_dict = self.readjson(os.path.join(current_dir, b'data', b'hmm_py2hz.json'))
        self.start_dict = self.readjson(os.path.join(current_dir, b'data', b'hmm_start.json'))
        self.emission_dict = self.readjson(os.path.join(current_dir, b'data', b'hmm_emission.json'))
        self.transition_dict = self.readjson(os.path.join(current_dir, b'data', b'hmm_transition.json'))
...
class DefaultDagParams(AbstractDagParams):
    ...
    def __init__(self):
        current_dir = self.pwd()
        self.char_dict = self.readjson(os.path.join(current_dir, b'data', b'dag_char.json'))
        self.phrase_dict = self.readjson(os.path.join(current_dir, b'data', b'dag_phrase.json'))
    ...
    def get_phrase(self, pinyin_list, num=6):
        u""" pinyin_list是拼音组成的list，例如['yi', 'ge'] """
