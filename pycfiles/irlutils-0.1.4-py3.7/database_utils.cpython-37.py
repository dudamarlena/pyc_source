...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/url/crawl/database_utils.py
# Compiled at: 2019-11-16 18:33:01
# Size of source mod 2**32: 6489 bytes
import zlib, plyvel, jsbeautifier
...
def optimize_db(cursor):
    ...
    cursor.execute('PRAGMA cache_size = -%i' % 1000000.0)
...
def build_index(cursor, column, tables):
    ...
    if not type(tables) == list:
        tables = [
         tables]
    for table in tables:
        cursor.execute('CREATE INDEX IF NOT EXISTS %s_%s_index ON %s(%s);' % (
         table, column, table, column))


def get_ldb_content(ldb_addr, content_hash):
    db = plyvel.DB(ldb_addr)
...
def get_leveldb(db_path, compression='snappy'):
    ...
    """
    db = plyvel.DB(db_path, lru_cache_size=1000000000,
      write_buffer_size=1280000,
...
def get_url_content_with_hash(url, sqlite_cur, ldb_con, beautify=True, visit_id=None):
    ...
    if visit_id is not None:
        sqlite_cur.execute('SELECT content_hash FROM http_responses WHERE visit_id = ? AND url = ? LIMIT 1;', (
         visit_id, url))
    else:
        sqlite_cur.execute('SELECT content_hash FROM http_responses WHERE url = ? LIMIT 1;', (
         url,))
...
def get_channel_content(visit_id, channel_id, sqlite_cur, ldb_con, beautify=True):
    ...
    beautify : boolean
        Control weather or not to beautify output
    """
    return get_channel_content_with_hash(visit_id, channel_id, sqlite_cur, ldb_con, beautify)[1]


def get_channel_content_with_hash(visit_id, channel_id, sqlite_cur, ldb_con, beautify=True):
    """Return javascript content for given channel_id.
    Parameters
    ----------
    visit_id : int
    ...
    beautify : boolean
        Control weather or not to beautify output
    """
    sqlite_cur.execute('SELECT content_hash FROM http_responses WHERE channel_id = ? AND visit_id = ? LIMIT 1;', (
     channel_id, visit_id))
    content_hash = sqlite_cur.fetchone()
    if content_hash is None or len(content_hash) == 0 or content_hash[0] is None:
        return
...
def get_content(db, content_hash, compression='snappy', beautify=True):
    ...
    if content_hash is None:
        ...
        if compression not in supported:
            print('Unsupported compression type %s. Only %s are the supported options.' % (
             compression, str(supported)))
            return
...
def drop_tables(sqlite_con, tables=[]):
    for table in tables:
        sqlite_con.cursor().execute('drop table if exists {}'.format(table))
