...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/cliutils/process.py
# Compiled at: 2008-09-18 12:50:27
__all__ = [
 'Process', 'sh', 'AlreadyExecuted', 'InvalidCommand']
import os, shlex
...
class Process(object):
    ...
    def __or__(self, proc):
        ...
        if self.hasExecuted or proc.hasExecuted:
            raise AlreadyExecuted("You can't pipe processes after they've beenexecuted.")
    ...
    def _refreshProcess(self):
        ...
        try:
            self._process = Popen(self._command, stdin=self._stdin, stdout=self._stdout, stderr=self._stderr)
        except OSError, e:
