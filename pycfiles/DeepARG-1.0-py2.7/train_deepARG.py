...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/deeparg/argdb/train_deepARG.py
...
def make_xy_per_class(samples, X, Y, features):
    ...
    for label in list(set(Y)):
        ...
        s_sel = samples[index]
        train_size = 1
        ixtrain, ixtest, ytrain, ytest = train_test_split(range(x_sel.shape[0]), y_sel, random_state=0, train_size=train_size)
...
def main(data):
    ...
    clf = NeuralNet(layers=model(x_train.shape[1], len(set(Y))), update=nesterov_momentum, update_learning_rate=0.01, update_momentum=0.9, regression=False, max_epochs=100, verbose=2)
