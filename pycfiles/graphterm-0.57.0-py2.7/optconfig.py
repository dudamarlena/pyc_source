...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/graphterm/optconfig.py
...
class OptConfig(object):
    ...
    def __init__(self, usage=None, config_file=None, config_opt='config', select_opt='select', use_default_section=True, file_escape='@'):
        """
    ...
    def add_option(self, option, short=None, opt_type='str', default=None, help=None, _internal=False):
        ...
        if opt_type == 'flag':
            ...
            self.option_parser.add_option('-' + short if short else '', '--' + option, dest=option, default=None, action='store_false' if default else 'store_true', help=help or None)
        else:
            ...
            self.option_parser.add_option('-' + short if short else '', '--' + option, dest=option, type=opt_type if opt_type in ('int',
                                                                                                                                  'float') else 'str', default=None, help=help or None)
        return
    ...
    def getopt(self, option, default=None, section=None, config_only=False):
        ...
        if not config_only:
            value = self.cmd_optvalues.get(option) if 1 else None
            if value is None:
                if self.config_parser.has_option(section, option):
                    value = self.config_parser.get(section, option)
                else:
                    if self.use_default_section and section != ConfigParser.DEFAULTSECT and self.config_parser.has_option(ConfigParser.DEFAULTSECT, option):
                        value = self.config_parser.get(ConfigParser.DEFAULTSECT, option)
                    if value is None:
                        if option in self.flags:
                            return self.flags[option]
                        return default
            if opt_type == 'int':
            ...
            if opt_type == 'float' and (value is None or value == '') and default is not None:
                return default
            try:
                return float(value)
            except Exception as excp:
