...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/graphterm/bin/tornado/template.py
...
import cStringIO, datetime, linecache, logging, os.path, posixpath, re, threading
...
class Template(object):
    ...
    def __init__(self, template_string, name='<string>', loader=None, compress_whitespace=None, autoescape=_UNSET):
        ...
        if autoescape is not _UNSET:
            ...
            self.loader = loader
            try:
                self.compiled = compile(escape.to_unicode(self.code), '%s.generated.py' % self.name.replace('.', '_'), 'exec')
            except Exception:
    ...
    def generate(self, **kwargs):
        ...
        namespace = {'escape': escape.xhtml_escape, 'xhtml_escape': escape.xhtml_escape, 
    ...
    def _generate_python(self, loader, compress_whitespace):
        ...
        try:
            ...
            writer = _CodeWriter(buffer, named_blocks, loader, ancestors[0].template, compress_whitespace)
...
class _Expression(_Node):
    ...
    def generate(self, writer):
        ...
        writer.write_line('if isinstance(_tmp, _string_types): _tmp = _utf8(_tmp)', self.line)
        writer.write_line('else: _tmp = _utf8(str(_tmp))', self.line)
        if not self.raw and writer.current_template.autoescape is not None:
            writer.write_line('_tmp = _utf8(%s(_tmp))' % writer.current_template.autoescape, self.line)
...
class _Module(_Expression):
    ...
    def __init__(self, expression, line):
        super(_Module, self).__init__('_modules.' + expression, line, raw=True)
...
def _parse(reader, template, in_block=None):
    ...
    while True:
        ...
        if start_brace == '{{':
            ...
            continue
        if not start_brace == '{%':
            raise AssertionError(start_brace)
            end = reader.find('%}')
            if end == -1:
                raise ParseError('Missing end block %%} on line %d' % line)
            contents = reader.consume(end).strip()
            reader.consume(2)
            raise (contents or ParseError)('Empty block tag ({%% %%}) on line %d' % line)
        operator, space, suffix = contents.partition(' ')
        suffix = suffix.strip()
        intermediate_blocks = {'else': set(['if', 'for', 'while']), 
           'elif': set(['if']), 
           'except': set(['try']), 
           'finally': set(['try'])}
        ...
        if allowed_parents is not None:
            ...
            if operator == 'end':
                ...
                return body
            if operator in ('extends', 'include', 'set', 'import', 'from', 'comment',
