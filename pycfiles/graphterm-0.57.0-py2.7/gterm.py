...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/graphterm/bin/gterm.py
# Compiled at: 2014-07-18 14:04:16
"""
gterm: API module for GraphTerm-aware programs; also serves as terminal launcher
"""
from __future__ import absolute_import, print_function
import base64, getpass, hashlib, hmac, json, logging, mimetypes, os, random, re, stat, subprocess, sys, termios, threading, tty, uuid
...
Dimensions = env('DIMENSIONS', lc=True)
...
Html_escapes = [Esc_prefix + ';%sh' % (Cookie or 0),
 Esc_prefix + 'l']
INTERPRETERS = {'python': ('py', 'python', ('>>> ', '... ')), 'ipython': (
             'py', 'python', ('In ', '   ...: ', '   ....: ', '   .....: ', '   ......: ')), 
   'python3': (
             'py', 'python', ('>>> ', '... ')), 
   'ipython3': (
...
APP_IPYNB_FILENAME = 'gterm_ipynb.txt'
...
def auth_token(secret, connection_id, host, port, client_nonce, server_nonce):
    ...
    return [ hmac.new(to_bytes(secret), to_bytes(prefix + conn_type), digestmod=hashlib.sha256).hexdigest()[:24] for conn_type in ('client',
                                                                                                                                   'server') ]
...
def create_app_directory(appdir=App_dir):
    ...
    if os.path.isdir(appdir) and os.stat(appdir).st_mode != 448:
        os.chmod(appdir, 448)
...
GTERM_DIRECTIVE_RE = re.compile('^\\s*<!--gterm\\s+(\\w+)(\\s.*)?-->')
...
def wrap_encoded_file_or_data(filepath, content=None, headers={}, stderr=False):
    ...
    if content is None and (not Export_host or not filepath):
        ...
        headers.update({'x_gterm_encoding': 'base64', 'content_length': len(content)})
...
def write_pagelet_old(html, display='block', dir='', add_headers={}, stderr=False):
    ...
    params = {'display': display, 'scroll': 'top', 
       'current_directory': dir}
    params.update(add_headers)
    html_headers = {'content_type': 'text/html', 'x_gterm_response': 'pagelet', 
...
def write_form(html, command='', dir='', stderr=False):
    ...
    html_headers = {'content_type': 'text/html', 'x_gterm_response': 'pagelet', 
       'x_gterm_parameters': {'display': 'fullpage', 'scroll': 'top', 'current_directory': dir, 'form_input': True, 
...
def iframe_html(src_url='', html='', add_classes='', width=None, height=None, header=False, fullscreen=False, host='', untrusted=False):
    ...
    frameId = 'gframe%09d' % random.randrange(0, 1000000000)
...
def iframe_header_html(out_html, fullscreen=False):
    if fullscreen:
        ...
        container_id = 'gframecontainer%09d' % random.randrange(0, 1000000000)
...
def open_url(url, target='_blank', stderr=False):
    ...
    url_headers = {'x_gterm_response': 'open_url', 'x_gterm_parameters': {'url': url, 'target': target}}
    wrap_write('', headers=url_headers, stderr=stderr)


def auto_print(text):
    """Auto print line output from interpreter shell"""
    headers = {'x_gterm_response': 'auto_print', 'x_gterm_parameters': {}}
...
def edit_file(filename='', dir='', content=None, create=False, editor='ace', stderr=False):
    ...
    headers = {'x_gterm_response': 'edit_file', 'x_gterm_parameters': params}
...
def open_notebook(filename='', dir='', content=None, command_path='', prompts=[], params={}, stderr=False):
    ...
    headers = {'x_gterm_response': 'open_notebook', 'x_gterm_parameters': {'filepath': filepath, 'prompts': prompts, 'nb_params': params, 'current_directory': dir}}
    wrap_encoded_file_or_data(filepath, content=content, headers=headers, stderr=stderr)
    return
...
def save_notebook(filename='', dir='', stderr=False):
    ...
    if Export_host:
        params['location'] = 'remote'
    headers = {'x_gterm_response': 'save_notebook', 'x_gterm_parameters': params}
    wrap_write('', headers=headers, stderr=stderr)
    if Export_host:
        errmsg, headers, content = receive_data(stderr=stderr)
        if errmsg:
...
def menu_op(target, value=None, stderr=False):
    ...
    headers = {'x_gterm_response': 'menu_op', 'x_gterm_parameters': {'target': target, 'value': value}}
    wrap_write('', headers=headers, stderr=stderr)
    ...
    if exists and not os.path.exists(filepath):
        ...
        if plain and not os.path.isfile(filepath):
            ...
            if relative:
                ...
                if untrusted:
                    return get_untrusted_url(server_url=server_url) + rel_path
                return rel_path
...
def create_blob_id(blob_id='', untrusted=False):
    if blob_id:
        ...
        elif not blob_id.startswith(TRUSTED_PREFIX):
...
def create_blob(content=None, from_file='', content_type='', blob_id='', host='', untrusted=False, stderr=False):
    ...
    headers = {'x_gterm_response': 'create_blob', 'x_gterm_parameters': params, 
...
def preload_images(urls, stderr=False):
    params = {'urls': urls}
    headers = {'x_gterm_response': 'preload_images', 'x_gterm_parameters': params}
    wrap_write('', headers=headers, stderr=stderr)
...
def setup_logging(log_level=logging.ERROR, filename='', file_level=None):
    ...
    formatter = logging.Formatter('%(levelname).1s%(asctime)s %(module).8s.%(lineno).04d %(message)s', '%y%m%d/%H:%M')
    if logger.handlers:
...
class FormParser(object):
    ...
    def add_option(self, name, default_value='', short='', label='', help='', raw=False):
        ...
        if self.parser:
            ...
            if isinstance(default, bool):
                self.parser.add_option(short, '--' + name, dest=name, default=default, help=help, action='store_false' if default else 'store_true')
            else:
                self.parser.add_option(short, '--' + name, dest=name, default=default, help=help)
    ...
    def create_form(self, id_suffix=None, prefill=None, errmsg=''):
        ...
        """
        if not id_suffix:
            id_suffix = '1%09d' % random.randrange(0, 1000000000)
        prefill_opts = {}
        if prefill:
...
def auth_request(http_addr, http_port, nonce, timeout=None, client_auth=False, user='', protocol='http'):
    ...
    request = tornado.httpclient.HTTPRequest(url, validate_cert=True, ca_certs=ca_certs, **ssl_options)
...
def main():
    ...
    parser = OptionParser(usage=usage)
    parser.add_option('', '--https', dest='https', action='store_true', help='Use SSL (TLS) connections for security')
    parser.add_option('-n', '--noauth', dest='noauth', action='store_true', help='No authentication required')
    parser.add_option('-r', '--read_code', dest='read_code', action='store_true', help='Read access code from terminal')
    parser.add_option('-b', '--browser', dest='browser', default='', help='Browser application name (OS X only)')
    parser.add_option('-p', '--port', dest='port', default=0, help='Remote server port', type='int')
    parser.add_option('-s', '--server', dest='server', default='', help='Remote server name')
    parser.add_option('-u', '--user', dest='user', default='', help='User name')
    parser.add_option('', '--client_cert', dest='client_cert', default='', help="Path to client CA cert (or '.')")
    ...
    client_nonce = '1%018d' % random.randrange(0, 1000000000000000000)
