...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/graphterm/bin/tornado/options.py
...
for define() below.
"""
import datetime, logging, logging.handlers, re, sys, time
...
def define(name, default=None, type=None, help=None, metavar=None, multiple=False, group=None):
    ...
    options[name] = _Option(name, file_name=file_name, default=default, type=type, help=help, metavar=metavar, multiple=multiple, group_name=group_name)
    return
...
class _Option(object):
    ...
    def __init__(self, name, default=None, type=str, help=None, metavar=None, multiple=False, file_name=None, group_name=None):
    ...
    def parse(self, value):
        _parse = {datetime.datetime: self._parse_datetime, 
           ...
           str: self._parse_string}.get(self.type, self.type)
        if self.multiple:
            if self._value is None:
                self._value = []
    ...
    def set(self, value):
        ...
        elif value != None and not isinstance(value, self.type):
...
def enable_pretty_logging():
    ...
    if options.log_to_stderr or options.log_to_stderr is None and not root_logger.handlers:
        color = False
...
define('log_file_max_size', type=int, default=100000000, help='max size of log files before rollover')
