...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/graphterm/gtermhost.py
# Compiled at: 2014-07-17 13:14:11
"""gtermhost: GraphTerm host connector
"""
import base64, cgi, calendar, collections, datetime, email.utils, functools, hashlib, logging, mimetypes, otrace, os, re, signal, stat, sys, threading, time, urllib, random
try:
...
import about, lineterm, packetserver
RETRY_SEC = 15
...
HTML_ESCAPES = [
 '\x1b[?1155;', 'h',
 '\x1b[?1155l']
USER_RE = re.compile('^[a-z][a-z0-9]*$')
HOST_RE = re.compile('^[a-z][a-z0-9\\-\\*\\?\\[\\]]*$')
SESSION_RE = re.compile('^[a-zA-Z\\*\\?\\[\\]][\\w\\*\\?\\[\\]]*$')
Host_secret = None
IO_loop = None
...
class TerminalClient(packetserver.RPCLink, packetserver.PacketClient):
    ...
    def __init__(self, host, port, host_secret='', io_loop=None, ssl_options={}, key_secret=None, key_version=None, key_id=None, lterm_logfile=''):
        super(TerminalClient, self).__init__(host, port, io_loop=io_loop, ssl_options=ssl_options, max_packet_buf=3, reconnect_sec=RETRY_SEC, server_type='frame', key_secret=key_secret, key_version=key_version, key_id=key_id)
    ...
    def connection_validated(self):
        ...
        self.remote_response('', '', [
         ['term_params',
          {'version': about.version, 'min_version': about.min_version, 
    ...
    def xterm(self, term_name='', height=25, width=80, winheight=0, winwidth=0, parent='', command=''):
        if not self.lineterm:
            ...
            self.lineterm = lineterm.Multiplex(self.screen_callback, command=command or self.host_settings['command'], shared_secret=self.host_secret, host=self.connection_id, server_url=self.host_settings['server_url'], term_type=self.host_settings['term_type'], api_version=version_str, widget_port=self.widget_port, prompt_list=self.host_settings['prompt_list'], blob_server=self.blob_server, term_params=self.host_settings['lterm_params'], logfile=self.lterm_logfile)
        term_name, lterm_cookie, alert_msg = self.lineterm.terminal(term_name, height=height, width=width, winheight=winheight, winwidth=winwidth, parent=parent)
        ...
        try:
            ...
            for j, cmd in enumerate(req_list):
                ...
                if action == 'shutdown':
                    ...
                    resp_list.append(['completed_input', options])
                elif action == 'input':
                    cmd_input = cmd[0].encode('ascii', 'ignore').lstrip()
                    ...
                    resp_list.append(['file_response', request_id,
                     dict(status=status, last_modified=last_modified, etag=etag, content_type=content_type, content_length=content_length, content_b64=None if content_b64 else '')])
                    if content_b64:
...
class GTCallbackMixin(object):
    ...
    def editback(self, content, filepath='', filetype='', editor='', modify=False):
        if editor and editor not in AJAX_EDITORS:
            return otrace.TraceCallback.editback(self, content, filepath=filepath, filetype=filetype, editor=editor, modify=modify)
        else:
            params = {'editor': editor, 'modify': modify, 'command': 'edit -f ' + filepath if modify else '', 'filepath': filepath, 
               'filetype': filetype}
...
class WidgetStream(object):
    ...
    def receive_packet(self, data):
        ...
        if resp_type == 'capture_chat':
            ...
            if chat_cookie == self.packet_cookie:
            ...
            args = [
             params, base64.b64encode(content) if content else '']
...
def gterm_connect(host_name, server_addr, server_port=gterm.DEFAULT_HOST_PORT, connect_kw={}, oshell_globals=None, oshell_thread=False, oshell_unsafe=False, oshell_workdir='', oshell_init='', oshell_db_interface=None, oshell_web_interface=None, oshell_hold_wrapper=None, oshell_no_input=True, gterm_callback=None, io_loop=None):
    ...
    if io_loop:
        ...
        io_loop = IO_loop
    host_secret = '%016x' % random.randrange(0, 18446744073709551616)
    host_connection = TerminalClient.get_client(host_name, connect=(
     server_addr, server_port, host_secret), connect_kw=connect_kw)
    ...
    if not oshell_globals:
        ...
        trace_shell = otrace.OShell(locals_dict=oshell_globals, globals_dict=oshell_globals, new_thread=oshell_thread, allow_unsafe=oshell_unsafe, work_dir=oshell_workdir, add_env={gterm.GT_PREFIX + 'COOKIE': host_connection.osh_cookie, gterm.GT_PREFIX + 'SHARED_SECRET': host_secret}, init_file=oshell_init, db_interface=oshell_db_interface, web_interface=oshell_web_interface, hold_wrapper=oshell_hold_wrapper, no_input=oshell_no_input, eventloop_callback=io_loop.add_callback)
        host_connection.add_oshell()
        return (
         host_connection, host_secret, trace_shell)
...
def run_host(options, args):
    ...
    Gterm_host, Host_secret, Trace_shell = gterm_connect(host_name, options.server_addr, server_port=server_port, connect_kw={'ssl_options': internal_client_ssl, 'key_secret': auth_code or None, 
       'key_version': key_version, 
       'key_id': str(remote_port), 
       'lterm_logfile': options.lterm_logfile}, oshell_globals=oshell_globals, oshell_unsafe=True, oshell_no_input=not options.oshell_input)
...
def main():
    ...
    parser.add_option('', '--server_addr', dest='server_addr', default='localhost', help='Server hostname (or IP address) (default: localhost)')
    parser.add_option('', '--server_port', dest='server_port', default=0, help='Server port (default: %d)' % gterm.DEFAULT_HOST_PORT, type='int')
    parser.add_option('', '--external_host', dest='external_host', default='', help='external host name (or IP address), if different from server_addr')
    parser.add_option('', '--remote_port', dest='remote_port', default=0, help='actual remote port, if different from server_port', type='int')
    parser.add_option('', '--auth_file', dest='auth_file', default='', help='Server auth file')
    ...
    parser.add_option('', '--internal_certfile', dest='internal_certfile', default='', help='Internal SSL certfile')
    parser.add_option('', '--logging', dest='logging', action='store_true', help='Log to ~/.graphterm/gtermhost.log')
    parser.add_option('', '--lterm_logfile', dest='lterm_logfile', default='', help='Lineterm logfile')
    parser.add_option('', '--daemon', dest='daemon', default='', help='daemon=start/stop/restart/status')
