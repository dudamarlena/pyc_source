...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/graphterm/packetserver.py
# Compiled at: 2014-07-17 00:04:15
"""
Packet server: for flash etc.
"""
import hashlib, time, errno, functools, hmac, hashlib, logging, socket, ssl, struct, thread, traceback, uuid
try:
...
class PacketConnector(object):
    """ Serves Flash policy and other requests, delimited by "\x00" or framed
    ...
    """
    _all_connections = None
    CONTENT_DELIM = '\r\n\r\n'

    def __init__(self, client=False, server_type='', delimiter=None, framelen_format=None, single_request=False, ssl_options={}, max_packet_buf=0):
    ...
    def send_raw_packet(self, data, finish=False, buffer=False, nobuffer=False):
        ...
        if self.closed:
            ...
            if not nobuffer:
                ...
                self.packet_id = (self.packet_id + 1) % 1073741824
...
class PacketClient(PacketConnector):
    ...
    def __init__(self, host, port, io_loop=None, noresponse=False, server_type='', delimiter=None, framelen_format=None, single_request=False, ssl_options={}, max_packet_buf=0, reconnect_sec=0, reconnect_timeout=0):
        super(PacketClient, self).__init__(client=True, server_type=server_type, delimiter=delimiter, framelen_format=framelen_format, ssl_options=ssl_options, max_packet_buf=max_packet_buf, single_request=single_request)
        self.host = host
        self.port = port
        if not io_loop:
    ...
    def on_close(self):
        ...
        self.handle_close()
        cur_time = time.time()
        if self.reconnect_sec <= 0 or self.reconnect_timeout and self.reconnect_time and cur_time - self.reconnect_time > self.reconnect_timeout or self._all_connections.get(self.connection_id) is not self:
            self.shutdown()
        else:
            self.shutdown_aux()
            if not self.reconnect_cb:
...
class PacketConnection(PacketConnector):
    ...
    def __init__(self, stream, address, server_address, server_type='', delimiter=None, framelen_format=None, ssl_options={}, max_packet_buf=0, single_request=False):
        super(PacketConnection, self).__init__(client=False, server_type=server_type, delimiter=delimiter, framelen_format=framelen_format, ssl_options=ssl_options, max_packet_buf=max_packet_buf, single_request=single_request)
...
class RPCLink(object):
    ...
    def on_connect(self):
        ...
        self.rpc_expect = 'connect'
        self.send_json([0, ['connect', [self.connection_id, self.rpc_key_version, self.rpc_nonce, None]]], nobuffer=True)
        return
    ...
    def rpc_connect(self, connection_id, key_version, nonce, token):
        ...
        if self.client:
            ...
            if key_version != self.rpc_key_version:
                self.send_json([0, ['shutdown', ['Invalid key version: %s, c=%s' % (key_version, connection_id)]]], nobuffer=True)
                self.shutdown()
            ...
            self.rpc_client_token = client_token
            self.send_json([0, ['connect', [None, None, self.rpc_nonce, server_token]]], nobuffer=True)
        return
    ...
    def sign_token(self, connection_id, client_nonce, server_nonce):
        ...
        return [ hmac.new(str(key_secret), prefix + conn_type, digestmod=SIGN_HASH).hexdigest()[:SIGN_HEXDIGITS] for conn_type in ('client',
                                                                                                                                   'server') ]
...
class AsyncTimeoutRequestHandler(web.RequestHandler):
    ...
    def setup_timeout(self):
        ...
        self.timeout_callback = ioloop.IOLoop.instance().add_timeout(time.time() + self.REQUEST_TIMEOUT, functools.partial(self.__class__.timeout_async_request, self.async_id))
...
class PolicyServer(PacketConnection):
    ...
    def __init__(self, stream, address, server_address, allow_domain='*', allow_ports='843'):
        super(PolicyServer, self).__init__(stream, address, server_address, single_request=True, server_type='flash')
...
if __name__ == '__main__':
    ...
    class TestFlashSocket(FlashSocket):
        ...
        def get_access_info(self, key_version):
            return (
             False, self.group_zid + ',' + key_version)
        ...
        def handle_message_obj(self, message_obj):
            data = [
             'CHAT', 'testuser', 'black', 'Test message!']
            self.send_json(data)
