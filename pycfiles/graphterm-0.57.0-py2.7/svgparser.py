...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/graphterm/bin/svgwrite/data/svgparser.py
...
sign = oneOf('+ -')
comma = Literal(',') * (0, 1)
semicolon = Literal(';') * (0, 1)
integer_constant = Word(nums)
exponent = CaselessLiteral('E') + Optional(sign) + integer_constant
fractional_constant = Combine(Optional(integer_constant) + '.' + integer_constant) ^ Combine(integer_constant + '.')
scientific_constant = Combine(fractional_constant + Optional(exponent)) ^ Combine(integer_constant + exponent)
number = Combine(Optional(sign) + integer_constant) ^ Combine(Optional(sign) + scientific_constant)
...
def _build_pathdata_parser():
    ...
    smooth_curveto = oneOf('S s') + smooth_curveto_argument_sequence + ZeroOrMore(comma + smooth_curveto_argument_sequence)
    quadratic_bezier_curveto_argument_sequence = coordinate_pair + comma + coordinate_pair
    quadratic_bezier_curveto = oneOf('Q q') + quadratic_bezier_curveto_argument_sequence + ZeroOrMore(comma + quadratic_bezier_curveto_argument_sequence)
    ...
    elliptical_arc = oneOf('A a') + elliptical_arc_argument + ZeroOrMore(comma + elliptical_arc_argument)
...
def _build_animation_timing_parser():
    ...
    begin_value = offset_value | syncbase_value | event_value | repeat_value | accesskey_value | wallclock_sync_value | Literal('indefinite')
