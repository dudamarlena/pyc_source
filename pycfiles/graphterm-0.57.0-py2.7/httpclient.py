...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/graphterm/bin/tornado/httpclient.py
...
"""
import calendar, email.utils, httplib, time, weakref
...
class AsyncHTTPClient(object):
    ...
    def _async_clients(cls):
        if not cls is not AsyncHTTPClient:
            raise AssertionError('should only be called on subclasses')
            cls._async_client_dict = hasattr(cls, '_async_client_dict') or weakref.WeakKeyDictionary()
        return cls._async_client_dict

    def __new__(cls, io_loop=None, max_clients=10, force_instance=False, **kwargs):
...
class HTTPResponse(object):
    ...
    * time_info: dictionary of diagnostic timing information from the request.
    ...
    def __init__(self, request, code, headers={}, buffer=None, effective_url=None, error=None, request_time=None, time_info={}):
