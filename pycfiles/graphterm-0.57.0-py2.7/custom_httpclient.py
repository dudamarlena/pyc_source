...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/graphterm/bin/tornado/custom_httpclient.py
# Compiled at: 2012-10-23 23:27:25
...
class SingleAsyncHTTPClient(object):
    ...
    def __init__(self, io_loop=None, max_clients=10, source_addr='', max_simultaneous_connections=None, hostname_mapping=None, max_buffer_size=104857600):
    ...
    def fetch(self, request, callback, **kwargs):
        ...
        self.http_connection = _HTTPConnection(self.io_loop, self, request, self._release_fetch, callback, self.max_buffer_size, self.source_addr)

    def _release_fetch(self):
        pass

    def shutdown(self):
        if self.http_connection:
            self.http_connection.shutdown()


class _HTTPConnection(object):
    _SUPPORTED_METHODS = set(['GET', 'HEAD', 'POST', 'PUT', 'DELETE', 'PATCH', 'OPTIONS'])

    def __init__(self, io_loop, client, request, release_callback, final_callback, max_buffer_size, source_addr):
        ...
        with stack_context.StackContext(self.cleanup):
            ...
            if parsed.scheme == 'https':
                ...
                self.stream = SSLIOStream(sock, io_loop=self.io_loop, ssl_options=ssl_options, max_buffer_size=max_buffer_size)
            else:
                sock = socket.socket(af, socktype, proto)
                if source_addr:
                    sock.bind((source_addr, 0))
                self.stream = IOStream(sock, io_loop=self.io_loop, max_buffer_size=max_buffer_size)
    ...
    def shutdown(self):
        self.stream.close()
        self._run_callback(HTTPResponse(self.request, 200))
    ...
    def _on_connect(self, parsed, parsed_hostname):
        ...
        if self.request.validate_cert and isinstance(self.stream, SSLIOStream):
            match_hostname(self.stream.socket.getpeercert(), parsed_hostname)
...
if __name__ == '__main__':
    AsyncHTTPClient.configure(SingleAsyncHTTPClient)
    main()