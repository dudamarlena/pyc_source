...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/graphterm/lineterm.py
# Compiled at: 2014-07-19 00:56:29
...
import array, cgi, copy, fcntl, glob, logging, mimetypes, optparse, os, pty, re, signal, select, socket, sys, threading, time, termios, tty, struct, pwd
try:
...
import base64, errno, glob, hashlib, hmac, pipes, platform, Queue, shlex, subprocess, traceback, urllib, uuid
from bin import gterm
GT_PREFIX = gterm.GT_PREFIX
ESCAPE_BUF_LEN = 256
MAX_SCROLL_LINES = 1000
CHUNK_BYTES = 4096
MAX_PAGELET_BYTES = 5000000
IDLE_TIMEOUT = 300
...
COMMAND_DELIMITERS = '<>;'
MD_BLOB_RE = re.compile('^\\s*!\\[([^\\]]*)\\]\\s*\\((' + gterm.BLOB_PREFIX + '[^\\)]+)\\)')
MD_IMAGELINK_RE = re.compile('^\\s*!\\[([^\\]]*)\\]\\s*\\(([^\\)\\s]+)([^\\)]*)\\)')
MD_IMAGEREF_RE = re.compile('^\\s*!\\[([^\\]]*)\\]\\s*\\[([^\\]]+)\\]')
MD_REF_RE = re.compile('^\\s*\\[([^\\]]+)\\]:\\s*data:')
MD_FENCE_RE = re.compile('^\\s*{(\\S+)(\\s.*)?}')
...
Exec_errmsg = False
ENCODING = 'utf-8'
BASH_PROMPT_CMD = 'export PS1=$GTERM_PROMPT; echo -n "\x1b[?%s;${GTERM_COOKIE}h$PWD\x1b[?%s;l"'
...
def make_lterm_cookie():
    return '%016d' % random.randrange(1000000000000000, 10000000000000000)
...
def dump(data, trim=False, encoded=False):
    ...
    return uclean(('').join(map(unichr, ucodes)), trim=trim, encoded=encoded)


def uclean(ustr, trim=False, encoded=False):
    """Return cleaned up unicode string, trimmed and encoded to str, if need be"""
    if trim:
        ustr = ustr.rstrip('\x00')
    ustr = ustr.replace('\x00', ' ').replace('\x7f', '?')
    if encoded:
        return ustr.encode(ENCODING, 'replace')
    return ustr
...
def parse_headers(text):
    ...
    if ANGLE_BRACKET_RE.match(text):
        ...
        if offset:
            ...
            if directive == 'data':
                ...
                if not encoding == 'base64':
                    raise AssertionError('Invalid data URI encoding: ' + encoding)
        ...
        if sep and head_str.startswith('{'):
            try:
...
def split_file_url(url, check_host_secret=None):
    ...
    if check_host_secret:
        ...
        if query.lower() == filehmac.lower():
            hostname = ''
    return [
     server_port, hostname, filename, filepath, query]
...
class ScreenBuf(object):
    ...
    def __init__(self, pdelim, fg_color=0, bg_color=7, colors=False):
        ...
        self.colors = colors
        self.default_style = (bg_color ^ 7) << STYLE4 | fg_color
        self.inverse_style = (fg_color ^ 7) << STYLE4 | bg_color
        self.bold_style = 8
    ...
    def blank_last_entry(self):
        ...
        self.scroll_lines[(-1)][JPARAMS] = ['pagelet',
         {'add_class': '', 'pagelet_id': '%d-%d' % (self.cur_note, self.current_scroll_count)}]
    ...
    def update(self, active_rows, width, height, cursorx, cursory, main_screen, alt_screen=None, pdelim=[], reconnecting=False):
    ...
    def dumprichtext(self, data, trim=False):
        ...
        for scode in data:
            ...
            if span_style != char_style:
                ...
                if span_style & 119 == self.inverse_style:
                    style_list.append('inverse')
                if self.colors:
                    fg_color = span_style & 7
                    bg_color = span_style >> STYLE4 & 7 ^ 7
...
class Terminal(object):
    ...
    def __init__(self, term_name, fd, pid, screen_callback, height=25, width=80, winheight=0, winwidth=0, cookie=0, shared_secret='', host='', server_url='', pdelim=[], term_params={}, logfile=''):
    ...
    def init(self):
        ...
        d = {'\\[\\??([0-9;]*)([@ABCDEFGHJKLMPXacdefghlmnqrstu`])': self.csi_dispatch, '\\]([^\\x07]+)\\x07': self.esc_ignore}
        self.esc_re = []
        for k, v in d.items():
    ...
    def open_notebook(self, filepath, prompts=[], params={}, content=None):
        ...
        self.note_start = None
        self.note_count += 1
        note_shell = bool(self.active_rows and self.main_screen.meta[(self.active_rows - 1)])
        ...
        self.note_params = {'name': note_name, 'file': filepath, 'dir': note_dir, 'command': note_command, 'lang': note_lang, 
           'shell': note_shell, 'form': note_form, 'lock_offset': lock_offset, 
           'mod_offset': self.note_mod_offset, 'submit': submit_opt, 
           'master': params.get('master', ''), 'autosave': writable}
                ...
                for cell_index in self.note_cells['cellIndices']:
                    ...
                    if cell['cellType'] in MARKUP_TYPES:
                        if cell_lines:
                            row_params = [
                             'markdown', {}]
                            self.screen_buf.scroll_buf_up('', '', row_params=row_params, add_class='gterm-cell-input', markup=('\n').join(cell_lines))
                    else:
                        if cell_lines:
            ...
            if self.note_params['form']:
                ...
                if fext in ('.ipynb', '.json'):
                    ...
                    if self.note_params['name'] != fname and not lang_format:
                        ...
                        self.note_params['autosave'] = writable
                    ...
                    def match_image_link(line, prefix):
                        ...
                        if link_url in Bundled_image_ref:
                            return Bundled_image_ref[link_url]
                        else:
                            data_uri = None
                            ...
                            if link_url.startswith('http:') or link_url.startswith('https://'):
                                ...
                                if not link_url.startswith('/') and '..' not in link_url and '?' not in link_url and '#' not in link_url:
                                ...
                                if data_uri:
                                    ...
                                    return fig_line
                            Bundled_image_ref[link_url] = None
                            return
                            return
                ...
                if cell['cell_type'] == 'markdown':
                    ...
                    for output in cell['outputs']:
                        if output['output_type'] in ('pyout', 'stream'):
                            ...
                            self.note_screen_buf.scroll_buf_up('', None, markup=markup, row_params=[
                             'pagelet', {'blob': blob_id}])
    ...
    def read_md(self, content):
        ...
        try:
            ...
            for line in split_lines(content, chomp=True):
                ...
                elif leaving_block is None and raw_lines and raw_lines[(-1)]:
                    raw_lines.append(line)
                else:
                    skipped_blanks += 1
    ...
    def add_cell(self, new_cell_type='', init_text='', before_cell_number=0, filename=''):
        ...
        if self.note_params['form'] and self.note_initialized:
            ...
            new_cell = {'cellIndex': cell_index, 'cellType': new_cell_type, 'cellFile': filename, 'cellInput': [], 'cellFillInput': [], 'cellOutput': [], 'cellExpectOutput': [], 'cellTypeExtra': new_cell_extra, 
    ...
    def next_index(self, move_up=False, switch=False):
        ...
        if not move_up and cur_location < ncells - 1:
            return self.note_cells['cellIndices'][(cur_location + 1)]
        return 0
    ...
    def select_page(self, move=0, endpoint=False, slide=False):
        ...
        if not self.note_cells:
            ...
            if slide:
                self.note_slide = [
                 select_cell_index, last_cell_index]
            else:
                self.note_slide = None
        ...
        if not self.note_cells:
            ...
            if not cell_index == cur_index:
                ...
                if self.note_params['form'] and (cur_cell['cellParams']['filled'] or cur_cell['cellType'] in MARKUP_TYPES):
                    ...
                    cell_lines = split_lines(uclean(input_data.replace('\x00', ''), encoded=True))
                    self.note_update_time = time.time()
                if save:
                    if self.note_params['form'] == 'shared' and not form_advance:
                        return
                    self.update_mod_offset()
                    cur_cell['cellInput'] = self.note_params['form'] or cell_lines
                    ...
                    if cur_loc:
                        ...
                        prev_cell = (self.is_page_break(prev_index) or self.note_cells['cells'])[prev_index] if 1 else None
                ...
                if self.note_params['form'] and not cur_cell['cellParams']['filled'] and cur_cell['cellType'] not in MARKUP_TYPES:
            ...
            if 'no_pyindent' not in self.term_opts and self.note_params['lang'] == 'python':
                ...
                for line in input_lines:
                    ...
                    if not new_indent and indent:
                        ...
                        if comps and comps[0] not in ('else:', 'elif', 'except:', 'except',
                                                      'finally:'):
                            append_blank = True
                    if append_blank:
                        tem_lines.append('')
                ...
                input_lines = [
                 '_gterm_cell_start_hook() if "_gterm_cell_start_hook" in globals() else None'] + tem_lines
    ...
    def update_callback(self, response_id=''):
        ...
        if not self.note_cells or self.screen_buf.full_update or reconnecting:
            ...
            full_update, update_rows, update_scroll = self.screen_buf.update(active_rows, self.width, self.height, cursor_x, cursor_y, self.main_screen, alt_screen=alt_screen, pdelim=self.pdelim, reconnecting=reconnecting)
            ...
            self.screen_callback(self.term_name, response_id, 'row_update', [
             dict(alt_mode=self.alt_mode, reset=full_update, command=command, active_rows=self.active_rows, pre_offset=pre_offset),
        ...
        if self.note_cells:
                ...
                for cell_index in self.note_cells['cellIndices']:
                    ...
                    if cell['cellIndex'] != self.note_cells['curIndex']:
                        self.screen_callback(self.term_name, response_id, 'note_row_update', [
                         dict(alt_mode=False, reset=True, command=self.note_params['command'], active_rows=self.active_rows, pre_offset=0),
                         self.width, self.height,
                         0, 0, [], cell['cellOutput']])
            ...
            full_update, update_rows, update_scroll = self.note_screen_buf.update(self.active_rows, self.width, self.height, self.cursor_x, self.cursor_y, self.main_screen, alt_screen=False, pdelim=[], reconnecting=reconnecting)
            update_scroll = strip_prompt_lines(update_scroll, self.note_prompts)
            self.screen_callback(self.term_name, response_id, 'note_row_update', [
             dict(alt_mode=False, reset=full_update, active_rows=self.active_rows, pre_offset=0, note_prompt=self.note_found_prompt),
             self.width, self.height,
             self.cursor_x, self.cursor_y,
             update_rows, update_scroll])
        ...
        if self.cursor_y >= self.scroll_top and self.cursor_y <= self.scroll_bot:
            ...
            if q:
                ...
                if self.note_cells:
                    if meta and meta[JCONTINUATION]:
                        ...
                        self.note_screen_buf.scroll_buf_up(dump(row, trim=True, encoded=True), meta, offset=prompt_offset(dump(row), self.pdelim, self.screen.meta[self.scroll_top]))
                elif not self.alt_mode:
                    row = self.peek(self.scroll_top, 0, self.scroll_top, self.width)
                    self.screen_buf.scroll_buf_up(dump(row, trim=True, encoded=True), meta, offset=prompt_offset(dump(row), self.pdelim, self.screen.meta[self.scroll_top]))
            ...
            if not self.alt_mode:
                ...
                if self.current_meta and not self.screen.meta[(self.active_rows - 1)]:
                    self.current_meta = (
                     self.current_meta[JCURDIR], self.current_meta[JCONTINUATION] + 1)
    ...
    def echo(self, char):
        char_code = ord(char)
        if ENCODING == 'utf-8' and char_code & 128:
            if char_code & 64:
                self.echobuf = char
                if not char_code & 32:
            ...
            self.cursor_right()
            self.active_rows = self.alt_mode or max(self.cursor_y + 1, self.active_rows)
    ...
    def csi_m(self, l):
        ...
        for i in l:
            if i == 0 or i == 27:
                ...
                self.current_nul = self.current_nul & 4177526783 | c << UNI24
            elif i >= 40 and i <= 47 or i == 49:
                c = (i - 40 if i <= 47 else self.screen_buf.bg_color) ^ 7
                self.current_nul = self.current_nul & 2415919103 | c << UNI24 + STYLE4
            ...
            if self.gterm_buf_size > MAX_PAGELET_BYTES:
                ...
                if not self.gterm_validated and 'no_untrusted' not in self.term_opts:
                    if response_type == 'display_data' and headers.get('content_type', '').startswith('image/'):
                        ...
                        new_params = {'display': response_params.get('display', ''), 'overwrite': response_params.get('overwrite', False)}
                        ...
                        self.blob_data(headers, content)
                ...
                if handled_untrusted_content:
                    ...
                    if not response_type or response_type == 'pagelet':
                        ...
                        if not self.note_start and not self.note_cells:
                            ...
                            if prompts:
                                self.note_start = (
                                 prompts[0], filepath, prompts, nb_params, content)
                            else:
                                self.note_start = (
                                 '>>> ', filepath, [], nb_params, content)
    ...
    def blob_data(self, headers, content=None, untrusted=False):
        ...
        blob_id = response_params.get('blob', '')
        if not (blob_id or content is not None):
            raise AssertionError('No blob content for ' + response_type)
        ...
        if blob_id and response_type in ('display_data', 'display_blob'):
            ...
            if untrusted and blob_id.startswith(gterm.TRUSTED_PREFIX):
                ...
                if response_type == 'display_blob' or content_type.startswith('image/'):
                    ...
                    params = {'display': response_params.get('display', ''), 'blob': urllib.quote(blob_id), 'overwrite': response_params.get('overwrite', False)}
                    for key in ('autoerase', 'exit_page'):
                    ...
                    params = {'display': 'fullwindow', 'blob': urllib.quote(blob_id), 'autoerase': True, 
                       'overwrite': True, 'untrusted': True}
    ...
    def graphterm_output(self, params={}, content='', response_id='', from_buffer=False):
        if not from_buffer:
            self.gterm_output_buf = [
             params, base64encode(content) if content else '']
    ...
    def get_finder(self, kind, directory=''):
        ...
        row_content = test_finder_row % {'fullpath': directory, 'filetype': 'directory', 
        ...
        headers = {'content_type': 'text/html', 'x_gterm_response': 'display_finder', 
    ...
    def pty_write(self, data):
        if not self.alt_mode and self.current_meta and ('\r' in data or '\n' in data):
...
class Multiplex(object):
    ...
    def __init__(self, screen_callback, command=None, shared_secret='', host='', server_url='', term_type='linux', api_version='', widget_port=0, prompt_list=[], blob_server='', term_params={}, logfile='', app_name='graphterm'):
    ...
    def terminal(self, term_name=None, height=25, width=80, winheight=0, winwidth=0, parent='', command=''):
        ...
        with self.lock:
            ...
            if pid == 0:
                ...
                if command:
                    comps = command.split()
                    if comps and re.match('^[/\\w]*/(ba|c|k|tc)?sh$', comps[0]):
                        cmd = comps
                    else:
                        cmd = [
                         '/bin/sh', '-c', command]
                ...
                self.proc[term_name] = Terminal(term_name, fd, pid, self.screen_callback, height=height, width=width, winheight=winheight, winwidth=winwidth, cookie=cookie, host=self.host, server_url=self.server_url, shared_secret=self.shared_secret, pdelim=self.pdelim, term_params=self.term_params, logfile=self.logfile)
                self.set_size(term_name, height, width, winheight, winwidth)
                alert_msg = ''
    ...
    def export_environment(self, term_name, profile=False):
        ...
        if term:
            ...
            for name, value in self.term_env(term_name, term.cookie, term.height, term.width, term.winheight, term.winwidth, export=True):
                try:
