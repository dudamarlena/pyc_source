...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/graphterm/bin/tornado/auth.py
# Compiled at: 2013-08-26 18:19:12
...
"""
import base64, binascii, hashlib, hmac, logging, time, urllib, urlparse, uuid
...
class OpenIdMixin(object):
    ...
    def authenticate_redirect(self, callback_uri=None, ax_attrs=[
 'name', 'email', 'language', 'username']):
    ...
    def get_authenticated_user(self, callback, http_client=None):
        ...
        http_client.fetch(url, self.async_callback(self._on_authentication_verified, callback), method='POST', body=urllib.urlencode(args))
        return
    ...
    def _openid_args(self, callback_uri, ax_attrs=[], oauth_scope=None):
        ...
        if ax_attrs:
            args.update({'openid.ns.ax': 'http://openid.net/srv/ax/1.0', 
            ...
            if 'name' in ax_attrs:
                ...
                args.update({'openid.ax.type.firstname': 'http://axschema.org/namePerson/first', 
                   'openid.ax.type.fullname': 'http://axschema.org/namePerson', 
                   'openid.ax.type.lastname': 'http://axschema.org/namePerson/last'})
            known_attrs = {'email': 'http://axschema.org/contact/email', 
    ...
    def _on_authentication_verified(self, callback, response):
        if response.error or b('is_valid:true') not in response.body:
            logging.warning('Invalid OpenID response: %s', response.error or response.body)
            callback(None)
            return
        else:
            ax_ns = None
            for name in self.request.arguments.iterkeys():
                if name.startswith('openid.ns.') and self.get_argument(name) == 'http://openid.net/srv/ax/1.0':
                    ax_ns = name[10:]
                    break
...
class OAuthMixin(object):
    ...
    def authorize_redirect(self, callback_uri=None, extra_params=None, http_client=None):
        ...
        if getattr(self, '_OAUTH_VERSION', '1.0a') == '1.0a':
            http_client.fetch(self._oauth_request_token_url(callback_uri=callback_uri, extra_params=extra_params), self.async_callback(self._on_request_token, self._OAUTH_AUTHORIZE_URL, callback_uri))
        else:
            http_client.fetch(self._oauth_request_token_url(), self.async_callback(self._on_request_token, self._OAUTH_AUTHORIZE_URL, callback_uri))
        return
    ...
    def get_authenticated_user(self, callback, http_client=None):
        ...
        if not request_cookie:
            ...
            if http_client is None:
                http_client = httpclient.AsyncHTTPClient()
            http_client.fetch(self._oauth_access_token_url(token), self.async_callback(self._on_access_token, callback))
            return

    def _oauth_request_token_url(self, callback_uri=None, extra_params=None):
        consumer_token = self._oauth_consumer_token()
        url = self._OAUTH_REQUEST_TOKEN_URL
        args = dict(oauth_consumer_key=consumer_token['key'], oauth_signature_method='HMAC-SHA1', oauth_timestamp=str(int(time.time())), oauth_nonce=binascii.b2a_hex(uuid.uuid4().bytes), oauth_version=getattr(self, '_OAUTH_VERSION', '1.0a'))
        if getattr(self, '_OAUTH_VERSION', '1.0a') == '1.0a':
            if callback_uri:
    ...
    def _on_request_token(self, authorize_url, callback_uri, response):
        ...
        data = base64.b64encode(request_token['key']) + b('|') + base64.b64encode(request_token['secret'])
    ...
    def _oauth_access_token_url(self, request_token):
        ...
        args = dict(oauth_consumer_key=consumer_token['key'], oauth_token=request_token['key'], oauth_signature_method='HMAC-SHA1', oauth_timestamp=str(int(time.time())), oauth_nonce=binascii.b2a_hex(uuid.uuid4().bytes), oauth_version=getattr(self, '_OAUTH_VERSION', '1.0a'))
    ...
    def _on_access_token(self, callback, response):
        if response.error:
            ...
            self._oauth_get_user(access_token, self.async_callback(self._on_oauth_get_user, access_token, callback))
            return
...
class TwitterMixin(OAuthMixin):
    ...
    def authenticate_redirect(self, callback_uri=None):
        ...
        http.fetch(self._oauth_request_token_url(callback_uri=callback_uri), self.async_callback(self._on_request_token, self._OAUTH_AUTHENTICATE_URL, None))
        return

    def twitter_request(self, path, callback, access_token=None, post_args=None, **args):
    ...
    def _oauth_consumer_token(self):
        ...
        return dict(key=self.settings['twitter_consumer_key'], secret=self.settings['twitter_consumer_secret'])

    def _oauth_get_user(self, access_token, callback):
        callback = self.async_callback(self._parse_user_response, callback)
        self.twitter_request('/users/show/' + access_token['screen_name'], access_token=access_token, callback=callback)
...
class FriendFeedMixin(OAuthMixin):
    ...
    def _oauth_consumer_token(self):
        ...
        return dict(key=self.settings['friendfeed_consumer_key'], secret=self.settings['friendfeed_consumer_secret'])

    def _oauth_get_user(self, access_token, callback):
        callback = self.async_callback(self._parse_user_response, callback)
        self.friendfeed_request('/feedinfo/' + access_token['username'], include='id,name,description', access_token=access_token, callback=callback)
...
class GoogleMixin(OpenIdMixin, OAuthMixin):
    ...
    def authorize_redirect(self, oauth_scope, callback_uri=None, ax_attrs=[
 'name', 'email', 'language', 'username']):
    ...
    def _oauth_consumer_token(self):
        ...
        return dict(key=self.settings['google_consumer_key'], secret=self.settings['google_consumer_secret'])
...
class FacebookMixin(object):
    ...
    def authenticate_redirect(self, callback_uri=None, cancel_uri=None, extended_permissions=None):
        ...
        args = {'api_key': self.settings['facebook_api_key'], 
           ...
           'return_session': 'true'}
        if cancel_uri:
            args['cancel_url'] = urlparse.urljoin(self.request.full_url(), cancel_uri)
    ...
    def authorize_redirect(self, extended_permissions, callback_uri=None, cancel_uri=None):
    ...
    def get_authenticated_user(self, callback):
        ...
        self.facebook_request(method='facebook.users.getInfo', callback=self.async_callback(self._on_get_user_info, callback, session), session_key=session['session_key'], uids=session['uid'], fields='uid,first_name,last_name,name,locale,pic_square,profile_url,username')
    ...
    def facebook_request(self, method, callback, **args):
        ...
        args['v'] = '1.0'
        args['method'] = method
        args['call_id'] = str(long(time.time() * 1000000.0))
        args['format'] = 'json'
        args['sig'] = self._signature(args)
        url = 'http://api.facebook.com/restserver.php?' + urllib.urlencode(args)
        http = httpclient.AsyncHTTPClient()
...
class FacebookGraphMixin(OAuth2Mixin):
    ...
    def _on_access_token(self, redirect_uri, client_id, client_secret, callback, fields, response):
        if response.error:
            ...
            self.facebook_request(path='/me', callback=self.async_callback(self._on_get_user_info, callback, session, fields), access_token=session['access_token'], fields=(',').join(fields))
            return
...
def _oauth_parse_response(body):
    ...
    special = (
     b('oauth_token'), b('oauth_token_secret'))
