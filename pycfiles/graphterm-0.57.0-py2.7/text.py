...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/graphterm/bin/svgwrite/text.py
...
class TSpan(BaseElement, Presentation):
    ...
    **TSpan** element.
    ...
    def __init__(self, text, insert=None, x=[], y=[], dx=[], dy=[], rotate=[], **extra):
        """
        ...
        if insert is not None:
            if is_string(insert):
                raise TypeError("'insert' should be a <tuple> or a <list>  with at least two elements.")
            if x or y:
                raise ValueError("Use 'insert' and 'x' or 'y' parameter not at the same time!")
            x = [insert[0]]
            y = [insert[1]]
        if x:
...
class TextPath(BaseElement, XLink, Presentation):
    ...
    def __init__(self, path, text, startOffset=None, method='align', spacing='exact', **extra):
        """
