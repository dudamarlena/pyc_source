...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/graphterm/bin/tornado/websocket.py
# Compiled at: 2012-01-30 13:53:59
...
"""
import array, functools, hashlib, logging, struct, time, base64, tornado.escape, tornado.web
...
class WebSocketHandler(tornado.web.RequestHandler):
    ...
    def _execute(self, transforms, *args, **kwargs):
        ...
        if self.request.method != 'GET':
            self.stream.write(tornado.escape.utf8('HTTP/1.1 405 Method Not Allowed\r\n\r\n'))
            self.stream.close()
            return
        if self.request.headers.get('Upgrade', '').lower() != 'websocket':
            self.stream.write(tornado.escape.utf8('HTTP/1.1 400 Bad Request\r\n\r\nCan "Upgrade" only to "WebSocket".'))
            self.stream.close()
            return
        ...
        elif self.allow_draft76() and 'Sec-WebSocket-Version' not in self.request.headers:
            self.ws_connection = WebSocketProtocol76(self)
            self.ws_connection.accept_connection()
        else:
            self.stream.write(tornado.escape.utf8('HTTP/1.1 426 Upgrade Required\r\nSec-WebSocket-Version: 8\r\n\r\n'))
            self.stream.close()
    ...
    def async_callback(self, callback, *args, **kwargs):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                logging.error('Uncaught exception in %s', self.request.path, exc_info=True)
                self._abort()
...
class WebSocketProtocol76(WebSocketProtocol):
    ...
    def accept_connection(self):
        ...
        self.stream.write(tornado.escape.utf8('HTTP/1.1 101 WebSocket Protocol Handshake\r\nUpgrade: WebSocket\r\nConnection: Upgrade\r\nServer: TornadoServer/%(version)s\r\nSec-WebSocket-Origin: %(origin)s\r\nSec-WebSocket-Location: %(scheme)s://%(host)s%(uri)s\r\n%(subprotocol)s\r\n' % dict(version=tornado.version, origin=self.request.headers['Origin'], scheme=scheme, host=self.request.host, uri=self.request.uri, subprotocol=subprotocol_header)))
    ...
    def _on_frame_type(self, byte):
        frame_type = ord(byte)
        if frame_type == 0:
            self.stream.read_until(b(b'\xff'), self._on_end_delimiter)
        elif frame_type == 255:
    ...
    def _on_end_delimiter(self, frame):
        if not self.client_terminated:
            self.async_callback(self.handler.on_message)(frame[:-1].decode('utf-8', 'replace'))
        if not self.client_terminated:
            self._receive_message()
    ...
    def write_message(self, message, binary=False):
        ...
        self.stream.write(b('\x00') + message + b(b'\xff'))

    def close(self):
        """Closes the WebSocket connection."""
        if not self.server_terminated:
            if not self.stream.closed():
                self.stream.write(b'\xff\x00')
            self.server_terminated = True
...
class WebSocketProtocol13(WebSocketProtocol):
    ...
    def _challenge_response(self):
        sha1 = hashlib.sha1()
        sha1.update(tornado.escape.utf8(self.request.headers.get('Sec-Websocket-Key')))
    ...
    def _accept_connection(self):
        ...
        if subprotocols:
            ...
            if selected:
                ...
                subprotocol_header = 'Sec-WebSocket-Protocol: %s\r\n' % selected
        self.stream.write(tornado.escape.utf8('HTTP/1.1 101 Switching Protocols\r\nUpgrade: websocket\r\nConnection: Upgrade\r\nSec-WebSocket-Accept: %s\r\n%s\r\n' % (
