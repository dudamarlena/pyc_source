...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/graphterm/gtermserver.py
# Compiled at: 2014-07-17 15:36:59
"""gtermserver: WebSocket server for GraphTerm
"""
import base64, cgi, collections, datetime, email.utils, functools, getpass, hashlib, hmac, logging, os, Queue, re, ssl, stat, subprocess, sys, threading, time, urllib, urlparse, uuid, random
try:
...
import about, gtermhost, lineterm, optconfig, packetserver
from bin import gterm
import tornado.auth, tornado.httpserver, tornado.ioloop, tornado.options, tornado.web, tornado.websocket
try:
...
server_cert_gen_cmds = ['openssl req -x509 -nodes -days %(expdays)d -newkey rsa:%(keysize)d -batch -subj /O=GraphTerm/CN=%(hostname)s -keyout %(keyfile)s -out %(certfile)s',
...
def ssl_cert_gen(certfile, keyfile='', hostname='localhost', cwd=None, new=False, clientname=''):
    ...
    params = {'certfile': certfile, 'keyfile': keyfile or certfile, 'hostname': hostname, 
       'keysize': 1024, 'expdays': 1024, 'clientname': clientname, 
       'clientprefix': '%s-%s' % (hostname, clientname), 'clientpassword': 'password'}
...
def get_user(ws):
    if ws and ws.authorized:
        return ws.authorized.get('user', '')
    return ''
...
class GTSocket(tornado.websocket.WebSocketHandler):
    ...
    def get_connect_cookie(cls):
        ...
        cls._connect_cookies[new_cookie] = {}
        return new_cookie
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.req_path = ('/').join(rpath.split('/')[2:])
    ...
    def open(self):
        ...
        try:
            ...
            if connect_data is None:
                    ...
                    if self._auth_type == self.NULL_AUTH:
                        ...
                        if not user:
                            ...
                            if code == gterm.compute_hmac(self._auth_code, cauth):
                                ...
                                if not group_secret:
                                    raise AssertionError('Null group secret')
                                    key_version = '1'
                                    if gterm.is_user(user):
                                        if auth_email:
                                            validated = True
                                        ...
                                        self.setup_user(user, email=new_user_email) or self.write_json([['abort', 'Error in creating new user ' + user]])
                                        self.close()
                                        return
                ...
                if not self.authorized:
                    if valid_webcast_req:
                        ...
                        auth_params = {'need_user': need_user, 'need_code': need_code, 'hmac_auth': self._auth_type != self.LOGIN_AUTH, 'goog_auth': self._auth_type == self.MULTI_AUTH and not Server_settings['nogoog_auth'], 
                ...
                if wildhost:
                    ...
                    if len(path_comps) < 2 or not path_comps[1]:
                        ...
                        connect_params = {'state_id': self.authorized['state_id'], 'cookie': self.get_connect_cookie(), 
                           'user': user, 
                           'host': host, 'allow_new': allow_new}
                ...
                if path not in self._terminal_params:
                    ...
                    if is_super_user:
                        ...
                        if self.authorized['auth_type'] > self.WEBCAST_AUTH and self._auth_type <= self.SINGLE_AUTH:
                            allow_host_access = True
                        else:
                    ...
                    if local_or_osh:
                        ...
                        term_owner = host
                    terminal_params = {'share_locked': self._auth_type > self.SINGLE_AUTH, 'share_private': self._auth_type > self.SINGLE_AUTH, 
                       'share_tandem': False, 
                       'alert_status': False, 
                       'widget_token': '', 'last_active': time.time(), 'nb_name': '', 
                       'nb_file': '', 'nb_mod_offset': 0, 'nb_form': '', 
                       'nb_submit': '', 'nb_content': '', 'nb_master': '', 'nb_hosts': {}, 'owner': term_owner, 
                       'state_id': self.authorized['state_id'], 'auth_type': self.authorized['auth_type']}
                ...
                if not self.websocket_id not in self._watch_dict[path]:
                    raise AssertionError
                    controller = False
                    if self.authorized['auth_type'] == self.WEBCAST_AUTH or local_broadcast:
                        controller = False
                    else:
                        if host == gterm.LOCAL_HOST and not is_super_user and self._auth_type >= self.MULTI_AUTH:
                            controller = False
                        ...
                        self.wildcard or TerminalConnection.send_to_connection(host, 'request', term_name, user, [['reconnect', self.websocket_id, Host_settings]])
            ...
            self.write_json([
             ['setup',
              {'user': user, 'host': host, 'term': term_name, 'oshell': self.oshell, 'host_secret': host_secret, 
                 'normalized_host': normalized_host, 'about_version': about.version, 
                 'about_authors': about.authors, 'about_url': about.url, 
                 'about_description': about.description, 'state_values': state_values, 
                 'watchers': users, 'nb_server': Server_settings['nb_server'] or self._auth_type <= self.SINGLE_AUTH, 
                 'nb_autosave': Server_settings['nb_autosave'], 
                 'mathjax': Server_settings['mathjax'], 
                 'auth_type': self.authorized['auth_type'], 
                 'controller': controller, 'super_user': is_super_user, 
                 'parent_term': parent_term, 'wildcard': bool(self.wildcard), 
                 'display_splash': display_splash, 'apps_url': APPS_URL, 
                 'chat': term_chat, 'update_opts': {}, 'state_id': self.authorized['state_id'], 
            ...
            if isinstance(message, bytes):
                ...
                try:
                        ...
                        if msg[0] == 'osh_stdout':
                            ...
                            if msg[0] == 'open_notebook' and msg[1].startswith('/') and msg[1].count('/') == 2:
                                ...
                                if tparams and tparams['nb_content']:
                                    ...
                                    msg[2] = []
                                    msg[3] = {'share': '', 'submit': tparams['nb_submit'], 'master': tempath, 
                            ...
                            if msg[0] == 'chat' and msg[3].strip() in ('alerttrue',
                                                                       'alertfalse'):
                    ...
                    for matchpath in matchpaths:
                        ...
                        if req_list:
                            TerminalConnection.send_to_connection(matchhost, 'request', matchterm, from_user, req_list, _content=send_content)
                        if kill_term:
...
class TerminalConnection(packetserver.RPCLink, packetserver.PacketConnection):
    ...
    def get_matching_paths(cls, matchpath, user, state_id, auth_type, nb_name=''):
        ...
        if isinstance(matchpath, basestring):
            ...
            if terminal_params and (is_super_user or user and user == terminal_params['owner'] or not user and state_id == terminal_params['state_id']):
                return [matchpath]
            return []
        for host, host_connect in cls._all_connections.iteritems():
            for term_name in host_connect.term_dict:
                ...
                if matchpath.match(path):
                    ...
                    if match_params and (is_super_user or user and user == match_params['owner'] or not user and state_id == match_params['state_id']):
    ...
    def __init__(self, stream, address, server_address, key_secret=None, key_version=None, key_id=None, ssl_options={}):
        super(TerminalConnection, self).__init__(stream, address, server_address, server_type='frame', key_secret=key_secret, key_version=key_version, key_id=key_id, ssl_options=ssl_options, max_packet_buf=2)
        self.term_dict = dict()
        self.term_count = 0
            ...
            if msg[0] == 'term_params':
                ...
                if msg[1] == 'note_mod_offset':
                    ...
                    if is_super_user and terminal_params['nb_name'] and terminal_params['nb_form'] == 'share':
                        matchpaths = TerminalConnection.get_matching_paths(wildcard2re('*'), terminal_params['owner'], terminal_params['state_id'], terminal_params['auth_type'], nb_name=terminal_params['nb_name'].replace('-share.', '-shared.'))
                ...
                matchpaths = TerminalConnection.get_matching_paths(checkpath, terminal_params['owner'], terminal_params['state_id'], terminal_params['auth_type'])
                ...
                if params['headers']['x_gterm_response'] != 'admin_command':
                    ...
                    if not is_super_user:
                        ...
                        try:
                            if action == 'sessions':
                                ...
                                if action_autosave:
                                    save_msg = [
                                     'save_notebook', '', None, {'auto_save': True}]
                                    for tpath in all_paths:
                                        thost, tname = tpath.split('/')
                                        TerminalConnection.send_to_connection(thost, 'request', tname, terminal_params['owner'], [save_msg])

                                if action_exec:
                                    exec_msg = [
                                     'keypress', action_exec + '\n']
                                    for tpath in all_paths:
                                        thost, tname = tpath.split('/')
                                        TerminalConnection.send_to_connection(thost, 'request', tname, terminal_params['owner'], [exec_msg])

                                if action_js:
                                    js_headers = {'content_type': 'text/plain', 'x_gterm_response': 'eval_js', 
                                       'x_gterm_parameters': {'echo': 'chat', 'path': term_path, 
                                                              'token': terminal_params['widget_token']}}
                                    js_msg = [
                                     'terminal', 'graphterm_output', [{'headers': js_headers}, base64.b64encode(action_js)]]
                                    for tpath in all_paths:
                    ...
                    save_params = {'content_type': content_type, 'x_gterm_location': 'remote', 'x_gterm_filepath': '', 
                       'x_gterm_encoding': 'base64'}
                    if errmsg:
                        save_params['x_gterm_error'] = errmsg
                    resp_list = [
                     [
                      'save_data', save_params, base64.b64encode(content)]]
...
class ProxyFileHandler(tornado.web.RequestHandler):
    ...
    def get(self, path):
        ...
        if bheaders:
            ...
            if self.request.path.startswith(gterm.BLOB_PREFIX):
                if last_modified_datetime and if_mod_since_datetime and if_mod_since_datetime >= last_modified_datetime:
                    self.set_status(304)
                    self.finish()
                    return
    ...
    def complete_get(self, status=(), last_modified=None, etag=None, content_type=None, content_length=None, content_b64=''):
        if not status:
            self.send_error(408)
            return
        ...
        if self.request.path.startswith(gterm.BLOB_PREFIX):
            headers.append(('Expires',
             datetime.datetime.utcnow() + datetime.timedelta(seconds=MAX_CACHE_TIME)))
        ...
        cache = self.request.method != 'HEAD' and (self.request.path.startswith(gterm.BLOB_PREFIX) or Cache_files and last_modified)
...
def run_server(options, args):
    ...
    class AuthHandler(tornado.web.RequestHandler):
        ...
        def get(self):
            ...
            if auth_type >= GTSocket.MULTI_AUTH and user:
                ...
                if auth_type >= GTSocket.SINGLE_AUTH:
    ...
    class GoogleOAuth2LoginHandler(tornado.web.RequestHandler, tornado.auth.GoogleOAuth2Mixin):
        ...
        def get(self):
            ...
            if not self.get_argument('code', False):
                ...
                yield self.authorize_redirect(redirect_uri=auth_uri, client_id=self.settings['google_oauth']['key'], scope=[
                 'profile', 'email'], response_type='code', extra_params={'approval_prompt': 'auto', 'state': state_param})
            else:
                state_value = self.get_argument('state', '')
                user = yield self.get_authenticated_user(redirect_uri=auth_uri, code=self.get_argument('code'))
                try:
    ...
    Server_settings = {'host': http_host, 'port': http_port, 'https': options.https, 'external_host': external_host, 
       'external_port': external_port, 'internal_host': internal_host, 
       'internal_port': internal_port, 'allow_embed': options.allow_embed, 
       'allow_share': options.allow_share, 'user_setup': options.user_setup, 
       'no_formcheck': options.no_formcheck, 'nb_autosave': options.nb_autosave, 
       'nb_server': options.nb_server, 'nogoog_auth': options.nogoog_auth, 
       'user_groups': membership_dict, 'users_dir': options.users_dir, 
       'gtermhost_args': gtermhost_args, 'mathjax': not options.nomathjax, 
       'max_terminals': options.max_terminals}
    Host_settings = {'lterm_params': {'nb_ext': options.nb_ext, 'term_opts': options.term_opts, 'lc_export': options.lc_export}, 
       'term_type': options.term_type, 
       'term_encoding': options.term_encoding, 'blob_host': options.blob_host, 
       'command': options.shell_command, 'prompt_list': options.prompts.split(',') if options.prompts else gterm.DEFAULT_PROMPTS, 
       'https': options.https, 
       'logging': options.logging, 'widget_port': options.widget_port, 
    ...
    if options.auth_users:
        if not options.auth_type == 'multiuser':
            raise AssertionError('Must specify auth_type=multiuser ')
        ...
        handlers = [
         (
          '/_auth/.*', AuthHandler),
         (
          '/_gauth.*', GoogleOAuth2LoginHandler),
         (
          '/_form/.*', FormHandler),
         (
          '/_steal/.*', ActionHandler),
         (
          '/_watch/.*', ActionHandler),
         (
          '/_websocket/.*', GTSocket),
         (
          gterm.STATIC_PREFIX + '(.*)', tornado.web.StaticFileHandler, {'path': Doc_rootdir}),
         (
          gterm.BLOB_PREFIX + '([\\w\\-]+/' + gterm.TRUSTED_PREFIX + '.*)', ProxyFileHandler, {}),
         (
          gterm.FILE_PREFIX + '(.*)', ProxyFileHandler, {}),
         (
          '/().*', tornado.web.StaticFileHandler, {'path': Doc_rootdir, 'default_filename': 'index.html'})]
        alt_handlers = [
         (
          gterm.STATIC_PREFIX + '(.*)', tornado.web.StaticFileHandler, {'path': Doc_rootdir}),
         (
          gterm.BLOB_PREFIX + '([\\w\\-]+/' + gterm.UNTRUSTED_PREFIX + '.*)', ProxyFileHandler, {}),
         (
          gterm.FILE_PREFIX + '(.*)', ProxyFileHandler, {})]
        ...
        try:
            TCP_server = TerminalConnection.start_tcp_server(internal_host, internal_port, io_loop=IO_loop, key_secret=key_secret, key_version=key_version, key_id=str(internal_port), ssl_options=internal_server_ssl)
        except Exception as excp:
        ...
        if options.internal_https or options.nolocal:
            ...
            Local_client, Host_secret, Trace_shell = gtermhost.gterm_connect(gterm.LOCAL_HOST, internal_host, server_port=internal_port, connect_kw={'ssl_options': internal_client_ssl, 'key_secret': local_key_secret, 
               'key_version': key_version, 
               'key_id': str(internal_port), 
               'lterm_logfile': options.lterm_logfile}, oshell_globals=oshell_globals, oshell_init='gtermserver.trc', oshell_unsafe=True, oshell_thread=not options.oshell_input, oshell_no_input=not options.oshell_input, oshell_web_interface=TraceInterface, io_loop=IO_loop)
        ...
        Server_settings['group_secret'] = group_secret
        action = 'activate' if options.auth_type == 'multiuser' and options.user_setup and (options.no_reset or options.daemon == 'activate') else 'restart'
        cmd_args = ['sudo'] if os.geteuid() else []
    ...
    if options.terminal:
        ...
        if options.auth_type == 'singleuser':
            ...
            if options.auth_type == 'multiuser' and super_users:
...
def main():
    ...
    parser.add_option('auth_type', default='singleuser', help='Authentication type: none/name/singleuser/multiuser (default: singleuser)')
    parser.add_option('auth_users', default='', help='Comma-separated list of authenticated user names')
    parser.add_option('host', default='localhost', help='Hostname (or IP address) (default: localhost)')
    parser.add_option('port', default=gterm.DEFAULT_HTTP_PORT, help='IP port (default: %d)' % gterm.DEFAULT_HTTP_PORT, opt_type='int')
    parser.add_option('terminal', default=False, opt_type='flag', help='Open new terminal window')
    parser.add_option('external_host', default='', help='external host name (or IP address), if different from host')
    parser.add_option('external_port', default=0, help='external port, if different from port', opt_type='int')
    parser.add_option('internal_host', default='', help='internal host name (or IP address) (default: localhost)')
    parser.add_option('internal_port', default=0, help='internal port (default: PORT-1)', opt_type='int')
    parser.add_option('blob_host', default='', help='blob server host name (or IP address) (default: same as server)')
    parser.add_option('nomathjax', default=False, opt_type='flag', help='Disable MathJax')
    parser.add_option('nolocal', default=False, opt_type='flag', help="Disable connection to 'local' host")
    parser.add_option('nogoog_auth', default=False, opt_type='flag', help='Disable google authentication for multiuser type')
    parser.add_option('super_users', default='', help='Super user list: root,admin,...')
    parser.add_option('shell_command', default=gterm.SHELL_CMD, help='Shell command (default: %s)' % gterm.SHELL_CMD)
    parser.add_option('oshell', default=False, opt_type='flag', help='Activate otrace/oshell')
    parser.add_option('oshell_input', default=False, opt_type='flag', help='Allow stdin input otrace/oshell')
    parser.add_option('https', default=False, opt_type='flag', help='Use SSL (TLS) connections for security')
    parser.add_option('internal_https', default=False, opt_type='flag', help='Use https for internal connections')
    parser.add_option('user_setup', default='', help="User setup: '' or 'auto' or 'manual'")
    parser.add_option('prompts', default='', help="Inner prompt formats delim1,delim2,fmt,remote_fmt (default:',$,\\W,\\h:\\W')")
    parser.add_option('lc_export', default='', help="Export environment as locale (values: '' or 'graphterm' or 'telephone') NOTE: Use with caution; insecure on untrusted computers")
    parser.add_option('nb_ext', default='', help="File extension for python notebooks ('ipynb' (default) or 'py.gnb.md')")
    parser.add_option('nb_autosave', default=300, help='Notebook autosave interval (default: 300)', opt_type='int')
    parser.add_option('nb_server', default=False, opt_type='flag', help='Enable PUBLIC ipython notebook server for multiuser case')
    parser.add_option('allow_embed', default=False, opt_type='flag', help='Allow iframe embedding of terminal on other domains (possibly insecure)')
    parser.add_option('allow_share', default=False, opt_type='flag', help='Allow sharing of terminals between multiple users')
    parser.add_option('users_dir', default=default_users_dir, help='Users home directory (default: %s)' % default_users_dir)
    parser.add_option('no_reset', default=False, opt_type='flag', help='Do not reset existing host connections')
    parser.add_option('no_formcheck', default=False, opt_type='flag', help='Disable form checking (INSECURE)')
    parser.add_option('certfile', default='', help='Path to server cert file')
    parser.add_option('client_cert', default='', help="Path to client CA cert (or '.')")
    parser.add_option('term_type', default='', help='Terminal type (linux/screen/xterm)')
    parser.add_option('term_encoding', default='utf-8', help='Terminal character encoding (utf-8/latin-1/...)')
    parser.add_option('term_opts', default='', help='Terminal options: no_colors,no_pyindent,no_untrusted,...')
    parser.add_option('term_settings', default='{}', help='Terminal settings (JSON)')
    parser.add_option('max_terminals', default=10, help='maximum no. of terminals per user (default: 10)', opt_type='int')
    parser.add_option('lterm_logfile', default='', help='Lineterm logfile')
    parser.add_option('logging', default=False, opt_type='flag', help='Log to ~/.graphterm/gtermserver.log')
    parser.add_option('widget_port', default=-1, opt_type='int', help='Port number for widget socket (default: -1 for auto)')
    parser.add_option('daemon', default='', help='daemon=start/stop/restart/status')
