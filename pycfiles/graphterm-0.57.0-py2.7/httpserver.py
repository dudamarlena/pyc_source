...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/graphterm/bin/tornado/httpserver.py
...
class HTTPConnection(object):
    ...
    def write(self, chunk, callback=None):
        ...
        if not self._request:
            raise AssertionError('Request closed')
            self._write_callback = self.stream.closed() or stack_context.wrap(callback)
    ...
    def finish(self):
        ...
        if not self._request:
            ...
            self.stream.writing() or self._finish_request()
    ...
    def _finish_request(self):
        if self.no_keep_alive:
            ...
            elif 'Content-Length' in self._request.headers or self._request.method in ('HEAD',
                                                                                       'GET'):
        ...
        if self._request.method in ('POST', 'PUT'):
            if content_type.startswith('application/x-www-form-urlencoded'):
                ...
                for field in fields:
                    ...
                    if k == 'boundary' and v:
                        httputil.parse_multipart_form_data(utf8(v), data, self._request.arguments, self._request.files)
                        break
                else:
