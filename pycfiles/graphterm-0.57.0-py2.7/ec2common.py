...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/graphterm/bin/ec2common.py
# Compiled at: 2014-07-01 12:56:16
"""
ec2common: Common code to manage Amazon AWS EC2 instances
"""
from __future__ import absolute_import, print_function
import boto, os, re, sys, time
...
def check_auth_file(auth_file):
    ...
    if not os.path.isfile(auth_file):
        ...
        try:
            ...
            with os.fdopen(os.open(auth_file, os.O_WRONLY | os.O_CREAT, 384), 'w') as (f):
...
def cname(route53conn, zone, domain_name, alt_name, ttl=60, remove=False):
    ...
    change = changes.add_change('DELETE' if remove else 'CREATE', name=domain_name, type='CNAME', ttl=ttl)
    if alt_name:
...
def get_instance_props(name=None):
    ...
    for res in all_instances:
        ...
        if name_re:
            ...
            matched = True
        if matched:
            props = {'id': iobj.id, 'public_dns': iobj.public_dns_name, 'key': iobj.key_name, 
               'state': iobj.state, 
               'tags': iobj.tags}
