...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/graphterm/bin/tornado/web.py
...
import Cookie, base64, binascii, calendar, datetime, email.utils, functools, gzip, hashlib, hmac, httplib, itertools, logging, mimetypes, os.path, re, stat, sys, threading, time, tornado, traceback, types, urllib, urlparse, uuid
...
class RequestHandler(object):
    ...
    def __init__(self, application, request, **kwargs):
        ...
        self.ui = ObjectDict((n, self._ui_method(m)) for n, m in application.ui_methods.iteritems())
        self.ui['_modules'] = ObjectDict((n, self._ui_module(n, m)) for n, m in application.ui_modules.iteritems())
    ...
    def clear(self):
        ...
        self._headers = {'Server': 'TornadoServer/%s' % tornado.version, 
    ...
    def render_string(self, template_name, **kwargs):
        ...
        args = dict(handler=self, request=self.request, current_user=self.current_user, locale=self.locale, _=self.locale.translate, static_url=self.static_url, xsrf_form_html=self.xsrf_form_html, reverse_url=self.application.reverse_url)
        args.update(self.ui)
        args.update(kwargs)
    ...
    def write_error(self, status_code, **kwargs):
        ...
        if self.settings.get('debug') and 'exc_info' in kwargs:
            ...
            self.finish('<html><title>%(code)d: %(message)s</title><body>%(code)d: %(message)s</body></html>' % {'code': status_code, 
    ...
    def locale(self):
        ...
        header.
        """
        self._locale = hasattr(self, '_locale') or self.get_user_locale()
        if not self._locale:
    ...
    def require_setting(self, name, feature='this feature'):
        ...
        if not self.application.settings.get(name):
            raise Exception("You must define the '%s' setting in your application to use %s" % (
             name, feature))
    ...
    def _generate_headers(self):
        lines = [
         utf8(self.request.version + ' ' + str(self._status_code) + ' ' + httplib.responses[self._status_code])]
        lines.extend([ utf8(n) + b(': ') + utf8(v) for n, v in itertools.chain(self._headers.iteritems(), self._list_headers)
                     ])
...
class Application(object):
    ...
    def __init__(self, handlers=None, default_host='', transforms=None, wsgi=False, **settings):
        ...
        self.ui_modules = {'linkify': _linkify, 'xsrf_form_html': _xsrf_form_html, 
...
class GZipContentEncoding(OutputTransform):
    ...
    def transform_first_chunk(self, headers, chunk, finishing):
        if self._gzipping:
            ...
            self._gzipping = ctype in self.CONTENT_TYPES and (not finishing or len(chunk) >= self.MIN_LENGTH) and (finishing or 'Content-Length' not in headers) and 'Content-Encoding' not in headers
        if self._gzipping:
...
class TemplateModule(UIModule):
    ...
    def render(self, path, **kwargs):
        ...
        def set_resources(**kwargs):
            ...
            elif self._resource_dict[path] != kwargs:
...
class URLSpec(object):
    ...
    def _find_groups(self):
        ...
        if self.regex.groups != pattern.count('('):
            ...
            return (
             ('').join(pieces), self.regex.groups)

    def reverse(self, *args):
        if not self._path is not None:
            raise AssertionError('Cannot reverse url regex ' + self.regex.pattern)
            assert len(args) == self._group_count, 'required number of arguments not found'
            return len(args) or self._path
        else:
...
def decode_signed_value(secret, name, value, max_age_days=31):
    if not value:
        ...
        if timestamp < time.time() - max_age_days * 86400:
            ...
            return
        if timestamp > time.time() + 2678400:
