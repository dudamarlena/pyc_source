...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/graphterm/bin/tornado/wsgi.py
...
"""
import Cookie, cgi, httplib, logging, sys, time, tornado, urllib
...
class HTTPRequest(object):
    ...
    def __init__(self, environ):
        ...
        if self.headers.get('Content-Length'):
            self.body = environ['wsgi.input'].read(int(self.headers['Content-Length']))
        else:
            self.body = ''
        ...
        if content_type.startswith('application/x-www-form-urlencoded'):
            ...
            if 'boundary=' in content_type:
                ...
                if boundary:
                    httputil.parse_multipart_form_data(utf8(boundary), self.body, self.arguments, self.files)
            else:
...
class WSGIContainer(object):
    ...
    def environ(request):
        ...
        environ = {'REQUEST_METHOD': request.method, 'SCRIPT_NAME': '', 
