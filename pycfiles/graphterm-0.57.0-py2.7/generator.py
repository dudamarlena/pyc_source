...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/graphterm/bin/landslide/generator.py
# Compiled at: 2014-02-02 09:23:11
import os, re, codecs, inspect, jinja2, shutil, tempfile, utils, ConfigParser
...
class Generator(object):
    ...
    def __init__(self, source, **kwargs):
        ...
        if self.destination_file.endswith('.html'):
            ...
            raise IOError('This program can only write html or pdf files. Please use one of these file extensions in the destination')
    ...
    def add_user_css(self, css_list):
        ...
        for css_path in css_list:
            if css_path and css_path not in self.user_css:
                ...
                self.user_css.append({'path_url': utils.get_path_url(css_path, self.relative), 
    ...
    def add_user_js(self, js_list):
        ...
        for js_path in js_list:
            if js_path and js_path not in self.user_js:
                ...
                self.user_js.append({'path_url': utils.get_path_url(js_path, self.relative), 
    ...
    def add_toc_entry(self, title, level, slide_number):
        ...
        """
        self.__toc.append({'title': title, 'number': slide_number, 'level': level})
    ...
    def execute(self):
        ...
        if self.direct:
            if self.file_type == 'pdf':
                raise IOError('Direct output mode is not available for PDF export')
            else:
    ...
    def fetch_contents(self, source):
        ...
        if type(source) is list:
            ...
            self.log('Adding   %s (%s)' % (source, parser.format))
            try:
                file = codecs.open(source, encoding=self.encoding)
                file_contents = file.read()
            except UnicodeDecodeError:
                self.log('Unable to decode source %s: skipping' % source, 'warning')

            inner_slides = re.split('<hr.+>', parser.parse(file_contents))
            for inner_slide in inner_slides:
                slides.append(self.get_slide_vars(inner_slide, source))

        if not slides:
            self.log('Exiting  %s: no contents found' % source, 'notice')
        return slides
    ...
    def get_css(self):
        ...
        if not os.path.exists(print_css):
            ...
            if not os.path.exists(print_css):
                raise IOError('Cannot find css/print.css in default theme')
        ...
        if os.path.exists(screen_css):
            css['screen'] = {'path_url': utils.get_path_url(screen_css, self.relative), 'contents': open(screen_css).read()}
        else:
            self.log('No screen stylesheet provided in current theme', 'warning')
        return css
    ...
    def get_js(self):
        ...
        if not os.path.exists(js_file):
            ...
            if not os.path.exists(js_file):
                raise IOError('Cannot find slides.js in default theme')
    ...
    def get_slide_vars(self, slide_src, source=None):
        ...
        """
        find = re.search('(<h(\\d+?).*?>(.+?)</h\\d>)\\s?(.+)?', slide_src, re.DOTALL | re.UNICODE)
        ...
        if content:
            ...
            find = re.search('<h\\d[^>]*>presenter notes</h\\d>', content, re.DOTALL | re.UNICODE | re.IGNORECASE)
            if find:
                presenter_notes = content[find.end():].strip()
                content = content[:find.start()]
        source_dict = {}
        if source:
            source_dict = {'rel_path': source, 'abs_path': os.path.abspath(source)}
        if header or content:
            return {'header': header, 'title': title, 'level': level, 'content': content, 
               'classes': slide_classes, 'source': source_dict, 
    ...
    def get_template_vars(self, slides):
        ...
        for slide_index, slide_vars in enumerate(slides):
            ...
            if slide_vars['level'] and slide_vars['level'] <= TOC_MAX_LEVEL:
                ...
                self.add_toc_entry('-', 1, slide_number)

        return {'head_title': head_title, 'num_slides': str(self.num_slides), 'slides': slides, 'toc': self.toc, 'embed': self.embed, 'css': self.get_css(), 
           'js': self.get_js(), 'user_css': self.user_css, 
           'user_js': self.user_js}

    def linenos_check(self, value):
        """ Checks and returns a valid value for the ``linenos`` option.
        """
        if value in VALID_LINENOS:
            return value
        return 'inline'
    ...
    def log(self, message, type='notice'):
        ...
        if self.logger and not callable(self.logger):
            raise ValueError('Invalid logger set, must be a callable')
    ...
    def parse_config(self, config_source):
        ...
        config = {}
        config['source'] = raw_config.get('landslide', 'source').replace('\r', '').split('\n')
    ...
    def process_macros(self, content, source=None):
        ...
        for macro_class in self.macros:
            try:
                ...
                self.log('%s processing failed in %s: %s' % (
                 macro, source, e))
    ...
    def register_macro(self, *macros):
        ...
        for m in macros:
            if inspect.isclass(m) and issubclass(m, macro_module.Macro):
                ...
                raise TypeError("Coundn't register macro; a macro must inherit from macro.Macro")
    ...
    def render(self):
        ...
        if self.embed:
            images = re.findall('\\s+background(?:-image)?:\\surl\\((.+?)\\).+;', html, re.DOTALL | re.UNICODE)
    ...
    def write_pdf(self, html):
        ...
        try:
            try:
                command = [
                 'prince', f.name, self.destination_file]
                Popen(command, stderr=dummy_fh).communicate()
            except Exception:
                raise EnvironmentError('Unable to generate PDF file using prince. Is it installed and available?')
