...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\zhpy\pyparsing.py
...
import copy, sys, warnings, re, sre_constants
__all__ = [
...
class WordStart(_PositionToken):
    """Matches if the current position is at the beginning of a Word, and
       ...
       (default=printables). To emulate the \x08 behavior of regular expressions,
...
def _makeTags(tagStr, xml):
    ...
    if xml:
        ...
        openTag = Suppress('<') + tagStr + Dict(ZeroOrMore(Group(tagAttrName + Suppress('=') + tagAttrValue))) + Optional('/', default=[False]).setResultsName('empty').setParseAction(lambda s, l, t: t[0] == '/') + Suppress('>')
    else:
...
def indentedBlock(blockStatementExpr, indentStack, indent=True):
    ...
    if indent:
        smExpr = Group(Optional(NL) + FollowedBy(blockStatementExpr) + INDENT + OneOrMore(PEER + Group(blockStatementExpr) + Optional(NL)) + UNDENT)
    else:
