...
# [GCC 8.4.0]
# Embedded file name: /home/kylin/pyenvs/kylins-flask/lib/python2.7/site-packages/test_service/curl_reader.py
# Compiled at: 2015-02-14 10:10:12
...
class ScriptExecute(object):
    ...
    def __path_result_file(self):
        u"""生成日志的目录及文件形式.
        """
        result_file_logs = os.path.join(os.path.dirname(project_path), 'log')
        if not os.path.exists(result_file_logs):
            os.makedirs(result_file_logs)
        if not self.log_file_name:
            log_file_name = os.path.join(result_file_logs, 'curl_%s.htm')
    ...
    def __read_script_file(self):
        ...
        if not self.script_lines and os.path.exists(self.script_file):
            ...
            if lines:
                ...
                six.print_('测试脚本, 指定的文件内容为空.')
        else:
            six.print_('测试脚本, 指定的文件不存在.')
