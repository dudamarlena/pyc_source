...
# [GCC 8.4.0]
# Embedded file name: /home/antonio/Projects/ipkiss_manager/ipkiss_manager/pyhg/pyhg.py
# Compiled at: 2013-01-23 05:44:14
...
class Repo(object):
    ...
    def hg_command(self, *args):
        ...
        if os.name == 'posix':
            ...
            proc = Popen(['hg', '--cwd', self.path, '--encoding', 'UTF-8'] + list(args), stdout=PIPE, stderr=PIPE, env=self._env, shell=True)
    ...
    def hg_clone(self, uri, dest, username=None, password=None):
        """Clone a
        mercurial repository"""
        res = self.hg_command('clone', uri, dest)
        return res
    ...
    def revisions(self, slice_):
        ...
        out = self.hg_log(identifier=(':').join([ str(x) for x in (slice_.start, slice_.stop) ]), template=self.rev_log_tpl)
        revs = []
