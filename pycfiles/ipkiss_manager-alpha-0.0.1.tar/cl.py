...
# [GCC 8.4.0]
# Embedded file name: /home/antonio/Projects/ipkiss_manager/ipkiss_manager/app/cl.py
# Compiled at: 2013-01-23 05:07:06
"""
...
class CommandLineInterface(object):
    ...
    def command_remove(self, args):
        if args['version'] is None:
            ...
            except RemoveByVersionError as e:
    ...
    def command_set_default(self, args):
        path = args['path']
        version, release = parse_version(args['version'], args['release'])
        self.manager.set_default_release(path=path, version=version, release=release)
    ...
    def command_install(self, args):
        ...
        if version == 'newest':
            try:
                self.manager.install_latest_version(path=args['path'], console=True)
            except VersionError:
                print >> sys.stderr, ('It was not possible to install IPKISS at {}').format(args['path'])

        else:
            try:
                self.manager.install_ipkiss(version=version, path=args['path'], console=True)
            except VersionError:
