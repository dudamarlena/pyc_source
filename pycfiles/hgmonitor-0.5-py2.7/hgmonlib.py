...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hgmonitor/hgmonlib.py
# Compiled at: 2012-07-25 17:55:02
"""
...
import mercurial
from hglib import *
import os, re, platform, datetime, zipfile, shutil, email.mime.text as textmsg
...
def monitor_cmd(ui, **opts):
    ...
    if len(dirty_file_list) > 0 and opts['email'] != '':
        ...
        for repo, reason, file in dirty_file_list:
            if (
             repo, reason) != (lastrepo, lastreason) and lastreason is not None:
                body += '</ul>\n'
            if repo != lastrepo:
...
def mbackup_cmd(ui, **opts):
    ...
    for r in repo_list(opts['dir'], opts.has_key('mq') and opts['mq']):
        ...
        dest = dest.format(clone=os.path.basename(r), fullclone=r.replace('/', '_').replace('\\', '_'), date=d.strftime('%Y-%m-%d'), datetime=d.strftime('%Y-%m-%d-%H-%M-%S'), time=d.strftime('%H-%M-%S'), sysname=platform.node())
        ...
        if os.path.exists(os.path.join(r, '.hg', 'patches')) and not (opts.has_key('mq') and opts['mq']):
            for f in mq_files(r):
