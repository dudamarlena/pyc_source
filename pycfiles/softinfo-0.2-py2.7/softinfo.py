...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/softinfo.py
# Compiled at: 2019-02-02 10:07:08
import commands, json, logging, re, sys, os, pwd, grp
reload(sys)
...
class ps_aux(object):
    ...
    def __init__(self, cmd, appType=None):
        u"""Initialize,cmd 用于 ps axu或是 ps -ef 等筛选语句为str"""
        self.cmd = cmd
...
class fifter(ps_aux):
    ...
    @property
    def takelook(self):
        u"""对于lookfor方法的输出进行json格式的输出"""
        ini = fifter(self.cmd)
        resultList = ini.lookfor
        node_dict = json.dumps(resultList, sort_keys=True, indent=4, separators=(',',
                                                                                 ': '), encoding='utf8', ensure_ascii=True)
        return node_dict
