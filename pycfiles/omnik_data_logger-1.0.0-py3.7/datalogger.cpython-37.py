...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/omnik/datalogger.py
# Compiled at: 2020-01-12 07:07:31
...
class DataLogger(object):
    ...
    def __init__(self, config, debug=False):
        self.config = config
        self.plugins = []
        if not (self.config.has_option('omnikportal', 'username') and self.config.has_option('omnikportal', 'password')):
            logger.error('No username/password for omnikportal found')
            sys.exit(1)
        if debug:
            logger.setLevel(logging.DEBUG)
        self.client = OmnikPortalClient(logger=logger,
        ...
        if len(self.plugins) > 0:
            sys.path.append(self._DataLogger__expand_path('plugins'))
