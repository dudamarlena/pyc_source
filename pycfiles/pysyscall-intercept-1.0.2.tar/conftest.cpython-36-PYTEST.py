...
# [GCC 8.4.0]
# Embedded file name: /home/lukasz/projects/pysyscall_intercept/external/pybind11/tests/conftest.py
# Compiled at: 2018-09-08 23:11:04
# Size of source mod 2**32: 6671 bytes
"""pytest configuration

Extends output capture as needed by pybind11: ignore constructors, optional unordered lines.
Adds docstring and exceptions message sanitizers: ignore Python 2 vs 3 differences.
"""
import builtins as @py_builtins, _pytest.assertion.rewrite as @pytest_ar, pytest, textwrap, difflib, re, sys, contextlib, platform, gc
_unicode_marker = re.compile("u(\\'[^\\']*\\')")
_long_marker = re.compile('([0-9])L')
_hexadecimal = re.compile('0x[0-9a-fA-F]+')
...
class Output(object):
    __doc__ = 'Basic output post-processing and comparison'
...
class Unordered(Output):
    __doc__ = 'Custom comparison for output without strict line ordering'
...
def _sanitize_general(s):
    ...
    s = _long_marker.sub('\\1', s)
    s = _unicode_marker.sub('\\1', s)
    return s
...
def pytest_namespace():
    ...
    return {'suppress':suppress, 
     ...
     'requires_eigen_and_numpy':skipif(not have_eigen or not np, reason='eigen and/or numpy are not installed'), 
