...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\inspie\api.py
# Compiled at: 2019-02-27 02:15:41
# Size of source mod 2**32: 12395 bytes
import requests, json, hashlib, math, time, copy
...
class InspieAPI(object):
    ...
    def login(self, force=False):
        if (not self.is_logged_in or force) and self.send_request('si/fetch_headers/?challenge_type=signup&guid={}'.format(generate_UUID(False)), None, True):
        ...
        if self.send_request('accounts/login/', generate_signature(json.dumps(data)), True):
    ...
    def media_configure(self, upload_id, photo, caption=''):
        ...
        data = json.dumps({'_csrftoken': self.token, 
         ...
         'edits': {'crop_original_size': [w * 1.0, h * 1.0], 
                   'crop_center': [0.0, 0.0], 
                   'crop_zoom': 1.0}, 
         
         'extra': {'source_width': w, 
                   'source_height': h}})
    ...
    def video_configure(self, upload_id, video, thumbnail, caption=''):
        ...
        data = json.dumps({'upload_id': upload_id, 
         ...
         'clips': {'length': clip.duration, 
                   'source_type': '3', 
                   'camera_position': 'back'}, 
         
         'extra': {'source_width': clip.size[0], 
    ...
    def upload_photo(self, photo, caption=None, upload_id=None, is_sidecar=None):
        ...
        if response.status_code == 200 and self.media_configure(upload_id, photo, caption):
            self.expose()
    ...
    def upload_Video(self, video, thumbnail, caption=None, upload_id=None, is_sidecar=None):
        ...
        if response.status_code == 200:
            ...
            for i in range(0, 4):
                ...
                content_range = 'bytes {start}-{end}/{lenVideo}'.format(start=start, end=end - 1, lenVideo=len(video_data)).encode('utf-8')
            ...
            if response.status_code == 200 and self.video_configure(upload_id, video, thumbnail, caption):
                self.expose()
                return True
        return False
