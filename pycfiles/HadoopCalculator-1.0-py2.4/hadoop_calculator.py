...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hadoopcalculator/hadoop_calculator.py
# Compiled at: 2009-07-27 23:12:37
...
if __name__ == '__main__':
    ...
    parser.add_option('-d', '--data', dest='data', default='100g', help="Amount of data we're talking about. Filesizes can be in exa(e), peta(p), tera(t), gig(g), mega(m), kilo(k)")
    parser.add_option('-m', '--mbps', dest='mbps', default=150, help='aggregate throughput for cluster processing in Mbps')
    parser.add_option('-n', '--nodes', dest='nodes', default=10, help='number of nodes in your cluster')
    parser.add_option('-t', '--time', dest='time', default='30m', help='How much time are you giving us? Time can be in second(s), minute(m), hour(h), day(d), week(w)')
    parser.add_option('-q', '--quick', dest='quick', action='store_true', default=False, help='Quickly and quietly, just give me the number without the fluff')
    (opt, args) = parser.parse_args()
    if not args or args[0] not in ['gigs', 'mbps', 'nodes', 'seconds']:
        parser.print_help()
        sys.exit(1)
    defs = {'megs': filesize(opt.data), 'mbps': int(opt.mbps), 'nodes': int(opt.nodes), 'seconds': unitoftime(opt.time)}
    if args[0] == 'gigs':
