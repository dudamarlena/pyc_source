...
# Embedded file name: build/bdist.macosx-10.5-i386/egg/brainfreeze/doc/build/rst2wiki.py
# Compiled at: 2008-07-31 21:48:27
__doc__ = '\nReStructured Text -> Google Code Wiki translator\nIan Charnas <ian.charnas@gmail.com>\nModified from the rst2wiki.py from the python-nose project\n'
import sys
...
class WikiVisitor(SparseNodeVisitor):
    ...
    def visit_reference(self, node):
        if node.has_key('refuri'):
            href = node['refuri']
        elif node.has_key('refid') and node.has_key('name'):
            href = '#' + node['name'].replace(' ', '_')
        else:
            href = None
