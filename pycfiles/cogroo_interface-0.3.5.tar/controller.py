...
# Embedded file name: build/bdist.darwin-8.8.0-Power_Macintosh/egg/cogplanet/admin/controller.py
# Compiled at: 2006-12-30 03:23:06
import cherrypy
...
class PlanetFields(WidgetsList):
    ...
    display_entries = TextField(name='display_entries', label='Entries to display', validator=validators.Number(), attrs={'size': 60})
    update_interval = TextField(name='update_interval', label='Update Interval (Minutes)', validator=validators.Number(), attrs={'size': 60})
...
class AdminController(controllers.Controller, Restful):
    ...
    def entry(self, id):
        ...
        return {'entry': entry, 'planet': planet}
    ...
    def import_feeds(self, opml_file=None):
        ...
        planet = Planet.selectone()
        return {'import_form': self.import_form, 'planet': planet}

    @expose(template='cogplanet.templates.admin.planet')
    @identity.require(identity.in_group('cp_admin'))
    def planet_view(self):
        planet = Planet.selectone()
        return {'planet': planet, 'planet_form': self.planet_form}
    ...
    def view(self, id):
        ...
        planet = Planet.selectone()
        return {'feed': feed, 'feed_form': self.feed_form, 'planet': planet}
