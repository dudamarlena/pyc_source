...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/app/main/service/metrics_service.py
# Compiled at: 2020-04-16 16:30:09
# Size of source mod 2**32: 16868 bytes
...
def format_label(string):
    label = re.sub('[().\\\'\\"]|((?<![/:])/)', '', string)
    label = re.sub('^%(?=\\w)', 'percent ', label)
    label = re.sub('%', ' percent', label)
    label = re.sub('>', 'greater than', label)
    label = re.sub('<', 'less than', label)
    label = re.sub('SQL\\*Net', 'Net8', label)
...
def get_appdynamics_metrics(job_name, services, application, metric_path, minutes, static_labels=()):
    collector_metrics = {}
    ...
    for service_name in services:
        ...
        options = {'time_range_type':'BEFORE_NOW', 
        ...
        for metric_obj in result:
            ...
            if len(pushgateways):
                if instance is not None:
                ...
                elif application == 'Database Monitoring':
                    ...
                    if components[0] == 'Backends':
                        pattern = re.compile('^(?P<node>Discovered backend call) - (?P<value>.+)$', re.X)
                        ...
                        if m is not None:
                            ...
                            if components[0] == 'Service Endpoints':
                                ...
                                if len(components) > 2:
                                    ...
                                    while len(components) > 3:
                                        if components[1] == 'External Calls':
                                            pattern = re.compile('^(?P<node>.+)(?= to Discovered) to Discovered backend call - (?P<value>.+)$', re.X)
                        ...
                        if components[0] == 'Application Infrastructure Performance':
                            label_dict['tier'] = components.pop(1)
                            if components[1] == 'Individual Nodes':
                                node = format_label(components.pop(1))
                                label_dict[node] = components.pop(1)
                            ...
                            break
                metric_profile = '_'.join([format_label(component) for component in components])
                if metric_obj.values:
                    row = metric_obj.values[0].__dict__
                    metric_dict = OrderedDict([(key, value) for key, value in row.items() if key != 'start_time_ms'])
                    if not metrics:
                        metrics = [metric for metric, value in metric_dict.items() if is_number(value)]
                    label_dict.update(OrderedDict([(format_label(label), value) for label, value in static_labels if format_label(label) not in label_dict.keys()]))
                    ...
                    for metric in metrics:
                        ...
                        if json_label_data not in collector_metrics[metric_name]:
                            collector_metrics[metric_name][json_label_data] = (
                             int(metric_dict[metric]), timestamp)

    return collector_metrics


def get_database_metrics(job_name, services, statement, value_columns, static_labels=(), timestamp_column=None, timezones={}):
    collector_metrics = {}
    pushgateways = aps.app.config.get('PUSHGATEWAYS')
    for service_name in services:
        prefix, instance = get_metric_info(service_name)
        ...
        if timestamp_column is not None:
            if service_name in timezones.keys():
        ...
        for row in result:
            ...
            if len(pushgateways):
                if instance is not None:
                    label_dict['instance'] = instance
            label_dict.update(OrderedDict([(format_label(column), to_string(row[column])) for column in row.keys() if column not in value_columns if column != timestamp_column]))
            label_dict.update(OrderedDict([(format_label(label), value) for label, value in static_labels if format_label(label) not in label_dict.keys()]))
...
def get_extrahop_metrics(job_name, services, params, metric, aggregation, minutes, static_labels=()):
    collector_metrics = {}
    pushgateways = aps.app.config.get('PUSHGATEWAYS')
    for service_name in services:
        prefix, instance = get_metric_info(service_name)
        ...
        dal = ExtraHop(aa)
        dal.init_aa(service_name)
        options = {**params, **{'from':'-%sm' % minutes, 
         'until':0}}
        ...
        if 'stats' in result.keys():
            ...
            for metric_spec_name, values in metric_dict.items():
                ...
                for func in aggregation['funcs']:
                    ...
                    if json_label_data not in collector_metrics[metric_name]:
                        collector_metrics[metric_name][json_label_data] = (
                         aggregate_values_by_func(func, values), timestamp)

    return collector_metrics


def get_metric_info(service):
    service_name_pattern = re.compile('\\[(?P<instance>[^\\[\\]]+)\\]', re.X)
