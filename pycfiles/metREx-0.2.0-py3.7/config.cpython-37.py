...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/app/main/config.py
...
if os.getenv('VCAP_SERVICES') is None:
    ...
    if isinstance(yaml_data, dict):
        vcap_services = {'user_provided': []}
        for service, credentials in yaml_data.items():
            if isinstance(credentials, dict):
                vcap_services['user_provided'].append({'name':service, 
                 'label':'user_provided', 
                 'credentials':credentials})

        os.environ['VCAP_SERVICES'] = json.dumps(vcap_services, separators=(', ', ': '), sort_keys=True)
app_prefix = 'METREX'
service_prefix = {'APIALCHEMY':os.getenv('API_PREFIX', app_prefix + '_API_'), 
...
class Config:
    ...
    SCHEDULER_EXECUTORS = {'default':{'type':'threadpool', 
    ...
    SCHEDULER_JOB_DEFAULTS = {'misfire_grace_time': int(os.getenv('MISFIRE_GRACE_TIME', '5'))}
    ...
    def apialchemy_binds(self, value):
        ...
        self.APIALCHEMY_BINDS = api_helper.build_bind_dict(self._apialchemy_binds, self.SECRET_KEY)
    ...
    def sqlalchemy_binds(self, value):
        ...
        self.SQLALCHEMY_BINDS = sqlalchemy_helper.build_bind_dict(self._sqlalchemy_binds, self.SECRET_KEY)
