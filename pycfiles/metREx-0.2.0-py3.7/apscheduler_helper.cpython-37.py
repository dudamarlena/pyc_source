...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/app/main/util/apscheduler_helper.py
# Compiled at: 2020-04-16 16:30:09
# Size of source mod 2**32: 11373 bytes
...
def build_job(category, name, bind, seconds, *args):
    return {'func':'app.main.service.metrics_service:generate_metrics', 
     'trigger':'interval', 
     'args':(
      category, name, bind) + args, 
     'id':name, 
     'seconds':seconds}
...
def build_job_list(jobs, apialchemy_info, sqlalchemy_info):
    ...
    service_name_pattern = re.compile('^(?:(?:' + re.escape(apialchemy_prefix) + ')|(?:' + re.escape(sqlalchemy_prefix) + '))(?P<name>.+)$', re.X)
    for job_name, credentials in jobs.items():
        if 'services' in credentials.keys():
            if 'interval_minutes' in credentials.keys():
                service_names = {}
                    ...
                    if vendor == 'appdynamics':
                        if 'application' in credentials.keys():
                            ...
                            if 'metric_path' in credentials.keys():
                                service_job_args = [credentials['metric_path'],
                                 credentials['interval_minutes']]
                elif vendor == 'extrahop' and 'metric_params' in credentials.keys() and 'metric_name' in credentials.keys():
                    if 'aggregation' in credentials.keys():
                        service_job_args = [
                         credentials['metric_params'],
                         credentials['metric_name'],
                         credentials['aggregation'],
                         credentials['interval_minutes']]
                elif service_job_args is not None:
                    job_args += service_job_args
                    if 'static_labels' in credentials.keys():
                        static_label_list = list(filter(None, re.split('\\s*,\\s*', credentials['static_labels'])))
                        job_args.append([tuple(filter(None, label_str.split(':'))) for label_str in static_label_list])
                    job_list.append(build_job(*job_args))
                    continue
                else:
                    if all((service_name in sqlalchemy_binds.keys() for service_name in service_names.values())):
                        if 'statement' in credentials.keys() and 'value_columns' in credentials.keys():
                            job_args = ['database',
                             ...
                             list(filter(None, re.split('\\s*,\\s*', credentials['value_columns'])))]
                            if 'static_labels' in credentials.keys():
                                static_label_list = list(filter(None, re.split('\\s*,\\s*', credentials['static_labels'])))
                                job_args.append([tuple(filter(None, label_str.split(':'))) for label_str in static_label_list])
...
def get_jobs_from_services(job_prefix, template_prefix, services):
    jobs = {}
    templates = {}
    job_name_pattern = re.compile('^' + re.escape(job_prefix) + '(?P<name>.+)$', re.X)
    template_name_pattern = re.compile('^' + re.escape(template_prefix) + '.+$', re.X)
                    ...
                    if components['vendor'] == 'github':
                        ...
                        if jobs_source_org is not None and jobs_source_repo is not None:
                            ...
                            if jobs_source_path is not None:
                                ...
                                contents = dal.service.get_file_contents(dal.client, jobs_source_org, jobs_source_repo, jobs_source_path, jobs_source_branch)
                                ...
                                if jobs_source_templates_path is not None:
                                    contents = dal.service.get_file_contents(dal.client, jobs_source_org, jobs_source_repo, jobs_source_templates_path, jobs_source_branch)
...
def populate_env_vars(string):
    undefined_env_vars = []
    env_var_pattern = re.compile('%%(?P<var>\\w+)%%', re.X)
    for m in env_var_pattern.finditer(string):
        ...
        if env_var is None:
            ...
            current_env_var_pattern = re.compile('%%' + re.escape(components['var']) + '%%', re.X)
            string = current_env_var_pattern.sub(env_var, string, 1)

    return (
     undefined_env_vars, string)