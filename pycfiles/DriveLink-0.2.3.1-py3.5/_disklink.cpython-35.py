...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/drivelink/_disklink.py
# Compiled at: 2017-09-17 22:36:49
# Size of source mod 2**32: 12260 bytes
try:
...
class Link(object):
    ...
    def __repr__(self):
        return self.__class__.__name__ + "('" + self._file_basename + "', " + str(self.size_limit) + ', ' + str(self.max_pages) + ", '" + self._file_loc + "')"
    ...
    def close(self):
        ...
        """
        if self is None or not hasattr(self, '_save_page_to_disk') or not hasattr(self, '_file_base') or self._file_base is None:
            return
