...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/dovetail/main.py
# Compiled at: 2012-08-13 23:01:00
...
from dovetail.util import InvalidTask, InvalidBuildFile, NoSuchTask, CommandLineException, Terminate, tree, get_virtual_environment
...
def report_tasks(module):
    ...
    children = [ Node(text=Task.find(task, module).report_line(full_name=False)) for task in build_module.tasks
               ]
    root = Node(children=children)
    tree(root, get_children=lambda x: x.children, node_text=lambda x: x.text, display_root=False)
...
def main(*kw):
    ...
    try:
        ...
        elif exception.message is not None:
