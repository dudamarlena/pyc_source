...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/dovetail/loader.py
# Compiled at: 2012-08-13 23:01:23
...
WARNING = '{0} is not being imported from under the build root. It will not participate in automatic dependency propagation'

class PackageNode():
    r"""A data structure for describing package structure and which
    ...
    def _validate():
        ...
        if PackageNode._root is None:
            ...
            while True:
                leaves = [ node for node in PackageNode._index_name.values() if node.is_leaf() and node.build_module is None
                         ]
                if leaves:
                    for leaf in leaves:
                        leaf.delete()
    ...
    def find_task(self, name):
        ...
        def _find_package(current_package, package_name):
            ...
            for sub in package_name.split('.'):
                ...
                if child is None:
                    ...
                    raise NoSuchTask(("There is no package '{0}'{1}. {2}").format(sub, package_msg, msg))
        ...
        def _find_task_in_package(package, task_name):
            ...
            if package.build_module is None:
                raise NoSuchTask(("Task '{0}' does not exist in '{1}' - this package was not loaded by dovetail.load()").format(name, package.full_name))
            try:
                ...
                raise NoSuchTask(("Task '{0}' does not exist in module '{1}'. {2}").format(task_name, package.full_name, msg))
...
class BuildLoader(object):
    ...
    def find_module(self, fullname, locations=None):
        ...
        if path.isfile(path.join(location, '__init__.py')):
            ...
            if path.isdir(location) and access(location, R_OK):
    ...
    for frame in stack():
        ...
        if not file_path.startswith(installed_at):
            if module_name == 'pkg_resources' and func_name == 'load':
                ...
                break
            if module_name == '__main__' and func_name == '<module>' and file_path.endswith('bootstrap.py'):
                Logger.debug('      Detected an invocation through virtual machine bootstrap')
                break
            if module_name == '__main__' and func_name == '<module>':
...
def load(file_path):
    r"""Load a build file which may be specified as a relative or absolute
    ...
    if not file_path is not None:
        raise AssertionError, 'load() was called with the file_path argument set to None'
        ...
        if access(request, R_OK) or relative:
    ...
    if BuildLoader.loaded():
        ...
        if rel.startswith('..'):
            if relative:
                raise InvalidBuildFile(('{0} called from {1} is not in or underneath the build root: {2}').format(file_path, build_module.file_name, root))
            else:
                raise InvalidBuildFile(('{0} is not in or underneath the build root: {1}').format(file_path, root))
    else:
