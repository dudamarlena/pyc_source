...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/dovetail/model.py
...
NOT_LOADED = "Task.find('{0}') was not called from a file loaded by Dovetail. Did you use dovetail.load()?"
...
class TaskWrapper(object):
    ...
    def __init__(self, name, func, before, after):
        ...
        try:
            ...
            TaskWrapper.WRAPPERS[self._task_name] = [
             self]
...
class Task(object):
    r"""An object representing a Task in a build system.
        ...
        if isinstance(name_or_function, basestring):
            ...
            if hasattr(name_or_function, '__call__'):
                ...
                try:
                    ...
                    raise NoSuchTask(('Function {0} in has not been registered as a Task. Did you decorate with @task?').format(name_or_function))
    ...
    def all_tasks():
        ...
        return {task for task in Task._tasks.itervalues() if not isinstance(task, NullTask) if not isinstance(task, NullTask)}
