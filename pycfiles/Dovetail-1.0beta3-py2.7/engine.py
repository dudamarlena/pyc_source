...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/dovetail/engine.py
# Compiled at: 2012-08-10 04:32:09
...
from dovetail.util import CircularTaskDependency, NoSuchDirectory, Logger, pp_exception, tree, NoSuchTask
...
STATE = enum(STARTED=0, DEPENDENTS=1, RUNNING=2, CALLING=3, SUCCEEDED=4, FAILED=5, SKIPPED=6, ABORTED=7, REPEATED=8)
...
class Result(object):
 ...
 L. 139        18  LOAD_GLOBAL           2  'isinstance'
               21  LOAD_FAST             1  'execution'
               24  LOAD_GLOBAL           3  'Execution'
               27  CALL_FUNCTION_2       2  None
               30  POP_JUMP_IF_TRUE     39  'to 39'
               33  LOAD_ASSERT              AssertionError
               36  RAISE_VARARGS_1       1  None

 L. 140        39  LOAD_FAST             1  'execution'
               42  LOAD_ATTR             4  'is_complete'
               45  CALL_FUNCTION_0       0  None
               48  POP_JUMP_IF_TRUE     57  'to 57'
               51  LOAD_ASSERT              AssertionError
               54  RAISE_VARARGS_1       1  None

 L. 141        57  LOAD_FAST             1  'execution'
               60  LOAD_ATTR             5  'task'
               63  LOAD_FAST             0  'self'
               66  STORE_ATTR            5  'task'

 L. 142        69  LOAD_FAST             1  'execution'
               72  LOAD_FAST             0  'self'
               75  STORE_ATTR            6  'execution'

 L. 143        78  LOAD_FAST             1  'execution'
               81  LOAD_ATTR             7  'result'
               84  LOAD_FAST             0  'self'
               87  STORE_ATTR            7  'result'

 L. 144        90  LOAD_FAST             1  'execution'
               93  LOAD_ATTR             8  'exception'
               96  LOAD_FAST             0  'self'
               99  STORE_ATTR            8  'exception'

 L. 145       102  LOAD_FAST             1  'execution'
              105  LOAD_ATTR             9  'state'
              108  LOAD_GLOBAL          10  'STATE'
              111  LOAD_ATTR            11  'SUCCEEDED'
              114  LOAD_GLOBAL          10  'STATE'
              117  LOAD_ATTR            12  'REPEATED'
              120  LOAD_GLOBAL          10  'STATE'
              123  LOAD_ATTR            13  'SKIPPED'
              126  BUILD_LIST_3          3 
              129  COMPARE_OP            6  in
              132  JUMP_IF_FALSE_OR_POP   147  'to 147'

 L. 146       135  LOAD_FAST             0  'self'
              138  LOAD_ATTR             8  'exception'
              141  LOAD_CONST               None
              144  COMPARE_OP            8  is
            147_0  COME_FROM           132  '132'
              147  LOAD_FAST             0  'self'
              150  STORE_ATTR           14  'success'

 L. 148       153  LOAD_FAST             0  'self'
              156  LOAD_ATTR            14  'success'
              159  POP_JUMP_IF_TRUE    216  'to 216'

 L. 149       162  LOAD_FAST             0  'self'
              165  LOAD_ATTR             8  'exception'
              168  LOAD_CONST               None
              171  COMPARE_OP            9  is-not
              174  POP_JUMP_IF_TRUE    186  'to 186'
              177  LOAD_ASSERT              AssertionError

 L. 150       180  LOAD_CONST               'The Result is not successful, but no exception has been set'
              183  RAISE_VARARGS_2       2  None

 L. 151       186  LOAD_GLOBAL           2  'isinstance'
              189  LOAD_FAST             0  'self'
              192  LOAD_ATTR             8  'exception'
              195  LOAD_GLOBAL          15  'BaseException'
              198  CALL_FUNCTION_2       2  None
              201  POP_JUMP_IF_TRUE    216  'to 216'
              204  LOAD_ASSERT              AssertionError

 L. 153       207  LOAD_CONST               'The Result is not successful and the object set as an Exception                      is not an instance of a subclass of BaseException'
              210  RAISE_VARARGS_2       2  None
...
Parse error at or near `LOAD_CONST' instruction at offset 216
    ...
    def report_tree(self):
        ...
        def execution_timing(execution):
            ...
            in this report"""
            return ('{0}\nElapsed: {1:.3f}s, internal: {2:.3f}').format(str(execution), execution.duration(), execution.duration_internal())

        print 'Task structure and results'
        tree(self.execution, lambda e: e.dependents, display_root=self.execution.task is not NULL_TASK, node_text=execution_timing)
    ...
    def banner(self):
        ...
        return Result.BANNER.format(now.isoformat(sep=' '), (', ').join(tasks), self.__str__(), node(), getuser(), python, path.dirname(BuildModule.root.file_name), path.basename(BuildModule.root.file_name), self.execution.count(), self.execution.duration(), self.format_exception())
    ...
    def __str__(self):
        if self.success:
            ...
            if failed:
                task = failed.task
                return ('FAILED\n    In file:  {0}\n    Task:     {1}\n    Message:  {2}').format(task.module.__file__, task.func_name, pp_exception(self.exception))
            return 'FAILED\n    No task ran'


class Execution(object):
    r"""A stack frame recording the internal and external aspects of
    ...
    def _log_transition(self, to_state):
        ...
        Logger.debug(('State transition: {0} {1} -> {2}').format(self.task, STATE.as_str(self.state), STATE.as_str(to_state)))
    ...
    def add_dependent(self, execution):
        ...
        assert self.state is STATE.STARTED or self.state is STATE.DEPENDENTS or self.state is STATE.RUNNING or self.state is STATE.CALLING
    ...
    def internal(self):
        ...
        assert self.state is STATE.STARTED or self.state is STATE.DEPENDENTS
    ...
    def fail(self, exception):
        ...
        if self.state is STATE.ABORTED:
            ...
            return
        assert self.state is STATE.STARTED or self.state is STATE.RUNNING or self.state is STATE.SKIPPED or self.state is STATE.CALLING
    ...
    def executions_by_duration(self):
        ...
        executions = sorted(self.flatten(), key=lambda execution: execution.duration_internal())
        executions.reverse()
        return executions
    ...
    def is_complete(self):
        ...
        :type:   boolean
        """
        return self.state is STATE.SKIPPED or self.state is STATE.SUCCEEDED or self.state is STATE.FAILED or self.state is STATE.ABORTED or self.state is STATE.REPEATED
    ...
    def is_ok(self):
        ...
        :type:   boolean
        """
        return self.state is STATE.SKIPPED or self.state is STATE.SUCCEEDED or self.state is STATE.REPEATED
        ...
        try:
            try:
                ...
                if execution.state not in [STATE.SKIPPED, STATE.FAILED]:
                    ...
                    if dependencies:
                        Logger.log(('Running dependencies: {0}').format((', ').join(d.name for d in dependencies)))
                    else:
                        Logger.debug('No dependencies')
                    for dependent in dependencies:
                        if not dependent is not None:
                            raise AssertionError
                            Logger.debug(('{0} needs {1}').format(task, dependent))
                            execute_dep = self._execute(dependent)
                            execute_dep.is_ok() or Logger.log(('Dependency failed: {0}').format(execute_dep))
                            execution.abort()
                            break
                ...
                Logger.log(('{0} Time: Elapsed={1:.3f}s, internal={2:.3f}').format(task, execution.duration(), execution.duration_internal()))
            except Exception as exception:
