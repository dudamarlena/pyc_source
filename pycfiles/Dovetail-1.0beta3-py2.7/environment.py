...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/dovetail/directives/environment.py
...
def apply_environment(environment):
    ...
    :param environment: A :ref:`modification_dictionary` that will be applied                        to :obj:`os.environ`.
...
def adjust_env(*remove, **merge):
    ...
    :type remove:  string
    :param merge:  List of environment variables to set, the argument name being                   mapped to the environment variable name
    :type merge:   string
    ...
    assert not any((k, v) for k, v in merge.iteritems() if v is not None and not isinstance(v, basestring))
    assert not any(k for k in remove if not isinstance(k, basestring))
...
class Env(object):
    ...
    def __init__(self, *if_set, **if_equals):
        assert not any((k, v) for k, v in if_equals.iteritems() if v is not None and not isinstance(v, basestring))
        assert not any(k for k in if_set if not isinstance(k, basestring))
        ...
        for key, value in self.tests.iteritems():
            if value is None:
                if environ.has_key(key):
                    ...
                    return False
            elif environ.has_key(key) and environ[key] == value:
    ...
    def _pp_test_env(self):
        ...
        if been_unset:
            ...
            been_unset = None
        been_set = [ ('${0}="{1}"').format(k, v) for k, v in self.tests.iteritems() if v is not None
                   ]
        if been_set:
...
class EnvRE(object):
    r"""A Predicate for testing environment variables against regular
    expressions.

    :param tests: List of NAME=RE arguments where NAME is the environment                  variable to be checked, and RE is the regular expression
    :type tests: string
    :rtype: boolean
    ...
    def __init__(self, **tests):
        assert len(tests) > 0
        assert not any((k, v) for k, v in tests.iteritems() if v is not None and not isinstance(v, basestring))
        self.tests = tests
        self.compiled = {}
        for key, expression in tests.iteritems():
            self.compiled[key] = re.compile(expression)

        description = [ ('${0}=~"{1}"').format(k, v) for k, v in self.tests.iteritems() if v is not None
                      ]
    ...
    def __call__(self):
        for key, expression in self.compiled.iteritems():
            if environ.has_key(key):
                ...
                Logger.debug(('EnvRE on ${0}: "{1}"=~"{2}"? {3}').format(key, value, self.tests[key], hits is not None))
                if hits is None:
                    return False
            else:
