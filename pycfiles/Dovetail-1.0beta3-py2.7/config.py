...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/dovetail/config.py
...
def create_parser():
    ...
    virtual.add_argument('-e', '--virtualenv', action='store', help='Location of a virtualenv environment to use. Will be created if it does not exist.')
    virtual.add_argument('--clear', action='store_true', default=False, help='Recreates (clears) the virtual environment every time, even if it exists')
    execution.add_argument('-f', dest='build_file', action='store', default='build.py', help='Python file containing build plan, default: build.py')
    execution.add_argument('task', action='store', nargs='*', help='One or more Tasks to build. If none, a list of Tasks is presented')
    level = parser.add_mutually_exclusive_group()
    level.add_argument('-q', dest='loglevel', action='store_const', const='ERROR', help='Dovetail will report only output from Tasks and its own errors')
    level.add_argument('-v', dest='loglevel', action='store_const', const='INFO', help='Report more information about the decisions in the build')
    level.add_argument('-vv', dest='loglevel', action='store_const', const='DEBUG', help='Debug output from Dovetail')
    nesting = parser.add_mutually_exclusive_group()
    nesting.add_argument('-n', '--nested', dest='nested', action='store_true', default=False, help='Presents output in a structured format')
    nesting.add_argument('-nn', '--no-nest', dest='nested', action='store_false', help='Switches off nesting (if set in a configuration file)')
    report = parser.add_argument_group('Reports')
    report.add_argument('-r', dest='reports', action='append', default=None, choices=[ name.lower() for name in REPORTS.names() ], help='Run additional reports after the build')
    info = parser.add_argument_group('Information')
    info.add_argument('--version', action='version', version='%(prog)s ' + VERSION + ' http://www.aviser.asia/dovetail')
    info.add_argument('--usage', action='store_true', default=False, help='Display usage information')
    info.add_argument('-h', '--help', action='store_true', default=False, help='Display this help. If a task is specified, displays help on the task.')
    info.add_argument('--license', action='store_true', help=SUPPRESS)
    info.add_argument('--warranty', action='store_true', help=SUPPRESS)
    return parser
...
def is_a_boolean(boolean_as_string):
    ...
    """
    if boolean_as_string.lower() not in BOOLEAN:
