...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/dovetail/util/utilities.py
...
def stamp_environment():
    ...
    os.environ['DOVETAIL_MACHINE'] = platform.machine()
    os.environ['DOVETAIL_PROCESSOR'] = platform.processor()
    os.environ['DOVETAIL_OS_NAME'] = os.name
    os.environ['DOVETAIL_SYSTEM'] = platform.system()
    os.environ['DOVETAIL_RELEASE'] = platform.release()
    ...
    os.environ['DOVETAIL_NODE'] = platform.node()
    os.environ['DOVETAIL_USER'] = getuser()
    if get_virtual_environment():
        os.environ['DOVETAIL_VIRTUALENV_SLAVE'] = get_virtual_environment()


def tree(root, get_children, node_text=str, display_root=True, indent=1, separator=1, node_style=False, margin='  '):
...
def call(arguments, shell=False, stdout=None, stdout_mode='w', stdout_buffer=-1, stderr=None, stderr_mode='w', stderr_buffer=-1):
