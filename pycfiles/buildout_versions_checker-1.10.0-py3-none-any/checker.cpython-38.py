...
class VersionsChecker(object):
    ...
    def fetch_last_versions(self, packages, allow_pre_releases, service_url, timeout, threads):
        """
        ...
        if threads > 1:
            with futures.ThreadPoolExecutor(max_workers=threads) as (executor):
                tasks = [executor.submit(self.fetch_last_version, package, allow_pre_releases, service_url, timeout) for package in packages]
                for task in futures.as_completed(tasks):
                    versions.append(task.result())

        else:
            for package in packages:
                versions.append(self.fetch_last_version(package, allow_pre_releases, service_url, timeout))
            else:
                return versions
        ...
        try:
            ...
            for version in specifier.filter(results['releases']):
                ...
                if version > max_version:
                    max_version = version
                logger.debug('-> Last version of %s%s is %s.', package, specifier, max_version)
                return (
    ...
    def find_updates(self, versions, last_versions):
        ...
        for package, current_version in versions.items():
            ...
            if last_version != current_version:
                logger.debug('=> %s current version (%s) and last version (%s) are different.', package, current_version, last_version)
                updates.append((
