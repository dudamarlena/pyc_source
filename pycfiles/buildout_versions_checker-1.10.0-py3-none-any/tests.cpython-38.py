...
class FindUnusedVersionsTestCase(LogsTestCase, StdOutTestCase, StubbedListDirTestCase):
    ...
    def test_write(self):
        ...
        self.assertLogs(info=[
         ...
         '- Unused-egg is unused.'])
        self.assertStdOut('- Unused-egg is unused.\n')
        self.assertEquals(config_file.read().decode('utf-8'), '[versions]\nEgg = 1.0\n')
    ...
    def test_no_source(self):
        ...
        self.assertLogs(warning=[
         "'versions.cfg' cannot be read."],
          info=[
         '- 0 packages need to be checked for updates.'])
    ...
    def test_output_max(self):
        ...
        self.assertLogs(info=[
         ...
         '- 2 packages need to be checked for updates.'],
          warning=[
         '- Unused-egg is unused.'])
        self.assertStdOut('- 2 versions found in %s.\n- 2 packages need to be checked for updates.\n- Unused-egg is unused.\n' % config_file.name)
        config_file.close()
    ...
    def test_handle_error(self):
        ...
        self.assertEqual(context.exception.code, "'int' object is not iterable")
...
class IndentCommandLineTestCase(LogsTestCase, StdOutTestCase):
    ...
    def test_simple(self):
        ...
        self.assertEqual(context.exception.code, 0)
        self.assertLogs(warning=[
         '- %s (re)indented at 4 spaces.' % config_file.name])
        self.assertStdOut('- %s (re)indented at 4 spaces.\n' % config_file.name)
        self.assertEquals(config_file.read().decode('utf-8'), '[sections]\nKey = Value\n')
        config_file.close()
    ...
    def test_indentation(self):
        ...
        self.assertEqual(context.exception.code, 0)
        self.assertLogs(warning=[
         '- %s (re)indented at 8 spaces.' % config_file.name])
        self.assertStdOut('- %s (re)indented at 8 spaces.\n' % config_file.name)
        self.assertEquals(config_file.read().decode('utf-8'), '[sections]\nKey     = Value\n')
        config_file.close()
...
class CheckUpdatesCommandLineTestCase(LogsTestCase, StdOutTestCase, StubbedURLOpenTestCase):
    ...
    def test_write_include_in_blank(self):
        config_file = NamedTemporaryFile()
        with self.assertRaises(SystemExit) as (context):
            check_buildout_updates.cmdline('-i egg -w %s' % config_file.name)
        self.assertEqual(context.exception.code, 0)
        config_file.seek(0)
        self.assertEquals(config_file.read().decode('utf-8'), '[versions]\negg = 0.3\n')
    ...
    def test_write_include_in_blank_with_indentation(self):
        config_file = NamedTemporaryFile()
        with self.assertRaises(SystemExit) as (context):
            check_buildout_updates.cmdline('-i egg --indent 8 -w %s' % config_file.name)
        self.assertEqual(context.exception.code, 0)
        config_file.seek(0)
        self.assertEquals(config_file.read().decode('utf-8'), '[versions]\negg     = 0.3\n')
        self.assertStdOut('[versions]\negg     = 0.3        #  0.0.0\n')
    ...
    def test_write_in_existing_file_with_exclude(self):
        config_file = NamedTemporaryFile()
        config_file.write('[buildout]\ndevelop=.\n[versions]\nexcluded=1.0\negg=0.1'.encode('utf-8'))
        config_file.seek(0)
        with self.assertRaises(SystemExit) as (context):
            check_buildout_updates.cmdline('-e excluded -w %s' % config_file.name)
        self.assertEqual(context.exception.code, 0)
        self.assertLogs(debug=[
         '-> Last version of egg is 0.3.',
         '=> egg current version (0.1) and last version (0.3) are different.'],
          info=[
         '- 2 versions found in %s.' % config_file.name,
         '- 1 packages need to be checked for updates.',
         '> Fetching latest datas for egg...',
         ...
         'egg = 0.3          #  0.1'])
        config_file.seek(0)
        self.assertEquals(config_file.read().decode('utf-8'), '[buildout]\ndevelop     = .\n\n[versions]\nexcluded    = 1.0\negg         = 0.3\n')
        self.assertStdOut('[versions]\negg = 0.3          #  0.1\n')
    ...
    def test_specifiers_errors(self):
        ...
        self.assertInStdOut('error: argument -s/--specifier: key:value syntax not followed')
        with self.assertRaises(SystemExit) as (context):
            check_buildout_updates.cmdline('-i egg -s egg:')
        self.assertEqual(context.exception.code, 2)
        self.assertInStdOut('error: argument -s/--specifier: key or value are empty')

    def test_handle_error(self):
        with self.assertRaises(SystemExit) as (context):
            check_buildout_updates.cmdline('-i error-egg')
        self.assertTrue(context.exception.code.startswith('list indices must be integers'))
