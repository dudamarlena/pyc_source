...
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/abseqPy/IgRepertoire/IgRepertoire.py
# Compiled at: 2019-06-21 04:18:28
__doc__ = '\n    Short description: Quality Control Analysis of Immunoglobulin Repertoire NGS (Paired-End MiSeq)    \n    Author: Monther Alhamdoosh    \n    Python Version: 2.7\n    Changes log: check git commits. \n'
from __future__ import division
import gc, os, logging, sys, inspect
...
from abseqPy.IgRepAuxiliary.upstreamAuxiliary import plotUpstreamLenDist, extractUpstreamSeqs, findUpstreamMotifs
...
from abseqPy.IgRepertoire.igRepUtils import gunzip, fastq2fasta, mergeReads, writeListToFile, writeSummary, createIfNot, detectFileFormat, countSeqs
...
from abseqPy.IgRepAuxiliary.diversityAuxiliary import annotateSpectratypes, annotateClonotypes
...
class IgRepertoire:
    ...
    def __init__(self, f1, f2=None, name=None, fmt=None, chain='hv', seqtype='dna', domainSystem='imgt', merger=DEFAULT_MERGER, outdir='.', threads=1, bitscore=(0, Inf), alignlen=(0, Inf), sstart=(
 1, Inf), qstart=(1, Inf), clonelimit=DEFAULT_TOP_CLONE_VALUE, detailedComposition=False, actualqstart=-1, trim5=0, trim3=0, fr4cut=True, primer5endoffset=0, primer5end=None, primer3end=None, upstream=None, sites=None, database='$IGBLASTDB', task=DEFAULT_TASK, log=None, yaml=None):
        """
    ...
    def runFastqc(self):
        ...
        if self.format == 'fasta':
            ...
            if not os.path.isdir(outDir):
                os.makedirs(outDir)
            filename = os.path.join(outDir, self.readFile1.split(os.path.sep)[(-1)].replace('.fastq', '').replace('.gz', '') + '_fastqc.html')
            if os.path.exists(filename):
                printto(logger, 'fastqc was already performed on this library.', LEVEL.WARN)
                return
            printto(logger, 'Fastqc is running ... ')
            fastqc = ShortOpts(exe=FASTQC, o=outDir, t=self.threads).append(quote(self.readFile1) + ' ' + (quote(self.readFile2) if self.readFile2 is not None else ''))
            fastqc()
    ...
    def mergePairedReads(self):
        logger = logging.getLogger(self.name)
        if self.merge:
            mergedFastq = mergeReads(self.readFile1, self.readFile2, self.threads, self.merger, self.hdfDir, stream=logger)
    ...
    def annotateClones(self, filterOutDir=None, inplaceProductive=False):
        ...
        if os.path.exists(cloneAnnotFile):
            if self.task == 'annotate':
                printto(logger, '\tClones annotation file found and no further work needed ... ' + os.path.basename(cloneAnnotFile))
                return
            printto(logger, '\tClones annotation file found and being loaded ... ' + os.path.basename(cloneAnnotFile))
            ...
            self.cloneAnnot, filteredIDs = annotateIGSeqRead(readFasta, self.chain, self.db, self.threads, self.seqsPerFile, self.seqType, outdir=outHdfDir, domainSystem=self.domainSystem, stream=logger)
            sys.stdout.flush()
            gc.collect()
            if len(filteredIDs):
                writeListToFile(filteredIDs, os.path.join(outHdfDir, self.name + '_unmapped_clones.txt'))
            printto(logger, '\tClones annotation file is being written to ' + os.path.basename(cloneAnnotFile))
            self.cloneAnnot.to_hdf(cloneAnnotFile, 'cloneAnnot', mode='w')
            paramFile = writeParams(self.args, outResDir)
            printto(logger, 'The analysis parameters have been written to ' + paramFile)
            writeSummary(self._summaryFile, 'AnnotatedReads', self.cloneAnnot.shape[0])
        printto(logger, ('Number of clones that are annotated is {0:,}').format(int(self.cloneAnnot.shape[0])), LEVEL.INFO)
        if self.cloneAnnot.shape[0] <= 0:
            ...
            if len(filteredIDs) > 0:
                ...
                filteredIDs.to_csv(os.path.join(filterOutDir, self.name + '_filtered_out_clones.txt'), sep='\t', header=True, index=True)
            retained = int(self.cloneAnnot.shape[0]) - len(filteredIDs)
            printto(logger, ('Percentage of retained clones is {:.2%} ({:,}/{:,})').format(retained / self.cloneAnnot.shape[0], retained, int(self.cloneAnnot.shape[0])), LEVEL.INFO)
            ...
            if not eitherExists(outputFile) or not eitherExists(noOutlierOutputFile):
                ...
                plotSeqLenDist(count, self.name, outputFile, self.format, maxbins=40, histtype='bar', removeOutliers=False, normed=True, stream=logger)
                plotSeqLenDist(count, self.name, noOutlierOutputFile, self.format, maxbins=40, histtype='bar', removeOutliers=True, normed=True, stream=logger)
            else:
            ...
            if inplaceProductive:
                self.cloneAnnot = self.cloneAnnot[((self.cloneAnnot['stopcodon'] == 'No') & (self.cloneAnnot['v-jframe'] == 'In-frame'))]
    ...
    def analyzeProductivity(self, inplaceProductive=True, inplaceFiltered=True):
        ...
        if not os.path.exists(refinedCloneAnnotFile) or not os.path.exists(cloneSeqFile):
            ...
            if self.cloneAnnot.shape[0] > 0:
                self._reloadAnnot()
                self.cloneAnnot, self.cloneSeqs = refineClonesAnnotation(outHdfDir, self.name, self.cloneAnnot, self.readFile, self.format, self.actualQstart, self.chain, self.fr4cut, self.trim5End, self.trim3End, self.seqsPerFile, self.threads, stream=logger)
                gc.collect()
                printto(logger, 'The refined clone annotation file is being written to ' + os.path.basename(refinedCloneAnnotFile))
            ...
            printto(logger, 'The refined clone annotation files were found and being loaded ... ' + os.path.basename(refinedCloneAnnotFile))
        ...
        if before > 0:
            ...
            printto(logger, ('\tPercentage of retained clones is {:.2%} ({:,}/{:,})').format(self.cloneAnnot.shape[0] / before, self.cloneAnnot.shape[0], before))
            writeSummary(self._summaryFile, 'FilteredReads', self.cloneAnnot.shape[0])
            ...
            printto(logger, ('Percentage of productive clones {:.2%} ({:,}/{:,})').format(0 if before == 0 else cloneAnnot.shape[0] / before, int(cloneAnnot.shape[0]), int(before)), LEVEL.INFO)
    ...
    def analyzeDiversity(self):
        ...
        if self.cloneAnnot.shape[0] <= 0:
            printto(logger, ('There are no productive sequences found (post-refinement) in {},').format(self.name) + ' skipping diversity analysis.', LEVEL.WARN)
            return
        else:
            createIfNot(outResDir)
            createIfNot(outHdfDir)
            ...
            generateDiversityReport(spectraTypes, clonoTypes, self.name, outResDir, self.clonelimit, threads=self.threads, segregate=self.detailedComposition, stream=logger)
            paramFile = writeParams(self.args, outResDir)
            printto(logger, 'The analysis parameters have been written to ' + paramFile)
            return
    ...
    def analyzeRestrictionSites(self, simple):
        ...
        if not os.path.isdir(outHdfDir):
            ...
            printto(logger, "WARNING: remove the 'restriction_sites' directory if you changed the filtering criteria.", LEVEL.WARN)
        siteHitsFile = os.path.join(outResDir, self.name + ('_{}_rsa{}.csv').format(os.path.splitext(os.path.basename(self.sitesFile))[0], ssimple))
        overlap2File = siteHitsFile.replace('.csv', '_overlap_order2.csv')
        if os.path.exists(siteHitsFile):
            printto(logger, ('Restriction sites were already scanned at {}').format(os.path.basename(siteHitsFile)) + (', skipping RSA({}) ...').format(ssimple), LEVEL.WARN)
            return
        else:
            printto(logger, 'Searching for cloneAnnot dataframe, filtering for FR consensus lengths and productive reads only', LEVEL.INFO)
            if simple:
            ...
            if self.readFile is None:
                self.mergePairedReads()
            if self.cloneAnnot.shape[0] > 0:
                rsaResults, overlapResults = scanRestrictionSites(self.name, self.readFile, self.cloneAnnot, self.sitesFile, self.threads, simple=simple, stream=logger)
                ...
                return
            generateOverlapFigures(overlapResults, rsaResults.loc[(rsaResults.shape[0] - 1, 'No.Molecules')], self.name, siteHitsFile, stream=logger)
            paramFile = writeParams(self.args, outResDir)
            printto(logger, 'The analysis parameters have been written to ' + paramFile)
            return
    ...
    def analyzeSecretionSignal(self):
        ...
        if not os.path.exists(outHdfDir):
            ...
            printto(logger, "WARNING: Remove 'secretion' directory if you've changed the filtering criteria.", LEVEL.WARN)
        if self.cloneAnnot is None:
            self.annotateClones(outHdfDir)
        if self.cloneAnnot.shape[0] <= 0:
            ...
            upstreamFile = os.path.join(outHdfDir, self.name + ('_secsig_{:.0f}_{:.0f}.fasta').format(self.upstream[0], self.upstream[1]))
            if not os.path.exists(upstreamFile):
                ...
                printto(logger, ('\tUpstream sequences file {} was found! Loading file ... ').format(os.path.basename(upstreamFile)), LEVEL.WARN)
            ...
            if expectLength != Inf:
                ...
                for level in ['variant', 'gene', 'family']:
                    findUpstreamMotifs(upstreamFile, self.name, outHdfDir, outResDir, [
                     expectLength, expectLength], level=level, startCodon=True, threads=self.threads, stream=logger)

                printto(logger, '\tAnalyzing trimmed secretion signals', LEVEL.DEBUG)
                for level in ['variant', 'gene', 'family']:
                    findUpstreamMotifs(upstreamFile, self.name, outHdfDir, outResDir, [1, expectLength - 1], level=level, startCodon=True, threads=self.threads, stream=logger)
    ...
    def analyze5UTR(self):
        ...
        if not os.path.exists(outHdfDir):
            ...
            printto(logger, "WARNING: Remove 'utr5' directory if you've changed the filtering criteria", LEVEL.WARN)
        if self.cloneAnnot is None:
            self.annotateClones(outHdfDir)
        if self.cloneAnnot.shape[0] <= 0:
            ...
            printto(logger, 'The diversity of the upstream of IGV genes is being analyzed ... ')
            upstreamFile = os.path.join(outHdfDir, self.name + ('_5utr_{:.0f}_{:.0f}.fasta').format(self.upstream[0], self.upstream[1]))
            if not os.path.exists(upstreamFile):
                extractUpstreamSeqs(self.cloneAnnot, self.readFile, self.upstream, upstreamFile, stream=logger)
            else:
                printto(logger, ('\tUpstream sequences file {} was found! Loading file... ').format(os.path.basename(upstreamFile)), LEVEL.WARN)
            ...
            if expectLength != Inf:
                for level in ['variant', 'gene', 'family']:
                    findUpstreamMotifs(upstreamFile, self.name, outHdfDir, outResDir, [expectLength, expectLength], level=level, startCodon=True, type='5utr', clusterMotifs=True, threads=self.threads, stream=logger)
    ...
    def analyzePrimerSpecificity(self):
        ...
        if not os.path.exists(primerAnnotFile):
            ...
            if self.cloneAnnot.shape[0] <= 0:
                ...
                return
            self._reloadAnnot()
            self.cloneAnnot = addPrimerData(self.cloneAnnot, self.readFile, self.format, self.fr4cut, self.trim5End, self.trim3End, self.actualQstart, self.end5, self.end3, self.end5offset, self.threads, stream=logger)
    ...
    def analyzeSeqLen(self, klass=False):
        ...
        if not not klass:
            ...
            os.path.exists(outResdir) or os.makedirs(outResdir)
        if self.readFile is None:
            self.mergePairedReads()
        if klass:
            ...
            plotSeqLenDist(self.readFile, self.name, outputFile, self.format, removeOutliers=True, maxbins=-1, stream=logger)
        paramFile = writeParams(self.args, outResdir)
        printto(logger, 'The analysis parameters have been written to ' + paramFile)
        return
    ...
    def _cloneAnnotFilteredRows(self, logger):
        ...
        selectedRows = (self.cloneAnnot['bitscore'] >= self.bitScore[0]) & (self.cloneAnnot['bitscore'] <= self.bitScore[1]) & (self.cloneAnnot['alignlen'] >= self.alignLen[0]) & (self.cloneAnnot['alignlen'] <= self.alignLen[1]) & (self.cloneAnnot['vstart'] >= self.sStart[0]) & (self.cloneAnnot['vstart'] <= self.sStart[1]) & (self.cloneAnnot['vqstart'] >= self.qStart[0]) & (self.cloneAnnot['vqstart'] <= self.qStart[1])
        return selectedRows
    ...
    def _getBestCloneAnnot(self, outHdfDir, inplaceFiltered, inplaceProductive, stream=None):
        ...
        if os.path.exists(refinedCloneAnnotFile):
            printto(stream, ('Found refined clone annotation file {}, loading dataframe ...').format(os.path.basename(refinedCloneAnnotFile)))
            ...
            printto(stream, 'Refined clone annotation file not found, falling back to unrefined clone annotation dataframe.')
            printto(stream, '\tFiltered for:')
            printto(stream, ('\t\tFR1, FR2, FR3, FR4 consensus lengths: {}').format(str(inplaceFiltered)))
    ...
    def _setupTasks(self):
        ...
        if self.task != 'primer' and (self.end3 or self.end5):
            printto(logger, ('Primer file detected, will conduct primer specificity analysis in addition to {} ... ').format(self.task), LEVEL.INFO)
            todo.append(AbSeqWorker.PRIM)
        return todo[::-1]
