...
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/abseqPy/utilities.py
# Compiled at: 2019-04-26 02:31:12
from __future__ import print_function
import os, sys, subprocess, shlex, platform, functools, importlib
...
class CommandLine:
    ...
    def __call__(self, stdout=sys.stdout, stderr=sys.stderr):
        ...
        if sys.version_info[0] == 2:
            ...
            subprocess.check_call(shlex.split(str(self).encode('unicode_escape').decode()), stdout=stdout, stderr=stderr)
        if closeOut:
            stdout.close()
        if closeErr:
            stderr.close()
    ...
    def __repr__(self):
        ...
        """
        return (' ').join([
         str(self._exe)] + [ str(k) for k in self._args ] + [ self._dash() + str(k) + (' ' + str(v) if v else '') for k, v in self._kwargs.items()
                                                            ]) + self._ext
...
def disableFor(operatingSys):
    ...
    if isinstance(operatingSys, str):
        ...
        if isinstance(operatingSys, list) or isinstance(operatingSys, tuple):
        ...
        for s in systems:
            if s.lower() not in OPS:
                raise ValueError(str(operatingSys) + ' is not a valid operating system name. Only Win, Lin, and Mac is supported')
    ...
    def _decorator(func):
        ...
        def _call(*args, **kwargs):
            if platform.system() in unsupported:
                raise NotImplementedError(('Sorry, {} is not implemented for {}.').format(func.__name__, platform.system()))
...
def requires(package, fatal=False, stderr=sys.stderr):
    ...
    def _decorator(func):
        ...
        def _call(*args, **kwargs):
            if _hasPackage():
                return func(*args, **kwargs)
            print(("one of '{}' cannot be found in your python path. Skipping '{}' function call which depends on it.").format(package, func.__name__), file=stderr)
