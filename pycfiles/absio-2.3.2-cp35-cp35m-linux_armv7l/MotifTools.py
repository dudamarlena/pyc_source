...
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/TAMO/MotifTools.py
...
import TAMO.paths
COPY = copy
one2two = {'W': 'AT', 'M': 'AC', 'R': 'AG', 'S': 'CG', 
   'Y': 'CT', 'K': 'GT'}
two2one = {'AT': 'W', 'AC': 'M', 'AG': 'R', 'CG': 'S', 
   'CT': 'Y', 'GT': 'K'}
revcomp = {'A': 'T', 'T': 'A', 'C': 'G', 'G': 'C', 'W': 'W', 
   'S': 'S', 'K': 'M', 'M': 'K', 'Y': 'R', 
   'R': 'Y', 'N': 'N', 'B': 'N', 
   'D': 'N', 'H': 'N', 'V': 'N', ' ': 'N'}
ACGT = list('ACGT')
...
def Motif_from_counts(countmat, beta=0.01, bg={'A': 0.25, 'C': 0.25, 'G': 0.25, 'T': 0.25}, id=None):
    """
...
class Motif():
    ...
    def __init__(self, list_of_seqs_or_text=[], backgroundD=None, id=None):
        self.id = id
        self.MAP = 0
        self.evalue = None
        self.oneletter = ''
        ...
        if backgroundD:
            ...
            self.background = {'A': 0.31, 'C': 0.19, 'G': 0.19, 'T': 0.31}
    ...
    def summary(self):
        ...
        if m.binomial != None:
            txt = txt + '  Bi: %6.2f' % nlog10(m.binomial)
        if m.church != None:
            txt = txt + '  ch: %6.2f' % nlog10(m.church)
        if m.frac != None:
            txt = txt + '  f: %5.3f' % m.frac
        if m.E_site != None:
            txt = txt + '  Es: %6.2f' % nlog10(m.E_site)
        if m.E_seq != None:
            txt = txt + '  Eq: %6.2f' % nlog10(m.E_seq)
        if m.MNCP != None:
            txt = txt + '  mn: %6.2f' % m.MNCP
        if m.ROC_auc != None:
            txt = txt + '  Ra: %6.4f' % m.ROC_auc
        if m.E_chi2 != None:
            if m.E_chi2 == 0:
                m.E_chi2 = 1e-20
            txt = txt + ' x2: %5.2f' % nlog10(m.E_chi2)
        if m.CRA != None:
            txt = txt + '  cR: %6.4f' % m.CRA
        if m.Cfrac != None:
            txt = txt + '  Cf: %5.3f' % m.Cfrac
        if m.realpvalue != None:
            txt = txt + '  P: %6.4e' % m.realpvalue
        if m.kellis != None:
            txt = txt + '  k: %6.2f' % m.kellis
        if m.numbound:
            txt = txt + '  b: %3d' % m.numbound
        if m.nummotif:
            txt = txt + '  nG: %3d' % m.nummotif
        if m.numboundmotif:
            txt = txt + '  bn: %3d' % m.numboundmotif
        return txt
    ...
    def _compute_ll(self):
        ...
        for i in range(self.width):
            ...
            for key in self.counts[i].keys():
                ...
                Df[key] = Pij
                temp1 = (self.nseqs + self.bgscale) * self.background[key]
                temp = (self.counts[i][key] + self.bgscale * self.background[key]) / temp1
                if temp == 0:
                    print (
                     self.counts[i][key], self.bgscale, self.background[key])
                Dll[key] = math.log(temp) / math.log(2)
                if Pij > 0:
    ...
    def compute_from_nmer(self, nmer, beta=0.001):
        """
    ...
    def _compute_logP_from_ll(self):
        ...
        for i in range(self.width):
            D = {}
            for L in ACGT:
                D[L] = self.ll[i][L] + log(self.background[L]) / log(2.0)
    ...
    def scansum(self, seq, threshold=-1000):
        ...
        return (
         total, hits, ave, math.log(etotal))
    ...
    def _scan(self, seq, threshold='', forw_only='', factor=0.7):
        ...
        ll = self.ll
        if not threshold:
            threshold = factor * self.maxscore
        matches = []
        endpoints = []
        scores = []
        width = self.width
        ...
        seq = seq.upper()
        seqcomp = seq.translate(revcompTBL)
        for offset in range(len(seq) - self.width + 1):
            total_f = 0
            total_r = 0
            for i, ir in width_ranges:
                pos = offset + i
    ...
    def _scan_smaller(self, seq, threshold=''):
        ...
        oseq = seq
        seq = seq.upper()
        for offset in range(self.width - len(seq) + 1):
            maximum = 0
    ...
    def maxdiff(self):
        ...
        for i in range(self.width):
            ...
            for L in ACGT:
                if L == _minL:
                    delta = 1 - POW(2, self.logP[i][L])
                    D = D + delta * delta
                else:
                    D = D + POW(POW(2, self.logP[i][L]), 2)
    ...
    def trimmed(self, thresh=0.1):
        ...
        for start in range(0, self.width - 1):
            if self.bits[start] >= thresh:
                break

        for stop in range(self.width, 1, -1):
            if self.bits[(stop - 1)] >= thresh:
                break
    ...
    def random_kmer(self):
        ...
        """
        if not self._bestseqs:
            self._bestseqs = self.bestseqs()
        seqs = self._bestseqs
    ...
    if diffmethod == 'diff':
        ...
        for B in [Borig, Borig.revcomp()]:
            for start in range(0, lastA + 1):
                ...
                if switch:
                    _diff = diff_fcn(subB, subA)
                else:
                    _diff = diff_fcn(subA, subB)
                Dmin = min(Dmin, _diff)
...
def bestseqs(motif, thresh, seq='', score=0, depth=0, bestcomplete=None, SEQS=[]):
    ...
    if score + bestcomplete[depth] < thresh:
        return
    for L in ACGT:
        newseq = seq + L
...
def compare_seqs(s1, s2):
    ...
    return (
     maxfrac, short, long[max_i:max_i + len(short)])
...
def submotif(self, beg, end):
    ...
    for i in range(start, stop):
        D = {}
        for L in ACGT:
            D[L] = math.pow(2.0, self.logP[i][L])
...
def shuffledP(self):
    ...
    for i in range(0, self.width):
        ...
        for L, _L in zip(ACGT, _s):
            D[L] = math.pow(2.0, self.logP[i][_L])
...
def revcompmotif(self):
    ...
    for i in range(self.width):
        D = {}
        for L in ACGT:
            D[L] = math.pow(2.0, self.logP[(self.width - i - 1)][revcomp[L]])
...
def sum_counts(motifs, weights=[]):
    C = []
    for c in motifs[0].counts:
        D = {}
        for L in ACGT:
            D[L] = 0.0
    ...
    for m in motifs:
        for i in range(m.width):
            for L in ACGT:
                C[i][L] = C[i][L] + m.counts[i][L]

    motif = Motif_from_counts(C, 0.0, bg=motifs[0].background, id=('|').join([ m.id for m in motifs ]))
    if m.nseqs == 1:
        print (
         motif.id, motif)
        print [ str(m) for m in motifs ]
        print C
        sys.exit()
...
def sum(motifs, weights=[]):
    ...
    if motif.nseqs < 1:
        print (
         motif.id, motif)
        print [ m.id + ' ' + str(m) for m in motifs ]
        print C
    return motif.trimmed()
...
def giflogo(motif, id, title=None, scale=0.8):
    ...
    """
    SEQLOGO = TAMO.paths.weblogodir + 'seqlogo'
    TAMO.paths.CHECK(SEQLOGO, '', 'Weblogo/Seqlogo')
    ...
    cmd = '%s -F GIF -acpY -w%d -h%d -k 1 -F GIF -M -f %s -o %s -t "%s" ' % (
     SEQLOGO, width, height, tmp, id, title)
    os.system(cmd)
    return '%s.gif' % id
...
def print_motifs(motifs, kmer_count=20, istart=0):
    ...
    for m in motifs:
        ...
        if m.id is None:
            ...
            id = m.id
        print 'Log-odds matrix for Motif %s %s' % (id, m)
        m._print_ll()
        print 'Sequence Logo'
        m._print_bits()
        for newprop in ['gamma', 'church', 'E_site', 'E_seq', 'E_chi2', 'realpvalue',
         'kellis', 'MNCP', 'ROC_auc', 'CRA', 'Cfrac', 'frac', 'binomial']:
            if not m.__dict__.has_key(newprop):
                m.__dict__[newprop] = None

        if m.seedtxt:
            print 'Seed: %3d %s' % (i, m.seedtxt)
        if m.gamma:
            print 'Gamma: %7.5f' % m.gamma
        if m.evalue != None:
            print 'Evalue: %6.3e' % m.evalue
        if m.family:
            print 'Family: ', m.family
        if m.source:
            print 'Source: ', m.source
        print 'Motif %s %-25s (Bits: %5.2f  MAP: %5.2f   D: %5.3f  %2d) E: %6.3f' % (
         id, m, m.totalbits, m.MAP, m.seeddist, m.seednum, nlog10(m.pvalue)),
        if m.binomial != None:
            print ' Bi: %5.2f' % nlog10(m.binomial),
        if m.church != None:
            print ' ch: %5.2f' % nlog10(m.church),
        if m.frac != None:
            print ' f: %5.2f' % m.frac,
        if m.E_site != None:
            print ' Es: %5.2f' % nlog10(m.E_site),
        if m.E_seq != None:
            print ' Eq: %5.2f' % nlog10(m.E_seq),
        if m.MNCP != None:
            print ' mn: %5.2f' % m.MNCP,
        if m.ROC_auc != None:
            print ' Ra: %6.4f' % m.ROC_auc,
        if m.E_chi2 != None:
            if m.E_chi2 == 0:
                m.E_chi2 = 1e-20
        ...
        if m.CRA != None:
            print ' cR: %6.4f' % m.CRA,
        if m.Cfrac != None:
            print ' Cf: %6.4f' % m.Cfrac,
        if m.realpvalue != None:
            print ' P: %6.4e' % m.realpvalue,
        if m.kellis != None:
            print ' k: %5.2f' % m.kellis,
        try:
            if m.numbound:
                print ' b: %3d' % m.numbound,
            if m.nummotif:
                print ' nG: %3d' % m.nummotif,
            if m.numboundmotif:
                print ' bn: %3d' % m.numboundmotif,
        except:
            pass
