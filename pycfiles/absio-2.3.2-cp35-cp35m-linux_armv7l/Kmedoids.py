...
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/TAMO/Clustering/Kmedoids.py
# Compiled at: 2019-04-23 02:08:32
import string, sys, glob, random, pickle

def JumpWithinKMedoids_cluster(distanceMatrix, kmax=10, num_cycles=5, max_kMedoids_iterations=1000, min_dist=0.15, seeds=None, verbose=0, print_fnctn=None, data=None):
    """
    ...
    for k in range(1, kmax, 1):
        Wdist, medoids, clusters, distances = MinWithinKMedoids_cluster(distanceMatrix, k, num_cycles, max_kMedoids_iterations, seeds, verbose, print_fnctn, data, return_WD=1)
        Wdists.append(Wdist)
    ...
    sys.stdout.flush()
    return (
     Results[kbest][0], Results[kbest][1], Results[kbest][2])


def bestKMedoids_cluster(distanceMatrix, kmax=10, num_cycles=5, max_kMedoids_iterations=1000, min_dist=0.15, seeds=None, verbose=0, print_fnctn=None, data=None):
    """
    ...
    for k in range(kmax, 0, -1):
        medoids, clusters, distances = metaKMedoids_cluster(distanceMatrix, k, num_cycles, max_kMedoids_iterations, seeds, verbose, print_fnctn, data)
        numzerodist = 0
    ...
    try:
        print '# min,max dist:  %6.4f  %6.4f' % (min(dist), max(dist))
    except:
        pass

    return (
     medoids, clusters, distances)


def bestaveKMedoids_cluster(distanceMatrix, kmax=10, num_cycles=5, max_kMedoids_iterations=1000, min_dist=0.15, seeds=None, verbose=0, print_fnctn=None, data=None, kmin=0):
    """
    ...
    for k in range(kmax, kmin, -1):
        medoids, clusters, distances = MinWithinKMedoids_cluster(distanceMatrix, k, num_cycles, max_kMedoids_iterations, seeds, verbose, print_fnctn, data)
        numzerodist = 0
        avedists = []
        ...
        avedists.sort()
        if k != 1:
            print '# K = %2d  mindist = %5.3f' % (k, avedists[0])
        sys.stdout.flush()
    ...
    try:
        print '# min,max dist:  %6.4f  %6.4f' % (min(avedists), max(avedists))
    except:
        pass
...
def metaminDiamKMedoids_cluster(distanceMatrix, kstart=1, num_cycles=5, max_kMedoids_iterations=1000, min_dist=0.15, seeds=None, verbose=0, print_fnctn=None, data=None):
    """
    ...
    for k in range(kstart, len(distanceMatrix.keys())):
        medoids, clusters, distances = minDiamKMedoids_cluster(distanceMatrix, k, num_cycles, max_kMedoids_iterations, seeds, verbose, print_fnctn, data)
        numzerodist = 0
...
def bestupKMedoids_cluster(distanceMatrix, kmax=10, num_cycles=5, max_kMedoids_iterations=1000, min_dist=0.15, seeds=None, verbose=0, print_fnctn=None, data=None):
    """
    ...
    medoids, clusters, distances = metaKMedoids_cluster(distanceMatrix, kbest, num_cycles, max_kMedoids_iterations, seeds, verbose, print_fnctn, data)
    numzerodist = 0
    medidxs = medoids.values()
    dist = []
    for I in medidxs:
...
def MinWithinKMedoids_cluster(distanceMatrix, k, num_cycles=5, max_kMedoids_iterations=1000, seeds=None, verbose=0, print_fnctn=None, data=None, return_WD=None):
    ...
    for i in range(num_cycles):
        ...
        if verbose and print_fnctn:
            print_fnctn(clusters, data, distances, k)
        if not clusters:
            continue
...
def minDiamKMedoids_cluster(distanceMatrix, k, num_cycles=5, max_kMedoids_iterations=1000, seeds=None, verbose=0, print_fnctn=None, data=None):
    """
        """
    if seeds and len(seeds) != num_cycles:
        print 'num seeds != num cycles'
...
def kMedoids_cluster(distanceMatrix, k, max_iterations=1000, seed='12345'):
    ...
    return (
     medoids, cluster, avg_dist_in_cluster)
...
def generate_random_1d(x=0, mult=1.0, n=100):
    data = []
    for i in range(n):
...
def generate_random_2d(x=0, y=0, mult=1.0, n=100):
    data = []
    for i in range(n):
...
if __name__ == '__main__':
    Data = []
    Data += generate_random_1d(10.0, 1, 5)
    Data += generate_random_1d(20.0, 1, 5)
    Data += generate_random_1d(30.0, 1, 5)
    random.shuffle(Data)
    k = 3
    distanceMatrix = {}
