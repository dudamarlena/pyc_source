...
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/TAMO/DataSources/SGD.py
...
import sys, re, math, pickle, TAMO.paths
    ...
    if not _orf2ext:
        ...
        for line in lines:
            ...
            if toks[1] in ('SIB', 'DIP', 'IUBMB', 'CandidaDB'):
                ...
                if toks[1] == 'NCBI' and toks[2] == 'RefSeq protein version ID':
...
def feature_dist(Astart, Astop, Bstart, Bstop):
    FABS = math.fabs
    dist = min(FABS(Astart - Bstart), FABS(Astart - Bstop), FABS(Astop - Bstart), FABS(Astop - Bstop))
    return dist
...
def feature_pos(_feature):
    F = feature(_feature)
    if not F:
        F = feature(gene2orf(_feature))
    if not F:
        return
    else:
        return (
         F.chr, F.start, F.stop)
        return
...
def _load_features():
    ...
    for line in lines:
        ...
        try:
            ...
            if not _featuresD.has_key(ftype):
                _featuresD[ftype] = {}
...
def gene2orf(gene_query):
    ...
    if gene_query in special_cases.keys():
        ...
        if gene_query in _gene2orf.values():
            return gene_query
        else:
            return gene_query
...
def orf2gene(orf_query):
    ...
    if _orf2gene.has_key(orf_query):
        ...
        if orf_query in _orf2gene.values():
            return orf_query
        else:
            return
...
def orf2pseq(orf):
    ...
    if not _orfpseqs:
        ...
        for _orf, pseq in _orfpseqs.items():
            if pseq[(-1)] == '*':
                _orfpseqs[_orf] = pseq[:-1]

    if _orfpseqs.has_key(orf):
        return _orfpseqs[orf]
    else:
        return ''


def genenonnuclear(gene):
    return orfnonnuclear(gene2orf(gene))


def genelocalization(gene):
    return orflocalization(gene2orf(gene))


def geneabundance(gene):
    return orfabundance(gene2orf(gene))


def orfabundance(orf):
    if not _abundance:
        load_localization()
    if not _abundance.has_key(orf):
        return None
    else:
...
def load_localization_Kumar():
    if not _localize:
        FID = open(_LOCALIZE, 'r')
        lines = FID.readlines()
        FID.close()
        for line in lines[1:]:
            D = {}
...
def load_localization():
    if not _localize:
        ...
        for line in lines[1:]:
            toks = [ x.strip() for x in line.split('\t') ]
            orf, name, abund, locs = (
             toks[1], toks[2], toks[6], toks[8])
            _localize[orf] = locs.split(',')
            if abund.isdigit():
                _abundance[orf] = int(abund)
            else:
                _abundance[orf] = -1
...
special_cases = {'GRF10(Pho2)': 'PHO2', 'SIG1': 'MOT2', 
   'YAP8': 'ARR1', 
   'HDT80': 'NDT80', 
   'MATa1': 'HMRA1', 
   'MATA1': 'HMRA1', 
