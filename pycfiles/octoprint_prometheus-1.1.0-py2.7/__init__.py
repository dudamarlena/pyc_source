...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/octoprint_prometheus/__init__.py
# Compiled at: 2019-03-10 13:10:50
...
class PrometheusPlugin(octoprint.plugin.StartupPlugin, octoprint.plugin.SettingsPlugin, octoprint.plugin.TemplatePlugin, octoprint.plugin.ProgressPlugin, octoprint.plugin.EventHandlerPlugin):
    DESCRIPTIONS = {'temperature_bed_actual': 'Actual Temperature in Celsius of Bed', 'temperature_bed__target': 'Target Temperature in Celsius of Bed', 
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.gauges['printer_state'] = Enum('printer_state', 'State of printer', states=[
         'init', 'printing', 'done', 'failed', 'cancelled', 'idle'])
    ...
    def on_event(self, event, payload):
        if event == 'ZChange':
            ...
            self.get_gauge('print').info({'name': payload.get('name', ''), 'path': payload.get('path', ''), 
...
def __plugin_load__():
    ...
    __plugin_hooks__ = {'octoprint.comm.protocol.temperatures.received': plugin.temperatures_handler, 'octoprint.comm.protocol.gcode.sent': plugin.gcodephase_hook}