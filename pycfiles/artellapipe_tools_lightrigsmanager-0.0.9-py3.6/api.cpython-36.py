...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/tools/lightrigsmanager/core/api.py
# Compiled at: 2020-04-15 09:21:35
...
def get_light_rigs_path(project=None, config=None):
    ...
    if not light_rigs_template_name:
        ...
        if not template:
            LOGGER.warning('"{}" template is not defined in project files configuration file!'.format(light_rigs_template_name))
            return
        template_dict = {'project_id':project.id, 
...
def get_light_rig_file_type_instance(light_rig_name, light_rig_folder=None, project=None, config=None, light_rigs_path=None):
    ...
    if not light_rig_file_type:
        ...
        if not light_rig_file_class:
            LOGGER.warning('Impossible to reference Light Rig: {} | {} | {}'.format(light_rig_name, light_rigs_path, light_rig_file_type))
            return
        ...
        if not os.path.isdir(light_rig_path):
            LOGGER.warning('Impossible to reference Light Rig: {} | {} | {}'.format(light_rig_name, light_rig_path, light_rig_file_type))
            return
...
def open_light_rig(light_rig_name, light_rig_folder=None, project=None, config=None, light_rigs_path=None):
    ...
    :return: bool
    """
    light_rig_file_inst = get_light_rig_file_type_instance(light_rig_name,
      light_rig_folder=light_rig_folder, project=project, config=config, light_rigs_path=light_rigs_path)
