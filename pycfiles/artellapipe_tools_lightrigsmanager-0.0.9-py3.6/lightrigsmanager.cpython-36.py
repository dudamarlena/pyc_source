...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/tools/lightrigsmanager/widgets/lightrigsmanager.py
...
class LightRig(base.BaseWidget, object):
    ...
    def ui(self):
        ...
        reference_action = QAction(tpDcc.ResourcesMgr().icon('reference'), 'Reference Light Rig', self._light_menu)
    ...
    def _on_open_light_rig(self):
        ...
        if not self._path:
            ...
            return api.open_light_rig(light_rig_name=light_rig_name,
              light_rig_folder=(self._name),
              project=(self._project),
              config=(self._config),
              light_rigs_path=(self._path))
    ...
    def _on_import_light_rig(self):
        ...
        if not self._path:
            ...
            light_rig_name = self._get_light_rig_name()
            return api.import_light_rig(light_rig_name=light_rig_name,
              light_rig_folder=(self._name),
              project=(self._project),
              config=(self._config),
              light_rigs_path=(self._path))
    ...
    def _on_reference_light_rig(self):
        ...
        if not self._path:
            ...
            light_rig_name = self._get_light_rig_name()
            return api.reference_light_rig(light_rig_name=light_rig_name,
              light_rig_folder=(self._name),
              project=(self._project),
              config=(self._config),
              light_rigs_path=(self._path))
...
class ArtellaLightRigManager(artellapipe.ToolWidget, object):
    ...
    def _update_ui(self, allow_sync=True):
        ...
        if not os.path.exists(light_rigs_path):
            if allow_sync:
                result = qtutils.show_question(None, 'Light Rigs Folder is not available!', 'Do you want to synchronize Light Rigs Folder from Artella to your computer? \n\n{}'.format(light_rigs_path))
                if result == QMessageBox.Yes:
                    self.synchronize_light_rigs()
                    if not os.path.exists(light_rigs_path):
                        msg = 'Was not possible to synchronize Light Rigs folder from Artella: "{}"'.format(light_rigs_path)
                        self.show_warning_message(msg)
                        LOGGER.warning(msg)
                        return
                else:
