...
# Embedded file name: build/bdist.linux-x86_64/egg/apgl/util/Parameter.py
# Compiled at: 2012-04-03 14:24:40
__doc__ = '\nCreated on 31 Jul 2009\n\n@author: charanpal\n'
import numpy, scipy.sparse, logging
...
class Parameter(object):
    ...
    @staticmethod
    def checkSymmetric(A, tol=1e-06, softCheck=False, arrayInfo='?'):
        """
        ...
        if scipy.sparse.issparse(A) and (A.T - A).multiply(A.T - A).sum() > tol ** 2:
            return Parameter.whatToDo('A=' + arrayInfo + ' is not a symmetric matrix, ||A.T - A||_F^2 = ' + str((A.T - A).multiply(A.T - A).sum()), softCheck)
        return True

    @staticmethod
    def checkOrthogonal(A, tol=1e-06, softCheck=False, investigate=False, arrayInfo='?'):
        """
