...
# Embedded file name: build/bdist.linux-x86_64/egg/apgl/util/Util.py
# Compiled at: 2014-02-18 12:34:28
__doc__ = '\nCreated on 31 Jul 2009\n\n@author: charanpal\n'
from __future__ import print_function
import sys, os, numpy
from contextlib import contextmanager
import numpy.random as rand, logging, scipy.linalg, scipy.sparse as sparse, scipy.special, pickle
...
class Util(object):
    ...
    def fitDiscretePowerLaw(x, xmins=None):
        ...
        if xmins.shape[0] == 0:
            ...
            return (
             ksAlpha[(i, 0)], ksAlpha[(i, 1)], xmins[i])
    ...
    def svd(A, eps=1e-08, tol=1e-08):
        ...
        if A.shape[0] > A.shape[1]:
            ...
            return (
             Qh.conj().T, s.conj(), P.conj().T)

    @staticmethod
    def svd_from_eigh(A, eps=1e-08, tol=1e-08):
        """
    ...
    @staticmethod
    def safeSvd(A, eps=1e-08, tol=1e-08):
        """
        ...
        if ProfileUtils.memory() > 1000000000:
    ...
    def safeEigh(a, b=None, lower=True, eigvals_only=False, overwrite_a=False, overwrite_b=False, turbo=True, eigvals=None, type=1):
        ...
        try:
            ...
            return (
             lmbda[eigvals[0]:eigvals[1]], q[eigvals[0]:eigvals[1]])
