...
# [GCC 8.4.0]
# Embedded file name: /media/sumit/DATA/uChicago/Classes/Q4/Advanced Machine Learning/Advanced Machine Learning and Artificial Intelligence (MScA 32017)_Reinforced Learning/project2/pycman/pycman/pacman.py
# Compiled at: 2018-07-25 16:16:05
# Size of source mod 2**32: 14151 bytes
import numpy as np, random, math, json, pkg_resources
...
class PacmanGame:
    ...
    def __init__(self, field_shape, nmonsters, ndiamonds, nwalls, monster_vision_range, max_moves=1000):
    ...
    def render(self, pixels_per_square=50, score_height=100, current_mean=0, game_number=0):
        import pyglet, os
        if self.game_window is None:
            self.game_window = pyglet.window.Window(width=pixels_per_square * self.field_shape[1], height=pixels_per_square * self.field_shape[0] + score_height)
        ...
        sprite = pyglet.sprite.Sprite(self.player_img, x=int(self.player[1] * pixels_per_square), y=int((self.field_shape[0] - 1 - self.player[0]) * pixels_per_square + score_height))
        sprite.scale = min(pixels_per_square / sprite.width, pixels_per_square / sprite.height)
        sprite.draw()
        for dc in self.diamonds:
            sprite = pyglet.sprite.Sprite(self.diamond_img, x=int(dc[1] * pixels_per_square), y=int((self.field_shape[0] - 1 - dc[0]) * pixels_per_square + score_height))
            sprite.scale = min(pixels_per_square / sprite.width, pixels_per_square / sprite.height)
            sprite.draw()

        for mc in self.monsters:
            sprite = pyglet.sprite.Sprite(self.monster_img, x=int(mc[1] * pixels_per_square), y=int((self.field_shape[0] - 1 - mc[0]) * pixels_per_square + score_height))
            sprite.scale = min(pixels_per_square / sprite.width, pixels_per_square / sprite.height)
            sprite.draw()

        for wc in self.walls:
            sprite = pyglet.sprite.Sprite(self.wall_img, x=int(wc[1] * pixels_per_square), y=int((self.field_shape[0] - 1 - wc[0]) * pixels_per_square + score_height))
            sprite.scale = min(pixels_per_square / sprite.width, pixels_per_square / sprite.height)
            sprite.draw()

        label0 = pyglet.text.Label(str(game_number), font_name='Times New Roman', font_size=score_height, x=0, y=0)
        label0.draw()
        label = pyglet.text.Label(str(self.total_score), font_name='Times New Roman', font_size=score_height, x=400, y=0)
        label.draw()
        label2 = pyglet.text.Label(str(current_mean), font_name='Times New Roman', font_size=score_height, x=170, y=0)
        label2.draw()
        if self.end_game:
            label = pyglet.text.Label('Oops!', color=(255, 0, 0, 255), font_name='Times New Roman', font_size=int(score_height / 3), x=self.game_window.width, y=int(score_height), anchor_x='right', anchor_y='top')
            label.draw()
        ...
        for i in range(len(self.monsters)):
            ...
            for dx in [-1, 0, 1]:
                for dy in [-1, 0, 1]:
                    new_coord = (
                     mc[0] + dx, mc[1] + dy)
                    if new_coord[0] in range(self.field_shape[0]) and new_coord[1] in range(self.field_shape[1]) and new_coord not in self.monsters and new_coord not in self.walls or new_coord == mc:
    ...
    def player_possible_actions(self):
        ...
        for a, (dx, dy) in action_to_dxdy.items():
            x = self.player[0] + dx
            y = self.player[1] + dy
            if x in range(self.field_shape[0]) and y in range(self.field_shape[1]) and (
    ...
    def make_action(self, a):
        ...
        if new_coord[0] in range(self.field_shape[0]) and new_coord[1] in range(self.field_shape[1]) and new_coord not in self.walls:
        ...
        if self.player in self.diamonds:
            ...
            while 1:
                ...
                y = np.random.randint(self.field_shape[1])
                diamond_coord = (x, y)
                if diamond_coord not in self.diamonds and diamond_coord != self.player and diamond_coord not in self.walls and diamond_coord not in self.monsters:
                    break
...
def naive_strategy(obs):
    distances_to_monsters = [distance(obs['player'], monster) for monster in obs['monsters']]
    if min(distances_to_monsters) >= 2:
        best_dist = float('inf')
        best_act = 5
        for a in obs['possible_actions']:
            dx, dy = action_to_dxdy[a]
            p = (obs['player'][0] + dx, obs['player'][1] + dy)
            distances_to_diamonds = [distance(p, diamond) for diamond in obs['diamonds']]
            m = min(distances_to_diamonds)
            if m < best_dist:
                best_dist = m
                best_act = a
