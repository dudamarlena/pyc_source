...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/compose_flow/commands/subcommands/publish.py
...
class Publish(BaseBuildSubcommand):
    ...
    def fill_subparser(cls, parser, subparser):
        ...
        subparser.add_argument('--tag-major-minor',
          default=False,
          action='store_true',
    ...
    def get_built_tagged_image_names(self) -> List[str]:
        ...
        for service_name, service_data in profile.data['services'].items():
            if service_data.get('build'):
                ...
                tagged_image_name or print(f"service_name={service_name} does not have an image directive",
                  file=(sys.stderr))
                continue
    ...
    def get_built_docker_images(self) -> List[DockerImage]:
        ...
        docker_images = [DockerImage(tagged_image_name=tagged_image_name, publish_callable=(self.execute_publish), tag_callable=(self.execute_tag)) for tagged_image_name in tagged_image_names]
    ...
    def push(self):
        ...
        for docker_image in docker_images:
            ...
            if self.workflow.args.dry_run:
                ...
                if self.workflow.args.tag_major_minor:
