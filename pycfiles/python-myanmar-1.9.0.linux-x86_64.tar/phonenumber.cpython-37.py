...
# [GCC 8.4.0]
# Embedded file name: /home/trhura/Code/pythonenv/lib/python3.7/site-packages/myanmar/phonenumber.py
# Compiled at: 2020-05-10 22:49:32
...
mpt = '(?:{}|{}|{}|{}|{}|{}|{}|{})$'.format(mpt_2_series, mpt_3_series, mpt_4_series, mpt_5_series, mpt_6_series, mpt_7_series, mpt_8_series, mpt_9_series)
all_operators_re = '({0}|{1}|{2}|{3})'.format(ooredoo, telenor, mpt, mytel)
mm_phone_re = re.compile('^({0}|{1})?{2}'.format(country_code, mobile_code, all_operators_re))
mpt_re = re.compile('^({0}|{1})?{2}'.format(country_code, mobile_code, mpt))
ooredoo_re = re.compile('^({0}|{1})?{2}'.format(country_code, mobile_code, ooredoo))
mytel_re = re.compile('^({0}|{1})?{2}'.format(country_code, mobile_code, mytel))
telenor_re = re.compile('^({0}|{1})?{2}'.format(country_code, mobile_code, telenor))
...
def normalize_phonenumber(phonenumber):
    ...
    if not match:
        raise RuntimeError('%s is not a valid Myanmar phonenumber.' % phonenumber)
