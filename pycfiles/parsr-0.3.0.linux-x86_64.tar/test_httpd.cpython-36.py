...
# [GCC 8.4.0]
# Embedded file name: /home/csams/git_repos/personal/parsr/lib/python3.6/site-packages/parsr/examples/tests/test_httpd.py
# Compiled at: 2019-05-28 16:55:00
# Size of source mod 2**32: 4696 bytes
...
def test_httpd_conf_nest_one():
    ...
    if not len(val['IfModule']) == 2:
        raise AssertionError
    else:
        if not len(val[('IfModule', 'mod_rewrite.c')]) == 1:
            raise AssertionError
        else:
            if not len(val[('IfModule', '!php5_module')]) == 1:
                raise AssertionError
            else:
