...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/soynlp/predicator/_predicator.py
# Compiled at: 2018-11-26 12:18:34
# Size of source mod 2**32: 18676 bytes
""" TERM DEFINITION
...
class PredicatorExtractor:
    ...
    def __init__(self, nouns, josas=None, adjectives=None, verbs=None, eomis=None, extract_eomi=False, extract_stem=False, verbose=True, ensure_normalized=False):
        if not josas:
        ...
        self._nouns = nouns
        self._eomis_ = {eomi for eomi in self._eomis}
    ...
    def _print(self, message, replace=False, newline=True):
        ...
        if replace:
            print(('\r{} {}'.format(header, message)), end=('\n' if newline else ''),
              flush=True)
        else:
            print(('{} {}'.format(header, message)), end=('\n' if newline else ''),
              flush=True)
    ...
    def train_extract(self, inputs, min_eojeol_frequency=2, filtering_checkpoint=100000, candidates=None, min_predicator_frequency=1, reset_lrgraph=True, min_num_of_features=5, min_eomi_score=0.3, min_eomi_frequency=1, min_num_of_unique_R_char=10, min_entropy_of_R_char=0.5, min_entropy_of_R=1.5, min_stem_score=0.7, min_stem_frequency=100):
        self.train(inputs, min_eojeol_frequency, filtering_checkpoint, min_num_of_features, min_eomi_score, min_eomi_frequency, min_num_of_unique_R_char, min_entropy_of_R_char, min_entropy_of_R, min_stem_score, min_stem_frequency)
        predicators = self.extract(candidates, min_predicator_frequency)
        return predicators

    def train(self, inputs, min_eojeol_frequency=2, filtering_checkpoint=100000, min_num_of_features=5, min_eomi_score=0.3, min_eomi_frequency=1, min_num_of_unique_R_char=10, min_entropy_of_R_char=0.5, min_entropy_of_R=1.5, min_stem_score=0.7, min_stem_frequency=100):
        if isinstance(inputs, LRGraph):
            self._train_with_eojeol_counter(inputs.to_EojeolCounter(), min_eojeol_frequency)
        else:
        ...
        if self.extract_stem:
            ...
            self._extract_stem(lrgraph, min_num_of_unique_R_char, min_entropy_of_R_char, min_entropy_of_R, min_stem_score, min_stem_frequency)
        if self.verbose:
            message = 'has been trained'
            self._print(message, replace=False, newline=True)

    def _train_with_sentences(self, sentences, min_eojeol_frequency=2, filtering_checkpoint=100000):
        check = filtering_checkpoint > 0
        if self.verbose:
            message = 'counting eojeols ... '
            self._print(message, replace=False, newline=False)
        elif self.ensure_normalized:
    ...
    def _train_with_eojeol_counter(self, eojeol_counter, min_eojeol_frequency=2):
        eojeol_counter._counter = {eojeol:count for eojeol, count in eojeol_counter._counter.items() if count >= min_eojeol_frequency}
    ...
    def _prepare_predicator_lrgraph(self):
        ...
        eojeols = {eojeol:count for eojeol, count in eojeols.items() if not contains_noun(eojeol)}
        lrgraph = EojeolCounter()._to_lrgraph(eojeols)
        return lrgraph

    def _extract_eomi(self, lrgraph, min_num_of_features=5, min_eomi_score=0.3, min_eomi_frequency=1):
        eomi_extractor = EomiExtractor(lrgraph=lrgraph,
          stems=(self._stems),
          nouns=(self._nouns),
          min_num_of_features=min_num_of_features,
          verbose=(self.verbose),
          logpath=None)
        extracted_eomis = eomi_extractor.extract(condition=None,
    ...
    def _extract_stem(self, lrgraph, min_num_of_unique_R_char=10, min_entropy_of_R_char=0.5, min_entropy_of_R=1.5, min_stem_score=0.7, min_stem_frequency=100):
        stem_extractor = StemExtractor(lrgraph=lrgraph,
          stems=(self._stems),
          eomis=(self._eomis),
        ...
        extracted_stems = stem_extractor.extract(L_ignore=None,
    ...
    def _extract_predicator(self, eojeol_counter=None, min_frequency=1):
        ...
        if not (eojeol_counter is None or eojeol_counter):
            eojeol_counter = {eojeol:count for eojeol, count in self.eojeol_counter.items() if all_character_are_complete_korean(eojeol)}
    ...
    def _remove_wrong_eomis(self, lemmas, eomi_to_word_count):
        ...
        def noun_proportion(word_count):
            ...
            prop = sum((1 for w, v in word_count if w in self._nouns if len(w) == 2))
            prop_len2 = 0
            if sum_ > 0:
                prop /= sum_
        ...
        for eomi, word_count in eomi_to_word_count.items():
            if len(eomi) >= 3:
                ...
                if prop_len2 == 1:
                    ...
                    remove_words = tuple((word for word, _ in word_count if len(word) == 2 and word in self._nouns or len(word) == 3 and len(eomi) == 2))
        ...
        if self.verbose:
            ...
            message = '{} eomis are removed, {} words are modified.'.format(len(remove_eomis), len(words))
            self._print(message, replace=True, newline=True)
