...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/soynlp/noun/_noun_ver2.py
# Compiled at: 2018-11-27 14:24:53
# Size of source mod 2**32: 23390 bytes
...
class LRNounExtractor_v2:
    ...
    def __init__(self, max_left_length=10, max_right_length=9, predictor_headers=None, verbose=True, min_num_of_features=1, max_frequency_when_noun_is_eojeol=30, eojeol_counter_filtering_checkpoint=500000, extract_compound=True, extract_pos_feature=False, extract_determiner=False, ensure_normalized=False, postprocessing=None, logpath=None):
        ...
        if not postprocessing:
            ...
            if isinstance(postprocessing) == str:
                postprocessing = [
                 postprocessing]
            else:
                self.postprocessing = postprocessing
                predictor_headers = predictor_headers or self._set_default_predictor_header()
    ...
    def _load_predictor(self, headers):
        ...
        pos = {feature for feature in pos if feature not in common}
        neg = {feature for feature in neg if feature not in common}
        if self.verbose:
            print('[Noun Extractor] num features: pos={}, neg={}, common={}'.format(len(pos), len(neg), len(common)))
    ...
    def _append_features(self, feature_type, features):
        ...
        if feature_type == 'pos':
            ...
            self._pos_features.update({f for f in features if f not in commons})
        else:
            if feature_type == 'neg':
                commons = {f for f in features if f in self._pos_features}
                self._neg_features.update({f for f in features if f not in commons})
            else:
        ...
        if self.verbose:
            message = 'pos={} -> {}, neg={} -> {}, common={} -> {}'.format(n_pos, n_pos_, n_neg, n_neg_, n_common, n_common_)
            print('[Noun Extractor] features appended. {}'.format(message))

    def train_extract(self, inputs, min_noun_score=0.3, min_noun_frequency=1, min_eojeol_frequency=1, reset_lrgraph=True):
    ...
    def train(self, inputs, min_eojeol_frequency=1):
        if isinstance(inputs, LRGraph):
            ...
            if isinstance(inputs, EojeolCounter):
    ...
    def _train_with_sentences(self, sentences, min_eojeol_frequency=1):
        ...
        elif not self.ensure_normalized:
    ...
    def _train_with_eojeol_counter(self, eojeol_counter):
        lrgraph = eojeol_counter.to_lrgraph(self.max_left_length, self.max_right_length)
    ...
    def _train_with_lrgraph(self, lrgraph, num_of_eojeols=-1):
        ...
        if self.verbose:
            print('[Noun Extractor] has been trained. #eojeols={}, mem={} Gb'.format(num_of_eojeols, '%.3f' % get_process_memory()))

    def _extract_determiner(self):
        raise NotImplemented

    def extract_domain_pos_features(self, noun_candidates=None, ignore_features=None, append_extracted_features=True, min_noun_score=0.3, min_noun_frequency=100, min_pos_score=0.3, min_pos_feature_frequency=1000, min_num_of_unique_lastchar=4, min_entropy_of_lastchar=0.5, min_noun_entropy=1.5):
        if self.verbose:
        ...
        prediction_scores = self._batch_predicting_nouns(noun_candidates, min_noun_score)
        self.lrgraph.reset_lrgraph()
        self._pos_features_extracted = extract_domain_pos_features(prediction_scores, self.lrgraph, self._pos_features, ignore_features, min_noun_score, min_noun_frequency, min_pos_score, min_pos_feature_frequency, min_num_of_unique_lastchar, min_entropy_of_lastchar, min_noun_entropy)
        if append_extracted_features:
            self._append_features('pos', self._pos_features_extracted)
        if self.verbose:
            print('[Noun Extractor] {} pos features were extracted'.format(len(self._pos_features_extracted)))
    ...
    def extract(self, min_noun_score=0.3, min_noun_frequency=1, reset_lrgraph=True):
        ...
        if self.extract_pos_feature:
            ...
            if self.extract_compound:
                candidates = {l:sum(rdict.values()) for l, rdict in self.lrgraph._lr.items() if len(l) >= 4 if len(l) >= 4}
                compounds = self.extract_compounds(candidates, prediction_scores, min_noun_score)
            else:
                compounds = {}
        nouns = {noun:score for noun, score in prediction_scores.items() if score[1] >= min_noun_score}
        nouns.update(compounds)
        nouns = {noun:score for noun, score in nouns.items() if score[0] >= min_noun_frequency}
        nouns = self._post_processing(nouns, prediction_scores, compounds)
        if self.verbose:
            print(('[Noun Extractor] {} nouns ({} compounds) with min frequency={}'.format(len(nouns), len(compounds), min_noun_frequency)),
              flush=True)
    ...
    def _get_nonempty_features(self, word, features):
        return [self._exist_longer_neg(word, r) or r for r, _ in features if r in self._pos_features if self._exist_longer_pos(word, r) if r in self._neg_features]
    ...
    def predict(self, word, min_noun_score=0.3, debug=False):
        ...
        if debug:
            print('pos={}, common={}, neg={}, unk={}, end={}, n_features_={}'.format(pos, common, neg, unk, end, n_features_))
        ...
        for r, _ in features:
            ...
            if r in self._pos_features or r in self._common_features:
                if not self._exist_longer_pos(word, r):
                    first_chars.add(r[0])
                r in self._pos_features or r in self._common_features or first_chars.add(r[0])
    ...
    def _predict(self, word, features):
        ...
        for r, freq in features:
            if r == '':
                ...
                continue
            if self._exist_longer_pos(word, r):
                continue
            if self._exist_longer_neg(word, r):
                continue
    ...
    def _batch_predicting_nouns(self, noun_candidates, min_noun_score=0.3):
        ...
        for i, word in enumerate(sorted(noun_candidates, key=(lambda x: -len(x)))):
            if self.verbose:
                if i % 1000 == 999:
                    ...
                    print(('\r  -- batch prediction {} % of {} words'.format(percentage, n)),
                      flush=True, end='')
        ...
        if self.verbose:
            print(('\r[Noun Extractor] batch prediction was completed for {} words'.format(n)),
              flush=True)
        return prediction_scores

    def extract_compounds(self, candidates, prediction_scores, min_noun_score=0.3):
        noun_scores = {noun:len(noun) for noun, score in prediction_scores.items() if len(noun) > 1}
        self._compound_decomposer = MaxScoreTokenizer(scores=noun_scores)
        candidates = {l:rdict.get('', 0) for l, rdict in self.lrgraph._lr_origin.items() if l not in noun_scores}
        n = len(candidates)
        compounds_scores = {}
        compounds_counts = {}
        ...
        if self.verbose:
            print('\r[Noun Extractor] checked compounds. discovered {} compounds'.format(len(compounds_scores)))
        compounds = {noun:(score, compounds_counts.get(noun, 0)) for noun, score in compounds_scores.items()}
    ...
    def _parse_compound(self, tokens):
        ...
        if len(tokens) >= 3:
            if tokens[(-1)][0] in self._pos_features:
    ...
    def _post_processing(self, nouns, prediction_scores, compounds):
        ...
        def print_status(method, nouns, removals):
            ...
            print('[Noun Extractor] postprocessing {} : {} -> {}'.format(method, n_before, n_after))
        ...
        for method in self.postprocessing:
            if method == 'detaching_features':
                ...
                nouns, removals = detaching_features(nouns, self._pos_features, logpath, logheader)
                if self.verbose:
    ...
    def _check_covered_eojeols(self, nouns):
        ...
        for i, word in enumerate(sorted(noun_candidates, key=(lambda x: -len(x)))):
            if self.verbose:
                if i % 1000 == 999:
                    percentage = '%.3f' % (100 * (i + 1) / n)
                    print(('\r[Noun Extractor] flushing ...  {} %'.format(percentage)),
                      flush=True, end='')
        ...
        if self.verbose:
            print(('\r[Noun Extractor] flushing was done. mem={} Gb{}'.format('%.3f' % get_process_memory(), '                    ')),
              flush=True)
            coverage = '%.2f' % (100 * self._num_of_covered_eojeols / self._num_of_eojeols)
