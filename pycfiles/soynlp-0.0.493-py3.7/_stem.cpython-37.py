...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/soynlp/predicator/_stem.py
# Compiled at: 2018-11-26 12:18:34
# Size of source mod 2**32: 9082 bytes
import math
...
class StemExtractor:
    ...
    def __init__(self, lrgraph, stems, eomis, min_num_of_unique_R_char=10, min_entropy_of_R_char=0.5, min_entropy_of_R=1.5, verbose=True):
    ...
    def _conjugate_stem_and_eomi(self, lrgraph, stems, eomis):
        ...
        if self.verbose:
            message = 'Initializing was done with {} stems and {} eomis{}'.format(len(stems), len(eomis), '          ')
            self._print(message, replace=True, newline=True)
        del eojeol_counter
        return (stem_surfaces, eomi_surfaces)

    def extract(self, L_ignore=None, min_stem_score=0.7, min_stem_frequency=100):
        if L_ignore is None:
            L_ignore = {}
        candidates = {}
        for r in self.R:
        ...
        if self.verbose:
            message = '{} stems, {} surfacial stems, {} removals'.format(len(self.stems), len(self.stem_surfaces), len(self.removals))
            self._print(message, replace=False, newline=True)
        return self.stems

    def _batch_prediction(self, candidates, min_stem_score, min_frequency):
        extracted = {l:None for l in self.L}
        for l in sorted(candidates, key=(lambda x: -len(x))):
            if not l in self.L:
                if l in self.R or len(l) == 1 or l[(-1)] == '다' or l in extracted:
                    continue
        ...
        extracted = {l:score for l, score in extracted.items() if l not in self.L}
        return extracted
    ...
    def predict(self, l, min_stem_score=0.7, min_frequency=1, debug=False):
        ...
        if debug:
            print('pos={}, neg={}, unk={}, n_features_={}, n_char={}, entropy_r={}'.format(pos, neg, unk, len(features), unique_of_char, entropy_of_char))
    ...
    def _post_processing(self, extracted):
        ...
        def is_stem_and_eomi(l):
            n = len(l)
            for i in range(1, n):
                if not l[:i] in self.L:
                    if not l[:i] in extracted:
                        continue
    ...
    def _to_stem(self, surfaces):
        ...
        def merge_score(freq0, score0, freq1, score1):
            return (
             freq0 + freq1, (score0 * freq0 + score1 * freq1) / (freq0 + freq1))
                ...
                try:
                    for stem, eomi in lemma_candidate(l, r):
                        if eomi in self.eomis:
                            continue
                        stems[stem] = merge_score(freq0, score0, *stems.get(stem, (0,
                                                                                   0)))
