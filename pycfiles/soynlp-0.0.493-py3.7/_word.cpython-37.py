...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/soynlp/word/_word.py
# Compiled at: 2019-06-06 15:17:51
# Size of source mod 2**32: 13840 bytes
from collections import defaultdict
from collections import namedtuple
import math, numpy as np, pickle, sys
...
class WordExtractor:
    ...
    def __init__(self, sents=None, max_left_length=10, max_right_length=6, min_frequency=5, verbose_points=100000, min_cohesion_forward=0.05, min_cohesion_backward=0.0, max_droprate_cohesion=0.98, max_droprate_leftside_frequency=0.98, min_left_branching_entropy=0.0, min_right_branching_entropy=0.0, min_left_accessor_variety=0, min_right_accessor_variety=0, remove_subwords=False):
    ...
    def train(self, sents, num_for_pruning=0, cumulate=True):
        ...
        def prune_extreme_case_a():
            self._aL = defaultdict(lambda : 0, {w:f for w, f in self._aL.items() if f > 1 if f > 1})
            self._aR = defaultdict(lambda : 0, {w:f for w, f in self._aR.items() if f > 1 if f > 1})
        ...
        for num_sent, sent in enumerate(sents):
            ...
            if num_for_pruning > 0:
                if num_sent % num_for_pruning == 0:
                    prune_extreme_case()
    ...
    def extract(self, scores=None):
        ...
        for word, score in sorted((scores.items()), key=(lambda x: len(x[0]))):
            if score.left_branching_entropy < self.min_left_branching_entropy or score.right_branching_entropy < self.min_right_branching_entropy or score.left_accessor_variety < self.min_left_accessor_variety or score.right_accessor_variety < self.min_right_accessor_variety or max(score.leftside_frequency, score.rightside_frequency) < self.min_frequency:
                continue
            if len(word) >= 2:
                if score.cohesion_forward < self.min_cohesion_forward or score.cohesion_backward < self.min_cohesion_backward:
                    continue
            scores_[word] = score
            if not self.remove_subwords:
                continue
            subword = word[:-1]
            droprate_leftside_frequency = 0 if subword not in self.L else score.leftside_frequency / self.L[subword]
    ...
    def frequency(self, word):
        return (
         self.L.get(word, 0), self.R.get(word, 0))
    ...
    def branching_entropy(self, word):
        word_len = len(word)
        lsb = {w:f for w, f in self.R.items() if w[1:] == word}
        lsb.update({w:f for w, f in self._aR.items() if w[2:] == word})
        rsb = {w:f for w, f in self.L.items() if w[:-1] == word}
        rsb.update({w:f for w, f in self._aL.items() if w[:-2] == word})
    ...
    def save(self, fname):
        configuration = {'max_left_length':self.max_left_length, 
