...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/soynlp/postagger/_pos_extractor.py
...
class POSExtractor:
    ...
    def extract(self, sentences):
        ...
        del self._lrgraph
        if self.verbose:
            message = '{} nouns, {} predicators were extracted'.format(len(nouns), len(predicators))
    ...
    def _extract_nouns(self, sentences):
        ...
        nouns = noun_extractor.extract(reset_lrgraph=False,
          min_count=10,
          minimum_noun_score=0.4)
        self._lrgraph = LRGraph({l:{r:v for r, v in rdict.items()} for l, rdict in noun_extractor.lrgraph._lr.items()})
        ...
        if self.verbose:
            message = 'noun extraction was done. {} % eojeols are covered'.format('%.2f' % (100 * self._num_of_covered_eojeols / self._num_of_eojeols))
            self._print(message, replace=True, newline=True)
        return nouns

    def _extract_predicators(self, sentences_or_lrgraph, nouns=None):
        predicator_extractor = PredicatorExtractor(nouns,
          extract_eomi=True,
          extract_stem=True,
          verbose=(self.verbose))
        predicator_extractor.train(sentences_or_lrgraph, min_eojeol_count=2)
        predicators = predicator_extractor.extract(candidates=None,
          min_count=10,
          reset_lrgraph=True,
        ...
        if self.verbose:
            message = 'predicator extraction was done. {} % eojeols are covered (cum)'.format('%.2f' % (100 * self._num_of_covered_eojeols / self._num_of_eojeols))
            self._print(message, replace=True, newline=True)
        return predicators