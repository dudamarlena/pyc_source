...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/soynlp/predicator/_eomi.py
...
class EomiExtractor:
    ...
    def __init__(self, lrgraph, stems, nouns, min_num_of_features=5, verbose=True, logpath=None):
    ...
    def extract(self, condition=None, min_eomi_score=0.3, min_eomi_frequency=1, reset_lrgraph=True):
        ...
        lemmas = {eomi:score for eomi, score in lemmas.items() if score[1] >= min_eomi_score}
        if self.logpath:
        ...
        if self.verbose:
            message = '{} eomis extracted with min frequency = {}, min score = {}'.format(len(lemmas), min_eomi_frequency, min_eomi_score)
            self._print(message, replace=False, newline=True)
    ...
    def predict(self, r, min_eomi_score=0.3, min_num_of_features=5, debug=False):
        ...
        if debug:
            print('pos={}, neg={}, unk={}, n_features_={}'.format(pos, neg, unk, n_features_))
    ...
    def _refine_features(self, features, r):
        return [(l, count) for l, count in features if l in self._stem_surfaces if not self._exist_longer_pos(l, r)]
    ...
    def _batch_prediction(self, eomi_candidates, min_eomi_score=0.3, min_num_of_features=5):
        prediction_scores = {}
        ...
        for i, r in enumerate(sorted(eomi_candidates, key=(lambda x: -len(x)))):
            ...
            if score >= min_eomi_score:
                for l, count in self.lrgraph.get_l(r, -1):
                    if l in self._stem_surfaces or self._is_aNoun_Verb(l):
                ...
                try:
                    for stem_, eomi_ in lemma_candidate(stem_surface, eomi):
                        if stem_ not in self._stems:
                            continue
                        eomis_[eomi_] = merge_score(count, score0, *eomis_.get(eomi_, (0,
                                                                                       0)))
