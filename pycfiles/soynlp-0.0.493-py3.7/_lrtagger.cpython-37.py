...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/soynlp/postagger/_lrtagger.py
# Compiled at: 2018-10-01 03:38:35
# Size of source mod 2**32: 13285 bytes
...
class LREvaluator:
    ...
    def evaluate(self, candidates, preference=None):
        scores = []
        for c in candidates:
            score = self._evaluate(self.make_scoretable(c.L[0], c.L[1], c.R[0], c.R[1], c.cohesion_l, c.droprate_l, c.lcount, c.lr_prop, c.lr_count, c.length))
            if preference:
                if c.L[1]:
                    if c.L[1] in preference:
                        score += preference.get(c.L[1], {}).get(c.L[0], 0)
                if c.R[1]:
                    if c.R[1] in preference:
    ...
    def make_scoretable(self, l, pos_l, r, pos_r, cohesion, droprate, lcount, lr_prop, lr_count, len_LR):
        return ScoreTable(cohesion, droprate, log(lcount + 1), lr_prop, log(lr_count + 1), 1 if (pos_l and pos_r) else 0, 1 if len(r) == 1 else 0, log(len_LR))
...
class LRMaxScoreTagger:
    ...
    def __init__(self, domain_dictionary_folders=None, use_base_dictionary=True, dictionary_word_mincount=3, evaluator=None, sents=None, lrgraph=None, lrgraph_lmax=12, lrgraph_rmax=8, base_tokenizer=None, preference=None, verbose=False):
    ...
    def _build_lrgraph(self, sents, lmax=12, rmax=8):
        ...
        eojeols = Counter((eojeol for sent in sents for eojeol in sent.split()))
    ...
    def _initialize_scores(self, lrgraph):
        ...
        droprate_l = {w:c / lcount[w[:-1]] for w, c in lcount.items() if w[:-1] in lcount}
        return (
         lrgraph_norm, lcount, cohesion_l, droprate_l)
    ...
    def _remove_l_subsets(self, candidates):
        ...
        for pos in ('Noun', 'Verb', 'Adjective', 'Adverb', 'Exclamation'):
            ...
            while sorted_:
                ...
                removals = [i for i, c in enumerate(sorted_) if b <= c[2] if e >= c[3]]
    ...
    def _initialize_LR(self, t, candidates, threshold_prop=0.001, threshold_count=2):
        ...
        for l, pos, b, e, len_l in candidates:
            for len_r in range(min(self.dictionary._rmax, n - e) + 1):
                ...
                if r:
                    if lr_prop <= threshold_prop or lr_count <= threshold_count:
                        continue
    ...
    def _remove_r_subsets(self, expanded):
        ...
        for pos in ('Josa', 'Verb', 'Adjective', None):
            ...
            while sorted_:
                ...
                removals = [i for i, c in enumerate(sorted_) if b <= c[3] if e >= c[4]]
                for idx in reversed(removals):
                    del sorted_[idx]
    ...
    def _to_table(self, c):
        return Table(c[0], c[1], c[2], c[3], c[4], c[5], c[6], self.cohesion_l.get(c[0][0], 0), self.droprate_l.get(c[0][0], 0), self.lcount.get(c[0][0], 0))
    ...
    def _find_best(self, scores):
        ...
        while sorted_:
            ...
            removals = [i for i, (c, _) in enumerate(sorted_) if b < c[3] if e > c[2]]
            for idx in reversed(removals):
                del sorted_[idx]
    ...
    def add_words_into_dictionary(self, words, tag):
        if tag not in self.dictionary._pos:
