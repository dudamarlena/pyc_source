...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/soynlp/tokenizer/_noun_tokenizer.py
# Compiled at: 2018-05-14 10:30:19
...
class NounLMatchTokenizer:
    ...
    def tokenize(self, sentence, compose_compound=True):
        tokens = [self._max_length_l_tokenize(token) for token in sentence.split() if token]
        tokens = [token for token in tokens if token if token[0]]
