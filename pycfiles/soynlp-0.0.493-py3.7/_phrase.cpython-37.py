...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/soynlp/word/_phrase.py
# Compiled at: 2018-10-01 05:23:38
...
class Bigram:
    ...
    def __init__(self, sentences=None, min_frequency=5, verbose=True, score='frequency', filtering_checkpoint=100000, tokenizer=None, ngram_selector=None):
        """
        Attribute:
        ----------
    ...
    def train(self, sentences):
        ...
        for i_sent, sent in enumerate(sentences):
            if self.filtering_checkpoint > 0:
                if i_sent % self.filtering_checkpoint == 0:
                    self._counter = {bigram:freq for bigram, freq in self._counter.items() if freq >= self.min_frequency}
            if self.verbose:
                if i_sent % 3000 == 0:
                    print(('\r[Bigram Extractor] scanning {} bigrams from {} sents'.format(len(self._counter), i_sent)),
                      end='', flush=True)
        ...
        if self.verbose:
            print(('\r[Bigram Extractor] scanning {} unigrams, {} bigrams from {} sents'.format(len(self._unigram), len(self._counter), i_sent)),
              flush=True)
    ...
    def _extract_by_pmi(self, topk=-1, threshold=0):
        ...
        bigrams = {word:NgramScore(freq, pmis[word]) for word, freq in self._counter.items() if word in pmis}
        return bigrams
    ...
    def _extract_by_mikolov(self, topk=-1, threshold=0):
        ...
        def score(bigram, freq, N):
            ...
            if base == 0:
                return 0
            return (freq - self.min_frequency) / base
        ...
        bigrams = {word:NgramScore(freq, scores[word]) for word, freq in self._counter.items() if word in scores}
        return bigrams