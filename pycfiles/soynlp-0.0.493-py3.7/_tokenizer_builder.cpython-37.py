...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/soynlp/tokenizer/_tokenizer_builder.py
# Compiled at: 2018-10-01 05:50:15
...
class EojeolPatternTrainer:
    ...
    def train(self, sents, wordset_l=None, wordset_r=None):
        if not (wordset_l and wordset_r):
    ...
    def _scan_vocabulary(self, sents):
        ...
        for i, sent in enumerate(sents):
            ...
            if self.verbose and i % _ckpt == 0:
                args = (
                 '#' * int(i / _ckpt), '-' * (40 - int(i / _ckpt)), 100.0 * i / len(sent), '%', get_process_memory())
    ...
    def _build_graph(self, sents, wordset_l, wordset_r):
        ...
        for i, sent in enumerate(sents):
            ...
            if self.verbose and i % _ckpt == 0:
                args = (
                 '#' * int(i / _ckpt), '-' * (40 - int(i / _ckpt)), 100.0 * i / len(sents), '%', get_process_memory())
