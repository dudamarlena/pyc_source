...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/soynlp/postagger/_template.py
# Compiled at: 2018-10-25 15:21:12
...
class EojeolTemplateMatcher(BaseTemplateMatcher):
    ...
    def __init__(self, dictionary, single_tags=None, lr_templates=None):
        ...
        if not lr_templates:
            lr_templates = [
             ('Noun', 'Verb'), ('Noun', 'Adjective'), ('Noun', 'Josa')]
...
class LRTemplateMatcher(BaseTemplateMatcher):
    ...
    def _remove_subset_l(self, candidates):
        ...
        for l_tag in self.ltags:
            sorted_ = sorted((filter(lambda x: x[1] == l_tag, candidates)),
    ...
    def _remove_subset_r(self, expanded):
        ...
        for r_tag in rtags:
            sorted_ = sorted((filter(lambda x: x.r_tag == r_tag, expanded)),
              key=(lambda x: -(x.e - x.m)))
            while sorted_:
                expanded_.append(sorted_.pop(0))
                l_tag = expanded_[(-1)].l_tag
                b, m, e = expanded_[(-1)][-3:]
                removals = [i for i, c in enumerate(sorted_) if c.l_tag == l_tag if b == c.b if m == c.m if m <= c.m if e >= c.e]
                for idx in reversed(removals):
                    del sorted_[idx]
