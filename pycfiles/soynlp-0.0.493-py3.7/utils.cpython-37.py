...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/soynlp/utils/utils.py
# Compiled at: 2019-04-29 02:25:20
...
def get_process_memory():
    ...
    return process.memory_info().rss / 1073741824
            ...
            try:
                ...
                for doc_idx, doc in enumerate(f):
                    ...
                    if num_sent > 0 and num_sent_ > num_sent:
                        return (
                         doc_idx + 1, min(num_sent, num_sent_))
...
class EojeolCounter:
    ...
    def _counting_from_sents(self, sents):
        ...
        for i_sent, sent in enumerate(sents):
            ...
            if self.min_count > 1:
                if self.filtering_checkpoint > 0:
                    if i_sent > 0:
                        if i_sent % self.filtering_checkpoint == 0:
                            _counter = {k:v for k, v in _counter.items() if v >= self.min_count}
            for eojeol in sent.split():
                if eojeol:
                    if len(eojeol) > self.max_length:
                        continue
                    _counter[eojeol] = _counter.get(eojeol, 0) + 1

            if self.verbose and i_sent % 100000 == 99999:
                print(('\r[EojeolCounter] n eojeol = {} from {} sents. mem={} Gb{}'.format(len(_counter), i_sent + 1, '%.3f' % get_process_memory(), '                    ')),
                  flush=True, end='')
    ...
    def _to_lrgraph(self, counter, l_max_length=10, r_max_length=9, ignore_one_syllable=False):
        ...
        lrgraph = LRGraph(lrgraph=_lrgraph, l_max_length=l_max_length,
    ...
    def save(self, path):
        ...
        with open(path, 'w', encoding='utf-8') as (f):
            for eojeol, count in sorted((self._counter.items()), key=(lambda x: (
             -x[1], x[0]))):
                f.write('{} {}\n'.format(eojeol, count))
...
class LRGraph:
    ...
    def __init__(self, lrgraph=None, sents=None, l_max_length=10, r_max_length=9):
        if l_max_length > 1:
            raise type(l_max_length) == int or AssertionError
        elif r_max_length > 0:
            if not type(r_max_length) == int:
                raise AssertionError
            self.l_max_length = l_max_length
            self.r_max_length = r_max_length
            if sents:
                if lrgraph:
                    raise ValueError('Inserted lrgraph will be ignored. Insert only one (lrgraph, sents)')
                lrgraph = self._construct_graph(sents)
            if lrgraph:
                self._lr, self._rl = self._check_lrgraph(lrgraph)
        else:
            self._lr, self._rl = {}, {}
        self._lr_origin = {l:{r:c for r, c in rdict.items()} for l, rdict in self._lr.items()}
    ...
    def _check_lrgraph(self, lrgraph):
        if type(lrgraph) is not dict:
            try:
                ...
                raise ValueError('lrgraph type should be dict of dict, not {}'.format(type(lrgraph)))

        nested_dict_type = type(list(lrgraph.values())[0])
        if nested_dict_type == defaultdict:
            lrgraph = {l:dict(rdict) for l, rdict in lrgraph.items()}
        else:
            if not nested_dict_type == dict:
                raise ValueError('nested value type should be dict, not {}'.format(nested_dict_type))
    ...
    def reset_lrgraph(self):
        if not self._lr_origin:
            return
        self._lr, self._rl = self._check_lrgraph({l:{r:c for r, c in rdict.items()} for l, rdict in self._lr_origin.items()})
    ...
    def copy_compatified_lrgraph_origin(self):
        ...
        lr_graph = LRGraph(l_max_length=(self.l_max_length),
          r_max_length=(self.r_max_length))
        lr_graph._lr, lr_graph._rl = self._check_lrgraph({l:{r:c for r, c in rdict.items()} for l, rdict in self._lr_origin.items()})
        return lr_graph
