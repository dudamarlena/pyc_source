...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/soynlp/tokenizer/_tokenizer.py
# Compiled at: 2018-10-01 05:49:22
# Size of source mod 2**32: 15970 bytes
import sys
...
class RegexTokenizer:
    ...
    def __init__(self):
        self._patterns = [
         (
          'number', re.compile('[-+]?\\d*[\\.]?[\\d]+|[-+]?\\d+', re.UNICODE)),
         (
          'korean', re.compile('[가-힣]+', re.UNICODE)),
         (
          'jaum', re.compile('[ㄱ-ㅎ]+', re.UNICODE)),
         (
          'moum', re.compile('[ㅏ-ㅣ]+', re.UNICODE)),
         (
          'english & latin', re.compile("[a-zA-ZÀ-ÿ]+[[`']?s]*|[a-zA-ZÀ-ÿ]+", re.UNICODE))]
...
class MaxScoreTokenizer:
    ...
    def _recursive_tokenize(self, token, range_l=0, debug=False):
        ...
        if length <= 2:
            return [
             (
              token, 0, length, self._ds, length)]
        if range_l == 0:
...
class MaxLRScoreTokenizer:
    ...
    def __init__(self, Dl=None, Dr=None, preference_l=None, preference_r=None, lrgraph=None, tokenizer_builder=None, max_lscore_difference=0.3, max_lscore_diffratio=0.5, ensurable_score_l=0.5, ensurable_score_lr_diff=0.3):
    ...
    def _remove_l_subset(self, candidates):
        ...
        while candidates:
            ...
            exist_longer = False
            for c in candidates:
                if not (c[2] > b or c[3] < e or c[2] < b):
                    if not c[3] > e:
                        continue
                if not lscore - c[(-2)] < self.max_lscore_difference:
                    if not self.ensurable_score_l * 0.5 < lscore or (lscore + 1e-05) / (c[(-2)] + 1e-05) < self.max_lscore_diffratio:
                        exist_longer = True
                        break

            exist_longer or candidates_.append(best)
    ...
    def _find_best(self, scores):
        ...
        while sorted_:
            ...
            removals = [i for i, c in enumerate(sorted_) if b < c[4] if e > c[2]]
