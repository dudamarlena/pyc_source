...
# [GCC 8.4.0]
# Embedded file name: /Users/davidchen/repos/django-video-composer/venv/lib/python2.7/site-packages/pafy/backend_internal.py
# Compiled at: 2017-12-12 20:44:10
import os, hashlib, tempfile, json, re, sys, time, logging
...
class InternPafy(BasePafy):
    ...
    def _fetch_basic(self):
        ...
        if self._have_basic:
            ...
            watchinfo = fetch_decode(watch_url)
            dbg('Fetched watch page')
            self.callback('Fetched watch page')
            self.age_ver = re.search('player-age-gate-content">', watchinfo) is not None
            if self.ciphertag:
                ...
                if not self.age_ver:
                    ...
                    self.js_url = js_url
                    dashsig = re.search('/s/([\\w\\.]+)', self._dashurl).group(1)
                    dbg('decrypting dash sig')
                    goodsig = _decodesig(dashsig, js_url, self.callback)
                    self._dashurl = re.sub('/s/[\\w\\.]+', '/signature/%s' % goodsig, self._dashurl)
                else:
                    s = re.search('/s/([\\w\\.]+)', self._dashurl).group(1)
                    s = s[2:63] + s[82] + s[64:82] + s[63]
                    self._dashurl = re.sub('/s/[\\w\\.]+', '/signature/%s' % s, self._dashurl)
...
class InternStream(BaseStream):
    ...
    def __init__(self, sm, parent):
        ...
        if is_dash:
            ...
            self._dimensions = tuple([ int(x) if x.isdigit() else x for x in self._dimensions
                                     ])
    ...
    def url(self):
        ...
        if not self._url:
            if self._parent.age_ver:
                ...
                self._sig = _decodesig(self._sig, self._parent.js_url, self._parent.callback)
...
def _extract_dash(dashurl):
    ...
    for x in tlist:
        ...
        dashmap.append(dict(bitrate=bitrate, dash=True, itag=itag, width=width, height=height, url=url, size=size))
...
def prune_files(path, prefix='', age_max=1209600, count_max=4):
...
def get_js_sm(watchinfo, callback):
    ...
    if not mainfunc:
        ...
        javascript = fetch_cached(js_url, callback, encoding='utf8', dbg_ref='javascript', file_prefix='js-')
