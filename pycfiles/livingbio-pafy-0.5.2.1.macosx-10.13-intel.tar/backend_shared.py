...
# [GCC 8.4.0]
# Embedded file name: /Users/davidchen/repos/django-video-composer/venv/lib/python2.7/site-packages/pafy/backend_shared.py
# Compiled at: 2017-12-12 20:44:10
import os, re, sys, time, logging, subprocess
...
class BasePafy(object):
    ...
    def __init__(self, video_url, basic=True, gdata=False, size=False, callback=None, ydl_opts=None):
        """ Set initial values. """
    ...
    def _get_video_gdata(self, video_id):
        ...
        query = {'part': 'id,snippet,statistics', 'maxResults': 1, 
           'id': video_id}
    ...
    def __repr__(self):
        ...
        if self._have_basic:
            ...
            nfo = 'Pafy object: %s [%s]' % (self.videoid,
             self.title[:45] + '..')
        if pyver == 2:
            return nfo.encode('utf8', 'replace')
        return nfo
...
class BaseStream(object):
    ...
    def download(self, filepath='', quiet=False, callback=lambda *x: None, meta=False, remux_audio=False):
