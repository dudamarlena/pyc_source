...
# [GCC 8.4.0]
# Embedded file name: /Users/davidchen/repos/django-video-composer/venv/lib/python2.7/site-packages/pafy/pafy.py
...
if os.environ.get('PAFY_BACKEND') != 'internal':
    try:
        ...
        logging.warning('pafy: youtube-dl not found; falling back to internal backend. This is not as well maintained as the youtube-dl backend. To hide this message, set the environmental variable PAFY_BACKEND to "internal".')
...
def fetch_decode(url, encoding=None):
    ...
    if encoding:
        ...
        if 'charset=' in ct:
            dbg('charset: %s', ct)
            encoding = re.search('charset=([\\w-]+)\\s*(:?;|$)', ct).group(1)
            return req.read().decode(encoding)
...
def set_categories(categories):
    ...
    """
    timestamp = time.time()
    idlist = [ cid for cid, item in categories.items() if item.get('updated', 0) < timestamp - g.lifespan
             ]
    if len(idlist) > 0:
