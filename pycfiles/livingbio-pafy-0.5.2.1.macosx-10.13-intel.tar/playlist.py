...
# [GCC 8.4.0]
# Embedded file name: /Users/davidchen/repos/django-video-composer/venv/lib/python2.7/site-packages/pafy/playlist.py
...
def extract_playlist_id(playlist_url):
    idregx = re.compile('((?:RD|PL|LL)[-_0-9a-zA-Z]+)$')
    playlist_id = None
...
def get_playlist(playlist_url, basic=False, gdata=False, size=False, callback=lambda x: None):
    ...
    allinfo = fetch_decode(url)
    allinfo = json.loads(allinfo)
    playlist = dict(playlist_id=playlist_id, likes=allinfo.get('likes'), title=allinfo.get('title'), author=allinfo.get('author'), dislikes=allinfo.get('dislikes'), description=allinfo.get('description'), items=[])
    for v in allinfo['video']:
        vid_data = dict(added=v.get('added'), is_cc=v.get('is_cc'), is_hd=v.get('is_hd'), likes=v.get('likes'), title=v.get('title'), views=v.get('views'), rating=v.get('rating'), author=v.get('author'), user_id=v.get('user_id'), privacy=v.get('privacy'), start=v.get('start', 0.0), dislikes=v.get('dislikes'), duration=v.get('duration'), comments=v.get('comments'), keywords=v.get('keywords'), thumbnail=v.get('thumbnail'), cc_license=v.get('cc_license'), category_id=v.get('category_id'), description=v.get('description'), encrypted_id=v.get('encrypted_id'), time_created=v.get('time_created'), time_updated=v.get('time_updated'), length_seconds=v.get('length_seconds'), end=v.get('end', v.get('length_seconds')))
        try:
            pafy_obj = new(vid_data['encrypted_id'], basic=basic, gdata=gdata, size=size, callback=callback)
        except IOError as e:
...
def parseISO8591(duration):
    ...
    regex = re.compile('PT((\\d{1,3})H)?((\\d{1,3})M)?((\\d{1,2})S)?')
    if duration:
...
class Playlist(object):
    ...
    def __iter__(self):
        if self._items is not None:
            ...
            return
        items = []
        query = {'part': 'snippet', 'maxResults': 50, 
           'playlistId': self.plid}
        while True:
            playlistitems = call_gdata('playlistItems', query)
            query2 = {'part': 'contentDetails,snippet,statistics', 'maxResults': 50, 
               'id': (',').join(i['snippet']['resourceId']['videoId'] for i in playlistitems['items'])}
            wdata = call_gdata('videos', query2)
            for v, vextra in zip(playlistitems['items'], wdata['items']):
                vid_data = dict(title=v['snippet']['title'], author=v['snippet']['channelTitle'], thumbnail=v['snippet'].get('thumbnails', {}).get('default', {}).get('url'), description=v['snippet']['description'], length_seconds=parseISO8591(vextra['contentDetails']['duration']), category=get_categoryname(vextra['snippet']['categoryId']), views=vextra['statistics'].get('viewCount', 0), likes=vextra['statistics'].get('likeCount', 0), dislikes=vextra['statistics'].get('dislikeCount', 0), comments=vextra['statistics'].get('commentCount', 0))
                try:
                    pafy_obj = new(v['snippet']['resourceId']['videoId'], basic=self._basic, gdata=self._gdata, size=self._size, callback=self._callback)
                except IOError as e:
...
def get_playlist2(playlist_url, basic=False, gdata=False, size=False, callback=lambda x: None):
