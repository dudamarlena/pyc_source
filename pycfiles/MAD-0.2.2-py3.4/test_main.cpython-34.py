...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\tests\test_main.py
# Compiled at: 2016-04-27 11:23:55
...
class TestXXX(TestCase):
    ...
    def test_client_server_with_autoscaling(self):
        self.file_system.define('test.mad', 'service DB {   settings {      autoscaling {          period: 10          limits: [3, 6]      }   }   operation Select {      think 9   }}client Browser {  every 2 {      query DB/Select  }}')
        controller = Controller(StringIO(), self.file_system)
...
class TestMain(TestCase):
    ...
    def test_worker_do_not_wait_for_response(self):
        expression = Sequence(DefineService('Database', DefineOperation('op', Think(8))), DefineService('Server', DefineOperation('op', Query('Database', 'op'))), DefineClientStub('Client', 5, Query('Server', 'op')))
        simulation = self.evaluate(expression)
        self.run_until(simulation, 20)
        self.verify_trace(simulation, [
    ...
    def test_simple_example(self):
        model = 'service S1:   operation op:       think 5client C1:   every 10:       query S1::op'
        expression = Sequence(DefineService('S1', DefineOperation('op', Think(3))), DefineClientStub('C1', 10, Query('S1', 'op')))
        simulation = self.evaluate(expression)
    ...
    def verify_trace(self, simulation, expected_trace):
        ...
        self.assertEqual(len(actual_trace), len(expected_trace), 'Traces should have the same length (expected %d, found %d)\n%s' % (len(expected_trace), len(actual_trace), str(actual_trace)))
    ...
    def verify_event(self, expected, actual):
        self.assertEqual(expected.time, actual.time, 'Time mismatch (expected time is %d)\n%s' % (expected.time, str(actual)))
        self.assertEqual(expected.message, actual.message, "Message mismatch (expected message is '%s')\n%s" % (expected.message, str(actual)))
        self.assertEqual(expected.context, actual.context, 'Context mismatch (expected context is %s)\n%s' % (expected.context, str(actual)))
