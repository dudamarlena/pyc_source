...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\mad\validation\engine.py
# Compiled at: 2016-04-25 06:38:05
...
class SymbolTable:
    ...
    def miss_operation(self, service, operation):
        return not (service in self.entities and operation in self.entities[service].operations)

    def invoke(self, service, operation):
        if service in self.entities:
            if operation in self.entities[service].operations:
...
class Validator:
    __doc__ = "Traverse the AST searching for inconsistencies, so called 'Semantic Errors'"
