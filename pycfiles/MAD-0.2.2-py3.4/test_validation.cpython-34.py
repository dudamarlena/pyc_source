...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\tests\test_validation.py
# Compiled at: 2016-04-12 03:23:25
...
class ValidatorTests(TestCase):
    ...
    def _verify_issues(self, expression, actual, expected):
        for each_expected_error in expected:
            self.assertTrue(each_expected_error in actual, "Expression '%s'\nshould raise %s\nbut found %s" % (
             str(expression), expected, actual))
    ...
    def unknown_operation(self):
        return {'expression': Sequence(DefineService('DB', DefineOperation('Insert', Think(5))), DefineClientStub('Erroneous', 2, Trigger('DB', 'Select'))), 
         'expected_errors': [
                             UnknownOperation('DB', 'Select')]}

    def duplicate_operation(self):
        return {'expression': DefineService('DB', Sequence(DefineOperation('Insert', Think(5)), DefineOperation('Insert', Think(5)))), 
         'expected_errors': [
                             DuplicateOperation('DB', 'Insert')]}

    def duplicate_service(self):
        return {'expression': Sequence(DefineService('DB', DefineOperation('Insert', Think(5))), DefineService('DB', DefineOperation('Select', Think(5)))), 
         'expected_errors': [
                             DuplicateIdentifier('DB')]}

    def empty_service(self):
        return {'expression': DefineService('DB', Sequence()), 
         'expected_errors': [
                             EmptyService('DB')]}

    def never_invoked_operation(self):
        return {'expression': DefineService('DB', Sequence(DefineOperation('Insert', Think(5)))), 
         'expected_errors': [
                             NeverInvokedOperation('DB', 'Insert')]}

    def unknown_service(self):
        return {'expression': DefineClientStub('Browser', 2, Trigger('DB', 'Select')), 
         'expected_errors': [
                             UnknownService('DB')]}
