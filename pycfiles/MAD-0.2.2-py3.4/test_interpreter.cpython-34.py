...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\tests\test_interpreter.py
# Compiled at: 2016-04-27 14:59:34
...
class TestInterpreter(TestCase):
    ...
    def test_query_make_services_busy(self):
        ...
        self.assertEqual(100.0, front_end.utilisation)
        self.simulate_until(2)
        self.assertEqual(0.0, front_end.utilisation)
        self.assertEqual(0.0, db.utilisation)
        self.simulate_until(3)
        self.assertEqual(0.0, front_end.utilisation)
        self.assertEqual(100.0, db.utilisation)
    ...
    def test_evaluate_timeout_queries(self):
        ...
        front_end = self.evaluate(DefineService('Front-end', DefineOperation('checkout', Query('DB', 'op', timeout=5)))).value
    ...
    def test_evaluate_timeout_no_expiration(self):
        ...
        front_end = self.evaluate(DefineService('Front-end', DefineOperation('checkout', Retry(expression=Query('DB', 'op', timeout=1), delay=Delay(2, 'constant'), limit=5)))).value
        request = self.send_request('Front-end', 'checkout')
    ...
    def test_sequence_evaluation(self):
        ...
        self.evaluate(DefineService('Front-end', DefineOperation('checkout', Sequence(Trigger('DB', 'op'), Trigger('DB', 'op')))))
        self.send_request('Front-end', 'checkout')
    ...
    def test_retry_and_succeed(self):
        ...
        self.evaluate(DefineService('Front-end', DefineOperation('checkout', Retry(Query('DB', 'insert'), limit=5, delay=Delay(10, 'constant')))))
    ...
    def test_ignore_error(self):
        ...
        self.evaluate(DefineService('Front-end', DefineOperation('checkout', Sequence(IgnoreError(Query('DB1', 'op')), Trigger('DB2', 'op')))))
        self.send_request('Front-end', 'checkout')
        self.simulate_until(20)
    ...
    def test_client_stub_definition(self):
        client = self.evaluate(Sequence(DefineService('Service X', DefineOperation('op', Think(2))), DefineClientStub('Client', 10, Query('Service X', 'op')))).value
