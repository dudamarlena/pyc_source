...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\mad\ui.py
# Compiled at: 2016-04-25 03:49:17
...
class Controller:
    ...
    def _load(self, arguments):
        self.storage = DataStorage(Parser(self.file_system, arguments._file_name), FileLog(self.file_system.open_output_stream(arguments.log_file), Arguments.LOG_FORMAT), lambda name, format: CSVReport(self.file_system.open_output_stream(arguments.report_for(name)), format))
...
class Display:
    ...
    def unknown_service(self, error):
        self._format(Messages.ERROR_UNKNOWN_SERVICE, severity=self._severity_of(error), service=error.service)

    def unknown_operation(self, error):
        self._format(Messages.ERROR_UNKNOWN_OPERATION, severity=self._severity_of(error), service=error.service, operation=error.operation)

    def never_invoked_operation(self, error):
        self._format(Messages.ERROR_NEVER_INVOKED_OPERATION, severity=self._severity_of(error), service=error.service, operation=error.operation)

    def duplicate_identifier(self, error):
        self._format(Messages.ERROR_DUPLICATE_IDENTIFIER, severity=self._severity_of(error), identifier=error.identifier)

    def duplicate_operation(self, error):
        self._format(Messages.ERROR_DUPLICATE_OPERATION, severity=self._severity_of(error), operation=error.operation, service=error.service)
...
class Arguments:
    ...
    def __init__(self, arguments):
        ...
        self._Arguments__output_directory = None
    ...
    @property
    def log_file(self):
        return self.PATH_TO_LOG_FILE.format(directory=self._output_directory, log_file=self.LOG_FILE)

    @property
    def _output_directory(self):
        if self._Arguments__output_directory is None:
            self._Arguments__output_directory = self.OUTPUT_DIRECTORY.format(name=self._model_name, identifier=self._identifier())
        return self._Arguments__output_directory
    ...
    def report_for(self, entity):
        return self.REPORT.format(directory=self._output_directory, entity=entity)
