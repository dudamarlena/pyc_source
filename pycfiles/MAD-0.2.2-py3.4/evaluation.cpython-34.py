...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\mad\evaluation.py
# Compiled at: 2016-04-27 15:15:40
# Size of source mod 2**32: 14406 bytes
...
class Evaluation:
    ...
    def of_trigger(self, trigger):
        return self._get_busy_for(duration=1, after=lambda status: self._do_trigger(trigger))

    def of_query(self, query):
        return self._get_busy_for(duration=1, after=lambda status: self._send_query(query))
    ...
    def of_think(self, think):
        ...
        """
        return self._get_busy_for(duration=think.duration, after=lambda status: self.continuation(status))
    ...
    def _do_trigger(self, trigger):
        ...
        request = self.factory.create_request(sender, 1, trigger.operation, trigger.priority, on_accept=on_accept, on_reject=on_reject)
    ...
    def _send_query(self, query):
        ...
        request = self.factory.create_request(sender, 0, query.operation, query.priority, on_reject=on_reject, on_success=on_success, on_error=on_error)
