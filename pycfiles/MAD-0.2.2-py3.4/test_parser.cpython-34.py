...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\tests\test_parser.py
# Compiled at: 2016-04-27 08:53:56
...
class CorrectExpressionTests(ParserTests):
    ...
    def _all_expressions(self):
        return [
         ...
         (
          'think 5 invoke DB/Select query DB/Insert ignore {  fail }',
          Sequence(Think(5), Trigger('DB', 'Select'), Query('DB', 'Insert'), IgnoreError(Fail())),
          'action_list'),
         (
          'retry { think 5 }',
          Retry(Think(5)),
         ...
         (
          ...
          'define_client'),
         (
          'service DB {   operation Select {       think 4   }}client Browser {   every 5 {       query DB/Select  }}',
          Sequence(DefineService('DB', DefineOperation('Select', Think(4))), DefineClientStub('Browser', 5, Query('DB', 'Select'))),
          'unit'),
         (
          'settings { queue: LIFO }', Settings(queue=LIFO()), 'settings'),
         (
          'autoscaling {  period: 134  limits: [27, 52]}', {'autoscaling': Autoscaling(period=134, limits=(27, 52))},
          'autoscaling'),
         (
          'throttling: none', {'throttling': NoThrottlingSettings()},
          'throttling'),
         (
          'throttling: tail-drop(50)', {'throttling': TailDropSettings(capacity=50)},
          'throttling'),
         (
          'settings {  queue: FIFO  autoscaling {      limits: [1, 5]  }  throttling: tail-drop(50)}',
          Settings(queue=FIFO(), autoscaling=Autoscaling(limits=(1, 5)), throttling=TailDropSettings(50)),
          'settings'),
         (
          'service DB {   settings {      queue: LIFO  }  operation Select {       think 4   }}client Browser {   every 5 {       query DB/Select  }}',
          Sequence(DefineService('DB', Sequence(Settings(queue=LIFO()), DefineOperation('Select', Think(4)))), DefineClientStub('Browser', 5, Query('DB', 'Select'))),
          'unit')]
