...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\tests\test_acceptance.py
# Compiled at: 2016-04-27 06:38:10
# Size of source mod 2**32: 23143 bytes
...
class AcceptanceTests(TestCase):
    ...
    def test_client_server(self):
        self.file_system.define('test.mad', 'service DB {   operation Insert {      think 5      query Mirror/Insert      fail 0.5   }   operation Select {      think 5   }}service Mirror {   operation Insert {      think 5   }}client Browser {   every 5 {      query DB/Select      retry (limit: 5, delay:exponential(5)) {           invoke DB/Insert      }   }}')
        self._execute()
    ...
    def test_priority_scheme(self):
        self.file_system.define('test.mad', 'service DB {   operation Select {      think 5   }}client Browser_A {   every 1 {      query DB/Select {priority: 10}   }}client Browser_B {   every 10 {      query DB/Select {priority: 0}   }}')
    ...
    def test_fail(self):
        self.file_system.define('test.mad', 'service DB {   operation Select {      fail   }}client Browser {   every 2 {       query DB/Select    }}')
    ...
    def test_retry(self):
        self.file_system.define('test.mad', 'service DB {   operation Select {      fail   }}client Browser {   every 100 {       retry(delay:constant(2), limit:10) {           query DB/Select       }   }}')
    ...
    def test_ignore_error(self):
        self.file_system.define('test.mad', 'service DB {   operation Select {      fail   }}client Browser {   every 20 {       ignore {           query DB/Select       }   }}')
    ...
    def test_timeouts(self):
        self.file_system.define('test.mad', 'service DB {   operation Select {      think 10   }}client Browser {   every 20 {      query DB/Select {timeout: 5}   }}')
        self._execute([self.LOCATION, 100])
        self._verify_opening()
        self._verify_valid_model()
        self._verify_no_warnings()
    ...
    def test_invalid_parameter_count(self):
        ...
        invalid_command_line = [
         'test.mad', 1000, 'an extra parameter']
        self._execute(invalid_command_line)
        self._verify_opening()
    ...
    def test_error_unknown_operation(self):
        self.file_system.define('test.mad', 'service DB {   operation Select {      think 5   }}client Browser {   every 5 {      query DB/Insert   }}')
        self._execute()
        self._verify_opening()
        self._verify_model_loaded()
        self._verify_invalid_model()
    ...
    def test_error_duplicate_service(self):
        self.file_system.define('test.mad', 'service DB {   operation Select {      think 5   }}service DB {   operation Insert {      think 17   }}client Browser {   every 5 {      query DB/Select   }}')
        self._execute()
        self._verify_opening()
        self._verify_model_loaded()
        self._verify_invalid_model()
    ...
    def test_error_duplicate_client(self):
        self.file_system.define('test.mad', 'service DB {   operation Select {      think 5    }}client Browser {   every 5 {      query DB/Select   }}client Browser {    every 10 {       invoke DB/Select   }}')
        self._execute()
        self._verify_opening()
        self._verify_model_loaded()
        self._verify_invalid_model()
    ...
    def test_error_duplicate_entity_name(self):
        self.file_system.define('test.mad', 'service DB {   operation Select {      think 5   }}client DB {   every 5 {      query DB/Select   }}')
        self._execute()
        self._verify_opening()
        self._verify_model_loaded()
        self._verify_invalid_model()
    ...
    def test_error_duplicate_operation(self):
        self.file_system.define('test.mad', 'service DB {   operation Select {      think 5   }   operation Select  {      think 17   }}client Browser {   every 5 {      query DB/Select   }}')
        self._execute()
        self._verify_opening()
        self._verify_model_loaded()
        self._verify_invalid_model()
    ...
    def test_warning_operation_never_invoked(self):
        self.file_system.define('test.mad', 'service DB {   operation Select {      think 5   }   operation Insert {      think 17   }}client Browser {   every 5 {      query DB/Select   }}')
        self._execute()
        self._verify_opening()
        self._verify_model_loaded()
        self._verify_valid_model()
    ...
    def test_error_unknown_service(self):
        self.file_system.define('test.mad', 'service DB {   operation Select {      think 5   }}client Browser {   every 5 {      query DBBBB/Select   }}')
        self._execute()
        self._verify_opening()
        self._verify_model_loaded()
        self._verify_invalid_model()
    ...
    def _verify_unknown_operation(self, service_name, operation_name):
        self._verify_output(Messages.ERROR_UNKNOWN_OPERATION, severity=Messages.SEVERITY_ERROR, service=service_name, operation=operation_name)

    def _verify_duplicate_service(self, service_name):
        self._verify_output(Messages.ERROR_DUPLICATE_IDENTIFIER, severity=Messages.SEVERITY_ERROR, identifier=service_name)

    def _verify_duplicate_client(self, client_name):
        self._verify_output(Messages.ERROR_DUPLICATE_IDENTIFIER, severity=Messages.SEVERITY_ERROR, identifier=client_name)

    def _verify_duplicate_entity_name(self, name):
        self._verify_output(Messages.ERROR_DUPLICATE_IDENTIFIER, severity=Messages.SEVERITY_ERROR, identifier=name)

    def _verify_duplicate_operation(self, service_name, operation_name):
        self._verify_output(Messages.ERROR_DUPLICATE_OPERATION, severity=Messages.SEVERITY_ERROR, operation=operation_name, service=service_name)

    def _verify_operation_never_invoked(self, service_name, operation_name):
        self._verify_output(Messages.ERROR_NEVER_INVOKED_OPERATION, severity=Messages.SEVERITY_WARNING, operation=operation_name, service=service_name)

    def _verify_unknown_service(self, service_name):
        self._verify_output(Messages.ERROR_UNKNOWN_SERVICE, severity=Messages.SEVERITY_ERROR, service=service_name)
