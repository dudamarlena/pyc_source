...
# [GCC 8.4.0]
# Embedded file name: /mnt/d/UNC Drive/pymemsci/membrane_toolkit/core/potential.py
# Compiled at: 2020-05-10 20:17:25
...
def apparent_permselectivity(E_mem: float, E_ideal: float, t_counter: float=0.5) -> float:
    r"""
    ...
    if t_counter < 0 or t_counter > 1:
        raise ValueError('Counter-ion transport number must be between 0 and 1. t_counter={:.3f}'.format(t_counter))
...
def nernst_potential(a0: float, aL: float, z_ct: int=1, temperature: float=25) -> float:
    r"""
    ...
    """
    if a0 < 0:
        raise ValueError('Received invalid activity argument of a0 = {}. Electrolyteactivity must be positive.'.format(a0))
    if aL < 0:
        raise ValueError('Received invalid activity argument of aL = {}. Electrolyteactivity must be positive.'.format(aL))
    if temperature < -273.15:
        raise ValueError('Received invalid temperature argument of {}. Temperatureis below absolute zero!'.format(temperature))
