...
# Embedded file name: build/bdist.linux-x86_64/egg/ambhas/richards.py
...
class RICHARDS_1D():
    ...
    def _read_input(self):
        ...
        """
        input_sheets = [
         'ind', 'forcing', 'initial_condition', 'units', 'temporal_info',
...
class RICHARDS_1D_ENKF(RICHARDS_1D):
    ...
    def _enkf_par(self):
        ...
        ev = e + v
        cov_ee = np.dot(e.T, e) + 1e-06 * np.eye(self.no_layer + 6)
    ...
    def _enkf_par_depth(self):
        ...
        for i in range(self.no_layer + 6):
            if i < self.no_layer:
                ...
                ev[:, i] = e[:, i] + 2.5e-05 * np.random.normal(size=self.n_ens)
    ...
    def _read_shp_ens(self):
        ...
        shp_ens = {}
        shp_ens['thetar'] = (
         sheet.cell_value(j + 1, 13), sheet.cell_value(j + 1, 19))
...
class RICHARDS_1D_GLUE(RICHARDS_1D):
    ...
    def _shp_cur(self):
        ...
        soil_par['m'] = 1 - 1 / soil_par['n']
        soil_par['evap_1'] = self.psi2theta(-0.33, soil_par['thetar'], soil_par['thetas'], soil_par['alpha'], soil_par['m'], soil_par['n'])
        soil_par['evap_0'] = self.psi2theta(-15, soil_par['thetar'], soil_par['thetas'], soil_par['alpha'], soil_par['m'], soil_par['n'])
        self.soil_par = soil_par
