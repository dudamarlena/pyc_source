...
# Compiled at: 2018-03-07 07:57:33
    ...
    def _are_all_random_codes_included(self, target, language, clean_headers):
        ...
        for code, heading_parent in self.random_codes.items():
            for item in target['categories']:
                if item[code_col_name] == code and item[heading_col_name] == heading_parent[language] and item[parent_col_name] == heading_parent['parent'] and item[related_col_name] == heading_parent['related_categories']:
                    return True
    ...
    def test_po_content(self):
        ...
        for lang in self.LANGUAGES_TO_TEST:
            ...
            for code, value in self.random_codes.items():
                po_translation = next((item for item in all_translations_in_po if item[0] == value['en']), None)
                self.assertEqual(po_translation[1].encode('utf-8') if _IS_PYTHON2 else str(po_translation[1]), value[lang])
