...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        en_thema_categories = json.loads((open(THEMA_JSON_EN).read()),
        ...
        for thema_category in en_thema_categories:
            ...
            if current_categories.exists():
                current_categories.update(header=(thema_category[HEADING_KEY_NAME]),
                  notes=(thema_category[NOTES_KEY_NAME]),
                  parent=parent)
                updated += 1
            else:
                ThemaCategory.objects.create(code=(thema_category[CODE_KEY_NAME]),
