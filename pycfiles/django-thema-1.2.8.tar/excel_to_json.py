...
# [GCC 8.4.0]
# Embedded file name: /home/syre/work/django-thema/thema/scripts/excel_to_json.py
# Compiled at: 2018-03-07 07:40:11
...
import os, sys, xlrd, json, argparse
from xlrd.biffh import XLRDError
HEADING_COLNAME_BY_LANGUAGE = {'ar': 'Arabic Heading', 
   'da': 'Danish Heading', 
   'de': 'German Heading', 
   'en': 'English Heading', 
   'es': 'Materia español', 
   ...
   'sv': 'Swedish Heading', 
   'tr': 'Başlık'}
NOTES_COLNAME_BY_LANGUAGE = {'ar': 'Arabic Notes', 
   'da': 'Danish Notes', 
   'de': 'German Notes', 
   'en': 'Notes', 
   'es': 'Notas español', 
   ...
   'sv': 'Swedish Notes', 
   'tr': 'Notlar'}
LANGUAGE_INFO_COLNAME_BY_LANGUAGE = {x:'Original language' if x == 'en' else 'New language' for x in NOTES_COLNAME_BY_LANGUAGE}
CODE_COLNAME = 'Code'
PARENT_COLNAME = 'Parent'
RELATED_COLNAME = 'Related (see also)'
ADDITIONS_COLNAME = 'Additions SINCE VERSION 1.1\nChanges SINCE VERSION 1.1\nAdded National Extensions'
...
HEADERS_TO_CHANGE = {CODE_COLNAME: CODE_NEW_COLNAME, 
   PARENT_COLNAME: PARENT_NEW_COLNAME, 
   RELATED_COLNAME: RELATED_NEW_COLNAME}
HEADERS_TO_CHANGE.update({native_name:HEADING_NEW_COLNAME for native_name in HEADING_COLNAME_BY_LANGUAGE.values()})
HEADERS_TO_CHANGE.update({native_name:NOTES_NEW_COLNAME for native_name in NOTES_COLNAME_BY_LANGUAGE.values()})
HEADERS_TO_CHANGE.update({native_name:LANGUAGE_INFO_NEW_COLNAME for native_name in LANGUAGE_INFO_COLNAME_BY_LANGUAGE.values()})
...
class ExcelThemaFileParser(ThemaFileParser):
    ...
    def __init__(self, input_file, thema_version='1.2'):
        ...
        self.xl_sheet = self.xl_book.sheet_by_name(self.THEMA_SHEET_NAME_PATTERN.format(thema_version))
        self.headers = [ cell.value.encode('utf-8') if _IS_PYTHON2 else str(cell.value) for cell in self.xl_sheet.row(0)
                       ]

    def _add_language_info(self, row_values, language):
        """
        Add related category info to the row.

        e.g for category DNP we add related_categories: ["WTL*", "AJF"]
        and the translations for "See also:" and "and its subcategories".
        ...
        see_also_translation = self.xl_sheet.row(SEE_ALSO_TRANSLATION_ROW_INDEX)[language_info_index].value
        and_subcategories_translation = self.xl_sheet.row(AND_SUBCATEGORIES_TRANSLATION_ROW_INDEX)[language_info_index].value
        language_code = self.xl_sheet.row(LANGUAGE_CODE_ROW_INDEX)[language_info_index].value
        language_info = {'language': language_code, 
           'see_also_translation': see_also_translation, 
           'and_subcategories_translation': and_subcategories_translation}
        return (
         language_info, row_values)
    ...
    def parse(self, filter_by=None, clean_headers_language=None):
        ...
        for row_index in range(self.xl_sheet.nrows)[1:]:
            ...
            related_categories = [ category for category in row_values[related_index:additions_index] if category
                                 ]
            row_values[related_index] = related_categories
            if clean_headers_language:
                language_info, row_values = self._add_language_info(row_values, clean_headers_language)
            sheet_as_dict.append(dict(zip(self.headers, row_values)))

        if filter_by:
            sheet_as_dict = [ {k:v for k, v in row.items() if k in filter_by} for row in sheet_as_dict
                            ]
        if clean_headers_language:
            clean_list = []
            for row in sheet_as_dict:
                ...
                for k, v in row.items():
                    item.update({k: v} if k not in HEADERS_TO_CHANGE.keys() else {HEADERS_TO_CHANGE[k]: v})
    ...
    for row in rows:
        ...
        def generate_category_string(code):
            if is_wildcard_category(code):
                lookup = code[:-1]
                postfix = (' {}').format(and_subcategories_translation)
            else:
                lookup = code
                postfix = ''
            related_category = fetch_category(lookup)
            return ('{} {}{}').format(code, related_category['heading'], postfix)

        row[NOTES_NEW_COLNAME] += ('{}{} ').format('' if not row[NOTES_NEW_COLNAME] else '. ', see_also_translation)
        row[NOTES_NEW_COLNAME] += (', ').join(generate_category_string(code) for code in row[RELATED_NEW_COLNAME])
...
def main(language, thema_filepath):
    ...
    try:
        ...
        data = xl_parser.parse([
         ...
         RELATED_COLNAME], language)
        data = add_related_category_info_to_notes(data)
    except XLRDError:
        print('The given file is not valid, please ensure to provide a valid EDItEUR Thema Excel file.')
        exit(1)

    json_file_path = os.path.join(dir_name, JSON_FILENAME_PATTERN.format(language))
    print(('Saving to JSON file {}').format(json_file_path))
    json_file = open(json_file_path, 'w+')
    json_file.write(json.dumps(data))
    json_file.close()
    print(('Processing completed, total amount of thema codes: {}').format(len(data['categories'])))


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('-l', '--language', type=str, choices=HEADING_COLNAME_BY_LANGUAGE.keys(), help='Language to process, e.g. `en` for English, `da` for Danish. Find all supported languages at http://www.editeur.org/151/Thema/', required=True)
    parser.add_argument('-i', '--input_file', type=str, help='File to process', required=True)
