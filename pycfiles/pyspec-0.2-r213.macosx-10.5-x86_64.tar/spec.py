...
# [GCC 8.4.0]
# Embedded file name: /Library/Frameworks/EPD64.framework/Versions/7.0/lib/python2.7/site-packages/pyspec/spec.py
# Compiled at: 2011-05-30 10:43:26
...
__version__ = '$Revision: 212 $'
__author__ = 'Stuart B. Wilkins <swilkins@bnl.gov>'
__date__ = '$LastChangedDate: 2011-05-30 10:43:25 -0400 (Mon, 30 May 2011) $'
__id__ = '$Id: spec.py 212 2011-05-30 14:43:25Z stuwilkins $'
__verbose__ = 1
    ...
    def index(self):
        ...
        while line != '':
            if line[0:2] == '#S':
                ...
                s = int(a[1])
                if s % 5 is 0:
                    print '\x08\x08\x08\x08\x08\x08\x08%5d ' % s,
                    sys.stdout.flush()
                self.findex[s] = pos
            pos = self.file.tell()
            line = self.file.readline()

        print '\x08\x08\x08\x08\x08\x08\x08DONE  '
        self.file.close()
...
class SpecPlot():
    ...
    def show(self, xcol=None, ycol=None, mcol=None, norm=True, doplot=True, errors=True, check_errorbars=True, fmt='ro', new=True, xint=200, yint=200, notitles=False, log=False, twodtype='contour'):
        ...
        if (type(xcol) == list) | (type(xcol) == ndarray):
            x2col = xcol[1]
            xcol = xcol[0]
        elif (type(xcol) != int) & (type(xcol) != str):
...
class SpecPlotCCD():
    ...
    def show(self, imagen=None, size=(4, 3), dark=True, log=False, limits=None):
        """Plot out CCD images.
