...
# [GCC 8.4.0]
# Embedded file name: /Library/Frameworks/EPD64.framework/Versions/7.0/lib/python2.7/site-packages/pyspec/ccd/plotter.py
# Compiled at: 2011-04-25 14:13:36
import numpy as np, matplotlib.pyplot as plt
from matplotlib.colors import LogNorm
from pyspec import fit, fitfuncs, utils
__version__ = '$Revision: 205 $'
__author__ = 'Stuart B. Wilkins <stuwilkins@mac.com>, Sven Partzsch <SvenPartzsch@gmx.de>'
__date__ = '$LastChangedDate: 2011-04-15 14:13:45 -0400 (Fri, 15 Apr 2011) $'
__id__ = '$Id: plotter.py 205 2011-04-15 18:13:45Z stuwilkins $'
...
class CCDPlot():
    ...
    def _draw2D(self):
        ...
        if self.iscale is not None:
            ...
            vmax = data.max()
        if self._log:
            ii = ax1.imshow(np.log10(np.abs(data)), aspect='auto', vmin=vmin, vmax=vmax)
            plt.colorbar(ii, cax=ax4, format='%.2e')
        else:
            ii = ax1.imshow(data, aspect='auto', vmin=vmin, vmax=vmax)
            plt.colorbar(ii, cax=ax4, format='%.2e')
    ...
    def _draw1D(self):
        ...
        ax2.xaxis.set_ticks(np.arange(minmax[0], minmax[1], (minmax[1] - minmax[0]) / 5).tolist(), minor=True)
        ax2.set_xlim(minmax)
        ...
        ax3.yaxis.set_ticks(np.arange(minmax[0], minmax[1], (minmax[1] - minmax[0]) / 5).tolist(), minor=True)
        ax3.set_ylim(minmax)
...
class PlotGrid3D():
    ...
    def plot3D(self):
        ...
        print extent
        mlab.contour3d(data, contours=hm, vmax=ma, transparent=True, vmin=mb, extent=extent)
...
class PlotWindow():
    ...
    def getPlotDetails(self):
        ...
        histBins    : list of number of bins for histograms, default 50"""
        return (
         self._plotType, self._plotLog, self._plotErr, self._plotOrigins, self._plotExtents, self._histBins)
    ...
    def getWinLayout(self):
        ...
        winTitle : title of the window"""
        return (
         self._fig, self._allax, self._figSize, self._plotHor, self._plotVer, self._plotOrd, self._winTitle)
    ...
    def getPlotLayouts(self):
        ...
        plotLegends : list of boolean, show legend if True (only 1D)"""
        return (
         self._plotTitles, self._axesLabels, self._plotKinds, self._dataLabels, self._plotLegends)
    ...
    def getPlotMarker(self, markerSizes=None, markerEdgeWidths=None, errLineWidths=None, capSizes=None):
        ...
        capSizes         : list of error bar cap sizes in pt  , default 5"""
        return (
         self._markerSizes, self._markerEdgeWidths, self._errLineWidths, self._capSizes)
    ...
    def _plot1D(self, i):
        ...
        if self._plotLog[i] == True:
            ax.set_yscale('log')
        ax.errorbar(xVal, yVal, yErr, fmt=plotKind, label=plotLabel, markersize=self._markerSizes[i], markeredgewidth=self._markerEdgeWidths[i], elinewidth=self._errLineWidths[i], capsize=self._capSizes[i])
...
class PlotImages():
    ...
    def getPlotContent(self):
        ...
        plotType   : plot raw images ('raw'), dark image substracted ('dark'),
                     ...
                     'norm' if None"""
        return (
         self._plotSelect, self._plotType)
    ...
    def getPlotLayout(self):
        ...
        plotOrd : order of plotting, horizontal-vertical ('hv') of vertical-horizontal ('vh')"""
        return (
         self._figSize, self._plotHor, self._plotVer, self._plotOrd)
        ...
        for i in range(self._totImNum):
            ...
            if self._plotFlag & 1:
                ...
                if self._logFlag & 1:
                    ...
                    cax = ax.imshow(image, norm=LogNorm())
                else:
                    cax = ax.imshow(image)
...
class PlotGrid():
    ...
    def __init__(self, imProc):
        ...
        if self.imProc.frameMode != 4:
            self.setAxesLabels(['Qx (Å$^{-1}$)', 'Qy (Å$^{-1}$)', 'Qz (Å$^{-1}$)'])
        else:
    ...
    def getPlotFlags(self):
        ...
        flag & 4: histogram of occupation of the grid parts (bins)"""
        return (
         self.plotFlag2D, self.plotFlag1D)
...
class PlotGrid2():
    ...
    def getPlotFlags(self):
        ...
        flag & 4: histogram of occupation of the grid parts (bins)"""
        return (
         self._plotFlag2D, self._plotFlag1D)
    ...
    def getLogFlags(self):
        ...
        binary code, flag & 1: intensity, flag & 2: missing grid parts"""
        return (
         self._logFlag1D, self._logFlag2D)
    ...
    def plotGrid1D(self, calcMode='sum'):
        ...
        plotWin.setWinLayout(figSize=(11, 8.5), plotHor=plotHor, plotVer=3, plotOrd='vh', winTitle=winTitle)
    ...
    def plotGrid2D(self, calcMode='sum'):
        ...
        ax2DLabel = [
         [
          self._axesLabels[2], self._axesLabels[1]],
         [
...
if __name__ == '__main__':
    ...
    sf = spec.SpecDataFile('/home/tardis/spartzsch/data/2010_09_X1A2/ymn2o5_sep10_1', ccdpath='/mounts/davros/nasshare/images/sept10')
    scan = sf[244]
    fp = FileProcessor()
