...
# [GCC 8.4.0]
# Embedded file name: /Library/Frameworks/EPD64.framework/Versions/7.0/lib/python2.7/site-packages/pyspec/calcs/rod.py
# Compiled at: 2010-10-28 09:12:48
from scipy import *
import scipy.fftpack as fft
from scipy.interpolate import splrep, splev
import numpy.ma as masked, pylab, numpy
try:
    import psyco
    psyco.profile()
except ImportError:
...
class BraggRod(object):
    ...
    def _calcFctr(self):
        ...
        self.Fctr = 1.0 / (1 - self.Fctr)

    def _calcF(self, *args, **kwargs):
        r"""Calculate structure factors for all crystal objects
        ...
        height = array([0, 0, self.structure[0].getLattice()[2]])
    ...
    def _calcIntensity(self):
        ...
        if False:
            ...
            points = where(abs(self.A) < 1e-05)[0]
            for p in points:
                sr = splrep(concatenate((self.q[p - 5:p - 1][:, 2], self.q[p + 1:p + 5][:, 2])), concatenate((self.A[p - 5:p - 1].real, self.A[p + 1:p + 5].real)))
                si = splrep(concatenate((self.q[p - 5:p - 1][:, 2], self.q[p + 1:p + 5][:, 2])), concatenate((self.A[p - 5:p - 1].imag, self.A[p + 1:p + 5].imag)))
    ...
    def _calcCorrections(self):
        ...
        if self.params.has_key('footprint'):
            ...
            if p:
                ...
                self.footprintCorrection[self.footprintCorrection > 1] = 1.0
...
def IBraggRoughness(l, theta):
    ...
    I = 1 - 2 * theta * (1 - theta) * (1 - cos(2.0 * pi * l))
    return I
...
def IDiffRoughness(l, theta, L, a1, a2, deltah):
    ...
    I = I * a2 * L * sqrt(pi) * exp(-1.0 * pow(pi * L * deltah / a1, 2))
    return I
...
def gauss(x, c, sigma):
    ...
    y = a * exp(-1.0 * pow(x - c, 2) / (2.0 * pow(sigma, 2)))
    return y
...
def hklmesh(h=0.0, k=0.0, lmin=0.0, lmax=2, lstep=0.01):
