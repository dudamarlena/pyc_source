...
# [GCC 8.4.0]
# Embedded file name: /Library/Frameworks/EPD64.framework/Versions/7.0/lib/python2.7/site-packages/pyspec/calcs/crystal.py
# Compiled at: 2011-06-01 12:07:00
r"""
...
"""
import scipy, sfact, rod, spacegroup, structures, copy as _copy, numpy as np, numpy.ma as masked
try:
    import psyco
except ImportError:
    pass
...
class Atom(object):
    ...
    def __init__(self, name='', Z='', charge='', x=None, y=None, z=None, occ=None, U=None, sg=False, r=None):
    ...
    def setPosition(self, x=0.0, y=0.0, z=0.0, o=1.0):
    ...
    def getElectronDensity(self, x=0.0):
...
class Crystal(object):
    ...
    """
    N_A = 6.02214179e-23
    R_0 = 2.8179402894e-05

    def __init__(self, a=None, b=None, c=None, alpha=90.0, beta=90.0, gamma=90.0, lattice=None):
        ...
        if lattice is None:
            if a is None:
                self.lattice = scipy.array([5.0, 5.0, 5.0, 90.0, 90.0, 90.0])
            else:
    ...
    def calcF(self, q, anom=True):
        ...
        for a in self.atoms:
            r = lattice * a.r
            phase = exp(complex(0, 1.0) * inner(r, q))
                ...
                for atom in self.atoms:
                    for generator in generators:
                        ...
                        if self.sg.isCentric():
                            centric_generator = generator * -1.0
    ...
    def applyGenerator(self, atom, generator):
        r = atom.getPositionVector()
        r = hstack((r, array([1.0])))
        r_eq = dot(generator, r.T)
        while (r_eq[:3] < 0.0).sum() or (r_eq[:3] >= 1.0).sum():
            r_eq = r_eq + hstack(((r_eq[:3] < 0.0) * 1.0, array([1])))
            r_eq = r_eq - hstack(((r_eq[:3] >= 1.0) * 1.0, array([1])))
        ...
        if not self.isAtomAt(r_eq[0:3], atom.getOccupancy()):
            ...
            U = atom.getU()
            self.addAtom(r=r_eq[0:3], occ=occ, name=name, Z=Z, U=U, charge=charge, sg=True)
            return True
        return False

    def isAtomAt(self, r, occ=1.0, tol=5e-05):
