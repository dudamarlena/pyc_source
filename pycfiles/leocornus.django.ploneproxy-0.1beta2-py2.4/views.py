...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/leocornus/django/ploneproxy/views.py
# Compiled at: 2010-05-31 09:12:25
"""
...
def login(request, template_name='login.html', redirect_field_name=PLONEPROXY_REDIRECT_FIELD_NAME):
    """
    ...
    if request.method == 'POST':
        ...
        if not redirect_to or '//' in redirect_to or ' ' in redirect_to:
            ...
            user = authenticate(username=username, password=password, loginurl=loginurl)
            if user is None:
    ...
    resDict['forgot_pw_url'] = prepareForgotPasswordURL(request, redirect_field_name, lang)
...
def mailPassword(request, template_name='mail_password.html', redirect_field_name=PLONEPROXY_REDIRECT_FIELD_NAME):
    """
    ...
    return render_to_response(template_name, responseDict, context_instance=RequestContext(request))


def passwordReset(request, template_name='password_reset.html', redirect_field_name=PLONEPROXY_REDIRECT_FIELD_NAME):
    """
    ...
    lang = get_language()
    (lang_name, lang_link) = prepareOtherLang(request, redirect_field_name, lang, PLONEPROXY_TOKEN_FIELD_NAME)
    responseDict[settings.PLONEPROXY_LANG_FIELD_NAME] = lang
    responseDict['lang_name'] = lang_name
    responseDict['lang_link'] = lang_link
    if request.method == 'POST':
        ...
        elif resetPlonePassword(request, redirect_to, token, userId, password1):
            responseDict['confirm_password_reset'] = 'password success!'
            responseDict['redirect_link'] = '%s%s' % (getBaseURL(request), redirect_to)
        else:
            responseDict['error'] = 'we got error!'
