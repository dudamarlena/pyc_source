...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/leocornus/django/ploneproxy/utils.py
# Compiled at: 2010-06-01 08:59:24
"""
...
def prepareForgotPasswordURL(request, redirect_field, current_lang, view='leocornus.django.ploneproxy.views.mailPassword'):
    """
    build the URL based on the request and view class name.
    """
    base_url = reverse(view)
    url = '%s?%s=%s' % (base_url, settings.PLONEPROXY_LANG_FIELD_NAME, current_lang)
...
def resetPlonePassword(request, redirect_to, token, userId, newPassword):
    ...
    if response['status'] == '302' and response['location'].endswith(PLONE_PASSWORD_RESET_FINISH):
        return True
    else:
        return False
