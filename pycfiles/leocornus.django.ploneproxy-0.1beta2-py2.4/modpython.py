...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/leocornus/django/ploneproxy/authen/handlers/modpython.py
# Compiled at: 2010-06-04 14:11:42
"""
...
def authenhandler(request):
    ...
    if len(pwreset) > 0:
        ...
        util.redirect(request, '%s?%s=%s&%s=%s' % (pwreset_url, PLONEPROXY_REDIRECT_FIELD_NAME, pwreset[0][0], PLONEPROXY_TOKEN_FIELD_NAME, pwreset[0][1]))
        return apache.OK
    if isValidSession(request):
        return apache.OK
    else:
        login_url = options.get('PLONEPROXY_LOGIN_URL', '/ext/login')
        util.redirect(request, '%s?%s=%s' % (login_url, PLONEPROXY_REDIRECT_FIELD_NAME, request.unparsed_uri))
...
def isGreenRequest(req):
    ...
    """
    if not (req.unparsed_uri.endswith(PLONE_LOGIN_FORM) or req.unparsed_uri.endswith(PLONE_MAIL_PASSWORD_FORM) or req.unparsed_uri.endswith(PLONE_PASSWORD_RESET_FORM)):
        return False
...
def checkPasswordresetRequest(req):
    ...
    """
    return re.findall('(.*)/passwordreset/([0-9a-zA-Z]{32})$', req.unparsed_uri)
...
def isResourcesRequest(req):
    ...
    """
    if not (req.unparsed_uri.endswith('css') or req.unparsed_uri.endswith('js') or req.unparsed_uri.endswith('kss') or req.unparsed_uri.endswith('gif') or req.unparsed_uri.endswith('jpg') or req.unparsed_uri.endswith('png')):
        return True
    return True