...
class rplx(object):
    ...
    def fullmatch(self, string, pos=1, endpos=None, **kwargs):
        if endpos is not None:
            ...
            endposIndex = len(string)
        m, l, a, ttotal, tmatch = self._internal_rplx.engine.match(self._internal_rplx, adapt23.bytes23(string[:endposIndex]), pos, adapt23.bytes23('json'))
        if a and m == False:
            raise_halt_error()
        else:
            if m == False or l != 0:
