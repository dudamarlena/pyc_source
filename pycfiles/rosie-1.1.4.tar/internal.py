...
librosie_system = _librosie_config(b'*system*')
librosie_local = _librosie_config(b'*local*')
_librosie_name = None
...
def _new_cstr(bstring=None):
    if bstring is None:
        obj = ffi.new(b'struct rosie_string *')
...
def load(path=None, quiet=False):
    ...
    if _lib:
        if quiet:
            return
        raise RuntimeError(b'librosie has already been loaded from ' + _librosie_path)
    if path == None:
        try:
            _load_from(b'//')
        except OSError:
            try:
                ...
                raise RuntimeError(b'Cannot find librosie in local directory or system installation')

    elif path == librosie_system:
        try:
            _load_from(b'')
        except OSError:
            raise RuntimeError(b'Cannot find librosie (no system installation)')

    elif path == librosie_local:
        try:
            _load_from(b'//')
        except OSError:
            raise RuntimeError(b'Cannot find librosie in current directory')

    else:
        try:
            _load_from(path)
        except OSError:
            raise RuntimeError((b'Cannot find librosie at specified path {}').format(path))
...
def _load_from(path_string):
    ...
    if not _librosie_name:
        ...
        if ostype == b'Darwin':
            _librosie_name = b'librosie.dylib'
        else:
            _librosie_name = b'librosie.so'
    if path_string[0:2] == b'//':
...
class engine(object):
    ...
    def __init__(self):
        ...
        if self.engine == ffi.NULL:
            raise RuntimeError(b'librosie: ' + str(_read_cstr(Cerrs)))
    ...
    def compile(self, exp):
        ...
        if ok != 0:
            raise RuntimeError(b'compile() failed (please report this as a bug)')
        if pat.id[0] == 0:
            pat = None
        return (
    ...
    def load(self, src):
        Cerrs = _new_cstr()
        Csrc = _new_cstr(src)
        Csuccess = ffi.new(b'int *')
        Cpkgname = _new_cstr()
        ok = _lib.rosie_load(self.engine, Csuccess, Csrc, Cpkgname, Cerrs)
        if ok != 0:
            raise RuntimeError(b'load() failed (please report this as a bug)')
    ...
    def loadfile(self, fn):
        ...
        if ok != 0:
            raise RuntimeError(b'loadfile() failed (please report this as a bug)')
        errs = _read_cstr(Cerrs)
        pkgname = _read_cstr(Cpkgname)
        return (Csuccess[0], pkgname, errs)
    ...
    def import_pkg(self, pkgname, as_name=None):
        ...
        if ok != 0:
            raise RuntimeError(b'import() failed (please report this as a bug)')
    ...
    def match(self, pat, input, start, encoder):
        if pat is None or pat.id[0] == 0:
            raise ValueError(b'invalid compiled pattern')
        Cmatch = ffi.new(b'struct rosie_matchresult *')
        Cinput = _new_cstr(input)
        ok = _lib.rosie_match(self.engine, pat.id[0], start, encoder, Cinput, Cmatch)
        if ok != 0:
            raise RuntimeError(b'match() failed (please report this as a bug)')
        ...
        if Cmatch.data.ptr == ffi.NULL:
            ...
            if Cmatch.data.len == 2:
                raise ValueError(b'invalid output encoder')
    ...
    def trace(self, pat, input, start, style):
        if pat.id[0] == 0:
            raise ValueError(b'invalid compiled pattern')
        Cmatched = ffi.new(b'int *')
        Cinput = _new_cstr(input)
        Ctrace = _new_cstr()
        ok = _lib.rosie_trace(self.engine, pat.id[0], start, style, Cinput, Cmatched, Ctrace)
        if ok != 0:
            raise RuntimeError(b'trace() failed (please report this as a bug): ' + str(_read_cstr(Ctrace)))
        if Ctrace.ptr == ffi.NULL:
            if Ctrace.len == 2:
                raise ValueError(b'invalid trace style')
            elif Ctrace.len == 1:
                raise ValueError(b'invalid compiled pattern')
    ...
    def matchfile(self, pat, encoder, infile=None, outfile=None, errfile=None, wholefile=False):
        ...
        if ok != 0:
            raise RuntimeError(b'matchfile() failed: ' + str(_read_cstr(Cerrmsg)))
        if Ccin[0] == -1:
            if Ccout[0] == 2:
                raise ValueError(b'invalid encoder')
            elif Ccout[0] == 3:
                raise ValueError(str(_read_cstr(Cerrmsg)))
            elif Ccout[0] == 4:
                raise ValueError(b'invalid compiled pattern (already freed?)')
            else:
                raise ValueError(b'unknown error caused matchfile to fail')
        return (
         Ccin[0], Ccout[0], Ccerr[0])

    def read_rcfile(self, filename=None):
        Cfile_exists = ffi.new(b'int *')
        if filename is None:
        ...
        if ok != 0:
            raise RuntimeError(b'read_rcfile() failed (please report this as a bug)')
    ...
    def execute_rcfile(self, filename=None):
        ...
        if ok != 0:
            raise RuntimeError(b'execute_rcfile() failed (please report this as a bug)')
        messages = _read_cstr(Cmessages)
        messages = messages and json.loads(messages)
        if Cfile_exists[0] == 0:
            return (None, messages)
    ...
    def parse_expression(self, exp):
        ...
        if ok != 0:
            raise RuntimeError(b'parse_expression failed (please report this as a bug)')
        return (
    ...
    def parse_block(self, block):
        ...
        if ok != 0:
            raise RuntimeError(b'parse_block failed (please report this as a bug)')
        return (
         _read_cstr(Cparsetree), _read_cstr(Cmessages))
    ...
    def expression_refs(self, exp):
        ...
        if ok != 0:
            raise RuntimeError(b'expression_refs failed (please report this as a bug)')
        return (
    ...
    def block_refs(self, block):
        ...
        if ok != 0:
            raise RuntimeError(b'block_refs failed (please report this as a bug)')
        return (
         _read_cstr(Crefs), _read_cstr(Cmessages))
    ...
    def expression_deps(self, exp):
        ...
        if ok != 0:
            raise RuntimeError(b'expression_deps failed (please report this as a bug)')
        return (
    ...
    def block_deps(self, block):
        ...
        if ok != 0:
            raise RuntimeError(b'block_deps failed (please report this as a bug)')
        return (
         _read_cstr(Cdeps), _read_cstr(Cmessages))
    ...
    def config(self):
        ...
        if ok != 0:
            raise RuntimeError(b'config() failed (please report this as a bug)')
    ...
    def libpath(self, libpath=None):
        ...
        if ok != 0:
            raise RuntimeError(b'libpath() failed (please report this as a bug)')
        if libpath is None:
    ...
    def alloc_limit(self, newlimit=None):
        limit_arg = ffi.new(b'int *')
        usage_arg = ffi.new(b'int *')
        if newlimit is None:
            limit_arg[0] = -1
        else:
            if newlimit != 0 and newlimit < 8192:
                raise ValueError(b'new allocation limit must be 8192 KB or higher (or zero for unlimited)')
            limit_arg = ffi.new(b'int *')
            limit_arg[0] = newlimit
        ok = _lib.rosie_alloc_limit(self.engine, limit_arg, usage_arg)
        if ok != 0:
            raise RuntimeError(b'alloc_limit() failed (please report this as a bug)')
        return (
         limit_arg[0], usage_arg[0])

    def __del__(self):
        if hasattr(self, b'engine') and self.engine != ffi.NULL:
            e = self.engine
