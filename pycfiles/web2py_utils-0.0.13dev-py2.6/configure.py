...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/web2py_utils/configure.py
...
class Configure:
    ...
    def define_tables(self, migrate=True, fake_migrate=False):
        ...
        """
        self.db.define_table('settings', Field('kkey'), Field('name'), Field('value', 'text'), Field('value_type'), Field('description', 'text'), Field('created_on', 'datetime'), Field('modified_on', 'datetime'), migrate=migrate, fake_migrate=fake_migrate)
    ...
    def verify(self, kkey, settings):
        ...
        for (name, info) in settings.iteritems():
            row = self.db((self.db.settings.kkey == kkey) & (self.db.settings.name == name)).select().first()
            if not row:
                ...
                if isinstance(value, bool):
                    ...
                    type = 'str'
                record = self.db.settings.insert(kkey=kkey, name=name, value=value, value_type=type, description=info.get('description', None))
