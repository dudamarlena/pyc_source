...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/web2py_utils/test_runner.py
# Compiled at: 2010-05-29 23:53:12
"""
Web2py Test Runner
==================
...
"""
import unittest, glob, fnmatch, sys, os, doctest, cStringIO
...
def webtest():
    ...
    app = TestApp(wsgibase, extra_environ=dict(TEST_APP=_TEST_APP_KEY, TEST_METHOD='WebTest'))
    return app
...
def new_env(app='init', controller=None):
    ...
    _env = shell.env(app, c=controller, import_models=True, dir=dir, extra_request={'wsgi': {'environ': {'TEST_APP': _TEST_APP_KEY, 
                            'TEST_METHOD': 'FakeEnv'}}})
    if controller:
...
def run(path=None, app='welcome', test_key='secret', test_options={'verbosity': 1}, test_report=None, coverage_report=None, coverage_exclude=None, coverage_include=None, DO_COVER=True, DO_NOSE=True):
    """
    ...
    if _NOSE and DO_NOSE:
        nose.core.run(suite=suite, config=nose.config.Config(**test_options))
    else:
    ...
    if _COVER and DO_COVER:
        ...
        if isinstance(coverage_report, str) and not test_report:
            ...
            STRINGIO = True
        omit = [
         'gluon', '/usr', os.path.abspath(__file__)]
        folders_to_omit = [
         'languages', 'tests', 'docs', 'private']
