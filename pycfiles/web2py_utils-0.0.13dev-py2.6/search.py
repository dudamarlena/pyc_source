...
class Levenshtein():
    ...
    def suggestion(original, searchable, number_of_matches=3):
        ...
        for txt in searchable:
            set = (
             Levenshtein.length(original, txt), txt)
   ...
   """
    ...
    def __init__(self, haystack, **kwargs):
      ...
      """
        ...
        if kwargs.has_key('min_sim'):
            self.min_sim(kwargs['min_sim'])
        if kwargs.has_key('warp'):
            self.warp(kwargs['warp'])
        if kwargs.has_key('ic'):
            self.ic(kwargs['ic'])
        if kwargs.has_key('only_alnum'):
            self.only_alnum(kwargs['only_alnum'])
        if kwargs.has_key('ngram_len'):
            self.ngram_len(kwargs['ngram_len'])
    ...
    def ngramify(self, haystack, ic=None, only_alnum=None, padding=None, noise=None):
        ...
        if ic is None:
            ic = self.__ic
        if only_alnum is None:
            only_alnum = self.__only_alnum
        if padding is None:
            padding = 'X' * self.__padding
        if noise is None:
            noise = self.__noise
        seen = {}
        grams = {}
    ...
    def getSimilarStrings(self, string):
      ...
      """
        ...
        for i in xrange(numgram):
            ...
            if not self.__ngram_index.has_key(ngram):
                continue
            ...
            for match in matches:
                ...
                if not ngram_buf.has_key(ngram):
                    ngram_buf[ngram] = {}
                ...
                if ngram_buf[ngram][actName] > 0:
                    ...
                    if not siminfo.has_key(actName):
                        siminfo[actName] = {'name': 0, 'len': 0}
    ...
    def getBestMatch(self, needle, count=1):
      ...
      RETURNS:
         ...
         which it matched.
      """
        if type(needle) is not StringType:
            raise TypeError, 'needle must be of type string!'
        if type(count) is not IntType:
