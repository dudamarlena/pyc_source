...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/web2py_utils/wordpress2py.py
...
plugin_wordpress2py.meta = {'title': 'Wordpress To Web2py', 
   ...
   'copyright': 'GPL v2'}
default_mengu_blog_schema = {'categories': {'name': 'category/title'}, 
   'posts': {'doit = True if data["type"] == "post" else False': {'title': 'post/title', 
                                                                  'content': 'post/body', 
                                                                  'post_date': 'post/dateline'}, 
             'doit = True if data["type"] == "page" else False': {'title': 'page/title', 
                                                                  'content': 'page/content'}}, 
   'comments': {'id_post': 'comment/post_id', 
                'author': 'comment/name', 
                'author_email': 'comment/email', 
                'content': 'comment/comment', 
                'date': 'comment/dateline'}, 
   'post_categories': {'id_category': 'relations/category', 
                       'id_post': 'relations/post'}}
default_schema = {'categories': {'name': 'category/title', 
   ...
   'comments': {'id_post': 'comment/id_post', 
   ...
   'post_categories': {'id_category': 'category_relations/category', 
                       'id_post': 'category_relations/post'}, 
   'post_tags': {'id_tag': 'tag_relations/tag', 
...
def custom_migrate_to_mengu_database(db):
    ...
    for post in data['posts']:
        if post['type'] == 'post':
            post_id = db.post.insert(title=post['title'], body=post['content'], dateline=post['pub_date'])
            for c in post['categories']:
                db.relations.insert(post=post_id, category=category_ids[c])

            for c in post['comments']:
                comment_id = db.comment.insert(post_id=post_id, name=c['author'], email=c['author_email'], comment=c['content'], dateline=c['date'])

        elif post['type'] == 'page':
            post_id = db.page.insert(title=post['title'], content=post['content'])
...
def word2py(xml_file):
    ...
    db = {'title': tree.find('channel/title').text.encode('utf-8'), 
    ...
    for c in tree.findall('channel/{http://wordpress.org/export/1.0/}category'):
        a_cat = {'name': c.find('{http://wordpress.org/export/1.0/}cat_name').text, 
    ...
    for t in tree.findall('channel/{http://wordpress.org/export/1.0/}tag'):
        a_tag = {'name': t.find('{http://wordpress.org/export/1.0/}tag_name').text, 
    ...
    for item in tree.findall('channel/item'):
        ...
        for comment in comments:
            the_comment = {'id': comment.find('{http://wordpress.org/export/1.0/}comment_id').text, 
