...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/web2py_utils/py2jquery/py2jquery.py
...
__events_live_notsupported__ = [
 'blur', 'focus', 'mouseenter',
 'mouseleave', 'change', 'submit']
__counters__ = [
 'up', 'down']
__js_scripts__ = [
 'embed', 'var', 'setvar', 'function',
 'alert', 'ajax', 'replace',
 'datetime_format']
__jquery_plugins__ = {'jquery': {'name': 'jquery', 'version': 1.32, 
              'provides': [
                         'jquery'], 
              'requires': [], 'files': [
                      ('jquery.min.js', 'http://ajax.googleapis.com/ajax/libs/jquery/1.3.2/jquery.min.js')]}, 
   'jquery.ui': {'name': 'jquery.ui', 'version': 1.72, 
                 'provides': [
                            'jquery.ui'], 
                 'requires': [
                            'jquery'], 
   ...
   'jquery.rating': {'name': 'jquery.rating', 'version': 1.5, 
                     'provides': [
                                'jquery.rating'], 
                     'requires': [
                                'jquery'], 
                     'files': [
                             ('jquery.rating.js', 'http://hg.thadeusb.com/public/.r/Web/py2jquery/raw-file/4caabf1997c8/jquery.plugins/jquery.rating/jquery.rating.js'),
   ...
   'jquery.jeditable': {'name': 'jquery.jeditable', 'version': 1.0, 
                        'provides': [
                                   'jquery.jeditable'], 
                        'requires': [
                                   'jquery'], 
                        'files': [
                                ('jquery.jeditable.mini.js', 'http://www.appelsiini.net/download/jquery.jeditable.mini.js')]}, 
   'jquery.timer': {'name': 'jquery.timer', 'version': 1.2, 
                    'provides': [
                               'jquery.timer'], 
                    'requires': [
                               'jquery'], 
                    'files': [
                            ('jquery.timer.js', 'http://plugins.jquery.com/files/jquery.timers-1.2.js.txt')]}, 
   'date.format': {'name': 'date.format', 'version': 1.2, 
                   'provides': 'date.format', 
                   'requires': [], 'files': [
                           ('date.format.js', 'http://stevenlevithan.com/assets/misc/date.format.js')]}}
__cdn_resources__ = {'jquery': {'name': 'jquery', 
              'provides': [
                         'jquery'], 
              'requires': [], 'files': [
   ...
   'jquery.ui.theme.darkness': {'name': 'jquery.ui.theme.darkness', 
                                'provides': [
   ...
   'jquery.ui.theme.smoothness': {'name': 'jquery.ui.theme.smoothness', 
                                  'provides': [
   ...
   'jquery.ui.theme.redmond': {'name': 'jquery.ui.theme.redmond', 
                               'provides': [
   ...
   'jquery.ui.theme.humanity': {'name': 'jquery.ui.theme.humanity', 
                                'provides': [
...
def is_valid_selector(obj, r=True):
    ...
    Returns True/False
    """
    if isinstance(obj, str) or isinstance(obj, Script) or hasattr(obj, 'xml') and obj['_id']:
        return True
        if r:
            if not hasattr(obj, 'xml'):
                raise TypeError('Invalid object for event. No XML Function.')
            raise obj['_id'] or ValueError('Invalid object for event. No ID Attribute.')
        else:
...
class js(object):
    ...
    def ajax(self, type='POST', url='#', extra_data='', data='form:first', success='eval(msg);'):
        ...
        return 'jQuery.ajax({type:"%s", url:"%s", data:%s jQuery("%s").serialize(), success: function(msg){%s} });' % (
         type, url, extra_data, data, success)
...
class Manager(object):
    ...
    def require(self, name, subfolder='', upgrade=False):
        ...
        pieces = (
         request.folder, 'static', subfolder, plugin['name'])
...
class Call(Script):
    ...
    def xml(self):
        xml = ''
        xml += js.ajax(type='POST', url=self.url, extra_data=self.extra_data, data=self.data, success=self.success)
        xml = self.wrap(xml)
        return xml
...
class Event(Script):
    ...
    def __init__(self, event='click', event_obj=None, call=None, extra_data='', event_args=False, rebind=False, **kwargs):
        ...
        if event_args:
            call.extra_data += '"event_target_id=" + encodeURIComponent(e.target.id) + "&event_target_html=" + encodeURIComponent(jQuery(e.target).wrap("<div></div>").parent().html()) + "&event_pageX=" + e.pageX + "&event_pageY=" + e.pageY + "&event_timeStamp=" + e.timeStamp + "&" +'
        if rebind:
            self.bind = 'live'
...
class Rating(object):
    ...
    def form_widget(field, value, **attributes):
        ...
        return DIV(INPUT(_name=field.name, _type='hidden', _id='rating_input_%s' % id(field)), DIV(_class='rating_widget', _val=value, _num=len(requires.theset), _dbid=id(field)), _class='rating')
...
class Stars(object):
    ...
    def dom(self, manager):
        ...
        for i in range(1, self.num + 1):
            ...
            call = Call(callback=self.call.url, args=self.call.args, success=self.call.success, data=self.call.data, extra_data=self.call.extra_data, uuid=self.call.uuid, name=self.call.name, is_function=self.call.is_function)
