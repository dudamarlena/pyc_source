...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/web2py_utils/utils.py
# Compiled at: 2010-07-30 03:11:11
import os
from gluon.html import DIV, URL, H2, FORM, LABEL, P, XML, OL, LI
from gluon.utils import hash
STRFTIME = {'time_date': '%I:%M %p %d-%m-%Y'}
...
class LayoutManager:
    ...
    def __init__(self, request, response, url, name, base='layout.html', autooverride=True):
        """
...
class AdminManager:
    ...
    def url(self, function, args=None, vars=None):
        return self.URL(r=self.request, c='admin', f='dispatch/%s' % function.__name__, args=args, vars=vars)
...
def CONFIRM_BOX(request, session, title='Delete Record', label='Are you sure you want to delete this record?', content='', func_yes=lambda v: v, func_no=lambda v: v):
    form = FORM(DIV(P(LABEL(label), _class='centered'), P(BUTTON('Yes', _type='submit', _name='yes', _value='yes'), _class='centered'), P(BUTTON('No', _type='submit', _name='no', _value='no'), _class='centered')))
    html = DIV(H2(title), DIV(DIV(form, P(content), _id='padding'), _id='user_action'))
