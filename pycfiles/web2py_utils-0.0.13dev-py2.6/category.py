...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/web2py_utils/category.py
...
class CategoryPlugin:
    ...
    def define_tables(self, migrate_db=True):
        self._table = self._db.define_table(self._tablename, Field('name', 'string', unique=True), Field('description', 'text'), Field('lft', 'integer'), Field('rgt', 'integer'), migrate=migrate_db)
    ...
    def ancestors(self, name, *fields):
        ...
        """
        node = self._db(self._db[self._tablename].name == name).select().first()
        return self._db(self._db[self._tablename].lft < node.lft)(self._db[self._tablename].rgt > node.rgt).select(orderby=self._db[self._tablename].lft, *fields)
    ...
    def as_dict(self):
        categories = self._db().select(self._db[self._tablename].ALL, orderby=self._db[self._tablename].lft)
        root = {}
    ...
    def select_widget(self):
        ...
        def widget(field, value, **attributes):
            ...
            for cat in categories:
                ...
                s += cat.name
                branch = OPTION(XML(s), _value=cat.id, _class='branch')
                tree.append(branch)
                rgt.append(cat.rgt)
    ...
    def checkboxes_widget(self):
        ...
        def widget(field, value, **attributes):
            ...
            for cat in categories:
                ...
                p.append(LI(INPUT(_type='checkbox', _value=cat.id, value=str(cat.id) in values, _name=field.name, requires=field.requires), cat.name, _class='leaf'))
                tree.append(branch)
                rgt.append(cat.rgt)
