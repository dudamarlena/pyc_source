...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyf/services/core/router.py
...
class Router(object):
    ...
    def process_using_dispatch(self, doc_file, variant=None, encoding=None, options=None):
        dispatch = DBSession.merge(self.dispatch)
        try:
            returns = dispatch.process_file(doc_file, progression_callback=create_function(self.emit_event, before_args=[
             'progress'], caller_args_count=1), message_callback=create_function(self.emit_event, before_args=[
             'info'], caller_args_count=1), variant=variant, encoding=encoding, options=options)
    ...
    def process_tube(self, variant=None, options=None, source=None, raises_error=False):
        tube = DBSession.merge(self.tube)
        try:
            returns = tube.flow(variant=variant, progression_callback=create_function(self.emit_event, before_args=[
             'progress'], caller_args_count=1), message_callback=create_function(self.emit_event, before_args=[
             'info'], caller_args_count=1), options=options, source=source)
            self.emit_event('success', returns)
            self.update_run_date(variant_name=variant)
        except Exception, e:
            logger.exception(e)
