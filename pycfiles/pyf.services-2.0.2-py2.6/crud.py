...
class SecureCrudRestController(RestController):
    ...
    def docs(self):
        return dict(title=self.model.__name__, description=publish_parts(trim(self.model.__doc__), writer_name='html')['html_body'], contents=((key, publish_parts(trim(value.__doc__), writer_name='html')['html_body']) for (key, value) in self.model.__dict__.iteritems() if not key.startswith('_')))
