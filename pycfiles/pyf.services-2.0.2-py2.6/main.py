...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyf/services/model/main.py
...
class Dispatch(DeclarativeBase):
    ...
    def process_file(self, file_object, variant=None, progression_callback=None, message_callback=None, encoding=None, options=None):
        variant_name = None
        if variant is not None:
            variant_name = variant
        return self.tube.flow(variant=variant_name, progression_callback=progression_callback, message_callback=message_callback, source=file_object, descriptor=self.descriptor.get_descriptor_object(encoding=encoding), options=options)
...
class Tube(DeclarativeBase):
    ...
    def __json__(self):
        props = dict(variant_names=self.variant_names, dispatchs=[ dispatch.id for dispatch in self.dispatchs ])
        for attribute in self._sa_class_manager.mapper.c:
            key = attribute.key
            props[key] = getattr(self, key)
        ...
        for parent in config_tree.getiterator():
            for node in parent:
                if node.tag == 'include':
                    ...
                    remove_producer = node.get('remove_producer', 'true').lower() in ('true',
                                                                                      'on',
                                                                                      'yes',
                                                                                      '1')
                    if other_tube:
                        if is_layered:
                            other_conf = other_tube.get_layered_config(variant=variant_name)
                        else:
                            other_conf = other_tube.get_layered_config(variant=None)
                        root_node = other_conf.find('process/node')
                        nodes_to_include = list()
                        if remove_producer and root_node.get('type') == 'producer':
                            nodes_to_include = root_node.findall('children/node')
                        else:
    ...
    def get_post_process_config(self, variant=None):
        ...
        if post_process_node is None:
            ...
            if first_node.get('type') != 'producer':
                new_node = ET.Element('node', type='producer', pluginname='descriptorsource', name='postprocess_source')
                ...
                for node in cnode:
                    if len(node) > 0:
                        ...
                        for attribute in node.keys():
                            pdict['%s_%s' % (node.tag, attribute)] = node.get(attribute)
    ...
    def get_valid_options(self):
        ...
        if params_node is not None:
            ...
            other_options = valid_options_node is not None and map(str.strip, valid_options_node.text.split(',')) or list()
    ...
    def flow(self, source=None, variant=None, process_name=None, progression_callback=None, message_callback=None, descriptor=None, params=None, options=None):
        """ Launches the tube.
        ...
        if self.needs_source and source is None:
            ...
            raise ValueError('You must define a file-like source if you provide a descriptor')
        if params is None:
            params = dict()
        if descriptor is not None and source is not None:
            ...
            params['data'] = source
        if 'last_run_date' not in params:
            params['last_run_date'] = self.get_last_run_date(variant=variant)
        manager = self.get_manager(variant)
        return manager.process(process_name, params=params, progression_callback=progression_callback, message_callback=message_callback, finalize=False, options=options)

    def launch_post_process(self, source, variant=None, progression_callback=None, message_callback=None):
        ...
        return manager.process(self.process_name, params=params, progression_callback=progression_callback, message_callback=message_callback, finalize=True, process_config=post_process_config)
    ...
    def get_variant_names(self, show_inactive=True):
        ...
        for layer in self.layers:
            if layer.variant_name not in names and (layer.active or show_inactive):
    ...
    def get_tree_graph(self, variant=None):
        ...
        def get_step_info(node):
            ...
            step_dict = dict(name=node.get('name'), data=dict(type=node.get('type'), plugin_name=source), id=random.randint(1, 1000000), children=list())
    ...
    def get_network_graph(self, full_tree=True, variant=None, all_infos=False, stripped_nodes=[
 'children',
 'code',
 'joiner'], compute_position=True):
        if full_tree:
            etree = ET
            config = self.get_layered_config(variant=variant)
        else:
        ...
        for parent in config.getiterator():
            for node in parent:
                if node.tag == 'node':
                    node_info = dict(type=node.get('type', 'adapter'), source=node.get('from', 'plugin'), pluginname=node.get('pluginname'), name=node.get('name'))
                    ...
                    if all_infos:
                        conf_nodes = filter(lambda n: n.tag not in stripped_nodes, node)
...
class TubeLayer(DeclarativeBase):
    ...
    def apply_on_node(self, primary_node):
        from pyf.services.core.modifiers import apply_modifier_set
        return apply_modifier_set(self.get_modifier_tree(), primary_node)
