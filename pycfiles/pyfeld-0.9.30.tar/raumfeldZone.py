...
# [GCC 8.4.0]
# Embedded file name: pyfeld/raumfeldZone.py
...
class RaumfeldZone:
    ...
    def update_position_info(self):
        ...
        if 'TrackDuration' in self.position:
            ...
            self.position['TrackDurationInfo'] = {'Tsecs': int(components[0]) * 3600 + int(components[1]) * 60 + int(components[2]), 
        ...
        if 'AbsTime' in self.position:
            ...
            self.position['AbsTimeInfo'] = {'Tsecs': int(components[0]) * 3600 + int(components[1]) * 60 + int(components[2]), 
               'hour': int(components[0]), 
               'minute': int(components[1]), 
               'seconds': int(components[2])}
        else:
        ...
        if 'RelTime' in self.position:
            ...
            self.position['RelTimeInfo'] = {'Tsecs': int(components[0]) * 3600 + int(components[1]) * 60 + int(components[2]), 
               'hour': int(components[0]), 
               'minute': int(components[1]), 
               'seconds': int(components[2])}
        else:
