...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\tdm_loader\tdm_loader.py
# Compiled at: 2019-10-08 08:27:30
...
"""
import os.path, re, xml.etree.ElementTree as ElementTree, warnings, numpy as np
try:
...
class OpenFile(object):
    ...
    def __init__(self, tdm_path, tdx_path='', encoding='utf-8'):
        ...
        self._xml_chgs = list(map(lambda usi: self._root.find(".//tdm_channelgroup[@id='{0}']".format(usi)), re.findall('id\\("(.+?)"\\)', self._xml_tdm_root.findtext('channelgroups'))))
    ...
    def _channels_xml(self, channel_group, occurrence=0):
        ...
        chs = list(map(lambda usi: self._root.find(".//tdm_channel[@id='{0}']".format(usi)), OpenFile._get_usi_from_txt(chg.findtext('channels'.format(chg.get('id'))))))
        return chs
    ...
    def channel_group_search(self, search_term):
        ...
        chg_names = [x.text for x in self._root.findall('.//tdm_channelgroup/name') if x.text is not None]
        search_term = search_term.upper().replace(' ', '')
        found_terms = [name for name in chg_names if name.upper().replace(' ', '').find(search_term) >= 0]
        ind = []
    ...
    def channel_search(self, search_term):
        ...
        for j, chg in enumerate(self._xml_chgs):
            ...
            if search_term == '':
                ...
                found_terms = [ch.findtext('name') for ch in chs if ch.findtext('name') is not None if ch.findtext('name').upper().replace(' ', '').find(str(search_term)) >= 0]
            for name in found_terms:
    ...
    def channel(self, channel_group, channel, occurrence=0, ch_occurrence=0):
        ...
        if inc.get('external') is None:
            data = list(map(lambda x: x.text, self._root.findall(".//{0}[@id='{1}']/values/*".format(datatype, data_usi))))
        else:
            ext_attribs = self._root.find(".//file/block[@id='{0}']".format(inc.get('external'))).attrib
            data = np.memmap((self._tdx_path), offset=(int(ext_attribs['byteOffset'])),
              shape=(
             int(ext_attribs['length']),),
    ...
    def channel_dict(self, channel_group, occurrence=0):
        ...
        if isinstance(channel_group, int):
            ...
            if len(name_doublets) > 0:
                warnings.warn('Duplicate channel name(s): {}'.format(name_doublets))
            return channel_dict
        if isinstance(channel_group, str):
    ...
    def channel_group_index(self, channel_group_name, occurrence=0):
        ...
        list_len = -1
        try:
            chgn = [i for i, chg in enumerate(self._xml_chgs) if chg.findtext('name') == channel_group_name]
            list_len = len(chgn)
            ...
            if list_len == 0:
                ...
                raise IndexError('The channel group name {0} does only occur {1} time(s)'.format(channel_group_name, str(list_len)))
    ...
    def no_channels(self, channel_group):
        ...
        """
        if isinstance(channel_group, str):
            return len(OpenFile._get_usi_from_txt(list(filter(lambda x: x.findtext('name') == channel_group, self._xml_chgs))[0].findtext('channels')))
        if isinstance(channel_group, int):
            if len(self._xml_chgs) <= channel_group or channel_group < -len(self._xml_chgs):
                raise IndexError('Channel group {0} out of range'.format(channel_group))
    ...
    def __getitem__(self, key):
        if isinstance(key, tuple):
            ...
            if isinstance(channel_group, str) or isinstance(channel_group, int):
                if isinstance(channel, str) or isinstance(channel, int):
