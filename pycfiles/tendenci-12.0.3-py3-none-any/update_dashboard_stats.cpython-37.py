...
# [GCC 8.4.0]
# Embedded file name: /Users/jennyq/.pyenv/versions/venv_t12/lib/python3.7/site-packages/tendenci/apps/dashboard/management/commands/update_dashboard_stats.py
# Compiled at: 2020-03-30 17:48:03
# Size of source mod 2**32: 20368 bytes
...
class Command(BaseCommand):
    ...
    def get_forms(self, items, days):
        ...
        forms = Form.objects.extra(select={'submissions': 'SELECT COUNT(*) FROM forms_formentry WHERE forms_formentry.form_id = forms_form.id AND ' + "forms_formentry.create_dt >= TIMESTAMP '%s'" % dt})
    ...
    def get_expiring_corp_memberships(self, items, days):
        ...
        corp_memberships = CorpMembership.objects.filter(expiration_dt__gte=now, expiration_dt__lte=dt,
    ...
    def get_membership_count(self, days):
        ...
        expiring = memberships.filter(expire_dt__gte=now, expire_dt__lte=dt).count()
        return [
         [
          active, expiring, new, pending, expired]]

    def get_new_memberships(self, items, days):
        from tendenci.apps.memberships.models import MembershipDefault
        dt = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(days=days)
        memberships = MembershipDefault.objects.filter(application_approved_dt__gte=dt, status_detail='active')
    ...
    def get_expiring_memberships(self, items, days):
        ...
        now = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
        dt = datetime.now().replace(hour=23, minute=59, second=59, microsecond=999999) + timedelta(days=days)
        memberships = MembershipDefault.objects.filter(expire_dt__gte=now, expire_dt__lte=dt,
          status_detail='active')
        count = memberships.count()
        mem_list = [count]
        memberships = memberships.order_by('expire_dt')[:items]
