...
# [GCC 8.4.0]
# Embedded file name: /Users/jennyq/.pyenv/versions/venv_t12/lib/python3.7/site-packages/tendenci/apps/base/migrations/0003_auto_20190913_1537.py
...
def update_jquery_in_theme(apps, schema_editor):
    ...
    if os.path.isfile(file_path):
        ...
        with open(file_path, 'r') as (f):
            content = f.read()
            p = '({0}[\\d\\D\\s\\S\\w\\W]*?<script (type="text/javascript" )?)({1})([\\d\\D\\s\\S\\w\\W]*?{2})'.format(re.escape('{% block jquery_script %}'), re.escape('src="//ajax.googleapis.com/ajax/libs/jquery/2.1.1/jquery.min.js"></script>'), re.escape('{% endblock jquery_script %}'))
            if re.search(p, content):
                content = re.sub(p, '\\1{}\\4'.format('src="//ajax.googleapis.com/ajax/libs/jquery/3.4.1/jquery.min.js"></script>\n\t<script src="https://code.jquery.com/jquery-migrate-3.1.0.min.js"></script>'), content)
                file_changed = True
        if file_changed:
