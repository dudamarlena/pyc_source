...
# [GCC 8.4.0]
# Embedded file name: /Users/jennyq/.pyenv/versions/venv_t12/lib/python3.7/site-packages/tendenci/apps/directories/migrations/0005_auto_20171013_1045.py
# Compiled at: 2020-03-30 17:48:03
# Size of source mod 2**32: 9926 bytes
import os
...
def migrate_customized_directories_templates():
   ...
   {% if request.user.is_superuser %}
    ...
    files_list = [
     '{}/templates/directories/add.html'.format(dir_path),
     '{}/templates/directories/edit.html'.format(dir_path)]
    for file_path in files_list:
        if os.path.isfile(file_path):
            with open(file_path, 'r') as (f):
                content = f.read()
                p = '{0}\\s*{1}'.format(re.escape('<script type="text/javascript" src="{% static \'js/email-verification.js\' %}">'), re.escape('</script>'))
    ...
    if os.path.isfile(file_path):
        find_replace_list = [
         ('{% with directory.category_set as directory_cat %}', '{% with directory.cat as directory_cat %}'),
         ...
         ('sub_category={{ directory_cat.sub_category.id }}">{{ directory_cat.sub_category }}',
 'cat={{ directory_cat.pk }}&sub_cat={{ directory.sub_cat.pk }}">{{ directory.sub_cat.name }}')]
        with open(file_path, 'r') as (f):
            content = f.read()
            ...
            p = '{0}\\s+{1}\\s+{2}'.format(re.escape('<li>'), re.escape('<a href="{% url \'category.update\' directory.opt_app_label directory.opt_module_name directory.pk %}">{% trans "Edit Categories" %}</a>'), re.escape('</li>'))
    ...
    if os.path.isfile(file_path):
        ...
        with open(file_path, 'r') as (f):
            content = f.read()
            p = '{0}\\s*{1}\\s*{2}'.format(re.escape('{% for form.category in form.category_list %}'), re.escape(' {{ form.category.name }}'), re.escape('{% endfor %}'))
            content = re.sub(p, '', content)
            for string_to_find, string_to_replace in find_replace_list:
                content = content.replace(string_to_find, string_to_replace)
    ...
    if os.path.isfile(file_path):
        with open(file_path, 'r') as (f):
            content = f.read()
            content = content.replace("var $catAndSubcatSelect = $('#id_category, #id_sub_category')", "var $catAndSubcatSelect = $('#id_cat, #id_sub_cat')")
        with open(file_path, 'w') as (f):
            f.write(content)
    ...
    if os.path.isfile(file_path):
        with open(file_path, 'r') as (f):
            content = f.read()
            p = '{0}\\s+{1}\\s+{2}\\s+{3}\\s+{4}'.format(re.escape('<li class="content-item">'), re.escape('<span class="app-name">'), re.escape('<a href="{% url \'category.update\' app_object.opt_app_label app_object.opt_module_name app_object.pk %}">{% trans "Edit Categories" %}</a>'), re.escape('</span>'), re.escape('</li>'))
            manage_cat = '\n            {% if request.user.is_superuser %}\n            <li class="content-item">\n                <span class="app-name">\n                    <a href="{% url \'admin:directories_category_changelist\' %}">{% trans "Manage Categories" %}</a>\n                </span>\n            </li>\n            {% endif %}\n            '
...
def migrate_categories_data(apps, schema_editor):
    ...
    import tendenci.apps.directories.models as DirectoryCategory
    for directory in Directory.objects.raw('SELECT * FROM directories_directory'):
        ...
        for cat_item in cat_items:
            if cat_item.category:
                ...
                if cat_name:
                    directory_cat, = DirectoryCategory.objects.filter(name=cat_name, parent__isnull=True)[:1] or [None]
                    if not directory_cat:
                        ...
                        if sub_cat_name:
                            directory_sub_cat, = DirectoryCategory.objects.filter(name=sub_cat_name, parent=directory_cat)[:1] or [None]
                            directory_sub_cat = directory_sub_cat or DirectoryCategory(name=sub_cat_name, slug=sub_cat_name)
