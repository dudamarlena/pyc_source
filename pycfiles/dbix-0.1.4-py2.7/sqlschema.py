...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/dbix/sqlschema.py
# Compiled at: 2017-10-18 10:26:17
...
class SQLResultSet(ResultSet):
    ...
    def __init__(self, schema, name, select_columns=None, dict_record=False):
        super(SQLResultSet, self).__init__(schema, name, select_columns=select_columns, dict_record=dict_record)

    def update_operator(self, left, op, right):
        update_operators = {'=': lambda l, r: '%s=%s' % (l, r), 
           ':': lambda l, r: '%s=%s' % (l, r), 
           '+': lambda l, r: '%s=%s+%s' % (l, l, r), 
           '||': lambda l, r: '%s=%s' % (l, self.schema.render_concat(l, r)), 
           '-': lambda l, r: '%s=%s-%s' % (l, l, r), 
           '*': lambda l, r: '%s=%s*%s' % (l, l, r), 
           '/': lambda l, r: '%s=%s/%s' % (l, l, r)}
        ...
        if op in update_operators:
            ...
            return (
             update_operators[op](left, param), [right])
        return (
         '', [])
    ...
    def create(self, **kw):
        ...
        new_key = self.schema.perform_insert(insert, values, pk_fields, self.entity['table'], None if auto_increment else new_key)
        try:
    ...
    def do_filter(self, filter=None):
        select_from = 'select %s\nfrom %s\n' % (
         (',').join([ self.schema.render_name(field) for field in self.select_columns
           ]),
         self.schema.render_name(self.entity['table']))
    ...
    def render_pk_in(self, args):
        ...
        len_pk = len(pk)
        args = [ list(arg) if isinstance(arg, (list, tuple)) else [arg] for arg in args if len_pk == 1 or isinstance(arg, (list, tuple)) and len(arg) == len_pk
               ]
        if args:
            if len_pk > 1:
                one_rec = (' and ').join([ '%s=%s' % (self.schema.render_name(field), self.schema.render_paramplace) for field in pk
                                         ])
    ...
    def render_where(self, filter=None):
        ...
        for field, value in kw.items():
            ...
            if isinstance(arg, (list, tuple)):
                if not self.operator_has_list(op):
                    continue
                arg = [ _arg for _arg in arg if not isinstance(_arg, (list, tuple)) ]
            else:
                arg = [
                 arg]
            exp = '(%s%s(%s))' % (
        ...
        sql = (' and ').join([ part for part in sql if part ])
        where = 'where (%s)' % sql if sql else ''
        return (
         where, values)
...
class SQLSchema(Schema):
    ...
    type_render = dict(integer='render_number', tinyint='render_number', smallint='render_number', enum='render_string', float='render_number', decimal='render_number', numeric='render_number', char='render_string', varchar='render_string', text='render_string', mediumtext='render_string', date='render_date', time='render_time', datetime='render_datetime', timestamp='render_datetime', boolean='render_bool')
    inline_timestamps = True
    attr_render = dict(data_type=(
     None, lambda c, x: type_render.get(x)), is_nullable=(
     'required', lambda c, x: not int(x)), default_value=('default', None), size=('size',
                                                                                  None), extra=(
     'domain', lambda c, x: x['list']), set_on_create=lambda c, x: int(x), set_on_update=lambda c, x: int(x), is_auto_increment=lambda c, x: int(x))
    render_pk = True
    inline_fk = True
    inline_domain = False
    triggers = list()
    ...
    def type_conv(self, attrs, entity, name):
        data_type = self._type_conv.get((
         attrs['data_type'], name), self._type_conv.get(attrs['data_type'], attrs['data_type']))
        return data_type
    ...
    def render_autoincrement(self, attrs, entity, name):
        if 'is_auto_increment' in attrs and entity['primary_key'] != (name,):
            print('autoincrement on %(table)s not the primary key' % entity, file=sys.stderr)
            return (
             dict(), None)
        else:
            return (
    ...
    def render_field(self, name, attrs, entity, prefix='\t'):
        ...
        attrs, res_autoincrement = self.render_autoincrement(attrs, entity, name)
        ...
        res = '%s %%(data_type)s' % self.render_name(name)
        if 'extra' in attrs:
            if 'list' in attrs['extra']:
                domain = (',').join([ _type_render(part) for part in attrs['extra']['list'] ])
        ...
        params = dict(field=name, getdate=getdate, getdate_tr=getdate.replace('%', '%%'), table=entity['table'], default_delimiter=self.default_delimiter)
        if 'set_on_create' in attrs and name not in entity['primary_key']:
            ...
            if getattr(self, 'trigger_field_action_before', None):
                ...
                trigger_actions[event].append(self.trigger_field_action_before['insert'] % params)
            if getattr(self, 'trigger_field_action_after', None):
                event = ('after', 'insert')
                trigger_actions.setdefault(event, list())
                trigger_actions[event].append(self.trigger_field_action_after % params)
        ...
        if 'set_on_update' in attrs and name not in entity['primary_key']:
            ...
            trigger_templates = dict()
            triggers = list()
            other_fields = (', ').join([ '%s' % self.render_name(field) for field in fields if field != name
                                       ])
            where_pk = (' and ').join([ '%s.%s=%s' % (self.render_name('new'), self.render_name(field), self.render_name(field)) for field in entity['primary_key']
                                      ])
            params.update(where_pk=where_pk, other_fields=other_fields)
            if getattr(self, 'trigger_field_action_before', None):
                event = ('before', 'update')
                trigger_actions.setdefault(event, list())
                trigger_actions[event].append(self.trigger_field_action_before['update'] % params)
            if getattr(self, 'trigger_field_action_after', None):
                event = ('after', 'update')
                trigger_actions.setdefault(event, list())
                trigger_actions[event].append(self.trigger_field_action_after % params)
    ...
    def get_entity(self, name, prefix='\t', with_fk=True):
        ...
        fields = [ self.render_field(name, attrs, entity, prefix=prefix) for name, attrs in entity['fields'].items()
                 ]
        constraints = [ '%s %s' % (self.render_name('check%d' % c), check) for c, check in enumerate(self.check_constraints)
                      ]
        if self.this_render_pk:
            constraints.append('%s primary key (%s)' % (
             self.render_name('pk_%s' % entity['table']),
             (', ').join([ '%s' % self.render_name(column) for column in entity['primary_key']
             ])))
        for c, unique in enumerate(entity['unique_constraints']):
            name, columns = unique
            if name is None:
                name = '%s_unique%d' % (entity['table'], c)
            constraints.append('%s unique (%s)' % (
             self.render_name(name),
             (', ').join([ '%s' % self.render_name(column) for column in columns ])))
        ...
        for name, other, ours, remotes, extra in entity['belongs_to']:
            ...
            constraint = '%s foreign key (%s) references %s (%s)' % (
             self.render_name('fk_%s_%s' % (entity['table'], name)),
             (', ').join([ '%s' % self.render_name(our) for our in ours ]),
             self.render_name(other_table),
             (', ').join([ '%s' % self.render_name(remote) for remote in remotes ]))
            if not with_fk:
                continue
            if not self.inline_fk:
                self.fk_constraints.append('ALTER TABLE %s ADD CONSTRAINT %s' % (
                 self.render_name(entity['table']), constraint))
            else:
                constraints.append(constraint)

        named_constraints = [ '%sconstraint %s' % (prefix, constraint) for constraint in constraints
                            ]
        create = [
         'create table %s (\n%s\n) %s' % (
    ...
    def ddl(self, pm_location, prefix='\t', only_tables=None, with_fk=True):
        entities = self.ddl_list(prefix=prefix, only_tables=only_tables, with_fk=with_fk)
        return self.script_combine(entities)

    def script_combine(self, statements):
        return ('\n').join([ '%s%s' % (statement, self.default_delimiter) for statement in statements if statement.strip()
                           ])
    ...
    def load_ddl(self, pm_location, only_tables=None, with_fk=True, with_prefix=[]):
        super(SQLSchema, self).load_ddl(pm_location, only_tables=only_tables, with_fk=with_fk)
