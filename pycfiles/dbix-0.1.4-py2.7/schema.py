...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/dbix/schema.py
# Compiled at: 2017-10-18 10:36:44
...
class ResultSet(object):
    ...
    def set_columns(self, select_columns):
        self.select_columns = [ column for column in select_columns if column in self.entity['fields'] ] if select_columns else self.entity['fields'].keys()
        field_names = list(self.entity['fields'].keys())
        self.select_columns_ix = [ field_names.index(column) for column in self.select_columns ]

    def query_operator(self, left, op, right):
        query_operators = {'=': lambda l, r: l == r, 
           '!=': lambda l, r: l != r, 
           '<>': lambda l, r: l != r, 
           '<': lambda l, r: l < r, 
           '<=': lambda l, r: l <= r, 
    ...
    def field_init(self, field, value, pk=None):
        ...
        if 'default_value' in entityfield:
        ...
        auto_increment = pk and entityfield.get('is_auto_increment') and len(pk) == 1
        nullable = entityfield.get('is_nullable', 1)
        return (
         value, nullable, auto_increment)
    ...
    def primary_key(self, kw):
        ...
        for field in pk:
            value, nullable, auto_increment = self.field_init(field, kw.get(field), pk=pk)
            if value is None and not auto_increment:
                raise Exception('missing value for field %s in primary key of %s' % (
    ...
    def create(self, **kw):
        ...
        for field in self.entity['fields']:
            value, nullable, auto_increment = self.field_init(field, kw.get(field))
            if value is None and not nullable:
                raise Exception('missing value for field %s in %s' % (
                 field, self.entity['table']))
            record.append(value)
        ...
        if key in self.data:
            print('key %s already in %s' % (repr(key), self.entity['table']), file=sys.stderr)
            return
        else:
            ...
            for c, field in enumerate(self.entity['fields']):
                ...
                if field in kw:
                    ...
                    if isinstance(value, (list, tuple)) and len(value) == 2:
                        op = list(value)
                    self.data[key][c] = self.update_operator(self.data[key][c], *op)
...
class Schema(object):
    ...
    type_converter = dict(integer=lambda x: int(x), tinyint=lambda x: int(x), smallint=lambda x: int(x), enum=lambda x: x, float=lambda x: float(x), decimal=lambda x: Decimal(x), numeric=lambda x: Decimal(x), char=lambda x: x, varchar=lambda x: x, text=lambda x: x, mediumtext=lambda x: x, date=lambda x: datetime.strptime(x, '%Y-%m-%d').date(), time=lambda x: datetime.strptime(x, '%H:%M:%f').time(), datetime=lambda x: datetime.strptime(x, '%Y-%m-%d %H:%M:%f'), boolean=lambda x: Schema.isfalse(x) or 1)
    field_attr = dict(data_type0=lambda c, x: Schema.type_converter.get(x, (None, ))[0], data_type=lambda c, x: x, is_nullable=lambda c, x: int(x), default_value=lambda c, x: c(x), size=lambda c, x: int(x), is_auto_increment=lambda c, x: int(x), set_on_create=lambda c, x: int(x), set_on_update=lambda c, x: int(x))

    def new_entity(self, name):
        entity = dict(table=None, fields=OrderedDict(), primary_key=list(), unique_constraints=list(), belongs_to=list(), has_many=list(), components=list(), parent_column=None)
    ...
    def get_entity(self, entityname, attr_conv=dict(), only_in_conv=False):
        ...
        entity['fields'] = dict([ (name, dict([ (attr_conv.get(key, (0, 0))[0] if attr_conv.get(key, (0, 0))[0] else key, attr_conv.get(key, (0, 0))[1](0, val) if attr_conv.get(key, (0, 0))[1] else val) for key, val in entity['fields'][name].items() if key in attr_conv or not only_in_conv ])) for name, attrs in entity['fields'].items()
                                ])
    ...
    def resultset(self, name, select_columns=None, dict_record=False):
        return self.rs_class(self, name, select_columns=select_columns, dict_record=dict_record)