...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/pgpu/tk_utils.py
# Compiled at: 2012-10-16 12:41:59
"""
...
OLDSTYLE_FONTS = [
 'ShadowedBlack', 'Benighted', 'Old English Text']
...
class FontDialog(tk.Toplevel):
    ...
    def get_font(self):
        modifiers = ('bold ' if self.bold.get() else 'italic' if self.italic.get() else '').rstrip()
        if not modifiers:
            modifiers = 'normal'
        return (
         self.font.get(), self.size.get(), modifiers)
