...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/maruval/marufind.py
# Compiled at: 2019-07-31 05:23:29
# Size of source mod 2**32: 4051 bytes
...
def listed(s):
    ...
    if not got:
        raise ValueError('Please provide one or more JSON file types, separated by comma')
    return got
...
def _parse_cmdline_args():
    ...
    parser.add_argument('json_types',
      type=listed, help='Comma-separated list of JSON file types')
...
def _get_first_file(folder, atom):
    ...
    if not atom:
        return ''
    files = [i for i in os.listdir(folder) if os.path.isfile(os.path.join(folder, i)) if not i.startswith('.')]
    if files:
