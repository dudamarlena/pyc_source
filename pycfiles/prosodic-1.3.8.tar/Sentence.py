...
# [GCC 8.4.0]
# Embedded file name: lib/Sentence.py
# Compiled at: 2017-04-22 21:41:50
    ...
    def grid(self, nspace=10):
        ...
        for LINE in self.lines():
            ...
            for datak in ['mean', 'mean_line', 'phrasal_stress_peak', 'phrasal_head']:
                ...
                datak_name = datak
                if datak == 'mean':
                    datak_name = 'stress [sent]'
                if datak == 'mean_line':
                    datak_name = 'stress [line]'
                if datak == 'norm_mean':
                    datak_name = 'stress [sent norm]'
                if datak == 'norm_mean_line':
                    datak_name = 'stress [line norm]'
                if datak == 'phrasal_stress_peak':
                    datak_name = 'peak/valley'
                if datak == 'phrasal_head':
                    datak_name = 'head/foot'
...
def find_phrasal_heads(tree):
    for subtree in tree:
        if type(subtree) not in [str, unicode]:
