...
# [GCC 8.4.0]
# Embedded file name: /Users/ryan/DH/prosodic/lib/entity.py
...
class entity(being):
    ...
    def rimestr(self):
        ...
        if last_sylls:
            last_sylls[0] = last_sylls[0].rimes()[0]
    ...
    def wordtokens(self, include_punct=True):
        ...
        if not include_punct:
            return [ w for w in ws if not w.is_punct ]
        return ws
    ...
    def dir(self, methods=True, showall=True):
        ...
        for k, v in sorted(self.__dict__.items()):
            if k.startswith('_'):
                continue
            print (
             makeminlength('.' + k, being.linelen), '\t', v)
        ...
        for x in [ x for x in dir(self) if 'bound method ' + self.classname() in str(getattr(self, x)) and not x.startswith('_') ]:
            if not showall and x in entmethods:
                continue
            attr = getattr(self, x)
            doc = attr.__doc__
            if not doc:
            ...
            print (
             makeminlength('.' + x + '(' + y + ')', being.linelen), '\t', doc)
            if showall:
                print ()
            ...
            for parse in self.bestParses():
                ...
                for pos in parse.positions:
                    ...
                    for k, v in list(posfeats.items()):
                        v = tuple(v)
                        if k not in init.unitfeats:
                            init.unitfeats[k] = {}
                        if v not in init.unitfeats[k]:
                            init.unitfeats[k][v] = {}
                ...
                for parse in self.bestParses():
                    ...
                    for domainfeat, domainfeatvals in list(stats.items()):
                        for domainfeatval, targetfeats in list(domainfeatvals.items()):
                            ...
                            if pkey not in pkeydict:
                                pkeydict[pkey] = {}
                            if posnum not in pkeydict[pkey]:
    ...
    def genfsms(self, meter=None):
        ...
        if hasattr(self, 'allParses'):
            ...
            gs['meter'] = [
             'str_meter']
            Gs = {}
            if len(self.bestParses(meter=meter)) == 1:
                parses = []
    ...
    def scansion_prepare(self, meter=None, conscious=False):
        ...
        config = being.config
        if not meter:
            if not hasattr(self, '_Text__bestparses'):
                return
    ...
    def namestr(self, namestr=[]):
        if hasattr(self, 'name'):
            ...
            if hasattr(self, 'parent') and self.parent and hasattr(self.parent, 'namestr'):
        ...
        for child in self.children:
            ...
            if child.isBroken():
                ...
                if '<' not in string:
                    ...
                    newline += '\t' + string
                if len(child.feats):
                    if child.classname() not in nofeatsplease:
    ...
    def writeViols(self, sheet, units, viols, unitviols, row=0, bool=True, posmark='+', negmark=False):
        for violtype in viols:
            ...
            for unit in units:
                i += 1
                col += 1
                if i not in unitviols:
                    continue
                if violtype not in unitviols[i]:
                    continue
    ...
    def tiergraph(self, parses, colwidth=24):
        ...
        for parse in parses:
            ...
            viols.sort()
            feats.sort()
            row = self.writeFeats(sheet, proms, bool=True, posmark='+', row=1)
            row = self.writeUnits(sheet, units, row=row + 1)
            row = self.writeFeats(sheet, proms, bool=True, posmark='+', row=1)
            book.save('results/tiergraphs/' + str(self) + '.xls')

    def _searchSingleValue(self, value, feature):
        matches = []
        if type(feature) not in [str, str]:
            print (
             '>> Query failed:', self, value, feature)
            return []
        ...
        def reduce_word(word0):
            ...
            if sylls[(-2)].feature('prom.stress') and not sylls[(-1)].feature('prom.stress'):
                for i in reversed(list(range(len(sylls)))):
                    to_keep.insert(0, i)
                    if sylls[i].feature('prom.stress'):
                        break

            else:
                to_keep = [
                 list(range(len(sylls)))[(-1)]]
            obj = entity()
            children = [ word.children[i] for i in to_keep ]
            sbody = children[0].children[0]
            for x in sbody.children[1:]:
    ...
    def phonetic_distance(self, other, normalized=False):
        ...
        for p in phonemes1 + phonemes2:
            pstr = p.phon_str
            if pstr not in p2chr:
                p2chr[pstr] = chr(len(p2chr))
        ...
        try:
            if phonemes1[(-1)] != phonemes2[(-1)]:
                dist += 2
        except IndexError:
            dist += 2

        if normalized:
            return dist / float(max(len(phonemes1), len(phonemes2)))
        return dist