...
# [GCC 8.4.0]
# Embedded file name: /Users/ryan/DH/prosodic/lib/tools.py
...
def assess(fn, meter=None, key_meterscheme=None, key_line='line', key_parse='parse'):
    ...
    def _writegen():
        ...
        for di, d in enumerate(ld):
            line = d[key_line]
            parse_human = ('').join([ x for x in d[key_parse].lower() if x in ('s',
                                                                               'w') ])
            if not parse_human:
                continue
            t = Text(line)
            ...
            parse_human2 = ('').join([ x for x in d.get('parse_human2', '').lower() if x in ('s',
                                                                                             'w') ])
            if not otf_nl:
                header = [
                 '', '', '']
            ...
            for _i, _parses in enumerate(t.allParses()):
                ...
                for _pi, _parse in enumerate(_parses):
                    ...
                    row = [(_pi or line.encode)('utf-8', errors='ignore') if 1 else '', str(_parse) + (' [*Bounded]' if _parse.isBounded else ''), str(humans.count(code)) if code in humans else '']
                    for c in meter.constraints:
                        row += [str(_parse.constraintCounts[c]) if _parse.constraintCounts[c] else '']
            ...
            if parse_human2:
                this_sylls_correct_human2 = get_num_sylls_correct(parse_human, parse_human2)
            ...
            if parse_human2:
                num_sylls_correct_human2 = sum(this_sylls_correct_human2)
            ...
            if parse_human2:
                sylls_iscorrect_human2 += this_sylls_correct_human2
            ...
            if parse_human2:
                line_iscorrect_human2 = int(parse_human2 == parse_human)
            lines_iscorrect_control += [line_iscorrect_dummy]
            lines_iscorrect_control2 += [line_iscorrect_dummy2]
            if parse_human2:
                lines_iscorrect_human2 += [line_iscorrect_human2]
