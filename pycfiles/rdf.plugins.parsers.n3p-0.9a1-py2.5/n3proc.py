...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/n3p/n3proc.py
...
from rdf.term import NamespaceHack as Namespace
from rdf.graph import QuotedGraph
import sys, os.path, re, time, urllib, n3p
try:
    from uripath import join as urijoin
except ImportError:
...
r_unilower = re.compile('(?<=\\\\u)([0-9a-f]{4})|(?<=\\\\U)([0-9a-f]{8})')
r_hibyte = re.compile('[\\x00-\\x08\\x0B-\\x0C\\x0E-\\x1F\\x7F-\\xFF]')
...
def unquote(s, triplequoted=False, r_safe=re.compile('([\\x20\\x21\\x23-\\x5B\\x5D-\\x7E\xa0-\uffff]+)')):
...
class N3Processor(n3p.N3Parser):
    ...
    def onToken(self, prod, tok):
        if self.productions:
            ...
            raise Exception('Token has no parent production.')
    ...
    def nodeToken(self, prod, tok):
        ...
        def ointerp(prod, tok):
            ...
            if self.anonsubj:
                self.anonsubj = False
            if not self.triples or False not in map(lambda s: not len(s), self.triples) or len(self.triples[(-1)]) == 3 or len(self.triples) > 1 and len(self.triples[(-2)]) == 3 and not len(self.triples[(-1)]):
                self.anonsubj = True
            if self.paths and self.paths[(-1)] and self.paths[(-1)][(-1)] in '!^':
                self.anonsubj = 'path'
            if self.paths:
        ...
        def cinterp(prod, tok):
            if not self.anonsubj or self.paths and len(self.paths[(-1)]) == 1:
                self.triples.pop()
        ...
        def this(prod, tok):
            ...
            if self.paths:
                ...
                self.triples[(-1)].append(formula)

        nodedict['@this'] = this
        try:
            nodedict[prod](prod, tok)
        except KeyError:
            pass
    ...
    def verbToken(self, prod, tok):
        vkwords = {'@a': RDF.type, '=': OWL.sameAs, '=>': LOG.implies, 
           '<=': LOG.implies}
    ...
    def triple(self, subj, pred, objt):
        ...
        if not isinstance(pred, tuple):
            ...
            self.sink.statement(objt, pred[0], subj, scp)
    ...
    def qname(self, tok):
        if ':' in tok:
            ...
            raise ParseError('Set user @keywords to use barenames.')
    ...
    def uri(self, tok):
        ...
        if self.universals.has_key(u):
            ...
            if formula in self.formulae:
                return var
        if self.existentials.has_key(u):
    ...
    def literal(self, content, language, datatype):
        if content.startswith('"""'):
            ...
            content = unquote(content[1:-1].decode('utf-8'))
...
def main(argv=None):
    ...
    class MyHelpFormatter(optparse.HelpFormatter):
        ...
        def __init__(self):
            kargs = {'indent_increment': 2, 'short_first': 1, 'max_help_position': 25, 
               'width': None}
    ...
    parser.add_option('-b', '--baseURI', dest='baseURI', default=False, help='set the baseURI', metavar='URI')
    parser.add_option('-r', '--root', dest='root', action='store_true', default=False, help='print triples in the root formula only')
