...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/n3p/n3p.py
...
"""
import sys, os, re, urllib, cPickle as pickle
try:
    set()
except NameError:
...
r_whitespace = re.compile('[ \\t\\r\\n]*(?:(?:#[^\\n]*)?\\r?(?:$|\\n))?')
singleCharacterSelectors = '\t\r\n !"#$%&\'()*.,+/;<=>?[\\]^`{|}~'
r_qname = re.compile('([A-Za-z0-9_:]*)')
r_name = re.compile('([A-Za-z0-9_]*)')
