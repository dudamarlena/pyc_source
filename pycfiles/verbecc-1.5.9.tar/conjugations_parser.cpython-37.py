...
# [GCC 8.4.0]
# Embedded file name: c:\users\brett\git\verbecc\verbecc\conjugations_parser.py
# Compiled at: 2019-07-07 12:00:44
...
class ConjugationsParser:
    ...
    def __init__(self, lang='fr'):
        ...
        tree = etree.parse(resource_filename('verbecc', 'data/conjugations-{}.xml'.format(lang)), parser)
        root = tree.getroot()
        root_tag = 'conjugation-{}'.format(lang)
        if root.tag != root_tag:
            raise exceptions.ConjugationsParserError('Root XML Tag {} Not Found'.format(root_tag))
        for child in root:
