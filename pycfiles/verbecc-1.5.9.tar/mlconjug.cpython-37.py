...
# [GCC 8.4.0]
# Embedded file name: c:\users\brett\git\verbecc\verbecc\mlconjug.py
# Compiled at: 2019-07-13 09:46:09
...
def save_model(model):
    ...
    with ZipFile((pkg_resources.resource_filename('verbecc', zip_filename)),
      mode='w') as (zf):
...
class Model:
    ...
    def __init__(self, vectorizer=None, feature_selector=None, classifier=None, lang=None):
        if not vectorizer:
            vectorizer = CountVectorizer(analyzer=partial(extract_verb_features, lang=lang, ngram_range=(2,
                                                                                                         7)), binary=True)
        else:
            if not feature_selector:
                feature_selector = SelectFromModel(LinearSVC(penalty='l1', max_iter=12000, dual=False, verbose=2))
            classifier = classifier or SGDClassifier(loss='log', penalty='elasticnet', l1_ratio=0.15, max_iter=4000,
              alpha=1e-05,
              random_state=42,
              verbose=2)
