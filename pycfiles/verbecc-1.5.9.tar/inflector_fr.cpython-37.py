...
# [GCC 8.4.0]
# Embedded file name: c:\users\brett\git\verbecc\verbecc\inflector_fr.py
# Compiled at: 2019-07-28 18:45:52
...
class InflectorFr(inflector.Inflector):
    ...
    def get_verbs_that_start_with(self, query, max_results):
        ...
        if is_reflexive:
            matches = [self._add_reflexive_pronoun(m) for m in matches if self._verb_can_be_reflexive(m)]
        return matches
    ...
    def _verb_can_be_reflexive(self, infinitive):
        return not self._is_impersonal_verb(infinitive) and infinitive not in VERBS_THAT_CANNOT_BE_REFLEXIVE_OTHER_THAN_IMPERSONAL_VERBS
    ...
    def _add_present_participle_if_applicable(self, s, is_reflexive, tense_name):
        ret = s
        if is_reflexive:
            if tense_name == self._get_participle_tense_name():
                ret += 'étant '
        return ret
    ...
    def _compound_conjugation_not_applicable(self, is_reflexive, mood_name, hv_tense_name):
        return is_reflexive and mood_name == 'imperatif' and hv_tense_name == 'imperatif-présent'

    def _get_compound_conjugations_hv_map(self):
        return {'indicatif':{'passé-composé':'présent', 
