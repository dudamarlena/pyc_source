...
# [GCC 8.4.0]
# Embedded file name: c:\users\brett\git\verbecc\verbecc\inflector.py
# Compiled at: 2019-07-28 19:34:41
...
class Inflector(ABC):
    ...
    def _get_verb_stem(self, infinitive, template_name):
        template_beg, template_ending = template_name.split(':')
        if not infinitive.endswith(template_ending):
            raise exceptions.ConjugatorError("Template {} ending doesn't match infinitive {}".format(template_name, infinitive))
    ...
    class ConjugationObjects:
        ...
        def __repr__(self):
            return 'infinitive={} verb={} template={} verb_stem={} is_reflexive={}'.format(self.infinitive, self.verb, self.template, self.verb_stem, self.is_reflexive)

    def _get_conj_obs(self, infinitive):
        infinitive = infinitive.lower()
        is_reflexive, infinitive = self._split_reflexive(infinitive)
        if is_reflexive:
            if not self._verb_can_be_reflexive(infinitive):
        ...
        return Inflector.ConjugationObjects(infinitive, verb, template, verb_stem, is_reflexive)
    ...
    def _conjugate_mood_tense(self, co, mood_name, tense_name, alternate=False):
        ...
        return self._conjugate_simple_mood_tense(co.verb_stem, mood_name, tense_template, co.is_reflexive, alternate)
    ...
    def _conjugate_simple_mood_tense(self, verb_stem, mood_name, tense_template, is_reflexive=False, alternate=False):
        ret = []
        ...
        if tense_name in self._get_tenses_conjugated_without_pronouns():
            for person_ending in tense_template.person_endings:
                ...
                s += ending
                if ending != '-':
                    s = self._add_reflexive_pronoun_or_pronoun_suffix_if_applicable(s, is_reflexive, mood_name, tense_name, person_ending.get_person())
                if ending != '-':
            ...
            for person_ending in tense_template.person_endings:
                pronoun = self._get_default_pronoun((person_ending.get_person()),
    ...
    def _conjugate_compound(self, co, mood_name, tense_name, hv_tense_name):
        ...
        if self._compound_conjugation_not_applicable(co.is_reflexive, mood_name, hv_tense_name):
            return ret
            persons = [pe.person for pe in co.template.moods[mood_name].tenses[hv_tense_name].person_endings]
            helping_verb = self._get_auxilary_verb(co, mood_name, tense_name)
            hvco = self._get_conj_obs(helping_verb)
            hvtense_template = copy.deepcopy(hvco.template.moods[mood_name].tenses[hv_tense_name])
            hvperson_endings = []
            ...
            hvconj = self._conjugate_simple_mood_tense(hvco.verb_stem, '', hvtense_template, co.is_reflexive)
            pmood = self._get_participle_mood_name()
            ptense = self._get_participle_tense_name()
            participle = self._conjugate_simple_mood_tense(co.verb_stem, pmood, co.template.moods[pmood].tenses[ptense])
            ...
            for i, hv in enumerate(hvconj):
                participle_inflection = self._get_default_participle_inflection_for_person(persons[i])
