...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fms/engines/asynchronousrandwreplace.py
# Compiled at: 2009-02-06 04:14:19
"""
        ...
        for day in range(self.days):
            for time in range(self.daylength):
                ...
                if market.is_valid(agents[agt], order):
                    ...
                    market.record_order(order, world.tick, self.unique_by_agent)
                    if self.showbooks:
                        market.output_books(world.tick)
                    market.do_clearing(world.tick)
                    world.lastmarketinfo.update({'sellbook': market.sellbook, 'buybook': market.buybook})
                world.tick += 1
