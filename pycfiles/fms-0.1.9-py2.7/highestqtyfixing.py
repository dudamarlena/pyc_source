...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fms/markets/highestqtyfixing.py
# Compiled at: 2009-02-13 07:56:42
"""
...
class HighestQtyFixing(markets.Market):
    ...
    def info(self):
        ...
        if self.buybook:
            ...
            buybook = [
             [
              'unset buybook']]
        infodict = {'sellbook': sellbook, 'buybook': buybook, 'lastprice': self.lastprice, 
    ...
    def do_clearing(self, fixingtime):
        ...
        fixingprice = max((min(i, j), price) for price, (i, j) in fixdict.items())[1]
        logger.info('Fixing price is %.2f' % fixingprice)
        if len(self.buybook) and len(self.sellbook):
            while len(self.sellbook) and len(self.buybook) and self.sellbook[0][0] <= self.buybook[(-1)][0]:
