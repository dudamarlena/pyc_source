...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fms/utils/parsers.py
        ...
        for engine in self['engines']:
            if 'args' in engine:
                if 'args' in engine['market']:
                    logger.info(' %s (%s): %d days of %d instants on %s market (%s)' % (
                     engine['classname'],
                     engine['args'],
                     engine['days'],
                     engine['daylength'],
                    ...
                    logger.info(' %s (%s): %d days of %d instants on %s market' % (
                     engine['classname'],
                     engine['args'],
                     engine['days'],
                     engine['daylength'],
...
class XmlParamsParser(_ParamsParser):
    ...
    def __init__(self, xmlfilename):
        ...
        try:
            self['outputfilename'] = os.path.join(self.exp_path, outputfilename.text)
        except AttributeError:
            self['outputfilename'] = 'sys.stdout'

        orderslogfilename = config.find('.//ordersLogFilename')
        try:
            self['orderslogfilename'] = os.path.join(self.exp_path, orderslogfilename.text)
        except AttributeError:
        ...
        try:
            ...
            self['engines'].append({})
            try:
                self['engines'][0]['classname'] = engineclass.text.split('.')[(-1)]
            except AttributeError:
        ...
        try:
            ...
            try:
                self['engines'][0]['market']['classname'] = marketclass.text.split('.')[(-1)]
            except AttributeError:
...
class YamlParamsParser(_ParamsParser):
    ...
    def __init__(self, yamlfilename):
        ...
        if 'repeat' not in self:
            self['repeat'] = 1
        if 'randomseed' not in self:
            self['randomseed'] = None
        if 'unique_by_agent' not in self:
            self['unique_by_agent'] = True
        if 'csvdelimiter' in self:
            if self['csvdelimiter'] not in CSVDELIMITERS:
                self['csvdelimiter'] = ';'
        else:
            self['csvdelimiter'] = ';'
        if 'outputfilename' in self:
            self['outputfilename'] = os.path.join(self.exp_path, self['outputfilename'])
        else:
            self['outputfilename'] = 'sys.stdout'
        if 'orderslogfilename' in self:
            self['orderslogfilename'] = os.path.join(self.exp_path, self['orderslogfilename'])
        else:
            self['orderslogfilename'] = None
        for paramkey in ('world', 'engines', 'agents'):
            if paramkey not in self:
                raise MissingParameter, paramkey

        if 'classname' not in self['world']:
            raise MissingParameter, 'world classname'
        self['world']['modulename'], self['world']['classname'] = self.splitclassname(self['world']['classname'])
        for paramkey in ('engines', 'agents'):
            for item in self[paramkey]:
                if 'classname' not in item:
                    raise MissingParameter, paramkey + ' classname'
                item['modulename'], item['classname'] = self.splitclassname(item['classname'])

        for paramkey in ('number', 'stocks', 'money'):
            for agent in self['agents']:
                if paramkey not in agent:
                    agent[paramkey] = 1

        for paramkey in ('days', 'daylength', 'clearbooksateod'):
            for engine in self['engines']:
                if paramkey not in engine:
                    engine[paramkey] = 1

        for engine in self['engines']:
            if 'market' not in engine:
                raise MissingParameter, "engine['market']"
            if 'classname' not in engine['market']:
                raise MissingParameter, "engine['market']['classname']"
            engine['market']['modulename'], engine['market']['classname'] = self.splitclassname(engine['market']['classname'])
