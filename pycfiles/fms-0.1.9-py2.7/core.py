...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fms/core.py
# Compiled at: 2012-03-01 01:38:17
"""
FMS core module.
"""
...
def _apply_opts(params, opts):
    ...
    if opts.replay:
        ...
        if not params['agents'][0]['classname'] == 'PlayOrderLogFile':
            ...
            params['agents'][0]['args'] = [os.path.abspath(params['orderslogfilename'])]
...
def set_parser():
    ...
    """
    optp = optparse.OptionParser(description='run a Financial Market Simulator simulation', prog='%s' % sys.argv[0], usage='%prog [options] [command] simulationconffile', version='This is FMS %s' % VERSION)
    optp.add_option('-L', '--loglevel', metavar='LEVEL', dest='loglevel', help='set logging level to LEVEL: debug, info, warning, error, critical')
    optp.add_option('-v', '--verbose', action='store_true', help="set logging level to 'info', overrided by --loglevel")
    optp.add_option('--show_books', '--show_limits', action='store_true', dest='show_books', help='show best limits on each step')
    optp.add_option('-r', '--replay', action='store_true', help='Replay an orders logfile.')
    optp.add_option('-t', '--timer', action='store_true', help='Print a timer.')
    optp.add_option('--unique_by_agent', action='store_true', help='Only one order by agent in books.')
    optp.add_option('--no_unique_by_agent', action='store_false', dest='unique_by_agent', help='More than one order by agent allowed in books.')
    optp.add_option('--orderslogfilename', dest='orderslogfilename', help='orders log filename')
    optp.add_option('--outputfilename', '-o', dest='outputfilename', help='output filename')
    optp.add_option('--randomseed', help='random seed')
    optp.add_option('--csvdelimiter', help='csv delimiter')
    optp.add_option('--repeat', help='repeat experiment N times')
    return optp
...
def _set_world(params):
    ...
    if params['world']['modulename']:
        worldmodule = _import_class(('.').join(('fms.contrib', params['world']['modulename'], 'worlds')), params['world']['classname'])
    else:
...
def _set_agents(params):
    ...
    for offset, a in enumerate(params['agents']):
        if a['modulename']:
            agentmodule = _import_class(('.').join(('fms.contrib', a['modulename'], 'agents')), a['classname'])
...
def _set_engines(params):
    ...
    for offset, e in enumerate(params['engines']):
        if e['market']['modulename']:
            marketmodule = _import_class(('.').join(('fms.contrib', e['market']['modulename'], 'markets')), e['market']['classname'])
        else:
            marketmodule = _import_class('fms.markets', e['market']['classname'])
        e['market']['instance'] = getattr(marketmodule, e['market']['classname'])(params)
        if e['modulename']:
            enginemodule = _import_class(('.').join(('fms.contrib', e['modulename'], 'engines')), e['classname'])
        else:
