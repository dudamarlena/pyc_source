...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/eschool/eschool.py
# Compiled at: 2019-10-01 15:41:25
...
class EschoolClient(EschoolBase):
    ...
    def marks(self) -> list:
        ...
        return [[lesson['markVal'], lesson['mktWt'], lesson['startDt'], lesson['lessonId'], lesson['lptName'], units[lesson['unitId']]] for lesson in result if lesson.get('markVal')]
    ...
    def diary(self, d1=None, d2=None):
        ...
        """
        return self.get('diary', d1=((d1 or int(time.time() - 172800)) * 1000), d2=((d2 or int((d1 or int(time.time() - 172800)) + 1209600)) * 1000))['lesson']
    ...
    def homeworks(self, d1=None, d2=None):
        ...
        for lesson in self.diary(d1, d2):
            ...
            if not part:
                continue
            part = part[0]
            if not part['variant'][0].get('text'):
                if not part['variant'][0].get('file'):
                    continue
    ...
    def messages(self, thread_id):
        ...
        """
        return self.get('messages', prefix='chat', getNew='false', isSearch='false', rowStart=1, rowsCount=3, threadId=thread_id)
    ...
    def handling(self):
        ...
        self.handled_msgs = self.handled_msgs or [msg['msgId'] for chat in self.chats() for msg in self.messages(chat['threadId'])]
        ...
        def messages_loop():
            while True:
                try:
                    msgs = list(filter(lambda msg: msg['msgId'] not in self.handled_msgs, [msg for chat in self.chats() for msg in self.messages([chat['threadId'], time.sleep(30)][0])]))
