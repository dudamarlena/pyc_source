...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/sqlalchemy_mate-project/sqlalchemy_mate/credential.py
# Compiled at: 2019-04-26 17:27:17
# Size of source mod 2**32: 13584 bytes
"""
Safe database credential loader.
"""
import os, json, string, sqlalchemy as sa
try:
...
class Credential(object):
    ...
    def __repr__(self):
        return "{classname}(host='{host}', port={port}, database='{database}', username={username}, password='xxxxxxxxxxxx')".format(classname=self.__class__.__name__, host=self.host, port=self.port, database=self.database, username=self.username)
    ...
    def uri(self):
        ...
        """
        return self.uri_template.format(host=self.host, port='' if self.port is None else self.port, database=self.database, username=self.username, password='' if self.password is None else self.password, has_password='' if self.password is None else ':', has_port='' if self.port is None else ':')
    ...
    def _validate_key_mapping(cls, key_mapping):
        if key_mapping is not None:
            ...
            if keys != ['database', 'host', 'password', 'port', 'username']:
                msg = '`key_mapping` is the credential field mapping from `Credential` to custom json! it has to be a dictionary with 5 keys: host, port, password, port, username!'
    ...
    @classmethod
    def from_home_db_json(cls, identifier, key_mapping=None):
        """
        ...
        """
        return cls.from_json(json_file=cls.path_db_json, json_path=identifier, key_mapping=key_mapping)

    @classmethod
    def from_s3_json(cls, bucket_name, key, json_path=None, key_mapping=None, aws_profile=None, aws_access_key_id=None, aws_secret_access_key=None, region_name=None):
        """
    ...
    def from_env(cls, prefix, kms_decrypt=False, aws_profile=None):
        ...
        if not prefix.endswith('_'):
            prefix = prefix + '_'
        data = dict(host=os.getenv(prefix + 'HOST'), port=os.getenv(prefix + 'PORT'), database=os.getenv(prefix + 'DATABASE'), username=os.getenv(prefix + 'USERNAME'), password=os.getenv(prefix + 'PASSWORD'))
        if kms_decrypt is True:
            import boto3
            ...
            def decrypt(kms, text):
                return kms.decrypt(CiphertextBlob=b64decode(text.encode('utf-8')))['Plaintext'].decode('utf-8')

            data = {key:(value if value is None else decrypt(kms, str(value))) for key, value in data.items()}
        return cls(**data)
    ...
    def to_dict(self):
        ...
        """
        return dict(host=self.host, port=self.port, database=self.database, username=self.username, password=self.password)


class EngineCreator(Credential):
    ...
    def create_postgresql(self, **kwargs):
        ...
        :rtype: Engine
        """
        return self._ce(self._ccs(self.DialectAndDriver.psql), **kwargs)

    def create_postgresql_psycopg2(self, **kwargs):
        """
        :rtype: Engine
        """
        return self._ce(self._ccs(self.DialectAndDriver.psql_psycopg2), **kwargs)

    def create_postgresql_pg8000(self, **kwargs):
        """
        :rtype: Engine
        """
        return self._ce(self._ccs(self.DialectAndDriver.psql_pg8000), **kwargs)

    def _create_postgresql_pygresql(self, **kwargs):
        """
        :rtype: Engine
        """
        return self._ce(self._ccs(self.DialectAndDriver.psql_pygresql), **kwargs)

    def create_postgresql_psycopg2cffi(self, **kwargs):
        """
        :rtype: Engine
        """
        return self._ce(self._ccs(self.DialectAndDriver.psql_psycopg2cffi), **kwargs)

    def create_postgresql_pypostgresql(self, **kwargs):
        """
        :rtype: Engine
        """
        return self._ce(self._ccs(self.DialectAndDriver.psql_pypostgresql), **kwargs)

    def create_mysql(self, **kwargs):
        """
        :rtype: Engine
        """
        return self._ce(self._ccs(self.DialectAndDriver.mysql), **kwargs)

    def create_mysql_mysqldb(self, **kwargs):
        """
        :rtype: Engine
        """
        return self._ce(self._ccs(self.DialectAndDriver.mysql_mysqldb), **kwargs)

    def create_mysql_mysqlconnector(self, **kwargs):
        """
        :rtype: Engine
        """
        return self._ce(self._ccs(self.DialectAndDriver.mysql_mysqlconnector), **kwargs)

    def create_mysql_oursql(self, **kwargs):
        """
        :rtype: Engine
        """
        return self._ce(self._ccs(self.DialectAndDriver.mysql_oursql), **kwargs)

    def create_mysql_pymysql(self, **kwargs):
        """
        :rtype: Engine
        """
        return self._ce(self._ccs(self.DialectAndDriver.mysql_pymysql), **kwargs)
    ...
    def create_oracle(self, **kwargs):
        ...
        :rtype: Engine
        """
        return self._ce(self._ccs(self.DialectAndDriver.oracle), **kwargs)

    def create_oracle_cx_oracle(self, **kwargs):
        """
        :rtype: Engine
        """
        return self._ce(self._ccs(self.DialectAndDriver.oracle_cx_oracle), **kwargs)

    def create_mssql_pyodbc(self, **kwargs):
        """
        :rtype: Engine
        """
        return self._ce(self._ccs(self.DialectAndDriver.mssql_pyodbc), **kwargs)

    def create_mssql_pymssql(self, **kwargs):
        """
        :rtype: Engine
        """
        return self._ce(self._ccs(self.DialectAndDriver.mssql_pymssql), **kwargs)

    def create_redshift(self, **kwargs):
        """
        :rtype: Engine
        """
        return self._ce(self._ccs(self.DialectAndDriver.redshift_psycopg2), **kwargs)


if __name__ == '__main__':
    import boto3
    from base64 import b64decode
    cred = Credential.from_s3_json('sanhe-credential', 'db/elephant-dupe-remove.json', aws_profile='sanhe')
    print(cred)