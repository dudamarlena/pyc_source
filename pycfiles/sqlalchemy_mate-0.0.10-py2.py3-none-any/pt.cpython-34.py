...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/sqlalchemy_mate-project/sqlalchemy_mate/pt.py
...
def from_everything(everything, engine, limit=None, **kwargs):
    """
    ...
    if isinstance(everything, TextClause):
        return from_stmt(everything, engine, **kwargs)
    if isinstance(everything, Table):
        return from_table(everything, engine, limit=limit, **kwargs)
    if type(everything) is DeclarativeMeta:
        return from_object(everything, engine, limit=limit, **kwargs)
    if isinstance(everything, Query):
        return from_query(everything, engine, limit=limit, **kwargs)
    if isinstance(everything, Select):
        return from_sql(everything, engine, limit=limit, **kwargs)
