...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/sqlalchemy_mate-project/sqlalchemy_mate/orm/extended_declarative_base.py
# Compiled at: 2019-04-26 18:51:41
# Size of source mod 2**32: 13001 bytes
"""
...
class ExtendedBase(object):
    ...
    def glance(self):
        ...
        if self._settings_major_attrs is None:
            msg = 'Please specify attributes you want to include in `class._settings_major_attrs`!'
            raise NotImplementedError(msg)
        kwargs = [(attr, getattr(self, attr)) for attr in self._settings_major_attrs]
        text = '{classname}({kwargs})'.format(classname=(self.__class__.__name__),
          kwargs=(', '.join(['%s=%r' % (attr, value) for attr, value in kwargs])))
        print(text)
