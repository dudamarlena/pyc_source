...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/HTMLTable/table.py
# Compiled at: 2019-04-08 23:07:54
...
class HTMLTable(list, HTMLTag):
    ...
    def append_rows(self, rows, is_header=False):
        for row in rows:
            self.append(HTMLTableRow(cells=row,
              is_header=is_header))
    ...
    def set_basic_style(self):
        border_style = {'border-color':'#000',  'border-width':'1px', 
         'border-style':'solid', 
         'border-collapse':'collapse'}
        self.set_style(border_style)
        self.set_cell_style(border_style)
        header_cell_style = {'padding':'15px', 
