...
class AvishanModelPanelEnabled:
    ...
    def panel_create(cls: Union[(AvishanModel, 'AvishanModelPanelEnabled')], edit_mode: bool=False):
        ...
        if edit_mode:
            ...
            action_url += 'create'
        cls.panel_view.form = Form(action_url=action_url,
          name='create',
          button=Button(text=('ویرایش' if edit_mode else 'ایجاد')),
          disabled=True if (not edit_mode and created) else False).add_items(cls.panel_create_form_items(item=created))
        related_forms = []
        for model, related_field_name in cls.panel_create_related_models():
            created_id = 0
            if created:
                created_id = created.id
            action_url = f"/{get_avishan_config().PANEL_ROOT}/{cls.class_plural_snake_case_name()}/{created_id}/"
            if edit_mode:
                action_url += 'edit'
            else:
                action_url += 'create'
            action_url += f"?on={model.class_name()}"
            related_forms.append(Row().add_item(Col(6).add_item(Card(header=CardHeader(title=(model.panel_plural_name())),
              body=(Row().add_item(Col(12).add_item(UnorderedList().add_items([UnorderedListItem(data=(str(item)), link=f"/{get_avishan_config().PANEL_ROOT}/{model.class_plural_snake_case_name()}/{item.id}") for item in (model.objects.filter)(**{related_field_name: created})]))).add_item(Col(12).add_item(Form(action_url=action_url,
              name=f"{model.class_snake_case_name()}_create",
              button=Button(text='افزودن'),
              disabled=(False if created else True)).add_items(model.panel_create_form_items(values_list=[
             (
              related_field_name, str(created))],
              disabled_list=[
             related_field_name])))))))))
        ...
        if current_request['request'].method == 'POST':
            if not current_request['request'].GET.get('on'):
                ...
                if edit_mode:
                    created = (created.panel_edit_method)(**cls.panel_view.form.parse(data))
                else:
                    created = (cls.panel_create_method)(**cls.panel_view.form.parse(data))
            else:
                related_model, related_field = cls.panel_create_related_model_find(current_request['request'].GET.get('on'))
                data = {related_field: created}
        ...
        if current_request['request'].method == 'GET':
            cls.panel_view.contents.append(Row().add_item(Col(large=6, extra_large=6, medium=12).add_item(Card(header=CardHeader(buttons=(created.panel_edit_form_buttons() if edit_mode else cls.panel_create_form_buttons()),
    ...
    def panel_create_form_items(cls: Union[(AvishanModel, 'AvishanModelPanelEnabled')], item: Union[(AvishanModel, 'AvishanModelPanelEnabled')]=None, values_list: List[Tuple[(str, str)]]=(), disabled_list: List[str]=()) -> List[FormElement]:
        ...
        for field in cls.panel_create_form_fields():
            ...
            if isinstance(field, models.ForeignKey):
                if field.related_model in [Image, File]:
