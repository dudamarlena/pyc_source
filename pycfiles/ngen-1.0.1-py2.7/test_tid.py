...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ngen/tests/test_tid.py
...
class TimestampTests(TestCase):
    ...
    def _test_timestamp_template(self, stamp, has_microseconds=False):
        ...
        self.assertEqual(len(stamp.split(b'-')), 3)
        self.assertTrue(b'T' in stamp)
        self.assertTrue(b'Z' in stamp)
        date, time = stamp.split(b'T')
        time = time.strip(b'Z')
        hour, minute, seconds = time.split(b':')
        if has_microseconds:
            self.assertTrue(b'.' in seconds)
