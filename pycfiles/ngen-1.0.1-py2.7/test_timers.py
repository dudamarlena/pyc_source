...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ngen/tests/test_timers.py
...
class TimerTests(TestCase):
    ...
    def setUp(self):
        ...
        self.patcher = mock.patch(b'ngen.utils.open', mocked_open)
    ...
    def test_TimerContext_success_with_default_and_custom_name(self):
        context_success(name=b'asdf')
        self._test_file_actions(DEFAULT_PATH, b'asdf')

    def test_TimerContext_success_with_custom_path(self):
        context_success(b'asdf.log')
        self._test_file_actions(b'asdf.log', b'None')

    def test_TimerContext_success_with_custom_path_and_custom_name(self):
        context_success(b'asdf.log', name=b'asdf')
        self._test_file_actions(b'asdf.log', b'asdf')

    def test_timer_on_method(self):
        self.dummy.method()
        self._test_file_actions(DEFAULT_PATH, b'method')

    def test_timer_on_custom_path(self):
        self.dummy.custom_path()
        self._test_file_actions(b'asdf.log', b'custom_path')

    def test_timer_on_static(self):
        self.dummy.static()
        self._test_file_actions(DEFAULT_PATH, b'static')

    def test_timer_on_classy(self):
        self.dummy.classy()
        self._test_file_actions(DEFAULT_PATH, b'classy')

    def test_timer_static_arg_integrity(self):
        _mock = self.dummy.static(mock.MagicMock(), 1, b'test', x=3)
        _mock.assert_called_once_with(1, b'test', x=3)

    def test_timer_classy_arg_integrity(self):
        self.dummy.classy(1, b'test', x=3)
        self.dummy.cls_callable.assert_called_once_with(1, b'test', x=3)

    def test_timer_method_arg_integrity(self):
        self.dummy.method(1, b'test', x=3)
        self.dummy.callable.assert_called_once_with(1, b'test', x=3)

    def test_timer_error_behaviour(self):
        self.dummy.callable.side_effect = RuntimeError(b'haha!')
        self.assertRaises(RuntimeError, self.dummy.method, 1, b'test', x=3)
        self.dummy.callable.assert_called_once_with(1, b'test', x=3)
        self.assertFalse(self._open.called)

    def test_simple_timer(self):
        simple()
        self._test_file_actions(DEFAULT_PATH, b'simple')

    def test_simple_timer_arg_integrity(self):
        _mock = simple(mock.MagicMock(), 1, b'test', x=3)
        _mock.assert_called_once_with(1, b'test', x=3)

    def test_simple_timer_error_behaviour(self):
        __mock = mock.MagicMock()
        __mock.side_effect = RuntimeError(b'haha!')
        self.assertRaises(RuntimeError, simple, __mock, 1, b'test', x=3)
        __mock.assert_called_once_with(1, b'test', x=3)
        self.assertFalse(self._open.called)