...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ngen/models.py
...
class BaseOptions(six.with_metaclass(ABCMeta, Mapping)):
    ...
    def __iter__(self):
        return iter({key:value for key, value in self.__dict__.items() if not key.startswith(b'_') if not key.startswith(b'_')})

    def __len__(self):
        return len([ key for key in self.__dict__ if not key.startswith(b'_') ])

    def __str__(self):
        return (b'({})').format((b', ').join([ (b'{}={}').format(key, value) for key, value in self.items() ]))

    def __repr__(self):
        return (b'<{}: {}>').format(self.__class__.__name__, self)
...
class ModelOptions(BaseOptions):
    ...
    def __init__(self, meta_cls=None):
        ...
        kwargs = {}
        if meta_cls:
            meta_cls_options = {key:value for key, value in meta_cls.__dict__.items() if not key.startswith(b'_') if not key.startswith(b'_')}
            kwargs.update(meta_cls_options)
        self.abstract = kwargs.pop(b'abstract', False)
        if kwargs:
            raise_with_traceback(ImproperlyConfigured((b'Unsupported model options: {}').format((b', ').join(kwargs.keys()))))
...
class ModelMeta(object):
    ...
    def __str__(self):
        return (b'{}').format((b', ').join(self.field_names))

    def __repr__(self):
        return (b'<Meta: {}>').format(self.model)
...
class ModelType(ABCMeta):
    ...
    def __new__(mcs, name, bases, attrs):
        ...
        _meta = attrs.pop(b'Meta', None)
        if not parents:
            return super_new(mcs, name, bases, attrs)
        else:
            new_class = super_new(mcs, name, bases, {b'__module__': attrs[b'__module__']})
            ...
            for attrname, value in attrs.items():
                if hasattr(value, b'add_to_class'):
    ...
    def add_fields(cls, attrs):
        """
        """
        fields = [ (attrname, attrs.pop(attrname)) for attrname, _ in list(attrs.items()) if isinstance(_, Field)
                 ]
...
class Model(six.with_metaclass(ModelType)):
    ...
    def __init__(self, _data=None, **kwargs):
        ...
        if kwargs:
            raise_with_traceback(ValueError((b'Unsupported model kwargs: {}').format((b', ').join(kwargs))))
        return
    ...
    def __str__(self):
        return (b'{}').format(getattr(self, self.meta.field_names[0], None))
...
class FieldOptions(BaseOptions):
    ...
    def __init__(self, field, **kwargs):
        ...
        self._field = field
        self.source = kwargs.pop(b'source', None)
        if self.source is not None and not isinstance(self.source, six.string_types):
            raise_with_traceback(ImproperlyConfigured((b'Field option {}.source is the wrong type. Excpect a string and got {}.').format(self._field, type(self.source))))
        self.default = kwargs.pop(b'default', None)
        self.delimiter = kwargs.pop(b'delimiter', b'.')
        if not isinstance(self.delimiter, six.string_types):
            raise_with_traceback(ImproperlyConfigured((b'Field option {}.delimiter is the wrong type. Expected a string and got {}').format(self._field, type(self.delimiter))))
        self.validators = kwargs.pop(b'validators', ())
        self.required = kwargs.pop(b'required', False)
        self.allow_null = kwargs.pop(b'allow_null', True)
        self.child = kwargs.pop(b'child', None)
        if self.child is not None and not isinstance(self.child, Field):
            raise_with_traceback(ImproperlyConfigured((b'Field option {}.child is the wrong type. Expected an instance of a Field subclass but got {}').format(self._field, type(self.child))))
        self.model = kwargs.pop(b'model', None)
        if self.model is not None and not isinstance(self.model, ModelType):
            raise_with_traceback(ImproperlyConfigured((b'Field option {}.model is the wrong type. Expected a subclass of Model but got {}').format(self._field, type(self.model))))
        for key in list(kwargs.keys()):
            if not key.startswith(b'_'):
                setattr(self, key, kwargs.pop(key))

        if kwargs:
            raise_with_traceback(ImproperlyConfigured((b'Unsupported field options: {}').format((b', ').join(kwargs.keys()))))
        return
...
class Field(six.with_metaclass(ABCMeta)):
    ...
    def parse(self, value):
        ...
        if value is None and not self.options.allow_null:
            raise_with_traceback(FieldError((b'{} expected a non-null value').format(self)))
        elif value is NOT_FOUND and self.options.required:
            raise_with_traceback(FieldError((b'{} is required and could not be found.').format(self)))
        else:
            if value is None and self.options.allow_null:
                return value
            if value is NOT_FOUND and not self.options.required:
                return self.default
    ...
    def traverse_path(self, value):
        ...
        try:
            ...
            LOGGER.debug(exc)
            raise_with_traceback(FieldError((b'Unexpected data type at {}: Got {} expected dict').format(name, type(value))))
    ...
    def __str__(self):
        return (b'{}.{}').format(self.model.__name__, self.name)
