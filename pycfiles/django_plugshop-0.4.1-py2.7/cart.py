...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/plugshop/cart.py
...
class Cart(list):
    ...
    def save(self, **kwargs):
        self.request.session[self.name] = tuple((item.product, item.price, item.quantity) for item in self)
    ...
    def append(self, product, price=0, quantity=1, **kwargs):
        ...
        if item:
            ...
            super(Cart, self).append(CartItem(product, price, quantity))
        if not kwargs.get('stop_signal', None):
            cart_append.send(sender=self, item=item or product, price=price, quantity=quantity)
        return
    ...
    def get_products(self):
        return [ {'product': serialize_model(item.product), 'price': item.price, 'price_total': item.price_total(), 'quantity': item.quantity} for item in self
               ]
