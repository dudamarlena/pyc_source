...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/plugshop/views.py
...
class CategoryView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        category_list = [
         category] + list(category.get_children())
...
class CartView(TemplateResponseMixin, View):
    ...
    def post(self, request, **kwargs):
        ...
        cart.save()
        if request.is_ajax():
            return HttpResponse(json.dumps({'cart': cart.serialize()}), content_type='application/json', **kwargs)
        else:
...
class OrderCreateView(FormView):
    ...
    def get_initial(self, *args, **kwargs):
        ...
        if user.is_authenticated():
            initial.update({'name': '%s %s' % (user.first_name, user.last_name), 
               'email': user.email})
        return initial
    ...
    def notify_managers(self, order):
        ...
        msg = render_to_string('plugshop/email/order_admin.html', {'cart': cart, 
           'order': order, 
    ...
    def notify_customer(self, order):
        cart = get_cart(self.request)
        msg = render_to_string('plugshop/email/order_user.html', {'cart': cart, 
           'order': order, 
           'total': cart.price_total()})
        mail = EmailMessage(self.get_customer_mail_title(order), msg, django_settings.SERVER_EMAIL, [order.user.email])
