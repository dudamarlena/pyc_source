...
# Embedded file name: /home/travis/build/universalcore/consular/consular/clients.py
...
class JsonClient(object):
    ...
    def request(self, method, path, query=None, endpoint=None, json_data=None, **kwargs):
        """
        ...
        requester_kwargs = {'headers': {'Content-Type': 'application/json', 
...
class MarathonClient(JsonClient):
    ...
    def _get_json_field(self, response_json, field_name):
        ...
        if field_name not in response_json:
            raise KeyError('Unable to get value for "%s" from Marathon response: "%s"' % (
    ...
    def get_event_subscriptions(self):
        ...
        callback URLs.
        """
        return self.get_json_field('/v2/eventSubscriptions', 'callbackUrls')

    def post_event_subscription(self, callback_url):
        """
        Post a new Marathon event subscription with the given callback URL.
        """
        d = self.request('POST', '/v2/eventSubscriptions', {'callbackUrl': callback_url})
...
class ConsulClient(JsonClient):
    ...
    def _get_agent_endpoint(self, agent_address):
        ...
        """
        return uricompose(scheme=self.endpoint.scheme, host=agent_address, port=self.endpoint.port)
    ...
    def register_agent_service(self, agent_address, registration):
        ...
        d = self.request('PUT', '/v1/agent/service/register', endpoint=agent_endpoint, json_data=registration)
    ...
    def _register_agent_service_fallback(self, failure, registration):
        ...
        return self.request('PUT', '/v1/agent/service/register', json_data=registration, timeout=self.fallback_timeout)
