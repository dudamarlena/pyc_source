...
# Embedded file name: /home/travis/build/universalcore/consular/consular/main.py
# Compiled at: 2016-08-23 13:23:49
import json
...
class Consular(object):
    ...
    def __init__(self, consul_endpoint, marathon_endpoint, enable_fallback, registration_id):
        ...
        self.event_dispatch = {'status_update_event': self.handle_status_update_event}
    ...
    def register_marathon_event_callback(self, events_url):
        ...
        .. _`Marathon is configured`:
            ...
            #configuration
        """
        existing_callbacks = yield self.marathon_client.get_event_subscriptions()
        already_registered = any([ events_url == url for url in existing_callbacks ])
        if already_registered:
            log.msg('Consular event callback already registered.')
            returnValue(True)
        registered = yield self.marathon_client.post_event_subscription(events_url)
        if registered:
    ...
    def events(self, request):
        ...
        handler = self.event_dispatch.get(event.get('eventType'), self.handle_unknown_event)
    ...
    def handle_status_update_event(self, request, event):
        ...
        :param dict event:
            The Marathon event
        """
        dispatch = {'TASK_STAGING': self.noop, 
    ...
    def noop(self, request, event):
        return succeed(json.dumps({'status': 'ok'}))

    @inlineCallbacks
    def update_task_running(self, request, event):
        """ Use a running event to register a new Consul service. """
        yield self.register_task_service(event['appId'], event['taskId'], event['host'], event['ports'])
        app = yield handle_not_found_error(self.marathon_client.get_app, event['appId'])
        if app is not None:
            yield self.sync_app_labels(app)
        else:
            log.msg('Warning. App with ID "%s" could not be found for new task with ID "%s"' % (
    ...
    def handle_unknown_event(self, request, event):
        ...
        return json.dumps({'error': 'Event type %s not supported.' % (event_type,)})
    ...
    def _find_consular_tag(self, tags, tag_name):
        ...
        if not matches:
            ...
            if len(matches) > 1:
                raise RuntimeError('Multiple (%d) Consular tags found for key "%s": %s' % (
    ...
    def _create_service_registration(self, app_id, service_id, address, port):
        ...
        """
        registration = {'Name': get_app_name(app_id), 
           'ID': service_id, 
           'Address': address, 
           'Tags': [
                  self.reg_id_tag(),
    ...
    def register_task_service(self, app_id, task_id, host, ports):
        ...
        if not ports:
            ...
            port = min(ports)
            log.msg('Warning. %d ports found for app "%s". Consular currently only supports a single port. Only the lowest port (%s) will be used.' % (
             len(ports), app_id, port))
        log.msg('Registering %s at %s with %s at %s:%s.' % (
         app_id, host, task_id, host, port))
        registration = self._create_service_registration(app_id, task_id, host, port)
    ...
    def deregister_consul_service(self, agent_endpoint, service_id):
        ...
        return self.consul_client.deregister_agent_service(agent_endpoint, service_id)
    ...
    def check_apps_namespace_clash(self, apps):
        ...
        collisions = {name:ids for name, ids in name_ids.items() if len(ids) > 1 if len(ids) > 1}
        if collisions:
            collisions_string = ('\n').join(sorted([ '%s => %s' % (name, (', ').join(ids)) for name, ids in collisions.items()
                                                   ]))
            raise RuntimeError('The following Consul service name(s) will resolve to multiple Marathon app names: \n%s' % (
             collisions_string,))
        return apps
    ...
    def put_consul_app_labels(self, app_name, labels):
        ...
        k/v store.
        """
        return self.put_consul_kvs({'consular/%s/%s' % (app_name, key):value for key, value in labels.items()})
    ...
    def clean_consul_app_labels(self, app_name, labels):
        ...
        if keys is None:
            ...
            if self._debug:
                log.msg('%d labels stored in Marathon, %d keys found in Consul for app "%s"' % (
                 len(labels), len(keys), app_name))
    ...
    def _filter_marathon_labels(self, consul_keys, marathon_labels):
        ...
        return [ key for key in consul_keys if self._consul_key_to_marathon_label_key(key) not in label_key_set
               ]
    ...
    def sync_app_tasks(self, app):
        tasks = yield handle_not_found_error(self.marathon_client.get_app_tasks, app['id'])
        if tasks is None:
            ...
            for task in tasks:
                if task['state'] == 'TASK_RUNNING':
                    yield self.register_task_service(app['id'], task['id'], task['host'], task['ports'])
    ...
    def purge_dead_app_labels(self, apps):
        ...
        if keys is None:
            ...
            if self._debug:
                log.msg('After filtering out running apps, %d Consul keys remain to be purged' % len(keys))
            for key in keys:
    ...
    def _filter_marathon_apps(self, consul_keys, marathon_apps):
        ...
        return [ key for key in consul_keys if self._consul_key_to_marathon_app_name(key) not in app_name_set
               ]
        ...
        for service_id, service in data.items():
            ...
            if tags and self.reg_id_tag() in tags:
                ...
                if app_id:
                    ...
                    log.msg('Service "%s" does not have an app ID in its tags, it cannot be purged.' % (
                     service['Service'],))
            elif self._debug:
                log.msg('Service "%s" is not tagged with our registration ID, not touching it.' % (
                 service['Service'],))
    ...
    def purge_service_if_dead(self, agent_endpoint, app_id, consul_task_ids):
        tasks = yield handle_not_found_error(self.marathon_client.get_app_tasks, app_id)
        if tasks is None:
    ...
    def _filter_marathon_tasks(self, marathon_tasks, consul_service_ids):
        ...
        task_id_set = set([ task['id'] for task in marathon_tasks if task['state'] == 'TASK_RUNNING'
                          ])
        return [ service_id for service_id in consul_service_ids if service_id not in task_id_set
               ]