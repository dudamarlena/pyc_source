...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bika/health/browser/insurancecompany/invoicefolder.py
...
class InvoiceFolderView(BikaListingView):
    ...
    def __init__(self, context, request):
        ...
        self.contentFilter = {'portal_type': 'Invoice', 'sort_on': 'sortable_title', 
           'sort_order': 'reverse'}
        ...
        self.pagesize = 25
        self.columns = {'id': {'title': _('Invoice Number'), 'toggle': True}, 
           'client': {'title': _('Client'), 'toggle': True}, 
           'invoicedate': {'title': _('Invoice Date'), 'toggle': True}, 
           'subtotal': {'title': _('Subtotal')}, 'vatamount': {'title': _('VAT')}, 'total': {'title': _('Total')}, 'patient': {'title': _('Patient'), 'toggle': True}}
        self.review_states = [
         {'id': 'default', 
            'contentFilter': {}, 'title': _('Default'), 
            'transitions': [], 'columns': [
                      'id',
                      'client',
                      'invoicedate',
    ...
    def isItemAllowed(self, obj):
        ...
        icuid = patient.getInsuranceCompany().UID() if patient and patient.getInsuranceCompany() else None
