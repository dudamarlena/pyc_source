...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bika/health/browser/aetiologicagent/getaetiologicagents.py
...
class ajaxGetAetiologicAgents(BrowserView):
    ...
    def __call__(self):
        ...
        rows = []
        agents = self.bika_setup_catalog(portal_type='AetiologicAgent', inactive_state='active')
        if agents and searchTerm:
            agents = [ agent for agent in agents if agent.Title.lower().find(searchTerm) > -1 or agent.Description.lower().find(searchTerm) > -1 ]
        for agent in agents:
            agent = agent.getObject()
            if len(title) > 0 and agent.Title() == title:
                rows.append({'Title': agent.Title(), 'Description': agent.Description(), 
                   'AgentUID': agent.UID()})
