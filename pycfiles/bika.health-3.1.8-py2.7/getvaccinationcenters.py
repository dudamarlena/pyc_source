...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bika/health/browser/vaccinationcenter/getvaccinationcenters.py
...
class ajaxGetVaccinationCenters(BrowserView):
    ...
    def __call__(self):
        ...
        rows = []
        brains = self.bika_setup_catalog(portal_type='VaccinationCenter', inactive_state='active')
        if brains and searchTerm:
            brains = [ p for p in brains if p.Title.lower().find(searchTerm) > -1 ]
        for p in brains:
            rows.append({'Title': p.Title, 'VaccinationCenter': p.Title})
