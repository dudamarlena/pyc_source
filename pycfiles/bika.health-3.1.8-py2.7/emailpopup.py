...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bika/health/browser/analysisrequest/emailpopup.py
    ...
    def __call__(self):
        ...
        if ar:
            ...
            if contact:
                ...
                if email:
                    self.recipients = [
                     {'uid': contact.UID(), 'name': contact.Title(), 'email': email}]
            for cc in ar.getCCContact():
                ccemail = cc.getEmailAddress()
                if ccemail:
                    self.ccs.append({'uid': cc.UID(), 'name': cc.Title(), 
                       'email': ccemail})
            ...
            for analysis in analyses:
                ...
                if panic_alerts:
                    ...
                    strans.append(('- {0}, {1}: {2}').format(serviceTitle, translate(_('Result')), result))

            stran = ('\n').join(strans)
            laboratory = self.context.bika_setup.laboratory
            lab_address = ('\n').join(laboratory.getPrintAddress())
            self.body = translate(_('Some results from the Analysis Request ${ar} exceeded the panic levels that may indicate an imminent life-threatening condition: \n\n${arlist}\n\n\n${lab_address}', mapping={'ar': ar.getRequestID(), 'arlist': stran, 
               'lab_address': lab_address}))
            self.subject = translate(_('Some results from ${ar} exceeded panic range', mapping={'ar': ar.getRequestID()}))
