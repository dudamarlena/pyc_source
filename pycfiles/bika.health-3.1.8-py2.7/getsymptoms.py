...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bika/health/browser/symptoms/getsymptoms.py
...
class ajaxGetSymptoms(BrowserView):
    ...
    def __call__(self):
        ...
        rows = []
        brains = self.bika_setup_catalog(portal_type='Symptom', inactive_state='active')
        if brains and self.searchTerm:
            for brain in brains:
                obj = brain.getObject()
                symptom = {'Code': obj.getCode(), 'Title': obj.Title(), 
                   'Description': obj.Description()}
                rows.append(symptom)

        for icd9 in icd9_codes['R']:
            rows.append({'Code': icd9['code'], 'Title': icd9['short'], 
    ...
    def filterBySearchCriteria(self, rows):
        ...
        """
        return [ r for r in rows if r.get('Code', '').lower().find(self.searchTerm) > -1 or r.get('Title', '').lower().find(self.searchTerm) > -1 or r.get('Description', '').lower().find(self.searchTerm) > -1
               ]
