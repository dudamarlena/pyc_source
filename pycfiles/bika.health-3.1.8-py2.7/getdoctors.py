...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bika/health/browser/doctors/getdoctors.py
    ...
    def __call__(self):
        ...
        for doctor in proxies:
            ...
            if self.portal_workflow.getInfoFor(doctor, 'inactive_state', 'active') == 'inactive':
                continue
            if doctor.Title().lower().find(searchTerm) > -1 or doctor.getDoctorID().lower().find(searchTerm) > -1:
                rows.append({'Title': doctor.Title() or '', 'DoctorID': doctor.getDoctorID(), 
