...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bika/health/browser/immunization/getimmunizations.py
...
class ajaxGetImmunizations(BrowserView):
    ...
    def __call__(self):
        ...
        rows = []
        brains = self.bika_setup_catalog(portal_type='Immunization', inactive_state='active')
        if brains and searchTerm:
            brains = [ p for p in brains if p.Title.lower().find(searchTerm) > -1 ]
        for p in brains:
            rows.append({'Title': p.Title, 'UID': p.UID, 
               'Description': p.Description, 
               'Immunization': p.Title})
