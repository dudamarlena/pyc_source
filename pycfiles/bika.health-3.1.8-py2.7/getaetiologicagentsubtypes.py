...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bika/health/browser/batch/getaetiologicagentsubtypes.py
    ...
    def __call__(self):
        ...
        for subtype in subtypes:
            if len(title) > 0 and subtype['Subtype'] == title:
                rows.append({'Subtype': subtype['Subtype'], 'SubtypeRemarks': subtype.get('SubtypeRemarks', '')})
            elif len(title) == 0:
                rows.append({'Subtype': subtype['Subtype'], 'SubtypeRemarks': subtype.get('SubtypeRemarks', '')})
