...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bika/health/widgets/patientmenstrualstatuswidget.py
...
class PatientMenstrualStatusWidget(ATRecordsWidget):
    ...
    _properties.update({'macro': 'bika_health_widgets/patientmenstrualstatuswidget', 
    ...
    def process_form(self, instance, field, form, empty_marker=None, emptyReturnsMarker=False):
        ...
        for value in values:
            outvalues.append({'Hysterectomy': bool(value.get('Hysterectomy', False)), 'HysterectomyYear': value.get('HysterectomyYear', ''), 
    ...
    def getPatientsGender(self):
        ...
        if patientid:
            ...
            gender = len(patient) > 0 and patient[0].getObject().getGender() or 'dk'
        return gender

    def getMenstrualStatus(self):
        statuses = [
         {'Hysterectomy': False, 'HysterectomyYear': '', 
...
registerWidget(PatientMenstrualStatusWidget, title='PatientMenstrualStatusWidget', description='Menstrual status information')