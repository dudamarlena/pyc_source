...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bika/health/browser/worksheet/workflow.py
...
from bika.lims.browser.worksheet.workflow import WorksheetWorkflowAction as BaseClass
    ...
    def __call__(self):
        ...
        if action[0] == 'submit' and isActive(self.context):
            ...
            if alerts:
                ...
                if hasattr(bs, 'getEnablePanicAlert') and bs.getEnablePanicAlert():
                    ...
                    for analysis_uid, alertlist in alerts:
                        ...
                        for alert in alertlist:
                            strans.append(('- {0}, {1}: {2}').format(analysis.getService().Title(), translate(_('Result')), analysis.getResult()))

                    stran = ('<br/>').join(strans)
                    text = translate(_('Some results from ${items} exceeded the panic levels that may indicate an imminent life-threatening condition: <br/><br/>{analysisresults}<br/><br/><b>Please, check the Analysis Request if you want to re-test the analysis or immediately alert the client.</b><br/><br/>{lab_address}', mapping={'items': self.context.getId(), 'analysisresults': stran, 
                       'lab_address': lab_address}))
                    msg_txt = MIMEText(safe_unicode(text).encode('utf-8'), _subtype='html')
                    ...
                    try:
                        ...
                        message = self.context.translate(_('Unable to send an email to alert lab managers that some analyses exceeded the panic levels') + ': %s' % str(msg))
