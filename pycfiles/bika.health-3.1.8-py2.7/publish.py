...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bika/health/browser/batch/publish.py
...
class BatchPublishView(BrowserView):
    ...
    def __call__(self):
        ...
        if proxies:
            ...
            client_address = self.client.getPostalAddress() or self.contact.getBillingAddress() or self.contact.getPhysicalAddress()
            if client_address:
                _keys = [
        ...
        c = [ x for x in self.bika_setup_catalog(portal_type='LabContact') if x.getObject().getUsername() == self.username
            ]
        if c:
        ...
        for ar in self.context.getAnalysisRequests():
            ...
            for analysis in ar.getAnalyses(full_objects=True):
                ...
                datalines.append({_b('Analysis Service'): analysis.getService().Title(), _b('Method'): method and method.Title() or '', 
                   _b('Result'): formatted_result, 
                   _b('Analyst'): self.user_fullname(analysis.getAnalyst()), 
                   _b('Remarks'): analysis.getRemarks()})

            self.ars.append({_b('Request ID'): ar.getRequestID(), 
               _b('Date Requested'): self.ulocalized_time(datecreated), 
