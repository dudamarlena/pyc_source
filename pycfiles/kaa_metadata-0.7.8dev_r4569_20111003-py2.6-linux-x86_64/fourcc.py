...
# [GCC 8.4.0]
# Embedded file name: /net/orion/data/home/tack/projects/kaa/metadata/build/lib.linux-x86_64-2.6/kaa/metadata/fourcc.py
# Compiled at: 2007-03-09 14:26:17
...
def resolve(code):
    ...
    if isinstance(code, basestring):
        codec = 'Unknown'
        if re.match('^0x[\\da-f]{1,4}$', code, re.I):
            return (
             code, TWOCC.get(int(code, 16), codec))
        if code.isdigit() and 0 <= int(code) <= 255:
            return (
             hex(int(code)), TWOCC.get(int(code), codec))
        if len(code) == 2:
            code = struct.unpack('H', code)[0]
            return (
             hex(code), TWOCC.get(code, codec))
        if len(code) != 4 and len([ x for x in code if x not in string.printable ]) == 0:
            codec = unicode(code)
...
TWOCC = {0: 'Unknown Wave Format', 
   1: 'PCM', 
   2: 'Microsoft ADPCM', 
   3: 'IEEE Float', 
   4: 'Compaq Computer VSELP', 
   5: 'IBM CVSD', 
   6: 'A-Law', 
   7: 'mu-Law', 
   8: 'Microsoft DTS', 
   ...
   148: 'Aculab PLC Prosody 8KBPS', 
   151: 'ZyXEL ADPCM', 
   152: 'Philips LPCBB', 
   153: 'Packed', 
   160: 'Malden Electronics PHONYTALK', 
   255: 'AAC', 
   256: 'Rhetorex ADPCM', 
   257: 'IBM mu-law', 
   258: 'IBM A-law', 
   259: 'IBM AVC Adaptive Differential Pulse Code Modulation', 
   273: 'Vivo G.723', 
   274: 'Vivo Siren', 
   291: 'Digital G.723', 
   293: 'Sanyo LD ADPCM', 
   304: 'Sipro Lab Telecom ACELP.net', 
   305: 'Sipro Lab Telecom ACELP.4800', 
   306: 'Sipro Lab Telecom ACELP.8V3', 
   307: 'Sipro Lab Telecom ACELP.G.729', 
   308: 'Sipro Lab Telecom ACELP.G.729A', 
   309: 'Sipro Lab Telecom ACELP.KELVIN', 
   320: 'Windows Media Video V8', 
   336: 'Qualcomm PureVoice', 
   337: 'Qualcomm HalfRate', 
   341: 'Ring Zero Systems TUB GSM', 
   352: 'Windows Media Audio V1 / DivX audio (WMA)', 
   353: 'Windows Media Audio V7 / V8 / V9', 
   ...
   528: 'UHER Informatic ADPCM', 
   533: 'Ulead DV ACM', 
   534: 'Ulead DV ACM', 
   544: 'Quarterdeck', 
   560: 'I-link Worldwide ILINK VC', 
   576: 'Aureal Semiconductor RAW SPORT', 
   577: 'ESST AC3', 
   592: 'Interactive Products HSX', 
   593: 'Interactive Products RPELP', 
   608: 'Consistent Software CS2', 
   624: 'Sony ATRAC3 (SCX, same as MiniDisk LP2)', 
   ...
   1665: 'AT&T Labs TPC', 
   1792: 'YMPEG Alpha', 
   2222: 'ClearJump LiteWave', 
   4096: 'Olivetti GSM', 
   4097: 'Olivetti ADPCM', 
   4098: 'Olivetti CELP', 
   4099: 'Olivetti SBC', 
   4100: 'Olivetti OPR', 
   4352: 'Lernout & Hauspie LH Codec', 
   ...
   65534: 'Extensible wave format', 
   65535: 'development'}
FOURCC = {'1978': 'A.M.Paredes predictor (LossLess)', 
