...
# [GCC 8.4.0]
# Embedded file name: /net/orion/data/home/tack/projects/kaa/metadata/build/lib.linux-x86_64-2.6/kaa/metadata/video/asf.py
# Compiled at: 2009-05-16 10:12:39
__all__ = [
 'Parser']
import struct, string, logging, core, kaa.metadata.audio.core as audiocore
...
GUIDS = {'ASF_Header_Object': _guid('75B22630-668E-11CF-A6D9-00AA0062CE6C'), 
...
class Asf(core.AVContainer):
    ...
    def _parsekv2(self, s):
        ...
        if descriptortype == 0:
            ...
            log.debug('Unknown Descriptor Type %d' % descriptortype)
        return (
         pos, descriptorname, value, strno)
    ...
    def _getnextheader(self, s):
        ...
        if guid == GUIDS['ASF_File_Properties_Object']:
            ...
            val = struct.unpack('<16s6Q4I', s[24:104])
            ...
            if streamtype == GUIDS['ASF_Video_Media']:
                vi = core.VideoStream()
                vi.width, vi.height, depth, codec = struct.unpack('<4xII2xH4s', s[89:109])
                vi.codec = codec
                vi.id = strno
                self.video.append(vi)
            elif streamtype == GUIDS['ASF_Audio_Media']:
                ai = core.AudioStream()
                twocc, ai.channels, ai.samplerate, bitrate, block, ai.samplebits = struct.unpack('<HHIIHH', s[78:94])
            ...
            bitrate, = struct.unpack('<I', s[40:44])
            if streamid not in self._extinfo:
                self._extinfo[streamid] = [
                 None, None, None, {}]
            if frametime == 0:
                frametime = 10000.0
            ...
            val = struct.unpack('<5H', s[24:34])
            pos = 34
            strings = []
            for i in val:
                ss = s[pos:pos + i].replace('\x00', '').lstrip().rstrip()
                strings.append(ss)
                pos += i
            ...
            for i in range(0, count):
                ...
                idstring = unicode(idstring, 'utf-16').replace('\x00', '')
            ...
            pass
        elif guid == GUIDS['ASF_Content_Encryption_Object'] or guid == GUIDS['ASF_Extended_Content_Encryption_Object']:
            self._set('encrypted', True)
        else:
            for h in GUIDS.keys():
                if GUIDS[h] == guid:
