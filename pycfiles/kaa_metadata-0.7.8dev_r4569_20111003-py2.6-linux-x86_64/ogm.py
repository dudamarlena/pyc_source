...
# [GCC 8.4.0]
# Embedded file name: /net/orion/data/home/tack/projects/kaa/metadata/build/lib.linux-x86_64-2.6/kaa/metadata/video/ogm.py
# Compiled at: 2008-04-13 12:22:11
__all__ = [
 'Parser']
import struct, re, stat, os, logging, core
log = logging.getLogger('metadata')
PACKET_TYPE_HEADER = 1
PACKED_TYPE_METADATA = 3
PACKED_TYPE_SETUP = 5
PACKET_TYPE_BITS = 7
PACKET_IS_SYNCPOINT = 8
STREAM_HEADER_VIDEO = '<4sIQQIIHII'
STREAM_HEADER_AUDIO = '<4sIQQIIHHHI'
VORBISCOMMENT = {'TITLE': 'title', 'ALBUM': 'album', 
   'ARTIST': 'artist', 
   'COMMENT': 'comment', 
   'ENCODER': 'encoder', 
   'TRACKNUMBER': 'trackno', 
...
class Ogm(core.AVContainer):
            ...
            for i in range(len(self.all_header)):
                ...
                if self.all_header[i].has_key('CHAPTER01') and not self.chapters:
                    while 1:
                        ...
                        if self.all_header[i].has_key(s) and self.all_header[i].has_key(s + 'NAME'):
    ...
    def _parseOGGS(self, file):
        ...
        for i in range(len(tab)):
            ...
            if len(self.all_streams) > serial:
                ...
                if hasattr(stream, 'samplerate') and stream.samplerate:
                    stream.length = granulepos / stream.samplerate
                elif hasattr(stream, 'bitrate') and stream.bitrate:
                    stream.length = granulepos / stream.bitrate
            return (
             granulepos, nextlen + 27 + pageSegCount)
    ...
    def _parseHeader(self, header, granule):
        ...
        if headerlen >= 30 and header[1:7] == 'vorbis':
            ai = core.AudioStream()
            ai.version, ai.channels, ai.samplerate, bitrate_max, ai.bitrate, bitrate_min, blocksize, framing = struct.unpack('<IBIiiiBB', header[7:30])
            ai.codec = 'Vorbis'
    ...
    def _extractHeaderString(self, header):
        ...
        try:
            return (
             len + 4, unicode(header[4:4 + len], 'utf-8'))
