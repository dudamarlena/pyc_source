...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/etl/tsvtojson.py
# Compiled at: 2016-06-30 19:10:09
import csv, json, sys, getopt, uuid, os
...
def near_dedup_jaccard(dicts, threshold=0.1):
    ...
    for i in range(1, len(sorted_d) - 1):
        ...
        score_diff = float(len(pivot_feats & d_feats)) / float(len(pivot_feats | d_feats))
    ...
    try:
        ...
        with open(tsvFilePath) as (tsv):
            ...
            for line in csv.reader(tsv, dialect='excel-tab'):
                ...
                if 'id' not in jsonStruct:
                    id = uuid.uuid4()
                    jsonStruct['id'] = str(id)
                if uniqueField != None:
                    if uniqueField in jsonStruct:
                        if jsonStruct[uniqueField] not in fieldCache:
