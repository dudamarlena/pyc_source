...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/etl/etllib.py
# Compiled at: 2016-06-28 18:34:06
import urllib2
urllib2.build_opener(urllib2.HTTPHandler(debuglevel=1))
import json, os, operator, ast, sys, re
try:
    import tika
...
def cleanseBody(theDoc):
    if not tika_support:
        print 'cleanseBody requires Tika to be installed.Please check the documentation on how to install ETLlib with Tika support.'
...
def compareKeySimilarity(fileDir):
    ...
    return (
     sorted_resemblance_scores, file_parsed_data)
