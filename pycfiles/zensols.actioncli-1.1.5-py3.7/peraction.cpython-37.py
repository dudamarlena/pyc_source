...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/actioncli/peraction.py
# Compiled at: 2020-04-17 19:19:42
# Size of source mod 2**32: 13351 bytes
import re, os, sys, logging, inspect
...
class PrintActionsOptionParser(OptionParser):
    __doc__ = 'Implements a human readable implementation of print_help for action based\n    command line handlers (i.e. OneConfPerActionOptionsCli).\n\n    '
    ...
    def action_options(self, opts):
        ...
        self.usage = '%prog <list|{}> [options]'.format('|'.join(self.action_names))

    def print_help(self, file=sys.stdout):
        logger.debug('print help: %s' % self.invokes)
        logger.debug('action options: %s' % self.action_options)
        OptionParser.print_help(self, file)
        action_name_len = reduce(lambda x, y: max(x, y), map(lambda x: len(x), self.action_names))
        ...
        for action_name in self.action_names:
            if action_name in self.invokes:
                ...
                opts = map(lambda x: x['opt_obj'], self.action_options[action_name])
                logger.debug('{} -> {}, {}'.format(action_name, action_doc, opts))
                opt_strs = []
                for opt in opts:
                    ...
                    if len(short_opt) > 0:
                        if len(long_opt) > 0:
                            sep = ', '
                    opt_str = '  {}{}{}{}'.format(short_opt, sep, long_opt, otype)
                    if opt.default:
                        if opt.default != ('NO', 'DEFAULT'):
                            default = str(opt.default)
                    opt_strs.append({'str':opt_str,  'default':default, 
                     'help':opt.help})
                    opt_str_len = max(opt_str_len, len(opt_str))
                    def_str_len = max(def_str_len, len(default))

                action_help.append({'doc':action_fmt_str.format(action_name, action_doc), 
                 'opts':opt_strs})

        opt_str_fmt = '{:<' + str(opt_str_len) + '}  {:<' + str(def_str_len) + '}  {}\n'
        file.write('Actions:\n')
        for i, ah in enumerate(action_help):
            file.write(ah['doc'] + '\n')
            for op in ah['opts']:
                file.write(opt_str_fmt.format(op['str'], op['default'], op['help']))
...
class PerActionOptionsCli(SimpleActionCli):
    ...
    def _create_parser(self, usage):
        return PrintActionsOptionParser(usage=usage,
...
class OneConfPerActionOptionsCli(PerActionOptionsCli):
    ...
    def _config_executor(self, oc):
        ...
        for action in oc['actions']:
            ...
            meth = action['meth'] if 'meth' in action else re.sub('[- ]', '_', action_name)
            doc = action['doc'] if 'doc' in action else re.sub('[-_]', ' ', meth)
    ...
    def _find_conf_file(self, conf, params):
        ...
        if conf_file is not None:
            if not os.path.isfile(conf_file):
                if 'expect' not in conf or conf['expect']:
...
class OneConfPerActionOptionsCliEnv(OneConfPerActionOptionsCli):
    __doc__ = 'A command line option parser that first parses an ini file and passes that\n    configuration on to the rest of the CLI action processing in the super\n    class.\n\n    '

    def __init__(self, opt_config, config_env_name=None, no_os_environ=False, *args, **kwargs):
        """Initialize.
 ...
 opt_config, *args, **kwargs)
        if config_env_name is None:
            ...
            if conf_env_var in os.environ:
                ...
                default_config_file = os.path.expanduser('~/.{}'.format(config_env_name))
            logger.debug('configured default config file: {}'.format(default_config_file))
    ...
    def _create_config(self, conf_file, default_vars):
        ...
        return super(OneConfPerActionOptionsCliEnv, self)._create_config(conf_file, defs)

    def _find_conf_file(self, conf, params):
        logger.debug('finding config: {}'.format(self.default_config_file))
        if self.default_config_file is None:
            conf_file = super(OneConfPerActionOptionsCliEnv, self)._find_conf_file(conf, params)
        else:
            conf_name = conf['name']
            conf_file = params[conf_name]
            logger.debug('config: {}, name: {}, params: {}, default_config_file: {}'.format(conf, conf_name, params, self.default_config_file))
            if conf_file is None:
