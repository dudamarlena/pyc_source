...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/actioncli/factory.py
# Compiled at: 2020-04-17 19:19:42
# Size of source mod 2**32: 16274 bytes
import logging
from abc import ABC
...
from zensols.actioncli import Configurable, Stash
...
class ConfigFactory(object):
    ...
    def __init__(self, config: Configurable, pattern: str='{name}', config_param_name: str='config', name_param_name: str='name', default_name: str='default', class_importer: ClassImporter=None):
...
class ImportConfigFactory(ConfigFactory):
    ...
    CHILD_REGEXP = re.compile('^instance:\\s*(.+)$')
    ...
    def __init__(self, *args, reload=False, **kwargs):
        ...
        (super(ImportConfigFactory, self).__init__)(args, **kwargs, **{'class_importer': class_importer})

    def _class_name_params(self, name):
        class_name, params = super(ImportConfigFactory, self)._class_name_params(name)
        insts = {}
        for k, v in params.items():
            if isinstance(v, str):
...
class CachingConfigFactory(object):
    __doc__ = 'Just like ``ConfigFactory`` but caches instances in memory by name.\n\n    '
