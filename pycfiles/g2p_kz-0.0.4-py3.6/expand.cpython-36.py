...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\kzphoneme\expand.py
# Compiled at: 2019-09-27 07:22:06
...
class Number(object):
    ...
    def __init__(self, comma=',', zero='nohl', one='bihr', decimal='buhtihn', *args, **kwargs):
        ...
        self._comma_number_re = re.compile('([0-9][0-9,]+[0-9])')
        self._decimal_number_re = re.compile('([0-9]+\\.[0-9]+)')
        self._pounds_re = re.compile('£([0-9,]*[0-9]+)')
        self._yuan_re = re.compile('([0-9.,]*[0-9]+)￥')
        self._tenge_re = re.compile('([0-9.,]*[0-9]+)₸')
        self._tenge2_re = re.compile('₸([0-9.,]*[0-9]+)')
        self._dollars_re = re.compile('\\$([0-9.,]*[0-9]+)')
        self._ordinal_re = re.compile('([0-9]+)\\s*?([-])')
    ...
    def _enword(self, num, group):
        if group == 1:
            num = re.sub('(\\d)', self._group_sub, num)
        else:
            if int(num) == 0:
                ...
                if int(num) == 1:
                    ...
                    self._mill_count = 0
                    mo = re.search('(\\d)(\\d)(\\d)(?=\\D*\\Z)', num)
                    while mo:
                        num = re.sub('(\\d)(\\d)(\\d)(?=\\D*\\Z)', self._hundred_sub, num, 1)
                        mo = re.search('(\\d)(\\d)(\\d)(?=\\D*\\Z)', num)

                    num = re.sub('(\\d)(\\d)(?=\\D*\\Z)', self._ten_sub, num, 1)
                    num = re.sub('(\\d)(?=\\D*\\Z)', self._unit_sub, num, 1)
        return num
    ...
    def _ten_fn(self, tens, units, m_index=0):
        return '{} {}{}'.format(ten[tens], unit[units], self._mill_fn(m_index))

    def _hundred_fn(self, hundreds, tens, units, m_index):
        if hundreds:
            return '{} juz {} {}, '.format(unit[hundreds], self._ten_fn(tens, units), self._mill_fn(m_index))
        else:
            if tens or units:
    ...
    def _hundred_sub(self, mo):
        ret = self._hundred_fn(int(mo.group(1)), int(mo.group(2)), int(mo.group(3)), self._mill_count)
        self._mill_count += 1
        return ret
    ...
    def normalize_numbers(self, text):
        ...
        text = re.sub(self._pounds_re, '\\1 fwnt', text)
        text = re.sub(self._dollars_re, self._expand_dollars, text)
        text = re.sub(self._yuan_re, '\\1 yuan', text)
