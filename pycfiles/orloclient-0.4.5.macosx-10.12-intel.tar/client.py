...
# [GCC 8.4.0]
# Embedded file name: /Users/alforbes/src/public/orloclient/orloclient/client.py
# Compiled at: 2017-05-11 07:54:02
...
class OrloClient(BaseClient):
    ...
    def __init__(self, uri, timeout=10, verify_ssl=True):
        super(OrloClient, self).__init__(timeout=timeout, verify_ssl=verify_ssl)
        self.uri = uri
    ...
    def _expect_200_json_response(self, response, status_code=200):
        ...
        if response.status_code == status_code:
            try:
                ...
                raise ClientError(('Could not decode json from Orlo response:\n{}').format(str(response.text)))

        else:
            msg = ('Orlo server returned code {code}:\n{text}').format(code=response.status_code, text=response.text)
    ...
    def create_release(self, user, platforms, team=None, references=None, note=None, metadata=None):
        """
        ...
        response = self._post(req_url, json=data, allow_redirects=False)
    ...
    def create_package(self, release, name, version):
        ...
        :return: package id
        """
        response = self._post(('{}/releases/{}/packages').format(self.uri, release.release_id), json={'name': name, 
           'version': version}, allow_redirects=False)
        self._expect_200_json_response(response)
        pkg = response.json()
    ...
    def release_stop(self, release):
        ...
        response = self._post(('{}/releases/{}/stop').format(self.uri, release_id), allow_redirects=False)
    ...
    def get_package(self, package_id):
        ...
        packages_list = [ Package(p['release_id'], p['id'], p['name'], p['version']) for p in response_dict['packages']
                        ]
    ...
    def get_packages(self, raw=False, **kwargs):
        ...
        if raw:
            ...
            return [ Package(None, p['id'], p['name'], p['version']) for p in response_dict['packages'] ]
            return
    ...
    def package_start(self, package):
        ...
        """
        response = self._post(('{}/releases/{}/packages/{}/start').format(self.uri, package.release_id, package.id), allow_redirects=False)
        if response.status_code != 204:
            logger.debug(response)
            raise ServerError(('Orlo server returned non-204 status code: {}').format(response.json()))
        return self._expect_200_json_response(response, status_code=204)
    ...
    def package_stop(self, package, success=True):
        ...
        response = self._post(('{}/releases/{}/packages/{}/stop').format(self.uri, release_id, package_id), json={'success': success}, allow_redirects=False)
        return self._expect_200_json_response(response, status_code=204)
    ...
    def package_add_results(self, package, results):
        ...
        release_id = package.release_id
        package_id = package.id
        response = self._post(('{}/releases/{}/packages/{}/results').format(self.uri, release_id, package_id), json={'content': results}, allow_redirects=False)
        return self._expect_200_json_response(response, status_code=204)
    ...
    def get_info(self, field, name=None, platform=None):
        ...
        :return:
        """
        url_path = {'uri': self.uri, 
           'field': field if field else '', 
           'name': '/' + name if name else ''}
        url_query = {'platform': platform} if platform else {}
        response = self._get(('{uri}/info/{field}{name}').format(**url_path), params=url_query)
        return self._expect_200_json_response(response)
    ...
    def get_stats(self, field=None, name=None, platform=None, stime=None, ftime=None):
        ...
        response = self._get(('{uri}/stats{field}{name}').format(**url_path), params=url_query)
        return self._expect_200_json_response(response)
    ...
    def get_versions(self, platform=None):
        ...
        :return:
        """
        url = ('{url}/info/packages/versions{platform}').format(url=self.uri, platform=platform if platform else '')
        response = self._get(url)
        logger.debug(response)
        return self._expect_200_json_response(response)