...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/readlif/reader.py
# Compiled at: 2019-12-27 17:09:54
# Size of source mod 2**32: 16402 bytes
import struct
...
def _check_truncated(handle):
    ...
    handle.seek(-4, 1)
    if handle.read(100) == b'':
        handle.seek(-100, 1)
        return True
    handle.seek(-100, 1)
    return False
...
def _check_magic(handle, bool_return=False):
    ...
    if handle.read(4) == b'p\x00\x00\x00':
        return True
        assert bool_return, 'This is probably not a LIF file. Expected LIF magic byte at ' + str(handle.tell())
    else:
        return False
...
def _check_mem(handle, bool_return=False):
    ...
    if handle.read(1) == b'*':
        return True
        assert bool_return, 'Expected LIF memory byte at ' + str(handle.tell())
    else:
        return False
...
class LifFile:
    ...
    def _recursive_image_find(self, tree, return_list=None, path=''):
        ...
        for item in children:
            ...
            is_image = len(item.findall('./Data/Image/ImageDescription/Dimensions')) > 0
            if has_sub_children:
                self._recursive_image_find(item, return_list, appended_path)
            elif is_image:
                try:
                    dim_t = item.find('./Data/Image/ImageDescription/Dimensions/DimensionDescription[@DimID="4"]').attrib['NumberOfElements']
                except AttributeError:
                    dim_t = 1

                dim_x = item.find('./Data/Image/ImageDescription/Dimensions/DimensionDescription[@DimID="1"]').attrib['NumberOfElements']
                dim_y = item.find('./Data/Image/ImageDescription/Dimensions/DimensionDescription[@DimID="2"]').attrib['NumberOfElements']
                try:
                    dim_z = item.find('./Data/Image/ImageDescription/Dimensions/DimensionDescription[@DimID="3"]').attrib['NumberOfElements']
                except AttributeError:
                    dim_z = 1

                channel_list = item.findall('./Data/Image/ImageDescription/Channels/ChannelDescription')
                n_channels = len(channel_list)
                len_x = item.find('./Data/Image/ImageDescription/Dimensions/DimensionDescription[@DimID="1"]').attrib['Length']
                scale_x = (int(dim_x) - 1) / (float(len_x) * 1000000)
                len_y = item.find('./Data/Image/ImageDescription/Dimensions/DimensionDescription[@DimID="2"]').attrib['Length']
                scale_y = (int(dim_y) - 1) / (float(len_y) * 1000000)
                try:
                    len_z = item.find('./Data/Image/ImageDescription/Dimensions/DimensionDescription[@DimID="3"]').attrib['Length']
                    scale_z = int(dim_z) / (float(len_z) * 1000000)
                except (AttributeError, ZeroDivisionError):
                    scale_z = None

                try:
                    len_t = item.find('./Data/Image/ImageDescription/Dimensions/DimensionDescription[@DimID="4"]').attrib['Length']
    ...
    def __init__(self, filename):
        ...
        while f.tell() < f_len:
            try:
                ...
                if _check_truncated(f):
                    ...
                    warnings.warn('LIF file is likely truncated. Be advised, it appears that some images are blank. ', UserWarning)
                    truncated = True
                    f.seek(0, 2)
                else:
                    raise
        ...
        if len(self.image_list) != len(self.offsets) and not truncated:
            raise ValueError('Number of images is not equal to number of offsets, and this file does not appear to be truncated. Something has gone wrong.')
        else:
