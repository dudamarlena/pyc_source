...
# [GCC 8.4.0]
# Embedded file name: /home/robin/devel/athlib/athlib/wma/agegrader.py
# Compiled at: 2020-02-19 16:21:40
import json, re, os
...
class AgeGrader(object):
    ...
    def calculate_factor(self, gender, age, event, distance=None):
        ...
        nt = len(table)
        if distance is None:
            self.find_row_by_event(event, table, x=self.event_column, label='wma.%s.%s' % (self.data_year,
             gender))
        else:
            self.find_row_by_distance(distance, table, x=1, label='wma.%s.%s' % (self.data_year,
             gender))
    ...
    def calculate_age_grade(self, gender, age, event, performance, verbose=False):
...
class AthlonsAgeGrader(AgeGrader):
    ...
    def calculate_factor(self, gender, age, event):
        ...
        table = data[gender]
        ages = data['ages']
        self.find_row_by_event(event, table, x=2, label='wma-athlons.%s.%s' % (self.data_year,
         gender))
