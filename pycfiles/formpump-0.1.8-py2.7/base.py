...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/formpump/test/base.py
# Compiled at: 2011-10-18 22:11:51
...
class InputTests(object):
    ...
    def test_dynamic_name(self):
        tpl = self._run_template(self.dynamic_name(), var='est')
        self.assertHTMLEqual(tpl, '<input type="text" name="test" value="" />')

    @skipIfUndef('attrs')
    def test_attrs(self):
        tpl = self._run_template(self.attrs(), var='est')
...
class FillTests(object):
    ...
    def test_checkbox_fill(self):
        tpl = self._run_template(self.checkbox_fill(), form_vars={'test': {'var_a': True, 'var_c': 'c'}})
        self.assertHTMLEqual(tpl, '<form action="" method="post"><input checked="checked" type="checkbox" name="var_a" value="1" /><input type="checkbox" name="var_b" value="1" /><input checked="checked" type="checkbox" name="var_c" value="c" /></form>')

    @skipIfUndef('email_fill')
    def test_email_fill(self):
        tpl = self._run_template(self.email_fill(), form_vars={'test': {'var': 'val'}})
        self.assertHTMLEqual(tpl, '<form action="" method="post"><input type="email" name="var" value="val" /></form>')

    @skipIfUndef('hidden_fill')
    def test_hidden_fill(self):
        tpl = self._run_template(self.hidden_fill(), form_vars={'test': {'var': 'val'}})
        self.assertHTMLEqual(tpl, '<form action="" method="post"><input type="hidden" name="var" value="val" /></form>')

    @skipIfUndef('password_fill')
    def test_password_fill(self):
        tpl = self._run_template(self.password_fill(), form_vars={'test': {'var': 'val'}})
        self.assertHTMLEqual(tpl, '<form action="" method="post"><input type="password" name="var" value="val" /></form>')

    @skipIfUndef('radio_fill')
    def test_radio_fill(self):
        tpl = self._run_template(self.radio_fill(), form_vars={'test': {'var': 'b'}})
        self.assertHTMLEqual(tpl, '<form action="" method="post"><input type="radio" name="var" value="a" /><input checked="checked" type="radio" name="var" value="b" /><input type="radio" name="var" value="c" /></form>')

    @skipIfUndef('submit_fill')
    def test_submit_fill(self):
        tpl = self._run_template(self.submit_fill(), form_vars={'test': {'var': 'val'}})
        self.assertHTMLEqual(tpl, '<form action="" method="post"><input type="submit" name="var" value="val" /></form>')

    @skipIfUndef('text_fill')
    def test_text_fill(self):
        tpl = self._run_template(self.text_fill(), form_vars={'test': {'var': 'val'}})
        self.assertHTMLEqual(tpl, '<form action="" method="post"><input type="text" name="var" value="val" /></form>')

    @skipIfUndef('textarea_fill')
    def test_textarea_fill(self):
        tpl = self._run_template(self.textarea_fill(), form_vars={'test': {'var': 'val'}})
        self.assertHTMLEqual(tpl, '<form action="" method="post"><textarea name="var">val</textarea></form>')
...
class FormContextTests(object):
    ...
    def test_form_context_fill(self):
        tpl = self._run_template(self.form_context_fill(), form_vars={'test': {'var': 'test'}, 'ctx': {'var': 'ctx'}})
...
class LabelTests(object):
    ...
    def test_label_match_forward(self):
        tpl = self._run_template(self.label_match_forward(), strip_id=False)
    ...
    def test_label_match_forward_multi(self):
        ...
        tags = self.get_tags(tpl)
        self.assertEqual(tags[1]['attrs']['for'], tags[4]['attrs']['id'])
        self.assertEqual(tags[5]['attrs']['for'], tags[8]['attrs']['id'])

    @skipIfUndef('label_match_back_multi')
    def test_label_match_back_multi(self):
        tpl = self._run_template(self.label_match_back_multi(), strip_id=False)
        ...
        tags = self.get_tags(tpl)
        self.assertEqual(tags[2]['attrs']['for'], tags[1]['attrs']['id'])
        self.assertEqual(tags[6]['attrs']['for'], tags[5]['attrs']['id'])
...
class ErrorTests(object):
    ...
    def test_error(self):
        ...
        self.assertHTMLEqual(tpl, '<form action="" method="post">ok</form>')
        tpl = self._run_template(self.error(), form_errors={'test': {'a': 'a'}})
        self.assertHTMLEqual(tpl, '<form action="" method="post"><span class="error">a</span>ok</form>')

    @skipIfUndef('multi_error')
    def test_multi_error(self):
        tpl = self._run_template(self.multi_error(), form_errors={'test': {'a': 'a'}})
        self.assertHTMLEqual(tpl, '<form action="" method="post"><span class="error">a</span>ok</form>')
        tpl = self._run_template(self.multi_error(), form_errors={'test': {'b': 'b'}})
        self.assertHTMLEqual(tpl, '<form action="" method="post">ok<span class="error">b</span></form>')
        tpl = self._run_template(self.multi_error(), form_errors={'test': {'a': 'a', 'b': 'b'}})
        self.assertHTMLEqual(tpl, '<form action="" method="post"><span class="error">a</span>ok<span class="error">b</span></form>')
        tpl = self._run_template(self.multi_error(), form_errors={'test': {}})
        self.assertHTMLEqual(tpl, '<form action="" method="post">ok</form>')
    ...
    def test_error_renderer(self):
        ...
        with self.assertRaises(ValueError):
            tpl = self._run_template(self.error_renderer(), form_errors={'test': {'a': 'a'}})
...
class HTMLQueueParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        self.tags.append({'type': self.START_TAG, 'tag': tag, 
           'attrs': self.attrs_to_dict(attrs)})

    def handle_endtag(self, tag):
        self.tags.append({'type': self.END_TAG, 'tag': tag, 
           'attrs': {}})

    def handle_startendtag(self, tag, attrs):
        self.tags.append({'type': self.STARTEND_TAG, 'tag': tag, 
           'attrs': self.attrs_to_dict(attrs)})

    def handle_data(self, data):
        self.tags.append({'type': self.DATA, 'tag': data, 
           'attrs': {}})
