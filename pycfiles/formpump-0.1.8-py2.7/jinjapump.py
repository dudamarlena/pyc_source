...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/formpump/jinjapump.py
# Compiled at: 2013-10-08 22:13:58
...
class JinjaPump(Extension):
    ...
    def __init__(self, environment):
        ...
        environment.extend(default_form_action='', error_renderers={}, value_dict_name='form_vars', error_dict_name='form_errors', form_name_key=None, form_ctx_key=None)
        self.form = StubForm()
        return
    ...
    def parse(self, parser):
        ...
        if tag.value == 'form_ctx':
            return self._form_ctx(parser, tag)
        if tag.value in ('email', 'file', 'hidden', 'password', 'text'):
            return self._input(parser, tag)
        if tag.value == 'checkbox':
            return self._check(parser, tag)
        if tag.value == 'iferror':
            return self._iferror(parser, tag)
        if tag.value == 'ifnoterror':
    ...
    def _form(self, parser, tag):
        ...
        return nodes.CallBlock(self.call_method('_form_block', args=[
         form_name, attrs,
         self._form_vars_node(),
         self._form_errors_node()]), [], [], body).set_lineno(tag.lineno)

    def _form_block(self, form_name, attrs, form_vars, form_errors, caller):
        form = Form(form_name, self.environment.form_name_key, self.environment.form_ctx_key, attrs, self.environment.default_form_action, form_vars, form_errors)
    ...
    def _label(self, parser, tag):
        ...
        attrs = nodes.Dict([ nodes.Pair(nodes.Const(k), v) for k, v in attrs.items() ])
        return nodes.CallBlock(self.call_method('_label_block', args=[attrs]), [], [], body).set_lineno(tag.lineno)
