...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/paddle/tight.py
# Compiled at: 2011-01-27 04:26:39
"""
Implementation of tight-frames (TF) version of PADDLE.
"""
import time, numpy as NP, numpy.random as RA
...
def _pgd(D0, X, U, XUt, UUt, pars, maxiter=500, bound=1):
    ...
    (d, K) = D0.shape
    N = U.shape[1]
    sigma = 2.0 / N * (U ** 2).sum() + 4 * pars['eta'] * (1 + K + d) / d ** 2 * (D0 ** 2).sum()
    D = D0.copy()
    t = 1
    Y = D.copy()
    f1 = 2.0 * N * pars['eta'] / d
