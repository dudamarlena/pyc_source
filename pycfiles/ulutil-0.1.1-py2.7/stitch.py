...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/ulutil/stitch.py
# Compiled at: 2014-12-19 21:47:34
...
def stitch(record1, record2):
    ...
    log10p_consensus_1 = log1p(-power(10, -quals1 / 10.0)) / log(10)
    log10p_consensus_2 = log1p(-power(10, -quals2 / 10.0)) / log(10)
    log10p_error_1 = -log10(3) - quals1 / 10.0
    log10p_error_2 = -log10(3) - quals2 / 10.0
    min_overlap = 1
    ...
    for overlap in range(1, max_overlap):
        ...
        score = sum(choose(consensus == s1, [lpe1, lpc1])) + sum(choose(consensus == s2, [lpe2, lpc2])) + len(consensus) * log10(4) * 2
