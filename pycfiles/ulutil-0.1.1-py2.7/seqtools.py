...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/ulutil/seqtools.py
# Compiled at: 2014-12-19 21:47:27
import copy, json, string, random, itertools
from Bio import Alphabet, pairwise2
from Bio.Seq import Seq
from Bio.SeqRecord import SeqRecord
from Bio.SeqFeature import SeqFeature, FeatureLocation
import numpy as np, scipy as sp, scipy.stats
...
global_align = lambda seq1, seq2: pairwise2.align.globalms(seq1, seq2, 0.5, -0.75, -2.0, -1.5, one_alignment_only=True)[0]

def percent_id(seq1, seq2):
    alignment = global_align(seq1, seq2)
    return (1.0 - hamming_distance(alignment[0], alignment[1]) / float(len(alignment[0]))) * 100.0
...
def barcode_probabilities(observed, barcodes):
    ...
    E = np.log1p(-np.power(10, -obs_qual / 10.0))
    D = -np.log(3) - obs_qual / 10.0 * np.log(3)
...
def simplifySeqRecord(record):
    ...
    obj['letter_annotations'] = record.letter_annotations
