...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/ulutil/unafold.py
# Compiled at: 2014-12-19 21:47:45
import os, glob, random, tempfile, subprocess
...
def hybrid_min_list(seqlist1, seqlist2, NA='RNA', tmin=37, tinc=1, tmax=37, sodium=1, magnesium=0):
    ...
    for i, seq in enumerate(seqlist1):
        print >> seqfile1, '>1_%i\n%s' % (i, seq)

    for i, seq in enumerate(seqlist2):
        print >> seqfile2, '>2_%i\n%s' % (i, seq)
    ...
    seqfile1.close()
    seqfile2.close()
    for filename in glob.glob(temp_out_prefix + '*'):
        os.remove(filename)
