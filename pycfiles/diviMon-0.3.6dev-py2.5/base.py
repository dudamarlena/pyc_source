...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/divimon/lib/base.py
# Compiled at: 2008-08-07 07:03:50
...
import divimon.lib.helpers as h, divimon.model as model
...
class BaseController(WSGIController):
    ...
    def __after__(self):
        ...
        history = model.History(user=ht_session.user.id, controller=unicode(wsgi['controller']), action=unicode(wsgi['action']), arguments=unicode(env['QUERY_STRING']))
    ...
    def _get_cookie(self):
        ...
        elif not request.path_info.endswith('error') and not request.path_info.endswith('login'):
    ...
    def _new_session(self, user):
        ...
        """
        ht_session = g.new_session(id=str(id(request.GET)), user=user)
