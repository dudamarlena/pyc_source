...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/divimon/controllers/budget.py
# Compiled at: 2008-08-04 09:33:56
import logging
...
class BudgetController(ListController):
    table = model.Budget
    children = dict(budget_expense=dict(table=model.BudgetExpense, columns=('expense', ), parent=dict(expense=dict(table=model.Expense, column='id'))))
    ...
    def cashflow_particular_date_range(self):
        if 'start_day' not in request.params:
