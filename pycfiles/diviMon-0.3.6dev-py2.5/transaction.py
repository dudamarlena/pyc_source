...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/divimon/controllers/transaction.py
# Compiled at: 2008-07-30 10:44:05
import logging
...
class TransactionController(ListController):
    ...
    parent = dict(area=dict(table=model.Area, column='name'), type=dict(table=model.TransType, column='name'), agent=dict(table=model.Agent, column='name'), customer=dict(table=model.Customer, column='name'), pay_type=dict(table=model.PayType, column='name'), pay_status=dict(table=model.PayStatus, column='name'))
    children = dict(trans_item=dict(table=model.TransItem, columns=('item', 'qty',
                                                                    'price', 'received_qty'), parent=dict(item=dict(table=model.Item, column='name'))))
    prices = dict()
    total = 0
    column_hidden = set([
     'pay_status'])
