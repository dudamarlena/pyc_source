...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/simpleweb/extlib/selector.py
...
def method_not_allowed(environ, start_response):
    ...
    start_response('405 Method Not Allowed', [
     (
      'Allow', (', ').join(environ['selector.methods'])), ('Content-Type', 'text/plain')])
    return [
...
class Selector(object):
    ...
    def __call__(self, environ, start_response):
        ...
        (app, svars, methods) = self.select(environ['PATH_INFO'], environ['REQUEST_METHOD'])
        unnamed, named = [], {}
        ...
        for (regex, method_dict) in self.mappings:
            ...
            if match:
                ...
                if method_dict.has_key(method):
                    return (
                     method_dict[method], match.groupdict(), methods)
                elif method_dict.has_key('_ANY_'):
                    return (
                     method_dict['_ANY_'], match.groupdict(), methods)
                else:
                    return (
                     self.status405, {}, methods)
...
class SimpleParser(object):
    ...
    (start, end) = '{}'
    (ostart, oend) = '[]'
    _patterns = {'word': '\\w+', 'alpha': '[a-zA-Z]+', 'digits': '\\d+', 'number': '\\d*.?\\d+', 'chunk': '[^/^.]+', 'segment': '[^/]+', 'any': '.+'}
...
class ByMethod(object):
    ...
    def __call__(self, environ, start_response):
        ...
        return getattr(self, environ['REQUEST_METHOD'], self._method_not_allowed)(environ, start_response)
