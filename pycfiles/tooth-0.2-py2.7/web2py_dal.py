...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tooth/web2py_dal.py
...
>>> db = DAL(('sqlite://storage.sqlite','mysql://a:b@localhost/x'), ... folder=None)
...
TIMINGSSIZE = 100
SPATIALLIBS = {'Windows': 'libspatialite', 'Linux': 'libspatialite.so', 'Darwin': 'libspatialite.dylib'}
DEFAULT_URI = 'sqlite://dummy.db'
import re, sys, locale, os, types, datetime, threading, time, csv, cgi, copy, socket, logging, base64, shutil, marshal, decimal, struct, urllib, hashlib, uuid, glob, traceback, platform
PYTHON_VERSION = sys.version_info[:3]
if PYTHON_VERSION[0] == 2:
    import cPickle as pickle, cStringIO as StringIO, copy_reg as copyreg
...
if PYTHON_VERSION[:2] < (2, 7):
    raise NotImplemented('Check dependencies >> ')
else:
    from collections import OrderedDict
CALLABLETYPES = (
 types.LambdaType, types.FunctionType, types.BuiltinFunctionType, types.MethodType, types.BuiltinMethodType)
TABLE_ARGS = set((
 'migrate', 'primarykey', 'fake_migrate', 'format', 'redefine', 'singular', 'plural', 'trigger_name', 'sequence_name', 'fields', 'common_filter',
 'polymodel', 'table_class', 'on_define', 'rname'))
...
PLURALIZE_RULES = [
 (
  re.compile('child$'), re.compile('child$'), 'children'), (re.compile('oot$'), re.compile('oot$'), 'eet'),
 (
  re.compile('ooth$'), re.compile('ooth$'), 'eeth'), (re.compile('l[eo]af$'), re.compile('l([eo])af$'), 'l\\1aves'),
 (
  re.compile('sis$'), re.compile('sis$'), 'ses'), (re.compile('man$'), re.compile('man$'), 'men'),
 (
  re.compile('ife$'), re.compile('ife$'), 'ives'), (re.compile('eau$'), re.compile('eau$'), 'eaux'),
 (
  re.compile('lf$'), re.compile('lf$'), 'lves'), (re.compile('[sxz]$'), re.compile('$'), 'es'),
 (
  re.compile('[^aeioudgkprt]h$'), re.compile('$'), 'es'), (re.compile('(qu|[^aeiou])y$'), re.compile('y$'), 'ies'),
 (
...
def varquote_aux(name, quotestr='%s'):
    if REGEX_W.match(name):
        return name
    return quotestr % name
...
class ConnectionPool(object):
    ...
    def find_or_make_work_folder(self):
        ...
        if os.path.isabs(self.folder) and isinstance(self, UseDatabaseStoredFile) and self.folder.startswith(os.getcwd()):
...
"""
    ...
    def __call__(cls, *args, **kwargs):
        ...
        obj.REGEX_TABLE_DOT_FIELD = re.compile('^' + quot % regex_ent + '\\.' + quot % regex_ent + '$')
        return obj
...
class BaseAdapter(ConnectionPool):
    ...
    T_SEP = ' '
    QUOTE_TEMPLATE = '"%s"'
    types = {'boolean': 'CHAR(1)', 'string': 'CHAR(%(length)s)', 'text': 'TEXT', 'json': 'TEXT', 'password': 'CHAR(%(length)s)', 'blob': 'BLOB', 'upload': 'CHAR(%(length)s)', 
       'integer': 'INTEGER', 'bigint': 'INTEGER', 'float': 'DOUBLE', 'double': 'DOUBLE', 'decimal': 'DOUBLE', 'date': 'DATE', 
       'time': 'TIME', 'datetime': 'TIMESTAMP', 'id': 'INTEGER PRIMARY KEY AUTOINCREMENT', 'reference': 'INTEGER REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s', 
       'list:integer': 'TEXT', 'list:string': 'TEXT', 'list:reference': 'TEXT', 
       'big-id': 'BIGINT PRIMARY KEY AUTOINCREMENT', 
       'big-reference': 'BIGINT REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s', 'reference FK': ', CONSTRAINT  "FK_%(constraint_name)s" FOREIGN KEY (%(field_name)s) REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s'}
    ...
    def find_driver(self, adapter_args, uri=None):
        ...
        if getattr(self, 'driver', None) != None:
            ...
            drivers_available = [ driver for driver in self.drivers if driver in globals() ]
            if uri:
    ...
    def __init__(self, db, uri, pool_size=0, folder=None, db_codec='UTF-8', credential_decoder=IDENTITY, driver_args={}, adapter_args={}, do_connect=True, after_connection=None):
        self.db = db
        self.dbengine = 'None'
        self.uri = uri
    ...
    def create_table(self, table, migrate=True, fake_migrate=False, polymodel=None):
        ...
        for field in table:
            ...
            if isinstance(field_type, SQLCustomType):
                ...
                if getattr(rtable, '_primarykey', None) and rfieldname in rtable._primarykey or rfield.unique:
                    ...
                    if not rfield.unique and len(rtable._primarykey) > 1:
                        ...
                        ftype = ftype + types['reference FK'] % dict(constraint_name=constraint_name, foreign_key=rtable.sqlsafe + ' (' + rfield.sqlsafe_name + ')', table_name=table.sqlsafe, field_name=field.sqlsafe_name, on_delete_action=field.ondelete)
                else:
                    if referenced in db:
                    ...
                    if referenced == tablename:
                        ...
                        real_referenced = referenced in db and db[referenced].sqlsafe or referenced
                    rfield = db[referenced]._id
                    ftype = types[field_type[:9]] % dict(index_name=self.QUOTE_TEMPLATE % (field_name + '__idx'), field_name=field.sqlsafe_name, constraint_name=self.QUOTE_TEMPLATE % constraint_name, foreign_key='%s (%s)' % (real_referenced, rfield.sqlsafe_name), on_delete_action=field.ondelete)
                ...
                if geotype not in types:
                    raise SyntaxError('Field: unknown field type: %s for %s' % (field_type, field_name))
                ftype = types[geotype]
                if self.dbengine == 'postgres' and geotype == 'geometry':
                    ...
                    ftype = ftype % dict(schema=schema, tablename=tablename, fieldname=field_name, srid=srid, dimension=dimension)
                    postcreation_fields.append(ftype)
            elif field_type not in types:
                raise SyntaxError('Field: unknown field type: %s for %s' % (field_type, field_name))
            else:
                ftype = types[field_type] % dict(length=field.length)
            if not field_type.startswith('id') and not field_type.startswith('reference'):
                if field.notnull:
                    ftype += ' NOT NULL'
                else:
            ...
            sql_fields[field_name] = dict(length=field.length, unique=field.unique, notnull=field.notnull, sortable=sortable, type=str(field_type), sql=ftype)
        ...
        for rtablename in TFK:
            ...
            fields = fields + ',\n    ' + types['reference TFK'] % dict(table_name=table.sqlsafe, field_name=(', ').join(fkeys), foreign_table=table.sqlsafe, foreign_key=(', ').join(pkeys), on_delete_action=field.ondelete)

        table_rname = table.sqlsafe
        if getattr(table, '_primarykey', None):
            query = 'CREATE TABLE %s(\n    %s,\n    %s) %s' % (
             table.sqlsafe, fields, self.PRIMARY_KEY((', ').join([ self.QUOTE_TEMPLATE % pk for pk in table._primarykey ])), other)
        else:
            query = 'CREATE TABLE %s(\n    %s\n)%s' % (table.sqlsafe, fields, other)
        ...
        if not migrate:
            ...
            if self.uri.startswith('sqlite:memory') or self.uri.startswith('spatialite:memory'):
                table._dbt = None
            else:
                if isinstance(migrate, str):
                    table._dbt = pjoin(dbpath, migrate)
                else:
                    table._dbt = pjoin(dbpath, '%s_%s.table' % (table._db._uri_hash, tablename))
                ...
                if sql_fields != sql_fields_old:
                    self.migrate_table(table, sql_fields, sql_fields_old, sql_fields_aux, None, fake_migrate=fake_migrate)
            return query

    def migrate_table(self, table, sql_fields, sql_fields_old, sql_fields_aux, logfile, fake_migrate=False):
        db = table._db
        ...
        def fix(item):
            ...
            if self.ignore_field_case is not True:
                return (k, v)
            return (
             k.lower(), v)
        ...
        for key in keys:
            ...
            if key not in sql_fields_old:
                ...
                if self.dbengine in ('postgres', ) and sql_fields[key]['type'].startswith('geometry'):
                ...
                if self.dbengine in ('postgres', ) and ftype.startswith('geometry'):
                    ...
                    query = [
                     "SELECT DropGeometryColumn ('%(schema)s', " + "'%(table)s', '%(field)s');" % dict(schema=schema, table=tablename, field=key)]
                elif self.dbengine in ('firebird', ):
                    query = [
                     'ALTER TABLE %s DROP %s;' % (self.QUOTE_TEMPLATE % tablename, self.QUOTE_TEMPLATE % key)]
                else:
                    query = [
                     'ALTER TABLE %s DROP COLUMN %s;' % (self.QUOTE_TEMPLATE % tablename, self.QUOTE_TEMPLATE % key)]
                metadata_change = True
            elif sql_fields[key]['sql'] != sql_fields_old[key]['sql'] and not (key in table.fields and isinstance(table[key].type, SQLCustomType)) and not sql_fields[key]['type'].startswith('reference') and not sql_fields[key]['type'].startswith('double') and not sql_fields[key]['type'].startswith('id'):
                sql_fields_current[key] = sql_fields[key]
                t = tablename
                ...
                query = ['ALTER TABLE %s ADD %s %s;' % (self.QUOTE_TEMPLATE % t, self.QUOTE_TEMPLATE % key_tmp, tt),
                 'UPDATE %s SET %s=%s;' % (self.QUOTE_TEMPLATE % t, self.QUOTE_TEMPLATE % key_tmp, self.QUOTE_TEMPLATE % key),
                 drop_expr % (self.QUOTE_TEMPLATE % t, self.QUOTE_TEMPLATE % key),
                 'ALTER TABLE %s ADD %s %s;' % (self.QUOTE_TEMPLATE % t, self.QUOTE_TEMPLATE % key, tt),
                 'UPDATE %s SET %s=%s;' % (self.QUOTE_TEMPLATE % t, self.QUOTE_TEMPLATE % key, self.QUOTE_TEMPLATE % key_tmp),
    ...
    def COUNT(self, first, distinct=None):
        return ('COUNT(%s)' if not distinct else 'COUNT(DISTINCT %s)') % self.expand(first)
    ...
    def IN(self, column, values):
        if not values:
            return '0'
        return '%s IN (%s)' % (self.expand(column), (', ').join(values))
    ...
    def COALESCE(self, first, second):
        expressions = [
         self.expand(first)] + [ self.expand(e) for e in second ]
    ...
    def CONTAINS(self, first, second, case_sensitive=False):
        if first.type in ('string', 'text', 'json'):
            if isinstance(second, Expression):
                second = Expression(None, self.CONCAT('%', Expression(None, self.REPLACE(second, ('%',
                                                                                                  '%%'))), '%'))
            else:
                second = '%' + str(second).replace('%', '%%') + '%'
        elif first.type.startswith('list:'):
            if isinstance(second, Expression):
                second = Expression(None, self.CONCAT('%|', Expression(None, self.REPLACE(Expression(None, self.REPLACE(second, ('%',
                                                                                                                                 '%%'))), ('|',
                                                                                                                                           '||'))), '|%'))
            else:
    ...
    def is_numerical_type(self, ftype):
        return ftype in ('integer', 'boolean', 'double', 'bigint') or ftype.startswith('decimal')

    def REPLACE(self, first, (second, third)):
        return 'REPLACE(%s,%s,%s)' % (self.expand(first, 'string'), self.expand(second, 'string'), self.expand(third, 'string'))
    ...
    def expand(self, expression, field_type=None, colnames=False):
        if isinstance(expression, Field):
            ...
            if field_type == 'string' and expression.type not in ('string', 'text',
                                                                  'json', 'password'):
    ...
    def _truncate(self, table, mode=''):
        return [
         'TRUNCATE TABLE %s %s;' % (table.sqlsafe, mode or '')]
    ...
    def _update(self, tablename, query, fields):
        if query:
            ...
            sql_w = ''
        sql_v = (',').join([ '%s=%s' % (field.sqlsafe_name, self.expand(value, field.type)) for field, value in fields ])
    ...
    def update(self, tablename, query, fields):
        ...
        for i in range(0, 11):
            try:
                ...
                if err.__class__.__name__ == 'OperationalError' and err[0] == 1213 and i < 10:
                    time.sleep(0.2)
                    continue
                else:
                    e = sys.exc_info()[1]
    ...
    def delete(self, tablename, query):
        ...
        if self.dbengine in ('sqlite', 'spatialite') and counter:
            for field in table._referenced_by:
                if field.type == 'reference ' + table._tablename and field.ondelete == 'CASCADE':
    ...
    def _select(self, query, fields, attributes):
        ...
        if inner_join:
            ...
            itables_to_merge = {}
            [ itables_to_merge.update(dict.fromkeys(tables(t))) for t in ijoinon ]
            ijoinont = [ t.first._tablename for t in ijoinon ]
            [ itables_to_merge.pop(t) for t in ijoinont if t in itables_to_merge ]
            iimportant_tablenames = ijoint + ijoinont + itables_to_merge.keys()
            iexcluded = [ t for t in tablenames if t not in iimportant_tablenames ]
        if left:
            ...
            tables_to_merge = {}
            [ tables_to_merge.update(dict.fromkeys(tables(t))) for t in joinon ]
            joinont = [ t.first._tablename for t in joinon ]
            [ tables_to_merge.pop(t) for t in joinont if t in tables_to_merge ]
            tablenames_for_common_filters = [ t for t in tablenames if t not in joinont ]
        ...
        if inner_join and not left:
            sql_t = (', ').join([ self.table_alias(t) for t in iexcluded + itables_to_merge.keys() ])
            for t in ijoinon:
                sql_t += ' %s %s' % (icommand, t)

        elif not inner_join and left:
            sql_t = (', ').join([ self.table_alias(t) for t in excluded + tables_to_merge.keys() ])
            if joint:
            ...
            all_tables_in_query = set(important_tablenames + iimportant_tablenames + tablenames)
            tables_in_joinon = set(joinont + ijoinont)
            tables_not_in_joinon = all_tables_in_query.difference(tables_in_joinon)
        ...
        if limitby and not groupby and tablenames and orderby_on_limitby and not orderby:
            sql_o += ' ORDER BY %s' % (', ').join([ self.db[t].sqlsafe + '.' + self.db[t][x].sqlsafe_name for t in tablenames for x in hasattr(self.db[t], '_primarykey') and self.db[t]._primarykey or ['_id']
                                                  ])
    ...
    def select(self, query, fields, attributes):
        ...
        if cache and attributes.get('cacheable', False):
            ...
            if len(key) > 200:
                key = hashlib_md5(key).hexdigest()
            args = (
             sql, fields, attributes)
            return cache_model(key, lambda self=self, args=args: self._select_aux(*args), time_expire)
        else:
    ...
    def represent(self, obj, fieldtype):
        ...
        if isinstance(obj, (Expression, Field)):
            ...
            if obj is None:
                return 'NULL'
            if obj == '' and fieldtype[:2] not in ('st', 'te', 'js', 'pa', 'up'):
                return 'NULL'
            r = self.represent_exceptions(obj, fieldtype)
            if r is not None:
                return r
            if fieldtype == 'boolean':
                if obj and str(obj)[:1].upper() not in '0F':
    ...
    def parse_value(self, value, field_type, blob_decode=True):
        ...
        if not isinstance(field_type, str) or value is None:
            return value
        if field_type in ('string', 'text', 'password', 'upload', 'dict'):
            return value
        else:
            if field_type.startswith('geo'):
                return value
            else:
                if field_type == 'blob' and not blob_decode:
                    return value
    ...
    def build_parsemap(self):
        self.parsemap = {'id': self.parse_id, 'integer': self.parse_integer, 'bigint': self.parse_integer, 'float': self.parse_double, 'double': self.parse_double, 
           'reference': self.parse_reference, 'boolean': self.parse_boolean, 'date': self.parse_date, 'time': self.parse_time, 
           'datetime': self.parse_datetime, 'blob': self.parse_blob, 'decimal': self.parse_decimal, 'json': self.parse_json, 
           'list:integer': self.parse_list_integers, 'list:reference': self.parse_list_references, 'list:string': self.parse_list_strings}
    ...
    def parse(self, rows, fields, colnames, blob_decode=True, cacheable=False):
                ...
                if tmp:
                    ...
                    if ft == 'id' and not cacheable:
                        ...
                        for rfield in table._referenced_by:
                            referee_link = db._referee_name and db._referee_name % dict(table=rfield.tablename, field=rfield.name)
                            if referee_link and referee_link not in colset:
                                colset[referee_link] = LazySet(rfield, id)

                else:
                    if '_extra' not in new_row:
                        new_row['_extra'] = Row()
                    new_row['_extra'][colname] = self.parse_value(value, fields[j].type, blob_decode)
                    new_column_name = REGEX_SELECT_AS_PARSER.search(colname)
        ...
        for tablename in virtualtables:
            table = db[tablename]
            fields_virtual = [ (f, v) for f, v in table.iteritems() if isinstance(v, FieldVirtual) ]
            fields_lazy = [ (f, v) for f, v in table.iteritems() if isinstance(v, FieldMethod) ]
    ...
    def CASE(self, query, t, f):
        ...
        return Expression(self.db, 'CASE WHEN %s THEN %s ELSE %s END' % (self.expand(query), represent(t), represent(f)))
...
class SQLiteAdapter(BaseAdapter):
    ...
    def web2py_extract(lookup, s):
        table = {'year': (0, 4), 'month': (5, 7), 'day': (8, 10), 'hour': (11, 13), 'minute': (14, 16), 'second': (17, 19)}
        try:
            if lookup != 'epoch':
                i, j = table[lookup]
                return int(s[i:j])
    ...
    def __init__(self, db, uri, pool_size=0, folder=None, db_codec='UTF-8', credential_decoder=IDENTITY, driver_args={}, adapter_args={}, do_connect=True, after_connection=None):
        ...
        if uri.startswith('sqlite:memory'):
            ...
            if self.dbpath[0] != '/':
                if PYTHON_VERSION[0] == 2:
                    self.dbpath = pjoin(self.folder.decode(path_encoding).encode('utf8'), self.dbpath)
                else:
                    self.dbpath = pjoin(self.folder, self.dbpath)
        if 'check_same_thread' not in driver_args:
            driver_args['check_same_thread'] = False
        if 'detect_types' not in driver_args and do_connect:
    ...
    def after_connection(self):
        ...
        self.connection.create_function('REGEXP', 2, SQLiteAdapter.web2py_regexp)
        if self.adapter_args.get('foreign_keys', True):
            self.execute('PRAGMA foreign_keys=ON;')

    def _truncate(self, table, mode=''):
        tablename = table._tablename
        return ['DELETE FROM %s;' % tablename, "DELETE FROM sqlite_sequence WHERE name='%s';" % tablename]

    def lastrowid(self, table):
        return self.cursor.lastrowid

    def REGEXP(self, first, second):
        return '(%s REGEXP %s)' % (self.expand(first), self.expand(second, 'string'))
...
class SpatiaLiteAdapter(SQLiteAdapter):
    ...
    def __init__(self, db, uri, pool_size=0, folder=None, db_codec='UTF-8', credential_decoder=IDENTITY, driver_args={}, adapter_args={}, do_connect=True, srid=4326, after_connection=None):
        self.db = db
    ...
    def after_connection(self):
        ...
        self.execute('SELECT load_extension("%s");' % libspatialite)
        self.connection.create_function('web2py_extract', 2, SQLiteAdapter.web2py_extract)
        self.connection.create_function('REGEXP', 2, SQLiteAdapter.web2py_regexp)

    def ST_ASGEOJSON(self, first, second):
        return 'AsGeoJSON(%s,%s,%s)' % (self.expand(first), second['precision'], second['options'])
...
class JDBCSQLiteAdapter(SQLiteAdapter):
    ...
    def __init__(self, db, uri, pool_size=0, folder=None, db_codec='UTF-8', credential_decoder=IDENTITY, driver_args={}, adapter_args={}, do_connect=True, after_connection=None):
        ...
        def connector(dbpath=self.dbpath, driver_args=driver_args):
            return self.driver.connect(self.driver.getConnection('jdbc:sqlite:' + dbpath), **driver_args)
...
class MySQLAdapter(BaseAdapter):
    ...
    types = {'boolean': 'CHAR(1)', 'string': 'VARCHAR(%(length)s)', 'text': 'LONGTEXT', 'json': 'LONGTEXT', 'password': 'VARCHAR(%(length)s)', 'blob': 'LONGBLOB', 
       'upload': 'VARCHAR(%(length)s)', 'integer': 'INT', 'bigint': 'BIGINT', 'float': 'FLOAT', 'double': 'DOUBLE', 'decimal': 'NUMERIC(%(precision)s,%(scale)s)', 
       'date': 'DATE', 'time': 'TIME', 'datetime': 'DATETIME', 'id': 'INT AUTO_INCREMENT NOT NULL', 
       ...
       'list:string': 'LONGTEXT', 'list:reference': 'LONGTEXT', 'big-id': 'BIGINT AUTO_INCREMENT NOT NULL', 'big-reference': 'BIGINT, INDEX %(index_name)s (%(field_name)s), FOREIGN KEY (%(field_name)s) REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s', 
    ...
    def _drop(self, table, mode):
        table_rname = table.sqlsafe
        return ['SET FOREIGN_KEY_CHECKS=0;', 'DROP TABLE %s;' % table_rname, 'SET FOREIGN_KEY_CHECKS=1;']
    ...
    def __init__(self, db, uri, pool_size=0, folder=None, db_codec='UTF-8', credential_decoder=IDENTITY, driver_args={}, adapter_args={}, do_connect=True, after_connection=None):
        ...
        driver_args.update(db=db, user=credential_decoder(user), passwd=credential_decoder(password), host=host, port=port, charset=charset)
...
class PostgreSQLAdapter(BaseAdapter):
    ...
    QUOTE_TEMPLATE = '"%s"'
    support_distributed_transaction = True
    types = {'boolean': 'CHAR(1)', 'string': 'VARCHAR(%(length)s)', 'text': 'TEXT', 'json': 'TEXT', 'password': 'VARCHAR(%(length)s)', 'blob': 'BYTEA', 
       'upload': 'VARCHAR(%(length)s)', 'integer': 'INTEGER', 'bigint': 'BIGINT', 'float': 'FLOAT', 'double': 'FLOAT8', 'decimal': 'NUMERIC(%(precision)s,%(scale)s)', 
       'date': 'DATE', 'time': 'TIME', 'datetime': 'TIMESTAMP', 'id': 'SERIAL PRIMARY KEY', 'reference': 'INTEGER REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s', 
       'list:integer': 'TEXT', 'list:string': 'TEXT', 'list:reference': 'TEXT', 
       'geometry': 'GEOMETRY', 'geography': 'GEOGRAPHY', 'big-id': 'BIGSERIAL PRIMARY KEY', 'big-reference': 'BIGINT REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s', 
       'reference FK': ', CONSTRAINT  "FK_%(constraint_name)s" FOREIGN KEY (%(field_name)s) REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s', 
    ...
    def __init__(self, db, uri, pool_size=0, folder=None, db_codec='UTF-8', credential_decoder=IDENTITY, driver_args={}, adapter_args={}, do_connect=True, srid=4326, after_connection=None):
        ...
        if sslmode:
            msg = "dbname='%s' user='%s' host='%s' port=%s password='%s' sslmode='%s'" % (
    ...
    def try_json(self):
        ...
        elif self.driver_name == 'psycopg2' and self.driver.__version__ >= '2.0.12':
    ...
    def REGEXP(self, first, second):
        return '(%s ~ %s)' % (self.expand(first), self.expand(second, 'string'))
    ...
    def ST_ASGEOJSON(self, first, second):
        ...
        """
        return 'ST_AsGeoJSON(%s,%s,%s,%s)' % (second['version'], self.expand(first), second['precision'], second['options'])
    ...
    def ST_DWITHIN(self, first, (second, third)):
        ...
        """
        return 'ST_DWithin(%s,%s,%s)' % (self.expand(first), self.expand(second, first.type), self.expand(third, 'double'))
    ...
    def _drop(self, table, mode='restrict'):
        ...
        return [
         'DROP TABLE ' + table.sqlsafe + ' ' + str(mode) + ';']
...
class NewPostgreSQLAdapter(PostgreSQLAdapter):
    ...
    types = {'boolean': 'CHAR(1)', 'string': 'VARCHAR(%(length)s)', 'text': 'TEXT', 'json': 'TEXT', 'password': 'VARCHAR(%(length)s)', 'blob': 'BYTEA', 
       'upload': 'VARCHAR(%(length)s)', 'integer': 'INTEGER', 'bigint': 'BIGINT', 'float': 'FLOAT', 'double': 'FLOAT8', 'decimal': 'NUMERIC(%(precision)s,%(scale)s)', 
       'date': 'DATE', 'time': 'TIME', 'datetime': 'TIMESTAMP', 'id': 'SERIAL PRIMARY KEY', 'reference': 'INTEGER REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s', 
       'list:integer': 'BIGINT[]', 'list:string': 'TEXT[]', 'list:reference': 'BIGINT[]', 
       'geometry': 'GEOMETRY', 'geography': 'GEOGRAPHY', 'big-id': 'BIGSERIAL PRIMARY KEY', 'big-reference': 'BIGINT REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s'}
...
class OracleAdapter(BaseAdapter):
    drivers = ('cx_Oracle', )
    commit_on_alter_table = False
    types = {'boolean': 'CHAR(1)', 'string': 'VARCHAR2(%(length)s)', 'text': 'CLOB', 'json': 'CLOB', 'password': 'VARCHAR2(%(length)s)', 'blob': 'CLOB', 
       'upload': 'VARCHAR2(%(length)s)', 'integer': 'INT', 'bigint': 'NUMBER', 'float': 'FLOAT', 'double': 'BINARY_DOUBLE', 'decimal': 'NUMERIC(%(precision)s,%(scale)s)', 
       'date': 'DATE', 'time': 'CHAR(8)', 'datetime': 'DATE', 'id': 'NUMBER PRIMARY KEY', 'reference': 'NUMBER, CONSTRAINT %(constraint_name)s FOREIGN KEY (%(field_name)s) REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s', 
       'list:integer': 'CLOB', 
       'list:string': 'CLOB', 'list:reference': 'CLOB', 'big-id': 'NUMBER PRIMARY KEY', 'big-reference': 'NUMBER, CONSTRAINT %(constraint_name)s FOREIGN KEY (%(field_name)s) REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s', 
       'reference FK': ', CONSTRAINT FK_%(constraint_name)s FOREIGN KEY (%(field_name)s) REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s', 
       'reference TFK': ' CONSTRAINT FK_%(foreign_table)s_PK FOREIGN KEY (%(field_name)s) REFERENCES %(foreign_table)s (%(foreign_key)s) ON DELETE %(on_delete_action)s'}
    ...
    def select_limitby(self, sql_s, sql_f, sql_t, sql_w, sql_o, limitby):
        if limitby:
            ...
            return 'SELECT %s %s FROM (SELECT w_tmp.*, ROWNUM w_row FROM (SELECT %s FROM %s%s%s) w_tmp WHERE ROWNUM<=%i) %s %s %s;' % (
             sql_s, sql_f, sql_f, sql_t, sql_w, sql_o, lmax, sql_t, sql_w_row, sql_o)
        return 'SELECT %s %s FROM %s%s%s;' % (sql_s, sql_f, sql_t, sql_w, sql_o)
    ...
    def __init__(self, db, uri, pool_size=0, folder=None, db_codec='UTF-8', credential_decoder=IDENTITY, driver_args={}, adapter_args={}, do_connect=True, after_connection=None):
        ...
        self.find_or_make_work_folder()
        ruri = uri.split('://', 1)[1]
        if 'threaded' not in driver_args:
    ...
    def sqlsafe_table(self, tablename, ot=None):
        if ot is not None:
            return (self.QUOTE_TEMPLATE + ' ' + self.QUOTE_TEMPLATE) % (ot, tablename)
        else:
            return self.QUOTE_TEMPLATE % tablename
...
class MSSQLAdapter(BaseAdapter):
    ...
    T_SEP = 'T'
    QUOTE_TEMPLATE = '"%s"'
    types = {'boolean': 'BIT', 'string': 'VARCHAR(%(length)s)', 'text': 'TEXT', 'json': 'TEXT', 'password': 'VARCHAR(%(length)s)', 'blob': 'IMAGE', 'upload': 'VARCHAR(%(length)s)', 
       'integer': 'INT', 'bigint': 'BIGINT', 'float': 'FLOAT', 'double': 'FLOAT', 'decimal': 'NUMERIC(%(precision)s,%(scale)s)', 
       'date': 'DATETIME', 'time': 'CHAR(8)', 'datetime': 'DATETIME', 'id': 'INT IDENTITY PRIMARY KEY', 
       ...
       'list:integer': 'TEXT', 
       'list:string': 'TEXT', 'list:reference': 'TEXT', 'geometry': 'geometry', 'geography': 'geography', 'big-id': 'BIGINT IDENTITY PRIMARY KEY', 
    ...
    def __init__(self, db, uri, pool_size=0, folder=None, db_codec='UTF-8', credential_decoder=IDENTITY, driver_args={}, adapter_args={}, do_connect=True, srid=4326, after_connection=None):
        ...
        if '@' not in ruri:
            try:
                ...
                if not m:
                    raise SyntaxError('Parsing uri string(%s) has no result' % self.uri)
                dsn = m.group('dsn')
                if not dsn:
            ...
            cnxn = 'SERVER=%s;PORT=%s;DATABASE=%s;UID=%s;PWD=%s;%s' % (host, port, db, user, password, urlargs)
...
class MSSQL3Adapter(MSSQLAdapter):
    ...
    def select_limitby(self, sql_s, sql_f, sql_t, sql_w, sql_o, limitby):
        if limitby:
            ...
            return 'SELECT %s %s FROM (SELECT %s ROW_NUMBER() OVER (ORDER BY %s) AS w_row, %s FROM %s%s%s) TMP WHERE w_row BETWEEN %i AND %s;' % (
             sql_s, sql_f_oproxy, sql_s, sql_f, sql_f_iproxy, sql_t, sql_w, sql_g_inner, lmin, lmax)
        return 'SELECT %s %s FROM %s%s%s;' % (sql_s, sql_f, sql_t, sql_w, sql_o)
...
class MSSQL2Adapter(MSSQLAdapter):
    drivers = ('pyodbc', )
    types = {'boolean': 'CHAR(1)', 'string': 'NVARCHAR(%(length)s)', 'text': 'NTEXT', 'json': 'NTEXT', 'password': 'NVARCHAR(%(length)s)', 'blob': 'IMAGE', 
       'upload': 'NVARCHAR(%(length)s)', 'integer': 'INT', 'bigint': 'BIGINT', 'float': 'FLOAT', 'double': 'FLOAT', 'decimal': 'NUMERIC(%(precision)s,%(scale)s)', 
       'date': 'DATETIME', 'time': 'CHAR(8)', 'datetime': 'DATETIME', 'id': 'INT IDENTITY PRIMARY KEY', 
       ...
       'list:integer': 'NTEXT', 
       'list:string': 'NTEXT', 'list:reference': 'NTEXT', 'big-id': 'BIGINT IDENTITY PRIMARY KEY', 'big-reference': 'BIGINT, CONSTRAINT %(constraint_name)s FOREIGN KEY (%(field_name)s) REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s', 
       'reference FK': ', CONSTRAINT FK_%(constraint_name)s FOREIGN KEY (%(field_name)s) REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s', 
       'reference TFK': ' CONSTRAINT FK_%(foreign_table)s_PK FOREIGN KEY (%(field_name)s) REFERENCES %(foreign_table)s (%(foreign_key)s) ON DELETE %(on_delete_action)s'}
...
class VerticaAdapter(MSSQLAdapter):
    drivers = ('pyodbc', )
    T_SEP = ' '
    types = {'boolean': 'BOOLEAN', 'string': 'VARCHAR(%(length)s)', 'text': 'BYTEA', 'json': 'VARCHAR(%(length)s)', 'password': 'VARCHAR(%(length)s)', 'blob': 'BYTEA', 
       'upload': 'VARCHAR(%(length)s)', 'integer': 'INT', 'bigint': 'BIGINT', 'float': 'FLOAT', 'double': 'DOUBLE PRECISION', 'decimal': 'DECIMAL(%(precision)s,%(scale)s)', 
       'date': 'DATE', 'time': 'TIME', 'datetime': 'DATETIME', 'id': 'IDENTITY', 'reference': 'INT REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s', 
       'list:integer': 'BYTEA', 'list:string': 'BYTEA', 'list:reference': 'BYTEA', 
...
class SybaseAdapter(MSSQLAdapter):
    drivers = ('Sybase', )
    types = {'boolean': 'BIT', 'string': 'CHAR VARYING(%(length)s)', 'text': 'TEXT', 'json': 'TEXT', 'password': 'CHAR VARYING(%(length)s)', 'blob': 'IMAGE', 
       'upload': 'CHAR VARYING(%(length)s)', 'integer': 'INT', 'bigint': 'BIGINT', 'float': 'FLOAT', 'double': 'FLOAT', 'decimal': 'NUMERIC(%(precision)s,%(scale)s)', 
       'date': 'DATETIME', 'time': 'CHAR(8)', 'datetime': 'DATETIME', 'id': 'INT IDENTITY PRIMARY KEY', 
       'reference': 'INT NULL, CONSTRAINT %(constraint_name)s FOREIGN KEY (%(field_name)s) REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s', 
       'list:integer': 'TEXT', 
       'list:string': 'TEXT', 'list:reference': 'TEXT', 'geometry': 'geometry', 'geography': 'geography', 'big-id': 'BIGINT IDENTITY PRIMARY KEY', 
    ...
    def __init__(self, db, uri, pool_size=0, folder=None, db_codec='UTF-8', credential_decoder=IDENTITY, driver_args={}, adapter_args={}, do_connect=True, srid=4326, after_connection=None):
        ...
        if '@' not in ruri:
            ...
            driver_args.update(user=credential_decoder(user), password=credential_decoder(password))
...
class FireBirdAdapter(BaseAdapter):
    ...
    commit_on_alter_table = False
    support_distributed_transaction = True
    types = {'boolean': 'CHAR(1)', 'string': 'VARCHAR(%(length)s)', 'text': 'BLOB SUB_TYPE 1', 'json': 'BLOB SUB_TYPE 1', 'password': 'VARCHAR(%(length)s)', 
       'blob': 'BLOB SUB_TYPE 0', 'upload': 'VARCHAR(%(length)s)', 'integer': 'INTEGER', 'bigint': 'BIGINT', 'float': 'FLOAT', 
       'double': 'DOUBLE PRECISION', 'decimal': 'DECIMAL(%(precision)s,%(scale)s)', 'date': 'DATE', 'time': 'TIME', 'datetime': 'TIMESTAMP', 
       'id': 'INTEGER PRIMARY KEY', 'reference': 'INTEGER REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s', 'list:integer': 'BLOB SUB_TYPE 1', 
       'list:string': 'BLOB SUB_TYPE 1', 'list:reference': 'BLOB SUB_TYPE 1', 'big-id': 'BIGINT PRIMARY KEY', 'big-reference': 'BIGINT REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s'}
    ...
    def CONTAINS(self, first, second, case_sensitive=False):
        if first.type.startswith('list:'):
            second = Expression(None, self.CONCAT('|', Expression(None, self.REPLACE(second, ('|',
                                                                                              '||'))), '|'))
        return '(%s CONTAINING %s)' % (self.expand(first), self.expand(second, 'string'))
    ...
    def _truncate(self, table, mode=''):
        return ['DELETE FROM %s;' % table._tablename, 'SET GENERATOR %s TO 0;' % table._sequence_name]
    ...
    def __init__(self, db, uri, pool_size=0, folder=None, db_codec='UTF-8', credential_decoder=IDENTITY, driver_args={}, adapter_args={}, do_connect=True, after_connection=None):
        ...
        if not db:
            raise SyntaxError('Database name required')
        charset = m.group('charset') or 'UTF8'
        driver_args.update(dsn='%s/%s:%s' % (host, port, db), user=credential_decoder(user), password=credential_decoder(password), charset=charset)
...
class FireBirdEmbeddedAdapter(FireBirdAdapter):
    ...
    def __init__(self, db, uri, pool_size=0, folder=None, db_codec='UTF-8', credential_decoder=IDENTITY, driver_args={}, adapter_args={}, do_connect=True, after_connection=None):
        ...
        if not charset:
            charset = 'UTF8'
        host = ''
        driver_args.update(host=host, database=pathdb, user=credential_decoder(user), password=credential_decoder(password), charset=charset)
...
class InformixAdapter(BaseAdapter):
    ...
    types = {'boolean': 'CHAR(1)', 'string': 'VARCHAR(%(length)s)', 'text': 'BLOB SUB_TYPE 1', 'json': 'BLOB SUB_TYPE 1', 'password': 'VARCHAR(%(length)s)', 
       'blob': 'BLOB SUB_TYPE 0', 'upload': 'VARCHAR(%(length)s)', 'integer': 'INTEGER', 'bigint': 'BIGINT', 'float': 'FLOAT', 
       'double': 'DOUBLE PRECISION', 'decimal': 'NUMERIC(%(precision)s,%(scale)s)', 'date': 'DATE', 'time': 'CHAR(8)', 'datetime': 'DATETIME', 
       'id': 'SERIAL', 'reference': 'INTEGER REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s', 'list:integer': 'BLOB SUB_TYPE 1', 
       'list:string': 'BLOB SUB_TYPE 1', 'list:reference': 'BLOB SUB_TYPE 1', 'big-id': 'BIGSERIAL', 'big-reference': 'BIGINT REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s', 
...
class DB2Adapter(BaseAdapter):
    drivers = ('pyodbc', )
    types = {'boolean': 'CHAR(1)', 'string': 'VARCHAR(%(length)s)', 'text': 'CLOB', 'json': 'CLOB', 'password': 'VARCHAR(%(length)s)', 'blob': 'BLOB', 'upload': 'VARCHAR(%(length)s)', 
       'integer': 'INT', 'bigint': 'BIGINT', 'float': 'REAL', 'double': 'DOUBLE', 'decimal': 'NUMERIC(%(precision)s,%(scale)s)', 
       'date': 'DATE', 'time': 'TIME', 'datetime': 'TIMESTAMP', 'id': 'INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY NOT NULL', 
       'reference': 'INT, FOREIGN KEY (%(field_name)s) REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s', 
       'list:integer': 'CLOB', 'list:string': 'CLOB', 
       'list:reference': 'CLOB', 'big-id': 'BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY NOT NULL', 'big-reference': 'BIGINT, FOREIGN KEY (%(field_name)s) REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s', 
       'reference FK': ', CONSTRAINT FK_%(constraint_name)s FOREIGN KEY (%(field_name)s) REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s', 
       'reference TFK': ' CONSTRAINT FK_%(foreign_table)s_PK FOREIGN KEY (%(field_name)s) REFERENCES %(foreign_table)s (%(foreign_key)s) ON DELETE %(on_delete_action)s'}
...
class TeradataAdapter(BaseAdapter):
    drivers = ('pyodbc', )
    types = {'boolean': 'CHAR(1)', 'string': 'VARCHAR(%(length)s)', 'text': 'VARCHAR(2000)', 'json': 'VARCHAR(4000)', 'password': 'VARCHAR(%(length)s)', 
       'blob': 'BLOB', 'upload': 'VARCHAR(%(length)s)', 'integer': 'INT', 'bigint': 'BIGINT', 'float': 'REAL', 
       'double': 'DOUBLE', 'decimal': 'NUMERIC(%(precision)s,%(scale)s)', 'date': 'DATE', 'time': 'TIME', 'datetime': 'TIMESTAMP', 
       'id': 'INT GENERATED ALWAYS AS IDENTITY', 
       'reference': 'INT', 
       'list:integer': 'VARCHAR(4000)', 'list:string': 'VARCHAR(4000)', 'list:reference': 'VARCHAR(4000)', 'big-id': 'BIGINT GENERATED ALWAYS AS IDENTITY', 
       'big-reference': 'BIGINT', 
       'reference FK': ' REFERENCES %(foreign_key)s', 'reference TFK': ' FOREIGN KEY (%(field_name)s) REFERENCES %(foreign_table)s (%(foreign_key)s)'}
...
class IngresAdapter(BaseAdapter):
    ...
    def create_sequence_and_triggers(self, query, table, **args):
        if hasattr(table, '_primarykey'):
            modify_tbl_sql = 'modify %s to btree unique on %s' % (table._tablename, (', ').join([ "'%s'" % x for x in table.primarykey ]))
...
class IngresUnicodeAdapter(IngresAdapter):
    drivers = ('pyodbc', )
    types = {'boolean': 'CHAR(1)', 
       'string': 'NVARCHAR(%(length)s)', 'text': 'NCLOB', 'json': 'NCLOB', 'password': 'NVARCHAR(%(length)s)', 'blob': 'BLOB', 
       'upload': 'VARCHAR(%(length)s)', 'integer': 'INTEGER4', 
       'bigint': 'BIGINT', 
       'float': 'FLOAT', 'double': 'FLOAT8', 'decimal': 'NUMERIC(%(precision)s,%(scale)s)', 'date': 'ANSIDATE', 'time': 'TIME WITHOUT TIME ZONE', 
       'datetime': 'TIMESTAMP WITHOUT TIME ZONE', 'id': 'INTEGER4 not null unique with default next value for %s' % INGRES_SEQNAME, 
       'reference': 'INTEGER4, FOREIGN KEY (%(field_name)s) REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s', 
       'list:integer': 'NCLOB', 'list:string': 'NCLOB', 
       'list:reference': 'NCLOB', 'big-id': 'BIGINT not null unique with default next value for %s' % INGRES_SEQNAME, 'big-reference': 'BIGINT, FOREIGN KEY (%(field_name)s) REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s', 
       'reference FK': ', CONSTRAINT FK_%(constraint_name)s FOREIGN KEY (%(field_name)s) REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s', 
       'reference TFK': ' CONSTRAINT FK_%(foreign_table)s_PK FOREIGN KEY (%(field_name)s) REFERENCES %(foreign_table)s (%(foreign_key)s) ON DELETE %(on_delete_action)s'}
...
class SAPDBAdapter(BaseAdapter):
    drivers = ('sapdb', )
    support_distributed_transaction = False
    types = {'boolean': 'CHAR(1)', 'string': 'VARCHAR(%(length)s)', 'text': 'LONG', 'json': 'LONG', 'password': 'VARCHAR(%(length)s)', 'blob': 'LONG', 'upload': 'VARCHAR(%(length)s)', 
       'integer': 'INT', 'bigint': 'BIGINT', 'float': 'FLOAT', 'double': 'DOUBLE PRECISION', 'decimal': 'FIXED(%(precision)s,%(scale)s)', 
       'date': 'DATE', 'time': 'TIME', 'datetime': 'TIMESTAMP', 'id': 'INT PRIMARY KEY', 'reference': 'INT, FOREIGN KEY (%(field_name)s) REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s', 
       'list:integer': 'LONG', 'list:string': 'LONG', 
       'list:reference': 'LONG', 'big-id': 'BIGINT PRIMARY KEY', 'big-reference': 'BIGINT, FOREIGN KEY (%(field_name)s) REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s'}
    ...
    def select_limitby(self, sql_s, sql_f, sql_t, sql_w, sql_o, limitby):
        if limitby:
            ...
            if len(sql_w) > 1:
                ...
                sql_w_row = 'WHERE w_row > %i' % lmin
            return '%s %s FROM (SELECT w_tmp.*, ROWNO w_row FROM (SELECT %s FROM %s%s%s) w_tmp WHERE ROWNO=%i) %s %s %s;' % (
             sql_s, sql_f, sql_f, sql_t, sql_w, sql_o, lmax, sql_t, sql_w_row, sql_o)
        return 'SELECT %s %s FROM %s%s%s;' % (sql_s, sql_f, sql_t, sql_w, sql_o)
    ...
    def __init__(self, db, uri, pool_size=0, folder=None, db_codec='UTF-8', credential_decoder=IDENTITY, driver_args={}, adapter_args={}, do_connect=True, after_connection=None):
        ...
        def connector(user=user, password=password, database=db, host=host, driver_args=driver_args):
            return self.driver.Connection(user, password, database, host, **driver_args)
...
class CubridAdapter(MySQLAdapter):
    ...
    def __init__(self, db, uri, pool_size=0, folder=None, db_codec='UTF-8', credential_decoder=IDENTITY, driver_args={}, adapter_args={}, do_connect=True, after_connection=None):
        ...
        def connector(host=host, port=port, db=db, user=user, passwd=password, driver_args=driver_args):
...
class DatabaseStoredFile():
    ...
    def close_connection(self):
        if self.db is not None:
            ...
            query = "INSERT INTO web2py_filesystem(path,content) VALUES ('%s','%s')" % (self.filename, self.data.replace("'", "''"))
    ...
    def exists(db, filename):
        ...
        try:
            ...
            if not (db._adapter.isOperationalError(e) or db._adapter.isProgrammingError(e)):
                raise
...
class GoogleSQLAdapter(UseDatabaseStoredFile, MySQLAdapter):
    ...
    def __init__(self, db, uri='google:sql://realm:domain/database', pool_size=0, folder=None, db_codec='UTF-8', credential_decoder=IDENTITY, driver_args={}, adapter_args={}, do_connect=True, after_connection=None):
        self.db = db
        self.dbengine = 'mysql'
        self.uri = uri
        self.pool_size = pool_size
...
class NoSQLAdapter(BaseAdapter):
        ...
        if isinstance(fieldtype, SQLCustomType):
            ...
            if is_list:
                ...
                if not isinstance(obj, (list, tuple)):
                    obj = [
                     obj]
            if obj == '' and not (is_string and fieldtype[:2] in ('st', 'te', 'pa',
                                                                  'up')):
                return
            if obj is not None:
                if isinstance(obj, list) and not is_list:
                    ...
                    if obj and str(obj)[0].upper() not in '0F':
                        obj = True
                    else:
                        obj = False
                elif fieldtype == 'date':
                    ...
                    if is_string and field_is_type('list:string'):
    ...
    def STARTSWITH(self, first, second=None):
        raise SyntaxError('Not supported')

    def ENDSWITH(self, first, second=None):
        raise SyntaxError('Not supported')
...
class GoogleDatastoreAdapter(NoSQLAdapter):
    ...
    def __init__(self, db, uri, pool_size=0, folder=None, db_codec='UTF-8', credential_decoder=IDENTITY, driver_args={}, adapter_args={}, do_connect=True, after_connection=None):
        ...
        if self.use_ndb is True:
            self.types.update({'boolean': ndb.BooleanProperty, 'string': lambda **kwargs: ndb.StringProperty(**kwargs), 'text': ndb.TextProperty, 'json': ndb.TextProperty, 
               'password': ndb.StringProperty, 'blob': ndb.BlobProperty, 'upload': ndb.StringProperty, 'integer': ndb.IntegerProperty, 
               'bigint': ndb.IntegerProperty, 'float': ndb.FloatProperty, 'double': ndb.FloatProperty, 'decimal': NDBDecimalProperty, 
               'date': ndb.DateProperty, 'time': ndb.TimeProperty, 'datetime': ndb.DateTimeProperty, 'id': None, 
               'reference': ndb.IntegerProperty, 'list:string': lambda **kwargs: ndb.StringProperty(repeated=True, default=None, **kwargs), 
            ...
            self.types.update({'boolean': gae.BooleanProperty, 'string': lambda **kwargs: gae.StringProperty(multiline=True, **kwargs), 
               'text': gae.TextProperty, 'json': gae.TextProperty, 
               'password': gae.StringProperty, 'blob': gae.BlobProperty, 'upload': gae.StringProperty, 'integer': gae.IntegerProperty, 
               'bigint': gae.IntegerProperty, 'float': gae.FloatProperty, 'double': gae.FloatProperty, 'decimal': GAEDecimalProperty, 
               'date': gae.DateProperty, 'time': gae.TimeProperty, 'datetime': gae.DateTimeProperty, 'id': None, 'reference': gae.IntegerProperty, 
               'list:string': lambda **kwargs: gae.StringListProperty(default=None, **kwargs), 'list:integer': lambda **kwargs: gae.ListProperty(int, default=None, **kwargs), 
        ...
        if isinstance(expression, Field):
            ...
            if isinstance(expression, (Expression, Query)):
                if expression.second is not None:
                    ...
                    if expression.first is not None:
    ...
    def EQ(self, first, second=None):
        ...
        return [
         GAEF(first.name, '=', self.represent(second, first.type), lambda a, b: a == b)]
    ...
    def NE(self, first, second=None):
        if first.type != 'id':
            ...
            return [
             GAEF(first.name, '!=', second, lambda a, b: a != b)]
            return
    ...
    def BELONGS(self, first, second=None):
        ...
        return [
         GAEF(first.name, 'in', second, lambda a, b: a in b)]

    def CONTAINS(self, first, second, case_sensitive=False):
        if not first.type.startswith('list:'):
            raise SyntaxError('Not supported')
        return [
         GAEF(first.name, '=', self.expand(second, first.type[5:]), lambda a, b: b in a)]

    def NOT(self, first):
        nops = {self.EQ: self.NE, self.NE: self.EQ, self.LT: self.GE, self.GT: self.LE, self.LE: self.GT, self.GE: self.LT}
    ...
    GAE_FILTER_OPTIONS = {'=': lambda q, t, p, v: q.filter(getattr(t, p) == v), 
       '>': lambda q, t, p, v: q.filter(getattr(t, p) > v), '<': lambda q, t, p, v: q.filter(getattr(t, p) < v), 
       '<=': lambda q, t, p, v: q.filter(getattr(t, p) <= v), '>=': lambda q, t, p, v: q.filter(getattr(t, p) >= v), 
       '!=': lambda q, t, p, v: q.filter(getattr(t, p) != v), 'in': lambda q, t, p, v: q.filter(getattr(t, p).IN(v))}
    ...
    def select_raw(self, query, fields=None, attributes=None):
        ...
        if query:
            ...
            if len(db[tablename].fields) == len(fields):
                ...
                for f in fields:
                    if f.type in ('text', 'blob', 'json'):
                        raise SyntaxError('text and blob field types not allowed in projection queries')
                    else:
            ...
            query_projection = [ p for p in projection if p != db[tablename]._id.name ] if projection and args_get('projection') == True else None
            cursor = None
            ...
            for filter in filters:
                if args_get('projection') == True and filter.name in query_projection and filter.op in ('=',
                                                                                                        '<=',
                                                                                                        '>='):
                ...
                if filter.name == '__key__' and filter.op == '>' and filter.value == 0:
                    ...
                    items = [ i for i in items if filter.apply(getattr(item, filter.name), filter.value) ]
                else:
                    if filter.name == '__key__' and filter.op != 'in':
                        if self.use_ndb:
                            ...
                            items.order('__key__')
                    items = self.filter(items, tableobj, filter.name, filter.op, filter.value) if self.use_ndb else items.filter('%s %s' % (filter.name, filter.op), filter.value)
        ...
        if not isinstance(items, list):
            ...
            if args_get('limitby', None):
                ...
                items = rows
        return (
         items, tablename, projection or db[tablename].fields)
    ...
    def select(self, query, fields, attributes):
        ...
        rows = [ [ t == self.db[tablename]._id.name and item or t == 'nativeRef' and item or getattr(item, t) for t in fields ] for item in items ]
...
class CouchDBAdapter(NoSQLAdapter):
    drivers = ('couchdb', )
    uploads_in_blob = True
    types = {'boolean': bool, 'string': str, 'text': str, 'json': str, 'password': str, 'blob': str, 'upload': str, 'integer': long, 'bigint': long, 'float': float, 
       'double': float, 'date': datetime.date, 'time': datetime.time, 'datetime': datetime.datetime, 'id': long, 'reference': long, 
       'list:string': list, 'list:integer': list, 'list:reference': list}
    ...
    def represent(self, obj, fieldtype):
        ...
        if fieldtype in ('date', 'time', 'datetime', 'boolean'):
            return serializers.json(value)
        return repr(not isinstance(value, unicode) and value or value and value.encode('utf8'))

    def __init__(self, db, uri='couchdb://127.0.0.1:5984', pool_size=0, folder=None, db_codec='UTF-8', credential_decoder=IDENTITY, driver_args={}, adapter_args={}, do_connect=True, after_connection=None):
        self.db = db
        self.uri = uri
        if do_connect:
            self.find_driver(adapter_args)
    ...
    def _select(self, query, fields, attributes):
        ...
        fn = '(function(%(t)s){if(%(query)s)emit(%(order)s,[%(fields)s]);})' % dict(t=tablename, query=self.expand(query), order='%s._id' % tablename, fields=fields)
        return (fn, colnames)
...
class IMAPAdapter(NoSQLAdapter):
    drivers = ('imaplib', )
    types = {'string': str, 'text': str, 'date': datetime.date, 'datetime': datetime.datetime, 'id': long, 'boolean': bool, 'integer': int, 'bigint': long, 
       'blob': str, 'list:string': str}
    dbengine = 'imap'
    ...
    def __init__(self, db, uri, pool_size=0, folder=None, db_codec='UTF-8', credential_decoder=IDENTITY, driver_args={}, adapter_args={}, do_connect=True, after_connection=None):
        ...
        self.imap4 = None
        uri = uri.split('://')[1]
        self.flags = {'deleted': '\\Deleted', 'draft': '\\Draft', 'flagged': '\\Flagged', 'recent': '\\Recent', 'seen': '\\Seen', 'answered': '\\Answered'}
        self.search_fields = {'id': 'MESSAGE', 'created': 'DATE', 'uid': 'UID', 'sender': 'FROM', 'to': 'TO', 'cc': 'CC', 'bcc': 'BCC', 'content': 'TEXT', 
           'size': 'SIZE', 'deleted': '\\Deleted', 'draft': '\\Draft', 'flagged': '\\Flagged', 'recent': '\\Recent', 
           'seen': '\\Seen', 'subject': 'SUBJECT', 'answered': '\\Answered', 'mime': None, 'email': None, 'attachments': None}
        db['_lastsql'] = ''
        m = self.REGEX_URI.match(uri)
        user = m.group('user')
    ...
    def convert_date(self, date, add=None, imf=False):
        ...
        if isinstance(date, basestring):
            try:
                ...
                return datetime.datetime(year, month, day, hms[0], hms[1], hms[2]) + add
            except (ValueError, AttributeError, IndexError) as e:
                LOGGER.error('Could not parse date text: %s. %s' % (date, e))
                return
        ...
        if isinstance(query, Query):
            ...
            if mailbox is None:
                ...
                if search_result[0] == 'OK':
                    ...
                    if any([ field.name in ('content', 'size', 'attachments', 'email') for field in fields ]):
                    ...
                    if len(messages_set) > 0:
                        for uid in messages_set:
                            ...
                            if typ == 'OK':
                                fr = {'message': int(data[0][0].split()[0]), 'uid': long(uid), 'email': email.message_from_string(data[0][1]), 'raw_message': data[0][1]}
            ...
            if isinstance(query, (Expression, basestring)):
                raise NotImplementedError()
            else:
            ...
            if len(fields) == 1 and isinstance(fields[0], SQLALL):
                ...
                for fr in fetch_results:
                    ...
                    for part in message.walk():
                        ...
                        if '%s.attachments' % tablename in colnames or '%s.content' % tablename in colnames:
                            payload = part.get_payload(decode=True)
                            if payload:
                                filename = part.get_filename()
                                values = {'mime': part.get_content_type()}
                                if (filename or 'text' not in maintype) and '%s.attachments' % tablename in colnames:
                                    values.update({'payload': payload, 'filename': filename, 'encoding': part.get_content_charset(), 'disposition': part['Content-Disposition']})
                                    attachments.append(values)
                                elif 'text' in maintype and '%s.content' % tablename in colnames:
                                    values.update({'text': self.encode_text(payload, self.get_charset(part))})
    ...
    def insert(self, table, fields):
        ...
        if len(d) > 0:
            ...
            flags = (' ').join([ '\\%s' % flag.capitalize() for flag in ('answered',
                                                                         'deleted',
                                                                         'draft',
                                                                         'flagged',
                                                                         'recent',
                                                                         'seen') if d.get(flag, False) ])
            if not message:
                ...
                if not message.is_multipart() and not message.get_content_type().startswith('multipart'):
...
ADAPTERS = {'sqlite': SQLiteAdapter, 'spatialite': SpatiaLiteAdapter, 'sqlite:memory': SQLiteAdapter, 'spatialite:memory': SpatiaLiteAdapter, 'mysql': MySQLAdapter, 
   'postgres': PostgreSQLAdapter, 'postgres:psycopg2': PostgreSQLAdapter, 'postgres:pg8000': PostgreSQLAdapter, 'postgres2:psycopg2': NewPostgreSQLAdapter, 
   'postgres2:pg8000': NewPostgreSQLAdapter, 'oracle': OracleAdapter, 'mssql': MSSQLAdapter, 'mssql2': MSSQL2Adapter, 
   'mssql3': MSSQL3Adapter, 'mssql4': MSSQL4Adapter, 'vertica': VerticaAdapter, 'sybase': SybaseAdapter, 'db2': DB2Adapter, 
   'teradata': TeradataAdapter, 'informix': InformixAdapter, 'informix-se': InformixSEAdapter, 'firebird': FireBirdAdapter, 
   'firebird_embedded': FireBirdAdapter, 'ingres': IngresAdapter, 'ingresu': IngresUnicodeAdapter, 'sapdb': SAPDBAdapter, 
   'cubrid': CubridAdapter, 'jdbc:sqlite': JDBCSQLiteAdapter, 'jdbc:sqlite:memory': JDBCSQLiteAdapter, 'jdbc:postgres': JDBCPostgreSQLAdapter, 
   'gae': GoogleDatastoreAdapter, 'google:datastore': GoogleDatastoreAdapter, 
   'google:sql': GoogleSQLAdapter, 'couchdb': CouchDBAdapter, 'imap': IMAPAdapter}
...
def sqlhtml_validators(field):
    ...
    if field_type in ('string', 'text', 'password'):
        ...
        requires.append(validators.IS_FLOAT_IN_RANGE(-1e+100, 1e+100))
    elif field_type == 'integer':
        requires.append(validators.IS_INT_IN_RANGE(-2147483648, 2147483648))
    elif field_type == 'bigint':
        requires.append(validators.IS_INT_IN_RANGE(-9223372036854775808, 9223372036854775808))
    elif field_type.startswith('decimal'):
        requires.append(validators.IS_DECIMAL_IN_RANGE(-10000000000, 10000000000))
        ...
        if db and field_type.startswith('reference') and field_type.find('.') < 0 and field_type[10:] in db.tables:
            ...
            if db and field_type.startswith('list:reference') and field_type.find('.') < 0 and field_type[15:] in db.tables:
                ...
                if hasattr(referenced, '_format') and referenced._format:
                    requires = validators.IS_IN_DB(db, referenced._id, referenced._format, multiple=True)
                else:
                    requires = validators.IS_IN_DB(db, referenced._id, multiple=True)
                if field.unique:
                    requires._and = validators.IS_NOT_IN_DB(db, field)
                requires = field.notnull or validators.IS_EMPTY_OR(requires)
            return requires
    ...
    sff = [
     'in', 'do', 'da', 'ti', 'de', 'bo']
    if field.notnull and field_type[:2] not in sff:
...
class Row(object):
    ...
    def as_xml(self, row_name='row', colnames=None, indent='  '):
        ...
        def f(row, field, indent='  '):
            if isinstance(row, Row):
                ...
                return '%s<%s>\n%s\n%s</%s>' % (
                 indent, field, spc.join(item for item in items if item), indent, field)
            else:
                if not callable(row):
    ...
    def as_json(self, mode='object', default=None, colnames=None, serialize=True, **kwargs):
        """
        ...
        if serialize:
            if have_serializers:
                return serializers.json(item, default=default or serializers.custom_json)
            if simplejson:
...
def smart_query(fields, text):
    ...
    for a, b in [('&', 'and'), ('|', 'or'), ('~', 'not'), ('==', '='), ('<', '<'), ('>', '>'), ('<=', '<='), ('>=', '>='), ('<>', '!='), ('=<', '<='),
     ('=>', '>='), ('=', '='), (' less or equal than ', '<='), (' greater or equal than ', '>='), (' equal or less than ', '<='),
     (' equal or greater than ', '>='), (' less or equal ', '<='), (' greater or equal ', '>='), (' equal or less ', '<='),
     (' equal or greater ', '>='), (' not equal to ', '!='), (' not equal ', '!='), (' equal to ', '='), (' equal ', '='),
     (' equals ', '='), (' less than ', '<'), (' greater than ', '>'), (' starts with ', 'startswith'), (' ends with ', 'endswith'),
     (' not in ', 'notbelongs'), (' in ', 'belongs'), (' is ', '=')]:
        if a[0] == ' ':
    ...
    for item in text.split():
        if field is None:
            ...
            elif field._db._adapter.dbengine == 'google:datastore' and field.type in ('list:integer',
                                                                                      'list:string',
                                                                                      'list:reference'):
                if op == 'contains':
                    new_query = field.contains(value)
...
class DAL(object):
    ...
    def get_instances():
        ...
        for db_uid, db_group in dbs:
            for db in db_group:
                ...
                infos[k] = dict(dbstats=[ (row[0], row[1]) for row in db._timings ], dbtables={'defined': sorted(list(set(db.tables) - set(db._LAZY_TABLES.keys()))), 'lazy': sorted(db._LAZY_TABLES.keys())})
    ...
    def distributed_transaction_begin(*instances):
        ...
        for i, db in instances:
            if not db._adapter.support_distributed_transaction():
                raise SyntaxError('distributed transaction not suported by %s' % db._dbname)
    ...
    def distributed_transaction_commit(*instances):
        ...
        for i, db in instances:
            if not db._adapter.support_distributed_transaction():
                raise SyntaxError('distributed transaction not suported by %s' % db._dbanme)
        ...
        * '<adaptername>_nonreserved' Checks against the specific adapters
        ...
        if uri == '<zombie>' and db_uid is not None:
            ...
            if uri:
                ...
                for k in range(attempts):
                    for uri in uris:
                        try:
                            ...
                            kwargs = dict(db=self, uri=uri, pool_size=pool_size, folder=folder, db_codec=db_codec, credential_decoder=credential_decoder, driver_args=driver_args or {}, adapter_args=adapter_args or {}, do_connect=do_connect, after_connection=after_connection, entity_quoting=entity_quoting)
                ...
                self._adapter = BaseAdapter(db=self, pool_size=0, uri='None', folder=folder, db_codec=db_codec, after_connection=after_connection, entity_quoting=entity_quoting)
                migrate = fake_migrate = False
            adapter = self._adapter
            self._uri_hash = hashlib_md5(adapter.uri).hexdigest()
            self.check_reserved = check_reserved
            if self.check_reserved:
                raise NotImplemented('Cant check reserved keywords')
    ...
    def import_table_definitions(self, path, migrate=False, fake_migrate=False, tables=None):
        if tables:
            for table in tables:
            ...
            for filename in glob.glob(pattern):
                ...
                try:
                    ...
                    mf = [ (value['sortable'], Field(key, type=value['type'], length=value.get('length', None), notnull=value.get('notnull', False), unique=value.get('unique', False))) for key, value in sql_fields.iteritems()
                         ]
                    mf.sort(lambda a, b: cmp(a[0], b[0]))
                    self.define_table(name, *[ item[1] for item in mf ], **dict(migrate=migrate, fake_migrate=fake_migrate))
                finally:
    ...
    def check_reserved_keyword(self, name):
        ...
        for backend in self.check_reserved:
            if name.upper() in self.RSK[backend]:
                raise SyntaxError('invalid table/column name "%s" is a "%s" reserved SQL/NOSQL keyword' % (name, backend.upper()))
...
def index():
    ...
    return locals()
        ...
        if ('/').join(args) == 'patterns':
            return Row({'status': 200, 'pattern': 'list', 'error': None, 'response': patterns})
        else:
            for pattern in patterns:
                ...
                for tag in tags:
                    if re1.match(tag):
                        ...
                        if re2.match(tag) and args[i] == tag[:tag.find('[')]:
                            ...
                            if '.' in ref and otable:
                                ...
                                if nested_select:
                                    try:
                                        ...
                                        return Row({'status': 400, 'pattern': pattern, 'error': 'invalid path', 'response': None})
                            ...
                            if not db[table][field].readable:
                                return Row({'status': 418, 'pattern': pattern, 'error': "I'm a teapot", 'response': None})
                            try:
                            ...
                            if items:
                                return Row({'status': 200, 'response': items, 'pattern': pattern})
                            return Row({'status': 404, 'pattern': pattern, 'error': 'no record found', 'response': None})
                        elif tag != args[i]:
                            break
                        otable = table
                        i += 1
                        if i == len(tags) and table:
                            ...
                            try:
                                orderby = [ (f.startswith('~') or db[table])[f] if 1 else ~db[table][f[1:]] for f in ofields ]
                            except (KeyError, AttributeError):
                            ...
                            return Row({'status': 200, 'response': response, 'pattern': pattern, 'count': count})
    ...
    def define_table(self, tablename, *fields, **args):
        ...
        if not isinstance(tablename, str):
        ...
        elif tablename.startswith('_') or hasattr(self, tablename) or REGEX_PYTHON_KEYWORDS.match(tablename):
            ...
            if invalid_args:
                raise SyntaxError('invalid table "%s" attributes: %s' % (tablename, invalid_args))
        if self._lazy_tables and tablename not in self._LAZY_TABLES:
            self._LAZY_TABLES[tablename] = (
    ...
    def lazy_define_table(self, tablename, *fields, **args):
        ...
        if migrate and self._uri not in (None, 'None') or self._adapter.dbengine == 'google:datastore':
            fake_migrate = self._fake_migrate_all or args_get('fake_migrate', self._fake_migrate)
            polymodel = args_get('polymodel', None)
            try:
                GLOBAL_LOCKER.acquire()
                self._lastsql = self._adapter.create_table(table, migrate=migrate, fake_migrate=fake_migrate, polymodel=polymodel)
            finally:
                GLOBAL_LOCKER.release()
    ...
    def as_dict(self, flat=False, sanitize=True):
        ...
        db_as_dict = dict(tables=[], uri=uri, db_uid=db_uid, **dict([ (k, getattr(self, '_' + k, None)) for k in ('pool_size',
                                                                                                                  'folder',
                                                                                                                  'db_codec',
                                                                                                                  'check_reserved',
                                                                                                                  'migrate',
    ...
    def __getattr__(self, key):
        if ogetattr(self, '_lazy_tables') and key in ogetattr(self, '_LAZY_TABLES'):
    ...
    def executesql(self, query, placeholders=None, as_dict=False, fields=None, colnames=None, as_ordered_dict=False):
        """
        ...
        if as_dict or as_ordered_dict:
            ...
            if fields or colnames:
                ...
                if not colnames:
                    colnames = [ '%s.%s' % (f.tablename, f.name) for f in extracted_fields ]
                data = adapter.parse(data, fields=extracted_fields, colnames=colnames)
            return data

    def _remove_references_to(self, thistable):
        for table in self:
            table._referenced_by = [ field for field in table._referenced_by if not field.table == thistable ]
    ...
    def export_to_csv_file(self, ofile, *args, **kwargs):
        ...
        for table in self.tables:
            ...
            for k in range(0, nrows, step):
                self(query).select(limitby=(k, k + step)).export_to_csv_file(ofile, *args, **kwargs)
    ...
    def import_from_csv_file(self, ifile, id_map=None, null='<NULL>', unique='uuid', map_tablenames=None, ignore_missing_tables=False, *args, **kwargs):
        id_offset = {}
        ...
        for line in ifile:
            ...
            if not line:
                ...
                if line == 'END':
                    return
                if not line.startswith('TABLE ') or line[6:] not in self.tables:
                    raise SyntaxError('invalid file format')
                else:
                    tablename = line[6:]
                    tablename = map_tablenames.get(tablename, tablename)
                    if tablename is not None and tablename in self.tables:
                        self[tablename].import_from_csv_file(ifile, id_map, null, unique, id_offset, *args, **kwargs)
...
class Reference(long):
    ...
    def __allocate(self):
        ...
        if not self._record:
            raise RuntimeError('Using a recursive select but encountered a broken reference: %s %d' % (self._table, long(self)))
...
def Reference_pickler(data):
    ...
    return (
     Reference_unpickler, (marshal_dump,))
...
class Table(object):
    ...
    def __init__(self, db, tablename, *fields, **args):
        ...
        if not isinstance(tablename, str) or tablename[0] == '_' or hasattr(DAL, tablename) or '.' in tablename or REGEX_PYTHON_KEYWORDS.match(tablename):
            raise SyntaxError('Field: invalid table name: %s, use rname for "funny" names' % tablename)
        self._ot = None
        ...
        self._trigger_name = args.get('trigger_name') or db and db._adapter.trigger_name(tablename)
        ...
        self._plural = args.get('plural', pluralize(self._singular.lower()).capitalize())
        ...
        if _primarykey is not None:
            ...
            if len(_primarykey) == 1:
                self._id = [ f for f in fields if isinstance(f, Field) and f.name == _primarykey[0] ][0]
        else:
            if not [ f for f in fields if isinstance(f, Field) and f.type == 'id' or isinstance(f, dict) and f.get('type', None) == 'id' ]:
                field = Field('id', 'id')
            ...
            for field in fields:
                if isinstance(field, (FieldMethod, FieldVirtual)):
                    ...
                    table = field
                    for field in table:
                        if field.name not in fieldnames and not field.type == 'id':
                    ...
                    raise SyntaxError('define_table argument is not a Field or Table: %s' % field)
            ...
            if db and db._adapter.uploads_in_blob is True:
                ...
                for field in fields:
                    ...
                    if isinstance(field, Field) and field.type == 'upload' and fn is True:
                        fn = field.uploadfield = '%s_blob' % field.name
                    if isinstance(fn, str) and fn not in uploadfields:
                        fields.append(Field(fn, 'blob', default='', writable=False, readable=False))
    ...
    def _enable_record_versioning(self, archive_db=None, archive_name='%(tablename)s_archive', is_active='is_active', current_record='current_record', current_record_label=None):
        ...
        for field in self:
            ...
            clones.append(field.clone(unique=False, type=field.type if nfk else 'bigint'))

        archive_db.define_table(archive_name, Field(current_record, field_type, label=current_record_label), *clones, **dict(format=self._format))
        self._before_update.append(lambda qset, fs, db=archive_db, an=archive_name, cn=current_record: archive_record(qset, fs, db[an], cn))
        if is_active and is_active in fieldnames:
            self._before_delete.append(lambda qset: qset.update(is_active=False))
            newquery = lambda query, t=self, name=self._tablename: reduce(AND, [ db[tn].is_active == True for tn in db._adapter.tables(query) if tn == name or getattr(db[tn], '_ot', None) == name ])
            ...
            if isinstance(field_type, str) and field_type[:10] == 'reference ':
                ...
                if rfieldname:
                    if not hasattr(rtable, '_primarykey'):
                        raise SyntaxError('keyed tables can only reference other keyed tables (for now)')
                    if rfieldname not in rtable.fields:
                        raise SyntaxError("invalid field '%s' for referenced table '%s' in table '%s'" % (
    ...
    def _filter_fields(self, record, id=False):
        return dict([ (k, v) for k, v in record.iteritems() if k in self.fields and (self[k].type != 'id' or id) ])
    ...
    def __getitem__(self, key):
        if not key:
            ...
            if str(key).isdigit() or 'google' in DRIVERS and isinstance(key, Key):
    ...
    def __call__(self, key=DEFAULT, **kwargs):
        ...
        if key is not DEFAULT:
            if isinstance(key, Query):
                record = self._db(key).select(limitby=(0, 1), for_update=for_update, orderby=orderby, orderby_on_limitby=False).first()
            elif not str(key).isdigit():
                record = None
            else:
                record = self._db(self._id == key).select(limitby=(0, 1), for_update=for_update, orderby=orderby, orderby_on_limitby=False).first()
            if record:
    ...
    def __setitem__(self, key, value):
        if isinstance(key, dict) and isinstance(value, dict):
            ...
            elif self._db(self._id == key).update(**self._filter_fields(value)) is None:
    ...
    def import_from_csv_file(self, csvfile, id_map=None, null='<NULL>', unique='uuid', id_offset=None, *args, **kwargs):
        """
        ...
        def fix(field, value, id_map, id_offset):
            ...
            if value == null:
                ...
                if id_map is not None:
                    value = [ id_map[ref_table][long(v)] for v in bar_decode_string(value) ]
                else:
            ...
            if not colnames:
                ...
                for i, field in cols:
                    try:
                        ...
                        raise RuntimeError("Unable to parse line:%s field:%s value:'%s'" % (lineno + 1, field, line[i]))
                ...
                if not (id_map or cid is None or id_offset is None or unique_idx):
                    ...
                    if first:
                        first = False
                        id_offset[self._tablename] = curr_id - csv_id if curr_id > csv_id else 0
    ...
    def as_dict(self, flat=False, sanitize=True):
        table_as_dict = dict(tablename=str(self), fields=[], sequence_name=self._sequence_name, trigger_name=self._trigger_name, common_filter=self._common_filter, format=self._format, singular=self._singular, plural=self._plural)
        for field in self:
            if field.readable or field.writable or not sanitize:
                table_as_dict['fields'].append(field.as_dict(flat=flat, sanitize=sanitize))
...
class Expression(object):
    ...
    def __getslice__(self, start, stop):
        ...
        return Expression(db, db._adapter.SUBSTRING, self, (pos0, length), self.type)
    ...
    def oneof(self, values):
        db = self.db
        escaped_values = [ str(v) if isinstance(v, (int, long)) else '"%s"' % MySQLdb.escape_string(str(v)) for v in values ]
        return Query(db, db._adapter.IN, self, escaped_values)
    ...
    def contains(self, value, all=False, case_sensitive=False):
        ...
        if isinstance(value, (list, tuple)):
            subqueries = [ self.contains(str(v).strip(), case_sensitive=case_sensitive) for v in value if str(v).strip() ]
            if not subqueries:
    ...
    def st_asgeojson(self, precision=15, options=0, version=1):
        return Expression(self.db, self.db._adapter.ST_ASGEOJSON, self, dict(precision=precision, options=options, version=version), 'string')
...
class SQLCustomType(object):
    ...
    def __init__(self, type='string', native=None, encoder=None, decoder=None, validator=None, _class=None):
        self.type = type
        self.native = native
...
class Field(Expression):
    ...
    def __init__(self, fieldname, type='string', length=None, default=DEFAULT, required=False, requires=DEFAULT, ondelete='CASCADE', notnull=False, unique=False, uploadfield=True, widget=None, label=None, comment=None, writable=True, readable=True, update=None, authorize=None, autodelete=False, represent=None, uploadfolder=None, uploadseparate=False, uploadfs=None, compute=None, custom_store=None, custom_retrieve=None, custom_retrieve_file_properties=None, custom_delete=None, filter_in=None, filter_out=None, custom_qualifier=None, map_none=None, rname=None):
        ...
        if not isinstance(fieldname, str) or hasattr(Table, fieldname) or fieldname[0] == '_' or '.' in fieldname or REGEX_PYTHON_KEYWORDS.match(fieldname):
            raise SyntaxError('Field: invalid field name: %s, use rname for "funny" names' % fieldname)
        ...
        self.represent = list_represent if represent is None and type in ('list:integer',
                                                                          'list:string') else represent
    ...
    def store(self, file, filename=None, path=None):
        ...
        if isinstance(self_uploadfield, Field):
            ...
            keys = {self_uploadfield.name: newfilename, blob_uploadfield_name: file.read()}
    ...
    def validate(self, value):
        if not self.requires or self.requires == DEFAULT:
            ...
            return (
             value if value != self.map_none else None, None)

    def count(self, distinct=None):
        return Expression(self.db, self.db._adapter.COUNT, self, distinct, 'integer')

    def as_dict(self, flat=False, sanitize=True):
        attrs = ('name', 'authorize', 'represent', 'ondelete', 'custom_store', 'autodelete',
                 'custom_retrieve', 'filter_out', 'uploadseparate', 'widget', 'uploadfs',
                 'update', 'custom_delete', 'uploadfield', 'uploadfolder', 'custom_qualifier',
                 'unique', 'writable', 'compute', 'map_none', 'default', 'type',
                 'required', 'readable', 'requires', 'comment', 'label', 'length',
                 'notnull', 'custom_retrieve_file_properties', 'filter_in')
...
class Query(object):
    ...
    def __init__(self, db, op, first=None, second=None, ignore_common_filters=False, **optional_args):
        self.db = self._db = db
        self.op = op
        self.first = first
        self.second = second
    ...
    def as_dict(self, flat=False, sanitize=True):
        ...
        """
        SERIALIZABLE_TYPES = (
         tuple, dict, set, list, int, long, float, basestring, type(None), bool)
...
class Set(object):
        ...
        if query is None:
            ...
            if isinstance(query, Table):
                ...
                if isinstance(query, str):
                    ...
                    query = query != None
                if self.query:
                    return Set(self.db, self.query & query, ignore_common_filters=ignore_common_filters)
    ...
    def _select(self, *fields, **attributes):
        ...
        tablenames = adapter.tables(self.query, attributes.get('join', None), attributes.get('left', None), attributes.get('orderby', None), attributes.get('groupby', None))
    ...
    def build(self, d):
        ...
        if op in ('AND', 'OR'):
            if not (
             type(first), type(second)) == (dict, dict):
            ...
            elif op in ('LOWER', 'UPPER', 'EPOCH', 'PRIMARY_KEY', 'COALESCE_ZERO',
                        'RAW', 'INVERT'):
                built = Expression(self.db, opm, left)
            elif op in ('COUNT', 'EXTRACT', 'AGGREGATE', 'SUBSTRING', 'REGEXP', 'LIKE',
                        'ILIKE', 'STARTSWITH', 'ENDSWITH', 'ADD', 'SUB', 'MUL', 'DIV',
                        'MOD', 'AS', 'ON', 'COMMA', 'NOT_NULL', 'COALESCE', 'CONTAINS',
                        'BELONGS'):
    ...
    def count(self, distinct=None, cache=None):
        ...
        if cache:
            ...
            if len(key) > 200:
                key = hashlib_md5(key).hexdigest()
            return cache_model(key, lambda self=self, distinct=distinct: db._adapter.count(self.query, distinct), time_expire)
    ...
    def delete_uploaded_files(self, upload_fields=None):
        ...
        if upload_fields:
            ...
            fields = table.fields
        fields = [ f for f in fields if table[f].type == 'upload' and table[f].uploadfield == True and table[f].autodelete ]
        if not fields:
            return False
        else:
...
class Rows(object):
    ...
    def setvirtualfields(self, **keyed_virtualfields):
        ...
        from dal import lazy_virtualfield
    ...
    def __and__(self, other):
        ...
        return Rows(self.db, records, self.colnames, compact=self.compact or other.compact)
    ...
    def sort(self, f, reverse=False):
        ...
        rows.records = [ r for r, s in sorted(zip(self.records, self), key=lambda r: f(r[1]), reverse=reverse)
                       ]
        return rows
    ...
    def as_dict(self, key='id', compact=True, storage_to_dict=True, datetime_to_str=False, custom_types=None):
        """
    ...
    def as_json(self, mode='object', default=None):
        ...
        object structure)
        """
        items = [ record.as_json(mode=mode, default=default, serialize=False, colnames=self.colnames) for record in self ]
        if have_serializers:
