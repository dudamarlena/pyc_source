...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/aioetcd3/cluster.py
# Compiled at: 2018-05-26 21:48:07
# Size of source mod 2**32: 3087 bytes
import functools, aiogrpc, grpc
...
class Cluster(StubMixin):
    ...
    @call_grpc(lambda peerurls: rpc.MemberAddRequest(peerURLs=peerurls), lambda r: r.member, lambda s: s._cluster_stub.MemberAdd)
    async def member_add(self, peerurls):
        pass

    @call_grpc(lambda mid: rpc.MemberRemoveRequest(ID=mid), lambda r: [m for m in r.members], lambda s: s._cluster_stub.MemberRemove)
    async def member_remove(self, mid):
        pass

    @call_grpc(lambda mid, urls: rpc.MemberUpdateRequest(ID=mid, peerURLs=urls), lambda r: [m for m in r.members], lambda s: s._cluster_stub.MemberUpdate)
    async def member_update(self, mid, peerurls):
        pass

    @call_grpc(lambda : rpc.MemberListRequest(), lambda r: [m for m in r.members], lambda s: s._cluster_stub.MemberList)
    ...
    async def member_healthy(self, members=None):
        ...
        for m in members:
            ...
            if m:
                ...
                if self._credentials:
                    channel = aiogrpc.secure_channel(server_endpoint, (self._credentials), options=(self._options), loop=(self._loop),
                      executor=(self._executor),
                      standalone_pool_for_streaming=True)
                else:
                    channel = aiogrpc.insecure_channel(server_endpoint, options=(self._options), loop=(self._loop), executor=(self._executor),
                      standalone_pool_for_streaming=True)
                try:
        ...
        return (
         health_members, unhealth_members)