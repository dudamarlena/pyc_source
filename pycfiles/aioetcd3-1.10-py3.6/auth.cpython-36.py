...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/aioetcd3/auth.py
...
class Auth(StubMixin):
    ...
    @call_grpc(lambda username, password: rpc.AuthenticateRequest(name=username, password=password), lambda r: r.token, lambda s: s._auth_stub.Authenticate)
    ...
    @call_grpc(lambda username: rpc.AuthUserGetRequest(name=username), lambda r: [r for r in r.roles], lambda s: s._auth_stub.UserGet)
    async def user_get(self, username):
        pass

    @call_grpc(lambda username, password: rpc.AuthUserAddRequest(name=username, password=password), lambda r: None, lambda s: s._auth_stub.UserAdd)
    async def user_add(self, username, password):
        pass

    @call_grpc(lambda username: rpc.AuthUserDeleteRequest(name=username), lambda r: None, lambda s: s._auth_stub.UserDelete)
    async def user_delete(self, username):
        pass

    @call_grpc(lambda username, password: rpc.AuthUserChangePasswordRequest(name=username, password=password), lambda r: None, lambda s: s._auth_stub.UserChangePassword)
    async def user_change_password(self, username, password):
        pass

    @call_grpc(lambda username, role: rpc.AuthUserGrantRoleRequest(user=username, role=role), lambda r: None, lambda s: s._auth_stub.UserGrantRole)
    async def user_grant_role(self, username, role):
        pass

    @call_grpc(lambda username, role: rpc.AuthUserRevokeRoleRequest(name=username, role=role), lambda r: None, lambda s: s._auth_stub.UserRevokeRole)
    async def user_revoke_role(self, username, role):
        pass

    @call_grpc(lambda : rpc.AuthRoleListRequest(), lambda r: [role for role in r.roles], lambda s: s._auth_stub.RoleList)
    async def role_list(self):
        pass

    @call_grpc(lambda name: rpc.AuthRoleGetRequest(role=name), lambda r: [p for p in r.perm], lambda s: s._auth_stub.RoleGet)
