...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/test/test_auth.py
...
class AuthTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.root_client = ssl_client(endpoint=auth_etcd_url, ca_file='test/cfssl/ca.pem', cert_file='test/cfssl/client-root.pem',
          key_file='test/cfssl/client-root-key.pem')
        self.client_client = ssl_client(endpoint=auth_etcd_url, ca_file='test/cfssl/ca.pem', cert_file='test/cfssl/client.pem',
    ...
    async def test_auth_3(self):
        ...
        await self.client.role_add(name=TEST_ROLE_NAME)
        await self.client.user_grant_role(username=TEST_USER_NAME, role=TEST_ROLE_NAME)
        await self.client.role_grant_permission(name=TEST_ROLE_NAME, key_range=(range_all()),
          permission=PER_RW)
        await self.client.user_revoke_role(username=TEST_USER_NAME, role=TEST_ROLE_NAME)
        await self.client.role_revoke_permission(name=TEST_ROLE_NAME, key_range=(range_all()))
