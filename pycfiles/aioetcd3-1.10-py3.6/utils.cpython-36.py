...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/aioetcd3/utils.py
...
def ipv4_endpoints(server_list):
    return 'ipv4:///' + ','.join((s + ':2379' if ':' not in s else s) for s in server_list)
    ...
    with open(ca_cert, 'rb') as (ca_cert_file):
        with open(cert_key, 'rb') as (cert_key_file):
            with open(cert_cert, 'rb') as (cert_cert_file):
                return (
                 ca_cert_file.read(), cert_key_file.read(), cert_cert_file.read())