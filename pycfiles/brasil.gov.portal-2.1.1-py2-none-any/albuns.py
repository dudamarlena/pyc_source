...
# [GCC 8.4.0]
# Embedded file name: /home/hvelarde/forcontent/idg/src/brasil.gov.portal/src/brasil/gov/portal/browser/album/albuns.py
...
class Pagination(object):
    ...
    def __init__(self, context, request, data_type):
    ...
    def _set_album_attributes(self):
        ...
        brains = catalog(Type=self.data_type, path={'query': path})
        for brain in brains:
            item = brain.getObject()
            path = ('/').join(item.getPhysicalPath())
            childs = catalog(Type=['Image', 'Folder'], path={'query': path, 'depth': 1}, sort_on='getObjPositionInParent')
            if len(childs) > 0:
                child = childs[0]
                if child.Type == 'Image':
    ...
    def _get_brains(self):
        ...
        catalog = getToolByName(self.context, 'portal_catalog')
        path = ('/').join(self.context.getPhysicalPath())
        brains = catalog(Type=self.data_type, path={'query': path}, sort_on='effective', sort_order='reverse', review_state='published')
        albuns = []
        for brain in brains:
            obj = brain.getObject()
    ...
    def get_pagination(self):
        ...
        if len(pagination) > self.pages_visible:
            dots = {'link': False, 'href': '', 
               'content': '...', 
               'class': 'reticencias', 
               'is_prev': False, 
               'is_next': False}
            limit_start = self.pages_visible // 2 + 2
            limit_end = len(pagination) - self.pages_visible // 2 - 1
            if self.current_page <= limit_start:
                ...
                start = self.current_page - 1 - self.pages_visible // 2
                end = self.current_page - 1 + self.pages_visible // 2 + 1
        ...
        if self.current_page > 1:
            item = {'link': True, 'href': ('?pagina={0}').format(self.current_page - 1), 
               'content': _('label_previous'), 
               'class': 'anterior', 
               'is_prev': True, 
               'is_next': False}
...
class GaleriaDeAlbunsView(BrowserView):
    ...
    def setup(self):
        pagination = Pagination(self.context, self.request, 'Folder')
    ...
    def _toLocalizedTime(self, time, long_format=None, time_only=None):
        plone_view = getMultiAdapter((self.context,
         self.request), name='plone')
        return plone_view.toLocalizedTime(time, long_format, time_only)

    def album_total_images(self, item):
        catalog = getToolByName(self.context, 'portal_catalog')
        path = ('/').join(item.getPhysicalPath())
        brains = catalog(Type='Image', path={'query': path, 'depth': 1}, sort_on='getObjPositionInParent')
        return len(brains)
    ...
    def thumbnail(self, item):
        catalog = getToolByName(self.context, 'portal_catalog')
        path = ('/').join(item.getPhysicalPath())
        brains = catalog(Type=['Image', 'Folder'], path={'query': path, 'depth': 1}, sort_on='getObjPositionInParent')
        if len(brains) > 0:
            brain = brains[0]
            if brain.Type == 'Image':
                ...
                return {'src': thumb.url if thumb else image.absolute_url(), 
