...
# Embedded file name: build/bdist.linux-i686/egg/pykaryote/utils/analysis.py
...
def aggregate_batch(analyzer_dirs, output_analyzer_dir):
    ...
    for metric_datafile in os.listdir(analyzer_dirs[0]):
        ...
        if metric_datafile[:27] == 'time_spent_in_gaussian_gen_':
            ...
            mean, stddev, blanks = ann.quick_read(metric_datafile, mode='threshold')
    ...
    *Data*:
    ...
    def __init__(self, data_folders):
        ...
        if data_folders == [] or data_folders is None:
            raise Exception('Analyzer must be provided with a list of data folders')
        ...
        try:
            ...
            raise Exception('Failed to read config: %s' % os.path.join(self.base[0], 'used.cfg'))

        if len(data_folders) == 1:
            self.single = True
        else:
            self.single = False
        warnings.filterwarnings('ignore', '.*Attempting to set identical bottom==top results.*')
        return
    ...
    def plot_fitness(self, breakdown=True, **kwargs):
        ...
        *Args*:
        ...
        if self.single:
            ...
            min, lower, avg, upper, max = self.quick_read('fitness_avg', mode='breakdown')
            x = arange(0, len(avg))
            fill_between(x, min, max, alpha=0.1)
            fill_between(x, lower, upper, alpha=0.2)
            plot(avg)
    ...
    def plot_complexity_and_fitness(self, breakdown=True, mass_mode=False, **kwargs):
    ...
    def plot_genome_length(self, breakdown=True, **kwargs):
        ...
        *Args*:
        ...
        if self.single:
            ...
            min, lower, avg, upper, max = self.quick_read('genome_length_avg', mode='breakdown')
            x = arange(0, len(avg))
            fill_between(x, min, max, alpha=0.1)
            fill_between(x, lower, upper, alpha=0.2)
            plot(avg)
    ...
    def plot_avg_time_spent(self, **kwargs):
        ...
        *Args*:
        ...
        fill_between(x, 0, y_data_stacked[0, :], facecolor=(0.1, 0.5, 0.1), alpha=0.7)
        fill_between(x, y_data_stacked[0, :], y_data_stacked[1, :], facecolor=(0.3,
                                                                               0.3,
                                                                               0.8), alpha=0.7)
        fill_between(x, y_data_stacked[1, :], y_data_stacked[2, :], facecolor=(1, 0.3,
                                                                               0.3))
        plot(y_data_stacked[0, :], label='gather', color=(0, 0.5, 0), **kwargs)
        plot(y_data_stacked[1, :], label='move', color=(0, 0, 0.8), **kwargs)
        plot(y_data_stacked[2, :], label='build', color=(1, 0, 0), **kwargs)
        legend(loc=3, borderaxespad=0.0)
    ...
    def plot_failed_builds(self, **kwargs):
        ...
        *Args*:
        ...
        if self.single:
            ...
            min, lower, avg, upper, max = self.quick_read('failed_builds', mode='breakdown')
            x = arange(0, len(avg))
            fill_between(x, min, max, facecolor=color_list[0], alpha=0.2, edgecolor='white')
            fill_between(x, lower, upper, facecolor=color_list[0], alpha=0.2, edgecolor='white')
    ...
    def plot_gen_time(self, **kwargs):
        ...
        *Args*:
        ...
        if self.single:
            ...
            min, lower, avg, upper, max = self.quick_read('gen_time', mode='breakdown')
            x = arange(0, len(avg))
            fill_between(x, min, max, facecolor=color_list[0], alpha=0.2, edgecolor='white')
            fill_between(x, lower, upper, facecolor=color_list[0], alpha=0.2, edgecolor='white')
            plot(avg, color=color_list[0])
    ...
    def plot_chemicals(self, **kwargs):
        ...
        *Args*:
        ...
        legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.0, title='Chem #')
    ...
    def plot_complexes(self, **kwargs):
        ...
        *Args*:
        ...
        legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.0, title='Length')
    ...
    def plot_invented_complexes(self, **kwargs):
        ...
        *Args*:
        ...
        if self.single:
            ...
            min, lower, avg, upper, max = self.quick_read('complexes_invented', mode='breakdown')
            x = arange(0, len(avg))
            for c in range(gv.max_complex_length):
                fill_between(x, min[:, c], max[:, c], facecolor=color_list[c], alpha=0.2, edgecolor='white')
                fill_between(x, lower[:, c], upper[:, c], facecolor=color_list[c], alpha=0.2, edgecolor='white')
    ...
    def plot_component_percent_functional(self, **kwargs):
        ...
        *Args*:
        ...
        if self.single:
            ...
            data_array = self.quick_read('component_percent_functional', aggregate=False)
        ylim(ymin=0, ymax=1)
    ...
    def plot_family_sizes(self, **kwargs):
        ...
        *Args*:
        ...
        """
        subplots_adjust(right=0.8)
        data = self.quick_read('family_sizes') + 0.1
        if gv.max_complex_length > 1:
            for c in range(gv.max_complex_length):
                semilogy(data[:, c], label=str(c + 1), **kwargs)
    ...
    def plot_family_lengths(self, **kwargs):
        ...
        *Args*:
        ...
        if self.single:
            ...
            min, lower, avg, upper, max = self.quick_read('family_lengths', mode='breakdown')
            x = arange(0, len(avg))
            for c in range(gv.max_complex_length):
                fill_between(x, min[:, c], max[:, c], facecolor=color_list[c], alpha=0.2, edgecolor='white')
                fill_between(x, lower[:, c], upper[:, c], facecolor=color_list[c], alpha=0.2, edgecolor='white')
    ...
    def plot_num_distinct(self, comp_or_fam, timeframe='current', functional=None, derivative=False, **kwargs):
        """
        ...
        *Args*:
        ...
        if functional:
            ...
            if self.single:
                ...
                min, lower, avg, upper, max = self.quick_read(file_to_read, mode='breakdown')
                x = arange(0, len(avg))
                for c in range(gv.max_complex_length):
                    fill_between(x, min[:, c], max[:, c], facecolor=color_list[c], alpha=0.2, edgecolor='white')
                    fill_between(x, lower[:, c], upper[:, c], facecolor=color_list[c], alpha=0.2, edgecolor='white')
    ...
    def plot_complexity(self, breakdown=True, mass_mode=False, **kwargs):
        ...
        *Args*:
        ...
        """
        if mass_mode:
            plot(self.quick_read('complexity_avg'), label='complexity_avg', **kwargs)
        ...
        if self.single:
            if breakdown:
                ...
                plot(avg, label='irreducible_complexity', color=color_list[1])
            else:
            ...
            min, lower, avg, upper, max = self.quick_read('complexity_avg', mode='breakdown')
            x = arange(0, len(avg))
            fill_between(x, min, max, alpha=0.1, color=color_list[0])
            fill_between(x, lower, upper, alpha=0.2, color=color_list[0])
            plot(avg, color=color_list[0], label='complexity')
            min, lower, avg, upper, max = self.quick_read('irreducible_complexity_avg', mode='breakdown')
            fill_between(x, min, max, alpha=0.1, color=color_list[1])
            fill_between(x, lower, upper, alpha=0.2, color=color_list[1])
    ...
    def plot_time_spent_in_gaussian(self):
        ...
        for index, datafile in enumerate(datafiles):
            ...
            if index == 0:
                ...
                plt.figure(figsize=(16 * num_runs, 6 * len(datafiles)))
                gaussian_colors = environment_draw.make_chemical_colors(data.shape[2] - 1, format='matplotlib')
            ...
            for run_num in range(num_runs):
                ...
                for gaussian_index, gaussian_data in enumerate(data[run_num].T):
                    plt.bar(ind, gaussian_data, color=gaussian_colors[gaussian_index], bottom=bottom, width=width, align='center')
                    bottom += gaussian_data

                plt.ylabel('Time')
                plt.title(('Time spent on chemical mound (generation {}, run {})').format(gen_number, run_num))
                plt.xlabel('Organisms')
                plt.xticks(ind + width / 2.0, range(len(ind)))
                legend_boxes = [ Rectangle((0, 0), 1, 1, color=c) for c in gaussian_colors
                               ]
                legend_labels = [ ('mound {}').format(x) for x in range(len(legend_boxes))
                                ]
                legend_labels[-1] = 'no mound'
                plt.legend(legend_boxes, legend_labels, loc='best', fancybox=True)
    ...
    def save_environment_drawing(self, out_dir):
        ...
        environment_draw.draw_environment(grid, os.path.join(out_dir, 'environment.png'))
    ...
    def save_all(self, out_dir):
        ...
        if gv.max_complex_length > 0:
            ...
            figure()
            subplot(211)
            self.plot_num_distinct('complexes', timeframe='current', functional=True)
            subplot(212)
            self.plot_num_distinct('complexes', timeframe='current', functional=False)
            save()
            figure()
            subplot(211)
            self.plot_num_distinct('complexes', timeframe='ever', functional=True)
            subplot(212)
            self.plot_num_distinct('complexes', timeframe='ever', functional=False)
            save()
            figure()
            subplot(211)
            self.plot_num_distinct('families', timeframe='current', functional=True)
            subplot(212)
            self.plot_num_distinct('families', timeframe='current', functional=False)
            save()
            figure()
            subplot(211)
    ...
    def quick_read(self, data, mode='standard', ndmin=1, format='text'):
        ...
        *Returns*:
        ...
        if mode == 'breakdown':
            if self.single:
                raise Exception('Breakdown only valid for Analyzers with                                 multiple runs')
    ...
    def export_all_as_csv(self, output_file):
        ...
        *Args*:
        ...
        data, headings = self.add_data(data, headings, 'chemicals', gv.num_chemicals)
        data = concatenate((data, self.quick_read('time_spent')), axis=1)
        headings += 'time_gathering, time_moving, time_building, '
        data, headings = self.add_data(data, headings, 'chemical_complexity', gv.num_chemicals)
        data, headings = self.add_data(data, headings, 'complexes', gv.max_complex_length)
        data, headings = self.add_data(data, headings, 'complex_diversity', gv.max_complex_length)
        data, headings = self.add_data(data, headings, 'family_sizes', gv.max_complex_length)
        data, headings = self.add_data(data, headings, 'family_lengths', gv.max_complex_length)
        data, headings = self.add_data(data, headings, 'percent_functional', gv.max_complex_length)
        data, headings = self.add_data(data, headings, 'component_percent_functional', gv.max_complex_length)
...
class CreativeAnalyzer(object):
    ...
    *Args*:
    ...
    def __open_data_files(self):
        ...
        self.data_files = {}
        metrics = [
         'fitness_avg', 'genome_length_avg', 'chemicals',
        ...
        metrics_not_comparison = [
         'complexity_min', 'complexity_lower',
    ...
    def close_all(self):
        ...
        np.save(os.path.join(self.base, 'environment'), self.sim.environment.grid)
    ...
    def update(self):
        ...
        chemicals = [ avg([ chem[i] for chem in chemall ]) for i in range(gv.num_chemicals)
                    ]
        ...
        if self.sim.comparison:
            ...
            c_min, c_lower, c_avg, c_upper, c_max = breakdown_points(ci_array[:, 0])
            ...
            ic_min, ic_lower, ic_avg, ic_upper, ic_max = breakdown_points(ci_array[:, 1])
        ...
        complexes_f = np.zeros(gv.max_complex_length, int)
        complexes_nf = np.zeros(gv.max_complex_length, int)
        ...
        self.quick_append('num_complexes_current_nonfunctional', list(complexes_nf))
        complexes = np.zeros(gv.max_complex_length, int)
        complexes_f = np.zeros(gv.max_complex_length, int)
        complexes_nf = np.zeros(gv.max_complex_length, int)
        ...
        fams_f = np.zeros(gv.max_complex_length, int)
        fams_nf = np.zeros(gv.max_complex_length, int)
        for fam in cur_fams:
            fams[(fam.length - 1)] += 1
            if max(fam.function_vector) > gv.weak_factor * fam.length * gv.mutate_factor:
        ...
        for metric, thresholds in zip(gv.metrics, gv.thresholds):
            for i, threshold in enumerate(thresholds):
                if self._threshold_metrics[metric][i] == -1 and self._current_metric_values.get(metric, -1) >= threshold:
    ...
    def quick_append(self, metric, new_data):
        ...
        *Args*:
        ...
        if type(new_data) == list or type(new_data) == array:
        ...
        if comp.complex.is_functional:
            if i > 1:
                ...
                if component_func_vec[(i - 1)] == 0.0:
                    component_func_vec[i - 1] = 0.005 * i
...
def compute_complexity(org):
    ...
    return (
     complexity, irreducible_complexity)
