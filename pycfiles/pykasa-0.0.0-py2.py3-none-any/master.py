...
class Master(object):
    ...
    def add_comparisons(self, batch_config, num_runs=None, clean=True):
        ...
        for comparison_name in config.sections():
            cmp_cfg = os.path.expanduser(config.get(comparison_name, 'comparison_config'))
            sim_cfg = os.path.expanduser(config.get(comparison_name, 'simulation_config'))
            if num_runs is None:
                runs = config.getint(comparison_name, 'num_runs')
            else:
                runs = num_runs
            self.add_comparison(cmp_cfg, sim_cfg, comparison_name, runs, clean=clean)

        return

    def add_comparison(self, comparison_config_filename, sim_config_filename, comparison_name, num_runs, clean=True):
        """Queues a comparison to run.
        """
        vary_attrs, values = load_from_cmp(comparison_config_filename)
        if 'omparison' not in comparison_name:
        ...
        config_filenames = []
        config_dir = os.path.join(self.dest_dir, comparison_name, 'sim_configs')
        ...
        shutil.copyfile(comparison_config_filename, os.path.join(self.dest_dir, comparison_name, 'cmp.cfg'))
        comparison = {}
        ...
        for run_num in range(num_runs):
            for sim_num, config_filename in enumerate(config_filenames):
                ...
                job = self._create_comparison_job(config_filename, comparison_name, run_num, sim_num, target_gen)
                self._add_job(job)

        self.log(('added comparison: {}').format(comparison_name))

    def _create_comparison_job(self, config_filename, comparison_name, run_num, sim_num, target_gen):
        job = {}
    ...
    def wait_and_log(self):
        ...
        while self._busy_processes:
            ...
            if status.tag == tags.SUCCESS_TAG:
                ...
                self._on_recive_status(message[0], message[1], message[2], process_id)
    ...
    def _report_status(self):
        ...
        for name, comp in self._comparisons.iteritems():
            ...
            header = ('{name}: {percent_done}').format(name=name, percent_done=percent_done)
            summary = ('({} running, {} done, {} failed, {} queued)').format(comp['num_running'], comp['num_done'], comp['num_failed'], comp['num_waiting'])
        ...
        print ('\nTotal: {} ({}/{})').format(total_progress, total_gens_done, total_gens_target)
    ...
    def _comparison_ready_to_graph(self, cmp_name):
        ...
        """
        comp = self._comparisons[cmp_name]
        return comp['num_done'] + comp['num_failed'] == comp['total_processes']
