...
# Embedded file name: build/bdist.linux-x86_64/egg/ally/container/_impl/_setup.py
...
from ._call import WithType, WithCall, CallEvent, CallEventOnCount, WithListeners, CallConfig, CallEntity, CallStart, CallEventControlled
...
def register(setup, register):
    ...
    setups = register.get('__ally_setups__')
    if setups is None:
        setups = register['__ally_setups__'] = []
    setups.append(setup)
    return setup
...
class SetupFunction(Setup):
    ...
    def __init__(self, function, name=None, group=None):
        ...
        if not not group:
            ...
            if name:
                ...
                assert not (fnArgs.args or fnArgs.varargs or fnArgs.varkw), "The setup function '%s' cannot have any type of arguments" % self.name
    ...
    def __str__(self):
        return '%s at:%s' % (self.__class__.__name__, locationStack(self._function))
    ...
    def assemble(self, assembly):
        ...
        if self._types:
            ...
            if call.types:
                ...
                if not found:
                    raise SetupError('There is no common class for replaced classes %s and replace classes %s' % ([str(clazz) for clazz in self._types], [str(clazz) for clazz in call.types]))
        ...
        for name, val in assembly.configExtern.items():
            if name == self.name or self.name.endswith('.' + name):
                if name in assembly.configUsed:
                    raise SetupError('The configuration %r is already in use and the configuration "%s" cannot use it again, provide a more detailed path for the configuration (ex: "ally_core.url" instead of "url")' % (
                     name, self.name))
    ...
    def __init__(self, function, target, event, auto, **keyargs):
        ...
        for target in targets:
            if not isinstance(target, str):
                raise AssertionError('Invalid target %s' % target)
        ...
        for target in self._targets:
            ...
            try:
                if self._event == self.BEFORE:
                    call.addBefore(partial(assembly.processForName, self.name), self._auto)
                elif self._event == self.AFTER:
                    call.addAfter(partial(assembly.processForName, self.name), self._auto)
            except SetupError:
...
class SetupEventCancel(Setup):
    ...
    def __str__(self):
        return '%s for:%s' % (self.__class__.__name__, self.target)
...
class SetupEventControlled(SetupFunction):
    ...
    def __init__(self, function, priority, triggers, **keyargs):
        ...
        for trigger in triggers:
            if not isinstance(trigger, ITrigger):
                raise AssertionError('Invalid trigger %s' % trigger)
