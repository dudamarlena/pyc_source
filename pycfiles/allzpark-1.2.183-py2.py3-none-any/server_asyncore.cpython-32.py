...
# Embedded file name: build/bdist.linux-x86_64/egg/ally/http/server/server_asyncore.py
...
class RequestHandler(dispatcher, BaseHTTPRequestHandler):
    ...
    def _1_handle_data(self, data):
        ...
        if index >= 0:
            ...
            if index < len(data) and self.handle_data:
    ...
    def _3_handle_write(self):
        ...
        if what == WRITE_ITER:
            ...
            if what == WRITE_BYTES:
                ...
                dataLen = len(data)
                try:
                    if dataLen > self.bufferSize:
                        sent = self.send(data[:self.bufferSize])
                    else:
                        sent = self.send(data)
                except socket.error:
                ...
                if sent < dataLen:
                    if what == WRITE_ITER:
                        self._writeq.appendleft((WRITE_BYTES, data[sent:]))
                    elif what == WRITE_BYTES:
                        self._writeq[0] = (WRITE_BYTES, data[sent:])
                elif what == WRITE_BYTES:
                    del self._writeq[0]
    ...
    def _process(self, method):
        ...
        chain = Chain(proc)
        chain.process(**proc.fillIn(request=request, requestCnt=requestCnt, response=proc.ctx.response(), responseCnt=proc.ctx.responseCnt()))
        ...
        def respond():
            ...
            if ResponseContentHTTP.source in responseCnt and responseCnt.source is not None:
                if isinstance(responseCnt.source, IInputStream):
                    source = readGenerator(responseCnt.source, self.bufferSize)
                else:
...
class AsyncServer(dispatcher):
    ...
    def __init__(self):
        ...
        self.processing = self.assembly.create(request=RequestHTTP, requestCnt=RequestContentHTTPAsyncore, response=ResponseHTTP, responseCnt=ResponseContentHTTP)
