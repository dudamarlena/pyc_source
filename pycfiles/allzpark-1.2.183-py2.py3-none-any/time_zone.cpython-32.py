...
# Embedded file name: build/bdist.linux-x86_64/egg/ally/core/http/impl/processor/time_zone.py
...
class TimeZoneHandler(HandlerProcessorProceed):
    ...
    def __init__(self):
        ...
        assert isinstance(self.nameContentTimeZone, str), 'Invalid time zone content header name %s' % self.nameContentTimeZone
    ...
    def process(self, request: Request, response: Response, **keyargs):
        ...
        if not failed and timeZoneContent:
            try:
                timeZoneContent = timezone(timeZoneContent)
            except UnknownTimeZoneError:
                failed = True
                response.code, response.status, response.isSuccess = TIME_ZONE_ERROR
