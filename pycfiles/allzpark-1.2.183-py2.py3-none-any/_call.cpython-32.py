...
# Embedded file name: build/bdist.linux-x86_64/egg/ally/container/_impl/_call.py
...
class WithType:
    ...
    def setType(self, types):
        ...
        for clazz in types:
            if not isclass(clazz):
                raise AssertionError('Invalid type class %s' % clazz)
    ...
    def isOf(self, clazz):
        ...
        for typ in self._types:
            if clazz == typ or issubclass(typ, clazz):
                return True
...
class CallEvent(WithCall, WithListeners):
    ...
    def validateAcceptListeners(self):
        ...
        """
        if self._processed:
            raise SetupError("Already processed cannot add anymore listeners to '%s'" % self.name)
    ...
    def __call__(self):
        ...
        if self._processed:
            ...
            for listener, _auto in self._listenersBefore:
                listener()

            ret = self.call()
            if ret is not None:
                raise SetupError("The event call '%s' cannot return any value" % self.name)
            for listener, _auto in self._listenersAfter:
                listener()
...
class CallEntity(WithCall, WithType, WithListeners):
    ...
    def __call__(self):
        ...
        if not self._hasValue:
            ...
            ret = self.call()
            if isgenerator(ret):
                value, followUp = next(ret), partial(next, ret, None)
            else:
                value, followUp = ret, None
            if value is not None:
                valueId, callsOfValue = id(value), self.assembly.callsOfValue
                calls = callsOfValue.get(valueId)
                if calls is None:
                    callsOfValue[valueId] = calls = [self]
                else:
                    calls.append(self)
            else:
                valueId = None
            if not log.debug("Processed entity '%s' with value %s", self.name, value):
                assert True
                ...
                for inter in self._interceptors:
                    try:
                        ...
                        raise SetupError('Problems for interceptor %s' % inter)
                ...
                if not log.debug('Finalized %r with value %s', self.name, value):
                    assert True
                return self._value
...
class CallConfig(WithType, WithListeners):
    ...
    def __call__(self):
        ...
        if isinstance(self._value, Exception):
            raise self._value
        if not self._hasValue:
            raise ConfigError('No value for configuration %s' % self.name)
        return self._value
