...
# Embedded file name: build/bdist.linux-x86_64/egg/ally/api/operator/extract.py
...
from .type import TypeContainer, TypeCriteria, TypeQuery, TypeModelProperty, TypeModel
...
def extractProperties(namescape):
    ...
    for name, value in namescape.items():
        if not name.startswith('_'):
            ...
            typ = typeFor(value)
            if typ is None:
                log.warning('Cannot extract property for class %s attribute "%s" of value %s', namescape.get('__name__'), name, value)
        else:
            properties[name] = typ
...
def extractCriterias(namespace):
    ...
    for name, value in namespace.items():
        if not name.startswith('_'):
            ...
            log.warning('Cannot extract criteria for class %s attribute "%s" of value %s', namespace.get('__name__'), name, value)
...
def extractOuputInput(function, types=None, modelToId=False):
    ...
    if types:
        ...
        assert not annotations, 'The types for the input arguments cannot be declared as annotations %s and call arguments %s' % (
         annotations, types)
        annotations['return'] = types[0]
        annotations.update({args[k]:types[k] for k in })
    ...
    for k, arg in enumerate(args[1:]):
        if arg in args:
            if arg not in annotations:
                raise DevelError('There is no type for %s' % arg)
            ...
            if k < mandatory:
                inputs.append(Input(arg, typ))
            else:
                inputs.append(Input(arg, typ, True, defaults[(k - mandatory)]))
                continue
...
def processGenericType(forType, generic):
    ...
    if isinstance(forType, TypeProperty):
        ...
        if isinstance(forType, TypeModel):
            assert isinstance(forType, TypeModel)
            genericModelClass = generic.get(forType.clazz)
            if genericModelClass is not None:
                newType = typeFor(genericModelClass)
                if not newType is not None:
                    raise AssertionError('Invalid generic model class %s, has no model type' % newType)
        else:
            if isinstance(forType, TypeQuery):
                ...
                if genericQueryClass is not None:
                    ...
                    if not newType is not None:
                        raise AssertionError('Invalid generic query class %s, has no query type' % newType)
            elif isinstance(forType, Iter):
                assert isinstance(forType, Iter)
                itemType = processGenericType(forType.itemType, generic)
                if itemType:
                    if isinstance(forType, List):
                        newType = List(itemType)
                    else:
                        newType = Iter(itemType)
    return newType