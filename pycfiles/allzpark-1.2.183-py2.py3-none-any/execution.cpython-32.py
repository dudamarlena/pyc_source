...
# Embedded file name: build/bdist.linux-x86_64/egg/ally/design/processor/execution.py
        ...
        if contexts:
            ...
            for key, clazz in contexts.items():
                ...
                if not isinstance(clazz, ContextMetaClass):
                    raise AssertionError('Invalid context class %s for %s' % (clazz, key))
...
class Chain:
    ...
    def __init__(self, processing):
        ...
        for call in self._calls:
            if not callable(call):
                raise AssertionError('Invalid processor call %s' % call)
    ...
    def do(self):
        ...
        if not log.debug('Processing %s', call):
            assert True
        self._proceed = False
        try:
            ...
            if self._callBacksErrors:
                self._proceed = False
                while self._callBacksErrors:
                    self._callBacksErrors.pop()()

            else:
                raise

        if not log.debug("Processing finalized '%s'", call):
            assert True
        if self._proceed:
            if not log.debug('Proceed signal received, continue execution'):
                assert True
                if self._calls:
                    return True
                if not log.debug('Processing finalized by consuming'):
                    assert True
                    self._consumed = True
        else:
            self._calls.clear()
