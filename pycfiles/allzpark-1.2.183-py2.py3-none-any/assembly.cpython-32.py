...
# Embedded file name: build/bdist.linux-x86_64/egg/ally/design/processor/assembly.py
    ...
    def __init__(self, *processors):
        ...
        for processor in self.processors:
            if not isinstance(processor, IProcessor):
                raise AssertionError('Invalid processor %s' % processor)
...
class Assembly(Container):
    ...
    def move(self, processor, before=None, after=None):
        ...
        """
        if not before is not None:
            assert after is not None, 'You need to provide an after or a before processor in order to move'
            processor = self._processorFrom(processor)
            try:
                self.processors.remove(processor)
            except ValueError:
                raise AssemblyError('Unknown processor %s in assembly' % processor)
    ...
    def replace(self, replaced, replacer):
        ...
        """
        try:
            index = self.processors.index(self._processorFrom(replaced))
        except ValueError:
            raise AssemblyError('Invalid replaced processor %s' % replaced)
    ...
    def remove(self, *processors):
        ...
        for processor in processors:
            try:
                index = self.processors.index(self._processorFrom(processor))
            except ValueError:
                raise AssemblyError('Invalid processor %s to be removed' % processor)
    ...
    def create(self, **contexts):
        ...
        if lines:
            ...
            log.info("Nothing to report for '%s', everything fits nicely", self.name)
        return processing
    ...
    def _processorsFrom(self, processors):
        ...
        for processor in processors:
            if isinstance(processor, (list, tuple)):
                for processor in self._processorsFrom(processor):
                    yield processor
            ...
            elif isinstance(processor, IProcessor):
                yield processor
            else:
                raise AssemblyError('Invalid processor or container %s' % processor)
    ...
    def _indexFrom(self, before=None, after=None):
        ...
        if before is not None:
            ...
            try:
                index = self.processors.index(before)
            except ValueError:
                raise AssemblyError('Unknown before processor %s in assembly' % before)

        else:
            if after is not None:
                after = self._processorFrom(after)
                try:
                    index = self.processors.index(after) + 1
                except ValueError:
                    raise AssemblyError('Unknown after processor %s in assembly' % after)
