...
# Embedded file name: build/bdist.linux-x86_64/egg/ally/container/_impl/_support.py
...
class SetupEntityWire(Setup):
                ...
                if wiring:
                    ...
                    for wentity in wiring.entities:
                        ...
                        if wentity.name not in value.__dict__:
                            try:
                                setattr(value, wentity.name, entityFor(wentity.type, wentity.name))
                            except:
                                raise SetupError("Cannot solve wiring '%s' at: %s" % (
    ...
    def __str__(self):
        return '%s for %s' % (self.__class__.__name__, ', '.join(self._wirings.keys()))
        ...
        if not group is None:
            ...
            for call in listeners:
                if not callable(call):
                    raise AssertionError('Invalid listener %s' % call)
...
class SetupEntityProxy(Setup):
    ...
    def __init__(self, group, classes, binders):
        ...
        for call in binders:
            if not callable(call):
                raise AssertionError('Invalid binder %s' % call)
            ...
            for clazz in self._classes:
                if isinstance(value, clazz):
                    ...
                    for binder in self._binders:
                        if not log.debug('Binded %s to %s', binder, value):
                            assert True
                            binder(valueProxy)
...
def classesFrom(classes):
    ...
    for clazz in classes:
        if isinstance(clazz, str) or ismodule(clazz):
            ...
            raise SetupError('Cannot use class %s' % clazz)
