...
# Embedded file name: build/bdist.linux-x86_64/egg/ally/core/http/impl/processor/path_encoder_resource.py
...
class ResourcePathEncoderHandler(HandlerProcessorProceed):
    ...
    def __init__(self):
        if not self.resourcesRootURI is None:
            assert isinstance(self.resourcesRootURI, str), 'Invalid root URI %s' % self.resourcesRootURI
    ...
    def process(self, request: Request, response: Response, **keyargs):
        ...
        if Request.extension in request:
            extension = request.extension
        else:
            extension = None
        if response.encoderPath is None:
            response.encoderPath = EncoderPathResource(self, extension)
        else:
            response.encoderPath = EncoderPathResource(self, extension, response.encoderPath)
        return
...
class EncoderPathResource(IEncoderPath):
    ...
    def __init__(self, handler, extension=None, wrapped=None):
        ...
        if not extension is None:
            assert isinstance(extension, str), 'Invalid extension %s' % extension
    ...
    def encode(self, path, invalid=None, quoted=None, **keyargs):
        ...
        if isinstance(path, Path):
            ...
            if self.extension:
                ...
                if quoted:
                    uri = quote(uri)
                if self.handler.resourcesRootURI:
                    uri = self.handler.resourcesRootURI % uri
            if self.wrapped:
                return self.wrapped.encode(uri, **keyargs)
            return uri
        else:
    ...
    def encodePattern(self, path, invalid=None, **keyargs):
        ...
        if isinstance(path, Path):
            ...
            if self.extension:
                ...
                uri.append(self.extension)
            uri = ''.join(uri)
            if self.handler.resourcesRootPattern:
                uri = self.handler.resourcesRootPattern % uri
            if self.wrapped:
                return self.wrapped.encodePattern(uri, **keyargs)
            return uri
        else:
            if self.wrapped is None:
                assert invalid is not None, 'No invalid replacer expected %s' % invalid
