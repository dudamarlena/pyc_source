...
# Embedded file name: build/bdist.linux-x86_64/egg/ally/support/core/util_resources.py
...
from ally.core.spec.resources import Match, Node, Path, ConverterPath, IResourcesRegister, Invoker, PathExtended
...
def findGetModel(fromPath, typeModel):
    ...
    for index in range(len(fromPath.matches), 0, -1):
        matchNodes.clear()
        path = _findGetModel(typeModel, fromPath, fromPath.matches[(index - 1)].node, index, True, matchNodes, fromPath.matches[index].node if index < len(fromPath.matches) else None)
        if path:
            return path
...
def findGetAllAccessible(pathOrNode):
    ...
    for child in node.children:
        ...
        if isinstance(child, NodePath):
            ...
            if path is None:
                extended = Path(matches, child)
            else:
                extended = PathExtended(path, matches, child)
            if child.get:
...
def findNodesFor(node, typeService, name):
    ...
    for node in iterateNodes(node):
        ...
        if typeService == typeFor(invoker.implementation) and invoker.call.name == name:
            nodes.append(node)
            continue
...
def propertyTypesOf(pathOrNode, invoker):
    ...
    """
    if isinstance(pathOrNode, Node):
        path = pathForNode(pathOrNode)
    else:
        path = pathOrNode
    assert isinstance(path, Path), 'Invalid path %s' % path
    assert isinstance(invoker, Invoker), 'Invalid invoker %s' % invoker
...
class ResourcesRegisterDelegate(IResourcesRegister):
    ...
    def __init__(self, main, *others):
        ...
        for register in others:
            if not isinstance(register, IResourcesRegister):
                raise AssertionError('Invalid register %s' % main)
...
def _findGetModel(modelType, fromPath, node, index, inPath, matchNodes, exclude=None):
    ...
    if not exclude is None:
        assert isinstance(exclude, Node), 'Invalid exclude node %s' % exclude
    added = False
        ...
        if node.name == modelType.container.name:
            for nodeId in node.children:
                if isinstance(nodeId, NodeProperty):
                    ...
                    if nodeId.get is None:
                        continue
                    assert isinstance(nodeId.get, Invoker)
                    if not nodeId.get.output.isOf(modelType):
                        continue
                    for typ in nodeId.typesProperties:
                        ...
                        for matchNode in matchNodes:
                            pushMatch(matches, matchNode.newMatch())
