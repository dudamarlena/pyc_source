...
# Embedded file name: build/bdist.linux-x86_64/egg/ally/gateway/http/impl/processor/respository_authorized.py
# Compiled at: 2013-10-02 09:54:40
__doc__ = '\nCreated on Apr 12, 2012\n\n@package: gateway service\n@copyright: 2011 Sourcefabric o.p.s.\n@license: http://www.gnu.org/licenses/gpl-3.0.txt\n@author: Gabriel Nistor\n\nProvides the gateway repository processor.\n'
from . import respository
from .respository import GatewayRepositoryHandler, Repository, Identifier, Response
from ally.container.ioc import injected
from ally.design.processor.attribute import requires, defines
from ally.design.processor.context import Context
from ally.design.processor.execution import Processing
from ally.gateway.http.spec.gateway import IRepository, RepositoryJoined
from ally.http.spec.codes import BAD_REQUEST, BAD_GATEWAY, INVALID_AUTHORIZATION, isSuccess
    ...
    def performCleanup(self):
        ...
        for authentication, lastAccess in self._lastAccess.items():
            if current > lastAccess:
                expired.append(authentication)
                continue

        if not log.debug('Clearing %s sessions at %s' % (len(expired), datetime.now())):
            assert True
