...
# Embedded file name: build/bdist.linux-x86_64/egg/ally/api/operator/container.py
    ...
    def __init__(self, properties):
        ...
        for typName, typ in properties.items():
            ...
            if not isinstance(typ, Type):
                raise AssertionError('Not a criteria type %s' % typ)
...
class Model(Container):
    ...
    def __init__(self, properties, name, propertyId=None, hints={}):
        ...
        for hintn in hints:
            if not isinstance(hintn, str):
                raise AssertionError('Invalid hint name %s' % hintn)
    ...
    def __init__(self, properties, main=()):
        ...
        for typ in properties.values():
            ...
            if not typ.isPrimitive:
                raise AssertionError('Not a primitive criteria type for %s' % typ)

        typ = None
        for prop in main:
            assert prop in properties, 'Invalid main property %s, is not found in the provided properties' % prop
            if typ is not None:
                pass
            if not properties[prop].isOf(typ):
                raise AssertionError('Invalid main property %s with type %s' % (prop, properties[prop]))
            else:
                typ = properties[prop]
...
class Query:
    ...
    def __init__(self, criterias):
        ...
        for crtName, crtClass in criterias.items():
            ...
            if not isclass(crtClass):
                raise AssertionError('Not a criteria class %s' % crtClass)
    ...
    def __init__(self, calls):
        ...
        for call in calls:
            if not isinstance(call, Call):
                raise AssertionError('Invalid call %s' % call)
