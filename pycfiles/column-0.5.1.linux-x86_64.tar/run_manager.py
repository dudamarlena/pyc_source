...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/column/api/manager/run_manager.py
...
class RunManager(object):
    ...
    def _parse_result(self, run_id, result):
        ...
        if unreachable_hosts:
            error_msg = ('{} failed because the following nodes were unreachable: {}.\n').format(run['playbook_path'], unreachable_hosts)
            self._update_run(run['id'], objects.State.ERROR, error_msg)
        elif failed_hosts:
            error_msg = ('{} failed on the following nodes: {}').format(run['playbook_path'], failed_hosts)
            self._update_run(run['id'], objects.State.ERROR, error_msg)
        else:
    ...
    def _run_playbook(self, run):
        play_context.PlayContext._attributes = copy.deepcopy(api.context_attributes)
        ...
        try:
            result = self.column_manager.run_playbook(column_opts['playbook_path'], inventory_file, **options)
    ...
    def create_run(self, run):
        ...
        LOG.debug('Triggering a new run with %s', run_model.format_response(run))
    ...
    def delete_run(self, run):
        timeout = 10
        if run['state'] == objects.State.COMPLETED or run['state'] == objects.State.ERROR or run['state'] == objects.State.ABORTED:
            return True
