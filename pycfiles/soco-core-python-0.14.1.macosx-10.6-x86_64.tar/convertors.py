...
# [GCC 8.4.0]
# Embedded file name: /Users/tonyzhao/anaconda2/lib/python2.7/site-packages/soco_core/convertors.py
...
class DocConvert(object):
    ...
    def _get_context(cls, data, current_idx, prev_char, next_char, last_title, last_section):
        ...
        return {'context': context, 'answer_start': answer_start, 
           'value': answer, 
           'displayable': False, 
    ...
    def _normalize_whitespace(cls, text):
        text = text.replace('\n', ' ').replace('\r', '').replace('\t', '').replace(b'\xa0', ' ')
        return text
    ...
    def document_to_frames(cls, doc, lang, doc_meta=None, verbose=False, min_ans_len=5, max_ans_len=500):
        ...
        for f_id, f_data in enumerate(flatten_data):
            ...
            if chunk_type in ('title', 'section'):
                context = cls._get_context(flatten_data, f_id, prev_char=0, next_char=context_size, last_title=last_title, last_section=last_section)
            else:
                context = cls._get_context(flatten_data, f_id, prev_char=context_size, next_char=context_size, last_title=last_title, last_section=last_section)
        ...
        if verbose:
            print ('DONE PROCESS {} RAW DOCUMENTS with {} too short skip {} too long skip').format(len(frames), too_short_cnt, too_long_cnt)
        return frames