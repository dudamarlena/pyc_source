...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hjb/demoutil.py
# Compiled at: 2006-06-04 06:11:09
"""
...
import time, logging, logging.handlers, cPickle
...
__all__ = [
 'accept_jms_log', 'write_message', 'heartbeat_for', 'five_simple_messages', 'five_simple_messages_generator', 'filesystem_message_generator', 'ResponseFromFile', 'RepeatingTimer', 'Heartbeat', 'HeartbeatMonitor', 'JMSHandler', 'PeriodicFlushMemoryHandler']
...
def heartbeat_for(name, status='alive', **kw):
    ...
    """
    headers = {'hjb_demo': hjb_bool(True), 'hjb_send_to': 'heartbeat'}
    return HJBMapMessage(application=name, status=status, headers=headers, **kw)
...
five_simple_messages = [
 HJBMessage('sent by PyHJB!', headers={'hjb_demo': hjb_bool(True), 'hjb_useful': 'for sure'}), HJBMapMessage(value_foo=hjb_long(100), value_bar=hjb_int(300), value_baz=hjb_byte_array('boo!'), value_bool=hjb_bool(True), value_str='anything', headers={'hjb_demo': hjb_bool(True)}), HJBStreamMessage(values=[hjb_long(1000), hjb_float(6000), hjb_byte(3)], headers={'hjb_demo': hjb_bool(True), 'hjb_useful': 'maybe'}), HJBBytesMessage(hjb_byte_array('sent (as bytes) by PyHJB!'), headers={'hjb_demo': hjb_bool(True), 'hjb_useful': 'not sure'}), HJBObjectMessage('(base64 rO0ABXNyAA5qYXZhLnV0aWwuRGF0ZWhqgQFLWXQZAwAAeHB3CAAAAQTrseBAeA==)', headers={'hjb_demo': hjb_bool(True), 'hjb_useful': 'unlikely'})]
...
def filesystem_message_generator(paths=None, exts=None):
...
class Heartbeat(object):
    ...
    def start(self):
        ...
        def send_heartbeat():
            self.client.send_message(heartbeat_for(self.name, 'alive'), self.producer_url)

        t = RepeatingTimer(self.period, send_heartbeat)
        t.setDaemon(True)
        self.client.send_message(heartbeat_for(self.name, 'starting'), self.producer_url)
        self._timer = t
        t.start()
    ...
    def stop(self):
        ...
        if hasattr(self, '_timer'):
            self.client.send_message(heartbeat_for(self.name, 'stopping'), self.producer_url)
            self._timer.cancel()
            del self._timer
...
class HeartbeatMonitor(object):
    ...
    def check_for_lapsing(self):
        ...
        self._lock.acquire()
        try:
            self.recorded_statuses = dict(((a, update_lapsed(a, s, t)) for (a, s, t) in self.recorded_statuses.values()))
        finally:
            self._lock.release()
...
class JMSHandler(logging.Handler):
    ...
    def make_message(self, record):
        ...
        return HJBBytesMessage(hjb_byte_array(p), headers={'hjb_log': hjb_bool(True), 'hjb_demo': hjb_bool(True), 'hjb_useful': 'darn right'})
