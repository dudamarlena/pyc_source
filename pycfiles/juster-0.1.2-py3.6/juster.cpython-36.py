...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\juster\juster.py
# Compiled at: 2019-06-09 13:51:55
...
def justify(content, grid=True, frame=False, enumerator=False, header=False, topbar='', newline='\n', delimiter=';', justsize=4):
    ...
    if type(content) is str:
        ...
        if header:
            ...
            line = '+'.join(['-' * len(item) for item in justifiedParts[0]])
            if frame:
                edgeLine = '-'.join(['-' * len(item) for item in justifiedParts[0]]).join(['+'] * 2)
                line = line.join(['|'] * 2)
