...
# [GCC 8.4.0]
# Embedded file name: /Users/chris/Projects/chris/various/pdfminer/venv/lib/python2.7/site-packages/pdfminer/converter.py
...
class HTMLConverter(PDFConverter):
    RECT_COLORS = {'figure': 'yellow', 
       'textline': 'magenta', 
       'textbox': 'cyan', 
       'textgroup': 'red', 
       'curve': 'black', 
       'page': 'gray'}
    TEXT_COLORS = {'textbox': 'blue', 
       'char': 'black'}

    def __init__(self, rsrcmgr, outfp, codec='utf-8', pageno=1, laparams=None, scale=1, fontscale=1.0, layoutmode='normal', showpageno=True, pagemargin=50, imagewriter=None, debug=0, rect_colors={'curve': 'black', 'page': 'gray'}, text_colors={'char': 'black'}):
        PDFConverter.__init__(self, rsrcmgr, outfp, codec=codec, pageno=pageno, laparams=laparams)
        self.scale = scale
    ...
    def place_rect(self, color, borderwidth, x, y, w, h):
        ...
        if color is not None:
            self.write('<span style="position:absolute; border: %s %dpx solid; left:%dpx; top:%dpx; width:%dpx; height:%dpx;"></span>\n' % (
             color, borderwidth,
    ...
    def place_image(self, item, borderwidth, x, y, w, h):
        if self.imagewriter is not None:
            ...
            self.write('<img src="%s" border="%d" style="position:absolute; left:%dpx; top:%dpx;" width="%d" height="%d" />\n' % (
    ...
    def begin_div(self, color, borderwidth, x, y, w, h, writing_mode=False):
        ...
        self._font = None
        self.write('<div style="position:absolute; border: %s %dpx solid; writing-mode:%s; left:%dpx; top:%dpx; width:%dpx; height:%dpx;">' % (
...
class XMLConverter(PDFConverter):
    ...
    def __init__(self, rsrcmgr, outfp, codec='utf-8', pageno=1, laparams=None, imagewriter=None, stripcontrol=False):
        PDFConverter.__init__(self, rsrcmgr, outfp, codec=codec, pageno=pageno, laparams=laparams)
        self.imagewriter = imagewriter
