...
# [GCC 8.4.0]
# Embedded file name: /Users/chris/Projects/chris/various/pdfminer/venv/lib/python2.7/site-packages/pdfminer/layout.py
...
class LAParams(object):
    ...
    def __init__(self, line_overlap=0.5, char_margin=2.0, line_margin=0.5, word_margin=0.1, boxes_flow=0.5, detect_vertical=False, all_texts=False):
...
class LTTextLineHorizontal(LTTextLine):
    ...
    def find_neighbors(self, plane, ratio):
        ...
        return [ obj for obj in objs if isinstance(obj, LTTextLineHorizontal) and abs(obj.height - self.height) < d and (abs(obj.x0 - self.x0) < d or abs(obj.x1 - self.x1) < d)
               ]
...
class LTTextLineVertical(LTTextLine):
    ...
    def find_neighbors(self, plane, ratio):
        ...
        return [ obj for obj in objs if isinstance(obj, LTTextLineVertical) and abs(obj.width - self.width) < d and (abs(obj.y0 - self.y0) < d or abs(obj.y1 - self.y1) < d)
               ]
...
class LTTextGroupLRTB(LTTextGroup):
    ...
    def analyze(self, laparams):
        ...
        self._objs = csort(self._objs, key=lambda obj: (1 - laparams.boxes_flow) * obj.x0 - (1 + laparams.boxes_flow) * (obj.y0 + obj.y1))
...
class LTTextGroupTBRL(LTTextGroup):
    ...
    def analyze(self, laparams):
        LTTextGroup.analyze(self, laparams)
        self._objs = csort(self._objs, key=lambda obj: -(1 + laparams.boxes_flow) * (obj.x0 + obj.x1) - (1 - laparams.boxes_flow) * obj.y1)
...
class LTLayoutContainer(LTContainer):
    ...
    def group_textboxes(self, laparams, boxes):
        ...
        while dists:
            ...
            plane.remove(obj1)
            plane.remove(obj2)
            dists = [ (c, d, obj1, obj2) for c, d, obj1, obj2 in dists if obj1 in plane and obj2 in plane
                    ]
            for other in plane:
...
class LTFigure(LTLayoutContainer):
    ...
    def __init__(self, name, bbox, matrix):
        ...
        self.matrix = matrix
        x, y, w, h = bbox
        bbox = get_bound(apply_matrix_pt(matrix, (p, q)) for p, q in (
         (
