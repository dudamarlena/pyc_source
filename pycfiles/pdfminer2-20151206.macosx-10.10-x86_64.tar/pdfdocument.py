...
# [GCC 8.4.0]
# Embedded file name: /Users/chris/Projects/chris/various/pdfminer/venv/lib/python2.7/site-packages/pdfminer/pdfdocument.py
# Compiled at: 2015-11-01 16:27:52
import re, struct, logging, six
try:
...
class PDFXRefStream(PDFBaseXRef):
    ...
    def load(self, parser):
        ...
        logging.info('xref stream: objid=%s, fields=%d,%d,%d', (', ').join(map(repr, self.ranges)), self.fl1, self.fl2, self.fl3)
...
class PDFStandardSecurityHandler(object):
    ...
    def compute_u(self, key):
        if self.r == 2:
            ...
            for i in range(1, 20):
                k = ('').join(six.int2byte(c ^ i) for c in six.iterbytes(key))
...
class PDFStandardSecurityHandlerV5(PDFStandardSecurityHandlerV4):
    ...
    def authenticate(self, password):
        ...
        if hash.digest() == self.o_hash:
            ...
            hash.update(self.u)
            return AES.new(hash.digest(), mode=AES.MODE_CBC, IV='\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00').decrypt(self.oe)
        else:
            hash = SHA256.new(password)
            ...
            if hash.digest() == self.u_hash:
                ...
                return AES.new(hash.digest(), mode=AES.MODE_CBC, IV='\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00').decrypt(self.ue)
            return
...
class PDFDocument(object):
    ...
    """
    security_handler_registry = {1: PDFStandardSecurityHandler, 
    ...
    def _getobj_objstm(self, stream, index, objid):
        if stream.objid in self._parsed_objs:
            ...
            if self.caching:
                self._parsed_objs[stream.objid] = (
                 objs, n)
            i = n * 2 + index
            try:
                obj = objs[i]
    ...
    def getobj(self, objid):
        if not objid != 0:
            ...
            raise (self.xrefs or PDFException)('PDFDocument is not initialized')
