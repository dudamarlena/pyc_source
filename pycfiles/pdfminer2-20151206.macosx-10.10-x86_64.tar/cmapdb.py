...
# [GCC 8.4.0]
# Embedded file name: /Users/chris/Projects/chris/various/pdfminer/venv/lib/python2.7/site-packages/pdfminer/cmapdb.py
...
class CMapParser(PSStackParser):
    ...
    KEYWORD_BEGINCMAP = KWD('begincmap')
    KEYWORD_ENDCMAP = KWD('endcmap')
    KEYWORD_USECMAP = KWD('usecmap')
    KEYWORD_DEF = KWD('def')
    KEYWORD_BEGINCODESPACERANGE = KWD('begincodespacerange')
    KEYWORD_ENDCODESPACERANGE = KWD('endcodespacerange')
    KEYWORD_BEGINCIDRANGE = KWD('begincidrange')
    KEYWORD_ENDCIDRANGE = KWD('endcidrange')
    KEYWORD_BEGINCIDCHAR = KWD('begincidchar')
    KEYWORD_ENDCIDCHAR = KWD('endcidchar')
    KEYWORD_BEGINBFRANGE = KWD('beginbfrange')
    KEYWORD_ENDBFRANGE = KWD('endbfrange')
    KEYWORD_BEGINBFCHAR = KWD('beginbfchar')
    KEYWORD_ENDBFCHAR = KWD('endbfchar')
    KEYWORD_BEGINNOTDEFRANGE = KWD('beginnotdefrange')
    KEYWORD_ENDNOTDEFRANGE = KWD('endnotdefrange')
    ...
    def do_keyword(self, pos, token):
        ...
        if token is self.KEYWORD_ENDCIDRANGE:
            ...
            for s, e, cid in choplist(3, objs):
                if not isinstance(s, str) or not isinstance(e, str) or not isinstance(cid, int) or len(s) != len(e):
                    continue
                sprefix = s[:-4]
                eprefix = e[:-4]
                if sprefix != eprefix:
        ...
        if token is self.KEYWORD_ENDBFRANGE:
            ...
            for s, e, code in choplist(3, objs):
                if not isinstance(s, bytes) or not isinstance(e, bytes) or len(s) != len(e):
                    continue
                s1 = nunpack(s)
                e1 = nunpack(e)
