...
# [GCC 8.4.0]
# Embedded file name: /Users/chris/Projects/chris/various/pdfminer/venv/lib/python2.7/site-packages/pdfminer/pdffont.py
...
class CFFFont(object):
    STANDARD_STRINGS = ('.notdef', 'space', 'exclam', 'quotedbl', 'numbersign', 'dollar',
                ...
                for ec, sc, idd, idr in zip(ecs, scs, idds, idrs):
                    if idr:
                        ...
                        for c in range(sc, ec + 1):
                            char2gid[c] = struct.unpack('>H', fp.read(2))[0] + idd & 65535
...
class PDFFont(object):
    ...
    def decode(self, bytes):
        return [ six.indexbytes(bytes, i) for i, _ in enumerate(bytes) ]
...
class PDFCIDFont(PDFFont):
    ...
    def __init__(self, rsrcmgr, spec, STRICT=False):
        ...
        self.cidcoding = '%s-%s' % (self.cidsysteminfo.get('Registry', 'unknown').decode('latin1'),
         self.cidsysteminfo.get('Ordering', 'unknown').decode('latin1'))
        try:
