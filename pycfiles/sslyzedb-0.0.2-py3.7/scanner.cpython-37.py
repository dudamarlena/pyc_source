...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\sslyzedb\core\scanner.py
# Compiled at: 2019-02-25 14:48:48
...
def create_server_info(target):
    ...
    server_tester = ServerConnectivityTester(hostname=(target.domain_name if target.domain_name is not None else target.ip_address),
      port=(target.port),
...
def start_scanner(session, scan_id, with_progressbar=False):
    ...
    for target in enum_targets(session, scan_id, with_progressbar):
        ...
        server_info.target_id = target.id
        tci = TargetConnectivityInfo()
        tci.scan_id = scan.id
        tci.target_id = target.id
