...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\sslyzedb\core\report.py
# Compiled at: 2019-02-25 14:15:37
...
class FullResult:
    HEADER = [
     'target', 'compression_supported', 'supports_fallback_scsv', 'is_vulnerable_to_heartbleed', 'is_vulnerable_to_ccs_injection',
    ...
    def from_db(session, scanid):
        ...
        query = session.query(Target, CompressionScanResult.compression_name, FallbackScsvScanResult.supports_fallback_scsv, HeartbleedScanResult.is_vulnerable_to_heartbleed, OpenSslCcsInjectionScanResult.is_vulnerable_to_ccs_injection, SessionRenegotiationScanResult.accepts_client_renegotiation, SessionRenegotiationScanResult.supports_secure_renegotiation, RobotScanResult.robot_result_enum, EarlyDataScanResult.is_early_data_supported).filter(Target.id.in_(active_targets)).join(CompressionScanResult,
          (and_(CompressionScanResult.target_id == Target.id, CompressionScanResult.scan_id == scanid)), isouter=True).join(FallbackScsvScanResult,
          (and_(FallbackScsvScanResult.target_id == Target.id, FallbackScsvScanResult.scan_id == scanid)), isouter=True).join(HeartbleedScanResult,
          (and_(HeartbleedScanResult.target_id == Target.id, HeartbleedScanResult.scan_id == scanid)), isouter=True).join(OpenSslCcsInjectionScanResult,
          (and_(OpenSslCcsInjectionScanResult.target_id == Target.id, OpenSslCcsInjectionScanResult.scan_id == scanid)), isouter=True).join(SessionRenegotiationScanResult,
          (and_(SessionRenegotiationScanResult.target_id == Target.id, SessionRenegotiationScanResult.scan_id == scanid)), isouter=True).join(RobotScanResult,
          (and_(RobotScanResult.target_id == Target.id, RobotScanResult.scan_id == scanid)), isouter=True).join(EarlyDataScanResult,
          (and_(EarlyDataScanResult.target_id == Target.id, EarlyDataScanResult.scan_id == scanid)), isouter=True)
        logger.debug('Report queriing for target properties')
        for target, compression_name, supports_fallback_scsv, is_vulnerable_to_heartbleed, is_vulnerable_to_ccs_injection, accepts_client_renegotiation, supports_secure_renegotiation, robot_result_enum, is_early_data_supported in query.all():
            fr = FullResult()
            fr.target = target
    ...
    def cipher_stat_map(self, sslversion):
        if sslversion in self.accepted_ciphers:
            return 'SUPPORTED'
        if sslversion in self.rejected_ciphers:
            return 'REJECTED'
        if sslversion in self.errored_ciphers:
            return 'ERRORED'
        return 'DISABLED'
