...
# [GCC 8.4.0]
# Embedded file name: /Users/krogager/Projects/VoigtFit/build/lib/VoigtFit/dataset.py
# Compiled at: 2020-03-29 15:55:41
# Size of source mod 2**32: 84945 bytes
...
def mask_vel(dataset, line_tag, v1, v2):
    ...
    for reg in regions_of_line:
        ...
        z = dataset.redshift
        vel = (reg.wl / (l0 * (z + 1)) - 1.0) * 299792.458
        mask = (vel > v1) * (vel < v2)
...
class DataSet(object):
    ...
    def __init__(self, redshift, name=''):
        ...
        molecules : dict
            ...
            for each molecule:
            ``{molecule* : [[band1, Jmax1],\xa0[band2, Jmax2], etc...]}``
    ...
    def add_data(self, wl, flux, res, err=None, normalized=False, mask=None, nsub=1):
        ...
        if mask is None:
            ...
            if np.sum(mask) == 0:
                ...
                return
        self.data.append({'wl':wl,  'flux':flux,  'error':err, 
         'res':res,  'norm':normalized, 
         'specID':specid,  'mask':mask, 
         'nsub':nsub})

    def reset_region(self, reg):
        """Reset the data in a given :class:`regions.Region` to use the raw input data."""
        for chunk in self.data:
            if reg.res == chunk['res']:
                if chunk['wl'].min() < reg.wl.min() < chunk['wl'].max():
                    raw_data = chunk
        ...
        if line_tag in lineList['trans']:
            ...
            if self.verbose:
                ...
                if velspan is None:
                    ...
                    l_center = new_line.l0 * (self.redshift + 1.0)
                    if self.data:
                        success = False
                        for chunk in self.data:
                            if chunk['wl'].min() < l_center < chunk['wl'].max():
                                wl = chunk['wl']
                                vel = (wl - l_center) / l_center * 299792.0
                                ...
                                if len(self.regions) > 0:
                                    for num, region in enumerate(self.regions):
                                        if chunk['specID'] == region.specID:
                                            wl_overlap = np.intersect1d(new_wavelength, region.wl)
                                            if wl_overlap.any():
                                                merge = num
                                ...
                                if merge >= 0:
                                    old_mask_i = np.interp((new_region.wl), old_wl, old_mask, left=1,
                                      right=1)
                                    old_mask_i = old_mask_i.astype(bool)
                                    tmp_mask_i = np.interp((new_region.wl), tmp_wl, tmp_mask, left=1,
                                      right=1)
                                    region_overlap = np.interp((new_region.wl), old_wl,
                                      (1 + 0 * old_mask), right=0,
                                      left=0)
                        ...
                        if not success:
                            if self.verbose:
                                err_msg = '\n [ERROR] - The given line is not covered by the spectral data: %s \n'
    ...
    def add_component(self, ion, z, b, logN, var_z=True, var_b=True, var_N=True, tie_z=None, tie_b=None, tie_N=None):
        """
    ...
    def add_component_velocity(self, ion, v, b, logN, var_z=True, var_b=True, var_N=True, tie_z=None, tie_b=None, tie_N=None):
        """
        Same as for :meth:`add_component <VoigtFit.DataSet.add_component>`
        but input is given as relative velocity instead of redshift.
        """
        z = self.redshift + v / 299792.458 * (self.redshift + 1.0)
        this_comp = Component(z, b, logN, var_z, var_b, var_N, tie_z, tie_b, tie_N)
        if ion in list(self.components.keys()):
            self.components[ion].append(this_comp)
        else:
    ...
    def interactive_components(self, line_tag, velocity=False):
        ...
        for region in regions_of_line:
            ...
            line = self.lines[line_tag]
            if velocity:
                l_ref = line.l0 * (self.redshift + 1.0)
            ...
            if line.element in list(self.components.keys()):
                ...
                if region.normalized:
                    ...
                    cont_level_point = plt.ginput(1, 30.0)
            ...
            for x0, y0 in comps:
                if velocity:
                    z0 = self.redshift + x0 / 299792.458 * (self.redshift + 1.0)
                else:
                    z0 = x0 / line.l0 - 1.0
    ...
    def copy_components(self, to_ion='', from_ion='', logN=0, ref_comp=None, tie_z=True, tie_b=True):
        ...
        if to_ion == '' and from_ion == '':
            ...
            if from_ion not in list(self.components.keys()):
                ...
                solar_elements = list(Asplund.solar.keys())
    ...
    def prepare_dataset(self, norm=True, mask=False, verbose=True, active_only=False, force_clean=True, velocity=False, check_lines=True, f_lower=0.0, f_upper=100.0, l_lower=0.0, l_upper=10000.0):
        """
        ...
        if norm:
            for region in self.regions:
                if not region.normalized:
                    go_on = 0
                    while go_on == 0:
                        go_on = region.normalize(norm_method=(self.norm_method), z_sys=z_sys)
        ...
        for ion in list(self.components.keys()):
            for n, comp in enumerate(self.components[ion]):
                ...
                self.pars.add(b_name, value=(myfloat(b)), vary=(comp.var_b), min=0.0)
                ...
                if region.new_mask:
                    if active_only:
                        if region.has_active_lines():
                            region.define_mask(z=(self.redshift), dataset=self, z_sys=z_sys)
                    active_only or region.define_mask(z=(self.redshift), dataset=self, z_sys=z_sys)
        ...
        for line_tag in self.all_lines:
            ion = line_tag.split('_')[0]
            line = self.lines[line_tag]
            if ion not in list(self.components.keys()):
                if line.active:
                    if self.verbose:
                        print('')
        ...
        if check_lines:
            ...
            for this_ion in list(self.components.keys()):
                for chunk in self.data:
                    wl_tot = chunk['wl']
                    lmin = wl_tot.min() / (self.redshift + 1.0)
                    lmax = wl_tot.max() / (self.redshift + 1.0)
                    ...
                    for entry in lineList[cut]:
                        if entry['trans'] in list(self.lines.keys()):
                            continue
                        lines_not_defined.append(entry)

            if len(lines_not_defined) > 0:
                if self.verbose:
                    print('')
                    print(term.red)
                    ...
                    for reg_num, region in enumerate(self.regions):
                        if region.has_active_lines():
                            ...
                            if rebin > 1:
                                ...
                                nsub = region.kernel_nsub
                            profile_obs = evaluate_profile(x, pars, (self.redshift), (list(self.lines.values())),
                              (self.components), (region.kernel),
    ...
    def plot_fit(self, rebin=1, fontsize=12, xmin=None, xmax=None, max_rows=4, ymin=None, ymax=None, filename=None, subsample_profile=1, npad=50, loc='left', highlight_props=None, residuals=True, norm_resid=False, default_props={}, element_props={}, legend=True, label_all_ions=False, xunit='vel', line_props=None, hl_line_props=None):
        """
        Plot *all* the absorption lines and the best-fit profiles.
        For details, see :func:`VoigtFit.output.plot_all_lines`.
        """
        output.plot_all_lines(self, plot_fit=True, rebin=rebin, fontsize=fontsize, xmin=xmin,
          xmax=xmax,
          max_rows=max_rows,
          ymin=ymin,
          ymax=ymax,
    ...
    def plot_line(self, line_tag, index=0, plot_fit=False, loc='left', rebin=1, nolabels=False, axis=None, fontsize=12, xmin=None, xmax=None, ymin=None, ymax=None, show=True, subsample_profile=1, npad=50, residuals=True, norm_resid=False, legend=True, default_props={}, element_props={}, highlight_props=None, label_all_ions=False, xunit='velocity', line_props=None, hl_line_props=None):
        """
        ...
        """
        output.plot_single_line(self, line_tag, index=index, plot_fit=plot_fit, loc=loc,
          rebin=rebin,
          nolabels=nolabels,
          axis=axis,
          fontsize=fontsize,
    ...
    def get_NHI(self):
        if 'HI' in list(self.components.keys()):
            if hasattr(self.best_fit, 'keys'):
