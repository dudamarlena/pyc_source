...
# [GCC 8.4.0]
# Embedded file name: /Users/andres/Dropbox/Development/Unholster/django-lookup/lookup/__init__.py
...
class LookupTable(object):
    ...
    def __init__(self, modelclass, key_prefix=None, persist=True, filters=None):
    ...
    def clean(self):
        if self.persist:
            for alias in Alias.objects.filter(target_type=ContentType.objects.get_for_model(self.model)):
    ...
    def refresh_cache(self):
        aliases = Alias.objects.filter(target_type=ContentType.objects.get_for_model(self.model), target_id__in=self.filtered)
    ...
    def prompt(self, text, lookup=True, create=False, defaults={}, **kwargs):
        ...
        if lookup:
            try:
                return (
                 self.lookup(text), created)
            except KeyError:
                pass

        model_label = self.model.__name__.title()
        while not obj:
            print '%s not found for %s <%s>' % (model_label, text, self.normalize(text))
            print 'Options:'
            print '  0 - Ignore'
            offset = 1
            suggestions = self.suggestions(text, **kwargs).items()
    ...
    def add_alias(self, text, target):
        ...
        if self.persist:
            Alias.objects.create(key=key, target=target)
        self._cache[key] = target

    def suggestions(self, text, n=3, cutoff=0.5):
        normalized = self.normalize(text)
        matches = get_close_matches(normalized, self._cache.keys(), n=n, cutoff=cutoff)
