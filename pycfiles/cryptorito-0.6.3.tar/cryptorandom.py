...
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/cryptorandom/cryptorandom.py
# Compiled at: 2018-09-11 00:48:42
__doc__ = '\nSHA-256 PRNG prototype in Python\n'
from __future__ import division
import numpy as np, sys, struct, random, hashlib
BPF = 53
RECIP_BPF = 2 ** (-BPF)
HASHLEN = 256
...
class SHA256(random.Random):
    ...
    def setstate(self, baseseed=None, counter=0):
        ...
        self._basehash()
        self.basehash.update('\x00' * counter)
    ...
    def randint_trunc(self, a, b, size=None):
        ...
        if size == None:
            ...
            return np.reshape(np.array([ a + int_from_hash(self.nextRandom()) % (b - a) for i in np.arange(np.prod(size))
                                       ]), size)
            return
    ...
    def randint(self, a, b, size=None):
        ...
        if size == None:
            ...
            return np.reshape(np.array([ a + self.randbelow_from_randbits(b - a) for i in np.arange(np.prod(size))
                                       ]), size)
            return