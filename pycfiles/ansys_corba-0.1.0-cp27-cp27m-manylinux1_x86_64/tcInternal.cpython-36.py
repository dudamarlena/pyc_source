...
# [GCC 8.4.0]
# Embedded file name: ./tcInternal.py
# Compiled at: 2018-07-20 10:03:27
# Size of source mod 2**32: 31849 bytes
import omniORB
c = omniORB._CORBA_dict
g = globals()
for k, v in c.items():
    if k.startswith('tk_') or k in ('BAD_PARAM', 'COMPLETED_NO', 'VM_TRUNCATABLE',
                                    'INTERNAL', 'BAD_TYPECODE', 'TCKind', 'TypeCode'):
        g[k] = v
...
def createUnionTC(id, name, discriminator_type, members):
    ...
    for m in members:
        val = m.label.value()
        if m.label.typecode().kind() == tk_octet:
            if val == 0:
                val = -1
                defused = count
        tup = (
         val, m.name, m.type._d)
        if defused != count:
            mmap[val] = tup
        mlist.append(tup)
        count = count + 1
...
def createAliasTC(id, name, original_type):
    d = (
     tv_alias, id, name, original_type._d)
    return createTypeCode(d)


def createExceptionTC(id, name, members):
    dlist = [
     tv_except, None, id, name]
    mnames = []
    for m in members:
        mnames.append(m.name)
        dlist.append(m.name)
...
def createSequenceTC(bound, element_type):
    d = (
     tv_sequence, element_type._d, bound)
    return createTypeCode(d)


def createArrayTC(length, element_type):
    d = (
     tv_array, element_type._d, length)
    return createTypeCode(d)


def createRecursiveTC(id):

    class recursivePlaceHolder(object):
        pass

    recursivePlaceHolder._d = (
     tv__indirect, [id])
...
def createValueBoxTC(id, name, boxed_type):
    d = (
     tv_value_box, omniORB.createUnknownValue(id, tv_null),
...
def createAbstractInterfaceTC(id, name):
    d = (
     tv_abstract_interface, id, name)
    return createTypeCode(d)


def createLocalInterfaceTC(id, name):
    d = (
     tv_local_interface, id, name)
    return createTypeCode(d)
...
def createTypeCode(d):
    ...
    k = d[0]
    if k == tv_string:
        return TypeCode_string(d)
    if k == tv_wstring:
        return TypeCode_wstring(d)
    if k == tv_fixed:
        return TypeCode_fixed(d)
    if k in [tv_objref, tv_abstract_interface, tv_local_interface]:
    ...
    if k == tv__indirect:
        if isinstance(d[1][0], str):
            ...
            if nd is None:
                raise BAD_TYPECODE(omniORB.BAD_TYPECODE_InvalidIndirection, COMPLETED_NO)
            d[1][0] = nd
...
class TypeCode_objref(TypeCode_base):
    ...
    def __init__(self, desc):
        if not isinstance(desc, tuple) or desc[0] not in [tv_objref,
...
class TypeCode_struct(TypeCode_base):
    ...
    def member_count(self):
        return (len(self._d) - 4) / 2

    def member_name(self, index):
        off = index * 2 + 4
        if index < 0 or off >= len(self._d):
            raise TypeCode.Bounds()
        return self._d[off]
...
class TypeCode_union(TypeCode_base):
    ...
    def member_name(self, index):
        if index < 0 or index >= len(self._d[6]):
            raise TypeCode.Bounds()
...
class TypeCode_enum(TypeCode_base):
    ...
    def member_name(self, index):
        if index < 0 or index >= len(self._d[3]):
            raise TypeCode.Bounds()
...
basicTypeCodes = {tv_null: TypeCode_empty(tv_null), 
    ...
    try:
        if a == b:
            ...
            if not (isinstance(a, tuple) and isinstance(b, tuple)):
                ...
                while isinstance(a, tuple) and (a[0] == tv_alias or a[0] == tv__indirect):
                    if a[0] == tv_alias:
                        if a[1] != '':
                            a_ids[a[1]] = a
                        a = a[3]
                ...
                while isinstance(b, tuple) and (b[0] == tv_alias or b[0] == tv__indirect):
                    if b[0] == tv_alias:
                        if b[1] != '':
                            b_ids[b[1]] = b
                        b = b[3]
                ...
                if a[0] == tv_struct:
                    ...
                    if a[2] != '' and b[2] != '':
                        ...
                        for i in range(4, len(a), 2):
                            if not equivalentDescriptors(a[(i + 1)], b[(i + 1)], seen, a_ids, b_ids):
                                return 0
                ...
                if a[0] == tv_union:
                    if a[2] != '':
                        ...
                        if a[2] != '' and b[2] != '':
                            ...
                            return equivalentDescriptors(a[4], b[4], seen, a_ids, b_ids) or 0
                    if a[5] != b[5]:
                        return 0
                    else:
                        if len(a[6]) != len(b[6]):
                        ...
                        for i in range(len(a[6])):
                            if a[6][i][0] != b[6][i][0]:
                                return 0
                            if not equivalentDescriptors(a[6][i][2], b[6][i][2], seen, a_ids, b_ids):
                                return 0
                                                                    ...
                                                                    if k == tv_longdouble:
                                                                        ...
                                                                        if k == tv_struct:
                                                                            ...
                                                                            if k == tv_union:
                                                                                ...
                                                                                if d[7] is not None:
                                                                                    c[7] = (
                                                                                     d[7][0], '', r_getCompactDescriptor(d[7][2], seen, ind))
                                                                                r = tuple(c)
                                                                                seen[d[2]] = r
                                                                                seen[id(d)] = r
                                                                            else:
                                                                                if k == tv_enum:
                                                                                    ...
                                                                                    if k == tv_sequence:
                                                                                        r = (
                                                                                         k, r_getCompactDescriptor(d[1], seen, ind), d[2])
                                                                                    else:
                                                                                        if k == tv_array:
                                                                                            r = (
                                                                                             k, r_getCompactDescriptor(d[1], seen, ind), d[2])
                                                                                            ...
                                                                                            if k == tv_alias:
                                                                                                r = (
                                                                                                 k, d[1], '', r_getCompactDescriptor(d[3], seen, ind))
                                                                                            else:
                                                                                                if k == tv_except:
                                                                                                    c = list(d)
                                                                                                    c[3] = ''
                                                                                                    ...
                                                                                                    if k == tv_value:
                                                                                                        ...
                                                                                                        if k == tv_value_box:
                                                                                                            ...
                                                                                                            if k == tv_abstract_interface:
                                                                                                                r = d
                                                                                                            else:
                                                                                                                if k == tv_local_interface:
                                                                                                                    r = d
                                                                                                                else:
                                                                                                                    if k == tv__indirect:
                                                                                                                        l = [
