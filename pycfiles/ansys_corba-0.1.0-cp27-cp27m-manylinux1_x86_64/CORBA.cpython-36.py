...
# [GCC 8.4.0]
# Embedded file name: ./CORBA.py
# Compiled at: 2018-07-20 10:03:27
# Size of source mod 2**32: 38184 bytes
"""
Main omniORB CORBA module
"""
import _omnipy, omniORB, threading, types, time
try:
    property
except NameError:
...
class UserException(Exception):
    ...
    def __init__(self, *args):
        self._UserException__args = args
    ...
    def __getitem__(self, i):
        return self._UserException__args[i]
    ...
    def __reduce__(self):
        return (
         self.__class__, self._UserException__args)
...
class TypeCode(object):
    ...
    def equal(self, tc):
        return self._t.equal(tc)

    def equivalent(self, tc):
        return self._t.equivalent(tc)

    def get_compact_typecode(self):
        return self._t.get_compact_typecode()
    ...
    def member_count(self):
        return self._t.member_count()

    def member_name(self, index):
        return self._t.member_name(index)

    def member_type(self, index):
        return self._t.member_type(index)

    def member_label(self, index):
        return self._t.member_label(index)

    def discriminator_type(self):
        return self._t.discriminator_type()

    def default_index(self):
        return self._t.default_index()

    def length(self):
        return self._t.length()

    def content_type(self):
        return self._t.content_type()

    def fixed_digits(self):
        return self._t.fixed_digits()

    def fixed_scale(self):
        return self._t.fixed_scale()

    def member_visibility(self, index):
        return self._t.member_visibility(index)
...
TC_longdouble = _tc_longdouble = tcInternal.createTypeCode(tcInternal.tv_longdouble)
TC_wchar = _tc_wchar = tcInternal.createTypeCode(tcInternal.tv_wchar)
TC_wstring = _tc_wstring = tcInternal.createTypeCode((tcInternal.tv_wstring, 0))
builtin_id = id
...
class ORB(object):
    ...
    _d_InvalidName = (
     omniORB.tcInternal.tv_except, InvalidName,
...
class Object(object):
    __doc__ = ' CORBA::Object base class '
    _NP_RepositoryId = ''
    _nil = None
    ...
    def _get_interface(self):
        if omniORB.orb is None:
            raise BAD_INV_ORDER(omniORB.BAD_INV_ORDER_ORBHasShutdown, COMPLETED_NO)
...
_d_Object = (
 omniORB.tcInternal.tv_objref, Object._NP_RepositoryId, 'Object')
...
_d_LocalObject = (
 omniORB.tcInternal.tv_local_interface, LocalObject._NP_RepositoryId, 'LocalObject')
...
class ValueBase(object):
    __doc__ = ' CORBA::ValueBase base class '
    _NP_RepositoryId = 'IDL:omg.org/CORBA/ValueBase:1.0'
    _NP_ClassName = None
    _ValueBase__in_repr = 0
    ...
    def __repr__(self):
        if self._ValueBase__in_repr:
            return '...'
        self._ValueBase__in_repr = 1
        try:
            cname = self._NP_ClassName
            if cname is None:
                cname = '%s.%s' % (self.__module__, self.__class__.__name__)
...
_d_ValueBase = (
 omniORB.tcInternal.tv_value, ValueBase,
...
class Context(Object):
    ...
    def __init__(self, name, parent, values=None):
        self._Context__name = name
        self._Context__parent = parent
        if values:
            self._Context__values = values
        else:
            self._Context__values = {}

    def set_one_value(self, name, val):
        if not (isinstance(name, str) and isinstance(val, str)):
            raise BAD_PARAM(omniORB.BAD_PARAM_WrongPythonType, COMPLETED_NO)
        self._Context__values[name] = val
    ...
    def set_values(self, values):
        ...
        self._Context__values.update(values)
    ...
    def get_values(self, pattern, start_scope=None):
        if not isinstance(pattern, str):
            ...
            ctxt = self
            if start_scope:
                while ctxt and ctxt._Context__name != start_scope:
                    ctxt = ctxt._Context__parent

                if ctxt is None:
                    raise BAD_CONTEXT(omniORB.BAD_CONTEXT_StartingScopeNotFound, COMPLETED_NO)
            r = ctxt._get_values([pattern])
            if r == {}:
                raise BAD_CONTEXT(omniORB.BAD_CONTEXT_NoMatchingProperty, COMPLETED_NO)
        return r
    ...
    def delete_values(self, pattern):
        ...
        try:
            if pattern[(-1)] == '*':
                pattern = pattern[:-1]
                pl = len(pattern)
                for k in self._Context__values.keys():
                    if k[:pl] == pattern:
                        found = 1
                        del self._Context__values[k]

            else:
                del self._Context__values[pattern]
                found = 1
        except KeyError:
            pass
    ...
    def _get_values(self, patterns, values=None):
        ...
        for pattern in patterns:
            if pattern[(-1)] == '*':
                pattern = pattern[:-1]
                pl = len(pattern)
                for k, v in self._Context__values.items():
                    if k[:pl] == pattern and k not in values:
                        values[k] = v

            else:
                if pattern not in values:
                    v = self._Context__values.get(pattern)
                    if v:
                        values[pattern] = v

        if self._Context__parent:
            self._Context__parent._get_values(patterns, values)
        return values
...
_d_completion_status = (
 tcInternal.tv_enum, completion_status._NP_RepositoryId, 'completion_status', completion_status._items)
...
_d_PolicyError = (
 tcInternal.tv_except, PolicyError, PolicyError._NP_RepositoryId, 'PolicyError', 'reason', omniORB.typeMapping['IDL:omg.org/CORBA/PolicyErrorCode:1.0'])
