...
sys.modules."""
    if not os.path.isfile(idlname):
        ...
        pipe = os.popen('omniidl -q -bpython -Wbstdout ' + inline_str + argstr + ' ' + idlname)
        try:
            ...
            for mod in mods:
                for m in (mod, skeletonModuleName(mod)):
                    if m in _partialModules:
                        if m in sys.modules:
                            sys.modules[m].__dict__.update(_partialModules[m].__dict__)
                        else:
...
CORBA.BAD_INV_ORDER if the ORB has not been initialised."""
    if orb is None:
        raise CORBA.BAD_INV_ORDER(BAD_INV_ORDER_ORBNotInitialised, CORBA.COMPLETED_NO)
...
class StructBase(object):
    ...
    def _tuple(self):
        desc = findType(self._NP_RepositoryId)
        if desc is None:
            raise CORBA.BAD_PARAM(BAD_PARAM_IncompletePythonType, CORBA.COMPLETED_NO)
        vals = []
        for i in range(4, len(desc), 2):
            attr = desc[i]
    ...
    def __getattr__(self, mem):
        try:
            ...
            if mem == self._def_m or mem in self._m_to_d:
                raise CORBA.BAD_PARAM(BAD_PARAM_WrongUnionMemberSelected, CORBA.COMPLETED_NO)
            else:
    ...
    if fd == td:
        ...
        try:
            if fd == td:
                ...
                if fd[0] == tcInternal.tv_objref:
                    return _omnipy.narrow(v, td[1])
                else:
                    if fd[0] == tcInternal.tv_struct:
                        l = list(v._values)
                        ...
                        if fd[0] == tcInternal.tv_union:
                            return td[1](v._d, coerceAny(v._v, fd[6][v._d], td[6][v._d]))
                        else:
                            if fd[0] == tcInternal.tv_enum:
                                return td[3][v._v]
                            if fd[0] == tcInternal.tv_sequence:
                                l = v[:]
                                for i in range(len(l)):
                                    l[i] = coerceAny(v[i], fd[1], td[1])

                                return l
                        if fd[0] == tcInternal.tv_array:
                            l = v[:]
                            for i in range(len(l)):
                                l[i] = coerceAny(v[i], fd[1], td[1])

                            return l
                    if fd[0] == tcInternal.tv_except:
                        l = list(v._values)
                        for i in range(len(l)):
                            l[i] = coerceAny(l[i], fd[(i * 2 + 5)], td[(i * 2 + 5)])

                        return (td[1])(*l)
                if fd[0] == tcInternal.tv__indirect:
