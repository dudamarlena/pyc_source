...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/decocare/stick.py
# Compiled at: 2016-03-06 17:04:33
import lib, logging, time
...
class UnresponsiveError(StickError):
    pass
...
class StickCommand(object):
  ...
  """
    ...
    def format_cl2(self, msg, a2=0, a3=0):
...
class ProductInfo(StickCommand):
  ...
  """
    ...
    rf_table = {1: '868.35Mhz', 0: '916.5Mhz', 
       255: '916.5Mhz'}
    iface_key = {3: 'USB', 1: 'Paradigm RF'}
    ...
    def decode(klass, data):
        return {'rf.freq': klass.rf_table.get(data[5], 'UNKNOWN'), 
...
class LinkStatus(StickCommand):
  ...
  """
    ...
    def record_error(self, result):
        ...
        if self.ack == 0 and self.status & 1 > 0:
            self.error = False
...
class ReadRadio(StickCommand):
  ...
  """
    ...
    """
        lb, hb = raw[5] & 127, raw[6]
        self.eod = raw[5] & 128 > 0
...
class Stick(object):
  ...
  * download - loop
  ...
  """
    ...
    """
        msg = (':').join(['PROCESS', 'START'] + map(str, [self.timer.millis(), self.command]))
        log.info(msg)
        ...
        msg = (':').join(['PROCESS', 'END'] + map(str, [self.timer.millis(), self.command]))
        log.info(msg)
        return info
    ...
    """
        ...
        while not eod:
            ...
            data = bytearray()
            if size is None:
                log.info('%s:begin first poll first sleep .250' % stats.format(self, i, 0, len(results), len(data)))
                time.sleep(0.25)
                size = self.poll_size()
                log.info('%s:end first poll' % stats.format(self, i, size, len(results), len(data)))
            if size == 0:
                if i % 3 == 0:
                    time.sleep(1.5)
                size = self.poll_size()
            if size == 0 and i > 1:
                log.warn('%s:BAD AILING' % stats.format(self, i, size, len(results), len(data)))
                ailing = ailing + 1
                if ailing > 1:
                    break
                continue
                ...
                ailing = ailing - 1
            log.info('%s:proceed to download packet' % stats.format(self, i, size, len(results), len(data)))
            data = self.download_packet(size)
            expect_eod = False
            if data:
                results.extend(data)
                expect_eod = self.command.eod
                log.info('%s:adding segment' % stats.format(self, i, size, len(results), len(data)))
            else:
                log.info('%s:no data, try again sleep .400' % stats.format(self, i, size, len(results), len(data)))
                time.sleep(0.4)
            eod = expect_eod
            if not eod:
                log.info('%s:no eod, sleep .200 try again' % stats.format(self, i, size, len(results), len(data)))
                time.sleep(0.2)
                size = self.poll_size()

        log.info('%s:DONE' % stats.format(self, i, size, len(results), len(data)))
        self._download_i = False
        return results
    ...
    """
        ...
        for attempt in xrange(3):
            ...
            if size == 0:
                break
            seg_stats = (
             len(segments), sum(map(len, segments)), len(raw))
            log_detail = segs_vs_raw.format(*seg_stats)
