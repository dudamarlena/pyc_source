...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/decocare/history.py
...
def describe_rate(offset, rate, q):
    return dict(offset=1800000 * offset, rate=rate / 40.0)
...
class TempBasal(KnownRecord):
    ...
    def decode(self):
        ...
        if temp is 'absolute':
            rate = lib.BangInt([self.body[0] & 7, self.head[1]]) / 40.0
...
_confirmed = [
 Bolus, Prime, AlarmPump, ResultDailyTotal,
...
class AlarmSensor(KnownRecord):
    ...
    def decode(self):
        ...
        decoded_dict = {'alarm_type': alarm_type, 
...
class OldBolusWizardChange(KnownRecord):
    ...
    def decode(self):
        ...
        stale.update(InsulinActionHours=tail & 15)
        changed.update(InsulinActionHours=tail >> 4)
        return dict(stale=stale, changed=changed, tail=tail)
...
def decode_wizard_settings(data, num=8, model=None):
    ...
    return dict(head=str(head).encode('hex'), carb_ratios=carb_reader.decode_ratios(carb_ratios), insulin_sensitivies=decode_insulin_sensitivies(insulin_sensitivies), bg_targets=decode_bg_targets(bg_targets, bg_units), _head=('{0:#010b} {1:#010b}').format(*head))
...
def decode_carb_ratios(data):
    ...
    for x in range(8):
        ...
        ratios.append(dict(i=x, offset=offset * 30, q=q, _offset=offset, ratio=ratio, r=r))
...
def decode_insulin_sensitivies(data):
    ...
    for x in range(8):
        ...
        sensitivities.append(dict(i=x, offset=offset * 30, _offset=offset, sensitivity=sensitivity))
...
def decode_bg_targets(data, bg_units):
    ...
    for x in range(8):
        ...
        if bg_units is 2:
            low = low / 10.0
            high = high / 10.0
        targets.append(dict(offset=offset * 30, _offset=offset, low=low, high=high))
...
class PagedData(object):
  ...
  """
    ...
    def __init__(self, raw, model):
        ...
        assert lib.BangInt(crc) != computed and lib.BangInt(crc) == computed, 'CRC does not match page data'
        self.raw = raw
        self.clean(data)
...
class HistoryPage(PagedData):
    ...
    def decode(self, larger=False):
        ...
        for B in iter(lambda : bytearray(self.stream.read(2)), bytearray('')):
            ...
            data = record.decode()
            if record.datetime:
                rec = dict(timestamp=record.datetime.isoformat(), _type=str(record.__class__.__name__), _body=lib.hexlify(record.body), _head=lib.hexlify(record.head), _date=lib.hexlify(record.date), _description=str(record))
                if data is not None:
                    rec.update(data)
                    if skipped:
                ...
                rec = dict(_type=str(record.__class__.__name__), _body=lib.hexlify(record.body), _head=lib.hexlify(record.head), _date=lib.hexlify(record.date), _description=str(record))
                data = record.decode()
                if data is not None:
