...
# Size of source mod 2**32: 29349 bytes
"""
@author: 杨瑞杰
...
from albumentations import Compose, VerticalFlip, RandomRotate90, OneOf, ElasticTransform, GridDistortion, JpegCompression, CLAHE, OpticalDistortion, RandomBrightnessContrast, RandomGamma, RandomCrop, Resize
...
def get_image_from_csv(csv_path, is_aug=False, input_bands=3, output_bands=1, image_size=None, generate=False, batch_size=None):
    ...
    def read_xy(x_file, y_file):
        x_image, y = rasterio.open(x_file).read()[:input_bands, ...], np.array(Image.open(y_file)) if y_file.strip().endswith('png') else rasterio.open(y_file).read(1)
...
def get_scene_image_from_csv(csv_path, label_dict, input_bands=3, is_aug=False, image_size=None, generate=False, batch_size=None):
    ...
    def read_xy(x_file, label_id):
        x_image = rasterio.open(x_file).read()[:input_bands, ...]
        x_image = np.transpose(x_image, (1, 2, 0))
        if is_aug:
            aug_list = [
             VerticalFlip(p=0.5),
