...
from iobjectspy.ml.toolkit._create_training_data_util import get_image_transf
...
from toolkit._toolkit import _is_image_file
...
class DataPreparation:
    ...
    def create_training_data(input_data, input_label, label_class_field, output_path, output_name, training_data_format, tile_format='jpg', tile_size_x=1024, tile_size_y=1024, tile_offset_x=512, tile_offset_y=512, tile_start_index=0, save_nolabel_tiles=False, **kwargs):
        ...
        :type input_label: DatasetVector
        ...
        if isinstance(input_data, str) and not os.path.exists(input_data):
            ...
            if _is_image_file(input_data):
                raise Exception('input_data must be str ')
            elif isinstance(input_label, str) and not os.path.exists(input_label):
                raise Exception('input_label does not exist ')
            else:
                if not isinstance(input_label, Dataset):
                    raise TypeError('input_label must be DatasetVector ')
                    ...
                    transf = get_image_transf(input_data)
                    if transf.e == 1.0:
                        raise TypeError('Images must have geographic coordinates')
                    ...
                    if tile_start_index < -1:
                        ...
                        if os.path.exists(outpath) & (tile_start_index != -1):
                            ...
                            if training_data_format == 'VOC':
                                create_voc_data(input_data, input_label, label_class_field, output_path, output_name, training_data_format, tile_format, tile_size_x, tile_size_y, tile_offset_x, tile_offset_y, tile_start_index, save_nolabel_tiles)
                        elif training_data_format == 'VOC_MASK':
                            create_voc_mask_data(input_data, input_label, label_class_field, output_path, output_name, training_data_format, tile_format, tile_size_x, tile_size_y, tile_offset_x, tile_offset_y, tile_start_index, save_nolabel_tiles)
                        else:
                            if training_data_format == 'MULTI_C':
                                create_multi_classification_data(input_data, input_label, label_class_field, output_path, output_name, 
                                ...
                                if training_data_format == 'BINARY_C':
                                    create_binary_classification_data(input_data, input_label, label_class_field, output_path, output_name, training_data_format, tile_format, tile_size_x, tile_size_y, tile_offset_x, tile_offset_y, tile_start_index, save_nolabel_tiles)
                                else:
                                    if training_data_format == 'SCENE_C':
                                        create_scene_classification_data(input_data, input_label, label_class_field, output_path, output_name, training_data_format, tile_format, tile_size_x, tile_size_y, tile_offset_x, tile_offset_y, tile_start_index, save_nolabel_tiles)
                                    else:
