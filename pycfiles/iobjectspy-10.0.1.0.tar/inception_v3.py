...
# [GCC 8.4.0]
# Embedded file name: D:\BuildAgent\work\test/iobjectspy/ml\vision\_models\semantic_seg\_seg_models\backbones\inception_v3.py
...
def conv2d_bn(x, filters, num_row, num_col, padding='same', strides=(1, 1), name=None):
    ...
    if K.image_data_format() == 'channels_first':
        ...
        bn_axis = 3
    x = Conv2D(filters,
      (num_row, num_col), strides=strides,
      padding=padding,
      use_bias=False,
      name=conv_name)(x)
        ...
        if not os.path.exists(weights):
            ...
            if include_top:
                ...
                branch3x3dbl = conv2d_bn(branch3x3dbl,
                  96, 3, 3, strides=(2, 2), padding='same')
                ...
                branch3x3 = conv2d_bn(branch3x3, 320, 3, 3, strides=(2, 2),
                  padding='same')
                branch7x7x3 = conv2d_bn(x, 192, 1, 1)
                branch7x7x3 = conv2d_bn(branch7x7x3, 192, 1, 7)
                branch7x7x3 = conv2d_bn(branch7x7x3, 192, 7, 1)
                branch7x7x3 = conv2d_bn(branch7x7x3,
                  192, 3, 3, strides=(2, 2), padding='same')
                branch_pool = MaxPooling2D((3, 3), strides=(2, 2), padding='same')(x)
                x = layers.concatenate([
