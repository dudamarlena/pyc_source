...
# [GCC 8.4.0]
# Embedded file name: D:\BuildAgent\work\test/iobjectspy/ml\vision\_models\semantic_seg\_seg_models\unet\blocks.py
...
def Upsample2D_block(filters, stage, kernel_size=(3, 3), upsample_rate=(2, 2), use_batchnorm=False, skip=None):
    ...
    def layer(input_tensor):
        ...
        x = ConvRelu(filters, kernel_size, use_batchnorm=use_batchnorm, conv_name=(conv_name + '1'),
          bn_name=(bn_name + '1'),
          relu_name=(relu_name + '1'))(x)
        x = ConvRelu(filters, kernel_size, use_batchnorm=use_batchnorm, conv_name=(conv_name + '2'),
          bn_name=(bn_name + '2'),
...
def Transpose2D_block(filters, stage, kernel_size=(3, 3), upsample_rate=(2, 2), transpose_kernel_size=(4, 4), use_batchnorm=False, skip=None):
    ...
    def layer(input_tensor):
        ...
        x = Conv2DTranspose(filters, transpose_kernel_size, strides=upsample_rate, padding='same',
          name=up_name,
          use_bias=(not use_batchnorm))(input_tensor)
        if use_batchnorm:
