...
def resnet_arg_scope(is_training=True, batch_norm_decay=0.997, batch_norm_epsilon=1e-05, batch_norm_scale=True):
...
class resnetv1(Network):
    ...
    def _head_to_tail(self, pool5, is_training, reuse=None):
        with slim.arg_scope(resnet_arg_scope(is_training=is_training)):
            fc7, _ = resnet_v1.resnet_v1(pool5, (self._blocks[-1:]),
              global_pool=False,
              include_root_block=False,
              reuse=reuse,
              scope=(self._scope))
    ...
    def fix_variables(self, sess, pretrained_model):
        ...
        with tf.variable_scope('Fix_Resnet_V1') as (scope):
            with tf.device('/cpu:0'):
                ...
                sess.run(tf.assign(self._variables_to_fix[(self._scope + '/conv1/weights:0')], tf.reverse(conv1_rgb, [2])))