...
# Size of source mod 2**32: 67270 bytes
...
__all__ = ['GeometriesRelation', 'aggregate_points_geo', 'can_contain', 'has_area_intersection', 'has_cross',
 'has_overlap', 'is_perpendicular',
 'has_touch', 'has_common_point', 'has_common_line', 'has_hollow', 'is_disjointed', 'is_identical',
 'is_within', 'is_left', 'is_right', 'is_on_same_side', 'is_parallel', 'is_point_on_line', 'has_intersection',
 'is_project_on_line_segment', 'nearest_point_to_vertex', 'clip', 'erase', 'identity', 'intersect',
 'intersect_line', 'intersect_polyline', 'union', 'update', 'xor', 'compute_concave_hull',
...
class GeometriesRelation(JVMBase):
    ...
    def intersect_extents(self, rc):
        ...
        if isinstance(rc, Rectangle):
            return self._jobject.intersectExtents(rc._jobject)
    ...
    def matches(self, data, mode, excludes=None):
        ...
        if not isinstance(data, (Point2D, Rectangle, Geometry)):
            raise ValueError('data required Point2D, Rectangle and Geometry, but now is ' + str(type(data)))
        if isinstance(data, Rectangle):
            data = data.to_region()
        if excludes is not None:
            return self._jobject.matches(data._jobject, spatial_mode._jobject, excludes)
        return self._jobject.matches(data._jobject, spatial_mode._jobject)
