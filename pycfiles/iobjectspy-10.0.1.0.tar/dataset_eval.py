...
# [GCC 8.4.0]
# Embedded file name: D:\BuildAgent\work\test/iobjectspy/ml\vision\_models\faster_rcnn\datasets\dataset_eval.py
# Compiled at: 2019-12-31 04:09:01
# Size of source mod 2**32: 2919 bytes
import os
from iobjectspy import Rectangle, intersect

def dataset_eval(inference_data, gt_data, classname, ovthresh=0.5, out_path=False):
    ...
    if recordset_inference.get_geometry() != None:
        for feature_inference in recordset_inference.get_features():
            ...
            tile_box = Rectangle(left, bottom, right, top)
            recordset_gt = gt_data.query_with_bounds(tile_box, cursor_type='STATIC')
            if recordset_gt.get_geometry() != None:
