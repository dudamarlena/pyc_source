...
from iobjectspy._version import get_version
...
def _get_jar_file():
    ...
    jar_file = '%s/jars/%s' % (currdir, jar_name)
    if not os.path.exists(jar_file):
        jar_name = 'iobjects-py4j.jar'
        return '%s/jars/%s' % (currdir, jar_name)
    return jar_file
...
class _JavaGatewayLaunchThread(threading.Thread):
    ...
    def run(self):
        ...
        print(comment)
        subprocess.Popen(comment, shell=True, start_new_session=True)
