...
# [GCC 8.4.0]
# Embedded file name: D:\BuildAgent\work\test/iobjectspy/ml\vision\_models\faster_rcnn\utils\freeze_model.py
...
def freeze_model(input_checkpoint, net, output_graph, num_category):
    ...
    if not os.path.isfile(input_checkpoint + '.meta'):
        raise IOError('{:s} not found.\nDid you download the proper networks from our server and place them properly?'.format(input_checkpoint + '.meta'))
    ...
    net.create_architecture('TEST', (num_category + 1), tag='default',
...
def export_tfserving_model(output_graph_temp, output_graph):
    ...
    with tf.Session(graph=detection_graph) as (sess):
        signature = predict_signature_def(inputs={'im_data':sess.graph.get_tensor_by_name('Placeholder:0'),  'im_info':sess.graph.get_tensor_by_name('Placeholder_1:0')},
          outputs={'score':sess.graph.get_tensor_by_name('resnet_v1_101_5/cls_score/BiasAdd:0'), 
         'prob':sess.graph.get_tensor_by_name('resnet_v1_101_5/cls_prob:0'), 
         'pred':sess.graph.get_tensor_by_name('resnet_v1_101_5/bbox_pred/BiasAdd:0'), 
