...
# [GCC 8.4.0]
# Embedded file name: D:\BuildAgent\work\test/iobjectspy/ml\vision\_models\faster_rcnn\nets\network.py
# Compiled at: 2019-12-31 04:09:02
# Size of source mod 2**32: 26067 bytes
...
import layer_utils.anchor_target_layer as anchor_target_layer
from layer_utils.proposal_layer import proposal_layer, proposal_layer_tf
import layer_utils.proposal_target_layer as proposal_target_layer
...
class Network(object):
    ...
    def _proposal_top_layer(self, rpn_cls_prob, rpn_bbox_pred, name):
        with tf.variable_scope(name) as (scope):
            if cfg.USE_E2E_TF:
                rois, rpn_scores = proposal_top_layer_tf(rpn_cls_prob, rpn_bbox_pred, self._im_info, self._feat_stride, self._anchors, self._num_anchors)
            else:
    ...
    def _roi_pool_layer(self, bootom, rois, name):
        with tf.variable_scope(name) as (scope):
            return tf.image.roi_pooling(bootom, rois, pooled_height=(cfg.POOLING_SIZE),
              pooled_width=(cfg.POOLING_SIZE),
              spatial_scale=0.0625)[0]
    ...
    def _add_losses(self, sigma_rpn=3.0):
        with tf.variable_scope('LOSS_' + self._tag) as (scope):
            ...
            rpn_loss_box = self._smooth_l1_loss(rpn_bbox_pred, rpn_bbox_targets, rpn_bbox_inside_weights, rpn_bbox_outside_weights,
    ...
    def val_loss(self, sess, blobs):
        feed_dict = {self._image: blobs['data'], self._im_info: blobs['im_info'], 
         self._gt_boxes: blobs['gt_boxes']}
        loss = sess.run((self._losses['total_loss']), feed_dict=feed_dict)
        return loss
