...
# [GCC 8.4.0]
# Embedded file name: /home/romain/dev/buildouts/xnet4.1/src/atreal.mailservices/atreal/mailservices/browser/views.py
...
class MailServicesView(BrowserView):
        ...
        if results is None:
            ...
            for result in results:
                if len(result) > 2:
                    ...
                    if flag == True:
                        info.append(dict(id=result.id, title=result.title, recipients=recipients))
    ...
    def _principal_search_results(self, search_for_principal, get_principal_by_id, get_principal_title, principal_type, id_key):
        ...
        if not search_term:
            ...
            for principal_info in search_for_principal(hunter, search_term):
                ...
                if principal is None:
                    continue
                info.append(dict(id=principal_id, title=get_principal_title(principal, principal_id), recipients=recipients))
    ...
    def user_search_results(self):
        ...
        def search_for_principal(hunter, search_term):
            if self.request.form.get('type', None) == 'searchalluser':
                return merge_search_results(chain(*[ hunter.searchUsers() for field in [
                 'login', 'fullname']
                                                   ]), 'userid')
            else:
                return merge_search_results(chain(*[ hunter.searchUsers(**{field: search_term}) for field in [
                 'login', 'fullname']
                                                   ]), 'userid')
                return
        ...
        return self._principal_search_results(search_for_principal, get_principal_by_id, get_principal_title, 'user', 'userid')
    ...
    def group_search_results(self):
        ...
        return self._principal_search_results(search_for_principal, get_principal_by_id, get_principal_title, 'group', 'groupid')
