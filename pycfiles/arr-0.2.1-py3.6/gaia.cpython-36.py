...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/arr/gaia.py
# Compiled at: 2019-09-27 02:18:03
...
class Gaia:
    ...
    def __init__(self, coordinate: SkyCoord):
        ...
        self.coord = SkyCoord(ra.nominal_value * u.deg, dec.nominal_value * u.deg)
        ...
        t_err = max(abs(float(query['teff_val']) - float(query['teff_percentile_lower'])), abs(float(query['teff_val']) - float(query['teff_percentile_upper'])))
        self.t_eff = ufloat(float(query['teff_val']), t_err)
        r_err = max(abs(float(query['radius_val']) - float(query['radius_percentile_lower'])), abs(float(query['radius_val']) - float(query['radius_percentile_upper'])))
        self.radius = ufloat(float(query['radius_val']), r_err)
        l_err = max(abs(float(query['lum_val']) - float(query['lum_percentile_lower'])), abs(float(query['lum_val']) - float(query['lum_percentile_upper'])))
