...
def load_model(name_path):
    ...
    model = tf.keras.models.load_model(name_path,
      custom_objects=None,
      compile=True)
    return model
...
def generate_encapsulate_model(model, output_layer_names):
    ...
    display_model = tf.keras.models.Model(inputs=(model.input),
...
def save(enc_model, output_path):
    ...
    enc_model.compile(optimizer='adam', loss='sparse_categorical_crossentropy',
      metrics=[
     'accuracy'])
    tf.keras.models.save_model(enc_model,
      output_path,
      overwrite=True,
