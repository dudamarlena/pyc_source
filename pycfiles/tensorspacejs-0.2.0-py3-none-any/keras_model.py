...
# [GCC 8.4.0]
# Embedded file name: /Users/ss/tensorspace-converter/tensorspacejs/tf/keras_model.py
# Compiled at: 2019-03-26 17:33:39
"""
...
def load_model(name_path):
    ...
    model = tf.keras.models.load_model(name_path, custom_objects=None, compile=True)
    return model
...
def generate_encapsulate_model(model, output_layer_names):
    ...
    display_model = tf.keras.models.Model(inputs=model.input, outputs=list(map(lambda oln: model.get_layer(oln).output, layer_name_list)))
    return display_model


def save(enc_model, output_path):
    print 'Saving temp multi-output .h5 model...'
    enc_model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=[
     'accuracy'])
    tf.keras.models.save_model(enc_model, output_path, overwrite=True, include_optimizer=True)
