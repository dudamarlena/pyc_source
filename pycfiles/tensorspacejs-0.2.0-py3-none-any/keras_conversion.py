...
# [GCC 8.4.0]
# Embedded file name: ./src/krs/keras_conversion.py
# Compiled at: 2019-01-17 05:53:40
"""
...
def preprocess_keras_model(input_format, path_model, path_output_dir, output_node_names=None):
    if input_format == 'topology_weights_combined':
        preprocess_from_model(path_model, path_output_dir, output_node_names)
    elif input_format == 'topology_weights_separated':
        model_paths = path_model.split(',')
        preprocess_from_weights(model_paths[0], model_paths[1], path_output_dir, output_node_names)
...
def generate_encapsulate_model(model, output_layer_names=None):
    ...
    enc_model = Model(inputs=model.inputs, outputs=transfer_outputs)
    return enc_model
...
def convert_tfjs(path_output_dir):
    ...
    dispatch_keras_h5_to_tensorflowjs_conversion(path_output_dir + '/enc_model.h5', path_output_dir)
