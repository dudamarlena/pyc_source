...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/turtl/test/test_proxy.py
...
class TurtlProxyRequestTestCase(unittest.TestCase):
    ...
    def test_processWithData(self):
        ...
        to forward it.
        """
        return self._testProcess('/foo/bar', '/foo/bar', 'POST', 'Some content')
    ...
    def test_filtering(self):
        ...
        request = proxy.TurtlProxyRequest(channel, False, reactor)
        request.gotLength(0)
        request.requestReceived('GET', 'http://delicious.com:1234/foo/bar', 'HTTP/1.0')
        self.assertEquals(len(reactor.connect), 1)
