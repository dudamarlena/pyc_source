...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/turtl/proxy.py
...
class TurtlProxyRequest(proxy.ProxyRequest):
    ...
    def process(self):
        ...
        processed = self.channel.factory.process(host, port, clientFactory, priority, self.completed)
        if not processed:
