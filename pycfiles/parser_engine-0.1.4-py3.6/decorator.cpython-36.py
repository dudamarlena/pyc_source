...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/parser_engine/decorator.py
# Compiled at: 2019-04-16 05:55:04
# Size of source mod 2**32: 8596 bytes
import copy, six
...
class Template(object):
    ...
    def get_rule_by_id(cls, tpl_id, **kwargs):
        ...
        return Rule((LinkExtractor()), callback=parse_with_tpl_id,
          cb_kwargs=kwargs)
    ...
    def get_rule(cls, tpl, **kwargs):
        ...
        kwargs['tpl'] = tpl
        return Rule((tpl.get_link_extractor()), callback=parse_with_tpl,
          cb_kwargs=kwargs)
    ...
    def PageTemplate(cls, **kw):
        ...
        def _deco(spcls):
            ...
            if tpls:
                if issubclass(spcls, CrawlSpider):
                    spcls.rules = (cls.get_rules)(tpls, **kw)
                    if not cls.tpl_src:
                        if not customize_link_extractor:
