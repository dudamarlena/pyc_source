...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/parser_engine/spider.py
# Compiled at: 2019-04-16 05:55:04
# Size of source mod 2**32: 8028 bytes
import copy, six, simplejson as json, logging
...
class PERule(Rule):
    ...
    def __init__(self, template, link_extractor=default_link_extractor, callback=None, cb_kwargs=None, follow=None, process_links=None, process_request=identity):
        """
...
class PESpider(RedisCrawlSpider):
    ...
    def make_request_from_data(self, data):
        try:
            scheduled = TaskRequest(**json.loads(data.decode(self.redis_encoding)))
    ...
    def task_to_request(scheduled):
        return Request(url=(scheduled.url),
    ...
    def request_to_task(request):
        ...
        return TaskRequest(url=(request.url), method=(request.method), body=(request.body),
    ...
    def reroute_request(self, request):
        ...
        :return:
        """
        self.route('%s:%s:start_urls' % (request.meta.get('project', self.project), request.meta.get('spider', self.name)), self.request_to_task(request))
