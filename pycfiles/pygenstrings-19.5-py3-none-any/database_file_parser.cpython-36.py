...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/pygenprop/database_file_parser.py
...
def parse_genome_property(genome_property_record):
        ...
        if marker == 'RN':
            ...
            if marker == 'DC':
                ...
                if marker == '--':
                    ...
                    if marker in core_genome_property_markers:
                        if marker == 'TH':
                            content = int(content)
    ...
    if step_index:
        ...
        steps = []
    new_genome_property = GenomeProperty(accession_id=(gathered_core_genome_property_markers.get('AC')), name=(gathered_core_genome_property_markers.get('DE')),
    ...
    for marker, content in genome_property_record:
        if marker in database_reference_markers:
            if marker in current_database_reference:
                database_references.append(DatabaseReference(record_title=(current_database_reference.get('DC')), database_name=(current_database_reference.get('DN')),
    ...
    for marker, content in genome_property_record:
        if marker in literature_reference_markers:
            if marker in current_literature_reference:
                literature_references.append(LiteratureReference(number=(current_literature_reference.get('RN')), pubmed_id=(current_literature_reference.get('RM')),
    ...
    for marker, content in genome_property_record:
        if marker in functional_element_markers:
            if marker in current_functional_element:
                ...
                current_evidence = []
                functional_elements.append(FunctionalElement(identifier=(current_functional_element.get('ID')), name=(current_functional_element.get('DN')),
...
def parse_single_evidence(current_evidence_dictionary):
    ...
    new_evidence = Evidence(evidence_identifiers=evidence_identifiers, gene_ontology_terms=gene_ontology_identifiers,
