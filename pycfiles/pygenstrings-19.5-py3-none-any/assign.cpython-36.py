...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/pygenprop/assign.py
# Compiled at: 2020-02-12 23:26:22
# Size of source mod 2**32: 24154 bytes
...
class AssignmentCache(object):
    ...
    def bootstrap_missing_step_assignments(self, properties_tree: GenomePropertiesTree):
        ...
        for property_identifier in self.property_identifiers:
            ...
            if len(property_steps_from_tree) != len(property_steps_from_cache):
                ...
                missing_property_steps = [step for step in property_steps_from_tree if step.number in missing_step_numbers]
    ...
    def bootstrap_assignments_from_genome_property(self, genome_property: GenomeProperty):
        ...
        if cached_property_assignment:
            ...
            if required_steps:
                ...
                required_step_values = [step_value for step_number, step_value in current_step_assignments.items() if step_number in required_step_numbers]
                genome_property_assignment = calculate_property_assignment_from_required_steps(required_step_values, genome_property.threshold)
            else:
                genome_property_assignment = calculate_property_assignment_from_all_steps(list(current_step_assignments.values()))
    ...
    def bootstrap_assignments_from_step(self, step: Step):
        ...
        if cached_step_assignment:
            ...
            step_assignment = calculate_step_or_functional_element_assignment(functional_element_assignments, sufficient_scheme=True)
    ...
    def bootstrap_assignments_from_functional_element(self, functional_element: FunctionalElement):
        ...
        sufficient_evidence_assignments = [assignment for assignment, sufficient in evidence_assignments_and_sufficients if sufficient]
        if sufficient_evidence_assignments:
            sufficient_assignment = calculate_step_or_functional_element_assignment(sufficient_evidence_assignments, sufficient_scheme=True)
    ...
    def create_results_tables(self, properties_tree: GenomePropertiesTree):
        ...
        property_table = pd.DataFrame.from_dict((self.property_assignments), orient='index',
          columns=['Property_Result'])
        property_table.index.names = ['Property_Identifier']
        step_table = pd.DataFrame((self.create_step_table_rows(self.step_assignments)), columns=[
         'Property_Identifier', 'Step_Number', 'Step_Result'])
        step_table.set_index(['Property_Identifier', 'Step_Number'], inplace=True)
        return (
         property_table, step_table)
    ...
    def create_step_table_rows(step_assignments):
        ...
        for genome_property_id, step in step_assignments.items():
            for step_number, step_result in step.items():
                yield (
                 genome_property_id, step_number, step_result)
...
class AssignmentCacheWithMatches(AssignmentCache):
    ...
    def __init__(self, match_info_frame=None, sequence_frame=None, sample_name=None):
        ...
        if match_info_frame is None or sequence_frame is None:
            ...
            self.matches = match_info_frame.merge(sequence_frame, left_on='Protein_Accession', right_on='Protein_Accession',
