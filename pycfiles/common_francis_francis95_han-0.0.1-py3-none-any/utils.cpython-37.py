...
# Embedded file name: /home/marc/Git/common-framework/common/utils.py
# Compiled at: 2019-08-13 10:19:34
# Size of source mod 2**32: 48639 bytes
import abc, collections, inspect, json, logging, mimetypes, os, re, sys, threading
...
DMY_DATE_REGEX = re.compile('^(\\d{2})[^\\d]?(\\d{2})[^\\d]?(\\d{2,4})([^\\d]?(\\d{2})[^\\d]?(\\d{2})[^\\d]?(\\d{2}))?$')
    ...
    if not _date:
        ...
        if dmy:
            ...
            if match:
                ...
                _date = (date_format.format)(*[group or  for group in match.groups()])
...
class TemporaryFileHandler(TemporaryFileUploadHandler):
    ...
    def new_file(self, file_name, *args, **kwargs):
        ...
        self.file = TemporaryFile((self.file_name),
          (self.content_type), 0, (self.charset), (self.content_type_extra), folder=(self.folder))
...
def download_file(function):
    ...
    def wrapper(*args, **kwargs):
        ...
        if isinstance(file, DownloadFile):
            ...
            mimetype, charset = mimetype or , charset or 
...
FORMAT_TYPES = {'application/json':lambda response: json_encode(response), 
 'text/json':lambda response: json_encode(response)}
try:
    import yaml
    FORMAT_TYPES.update({'application/yaml':yaml.dump, 
     'text/yaml':yaml.dump})
except ImportError:
    pass
    ...
    def wrapper(request, *args, **kwargs):
        ...
        if not isinstance(response, HttpResponse):
            if hasattr(settings, 'FORMAT_TYPES'):
                ...
                if isinstance(format_type_handler, str):
...
SAFE_GLOBALS = dict(__builtins__=dict(abs=abs,
  all=all,
  any=any,
  ascii=ascii,
  bin=bin,
...
def evaluate(expression, _globals=None, _locals=None, default=False):
    ...
    if _globals is None:
        ...
        default or _globals.update(SAFE_GLOBALS)
...
def recursive_dict_product(input_dict, all_keys=None, long_keys=False, separator='_', ignore='*', auto_id='id', prefix=''):
    """
    ...
    if auto_id:
        if prefix is not None:
            input_dict[auto_id] = auto_id not in input_dict or input_dict[auto_id] or 
    ...
    for nested_combos in product(*nested.values()):
        ...
        for nested_key, nested_value in zip(nested, nested_combos):
            if isinstance(nested_value, dict):
                results = [dict(r, **result) for result in recursive_dict_product(nested_value, all_keys, long_keys, separator, ignore, auto_id, nested_key) for r in results]
...
def get_choices_fields(*included_apps):
    ...
    results = {}
    choices_fields = []
    included_apps = included_apps or 
    ...
    def choices_by_application():
        for app_label, choices in sorted(results.items()):
            yield (
             str(apps.get_app_config(app_label).verbose_name), tuple(ordered_choices(choices)))
...
def prefetch_metadata(model, lookup=None, name=None):
    ...
    for field in model._meta.private_fields:
        if field.related_model is MetaData:
            ...
            return [
             Prefetch(lookup, queryset=(MetaData.objects.select_valid()), to_attr=name)]

    return []


def get_prefetchs(parent, depth=1, height=1, foreign_keys=False, one_to_one=True, one_to_many=False, many_to_many=False, metadata=False, excludes=None, null=False, _model=None, _prefetch='', _level=1):
    """
    ...
    for field in model._meta.related_objects + model._meta.many_to_many:
        if field.name in excludes or field.related_model in excludes:
            continue
        if not field.one_to_one or one_to_one or :
            ...
            if model == parent or _level < depth:
                prefetchs = get_prefetchs(parent,
                  depth=depth,
        ...
        if height:
            if not field.many_to_many:
                for related in get_related((field.related_model),
                  excludes=excludes,
            ...
            if metadata:
                ...
                prefetchs or results.append(recursive_prefetch)

    return results


def get_related(model, dest=None, excludes=None, foreign_keys=True, one_to_one=False, null=False, height=1, _related='', _models=None, _level=0):
    """
    ...
    excludes = excludes or 
    results = []
    if dest or _level > height or :
        if model in _models:
            return results
    models = (_models or ) + [model]
    if _related and dest == model or :
        if _related:
            results.append(_related)
    if foreign_keys:
        for field in model._meta.fields:
            if isinstance(field, (ForeignKey, OneToOneField)):
                if field.name in excludes or field.remote_field and not field.related_model in excludes:
                    if not null:
                        if field.null:
                            continue
                related_path = '__'.join((_related, field.name)) if _related else field.name
                results += get_related((field.related_model),
                  dest=dest, excludes=excludes, height=height, null=null, _related=related_path,
                  _models=models,
                  _level=(_level + 1))
...
def str_to_bool(value):
    ...
    """
    TRUE_VALUES = {
     'true', 'yes', 'y', '1', _('vrai'), _('oui'), _('o'), _('v')}
...
REGEX_URL_PARAMS = re.compile('\\(\\?P<([\\w_]+)>[^\\)]+\\)')
...
def recursive_get_urls(module=None, namespaces=None, attributes=None, model=None, _namespace=None, _current='/'):
    ...
    try:
        ...
        patterns = module
        patterns = list(chain(*(getattr(module, attribute, []) for attribute in attributes))) or 
        if patterns:
    ...
    for pattern in patterns:
        try:
            namespace = _namespace or 
            if namespaces:
                if namespace not in namespaces:
                    continue
            else:
                url = _current + pattern.pattern.regex.pattern.strip('^$').replace('\\', '')
                url = re.sub(REGEX_URL_PARAMS, ':\\1:', url).replace('?', '')
                url = url.replace('(.+)', ':pk:')
                if getattr(pattern.pattern, 'name', None):
                    ...
                    if not model or model is current_model:
                        yield (
                         key, url)
                elif getattr(pattern, 'namespace', None):
                    if pattern.urlconf_module:
                        yield from recursive_get_urls((pattern.urlconf_module),
                          namespaces=namespaces, attributes=attributes, model=model, _namespace=(_namespace or ),
                          _current=url)
...
def process_file(file_path, sleep=5, extract_directory=None):
    ...
    extract_directory = extract_directory or 
    while True:
        try:
            chunk = None
...
def json_encode(data, cls=None, **options):
    return (json.dumps)(data, cls=cls or , **options)
...
def abort_sql(name, kill=False, using=None, timeout=None, state='active'):
    ...
    connection = connections[(using or )]
    assert connection.vendor == 'postgresql', _('Cette fonction ne peut être utilisée que sur PostgreSQL.')
    with connection.cursor() as (cursor):
        query = 'SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE application_name = %s' if kill else 'SELECT pg_cancel_backend(pid) FROM pg_stat_activity WHERE application_name = %s'
        params = [name]
        if timeout:
