...
# Embedded file name: /home/marc/Git/common-framework/common/admin.py
# Compiled at: 2019-07-04 06:11:19
# Size of source mod 2**32: 22811 bytes
...
from common.models import CommonModel, Entity, Global, GroupMetaData, History, HistoryField, MetaData, PerishableEntity, ServiceUsage, UserMetaData, Webhook
...
class CommonAdmin(admin.ModelAdmin):
    ...
    def get_model_perms(self, request):
        return {'add':self.has_add_permission(request), 
...
class GlobalAdmin(admin.ModelAdmin):
    ...
    def entity_url(self, obj):
        if obj.entity:
            try:
                pattern = 'admin:{app_label}_{model}_change'.format(app_label=(obj.content_type.app_label),
...
class MetaDataAdmin(admin.ModelAdmin):
    ...
    list_display = ('id', 'entity_url', 'content_type', 'key', 'value', 'creation_date',
...
def restore(modeladmin, request, queryset, all_fields=False):
    ...
    for history in queryset.order_by('-creation_date'):
        try:
            result = history.restore(current_user=(request.user),
              from_admin=True,
    ...
    if fail > 0:
        messages.warning(request, _("{} élément(s) n'ont pas pu être restaurés car leurs relations sont manquantes !").format(fail))
...
class HistoryAdmin(admin.ModelAdmin):
    """HistoryAdmin"""
    date_hierarchy = 'creation_date'
    readonly_fields = ('user', 'status', 'object_str', 'content_type', 'object_id',
                       'object_uid', 'admin', 'reason', 'data', 'collector_update',
                       'collector_delete')
    list_display = ('id', 'creation_date', 'user', 'status', 'entity_url', 'content_type',
                    'object_id', 'data_size', 'restoration_date', 'restored', 'admin',
                    'has_reason', 'fields_count')
    list_display_links = ('id', )
    list_filter = ('creation_date', 'user', 'status', 'content_type', 'restoration_date',
                   'restored', 'admin')
    list_select_related = True
    ...
    def fields_count(self, obj):
        if obj.fields_count:
            try:
                pattern = 'admin:{app_label}_{model}_changelist'.format(app_label=(HistoryField._meta.app_label),
                  model=(HistoryField._meta.model_name))
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related('content_type', 'user').prefetch_related('entity').annotate(fields_count=(Count('fields'))).order_by('-creation_date')
...
class HistoryFieldAdmin(admin.ModelAdmin):
    """HistoryFieldAdmin"""
    date_hierarchy = 'creation_date'
    readonly_fields = ('history', 'field_name', 'old_value', 'new_value', 'status_m2m',
                       'editable', 'data')
    list_display = ('id', 'creation_date', 'history_url', 'field', 'editable', 'old_inner_value',
                    'new_inner_value', 'data_size', 'status_m2m', 'restoration_date',
                    'restored')
    list_display_links = ('id', )
    list_filter = ('creation_date', 'history__user', 'history__content_type', 'editable',
                   'restoration_date', 'restored', 'status_m2m')
    list_select_related = True
    ordering = ('-creation_date', )
    ...
    def field(self, obj):
        label = getattr(obj.field, 'verbose_name', '') or 
...
class WebhookAdmin(admin.ModelAdmin):
    ...
    list_filter = ('method', 'format', 'is_create', 'is_update', 'is_delete', 'is_restore',
                   'is_m2m')
    ordering = ('name', )
    search_fields = ('name', )
    fieldsets = (
    ...
    raw_id_fields = ('types', )
    autocomplete_lookup_fields = {'m2m': ('types', )}
...
def create_admin(*args, **kwargs):
    ...
    for model in args:
        if not model:
            continue
        admin_superclass = PerishableEntityAdmin if issubclass(model, PerishableEntity) else EntityAdmin if issubclass(model, Entity) else CommonAdmin
        fk_fields = tuple((field for field in model._meta.get_fields() if isinstance(field, (models.ForeignKey, models.OneToOneField))))
        m2m_fields = tuple((field for field in model._meta.get_fields() if isinstance(field, models.ManyToManyField)))
        properties = dict(list_display=(tuple((field.name for field in model._meta.concrete_fields if not field.primary_key if field.editable if not isinstance(field, (
         models.TextField, JsonField, PickleField))))),
          list_filter=(tuple((field.name for field in model._meta.get_fields() if getattr(field, 'choices', None) or isinstance(field, (
         models.BooleanField, models.NullBooleanField, models.DateField, models.DateTimeField))))),
          search_fields=(tuple((field.name for field in model._meta.get_fields() if not getattr(field, 'choices', None) if isinstance(field, (
        ...
        if grappelli:
            ...
            properties.update(autocomplete_lookup_fields=dict(fk=fk_fields, m2m=m2m_fields),
        ...
        admins.append(admin.register(model)(type('{}GenericAdmin'.format(model._meta.object_name), (admin_superclass,), properties)))
