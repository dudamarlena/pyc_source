...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tax2vec/models.py
# Compiled at: 2019-06-14 15:21:13
...
def simple_dnn(train_x, test_x, train_y, test_y, dnn_setting='1000,500,100', batch_size=10, epochs=10):
    ...
    for j in range(epochs):
        model.fit_generator(generator=batchgen, use_multiprocessing=False,
          workers=1,
          verbose=1,
          steps_per_epoch=1)
...
def SRNA(train_features, test_features, train_y, test_y, maxlen=0, epoch_num=5):
    ...
    callbacks = []
    print(model.summary())
    model.fit([x_train, semantic_train], labels_train, batch_size=batch_size,
      epochs=epochs)
