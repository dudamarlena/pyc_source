...
# [GCC 8.4.0]
# Embedded file name: /home/blazs/tax2vec/tax2vec/preprocessing.py
# Compiled at: 2019-06-14 14:03:02
# Size of source mod 2**32: 14745 bytes
...
def remove_stopwords_and_articles(text):
    ...
    stops.add('less')
    text = text.split()
    text = [x.lower() for x in text if x.lower() not in stops if len(x) > 2]
    return ' '.join(text)


def remove_mentions(text, replace_token):
    return re.sub('(?:@[\\w_]+)', replace_token, text)


def remove_hashtags(text, replace_token):
    return re.sub("(?:\\#+[\\w_]+[\\w\\'_\\-]*[\\w_]+)", replace_token, text)
    ...
    if simple_clean:
        ...
        if mode == 'matrix_word':
            ...
            if mode == 'matrix_pan':
                ...
                character_vectorizer = CountVectorizer(analyzer='char_wb', ngram_range=(4,
                                                                                        4), lowercase=False, min_df=4, max_df=0.8)
