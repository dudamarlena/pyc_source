...
# [GCC 8.4.0]
# Embedded file name: D:\Projects\usersapi\midaxusersutils\migrations\versions\427f14007618_.py
# Compiled at: 2018-11-21 12:24:27
...
def upgrade():
    trigger_users = sa.DDL('create or replace trigger USERS_id_trigger before insert on USERS for each row WHEN (new.id IS NULL) begin SELECT USERS_ID_SEQ.nextval INTO :new.id from dual; end; ')
    tusers_callable = trigger_users.execute_if(dialect='oracle')
    trigger_logins = sa.DDL('create or replace trigger logins_id_trigger before insert on USER_LOGINS for each row WHEN (new.id IS NULL) begin SELECT LOGINS_ID_SEQ.nextval INTO :new.id from dual; end; ')
