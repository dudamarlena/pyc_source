...
# Embedded file name: D:\Projects\A_MSU_NASA\VS\pyiomica\pyiomica\visualizationFunctions.py
# Compiled at: 2020-02-26 12:08:49
# Size of source mod 2**32: 44357 bytes
__doc__ = 'Visualization functions'
import matplotlib
matplotlib.use('Agg')
...
from . import visibilityGraphAuxiliaryFunctions, clusteringFunctions, coreFunctions, extendedDataFrame, visibilityGraphCommunityDetection, utilityFunctions
...
def makeVisibilityGraph(intensities, positions, saveDir, fileName, communities=None, fontsize=16, nodesize=500, level=0.5, commLineWidth=3.0, lineWidth=2.0, layout='circle', horizontal=False, radius=0.03, figsize=(10, 10), addColorbar=True, colorbarAxisCoordinates=[0.9, 0.7, 0.02, 0.2], colorbarLabelsize=12, colorbarPrecision=2, extension='png', dpi=300):
    ...
    addVisibilityGraph(intensities, positions, fig=fig, fontsize=fontsize, nodesize=nodesize, level=level, commLineWidth=commLineWidth,
      lineWidth=lineWidth,
      withLabel=False,
      layout=layout,
      printCommunities=False,
      radius=radius,
            ...
            if i > j:
                if A[(i, j)] == 1:
                    if horizontal:
                        ...
                        level1 = data[i]
                        level2 = data[j]
                ax.annotate(s='', xy=(times[i], level1), xytext=(times[j], level2), arrowprops=dict(arrowstyle='<->', shrinkA=0, shrinkB=0, linestyle='--'))
...
def addColorbarToFigure(fig, data, axisCoordinates=[
 0.9, 0.7, 0.02, 0.2], cmap=None, norm=None, labelsize=12, precision=2):
            ...
            if i > j:
                if A[(i, j)] > 0:
                    if data[i] > 0 and data[j] > 0:
                        ...
                        level = 0
                    ax.annotate(s='', xy=(times[i], level), xytext=(times[j], level), arrowprops=dict(arrowstyle='->', shrinkA=0, shrinkB=0, linestyle='--', color='r'))
            if i > j and A[(i, j)] < 0:
                if data[i] > 0 and data[j] > 0:
                    level = np.min([data[i], data[j]])
                elif data[i] < 0 and data[j] < 0:
                    ...
                    level = 0
                ax.annotate(s='', xy=(times[i], level), xytext=(times[j], level), arrowprops=dict(arrowstyle='->', shrinkA=0, shrinkB=0, linestyle='--', color='b'))
...
def PlotNVGBarGraph_Dual(A, data, times, fileName, title='', fontsize=8, barwidth=0.05, figsize=(8, 4), dpi=600):
    ...
    for i in range(A.shape[0]):
        for j in range(A.shape[1]):
            if i > j:
                if A[(i, j)] > 0:
                    ax.annotate(s='', xy=(times[i], data[i]), xytext=(times[j], data[j]), arrowprops=dict(arrowstyle='->', shrinkA=0, shrinkB=0, linestyle='--', color='r'))
            if i > j and A[(i, j)] < 0:
                ax.annotate(s='', xy=(times[i], data[i]), xytext=(times[j], data[j]), arrowprops=dict(arrowstyle='->', shrinkA=0, shrinkB=0, linestyle='--', color='b'))
