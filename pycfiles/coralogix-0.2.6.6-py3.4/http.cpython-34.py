...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\coralogix\http.py
# Compiled at: 2016-09-20 19:24:20
...
class AuthRequest(Serializable):
    __doc__ = 'Sent to the Coralogix service to establish a new session.'
    __serializable__ = (('company_id', 'companyId'), ('private_key', None), ('private_key_str', 'privateKey'),
                        ('application_name', 'applicationName'), ('subsystem_name', 'subsystemName'),
                        ('sdk_version', 'version'), ('ip_address', 'IPAddress'),
                        ('computer_name', 'computerName'), ('process_name', 'processName'))
...
class Session(AuthRequest):
    __doc__ = 'Retains information about the connection with Coralogix.'
    __serializable__ = AuthRequest.__serializable__ + (('sdk_id', 'id'), ('queue', 'queue'),
                                                       ('token', 'token'), ('utc_timestamp', 'GMTTimestamp'))


class Log(Serializable):
    __doc__ = 'Wraps log entries for sending into a Coralogix queue.'
    __serializable__ = (('session', 'sdk'), ('serialized_entries', 'logEntries'))
...
class Client(object):
    __doc__ = 'HTTP client for the Coralogix Web service.'
    ...
    def authenticate(self, company_id, private_key, app_name, subsystem_name, sdk_version, ip_addr, computer_name, proc_name):
        ...
        if self.is_connected():
            return
        req = AuthRequest(company_id=company_id, private_key=str(private_key), application_name=app_name, subsystem_name=subsystem_name, sdk_version=sdk_version, ip_address=ip_addr, computer_name=computer_name, process_name=proc_name)
        try:
        ...
        assert self.session.queue.startswith(self._url)
