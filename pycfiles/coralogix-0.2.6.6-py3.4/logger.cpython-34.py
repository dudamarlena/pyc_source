...
class LogEntry(Serializable):
    __doc__ = 'Represents a single log entry.'
    AVERAGE_SIZE = 64
...
class CoralogixLogger(object):
        ...
        try:
            try:
                ...
                if self._elapsed_time % config.settings.RELOAD_CONFIG_INTERVAL < SECOND:
                    self._reload_config()
                if config.settings.IS_ACTIVE:
                    if self._elapsed_time % config.settings.BULK_TIME_IN_MILLI_THRESHOLD < SECOND:
                        if self._client.status != ConnectionStatus.NotInitiated:
    ...
    def _drop_records_if_needed(self):
        ...
        try:
            ...
            if self._client.is_connected():
                if self._timer.interval != config.settings.INTERVAL_IN_MILLI_FOR_TIMER_ON_DISCONNECTION:
    ...
    def _validate_log_timestamps(self, temp_buffer):
        ...
        try:
            for entry in temp_buffer:
                if self._last_entry_timestamp is not None:
                    if entry.timestamp <= self._last_entry_timestamp:
