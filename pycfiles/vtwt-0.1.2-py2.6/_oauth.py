...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/vtwt/_oauth.py
# Compiled at: 2010-07-12 20:31:43
import os, sys
...
class OAuther(cli.CommandBase):
    ...
    def buildConsumer(self):
        return oauth.OAuthConsumer(self.config['consumer-key'], self.config['consumer-secret'])

    @inlineCallbacks
    def getRequestToken(self):
        tokenReq = oauth.OAuthRequest.from_consumer_and_token(self.consumer, http_url=self.TOKEN_REQUEST_URL)
    ...
    def getAuthorizationURL(self):
        authReq = oauth.OAuthRequest.from_token_and_callback(token=self.requestToken, http_url=self.AUTHORIZATION_URL)
        authReq.sign_request(self.sig, self.consumer, self.requestToken)
        return authReq.to_url()

    @inlineCallbacks
    def getAccessToken(self, pin):
        accessReq = oauth.OAuthRequest.from_consumer_and_token(self.consumer, token=self.requestToken, verifier=pin, http_url=self.ACCESS_TOKEN_URL)
