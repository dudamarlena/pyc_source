...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/vtwt/watch.py
# Compiled at: 2010-07-12 22:42:07
import os, sys, time
...
class Watcher(cli.Command):
    ...
    def showTimeline(self):
        try:
            log.debug(('Requesting {0.config[watchee]} timeline since {0._lastPrintedId}').format(self))
    ...
    def _limitMessages(messages, limit=None):
        if limit and limit < len(messages):
            log.trace(('Limiting messages: {0}').format((', ').join(m.id for m in messages[:-limit])))
            del messages[:-limit]
        return messages
