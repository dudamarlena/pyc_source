...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/vtwt/grep.py
# Compiled at: 2010-07-11 21:03:10
...
class Grepper(cli.Command):
    ...
    def execute(self):
        try:
            ...
            printMatches = not (self.config['count'] or self.config['percentage'] or self.config['quiet'])
            page = 1
            maxId = None
            messages = None
    ...
    def _printSummary(self, matchCount, totalCount):
        self._print(self.config.summaryFormat.format(count=matchCount, total=totalCount, percentage=float(matchCount) / totalCount))
