...
# Embedded file name: build/bdist.linux-x86_64/egg/cbayes/distributions.py
# Compiled at: 2018-08-09 18:27:24
# Size of source mod 2**32: 13480 bytes
import numpy as np, scipy.stats as sstats
...
def supported_distributions(distname=None):
    ...
    if distname is not None:
        if distname.lower() in ('gaussian', 'gauss', 'normal', 'norm', 'n'):
            ...
            if distname.lower() in ('uniform', 'uni', 'u'):
                distname = 'uniform'
            else:
                if distname.lower() in ('chi2', 'c2', 'chisquared', 'chi_squared'):
                    distname = 'chi2'
                else:
                    if distname.lower() in ('gamma', 'g', 'gam', 'gm'):
                        distname = 'gamma'
                    else:
                        if distname.lower() in ('beta', 'bt', 'b'):
                            distname = 'beta'
        try:
            return D[distname]
        except KeyError:
