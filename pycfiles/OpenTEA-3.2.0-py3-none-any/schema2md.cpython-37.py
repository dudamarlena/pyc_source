...
# [GCC 8.4.0]
# Embedded file name: /Users/dauptain/GITLAB/opentea/src/opentea/tools/schema2md.py
# Compiled at: 2020-01-16 08:16:13
    ...
    if 'type' in nob.keys():
        ...
        if nob['type'] == 'object':
            ...
            for key in nob['properties'].keys():
                out += to_table_line(nob['properties'][key], path + '/' + key)
...
def array2md(nob, path):
    ...
    if array_type == 'object':
        out += ' array | list of items with a structure detailed in the following table |'
        out += '\n'
        out += '## List of objects for ' + path
        out += '\n'
        out += md_table_header()
        for key in nob['items']['properties'].keys():
            out += to_table_line(nob['items']['properties'][key], path + '/' + key)
...
def xor2md(nob, path):
    ...
    out = str()
    out += ' oneOf (XOR) | must match one of the options detailed in the following table |'
    out += '\n'
    ...
    for option in nob['oneOf']:
        ...
        for key in option['properties'][opt_name]['properties']:
            out += to_table_line(option['properties'][opt_name]['properties'][key], opt_path + '/' + key)
