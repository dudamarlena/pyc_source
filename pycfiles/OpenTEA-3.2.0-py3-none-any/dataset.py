...
# [GCC 8.4.0]
# Embedded file name: /Users/dauptain/GITLAB/opentea/opentea/dataset.py
...
__all__ = [
 'Dataset']
import os, codecs, logging, warnings
from itertools import izip
from collections import OrderedDict
from xml.parsers.expat import ExpatError
from xml.dom.minidom import parse
from .path_tools import PathTools
from .exceptions import OTNoNodeException, OTTooManyNodesException, OTException
...
class Dataset(object):
    ...
    def __init__(self, xmlfile):
        ...
        try:
            ...
            self.log.error("%s from c3sm wasn't correct xmlRemark : check that you have enough free space or permission. ", xmlfile)
    ...
    def _printNode(self, sep=''):
        ...
        if node.nodeType == 1:
            ...
            for _, valeur in node.attributes.items():
                desc += ' ' + valeur + '   '
    ...
    def _searchNode(self, key, *path):
        ...
        if result.length == 0:
            raise OTNoNodeException('OTError : no node found with key ' + str(key) + ' and path containing ' + str(path))
        else:
            result_to_del = []
            ...
            if result.length == 0:
                ...
                err_msg = 'OTError :' + str(result.length) + " nodes found with key '" + str(key) + "' and path " + str(path) + ' :\n'
                for elt in result:
    ...
    def _getAddress(node):
        ...
        while node.nodeName != '#document':
            security -= 1
            if security == 0:
                raise OTException('Infinite loop while searching for all the parents ...')
    ...
    def addChild(self, nodeName, value, fatherName, *path):
        ...
        if self.nodeExists(nodeName, fatherName, *path):
            self.log.error('Node %s already exists. addChild only adds new nodes.', nodeName)
            raise OTException
    ...
    def tryGetValue(self, default, nodeName, *path):
        ...
        if self.nodeExists(nodeName, *path):
            return self.getValue(nodeName, *path)
        return default
    ...
    def getFileList(path, filter_search='', output='listonly'):
        ...
        warnings.warn('getFileList should be called from PathTools', DeprecationWarning)
        ptl = PathTools()
    ...
    def getSelection(self, key, *path):
        ...
        sel_dict = dict(zip(selection_list[::2], [ int(float(x)) for x in selection_list[1::2] ]))
    ...
    def setValue(self, value, nodeName, *path):
        ...
        try:
            ...
            self.log.warning('Note : Could not find node %s for path %s. Skipping setValue', nodeName, str(path))
    ...
    def save2file(self, fileName):
        ...
        try:
            ...
            self.log.error('Could not save XML content to file %s from %s.\nCheck path, permissions and disk space', fileName, os.getcwd())
...
class Multiple(object):
    ...
    def __init__(self, dataset, node_name, *path):
        ...
        assert all([ node[:5] == 'item_' for node in self.ids ]), 'Node ' + node_name + ' does not seem to be a multiple'
        self.headers = []
        self.rows = []
        self._read_headers()
        self._read_rows()
    ...
    def _read_headers(self):
        ...
        self.headers = [
         'label'] + self.dts.getChildrenName(self.ids[0])

    def _read_rows(self):
        """Read full data from multiple"""
        for identifier in self.ids:
            entry = [
             self.dts.getValue(identifier)]
            entry += [ self.dts.getValue(header, identifier) for header in self.headers[1:]
                     ]
    ...
    def _columns_widths(self):
        ...
        for i, col in enumerate(self.columns):
            lengths.append(max(len(entry) for entry in [self.headers[i]] + col))
    ...
    def _format_line(self, line):
        ...
        return (' | ').join(formatters[i].format(entry) for i, entry in enumerate(line))
    ...
    def __str__(self):
        ...
        out = [
         self._format_line(self.headers),
