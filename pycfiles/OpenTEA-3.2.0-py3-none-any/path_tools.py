...
# [GCC 8.4.0]
# Embedded file name: /Users/dauptain/GITLAB/opentea/opentea/path_tools.py
...
import os, shutil, logging, errno, glob
__all__ = [
 'PathTools']
...
class PathTools(object):
    ...
    def abspath(self, *args):
        ...
        """
        interpret_path_from = os.path.join(*args) if os.path.isabs(args[0]) else os.path.join(self.base_dir, *args)
        return os.path.normpath(os.path.realpath(os.path.abspath(interpret_path_from)))

    def relpath(self, *args):
        """Get path as clean relative from base_dir"""
        return './' + os.path.normpath(os.path.relpath(os.path.realpath(self.abspath(*args)), self.rel_dir))
    ...
    def _file_checks(self, src, dest):
        ...
        if os.path.isfile(src) and os.path.isfile(dest) and os.path.samefile(src, dest):
    ...
    def copy_dir(self, src, dest):
        ...
        dest = self.abspath(dest)
        if os.path.isfile(dest):
            self.log.error('Trying to create/populate directory %s, but it is already a regular file', dest)
            raise OSError(errno.EEXIST)
        if os.path.isdir(src) and os.path.isdir(dest) and os.path.samefile(src, dest):
            self.log.warning('No copy for the same file: %s and %s', src, dest)
            return
        ...
        for path in os.listdir(src):
            if os.path.isdir(path):
                self.copy_dir(os.path.join(src, path), os.path.join(dest, path))
            else:
