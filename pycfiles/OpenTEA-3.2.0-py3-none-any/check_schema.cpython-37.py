...
# [GCC 8.4.0]
# Embedded file name: /Users/dauptain/GITLAB/opentea/opentea/noob/check_schema.py
...
class NobSchemaError(Exception):
    __doc__ = 'Error due to schema structure'
    ...
    if path is None:
        ...
        if isinstance(schema, dict):
            if 'type' not in schema:
                out.append('ERROR :  node ' + str_address(path) + ': -type- is missing')
            else:
                if schema['type'] == 'object':
                    if 'oneOf' in schema:
                        out.extend(rec_check_xor(schema, path))
                        if 'properties' in schema:
                            out.append('ERROR :  object ' + str_address(path) + ': -properties-  cannot be ' + 'at the same leval as a oneOf')
                    elif 'properties' not in schema:
                        out.append('ERROR :  object ' + str_address(path) + ': -properties- is missing')
                    else:
                        out.extend(rec_check_properties(schema['properties'], path))
                else:
                    if schema['type'] in ('string', 'integer', 'number', 'boolean'):
                        ...
                        if schema['type'] == 'array':
                            ...
                            out.append('ERROR :  node ' + str_address(path) + ': dead en in check of dict')
        else:
            out.append('ERROR :  node ' + str_address(path) + ': dead en in check; Non dict input ')
    return out
...
def rec_check_leafs(schema, path):
    ...
    if 'default' not in schema:
        out.append('WARNING :  leaf ' + str_address(path) + ': -default- is missing')
    return out
...
def rec_check_array(schema, path):
    ...
    if 'default' in schema:
        out.append('ERROR :  array ' + str_address(path) + ': -default- not supported yet')
    if 'items' not in schema:
        out.append('ERROR :  array ' + str_address(path) + ': -items- is missing')
    if 'type' not in schema['items']:
        out.append('ERROR :  array ' + str_address(path) + ': -type- of items is missing')
    key = 'items'
    out.extend(rec_check_schema(schema[key], [*path, key]))
    return out
...
def rec_check_xor(schema, path):
    ...
    out = list()
    if 'default' in schema:
        out.append('WARNING :  oneof ' + str_address(path) + ': -default- is not used.' + ' The first item of oneOf is the default')
    for idx, option_schema in enumerate(schema['oneOf']):
        if 'required' not in option_schema:
            out.append('ERROR :  oneof ' + str_address(path) + ' item ' + str(idx) + ': -required- is needed to enable json validation')
        if 'properties' not in option_schema:
            out.append('ERROR :  oneof ' + str_address(path) + ' item ' + str(idx) + ': -properties- is needed to enable json validation')
        list_prop = list(option_schema['properties'].keys())
        if list_prop != option_schema['required']:
            out.append('ERROR :  oneof ' + str_address(path) + ' item ' + str(idx) + ' -properties- single item must match -required- list:' + str(list_prop) + '.vs.' + str(option_schema['required']))
        if len(list_prop) != 1:
            out.append('ERROR :  oneof ' + str_address(path) + ' item ' + str(idx) + '-properties- must have a single item')
    ...
    with open(fname, 'r') as (fin):
        if ext in ('.yml', '.yaml'):
            ...
            if ext in ('.json', ):
                ...
                raise NotImplementedError('Cannot read file with extension :' + ext)
    return data
