...
# [GCC 8.4.0]
# Embedded file name: /Users/dauptain/GITLAB/opentea/opentea/wincanvas.py
...
class WinCanvas(PathTools):
    ...
    def __init__(self, dataset, target, prefix, balise):
        ...
        file_list = self.get_file_list('./' + self.folder + '/', filter_search=self.prefix + '*@' + self.target + '.can', output='absolute')
        for filename in file_list:
            self.log.debug('Removal of file ' + filename)
            os.remove(filename)

        glancevar = (';').join([str(time()), self.target,
         './' + self.folder + '/' + self.prefix + '*@' + self.target + '.can'])
        dataset.setValue(glancevar, balise)

    def add_point(self, coords, name='none', title='none', info='none', moveable='no', color='black', size='1'):
        """ point object
    ...
    def add_segment(self, coords, name='none', title='none', info='none', moveable='no', color='black', fillcolor='', refinement='no', style='open'):
        """ segment object
        ...
        for forbid in ['-', ' ']:
            if '-' in name:
                msgerr = 'Function add segment: name ' + name + ' not valid. Character ' + forbid + ' forbidden'
    ...
    def add_slider(self, coords, support, name='none', title='none', info='none', moveable='no', npts=3, color='black'):
        """ segment object
    ...
    def dump_object(self):
        """ save the object """
        tmpFile = open(os.path.join(self.folder, self.prefix) + '@' + self.target + '.can', 'w')
