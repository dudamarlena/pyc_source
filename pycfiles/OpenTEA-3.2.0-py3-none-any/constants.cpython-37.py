...
# [GCC 8.4.0]
# Embedded file name: /Users/dauptain/GITLAB/opentea/src/opentea/gui_forms/constants.py
# Compiled at: 2020-03-13 11:46:39
# Size of source mod 2**32: 8042 bytes
"""Constants definitions."""
import os
from glob import glob
import inspect, tkinter as tk
...
class SetException(Exception):
    __doc__ = 'Define an exception on the widget setters.'


class GetException(Exception):
    __doc__ = 'Define an exception on the widget getters.'
...
def set_constants(tksession, calling_dir, theme):
    ...
    if theme == 'alt':
        ...
        PARAMS['bg_lbl'] = '#%02x%02x%02x' % (bgc[0] - 7, bgc[1] - 7, bgc[2] - 7)
    if theme == 'aqua':
        bgc = (240, 240, 240)
        PARAMS['bg'] = '#%02x%02x%02x' % bgc
        PARAMS['bg_lbl'] = '#%02x%02x%02x' % (bgc[0] - 7, bgc[1] - 7, bgc[2] - 7)
    ...
    if theme == 'classic':
        bgc = (224, 224, 224)
        PARAMS['bg'] = '#%02x%02x%02x' % bgc
        PARAMS['bg_lbl'] = '#%02x%02x%02x' % (bgc[0] - 6, bgc[1] - 6, bgc[2] - 6)
    if theme == 'default':
        bgc = (220, 218, 213)
        PARAMS['bg'] = '#%02x%02x%02x' % bgc
        PARAMS['bg_lbl'] = '#%02x%02x%02x' % (bgc[0] - 3, bgc[1] - 1, bgc[2] + 4)
...
class SwitchForm(ttk.Frame):
    __doc__ = 'Overriden Frame class to mimick notebooks without tabs.'

    def add(self, name, title=None):
        """Add a tab-like Frame."""
        if name[0] != name[0].lower():
            raise RuntimeError('Switchforms cannot start with  upper case letters')
        tab_id = ttk.LabelFrame(self,
          name=name,
          text=title,
          relief='sunken')
...
def create_scrollable_canvas(holder_frame):
    ...
    frm.sby = ttk.Scrollbar(frm,
      orient='vertical',
      command=(frm.can.yview))
    frm.sbx = ttk.Scrollbar(frm,
      orient='horizontal',
...
class TextConsole:
    __doc__ = ' Text widget with search and auto -refresh capabilities.'

    def __init__(self, holder, content, height=None, width=None, search=True, disabled=True):
        """Startup class.
        ...
        if search:
            ...
            if disabled:
                ...
                bg_color = 'white'
        self.txt = ScrolledText((self.body),
          background=bg_color,
          highlightbackground=(PARAMS['bg_lbl']))
    ...
    def highlight_pattern(self, *args):
        ...
        if pattern:
            while True:
                index = self.txt.search((self.search_var.get()), 'matchEnd',
                  'searchLimit',
                  count=count,
                  regexp=True)
                if index == '':
