...
# [GCC 8.4.0]
# Embedded file name: /Users/dauptain/GITLAB/opentea/opentea/viewer3d_case_utilities.py
...
class View3dObject(PathTools):
    ...
    def __init__(self, dataset, anchor, balise, default_color='#ff0000', default_aspect='edges', base_dir=os.getcwd()):
    ...
    def add_part(self, part_name, color=None, aspect=None):
        """ initiate a new part
        """
        if color is None:
    ...
    def test_part(self, part):
        ...
        if part not in self.list_parts:
            msgerror = 'part ' + part + ' not in declared parts ' + str(self.list_parts)
        ...
        for i, part in enumerate(self.list_parts):
            ...
            for elt in self.supported_elements:
                if self.parts[(part, elt)]:
                    content += 'bar2\n'
                    content += ('{0: 8}').format(len(self.parts[(part, elt)])) + '\n'
    ...
    def add_box(self, part, xyz1, xyz2, nsampling):
        ...
        for sign in [-1, 1]:
            ...
            normal = [1, 0, 0]
            tangent1 = [0, 1, 0]
            self.add_plane(part, center, normal, tangent1, box_dim[1], box_dim[2], nsampling, nsampling)
            shift = np.zeros(3)
            shift[1] = sign * 0.5 * box_dim[1]
            center = box_center + shift
            normal = [0, 1, 0]
            tangent1 = [0, 0, 1]
            self.add_plane(part, center, normal, tangent1, box_dim[2], box_dim[0], nsampling, nsampling)
            shift = np.zeros(3)
            shift[2] = sign * 0.5 * box_dim[2]
            center = box_center + shift
            normal = [0, 0, 1]
            tangent1 = [1, 0, 0]
            self.add_plane(part, center, normal, tangent1, box_dim[0], box_dim[1], nsampling, nsampling)

    def add_revsurf(self, part, origin, direction, dir_zeroangle, x1, x2, r1, r2, angle_start, angle_end, samples_longi, samples_azi):
    ...
    def add_disc(self, part, origin, direction, rmin, rmax):
        ...
        if np.dot(direction, xaxis) == 1.0:
            ...
            dir2 /= np.linalg.norm(dir2)
        self.add_revsurf(part, origin, direction, dir2, 0, 0, rmin, rmax, -180, 180, SAMPLING_RAD, SAMPLING_AZI)
    ...
    def add_sphere(self, part, origin, radius, sampling=15):
        ...
        for i in range(sampling_revsurf):
            ...
            self.add_revsurf(part, origin, direction, dir2, x0, x1, r0, r1, -180, 180, 2, sampling)
    ...
    def add_cylinder(self, part, origin, direction, length, radius):
        ...
        if np.dot(direction, xaxis) == 1.0:
            ...
            dir2 = np.cross(direction, tmp)
            dir2 /= np.linalg.norm(dir2)
        self.add_revsurf(part, origin, direction, dir2, 0, length, radius, radius, -180, 180, SAMPLING_RAD, SAMPLING_AZI)
    ...
    def add_cone(self, part, origin, direction, length, radius):
        ...
        if np.abs(np.dot(direction, xaxis)) == 1.0:
            ...
            dir2 = np.cross(direction, tmp)
            dir2 /= np.linalg.norm(dir2)
        self.add_revsurf(part, origin, direction, dir2, 0, length, 0, radius, -180, 180, SAMPLING_RAD, SAMPLING_AZI)