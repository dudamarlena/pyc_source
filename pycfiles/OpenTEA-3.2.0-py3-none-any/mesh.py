...
# [GCC 8.4.0]
# Embedded file name: /Users/dauptain/GITLAB/opentea/opentea/mesh.py
# Compiled at: 2019-07-17 05:02:40
"""Mesh.py
...
class Mesh(PathTools):
    ...
    def __init__(self, filename, process, **kwargs):
        ...
        self.root_filename = self.h5_file.replace('.mesh.h5', '').split('/')[(-1)]
        ...
        self.listofsurfaces = None
        self.listbndmin = None
    ...
    def process_hip(self):
        ...
        self.copy_file(tmp_dir_abs + '/' + self.root_filename + '.mesh.h5', self.abspath(self.h5_file))
        self.copy_file(tmp_dir_abs + '/' + self.root_filename + '.mesh.xmf', self.abspath(self.xmf_file))
        self.copy_file(tmp_dir_abs + '/' + self.root_filename + '.asciiBound', self.abspath(self.asciibound_file))
        extor.finish()
    ...
    def execute_meshsample(self, prefix):
        ...
        if len(prefix) > 5:
            prefix = prefix[:5]
            self.log.warning('The name of the prefix has been trimmed down to 5 char')
        elif len(prefix) < 5:
            self.log.error('The name of the prefix is too short, should be 5 char')
            raise OTException
        extor = self.process.get_executor('hip_meshsample', [local_common])
        extor.appli = 'tool_hip'
        hipscript_ct = "set check 0\nread hdf './../" + self.h5_file + "'\ndecimate '" + self.h5_file + "'\nwrite ensight -a -s0 " + prefix + ' \nexit() \n'
        fname = os.path.join(self.process.base_dir, extor.execute_dir, 'hipscript')
        with open(fname, 'w') as (fout):
            fout.write(hipscript_ct)
        extor.execute()
        ex_dir = extor.abspath(extor.execute_dir)
        self.copy_file(ex_dir + '/' + prefix + '.case', local_common)
        self.copy_file(ex_dir + '/' + prefix + '.geo', local_common)
        extor.finish()

    def loadmesh_local(self):
        """ Load the mesh files in the COMMON directory"""
        abs_common = self.abspath(COMMON)
        self.h5_file = self.copy_file(self.h5_file, abs_common)
        self.asciibound_file = self.copy_file(self.asciibound_file, abs_common)
        try:
            self.xmf_file = self.copy_file(self.xmf_file, abs_common)
        except OSError:
    ...
    def get_mesh_version(self):
        ...
        h5file.close()
        match = re.findall('\\d+\\.\\d+\\.\\d+', version_line)
        if not match:
            match = re.findall('\\d+\\.\\d+', version_line)
        if not match:
        ...
        if len(version_numbers) > 3 or len(version_numbers) < 2:
            raise OTInterrupt(('Wrong number of digits in mesh version: {mesh_version}.\n').format(**locals()) + 'Examples of recognized mesh versions: 1.48, 1.48.1, 16.10.1')
    ...
    def get_infos(self):
        ...
        def striplist(self):
            ...
            return [ element.strip(' ') for element in self ]
        ...
        if mesh_version < '17.00.00':
            self.log.warning('The version of the mesh is old, launching latest hip to update it')
            self.process_hip()
            mesh_version = self.get_mesh_version()
            if mesh_version < '17.00.00':
                raise OTInterrupt('Failed at updating the mesh because ' + 'your version of hip is too old: ' + str(mesh_version))
        ...
        if self.ndim == 2:
            self.ncells = 0
            if 'tri->node' in h5file['Connectivity'].keys():
                self.ncells += len(h5file['Connectivity']['tri->node']) / 3
            if 'qua->node' in h5file['Connectivity'].keys():
                self.ncells += len(h5file['Connectivity']['qua->node']) / 4
        else:
            self.ncells = 0
            if 'tet->node' in h5file['Connectivity'].keys():
                self.ncells += len(h5file['Connectivity']['tet->node']) / 4
            if 'pri->node' in h5file['Connectivity'].keys():
                self.ncells += len(h5file['Connectivity']['pri->node']) / 6
            if 'hex->node' in h5file['Connectivity'].keys():
                self.ncells += len(h5file['Connectivity']['hex->node']) / 8
            if 'Patch->area' in h5file['Boundary'].keys():
                self.listofsurfaces = list(h5file['Boundary/Patch->area'][:])
            elif 'patch_area' in h5file['Boundary'].keys():
                self.listofsurfaces = list(h5file['Boundary/patch_area'][:])
            else:
                self.listofsurfaces = [
                 1] * len(self.listofpatches)
                self.log.warning('The list of patches area is not found in the h5 file, use hip 16.11 or newer to process mesh')
            ...
            try:
                ...
                self.log.warning('The rtheta bounding box is not found in the h5 file, use hip 16.11 or newer to process mesh')

        self.info_mesh_content = ('ndim : {0} ({1})\n').format(self.ndim, self.ndimlist)
    ...
    def _ensure_hmin_hmax(self):
        ...
        try:
            ...
            self.log.warning('Requesting hmin/hmax. Launching latest hip to update the mesh.')
            self.process_hip()
            try:
                self._hmin = h5file['Parameters/h_min/'][0]
                self._hmax = h5file['Parameters/h_max/'][0]
                ...
                if mesh_version < '17.07.04':
                    raise OTInterrupt('hmin/hmax is a feature available only with hip 17.07.4 and later. Your version is: ' + str(mesh_version))
                else:
                    self.log.error('hmin/hmax not found.')
                    self.log.error(('Hip version: {mesh_version}').format(**locals()))
