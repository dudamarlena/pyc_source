...
# [GCC 8.4.0]
# Embedded file name: /Users/dauptain/GITLAB/opentea/opentea/executor.py
...
class Executor(PathTools):
    ...
    def _inits(self, process):
        ...
        if self.action == '-code_exe-':
            self.execute_dir = process.this_run
            PathTools.__init__(self, process.base_dir, rel_dir=self.abspath(process.base_dir, self.execute_dir))
    ...
    def _setup_distant_bundle(self):
        ...
        for filename in ['constants.py', 'exceptions.py',
         'dataset.py', 'path_tools.py']:
            self.copy_file(join(opentea_dir_path, filename), join(self.execute_dir, 'distant_bundle/'))
    ...
    def load_code_plugin(self, name=None):
        ...
        if name is None:
            try:
                ...
                name = self.dataset.getValue('code_plugins', self.dataset.getValue('name', 'solver', 'meta'))
    ...
    def _load_plugin(self, name):
        """Load plugin 'name'"""
        plugin_path = join(self.dataset.getValue('pluginsPath', 'engine', 'meta'), 'scripts', name + '.py')
        self.log.debug(('Loading plugin {0} at path {1} ').format(name, plugin_path))
    ...
    def execute(self):
        ...
        self.log.debug(('Params : action: {0.action} - execute_dir: {0.execute_dir} - additional dirs: {0.add_dirs}').format(self))
        self.stdout = self.plugin.execute_action(self.action, self.execute_dir, self.appli, self.add_dirs, retrieve=self.retrieve, options=self.options)
