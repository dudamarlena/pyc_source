...
# [GCC 8.4.0]
# Embedded file name: /Users/dauptain/GITLAB/opentea/src/opentea/gui_forms/leaf_widgets.py
# Compiled at: 2020-03-13 12:05:05
...
"""
import operator
from tkinter import ttk, Variable, StringVar, BooleanVar, Label, Toplevel, Text, Listbox, filedialog
from nob import Nob
from opentea.gui_forms.constants import PARAMS, WIDTH_UNIT, LINE_HEIGHT, IMAGE_DICT, GetException, SetException, TextConsole

class LeafWidget:
    __doc__ = 'Factory for OpenTea Widgets.'
...
class _OTEntry(LeafWidget):
    __doc__ = 'Factory for OpenTea Entries.'
    ...
    def _inits_entry(self, schema):
        """Initialise entry."""
        self.tkvar = Variable()
        self._status = Label((self._holder), text='no status yet',
          background=(PARAMS['bg_lbl']),
          foreground='red',
          justify='left',
          compound='left',
          ...
          width=WIDTH_UNIT)
        self._entry = ttk.Entry((self._holder), textvariable=(self.tkvar),
          state=(self.state))
                ...
                if operators[i](value, self._bounds[i]):
                    if self._exclusive_bounds[i]:
                        if value == self._bounds[i]:
                            error_msg = 'Invalid :%s %s' % (str_operators[i],
                             str(self._bounds[i]))
                        else:
                            error_msg = 'Invalid:%s= %s' % (str_operators[i],
                             str(self._bounds[i]))
...
class OTBoolean(LeafWidget):
    ...
    def __init__(self, schema, root_frame, name):
        ...
        self._cbutt = ttk.Checkbutton((self._holder), variable=(self.tkvar),
...
class OTChoice(LeafWidget):
    __doc__ = 'OT choices widget.'

    def __init__--- This code section failed: ---

 L. 359         0  LOAD_GLOBAL              super
                2  CALL_FUNCTION_0       0  '0 positional arguments'
                4  LOAD_METHOD              __init__
                6  LOAD_FAST                'schema'
                8  LOAD_FAST                'root_frame'
               10  LOAD_FAST                'name'
               12  CALL_METHOD_3         3  '3 positional arguments'
               14  POP_TOP          

 L. 360        16  LOAD_GLOBAL              StringVar
               18  CALL_FUNCTION_0       0  '0 positional arguments'
               20  LOAD_FAST                'self'
               22  STORE_ATTR               tkvar

 L. 361        24  LOAD_FAST                'self'
               26  LOAD_ATTR                _label
               28  LOAD_ATTR                place
               30  LOAD_CONST               0.5
               32  LOAD_CONST               1
               34  LOAD_STR                 'se'
               36  LOAD_CONST               ('relx', 'rely', 'anchor')
               38  CALL_FUNCTION_KW_3     3  '3 total positional and keyword args'
               40  POP_TOP          

 L. 362        42  LOAD_FAST                'dynamic'
               44  LOAD_FAST                'self'
               46  STORE_ATTR               dynamic

 L. 363        48  LOAD_FAST                'self'
               50  LOAD_ATTR                dynamic
               52  POP_JUMP_IF_TRUE     96  'to 96'

 L. 364        54  LOAD_GLOBAL              len
               56  LOAD_FAST                'schema'
               58  LOAD_STR                 'enum'
               60  BINARY_SUBSCR    
 ...
 L. 365        70  LOAD_FAST                'self'
               72  LOAD_METHOD              _pack_with_combobox
               74  LOAD_FAST                'schema'
               76  LOAD_STR                 'enum'
               78  BINARY_SUBSCR    
...
Parse error at or near `COME_FROM' instruction at offset 96_0
    ...
    def _pack_with_combobox(self, option):
        ...
        self.combo = ttk.Combobox((self._holder),
          values=option,
...
class OTFileBrowser(LeafWidget):
    __doc__ = 'OT file/folder browser widget.'
    ...
    def __init__(self, schema, root_frame, name):
        ...
        self._label.place(relx=0.5, rely=0.5, anchor='e')
        self._path = ttk.Entry((self._holder), textvariable=(self.tkvar),
          state='disabled',
          foreground='black')
        self._path.place(relx=0.5, rely=0.5, relwidth=0.4, anchor='w')
        self._btn = ttk.Button((self._holder), image=(IMAGE_DICT['load']),
    ...
    def _browse(self, event=None):
        ...
        if self._isdirectory:
            ...
            path = filedialog.askopenfilename(title=(self._title), filetypes=(self._filter))
        self.tkvar.set(path)
...
class OTDocu(LeafWidget):
    ...
    def __init__(self, schema, root_frame, name):
        super().__init__(schema, root_frame, name)
        self.tkvar = StringVar()
        self._btn = ttk.Button((self._holder), width=(0.01 * WIDTH_UNIT),
          compound='center',
    ...
    def _popup_dialog(self):
        ...
        dlg_frame = ttk.Frame((self._dialog), width=(3 * WIDTH_UNIT),
          height=(3 * WIDTH_UNIT))
...
class OTDescription(LeafWidget):
    ...
    def __init__(self, schema, root_frame, name):
        ...
        self._label = Label((self._holder),
          ...
          justify='right',
          wraplength=(int(0.5 * WIDTH_UNIT)))
        self._label.config(justify='left', textvariable=(self.tkvar),
...
class OTComment(LeafWidget):
    ...
    def __init__(self, schema, root_frame, name):
        ...
        self.txt = TextConsole((self._holder), (self.tkvar),
          height=height,
          width=10,
          disabled=disabled)
...
class OTEmpty(LeafWidget):
    __doc__ = 'OT widget for unimplemented types.'
...
class OTList(LeafWidget):
    __doc__ = 'Factory for OpenTea Lists.'
    ...
    def _inits_list(self):
        ...
        self.entrylistholder.place(relwidth=0.5, relx=0.5,
          rely=0.0,
          anchor='nw')
        self._status.place(relwidth=0.9, relx=0.9,
          rely=1.0,
          anchor='se')
        ...
        if self.state == 'disabled':
            self.resizable = False
        if self.resizable:
            self.additem_bt = ttk.Button((self._holder), text='+',
              command=(self.additem))
            self.delitem_bt = ttk.Button((self._holder), text='-',
              command=(self.delitem))
            self.additem_bt.place(relwidth=0.05, relx=0.95,
              rely=1.0,
              anchor='se')
            self.delitem_bt.place(relwidth=0.05, relx=1,
              rely=1.0,
              anchor='se')
    ...
    def _update_entrylist(self):
        ...
        if self.state == 'disabled':
            nlines = 6
            lbx = Listbox((self.entrylistholder),
              height=nlines)
            ...
            if not self.variable:
                self.entrywidgets.append(ttk.Label((self.entrylistholder), text='void'))
        ...
        if self.item_type == 'number':
            ...
            if self.item_type == 'string':
                ...
                if self.item_type == 'integer':
                    ...
                    raise NotImplementedError('Type ' + self.item_type + ' not implemented.')
    ...
    def _single_entry_callback(self, event):
        ...
        for entry in self.entrywidgets:
            ...
            try:
                ...
                self._status.config(text=status, fg='red',
                  image=(IMAGE_DICT['invalid']))
                self.status = -1
