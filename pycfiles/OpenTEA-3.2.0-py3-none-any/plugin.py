...
# [GCC 8.4.0]
# Embedded file name: /Users/dauptain/GITLAB/opentea/opentea/plugin.py
...
import os, sys, shlex, logging, subprocess, json, warnings
...
class Plugin(PathTools):
    ...
    def _check_version(self):
        ...
        assert all((ot_version >= self.version,
         ot_version.split()[0] == self.version.split()[0])), ('The version detected for your plugin is {slf}, but the OpenTEA version is {ot}. Either you have not declared your version in the plugin or it is incompatible.').format(slf=self.version, ot=ot_version)

    def check_support(self):
        """Check that appli is indeed supported by plugin"""
        warnings.warn('check_support is deprecated from version 2.2', DeprecationWarning)
        if self.appli not in self.supported_tools + self.supported_codes:
            raise OTInterrupt(('Application {0} is neither in supported tools nor codes').format(self.appli))
        self.log.debug(('Plugin {0} : Running execute_action {1} in {2} ({3})').format(self.name, self.action, self.exec_directory, self.appli))
    ...
    def env(self):
        ...
        if self.read_json:
            ...
            assert isfile(env_file), ('This plugin expects to find a {} file.\n').format(env_file) + 'Please create it using the $OPENTEA_HOME/tools/ensure_env.py tool'
            env.update(dict((k, v.encode('utf8')) for k, v in json.load(open(env_file)).items()))
        return env
    ...
    def execute_local(self, command=None, from_dir=None):
        ...
        if '<' in command:
            read_from = command[(-1)]
            command = command[:-2]
        subp = subprocess.Popen(command, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=self.env)
        if read_from:
        ...
        if not returncode:
            ...
            self.log.error('=============StdErr=================' + stderr_data)
            raise RuntimeError
        os.chdir(start_dir)
    ...
    def execute_ssh(self, command=None, options=''):
        ...
        return self.execute_local(("ssh {0} {1} '{2}'").format(options, self.full_host, command))

    def ssh_prepare_directory(self):
        """ Prepare (creates) a directory on a distant server """
        if not self.distant_directory:
            self.log.error('Please specify a distant directory, e.g.: /home/toto/')
        if self.distant_directory[0] != '/' and self.distant_directory[0] != '~':
            self.log.error('Relative path given. Please specify an absolute path')
    ...
    def execute_action(self, action, exec_directory, appli, add_dirs=None):
        ...
        raise NotImplementedError('Plugin : execute_action() is not implemented in current plugin ...Please implement this command in your current plugin.')
    ...
    def ssh_send(self, local_directories_list):
        ...
        if not local_directories_list:
            ...
            return
        self.log.debug('ssh_send : Sending and extracting archive if needed...')
        ...
        def ssh_send_one(path):
            ...
            command = ('bash -c "tar cvfh - -C {0}/../ $(basename {0}) | ssh {1.full_host} \\"tar xfh - -C {1.distant_directory}\\" "').format(self.abspath(path), self)
    ...
    def send_directory(self, directory):
        ...
        if self.dir2send:
            if os.getcwd() != self._dir2send_curdir:
                raise OTInterrupt('Current directory changed between calls to sendDirectory(). This is not supported !')
        else:
    ...
    def retrieve_directory(self, directory):
        ...
        raise NotImplementedError('Plugin : retrieveDirectory is not implemented in current plugin ...')

    def remove_directory(self, directory):
        """This method removes directory from the distant place """
        raise NotImplementedError('Plugin : removeDirectory is not implemented in current plugin ...')

    def get_plugin_param(self, param_name):
        """getValue wrapper for specific plugin type"""
        try:
            return self.dataset.getValue(param_name, self.name, self.type + '_plugins')
        except OTTooManyNodesException:
            return self.dataset.getValue(param_name, self.name, self.type + '_plugins', self.dataset.getValue('name', 'solver', 'meta'))