...
# [GCC 8.4.0]
# Embedded file name: /Users/dauptain/GITLAB/opentea/opentea/process.py
...
__all__ = [
 ...
 'CodeProcess',
 'LibProcess']
import time, os, shutil, logging, inspect, warnings
...
class BaseProcess(PathTools):
    ...
    def __init__(self, dataset_name='dataset.xml', base_dir=None):
        if base_dir is not None:
            warnings.warn('The use of base_dir is deprecated from version 2.2. Instead, the directory of dataset_name is used. See issue #42', DeprecationWarning)
        else:
    ...
    def _init_defs_base(self):
        ...
        self.verbose = 0
        PathTools.__init__(self, base_dir=self.base_dir)
        self.script_dir = dirname(inspect.getfile(self.__class__))
        self.original_application_name = self.ds.getValue('name', 'solver', 'meta')
        ...
        self.log.info(('Use of OpenTEA for project {0} ({1})').format(self.project_name, self.original_application_name))
    ...
    def get_executor(self, action, add_dirs=None, options=''):
        ...
        self.executors[action] = Executor(self, action, add_dirs=add_dirs, options=options)
        return self.executors[action]

    def progress(self, progr_frac, text):
        r""" give a visual cue on progress
...
class CodeProcess(BaseProcess):
    ...
    def _update_run_dirs(self):
        ...
        self.this_run = None if run_nb() == 0 else ('RUN_{:03d}').format(run_nb())
        self.next_run = ('RUN_{:03d}').format(run_nb() + 1)
        self.log.debug(('Current run: {0}. Next run: {1}').format(self.this_run, self.next_run))
        return
...
class LibProcess(BaseProcess):
    ...
    def update_libobjs(self):
        ...
        self.unwritten_libobjs = [ f for f in self.ds.getListValue('list_libobjs_in_project') if f not in self.ds.getListValue('list_written_libobjs')
                                 ]
        self.log.debug('Unwritten libobjs in xml: ' + repr(self.unwritten_libobjs))
    ...
    def rename_libobj(self, src_name, dest_name):
        ...
        if src_name not in self.libobjs + self.unwritten_libobjs:
            raise OTInterrupt(('No object {} exists, cannot rename').format(src_name))
    ...
    def del_libobj(self, name):
        ...
        if name in self.libobjs:
            ...
            if name in self.unwritten_libobjs:
                self.log.info('Forgetting unwritten libobj ' + name)
            else:
                self.log.warning(name + ' is not a known libobj. Nothing to delete.')
                return
