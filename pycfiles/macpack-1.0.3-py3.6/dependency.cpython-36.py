...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/macpack/dependency.py
# Compiled at: 2017-03-20 21:50:43
# Size of source mod 2**32: 2394 bytes
import argparse, asyncio, re, pathlib, subprocess
...
class Dependency:
    ...
    def __repr__(self):
        return "Dependency('" + str(self.path) + "', symlinks=" + str(len(self.symlinks)) + "', dependencies=" + str(len(self.dependencies)) + ')'
    ...
    async def find_dependencies(self):
        process = await asyncio.create_subprocess_exec('otool', '-L', (str(self.path)), stdout=(subprocess.PIPE),
