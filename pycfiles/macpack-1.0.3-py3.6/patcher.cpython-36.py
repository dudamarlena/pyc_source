...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/tmp/patcher.py
# Compiled at: 2017-03-06 22:10:19
# Size of source mod 2**32: 5844 bytes
import argparse, asyncio, os, sys, subprocess, pathlib, shutil
...
async def patch(root_dep, dest_path, root_loader_path):
    ...
    for dep in patch_deps:
        if dep == root_dep:
            pargs = [
             'install_name_tool', str(root_dep.path)]
        ...
        process_coros.append((asyncio.create_subprocess_exec)(*pargs, stdout=subprocess.PIPE, 
...
def main():
    ...
    if not args.dry_run:
        try:
            ...
            if not args.verbose:
                print('Run with -v for more information', file=(sys.stderr))
            sys.exit(1)
