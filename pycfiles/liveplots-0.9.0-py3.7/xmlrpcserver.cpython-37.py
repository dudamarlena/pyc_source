...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/liveplots/xmlrpcserver.py
# Compiled at: 2019-11-08 08:25:02
# Size of source mod 2**32: 13175 bytes
...
class RTplot:
    ...
    def scatter(self, x, y, labels=[], title='', style='points', jitter=True, multiplot=0):
        ...
        if not len(x) == len(y):
            ...
            y = numpy.array(y)
            if jitter:
                jt = numpy.random.normal(1, 0.0001, 1)[0]
            else:
                jt = 1.0
            if x.shape != y.shape:
