...
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/echoip/providers.py
# Compiled at: 2015-05-21 02:34:11
...
import collections, time, random, zope.interface
...
class IPProvider(object):
    ...
    def get_info(self, required_info_keys=None):
        ...
        :rtype: dict
        """
        if not self.is_cache_valid or not self._cache_info or not self._verify_required_keys(self._cache_info, required_info_keys):
...
class MultisourceIPProvider(IPProvider):
    ...
    def _fetch_from_sources(self, required_info_keys=None):
        if self.num_sources < self._min_source_agreement:
            raise InsufficientSourcesForAgreementError(('{} sources are required but only {} configured').format(self._min_source_agreement, self.num_sources))
        infos = dict()
