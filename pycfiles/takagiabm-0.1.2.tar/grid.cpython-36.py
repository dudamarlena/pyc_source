...
# [GCC 8.4.0]
# Embedded file name: /media/hzy/程序/novalide/forgitcommit/NovalIDE/plugins/takagi/takagiabm/grid.py
...
class Cell(BaseAgent):
    ...
    def setProperty(self, propertyName: str, property):
        if type(propertyName) != type(''):
            raise Exception('警告:agent或者cell的属性只能以字符串命名.你输入的是%s' % repr(property))
...
class Grid:
    ...
    def __init__(self, model=None, width=0, height=0):
        global neighborDic
        self.model = model
        self.width = width
        self.height = height
    ...
    def getCellByPosInt(self, pos: np.ndarray):
        self.xWrapAction(self.width, pos)
    ...
    def getNeighbors(self, pos: np.ndarray, shape='') -> list:
        ...
        if (0 < pos[0] < self.width - 1) & (0 < pos[1] < self.height - 1):
    ...
    def getNeighborProperties(self, pos, propertyName: str, shape='') -> list:
        ...
        if propertyName in self.mat.keys():
            if (0 < pos[0] < self.width - 1) & (0 < pos[1] < self.height - 1):
                a = self.mat['alive'][pos[0] - 1:pos[0] + 2, pos[1] - 1:pos[1] + 2]
                l = a.tolist()
                l[1].pop(1)
                p = sum(l, [])
                return p
    ...
    def getCellPosInt(self, pos: np.ndarray):
        self.xWrapAction(self.width, pos)
        self.yWrapAction(self.height, pos)
        return pos

    def getAllCellsIter(self):
        iterChain = chain.from_iterable(self.cells)
        return iterChain
    ...
    def getCellColorArray(self, pos=None) -> np.array:
    ...
    def moveAgentTo(self, agent, targetPos0=np.array([0, 0])):
        ...
        if agent in self.cellList[lastPos[1]][lastPos[0]]:
            self.cellList[lastPos[1]][lastPos[0]].remove(agent)
        else:
    ...
    def getAgentNumInCell(self, pos):
