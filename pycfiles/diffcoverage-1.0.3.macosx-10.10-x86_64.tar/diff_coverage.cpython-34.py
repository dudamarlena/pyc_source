...
# [GCC 8.4.0]
# Embedded file name: /Users/andrewcrosio/projects/expenses/virtualenv/lib/python3.4/site-packages/diffcoverage/diff_coverage.py
# Compiled at: 2016-02-03 10:02:13
# Size of source mod 2**32: 13350 bytes
"""
diff-coverage
...
import logging, os, re, string, subprocess, sys, coverage, patch, settings
PERCENT_COMPARISON = lambda x, y: cmp(y[1]['coverage_percent'], x[1]['coverage_percent'])
NUMCOVERED_COMPARISON = lambda x, y: cmp(y[1]['coverage_executed'], x[1]['coverage_executed'])
FILENAME_COMPARISON = lambda x, y: cmp(x[0], y[0])
COMPARERS = {'filename': FILENAME_COMPARISON, 
...
def get_jenkins_path(file_name, root_package=None, src_file_link_prefix=None):
    ...
    if root_package:
        ...
        if len(file_name_parts) > 1:
            file_name_parts = [
             '_'.join(file_name_parts[:2])] + file_name_parts[2:]
...
def get_current_git_branch():
    ...
    if process.wait():
        raise RuntimeError('Error retrieving current branch: %s' % process.stderr.read() or 'No error available')
...
def diff_coverage(patch_file, show_all=False, coverage_file=settings.COVERAGE_PATH, html_file_path=None, root_package=None, sort_by='filename', link_prefix='', retain_build_no=False):
    ...
    for file_name, missing in missing_lines.iteritems():
        ...
        report[file_name] = {'coverage_percent': coverage_percent, 
    ...
    if report:
        ...
        max_covered_size = len(str(max(info['coverage_covered'] for info in report.values())))
        max_covered_size = max(max_covered_size, len(COVERAGE_LINE_HEADER))
        max_executed_size = len(str(max(info['coverage_executed'] for info in report.values())))
        ...
        print(header_format_string.format(FILE_NAME_HEADER, PERCENT_COVERED_HEADER, COVERAGE_LINE_HEADER, TOTAL_LINE_COV_HEADER))
        print(line_separator)
        ...
        for file_name, coverage_info in sorted_report:
            ...
            if retain_build_no:
                ...
                relative_path = '../..'
            rows.append(row_template.substitute(file_name=file_name, coverage_percent=coverage_percent, coverage_executed=coverage_executed, coverage_covered=coverage_covered, jenkins_coverage_path=jenkins_coverage_path, relative_path=relative_path))
            print(print_format_string.format(file_name, coverage_percent, coverage_covered, coverage_executed))

        print(line_separator)
        num_items = len(report)
        total_avg_coverage_percent = '%.1f%%' % (float(total_coverage_covered) / total_coverage_executed * 100)
        print(print_format_string.format('TOTAL', total_avg_coverage_percent, total_coverage_covered, total_coverage_executed))
        ...
        avg_print_format_string = '| {0:<%ds} | {1:>9s} | {2:>%d.1f} / {3:<%d.1f} |' % (
         max_filename_size, max_covered_size,
         max_executed_size)
        print(avg_print_format_string.format('AVERAGE', average_coverage_percent, average_coverage_covered, average_coverage_executed))
        print(line_separator)
        all_rows = ''.join(rows)
        if html_file_path:
...
def main():
    ...
    opt.add_option('-a', '--show-all', dest='show_all', default=False, action='store_true', help='Show even 100% coveraged files')
    opt.add_option('-c', '--coverage-file', dest='coverage_file', default=settings.COVERAGE_PATH, help='Set the coverage file path')
    opt.add_option('-o', '--output-file', dest='html_file_path', default=None, help='Set the path to save the html diff coverage report.')
    opt.add_option('-r', '--root-package', dest='root_package', default=None, help='Set the root package name for the XML report')
    opt.add_option('-s', '--sort-by', dest='sort_by', default='filename', help='Sort by type: [filename, percentage, numcovered]', choices=SORT_BY_CHOICES)
    opt.add_option('-b', '--retain-build-no', dest='retain_build_no', default=False, action='store_true', help='Set the build number to be used while creating links for source files')
    opt.add_option('-p', '--link-prefix', dest='link_prefix', default='', help='Set link prefix to be used while creating links for source file')
    ...
    patch_file = args[0]
    diff_coverage(patch_file, show_all=show_all, coverage_file=coverage_file, html_file_path=html_file_path, root_package=root_package, sort_by=sort_by, retain_build_no=retain_build_no, link_prefix=link_prefix)
