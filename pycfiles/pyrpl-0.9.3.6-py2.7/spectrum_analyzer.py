...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyrpl/software_modules/spectrum_analyzer.py
...
import sys, scipy.signal as sig, scipy.fftpack
...
class RbwProperty(FilterProperty):
    ...
    def valid_frequencies(self, module):
        sample_rates = [ 1.0 / st for st in Scope.sampling_times ]
        return [ module.equivalent_noise_bandwidth() * sr for sr in sample_rates ]

    def get_value(self, obj):
        sampling_rate = 1.0 / (8e-09 * obj.decimation)
    ...
    def set_value(self, obj, value):
        ...
        obj.decimation = int(round(1.0 / (sample_rate * 8e-09)))
...
class SpanFilterProperty(FilterProperty):
    ...
    def get_value(self, obj):
        return 1.0 / (8e-09 * obj.decimation)

    def set_value(self, obj, value):
        if np.iterable(value):
            value = value[0]
        sampling_time = 1.0 / value
        obj.decimation = int(round(sampling_time / 8e-09))
...
class SpectrumAnalyzer(AcquisitionModule):
    ...
    display_unit = DisplayUnitProperty(default='dB(Vpk^2)', options=[
     'Vpk^2',
     'dB(Vpk^2)',
     'Vpk',
     'Vrms^2',
    ...
    input = InputSelectProperty(options=all_inputs, default='in1', call_setup=True, ignore_errors=True)
    input1_baseband = InputSelectProperty(options=all_inputs, default='in1', call_setup=True, ignore_errors=True, doc='input1 for baseband mode')
    input2_baseband = InputSelectProperty(options=all_inputs, default='in2', call_setup=True, ignore_errors=True, doc='input2 for baseband mode')
    display_input1_baseband = BoolProperty(default=True, doc='should input1 spectrum be displayed in baseband-mode?')
    display_input2_baseband = BoolProperty(default=True, doc='should input2 spectrum be displayed in baseband-mode?')
    display_cross_amplitude = BoolProperty(default=True, doc='should cross-spectrum amplitude be displayed in baseband-mode?')
    display_cross_phase = BoolProperty(default=False, doc='should cross-spectrum amplitude be displayed in baseband-mode?')
    rbw = RbwProperty(doc='Residual Bandwidth, this is a readonly attribute, only span can be changed.')
    decimation = DecimationProperty(options=Scope.decimations, doc='Decimation setting for the scope.')
    ...
    def sampling_time(self):
        return 1.0 / self.nyquist_margin / self.span
    ...
    def _get_iq_data(self):
        ...
        if self.baseband:
            return res[0][:self.data_length] + complex(0.0, 1.0) * res[1][:self.data_length]
        else:
            return (res[0] + complex(0.0, 1.0) * res[1])[:self.data_length]

    def _get_filtered_iq_data(self):
        """
        :return: the product between the complex iq data and the filter_window
        """
        return self._get_iq_data() * np.asarray(self.filter_window(), dtype=np.complex)
    ...
    def useful_index_obsolete(self):
        ...
        length = self.points
        if self.baseband:
            return slice(middle - 1, middle + length / 2 + 1)
        else:
    ...
    def frequencies(self):
        ...
        """
        if self.baseband:
            return np.fft.rfftfreq(self.data_length * self.PADDING_FACTOR, self.sampling_time)
        else:
            return self.center + scipy.fftpack.fftshift(scipy.fftpack.fftfreq(self.data_length * self.PADDING_FACTOR, self.sampling_time))
        ...
        if not self.baseband:
            ...
            for f in self._iq_bandwidth():
                if f == 0:
                    ...
                    tf_iq *= 1.0 / (1.0 + complex(0.0, 1.0) * displaced_freqs / f)
                elif f < 0:
                    tf_iq *= 1.0 / (1.0 + complex(0.0, 1.0) * f / displaced_freqs)
    ...
    def transfer_function_scope(self, frequencies):
        ...
        norm_freq = self._scope_decimation() * displaced_freqs / 125000000.0
    ...
    def transfer_function(self, frequencies):
        ...
        exact.
        """
        return self.transfer_function_iq(frequencies) * self.transfer_function_scope(frequencies)
    ...
    def _get_curve(self):
        ...
        :return:
        """
        iq_data = self._get_filtered_iq_data()
        if self.running_state not in ('running_single', 'running_continuous'):
            self.pyrpl.scopes.free(self.scope)
        if self.baseband:
            fft1 = np.fft.fftpack.rfft(np.real(iq_data), self.data_length * self.PADDING_FACTOR)
            fft2 = np.fft.fftpack.rfft(np.imag(iq_data), self.data_length * self.PADDING_FACTOR)
            cross_spectrum = np.conjugate(fft1) * fft2
            res = np.array([abs(fft1) ** 2,
             abs(fft2) ** 2,
             np.real(cross_spectrum),
             np.imag(cross_spectrum)])
            self._last_curve_raw = res
            return res / abs(self.transfer_function(self.frequencies)) ** 2
        else:
            res = scipy.fftpack.fftshift(scipy.fftpack.fft(iq_data, self.data_length * self.PADDING_FACTOR))
            self._last_curve_raw = np.abs(res) ** 2
            return self._last_curve_raw / abs(self.transfer_function(self.frequencies)) ** 2
    ...
    def _iq_bandwidth(self):
        return self.iq.__class__.bandwidth.validate_and_normalize(self.iq, [
         self.span * self.if_filter_bandwidth_per_span] * 4)

    def _scope_decimation(self):
        return self.scope.__class__.decimation.validate_and_normalize(self.scope, int(round(self.sampling_time / 8e-09)))

    def _scope_duration(self):
        return self._scope_decimation() * 8e-09 * self.data_length

    def _start_acquisition(self):
        autosave_backup = self._autosave_active
        if not self.baseband:
            raise NotImplementedError('iq mode is not supported in the current release of Pyrpl.')
            self.iq.setup(input=self.input, bandwidth=self._iq_bandwidth(), gain=0, phase=0, acbandwidth=self.acbandwidth, amplitude=0, output_direct='off', output_signal='quadrature', quadrature_factor=self.quadrature_factor)
        ...
        self.scope.setup(input1=input1, input2=input2, average=True, duration=self.scope.data_length * self._scope_decimation() * 8e-09, trigger_source='immediately', ch1_active=True, ch2_active=True, rolling_mode=False, running_state='stopped')
    ...
    def save_curve(self):
        ...
        if not self.baseband:
            ...
            if self.display_cross_amplitude:
                d.update({'ch': 'cross', 'name': self.curve_name + ' cross'})
                curves.append(self._save_curve(self._run_future.data_x, (self._run_future.data_avg[2] + complex(0.0, 1.0) * self._run_future.data_avg[3]), **d))
            return curves
            return
    ...
    def equivalent_noise_bandwidth(self):
        ...
        sample rate."""
        if self.window not in self._enb_cached:
            filter_window = self.filter_window()
            self._enb_cached[self.window] = sum(filter_window ** 2) / sum(filter_window) ** 2
