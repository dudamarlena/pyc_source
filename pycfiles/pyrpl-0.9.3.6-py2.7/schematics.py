...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyrpl/widgets/module_widgets/schematics.py
...
class MyItem(QtWidgets.QWidget):
    ...
    def __init__(self, widget_name, y, label, parent, x_offset=0):
        ...
        self.item.setStyleSheet('QLabel{border: 1px solid black; border-radius: 5px; font-size: 15px; background-color:white}')
    ...
    def move_to_right_position(self):
        ...
        x = widget.pos().x() + widget.width() / 2 + self.x_offset - self.item.width() / 2 + self.item.x()
