...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyrpl/attributes.py
...
from .widgets.attribute_widgets import BoolAttributeWidget, FloatAttributeWidget, FilterAttributeWidget, IntAttributeWidget, SelectAttributeWidget, StringAttributeWidget, BoolIgnoreAttributeWidget, TextAttributeWidget, CurveAttributeWidget, DataAttributeWidget, CurveSelectAttributeWidget, LedAttributeWidget, PlotAttributeWidget, BasePropertyListPropertyWidget, ComplexAttributeWidget
from .curvedb import CurveDB
from collections import OrderedDict
import logging, sys, numpy as np, numbers
...
class BaseProperty(BaseAttribute):
    ...
    * a function get_value(instance) that reads the value from
    ...
    def __init__(self, default=None, doc='', ignore_errors=False, call_setup=False):
        """
    ...
    def launch_signal(self, module, new_value, appendix=[]):
        ...
        try:
            ...
            module._logger.error('Erro in launch_signal of %s: %s', module.name, e)
    ...
    def _create_widget(self, module, widget_name=None):
        ...
        if self._widget_class is None:
            logger.warning('Module %s of type %s is trying to create a widget for %s, but no _widget_class is defined!', str(module), type(module), self.name)
            return
        else:
...
class LedProperty(BoolProperty):
    ...
    def __init__(self, true_function=None, false_function=None, **kwargs):
        """
        default: if provided, the value is initialized to it
        """
    ...
    def set_value(self, obj, val):
        try:
            ...
            obj._logger.debug('Cannot call %s of %s.%s: %s', 'true_function' if val else 'false_function', obj.name, self.name, e)
        else:
...
class NumberProperty(BaseProperty):
    ...
    def _create_widget(self, module, widget_name=None):
        widget = BaseProperty._create_widget(self, module, widget_name=widget_name)
        return widget
...
class IntProperty(NumberProperty):
    ...
    def __init__(self, min=-np.inf, max=np.inf, increment=1, log_increment=False, **kwargs):
        super(IntProperty, self).__init__(min=min, max=max, increment=increment, log_increment=log_increment, **kwargs)
    ...
    def validate_and_normalize(self, obj, value):
        ...
        if value is None:
            value = self.get_value(obj)
        return NumberProperty.validate_and_normalize(self, obj, int(round(value)))
...
class LongRegister(IntRegister):
    ...
    def set_value(self, obj, val):
        ...
        if self.bitmask is None:
            for i in range(self.size):
                values[i] = val >> 32 * i & 4294967295

        else:
            act = obj._reads(self.address, self.size)
            for i in range(self.size):
                localbitmask = self.bitmask >> 32 * i & 4294967295
...
class FloatProperty(NumberProperty):
    ...
    def validate_and_normalize(self, obj, value):
        ...
        """
        return NumberProperty.validate_and_normalize(self, obj, float(value))
...
class FloatRegister(IntRegister, FloatProperty):
    ...
    def __init__(self, address, bits=14, bitmask=None, norm=1.0, signed=True, invert=False, **kwargs):
    ...
    def validate_and_normalize(self, obj, value):
        ...
        if not self.signed:
            value = abs(value)
        return FloatProperty.validate_and_normalize(self, obj, round(value / self.increment) * self.increment)
    ...
    def validate_and_normalize(self, obj, value):
        ...
        if rounded_value == 0 and value != 0:
            if self.avoid_round_off_to_zero:
                rounded_value = FloatRegister.validate_and_normalize(self, obj, np.abs(self.increment) * np.sign(value))
                obj._logger.warning('Avoided rounding value %.1e of the gain register %s to zero. Setting it to %.1e instead. ', value, self.name, rounded_value)
            else:
                obj._logger.warning('Rounding value %.1e of the gain register %s to zero. ', value, self.name)
        if value > self.max or value < self.min:
            obj._logger.warning('Requested gain for %s.%s is outside the bounds allowed by the hardware. Desired gain of %.1e is capped to %.1e. ', obj.name, self.name, value, rounded_value)
...
class FrequencyRegister(FloatRegister, FrequencyProperty):
    ...
    def from_python(self, obj, value):
        ...
        if value == epsilon:
            ...
            value = int(round(value / self.CLOCK_FREQUENCY * 2 ** self.bits))
        return value

    def to_python(self, obj, value):
        return 125000000.0 / 2 ** self.bits * float(value) * obj._frequency_correction

    def validate_and_normalize(self, obj, value):
        """
        Same as FloatRegister, except the value should be positive.
        """
        return FrequencyProperty.validate_and_normalize(self, obj, FloatRegister.validate_and_normalize(self, obj, value))
...
class PhaseProperty(FloatProperty):
    ...
    def validate_and_normalize(self, obj, value):
        ...
        """
        return FloatProperty.validate_and_normalize(self, obj, value % 360.0)
...
class PhaseRegister(FloatRegister, PhaseProperty):
    ...
    def __init__(self, address, bits=32, bitmask=None, invert=False, **kwargs):
        FloatRegister.__init__(self, address=address, bits=bits, bitmask=bitmask, invert=invert)
        PhaseProperty.__init__(self, increment=(360.0 / 2 ** bits), **kwargs)
    ...
    def validate_and_normalize(self, obj, value):
        ...
        """
        return int(round(float(value) / 360 * 2 ** self.bits)) / 2 ** self.bits * 360.0 % 360.0
...
class FilterProperty(BaseProperty):
    ...
    def valid_frequencies(self, module):
        raise NotImplementedError('this is a baseclass, your derived class must implement the following function')
...
class FilterRegister(BaseRegister, FilterProperty):
    ...
    def valid_frequencies(self, obj):
        ...
        pos = list([ self.to_python(obj, b | 128) for b in valid_bits ])
        pos = [ val if not np.iterable(val) else val[0] for val in pos ]
        neg = [ -val for val in reversed(pos) ]
        valid_frequencies = neg + [0] + pos
        if obj is not None and not hasattr(obj.__class__, self.name + '_options') and not hasattr(obj, self.name + '_options'):
            setattr(obj, self.name + '_options', valid_frequencies)
        return valid_frequencies

    correction_factors = {0.5: 0.7, 0.25: 1.65, 
       0.125: 1.17, 
       0.0625: 1.08, 
       0.03125: 1.04, 
       0.015625: 1.02, 
    ...
    def to_python(self, obj, value):
        ...
        for i in range(self._FILTERSTAGES(obj)):
            v = filter_shifts >> i * 8 & 255
            shift = v & 2 ** self._SHIFTBITS(obj) - 1
            filter_on = v >> 7 == 1
            highpass = v >> 6 & 1 == 1
            if filter_on:
                alpha = float(2 ** shift) / 2 ** self._MAXSHIFT(obj)
                bandwidth = -np.log(1.0 - alpha) / 2.0 / np.pi * 125000000.0
    ...
    def from_python(self, obj, value):
        ...
        for i in range(self._FILTERSTAGES(obj)):
            ...
            if bandwidth == 0:
                ...
                alpha = 1.0 - np.exp(-np.abs(bandwidth) * 2.0 * np.pi / 125000000.0)
                if alpha in self.correction_factors:
                    bandwidth /= self.correction_factors[alpha]
                    alpha = 1.0 - np.exp(-np.abs(bandwidth) * 2.0 * np.pi / 125000000.0)
...
class PWMRegister(FloatRegister):
    ...
    def __init__(self, address, CFG_BITS=24, PWM_BITS=8, **kwargs):
        ...
        self.min = 0
        self.max = 1.8
        self.increment = (self.max - self.min) / 2 ** (self.bits - 1)
    ...
    def to_python(self, obj, value):
        ...
        if voltage > 1.8:
            logger.error('Readout value from PWM (%h) yields wrong voltage %f', value, voltage)
        return voltage
...
class SelectProperty(BaseProperty):
    ...
    def get_default(self, instance):
        ...
        if default is None:
            logger.warning('Default of SelectProperty %s is None. ', self.name)
        return default
...
class SelectRegister(BaseRegister, SelectProperty):
    ...
    def get_value(self, obj):
        ...
        if raw_value != expected_value:
            obj._logger.warning("Register %s of module %s has value %s, which does not correspond to selected option %s. Setting to '%s'. ", self.name, obj.name, raw_value, expected_value, value)
...
class ProxyProperty(BaseProperty):
    ...
    def __init__(self, path_to_target, **kwargs):
        self.path_to_target = path_to_target
        lastpart = path_to_target.split('.')[(-1)]
        self.target_attribute = lastpart
        self.path_to_target_module = path_to_target[:-(len(lastpart) + 1)]
        self.path_to_target_descriptor = self.path_to_target_module + '.__class__.' + lastpart
        BaseProperty.__init__(self, **kwargs)
    ...
    def __get__(self, instance, owner):
        if instance is None:
            ...
            return self._target_to_proxy(instance, recursive_getattr(instance, self.path_to_target))

    def __set__(self, obj, value):
        self.instance = obj
        self.connect_signals(obj)
        recursive_setattr(obj, self.path_to_target, self._proxy_to_target(obj, value))

    def __getattribute__(self, item):
        try:
            return BaseProperty.__getattribute__(self, item)
        except AttributeError:
            attr = recursive_getattr(self.instance, self.path_to_target_descriptor + '.' + item)
            return attr
    ...
    def options(self, obj):
        ...
        options = recursive_getattr(obj, self.path_to_target_descriptor + '.options')(module)
        return OrderedDict([ (self._target_to_proxy(obj, k), v) for k, v in options.items()
                           ])
    ...
    def __repr__(self):
        try:
            targetdescr = ' (target: ' + recursive_getattr(self.instance, self.path_to_target_descriptor).__repr__() + ')'
        except:
            targetdescr = ''
    ...
    def connect_signals(self, instance):
        ...
        def forward_update_attribute_by_name(name, value):
            ...
            if name == self.target_attribute:
                instance._signal_launcher.update_attribute_by_name.emit(self.name, [
                 self._target_to_proxy(instance, value[0])])
                if self.call_setup:
                    instance.setup()

        module._signal_launcher.update_attribute_by_name.connect(forward_update_attribute_by_name)
        ...
        module._signal_launcher.change_options.connect(forward_change_options)
    ...
    def _create_widget(self, module, widget_name=None, **kwargs):
        ...
        self._widget_class = recursive_getattr(module, self.path_to_target_descriptor + '._widget_class')
        try:
            return recursive_getattr(module, self.path_to_target_descriptor + '.__class__._create_widget')(self, module, **kwargs)
        except:
            return recursive_getattr(module, self.path_to_target_descriptor + '.__class__._create_widget')(target_descriptor, target_module, widget_name=widget_name, **kwargs)
...
class CurveSelectProperty(SelectProperty):
    ...
    def __init__(self, no_curve_first=False, show_childs=False, **kwargs):
    ...
    def _default_options(self):
        if self.no_curve_first:
            return [
             -1] + [ curve.pk for curve in CurveDB.all() ]
