...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyrpl/widgets/module_widgets/base_module_widget.py
...
class SaveLabel(MyMenuLabel):
    ...
    def new_state(self):
        state, accept = QtWidgets.QInputDialog.getText(self, 'Save %s state' % self.module.name, 'Enter new state name:')
        state = str(state)
        if accept:
    ...
    def init_attribute_layout(self):
        ...
        for attr_name in self.module._gui_attributes:
            if attr_name == '\n':
                ...
                attribute_value = getattr(self.module, attr_name)
...
class ModuleWidget(ReducedModuleWidget):
    ...
    def set_title(self, title):
        ...
        if hasattr(self, 'title_label'):
            ...
            self.erase_label.move(self.save_label.width() + self.save_label.pos().x(), self.title_pos[1])
            self.edit_label.move(self.erase_label.width() + self.erase_label.pos().x(), self.title_pos[1])
            self.hideshow_label.move(self.edit_label.width() + self.edit_label.pos().x(), self.title_pos[1])
