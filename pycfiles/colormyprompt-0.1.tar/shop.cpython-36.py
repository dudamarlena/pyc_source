...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/colormeshop/models/shop.py
# Compiled at: 2018-07-29 03:49:51
# Size of source mod 2**32: 31977 bytes
...
class Shop(object):
    ...
    def state(self, state):
        ...
        """
        allowed_values = [
         'enabled', 'suspended', 'unsigned']
        if state not in allowed_values:
            raise ValueError('Invalid value for `state` ({0}), must be one of {1}'.format(state, allowed_values))
        self._state = state
    ...
    def domain_plan(self, domain_plan):
        ...
        """
        allowed_values = [
         'cmsp_sub_domain', 'own_domain', 'own_sub_domain']
        if domain_plan not in allowed_values:
            raise ValueError('Invalid value for `domain_plan` ({0}), must be one of {1}'.format(domain_plan, allowed_values))
    ...
    def contract_plan(self, contract_plan):
        ...
        """
        allowed_values = [
         'unknown', 'economy', 'small', 'regular', 'lolipop', 'heteml', 'platinum', 'goope', 'large']
        if contract_plan not in allowed_values:
            raise ValueError('Invalid value for `contract_plan` ({0}), must be one of {1}'.format(contract_plan, allowed_values))
    ...
    def open_state(self, open_state):
        ...
        """
        allowed_values = [
         'opened', 'closed', 'prepare', 'paused']
        if open_state not in allowed_values:
            raise ValueError('Invalid value for `open_state` ({0}), must be one of {1}'.format(open_state, allowed_values))
    ...
    def mobile_open_state(self, mobile_open_state):
        ...
        if mobile_open_state not in allowed_values:
            raise ValueError('Invalid value for `mobile_open_state` ({0}), must be one of {1}'.format(mobile_open_state, allowed_values))
