...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\sc2maptool\cli.py
# Compiled at: 2018-10-07 15:15:56
...
def optionsParser(passedParser=None):
    ...
    matchOptn.add_argument('--season', default=None, type=int, choices=[1, 2, 3, 4], help='the specific ladder season within a calendar year.')
    matchOptn.add_argument('--mode', default=None, choices=['1v1', '2v2', '3v3', '4v4'], help='the official ladder category to play.')
    return parser


def getSelectionParams(options):
    return {k:v for k, v in options._get_kwargs() if v != None if k in c.INCLUDED_KEYS if k in c.INCLUDED_KEYS}
...
def main():
    ...
    if options.list or options.details:
        specifiedMaps = filterMapNames((options.mapname),
 ...
 options.mapname, excludeName=options.exclude, 
             closestMatch=options.best, **params)
