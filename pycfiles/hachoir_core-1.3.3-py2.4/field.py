...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_core/field/field.py
...
class Field(Logger):
        ...
        if self._description is None:
            try:
                ...
                if isinstance(self._description, str):
                    self._description = makePrintable(self._description, 'ISO-8859-1', to_unicode=True)
    ...
    description = property(_getDescription, doc='Description of the field (string)')
    ...
    def __repr__(self):
        return '<%s path=%r, address=%s, size=%s>' % (self.__class__.__name__, self.path, self._address, self._size)
    ...
    display = property(lambda self: self._getDisplay(), doc='Short (unicode) string which represents field content')
    ...
    raw_display = property(lambda self: self._getRawDisplay(), doc='(Unicode) string which represents raw field content')

    def _getName(self):
        return self._name

    name = property(_getName, doc='Field name (unique in its parent field set list)')
    ...
    path = property(_getPath, doc='Full path of the field starting at root field')

    def _getAddress(self):
        return self._address

    address = property(_getAddress, doc='Relative address in bit to parent address')
    ...
    absolute_address = property(_getAbsoluteAddress, doc='Absolute address (from stream beginning) in bit')
