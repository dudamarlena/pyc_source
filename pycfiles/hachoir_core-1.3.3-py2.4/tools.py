...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_core/tools.py
...
controlchars = tuple(({ord('\n'): '\\n', ord('\r'): '\\r', ord('\t'): '\\t', ord('\x07'): '\\a', ord('\x08'): '\\b'}.get(code, '\\x%02x' % code) for code in xrange(128)))
...
def makePrintable(data, charset, quote=None, to_unicode=False, smart=True):
    ...
    if data:
        if not isinstance(data, unicode):
            ...
            charset = 'ASCII'
        data = regex_control_code.sub(lambda regs: controlchars[ord(regs.group(1))], data)
        if quote:
            if quote in '"\'':
...
def humanUnixAttributes(mode):
    ...
    chars = [
     ftypelet(mode), 'r', 'w', 'x', 'r', 'w', 'x', 'r', 'w', 'x']
