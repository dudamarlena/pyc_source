...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_core/field/padding.py
...
class PaddingBits(Bits):
    ...
    def checkPattern(self):
        ...
        if self.MAX_SIZE < self._size:
            value = self._parent.stream.readBits(self.absolute_address, self.MAX_SIZE, self._parent.endian)
        else:
            value = self.value
        if value != 0:
...
class PaddingBytes(Bytes):
    ...
    Optional arguments:
     * pattern (str): Content pattern, eg. "\x00" for nul bytes
    """
    __module__ = __name__
...
class NullBytes(PaddingBytes):
    ...
    def __init__(self, parent, name, nbytes, description=None):
        PaddingBytes.__init__(self, parent, name, nbytes, description, pattern='\x00')
