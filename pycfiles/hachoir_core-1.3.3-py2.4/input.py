...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_core/stream/input.py
...
class InputStream(Logger):
    ...
    def sizeGe(self, size, const=False):
        return self._current_size >= size or not (None < self._size < size or const or self._feed(size))
...
class InputPipe(object):
    ...
    def read(self, size):
        ...
        data = ('').join((self._get(index) for index in xrange(block, (end - 1 >> self.buffer_size) + 1)))[offset:offset + size]
        self._flush()
