...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_core/field/float.py
...
assert struct.unpack('>d', b'\xc0\x00\x00\x00\x00\x00\x00\x00')[0] == -2.0
    ...
    class Float(FieldSet):
        ...
        def createValue(self):
            ...
            if self.struct_format:
                raw = self._parent.stream.readBytes(self.absolute_address, self._size // 8)
                try:
                    return struct.unpack(self.struct_format, raw)[0]
                except struct.error, err:
                    raise ValueError('[%s] conversion error: %s' % (self.__class__.__name__, err))
                ...
                try:
                    ...
                    raise ValueError('[%s] floating point overflow' % self.__class__.__name__)
...
Float32 = floatFactory('Float32', 'f', 23, 8, 'Floating point number: format IEEE 754 int 32 bit')
Float64 = floatFactory('Float64', 'd', 52, 11, 'Floating point number: format IEEE 754 in 64 bit')
Float80 = floatFactory('Float80', None, 64, 15, 'Floating point number: format IEEE 754 in 80 bit')