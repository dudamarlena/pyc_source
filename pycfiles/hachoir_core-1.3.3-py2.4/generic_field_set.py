...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_core/field/generic_field_set.py
# Compiled at: 2009-09-07 17:44:28
from hachoir_core.field import MissingField, BasicFieldSet, Field, ParserError, createRawField, createNullField, createPaddingField, FakeArray
...
class GenericFieldSet(BasicFieldSet):
    ...
    def __str__(self):
        return '<%s path=%s, current_size=%s, current length=%s>' % (self.__class__.__name__, self.path, self._current_size, len(self._fields))
    ...
    def replaceField(self, name, new_fields):
        ...
        if old_field.size != total_size:
            raise ParserError("Unable to replace %s: new field(s) hasn't same size (%u bits instead of %u bits)!" % (name, total_size, old_field.size))
        field = new_fields[0]
        if field._name.endswith('[]'):
            self.setUniqueFieldName(field)
        field._address = old_field.address
        if field.name != name and field.name in self._fields:
            raise ParserError('Unable to replace %s: name "%s" is already used!' % (name, field.name))
