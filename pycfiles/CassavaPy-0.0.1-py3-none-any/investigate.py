...
# Embedded file name: /Users/moses/GitHub/cassava_public/cassava/investigate/investigate.py
# Compiled at: 2015-08-04 20:17:17
import json, re, requests, urlparse
...
class Investigate(object):
    ...
    UNSUPPORTED_DNS_QUERY = ValueError(('supported query types are: {}').format(SUPPORTED_DNS_TYPES))

    def __init__(self, api_key, proxies={}):
        self.api_key = api_key
        self.proxies = proxies
        self._uris = {'categorization': 'domains/categorization/', 
    ...
    def get(self, uri, params={}):
        ...
        on the given URI.
        """
        return requests.get(urlparse.urljoin(Investigate.BASE_URL, uri), params=params, headers=self._auth_header, proxies=self.proxies)

    def post(self, uri, params={}, data={}):
        """A generic method to make POST requests to the OpenDNS Investigate API
        on the given URI.
        """
        return requests.post(urlparse.urljoin(Investigate.BASE_URL, uri), params=params, data=data, headers=self._auth_header, proxies=self.proxies)
    ...
    def categorization(self, domains, labels=False):
        ...
        """
        if type(domains) is str or type(domains) is unicode:
