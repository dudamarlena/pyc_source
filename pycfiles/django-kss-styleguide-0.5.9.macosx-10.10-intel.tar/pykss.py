...
# [GCC 8.4.0]
# Embedded file name: /Users/tim/Projects/django_kss/VENV/lib/python2.7/site-packages/django_kss/templatetags/pykss.py
# Compiled at: 2014-12-20 07:08:14
...
class BaseStyleguideNode(template.Node):
    ...
    def as_tag(cls, parser, token):
        ...
        if len(bits) < 2:
            ...
            raise template.TemplateSyntaxError('styleguideblock expected using as the third argument')
        elif len(bits) == 3:
            raise template.TemplateSyntaxError("styleguideblock expects a template name after 'using'")
        else:
    ...
    def render(self, context):
        ...
        sections = sorted([ sec for ref, sec in styleguide.sections.iteritems() if ref.startswith(reference)
...
class StyleguideBlockNode(BaseStyleguideNode):
    ...
    def dispatch(cls, parser, styleguide, reference, template_name):
        ...
        return cls(styleguide=parser.compile_filter(styleguide), reference=parser.compile_filter(reference), template_name=parser.compile_filter(template_name), nodelist=nodelist)
...
class RenderStyleguideNode(BaseStyleguideNode):
    ...
    @classmethod
    def dispatch(cls, parser, styleguide, reference, template_name):
        return cls(styleguide=parser.compile_filter(styleguide), reference=parser.compile_filter(reference), template_name=parser.compile_filter(template_name), nodelist=None)
