...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pirogue/simple_joins.py
...
class SimpleJoins:
    ...
    def __init__(self, definition: dict, pg_service: str=None):
        ...
        for alias, table_def in definition['joins'].items():
            for key in table_def.keys():
                if key not in ('table', 'short_alias', 'fkey', 'prefix', 'skip_columns',
                               'remap_columns'):
        ...
        for alias, table_def in definition['joins'].items():
            ...
            try:
                child.parent_referenced_key, child.ref_parent_key = reference_columns(self.cursor, self.parent_schema, self.parent_table, child.schema_name, child.table_name)
    ...
    def __view(self) -> str:
        ...
        sql = '\nCREATE OR REPLACE VIEW {vs}.{vn} AS SELECT\n  {parent_cols},\n  {child_cols}\n  FROM {ps}.{pt}\n  {joins};\n'.format(vs=(self.view_schema), vn=(self.view_name),
          ...
          child_cols=(',\n  '.join([select_columns((self.cursor), (child_def.schema_name), (child_def.table_name), table_alias=alias, remap_columns=(child_def.remap_columns), prefix=(child_def.prefix)) for alias, child_def in self.child_tables.items()])),
          ps=(self.parent_schema),
          pt=(self.parent_table),
          joins=('\n  '.join(['LEFT JOIN {cs}.{ct} {alias} ON {alias}.{cpk} = {pt}.{rpk}'.format(cs=(child.schema_name), ct=(child.table_name), alias=alias, cpk=(child.pkey), pt=(self.parent_table), rpk=(child.parent_referenced_key)) for alias, child in self.child_tables.items()])))
        return sql