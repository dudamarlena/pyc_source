...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/scripts/pirogue.py
...
def main():
    ...
    single_inheritance_parser.add_argument('-d', '--pkey-default-value', action='store_true', help='The primary key column of the view will have a default value according to the child primary key table')
    ...
    multiple_inheritance_parser.add_argument('-j', '--create-joins', action='store_true', help='Create simple join view for all joined tables.')
    multiple_inheritance_parser.add_argument('-d', '--drop', action='store_true', help='Drop existing views, type and triggers.')
    multiple_inheritance_parser.add_argument('-v', '--var', nargs=3, help='Assign variable for running SQL deltas. Format is: (string|float|int) name value. ',
      action='append',
      default=[])
    ...
    if args.command == 'single_inheritance':
        success = SingleInheritance(parent_table=(args.parent_table), child_table=(args.child_table),
        ...
        if args.command == 'multiple_inheritance':
            ...
            MultipleInheritance(yaml_definition, variables=variables,
              create_joins=(args.create_joins),
              pg_service=pg_service).create()
        else:
            if args.command == 'simple_joins':
                yaml_definition = yaml.safe_load(args.definition_file)
