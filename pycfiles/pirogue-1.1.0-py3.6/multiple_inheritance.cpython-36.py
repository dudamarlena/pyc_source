...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pirogue/multiple_inheritance.py
# Compiled at: 2019-11-29 08:49:00
# Size of source mod 2**32: 26282 bytes
...
class MultipleInheritance:
    ...
    def __init__(self, definition: dict, pg_service: str=None, variables: dict={}, create_joins: bool=False, drop: bool=False):
        """
        ...
        for key in definition.keys():
            if key not in ('table', 'view_schema', 'view_name', 'alias', 'short_alias',
                           'type_name', 'joins', 'insert_trigger', 'update_trigger',
                           'allow_type_change', 'allow_parent_only', 'additional_columns',
                           'additional_joins', 'merge_columns', 'merge_geometry_columns',
                           'pkey_default_value'):
        ...
        for alias, table_def in definition['joins'].items():
            for key in table_def.keys():
                if key not in ('table', 'short_alias', 'skip_columns', 'fkey', 'remap_columns',
                               'prefix', 'insert_values', 'update_values'):
        ...
        for alias, table_def in self.joins.items():
            ...
            if 'fkey' in table_def:
                ...
                table_def['ref_master_key'] = reference_columns(self.cursor, table_def['table_schema'], table_def['table_name'], self.master_schema, self.master_table)[0]
            try:
    ...
    def create(self) -> bool:
        ...
        success = True
        if self.drop:
            queries.append(self._MultipleInheritance__drops())
        queries.append(self._MultipleInheritance__type())
        queries.append(self._MultipleInheritance__view())
        queries.append(self._MultipleInheritance__insert_trigger())
        queries.append(self._MultipleInheritance__update_trigger())
        queries.append(self._MultipleInheritance__delete_trigger())
        queries.append(self._MultipleInheritance__extras())
        for sql in queries:
            if not sql:
                pass
            else:
                try:
                    ...
                    success = False
                    print('*** Failing:\n{}\n***'.format(sql))
                    raise VariableError('An error in a SQL variable is probable. Check the variables in the SQL code (were given: {svars}). Also, any % character shall be escaped with %%'.format(svars=(list(self.variables.keys()))))
        ...
        if self.create_joins:
            for alias, table_def in self.joins.items():
                success &= SingleInheritance(pg_service=(self.pg_service), parent_table='{s}.{t}'.format(s=(self.master_schema), t=(self.master_table)),
                  child_table='{s}.{t}'.format(s=(table_def['table_schema']), t=(table_def['table_name'])),
                  view_name='vw_{a}'.format(a=alias)).create()

        return success

    def __drops(self) -> str:
        sql = 'DROP VIEW IF EXISTS {vs}.{vn};DROP TYPE IF EXISTS {vs}.{tn};'.format(vs=(self.view_schema),
          tn=(self.type_name),
          vn=(self.view_name))
        return sql
    ...
    def __type(self) -> str:
        ...
        :return:
        """
        sql = "CREATE TYPE {vs}.{tn} AS ENUM ('{pt}', {ct} );".format(vs=(self.view_schema),
          tn=(self.type_name),
    ...
    def __view(self) -> str:
        ...
        sql = '\nCREATE OR REPLACE VIEW {vs}.{vn} AS\n  SELECT\n    CASE\n      {types}\n      ELSE {no_subtype}\n    END AS {type_name},\n    {master_columns},{merge_columns}\n    {joined_columns}{additional_columns}\n  FROM {mt}.{ms} {sa}\n    {joined_tables}{additional_joins};        \n'.format(vs=(self.view_schema), vn=(self.view_name),
          types=('\n      '.join(["WHEN {shal}.{mrf} IS NOT NULL THEN '{al}'::{vs}.{tn}".format(shal=(table_def['short_alias']), mrf=(table_def['ref_master_key']), al=alias, vs=(self.view_schema), tn=(self.type_name)) for alias, table_def in self.joins.items()])),
          no_subtype="'{type}'::{vs}.{tn}".format(type=(self.view_alias if self.allow_parent_only else 'unknown'), vs=(self.view_schema),
          tn=(self.type_name)),
          type_name=(self.type_name),
          master_columns=select_columns((self.cursor), (self.master_schema), (self.master_table), table_alias=(self.view_alias),
          skip_columns=(self.master_skip_colums),
          prefix=(self.master_prefix),
          remap_columns=(self.master_remap_columns),
          indent=4),
          merge_columns=('\n      '.join(['\n    CASE\n      {conditions}\n      ELSE NULL{cast}\n    END AS {col},'.format(col=col, conditions=('\n      '.join(['WHEN {ta}.{rmk} IS NOT NULL THEN {ta}.{col}'.format(ta=(table_def['short_alias']), rmk=(table_def['ref_master_key']), col=col) for alias, table_def in self.joins.items() if col in columns((self.cursor), (table_def['table_schema']), (table_def['table_name']), skip_columns=(table_def.get('skip_columns', [])))])),
          cast=(self.merge_column_cast.get(col, ''))) for col in self.merge_columns])),
          joined_columns=(',\n    '.join([select_columns((self.cursor), (table_def['table_schema']), (table_def['table_name']), table_alias=(table_def['short_alias']), skip_columns=(table_def.get('skip_columns', []) + [table_def['ref_master_key']]), safe_skip_columns=(self.merge_columns), prefix=(table_def.get('prefix', None)), remove_pkey=False, remap_columns=(table_def.get('remap_columns', {})), indent=4) for alias, table_def in self.joins.items()])),
          ...
          sa=(self.short_alias),
          joined_tables=('\n    '.join(['LEFT JOIN {tbl} {tal} ON {tal}.{rmk} = {msa}.{mpk}'.format(tbl=(table_def['table']), tal=(table_def['short_alias']), rmk=(table_def['ref_master_key']), msa=(self.short_alias), mpk=(self.master_pkey)) for table_def in self.joins.values()])),
    ...
    def __insert_trigger(self) -> str:
        ...
        sql = '-- INSERT TRIGGER\nCREATE OR REPLACE FUNCTION {vs}.ft_{vn}_insert() RETURNS trigger AS\n$BODY$\nDECLARE\n  {declare}\nBEGIN\n  {insert_trigger_pre}\n  {insert_master}\n\n  CASE \n    {insert_joins}\n  ELSE\n    {raise_notice}\n  END CASE;\n\n  {insert_trigger_post}\nRETURN NEW;\nEND;\n$BODY$\nLANGUAGE plpgsql;\n\nDROP TRIGGER IF EXISTS tr_{vn}_on_insert ON {vs}.{vn};\n\nCREATE TRIGGER tr_{vn}_on_insert\n    INSTEAD OF INSERT ON {vs}.{vn}\n    FOR EACH ROW EXECUTE PROCEDURE {vs}.ft_{vn}_insert();\n'.format(vs=(self.view_schema), vn=(self.view_name),
          ...
          insert_master=insert_command((self.cursor), (self.master_schema), (self.master_table), skip_columns=(self.master_skip_colums),
          prefix=(self.master_prefix),
          remap_columns=(self.master_remap_columns),
          remove_pkey=False,
          indent=8,
          ...
          insert_joins=('\n    '.join(["WHEN NEW.{type_name} = '{alias}'::{vs}.{type_name} THEN\n      {insert_join}".format(type_name=(self.type_name), alias=alias, vs=(self.view_schema), insert_join=insert_command((self.cursor), (table_def['table_schema']), (table_def['table_name']), table_alias=(table_def['short_alias']), skip_columns=(table_def.get('skip_columns', [])), prefix=(table_def.get('prefix', None)), insert_values={**{table_def['ref_master_key']: 'NEW.{c}'.format(c=(self.master_pkey))}, **(table_def.get('insert_values', {}))}, remap_columns=(table_def.get('remap_columns', {})), remove_pkey=False, indent=4)) for alias, table_def in self.joins.items()])),
          raise_notice=('NULL;' if self.allow_parent_only else "RAISE NOTICE '{vn} type not known ({percent_char})', NEW.{type_name}; -- ERROR".format(vn=(self.view_name),
    ...
    def __update_trigger(self):
        sql = '-- UPDATE TRIGGER\nCREATE OR REPLACE FUNCTION {vs}.ft_{vn}_update() RETURNS trigger AS\n$BODY$\nDECLARE\n  {declare}\nBEGIN\n  {update_trigger_pre}\n  {update_master}\n\n  IF OLD.{type_name} <> NEW.{type_name} THEN\n    {type_change}\n  END IF;\n\n  CASE {update_joins}\n  ELSE\n     {raise_notice}\n  END CASE;\n  {update_trigger_post}\nRETURN NEW;\nEND;\n$BODY$\nLANGUAGE plpgsql;\n\nDROP TRIGGER IF EXISTS tr_{vn}_on_update ON {vs}.{vn};\n\nCREATE TRIGGER tr_{vn}_on_update\n    INSTEAD OF update ON {vs}.{vn}\n    FOR EACH ROW EXECUTE PROCEDURE {vs}.ft_{vn}_update();\n        '.format(vs=(self.view_schema), vn=(self.view_name),
          ...
          update_master=update_command((self.cursor), (self.master_schema), (self.master_table), skip_columns=(self.master_skip_colums),
          prefix=(self.master_prefix),
          remap_columns=(self.master_remap_columns),
          indent=8),
          type_name=(self.type_name),
          type_change=("RAISE EXCEPTION 'Type change not allowed for {alias}'\n      USING HINT = 'You cannot switch from ' || OLD.{type_name} || ' to ' || NEW.{type_name};".format(alias=(self.view_alias), type_name=(self.type_name)) if not self.allow_type_change else 'CASE\n      {deletes}\n    END CASE;\n    CASE\n      {inserts}\n      ELSE -- do nothing\n    END CASE;'.format(deletes=('\n      '.join(["WHEN OLD.{type_name} = '{alias}'::{vs}.{type_name} THEN DELETE FROM {ts}.{tn} WHERE {rmk} = OLD.{mpk};".format(type_name=(self.type_name), alias=alias, vs=(self.view_schema), ts=(table_def['table_schema']), tn=(table_def['table_name']), rmk=(table_def['ref_master_key']), mpk=(self.master_pkey)) for alias, table_def in self.joins.items()])),
          inserts=('\n      '.join(["WHEN NEW.{type_name} = '{alias}'::{vs}.{type_name} THEN INSERT INTO {ts}.{tn} ({rmk}) VALUES (OLD.{mpk});".format(type_name=(self.type_name), alias=alias, vs=(self.view_schema), ts=(table_def['table_schema']), tn=(table_def['table_name']), rmk=(table_def['ref_master_key']), mpk=(self.master_pkey)) for alias, table_def in self.joins.items()])))),
          update_joins=('\n    '.join(["WHEN NEW.{type_name} = '{alias}'::{vs}.{type_name} THEN\n      {update_join}".format(type_name=(self.type_name), alias=alias, vs=(self.view_schema), update_join=update_command((self.cursor), (table_def['table_schema']), (table_def['table_name']), table_alias=(table_def['short_alias']), pkey=(table_def['pkey']), skip_columns=(table_def.get('skip_columns', [])), prefix=(table_def.get('prefix', None)), remap_columns=(table_def.get('remap_columns', {})), update_values={**{table_def['pkey']: 'OLD.{c}'.format(c=(self.master_pkey))}, **(table_def.get('update_values', {}))}, indent=4)) for alias, table_def in self.joins.items()])),
          raise_notice=('NULL;' if self.allow_parent_only else "RAISE NOTICE '{vn} type not known ({percent_char})', NEW.{type_name}; -- ERROR".format(vn=(self.view_name),
          percent_char=('%%' if self.variables else '%'),
          type_name=(self.type_name))),
    ...
    def __extras(self):
        ...
        if self.pkey_default_value:
            sql += 'ALTER VIEW {vs}.{vn} ALTER {master_pkey} SET DEFAULT {dv};'.format(vs=(self.view_schema),
              vn=(self.view_name),
