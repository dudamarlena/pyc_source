...
# Size of source mod 2**32: 11788 bytes
import logging
...
def add():
    ...
    if isinstance(formatted_patient, int):
        ...
        if controllers.get_nodes(database=(current_app.db)):
            match_external = True
        modified, inserted, matching_obj = backend_add_patient(mongo_db=(current_app.db), patient=formatted_patient, match_external=match_external,
        ...
        if current_app.config.get('MAIL_SERVER'):
            if matching_obj:
                if len(matching_obj.get('results')):
...
def match_external(patient_id):
    ...
    if node:
        if not current_app.db['nodes'].find({'_id': node}).count():
...
def asynch_match_response():
    ...
    if isinstance(data, int):
        ...
        if async_match_obj:
            ...
            resp.status_code = 200
            return resp
        if len(async_match_obj['results']) > 0:
            if current_app.config.get('MAIL_SERVER'):
                notify_match_external(match_obj=async_match_obj, admin_email=(current_app.config.get('MAIL_USERNAME')), mail=(current_app.mail),
                  notify_complete=(current_app.config.get('NOTIFY_COMPLETE')))
...
def match_internal():
    ...
    if isinstance(query_patient, int):
        ...
        if current_app.config.get('MAIL_SERVER'):
            if len(matches):
                notify_match_internal(database=(current_app.db), match_obj=match_obj, admin_email=(current_app.config.get('MAIL_USERNAME')),
