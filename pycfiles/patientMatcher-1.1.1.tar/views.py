...
# [GCC 8.4.0]
# Embedded file name: /Users/chiararasi/Documents/work/GITs/patientMatcher/patientMatcher/server/views.py
# Compiled at: 2019-04-23 06:15:21
import logging
...
def add():
    ...
    if isinstance(formatted_patient, int):
        return controllers.bad_request(formatted_patient)
    match_external = False
    if controllers.get_nodes(database=current_app.db):
        match_external = True
    modified, inserted, matching_obj = backend_add_patient(mongo_db=current_app.db, patient=formatted_patient, match_external=match_external, host=current_app.config.get('MME_HOST'))
    message = {}
    if modified:
    ...
    if current_app.config.get('MAIL_SERVER') and matching_obj and len(matching_obj.get('results')):
        notify_match_external(match_obj=matching_obj, admin_email=current_app.config.get('MAIL_USERNAME'), mail=current_app.mail, notify_complete=current_app.config.get('NOTIFY_COMPLETE'))
...
def asynch_match_response():
    ...
    if len(async_match_obj['results']) > 0 and current_app.config.get('MAIL_SERVER'):
        notify_match_external(match_obj=async_match_obj, admin_email=current_app.config.get('MAIL_USERNAME'), mail=current_app.mail, notify_complete=current_app.config.get('NOTIFY_COMPLETE'))
...
def match_internal():
    ...
    if isinstance(query_patient, int):
        return controllers.bad_request(query_patient)
    LOG.info('Matching a query patient against database')
    max_pheno_score = current_app.config.get('MAX_PHENO_SCORE', 0.25)
    max_geno_score = current_app.config.get('MAX_GT_SCORE', 0.75)
    ...
    if current_app.config.get('MAIL_SERVER') and len(matches):
        notify_match_internal(database=current_app.db, match_obj=match_obj, admin_email=current_app.config.get('MAIL_USERNAME'), mail=current_app.mail, notify_complete=current_app.config.get('NOTIFY_COMPLETE'))
    validate_response = controllers.validate_response({'results': matches})
    if isinstance(validate_response, int):
