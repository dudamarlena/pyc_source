...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/misc/lnk.py
...
from hachoir_core.field import FieldSet, CString, String, UInt32, UInt16, UInt8, Bit, Bits, PaddingBits, TimestampWin64, DateTimeMSDOS32, NullBytes, PaddingBytes, RawBytes, Enum
...
class ItemId(FieldSet):
    __module__ = __name__
    ITEM_TYPE = {31: 'GUID', 35: 'Drive', 37: 'Drive', 41: 'Drive', 46: 'GUID', 47: 'Drive', 48: 'Dir/File', 49: 'Directory', 50: 'File', 52: 'File [Unicode Name]', 65: 'Workgroup', 66: 'Computer', 70: 'Net Provider', 71: 'Whole Network', 97: 'MSITStore', 112: 'Printer/RAS Connection', 177: 'History/Favorite', 195: 'Network Share'}
    ...
    def createFields(self):
        ...
        if entrytype in (31, 46, 112):
            yield RawBytes(self, 'dummy', 1, 'should be 0x50')
            yield GUID(self, 'guid')
        elif entrytype in (35, 37, 41, 47):
            yield String(self, 'drive', self['length'].value - 3, strip='\x00')
        elif entrytype in (48, 49, 50):
...
def formatVolumeSerial(field):
    val = field.value
    return '%04X-%04X' % (val >> 16, val & 65535)


class LocalVolumeTable(FieldSet):
    __module__ = __name__
    VOLUME_TYPE = {1: 'No root directory', 2: 'Removable (Floppy, Zip, etc.)', 3: 'Fixed (Hard disk)', 4: 'Remote (Network drive)', 5: 'CD-ROM', 6: 'Ram drive'}
...
class FileLocationInfo(FieldSet):
    ...
    def createFields(self):
        ...
        has_unicode_paths = self['first_offset_pos'].value == 36
...
class ExtraInfo(FieldSet):
    ...
    def createFields(self):
        ...
        if self['signature'].value == 2684354563:
            yield UInt32(self, 'remaining_length')
            yield UInt32(self, 'unknown[]')
            yield String(self, 'hostname', 16, 'Computer hostname on which shortcut was last modified', strip='\x00')
            yield RawBytes(self, 'unknown[]', 32)
            yield RawBytes(self, 'unknown[]', 32)
        elif self['signature'].value == 2684354565:
        ...
        elif self['signature'].value in (2684354561, 2684354566, 2684354567):
            if self['signature'].value == 2684354561:
                object_name = 'target'
            elif self['signature'].value == 2684354566:
            ...
            yield CString(self, object_name + '_unicode', 'Data (Unicode format)', charset='UTF-16-LE', truncate='\x00')
            remaining = self['length'].value - self.current_size / 8
            if remaining:
                yield RawBytes(self, 'slack_space[]', remaining, 'Data beyond end of string')
        elif self['signature'].value == 2684354562:
            ...
            yield String(self, 'font_name_unicode', 64, 'Font Name (Unicode format)', charset='UTF-16-LE', truncate='\x00')
        ...
        elif self['signature'].value == 2684354564:
...
HOT_KEYS = {0: 'None', 19: 'Pause', 20: 'Caps Lock', 33: 'Page Up', 34: 'Page Down', 35: 'End', 36: 'Home', 37: 'Left', 38: 'Up', 39: 'Right', 40: 'Down', 45: 'Insert', 46: 'Delete', 106: 'Num *', 107: 'Num +', 109: 'Num -', 110: 'Num .', 111: 'Num /', 144: 'Num Lock', 145: 'Scroll Lock', 186: ';', 187: '=', 188: ',', 189: '-', 190: '.', 191: '/', 192: '`', 219: '[', 220: '\\', 221: ']', 222: "'"}
...
class LnkFile(Parser):
    ...
    PARSER_TAGS = {'id': 'lnk', 'category': 'misc', 'file_ext': ('lnk', ), 'mime': ('application/x-ms-shortcut', ), 'magic': ((MAGIC, 0),), 'min_size': len(MAGIC) * 8, 'description': 'Windows Shortcut (.lnk)'}
    endian = LITTLE_ENDIAN
    SHOW_WINDOW_STATE = {0: 'Hide', 1: 'Show Normal', 2: 'Show Minimized', 3: 'Show Maximized', 4: 'Show Normal, not activated', 5: 'Show', 6: 'Minimize', 7: 'Show Minimized, not activated', 8: 'Show, not activated', 9: 'Restore', 10: 'Show Default'}

    def validate(self):
        if self['signature'].value != 76:
