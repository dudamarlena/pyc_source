...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/parser_list.py
...
class ParserList(object):
    __module__ = __name__
    VALID_CATEGORY = ('archive', 'audio', 'container', 'file_system', 'game', 'image',
                      'misc', 'program', 'video')
    ...
    def translate(self, name, value):
        if name in ('magic', ):
            ...
            if parser:
                return 'Duplicate parser id: %s already used by %s' % (value, parser[0].__name__)
