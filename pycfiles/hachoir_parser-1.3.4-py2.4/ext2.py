...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/file_system/ext2.py
...
from hachoir_core.field import FieldSet, ParserError, Bit, Bits, UInt8, UInt16, UInt32, Enum, String, TimestampUnix32, RawBytes, NullBytes
from hachoir_core.tools import alignValue, humanDuration, humanFilesize
...
class DirectoryEntry(FieldSet):
    __module__ = __name__
    file_type = {1: 'Regular', 2: 'Directory', 3: 'Char. dev.', 4: 'Block dev.', 5: 'Fifo', 6: 'Socket', 7: 'Symlink', 8: 'Max'}
    ...
    def createDescription(self):
        name = self['name'].value.strip('\x00')
        if name:
...
class Inode(FieldSet):
    __module__ = __name__
    inode_type_name = {1: 'list of bad blocks', 2: 'Root directory', 3: 'ACL inode', 4: 'ACL inode', 5: 'Boot loader', 6: 'Undelete directory', 8: 'EXT3 journal'}
    file_type = {1: 'Fifo', 2: 'Character device', 4: 'Directory', 6: 'Block device', 8: 'Regular', 10: 'Symbolic link', 12: 'Socket'}
    file_type_letter = {1: 'p', 4: 'd', 2: 'c', 6: 'b', 10: 'l', 12: 's'}
    ...
    def getMode(self):
        names = (
         ('owner_read', 'owner_write', 'owner_exec'), ('group_read', 'group_write', 'group_exec'), ('other_read', 'other_write', 'other_exec'))
        letters = 'rwx'
...
class SuperBlock(FieldSet):
    ...
    OS_LINUX = 0
    OS_HURD = 1
    os_name = {0: 'Linux', 1: 'Hurd', 2: 'Masix', 3: 'FreeBSD', 4: 'Lites', 5: 'WinNT'}
    state_desc = {1: 'Valid (Unmounted cleanly)', 2: 'Error (Errors detected)', 4: 'Orphan FS (Orphans being recovered)'}
    ...
    def createFields(self):
        ...
        yield String(self, 'volume_name', 16, 'Volume name', strip='\x00')
        yield String(self, 'last_mounted', 64, 'Directory where last mounted', strip='\x00')
...
class Group(FieldSet):
    ...
    def createFields(self):
        ...
        self.superblock_copy = self.stream.readBytes(addr, 2) == b'S\xef'
        ...
        addr = min(self.parent.size / 8, (self.uniq_id + 1) * superblock['blocks_per_group'].value * block_size)
...
class EXT2_FS(Parser):
    ...
    """
    __module__ = __name__
    PARSER_TAGS = {'id': 'ext2', 'category': 'file_system', 'description': 'EXT2/EXT3 file system', 'min_size': 1024 * 2 * 8, 'magic': ((b'S\xef\x01\x00', 1080 * 8), (b'S\xef\x02\x00', 1080 * 8), (b'S\xef\x04\x00', 1080 * 8))}
    endian = LITTLE_ENDIAN

    def validate(self):
        if self.stream.readBytes((1024 + 56) * 8, 2) != b'S\xef':
    ...
    def createFields(self):
        ...
        self.block_size = 1024 << superblock['log_block_size'].value
    ...
    def createDescription(self):
        ...
        if 'group[0]/inode_table/inode[7]/blocks' in self:
            if 0 < self['group[0]/inode_table/inode[7]/blocks'].value:
                ...
                desc = 'EXT2'
        return desc + ' file system: total=%s, used=%s, block=%s' % (humanFilesize(total), humanFilesize(used), humanFilesize(block_size))