...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/image/tiff.py
...
from hachoir_core.field import FieldSet, SeekableFieldSet, ParserError, RootSeekableFieldSet, UInt16, UInt32, Bytes, String
...
class IFD(FieldSet):
    ...
    def createFields(self):
        ...
        if MAX_COUNT < self['count'].value:
            raise ParserError('TIFF IFD: Invalid count (%s)' % self['count'].value)
    ...
    def createFields(self):
        ...
        for entry in self._ifd:
            ...
            for c in entry:
                ...
                if entryType == String:
                    yield String(self, entry.name, count, desc, '\x00', 'ISO-8859-1')
                else:
...
class TiffFile(RootSeekableFieldSet, Parser):
    __module__ = __name__
    PARSER_TAGS = {'id': 'tiff', 'category': 'image', 'file_ext': ('tif', 'tiff'), 'mime': ('image/tiff', ), 'min_size': 8 * 8, 'magic': (('II*\x00', 0), ('MM\x00*', 0)), 'description': 'TIFF picture'}
