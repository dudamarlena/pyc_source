...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/game/laf.py
...
from hachoir_core.field import FieldSet, UInt8, UInt16, UInt32, GenericVector
...
class CharBitmap(FieldSet):
    ...
    def createFields(self):
        ...
        for line in xrange(self.char['height_pixels'].value):
            yield GenericVector(self, 'line[]', width, UInt8, 'pixel')
...
class LafFile(Parser):
    __module__ = __name__
    PARSER_TAGS = {'id': 'lucasarts_font', 'category': 'game', 'file_ext': ('laf', ), 'min_size': 32 * 8, 'description': 'LucasArts Font'}
    ...
    def createFields(self):
        ...
        yield GenericVector(self, 'char_codes', self['num_chars'].value, UInt16, 'char')
        yield GenericVector(self, 'chars', self['num_chars'].value, CharInfo, 'char')
