...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/image/jpeg.py
...
from hachoir_core.field import FieldSet, ParserError, UInt8, UInt16, Enum, Bit, Bits, NullBits, NullBytes, String, RawBytes
...
class JpegChunkApp0(FieldSet):
    ...
    def createFields(self):
        ...
        if self['jfif'].value != 'JFIF\x00':
            raise ParserError("Stream doesn't look like JPEG chunk (wrong JFIF signature)")
...
class Ducky(FieldSet):
    __module__ = __name__
    BLOCK_TYPE = {0: 'end', 1: 'Quality', 2: 'Comment', 3: 'Copyright'}
...
class Comment(FieldSet):
    ...
    def createFields(self):
        yield String(self, 'comment', self.size // 8, strip='\x00')


class AdobeChunk(FieldSet):
    __module__ = __name__
    COLORSPACE_TRANSFORMATION = {1: 'YCbCr (converted from RGB)', 2: 'YCCK (converted from CMYK)'}
    ...
    def createFields(self):
        ...
        yield Enum(Bit(self, 'flag00'), {False: 'Chop down or subsampling', True: 'Blend'})
...
class JpegChunk(FieldSet):
    ...
    TAG_DQT = 219
    TAG_DRI = 221
    TAG_INFO = {196: ('huffman[]', 'Define Huffman Table (DHT)', None), 216: ('start_image', 'Start of image (SOI)', None), 217: ('end_image', 'End of image (EOI)', None), 218: ('start_scan', 'Start Of Scan (SOS)', StartOfScan), 219: ('quantization[]', 'Define Quantization Table (DQT)', DefineQuantizationTable), 220: ('nb_line', 'Define number of Lines (DNL)', None), 221: ('restart_interval', 'Define Restart Interval (DRI)', RestartInterval), 224: ('app0', 'APP0', JpegChunkApp0), 225: ('exif', 'Exif metadata', Exif), 226: ('icc', 'ICC profile', None), 236: ('app12', 'APP12', APP12), 237: ('photoshop', 'Photoshop', PhotoshopMetadata), 238: ('adobe', 'Image encoding information for DCT filters (Adobe)', AdobeChunk), 254: ('comment[]', 'Comment', Comment)}
    START_OF_FRAME = {192: 'Baseline', 193: 'Extended sequential', 194: 'Progressive', 195: 'Lossless', 197: 'Differential sequential', 198: 'Differential progressive', 199: 'Differential lossless', 201: 'Extended sequential, arithmetic coding', 202: 'Progressive, arithmetic coding', 203: 'Lossless, arithmetic coding', 205: 'Differential sequential, arithmetic coding', 206: 'Differential progressive, arithmetic coding', 207: 'Differential lossless, arithmetic coding'}
    for (key, text) in START_OF_FRAME.iteritems():
        TAG_INFO[key] = (
         'start_frame', 'Start of frame (%s)' % text.lower(), StartOfFrame)
...
class JpegFile(Parser):
    __module__ = __name__
    endian = BIG_ENDIAN
    PARSER_TAGS = {'id': 'jpeg', 'category': 'image', 'file_ext': ('jpg', 'jpeg'), 'mime': ('image/jpeg', ), 'magic': ((b'\xff\xd8\xff\xe0', 0), (b'\xff\xd8\xff\xe1', 0), (b'\xff\xd8\xff\xee', 0)), 'min_size': 22 * 8, 'description': 'JPEG picture', 'subfile': 'skip'}
    ...
    def createFields(self):
        ...
        if size:
            if 2 < size and self.stream.readBytes(self._size - 16, 2) == b'\xff\xd9':
                has_end = True
                size -= 2
