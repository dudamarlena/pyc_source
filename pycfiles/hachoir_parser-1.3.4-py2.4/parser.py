...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/parser.py
...
class HachoirParser(object):
    ...
    def _getDescription(self):
        if self._description is None:
            try:
                ...
                error('Error getting description of %s: %s' % (self.path, unicode(err)))
                self._description = self.PARSER_TAGS['description']

        return self._description

    description = property(_getDescription, doc='Description of the parser')
    ...
    def _getMimeType(self):
        if not self._mime_type:
            try:
                ...
                if not self._mime_type and self.createMimeType != Parser.createMimeType:
    ...
    def print_(cls, out, verbose):
        ...
        if verbose:
            ...
            if 'file_ext' in tags:
                file_ext = (', ').join(('.%s' % file_ext for file_ext in tags['file_ext']))
