...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/game/zsnes.py
...
from hachoir_core.field import FieldSet, StaticFieldSet, UInt8, UInt16, UInt32, String, PaddingBytes, Bytes, RawBytes
from hachoir_core.endian import LITTLE_ENDIAN

class ZSTHeader(StaticFieldSet):
    __module__ = __name__
    format = ((String, 'zs_mesg', 26, 'File header', {'charset': 'ASCII'}), (UInt8, 'zs_mesglen', 'File header string len'), (UInt8, 'zs_version', 'Version minor #'), (UInt8, 'curcyc', 'cycles left in scanline'), (UInt16, 'curypos', 'current y position'), (UInt8, 'cacheud', 'update cache every ? frames'), (UInt8, 'ccud', 'current cache increment'), (UInt8, 'intrset', 'interrupt set'), (UInt8, 'cycpl', 'cycles per scanline'), (UInt8, 'cycphb', 'cycles per hblank'), (UInt8, 'spcon', 'SPC Enable (1=enabled)'), (UInt16, 'stackand', 'value to and stack to keep it from going to the wrong area'), (UInt16, 'stackor', 'value to or stack to keep it from going to the wrong area'))


class ZSTcpu(StaticFieldSet):
    __module__ = __name__
    format = ((UInt16, 'xat'), (UInt8, 'xdbt'), (UInt8, 'xpbt'), (UInt16, 'xst'), (UInt16, 'xdt'), (UInt16, 'xxt'), (UInt16, 'xyt'), (UInt8, 'xp'), (UInt8, 'xe'), (UInt16, 'xpc'), (UInt8, 'xirqb', 'which bank the irqs start at'), (UInt8, 'debugger', 'Start with debugger (1: yes, 0: no)'), (UInt32, 'CurtableCurrent table address'), (UInt8, 'curnmi', 'if in NMI (1=yes)'), (UInt32, 'cycpbl', 'percentage left of CPU/SPC to run (3.58 = 175)'), (UInt32, 'cycpblt', 'percentage of CPU/SPC to run'))
...
class ZSNESFile(Parser):
    __module__ = __name__
    PARSER_TAGS = {'id': 'zsnes', 'category': 'game', 'description': 'ZSNES Save State File (only version 143)', 'min_size': 3091 * 8, 'file_ext': ('zst', 'zs1', 'zs2', 'zs3', 'zs4', 'zs5', 'zs6', 'zs7', 'zs8', 'zs9')}
    ...
    def createFields(self):
        yield ZSTHeader(self, 'header', 'ZST header')
        yield ZSTcpu(self, 'cpu', 'ZST cpu registers')
        yield ZSTppu(self, 'ppu', 'ZST CPU registers')
        yield RawBytes(self, 'wram7E', 65536)
        yield RawBytes(self, 'wram7F', 65536)
        yield RawBytes(self, 'vram', 65536)
