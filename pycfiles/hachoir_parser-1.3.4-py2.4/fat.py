...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/file_system/fat.py
# Compiled at: 2009-09-07 17:44:28
from hachoir_core.compatibility import sorted
from hachoir_parser import Parser
from hachoir_core.field import FieldSet, StaticFieldSet, RawBytes, PaddingBytes, createPaddingField, Link, Fragment, Bit, Bits, UInt8, UInt16, UInt32, String, Bytes, NullBytes
...
class FSInfo(StaticFieldSet):
    __module__ = __name__
    format = ((String, 'lead_sig', 4, 'Signature ("RRaA")'), (NullBytes, 'reserved[]', 480), (String, 'struct_sig', 4, 'Signature ("rrAa")'), (UInt32, 'free_count', 'Last known free cluster count on the volume'), (UInt32, 'nxt_free'), (NullBytes, 'reserved[]', 12), (Bytes, 'trail_sig', 4, 'Signature (0x00 0x00 0x55 0xAA)'))
...
class Date(FieldSet):
    ...
    def __init__(self, parent, name):
        FieldSet.__init__(self, parent, name, size={'create': 5, 'access': 2, 'modify': 4}[name] * 8)
...
class InodeLink(Link):
    ...
    def _getTargetPath(self):
        if not self.target:
            parent = self.parent
            self.target = strip_index.sub('\\\\', parent.parent._name) + parent.getFilename().rstrip('/')
        return self.target
...
class FileEntry(FieldSet):
    ...
    def createFields(self):
        if not self.LFN:
            yield String(self, 'name', 8, 'DOS file name (padded with spaces)', strip=' ', charset='ASCII')
            yield String(self, 'ext', 3, 'DOS file extension (padded with spaces)', strip=' ', charset='ASCII')
            ...
            yield String(self, 'name[]', 10, '(5 UTF-16 characters)', charset='UTF-16-LE')
            ...
            yield String(self, 'name[]', 12, '(6 UTF-16 characters)', charset='UTF-16-LE')
            yield UInt16(self, 'first_cluster', '(always 0)')
            yield String(self, 'name[]', 4, '(2 UTF-16 characters)', charset='UTF-16-LE')
...
class FAT_FS(Parser):
    ...
    PARSER_TAGS = {'category': 'file_system', 'min_size': 512 * 8, 'file_ext': ('', )}
    ...
    def _validate(self, type_offset):
        ...
        if self.stream.readBytes(510 * 8, 2) != b'U\xaa':
        ...
        if 1 < cluster < max_entry:
            ...
            while True:
                ...
                if cluster + clus_nb == next:
                    ...
                    yield (
                     self.data_start + cluster * self.cluster_size, clus_nb * self.cluster_size, False)
                    cluster = next
                    clus_nb = 1

            yield (
             self.data_start + cluster * self.cluster_size, clus_nb * self.cluster_size, True)
    ...
    def createFields(self):
        ...
        if self.version == 32:
            for field in sorted(((boot['inf_sector'].value, lambda : FSInfo(self, 'fsinfo')),
...
class FAT12(FAT_FS):
    __module__ = __name__
    PARSER_TAGS = {'id': 'fat12', 'description': 'FAT12 filesystem', 'magic': (('FAT12   ', 54 * 8),)}
    version = 12
...
class FAT16(FAT_FS):
    __module__ = __name__
    PARSER_TAGS = {'id': 'fat16', 'description': 'FAT16 filesystem', 'magic': (('FAT16   ', 54 * 8),)}
    version = 16
...
class FAT32(FAT_FS):
    __module__ = __name__
    PARSER_TAGS = {'id': 'fat32', 'description': 'FAT32 filesystem', 'magic': (('FAT32   ', 82 * 8),)}
    version = 32
