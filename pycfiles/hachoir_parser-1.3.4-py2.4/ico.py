...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/image/ico.py
...
from hachoir_core.field import FieldSet, ParserError, UInt8, UInt16, UInt32, Enum, RawBytes
...
class IconHeader(FieldSet):
    ...
    def createDescription(self):
        return 'Icon: %ux%u pixels, %u bits/pixel' % (self['width'].value, self['height'].value, self['bpp'].value)
...
class IcoFile(Parser):
    ...
    PARSER_TAGS = {'id': 'ico', 'category': 'image', 'file_ext': ('ico', 'cur'), 'mime': ('image/x-ico', ), 'min_size': (22 + 40) * 8, 'magic_regex': (('\x00\x00[\x01\x02]\x00[\x01-\x14].(\x10\x10|  |00|@@)[\x00\x10]\x00[\x00\x01\x04][\x00\x08\x18 ]\x00', 0), ), 'description': 'Microsoft Windows icon or cursor'}
    TYPE_NAME = {1: 'icon', 2: 'cursor'}
    ...
    def createDescription(self):
        ...
        for header in self.array('icon_header'):
            size.append('%ux%ux%u' % (header['width'].value, header['height'].value, header['bpp'].value))
