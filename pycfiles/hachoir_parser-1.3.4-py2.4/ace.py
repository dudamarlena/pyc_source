...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/archive/ace.py
...
from hachoir_core.field import StaticFieldSet, FieldSet, Bit, Bits, NullBits, RawBytes, Enum, UInt8, UInt16, UInt32, PascalString8, PascalString16, String, TimeDateMSDOS32
...
OS_MSDOS = 0
OS_WIN32 = 2
HOST_OS = {0: 'MS-DOS', 1: 'OS/2', 2: 'Win32', 3: 'Unix', 4: 'MAC-OS', 5: 'Win NT', 6: 'Primos', 7: 'APPLE GS', 8: 'ATARI', 9: 'VAX VMS', 10: 'AMIGA', 11: 'NEXT'}
COMPRESSION_TYPE = {0: 'Store', 1: 'Lempel-Ziv 77', 2: 'ACE v2.0'}
COMPRESSION_MODE = {0: 'fastest', 1: 'fast', 2: 'normal', 3: 'good', 4: 'best'}

class MarkerFlags(StaticFieldSet):
    __module__ = __name__
    format = ((Bit, 'extend', 'Whether the header is extended'), (Bit, 'has_comment', 'Whether the archive has a comment'), (NullBits, 'unused', 7, 'Reserved bits'), (Bit, 'sfx', 'SFX'), (Bit, 'limited_dict', 'Junior SFX with 256K dictionary'), (Bit, 'multi_volume', 'Part of a set of ACE archives'), (Bit, 'has_av_string', 'This header holds an AV-string'), (Bit, 'recovery_record', 'Recovery record preset'), (Bit, 'locked', 'Archive is locked'), (Bit, 'solid', 'Archive uses solid compression'))
...
def markerHeader(self):
    ...
    if flags['has_comment'].value:
        ...
        yield size
        if size.value > 0:
            yield RawBytes(self, 'compressed_comment', size.value, 'Compressed comment')


class FileFlags(StaticFieldSet):
    __module__ = __name__
    format = ((Bit, 'extend', 'Whether the header is extended'), (Bit, 'has_comment', 'Presence of file comment'), (Bits, 'unused', 10, 'Unused bit flags'), (Bit, 'encrypted', 'File encrypted with password'), (Bit, 'previous', 'File continued from previous volume'), (Bit, 'next', 'File continues on the next volume'), (Bit, 'solid', 'File compressed using previously archived files'))
...
def recoveryHeader(self):
    ...
    yield textHandler(UInt32(self, 'relative_start', 'Relative start (to this block) of the data this block is mode of'), hexadecimal)
...
def newRecoveryHeader(self):
    ...
    yield textHandler(UInt32(self, 'unknown[]', 'Unknown field, probably 0'), hexadecimal)
    yield String(self, 'signature', 7, "Signature, normally '**ACE**'")
    yield textHandler(UInt32(self, 'relative_start', "Offset (=crc16's) of this block in the file"), hexadecimal)
    yield textHandler(UInt32(self, 'unknown[]', 'Unknown field, probably 0'), hexadecimal)


class BaseFlags(StaticFieldSet):
    __module__ = __name__
    format = ((Bit, 'extend', 'Whether the header is extended'), (NullBits, 'unused', 15, 'Unused bit flags'))
...
class Block(FieldSet):
    __module__ = __name__
    TAG_INFO = {0: ('header', 'Archiver header', markerFlags, markerHeader, None), 1: ('file[]', fileDesc, fileFlags, fileHeader, fileBody), 2: ('recovery[]', recoveryDesc, recoveryHeader, None, None), 5: ('new_recovery[]', None, None, newRecoveryHeader, None)}
...
class AceFile(Parser):
    ...
    PARSER_TAGS = {'id': 'ace', 'category': 'archive', 'file_ext': ('ace', ), 'mime': ('application/x-ace-compressed', ), 'min_size': 50 * 8, 'description': 'ACE archive'}
