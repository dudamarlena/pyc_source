...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/container/riff.py
...
from hachoir_core.field import FieldSet, ParserError, UInt8, UInt16, UInt32, Enum, Bit, NullBits, NullBytes, RawBytes, String, PaddingBytes, SubFile
...
def parseText(self):
    yield String(self, 'text', self['size'].value, strip=' \x00', truncate='\x00', charset='ISO-8859-1')
...
def parseAVIStreamFormat(self):
    ...
    TYPE_HANDLER = {'vids': (parseVideoFormat, 40), 'auds': (parseAudioFormat, 16)}
...
def parseAVIStreamHeader(self):
    ...
    field = String(self, 'fourcc', 4, 'Stream four character code', strip=' \x00', charset='ASCII')
    ...
    yield String(self, 'language', 2, 'Stream language', charset='ASCII', strip='\x00')
...
class Chunk(FieldSet):
    __module__ = __name__
    TAG_INFO = {'LIST': ('list[]', None, 'Sub-field list'), 'JUNK': ('junk[]', None, 'Junk (padding)'), 'INAM': ('title', parseText, 'Document title'), 'IART': ('artist', parseText, 'Artist'), 'ICMT': ('comment', parseText, 'Comment'), 'ICOP': ('copyright', parseText, 'Copyright'), 'IENG': ('author', parseText, 'Author'), 'ICRD': ('creation_date', parseText, 'Creation date'), 'ISFT': ('producer', parseText, 'Producer'), 'IDIT': ('datetime', parseText, 'Date time')}
    subtag_info = {'INFO': ('info', 'File informations'), 'hdrl': ('headers', 'Headers'), 'strl': ('stream[]', 'Stream header list'), 'movi': ('movie', 'Movie stream'), 'odml': ('odml', 'ODML')}
...
class ChunkAVI(Chunk):
    ...
    TAG_INFO.update({'strh': ('stream_hdr', parseAVIStreamHeader, 'Stream header'), 'strf': ('stream_fmt', parseAVIStreamFormat, 'Stream format'), 'avih': ('avi_hdr', parseAviHeader, 'AVI header'), 'idx1': ('index', parseIndex, 'Stream index'), 'dmlh': ('odml_hdr', parseODML, 'ODML header')})


class ChunkCDDA(Chunk):
    __module__ = __name__
    TAG_INFO = Chunk.TAG_INFO.copy()
    TAG_INFO.update({'fmt ': ('cdda', parseCDDA, 'CD audio informations')})


class ChunkWAVE(Chunk):
    __module__ = __name__
    TAG_INFO = Chunk.TAG_INFO.copy()
    TAG_INFO.update({'fmt ': ('format', parseWAVFormat, 'Audio format'), 'fact': ('nb_sample', parseWAVFact, 'Number of samples'), 'data': ('audio_data', None, 'Audio stream data')})
...
class ChunkACON(Chunk):
    __module__ = __name__
    TAG_INFO = Chunk.TAG_INFO.copy()
    TAG_INFO.update({'anih': ('anim_hdr', parseAnimationHeader, 'Animation header'), 'seq ': ('anim_seq', parseAnimationSequence, 'Animation sequence'), 'rate': ('anim_rate', parseAnimationRate, 'Animation sequence'), 'icon': ('icon[]', parseIcon, 'Icon')})


class RiffFile(Parser):
    __module__ = __name__
    PARSER_TAGS = {'id': 'riff', 'category': 'container', 'file_ext': ('avi', 'cda', 'wav', 'ani'), 'min_size': 16 * 8, 'mime': ('video/x-msvideo', 'audio/x-wav', 'audio/x-cda'), 'magic': (('AVI LIST', 8 * 8), ('WAVEfmt ', 8 * 8), ('CDDAfmt ', 8 * 8), ('ACONanih', 8 * 8)), 'description': 'Microsoft RIFF container'}
    VALID_TYPES = {'WAVE': (ChunkWAVE, 'audio/x-wav', 'Microsoft WAVE audio', '.wav'), 'CDDA': (ChunkCDDA, 'audio/x-cda', 'Microsoft Windows audio CD file (cda)', '.cda'), 'AVI ': (ChunkAVI, 'video/x-msvideo', 'Microsoft AVI video', '.avi'), 'ACON': (ChunkACON, 'image/x-ani', 'Microsoft Windows animated cursor', '.ani')}
