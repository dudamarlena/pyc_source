...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/container/realmedia.py
...
from hachoir_core.field import FieldSet, UInt16, UInt32, Bit, RawBits, RawBytes, String, PascalString8, PascalString16
...
def parseContentDescription(self):
    yield PascalString16(self, 'title', charset='ISO-8859-1', strip=' \x00')
    yield PascalString16(self, 'author', charset='ISO-8859-1', strip=' \x00')
    yield PascalString16(self, 'copyright', charset='ISO-8859-1', strip=' \x00')
    yield PascalString16(self, 'comment', charset='ISO-8859-1', strip=' \x00')
...
class NameValueProperty(FieldSet):
    ...
    def createFields(self):
        ...
        yield PascalString16(self, 'value', charset='ISO-8859-1', strip=' \x00')
...
class Chunk(FieldSet):
    __module__ = __name__
    tag_info = {'.RMF': ('header', parseHeader), 'PROP': ('file_prop', parseFileProperties), 'CONT': ('content_desc', parseContentDescription), 'MDPR': ('stream_prop[]', parseMediaPropertiesHeader), 'DATA': ('data[]', None), 'INDX': ('file_index[]', None)}
...
class RealMediaFile(Parser):
    __module__ = __name__
    MAGIC = '.RMF\x00\x00\x00\x12\x00\x01'
    PARSER_TAGS = {'id': 'real_media', 'category': 'container', 'file_ext': ('rm', ), 'mime': ('video/x-pn-realvideo', 'audio/x-pn-realaudio', 'audio/x-pn-realaudio-plugin', 'audio/x-real-audio',
 'application/vnd.rn-realmedia'), 'min_size': len(MAGIC) * 8, 'magic': ((MAGIC, 0),), 'description': 'RealMedia (rm) Container File'}
    endian = BIG_ENDIAN
