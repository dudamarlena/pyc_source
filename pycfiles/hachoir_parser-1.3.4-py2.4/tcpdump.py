...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/network/tcpdump.py
...
from hachoir_core.field import FieldSet, ParserError, Enum, Bytes, NullBytes, RawBytes, UInt8, UInt16, UInt32, Int32, TimestampUnix32, Bit, Bits, NullBits
...
class TCP_Option(FieldSet):
    ...
    SACK = 4
    TIMESTAMP = 8
    code_name = {NOP: 'NOP', MAX_SEGMENT: 'Max segment size', WINDOW_SCALE: 'Window scale', SACK: 'SACK permitted', TIMESTAMP: 'Timestamp'}
...
class TCP(Layer):
    __module__ = __name__
    port_name = {13: 'daytime', 20: 'ftp data', 21: 'ftp', 23: 'telnet', 25: 'smtp', 53: 'dns', 63: 'dhcp/bootp', 80: 'HTTP', 110: 'pop3', 119: 'nntp', 123: 'ntp', 139: 'netbios session service', 1863: 'MSNMS', 6667: 'IRC'}
...
class UDP(Layer):
    __module__ = __name__
    port_name = {12: 'daytime', 22: 'ssh', 53: 'DNS', 67: 'dhcp/bootp', 80: 'http', 110: 'pop3', 123: 'ntp', 137: 'netbios name service', 138: 'netbios datagram service'}
...
class ICMPv6(Layer):
    ...
    ECHO_REQUEST = 128
    ECHO_REPLY = 129
    TYPE_DESC = {128: 'Echo request', 129: 'Echo reply'}
...
class IP(Layer):
    __module__ = __name__
    PROTOCOL_INFO = {1: ('icmp', ICMP, 'ICMP'), 6: ('tcp', TCP, 'TCP'), 17: ('udp', UDP, 'UDP'), 58: ('icmpv6', ICMPv6, 'ICMPv6'), 60: ('ipv6_opts', None, 'IPv6 destination option')}
...
class IPv4(IP):
    __module__ = __name__
    precedence_name = {7: 'Network Control', 6: 'Internetwork Control', 5: 'CRITIC/ECP', 4: 'Flash Override', 3: 'Flash', 2: 'Immediate', 1: 'Priority', 0: 'Routine'}
...
class Layer2(Layer):
    __module__ = __name__
    PROTO_INFO = {2048: ('ipv4', IPv4, 'IPv4'), 2054: ('arp', ARP, 'ARP'), 34525: ('ipv6', IPv6, 'IPv6')}
...
class Unicast(Layer2):
    __module__ = __name__
    packet_type_name = {0: 'Unicast to us'}
...
class Ethernet(Layer2):
    ...
    def createDescription(self):
        return 'Ethernet: %s>%s (%s)' % (self['src'].display, self['dst'].display, self['protocol'].display)
...
class TcpdumpFile(Parser):
    __module__ = __name__
    PARSER_TAGS = {'id': 'tcpdump', 'category': 'misc', 'min_size': 24 * 8, 'description': 'Tcpdump file (network)', 'magic': ((b'\xd4\xc3\xb2\xa1', 0), )}
    endian = LITTLE_ENDIAN
    LINK_TYPE = {1: ('ethernet', Ethernet), 113: ('unicast', Unicast)}
