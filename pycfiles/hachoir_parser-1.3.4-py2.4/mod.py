...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/audio/mod.py
# Compiled at: 2010-01-20 18:05:14
"""
...
from hachoir_core.field import FieldSet, Bits, UInt16, UInt8, RawBytes, String, GenericVector
from hachoir_core.endian import BIG_ENDIAN
from hachoir_core.text_handler import textHandler
MODULE_TYPE = {'M.K.': ('Noise/Pro-Tracker', 4), 'M!K!': ('Noise/Pro-Tracker', 4), 'M&K&': ('Noise/Pro-Tracker', 4), 'RASP': ('StarTrekker', 4), 'FLT4': ('StarTrekker', 4), 'FLT8': ('StarTrekker', 8), '6CHN': ('FastTracker', 6), '8CHN': ('FastTracker', 8), 'CD81': ('Octalyser', 8), 'OCTA': ('Octalyser', 8), 'FA04': ('Digital Tracker', 4), 'FA06': ('Digital Tracker', 6), 'FA08': ('Digital Tracker', 8)}

def getFineTune(val):
    return ('0', '1', '2', '3', '4', '5', '6', '7', '8', '-8', '-7', '-6', '-5', '-4',
            '-3', '-2', '-1')[val.value]
...
class SampleInfo(FieldSet):
    ...
    def createFields(self):
        yield String(self, 'name', 22, strip='\x00')
...
class Header(FieldSet):
    ...
    def createFields(self):
        yield String(self, 'name', 20, strip='\x00')
...
class AmigaModule(Parser):
    __module__ = __name__
    PARSER_TAGS = {'id': 'mod', 'category': 'audio', 'file_ext': ('mod', 'nst', 'wow', 'oct', 'sd0'), 'mime': ('audio/mod', 'audio/x-mod', 'audio/mod', 'audio/x-mod'), 'min_size': 1084 * 8, 'description': 'Uncompressed amiga module'}
    endian = BIG_ENDIAN
    ...
    def createFields(self):
        ...
        for index in xrange(128):
            patterns = max(patterns, header[('patterns/position[%u]' % index)].value)
        ...
        for index in xrange(31):
            ...
            if count:
                ...
                yield RawBytes(self, 'sample_data[]', 2 * count, 'Sample %u' % index)