...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/program/exe_res.py
...
"""
from hachoir_core.field import FieldSet, ParserError, Enum, Bit, Bits, SeekableFieldSet, UInt16, UInt32, TimestampUnix32, RawBytes, PaddingBytes, NullBytes, NullBits, CString, String
...
MAJOR_OS_NAME = {1: 'DOS', 2: 'OS/2 16-bit', 3: 'OS/2 32-bit', 4: 'Windows NT'}
MINOR_OS_BASE = 0
MINOR_OS_NAME = {0: 'Base', 1: 'Windows 16-bit', 2: 'Presentation Manager 16-bit', 3: 'Presentation Manager 32-bit', 4: 'Windows 32-bit'}
FILETYPE_DRIVER = 3
FILETYPE_FONT = 4
FILETYPE_NAME = {1: 'Application', 2: 'DLL', 3: 'Driver', 4: 'Font', 5: 'VXD', 7: 'Static library'}
DRIVER_SUBTYPE_NAME = {1: 'Printer', 2: 'Keyboard', 3: 'Language', 4: 'Display', 5: 'Mouse', 6: 'Network', 7: 'System', 8: 'Installable', 9: 'Sound', 10: 'Communications'}
FONT_SUBTYPE_NAME = {1: 'Raster', 2: 'Vector', 3: 'TrueType'}
...
class VersionInfoBinary(FieldSet):
    ...
    def createFields(self):
        ...
        if self['magic'].value != 4277077181:
    ...
    def createFields(self):
        ...
        if size:
            if self['type'].value == self.TYPE_STRING:
                if self.is_32bit:
                    size *= 2
                yield String(self, 'value', size, charset='UTF-16-LE', truncate='\x00')
...
RESOURCE_TYPE = {1: ('cursor[]', 'Cursor', None), 2: ('bitmap[]', 'Bitmap', None), 3: ('icon[]', 'Icon', parseIcon), 4: ('menu[]', 'Menu', None), 5: ('dialog[]', 'Dialog', None), 6: ('string_table[]', 'String table', parseStringTable), 7: ('font_dir[]', 'Font directory', None), 8: ('font[]', 'Font', None), 9: ('accelerators[]', 'Accelerators', None), 10: ('raw_res[]', 'Unformatted resource data', None), 11: ('message_table[]', 'Message table', None), 12: ('group_cursor[]', 'Group cursor', None), 14: ('group_icon[]', 'Group icon', None), 16: ('version_info', 'Version information', parseVersionInfo)}
...
class Entry(FieldSet):
    ...
    def createDescription(self):
        return 'Entry #%u: offset=%s size=%s' % (self.inode['offset'].value, self['rva'].display, self['size'].display)
...
class ResourceContent(FieldSet):
    ...
    def createDescription(self):
        return 'Resource #%u content: type=%s' % (self.getResID(), self.getResType())
...
class Directory(FieldSet):
    ...
    def createFields(self):
        ...
        if MAX_NAME_PER_HEADER < self['header/nb_name'].value:
            raise ParserError('EXE resource: invalid number of name (%s)' % self['header/nb_name'].value)
        if MAX_INDEX_PER_HEADER < self['header/nb_index'].value:
            raise ParserError('EXE resource: invalid number of index (%s)' % self['header/nb_index'].value)
        hdr = self['header']
