...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/container/ogg.py
# Compiled at: 2010-07-25 21:01:04
from hachoir_parser import Parser
from hachoir_core.field import Field, FieldSet, createOrphanField, NullBits, Bit, Bits, Enum, Fragment, MissingField, ParserError, UInt8, UInt16, UInt24, UInt32, UInt64, RawBytes, String, PascalString32, NullBytes
...
PIXEL_FORMATS = {0: '4:2:0', 2: '4:2:2', 3: '4:4:4'}
...
class Chunk(FieldSet):
    __module__ = __name__
    tag_info = {'vorbis': {3: ('comment', parseVorbisComment), 1: ('vorbis_hdr', parseVorbisHeader)}, 'theora': {128: ('theora_hdr', parseTheoraHeader), 129: ('comment', parseVorbisComment)}, 'video\x00': {1: ('video_hdr', parseVideoHeader)}}
...
class OggFile(Parser):
    __module__ = __name__
    PARSER_TAGS = {'id': 'ogg', 'category': 'container', 'file_ext': ('ogg', 'ogm'), 'mime': ('application/ogg', 'application/x-ogg', 'audio/ogg', 'audio/x-ogg', 'video/ogg', 'video/x-ogg',
 'video/theora', 'video/x-theora'), 'magic': ((OggPage.MAGIC, 0),), 'subfile': 'skip', 'min_size': 28 * 8, 'description': 'Ogg multimedia container'}
    ...
    def createLastPage(self):
        ...
        if True:
            offset = self.stream.searchBytes('OggS\x00\x05', start, end)
            if offset is None:
                offset = self.stream.searchBytes('OggS\x00\x04', start, end)
            if offset is None:
                return
            ...
            page = None
            while True:
                offset = self.stream.searchBytes('OggS\x00', start, end)
                if offset is None:
                    break
...
class OggStream(Parser):
    __module__ = __name__
    PARSER_TAGS = {'id': 'ogg_stream', 'category': 'container', 'subfile': 'skip', 'min_size': 7 * 8, 'description': 'Ogg logical stream'}
    endian = LITTLE_ENDIAN
