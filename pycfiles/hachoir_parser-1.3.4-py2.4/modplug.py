...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/audio/modplug.py
...
"""
from hachoir_core.field import FieldSet, UInt32, UInt16, UInt8, Int8, Float32, RawBytes, String, GenericVector, ParserError
...
class MidiOut(FieldSet):
    ...
    def createFields(self):
        for name in ('start', 'stop', 'tick', 'noteon', 'noteoff', 'volume', 'pan',
                     'banksel', 'program'):
...
class Command(FieldSet):
    ...
    def createFields(self):
        ...
        size = self.stream.searchBytesLength('\x00', False, start)
        if size > 0:
            self.info('Command: %s' % self.stream.readBytes(start, size))
            yield String(self, 'command', size, strip='\x00')
...
class SoundMixPluginInfo(FieldSet):
    ...
    def createFields(self):
        ...
        yield String(self, 'name', 32, strip='\x00')
        yield String(self, 'dll_name', 64, desc='Original DLL name', strip='\x00')
...
class MPField(FieldSet):
    ...
    def createDescription(self):
        return "Element '%s', size %i" % (self['code']._description, self['data_size'].value)
...
def parseFields(parser):
    ...
    parser.info("End of extension '%s' when finding '%s'" % (parser['block_type'].value, parser.stream.readBytes(addr, 4)))
    return


class ModplugBlock(FieldSet):
    __module__ = __name__
    BLOCK_INFO = {'TEXT': ('comment', True, 'Comment', parseComments), 'MIDI': ('midi_config', True, 'Midi configuration', parseMidiConfig), 'XFHC': ('channel_settings', True, 'Channel settings', parseChannelSettings), 'XTPM': ('instrument_ext', False, 'Instrument extensions', parseFields), 'STPM': ('song_ext', False, 'Song extensions', parseFields)}
