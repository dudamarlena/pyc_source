...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/audio/real_audio.py
...
from hachoir_core.field import FieldSet, UInt8, UInt16, UInt32, Bytes, RawBytes, String, PascalString8
...
class RealAudioFile(Parser):
    __module__ = __name__
    MAGIC = b'.ra\xfd'
    PARSER_TAGS = {'id': 'real_audio', 'category': 'audio', 'file_ext': ['ra'], 'mime': ('audio/x-realaudio', 'audio/x-pn-realaudio'), 'min_size': 6 * 8, 'magic': ((MAGIC, 0),), 'description': 'Real audio (.ra)'}
    endian = BIG_ENDIAN
    ...
    def createFields(self):
        yield Bytes(self, 'signature', 4, "RealAudio identifier ('.ra\\xFD')")
    ...
    def createDescription(self):
        if self['version'].value == 3:
            ...
            return "RealAudio v4 file, '%s' codec, %s, %u channels" % (self['FourCC'].value, self['sample_rate'].display, self['channels'].value)
        else:
            return 'Real audio'