...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/image/png.py
# Compiled at: 2010-03-25 21:56:46
"""
...
from hachoir_core.field import FieldSet, Fragment, ParserError, MissingField, UInt8, UInt16, UInt32, String, CString, Bytes, RawBytes, Bit, NullBits, Enum, CompressedField
...
def headerDescription(parent):
    return 'Header: %ux%u pixels and %u bits/pixel' % (parent['width'].value, parent['height'].value, getBitsPerPixel(parent))
...
def timestampValue(parent):
    value = datetime(parent['year'].value, parent['month'].value, parent['day'].value, parent['hour'].value, parent['minute'].value, parent['second'].value)
    return value
...
class Chunk(FieldSet):
    __module__ = __name__
    TAG_INFO = {'tIME': ('time', timestampParse, 'Timestamp', timestampValue), 'pHYs': ('physical', physicalParse, physicalDescription, None), 'IHDR': ('header', headerParse, headerDescription, None), 'PLTE': ('palette', paletteParse, paletteDescription, None), 'gAMA': ('gamma', gammaParse, gammaDescription, gammaValue), 'tEXt': ('text[]', textParse, textDescription, None), 'tRNS': ('transparency', parseTransparency, 'Transparency Info', None), 'bKGD': ('background', parseBackgroundColor, backgroundColorDesc, None), 'IDAT': ('data[]',
              lambda parent: (
               ImageData(parent),), 'Image data', None), 
       'iTXt': ('utf8_text[]', None, 'International text (encoded in UTF-8)', None), 'zTXt': ('comp_text[]', None, 'Compressed text', None), 'IEND': ('end', None, 'End', None)}
    ...
    def __init__(self, parent, name, description=None):
        ...
        if MAX_CHUNK_SIZE < self._size // 8:
            raise ParserError('PNG: Chunk is too big (%s)' % humanFilesize(self._size // 8))
        tag = self['tag'].value
        self.desc_func = None
...
class PngFile(Parser):
    __module__ = __name__
    PARSER_TAGS = {'id': 'png', 'category': 'image', 'file_ext': ('png', ), 'mime': ('image/png', 'image/x-png'), 'min_size': 8 * 8, 'magic': [(b'\x89PNG\r\n\x1a\n', 0)], 'description': 'Portable Network Graphics (PNG) picture'}
    ...
    def createFields(self):
        yield Bytes(self, 'id', 8, "PNG identifier ('\\x89PNG\\r\\n\\x1A\\n')")
        while not self.eof:
            yield Chunk(self, 'chunk[]')

    def createDescription(self):
        header = self['header']
        desc = 'PNG picture: %ux%ux%u' % (header['width'].value, header['height'].value, getBitsPerPixel(header))
        if header['has_alpha'].value:
            desc += ' (alpha layer)'
        return desc
    ...
    def createContentSize(self):
        ...
        pos = self.stream.searchBytes(b'\x00\x00\x00\x00IEND\xaeB`\x82', start, end)
        if pos is not None:
            return pos + 12 * 8
        return