...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/video/mpeg_video.py
...
from hachoir_core.field import FieldSet, FieldError, ParserError, Bit, Bits, Bytes, RawBits, PaddingBits, NullBits, UInt8, UInt16, RawBytes, PaddingBytes, Enum
...
class PackHeader(FieldSet):
    ...
    def createFields(self):
        if self.stream.readBits(self.absolute_address, 2, self.endian) == 1:
            ...
            if count:
                yield PaddingBytes(self, 'stuffing', count, pattern=b'\xff')
        else:
    ...
    def validate(self):
        ...
        sync0 = self['sync[0]']
        if sync0.size == 2:
            if sync0.value == 1 and (not self['sync[1]'].value or not self['sync[2]'].value or self['sync[3]'].value != 3):
                return 'Invalid synchronisation bits'
        elif sync0.size == 4:
            if sync0.value == 2 and (not self['sync[1]'].value or not self['sync[2]'].value or self['sync[3]'].value != 3 or not self['sync[4]'].value):
                return 'Invalid synchronisation bits'
        else:
...
class VideoExtension1(FieldSet):
    ...
    def createFields(self):
        ...
        if self['has_pstd_buffer'].value:
            yield Bits(self, 'sync[]', 2)
            yield Enum(Bit(self, 'pstd_buffer_scale'), {True: '128 bytes', False: '1024 bytes'})
            yield Bits(self, 'pstd_size', 13)


class VideoSeqHeader(FieldSet):
    __module__ = __name__
    ASPECT = ['forbidden', '1.0000 (VGA etc.)', '0.6735', '0.7031 (16:9, 625line)', '0.7615', '0.8055', '0.8437 (16:9, 525line)', '0.8935', '0.9157 (CCIR601, 625line)', '0.9815', '1.0255', '1.0695', '1.0950 (CCIR601, 525line)', '1.1575', '1.2015', 'reserved']
    FRAMERATE = [
     'forbidden', '23.976 fps', '24 fps', '25 fps', '29.97 fps', '30 fps', '50 fps', '59.94 fps', '60 fps']
...
class VideoPicture(FieldSet):
    __module__ = __name__
    CODING_TYPE = ['forbidden', 'intra-coded (I)', 'predictive-coded (P)', 'bidirectionally-predictive-coded (B)', 'dc intra-coded (D)', 'reserved', 'reserved', 'reserved']
...
class VideoSlice(FieldSet):
    ...
    def createFields(self):
        ...
        pos = self.stream.searchBytes('\x00\x00\x01', start, start + 1024 * 1024 * 8)
        if pos is None:
            pos = self.root.size
...
class VideoChunk(FieldSet):
    __module__ = __name__
    tag_info = {0: ('pict_start[]', VideoPicture, 'Picture start'), 178: ('data_start[]', None, 'Data start'), 179: ('seq_hdr[]', VideoSeqHeader, 'Sequence header'), 180: ('seq_err[]', None, 'Sequence error'), 181: ('ext_start[]', VideoExtension, 'Extension start'), 183: ('seq_end[]', None, 'Sequence end'), 184: ('group_start[]', GroupStart, 'Group start')}
    ...
    def createFields(self):
        ...
        if self.parser and self['tag'].value != 183:
...
class VideoStream(Parser):
    ...
    def createFields(self):
        while self.current_size < self.size:
            pos = self.stream.searchBytes('\x00\x00\x01', self.current_size, self.current_size + 1024 * 1024 * 8)
            if pos is not None:
    ...
    def createFields(self):
        ...
        while True:
            ...
            if next == 255:
                ...
                yield Bits(self, 'scale_marker', 2)
...
class Chunk(FieldSet):
    __module__ = __name__
    ISO_END_CODE = 185
    tag_info = {185: ('end', None, 'End'), 186: ('pack_start[]', PackHeader, 'Pack start'), 187: ('system_start[]', SystemHeader, 'System start'), 189: ('private[]', Stream, 'Private elementary'), 190: ('padding[]', Stream, 'Padding'), 255: ('directory[]', Stream, 'Program Stream Directory')}

    def __init__(self, *args):
        FieldSet.__init__(self, *args)
        if not hasattr(self.root, 'streamgroups'):
            self.root.streamgroups = {}
            for tag in range(188, 256):
...
class MPEGVideoFile(Parser):
    __module__ = __name__
    PARSER_TAGS = {'id': 'mpeg_video', 'category': 'video', 'file_ext': ('mpeg', 'mpg', 'mpe', 'vob'), 'mime': ('video/mpeg', 'video/mp2p'), 'min_size': 12 * 8, 'description': 'MPEG video, version 1 or 2'}
    endian = BIG_ENDIAN
    version = None
    ...
    def validate(self):
        ...
        if pack['sync'].value != '\x00\x00\x01':
