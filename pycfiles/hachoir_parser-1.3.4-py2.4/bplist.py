...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/misc/bplist.py
...
from hachoir_core.field import RootSeekableFieldSet, FieldSet, Enum, Bits, GenericInteger, Float32, Float64, UInt8, UInt64, Bytes, NullBytes, RawBytes, String
...
class BPListObject(FieldSet):
    ...
    def createFields(self):
        yield Enum(Bits(self, 'marker_type', 4), {0: 'Simple', 1: 'Int', 2: 'Real', 3: 'Date', 4: 'Data', 5: 'ASCII String', 6: 'UTF-16-BE String', 8: 'UID', 10: 'Array', 13: 'Dict'})
        markertype = self['marker_type'].value
        if markertype == 0:
            yield Enum(Bits(self, 'value', 4), {0: 'Null', 8: 'False', 9: 'True', 15: 'Fill Byte'})
            ...
            yield Bits(self, 'size', 4, 'log2 of number of bytes')
            if self['size'].value == 2:
                yield Float32(self, 'value')
            elif self['size'].value == 3:
...
class BPList(HachoirParser, RootSeekableFieldSet):
    ...
    endian = BIG_ENDIAN
    MAGIC = 'bplist00'
    PARSER_TAGS = {'id': 'bplist', 'category': 'misc', 'file_ext': ('plist', ), 'magic': ((MAGIC, 0),), 'min_size': 8 + 32, 'description': 'Apple/NeXT Binary Property List'}
