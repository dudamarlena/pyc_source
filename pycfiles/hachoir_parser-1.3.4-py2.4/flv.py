...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/video/flv.py
...
from hachoir_core.field import FieldSet, UInt8, UInt24, UInt32, NullBits, NullBytes, Bit, Bits, String, RawBytes, Enum
from hachoir_core.endian import BIG_ENDIAN
from hachoir_parser.audio.mpeg_audio import Frame
from hachoir_parser.video.amf import AMFObject
from hachoir_core.tools import createDict
SAMPLING_RATE = {0: (5512, '5.5 kHz'), 1: (11025, '11 kHz'), 2: (22050, '22.1 kHz'), 3: (44100, '44.1 kHz')}
SAMPLING_RATE_VALUE = createDict(SAMPLING_RATE, 0)
SAMPLING_RATE_TEXT = createDict(SAMPLING_RATE, 1)
AUDIO_CODEC_MP3 = 2
AUDIO_CODEC_NAME = {0: 'Uncompressed', 1: 'ADPCM', 2: 'MP3', 5: 'Nellymoser 8kHz mono', 6: 'Nellymoser'}
VIDEO_CODEC_NAME = {2: 'Sorensen H.263', 3: 'Screen video', 4: 'On2 VP6'}
FRAME_TYPE = {1: 'keyframe', 2: 'inter frame', 3: 'disposable inter frame'}
...
class Chunk(FieldSet):
    __module__ = __name__
    tag_info = {8: ('audio[]', parseAudio, ''), 9: ('video[]', parseVideo, ''), 18: ('metadata', parseAMF, '')}
...
class FlvFile(Parser):
    __module__ = __name__
    PARSER_TAGS = {'id': 'flv', 'category': 'video', 'file_ext': ('flv', ), 'mime': ('video/x-flv', ), 'min_size': 9 * 4, 'magic': (('FLV\x01\x05\x00\x00\x00\t', 0), ('FLV\x01\x01\x00\x00\x00\t', 0)), 'description': 'Macromedia Flash video'}
    endian = BIG_ENDIAN
