...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/misc/chm.py
...
from hachoir_core.field import Field, FieldSet, ParserError, Int32, UInt32, UInt64, RawBytes, PaddingBytes, Enum, String
...
class PMGL(FieldSet):
    ...
    def createFields(self):
        ...
        yield filesizeHandler(Int32(self, 'free_space', 'Length of free space and/or quickref area at end of directory chunk'))
...
class PMGI(FieldSet):
    ...
    def createFields(self):
        ...
        yield filesizeHandler(UInt32(self, 'free_space', 'Length of free space and/or quickref area at end of directory chunk'))
        stop = self.size - self['free_space'].value * 8
        while self.current_size < stop:
...
class ChmFile(Parser):
    __module__ = __name__
    PARSER_TAGS = {'id': 'chm', 'category': 'misc', 'file_ext': ('chm', ), 'min_size': 4 * 8, 'magic': (('ITSF\x03\x00\x00\x00', 0), ), 'description': "Microsoft's HTML Help (.chm)"}
