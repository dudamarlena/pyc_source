...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/file_system/iso9660.py
...
from hachoir_core.field import FieldSet, ParserError, UInt8, UInt32, UInt64, Enum, NullBytes, RawBytes, String
...
class PrimaryVolumeDescriptor(FieldSet):
    ...
    def createFields(self):
        ...
        yield NullBytes(self, 'unused[]', 1)
        yield String(self, 'app_use', 512, 'Application use', strip=' \x00')
...
class BootRecord(FieldSet):
    ...
    def createFields(self):
        yield String(self, 'sys_id', 31, 'Boot system identifier', strip='\x00')
        yield String(self, 'boot_id', 31, 'Boot identifier', strip='\x00')
...
class Volume(FieldSet):
    ...
    endian = BIG_ENDIAN
    TERMINATOR = 255
    type_name = {0: 'Boot Record', 1: 'Primary Volume Descriptor', 2: 'Supplementary Volume Descriptor', 3: 'Volume Partition Descriptor', TERMINATOR: 'Volume Descriptor Set Terminator'}
    static_size = 2048 * 8
    content_handler = {0: BootRecord, 1: PrimaryVolumeDescriptor, TERMINATOR: Terminator}
...
class ISO9660(Parser):
    ...
    MAGIC = '\x01CD001'
    NULL_BYTES = 32768
    PARSER_TAGS = {'id': 'iso9660', 'category': 'file_system', 'description': 'ISO 9660 file system', 'min_size': (NULL_BYTES + 6) * 8, 'magic': ((MAGIC, NULL_BYTES * 8),)}
