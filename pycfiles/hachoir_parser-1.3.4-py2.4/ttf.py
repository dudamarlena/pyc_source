...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/misc/ttf.py
...
from hachoir_core.field import FieldSet, ParserError, UInt16, UInt32, Bit, Bits, PaddingBits, NullBytes, String, RawBytes, Bytes, Enum, TimestampMac32
from hachoir_core.endian import BIG_ENDIAN
from hachoir_core.text_handler import textHandler, hexadecimal, filesizeHandler
MAX_NAME_COUNT = 300
MIN_NB_TABLE = 3
MAX_NB_TABLE = 30
DIRECTION_NAME = {0: 'Mixed directional', 1: 'Left to right', 2: 'Left to right + neutrals', -1: 'Right to left', -2: 'Right to left + neutrals'}
NAMEID_NAME = {0: 'Copyright notice', 1: 'Font family name', 2: 'Font subfamily name', 3: 'Unique font identifier', 4: 'Full font name', 5: 'Version string', 6: 'Postscript name', 7: 'Trademark', 8: 'Manufacturer name', 9: 'Designer', 10: 'Description', 11: 'URL Vendor', 12: 'URL Designer', 13: 'License Description', 14: 'License info URL', 16: 'Preferred Family', 17: 'Preferred Subfamily', 18: 'Compatible Full', 19: 'Sample text', 20: 'PostScript CID findfont name'}
PLATFORM_NAME = {0: 'Unicode', 1: 'Macintosh', 2: 'ISO', 3: 'Microsoft', 4: 'Custom'}
CHARSET_MAP = {0: {3: 'UTF-16-BE'}, 1: {0: 'MacRoman'}, 3: {1: 'UTF-16-BE'}}
...
def parseFontHeader(self):
    ...
    yield textHandler(UInt32(self, 'checksum'), hexadecimal)
    yield Bytes(self, 'magic', 4, 'Magic string (\\x5F\\x0F\\x3C\\xF5)')
    if self['magic'].value != b'_\x0f<\xf5':
...
def parseNames(self):
    ...
    for entry in entries:
        ...
        if last and last == new:
            ...
            continue
        last = (
         entry['offset'].value, entry['length'].value)
...
class Table(FieldSet):
    __module__ = __name__
    TAG_INFO = {'head': ('header', 'Font header', parseFontHeader), 'name': ('names', 'Names', parseNames)}
...
class TrueTypeFontFile(Parser):
    __module__ = __name__
    endian = BIG_ENDIAN
    PARSER_TAGS = {'id': 'ttf', 'category': 'misc', 'file_ext': ('ttf', ), 'min_size': 10 * 8, 'description': 'TrueType font'}
