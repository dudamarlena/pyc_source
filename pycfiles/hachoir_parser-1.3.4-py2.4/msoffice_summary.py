...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/misc/msoffice_summary.py
# Compiled at: 2010-01-20 18:06:29
"""
...
from hachoir_core.field import FieldSet, ParserError, RootSeekableFieldSet, SeekableFieldSet, Bit, Bits, NullBits, UInt8, UInt16, UInt32, TimestampWin64, TimedeltaWin64, Enum, Bytes, RawBytes, NullBytes, String, Int8, Int32, Float32, Float64, PascalString32
...
OS_MAC = 1
OS_NAME = {0: 'Windows 16-bit', 1: 'Macintosh', 2: 'Windows 32-bit'}
...
class PropertyIndex(FieldSet):
    __module__ = __name__
    TAG_CODEPAGE = 1
    COMMON_PROPERTY = {0: 'Dictionary', 1: 'CodePage', 2147483648: 'LOCALE_SYSTEM_DEFAULT', 2147483651: 'CASE_SENSITIVE'}
    DOCUMENT_PROPERTY = {2: 'Category', 3: 'PresentationFormat', 4: 'NumBytes', 5: 'NumLines', 6: 'NumParagraphs', 7: 'NumSlides', 8: 'NumNotes', 9: 'NumHiddenSlides', 10: 'NumMMClips', 11: 'Scale', 12: 'HeadingPairs', 13: 'DocumentParts', 14: 'Manager', 15: 'Company', 16: 'LinksDirty', 17: 'DocSumInfo_17', 18: 'DocSumInfo_18', 19: 'DocSumInfo_19', 20: 'DocSumInfo_20', 21: 'DocSumInfo_21', 22: 'DocSumInfo_22', 23: 'DocSumInfo_23'}
    DOCUMENT_PROPERTY.update(COMMON_PROPERTY)
    COMPONENT_PROPERTY = {2: 'Title', 3: 'Subject', 4: 'Author', 5: 'Keywords', 6: 'Comments', 7: 'Template', 8: 'LastSavedBy', 9: 'RevisionNumber', 10: 'TotalEditingTime', 11: 'LastPrinted', 12: 'CreateTime', 13: 'LastSavedTime', 14: 'NumPages', 15: 'NumWords', 16: 'NumCharacters', 17: 'Thumbnail', 18: 'AppName', 19: 'Security'}
...
class Thumbnail(FieldSet):
    ...
    __module__ = __name__
    FORMAT_CLIPBOARD = -1
    FORMAT_NAME = {-1: 'Windows clipboard', -2: 'Macintosh clipboard', -3: 'GUID that contains format identifier', 0: 'No data', 2: 'Bitmap', 3: 'Windows metafile format', 8: 'Device Independent Bitmap (DIB)', 14: 'Enhanced Windows metafile'}
    DIB_BMP = 8
    DIB_FORMAT = {2: 'Bitmap Obsolete (old BMP)', 3: 'Windows metafile format (WMF)', 8: 'Device Independent Bitmap (BMP)', 14: 'Enhanced Windows metafile (EMF)'}
...
class PropertyContent(FieldSet):
    ...
    def createFields(self):
        ...
        if not handler:
            raise ParserError('OLE2: Unable to parse property of type %s' % self['type'].display)
...
PropertyContent.TYPE_INFO[12] = (
 'VARIANT', PropertyContent)
...
class SummarySection(SeekableFieldSet):
    ...
    def createFields(self):
        ...
        for index in xrange(self['property_count'].value):
            ...
            yield field
            if not self.osconfig.charset and findex['id'].value == PropertyIndex.TAG_CODEPAGE:
...
class SummaryParser(BaseSummary, HachoirParser, RootSeekableFieldSet):
    __module__ = __name__
    PARSER_TAGS = {'description': 'Microsoft Office summary'}
...
class CompObj(FieldSet):
    ...
    def createFields(self):
        ...
        yield PascalString32(self, 'user_type', strip='\x00')
        if self['os_type'].value == OS_MAC:
            yield Int32(self, 'unused[]', '(=-2)')
            yield String(self, 'clipboard_format', 4)
        else:
            yield PascalString32(self, 'clipboard_format', strip='\x00')
        if self.current_size == self.size:
            return
        yield PascalString32(self, 'prog_id', strip='\x00')
        if self['os_type'].value != OS_MAC:
            yield textHandler(UInt32(self, 'magic', 'Magic number (0x71B239F4)'), hexadecimal)
            yield PascalStringWin32(self, 'user_type_unicode', strip='\x00')
            yield PascalStringWin32(self, 'clipboard_format_unicode', strip='\x00')
            yield PascalStringWin32(self, 'prog_id_unicode', strip='\x00')
        size = (self.size - self.current_size) // 8
        if size:
            yield NullBytes(self, 'end_padding', size)