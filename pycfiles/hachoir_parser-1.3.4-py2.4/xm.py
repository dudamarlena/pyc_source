...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/audio/xm.py
...
from hachoir_core.field import StaticFieldSet, FieldSet, Bit, RawBits, Bits, UInt32, UInt16, UInt8, Int8, Enum, RawBytes, String, GenericVector
...
SEMITONE_BASE = 1.0594630943592953
PITCH_BASE = 1.0002256593050698
...
class SampleHeader(FieldSet):
    ...
    def createFields(self):
        ...
        yield String(self, 'name', 22, charset='ASCII', strip=' \x00')

    def createValue(self):
        bytes = 1 + self['type/16bits'].value
        C5_speed = int(16726.0 * pow(SEMITONE_BASE, self['relative_note'].value) * pow(PITCH_BASE, self['fine_tune'].value * 2))
        return '%s, %ubits, %u samples, %uHz' % (self['name'].display, 8 * bytes, self['length'].value / bytes, C5_speed)


class StuffType(StaticFieldSet):
    __module__ = __name__
    format = ((Bits, 'unused', 5), (Bit, 'loop'), (Bit, 'sustain'), (Bit, 'on'))
...
class InstrumentSecondHeader(FieldSet):
    ...
    def createFields(self):
        ...
        yield UInt8(self, 'volume_points', 'Number of volume points')
        yield UInt8(self, 'panning_points', 'Number of panning points')
...
class Instrument(FieldSet):
    ...
    def createDescription(self):
        return "Instrument '%s': %i samples, header %i bytes" % (self['name'].value, self['samples'].value, self['size'].value)


VOLUME_NAME = ('Volume slide down', 'Volume slide up', 'Fine volume slide down', 'Fine volume slide up',
               'Set vibrato speed', 'Vibrato', 'Set panning', 'Panning slide left',
               'Panning slide right', 'Tone porta', 'Unhandled')
...
class NoteInfo(StaticFieldSet):
    __module__ = __name__
    format = ((RawBits, 'unused', 2), (RealBit, 'has_parameter'), (RealBit, 'has_type'), (RealBit, 'has_volume'), (RealBit, 'has_instrument'), (RealBit, 'has_note'))


EFFECT_NAME = ('Arppegio', 'Porta up', 'Porta down', 'Tone porta', 'Vibrato', 'Tone porta+Volume slide',
               'Vibrato+Volume slide', 'Tremolo', 'Set panning', 'Sample offset',
               'Volume slide', 'Position jump', 'Set volume', 'Pattern break', None,
               'Set tempo/BPM', 'Set global volume', 'Global volume slide', 'Unused',
               'Unused', 'Unused', 'Set envelope position', 'Unused', 'Unused', 'Panning slide',
               'Unused', 'Multi retrig note', 'Unused', 'Tremor', 'Unused', 'Unused',
               'Unused', None)
EFFECT_E_NAME = ('Unknown', 'Fine porta up', 'Fine porta down', 'Set gliss control',
                 'Set vibrato control', 'Set finetune', 'Set loop begin/loop', 'Set tremolo control',
                 'Retrig note', 'Fine volume slide up', 'Fine volume slide down',
...
class Effect(RawBits):
    ...
    def createValue(self):
        ...
        if t == 14:
            return EFFECT_E_NAME[(param >> 4)] + ' %i' % (param & 7)
        elif t == 33:
...
class Note(FieldSet):
    ...
    def createFields(self):
        self.info('Note info: 0x%02X' % self.stream.readBits(self.absolute_address, 8, LITTLE_ENDIAN))
    ...
    def createDescription(self):
        if 'info' in self:
            ...
            desc = (
             self['note'].display, 'instrument %i' % self['instrument'].value, self['has_volume'].display, 'effect %s' % self['effect_type'].value, 'parameter %i' % self['effect_parameter'].value)
        if desc:
...
def createPatternContentSize(s, addr):
    return 8 * (s.stream.readBits(addr, 32, LITTLE_ENDIAN) + s.stream.readBits(addr + 7 * 8, 16, LITTLE_ENDIAN))
...
class Pattern(FieldSet):
    ...
    def createFields(self):
        yield UInt32(self, 'header_size', 'Header length (9)')
        yield UInt8(self, 'packing_type', 'Packing type (always 0)')
        yield UInt16(self, 'rows', 'Number of rows in pattern (1..256)')
        yield UInt16(self, 'data_size', 'Packed patterndata size')
...
class Header(FieldSet):
    ...
    def createDescription(self):
        return "'%s' by '%s'" % (self['title'].value, self['tracker_name'].value)


class XMModule(Parser):
    __module__ = __name__
    PARSER_TAGS = {'id': 'fasttracker2', 'category': 'audio', 'file_ext': ('xm', ), 'mime': ('audio/xm', 'audio/x-xm', 'audio/module-xm', 'audio/mod', 'audio/x-mod'), 'magic': ((Header.MAGIC, 0),), 'min_size': Header.static_size + 29 * 8, 'description': 'FastTracker2 module'}
