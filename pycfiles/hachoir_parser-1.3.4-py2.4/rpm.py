...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/archive/rpm.py
...
from hachoir_core.field import FieldSet, ParserError, UInt8, UInt16, UInt32, UInt64, Enum, NullBytes, Bytes, RawBytes, SubFile, Character, CString, String
...
class ItemContent(FieldSet):
    __module__ = __name__
    format_type = {0: UInt8, 1: Character, 2: UInt8, 3: UInt16, 4: UInt32, 5: UInt64, 6: CString, 7: RawBytes, 8: CString, 9: CString}
...
class Item(FieldSet):
    __module__ = __name__
    type_name = {0: 'NULL', 1: 'CHAR', 2: 'INT8', 3: 'INT16', 4: 'INT32', 5: 'INT64', 6: 'CSTRING', 7: 'BIN', 8: 'CSTRING_ARRAY', 9: 'CSTRING?'}
    tag_name = {1000: 'File size', 1001: '(Broken) MD5 signature', 1002: 'PGP 2.6.3 signature', 1003: '(Broken) MD5 signature', 1004: 'MD5 signature', 1005: 'GnuPG signature', 1006: 'PGP5 signature', 1007: 'Uncompressed payload size (bytes)', 256 + 8: 'Broken SHA1 header digest', 256 + 9: 'Broken SHA1 header digest', 256 + 13: 'Broken SHA1 header digest', 256 + 11: 'DSA header signature', 256 + 12: 'RSA header signature'}
...
class PropertySet(FieldSet):
    ...
    def createFields(self):
        yield Bytes(self, 'signature', 3, 'Property signature (\\x8E\\xAD\\xE8)')
...
class RpmFile(Parser):
    __module__ = __name__
    PARSER_TAGS = {'id': 'rpm', 'category': 'archive', 'file_ext': ('rpm', ), 'mime': ('application/x-rpm', ), 'min_size': (96 + 16 + 16) * 8, 'magic': ((b'\xed\xab\xee\xdb', 0), ), 'description': 'RPM package'}
    TYPE_NAME = {0: 'Binary', 1: 'Source'}
    endian = BIG_ENDIAN
    ...
    def createFields(self):
        yield Bytes(self, 'signature', 4, 'RPM file signature (\\xED\\xAB\\xEE\\xDB)')
        ...
        yield String(self, 'name', 66, 'Archive name', strip='\x00', charset='ASCII')
