...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/game/blp.py
...
class BLP1File(Parser):
    __module__ = __name__
    MAGIC = 'BLP1'
    PARSER_TAGS = {'id': 'blp1', 'category': 'game', 'file_ext': ('blp', ), 'mime': ('application/x-blp', ), 'magic': ((MAGIC, 0),), 'min_size': 7 * 32, 'description': 'Blizzard Image Format, version 1'}
    ...
    def createFields(self):
        ...
        yield Enum(UInt32(self, 'compression'), {0: 'JPEG Compression', 1: 'Uncompressed'})
        yield UInt32(self, 'flags')
        yield UInt32(self, 'width')
        yield UInt32(self, 'height')
        yield Enum(UInt32(self, 'type'), {3: 'Uncompressed Index List + Alpha List', 4: 'Uncompressed Index List + Alpha List', 5: 'Uncompressed Index List'})
...
class DXT1(FieldSet):
    ...
    def createFields(self):
        ...
        for i in (0, 1):
            ...
            values[i] = [self[('red[%i]' % i)].value, self[('green[%i]' % i)].value, self[('blue[%i]' % i)].value]
...
class BLP2File(Parser):
    __module__ = __name__
    MAGIC = 'BLP2'
    PARSER_TAGS = {'id': 'blp2', 'category': 'game', 'file_ext': ('blp', ), 'mime': ('application/x-blp', ), 'magic': ((MAGIC, 0),), 'min_size': 5 * 32, 'description': 'Blizzard Image Format, version 2'}
    endian = LITTLE_ENDIAN
    ...
    def createFields(self):
        ...
        yield Enum(UInt32(self, 'compression', 'Compression type'), {0: 'JPEG Compressed', 1: 'Uncompressed or DXT/S3TC compressed'})
        yield Enum(UInt8(self, 'encoding', 'Encoding type'), {1: 'Raw', 2: 'DXT/S3TC Texture Compression (a.k.a. DirectX)'})
        yield UInt8(self, 'alpha_depth', 'Alpha channel depth, in bits (0 = no alpha)')
        yield Enum(UInt8(self, 'alpha_encoding', 'Encoding used for alpha channel'), {0: 'DXT1 alpha (0 or 1 bit alpha)', 1: 'DXT3 alpha (4 bit alpha)', 7: 'DXT5 alpha (8 bit interpolated alpha)'})
        yield Enum(UInt8(self, 'has_mips', 'Are mip levels present?'), {0: 'No mip levels', 1: 'Mip levels present; number of levels determined by image size'})
