...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/archive/gzip_parser.py
...
from hachoir_core.field import UInt8, UInt16, UInt32, Enum, TimestampUnix32, Bit, CString, SubFile, NullBits, Bytes, RawBytes
...
class GzipParser(Parser):
    ...
    PARSER_TAGS = {'id': 'gzip', 'category': 'archive', 'file_ext': ('gz', ), 'mime': ('application/x-gzip', ), 'min_size': 18 * 8, 'magic_regex': ((b'\x1f\x8b\x08.{5}[\x00\x02\x04\x06][\x00-\r]', 0), ), 'description': 'gzip archive'}
    os_name = {0: 'FAT filesystem', 1: 'Amiga', 2: 'VMS (or OpenVMS)', 3: 'Unix', 4: 'VM/CMS', 5: 'Atari TOS', 6: 'HPFS filesystem (OS/2, NT)', 7: 'Macintosh', 8: 'Z-System', 9: 'CP/M', 10: 'TOPS-20', 11: 'NTFS filesystem (NT)', 12: 'QDOS', 13: 'Acorn RISCOS'}
    COMPRESSION_NAME = {8: 'deflate'}
    ...
    def createFields(self):
        yield Bytes(self, 'signature', 2, 'GZip file signature (\\x1F\\x8B)')
        ...
        size = (self._size - self.current_size) // 8 - 8
        if 0 < size:
            if self['has_filename'].value:
