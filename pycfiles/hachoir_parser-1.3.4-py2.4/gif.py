...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/image/gif.py
# Compiled at: 2010-01-20 18:01:20
"""
GIF picture parser.
...
from hachoir_core.field import FieldSet, ParserError, Enum, UInt8, UInt16, Bit, Bits, NullBytes, String, PascalString8, Character, NullBits, RawBytes
...
class Image(FieldSet):
    ...
    def createDescription(self):
        return 'Image: %ux%u pixels at (%u,%u)' % (self['width'].value, self['height'].value, self['left'].value, self['top'].value)


DISPOSAL_METHOD = {0: 'No disposal specified', 1: 'Do not dispose', 2: 'Restore to background color', 3: 'Restore to previous'}
NETSCAPE_CODE = {1: 'Loop count'}
...
def parseComments(parent):
    while True:
        field = PascalString8(parent, 'comment[]', strip=' \x00\r\n\t')
        yield field
        if field.length == 0:
            break
...
class Extension(FieldSet):
    __module__ = __name__
    ext_code = {249: ('graphic_ctl[]', parseGraphicControl, 'Graphic control'), 254: ('comments[]', parseComments, 'Comments'), 255: ('app_ext[]', parseApplicationExtension, 'Application extension'), 1: ('text_ext[]', parseTextExtension, 'Plain text extension')}
...
class ScreenDescriptor(FieldSet):
    ...
    def createDescription(self):
        ...
        return 'Screen descriptor: %ux%u pixels %u colors' % (self['width'].value, self['height'].value, colors)


class GifFile(Parser):
    __module__ = __name__
    endian = LITTLE_ENDIAN
    separator_name = {'!': 'Extension', ',': 'Image', ';': 'Terminator'}
    PARSER_TAGS = {'id': 'gif', 'category': 'image', 'file_ext': ('gif', ), 'mime': ('image/gif', ), 'min_size': (6 + 7 + 1 + 9) * 8, 'magic': (('GIF87a', 0), ('GIF89a', 0)), 'description': 'GIF picture'}
    ...
    def createContentSize(self):
        ...
        pos = self.stream.searchBytes('\x00;', start, end)
        if pos:
            return pos + 16
        return