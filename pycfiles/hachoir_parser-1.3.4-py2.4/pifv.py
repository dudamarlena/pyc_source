...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_parser/misc/pifv.py
...
from hachoir_core.field import FieldSet, UInt8, UInt16, UInt24, UInt32, UInt64, Enum, CString, String, PaddingBytes, RawBytes, NullBytes
...
class BlockMap(FieldSet):
    ...
    def createDescription(self):
        return '%d blocks of %s' % (self['num_blocks'].value, humanFilesize(self['len'].value))


class FileSection(FieldSet):
    __module__ = __name__
    COMPRESSION_TYPE = {0: 'Not Compressed', 1: 'Standard Compression'}
    ...
    def __init__(self, *args, **kw):
        ...
        if section_type in (EFI_SECTION_DXE_DEPEX, EFI_SECTION_PEI_DEPEX):
            if self.stream.readBytes(self.absolute_address + self._size, 1) == '\x00':
    ...
    def createFields(self):
        ...
        if section_type == EFI_SECTION_COMPRESSION:
            ...
            yield RawBytes(self, 'content', content_size, EFI_SECTION_TYPE.get(self['type'].value, 'Unknown Section Type'))

    def createDescription(self):
        return EFI_SECTION_TYPE.get(self['type'].value, 'Unknown Section Type')
...
class File(FieldSet):
    ...
    def createDescription(self):
        return '%s: %s containing %d section(s)' % (self['name'].value, self['type'].display, len(self.array('section')))
...
class PIFVFile(Parser):
    ...
    MAGIC = '_FVH'
    PARSER_TAGS = {'id': 'pifv', 'category': 'program', 'file_ext': ('bin', ''), 'min_size': 64 * 8, 'magic_regex': (('\x00{16}.{24}%s' % MAGIC, 0),), 'description': 'EFI Platform Initialization Firmware Volume'}

    def validate(self):
        if self.stream.readBytes(40 * 8, 4) != self.MAGIC:
            return 'Invalid magic number'
        if self.stream.readBytes(0, 16) != '\x00' * 16:
