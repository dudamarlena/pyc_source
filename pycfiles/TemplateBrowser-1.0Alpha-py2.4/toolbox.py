...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\templatebrowser\toolbox.py
# Compiled at: 2007-01-30 21:24:25
"""Graphical user interface for template designer"""
import os, turbogears
...
class TemplateBrowser(controllers.Controller):
    ...
    def project_files(self):
        ...
        def collect_files(file_list, dirpath, namelist):
            ...
            slot0 = {'dir': os.path.dirname(dirpath), 'file_name': os.path.basename(dirpath), 'path': dirpath, 'isdir': True, 'level': level}
            slot1 = []
            slot2 = []
            slots = (
             slot0, slot1, slot2)
            ...
            for name in namelist[:]:
                ...
                if os.path.isfile(p) and os.path.splitext(name)[(-1)] in ['.kid', '.tmpl', '.html']:
                    slot2.append({'dir': dirpath, 'file_name': name, 'path': p, 'isdir': os.path.isdir(p), 'level': level + 1})
        ...
        return [ x for x in turbogears.util.flatten_sequence(fl) if not x['isdir'] or visibility[x['path']] ]
