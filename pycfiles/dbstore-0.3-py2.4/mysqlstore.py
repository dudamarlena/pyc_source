...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/dbstore/mysqlstore.py
# Compiled at: 2006-10-25 11:01:24
...
def mysql_storage(*args, **kwargs):
    ...
    def MySQLStore(*args, **kwargs):
        return DBSessionStore(get_connection, escape_string, DatabaseError, *args, **kwargs)
...
if __name__ == '__main__':
    sessionized = mysql_sessionizer(host='localhost', user='foo', passwd='foo', db='foo')
    ...
    def foo_app(environ, start_response):
        ...
        if not session.has_key('foo'):
            session['foo'] = 0
            session['bar'] = {'a': 'A', 'b': 'B'}
            session['baz'] = 'Try these !@#$%^&*()_+-=|}{\\][\\":\';?></.,'
        session['foo'] += 1
        start_response('200 Ok', [('content-type', 'text/plain')])
        return ['Session id: %s\n' % session.identifier, "Session['foo'] == %s\n" % session['foo'], 'a == %(a)r and b == %(b)r\n' % session['bar'], session['baz']]
