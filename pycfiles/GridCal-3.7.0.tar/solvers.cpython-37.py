...
# [GCC 8.4.0]
# Embedded file name: /home/santi/Documentos/GitHub/GridCal/src/GridCal/ThirdParty/pulp/solvers.py
# Compiled at: 2019-08-27 13:05:30
...
from GridCal.ThirdParty.pulp import sparse
import warnings
from tempfile import mktemp
from GridCal.ThirdParty.pulp.constants import *
import logging
...
if os.name == 'posix':
    if sys.version_info[0] < 3:
        try:
            ...
            log.debug('Thread-safe subprocess32 module not found! Using unsafe built-in subprocess module instead.')
            import subprocess
    ...
    """
        ...
        try:
            try:
                ilm_cplex_license = config.get('licenses', 'ilm_cplex_license').decode('string-escape').replace('"', '')
            except AttributeError:
        ...
        try:
            ilm_cplex_license_signature = config.getint('licenses', 'ilm_cplex_license_signature')
        except configparser.Error:
...
if __name__ != '__main__':
    ...
    import GridCal.ThirdParty.pulp as fname
    DIRNAME = os.path.dirname(fname)
    config_filename = os.path.join(DIRNAME, PULPCFGFILE)
cplex_dll_path, ilm_cplex_license, ilm_cplex_license_signature, coinMP_path, gurobi_path, cbc_path, glpk_path, pulp_cbc_path, scip_path = initialize(config_filename, operating_system, arch)
try:
    import ctypes
...
class LpSolver:
    ...
    def getCplexStyleArrays(self, lp, senseDict={LpConstraintEQ: 'E', LpConstraintLE: 'L', LpConstraintGE: 'G'}, LpVarCategories={LpContinuous: 'C', LpInteger: 'I'}, LpObjSenses={LpMaximize: -1, LpMinimize: 1}, infBound=1e+20):
        """
...
class LpSolver_CMD(LpSolver):
    __doc__ = 'A generic command line LP Solver'
    ...
    def setTmpDir(self):
        ...
        if not os.path.isdir(self.tmpDir):
            ...
            if not os.access(self.tmpDir, os.F_OK + os.W_OK):
                self.tmpDir = ''
