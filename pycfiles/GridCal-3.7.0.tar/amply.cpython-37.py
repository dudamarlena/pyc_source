...
# [GCC 8.4.0]
# Embedded file name: /home/santi/Documentos/GitHub/GridCal/src/GridCal/ThirdParty/pulp/amply.py
# Compiled at: 2019-07-13 06:16:34
# Size of source mod 2**32: 24936 bytes
"""
...
try:
        ...
        def eval(self, amply):
            for i, param_name in enumerate(self.params):
                if param_name in amply.symbols:
                    ...
                    raise AmplyError('Param %s not previously defined' % param_name)
    ...
    class ParamObject(AmplyObject):
            ...
            for record in data:
                if isinstance(record, SliceRecord):
                    ...
                    if len(record) % rec_len != 0:
                        raise AmplyError('Incomplete data record, expecting %d subscripts per value' % len(self.free_indices))
        ...
        def _setSlice(self, slice):
            self.current_slice = list(slice.components)
            self.free_indices = [i for i, v in enumerate(self.current_slice) if v == '*']
    ...
    class SetObject(AmplyObject):
        ...
        def _addSimpleData(self, data_list, data):
            ...
            if len(self.free_indices) == inferred_dimen:
                ...
                if len(self.free_indices) > 1 and inferred_dimen:
                    for c in chunk(data, len(self.free_indices)):
                        self._addValue(data_list, c)

                else:
                    raise AmplyError('Dimension of elements (%d) does not match declared dimension, (%d)' % (
    ...
    number = Combine(Word('+-' + nums, nums) + Optional('.' + Optional(Word(nums))) + Optional(oneOf('e E') + Word('+-' + nums, nums))).setParseAction(lambda t: float(t[0]))
    LPAREN = Suppress('(')
    RPAREN = Suppress(')')
    LBRACE = Suppress('{')
    RBRACE = Suppress('}')
    ...
    subscript_domain = LBRACE + Group(delimitedList(symbol)).setResultsName('subscripts') + RBRACE
    data = single | tuple_
    ...
    matrix_data = ':' + OneOrMore(single).setResultsName('columns') + ':=' + OneOrMore(matrix_row).setResultsName('data')
    ...
    set_def_stmt = 'set' + symbol + Optional(subscript_domain) + Optional('dimen' + integer.setResultsName('dimen')) + END
    set_def_stmt.setParseAction(SetDefStmt)
    set_member = LBRACKET + delimitedList(data) + RBRACKET
    set_stmt = 'set' + symbol + Optional(set_member).setResultsName('member') + Group(non_dimen_set_record + ZeroOrMore(Optional(Suppress(',')) + set_record)).setResultsName('records') + END
    set_stmt.setParseAction(SetStmt)
    subscript = single
    param_data = data | '.'
    plain_data = param_data | subscript + ZeroOrMore(Optional(Suppress(',')) + subscript) + param_data
    plain_data_record = Group(NotAny('(tr)') + plain_data + NotAny(plain_data) | plain_data + OneOrMore(plain_data) + NotAny(plain_data))
    tabular_record = ':' + OneOrMore(single).setResultsName('columns') + ':=' + OneOrMore(single | '.').setResultsName('data')
    ...
    param_record = param_slice_record | plain_data_record | tabular_record | tr_tabular_record | Suppress(':=')
    param_default = Optional('default' + data.setResultsName('default'))
    param_stmt = 'param' + symbol.setResultsName('name') + param_default + Group(OneOrMore(param_record)).setResultsName('records') + END
    param_stmt.setParseAction(ParamStmt)
    param_tabbing_stmt = 'param' + param_default + ':' + Optional(symbol + ':') + OneOrMore(data).setResultsName('params') + ':=' + OneOrMore(single).setResultsName('data') + END
    param_tabbing_stmt.setParseAction(ParamTabbingStmt)
    param_def_stmt = 'param' + symbol + Optional(subscript_domain) + param_default + END
