...
# [GCC 8.4.0]
# Embedded file name: /home/santi/Documentos/GitHub/GridCal/src/GridCal/ThirdParty/pulp/solver_interfaces/cplex.py
# Compiled at: 2019-08-27 13:05:03
# Size of source mod 2**32: 34679 bytes
from GridCal.ThirdParty.pulp.solvers import *

class CPLEX_CMD(LpSolver_CMD):
    __doc__ = 'The CPLEX LP solver'

    def __init__(self, path=None, keepFiles=0, mip=1, msg=0, options=[], timelimit=None):
    ...
    def actualSolve(self, lp):
        ...
        if not self.executable(self.path):
            ...
            if not self.msg:
                cplex = subprocess.Popen((self.path), stdin=(subprocess.PIPE), stdout=(subprocess.PIPE),
            ...
            if cplex.returncode != 0:
                ...
                status = os.path.exists(tmpSol) or LpStatusInfeasible
            else:
    ...
    def readsol(self, filename):
        ...
        cplexStatus = {'optimal': LpStatusOptimal}
...
try:
    ...
    class CPLEX_DLL(LpSolver):
        ...
        def changeEpgap(self, epgap=0.0001):
            """
        ...
        def findSolutionValues(self, lp, numcols, numrows):
            ...
            if lp.isMIP():
                ...
                status = CPLEX_DLL.lib.CPXsolution(self.env, self.hprob, byref(solutionStatus), byref(objectiveValue), byref(x), byref(pi), byref(slack), byref(dj))
        ...
        def grabLicence(self):
            ...
            """
            status = ctypes.c_int()
            if ilm_cplex_license:
                if ilm_cplex_license_signature:
                    runtime_status = CPLEX_DLL.lib.CPXsetstaringsol(ilm_cplex_license, ilm_cplex_license_signature)
                    if not runtime_status == 0:
                        if self.msg:
                            print('CPLEX library failed to load the runtime license' + 'the call returned status=%s' % str(runtime_status) + 'Please check the pulp config file.')
        ...
        def actualSolve(self, lp):
            ...
            numcols, numrows, numels, rangeCount, objSense, obj, objconst, rhs, rangeValues, rowSense, matbeg, matcnt, matind, matval, lb, ub, initValues, colname, rowname, xctype, n2v, n2c = self.getCplexStyleArrays(lp)
            status.value = CPLEX_DLL.lib.CPXcopylpwnames(self.env, self.hprob, numcols, numrows, objSense, obj, rhs, rowSense, matbeg, matcnt, matind, matval, lb, ub, None, colname, rowname)
            if status.value != 0:
        ...
        def actualResolve(self, lp):
            ...
            ccnt, nzcnt, obj, cmatbeg, cmatlen, cmatind, cmatval, lb, ub, initvals, colname, coltype = self.getSparseCols(newVars, lp, oldVars, defBound=1e+20)
            CPXaddcolsStatus = CPLEX_DLL.lib.CPXaddcols(self.env, self.hprob, ccnt, nzcnt, obj, cmatbeg, cmatind, cmatval, lb, ub, colname)
            if lp.isMIP():
                if self.mip:
                    ...
                    CPXchgctypeStatus = CPLEX_DLL.lib.CPXchgctype(self.env, self.hprob, ccnt, indices, coltype)
            self.callSolver(lp.isMIP())
        ...
        def getSparseCols(self, vars, lp, offset=0, defBound=1e+20):
            """
        ...
        def objSa(self, vars=None):
            ...
            hi = row_t()
            status = ctypes.c_int()
            status.value = CPLEX_DLL.lib.CPXobjsa(self.env, self.hprob, ifirst, ilast, lo, hi)
            if status.value != 0:
...
try:
    ...
    class CPLEX_PY(LpSolver):
        ...
        def buildSolverModel(self, lp):
            ...
            lp.solverModel.linear_constraints.add(lin_expr=rows, senses=senses, rhs=rhs,
              names=rownames)
