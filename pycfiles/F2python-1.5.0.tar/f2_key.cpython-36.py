...
# [GCC 8.4.0]
# Embedded file name: /Users/thibault/Work/Recherche/F2/F2Python/F2/f2_key.py
...
class F2_Key(F2_Object):
    __doc__ = "an F2 oid for a 'Kernel.Key' object."
    ...
    def post_create(self, **attr_values):
        ...
        for obj in ofClass():
            if isinstance(keyAttrs, list):
                keyStoreTuple = tuple([nakedValue(getattr(obj, a.attributeName, F2_NULL)) for a in keyAttrs])
            else:
                keyStoreTuple = (
                 nakedValue(getattr(obj, keyAttrs.attributeName)),)
...
def currentClassKeySchema():
    ...
    return (
     classKey, attrKeyAttrs, attrOfClass)
...
class KeyStorage(Persistent):
    __doc__ = 'a storage for the index added to a key'
