...
# [GCC 8.4.0]
# Embedded file name: /Users/thibault/Work/Recherche/F2/F2Python/F2/OFF.py
# Compiled at: 2017-03-11 16:12:29
...
"""OFF.py
   ...
   function r_f_a may optionnaly be stored (indexed), for obvious optimization reasons. 
...
class OFFStore:
            ...
            if store_name[:5] == 'file:':
                ...
                if store_name[:4] == 'rpc:':
                    ...
                    if server_addr.find(':') >= 0:
                        ...
                        serv_port = [
                         server_addr, 8080]
                    storage = ClientStorage.ClientStorage(addr=(tuple(serv_port)), username=username,
                      password=passwd)
                    self.db = DB(storage)
                    self.connection = self.db.open()
                else:
                    if store_name[:7] == 'shared:':
    ...
    def new_db_oid(self):
        ...
        no = self.connection.new_oid()
        if isinstance(no, str):
            return reduce(lambda x, y: 256 * x + ord(y), no, 0)
        else:
            return reduce(lambda x, y: 256 * x + y, no, 0)


class F2StorageError(Exception):
    __doc__ = 'OFF exception'
...
from . import metadict

def cache_functions(offStore):
    global attributeName
    global baseType
    global className
    global classStateAttr
    global domainClass
    global infValue
    global maxCard
    global minCard
    global rangeClass
    global stateAtomClass
    global stateAttribute
    global stateCLASS
    global stateSubClass
    global stateTupleClass
    global supValue
    global superClass
