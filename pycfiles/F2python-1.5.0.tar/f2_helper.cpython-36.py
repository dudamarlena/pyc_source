...
# [GCC 8.4.0]
# Embedded file name: /Users/thibault/Work/Recherche/F2/F2Python/F2/f2_helper.py
...
class F2_Helper:
    __doc__ = 'an object provinding services to a F2 connection.'
        ...
        for attr_name, attr_vals in list(schema.items()):
            if isinstance(attr_vals, tuple):
                ...
                if maxCard in ('n', 'N', '*'):
                    ...
                    minCard = None
                    maxCard = None
                n = Attribute.create(attributeName=attr_name, domainClass=newC,
                  rangeClass=rangeC,
                  visibility=1)
    ...
    def json_repr(self, some_value, visited_objs=None, using=None, depth=1):
        ...
        if not visited_objs:
            ...
            if isinstance(some_value, F2_Object):
                if depth <= 0 or some_value in visited_objs or not some_value:
                    ...
                    attr_list = [a.attributeName for a in o_class.all_attributes() if a.visibility == 1 if a.attributeName in using or not using]
                    res = ['"' + aName + '":' + self.json_repr((getattr(some_value, aName)), (visited_objs + [some_value]), using=using, depth=(depth - 1)) for aName in attr_list]
                    return '{' + ','.join(res) + '}'
            else:
        ...
        if not visited_objs:
            ...
            if isinstance(some_value, list):
                if len(some_value) == 0:
                    ...
                    for o in some_value:
                        res = res + '<tr><td>%s</td></tr>\n' % self.html_repr(o, (visited_objs + some_value),
                          using=using,
                          depth=(depth - 1))
                ...
                if isinstance(some_value, F2_Object):
                    if depth <= 0 or some_value in visited_objs or not some_value:
                        ...
                        return self.html_table_repr(F2_Object_list([some_value]), visited_objs + [some_value], using, depth - 1)
                else:
                    if some_value != '?':
                        return html_encode(str(some_value))
                    else:
    ...
    def html_table_repr(self, value_list, visited_objs=None, using=None, depth=1):
        ...
        attr_list = [(a.attributeName, a.rangeClass.className) for a in o_class.all_attributes() if a.visibility == 1 if a.attributeName in using or not using]
        ...
        for obj in value_list:
            res = res + '</tr>\n<tr>'
            for a, c in attr_list:
                res = res + '<td>%s</td>' % self.html_repr((getattr(obj, a)), (visited_objs + [obj]),
                  using=using,
                  depth=(depth - 1))
