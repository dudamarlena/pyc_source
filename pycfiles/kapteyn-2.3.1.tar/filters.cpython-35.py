...
# [GCC 8.4.0]
# Embedded file name: /net/jansky/data/users/breddels/src/kapteyn-sky2/kapteyn/filters.py
# Compiled at: 2016-03-21 11:20:24
# Size of source mod 2**32: 34205 bytes
import math, numpy
from . import _ni_support
from kapteyn import _nd_image
from kapteyn import doccer
...
_mode_doc = "mode : {'reflect','constant','nearest','mirror', 'wrap'}, optional\n    The ``mode`` parameter determines how the array borders are\n    handled, where ``cval`` is the value when mode is equal to\n    'constant'. Default is 'reflect'"
...
@docfiller
def gaussian_filter1d(input, sigma, axis=-1, order=0, output=None, mode='reflect', cval=0.0):
...
def generic_laplace(input, derivative2, output=None, mode='reflect', cval=0.0, extra_arguments=(), extra_keywords=None):
    ...
    if len(axes) > 0:
        ...
        for ii in range(1, len(axes)):
            tmp = derivative2(input, axes[ii], output.dtype, mode, cval, *extra_arguments, **extra_keywords)
            output += tmp
...
def gaussian_laplace(input, sigma, output=None, mode='reflect', cval=0.0):
    ...
    return generic_laplace(input, derivative2, output, mode, cval, extra_arguments=(
     sigma,))
...
def generic_gradient_magnitude(input, derivative, output=None, mode='reflect', cval=0.0, extra_arguments=(), extra_keywords=None):
    ...
    if len(axes) > 0:
        ...
        for ii in range(1, len(axes)):
            tmp = derivative(input, axes[ii], output.dtype, mode, cval, *extra_arguments, **extra_keywords)
...
def _min_or_max_filter(input, size, footprint, structure, output, mode, cval, origin, minimum):
    ...
    if separable:
        ...
        if len(axes) > 0:
            for axis, size, origin in axes:
                list(filter(input, int(size), axis, output, mode, cval, origin))
                input = output
...
def minimum_filter(input, size=None, footprint=None, output=None, mode='reflect', cval=0.0, origin=0):
    ...
    %(origin)s
    """
    return _min_or_max_filter(input, size, footprint, None, output, mode, cval, origin, 1)
...
@docfiller
def _rank_filter(input, rank, size=None, footprint=None, output=None, mode='reflect', cval=0.0, origin=0, operation='rank'):
    input = numpy.asarray(input)
    if numpy.iscomplexobj(input):
        raise TypeError('Complex type not supported')
    origins = _ni_support._normalize_sequence(origin, input.ndim)
...
def rank_filter(input, rank, size=None, footprint=None, output=None, mode='reflect', cval=0.0, origin=0):
    ...
    %(origin)s
    """
    return _rank_filter(input, rank, size, footprint, output, mode, cval, origin, 'rank')
...
def median_filter(input, size=None, footprint=None, output=None, mode='reflect', cval=0.0, origin=0):
    ...
    %(origin)s
    """
    return _rank_filter(input, 0, size, footprint, output, mode, cval, origin, 'median')
...
@docfiller
def generic_filter(input, function, size=None, footprint=None, output=None, mode='reflect', cval=0.0, origin=0, extra_arguments=(), extra_keywords=None):
