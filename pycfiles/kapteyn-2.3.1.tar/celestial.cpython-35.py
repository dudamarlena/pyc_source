...
# [GCC 8.4.0]
# Embedded file name: /net/jansky/data/users/breddels/src/kapteyn-sky2/kapteyn/celestial.py
# Compiled at: 2016-04-11 08:58:45
# Size of source mod 2**32: 125461 bytes
"""
Module Celestial
================
   ...
   """
    if month > 2:
        ...
        if calday > 0.0229:
            ...
            jd = int(365.25 * y - 0.75) + int(30.6001 * (m + 1)) + day + 1720994.5 + B
    return jd
   ...
   """
    ...
    if tex:
        if prec > 0:
            ...
            s1 = '$%d^h%.2d^m%.2d^s$' % (Ihours, Imin, secleft)
            if delta == None:
                s = s1
            else:
                if delta * 3600.0 % 54000.0 == 0.0:
                    s = '$%d^h$' % Ihours
                else:
                    if delta * 3600.0 % 900.0 == 0.0:
                        s = '$%d^h%.2d^m$' % (Ihours, Imin)
                    else:
                        s = s1
    else:
        if prec > 0:
...
:Notes:
   ...
   """
    ...
    if tex:
        if prec > 0:
            dms = '%c%d^{\\circ}%.2d^{\\prime}%.2d^{\\prime\\prime}' % (si, Ideg, Imin, secleft)
            fsec = '.%*.*d' % (prec, prec, int(round(Fsec * 10.0 ** prec, 0)))
            s = '$' + dms + fsec + '$'
        else:
            s1 = '$%c%d^{\\circ}%.2d^{\\prime}%.2d^{\\prime\\prime}$' % (si, Ideg, Imin, secleft)
            if delta == None:
                s = s1
            else:
                if delta * 3600.0 % 3600 == 0.0:
                    s = '$%c%d^{\\circ}$' % (si, Ideg)
                else:
                    if delta * 3600.0 % 60 == 0.0:
                        s = '$%c%d^{\\circ}%.2d^{\\prime}$' % (si, Ideg, Imin)
                    else:
                        s = s1
    else:
        if prec > 0:
...
:Examples:
   ...
   """
    ...
    if tex:
        if prec > 0:
            dms = '%d^{\\circ}%.2d^{\\prime}%.2d^{\\prime\\prime}' % (Ideg, Imin, secleft)
            fsec = '.%*.*d' % (prec, prec, int(round(Fsec * 10.0 ** prec, 0)))
            s = '$' + dms + fsec + '$'
        else:
            s1 = '$%d^{\\circ}%.2d^{\\prime}%.2d^{\\prime\\prime}$' % (Ideg, Imin, secleft)
            if delta == None:
                s = s1
            else:
                if delta * 3600.0 % 3600 == 0.0:
...
:Notes:
   ...
   """
    ...
    eps = (84381.448 + (-46.815 + (-0.00059 + 0.001813 * T) * T) * T) / 3600.0
    return eps
...
:Notes:
   ...
   """
    T = (jd - 2451545.0) / 36525.0
    eps = (84381.406 + (-46.836769 + (-0.0001831 + (0.0020034 + (-5.76e-07 + -4.34e-08 * T) * T) * T) * T) * T) / 3600.0
    return eps
...
:Notes:
   ...
   *  Longitude increases from 0 to 360 deg. The sense is
   ...
   """
    return rotZ(57.0) * rotY(62.6) * rotZ(192.25)
...
:Notes:      
   ...
   """
    return rotZ(90.0) * rotY(83.68) * rotZ(47.37)
...
:Notes:     
   ...
   """
    ...
    ec = 0.01673011 - (4.193e-05 + 1.26e-07 * T) * T
    ob = 84404.836 - (46.8495 + (0.00319 + 0.00181 * T) * T) * T
    ob = d2r(ob / 3600.0)
    p = 1015489.951 + (6190.67 + (1.65 + 0.012 * T) * T) * T
    p = d2r(p / 3600.0)
    ek = ec * d2r(0.005693116666666667)
    cp = n.cos(p)
    return (
     ek * n.sin(p), -ek * cp * n.cos(ob), -ek * cp * n.sin(ob))
...
:Notes:     
   ...
   """
    ...
    for i in range(xyz.shape[1]):
        ...
        lambda1 = (-w + n.sqrt(w * w - 4.0 * p)) / 2.0
...
----------------------------------------------------------------------
   """
    r0 = [
     0.999925678186902, -0.011182059642247, -0.00485794655896]
    r1 = [0.011182059571766, 0.999937478448132, -2.717441185e-05]
    r3 = [0.004857946721186, -2.7147426498e-05, 0.99998819973877]
...
----------------------------------------------------------------------
   """
    ...
    r1 = [0.011182061, 0.9999374784, -2.71765e-05]
    r3 = [0.0048579479, -2.71474e-05, 0.9999881997]
    return n.matrix((r0, r1, r3))
...
----------------------------------------------------------------------
   """
    ...
    pmf = 129600000.0 / twopi
    d = 1950.0
    mjd = 15019.81352 + (d - 1900) * 365.242198781
    Julianepoch = 2000.0 + (mjd - 51544.5) / 365.25
    w = (Julianepoch - 2000.0) / pmf
    xyzpm = xyz.copy()
    r0 = [-0.000551, 0.238514, -0.435623]
...
----------------------------------------------------------------------
   """
    ...
    juliancenturies = (jd2 - jd1) / 36525.0
    E0 = 0.525
    E1 = 1.275
...
:Notes:     
   ...
   """
    eta0 = -1.8942222222222224e-06
    xi0 = -4.615833333333333e-06
    da0 = -4.055555555555556e-06
    return rotX(-eta0) * rotY(xi0) * rotZ(da0)
   ...
   """
    if skyin == equatorial:
        if skyout == equatorial:
            M1 = MatrixEpoch12Epoch2(epoch1, epoch2, S1, S2, epobs)
            return M1
        if skyout == ecliptic:
        ...
        if skyin == ecliptic:
            if skyout == equatorial:
                M1 = MatrixEq2Ecl(epoch1, S1).T
                M2 = MatrixEpoch12Epoch2(epoch1, epoch2, S1, S2)
                return M2 * M1
            if skyout == ecliptic:
                M1 = MatrixEq2Ecl(epoch1, S1).T
                M2 = MatrixEpoch12Epoch2(epoch1, epoch2, S1, S2)
                M3 = MatrixEq2Ecl(epoch2, S2)
                return M3 * M2 * M1
            if skyout == galactic:
                M1 = MatrixEq2Ecl(epoch1, S1).T
                M2 = MatrixEpoch12Epoch2(epoch1, 1950.0, S1, fk4)
                M3 = MatrixEqB19502Gal()
                return M3 * M2 * M1
            if skyout == supergalactic:
                M1 = MatrixEq2Ecl(epoch1, S1).T
                M2 = MatrixEpoch12Epoch2(epoch1, 1950.0, S1, fk4)
            ...
            if skyin == galactic:
                if skyout == equatorial:
                    M1 = MatrixEqB19502Gal().T
                    M2 = MatrixEpoch12Epoch2(1950.0, epoch2, fk4, S2)
                    return M2 * M1
                if skyout == ecliptic:
                    M1 = MatrixEqB19502Gal().T
                    M2 = MatrixEpoch12Epoch2(1950.0, epoch2, fk4, S2)
                ...
                if skyin == supergalactic:
                    if skyout == equatorial:
                        M1 = MatrixGal2Sgal().T
                        M2 = MatrixEqB19502Gal().T
                        M3 = MatrixEpoch12Epoch2(1950.0, epoch2, fk4, S2)
                        return M3 * M2 * M1
                    if skyout == ecliptic:
                        M1 = MatrixGal2Sgal().T
                        M2 = MatrixEqB19502Gal().T
                        M3 = MatrixEpoch12Epoch2(1950.0, epoch2, fk4, S2)
                        M4 = MatrixEq2Ecl(epoch2, S2)
                        return M4 * M3 * M2 * M1
                    if skyout == galactic:
                        M1 = MatrixGal2Sgal().T
                        return M1
   ...
   """
    ...
    for element in skyin:
        if type(element) == int:
            ...
            if isinstance(element, six.string_types):
                if first and element.find('_') == -1:
                    epochinset = epochs(element)
                    first = False
                else:
                    epobs = epochs(element)[0]
            elif element != None:
    ...
    if sysin == None:
        ...
        if epochinset == None:
            ...
            if refin == fk4 or refin == fk4_no_e:
                epochin = epochinset[0]
            else:
                epochin = epochinset[1]
...
in {}, then return None
   """
    ...
    if bs and be:
        skydef = skydef_in[1:-1]
        if len(skydef.strip()) == 0:
            return
    else:
        skydef = skydef_in
    tokens = re_split('[,\\s]+', skydef.strip())
    if len(tokens) > 4:
    ...
    for t in tokens:
        t.strip()
        errmes = ''
        s, found = skyrefsystems.minmatch2skyref(t)
        if s != None:
            if found > 1:
...
def isparsed(skytuple):
   ...
   """
    if type(skytuple) == tuple and len(skytuple) == 4 and type(skytuple[0]) == int and type(skytuple[1]) != bytes and type(skytuple[2]) != bytes and type(skytuple[3]) != bytes:
        return True
    return False
...
:Examples:
   ...
   """
    ...
    if sysout == eq:
        ...
        if refout == fk4_no_e:
            refout = fk4
    if sysin == ecl and refin == fk4_no_e:
        refin = fk4
    if sysout == ecl and refout == fk4_no_e:
        refout = fk4
    epobs = None
