...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/blipapi/_utils.py
# Compiled at: 2010-12-26 11:28:50
import mimetypes, os.path, random, urllib

def arr2qstr(arr, all=False):
    """ Create urlencoded query string """
    return ('&').join(('%s=%s' % (urllib.quote_plus(str(k)), urllib.quote_plus(str(v), ',')) for (k, v) in arr.items() if all or v))
 ...
 """
    ...
    for (k, v) in fields.items():
        ...
        if type(v) in (str, unicode):
            ...
            output.append('Content-Disposition: form-data; name="' + k + '"; filename="' + os.path.basename(v[0]) + '"')
