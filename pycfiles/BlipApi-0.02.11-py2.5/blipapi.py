...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/blipapi/blipapi.py
# Compiled at: 2010-12-26 11:28:50
import copy, httplib, socket, time
try:
...
class BlipApi(object):
    ...
    def __init__(self, oauth_consumer=None, oauth_token=None, dont_connect=False):
        ...
        self._headers = {'Accept': self._format, 
           'X-Blip-API': '0.02'}
    ...
    def __execute(self, method, args, kwargs):
        ...
        if self._oauth_token and self._oauth_consumer:
            oauth_request = oauth.OAuthRequest.from_consumer_and_token(self._oauth_consumer, token=self._oauth_token, http_method=req_data['method'].upper(), http_url=url, parameters=params)
            oauth_request.sign_request(oauth.OAuthSignatureMethod_HMAC_SHA1(), self._oauth_consumer, token=self._oauth_token)
        ...
        return dict(headers=dict(((k.lower(), v) for (k, v) in response.getheaders())), body=body, body_parsed=body_parsed, status_code=response.status, status_body=response.reason)
