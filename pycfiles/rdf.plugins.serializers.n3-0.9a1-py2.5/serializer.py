# uncompyle6 version 3.7.4
# Python bytecode 2.5 (62131)
# Decompiled from: Python 3.6.9 (default, Apr 18 2020, 01:56:04) 
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/n3/serializer.py
# Compiled at: 2008-04-06 16:07:26
"""
"""
from rdf.graph import Graph
from rdf.plugins.serializers.n3.turtle import TurtleSerializer, SUBJECT, VERB, OBJECT

class N3(TurtleSerializer):
    short_name = 'n3'

    def __init__(self, store, parent=None):
        super(N3, self).__init__(store)
        self.parent = parent

    def reset(self):
        super(N3, self).reset()
        self._stores = {}

    def getQName(self, uri):
        qname = None
        if self.parent is not None:
            qname = self.parent.getQName(uri)
        if qname is None:
            qname = super(N3, self).getQName(uri)
        return qname

    def indent(self, modifier=0):
        indent = super(N3, self).indent(modifier)
        if self.parent is not None:
            indent += self.parent.indent(modifier)
        return indent

    def p_clause(self, node, ignore=SUBJECT):
        if isinstance(node, Graph):
            self.subjectDone(node)
            self.write(' {')
            self.depth += 1
            serializer = N3(node, parent=self)
            serializer.serialize(self.stream)
            self.depth -= 1
            self.write('\n' + self.indent() + ' }')
            return True
        else:
            return False

    def s_clause(self, subject):
        if isinstance(subject, Graph):
            self.write('\n' + self.indent())
            self.p_clause(subject, SUBJECT)
            self.predicateList(subject)
            self.write('. ')
            return True
        else:
            return False

    def preprocessTriple(self, triple):
        super(N3, self).preprocessTriple(triple)
        if isinstance(triple[0], Graph):
            for t in triple[0]:
                self.preprocessTriple(t)

        if isinstance(triple[2], Graph):
            for t in triple[2]:
                self.preprocessTriple(t)

    def statement(self, subject):
        self.subjectDone(subject)
        properties = self.buildPredicateHash(subject)
        if len(properties) == 0:
            return
        if not self.s_clause(subject):
            super(N3, self).statement(subject)

    def path(self, node, position):
        if not self.p_clause(node, position):
            super(N3, self).path(node, position)

    def startDocument(self):
        ns_list = list(self.namespaces.items())
        ns_list.sort()
        for (prefix, uri) in ns_list:
            self.write('\n' + self.indent() + '@prefix %s: <%s>.' % (prefix, uri))

        if len(ns_list) > 0:
            self.write('\n')