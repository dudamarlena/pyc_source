...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/WellDone/pymomo/pymomo/config/site_scons/site_tools/xc8_compiler.py
# Compiled at: 2015-03-19 14:45:48
import SCons.Builder, os.path, sys
...
def make_chip(env):
    return [
     '--chip=%s' % env['ARCH'].property('xc8_target')]
...
def make_int_files(cfile, folder):
    ...
    if ext == '.c':
        ints = [
         name + '.d', name + '.p1', name + '.pre']
    elif ext == '.as':
        ints = [
         name + '.d', name + '.lst', name + '.pre', name + '.obj', name + '.rlf']
...
_xc8_hex = SCons.Builder.Builder(generator=xc8_generator, emitter=xc8_emit_int_files)
