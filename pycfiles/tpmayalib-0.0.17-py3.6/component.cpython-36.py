...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpMayaLib/core/component.py
...
from tpMayaLib.core import exceptions, node, shape as shape_utils
component_filter = [
 28, 30, 31, 32, 34, 35, 36, 37, 38, 46, 47]
mesh_filter = [
 31, 32, 34, 35]
    ...
    if shapes:
        for shape in shapes:
            ...
            if maya.cmds.nodeType(shape) == 'nurbsSurface' or maya.cmds.nodeType(shape) == 'nurbsCurve':
                ...
                if maya.cmdsnodeType(shape) == 'mesh':
