...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpMayaLib/core/node.py
# Compiled at: 2020-01-16 21:52:40
# Size of source mod 2**32: 30842 bytes
"""
        ...
        if is_a_transform(node_name=node_name):
            ...
            if no_user_attributes:
                ...
                if attrs:
                    return False
            default_nodes = [
             'defaultLightSet', 'defaultObjectSet', 'initialShadingGroup', 'uiConfigurationScriptNode',
...
def check_node(node):
    ...
    if isinstance(node, str):
        ...
        if isinstance(node, maya.OpenMaya.MObject):
            return node.isNull()
    return False
    ...
    for part in path_part:
        if not part:
            ...
            if not maya.cmds.objExists(part):
                ...
                continue
            elif not maya.cmds.getAttr(part + '.visibility'):
                is_visible_ = False
            else:
                if check_lod_vis:
        ...
        if isinstance(node, str) or isinstance(node, unicode):
            if maya.cmds.nodeType(node) == 'transform':
                ...
                for shape in shapes:
                    ...
                    if intermediate:
                        if is_intermediate:
                            if maya.cmds.listConnections(shape, source=False):
                                return shape
                ...
                if maya.cmds.nodeType(node) in ('mesh', 'nurbsCurve', 'nurbsSurface'):
...
def get_node_attr_source(node, attr):
    ...
    if not attr_connection:
        return
    if isinstance(attr_connection, list):
        return attr_connection
    if isinstance(attr_connection, unicode):
...
def get_plug_value(plug):
    ...
    if api_type in [maya.OpenMaya.MFn.kAttribute3Double, maya.OpenMaya.MFn.kAttribute3Float,
     maya.OpenMaya.MFn.kCompoundAttribute]:
        ...
        if api_type in [maya.OpenMaya.MFn.kDoubleLinearAttribute, maya.OpenMaya.MFn.kFloatLinearAttribute]:
            return plug.asMDistance().asCentimeters()
    if api_type in [maya.OpenMaya.MFn.kDoubleAngleAttribute, maya.OpenMaya.MFn.kFloatAngleAttribute]:
        return plug.asMAngle().asDegrees()
    if api_type == maya.OpenMaya.MFn.kTypedAttribute:
        ...
        if plug_type == maya.OpenMaya.MFnData.kString:
            return plug.asString()
    else:
        if api_type == maya.OpenMaya.MFn.kMatrixAttribute:
            return maya.OpenMaya.MFnMatrixData(plug.asMObject()).matrix()
        if api_type == maya.OpenMaya.MFn.kNumericAttribute:
            plug_type = maya.OpenMaya.MFnNumericAttribute(plug_attr).numericType()
            if plug_type == maya.OpenMaya.MFnNumericData.kBoolean:
                return plug.asBool()
            if plug_type in [maya.OpenMaya.MFnNumericData.kShort, maya.OpenMaya.MFnNumericData.kInt, maya.OpenMaya.MFnNumericData.kLong,
             maya.OpenMaya.MFnNumericData.kByte]:
                return plug.asInt()
            if plug_type in [maya.OpenMaya.MFnNumericData.kFloat, maya.OpenMaya.MFnNumericData.kDouble,
...
def normalize_attribute_short_name(short_name, unique_on_obj=None):
    ...
    if len(short_name):
        short_name = short_name[0] + re.sub('[a-z]', '', short_name[1:])
...
def display_override(obj, override_enabled=False, override_display=0, override_lod=0, override_visibility=True, override_shading=True):
    """
...
def get_input_attributes(node):
    ...
    if not maya.cmds.objExists(node):
        ...
        inputs = maya.cmds.listConnections(node, connections=True, destination=False, source=True, plugs=True, skipConversionNodes=True)
        if inputs:
            inputs.reverse()
        else:
            inputs = list()
...
def get_output_attributes(node):
    ...
    if not maya.cmds.objExists(node):
        raise Exception('Object "{}" does not exists!'.format(node))
    outputs = maya.cmds.listConnections(node, connections=True, destination=True, source=False, plugs=True, skipConversionNodes=True)
    if not outputs:
        outputs = list()
    return outputs
