...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpMayaLib/core/geometrycache.py
# Compiled at: 2020-01-16 21:52:40
# Size of source mod 2**32: 13222 bytes
"""
Module that contains Geometry Cache related functionality for Maya
"""
from __future__ import print_function, division, absolute_import
import os, tpMayaLib as maya
...
def disconnect_time(cache_node):
    ...
    """
    time_connection = maya.cmds.listConnections(('{}.time'.format(cache_node)),
...
def import_cache_to_geometries(geo_list, cache_directory, cache_file_list=None, extension='.abc'):
    ...
    if not os.path.isdir(cache_directory):
        ...
        if not cache_directory.endswith('/'):
            ...
            if cache_file_list:
                if not len(cache_file_list) == len(geo_list):
                    raise Exception('Cache files and gemetries list have not the same amount of elements!')
        extension = extension.endswith('.') or '.{}'.format(extension)
...
def export_cache_from_geometry(geometry, cache_file, start_frame=1, end_frame=100, use_time_line=True, file_per_frame=False, cache_per_geo=True, force_export=False):
    ...
    maya.mel.eval('doCreateGeometryCache ' + str(version) + ' {"0","' + str(start_frame) + '","' + str(end_frame) + '","' + cache_dist + '","' + str(refresh) + '","' + cache_dir + '","' + str(int(cache_per_geo)) + '","' + base_name + '","' + str(use_prefix) + '","' + cache_action + '","' + str(int(force_export)) + '","1","1","0","0","mcc" };')
...
def load_abc_from_gpu_cache(gpu_cache_node, debug=False):
    ...
    if not cache_parent:
        cache_parent = ''
    cache_namespace = namespace_utils.get_namespace(name=gpu_cache_node)
    cache_name = gpu_cache_node
    maya.cmds.delete(cache_xform)
    cache_node = import_abc_cache(cache_path,
      cache_name=cache_name, namespace=cache_namespace, parent=cache_parent, mode='import', debug=debug)
    if cache_parent:
