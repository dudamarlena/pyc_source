...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpMayaLib/core/transform.py
# Compiled at: 2020-01-16 21:52:40
# Size of source mod 2**32: 56153 bytes
"""
    ...
    if type(point) == list or type(point) == tuple:
        ...
        if type(point) == str or type(point) == unicode:
            ...
            if mobj.hasFn(maya.OpenMaya.MFn.kTransform):
                ...
                pos or LOGGER.exception('Invalid point value supplied! Unable to determine type of point "{0}"!'.format(str(point)))
                return
        else:
...
def find_transform_right_side(transform, check_if_exists=True):
    ...
    for i, side in enumerate(TRANSFORM_SIDES['start']['short']):
        if transform.startswith(side[0]):
            if not transform.endswith(TRANSFORM_SIDES['end']['short'][i][1]):
...
def find_transform_left_side(transform, check_if_exists=True):
    ...
    for i, side in enumerate(TRANSFORM_SIDES['start']['short']):
        if transform.startswith(side[1]):
            if not transform.endswith(TRANSFORM_SIDES['start']['short'][i][0]):
