...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpMayaLib/meta/metanode.py
# Compiled at: 2020-01-16 21:52:40
# Size of source mod 2**32: 88460 bytes
"""
...
class MetaNode(object):
    ...
    def __bind_data__(self, *args, **kwargs):
        ...
        - To bind a new attr to the Python object only, not serialized --> self.attr = None or self.__setattr__('attr', None)
        """
        pass
    ...
    def add_attribute(self, attr, value=None, attr_type=None, hidden=False, **kwargs):
        ...
        add_cmd_edit_flags = ['min', 'minValue', 'max', 'maxValue', 'defaultValue', 'dv', 'softMinValue', 'smn', 'softMaxValue', 'smx', 'enumName']
    ...
    def connect_children(self, nodes, attr, source_attr=None, clean_current=False, force=True, allow_incest=True, source_simple=False, **kwargs):
        ...
        :param attr: str, name of the message attribute
        :param source_attr: if given, this becomes the attribute on the child node which connect it to the MetaNode. If not given, this attribute is set to self.meta_node_id
        :param clean_current: bool, Disconnect and clean any currently connected nodes to this attribute
        :param force: bool, Force the connection of the nodes
        :param allow_incest: bool, Overrides the behaviour when dealing with child nodes that are standard native nodes not MetaNodes.
        :param source_simple: bool, By default when child are wired we expect arrays, so plugs in source and destination are index managed.
        If True, index is not used and a a single simple wire on the source attribute is used (the child)
        :param kwargs:
        """
        self.add_attribute(attr, attr_type='message')
    ...
    def get_message(self, attr, full_path=True, as_meta=False, data_attr=None, data_key=None, simple=True, *args, **kwargs):
        ...
        to an attribute. To do this, we devised a method of creating a compatible attr on the object to receive the message,
...
def validate_obj_list_arg(list_arg=None, meta_class=None, none_valid=False, default_meta_type=None, maya_type=None, update_class=False):
    if type(list_arg) not in [list, tuple]:
        list_arg = [
         list_arg]
    return_list = list()
    kwargs = {'meta_class':meta_class,  'none_valid':none_valid,  'default_meta_type':default_meta_type,  'maya_type':maya_type,  'update_class':update_class}
    for arg in list_arg:
