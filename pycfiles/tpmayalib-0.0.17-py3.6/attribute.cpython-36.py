...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpMayaLib/core/attribute.py
# Compiled at: 2020-01-16 21:52:40
# Size of source mod 2**32: 143605 bytes
"""
Module that contains functions and classes related with attributes
"""
from __future__ import print_function, division, absolute_import
import os, re, sys, copy, random, string, logging, traceback, tpDccLib as tp
...
class AttributeValidator(object):
    ...
    def file_path(self, file_path=None, file_mode=0, file_filter='Text files (*.txt)', start_dir=None):
        ...
        if file_path is None:
            ...
            file_path = maya.cmds.fileDialog2(dialogStyle=2, fileMode=file_mode, startingDirectory=start_dir, fileFilter=file_filter)
            if file_path:
...
def get_color_of_side(side='C', sub_color=False):
    ...
    if tp.Dcc.name_is_center(side):
        ...
        if tp.Dcc.name_is_left(side):
            side = 'L'
        else:
            if tp.Dcc.name_is_right(side):
                side = 'R'
            else:
                side = 'C'
        if not sub_color:
