...
# Compiled at: 2016-02-18 20:03:52
# Size of source mod 2**32: 3805 bytes
import asyncio, time, io, datetime, os
...
def atoms(resp, req, environ, request_time):
    ...
    if hasattr(req, 'headers'):
        ...
        req_headers = req
    atoms.update(dict([('{%s}i' % k.lower(), v) for k, v in req_headers.items()]))
    atoms.update(dict([('{%s}o' % k.lower(), v) for k, v in resp.headers.items()]))
    return atoms
...
class AiopyramidHttpServerProtocol(WSGIServerHttpProtocol):
    ...
    def log_access(self, request, environ, response, time):
        catoms = atoms(response, request, environ, datetime.timedelta(0, 0, time))
        safe_atoms = self.logger.atoms_wrapper_class(catoms)
        self.logger.access_log.info(self.logger.cfg.access_log_format % safe_atoms)
