...
# [GCC 8.4.0]
# Embedded file name: /home/kyle/projects/top30/pyenv/lib/python3.7/site-packages/top30/handlers.py
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def init_ui(self):
        ...
        self.setWindowIcon(QtGui.QIcon(pkg_resources.resource_filename(__name__, 'icon.ico')))
        self.show()
    ...
    def on_add_clip_clicked(self):
        ...
        filenames, mime = QtWidgets.QFileDialog.getOpenFileNames(self, 'Add clip', top30.SETTINGS.last_open_directory(), 'Audio(*.mp3 *.ogg);;All Files(*)')
    ...
    def on_move_up_clicked(self):
        ...
        if not item == None:
            if item.row() > 0:
                self.clip_model.moveRows(QtCore.QModelIndex(), item.row(), item.row(), QtCore.QModelIndex(), item.row() - 1)

    def on_move_down_clicked(self):
        """ Event listener for the move down button """
        item = self.get_selected_clip()
        if not item == None:
            if item.row() < self.clip_model.rowCount() - 1:
                self.clip_model.moveRows(QtCore.QModelIndex(), item.row() + 1, item.row() + 1, QtCore.QModelIndex(), item.row())
    ...
    def on_create_clip_clicked(self):
        ...
        if self.clip_model.rowCount() == 0:
            QtWidgets.QMessageBox.warning(self, 'No Clips', 'Please add clips to use')
            return
        else:
            self.save_settings()
            rundown_name, file_type = self.save_clip()
            return rundown_name or None
        ...
        QtWidgets.QMessageBox.information(self, 'Complete', 'Clip ' + rundown_name + ' created.')

    def on_about_clicked(self):
        """ Displays the about dialog """
        about_text = 'top30\nVersion ' + top30.VERSION + '\n\ntop30 automatically creates rundowns of a top 30 chart.\n\n' + 'This project comes with NO WARRENTY, to the extent permitted by the ' + 'law. You may redistribute it under the terms of the GNU General ' + 'Public License version 3\n\nWritten by Kyle Robbertze'
    ...
    def save_clip(self):
        ...
        filename = QtWidgets.QFileDialog.getSaveFileName(self, 'Add clip', top30.SETTINGS.last_save_directory(), 'Audio (*.mp3)')
