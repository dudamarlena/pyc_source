...
# [GCC 8.4.0]
# Embedded file name: /home/ax/Workspace/norm/norm/models/revision.py
...
class Revision(Model, ParametrizedMixin):
    __doc__ = 'Revision of the Lambda. All revisions for the same version are executed in memory.'
...
revision_variable = Table('revision_variable', metadata, Column('id', Integer, primary_key=True), Column('revision_id', Integer, ForeignKey('revisions.id')), Column('variable_id', Integer, ForeignKey('variables.id')))

class SchemaRevision(Revision):
    __mapper_args__ = {'polymorphic_identity': 'revision_schema'}
...
class AddVariableRevision(SchemaRevision):
    __mapper_args__ = {'polymorphic_identity': 'revision_schema_add'}
...
class RenameVariableRevision(SchemaRevision):
    __mapper_args__ = {'polymorphic_identity': 'revision_schema_rename'}
...
class RetypeVariableRevision(SchemaRevision):
    __mapper_args__ = {'polymorphic_identity': 'revision_schema_retype'}
...
class DeleteVariableRevision(SchemaRevision):
    __mapper_args__ = {'polymorphic_identity': 'revision_schema_delete'}
...
class DeltaRevision(Revision):
    __mapper_args__ = {'polymorphic_identity': 'revision_delta'}
...
class ConjunctionRevision(DeltaRevision):
    ...
    __mapper_args__ = {'polymorphic_identity': 'revision_delta_conjunction'}
...
class DisjunctionRevision(DeltaRevision):
    ...
    __mapper_args__ = {'polymorphic_identity': 'revision_delta_disjunction'}
...
class FitRevision(DeltaRevision):
    __mapper_args__ = {'polymorphic_identity': 'revision_delta_fit'}
