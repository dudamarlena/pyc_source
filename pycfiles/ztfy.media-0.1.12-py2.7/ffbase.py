...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ztfy/media/ffbase.py
# Compiled at: 2016-08-31 06:13:35
import json, logging
...
__all__ = [
 'FFmpeg', 'FFVideoEffect', 'FFAudioEffect']
...
class FFmpeg(object):
    ...
    def __exec__(self, *args, **kwargs):
        ...
        if use_stdin is not None:
            ...
            if 'partial file' in stderr:
                ...
                return self.__exec__(allow_stdin=False, *args)
        else:
    ...
    def render(self, effectchain, output):
        ...
        for index, effect in enumerate(effectchain):
            if index == 1 and effect not in inputs:
...
class FFEffect():
    ...
    def __init__(self, inputfile, **args):
        ...
        for opt in args.keys():
            if opt not in ('b', 'vframes', 'r', 's', 'aspect', 'croptop', 'cropbottom',
                           'cropleft', 'cropright', 'padtop', 'padbottom', 'padleft',
                           'padright', 'padcolor', 'vn', 'bt', 'maxrate', 'minrate',
                           'bufsize', 'vcodec', 'sameq', 'pass', 'newvideo', 'pix_fmt',
                           'sws_flag', 'g', 'intra', 'vdt', 'qscale', 'qmin', 'qmax',
                           'qdiff', 'qblur', 'qcomp', 'lmin', 'lmax', 'mblmin', 'mblmax',
                           'rc_init_cplx', 'b_qfactor', 'i_qfactor', 'b_qoffset',
                           'i_qoffset', 'rc_eq', 'rc_override', 'me_method', 'dct_algo',
                           'idct_algo', 'er', 'ec', 'bf', 'mbd', '4mv', 'part', 'bug',
                           'strict', 'aic', 'umv', 'deinterlace', 'ilme', 'psnr',
                           'vhook', 'top', 'dc', 'vtag', 'vbsf', 'aframes', 'ar',
                           'ab', 'ac', 'an', 'acodec', 'newaudio', 'alang', 't',
...
class FFVideoEffect(FFEffect):
    ...
    def size(self, s=None):
        """ set frame size """
        if s in ('sqcif', 'qcif', 'cif', '4cif', 'qqvga', 'qvga', 'vga', 'svga', 'xga',
                 'uxga', 'qxga', 'sxga', 'qsxga', 'hsxga', 'wvga', 'wxga', 'wsxga',
                 'wuxga', 'wqxga', 'wqsxga', 'wquxga', 'whsxga', 'whuxga', 'cga',
                 'ega', 'hd480', 'hd720', 'hd1080'):
    ...
    def crop(self, top=0, bottom=0, left=0, right=0):
        ...
        return (
         self.__effects__.get('croptop'), self.__effects__.get('cropbottom'), self.__effects__.get('cropleft'), self.__effects__.get('cropright'))
    ...
    def pad(self, top=0, bottom=0, left=0, right=0, color='000000'):
        ...
        return (
         self.__effects__.get('padtop'), self.__effects__.get('padbottom'), self.__effects__.get('padleft'), self.__effects__.get('padright'), self.__effects__.get('padcolor'))
    ...
    def quantizerrange(self, qmin=None, qmax=None, qdiff=None):
        ...
        return (
         self.__effects__.get('qmin'), self.__effects__.get('qmax'), self.__effects__.get('qdiff'))
    ...
    def lagrangefactor(self, lmin=None, lmax=None):
        ...
        return (
         self.__effects__.get('lmin'), self.__effects__.get('lmax'))
    ...
    def macroblock(self, mblmin=None, mblmax=None):
        ...
        return (
         self.__effects__.get('mblmin'), self.__effects__.get('mblmax'))
