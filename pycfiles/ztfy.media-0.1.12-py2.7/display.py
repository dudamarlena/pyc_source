...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ztfy/media/browser/display.py
# Compiled at: 2013-06-19 06:51:45
...
class VideoMediaDisplayAdapter(ImageDisplayAdapter):
    ...
    def image(self):
        ...
        if image is None:
            pipe = sp.Popen(('ffmpeg', '-i', '-', '-ss', '5', '-f', 'image2', '-vframes',
                             '1', '-'), stdin=sp.PIPE, stdout=sp.PIPE, stderr=sp.PIPE)
            if pipe:
                ...
                if not stdout:
                    ...
                    output.file.flush()
                    pipe = sp.Popen(('ffmpeg', '-i', output.name, '-ss', '5', '-f', 'image2', '-vframes', '1', '-'), stdin=sp.PIPE, stdout=sp.PIPE, stderr=sp.PIPE)
                    if pipe:
