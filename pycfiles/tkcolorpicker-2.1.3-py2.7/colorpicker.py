...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tkcolorpicker/colorpicker.py
# Compiled at: 2018-09-25 03:44:26
"""
...
from tkcolorpicker.functions import tk, ttk, round2, create_checkered_image, overlay, PALETTE, hsv_to_rgb, hexa_to_rgb, rgb_to_hexa, col2hue, rgb_to_hsv
...
import re
EN = {}
FR = {'Red': 'Rouge', 'Green': 'Vert', 'Blue': 'Bleu', 'Hue': 'Teinte', 
   'Saturation': 'Saturation', 'Value': 'Valeur', 'Cancel': 'Annuler', 
   'Color Chooser': 'Sélecteur de couleur', 'Alpha': 'Alpha'}
try:
...
class ColorPicker(tk.Toplevel):
    ...
    def __init__(self, parent=None, color=(255, 0, 0), alpha=False, title=_('Color Chooser')):
        """
        ...
        style.map('palette.TFrame', relief=[('focus', 'sunken')], bordercolor=[
         ('focus', '#4D4D4D')])
        self.configure(background=style.lookup('TFrame', 'background'))
        if isinstance(color, str):
            if re.match('^#[0-9A-F]{8}$', color.upper()):
                ...
                if alpha:
                    ...
                    old_color = color[:7]
            elif re.match('^#[0-9A-F]{6}$', color.upper()):
        ...
        self.square = ColorSquare(square, hue=hue, width=200, height=200, color=rgb_to_hsv(*self._old_color), highlightthickness=0)
        self.square.pack()
        ...
        if alpha:
            ...
            transparent_bg_old = create_checkered_image(42, 32, (100, 100, 100, 255), (154,
                                                                                       154,
                                                                                       154,
                                                                                       255))
            ...
            old_color_prev = tk.Label(preview_frame, padx=0, pady=0, image=self._im_old_color, borderwidth=0, highlightthickness=0)
            self.color_preview = tk.Label(preview_frame, pady=0, padx=0, image=self._im_color, borderwidth=0, highlightthickness=0)
        else:
            old_color_prev = tk.Label(preview_frame, background=old_color[:7], width=5, highlightthickness=0, height=2, padx=0, pady=0)
            self.color_preview = tk.Label(preview_frame, width=5, height=2, pady=0, background=old_color[:7], padx=0, highlightthickness=0)
        ...
        s_h = Spinbox(hsv_frame, from_=0, to=360, width=4, name='spinbox', textvariable=self.hue, command=self._update_color_hsv)
        s_s = Spinbox(hsv_frame, from_=0, to=100, width=4, textvariable=self.saturation, name='spinbox', command=self._update_color_hsv)
        s_v = Spinbox(hsv_frame, from_=0, to=100, width=4, name='spinbox', textvariable=self.value, command=self._update_color_hsv)
        ...
        ttk.Label(hsv_frame, text=_('Hue')).grid(row=0, column=0, sticky='e', padx=4, pady=4)
        ttk.Label(hsv_frame, text=_('Saturation')).grid(row=1, column=0, sticky='e', padx=4, pady=4)
        ttk.Label(hsv_frame, text=_('Value')).grid(row=2, column=0, sticky='e', padx=4, pady=4)
        ...
        s_red = Spinbox(rgb_frame, from_=0, to=255, width=4, name='spinbox', textvariable=self.red, command=self._update_color_rgb)
        s_green = Spinbox(rgb_frame, from_=0, to=255, width=4, name='spinbox', textvariable=self.green, command=self._update_color_rgb)
        s_blue = Spinbox(rgb_frame, from_=0, to=255, width=4, name='spinbox', textvariable=self.blue, command=self._update_color_rgb)
        s_red.delete(0, 'end')
        ...
        ttk.Label(rgb_frame, text=_('Red')).grid(row=0, column=0, sticky='e', padx=4, pady=4)
        ttk.Label(rgb_frame, text=_('Green')).grid(row=1, column=0, sticky='e', padx=4, pady=4)
        ttk.Label(rgb_frame, text=_('Blue')).grid(row=2, column=0, sticky='e', padx=4, pady=4)
        ...
        if alpha:
            ...
            self.alphabar = AlphaBar(alphabar, alpha=self._old_alpha, width=200, color=self._old_color, highlightthickness=0)
            self.alphabar.pack()
            s_alpha = Spinbox(alpha_frame, from_=0, to=255, width=4, textvariable=self.alpha, command=self._update_alpha)
            s_alpha.delete(0, 'end')
            s_alpha.insert(0, self._old_alpha)
            alphabar.grid(row=0, column=0, padx=(0, 4), pady=4, sticky='w')
            ttk.Label(alpha_frame, text=_('Alpha')).grid(row=0, column=1, sticky='e', padx=4, pady=4)
            s_alpha.grid(row=0, column=2, sticky='w', padx=(4, 6), pady=4)
        button_frame = ttk.Frame(self)
        ttk.Button(button_frame, text='Ok', command=self.ok).pack(side='right', padx=10)
        ttk.Button(button_frame, text=_('Cancel'), command=self.destroy).pack(side='right', padx=10)
        bar.grid(row=0, column=0, padx=10, pady=(10, 4), sticky='n')
        square.grid(row=1, column=0, padx=10, pady=(9, 0), sticky='n')
        if alpha:
            alpha_frame.grid(row=2, column=0, columnspan=2, padx=10, pady=(1, 4), sticky='ewn')
    ...
    def _update_color_hexa(self, event=None):
        ...
        elif self.alpha_channel and re.match('^#[0-9A-F]{8}$', color):
