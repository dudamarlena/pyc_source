...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tkcolorpicker/spinbox.py
...
class Spinbox(tk.Spinbox):
    ...
    def __init__(self, parent, **kwargs):
        ...
        self.frame = ttk.Frame(parent, class_='ttkSpinbox', relief=kwargs.get('relief', 'sunken'), borderwidth=1)
        self.style.configure('%s.spinbox.TFrame' % self.frame, background=self.style.lookup('TSpinbox', 'fieldbackground', default='white'))
        self.frame.configure(style='%s.spinbox.TFrame' % self.frame)
        kwargs['relief'] = 'flat'
        kwargs['highlightthickness'] = 0
        kwargs['selectbackground'] = self.style.lookup('TSpinbox', 'selectbackground', ('focus', ))
        kwargs['selectforeground'] = self.style.lookup('TSpinbox', 'selectforeground', ('focus', ))
        kwargs['background'] = self.style.lookup('TSpinbox', 'fieldbackground', default='white')
        kwargs['foreground'] = self.style.lookup('TSpinbox', 'foreground')
        kwargs['buttonbackground'] = self.style.lookup('TSpinbox', 'background')
    ...
    def focusout(self, event):
        ...
        self.style.configure('%s.spinbox.TFrame' % self.frame, bordercolor=bc, darkcolor=dc, lightcolor=lc)
