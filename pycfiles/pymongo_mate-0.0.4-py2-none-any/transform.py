...
# [GCC 8.4.0]
# Embedded file name: pymongo_mate/pkg/pandas_mate/transform.py
...
def itertuple(df):
    u"""High performance tuple style iterator.
...
def grouper_df(df, chunksize):
    u"""Evenly divide pd.DataFrame into n rows piece, no filled value 
...
def to_index_row_dict(df, index_col=None, use_ordered_dict=True):
    u"""Transform data frame to list of dict.
...
def to_dict_list(df, use_ordered_dict=True):
    u"""Transform each row to dict, and put them into a list.
...
def to_dict_list_generic_type(df, int_col=None, binary_col=None):
    u"""Transform each row to dict, and put them into a list. And automatically
