...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/formskit/tests/test_treeform.py
# Compiled at: 2015-07-25 09:04:48
# Size of source mod 2**32: 17080 bytes
...
class TreeFormTest(TestCase):
    ...
    def test_error_decoding_name(self):
        ...
        raw_data = {name.replace(b'x', b'y'): ['value']}
...
class TreeFormsTest(TestCase):
    ...
    def _get_raw_data(self, value='value3'):
        raw_data = {self.form.form_name_value: [self.form.get_name()], 
         self.form.fields['one'].get_name(): ['value1']}
        name = self.form.get_or_create_sub_form('TreeForm', 2).fields['two'].get_name()
        raw_data[name] = [
         'value2.1', 'value2.2']
        name = self.form.get_or_create_sub_form('TreeForm', 3).fields['two'].get_name()
        raw_data[name] = [
         value]
        return raw_data
...
class GetDataDictTreeTest(TestCase):
    ...
    def setUp(self):
        ...
        data = {self.form.form_name_value: [self.form.get_name()], 
         self.form.fields['name1'].get_name(): ['one'], 
         self.form.get_or_create_sub_form('TreeForm', 0).fields['name2'].get_name(): [
                                                                                      'two',
                                                                                      'three'], 
         
         self.form.get_or_create_sub_form('TreeForm', 1).fields['name2'].get_name(): [
                                                                                      'four'], 
         
         self.form.get_or_create_sub_form('TreeForm', 2).fields['name2'].get_name(): []}
        self.form.validate(data)

    def test_tree(self):
        assert self.form.get_data_dict() == {'name1': ['one'], 
         'TreeForm': {0: {'name2': ['two', 'three']}, 
                      1: {'name2': ['four']}, 
                      2: {'name2': []}}}

    def test_tree_minified(self):
        assert self.form.get_data_dict(True) == {'name1': 'one', 
         'TreeForm': {0: {'name2': ['two', 'three']}, 
                      1: {'name2': 'four'}, 
                      2: {}}}
...
class GetReportTreeTest(TestCase):
    ...
    def test_success(self):
        self.form.parse_dict({'name1': ['one'], 
         'TreeForm': [
                      {'name1': ['two']}]})
        assert self.run_form() is True
        assert self.form.get_report() == {'childs': {'TreeForm': {0: {'childs': {}, 
    ...
    def test_subform_error_at_field_value_validator(self):
        ...
        assert self.run_form() is False
        assert self.form.get_report() == {'childs': {'TreeForm': {0: {'childs': {}, 
                                     'fields': {'name1': {'messages': ['NotEmpty'], 
                                                          'success': False, 
                                                          'values': []}, 
