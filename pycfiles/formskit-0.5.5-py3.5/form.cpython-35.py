...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/formskit/form.py
# Compiled at: 2015-12-02 08:14:23
# Size of source mod 2**32: 8125 bytes
...
class Form(Translable):
    ...
    def _validate(self):
        self.success = True
        self.success &= self._validate_fields() and self._validate_form_validators()
        return self.success
    ...
    def parse_dict(self, data, force=False):
        ...
        for name, values in data.items():
            self._parsed_field(name, values, force) or self._parsed_sub_form(name, values) or raise_error(name)
    ...
    def get_report(self):
        ...
        for name, field in self.fields.items():
            ...
            report['fields'][name] = {'success': not field.error, 
