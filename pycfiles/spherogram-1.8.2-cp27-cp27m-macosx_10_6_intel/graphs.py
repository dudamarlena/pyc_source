...
# [GCC 8.4.0]
# Embedded file name: build/lib.macosx-10.6-intel-2.7/spherogram/graphs.py
# Compiled at: 2018-08-17 14:58:51
from __future__ import print_function
import networkx as nx
_within_sage = False
try:
    import sage.all, sage.graphs.graph
    _within_sage = True
except ImportError:
    try:
...
class Graph(object):
    ...
    def components(self, deleted_vertices=[]):
        ...
        while vertices:
            ...
            component.add(start)
            generator = self.breadth_first_edges(source=start, forbidden=forbidden)
        ...
        if sink == source:
            ...
            if capacity is None:
                ...
                while True:
                    ...
                    generator = self.breadth_first_edges(source=source, forbidden=full_edges, for_flow=True)
                    for parent, vertex, child in generator:
                        parents[child] = (
                         parent, vertex)
                ...
                for vertex in cut_set:
                    cut_edges |= set([ edge for edge in self.edges if vertex in edge and edge(vertex) not in cut_set
                                     ])
            ...
            return {'set': cut_set, 'edges': cut_edges, 'paths': path_list, 'residuals': residual, 
               'unsaturated': unsaturated, 'flow': flow_dict}
...
class Digraph(Graph):
    ...
    def outgoing(self, vertex):
        ...
        """
        return set(e for e in self(vertex) if e.tail is vertex and e.head is not vertex)
    ...
    def incoming(self, vertex):
        ...
        """
        return set(e for e in self(vertex) if e.head is vertex and e.tail is not vertex)
    ...
    def is_connected(self):
        ...
        if not DAG.is_weakly_connected():
            ...
            if [ v for v in DAG.vertices if DAG.indegree(v) > 1 or DAG.outdegree(v) > 1
               ]:
                return False
            return True
    ...
    def closed_subsets(self):
        ...
        for X in powerset(self.elements):
            if 0 < len(X):
                ...
                for x in X:
                    if X.intersection(self.smaller[x]) or X.intersection(self.larger[x]):
