...
# [GCC 8.4.0]
# Embedded file name: build/lib.macosx-10.6-intel-2.7/spherogram/links/invariants.py
...
class Link(links_base.Link):
        ...
        for i in range(len(faces) - 1):
            for j in range(i + 1, len(faces)):
                ...
                for x in range(len(self.crossings)):
                    crossings = [
                     self.crossings[x][0], self.crossings[x][1], self.crossings[x][2], self.crossings[x][3]]
                    total = set(crossings)
                    if total.issubset(s):
    ...
    def signature(self):
        ...
        correction = sum([ e[2]['sign'] for e in G.edges(sort=False) if e[2]['sign'] == e[2]['crossing'].sign
                         ])
