...
# [GCC 8.4.0]
# Embedded file name: build/lib.macosx-10.6-intel-2.7/spherogram/presentations.py
# Compiled at: 2019-05-09 15:47:33
...
abc = Alphabet('1', 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ')
ABC = Alphabet('1', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz')
...
class CyclicWord(Word):
    ...
    def complexity(self, size, ordering=[], spin=0):
        ...
        return (
         Complexity(complexity), the_ordering)
...
class Presentation(object):
    ...
    def __eq__(self, other):
        return self.relators == other.relators and self.generators == other.generators
        ...
        for generator, cut in levels:
            ...
            for subset in P.closed_subsets():
                if 1 < len(subset) < len(P) - 1:
                    yield (
                     generator, frozenset.union(*subset))
        ...
        while queue:
            ...
            for a, A in pres.level_transformations():
                ...
                if signature not in seen:
                    WM = (
                     WhiteheadMove(a, A, pres.generators, self.alphabet),)
                    queue.append((pres, WM, P, signature))
                    seen.add(signature)

            if verbose:
                yield (
                 parent, move, pres, Presentation(*sig))
            else:
