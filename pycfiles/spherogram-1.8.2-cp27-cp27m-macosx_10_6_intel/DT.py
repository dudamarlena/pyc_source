...
# [GCC 8.4.0]
# Embedded file name: build/lib.macosx-10.6-intel-2.7/spherogram/codecs/DT.py
# Compiled at: 2018-08-17 14:49:46
from spherogram import FatGraph, FatEdge, CyclicList, Link, Crossing
from spherogram.links.links import CrossingEntryPoint
from spherogram.codecs.Base64LikeDT import decode_base64_like_DT_code, encode_base64_like_DT_code
import string, sys
...
class DTFatGraph(FatGraph):
    ...
    def push(self, flips):
        ...
        self.stack.append([
         flips,
         [ ((e[0], e.slots[0]), (e[1], e.slots[1]), e.component) for e in self.edges if e.marked
         ],
         [ ((e[0], e.slots[0]), (e[1], e.slots[1]), e.component) for e in self.edges if not e.marked
         ]])
    ...
    def flipped(self, vertex):
        ...
        """
        return bool(len([ e for e in self(vertex) if e[1] is vertex and e.slots[1] in (2,
                                                                                       3)
                        ]) % 2)
    ...
    def sign(self, vertex):
        ...
        if flipped ^ vertex[2] ^ even_first:
            return -1
        return 1
    ...
    def KLP_strand(self, vertex, edge):
        ...
        W = edge(vertex)
        slot = edge.slot(W)
        if (slot == 0 or slot == 2) ^ self.flipped(W):
            return 'X'
        return 'Y'
...
class DTcodec(object):
    ...
    def link(self):
        ...
        if not link.all_crossings_oriented():
            ...
            raise (link.is_planar() or ValueError)('DT code does not seem to define a *planar* diagram')
        return link
