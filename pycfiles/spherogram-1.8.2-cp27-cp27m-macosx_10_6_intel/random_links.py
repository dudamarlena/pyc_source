...
# [GCC 8.4.0]
# Embedded file name: build/lib.macosx-10.6-intel-2.7/spherogram/links/random_links.py
...
def random_link(crossings, num_components='any', initial_map_gives_link=False, alternating=False, consistent_twist_regions=False, simplify='basic', prime_decomposition=True, return_all_pieces=False, max_tries=100):
    """
    ...
    if num_components == 'any':
        ...
        if initial_map_gives_link:
            plane_map = random_map(crossings, edge_conn_param, num_components, max_tries)
            link = map_to_link(plane_map)
        else:
