...
# [GCC 8.4.0]
# Embedded file name: build/lib.macosx-10.6-intel-2.7/spherogram/links/simplify.py
    ...
    for face in link.faces():
        if len(face) == 3:
            if sum(ce.strand_index % 2 for ce in face) in (1, 2):
                while face[1][1] % 2 != 0 or face[2][1] % 2 != 1:
                    face = [
                     face[1], face[2], face[0]]

                if len(set([ e.crossing for e in face ])) == 3:
                    ans.append(face)
...
def reidemeister_III(link, triple):
    ...
    old_border = [
     (
      C, c - 1), (C, c - 2), (A, a - 1), (A, a - 2), (B, b - 1), (B, b - 2)]
...
def cross_over(link, cep_to_cross, loose_end, label):
    ...
    return (
     new_crossing, new_crossing.crossing_strands()[3])
...
def backtrack(link, num_steps=10, prob_type_1=0.3, prob_type_2=0.3):
    """
