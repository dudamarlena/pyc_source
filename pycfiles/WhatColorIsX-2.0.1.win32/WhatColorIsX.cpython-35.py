...
# [GCC 8.4.0]
# Embedded file name: \Users\Thomas\Documents\Code\Projects\WhatColorIsX\WhatColorIsX\Lib\site-packages\WhatColorIsX.py
# Compiled at: 2015-11-24 09:59:30
# Size of source mod 2**32: 7287 bytes
import argparse, json, codecs, urllib.parse, urllib.request, io, math, operator
...
class whatcolorisx(object):
    ...
    def __init__(self, input, images_to_try=10):
        ...
        if isinstance(input, Image.Image):
            ...
            if input.lower().endswith(('.jpg', '.png')):
    ...
    def _search_image(self, images_to_try=10):
        ...
        for attempt in range(search_attempts):
            ...
            searchUrl = 'http://ajax.googleapis.com/ajax/services/search/images?v=1.0&start={0}&num={1}&q={2}'.format(str(start_index + images_already_searched), str(results_per_search + images_already_searched), search_string_safe)
...
def main():
    ...
    parser.add_argument('x', help='string/file to find colour of')
    parser.add_argument('-b', '--bright_hue', action='store_true', help='return a bright colour; hsl=(x,1.0,0.5)')
    parser.add_argument('-m', '--method', choices=['average_color', 'common_color'], help='Helper method to use for colour picking. Defaults to average')
    parser.add_argument('--images_to_try', type=int, default=10, help='number of images to try processing before erroring')
