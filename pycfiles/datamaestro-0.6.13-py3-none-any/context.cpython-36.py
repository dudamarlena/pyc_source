...
# [GCC 8.4.0]
# Embedded file name: /root/project/datamaestro/context.py
# Compiled at: 2020-02-25 07:34:23
# Size of source mod 2**32: 12420 bytes
from pathlib import Path
import sys, importlib, os, hashlib, logging, inspect, urllib, shutil
from itertools import chain
import json, pkg_resources
from tqdm import tqdm
from typing import Iterable, List
import re, marshmallow as mm
...
class DownloadReportHook(tqdm):
    __doc__ = 'Report hook for tqdm when downloading from the Web'
...
class PathField(mm.fields.Field):
    __doc__ = 'Field that serializes to a title case string and deserializes\n    to a lower case string.\n    '
...
class ResolvablePath:
    __doc__ = 'An object than can be resolved into a Path'
...
class Datasets:
    ...
    def __iter__(self) -> Iterable['definitions.DatasetDefinition']:
        for key, value in self.module.__dict__.items():
            if hasattr(value, '__datamaestro__') and value.__datamaestro__.aliases and self.module.__name__ == value.__datamaestro__.t.__module__:
                yield value


class Repository:
    __doc__ = 'A repository regroup a set of datasets and their corresponding specific handlers (downloading, filtering, etc.)'
