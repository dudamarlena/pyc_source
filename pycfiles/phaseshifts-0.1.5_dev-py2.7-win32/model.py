...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\phaseshifts\model.py
# Compiled at: 2014-09-04 16:50:29
"""
**model.py**
 ...
 0.0, 0.0, 0.0], **kwargs):
    ...
    def set_mufftin_radius(self, radius):
        ...
        try:
            ...
            self._radius = self.radius / 0.529
        except:
            pass
...
class Unitcell(object):
    ...
    def __eq__(self, other):
        if isinstance(other, Atom):
            return self.a == other.a and self.c == other.c and self.alpha == other.alpha and self.beta == other.beta and self.gamma == other.gamma
        else:
            return False
...
class Model(object):
    ...
    def _nineq_atoms(self):
        ...
        for element in elements:
            ...
            nineq_atoms += n_atoms
            atom_dict = {'nineq_atoms': n_atoms, 'n_atoms': len(atoms), 'atom_list': atoms}
            element_dict[element] = atom_dict

        return (
         nineq_atoms, element_dict)
...
class MTZ_model(Model):
        ...
        try:
            with open(filename, 'r') as (f):
                ...
                while line.split()[0].isalpha():
                    n, Z, val, rad = [ t(s) for t, s in zip((int, float, float, float), f.readline().split('#')[0].split()[:4])
                                     ]
                    for i in range(0, n):
                        position = [ t(s) for t, s in zip((float, float, float), f.readline().split('#')[0].split()[:3])
                                   ]
                        atom = Atom(line.split()[0].capitalize(), coordinates=position, tag=line.split()[1], valence=val, radius=rad, Z=int(Z))
    ...
    def create_atorbs(self, **kwargs):
        ...
        if 'output_dir' in kwargs:
            ...
            for element in set([ str(atom.element.symbol) for atom in self.atoms ]):
                ...
                if not os.path.isfile(atorb_file):
                ...
                if not os.path.isfile(at_file):
                    at_file = atorb.Atorb.calculate_Q_density(input=atorb_file, output_dir=os.path.join(output_dir, 'Atorb'))
    ...
    def get_MTZ(self, filename):
        ...
        try:
            with open(filename, 'r') as (f):
                self.mtz = float([ line for line in f ][0])
        except IOError:
            raise IOError
    ...
    def calculate_MTZ(self, mtz_string='', **kwargs):
        ...
        self.mtz = libphsh.cavpot(mtz_string, int(slab), atomic_file, cluster_file, mufftin_file, output_file, info_file)
    ...
    def gen_input(self, **kwargs):
        ...
        with open(filename, 'w') as (f):
            ...
            f.write(str(' %7.4f %7.4f %7.4f' % (self.unitcell.basis[0][0] / a,
             ...
             self.unitcell.basis[0][2] / a)).ljust(33) + '# SPA coordinates of unit cell\n')
            f.write(str(' %7.4f %7.4f %7.4f' % (self.unitcell.basis[1][0] / a,
             self.unitcell.basis[1][1] / a,
             self.unitcell.basis[1][2] / a)).ljust(33) + '\n')
            f.write(str(' %7.4f %7.4f %7.4f' % (self.unitcell.basis[2][0] / a,
             self.unitcell.basis[2][1] / a,
             self.unitcell.basis[2][2] / a)).ljust(33) + '# Notice the value %.2f (%s calculation)\n' % (
            ...
            if isinstance(nineq_atoms, tuple):
                ...
                for element in elements_dict:
                    ...
                    for ineq_atom in set(atoms):
                        ...
                        while ineq_atom.tag in tags:
                            ...
                            ineq_atom.tag = ('').join([ ch for ch in ineq_atom.tag if ch.isalpha() or ch in ('_',
                                                                                                             '-',
                                                                                                             '+') ]) + '_' + str(number)

                        f.write(('{0} {1}').format(ineq_atom.element.name.capitalize(), ineq_atom.tag).ljust(33) + '# element, name tag\n')
                        f.write(str('%4i %7.4f %7.4f %7.4f' % (len(ineq_atoms),
                         ineq_atom.Z, ineq_atom.valence,
                         ineq_atom.radius / 0.529)).ljust(33) + '# atoms in unit cell, Z, valence, Muffin-tin radius (Bohr radii)\n')
                        for atom in ineq_atoms:
                            f.write(str(' %7.4f %7.4f %7.4f' % (
                             atom.coordinates[0] / a,
                             atom.coordinates[1] / a,
                             atom.coordinates[2] / a)).ljust(33) + '# coordinates in SPA units\n')
                ...
                for atom in set(self.atoms):
                    while atom.tag in tags:
                        ...
                        atom.tag = ('').join([ ch for ch in atom.tag if ch.isalpha() or ch in ('_',
                                                                                               '-',
                                                                                               '+') ]) + '_' + str(number)

                    tags.append(atom.tag)
                    f.write(('{0} {1}').format(atom.element.name.capitalize(), atom.tag).ljust(33) + '# element, name tag\n')
                    f.write(str('%4i %7.4f %7.4f %7.4f' % (
                     tags.count(atom.tag), atom.Z, atom.valence,
                     atom.radius / 0.529)).ljust(33) + '# atoms in unit cell, Z, valence, Muffin-tin radius (Bohr radii)\n')
                    f.write(str(' %7.4f %7.4f %7.4f' % (
                     atom.coordinates[0] / a,
                     atom.coordinates[1] / a,
                     atom.coordinates[2] / a)).ljust(33) + '# coordinates in SPA units\n')
