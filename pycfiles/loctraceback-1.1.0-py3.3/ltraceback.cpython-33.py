...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/loctraceback/ltraceback.py
# Compiled at: 2018-03-13 19:34:45
# Size of source mod 2**32: 26923 bytes
"""Extract, format and print information about Python stack traces."""
import collections, itertools, loctraceback.linecache as linecache, sys, tempfile
from io import StringIO
from hashlib import sha1
from uncompyle6.semantics.fragments import code_deparse, deparse_code_around_offset, deparsed_find
...
class FrameSummary:
    ...
    def __iter__(self):
        return iter([self.filename, self.lineno, self.name,
         self.line, self.code, self.last_i])

    def __repr__(self):
        return '<FrameSummary file {filename}, line {lineno} in {name} <code {code}> @{last_i} >'.format(filename=self.filename, lineno=self.lineno, name=self.name, code=self.code, last_i=self.last_i)
...
class StackSummary(list):
    __doc__ = 'A stack of frames.'

    @classmethod
    def extract(klass, frame_gen, *, limit=None, lookup_lines=True, capture_locals=False, lasti=None):
        ...
        try:
            for f, lineno, lasti in frame_gen:
                ...
                if capture_locals:
                    ...
                    f_locals = None
                result.append(FrameSummary(filename, lineno, name, lookup_line=False, locals=f_locals, code=co, last_i=lasti))
    ...
    def from_list(klass, a_list):
        ...
        Each tuple should be a 4-tuple with (filename, lineno, name,
        line) elements.
        """
        result = StackSummary()
        for frame in a_list:
            if isinstance(frame, FrameSummary):
                ...
                filename, lineno, name, line, code, last_i = frame
                result.append(FrameSummary(filename, lineno, name, line=line, code=code, last_i=last_i))
    ...
    def format(self):
        ...
        for frame in self:
            ...
            if count >= 3:
                continue
            row = []
            row.append('  File "{}", line {}, in {}{}\n'.format(frame.filename, frame.lineno, frame.name, ' at offset {}'.format(frame.last_i if frame.last_i else '')))
            if frame.line:
                row.append('    {}\n'.format(frame.line.strip()))
            if frame.code:
                if frame.last_i:
                    text = deparse_text(frame.code, frame.name, frame.last_i)
                    if text:
                        row.append('\n'.join(text[1:]))
            if frame.locals:
...
if __name__ == '__main__':
    ...
    stack = extract_stack(f=frame, limit=None)
