...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/harold/publishers/pythonmodule.py
# Compiled at: 2006-08-02 05:57:50
import imp, os, sys, types
from harold.lib import import_wrapper, keys
from harold.log import logger
from harold.publishers.common import request_args, publishable_items, TemplateMixin
...
class CodeTemplate(TemplateMixin):
    ...
    def execute_class(self, cls, args, query, context):
        ...
        exposed = cls.expose
        if init:
            (init_args, init_kwds) = request_args(init, args, query, context, ignore_additional_args=True)
        ...
        if args:
            ...
            if check_meth:
                ...
                meth = check_meth
        if meth:
            if meth_name not in cls.expose:
                raise self.pub_exc
            (meth_args, meth_kwds) = request_args(meth, args, query, context, ignore_additional_args=True)
    ...
    def execute_plain(self, call, args, query, context):
        ...
        """
        try:
            (cargs, ckwds) = request_args(call, args, query=query, context=context, ignore_additional_args=False)
        except (TypeError,):
            raise self.pub_exc
    ...
    def report_callable(self, call, environ):
        ...
        pairs = (
         (
          'content_type', keys.content_type), ('view', keys.view), ('layout', keys.layout), ('defaults', keys.defaults))
