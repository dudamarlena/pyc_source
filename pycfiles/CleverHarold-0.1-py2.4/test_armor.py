...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/harold/tests/test_armor.py
# Compiled at: 2006-08-02 05:57:50
from inspect import getargspec
import unittest
from harold.lib import ValidationError, DecorationError
from harold.armor import email_formatted, bound_len, safe_sniff, alphanum_only, hostname_formatted, add, fuse
...
class ArmorEmailInput__Test(ArmorTestBase, unittest.TestCase):
    __module__ = __name__
    good_inputs = ('a@b.cd', 'john@smith.com', 'fred@example.com', 'john.smith@example.com',
                   'john__smith@e.net', 't@b.ne', 'troy@example.com', 'base.base@gmail.com')
    bad_inputs = ('troy', 'www', 'john smith@example.com', '$\\with a space@example.com',
                  'nodomain@.tld', 'notld@domain', '@example.com')
    format_calls = (
     email_formatted,)
...
class ArmorSimpleInput__Test(ArmorTestBase, unittest.TestCase):
    __module__ = __name__
    good_inputs = ('one', 'two', 'three')
    bad_inputs = ("fo'ur", 'fiv--e', 'si;x')
    format_calls = (
     safe_sniff, alphanum_only)
...
class ArmorHostNameInput__Test(ArmorTestBase, unittest.TestCase):
    __module__ = __name__
    good_inputs = ('four', 'five', 'six.seven')
    bad_inputs = ('eight space', )
    format_calls = (
     hostname_formatted,)
...
class ValidatorParameter__Test(unittest.TestCase):
    ...
    def test_chained_messages(self):
        ...
        try:
            ...
            self.failUnless('a' in invalids)
            self.failUnless('a' in valids)
            self.assertEqual(valids['a'], [('11', ), ('1', ), (1, )])
            self.failUnless('b' in invalids)
            self.failUnless('b' in valids)
            self.assertEqual(valids['b'], [(4, ), (2, ), ('2', )])
