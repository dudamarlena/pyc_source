...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/harold/tests/test_kidpub.py
# Compiled at: 2006-08-02 05:57:50
import unittest
from harold.lib import con_type
from harold.publishers import kid_publisher
from harold.tests.lib import test_app, make_start_response, kid_template_dirs, layout_template, default_templates, wsgi_env
...
class FsPublisher_FsKidTemplate__Test(unittest.TestCase):
    ...
    def setUp(self):
        self.headers = []
        self.publisher = kid_publisher(app=test_app, dirs=kid_template_dirs, layout=layout_template, defaults=default_templates, debug=True)
    ...
    def test__15__ntf_posargs_keyargs_defaults_query(self):
        ...
        keys = [
         'juice', 'snail', 'slug', 'cookies', 'environ', 'publisher', 'form']
        for key in keys:
    ...
    def test__19__nmf_defaults(self):
        ...
        results = [
         self.run_publisher('/funcs/S/1'), self.run_publisher('/funcs/S', 'a=1')]
        for result in results:
    ...
    def test__22__nmf_posargs_extraposargs(self):
        ...
        results = [
         self.run_publisher('/funcs/V/sloth', 'b=slug&b=snail'), self.run_publisher('/funcs/V/sloth/slug/snail', ''), self.run_publisher('/funcs/V/', 'a=sloth&b=slug&b=snail')]
        for result in results:
    ...
    def test__23__nmf_posargs_extraposargs_keywordargs(self):
        ...
        results = [run('/funcs/W/sloth', 'b=slug&b=snail'), run('/funcs/W/sloth/slug/snail', ''), run('/funcs/W/', 'a=sloth&b=slug&b=snail'), run('/funcs/W/sloth', 'c=slug&d=snail'), run('/funcs/W/sloth/slug', 'b=snail')]
        for result in results:
            self.failUnless('sloth' in result)
            self.failUnless('slug' in result)
            self.failUnless('snail' in result)
