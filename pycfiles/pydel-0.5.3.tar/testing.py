...
# Embedded file name: /Users/will/.virtualenvs/datasight-backend/lib/python2.7/site-packages/dejavu/testing.py
...
import traceback, fnmatch, os, re, ast, subprocess, random, logging
...
def generate_test_files(src, dest, nseconds, fmts=[
 '.mp3', '.wav'], padding=10):
    """
...
def autolabeldoubles(rects, ax):
    for rect in rects:
        height = rect.get_height()
        ax.text(rect.get_x() + rect.get_width() / 2.0, 1.05 * height, '%s' % round(float(height), 3), ha='center', va='bottom')
...
class DejavuTest(object):
    ...
    def __init__(self, folder, seconds):
        ...
        self.test_files = [ f for f in os.listdir(self.test_folder) if os.path.isfile(os.path.join(self.test_folder, f)) and re.findall('[0-9]*sec', f)[0] in self.test_seconds
                          ]
        ...
        for f in self.test_files:
            ...
            if result.strip() == 'None':
                ...
                if song_result != song:
                    ...
                    result_start_time = round(result[Dejavu.OFFSET] * DEFAULT_WINDOW_SIZE * DEFAULT_OVERLAP_RATIO / DEFAULT_FS, 0)
