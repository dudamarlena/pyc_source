...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/wlogger/log.py
# Compiled at: 2015-12-24 07:06:45
from __future__ import print_function
import argparse, argcomplete, datetime, os, sys, time, pickle, markdown, datetime, wlogger.engine
...
def remove_util(args):
    ...
    if from_section in todo.keys():
        ...
        if from_section in done.keys():
             ...
             (
              (' ').join(args.remove), datetime.datetime.now())]
    else:
...
def progress_util(args):
    ...
    if to_section in done.keys():
         ...
         (
          (' ').join(args.progress), datetime.datetime.now())]
    pickle.dump(done, open(location + '/wlogger/data/done.p', 'w'))
    return
