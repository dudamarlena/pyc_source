...
# [GCC 8.4.0]
# Embedded file name: /Users/idohyeon/Projects/motty/motty/app/views.py
# Compiled at: 2017-11-23 22:02:56
...
def save_action(request, resource_id, action_id=None):
    ...
    if request.method == 'GET':
        if action_id is None:
            return render(request, 'app/action/form.html', {'resource': resource})
        else:
...
def save_resource(request):
    try:
        ...
        serializer = ResourceSerializer(resource, data=data) if data.get('id') is None else ResourceSerializer(resource, data=data, partial=True)
        if serializer.is_valid():
            serializer.save()
            return JsonResponse((serializer.data), safe=False)
        else:
