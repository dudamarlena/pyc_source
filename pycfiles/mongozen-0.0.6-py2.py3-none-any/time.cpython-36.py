...
# [GCC 8.4.0]
# Embedded file name: /home/zencity/clones/mongozen/mongozen/queries/time/time.py
# Compiled at: 2018-01-10 10:48:40
...
def group_by_timeframe_with_date(collection_obj, timeframe_len_in_sec, timefield_name, match_op=None):
...
def timeframe_size_dist(collection_obj, timefield, timefield_type, timeframe_len, matchop=None, second_group=None):
    ...
    if matchop is None:
        ...
        if timefield_type is int:
            ...
            if timefield_type is datetime:
                timestamp_exp = {'$subtract': [
                               timefield_name, epoch_datetime()]}
            else:
                raise TypeError('timeframe_size_dist() should recieve ' + 'timefield_type parameter either an int or datetime.datetime!')
    first_project_stage = {'$project': {'timeframe': {'$subtract': [
                                              timestamp_exp,
                                              {'$mod': [
                                                        timestamp_exp,
                                                        timeframe_len]}]}}}
    first_group_stage = {'$group': {'_id':{'timeframe': '$timeframe'}, 
                'x_i':{'$sum': 1}}}
    if second_group:
        first_group_stage['$group']['_id']['second_group'] = second_group
    else:
        second_project_stage = {'$project': {'_id':1, 
                      'x_i':1,  'x_i_sqr':{'$multiply': ['$x_i', '$x_i']}}}
        if second_group:
            second_project_stage['$project'][second_group] = 1
        second_group_stage = {'$group': {'_id':second_group, 
                    'mean':{'$avg': '$x_i'}, 
                    'Sx_i':{'$sum': '$x_i'}, 
                    'Sx_i_sqr':{'$sum': '$x_i_sqr'}, 
                    'n':{'$sum': 1}}}
        third_project_stage = {'$project': {'_id':0, 
                      'n':1, 
                      'Sx_i':1,  'Sx_i_sqr':1,  'mean':1,  'for_all':{'$literal': second_group is None}, 
