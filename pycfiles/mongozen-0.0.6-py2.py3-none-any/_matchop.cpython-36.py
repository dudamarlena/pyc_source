...
# [GCC 8.4.0]
# Embedded file name: /home/zencity/clones/mongozen/mongozen/matchop/_matchop.py
...
def _resolve_compar_ops(compar_ops, compar_vals):
    if any([isinstance(val, dict) for val in compar_vals]):
        ...
        if len(set(equ)) > 1:
            warnings.warn('More than one $eq for the same field in Matchops joined by &, with different values. Resulting Matchop matches the empty set.')
        ...
        if ins_intersection:
            if len(ins_intersection.intersection(nins)) > 1:
                warnings.warn('Matchop and operation resulted in a Matchop demanding')
            ins_intersection = segment & ins_intersection
        if len(set(equ)) == 1:
            res['$eq'] = equ[0]
            if equ[0] in segment:
                if _val_in_inter(equ[0], ins_intersection):
                    return {'$eq': equ[0]}
            warnings.warn('$eq value not in resulting range of Matchops joined by &. Resulting Matchop matches the empty set.')
    return res
                ...
                for subkey in matchop[key]:
                    if subkey in __COMPAR_OPS:
                        ...
                        if subkey in non_compar_seen:
                            raise ValueError('Non-comparison operators in intersection. Performing a simple and.')
...
class Matchop(dict):
    __doc__ = 'Defines a matching operator for mongodb operations.'
...
def regex_matchop(field_name, pattern_obj):
    ...
    return Matchop({field_name: {'$regex': pattern_obj}})
