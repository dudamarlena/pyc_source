...
# [GCC 8.4.0]
# Embedded file name: /home/zencity/clones/mongozen/mongozen/param_inference.py
...
from .shared import _get_map, ParamInferMap, _mongozen_cfg, CfgKey
...
def _infer_get_db_params(db_name, server_name, env_name):
    if env_name is None:
        if not INFER_PARAM:
            ...
            if server_name is None:
                ...
                env_name = get_priority_elem_in_set(possible_envs, _mongozen_cfg()[CfgKey.ENV_PRIORITY.value])
            else:
    ...
    if server_name is None:
        ...
        possible_servers = _get_map(ParamInferMap.DB_N_ENV_TO_SERVER)[(
         db_name, env_name)]
        server_name = get_priority_elem_in_set(possible_servers, _mongozen_cfg()[CfgKey.SERVER_PRIORITY.value][env_name])
    return (
     server_name, env_name)
...
def _filter_dbs(db_names):
    try:
        ...
        return [name for name in db_names if all([term not in name for term in bad_terms])]
    except KeyError:
        return list(db_names)


def _infer_get_collection_params(collection_name, db_name, server_name, env_name):
    if env_name is None:
        if not INFER_PARAM:
            raise ValueError('Missing value for the env_name parameter!')
        else:
            if server_name is None:
                if db_name is None:
                    ...
                    possible_envs = _get_map(ParamInferMap.COL_N_DB_2_ENV)[(collection_name, db_name)]
                    env_name = get_priority_elem_in_set(possible_envs, _mongozen_cfg()[CfgKey.ENV_PRIORITY.value])
            else:
                if db_name is None:
                    possible_envs = _get_map(ParamInferMap.COL_N_SERVER_2_ENV)[(
                     collection_name, server_name)]
                    env_name = get_priority_elem_in_set(possible_envs, _mongozen_cfg()[CfgKey.ENV_PRIORITY.value])
                else:
                    possible_envs = _get_map(ParamInferMap.COL_N_DB_N_SERVER_2_ENV)[(
