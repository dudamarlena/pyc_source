...
# [GCC 8.4.0]
# Embedded file name: /home/zencity/clones/mongozen/mongozen/shared.py
...
import yaml, comath
from .util_constants import MONGO_CRED_FPATH, ACCESS_MODES, PERSONAL_MONGO_CFG_FPATH, DATA_DIR_PATH

class CfgKey(Enum):
    __doc__ = 'Details the keys of the mongozen config yaml file.'
    ENVS = 'envs'
...
DEFAULT_CFG = {CfgKey.INFER_PARAM: False}
...
def _get_mongo_cred():
    try:
        ...
        msg += ' connect to. See {} for further details.'.format('https://github.com/shaypal5/mongozen')
        warn(msg)
        return {}
...
def _mongozen_cfg():
    try:
        ...
        warn('A ~/.mongozen/mongozen_cfg.yml file was not found. A file was created with default values.See {} for further details.'.format('https://github.com/shaypal5/mongozen'))
        return DEFAULT_CFG
...
def _build_mongo_uri(server_cfg, mode_cred):
    return 'mongodb://{user}:{pswd}@{host}'.format(user=(mode_cred['username']),
      pswd=(quote(mode_cred['password'])),
      host=(_host_str_from_server_cfg(server_cfg)))


MISSING_CRED_ERROR_MSG = 'Missing mongozen credentials for {} from/to {}.{}! Please see the documentation.'
...
def _get_server_cfg(server_name, env_name, mode='reading'):
    ...
    try:
        ...
        raise ValueError('Bad env or server name: env={}, server={}. '.format(env_name, server_name), 'Found cfg={}. Found cred={}'.format(cfg, cred))

    try:
        mode_cred = cred[env_name][server_name][mode].copy()
    except KeyError:
        if mode in ACCESS_MODES:
            raise Exception(MISSING_CRED_ERROR_MSG.format(mode, env_name, server_name))
        raise ValueError('Bad access mode name: {}. Found cfg={}. Found cred={}'.format(mode, cfg, cred))
    except TypeError:
        raise Exception(MISSING_CRED_ERROR_MSG.format(mode, env_name, server_name))

    server_params = server_cfg.get(CfgKey.MONGOZEN_SERVER_PARAMS.value, {})
    resolved_server_cfg = {'host': _build_mongo_uri(server_cfg, mode_cred)}
...
def _env_list():
    ...
    try:
        ...
        final_priority = env_priority.copy() + [x for x in envs if x not in env_priority]
        final_priority.reverse()
        envs = [env for env in final_priority if env in _get_mongo_cred()]
        return envs
    except (KeyError, TypeError):
        envs = list(_get_mongo_cred().keys())
        envs.reverse()
        envs = [env for env in envs if env in _get_mongo_cred()]
        return envs


@lru_cache(maxsize=(comath.func.closest_larger_power_of_2(len(_get_mongo_cred().keys()))))
def _server_list(env_name):
    try:
        server_priority = _mongozen_cfg()[CfgKey.SERVER_PRIORITY.value][env_name]
        server_priority.reverse()
        servers = [server for server in server_priority if server in _get_mongo_cred()[env_name].keys()]
        return servers
    except (KeyError, TypeError):
...
def get_server_list(env):
    ...
    try:
        return [srv for srv in _mongozen_cfg()[CfgKey.ENVS.value][env].keys() if srv != CfgKey.MONGOZEN_ENV_PARAMS.value]
    except KeyError:
        return []
...
class ParamInferMap(Enum):
    __doc__ = 'Names of maps used for parameter inference.'
