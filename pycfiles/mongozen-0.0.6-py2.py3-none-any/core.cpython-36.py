...
# [GCC 8.4.0]
# Embedded file name: /home/zencity/clones/mongozen/mongozen/core.py
...
from .shared import _get_server_cfg, _env_list, _server_list
from .mongozen_objs import MongozenClient
from .param_inference import _infer_get_client_params, _infer_get_db_params, _infer_get_collection_params
...
def _server_descriptor_builder(server_name):
    ...
    class _ServerDescriptor(object):
        ...
        def __get__(self, instance, owner):
            return get_reading_client(server_name=(self.server_name),
...
class _EnvProperty(object):
    __doc__ = 'Env property.'

    def __init__(self, env_name, server_list):
        self._env_name = env_name
        self._server_list = server_list
        for server in server_list:
            setattr(_EnvProperty, server, _server_descriptor_builder(server))

    def __getattr__(self, name):
        if name in self._server_list:
            return get_reading_client(server_name=name,
              env_name=(self._env_name))
        raise AttributeError('Environment {} has no server named {}'.format(self._env_name, name))
...
def get_db(db_name, server_name=None, env_name=None, mode='reading'):
    ...
    """
    server_name, env_name = _infer_get_db_params(db_name, server_name, env_name)
    return _get_client(server_name, env_name, mode)[db_name]


def get_collection(collection_name, db_name=None, server_name=None, env_name=None, mode='reading'):
    ...
    """
    db_name, server_name, env_name = _infer_get_collection_params(collection_name, db_name, server_name, env_name)
