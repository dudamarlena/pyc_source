...
# [GCC 8.4.0]
# Embedded file name: /home/zencity/clones/mongozen/mongozen/queries/common/common.py
# Compiled at: 2018-01-10 10:48:40
# Size of source mod 2**32: 5286 bytes
import operator
...
def count_keys_in(keys_list, collection_obj, verbose=False):
    ...
    for key in keys_list:
        group_params[key] = {'$sum': {'$cond': {'if':{'$ifNull': ['$' + key, False]}, 
                            'then':1,  'else':0}}}
...
def key_value_counts(key, collection_obj, pre_operators=(), verbose=False):
    ...
    if verbose:
        print('\nCounting values of key {} in collection {}.'.format(key, collection_obj.full_name))
...
def get_pretty_frequency_table(type_name, frequency_dict, total_count):
    ...
    for item in sorted_items:
        ...
        occurrence_percent = round(total_key_count * 100.0 / total_count, 2) if total_count else 0.0
