...
# [GCC 8.4.0]
# Embedded file name: /home/zencity/clones/mongozen/mongozen/util/_util.py
# Compiled at: 2018-02-12 08:15:25
...
import copy, numpy as np, utilitime
from strct.dicts import flatten_dict
import bson
from bson.json_util import dumps, loads
NUM_BYTES_IN_MB = 1048576
MONGODB_DOC_SIZE_LIMIT_IN_MEGABYTES = 16
MONGODB_DOC_SIZE_LIMIT_IN_BYTES = MONGODB_DOC_SIZE_LIMIT_IN_MEGABYTES * NUM_BYTES_IN_MB - 1024
...
MONGOTEMP_CMD = '{cmd} --host="{host}" {usr} {pwd} --db="{db}" {authdb} {readpreference} {verbosity}'
...
def _mongo_cmd(cmd, msg, db_obj, mode, verbose=None, auto=None):
    ...
    if verbose is None:
        ...
        if verbose:
            verbosity_flag = '-v'
    mongo_cmd = MONGOTEMP_CMD.format(cmd=cmd,
      host=host_str,
      usr=usr,
      pwd=pwd,
      db=db_name,
    ...
    try:
        try:
            output = subprocess.check_output(mongo_cmd,
...
def dump_collection(source_collection, output_dir_path, query=None, verbose=True, auto=False):
    ...
    if verbose:
        ...
        if doc_count > 100000:
            print('The collection is very large ({} documents)!'.format(doc_count))
            response = input("Please confirm by typing 'y': ")
            if response != 'y':
                return
    partially_formatted_cmd = MONGOTEMP_CMD.format(cmd='mongodump',
      collection=('--collection {}'.format(source_collection.name)),
      dir_flag='--out')
    partially_formatted_msg = CMD_MSG.format(action='Dumping',
      collection=(source_collection.name),
      direction='to')
    _mongo_cmd(partially_formatted_cmd, partially_formatted_msg, source_collection.database, output_dir_path, 'reading', verbose)
...
def restore_collection(target_db, input_file_path, verbose=True, auto=False):
    ...
    auto : bool, default False
        ...
        Otherwise, confirmation is asked if verbose is set to True.
    """
    partially_formatted_cmd = MONGOTEMP_CMD.format(cmd='mongorestore',
      collection='',
      dir_flag='--dir')
    collection_name = basename(input_file_path)
    partially_formatted_msg = CMD_MSG.format(action='Resgtoring',
      collection=collection_name,
      direction='from')
    _mongo_cmd(partially_formatted_cmd, partially_formatted_msg, target_db, input_file_path, 'writing', verbose)


EXPORT_CMD = 'mongoexport {fields} {query} {type}'

def export_collection(collection, output_fpath, fields=None, query=None, ftype=None, escape_dollar=None, verbose=None, auto=None):
    ...
    if ftype is None:
        ...
        if escape_dollar is None:
            ...
            cmd = 'mongoexport --collection="{}" --out="{}"'.format(collection.name, output_fpath)
    ...
    _mongo_cmd(cmd=cmd, msg=msg, db_obj=(collection.database), mode='reading', verbose=verbose,
      auto=auto)


DUMPS_DIR_PATH = os.path.expanduser('~/.mongozen_temp_dump')

def copy_collection(source_collection, target_db, temp_dir_path=None, verbose=True):
    ...
    """
    if verbose:
        print('Copying collection {}.{}.{}.{} to {}.{}.{}'.format(source_collection.database.client.env, source_collection.database.client.server, source_collection.database.name, source_collection.name, target_db.client.env, target_db.client.server, target_db.name))
    else:
        temp_dir_name = '{}/{}'.format(source_collection.database.client.env, source_collection.database.client.server)
        ...
        temp_dump_subdir_path = os.path.join(temp_dump_dir_path, source_collection.database.name)
        temp_dump_file_path = os.path.join(temp_dump_subdir_path, source_collection.name + '.bson')
...
def dump_document_cursor_to_csv(doc_cursor, file_path, fieldnames=None, missing_val=None, flatten=False):
    ...
    if flatten:
        ...
        def doc_trans(doc):
            return flatten_dict(dict_obj=doc,
              separator='.',
              flatten_lists=True)
    ...
    with open(file_path, 'w+') as (file_obj):
        writer = csv.DictWriter(file_obj, fieldnames, restval=missing_val, extrasaction='ignore',
          dialect='excel')
        writer.writeheader()
        if first_doc:
