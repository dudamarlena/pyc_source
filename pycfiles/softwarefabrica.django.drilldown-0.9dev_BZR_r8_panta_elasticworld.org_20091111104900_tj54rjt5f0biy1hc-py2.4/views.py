...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/softwarefabrica/django/drilldown/views.py
# Compiled at: 2009-11-11 04:10:44
"""
Drilldown views.
...
from softwarefabrica.django.crud.crud import ViewCall, GenericView, POPUP_VARIABLE, CONTEXT_POPUP_VARIABLE
...
NONE_REPR = '__none__'
FALSE_REPR = '__false__'
TRUE_REPR = '__true__'
EMPTY_REPR = '__empty__'
...
def _db_python_to_text(db_field, value):
    ...
    if value is None:
        ...
        if isinstance(value, models.Model):
            ...
            return '%s' % value.pk
        return '%s' % value
    elif isinstance(db_field, models.DateTimeField) or isinstance(db_field, models.DateField) or isinstance(db_field, models.TimeField):
...
def _db_text_to_python(db_field, text):
    ...
    if text == NONE_REPR or text is None:
        ...
        return text
    elif isinstance(db_field, models.ForeignKey) or isinstance(db_field, models.ManyToManyField):
...
class DrillDownField(object):
    ...
    def __init__(self, ddinfo, db_field, raw_value, url=None, auto_url=False, empty_value=None, use_value=True):
    ...
    def construct_search(self):
        ...
        if isinstance(self.field, RelatedField):
            ...
            query_kw = '%s' % field_name
        elif isinstance(self.field, models.DateTimeField) or isinstance(self.field, models.DateField) or isinstance(self.field, models.TimeField):
            query_kw = '%s' % field_name
...
class DrillDownInfo(object):
    ...
    def __init__(self, vcall, request, url, queryset, drilldown_fields, view_name, view_args, view_kwargs, ddfield_class=None):
        ...
        self._trim_view_arguments()
        self.url_components = None
        self.field_values = MultiValueDict()
    ...
    def _trim_view_arguments(self):
        self.view_args = list(self.vcall.view_args)
        self.view_kwargs = dict(self.vcall.view_kwargs)
        for k in ('request', 'url', 'queryset', 'fields', 'paginate_by', 'page', 'template_name',
                  'template_loader', 'extra_context', 'login_required', 'context_processors',
                  'mimetype', 'lookup_kwargs', 'ddinfo_class', 'ddfield_class', 'template_object_name',
                  'view_name'):
            if k in self.view_kwargs:
                del self.view_kwargs[k]
...
class DrilldownParamEncoder(object):
    ...
    def encode(self):
        ...
        param_str = '%s' % self.param
        if '/' in param_str:
            b64pfx_param_str = ':base64:' + param_str
    ...
    def decode(self, encoded):
        ...
        try:
            ...
            if b64pfx_param_str and b64pfx_param_str.startswith(':base64:'):
...
class DrillDownView(GenericView):
    ...
    def __init__(self, queryset=None, paginate_by=None, model=None, app_label=None, model_name=None, fields=None, mimetype=None, lookup_kwargs=None, ddinfo_class=None, ddfield_class=None, allow_empty=None, template_object_name=None, view_name=None, *args, **kwargs):
    ...
    def get_paginator(self, vcall, request, queryset=None, paginate_by=None, allow_empty=None, page=None):
        ...
        if queryset is None:
            if vcall.ddinfo:
                assert isinstance(vcall.ddinfo, DrillDownInfo)
                queryset = vcall.ddinfo.filtered_queryset
        if queryset is None:
    ...
    def __call__(self, request, url, *args, **kwargs):
        from django.core.paginator import InvalidPage
        from softwarefabrica.django.common.models import get_sfapp_field, CommonModel
        from django.db.models.query import QuerySet
        vcall = self.viewcall_class(request, self, args, kwargs, url)
        assert isinstance(vcall, ViewCall)
        vcall.url = url
        ...
        app_label = vcall.app_label or self.app_label
        model_name = vcall.model_name or self.model_name
        vcall.c_model = model
        vcall.c_app_label = app_label
        vcall.c_model_name = model_name
        logging.debug('vcall:%s' % repr(vcall))
        queryset = vcall.queryset
        if queryset is None:
            queryset = self.queryset
        vcall.c_queryset = queryset
        fields = vcall.fields or self.fields
        paginate_by = vcall.paginate_by or self.paginate_by
        page = vcall.page
        allow_empty = vcall.allow_empty
        if allow_empty is None:
            allow_empty = self.allow_empty
        template_name = vcall.template_name or self.template_name
        template_loader = vcall.template_loader or self.template_loader
        extra_context = vcall.extra_context or {}
        login_required = vcall.login_required
        if login_required is None:
            login_required = self.login_required
        context_processors = vcall.context_processors or self.context_processors
        mimetype = vcall.mimetype or self.mimetype
        lookup_kwargs = vcall.lookup_kwargs or self.lookup_kwargs
        ddinfo_class = vcall.ddinfo_class or self.ddinfo_class
        ddfield_class = vcall.ddfield_class or self.ddfield_class
        template_object_name = vcall.template_object_name or self.template_object_name
        view_name = vcall.view_name or self.view_name
        ...
        if app_label and model_name and model is None:
            model = self.lookup_model(vcall, app_label, model_name)
        vcall.c_model = model
        ...
        if issubclass(model, CommonModel):
            drilldown_fields = get_sfapp_field(model, 'drilldown_fields', get_sfapp_field(model, 'fields', fields))
        if drilldown_fields is None:
            drilldown_fields = [ f.name for f in opts.fields ]
        ddinfo = ddinfo_class(vcall, request, url, queryset, drilldown_fields, view_name, args, kwargs, ddfield_class)
        assert isinstance(ddinfo, DrillDownInfo)
        vcall.ddinfo = ddinfo
        ...
        is_paginated = False
        if paginate_by:
            paginator = self.get_paginator(vcall, request, ddinfo.filtered_queryset)
            ...
            try:
                ...
                try:
                    ...
                    is_paginated = True
        vcall.c_object_list = object_list
        vcall.c_is_paginated = is_paginated
        self.apply_extra_context(vcall, extra_context, self.extra_context)
        c = self.get_context(vcall, request, {'model': model, 'meta': model._meta, 'url': url, 'fields': drilldown_fields, 'ddinfo': ddinfo, 'used_fields': ddinfo.used_fields, 'remaining_fields': ddinfo.remaining_fields, 'pivot_field': ddinfo.pivot_field, 'pivot_field_values': ddinfo.pivot_field_values, 'select_field': select_field, 'select_value': select_value, 'found': ddinfo.filtered_queryset, 'count': ddinfo.filtered_queryset.count(), '%s_list' % template_object_name: object_list, '%s_count' % template_object_name: object_list.count(), 'paginator': paginator, 'page_obj': page_obj, 'is_paginated': is_paginated})
    ...
    @classmethod
    def MakeUrl(cls, model, view_name, field_values, pivot_field=None, kwargs=None):
        """
        ...
        if filter_url and not filter_url.endswith('/'):
            filter_url = filter_url + '/'
        reverse_kwargs = dict(kwargs or {}, url=filter_url)
        url = reverse(view_name, kwargs=reverse_kwargs)
        return url

    def make_url(self, field_values, pivot_field=None, model=None, view_name=None, kwargs=None):
        view_name = view_name or self.view_name
        model = model or self.model
        return self.MakeUrl(model=model, view_name=view_name, field_values=field_values, pivot_field=pivot_field, kwargs=kwargs)
    ...
    def make_pivot_values(self, ddinfo):
        ...
        for kv in pivot_field_keyvalues:
            ...
            view_kwargs = dict(ddinfo.view_kwargs or {}, url=urlparse.urljoin(base_url, '%s' % DrilldownParamEncoder.Encode(_db_python_to_text(ddinfo.pivot_field, raw_value))))
            url = reverse(ddinfo.view_name, kwargs=view_kwargs)
            item = ddinfo.ddfield_class(ddinfo=ddinfo, db_field=ddinfo.pivot_field, raw_value=raw_value, url=url)
        ...
        logging.info('pivot field values:%s' % repr(pivot_field_values))
        return self
    ...
    def make_remaining_field_values(self, ddinfo):
        ...
        for field_name in ddinfo.remaining_field_names:
            ...
            view_kwargs = dict(ddinfo.view_kwargs, url=urlparse.urljoin(ddinfo.url, field_name))
            item = ddinfo.ddfield_class(ddinfo=ddinfo, db_field=db_field, raw_value=None, use_value=False, url=reverse(ddinfo.view_name, kwargs=view_kwargs))
    ...
    def make_bread_crumbs(self, ddinfo):
        ...
        assert isinstance(ddinfo, DrillDownInfo)
        ddinfo.bread_crumbs_root = self.make_url({}, model=ddinfo.model, view_name=ddinfo.view_name, kwargs=ddinfo.view_kwargs)
        field_values = SortedDict()
        for dd_field in ddinfo.used_fields:
            assert isinstance(dd_field, DrillDownField)
            url_field = self.make_url(field_values, pivot_field=dd_field.name, model=ddinfo.model, view_name=ddinfo.view_name, kwargs=ddinfo.view_kwargs)
            field_values[dd_field.name] = dd_field.raw_value
            url_value = self.make_url(field_values, model=ddinfo.model, view_name=ddinfo.view_name, kwargs=ddinfo.view_kwargs)
            ddinfo.bread_crumbs.append({'field': dd_field.field, 'name': dd_field.name, 'value': dd_field.value, 'url_field': url_field, 'url_value': url_value})

        return self

    def lookup_model(self, vcall, app_label, model_name):
        """
        Return the ``model`` class given ``app_label`` and ``model_name``.
        """
        from django.contrib.contenttypes.models import ContentType
        if not (app_label and model_name):
            raise GenericViewError('%s view must be called with either a model class or an app_label/model_name.' % (self.__class__.__name__,))
        try:
    ...
    def get_template(self, vcall, template_name=None, template_loader=None):
        ...
        template_name = template_name or ('%s/%s_drilldown.html' % (app_label, object_name), '%s/%s_browse.html' % (app_label, object_name), '%s/object_drilldown.html' % app_label, '%s/object_browse.html' % app_label, '%s_drilldown.html' % object_name, '%s_browse.html' % object_name, 'object_drilldown.html', 'object_browse.html', 'drilldown/object_drilldown.html', 'drilldown/object_browse.html')
        vcall.c_template_name = template_name
        return super(DrillDownView, self).get_template(vcall, template_name=template_name, template_loader=template_loader)
