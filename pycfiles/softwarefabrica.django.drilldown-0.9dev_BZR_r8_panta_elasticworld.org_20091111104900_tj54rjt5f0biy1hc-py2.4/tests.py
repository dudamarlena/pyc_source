...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/softwarefabrica/django/drilldown/tests/tests.py
# Compiled at: 2009-10-23 09:19:38
import os
...
class TestDrillDownView(TestCase):
    ...
    def setUp(self):
        self.author_dante = Author(first_name='Dante', last_name='Alighieri', birth_date='1265-06-01', death_date='1321-09-14')
    ...
    def test_drilldown_author(self):
        ...
        c = Client()
        url = reverse('drilldown-generic-drilldown', kwargs={'app_label': 'tests', 'model_name': 'author', 'url': ''})
        response = c.get(url)
        ...
        url = reverse('drilldown-generic-drilldown', kwargs={'app_label': 'tests', 'model_name': 'author', 'url': 'last_name/'})
        response = c.get(url)
        self.failUnlessEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'drilldown/object_drilldown.html')
        context = response.context[(-1)]
        ...
        url = reverse('drilldown-generic-drilldown', kwargs={'app_label': 'tests', 'model_name': 'author', 'url': 'last_name/Alighieri/'})
        response = c.get(url)
        self.failUnlessEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'drilldown/object_drilldown.html')
        context = response.context[(-1)]
