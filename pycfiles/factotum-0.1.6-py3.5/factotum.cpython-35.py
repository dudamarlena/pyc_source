...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/factotum/factotum.py
# Compiled at: 2017-01-20 23:06:59
# Size of source mod 2**32: 6864 bytes
"""
FactoryFactoum -- a CLI for managing the headless linux factorio server
By: Brian Ballsun-Stanton
GPL v3
"""
import os, click, time, subprocess, json, codecs, sys, circus, inspect, signal, glob
...
def fulldeploy(username, password, servername, description, tag, admin):
    ...
    configAuthenticate(username, password)
    newFactorioMap()
    configSetup(servername=servername, description=description, tag=tag, visibility='public', serverpassword=None, genserverpasswordwords=4, admins=admin, ignoreplayerlimit=True, afk=5, uploadrate=None, updatepassword=True)
    runFactorio(False)
