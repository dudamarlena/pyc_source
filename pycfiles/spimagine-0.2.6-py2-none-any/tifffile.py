...
# [GCC 8.4.0]
# Embedded file name: /home/mweigert/python/spimagine/spimagine/lib/tifffile.py
# Compiled at: 2016-09-28 14:38:28
...
import sys, os, re, glob, math, zlib, time, json, struct, warnings, tempfile, datetime, collections
...
try:
    ...
    warnings.warn('failed to import the optional _tifffile C extension module.\nLoading of some compressed images will be slow.\nTifffile.c can be obtained at http://www.lfd.uci.edu/~gohlke/')
...
def imsave(filename, data, **kwargs):
    ...
    if 'bigtiff' not in tifargs and data.size * data.dtype.itemsize > 2000 * 1048576:
...
class TiffWriter(object):
    ...
    """
    TYPES = {'B': 1, 's': 2, 'H': 3, 'I': 4, '2I': 5, 'b': 6, 'h': 8, 
       'i': 9, 'f': 11, 'd': 12, 'Q': 16, 'q': 17}
    TAGS = {'new_subfile_type': 254, 
       'subfile_type': 255, 'image_width': 256, 
       ...
       'datetime': 306, 'predictor': 317, 'color_map': 320, 'tile_width': 322, 
       'tile_length': 323, 'tile_offsets': 324, 'tile_byte_counts': 325, 
       'extra_samples': 338, 'sample_format': 339, 'image_depth': 32997, 
       'tile_depth': 32998}

    def __init__(self, filename, bigtiff=False, byteorder=None, software='tifffile.py'):
        ...
        self._fh.write({'<': 'II', '>': 'MM'}[byteorder])
        if bigtiff:
            self._bigtiff = True
            self._offset_size = 8
            self._tag_size = 20
    ...
    def save(self, data, photometric=None, planarconfig=None, resolution=None, description=None, volume=False, writeshape=False, compress=0, extratags=()):
        """Write image data to TIFF file.
        ...
        if photometric == 'rgb':
            ...
            if planarconfig == 'contig':
                ...
                data = data.reshape((-1, ) + shape[-4 if volume else -3:] + (1, ))
                samplesperpixel = data.shape[1]
            ...
            if len(shape) < 3:
                volume = False
            if False and len(shape) > (3 if volume else 2) and shape[(-1)] < 5 and all(shape[(-1)] < i for i in shape[-4 if volume else -3:-1]):
                planarconfig = 'contig'
        ...
        shape = data.shape
        bytestr = bytes if sys.version[0] == '2' else (lambda x: bytes(x, 'utf-8') if isinstance(x, str) else x)
        tags = []
        if volume:
        ...
        if description:
            ...
            if writeshape and shape[0] > 1 and shape != data_shape:
                addtag('image_description', 's', 0, 'shape=(%s)' % (',').join('%i' % i for i in data_shape), writeonce=True)
            addtag('datetime', 's', 0, datetime.datetime.now().strftime('%Y:%m:%d %H:%M:%S'), writeonce=True)
            ...
            addtag('photometric', 'H', 1, {'miniswhite': 0, 'minisblack': 1, 'rgb': 2}[photometric])
            ...
            addtag('rows_per_strip', 'I', 1, shape[(-3)] * (shape[(-4)] if volume else 1))
            strip_byte_counts = (
            ...
            if not self._bigtiff and fh.tell() + data.size * data.dtype.itemsize > 2147483647:
...
class TiffFile(object):
    ...
    def __init__(self, arg, name=None, offset=None, size=None, multifile=True, multifile_close=True):
        ...
        self._files = {self._fh.name: self}
        try:
            self._fromfile()
        except Exception:
            self._fh.close()
    ...
    def _fromfile(self):
        ...
        self._fh.seek(0)
        try:
            self.byteorder = {'II': '<', 'MM': '>'}[self._fh.read(2)]
        except KeyError:
            raise ValueError('not a valid TIFF file')

        version = struct.unpack(self.byteorder + 'H', self._fh.read(2))[0]
        if version == 43:
            self.offset_size, zero = struct.unpack(self.byteorder + 'HH', self._fh.read(4))
    ...
    def _fix_lsm_strip_byte_counts(self):
        ...
        for page in self.pages:
            ...
            for offset, bytecount in zip(page.strip_offsets, page.strip_byte_counts):
        ...
        for page in self.pages:
            if page.compression:
                page.strip_byte_counts = tuple(strips[offset] for offset in page.strip_offsets)
    ...
    def series(self):
        ...
        if self.is_ome:
            ...
            dims = {'X': 'X', 'Y': 'Y', 'Z': 'Z', 'T': 'T', 'WAVELENGTH': 'C', 'TIME': 'T', 'XY': 'R', 'EVENT': 'V', 
               'EXPOSURE': 'L'}
            mmhd = list(reversed(page0.mm_header.dimensions))
            series = [
             Record(axes=('').join(dims.get(i[0].strip().upper(), 'Q') for i in mmhd if i[1] > 1), shape=tuple(int(i[1]) for i in mmhd if i[1] > 1), pages=self.pages, dtype=numpy.dtype(page0.dtype))]
        elif self.is_lsm:
            ...
            axes = ('').join(axes)
            series = [
             Record(pages=self.pages, shape=tuple(shape), axes=axes, dtype=numpy.dtype(page0.dtype))]
        elif self.is_nih:
            if len(self.pages) == 1:
                shape = page0.shape
                axes = page0.axes
                ...
                shape = (
                 len(self.pages),) + page0.shape
                axes = 'I' + page0.axes
            series = [Record(pages=self.pages, shape=shape, axes=axes, dtype=numpy.dtype(page0.dtype))]
        elif page0.is_shaped:
            shape = page0.tags['image_description'].value[7:-1]
            shape = tuple(int(i) for i in shape.split(','))
            series = [
             Record(pages=self.pages, shape=shape, axes='Q' * len(shape), dtype=numpy.dtype(page0.dtype))]
        if not series:
            shapes = []
            pages = {}
            for page in self.pages:
        ...
        if key is None:
            ...
            if self.is_nih:
                if pages[0].is_palette:
                    ...
                    result = stack_pages(pages, memmap=memmap, colormapped=False, squeeze=False)
            elif len(pages) == 1:
            ...
            if self.is_ome:
                ...
                if any(p is None for p in pages):
                    ...
                    nopage = numpy.zeros_like(firstpage.asarray(memmap=False))
                ...
                for page in pages:
                    keep.open(page)
                    if page:
                        a = page.asarray(memmap=False, colormapped=False, reopen=False)
                    else:
                        a = nopage
                    try:
    ...
    def _omeseries(self):
        ...
        for element in root:
            ...
            for pixels in element:
                ...
                result.append(Record(axes=axes, shape=shape, pages=ifds, dtype=numpy.dtype(dtype)))
    ...
    def __str__(self):
        ...
        result = [
         ...
         format_size(self._fh.size), {'<': 'little endian', '>': 'big endian'}[self.byteorder]]
        if self.is_bigtiff:
...
class TiffPage(object):
    ...
    def _fromfile(self):
        ...
        if self.is_lsm:
            ...
            tags['uic1tag'].value = Record(read_uic1tag(fh, byteorder, uic1tag.dtype, uic1tag.count, tags['uic2tag'].count))
        fh.seek(pos)
    ...
    def _process_tags(self):
        ...
        for code, (name, default, dtype, count, validate) in TIFF_TAGS.items():
            if not (name in tags or default is None):
                tags[name] = TiffTag(code, dtype=dtype, count=count, value=default, name=name)
        ...
        if self.is_stk:
            ...
            if self.is_contig:
                self._shape = (
                 planes, 1, 1, image_length, image_width,
                 samples_per_pixel)
                if samples_per_pixel == 1:
                    self.shape = (
                     planes, image_length, image_width)
                    self.axes = 'YX'
                else:
                    self.shape = (
                     planes, image_length, image_width,
                ...
                self._shape = (
                 planes, samples_per_pixel, 1, image_length,
                 image_width, 1)
                if samples_per_pixel == 1:
                    self.shape = (
                     planes, image_length, image_width)
                    ...
                    self.shape = (
                     planes, samples_per_pixel, image_length,
                     image_width)
                    self.axes = 'SYX'
            if planes == 1:
            ...
            if self.is_palette:
                ...
                if self.color_map.shape[1] >= 2 ** self.bits_per_sample:
                    if image_depth == 1:
                        self.shape = (
                         3, planes, image_length, image_width)
                    else:
                        self.shape = (
                         3, planes, image_depth, image_length,
                         image_width)
            ...
            if self.is_contig:
                self._shape = (
                 1, 1, image_depth, image_length, image_width,
                 samples)
            else:
                self._shape = (
                 1, samples, image_depth, image_length,
                 image_width, 1)
            if self.color_map.shape[1] >= 2 ** self.bits_per_sample:
                if image_depth == 1:
                    self.shape = (
                     3, image_length, image_width)
                    self.axes = 'CYX'
                else:
                    self.shape = (
                     3, image_depth, image_length, image_width)
                    self.axes = 'CZYX'
            else:
                warnings.warn('palette cannot be applied')
                self.is_palette = False
            ...
            if self.is_contig:
                ...
                if image_depth == 1:
                    self.shape = (
                     image_length, image_width, samples_per_pixel)
                    self.axes = 'YXS'
                else:
                    self.shape = (
                     image_depth, image_length, image_width,
                ...
                self._shape = (
                 1, samples_per_pixel, image_depth,
                 image_length, image_width, 1)
                if image_depth == 1:
                    self.shape = (
                     samples_per_pixel, image_length, image_width)
                    self.axes = 'SYX'
                else:
                    self.shape = (
                     samples_per_pixel, image_depth,
                     image_length, image_width)
                    self.axes = 'SZYX'
            ...
            self._shape = (
             1, 1, image_depth, image_length, image_width, 1)
            if image_depth == 1:
                self.shape = (
                 image_length, image_width)
                self.axes = 'YX'
    ...
    def asarray(self, squeeze=True, colormapped=True, rgbonly=False, scale_mdgel=False, memmap=False, reopen=True):
        ...
        if not self._shape:
                ...
                if bits_per_sample in (8, 16, 32, 64, 128):
                    ...
                    def unpack(x):
                        try:
                            ...
                            xlen = len(x) // (bits_per_sample // 8) * (bits_per_sample // 8)
                    ...
                    if self.compression == 'jpeg':
                        table = self.jpeg_tables if 'jpeg_tables' in self.tags else ''
                    ...
                    if self.is_tiled:
                        ...
                        for offset, bytecount in zip(offsets, byte_counts):
                            ...
                            result[0, pl, td:td + tile_depth, tl:tl + tile_length, tw:tw + tile_width, :] = tile
                            del tile
                            tw += tile_width
                            if tw >= shape[4]:
                        ...
                        strip_size = self.rows_per_strip * self.image_width * self.samples_per_pixel
            ...
            if self.predictor == 'horizontal' and not (self.is_tiled and not self.is_contiguous):
    ...
    def _is_memmappable(self, rgbonly, colormapped):
        ...
        if not self.parent.filehandle.is_file or not self.is_contiguous:
            return False
        return not (self.predictor or rgbonly and 'extra_samples' in self.tags or colormapped and self.is_palette or {'big': '>', 'little': '<'}[sys.byteorder] != self.parent.byteorder)
    ...
    def is_contiguous(self):
        ...
        if self.compression or self.bits_per_sample not in (8, 16, 32, 64):
            return
        if self.is_tiled:
            if self.image_width != self.tile_width or self.image_length % self.tile_length or self.tile_width % 16 or self.tile_length % 16:
                return
            if 'image_depth' in self.tags and 'tile_depth' in self.tags and (self.image_length != self.tile_length or self.image_depth % self.tile_depth):
                return
            offsets = self.tile_offsets
            byte_counts = self.tile_byte_counts
        else:
        ...
        if self.is_stk or all(offsets[i] + byte_counts[i] == offsets[(i + 1)] or byte_counts[(i + 1)] == 0 for i in range(len(offsets) - 1)):
    ...
    def __str__(self):
        ...
        s = (', ').join(s for s in (
         ...
         ('|').join(t[3:] for t in ('is_stk', 'is_lsm', 'is_nih', 'is_ome', 'is_imagej', 'is_micromanager',
                           'is_fluoview', 'is_mdgel', 'is_mediacy', 'is_sgi', 'is_reduced',
                           'is_tiled', 'is_contiguous') if getattr(self, t))) if s)
    ...
    def uic_tags(self):
        ...
        try:
            result.datetime_created = [ julian_datetime(*dt) for dt in zip(uic2tag.date_created, uic2tag.time_created)
                                      ]
            result.datetime_modified = [ julian_datetime(*dt) for dt in zip(uic2tag.date_modified, uic2tag.time_modified)
                                       ]
        except ValueError as e:
    ...
    def imagej_tags(self):
        ...
        if 'imagej_metadata' in tags:
            try:
                result.update(imagej_metadata(tags['imagej_metadata'].value, tags['imagej_byte_counts'].value, self.parent.byteorder))
    ...
    def is_contig(self):
        ...
        return 'planar_configuration' in self.tags and self.tags['planar_configuration'].value == 1
    ...
    def is_mediacy(self):
        ...
        return 'mc_id' in self.tags and self.tags['mc_id'].value.startswith('MC TIFF')
    ...
    def is_ome(self):
        ...
        return 'image_description' in self.tags and self.tags['image_description'].value.startswith('<?xml version=')

    @lazyattr
    def is_shaped(self):
        """True if page contains shape in image_description tag."""
        return 'image_description' in self.tags and self.tags['image_description'].value.startswith('shape=(')

    @lazyattr
    def is_imagej(self):
        """True if page contains ImageJ description."""
        return 'image_description' in self.tags and self.tags['image_description'].value.startswith('ImageJ=') or 'image_description_1' in self.tags and self.tags['image_description_1'].value.startswith('ImageJ=')
...
class TiffTag(object):
    ...
    def _fromfile(self, parent):
        ...
        if code not in CUSTOM_TAGS and code not in (273, 279, 324, 325):
            if len(value) == 1:
                value = value[0]
        if dtype.endswith('s') and isinstance(value, bytes) and self._type != 7:
...
class FileHandle(object):
    ...
    * Read numpy arrays and records from file like objects.
    ...
    """
    __slots__ = ('_fh', '_arg', '_mode', '_name', '_dir', '_offset', '_size', '_close',
                 'is_file')
    ...
    def open(self):
        ...
        if self._fh:
            ...
            if isinstance(self._arg, basestring):
                ...
                if isinstance(self._arg, FileHandle):
    ...
    def read_record(self, dtype, shape=1, byteorder=None):
        ...
        try:
            rec = numpy.rec.fromfile(self._fh, dtype, shape, byteorder=byteorder)
        except Exception:
            ...
            data = self._fh.read(size)
            return numpy.rec.fromstring(data, dtype, shape, byteorder=byteorder)
...
def read_uic2tag(fh, byteorder, dtype, plane_count):
    ...
    return {'z_distance': values[:, 0] / values[:, 1], 
       'date_created': values[:, 2], 
       'time_created': values[:, 3], 
       'date_modified': values[:, 4], 
       'time_modified': values[:, 5]}
...
def read_uic_tag(fh, tagid, plane_count, offset):
    ...
    if dtype is None:
        ...
        if '(' in dtype:
            ...
            if value.shape[(-1)] == 2:
                value = value[(Ellipsis, 0)] / value[(Ellipsis, 1)]
        else:
...
def read_micromanager_metadata(fh):
    ...
    results['index_map'] = {'channel': data[::5], 
       'slice': data[1::5], 'frame': data[2::5], 'position': data[3::5], 
       'offset': data[4::5]}
...
def imagej_metadata(data, bytecounts, byteorder):
    ...
    metadata_types = {'info': (
       ...
       'over': (
              'overlays', read_bytes)}
    metadata_types.update(dict((k[::-1], v) for k, v in metadata_types.items()))
    if not bytecounts:
        raise ValueError('no ImageJ metadata')
    if data[:4] not in ('IJIJ', 'JIJI'):
...
def decodejpg(encoded, tables='', photometric=None, ycbcr_subsampling=None, ycbcr_positioning=None):
...
def decodelzw(encoded):
    ...
    switchbitch = {255: (
       ...
       2047: (
            12, 20, int('111111111111' + '00000000000000000000', 2))}
...
def stripascii(string):
    r"""Return string truncated at last byte that is 7bit ASCII.
...
def julian_datetime(julianday, milisecond=0):
    ...
    if julianday <= 1721423:
        ...
        hour, milisecond = divmod(milisecond, 3600000)
        minute, milisecond = divmod(milisecond, 60000)
...
UIC_TAGS = {0: (
   ...
   27: (
      'wavelength', int), 
   28: (
      'stage_position', '(%i,2,2)u4'), 
   29: (
      'camera_chip_offset', '(%i,2,2)u4'), 
   30: (
      'overlay_mask', None), 
   31: (
...
CZ_LSM_INFO = [
 ...
 (
  'reserved_2', '21u4')]
CZ_LSM_INFO_READERS = {'scan_info': read_cz_lsm_scan_info, 
...
CZ_DATA_TYPES = {0: 'varying data types', 
   ...
   5: '32 bit float'}
CZ_TYPE_OF_DATA = {0: 'Original scan data', 
   1: 'Calculated data', 
   2: '3D reconstruction', 
   3: 'Topography height map'}
CZ_LSM_SCAN_INFO_ARRAYS = {536870912: 'tracks', 
   805306368: 'lasers', 
   1610612736: 'detection_channels', 
   2147483648: 'illumination_channels', 
   2684354560: 'beam_splitters', 
   3221225472: 'data_channels', 
   285212672: 'timers', 
   318767104: 'markers'}
CZ_LSM_SCAN_INFO_STRUCTS = {1073741824: 'track', 
   1342177280: 'laser', 
   1879048192: 'detection_channel', 
   2415919104: 'illumination_channel', 
   2952790016: 'beam_splitter', 
   3489660928: 'data_channel', 
   301989888: 'timer', 
   335544320: 'marker'}
CZ_LSM_SCAN_INFO_ATTRIBUTES = {268435457: 'name', 
   ...
   268435527: 'user', 
   268435528: 'use_bc_correction', 
   268435529: 'position_bc_correction1', 
   268435536: 'position_bc_correction2', 
   ...
   268435545: 'rt_binning', 
   268435546: 'rt_frame_width', 
   268435547: 'rt_frame_height', 
   268435548: 'rt_region_width', 
   268435549: 'rt_region_height', 
   268435550: 'rt_offset_x', 
   268435551: 'rt_offset_y', 
   268435552: 'rt_zoom', 
   268435553: 'rt_line_period', 
   268435554: 'prescan', 
   268435555: 'scan_direction_z', 
   ...
   1879048212: 'channel_name', 
   1879048213: 'detector_gain_bc1', 
   1879048214: 'detector_gain_bc2', 
   1879048215: 'amplifier_gain_bc1', 
   1879048216: 'amplifier_gain_bc2', 
   1879048217: 'amplifier_offset_bc1', 
   1879048224: 'amplifier_offset_bc2', 
   1879048225: 'spectral_scan_channels', 
   1879048226: 'spi_wavelength_start', 
   1879048227: 'spi_wavelength_stop', 
   1879048230: 'dye_name', 
   1879048231: 'dye_folder', 
   2415919105: 'name', 
   2415919106: 'power', 
...
TIFF_TAGS = {254: (
   ...
   255: (
       'subfile_type', None, 3, 1, {0: 'undefined', 1: 'image', 2: 'reduced_image', 3: 'page'}), 
   256: (
       'image_width', None, 4, 1, None), 
   257: (
       'image_length', None, 4, 1, None), 
   258: (
       'bits_per_sample', 1, 3, 1, None), 
   259: (
   ...
   269: (
       'document_name', None, 2, None, None), 
   270: (
       'image_description', None, 2, None, None), 
   271: (
       'make', None, 2, None, None), 
   272: (
       'model', None, 2, None, None), 
   273: (
       'strip_offsets', None, 4, None, None), 
   274: (
       'orientation', 1, 3, 1, TIFF_ORIENTATIONS), 
   277: (
       'samples_per_pixel', 1, 3, 1, None), 
   278: (
       'rows_per_strip', 2 ** 32 - 1, 4, 1, None), 
   279: (
       'strip_byte_counts', None, 4, None, None), 
   280: (
       'min_sample_value', None, 3, None, None), 
   281: (
       'max_sample_value', None, 3, None, None), 
   282: (
       'x_resolution', None, 5, 1, None), 
   283: (
       'y_resolution', None, 5, 1, None), 
   284: (
       'planar_configuration', 1, 3, 1, {1: 'contig', 2: 'separate'}), 
   285: (
       'page_name', None, 2, None, None), 
   286: (
       'x_position', None, 5, 1, None), 
   287: (
       'y_position', None, 5, 1, None), 
   296: (
       'resolution_unit', 2, 4, 1, {1: 'none', 2: 'inch', 3: 'centimeter'}), 
   297: (
       'page_number', None, 3, 2, None), 
   305: (
       'software', None, 2, None, None), 
   306: (
       'datetime', None, 2, None, None), 
   315: (
       'artist', None, 2, None, None), 
   316: (
       'host_computer', None, 2, None, None), 
   317: (
       'predictor', 1, 3, 1, {1: None, 2: 'horizontal'}), 
   318: (
       'white_point', None, 5, 2, None), 
   319: (
       'primary_chromaticities', None, 5, 6, None), 
   320: (
       'color_map', None, 3, None, None), 
   322: (
       'tile_width', None, 4, 1, None), 
   323: (
       'tile_length', None, 4, 1, None), 
   324: (
       'tile_offsets', None, 4, None, None), 
   325: (
       'tile_byte_counts', None, 4, None, None), 
   338: (
       'extra_samples', None, 3, None, {0: 'unspecified', 1: 'assocalpha', 2: 'unassalpha'}), 
   339: (
       'sample_format', 1, 3, 1, TIFF_SAMPLE_FORMATS), 
   340: (
       'smin_sample_value', None, None, None, None), 
   341: (
       'smax_sample_value', None, None, None, None), 
   347: (
       'jpeg_tables', None, 7, None, None), 
   530: (
       'ycbcr_subsampling', 1, 3, 2, None), 
   531: (
       'ycbcr_positioning', 1, 3, 1, None), 
   32996: (
         'sgi_matteing', None, None, 1, None), 
   32996: (
         'sgi_datatype', None, None, 1, None), 
   32997: (
         'image_depth', None, 4, 1, None), 
   32998: (
         'tile_depth', None, 4, 1, None), 
   33432: (
         'copyright', None, 1, None, None), 
   33445: (
         'md_file_tag', None, 4, 1, None), 
   33446: (
         'md_scale_pixel', None, 5, 1, None), 
   33447: (
         'md_color_table', None, 3, None, None), 
   33448: (
         'md_lab_name', None, 2, None, None), 
   33449: (
         'md_sample_info', None, 2, None, None), 
   33450: (
         'md_prep_date', None, 2, None, None), 
   33451: (
         'md_prep_time', None, 2, None, None), 
   33452: (
         'md_file_units', None, 2, None, None), 
   33550: (
         'model_pixel_scale', None, 12, 3, None), 
   33922: (
         'model_tie_point', None, 12, None, None), 
   34665: (
         'exif_ifd', None, None, 1, None), 
   34735: (
         'geo_key_directory', None, 3, None, None), 
   34736: (
         'geo_double_params', None, 12, None, None), 
   34737: (
         'geo_ascii_params', None, 2, None, None), 
   34853: (
         'gps_ifd', None, None, 1, None), 
   37510: (
         'user_comment', None, None, None, None), 
   42112: (
         'gdal_metadata', None, 2, None, None), 
   42113: (
         'gdal_nodata', None, 2, None, None), 
   50289: (
         'mc_xy_position', None, 12, 2, None), 
   50290: (
         'mc_z_position', None, 12, 1, None), 
   50291: (
         'mc_xy_calibration', None, 12, 3, None), 
   50292: (
         'mc_lens_lem_na_n', None, 12, 3, None), 
   50293: (
         'mc_channel_name', None, 1, None, None), 
   50294: (
         'mc_ex_wavelength', None, 12, 1, None), 
   50295: (
         'mc_time_stamp', None, 12, 1, None), 
   50838: (
         'imagej_byte_counts', None, None, None, None), 
   65200: (
         'flex_xml', None, 2, None, None)}
CUSTOM_TAGS = {700: (
       'xmp', read_bytes), 
   34377: (
   ...
   33723: (
         'iptc', read_bytes), 
   34675: (
         'icc_profile', read_bytes), 
   33628: (
         'uic1tag', read_uic1tag), 
   33629: (
         'uic2tag', read_uic2tag), 
   33630: (
         'uic3tag', read_uic3tag), 
   33631: (
         'uic4tag', read_uic4tag), 
   34361: (
   ...
   40100: (
         'mc_id_old', read_bytes), 
   50288: (
         'mc_id', read_bytes), 
   50296: (
         'mc_frame_properties', read_bytes), 
   50839: (
         'imagej_metadata', read_bytes), 
   51123: (
         'micromanager_metadata', read_json)}
PRINT_LINE_LEN = 79

def imshow(data, title=None, vmin=0, vmax=None, cmap=None, bitspersample=None, photometric='rgb', interpolation='nearest', dpi=96, figure=None, subplot=111, maxdim=8192, **kwargs):
    ...
    if dims < 2:
        ...
        elif not isrgb and data.shape[(-1)] < data.shape[(-2)] // 16 and data.shape[(-1)] < data.shape[(-3)] // 16 and data.shape[(-1)] < 5:
    ...
    if figure is None:
        ...
        figure = pyplot.figure(dpi=dpi, figsize=(10.3, 6.3), frameon=True, facecolor='1.0', edgecolor='w')
        try:
            figure.canvas.manager.window.title(title)
        except Exception:
            pass

        pyplot.subplots_adjust(bottom=0.03 * (dims + 2), top=0.9, left=0.1, right=0.95, hspace=0.05, wspace=0.0)
    ...
    if cmap is None:
        ...
        if photometric == 'miniswhite':
            cmap += '_r'
    image = pyplot.imshow(data[((0, ) * dims)].squeeze(), vmin=vmin, vmax=vmax, cmap=cmap, interpolation=interpolation, **kwargs)
    if not isrgb:
        pyplot.colorbar()
...
def main(argv=None):
    ...
    if float(sys.version[0:3]) < 2.6:
        ...
        if argv is None:
            argv = sys.argv
        import optparse
        parser = optparse.OptionParser(usage='usage: %prog [options] path', description='Display image data in TIFF files.', version='%%prog %s' % __version__)
        ...
        opt('--nomultifile', dest='nomultifile', action='store_true', default=False, help="don't read OME series from multiple files")
        ...
        opt('--doctest', dest='doctest', action='store_true', default=False, help='runs the docstring examples')
        ...
        for i, page in images:
            ...
            for attr in ('cz_lsm_info', 'cz_lsm_scan_info', 'uic_tags', 'mm_header',
                         'imagej_tags', 'micromanager_metadata', 'nih_image_header'):
                    ...
                    if page.is_stk:
                        try:
                            ...
                            if vmax <= vmin:
                                vmin, vmax = (None, None)
                    title = '%s\n %s' % (str(tif), str(page))
                    imshow(img, title=title, vmin=vmin, vmax=vmax, bitspersample=page.bits_per_sample, photometric=page.photometric, interpolation=settings.interpol, dpi=settings.dpi)
