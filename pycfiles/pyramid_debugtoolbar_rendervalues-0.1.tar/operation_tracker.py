...
# Embedded file name: build/bdist.macosx-10.10-intel/egg/pyramid_debugtoolbar_mongo/operation_tracker.py
# Compiled at: 2014-12-29 21:14:01
import functools, time, inspect, os, pymongo, pymongo.collection, pymongo.cursor, pyramid
from pyramid.threadlocal import get_current_request
__all__ = [
 'queries', 'inserts', 'updates', 'removes', 'install_tracker',
 'uninstall_tracker', 'reset']
_original_methods = {'insert': pymongo.collection.Collection.insert, 
...
def _get_stacktrace():
    ...
    if get_trace:
        ...
        try:
            ...
            return [('', 0, 'Error retrieving stack', 'Could not retrieve stack. IndexError exception occured in inspect.stack().')]
...
def _insert(collection_self, doc_or_docs, manipulate=True, safe=False, check_keys=True, **kwargs):
    global inserts
    start_time = time.time()
    result = _original_methods['insert'](collection_self, doc_or_docs, manipulate=manipulate, safe=safe, check_keys=check_keys, **kwargs)
    total_time = (time.time() - start_time) * 1000
    __traceback_hide__ = True
    inserts.append({'document': doc_or_docs, 
       'safe': safe, 
       'time': total_time, 
...
def _update(collection_self, spec, document, upsert=False, maniuplate=False, safe=False, multi=False, **kwargs):
    global updates
    start_time = time.time()
    result = _original_methods['update'](collection_self, spec, document, upsert=upsert, safe=safe, multi=multi, **kwargs)
    total_time = (time.time() - start_time) * 1000
    __traceback_hide__ = True
    updates.append({'document': document, 
       'upsert': upsert, 
       'multi': multi, 
       'spec': spec, 
       'safe': safe, 
...
def _remove(collection_self, spec_or_id, safe=False, **kwargs):
    global removes
    start_time = time.time()
    result = _original_methods['remove'](collection_self, spec_or_id, safe=safe, **kwargs)
    total_time = (time.time() - start_time) * 1000
    removes.append({'spec_or_id': spec_or_id, 
       'safe': safe, 
       'time': total_time, 
       'stack_trace': _get_stacktrace()})
    return result
