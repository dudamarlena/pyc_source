...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/src/ac/ac.py
...
parser.add_argument('-v', '--verbose', dest='verbose', default=False, help='Turn on verbose output', action='store_true')
parser.add_argument('-c', '--compiler', dest='compiler', default='g++', help='Default C/C++ Compiler to use', action='store')
parser.add_argument('--nocolor', dest='nocolor', default=False, action='store_true', help='Turn this on to disable output coloring. This is useful if your output looks garbled, or you find colors more annoying than helpful.')
parser.add_argument('--width', default=80, type=int, dest='width', action='store', help='Control the width of the output.')
parser.add_argument('--shell', default='bash', type=str, dest='shell', action='store', help='The shell to use when executing fail alternative scripts.')
parser.add_argument('--yes', default=False, action='store_true', help='Automatically answer yes to all questions.')
...
def test(testname, result, pass_required=False):
    ...
    if not result:
        color = COLORS.FAIL
        write = sys.stderr
    write.write(format_output(colorize(color, '%s OK?' % testname), colorize(color, ' %s\n' % result)))
    if not result and pass_required:
        write.write(colorize(COLORS.WARNING, 'Success is required before configuration can continue. Aborting.\n'))
        sys.exit()
...
def testheader(header, args=None, required=False):
    ...
    test(header, not result, required)
...
def testlib(libname, func=None, header=None, args=None, required=False):
    ...
    result = get_shell_output(cmd)
    test(libname, not result, required)
