...
# Embedded file name: build/bdist.linux-x86_64/egg/compose_flow/kube/mixins.py
# Compiled at: 2020-05-06 10:05:57
# Size of source mod 2**32: 23396 bytes
__doc__ = '\nCompose subcommand\n'
import base64
from functools import lru_cache
import os, pathlib, sh, shutil, typing
...
class KubeMixIn(object):
    ...
    def _check_rancher_namespace(self):
        ...
        If not found, attempt to create it.
        """
        namespaces = self.execute('rancher namespace ls --quiet').stdout.decode('utf8').strip().split('\n')
    ...
    def _get_secret(self, name: str):
        return self.execute(f"{self.kubectl_command} get secrets --namespace {self.namespace} -o yaml {name}")
    ...
    def switch_kube_context(self):
        ...
        try:
            ...
            raise errors.InvalidTargetCluster("No context is defined for profile {}!\n\nPlease specify a corresponding context in your kubeconfig file or map this profile name to an existing context in the 'kubecontexts' section of compose-flow.yml".format(profile_name))
    ...
    def cluster_name(self):
        ...
        if profile_name in EXCLUDE_PROFILES:
            raise errors.InvalidTargetCluster("Invalid profile '{0}' for default cluster logic - please specify an explicit cluster mapping in compose-flow.yml and use a profile other than '{0}'".format(profile_name))
    ...
    def project_name(self):
        ...
        raise errors.MissingRancherProject('ERROR: You must configure a Rancher project in compose-flow.yml or .compose/config.yml in order to use Rancher as a backend or deployment target!')
    ...
    def get_app_deploy_command(self, app: dict, target: str='rancher') -> str:
        ...
        if answers_path:
            ...
            if answers_path:
                if target == 'helm':
                    self.logger.warning('Helm only supports nested values - please switch from answers to values in compose-flow.yml')
    ...
    def get_helm_app_install_command(self, app_name: str, rendered_path: str, namespace: str, chart: str, version: str, use_answers: bool=False):
        return f"helm install --name {app_name} -f {rendered_path} --namespace {namespace} --version {version} {chart}"

    def get_helm_app_upgrade_command(self, app_name: str, rendered_path: str, chart: str, version: str, use_answers: bool=False):
    ...
    def get_rancher_app_install_command(self, app_name: str, rendered_path: str, namespace: str, chart: str, version: str, use_answers: bool):
    ...
    def get_rancher_app_upgrade_command(self, app_name: str, rendered_path: str, chart: str, version: str, use_answers: bool):
    ...
    def get_manifest_filename(self, manifest_path: str) -> str:
        escaped_path = manifest_path.replace('../', '').replace('./', '').replace('/', '-').replace('.yaml', '.yml').rstrip('-')
    ...
    def render_single_yaml(self, input_path: str, output_path: str, checker: BaseChecker=None, raw: bool=False) -> None:
        """
    ...
    def render_nested_manifests(self, dir_path: str, raw: bool) -> str:
        ...
        for manifest in manifests:
            render_dest = os.path.join(rendered_path, str(manifest).lstrip(dir_path))
            print(render_dest)
            parent_dest = os.path.dirname(render_dest)
            os.makedirs(parent_dest, mode=488, exist_ok=True)
