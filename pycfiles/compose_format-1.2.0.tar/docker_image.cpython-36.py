...
# Embedded file name: build/bdist.linux-x86_64/egg/compose_flow/docker_image.py
...
OFFICIAL_RELEASE_REGEX = re.compile('^\\d+\\.\\d+\\.\\d+$')
...
class DockerImage:
    ...
    def _parse_tagged_image_name(self):
        ...
        self.name = re.match(f"{self.repository}/(.*):{self.tag}", self._tagged_image_name).group(1)
        try:
    ...
    def publish_with_major_minor_tags(self):
        ...
        if self.official_release:
            ...
            raise PublishMajorMinorTagsError(f"Publishing with auto tags is only allowed for official release MAJOR.MINOR.PATCH tags.Current tag: {self.version_info}")
