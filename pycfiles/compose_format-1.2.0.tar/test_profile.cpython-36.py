...
# Embedded file name: build/bdist.linux-x86_64/egg/tests/test_profile.py
# Compiled at: 2020-05-06 10:05:57
# Size of source mod 2**32: 8457 bytes
...
class ProfileTestCase(TestCase):
    ...
    def test_check_no_constraints(self, *mocks):
        ...
        errors = profile._check_services(profile.check_constraints, profile.data)
        self.assertRegex(' '.join(errors), 'constraints not found')
    ...
    def test_check_no_node_constraints(self, *mocks):
        ...
        errors = profile._check_services(profile.check_constraints, profile.data)
        self.assertRegex(' '.join(errors), 'node constraints not found')
    ...
    def test_check_no_resources(self, *mocks):
        ...
        self.assertRegex(' '.join(errors), 'resource constraints not found')
    ...
    def test_expand_services(self, *mocks):
        data = {'services':{'foo': {'build':'..', 
                  ...
                  'ports':[
                   '8000:8000'], 
                  'deploy':{'replicas': 3}}}, 
         'compose_flow':{'expand': {'foo': {'increment': {'env':[
                                            'SPARK_WORKER_PORT', 'SPARK_WORKER_WEBUI_PORT'], 
                                           'ports':{'source_port':True, 
        ...
        self.assertEqual([x['ports'] for x in new_data['services'].values()], [
         [
          '8000:8000'], ['8001:8001'], ['8002:8002']])
        self.assertEqual([x['environment'] for x in new_data['services'].values()], [
         [
    ...
    def test_set_resources_memory_limit_no_reservation(self, *mocks):
        ...
        profile = Profile(self.workflow)
        param = {}
        content = profile._compile(param)
