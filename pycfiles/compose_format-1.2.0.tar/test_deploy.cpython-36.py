...
# Embedded file name: build/bdist.linux-x86_64/egg/tests/test_deploy.py
...
class DeployTestCase(BaseTestCase):
    ...
    @mock.patch('compose_flow.commands.subcommands.profile.Profile.data',
    ...
    def test_checks_called(self, *mocks):
        ...
        profile_data_mock.return_value = {'services': {'app': {'image': 'foo:test'}}}
        command = shlex.split('-e dev deploy')
        workflow = Workflow(argv=command)
        workflow.environment.write = mock.Mock()
    ...
    @mock.patch('compose_flow.commands.subcommands.deploy.Deploy.get_apps',
      return_value=[])
    @mock.patch('compose_flow.commands.subcommands.deploy.Deploy.get_rancher_manifests',
      return_value=[])
    @mock.patch('compose_flow.commands.subcommands.deploy.Deploy.get_rancher_namespaces',
      return_value=[
     'duplicate-namespace', 'my-namespace'])
    @mock.patch('compose_flow.commands.subcommands.deploy.Deploy.list_rancher_namespaces',
      return_value=[
     'duplicate-namespace', 'other-namespace'])
