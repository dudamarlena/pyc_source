...
# Embedded file name: build/bdist.linux-x86_64/egg/compose_flow/kube/checks.py
# Compiled at: 2020-05-06 10:05:57
# Size of source mod 2**32: 8126 bytes
__doc__ = 'Define checks to be run against Kubernetes YAML'
from abc import ABC
import logging
...
class BaseChecker(ABC):
    ...
    def _get_all_checks(self):
        ...
        check_list = [getattr(self, func) for func in dir(self.__class__) if callable(getattr(this_class, func)) if func.startswith(self.check_prefix)]
        return check_list
...
class ManifestChecker(BaseChecker):
    ...
    def _check_manifest_ingress_annotations(self, documents: list) -> str:
        ...
        missing_metadata_msg = 'Ingress resources MUST specify a metadata section includingname, namespace, and appropriate annotations!'
        ingress_error_msg = 'Ingress resources MUST specify a scheme annotation to avoid unintentionally exposing a service to the public Internet!'
        public_ingress_warning = 'An ingress resource in this deployment is internet-facing!\n\nPlease ensure you are deploying to a production cluster and you intend to make your services PUBLICLY accessible!'
        ...
        for doc in documents:
            ...
            if kind in JOB_TEMPLATE_RESOURCES:
                ...
                if kind in POD_TEMPLATE_RESOURCES:
                    ...
                    missing_resources_msg = f"All containers and initContainers in a {kind}must define resource constraints!"
...
class AnswersChecker(BaseChecker):
    ...
    def _check_answers_flat_resources(self, documents: list) -> str:
        ...
        if not has_resources:
            self.logger.warning('Helm answers do not contain resource constraints! Please verify that the specified chart has default resources defined for all containers!')
        else:
