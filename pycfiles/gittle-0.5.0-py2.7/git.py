...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/gittle/utils/git.py
# Compiled at: 2014-06-26 14:06:14
import os
...
def is_readable_change(store):
    ...
    def fn(change):
        return all(map(is_readable(store), change))
...
def changes_to_pairs(changes):
    return [ ((oldpath, oldmode, oldsha), (newpath, newmode, newsha)) for (oldpath, newpath), (oldmode, newmode), (oldsha, newsha) in changes
           ]


def _diff_pairs(object_store, pairs, diff_func, diff_type='text'):
    for old, new in pairs:
        yield {'diff': diff_func(object_store, old, new), 'new': change_to_dict(new), 'old': change_to_dict(old), 
           'type': diff_type}
...
def changes_to_blobs(object_store, basepath, pairs):
    return [ (obj_blob(object_store, old), path_blob(basepath, new)) for old, new in pairs
           ]
...
def blob_from_path(basepath, path):
    ...
    return (
     path, os.stat(fullpath).st_mode, blob)
...
def subrefs(refs_dict, base):
    ...
    base = base or ''
    keys = refs_dict.keys()
    subkeys = map(partial(subkey, base), keys)
    key_pairs = zip(keys, subkeys)
    return {newkey:refs_dict[oldkey] for oldkey, newkey in key_pairs if newkey}


def clean_refs(refs):
    return {ref:sha for ref, sha in refs.items() if not ref.endswith('^{}') if not ref.endswith('^{}')}