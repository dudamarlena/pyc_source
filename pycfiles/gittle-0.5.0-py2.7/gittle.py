...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/gittle/gittle.py
# Compiled at: 2015-01-01 10:58:01
...
class Gittle(object):
    ...
    DEFAULT_USER_INFO = {'name': None, 
       'email': None}
    DIFF_FUNCTIONS = {'classic': utils.git.classic_tree_diff, 
    ...
    MODE_DIRECTORY = 16384
    MAX_TREE_DEPTH = 1000
    ROOT_PATHS = (
     os.path.curdir, os.path.sep)
    ...
    def _commit(self, committer=None, author=None, message=None, files=None, tree=None, *args, **kwargs):
        ...
        return self.repo.do_commit(message=message, author=author_msg, committer=committer_msg, encoding='UTF-8', tree=tree, *args, **kwargs)
    ...
    def lookup_entry(self, relpath, trackable_files=set()):
        if relpath not in trackable_files:
            raise KeyError
        abspath = self.abspath(relpath)
        with open(abspath, 'rb') as (git_file):
            data = git_file.read()
            s = sha1()
            s.update('blob %u\x00' % len(data))
            s.update(data)
        return (
         s.hexdigest(), os.stat(abspath).st_mode)
    ...
    def _changed_entries_by_pattern(self, pattern):
        ...
        filtered_paths = [ funky.first_true(names) for names, modes, sha in changed_entries if tuple(map(bool, names)) == pattern and funky.first_true(names)
                         ]
    ...
    def pending_files_by_state(self):
        files = {'modified': self.modified_files, 
    ...
    def diff_working(self, ref=None, filter_binary=True):
        ...
        """
        return utils.git.diff_changes_paths(self.repo.object_store, self.path, self._changed_entries(ref=ref), filter_binary=filter_binary)
    ...
    def get_commit_files(self, commit_sha, parent_path=None, is_tree=None, paths=None):
        ...
        for entry in tree.items():
            if entry.mode == self.MODE_DIRECTORY:
                context.update(self.get_commit_files(entry.sha, parent_path=os.path.join(parent_path, entry.path), is_tree=True, paths=paths))
                continue
            subpath = os.path.join(parent_path, entry.path)
            if not (paths is None or subpath in paths):
                continue
            context[subpath] = {'name': entry.path, 
               'path': subpath, 
               'mode': entry.mode, 
               'sha': entry.sha, 
    ...
    def remotes(self):
        ...
        return {keys[1]:values['url'] for keys, values in config.items() if keys[0] == 'remote' if keys[0] == 'remote'}
    ...
    def remove_ref(self, ref_name):
        if ref_name not in self.repo.refs:
            return False
