...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\daversy\state.py
# Compiled at: 2016-01-15 00:48:19
...
class FileState:
    ...
    def load(self, location, filters={}):
        ...
        return self._load_element(document.getroot(), db.__state__, filters)
        ...
        if hasattr(builder, 'PropertyList'):
            for key, value in builder.PropertyList.items():
                if not value.exclude and object[value.name]:
                    ...
                    elif not hasattr(object, 'SubElements'):
        ...
        if not self.can_load(location):
            ...
            for builder in db.__builders__:
                ...
                if hasattr(builder, 'Query'):
                    self._load_object(connection.cursor(), state, builder, filters.get(builder.XmlTag))
