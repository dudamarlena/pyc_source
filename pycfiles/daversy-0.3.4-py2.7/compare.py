...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\daversy\command\compare.py
...
class Compare(Command):
    ...
    __options__ = [
     ...
     make_option('--html', dest='html', action='store_true', help='generate a HTML inline difference report'),
     make_option('--context', dest='lines', default=None, type='int', help='number of context lines (default: all lines included).')]
    ...
    def print_html_diff(self, source, target, context):
        ...
        for op, path in self.diff:
            ...
            if len(path) == 1 and op == 'A':
                table = difflib_ext.make_table(get_id(target), name, [], self.get_sql(target), context)
                if table:
                    headers.append(get_link('add', target))
                    diffs.append(table)
            elif len(path) == 1 and op == 'D':
                table = difflib_ext.make_table(get_id(src), name, self.get_sql(src), [], context)
                if table:
                    headers.append(get_link('rem', src))
                    diffs.append(table)
            elif not seen.has_key((src.__class__, name)):
                table = difflib_ext.make_table(get_id(target), name, self.get_sql(src), self.get_sql(target), context)
