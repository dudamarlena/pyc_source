...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/single_file_module-project/sfm/winzip.py
# Compiled at: 2019-04-21 23:27:02
r"""
...
比如你有一个路径 ``C:\download
eadme.txt``, 如果当前路径是 ``C:\``,
而此时你将 ``readme.txt`` 添加到压缩包时则是在压缩包内添加一个: ``download
eadme.txt``,
...
def zip_a_folder(src, dst):
    u"""Add a folder and everything inside to zip archive.
...
def zip_everything_in_a_folder(src, dst):
    u"""Add everything in a folder except the root folder it self to zip archive.
...
def zip_many_files(list_of_abspath, dst):
    u"""Add many files to a zip archive.
