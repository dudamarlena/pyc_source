...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/single_file_module-project/sfm/iterable.py
# Compiled at: 2019-04-21 23:17:39
"""
...
if sys.version_info[0] == 2:
    from itertools import ifilterfalse as filterfalse, izip_longest as zip_longest, izip as zip
    string_types = (
     basestring,)
else:
...
def flatten(iterable):
    u"""Flatten one layer of nesting.
...
def flatten_all(nested_iterable):
    u"""Flatten arbitrary depth of nesting. Good for unknown nesting structure
    iterable object.
...
def take(iterable, n):
    u"""Return first n items of the iterable as a list.
...
def pull(iterable, n):
    u"""Return last n items of the iterable as a list.
...
def shuffled(iterable):
    u"""Returns the shuffled iterable.
...
def grouper_list(l, n):
    u"""Evenly divide list into fixed-length piece, no filled value if chunk
...
def grouper_dict(d, n, dict_type=dict):
    u"""Evenly divide dictionary into fixed-length piece, no filled value if
...
def running_window(iterable, size):
    u"""Generate n-size running window.
...
def cycle_running_window(iterable, size):
    u"""Generate n-size cycle running window.
...
def cycle_slice(sliceable, start, end):
    u"""Given a list, return right hand cycle direction slice from start to end.
...
def cycle_dist(x, y, perimeter):
    u"""Find Distance between x, y by means of a n-length cycle.
...
def size_of_generator(generator, memory_efficient=True):
    u"""Get number of items in a generator function.
