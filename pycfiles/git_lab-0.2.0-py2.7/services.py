...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/git_lab/apis/mergerequest/services.py
# Compiled at: 2014-03-14 05:17:53
...
class ReferService(object):
    ...
    def show_merge_request(self, req_id):
        ...
        if mr is not None:
            merge_request = ('\n[ {state} ] #{iid} {title}\n--------------------------------------------------------------------------------\nmerge request id  : {id}\n\nauthor            : {author}\nassignee          : {assignee}\nopened at         : {opened_at}\n\nupvotes/downbotes : +{up} / -{down}\n\n[ merge to ] : {target} <----- [ merge from ] {source}\n--------------------------------------------------------------------------------').format(state=mr.get_state(), iid=mr.get_iid(), title=mr.get_title(), id=mr.get_id(), author=mr.get_author().get_name(), assignee=mr.get_assignee().get_name(), opened_at=mr.get_author().get_created_at(), up=mr.get_upvotes(), down=mr.get_downvotes(), target=mr.get_target_branch(), source=mr.get_source_branch())
            print merge_request
            notes = self.repo.get_notes(req_id)
            for note in notes:
                n = ('\n    {author} posted at {posted_at}\n    ----------------------------------------------------------------------------\n    {body}').format(author=note.get_author().get_name(), posted_at=note.get_created_at(), body=note.get_body())
                print n
    ...
    def list_merge_request(self, page, count):
        ...
        if len(mrs) != 0:
            ...
            print header
        for mr in mrs:
            m = (' #{id: <7} #{mid: <7} {state: ^8} {author: ^10} {title: <40}').format(id=mr.get_iid(), mid=mr.get_id(), state=mr.get_state(), author=mr.get_author().get_name(), title=mr.get_title())
            print m
...
class EditService(object):
    ...
    def parse_dest_spec(dest):
        ...
        if dest is None:
            return {'dest_project': get_project(), 
               'dest_branch': 'master'}
        else:
            if ':' in dest:
                proj, br = dest.split(':', 2)
                project = replace(proj, '/', '%2F') if proj != '' else get_project()
                branch = br if br != '' else 'master'
                return {'dest_project': project, 
                   'dest_branch': branch}
            else:
                if ':' not in dest:
                    return {'dest_project': replace(dest, '/', '%2F'), 
                       'dest_branch': 'master'}
                project, branch = dest.split(':', 2)
                return {'dest_project': replace(project, '/', '%2F'), 
                   'dest_branch': branch}
