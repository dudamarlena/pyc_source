...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/git_lab/utils.py
# Compiled at: 2014-03-14 05:11:34
from gitlab import Gitlab

class GitlabClient(Gitlab):
    """元のクライアントが対応してないAPIに対応する用"""
    ...
    def getmergerequestnotes(self, project_id, mergerequest_id):
        ...
        url_str = ('{0}/{1}/merge_requests/{2}/notes').format(self.projects_url, project_id, mergerequest_id)
    ...
    def createmergerequest2(self, project_id, sourcebranch, targetprojectid, targetbranch, title, assignee_id=None, sudo=''):
        """
        ...
        data = {'source_branch': sourcebranch, 'target_project_id': targetprojectid, 
           'target_branch': targetbranch, 
           'title': title, 
           'assignee_id': assignee_id}
        if sudo != '':
            data['sudo'] = sudo
        request = requests.post(url_str, data=data, headers=self.headers, verify=self.verify_ssl)
...
def get_client():
    ...
    return GitlabClient(get_or_rise('gitlab.host'), get_or_rise('gitlab.private-token'))
