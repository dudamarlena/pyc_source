...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pysvnmanager/hooks/init/hook1.5/scripts/svn_mirror.py
# Compiled at: 2010-09-24 12:44:56
"""Svnsync program
...
"""
import commands, sys, os, getopt, re, logging, logging.handlers
...
def parse_options(argv=None):
    try:
        (opts, args) = getopt.getopt(argv, 'hvqPR:r:u:p:U:l:f:', [
...
def main(argv=None):
    ...
    try:
        ...
        if opts.args:
            ...
            do_sync_repos(opts.repo, opts.rev, opts.urls, opts.username, opts.password, opts.prop)
    except Exception, e:
...
def check_repos(repos_root):
    for repos in find_repos(repos_root):
        ...
        print '        ' + 'rev      : %-8s, uuid     : %s' % (svninfo.rev, svninfo.uuid)
        if repo_type == 'slave':
            print '        ' + 'sync_last: %-8s, sync_from: %s' % (svninfo.sync_last_merged_rev, svninfo.sync_from_url)
            if svninfo.sync_lock:
                print '        **LOCKED** !'
        elif repo_type == 'master':
            for mirror in mirrors_info:
                print '     ' + '>> %s' % mirror.url
                print '     ' + '   rev      : %-8s, uuid     : %s' % (mirror.rev, mirror.uuid)
                print '     ' + '   sync_last: %-8s, sync_from: %s' % (mirror.sync_last_merged_rev, mirror.sync_from_url)
                if mirror.sync_lock:
                    print '        **LOCKED** !'
