...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pysvnmanager/model/rcsbackup.py
# Compiled at: 2010-08-09 03:53:44
...
def backup(wcfile, comment='', user=''):
    ...
    if not is_rcs_exist(wcfile):
        cmd.append('%(cmd)s -i -q -l -t-"%(msg)s" -w"%(user)s" "%(file)s" 2>&1' % {'cmd': CMD_CI, 'file': wcfile, 'msg': comment, 'user': user})
        cmd.append('%(cmd)s -U -u -M -q "%(file)s"' % {'cmd': CMD_RCS, 'file': wcfile})
    else:
        cmd.append('%(cmd)s -q -u -m"%(msg)s" -w"%(user)s" "%(file)s" 2>&1' % {'cmd': CMD_CI, 'file': wcfile, 'msg': comment, 'user': user})
    for i in cmd:
        log.debug('Command: ' + i)
        try:
...
class RcsLog(object):
    ...
    def __init__(self, filename):
        ...
        self.p = {}
        self.p['rcs'] = re.compile('^RCS file:\\s*(.*)$', re.M)
        self.p['head'] = re.compile('^head:\\s*(.*)$', re.M)
        self.p['total'] = re.compile('^total revisions:\\s*(.*)$', re.M)
        self.p['revision'] = re.compile('^revision\\s+([^\\s]*)', re.M)
        self.p['date'] = re.compile('^date:\\s+(.*);\\s+author:', re.M)
        self.p['author'] = re.compile('^date:.*;\\s+author:\\s*(.*?);', re.M)
        self.reload()
    ...
    def get_logs(self, rev1='', rev2='', rev3=''):
        ...
        while True:
            ...
            self.revs.append({'revision': commit_revision, 'date': commit_time, 
