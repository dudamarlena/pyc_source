...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pysvnmanager/tests/test_models.py
...
class TestModels(TestController):
    ...
    def testUser(self):
        ...
        self.assert_('Tom' not in user2)
        alias1 = Alias('Tom')
        self.assert_(alias1 not in user1)
        alias1.user = user2
        self.assert_(alias1 in user2)
        self.assert_(alias1 not in user3)
        self.assert_(None not in user3)
        return
    ...
    def testGroup(self):
        ...
        self.assert_('admin' not in group1)
        ...
        self.assert_('nobody' not in group1)
        ...
        self.assert_('*' in group1)
        self.assert_('nobody' in group1)
        self.assert_(None not in group1)
        self.assert_('  ' not in group1)
        group1.remove_all()
    ...
    def testModule(self):
        module = self.authz.get_module('版本库1', '/项目1')
        self.assert_(module.fullname == '版本库1:/项目1', module.fullname.encode('utf8'))
        self.assert_(unicode(module) == '[版本库1:/项目1]\n* = \n@组1 = rw\n用户3 = r\n', unicode(module).encode('utf8'))
        module = Module('/', '/trunk')
        ...
        module.update_rule(obj, 'r')
        self.assert_(unicode(module) == '[/trunk]\n$authenticated = r\n* = r\n@admins = rw\njiang = \n', repr(unicode(module)))
    ...
    def testRepos(self):
        repos1 = self.authz.get_repos('版本库1')
        self.assert_(repos1.name == '版本库1', repos1.name.encode('utf8'))
        user1 = User('user1')
        user2 = User('user2')
        user3 = User('user3')
        alias1 = Alias('alias1')
    ...
    def testAuthzConfRepos(self):
        ...
        self.assert_((',').join(map(lambda x: x.name, self.authz.reposlist)) == '/,repos1,repos2', (',').join(map(lambda x: x.name, self.authz.reposlist)))
        ...
        self.assert_((',').join(map(lambda x: x.repos + ':' + x.path, self.authz.modulelist())) == '/:/trunk,repos1:/', (',').join(map(lambda x: x.repos + ':' + x.path, self.authz.modulelist())))
        ...
        self.assert_((',').join(map(lambda x: x.name, self.authz.aliaslist)) == 'root,superuser', (',').join(map(lambda x: x.name, self.authz.aliaslist)))
        ...
        self.assert_((',').join(map(lambda x: x.name, self.authz.grouplist)) == 'myteam', (',').join(map(lambda x: x.name, self.authz.grouplist)))
        ...
        self.assert_(unicode(self.authz.grouplist.get('@myteam')) == 'myteam = $authenticated, *, @team1, @team2, user1, user2, user3', repr(unicode(self.authz.grouplist.get('@myteam'))))
        self.assert_(isinstance(self.authz.add_group_member('myteam', '@team3, @team4, @team5', True), Group))
        self.assert_(unicode(self.authz.grouplist.get('@myteam')) == 'myteam = $authenticated, *, @team1, @team2, @team5, user1, user2, user3', repr(unicode(self.authz.grouplist.get('@myteam'))))
        self.assertRaises(Exception, self.authz.del_group, '@team2')
        self.assert_(unicode(self.authz.grouplist.get('@team2')) == 'team2 = $authenticated, *', self.authz.grouplist.get('@team2'))
        self.authz.del_group('@team2', force=True)
        self.assert_(self.authz.grouplist.get('@team2') == None, unicode(self.authz.grouplist.get('@team2')))
        self.assert_(unicode(self.authz.grouplist.get('@myteam')) == 'myteam = $authenticated, *, @team1, @team5, user1, user2, user3', repr(unicode(self.authz.grouplist.get('@myteam'))))
        self.assert_(unicode(self.authz.grouplist.get('@team1')) == 'team1 = ', repr(unicode(self.authz.grouplist.get('@team1'))))
        ...
        tmpstr = '%s' % module
        self.assert_(tmpstr == '[/trunk]\nuser1 = rw\nuser2 = \n', repr(tmpstr))
        self.authz.set_rules('/', '/trunk', '')
        tmpstr = '%s' % module
        self.assert_(tmpstr == '', repr(tmpstr))
        ...
        self.assert_(unicode(self.authz.get_module('repos1', '/')) == '[repos1:/]\n* = \n@team1 = rw\n', repr(unicode(self.authz.get_module('repos1', '/'))))
        self.assert_((',').join(map(lambda x: unicode(x), self.authz.rulelist())) == '&superuser = rw,* = r,* = ,@team1 = rw', (',').join(map(lambda x: unicode(x), self.authz.rulelist())))
        ...
        self.assert_(unicode(self.authz.get_module('repos1', '/')) == '[repos1:/]\n@team1 = rw\n', repr(unicode(self.authz.get_module('repos1', '/'))))
        self.assert_((',').join(map(lambda x: x.name, self.authz.grouplist)) == '$authenticated,*,myteam,team1,team3,team4,team5', (',').join(map(lambda x: x.name, self.authz.grouplist)))
        ...
        self.assert_((',').join(map(lambda x: x.name, self.authz.aliaslist)) == 'superuser', (',').join(map(lambda x: x.name, self.authz.aliaslist)))
        self.assertRaises(Exception, self.authz.chk_grp_ref_by_rules, '@team1')
        ...
        self.assert_((',').join(map(lambda x: x.name, self.authz.grouplist)) == '$authenticated,*,team1,team3,team4,team5', (',').join(map(lambda x: x.name, self.authz.grouplist)))
        ...
        self.assert_(unicode(self.authz) == '# version : 0.0\n# admin : / = admin1, admin2\n\n[groups]\nmyteam = user2\nteam1 = \nteam3 = @team4\nteam4 = \nteam5 = \n\n[aliases]\nsuperuser = jiangxin\n\n', repr(unicode(self.authz)))
        return
    ...
    def testAuthzConfDefault(self):
        ...
        self.assert_(unicode(self.authz) == '# version : 0.1.3\n# admin : / = admin1, admin2\n# admin : repos1 = admin1, admin2\n# admin : repos2 = admin2\n# admin : repos3 = admin3\n# admin : 版本库1 = @管理组1\n\n[groups]\nadmin = &admin, admin1, admin2, admin3\nadmins = &007, &admin\nall = @team1, user3, user4\nteam1 = @team2, user1, user11\nteam2 = @team3, user2, user22\nteam3 = user3, user33\n管理组1 = &别名1, 蒋鑫\n组1 = &别名1, 用户2\n\n[aliases]\n007 = james\nadmin = jiangxin\n别名1 = 用户1\n\n[/]\nuser3 = r\nuser4 = r\n\n[/branches]\n$authenticated = r\n@admins = rw\n\n[/tags]\n* = \n@admins = rw\n@all = r\n\n[/trunk]\nuser2 = \n\n[/trunk/src]\nuser1 = r\nuser2 = r\n\n[repos1:/]\n@admins = rw\nuser3 = \n\n[repos1:/trunk]\nuser1 = r\nuser2 = \n\n[repos1:/trunk/src]\n&007 = r\nuser1 = \n\n[版本库1:/]\n* = \n@管理组1 = rw\n\n[版本库1:/项目1]\n* = \n@组1 = rw\n用户3 = r\n\n', repr(unicode(self.authz)))
        ...
        self.assert_(self.authz.get_access_map('', 'reposnoexist', descend=False) == None, self.authz.get_access_map('', 'reposnoexist', descend=False))
        ...
        self.assert_(self.authz.get_path_access_msgs('jiangxin', 'noexist', '/branches/1.0') == ['User jiangxin has Full (RW) rights for module noexist:/branches/1.0'], self.authz.get_path_access_msgs('jiangxin', 'noexist', '/branches/1.0'))
        self.assert_(self.authz.get_path_access_msgs('jiangxin', 'noexist', '/branches/1.0', abbr=True) == ['[noexist:/branches/1.0] jiangxin = rw'], self.authz.get_path_access_msgs('jiangxin', 'noexist', '/branches/1.0', abbr=True))
        self.assert_(self.authz.get_path_access_msgs('jiangxin', 'noexist', '/trunk/src') == ['User jiangxin has ReadOnly (RO) rights for module noexist:/trunk/src'], self.authz.get_path_access_msgs('jiangxin', 'noexist', '/trunk/src'))
        self.assert_(self.authz.get_path_access_msgs('jiangxin', 'noexist', '/trunk/src', abbr=True) == ['[noexist:/trunk/src] jiangxin = r'], self.authz.get_path_access_msgs('jiangxin', 'noexist', '/trunk/src', abbr=True))
        self.assert_(self.authz.get_path_access_msgs('jiangxin', 'noexist', '/new') == ['User jiangxin can *NOT* access to module noexist:/new'], self.authz.get_path_access_msgs('jiangxin', 'noexist', '/new'))
        self.assert_(self.authz.get_path_access_msgs('jiangxin', 'noexist', '/new', abbr=True) == ['[noexist:/new] jiangxin ='], self.authz.get_path_access_msgs('jiangxin', 'noexist', '/new', abbr=True))
        ...
        self.assert_(self.authz.get_path_access_msgs(' ', '*', '/trunk/src/mod1', abbr=True) == [
         '[/:/trunk/src/mod1] * =',
         '[repos1:/trunk/src/mod1] * =',
         '[repos2:/trunk/src/mod1] * =',
         '[repos3:/trunk/src/mod1] * =',
         '[版本库1:/trunk/src/mod1] * ='], self.authz.get_path_access_msgs(' ', '*', '/trunk/src/mod1', abbr=True))
        ...
        self.assert_('&admin' not in gadmin)
        self.assert_('&admin' not in gadmins)
