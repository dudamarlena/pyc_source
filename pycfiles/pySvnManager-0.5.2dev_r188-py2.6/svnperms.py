...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pysvnmanager/hooks/init/hook1.5/scripts/svnperms.py
# Compiled at: 2010-09-24 12:39:25
import commands, sys, os, getopt, re
...
SECTION = re.compile('\\[([^]]+?)(?:\\s+extends\\s+([^]]+))?\\]')
OPTION = re.compile('(\\S+)\\s*=\\s*(.*)$')
                ...
                for groupuser in groupsusers:
                    if groupuser[0] == '@':
                        try:
                            ...
                            raise Error, "group '%s' not found" % groupuser[1:]
...
def parse_options():
    ...
    if not (os.path.isdir(obj.repository) and os.path.isdir(os.path.join(obj.repository, 'db')) and os.path.isdir(os.path.join(obj.repository, 'hooks')) and os.path.isfile(os.path.join(obj.repository, 'format'))):
        raise Error, "path '%s' doesn't look like a repository" % obj.repository
    return obj
...
def main():
    try:
        opts = parse_options()
        check_perms(opts.filename, opts.section, opts.repository, opts.transaction, opts.revision, opts.author)
