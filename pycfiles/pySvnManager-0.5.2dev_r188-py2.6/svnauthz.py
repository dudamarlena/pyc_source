...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pysvnmanager/model/svnauthz.py
...
import rcsbackup as rcs, re, sys, os, my_fnmatch, StringIO
...
class User(object):
    ...
    def __get_nice_name(self):
        if self.nickname:
            ...
            return not self.firstname and (None if not self.lastname else (self.lastname + ' ' + self.firstname if self.firstname and self.lastname else self.firstname or self.lastname))
            return
...
class Alias(object):
    ...
    def __str__(self):
        return '%s = %s' % (self.aliasname, self.username)
    ...
    def __contains__(self, obj):
        ...
        if not obj:
            return False
        obj = obj.lower()
        if obj == self.uname.lower() or obj == self.username.lower():
            return True
        return False
        ...
        for member in members:
            ...
            if member not in self.__members:
                ...
                if not self.__valid_group(self):
                    ...
                    if not autodrop:
                        raise Exception, _('Recursive group membership for %s') % member.uname
            ...
            for group in self._group_list:
                ...
                if item in group.memberobjs:
                    if force:
                        ...
                        assert item not in group.memberobjs
                    else:
                        raise Exception, _('Group %s is referenced by group %s.') % (
                         name, group.uname)
...
class Rule(object):
    ...
    def get_permission(self, userobj):
        ...
        """
        if userobj not in self.userobj:
...
class Module(object):
    ...
    def access_is_granted(self, user, required):
        ...
        if deny & required == RIGHTS_NONE:
            ...
            if perm & required == required & RIGHTS_ALL:
                return True
            return False
...
class SvnAuthz(object):
    ...
    * SvnAuthz
    ...
    def __get_nice_userlist(self):
        if not self.nice_userlist_init:
            ...
            for person in Session.query(Person).all():
                self.userlist.get_or_set(name=person.uid, firstname=person.firstname, lastname=person.lastname, nickname=person.nickname, mail=person.mail)
    ...
    def parse_acl(self):
        ...
        pattern = re.compile('^#\\s*admin\\s*:\\s*(.+?)\\s*=\\s*(.+?)\\s*(?:#.*)?$')
        if acls:
            for acl in acls:
    ...
    def parse_version(self):
        ...
        pattern = re.compile('^#\\s*version\\s*[:=]\\s*(.+?)\\s*(?:#.*)?$')
        if ic:
            for line in ic:
        ...
        for group in self.__grouplist:
            if alias in group.memberobjs:
                if force:
                    ...
                    raise Exception, _('Alias %s is referenced by group %s.') % (
    ...
    def get_access_map(self, user=None, reposname='/', descend=True):
        ...
        if not repos:
            ...
            deny_path_set = set()
            if reposname != '/':
                root_path_set = set(map(lambda x: x.path, self.get_repos('/').module_list))
