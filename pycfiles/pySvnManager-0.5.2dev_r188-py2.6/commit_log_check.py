...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pysvnmanager/hooks/init/hook1.5/scripts/commit_log_check.py
...
def main(argv=None):
    ...
    log_msg = ''
    try:
        (opts, args) = getopt.getopt(argv, 'hs:p:m:P:', [
...
def check_strlen(log_msg, size):
    ...
    if log_length < size:
        error_msg = 'COMMIT_LOG_TOO_SIMPLE : 提交说明至少应包含 %d 个字符, 或者太简单了。\n' % size
...
def check_pattern(log_msg, permit=None, prohibit=None):
    ...
    if permit:
        ...
        if not matched:
            error_msg = 'PREDEFINED_PATTERN_NOT_IN_COMMIT_LOG: 无法在提交说明中匹配表达式: \n%s。\n' % (',\n').join(permit)
            sys.stderr.write(error_msg.encode('utf-8'))
            sys.exit(1)
    if prohibit:
        matched = False
        ...
        if matched:
            error_msg = 'BAD_COMMIT_LOG : 不允许在log中出现类似表达式: \n%s。\n' % pat
            sys.stderr.write(error_msg.encode('utf-8'))
            sys.exit(1)
