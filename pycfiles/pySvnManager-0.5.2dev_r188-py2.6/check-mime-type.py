...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pysvnmanager/hooks/init/hook1.5/scripts/check-mime-type.py
...
def main(repos, txn, force=''):
    ...
    pmime = re.compile('\\s*svn:mime-type : (\\S+)')
    peol = re.compile('\\s*svn:eol-style : (\\S+)')
    ptext = re.compile('^text/')
    pspecial = re.compile('\\s*svn:special : (\\S+)')
    errmsg = []
