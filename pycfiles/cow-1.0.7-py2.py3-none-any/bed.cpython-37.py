...
# Embedded file name: /Users/joe/devel/covviz/covviz/bed.py
# Compiled at: 2020-01-24 15:00:46
# Size of source mod 2**32: 17337 bytes
...
def validate_samples(samples, groups):
    group_samples = set([i for sublist in [v for k, v in groups.items()] for i in sublist])
    samples = set(samples)
...
def get_traces(data, samples, outliers, distance_threshold, slop):
    ...
    for sample in samples:
        for idx, consecutive_points in groupby(enumerate(outliers[sample]), lambda x: x[0] - x[1]['index']):
            index_values = []
            x_values = []
            y_values = []
    ...
    for sample in samples:
        if traces[sample]['x']:
            x_values, y_values = clean_regions(traces[sample]['x'], traces[sample]['y'], distance_threshold)
    ...
    with open(filename) as (fh):
        ...
        for row in reader:
            if sample_col not in row:
                logger.warning('sample column [%s] was not found in the header of %s' % (
                 sample_col, filename))
                logger.warning('this will likely result in strange plotting behavior on sex chromosomes.')
                break
            if sex_col not in row:
                logger.warning('sex column [%s] was not found in the header of %s' % (
                 sex_col, filename))
                logger.warning('this will likely result in strange plotting behavior on sex chromosomes.')
                break
...
def parse_bed(path, exclude, ped, sample_col='sample_id', sex_col='sex', sex_chroms='X,Y', z_threshold=3.5, distance_threshold=150000, slop=500000, min_samples=8, skip_norm=False):
    bed_traces = dict()
        ...
        for chr, entries in groupby(reader, key=(lambda i: i[header[0]])):
            ...
            for x_index, row in enumerate(entries):
                if not samples:
                    ...
                    for group_index, (gid, samples_of_group) in enumerate(sample_groups.items()):
                        ...
                        if len(samples) <= min_samples:
                            for sample in samples_of_group:
                                outliers[sample].append(dict(index=x_index, x=x_value, y=(data[sample][(-1)])))
                        ...
                        if len(set(sample_values)) == 1:
                            ...
                            for sample in [samples_of_group[j] for j in passing]:
                                if not data[sample][(-1)] > upper:
                                    if data[sample][(-1)] < lower:
                                        pass
                                    outliers[sample].append(dict(index=x_index, x=x_value, y=(data[sample][(-1)])))
            ...
            for sample, trace_data in traces.items():
                trace = dict(x=(trace_data['x']),
                  ...
                  name='significant',
                  marker={'width':1, 
                 'color':sample_trace_color[sample]})
                if trace_data['x']:
                    len_traces += 1
                json_output.append(trace)
