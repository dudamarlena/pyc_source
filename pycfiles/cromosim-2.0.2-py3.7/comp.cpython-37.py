...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/cromosim/comp.py
# Compiled at: 2020-04-22 16:42:03
# Size of source mod 2**32: 5746 bytes
...
def iteration(it, Nrooms, DoorRoomCapacity, NPir, NPrir, NPwir, Nior, List_iOr, T_iOr, Flux):
    ...
    return (
     Flux, NPir, NPwir, NPrir)


def plot_compt(ifig, RoomNames, RoomCenters, DoorCenters, CircAngles, NPir, NPrir, NPwir, Nior, List_iOr, dom, area, savefig=False, filename='fig.png', title=''):
    """
    To draw
    ...
    plt.clf()
    ax0 = plt.subplot(111)
    cf0 = ax0.imshow((dom.image), interpolation='nearest', extent=[
    ...
    for id, name in enumerate(RoomNames):
        wedge = Wedge((DoorCenters[(id, 0)], DoorCenters[(id, 1)]), (R[id]), (0 + CircAngles[id]),
          (180 + CircAngles[id]), ec='none')
        circles0.append(wedge)
        ax0.text((RoomCenters[(id, 0)]), (RoomCenters[(id, 1)]), (str(int(NPir[id]))),
          fontsize=10)
        colors0.append(0.0)
    ...
    patches0 = PatchCollection(circles0, cmap=(matplotlib.cm.prism), alpha=0.6, linewidth=0)
