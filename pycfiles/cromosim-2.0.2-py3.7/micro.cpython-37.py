...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/cromosim/micro.py
# Compiled at: 2020-04-23 17:50:42
# Size of source mod 2**32: 65199 bytes
...
def compute_contacts(dom, xyrv, dmax):
    ...
    first_elements = np.arange(xyrv.shape[0])
    other_elements = list(map(lambda x: x[1:], neighbors))
...
def compute_forces(F, Fwall, xyrv, contacts, U, Vd, lambda_, delta, k, eta):
    ...
    for ic in np.arange(Nc):
        ...
        if j > -1:
            ...
            fij_friction = eta * dij_moins * (-(U[(i, 0)] - U[(j, 0)]) * eij_y + (U[(i, 1)] - U[(j, 1)]) * eij_x)
...
def projection(dt, xyrv, contacts, Vd, dmin_people=0, dmin_walls=0, nb_iter_max=100000, rho=0.1, tol=0.01, log=False, method='cvxopt'):
    ...
    desired one among all the feasible fields (i.e. fields which do not lead\xa0
    ...
    if Nc == 0:
        ...
        if method == 'cvxopt' or method == 'mosek':
            ...
            cvxopt.solvers.abstol = 1e-08
            cvxopt.solvers.reltol = 1e-07
            L = None
            P = None
            U = np.zeros((2 * Np,))
            V = np.zeros((2 * Np,))
            ...
            if Nc > 0:
                ...
                cvxoptB = cvxopt.spmatrix((np.array(data)), (np.array(row)), (np.array(col)),
                  size=(Nc, 2 * Np))
                if method == 'mosek':
                ...
                if log:
                    C = Z - B @ U
                    if method == 'mosek':
                        print('    projection (mosek): nb of contacts = ', Nc, ', contrainte (Z-B@U).min() = ', C.min())
                    else:
                        print('    projection (cvxopt): nb of contacts = ', Nc, ', nb of iterations = ', solution['iterations'], ', status = ', solution['status'], ', contrainte (Z-B@U).min() = ', C.min())
            ...
            if log:
                print('    projection (uzawa): nb of contacts = ', Nc, ', nb of iterations = ', k, ', min = ', R.min(), ', max = ', R.max(), ', tol = ', tol)
            elif k == nb_iter_max:
...
def move_people(time, dt, people, sensors):
    ...
    for il, s in enumerate(sensors):
        l = s['line']
        id, pt, dir, times, entries, exits = sensor(l, xyrv_old[:, :2], people['xyrv'][:, :2], time, time + dt)
...
def create_people_in_box(nn, box, dest_name, radius_distribution, velocity_distribution, dom, rng, verbose=True):
    """
    To create nn persons in a given box. The overlaps are not treated for     the moment but one checks that the individuals are located in an area where\xa0    the desired velocity is well defined (outside inaccessible areas or     obstacles). If it is not the case, one changes their coordinates in     consequence.
    ...
    """
    if verbose:
        print('------ create_people_in_box --> Create ' + str(nn) + ' individuals in the box ' + str(box) + ', overlaps can occur...')
    else:
        xyrv = np.zeros((nn, 4))
        ...
        if radius_distribution[0] == 'uniform':
            ...
            if radius_distribution[0] == 'normal':
                mean = radius_distribution[1]
                sigma = radius_distribution[2]
                xyrv[:, 2] = rng.normal(mean, sigma, nn)
            else:
                print('------ create_people_in_box --> unknown distribution for                people radius ')
        p_rmax = xyrv[:, 2].max()
        if velocity_distribution[0] == 'uniform':
            vmin = velocity_distribution[1]
            vmax = velocity_distribution[2]
            xyrv[:, 3] = rng.uniform(vmin, vmax, nn)
        else:
            if velocity_distribution[0] == 'normal':
                mean = velocity_distribution[1]
                sigma = velocity_distribution[2]
                xyrv[:, 3] = rng.normal(mean, sigma, nn)
            else:
                print('------ create_people_in_box --> unknown distribution for                people velocity')
...
def check_people_in_box(dom, box, xyrv, dest_names, rng, verbose=True):
    ...
    """
    if verbose:
        print('------ check_people_in_box --> To verify that ' + str(xyrv.shape[0]) + ' individuals are in the domain, in the box and with a defined' + ' desired velocity')
    p_rmax = xyrv[:, 2].max()
    xmin, xmax, ymin, ymax = box
    info = False
    while True:
        ...
        if ind1.shape[0] > 0:
            if verbose:
                print('------ check_people_in_box --> ' + str(ind1.shape[0]) + ' individuals outside the domain')
            info = True
            ...
            I, J, Vd = dom.people_desired_velocity(xyrv, dest_names)
            normVd = Vd[:, 0] ** 2 + Vd[:, 1] ** 2
            test2 = (xyrv[:, 0] > xmin + p_rmax) * (xyrv[:, 0] < xmax - p_rmax) * (xyrv[:, 1] > ymin + p_rmax) * (xyrv[:, 1] < ymax - p_rmax) * (normVd > 0)
            ind2 = np.where(test2 == 0)[0]
            if ind2.shape[0] > 0:
                if verbose:
                    print('------ check_people_in_box --> ' + str(ind2.shape[0]) + ' individuals with an undefined desired velocity ')
                info = True
...
def remove_overlaps_in_box(dom, box, xyrv, dest_names, dt, rng, projection_method='cvxopt', dmin_people=0, dmin_walls=0, itermax=10, verbose=True):
    """
    ...
    dmax_init = xyrv[:, 2].max()
    it1 = 0
    t = 0
    while 1:
        if verbose:
            print('------ remove_overlaps_in_box --> Remove overlaps in box ' + str(box) + ': iteration ' + str(it1) + ' / ' + str(itermax))
        else:
            c = compute_contacts(dom, xyrv, dmax_init)
            I, J, Vd = dom.people_desired_velocity(xyrv, dest_names)
            info1, B, U, L, P = projection(dt, xyrv, c, Vd, method=projection_method,
            ...
            xyrv[:, 1] += dt * U[:, 1]
            it1 += 1
            info2, people = check_people_in_box(dom, box, xyrv, dest_names, rng,
              verbose=verbose)
...
def people_initialization(dom, groups, dt, dmin_people=0, dmin_walls=0, seed=0, itermax=10, projection_method='cvxopt', verbose=True):
    ...
    for igp, gp in enumerate(groups):
        nn = gp['nb']
        if verbose:
            print('=================> INITIALIZATION: ' + str(nn) + ' IN BOX ' + str(gp['box']))
        pp, pp_dest = create_people_in_box(nn, (gp['box']), (gp['destination']),
          (gp['radius_distribution']),
          (gp['velocity_distribution']),
          dom,
          rng, verbose=verbose)
        pp = remove_overlaps_in_box(dom, (gp['box']), pp, pp_dest,
          dt, rng, dmin_people=dmin_people,
          dmin_walls=dmin_walls,
          itermax=itermax,
    ...
    if verbose:
        print('=================> INITIALIZATION: LAST STEP (remove the overlaps between individuals in different boxes...)')
    xyrv = remove_overlaps_in_box(dom, [dom.xmin, dom.xmax, dom.ymin, dom.ymax], xyrv,
      dest, dt, rng, dmin_people=dmin_people,
      dmin_walls=dmin_walls,
      projection_method=projection_method,
      itermax=itermax,
      verbose=verbose)
    ...
    people['paths'] = {}
    people['rng'] = rng
    people['id'] = np.char.add([dom.name + '_'] * xyrv.shape[0], np.arange(xyrv.shape[0]).astype('<U3'))
    ...
    for idom, name in enumerate(domains):
        ...
        for dest_name in dom.destinations:
            ...
            if box is not None:
                ...
                if inside.shape[0] > 0:
                    virtual_people[next_domain_name]['xyrv'], indices = np.unique((virtual_people[next_domain_name]['xyrv']), return_index=True,
                      axis=0)
                    virtual_people[next_domain_name]['Vd'] = virtual_people[next_domain_name]['Vd'][indices]
                    virtual_people[next_domain_name]['U'] = virtual_people[next_domain_name]['U'][indices]
                    virtual_people[next_domain_name]['Uold'] = virtual_people[next_domain_name]['Uold'][indices]
                ...
                if inside.shape[0] > 0:
                    virtual_people[name]['xyrv'], indices = np.unique((virtual_people[name]['xyrv']), return_index=True,
                      axis=0)
                    virtual_people[name]['Vd'] = virtual_people[name]['Vd'][indices]
                    virtual_people[name]['U'] = virtual_people[name]['U'][indices]
                    virtual_people[name]['Uold'] = virtual_people[name]['Uold'][indices]
...
def people_update_destination(all_people, domains, thld, box=None):
    ...
    for idom, name in enumerate(domains):
        ...
        dom = domains[name]
        if box is None:
            S = (p[:, 0] - p[:, 2] <= dom.xmin + dom.pixel_size) + (p[:, 0] - p[:, 2] >= dom.xmax - dom.pixel_size) + (p[:, 1] - p[:, 2] <= dom.ymin + dom.pixel_size) + (p[:, 1] - p[:, 2] >= dom.ymax - dom.pixel_size)
        else:
            S = (p[:, 0] - p[:, 2] <= box[0] + dom.pixel_size) + (p[:, 0] - p[:, 2] >= box[1] - dom.pixel_size) + (p[:, 1] - p[:, 2] <= box[2] + dom.pixel_size) + (p[:, 1] - p[:, 2] >= box[3] - dom.pixel_size)
    ...
    for idom, name in enumerate(domains):
        ...
        if in_wall.shape[0] > 0:
            ...
            for ik, key in enumerate(all_people[name]):
                try:
                    if all_people[name][key].shape[0] == p.shape[0]:
                        all_people[name][key] = np.delete(all_people[name][key], in_wall, 0)
                except:
                    pass
        ...
        for ip, pid in enumerate(all_people[name]['id']):
            try:
                all_people[name]['paths'][pid] = np.hstack((all_people[name]['paths'][pid], p[ip, :2]))
            except:
        ...
        for id, dest_name in enumerate(np.unique(destinations)):
            ...
            if ind2.shape[0] > 0:
                if dom.destinations[dest_name].next_destination is not None:
                    ...
                    if next_domain != name:
                        all_people[name]['destinations'][ind[ind2]] = dom.destinations[dest_name].next_destination
        ...
        if id_rm is not None:
            for ik, key in enumerate(all_people[name]):
                try:
                    if all_people[name][key].shape[0] == p.shape[0]:
                        all_people[name][key] = np.delete(all_people[name][key], id_rm, 0)
                except:
                    pass
...
def plot_people(ifig, dom, people, contacts, colors, time=-1, axis=None, virtual_people=None, plot_people=True, plot_contacts=True, plot_velocities=False, plot_desired_velocities=False, plot_paths=False, plot_sensors=False, sensors=[], savefig=False, filename='fig.png', dpi=150, cmap='winter'):
    """
    ...
    ax1.imshow((dom.image), interpolation='nearest', extent=[
     dom.xmin, dom.xmax, dom.ymin, dom.ymax],
      origin='lower')
    if plot_people:
        try:
            offsets = people['xyrv'][:, :2]
            ec = EllipseCollection(widths=(2 * people['xyrv'][:, 2]), heights=(2 * people['xyrv'][:, 2]),
              angles=0,
              units='xy',
        ...
        try:
            ...
            ecv = EllipseCollection(widths=(2 * virtual_people['xyrv'][:, 2]), heights=(2 * virtual_people['xyrv'][:, 2]),
              angles=0,
              units='xy',
              facecolors='black',
              alpha=0.5,
    ...
    if plot_velocities:
        try:
            ...
            for ip in np.arange(Np):
                arrow = Arrow((people['xyrv'][(ip, 0)]), (people['xyrv'][(ip, 1)]),
                  (people['U'][(ip, 0)]),
                  (people['U'][(ip, 1)]),
                  width=0.3,
                  color='red')
            ...
            if virtual_people['xyrv'] is not None:
                ...
                for ip in np.arange(Np):
                    arrow = Arrow((virtual_people['xyrv'][(ip, 0)]), (virtual_people['xyrv'][(ip, 1)]),
    ...
    if plot_paths:
        ...
        pathlinecoll = LineCollection(pathlines, color='black', linewidths=0.5,
          linestyle='solid',
          alpha=0.5,
          cmap=(plt.get_cmap(cmap)))
    ...
    if plot_sensors:
        for ss in sensors:
            line = Line2D((ss['line'][::2]), (ss['line'][1::2]), lw=1.0,
              alpha=0.6,
              color='g')
            ax1.add_line(line)
...
def plot_sensors(ifig, sensors, time, flux_timestep=1, savefig=False, filename='fig.png', dpi=150, cmap='winter'):
    """
    ...
    if Ns > 0:
        ...
        for i, s in enumerate(sensors):
            ...
            ax2.set_title((name + ': entries (green) and exits (red) per ' + str(flux_timestep) + 's'),
              fontsize='small')
