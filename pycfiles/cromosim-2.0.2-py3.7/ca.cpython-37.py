...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/cromosim/ca.py
# Compiled at: 2020-04-22 16:41:59
...
def plot_people_according_to_current_door_distance(ifig, people, domain, axis=None, savefig=False, filename='fig.png'):
    """
    ...
    ax1.imshow((domain.image), interpolation='nearest', extent=[
     domain.xmin, domain.xmax, domain.ymin, domain.ymax],
      origin='lower')
    ax1.imshow((domain.destinations['door'].distance * (people == 1)), interpolation='nearest', extent=[
     domain.xmin, domain.xmax, domain.ymin, domain.ymax],
      origin='lower',
      alpha=1.0,
      cmap='Greys')
...
def plot_people_according_to_initial_door_distance(ifig, people, domain, results, axis=None, savefig=False, filename='fig.png'):
    """
    ...
    tmp[(I, J)] = D0
    ax1.imshow((tmp * (people == 1)), interpolation='nearest', extent=[
     domain.xmin, domain.xmax, domain.ymin, domain.ymax],
      origin='lower',
      alpha=1.0,
      cmap='Greys')
...
def plot_people_according_to_exit_times(ifig, dt, people, domain, results, axis=None, savefig=False, filename='fig.png'):
    """
    ...
    people_init = np.ma.MaskedArray(np.zeros((people.shape), dtype=int), mask=(domain.wall_mask))
    people_init.data[(I0, J0)] = 1
    ax1.imshow((tmp * (people_init == 1)), interpolation='nearest', extent=[
     domain.xmin, domain.xmax, domain.ymin, domain.ymax],
      origin='lower',
      alpha=1.0,
      cmap='Greys')
...
def plot_people_paths(ifig, dt, pixel_size, people, domain, results, axis=None, savefig=False, filename='fig.png'):
    """
    ...
    ax1.imshow((tmp * (people_init == 1)), interpolation='nearest', extent=[
     domain.xmin, domain.xmax, domain.ymin, domain.ymax],
      ...
      alpha=1.0,
      cmap=cmap,
      norm=colors.BoundaryNorm(boundaries=(np.linspace(vmin, vmax, cmap.N + 1)), ncolors=(cmap.N)))
    mask = results == -1
    ...
    paths = LineCollection((0.5 * pixel_size + pixel_size * traj), linewidths=2, linestyle='solid',
      cmap=cmap,
...
def sequential_update(people, people_ij, weight, shuffle=None, randomstate=None):
    """
    To move all individuals sequentially according to the following rule:\xa0    the update of individual i is determined in a stochastic way by computing     transition probabilities on neighboring cells (including the current     position of i).
    ...
    for id in order:
        ...
        if i != -1:
            if j != -1:
                ...
                if i >= 1:
                    im = i - 1
                    if people[(im, j)] is not np.ma.masked:
                        if people[(im, j)] != 1:
                            I.append(im)
                            J.append(j)
                if i <= height - 2:
                    ip = i + 1
                    if people[(ip, j)] is not np.ma.masked:
                        if people[(ip, j)] != 1:
                            I.append(ip)
                            J.append(j)
                if j >= 1:
                    jm = j - 1
                    if people[(i, jm)] is not np.ma.masked:
                        if people[(i, jm)] != 1:
                            I.append(i)
                            J.append(jm)
                if j <= width - 2:
                    jp = j + 1
                    if people[(i, jp)] is not np.ma.masked:
                        if people[(i, jp)] != 1:
                            I.append(i)
                            J.append(jp)
...
def parallel_update(people, people_ij, weight, friction=0, randomstate=None):
    """
    To move all individuals in parallel according to the following rule:\xa0    first, desired moves are precomputed and then the conflicts (two individuals     at the same position) are resolved
    ...
    unique_ij, index, inverse, counts = np.unique((new_ij[ind, :2]), axis=0, return_index=True, return_inverse=True, return_counts=True)
    if unique_ij.shape[0] < ind.shape[0]:
        for ic, cc in enumerate(counts):
            if cc > 1:
                ind_conf = np.where(inverse == ic)[0]
                npc = ind_conf.shape[0]
                print('---------- Conflicts between ', npc, ' persons')
                choice = randomstate.choice(2, 1, p=[friction, 1 - friction])[0]
                if choice == 0:
                    ...
                    pos = randomstate.choice((ind_conf.shape[0]), 1, p=w)
                    for id in np.arange(ind_conf.shape[0]):
                        if id != pos:
                            new_ij[ind[ind_conf[id]], :2] = people_ij[ind[ind_conf[id]], :2]
