...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/tuitwi/ui.py
# Compiled at: 2010-06-30 10:58:42
import sys, curses, curses.ascii, unicodedata, datetime, locale, re
...
class LabelControl(Control):
    ...
    def _draw(self):
        ...
        for s in reduce(lambda x, y: x + y, map(lambda line: split_from_width(line, self.width - 1), lines), []):
            if i >= self.height:
                break
...
class FullStatusArea(Control):
    ...
    def _draw(self):
        if self._status is None:
            return
        name = '%s(%s)' % (self._status.user.name, self._status.user.screen_name)
        source = self._status.source
        info = '%s from %s' % (self._status.created_at + datetime.timedelta(hours=9), source)
        self._win.addstr(0, 0, adjust_n_width(name, self.width - 1, fill=''))
        (h, i) = (1, 0)
        lines = self._status.text.split('\n')
        strings = reduce(lambda x, y: x + y, map(lambda line: split_from_width(line, self.width - 1, translate=False), lines), [])
        rem = 0
        while h < self.height - 1 and i < len(strings):
            ...
            if self._keyword:
                while target[start:].find(self._keyword) >= 0:
                    ...
                    self._win.addstr(adjust_n_width(strings[i][end:min(end + len(self._keyword), len(strings[i]))]), curses.A_REVERSE)
        ...
        self._win.addstr(self.height - 1, 0, adjust_n_width(info, self.width - 1, fill=''))
        return
...
class EditLineControl(Control):
    ...
    def __init__(self, stdscr, begin_y, attr=curses.A_NORMAL, max_text_length=sys.maxint):
        (height, width) = stdscr.getmaxyx()
    ...
    def insert_rt(self, status):
        self._end()
        for ch in 'RT @' + unicode(status.user.screen_name) + ': ' + status.text:
            self._insert(ch)

        self._home()

    def _cur_x(self):
        return reduce(lambda x, y: x + y, map(lambda s: s['width'], self._char_array[self._begin_index:self._cur_index]), 0)
    ...
    def getTextFrom(self, begin):
        return ('').join(map(lambda s: s['char'], self._char_array[begin:]))
    ...
    @property
    def text(self):
        return ('').join(map(lambda s: s['char'], self._char_array[0:]))
...
class ListViewControl(Control):
    ...
    def reply_string(self):
        if self._index == -1:
            return ''
        return '@%s ' % self._items[(self._top + self._index)].user.screen_name
    ...
    def _move(self, check, line=1):
        ...
        for i in range(len(self._items)):
            if check(self._items[(new_top + new_index)]):
                ...
                new_index += line
            elif new_top + line >= 0 and new_top + new_index + line < len(self._items):
                new_top += line
    ...
    def move_to_reply_to(self):
        self._move(lambda status: status.id == self.current_status().in_reply_to_status_id, line=-1)

    def move_to_reply_from(self):
        self._move(lambda status: status.in_reply_to_status_id == self.current_status().id, line=1)

    def next_user_post(self):
        self._move(lambda status: status.user.id == self.current_status().user.id and status.id != self.current_status().id, line=1)

    def prev_user_post(self):
        self._move(lambda status: status.user.screen_name == self.current_status().user.screen_name and status.id != self.current_status().id, line=-1)

    def search_next_word(self, word):
        self._move(lambda status: status.text.find(word) >= 0 and status.id != self.current_status().id, line=1)
...
class TabControl(Control):
    ...
    def update_timeline(self, timeline):
        for i in range(len(self._wins)):
            ...
            if self._conf['tabs'][i]['users']:
                filtered = [ status for status in filtered if status.user.screen_name in self._conf['tabs'][i]['users'] ]
            filtered = [ status for status in filtered if self._conf['tabs'][i]['keyword'].search(status.text) ]
            filtered.reverse()
    ...
    def current_win(self):
        return self._wins[self._tab_index]['win']
