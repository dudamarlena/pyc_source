...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kds/raw.py
# Compiled at: 2020-02-20 02:30:47
...
def fetch_game_plays(s_id, game_no, database='kidaura_v1'):
    ...
    data = list(coll.aggregate([
     {'$match': {'S_Id': {'$in': [str(s_id), int(s_id)]}}},
     {'$addFields': {'_id':False, 
                     'sortField':{'$cond': [
    ...
    for i, r in tdf.iterrows():
        ...
        if fingure_id in record:
            if touch_phase == 'Began':
                ...
                if touch_phase == 'Ended' or touch_phase == 'Canceled':
                    tdf.loc[(i, 'touch_id')] = record[fingure_id]
                    del record[fingure_id]
                else:
                    tdf.loc[(i, 'touch_id')] = record[fingure_id]
        elif touch_phase != 'Began':
