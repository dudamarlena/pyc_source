...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mimo/workflow/workflow.py
...
class Workflow(Stream):
            ...
            for output in graph.get_children(fr):
                for input in graph.get_children(output):
                    for to in graph.get_children(input):
                        parameters = [
                         'sametail="{}"'.format(output), 'samehead="{}"'.format(input)]
    ...
    async def run(self, ins=None, outs=None):
        ...
        tasks = [stream.run(self.input_sets[stream_id], self.output_sets[stream_id]) for stream_id, stream in self.streams.items()]
