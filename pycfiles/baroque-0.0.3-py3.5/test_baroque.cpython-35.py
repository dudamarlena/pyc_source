...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/test_baroque.py
...
from baroque.datastructures.registries import EventTypesRegistry, ReactorsRegistry, TopicsRegistry
...
def test_validate_event_schema():
    brq = Baroque()
    et = MetricEventType()
    event_valid = Event(et, payload={'metric': 'temperature', 
     'value': 56.7793, 
...
def test_publish_on_topic():
    brq = Baroque()
    t = brq.topics.new('test-topic1', eventtypes=[
     MetricEventType(), GenericEventType()])
