...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/baroque/datastructures/registries.py
# Compiled at: 2017-05-05 06:02:13
# Size of source mod 2**32: 11011 bytes
import collections
from . import bags
...
class EventTypesRegistry:
    __doc__ = 'Interface adapter to an event bag.'
...
class ReactorsRegistry:
    ...
    def __repr__(self):
        return '<{}.{} - event types reactors: {} - jolly reactors: {}>'.format(__name__, self.__class__.__name__, ','.join(["'{}': {}".format(k, v) for k, v in self.registered_types.items()]) or 'None', ','.join(map(str, self.jolly_bag)))


class TopicsRegistry:
    __doc__ = 'A tracker for reactors to be executed upong event firing of events on\n    specified topics: the reactors-topics relationship is stored internally \n    using a dict'
