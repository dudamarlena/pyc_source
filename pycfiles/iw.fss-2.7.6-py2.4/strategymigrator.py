...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/iw/fss/bin/strategymigrator.py
...
def usedSpace(path):
    ...
    for (root, dirs, files) in os.walk(path):
        for filename in files:
            ...
            fsize = (fsize / block_size + 1) * block_size
            total += fsize
...
class FlatToDirectoryMigrator(BaseMigrator):
    ...
    def run(self):
        """Go baby, go..."""
        self.loginfo(1, 'Transforming FSS flat storage in %s into directory storage in %s' % (self.sourcepath, self.destpath))
...
class DirectoryToFlatMigrator(BaseMigrator):
    ...
    def run(self):
        """Let's go"""
        self.loginfo(1, 'Transforming FSS directory storage in %s into flat storage in %s' % (self.sourcepath, self.destpath))
...
_migrator_classes = (
 FlatToDirectoryMigrator, DirectoryToFlatMigrator)
...
def findMigrator(source_strat, dest_strat):
    ...
    for klass in _migrator_classes:
        if (
         klass._source_strategy, klass._destination_strategy) == (source_strat, dest_strat):
            return klass
...
def availableMigrations():
    ...
    for klass in _migrator_classes:
        out += '\n* --source-strategy=%s --destination-strategy=%s' % (klass._source_strategy, klass._destination_strategy)
...
class Application(object):
    ...
    def __init__(self):
        ...
        parser = optparse.OptionParser(usage=__doc__ + availableMigrations(), version=__version__)
        parser.add_option('-v', '--verbose', action='count', dest='verbosity', default=0, help='Add output verbosity for each -v.')
        parser.add_option('-s', '--source-strategy', action='store', type='string', dest='source_strategy', default='flat', metavar='STRATEGY', help="The FSS strategy used in SOURCEPATH, one of '%s'. Default: '%%default'." % ("', '").join(allStrategies()))
        parser.add_option('-d', '--destination-strategy', action='store', type='string', dest='destination_strategy', default='directory', metavar='STRATEGY', help="The FSS strategy used in DESTINATIONPATH, one of '%s'. Default: '%%default'." % ("', '").join(allStrategies()))
        parser.add_option('-r', '--remove-source', action='store_true', dest='remove_source', default=False, help='Remove source files successfully migrated (Make a backup of SOURCEPATH before using this option). Default: %default.')
        ...
        if not self.options.remove_source:
            ...
            if availableSpace(self.destpath) < used:
                print "Error: there is not enough free space in '%s'. You need %d free bytes in the destination path, or consider making a backup and use the '--remove-source' option." % (self.destpath, used)
                sys.exit(2)
            else:
                self.loginfo(1, 'Migration will take approximately %d bytes in %s' % (used, self.destpath))
        return
