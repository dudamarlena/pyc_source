...
# [GCC 8.4.0]
# Embedded file name: c:\Users\mastromatteo\Progetti\flows\flows\Actions\WebserverAction.py
...
class WebserverAction(Action):
    ...
    def on_init(self):
        ...
        flows.Global.LOGGER.info(str.format('Server Starts - {0}:{1}', self.host_name, self.host_port))
        threading.Thread(target=(self.my_server.serve_forever)).start()

    def on_cycle(self):
        super().on_cycle()
        MyServerRequestHandler.message['sleep_interval'] = str(flows.Global.CONFIG_MANAGER.sleep_interval)
...
class MyServerRequestHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, format, *args):
        string_to_log = str.format('{0} - - [{1}] {2}', self.address_string(), self.log_date_time_string(), format % args)
