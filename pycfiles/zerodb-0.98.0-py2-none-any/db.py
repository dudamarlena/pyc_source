...
# [GCC 8.4.0]
# Embedded file name: /zerodb/db.py
# Compiled at: 2016-07-10 14:36:33
import itertools, os, threading, six
...
class DbModel(object):
    ...
    def __init__(self, db, model):
        ...
        if not transaction.manager._txn and (self._intid_name not in db._root or self._catalog_name not in db._root):
            transaction.begin()
            commit = True
        else:
            commit = False
    ...
    def __getitem__(self, uids):
        ...
        if isinstance(uids, six.integer_types):
            ...
            if not hasattr(obj, '_p_uid'):
                obj._p_uid = uids
            return obj
        if isinstance(uids, (tuple, list, set)):
...
class DB(object):
    ...
    def __init__(self, sock, username=None, password=None, realm='ZERO', debug=False, pool_timeout=3600, pool_size=7, autoreindex=True, **kw):
        ...
        self.__storage_kwargs = {'sock': sock, 
           'username': username, 
           'password': password, 
           'realm': realm, 
           'cache_size': 1073741824, 
           'debug': debug}
        self.__db_kwargs = {'pool_size': pool_size, 
           'pool_timeout': pool_timeout, 
           'cache_size': 1000000, 
           'cache_size_bytes': 100 * 1048576}
    ...
    def _root(self):
        ...
        if os.getpid() != self.__pid:
            self.__pid = os.getpid()
            self._init_db()
        elif not hasattr(self.__thread_local, 'conn') or self.__thread_local.conn.opened is None:
            self._conn_open()
