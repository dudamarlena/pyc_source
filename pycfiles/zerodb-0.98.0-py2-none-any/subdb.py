...
# [GCC 8.4.0]
# Embedded file name: /zerodb/permissions/subdb.py
# Compiled at: 2016-04-24 09:04:25
import hashlib, six, struct, threading, time, transaction, warnings
from copy import copy
from ZODB.DB import Pickler, BytesIO, _protocol, z64, ConnectionPool, KeyedConnectionPool, IMVCCStorage
...
class StorageClass(ServerStorage):
    ...
    def loadBefore(self, oid, tid):
        ...
        if r is not None:
            ...
            self._check_permissions(data, oid)
            return (
             data[:-len(self.user_id)], serial, after)
        else:
            return r
            return
    ...
    extensions = [
     get_root_id, add_user, del_user, change_key]
...
class DB(BaseDB):
    ...
    def __init__(self, storage, pool_size=7, pool_timeout=2147483648, cache_size=400, cache_size_bytes=0, historical_pool_size=3, historical_cache_size=1000, historical_cache_size_bytes=0, historical_timeout=300, database_name='unnamed', databases=None, xrefs=True, large_record_size=16777216, **storage_args):
        ...
        if not hasattr(storage, 'tpc_vote') and not storage.isReadOnly():
            warnings.warn("Storage doesn't have a tpc_vote and this violates the storage API. Violently monkeypatching in a do-nothing tpc_vote.", DeprecationWarning, 2)
