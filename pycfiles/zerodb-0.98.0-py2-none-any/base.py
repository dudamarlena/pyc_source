...
# [GCC 8.4.0]
# Embedded file name: /zerodb/permissions/base.py
# Compiled at: 2016-07-12 01:04:50
import BTrees, hashlib, persistent, six, transaction, ZODB
from os import path
...
class User(persistent.Persistent):
    ...
    def __init__(self, username, pubkey, realm='ZEO', administrator=False, root=None, auth_method=None):
        """
...
class PermissionsDatabase(object):
    ...
    def __init__(self, filename, realm=None):
        ...
        with transaction.manager:
            ...
            if 'usernames' not in root:
                root['usernames'] = self.family.OI.BTree()
        self.filename = filename
        self.load()
        if realm:
            if self.realm and self.realm != realm:
                raise ValueError('Specified realm %r differs from database realm %r' % (
                 realm or '', self.realm))
            else:
                self.realm = realm
        ...
        with transaction.manager:
            for line in fd:
                ...
                if username not in usernames:
                    user = User(username, pub, self.realm, administrator=True, auth_method=auth_method)
                    uid = users.add(user)
    ...
    def add_user(self, username, pubkey, administrator=False):
        ...
        if transaction.manager._txn:
            ...
            commit = True
            transaction.begin()
        user = User(username, pubkey, self.realm, administrator=administrator, auth_method=auth_method)
        uid = users.add(user)
        usernames[username] = uid
        if commit:
            transaction.commit()
