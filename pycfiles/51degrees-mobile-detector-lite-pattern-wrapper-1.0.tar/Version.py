...
# Embedded file name: \Ft\Lib\DistExt\Version.py
# Compiled at: 2006-10-20 13:24:55
import re
from distutils.version import Version, StrictVersion
__all__ = ['CommonVersion', 'VersionPredicate', 'SplitProvision', 'SplitComparison']
...
class CommonVersion(Version):
    ...
    tag_re = re.compile('[_.-]?([a-zA-Z]+)?(\\d+(?:\\.\\d)*)?')
...
try:
    from distutils.versionpredicate import VersionPredicate, split_provision as SplitProvision, splitUp as SplitComparison
except ImportError:
    import operator
    re_validPackage = re.compile('(?i)^\\s*([a-z_]\\w*(?:\\.[a-z_]\\w*)*)(.*)')
    re_paren = re.compile('^\\s*\\((.*)\\)\\s*$')
    re_provision = re.compile('([a-zA-Z_]\\w*(?:\\.[a-zA-Z_]\\w*)*)(?:\\s*\\(\\s*([^)\\s]+)\\s*\\))?$')
