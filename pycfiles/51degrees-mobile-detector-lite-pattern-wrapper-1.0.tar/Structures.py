...
# Embedded file name: \Ft\Lib\DistExt\Structures.py
# Compiled at: 2006-09-23 17:03:57
__doc__ = '\nData structures that are to be used in a pkg file.\n'
from distutils.errors import DistutilsSetupError
__all__ = [
 'Localization', 'FileList', 'Script', 'Executable', 'File', 'ModulesDocument', 'ExtensionsDocument', 'Document']
...
def _string_sequence_check(arg):
    return isinstance(arg, (list, tuple)) and reduce(lambda a, b: a + isinstance(b, str), arg, 0) == len(arg)
...
class FileList(filelist.FileList):
    ...
    def __init__(self, dest, sources, recursive=False, excludes=None):
        ...
        assert _string_sequence_check(sources), "'sources' must be a sequence of strings"
        self.dest = dest
...
class Script:
    ...
    def __init__(self, name, module, function=None, application=None):
        ...
        assert function is None or _string_check(function), "'function' must be a string or None"
        assert application is None or _string_check(application), "'application' must be a string or None"
        if function is not None and application is not None:
            raise DistutilsSetupError("only one of 'function' or 'application' allowed")
        elif function is None and application is None:
            raise DistutilsSetupError("one of 'function' or 'application' is required")
        elif application:
    ...
    def __repr__(self):
        return 'Script(%r, %r, %r, %r)' % (self.name, self.module, self.function, self.application)
...
class File:
    ...
    def __repr__(self):
        return '%s(%r, %r, %r)' % (self.__class__.__name__, self.source, self.outdir, self.flags)
...
class ModulesDocument(GeneratedDocument):
    ...
    def __init__(self, title, packages):
        ...
        assert _string_sequence_check(packages), "'packages' must be a sequence of strings"
        self.title = title
    ...
    def __repr__(self):
        return '%s(%r, %r)' % (self.__class__.__name__, self.title, self.packages)
...
class ExtensionsDocument(GeneratedDocument):
    ...
    def __init__(self, name, modules, title):
        assert _string_check(name), "'name' must be a string"
        assert _string_sequence_check(modules), "'modules' must be a sequence of strings"
        assert _string_check(title), "'title' must be a string"
        self.name = name
    ...
    def __repr__(self):
        return '%s(%r, %r, %r)' % (self.__class__.__name__, self.name, self.modules, self.title)
...
class Document:
    ...
    def __init__(self, source, stylesheet, params=None, title=None, category=None, flags=None, mtime=None, outfile=None):
        assert _string_check(source), "'source' must be a string"
    ...
    def __repr__(self):
        return 'Document(%r, %r, %r, %r, %r, %r, %r, %r)' % (self.source, self.stylesheet, self.params, self.title, self.category, self.flags, self.mtime, self.outfile)