...
# Embedded file name: \Ft\Xml\Xslt\BuiltInExtElements.py
...
__all__ = [
 'RESERVED_NAMESPACE', 'ExtNamespaces', 'ExtElements', 'AssignElement', 'ChainToElement', 'CreateIndexElement', 'DumpKeysElement', 'DumpVarsElement', 'FtApplyImports', 'FtApplyTemplates', 'FtOutputElement', 'MsgControlElement', 'RawTextOutputElement', 'ReplaceElement', 'UriToElementElement', 'GettextElement', 'SetupTranslationsElement']
...
class FtApplyImports(XsltElement):
    ...
    def instantiate(self, context, processor):
        ...
        context.stylesheet.applyTemplates(context, processor, params=with_params, maxImport=self.importIndex)
        return
...
class FtOutputElement(XsltElement):
    ...
    def instantiate(self, context, processor):
        ...
        processor.addHandler(self._output_parameters, processor.writer.getStream())
        try:
            for child in self.children:
...
class AssignElement(XsltElement):
    ...
    __module__ = __name__
    legalAttrs = {'name': AttributeInfo.QName(required=1), 'select': AttributeInfo.Expression(), 'body-as-ns': AttributeInfo.YesNoAvt(default='no')}
...
class DumpKeysElement(XsltElement):
    ...
    legalAttrs = {'raw': AttributeInfo.YesNoAvt(default='no', description='Present keys in a compact non-XML format'), 'force-update': AttributeInfo.YesNoAvt(default='no', description='Force evaluation of all keys on all loaded documents')}
    ...
    def instantiate(self, context, processor):
        ...
        if (
         FT_EXT_NAMESPACE, 'indices') in processor.extensionParams:
        ...
        if self._raw.evaluate(context):
            ...
            writer.startElement('zz:KeyDump', RESERVED_NAMESPACE)
            for (k, v) in xkeys.items():
                writer.startElement('zz:Key', RESERVED_NAMESPACE)
                writer.attribute('name', k[1], EMPTY_NAMESPACE)
                for (kk, vv) in v.items():
                    writer.startElement('zz:MatchSet', RESERVED_NAMESPACE)
                    writer.attribute('value', kk, EMPTY_NAMESPACE)
                    for node in vv:
                        if node.nodeType == Node.ATTRIBUTE_NODE:
                            processor.writer.comment('Attribute: %s=%s' % (node.nodeName, node.value))
                        else:
                            CopyNode(processor, node)

                    writer.endElement('zz:MatchSet', RESERVED_NAMESPACE)

                writer.endElement('zz:Key', RESERVED_NAMESPACE)

            writer.endElement('zz:KeyDump', RESERVED_NAMESPACE)
        return
        return
...
class DumpVarsElement(XsltElement):
    ...
    legalAttrs = {'raw': AttributeInfo.YesNoAvt(default='no', description='Present variables and values in a compact non-XML format')}
    ...
    def instantiate(self, context, processor):
        ...
        if self._raw.evaluate(context):
            ...
            from Ft.Xml.Xslt.CopyOfElement import CopyNode
            writer = processor.writer
            writer.startElement('zz:VarDump', RESERVED_NAMESPACE)
            for (k, v) in context.varBindings.items():
                writer.startElement('zz:Var', RESERVED_NAMESPACE)
                writer.attribute('name', k[1], EMPTY_NAMESPACE)
                ...
                if type(v) in g_xpathPrimitiveTypes:
                    writer.text(Conversions.StringValue(v))
                if hasattr(v, 'nodeType'):
                    CopyNode(processor, v)
                writer.endElement('zz:Var', RESERVED_NAMESPACE)

            writer.endElement('zz:VarDump', RESERVED_NAMESPACE)
        return
...
class ReplaceElement(XsltElement):
    ...
    """
    __module__ = __name__
    legalAttrs = {'string': AttributeInfo.StringExpression(description='The string to be processed.  If not given, the string value of the context node is used.'), 'substring': AttributeInfo.StringExpression(required=1, description='The sub-string to be replaced.')}
...
class MsgControlElement(XsltElement):
    ...
    """
    __module__ = __name__
    legalAttrs = {'suppress': AttributeInfo.YesNoAvt(default='no', description='Disable display of all XSLT messages.')}
...
class CreateIndexElement(XsltElement):
    ...
    """
    __module__ = __name__
    legalAttrs = {'name': AttributeInfo.StringAvt(required=1, description='The name of the key to create'), 'select': AttributeInfo.Expression(required=1, description='Selects which nodes are to be indexed'), 'use': AttributeInfo.Expression(required=1, description='The expression that computes the index key value for each node')}
    ...
    def instantiate(self, context, processor):
        ...
        if (
         FT_EXT_NAMESPACE, 'indices') not in processor.extensionParams:
...
class RawTextOutputElement(XsltElement):
    ...
    """
    __module__ = __name__
    legalAttrs = {'select': AttributeInfo.Expression(required=1, description='An XPath expression that returns a Python Unicode object.')}
...
class ChainToElement(XsltElement):
    ...
    """
    __module__ = __name__
    legalAttrs = {'href': AttributeInfo.UriReferenceAvt(required=1, description='The URI of the next stylesheet in the chain')}
    content = ContentInfo.Rep(ContentInfo.QName(XSL_NAMESPACE, 'xsl:with-param'))
...
class SetupTranslationsElement(XsltElement):
    ...
    legalAttrs = {'domain': AttributeInfo.StringAvt(required=1, description='The domain name of the message catalog'), 'localedir': AttributeInfo.StringAvt(description='The message catalog path')}
    doesPrime = 1
    ...
    def prime(self, processor, context):
        ...
        processor.extensionParams[(FT_EXT_NAMESPACE, 'translations')] = translations
        return
        return
...
ExtElements = {(FT_EXT_NAMESPACE, 'apply-imports'): FtApplyImports, (FT_EXT_NAMESPACE, 'apply-templates'): FtApplyTemplates, (FT_EXT_NAMESPACE, 'output'): FtOutputElement, (FT_EXT_NAMESPACE, 'dump-keys'): DumpKeysElement, (FT_EXT_NAMESPACE, 'dump-vars'): DumpVarsElement, (FT_EXT_NAMESPACE, 'assign'): AssignElement, (FT_EXT_NAMESPACE, 'replace'): ReplaceElement, (FT_EXT_NAMESPACE, 'message-control'): MsgControlElement, (FT_EXT_NAMESPACE, 'create-index'): CreateIndexElement, (FT_EXT_NAMESPACE, 'raw-text-output'): RawTextOutputElement, (FT_EXT_NAMESPACE, 'chain-to'): ChainToElement, (FT_EXT_NAMESPACE, 'uri-to-element'): UriToElementElement, (FT_EXT_NAMESPACE, 'gettext'): GettextElement, (FT_EXT_NAMESPACE, 'setup-translations'): SetupTranslationsElement}