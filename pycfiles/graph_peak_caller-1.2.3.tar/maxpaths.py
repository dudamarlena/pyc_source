...
# [GCC 8.4.0]
# Embedded file name: /home/ivargry/dev/graph_peak_caller/graph_peak_caller/postprocess/maxpaths.py
...
class SparseMaxPaths:
    ...
    def __init__(self, sparse_values, graph, score_pileup):
        ...
        self._analyzer = SegmentSplitter(self._segments, self._node_ids, graph.node_indexes)
    ...
    def _handle_internal(self, mask):
        ...
        self.internal_paths = [ Peak(offset[0], offset[1], [_id], graph=self._graph) for offset, _id in zip(offsets, ids)
                              ]
    ...
    def get_node_ids(self):
        ...
        node_ids[:, 1] = np.digitize(self._segments[:, 1], self._node_indexes, True)
        return node_ids
    ...
    def run(self):
        ...
        scored_segments = [ np.vstack((self._analyzer._internal_ids[:, 0][mask], self.scores[mask])) for mask in [
         self._analyzer.internal_mask,
        ...
        linegraph = PosDividedLineGraph(scored_segments[2], scored_segments[3], scored_segments[1], self._graph)
        paths, infos, subgraphs = linegraph.max_paths()
        converted = self._convert_paths(paths, infos)
        small_subgraphs = [ SubGraph(path.region_paths, csr_matrix(([], ([], [])), shape=(1,
                                                                                          1))) for path in self.internal_paths
                          ]
        return (converted + self.internal_paths, subgraphs + small_subgraphs)

    def _convert_paths(self, paths, infos):
        reverse_map = np.concatenate([ np.flatnonzero(mask) for mask in [
         self._analyzer.end_mask,
         self._analyzer.full_mask,
         self._analyzer.start_mask]
                                     ])
        peaks = [ self._convert_path(path, reverse_map) for path in paths
                ]
    ...
    def get_segment_scores(self):
        ...
        pileup_idxs[:, 0] = np.digitize(self._segments[:, 0], self._score_pileup.indices)
        pileup_idxs[:, 1] = np.digitize(self._segments[:, 1], self._score_pileup.indices, True)
        weighted_values = np.ediff1d(self._score_pileup.indices, self._score_pileup.track_size - self._score_pileup.indices[(-1)]) * self._score_pileup.values
        pileup_cumsum = np.r_[(0, np.cumsum(weighted_values))]
        base_scores = pileup_cumsum[(pileup_idxs[:, 1] - 1)] - pileup_cumsum[(pileup_idxs[:, 0] - 1)]
