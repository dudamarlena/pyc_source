...
# [GCC 8.4.0]
# Embedded file name: /home/ivargry/dev/graph_peak_caller/graph_peak_caller/control/linearpileup.py
...
class LinearPileup(object):
    ...
    def get_event_sorter(self, linear_map, touched_nodes=None):
        ...
        node_starts_idxs = [ linear_map.get_node_start(node_id) for node_id in node_start_values
                           ]
        node_end_idxs = [ linear_map.get_node_end(node_id) for node_id in node_end_values
                        ]
        for start_idx, end_idx in zip(node_starts_idxs, node_end_idxs):
            assert start_idx < end_idx, '%s, %s' % (start_idx, end_idx)

        idxs = [
         node_end_idxs, self.indices, node_starts_idxs]
    ...
    def from_event_sorter(event_sorter):
        ...
        for index, code, value in event_sorter:
            if code == event_sorter.NODE_START:
                ...
                [ unmapped_indices[node_id].add_indexvalue(index, value) for node_id in cur_nodes ]
                cur_value = value
                cur_index = index
