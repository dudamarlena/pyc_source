...
# [GCC 8.4.0]
# Embedded file name: /home/ivargry/dev/graph_peak_caller/graph_peak_caller/sample/sparsegraphpileup.py
...
class NodeInfo:
    ...
    def __init__(self, dist_dict=None):
        self._dist_dict = dist_dict if dist_dict is not None else defaultdict(int)
        return
...
class ReadsAdder:
    ...
    def __init__(self, graph, pileup, keep_ends=False):
        ...
        self._neg_ends = {-node_id:[] for node_id in graph.blocks.keys()}
    ...
    def add_open_pos_interval(self, interval):
        ...
        try:
            ...
            raise InvalidPileupInterval('Interval has node(s) not part of graph/pileup: %s. Message %s' % (
             interval.region_paths, e))
...
class ReadsAdderWDirect(ReadsAdder):
    ...
    def _handle_interval(self, interval):
        ...
        rp = end_pos.region_path_id
        if rp < 0:
            self.neg_read_ends.append(self._node_indexes[(abs(rp) + 1 - self.min_id)] - end_pos.offset)
        else:
            self.pos_read_ends.append(self._node_indexes[(abs(rp) - self.min_id)] + end_pos.offset)
...
class SparseExtender:
    ...
    def run_linear(self, starts_dict):
        ...
        for counter, node_id in enumerate(node_ids):
            ...
            n_starts = len(starts)
            endsidxs = chain(enumerate((start + self._fragment_length for start in starts), start=cur_id), info._dist_dict.items())
...
class ReverseSparseExtender(SparseExtender):
    ...
    def get_node_ids(self):
        return [ -node_id for node_id in self._graph.get_topological_sorted_node_ids()[::-1]
               ]
...
class SamplePileupGenerator:
    ...
    def __init__(self, graph, extension):
        ...
        self._neg_extender = ReverseSparseExtender(graph, self._pileup, extension)
    ...
    def get_direct_pileup(self):
        ...
        sparsediff = SparseDiffs.from_pileup(new_pileup, self._graph.node_indexes)
        sparsediff.clean()
    ...
    def run(self, reads, reporter=None):
        ...
        sdiffs = SparseDiffs.from_pileup(self._pileup, self._graph.node_indexes)
