...
# [GCC 8.4.0]
# Embedded file name: /home/ivargry/dev/graph_peak_caller/graph_peak_caller/postprocess/segmentanalyzer.py
# Compiled at: 2018-03-06 04:26:57
import numpy as np
...
class SegmentAnalyzer:
    ...
    def _get_spanning_nodes(self, start_ids, end_ids):
        ...
        return list(chain.from_iterable(range(start + 1, end) for start, end in zip(start_ids[spanning], end_ids[spanning])))
    ...
    def handle_internal(self):
        is_starts = self._is_starts(self._internal_segments[:, 0], self._internal_ids[:, 0])
        is_ends = self._is_ends(self._internal_segments[:, 1], self._internal_ids[:, 1])
        full_mask = is_starts & is_ends
        self.add_fulls(self._internal_ids[:, 0][full_mask])
        start_mask = is_starts & ~full_mask
        self.add_starts(self._internal_ids[:, 1][start_mask], self._internal_segments[:, 1][start_mask])
        end_mask = is_ends & ~full_mask
        self.add_ends(self._internal_ids[:, 0][end_mask], self._internal_segments[:, 0][end_mask])
        self.internals = self._internal_segments[(~is_starts & ~is_ends)]

    def handle_multinode(self):
        self.add_fulls(self._get_spanning_nodes(self._spanning_ids[:, 0], self._spanning_ids[:, 1]))
        is_starts = self._is_starts(self._spanning_segments[:, 0], self._spanning_ids[:, 0])
        is_ends = self._is_ends(self._spanning_segments[:, 1], self._spanning_ids[:, 1])
        self.add_fulls(self._spanning_ids[:, 0][is_starts])
        self.add_fulls(self._spanning_ids[:, 1][is_ends])
        self.add_ends(self._spanning_ids[:, 0][(~is_starts)], self._spanning_segments[:, 0][(~is_starts)])
        self.add_starts(self._spanning_ids[:, 1][(~is_ends)], self._spanning_segments[:, 1][(~is_ends)])
...
class SegmentSplitter(SegmentAnalyzer):
    ...
    def handle_multinode(self):
        spanned = self._get_spanning_nodes(self._spanning_ids[:, 0], self._spanning_ids[:, 1])
        ...
        if n_old:
            ...
            splitted_segments[:n_old, 1][self.multinodes] = self.node_indexes[self._spanning_ids[:, 0]]
        splitted_segments[n_old:n_old + n_new, 0] = self.node_indexes[(self._spanning_ids[:, 1] - 1)]
