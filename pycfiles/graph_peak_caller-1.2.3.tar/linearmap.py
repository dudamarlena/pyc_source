...
# [GCC 8.4.0]
# Embedded file name: /home/ivargry/dev/graph_peak_caller/graph_peak_caller/control/linearmap.py
...
class LinearMap:
    ...
    def __init__(self, node_starts, node_ends, graph):
        ...
        assert np.all(self._node_starts < self._node_ends), np.where(self._node_starts >= self._node_ends)
    ...
    def to_sparse_pileup(self, unmapped_indices_dict, min_value=0):
        ...
        for i in range(node_idxs.size - 1):
            ...
            new_idxs = [ (idx - offset) // scale + node_idxs[i] for idx in unmapped_indices.indices
                       ]
            new_idxs[0] = max(node_idxs[i], new_idxs[0])
            all_indices.extend(new_idxs)
            all_values.extend(unmapped_indices.values)

        return SparseDiffs(np.array(all_indices, dtype='int'), np.diff(np.r_[(0, all_values)]))
    ...
    def to_file(self, filename):
        np.savez(filename, starts=self._node_starts, ends=self._node_ends)
