...
# [GCC 8.4.0]
# Embedded file name: /Users/nearl/projects/specutils/build/lib/specutils/spectra/spectrum_mixin.py
...
class OneDSpectrumMixin:
    ...
    def with_velocity_convention(self, velocity_convention):
        return self.__class__(flux=(self.flux), wcs=(self.wcs), meta=(self.meta), velocity_convention=velocity_convention)
    ...
    def velocity(self):
        ...
        if self.rest_value is None:
            raise ValueError('Cannot get velocity representation of spectral axis without specifying a reference value.')
        if self.velocity_convention is None:
            raise ValueError('Cannot get velocity representation of spectral axis without specifying a velocity convention.')
        equiv = getattr(u.equivalencies, 'doppler_{0}'.format(self.velocity_convention))(self.rest_value)
    ...
    def with_spectral_unit(self, unit, velocity_convention=None, rest_value=None):
        ...
        """
        new_wcs, new_meta = self._new_spectral_wcs(unit=unit,
    ...
    def _new_wcs_argument_validation(self, unit, velocity_convention, rest_value):
        ...
        if velocity_convention in DOPPLER_CONVENTIONS:
            ...
            if velocity_convention is not None:
                if velocity_convention not in DOPPLER_CONVENTIONS.values():
                    raise ValueError('Velocity convention must be radio, optical, or relativistic.')
            elif rest_value is not None:
                raise hasattr(rest_value, 'unit') and rest_value.unit.is_equivalent(u.m, u.spectral()) or ValueError('Rest value must be specified as an astropy quantity with spectral equivalence.')
            return unit

    def _new_spectral_wcs(self, unit, velocity_convention=None, rest_value=None):
        """
        ...
        if isinstance(unit, u.UnitBase):
            if unit.is_equivalent((self.wcs.unit[0]),
              equivalencies=(u.spectral())):
                return (
                 gwcs_from_array(self.spectral_axis), meta)
        logging.error('WCS units incompatible: {} and {}.'.format(unit, self._wcs_unit))
...
class InplaceModificationMixin:
    ...
    def substract_background(self, background):
        ...
        if callable(background):
            pass
        elif isinstance(background, np.ndarray) and background.shape == data[(-1)].shape:
