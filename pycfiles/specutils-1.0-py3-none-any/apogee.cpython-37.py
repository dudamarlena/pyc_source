...
# [GCC 8.4.0]
# Embedded file name: /Users/nearl/projects/specutils/build/lib/specutils/io/default_loaders/apogee.py
...
__all__ = [
 'apVisit_identify', 'apStar_identify', 'aspcapStar_identify',
...
def apVisit_identify(origin, *args, **kwargs):
    ...
    Registry.
    """
    return isinstance(args[0], str) and args[0].startswith('apVisit') and (fits.connect.is_fits)(origin, *args)
...
def apStar_identify(origin, *args, **kwargs):
    ...
    Registry.
    """
    return isinstance(args[0], str) and args[0].lower().split('.')[(-1)] == 'fits' and args[0].startswith('apStar')
...
def aspcapStar_identify(origin, *args, **kwargs):
    ...
    Registry.
    """
    return isinstance(args[0], str) and args[0].lower().split('.')[(-1)] == 'fits' and args[0].startswith('aspcapStar')
...
def apVisit_loader(file_name, **kwargs):
    ...
    hdulist.close()
    return Spectrum1D(data=(data * unit), uncertainty=uncertainty,
