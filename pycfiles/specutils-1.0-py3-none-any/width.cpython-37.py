...
# [GCC 8.4.0]
# Embedded file name: /Users/nearl/projects/specutils/build/lib/specutils/analysis/width.py
# Compiled at: 2020-03-19 15:15:50
# Size of source mod 2**32: 8995 bytes
"""
...
__all__ = [
 'gaussian_sigma_width', 'gaussian_fwhm', 'fwhm', 'fwzi']
...
def _compute_fwzi(spectrum, regions=None):
    ...
    def find_width(data):
        ...
        peak_ind = [
         peaks[np.argmin(np.abs(np.array(peaks) - np.argmin(np.abs(data - np.max(data)))))]]
        widths, _, _, _ = peak_widths(data, peak_ind, rel_height=0.9999999)
...
def _compute_single_fwhm(flux, spectral_axis):
    ...
    if i0.size == 0:
        ...
        left_value = (halfval - left_flux) * (spectral_value[i1] - left_spectral) / (flux_value[i1] - left_flux) + left_spectral
    ...
    if i1.size == 0:
        ...
        left_flux = flux_value[i0]
        left_spectral = spectral_value[i0]
        right_value = (halfval - left_flux) * (spectral_value[i1] - left_spectral) / (flux_value[i1] - left_flux) + left_spectral
