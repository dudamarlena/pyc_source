...
# [GCC 8.4.0]
# Embedded file name: /Users/nearl/projects/specutils/build/lib/specutils/analysis/flux.py
# Compiled at: 2020-03-19 16:04:08
...
__all__ = [
 'line_flux', 'equivalent_width', 'is_continuum_below_threshold',
...
def is_continuum_below_threshold(spectrum, threshold=0.01):
    ...
    if hasattr(threshold, 'unit'):
        if not threshold.unit == u.dimensionless_unscaled:
            return np.median(flux) < threshold
    if uncertainty:
        if uncertainty.uncertainty_type != 'std':
        ...
        def wrapper(*args, **kwargs):
            if conf.do_continuum_function_check:
                ...
                if not is_continuum_below_threshold(spectrum, threshold):
                    ...
                    message += "If you want to suppress this warning either type 'specutils.conf.do_continuum_function_check = False' or see http://docs.astropy.org/en/stable/config/#adding-new-configuration-items for other ways to configure the warning."
