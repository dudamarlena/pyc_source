...
# [GCC 8.4.0]
# Embedded file name: /Users/nearl/projects/specutils/build/lib/specutils/io/default_loaders/wcs_fits.py
# Compiled at: 2020-03-17 18:47:05
# Size of source mod 2**32: 16801 bytes
import logging, os
...
__all__ = [
 'wcs1d_fits_loader', 'wcs1d_fits_writer', 'non_linear_wcs1d_fits']

def identify_wcs1d_fits(origin, *args, **kwargs):
    with fits.open(args[0]) as (hdu):
        if hdu[0].header['NAXIS'] == 1:
            if hdu[0].header.get('WCSDIM', 1) == 1:
                if 'WAT1_001' not in hdu[0].header:
                    if 'CTYPE1' in hdu[0].header:
                        return True
    return False


@data_loader('wcs1d-fits', identifier=identify_wcs1d_fits, dtype=Spectrum1D,
  extensions=['fits'])
def wcs1d_fits_loader(file_name, spectral_axis_unit=None, flux_unit=None, hdu_idx=0, **kwargs):
    """
...
def identify_iraf_wcs(origin, *args):
    ...
    WCSDIM == 2
    """
    return (fits.connect.is_fits)(origin, *args) and 'WAT1_001' in fits.getheader(args[0]) and not (fits.getheader(args[0])['TELESCOP'] == 'SDSS 2.5-M' and fits.getheader(args[0])['FIBERID'] > 0)


@data_loader('iraf', identifier=identify_iraf_wcs, dtype=Spectrum1D, extensions=[
 'fits'])
def non_linear_wcs1d_fits(file_name, spectral_axis_unit=None, flux_unit=None, **kwargs):
    ...
    with (fits.open)(file_name, **kwargs) as (hdulist):
        ...
        if flux_unit is not None:
            ...
            if 'BUNIT' in header:
                ...
                logging.info('Flux unit was not provided, neither it was in theheader. Assuming ADU.')
...
def _read_non_linear_iraf_wcs(header, wcsdim):
    ...
    if 'spec1' in wat_wcs_dict.keys():
        ...
        wcs_dict = {'aperture':aperture,  'beam':beam, 
         'dtype':disp_type, 
         'dstart':disp_start, 
         'avdelt':disp_del_av, 
         'pnum':pix_num, 
    ...
    if wcs_dict['dtype'] == -1:
        ...
        if wcs_dict['dtype'] == 2:
            if wcs_dict['ftype'] == 1:
                ...
                raise SyntaxError('ftype {:d} is not defined in the standard'.format(wcs_dict['ftype']))
    else:
        raise SyntaxError('dtype {:d} is not defined in the standard'.format(wcs_dict['dtype']))
...
def _chebyshev(wcs_dict):
    ...
    """
    model = models.Chebyshev1D(degree=(wcs_dict['order'] - 1), domain=[
     wcs_dict['pmin'],
     wcs_dict['pmax']])
...
def _non_linear_legendre(wcs_dict):
    ...
    """
    model = models.Legendre1D(degree=(wcs_dict['order'] - 1), domain=[
     wcs_dict['pmin'],
     wcs_dict['pmax']])
    new_params = [wcs_dict['fpar'][i] for i in range(wcs_dict['order'])]
    model.parameters = new_params
