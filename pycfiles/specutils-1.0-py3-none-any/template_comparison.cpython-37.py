...
# [GCC 8.4.0]
# Embedded file name: /Users/nearl/projects/specutils/build/lib/specutils/analysis/template_comparison.py
# Compiled at: 2020-03-17 18:47:05
# Size of source mod 2**32: 9765 bytes
import numpy as np
from ..manipulation import FluxConservingResampler, LinearInterpolatedResampler, SplineInterpolatedResampler
from spectra.spectrum1d import Spectrum1D
__all__ = [
 'template_match', 'template_redshift']
...
def _chi_square_for_templates(observed_spectrum, template_spectrum, resample_method):
    ...
    return (
     normalized_template_spectrum, chi2)


def template_match(observed_spectrum, spectral_templates, resample_method='flux_conserving', redshift=None):
    """
    ...
    """
    if hasattr(spectral_templates, 'flux'):
        if len(spectral_templates.flux.shape) == 1:
            chi2_list = []
            if redshift is not None:
                _, redshifted_spectrum, chi2_list = template_redshift(observed_spectrum, spectral_templates, redshift=redshift)
            ...
            return (
             normalized_spectral_template, chi2, 0, chi2_list)
    chi2_min = None
    smallest_chi_spec = None
    chi2_list = []
    for index, spectrum in enumerate(spectral_templates):
        if redshift is not None:
            _, redshifted_spectrum, chi2_inner_list = template_redshift(observed_spectrum,
              spectrum, redshift=redshift)
            spectrum = redshifted_spectrum
            chi2_list.append(chi2_inner_list)
        normalized_spectral_template, chi2 = _chi_square_for_templates(observed_spectrum, spectrum, resample_method)
    ...
    return (
     smallest_chi_spec, chi2_min, smallest_chi_index, chi2_list)
...
def template_redshift(observed_spectrum, template_spectrum, redshift):
    ...
    for rs in redshift:
        redshifted_spectrum = Spectrum1D(spectral_axis=(template_spectrum.spectral_axis * (1 + rs)), flux=(template_spectrum.flux),
          uncertainty=(template_spectrum.uncertainty),
          meta=(template_spectrum.meta))
        normalized_spectral_template, chi2 = _chi_square_for_templates(observed_spectrum, redshifted_spectrum, 'flux_conserving')
        chi2_list.append(chi2)
        if np.isnan(chi2) or chi2_min is None or chi2 < chi2_min:
            chi2_min = chi2
            final_redshift = rs

    return (
     final_redshift, redshifted_spectrum, chi2_list)