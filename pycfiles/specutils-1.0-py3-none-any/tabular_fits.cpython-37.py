...
# [GCC 8.4.0]
# Embedded file name: /Users/nearl/projects/specutils/build/lib/specutils/io/default_loaders/tabular_fits.py
...
from ..parsing_utils import generic_spectrum_from_table, spectrum_from_column_mapping
__all__ = [
 'tabular_fits_loader', 'tabular_fits_writer']

def identify_tabular_fits(origin, *args, **kwargs):
    return isinstance(args[0], str) and (fits.connect.is_fits)(origin, *args) and len(fits.open(args[0])) > 1 and isinstance(fits.open(args[0])[1], fits.BinTableHDU) and not (fits.getheader(args[0])['TELESCOP'] == 'SDSS 2.5-M' and fits.getheader(args[0])['FIBERID'] > 0) and not (fits.getheader(args[0])['TELESCOP'] == 'HST' and fits.getheader(args[0])['INSTRUME'] in ('COS',
                                                                                                                                                                                                                                                                                                                                                                                 'STIS')) and not fits.getheader(args[0])['TELESCOP'] == 'JWST'


@data_loader('tabular-fits', identifier=identify_tabular_fits, dtype=Spectrum1D,
  extensions=['fits'])
...
def tabular_fits_writer(spectrum, file_name, update_header=False, **kwargs):
    ...
    if update_header:
        ...
        header.update([keyword for keyword in spectrum.meta.items() if isinstance(keyword[1], hdr_types)])
