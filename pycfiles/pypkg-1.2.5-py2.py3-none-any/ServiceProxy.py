...
# Embedded file name: ZSI/ServiceProxy.py
...
class ServiceProxy():
    ...
    def _call(self, name, soapheaders):
        ...
        def call_closure(*args, **kwargs):
            ...
            binding = _Binding(url=(self._url or callinfo.location), soapaction=callinfo.soapAction, **self._kw)
            kw = dict(unique=True)
            if callinfo.use == 'encoded':
                kw['unique'] = False
            if callinfo.style == 'rpc':
                request = TC.Struct(None, ofwhat=[], pname=(
                 callinfo.namespace, name), **kw)
                response = TC.Struct(None, ofwhat=[], pname=(
                 callinfo.namespace, name + 'Response'), **kw)
                ...
                if len(ipart) != 1 or not ipart[0].element_type or ipart[0].type is None:
                    raise RuntimeError, 'Bad Input Message "%s"' % callinfo.name
                if len(opart) not in (0, 1) or not opart[0].element_type or opart[0].type is None:
