...
# Embedded file name: ZSI/client.py
...
_AuthHeader.typecode = Struct(_AuthHeader, ofwhat=(String((ZSI_SCHEMA_URI, 'Name'), typed=False),
 String((ZSI_SCHEMA_URI, 'Password'), typed=False)), pname=(ZSI_SCHEMA_URI, 'BasicAuth'), typed=False)
...
class _Binding():
    ...
    def SetAuth(self, style, user=None, password=None):
        ...
        """
        self.auth_style, self.auth_user, self.auth_pass = style, user, password
        return self
    ...
    def SendSOAPData(self, soapdata, url, soapaction, headers={}, **kw):
        ...
        if self.auth_style & AUTH.httpbasic:
            ...
            if self.auth_style == AUTH.httpdigest and not headers.has_key('Authorization') and not headers.has_key('Expect'):
    ...
    def SendSOAPDataHTTPDigestAuth(self, response, soapdata, url, request_uri, soapaction, **kw):
        ...
        if dict_fetch(chaldict, 'challenge', '').lower() == 'digest' and dict_fetch(chaldict, 'nonce', None) and dict_fetch(chaldict, 'realm', None) and dict_fetch(chaldict, 'qop', None):
            ...
            headers = {'Authorization': build_authorization_arg(authdict), 
               'Expect': '100-continue'}
    ...
    def ReceiveSOAP(self, readerclass=None, **kw):
        ...
        if self.ps:
            ...
            self.ps = ParsedSoap(self.data, readerclass=readerclass or self.readerclass, encodingStyle=kw.get('encodingStyle'))
            if self.sig_handler is not None:
