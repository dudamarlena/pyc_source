...
# Embedded file name: ZSI/ServiceContainer.py
# Compiled at: 2018-06-29 21:47:06
__doc__ = 'Simple Service Container\n   -- use with wsdl2py generated modules.\n'
...
class SOAPActionNotSpecified(Exception):
    pass


class WSActionException(Exception):
    pass
...
class SOAPContext:
    ...
    def __init__(self, container, xmldata, ps, connection, httpheaders, soapaction):
...
class ServiceInterface:
    ...
    def getOperationName(self, ps, action):
        """Returns operation name.
           action -- soapAction value
        """
        method = self.root.get(_get_element_nsuri_name(ps.body_root)) or self.soapAction.get(action)
        if method is None:
...
class WSAResource(ServiceSOAPBinding):
    ...
    def do_POST(self):
        ...
        try:
            ...
            _contexts[thread_id] = SOAPContext(self.server, xml, ps, self.connection, self.headers, soapAction)
            try:
                _Dispatch(ps, self.server, self.send_xml, self.send_fault, post=post, action=soapAction)
            except Exception as e:
...
class ServiceContainer(HTTPServer):
    ...
    def __call__(self, ps, post, action, address=None):
        ...
        if isinstance(method.im_self, WSAResource) or isinstance(method.im_self, SimpleWSResource):
