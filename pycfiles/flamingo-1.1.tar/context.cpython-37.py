...
# Compiled at: 2020-03-29 10:59:29
# Size of source mod 2**32: 9827 bytes
...
class Context(OverlayObject):
    ...
    def render(self, content, template_name=''):
        ...
        if not template_name:
            ...
            if self.settings.PRE_RENDER_CONTENT and content.get('is_template', True):
                content['_content_body_rendered'] or self.logger.debug('pre rendering %s', content['path'] or content)
                try:
                    exitcode, output = self.templating_engine.pre_render_content(content, template_context)
                except Exception as e:
                    try:
                        self.logger.error('%s: exception raised while pre rendering content',
                          (content['path']),
                          exc_info=True)
                        self.errors.append(e)
    ...
    def build(self, clean=True):
        ...
        elif self.settings.CONTENT_PATHS:
