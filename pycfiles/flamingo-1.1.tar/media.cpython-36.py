...
# Compiled at: 2020-03-11 04:46:16
...
def add_media(context, content, name, **extra_meta_data):
    ...
    media_content = Content(path=path, 
     output=output, 
     url=url, 
     content=content, **extra_meta_data)
        ...
        for content in context.contents:
            if not content['media']:
                ...
                for media in content['media']:
                    context.cp(source=(os.path.join(context.settings.CONTENT_ROOT, media['path'])),
                      destination=(os.path.join(context.settings.OUTPUT_ROOT, media['output'])))