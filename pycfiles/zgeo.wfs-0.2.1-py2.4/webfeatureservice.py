...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/zgeo/wfs/webfeatureservice.py
...
class WebFeatureService(object):
    ...
    def getFeatureTypesDict(self):
        ...
        if not hasattr(self.context, 'geoFeaturetypes'):
            ft_dict = {'default': {'elements': {'Title': 'string'}, 'boundingbox': None}}
    ...
    def buildSimpleFilter(self, List_Nodes, filter):
        ...
        generalComparisonOperators = ('PropertyIsEqualTo', 'PropertyIsNotEqualTo',
    ...
    def addFeatureType(self, ftname):
        """
                """
        ft_dict = self.getFeatureTypesDict()
        if ftname not in ft_dict:
            ft_dict[ftname] = {'elements': {'Title': 'string'}, 'boundingbox': None}
            self.updateFeatureTypesDict(ft_dict)
        return
