...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/opends/opends.py
# Compiled at: 2016-05-13 02:42:28
import json, time, urllib, gzip, StringIO, requests
    ...
    def _request(self, url, payload=None, param=None):
        ...
        while True:
            try:
                ...
                _url = '%s?%s' % (url, params)
                if payload:
                    payload_str = '%s' % json.dumps(payload)
                    s = StringIO.StringIO()
                ...
                try_count += 1
                print 'can not connect to server, retry ... | reason: %s' % str(e)
                time.sleep(5)
                if try_count == 5:
                    raise e
        ...
        if not result:
            error_msg = ('\nstatus: {status}\nerror_str: {errstr}\nrequest_id: {req_id}').format(status='-1', errstr='connection failed', req_id='')
            raise OpenDSException(error_msg)
        if result['status'] != '0':
            error_msg = ('\nstatus: {status}\nerror_str: {errstr}\nrequest_id: {req_id}').format(status=result['status'], errstr=result['errstr'], req_id=result.get('request_id', ''))
            raise OpenDSException(error_msg)
