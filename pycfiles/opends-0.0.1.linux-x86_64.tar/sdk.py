...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/opends/sdk.py
# Compiled at: 2016-05-13 06:49:59
...
class Table(object):
    ...
    def __init__(self, ds, name, tb_id, schema=(), uniq_key=None, title=None):
        ...
        """
        if not name:
            raise OpenDSException('Table name is required.')
        if not ds and not type(ds) != DS:
            raise OpenDSException('Ds is required.')
        self._name = name
    ...
    def modify_schema(self):
        ...
        for fd in _need_del:
            try:
                ...
                print 'Delete field `%s` failed and ignored, reason: %s' % (fd.get('name'), e)

        for fd in _need_add:
            try:
                self._ds.opends.field_add(None, self._tb_id, fd.get('name'), fd.get('type'), 0)
            except Exception as e:
                print 'Add field `%s` failed and ignored, reason: %s' % (fd.get('name'), e)
...
class BDPClient:
    ...
    def delete_ds(self, name=None):
        ...
        if self.get_ds(name) is not None:
