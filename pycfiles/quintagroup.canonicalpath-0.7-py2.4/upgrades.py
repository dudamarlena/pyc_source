...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/quintagroup/canonicalpath/upgrades.py
...
class CanonicalConvertor(object):
    ...
    def _convert(self, obj, src_iface, dst_iface, converter):
        ...
        msg = 'Migrate %s into %s for %s object: ' % (src_msg, dst_iface.__name__, obj.absolute_url())
        try:
    ...
    def _convertL2P(self, src, dst):
        """Convert canonical link to canonical path."""
        raise NotImplementedError('Convertion from canonical link to canonical path not implemented')
