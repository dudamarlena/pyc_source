...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/quintagroup/canonicalpath/tests.py
...
class TestIndexerRegistration(TestCase):
    ...
    def testCanonicalPathForAT(self):
        wrapper = self.get_indexable_wrapper(self.my_doc)
        self.assertTrue(hasattr(wrapper, 'canonical_path'), "'canonical_path' attribute not accessible with indexer wrapper for Document object")

    def testCanonicalLinkForAT(self):
        wrapper = self.get_indexable_wrapper(self.my_doc)
        self.assertTrue(hasattr(wrapper, 'canonical_link'), "'canonical_link' attribute not accessible with indexer wrapper for Document object")
...
class TestDefaultCanonicalPathAdapter(TestCase):
    ...
    def testRegistration4Portal(self):
        ...
        self.assertFalse(cpadapter is None, "Can't get canonical path adapter for the plone site object")
        return

    def testRegistration4AT(self):
        cpadapter = queryAdapter(self.my_doc, ICanonicalPath)
        self.assertFalse(cpadapter is None, "Can't get canonical path adapter for the Document object")
        return

    def testGetDefault4Portal(self):
        cpadapter = queryAdapter(self.portal, ICanonicalPath)
        self.assertTrue(cpadapter.canonical_path == self.portal_cp, "Canonical path adapter return '%s' for portal, must be: '%s'" % (cpadapter.canonical_path, self.portal_cp))

    def testGetDefault4AT(self):
        cpadapter = queryAdapter(self.my_doc, ICanonicalPath)
        self.assertTrue(cpadapter.canonical_path == self.mydoc_cp, "Canonical path adapter return '%s' for document, must be: '%s'" % (cpadapter.canonical_path, self.mydoc_cp))
    ...
    def testSet4Portal(self):
        ...
        self.assertTrue(prop == newcp, "Canonical path adapter setter NOT SET new '%s' value to '%s' propery for the portal" % (newcp, PROPERTY_PATH))
        self.assertTrue(cpadapter.canonical_path == newcp, "Canonical path adapter GET '%s' canonical_path, for portal, must be: '%s'" % (cpadapter.canonical_path, newcp))
        return
    ...
    def testSet4AT(self):
        ...
        self.assertTrue(prop == newcp, "Canonical path adapter setter NOT SET new '%s' value to '%s' propery for the Document" % (newcp, PROPERTY_PATH))
        self.assertTrue(cpadapter.canonical_path == newcp, "Canonical path adapter GET '%s' canonical_path, for Document, must be: '%s'" % (cpadapter.canonical_path, newcp))
        return
    ...
    def testDeleteProperty(self):
        ...
        self.assertFalse(hasprop(PROPERTY_PATH), 'Not deleted Canonical path customization property for the portal')
    ...
    def testDelCustomization(self):
        ...
        del cpadapter.canonical_path
        self.assertTrue(cpadapter.canonical_path == self.portal_cp, 'After deleted Canonical path customization property not set to default value for the portal')
...
class TestDefaultCanonicalLinkAdapter(TestCase):
    ...
    def testRegistration4Portal(self):
        ...
        self.assertFalse(cladapter is None, "Can't get canonical link adapter for the plone site object")
        return

    def testRegistration4AT(self):
        cladapter = queryAdapter(self.my_doc, ICanonicalLink)
        self.assertFalse(cladapter is None, "Can't get canonical link adapter for the Document object")
        return

    def testGetDefault4Portal(self):
        cladapter = queryAdapter(self.portal, ICanonicalLink)
        self.assertTrue(cladapter.canonical_link == self.purl(), "Canonical link adapter return '%s' for portal, must be: '%s'" % (cladapter.canonical_link, self.purl()))

    def testGetDefault4AT(self):
        cladapter = queryAdapter(self.my_doc, ICanonicalLink)
        self.assertTrue(cladapter.canonical_link == self.mydoc_cl, "Canonical link adapter return '%s' for document, must be: '%s'" % (cladapter.canonical_link, self.mydoc_cl))
    ...
    def testSet4Portal(self):
        ...
        self.assertTrue(prop == newcl, "Canonical link adapter setter NOT SET new '%s' value to '%s' propery for the portal" % (newcl, PROPERTY_LINK))
        self.assertTrue(cladapter.canonical_link == newcl, "Canonical link adapter GET '%s' canonical_link, for portal, must be: '%s'" % (cladapter.canonical_link, newcl))
        return
    ...
    def testSet4AT(self):
        ...
        self.assertTrue(prop == newcl, "Canonical link adapter setter NOT SET new '%s' value to '%s' propery for the Document" % (newcl, PROPERTY_LINK))
        self.assertTrue(cladapter.canonical_link == newcl, "Canonical link adapter GET '%s' canonical_link, for Document, must be: '%s'" % (cladapter.canonical_link, newcl))
        return
    ...
    def testDeleteProperty(self):
        ...
        self.assertFalse(hasprop(PROPERTY_LINK), 'Not deleted Canonical link customization property for the portal')
    ...
    def test_DelCustomization(self):
        ...
        del cladapter.canonical_link
        self.assertTrue(cladapter.canonical_link == self.purl(), 'After deleted Canonical link customization property not set to default value for the portal')
...
class TestConvertor(unittest.TestCase):
    ...
    def test_convertIPathToLink(self):
        ...
        self.assertEqual(result, expect, 'Got %s canonical link, expect: %s' % (result, expect))
        self.assertEqual(item.hasProperty(ICanonicalPath(item).prop), False, "canonical path' property not deleted from the object")
    ...
    def test_convertPPathToLink(self):
        ...
        expect = 'http://domain.com/www/some/path'
        self.assertEqual(result, expect, 'Got %s canonical link, expect: %s' % (result, expect))
        self.assertEqual(item.hasProperty('custom_property'), False, 'custom_property not deleted from the object')
    ...
    def test_convertBadItems(self):
        ...
        expect = "ERROR: exceptions.AttributeError: NotPropertyProviderItem instance has no attribute 'hasProperty'"
    ...
    def test_loggingGet(self):
        ...
        assert logs != ''
        self.assertEqual(logs == logs2, True, 'logs not equal: "%s" != "%s"' % (logs, logs2))
        self.convertor.convertIPathToLink(bad)
        logs3 = self.convertor.getLogs()
        self.assertEqual(logs3 > logs2, True, 'Log was not updated - last: "%s", previous: "%s"' % (logs3, logs2))
...
class TestAdaptersRegistration(unittest.TestCase):
    ...
    def test_PropertyManagerAndTraversable(self):
        ...
        self.assertNotEqual(queryAdapter(item, ICanonicalLink), None, self.cant % ICanonicalLink.__name__ + 'IPropertyManager and ITraversable.')
        self.assertNotEqual(queryAdapter(item, ICanonicalPath), None, self.cant % ICanonicalPath.__name__ + 'IPropertyManager and ITraversable.')
        return
    ...
    def test_Traversable(self):
        ...
        self.assertNotEqual(queryAdapter(item, ICanonicalLink), None, self.cant % ICanonicalLink.__name__ + 'only ITraversable.')
        self.assertNotEqual(queryAdapter(item, ICanonicalPath), None, self.cant % ICanonicalPath.__name__ + 'only ITraversable.')
        return
    ...
    def test_PropertyManager(self):
        ...
        self.assertEqual(queryAdapter(item, ICanonicalLink), None, self.doget % ICanonicalLink.__name__ + 'only IPropertyManager.')
        self.assertEqual(queryAdapter(item, ICanonicalPath), None, self.doget % ICanonicalPath.__name__ + 'only IPropertyManager.')
        return
    ...
    def test_NotProvider(self):
        ...
        self.assertEqual(queryAdapter(item, ICanonicalLink), None, self.doget % ICanonicalLink.__name__ + 'nor ITraversabe not IPropertyManager.')
        self.assertEqual(queryAdapter(item, ICanonicalPath), None, self.doget % ICanonicalPath.__name__ + 'nor ITraversabe not IPropertyManager.')
        return


def test_suite():
    return unittest.TestSuite([unittest.makeSuite(TestIndexerRegistration), unittest.makeSuite(TestDefaultCanonicalPathAdapter), unittest.makeSuite(TestDefaultCanonicalLinkAdapter), unittest.makeSuite(TestConvertor), unittest.makeSuite(TestAdaptersRegistration)])
