...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\LTTL\Segmentation.py
# Compiled at: 2017-07-03 09:04:57
# Size of source mod 2**32: 27166 bytes
...
def get_chunk(segmentation, id):
    ...
    if (
     segmentation, id) not in segments_cache:
        return
    else:
...
def add_chunk(segmentation, id, array):
    ...
    if (
     segmentation, id) in segments_cache:
...
class Segmentation(object):
    ...
    def set_data(index, value_or_ref):
        ...
        if isinstance(value_or_ref, int):
            ...
            if 0 <= index < len(Segmentation.data):
                if isinstance(value_or_ref, int):
                    if not isinstance(Segmentation.data[index], int):
    ...
    def __setitem__(self, index, segment):
        ...
        if segment.annotations is not None or len(segment.annotations) is not 0:
            self.create_anotation()
            b = get_chunk(self, -(index // CHUNK_SIZE + 1))
            b[index % CHUNK_SIZE] = np.array((self.get_annotation_tab(segment)),
              dtype=(np.int16))
            ...
            for i in range(1, self.segments_nbr_in_chunk // CHUNK_SIZE + 1):
                ...
                if a is not None:
                    for index in range(len(a)):
                        annotation = None
                        if b is not None:
                            annotation = dict([self.id_to_key[x] for x in b[(index % CHUNK_SIZE)]])
    ...
    def create_anotation(self):
        if not self.has_annotation():
            ...
            while nb_segment > CHUNK_SIZE:
                add_chunk(self, id, np.array([np.empty(0, dtype=(np.int16)) for _ in range(CHUNK_SIZE)]))
                nb_segment -= CHUNK_SIZE
                id -= 1

            add_chunk(self, id, np.array([np.empty(0, dtype=(np.int16)) for _ in range(nb_segment)]))
            self.id_to_key = list()
            self.key_to_id = dict()
    ...
    def get_annotation_tab(self, segment):
        ...
        for key, value in iteritems(segment.annotations):
            ...
            annotations_id.append([id for id in self.key_to_id[hash((key, value))] if self.id_to_key[id] == (key, value)][0])
    ...
    def add_annotation_tuple(self, my_tuple):
        ...
        if my_tuple not in [self.id_to_key[x] for x in id_list]:
            ...
            self.key_to_id[hash(my_tuple)] = self.key_to_id.get(hash(my_tuple), list()) + [
    ...
    def extend(self, segments):
        if self.segments_nbr > 0:
            new_ptrs = self._get_str_index_ptr(segments, self[(-1)], self.segments_nbr)
    ...
    def store(self):
        ...
        for index in range(nbelement_to_store):
            ...
            if self.buffer[index].annotations is not None or len(self.buffer[index].annotations) is not 0:
                self.create_anotation()
        ...
        if self.has_annotation():
            add_chunk(self, -(self.segments_nbr_in_chunk // CHUNK_SIZE + 1), ex_annotation)
    ...
    def to_string(self, formatting=None, segment_delimiter='\n', header='', footer='', humanize_addresses=False, progress_callback=None, display_all=None):
...
')
        ...
        if display_all:
            ...
            if formatting is not None:
                default_dict = dict((k, '__none__') for k in self.get_annotation_keys())
                for match in re.finditer('(?<=%\\()(.+?)(?=\\))', formatting):
                    default_dict[match.group(0)] = '__none__'

            string += segment_delimiter.join([segment.to_string(formatting, humanize_addresses, index + 1, default_dict, progress_callback) for index, segment in enumerate(self)])
            string += footer
        else:
            string = header
            default_dict = None
            ...
            string += segment_delimiter.join([self[index].to_string(formatting, humanize_addresses, index + 1, default_dict, progress_callback) for index in range(min(len(self), NUM_SEGMENTS_SUMMARY))])
            string += '\n\n[%i segments not displayed...]\n\n' % (len(self) - 2 * NUM_SEGMENTS_SUMMARY)
            string += segment_delimiter.join([self[index].to_string(formatting, humanize_addresses, index + 1, default_dict, progress_callback) for index in range(max(len(self) - NUM_SEGMENTS_SUMMARY, 0), len(self))])
            string += footer
        return string

    def to_html(self, humanize_addresses=False, progress_callback=None, display_all=None):
        ...
        if display_all is None:
            ...
            if display_all:
                string = html_header
                string += '<br/>'.join([segment.to_html(offset, index + 1, progress_callback) for index, segment in enumerate(self)])
                string += '</body></html>'
            else:
                summarized = True
            string = html_header
            string += '<br/>'.join([self[index].to_html(offset, index + 1, progress_callback) for index in range(min(len(self), NUM_SEGMENTS_SUMMARY))])
            string += '<p><i>[%i segments not displayed...]</i></p>' % (len(self) - 2 * NUM_SEGMENTS_SUMMARY)
            string += '<br/>'.join([self[index].to_html(offset, index + 1, progress_callback) for index in range(max(len(self) - NUM_SEGMENTS_SUMMARY, 0), len(self))])
            string += '</body></html>'
    ...
    def get_annotation_keys(self):
        ...
        annotation_keys = set()
        for segment in self:
            annotation_keys = annotation_keys.union(list(segment.annotations))
