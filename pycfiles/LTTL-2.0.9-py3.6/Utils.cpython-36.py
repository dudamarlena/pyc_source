...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\LTTL\Utils.py
# Compiled at: 2019-08-02 09:59:11
# Size of source mod 2**32: 12133 bytes
"""Module Utils.py
...
def get_variety(dictionary, unit_weighting=False, category_weighting=False, category_delimiter=None):
    ...
    if category_delimiter is None:
        ...
        if not unit_weighting and not category_weighting:
            ...
            for k, v in iteritems(dictionary):
                category, unit = k.split(category_delimiter, 1)
                category_dict[category] = category_dict.get(category, 0) + v
                units_in_category_dict[(category, unit)] = units_in_category_dict.get((category, unit), 0) + v
                if unit_weighting:
            ...
            for category in category_dict:
                if category_weighting:
                    ...
                    local_unit_dict = tuple_to_simple_dict(units_in_category_dict, category)
                    if unit_weighting:
...
def get_expected_subsample_variety(dictionary, subsample_size):
    ...
    for freq in dictionary.values():
        expected_variety -= _prob_no_occurrence(sample_size, subsample_size, freq, num_subsamples)

    return expected_variety


@lru_cache(maxsize=None)
def _prob_no_occurrence(sample_size, subsample_size, sample_freq, num_subsamples):
    ...
    if sample_freq > sample_size - subsample_size:
        ...
        return binom((sample_size - sample_freq),
          subsample_size,
          exact=True) / num_subsamples
...
def tuple_to_simple_dict(dictionary, key):
    ...
    """
    return dict((k[1], v) for k, v in iteritems(dictionary) if k[0] == key if v > 0)
...
def tuple_to_simple_dict_transpose(dictionary, key):
    ...
    NB: keys with zero value are removed.
    """
    return dict((k[0], v) for k, v in iteritems(dictionary) if k[1] == key if v > 0)
...
def get_unused_char_in_segmentation(segmentation, annotation_key=None):
    ...
    for segment in segmentation:
        if annotation_key:
            text = segment.annotations.get(annotation_key, '__none__')
        else:
...
def prepend_unit_with_category(segmentation, category_delimiter, new_annotation_key, category_annotation_key, unit_annotation_key=None):
    ...
    for segment in segmentation:
        new_segment = Segment(segment.str_index, segment.start, segment.end, segment.annotations.copy())
        ...
        if category_annotation_key:
            category = segment.annotations.get(category_annotation_key, '__none__')
        else:
            category = segment.get_content()
        new_segment.annotations[new_annotation_key] = category_delimiter.join([category, unit])
...
if __name__ == '__main__':
    ...
    print(get_variety(recoded_unit_dict,
      unit_weighting=True,
      category_delimiter='#'))
    print(get_variety(recoded_unit_dict,
