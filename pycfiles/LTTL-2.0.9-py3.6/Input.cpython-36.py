...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\LTTL\Input.py
# Compiled at: 2016-06-29 04:54:12
...
import numpy as np
__version__ = '1.0.0'
MAX_TEXT_BEFORE_COMPRESS = 5000000000000

class Input(Segmentation):
    __doc__ = 'A class for representing a segmentation which imports a new string in\n    the system.\n    '
    ...
    def update(self, text=None, label=None, compressed=None):
        ...
        if compressed is None:
            ...
            if compressed:
                Segmentation.set_data(self[0].str_index, None if text is None else CompressedString(text))
            else:
...
class CompressedString:
    __doc__ = 'Compressing string by re-mapping the characters to an 8bit range.\n    If more than 256 unique characters are used in the same text, mapping to\n    16bits\n    '
    ...
    def __getslice__(self, i, j):
        """Return a slice of the string as another CompressedString with a
        view on this one
        """
