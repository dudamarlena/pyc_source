...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\LTTL\Table.py
# Compiled at: 2020-02-21 03:29:17
# Size of source mod 2**32: 52048 bytes
"""Module Table.py
...
class Table(object):
    __doc__ = 'Base class for tables in LTTL.'

    def __init__(self, row_ids, col_ids, values, header_row_id='__col__', header_row_type='string', header_col_id='__row__', header_col_type='string', col_type=None, class_col_id=None, missing=None, _cached_row_id=None):
        """Initialize a Table.
    ...
    def to_string(self, output_orange_headers=False, col_delimiter='\t', row_delimiter=None):
...
' elsewhere)
        ...
        row_strings = ('%s%s%s%s' % (row_delimiter, row_id, col_delimiter,
         col_delimiter.join([text(self.values.get((row_id, col_id), missing)) for col_id in self.col_ids])) for row_id in self.row_ids)
    ...
    if sys.version_info.major >= 3:
        ...
        def to_orange_table(self, encoding='iso-8859-15'):
            ...
            ordered_cols = [
             self.header_col_id]
            ordered_cols.extend([x for x in self.col_ids if x != self.class_col_id])
            if self.class_col_id:
                ...
                ordered_cols = list(sorted(ordered_cols,
                             ...
                             IntPivotCrosstab, IntWeightedFlatCrosstab)):
                                ...
                                if col_type == 'discrete':
                                    ...
                                    if col_id == self.header_col_id:
                                        ...
                                        for row_id in self.row_ids:
                                            if (
                                             row_id, col_id) in self.values:
                                                value = text(self.values[(row_id, col_id)])
                                                if value not in values:
                                                    values.append(value)

                                    var = Orange.data.DiscreteVariable(name=str_col_id,
                                      values=values)
                                    if col_id == self.class_col_id:
                                        class_vars.append(var)
                                    else:
        ...
        def to_orange_table(self, encoding='iso-8859-15'):
            ...
            for col_id in ordered_cols:
                str_col_id = text(col_id)
                encoded_col_id = str_col_id.encode(encoding,
                ...
                if col_type == 'string':
                    ...
                    if col_type == 'continuous':
                        ...
                        if col_type == 'discrete':
                            ...
                            feature = Orange.feature.Discrete(name=encoded_col_id,
            ...
            for row_id in self.row_ids:
                ...
                for col_id in ordered_cols:
                    ...
                    if value:
                        value = text(value).encode(encoding,
                          errors='xmlcharrefreplace')
                    row_data.append(value)
    ...
    def to_sorted(self, key_col_id=None, reverse_rows=False, key_row_id=None, reverse_cols=False):
        ...
        if key_col_id is not None:
            if key_col_id == self.header_col_id:
                ...
                values = [self.values.get((row_id, key_col_id), self.missing) for row_id in self.row_ids]
                new_row_ids.extend([x[1] for x in sorted((zip(values, self.row_ids)),
        ...
        if key_row_id is not None:
            if key_row_id == self.header_row_id:
                ...
                values = [self.values.get((key_row_id, col_id), self.missing) for col_id in self.col_ids]
                new_col_ids.extend([x[1] for x in sorted((zip(values, self.col_ids)),
        ...
        if isinstance(self, IntPivotCrosstab):
            ...
            if isinstance(self, PivotCrosstab):
                ...
                if isinstance(self, FlatCrosstab):
                    creator = FlatCrosstab
                else:
                    if isinstance(self, IntWeightedFlatCrosstab):
                        creator = IntWeightedFlatCrosstab
                    else:
                        if isinstance(self, WeightedFlatCrosstab):
                            creator = WeightedFlatCrosstab
                        else:
                            creator = Table
        return creator(new_row_ids, new_col_ids, self.values.copy(), self.header_row_id, self.header_row_type, self.header_col_id, self.header_col_type, self.col_type.copy(), self.class_col_id, self.missing, self._cached_row_id)
                ...
                if isinstance(self, FlatCrosstab):
                    ...
                    if isinstance(self, IntWeightedFlatCrosstab):
                        ...
                        if isinstance(self, WeightedFlatCrosstab):
                            ...
                            creator = Table
        return creator(self.row_ids[:], self.col_ids[:], self.values.copy(), self.header_row_id, self.header_row_type, self.header_col_id, self.header_col_type, self.col_type.copy(), self.class_col_id, self.missing, self._cached_row_id)


class Crosstab(Table):
    __doc__ = 'Base class for crosstabs (i.e. contingency tables).'
...
class PivotCrosstab(Crosstab):
    __doc__ = "A class for storing crosstabs in 'pivot' format.\n\n    Example:\n               --------+-------+\n               | unit1 | unit2 |\n    +----------+-------+-------+\n    | context1 |   1   |   3   |\n    +----------+-------+-------+\n    | context2 |   4   |   2   |\n    +----------+-------+-------+\n    "

    def to_transposed(self):
        """Return a transposed copy of the crosstab"""
        new_col_ids = self.row_ids[:]
        return PivotCrosstab(self.col_ids[:], new_col_ids, dict((tuple(reversed(key)), count) for key, count in iteritems(self.values)), self.header_col_id, self.header_col_type, self.header_row_id, self.header_row_type, dict([(col_id, 'continuous') for col_id in new_col_ids]), None, self.missing, self.header_col_id)
    ...
    def to_weighted_flat(self, progress_callback=None):
        ...
        new_header_col_type = 'continuous'
        ...
        if num_row_ids > 1:
            ...
            new_cached_row_id = self.header_col_id
        ...
        return WeightedFlatCrosstab(new_row_ids,
          new_col_ids,
          new_values,
    ...
    @classmethod
    def from_numpy(cls, row_ids, col_ids, np_array, header_row_id='__col__', header_row_type='string', header_col_id='__row__', header_col_type='string', col_type=None, class_col_id=None, missing=None, _cached_row_id=None):
        ...
        if cls == IntPivotCrosstab:
            if not issubclass(np_array.dtype.type, np.integer):
                raise ValueError('Cannot cast non-integer numpy array to IntPivotCrosstab.')
        return cls(row_ids, col_ids, table_values, header_row_id, header_row_type, header_col_id, header_col_type, col_type, class_col_id, missing, _cached_row_id)


class IntPivotCrosstab(PivotCrosstab):
    __doc__ = "A class for storing crosstabs in 'pivot' format, with integer values."
    ...
    def to_normalized(self, mode='rows', type='l1', progress_callback=None):
        ...
        if mode == 'rows':
            ...
            for row_id in self.row_ids:
                row_values = [self.values.get((row_id, col_id), 0) for col_id in col_ids]
                if type == 'l1':
                    ...
                    if denominator > 0:
                        new_values.update(zip([(row_id, col_id) for col_id in col_ids], [v / denominator for v in row_values]))
                    else:
                        new_values.update(zip([(row_id, col_id) for col_id in col_ids], [0 for v in row_values]))
                if progress_callback:
                    progress_callback()
            ...
            if mode == 'columns':
                ...
                for col_id in self.col_ids:
                    col_values = [self.values.get((row_id, col_id), 0) for row_id in row_ids]
                    if type == 'l1':
                        ...
                        if denominator > 0:
                            new_values.update(zip([(row_id, col_id) for row_id in row_ids], [v / denominator for v in col_values]))
                        else:
                            new_values.update(zip([(row_id, col_id) for row_id in row_ids], [0 for v in col_values]))
                    if progress_callback:
                        progress_callback()
                ...
                if mode == 'table':
                    table_class = PivotCrosstab
                    values = reduce(list.__add__, [[self.values.get((row_id, col_id), 0) for row_id in self.row_ids] for col_id in self.col_ids])
                    if type == 'l1':
                        ...
                        if denominator > 0:
                            new_values.update(zip(reduce(list.__add__, [[(row_id, col_id) for row_id in self.row_ids] for col_id in self.col_ids]), [v / denominator for v in values]))
                        else:
                            new_values.update(zip(reduce(list.__add__, [[(row_id, col_id) for row_id in self.row_ids] for col_id in self.col_ids]), [0 for v in values]))
                else:
                    if mode == 'presence/absence':
                            ...
                            for row_id in row_ids:
                                ...
                                for col_idx in xrange(len(col_ids)):
                                    ...
                                    if freq_under_indep > 0:
                                        new_values[(row_id, col_ids[col_idx])] = row_values[col_idx] * total / freq_under_indep
                                    if progress_callback:
                                        progress_callback()
                            ...
                            if mode == 'TF-IDF':
                                ...
                                for col_id in self.col_ids:
                                    ...
                                    if df > 0:
                                        ...
                                        new_values.update(zip([(row_id, col_id) for row_id in row_ids], [v * idf for v in col_values]))
                                    else:
                                        new_values.update(zip([(row_id, col_id) for row_id in row_ids], [0 for v in col_values]))
                                    if progress_callback:
                                        progress_callback()

        return table_class(list(self.row_ids), list(self.col_ids), new_values, self.header_row_id, self.header_row_type, self.header_col_id, self.header_col_type, dict(self.col_type), None, self.missing, self._cached_row_id)
    ...
    def to_document_frequency(self, progress_callback=None):
        ...
        document_freq = dict()
        for col_id in self.col_ids:
            unit_profile = tuple_to_simple_dict_transpose(self.values, col_id)
            document_freq[(context_type, col_id)] = len(unit_profile)
            if progress_callback:
                progress_callback()

        return IntPivotCrosstab([
         context_type], self.col_ids[:], document_freq, '__unit__', 'string', '__context__', 'string', self.col_type.copy(), None, 0, None)
    ...
    def to_association_matrix(self, bias='none', progress_callback=None):
        ...
        exchange = np.dot(np.transpose(freq), np.dot(np.diag(1 / sum_row), freq)) / total_freq
        if bias == 'frequent':
        ...
        for col_id_idx1 in xrange(len(col_ids)):
            ...
            values.update(dict(((col_id1, col_ids[i]), output_matrix[(col_id_idx1, i)]) for i in xrange(len(col_ids))))
            if progress_callback:
                progress_callback()

        new_header_row_id = self.header_row_id[:-2] + '2' + self.header_row_id[-2:]
        return PivotCrosstab((self.col_ids[:]),
          (self.col_ids[:]),
          values,
          new_header_row_id,
    ...
    def to_flat(self, progress_callback=None):
        ...
        return FlatCrosstab(new_row_ids,
          new_col_ids,
          new_values,
          header_col_id=new_header_col_id,
          header_col_type=new_header_col_type,
    ...
    def to_weighted_flat(self, progress_callback=None):
        ...
        integer values
        """
        weighted_flat = super(IntPivotCrosstab, self).to_weighted_flat(progress_callback=progress_callback)
        weighted_flat.__class__ = IntWeightedFlatCrosstab
        return weighted_flat


class FlatCrosstab(Crosstab):
    __doc__ = "A class for storing crosstabs in 'flat' format in LTTL.\n\n    Example:\n    +----------+-------+\n    | context  | unit  |\n    +==========+=======+\n    | context1 | unit1 |\n    +----------+-------+\n    | context1 | unit2 |\n    +----------+-------+\n    | context1 | unit2 |\n    +----------+-------+\n    | context1 | unit2 |\n    +----------+-------+\n    | context2 | unit1 |\n    +----------+-------+\n    | context2 | unit1 |\n    +----------+-------+\n    | context2 | unit1 |\n    +----------+-------+\n    | context2 | unit1 |\n    +----------+-------+\n    | context2 | unit2 |\n    +----------+-------+\n    | context2 | unit2 |\n    +----------+-------+\n    "
    ...
    def to_pivot(self, progress_callback=None):
        ...
        new_col_ids = Crosstab.get_unique_items([self.values[(row_id, new_header_row_id)] for row_id in self.row_ids])
        new_row_ids = list()
        new_values = dict()
        if len(self.col_ids) > 1:
            new_header_col_id = self.col_ids[1]
            new_row_ids.extend(Crosstab.get_unique_items([self.values[(row_id, new_header_col_id)] for row_id in self.row_ids]))
            for row_id in self.row_ids:
                pair = (
        ...
        return IntPivotCrosstab(new_row_ids, new_col_ids, new_values, new_header_row_id, new_header_row_type, new_header_col_id, new_header_col_type, dict([(col_id, 'continuous') for col_id in new_col_ids]), None, self.missing, self._cached_row_id)
    ...
    def to_weighted_flat(self, progress_callback=None):
        ...
        new_col_ids.append('__weight__')
        new_col_type['__weight__'] = 'continuous'
        return IntWeightedFlatCrosstab(new_row_ids, new_col_ids, new_values, self.header_row_id, self.header_row_type, self.header_col_id, self.header_col_type, new_col_type, None, self.missing, self._cached_row_id)


class WeightedFlatCrosstab(Crosstab):
    __doc__ = "A class for storing crosstabs in 'weighted and flat' format.\n\n    Example:\n    +----------+-------+-------+\n    | context  | unit  | count |\n    +==========+=======+=======+\n    | context1 | unit1 |   1   |\n    +----------+-------+-------+\n    | context1 | unit2 |   3   |\n    +----------+-------+-------+\n    | context2 | unit1 |   4   |\n    +----------+-------+-------+\n    | context2 | unit2 |   2   |\n    +----------+-------+-------+\n    "
    ...
    def to_pivot(self, progress_callback=None):
        ...
        return PivotCrosstab(new_row_ids, new_col_ids, new_values, new_header_row_id, new_header_row_type, new_header_col_id, new_header_col_type, dict([(col_id, 'continuous') for col_id in new_col_ids]), None, self.missing)


class IntWeightedFlatCrosstab(WeightedFlatCrosstab):
    __doc__ = "A class for storing crosstabs in 'weighted and flat' format in LTTL,\n    where weights are integer values.\n    "

    def to_pivot(self, progress_callback=None):
        """Return a copy of the crosstab in 'pivot' format, with int values"""
        pivot = super(IntWeightedFlatCrosstab, self).to_pivot(progress_callback=progress_callback)
    ...
    def to_flat(self, progress_callback=None):
        ...
        return FlatCrosstab(new_row_ids, new_col_ids, new_values, self.header_row_id, self.header_row_type, self.header_col_id, self.header_col_type, new_col_type, None, self.missing, self._cached_row_id)