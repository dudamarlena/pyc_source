...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/quintagroup/catalogupdater/tests.py
...
class TestUtility(TestCase):
    ...
    def testSingleColumnUpdate(self):
        ...
        self.assertFalse(mydoc.test_column == 'my_doc', "'test_column' metadata updated in catalog before utility call: '%s'" % mydoc.test_column)
        cu = queryUtility(ICatalogUpdater, name='catalog_updater')
        cu.updateMetadata4All(self.catalog, 'test_column')
        mydoc = self.catalog.unrestrictedSearchResults(id='my_doc')[0]
        self.assertTrue(mydoc.test_column == 'my_doc', "'test_column' metadata has wrong metadata in catalog: '%s'" % mydoc.test_column)
    ...
    def testOnlyPointedColumnUpdate(self):
        ...
        self.my_doc.setTitle('New my document')
        cu = queryUtility(ICatalogUpdater, name='catalog_updater')
        cu.updateMetadata4All(self.catalog, 'test_column')
        mydoc = self.catalog.unrestrictedSearchResults(id='my_doc')[0]
        self.assertTrue(mydoc.Title == 'My document', "Other metadata updated: Title='%s'" % mydoc.Title)
    ...
    def testAllRecordsUpdate(self):
        ...
        self.assertTrue(num_updated == num_recs, 'Only %d records updated, must be - %d' % (num_updated, num_recs))
    ...
    def testTransaction(self):
        ...
        cu = queryUtility(ICatalogUpdater, name='catalog_updater')
        cu.updateMetadata4All(self.catalog, 'test_column')
        self.assertTrue(sp_commits == num_subcommits, 'Wrong number of transaction subcommits: actual:%d, must be: %d' % (sp_commits, num_subcommits))
        transaction.savepoint = orig_trsp


def test_suite():
    return unittest.TestSuite([unittest.makeSuite(TestUtility)])
