...
# Embedded file name: build/bdist.linux-x86_64/egg/anybox/buildbot/capability/version.py
...
class Version(object):
    ...
    def parse(cls, as_string):
        ...
        if as_string is None:
            ...
            if len(split) > 2:
                raise VersionParseError(as_string, 'Only one dash is allowed, is this really a version?')
            elif len(split) == 2:
...
class VersionFilter(object):
    ...
    def __eq__(self, other):
        return (
         self.cap, self.criteria) == (other.cap, other.criteria)
    ...
    def boolean_parse(cls, reqline):
        ...
        if len(split) < 2:
            raise VersionParseError(reqline, 'Not enough tokens. Missing operator?')
        vreq = (
         split[0], Version.parse(split[1]))
        if len(split) == 2:
            return vreq
        return (
         'AND', vreq, cls.boolean_parse(split[2]))

    def boolean_match(self, version, criteria):
        op = criteria[0]
        if op.upper() == 'OR':
            return self.boolean_match(version, criteria[1]) or self.boolean_match(version, criteria[2])
        if op.upper() == 'AND':
            return self.boolean_match(version, criteria[1]) and self.boolean_match(version, criteria[2])
        crit_version = criteria[1]
        return not (op == '>=' and not version >= crit_version or op == '==' and not version == crit_version or op == '<=' and not version <= crit_version or op == '<' and not version < crit_version or op == '>' and not version > crit_version)
