...
# Embedded file name: build/bdist.linux-x86_64/egg/anybox/buildbot/capability/dispatcher.py
# Compiled at: 2017-10-20 04:47:35
__doc__ = 'Core functionnality to manipulate capabilities.'
import re
...
RE_PROP_CAP_OPT = re.compile('cap\\((\\w*)\\)')
...
class BuilderDispatcher(object):
    ...
    def __init__(self, workers, capabilities):
        self.all_workers = dict((worker.workername, worker) for worker in workers)
        ...
        * a return value of::
        ...
        for cap_name in cap_names:
            ...
            if capability is None:
                continue
            factory.addStep(SetCapabilityProperties(cap_name, description=[
             'Setting', cap_name, 'properties'], descriptionDone=[
             'Set', cap_name, 'properties'], name='props_' + cap_name, capability_version_prop=capability.get('version_prop')))
    ...
    def make_builders(self, name, factory, build_for=(), build_requires=(), **kw):
        ...
        if build_requires:
            base_conf['properties'] = dict(build_requires=[ str(req) for req in build_requires ])
        preconfs = [
         base_conf]
        for version_filter in build_for:
            preconfs = self.dispatch_builders_by_capability(preconfs, version_filter)
    ...
    def dispatch_builders_by_capability(self, builders, cap_vf):
        ...
        for builder in builders:
            for cap_version, workernames in self.split_workers_by_capability(cap, builder['workernames']).items():
                if cap_vf is not None and not cap_vf.match(Version.parse(cap_version)):
                    continue
    ...
    def filter_workers_by_requires(self, requires):
        ...
        return [ workername for workername, worker in self.all_workers.items() if does_meet_requirements(worker.properties['capability'], requires) and self.only_if_requires(worker).issubset(require_names)
               ]