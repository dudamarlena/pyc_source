...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/Products/HaufeWingDBG/DebugHttpServer.py
# Compiled at: 2012-12-06 03:42:06
import sys, os
if sys.platform != 'win32':
    ...
    if sys.hexversion >= 33685504:
        ...
        from FCNTL import F_SETFD
import asyncore, errno, thread, socket, select
            ...
            try:
                ...
                if callable(name):
                    ...
                    if hasattr(ZServerPublisher, 'publish_module'):
                        ZServerPublisher.publish_module(name, request=request, response=response)
                    else:
  ...
  our custom dispatchers and provide access to the debugger for them. """
    ...
    def _RegisterSocket(self, s, cb_fct):
      ...
      the debug server will substitute the socket that is used in its code."""
        ...
        self.fErr.out('################ Registered socket with Zope: ', s, 'sockname =', s.getsockname())
        return dispatcher
...
class CDebugZHttpServer(zhttp_server):
    ...
    def __init__(self, ip, port, resolver=None, logger_object=None, debugger=None, async_interrupt=None):
        ...
        socket_hook = CWingDbgSocketHook(debugger.fErr, self.zrend, async_interrupt)
    ...
    def __Shutdown(self):
        ...
        for dispatcher in asyncore.socket_map.values():
            if isinstance(dispatcher, CDebugZHttpChannel) and dispatcher.server == self:
                dispatcher.close()
