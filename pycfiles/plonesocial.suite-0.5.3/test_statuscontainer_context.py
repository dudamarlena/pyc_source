...
# Embedded file name: /home/gyst/plonesocial.buildout/src/plonesocial.microblog/plonesocial/microblog/tests/test_statuscontainer_context.py
...
class TestStatusContainer(unittest.TestCase):
    ...
    def test_items_context(self):
        ...
        container.add(su2)
        container.add(su3)
        values = [ x[1] for x in container.context_items(mockcontext, nested=False)
                 ]
    ...
    def test_items_tag_context(self):
        ...
        values = [ x[1] for x in container.context_items(mockcontext, tag='bar', nested=False)
                 ]
        self.assertEqual([su3, su2], values)
        values = [ x[1] for x in container.context_items(mockcontext, tag='foo', nested=False)
                 ]
        self.assertEqual([su2], values)
    ...
    def test_values_tag_context(self):
        ...
        container.add(su2)
        container.add(su3)
        values = list(container.context_values(mockcontext, nested=False))
        self.assertEqual([su3, su2], values)
        values = list(container.context_values(mockcontext, tag='bar', nested=False))
        self.assertEqual([su3, su2], values)
        values = list(container.context_values(mockcontext, tag='foo', nested=False))
        self.assertEqual([su2], values)
    ...
    def test_allowed_status_keys(self):
        ...
        uid_blacklist = [
         container._context2uuid(mockcontext1)]
        values = [ container.get(id) for id in container._allowed_status_keys(uid_blacklist)
                 ]
...
class TestNestedStatusContainer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        grandchild_id = self.child_context.invokeFactory('Folder', 'grandchild')
