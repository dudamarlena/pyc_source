...
# Embedded file name: /home/gyst/plonesocial.buildout/src/plonesocial.microblog/plonesocial/microblog/tests/test_uuid_integration.py
# Compiled at: 2013-07-03 09:10:15
...
class TestUUIDIntegration(unittest.TestCase):
    ...
    def test_context_api(self):
        ...
        su4 = StatusUpdate('test', context=mockcontext2, userid='bernard')
        container.add(su1)
        container.add(su2)
        container.add(su3)
        container.add(su4)
        values = [ x[1] for x in container.context_items(mockcontext1, tag='foo')
                 ]
        self.assertEqual([su1], values)
        values = [ x[1] for x in container.context_items(mockcontext1, tag='bar')
                 ]
        self.assertEqual([], values)
        values = [ x[1] for x in container.context_items(mockcontext2, tag='foo')
                 ]
