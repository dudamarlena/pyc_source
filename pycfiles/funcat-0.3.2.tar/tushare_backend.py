...
# [GCC 8.4.0]
# Embedded file name: /Users/cedricporter/git/funcat/funcat/data/tushare_backend.py
# Compiled at: 2017-04-24 23:16:55
...
class TushareDataBackend(DataBackend):
    ...
    def get_price(self, order_book_id, start, end, freq):
        ...
        is_index = False
        if order_book_id.startswith('0') and order_book_id.endswith('.XSHG') or order_book_id.startswith('3') and order_book_id.endswith('.XSHE'):
            is_index = True
        ktype = freq
        if freq[(-1)] == 'm':
            ktype = freq[:-1]
        ...
        if freq[(-1)] == 'm':
            df['datetime'] = df.apply(lambda row: int(row['date'].split(' ')[0].replace('-', '')) * 1000000 + int(row['date'].split(' ')[1].replace(':', '')) * 100, axis=1)
        elif freq == '1d':
