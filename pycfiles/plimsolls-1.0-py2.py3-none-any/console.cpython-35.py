...
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/plim/console.py
...
def plimc(args=None, stdout=None):
    ...
    cli_parser.add_argument('-p', '--preprocessor', default='plim:preprocessor', help='Preprocessor instance that will be used for parsing the template')
    cli_parser.add_argument('-H', '--html', action='store_true', help='Render HTML output instead of Mako template')
    cli_parser.add_argument('-V', '--version', action='version', version='Plim {}'.format(get_distribution('Plim').version))
    if args is None:
        args = sys.argv[1:]
    ...
    if args.html:
        ...
        lookup = TemplateLookup(directories=[root_dir], input_encoding=args.encoding, output_encoding=args.encoding, preprocessor=preprocessor)
