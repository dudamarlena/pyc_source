...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/neo/tests.py
...
from neo.utils import BRAND_ID, PROMO_CODE, ConsumerWrapper, dataloadtool_schema, normalize_username
...
class _MemberTestCase(object):
    ...
    @classmethod
    def setUpClass(cls):
        country, created = Country.objects.get_or_create(title='United States of America', slug='united-states-of-america', country_code='US')
        cls.member_attrs = {'first_name': 'firstname', 
           'last_name': 'lastname', 
           'dob': timezone.now().date() - timedelta(days=8030), 
           'country': country, 
           'address': 'address', 
           'city': 'city', 
           'province': 'province', 
...
class NeoTestCase(_MemberTestCase, TestCase):
    ...
    def test_update_member(self):
        member = self.create_member()
        new_dob = timezone.now().date() - timedelta(days=8760)
        new_country = Country.objects.create(title='South Africa', slug='south-africa', country_code='ZA')
        new_gender = 'M'
    ...
    def test_neoprofile_password_reset(self):
        ...
        self.assertEqual(api.authenticate(member.neoprofile.login_alias, 'password_new'), member.neoprofile.consumer_id)
    ...
    def test_add_promo_code(self):
        ...
        """
        member = self.immutable_member
        api.add_promo_code(member.neoprofile.consumer_id, 'added_promo_code', username=member.username, password=member.neoprofile.password)
        consumer = api.get_consumer_profile(member.neoprofile.consumer_id, username=member.username, password=member.neoprofile.password)
    ...
    def test_add_consumer_preferences(self):
        ...
        member = self.immutable_member
        cw = ConsumerWrapper()
        cw._set_preference(answer=AnswerType(OptionID=2), category_id=10, question_id=112, mod_flag=constants.modify_flag['INSERT'])
        cw.consumer.Preferences.PromoCode = 'special_preference_promo'
        api.update_consumer_preferences(member.neoprofile.consumer_id, cw.consumer.Preferences, username=member.username, password=member.neoprofile.password, category_id=10)
        prefs = api.get_consumer_preferences(member.neoprofile.consumer_id, username=member.username, password=member.neoprofile.password, category_id=10)
    ...
    def test_logging(self, mock_get, mock_post, mock_put, mock_handle):
        ...
        self.assertIn('create_consumer(consumer=', mock_handle.call_args_list[0][0][0].getMessage())
        self.assertIn("complete_registration(consumer_id='1'", mock_handle.call_args_list[1][0][0].getMessage())
        member.receive_sms = not member.receive_sms
        member.save()
        self.assertEqual(mock_handle.call_count, 3)
        self.assertIn("update_consumer(consumer_id='1', consumer=", mock_handle.call_args_list[2][0][0].getMessage())
        self.client.login(username=member.username, password='password')
        self.assertEqual(mock_handle.call_count, 4)
        self.assertIn("authenticate(username=%s, password='***', " % repr(unicode(member.neoprofile.login_alias)), mock_handle.call_args_list[3][0][0].getMessage())
        mocked_response.status_code = 500
        member.receive_sms = not member.receive_sms
        with self.assertRaises(Exception):
            member.save()
        self.assertEqual(mock_handle.call_count, 5)
        self.assertIn("update_consumer(consumer_id='1', consumer=", mock_handle.call_args_list[4][0][0].getMessage())
...
class DataLoadToolExportTestCase(_MemberTestCase, TestCase):
    ...
    def assertValidates(self, xml):
        ...
        self.assertTrue(self.consumers_schema.validate(tree), ('Validation failed: {0}').format(self.consumers_schema.error_log.last_error))
    ...
    def expected_consumer(self, member, password, **kwargs):
        ...
        consumerprofile = E.ConsumerProfile(E.Title(''), E.FirstName(member.first_name), E.LastName(member.last_name), E.DOB(member.dob.strftime('%Y-%m-%d')), E.Gender(constants.gender[genderkey]), E.Address(E.Address1(member.address), E.City(member.city), E.Country(member.country.country_code), E.ZipCode(member.zipcode), E.AddressType(constants.address_type['HOME']), E.StateOther('province'), E.ModifyFlag(constants.modify_flag['INSERT'])), E.Phone(E.PhoneNumber(member.mobile_number), E.PhoneType(constants.phone_type['MOBILE']), E.ModifyFlag(constants.modify_flag['INSERT'])), E.PromoCode(PROMO_CODE), E.Email(E.EmailId(member.mobile_number), E.EmailCategory(constants.email_category['MOBILE_NO']), E.IsDefaultFlag(1), E.ModifyFlag(constants.modify_flag['INSERT'])), E.Email(E.EmailId(member.email), E.EmailCategory(constants.email_category['PERSONAL']), E.IsDefaultFlag(0), E.ModifyFlag(constants.modify_flag['INSERT'])))
        preferences = E.Preferences(E.QuestionCategory(E.CategoryID(constants.question_category['GENERAL']), E.QuestionAnswers(E.QuestionID(92), E.Answer(E.OptionID(222), E.ModifyFlag(constants.modify_flag['INSERT'])))), E.QuestionCategory(E.CategoryID(constants.question_category['OPTIN']), E.QuestionAnswers(E.QuestionID(64), E.Answer(E.OptionID(99), E.ModifyFlag(constants.modify_flag['INSERT']), E.BrandID(BRAND_ID), E.CommunicationChannel(constants.comm_channel['EMAIL'])), E.Answer(E.OptionID(99), E.ModifyFlag(constants.modify_flag['INSERT']), E.BrandID(BRAND_ID), E.CommunicationChannel(constants.comm_channel['SMS'])))))
        credentials = [
         E.LoginName(member.username.lower())]
        if password:
            credentials.append(E.Password(password))
        useraccount = E.UserAccount(E.LoginCredentials(*credentials))
    ...
    def expected_consumers(self, members):
        ...
        E = objectify.ElementMaker(annotate=False)
        _consumers = [ self.expected_consumer(member, member.neoprofile.password, recordNumber=str(i)) for i, member in enumerate(members)
                     ]
    ...
    def test_dataloadtool_export(self):
        ...
        """
        members = [
         self.create_member_partial(commit=False), self.create_member_partial(commit=False)]
        members[0].gender = 'F'
        members[1].gender = 'M'
        members[0].save()
        members[1].save()
        consumers = self._dataloadtool_export(Member.objects.filter(pk__in=(members[0].pk, members[1].pk)))
        self.assertEqual(objectify.dump(self.expected_consumers(members)), objectify.dump(consumers))
    ...
    def test_dataloadtool_export_unicode(self):
        ...
        member.gender = 'F'
        member.first_name = 'fïrstnâmé'
        member.save()
        consumers = self._dataloadtool_export(Member.objects.filter(pk=member.pk))
        self.assertEqual(objectify.dump(self.expected_consumers([member])), objectify.dump(consumers))
    ...
    def test_password_callback(self):
        ...
        def mock_password(given_member):
            ...
            self.assertIn(given_member.username, passwords, ('Called with unexpected member: {0!r}').format(given_member))
            return passwords[given_member.username]

        consumers = self._dataloadtool_export(Member.objects.filter(pk__in=(m1.pk, m2.pk)), password_callback=mock_password)
        self.assertEqual(objectify.dump(expected), objectify.dump(consumers))
...
class DataLoadToolExportCommandTestCase(_MemberTestCase, TestCase):
    ...
    def create_member_named(self, first_name, with_neoprofile=True):
        ...
        if with_neoprofile:
            ...
            Member.save = stashed_save
        if with_neoprofile:
            assert NeoProfile.objects.filter(user=m).exists()
        else:
            try:
    ...
    def test_nomembers(self):
        ...
        """
        xml = self._call_command()
        self.assertEqual(objectify.dump(objectify.fromstring(xml)), objectify.dump(objectify.E.Consumers()))
    ...
    def test_members(self):
        ...
        self.assertEqual(set(c.ConsumerProfile.FirstName for c in consumers.Consumer), set(['foo', 'bar', 'baz']))
    ...
    def test_neoprofile_members(self):
        ...
        consumers = self._call_command_validated()
        self.assertEqual(set(c.ConsumerProfile.FirstName for c in consumers.Consumer), set(['foo', 'bar']))
        consumers = self._call_command_validated(all=True)
        self.assertEqual(set(c.ConsumerProfile.FirstName for c in consumers.Consumer), set(['foo', 'bar', 'NEO member']))
