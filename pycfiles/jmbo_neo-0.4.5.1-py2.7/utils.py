...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/neo/utils.py
...
from neo.constants import country_option_id, address_type, gender, modify_flag, phone_type, email_category, comm_channel, question_category
from neo.xml import Consumer, ConsumerProfileType, PreferencesType, UserAccountType, EmailDetailsType, PhoneDetailsType, AnswerType, CategoryType, LoginCredentialsType, QuestionAnswerType, AddressDetailsType
BRAND_ID = getattr(settings, 'NEO')['BRAND_ID']
PROMO_CODE = getattr(settings, 'NEO')['PROMO_CODE']

def normalize_username(username):
    normal = re.sub('[ +]', '', username.lower())
...
class ConsumerWrapper(object):
        ...
        if answers:
            for a in answers:
                if a.BrandID == brand_id and a.CommunicationChannel == comm_channel:
                    a.OptionID = 1 if value else 99
                    a.ModifyFlag = mod_flag
                    updated = True
                    break

        if not updated:
            answer = AnswerType(OptionID=1 if value else 99, BrandID=brand_id, CommunicationChannel=comm_channel)
    ...
    @property
    def dob(self):
        if self._consumer.ConsumerProfile is not None and self._consumer.ConsumerProfile.DOB is not None:
    ...
    def gender(self):
        if self._consumer.ConsumerProfile is not None and self._consumer.ConsumerProfile.Gender is not None:
            if self._consumer.ConsumerProfile.Gender == gender['MALE']:
                return 'M'
            return 'F'
        return
    ...
    def set_address(self, address_line, city, state, zipcode, country, mod_flag=modify_flag['INSERT']):
        if country:
            ...
            if not updated:
                profile.add_Address(AddressDetailsType(AddressType=address_type['HOME'], Address1=address_line if address_line else 'null', City=city if city else 'null', StateOther=state if state else 'null', ZipCode=zipcode if zipcode else 'null', Country=country.country_code, ModifyFlag=mod_flag))
    ...
    def set_email(self, email, mod_flag=modify_flag['INSERT']):
        if email:
            ...
            if not updated:
                prof.add_Email(EmailDetailsType(EmailId=email, EmailCategory=email_category['PERSONAL'], IsDefaultFlag=0 if len(prof.Email) > 0 else 1, ModifyFlag=mod_flag))

    def set_mobile_number(self, mobile_number, mod_flag=modify_flag['INSERT']):
        if mobile_number:
            prof = self._get_or_create_profile()
            if len(prof.Phone) == 0:
                prof.add_Phone(PhoneDetailsType(PhoneNumber=mobile_number, PhoneType=phone_type['MOBILE'], ModifyFlag=mod_flag))
                prof.add_Email(EmailDetailsType(EmailId=mobile_number, EmailCategory=email_category['MOBILE_NO'], IsDefaultFlag=0 if len(prof.Email) > 0 else 1, ModifyFlag=mod_flag))
            else:
