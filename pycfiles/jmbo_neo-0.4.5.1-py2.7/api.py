...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/neo/api.py
# Compiled at: 2014-01-17 10:34:23
import base64, logging, inspect, re, requests
...
try:
    ...
    r_kwargs = {'verify': CONFIG.get('VERIFY_CERT', True), 
       'headers': HEADERS}
...
def log_api_call(function_call_tup=None, log_level=logging.INFO, status_code=200, exception=None):
    """
    ...
    if not function_call_tup:
        try:
            ...
            function_call_tup = (None, None, None, None, None, None)
    ...
    logger.log(log_level, '%(function_name)s%(arg_str)s: %(status_code)s', {'function_name': function_call_tup[3], 'arg_str': arg_str, 
...
def _get_error(response):
        ...
        try:
            ...
            if not exception:
                ...
                for error in errors:
                    ...
                    elif error.ResponseCode == 'BAD_REQUEST' or response.request.method == 'POST' or response.request.method == 'PUT':
    ...
    log_api_call(function_call_tup=inspect.stack()[1], log_level=logging.ERROR, exception=exception)
    return exception
...
def get_kwargs(username=None, password=None, promo_code=None, no_content=False):
    ...
    if username and password and CONFIG.get('USE_MCAL', False):
        new_r_kwargs['headers']['Authorization'] = _get_auth_header(username, password, promo_code if promo_code else CONFIG['PROMO_CODE'])
    if no_content:
...
def authenticate(username=None, password=None, token=None, promo_code=None, acq_src=None):
    ...
    response = requests.get(('%s/consumers/useraccount/' % (BASE_URL,)), params=params, **get_kwargs())
...
def logout(consumer_id, promo_code=None, acq_src=None):
    ...
    if acq_src:
        params['acquisitionsource'] = acq_src
    response = requests.put(('%s/consumers/%s/useraccount/notifylogout' % (BASE_URL, consumer_id)), params=params, **get_kwargs(no_content=True))
    if response.status_code != 200:
...
def remember_me(consumer_id, token):
    ...
    """
    response = requests.put(('%s/consumers/%s/useraccount' % (BASE_URL, consumer_id)), params={'authtoken': token}, **get_kwargs())
    if response.status_code != 200:
        raise _get_error(response)
    log_api_call()
...
def create_consumer(consumer):
    ...
    response = requests.post(('%s/consumers' % (BASE_URL,)), data=data_stream.getvalue(), **get_kwargs())
    data_stream.close()
    if response.status_code == 201:
        log_api_call(status_code=201)
        uri = response.headers['Location']
        match = re.search('/consumers/(?P<id>\\d+)/', uri)
        if match:
...
def complete_registration(consumer_id, uri=None):
    ...
    """
    if not uri:
        response = requests.post(('%s/consumers/%s/registration' % (BASE_URL, consumer_id)), **get_kwargs(no_content=True))
    else:
...
def get_consumers(email_id, dob):
    ...
    response = requests.get(('%s/consumers/' % (BASE_URL,)), params={'dateofbirth': dob_str, 'emailid': email_id}, **get_kwargs())
    if response.status_code == 200:
        try:
...
def link_consumer(consumer_id, username, password, promo_code=None, acq_src=None):
    ...
    if acq_src:
        params['acquisitionsource'] = acq_src
    response = requests.put(('%s/consumers/%s/registration/' % (BASE_URL, consumer_id)), params=params, **get_kwargs())
    if response.status_code == 200:
        try:
...
def get_consumer(consumer_id, username=None, password=None, promo_code=None):
    ...
    """
    response = requests.get(('%s/consumers/%s/all' % (BASE_URL, consumer_id)), **get_kwargs(username=username, password=password, promo_code=promo_code))
    if response.status_code == 200:
        try:
            obj_from_xml = parseString(response.content)
            log_api_call()
...
def get_consumer_profile(consumer_id, username=None, password=None, promo_code=None):
    ...
    """
    response = requests.get(('%s/consumers/%s/profile' % (BASE_URL, consumer_id)), **get_kwargs(username=username, password=password, promo_code=promo_code))
    if response.status_code == 200:
        try:
            obj_from_xml = parseString(response.content)
            log_api_call()
...
def get_consumer_preferences(consumer_id, category_id=None, username=None, password=None, promo_code=None):
    """
...
def update_consumer(consumer_id, consumer, username=None, password=None, promo_code=None):
    ...
    data_stream = StringIO()
    consumer.export(data_stream, 0)
    response = requests.put(('%s/consumers/%s' % (BASE_URL, consumer_id)), data=data_stream.getvalue(), **get_kwargs(username=username, password=password, promo_code=promo_code))
    data_stream.close()
    if response.status_code != 200:
        raise _get_error(response)
    log_api_call()
...
def _update_question_answers(consumer_id, object, category_id=None, create=False, username=None, password=None, promo_code=None, root_tag_name=None, uri=None):
    data_stream = StringIO()
    if root_tag_name:
    ...
    if category_id:
        uri += '/category/%s' % category_id
    response = getattr(requests, 'post' if create else 'put')(uri, data=data_stream.getvalue(), **get_kwargs(username=username, password=password, promo_code=promo_code))
    data_stream.close()
    if response.status_code != 200:
        raise _get_error(response)
...
def update_consumer_preferences(consumer_id, preferences, category_id=None, create=False, username=None, password=None, promo_code=None):
    """
    Create consumer preferences
    Specify category_id to update preferences for a category,
    otherwise all preferences are updated
    """
    _update_question_answers(consumer_id, preferences, category_id, create, username, password, promo_code, 'Preferences')


def update_digital_interactions(consumer_id, digital_interactions, category_id=None, create=False, username=None, password=None, promo_code=None):
    """
    Add digital interactions to consumer
    """
    _update_question_answers(consumer_id, digital_interactions, category_id, create, username, password, promo_code, 'DigitalInteractions')


def update_conversion_locations(consumer_id, conversion_locations, category_id=None, create=False, username=None, password=None, promo_code=None):
    """
    Add conversion locations to consumer
    """
    _update_question_answers(consumer_id, conversion_locations, category_id, create, username, password, promo_code, 'ConversionLocations')
...
def change_password(username, new_password, old_password=None, token=None):
    ...
    if old_password:
        ...
        response = requests.put(('%s/consumers/useraccount' % (BASE_URL,)), params=params, **get_kwargs(no_content=True))
        if response.status_code == 200:
            log_api_call()
            return response.content
    raise _get_error(response)
...
def unsubscribe(consumer_id, unsubscribe_obj):
    ...
    response = requests.put(('%s/consumers/%s/preferences/unsubscribe' % (BASE_URL, consumer_id)), data=data_stream.getvalue(), **get_kwargs())
    data_stream.close()
    if response.status_code != 200:
        raise _get_error(response)
    log_api_call()
...
def add_promo_code(consumer_id, promo_code, acq_src=None, username=None, password=None):
    ...
    if acq_src:
        params['acquisitionsource'] = acq_src
    response = requests.put(('%s/consumers/%s' % (BASE_URL, consumer_id)), params=params, **get_kwargs(username=username, password=password, no_content=True))
    if response.status_code != 200:
        raise _get_error(response)
    log_api_call()
...
def do_age_check(dob, country_code, gateway_id, language_code=None):
    ...
    """
    dob_str = dob.strftime('%Y%m%d')
    params = {'dateofbirth': dob_str, 
       'countrycode': country_code, 
       'gatewayid': gateway_id}
    if language_code:
        params['language_code'] = language_code
    response = requests.get(('%s/consumers/affirmage' % (BASE_URL,)), params=params, **get_kwargs())
    if response.status_code == 200:
        try:
            obj_from_xml = parseString(response.content)
            log_api_call()
...
def get_country(country_code=None, ip_address=None):
    ...
    response = requests.get(('%s/country/' % (BASE_URL,)), params=params, **get_kwargs())
    if response.status_code == 200:
        try:
            obj_from_xml = parseString(response.content)
            log_api_call()
