...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/wrap_amqp.py
# Compiled at: 2015-08-06 04:07:38
__version__ = '1.0.4'
...
def check_function(f, n=1):
    ...
    assert len(inspect.getargspec(f).args) >= n, '%s must take at least %d argument(s)' % (f, n)
    return f
...
class Message:
    ...
    def _init_channel(self, prefetch_size=0, prefetch_count=4, a_global=True, *args, **kwargs):
        ...
        self._chan.basic_consume(queue=self._q, no_ack=False, callback=self._wrap_f, consumer_tag=self._tag)
