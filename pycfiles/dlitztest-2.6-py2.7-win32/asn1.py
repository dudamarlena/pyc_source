...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\Crypto\Util\asn1.py
...
__all__ = [
 'DerObject', 'DerInteger', 'DerOctetString', 'DerNull', 'DerSequence', 'DerObjectId']
...
class DerObject(object):
    """Base class for defining a single DER object.
        ...
        """
    typeTags = {'SEQUENCE': 48, 'BIT STRING': 3, 'INTEGER': 2, 'OCTET STRING': 4, 
       'NULL': 5, 'OBJECT IDENTIFIER': 6}
    ...
    def _decodeLen(self, idx, der):
        ...
        if length <= 127:
            return (length, idx + 1)
        payloadLength = bytes_to_long(der[idx + 1:idx + 1 + (length & 127)])
        if payloadLength <= 127:
            raise ValueError('Not a DER length tag.')
        return (
         payloadLength, idx + 1 + (length & 127))
