...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kutana/backends/telegram.py
...
class Telegram(Backend):
    ...
    def __init__(self, token, messages_per_second=29, session=None, proxy=None, api_url='https://api.telegram.org'):
        if not token:
    ...
    async def _request(self, method, kwargs={}):
        ...
        data = {k:v for k, v in kwargs.items() if v is not None if v is not None}
    ...
    def _make_attachment(self, raw_attachment, raw_attachment_type):
        ...
        if t == 'photo':
            ...
            id = photo['file_id']
            return Attachment._existing_full(id=id,
              type='image',
              title='',
              file_name=id,
              getter=(self._make_getter(id)),
              raw=d)
        if t == 'audio':
        ...
        if t == 'video':
            return Attachment._existing_full(id=id,
              ...
              getter=(self._make_getter(id)),
              raw=d)
        return Attachment._existing_full(id=None,
          type=t,
          title=None,
          file_name=None,
          getter=None,
    ...
    async def perform_updates_request(self, submit_update):
        try:
            response = await self._request('getUpdates', {'timeout':25,  'offset':self.offset})
    ...
    async def perform_send(self, target_id, message, attachments, kwargs):
        ...
        async with self.api_messages_lock:
            if message:
                result.append(await self._request('sendMessage', {**{'chat_id':chat_id, 
                 'text':message}, **kwargs}))
            ...
            for a in attachments:
                if isinstance(a, Attachment):
                    attach_type = {'doc':'document',  'image':'photo'}.get(a.type, a.type)
                    if a.uploaded:
            ...
            for atype, acontent in new_attachments:
                if atype not in ('document', 'photo', 'audio', 'video', 'voice', 'sticker'):
                    raise ValueError(f"Can't upload attachment '{atype}'")
                result.append(await self._request('send' + atype.capitalize(), {'chat_id': chat_id, atype: acontent}))
                await asyncio.sleep(self.api_messages_pause)
    ...
    async def on_start(self, app):
        ...
        if me.get('last_name'):
            name += ' ' + me['last_name']
        logger.info('logged in as "%s" ( https://t.me/%s )', name, me['username'])
