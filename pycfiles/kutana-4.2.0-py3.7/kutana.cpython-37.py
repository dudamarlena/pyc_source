...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kutana/kutana.py
# Compiled at: 2020-05-02 08:54:53
# Size of source mod 2**32: 6054 bytes
import asyncio
...
class Kutana:
    ...
    def __init__(self, concurrent_handlers_count=3000, storage=None, loop=None):
        self._plugins = []
        self._backends = []
    ...
    async def _main_loop(self):
        ...
        while True:
            ...
            ctx = await Context.create(app=self,
              config=(self.config),
              update=update,
              backend=backend)
            backend.prepare_context(ctx)
            task = asyncio.ensure_future((self._handle_update_with_logger(update, ctx)),
              loop=(self._loop))
