...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kutana/plugin.py
# Compiled at: 2020-05-02 08:54:53
# Size of source mod 2**32: 11034 bytes
import re
...
from .routers import CommandsRouter, AttachmentsRouter, ListRouter, AnyMessageRouter, AnyUpdateRouter
...
class Plugin:
    ...
    def _get_or_add_router(self, router_cls, priority=None):
        for router in self._routers:
            if not type(router) == router_cls or priority is None or priority == router.priority:
                return router
    ...
    def on_commands(self, commands, group_state='*', user_state='*', priority=0, router_priority=None):
        """
        ...
        def decorator(func):
            for command in commands:
                self._add_handler_for_router(CommandsRouter,
    ...
    def on_attachments(self, types, group_state='*', user_state='*', priority=0, router_priority=None):
        """
        Decorator for registering coroutine to be called when
        ...
        def decorator(func):
            for atype in types:
                self._add_handler_for_router(AttachmentsRouter,
                  handler=(self._make_handler(func, group_state, user_state, priority)),
                  handler_key=atype,
                  router_priority=router_priority)
    ...
    def on_any_message(self, group_state='*', user_state='*', priority=0, router_priority=None):
        ...
        def decorator(func):
            self._add_handler_for_router(AnyMessageRouter,
              handler=(self._make_handler(func, group_state, user_state, priority)),
              router_priority=router_priority)
            return func
    ...
    def on_any_unprocessed_message(self, group_state='*', user_state='*', priority=0, router_priority=-3):
        """
        Decorator for registering coroutine to be called when
        ...
        def decorator(func):
            ...
            async def wrapper(update, ctx):
                if update.type != UpdateType.MSG:
                    ...
                    return match or HandlerResponse.SKIPPED
                ctx.match = match
                return await func(update, ctx)

            self._add_handler_for_router(ListRouter,
    ...
    def on_any_update(self, group_state='*', user_state='*', priority=0, router_priority=None):
        ...
        def decorator(func):
            self._add_handler_for_router(AnyUpdateRouter,
              handler=(self._make_handler(func, group_state, user_state, priority)),
              router_priority=router_priority)
            return func
