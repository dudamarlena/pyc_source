...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/faiss/__init__.py
# Compiled at: 2019-06-19 18:23:13
# Size of source mod 2**32: 19446 bytes
import numpy as np, sys, inspect, pdb
...
def handle_Index(the_class):
    ...
    def replacement_add_with_ids(self, x, ids):
        ...
        if not d == self.d:
            raise AssertionError
        elif not ids.shape == (n,):
            raise AssertionError('not same nb of vectors as ids')
    ...
    def replacement_search_and_reconstruct(self, x, k):
        ...
        self.search_and_reconstruct_c(n, swig_ptr(x), k, swig_ptr(distances), swig_ptr(labels), swig_ptr(recons))
    ...
    replace_method(the_class, 'search_and_reconstruct', replacement_search_and_reconstruct,
      ignore_missing=True)
...
def handle_AutoTuneCriterion(the_class):
    ...
    def replacement_evaluate(self, D, I):
        if not I.shape == D.shape:
            raise AssertionError
        elif not I.shape == (self.nq, self.nnn):
            raise AssertionError
...
def copy_array_to_vector(a, v):
    ...
    n, = a.shape
    classname = v.__class__.__name__
    if not classname.endswith('Vector'):
        raise AssertionError
    else:
        dtype = np.dtype(vector_name_map[classname[:-6]])
...
class Kmeans:
    __doc__ = 'shallow wrapper around the Clustering object. The important method\n    is train().'
