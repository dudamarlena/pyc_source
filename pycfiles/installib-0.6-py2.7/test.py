...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/installib/test.py
...
def assert_source_files_are_installed_and_obfuscated(install_path, source_files, source_files_without_obfuscate_path=None):
    """
    ...
    :param source_files: list of tuples (file_path, operator, version_to_compare, check_obfuscation)
        For example:
            [("es
esource.dll", ">=", "1.3.0.0", False), ("core.dll", "", "", False)]
    ...
    for source_file, op, version, check_obfuscation in source_files:
        ...
        assert compare_versions_str(product_version, op, version, default=True), '%s ProductVersion %s is not %s %s' % (file_path, product_version, op, version)
...
def assert_assembly_files_are_installed(assembly_source_files, microsoft_assembly_source_files):
    ...
    for assembly_file in assembly_source_files:
        assert is_file_in_subdirectory('%s.dll' % assembly_file, WINDOWS_ASSEMBLY_GAC_MSIL_PATH + assembly_file), '%s file not found in directory %s' % (assembly_file, WINDOWS_ASSEMBLY_GAC_MSIL_PATH)

    for assembly_file in microsoft_assembly_source_files:
        assert is_file_in_subdirectory('%s.dll' % assembly_file, WINDOWS_DOTNET_ASSEMBLY_GAC_MSIL_PATH + assembly_file), '%s file not found in directory %s' % (assembly_file, WINDOWS_DOTNET_ASSEMBLY_GAC_MSIL_PATH)