...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/glespy/tools/test/test_convertation.py
# Compiled at: 2013-11-25 13:35:50
__author__ = 'yarnaid'
import unittest, os, glespy.test_data.data as test_data, glespy.tools.convertion as convert, glespy.ext.angles as angles
...
class FunctionsTests(unittest.TestCase, test_data.WithTestData):
    ...
    def test_map_to_alm_good(self):
        ...
        alm = convert.map_to_alm(map_name=self.map_name, alm_name=alm_from_map)
    ...
    def test_points_to_map_without_name_with_error(self):
        self.assertRaises(ValueError, convert.points_to_map, points_name=self.points_name)
    ...
    def test_correlate(self):
        correlated_map = convert.correlate(self.map_name, convert.points_to_map(self.points_name, **self.attrs), 180)
    ...
    def test_rotate_alm(self):
        rotated = convert.rotate_alm(alm_name=self.alm_100_name, dphi=2, dtheta=2)
    ...
    def test_alm_rotate_with_angle(self):
        rotated = convert.rotate_alm(self.alm_100_name, angle=angles.Angle(10, 100))
        self.check_existance(rotated)

    def test_reflaction(self):
        r_map = convert.reflect_map(map_name=self.map_name, rtype='h')
        self.check_existance(r_map)

    def test_mult(self):
        mult = convert.mult_map(self.map_name, mult=-1000)
