...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/glespy/ext/angles.py
# Compiled at: 2013-09-14 10:46:18
__author__ = 'yarnaid'
...
class Zone(object):
    ...
    def __init__(self, *args):
        ...
        if 2 > len(args) or len(args) > 4:
            raise IndexError(('*args number must be int [2, 3, 4] for circle or rectangle, {} got').format(len(args)))
        if len(args) == 2:
            args += (90, )
        self.angles = args
    ...
    def __invert__(self):
        ...
        if len(self.angles) == 4:
            self.angles[2] = self.angles[0] - abs(self.angles[2] - self.angles[0])
            self.angles[3] = self.angles[1] - abs(self.angles[3] - self.angles[1])
...
class Angle(object):
    ...
    def __init__(self, *args):
        ...
        if len(args) != 2:
            raise IndexError(('*args number must be 2, {} got').format(len(args)))
        self.theta = args[0]
        self.phi = args[1]
