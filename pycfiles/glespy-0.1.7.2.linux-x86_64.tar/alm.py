...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/glespy/alm.py
# Compiled at: 2013-12-04 10:41:58
__author__ = 'yarnaid'
import tools.convertion as convert, pixelmap, properties, os
...
class Alm(properties.Multipoled):
    ...
    def get_attrs(self, **kwargs):
        res = {'lmax': self.lmax, 'lmin': self.lmin}
        res.update(kwargs)
        return res
    ...
    def to_map(self, map_name=None, **kwargs):
        ...
        attrs['name'] = convert.alm_to_map(self.name, map_name=map_name, **attrs)
        pmap = pixelmap.gPixelMap(**attrs)
        return pmap

    def smooth(self, smooth_window, smoothed_name=None, **kwargs):
        smoothed_name = convert.smooth_alm(self.name, smooth_window, smoothed_name=smoothed_name, **kwargs)
        attrs = self.get_attrs(kwargs)
    ...
    def get_rotation(self, rotated_name=None, **kwargs):
        attrs = self.get_attrs(**kwargs)
        rotated_name = convert.rotate_alm(self.name, rotated_name=rotated_name, **attrs)
    ...
    def update_ls(self, update_name=None, **kwargs):
        self.name = convert.alm_update_ls(self.name, update_name, **self.get_attrs(**kwargs))
        return self.name