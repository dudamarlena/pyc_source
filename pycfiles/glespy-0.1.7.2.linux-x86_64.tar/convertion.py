...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/glespy/tools/convertion.py
# Compiled at: 2014-01-14 07:13:39
__author__ = 'yarnaid'
import os, cStringIO as StringIO, subprocess as sp, numpy as np, glespy.tools.tools as tools, glespy.properties as properties
...
def get_map_attrs(map_name):
    ...
    try:
        res = {'nx': int(output[(output.index('nx') + 2)][0:-1]), 'np': int(output[(output.index('np') + 2)])}
    except:
...
def alm_to_spec(alm_name, cl=True, **kwargs):
    if len(set(['lmax', 'l', 'lmin']).intersection(kwargs)) < 1:
        raise ValueError('[%s]: lmax or l or lmin must be given' % alm_to_cl.__name__)
    opt_args = kwargs_props_to_glesp_args(**kwargs)
...
def mask_map(map_name, mask_name, masked_map_name=None, **kwargs):
    ...
    masked_map_name = tools.get_out_name(masked_map_name, suffix='masked_from_map.fit')
    opt_args = kwargs_props_to_glesp_args(**kwargs)
    args = [tools.binaries['difmap'], map_name,
...
def asci_to_map(asci_name, tp, map_name=None, **kwargs):
    ...
    if len(set(['nx', 'np', 'lmax', 'l', 'lmin']).intersection(kwargs)) < 1:
        raise ValueError(('[{}]: nx or np or lmax or l or lmin must be given').format(alm_to_cl.__name__))
...
def smooth_alm(alm_name, smooth_window, smoothed_name=None, **kwargs):
    check_file(alm_name)
    smoothed_name = tools.get_out_name(smoothed_name, suffix='smoothed_alm.fit')
    opt_args = []
    try:
        opt_args = kwargs_props_to_glesp_args(**kwargs)
    except:
...
def correlate(map1, map2, correlation_window, correlated_map=None, **kwargs):
    check_file(map1)
    check_file(map2)
    correlated_map = tools.get_out_name(correlated_map, suffix='correlated_map.fit')
    try:
        opt_args = kwargs_props_to_glesp_args(**kwargs)
    except:
        opt_args = []
    ...
    args = [
     tools.binaries['difmap'], map1, map2,
...
def sum_map(map1_name, map2_name, mult=[
 1.0, -1.0], summed_map=None, **kwargs):
    check_file(map1_name)
    check_file(map2_name)
    attrs1 = get_map_attrs(map1_name)
    attrs2 = get_map_attrs(map2_name)
    if attrs1 != attrs2:
        raise ValueError(('[{}]: maps has different resolution').format(__name__))
    ...
    args = [
     tools.binaries['difmap'], '-c1', mult[0],
...
def rotate_alm(alm_name, rotated_name=None, angle=None, **kwargs):
    ...
    if not isinstance(angle, (Angle, type(None))):
        raise TypeError(('Angle must be an instance of {} class, {}.got').format(angle.Angle.__class__.__name__, type(angle)))
    check_file(alm_name)
...
def cut_map_zone(map_name, zone, cutted_map=None, **kwargs):
    assert isinstance(zone, Zone), ('zone must be an instance of {}').format(Zone.__class__.__name__)
    check_file(map_name)
...
def reflect_map(map_name, rtype, reflected_name=None, **kwargs):
    if rtype not in rtypes:
        raise ValueError(('Reflection type must be in {}, got {}').format(rtypes, rtype))
    reflected_name = tools.get_out_name(reflected_name, suffix='reflected_map.fit')
    check_file(map_name)
...
def mult_map(map_name, mult, mult_name=None, **kwargs):
    check_file(map_name)
    mult_name = tools.get_out_name(mult_name, suffix=('multed_{}_map_fit').format(mult))
    opt_args = kwargs_to_glesp_args(**kwargs)
...
def cl_to_smth(cl_name, out_key='o', lmax=None, nx=None, lmin=0, smth_name=None, **kwargs):
    ...
    return {'name': check_file(smth_name), 'lmax': lmax, 
       'nx': nx, 
       'lmin': lmin, 
       'kwargs': kwargs}
...
def cl_to_alm(cl_name, lmax=None, nx=None, lmin=0, alm_name=None, **kwargs):
    return cl_to_smth(cl_name, out_key='ao', lmax=lmax, nx=nx, lmin=lmin, smth_name=alm_name, **kwargs)


def cl_to_map(cl_name, lmax=None, nx=None, lmin=0, map_name=None, **kwargs):
    return cl_to_smth(cl_name, out_key='o', lmax=lmax, nx=nx, lmin=lmin, smth_name=map_name, **kwargs)