...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/glespy/pixelmap.py
# Compiled at: 2013-12-04 10:14:58
__author__ = 'yarnaid'
import os, properties, tools.convertion as conv, alm
...
class gPixelMap(properties.Rendered):
    ...
    def add_map(self, other, **kwargs):
        try:
            ...
            raise ValueError(('Can not sum PixelMap with {}').format(type(other)))
    ...
    def __invert__(self):
        inverted = conv.mult_map(map_name=self.name, mult=-1)
        attrs = self.get_attrs()
        attrs['name'] = inverted
        return gPixelMap(**attrs)
    ...
    def get_keep_zone(self, zone, cutted_map=None, **kwargs):
        ...
        cutted_map = conv.cut_map_zone(map_name=self.name, zone=zone, cutted_map=cutted_map, **attrs)
        attrs['name'] = cutted_map
        return gPixelMap(**attrs)

    def get_half(self, lat, lon, map_name=None, **kwargs):
        return self.get_keep_zone(angles.Zone(lat, lon), cutted_map=map_name, **kwargs)
    ...
    def get_reflection(self, rtype, reflected_name=None, **kwargs):
        attrs = self.get_attrs(**kwargs)
        reflected_name = conv.reflect_map(map_name=self.name, rtype=rtype, reflected_name=reflected_name, **attrs)
    ...
    def get_masked(self, mask_name, masked_map_name=None, **kwargs):
        attrs = self.get_attrs(**kwargs)
        masked_map_name = conv.mask_map(self.name, mask_name, masked_map_name, **attrs)
