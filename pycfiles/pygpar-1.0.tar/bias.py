...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/pygp/covar/bias.py
...
class BiasCF(CovarianceFunction):
    """
    Bias CF with   r$\x07lpha \\cdot \x08oldsymbol{1}$
    """
...
class BiasCFPsiStat(BiasCF, BayesianStatisticsCF):
    ...
    def psi(self):
        return (
         self.psi0, self.psi1, self.psi2)

    def psigrad_theta(self):
        return (
         2 * self.psi0, 2 * self.sigma, 4 * self.psi2[:, :, None, None])
