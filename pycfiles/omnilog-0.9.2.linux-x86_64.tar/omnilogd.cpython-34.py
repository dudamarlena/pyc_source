...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.4/dist-packages/omnilog/omnilogd.py
# Compiled at: 2016-03-11 14:12:35
# Size of source mod 2**32: 8248 bytes
import os, sys, signal, threading, time, collections
            ...
            try:
                ...
                if self.vertical_queue.empty() and self.booting:
                    if self.gen_log_handler_runner.is_set():
                        gen_log_handler = LogHandler(Config.config_dict, self.log_queue, self.gen_log_handler_runner, self.web_panel_queue, self.vertical_queue)
                        threads.append(gen_log_handler)
                        gen_log_handler.start()
                    if Config.config_dict['webPanel']['active']:
                        if self.web_panel_runner.is_set():
                            web_panel = WebPanel(self.web_panel_runner, Config.config_dict['webPanel'], self.web_panel_queue, self.vertical_queue)
