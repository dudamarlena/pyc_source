...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/com/dtmilano/android/adb/dumpsys.py
...
class Dumpsys:
    ...
    def parseGfxinfoFramestats(self, out):
        pd = '---PROFILEDATA---'
        l = re.findall('%s.*?%s' % (pd, pd), out, re.DOTALL)
        if l:
            s = ''
            for e in l:
                if not e:
                ...
                for s in sl:
                    ...
                    if pda[Dumpsys.FLAGS] == 'Flags':
                        if pda[Dumpsys.INTENDED_VSYNC] != 'IntendedVsync' and pda[Dumpsys.FRAME_COMPLETED] != 'FrameCompleted':
                    ...
                    if pda[Dumpsys.FLAGS] == '0':
                        ...
                        self.framestats.append((int(pda[Dumpsys.FRAME_COMPLETED]) - int(pda[Dumpsys.INTENDED_VSYNC])) / 1000000)
