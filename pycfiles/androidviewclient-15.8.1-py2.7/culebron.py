...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/com/dtmilano/android/culebron.py
...
"""
import StringIO, random, re, time, numpy
...
__version__ = '15.8.1'
import sys, threading, warnings, copy, string, os, platform
...
try:
    import Tkinter, tkSimpleDialog, tkFileDialog, tkFont, ScrolledText, ttk
...
class Operation():
    ...
    WAKE = 'wake'
    COMMAND_NAME_OPERATION_MAP = {'flingBackward': FLING_BACKWARD, 'flingForward': FLING_FORWARD, 'flingToBeginning': FLING_TO_BEGINNING, 
       'flingToEnd': FLING_TO_END, 'openNotification': OPEN_NOTIFICATION, 
...
class Culebron():
    ...
    KEYSYM_TO_KEYCODE_MAP = {'Home': 'HOME', 
       'BackSpace': 'BACK', 
       'Left': 'DPAD_LEFT', 
       'Right': 'DPAD_RIGHT', 
       'Up': 'DPAD_UP', 
       'Down': 'DPAD_DOWN', 
       'F12': 'F12'}
    KEYSYM_CULEBRON_COMMANDS = {'F1': None, 
       'F5': None}
    canvas = None
    imageId = None
    vignetteId = None
    ...
    def __init__(self, vc, device, serialno, printOperation, scale=1, concertina=False, concertinaConfigFile=None):
        ...
        try:
            f = resource_filename(Requirement.parse('androidviewclient'), 'share/pixmaps/culebra.png')
    ...
    def takeScreenshotAndShowItOnWindow(self):
        ...
        if self.isDarwin and self.scale != 1:
            if 14 < self.sdkVersion < 23:
            ...
            gridInfo or self.canvas.grid(row=1, column=1, rowspan=4)
        self.findTargets()
        self.hideVignette()
        if DEBUG:
            try:
    ...
    def createMessageArea(self, width, height):
        self.__message = Tkinter.Label(self.window, text='', background=Color.GOLD, font=('Helvetica',
                                                                                          16), anchor=Tkinter.W)
    ...
    def createVignette(self, width, height):
        ...
        self.vignetteId = self.canvas.create_rectangle(0, 0, width, height, fill=Color.MAGENTA, stipple='gray50')
        font = tkFont.Font(family='Helvetica', size=int(144 * self.scale))
        msg = 'Please\nwait...'
        self.waitMessageShadowId = self.canvas.create_text(width / 2 + 2, height / 2 + 2, text=msg, fill=Color.DARK_GRAY, font=font)
        self.waitMessageId = self.canvas.create_text(width / 2, height / 2, text=msg, fill=Color.LIGHT_GRAY, font=font)
        self.canvas.update_idletasks()
            ...
            if v is None:
                ...
                if self.vc.uiAutomatorHelper:
                    if v == self.vc.navBack:
                        self.pressBack()
                        return
                    if v == self.vc.navHome:
                        self.pressHome()
                        return
                    if v == self.vc.navRecentApps:
        ...
        if self.areEventsDisabled:
            ...
            if self.isLongTouchingPoint:
                ...
                if self.coordinatesUnit == Unit.DIP:
                    x = round(x / self.device.display['density'], 2)
                    y = round(y / self.device.display['density'], 2)
                self.printOperation(None, Operation.LONG_TOUCH_POINT, x, y, 2000, self.coordinatesUnit, self.device.display['orientation'])
                self.sleep(5)
                self.isLongTouchingPoint = False
                self.takeScreenshotAndShowItOnWindow()
                self.statusBar.clear()
    ...
    def onKeyPressed(self, event):
        if DEBUG_KEY:
            ...
            print >> sys.stderr, '    event', type(event.char), len(event.char), repr(event.char), 'keysym=', event.keysym, 'keycode=', event.keycode, event.type, 'state=', event.state
            ...
            if keysym in Culebron.KEYSYM_TO_KEYCODE_MAP:
                ...
                if keysym == 'F12' and (event.state == 8 or event.state == 24):
                    ...
                    for d in ['/dev/input/event2', '/dev/input/event6']:
                        self.device.longPress('MOVE_HOME', duration=0.1, dev=d, scancode=458979, repeat=50)
                        self.printOperation(None, Operation.LONG_PRESS, 'MOVE_HOME', 0.1, d, 458979, 50)
    ...
    def refresh(self):
        ...
        if changed:
            self.window.geometry('%dx%d' % (self.device.display['width'] * self.scale,
             self.device.display['height'] * self.scale + int(self.statusBar.winfo_height())))
            self.deleteVignette()
            self.canvas.destroy()
            self.canvas = None
            self.window.update_idletasks()
    ...
    def drag(self, start, end, duration, steps, units=Unit.DIP):
        ...
        if self.vc.uiAutomatorHelper:
            self.printOperation(None, Operation.SWIPE_UI_AUTOMATOR_HELPER, x0, y0, x1, y1, steps, units, self.device.display['orientation'])
        else:
    ...
    def markTarget(self, x1, y1, x2, y2, color='#ff00ff'):
        ...
        """
        _id = self.canvas.create_rectangle(x1 * self.scale, y1 * self.scale, x2 * self.scale, y2 * self.scale, fill=color, stipple='gray25')
    ...
    def drawDragLine(self, x0, y0, x1, y1):
        ...
        width = 15
        return self.canvas.create_line(x0 * self.scale, y0 * self.scale, x1 * self.scale, y1 * self.scale, width=width, fill=Color.MAGENTA, arrow='last', arrowshape=(50,
                                                                                                                                                                      50,
                                                                                                                                                                      30), dash=(50,
                                                                                                                                                                                 25))
            ...
            if rand > 1 - probabilities['systemKeys']:
                ...
                if _len > 0:
                    ...
                    if _tvli:
                        ...
                        box = self.targets[i]
                        if DEBUG_CONCERTINA:
                            print >> sys.stderr, 'CONCERTINA: selected', unicode(target.__smallStr__()), target.getContentDescription()
                            ...
                            for _i in _tvli:
                                print >> sys.stderr, 'CONCERTINA:    view class', self.targetViews[_i].getClass(), self.targetViews[_i].getContentDescription()
                        ...
                        if probabilities['views'] > 0 and clazz == 'android.widget.EditText':
                            id = target.getId()
                            txt = target.getText()
                            if target.isPassword() or re.search('password', id, re.IGNORECASE) or re.search('password', txt, re.IGNORECASE):
                            ...
                            self.sleep(15)
                            needToSleep = True
                        elif probabilities['views'] > 0 and random.choice(['SCROLL', 'TOUCH']) == 'SCROLL' and (isScrollable or parent.isScrollable() or parentClass == 'android.widget.ScrollView'):
                            if isScrollable:
                            ...
                            if random.choice(['FORWARD', 'REVERSE']) == 'REVERSE':
                                ...
                                sp = ep
                                ep = temp
                            elif DEBUG_CONCERTINA:
                                print >> sys.stderr, 'CONCERTINA: FORWARD'
                            d = 500
                            s = 20
                            _id = self.canvas.create_rectangle(l * self.scale, t * self.scale, r * self.scale, b * self.scale, fill='#00ffff', stipple='gray12')
                            self.window.update_idletasks()
                            units = Unit.PX
                ...
                elif self.dump and probabilities['views'] > 0:
                    ...
                    if self.noTargetViewsCount >= self.concertinaConfig['limits']['maxNoTargetViewsIterations']:
                        print >> sys.stderr, ('CONCERTINA: Cannot detect target Views after {} iterations').format(self.noTargetViewsCount)
                        sys.exit(1)
...
if TKINTER_AVAILABLE:
    ...
    class MainMenu(Tkinter.Menu):
        ...
        def __init__(self, culebron):
            ...
            self.viewMenu.add_checkbutton(label='Tree', underline=0, accelerator='Command-T', onvalue=True, offvalue=False, variable=self.showViewTree, state=state, command=self.onshowViewTreeChanged)
            self.showViewDetails = Tkinter.BooleanVar()
            self.showViewDetails.set(False)
            state = NORMAL if culebron.vc else DISABLED
            self.viewMenu.add_checkbutton(label='View details', underline=0, accelerator='Command-V', onvalue=True, offvalue=False, variable=self.showViewDetails, state=state, command=self.onShowViewDetailsChanged)
            self.add_cascade(label='View', underline=0, menu=self.viewMenu)
            self.uiDeviceMenu = Tkinter.Menu(self, tearoff=False)
            state = NORMAL if culebron.vc else DISABLED
            self.uiDeviceMenu.add_command(label='Open Notification', underline=6, state=state, command=lambda : culebron.executeCommandAndRefresh(self.culebron.vc.uiDevice.openNotification))
            state = NORMAL if culebron.vc else DISABLED
            self.uiDeviceMenu.add_command(label='Open Quick settings', underline=6, state=state, command=lambda : culebron.executeCommandAndRefresh(command=self.culebron.vc.uiDevice.openQuickSettings))
            state = NORMAL if culebron.vc else DISABLED
            self.uiDeviceMenu.add_command(label='Change Language', underline=7, state=state, command=self.culebron.changeLanguage)
    ...
    class DragDialog(Tkinter.Toplevel):
        ...
        def __init__(self, culebron):
            ...
            self.validate = (
             self.parent.register(self.onValidate), '%P')
    ...
    class ContextMenu(Tkinter.Menu):
        ...
        def __init__(self, culebron, view):
            ...
            items.append(ContextMenu.Command('Long touch View', 0, None, None, culebron.toggleLongTouchView))
