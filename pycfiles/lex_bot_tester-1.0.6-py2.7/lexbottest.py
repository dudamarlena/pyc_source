...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-intel/egg/lex_bot_tester/aws/lex/lexbottest.py
...
class LexBotTest(TestCase):
        ...
        for c in conversations:
            ...
            for ci in c:
                ...
                if verbose:
                    ...
                    print(Color.colorize(('User: {}').format(ci.send), Color.BRIGHT_YELLOW if use_tts else Color.BRIGHT_WHITE, Color.BRIGHT_BLACK))
                ...
                if before_slots:
                    for s in before_slots.keys():
                        if to_snake_case(s) not in expected_result and before_slots[s]:
                            if DEBUG:
                                print(('** adding key={} to expected_result with value "{}"').format(to_snake_case(s), before_slots[s]))
                ...
                self.assertEqual(expected_result.dialog_state, self.csc.get_dialog_state(), msg=('Invalid dialog state, response={}').format(response))
                if DEBUG:
                ...
                if len(expected_result) > 0:
                    for rk in expected_result.keys():
                        try:
                            ...
                            self.assertIsNotNone(a, msg=('{} slot is None: rk={} e={} a={} send="{}" elicit={} slots={}').format(to_camel_case(rk), rk, e, a, ci.send, slot_to_elicit, slots))
                ...
                elif before_dialog_state == DialogState.ELICIT_SLOT and slot_to_elicit is not None:
                    self.assertEqual(ci.send.lower(), self.csc.get_slot(slot_to_elicit).lower())
    ...
    def conversations_text_helper(self, bot_alias, bot_name, user_id, conversation_definition, verbose=VERBOSE, use_tts=False):
        """
        Helper method for tests using text conversations.
