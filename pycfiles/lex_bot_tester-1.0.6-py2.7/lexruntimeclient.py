...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-intel/egg/lex_bot_tester/aws/lex/lexruntimeclient.py
# Compiled at: 2018-02-19 20:29:14
"""
    Lex Bot Tester
...
class LexRuntimeClient:
    ...
    def post_text(self, text, request_attributes=None, session_attributes=None):
        ...
        if session_attributes is None:
            session_attributes = {}
        self.__response = self.__client.post_text(botName=self.bot_name, botAlias=self.bot_alias, userId=self.user_id, sessionAttributes=session_attributes, requestAttributes=request_attributes, inputText=text)
    ...
    def post_text_to_speech(self, text, request_attributes=None, session_attributes=None):
        ...
        self.__response = self.post_content('audio/l16; rate=16000; channels=1', speech, 'text/plain; charset=utf-8', request_attributes, session_attributes)
        return self.__response

    def post_content(self, content_type, input_stream, accept, request_attributes=None, session_attributes=None):
        """
        ...
        if session_attributes is None:
            session_attributes = {}
        self.__response = self.__client.post_content(botName=self.bot_name, botAlias=self.bot_alias, userId=self.user_id, sessionAttributes=session_attributes, requestAttributes=request_attributes, contentType=content_type, inputStream=input_stream, accept=accept)
        return self.__response
