...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-intel/egg/lex_bot_tester/aws/lex/lexmodelsclient.py
    ...
    def __init__(self, dialog_state, **kwargs):
        for key, value in kwargs.items():
            if key not in arg_names:
                raise TypeError(('Argument {} not valid for {}. Should be one of {}').format(key, self.__class__.__name__, arg_names))
...
class LexModelsClient:
    ...
    def create_result_classes(self, bot_name, bot_alias):
        ...
        for ina in intent_names:
            ...
            if DEBUG:
                for s in slots:
                    print(('>>>>>>> name = {} {} {}').format(s['name'], type(s['name']), s['name'].encode('ascii', 'ignore'), type(s['name'].encode('ascii', 'ignore'))))
