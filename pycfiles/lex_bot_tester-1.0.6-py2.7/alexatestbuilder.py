...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-intel/egg/lex_bot_tester/aws/alexa/alexatestbuilder.py
# Compiled at: 2018-03-07 23:30:08
"""
    Lex Bot Tester
...
class AlexaTestBuilder(object):
    ...
    def learn_conversation(skill_name, intent_name, simulated_input=None):
        ...
        try:
            if interaction_model.get_intent(intent_name)['confirmationRequired']:
                conversation.append({'slot': None, 'prompt': 'Confirmation', 'text': AlexaTestBuilder.__input_text('$confirmation_text', 'Confirmation', simulated_input)})
        except KeyError:
            pass
    ...
    def create_test(self, test_name=None, skill_name=None, intent_name=None, conversation=None):
        ...
        d = {'test_name': test_name, 'skill_name': skill_name, 'intent_name': intent_name, 'conversation': conversation, 'now': str(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')), 'generator': self.generator}
