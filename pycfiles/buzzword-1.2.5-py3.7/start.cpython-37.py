...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/buzzword/parts/start.py
...
from buzzword.parts.main import app, CORPORA, INITIAL_TABLES, CORPUS_META, ROOT, CORPORA_CONFIGS, GLOBAL_CONFIG
...
def _store_corpus(contents, filenames, slug):
    ...
    return (
     store_at, is_parsed, corpus_size)
...
def _upload_files(n_clicks, contents, names, corpus_lang, corpus_name, table_rows, session_configs):
    ...
    if msg:
        return (
         bool(msg), msg, table_rows, session_configs)
    ...
    corpus = corpus.load()
    CORPORA[slug] = corpus
    conf = dict(**GLOBAL_CONFIG, **{'slug':slug, 
     'len':len(corpus), 
...
@app.callback(Output('show-upload-files', 'children'), [
...
intro = html.P('Here you can create and explore parsed and annotated corpora. If you want to work with your own corpus, simply upload plain text files, annotated text files, or CONLL-U files.' + avail)
uphead = html.H3('Upload data', style=(style.VERTICAL_MARGINS))
link = "https://buzzword.readthedocs.io/en/latest/building/'"
md = 'You can upload either CONLL-U files, or plaintext with optional annotations. See [`Creating corpora`]({}) for more information.'.format(link)
upload_text = dcc.Markdown(md)
upload = _make_upload_parse_space()
hide = {'display': 'none'}
content = [
 header, intro, uphead, upload_text, upload]
if not is_empty:
