...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/buzzword/parts/tabs.py
# Compiled at: 2019-08-25 13:24:39
# Size of source mod 2**32: 15185 bytes
"""
buzzword: building tab layout and populating with initial data
"""
import dash_core_components as dcc, dash_daq as daq, dash_html_components as html, dash_table
from buzz.constants import SHORT_TO_COL_NAME
from buzz.corpus import Corpus
from buzz.dashview import CHART_TYPES, _df_to_figure
from buzzword.parts.helpers import _get_cols, _update_datatable, _drop_cols_for_datatable
from buzzword.parts.strings import _make_search_name, _make_table_name, _capitalize_first
...
def _build_dataset_space(df, config):
    ...
    pieces = [
     ...
     daq.BooleanSwitch(id='skip-switch',
       on=False,
    ...
    columns = [{'name':_capitalize_first(SHORT_TO_COL_NAME.get(i, i)).replace('_', ' '),  'id':i,  'deletable':False} for i in df.columns]
...
def _build_concordance_space(df, config):
    ...
    if 'speaker' in df.columns:
        just.append('speaker')
    df = df[just]
    columns = [{'name':SHORT_TO_COL_NAME.get(i, i),  'id':i,  'deletable':i not in ('left', 'match', 'right')} for i in df.columns]
    style_data = [
...
def _build_chart_space(table, config):
    ...
    for chart_num, kind in ((1, 'stacked_bar'), (2, 'line'), (3, 'area'), (4, 'heatmap'),
        ...
        chart_type = dcc.Dropdown(id=f"chart-type-{chart_num}",
          ...
          value=kind,
          style=(style.MARGIN_5_MONO))
        transpose = daq.BooleanSwitch(id=f"chart-transpose-{chart_num}",
          on=False,
...
def _make_tabs(corpus, table, config):
    ...
    drop_style = {**{'fontFamily':'monospace', 
     'width':'60%'}, **(style.HORIZONTAL_PAD_5), **(style.BLOCK_MIDDLE_35)}
    nav = {k:v for k, v in style.NAV_HEADER.items() if k != 'paddingTop' if k != 'paddingTop'}
    top_bit = [
     html.Img(src='../assets/bolt.jpg',
       height=42,
       width=38,
