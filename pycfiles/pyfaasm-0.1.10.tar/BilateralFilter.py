...
# Embedded file name: build/bdist.linux-x86_64/egg/pyF3D/filters/BilateralFilter.py
# Compiled at: 2017-04-27 19:05:13
...
class BilateralFilter:
    ...
    def makeKernel(self, r):
        radius = r + 1
        minWorkingGroup = 256
        if 'CPU' in self.clattr.device.name:
            minWorkingGroup = 64
    ...
    def runFilter(self):
        ...
        try:
            self.kernel.set_args(self.clattr.inputBuffer, self.clattr.outputBuffer, np.int32(self.atts.width), np.int32(self.atts.height), np.int32(self.clattr.maxSliceCount + self.getInfo().overlapZ), self.spatialKernel, np.int32((self.spatialRadius + 1) * 2 - 1), self.rangeKernel, np.int32((self.rangeRadius + 1) * 2 - 1))
