...
# Embedded file name: build/bdist.linux-x86_64/egg/pyF3D/ClAttributes.py
# Compiled at: 2017-04-19 19:48:05
import pyopencl as cl, sys, numpy as np, pkg_resources as pkg
...
class ClAttributes(object):
    ...
    def __init__(self, context, device, queue, inputBuffer, outputBuffer, outputTmpBuffer):
        ...
        if 'CPU' in self.device.name:
            self.globalMemSize = int(min(self.globalMemSize, 83886080))
    ...
    def computeWorkingGroupSize(self, localSize, globalSize, sizes):
        if not localSize or not globalSize or not sizes:
            return False
        if len(localSize) <= 0 or len(localSize) > 2 or len(globalSize) <= 0 or len(globalSize) > 2 or len(sizes) != 3:
            return False
