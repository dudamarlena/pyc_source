...
# [GCC 8.4.0]
# Embedded file name: /home/dadasu/demo/django/dj-translate/autotranslate/views.py
# Compiled at: 2016-10-06 05:48:51
...
import json, re, autotranslate, unicodedata, hashlib, os, six
...
def home(request):
    ...
    if storage.has('autotranslate_i18n_fn'):
        ...
        if 'filter' in request.GET:
            if request.GET.get('filter') in ('untranslated', 'translated', 'fuzzy',
                                             'all'):
        ...
        if '_next' in request.POST:
            rx = re.compile('^m_([0-9a-f]+)')
            rx_plural = re.compile('^m_([0-9a-f]+)_([0-9]+)')
            file_change = False
                ...
                if md5hash is not None:
                    ...
                    if entry:
                        ...
                        if old_msgstr != value or old_fuzzy != is_fuzzy:
                            entry_changed.send(sender=entry, user=request.user, old_msgstr=old_msgstr, old_fuzzy=old_fuzzy, pofile=autotranslate_i18n_fn, language_code=autotranslate_i18n_lang_code)
                    else:
                        storage.set('autotranslate_last_save_error', True)

            if file_change and autotranslate_i18n_write:
                try:
                    autotranslate_i18n_pofile.metadata['Last-Translator'] = unicodedata.normalize('NFKD', '%s %s <%s>' % (
                     getattr(request.user, 'first_name', 'Anonymous'),
                     getattr(request.user, 'last_name', 'User'),
                     getattr(request.user, 'email', 'anonymous@user.tld'))).encode('ascii', 'ignore')
                    autotranslate_i18n_pofile.metadata['X-Translated-Using'] = 'dj-translate %s' % autotranslate.get_version(False)
                ...
                try:
                    ...
                    if autotranslate_settings.WSGI_AUTO_RELOAD and 'mod_wsgi.process_group' in request.environ and request.environ.get('mod_wsgi.process_group', None) and 'SCRIPT_FILENAME' in request.environ and int(request.environ.get('mod_wsgi.script_reloading', '0')):
                        try:
        ...
        if _request_request('query', False) and _request_request('query', '').strip():
            ...
            paginator = Paginator([ e_ for e_ in autotranslate_i18n_pofile if not e_.obsolete and rx.search(six.text_type(e_.msgstr) + six.text_type(e_.msgid) + ('').join([ o[0] for o in e_.occurrences ])) ], autotranslate_settings.MESSAGES_PER_PAGE)
        else:
        ...
        return render(request, 'autotranslate/pofile.html', dict(version=autotranslate.get_version(True), ADMIN_MEDIA_PREFIX=ADMIN_MEDIA_PREFIX, ADMIN_IMAGE_DIR=ADMIN_IMAGE_DIR, ENABLE_REFLANG=autotranslate_settings.ENABLE_REFLANG, LANGUAGES=LANGUAGES, autotranslate_settings=autotranslate_settings, autotranslate_i18n_lang_name=autotranslate_i18n_lang_name, autotranslate_i18n_lang_code=autotranslate_i18n_lang_code, autotranslate_i18n_lang_bidi=autotranslate_i18n_lang_bidi, autotranslate_last_save_error=autotranslate_last_save_error, autotranslate_i18n_filter=autotranslate_i18n_filter, autotranslate_i18n_write=autotranslate_i18n_write, autotranslate_messages=autotranslate_messages, page_range=needs_pagination and page_range, needs_pagination=needs_pagination, main_language=main_language, autotranslate_i18n_app=autotranslate_i18n_app, page=page, query=query, paginator=paginator, autotranslate_i18n_pofile=autotranslate_i18n_pofile, ref_lang=ref_lang))
    else:
...
def list_languages(request, do_session_warn=False):
    ...
    return render(request, 'autotranslate/languages.html', dict(version=autotranslate.get_version(True), ADMIN_MEDIA_PREFIX=ADMIN_MEDIA_PREFIX, do_session_warn=do_session_warn, languages=languages, has_pos=has_pos, autotranslate_i18n_catalog_filter=autotranslate_i18n_catalog_filter))
...
def lang_sel(request, langid, idx):
    ...
    if langid not in [ l[0] for l in settings.LANGUAGES ] or not can_translate_language(request.user, langid):
        ...
        po = pofile(file_)
        for entry in po:
            entry.md5hash = hashlib.new('md5', (six.text_type(entry.msgid) + six.text_type(entry.msgstr) + six.text_type(entry.msgctxt or '')).encode('utf8')).hexdigest()
