...
# [GCC 8.4.0]
# Embedded file name: /home/dadasu/demo/django/dj-translate/autotranslate/services.py
# Compiled at: 2016-10-05 05:19:17
import sys
reload(sys)
sys.setdefaultencoding('utf-8')
import collections, six, urllib2, urllib, re
...
class GoSlateTranslatorService(BaseTranslatorService):
    ...
    def translate_strings(self, strings, target_language, source_language='en', optimized=True):
        ...
        for strr in strings:
            try:
                converted_strings = re.sub('\\s+', ' ', strr)
                translated_strings = self._translate(converted_strings, target_language, source_language)
...
class GoogleAPITranslatorService(BaseTranslatorService):
    ...
    def __init__(self, max_segments=128):
        ...
        assert self.developer_key, '`GOOGLE_TRANSLATE_KEY` is not configured, it is required by `GoogleAPITranslatorService`'
    ...
    def translate_string(self, text, target_language, source_language='en'):
        ...
        response = self.service.translations().list(source=source_language, target=target_language, q=[text]).execute()
    ...
    def translate_strings(self, strings, target_language, source_language='en', optimized=True):
        ...
        if len(strings) <= self.max_segments:
            ...
            response = self.service.translations().list(source=source_language, target=target_language, q=strings).execute()
            ...
            _translated_strings = self.translate_strings(strings[self.max_segments:], target_language, source_language, optimized)
            self.translated_strings = []
