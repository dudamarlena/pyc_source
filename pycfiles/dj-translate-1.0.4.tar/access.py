# uncompyle6 version 3.7.4
# Python bytecode 2.7 (62211)
# Decompiled from: Python 3.6.9 (default, Apr 18 2020, 01:56:04) 
# [GCC 8.4.0]
# Embedded file name: /home/dadasu/demo/django-transhette/dj_transhette/autotranslate/access.py
# Compiled at: 2016-10-05 04:53:13
from django.conf import settings
from autotranslate.conf import settings as autotranslate_settings
from django.core.exceptions import ImproperlyConfigured
try:
    import importlib
except ImportError:
    from django.utils import importlib

def can_translate(user):
    return get_access_control_function()(user)


def get_access_control_function():
    """
    Return a predicate for determining if a user can access the autotranslate views
    """
    fn_path = getattr(settings, 'AUTOTRANSLATE_ACCESS_CONTROL_FUNCTION', None)
    if fn_path is None:
        return is_superuser_staff_or_in_translators_group
    else:
        perm_module, perm_func = fn_path.rsplit('.', 1)
        perm_module = importlib.import_module(perm_module)
        return getattr(perm_module, perm_func)


def is_superuser_staff_or_in_translators_group(user):
    if not getattr(settings, 'AUTOTRANSLATE_REQUIRES_AUTH', True):
        return True
    try:
        if not user.is_authenticated():
            return False
        else:
            if user.is_superuser and user.is_staff:
                return True
            return user.groups.filter(name='translators').exists()

    except AttributeError:
        if not hasattr(user, 'is_authenticated') or not hasattr(user, 'is_superuser') or not hasattr(user, 'groups'):
            raise ImproperlyConfigured('If you are using custom User Models you must implement a custom authentication method for autotranslate. See AUTOTRANSLATE_ACCESS_CONTROL_FUNCTION here: https://dj-translate.readthedocs.org/en/latest/settings.html')
        raise


def can_translate_language(user, langid):
    try:
        if not autotranslate_settings.AUTOTRANSLATE_LANGUAGE_GROUPS:
            return can_translate(user)
        else:
            if not user.is_authenticated():
                return False
            if user.is_superuser and user.is_staff:
                return True
            return user.groups.filter(name='translators-%s' % langid).exists()

    except AttributeError:
        if not hasattr(user, 'is_authenticated') or not hasattr(user, 'is_superuser') or not hasattr(user, 'groups'):
            raise ImproperlyConfigured('If you are using custom User Models you must implement a custom authentication method for autotranslate. See AUTOTRANSLATE_ACCESS_CONTROL_FUNCTION here: https://dj-translate.readthedocs.org/en/latest/settings.html')
        raise