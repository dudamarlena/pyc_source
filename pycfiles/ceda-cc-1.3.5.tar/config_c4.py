...
# [GCC 8.4.0]
# Embedded file name: /data/work/tmp/git/ceda-cc/ceda_cc/ceda_cc_config/config_c4.py
# Compiled at: 2017-06-26 09:36:07
"""config_c4
##########
USAGE:
import config_c4
...
"""
import string, utils_config as utils, os, os.path as op, shutil, collections
...
validCmip5Frequencies = [
 'fx', 'yr', 'monClim', 'mon', 'day', '6hr', '3hr', 'subhr']
...
validSpecsExptFamilies = map(lambda x: string.strip(x), open(op.join(CC_CONFIG_DIR, 'specs_vocabs/exptFamily.txt')).readlines())
validCordexDomainsL = [
 'SAM-44', 'CAM-44', 'NAM-44', 'EUR-44', 'AFR-44', 'WAS-44', 'EAS-44', 'CAS-44', 'AUS-44', 'ANT-44', 'ARC-44', 'MED-44']
...
plevRequired = [
 'clh', 'clm', 'cll', 'ua850', 'va850', 'ta850', 'hus850', 'ua500', 'va500', 'ta500', 'zg500', 'ua200', 'va200', 'ta200', 'zg200']
...
heightRequired = [
 'tas', 'tasmax', 'tasmin', 'huss', 'sfcWind', 'sfcWindmax', 'wsgsmax', 'uas', 'vas']
heightValues = {}
heightRange = {}
...
validSpecsInstitutions = [
 'IC3', 'MPI-M', 'KNMI', 'UOXF', 'CNRM-CERFACS', 'ENEA', 'MOHC', 'SMHI', 'IPSL', 'UREAD', 'ECWMF']
  ...
  """
    ...
    def __init__(self, project, version=-1):
        ...
        if project == 'CORDEX':
            self.requiredGlobalAttributes = [
             'institute_id', 'contact', 'rcm_version_id', 'product', 'CORDEX_domain', 'creation_date',
            ...
            self.drsMappings = {'variable': '@var', 'institute': 'institute_id', 'product': 'product', 'experiment': 'experiment_id', 'ensemble': 'driving_model_ensemble_member', 
               'model': 'model_id', 'driving_model': 'driving_model_id', 'frequency': 'frequency', 
               'project': 'project_id', 
               'domain': 'CORDEX_domain', 'model_version': 'rcm_version_id'}
        elif project == 'SPECS':
            ...
            self.globalAttributesInFn = lrdr.driver(('simpleList', 'globalAttributesInFn_sv0101.txt',
                                                     None, None, 'noneMap'))
            self.requiredVarAttributes = ['long_name', 'standard_name', 'units']
            oo, self.drsMappings = lrdr.driver(('simpleList', 'drsMappings_sv0101.txt',
                                                0, None, 'returnMappings'))
        elif project == 'CMIP5':
            ...
            self.controlledGlobalAttributes = [
             'project_id', 'experiment_id', 'frequency', 'Conventions', 'modeling_realm',
             'initialization_method', 'physics_version', 'realization']
            self.globalAttributesInFn = [None, '@mip_id', 'model_id', 'experiment_id', '@ensemble']
            self.requiredVarAttributes = [
             'long_name', 'standard_name', 'units']
            self.drsMappings = {'variable': '@var', 'institute': 'institute_id', 'product': 'product', 'experiment': 'experiment_id', 'ensemble': '@ensemble', 
               'model': 'model_id', 'realm': 'modeling_realm', 'frequency': 'frequency', 
               'table': '@mip_id', 'project': 'project_id'}
        elif project == 'CCMI':
            ...
            self.requiredVarAttributes = ['long_name', 'standard_name', 'units']
            self.drsMappings = {'variable': '#var', 'platform': 'platform', 'sensor': 'sensor', 'level': '#level', 'standard_name': '*standard_name', 
               'activity': '=CLIPC', 
               'algorithm': '$algorithm:unset', 
               'frequency': '$frequency', 'spatial_resolution': 'spatial_resolution', 
               'ecv': '@ecv', 'version': '#version', 'convention_version': '#gdsv'}
        ...
        self.mipVocabPars = [
         self.mipVocabDir, self.mipVocabTl, self.mipVocabVgmap, self.mipVocabFnpat]
        if self.project == 'CORDEX':
            ...
            self.fnoptions['inFn'] = [
             [
              None, '*activity', '*level', '*project', '*var', '*product', '*additional', '*gdsv', '*version'],
             [
    ...
    def getVocabs(self):
        if self.projectV.id == 'SPECS':
            vocabs = {'variable': utils.mipVocab(self), 'Conventions': utils.listControl('Conventions', ['CF-1.6']), 
            ...
            lrdr = readVocab('cmip5_vocabs/')
            vocabs = {'variable': utils.mipVocab(self), 'Conventions': utils.listControl('Conventions', ['CF-1.4', 'CF-1.5']), 
            ...
            lrdr = readVocab('ccmi_vocabs/')
            vocabs = {'variable': utils.mipVocab(self), 'frequency': utils.listControl('frequency', validCcmiFrequencies), 
            ...
            vocabs = {'variable': utils.mipVocab(self), 'version': utils.patternControl('version', '^(fv[0-9]+(\\.[0-9]+){0,1})$', examples=['fv1.1']), 
            ...
            vocabs = {'variable': utils.mipVocab(self), 'driving_experiment_name': utils.listControl('driving_experiment_name', validCordexExperiment), 
