...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/phessianfree/objective.py
# Compiled at: 2012-11-20 21:58:30
...
class Objective(object):
    ...
    def make_hv(self, x, p):
        ...
        s, e = self.partRange(p)
        fdEps = linalg.norm(right_grad, inf) * self.props.get('fdEps', 1e-08)
    ...
    def __call__(self, x, expand=False):
        ...
        for p in range(self.parts):
            ...
            if p >= self.currentSubsetParts:
                ...
                if standardErr < self.errBound and fraction <= 0.8 and fraction >= 0.05 and p >= 4:
                    break

        self.currentSubsetParts = p + 1
        scale = self.ndata / float(self.partRange(p)[1])
        self.logger.debug('For objective eval used %d/%d of data (se: %1.2f)', self.currentSubsetParts, self.parts, standardErr)
        return (
         loss * scale, g * scale)
