...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/phessianfree/innersolve.py
# Compiled at: 2012-11-20 22:02:40
...
def cg(f, xk, gfk, k, vecs, props):
    ...
    pk, cginfo = scipy.sparse.linalg.cg(mulOp, -gfk, x0=x0, tol=1e-10, maxiter=maxiter, callback=callback)
    return pk
...
def lbfgs(f, xk, gfk, k, vecs, props):
    ...
    for i in range(maxiter):
        ...
        kmax = max_key(vecs)
        if pkHpk < 0:
            raise Exception('Hessian is not positive semi-definite. ' + 'Try using the Gauss-Newton approximation to the hessian.' + 'If your problem is convex, your gradient ' + 'calculation may just be wrong')
        else:
            vecs[kmax] = (
             pk, Hpk, 1.0 / numpy.dot(pk, Hpk))
            kmax = kmax + 1
            if wHw > 0:
                vecs[kmax] = (
                 w, Hw, 1.0 / numpy.dot(w, Hw))
        wp = w - sst * pk
