...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/phessianfree/linesearch.py
# Compiled at: 2012-11-20 23:23:28
import logging
from numpy import *
...
def weak_wolfe(f, xk, upper_val, grad, pk, props):
    ...
    for i in range(maxIter):
        ...
        if isinf(cval) or isnan(cval):
            ...
            logger.debug(' Armijo condition failed cval=%1.5f. Need: %1.5f < %1.5f', cval, cval, upper_val + t * c1 * directional_derivative)
            bracket_right = t
            bracket_right_values = (cval, cgrad)
            smallest_so_far = (
             t, cval, cgrad)
        elif useWolfe and dot(cgrad, pk) < c2 * directional_derivative:
            if abs(dot(cgrad, pk)) < 1e-05:
                logger.debug(' Wolfe condition failed cval=%1.5f. Need %1.1e > %1.1e', cval, dot(cgrad, pk), c2 * directional_derivative)
            else:
                logger.debug(' Wolfe condition failed cval=%1.5f. Need %1.5f > %1.5f (dd at t=0: %1.5f)', cval, dot(cgrad, pk), c2 * directional_derivative, directional_derivative)
            bracket_left = t
        ...
        if bracket_right < inf:
            ...
            rdd = dot(rgrad, pk)
            if rdd < 0 or ldd > 0:
                logger.error('directional derivatives suggest step outside valley:' + ' ldd: %1.1e (want <0), rdd: %1.1e (want >0)', ldd, rdd)
                t = 0.1 * t
            ...
            t = 2 * t
        logger.info('   LS %d %s: trying t = %1.4e (from bracket: [%1.1e, %1.1e])', i, interpMethod, t, bracket_left, bracket_right)
...
def strong_wolfe(f, xk, upper_val, grad, pk, props):
    ...
    def interp(k, lt, ldd, lval, rt, rdd, rval):
    ...
    def zoom(lt, lval, lgrad, rt, rval, rgrad):
        ...
        rdd = dot(rgrad, pk)
        for i in range(maxIter):
            logger.debug('Finding interp between %1.10e, and %1.10e', lt, rt)
            if lt > rt:
                t = interp(i, rt, rdd, rval, lt, ldd, lval)
            else:
                t = interp(i, lt, ldd, lval, rt, rdd, rval)
            logger.debug('Choose %1.4e', t)
            cval, cgrad = phi(t)
            tdd = dot(cgrad, pk)
            logger.info('cval: %1.5f, tdd: %1.4e, t=%1.1e [lt: %1.1e, rt: %1.1e]', cval, tdd, t, lt, rt)
            if cval > upper_val + c1 * t * dd or cval >= 1.1 * lval:
                logger.debug('want cval %1.4f <= %1.4f and < lval: %1.4f to exit', cval, upper_val + c1 * t * dd, 1.1 * lval)
                rval = cval
                rgrad = cgrad
                rt = t
            else:
                ...
                if tdd * (rt - lt) >= 0:
                    ...
                    rgrad = lgrad
                    rt = lt
                logger.debug('ZOOM: wolfe failed, want %1.4e <= %1.4e', abs(tdd), -c2 * dd)
                lval = cval
                lgrad = cgrad
                lt = t

        if cval < upper_val + c1 * t * dd:
            logger.error('Line search failed, but continuing anyway')
            return (
             t, cval, cgrad)
        raise Exception('Line search failed')
