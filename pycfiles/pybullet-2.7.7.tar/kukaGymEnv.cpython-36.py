...
# [GCC 8.4.0]
# Embedded file name: /home/erwincoumans/dev/tmp/bullet3/examples/pybullet/gym/pybullet_envs/bullet/kukaGymEnv.py
# Compiled at: 2020-04-29 23:02:34
# Size of source mod 2**32: 10511 bytes
import os, inspect
...
import numpy as np, time, pybullet as p
from . import kuka
...
class KukaGymEnv(gym.Env):
    ...
    def __init__(self, urdfRoot=pybullet_data.getDataPath(), actionRepeat=1, isEnableSelfCollision=True, renders=False, isDiscrete=False, maxSteps=1000):
        self._isDiscrete = isDiscrete
        self._timeStep = 0.004166666666666667
    ...
    def reset(self):
        ...
        p.loadURDF(os.path.join(self._urdfRoot, 'table/table.urdf'), 0.5, 0.0, -0.82, 0.0, 0.0, 0.0, 1.0)
        xpos = 0.55 + 0.12 * random.random()
        ypos = 0 + 0.2 * random.random()
        ang = 1.57 + 3.1415925438 * random.random()
    ...
    def step2(self, action):
        ...
        npaction = np.array([
         action[3]])
        actionCost = np.linalg.norm(npaction) * 10.0
    ...
    def _termination(self):
        ...
        if self.terminated or self._envStepCounter > self._maxSteps:
            ...
            if len(closestPoints):
                ...
                for i in range(100):
                    graspAction = [
                     0, 0, 0.0001, 0, fingerAngle]
                    self._kuka.applyAction(graspAction)
                    p.stepSimulation()
                    fingerAngle = fingerAngle - 0.003
                    if fingerAngle < 0:
                        fingerAngle = 0

                for i in range(1000):
                    graspAction = [
                     0, 0, 0.001, 0, fingerAngle]
                    self._kuka.applyAction(graspAction)
                    p.stepSimulation()
                    blockPos, blockOrn = p.getBasePositionAndOrientation(self.blockUid)
                    if blockPos[2] > 0.23:
