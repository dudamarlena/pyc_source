...
# Embedded file name: build/bdist.macosx-10.3-fat/egg/plone/recipe/cluster/ctl.py
# Compiled at: 2008-08-11 05:37:05
__doc__ = 'Control file\n\n$Id: ctl.py 60421 2008-03-11 09:19:36Z tarek $\n'
import sys, time, os
...
class Cluster(object):
    ...
    def __init__(self, stdin=sys.stdin, stdout=sys.stdout, stderr=sys.stderr, pidfile=None, args=sys.argv[1:], foreground=True):
    ...
    def _system(self, command, background=False):
        if not background:
            ...
            self.stderr.flush()
            p = Popen([command], shell=True, stderr=PIPE, stdin=PIPE, stdout=PIPE, close_fds=True)
