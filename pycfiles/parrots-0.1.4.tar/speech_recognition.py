...
# [GCC 8.4.0]
# Embedded file name: /Users/xuming06/Codes/parrots/parrots/speech_recognition.py
# Compiled at: 2018-09-13 07:33:16
"""
...
class SpeechRecognition(object):
    ...
    def __init__(self, pinyin_path=config.pinyin_hanzi_dict_path, model_path=config.speech_recognition_model_path, ms_output_size=1422):
    ...
    def initialize(self):
        ...
        if self.pinyin_path:
            t1 = time.time()
            try:
                self.pinyin_list = get_pinyin_list(self.pinyin_path)
            except IOError:
                pinyin_path = os.path.join(pwd_path, '..', self.pinyin_path)
                self.pinyin_list = get_pinyin_list(pinyin_path)

            default_logger.debug('Loading pinyin dict cost %.3f seconds.' % (time.time() - t1))
        if self.model_path:
            t2 = time.time()
            try:
            ...
            default_logger.debug('Loading model cost %.3f seconds.' % (time.time() - t2))
    ...
    def create_model(self):
        ...
        opt = Adam(lr=0.001, beta_1=0.9, beta_2=0.999, decay=0.0, epsilon=1e-07)
