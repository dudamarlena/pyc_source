...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/total_scattering/inelastic/incident_spectrum.py
...
from mantid.simpleapi import CalculateEfficiencyCorrection, ConvertToPointData, ConvertUnits, CreateWorkspace, Divide, LoadNexusMonitors, Multiply, Rebin, ResampleX, SplineSmoothing
...
def fitCubicSpline(x_fit, y_fit, x, s=1000000000000000.0):
...
def fitHowellsFunction(x_fit, y_fit, x):
    ...
    def calc_HowellsFunction(lambdas, phi_max, phi_epi, lam_t, lam_1, lam_2, a):
        term1 = phi_max * (lam_t ** 4.0 / lambdas ** 5.0) * np.exp(-(lam_t / lambdas) ** 2.0)
        term2 = phi_epi / lambdas ** (1.0 + 2.0 * a) * (1.0 / (1 + np.exp((lambdas - lam_1) / lam_2)))
        return term1 + term2

    def calc_HowellsFunction1stDerivative(lambdas, phi_max, phi_epi, lam_t, lam_1, lam_2, a):
        term1 = (2 * lam_t ** 2 / lambdas ** 2 - 5.0) * (1.0 / lambdas) * phi_max * (lam_t ** 4.0 / lambdas ** 5.0) * np.exp(-(lam_t / lambdas) ** 2.0)
        term2 = (1 + 2 * a) / lambdas * (1.0 / lambdas) * (phi_epi / lambdas ** (1.0 + 2.0 * a)) * (1.0 / (1 + np.exp((lambdas - lam_1) / lam_2)))
        return term1 + term2
...
def GetIncidentSpectrumFromMonitor(Filename, OutputWorkspace='IncidentWorkspace', IncidentIndex=0, TransmissionIndex=1, Binning='.1,6000,2.9', BinType='ResampleX'):
    monitor = 'monitor'
    LoadNexusMonitors(Filename=Filename, OutputWorkspace=monitor)
    ConvertUnits(InputWorkspace=monitor, OutputWorkspace=monitor, Target='Wavelength', EMode='Elastic')
    ...
    if BinType == 'ResampleX':
        ResampleX(InputWorkspace=monitor, OutputWorkspace=monitor, XMin=[
         lambdaMin], XMax=[
         lambdaMax], NumberBins=abs(int(lambdaBinning)), LogBinning=int(lambdaBinning) < 0, PreserveEvents=True)
    elif BinType == 'Rebin':
        Rebin(InputWorkspace=monitor, OutputWorkspace=monitor, Params=[
         lambdaMin, lambdaBinning, lambdaMax], PreserveEvents=True)
    ConvertToPointData(InputWorkspace=monitor, OutputWorkspace=monitor)
    lam = mtd[monitor].readX(IncidentIndex)
    bm = mtd[monitor].readY(IncidentIndex)
    p = 0.000794807
    thickness = 0.1
    abs_xs_3He = 5333.0
    p_to_rho = 2.43e-05
    e0 = abs_xs_3He * lam / 1.798 * p_to_rho * p * thickness
    print ('Efficiency:', 1.0 - np.exp(-e0))
    bmeff = bm / (1.0 - np.exp(-e0))
    print bmeff
    CreateWorkspace(DataX=lam, DataY=bmeff, OutputWorkspace=OutputWorkspace, UnitX='Wavelength')
    mtd[OutputWorkspace].setYUnit('Counts')
    return mtd[OutputWorkspace]


def FitIncidentSpectrum(InputWorkspace, OutputWorkspace, FitSpectrumWith='GaussConvCubicSpline', BinningForFit='0.15,0.05,3.2', BinningForCalc=None, PlotDiagnostics=False):
    ...
    if FitSpectrumWith == 'CubicSpline':
        fit, fit_prime = fitCubicSpline(x_fit, y_fit, x, s=10000000.0)
        if PlotDiagnostics:
...
if '__main__' == __name__:
    ...
    if run_file:
        ...
        if nomad_test:
            runs[0] = 'NOM_33943'
            binning = '0.0212406,0.00022,3.39828'
        print (
         'Processing Scan: ', runs[0])
        monitor = 'monitor'
        ...
        ConvertUnits(InputWorkspace=monitor, OutputWorkspace=monitor, Target='Wavelength', EMode='Elastic')
        Rebin(InputWorkspace=monitor, OutputWorkspace=monitor, Params=binning, PreserveEvents=False)
        ax_bm.plot(mtd[monitor], '-', wkspIndex=0, label='Monitor', distribution=True)
        ax_bm.ticklabel_format(style='sci', axis='x', scilimits=(0, 0))
        CalculateEfficiencyCorrection(InputWorkspace=monitor, ChemicalFormula='(He3)', DensityType='Number Density', Density=1.93138101e-08, Thickness=0.1, OutputWorkspace=incident_ws)
        Divide(LHSWorkspace=monitor, RHSWorkspace=incident_ws, OutputWorkspace=incident_ws)
        ax_bmeff.plot(mtd[incident_ws], '-', wkspIndex=0, label='Incident Spectrum (density)', distribution=True)
        CalculateEfficiencyCorrection(InputWorkspace=monitor, ChemicalFormula='(He3)', Efficiency=1.03e-05, OutputWorkspace=incident_ws)
        Divide(LHSWorkspace=monitor, RHSWorkspace=incident_ws, OutputWorkspace=incident_ws)
        ax_bmeff.plot(mtd[incident_ws], 'o', wkspIndex=0, label='Incident Spectrum (efficiency)', distribution=True)
        CalculateEfficiencyCorrection(InputWorkspace=monitor, Alpha=-5.72861786781e-06, OutputWorkspace=incident_ws)
        Divide(LHSWorkspace=monitor, RHSWorkspace=incident_ws, OutputWorkspace=incident_ws)
        ax_bmeff.plot(mtd[incident_ws], '--', wkspIndex=0, label='Incident Spectrum (alpha)', distribution=True)
        ax_bm.legend()
        ax_bmeff.legend()
        plt.show()
        exit()
    ...
    if howells_test:
        ...
        def delta(lam, lam_1, lam_2, lam_3=None, lam_4=None, alpha=None):
            retVal = 1.0 / (1.0 + np.exp((lam - lam_1) / lam_2))
            if lam_3 and lam_4 and alpha:
                term_2 = 1.0 + alpha / (1.0 + np.exp((lam_3 - lam) / lam_4))
                retVal *= term_2
            return retVal
        ...
        def phi_m(lam, **kwargs):
            ...
            lam_t = kwargs['lam_t']
            return phi_max * (lam_t ** 4.0 / lam ** 5.0) * np.exp(-(lam_t / lam) ** 2.0)
        ...
        incident_spectrums['Ambient 300K polyethylene'] = {'phi_max': 6324, 
           'phi_epi': 786, 
           'lam_t': 1.58, 
           'alpha': 0.099, 
           'lam_1': 0.67143, 
           'lam_2': 0.06075}
        incident_spectrums['Ambient 300K poisoned (Gd "meat" in polyethylene slabs)'] = {'phi_max': 1200, 
           'phi_epi': 786, 
           'lam_t': 1.58, 
           'alpha': 0.099, 
           'lam_1': 0.67143, 
           'lam_2': 0.06075}
        incident_spectrums['Cold 77K polyethylene (Howells)'] = {'phi_max': 3838, 
           'phi_epi': 1029, 
           'lam_t': 2.97, 
           'alpha': 0.089, 
           'lam_1': 1.3287, 
           'lam_2': 0.14735}
        incident_spectrums['Cold 77K polyethylene (MildnerEtAl)'] = {'phi_max': 3838, 
           'phi_epi': 1029, 
           'lam_t': 2.97, 
           'alpha': 0.089, 
           'lam_1': 1.3287, 
        ...
        for moderator, spectrum_params in incident_spectrums.items():
            ...
            CreateWorkspace(OutputWorkspace=incident_ws, NSpec=1, DataX=[0], DataY=[0], UnitX='Wavelength', VerticalAxisUnit='Text', VerticalAxisValues='IncidentSpectrum')
            ...
            eff_ws = 'efficiency'
            CalculateEfficiencyCorrection(InputWorkspace=incident_ws, Alpha=-0.693, OutputWorkspace=eff_ws)
