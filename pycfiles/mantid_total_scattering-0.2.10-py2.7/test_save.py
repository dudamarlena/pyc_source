...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/file_handling/test_save.py
...
from mantid.simpleapi import mtd, LoadNexusProcessed, LoadAscii, ConvertToHistogram

class TestSave(unittest.TestCase):

    def setUp(self):
        align_and_focus_args = {'CalFilename': os.path.join(EXAMPLE_DIR, 'isis', 'polaris_grouping.cal'), 
           'ResampleX': -6000, 
           'DSpacing': False, 
    ...
    def test_save_banks_check_contents(self):
        ...
        self.assertEqual(out_wksp.blocksize(), self.wksp.blocksize())
        self.assertEqual(out_wksp.getNumberHistograms(), self.wksp.getNumberHistograms())
        self.assertTrue(np.array_equal(out_wksp.getAxis(0).extractValues(), self.wksp.getAxis(0).extractValues()))

    def test_save_banks_binning(self):
        save_banks(self.wksp, self.out_nxs, 'wksp', '.', Binning='0,100,10000')
        out_wksp = LoadNexusProcessed(self.out_nxs)
        self.assertNotEqual(out_wksp.blocksize(), self.wksp.blocksize())
    ...
    def test_save_file_check_contents(self):
        ...
        self.assertEqual(out_wksp.blocksize(), self.wksp.blocksize())
        self.assertEqual(out_wksp.getNumberHistograms(), self.wksp.getNumberHistograms())
        self.assertTrue(np.allclose(out_wksp.getAxis(0).extractValues(), self.wksp.getAxis(0).extractValues()))
