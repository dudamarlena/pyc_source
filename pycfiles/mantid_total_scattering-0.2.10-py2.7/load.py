...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/total_scattering/file_handling/load.py
...
required_shape_keys = {'FlatPlate': [
               'Shape', 'Width', 'Height', 'Thick', 'Center', 'Angle'], 
   'Cylinder': [
              'Shape', 'Height', 'Radius', 'Center'], 
   'HollowCylinder': [
                    'Shape', 'Height', 'InnerRadius', 'OuterRadius', 'Center']}

def load(ws_name, input_files, geometry=None, chemical_formula=None, mass_density=None, **align_and_focus_args):
    AlignAndFocusPowderFromFiles(OutputWorkspace=ws_name, Filename=input_files, Absorption=None, **align_and_focus_args)
    NormaliseByCurrent(InputWorkspace=ws_name, OutputWorkspace=ws_name, RecalculatePCharge=True)
    if geometry and chemical_formula and mass_density:
        set_sample(ws_name, geometry, chemical_formula, mass_density)
    ConvertUnits(InputWorkspace=ws_name, OutputWorkspace=ws_name, Target='MomentumTransfer', EMode='Elastic')
    return ws_name


def set_sample(ws_name, geometry=None, chemical_formula=None, mass_density=None):
    if 'Center' not in geometry:
        geometry.update({'Center': [0.0, 0.0, 0.0]})
    if 'Shape' not in geometry:
        geometry.update({'Shape': 'Cylinder'})
    geometry = configure_geometry(geometry)
    SetSample(InputWorkspace=ws_name, Geometry=geometry, Material={'ChemicalFormula': chemical_formula, 
