...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/total_scattering/file_handling/save.py
# Compiled at: 2019-10-03 13:30:51
import os
from mantid import mtd
from mantid.api import IEventWorkspace
from mantid.simpleapi import CloneWorkspace, Rebin, ConvertToDistribution, DiffractionFocussing, SaveNexusProcessed, SaveAscii, DeleteWorkspace
...
def save_banks(InputWorkspace, Filename, Title, OutputDir, Binning=None, GroupingWorkspace=None):
    ...
    tmp_wksp = mtd['__tmp']
    if Binning:
        tmp_wksp = Rebin(InputWorkspace=tmp_wksp, Params=Binning, PreserveEvents=True)
    ...
    if isEventWksp and GroupingWorkspace and yunit == 'Counts':
        tmp_wksp = DiffractionFocussing(InputWorkspace=tmp_wksp, GroupingWorkspace=GroupingWorkspace, PreserveEvents=False)
    filename = os.path.join(os.path.abspath(OutputDir), Filename)
    SaveNexusProcessed(InputWorkspace=tmp_wksp, Filename=filename, Title=Title, Append=True, PreserveEvents=False, WorkspaceIndexList=range(tmp_wksp.getNumberHistograms()))
...
def save_file(ws, filename, header=None):
    ...
    SaveAscii(InputWorkspace=ws, Filename=filename, Separator='Space', ColumnHeader=False, AppendToFile=True)