...
# Embedded file name: /Users/david.jetelina/.local/share/virtualenvs/pyCardDeck/lib/python3.6/site-packages/pyCardDeck/deck.py
# Compiled at: 2018-10-23 07:40:23
# Size of source mod 2**32: 21020 bytes
import logging, os, yaml, jsonpickle
...
class Deck:
    ...
    def __init__(self, cards: Optional[List[CardType]]=None, reshuffle: object=True, name: str=None, discard: Optional['Deck']=None):
        """
        Create the deck
        """
        self.name = name
    ...
    def _get_card(self, position: str='top') -> CardType:
        ...
        """
        if len(self._cards):
            positions = {'top':0,  'bottom':len(self._cards) - 1, 
    ...
    def discard(self, card: CardType) -> None:
        ...
        if card or type(card) == int:
            ...
            log.warning('You tried to insert %s (rank(%s) into a discard pile', card, type(card).__name__)
            raise NotACard('You tried to insert {} (rank({}) into a discard pile'.format(card, type(card).__name__))
    ...
    def __repr__(self) -> str:
        ...
        :rtype:     string
        """
        return 'Deck(cards={0}, discarded={3}, reshuffle={1}, name={2})'.format(self.cards_left, self._reshuffle, self.name, self.discarded)
    ...
    if second_card == card:
        ...
        try:
            if second_card.__dict__ == card.__dict__:
                if type(second_card).__name__ == type(card).__name__:
                    identity = True
        except AttributeError:
            pass
