...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/errorreporter/formatter.py
# Compiled at: 2012-01-03 10:11:15
"""
...
class AbstractFormatter(object):
    ...
    def __init__(self, show_hidden_frames=False, include_reusable=True, show_extra_data=True, trim_source_paths=(), **kwargs):
    ...
    def format_collected_data(self, exc_data):
        ...
        for frame in frames:
            ...
            if source:
                lines.append(self.format_long_source(filename, source, long_source))
...
class TextFormatter(AbstractFormatter):
    ...
    def format_combine(self, data_by_importance, lines, exc_info):
        ...
        return (
         self.format_combine_lines(lines), '')
    ...
    def zebra_table(self, title, rows, table_class='variables'):
        ...
        table = [
         ...
         '<tr class="header"><th colspan="2">%s</th></tr>' % self.quote(title)]
        odd = False
        for (name, value) in rows:
            ...
            odd = not odd
            table.append('<tr class="%s"><td>%s</td>' % (
             odd and 'odd' or 'even', self.quote(name)))
            table.append('<td><tt>%s</tt></td></tr>' % self.quote(truncate(value)))
...
class XMLFormatter(AbstractFormatter):
    ...
    def format_collected_data(self, exc_data):
        ...
        for frame in frames:
            ...
            long_source = frame.get_source_line(2)
            if source:
                self.format_long_source(filename, source.decode(frame.source_encoding, 'replace'), long_source.decode(frame.source_encoding, 'replace'), newdoc, xml_frame)
            if frame.supplement:
        ...
        for (name, value_list) in exc_data.extra_data.iteritems():
            ...
            for value in value_list:
                ...
                for (k, v) in value.items():
                    ...
                    content.appendChild(self.create_text_node(newdoc, 'value', for_display(v)))
        ...
        top_element.appendChild(self.format_exception_info(exc_data, newdoc, frame))
    ...
    def format_exception_info(self, exc_data, newdoc, frame):
        ...
        evalue = evalue.decode(frame.source_encoding, 'replace').encode('ascii', 'xmlcharrefreplace')
    ...
    if pkg_resources and libs:
        ...
        for lib in libs:
            ...
            if lib in working_set.by_key:
                ...
                libraries.update(get_dependencies(circ_check, lib, working_set))
