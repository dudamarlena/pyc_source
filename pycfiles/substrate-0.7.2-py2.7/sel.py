...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/substrate/data/local/substrate/lib/webtest/sel.py
...
"""
import os, cgi, sys, time, signal, socket, types, webob, logging, warnings, tempfile, unittest, threading, subprocess
...
def context_manager(resp):
    ...
    if not is_available():
        ...
        fd = tempfile.NamedTemporaryFile(prefix='webtest-selenium-', suffix='.html')
        fd.write(resp.body)
        fd.flush()
...
def selenium(obj):
    ...
    if isinstance(obj, type):
        ...
        if isinstance(obj, testapp.TestResponse):
...
class Selenium(object):
    ...
    def start(self, url):
        self.driver = os.environ.get('SELENIUM_DRIVER', '*googlechrome')
        self.session_id = self.getNewBrowserSession(self.driver, url, '', 'captureNetworkTraffic=true', 'addCustomRequestHeader=true')
...
class SeleniumApp(testapp.TestApp):
    ...
    def _run_server(self, app):
        ...
        def run():
            httpd = simple_server.make_server(ip, port, app, server_class=WSGIServer, handler_class=WSGIRequestHandler)
            httpd.serve_forever()
...
class TestResponse(testapp.TestResponse):
    ...
    def follow(self, status=None, **kw):
        ...
        if not (self.status_int >= 300 and self.status_int < 400):
            raise ValueError('You can only follow 301 and 302. Not %s' % self.status_int)
    ...
    def click(self, description=None, linkid=None, href=None, anchor=None, index=None, verbose=False, extra_environ=None, timeout=None):
        link = self.doc.link(description=description, linkid=linkid, href=href, index=index)
        link.click()
        if timeout == 0:
            return self
...
class Element(object):
    ...
    def __getattr__(self, attr):
        ...
        def wrapped(*args):
            args = [
             cmd, self.locator] + [ str(a) for a in args ]
    ...
    def eval(self, *expr):
        ...
        """
        script = "(function(s) {var l = %r;var b = s.browserbot; var e = b.findElement(l);var res = %s; return res || 'null';}(this))" % (
...
class Document(object):
    ...
    def link(self, description=None, linkid=None, href=None, index=None):
        """Get a link"""
        return self.get('a', description=description, id=linkid, href=href, index=index)

    def input(self, value=None, name=None, inputid=None, index=None):
        """Get an input field"""
        return self.get('input', id=inputid, value=value, name=name, index=index)

    def button(self, description=None, buttonid=None, index=None):
        """Get a button"""
        return self.get('button', description=description, id=buttonid, index=index)
...
class Field(testapp.Field, Element):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._forced_values = []
        self.locator = _eval_xpath(self.tag, locator=self.form.locator, name=self.name)
...
class Radio(Field):
    ...
    def value__get(self):
        ...
        if not value:
            ...
            if len(value) == 1:
                return value[0]
            raise ValueError('Got more than one value for %r: %s' % (self, value))
            return
...
class Form(testapp.Form, Element):
    ...
    def submit(self, name=None, index=None, extra_environ=None, timeout=None):
        ...
        if name:
            selector = _eval_xpath('input', locator=self.locator, name=name, index=index)
...
class WSGIApplication(object):
    ...
    def __call__(self, environ, start_response):
        ...
        if '__file__' in environ['PATH_INFO']:
            ...
            body.replace('http://localhost/', 'http://%s/' % req.host)
            if PY3:
                resp.text = body
            else:
                resp.body = body
...
def is_available():
    ...
    running"""
    if json == False:
        warnings.warn('selenium is not available because no json module are available. Consider installing simplejson', SeleniumWarning)
