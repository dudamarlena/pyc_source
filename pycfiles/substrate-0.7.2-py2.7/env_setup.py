...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/substrate/data/env_setup.py
...
def setup_tests():
    ...
    if not hasattr(sys, 'version_info'):
        sys.stderr.write('Very old versions of Python are not supported. Please use version 2.7 or greater.\n')
        sys.exit(1)
    version_tuple = tuple(sys.version_info[:2])
    if version_tuple != (2, 7):
        sys.stderr.write('Warning: Python %d.%d is not supported. Please use version 2.7.\n' % version_tuple)
    if not dir_path:
