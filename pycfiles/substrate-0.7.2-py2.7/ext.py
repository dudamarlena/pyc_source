...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/substrate/data/local/substrate/lib/webtest/ext.py
...
import subprocess, threading, socket, time, sys, os
...
def casperjs(test_app):
    ...
    def run(script, *args):
        ...
        if binary:
            cmd = [
             binary, 'test'] + list(args) + [script]
            p = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            p.wait()
            output = p.stdout.read()
            if to_bytes('FAIL') in output:
                print to_string(output)
                raise AssertionError('Failure while running %s' % (' ').join(cmd))
