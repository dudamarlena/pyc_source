...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/substrate/data/lib/substrate/agar/counter.py
...
class TimedWriteBehindCounter(db.Model):
    ...
    def incr(cls, name, now=None, interval=5, countdown=0, value=1):
        ...
        try:
            deferred.defer(cls.flush_counter, name, now, _name=task_name, _countdown=countdown)
        except (taskqueue.TaskAlreadyExistsError, taskqueue.TombstonedTaskError):
            pass
...
class HourlyWriteBehindCounter(TimedWriteBehindCounter):
    ...
    def normalize_ts(cls, ts):
        if ts.tzinfo is not None:
            ts = utc.normalize(ts.astimezone(utc))
        return datetime.datetime(year=ts.year, month=ts.month, day=ts.day, hour=ts.hour)
