...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/substrate/data/local/substrate/lib/management.py
...
if version_tuple != (2, 5) and version_tuple != (2, 7):
    sys.stderr.write('Warning: Python %d.%d is not supported. Please use version 2.5 or 2.7.\n' % version_tuple)
if not DIR_PATH:
...
def print_subcommand_overviews(substrate_commands, usr_commands):
    ...
    cmd_width = max(len(os.path.splitext(os.path.basename(command))[0]) for command in substrate_commands + usr_commands)
...
def run_command(command, globals_, script_dir=SCRIPT_DIR):
    ...
    script_path = (glob(os.path.join(SUBSTRATE_COMMAND_DIR, arg + '.py')) or glob(os.path.join(USR_COMMAND_DIR, arg + '.py')))[0]
