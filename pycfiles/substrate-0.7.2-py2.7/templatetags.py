...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/substrate/data/lib/substrate/agar/django/templatetags.py
...
class URLNode(Node):
    ...
    def render(self, context):
        ...
        kwargs = dict([ (str(k), v.resolve(context)) for k, v in self.kwargs.items()
                      ])
        url = None
        try:
...
def uri_for(parser, token):
    ...
    if len(bits) < 2:
        raise TemplateSyntaxError("'%s' takes at least one argument (route name)" % bits[0])
    routename = bits[1]
    args = []
    kwargs = {}
    asvar = None
