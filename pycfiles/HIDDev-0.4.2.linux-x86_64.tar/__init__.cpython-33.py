...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.3/dist-packages/hiddev/__init__.py
# Compiled at: 2014-03-18 22:02:34
# Size of source mod 2**32: 27274 bytes
"""
...
HIDIOCGDEVINFO = _IOR('H', 3, hiddev_devinfo)
HIDIOCGVERSION = _IOR('H', 1, _integer)
HIDIOCAPPLICATION = _IO('H', 2)
HIDIOCGDEVINFO = _IOR('H', 3, hiddev_devinfo)
HIDIOCGNAME = lambda len: _IOC(2, ord('H'), 6, len)
HIDIOCGSTRING = _IOR('H', 4, hiddev_string_descriptor)
HIDIOCINITREPORT = _IO('H', 5)
HIDIOCGREPORT = _IOW('H', 7, hiddev_report_info)
HIDIOCSREPORT = _IOW('H', 8, hiddev_report_info)
HIDIOCGREPORTINFO = _IOWR('H', 9, hiddev_report_info)
HIDIOCGFIELDINFO = _IOWR('H', 10, hiddev_field_info)
HIDIOCGUSAGE = _IOWR('H', 11, hiddev_usage_ref)
HIDIOCSUSAGE = _IOW('H', 12, hiddev_usage_ref)
HIDIOCGUCODE = _IOWR('H', 13, hiddev_usage_ref)
HIDIOCGFLAG = _IOR('H', 14, _integer)
HIDIOCSFLAG = _IOW('H', 15, _integer)
HIDIOCGCOLLECTIONINDEX = _IOW('H', 16, hiddev_usage_ref)
HIDIOCGCOLLECTIONINFO = _IOWR('H', 17, hiddev_collection_info)
...
class Unit:
    __doc__ = ' Unit and Exponent for physical HID values '

    def __init__(self, unitval, unit_exponent):
        values = ((unitval & 15 << i) >> i for i in range(0, 32, 4))
        self.value = unitval
    ...
    def __str__(self):
        ...
        if unitval == 16777441 and unit_exponent == 2:
            return 'Cd'
        if unitval == 57841 and unit_exponent == 5:
            return 'Pa'
        if unitval == 57841 and unit_exponent == 1:
            return 'Pa'
        if unitval == 57841 and unit_exponent == 5:
            return 'bar'
        if unitval == 57841 and unit_exponent == 2:
            return 'mbar'
        if unitval == 57617 and unit_exponent == 5:
            return 'N'
        if unitval == 2117601 and unit_exponent == -7:
            return 'F'
        if unitval == 53537 and unit_exponent == 5:
            return 'W'
        if unitval == 14737697 and unit_exponent == 5:
            return 'H'
        if unitval == 14737697 and unit_exponent == 5:
            return 'Ω'
        if unitval == 15782177 and unit_exponent == 5:
            return 'V'
        if unitval == 496 and unit_exponent == 0:
            return 'Hz'
        if unitval == 15786241 and unit_exponent == 0:
            return 'Gs'
        if unitval == 15786241 and unit_exponent == -4:
            return 'T'
        units_a = []
        units_b = []
        if self.cm:
        ...
        if 'cm²' in units_a and unit_exponent >= 2:
            ...
            if 'cm²' in units_b and unit_exponent <= 1:
        ...
        if 'g' in units_a and unit_exponent >= 2:
            ...
            if unit_exponent:
                ...
                prefix = ''
        if unitval & 15 in (0, 5, 6, 7):
            return prefix
        if not units_a and not units_b:
            return prefix
        else:
            if units_a and not units_b:
                return prefix + ' '.join(units_a)
            if units_b and not units_a:
...
class Field:
    ...
    FLAGS = {1: (None, 'constant'), 
     2: ('array', 'variable'), 
     4: (None, 'relative'), 
     8: (None, 'wrap'), 
     16: (None, 'nonlinear'), 
     32: (None, 'no_preferred'), 
     64: (None, 'null_state'), 
     128: (None, 'volatile'), 
     256: (None, 'buffered_bytes')}
...
class Report:
    ...
    TYPES = {HID_REPORT_TYPE_INPUT: 'input', 
...
class HIDDevice:
    ...
    def get_name(self) -> str:
        ...
        return buffer.rstrip(b'\x00').decode('UTF-8')
    ...
    def find_input(self, usage_code) -> '(Report, int, int)':
        """
                ...
                """
        struct = hiddev_usage_ref(HID_REPORT_TYPE_INPUT, 4294967295, 0, 0, usage_code)
    ...
    def find_output(self, usage_code) -> '(Report, int, int)':
        """
                        ...
                        Returns the Report object, the field index and the usage index matching the usage code.
                """
        struct = hiddev_usage_ref(HID_REPORT_TYPE_OUTPUT, 4294967295, 0, 0, usage_code)
        r, struct = self._ioctl_readwrite_struct(HIDIOCGUSAGE, struct)
        rstruct = hiddev_report_info(struct.report_type, struct.report_id)
        r, rstruct = self._ioctl_readwrite_struct(HIDIOCGREPORTINFO, rstruct)
        return (Report(self, rstruct), struct.field_index, struct.usage_index)
    ...
    def find_feature(self, usage_code) -> '(Report, int, int)':
        """
                        ...
                        Returns the Report object, the field index and the usage index matching the usage code.
                """
        struct = hiddev_usage_ref(HID_REPORT_TYPE_FEATURE, 4294967295, 0, 0, usage_code)
        r, struct = self._ioctl_readwrite_struct(HIDIOCGUSAGE, struct)
        rstruct = hiddev_report_info(struct.report_type, struct.report_id)
        r, rstruct = self._ioctl_readwrite_struct(HIDIOCGREPORTINFO, rstruct)
        return (Report(self, rstruct), struct.field_index, struct.usage_index)
