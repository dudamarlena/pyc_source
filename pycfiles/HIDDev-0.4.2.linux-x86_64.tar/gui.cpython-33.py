...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.3/dist-packages/hiddev/gui.py
# Compiled at: 2013-10-27 20:58:33
# Size of source mod 2**32: 16083 bytes
...
class BufferedBytesEditorDialog(QWidget):
    ...
    def loadData(self):
        ...
        text = ''.join((chr(byte) if byte > 25 and byte != 92 else '\\x{0:02X}'.format(byte)) for byte in data)
    ...
    def __init__(self, report, parent):
        ...
        for field in report:
            if 'array' in field.flags:
                text = usages.getUsageName(field.logical_usage, vendor=report.device.vendor_id, model=report.device.model_id) if field.logical_usage else usages.getUsageName(field.usages[0], vendor=report.device.vendor_id, model=report.device.model_id)
