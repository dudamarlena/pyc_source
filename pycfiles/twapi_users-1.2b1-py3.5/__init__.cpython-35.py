...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/twapi_users/__init__.py
# Compiled at: 2017-06-02 03:40:43
...
User = Record.create_type('User', 'id', 'full_name', 'email_address', 'organization_name', 'job_title', 'url')
Group = Record.create_type('Group', 'id')
_USER_DATA_SCHEMA = Schema({'id': int, 
 'full_name': str, 
 'email_address': str, 
 'organization_name': str, 
 'job_title': str, 
 'url': str}, required=True, extra=REMOVE_EXTRA)
_USER_ID_SCHEMA = Schema(int)
_GROUP_DATA_SCHEMA = Schema({'id': int}, required=True, extra=False)
_PAGINATED_RESPONSE_SCHEMA = Schema({'count': int, 
 'next': Any(str, None), 
 'results': list}, required=True, extra=True)
...
def get_users(connection, updates_url=None):
    ...
    """
    users_data, future_updates_url = _get_paginated_data_flattened_with_future_updates_url(connection, updates_url or '/users/')
    return (
     (_make_user(u) for u in users_data), future_updates_url)
...
def get_deleted_users(connection, updates_url=None):
    ...
    users_data, future_updates_url = _get_paginated_data_flattened_with_future_updates_url(connection, updates_url or '/users/deleted/')
    return (
     (_USER_ID_SCHEMA(u) for u in users_data), future_updates_url)
...
def _get_paginated_data_flattened_with_future_updates_url(connection, url):
    ...
    return (
     paginated_data_flattened, future_updates_url)
