...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/twapi_users/testing.py
# Compiled at: 2017-06-01 12:15:51
...
class _PaginatedObjectsRetriever(metaclass=ABCMeta):
    ...
    def _get_api_call_for_page(self, page_objects):
        ...
        api_call = SuccessfulAPICall(self._get_page_url(page_number), 'GET', response=response)
        return api_call
    ...
    def _get_response_body_deserialization(self, page_objects):
        ...
        response_body_deserialization = {'count': self._objects_count, 
         'next': next_page_url, 
...
class _PaginatedObjectsRetrieverWithUpdates(_PaginatedObjectsRetriever):
    ...
    def __init__(self, objects, output_future_updates_url, input_future_updates_url=None):
    ...
    def _get_response_body_deserialization(self, page_objects):
        response_body_deserialization = super(_PaginatedObjectsRetrieverWithUpdates, self)._get_response_body_deserialization(page_objects)
        response_body_deserialization['future_updates'] = self.output_future_updates_url
        return response_body_deserialization
