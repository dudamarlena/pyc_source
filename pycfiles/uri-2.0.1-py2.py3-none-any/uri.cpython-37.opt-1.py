...
class URI(object):
    ...
    uri = URIPart(__parts__)
    safe_uri = URIPart(__safe_parts__, False)
    base = BasePart()
    ...
    def __init__(self, _uri=None, **parts):
        """Initialize a new URI from a passed in string and/or named parts.
                ...
                """
        if hasattr(_uri, '__link__'):
            _uri = _uri.__link__
            if callable(_uri):
                _uri = _uri()
        if hasattr(_uri, 'as_uri'):
            _uri = _uri.as_uri()
        self.uri = _uri
        if parts:
    ...
    def __html__(self):
        ...
        return '<a href="{address}">{summary}</a>'.format(address=(escape(self.uri)),
