...
# [GCC 8.4.0]
# Embedded file name: /Users/tulloch/.virtualenvs/sklearn-dev/lib/python2.7/site-packages/compiledtrees/code_gen.py
# Compiled at: 2014-04-04 11:15:55
...
def code_gen_tree(tree, evaluate_fn=EVALUATE_FN_NAME, gen=None):
    ...
    def recur(node):
        if tree.children_left[node] == -1:
            ...
            return
        branch = ('if (f[{feature}] <= {threshold}) {{').format(feature=tree.feature[node], threshold=tree.threshold[node])
    ...
    with gen.bracketed('extern "C" {', '}'):
        fn_decl = ('{inline} float {name}(float* f) {{').format(inline=ALWAYS_INLINE, name=evaluate_fn)
        with gen.bracketed(fn_decl, '}'):
            recur(0)
    return gen.lines


def code_gen_ensemble(trees, individual_learner_weight, initial_value, gen=None):
    """
    Writes code similar to:
    ...
    with gen.bracketed('extern "C" {', '}'):
        ...
        with gen.bracketed(fn_decl, '}'):
            ...
            for i, _ in enumerate(trees):
                increment = ('result += {name}_{index}(f) * {weight};').format(name=EVALUATE_FN_NAME, index=i, weight=individual_learner_weight)
                gen.write(increment)
