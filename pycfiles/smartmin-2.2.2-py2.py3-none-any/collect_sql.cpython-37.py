...
# [GCC 8.4.0]
# Embedded file name: /Users/nicp/nyaruka/smartmin/smartmin/management/commands/collect_sql.py
# Compiled at: 2020-03-23 16:28:57
...
class SqlObjectOperation(object):
    ...
    @classmethod
    def parse(cls, raw):
        tokens = [t for t in raw.tokens if not t.is_whitespace if not isinstance(t, sql.Comment)]
        if len(tokens) < 3:
            return
        elif not tokens[0].ttype != sql_tokens.DDL:
            if tokens[1].ttype != sql_tokens.Keyword:
                return
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--preserve-order',
          action='store_true', dest='preserve_order', default=False, help='Whether to preserve order of operations rather than sorting by object name.')
        parser.add_argument('--output-dir',
          action='store', dest='output_dir', default='temba/sql', help='The output directory for generated SQL files.')
