...
# [GCC 8.4.0]
# Embedded file name: /home/vagrant/src/nozama-cloudsearch/nozama-cloudsearch-service/nozama/cloudsearch/service/tests/conftest.py
...
"""
import os, sys, logging, StringIO, tempfile, subprocess, ConfigParser
...
class ServerRunner(object):
    ...
    def __init__(self, port=None, configfile=None, interface='localhost'):
        ...
        data = dict(interface=self.interface, port=int(self.port))
    ...
    def start(self):
        ...
        self.serverProcess = subprocess.Popen(args=self.cmd, shell=True, cwd=self.test_dir)
    ...
    def stop(self):
        ...
        if sys.platform.startswith('win'):
            subprocess.call(args='taskkill /F /T /IM pserve.exe', shell=True)
        else:
            subprocess.call(args='ps -a | grep -v grep | grep "pserve*" | awk \'{print "kill -15 "$1}\' | sh', shell=True)
...
def test_server(request):
    ...
    log.debug(("server: setting up REST client API for URI '{0}'.").format(test_server.URI))
    ...
    def teardown():
        ...
        test_server.stop()
        test_server.cleanup()
        log.debug(("teardown: '{0}' stop and cleanup called OK.").format(test_server.URI))
