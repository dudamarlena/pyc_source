...
# [GCC 8.4.0]
# Embedded file name: /home/vagrant/src/nozama-cloudsearch/nozama-cloudsearch-service/nozama/cloudsearch/service/restfulhelpers.py
...
"""
import json, httplib, logging, traceback
...
def status_body(status='ok', message='', error='', traceback='', to_json=False):
    ...
    """
    body = dict(status=status, message=message, error=error, traceback=traceback)
    if to_json:
...
def notfound_404_view(request):
    ...
    body = status_body(status='error', message="URI Not Found '%s'" % msg)
...
def xyz_handler(status):
    ...
    def handler(request):
        ...
        body = status_body(status='error', message=msg)
        rc = Response(body)
        rc.status = status
...
class JSONErrorHandler(object):
    ...
    def __call__(self, environ, start_response):
        try:
            ...
            return status_body(status='error', message=message, error=error, traceback=self.formatError())