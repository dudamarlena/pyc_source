...
# [GCC 8.4.0]
# Embedded file name: /Users/robert.mcmahon@phishme.com/development2/ise-python-libraries/intelligence/phishme_intelligence/core/phishme.py
# Compiled at: 2020-05-04 10:30:57
# Size of source mod 2**32: 57623 bytes
from __future__ import unicode_literals, absolute_import
import argparse, importlib, json, logging, os, random, re, six, sys, time, traceback
...
__all__ = [
 'PhishMeIntelligence', 'read_args', 'read_config']
...
class PhishMeBase(object):
    ...
    def search(self, product, description, action_domain=None, action_url_search=None, all_md5=None, begin_timestamp=None, brand=None, domain=None, drop_mail=None, end_timestamp=None, extracted_string=None, file=None, has_kit=None, ip=None, kit_file=None, kit_md5=None, language=None, malware_artifact_md5=None, malware_domain=None, malware_family=None, malware_file=None, malware_sender_ip=None, malware_sender_name=None, malware_subject=None, malware_watchlist_domain=None, malware_watchlist_ip=None, malware_watchlist_url_search=None, phishing_asn=None, phishing_asn_country_code=None, phishing_asn_organization=None, phishing_title=None, reported_domain=None, reported_url_search=None, threat_domain=None, threat_id=None, threat_ip=None, threat_type='all', threat_url_search=None, url_search=None, web_component_file=None, web_component_md5=None):
        r"""
    ...
    def _process_threat(self, mrti, mrti_format, sync_phase):
        ...
        if (sync_phase == 'backfill' or sync_phase) == 'updates':
            if expired_threat or mrti_format == 'json' and product == 'Intelligence':
    ...
    def _select_mrti_formats(self):
        ...
        :return:
        """
        mrti_types = {'CEF':self.config.getboolean('pm_format', 'cef'), 
    ...
    def _integration_backfill(self):
        ...
        if self.config.getboolean('pm_format', 'cef'):
            ...
            while temp_begin_timestamp < end_timestamp:
                ...
                payload = {'beginTimestamp':temp_begin_timestamp, 
                ...
                for index, cef_message in enumerate(cef_messages.splitlines()):
                    ...
                    if product == 'intelligence':
                        if self.config.getboolean('pm_product', 'intelligence'):
                            yield (
                             cef_message, 'cef')
                    if product == 'brand intelligence' and self.config.getboolean('pm_product', 'brand_intelligence'):
                        yield (
                         cef_message, 'cef')
        ...
        if self.config.getboolean('pm_format', 'json') or self.config.getboolean('pm_format', 'stix'):
            if self.config.getboolean('pm_product', 'intelligence') and self.config.getboolean('pm_product', 'brand_intelligence'):
                ...
                if self.config.getboolean('pm_product', 'intelligence'):
                    ...
                    if self.config.getboolean('pm_product', 'brand_intelligence'):
                        threat_type = 'phish'
                    else:
                        cur_page_number = 0
                        total_pages = 1
                        while cur_page_number < total_pages:
                            payload = {'threatType':threat_type, 
            ...
            for item in list_to_retrieve:
                if self.config.getboolean('pm_format', 'cef'):
                    if process_intelligence:
                        if item.get('threatType') == 'malware':
                    ...
                    elif self.config.getboolean('pm_format', 'stix'):
                        if process_intelligence:
                            if item.get('threatType') == 'malware':
    ...
    def _retrieve_from_threat_updates(self, end_timestamp):
        ...
        return (
         next_position, changelog_size, proper_changelog)
    ...
    def _retrieve_from_threat_search(self, payload, product='pm_api'):
        ...
        if payload.get('page') and payload.get('beginTimestamp') and payload.get('endTimestamp'):
            ...
            if payload.get('beginTimestamp') and payload.get('endTimestamp'):
    ...
    def _setup_phishme_connection(self, connection_type, threat_type=None, threat_id=None):
        ...
        if connection_type is self._PhishMeConnectionType.THREAT_SEARCH:
            ...
            if connection_type is self._PhishMeConnectionType.THREAT_UPDATES:
                url_values = '/threat/updates'
            else:
                if connection_type is self._PhishMeConnectionType.T3_CEF:
                    if threat_type:
                        if threat_id:
            ...
            try:
                with open(lock_file_full_path, 'r+') as (lock_file):
                    try:
                        ...
                        access_number = 0 if (lock_file_value.strip() == '' or int(lock_file_value.strip()) < 0) else (int(lock_file_value.strip()))
                    except ValueError:
                        access_number = 0
                    ...
                    if access_number in range(0, 20):
                        ...
                        self._write_access_number_to_lock_file('1', lock_file)
                        return True
...
class PhishMeIntelligence(PhishMeBase):
    ...
    def search(self, product, description, **kwargs):
        ...
        try:
            ...
            payload = {'resultsPerPage': self.config.get('pm_api', 'results_per_page')}
