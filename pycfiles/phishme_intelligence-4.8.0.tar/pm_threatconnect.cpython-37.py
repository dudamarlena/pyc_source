...
# [GCC 8.4.0]
# Embedded file name: /Users/robert.mcmahon@phishme.com/newdev/cofense_intelligence_tc/lib_3.7.3/phishme_intelligence/output/product/threatconnect/pm_threatconnect.py
# Compiled at: 2020-05-01 15:23:21
# Size of source mod 2**32: 27796 bytes
...
class PmThreatConnect(BaseIntegration):
    ...
    def __init__(self, config, product, additional_config):
        ...
        self.phishme_intel_auth = (
         config.get('pm_api', 'user'), config.get('pm_api', 'pass'))
        ...
        try:
            ...
            if threat_type == 'malware':
                ...
                if threat_type == 'phish':
                    ...
                    if mrti.action_url_list:
                        if len(mrti.action_url_list) > 0:
                            for action_url in mrti.action_url_list:
                                ...
                                self.logger.debug('test: {0}://{1}{2}'.format(action_url.protocol, action_url.host, action_url.path, action_url.query))
                    ...
                    if mrti.reported_url_list:
                        if len(mrti.reported_url_list) > 0:
    ...
    def _process_phish_group(self, intel):
        ...
        self.pm_intel_processor.add_group(group_type=(self.tc_group), group_name=group_name, published_date=(intel.first_published),
        ...
        if intel.brand:
            self.pm_intel_processor.add_group_attribute('Description', 'Credential phish targeting brand{0} {1}'.format('s' if len(intel.brand.split(',')) > 1 else '', intel.brand))
        ...
        self.pm_intel_processor.add_group_attribute('Source', '{0} via Threat ID {1}'.format(self.source, intel.threat_id))
    ...
    def _process_phish_ip_details(self, intel):
        ...
        self.pm_intel_processor.add_indicator_attribute('Additional Analysis and Context', 'ThreatHQ URL: ' + intel.threathq_url)
        self.pm_intel_processor.add_indicator_attribute('Source', '{0} via Threat ID {1}'.format(self.source, intel.threat_id))
    ...
    def _process_phish_url(self, incoming_url, threat_id, threathq_url, url_type):
        ...
        self.pm_intel_processor.add_indicator_attribute('Additional Analysis and Context', 'URL Type: {}'.format(url_type))
        self.pm_intel_processor.add_indicator_attribute('Source', '{0} via Threat ID {1}'.format(self.source, threat_id))
        self.pm_intel_processor.add_indicator_tag(self.source)
    ...
    def _set_standard_indicator_attributes(self, intel):
        ...
        """
        self.pm_intel_processor.add_indicator_attribute('Additional Analysis and Context', 'Active Threat Report URL: ' + intel.active_threat_report)
        self.pm_intel_processor.add_indicator_attribute('Additional Analysis and Context', 'Threat Detail Page URL: ' + intel.threathq_url)
    ...
    def _set_blockset_attributes(self, intel, block_set):
        ...
        """
        if block_set.malware_family_description is not None:
            if block_set.malware_family is not None:
                self.pm_intel_processor.add_indicator_attribute('Description', block_set.malware_family + ': ' + block_set.malware_family_description + ' (Threat ID ' + str(intel.threat_id) + ')')
        self.pm_intel_processor.add_indicator_attribute('Additional Analysis and Context', block_set.role + ': ' + block_set.role_description + ' (Threat ID ' + str(intel.threat_id) + ')')
    ...
    def _set_executableset_attributes(self, executable_set, intel):
        ...
        if not executable_set.malware_family_description:
            ...
            self.pm_intel_processor.add_indicator_attribute('Description', executable_set.malware_family_description + '( Threat ID ' + str(intel.threat_id) + ')')
            if executable_set.subtype is None:
                self.pm_intel_processor.add_indicator_attribute('Additional Analysis and Context', executable_set.type + '( Threat ID ' + str(intel.threat_id) + ')')
            else:
                self.pm_intel_processor.add_indicator_attribute('Additional Analysis and Context', executable_set.type + ': ' + executable_set.subtype + '( Threat ID ' + str(intel.threat_id) + ')')
    ...
    def _process_group(self, intel):
        ...
        if intel.executiveSummary is not None:
            self.pm_intel_processor.add_group_attribute('Description', self.tcex.s((intel.executiveSummary), errors='ignore'))
        self.logger.debug('adding ATR URL')
        self.pm_intel_processor.add_group_attribute('Additional Analysis and Context', 'Active Threat Report URL: ' + intel.active_threat_report)
        self.logger.debug('adding threat detail URL')
        self.pm_intel_processor.add_group_attribute('Additional Analysis and Context', 'Threat Detail Page URL: ' + intel.threathq_url)
    ...
    def _process_document(self, intel):
        ...
        if not intel.json['hasReport']:
            self.logger.info('Threat with Threat ID ' + str(intel.threat_id) + ' does not have Active Threat Report so not creating document...')
            return
        try:
            report = requests.get((intel.active_threat_report_api), auth=(self.phishme_intel_auth), proxies=(self.phishme_intel_proxy))
        except Exception as e:
            try:
                self.logger.error('Unable to download Active Threat Report ' + intel.active_threat_report + '! Skipping creation of Document...')
                return
            finally:
                e = None
                del e

        self.pm_intel_processor.add_document(document_name=document_name, file_name=file_name, active_threat_report=(report.content),
          group_type=(self.tc_group),
          threat_id=(intel.threat_id))
        self.pm_intel_processor.add_document_attribute('Source', self.source + ' via Threat ID ' + str(intel.threat_id))
