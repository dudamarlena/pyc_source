...
# [GCC 8.4.0]
# Embedded file name: /Users/robert.mcmahon@phishme.com/development2/ise-python-libraries/intelligence/phishme_intelligence/core/config_check.py
# Compiled at: 2020-05-04 10:30:57
# Size of source mod 2**32: 56139 bytes
...
class ConfigCheck(object):
    ...
    def validate_config(self):
        ...
        for section in self.config_copy.sections():
            if section not in self.mandatory_sections and self.config_copy.has_option(section, 'check_config') and self.config_copy.getboolean(section, 'check_config'):
                try:
    ...
    def _validate_standard_options(self):
        ...
        for section in self.config_copy.sections():
            ...
            if self.config_copy.has_option(section, option):
                if not self.config_copy.get(section, option).startswith('https://'):
                    if not self.config_copy.get(section, option).startswith('http://'):
    ...
    def _validate_config_section_local_sqlite(self, section):
        ...
        for option in self.config_copy.options(section):
            ...
            if option == 'use':
                continue
            if option == 'db_location':
                self._validate_option_folder_exists(section, option, verify_file=False)
            elif option == 'data_retention_use':
                if self.config_copy.getboolean(section, option):
                    self._validate_option_boolean(section, option)
                else:
                    if option == 'data_retention_days':
    ...
    def _validate_config_section_integration_raw_cef(self, section):
        ...
        for option in self.config_copy.options(section):
            ...
            if option == 'append_file_use':
                ...
                if option == 'multiple_file_location':
                    continue
                if option == 'multiple_file_split_by_date':
                    continue
                self._non_valid_option_found(section, option)
            ...
            if option == 'api_token':
                ...
                if self.config_copy.getboolean(section, option):
                    try:
                        ...
                        try:
                            self._log_warn(message=('%s: Excluded md5s are not formatted correctly. Ensure they are enclosed in [], double-quoted, and comma separated.' % e),
                              section=section,
                              option='excluded_md5')
                        finally:
                            e = None
    ...
    def _validate_option_alphanumeric(option):
        ...
        :rtype: bool
        """
        return not bool(re.search(string=option, pattern='[^a-zA-Z\\d]'))
