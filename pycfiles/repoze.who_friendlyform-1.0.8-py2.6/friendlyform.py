...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/repoze/who/plugins/friendlyform.py
# Compiled at: 2010-11-13 21:52:38
...
class FriendlyFormPlugin(object):
    ...
    * In the login URL, the amount of failed logins is available in the
    ...
    classifications = {IIdentifier: [
                   'browser'], 
       IChallenger: [
                   'browser']}

    def __init__(self, login_form_url, login_handler_path, post_login_url, logout_handler_path, post_logout_url, rememberer_name, login_counter_name=None, charset='iso-8859-1'):
        """
        ...
        if path_info == self.login_handler_path:
            ...
            try:
                ...
                if request.charset == 'us-ascii':
                    credentials = {'login': str(login), 'password': str(password)}
                else:
            ...
            if self.post_login_url:
                ...
                if 'came_from' in query:
                    destination = self._insert_qs_variable(destination, 'came_from', query['came_from'])
            ...
            if path_info == self.logout_handler_path:
                ...
                return
            if path_info == self.login_form_url or self._get_logins(environ):
    ...
    def challenge(self, environ, status, app_headers, forget_headers):
        ...
        if environ['PATH_INFO'] == self.logout_handler_path:
            ...
            if self.post_logout_url:
                ...
                if came_from:
                    destination = self._insert_qs_variable(destination, 'came_from', came_from)
            else:
                script_name = environ.get('SCRIPT_NAME', '')
                destination = came_from or script_name or '/'
        elif 'repoze.who.logins' in environ:
            environ['repoze.who.logins'] += 1
            destination = self._set_logins_in_url(destination, environ['repoze.who.logins'])
