...
# Compiled at: 2020-05-03 13:27:36
...
import vtkplotter.settings as settings, vtkplotter.addons as addons, vtkplotter.shapes as shapes, vtkplotter.utils as utils
            ...
            if settings.plotter_instance and settings.plotter_instance.camera:
                ...
                for ia in actors2show2:
                    ...
                    if isinstance(ia, Volume):
                        ...
                        kobj = k3d.points(ia.points().astype(numpy.float32), color=colors.rgb2int(iap.GetColor()), colors=kcols, opacity=iap.GetOpacity(), shader='3d', point_size=iap.GetPointSize() * sqsize / 800, name=name)
                        settings.notebook_plotter += kobj
                        ...
                        for i, ln_idx in enumerate(ia.lines(joined=True)):
                            ...
                            kobj = k3d.line(pts.astype(numpy.float32), color=colors.rgb2int(iap.GetColor()), opacity=iap.GetOpacity(), shader='thick', width=iap.GetLineWidth() * sqsize / 1000, name=name)
                            settings.notebook_plotter += kobj
