...
# [GCC 8.4.0]
# Embedded file name: /home/musy/Projects/vtkplotter/vtkplotter/base.py
# Compiled at: 2020-05-02 14:07:02
from __future__ import division, print_function
import numpy as np, vtk, vtkplotter.colors as colors, vtkplotter.docs as docs, vtkplotter.settings as settings, vtkplotter.utils as utils
...
class ActorBase(object):
    ...
    def lighting(self, style='', ambient=None, diffuse=None, specular=None, specularPower=None, specularColor=None, enabled=True):
        """
    ...
    def printHistogram(self, bins=10, height=10, logscale=False, minbin=0, horizontal=False, char='▉', c=None, bold=True, title='Histogram'):
        """
        ...
        :Example:
            ...
            |printhisto|
        """
        utils.printHistogram(self, bins, height, logscale, minbin, horizontal, char, c, bold, title)
        return self
    ...
    def setTransform(self, T):
        ...
        if isinstance(T, vtk.vtkMatrix4x4):
            ...
            try:
                ...
                colors.printc('~times Error in setTransform():', 'consider transformPolydata() instead.', c=1)
    ...
    def addPointArray(self, input_array, name):
        ...
        if len(input_array) != data.GetNumberOfPoints():
            colors.printc('~times addPointArray(): Number of elements != nr. of points', len(input_array), data.GetNumberOfPoints(), c=1)
            raise RuntimeError()
        ...
        if len(nparr.shape) == 1:
            ...
            colors.printc('~times addPointArray(): cannot understand shape:', nparr.shape, c=1)
            return self
        if hasattr(self._mapper, 'SetArrayName'):
            self._mapper.SetArrayName(name)
        self._mapper.SetScalarModeToUsePointData()
    ...
    def addCellArray(self, input_array, name):
        ...
        if len(input_array) != data.GetNumberOfCells():
            colors.printc('~times addCellArray(): Number of elements != nr. of Cells', len(input_array), data.GetNumberOfCells(), c=1)
            raise RuntimeError()
        nparr = np.ascontiguousarray(input_array)
        if len(nparr.shape) == 1:
            varr = numpy_to_vtk(nparr, deep=True)
            ...
            colors.printc('~times addCellArray(): cannot understand shape:', nparr.shape, c=1)
            return self
        if hasattr(self._mapper, 'SetArrayName'):
            self._mapper.SetArrayName(name)
        self._mapper.SetScalarModeToUseCellData()
    ...
    def addScalarBar(self, pos=(0.8, 0.05), title='', titleXOffset=0, titleYOffset=15, titleFontSize=12, nlabels=None, c=None, horizontal=False, useAlpha=True):
        """
        ...
        self.scalarbar = addons.addScalarBar(self, pos, title, titleXOffset, titleYOffset, titleFontSize, nlabels, c, horizontal, useAlpha)
        return self

    def addScalarBar3D(self, pos=None, sx=None, sy=None, title='', titleXOffset=-1.5, titleYOffset=0.0, titleSize=1.5, titleRotation=0.0, nlabels=9, labelOffset=0.375, italic=0, c=None, useAlpha=True, drawBox=True):
        """
        ...
        """
        import vtkplotter.addons as addons
        self.scalarbar = addons.addScalarBar3D(self, pos, sx, sy, title, titleXOffset, titleYOffset, titleSize, titleRotation, nlabels, labelOffset, italic, c, useAlpha, drawBox)
