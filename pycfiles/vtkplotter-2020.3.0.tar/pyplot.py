...
# [GCC 8.4.0]
# Embedded file name: /home/musy/Projects/vtkplotter/vtkplotter/pyplot.py
# Compiled at: 2020-05-03 16:52:41
from __future__ import division, print_function
import vtk, numpy as np
from vtk.util.numpy_support import numpy_to_vtk, vtk_to_numpy
import vtkplotter.docs as docs, vtkplotter.settings as settings, vtkplotter.utils as utils, vtkplotter.colors as colors, vtkplotter.shapes as shapes, vtkplotter.addons as addons
...
class Plot(Assembly):
    ...
    def __init__(self, *objs):
        ...
        self.xscale = 1
        self.yscale = 1
        self.aspect = 1.3333333333333333
        self.cut = True
        self.xlim = None
        self.ylim = None
        self.pad = 0.05
...
def _plotxy(data, format=None, aspect=1.3333333333333333, xlim=None, ylim=None, xerrors=None, yerrors=None, title='', xtitle='x', ytitle='y', titleSize=None, c='k', alpha=1, ec=None, lc='k', la=1, lw=3, line=False, dashed=False, spline=False, errorBand=False, marker='', ms=None, mc=None, ma=None, pad=0.05, axes={}):
    ...
    if y0lim == y1lim:
        ...
        if marker:
            ...
            if utils.isSequence(ms):
                ...
                msv[:, 0] = ms
                marked = shapes.Glyph(pts, glyphObj=mk, c=mc, orientationArray=msv, scaleByVectorSize=True)
            else:
                if ms is None:
                    ms = dx / 100.0
    ...
    if title:
        if titleSize is None:
            titleSize = dx / 40.0
        tit = shapes.Text(title, s=titleSize, c=c, depth=0, alpha=alpha, pos=(
         (x0lim + x1lim) / 2, y1lim + dy / 80, 0), justify='bottom-center')
...
def _plotFxy(z, xlim=(0, 3), ylim=(0, 3), zlim=(None, None), showNan=True, zlevels=10, c='b', bc='aqua', alpha=1, texture='paper4', bins=(100, 100), axes=True):
    if isinstance(z, str):
        try:
...
def _plotFz(z, x=(-1, 1), y=(-1, 1), zlimits=(None, None), cmap='PiYG', alpha=1, lw=0.1, bins=(75, 75), axes=True):
    if isinstance(z, str):
        try:
            z = z.replace('np.', '')
            namespace = locals()
...
def _plotPolar(rphi, title='', tsize=0.1, lsize=0.05, r1=0, r2=1, c='blue', bc='k', alpha=1, ps=5, lw=3, deg=False, vmax=None, fill=False, spline=False, smooth=0, showDisc=True, nrays=8, showLines=True, showAngles=True):
    if len(rphi) == 2:
        ...
        for t in np.linspace(0, 2 * np.pi, num=nrays, endpoint=False):
            ...
            if showLines:
                ...
                lm = shapes.DashedLine((0, 0, -0.01), (
                 r2e * ct2, r2e * st2, -0.01), spacing=0.25)
                rays.append(lm)
            elif showAngles:
                l = shapes.Line((
...
def _histogram1D(data, format=None, bins=25, aspect=1.3333333333333333, xlim=None, ylim=(0, None), errors=False, title='', xtitle=' ', ytitle='counts', titleSize=None, titleColor=None, logscale=False, fill=True, c='olivedrab', gap=0.02, alpha=1, outline=False, lw=2, lc='k', marker='', ms=None, mc=None, ma=None, pad=0.05, axes={}, bc='k'):
    ncolls = len(settings.collectable_actors)
    ...
    if title:
        ...
        if titleSize is None:
            titleSize = dx / 40.0
        tit = shapes.Text(title, s=titleSize, c=titleColor, depth=0, alpha=alpha, pos=(
         (x0lim + x1lim) / 2, y1lim + dy / 80, 0), justify='bottom-center')
...
def _histogram2D(xvalues, yvalues=None, format=None, bins=25, aspect=1, xlim=None, ylim=None, weights=None, cmap='cividis', alpha=1, title='', xtitle='x', ytitle='y', titleSize=None, titleColor=None, lw=0, scalarbar=True, axes=True, bc='k'):
    ncolls = len(settings.collectable_actors)
    offs = 0
    if format is not None:
        xlim = format.xlim
    ...
    if isinstance(bins, int):
        bins = (
         bins, bins)
    H, xedges, yedges = np.histogram2d(xvalues, yvalues, weights=weights, bins=bins, range=(xlim, ylim))
    ...
    acts = []
    g = shapes.Grid(pos=[
     (x0lim + x1lim) / 2, (y0lim + y1lim) / 2, 0], sx=dx, sy=dy * yscale, resx=bins[0], resy=bins[1])
    ...
    if title:
        ...
        if titleSize is None:
            titleSize = dx / 40.0
        tit = shapes.Text(title, s=titleSize, c=titleColor, depth=0, alpha=alpha, pos=(
         (x0lim + x1lim) / 2, y1lim + dy / 40, 0), justify='bottom-center')
        tit.pickable(False).z(2.5 * offs)
        acts.append(tit)
    if axes == 1 or axes == True:
...
def _histogramHexBin(xvalues, yvalues, xtitle='', ytitle='', bins=12, vrange=None, norm=1, fill=True, c=None, cmap='terrain_r', alpha=1):
    if xtitle:
        ...
        for j in range(m + 2):
            ...
            if not i % 2:
                ...
                p = (
                 i / ki, j / kj + 0.45, 0)
            q = (
             p[0] / n * 1.2 * dx + xmin, p[1] / m * dy + ymin, 0)
...
def _histogramPolar(values, weights=None, title='', tsize=0.1, bins=16, r1=0.25, r2=1, phigap=0.5, rgap=0.05, lpos=1, lsize=0.04, c='grey', bc='k', alpha=1, cmap=None, deg=False, vmin=None, vmax=None, labels=(), showDisc=True, nrays=8, showLines=True, showAngles=True, showErrors=False):
    k = 180 / np.pi
    if deg:
    ...
    for i, t in enumerate(thetas):
        if i < len(labels):
            ...
            lab.pos(r2e * np.cos(t) * (1 + rgap) * lpos / 2, r2e * np.sin(t) * (1 + rgap) * lpos / 2, 0.01)
            labs.append(lab)
...
def _histogramSpheric(thetavalues, phivalues, rmax=1.2, res=8, cmap='rainbow', lw=0.1, scalarbar=True):
...
def donut(fractions, title='', tsize=0.3, r1=1.7, r2=1, phigap=0, lpos=0.8, lsize=0.15, c=None, bc='k', alpha=1, labels=(), showDisc=False):
    ...
    dn = _histogramPolar(data, title=title, bins=360, r1=r1, r2=r2, phigap=phigap, lpos=lpos, lsize=lsize, tsize=tsize, c=cols, bc=bc, alpha=alpha, vmin=0, vmax=1, labels=labs, showDisc=showDisc, showLines=0, showAngles=0, showErrors=0)
    dn.name = 'donut'
    return dn


def quiver(points, vectors, cmap='jet', alpha=1, shaftLength=0.8, shaftWidth=0.05, headLength=0.25, headWidth=0.2, fill=True, scale=1):
    """
    ...
    spts = points - vectors
    epts = points + vectors
    arrs2d = shapes.Arrows2D(spts, epts, c=cmap, shaftLength=shaftLength, shaftWidth=shaftWidth, headLength=headLength, headWidth=headWidth, fill=fill, scale=scale, alpha=alpha)
    arrs2d.pickable(False)
    arrs2d.name = 'quiver'
    return arrs2d


def violin(values, bins=10, vlim=None, x=0, width=3, spline=True, fill=True, c='violet', alpha=1, outline=True, centerline=True, lc='darkorchid', lw=3):
    """
    Violin histogram.
...
def streamplot(X, Y, U, V, direction='both', maxPropagation=None, mode=1, lw=0.001, c=None, probes=[]):
    """
    ...
    if len(probes) == 0:
        ...
        sv = [
         (n - 1) / (xmax - xmin), (n - 1) / (ymax - ymin), 1]
        probes = probes - [xmin, ymin, 0]
        probes = np.multiply(probes, sv)
        probe = shapes.Points(probes)
    stream = streamLines(vol.imagedata(), probe, tubes={'radius': lw, 'varyRadius': mode}, lw=lw, maxPropagation=maxPropagation, direction=direction)
    if c is not None:
        stream.color(c)
    else:
        stream.addScalarBar()
...
def cornerHistogram(values, bins=20, vrange=None, minbin=0, logscale=False, title='', c='g', bg='k', alpha=1, pos=1, s=0.2, lines=True):
    """
...
class DirectedGraph(Assembly):
    ...
    def __init__(self, **kargs):
        ...
        if isinstance(s, int):
            ss = [
             '2d', 'fast2d', 'clustering2d', 'circular', 'circular3d',
             'cone', 'force', 'tree']
            s = ss[s]
        self.layout = s
        if '2d' in s:
            ...
            if opt is not None:
                ...
                self.strategy.SetGraphBounds(opt)
            opt = kargs.pop('maxNumberOfIterations', None)
            if opt is not None:
                self.strategy.SetMaxNumberOfIterations(opt)
            opt = kargs.pop('threeDimensional', True)
            if opt is not None:
                self.strategy.SetThreeDimensionalLayout(opt)
            opt = kargs.pop('randomInitialPoints', None)
            if opt is not None:
                self.strategy.SetRandomInitialPoints(opt)
        elif 'tree' in s:
    ...
    def build(self):
        ...
        if not diagsz:
            ...
            vertexLabels = dgraph.labels(self._vertexLabels, scale=self.vertexLabelScale, precision=0)
            vertexLabels.color(self._c).pickable(True)
            vertexLabels.name = 'DirectedGraphVertexLabels'
            edgeLabels = dgraph.labels(self._edgeLabels, cells=True, scale=self.edgeLabelScale, precision=0)
