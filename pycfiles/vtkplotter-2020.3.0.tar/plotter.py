...
# [GCC 8.4.0]
# Embedded file name: /home/musy/Projects/vtkplotter/vtkplotter/plotter.py
# Compiled at: 2020-04-30 19:12:57
from __future__ import division, print_function
import time, sys, vtk, numpy as np
from vtkplotter import __version__
import vtkplotter.vtkio as vtkio, vtkplotter.utils as utils, vtkplotter.colors as colors
...
import vtkplotter.docs as docs, vtkplotter.settings as settings, vtkplotter.addons as addons, vtkplotter.backends as backends
    ...
    if settings.plotter_instance and newPlotter is False:
        ...
        if utils.isSequence(at):
            ...
            if not utils.isSequence(actors):
                ...
                raise RuntimeError()
            at = range(len(actors))
        vp = Plotter(shape=shape, N=N, pos=pos, size=size, screensize=screensize, title=title, bg=bg, bg2=bg2, axes=axes, sharecam=sharecam, verbose=verbose, interactive=interactive, offscreen=offscreen)
    if utils.isSequence(at):
        for i, a in enumerate(actors):
            _vp_to_return = vp.show(a, at=i, zoom=zoom, resetcam=resetcam, viewup=viewup, azimuth=azimuth, elevation=elevation, roll=roll, camera=camera, interactive=interactive, interactorStyle=interactorStyle, q=q)

        vp.interactor.Start()
    else:
        _vp_to_return = vp.show(actors, at=at, zoom=zoom, resetcam=resetcam, viewup=viewup, azimuth=azimuth, elevation=elevation, roll=roll, camera=camera, interactive=interactive, interactorStyle=interactorStyle, q=q)
...
class Plotter():
 ...
 1, 1), N=None, pos=(
 0, 0), size='auto', screensize='auto', title='', bg='white', bg2=None, axes=settings.defaultAxesType, sharecam=True, verbose=False, interactive=None, offscreen=False, qtWidget=None):
        ...
        self._legend = []
        self.legendSize = 0.15
        self.legendBC = (0.96, 0.96, 0.9)
        self.legendPos = 2
        self.picked3d = None
        self.backgrcol = bg
        ...
        self.xtitle = settings.xtitle
        self.ytitle = settings.ytitle
        self.ztitle = settings.ztitle
        ...
        if screensize == 'auto':
            ...
            if aus and len(aus) == 2 and aus[0] > 100 and aus[1] > 100:
                if aus[0] / aus[1] > 2:
                    screensize = (
                     int(aus[0] / 2), aus[1])
                else:
    ...
    def addLight(self, pos, focalPoint=(
 0, 0, 0), deg=180, c='white', intensity=0.4, removeOthers=False, showsource=False):
        ...
        .. hint:: |lights.py|_
        """
        return addons.addLight(pos, focalPoint, deg, c, intensity, removeOthers, showsource)

    def addSlider2D(self, sliderfunc, xmin, xmax, value=None, pos=4, title='', font='arial', titleSize=1, c=None, showValue=True):
        ...
        """
        return addons.addSlider2D(sliderfunc, xmin, xmax, value, pos, title, font, titleSize, c, showValue)

    def addSlider3D(self, sliderfunc, pos1, pos2, xmin, xmax, value=None, s=0.03, t=1, title='', rotation=0, c=None, showValue=True):
        ...
        """
        return addons.addSlider3D(sliderfunc, pos1, pos2, xmin, xmax, value, s, t, title, rotation, c, showValue)

    def addButton(self, fnc, states=(
 'On', 'Off'), c=(
 'w', 'w'), bc=(
 'dg', 'dr'), pos=(
 20, 40), size=24, font='arial', bold=False, italic=False, alpha=1, angle=0):
        ...
        if settings.notebookBackend and settings.notebookBackend != 'panel' and settings.notebookBackend != '2d':
            ...
            for ia in actors2show:
                if ia:
                    ...
                    if hasattr(ia, 'GetTextProperty'):
                        ...
                        if np.linalg.norm(cacol - (0.5, 0.5, 0.5)) / 3 < 0.05:
                            c = (0.9, 0.9, 0.9)
                            if np.sum(self.renderer.GetBackground()) > 1.5:
                                c = (0.1, 0.1, 0.1)
            ...
            if camera is not None:
                ...
                if cm_pos is not None:
                    self.camera.SetPosition(cm_pos)
                if cm_focalPoint is not None:
                    self.camera.SetFocalPoint(cm_focalPoint)
                if cm_viewup is not None:
                    self.camera.SetViewUp(cm_viewup)
                if cm_distance is not None:
                    self.camera.SetDistance(cm_distance)
                if cm_clippingRange is not None:
                    self.camera.SetClippingRange(cm_clippingRange)
                if cm_parallelScale is not None:
                    self.camera.SetParallelScale(cm_parallelScale)
                if cm_thickness is not None:
                    self.camera.SetThickness(cm_thickness)
                if cm_viewAngle is not None:
                    self.camera.SetViewAngle(cm_viewAngle)
            if resetcam:
    ...
    def showInset(self, *actors, **options):
        ...
        if not self.renderer:
            colors.printc('Use showInset() after first rendering the scene.', c=3)
    ...
    def _keypress(self, iren, event):
        ...
        if key in ('q', 'Q', 'space', 'Return'):
                        ...
                        if key == 'comma':
                            ...
                            if key == 'period':
                                ...
                                if self.keyPressFunction:
                                    if key not in ('Shift_L', 'Control_L', 'Super_L',
                                                   'Alt_L'):
                                        if key not in ('Shift_R', 'Control_R', 'Super_R',
                                                       'Alt_R'):
                                            self.verbose = False
            ...
            if key == 's':
                ...
                for ia in self.getMeshes():
                    ...
                    if isinstance(ia, Mesh):
                        ...
                        if len(arnames):
                            ...
                            if arnam and 'normals' not in arnam.lower():
                ...
                asso = {'KP_Insert': 0, 'KP_0': 0, 'KP_End': 1, 
                   'KP_1': 1, 'KP_Down': 2, 
                   'KP_2': 2, 'KP_Next': 3, 
                   'KP_3': 3, 'KP_Left': 4, 
                   'KP_4': 4, 'KP_Begin': 5, 
                ...
                if key in asso.keys():
                    if self.axes_instances[clickedr]:
                        if hasattr(self.axes_instances[clickedr], 'EnabledOff'):
            ...
            if key == 'O':
                ...
                if self.clickedActor in self.getMeshes():
                    ...
                    acts = self.getMeshes()
                shds = ('default', 'metallic', 'plastic', 'shiny', 'glossy')
                for ia in acts:
                    if ia.GetPickable():
                        try:
                ...
                for ia in acts:
                    if ia.GetPickable():
                        ...
                        colors.printc('->  shading set to:', ia.GetProperty().GetInterpolationAsString(), c='g', bold=0)
                ...
                if self.justremoved is None:
                    if self.clickedActor in self.getMeshes() or isinstance(self.clickedActor, vtk.vtkAssembly):
                    ...
                    if hasattr(self.clickedActor, '_legend') and self.clickedActor._legend:
                        print('...removing actor: ' + str(self.clickedActor._legend) + ', press x to put it back')
                    else:
                ...
                if self.clickedActor:
                    ...
                    colors.printc('Click object and press X to open the cutter box widget.', c=4)
            elif key == 'E':
