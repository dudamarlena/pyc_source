...
# Compiled at: 2020-05-02 15:09:47
            ...
            if style == 1 or style == 'matplotlib':
                ...
                if mesh and ('mesh' in mode or 'color' in mode or 'displace' in mode):
                    ...
                    if cmap and actor.u_values is not None and c is None:
                        if u.value_rank() > 0:
                            actor.pointColors(utils.mag(actor.u_values), cmap=cmap, vmin=vmin, vmax=vmax)
                        else:
                            actor.pointColors(actor.u_values, cmap=cmap, vmin=vmin, vmax=vmax)
                    if 'displace' in mode:
                        actor.move(u)
