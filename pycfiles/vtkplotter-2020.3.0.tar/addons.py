...
# [GCC 8.4.0]
# Embedded file name: /home/musy/Projects/vtkplotter/vtkplotter/addons.py
# Compiled at: 2020-05-03 08:10:22
"""
...
import vtkplotter.shapes as shapes, vtkplotter.settings as settings, vtkplotter.docs as docs
...
def addLight(pos, focalPoint=(
 0, 0, 0), deg=180, c='white', intensity=0.4, removeOthers=False, showsource=False):
    """
...
def addScalarBar(obj, pos=(
 0.8, 0.05), title='', titleXOffset=0, titleYOffset=15, titleFontSize=12, nlabels=None, c=None, horizontal=False, useAlpha=True):
...
def addScalarBar3D(obj, pos=None, sx=None, sy=None, title='', titleXOffset=-1.5, titleYOffset=0.0, titleSize=1.5, titleRotation=0.0, nlabels=9, labelOffset=0.375, italic=0, c=None, useAlpha=True, drawBox=True):
    ...
    if isinstance(obj, Mesh):
        ...
        if isinstance(obj, Volume):
            ...
            return obj
        scale = shapes.Grid([-sx * labelOffset, 0, 0], c=c, alpha=1, sx=sx, sy=sy, resx=1, resy=256)
        ...
        for i, p in enumerate(ticks_pos):
            ...
            if i and tx:
                ...
                a = shapes.Text(tx, pos=[sx * labelOffset, y, 0], s=sy / 50, justify='center-left', c=c, italic=italic)
                tacts.append(a)
    ...
    if title:
        t = shapes.Text(title, (0, 0, 0), s=sy / 50 * titleSize, c=c, justify='centered', italic=italic)
...
def addSlider2D(sliderfunc, xmin, xmax, value=None, pos=4, title='', font='arial', titleSize=1, c=None, showValue=True):
...
def addSlider3D(sliderfunc, pos1, pos2, xmin, xmax, value=None, s=0.03, t=1, title='', rotation=0, c=None, showValue=True):
...
def addButton(fnc, states=(
 'On', 'Off'), c=(
 'w', 'w'), bc=(
 'dg', 'dr'), pos=(
 20, 40), size=24, font='arial', bold=False, italic=False, alpha=1, angle=0):
...
def addCutterTool(mesh):
    ...
    if isinstance(mesh, vtk.vtkImageData):
    ...
    act0 = Mesh(cpoly, alpha=mesh.alpha())
...
def buildAxes(obj=None, xtitle=None, ytitle=None, ztitle=None, xrange=None, yrange=None, zrange=None, c=None, numberOfDivisions=None, limitRatio=0.04, axesLineWidth=1, gridLineWidth=1, reorientShortTitle=True, titleDepth=0, xTitlePosition=0.95, yTitlePosition=0.95, zTitlePosition=0.95, xTitleOffset=0.06, yTitleOffset=0.06, zTitleOffset=0.06, xTitleJustify='top-right', yTitleJustify='bottom-right', zTitleJustify='bottom-right', xTitleRotation=0, yTitleRotation=90, zTitleRotation=135, xTitleSize=0.025, yTitleSize=0.025, zTitleSize=0.025, xTitleColor=None, yTitleColor=None, zTitleColor=None, xTitleBackfaceColor=None, yTitleBackfaceColor=None, zTitleBackfaceColor=None, xKeepAspectRatio=True, yKeepAspectRatio=True, zKeepAspectRatio=True, xyGrid=True, yzGrid=True, zxGrid=False, xyGrid2=False, yzGrid2=False, zxGrid2=False, xyGridTransparent=False, yzGridTransparent=False, zxGridTransparent=False, xyGrid2Transparent=False, yzGrid2Transparent=False, zxGrid2Transparent=False, xyPlaneColor=None, yzPlaneColor=None, zxPlaneColor=None, xyGridColor=None, yzGridColor=None, zxGridColor=None, xyAlpha=0.05, yzAlpha=0.05, zxAlpha=0.05, xyFrameLine=None, yzFrameLine=None, zxFrameLine=None, xLineColor=None, yLineColor=None, zLineColor=None, xOriginMarkerSize=0, yOriginMarkerSize=0, zOriginMarkerSize=0, xHighlightZero=False, yHighlightZero=False, zHighlightZero=False, xHighlightZeroColor=(
 1, 0, 0), yHighlightZeroColor=(0, 1, 0), zHighlightZeroColor=(0, 0, 1), showTicks=True, xTickLength=0.015, yTickLength=0.015, zTickLength=0.015, xTickThickness=0.0025, yTickThickness=0.0025, zTickThickness=0.0025, xTickColor=None, yTickColor=None, zTickColor=None, xMinorTicks=1, yMinorTicks=1, zMinorTicks=1, tipSize=None, xLabelSize=0.0175, yLabelSize=0.0175, zLabelSize=0.0175, xLabelOffset=0.015, yLabelOffset=0.015, zLabelOffset=0.01, xPositionsAndLabels=None, yPositionsAndLabels=None, zPositionsAndLabels=None, useGlobal=False, tol=0.0001):
    ...
    if not ssmax:
        ...
        if not zLineColor:
            zLineColor = c
        if not xTickColor:
            xTickColor = xLineColor
        if not yTickColor:
            yTickColor = yLineColor
        if not zTickColor:
            zTickColor = zLineColor
        if settings.useDepthPeeling:
            ...
            zxGrid2 = False
        ndiv = 4
        if not ztitle or not ytitle or not xtitle:
            ndiv = 6
            if not ztitle:
        ...
        if xtitle:
            xticks_float, xticks_str = make_ticks(vbb[0], vbb[1], rx, xPositionsAndLabels)
        if ytitle:
            yticks_float, yticks_str = make_ticks(vbb[2], vbb[3], ry, yPositionsAndLabels)
        if ztitle:
            zticks_float, zticks_str = make_ticks(vbb[4], vbb[5], rz, zPositionsAndLabels)
        x_aspect_ratio_scale = 1
        y_aspect_ratio_scale = 1
        z_aspect_ratio_scale = 1
        if xtitle:
            if ss[1] and ss[0] > ss[1]:
                x_aspect_ratio_scale = (
                 1, ss[0] / ss[1], 1)
            elif ss[0]:
                x_aspect_ratio_scale = (
                 ss[1] / ss[0], 1, 1)
        ...
        titles = []
        if xtitle:
            xt = shapes.Text(xtitle, pos=(0, 0, 0), s=xTitleSize, c=xTitleColor, justify=xTitleJustify, depth=titleDepth)
            if xTitleBackfaceColor:
                xt.backColor(xTitleBackfaceColor)
            if reorientShortTitle and len(ytitle) < 3:
                wpos = [
                 xTitlePosition, -xTitleOffset + 0.02, 0]
            else:
                wpos = [
                 xTitlePosition, -xTitleOffset, 0]
            if xKeepAspectRatio:
            ...
            titles.append(xt)
        if ytitle:
            yt = shapes.Text(ytitle, pos=(0, 0, 0), s=yTitleSize, c=yTitleColor, justify=yTitleJustify, depth=titleDepth)
            if yTitleBackfaceColor:
                yt.backColor(yTitleBackfaceColor)
            if reorientShortTitle and len(ytitle) < 3:
                wpos = [
                 -yTitleOffset + 0.03 - 0.01 * len(ytitle), yTitlePosition, 0]
                if yKeepAspectRatio:
                    yt.SetScale(x_aspect_ratio_scale)
            else:
                wpos = [
                 -yTitleOffset, yTitlePosition, 0]
                if yKeepAspectRatio:
            ...
            titles.append(yt)
        if ztitle:
            zt = shapes.Text(ztitle, pos=(0, 0, 0), s=zTitleSize, c=zTitleColor, justify=zTitleJustify, depth=titleDepth)
            if zTitleBackfaceColor:
                zt.backColor(zTitleBackfaceColor)
            if reorientShortTitle and len(ztitle) < 3:
                wpos = [
                ...
                if zKeepAspectRatio and isSequence(z_aspect_ratio_scale):
                    zr2 = (
                     z_aspect_ratio_scale[1], z_aspect_ratio_scale[0], z_aspect_ratio_scale[2])
                    zt.SetScale(zr2)
                zt.RotateX(90)
                zt.RotateY(45)
                zt.pos(wpos)
                ...
                wpos = [
                 -zTitleOffset / 1.42, -zTitleOffset / 1.42, zTitlePosition]
                zt.RotateY(-90)
        ...
        if xOriginMarkerSize:
            if xtitle:
                if min_bns[0] <= 0 and max_bns[1] > 0:
        ...
        if yOriginMarkerSize:
            if ytitle:
                if min_bns[2] <= 0 and max_bns[3] > 0:
        ...
        if zOriginMarkerSize:
            if ztitle:
                if min_bns[4] <= 0 and max_bns[5] > 0:
        ...
        if tipSize:
            if xtitle:
                cx = shapes.Cone((1, 0, 0), r=tipSize, height=tipSize * 2, axis=(1,
                                                                                 0,
                                                                                 0), c=xLineColor, res=10)
                cx.name = 'xTipCone'
                cones.append(cx)
            if ytitle:
                cy = shapes.Cone((0, 1, 0), r=tipSize, height=tipSize * 2, axis=(0,
                                                                                 1,
                                                                                 0), c=yLineColor, res=10)
                cy.name = 'yTipCone'
                cones.append(cy)
            if ztitle:
                cz = shapes.Cone((0, 0, 1), r=tipSize, height=tipSize * 2, axis=(0,
                                                                                 0,
                                                                                 1), c=zLineColor, res=10)
                cz.name = 'zTipCone'
                cones.append(cz)
        ...
        if showTicks:
            if xtitle:
                for i in range(1, len(xticks_float) - 1):
                    v1 = (
                     xticks_float[i] - xTickThickness / 2, -xTickLength / 2, 0)
            ...
            if ytitle:
                for i in range(1, len(yticks_float) - 1):
                    v1 = (
                     -yTickLength / 2, yticks_float[i] - yTickThickness / 2, 0)
            ...
            if ztitle:
                for i in range(1, len(zticks_float) - 1):
                    v1 = (
                     zticks_float[i] - zTickThickness / 2, -zTickLength / 2.84, 0)
            ...
            if zMinorTicks and ztitle and len(zticks) > 1:
                ...
                for i in range(1, len(zticks)):
                    ...
                    for j in range(1, zMinorTicks):
                        ...
                        v1 = (mt[0] - zTickThickness / 4, -zTickLength / 5.0, 0)
                        v2 = (mt[0] + zTickThickness / 4, zTickLength / 5.0, 0)
                        ticks.append(shapes.Rectangle(v1, v2))
        ...
        if zLabelSize and ztitle:
            for i in range(1, len(zticks_str)):
                ...
                if not t:
                    continue
                v = (
                 -zLabelOffset, -zLabelOffset, zticks_float[i])
...
def addGlobalAxes(axtype=None, c=None):
    ...
    if not vp.axes:
        ...
        if vp.axes == 1 or vp.axes is True or isinstance(vp.axes, dict):
            ...
            dx, dy, dz = x1 - x0, y1 - y0, z1 - z0
            acts = []
            if x0 * x1 <= 0 or y0 * z1 <= 0 or z0 * z1 <= 0:
            ...
            if len(vp.xtitle) and dx > aves / 100:
                ...
                xc = shapes.Cone(pos=[x1, 0, 0], c=xcol, alpha=alpha, r=aves / 100 * s, height=aves / 25 * s, axis=[1, 0, 0], res=10)
                wpos = [x1 - (len(vp.xtitle) + 1) * aves / 40 * s, -aves / 25 * s, 0]
                if centered:
                    wpos = [
                     (x0 + x1) / 2 - len(vp.xtitle) / 2 * aves / 40 * s, -aves / 25 * s, 0]
            ...
            if len(vp.ytitle) and dy > aves / 100:
                ...
                yc = shapes.Cone(pos=[0, y1, 0], c=ycol, alpha=alpha, r=aves / 100 * s, height=aves / 25 * s, axis=[0, 1, 0], res=10)
                wpos = [-aves / 40 * s, y1 - (len(vp.ytitle) + 1) * aves / 40 * s, 0]
                if centered:
                    wpos = [
                     -aves / 40 * s, (y0 + y1) / 2 - len(vp.ytitle) / 2 * aves / 40 * s, 0]
            ...
            if len(vp.ztitle) and dz > aves / 100:
                ...
                zc = shapes.Cone(pos=[0, 0, z1], c=zcol, alpha=alpha, r=aves / 100 * s, height=aves / 25 * s, axis=[0, 0, 1], res=10)
                wpos = [-aves / 50 * s, -aves / 50 * s, z1 - (len(vp.ztitle) + 1) * aves / 40 * s]
                if centered:
                    wpos = [
                     -aves / 50 * s, -aves / 50 * s, (z0 + z1) / 2 - len(vp.ztitle) / 2 * aves / 40 * s]
            ...
            gr = shapes.Grid((xpos, ypos, vbb[4]), sx=ss[0] * 8, sy=ss[1] * 8, resx=7, resy=7, c=c, alpha=0.2)
            ...
            polaxes.PolarLabelVisibilityOff()
...
def addLegend():
    ...
    if pos == 1:
        ...
        vtklegend.GetPositionCoordinate().SetValue(sx, sy)
    elif pos == 3:
