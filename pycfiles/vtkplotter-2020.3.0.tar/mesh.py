...
# Compiled at: 2020-05-03 11:25:12
...
class Mesh(vtk.vtkFollower, ActorBase):
    ...
    def pointColors(self, input_array=None, cmap='jet', alpha=1, vmin=None, vmax=None, arrayName='PointScalars'):
        """
    ...
    def cellColors(self, input_array=None, cmap='jet', alpha=1, vmin=None, vmax=None, arrayName='CellScalars'):
        """
        ...
        if input_array is None:
            ...
            if isinstance(input_array, str):
                ...
                if n != poly.GetNumberOfCells():
                    print('In cellColors(): nr. of scalars != nr. of Cells', n, poly.GetNumberOfCells(), '...skip coloring.')
                    return self
                input_array = np.ascontiguousarray(input_array)
                arr = numpy_to_vtk(input_array, deep=True)
                arr.SetName(arrayName)
    ...
    def cellIndividualColors(self, colorlist, alpha=1, alphaPerCell=False):
        ...
        if len(colorlist) != n or utils.isSequence(alpha) and len(alpha) != n:
            colors.printc('Error in cellIndividualColors(): mismatch in input list sizes.', len(colorlist), n, c=1)
            return self
        lut = vtk.vtkLookupTable()
        if alphaPerCell:
            ...
            ucolors, uids, inds = np.unique(colorlist, axis=0, return_index=True, return_inverse=True)
            nc = len(ucolors)
            if nc == 1:
    ...
    def labels(self, content=None, cells=False, scale=None, rotX=0, rotY=0, rotZ=0, ratio=1, precision=3):
        ...
        if arr is None and mode == 0:
            ...
            tapp.Update()
            ids = Mesh(tapp.GetOutput(), c=[0.5, 0.5, 0.5]).pickable(False)
