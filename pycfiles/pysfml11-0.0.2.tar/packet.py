...
# Embedded file name: build\bdist.win-amd64\egg\pysfm\core\packet.py
# Compiled at: 2018-02-22 22:09:05
__author__ = 'Brian Lee'
from HexByteConversion import HexToByte
UF_PACKET_START_CODE = 64
UF_NETWORK_PACKET_START_CODE = 65
UF_PACKET_END_CODE = 10
UF_PACKET_LEN = 13
...
UF_DEFAULT_DATA_PACKET_SIZE = 4096
...
def make_packet(command, param=0, size=0, flag=0):
    ...
    packet[UF_PACKET_PARAM_POS] = param & 255
    packet[UF_PACKET_PARAM_POS + 1] = param >> 8 & 255
    packet[UF_PACKET_PARAM_POS + 2] = param >> 16 & 255
    packet[UF_PACKET_PARAM_POS + 3] = param >> 24 & 255
    packet[UF_PACKET_SIZE_POS] = size & 255
    packet[UF_PACKET_SIZE_POS + 1] = size >> 8 & 255
    packet[UF_PACKET_SIZE_POS + 2] = size >> 16 & 255
    packet[UF_PACKET_SIZE_POS + 3] = size >> 24 & 255
...
def make_network_packet(command, terminal_id, param=0, size=0, flag=0):
    ...
    packet[UF_NETWORK_PACKET_TERMINALID_POS] = terminal_id & 255
    packet[UF_NETWORK_PACKET_TERMINALID_POS + 1] = terminal_id >> 8 & 255
    packet[UF_NETWORK_PACKET_COMMAND_POS] = command
    packet[UF_NETWORK_PACKET_PARAM_POS] = param & 255
    packet[UF_NETWORK_PACKET_PARAM_POS + 1] = param >> 8 & 255
    packet[UF_NETWORK_PACKET_PARAM_POS + 2] = param >> 16 & 255
    packet[UF_NETWORK_PACKET_PARAM_POS + 3] = param >> 24 & 255
    packet[UF_NETWORK_PACKET_SIZE_POS] = size & 255
    packet[UF_NETWORK_PACKET_SIZE_POS + 1] = size >> 8 & 255
    packet[UF_NETWORK_PACKET_SIZE_POS + 2] = size >> 16 & 255
    packet[UF_NETWORK_PACKET_SIZE_POS + 3] = size >> 24 & 255
...
def _parse_packet(recvData):
    ...
    endcode = recvData[12]
    return [
     startcode, command, param, size, flag, checksum, endcode]
...
if __name__ == '__main__':
    print make_packet(65, 305419896, 2271560481, 1)
    print make_network_packet(65, 1, 305419896, 2271560481, 1)
    a = [
     64, 3, 137, 0, 0, 0, 0, 3, 3, 22, 97, 73, 10]
