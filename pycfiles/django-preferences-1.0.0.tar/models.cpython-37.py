...
# [GCC 8.4.0]
# Embedded file name: /home/silly/dev/django-preferences/preferences/models.py
# Compiled at: 2018-12-20 03:01:48
...
def site_cleanup(sender, action, instance, **kwargs):
    ...
    if action == 'post_add':
        if isinstance(instance, Preferences):
            if hasattr(instance.__class__, 'objects'):
                site_conflicts = instance.__class__.objects.filter(sites__in=(instance.sites.all())).only('id').distinct()
