...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/Yass/kolibri/utils.py
# Compiled at: 2019-04-25 16:48:27
import os, re, yaml, mimetypes
from scss import Compiler
RE_SFC_TEMPLATE = re.compile('<template\\s*>\\n?([\\S\\s]*?)<\\/template\\s*>', re.IGNORECASE)
RE_SFC_SCRIPT = re.compile('<script\\s*(.*)\\s*>\\n?([\\S\\s]*?)<\\/script\\s*>', re.IGNORECASE)
RE_SFC_STYLE = re.compile('<style\\s*(.*)\\s*>\\n?([\\S\\s]*?)<\\/style\\s*>', re.IGNORECASE)
...
def destruct_sfc(content):
    ...
    if s_template:
        ...
        return (
         False, {'template': content, 'script': None, 'style': None, 'script_props': '', 'style_props': ''})
        return


RE_MACROS = re.compile('{%\\s*macro\\s+([\\S\\s]*?)\\s*\\(', re.IGNORECASE)
RE_MACROS_DOC = re.compile('{#\\s*([\\S\\s]*?)#}', re.IGNORECASE)
...
def get_macros_definition(tpl):
    ...
    :return dict:
    """
    macros = {m:'' for m in re.findall(RE_MACROS, tpl) if not m.startswith('_') if not m.startswith('_')}
    if macros:
...
def extract_sitename(s):
    return re.sub('https?://(www\\.)?', '', s).replace('www.', '')
