...
# Compiled at: 2019-04-11 21:47:55
"""
~ Mambo ~
"""
import os, re, sys, copy, json, time, uuid, yaml, arrow, shutil, jinja2, logging, frontmatter, pkg_resources, webassets.loaders
...
class Mambo(object):
    RE_BLOCK_BODY = re.compile('{%\\s*block\\s+__MAIN_CONTENT__\\s*%}')
    RE_BLOCK_BODY_PARSED = re.compile('{%\\s*block\\s+__MAIN_CONTENT__\\s*%}(.*?){%\\s*endblock\\s*%}')
    ...
    def __init__(self, root_dir, config=None):
        ...
        self.build_static_page_assets_dir = os.path.join(self.build_static_dir, 'page_assets')
        self.mambo_macros = {}
        config_file = os.path.join(self.root_dir, 'mambo.yml')
        ...
        self.site_config.setdefault('layout', DEFAULT_LAYOUT)
        self.layout = self.site_config.get('layout')
        ...
        self._init_jinja({'site': self.site_config, 
           'data': self._data, 
           '_env': self._mambo_vars()})

    def _mambo_vars(self):
        """ Global variables """
        utc = arrow.utcnow()
    ...
    def _init_jinja(self, global_context={}):
        macros_file = pkg_resources.resource_string(__name__, 'extras/macros.html')
        self.mambo_macros = utils.get_macros_definition(macros_file)
        loader = jinja2.ChoiceLoader([
         jinja2.DictLoader({'mambo.macros': macros_file}),
         jinja2.FileSystemLoader(self.templates_dir)])
        env_extensions = [
         'mambo.extras.md.MarkdownExtension',
         'mambo.extras.md.MarkdownTagExtension']
        if self.config.get('build.compress_html') is True:
            env_extensions.append('mambo.extras.htmlcompress.HTMLCompress')
        self.tpl_env = jinja2.Environment(loader=loader, extensions=env_extensions)
        self.tpl_env.globals.update(global_context)
        self.tpl_env.filters.update({'mambo_format_datetime': lambda dt, format: arrow.get(dt).format(format), 
           'mambo_create_page_link': self._create_page_link, 
           'mambo_get_page_url': self._get_page_url, 
           'mambo_get_page_info': self._get_page_info, 
           'mambo_get_static_url': self._get_static_url})
            ...
            if is_sfc is True:
                ...
                for o in sfc_o:
                    if sfc_c.get(o):
                        ...
                        _sff = _ff.replace(self.build_dir, '')
                        _context['page']['sfc_%s_path' % sfc_o[o]] = _sff
                        with open(_ff, 'w') as (f):
                            f.write(sfc_c.get(o))
                        if o == 'script':
                            content += '{% block __PAGE_COMPONENT_SCRIPT__ %}'
                            content += '{{ Mambo.script_tag("%s", module=True) }}' % _sff
                        elif o == 'style':
                            content += '{% block __PAGE_COMPONENT_STYLE__ %}'
                            content += '{{ Mambo.stylesheet_tag("%s") }}' % _sff
                        content += '{% endblock %}'
