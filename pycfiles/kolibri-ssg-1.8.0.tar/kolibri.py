...
# Compiled at: 2019-05-15 00:20:53
"""
~ Kolibri ~
"""
import os, re, sys, copy, json, time, uuid, yaml, arrow, shutil, jinja2, logging, datetime, frontmatter, pkg_resources, webassets.loaders
...
class Kolibri(object):
    ...
    def __init__(self, root_dir, options={}):
        ...
        self._init_jinja({'page_url': self._get_page_url, 
           ...
           'data': {}, '__info': self._kolibri_vars()})
        return
    ...
    def _init_jinja(self, global_context={}):
        ...
        self.tpl_env.filters.update({'kolibri_format_datetime': lambda dt, format: arrow.get(dt).format(format), 
           'kolibri_create_page_link': self._create_page_link, 
           'kolibri_get_page_url': self._get_page_url, 
           'kolibri_get_page_info': self._get_page_info, 
           'kolibri_get_static_url': self._get_static_url})
    ...
    def _create_page_link(self, page, text=None, title=None, _class='', id='', alt='', **kwargs):
        ...
        return ('<a href=\'{url}\' class=\'{_class}\' id=\'{id}\'  title="{title}">{text}</a>').format(url=self._make_url(meta.get('url', '/')) + anchor, text=text or meta.get('title') or title, title=title or '', _class=_class, id=id)
    ...
    def _reset_page_context_assets(self, context):
        context['page']['__assets'] = {'scripts': [], 'stylesheets': []}
                ...
                for o in sfc_o:
                    if sfc_c.get(o):
                        ...
                        with open(_ff, 'w') as (f):
                            ...
                            f.write(o_content)
                        if o == 'script':
                            _context['page']['__assets']['scripts'].append({'url': _sff, 
                               'attributes': sfc_c['script_props']})
                        elif o == 'style':
                            _context['page']['__assets']['stylesheets'].append({'url': _sff, 
                               'attributes': None})
