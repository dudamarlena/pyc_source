...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/zensols/pybuild/tag.py
...
class TagUtil(object):
    ...
    def get_entries(self):
        ...
        for tag in tags:
            ...
            if ver is not None:
                tag_entries.append({'name':name,  'ver':ver, 
                 'date':date, 
                 'tag':tag, 
    ...
    def get_info(self):
        ...
        if last_entry:
            ...
            if hasattr(tag.object, 'message'):
                message = tag.object.message
            inf.update({'tag':last_entry['ver'].format(prefix=''),  'name':last_entry['name'], 
             'message':message})
    ...
    def create(self):
        ...
        logger.info('creating {} with commit <{}>'.format(new_tag_name, self.message))
        if not self.dry_run:
