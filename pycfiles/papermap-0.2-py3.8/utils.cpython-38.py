...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\papermap\utils.py
# Compiled at: 2019-12-17 15:03:14
# Size of source mod 2**32: 18088 bytes
...
def wgs84_to_utm(lat: float, lon: float):
    ...
    M = R * ((1 - E / 4 - 3 * E ** 2 / 64 - 5 * E ** 3 / 256) * lat_rad - (3 * E / 8 + 3 * E ** 2 / 32 + 45 * E ** 3 / 1024) * sin(2 * lat_rad) + (15 * E ** 2 / 256 + 45 * E ** 3 / 1024) * sin(4 * lat_rad) - 35 * E ** 3 / 3072 * sin(6 * lat_rad))
    x = K0 * N * (A + (1 - T + C) * A ** 3 / 6 + (5 - 18 * T + T ** 2 + 72 * C - 58 * E_) * A ** 5 / 120) + 500000
    y = K0 * (M + N * lat_tan * (A ** 2 / 2 + (5 - T + 9 * C + 4 * C ** 2) * A ** 4 / 24 + (61 - 58 * T + T ** 2 + 600 * C - 330 * E_) * A ** 6 / 720))
    if lat < 0:
        y += 10000000
    return (x, y, z, l)
...
def utm_to_wgs84(x: float, y: float, z: int, l: str=None):
    ...
    LAT1 = mu + (3 * E1 / 2 - 27 * E1 ** 3 / 32 + 269 * E1 ** 5 / 512) * sin(2 * mu) + (21 * E1 ** 2 / 16 - 55 * E1 ** 4 / 32) * sin(4 * mu) + (151 * E1 ** 3 / 96 - 417 * E1 ** 5 / 128) * sin(6 * mu) + 1097 * E1 ** 4 / 512 * sin(8 * mu)
    C1 = E1 * cos(LAT1) ** 2
    T1 = tan(LAT1) ** 2
    N1 = R / sqrt(1 - E * sin(LAT1) ** 2)
    R1 = R * (1 - E) / (1 - E * sin(LAT1) ** 2) ** 1.5
    D = x / (N1 * K0)
    lat = degrees(LAT1 - N1 * tan(LAT1) / R1 * (D ** 2 / 2 - (5 + 3 * T1 + 10 * C1 - 4 * C1 ** 2 - 9 * E_) * D ** 4 / 24 + (61 + 90 * T1 + 298 * C1 + 45 * T1 ** 2 - 252 * E_ - 3 * C1 ** 2) * D ** 6 / 720))
    lon = LON0 + degrees((D - (1 + 2 * T1 + C1) * D ** 3 / 6 + (5 - 2 * C1 + 28 * T1 - 3 * C1 ** 2 + 8 * E_ + 24 * T1 ** 2) * D ** 5 / 120) / cos(LAT1))
    return (
     lat, lon)
...
def compute_grid_coordinates(image: Image, grid: str, grid_size: int, lat: float, lon: float, scale: int, dpi: int=300) -> Tuple[(List[Tuple[(int, str)]], List[Tuple[(int, str)]])]:
    """
    ...
    if grid == 'UTM':
        ...
        return (
         list(zip(x_grid_cs, map(str, x_labels))), list(zip(y_grid_cs, map(str, y_labels))))


def add_grid(image: Image, grid: str, grid_size: int, lat: float, lon: float, scale: int, dpi: int=300, font: ImageFont=ImageFont.truetype('arial.ttf', 35), color: str='black') -> None:
    """
