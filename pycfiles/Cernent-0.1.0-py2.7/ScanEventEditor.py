...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/gui/cGui/main/ScanEventEditor.py
# Compiled at: 2012-04-26 15:21:42
"""
...
class ScanEventEditor(object):
    ...
    def _closingHandler(self, event=None):
        ...
        if self.curEvent.isComplete():
            ...
            message = 'Are you sure you want to quit and lose all unsaved data?' + '\nThis ScanEvent is not complete, ' + 'thus it will not be run when it is scheduled:'
    ...
    def newScanEvent(self, event=None):
        ...
        """
        if self.hasChanged():
            if not tkMessageBox.askokcancel('New?', 'Would you like to lose all unsaved fields and start over?'):
                return
    ...
    def targetHasChanged(self):
        ...
        if self.targetText.size():
            ...
            selected = None
        if selected is not self.prevTarget:
            return True
        else:
            return False
    ...
    def _getDaysOfWeek(self, weekly):
        days = [
         self.reportFrame.days.mondV, self.reportFrame.days.tuesV, self.reportFrame.days.wedV,
    ...
    def _initScanName(self):
        ...
        self.scanName.grid(row=0, column=0, sticky='NEWS', pady=ScanEventEditor.PADY, padx=ScanEventEditor.PADX)
    ...
    def _initServerList(self):
        ...
        self.serverListBox = CListBox(master=self.serverFrame, selectmode=Tkinter.SINGLE, exportselection=0)
    ...
    def _initSchedDays(self):
        ...
        self.reportFrame.days.monday = CCheckButton(master=self.reportFrame.days, text='M', variable=self.reportFrame.days.mondV)
        ...
        self.reportFrame.days.tuesday = CCheckButton(master=self.reportFrame.days, text='T', variable=self.reportFrame.days.tuesV)
        ...
        self.reportFrame.days.wednesday = CCheckButton(master=self.reportFrame.days, text='W', variable=self.reportFrame.days.wedV)
        ...
        self.reportFrame.days.thursday = CCheckButton(master=self.reportFrame.days, text='TR', variable=self.reportFrame.days.thurV)
        ...
        self.reportFrame.days.friday = CCheckButton(master=self.reportFrame.days, text='F', variable=self.reportFrame.days.friV)
        ...
        self.reportFrame.days.saturday = CCheckButton(master=self.reportFrame.days, text='Sat', variable=self.reportFrame.days.satV)
        ...
        self.reportFrame.days.sunday = CCheckButton(master=self.reportFrame.days, text='Sun', variable=self.reportFrame.days.sunV)
    ...
    def _initNotifs(self):
        ...
        self.reportFrame.notif.startCheck = CCheckButton(master=self.reportFrame.notif, variable=self.reportFrame.notif.startV, command=self._startNotifPressed)
        ...
        self.reportFrame.notif.startNotif = CInitialValueEntry(master=self.reportFrame.notif, text='', textvariable=self.reportFrame.notif.startNotifV, state=Tkinter.DISABLED)
        ...
        self.reportFrame.notif.endCheck = CCheckButton(master=self.reportFrame.notif, variable=self.reportFrame.notif.endV, command=self._endNotifPressed)
        ...
        self.reportFrame.notif.endNotif = CInitialValueEntry(master=self.reportFrame.notif, text='', textvariable=self.reportFrame.notif.endNotifV, state=Tkinter.DISABLED)
        self.reportFrame.notif.endNotif.grid(row=4, column=1, sticky='NWE')
        self.reportFrame.notif.grid(row=0, column=1, sticky='NWE', pady=ScanEventEditor.PADY, padx=ScanEventEditor.PADX)
    ...
    def _gatherNotifs(self):
        if self.reportFrame.notif.startV.get():
            self.curEvent.setStartNotif(self.reportFrame.notif.startNotifV.get())
        else:
            self.curEvent.setStartNotif(None)
        if self.reportFrame.notif.endV.get():
            self.curEvent.setEndNotif(self.reportFrame.notif.endNotifV.get())
        else:
