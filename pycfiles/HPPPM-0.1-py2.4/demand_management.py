...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hpppm/demand_management.py
# Compiled at: 2012-12-19 03:22:47
import re
...
class DemandManagement(ErrorHandler):
    ...
    def post_request(self, url, request, host=None, port=None):
        ...
        if not (host and port):
            match = re.search('://(?P<host>.+?):(?P<port>\\d+)/', url)
            host, port = match.group('host'), match.group('port')
        logger.info('About to POST above request to ' + url)
        try:
            http = HTTPConnection(host, port)
            http.request('POST', url, body=request, headers={'SOAPAction': operation, 'Content-Type': 'text/xml; charset=UTF-8', 'Content-Length': len(request)})
