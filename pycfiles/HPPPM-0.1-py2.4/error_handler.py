...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hpppm/error_handler.py
# Compiled at: 2012-12-19 03:28:34
import re, sys, string, mechanize, getopt, logging, logging.config, os.path as op
__version__ = '0.1'
...
class ErrorHandler:
    ...
    def __set_ops_inputs_reqd(self):
        ...
        """
        self.data['OPS_INPUTS_REQD'] = {'createRequest': ['serviceUrl', 'requestType', 'fields'], 'addRequestNotes': ['serviceUrl', 'requestId', 'notes'], 'executeWFTransitions': ['serviceUrl', 'receiver', 'transition'], 'deleteRequests': ['serviceUrl', 'requestIds'], 'getRequests': ['serviceUrl', 'requestIds'], 'setRequestFields': ['serviceUrl', 'requestId', 'fields'], 'setRequestRemoteReferenceStatus': ['serviceUrl', 'receiver', 'source', 'status', 'fields']}

    def __set_ops_inputs(self):
        """Stores the mapping between operation and inputs/types.
        """
        self.data['OPS_INPUTS'] = {'createRequest': ['serviceUrl', 'requestType', 'fields', 'URLReferences', 'notes'], 'addRequestNotes': ['serviceUrl', 'requestId', 'notes'], 'executeWFTransitions': ['serviceUrl', 'receiver', 'transition'], 'deleteRequests': ['serviceUrl', 'requestIds'], 'getRequests': ['serviceUrl', 'requestIds'], 'setRequestFields': ['serviceUrl', 'requestId', 'fields'], 'setRequestRemoteReferenceStatus': ['serviceUrl', 'receiver', 'source', 'status', 'fields']}
    ...
    def get_reqd_inputs(self, operation=None):
        ...
        """
        if operation:
            return self.data['OPS_INPUTS_REQD'][operation]
        else:
            return self.data['OPS_INPUTS_REQD']
    ...
    def get_inputs(self, operation=None):
        ...
        """
        if operation:
            return self.data['OPS_INPUTS'][operation]
        else:
            return self.data['OPS_INPUTS']
    ...
    def __check_reqd_types(self, fields, reqd_types):
        ...
        if len(present_types) == len(reqd_types):
            return True
    ...
    def validate_tokens(self, fields=[]):
        ...
        def is_ill_token(x):
            return not re.search('^((?:REQD|REQ|UD|T)\\.?(?:VP|P)?\\.[A-Z_0-9]+?)$', x)
        ...
        try:
            ...
            for tag in to_extract:
                ...
                if value is not None:
                    values[tag] = value.text

        except ImportError, err:
            logger.debug('lxml.etree import failed!Trying with regexp' + str(err))
            for tag in to_extract:
                match = re.search('<%s>(.+)</%s>' % (tag, tag), resp)
                if match:
                    values[tag] = match.group(1)
