...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hpppm/field_parser.py
# Compiled at: 2012-12-13 02:30:22
import re, sys
__version__ = '0.1'
...
def parser(inp, extract, sep='"\\s+?"', wo='</|>|<|"|\'', it=[], ig=[]):
    ...
    if isinstance(inputs, str):
        for token in tokenizer(inputs, weed, sep, ignore=it):
            tokens.append(token)

    for token in inputs:
        tokens.append(weed(token))
