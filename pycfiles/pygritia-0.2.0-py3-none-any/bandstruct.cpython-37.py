...
# Compiled at: 2019-02-26 10:17:51
...
def plot_band_sturture(emin=-10.0, emax=10.0):
    ...
    for n in range(e_skn.shape[2]):
        ...
        ax.scatter(klist, (e_skn[0, :, n]), s=(psi_skn_f[0, :, n]), c='r',
          edgecolors='r')

    if len(e_skn) == 2:
        for n in range(e_skn.shape[2]):
            ax.plot(klist, e_skn[1, :, n], 'k--')
            ax.scatter(klist, (e_skn[1, :, n]), s=(psi_skn_f[1, :, n]), c='b',
              edgecolors='b')
