...
# Embedded file name: /home/ykent/GitLab/pygrisb/pygrisb/pygrisb/gutz/ginput.py
# Compiled at: 2019-02-23 22:41:15
# Size of source mod 2**32: 4955 bytes
import numpy as np, h5py, sys
from pygrisb.math.matrix_basis import sigmatomatrixbasis

def save_gparam(iso=1, ispin=1, ityp_list=[0], imap_list=[0], na2_list=[2], imix=0, amix=0.2, nbreset=0, iembeddiag=0, sigma_list=None, v2e_list=None, max_iter=10000, sx_list=None, sy_list=None, sz_list=None, lx_list=None, ly_list=None, lz_list=None, utrans_list=None, ldc=0, u_avg_list=None, j_avg_list=None, nelf_list=None, rotations_list=None, lie_odd_params_list=None, lie_even_params_list=None, jgenerator_list=None, sp_rotations_list=None, nval_bot_list=[
 0], nval_top_list=[2]):
    if '-n' in sys.argv:
    ...
    with h5py.File('GPARAM.h5', 'w') as (f):
        ...
        for i, na2 in enumerate(na2_list):
            ...
            if utrans_list is not None:
                f['/IMPURITY_' + str(i + 1) + '/DB_TO_SAB'] = utrans_list[i].T
            if rotations_list is not None:
                f['/IMPURITY_' + str(i + 1) + '/rotations'] = rotations_list[i]
            if lie_odd_params_list is not None:
                f['/IMPURITY_' + str(i + 1) + '/lie_odd_params'] = lie_odd_params_list[i]
                f['/IMPURITY_' + str(i + 1) + '/lie_even_params'] = lie_even_params_list[i]
                f['/IMPURITY_' + str(i + 1) + '/JGENERATOR'] = np.swapaxes(jgenerator_list[i], 1, 2)
                f['/IMPURITY_' + str(i + 1) + '/SP_ROTATIONS'] = np.swapaxes(sp_rotations_list[i], 1, 2)
                f['/IMPURITY_' + str(i + 1) + '/nsym_odd'] = np.asarray(sp_rotations_list[i]).shape[0:1]

        f['/dim_hs_imp'] = np.asarray(dim_hs_list)


def save_gparambands(kpt_wt, nelectron, nbmax, iso=1, ispin=1, symop=1, symie=1, ensemble=0, ismear=0, delta=0.01, ne_list=None, h1e_list=None):
    with h5py.File('GPARAMBANDS.h5', 'w') as (f):
        ...
        if ne_list is not None:
            assert ne_list.shape[0] == kpt_wt.shape[0], ' error: ne_list and kpt_wt do not match!'
            f['/NE_LIST'] = np.asarray(ne_list)
        f['/kptwt'] = kpt_wt
