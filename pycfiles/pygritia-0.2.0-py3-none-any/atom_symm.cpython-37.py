...
# Embedded file name: /home/ykent/GitLab/pygrisb/pygrisb/pygrisb/symm/atom_symm.py
# Compiled at: 2019-02-26 10:14:33
# Size of source mod 2**32: 17225 bytes
...
def get_3D_rotation_generator():
    return complex(-0.0, -1.0) * np.array([[[int((i - j) * (j - k) * (k - i) / 2) for k in range(3)] for j in range(3)] for i in range(3)])
...
def get_rotation(J, theta, method='eigh'):
    ...
    if method == 'eigh':
        ...
        rep = np.dot(np.dot(vecs, np.diag(np.exp(complex(0.0, 1.0) * vals))), np.conj(vecs).T)
    elif method == 'expm':
        from scipy.linalg import expm
        rep = expm(complex(0.0, 1.0) * exponent)
    else:
        raise AssertionError(' method =' + method + ': Unsupported in get_rotation!')
    return rep
...
def get_Lie_parameter_3D(g, plus2pi=False):
    ...
    list_theta = []
    assert len(g) == 3, ' Error: get_Lie_parameter_3D is exclusively for 3D rotations!'
    vals, vecs = np.linalg.eig(g)
    rotation_axes_label = np.abs(vals - 1.0).tolist().index(min(np.abs(vals - 1.0)))
    axis = vecs[:, rotation_axes_label].real
    fix = (np.trace(g) - 1.0) / 2.0
    if fix <= -1.0:
        assert np.abs((np.trace(g) - 1.0) / 2.0 - -1.0) < 1e-10
        fix = -1.0
    if fix >= 1.0:
        assert np.abs((np.trace(g) - 1.0) / 2.0 - 1.0) < 1e-10
        fix = 1.0
    ...
    if plus2pi:
        list_theta.append((alpha + np.pi * 2.0) * axis)
    return list_theta
...
def check_Lie_parameters(G, J, Lie_parameters):
    ...
    """
    N = len(Lie_parameters) // len(G)
    for i, g in enumerate(G):
        for j in range(i * N, (i + 1) * N):
            theta = Lie_parameters[j]
            gp = get_rotation(J, theta, method='eigh')
            err = np.max(np.abs(gp - g))
            assert err < 1e-07, ' check_Lie_parameters!'
...
def get_ratio_matab(a, b):
    ...
    for i, j in itertools.product((range(len(a))), repeat=2):
        if np.abs(a[i][j]) > 1e-05:
            ia = i
            ja = j
            break

    if ia >= 0:
        if np.abs(b[ia][ja]) > 1e-07:
            rat = a[(ia, ja)] / b[(ia, ja)]
            if np.max(np.abs(a - b * rat)) < 1e-06:
                ratio = rat
    return ratio
...
def check_strcture_constants(J, Jp):
    ...
    if np.max(np.abs(eijkp - eijk)) > 1e-06:
...
def get_representation(J, Lie_parameters):
    ...
    for theta in Lie_parameters:
        ...
        try:
            repr = repr.multiply(np.abs(repr) > 1e-08)
        except:
            pass
    ...
    for i, R in enumerate(R_list):
        ...
        if dist < 1e-06:
            if loc == -1:
                ...
                raise AssertionError('Found multiple identical elements!')

    if loc == -1:
        print(' Rotation operations list:')
        print_R_list(R_list)
        raise AssertionError('Failed to locate the element with min_dist =' + ' %12.3f' % min_dist)
    return loc
...
def get_eigen_space(H, l_list, tol=1e-06):
    """
...
def get_ired_representation(representation):
    ...
    from pygrisb.math.matrix_util import get_matrices_bk_list, sym_array
    from scipy.linalg import block_diag
    check = 'no'
    ...
    while check == 'no':
        X = []
        for l1 in l_list:
            X.append(np.random.rand(l1, l1) + np.random.rand(l1, l1) * complex(0.0, 1.0))
        ...
        list_chi, list_repr = get_sub_repr_characters(representation, list_eigen_space)
        check = check_sum_chi2(list_chi)
        if check == 'ok':
            break

    return (
     list_eigen_space, list_chi, list_repr)
...
def get_chi_repr(list_chi, list_repr, list_eigen_space):
    ...
    for i, product in enumerate(chi_product):
        ...
        print(' chi_space {} dimension = {}'.format(len(list_chi_space) - 1, np.asarray(chi_space).shape))

    return (
     list_chi_repr, list_chi_space, list_distinct_chi)
...
def check_U_equiv_repr(representationA, representationB, U):
    for i in range(len(representationA)):
        ...
        if err > 1e-05:
            raise AssertionError('Large error in check_U_equiv_repr = %10.5f' % err)
...
def get_U_Jnew(list_chi_space, list_chi_U, reorder=True):
    ...
    print(' U_Jnew unitary error = {}'.format(np.max(np.abs(U_Jnew.T.conj().dot(U_Jnew) - np.eye(U_Jnew.shape[0])))))
    return U_Jnew
...
def get_diag_sub_array(H, I1, I2, J1, J2):
    ...
    assert err < 1e-07, ' Subarray not diagonal, err > 1.e-7!'
    return H[I1:I2, J1:J2]


def get_atom_U_sigma_chi(representation):
    list_eigen_space, list_chi, list_repr = get_ired_representation(representation)
    list_chi_repr, list_chi_space, list_distinct_chi = get_chi_repr(list_chi, list_repr, list_eigen_space)
