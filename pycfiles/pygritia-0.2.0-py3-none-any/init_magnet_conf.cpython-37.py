...
# Embedded file name: /home/ykent/GitLab/pygrisb/pygrisb/pygrisb/gutz/init_magnet_conf.py
# Compiled at: 2019-02-26 10:16:59
# Size of source mod 2**32: 6495 bytes
import h5py, numpy, os
from pygrisb.math.matrix_util import sym_array
from pygrisb.gutz.usrqa import get_usr_input
...
def get_b_field_list():
    ...
    with h5py.File('ginit.h5', 'r') as (f):
        unit = f['/usrqa/unit'][()]
        unique_magmom_direction_list = f['/usrqa/unique_magmom_direction_list'][()]
    if 'ryd' in unit:
        unit = Ryd_eV
    else:
        unit = 1.0
...
def get_vext_given_1pdm_list(dm_list):
    ...
    vext_list = [-dc.get_vdc_hf(v2e, dm) + dc.get_vdc_fll(uavg, javg, numpy.trace(dm)) * numpy.eye(dm.shape[0]) for v2e, dm, uavg, javg in zip(v2e_list, dm_list, uavg_list, javg_list)]
    return vext_list
...
def chk_local_one_body(vext_list):
    ...
    with h5py.File('GPARAM.h5', 'r') as (f):
        ...
        for imp in range(num_imp):
            prepath = '/IMPURITY_' + str(imp + 1)
            db2sab_list.append(f[(prepath + '/DB_TO_SAB'.format(imp + 1))][()].T)
