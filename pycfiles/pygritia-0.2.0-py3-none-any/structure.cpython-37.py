...
# Embedded file name: /home/ykent/GitLab/pygrisb/pygrisb/pygrisb/gutz/structure.py
# Compiled at: 2019-02-26 10:14:12
# Size of source mod 2**32: 3644 bytes
import os, sys, h5py, glob, pymatgen, numpy
from pygrisb.gutz.gatom import gAtoms
...
def read_cif():
    ...
    struct = pymatgen.symmetry.analyzer.SpacegroupAnalyzer(struct).get_primitive_standard_structure()
...
def h5save_structure_params():
    ...
    with h5py.File('ginit.h5', 'a') as (f):
        ...
        symbols = [numpy.string_(s) for s in symbols]
...
def get_gatoms():
    ...
    with h5py.File('ginit.h5', 'r') as (f):
        ...
        if '/struct/locrot_list' in f:
            ...
            locrot_list = None
    return gAtoms(symbols=symbols, cell=cell, scaled_positions=scaled_positions,
      pbc=True,
      wiencase=case,
...
def check_material(material, f):
    ...
    print(' Mapping-table equivalent atoms: ', (material.idx_equivalent_atoms),
      file=f)
    print(' Labels of correlated atoms: ', (material.corr_list), file=f)
    print(' Orbital angular momentums of correlated electrons: ', (material.df_list),
      file=f)
    print(' Take into accout SOC or not: ', (material.iso),
      file=f)
    print(' Take into accout crystal-fields or not: ', (material.crystal_field),
      file=f)
    print(' Cut-off distance for symmetry analysis: ', (material.sym_dist_cut),
      file=f)
