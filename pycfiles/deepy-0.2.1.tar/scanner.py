...
# [GCC 8.4.0]
# Embedded file name: /home/shu/research/deepy/deepy/utils/scanner.py
...
class Scanner(object):
    ...
    def __init__(self, func, sequences=None, outputs_info=None, non_sequences=None, **kwargs):
        if sequences and type(sequences) != dict or outputs_info and type(outputs_info) != dict or non_sequences and type(non_sequences) != dict:
    ...
    def compute(self):
        results, updates = theano.scan(self._func_wrapper, sequences=filter(lambda t: t is not None, self._sequence_values), outputs_info=self._output_values, non_sequences=filter(lambda t: t is not None, self._non_sequence_values), **self._kwargs)
        if type(results) != list:
            results = [
             results]
        return (
         dict(zip(self._output_keys, results)), updates)