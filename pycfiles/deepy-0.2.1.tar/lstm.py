...
# [GCC 8.4.0]
# Embedded file name: /home/shu/research/deepy/deepy/layers/lstm.py
...
class LSTM(NeuralLayer):
    ...
    def produce_initial_states(self, x):
        if self.persistent_state:
            ...
            h0 = T.alloc(np.cast[FLOATX](0.0), x.shape[0], self._hidden_size)
            m0 = h0
            return (h0, m0)
    ...
    def compute_tensor(self, x):
        ...
        (hiddens, memories), _ = theano.scan(self.step, sequences=sequences, outputs_info=[
         h0, m0], n_steps=self._steps, go_backwards=self.go_backwards)
        if self.persistent_state:
    ...
    def _setup_params(self):
        ...
        if self._input_type == 'sequence':
            self.register_parameters(self.W_i, self.U_i, self.b_i, self.W_c, self.U_c, self.b_c, self.W_f, self.U_f, self.b_f, self.W_o, self.U_o, self.b_o)
        else:
            self.register_parameters(self.U_i, self.b_i, self.U_c, self.b_c, self.U_f, self.b_f, self.U_o, self.b_o)
