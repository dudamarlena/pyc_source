...
# [GCC 8.4.0]
# Embedded file name: /home/shu/research/deepy/deepy/networks/recursive_auto_encoder.py
# Compiled at: 2016-04-20 00:05:45
import logging as loggers, theano, theano.tensor as T
...
class RecursiveAutoEncoder(NeuralNetwork):
    ...
    def encode(self, x):
        ...
        if not self._encode_func:
            x_var = T.vector()
            self._encode_func = theano.function([x_var], self.layers[0].encode_func(x), allow_input_downcast=True, mode=theano.Mode(linker=THEANO_LINKER))
    ...
    def decode(self, rep, n_steps):
        ...
        if not self._decode_func:
            rep_var = T.vector()
            n_var = T.iscalar()
            self._decode_func = theano.function([rep_var, n_var], self.layers[0].decode_func(rep_var, n_var), allow_input_downcast=True, mode=theano.Mode(linker=THEANO_LINKER))
...
class RecursiveAutoEncoderCore(NeuralLayer):
    ...
    def _recursive_func(self, x):
        if self.additional_h:
            ...
            h0 = x[0]
            start_index = 1
        (reps, distances), _ = theano.scan(self._recursive_step, sequences=[T.arange(start_index, x.shape[0])], outputs_info=[
         h0, None], non_sequences=[x])
    ...
    def encode_func(self, x):
        if self.additional_h:
            ...
            h0 = x[0]
            start_index = 1
        (reps, _), _ = theano.scan(self._recursive_step, sequences=[T.arange(start_index, x.shape[0])], outputs_info=[
         h0, None], non_sequences=[x])
        return reps[(-1)]
    ...
    def _setup_params(self):
        ...
        self.register_parameters(self.W_e1, self.W_e2, self.W_d1, self.W_d2, self.B_e, self.B_d1, self.B_d2)
        return