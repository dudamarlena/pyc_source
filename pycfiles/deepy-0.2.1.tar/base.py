...
# [GCC 8.4.0]
# Embedded file name: /home/shu/research/deepy/deepy/trainers/base.py
# Compiled at: 2016-05-27 03:20:59
import sys, numpy as np, theano
...
class NeuralTrainer(object):
    ...
    def _compile_evaluation_func(self):
        if not self.evaluation_func:
            ...
            self.evaluation_func = theano.function(self.network.input_variables + self.network.target_variables, self.evaluation_variables, updates=self.network.updates, allow_input_downcast=True, mode=self.config.get('theano_mode', None))
        return
    ...
    def test_step(self, test_set):
        ...
        costs = list(zip(self.evaluation_names, np.mean([ self.evaluation_func(*x) for x in test_set ], axis=0)))
        return costs

    def valid_step(self, valid_set):
        self._compile_evaluation_func()
        costs = list(zip(self.evaluation_names, np.mean([ self.evaluation_func(*x) for x in valid_set ], axis=0)))
        return costs
