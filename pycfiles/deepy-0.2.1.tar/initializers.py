...
# [GCC 8.4.0]
# Embedded file name: /home/shu/research/deepy/deepy/utils/initializers.py
# Compiled at: 2016-05-18 19:32:57
import numpy as np
...
class UniformInitializer(WeightInitializer):
    ...
    def sample(self, shape):
        if not self.scale:
            scale = np.sqrt(6.0 / sum(get_fans(shape)))
        else:
            scale = self.scale
...
class XavierGlorotInitializer(WeightInitializer):
    ...
    def sample(self, shape):
        scale = np.sqrt(2.0 / sum(get_fans(shape)))
        if self.uniform:
