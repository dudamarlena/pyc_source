...
# [GCC 8.4.0]
# Embedded file name: /home/shu/research/deepy/deepy/trainers/scipy_trainer.py
...
import theano, numpy as np, theano.tensor as T, scipy
...
class ScipyTrainer(NeuralTrainer):
    ...
    def train_step(self, train_set, train_size=None):
        res = scipy.optimize.minimize(fun=self._function_at, jac=self._gradient_at, x0=self._arrays_to_flat(self.best_params[0]), args=(
         train_set,), method=self.method, options=dict(maxiter=self.scipy_updates))
    ...
    def _gradient_function(self):
        if not self._gradient_func:
            ...
            self._gradient_func = theano.function(inputs, T.grad(self.cost, params), allow_input_downcast=True, mode=theano.Mode(linker=THEANO_LINKER))
    ...
    def _flat_to_arrays(self, x):
        ...
        return [ x[o:o + n].reshape(s) for s, o, n in zip(self._shapes, self._starts, self._counts)
               ]
