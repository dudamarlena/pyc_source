...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/thrift4DL/server/tcp/connectors.py
...
class Validator(object):
    ...
    def process_invalid(self, connection_info):
        ...
        connection_info['result'] = TApplicationException(TApplicationException.UNKNOWN_METHOD, 'Unknown function %s' % name)
    ...
    def process(self, connection_info):
        if not len(self.func_names) > 0:
            raise AssertionError(ValueError('`func_names` in `Validator` must be not empty'))
        else:
