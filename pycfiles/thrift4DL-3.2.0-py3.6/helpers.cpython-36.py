...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/thrift4DL/helpers.py
...
def bytify_image(image_arr, ext='png', format_params=None):
    byte_img = cv2.imencode(('.' + ext), image_arr, params=format_params)[1].tostring()
    return byte_img
...
def encode_image(image_arr, encode_type='hex', ext='jpg', format_params=None):
    assert encode_type in ('hex', 'byte'), ValueError('`encode_type` must be `hex` or `byte`')
    if encode_type == 'hex':
