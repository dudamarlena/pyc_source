...
def list_example_datafiles():
    ...
    """
    valid_datafiles = [name for name in os.listdir(data_dir) if len(name.split('.')) >= 2 if len(name.split('.')[0]) > 0]
...
def get_example_matrix(name=None, print_description=True):
    ...
    if print_description:
        ...
        with open(file_name, 'r') as (f):
            lines = f.readlines()
            lines = [l for l in lines if len(l) > 0 if l[0] == '#']
...
def open_example_datafile(name=None, print_description=True):
    ...
    if not os.path.isfile(file_name):
        raise AssertionError('File %s does not exist!' % file_name)
    else:
        if print_description:
            print('Description of example matrix "%s":' % name)
            with open(file_name, 'r') as (f):
