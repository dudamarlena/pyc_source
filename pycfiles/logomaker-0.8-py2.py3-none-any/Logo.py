...
# Compiled at: 2019-05-08 11:02:30
...
class Logo:
    ...
    **kwargs:
    ...
    def __init__(self, df, color_scheme=None, font_name='sans', stack_order='big_on_top', center_values=False, baseline_width=0.5, flip_below=True, shade_below=0.0, fade_below=0.0, fade_probabilities=False, vpad=0.0, vsep=0.0, alpha=1.0, show_spines=None, ax=None, zorder=0, figsize=(10, 2.5), **kwargs):
        ...
        self.style_glyphs_below(shade=self.shade_below, fade=self.fade_below, flip=self.flip_below)
    ...
    def _input_checks(self):
        ...
        check(all([ isinstance(n, (int, float)) and n > 0 for n in self.figsize
            ...
            for c in self.cs:
                ...
                if v < 0:
                    ...
                    if alpha is None:
                        ...
                        this_alpha = alpha
                    g.set_attributes(color=(this_color * (1.0 - shade)), alpha=(this_alpha * (1.0 - fade)), flip=flip, **kwargs)
    ...
    def highlight_position_range(self, pmin, pmax, padding=0.0, color='yellow', edgecolor=None, floor=None, ceiling=None, zorder=-2, **kwargs):
        ...
        patch = Rectangle(xy=(x, y), width=width, height=height, facecolor=color, edgecolor=edgecolor, zorder=zorder, **kwargs)
    ...
    def draw_baseline(self, zorder=-1, color='black', linewidth=0.5, **kwargs):
        ...
        **kwargs:
        ...
        self.ax.axhline(zorder=zorder, color=color, linewidth=linewidth, **kwargs)
    ...
    @handle_errors
    def style_spines(self, spines=('top', 'bottom', 'left', 'right'), visible=True, linewidth=1.0, color='black', bounds=None):
        """
        ...
        for p in self.ps:
            ...
            if self.stack_order == 'big_on_top':
                ...
                for v, c in zip(vs, cs):
                    ...
                    glyph = Glyph(p, c, ax=self.ax, floor=floor, ceiling=ceiling, color=this_color, flip=flip, zorder=self.zorder, font_name=self.font_name, alpha=self.alpha, vpad=self.vpad, **self.glyph_kwargs)
