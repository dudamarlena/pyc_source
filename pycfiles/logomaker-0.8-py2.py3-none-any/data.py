...
def transform_matrix(df, from_type, to_type, background=None, pseudocount=1, center=False):
    ...
    if from_type == to_type:
        ...
        assert to_type in TO_TYPES, 'Error: invalid to_type="%s"' % from_type
        if from_type == 'probability':
            ...
            out_df = transform_matrix(prob_df, from_type='probability', to_type=to_type, background=background)
    if center:
...
def normalize_matrix(df):
    ...
    """
    df = validate_matrix(df)
    assert all(df.values.ravel() >= 0), 'Error: Some data frame entries are negative.'
...
def iupac_to_matrix(iupac_seq, to_type='probability', **kwargs):
    ...
    **kwargs:
    ...
    out_df = transform_matrix(counts_mat, pseudocount=0, from_type='counts', to_type=to_type)
    return out_df
...
def alignment_to_matrix(sequences, to_type='counts', characters_to_ignore='.-', **kwargs):
    ...
    **kwargs:
    ...
    out_df = transform_matrix(counts_df, from_type='counts', to_type=to_type, **kwargs)
    return out_df