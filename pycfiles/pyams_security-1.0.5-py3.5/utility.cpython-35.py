...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyams_security/utility.py
...
from pyams_security.interfaces import AuthenticatedPrincipalEvent, IAuthenticationPlugin, ICredentialsPlugin, IDirectoryPlugin, IGroupsAwareDirectoryPlugin, IJWTAuthenticationPlugin, IProtectedObject, ISecurityManager
...
class SecurityManager(Folder):
    __doc__ = 'Security manager utility'
        ...
        if context is not None:
            for parent in lineage(context):
                ...
                if protection is not None:
                    for principal in principals.copy():
                        principals |= set(map('role:{}'.format, protection.get_roles(principal)))
...
class PyAMSAuthenticationPolicy:
    __doc__ = 'PyAMS authentication policy\n\n    This authentication policy relies on a registered ISecurityManager utility.\n    Use same authentication ticket as AuthTktAuthenticationPolicy.\n\n    ``credentials`` is the list of credentials extraction utilities which can be\n    used to get credentials.\n\n    See `pyramid.authentication.AuthTktAuthenticationPolicy` to get description\n    of other constructor arguments.\n    '

    def __init__(self, secret, credentials=('jwt', 'http'), cookie_name='auth_ticket', secure=False, include_ip=False, timeout=None, reissue_time=None, max_age=None, path='/', http_only=False, wild_domain=True, hashalg='sha256', parent_domain=False, domain=None):
        self.credentials_names = credentials
        self.cookie = AuthTktCookieHelper(secret, cookie_name=cookie_name, secure=secure, include_ip=include_ip, timeout=timeout, reissue_time=reissue_time, max_age=max_age, http_only=http_only, path=path, wild_domain=wild_domain, hashalg=hashalg, parent_domain=parent_domain, domain=domain)

    @reify
    def credentials_plugins(self):
        """Get list of credentials plug-ins"""
        return [query_utility(ICredentialsPlugin, name=name) for name in self.credentials_names]

    @staticmethod
    def _get_security_manager(request=None):
    ...
    def effective_principals(self, request, context=None):
        ...
        try:
            LOGGER.debug('>>> getting principals for principal {0} ({1}) on {2!r}'.format(request.principal.title, request.principal.id, context or request.context))
    ...
    def remember(self, request, principal, **kw):
        ...
        if policy is not None and policy.unauthenticated_userid(request) == principal:
            return
    ...
    def forget(self, request):
        ...
        policy = self.get_jwt_policy()
        if policy is not None and policy.unauthenticated_userid(request) is not None:
            return
