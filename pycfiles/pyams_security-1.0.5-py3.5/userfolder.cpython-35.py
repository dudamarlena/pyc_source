...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyams_security/plugin/userfolder.py
# Compiled at: 2020-02-21 07:52:38
# Size of source mod 2**32: 13828 bytes
"""PyAMS_security.plugin.userfolder module

This module is used to define local users.
"""
import base64, hashlib, hmac, os, random, sys
...
class User(Persistent, Contained):
    __doc__ = 'Local user persistent class'
...
def user_principal_info_adapter(user):
    ...
    return PrincipalInfo(id='{prefix}:{login}'.format(prefix=user.__parent__.prefix, login=user.login), title=user.title)
...
try:
    ...
    class UserPrincipalMailInfoAdapter(ContextAdapter):
        __doc__ = 'User principal mail info adapter'

        def get_addresses(self):
            """Get user email address"""
            yield (
             '{0} {1}'.format(self.context.firstname, self.context.lastname), self.context.email)


@implementer(IUsersFolderPlugin)
class UsersFolder(Folder):
    __doc__ = 'Local users folder'
    ...
    def authenticate(self, credentials, request):
        ...
        if principal is not None:
            ...
            if principal.check_password(password):
                pass
            return '{prefix}:{login}'.format(prefix=self.prefix, login=principal.login)
    ...
    def get_principal(self, principal_id, info=True):
        ...
        user = self.get(login)
        if user is not None and info:
            return PrincipalInfo(id='{prefix}:{login}'.format(prefix=self.prefix, login=user.login), title=user.title)
        return user
    ...
    def find_principals(self, query):
        ...
        for user in self.values():
            if query == user.login or query in user.firstname.lower() or query in user.lastname.lower() or query in user.email.lower():
                yield PrincipalInfo(id='{prefix}:{login}'.format(prefix=self.prefix, login=user.login), title='{title} <{email}>'.format(title=user.title, email=user.email))
    ...
    def get_search_results(self, data):
        ...
        query = query.lower()
        for user in self.values():
            if query == user.login or query in user.firstname.lower() or query in user.lastname.lower():
                yield user


@vocabulary_config(name=USERS_FOLDERS_VOCABULARY_NAME)
class UsersFolderVocabulary(SimpleVocabulary):
    __doc__ = "'PyAMS users folders' vocabulary"
...
def handle_new_local_user(event):
    ...
    if message_text is not None:
        ...
        template = PageTemplateFile(os.path.join(os.path.split(__file__)[0], 'templates', template_name), translate=query_utility(IChameleonTranslate))
        message_text = template(request=request, user=user, site=site, settings=settings)
    body = PageTemplateFile(os.path.join(os.path.split(__file__)[0], 'templates', 'register-body.pt'))
    html_body = body(request=request, user=user, site=site, settings=settings, message=message_text)
    message = Message(subject=translate(_('{prefix}Please confirm registration')).format(prefix='{prefix} '.format(prefix=settings.subject_prefix) if settings.subject_prefix else ''), sender='{name} <{email}>'.format(name=settings.sender_name, email=settings.sender_email), recipients=(
     '{firstname} {lastname} <{email}>'.format(firstname=user.firstname, lastname=user.lastname, email=user.email),), html=Attachment(data=html_body, content_type='text/html; charset=utf-8', disposition='inline', transfer_encoding='quoted-printable'), body=Attachment(data=html_to_text(html_body), content_type='text/plain; charset=utf-8', disposition='inline', transfer_encoding='quoted-printable'))
    mailer.send(message)