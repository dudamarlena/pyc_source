...
# [GCC 8.4.0]
# Embedded file name: /cygdrive/c/Users/Nad/oneline/oneline/lib/lz4/nose-1.3.4-py2.7.egg/nose/sphinx/pluginopts.py
...
def autoplugin_directive(dirname, arguments, options, content, lineno, content_offset, block_text, state, state_machine):
    ...
    rst.append('------', '<autodoc>')
    rst.append('.. include :: %s\n' % utils.relative_path(state_machine.document['source'], os.path.abspath(mod.__file__.replace('.pyc', '.py'))), '<autodoc>')
    rst.append('   :literal:\n', '<autodoc>')
    rst.append('', '<autodoc>')
    node = nodes.section()
...
def setup(app):
    app.add_directive('autoplugin', autoplugin_directive, 1, (1, 0, 1), plugin=directives.unchanged)
