...
# [GCC 8.4.0]
# Embedded file name: /cygdrive/c/Users/Nad/oneline/oneline/lib/lz4/nose-1.3.4-py2.7.egg/nose/plugins/logcapture.py
...
class LogCapture(Plugin):
    ...
    def options(self, parser, env):
        ...
        """
        parser.add_option('--nologcapture', action='store_false', default=not env.get(self.env_opt), dest='logcapture', help='Disable logging capture plugin. Logging configuration will be left intact. [NOSE_NOLOGCAPTURE]')
        parser.add_option('--logging-format', action='store', dest='logcapture_format', default=env.get('NOSE_LOGFORMAT') or self.logformat, metavar='FORMAT', help='Specify custom format to print statements. Uses the same format as used by standard logging handlers. [NOSE_LOGFORMAT]')
        parser.add_option('--logging-datefmt', action='store', dest='logcapture_datefmt', default=env.get('NOSE_LOGDATEFMT') or self.logdatefmt, metavar='FORMAT', help='Specify custom date/time format to print statements. Uses the same format as used by standard logging handlers. [NOSE_LOGDATEFMT]')
        parser.add_option('--logging-filter', action='store', dest='logcapture_filters', default=env.get('NOSE_LOGFILTER'), metavar='FILTER', help='Specify which statements to filter in/out. By default, everything is captured. If the output is too verbose,\nuse this option to filter out needless output.\nExample: filter=foo will capture statements issued ONLY to\n foo or foo.what.ever.sub but not foobar or other logger.\nSpecify multiple loggers with comma: filter=foo,bar,baz.\nIf any logger name is prefixed with a minus, eg filter=-foo,\nit will be excluded rather than included. Default: exclude logging messages from nose itself (-nose). [NOSE_LOGFILTER]\n')
        parser.add_option('--logging-clear-handlers', action='store_true', default=False, dest='logcapture_clear', help='Clear all other logging handlers')
        parser.add_option('--logging-level', action='store', default='NOTSET', dest='logcapture_level', help='Set the log level to capture')
    ...
    def start(self):
        self.handler = MyMemoryHandler(self.logformat, self.logdatefmt, self.filters)
    ...
    def addCaptureToErr(self, ev, records):
        return ('\n').join([
         safe_str(ev), ln('>> begin captured logging <<')] + records + [
