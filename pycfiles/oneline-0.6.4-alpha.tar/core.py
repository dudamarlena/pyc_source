...
# [GCC 8.4.0]
# Embedded file name: /cygdrive/c/Users/Nad/oneline/oneline/lib/lz4/nose-1.3.4-py2.7.egg/nose/core.py
# Compiled at: 2014-09-06 21:58:19
"""Implements nose test program and collector.
"""
from __future__ import generators
import logging, os, sys, time, unittest
from nose.config import Config, all_config_files
from nose.loader import defaultTestLoader
from nose.plugins.manager import PluginManager, DefaultPluginManager, RestrictedPluginManager
...
__all__ = [
 'TestProgram', 'main', 'run', 'run_exit', 'runmodule', 'collector',
 'TextTestRunner']
...
class TextTestRunner(unittest.TextTestRunner):
    ...
    def _makeResult(self):
        return TextTestResult(self.stream, self.descriptions, self.verbosity, self.config)
    ...
    * addplugins: List of **extra** plugins to use. Pass a list of plugin
    ...
    def runTests(self):
        ...
        if self.testRunner is None:
            self.testRunner = TextTestRunner(stream=self.config.stream, verbosity=self.config.verbosity, config=self.config)
    ...
    def showPlugins(self):
        ...
        for p in self.config.plugins:
            ...
            if v >= 2:
                ...
                print ('\n').join(textwrap.wrap(p.help().strip(), initial_indent='  ', subsequent_indent='  '))
                if v >= 3:
                    parser = DummyParser()
                    p.addOptions(parser)
                    if len(parser.options):
                        ...
                        for opts, help in parser.options:
                            print '  %s' % (', ').join(opts)
                            if help:
                                print ('\n').join(textwrap.wrap(help.strip(), initial_indent='    ', subsequent_indent='    '))
    ...
    def usage(cls):
        ...
        try:
            ...
            f = open(os.path.join(os.path.dirname(__file__), 'usage.txt'), 'r')
            try:
                text = f.read()
            finally:
                f.close()
...
def collector():
    ...
    """
    setuptools_incompat = ('report', 'prepareTest', 'prepareTestLoader', 'prepareTestRunner',
                           'setOutputStream')
