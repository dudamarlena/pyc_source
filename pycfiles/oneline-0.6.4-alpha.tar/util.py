...
# [GCC 8.4.0]
# Embedded file name: /cygdrive/c/Users/Nad/oneline/oneline/lib/lz4/nose-1.3.4-py2.7.egg/nose/util.py
# Compiled at: 2014-09-06 21:58:19
"""Utility functions and classes used by nose internally.
"""
import inspect, itertools, logging, os, re, sys, types, unittest
...
skip_pattern = '(?:\\.svn)|(?:[^.]+\\.py[co])|(?:.*~)|(?:.*\\$py\\.class)|(?:__pycache__)'
try:
    set()
    set = set
except NameError:
...
def ls_tree(dir_path='', skip_pattern=skip_pattern, indent='|-- ', branch_indent='|   ', last_indent='`-- ', last_branch_indent='    '):
    return ('\n').join(_ls_tree_lines(dir_path, skip_pattern, indent, branch_indent, last_indent, last_branch_indent))


def _ls_tree_lines(dir_path, skip_pattern, indent, branch_indent, last_indent, last_branch_indent):
    if dir_path == '':
        dir_path = os.getcwd()
    lines = []
    ...
    entries = list(itertools.chain([ (name, False) for name in nondirs ], [ (name, True) for name in dirs ]))
    ...
    def ls_entry(name, is_dir, ind, branch_ind):
        if not is_dir:
            ...
            if not os.path.islink(path):
                yield ind + name
                subtree = _ls_tree_lines(path, skip_pattern, indent, branch_indent, last_indent, last_branch_indent)
                for x in subtree:
                    yield branch_ind + x
    ...
    if not head:
        try:
            ...
            if len(parts[0]) == 1:
                ...
                raise ValueError("Test name '%s' could not be parsed. Please format test names as path:callable or module:callable." % (
                 test,))
...
def test_address(test):
    ...
    if hasattr(test, 'address'):
        ...
        if t == types.MethodType:
            ...
            return (
             src(cls_adr[0]), cls_adr[1],
             '%s.%s' % (cls_adr[2], test.__name__))
        if isinstance(test, unittest.TestCase):
            if hasattr(test, '_FunctionTestCase__testFunc') or hasattr(test, '_testFunc'):
                try:
        ...
        if hasattr(test, '__class__') and test.__class__.__module__ not in ('__builtin__',
                                                                            'builtins'):
            ...
            if type(obj) == types.ModuleType:
                if isinstance(func, types.FunctionType):
                    ...
                    try:
                        ...
                        raise TypeError('Attribute %s of %r is not a python function. Only functions or callables may be used as fixtures.' % (
                         name, obj))
