...
# [GCC 8.4.0]
# Embedded file name: /cygdrive/c/Users/Nad/oneline/oneline/lib/lz4/nose-1.3.4-py2.7.egg/nose/suite.py
...
class LazySuite(unittest.TestSuite):
    ...
    _tests = property(_get_tests, _set_tests, None, 'Access the tests in this suite. Access is through a generator, so iteration may not be repeatable.')
...
class ContextSuite(LazySuite):
    ...
    was_setup = False
    was_torndown = False
    classSetup = ('setup_class', 'setup_all', 'setupClass', 'setupAll', 'setUpClass',
                  'setUpAll')
    classTeardown = ('teardown_class', 'teardown_all', 'teardownClass', 'teardownAll',
                     'tearDownClass', 'tearDownAll')
    moduleSetup = ('setup_module', 'setupModule', 'setUpModule', 'setup', 'setUp')
    moduleTeardown = ('teardown_module', 'teardownModule', 'tearDownModule', 'teardown',
                      'tearDown')
    packageSetup = ('setup_package', 'setupPackage', 'setUpPackage')
    packageTeardown = ('teardown_package', 'teardownPackage', 'tearDownPackage')

    def __init__(self, tests=(), context=None, factory=None, config=None, resultProxy=None, can_split=True):
    ...
    def setUp(self):
        ...
        if not self:
            ...
            if factory:
                ...
                if context not in factory.was_setup:
    ...
    def tearDown(self):
        ...
        if not self.was_setup or self.was_torndown:
            log.debug('No reason to teardown (was_setup? %s was_torndown? %s)' % (
            ...
            if factory:
                ...
                for ancestor in ancestors:
                    ...
                    if ancestor not in factory.was_setup:
    ...
    _tests = property(_get_wrapped_tests, LazySuite._set_tests, None, 'Access the tests in this suite. Tests are returned inside of a context wrapper.')
...
class ContextSuiteFactory(object):
    ...
    def findContext(self, tests):
        ...
        for test in tests:
            ...
            if context is None:
                ...
                raise MixedContextError('Tests with different contexts in same suite! %s != %s' % (
                 context, ctx))

        return context

    def makeSuite(self, tests, context, **kw):
        suite = self.suiteClass(tests, context=context, config=self.config, factory=self, resultProxy=self.resultProxy, **kw)
        if context is not None:
            self.suites.setdefault(context, []).append(suite)
            self.context.setdefault(suite, []).append(context)
            log.debug('suite %s has context %s', suite, getattr(context, '__name__', None))
    ...
    def mixedSuites(self, tests):
        ...
        if not tests:
            ...
            if context is not None:
                ancestors = [
                 context] + [ a for a in self.ancestry(context) ]
                for ancestor in ancestors:
                    common = [
                     suite]
                    remain = []
    ...
    def wrapTests(self, tests):
        ...
        for test in tests:
            ...
            if isinstance(test, Test) or isinstance(test, unittest.TestSuite):
                ...
                wrapped.append(Test(test, config=self.config, resultProxy=self.resultProxy))
...
class TestDir():
    ...
    def __init__(*arg, **kw):
        raise NotImplementedError('TestDir is not usable with nose 0.10. The class is present in nose.suite for backwards compatibility purposes but it may not be used.')
