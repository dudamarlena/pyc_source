...
# [GCC 8.4.0]
# Embedded file name: ol.py
# Compiled at: 2015-02-22 00:25:03
import argparse, json, bsonlib, operator, hashlib, ast, inspect, array, thread, weakref, string, memcache, socket, datetime, os, re, sys, cherrypy, uuid, random as _random, time as _time
try:
    import boto
except:
    pass
...
class server(object):
    ...
    def __init__(self, host=DEFAULTS['host'], port=DEFAULTS['port'], path=DEFAULTS['path']):
        """
        first obtain the 
        basic values found
        ...
        self.path = path
        self.ssl = False
        cherrypy.config.update({SERVERS['host']: host, SERVERS['port']: port, 
           SERVERS['path']: path})
    ...
    def start(self):
        ...
        if os.path.exists('/usr/local/oneline/modules/'):
            ...
            for i in files:
                ...
                config['/' + cname] = {'tools.websocket.on': True, 'request.module_md5': hashlib.md5(f).hexdigest(), 
                ...
                config['/' + module_name] = {'tools.websocket.on': True, 'request.module_md5': hashlib.md5(f).hexdigest(), 
                   'request.module_ctime': os.path.getmtime(os.path.realpath(i)), 
                   'request.module_object': module, 
                   'request.module_logger': logger(module_name), 
                   'request.module_uuid': uuid.uuid4().__str__(), 
...
class storage(object):
    ...
    def __init__(self, db_type='mysql', table='', host='localhost', username='root', password='', database='', port='', caller='', conf=''):
        global _OL_DB
        global _OL_TABLE
        import ol
        if conf == '':
        ...
        if table == '':
            try:
                ...
                if not has_config:
                    ...
                    try:
                        dbfolder = re.findall('db_omit\\s+\\=\\s+(.*)', f)[0]
                    except:
                        dbfolder = '/usr/bin/'
        ...
        if db_type in ('couchdb', ):
            ...
            if db_type not in ('mongodb', 'couchdb'):
                for i in tables:
                    table_name = i[0]
                    args = []
                    ...
                    if db_type in ('mysql', ):
                        ...
                        for j in schema:
                            ...
                            elif not has_id and j[3] == 'auto_increment':
                                args.append(Field(j[0]), type='id')
                                has_auto_increment = 1
                            else:
                    ...
                    if not has_id and not has_auto_increment:
                        if logger:
                            logger.append(dict(message="Could not find an auto increment key for table. If you're using this table make sure it has one!" % i[0], object=self.__str__()))
        ...
        if not cherrypy.engine.state == cherrypy.engine.states.STARTED:
            ...
            if self.memcache is not None:
                ...
                if m:
                    ...
                    return
            if len(re.findall('interop', message.__str__())) > 0:
            ...
            if self.config['upstream']:
                ...
                try:
                    ...
                    while True:
                        ...
                        if addr not in self.config.cluster:
                            continue
                        if addr in faddrs:
                            continue
                        faddrs.append(addr)
            ...
            try:
                ...
                m = dict(data=m, status='ok')
            except:
                m = dict(data=[], status='empty')
...
class geolocation(object):
    ...
    def run(self, message):
        ...
        if 'data' not in message.keys():
            ...
            for k in range(0, len(message['data'])):
                if float(message['data'][k]['lat']) >= lat and float(message['data'][k]['lat']) <= lat_ and float(message['data'][k]['lng']) >= lng and float(message['data'][k]['lng']) <= lng_:
                    if 'confidence' not in message['data'][k].keys():
                        message['data'][k]['confidence'] = 1
                    else:
                        message['data'][k]['confidence'] += 1
                else:
...
class sound(object):
    ...
    def run(self):
        ...
        if 'data' not in message.keys():
            if field not in getattr(_OL_TABLE, 'fields'):
            ...
            if field not in message['data'].keys():
                self.errors.append('This table did not have a: ' + field + ' field')
                raise NameError('ONELINE: this table does not have a ' + field + ' field')
            for i in message['data']:
                ...
                if 'confidence' not in message['data'][i].keys():
    ...
    def run(self, message):
        ...
        if 'data' not in message.keys():
            ...
            for i in range(0, amount):
                ...
                if 'confidence' not in message['data'][sel].keys():
...
class time(object):
    ...
    def run(self, message):
        ...
        if 'data' not in message.keys():
            ...
            queries.append(getattr(getattr(_OL_DB, _OL_TABLE), 'stime') <= getattr(getattr(_OL_DB, _OL_TABLE), 'etime'))
            query = reduce(lambda a, b: a & b, queries)
            rows = _OL_DB(query).select()
            return rows.as_list()
        else:
            for k in range(0, len(message['data'])):
                if int(message['data'][k]['stime']) >= start and int(message['data'][k]['etime']) <= end and int(message['data'][k]['stime']) <= int(message['data'][k]['etime']):
                    if 'confidence' not in message['data'][k].keys():
                        message['data'][k]['confidence'] = 1
                    else:
                        message['data'][k]['confidence'] += 1
    ...
    def run(self, message):
        ...
        for k, v in message['packet']['event'].iteritems():
            if k not in reserved:
                if k not in getattr(_OL_DB, _OL_TABLE).fields:
        ...
        if 'data' not in message.keys():
            ...
            for k in range(0, len(message['data'])):
                ...
                for i in opts:
                    ...
                    elif OPS[op](message['data'][k][i['key']], i['value']):
                        message['data'][k]['confidence'] += step
...
# global DEFAULTS ## Warning: Unused global
# global SETTINGS ## Warning: Unused global