...
# [GCC 8.4.0]
# Embedded file name: /var/www/oneline/lib/memcache/memcache.py
# Compiled at: 2015-03-28 13:51:26
"""
...
valid_key_chars_re = re.compile(b'[!-~\x80-\xff]+$')
...
class Client(local):
    ...
    def set_servers(self, servers):
        ...
        """
        self.servers = [ _Host(s, self.debug, dead_retry=self.dead_retry, socket_timeout=self.socket_timeout, flush_on_reconnect=self.flush_on_reconnect) for s in servers
                       ]
        self._init_buckets()
    ...
    def _incrdecr(self, cmd, key, delta):
        ...
        if not server:
            ...
            try:
                server.send_cmd(cmd)
                line = server.readline()
                if line == None or line.strip() == 'NOT_FOUND':
                    return
                return int(line)
            except socket.error as msg:
                if isinstance(msg, tuple):
    ...
    def _set(self, cmd, key, val, time, min_compress_len=0):
        ...
        def _unsafe_set():
            ...
            try:
                ...
                return server.expect('STORED', raise_exception=True) == 'STORED'
            except socket.error as msg:
                if isinstance(msg, tuple):
                    msg = msg[1]
                server.mark_dead(msg)
        ...
        if not server:
            ...
            def _unsafe_get():
                ...
                try:
                    ...
                    if cmd == 'gets':
                        rkey, flags, rlen, cas_id = self._expect_cas_value(server, raise_exception=True)
                        if rkey and self.cache_cas:
                            self.cas_ids[rkey] = cas_id
                    else:
                        rkey, flags, rlen = self._expectvalue(server, raise_exception=True)
                    if not rkey:
                        return
                    try:
    ...
    def _expect_cas_value(self, server, line=None, raise_exception=False):
        ...
        if line and line[:5] == 'VALUE':
            ...
            return (
             rkey, int(flags), int(len), int(cas_id))
        else:
...
class _Host(object):
    ...
    def __init__(self, host, debug=0, dead_retry=_DEAD_RETRY, socket_timeout=_SOCKET_TIMEOUT, flush_on_reconnect=0):
        self.dead_retry = dead_retry
        self.socket_timeout = socket_timeout
        self.debug = debug
        self.flush_on_reconnect = flush_on_reconnect
        ...
        if not m:
            m = re.match('^(?P<proto>inet6):\\[(?P<host>[^\\[\\]]+)\\](:(?P<port>[0-9]+))?$', host)
        if not m:
...
's applied """
    ...
    def recv(self, rlen):
        ...
        while len(buf) < rlen:
            ...
            buf += foo
            if not foo:
                raise _Error('Read %d bytes, expecting %d, read returned 0 length bytes' % (
                 len(buf), rlen))
