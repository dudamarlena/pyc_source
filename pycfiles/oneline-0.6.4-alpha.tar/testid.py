...
# [GCC 8.4.0]
# Embedded file name: /cygdrive/c/Users/Nad/oneline/oneline/lib/lz4/nose-1.3.4-py2.7.egg/nose/plugins/testid.py
...
class TestId(Plugin):
    ...
    def options(self, parser, env):
        ...
        parser.add_option('--id-file', action='store', dest='testIdFile', default='.noseids', metavar='FILE', help='Store test ids found in test runs in this file. Default is the file .noseids in the working directory.')
        parser.add_option('--failed', action='store_true', dest='failed', default=False, help='Run the tests that failed in the last test run.')
    ...
    def finalize(self, result):
        ...
        dump({'ids': ids, 'failed': self.failed, 
    ...
    def loadTestsFromNames(self, names, module=None):
        ...
        try:
            ...
            if self.ids:
                ...
                self.id = 1
            log.debug('Loaded test ids %s tests %s failed %s sources %s from %s', self.ids, self.tests, self.failed, self.source_names, self.idfile)
            fh.close()
        except IOError:
