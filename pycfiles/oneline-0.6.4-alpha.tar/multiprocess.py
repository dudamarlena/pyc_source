...
# [GCC 8.4.0]
# Embedded file name: /cygdrive/c/Users/Nad/oneline/oneline/lib/lz4/nose-1.3.4-py2.7.egg/nose/plugins/multiprocess.py
...
"""
import logging, os, sys, time, traceback, unittest, pickle, signal, nose.case
...
def _import_mp():
    ...
    try:
        ...
        warn('multiprocessing module is not available, multiprocess plugin cannot be used', RuntimeWarning)
...
class MultiProcess(Plugin):
    ...
    def options(self, parser, env):
        ...
        """
        parser.add_option('--processes', action='store', default=env.get('NOSE_PROCESSES', 0), dest='multiprocess_workers', metavar='NUM', help='Spread test run among this many processes. Set a number equal to the number of processors or cores in your machine for best results. Pass a negative number to have the number of processes automatically set to the number of cores. Passing 0 means to disable parallel testing. Default is 0 unless NOSE_PROCESSES is set. [NOSE_PROCESSES]')
        parser.add_option('--process-timeout', action='store', default=env.get('NOSE_PROCESS_TIMEOUT', 10), dest='multiprocess_timeout', metavar='SECONDS', help='Set timeout for return of results from each test runner process. Default is 10. [NOSE_PROCESS_TIMEOUT]')
    ...
    def prepareTestRunner(self, runner):
        ...
        """
        return MultiProcessTestRunner(stream=runner.stream, verbosity=self.config.verbosity, config=self.config, loaderClass=self.loaderClass)
        ...
        try:
            while tasks:
                ...
                try:
                    ...
                    log.debug('Timed out with %s tasks pending (empty testQueue=%r): %s', len(tasks), testQueue.empty(), str(tasks))
                    any_alive = False
    ...
    def nextBatch(self, test):
        if hasattr(test, 'context'):
            if not getattr(test.context, '_multiprocess_', True):
                return
        if isinstance(test, ContextSuite) and test.hasFixtures(self.checkCanSplit) or not getattr(test, 'can_split', True) or not isinstance(test, unittest.TestSuite):
            if isinstance(test, ContextSuite):
                contained = list(test)
                if len(contained) == 1 and getattr(contained[0], 'context', None) == test.context:
                    test = contained[0]
            yield test
        else:
            for case in test:
...
def runner(ix, testQueue, resultQueue, currentaddr, currentstart, keyboardCaught, shouldStop, loaderClass, resultClass, config):
    try:
        try:
            return __runner(ix, testQueue, resultQueue, currentaddr, currentstart, keyboardCaught, shouldStop, loaderClass, resultClass, config)
...
def __runner(ix, testQueue, resultQueue, currentaddr, currentstart, keyboardCaught, shouldStop, loaderClass, resultClass, config):
