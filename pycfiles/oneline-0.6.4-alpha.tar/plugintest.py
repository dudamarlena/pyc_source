...
# [GCC 8.4.0]
# Embedded file name: /cygdrive/c/Users/Nad/oneline/oneline/lib/lz4/nose-1.3.4-py2.7.egg/nose/plugins/plugintest.py
...
class PluginTester(object):
    ...
    def _execPlugin(self):
        ...
        suite = None
        stream = Buffer()
        conf = Config(env=self.env, stream=stream, plugins=PluginManager(plugins=self.plugins))
...
def remove_stack_traces(out):
    ...
    for block in blankline_separated_blocks(out):
        blocks.append(traceback_re.sub('\\g<hdr>\\n...\\n\\g<exception>\\g<msg>', block))
...
def simplify_warnings(out):
    ...
    return warn_re.sub('\\g<category>: \\g<detail>', out)


def remove_timings(out):
    return re.sub('Ran (\\d+ tests?) in [0-9.]+s', 'Ran \\1 in ...s', out)
