...
# [GCC 8.4.0]
# Embedded file name: /cygdrive/c/Users/Nad/oneline/oneline/lib/lz4/nose-1.3.4-py2.7.egg/nose/plugins/manager.py
...
__all__ = [
 'DefaultPluginManager', 'PluginManager', 'EntryPointPluginManager',
...
class PluginProxy(object):
    ...
    def makeCall(self, call):
        if call == 'loadTestsFromNames':
            ...
            if getattr(meth, 'chainable', False):
                return self.chain
            return self.simple
    ...
    def chain(self, *arg, **kw):
        ...
        """
        result = None
        static = [ a for static, a in zip(getattr(self.method, 'static_args', []), arg) if static
                 ]
...
class PluginManager(object):
    ...
    def addPlugin(self, plug):
        ...
        self._plugins[:] = [ p for p in self._plugins if getattr(p, 'name', None) != new_name
                           ]
    ...
    plugins = property(_get_plugins, _set_plugins, None, 'Access the list of plugins managed by\n                       this plugin manager')
...
class ZeroNinePlugin:
    ...
    def addError(self, test, err):
        ...
        if issubclass(ec, DeprecatedTest):
