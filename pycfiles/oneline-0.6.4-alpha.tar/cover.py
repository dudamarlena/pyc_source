...
# [GCC 8.4.0]
# Embedded file name: /cygdrive/c/Users/Nad/oneline/oneline/lib/lz4/nose-1.3.4-py2.7.egg/nose/plugins/cover.py
...
"""
import logging, re, sys, StringIO
...
class Coverage(Plugin):
    ...
    def options(self, parser, env):
        ...
        parser.add_option('--cover-min-percentage', action='store', dest='cover_min_percentage', default=env.get('NOSE_COVER_MIN_PERCENTAGE'), help='Minimum percentage of coverage for tests to pass [NOSE_COVER_MIN_PERCENTAGE]')
        ...
        if conf.worker:
            ...
            if options.cover_packages:
                if isinstance(options.cover_packages, (list, tuple)):
                    ...
                    cover_packages = [
                     options.cover_packages]
                for pkgs in [ tolist(x) for x in cover_packages ]:
                    self.coverPackages.extend(pkgs)

            self.coverInclusive = options.cover_inclusive
            if self.coverPackages:
                log.info('Coverage report will include only packages: %s', self.coverPackages)
            ...
            if self.enabled:
                ...
                self.coverInstance = coverage.coverage(auto_data=False, branch=self.coverBranches, data_suffix=None, source=self.coverPackages)
            return
    ...
    def report(self, stream):
        ...
        if self.coverMinPercentage:
            ...
            multiPackageRe = '-------\\s\\w+\\s+\\d+\\s+\\d+(?:\\s+\\d+\\s+\\d+)?\\s+(\\d+)%\\s+\\d*\\s{0,1}$'
            singlePackageRe = '-------\\s[\\w./]+\\s+\\d+\\s+\\d+(?:\\s+\\d+\\s+\\d+)?\\s+(\\d+)%(?:\\s+[-\\d, ]+)\\s{0,1}$'
    ...
    def wantModuleCoverage(self, name, module):
        ...
        if self.coverPackages:
            for package in self.coverPackages:
                if re.findall('^%s\\b' % re.escape(package), name) and (self.coverTests or not self.conf.testMatch.search(name)):
