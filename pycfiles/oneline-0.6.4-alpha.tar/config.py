...
# [GCC 8.4.0]
# Embedded file name: /cygdrive/c/Users/Nad/oneline/oneline/lib/lz4/nose-1.3.4-py2.7.egg/nose/config.py
# Compiled at: 2014-09-06 21:58:19
import logging, optparse, os, re, sys, ConfigParser
...
exe_allowed_platforms = (
 'win32', 'cli')
filterwarnings('always', category=DeprecationWarning, module='(.*\\.)?nose\\.config')
    ...
    def _applyConfigurationToValues(self, parser, config, values):
        for name, value, filename in config:
            ...
            try:
                ...
                self._file_error('Error reading config file %r: no such option %r' % (
                 filename, exc.name), name=name, filename=filename)
            except optparse.OptionValueError as exc:
                msg = str(exc).replace('--' + name, repr(name), 1)
                self._file_error('Error reading config file %r: %s' % (
                 filename, msg), name=name, filename=filename)
...
class Config(object):
    ...
    def __init__(self, **kw):
        self.env = env = kw.pop('env', {})
        self.args = ()
        self.testMatchPat = env.get('NOSE_TESTMATCH', '(?:^|[\\b_\\.%s-])[Tt]est' % os.sep)
        ...
        self.exclude = None
        self.getTestCaseNamesCompat = False
        self.includeExe = env.get('NOSE_INCLUDE_EXE', sys.platform in exe_allowed_platforms)
        self.ignoreFilesDefaultStrings = ['^\\.',
         '^_',
         '^setup\\.py$']
    ...
    def _parseArgs(self, argv, cfg_files):
        ...
        def warn_sometimes(msg, name=None, filename=None):
            if hasattr(self.plugins, 'excludedOption') and self.plugins.excludedOption(name):
                msg = 'Option %r in config file %r ignored: excluded by runtime environment' % (
                 name, filename)
                warn(msg, RuntimeWarning)
            else:
                raise ConfigError(msg)

        parser = ConfiguredDefaultsOptionParser(self.getParser(), self.configSection, file_error=warn_sometimes)
    ...
    def configureLogging(self):
        ...
        if self.debugLog:
            ...
            for h in logger.handlers:
                if type(h) == logging.FileHandler and h.baseFilename == debugLogAbsPath:
                    found = True

        else:
            for h in logger.handlers:
                if type(h) == logging.StreamHandler and h.stream == self.logStream:
                    found = True
    ...
    def getParser(self, doc=None):
        ...
        if self.parser:
            ...
            parser.add_option('-V', '--version', action='store_true', dest='version', default=False, help='Output nose version and exit')
            parser.add_option('-p', '--plugins', action='store_true', dest='showPlugins', default=False, help='Output list of available plugins and exit. Combine with higher verbosity for greater detail')
            parser.add_option('-v', '--verbose', action='count', dest='verbosity', default=self.verbosity, help='Be more verbose. [NOSE_VERBOSE]')
            parser.add_option('--verbosity', action='store', dest='verbosity', metavar='VERBOSITY', type='int', help='Set verbosity; --verbosity=2 is the same as -v')
            parser.add_option('-q', '--quiet', action='store_const', const=0, dest='verbosity', help='Be less verbose')
            parser.add_option('-c', '--config', action='append', dest='files', metavar='FILES', help='Load configuration from config file(s). May be specified multiple times; in that case, all config files will be loaded and combined')
            parser.add_option('-w', '--where', action='append', dest='where', metavar='WHERE', help='Look for tests in this directory. May be specified multiple times. The first directory passed will be used as the working directory, in place of the current working directory, which is the default. Others will be added to the list of tests to execute. [NOSE_WHERE]')
            parser.add_option('--py3where', action='append', dest='py3where', metavar='PY3WHERE', help="Look for tests in this directory under Python 3.x. Functions the same as 'where', but only applies if running under Python 3.x or above.  Note that, if present under 3.x, this option completely replaces any directories specified with 'where', so the 'where' option becomes ineffective. [NOSE_PY3WHERE]")
            parser.add_option('-m', '--match', '--testmatch', action='store', dest='testMatch', metavar='REGEX', help='Files, directories, function names, and class names that match this regular expression are considered tests.  Default: %s [NOSE_TESTMATCH]' % self.testMatchPat, default=self.testMatchPat)
            parser.add_option('--tests', action='store', dest='testNames', default=None, metavar='NAMES', help='Run these tests (comma-separated list). This argument is useful mainly from configuration files; on the command line, just pass the tests to run as additional arguments with no switch.')
            parser.add_option('-l', '--debug', action='store', dest='debug', default=self.debug, help='Activate debug logging for one or more systems. Available debug loggers: nose, nose.importer, nose.inspector, nose.plugins, nose.result and nose.selector. Separate multiple names with a comma.')
            parser.add_option('--debug-log', dest='debugLog', action='store', default=self.debugLog, metavar='FILE', help='Log debug messages to this file (default: sys.stderr)')
            parser.add_option('--logging-config', '--log-config', dest='loggingConfig', action='store', default=self.loggingConfig, metavar='FILE', help='Load logging config from this file -- bypasses all other logging config settings.')
            parser.add_option('-I', '--ignore-files', action='append', dest='ignoreFiles', metavar='REGEX', help='Completely ignore any file that matches this regular expression. Takes precedence over any other settings or plugins. Specifying this option will replace the default setting. Specify this option multiple times to add more regular expressions [NOSE_IGNORE_FILES]')
            parser.add_option('-e', '--exclude', action='append', dest='exclude', metavar='REGEX', help="Don't run tests that match regular expression [NOSE_EXCLUDE]")
            parser.add_option('-i', '--include', action='append', dest='include', metavar='REGEX', help='This regular expression will be applied to files, directories, function names, and class names for a chance to include additional tests that do not match TESTMATCH.  Specify this option multiple times to add more regular expressions [NOSE_INCLUDE]')
            parser.add_option('-x', '--stop', action='store_true', dest='stopOnError', default=self.stopOnError, help='Stop running tests after the first error or failure')
            parser.add_option('-P', '--no-path-adjustment', action='store_false', dest='addPaths', default=self.addPaths, help="Don't make any changes to sys.path when loading tests [NOSE_NOPATH]")
            parser.add_option('--exe', action='store_true', dest='includeExe', default=self.includeExe, help='Look for tests in python modules that are executable. Normal behavior is to exclude executable modules, since they may not be import-safe [NOSE_INCLUDE_EXE]')
            parser.add_option('--noexe', action='store_false', dest='includeExe', help='DO NOT look for tests in python modules that are executable. (The default on the windows platform is to do so.)')
            parser.add_option('--traverse-namespace', action='store_true', default=self.traverseNamespace, dest='traverseNamespace', help='Traverse through all path entries of a namespace package')
            parser.add_option('--first-package-wins', '--first-pkg-wins', '--1st-pkg-wins', action='store_true', default=False, dest='firstPackageWins', help="nose's importer will normally evict a package from sys.modules if it sees a package with the same name in a different location. Set this option to disable that behavior.")
            parser.add_option('--no-byte-compile', action='store_false', default=True, dest='byteCompile', help='Prevent nose from byte-compiling the source into .pyc files while nose is scanning for and running tests.')
            self.plugins.loadPlugins()
