...
# [GCC 8.4.0]
# Embedded file name: /cygdrive/c/Users/Nad/oneline/oneline/lib/lz4/nose-1.3.4-py2.7.egg/nose/plugins/errorclass.py
...
class ErrorClassPlugin(Plugin):
    ...
    def patchResult(self, result):
        ...
        result._orig_addError, result.addError = result.addError, add_error_patch(result)
        result._orig_wasSuccessful, result.wasSuccessful = result.wasSuccessful, wassuccessful_patch(result)
        if hasattr(result, 'printErrors'):
            result._orig_printErrors, result.printErrors = result.printErrors, print_errors_patch(result)
        if hasattr(result, 'addSkip'):
            result._orig_addSkip, result.addSkip = result.addSkip, add_skip_patch(result)
        result.errorClasses = {}
