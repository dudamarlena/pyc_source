...
# [GCC 8.4.0]
# Embedded file name: /cygdrive/c/Users/Nad/oneline/oneline/lib/lz4/nose-1.3.4-py2.7.egg/nose/plugins/doctests.py
...
from nose.util import anyp, getpackage, test_address, resolve_name, src, tolist, isproperty
try:
    from cStringIO import StringIO
except ImportError:
    from StringIO import StringIO

import sys, __builtin__ as builtin_mod
...
class Doctest(Plugin):
    ...
    def options(self, parser, env):
        ...
        parser.add_option('--doctest-tests', action='store_true', dest='doctest_tests', default=env.get('NOSE_DOCTEST_TESTS'), help='Also look for doctests in test modules. Note that classes, methods and functions should have either doctests or non-doctest tests, not both. [NOSE_DOCTEST_TESTS]')
        parser.add_option('--doctest-extension', action='append', dest='doctestExtension', metavar='EXT', help='Also look for doctests in files with this extension [NOSE_DOCTEST_EXTENSION]')
        parser.add_option('--doctest-result-variable', dest='doctest_result_var', default=env.get('NOSE_DOCTEST_RESULT_VAR'), metavar='VAR', help="Change the variable name set to the result of the last interpreter command from the default '_'. Can be used to avoid conflicts with the _() function used for text translation. [NOSE_DOCTEST_RESULT_VAR]")
        parser.add_option('--doctest-fixtures', action='store', dest='doctestFixtures', metavar='SUFFIX', help='Find fixtures for a doctest file in module with this name appended to the base name of the doctest file')
        parser.add_option('--doctest-options', action='append', dest='doctestOptions', metavar='OPTIONS', help='Specify options to pass to doctest. ' + "Eg. '+ELLIPSIS,+NORMALIZE_WHITESPACE'")
        ...
        if options.doctestOptions:
            ...
            for flag in flags:
                if not flag or flag[0] not in '+-':
                    raise ValueError('Must specify doctest options with starting ' + "'+' or '-'.  Got %s" % (flag,))
    ...
    def loadTestsFromModule(self, module):
        ...
        for test in tests:
            ...
            cases.append(DocTestCase(test, optionflags=self.optionflags, result_var=self.doctest_result_var))
    ...
    def loadTestsFromFile(self, filename):
        ...
        if self.extension and anyp(filename.endswith, self.extension):
            ...
            test = parser.get_doctest(doc, globs=globs, name=name, filename=filename, lineno=0)
            if test.examples:
                case = DocFileCase(test, optionflags=self.optionflags, setUp=getattr(fixture_context, 'setup_test', None), tearDown=getattr(fixture_context, 'teardown_test', None), result_var=self.doctest_result_var)
                if fixture_context:
        ...
        if doctests:
            for test in doctests:
                if len(test.examples) == 0:
                    continue
                yield DocTestCase(test, obj=obj, optionflags=self.optionflags, result_var=self.doctest_result_var)

    def matches(self, name):
        if name == '__init__.py':
            return False
        else:
            return (self.doctest_tests or not self.conf.testMatch.search(name) or self.conf.include and filter(None, [ inc.search(name) for inc in self.conf.include
                                                                                                                     ])) and (not self.conf.exclude or not filter(None, [ exc.search(name) for exc in self.conf.exclude
                                                                                                                                                                        ]))
    ...
    def wantFile(self, file):
        ...
        if file.endswith('.py'):
            ...
            if self.extension and anyp(file.endswith, self.extension) and (not self.conf.exclude or not filter(None, [ exc.search(file) for exc in self.conf.exclude
                                                                                                                     ])):
                return True
            return
...
class DocTestCase(doctest.DocTestCase):
    ...
    def __init__(self, test, optionflags=0, setUp=None, tearDown=None, checker=None, obj=None, result_var='_'):
        ...
        self._nose_obj = obj
        super(DocTestCase, self).__init__(test, optionflags=optionflags, setUp=setUp, tearDown=tearDown, checker=checker)
...
class DocFileCase(doctest.DocFileCase):
    ...
    def __init__(self, test, optionflags=0, setUp=None, tearDown=None, checker=None, result_var='_'):
        self._result_var = result_var
        super(DocFileCase, self).__init__(test, optionflags=optionflags, setUp=setUp, tearDown=tearDown, checker=None)
        return
