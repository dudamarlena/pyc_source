...
# [GCC 8.4.0]
# Embedded file name: /cygdrive/c/Users/Nad/oneline/oneline/lib/lz4/nose-1.3.4-py2.7.egg/nose/selector.py
...
__all__ = [
 'Selector', 'defaultSelector', 'TestAddress']
...
class Selector(object):
    ...
    def matches(self, name):
        ...
        """
        return (self.match.search(name) or self.include and filter(None, [ inc.search(name) for inc in self.include ])) and (not self.exclude or not filter(None, [ exc.search(name) for exc in self.exclude ]))
    ...
    def wantDirectory(self, dirname):
        ...
        if ispackage(dirname):
            wanted = not self.exclude or not filter(None, [ exc.search(tail) for exc in self.exclude ])
        else:
            wanted = self.matches(tail) or self.config.srcDirs and tail in self.config.srcDirs
    ...
    def wantModule(self, module):
        ...
        if declared is not None:
            ...
            wanted = self.matches(module.__name__.split('.')[(-1)]) or module.__name__ == '__main__'
...
class TestAddress(object):
    ...
    def __init__(self, name, workingDir=None):
        ...
        log.debug('Test name %s resolved to file %s, module %s, call %s', name, self.filename, self.module, self.call)
        ...
        log.debug('Final resolution of test name %s: file %s module %s call %s', name, self.filename, self.module, self.call)
        return
