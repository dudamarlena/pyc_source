...
# [GCC 8.4.0]
# Embedded file name: /cygdrive/c/Users/Nad/oneline/oneline/lib/lz4/nose-1.3.4-py2.7.egg/nose/loader.py
...
import logging, os, sys, unittest, types
...
from nose.util import func_lineno, getpackage, isclass, isgenerator, ispackage, regex_last_key, resolve_name, transplant_func, transplant_class, test_address
    ...
    * Support easy extensions of or changes to that behavior through plugins
    ...
    def loadTestsFromFile(self, filename):
        ...
        try:
            ...
            return self.suiteClass([
             Failure(exc[0], exc[1], exc[2], address=(
              filename, None, None))])
            ...
            try:
                for test in g():
                    ...
                    if ismethod(test_func):
                        ...
                        yield Failure(TypeError, '%s is not a callable or method' % test_func)
    ...
    def loadTestsFromModule(self, module, path=None, discovered=False):
        ...
        if not discovered or self.selector.wantModule(module):
            ...
            tests = map(lambda t: self.makeTest(t, parent=module), test_classes + test_funcs)
        module_paths = getattr(module, '__path__', [])
        if path:
            path = os.path.realpath(path)
        for module_path in module_paths:
            log.debug('Load tests from module path %s?', module_path)
            log.debug('path: %s os.path.realpath(%s): %s', path, module_path, os.path.realpath(module_path))
            if self.config.traverseNamespace or not path or os.path.realpath(module_path).startswith(path):
    ...
    def loadTestsFromName(self, name, module=None, discovered=False):
        ...
        if plug_tests:
            ...
            if module:
                ...
                if isclass(parent) and getattr(parent, '__module__', None) != module.__name__ and not isinstance(obj, Failure):
                ...
                try:
                    if addr.filename is None:
                        ...
                        try:
                            module = self.importer.importFromPath(addr.filename, addr.module)
                        finally:
                ...
                return self.loadTestsFromModule(module, addr.filename, discovered=discovered)
            elif addr.filename:
                path = addr.filename
                if addr.call:
                    ...
                    if package is None:
                        return suite([
                         Failure(ValueError, "Can't find callable %s in file %s: file is not a python module" % (
                          addr.call, path), address=addr.totuple())])
                ...
                return suite([
                 Failure(OSError, 'No such file %s' % path, address=addr.totuple())])
            else:
                return suite([
                 Failure(ValueError, 'Unresolvable test name %s' % name, address=addr.totuple())])
            return
    ...
    def loadTestsFromTestCase(self, testCaseClass):
        ...
        cases.extend([ case for case in super(TestLoader, self).loadTestsFromTestCase(testCaseClass)
                     ])
    ...
    def loadTestsFromTestClass(self, cls):
        ...
        cases = [ self.makeTest(getattr(cls, case), cls) for case in filter(wanted, dir(cls))
                ]
