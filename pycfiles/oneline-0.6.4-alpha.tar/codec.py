...
# [GCC 8.4.0]
# Embedded file name: bson/codec.py
# Compiled at: 2011-07-10 20:51:08
"""
Base codec functions for bson.
"""
import struct, cStringIO, calendar, pytz
...
def decode_cstring(data, base):
    ...
    return (
     base + length, data[base:base + length - 1].decode('utf8'))
...
def decode_double(data, base):
    return (
     base + 8, struct.unpack('<d', data[base:base + 8])[0])


ELEMENT_TYPES = {1: 'double', 
   2: 'string', 
   3: 'document', 
   4: 'array', 
   5: 'binary', 
...
def encode_UTCdatetime_element(name, value):
    ...
    return '\t' + encode_cstring(name) + struct.pack('<q', value)
