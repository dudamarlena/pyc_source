...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\lib_excel_package.py
# Compiled at: 2018-01-08 15:31:58
import openpyxl, lib_common
...
def get_worksheet_2(wb, ws_title):
    ws = wb.get_sheet_by_name(ws_title)
    return ws
...
def column_letter_to_number(col_letter):
    col_number = openpyxl.cell.column_index_from_string(col_letter)
    return col_number


def column_number_to_letter(col_number):
    col_letter = openpyxl.cell.get_column_letter(col_number)
    return col_letter
...
def set_border(ws, col_start, row_start, col_dim, row_dim, sides={}, edges={}):
    for n_row in range(row_start, row_start + row_dim):
        for n_col in range(col_start, col_start + col_dim):
            border = openpyxl.styles.Border(left=ws.cell(column=n_col, row=n_row).border.left, right=ws.cell(column=n_col, row=n_row).border.right, top=ws.cell(column=n_col, row=n_row).border.top, bottom=ws.cell(column=n_col, row=n_row).border.bottom)
            if sides != {}:
                if sides['left'] != 'none':
                    border.left = openpyxl.styles.Side(border_style=sides['left'], color='FF000000')
                if sides['right'] != 'none':
                    border.right = openpyxl.styles.Side(border_style=sides['right'], color='FF000000')
                if sides['top'] != 'none':
                    border.top = openpyxl.styles.Side(border_style=sides['top'], color='FF000000')
                if sides['bottom'] != 'none':
                    border.bottom = openpyxl.styles.Side(border_style=sides['bottom'], color='FF000000')
            if edges != {}:
                if n_row == row_start:
                    if edges['top'] != 'none':
