...
# [GCC 8.4.0]
# Embedded file name: /home/bruno/dev/Archive/archive/test/__init__.py
# Compiled at: 2012-02-05 19:09:25
...
class ArchiveTester(BaseArchiveTester):
    ...
    def test_extract_function_fileobject_noname(self):
        f = open(self.archive_path)
        del f.name
        extract(f, self.tmpdir)
        self.check_files(self.tmpdir)
...
class EvilArchiveTester(BaseArchiveTester):
    ...
    def test_extract_method(self):
        self.assertRaises(UnsafeArchive, Archive(self.archive).extract, self.tmpdir, safe=True)

    def test_extract_method_fileobject(self):
        f = open(self.archive)
        self.assertRaises(UnsafeArchive, Archive(f).extract, self.tmpdir, safe=True)

    def test_extract_method_no_to_path(self):
        os.chdir(self.tmpdir)
        self.assertRaises(UnsafeArchive, Archive(self.archive_path).extract, safe=True)

    def test_extract_function(self):
        self.assertRaises(UnsafeArchive, extract, self.archive_path, self.tmpdir, safe=True)
