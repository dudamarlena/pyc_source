...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/clue/secure/auth.py
# Compiled at: 2008-07-05 06:03:17
...
class ResponseWrapper(object):
    ...
    def __call__(self):
        ...
        def mock_start_response(status, headers, exc_info=None, res=res, write=sb.write):
...
class AuthFilter(object):
    ...
    def __call__(self, environ, start_response):
        ...
        if pathinfo.endswith('/logout'):
            if 'REMOTE_USER' not in environ or environ.get('clue.auth.logged_out', False) or query.get(LOGIN_QS):
...
def make_filter(app, global_conf, **kw):
    ...
    authform = form.FormPlugin(LOGIN_QS, rememberer_name='auth_tkt', formcallable=login_form)
    authform.classifications = {whoifaces.IIdentifier: ['browser'], whoifaces.IChallenger: [
                             'browser']}
    ...
    mdproviders = []
    return middleware.PluggableAuthenticationMiddleware(authfilter, identifiers, authenticators, challengers, mdproviders, classifiers.default_request_classifier, challenge_decider)
