...
# [GCC 8.4.0]
# Embedded file name: /Users/jannis/Documents/code/rtr-supercell/supercell/env3/lib/python3.6/site-packages/supercell/requesthandler.py
# Compiled at: 2019-03-29 06:21:06
# Size of source mod 2**32: 13658 bytes
from __future__ import absolute_import, division, print_function, with_statement
from datetime import datetime
import json, logging, time, inspect
...
from tornado.web import RequestHandler as rq, HTTPError, _has_stream_request_body
...
class RequestHandler(rq):
    ...
    def _request_summary(self):
        ...
        return '{method} {uri} (ip:{r_ip}, r_id:{r_id})'.format(method=(self.request.method),
    ...
    def _check_consumer(self):
        ...
        if verb in ('patch', 'post', 'put') and 'Content-Type' in headers:
            try:
                (model_type, validate), consumer_class = ConsumerBase.map_consumer(headers['Content-Type'], self)
                ...
                raise HTTPError(400, reason=(json.dumps(escape_contents(error_messages(e)))))
    ...
    def _add_cache_headers(self):
        ...
        if verb in ('get', 'head'):
            ...
            if cache_config:
                self.set_header('Cache-Control', compute_cache_header(cache_config))
    ...
    def _provide_result(self, verb, headers, result):
        ...
        if isinstance(result, ReturnInformationT):
            ...
            if result.message:
                ...
                isinstance(result, Model) or self.logger.error('Returning a non-model is not supported')
                raise HTTPError(500)
        else:
            try:
                provider_class, provider_config = ProviderBase.map_provider((headers.get('Accept', '')),
                  self, allow_default=True)
            except NoProviderFound:
                raise HTTPError(406, reason='Can not produce acceptable response')
    ...
    def write_error(self, status_code, **kwargs):
        if not ProviderBase.has_provider(self):
            ...
            return
        try:
            provider_class, _ = ProviderBase.map_provider((self.request.headers.get('Accept', '')),
              self, allow_default=True)
    ...
    def load_model_from_arguments(self, model_cls, validate=True, **kwargs):
        ...
        arguments = {key:self.get_arguments(key) for key in self.request.query_arguments}
        raw_data.update({key:(values if isinstance(fields.get(key, None), ListType) else values[0]) for key, values in arguments.items() if values})
