...
# [GCC 8.4.0]
# Embedded file name: /Users/jannis/Documents/code/rtr-supercell/supercell/env3/lib/python3.6/site-packages/supercell/environment.py
...
class Application(_TAPP):
    __doc__ = 'Overwrite :class:`tornado.web.Application` in order to give access to\n    environment and configuration instances.'
    ...
    def log_request(self, handler):
        if self.config is not None:
            if self.config.suppress_health_check_log:
...
class Environment(object):
    __doc__ = 'Environment for **supercell** processes.\n    '
    ...
    def add_handler(self, path, handler_class, init_dict=None, name=None, host_pattern='.*$', cache=None, expires=None):
        ...
        """
        if not not self._finalized:
            raise AssertionError('Do not change the environment at runtime')
        else:
            handler = Handler(host_pattern=host_pattern, path=path, handler_class=handler_class,
              init_dict=init_dict,
              name=name,
              cache=cache,
              expires=expires)
            ...
            assert expires and isinstance(expires, timedelta), 'expires not a timedelta'
    ...
    def add_managed_object(self, name, instance):
        ...
        elif not name not in self._managed_objects:
            raise AssertionError
    ...
    def add_health_check(self, name, check):
        ...
        if not not self._finalized:
            raise AssertionError
        elif not name not in self._health_checks:
            raise AssertionError
    ...
    def get_application(self, config=None):
        ...
        if not hasattr(self, '_app'):
            self._app = Application(self, config, **self.tornado_settings)
            ...
            for handler in self._handlers:
                if handler.init_dict:
                    spec = (
                     handler.path, handler.handler_class,
                     handler.init_dict)
                else:
                    spec = (
                     handler.path, handler.handler_class)
