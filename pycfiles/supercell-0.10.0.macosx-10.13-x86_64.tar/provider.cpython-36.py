...
# [GCC 8.4.0]
# Embedded file name: /Users/jannis/Documents/code/rtr-supercell/supercell/env3/lib/python3.6/site-packages/supercell/provider.py
        ...
        for ctype, params, q in accept:
            if ctype not in handler._PROD_CONTENT_TYPES:
                ...
                if c not in handler._PROD_CONTENT_TYPES[c.content_type]:
                    ...
                    known_types = [t for t in ProviderMeta.KNOWN_CONTENT_TYPES[c.content_type] if t[0] == c]
...
class JsonProvider(ProviderBase):
    __doc__ = 'Default `application/json` provider.'
    ...
    def provide(self, model, handler, **kwargs):
        ...
        try:
            ...
            raise HTTPError(500, reason=(json.dumps({'result_model': escape_contents(error_messages(e))})))
...
class TornadoTemplateProvider(ProviderBase):
    __doc__ = 'Default provider for `text/html`.'
    ...
    def error(self, status_code, message, handler):
        ...
        .. seealso:: :py:mod:`supercell.api.provider.ProviderBase.error`
        """
        handler.finish('<html><title>%(code)d: %(message)s</title><body>%(code)d: %(message)s</body></html>' % {'code':status_code, 
         'message':message})