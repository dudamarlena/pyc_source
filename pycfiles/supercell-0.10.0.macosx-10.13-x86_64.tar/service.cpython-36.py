...
# [GCC 8.4.0]
# Embedded file name: /Users/jannis/Documents/code/rtr-supercell/supercell/env3/lib/python3.6/site-packages/supercell/service.py
...
import os, signal, socket, sys, time, tornado.options
...
define('logfile', default='root-%(pid)s.log', help='Filename to store the logs. If the name contains "%%(pid)s" it will be replaced with the pid. If the filename equals to "-" the logs are being written to stdout')
define('loglevel', default='INFO', help='Log level')
define('logformat', default='%(asctime)s [%(levelname)s] %(hostname)s %(name)s:  %(message)s',
  help='format string for logging formatter')
define('suppress_health_check_log', default=False, help='Suppress the access logging for the system health check.')
...
define('max_grace_seconds', default=3, help='Wait up to this amount of seconds to finish requests before shutdown')
define('debug', default=False, help='If set, Tornado is started in debug mode')
define('show_config_file_order', default=False, help='Show the order of config files to be parsed')
define('show_config_name', default=False, help='Show the local config file name')
define('show_config', default=False, help='Show the effective configuration')

class Service(object):
    __doc__ = 'Main service implementation managing the\n    :class:`tornado.web.Application` and taking care of configuration.'
    ...
    def main(self, with_signals=True):
        ...
        if self.config.socketfd:
            sock = socket.fromfd(int(self.config.socketfd), socket.AF_INET, socket.SOCK_STREAM)
    ...
    def shutdown(self):
        ...
        def stop_loop():
            now = time.time()
            if now < dl:
                if self._has_callbacks(io_loop):
    ...
    def initialize_logging(self):
        ...
        hdlrs = [h for h in root.handlers if isinstance(h, clazz)]
        if len(hdlrs) == 0:
    ...
    def _has_callbacks(self, io_loop):
        ...
        """
        if hasattr(tornado.platform, 'asyncio'):
            if isinstance(io_loop, tornado.platform.asyncio.AsyncIOMainLoop):
