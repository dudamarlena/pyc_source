...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/maguey/api.py
# Compiled at: 2016-02-05 17:26:36
...
class Meta(object):
    ...
    def list(token, uuid=None, query=None):
        ...
        if query:
            ...
            parameters = {'q': query}
        url = ('{base}/{method}/{uuid}').format(base=AGAVE_TENANT_BASEURL, method=method_path, uuid=uuid if uuid else '')
    ...
    def delete(token, uuid):
        ...
        if not uuid or uuid == '':
            raise ValueError('uuid is a required parameter')
        method_path = 'meta/v2/data'
        url = ('{base}/{method}/{uuid}').format(base=AGAVE_TENANT_BASEURL, method=method_path, uuid=uuid)
        headers = {'Authorization': ('Bearer {}').format(token)}
