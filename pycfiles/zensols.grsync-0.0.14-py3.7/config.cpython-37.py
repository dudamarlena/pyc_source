...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/grsync/config.py
...
class AppConfig(YamlConfig):
    ...
    def __init__(self, config_file=None, default_vars=None):
        super(AppConfig, self).__init__(config_file,
          delimiter='^', default_vars=default_vars)
    ...
    def _split_profiles(profile_str):
        return re.split('\\s*,\\s*', profile_str)
    ...
    def _iterate_objects(self, profile):
        ...
        if opts is None:
            if profile == 'default':
                opts = ()
        if opts is None:
            logger.warning(f"no such profile for objects: {profile} for path {path}" + '--maybe entries exist in other profiles')
            opts = ()
