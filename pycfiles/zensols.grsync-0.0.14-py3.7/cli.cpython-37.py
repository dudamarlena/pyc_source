...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/grsync/cli.py
# Compiled at: 2020-04-26 00:22:04
# Size of source mod 2**32: 6610 bytes
import logging
from zensols.cli import OneConfPerActionOptionsCliEnv
from zensols.grsync import AppConfig, DistManager, RepoSpec
...
class DistManagerCli(object):
    ...
    def __init__(self, config=None, dist_dir=None, target_dir=None, move_dir=None, wheel_dependency='zensols.grsync', profiles=None, dir_reduce=False, repopref=None, dry_run=None):
        self.move_dir = move_dir
        self.dm = DistManager(config, dist_dir, target_dir, profiles=profiles,
          repo_preference=repopref,
          dry_run=dry_run)
...
class ConfAppCommandLine(OneConfPerActionOptionsCliEnv):
    ...
    def __init__(self):
        ...
        target_dir_op = ['-t', '--targetdir', False,
         {'dest':'target_dir', 
          'metavar':'DIRECTORY',  'help':'the location of build out target dir'}]
        move_dir_op = ['-m', '--movedir', False,
         {'dest':'move_dir', 
          'metavar':'DIRECTORY',  'help':'the location of build out move dir'}]
        ...
        rp_format_op = ['-f', '--format', False,
         {'dest':'fmt', 
          'default':'{path}', 
          'metavar':'STRING', 
          'help':f"format string (i.e. {RepoSpec.DEFAULT_FORMAT})"}]
        ...
        dry_run_op = [None, '--dryrun', False,
         {'dest':'dry_run', 
          'action':'store_true', 
          'default':False,  'help':'do not do anything, just act like it'}]
        dir_reduce_op = [None, '--reduce', False,
         {'dest':'dir_reduce', 
          'action':'store_true', 
          'default':False,  'help':'remove empty directories'}]
        cnf = {'executors':[
          {'name':'info', 
           ...
           'actions':[
            ...
            {'name':'repos', 
             ...
             'opts':[
              rp_format_op, profile_op]},
            {'name':'repoinfo', 
             'meth':'repo_info', 
             'doc':'get information on repositories', 
             'opts':[
              repo_name_op, profile_op]}]},
          {'name':'dist', 
           ...
           'actions':[
            {'name':'freeze', 
             ...
             'opts':[
              dist_dir_op, wheel_dir_op, repopref_op,
              profile_op]},
            {'name':'thaw', 
             'doc':'build out a distribution', 
             'opts':[
              dist_dir_op, target_dir_op,
        ...
        super(ConfAppCommandLine, self).__init__(cnf,
          config_env_name='grsyncrc', config_type=AppConfig, pkg_dist='zensols.grsync')
