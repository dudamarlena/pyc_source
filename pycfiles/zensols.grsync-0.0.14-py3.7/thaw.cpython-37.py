...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/grsync/thaw.py
# Compiled at: 2020-04-26 00:22:04
# Size of source mod 2**32: 5382 bytes
import logging, traceback, os, zipfile, shutil
...
class ThawManager(object):
    ...
    def __init__(self, dist: Distribution, defs_file: Path, path_translator: PathTranslator, app_version, dry_run=bool):
        self.dist = dist
    ...
    def assert_version(self):
        ...
        if self.app_version != self.dist.version:
            raise ValueError('distribution has incompatable version: ' + self.dist.version)
    ...
    def _thaw_empty_dirs(self):
        ...
        for entry in self.dist.empty_dirs:
            ...
            if path.exists():
                ...
                self.dry_run or path.mkdir(mode=(entry.mode), parents=True, exist_ok=True)
        ...
        for entry in self.dist.files:
            ...
            if not parent.exists():
                ...
                if not self.dry_run:
                    ...
                    if path.exists():
                        logger.warning(f"path already exists: {path}")
                logger.info(f"{path}: mode={entry.modestr}, " + f"time={entry.modify_time}")
                if not self.dry_run:
                    with zf.open(str(entry.relative)) as (fin):
                        with open(str(path), 'wb') as (fout):
                            shutil.copyfileobj(fin, fout)
                logger.debug(f"setting mode of {path} to {entry.mode} " + f"({entry.modestr}, {entry.modify_time})")
                self.dry_run or path.chmod(entry.mode)
        ...
        for link in self.dist.links:
            if link.source.exists():
                ...
                if not link.target.exists():
                    logger.warning(f"link target does not exist: {link}--skipping")
