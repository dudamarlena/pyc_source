...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/grsync/freeze.py
# Compiled at: 2020-04-26 00:22:04
# Size of source mod 2**32: 13651 bytes
import os, stat, socket, logging, json, zipfile
from pathlib import Path
from datetime import datetime
from zensols.persist import persisted
from zensols.grsync import RepoSpec, SymbolicLink, BootstrapGenerator, PathTranslator, AppConfig
...
class Discoverer(object):
    ...
    def __init__(self, config: AppConfig, profiles: list, path_translator: PathTranslator, repo_preference: str):
        self.config = config
    ...
    def discover(self, flatten):
        ...
        dirs_or_gits = tuple(filter(lambda x: x.is_dir() and not x.is_symlink(), dobjs))
        git_paths = self._get_repo_paths(dirs_or_gits)
        links = tuple(map(lambda l: SymbolicLink(l, self.path_translator), filter(lambda x: x.is_symlink(), dobjs)))
        ...
        for ed in self.config.get_empty_dirs(self.profiles):
            ...
            empty_dirs.append(self._create_file(ed,
              no_path_obj=True, robust=True))

        dec_links = self.config.get_option(self.TARG_LINKS)
        if dec_links is not None:
            for link in map(lambda x: x['link'], filter(lambda x: 'link' in x, dec_links)):
                src = Path(link['source']).expanduser().absolute()
                targ = Path(link['target']).expanduser().absolute()
                pattern_links.append({'source':str(path_trans.relative_to(src)), 
        ...
        for link in links:
            if link.use_count == 0:
                try:
                    pattern_links.append({'source':str(link.source_relative), 
                ...
                if link.target in files_by_name:
                    ...
                    logger.warning(f"hanging link with no target: {link}--" + 'proceeding anyway')
    ...
    def freeze(self, flatten=False):
        ...
        files = disc['files']
        logger.info('freezing with git repository preference: ' + self.repo_preference)
        disc.update({'repo_specs':repo_specs,  'repo_pref':self.repo_preference, 
         'files':files, 
         'source':socket.gethostname(), 
         'create_date':datetime.now().isoformat(timespec='minutes')})
        return disc
