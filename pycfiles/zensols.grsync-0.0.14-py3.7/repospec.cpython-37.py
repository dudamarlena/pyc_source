...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/grsync/repospec.py
# Compiled at: 2020-04-26 00:22:04
# Size of source mod 2**32: 6947 bytes
import logging, sys
from pathlib import Path
from git import Repo
from zensols.persist import persisted
from zensols.grsync import RemoteSpec, PathTranslator
logger = logging.getLogger(__name__)
MASTER_SECTION = 'branch "master"'

class RepoSpec(object):
    __doc__ = "This class represents a git repository and all the symbolic links from the\n    distribution (usually the user's home directory) that link into it.\n\n    "
    DEFAULT_FORMAT = '{name}: {path}, remotes={remotes}, dirty={dirty}'
    SHORT_FORMAT = '{name}: {path} ({remotes})'

    def __init__(self, path: Path, path_translator: PathTranslator, repo: Repo=None):
    ...
    def master_remote(self):
        if not hasattr(self, '_master_remote'):
            ...
            if config.has_section(MASTER_SECTION) and config.has_option(MASTER_SECTION, 'remote'):
                self._master_remote = config.get(MASTER_SECTION, 'remote')
            else:
                self._master_remote = None
            logger.debug('path: {}, master remote: {}'.format(self.path.resolve(), self._master_remote))
    ...
    def format(self, fmt=None, writer=sys.stdout):
        ...
        rs = {'name':self.name,  'path':self.path_translator.to_relative(self.path), 
...
class FrozenRepo(object):
    ...
    def __init__(self, remotes: list, links: list, target_dir: Path, path: Path, repo_pref: str, path_translator: PathTranslator):
        """Initialize.
    ...
    def thaw(self):
        ...
        """
        if self.path.exists():
            logger.warning('path already exists: {}--skipping repo clone'.format(self.path))
