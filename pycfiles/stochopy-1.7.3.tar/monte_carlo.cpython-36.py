...
# [GCC 8.4.0]
# Embedded file name: ..\stochopy\monte_carlo.py
# Compiled at: 2018-04-26 12:38:34
# Size of source mod 2**32: 21964 bytes
"""
...
class MonteCarlo:
    ...
    _ATTRIBUTES = [
     'solution', 'fitness', 'acceptance_ratio']

    def __init__(self, func, lower=None, upper=None, n_dim=1, max_iter=1000, constrain=True, random_state=None, args=(), kwargs={}):
        ...
        if lower is not None:
            if upper is not None:
                ...
                self._lower = np.full(n_dim, -1.0)
                self._upper = np.full(n_dim, 1.0)
                self._n_dim = n_dim
        else:
    ...
    def _print_attr(self, attr):
        ...
        if attr == 'acceptance_ratio':
            return '%.2f' % self._acceptance_ratio

    def sample(self, sampler='hastings', stepsize=0.1, xstart=None, perc=1.0, n_leap=10, fprime=None, delta=0.001, snap_leap=False, args=(), kwargs={}):
        """
        ...
        if not isinstance(sampler, str) or sampler not in ('pure', 'hastings', 'hamiltonian'):
            ...
            if not isinstance(xstart, (list, tuple, np.ndarray)) or len(xstart) != self._n_dim:
                raise ValueError('xstart must be a list, tuple or ndarray of length n_dim')
            if sampler == 'hamiltonian':
                if not isinstance(stepsize, (float, int)) or stepsize <= 0.0:
            ...
            if sampler == 'pure':
                ...
                if sampler == 'hastings':
                    xopt, gfit = self._hastings(stepsize=stepsize, perc=perc,
                      xstart=xstart)
                elif sampler == 'hamiltonian':
                    xopt, gfit = self._hamiltonian(fprime=fprime, stepsize=stepsize,
                      n_leap=n_leap,
                      xstart=xstart,
                      delta=delta,
                      snap_leap=snap_leap,
    ...
    def _pure(self):
        ...
        gfit : scalar
            Energy of the MAP model.
        """
        self._models = np.random.uniform(-1.0, 1.0, (self._max_iter, self._n_dim))
        ...
        self._acceptance_ratio = 1.0
        return (self._xopt, self._gfit)

    def _hastings(self, stepsize=0.1, perc=1.0, xstart=None):
        """
        ...
        if not isinstance(stepsize, (float, int, list, np.ndarray)):
            ...
            if isinstance(stepsize, (float, int)):
                ...
                elif isinstance(stepsize, (list, np.ndarray)):
                    if len(stepsize) != self._n_dim:
            ...
            if xstart is None:
                self._models[0, :] = np.random.uniform(-1.0, 1.0, self._n_dim)
            else:
        ...
        while i < self._max_iter - 1:
            for j in np.arange(0, self._n_dim, n_dim_per_iter):
                ...
                if self._in_search_space(self._models[i, :]):
                    ...
                    log_alpha = min(0.0, self._energy[(i - 1)] - self._energy[i])
        ...
        self._xopt = self._models[idx]
        self._gfit = self._energy[idx]
        self._acceptance_ratio = 1.0 - rejected / self._max_iter
        return (self._xopt, self._gfit)

    def _hamiltonian(self, fprime=None, stepsize=0.01, n_leap=10, xstart=None, delta=0.001, snap_leap=False, args=(), kwargs={}):
        """
        ...
        for i in range(1, self._max_iter):
            ...
            if log_alpha < np.log(np.random.rand()) or not self._in_search_space(q):
                rejected += 1
                self._models[i, :] = self._models[i - 1, :]
                self._energy[i] = self._energy[(i - 1)]
            else:
    ...
    def _approx_grad(self, x, delta=0.001):
    ...
    def _in_search_space(self, x):
        if self._constrain:
            return np.logical_and(np.all(x <= 1.0), np.all(x >= -1.0))
        else:
            return True
