...
# [GCC 8.4.0]
# Embedded file name: ..\stochopy\gui\gui.py
# Compiled at: 2018-03-04 18:17:45
# Size of source mod 2**32: 29558 bytes
"""
...
if sys.version_info[0] < 3:
    import Tkinter as tk, tkFileDialog as tkfile, tkMessageBox as tkmessage, ttk, tkFont as font
else:
    import tkinter as tk, tkinter.filedialog as tkfile, tkinter.messagebox as tkmessage, tkinter.ttk as ttk
...
class StochOGUI:
    ...
    first_run = True
    MAX_SEED = 999999
    FUNCOPT = ('Ackley', 'Quartic', 'Quartic noise', 'Rastrigin', 'Rosenbrock', 'Sphere',
               'Styblinski-Tang')
    ...
    def frame1(self):
        ...
        function_option_menu = (ttk.OptionMenu)(self.frame1, self.function, self.function.get(), *sorted(self.FUNCOPT))
        max_iter_label = ttk.Label((self.frame1), text='Maximum number of iterations')
        max_iter_spinbox = Spinbox((self.frame1), from_=2, to_=9999, increment=1,
          textvariable=(self.max_iter),
          width=6,
          justify='right',
          takefocus=True)
        fps_label = ttk.Label((self.frame1), text='Delay between frames (ms)')
        fps_spinbox = Spinbox((self.frame1), from_=1, to_=1000, increment=1,
          textvariable=(self.interval),
          width=6,
          justify='right',
          takefocus=True)
        seed_button = ttk.Checkbutton((self.frame1), text='Fix seed', variable=(self.fix_seed),
          takefocus=False)
        seed_spinbox = Spinbox((self.frame1), from_=0, to_=(self.MAX_SEED), increment=1,
          textvariable=(self.seed),
          width=6,
          justify='right',
          takefocus=True)
        solver_label = ttk.Label((self.frame1), text='Solver')
        solver_option_menu = (ttk.OptionMenu)(self.frame1, self.solver_name, self.solver_name.get(), *(self.EAOPT + self.MCOPT), **{'command': self.select_widget})
        constrain_button = ttk.Checkbutton((self.frame1), text='Constrain', variable=(self.constrain),
          takefocus=False)
    ...
    def frame1_pop(self):
        ...
        if self.solver_name.get() in self.EAOPT:
            ...
            popsize_spinbox = Spinbox((self.frame1.pop), from_=1, to_=999, increment=1,
    ...
    def frame1_sync(self):
        ...
        if self.solver_name.get() in ('CPSO', 'PSO', 'DE'):
            sync_button = ttk.Checkbutton((self.frame1.sync), text='Synchronize', variable=(self.sync),
              takefocus=False)
    ...
    def init_widget(self):
        ...
        self.frame1.sliders.place(relwidth=0.45, relheight=1.0, relx=0.55, anchor='nw')
    ...
    def _scale(self, from_, to, resolution, variable, position, command=None, kwargs={}):
        ...
        scale = (ttk.Scale)(self.frame1.sliders, from_=from_, to=to, variable=variable, orient='horizontal', 
         length=20, command=command, takefocus=False, **kwargs)
        if position == 1:
    ...
    def _entry(self, variable, position, kwargs={}):
        entry = (ttk.Entry)(self.frame1.sliders, textvariable=variable, justify='right', takefocus=True, **kwargs)
        if position == 1:
    ...
    def de_widget(self):
        ...
        self.strategy_option_menu = (ttk.OptionMenu)(self.frame1.sliders, self.strategy, self.strategy.get(), *sorted(self.STRATOPT))
    ...
    def hastings_widget(self):
        ...
        ss = self._scale(-4.0, 0.0, 0.01, self.log_mcmc_stepsize_x1, 1, lambda val: self.mcmc_stepsize_x1.set(round(10.0 ** float(val), 4)))
        ss.set(np.log10(self.mcmc_stepsize_x1.get()))
        self._entry((self.mcmc_stepsize_x1), 1, kwargs=dict(validate='key', validatecommand=(
         vcmd1, '%P')))
        self._label('Step size X2', 2)
        ss = self._scale(-4.0, 0.0, 0.01, self.log_mcmc_stepsize_x2, 2, lambda val: self.mcmc_stepsize_x2.set(round(10.0 ** float(val), 4)))
        ss.set(np.log10(self.mcmc_stepsize_x2.get()))
        self._entry((self.mcmc_stepsize_x2), 2, kwargs=dict(validate='key', validatecommand=(
         vcmd2, '%P')))
    ...
    def hamiltonian_widget(self):
        ...
        self._label('Step size', 1)
        ss = self._scale(-4.0, 0.0, 0.01, self.log_hmc_stepsize, 1, lambda val: self.hmc_stepsize.set(round(10.0 ** float(val), 4)))
        ss.set(np.log10(self.hmc_stepsize.get()))
        self._entry((self.hmc_stepsize), 1, kwargs=dict(validate='key', validatecommand=(
         vcmd, '%P')))
            ...
            if self.first_run:
                ...
                if solver_name in ('hastings', 'hamiltonian'):
                    if solver_name == 'hastings':
                        stepsize = [
                         self.mcmc_stepsize_x1.get(), self.mcmc_stepsize_x2.get()]
                    else:
                        stepsize = self.hmc_stepsize.get()
                    self.solver = MonteCarlo(max_iter=self.max_iter.get(), constrain=bool(self.constrain.get()), **self.bf.get())
                    self.solver.sample(sampler=solver_name, stepsize=stepsize,
                      n_leap=(self.n_leap.get()))
                elif solver_name in ('cpso', 'pso', 'de', 'cmaes', 'vdcma'):
                    self.solver = Evolutionary(popsize=self.popsize.get(), max_iter=self.max_iter.get(), 
                     constrain=bool(self.constrain.get()), 
                     snap=True, **self.bf.get())
                    self.solver.optimize(solver=solver_name, sync=(bool(self.sync.get())),
                      w=(self.w.get()),
                      c1=(self.c1.get()),
                      c2=(self.c2.get()),
    ...
    def animate(self, interval=100, nx=101, ny=101, n_levels=10, yscale='linear', repeat=True, kwargs={}):
        ...
        if self.solver._solver in ('hastings', 'hamiltonian'):
            ...
            if self.solver._solver in ('cpso', 'pso', 'de', 'cmaes', 'vdcma'):
        ...
        self.scatplot, = ax1.plot([], [], linestyle=linestyle, color='black', marker='o',
          markersize=12,
          markerfacecolor='white',
          markeredgecolor='black',
          animated=True)
        ax2.plot(it, gfit, linestyle='-.', linewidth=1, color='black')
        self.enerplot, = ax2.plot([], [], linestyle='-', linewidth=2, color='red')
        ...
        self.iter = ax2.text(0.99, 0.99, '', va='top', ha='right', fontsize=10,
          transform=(ax2.transAxes),
          animated=True)
        self.anim_running = True
        self.anim = animation.FuncAnimation((self.fig), func, fargs=(
         models, gfit),
          frames=frames,
          interval=interval,
          repeat=repeat,
    ...
    def export_models(self):
        if self._check_run():
            filename = tkfile.asksaveasfilename(title='Export models', filetypes=[
             ('Pickle', '.pickle')],
    ...
    def export_fitness(self):
        if self._check_run():
            filename = tkfile.asksaveasfilename(title='Export fitness', filetypes=[
             ('Pickle', '.pickle')],
              defaultextension='.pickle')
            if len(filename) > 0:
                with open(filename, 'wb') as (f):
