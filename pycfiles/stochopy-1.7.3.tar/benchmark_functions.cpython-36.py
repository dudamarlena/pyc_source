...
# [GCC 8.4.0]
# Embedded file name: ..\stochopy\benchmark_functions.py
# Compiled at: 2018-03-04 17:50:57
...
class BenchmarkFunction:
    ...
    def plot(self, nx=101, ny=101, n_levels=10, axes=None, figsize=(8, 8), projection='2d', cmap=None, cont_kws={}, surf_kws={}):
        """
        ...
        if not isinstance(nx, int) or nx < 1:
        ...
        elif not isinstance(ny, int) or ny < 1:
            ...
            if not isinstance(n_levels, int) or n_levels < 1:
                ...
                if axes is not None:
                    ...
                    raise isinstance(surf_kws, dict) or ValueError('surf_kws must be a dictionary')
                if cmap is None:
                    cmap = self._set_cmap()
            ax = np.linspace(self._lower[0], self._upper[0], nx)
            ay = np.linspace(self._lower[1], self._upper[1], ny)
            X, Y = np.meshgrid(ax, ay)
            funcgrid = np.array([self._func([x, y]) for x, y in zip(X.ravel(), Y.ravel())]).reshape((nx, ny))
            if projection == '2d':
                if axes is None:
                ...
                if projection == '3d':
                    if axes is None:
                        ...
                        ax1 = axes
                    (ax1.plot_surface)(X, Y, funcgrid, rstride=1, cstride=1, linewidth=0.5, 
                     shade=False, antialiased=False, cmap=cmap, **surf_kws)
                    (ax1.contour)(X, Y, funcgrid, n_levels, zdir='Z', offset=0.0, cmap=cmap, **cont_kws)
