...
# Embedded file name: build/bdist.macosx-10.8-x86_64/egg/cmsplugin_phlog/cms_plugins.py
# Compiled at: 2013-06-26 16:51:19
...
log = getLogger('cmsplugin_phlog.cms_plugins')
...
class CMSPhlogPlugin(CMSPluginBase):
    ...
    def render(self, context, instance, placeholder):
        if instance.order == 'random':
            images = instance.gallery.sample(limit=instance.limit, public=instance.public)
        elif instance.order == 'date':
            images = instance.gallery.latest(limit=instance.limit, public=instance.public)
        else:
            images = instance.gallery.in_order(public=instance.public, limit=instance.limit)
...
class PhlogPhotoPlugin(CMSPluginBase):
    ...
    def render(self, context, instance, placeholder):
        try:
            ...
            log.exception('Unable to get a photo for a plugin')
            photo = None

        context.update({'photo': photo, 
           'instance': instance})
        return context
    ...
    def icon_src(self, instance):
        try:
            ...
            return settings.CMS_MEDIA_URL + 'images/plugins/image.png'
...
class PhlogGalleryPlugin(CMSPluginBase):
    ...
    def render(self, context, instance, placeholder):
        context.update({'placeholder': placeholder, 
           'gallery': instance})
        context.update({'plugins': render_plugins(instance.get_children().order_by('position', 'id'), context, placeholder)})
        return context

    def render_change_form(self, request, context, add=False, change=False, form_url='', obj=None):
        plugin = getattr(self, 'cms_plugin_instance', None)
        if plugin:
            context.update({'base_plugin': plugin, 
