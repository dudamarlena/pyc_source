...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cheesecake/model.py
...
import sys, os, cPickle as pickle, __builtin__
from compiler.transformer import parse, parseFile
from compiler.visitor import walk
import ast_pp

def get_call_name(node):
    assert isinstance(node, ast.CallFunc)
        ...
        if isinstance(node, ast.CallFunc):
            ...
            if isinstance(node, tuple):
                if len(node) == 1:
                    ...
                    return '%s.%s' % (get_name(node[:-1][0]), node[(-1)])

            else:
                if isinstance(node, ast.Getattr):
                    return get_name(node.asList())
                else:
                    return
...
def get_function_calls(node, fc):
    ...
    for child in node.getChildren():
        if isinstance(child, ast.CallFunc):
            func_called = get_call_name(child)
            if func_called:
                fc[func_called] = 1
        get_function_calls(child, fc)
...
class Documentable(object):
    ...
    def resolveDottedName(self, dottedname, verbose=False):
        ...
        if verbose:
            print (
             dottedname, '->', obj.fullName(), 'in', self.fullName())
        return obj
...
class ModuleVistor(object):
    ...
    def visitFunction(self, node):
        ...
        func.argspec = (
         argnames2, starargname, kwname, tuple(defaults))
...
class ImportStarFinder(object):
    ...
    def visitFrom(self, node):
        if node.names[0][0] == '*':
            ...
            self.system.importstargraph.setdefault(self.modfullname, []).append(modname)
