...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\winstrument\modules\dlls.py
# Compiled at: 2020-02-05 20:03:04
# Size of source mod 2**32: 9102 bytes
import frida, sys, win32api, win32con, win32security, ntsecuritycon, pywintypes, collections, os, uuid
...
class DLLs(BaseInstrumentation):
    ...
    def _get_dll_search_path(self):
        ...
        pathdirs = filter(lambda path: path != '' and not path.isspace(), pathdirs)
    ...
    def _resolve_relative_dll_path(self, dllpath):
        if not dllpath.lower().endswith('.dll'):
            ...
            if dllpath.lower() in self._loaded_dlls:
                return
            searched = []
            self._known_dlls = self._known_dlls or self._get_known_dlls()
        if dllpath.lower() in self._known_dlls:
            dll_abspath = os.path.join(win32api.GetSystemDirectory(), dllpath)
    ...
    def on_message(self, message, data):
        ...
        elif self._is_path_dangerous(dllpath):
            data = {'dll':dllname, 
             'dangerous_path':dllpath}
            self.write_message(data)