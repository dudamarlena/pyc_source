...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/librapy/lib/lcs.py
# Compiled at: 2019-09-05 07:32:14
# Size of source mod 2**32: 10359 bytes
"""
This implements LCS as defined in
https://github.com/libra/libra/tree/master/common/canonical_serialization
"""
import array, struct, unittest
ARRAY_MAX_LENGTH = 2147483647
STRING_CODEC = 'utf8'
...
class Serializer:
    ...
    def encode_bytes(self, value):
        ...
        if len_of_value > ARRAY_MAX_LENGTH:
            raise EncoderException('Array length exceeds maximum length. Found: {}, expected: {}'.format(len_of_value, ARRAY_MAX_LENGTH))
    ...
    def encode_map(self, value, key_encoder, value_encoder):
        len_of_value = len(value)
        if len_of_value > ARRAY_MAX_LENGTH:
            raise EncoderException('Map length exceeds maximum length. Found: {}, expected: {}'.format(len_of_value, ARRAY_MAX_LENGTH))
        self.encode_u32(len_of_value)
        serialized_value = {}
    ...
    def encode_vec(self, value, encoder):
        len_of_value = len(value)
        if len_of_value > ARRAY_MAX_LENGTH:
            raise EncoderException('Vector length exceeds maximum length. Found: {}, expected: {}'.format(len_of_value, ARRAY_MAX_LENGTH))
        self.encode_u32(len_of_value)
        for val in value:
            encoder(val)
...
class Deserializer:
    ...
    def decode_bool(self):
        ...
        if value != 0:
            if value != 1:
                raise EncoderException('Invalid boolean. Found: {}, expected 0 or 1.'.format(value))
        return value == 1
...
class Bar:
    ...
    def deserialize(deserializer):
        return Bar(deserializer.decode_u64(), deserializer.decode_bytes(), deserializer.decode_struct(Addr), deserializer.decode_u32())
...
class Foo:
    ...
    def deserialize(deserializer):
        return Foo(deserializer.decode_u64(), deserializer.decode_bytes(), deserializer.decode_struct(Bar), deserializer.decode_bool(), deserializer.decode_map(byte_decoder, byte_decoder))
...
class TestLCS(unittest.TestCase):
    test_vector = bytes.fromhex('ffffffffffffffff060000006463584d42376400000000000000090000000001020304050607082000000005050505050505050505050505050505050505050505050505050505050505056300000001030000000100000001030000001615430300000000381503000000160a05040000001415596903000000c9175a')

    def test_correctness(self):
        """
        Verify correctness of implementation using common data structure
        """
        bar = Bar(100, bytes([0, 1, 2, 3, 4, 5, 6, 7, 8]), Addr(bytes([5] * 32)), 99)
        map_value = {bytes([0, 56, 21]): bytes([22, 10, 5]), 
         bytes([1]): bytes([22, 21, 67]), 
         bytes([20, 21, 89, 105]): bytes([201, 23, 90])}
        foo = Foo(18446744073709551615, bytes([100, 99, 88, 77, 66, 55]), bar, True, map_value)
