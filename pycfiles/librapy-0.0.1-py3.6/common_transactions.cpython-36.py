...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/librapy/common_transactions.py
# Compiled at: 2019-09-05 08:02:04
...
def create_account(ac_uri, faucet_account, account_address, initial_amount):
    ...
    signed_txn = serializer.encode_struct(RawTransaction(faucet_account.get_address(), faucet_account.sequence_number, CreateAccount(account_address, initial_amount * 1000000), MAX_GAS_AMOUNT, GAS_UNIT_PRICE, int(time.time()) + EXPIRATION_TIME).sign(faucet_account.get_signing_key())).get_buffer()
...
def get_account_state(ac_uri, account):
    ...
    requested_item.get_account_state_request.address = account.get_address().value
    response = ac.UpdateToLatestLedger(request).response_items[0].get_account_state_response.account_state_with_proof
...
def mint_remotely(mint_uri, account, amount):
    post_data = {'amount':amount * 1000000, 
     'address':account.get_address().value.hex()}
    request = urllib.request.Request(mint_uri + '/?' + urllib.parse.urlencode(post_data), b'')
    try:
...
def mint_with_key(ac_uri, faucet_account, account_address, amount):
    ...
    request = SubmitTransactionRequest()
    serializer = Serializer()
    signed_txn = serializer.encode_struct(RawTransaction(faucet_account.get_address(), faucet_account.sequence_number, Mint(account_address, amount * 1000000), MAX_GAS_AMOUNT, GAS_UNIT_PRICE, int(time.time()) + EXPIRATION_TIME).sign(faucet_account.get_signing_key())).get_buffer()
    request.signed_txn.signed_txn = signed_txn
    return ac.SubmitTransaction(request)
...
def transfer(ac_uri, from_account, to_account_address, amount):
    ...
    request = SubmitTransactionRequest()
    serializer = Serializer()
    signed_txn = serializer.encode_struct(RawTransaction(from_account.get_address(), from_account.sequence_number, PeerToPeerTransfer(to_account_address, amount * 1000000), MAX_GAS_AMOUNT, GAS_UNIT_PRICE, int(time.time()) + EXPIRATION_TIME).sign(from_account.get_signing_key())).get_buffer()
    request.signed_txn.signed_txn = signed_txn
    return ac.SubmitTransaction(request)