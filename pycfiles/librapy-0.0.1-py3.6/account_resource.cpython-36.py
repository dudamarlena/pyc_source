...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/librapy/types/account_resource.py
# Compiled at: 2019-09-05 06:50:52
...
class AccountResource:
    ...
    def __init__(self, authentication_key, balance, delegated_withdrawal_capability, received_events, sent_events, sequence_number):
    ...
    def __repr__(self):
        return 'AccountResource (\n\tbalance: {}\n\tsequence_number: {}\n\tauthentication_key: {}\n\tdelegated_withdrawal_capability: {}\n\tsent_events: {}\n\treceived_events: {}\n)'.format(self.balance, self.sequence_number, self.authentication_key.hex(), self.delegated_withdrawal_capability, self.sent_events, self.received_events)

    def deserialize(deserializer):
        return AccountResource(deserializer.decode_bytes(), deserializer.decode_u64(), deserializer.decode_bool(), deserializer.decode_struct(EventHandle), deserializer.decode_struct(EventHandle), deserializer.decode_u64())

    def from_blob(blob):
        deserializer = Deserializer(blob)
        account_map = deserializer.decode_map(byte_decoder, byte_decoder)
        tag = StructTag(AccountAddress.default(), AccountResource.module_name, AccountResource.struct_name, [])
...
class EventHandle:
    ...
    def deserialize(deserializer):
        return EventHandle(deserializer.decode_u64(), deserializer.decode_struct(EventKey))
