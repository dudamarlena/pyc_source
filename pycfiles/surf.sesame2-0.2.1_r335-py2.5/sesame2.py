...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sesame2/sesame2.py
...
class Sesame2(httplib.HTTPConnection):
    ...
    response_format = {'nt': 'application/rdf+ntriples', 
    ...
    def __query(self, id, query, infer=False, queryLn='SPARQL', limit=None, headers={}):
        params = {'queryLn': queryLn, 'infer': 'true' if infer else 'false', 
           'query': query}
    ...
    def sparql_query(self, id, query, infer=False, format='sparql+json'):
        ...
        if type:
            ...
            return self.__query(id, query, infer=infer, queryLn='SPARQL', limit=None, headers={'Accept': Sesame2.request_format[format]})
        return
    ...
    def statements(self, id, s=None, p=None, o=None, context=None, infer=False, format='xml'):
        ...
        stmts = self.sesame2_request('GET', 'statements', {'id': id}, params=params, headers={'Accept': Sesame2.request_format[format]})
        return stmts
    ...
    def add_statements(self, id, rdf_data=None, context=None, baseURI=None, saveStrings=False, update=True, content_type='xml'):
        ...
        try:
            response = self.sesame2_request(method, 'statements', {'id': id}, body=rdf_data, params=params, headers={'Content-type': Sesame2.request_format[content_type]})
    ...
    def transaction(self, id, rdf_transaction):
        try:
            response = self.sesame2_request('POST', 'statements', {'id': id}, body=rdf_transaction.xml(), headers={'Content-type': RDFTransaction.mime})
        except Sesame2Exception, e:
            return False
