...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sesame2/writer.py
...
class WriterPlugin(RDFWriter):
    ...
    def _save(self, *resources):
        for resource in resources:
            ...
            allegro.add_statements(self.__repository, graph.serialize(format='nt'), update=True, content_type='nt', context=context)
    ...
    def _remove(self, *resources, **kwargs):
        ...
        for resource in resources:
            context = resource.context and resource.context.n3() or None
            allegro.remove_statements(self.__repository, s=resource.subject.n3(), context=context)
            if inverse:
                allegro.remove_statements(self.__repository, o=resource.subject.n3(), context=context)
    ...
    def _add_triple(self, s=None, p=None, o=None, context=None):
        self.get_allegro().add_statements(self.__repository, self.__tontriples(s, p, o), update=True, content_type='nt')
    ...
    def _set_triple(self, s=None, p=None, o=None, context=None):
        ...
        allegro.remove_statements(self.__repository, s=sn3, p=pn3, context=context)
        allegro.add_statements(self.__repository, self.__tontriples(s, p, o), update=True, content_type='nt')
        return
    ...
    def load_triples(self, **kwargs):
        ...
        self.get_allegro().load_statements(self.__repository, location, update=update, format=format, context=context, baseURI=baseURI, externalFormat=externalFormat, saveStrings=saveStrings)
        return True