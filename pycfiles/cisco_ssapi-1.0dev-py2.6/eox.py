...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.4-i386/egg/cisco_ssapi/eox.py
# Compiled at: 2010-07-12 10:53:38
import logging
...
class Server(object):
    bulkMethods = [
     'showAllProductIDs', 'showEOXByDates']
        ...
        if not first_response:
            ...
            while True:
                ...
                if len(threads.keys()) == 0 and next_page > total_pages:
                    break
                while next_page <= total_pages and len(threads.keys()) < self._threads:
    ...
    def getChunkedResponses(self, method, args):
        ...
        while True:
            ...
            if len(threads.keys()) == 0 and next_chunk > total_chunks:
                break
            while next_chunk <= total_chunks and len(threads.keys()) < self._threads:
...
class PagingThread(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            try:
                self._response = getattr(self._client.service, self._method)(*args)
                break
            except WebFault, ex:
        ...
        pager = getattr(self._response, 'PaginationResponseRecord', None)
        if pager:
            log.info('received %s of %s records on page %s of %s', pager.PageRecords, pager.TotalRecords, pager.PageIndex, pager.LastIndex)
        return
...
class ChunkingThread(threading.Thread):
    ...
    def run(self):
        ...
        response_gen = self._server.getPaginatedResponses(self._method, self._args)
