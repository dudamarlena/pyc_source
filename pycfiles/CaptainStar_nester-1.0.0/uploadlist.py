...
# Embedded file name: build/bdist.linux-x86_64/egg/cptsoul/downloadmanager/uploadlist.py
...
class UploadList(gtk.TreeView, CptCommon):
    ...
    def fileStartEvent(self, widget, info, name, ip, port):
        if (
         info.login, name) in self._fileToSend:
            path, key = self._fileToSend[(info.login, name)]
            del self._fileToSend[(info.login, name)]
            sendFile(path, ip, port, PreparedCaller(self.progressCallback, key=key), PreparedCaller(self.endCallback, key=key), PreparedCaller(self.errorCallback, key=key))

    def startFileTransfer(self, login):
        dialog = gtk.FileChooserDialog(title='CatpainSoul - Choose file to upload', action=gtk.FILE_CHOOSER_ACTION_OPEN, buttons=(
