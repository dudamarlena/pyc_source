...
# Embedded file name: build/bdist.linux-x86_64/egg/cptsoul/mainwindow/loginlist.py
...
class Buddy(object):
    ...
    def __init__(self, no, login, state, ip, location):
        self._no, self._login, self._state, self._ip, self._location = (
         int(no), login, state, ip, location)
...
class LoginList(CptCommon):
    ...
    def loginWatchlist(self, login):
        return (
         login, self.getState(login), self.atSchool(login))
    ...
    def getState(self, login):
        ...
        for buddy in self.getFromLogin(login):
            if state == 'logout' and buddy.state in ('away', 'lock', 'actif') or state in ('away',
                                                                                           'lock') and buddy.state == 'actif':
                state = buddy.state
