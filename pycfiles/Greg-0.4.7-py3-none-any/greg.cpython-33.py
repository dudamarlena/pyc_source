...
# [GCC 8.4.0]
# Embedded file name: ./greg.py
# Compiled at: 2013-12-28 13:15:15
# Size of source mod 2**32: 29253 bytes
import configparser, os, pickle, shlex, subprocess, sys, time, re, unicodedata, string
...
def check_directory(placeholders):
    ...
    if 'no' in subdirectory:
        ...
        subdname = substitute_placeholders(subdnametemplate, placeholders, 'safe')
...
def tag(placeholders):
    ...
    for tag in tagdict:
        metadata = substitute_placeholders(tagdict[tag], placeholders, 'normal')
...
def download_entry(feed, entry):
    ...
    for podname in downloadlinks:
        if podname not in feed.entrylinks:
            ...
            try:
                placeholders = Placeholders(feed, downloadlinks[podname], podname, title, sanitizedsummary)
...
def substitute_placeholders(string, placeholders, mode):
    if mode == 'safe':
        newst = string.format(link=placeholders.link, filename=placeholders.filename, directory=placeholders.directory, fullpath=placeholders.fullpath, title=placeholders.title, filename_title=placeholders.filename_title, date=placeholders.date_string(), podcasttitle=placeholders.podcasttitle, filename_podcasttitle=placeholders.filename_podcasttitle, name=placeholders.name, subtitle=placeholders.sanitizedsubtitle, entrysummary=placeholders.entrysummary)
    if mode == 'normal':
        newst = string.format(link=shlex.quote(placeholders.link), filename=shlex.quote(placeholders.filename), directory=shlex.quote(placeholders.directory), fullpath=shlex.quote(placeholders.fullpath), title=shlex.quote(placeholders.title), filename_title=shlex.quote(placeholders.filename_title), date=placeholders.date_string(), podcasttitle=shlex.quote(placeholders.podcasttitle), filename_podcasttitle=shlex.quote(placeholders.filename_podcasttitle), name=shlex.quote(placeholders.name), subtitle=shlex.quote(placeholders.sanitizedsubtitle), entrysummary=shlex.quote(placeholders.entrysummary))
    return newst
...
def sync(args):
    ...
    for target in targetfeeds:
        ...
        if not feed.wentwrong:
            ...
            entrycounter = 0
            for entry in feed.podcast.entries[::-1]:
                feed.fix_linkdate(entry)
                if feed.linkdate > currentdate:
