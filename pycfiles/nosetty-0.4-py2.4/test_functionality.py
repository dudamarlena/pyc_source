...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.4-fat/egg/nosetty/test/test_functionality.py
# Compiled at: 2007-05-18 19:33:30
...
class NoseTTYTester(PluginTester):
    ...
    def _makeNose(self):
        ...
        return NoseStream(subprocess.Popen(self._args, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, stdin=subprocess.PIPE, cwd=self.suitepath, env=self.env, bufsize=1, universal_newlines=True))
...
class WithTestMethod(object):
    ...
    def makeSuite(self):
        self.tmp = TempIO()
        self.tmp.putfile('test_to_fail.py', 'def test_bad():\n    print "it\'s no good!!!"\n    assert False')
        return self.tmp
...
class WithTestClass(object):
    ...
    def makeSuite(self):
        self.tmp = TempIO()
        self.tmp.putfile('test_to_fail_testclass.py', 'class TestToFail(object):\n    def test_bad_from_testclass(self):\n        assert False\n            ')
        return self.tmp
...
class TestEditingErrorsByNumberWithMethod(WithTestMethod, EditingErrorsByNumber):
    __module__ = __name__


class TestEditingErrorsByNumberWithUnittest(WithUnitTest, EditingErrorsByNumber):
    __module__ = __name__


class TestEditingErrorsByNumberWithTestClass(WithTestClass, EditingErrorsByNumber):
    __module__ = __name__
...
class EditErrorsByNumber(EditingErrorsByNumber):
    ...
    def makeSuite(self):
        self.tmp = TempIO()
        self.tmp.putfile('test_to_fail_nested.py', 'def two():\n    raise ValueError\ndef three():\n    two()\ndef four():\n    three()\ndef test_nested_fail():\n    four()')
        return self.tmp
...
class TestEditCommandVar(WithTestMethod, NoseTTYTester):
    ...
    env = os.environ.copy()
    env['NOSE_TTY_EDIT_CMD'] = '%(editor)s %(filename)s --any-custom-opt=%(lineno)s'
...
class TestInvalidEditCommand(WithTestMethod, NoseTTYTester):
    __module__ = __name__
    addargs = ['--tty-edit-cmd=%(editor)s %(noop)s']
...
class TestEditCommandOption(WithTestMethod, NoseTTYTester):
    __module__ = __name__
    addargs = ['--tty-edit-cmd="%(editor)s %(filename)s --any-custom-opt=%(lineno)s"', '--tty-editor=echo']
...
class TestRerunCommand(WithTestMethod, NoseTTYTester):
    ...
    def onrun(self, run_number):
        ...
        if run_number == 2:
            assert os.path.exists(self.tmp.test.join('foo.out')), "expected package level teardown() to generate 'foo.out' in %s" % os.listdir(self.tmp.test)
...
class OutputsavePlugin(NoseTTYTester):
    __module__ = __name__
    addargs = ['--tty-edit-cmd', "%(editor)s 'OPENFILE:%(filename)s LINE:%(lineno)s'", '--tty-editor=echo', '--with-outputsave', '--save-directory=output']
...
class TestOutputsavePlugin(WithTestMethod, OutputsavePlugin):
    ...
    def assert_failure_edited(self):
        self.inputLines('o')
        filename = os.path.join(self.tmp, 'output', 'fail-test_to_fail.test_bad.txt')
