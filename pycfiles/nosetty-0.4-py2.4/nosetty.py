...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.4-fat/egg/nosetty/nosetty.py
# Compiled at: 2007-05-24 09:53:56
"""core nosetty components"""
import os, sys, re, gc, stat, inspect, signal, time
from os import path
...
def reproduce_program(argv=sys.argv):
    ...
    if os.path.exists(prog) and prog.endswith('.py'):
        if not os.stat(prog)[stat.ST_MODE] & (stat.S_IXUSR | stat.S_IXGRP | stat.S_IXOTH):
...
class NoseTTY(Plugin):
    ...
    def add_options(self, parser, env=os.environ):
        ...
        env_opt = 'NOSE_TTY'
        parser.add_option('--tty', action='store_true', dest=self.enableOpt, default=env.get(env_opt), help='Enable plugin %s: %s [%s]' % (self.__class__.__name__, self.help(), env_opt))
        env_opt = 'NOSE_TTY_EDITOR'
        default = env.get(env_opt, None)
        if not default:
            default = os.environ.get('EDITOR')
        parser.add_option('--tty-editor', metavar=env_opt, action='store', default=default, help='editor program [%s or EDITOR] (currently: `%s`)' % (env_opt, default))
        env_opt = 'NOSE_TTY_EDIT_CMD'
        default = env.get(env_opt, None)
        if not default:
            default = self.tty_edit_cmd
        parser.add_option('--tty-edit-cmd', action='store', default=default, metavar=env_opt, help='template to invoke edit command.  [%s] (currently: `%s`)' % (env_opt, default))
        env_opt = 'NOSE_TTY_SESSION_FILE'
        default = env.get(env_opt, None)
        if not default:
            default = self.tty_session_file
        parser.add_option('--tty-session-file', action='store', default=default, metavar=env_opt, help='path to session file used internally.  [%s] (currently: %s)' % (env_opt, default))
        return
    ...
    def compileEditCommand(self, filename, lineno):
        params = {'editor': self.tty_editor, 'filename': filename, 'lineno': lineno, 'lineindex': lineno - 1}
        try:
            cmd = self.tty_edit_cmd % params
        except KeyError:
            raise InteractiveError("Invalid edit command '%s' (available format strings: %s)" % (self.tty_edit_cmd, (', ').join([ '%%(%s)s' % k for k in params.keys() ])))
    ...
    def configure(self, options, conf):
        ...
        if 'HOME' not in os.environ:
            raise EnvironmentError('could not find HOME in environment.  are you running from a shell?')
        ...
        if options.tty_editor:
            ...
            raise EnvironmentError('could not locate editor (tried --tty-editor, NOSE_TTY_EDITOR, EDITOR)')
    ...
    def enterErrorLoop(self, test, err, capt, error_prompt='ERROR'):
        ...
        for ext in extract_tb(tb):
            d = {'extracted': ext, 'filename': ext[0], 'lineno': ext[1]}
            ...
            for plug in self.conf.plugins:
                if plug.name == 'outputsave':
                    ...
                    if calc_testname is None:
                        raise ValueError('calc_testname was None but outputsave plugin was enabled.  whuh?')
                    outputsaved_as = os.path.join(self.outputsave.save_directory, '%s-%s.txt' % (error_prompt.lower(), calc_testname(test)))
                    break
        ...
        while 1:
            ...
            print_('')
            print_(separator1)
            print_('%s: %s' % (error_prompt, test.shortDescription() or str(test)))
            print_(separator2)
            ...
            for i in range(0, tblen):
                ...
                print_('  %s File "%s", line %d, in %s' % (stack_marker, filename, lineno, name))
                if line:
                    print_('       %s' % line.strip())
                marker -= 1
            ...
            try:
                ...
                if line.isdigit():
                    ...
                    try:
                        ...
                        raise InteractiveError('trace number %s is non-existant' % num)
                    else:
                        self.editAtTb(self.getTbInfo(tb_position=tb_position, stack=index))
                        continue
                elif line == 'R':
        ...
        try:
            ...
            try:
                ...
                if child:
                    time.sleep(1)
                    try:
                        ps = subprocess.Popen(['ps', '-p', str(child.pid), '-o', 'pid'], stdout=subprocess.PIPE)
                    except OSError:
