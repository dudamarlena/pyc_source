...
# [GCC 8.4.0]
# Embedded file name: /data/django-chuck/django_chuck/template/notch_interactive/test.py
# Compiled at: 2012-06-13 10:12:50
...
class TemplateEngineTest(unittest.TestCase):
    ...
    def test_appends(self):
        ...
        with open('test/project_dir/base.html') as (f):
            content = f.read()
            self.assertTrue(re.search('One must have chaos in oneself to give birth to a dancing star.+The big MOOOOOH has pwned you', content, re.MULTILINE | re.DOTALL), content)
    ...
    def test_prepends(self):
        ...
        with open('test/project_dir/base.html') as (f):
            content = f.read()
            self.assertTrue(re.search('The big MOOOOOH has pwned you.+One must have chaos in oneself to give birth to a dancing star', content, re.MULTILINE | re.DOTALL), content)
    ...
    def test_complex_example(self):
        ...
        with open('test/project_dir/common.py') as (f):
            content = f.read()
            self.assertIn('# -*- coding', content.splitlines()[0])
            self.assertIn('CMS_TEMPLATES', content)
            self.assertIn("ROOT_URLCONF = 'test.urls'", content)
            self.assertFalse('#!chuck' in content, content)
