...
# [GCC 8.4.0]
# Embedded file name: /data/django-chuck/django_chuck/template/notch_interactive/engine.py
...
class TemplateEngine(BaseEngine):
    ...
    input = ''
    output = ''
    keyword_patterns = {'extends': re.compile('#!chuck_extends [\\\'\\"]?(.+)[\\\'\\"]?', re.IGNORECASE), 
     'extends_if_exists': re.compile('#!chuck_extends_if_exists [\\\'\\"]?(.+)[\\\'\\"]?', re.IGNORECASE), 
     'renders': re.compile('#!chuck_renders ([\\w\\d\\_\\-]+)', re.IGNORECASE), 
     'appends': re.compile('#!chuck_appends ([\\w\\d\\_\\-]+)', re.IGNORECASE), 
     'prepends': re.compile('#!chuck_prepends ([\\w\\d\\_\\-]+)', re.IGNORECASE)}
    ...
    def get_block_content(self, content, block_name='', keyword='renders'):
        ...
        """
        block_content = ''
        match = re.search('#!chuck_' + keyword + ' ' + re.escape(block_name) + '([\r\n\\s]+.*?)#!end', content, re.MULTILINE | re.DOTALL)
        if match:
    ...
    def update_block_content(self, block_name, keyword='renders'):
        ...
        if old_block_content:
            ...
            if new_block_content:
                ...
                self.output = re.sub('#!chuck_renders ' + block_name + re.escape(old_block_content) + '#!end', '#!chuck_renders ' + block_name + update_content + '#!end', self.output, re.IGNORECASE | re.DOTALL | re.MULTILINE)
            else:
            ...
            if keyword:
                if self.keyword_patterns.get(keyword):
                    ...
                    if match and keyword == 'extends':
                        ...
                        if match and keyword == 'extends_if_exists':
                            self.extension_file = match.group(1).rstrip('"').rstrip("'")
                            ...
                            if match and match.group(1) and keyword == 'renders' and self.base_file:
        ...
        with open(input_file, 'r') as (f):
            for line in f.xreadlines():
                ...
                if keyword:
                    ...
                    if match:
                        line = re.sub('#!chuck_' + keyword + ' ' + re.escape(match.group(1)) + '\\s?\r?\n?', '', line, re.IGNORECASE)
                line = re.sub('#!end\\s*\\r?\\n?', '', line, re.IGNORECASE)
                output.append(line)
