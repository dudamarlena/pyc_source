...
# [GCC 8.4.0]
# Embedded file name: /data/django-chuck/django_chuck/utils.py
# Compiled at: 2012-06-01 04:12:37
import os, sys, functools, django_chuck
...
def find_commands():
    ...
    if command_path:
        for f in os.listdir(command_path):
            if not f.startswith('_') and f.endswith('.py') and not f == 'base.py' and not f == 'test.py':
                commands.append(f[:-3])
...
def compile_template(input_file, output_file, placeholder, site_dir, project_dir, engine_obj=None, debug=False):
    ...
    """
    result = True
    if output_file.endswith('.pyc') or output_file.endswith('.mo'):
        return None
    ...
    except django_chuck.exceptions.TemplateError as e:
