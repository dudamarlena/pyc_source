...
# [GCC 8.4.0]
# Embedded file name: /Volumes/data/proj/subarulink/subarulink/cli.py
# Compiled at: 2020-04-06 08:47:06
# Size of source mod 2**32: 13375 bytes
"""
...
class CLI:
    __doc__ = "A basic shell for interacting with Subaru's Remote Services API."
            ...
            if args[0] == 'set':
                ...
                if args[0] == 'stop':
                    ...
                    if args[0] == 'start':
                        ...
                        await self._ctrl.remote_start(self._current_vin, self._hvac_temp, self._hvac_mode, sc.HEAT_SEAT_OFF, sc.HEAT_SEAT_OFF, sc.REAR_DEFROST_OFF, self._hvac_speed, sc.RECIRCULATE_ON, sc.REAR_AC_OFF)
                    else:
            ...
            if await self._ctrl.connect():
                ...
                if self._current_api_gen == 'g2':
                    ...
                    if self._current_api_gen == 'g1':
                        print('%s is a Generation 1 telematics vehicle which has not been tested.' % self._current_name)
                    else:
                                ...
                                if cmd == 'unlock':
                                    ...
                                    if cmd == 'lights':
                                        ...
                                        if cmd == 'horn':
                                            ...
                                            if cmd == 'show' and self._current_api_gen == 'g2':
                                                self._show(args)
                                            else:
                                                if cmd == 'update' and self._current_api_gen == 'g2':
                                                    await self._ctrl.update(self._current_vin)
                                                else:
                                                    if cmd == 'fetch' and self._current_api_gen == 'g2':
                                                        await self._ctrl.fetch(self._current_vin)
                                                    else:
                                                        if cmd == 'charge' and self._current_api_gen == 'g2':
                                                            await self._ctrl.charge_start(self._current_vin)
                                                        else:
                                                            if cmd == 'hvac' and self._current_api_gen == 'g2':
    ...
    async def run(self):
        ...
        self._ctrl = Controller(self._session, self._config['username'], self._config['password'], self._config['device_id'], self._config['pin'], self._config['device_name'])
        try:
...
def main():
    ...
    parser.add_argument('-r',
      '--reset', help='Reset saved account information', action='store_true')
