...
# [GCC 8.4.0]
# Embedded file name: /Volumes/data/proj/subarulink/subarulink/connection.py
# Compiled at: 2020-04-12 13:48:06
...
"""
import asyncio, logging, pprint, time, aiohttp
from yarl import URL
from subarulink.exceptions import IncompleteCredentials, SubaruException
_LOGGER = logging.getLogger(__name__)

class Connection:
    __doc__ = 'Connection to Subaru Starlink API.'
    ...
    def __init__(self, websession: aiohttp.ClientSession, username, password, device_id, device_name) -> None:
        ...
        self.head = {}
        self.head['User-Agent'] = 'Mozilla/5.0 (Linux; Android 10; Android SDK built for x86 Build/QSR1.191030.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/74.0.3729.185 Mobile Safari/537.36'
    ...
    async def validate_session(self):
        ...
        resp = await self._Connection__open('/validateSession.json', 'get')
    ...
    async def get(self, command, params=None, data=None, json=None):
        ...
        if self.authenticated:
            resp = await self._Connection__open((f"{command}"),
              method='get',
              headers=(self.head),
              params=params,
              data=data,
            ...
            if self.password:
                if self.device_id:
                    post_data = {'env':'cloudprod', 
                     ...
                     'pushToken':None, 
                     'deviceType':'android'}
                    resp = await self._Connection__open('/login.json',
                      'post', data=post_data, headers=(self.head))
    ...
    async def _refresh_vehicles(self):
        resp = await self._Connection__open('/refreshVehicles.json',
            ...
            if self.password:
                if self.device_id:
                    post_data = {'username':self.username, 
                     'password':self.password, 
                     'deviceId':self.device_id}
                    resp = await self._Connection__open('/login',
                      'post', data=post_data, baseurl=web_baseurl)
        resp = await self._Connection__open('/profile/updateDeviceEntry.json',
          'get',
    ...
    async def _set_device_name(self):
        ...
        web_baseurl = 'https://www.mysubaru.com'
        resp = await self._Connection__open('/profile/addDeviceName.json',
          'get',
          params={'deviceId':self.device_id, 
