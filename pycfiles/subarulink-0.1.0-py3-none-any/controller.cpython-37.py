...
# [GCC 8.4.0]
# Embedded file name: /Volumes/data/proj/subarulink/subarulink/controller.py
# Compiled at: 2020-04-12 14:43:50
# Size of source mod 2**32: 12770 bytes
"""
...
"""
import asyncio, logging, pprint, time
...
class Controller:
    __doc__ = 'Controller for connections to Subaru Starlink Gen2 API.'

    def __init__(self, websession, username, password, device_id, pin, device_name, update_interval=7200):
    ...
    async def remote_start(self, vin, temp, mode, heat_left_seat, heat_right_seat, rear_defrost, fan_speed, recirculate, rear_ac):
        """Send remote start command."""
        form_data = {sc.TEMP: temp, 
    ...
    async def _remote_query(self, vin, cmd, data=None):
        ...
        async with self._lock[vin]:
            js_resp = await self._get(('service/%s/%s/execute.json' % (api_gen, cmd)),
              json=data)
    ...
    async def _remote_command(self, vin, cmd, data=None, poll_url='/service/api_gen/remoteService/status.json'):
        ...
        req_id = ''
        async with self._lock[vin]:
            js_resp = await self._post(('service/%s/%s/execute.json' % (api_gen, cmd)),
              json=form_data)
            _LOGGER.debug(pprint.pformat(js_resp))
            if js_resp['success']:
    ...
    async def _locate(self, vin):
        js_resp = await self._remote_command(vin,
          'vehicleStatus',
    ...
    async def _wait_request_status(self, req_id, api_gen, poll_url, attempts=20):
        ...
        while not success:
            if attempt < attempts:
                js_resp = await self._connection.get((poll_url.replace('api_gen', api_gen)),
                  params=params)
                _LOGGER.debug(pprint.pformat(js_resp))
    ...
    async def _select_vehicle(self, vin=None):
        ...
        if self._vin_id_map.get(vin):
            ...
            if js_resp['success']:
                ...
                _LOGGER.debug('Current vehicle: vin=%s, api_gen=%s', self._current_vin, api_gen)
                return api_gen
        else:
