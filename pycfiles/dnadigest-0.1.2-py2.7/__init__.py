...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dnadigest/__init__.py
# Compiled at: 2015-03-19 12:40:31
import re, yaml, logging, math, svgwrite
...
class Dnadigest:
    ...
    def __init__(self, enzyme_data_file=None):
        ...
        """
        self.data = ''
        self.dna_regex_translations = {'A': 'A', 
           'T': 'T', 
           'C': 'C', 
           'G': 'G', 
           'N': '.', 
    ...
    def load_enzyme_data(self, data_handle):
        ...
        for enzyme_key in data_structure:
            ...
            if len(enzyme['cut'][0]) != len(enzyme['cut'][1]):
                ...
                enzyme['recognition_sequence'] = {x[0]:x[3:] for x in enzyme['recognition_sequence']}
                enzyme['cut'] = {x[0]:x[3:-3] for x in enzyme['cut']}
    ...
    def string_cutter(self, sequence, recognition_fr, recog_nucl_index, status):
        ...
        if status == 'circular':
            ...
            match_list = self.__merged_iter(rec_seq_f.finditer(mod_sequence), rec_seq_r.finditer(mod_sequence))
            first_cut = None
            ...
            for match in match_list:
                ...
                if first_cut is None:
                    if cut_location < len(sequence):
                        reopened_sequence = sequence[cut_location:] + sequence[0:cut_location]
                    else:
                        reopened_sequence = mod_sequence[cut_location:] + mod_sequence[wrap_around:cut_location]
                    return self.string_cutter(reopened_sequence, recognition_fr, adjusted_recog, 'linear')
                remove_first_fragment = True
                if cut_location < len(sequence):
                    fragments.append(mod_sequence[prev_index:cut_location])
                    prev_index = cut_location
                elif first_cut == cut_location - len(sequence):
                    fragments.append(mod_sequence[prev_index:cut_location])
                    break
                else:
            ...
            match_list = self.__merged_iter(rec_seq_f.finditer(sequence), rec_seq_r.finditer(sequence))
            for match in match_list:
                adjusted_recog = self.__adjust_recog_for_strand(recog_nucl_index, rec_f_exp, match.group(0))
                cut_location = match.start() + adjusted_recog
    ...
    def find_cut_sites(self, sequence, enzyme_list, status='circular'):
        ...
        for enzyme in enzymes:
            sites, status = self.__find_cut_sites(sequence, self.enzyme_dict[enzyme]['recognition_sequence'], self.enzyme_dict[enzyme]['sense_cut_idx'], 'circular')
            for site in sites:
                try:
    ...
    def string_processor(self, fragment_list, recognition_fr, recog_nucl_index, status):
        ...
        if len(new_fragment_list) == 0:
            ...
            return (
             new_fragment_list, status, did_cut)
    ...
    def expand_multiple(self, base_str):
        ...
        try:
            ...
            replaced = base_str[0:m.start('base')] + base * count + base_str[m.end('count'):]
    ...
    def process_data(self, seq, cut_with, status='circular'):
        filtered_enzyme_dict = self.enzyme_dict_filter(self.enzyme_dict, cut_with)
        fragment_list = [
         seq]
        cuts = []
        for enzyme in filtered_enzyme_dict:
            log.info('Cutting [%s] with %s' % ((',').join(fragment_list),
             enzyme))
            fragment_list, status, did_cut = self.string_processor(fragment_list, filtered_enzyme_dict[enzyme]['recognition_sequence'], filtered_enzyme_dict[enzyme]['sense_cut_idx'], status)
            if did_cut:
                cuts.append(enzyme)

        return {'fragment_list': fragment_list, 'cut_with': cuts, 
           'status': status}

    @classmethod
    def drawer(cls, sequence_length, cut_sites, sequence_id='PhiX', radius=250, border=50):
        """Print SVG in plasmid digest style
        """
        image_size = 2 * (radius + border)
        center = (radius + border, radius + border)
        svg_document = svgwrite.Drawing(size=('%spx' % image_size,
         '%spx' % image_size))
        svg_document.add(svg_document.circle(center=center, r=radius, fill='rgb(255, 255, 255)', stroke='black'))
        ...
        for cut_site in sorted(cut_sites.keys()):
            ...
            svg_document.add(svg_document.line(start=point_from, end=point_to, stroke='black'))
    ...
    def __cast_ray(cls, center, angle, length):
        return [
         length * math.cos(angle) + center[0],
