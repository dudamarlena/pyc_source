...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/persist/annotation.py
...
class PersistedWork(object):
    ...
    def __init__(self, path: Union[(str, Path)], owner: object, cache_global: bool=False, transient: bool=False, initial_value: Any=None):
...
class PersistableContainerMetadata(object):
    ...
    def write(self, indent=0, include_content=False, recursive=False, writer=sys.stdout):
        sp = ' ' * indent
        spe = ' ' * (indent + 1)
        for k, v in self.container.__dict__.items():
            if isinstance(v, PersistedWork):
...
class persisted(object):
    ...
    def __init__(self, name, path=None, cache_global=False, transient=False):
        logger.debug('persisted decorator on attr: {}, global={}'.format(name, cache_global))
        self.attr_name = name
        self.path = path
        self.cache_global = cache_global
        self.transient = transient
