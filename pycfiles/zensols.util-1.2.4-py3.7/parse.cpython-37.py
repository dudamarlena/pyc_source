...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/config/parse.py
# Compiled at: 2020-05-11 00:54:36
# Size of source mod 2**32: 17240 bytes
...
class Writable(ABC):
    __doc__ = 'An interface for classes that have multi-line debuging capability.\n\n    '
    ...
    def _write_line(self, line: str, depth: int=0, writer: TextIOWrapper=sys.stdout):
    ...
    def _write_dict(self, data: dict, depth: int=0, writer: TextIOWrapper=sys.stdout):
...
class Settings(object):
    __doc__ = 'A default object used to populate in ``Configurable.populate``.\n\n    '
...
class Configurable(Writable, metaclass=ABCMeta):
    ...
    @classmethod
    def populate_state(self, state: Dict[(str, str)], obj: Union[(dict, object)]=None, parse_types: bool=True) -> Union[(dict, object)]:
                ...
                if isinstance(v, str):
                    if v == 'None':
                        ...
                        if self.FLOAT_REGEXP.match(v):
                            ...
                            if self.INT_REGEXP.match(v):
                                v = int(v)
                            else:
                                if self.BOOL_REGEXP.match(v):
                                    v = v == 'True'
                                else:
                                    if self.PATH_REGEXP.match(v):
...
class Config(Configurable):
    __doc__ = 'Application configuration utility.  This reads from a configuration and\n    returns sets or subsets of options.\n\n    '

    def __init__(self, config_file=None, default_section='default', robust=False, default_vars=None, default_expect=False, create_defaults=None):
    ...
    def get_options(self, section='default', opt_keys=None, vars=None):
        ...
        if opt_keys is None:
            if conf is None:
                opt_keys = {}
            elif not self.robust or conf.has_section(section):
    ...
    def get_option(self, name, section=None, vars=None, expect=None):
        ...
        if self._narrow_expect(expect):
            raise ValueError("no option '{}' found in section {}".format(name, section))

    def get_option_list(self, name, section=None, vars=None, expect=None, separator=','):
    ...
    def get_option_path(self, name, section=None, vars=None, expect=None, create=None):
    ...
    def __str__(self):
        return str('file: {}, section: {}'.format(self.config_file, self.sections))
...
class ExtendedInterpolationConfig(Config):
    __doc__ = 'Configuration class extends using advanced interpolation with\n    ``configparser.ExtendedInterpolation``.\n\n    '
...
class ExtendedInterpolationEnvConfig(ExtendedInterpolationConfig):
    __doc__ = 'A ``Config`` implementation that creates a section called ``env`` with\n    environment variables passed.\n\n    '

    def __init__(self, *args, remove_vars=None, env=None, env_sec='env', **kwargs):
    ...
    def _munge_default_vars(self, vars):
        if vars is not None:
            if self.remove_vars is not None:
