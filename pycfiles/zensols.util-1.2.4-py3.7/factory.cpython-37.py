...
# Size of source mod 2**32: 18215 bytes
...
class ConfigFactory(object):
    ...
    def _instance(self, cls_desc, cls, *args, **kwargs):
        ...
        try:
            ...
            try:
                logger.error(f"can not create '{cls_desc}' for class " + f"{cls}({args})({kwargs}): {e}")
                raise e
            finally:
                e = None
                del e
...
class ImportConfigFactory(ConfigFactory):
    ...
    def __init__(self, *args, reload=False, shared=False, reload_root=None, **kwargs):
        ...
        if pw_param_set is not None:
            ...
            for params in eval(pw_param_set):
                ...
                if prop_name not in kwargs:
                    raise ValueError(f"no property '{prop_name}' found in '" + f"section '{sec_name}'")
    ...
    def _instance(self, cls_desc, cls, *args, **kwargs):
        ...
        logger.debug(f"import instance: section name: {sec_name}, " + f"cls={class_name}, args={args}, kwargs={kwargs}")
        pw_injects = self._process_injects(sec_name, kwargs)
        prev_defined_sec = self.INJECTS.get(class_name)
        if prev_defined_sec is not None:
            if prev_defined_sec != sec_name:
                msg = 'attempt redefine or reuse injectsion for class ' + f"{class_name} in section {sec_name} previously " + f"defined in section {prev_defined_sec}"
                raise RedefinedInjectionError(msg)
        if len(pw_injects) > 0:
            if class_name not in self.INJECTS:
