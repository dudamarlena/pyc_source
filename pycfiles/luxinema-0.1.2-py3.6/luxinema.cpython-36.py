...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/luxinema/luxinema.py
# Compiled at: 2017-09-12 17:39:53
...
MOVIEINFO = [
 'Title', 'Showtime', 'Rating', 'URL', 'Description']
...
def request_imdb_json(movie_id):
    ...
    :rtype: dict
    """
    data = requests.get(IMDBAPI.format(movie_id=movie_id), headers=HEADERS).json()
    return data
...
def verify_movie_id(title, movie_id):
    ...
    name_ok = levenshtein_distance(title.lower(), title_id) < 3
...
def print_best_rated(moviedf, howmany=1):
    ...
    for index in range(0, howmany):
        ...
        print(template.format(title=(movie['Title']), rating=(movie['Rating']),
          showtime=showtime))
...
def get_cli_parser():
    ...
    parser.add_argument('date', metavar='DATE', type=str, default=(get_today()), help='Insert date in isoformat(YYYY-MM-DD) or simply DD')
    parser.add_argument('-b', '--best', help='display only the 3 best movies of the day', action='store_true')
    parser.add_argument('-v', '--version', help='displays the current version of luxinema', action='store_true')
    return parser
