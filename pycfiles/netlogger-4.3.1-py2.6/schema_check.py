...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/netlogger/analysis/schema/schema_check.py
# Compiled at: 2012-04-11 12:21:26
"""
Code to handle various aspects of transitioning to a new verson of the 
Stampede schema.
"""
__rcsid__ = '$Id: schema_check.py 31137 2012-04-11 16:21:21Z dang $'
...
class ErrorStrings:
    ...
    def get_init_error(e):
        ...
        er = ''
        if action_error == ErrorStrings.create_failure and table_error == ErrorStrings.schema_info:
...
class SchemaCheck(DoesLogging):
    ...
    def _version_check(self, version_number):
        ...
        if float(version_number) == CURRENT_SCHEMA_VERSION:
            ...
            return True
        if float(version_number) < CURRENT_SCHEMA_VERSION:
            self.log.error('check_schema', msg='Schema version %s found - expecting %s - database admin will need to run upgrade tool' % (
    ...
    def check_schema(self):
        ...
        for t in table_scan:
            if self.session.connection().dialect.name == 'sqlite':
                ...
                self.log.error('check_schema', msg='Dialect %s not available for scanning' % self.session.connection().dialect.name)
    ...
    def upgrade_to_4_0(self):
        ...
        success = [
         'JOB_SUCCESS', 'POST_SCRIPT_SUCCESS']
        ...
        for r in q.all():
            ...
            for rr in qq.all():
                if rr.state in success:
                    self.session.execute('UPDATE job_instance set exitcode = 0 where job_instance_id = %s' % r.job_instance_id)
                elif rr.state in failure:
                    self.session.execute('UPDATE job_instance set exitcode = 256 where job_instance_id = %s' % r.job_instance_id)
