...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/netlogger/nllog.py
# Compiled at: 2011-02-07 20:52:41
"""
NetLogger interactions with the Python logging module.
"""
__rcsid__ = '$Id: logutil.py 772 2008-05-23 22:59:22Z dang $'
import logging, logging.handlers, optparse, os, sys, time, traceback, types
from netlogger import nlapi
from netlogger.nlapi import Level
from netlogger.version import *
...
class BPLogger(logging.Logger):
    ...
    def exception(self, event, err, **kwargs):
        ...
        self.log(logging.ERROR, Level.ERROR, event, msg=str(err), status=(-1), traceback=estr, **kwargs)
...
class Profiler(type):
    ...
    def __new__(cls, classname, bases, classdict):
        ...
        classdict['_log'] = log = _logger('%s.%s' % (classdict['__module__'], classname))
        log.set_meta(pid=os.getpid(), ppid=os.getppid(), gpid=os.getgid())
        keys = []
        if not classdict.get('profiler_skip_all', cls.profiler_skip_all):
            keys = filter(lambda k: type(classdict[k]) is types.FunctionType and k not in classdict.get('profiler_skip_methods', cls.profiler_skip_methods), classdict.keys())
        for k in keys:
...
class OptionParser(optparse.OptionParser):
    ...
    *******************************************************
    ...
    DEST_DAEMON = 'daemon'
    DEST_ROT = 'log_rotate'
    OPTIONS = {DEST_LOG: ('-L', '--log'), DEST_VERBOSE: ('-v', '--verbose'), 
       DEST_QUIET: ('-q', '--quiet'), 
       DEST_DAEMON: (None, '--daemon'), 
       DEST_ROT: ('-R', '--logrotate')}
    VBMAP = (
     logging.WARN, logging.INFO, logging.DEBUG, TRACE)
    ...
    def _add_options(self):
        ...
        if self._dmn:
            self.add_option(self.OPTIONS[self.DEST_DAEMON][1], action='store_true', dest=self.DEST_DAEMON, default=False, help='run in daemon mode')
            logfile_default = 'required'
        else:
            logfile_default = 'default=stderr'
        group.add_option(self.OPTIONS[self.DEST_LOG][0], self.OPTIONS[self.DEST_LOG][1], default=None, action='store', dest=self.DEST_LOG, metavar='FILE', help='write logs to FILE (%s)' % logfile_default)
        group.add_option(self.OPTIONS[self.DEST_ROT][0], self.OPTIONS[self.DEST_ROT][1], default=None, action='store', dest=self.DEST_ROT, metavar='TIME', help='rotate logs at an interval (<N>d or <N>h or <N>m)')
        group.add_option(self.OPTIONS[self.DEST_VERBOSE][0], self.OPTIONS[self.DEST_VERBOSE][1], action='count', default=0, dest=self.DEST_VERBOSE, help='more verbose logging')
        group.add_option(self.OPTIONS[self.DEST_QUIET][0], self.OPTIONS[self.DEST_QUIET][1], action='store_true', default=False, dest=self.DEST_QUIET, help='quiet mode, no logging')
    ...
    def parse_args(self, args=None):
        ...
        if not logfile or logfile == '-':
            ...
            handler = None
            if is_daemon and (logfile is None or logfile == '' or logfile == '-'):
        ...
        elif logfile is None or logfile == '':
