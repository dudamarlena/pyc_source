...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/netlogger/parsers/base.py
...
def parse_ts(ts):
    ...
    (ts, subs) = ts.split('.')
    subs = float('.' + subs[:-1])
    return calendar.timegm(time.strptime(ts, '%Y-%m-%dT%H:%M:%S')) + subs
...
def _bp_extract(s, as_dict=True, validate=False):
    ...
    if not validate:
        ...
        for (ws1, n, v, q1, vq, q2, ws2) in BP_EXPR_WS.findall(s):
            if not n:
                raise BPParseError(("Bad key: '{0}'").format(n))
            valid_data_len += sum(map(len, (ws1, n, v, q1, vq, q2, ws2))) + 1
            if vq:
                v = vq.replace('\\"', '"')
            if as_dict:
                result[n] = v
...
class BaseParser(ProcessInterface, DoesLogging):
    ...
    def __init__(self, input_file, fullname='unknown', unparsed_file=None, parse_date=True, add_hash='no', **kw):
        ...
        if hasattr(input_file, 'fileno'):
            ...
            self._infile_rlist = (input_file.fileno(),)
        else:
        ...
        self._const_nvp_str = (' ').join([ '%s=%s' % (k, v) for (k, v) in self._const_nvp.items()
                                         ])
