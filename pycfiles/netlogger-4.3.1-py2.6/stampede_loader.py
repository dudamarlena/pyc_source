...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/netlogger/analysis/modules/stampede_loader.py
# Compiled at: 2012-04-11 12:17:46
"""
...
class Analyzer(BaseAnalyzer, SQLAlchemyInit):
    ...
    def __init__(self, connString=None, perf='no', batch='no', mysql_engine=None, **kw):
        ...
        self.eventMap = {'stampede.wf.plan': self.workflow, 
           'stampede.wf.map.task_job': self.task_map, 
           'stampede.static.start': self.noop, 
           ...
           'stampede.job_inst.grid.submit.start': self.noop, 
           'stampede.job_inst.grid.submit.end': self.jobstate, 
           'stampede.job_inst.globus.submit.start': self.noop, 
           'stampede.job_inst.globus.submit.end': self.jobstate, 
           'stampede.inv.start': self.noop, 
        ...
        self._batch_cache = {'batch_events': [], 'update_events': [], 'host_map_events': []}
    ...
    def process(self, linedata):
        ...
        try:
            ...
            if linedata['event'].startswith('stampede.job_inst.'):
                self.log.warn('process', msg='Corner case jobstate event: "%s"' % linedata['event'])
                self.jobstate(linedata)
            else:
                self.log.error('process', msg='no handler for event type "%s" defined' % linedata['event'])
        except exceptions.IntegrityError, e:
            self.log.error('process', msg='Insert failed for event "%s" : %s' % (linedata['event'], e))
    ...
    def linedataToObject(self, linedata, o):
        ...
        for (k, v) in linedata.items():
            ...
            try:
                ...
                self.log.error('linedataToObject', msg='unable to process attribute %s with values: %s' % (k, v))
    ...
    def hard_flush(self, batch_flush=True):
        ...
        try:
            ...
            self.log.error('batch_flush', msg='Integrity error on batch flush: %s - batch will need to be committed per-event which will take longer' % e)
            self.session.rollback()
            self.hard_flush(batch_flush=False)
        except exceptions.OperationalError, e:
            self.log.error('batch_flush', msg='Connection problem during commit: %s - reattempting batch' % e)
            self.session.rollback()
            self.hard_flush()
    ...
    def workflowstate(self, linedata):
        ...
        state = {'stampede.xwf.start': 'WORKFLOW_STARTED', 
    ...
    def job_instance(self, linedata):
        ...
        if job_instance.wf_id == None:
            ...
            if not job_instance.job_id:
                self.log.error('job_instance', msg='Could not determine job_id for job_instance: %s' % job_instance)
                return
            if job_instance.event == 'stampede.job_inst.submit.start' or job_instance.event == 'stampede.job_inst.pre.start':
                ...
                if job_instance.event == 'stampede.job_inst.pre.start':
                    self.jobstate(linedata)
                return
            if job_instance.event == 'stampede.job_inst.main.end' or job_instance.event == 'stampede.job_inst.post.end':
    ...
    def jobstate(self, linedata):
        ...
        states = {'stampede.job_inst.pre.start': [
           ...
           'stampede.job_inst.post.end': [
                                        'POST_SCRIPT_FAILED', 'POST_SCRIPT_SUCCESS'], 
    ...
    def invocation(self, linedata):
        ...
        if invocation.job_instance_id == None:
            self.log.error('invocation', msg='Could not determine job_instance_id for invocation: %s' % invocation)
            return
        else:
            if self._batch:
    ...
    def task_map(self, linedata):
        ...
        if not job_id:
            self.log.error('task_map', msg='Could not determine job_id for task map: %s' % linedata)
            return
        try:
    ...
    def wf_uuid_to_id(self, wf_uuid):
        ...
        if not self.wf_id_cache.has_key(wf_uuid):
            ...
            try:
                ...
                self.log.error('wf_uuid_to_id', msg='Multiple wf_id results for wf_uuid %s : %s' % (wf_uuid, e))
                return
            except orm.exc.NoResultFound, e:
                self.log.error('wf_uuid_to_id', msg='No wf_id results for wf_uuid %s : %s' % (wf_uuid, e))
                return
    ...
    def wf_uuid_to_root_id(self, wf_uuid):
        ...
        if not self.root_wf_id_cache.has_key(wf_uuid):
            ...
            try:
                ...
                self.log.error('wf_uuid_to_root_id', msg='Multiple wf_id results for wf_uuid %s : %s' % (wf_uuid, e))
                return
            except orm.exc.NoResultFound, e:
                self.log.error('wf_uuid_to_root_id', msg='No wf_id results for wf_uuid %s : %s' % (wf_uuid, e))
                return
    ...
    def get_job_id(self, wf_id, exec_id):
        ...
        if not self.job_id_cache.has_key((wf_id, exec_id)):
            ...
            try:
                ...
                self.log.error('get_job_id', msg='Multiple results found for wf_uuid/exec_job_id: %s/%s' % (wf_id, exec_id))
                return
            except orm.exc.NoResultFound, e:
                self.log.error('get_job_id', msg='No results found for wf_uuid/exec_job_id: %s/%s' % (wf_id, exec_id))
                return
    ...
    def get_job_instance_id(self, o, quiet=False):
        ...
        if not self.job_instance_id_cache.has_key(uniqueIdIdx):
            ...
            try:
                ...
                if not quiet:
                    self.log.error('get_job_instance_id', msg='Multple job_instance_id results for tuple %s : %s' % (uniqueIdIdx, e))
                return
            except orm.exc.NoResultFound, e:
                if not quiet:
                    self.log.error('get_job_instance_id', msg='No job_instance_id results for tuple %s : %s' % (uniqueIdIdx, e))
                return
        ...
        if not self.host_cache.has_key((cached_job_id, host.job_submit_seq)):
            if not host.host_id:
                try:
                    ...
                    self.log.error('map_host_to_job_instance', msg='Multiple host_id results for host: %s' % host)
    ...
    def finish(self):
        ...
        if self._perf:
            ...
            self.log.info('performance', insert_time=self._insert_time, insert_num=self._insert_num, total_time=run_time, run_time_delta=run_time - self._insert_time, mean_time=self._insert_time / self._insert_num)
