...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/netlogger/analysis/workflow/stampede_statistics.py
# Compiled at: 2012-05-12 14:50:13
"""
...
"""
__rcsid__ = '$Id: stampede_statistics.py 31646 2012-05-02 15:09:12Z mgoode $'
...
class StampedeStatistics(SQLAlchemyInit, DoesLogging):
    ...
    def initialize(self, root_wf_uuid):
        ...
        try:
            ...
            self.log.error('initialize', msg='Multiple results found for wf_uuid: %s' % root_wf_uuid)
            return False
        except orm.exc.NoResultFound, e:
            self.log.error('initialize', msg='No results found for wf_uuid: %s' % root_wf_uuid)
            return False
        else:
            if self._expand:
                ...
                if not len(self._wfs):
                    self.log.error('initialize', msg='Unable to expand wf_uuid: %s - not a root_wf_id?' % root_wf_uuid)
                    return False
            else:
    ...
    def get_total_succeeded_jobs_status(self):
        ...
        sq_1 = self.session.query(func.max(JobInstanceSub.job_submit_seq).label('jss'), JobInstanceSub.job_id.label('jobid'), JobInstanceSub.exitcode.label('ec'))
        if self._expand:
    ...
    def _base_task_status_query_old(self):
        ...
        sq_1 = self.session.query(WorkflowSub1.wf_id.label('wid'), func.max(JobInstanceSub1.job_submit_seq).label('jss'), JobInstanceSub1.job_id.label('jobid'))
        if self._expand:
    ...
    def _base_task_statistics_query(self, success=True):
        ...
        sq_1 = self.session.query(w.wf_id, j.job_id, ji.job_instance_id.label('jiid'), ji.job_submit_seq.label('jss'), func.max(ji.job_submit_seq).label('maxjss'))
    ...
    def get_workflow_states(self):
        ...
        """
        q = self.session.query(Workflowstate.wf_id, Workflowstate.state, Workflowstate.timestamp, Workflowstate.restart_count, Workflowstate.status)
    ...
    def get_workflow_details(self):
        ...
        """
        q = self.session.query(Workflow.wf_id, Workflow.wf_uuid, Workflow.parent_wf_id, Workflow.root_wf_id, Workflow.dag_file_name, Workflow.submit_hostname, Workflow.submit_dir, Workflow.planner_arguments, Workflow.user, Workflow.grid_dn, Workflow.planner_version, Workflow.dax_label, Workflow.dax_version)
    ...
    def get_job_statistics(self):
        ...
        sq_1 = sq_1.filter(or_(Jobstate.state == 'GRID_SUBMIT', Jobstate.state == 'GLOBUS_SUBMIT', Jobstate.state == 'EXECUTE'))
        sq_1 = sq_1.subquery()
    ...
    def get_job_instance_info(self, job_instance_id=None):
        ...
        q = self.session.query(JobInstance.job_instance_id, JobInstance.site, JobInstance.stdout_file, JobInstance.stderr_file, JobInstance.stdout_text, JobInstance.stderr_text, JobInstance.work_dir, sq_0.as_scalar().label('subwf_dir'), sq_1.as_scalar().label('job_name'), sq_2.as_scalar().label('submit_file'), sq_3.as_scalar().label('executable'), sq_4.as_scalar().label('argv'), sq_5.as_scalar().label('submit_dir'), sq_7.as_scalar().label('state'), sq_8.as_scalar().label('pre_executable'), sq_9.as_scalar().label('pre_argv'), sq_10.as_scalar().label('hostname'))
        if job_instance_id:
    ...
    def get_invocation_info(self, ji_id=None):
        ...
        if self._expand or not ji_id:
            return []
        q = self.session.query(Invocation.task_submit_seq, Invocation.exitcode, Invocation.executable, Invocation.argv, Invocation.transformation, Invocation.abs_task_id)
    ...
    def get_job_name(self):
        ...
        if self._expand:
            return []
        q = self.session.query(Job.job_id, JobInstance.job_instance_id, JobInstance.job_submit_seq, Job.exec_job_id.label('job_name'))
        q = q.filter(Job.job_id == JobInstance.job_id)
    ...
    def get_job_site(self):
        ...
        if self._expand:
            return []
        q = self.session.query(Job.job_id, JobInstance.job_instance_id, JobInstance.job_submit_seq, JobInstance.site)
        q = q.filter(Job.wf_id.in_(self._wfs))
    ...
    def get_job_kickstart(self):
        ...
        sq_1 = sq_1.subquery()
        q = self.session.query(Job.job_id, JobInstance.job_instance_id, JobInstance.job_submit_seq, cast(sq_1.as_scalar(), Float).label('kickstart'))
        q = q.filter(JobInstance.job_id == Job.job_id)
        q = q.filter(Job.wf_id.in_(self._wfs))
        q = q.order_by(JobInstance.job_submit_seq)
        return q.all()
    ...
    def get_job_runtime(self):
        ...
        if self._expand:
            return []
        q = self.session.query(Job.job_id, JobInstance.job_instance_id, JobInstance.job_submit_seq, JobInstance.local_duration.label('runtime'))
        q = q.filter(Job.job_id == JobInstance.job_id)
        q = q.filter(Job.wf_id.in_(self._wfs))
    ...
    def get_job_seqexec(self):
        ...
        if self._expand:
            return []
        q = self.session.query(Job.job_id, JobInstance.job_instance_id, JobInstance.job_submit_seq, JobInstance.cluster_duration)
        q = q.filter(Job.job_id == JobInstance.job_id)
        q = q.filter(Job.wf_id.in_(self._wfs))
    ...
    def get_condor_q_time(self):
        ...
        sq_2 = sq_2.filter(Jobstate.state == 'SUBMIT')
        sq_2 = sq_2.subquery()
        q = self.session.query(Job.job_id, JobInstance.job_instance_id, JobInstance.job_submit_seq, cast(sq_1.as_scalar() - sq_2.as_scalar(), Float).label('condor_q_time'))
        q = q.filter(JobInstance.job_id == Job.job_id)
        q = q.filter(Job.wf_id.in_(self._wfs))
        q = q.order_by(JobInstance.job_submit_seq)
        return q.all()
    ...
    def get_resource_delay(self):
        ...
        sq_2 = sq_2.subquery()
        q = self.session.query(Job.job_id, JobInstance.job_instance_id, JobInstance.job_submit_seq, cast(sq_1.as_scalar() - sq_2.as_scalar(), Float).label('resource_delay'))
        q = q.filter(JobInstance.job_id == Job.job_id)
        q = q.filter(Job.wf_id.in_(self._wfs))
        q = q.order_by(JobInstance.job_submit_seq)
        return q.all()
    ...
    def get_post_time(self):
        ...
        sq_2 = sq_2.subquery()
        q = self.session.query(Job.job_id, JobInstance.job_instance_id, JobInstance.job_submit_seq, cast(sq_1.as_scalar() - sq_2.as_scalar(), Float).label('post_time'))
        q = q.filter(JobInstance.job_id == Job.job_id)
        q = q.filter(Job.wf_id.in_(self._wfs))
        q = q.order_by(JobInstance.job_submit_seq)
        return q.all()
    ...
    def _get_host_filter(self):
        if self._host_filter == None:
            ...
            if type(self._host_filter) == type('str'):
                return Host.hostname == self._host_filter
            else:
                if type(self._host_filter) == type([]):
                    return Host.hostname.in_(self._host_filter)
                return
            return

    def _get_xform_filter(self):
        if self._xform_filter['include'] != None and self._xform_filter['exclude'] != None:
            self.log.error('_get_xform_filter', msg="Can't set both transform include and exclude - reset s.set_transformation_filter()")
            return
        else:
            if self._xform_filter['include'] == None and self._xform_filter['exclude'] == None:
                return
            if self._xform_filter['include'] != None:
                if type(self._xform_filter['include']) == type('str'):
                    return Invocation.transformation == self._xform_filter['include']
                else:
                    if type(self._xform_filter['include']) == type([]):
                ...
                if type(self._xform_filter['exclude']) == type('str'):
                    ...
                    if type(self._xform_filter['exclude']) == type([]):
    ...
    def get_invocation_by_time(self):
        ...
        """
        q = self.session.query(cast(Invocation.start_time / self._get_date_divisors(), Integer).label('date_format'), func.count(Invocation.invocation_id).label('count'), cast(func.sum(Invocation.remote_duration), Float).label('total_runtime'))
        q = q.filter(Workflow.root_wf_id == self._root_wf_id)
    ...
    def get_jobs_run_by_time(self):
        ...
        https://confluence.pegasus.isi.edu/display/pegasus/Additional+queries
        """
        q = self.session.query(cast(Jobstate.timestamp / self._get_date_divisors(), Integer).label('date_format'), func.count(JobInstance.job_instance_id).label('count'), cast(func.sum(JobInstance.local_duration), Float).label('total_runtime'))
        q = q.filter(Workflow.root_wf_id == self._root_wf_id)
    ...
    def get_invocation_by_time_per_host(self, host=None):
        ...
        https://confluence.pegasus.isi.edu/display/pegasus/Additional+queries
        """
        q = self.session.query(cast(Invocation.start_time / self._get_date_divisors(), Integer).label('date_format'), Host.hostname.label('host_name'), func.count(Invocation.invocation_id).label('count'), cast(func.sum(Invocation.remote_duration), Float).label('total_runtime'))
        q = q.filter(Workflow.root_wf_id == self._root_wf_id)
        q = q.filter(Invocation.wf_id == Workflow.wf_id)
    ...
    def get_jobs_run_by_time_per_host(self):
        ...
        https://confluence.pegasus.isi.edu/display/pegasus/Additional+queries
        """
        q = self.session.query(cast(Jobstate.timestamp / self._get_date_divisors(), Integer).label('date_format'), Host.hostname.label('host_name'), func.count(JobInstance.job_instance_id).label('count'), cast(func.sum(JobInstance.local_duration), Float).label('total_runtime'))
        q = q.filter(Workflow.root_wf_id == self._root_wf_id)
        q = q.filter(Workflow.wf_id == Job.wf_id)
        q = q.filter(Job.job_id == JobInstance.job_id)
        q = q.filter(Jobstate.job_instance_id == JobInstance.job_instance_id)
