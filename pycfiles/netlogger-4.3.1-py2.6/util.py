...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/netlogger/util.py
# Compiled at: 2011-02-08 15:09:11
"""
...
import os, Queue, re, signal, sys, tempfile, time, traceback, netlogger
from netlogger import configobj
NL_HOME = 'NETLOGGER_HOME'
EOF_EVENT = 'netlogger.EOF'
MAGICDATE_EXAMPLES = (', ').join([ '%s' % s for s in ('<N> weeks|days|hours|minutes|seconds time ago',
                                                      'Today', 'Now', 'Tomorrow',
                                                      'Yesterday', '4th [[Jan] 2003]',
                                                      'Jan 4th 2003', 'mm/dd/yyyy (first)',
                                                      'dd/mm/yyyy (second)', 'yyyy-mm-dd',
                                                      'yyyymmdd', 'next Tuesday',
                                                      'last Tuesday')
                                 ])
...
class ScriptOptionParser(OptionParser):
    standard_option_list = (
     make_option('-q', '--quiet', action='store_true', dest='quiet', help='only report errors'),
     make_option('-v', '--verbosity', default=0, action='count', dest='verbosity', help='show verbose status information, repeat for even more detail'))
...
def mostRecentFile(dir, file_pattern, after_time=None):
    ...
    if not os.path.isdir(dir):
        ...
        timed_files = [ (os.stat_result(os.stat(fname)).st_mtime, fname) for fname in glob.glob(search_path)
                      ]
        if not timed_files:
            return []
...
def daemonize(log=None, root_log=None, close_fds=True):
    ...
    if close_fds:
        ...
        if root_log and len(root_log.handlers) > 0:
            console = (
             sys.stderr.fileno(), sys.stdout.fileno())
            ...
            for line in f:
                ...
                if m:
                    ...
                    try:
                        ...
                        raise IOError("Cannot read file '%s' included from '%s'" % (
                         inc_path, f.name))
...
_TFAC = {None: 1, 's': 1, 'm': 60, 'h': 3600, 'd': 86400, 'seconds': 1, 
   'minutes': 60, 'hours': 3600, 'days': 86400, 'second': 1, 
...
def check_timeperiod(option, opt, value):
    try:
        ...
        raise OptionValueError('option %s: invalid time period value: %r' % (opt, value))


_BPAT = re.compile('(\\d+)\\s*(\\S+)')
_BFAC = {None: 1, 'b': 1, 'bytes': 1, 'kb': 1024, 
   'kilobytes': 1024, 'mb': 1048576, 
   'megabytes': 1048576}
...
def as_bool(x):
    ...
    if x is True or x is False:
        ...
        retval = None
        if isinstance(x, str):
            retval = {'yes': True, 'no': False, 'on': True, 
               'off': False, '1': True, 
               '0': False, 'true': True, 
               'false': False}.get(x.lower(), None)
        if retval is None:
