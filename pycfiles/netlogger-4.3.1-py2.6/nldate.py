...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/netlogger/nldate.py
# Compiled at: 2009-12-08 17:43:30
import calendar, magicdate, re, time
...
def guess(s, parse=True, is_gmt=False, set_gmt=False, try_iso=True, try_num=True, try_en=True):
...
def utcFormatISO(sec):
    ...
    """
    tm = time.gmtime(sec)
    usec = int((sec - int(sec)) * 1000000.0)
...
SYSLOG_DATE_RE = re.compile('\\s*(...)\\s+(...)\\s+(\\d\\d?) ' + '(\\d\\d):(\\d\\d):(\\d\\d)\\s+(\\d\\d\\d\\d)\\s*')
