...
# [GCC 8.4.0]
# Embedded file name: c:\users\landon\dropbox\documents\pycharmprojects\mezzanine-wiki\mezzanine_wiki\views.py
# Compiled at: 2018-02-01 12:15:07
# Size of source mod 2**32: 20900 bytes
...
def can_add_wikipage(user):
    if settings.WIKI_PRIVACY == wiki_settings.WIKI_PRIVACY_OPENED:
        ...
        if settings.WIKI_PRIVACY == wiki_settings.WIKI_PRIVACY_MODERATED:
            if user.has_perm('mezzanine_wiki.add_wikipage'):
                return True
        return False
...
class Wiki_page_list(TemplateView):
    ...
    def get(self, request, tag=None, username=None, category=None, template='mezawiki/wiki_page_list.html'):
        ...
        if category is not None:
            ...
            templates.append('mezawiki/wiki_post_list_%s.html' % str(category.slug))
        author = None
        if username is not None:
            author = get_object_or_404(User, username=username)
            wiki_pages = wiki_pages.filter(user=author)
            templates.append('mezawiki/wiki_page_list_%s.html' % username)
        if VERSION >= (1, 4):
            ...
            if wiki_pages:
                ...
                for cat in WikiCategory.objects.raw('SELECT * FROM mezzanine_wiki_wikicategory JOIN mezzanine_wiki_wikipage_categories ON mezzanine_wiki_wikicategory.id = wikicategory_id WHERE wikipage_id IN (%s)' % ids):
                    categories[cat.wikipage_id].append(cat)

            keywords = defaultdict(list)
            wikipage_type = ContentType.objects.get(app_label='mezzanine_wiki', model='wikipage')
            assigned = AssignedKeyword.objects.filter(wikipage__in=wiki_pages, content_type=wikipage_type).select_related('keyword')
            for a in assigned:
        ...
        for i, page in enumerate(wiki_pages):
            if VERSION < (1, 4):
                ...
                setattr(wiki_pages[i], 'category_list', page.categories.all())
                setattr(wiki_pages[i], 'keyword_list', [k.keyword for k in page.keywords.all()])

        wiki_pages = paginate(wiki_pages, request.GET.get('page', 1), settings.WIKI_PAGES_PER_PAGE, settings.MAX_PAGING_LINKS)
        context = {'wiki_pages':wiki_pages,  'tag':tag, 
    ...
    def get(self, request, slug, template='mezawiki/wiki_page_detail.html'):
        ...
        if slug != slug_original:
            ...
            try:
                ...
                if wiki_page_exist.status == 1:
                    if not wiki_page_exist.can_edit_wikipage(request.user):
            ...
            templates = ['mezawiki/wiki_page_detail.html_%s' % str(slug), template]
            return render(request, templates, context)
...
class Wiki_page_history(TemplateView):
    ...
    def get(self, request, template='mezawiki/wiki_page_history.html', **kwargs):
        ...
        if slug != slug_original:
            return HttpResponseRedirect(reverse('wiki_page_history', args=[slug]))
        try:
            wiki_pages = WikiPage.objects.published(for_user=(request.user))
            wiki_page = wiki_pages.get(slug=slug)
            ...
            if not wiki_page.can_view_wikipage(request.user):
                ...
                context = {'wiki_page':wiki_page, 
                 'revisions':revisions}
                templates = ['mezawiki/wiki_page_history_%s.html' % str(slug), template]
                return render(request, templates, context)
...
class Wiki_page_revision(TemplateView):
    ...
    def get(self, request, template='mezawiki/wiki_page_revision.html', **kwargs):
        ...
        try:
            ...
            if not wiki_page.can_view_wikipage(request.user):
                return HttpResponseForbidden(_("You don't have permission to view this wiki page revision."))
            else:
                context = {'wiki_page':wiki_page, 
                 'revision':revision}
                templates = ['mezawiki/wiki_page_detail_%s.html' % str(slug), template]
                return render(request, templates, context)
        ...
        try:
            ...
            try:
                ...
                if 'undo' in request.GET:
                    if request.GET['undo'] == 'error':
                        undo_error = True
                context = {'wiki_page':wiki_page, 
                 'from_revision':from_rev,  'to_revision':to_rev,  'diff':diff,  'undo_error':undo_error}
...
class Wiki_page_revert(TemplateView):
    ...
    def get(self, request, slug, revision_pk, template='mezawiki/wiki_page_edit.html'):
        ...
        slug = urlize_title(slug)
        if slug != slug_original:
            return HttpResponseRedirect(reverse('wiki_page_revert', args=[slug, revision_pk]))
        try:
            wiki_pages = WikiPage.objects.published(for_user=(request.user))
            wiki_page = wiki_pages.get(slug=slug)
        except WikiPage.DoesNotExist:
            ...
            new_revision = WikiPageRevision(page=wiki_page, user=(request.user if request.user.is_authenticated() else User.objects.get(id=(-1))))
            if request.method == 'POST':
                ...
                if src_revision.user:
                    description = _('Reverted to revision of %(time)s by %(user)s.') % {'time':src_revision.created, 
                     'user':src_revision.user.username}
                else:
                    description = _('Reverted to anonymous revision of %(time)s.') % {'time': src_revision.created}
                form = WikiPageForm(data=(request.POST or None), instance=wiki_page, initial={'content':src_revision.content, 
                 'summary':description})
            context = {'wiki_page':wiki_page, 
...
class Wiki_page_undo(TemplateView):
    ...
    def get(self, request, slug, revision_pk, template='mezawiki/wiki_page_edit.html'):
        ...
        slug = urlize_title(slug)
        if slug != slug_original:
            return HttpResponseRedirect(reverse('wiki_page_undo', args=[slug, revision_pk]))
        try:
            wiki_pages = WikiPage.objects.published(for_user=(request.user))
            wiki_page = wiki_pages.get(slug=slug)
        except WikiPage.DoesNotExist:
            ...
            if request.method == 'POST':
                ...
                if src_revision.user:
                    description = _('Undid revision of %(time)s by %(user)s.') % {'time':src_revision.created, 
                     'user':src_revision.user.username}
                else:
                    description = _('Undid anonymous revision of %(time)s.') % {'time': src_revision.created}
                prev_revision = None
                try:
                    prev_revision = WikiPageRevision.objects.filter(page=wiki_page,
                      created__lt=(src_revision.created)).order_by('-created')[0]
                ...
                if False in results:
                    ...
                    return HttpResponseRedirect('%s?%s' % (
                     ...
                     urlencode(urldata)))
                form = WikiPageForm(data=(request.POST or None), instance=wiki_page, initial={'content':content, 
                 'summary':description})
...
class Wiki_page_edit(TemplateView):
    ...
    def get(self, request, slug, template='mezawiki/wiki_page_edit.html'):
        ...
        if not wiki_page.can_edit_wikipage(request.user):
            return HttpResponseForbidden(_("You don't have permission to edit this wiki page."))
        else:
            context = {'wiki_page':wiki_page, 
             'form':form,  'title':deurlize_title(slug)}
            return render(request, template, context)
...
class Wiki_page_new(TemplateView):
    ...
    def get(self, request, template='mezawiki/wiki_page_new.html'):
        ...
        if not can_add_wikipage(request.user):
            return HttpResponseForbidden(_("You don't have permission to create wiki page."))
        else:
            if request.method == 'POST':
                ...
                if form.is_valid():
                    ...
                    revision.save()
                    return HttpResponseRedirect(reverse('wiki_page_detail', args=[page.slug]))
            else:
