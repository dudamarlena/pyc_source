...
# [GCC 8.4.0]
# Embedded file name: c:\users\landon\dropbox\documents\pycharmprojects\mezzanine-wiki\mezzanine_wiki\mdx_wikilinks_extra.py
...
class WikiLinkExtraExtension(markdown.Extension):
    ...
    def __init__(self, configs):
        self.config = {'base_url':[
         ...
         'build_url':[
          build_url, 'Callable formats URL from label.']}
        for key, value in configs.items():
            self.setConfig(key, value)

    def extendMarkdown(self, md, md_globals):
        self.md = md
        WIKILINK_RE = '\\[\\[([\\w0-9_ -]+)(\\|([\\w0-9_ - ]+))?\\]\\]'
