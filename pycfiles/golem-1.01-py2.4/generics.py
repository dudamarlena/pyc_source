...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/golem/helpers/generics.py
# Compiled at: 2008-09-30 12:16:36
    ...
    def __new__(cls, name, bases, dct):
        ...
        def __mod_value(obj):
            if obj.warnings and hasattr(obj, 'unit'):
                sys.stderr.write('WARNING: performing operation on unit-bearing quantity - unit info will be lost\n')

        for attr in ['__add__', '__sub__', '__mul__', '__floordiv__', '__mod__', '__divmod__', '__lshift__', '__rshift__', '__and__', '__xor__', '__or__', '__div__', '__truediv__', '__radd__', '__rsub__', '__rmul__', '__rdiv__', '__rtruediv__', '__rfloordiv__', '__rmod__', '__rdivmod__', '__rpow__', '__rlshift__', '__rrshift__', '__rand__', '__rxor__', '__ror__', '__iadd__', '__isub__', '__imul__', '__idiv__', '__itruediv__', '__ifloordiv', '__imod__', '__ilshift__', '__irshift__', '__iand__', '__ixor__', '__ior__', '__coerce__']:
        ...
        for attr in ['__neg__', '__pos__', '__abs__', '__invert__', '__complex__', '__int__', '__float__', '__long__', '__oct__', '__hex__', '__index__']:
        ...
        def __mod_cont(obj):
            if obj.warnings and hasattr(obj, 'unit'):
                sys.stderr.write('WARNING: modifying contents of unit-bearing container - unit info will be lost\n')
...
from URI 'resource', produce an RDF/XML fragment of the form: ::
    ...
    """
    descnode = etree.Element('{http://www.w3.org/1999/02/22-rdf-syntax-ns#}Description')
    descnode.attrib['{http://www.w3.org/1999/02/22-rdf-syntax-ns#}about'] = resource
    if x:
        for y in x:
            if property == None:
                property = x.uri
            ...
            payload = etree.Element('{http://www.w3.org/1999/02/22-rdf-syntax-ns#}Description')
            payload.attrib['{http://www.w3.org/1999/02/22-rdf-syntax-ns#}about'] = fragment_uri
            payload_value = etree.Element('golemrdf:value')
            payload_value.attrib['{http://www.w3.org/1999/02/22-rdf-syntax-ns#}datatype'] = 'http://example.com/json/'
            payload_value.text = json_single(y)
            payload.append(payload_value)
            if hasattr(y, 'unit'):
                payload_units = etree.Element('golemrdf:units')
...
def print_rdf(x, datatype_uri=None, about=None):
    if about == None:
        ...
        fileuri = about
    descnode = etree.Element('{http://www.w3.org/1999/02/22-rdf-syntax-ns#}Description')
    descnode.attrib['{http://www.w3.org/1999/02/22-rdf-syntax-ns#}about'] = fileuri
    if x:
        for y in x:
            if datatype_uri:
            ...
            valnode.attrib['{http://www.w3.org/1999/02/22-rdf-syntax-ns#}datatype'] = 'http://example.com/json'
