...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/golem/helpers/dataset.py
# Compiled at: 2008-09-09 16:33:39
...
``golem.db.fs`` driver (``dbformat`` in the class signature.)"""
    ...
    def __init__(self, dbfile, dbformat, dictionaryfile, dictionarynamespace):
        self.dblibrary = dbformat
        self.database = dbformat.xmldb(dbfile)
        self.dictionary = self.namespaced_dictionary(dictionaryfile, dictionarynamespace)

    def register(self, xconcepts, yconcepts, xpredicate=None, ypredicate=None, xreducer=None, yreducer=None):
        ...
        xclist = golem.db.conceptlist(*[ self.dictionary.concept(c) for c in xconcepts ])
        yclist = golem.db.conceptlist(*[ self.dictionary.concept(c) for c in yconcepts ])
        ...
        self.fit = golem.helpers.function.Fit(self.database, xclist, yclist, xreducer=xreducer, yreducer=yreducer)
    ...
    def makeplot(self, format, graphfile):
        ...
        plt = golem.helpers.output.pelote(title='%s v %s' % (self.xconcept.term, self.yconcept.term))
    ...
    def go(self, graphfile, output=sys.stdout, format='verbose', cache=None):
...
* ``dictionarynamespace``: Namespace of the above dictionary.
    ...
    def __init__(self, cachefile, dbfile, dbformat, dictionaryfile, dictionarynamespace):
        self.cachefile = cachefile
        super(self, caching_dataset).__init__(dbfile, dbformat, dictionaryfile, dictionarynamespace)
        self.loadcache()
        self.savecache()
    ...
    def go(self, graphfile, output=sys.stdout, format='verbose', cache=None):
        ...
        if cache == None:
            cache = self.cache
        res = super(caching_dataset, self).go(graphfile, output=sys.stdout, format=format, cache=cache)
        self.savecache()
        return res


def test():
    pvse = dataset('/Users/adw27/Documents/workspace/golem/trunk/bin/ossiaexamples', golem.db.fs, 'ossiakiln2.xml', 'http://www.esc.cam.ac.uk/ossia')
