...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/picka/picka.py
# Compiled at: 2016-09-01 11:59:55
...
from string import ascii_letters, ascii_uppercase, digits, punctuation, ascii_lowercase
...
import picka_utils
path = join(dirname(__file__) + '/db.sqlite')
...
def html_name():
    ...
    Examples:
      ...
      '808080'
    """
    res = engine.execute('SELECT name, hex FROM html_colors ORDER BY RANDOM() LIMIT 1;')
...
def mime_type():
    ...
    """
    res = engine.execute('SELECT name, extension FROM mimes ORDER BY                           random() LIMIT 1;')
    return AttrDict([ dict(d) for d in res.fetchall() ][0])


def ipv4():
    return inet_ntoa(pack('>I', randint(1, 4294967295)))
...
def areacode(state=None):
    ...
    if state:
        ...
        res = engine.execute('SELECT areacode, state FROM areacodes ORDER BY random() LIMIT 1;')
    return AttrDict([ dict(d) for d in res.fetchall() ][0])
...
def calling_code(country=False):
    ...
    if country:
        ...
        res = engine.execute('SELECT country, calling_code FROM calling_codes ORDER BY random() LIMIT 1;')
    return AttrDict([ dict(d) for d in res.fetchall() ][0])
...
def foreign_characters(i):
    foreign_chars = 'ƒŠŒŽšœžŸÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕ        ÖØÙÚÛÜÝÞßàáâãäåæçèéêëìíîïðñòóôõöøùúûüýþÿ'
...
def timezone_offset(dst=True, utc=True):
    ...
    data = {}
    if dst:
        res = engine.execute('SELECT DISTINCT(dst) FROM timezones ORDER BY random() LIMIT 1;')
        data['dst'] = res.fetchall()[0][0]
    if utc:
        res = engine.execute('SELECT DISTINCT(utc) FROM timezones ORDER BY random() LIMIT 1;')
...
def timezone_offset_country():
    ...
    Examples:
      ...
      AttrDict({'country': u'Maldives'})
    """
    res = engine.execute('SELECT country FROM timezones ORDER BY random() LIMIT 1;')
...
def month_day():
    ...
    if data.month in ('January', 'March', 'May', 'July', 'August', 'October', 'December'):
...
def name(sex=None):
    ...
    """
    _first = [
     initial(period=True).upper(), set_of_initials(2)[0]]
    ...
    return AttrDict({'first': unicode(choice(_first)), 
...
def email(length=8, domain='@example.com'):
    ...
    return AttrDict({'length': length, 
       'domain': domain, 
       'email': ('').join(choice(ascii_lowercase) for _ in range(length)) + domain})


def password(case='mixed', length=6, output_format='letters', special_chars=False):
    choices = ''
    if output_format in ('letters', 'alphanumeric'):
        cases = {'upper': ascii_uppercase, 'mixed': ascii_letters, 
...
def gender(extended=False):
    ...
    """
    if extended:
        res = engine.execute('SELECT gender FROM gender_extended ORDER BY random() LIMIT 1;')
...
def language():
    ...
    res = engine.execute('SELECT name FROM languages ORDER BY random() LIMIT 1;')
    return AttrDict([ dict(d) for d in res.fetchall() ][0])
...
def social_security_number(state='NY'):
    ...
    return ('{0}-{1}-{2}').format(randrange(x[0], x[1] + 1), number(2), number(4))
...
def drivers_license(state='NY'):
    ...
    if state == 'WA':
        ...
        return password(case='upper', length=i, output_format='alphanumeric', special_chars=False)
...
def business_title():
    ...
    This function will return business titles.     :tip: They are generic business titles.
...
def company_name():
    ...
    res = engine.execute('SELECT name FROM companies ORDER BY random() LIMIT 1;')
    return AttrDict([ dict(d) for d in res.fetchall() ][0])


def creditcard(card_type=None):
    card_types = {'amex': {'prefixes': [
                           '34', '37'], 
                'length': [
                         15]}, 
       'diners-carte-blance': {'prefixes': [
                                          300, 301, 302, 304, 305], 
                               'length': [
                                        14]}, 
       'diners-international': {'prefixes': [
                                           36], 
                                'length': [
                                         14]}, 
       'diners-uscanada': {'prefixes': [
                                      54, 55], 
                           'length': [
                                    16]}, 
       'discover': {'prefixes': [
                               '6011'] + [ str(i) for i in range(622126, 622926) ] + [
                               '644', '645', '646', '647', '648', '649', '65'], 
                    'length': [
                             16]}, 
       'jcb': {'prefixes': [ str(i) for i in range(3528, 3590) ], 'length': [
                        16]}, 
       'laser': {'prefixes': [
                            '6304', '6706', '6771', '6709'], 
                 'length': range(16, 20)}, 
    ...
    result += ('').join(str(choice(range(10))) for _ in range(length - len(result) - 1))
    total = sum(_return_int(result[-2::-2])) + sum(_return_int(('').join([ str(i * 2) for i in _return_int(result[::-2]) ])))
...
def street_name():
    ...
    res = engine.execute('SELECT name FROM streetnames ORDER BY random() LIMIT 1;')
    return AttrDict([ dict(d) for d in res.fetchall() ][0])
...
def street_type():
    ...
    res = engine.execute('SELECT name FROM us_street_types ORDER BY random() LIMIT 1;')
    return AttrDict([ dict(d) for d in res.fetchall() ][0])
...
def apartment_number():
    ...
    If you are looking for a specific format, you might be interested    in using string formatting instead.
    """
    ...
    alt = ('{} {}-{}{}').format(_type, choice(directions), randint(1, 999), letter)
    return AttrDict({'apartment_number': choice([short, _long, alt])})


def city():
    """This function will produce a city."""
    res = engine.execute('SELECT city FROM us_cities ORDER BY random() LIMIT 1;')
    return AttrDict([ dict(d) for d in res.fetchall() ][0])
...
def city_with_state():
    ...
    """
    res = engine.execute('SELECT city, state FROM us_cities ORDER BY random() LIMIT 1;')
...
def state_abbreviated():
    ...
    """
    res = engine.execute('SELECT abbreviation FROM states ORDER BY random() LIMIT 1;')
    return AttrDict([ dict(d) for d in res.fetchall() ][0])
...
def country():
    ...
    res = engine.execute('SELECT country_name FROM countries ORDER BY random() LIMIT 1;')
    return AttrDict([ dict(d) for d in res.fetchall() ][0])
...
def initial(period=False):
    ...
    """
    return ('{0}{1}').format(choice(ascii_uppercase), '.' if period else '')


def set_of_initials(i=3):
    """Returns initials with period seperators."""
    return [
     ('').join(initial(True) for _ in xrange(i))]


def surname():
    """Returns a randomly chosen surname."""
    res = engine.execute('SELECT name FROM surname ORDER BY random() LIMIT 1;')
    return AttrDict([ dict(d) for d in res.fetchall() ][0])
...
def unit_type():
    res = engine.execute('SELECT name, abbreviation FROM us_unit_types ORDER BY random() LIMIT 1;')
    return AttrDict([ dict(d) for d in res.fetchall() ][0])
...
class _Book:
    ...
    import os
    _path = os.path.join(os.path.abspath(os.path.dirname(__file__)) + '/book_sherlock.txt')
...
def sentence(num_words=20, chars=''):
    ...
    words = (' ').join(choice(word_list) for _ in xrange(num_words))
    if not chars:
        return words
    return words[:chars]
...
def sentence_actual(min_words=3, max_words=1000):
    ...
    raise Exception(("Couldn't find a sentence between         {0} and {1} words long").format(min_words, max_words))
    return


def trash(picka_function):
    """
     :Summary: This method takes a function you pass in, and joins     the output with _random punctuation.
     :Date: Tue Feb 22 15:31:12 EST 2011.
     :Usage: picka.trash(picka.name) >>> 'D#o}y>l~e^'
    """
    return ('').join([ str(char) + choice(str(punctuation)) for char in picka_function()
                     ])
...
def image(filepath, length=250, width=250, a=0):
    ...
    im = Image.new('RGBA', tuple((length, width)), tuple(rgba(format='list', a=0)))
