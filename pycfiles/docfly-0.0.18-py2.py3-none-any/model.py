...
# [GCC 8.4.0]
# Embedded file name: docfly/pkg/picage/model.py
            ...
            for p in self.path.iterdir():
                if p.is_dir():
                    if Path(p, '__init__.py').exists():
                        pkg = Package(name=name + '.' + p.basename, path=p, parent=self, is_single_file=False)
                        self.sub_packages[p.basename] = pkg
                elif p.ext == '.py' and p.fname != '__init__':
                    module = Module(name=name + '.' + p.fname, path=p, parent=self, is_single_file=True)
