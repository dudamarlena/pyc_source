...
# [GCC 8.4.0]
# Embedded file name: docfly/api_reference_doc.py
...
def is_ignored(mod_or_pkg, ignored_package):
    u"""Test, if this :class:`docfly.pkg.picage.Module`
...
def package_render(self, ignored_package):
    return TC.package.render(package=self, ignored_package=ignored_package, is_ignored=is_ignored)
    ...
    def fly(self):
        ...
        for pkg, parent, sub_packages, sub_modules in self.package.walk():
            if not is_ignored(pkg, self.ignored_package):
                ...
                make_file(init_path.abspath, self.generate_package_content(pkg))
                for mod in sub_modules:
                    if not is_ignored(mod, self.ignored_package):
                        module_path = Path(dir_path, mod.shortname + '.rst')
                        make_file(module_path.abspath, self.generate_module_content(mod))
