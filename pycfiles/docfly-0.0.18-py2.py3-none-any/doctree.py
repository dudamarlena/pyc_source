...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/docfly-project/docfly/doctree.py
# Compiled at: 2020-04-15 23:17:10
"""
...
class ArticleFolder(object):
    ...
    **中文文档**
        ...
        for cursor_line in rst_content.split('\n'):
            for header_bar_char in header_bar_char_list:
                if cursor_line.startswith(header_bar_char):
                    ...
                    if flag_full_bar_char and flag_line_length_greather_than_1 and flag_previous_line_not_empty:
    ...
    def toc_directive(self, maxdepth=1):
        ...
        :return:
        """
        articles_directive_content = TC.toc.render(maxdepth=maxdepth, article_list=self.sub_article_folders)
...
class DocTree(object):
    ...
    @classmethod
    def fly(cls, conf_path, docname, source, maxdepth=1):
        """
        ...
        """
        msg = '``.. articles::`` directive is going to be deprecated. use ``.. autodoctree`` instead.'
        warnings.warn(msg, FutureWarning)
        directive_pattern = '.. articles::'
        if directive_pattern not in source:
            return source
        af = ArticleFolder(dir_path=Path(Path(conf_path).parent, docname).parent.abspath)
