...
# [GCC 8.4.0]
# Embedded file name: docfly/pkg/textfile.py
...
_strip_method_mapping = {'none': no_strip, 
   'left': left_strip, 
   'right': right_strip, 
   'both': both_strip}

def readlines(path, encoding='utf-8', skiplines=None, nlines=None, strip='right'):
    u"""skip n lines and fetch the next n lines.
    ...
    **中文文档**
    ...
    if strip_method in _strip_method_mapping:
        ...
        raise ValueError("'strip' keyword has to be one of None, 'left', 'right', 'both'.")
...
def readchunks(path, encoding='utf-8', skiplines=None, chunksize=None, strip='right'):
    u"""skip n lines and fetch the next n lines as a chunk, and repeat fetching.
