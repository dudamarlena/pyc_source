...
# [GCC 8.4.0]
# Embedded file name: /home/aliman/src/github/alimanfoo/vcfnp/vcfnp/table.py
# Compiled at: 2016-07-16 04:07:55
...
from vcfnp.array import _filenames_from_arg, _warn_duplicates, _variants_fields
...
class VariantsTable(object):
    ...
    def __init__(self, filename, region=None, fields=None, exclude_fields=None, arities=None, flatten_filter=False, fill=b'.', flatteners=None):
    ...
    def __iter__(self):
        ...
        flatteners = tuple((flatteners[f] if f in flatteners else None) for f in fields)
        debug(flatteners)
        header = list()
        for f, flattener in zip(fields, flatteners):
            if self.flatten_filter and f == 'FILTER':
                ...
                if self.arities is not None and f in self.arities and self.arities[f] > 1:
        ...
        header = tuple(header)
        data = itervariantstable(vcf_fns=vcf_fns, region=self.region, fields=fields, arities=arities, info_types=info_types, parse_info=parse_info, filter_ids=filter_ids, flatten_filter=self.flatten_filter, fill=self.fill, flatteners=flatteners)
