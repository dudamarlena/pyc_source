...
# [GCC 8.4.0]
# Embedded file name: /home/niedbalski/src/vcs/vcs/backends/base.py
...
from vcs.exceptions import ChangesetError, EmptyRepositoryError, NodeAlreadyAddedError, NodeAlreadyChangedError, NodeAlreadyExistsError, NodeAlreadyRemovedError, NodeDoesNotExistError, NodeNotChangedError, RepositoryError
...
class BaseChangeset(object):
    ...
    **Attributes**
    ...
    def __unicode__(self):
        return '%s:%s' % (self.revision, self.short_id)
    ...
    def add(self, *filenodes):
        ...
        for node in filenodes:
            if node.path in (n.path for n in self.added):
                raise NodeAlreadyAddedError('Such FileNode %s is already marked for addition' % node.path)
    ...
    def check_integrity(self, parents=None):
        ...
        if not self.parents:
            ...
            if len(parents) == 0:
                try:
                    parents = [
                     self.repository.get_changeset(), None]
                except EmptyRepositoryError:
                    parents = [
                     None, None]
        ...
        for p in parents:
            for node in self.added:
                try:
                    ...
                    raise NodeAlreadyExistsError('Node at %s already exists at %s' % (
                     node.path, p))
        ...
        if self.changed and missing:
            raise NodeDoesNotExistError('Node at %s is missing (parents: %s)' % (
             node.path, parents))
        if self.changed and not_changed:
            raise NodeNotChangedError("Node at %s wasn't actually changed since parents' changesets: %s" % (
             not_changed.pop().path,
             parents))
        if self.removed and not parents:
            raise NodeDoesNotExistError('Cannot remove node at %s as there were no parents specified' % self.removed[0].path)
        really_removed = set()
        for p in parents:
        ...
        if not_removed:
            raise NodeDoesNotExistError('Cannot remove node at %s from following parents: %s' % (
             not_removed[0], parents))
        return
...
class EmptyChangeset(BaseChangeset):
    ...
    def __init__(self, cs='0' * 40, repo=None, requested_revision=None, alias=None, revision=-1, message='', author='', date=None):
        self._empty_cs = cs
