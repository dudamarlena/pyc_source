...
def get_num_lines(filename, ignore_comments=True):
    ...
    out = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT).communicate()[0]
...
def is_complete(filename):
    ...
    """
    if filename.endswith('.gz'):
        raise NotImplementedError('is_complete not implemented for compressed files')
...
def touch_file(filename, mode=438, times=None, dir_fd=None, ref=None, **kwargs):
    ...
    with os.fdopen(os.open(filename, flags=flags, mode=mode, dir_fd=dir_fd)) as (fhandle):
        os.utime((fhandle.fileno() if os.utime in os.supports_fd else filename), dir_fd=(None if os.supports_fd else dir_fd), **kwargs)
    return
...
def snip(filename, extension=None, alt_extension=None, strip_path=False):
...
def nested_iter(nested):
    ...
    if isinstance(nested, collections.Mapping):
        for (key, value) in nested.items():
            if not isinstance(value, collections.Mapping) and not isinstance(value, list):
                yield (
                 nested, key, value)
            else:
...
class FilePool:
    ...
    def __init__(self, output_pattern=None, header=None, force=True):
        self.mFiles = {}
...
def read_map(infile, columns=(0, 1), map_functions=(
 str, str), both_directions=False, has_header=True, dtype=dict):
...
def read_list(infile, column=0, map_function=str, map_category={}, with_title=False):
...
def readMultiMap(infile, columns=(0, 1), map_functions=(
 str, str), both_directions=False, has_header=False, dtype=dict):
...
def write_table(outfile, table, columns=None, fillvalue=''):
    ...
    if type(table) == dict:
        ...
        data = list(itertools.zip_longest(fillvalue=fillvalue, *data))
        for d in data:
...
def write_matrix(outfile, matrix, row_headers, col_headers, row_header=''):
    """write a numpy matrix to outfile.
    *row_header* gives the title of the rows
    """
