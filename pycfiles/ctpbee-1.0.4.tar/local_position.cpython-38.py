...
# [GCC 8.4.0]
# Embedded file name: /home/somewheve/PycharmProjects/ctpbee/ctpbee/looper/local_position.py
# Compiled at: 2019-11-12 01:47:55
# Size of source mod 2**32: 19590 bytes
"""
Notice : 神兽保佑 ，测试一次通过
//      
//      ┏┛ ┻━━━━━┛ ┻┓
...
from ctpbee.constant import PositionData, Offset, Direction, OrderRequest, OrderData, Exchange, TickData, EXCHANGE_MAPPING
...
class PositionHolding:
        ...
        if trade.direction == Direction.LONG:
            if trade.offset == Offset.OPEN:
                ...
                if trade.offset == Offset.CLOSETODAY:
                    ...
                    if trade.offset == Offset.CLOSEYESTERDAY:
            ...
            if order.offset == Offset.OPEN:
                ...
                if order.direction == Direction.LONG:
                    if order.offset == Offset.CLOSETODAY:
                        ...
                        if order.offset == Offset.CLOSEYESTERDAY:
    ...
    def convert_order_request_shfe(self, req: OrderRequest):
        ...
        if req.offset == Offset.OPEN:
            return [
             req]
        elif req.direction == Direction.LONG:
    ...
    def calculate_pnl(self):
        ...
        try:
            if self.long_pos == self.long_yd:
                self.long_pnl = self.long_pos * (self.last_price - self.long_price / (self.size * self.long_pos)) * self.size
            if self.long_pos != self.long_yd:
        ...
        try:
            if self.short_pos == self.short_yd:
                self.short_pnl = self.short_pos * (self.short_price / (self.size * self.short_pos) - self.last_price) * self.size
            if self.short_pos != self.short_yd:
    ...
    def calculate_stare_pnl(self):
        ...
        try:
            if self.long_pos == self.long_yd:
                self.long_stare_pnl = self.long_pos * (self.last_price - self.pre_close_price / (self.size * self.long_pos)) * self.size
            if self.long_pos != self.long_yd:
        ...
        try:
            if self.short_pos == self.short_yd:
                self.short_stare_pnl = self.short_pos * (self.pre_close_price / (self.size * self.short_pos) - self.last_price) * self.size
            if self.short_pos != self.short_yd:
    ...
    def get_position_by_direction(self, direction):
        if direction == Direction.LONG:
            return PositionData(symbol=(self.symbol),
