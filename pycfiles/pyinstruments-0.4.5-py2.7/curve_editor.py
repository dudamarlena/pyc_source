...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pyinstruments\curvefinder\gui\curve_editor.py
# Compiled at: 2013-12-16 17:29:56
...
class CurveEditor(QtGui.QMainWindow, object):
    ...
    def __init__(self):
        ...
        self.addDockWidget(QtCore.Qt.DockWidgetArea(QtCore.Qt.LeftDockWidgetArea), self.search_widget)
        self.toolbar = CurveEditorToolBar(self)
        self.addToolBar(self.toolbar)
        self.toolbar.popup_unread_activated.connect(self.activate_popup_unread)
        self.toolbar.popup_unread_deactivated.connect(self.deactivate_popup_unread)
        ...
        if default_json != '':
            ...
            for key in ['date_lte', 'date_gte', 'date_equals']:
                ...
                if val:
                    defaults[key] = (
                     enabled, dateutil.parser.parse(val))
        ...
        self.popup_timer.setInterval(500)
        self.popup_unread = False
        self.setWindowTitle('curvefinder: ' + DATABASE_FILE)
        icon_file = os.path.join(os.path.split(pyinstruments.__file__)[0], 'icons', 'plot.png')
