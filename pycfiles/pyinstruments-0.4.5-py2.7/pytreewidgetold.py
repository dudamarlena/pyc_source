...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pyhardware\config\gui\pytreewidgetold.py
...
class PyTreeWidget(QtGui.QTreeWidget):
    ...
    def __init__(self, col_names_and_types, main_key=0, is_editable=True):
        ...
        for index, (name, type_) in enumerate(col_names):
            self.headerItem().setText(index, QtGui.QApplication.translate('MainWindow', name, None, QtGui.QApplication.UnicodeUTF8))
    ...
    def __getitem__(self, val):
        ...
        raise KeyError('no element with value ' + str(val) + ' in column ' + str(self.main_key) + ' in the tree')
