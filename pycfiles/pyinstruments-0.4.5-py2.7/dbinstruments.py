...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pyinstruments\pyhardwaredb\dbinstruments.py
# Compiled at: 2013-12-17 06:45:32
...
def _get_initial_defaults(self):
    return {'default_name': 'curve_%s' % self.__class__.__name__, 'default_window': '%s' % self.__class__.__name__, 
       'tags': [], 'comment': ''}
...
def _save_curve(self, curve):
    ...
    try:
        try:
            ...
            self._save_defaults(default_name=curve.params['name'], default_window=curve.params['window'], comment=curve.params['comment'], tags=curve.tags)
...
def _setup_fetch_buttons(self, widget):
    ...
    self._dbwidget.curve_tag_widget.need_refresh = True
