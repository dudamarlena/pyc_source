...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pyinstruments\curvefinder\gui\curve_search_widget\list_curve_widget.py
# Compiled at: 2013-12-15 11:06:43
...
class ListCurveWidget(QtGui.QWidget, object):
    ...
    def __init__(self, parent):
        ...
        self._tree_widget.itemSelectionChanged.connect(self._current_item_changed)
    ...
    def ids(self):
        return [ self._tree_widget.topLevelItem(index).pk for index in range(self._tree_widget.topLevelItemCount())
               ]
    ...
    def refresh_one_id(self, id):
        ...
        item_list = self._tree_widget.findItems(str(id), QtCore.Qt.MatchFlag(0), column=0)
        if item_list:
    ...
    def refresh(self):
        ...
        for curve in curves[sl]:
            item_list = self._tree_widget.findItems(str(curve.id), QtCore.Qt.MatchFlag(0), column=0)
            if item_list:
                item = item_list[0]
                item.update(curve)
            else:
    ...
    def contextMenuEvent(self, event):
        ...
        def create_csv(curves, buffer):
            df = pandas.DataFrame(dict([ (str(curve.id) + '_' + curve.params['name'], curve.data) for curve in curves
                                       ]))
