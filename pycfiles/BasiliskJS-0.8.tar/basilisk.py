...
# [GCC 8.4.0]
# Embedded file name: /Users/rinat/project/BasiliskJS/basilisk/basilisk.py
# Compiled at: 2017-12-31 15:03:14
"""
python webkit
~~~~~~~~~~~~~~~~~~~~~
...
class PhantomJS(Basilisk):
    ...
    def __create_node(data):
        ...
        :return: node
        """
        necessary = "const webPage=require('webpage');const page=webPage.create();var data = {};var k = 0;var url = [];"
        if data['screenshot']:
            screenshot = "page.render('%s');" % data['image_name']
        else:
            screenshot = ''
        if data['content']:
        ...
        if data['method'].upper() == 'GET':
            request = "page.open( '%s',function(status){data['status']=status;%s%s});" % (
             data['url'], content,
             data['open_fun'])
        elif data['method'].upper() == 'POST':
            request = "var postBody = %s;page.open('%s','POST', postBody,function(status){data['status'] = status;%s%s});" % (
             data['post_data'],
             data['url'],
             content,
             data['open_fun'])
        ...
        image_size = 'page.viewportSize=%s;' % str(data['image_size'])
        if data['add_cookie']:
            add_cookie = 'phantom.addCookie(%s);' % str(data['add_cookie'])
        else:
            add_cookie = ''
        if data['load_images']:
        ...
        if data['get_cookies']:
            ...
            cookies = ''
        if data['user_agent']:
            user_agent = "page.settings.userAgent='%s';" % data['user_agent']
        else:
            user_agent = ''
        load_images = 'page.settings.loadImages=%s;%s;' % (
         data['load_images'],
         user_agent)
        param = ('{0}{1}{2}').format(image_size, add_cookie, load_images)
        load_finished = "page.onLoadFinished=function(){if (k==%i) {data['urls'] = url;%sconsole.log(JSON.stringify(data));%sphantom.exit();}%s};" % (
         data['conversion'],
         cookies,
         screenshot,
         data['load_finished_fun'])
        load_started = 'page.onLoadStarted=function() {%s};' % data['load_started_fun']
        if data['resource_error']:
            resource_error = "page.onResourceError=function(resourceError) {data['error']=resourceError;};"
        else:
            resource_error = ''
        url_changed = 'page.onUrlChanged=function(targetUrl) {k += 1;url.push(targetUrl);};'
        node = ('{0}{1}{2}{3}{4}{5}{6}').format(necessary, param, request, load_finished, load_started, resource_error, url_changed)
    ...
    def evaluate(self, js):
        ...
        """
        self.__js = "data['js']=page.evaluate(function() {%s});" % js
    ...
    def include_js(self, js):
        ...
        :return: function_js
        """
        self.__include += "page.includeJs('%s');" % js
    ...
    def get(self, url):
        ...
        self.__slots__['open_fun'] = ('{0}{1}').format(self.__js, self.__include)
        if self.core:
