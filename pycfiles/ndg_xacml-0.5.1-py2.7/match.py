...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/ndg/xacml/core/match.py
...
__contact__ = 'Philip.Kershaw@stfc.ac.uk'
__revision__ = '$Id: match.py 7955 2011-12-21 18:29:45Z rwilkinson $'
import logging
...
from ndg.xacml.core.functions import FunctionMap, functionMap, UnsupportedStdFunctionError, UnsupportedFunctionError
...
class MatchBase(XacmlCoreBase):
    ...
    __slots__ = ('__attributeValue', '__attributeDesignator', '__attributeSelector',
                 '__matchId', '__function', '__functionMap', '__loadFunctionFromId')
    ...
    def attributeValue(self, value):
        ...
        if not isinstance(value, AttributeValue):
            raise TypeError('Expecting %r type for "matchId" attribute; got %r' % (
    ...
    def attributeDesignator(self, value):
        ...
        if not isinstance(value, AttributeDesignator):
            raise TypeError('Expecting %r type for "attributeDesignator" attribute; got %r' % (
    ...
    def _setMatchId(self, value):
        ...
        if not isinstance(value, basestring):
            raise TypeError('Expecting string type for "matchId" attribute; got %r' % type(value))
        self.__matchId = value
        if self.__loadFunctionFromId:
            self.setFunctionFromMap(self.__functionMap)

    matchId = property(_getMatchId, _setMatchId, None, 'Match identifier for match function')
    ...
    def loadFunctionFromId(self, value):
        ...
        if not isinstance(value, bool):
            raise TypeError('Expecting %r type for "loadFunctionFromId" attribute; got %r' % (
             bool, type(value)))
    ...
    def setFunctionFromMap(self, functionMap):
        ...
        if self.matchId is None:
            raise AttributeError('"functionId" attribute must be set in order to retrieve the required function')
        functionClass = functionMap.get(self.matchId)
        if functionClass is NotImplemented:
            raise UnsupportedStdFunctionError('No match function class implemented for MatchId="%s"' % self.matchId)
        elif functionClass is None:
            raise UnsupportedFunctionError('<Apply> function namespace %r is not recognised' % self.matchId)
    ...
    def functionMap(self, value):
        ...
        if not isinstance(value, FunctionMap):
            raise TypeError('Expecting %r derived type for "functionMap" input; got %r instead' % (
             FunctionMap,
             type(value)))
    ...
    def evaluate(self, context):
        ...
        if self.attributeDesignator is not None:
            ...
            if self.attributeSelector is not None:
                ...
                raise XacmlContextError('No attribute designator or selector set for Target Match element %r with MatchId = %r and attributeValue = %r' % (
                 self.__class__.ELEMENT_LOCAL_NAME,
                 self.matchId,
                 self.attributeValue))
            attrMatchStatusValues = [
             False] * len(requestAttributeValues)
            ...
            for i, requestAttributeValue in enumerate(requestAttributeValues):
                attrMatchStatusValues[i] = matchFunction.evaluate(matchAttributeValue, requestAttributeValue)
                if attrMatchStatusValues[i] == True:
                    if log.getEffectiveLevel() <= logging.DEBUG:
                        log.debug('Target attribute value %r matches request attribute value %r matches using match function Id %r', matchAttributeValue, requestAttributeValue, self.matchId)
