...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/ndg/xacml/test/context/test_pdp_with_custom_attributevalue_types.py
...
__contact__ = 'Philip.Kershaw@stfc.ac.uk'
__revision__ = '$Id: test_pdp_with_custom_attributevalue_types.py 8078 2012-06-19 14:10:35Z pjkersha $'
...
from ndg.xacml.test import XACML_ESGFTEST1_FILEPATH, GroupRoleAttributeValue, ETreeGroupRoleDataTypeReader, GroupRoleBag, GroupRoleAtLeastOneMemberOf
from ndg.xacml.test.context import AnyUriAttributeValue, StringAttributeValue, SUBJECT_ID
...
class XacmlEvalPdpWithCustomAttrTypes(unittest.TestCase):
    ...
    SUBJECT_DOES_NOT_HAVE_ANY_OF_SPECIFIED_ROLES_ID = 'http://localhost/subject-does-not-have-any-of-specified-roles'

    @staticmethod
    def _createRequestCtx(resourceId, includeSubject=True, subjectGroupRoles=None, groupRoleAttributeId='urn:esg:attr', action='read'):
        ...
        if includeSubject:
            ...
            for group, role in subjectGroupRoles:
                ...
                groupRoleAttribute.attributeValues.append(GroupRoleAttributeValue())
    ...
    def test02SubjectDoesNotHaveAnyOfSpecifiedRolesForResource(self):
        request = self._createRequestCtx(self.__class__.SUBJECT_DOES_NOT_HAVE_ANY_OF_SPECIFIED_ROLES_ID, action='write')
        response = self.pdp.evaluate(request)
        self.failIf(response is None, 'Null response')
        for result in response.results:
