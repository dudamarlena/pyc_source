...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/ndg/xacml/parsers/etree/policydefaultsreader.py
# Compiled at: 2012-02-10 11:35:08
...
__contact__ = 'Philip.Kershaw@stfc.ac.uk'
__revision__ = '$Id: policydefaultsreader.py 7986 2012-01-13 11:39:34Z rwilkinson $'
...
class PolicyDefaultsReader(ETreeAbstractReader):
    ...
    def __call__(self, obj, common):
        ...
        if len(elem) != 1:
            raise XMLParseError('Expecting a single child element for PolicyDefaults element')
        if QName.getLocalPart(elem[0].tag) != xacmlType.XPATH_VERSION_ELEMENT_NAME:
            raise XMLParseError('Expecting a %r child element for PolicyDefaults element' % xacmlType.XPATH_VERSION_ELEMENT_NAME)
        xpathVersion = elem[0].text
        if xpathVersion is None:
            raise XMLParseError('No %r child element value set for PolicyDefaults element' % xacmlType.XPATH_VERSION_ELEMENT_NAME)
