...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/ndg/xacml/core/attributeselector.py
...
__contact__ = 'Philip.Kershaw@stfc.ac.uk'
__revision__ = '$Id: attributeselector.py 8010 2012-01-30 16:24:06Z rwilkinson $'
import logging
...
class AttributeSelector(Expression):
    ...
    __slots__ = ('__mustBePresent', '__requestContextPath', '__attributeValueFactory')
    ...
    def requestContextPath(self, value):
        ...
        if not isinstance(value, basestring):
            raise TypeError('Expecting %r type for "requestContextPath" attribute; got %r' % (
    ...
    def evaluate(self, context):
        ...
        if attributeValueClass is None:
            raise XMLParseError('No Attribute Value class available for type %r' % dataType)
        ...
        if len(attributeValueBag) == 0 and self.mustBePresent:
            raise MissingAttributeError('"MustBePresent" is set for AttrubuteSelector for path %r' % self.requestContextPath)
