...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/ndg/xacml/core/rule.py
...
__contact__ = 'Philip.Kershaw@stfc.ac.uk'
__revision__ = '$Id: rule.py 7955 2011-12-21 18:29:45Z rwilkinson $'
...
class Effect(object):
    ...
    def _setValue(self, value):
        ...
        if isinstance(value, Effect):
            ...
            raise TypeError('Expecting string or Effect instance for "value" attribute; got %r instead' % type(value))
        if value not in self.__class__.TYPES:
            raise AttributeError('Permissable effect types are %r; got %r instead' % (
             Effect.TYPES, value))
        self.__value = value
    ...
    def __eq__(self, effect):
        ...
        if isinstance(effect, Effect):
            ...
            raise TypeError('Expecting string or Effect instance for input effect value; got %r instead' % type(value))
        if value not in self.__class__.TYPES:
            raise AttributeError('Permissable effect types are %r; got %r instead' % (
             Effect.TYPES, value))
...
class Rule(XacmlCoreBase):
    ...
    __slots__ = ('__target', '__condition', '__description', '__id', '__effect')
    ...
    def _set_effect(self, value):
        ...
        if not isinstance(value, Effect):
            raise TypeError('Expecting %r type for "effect" attribute; got %r' % (
             Effect, type(value)))
        self.__effect = value

    effect = property(_get_effect, _set_effect, None, 'Rule effect attribute')
    ...
    description = property(_getDescription, _setDescription, doc='Rule Description text')
    ...
    def evaluate(self, context):
        ...
        try:
            ...
            if self.target is not None:
                ...
                if targetMatch:
                    log.debug('Match to request context for target in rule %r', self.id)
            else:
                log.debug('No target set in rule %r', self.id)
                targetMatch = True
            if not targetMatch:
                log.debug('No match to request context for target in rule %r returning NotApplicable status', self.id)
                decision = Decision.NOT_APPLICABLE
                return decision
            if self.condition is not None:
                conditionStatus = self.condition.evaluate(context)
            else:
                log.debug('No condition set for rule %r: setting condition status True', self.id)
            ...
            log.error('Error occurred evaluating rule %r, returning Indeterminate result to caller: %s', self.id, _traceback)
