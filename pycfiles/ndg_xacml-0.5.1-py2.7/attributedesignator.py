...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/ndg/xacml/core/attributedesignator.py
...
__contact__ = 'Philip.Kershaw@stfc.ac.uk'
__revision__ = '$Id: attributedesignator.py 7365 2010-08-25 15:41:20Z pjkersha $'
import logging
...
class AttributeDesignator(Expression):
    ...
    __slots__ = ('__attributeId', '__issuer', '__mustBePresent', '__attributeValueFactory')
    ...
    def attributeId(self, value):
        ...
        if not isinstance(value, basestring):
            raise TypeError('Expecting %r type for "attributeId" attribute; got %r' % (
    ...
    def issuer(self, value):
        ...
        """
        if not isinstance(value, basestring):
            raise TypeError('Expecting %r type for "issuer" attribute; got %r' % (
             basestring, type(value)))
        self.__issuer = value
    ...
    def mustBePresent(self, value):
        ...
        if not isinstance(value, bool):
            raise TypeError('Expecting %r type for "mustBePresent" attribute; got %r' % (
             bool, type(value)))
...
class SubjectAttributeDesignator(AttributeDesignator):
    ...
    def evaluate(self, context):
        ...
        _attributeMatch = lambda attr: attr.attributeId == attributeId and attr.dataType == dataType and _issuerMatch(attr.issuer)
        for subject in context.subjects:
            for attr in subject.attributes:
                if _attributeMatch(attr):
                    attributeValueBag.extend([ i for i in attr.attributeValues if i.dataType == dataType
                                             ])

            if context.ctxHandler is not None:
                log.debug('Making query to PIP for additional subject attributes to satify match')
                attributeValues = context.ctxHandler.pipQuery(context, self)
                if attributeValues is not None:
                    log.debug('PIP retrieved additional subject attributes: %r', attributeValues)
                    if len(attributeValueBag) > 0:
                        filtAttributeValues = [ attrVal for attrVal in attributeValues if attrVal not in attributeValueBag
                                              ]
                    else:
        ...
        if len(attributeValueBag) == 0 and self.mustBePresent:
            raise MissingAttributeError('"MustBePresent" is set for %r but no match for attributeId=%r, dataType=%r and issuer=%r' % (
