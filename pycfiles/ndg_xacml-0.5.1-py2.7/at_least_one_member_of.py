...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/ndg/xacml/core/functions/v1/at_least_one_member_of.py
...
__revision__ = '$Id: at_least_one_member_of.py 7955 2011-12-21 18:29:45Z rwilkinson $'
...
class AtLeastOneMemberOfBase(AbstractFunction):
    ...
    def evaluate(self, set1, set2):
        ...
        if not isinstance(set1, Bag):
            raise XacmlContextTypeError('Expecting %r derived type for "set1"; got %r' % (
             Bag, type(set1)))
        if set1.elementType != self.__class__.TYPE:
            raise XacmlContextTypeError('Expecting %r type elements for "set1"; got %r' % (
             self.__class__.TYPE, set1.elementType))
        if not isinstance(set2, Bag):
            raise XacmlContextTypeError('Expecting %r derived type for "set2"; got %r' % (
             Bag, type(set2)))
        if set2.elementType != self.__class__.TYPE:
            raise XacmlContextTypeError('Expecting %r type elements for "set2"; got %r' % (
...
class FunctionClassFactory(FunctionClassFactoryBase):
    ...
    """
    FUNCTION_NAMES = ('urn:oasis:names:tc:xacml:1.0:function:string-at-least-one-member-of',
