...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/ndg/xacml/parsers/etree/policysetreader.py
...
__contact__ = 'Philip.Kershaw@stfc.ac.uk'
__revision__ = '$Id: policysetreader.py 8078 2012-06-19 14:10:35Z pjkersha $'
...
class PolicySetReader(ETreeAbstractReader):
    ...
    def processElement(self, elem, common):
        ...
        policySet.version = elem.attrib.get(xacmlType.VERSION_ATTRIB_NAME) or xacmlType.DEFAULT_XACML_VERSION
        for childElem in getElementChildren(elem):
            ...
            if localName == xacmlType.DESCRIPTION_LOCAL_NAME:
                ...
                policySet.policyDefaults = PolicyDefaultsReader.parse(childElem, common)
                ...
                raise NotImplementedError()
            elif localName == xacmlType.POLICY_SET_COMBINER_PARAMETERS_LOCAL_NAME:
                raise NotImplementedError()
            elif localName == VariableDefinition.ELEMENT_LOCAL_NAME:
                VariableDefinitionReader = ReaderFactory.getReader(VariableDefinition)
                variableDefinition = VariableDefinitionReader.parse(childElem, common)
                ...
                policySet.policies.append(self._getReferencedPolicy(common, policyIdReference))
                ...
                policySet.policies.append(self._getReferencedPolicySet(common, policySetIdReference))
            elif localName == xacmlType.OBLIGATIONS_LOCAL_NAME:
                raise NotImplementedError('Parsing for Obligations element is not implemented')
            else:
                raise XMLParseError('XACML PolicySet child element name %r not recognised' % localName)
