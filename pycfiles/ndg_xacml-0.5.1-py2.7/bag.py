...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/ndg/xacml/core/functions/v1/bag.py
...
__revision__ = '$Id: bag.py 7955 2011-12-21 18:29:45Z rwilkinson $'
    ...
    def evaluate(self, *args):
        ...
        for i in args:
            if not isinstance(i, self.__class__.TYPE):
                raise TypeError('Expecting %r derived type for bag element; got %r' % (
                 self.__class__.TYPE, type(i)))
            bag.append(i)
...
class FunctionClassFactory(FunctionClassFactoryBase):
    ...
    """
    FUNCTION_NAMES = ('urn:oasis:names:tc:xacml:1.0:function:string-bag', 'urn:oasis:names:tc:xacml:1.0:function:boolean-bag',
                      'urn:oasis:names:tc:xacml:1.0:function:integer-bag', 'urn:oasis:names:tc:xacml:1.0:function:double-bag',
                      'urn:oasis:names:tc:xacml:1.0:function:time-bag', 'urn:oasis:names:tc:xacml:1.0:function:date-bag',
                      'urn:oasis:names:tc:xacml:1.0:function:dateTime-bag', 'urn:oasis:names:tc:xacml:1.0:function:anyURI-bag',
                      'urn:oasis:names:tc:xacml:1.0:function:hexBinary-bag', 'urn:oasis:names:tc:xacml:1.0:function:base64Binary-bag',
                      'urn:oasis:names:tc:xacml:1.0:function:dayTimeDuration-bag',
                      'urn:oasis:names:tc:xacml:1.0:function:yearMonthDuration-bag',
                      'urn:oasis:names:tc:xacml:1.0:function:x500Name-bag', 'urn:oasis:names:tc:xacml:1.0:function:rfc822Name-bag')
    FUNCTION_NS_SUFFIX = '-bag'
