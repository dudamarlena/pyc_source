...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/ndg/xacml/test/context/test_pdp_cmip5.py
# Compiled at: 2012-03-06 17:01:27
"""
...
class Test(XacmlContextBaseTestCase):
    ...
    def test01(self):
        request = self._createRequestCtx(self.__class__.RESOURCE_1_ID, subjectRoles=('cmip5_research', ))
    ...
    def test02(self):
        request = self._createRequestCtx(self.__class__.RESOURCE_2_ID, subjectRoles=('cmip5_research', ))
        print 'Starting request'
        start_time = time.time()
        response = self.pdp.evaluate(request)
        print 'Response received after %fs' % (time.time() - start_time)
    ...
    def test03(self):
        request = self._createRequestCtx(self.__class__.RESOURCE_3_ID, subjectRoles=('cmip5_research', ))
        print 'Starting request'
        start_time = time.time()
        response = self.pdp.evaluate(request)
        print 'Response received after %fs' % (time.time() - start_time)
    ...
    def test04(self):
        request = self._createRequestCtx(self.__class__.RESOURCE_4_ID, subjectRoles=('cmip5_research', ))
        print 'Starting request'
        start_time = time.time()
        response = self.pdp.evaluate(request)
        print 'Response received after %fs' % (time.time() - start_time)
