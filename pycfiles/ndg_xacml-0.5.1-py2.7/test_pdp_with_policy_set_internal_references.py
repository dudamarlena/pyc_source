...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/ndg/xacml/test/context/test_pdp_with_policy_set_internal_references.py
# Compiled at: 2012-06-19 10:10:36
"""NDG XACML PDP tests with policy set references resolved internally by ID
...
__contact__ = 'Philip.Kershaw@stfc.ac.uk'
__revision__ = '$Id: test_pdp_with_policy_set_internal_references.py 8078 2012-06-19 14:10:35Z pjkersha $'
...
class Test(XacmlContextBaseTestCase):
    ...
    XACML_POLICY_SET_FILENAME = 'policy_set_internal_references.xml'
    ...
    def test01(self):
        request = self._createRequestCtx(self.__class__.RESOURCE_DL1_ID, subjectRoles=('staff', ))
    ...
    def test02(self):
        request = self._createRequestCtx(self.__class__.RESOURCE_DL2_ID, subjectRoles=('staff', ))
        print 'Starting request'
        start_time = time.time()
        response = self.pdp.evaluate(request)
        print 'Response received after %fs' % (time.time() - start_time)
    ...
    def test03(self):
        request = self._createRequestCtx(self.__class__.RESOURCE_VIEW1_ID, subjectRoles=('admin', ))
        print 'Starting request'
        start_time = time.time()
        response = self.pdp.evaluate(request)
        print 'Response received after %fs' % (time.time() - start_time)
    ...
    def test04(self):
        request = self._createRequestCtx(self.__class__.RESOURCE_VIEW2_ID, subjectRoles=('staff', ))
        print 'Starting request'
        start_time = time.time()
        response = self.pdp.evaluate(request)
        print 'Response received after %fs' % (time.time() - start_time)
