...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/ndg/xacml/core/functions/v1/one_and_only.py
...
__revision__ = '$Id: one_and_only.py 7087 2010-06-25 11:23:09Z pjkersha $'
...
class OneAndOnlyBase(AbstractFunction):
    ...
    def evaluate(self, bag):
        ...
        if not isinstance(bag, Bag):
            raise XacmlContextError('Expecting %r derived type for "bag"; got %r' % (
             Bag, type(bag)))
        if bag.elementType != self.__class__.TYPE:
            raise XacmlContextError('Expecting %r type elements for "bag"; got %r' % (
             self.__class__.TYPE, bag.elementType))
        nBagElems = len(bag)
        if nBagElems != 1:
            raise XacmlContextError('Expecting single element for %r bag; got %r' % (
             self.__class__.TYPE, nBagElems))
        return bag[0]
...
class FunctionClassFactory(FunctionClassFactoryBase):
    ...
    """
    FUNCTION_NAMES = ('urn:oasis:names:tc:xacml:1.0:function:string-one-and-only',
