...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/ndg/xacml/core/rule_combining_alg.py
...
__contact__ = 'Philip.Kershaw@stfc.ac.uk'
__revision__ = '$Id: rule_combining_alg.py 7955 2011-12-21 18:29:45Z rwilkinson $'
import logging
log = logging.getLogger(__name__)
from abc import abstractmethod
from ndg.xacml.core.context.result import Decision
ALGORITHMS = ('urn:oasis:names:tc:xacml:1.0:rule-combining-algorithm:deny-overrides',
    ...
    def evaluate(self, rules, context):
        ...
        for rule in rules:
            ...
            if decision == Decision.DENY:
                ...
                continue
            if decision == Decision.PERMIT:
                log.debug('Rule %r permits, returning overall permit decision', rule.id)
                return Decision.PERMIT
            if decision == Decision.NOT_APPLICABLE:
                continue
            if decision == Decision.INDETERMINATE:
        ...
        if potentialPermit:
            log.debug('Rule found with potential permit but it evaluates to indeterminate, returning overall indeterminate decision')
            return Decision.INDETERMINATE
        if atLeastOneDeny:
            log.debug('At least one rule with a deny decision found, returning overall deny decision')
            return Decision.DENY
        if atLeastOneError:
            log.debug('At least one rule with an error found, returning overall indeterminate decision')
            return Decision.INDETERMINATE
        log.debug('No rules were applicable to the request, returning overall not applicable decision')
        return Decision.NOT_APPLICABLE
...
class FirstApplicableRuleCombiningAlg(RuleCombiningAlgInterface):
    ...
    def evaluate(self, rules, context):
        ...
        for rule in rules:
            decision = rule.evaluate(context)
            if decision == Decision.DENY:
                log.debug('Rule %r denies, returning overall deny decision', rule.id)
                return Decision.DENY
            if decision == Decision.PERMIT:
                log.debug('Rule %r permits, returning overall permit decision', rule.id)
                return Decision.PERMIT
            if decision == Decision.NOT_APPLICABLE:
                continue
            if decision == Decision.INDETERMINATE:
                log.debug('Rule %r is indeterminate, returning overall indeterminate decision', rule.id)
                return Decision.INDETERMINATE
...
class RuleCombiningAlgClassFactory(object):
    ...
    DEFAULT_MAP.update({'urn:oasis:names:tc:xacml:1.0:rule-combining-algorithm:deny-overrides': DenyOverridesRuleCombiningAlg, 
       'urn:oasis:names:tc:xacml:1.0:rule-combining-algorithm:permit-overrides': PermitOverridesRuleCombiningAlg, 
       'urn:oasis:names:tc:xacml:1.0:rule-combining-algorithm:first-applicable': FirstApplicableRuleCombiningAlg})
    __slots__ = ('__map', )
