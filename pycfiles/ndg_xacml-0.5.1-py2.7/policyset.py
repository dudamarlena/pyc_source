...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/ndg/xacml/core/policyset.py
...
__contact__ = 'Philip.Kershaw@stfc.ac.uk'
__revision__ = '$Id: policyset.py 8078 2012-06-19 14:10:35Z pjkersha $'
import logging
...
from ndg.xacml.core.policy_combining_alg import PolicyCombiningAlgClassFactory, PolicyCombiningAlgInterface
from ndg.xacml.core.functions import UnsupportedStdFunctionError, UnsupportedFunctionError
...
class PolicySet(PolicyBase):
    ...
    __slots__ = ('__policySetId', '__version', '__policyCombiningAlgId', '__description',
                 '__policySetDefaults', '__target', '__policies', '__obligations',
                 '__policyCombiningAlgFactory', '__policyCombiningAlg')
    ...
    def fromSource(cls, source, readerFactory):
        ...
        if not issubclass(readerFactory, AbstractReaderFactory):
            raise TypeError('Expecting %r derived class for reader factory method; got %r' % (
             AbstractReaderFactory,
             readerFactory))
        reader = readerFactory.getReader(cls)
        if not issubclass(reader, AbstractReader):
            raise TypeError('Expecting %r derived class for reader class; got %r' % (
    ...
    def _setPolicyCombiningAlgFactory(self, value):
        ...
        if not isinstance(value, PolicyCombiningAlgClassFactory):
            raise TypeError('Expecting %r derived type for "policyCombiningAlgFactory" attibute; got %r' % (
             PolicyCombiningAlgClassFactory, type(value)))
        self.__policyCombiningAlgFactory = value

    policyCombiningAlgFactory = property(_getPolicyCombiningAlgFactory, _setPolicyCombiningAlgFactory, doc='Policy Combining Algorithm Factory')
    ...
    def _setPolicySetId(self, value):
        ...
        if not isinstance(value, basestring):
            raise TypeError('Expecting string type for "policySetId" attribute; got %r' % type(value))
    ...
    def _setVersion(self, value):
        ...
        """
        if not isinstance(value, basestring):
            raise TypeError('Expecting string type for "version" attribute; got %r' % type(value))
    ...
    def _setPolicyCombiningAlgId(self, value):
        ...
        """
        if not isinstance(value, basestring):
            raise TypeError('Expecting string type for "policyCombiningAlgId" attribute; got %r' % type(value))
    ...
    def _setPolicyCombiningAlgFromId(self):
        ...
        """
        policyCombiningAlgClass = self.__policyCombiningAlgFactory(self.__policyCombiningAlgId)
        if not isinstance(policyCombiningAlgClass, type) or not issubclass(policyCombiningAlgClass, PolicyCombiningAlgInterface):
            raise TypeError('Expecting %r derived type for policy combining algorithm class; got %r type' % (
        ...
        if self.__policyCombiningAlg is NotImplemented:
            raise UnsupportedStdFunctionError('The policy combining algorithm %r is not currently implemented' % self.__policyCombiningAlgId)
        elif self.__policyCombiningAlg is None:
            raise UnsupportedFunctionError('%r is not recognised as a valid XACML policy combining algorithm' % self.__policyCombiningAlgId)
        return

    policyCombiningAlgId = property(_getPolicyCombiningAlgId, _setPolicyCombiningAlgId, None, doc='Policy Combining Algorithm Id')
    ...
    def _setTarget(self, value):
        ...
        if not isinstance(value, Target):
            raise TypeError('Expecting Target for "target" attribute; got %r' % type(value))
        self.__target = value
    ...
    def _setDescription(self, value):
        ...
        """
        if not isinstance(value, basestring):
            raise TypeError('Expecting string type for "description" attribute; got %r' % type(value))
        self.__description = value

    description = property(_getDescription, _setDescription, doc='Policy Description text')
    ...
    def _setPolicySetDefaults(self, value):
        ...
        if not isinstance(value, PolicyDefaults):
            raise TypeError('Expecting string type for "policyDefaults" attribute; got %r' % type(value))
        self.__policySetDefaults = value

    policySetDefaults = property(_getPolicySetDefaults, _setPolicySetDefaults, None, 'Policy Set PolicyDefaults element')
