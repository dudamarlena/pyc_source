...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/ndg/xacml/test/functions/test_string_functions.py
...
__revision__ = '$Id: test_string_functions.py 8078 2012-06-19 14:10:35Z pjkersha $'
import logging
from os import path
import unittest
...
class Test(XacmlContextBaseTestCase):
    ...
    def test01_01StringConcatenate2Values(self):
        ...
        """
        self.pdp = PDP.fromPolicySource(XACML_CONCATENATE_TEST_FILEPATH, ReaderFactory)
    ...
    def test02_03UrlStringConcatenate3Values(self):
        ...
        """
        self.pdp = PDP.fromPolicySource(XACML_CONCATENATE_TEST_FILEPATH, ReaderFactory)
        request = self._createRequestCtx(self.__class__.RESOURCE7_ID, subjectId='testuser1', subjectRoles=('role1', ))
        response = self.pdp.evaluate(request)
        self.failIf(response is None, 'Null response')
        for result in response.results:
            self.failIf(result.decision != Decision.PERMIT, 'Expecting Permit decision')
    ...
    def test02_04UrlStringConcatenate3Values(self):
        ...
        """
        self.pdp = PDP.fromPolicySource(XACML_CONCATENATE_TEST_FILEPATH, ReaderFactory)
        request = self._createRequestCtx(self.__class__.RESOURCE8_ID, subjectId='testuser1', subjectRoles=('role1', ))
        response = self.pdp.evaluate(request)
        self.failIf(response is None, 'Null response')
        for result in response.results:
            self.failIf(result.decision != Decision.DENY, 'Expecting Deny decision')
