...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/ndg/xacml/core/policybase.py
...
__contact__ = 'Philip.Kershaw@stfc.ac.uk'
__revision__ = '$Id: policybase.py 8078 2012-06-19 14:10:35Z pjkersha $'
...
class PolicyBase(XacmlCoreBase):
    ...
    def fromSource(cls, source, readerFactory, finder):
        ...
        if not isinstance(finder, PolicyFinderBase):
            raise TypeError('Expecting %r derived object for policy finder; got %r' % (
             PolicyFinderBase, type(finder)))
    ...
    def evaluateResponse(self, request):
        ...
        try:
            ...
            log.error('Exception raised evaluating request context, returning %r decision:%s', e.response.results[0].decision, traceback.format_exc())
    ...
    def evaluate(self, context):
        ...
        """
        try:
            log.debug('Evaluating %s %r ...', self.ELEMENT_LOCAL_NAME, self.ident)
            decision = Decision()
            if self.target is not None:
                targetMatch = self.target.match(context)
                if targetMatch:
                    log.debug('Match to request context for target in %s %r', self.ELEMENT_LOCAL_NAME, self.ident)
            else:
                log.debug('No target set in %s %r', self.ELEMENT_LOCAL_NAME, self.ident)
                targetMatch = True
            if not targetMatch:
                log.debug('No match to request context for target in %s %r returning NotApplicable status', self.ELEMENT_LOCAL_NAME, self.ident)
                decision = Decision.NOT_APPLICABLE
                return decision
            decision = self.evaluateCombiningAlgorithm(context)
        except Exception:
            log.error('No PDPError type exception raised evaluating request context, returning %r decision:%s', Decision.INDETERMINATE_STR, traceback.format_exc())
