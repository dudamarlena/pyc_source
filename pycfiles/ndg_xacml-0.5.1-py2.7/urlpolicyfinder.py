...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/ndg/xacml/finder/urlpolicyfinder.py
...
__contact__ = 'Philip.Kershaw@stfc.ac.uk'
__revision__ = '$Id: urlpolicyfinder.py 8078 2012-06-19 14:10:35Z pjkersha $'
import os
...
class UrlPolicyFinder(PolicyFinderBase):
    ...
    def findPolicySet(self, policySetIdReference, common):
        ...
        if policySet is None:
            policySet = self._findPolicyFromReference(policySetIdReference, common)
        policySet = self.policySetMap.get(policySetIdReference, None)
        if policySet is None:
    ...
    def _makeUrlFromReference(self, reference):
        ...
        if reference.startswith(self._FILE_SCHEME):
            ...
            if path.startswith(self._RELATIVE_PATH_PREFIX):
                ...
                url = reference
        elif [ True for scheme in self._NON_FILE_SCHEMES if reference.startswith(scheme)
             ]:
            url = reference
        elif reference.startswith(os.path.sep):
            url = self._DEFAULT_SCHEME + reference
        else:
            url = self._DEFAULT_SCHEME + os.path.join(self.basePath, reference)
        return url