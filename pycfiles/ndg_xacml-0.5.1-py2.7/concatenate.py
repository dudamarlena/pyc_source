...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/ndg/xacml/core/functions/v2/concatenate.py
...
__revision__ = '$Id: concatenate.py 8078 2012-06-19 14:10:35Z pjkersha $'
...
from ndg.xacml.core.context.exceptions import XacmlContextError, XacmlContextTypeError
...
class ConcatenateBase(AbstractFunction):
    ...
    """
    FUNCTION_NS = None
    TYPE_REST = attributeValueClassFactory('http://www.w3.org/2001/XMLSchema#string')
    ...
    def evaluate(self, *args):
        ...
        """
        if len(args) < 2:
            raise XacmlContextError('Argument list should have length two or greater')
        else:
            if not isinstance(args[0], self.__class__.TYPE):
                raise XacmlContextTypeError('Expecting %r derived type for argument 1; got %r' % (
                 self.__class__.TYPE,
                 type(args[0])))
            for n, arg in enumerate(args[1:]):
                if not isinstance(arg, self.__class__.TYPE_REST):
                    raise XacmlContextTypeError('Expecting %r derived type for argument %d; got %r' % (
...
class FunctionClassFactory(FunctionClassFactoryBase):
    ...
    """
    FUNCTION_NAMES = ('urn:oasis:names:tc:xacml:2.0:function:string-concatenate', 'urn:oasis:names:tc:xacml:2.0:function:url-string-concatenate')
