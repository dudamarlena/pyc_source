...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/ndg/xacml/parsers/etree/applyreader.py
# Compiled at: 2012-03-19 08:02:22
...
__contact__ = 'Philip.Kershaw@stfc.ac.uk'
__revision__ = '$Id: applyreader.py 8041 2012-03-16 11:51:01Z rwilkinson $'
import logging
...
from ndg.xacml.core.attributedesignator import SubjectAttributeDesignator, ResourceAttributeDesignator, ActionAttributeDesignator, EnvironmentAttributeDesignator
    ...
    def __call__(self, obj, common):
        ...
        for subElem in getElementChildren(elem):
            ...
            if localName == xacmlType.ELEMENT_LOCAL_NAME:
                ...
                applyObj.expressions.append(AttributeValueReader.parse(subElem, common))
            elif localName == SubjectAttributeDesignator.ELEMENT_LOCAL_NAME:
                SubjectAttributeDesignatorReader = ReaderFactory.getReader(SubjectAttributeDesignator)
                applyObj.expressions.append(SubjectAttributeDesignatorReader.parse(subElem, common))
            elif localName == ResourceAttributeDesignator.ELEMENT_LOCAL_NAME:
                ResourceAttributeDesignatorReader = ReaderFactory.getReader(ResourceAttributeDesignator)
                applyObj.expressions.append(ResourceAttributeDesignatorReader.parse(subElem, common))
            elif localName == EnvironmentAttributeDesignator.ELEMENT_LOCAL_NAME:
                EnvironmentAttributeDesignatorReader = ReaderFactory.getReader(EnvironmentAttributeDesignator)
                applyObj.expressions.append(EnvironmentAttributeDesignatorReader.parse(subElem, common))
            elif localName == ActionAttributeDesignator.ELEMENT_LOCAL_NAME:
                ActionAttributeDesignatorReader = ReaderFactory.getReader(ActionAttributeDesignator)
                applyObj.expressions.append(ActionAttributeDesignatorReader.parse(subElem, common))
            elif localName == EnvironmentAttributeDesignator.ELEMENT_LOCAL_NAME:
                EnvironmentAttributeDesignatorReader = ReaderFactory.getReader(EnvironmentAttributeDesignator)
                applyObj.expressions.append(EnvironmentAttributeDesignatorReader.parse(subElem, common))
            elif localName == AttributeSelector.ELEMENT_LOCAL_NAME:
                AttributeSelectorReader = ReaderFactory.getReader(AttributeSelector)
                applyObj.expressions.append(AttributeSelectorReader.parse(subElem, common))
            elif localName == Condition.ELEMENT_LOCAL_NAME:
                ConditionReader = ReaderFactory.getReader(Condition)
                applyObj.expressions.append(ConditionReader.parse(subElem, common))
            elif localName == self.__class__.FUNCTION_ELEMENT_LOCAL_NAME:
                raise NotImplementedError('%r Apply sub-element not implemented', localName)
            elif localName == VariableReference.ELEMENT_LOCAL_NAME:
                raise NotImplementedError('%r Apply sub-element not implemented', localName)
            else:
                raise XMLParseError('%r Apply sub-element not recognised', localName)
