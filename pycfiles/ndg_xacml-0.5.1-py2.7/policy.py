...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/ndg/xacml/core/policy.py
# Compiled at: 2012-02-10 11:36:07
...
__contact__ = 'Philip.Kershaw@stfc.ac.uk'
__revision__ = '$Id: policy.py 7955 2011-12-21 18:29:45Z rwilkinson $'
import logging
...
from ndg.xacml.core.rule_combining_alg import RuleCombiningAlgClassFactory, RuleCombiningAlgInterface
...
class Policy(PolicyBase):
    ...
    __slots__ = ('__policyId', '__version', '__ruleCombiningAlgId', '__description',
                 '__policyDefaults', '__target', '__attr', '__obligations', '__ruleCombiningAlgFactory',
                 '__ruleCombiningAlg')
    ...
    def _setRuleCombiningAlgFactory(self, value):
        ...
        if not isinstance(value, RuleCombiningAlgClassFactory):
            raise TypeError('Expecting %r derived type for "ruleCombiningAlgFactory" attibute; got %r' % (
             RuleCombiningAlgClassFactory, type(value)))
        self.__ruleCombiningAlgFactory = value

    ruleCombiningAlgFactory = property(_getRuleCombiningAlgFactory, _setRuleCombiningAlgFactory, doc='Rule Combining Algorithm Factory')
    ...
    def _setPolicyId(self, value):
        ...
        if not isinstance(value, basestring):
            raise TypeError('Expecting string type for "policyId" attribute; got %r' % type(value))
    ...
    def _setRuleCombiningAlgId(self, value):
        ...
        """
        if not isinstance(value, basestring):
            raise TypeError('Expecting string type for "ruleCombiningAlgId" attribute; got %r' % type(value))
    ...
    def _setRuleCombiningAlgFromId(self):
        ...
        """
        ruleCombiningAlgClass = self.__ruleCombiningAlgFactory(self.__ruleCombiningAlgId)
        if not isinstance(ruleCombiningAlgClass, type) or not issubclass(ruleCombiningAlgClass, RuleCombiningAlgInterface):
            raise TypeError('Expecting %r derived type for rule combining algorithm class; got %r type' % (
        ...
        if self.__ruleCombiningAlg is NotImplemented:
            raise UnsupportedStdFunctionError('The rule combining algorithm %r is not currently implemented' % self.__ruleCombiningAlgId)
        elif self.__ruleCombiningAlg is None:
            raise UnsupportedFunctionError('%r is not recognised as a valid XACML rule combining algorithm' % self.__ruleCombiningAlgId)
        return

    ruleCombiningAlgId = property(_getRuleCombiningAlgId, _setRuleCombiningAlgId, None, doc='Rule Combining Algorithm Id')
    ...
    policyDefaults = property(_getPolicyDefaults, _setPolicyDefaults, None, 'Policy PolicyDefaults element')
