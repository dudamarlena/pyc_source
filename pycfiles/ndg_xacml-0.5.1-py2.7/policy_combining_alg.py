...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/ndg/xacml/core/policy_combining_alg.py
...
__contact__ = 'Philip.Kershaw@stfc.ac.uk'
__revision__ = '$Id: policy_combining_alg.py 8078 2012-06-19 14:10:35Z pjkersha $'
import logging
log = logging.getLogger(__name__)
from abc import abstractmethod
from ndg.xacml.core.context.result import Decision
ALGORITHMS = ('urn:oasis:names:tc:xacml:1.0:policy-combining-algorithm:deny-overrides',
    ...
    def evaluate(self, policies, context):
        ...
        for policy in policies:
            ...
            if decision == Decision.DENY:
                ...
                continue
            if decision == Decision.PERMIT:
                log.debug('Policy %r permits, returning overall permit decision', policy.ident)
                return Decision.PERMIT
            if decision == Decision.NOT_APPLICABLE:
                continue
            if decision == Decision.INDETERMINATE:
        ...
        if atLeastOneDeny:
            log.debug('At least one policy with a deny decision found, returning overall deny decision')
            return Decision.DENY
        if atLeastOneError:
            log.debug('At least one policy with an error found, returning overall indeterminate decision')
            return Decision.INDETERMINATE
        log.debug('No policies were applicable to the request, returning overall not applicable decision')
        return Decision.NOT_APPLICABLE
...
class FirstApplicablePolicyCombiningAlg(PolicyCombiningAlgInterface):
    ...
    def evaluate(self, policies, context):
        ...
        for policy in policies:
            decision = policy.evaluate(context)
            if decision == Decision.DENY:
                log.debug('Policy %r denies, returning overall deny decision', policy.ident)
                return Decision.DENY
            if decision == Decision.PERMIT:
                log.debug('Policy %r permits, returning overall permit decision', policy.ident)
                return Decision.PERMIT
            if decision == Decision.NOT_APPLICABLE:
                continue
            if decision == Decision.INDETERMINATE:
                log.debug('Policy %r is indeterminate, returning overall indeterminate decision', policy.ident)
                return Decision.INDETERMINATE
...
class PolicyCombiningAlgClassFactory(object):
    ...
    DEFAULT_MAP.update({'urn:oasis:names:tc:xacml:1.0:policy-combining-algorithm:deny-overrides': DenyOverridesPolicyCombiningAlg, 
       'urn:oasis:names:tc:xacml:1.0:policy-combining-algorithm:permit-overrides': PermitOverridesPolicyCombiningAlg, 
       'urn:oasis:names:tc:xacml:1.0:policy-combining-algorithm:first-applicable': FirstApplicablePolicyCombiningAlg})
    __slots__ = ('__map', )
