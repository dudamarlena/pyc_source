...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/ndg/xacml/core/apply.py
# Compiled at: 2012-02-23 12:00:08
...
__contact__ = 'Philip.Kershaw@stfc.ac.uk'
__revision__ = '$Id: apply.py 8023 2012-02-23 17:00:08Z pjkersha $'
...
class Apply(Expression):
    ...
    __slots__ = ('__functionId', '__function', '__functionMap', '__loadFunctionFromId',
                 '__expressions')
    ...
    def _set_functionId(self, value):
        ...
        if not isinstance(value, basestring):
            raise TypeError('Expecting %r type for "functionId" attribute; got %r' % (
             basestring, type(value)))
        self.__functionId = value
        if self.__loadFunctionFromId:
            self.setFunctionFromMap(self.__functionMap)

    functionId = property(_get_functionId, _set_functionId, None, 'Apply type Function ID')
    ...
    def setFunctionFromMap(self, functionMap):
        ...
        if functionClass is NotImplemented:
            raise UnsupportedStdFunctionError('No match function class implemented for MatchId="%s"' % self.functionId)
        elif functionClass is None:
            raise UnsupportedFunctionError('<Apply> function namespace %r is not recognised' % self.functionId)
