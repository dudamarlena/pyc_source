...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/djutils/views/helpers.py
# Compiled at: 2017-08-19 14:46:52
import re
...
def sort_key_process(request, sort_key='sort'):
    ...
    current_param = None
    if current:
        mo = re.match('^(-?)(\\w+)$', current)
        if mo:
            current_reversed = mo.group(1) == '-'
            current_param = mo.group(2)
    return (
     current_param, current_reversed)