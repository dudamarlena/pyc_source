...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\segmentation_module\segmentation_api\predictor.py
# Compiled at: 2019-03-20 07:42:57
...
import cv2, segmentation_module.segmentation_tools.generator_tools
...
from segmentation_module.segmentation_tools.segmentation_tools import get_model, prepare_callbacks, result_map_to_img, result_map_to_raw_predict, raw_2_blob, save_any_img
from segmentation_module.segmentation_tools.utils2 import get_photos_from_dir, make_folder_to_model, prepare_folder_in_directory, timed

class Predictor(APIObject):
    __slots__ = ('model_path', 'model', 'input_width', 'input_height', 'n_classes',
                 'output_path', 'config_paths', 'config_parameters', 'config_classes',
                 'config_colors', 'predict_time', 'logger', 'model_folder')
    ...
    def _prepare_model_to_predict(self, model_name):
        ...
        if model:
            self.model = model(self.n_classes, input_shape, 0.0001, 0.0005)
        else:
