...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\segmentation_module\segmentation_support\h5_creator.py
# Compiled at: 2019-03-11 07:41:09
# Size of source mod 2**32: 5128 bytes
import glob, json, os, h5py, numpy as np, pandas as pd, PIL.Image as Image
...
class H5_Creator(APIObject):
    __slots__ = ('csv_path', 'images_folder', 'labels_folder', 'output_width', 'output_height',
                 'is_val_dataset', 'is_test_dataset')
    ...
    def _write_data(self, h5py_file, width, height, dataset_type, dataset_df):
        ...
        for index, row in dataset_df.iterrows():
            ...
            image = cv2.resize(image, (width, height), fx=1, fy=1, interpolation=(cv2.INTER_CUBIC))
