...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\segmentation_module\segmentation_tools\segmentation_tools.py
# Compiled at: 2019-04-15 07:08:22
...
def prepare_callbacks(path_to_model, config_parameters, config_colors, config_path):
    ...
    model_checkpoint = ModelCheckpoint(filepath=(os.path.join(path_to_model, model_name + '_weights_epoch-{epoch:02d}_loss-{loss:.4f}_val_loss-{val_loss:.4f}.h5')),
      monitor=monitor_parameter,
      verbose=1,
      save_best_only=True,
      save_weights_only=True,
      mode=mode,
      period=1)
    tensorboard_logger = TensorBoard(log_dir=path_to_log, histogram_freq=0,
      batch_size=train_batch_size,
      write_graph=True,
      write_grads=True,
      write_images=True)
    learning_rate_scheduler = LearningRateScheduler(lr_schedule)
    early_stopping = EarlyStopping(monitor=monitor_parameter, min_delta=0.001,
      patience=patience,
      mode=mode)
    ...
    callbacks = [
     model_checkpoint, tensorboard_logger, train_check, learning_rate_scheduler, early_stopping, reduce_lr]
    return callbacks
