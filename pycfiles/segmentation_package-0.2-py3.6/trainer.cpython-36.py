...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\segmentation_module\segmentation_api\trainer.py
# Compiled at: 2019-03-20 07:25:24
...
from segmentation_module.segmentation_tools.generator_tools import prepare_generators_adapt
from segmentation_module.segmentation_tools.segmentation_tools import prepare_callbacks, get_model
from segmentation_module.segmentation_tools.utils2 import cleaning_weights_in_folder, make_folder_to_model

class Trainer(APIObject):
    __slots__ = ('config_paths', 'config_parameters', 'config_classes', 'config_colors',
                 'model', 'path_to_saved_model', 'config_path', 'config_dict', 'model_path')
    ...
    def train(self):
        ...
        callbacks = prepare_callbacks(model_path, self.config_parameters, self.config_colors, self.config_paths)
        self.model.fit_generator(generator=train_gen, steps_per_epoch=steps_per_train_epoch,
          epochs=epochs,
          callbacks=callbacks,
    ...
    def _prepare_model(self, model_name):
        ...
        if model_name == 'pspnet':
            self.model = model(num_classes, input_shape, learning_rate, 0.0005)
        else:
            self.model = model(num_classes, input_shape, learning_rate, 0.0005, vgg_weight_path)