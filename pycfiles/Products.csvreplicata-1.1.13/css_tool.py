...
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Products/CSSManager/css_tool.py
...
class css_tool(UniqueObject, SimpleItem):
    ...
    def customizeProps(self, REQUEST=None):
      ...
      """
        ...
        REQUEST.RESPONSE.redirect('%s/prefs_cssmanager_form?chooser=%s' % (self.portal_url(), self._getCurrentPropSheetName(REQUEST)))
        return
    ...
    def setStyles(self, REQUEST=None):
      ...
      """
        ...
        self.redirectWithMessage('%s/prefs_cssmanager_form?chooser=%s' % (self.portal_url(), chooser), ms, REQUEST=REQUEST)
        return
