...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/dbsprockets/saprovider.py
...
class SAProvider(IProvider):
    ...
    def _generateWhereclause(self, table, pks, values={}, join='and'):
        l = [ getattr(table.c, key) == value for (key, value) in values.iteritems() if key in pks if hasattr(table.c, key) ]
        whereclause = None
        if len(l) == 1:
            whereclause = l[0]
        elif len(l) > 1:
    ...
    def _removePrimaryKeysIfNotForeign(self, tableName, values={}):
        ...
        for key in self.getPrimaryKeys(tableName):
            if key in values and key not in foreignKeys:
                values.pop(key)
    ...
    def getForeignKeyDict(self, tableName, foreignKeyFieldLabels=[
 'name', '_name', 'description', '_description', 'title']):
    ...
    def getManyToManyTables(self):
        if not hasattr(self, '_manyToManyTables'):
            self._manyToManyTables = [ table for table in self.tables if len(self.getForeignKeys(table)) == 2 if len(self.getColumns(table)) == 2 ]
    ...
    def getViewColumnName(self, tableName, possibleColumns=[
