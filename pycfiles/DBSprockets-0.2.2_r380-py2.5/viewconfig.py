...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/dbsprockets/viewconfig.py
...
class ViewConfigMeta(object):
    ...
    def _createHiddenFields(self):
        ...
        fields.extend([ HiddenField(id=field, identifier=field) for field in self.disabledFields if field not in self.omittedFields if field not in self.hiddenFields ])
...
class ViewConfig(ViewConfigMeta):
    pass
...
class EditableRecordViewConfig(ViewConfig):
    ...
    def _doGetManyToManyWidgets(self, fields):
        ...
        for fieldName in fields:
            table = fieldName[:-1]
            if fieldName not in hiddenFields and fieldName not in omittedFields and (limitFields is None or fieldName in limitFields):
    ...
    def _doGetChildWidgetArgsForWidgetType(self, field, widgetType):
        ...
        if widgetType == TextField and hasattr(field.type, 'length') and field.type.length is not None:
...
class AddRecordViewConfig(EditRecordViewConfig):
    ...
    def _doGetDisabledFields(self):
        ...
        return [ k for k in self.metadata.primaryKeys() if k in self.metadata.autoIncrementFields if k not in foreignKeys ]