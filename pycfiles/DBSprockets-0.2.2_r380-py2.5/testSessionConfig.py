...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/dbsprockets/test/testSessionConfig.py
...
class TestSessionConfig:
    ...
    def testCreateObjBad(self):
        ...
        for input in badInput[1:]:
            yield (
             self._create, '', self.provider, input)
...
class TestEditRecordSessionConfig(TestSessionConfig):
    ...
    def testValue(self):
        ...
        expected = {'user_id': 1, 'tableName': 'tg_user', 
           'password': 'asdf', 
           'user_name': 'asdf'}
...
class TestTableViewSessionConfig(TestSessionConfig):
    ...
    def testValue(self):
        ...
        assert len(actual) == 1
        expected = {'user_id': 1, 'user_name': 'asdf', 'password': '******'}
        actual = actual[0]
        for (key, value) in expected.iteritems():
            assert actual[key] == value, 'expected: %s\nactual: %s' % (expected, actual)
