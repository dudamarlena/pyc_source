...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/simplystatic/s2site.py
...
"""
import os, sys, os.path, shutil, glob, math, tempfile, codecs
...
class Site(object):
    ...
    def __init__(self, initial_dir=None):
        ...
        self._dirs = {'initial': initial_dir, 'run': os.getcwd()}
    ...
    def init_structure(self):
        ...
        if self._dirs['base'] != None:
            ...
            self._update_dirs_on_base()
            if not dir_empty(self._dirs['base']) or not os.access(self._dirs['base'], os.W_OK):
                raise ValueError
            pdl = package_data_location()
            datadirs = glob.glob(os.path.join(pdl, '*'))
            for dd in datadirs:
                if os.path.isdir(dd):
                    shutil.copytree(dd, os.path.join(self._dirs['base'], os.path.split(dd)[1]))

            for d in self._dirs:
                if d not in ('initial', 'run', 'base'):
    ...
    def get_page_names(self):
        ...
        fis = [ p for p in glob.glob(os.path.join(self._dirs['source'], '*')) if os.path.isdir(p)
              ]
    ...
    def generate(self):
        ...
        if title == '':
            title = '<No title>'
        feed = AtomFeed(title=title, subtitle=self.site_config['site_subtitle'], feed_url=os.path.join(self.site_config['site_url'], 'atom.xml'), url=self.site_config['site_url'], author=self.site_config['default_author'])
        themes_to_copy = []
        generated_page_info = []
        for slug in self._pages_to_generate():
            p = s2page.Page(self, slug, isslug=True)
            generated_page_info.append({'slug': p.slug, 'title': p.title, 
               'date': p.creation_date})
            t = p.theme_path
            if t not in themes_to_copy:
                themes_to_copy.append(t)
            self._wipe_www_page(slug)
            pg_content = p.generate()
            try:
                cdd = datetime.strptime(p.creation_date, '%Y-%m-%d')
            except:
            ...
            feed.add(title=p.title, content=pg_content, content_type='html', author=p.author, url=os.path.join(self.site_config['site_url'], 'atom.xml'), updated=cdd)
    ...
    def generate_front(self, generated_page_info, epp=10):
        ...
        template_path = os.path.join(self._dirs['themes'], self.site_config['default_theme'], self.site_config['default_template'])
        makotemplate = Template(filename=template_path, module_directory=self._makodir)
        ...
        for fpo in frontpage_iterator:
            i += 1
            rendition = makotemplate.render(pageContent=fpo['content'], isFrontPage=True, themePath=themepath, commonPath=commonpath, pageTitle='TOC - ' + str(i))
            if i == 1:
                fname = 'index.html'
            else:
                fname = str(i) + '.html'
    ...
    def renderfront_chronological_plain(self, generated_page_info, epp=10):
        innertemplate_path = os.path.join(self._dirs['themes'], self.site_config['default_theme'], 'chronological_plain_front.tpl')
        innertemplate = Template(filename=innertemplate_path, module_directory=self._makodir)
        ...
        while c <= numpages:
            ...
            frontpage_obj = {'content': None, 'next_page_url': None, 
               'back_page_url': None}
            if c < numpages:
            ...
            frontpage_obj['content'] = innertemplate.render(generatedPageInfo=chunk, backPageUrl=frontpage_obj['back_page_url'], nextPageUrl=frontpage_obj['next_page_url'])
            c += 1
            yield frontpage_obj
    ...
    def random_page(self, title=None, content=None, creation_date=None, tags=None):
