...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pepperedform/renderutils.py
# Compiled at: 2012-08-19 15:44:44
"""
...
tag = HTML.tag
default_field_part_orders = {'checkbox_field': ('input', 'label', 'help', 'example', 'error'), 
   None: ('label', 'help', 'example', 'input', 'error')}
default_css_classes = {'field_errors': 'field_errors', 
...
def render_example(css_class, example):
    if type(example) not in (list, tuple):
        example = [
         example]
    return tag('span', class_=css_class, *example)
...
default_extra_renderers = {'example': render_example, 
   'help': render_help}
...
class FormRenderer(object):
    ...
    def __init__(self, css_classes=default_css_classes, extra_renderers=default_extra_renderers, field_part_orders=default_field_part_orders, closed=False):
        """
        ``css_classess``
...
class FormRendererState(object):
    ...
    def clone(self, renderer=NotGiven, defaults=NotGiven, errors=NotGiven, id_stack=NotGiven, getter=NotGiven):
    ...
    def rename_descend(self, name):
        ...
        return FormRendererState(self.renderer, defaults, errors, id_stack, sequence_getter)

    def mapping_descend(self, name):
        defaults = self.field_value(name)
        errors = self.field_error(name)
        id_stack = self.push_id_stack(name)
        return FormRendererState(self.renderer, defaults, errors, id_stack, mapping_getter)
    ...
    def sequence_descend(self, name, getter=mapping_getter):
        ...
        if defaults:
            for index, new_defaults in enumerate(defaults):
                ...
                yield FormRendererState(self.renderer, new_defaults, new_errors, new_id_stack, sequence_getter)

        return

    def determine_css_classes(self, class_name, add_css_classes=None):
        """
    ...
    def build_id(self, *names):
        return ('_').join(self.push_id_stack(*names))
    ...
    def render_field_errors(self, name):
        ...
        if errors:
            ...
            error_div = tag('div', class_=css_class, *error_spans)
        return error_div
    ...
    def determine_extra_field_parts(self, field_parts, field_extras):
        ...
        if field_extras and 'example' in field_extras:
            ...
            if render_example:
                example_css_class = _to_css_class(self.determine_css_classes('field_example'))
                extra_field_parts['example'] = render_example(css_class=example_css_class, example=field_extras['example'])
        if field_extras and 'help' in field_extras:
            render_help = self.determine_extra_renderer('help')
            if render_help:
                help_css_class = _to_css_class(self.determine_css_classes('field_help'))
                extra_field_parts['help'] = render_help(class_=help_css_class, help_=field_extras['help'])
