...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pepperedform/tests/test_render.py
# Compiled at: 2012-06-27 00:55:16
"""
Test webhelper html wrappers.
"""
import unittest
from pepperedform.render import text_field, checkbox_field, textarea_field, multiple_select_field, single_select_field, mapping, sequence, rename, primary_form_action, secondary_form_action, sequence_repeater, file_field
...
class MockContext(object):
    ...
    def serialize(self):
        return ('').join(self.out).encode('utf8')


class MockCaller(object):

    def __init__(self, body_text='', callback=None):
...
class TestRender(unittest.TestCase):
    ...
    def test_single_select_field_brainless(self):
        ...
        options = [
         (
          selected_value, 'Dark'), ('light', 'Light'), ('medium', 'Medium')]
        context = MockContext()
        state = self._get_form_renderer_state(defaults)
    ...
    def test_multiple_select_field_brainless(self):
        ...
        defaults = {name: [
                'dark', 'light']}
        options = [
         ('dark', 'Dark'), ('light', 'Light'), ('medium', 'Medium')]
        context = MockContext()
        state = self._get_form_renderer_state(defaults)
    ...
    def test_mapping_brainless(self):
        ...
        field_name = 'roast'
        field_value = 'dark'
        defaults = {mapping_name: {field_name: field_value}}
    ...
    def test_sequence_brainless(self):
        ...
        self.assertEquals(values[0], 'French Roast', 'Only one value in list so value at 0 is French Roast')
    ...
    def test_sequence_repeater_brainless(self):
        ...
        html = context.serialize()
        assert sequence_name in html, html
        self.assertEquals(values[0], None, 'Template has no value so value at 0 is None.')
        self.assertEquals(values[1], 'French Roast', 'Only one value in actual list so value at 1 is French Roast')
