...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pepperedform/tests/test_renderutils.py
# Compiled at: 2012-06-24 14:59:25
import unittest
from pepperedform.renderutils import sequence_getter, mapping_getter, make_start_token, FormRendererState, FormRenderer
...
class TestRenderUtils(unittest.TestCase):
    ...
    def test_sequence_getter_missing(self):
        ...
        self.assertIs(result, if_missing, msg='Should return if_missing if items is falsy.')
        return
    ...
    def test_sequence_getter(self):
        ...
        if_missing = tuple()
        result = sequence_getter(items, None, if_missing=if_missing)
        self.assertEqual(result, '15', msg='Should return items if items is not falsy.')
        return
    ...
    def test_mapping_getter_missing(self):
        ...
        self.assertIs(result, if_missing, msg='Should return if_missing if name not in items.')
    ...
    def test_mapping_getter(self):
        ...
        name = 'day'
        result = mapping_getter(items, name, if_missing=if_missing)
        self.assertEqual(result, '15', 'Should return value for key name in items if it exists.')
    ...
    def test_field_value(self):
        day_value = 1
        name = 'day'
        state = self._get_renderer_state({name: day_value})
    ...
    def test_field_error(self):
        ...
        name = 'day'
        state = self._get_renderer_state({}, {name: day_error})
        error = state.field_error(name)
