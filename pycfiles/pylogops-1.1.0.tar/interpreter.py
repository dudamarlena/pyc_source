...
# Embedded file name: build/bdist.linux-i686/egg/pylogo/interpreter.py
...
class Interpreter(object):
    ...
    def expr(self):
        ...
        try:
            ...
            e.set_frame(self)
            raise
        except (LogoControl, SystemExit, KeyboardInterrupt, StopIteration):
            raise
        except Exception, e:
                ...
                try:
                    ...
                    for i in range(n):
                        try:
                            ...
                            raise LogoEndOfCode('Not enough arguments provided to %s: got %i and need %i' % (func_name, i, n))
    ...
    def special_to(self, greedy):
        ...
        while 1:
            tok = self.tokenizer.next()
            if lastNewline and isinstance(tok, str) and tok.lower() == 'end':
                break
            lastNewline = tok == '\n'
            if tok is EOF:
    ...
    def special_tell(self, greedy):
        ...
        if tok == '[' or isinstance(tok, list):
            ...
            def get_variable(var):
                try:
                    return obj[var]
                except (AttributeError, NameError, KeyError, IndexError, TypeError, ValueError):
                    return getlogoattr(obj, var)

            value = self.expr_inner(get_function=get_function, get_variable=get_variable)
            return value
    ...
    def expr_list(self):
        ...
        while 1:
            ...
            if tok == ']':
                ...
                tok = self.expr_list()
            if tok is EOF:
                raise LogoEndOfCode('The end of the code was not expected (waiting for ])')
            body.append(tok)
    ...
    def import_module(self, mod):
        ...
        for n in dir(mod):
            ...
            if type(obj) in (ClassType, type) and getattr(obj, 'logo_class', False):
    ...
    def input_loop(self, input, output):
        ...
        """
        tokenizer = reader.FileTokenizer(input, output=output, prompt=self.prompts)
        self.push_tokenizer(tokenizer)
        try:
            while 1:
                try:
...
class BoundUserFunction(object):
    ...
    def __call__(self, interp, *args, **kw):
        return self.func(interp, *args, **{'_extravars': {'self': self.obj}, '_actor': self.obj})

    def __repr__(self):
        return 'Function: %s for %s' % (self.func.name, self.obj)
...
class RootFrame(Interpreter):
    ...
    def __repr__(self):
        try:
            return '<RootFrame %x parsing "%r">' % (id(self), self.tokenizer)
        except:
    ...
    def get_nonactor_variable(self, v):
        ...
        if self.vars.has_key(v):
            return self.vars[v]
        raise LogoNameError('Variable :%s has not been set.' % v)
    ...
    def make_local(self, v):
        raise LogoSyntaxError('You can only use LOCAL in a function (TO).')
...
class Frame(RootFrame):
    ...
    def __repr__(self):
        return '<Frame %x parsing "%s">' % (id(self), self.tokenizer)
    ...
    def get_nonactor_variable(self, v):
        ...
        if self.local_vars.has_key(v):
            raise LogoUndefinedError('Variable :%s has not been set, but it has been declared LOCAL.' % v)
