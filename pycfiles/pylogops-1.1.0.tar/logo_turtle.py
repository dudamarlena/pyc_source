...
# Embedded file name: build/bdist.linux-i686/egg/pylogo/logo_turtle.py
# Compiled at: 2007-10-14 14:35:27
import turtle, math, weakref, threading, sys
from common import *
...
class Turtle:
    ...
    def __repr__(self):
        return '<%s %i>' % (self.__class__.__name__, self._count)
    ...
    @logofunc(aliases=['pc', 'color'], arity=1)
    ...
    def distance(self, other, orig=None):
        if orig is None:
            orig = self.pen
        return math.sqrt((orig.position()[0] - other.position()[0]) ** 2 + (orig.position()[1] - other.position()[1]) ** 2)
