...
# Embedded file name: build/bdist.linux-i686/egg/pylogo/builtins.py
...
import os, random, sys, operator, math, time, threading
try:
...
def substringp(thing1, thing2):
    ...
    """
    return type(thing2) is str and type(thing1) is str and thing2.find(thing1) != -1
...
def rseq(from_num, to_num, length):
    ...
    between FROM and TO, inclusive::
        ...
        [3 3.5 4 4.5 5]
    """
    result = [ from_num + float(i) * (to_num - from_num) / (length - 1) for i in range(length - 1) ]
...
def spawn(interp, block, starter=None):
    ...
    t = threading.Thread(target=logo_eval, args=(interp, block), name=threadname)
    t.start()
    return threadname
...
def newdict(lst=None):
    if lst:
        if len(lst) % 2:
            raise LogoError('You must give NEWDICT a list with an even number of items', description='Invalid function call')
        items = []
        while 1:
            if not lst:
                break
