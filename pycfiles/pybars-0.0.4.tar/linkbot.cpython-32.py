...
# Embedded file name: build/bdist.linux-armv6l/egg/barobo/linkbot.py
# Compiled at: 2014-09-16 14:39:13
import threading, time, struct, math
try:
    import Queue
except:
    import queue as Queue

import barobo, barobo._comms as _comms, barobo._util as _util, barobo.mobot as mobot
...
class Linkbot(mobot.Mobot):
    ...
    def disableAccelEventCallback(self):
        buf = bytearray([
         barobo.BaroboCtx.CMD_SET_ENABLE_ACCEL_EVENT, 4, 0, 0])
        self._transactMessage(buf)
        self.accelCallbackEnabled = False

    def disableJointEventCallback(self):
        buf = bytearray([
         barobo.BaroboCtx.CMD_SET_ENABLE_JOINT_EVENT, 4, 0, 0])
        self._transactMessage(buf)
    ...
    def enableJointEventCallback(self, cb):
        ...
        """
        buf = bytearray([
         barobo.BaroboCtx.CMD_SET_ENABLE_JOINT_EVENT, 4, 7, 0])
        self._transactMessage(buf)
    ...
    def enableAccelEventCallback(self, cb):
        ...
        """
        buf = bytearray([
         barobo.BaroboCtx.CMD_SET_ENABLE_ACCEL_EVENT, 4, 7, 0])
        self._transactMessage(buf)
    ...
    def getAccelerometerData(self):
        ...
        """
        buf = bytearray([barobo.BaroboCtx.CMD_GETACCEL, 3, 0])
        response = self._transactMessage(buf)
    ...
    def getBatteryVoltage(self):
        ...
        """
        buf = bytearray([barobo.BaroboCtx.CMD_GETBATTERYVOLTAGE, 3, 0])
        response = self._transactMessage(buf)
        voltage = barobo._unpack('<f', response[2:6])[0]
        return voltage
    ...
    def getColorRGB(self):
        ...
        """
        buf = bytearray([barobo.BaroboCtx.CMD_GETRGB, 3, 0])
        response = self._transactMessage(buf)
    ...
    def getHWRev(self):
        ...
        """
        buf = bytearray([barobo.BaroboCtx.CMD_GET_HW_REV, 3, 0])
        response = self._transactMessage(buf)
        return barobo._unpack('<3B', response[2:5])
    ...
    def ping(self, numbytes=4, sendbytes=[]):
        ...
        if response[2:2 + numbytes] != randbytes:
            raise barobo.BaroboException('Ping did not receive correct bytes. Expected {0}, got {1}'.format(list(map(hex, randbytes)), list(map(hex, response))))
    ...
    def reboot(self):
        ...
        """
        buf = bytearray([barobo.BaroboCtx.CMD_REBOOT, 3, 0])
        self._transactMessage(buf)
    ...
    def recordAnglesPlot(self):
        import pylab
        pylab.plot(self.recordThread.time, self.recordThread.angles[0], self.recordThread.time, self.recordThread.angles[1], self.recordThread.time, self.recordThread.angles[2])
        pylab.show()
    ...
    def setBuzzerFrequency(self, freq):
        ...
        """
        buf = bytearray([106, 5, int(freq) >> 8 & 255, int(freq) & 255, 0])
        self._transactMessage(buf)
    ...
    def setHWRev(self, major, minor, micro):
        ...
        """
        buf = bytearray([
         barobo.BaroboCtx.CMD_SET_HW_REV, 0, major, minor, micro, 0])
        buf[1] = len(buf)
        self._transactMessage(buf)
