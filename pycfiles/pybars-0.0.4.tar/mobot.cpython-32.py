...
# Embedded file name: build/bdist.linux-armv6l/egg/barobo/mobot.py
# Compiled at: 2014-09-16 14:39:13
import math, threading, time, struct
try:
    import Queue
except:
    import queue as Queue

import barobo, barobo._comms as _comms, barobo._util as _util
...
class Mobot:
    ...
    def __init__(self):
        ...
        self.serialID = None
        self.baroboCtx = None
        self.messageThread = threading.Thread(target=self._Mobot__messageThread)
        self.messageThread.daemon = True
        self.messageThread.start()
        self.messageLock = threading.Lock()
        self.eventThread = threading.Thread(target=self._Mobot__eventThread)
    ...
    def disableButtonCallback(self):
        ...
        self.callbackEnabled = False
        buf = bytearray([
         barobo.BaroboCtx.CMD_ENABLEBUTTONHANDLER, 4, 0, 0])
        self._transactMessage(buf)
    ...
    def driveJointToNB(self, joint, angle):
        ...
        buf = bytearray([barobo.BaroboCtx.CMD_SETMOTORANGLEPID, 8, joint - 1])
    ...
    def driveToNB(self, angle1, angle2, angle3, angle4):
        ...
        buf = bytearray([barobo.BaroboCtx.CMD_SETMOTORANGLESPID, 19])
    ...
    def enableButtonCallback(self, callbackfunc, userdata=None):
        ...
        buf = bytearray([
         barobo.BaroboCtx.CMD_ENABLEBUTTONHANDLER, 4, 1, 0])
        self._transactMessage(buf)
    ...
    def getFormFactor(self):
        ...
        """
        buf = bytearray([barobo.BaroboCtx.CMD_GETFORMFACTOR, 3, 0])
        response = self._transactMessage(buf)
        return response[2]
    ...
    def getJointAngle(self, joint):
        ...
        """
        buf = bytearray([
         barobo.BaroboCtx.CMD_GETMOTORANGLE, 4, joint - 1, 0])
        response = self._transactMessage(buf)
    ...
    def getJointState(self, joint):
        ...
        ROBOT_NEGATIVE=5
        """
        buf = bytearray([barobo.BaroboCtx.CMD_GETMOTORSTATE, 4, joint, 0])
        response = self._transactMessage(buf)
        return response[2]
    ...
    def getJointAnglesTime(self):
        ...
        """
        buf = bytearray([
         barobo.BaroboCtx.CMD_GETMOTORANGLESTIMESTAMPABS, 3, 0])
        response = self._transactMessage(buf)
        millis = barobo._unpack('<L', response[2:6])[0]
        data = barobo._unpack('<4f', response[6:22])
        rc = [millis / 1000.0]
    ...
    def moveJointToNB(self, joint, angle):
        ...
        """
        angle = _util.deg2rad(angle)
        buf = bytearray([barobo.BaroboCtx.CMD_SETMOTORANGLEABS, 8, joint - 1])
        buf += bytearray(struct.pack('<f', angle))
        buf += bytearray([0])
        self._transactMessage(buf)
    ...
    def moveNB(self, angle1, angle2, angle3, angle4):
        ...
        angle3 = _util.deg2rad(angle3)
        angle4 = _util.deg2rad(angle4)
        buf = bytearray([barobo.BaroboCtx.CMD_MOVE_MOTORS, 19])
        buf += bytearray(struct.pack('<4f', angle1, angle2, angle3, angle4))
        buf += bytearray([0])
        self._transactMessage(buf)
    ...
    def moveToNB(self, angle1, angle2, angle3, angle4):
        ...
        angle3 = _util.deg2rad(angle3)
        angle4 = _util.deg2rad(angle4)
        buf = bytearray([barobo.BaroboCtx.CMD_SETMOTORANGLESABS, 19])
    ...
    def recordAnglesEnd(self):
        ...
        return [
         map(lambda x: x - self.recordThread.time[0], self.recordThread.time),
    ...
    def recordAnglesPlot(self):
        import pylab
        pylab.plot(self.recordThread.time, self.recordThread.angles[0], self.recordThread.time, self.recordThread.angles[1], self.recordThread.time, self.recordThread.angles[2], self.recordThread.time, self.recordThread.angles[3])
        pylab.show()
    ...
    def reset(self):
        ...
        """
        buf = bytearray([barobo.BaroboCtx.CMD_RESETABSCOUNTER, 3, 0])
        self._transactMessage(buf)
    ...
    def setJointMovementState(self, joint, state):
        ...
        if self.form == ROBOTFORM_I and joint == 3:
            if state == ROBOT_FORWARD:
                ...
                if state == ROBOT_BACKWARD:
                    state = ROBOT_FORWARD
                else:
                    if state == ROBOT_POSITIVE:
                        state = ROBOT_FORWARD
                    elif state == ROBOT_NEGATIVE:
                        state = ROBOT_BACKWARD
        buf = bytearray([
         barobo.BaroboCtx.CMD_SETMOTORDIR, 5, joint - 1, state, 0])
        self._transactMessage(buf)
    ...
    def setJointSpeed(self, joint, speed):
        ...
        buf = bytearray([barobo.BaroboCtx.CMD_SETMOTORSPEED, 8, joint - 1])
    ...
    def setMotorPower(self, joint, power):
        ...
        """
        joint = joint - 1
        buf = bytearray([barobo.BaroboCtx.CMD_SETMOTORPOWER, 10, 1 << joint])
        buf += bytearray(struct.pack('>3h', power, power, power))
        buf += bytearray([0])
        self._transactMessage(buf)

    def setMotorPowers(self, power1, power2, power3, power4):
        buf = bytearray([barobo.BaroboCtx.CMD_SETMOTORPOWER, 10, 7])
    ...
    def setMovementState(self, state1, state2, state3, state4, time=-1):
        ...
        if self.form == barobo.ROBOTFORM_I:
            if state3 == barobo.ROBOT_FORWARD:
                ...
                if state3 == barobo.ROBOT_BACKWARD:
                    state3 = barobo.ROBOT_FORWARD
                else:
                    if state3 == barobo.ROBOT_POSITIVE:
                        state3 = barobo.ROBOT_FORWARD
        ...
        if time > 0:
            time = time * 1000
        buf = bytearray([barobo.BaroboCtx.CMD_TIMEDACTION, 0, 7])
        for state in states:
    ...
    def stop(self):
        buf = bytearray([barobo.BaroboCtx.CMD_STOP, 3, 0])
        self._transactMessage(buf)
    ...
    def __messageThread(self):
        while True:
            ...
            if pkt[0] == barobo.BaroboCtx.RESP_OK or pkt[0] == barobo.BaroboCtx.RESP_ERR or pkt[0] == barobo.BaroboCtx.RESP_ALREADY_PAIRED:
    ...
    def __eventThread(self):
        while 1:
            ...
            if evt[0] == barobo.BaroboCtx.EVENT_BUTTON and self.callbackEnabled:
                ...
                self.jointcallbackfunc(values[0], values[1] * 180.0 / math.pi, values[2] * 180.0 / math.pi, values[3] * 180.0 / math.pi, evt[22])
