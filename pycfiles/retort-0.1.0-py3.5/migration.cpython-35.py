...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/retort/migration.py
# Compiled at: 2017-10-06 18:57:21
# Size of source mod 2**32: 3550 bytes
import importlib, re, autopep8, sqlalchemy
...
class Migration:
    ...
    def get_code(self):
        ...
        code_without_comment_and_indent = '\n'.join([x for x in python_code.split('\n') if not x.startswith('#')])
    ...
    def get_globals(self, op):
        ...
        _globals = {'op': op, 
         'sa': sqlalchemy}
        matching_result = re.match('sqlalchemy\\.dialects\\.(\\w+)', type(dialect).__module__)
        if matching_result:
