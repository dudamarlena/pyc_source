...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/appy/pod/styles_manager.py
...
READING = 0
PARSING_STYLE = 1
MAPPING_NOT_DICT = 'The styles mapping must be a dictionary or a UserDict instance.'
MAPPING_ELEM_NOT_STRING = "The styles mapping dictionary's keys and values must be strings."
MAPPING_OUTLINE_DELTA_NOT_INT = 'When specifying "h*" as key in the styles mapping, you must specify an integer as value. This integer, which may be positive or negative, represents a delta that will be added to the html heading\'s outline level for finding an ODT style with the same outline level.'
MAPPING_ELEM_EMPTY = 'In your styles mapping, you inserted an empty key and/or value.'
UNSTYLABLE_TAG = 'You can\'t associate a style to element "%s". Unstylable elements are: %s'
...
HTML_PARA_ODT_TEXT = 'For XHTML element "%s", you must associate a paragraph-wide OpenDocument style. "%s" is a "text" style (that applies to only a chunk of text within a paragraph).'
HTML_TEXT_ODT_PARA = 'For XHTML element "%s", you must associate an OpenDocument "text" style (that applies to only a chunk of text within a paragraph). "%s" is a paragraph-wide style.'
...
class StylesManager:
    ...
    __module__ = __name__
    podSpecificStyles = {'podItemKeepWithNext': Style('podItemKeepWithNext', 'paragraph')}
    ...
    def checkStylesAdequation(self, htmlStyle, odtStyle):
        ...
        if htmlStyle in XHTML_PARAGRAPH_TAGS_NO_LISTS and odtStyle in self.textStyles:
            raise PodError(HTML_PARA_ODT_TEXT % (htmlStyle, odtStyle.displayName))
        if htmlStyle in XHTML_INNER_TAGS and odtStyle in self.paragraphStyles:
            raise PodError(HTML_TEXT_ODT_PARA % (htmlStyle, odtStyle.displayName))
    ...
    def checkStylesMapping(self, stylesMapping):
        ...
        res = {}
        if not isinstance(stylesMapping, dict) and not isinstance(stylesMapping, UserDict):
        ...
        for (xhtmlStyleName, odtStyleName) in stylesMapping.iteritems():
            ...
            if xhtmlStyleName == 'h*' and not isinstance(odtStyleName, int):
                raise PodError(MAPPING_OUTLINE_DELTA_NOT_INT)
            if xhtmlStyleName != 'h*' and not isinstance(odtStyleName, basestring):
                raise PodError(MAPPING_ELEM_NOT_STRING)
            if xhtmlStyleName != 'h*' and (not xhtmlStyleName or not odtStyleName):
                raise PodError(MAPPING_ELEM_EMPTY)
            if xhtmlStyleName in XHTML_UNSTYLABLE_TAGS:
                raise PodError(UNSTYLABLE_TAG % (xhtmlStyleName, XHTML_UNSTYLABLE_TAGS))
            if xhtmlStyleName != 'h*':
