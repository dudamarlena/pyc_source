...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/core/node.py
# Compiled at: 2020-05-03 00:51:52
# Size of source mod 2**32: 22142 bytes
"""
...
import os, string, inspect, logging
...
class ArtellaDCCNode(object):
    ...
    def update_info(self):
        ...
        if not self._node:
            ...
            if not is_referenced:
                self._nodes_list = tp.Dcc.list_children(node=(self._node),
                  all_hierarchy=True,
                  full_path=True,
                  children_type='transform') or list()
    ...
    def change_namespace(self, new_namespace):
        ...
        try:
            ...
            LOGGER.warning('Impossible to change namespace for reference node: "{0}" >> "{1}" to "{2}" --> {3}'.format(self.node, self.namespace, new_namespace, e))

        if result:
            LOGGER.info('Namespace for reference node: "{0}" >> "{1}" to "{2}" changed successfully!'.format(self.node, self.namespace, new_namespace))
        self.update_info()
        return result
    ...
    def change_filename(self, new_filename):
        ...
        try:
            ...
            LOGGER.error('Impossible to change filename for reference node: "{0}" > "{1}" to "{2}" --> {3}'.format(self.node, self.filename, new_filename, e))
    ...
    def convert_reference_to_absolute_path(self):
        ...
        if os.path.exists(fix_path):
            ...
            LOGGER.warning('Impossible to convert "{0}" to absolute path: "{1}", because new file does not exists!'.format(self.filename, fix_path))
        self.update_info()
    ...
    def import_objects(self, with_absolute_path=False):
        ...
        try:
            ...
            LOGGER.error('Impossible to import objects from reference node: "{0}" --> {1}'.format(self.node, e))
    ...
    def add_override(self, override_class=None):
        ...
        if not override_class:
            LOGGER.warning('Override creation tool is not working yet. Use Outliner to add overrides for now.')
            return
        else:
...
class ArtellaAssetNode(ArtellaDCCNode, object):
    ...
    def get_short_name(self, clean=False):
        ...
        """
        if clean:
            return tp.Dcc.node_short_name(self._name).rstrip(string.digits)
        else:
            return tp.Dcc.node_short_name(self._name)
    ...
    def get_renderable_shapes(self, remove_namespace=False, full_path=True):
        ...
        transform_relatives = tp.Dcc.list_relatives(node=(self._node),
          all_hierarchy=True,
          full_path=full_path,
          relative_type='transform',
          shapes=False,
          intermediate_shapes=False)
    ...
    def create_shape_operator(self, shape_name=None):
        ...
        :return: str
        """
        return artellapipe.Arnold().get_asset_shape_operator((self.id),
          shape_name, connect_to_asset_operator=True, create=True)
    ...
    def add_shape_operator_assignment(self, assignment_value, shape_name=None):
        ...
        if not shape_operator:
            LOGGER.warning('Impossible to set Asset Shape attribute for "{}" because asset shape operator for "{}" does not exists!'.format(shape_name, self.id))
            return
        else:
    ...
    def remove_shape_operator_assignment(self, assignment_value, shape_name=None):
        ...
        shape_operator = self.get_shape_operator(shape_name)
        if not shape_operator:
            LOGGER.warning('Impossible to remove Asset Shape attribute from "{}" because asset shape operator for "{}" does not exists!'.format(shape_name, self.id))
            return
        else:
