...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/widgets/assetsviewer.py
# Compiled at: 2020-05-03 00:51:52
# Size of source mod 2**32: 8832 bytes
"""
...
class AssetsViewer(grid.GridWidget, object):
    ...
    def change_category(self, category=None):
        ...
        if category != defines.ArtellaFileStatus.ALL:
            if category not in artellapipe.AssetsMgr().get_asset_categories():
                LOGGER.warning('Asset Type {} is not a valid asset type for project {}'.format(category, self._project.name.title()))
                category = defines.ArtellaFileStatus.ALL
        self.clear()
        new_assets = list()
        for new_asset in reversed(self._assets):
            ...
            elif new_asset.asset.get_category() == category:
                new_asset.setVisible(True)
                new_assets.insert(0, new_asset)
            else:
