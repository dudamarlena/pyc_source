...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/core/abstract.py
# Compiled at: 2020-05-03 00:51:52
# Size of source mod 2**32: 31238 bytes
"""
...
class AbstractFile(object):
    ...
    def get_template_dict(self, extension):
        ...
        :return: dict
        """
        raise NotImplementedError('get_template_dict function for {} is not implemented!'.format(self.__class__.__name__))
    ...
    def view_locally(self):
        ...
        if not os.path.isdir(asset_path):
            LOGGER.warning('Impossible to open asset path locally because path for Asset "{}" : "{}" does not exists!'.format(self.get_name(), asset_path))
            return
    ...
    def get_file(self, file_type, status, extension=None, version=None, fix_path=False, only_local=False, extra_dict=None, must_exist=True, next_version=False):
        """
        ...
        file_path = self.solve_path(file_type=file_type,
    ...
    def solve_path(self, file_type, template_dict, template=None, extra_dict=None, version=None, fix_path=False, only_local=False, status=defines.ArtellaFileStatus.WORKING, must_exist=True, next_version=False, check_file_type=True):
        if extra_dict is None:
            extra_dict = dict()
        else:
            ...
            if check_file_type:
                if file_type not in available_files_types:
                    LOGGER.warning('File Type "{}" is not valid! Supported File Types: {}'.format(file_type, available_files_types.keys()))
                    return
            if not defines.ArtellaFileStatus.is_valid(status):
                LOGGER.warning('Given File Artella Sync Status: {} is not valid! Supported Statuses: {}'.format(status, defines.ArtellaFileStatus.supported_statuses()))
                return
                ...
                for k, v in file_type_dict.items():
                    if k not in extra_dict:
                        ...
                        if v and v != extra_dict[k]:
                            LOGGER.info('Updating Template Dict from file type specific value: {} | {} >>> {}'.format(k, extra_dict[k], file_type_dict[k]))
                            extra_dict[k] = v
        ...
        if not template:
            ...
            template or LOGGER.warning('Impossible to retrieve file path because template "{}" is not in configuration file'.format(file_template_name))
            return
        else:
                ...
                if file_type_local_versions:
                    if version:
                        if version == file_type_local_versions[1]:
                            ...
                            if only_local:
                                LOGGER.warning('No local version "{}" found for File Type: "{}"'.format(version, file_type))
                                return
                            template_dict['version_folder'] = version
                    else:
    ...
    def import_file(self, status=None, extension=None, file_type=None, sync=False, reference=False):
        ...
        if not status:
            ...
            if extension not in available_extensions:
                LOGGER.warning('Impossible to reference file with extension "{}". Supported extensions: {}'.format(extension, available_extensions))
                return False
            file_types = artellapipe.FilesMgr().get_file_types_by_extension(extension)
            if not file_types:
                LOGGER.warning('Impossible to reference file by its extension ({}) because no file types are registered!'.format(extension))
                return False
            if len(file_types) > 1:
                if not file_type:
                    LOGGER.warning('Multiple file types found with extension: {}. Do not know which file should imported!'.format(extension))
                    return False
            if len(file_types) == 1:
    ...
    def supports_file_type(self, file_type, status=defines.ArtellaFileStatus.ALL):
        ...
        if status != defines.ArtellaFileStatus.ALL:
            if status not in self.FILES:
                LOGGER.warning('Impossible to sync "{}" because current Asset {} does not support it!'.format(file_type, self.__class__.__name__))
                return False
            if status not in self.project:
                LOGGER.warning('Impossible to sync "{}" because project "{}" does not support it!'.format(file_type, self.project.name.title()))
                return False
        return True
    ...
    def sync_latest_published_files(self, file_type=None, ask=False):
        ...
        """
        if ask:
            result = qtutils.show_question(None, 'Synchronizing Latest Published Files: {}'.format(self.get_name()), 'Are you sure you want to synchronize latest published files? This can take quite some time!')
        ...
        if not valid_types:
            ...
            for valid_type in valid_types:
                ...
                if sync_type == defines.ArtellaFileStatus.ALL or sync_type == defines.ArtellaFileStatus.WORKING:
                    ...
                    if working_path:
                        if working_path not in paths_to_sync:
...
class AbstractAsset(AbstractFile, object):
    ...
    def get_template_dict(self, extension):
        ...
        template_dict = {'project_id':self._project.id, 
