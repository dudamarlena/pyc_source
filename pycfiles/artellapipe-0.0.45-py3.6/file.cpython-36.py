...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/core/file.py
# Compiled at: 2020-05-03 00:51:52
# Size of source mod 2**32: 22102 bytes
"""
...
class ArtellaFile(object):
    ...
    def __init__(self, file_name, file_path=None, file_extension=None, project=None):
        ...
        self._latest_server_version = {defines.ArtellaFileStatus.WORKING: None, 
    ...
    def has_valid_object(self):
        ...
        :return: bool
        """
        raise NotImplementedError('has_valid_object function for {} is not implemented!'.format(self.__class__.__name__))
    ...
    def get_latest_published_versions(self):
        ...
        :return: str
        """
        raise NotImplementedError('get_latest_published_versions function for {} is not implemented!'.format(self.__class__.__name__))
    ...
    def get_file_paths(self, return_first=False, fix_path=True, **kwargs):
        ...
        if self.path:
            ...
            if not self.path.endswith(self.name):
                ...
                base_path = self.path
            self._extensions or LOGGER.warning('No valid extensions found for file: "{}"'.format(self.__class__.__name__))
            return base_path
        else:
            if return_first:
                return
    ...
    def import_file(self, status=defines.ArtellaFileStatus.WORKING, fix_path=True, sync=False, reference=False, *args, **kwargs):
        ...
        if not valid_path:
            msg = 'Impossible to reference file of type "{}". File Path "{}" does not exists!'.format(self.FILE_TYPE, file_path)
            LOGGER.warning(msg)
            return
        else:
            if reference:
                return (self._reference_file)(args, file_path=file_path, **kwargs)
            return (self._import_file)(args, file_path=file_path, **kwargs)
    ...
    def get_local_versions(self, status=None):
        ...
        if not file_path or not os.path.exists(file_path):
            ...
            if not file_type_template:
                LOGGER.warning('Impossible to retrieve local version because template for "{}" is not in configuration file'.format(self.FILE_TYPE))
                return local_versions
    ...
    def get_latest_local_versions(self, status=None, next_version=False):
        ...
        for version, version_folder in local_versions.items():
            if not latest_local_versions:
                latest_local_versions = [
                 int(version), version_folder]
            else:
                if int(latest_local_versions[0]) < int(version):
                    latest_local_versions = [
                     int(version), version_folder]

        if latest_local_versions:
            if next_version:
    ...
    def get_latest_local_published_path(self, sync_folder=False):
        ...
        if not latest_local_versions:
            ...
            published_path = self.get_file(status=(defines.ArtellaFileStatus.PUBLISHED), extension=(self.FILE_EXTENSIONS[0]), fix_path=False,
    ...
    def get_server_versions(self, status=None, all_versions=True, force_update=False):
        ...
        if not status:
            status = defines.ArtellaFileStatus.WORKING
        if not force_update:
            if self._latest_server_version.get(status):
            ...
            if hasattr(status, 'references'):
                for ref_name, ref_data in status.references.items():
                    server_data = self._get_working_server_versions(working_path=working_path,
                      artella_data=ref_data,
