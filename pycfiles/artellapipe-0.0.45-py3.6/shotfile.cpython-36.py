...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/core/shotfile.py
...
class ArtellaShotFile(file.ArtellaFile, object):
    ...
    def __init__(self, file_shot=None, file_path=None):
        ...
        super(ArtellaShotFile, self).__init__(file_name=(self._shot.get_name() if self._shot else None),
          file_path=file_path)
    ...
    def get_file(self, status=defines.ArtellaFileStatus.WORKING, extension=None, fix_path=False, version=None):
        ...
        :return: str
        """
        return self._shot.get_file(file_type=(self.FILE_TYPE),
          status=status,
          extension=extension,
          fix_path=fix_path,
          version=version)
