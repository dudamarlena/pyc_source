...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/random_words/random_words.py
# Compiled at: 2017-03-04 04:10:10
import os, json
...
class Random(dict):
    ...
    def __init__(self, file):
        self.types_file = {'nouns': self.load_nouns, 
...
class RandomWords(Random):
    ...
    def random_words(self, letter=None, count=1):
        ...
        words = []
        if letter is None:
            all_words = list(chain.from_iterable(self.nouns.values()))
            try:
                words = sample(all_words, count)
            except ValueError:
                len_sample = len(all_words)
                raise ValueError(('Param "count" must be less than {0}. (It is only {0} words)').format(len_sample + 1, letter))
...
class RandomNicknames(Random):
    ...
    def random_nicks(self, letter=None, gender='u', count=1):
        ...
        if letter is None:
            all_nicks = list(chain.from_iterable(self.nicknames[gender].values()))
            try:
            ...
            try:
                ...
                raise ValueError(('Param "count" must be less than {0}. (It is only {0} nicks for letter "{1}")').format(len_sample + 1, letter))
