...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/vsmclient/middleware/auth_token.py
...
"""
import datetime, httplib, json, os, logging, stat, tempfile, time, urllib, webob.exc, re
...
for app in ('nova', 'glance', 'quantum', 'cinder'):
    try:
        cfg = __import__('%s.openstack.common.cfg' % app, fromlist=[
         '%s.openstack.common' % app])
...
class AuthProtocol(object):
    ...
    def __init__(self, app, conf):
        ...
        self.LOG.info('Using %s as cache directory for signing certificate' % self.signing_dirname)
        if os.path.exists(self.signing_dirname):
            if not os.access(self.signing_dirname, os.W_OK):
                raise ConfigurationError('unable to access signing_dir %s' % self.signing_dirname)
            if os.stat(self.signing_dirname).st_uid != os.getuid():
                self.LOG.warning('signing_dir is not owned by %s' % os.getlogin())
        ...
        self.ec = client.Client(self.admin_user, self.admin_password, self.admin_tenant_name, auth_url)
        self._cache = None
        ...
        self._memcache_secret_key = self._conf_get('memcache_secret_key')
        ...
        self.token_revocation_list_cache_timeout = datetime.timedelta(seconds=self._conf_get('revocation_cache_time'))
        http_connect_timeout_cfg = self._conf_get('http_connect_timeout')
        self.http_connect_timeout = http_connect_timeout_cfg and int(http_connect_timeout_cfg)
    ...
    def _assert_valid_memcache_protection_config(self):
        if self._memcache_security_strategy:
            if self._memcache_security_strategy not in ('MAC', 'ENCRYPT'):
                raise Exception('memcache_security_strategy must be ENCRYPT or MAC')
            if not self._memcache_secret_key:
                raise Exception('mecmache_secret_key must be defined when a memcache_security_strategy is defined')
    ...
    def _choose_api_version(self):
        ...
        if self._conf_get('auth_version'):
            ...
            self.LOG.info('Auth Token proceeding with requested %s apis', version_to_use)
        else:
            version_to_use = None
            ...
            if version_to_use:
                self.LOG.info('Auth Token confirmed use of %s apis', version_to_use)
            else:
                self.LOG.error('Attempted versions [%s] not in list supported by server [%s]', (', ').join(LIST_OF_VERSIONS_TO_ATTEMPT), (', ').join(versions_supported_by_server))
    ...
    def _get_supported_versions(self):
        ...
        if response.status != 300:
            self.LOG.error('Unable to get version info from vsm: %s' % response.status)
            ...
            try:
                ...
                self.LOG.error('Invalid version response format from server', data)
                raise ServiceError('Unable to parse version response from vsm')

        self.LOG.debug('Server reports support for api versions: %s', (', ').join(versions))
        return versions

    @webob.dec.wsgify(RequestClass=webob.Request)
    def __call__(self, req):
        pattern = re.compile('^http://.*:.*/.*/.*/servers')
    ...
    def _remove_auth_headers(self, env):
        ...
        """
        auth_headers = ('X-Identity-Status', 'X-Domain-Id', 'X-Domain-Name', 'X-Project-Id',
                        'X-Project-Name', 'X-Project-Domain-Id', 'X-Project-Domain-Name',
                        'X-User-Id', 'X-User-Name', 'X-User-Domain-Id', 'X-User-Domain-Name',
                        'X-Roles', 'X-Service-Catalog', 'X-User', 'X-Tenant-Id',
                        'X-Tenant-Name', 'X-Tenant', 'X-Role')
        self.LOG.debug('Removing headers from request environment: %s' % (',').join(auth_headers))
    ...
    def _get_user_token_from_header(self, env):
        ...
        """
        token = self._get_header(env, 'X-Auth-Token', self._get_header(env, 'X-Storage-Token'))
        if token:
            return token
        if not self.delay_auth_decision:
            self.LOG.warn('Unable to find authentication token in headers')
    ...
    def _get_http_connection(self):
        if self.auth_protocol == 'http':
            return self.http_client_class(self.auth_host, self.auth_port, timeout=self.http_connect_timeout)
        else:
            return self.http_client_class(self.auth_host, self.auth_port, self.key_file, self.cert_file, timeout=self.http_connect_timeout)
    ...
    def _request_admin_token(self):
        ...
        """
        params = {'auth': {'passwordCredentials': {'username': self.admin_user, 
        ...
        try:
            ...
            self.LOG.warn('Unexpected response from vsm service: %s', data)
    ...
    def _build_user_headers(self, token_info):
        ...
        def get_tenant_info():
            ...
            def essex():
                ...
                return (
                 token['tenant']['id'], token['tenant']['name'])
        ...
        if self._token_is_v2(token_info):
            ...
            roles = (',').join([ role['name'] for role in token.get('roles', [])
                               ])
            catalog_root = token
            catalog_key = 'catalog'
            if 'domain' in token:
            ...
            user_id = user['id']
            user_name = user['name']
            rval = {'X-Identity-Status': 'Confirmed', 
    ...
    def _protect_cache_value(self, token, data):
        ...
        try:
            if self._memcache_security_strategy == 'ENCRYPT':
                ...
                if self._memcache_security_strategy == 'MAC':
    ...
    def _cache_store(self, token, data, expires=None):
        ...
        if self._use_vsm_cache:
            self._cache.set(key, data_to_store, time=self.token_cache_time)
        else:
            self._cache.set(key, data_to_store, timeout=self.token_cache_time)
    ...
    def _cache_store_invalid(self, token):
        ...
        if self._cache:
            self.LOG.debug('Marking token %s as unauthorized in memcache', token)
    ...
    def verify_uuid_token(self, user_token, retry=True):
        ...
        if self.auth_version == 'v3.0':
            headers = {'X-Auth-Token': self.get_admin_token(), 'X-Subject-Token': safe_quote(user_token)}
            response, data = self._json_request('GET', '/v3/auth/tokens', additional_headers=headers)
        else:
            headers = {'X-Auth-Token': self.get_admin_token()}
            response, data = self._json_request('GET', '/v2.0/tokens/%s' % safe_quote(user_token), additional_headers=headers)
        if response.status == 200:
            ...
            if response.status == 401:
                self.LOG.info('Keystone rejected admin token %s, resetting', headers)
                self.admin_token = None
            else:
                self.LOG.error('Bad response code while validating token: %s' % response.status)
            if retry:
                self.LOG.info('Retrying validation')
                return self._validate_user_token(user_token, False)
            self.LOG.warn('Invalid user token: %s. Keystone response: %s.', user_token, data)
    ...
    def is_signed_token_revoked(self, signed_text):
        ...
        for revoked_id in revoked_ids:
            if token_id == revoked_id:
                self.LOG.debug('Token %s is marked as having been revoked', token_id)
                return True
    ...
    def cms_verify(self, data):
        ...
        while True:
            try:
                ...
                if self.cert_file_missing(err.output, self.signing_cert_file_name):
    ...
    def fetch_revocation_list(self, retry=True):
        headers = {'X-Auth-Token': self.get_admin_token()}
        response, data = self._json_request('GET', '/v2.0/tokens/revoked', additional_headers=headers)
        if response.status == 401:
            if retry:
                self.LOG.info('Keystone rejected admin token %s, resetting admin token', headers)
                self.admin_token = None
    ...
    def fetch_signing_cert(self):
        response, data = self._http_request('GET', '/v2.0/certificates/signing')
        try:
    ...
    def fetch_ca_cert(self):
        response, data = self._http_request('GET', '/v2.0/certificates/ca')
        try:
