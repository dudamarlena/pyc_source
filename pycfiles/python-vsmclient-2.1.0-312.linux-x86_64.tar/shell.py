...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/vsmclient/v1/shell.py
...
@utils.arg('--vsm-os-username', metavar='<vsm-os-username>', help='The username of openstack keystone connected.')
@utils.arg('--vsm-os-password', metavar='<vsm-os-password>', help='The password of openstack keystone connected.')
@utils.arg('--vsm-os-tenant-name', metavar='<vsm-os-tenant-name>', help='The tenant name of openstack keystone connected.')
@utils.arg('--vsm-os-auth-url', metavar='<vsm-os-auth-url>', help='The auth url of openstack keystone connected.')
@utils.arg('--vsm-os-region-name', metavar='<vsm-os-region-name>', default='RegionOne', help='The region name of openstack keystone connected. Default=RegionOne.')
@utils.arg('--ssh-user', metavar='<ssh-user>', help='The ssh user to connect openstack keystone node.')
...
def do_appnode_create(cs, args):
    ...
    if not args.ssh_user:
        raise exceptions.CommandError('you need specify a password-less user to connect the openstack')
    appnode = {'os_username': args.vsm_os_username, 
...
@utils.arg('id', metavar='<id>', help='ID of appnode.')
@utils.service_type('vsm')
...
@utils.arg('--vsm-os-tenant-name', metavar='<vsm-os-tenant-name>', help='The tenant name of openstack keystone connected.')
@utils.arg('--vsm-os-auth-url', metavar='<vsm-os-auth-url>', help='The auth url of openstack keystone connected.')
@utils.arg('--vsm-os-region-name', metavar='<vsm-os-region-name>', default='RegionOne', help='The region name of openstack keystone connected.')
@utils.arg('--ssh-user', metavar='<ssh-user>', help='The ssh user to connect openstack keystone node.')
@utils.service_type('vsm')
def do_appnode_update(cs, args):
    ...
    ssh_status = ''
    log_info = ''
    new_appnode = {'os_username': vsm_os_username, 
...
@utils.arg('--name', metavar='<name>', default='default', help='Cluster name[Not used]. Default=default.')
@utils.arg('--file-system', metavar='<file-system>', default='xfs', help='File system[Not used]. Default=xfs.')
@utils.arg('--journal-size', metavar='<journal-size>', default='', help='File system[Not used]. Default="".')
@utils.arg('--size', metavar='<size>', default='', help='Size[Not used]. Default="".')
@utils.arg('--management-network', metavar='<management-network>', default='', help='Management network[Not used]. Default="".')
@utils.arg('--ceph-public-network', metavar='<ceph-public-network>', default='', help='Ceph public network[Not used]. Default="".')
@utils.arg('--cluster-network', metavar='<cluster-network>', default='', help='Cluster network[Not used]. Default="".')
@utils.arg('--primary-public-netmask', metavar='<primary-public-netmask>', default='', help='Primary public netmask[Not used]. Default="".')
@utils.arg('--secondary-public-netmask', metavar='<secondary-public-netmask>', default='', help='Secondary public netmask[Not used]. Default="".')
@utils.arg('--cluster-netmask', metavar='<cluster-netmask>', default='', help='Cluster netmask[Not used]. Default="".')
@utils.arg('--servers', metavar='<id=server-id,is-storage=boolean,is-monitor=boolean>', action='append', default=[], help='Servers to create ceph cluster, need 3 servers at least.Boolean value is True or False[Default=True].')
@utils.service_type('vsm')
...
@utils.arg('cluster', metavar='<cluster>', help='Name or ID of cluster.')
@utils.service_type('vsm')
...
@utils.arg('--really-refresh', action='store_true', help='Really refresh the cluster status. Default=False.')
@utils.service_type('vsm')
...
@utils.arg('id', metavar='<id>', help='ID of cluster.')
@utils.service_type('vsm')
...
@utils.arg('server-id', metavar='<server-id>', help='ID of server.')
@utils.service_type('vsm')
def do_device_list_available_disks(cs, args):
    """Lists available disks."""
    search_opts = {'server_id': args.server_id, 
...
@utils.arg('--device-id', metavar='<device-id>', help='ID of device.')
@utils.arg('--device-path', metavar='<device-path>', help='Path of device.')
@utils.service_type('vsm')
def do_device_show_smart_info(cs, args):
    ...
    search_opts = {'device_id': args.device_id, 'device_path': args.device_path}
    smart_info = cs.devices.get_smart_info(search_opts=search_opts)
    _print_device(smart_info)


@utils.arg('mds', metavar='<mds>', help='Name or ID of mds.')
@utils.service_type('vsm')
...
@utils.arg('mon', metavar='<mon>', help='Name or ID of mon.')
@utils.service_type('vsm')
...
@utils.arg('osd', metavar='<osd>', help='Name or ID of osd.')
@utils.service_type('vsm')
...
@utils.arg('--server-id', metavar='<server-id>', help='The id of server which to add new osd.')
@utils.arg('--storage-group-id', metavar='<storage-group-id>', help='The id of storage group.')
@utils.arg('--weight', metavar='<weight>', default='1.0', help='Weight of osd.')
@utils.arg('--journal', metavar='<journal>', help='Journal path.')
@utils.arg('--data', metavar='<data>', help='Data path.')
@utils.service_type('vsm')
def do_osd_add_new(cs, args):
    ...
    body = {'server_id': args.server_id, 'osd_info': [
...
@utils.arg('pg', metavar='<pg>', help='Name or ID of pg.')
@utils.service_type('vsm')
...
@utils.arg('--pools', metavar='<pool-id=pool-id,cinder-volume-host=hostname,appnode-id=appnode-id>', action='append', default=[], help='Each should have pool id, cinder volume host and appnode id.')
@utils.service_type('vsm')
...
def do_server_add(cs, args):
    """\x1b[1;31;40mAdds a new server.\x1b[0m"""
    _is_developing('server-add', 'Adds a new server.')


@utils.arg('--id', metavar='<id>', action='append', default=[], help='ID of server.')
@utils.service_type('vsm')
...
@utils.arg('id', metavar='<id>', help='ID of storage group.')
@utils.service_type('vsm')
...
@utils.arg('storage-pool', metavar='<storage-pool>', help='Name or ID of storage pool.')
@utils.service_type('vsm')
...
@utils.arg('name', metavar='<name>', help='Name of storage pool.')
@utils.arg('--storage-group-id', metavar='<storage-group-id>', help='ID of storage group.')
@utils.arg('--pg-num', metavar='<pg-num>', help='Placement group number.')
@utils.arg('--pool-quota', metavar='<pool-quota>', default=0, help='Pool quota. Default=0.')
@utils.arg('--cluster-id', metavar='<cluster-id>', default=0, help='Pool quota. Default=0.')
@utils.arg('--tag', metavar='<tag>', default='', help='Tag. Default=Name of pool.')
@utils.service_type('vsm')
...
@utils.arg('name', metavar='<name>', help='Name of storage pool.')
@utils.arg('--storage-group-id', metavar='<storage-group-id>', help='ID of storage group.')
@utils.arg('--ec-failure-domain', metavar='<ec-failure-domain>', help='EC failure domain[osd, host or zone].')
@utils.arg('--pool-quota', metavar='<pool-quota>', default=0, help='Pool quota. Default=0')
@utils.arg('--cluster-id', metavar='<cluster-id>', default=0, help='Pool quota. Default=0')
@utils.arg('--tag', metavar='<tag>', default='', help='Tag. Default=Name of pool.')
@utils.arg('--ec-profile-id', metavar='<ec-profile-id>', default=1, help='Tag. Default=1.')
@utils.service_type('vsm')
...
@utils.arg('--storage-pool-id', metavar='<storage-pool-id>', help='Pool id of storage pool as the storage pool of cache tier pool.')
@utils.arg('--cache-pool-id', metavar='<cache-pool-id>', help='Pool id of storage pool as the cache pool of cache tier pool.')
@utils.arg('--cache-mode', metavar='<cache-mode>', default='readonly', help='Mode of cache tier pool[readonly or writeback]. Default=readonly.')
@utils.arg('--force-nonempty', action='store_true', help='Force nonempty. Default=False.')
@utils.arg('--hit-set-type', metavar='<hit-set-type>', default='bloom', help='Hit set type. Default=bloom.')
@utils.arg('--hit-set-count', metavar='<hit-set-count>', default=1, help='Hit set count. Default=1.')
@utils.arg('--hit-set-period-s', metavar='<hit-set-period-s>', default=3600, help='Hit set period(s). Default=3600.')
@utils.arg('--target-max-mem-mb', metavar='<target-max-mem-mb>', default=1000000, help='Target max mem(mb). Default=1000000.')
@utils.arg('--target-dirty-ratio', metavar='<target-dirty-ratio>', default=0.4, help='Target dirty ratio. Default=0.4.')
@utils.arg('--target-full-ratio', metavar='<target-full-ratio>', default=0.8, help='Target full ratio. Default=0.8.')
@utils.arg('--target-max-objects', metavar='<target-max-objects>', default=1000000, help='Target max objects. Default=1000000.')
@utils.arg('--target-min-flush-age-m', metavar='<target-min-flush-age-m>', default=10, help='Target min flush age(m). Default=10.')
@utils.arg('--target-min-evict-age-m', metavar='<target-min-evict-age-m>', default=20, help='Target min evict age(m). Default=20.')
@utils.service_type('vsm')
def do_storage_pool_add_cache_tier(cs, args):
    ...
    cache_tier = {'cache_tier': {'storage_pool_id': storage_pool.id, 
                      'cache_pool_id': cache_pool.id, 
                      'cache_mode': args.cache_mode, 
                      'force_nonempty': args.force_nonempty, 
                      'options': {'hit_set_type': args.hit_set_type, 
...
@utils.arg('id', metavar='<id>', help='Pool id of storage pool as cache.')
@utils.service_type('vsm')
def do_storage_pool_remove_cache_tier(cs, args):
    """Removes cache tier pool."""
    try:
        body = {'cache_tier': {'cache_pool_id': args.id}}
...
@utils.arg('setting-name', metavar='<setting-name>', help='Name of setting.')
@utils.service_type('vsm')
...
@utils.arg('name', metavar='<name>', help='Name of setting.')
@utils.arg('--value', metavar='<value>', help='Value of setting.')
@utils.service_type('vsm')
...
@utils.arg('--host', metavar='<host>', help='The host of rgw.')
@utils.arg('--name', metavar='<name>', default='radosgw.gateway', help='The name of rgw instance. Default=radosgw.gateway.')
@utils.arg('--is-ssl', action='store_true', help='SSL of not. Default=False.')
@utils.arg('--uid', metavar='<uid>', default='johndoe', help='The user name. Default=johndoe.')
@utils.arg('--display-name', metavar='<display-name>', default='John Doe', help='The user display name. Default=John Doe.')
@utils.arg('--email', metavar='<email>', default='john@example.comjohn@example.com', help='The Email. Default=john@example.comjohn@example.com.')
@utils.arg('--sub-user', metavar='<sub-user>', default='johndoe:swift', help='The sub user name. Default=johndoe:swift.')
@utils.arg('--access', metavar='<access>', default='full', help='The access type. Default=full.')
@utils.arg('--key-type', metavar='<key-type>', default='swift', help='The key type. Default=swift.')
@utils.service_type('vsm')
def do_rgw_create(cs, args):
    ...
    try:
        cs.rgws.create(host, rgw_instance_name, is_ssl, uid, display_name, email, sub_user, access, key_type)
