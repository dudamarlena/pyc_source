...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/vsmclient/common/cms.py
...
def cms_verify(formatted, signing_cert_file_name, ca_file_name):
    ...
    process = subprocess.Popen(['openssl', 'cms', '-verify',
     ...
     '-inform', 'PEM',
     '-nosmimecap', '-nodetach',
     '-nocerts', '-noattr'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def verify_token(token, signing_cert_file_name, ca_file_name):
    return cms_verify(token_to_cms(token), signing_cert_file_name, ca_file_name)
