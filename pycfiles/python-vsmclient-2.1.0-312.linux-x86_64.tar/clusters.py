...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/vsmclient/v1/clusters.py
...
class ClusterManager(base.ManagerWithFind):
    ...
    def create(self, name='default', file_system='xfs', journal_size=None, size=None, management_network=None, ceph_public_network=None, cluster_network=None, primary_public_netmask=None, secondary_public_netmask=None, cluster_netmask=None, servers=[]):
        """
        Create a cluster.
        """
        body = {'cluster': {'name': name, 'file_system': file_system, 
    ...
    def list(self, detailed=False, search_opts=None):
        ...
        if detailed:
            detail = '/detail'
        ret = self._list('/clusters%s%s' % (detail, query_string), 'clusters')
        return ret
    ...
    def import_ceph_conf(self, cluster_name, ceph_conf_path=None):
        body = {'cluster': {'cluster_name': cluster_name, 
