...
# Embedded file name: build/bdist.linux-x86_64/egg/tests/tools.py
# Compiled at: 2017-02-24 16:57:38
__doc__ = 'Tools used for fullauth tests.'
from pyramid.view import view_config
from pyramid.compat import text_type
DEFAULT_USER = {'username': text_type('u1'), 
   'password': text_type('password1'), 
   'email': text_type('email@example.com'), 
   'address_ip': text_type('127.0.0.1')}

def authenticate(app, email=DEFAULT_USER['email'], password=DEFAULT_USER['password'], remember=False, response_code=303):
    """Authenticate user."""
    res = app.get('/login')
    form = res.form
    form['email'] = email
