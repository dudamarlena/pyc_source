...
# Embedded file name: build/bdist.linux-x86_64/egg/tests/views/test_events.py
# Compiled at: 2017-02-24 16:57:38
__doc__ = 'All events related tests.'
...
from pyramid_fullauth.events import BeforeLogIn, AfterLogIn, AlreadyLoggedIn, BeforeEmailChange, AfterEmailChange, AfterEmailChangeActivation, BeforeReset, AfterResetRequest, AfterReset, AfterSocialRegister, AfterSocialLogIn, SocialAccountAlreadyConnected, AfterActivate, BeforeRegister, AfterRegister
...
evented_config = factories.pyramid_config({'yml.location': 'tests:config', 
   'env': 'login', 
   'pyramid.includes': [
                      'pyramid_tm',
                      'pyramid_fullauth',
...
def test_error_beforelogin(active_user, beforelogin_app):
    ...
    post_data = {'email': DEFAULT_USER['email'], 
...
def test_aftersocialregister(aftersocialregister_config, aftersocialregister_app, db_session):
    ...
    profile = {'accounts': [{'domain': text_type('facebook.com'), 'userid': text_type('2343')}], 'displayName': text_type('teddy'), 
...
def test_alreadyconnected(alreadyconnected_config, alreadyconnected_app, facebook_user, db_session):
    ...
    """
    fresh_user = User(email=text_type('new@user.pl'), password=text_type('somepassword'), address_ip=text_type('127.0.0.1'))
    db_session.add(fresh_user)
    transaction.commit()
    user = db_session.merge(facebook_user)
    fresh_user = db_session.merge(fresh_user)
    profile = {'accounts': [{'domain': text_type('facebook.com'), 'userid': user.provider_id('facebook')}], 'displayName': text_type('teddy'), 
       'preferredUsername': text_type('teddy'), 
       'emails': [{'value': text_type('aasd@basd.pl')}], 'name': text_type('ted')}
    credentials = {'oauthAccessToken': '7897048593434'}
    provider_name = text_type('facebook')
    provider_type = text_type('facebook')
    request = testing.DummyRequest()
...
def test_beforeregister(db_session, beforeregister_app):
    ...
    res = beforeregister_app.post('/register', {'csrf_token': res.form['csrf_token'].value, 
       'email': 'test@test.co.uk', 
       'password': 'passmeplease', 
       'confirm_password': 'passmeplease'}, extra_environ={'REMOTE_ADDR': '0.0.0.0'}, xhr=True)
