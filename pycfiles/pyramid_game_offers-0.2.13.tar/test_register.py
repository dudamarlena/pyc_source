...
# Embedded file name: build/bdist.linux-x86_64/egg/tests/views/test_register.py
# Compiled at: 2017-02-24 16:57:38
__doc__ = 'Registration related tests.'
try:
...
def test_register_wrong_csrf(db_session, default_app):
    ...
    res = res.form.submit(extra_environ={'REMOTE_ADDR': '0.0.0.0'}, status=401)
    transaction.commit()
    assert db_session.query(User).count() == 0
