...
# Embedded file name: build/bdist.linux-x86_64/egg/pyramid_fullauth/views/basic/profile/password.py
# Compiled at: 2017-02-24 16:57:38
__doc__ = 'Profile related views.'
...
@view_defaults(route_name='password:reset', permission=NO_PERMISSION_REQUIRED, renderer='pyramid_fullauth:resources/templates/reset.mako')
class PasswordResetView(BaseView):
    ...
    def post(self):
        ...
        try:
            ...
            return {'status': False, 'msg': self.request._('user-not-exists', default='User does not exist', domain='pyramid_fullauth'), 
...
@view_defaults(route_name='password:reset:continue', permission=NO_PERMISSION_REQUIRED, renderer='pyramid_fullauth:resources/templates/reset.proceed.mako')
    ...
    def post(self):
        ...
        if password == password_confirm:
            try:
                ...
                user.reset_key = None
                try:
                    pyramid_basemodel.Session.query(AuthenticationProvider).filter(AuthenticationProvider.user_id == user.id, AuthenticationProvider.provider == text_type('email')).one()
                except NoResultFound:
                ...
                return {'status': False, 'msg': text_type(e), 
                   'csrf_token': self.request.session.get_csrf_token()}
            ...
            return {'status': False, 
               'msg': self.request._('password-mismatch', default="Password doesn't match", domain='pyramid_fullauth'), 
               'csrf_token': self.request.session.get_csrf_token()}
        return self.get()