...
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/cromosim/ftl.py
# Compiled at: 2020-04-22 16:42:12
...
def update_positions_ftl_order_1(L, X, t, dt, Phi, periodicity=True, V_leader=None):
    """
...
def update_positions_ftl_order_2(L, X, U, t, dt, tau, Phi, periodicity=True, V_leader=None):
    """
...
def plot_people(L, X, t, data, tgrid, periodicity=True, V_leader=None, speed_fct=None, savefig=False, filename='fig.png', ifig=100, spheresize=300):
    """
    ...
    colors = np.ones(N)
    scale = 20
    sc = ax1.scatter((XY[:, 0]), (XY[:, 1]), c=colors, s=spheresize, edgecolor='None', marker='.',
      cmap=(plt.get_cmap('Greys')),
      norm=(plt.Normalize(-0.2, 1)))
    line = Line2D([0, 0], [-0.05, 0.05], lw=3.0, alpha=1, c='k')
    ax1.add_line(line)
    line = Line2D([L, L], [-0.05, 0.05], lw=3.0, alpha=1, c='k')
    ax1.add_line(line)
