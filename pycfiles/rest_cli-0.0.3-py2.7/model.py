...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/generator/model.py
# Compiled at: 2018-06-28 14:30:50
import os
...
class SchemaUtil:
    ...
    @staticmethod
    def intercept_filter_set(php_model_name, php_property_name, php_property_type, mysql_table_name, mysql_column_name, mysql_column_type):
        """
    ...
    @staticmethod
    def intercept_filter_get(php_model_name, php_property_name, php_property_type, mysql_table_name, mysql_column_name, mysql_column_type):
        """
...
class ModelGenerator:
    ...
    def scan_by_model_file_name(self, model_file_name):
        ...
        with open(model_file_path, 'r') as (stream):
            ...
            if len(model_components) != 2:
                ...
                self._models[model_name] = {'content': model_content, 
                   'filters': annotations, 
    ...
    def scan_database(self):
        ...
        for table_name in tables:
            ...
            for column in columns:
                ...
                if column[2] == 'NO':
                    column_null = False
                table = {'php_model_name': SchemaUtil.convert_table_to_camel_name(table_name), 'php_property_name': SchemaUtil.convert_column_to_camel_name(column_name), 
    ...
    def __generate_constructor(model_name, columns):
        ...
        constructor = Helper.bind(MODEL_CONSTRUCTOR_TEMPLATE, {'default_setters': default_setters, 
    ...
    def __generate_setter_getter(model_name, fields):
        ...
        for field in fields:
            ...
            field_set_get_content = field_set_get_content.replace('{intercept_filter_set}', SchemaUtil.intercept_filter_set(php_model_name=model_name, php_property_name=field['php_property_name'], php_property_type=field['php_property_type'], mysql_table_name=field['mysql_table_name'], mysql_column_name=field['mysql_column_name'], mysql_column_type=field['mysql_column_type']))
            field_set_get_content = field_set_get_content.replace('{intercept_filter_get}', SchemaUtil.intercept_filter_get(php_model_name=model_name, php_property_name=field['php_property_name'], php_property_type=field['php_property_type'], mysql_table_name=field['mysql_table_name'], mysql_column_name=field['mysql_column_name'], mysql_column_type=field['mysql_column_type']))
            field_set_get_content = Helper.bind(field_set_get_content, {'table_name': model_name, 
    ...
    def create_model(self, table_name):
        ...
        if model_name not in self._models:
            ...
            model_content = Helper.bind(MODEL_TEMPLATE, {'model_name': model_name, 
    ...
    def create_service_files(service_folder_path, service_name, model_name):
        ...
        :param model_name:
        """
        service_interface_file_path = Helper.get_absolute_path(service_folder_path + '/' + service_name + '.php')
        repository_implementation_file_path = Helper.get_absolute_path(service_folder_path + '/' + service_name + 'Impl.php')
        service_name_with_space = Helper.convert_camel_to_description(service_name)
        service_interface_content = Helper.bind(SERVICE_INTERFACE_TEMPLATE, {'model_name': model_name, 
           'service_name': service_name, 
           'service_name_with_space': service_name_with_space})
        service_implementation_content = Helper.bind(SERVICE_IMPLEMENTATION_TEMPLATE, {'model_name': model_name, 
    ...
    def create_repository_files(repository_folder_path, repository_name, model_name):
        ...
        :param model_name:
        """
        repository_interface_file_path = Helper.get_absolute_path(repository_folder_path + '/' + repository_name + '.php')
        repository_implementation_file_path = Helper.get_absolute_path(repository_folder_path + '/' + repository_name + 'Impl.php')
        repository_name_with_space = Helper.convert_camel_to_description(repository_name)
        repository_interface_content = Helper.bind(REPOSITORY_INTERFACE_TEMPLATE, {'model_name': model_name, 
           'repository_name': repository_name, 
           'repository_name_with_space': repository_name_with_space})
        repository_implementation_content = Helper.bind(REPOSITORY_IMPLEMENTATION_TEMPLATE, {'model_name': model_name, 
           'repository_name': repository_name, 
           'repository_name_with_space': repository_name_with_space})
    ...
    def get_repository_builtin_content(template, repository_name, model_name):
        ...
        repository_implementation_content = Helper.bind(template, {'model_name': model_name, 
            ...
            for fields in filters[annotation_name]:
                ...
                for field_name in fields:
                    ...
                    if not (query_type == 'update' and field_name == 'data'):
                        constants_with_values.append('            ' + model_name + '::' + constant_name + ' => ' + variable_name)
                ...
                repository_implementation_content += Helper.bind(template, {'returned_type': returned_type, 
    ...
    def generate_repository(self, template_builtin, template_filter, repository_folder_path, repository_name, model_name):
        """
        ...
        with open(repository_file_path, 'r') as (stream):
            ...
            if len(repository_components) != 2:
                ...
                repository_builtin = self.get_repository_builtin_content(template_builtin, repository_name, model_name)
                repository_filter = self.get_repository_filter_content(template_filter, repository_name, model_name)
    ...
    def generate_repositories(self):
        ...
        for model in self._models:
            ...
            if table_name not in self._tables:
                ...
                repository_folder_path_each_model = Helper.get_absolute_path(repository_folder_path + '/' + repository_name)
                service_folder_path_each_model = Helper.get_absolute_path(service_folder_path + '/' + service_name)
                ...
                repository_file_path = Helper.get_absolute_path(repository_folder_path_each_model + '/' + repository_name + '.php')
                service_file_path = Helper.get_absolute_path(service_folder_path_each_model + '/' + service_name + '.php')
                ...
                self.generate_repository(REPOSITORY_INTERFACE_BUILTIN_TEMPLATE, REPOSITORY_INTERFACE_FILTER_TEMPLATE, repository_folder_path_each_model, repository_name, model_name=model)
                self.generate_repository(REPOSITORY_IMPLEMENTATION_BUILTIN_TEMPLATE, REPOSITORY_IMPLEMENTATION_FILTER_TEMPLATE, repository_folder_path_each_model, repository_name + 'Impl', model_name=model)
