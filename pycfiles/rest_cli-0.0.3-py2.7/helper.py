...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/generator/helper.py
# Compiled at: 2018-06-25 01:24:36
import re, json, os.path, mysql.connector
...
class Helper:
    ...
    def camel_case_to_snake_case(name):
        ...
        :return: string
        """
        string = re.sub('(.)([A-Z][a-z]+)', '\\1_\\2', name)
        string = re.sub('(.)([0-9]+)', '\\1_\\2', string)
        result = re.sub('([a-z0-9])([A-Z])', '\\1_\\2', string).lower()
        return result
    ...
    def split_camel_case(camel_case_name):
        ...
        :return:
        """
        return re.sub('(?!^)([A-Z][a-z]+)', ' \\1', camel_case_name).split()
    ...
    def connect_database():
        ...
        connection = mysql.connector.connect(user=db_username, password=db_password, host=db_host, database=db_name, raise_on_warnings=True)
