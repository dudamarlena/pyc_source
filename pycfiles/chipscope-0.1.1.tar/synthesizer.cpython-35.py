...
class Synthesizer:
    ...
    def add_wave_header(self, pcm_data):
        ...
        """
        header = struct.pack('<4sI8sIHHIIHH4sI', 'RIFF', len(pcm_data) + 44 - 8, 'WAVEfmt ', 16, 1, self._channels, self.framerate, self.framerate * self._channels * self._bits // 8, self._channels * self._bits // 8, self._bits, 'data', len(pcm_data))
