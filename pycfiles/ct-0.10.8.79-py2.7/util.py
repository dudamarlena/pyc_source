...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cantools/web/util.py
# Compiled at: 2019-11-03 18:24:27
...
def send_pdf(data, title=None):
    if title:
        _headers({'Content-Type': 'application/pdf; name="%s.pdf"' % (title,), 
...
def verify_recaptcha(cresponse, pkey):
    import os
    verification_result = urlopen(Request(url='https://www.google.com/recaptcha/api/siteverify', data=urlencode({'secret': pkey, 
       'remoteip': os.environ.get('REMOTE_ADDR', os.environ.get('REMOTE_HOST')), 
       'response': cresponse}).encode(), headers={'Content-type': 'application/x-www-form-urlencoded'}))
