...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cantools/_db.py
# Compiled at: 2020-05-06 18:59:13
import json
    ...
    if action == 'schema':
        ...
        if not res:
            ...
            if mname:
                ...
                res = get_page(mname, int(cgi_get('limit')), int(cgi_get('offset')), order, cgi_get('filters', default={}), count=cgi_get('count', required=False), exporter=exporter)
            elif keys:
        ...
        if cgi_get('delBlob', default=False):
            ...
            if not value and hasattr(ent, 'getBlob'):
                blob = ent.getBlob()
            if config.web.server == 'gae':
                send_file(blob)
            else:
        ...
        setmem('last_index', admin.index(cgi_get('kind', default='*'), getmem('last_index', False) or int(cgi_get('index', default=0))), False)
    return
