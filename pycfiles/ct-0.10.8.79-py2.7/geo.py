...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cantools/geo.py
# Compiled at: 2020-03-27 16:57:30
import os
try:
...
class Geo(object):
    ...
    def address2latlng(self, address):
        if self._no_cache(address):
            ...
            if not len(results):
                log('no results!!!', 4)
                self.cache[address] = {'lat': None, 
                   'lng': None}
            else:
                loc = results[0]['geometry']['location']
                self.cache[address] = {'lat': loc['lat'], 
                   'lng': loc['lng']}
            self.savecache()
        return [
         self.cache[address]['lat'], self.cache[address]['lng']]
