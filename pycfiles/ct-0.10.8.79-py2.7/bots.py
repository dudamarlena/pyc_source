...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cantools/scripts/pubsub/bots.py
...
class Bot(with_metaclass(BotMeta, Actor)):
    ...
    def pub(self, message):
        self.server.publish({'message': message, 
           'channel': self.channel.name}, self)
...
class Monitor(Bot):
    ...
    def __init__(self, server, channel, name='monitor'):
        import event
        self.current = {}
        self.alert = {}
    ...
    def _cpu(self):
        ...
        elif c >= config.admin.monitor.thresholds.cpu:
            self.alert['cpu'] = True
    ...
    def _tick(self):
        ...
        dmon = fetch(config.admin.host, '/_report', config.admin.port, True, protocol=config.admin.protocol)
        data = {'gc': dmon['gc'], 
           ...
           'totals': {'web': {'connections': dmon['web']['total_connections'], 
                              'requests': dmon['web']['total_requests'], 
                              'rolls': dmon['web']['rolls']}, 
                      'admin': {'connections': dmon['admin']['total_connections'], 
                                'requests': dmon['admin']['total_requests'], 
                                'rolls': dmon['admin']['rolls']}}, 
           'devices': {'web': dmon['web']['devices'], 
                       'admin': dmon['admin']['devices']}, 
           'ips': {'web': dmon['web']['ips'], 
                   'admin': dmon['admin']['ips']}}
        if config.admin.monitor.proxy:
            data['ips']['proxy'] = fetch(config.admin.host, '/_report', config.admin.monitor.proxy, True)['ips']
        self.log(data)
        return True