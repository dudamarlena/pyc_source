...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cantools/scripts/migrate.py
...
def load_model(model, host, port, session, filters={}, protocol='http', pw=None, action='put'):
    ...
    def push(data):
        log(post(host, '/_db', port, {'pw': pw, 
           'action': action, 
           'data': data}, protocol=protocol, ctjson=True))
...
def fixkeys(d, schema):
    if 'ctkey' in d:
        ...
        if d['index'] in indices:
            error('duplicate index! (%s)' % (d['index'],), "try running 'ctindex -m index' ;)")
        indices.add(d['index'])
    ...
    for d in blobs:
        for key, prop in list(db.get_schema(d['modelName']).items()):
            if prop == 'blob' and d[key]:
                ...
                d[key] = fetch(host, port=port, path='/_db?action=blob&key=%s&property=%s' % (entkey, key), protocol=port == 443 and 'https' or 'http')
...
def dump(host, port, session, binary, skip=[]):
    ...
    for model in schemas:
        ...
        offset = 0
        while 1:
            chunk = fetch(host, port=port, ctjson=True, path='/_db?action=get&modelName=%s&offset=%s&limit=%s' % (model, offset, LIMIT), protocol=port == 443 and 'https' or 'http')
            for c in chunk:
                fixkeys(c, schema)
                checkblobs(c, schema)
...
def go():
    ...
    parser.add_option('-d', '--domain', dest='domain', default='localhost', help='domain of target server (default: localhost)')
    parser.add_option('-p', '--port', dest='port', default=8080, help='port of target server (default: 8080)')
    parser.add_option('-f', '--filename', dest='filename', default='dump.db', help='name of sqlite data file for dumping/loading to/from (default: dump.db)')
    parser.add_option('-s', '--skip', dest='skip', default='', help="don't dump these tables - use '|' as separator, such as 'table1|table2|table3' (default: none)")
    parser.add_option('-n', '--no_binary', dest='binary', action='store_false', default=True, help='disable binary download')
    ...
    if mode in MODES:
        ...
        error("invalid mode specified ('%s')" % (mode,), "must be 'ctmigrate load' or ctmigrate dump'")
