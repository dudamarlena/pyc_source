...
# [GCC 8.4.0]
# Embedded file name: /Users/leon/.local/lib/python2.7/site-packages/ROIWidgets/__init__.py
# Compiled at: 2014-11-25 13:55:58
"""ROI Widgets. Create widgets in the ipython notebook that allow user
to select regions of interest from an image.
"""
import os, PIL, hashlib, numpy as np, math
...
class ROIMultiPointWidget(widgets.DOMWidget):
    ...
    imageURLBase = Unicode('/static/scratch/', sync=True)
    ...
    def __init__(self, image=None, description='', maxScale=1.0, scratchDir=None, imageURLBase='/static/scratch/', windowWidth=1024, windowHeight=1024, handleSize=10, handleAttrs={'stroke': '#000', 'fill': 'blue'}, pathAttrs={'stroke': '#000'}):
        """
        ...
        self.handles = {}
        self.get_state()
        self.scale = min(float(self.windowWidth) / self.imageWidth, float(self.windowHeight) / self.imageHeight, self.maxScale)
