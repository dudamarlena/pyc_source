...
# Embedded file name: /Users/cosmin/workspace/github/cleanmymac/build/lib/cleanmymac/target.py
# Compiled at: 2016-03-06 17:26:23
# Size of source mod 2**32: 12775 bytes
import click, os, re
...
class ShellCommandTarget(Target):
    ...
    def __init__(self, config, update=False, verbose=False):
        ...
        if 'PATH' in self._env:
            path = '{0}:{1}'.format(os.environ['PATH'], os.path.expanduser(self._env['PATH']))
        self._env['PATH'] = path
    ...
    def _to_remove(self):
        for entry in self.entries:
            ...
            if 'pattern' in entry:
                ...
                dirs = [os.path.join(_dir, d) for d in os.listdir(_dir) if os.path.isdir(os.path.join(_dir, d)) and re.match(_pattern, d)]
...
class YamlDirTarget(DirTarget):
    ...
    @property
    def update_message(self):
        if 'update_message' in self._spec:
            return self._spec['update_message']
        return ''