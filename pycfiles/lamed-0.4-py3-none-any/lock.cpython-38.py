...
# [GCC 8.4.0]
# Embedded file name: /private/var/folders/pb/598z8h910dvf2wrvwnbyl_2m0000gn/T/pip-install-03p63p8r/redis/redis/lock.py
...
class Lock(object):
    ...
    def owned(self):
        ...
        if stored_token:
            if not isinstance(stored_token, bytes):
                encoder = self.redis.connection_pool.get_encoder()
    ...
    def do_reacquire(self):
        timeout = int(self.timeout * 1000)
        if not bool(self.lua_reacquire(keys=[self.name], args=[
         self.local.token, timeout],
          client=(self.redis))):
