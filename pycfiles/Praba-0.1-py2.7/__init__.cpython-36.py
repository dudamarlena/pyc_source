...
class TextGrid(OrderedDict):
    ...
    def _format_binary(self):
        ...
        out = 'ooBinaryFile\x08TextGrid'
        out += struct.pack('>2d?i', self.xmin, self.xmax, True, len(self))
        for name, tier in self.items():
            typ = 'PointTier' if tier.is_point_tier else 'IntervalTier'
    ...
    def parse(self, data):
        ...
        if not isinstance(data, bytes):
            ...
            binary = 'ooBinaryFile\x08TextGrid'
    ...
    def _parse_binary(self, data):
        ...
        for i in range(tiers):
            ...
            desc = data.read(size)
            if desc in ('PointTier', 'TextTier'):
                point_tier = True
            else:
                if desc == 'IntervalTier':
                    point_tier = False
                else:
                    raise BinaryError
