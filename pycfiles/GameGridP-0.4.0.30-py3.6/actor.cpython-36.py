...
class Actor(pygame.sprite.DirtySprite):
    ...
    def __init__(self, grid, location: tuple=(0, 0), color: tuple=(0, 0, 255), title: str='Actor', img_path: str=None, size: tuple=(40, 40), img_action: str=None):
        ...
        self._Actor__bounding_box_size = None
    ...
    def image_rect(self):
        ...
        """
        try:
            left, top = self._Actor__get_image_coordinates_in_pixels()
    ...
    def image(self):
        ...
        """
        if self.has_image:
            cell_left, cell_top = self._Actor__get_image_coordinates_in_pixels()
            self._Actor__flip_x()
            ...
            if self.grid._show_direction_marker:
                ...
                x = round(self.rect.width / 2 + math.cos(math.radians(self.direction)) * self.rect.width)
                y = round(self.rect.height / 2 - math.sin(math.radians(self.direction)) * self.rect.height)
    ...
    def location(self, value: tuple):
        ...
        self._Actor__grid.update(no_logic=True)
        self.dirty = 1
    ...
    def add_image(self, img_path: str, img_action: str=None, size=None):
        ...
        if img_action is None:
            if self.grid.cell_size > 1:
                img_action = 'scale'
        self._logging.info('img_action:' + str(img_action))
        ...
        if img_path in self.grid.images_dict:
            ...
            _image = self._Actor__image_transform(_image, img_action, size)
    ...
    def _next_sprite(self):
        ...
        if self._animated:
            if self.grid.frame % self.animation_speed == 0:
                ...
                self.dirty = 1
                self._logging.info('actor.image_next() : Image Index:' + str(self._image_index) + '/' + str(self._original_images.__len__() - 1))
    ...
    def rotate(self, direction: int):
        ...
        if self.is_rotatable:
            self._logging.debug('actor.__rotate: rotate Image' + str(dir(self._original_images)) + ', Image: ' + str(self._image) + 'Index:' + str(self._image_index))
    ...
    def is_at_border(self) -> str:
        ...
        if self.is_at_left_border():
            ...
            if self.is_at_right_border():
                ...
                if self.is_at_bottom_border():
                    return 'bottom'
                if self.is_at_top_border():
                    return 'top'
            return False
    ...
    def get_all_actors_at_my_location(self, class_name) -> list:
        """
    ...
    def get_actor_in_front(self, class_name, distance=1) -> list:
        ...
        actors_at_location = self.grid.get_actor_at_location(location, class_name)
        return actors_at_location

    def get_actor_at_my_location(self, class_name, location=None) -> list:
        """
        Gibt *einen* Akteur  an der Position des Actors zurück.a
    ...
    def __image_transform(self, image: int, img_action: str, size: str=None) -> image:
        """
    ...
    def set_bounding_box_size(self, value):
        ...
        """
        self._Actor__bounding_box_size = value

    def __get_image_coordinates_in_pixels(self, location: tuple=None):
        """
        ...
        if location == None:
            ...
            column = location[0]
            row = location[1]
        cell_margin = self._Actor__grid.cell_margin
        cell_size = self._Actor__grid.cell_size
    ...
    def move(self, distance: int=1, move: bool=True):
        ...
        self._logging.debug('actor' + str(self) + '.move(): ...try to move from' + str(self.location) + ' to target' + str(target) + ' direction:' + str(self.direction))
    ...
    def is_valid_target(self, target: tuple=None):
        ...
        if target is None:
            ...
            if self.is_in_grid(target):
                ...
                valid = False
        actors_at_position = self._Actor__grid.get_all_actors_at_location(target)
        for actor in actors_at_position:
            if actor and actor.is_blocking:
                valid = False
