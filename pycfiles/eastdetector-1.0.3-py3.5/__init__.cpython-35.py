...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/eastdetector/__init__.py
# Compiled at: 2020-01-29 06:13:52
# Size of source mod 2**32: 8934 bytes
"""
...
import tensorflow as tf, numpy as np, cv2, eastdetector.model, lanms
...
def _restore_rectangle(origin: np.array, geometry: np.array) -> np.array:
    ...
    if origin_0.shape[0] > 0:
        ...
        rotate_matrix_x = np.array([np.cos(angle_0), np.sin(angle_0)]).transpose((1,
                                                                                  0))
        rotate_matrix_x = np.repeat(rotate_matrix_x, 5, axis=1).reshape(-1, 2, 5).transpose((0,
                                                                                             2,
                                                                                             1))
        rotate_matrix_y = np.array([-np.sin(angle_0), np.cos(angle_0)]).transpose((1,
                                                                                   0))
        rotate_matrix_y = np.repeat(rotate_matrix_y, 5, axis=1).reshape(-1, 2, 5).transpose((0,
                                                                                             2,
                                                                                             1))
    ...
    if origin_1.shape[0] > 0:
        p = np.array([-d_1[:, 1] - d_1[:, 3],
         ...
         -d_1[:, 1], -d_1[:, 2]])
        p = p.transpose((1, 0)).reshape((-1, 5, 2))
        rotate_matrix_x = np.array([np.cos(-angle_1), -np.sin(-angle_1)]).transpose((1,
                                                                                     0))
        rotate_matrix_x = np.repeat(rotate_matrix_x, 5, axis=1).reshape(-1, 2, 5).transpose((0,
                                                                                             2,
                                                                                             1))
        rotate_matrix_y = np.array([np.sin(-angle_1), np.cos(-angle_1)]).transpose((1,
                                                                                    0))
        rotate_matrix_y = np.repeat(rotate_matrix_y, 5, axis=1).reshape(-1, 2, 5).transpose((0,
                                                                                             2,
                                                                                             1))
...
class TextDetector:
    ...
    def __init__(self, checkpoint_path: str):
        ...
        self._TextDetector__closed = False
        ...
        self._TextDetector__input_image = tf.placeholder(np.float32, shape=[None, None, None, 3], name='input_image')
        self._TextDetector__f_score, self._TextDetector__f_geometry = eastdetector.model.model(self._TextDetector__input_image, is_training=False)
        self._TextDetector__session = tf.Session()
        global_step = tf.get_variable('global_step', [], initializer=tf.constant_initializer(0), trainable=False)
        variable_averages = tf.train.ExponentialMovingAverage(0.997, global_step)
        saver = tf.train.Saver(variable_averages.variables_to_restore())
        saver.restore(self._TextDetector__session, model_path)

    def __enter__(self):
        if self._TextDetector__closed:
    ...
    def close(self) -> None:
        self._TextDetector__session.close()
        self._TextDetector__closed = True
    ...
    def detect(self, image: np.ndarray, params=TextDetectorParams()) -> Union[(None, np.ndarray)]:
        ...
        score_map, geo_map = self._TextDetector__session.run([
         self._TextDetector__f_score, self._TextDetector__f_geometry], feed_dict={self._TextDetector__input_image: [image_resized]})
        if len(score_map.shape) == 4:
            score_map = score_map[0, :, :, 0]
            geo_map = geo_map[0, :, :]
        xy_text = np.argwhere(score_map > params.threshold_score_map)
