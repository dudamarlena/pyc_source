...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/microne/app.py
# Compiled at: 2011-04-03 21:41:19
...
class Microne(object):
    ...
    def cache(self, expire=0, namespace_func=None):
        ...
        def decorate(func, *args, **kws):
            ...
            def do_cache(*args, **kws):
                ...
                try:
                    p = urlsplit(self.request.url)[2]
                    memcache.set(p, {'hdr': resp.headers, 'body': out.read()}, expire, namespace=namespace)
    ...
    def get_handler(cls):
        ...
        """
        if not cls.hnd:
            raise ValueError('You must set handler by using set_hnd() method, before calling get_handler() method.')
        return cls.hnd
