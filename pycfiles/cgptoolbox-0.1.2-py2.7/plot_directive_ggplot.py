...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\cgp\utils\plot_directive_ggplot.py
...
The ``plot`` directive supports the following options:
    ...
    include-source : bool
        ...
        using the `ggplot_include_source` variable in conf.py
...
import matplotlib, matplotlib.cbook as cbook
matplotlib.use('Agg')
import matplotlib.pyplot as plt
...
try:
    ...
    if 'posix' in sys.builtin_module_names:
        ...
        def relpath(path, start=os.path.curdir):
            """Return a relative version of a path"""
            from os.path import sep, curdir, join, abspath, commonprefix, pardir, splitunc
            if not path:
                raise ValueError('no path specified')
            start_list = abspath(start).split(sep)
            path_list = abspath(path).split(sep)
...
def setup(app):
    ...
    try:
        ...
        raise SkipTest('Skipping incorrectly detected setup() function in plot_directive_ggplot.')

    setup.confdir = app.confdir
    options = {'alt': directives.unchanged, 'height': directives.length_or_unitless, 
    ...
    app.add_config_value('ggplot_html_show_formats', True, True)
    app.add_config_value('ggplot_rcparams', {}, True)
...
def render_figures(code, code_path, output_dir, output_base, context, function_name, config):
    """
    Run a rnumpy/ggplot script and save the low and high res PNGs and a PDF
    in outdir.
    ...
    *output_base*
    ...
    formats = []
    for fmt in config.ggplot_formats:
    ...
    if all_exists:
        ...
        for i, code_piece in enumerate(code_pieces):
            if not context:
                clear_state(config.ggplot_rcparams)
...
def run(arguments, content, options, state_machine, state, lineno):
    ...
    if len(arguments):
        if not config.ggplot_basedir:
            source_file_name = os.path.join(setup.app.builder.srcdir, directives.uri(arguments[0]))
        else:
            source_file_name = os.path.join(setup.confdir, config.ggplot_basedir, directives.uri(arguments[0]))
        caption = ('\n').join(content)
        if len(arguments) == 2:
        ...
        counter = document.attributes.get('_ggplot_counter', 0) + 1
        document.attributes['_ggplot_counter'] = counter
        base, ext = os.path.splitext(os.path.basename(source_file_name))
        output_base = '%s-ggplot-%d.py' % (base, counter)
        function_name = None
        caption = ''
    ...
    build_dir = os.path.join(os.path.dirname(setup.app.doctreedir), 'ggplot_directive', source_rel_dir)
    ...
    try:
        results = render_figures(code, source_file_name, build_dir, output_base, context, function_name, config)
        errors = []
    except PlotError as err:
        reporter = state.memo.reporter
        sm = reporter.system_message(2, 'Exception occurred in plotting %s\n from %s:\n%s' % (output_base,
         source_file_name, err), line=lineno)
        results = [(code, [])]
        errors = [sm]
    ...
    for j, (code_piece, images) in enumerate(results):
        ...
        if nofigs:
            images = []
        opts = [ ':%s: %s' % (key, val) for key, val in options.items() if key in ('alt',
                                                                                   'height',
                                                                                   'width',
                                                                                   'scale',
                                                                                   'align',
        ...
        if j == 0:
            ...
            src_link = None
        result = format_template(TEMPLATE, dest_dir=dest_dir_link, build_dir=build_dir_link, source_link=src_link, multi_image=len(images) > 1, only_html=only_html, only_latex=only_latex, options=opts, images=images, source_code=source_code, html_show_formats=config.ggplot_html_show_formats, caption=caption)
