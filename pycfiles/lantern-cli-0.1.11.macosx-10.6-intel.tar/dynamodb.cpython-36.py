...
# [GCC 8.4.0]
# Embedded file name: /Users/jceciliano/Documents/coding/lantern/lantern-cli/.virtualenv/lib/python3.6/site-packages/lantern_cli/dynamodb.py
# Compiled at: 2019-05-22 19:57:07
...
def delete_method():
    ...
    click.echo('We will use {amazon_profile}, which should be defined in {folder}'.format(amazon_profile=click.style(('[' + amazon_profile + ']'), fg='green'),
    ...
    while is_adding_filters:
        ...
        click.echo('New Filter ' + click.style(('%s %s %s' % (column_name, operator, value1)), fg='green') if not value2 else click.style(('%s %s (%s, %s)' % (column_name, operator, value1, value2)), fg='green'))
    ...
    click.echo('Fetching data from table: ' + click.style(dynamodb_table, fg='green') + ' with following filters \n' + '\n'.join(['\t - ' + '%s %s %s' % (x['column_name'], x['operator'], x['value1']) if not x['value2'] else '%s %s (%s, %s)' % (x['column_name'], x['operator'], x['value1'], x['value2']) + '\n' for x in filter_options]))
