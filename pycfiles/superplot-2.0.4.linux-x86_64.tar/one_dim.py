...
# [GCC 8.4.0]
# Embedded file name: /home/michael/anaconda2/lib/python2.7/site-packages/superplot/statslib/one_dim.py
...
import numpy as np, point, warnings
...
@memory.cache
def kde_posterior_pdf(parameter, posterior, npoints=500, bin_limits=None, norm_area=False, bw_method='scott', fft=True):
    """
    ...
    if bin_limits:
        ...
        upper = max(parameter)
        lower = min(parameter)
    kde_func = gaussian_kde(parameter, weights=posterior, bw_method=bw_method, fft=fft)
...
def posterior_pdf(parameter, posterior, nbins=50, bin_limits=None, norm_area=False):
    ...
    """
    pdf, bin_edges = np.histogram(parameter, nbins, range=bin_limits, weights=posterior, density=norm_area)
    if not norm_area:
        pdf = pdf / pdf.max()
...
def prof_data(parameter, chi_sq, nbins=50, bin_limits=None):
    ...
    """
    bin_edges = np.histogram(parameter, nbins, range=bin_limits)[1]
    bin_centers = (bin_edges[:-1] + bin_edges[1:]) * 0.5
...
def conf_interval(chi_sq, bin_centers, alpha):
    ...
    [-2409.8500561616, 2570.0887645632]
    """
    critical_chi_sq = stats.chi2.ppf(1.0 - alpha, 1)
...
def posterior_mode(pdf, bin_centers):
    ...
    if len(max_indices) > 1:
        warnings.warn(('posterior_mode: max count shared by {} bins').format(len(max_indices)))
