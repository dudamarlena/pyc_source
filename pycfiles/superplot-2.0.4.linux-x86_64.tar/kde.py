...
# [GCC 8.4.0]
# Embedded file name: /home/michael/anaconda2/lib/python2.7/site-packages/superplot/statslib/kde.py
...
class gaussian_kde(object):
    ...
    def _bin_dataset(self):
        ...
        if self.n_dims == 1:
            nbins = self.len_data
            binned_pdf, bin_edges = np.histogram(self.dataset[0], bins=nbins, normed=True, weights=self.weights)
            bin_centers = np.array((bin_edges[:-1] + bin_edges[1:]) * 0.5)
        elif self.n_dims == 2:
            nbins = int(self.len_data ** 0.5)
            binned_pdf, bin_edges_x, bin_edges_y = np.histogram2d(self.dataset[0], self.dataset[1], bins=nbins, normed=True, weights=self.weights)
    ...
    def _fft_kde(self):
        ...
        if self.n_dims == 1:
            ...
            kde = interp1d(bin_centers, pdf, bounds_error=False, fill_value=0.0)

            def kde_func(points):
                """ Pass array of points through KDE interpolation function. """
                kde_ = np.array([ max(0.0, kde(x)) for x in points ])
                return kde_
        ...
        if self.n_dims == 2:
            ...
            pdf /= pdf.sum() * bin_vol
            kde = interp2d(bin_centers_x, bin_centers_y, pdf.T, bounds_error=False, fill_value=0.0)

            def kde_func(points):
                """ Pass array of points through KDE interpolation function. """
                kde_ = np.array([ max(0.0, kde(x, y)) for x, y in points.T ])
                return kde_
    ...
    def _kde_func(self, points):
        ...
        gauss_norm = (2.0 * pi) ** (-0.5 * self.n_dims)
    ...
    def _silverman_factor(self):
        ...
        """
        neff = self.sum_weights_squared ** (-1)
        return (0.25 * neff * (self.n_dims + 2.0)) ** (-1.0 / (self.n_dims + 4.0))
    ...
    def _compute_covariance(self):
        ...
        unscaled_cov = residual_squared / (1.0 - self.sum_weights_squared)
