...
# [GCC 8.4.0]
# Embedded file name: /home/michael/anaconda2/lib/python2.7/site-packages/superplot/super_gui.py
# Compiled at: 2017-08-19 22:58:02
"""
This module provides the superplot GUI.
"""
import os, re, pickle, time, warnings
...
if version < required_version:
    raise ImportError('Superplot requires matplotlib %s. You are running matplotlib %s. Upgrade via e.g. pip install --force-reinstall --upgrade matplotlib' % (
     required_version, version))
import gtk, pygtk
try:
    from matplotlib.backends.backend_gtkagg import FigureCanvasGTKAgg as FigureCanvas
except ImportError as e:
    print 'Could not load matplotlib - GTK backend. Your version of matplotlib may not be compiled with GTK support. Reinstalling matplotlib may fix this problem - see README or user manual for instructions'
    raise

import data_loader, superplot.plotlib.plots as plots
from plot_options import plot_options, default
pygtk.require('2.0')

def open_file_gui(window_title='Open', set_name=None, add_pattern=None, allow_no_file=True):
    r"""
    ...
    if allow_no_file:
        buttons = (
         no_file, gtk.RESPONSE_CANCEL,
         gtk.STOCK_OPEN, gtk.RESPONSE_OK)
    else:
        buttons = (
         gtk.STOCK_OPEN, gtk.RESPONSE_OK)
    dialog = gtk.FileChooserDialog(title=window_title, action=gtk.FILE_CHOOSER_ACTION_OPEN, buttons=buttons)
...
def save_file_gui(window_title='Save As', set_name=None, add_pattern=None):
    ...
    buttons = (
     ...
     gtk.STOCK_OPEN, gtk.RESPONSE_OK)
    dialog = gtk.FileChooserDialog(title=window_title, action=gtk.FILE_CHOOSER_ACTION_SAVE, buttons=buttons)
    dialog.set_default_response(gtk.RESPONSE_OK)
    dialog.set_current_folder(os.getcwd())
    file_filter = gtk.FileFilter()
    if set_name:
...
def message_dialog(message_type, message):
    ...
    :type message: string
    """
    md = gtk.MessageDialog(None, gtk.DIALOG_DESTROY_WITH_PARENT, message_type, gtk.BUTTONS_CLOSE, message)
    md.run()
    md.destroy()
    return
...
class GUIControl(object):
    ...
    def __init__(self, data_file, info_file, default_plot_type=0):
        ...
        self.typebox.set_active(default_plot_type)
    ...
    def _calimits(self, textbox):
        ...
        if not textbox.get_text().strip():
            ...
            self.plot_limits = re.split('\\s*[,;]\\s*', textbox.get_text())
    ...
    def _cblimits(self, textbox):
        ...
        if not textbox.get_text().strip():
            self.bin_limits = default('bin_limits')
            return
        self.bin_limits = re.split('\\s*[,;]\\s*', textbox.get_text())
    ...
    def _pmakeplot(self, button):
        ...
        :type button:
        """
        args = {'xindex': self.xindex, 'yindex': self.yindex, 
           'zindex': self.zindex, 
        ...
        canvas = FigureCanvas(self.fig.figure)
    ...
    def _psave(self, button):
        ...
        if not (save_image or save_summary or save_pickle):
            ...
            return
        file_name = save_file_gui(set_name='Save plot as image', add_pattern=[
         '*.pdf',
         '*.png',
         '*.eps',
         '*.ps'])
...
def main():
    ...
    """
    data_file = open_file_gui(window_title='Select a MultiNest *.txt file', set_name='MultiNest *.txt file', add_pattern=[
     '*.txt'], allow_no_file=False)
    info_file = open_file_gui(window_title='Select an information file', set_name='Information file *.info describing *.txt file', add_pattern=[
     '*.info'], allow_no_file=True)
