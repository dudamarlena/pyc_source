...
# [GCC 8.4.0]
# Embedded file name: /home/michael/anaconda2/lib/python2.7/site-packages/superplot/statslib/two_dim.py
...
import point, numpy as np, warnings
DOCTEST_PRECISION = 10
_kde_posterior_pdf_2D = namedtuple('_kde_posterior_pdf_2D', ('pdf', 'bin_centers_x',
                                                             'bin_centers_y'))
_posterior_pdf_2D = namedtuple('_posterior_pdf_2D', ('pdf', 'bin_centers_x', 'bin_centers_y'))
_profile_data_2D = namedtuple('_profile_data_2D', ('prof_chi_sq', 'prof_like', 'bin_center_x',
                                                   'bin_center_y'))

@memory.cache
def kde_posterior_pdf(paramx, paramy, posterior, npoints=100, bin_limits=None, bw_method='scott', fft=True):
    r"""
    ...
    if bin_limits:
        ...
        upper_y = max(paramy)
        lower_y = min(paramy)
    kde_func = gaussian_kde(np.array((paramx, paramy)), weights=posterior, bw_method=bw_method, fft=fft)
...
def posterior_pdf(paramx, paramy, posterior, nbins=50, bin_limits=None):
    ...
    >>> assert len(y) == nbins
    """
    pdf, bin_edges_x, bin_edges_y = np.histogram2d(paramx, paramy, nbins, range=bin_limits, weights=posterior)
    pdf = pdf / pdf.max()
...
def profile_like(paramx, paramy, chi_sq, nbins, bin_limits=None):
    ...
    :rtype: named tuple (        profchi_sq: numpy.ndarray,         prof_like: numpy.ndarray,         bin_center_x: numpy.ndarray,         bin_center_y: numpy.ndarray)
    ...
    >>> assert len(y) == nbins
    """
    _, bin_edges_x, bin_edges_y = np.histogram2d(paramx, paramy, nbins, range=bin_limits, weights=None)
    ...
    for index in range(chi_sq.size):
        bin_numbers = (
         bin_numbers_x[index], bin_numbers_y[index])
...
def critical_density(pdf, alpha):
    ...
    try:
        _critical_density = bisect(delta_prob, 0.0, pdf.max())
...
def posterior_mode(pdf, bin_centers_x, bin_centers_y):
    ...
    if len(max_indices) > 1:
        warnings.warn(('posterior mode: max count shared by {} bins').format(len(max_indices)))
