...
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/contentlet/tests/test_provider.py
# Compiled at: 2010-05-23 04:58:05
__doc__ = ' Tests for contentlet.provider.'
import unittest
...
class TestGetProvider(unittest.TestCase):
    ...
    def test_get_provider_no_provider(self):
        self.assertRaises(LookupError, get_provider, 'name', registry=self.registry)

    def test_get_provider_for_context(self):
        from zope.interface import implementedBy
        self._registerProvider(DummyContentProvider('content'), 'name', context_iface=implementedBy(DummyContext))
        provider = get_provider('name', context=DummyContext(), registry=self.registry)
        from contentlet.interfaces import IContentProvider
        self.assertTrue(IContentProvider.providedBy(provider))
        self.assertEqual(provider(None, None), 'content')
        self.assertRaises(LookupError, get_provider, 'name', registry=self.registry)
...
class TestQueryProvider(unittest.TestCase):
    ...
    def test_query_provider_for_context(self):
        from zope.interface import implementedBy
        self._registerProvider(DummyContentProvider('content'), 'name', context_iface=implementedBy(DummyContext))
        provider = query_provider('name', context=DummyContext(), registry=self.registry)
        from contentlet.interfaces import IContentProvider
        self.assertTrue(IContentProvider.providedBy(provider))
        self.assertEqual(provider(None, None), 'content')
        provider = query_provider('name', registry=self.registry)
