...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/AutoPlanner/autoplanner/views.py
# Compiled at: 2018-01-03 02:25:35
# Size of source mod 2**32: 12089 bytes
...
def organization(request, organization_pk):
    ...
    template_values.update({'obj':obj, 
...
def schedule_task(request, organization_pk):
    ...
    model_admin = admin.site._registry[Organization]
    if not isinstance(model_admin, OrganizationAdmin):
        raise AssertionError
    else:
        opts = model_admin.model._meta
    ...
    new_url = reverse(('admin:%s_%s_change' % (opts.app_label, opts.model_name)), args=(
     quote(obj.pk),),
...
def apply_schedule_run(request, schedule_run_pk):
    ...
    get_object_or_404((Organization.query(request)), pk=organization_id)
    model_admin = admin.site._registry[Organization]
    if not isinstance(model_admin, OrganizationAdmin):
        raise AssertionError
    else:
        ...
        if not result_dict:
            ...
            try:
                ...
                ScheduleRun.objects.filter(organization__id=organization_id).exclude(pk=schedule_run_pk).update(is_selected=False)
...
def index(request):
    ...
    template_values = {'organizations':query, 
     'form':form}
