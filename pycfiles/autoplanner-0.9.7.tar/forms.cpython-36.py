...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/AutoPlanner/autoplanner/forms.py
# Compiled at: 2017-07-27 12:22:36
# Size of source mod 2**32: 9427 bytes
...
class MultiplyTaskForm(forms.Form):
    source_task = forms.ModelChoiceField(queryset=(Task.objects.all()), widget=(forms.HiddenInput()))
    until = forms.DateTimeField(label=(_('Duplicate this task until')), initial=default_day_start, widget=(AdminSplitDateTime()))
...
class OrganizationAccessTokenForm(forms.Form):
    access_token = forms.CharField(required=False, min_length=1, max_length=300, validators=[
...
class CategoryBalancingModeForm(forms.Form):
    balancing_mode = forms.ChoiceField(label=(_('Balancing mode')), required=False, choices=(
     (
      None, _('No balancing')),
     (
...
class CategoryBalancingToleranceTimeForm(forms.Form):
    balancing_tolerance = TimeDeltaField(label=(_('Tolerance while balancing tasks across resources')), required=False)


class CategoryBalancingToleranceNumberForm(forms.Form):
    balancing_tolerance = forms.FloatField(label=(_('Tolerance while balancing tasks across resources')), required=False)


class CategoryAutoAffinityForm(forms.Form):
    auto_affinity = forms.FloatField(label=(_('Affinity for allocating successive tasks of the same category to the same agent')))
...
class AgentCategoryPreferencesBalancingCountForm(forms.Form):
    balancing_count = forms.FloatField(label=(_('If a task of this category performed by this agent counts twice, set this number to 2.0.')), initial=1.0,
      required=False,
...
class TaskMultiplyForm(forms.Form):
    until = forms.SplitDateTimeField(label=(_('Duplicate this task until')), initial=default_day_start, widget=(AdminSplitDateTime()))
    every = forms.IntegerField(label=(_('Days between successive task starts')), min_value=1, initial=7)
