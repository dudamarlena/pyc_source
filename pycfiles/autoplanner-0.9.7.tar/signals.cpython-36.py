...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/AutoPlanner/autoplanner/signals.py
# Compiled at: 2018-01-03 13:10:47
# Size of source mod 2**32: 75916 bytes
import calendar, csv, datetime, json, re, markdown
from django import forms
from django.utils.timezone import utc
from django.utils.translation import ugettext as _
from autoplanner.forms import OrganizationDescriptionForm, OrganizationAccessTokenForm, OrganizationMaxComputeTimeForm, CategoryNameForm, CategoryBalancingModeForm, CategoryAutoAffinityForm, CategoryAddForm, AgentAddForm, AgentNameForm, AgentStartTimeForm, AgentEndTimeForm, AgentCategoryPreferencesAffinityForm, AgentCategoryPreferencesAddForm, AgentCategoryPreferencesBalancingOffsetForm, AgentCategoryPreferencesBalancingCountForm, MaxTaskAffectationAddForm, MaxTaskAffectationModeForm, MaxTaskAffectationCategoryForm, MaxTaskAffectationTaskMaximumCountForm, MaxTaskAffectationRangeTimeSliceForm, AgentCategoryPreferencesBalancingOffsetTimeForm, MaxTimeAffectationTaskMaximumTimeForm, MaxTimeAffectationAddForm, CategoryBalancingToleranceTimeForm, CategoryBalancingToleranceNumberForm, AgentStartDateForm, AgentEndDateForm, TaskNameForm, TaskStartTimeForm, TaskEndTimeForm, TaskStartDateForm, TaskEndDateForm, TaskAgentForm, TaskCategoriesForm, TaskAddForm, TaskMultiplyForm, TaskMultipleUpdateForm, TaskImportForm, TaskMultipleRemoveForm
from autoplanner.models import Organization, default_token, Category, Agent, AgentCategoryPreferences, MaxTaskAffectation, MaxTimeTaskAffectation, Task, ScheduleRun, API_KEY_VARIABLE
...
from djangofloor.signals.html import render_to_client, add_attribute, remove_class, add_class, remove, before, focus, content, replace_with
...
def change_tab(window_info, organization_pk: int, tab_name: str):
    ...
    fn = {'general':change_tab_general,  'categories':change_tab_categories,  'agents':change_tab_agents, 
     'balancing':change_tab_balancing,  'tasks':change_tab_tasks,  'schedules':change_tab_schedules, 
     'schedule':change_tab_schedule}.get(tab_name)
    if fn:
        fn(window_info, obj)
...
def change_tab_categories(window_info, organization):
    ...
    context = {'organization':organization,  'categories':queryset,  'balancing_modes':balancing_modes, 
...
def change_tab_balancing(window_info, organization):
    ...
    context = {'organization':organization,  'max_task_affectations':task_queryset,  'max_time_affectations':time_queryset, 
     'categories':categories_queryset,  'new_max_task_affectation':MaxTaskAffectation(), 
...
def change_tab_tasks(window_info, organization, order_by: Choice(Task.orders)='start_time', agent_id: int=None, category_id: int=None, pattern: str=''):
    ...
    context = {'organization':organization,  'tasks_categories':tasks_categories,  'categories':categories,  'agents':agents, 
     'agent_id':agent_id,  'order_by':order_by,  'category_id':category_id,  'new_task':Task(), 
     'empty_set':set(),  'pattern':pattern}
...
def set_category_balancing_mode(window_info, organization_pk: int, value: SerializedForm(CategoryBalancingModeForm), category_pk: int=None):
    can_update = Organization.query(window_info).filter(pk=organization_pk).count() > 0
    if can_update and value and value.is_valid():
        balancing_mode = value.cleaned_data['balancing_mode']
        if category_pk:
            Category.objects.filter(organization__id=organization_pk, pk=category_pk).update(balancing_mode=(balancing_mode or None))
        add_attribute(window_info, '#check_category_%s' % category_pk, 'class', 'fa fa-check')
        if balancing_mode == Category.BALANCE_TIME:
            ...
            if balancing_mode == Category.BALANCE_NUMBER:
...
def set_category_balancing_tolerance_time(window_info, organization_pk: int, category_pk: int, value: SerializedForm(CategoryBalancingToleranceTimeForm)):
    can_update = Organization.query(window_info).filter(pk=organization_pk).count() > 0
    ...
    if can_update:
        if value:
            if value.is_valid():
                ...
                Category.objects.filter(organization__id=organization_pk, pk=category_pk).update(balancing_tolerance=(balancing_tolerance.total_seconds()))
                add_attribute(window_info, '#check_category_%s' % category_pk, 'class', 'fa fa-check')
    if value:
        add_attribute(window_info, '#check_category_%s' % category_pk, 'class', 'fa fa-remove')
...
def set_category_balancing_tolerance_number(window_info, organization_pk: int, category_pk: int, value: SerializedForm(CategoryBalancingToleranceNumberForm)):
    can_update = Organization.query(window_info).filter(pk=organization_pk).count() > 0
    if can_update:
        if value:
            if value.is_valid():
                balancing_tolerance = value.cleaned_data['balancing_tolerance']
                Category.objects.filter(organization__id=organization_pk, pk=category_pk).update(balancing_tolerance=balancing_tolerance)
                add_attribute(window_info, '#check_category_%s' % category_pk, 'class', 'fa fa-check')
    if value:
        add_attribute(window_info, '#check_category_%s' % category_pk, 'class', 'fa fa-remove')
...
def set_category_auto_affinity(window_info, organization_pk: int, category_pk: int, value: SerializedForm(CategoryAutoAffinityForm)):
    can_update = Organization.query(window_info).filter(pk=organization_pk).count() > 0
    if can_update:
        if value:
            if value.is_valid():
                auto_affinity = value.cleaned_data['auto_affinity']
                Category.objects.filter(organization__id=organization_pk, pk=category_pk).update(auto_affinity=auto_affinity)
                add_attribute(window_info, '#check_category_%s' % category_pk, 'class', 'fa fa-check')
    if value:
        add_attribute(window_info, '#check_category_%s' % category_pk, 'class', 'fa fa-remove')
...
def add_category(window_info, organization_pk: int, value: SerializedForm(CategoryAddForm)):
    ...
    if can_update:
        if value:
            if value.is_valid():
                balancing_modes = Category._meta.get_field('balancing_mode').choices
                category = Category(organization_id=organization_pk, name=(value.cleaned_data['name']), balancing_mode=(value.cleaned_data['balancing_mode']),
                  balancing_tolerance=(value.cleaned_data['balancing_tolerance']),
                  auto_affinity=(value.cleaned_data['auto_affinity']))
                category.save()
            ...
            if value:
                if value.is_valid():
                    ...
                    if start_time is None:
                        ...
                        agent.start_time = (agent.start_time or datetime.datetime.now(tz=utc)).replace(hour=(start_time.hour),
            ...
            if value:
                if value.is_valid():
                    ...
                    if end_time is None:
                        ...
                        agent.end_time = (agent.end_time or datetime.datetime.now(tz=utc)).replace(hour=(end_time.hour),
            ...
            if value:
                if value.is_valid():
                    ...
                    if start_time is None:
                        ...
                        agent.start_time = (agent.start_time or datetime.datetime(1970, 1, 1, hour=0, minute=0, second=0, tzinfo=utc)).replace(year=(start_time.year),
            ...
            if value:
                if value.is_valid():
                    ...
                    if end_time is None:
                        ...
                        agent.end_time = (agent.end_time or datetime.datetime(1970, 1, 1, hour=0, minute=0, second=0, tzinfo=utc)).replace(year=(end_time.year),
...
def show_agent_infos(window_info, organization_pk: int, agent_pk: int):
    ...
    if organization:
        if agent:
            query = list(AgentCategoryPreferences.objects.filter(organization__id=organization_pk, agent__id=agent_pk).select_related('category'))
            ...
            context = {'agent_category_preferences':query, 
             'agent':agent,  'organization':organization,  'new_agent_category_preference':agent_category_preferences, 
...
def set_agent_category_preferences_affinity(window_info, organization_pk: int, agent_category_preferences_pk: int, value: SerializedForm(AgentCategoryPreferencesAffinityForm)):
    can_update = Organization.query(window_info).filter(pk=organization_pk).count() > 0
    if can_update:
        if value:
            if value.is_valid():
                affinity = value.cleaned_data['affinity']
                AgentCategoryPreferences.objects.filter(organization__id=organization_pk,
                  pk=agent_category_preferences_pk).update(affinity=affinity)
                add_attribute(window_info, '#check_agent_category_preferences_%s' % agent_category_preferences_pk, 'class', 'fa fa-check')
    if value:
        add_attribute(window_info, '#check_agent_category_preferences_%s' % agent_category_preferences_pk, 'class', 'fa fa-remove')


@signal(is_allowed_to=is_authenticated, path='autoplanner.forms.set_agent_category_preferences_balancing_offset', queue='fast')
def set_agent_category_preferences_balancing_offset(window_info, organization_pk: int, agent_category_preferences_pk: int, value: SerializedForm(AgentCategoryPreferencesBalancingOffsetForm)):
    can_update = Organization.query(window_info).filter(pk=organization_pk).count() > 0
    if can_update:
        if value:
            if value.is_valid():
                ...
                AgentCategoryPreferences.objects.filter(organization__id=organization_pk,
                  pk=agent_category_preferences_pk).update(balancing_offset=balancing_offset)
                add_attribute(window_info, '#check_agent_category_preferences_%s' % agent_category_preferences_pk, 'class', 'fa fa-check')
    if value:
        add_attribute(window_info, '#check_agent_category_preferences_%s' % agent_category_preferences_pk, 'class', 'fa fa-remove')


@signal(is_allowed_to=is_authenticated, path='autoplanner.forms.set_agent_category_preferences_balancing_offset_time', queue='fast')
def set_agent_category_preferences_balancing_offset_time(window_info, organization_pk: int, agent_category_preferences_pk: int, value: SerializedForm(AgentCategoryPreferencesBalancingOffsetTimeForm)):
    can_update = Organization.query(window_info).filter(pk=organization_pk).count() > 0
    if can_update:
        if value:
            if value.is_valid():
...
@signal(is_allowed_to=is_authenticated, path='autoplanner.forms.set_agent_category_preferences_balancing_count', queue='fast')
def set_agent_category_preferences_balancing_count(window_info, organization_pk: int, agent_category_preferences_pk: int, value: SerializedForm(AgentCategoryPreferencesBalancingCountForm)):
    can_update = Organization.query(window_info).filter(pk=organization_pk).count() > 0
    if can_update:
        if value:
            if value.is_valid():
                ...
                AgentCategoryPreferences.objects.filter(organization__id=organization_pk,
                  pk=agent_category_preferences_pk).update(balancing_count=balancing_count)
                add_attribute(window_info, '#check_agent_category_preferences_%s' % agent_category_preferences_pk, 'class', 'fa fa-check')
    if value:
        add_attribute(window_info, '#check_agent_category_preferences_%s' % agent_category_preferences_pk, 'class', 'fa fa-remove')
...
def add_agent_category_preferences(window_info, organization_pk: int, agent_pk: int, value: SerializedForm(AgentCategoryPreferencesAddForm)):
    organization = Organization.query(window_info).filter(pk=organization_pk).first()
    agent = Agent.objects.filter(organization__id=organization_pk, id=agent_pk).first()
    AgentCategoryPreferences.objects.filter(organization__id=organization_pk, agent__id=agent_pk).values_list('category_id')
    if organization and agent and value and value.is_valid():
        ...
        not_used = AgentCategoryPreferences.objects.filter(organization__id=organization_pk, agent__id=agent_pk, category__id=new_category_id).count() == 0
        if not_used:
            if category:
                agent_category_preferences = AgentCategoryPreferences(organization_id=organization_pk, agent_id=agent_pk, category=category)
                agent_category_preferences.save()
                context = {'organization':organization,  'obj':agent_category_preferences,  'agent':agent}
                content_str = render_to_string('autoplanner/include/agent_category_preference.html', context=context, window_info=window_info)
                before(window_info, '#row_agent_category_preferences_None', content_str)
        if Category.objects.filter(organization__id=organization_pk).count() == AgentCategoryPreferences.objects.filter(organization__id=organization_pk, agent__id=agent_pk).count():
...
def remove_agent_category_preferences(window_info, organization_pk: int, agent_category_preferences_pk: int, agent_pk: int):
    can_update = Organization.query(window_info).filter(pk=organization_pk).count() > 0
    if can_update:
        AgentCategoryPreferences.objects.filter(organization__id=organization_pk,
          id=agent_category_preferences_pk).delete()
...
def set_max_task_affectation_mode(window_info, organization_pk: int, max_task_affectation_pk: int, value: SerializedForm(MaxTaskAffectationModeForm)):
    can_update = Organization.query(window_info).filter(pk=organization_pk).count() > 0
    if can_update:
        if value:
            if value.is_valid():
                mode = value.cleaned_data['mode']
                MaxTaskAffectation.objects.filter(organization__id=organization_pk,
                  pk=max_task_affectation_pk).update(mode=mode)
                add_attribute(window_info, '#check_max_task_affectation_%s' % max_task_affectation_pk, 'class', 'fa fa-check')
    if value:
        add_attribute(window_info, '#check_max_task_affectation_%s' % max_task_affectation_pk, 'class', 'fa fa-remove')


@signal(is_allowed_to=is_authenticated, path='autoplanner.forms.set_max_task_affectation_category', queue='fast')
def set_max_task_affectation_mode(window_info, organization_pk: int, max_task_affectation_pk: int, value: SerializedForm(MaxTaskAffectationCategoryForm)):
    can_update = Organization.query(window_info).filter(pk=organization_pk).count() > 0
    if can_update:
        if value:
            if value.is_valid():
                ...
                MaxTaskAffectation.objects.filter(organization__id=organization_pk,
                  pk=max_task_affectation_pk).update(category=category)
                add_attribute(window_info, '#check_max_task_affectation_%s' % max_task_affectation_pk, 'class', 'fa fa-check')
    if value:
        add_attribute(window_info, '#check_max_task_affectation_%s' % max_task_affectation_pk, 'class', 'fa fa-remove')


@signal(is_allowed_to=is_authenticated, path='autoplanner.forms.set_max_task_affectation_range_time_slice', queue='fast')
def set_max_task_affectation_range_time_slice(window_info, organization_pk: int, max_task_affectation_pk: int, value: SerializedForm(MaxTaskAffectationRangeTimeSliceForm)):
    can_update = Organization.query(window_info).filter(pk=organization_pk).count() > 0
    if can_update:
        if value:
            if value.is_valid():
                ...
                MaxTaskAffectation.objects.filter(organization__id=organization_pk,
                  pk=max_task_affectation_pk).update(range_time_slice_days=days,
                  range_time_slice_hours=hours,
...
@signal(is_allowed_to=is_authenticated, path='autoplanner.forms.set_max_task_affectation_task_maximum_count', queue='fast')
def set_max_task_affectation_task_maximum_count(window_info, organization_pk: int, max_task_affectation_pk: int, value: SerializedForm(MaxTaskAffectationTaskMaximumCountForm)):
    can_update = Organization.query(window_info).filter(pk=organization_pk).count() > 0
    if can_update:
        if value:
            if value.is_valid():
                ...
                MaxTaskAffectation.objects.filter(organization__id=organization_pk,
                  pk=max_task_affectation_pk).update(task_maximum_count=task_maximum_count)
                add_attribute(window_info, '#check_max_task_affectation_%s' % max_task_affectation_pk, 'class', 'fa fa-check')
    if value:
        add_attribute(window_info, '#check_max_task_affectation_%s' % max_task_affectation_pk, 'class', 'fa fa-remove')
...
def add_max_task_affectation(window_info, organization_pk: int, value: SerializedForm(MaxTaskAffectationAddForm)):
    ...
    if organization and value and value.is_valid():
        ...
        days, hours, seconds = python_to_components(range_time_slice)
        if category:
            max_task_affectation = MaxTaskAffectation(organization_id=organization_pk, mode=mode, category=category,
              task_maximum_count=task_maximum_count,
              range_time_slice_days=days,
              range_time_slice_hours=hours,
              range_time_slice_seconds=seconds)
            max_task_affectation.save()
            context = {'organization':organization,  'obj':max_task_affectation,  'categories':Category.objects.filter(organization__id=organization_pk).order_by('name')}
            content_str = render_to_string('autoplanner/include/max_task_affectation.html', context=context, window_info=window_info)
...
def remove_max_task_affectation(window_info, organization_pk: int, max_task_affectation_pk: int):
    ...
    if can_update:
        MaxTaskAffectation.objects.filter(organization__id=organization_pk,
          id=max_task_affectation_pk).delete()
...
def set_max_time_affectation_mode(window_info, organization_pk: int, max_time_affectation_pk: int, value: SerializedForm(MaxTaskAffectationModeForm)):
    can_update = Organization.query(window_info).filter(pk=organization_pk).count() > 0
    if can_update:
        if value:
            if value.is_valid():
                mode = value.cleaned_data['mode']
                MaxTimeTaskAffectation.objects.filter(organization__id=organization_pk,
                  pk=max_time_affectation_pk).update(mode=mode)
                add_attribute(window_info, '#check_max_time_affectation_%s' % max_time_affectation_pk, 'class', 'fa fa-check')
    if value:
        add_attribute(window_info, '#check_max_time_affectation_%s' % max_time_affectation_pk, 'class', 'fa fa-remove')


@signal(is_allowed_to=is_authenticated, path='autoplanner.forms.set_max_time_affectation_category', queue='fast')
def set_max_time_affectation_mode(window_info, organization_pk: int, max_time_affectation_pk: int, value: SerializedForm(MaxTaskAffectationCategoryForm)):
    can_update = Organization.query(window_info).filter(pk=organization_pk).count() > 0
    if can_update:
        if value:
            if value.is_valid():
                category = value.cleaned_data['category']
                MaxTimeTaskAffectation.objects.filter(organization__id=organization_pk,
                  pk=max_time_affectation_pk).update(category=category)
                add_attribute(window_info, '#check_max_time_affectation_%s' % max_time_affectation_pk, 'class', 'fa fa-check')
    if value:
        add_attribute(window_info, '#check_max_time_affectation_%s' % max_time_affectation_pk, 'class', 'fa fa-remove')


@signal(is_allowed_to=is_authenticated, path='autoplanner.forms.set_max_time_affectation_range_time_slice', queue='fast')
def set_max_time_affectation_range_time_slice(window_info, organization_pk: int, max_time_affectation_pk: int, value: SerializedForm(MaxTaskAffectationRangeTimeSliceForm)):
    can_update = Organization.query(window_info).filter(pk=organization_pk).count() > 0
    if can_update:
        if value:
            if value.is_valid():
                ...
                days, hours, seconds = python_to_components(range_time_slice)
                MaxTimeTaskAffectation.objects.filter(organization__id=organization_pk,
                  pk=max_time_affectation_pk).update(range_time_slice_days=days,
                  range_time_slice_hours=hours,
                  range_time_slice_seconds=seconds)
                add_attribute(window_info, '#check_max_time_affectation_%s' % max_time_affectation_pk, 'class', 'fa fa-check')
    if value:
...
@signal(is_allowed_to=is_authenticated, path='autoplanner.forms.set_max_time_affectation_task_maximum_time', queue='fast')
def set_max_time_affectation_task_maximum_time(window_info, organization_pk: int, max_time_affectation_pk: int, value: SerializedForm(MaxTimeAffectationTaskMaximumTimeForm)):
    can_update = Organization.query(window_info).filter(pk=organization_pk).count() > 0
    if can_update:
        if value:
            if value.is_valid():
                ...
                MaxTimeTaskAffectation.objects.filter(organization__id=organization_pk,
                  pk=max_time_affectation_pk).update(task_maximum_time_days=days,
                  task_maximum_time_hours=hours,
                  task_maximum_time_seconds=seconds)
                add_attribute(window_info, '#check_max_time_affectation_%s' % max_time_affectation_pk, 'class', 'fa fa-check')
    if value:
        add_attribute(window_info, '#check_max_time_affectation_%s' % max_time_affectation_pk, 'class', 'fa fa-remove')
...
def add_max_time_affectation(window_info, organization_pk: int, value: SerializedForm(MaxTimeAffectationAddForm)):
    ...
    if organization and value and value.is_valid():
        ...
        if category:
            max_time_affectation = MaxTimeTaskAffectation(organization_id=organization_pk, mode=mode, category=category,
              range_time_slice_days=days,
              range_time_slice_hours=hours,
              range_time_slice_seconds=seconds,
              task_maximum_time_days=days_,
              task_maximum_time_hours=hours_,
              task_maximum_time_seconds=seconds_)
            max_time_affectation.save()
            context = {'organization':organization,  'obj':max_time_affectation,  'categories':Category.objects.filter(organization__id=organization_pk).order_by('name')}
            content_str = render_to_string('autoplanner/include/max_time_affectation.html', context=context, window_info=window_info)
...
def remove_max_time_affectation(window_info, organization_pk: int, max_time_affectation_pk: int):
    ...
    if can_update:
        MaxTimeTaskAffectation.objects.filter(organization__id=organization_pk,
          id=max_time_affectation_pk).delete()
    ...
    if can_update:
        if task:
            if value:
                if value.is_valid():
                    start_time = value.cleaned_data['start_time_1']
                    task.start_time = task.start_time.replace(hour=(start_time.hour), minute=(start_time.minute), second=(start_time.second))
                    task.save()
        ...
        if task:
            if value:
                if value.is_valid():
                    ...
                    if agent or task.agent is None:
...
def add_task(window_info, organization_pk: int, value: SerializedForm(TaskAddForm)):
    ...
    if organization and value and value.is_valid():
        ...
        task = Task(organization=organization, name=(value.cleaned_data['name']), start_time=(value.cleaned_data['start_time']),
        ...
        categories = list(Category.objects.filter(organization=organization).order_by('name'))
        agents = list(Agent.objects.filter(organization=organization).order_by('name'))
        context = {'organization':organization,  'obj':task,  'obj_categories':{x.id for x in obj_categories},  'agents':agents, 
         'categories':categories}
        content_str = render_to_string('autoplanner/include/task.html', context=context, window_info=window_info)
        check_task(window_info, task)
        before(window_info, '#row_task_None', content_str)
...
def filter_tasks(window_info, organization_pk: int, order_by: Choice(Task.orders)='start_time', agent_id: int_or_none=None, category_id: int_or_none=None, pattern: str=''):
    organization = Organization.query(window_info).filter(pk=organization_pk).first()
    if organization:
        change_tab_tasks(window_info, organization, order_by=order_by, agent_id=agent_id, category_id=category_id, pattern=pattern)
...
def task_multiply(window_info, organization_pk: int, task_pk: int, form: SerializedForm(TaskMultiplyForm)=None):
    ...
    if form and form.is_valid():
        ...
        limit = datetime.datetime(year=(until_src.year), month=(until_src.month), day=(until_src.day), hour=23,
          minute=59,
          second=59,
        ...
        while start_time < limit:
            new_task = Task(organization_id=(obj.organization_id), name=('%s (%d)' % (new_name, name_index)), start_time=start_time,
              end_time=end_time,
              task_serie=task_serie)
        ...
        if tasks_to_create:
            ...
            for new_task in tasks_to_create:
                new_task.save()
                all_categories_to_create += [cls(task_id=(new_task.pk), category_id=category_pk) for category_pk in current_category_pks]
        ...
        categories = list(Category.objects.filter(organization=organization).order_by('name'))
        agents = list(Agent.objects.filter(organization=organization).order_by('name'))
        context = {'organization':organization,  'obj':None,  'obj_categories':current_category_pks,  'agents':agents, 
         'categories':categories}
        for new_task in tasks_to_create:
...
def task_multiple_update_show(window_info, organization_pk: int):
    ...
    content_str = render_to_string('autoplanner/include/task_multiple_update.html', context=context, window_info=window_info)
    modal_show(window_info, content_str)


@signal(is_allowed_to=is_authenticated, path='autoplanner.forms.task_multiple_update', queue='fast')
def task_multiple_update(window_info, organization_pk: int, form: SerializedForm(TaskMultipleUpdateForm)=None, order_by: Choice(Task.orders)='start_time', agent_id: int_or_none=None, category_id: int_or_none=None, pattern: str=''):
    organization = Organization.query(window_info).filter(pk=organization_pk).first()
    if not organization:
        return
    if form and form.is_valid():
        ...
        if agent:
            ...
            if form.cleaned_data['fix']:
                kwargs['fixed'] = True
            else:
                if form.cleaned_data['unfix']:
                    kwargs['fixed'] = False
        if kwargs:
...
def task_multiple_remove(window_info, organization_pk: int, form: SerializedForm(TaskMultipleRemoveForm)=None, order_by: Choice(Task.orders)='start_time', agent_id: int_or_none=None, category_id: int_or_none=None, pattern: str=''):
    order_by, agent_id, category_id, pattern = (
     order_by, agent_id, category_id, pattern)
    organization = Organization.query(window_info).filter(pk=organization_pk).first()
    if not organization:
        return
...
def task_import(window_info, organization_pk: int, form: SerializedForm(TaskImportForm)=None, order_by: Choice(Task.orders)='start_time', agent_id: int_or_none=None, category_id: int_or_none=None, pattern: str=''):
    order_by, agent_id, category_id, pattern = (
     order_by, agent_id, category_id, pattern)
    organization = Organization.query(window_info).filter(pk=organization_pk).first()
    if not organization:
                ...
                if len(row) != 7:
                    ...
                    if not start is None:
                        if end is None:
                            ...
                            task = Task(name=(row[0].strip()), fixed=(bool(agent_id)), agent_id=agent_id, start_time=start, end_time=end, organization_id=organization_pk)
                            task.save()
                            if end < start:
                                msg = _('Finish time is before start time for %(t)s') % {'t': task}
                                notify(window_info, msg, level=WARNING, style=NOTIFICATION)
    ...
    context = {'organization':organization, 
     'example_c':example_c,  'example_1':example_1,  'example_2':example_2,  'example_a':example_a, 
     'example_b':example_b}
...
def prepare_schedule_stats(organization):
    ...
    statistics = {x.pk:{category.pk:[0, datetime.timedelta(0), None] for category in scheduler.categories} for x in scheduler.agents}
    for category in scheduler.categories:
        if category.balancing_mode == Category.BALANCE_NUMBER:
            for agent in scheduler.agents:
                offset, count, __ = scheduler.preferences_by_agent_by_category[category.pk].get(agent.pk, (0.0,
                                                                                                           1.0,
                                                                                                           0.0))
                if count is None:
            ...
            if category.balancing_mode == Category.BALANCE_TIME:
                for agent in scheduler.agents:
                    offset, count, __ = scheduler.preferences_by_agent_by_category[category.pk].get(agent.pk, (0.0,
                                                                                                               1.0,
                                                                                                               0.0))
    ...
    for task in scheduler.tasks:
        ...
        for category_pk in scheduler.categories_by_task[task.pk]:
            ...
            if scheduler.categories_by_pk[category_pk].balancing_mode is None:
                continue
            __, count, __ = scheduler.preferences_by_agent_by_category[category_pk].get(task.agent_id, (0.0,
                                                                                                        1.0,
                                                                                                        0.0))
    ...
    context = {'obj':organization,  'statistics':sorted_statistics,  'agents':agents,  'categories':categories, 
     'description':markdown.markdown(organization.description),  'api_key_variable':API_KEY_VARIABLE, 
...
def calendar_month(window_info, organization_pk: int, year: int=None, month: int=None):
    ...
    query = Task.objects.filter(organization=organization).exclude(end_time__lt=start).exclude(start_time__gt=end).select_related('agent').prefetch_related('categories')
    ...
    if next_month == 13:
        next_year += 1
        next_month = 1
    month_str = {1:_('January'), 
     2:_('February'),  3:_('March'),  4:_('April'),  5:_('May'), 
     6:_('June'),  7:_('July'),  8:_('August'),  9:_('September'), 
     10:_('October'),  11:_('November'),  12:_('December')}[month]
    context = {'matrix':task_matrix,  'month':month,  'year':year,  'organization':organization,  'previous_month':previous_month, 
     'previous_year':previous_year,  'next_year':next_year, 
     'next_month':next_month,  'month_str':month_str}
...
def calendar_week(window_info, organization_pk: int, year: int=None, month: int=None, day: int=None):
    ...
    try:
        ...
        month_str = {1:_('January'),  2:_('February'),  3:_('March'),  4:_('April'),  5:_('May'), 
         6:_('June'),  7:_('July'),  8:_('August'),  9:_('September'), 
         10:_('October'),  11:_('November'),  12:_('December')}[start.month]
        ...
        context = {'matrix':task_matrix,  'month':start.month,  'year':start.year,  'organization':organization,  'start':start, 
         'end':end,  'previous_week':previous_start,  'next_week':end,  'month_str':month_str,  'col_days':col_days}
...
def schedule_apply(window_info, organization_pk: int, schedule_pk: int):
    ...
    ScheduleRun.objects.filter(organization__id=organization_pk).exclude(pk=schedule_pk).update(is_selected=False)
    ...
    if old_schedule_run:
        ...
        content_str = render_to_string('autoplanner/include/schedule.html', context={'obj':old_schedule_run, 
         'organization':organization})
...
def schedule_info(window_info, organization_pk: int):
    ...
    if not organization:
        return
    context = prepare_schedule_stats(organization)
    content_str = render_to_string('autoplanner/include/schedule_info.html', context=context,
