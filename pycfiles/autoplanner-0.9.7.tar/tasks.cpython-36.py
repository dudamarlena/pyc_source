...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/AutoPlanner/autoplanner/tasks.py
# Compiled at: 2017-07-28 01:42:06
# Size of source mod 2**32: 9403 bytes
import celery, json, os, signal, subprocess, celery
...
def compute_schedule(self, organization_id, window_info_data=None):
    ...
    msg = _('Computation started at %(d)s, %(t)s') % {'d':date_format(start, use_l10n=True),  't':time_format(start, use_l10n=True)}
    count = Organization.objects.filter(pk=organization_id, celery_task_id=None).update(celery_task_id=celery_id,
      celery_start=start,
      message=msg)
    if count == 0:
        return
    ...
    if window_info:
        ...
        render_to_client(window_info, 'autoplanner/include/schedule_status.html', {'organization': organization}, '#schedule_status',
          to=[organization])
    ...
    level = SUCCESS
    try:
        result_list = scheduler.solve(verbose=False, max_compute_time=(organization.max_compute_time), schedule_run=schedule_run)
        ...
        if selected:
            ...
            msg = _('Computation finished at %(d)s, %(t)s') % {'d':date_format(end, use_l10n=True),  't':time_format(end, use_l10n=True)}
            ...
            msg = _('Unable to find a solution, maybe you should remove some constraints or relax the balancing values. Computation finished at %(d)s, %(t)s') % {'d':date_format(end, use_l10n=True), 
        ...
        ScheduleRun.objects.filter(pk=(schedule_run.pk)).update(celery_end=end, process_id=None, status=(bool(result_dict)), result_dict=serialized_result_dict,
          is_selected=selected,
          message=schedule_msg)
    except subprocess.TimeoutExpired:
        end = timezone.localtime(timezone.now())
        msg = _('%(d)s, %(t)s: Unable to find a schedule in the allowed time.') % {'d':date_format(end, use_l10n=True), 
         't':time_format(end, use_l10n=True)}
        schedule_msg = _('Max computation time reached.')
        ScheduleRun.objects.filter(pk=(schedule_run.pk)).update(celery_end=end, process_id=None, status=False, message=schedule_msg)
        level = DANGER
    except Exception as e:
        end = timezone.localtime(timezone.now())
        msg = _('%(d)s, %(t)s: Unable to compute a schedule %(msg)s') % {'msg':e, 
         'd':date_format(end, use_l10n=True),  't':time_format(end, use_l10n=True)}
        schedule_msg = _('Unexpected error happened: %(e)s') % {'e': e}
        ScheduleRun.objects.filter(pk=(schedule_run.pk)).update(celery_end=end, process_id=None, status=False, message=schedule_msg)
        level = DANGER

    Organization.objects.filter(pk=organization_id, celery_task_id=celery_id).update(celery_task_id=None,
      message=(str(msg)),
      celery_end=end)
    if window_info:
...
def apply_schedule(organization_id, result_dict):
    ...
    for agent_pk, task_pks in result_dict.items():
        ...
        if agent_pk not in available_agent_ids:
            ...
            if not task_pks.issubset(available_tasks_ids):
...
def kill_schedule(self, celery_task_id, window_info_data=None):
    ...
    ScheduleRun.objects.filter(celery_task_id=celery_task_id, process_id__in=all_process_ids).update(process_id=None, celery_end=end)
    msg = _('Computation killed at %(d)s, %(t)s') % {'d':date_format(end, use_l10n=True),  't':time_format(end, use_l10n=True)}
