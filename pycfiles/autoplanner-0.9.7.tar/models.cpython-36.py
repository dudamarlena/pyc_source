...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/AutoPlanner/autoplanner/models.py
# Compiled at: 2018-01-03 13:08:01
# Size of source mod 2**32: 13294 bytes
...
def default_day_start():
    ...
    return datetime.datetime(year=(x.year), month=(x.month), day=(x.day), hour=0, minute=0, second=0, tzinfo=(get_default_timezone()))
...
def default_token(length=64, allowed_chars='abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'):
...
class Organization(models.Model):
    ...
    celery_task_id = models.CharField((_('Celery task id')), db_index=True, max_length=60, blank=True, null=True, default=None)
    ...
    access_token = models.CharField((_('Access token')), default=default_token, max_length=300, validators=[
     RegexValidator('^[a-zA-Z0-9]{1,300}$')])
    max_compute_time = models.PositiveIntegerField((_('Maximum time, in seconds, for finding a solution')), default=None,
      blank=True,
      null=True,
    ...
    current_schedule = models.ForeignKey('ScheduleRun', default=None, blank=True, null=True, db_index=True, related_name='current_organizations',
...
class Agent(OrganizationObject):
    name = models.CharField((_('Name')), db_index=True, max_length=500)
    start_time = models.DateTimeField((_('Arrival time')), db_index=True, default=None, blank=True, null=True, help_text=(_('Before this date, the agent cannot performany task.')))
    end_time = models.DateTimeField((_('Leaving time')), db_index=True, default=None, blank=True, null=True, help_text=(_('After this date, the agent cannot perform any task.')))
...
class Category(OrganizationObject):
    ...
    name = models.CharField((_('Name')), db_index=True, max_length=500)
    balancing_mode = models.CharField((_('Balancing mode')), max_length=10,
     ...
     (
      ...
      null=True,
      default=None)
    balancing_tolerance = models.FloatField((_('Tolerance for balancing the total duration (in seconds)|tasks across agents')), default=0.0,
      blank=True,
      null=True)
    auto_affinity = models.FloatField((_('Affinity for allocating successive tasks of the same category to the same agent')), default=0.0,
      blank=True)
...
class MaxAffectation(OrganizationObject):
    ...
    def range_time_slice(self):
        return datetime.timedelta(days=(self.range_time_slice_days), hours=(self.range_time_slice_hours), seconds=(self.range_time_slice_seconds))
...
class MaxTimeTaskAffectation(MaxAffectation):
    ...
    def task_maximum_time(self):
        return datetime.timedelta(days=(self.task_maximum_time_days), hours=(self.task_maximum_time_hours), seconds=(self.task_maximum_time_seconds))
...
class Task(OrganizationObject):
    ...
    task_serie = models.ForeignKey('self', db_index=True, null=True, default=None, on_delete=(models.SET_NULL), blank=True)
...
class AgentCategoryPreferences(OrganizationObject):
    ...
    affinity = models.FloatField((_('Affinity of the agent for the category.')), default=0.0, blank=True)
    balancing_offset = models.FloatField((_('Number of time units already done')), default=0, blank=True)
    balancing_count = models.FloatField((_('If a task of this category performed by this agent counts twice, set this number to 2.0.')), default=1.0,
      blank=True,
      null=True,
...
class AgentTaskExclusion(OrganizationObject):
    agent = models.ForeignKey(Agent, db_index=True, on_delete=(models.CASCADE))
    task = models.ForeignKey(Task, db_index=True, verbose_name=(_('Task')), on_delete=(models.CASCADE), help_text=(_('Select the task that cannot be performed by the agent.')))
