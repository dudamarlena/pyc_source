...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/AutoPlanner/autoplanner/schedule.py
# Compiled at: 2017-09-11 13:28:48
# Size of source mod 2**32: 18623 bytes
import re, subprocess, tempfile, itertools
...
class Scheduler(object):
    ...
    def __init__(self, organization: Organization):
        ...
        self.tasks = {x for x in organization.task_set.all() if x.start_time < x.end_time if x.start_time < x.end_time}
        self.agent_category_preferences = {x for x in organization.agentcategorypreferences_set.all()}
        self.task_durations = {task.pk:(task.end_time - task.start_time).total_seconds() for task in self.tasks}
        ...
        self.available_agents_by_tasks = {task.pk:self.agent_pks - self.agent_exclusions_by_task[task.pk] for task in self.tasks}

    def get_categories_by_task(self):
        result = {x.pk:set() for x in self.tasks}
        for related in Task.categories.through.objects.filter(category__id__in=(self.categories_by_pk), task__id__in=result):
        ...
        for task in self.tasks:
            ...
            for agent in self.agents:
                if agent.start_time is not None and agent.start_time > task.start_time:
                    ...
                    if agent.end_time is not None and agent.end_time < task.end_time:
                        agent_task_exclusions[task_pk].add(agent.pk)
    ...
    def get_preferences_by_agent_by_category(self):
        ...
        for a in self.agent_category_preferences:
            preferences[a.category_id][a.agent_id] = (
             a.balancing_offset, a.balancing_count, a.affinity)
                ...
                for max_affectation in self.max_task_affectations_by_category[category_pk]:
                    ...
                    if isinstance(max_affectation, MaxTimeTaskAffectation):
                        for agent_pk in agent_pks:
                            variables = ['%d * %s' % (self.task_durations[task_pk], self.variable(agent_pk, task_pk)) for task_pk in current_tasks]
    ...
    def apply_balancing_constraints(self):
        for category in self.categories:
            if not category.balancing_mode is None:
                if category.balancing_tolerance is None:
                    pass
                else:
                    ...
                    for agent_pk in cat_agent_pks:
                        agent_preferences = self.preferences_by_agent_by_category[category_pk].get(agent_pk, (0,
                                                                                                              1.0,
                                                                                                              0.0))
                        if category.balancing_mode == category.BALANCE_NUMBER:
                            ag_sum = ['%g * %s' % (agent_preferences[1], self.variable(agent_pk, task_pk)) for task_pk in cat_task_pks]
                        else:
                            ag_sum = ['%g * %d * %s' % (agent_preferences[1], self.task_durations[task_pk], self.variable(agent_pk, task_pk)) for task_pk in cat_task_pks]
                    ...
                    for agent_pk_1, agent_pk_2 in itertools.product(cat_agent_pks, cat_agent_pks):
                        if agent_pk_1 >= agent_pk_2:
                            ...
                            yield Constraint('%s <= %s + %g' % (self.category_variable(category_pk, agent_pk_1),
                             self.category_variable(category_pk, agent_pk_2), category.balancing_tolerance))
                            yield Constraint('%s <= %s + %g' % (self.category_variable(category_pk, agent_pk_2),
    ...
    def compute_balancing(self, result_list):
        ...
        def is_affected_to(agent_pk_, task_pk_):
            if agent_pk_ in result_dict:
                if task_pk_ in result_dict[agent_pk_]:
                    return 1
            return 0
                ...
                if category.balancing_tolerance is None:
                    ...
                    for agent_pk in cat_agent_pks:
                        ...
                        if category.balancing_mode == category.BALANCE_NUMBER:
                            ...
                            ag_sum = sum(agent_preferences[1] * self.task_durations[task_pk] * is_affected_to(agent_pk, task_pk) for task_pk in cat_task_pks)
                    ...
                    if cat_min is not None:
                        balances[category_pk] = (
                         category.name, category.balancing_mode, cat_max - cat_min)
        ...
        for category in self.categories:
            ...
            for agent_pk in self.agent_pks - self.agent_exclusions_by_category[category_pk]:
                agent_preferences = self.preferences_by_agent_by_category[category_pk].get(agent_pk, (0,
                                                                                                      1.0,
                                                                                                      0.0))
                variables += ['%g * %s' % (agent_preferences[2], self.variable(agent_pk, x[0])) for x in cat_task_pks if agent_preferences[2]]
    ...
    def solve(self, verbose=False, max_compute_time=None, schedule_run=None):
        ...
        :rtype:
        """
        if max_compute_time is not None:
            if max_compute_time <= 0:
