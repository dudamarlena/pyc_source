...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/AutoPlanner/autoplanner/admin.py
# Compiled at: 2017-09-12 01:55:51
# Size of source mod 2**32: 7326 bytes
...
from autoplanner.models import Task, AgentCategoryPreferences, AgentTaskExclusion, MaxTaskAffectation, Category, Agent, Organization, MaxTimeTaskAffectation, ScheduleRun
...
class ScheduleRunInline(admin.TabularInline):
    ...
    def apply(obj):
        if obj and obj.pk and obj.status:
            if obj.is_selected:
                return format_html('<span class="button default" style="background-color: #0B6138;">{}</span>', _('Current schedule'))
            return format_html('<a href="{}" class="button default">{}</a>', reverse('apply_schedule_run', kwargs={'schedule_run_pk': obj.pk}), _('Select this schedule'))
        else:
            return ''
...
class OrganizationAdmin(admin.ModelAdmin):
    exclude = [
     'message', 'celery_task_id', 'celery_start', 'celery_end']
    ...
    def schedule_button(self, obj):
        if obj and obj.pk:
            if not obj.celery_task_id:
                return format_html('<a href="{}" class="button default">{}</a>', reverse('schedule_start', kwargs={'organization_pk': obj.pk}), _('Compute a new schedule!'))
            return format_html('<a href="{}" class="button default">{}</a>', reverse('cancel_schedule', kwargs={'organization_pk': obj.pk}), _('Cancel the currently running schedule'))
        else:
            return ''
...
class AgentAdmin(admin.ModelAdmin):
    inlines = [
     AgentCategoryPreferencesInline, AgentTaskExclusionInline]
...
class AgentCategoryPreferencesAdmin(admin.ModelAdmin):
    list_display = [
     'category', 'agent', 'affinity', 'balancing_offset', 'balancing_count']
