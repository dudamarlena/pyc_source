...
# [GCC 8.4.0]
# Embedded file name: /Users/ramusus/workspace/manufacture/env/src/django-vkontakte-api/vkontakte_api/models.py
# Compiled at: 2016-03-11 04:22:35
import sys
...
class VkontakteManager(models.Manager):
    ...
    def get_by_slug(self, slug):
        ...
        try:
            ...
            remote_id = int(re.findall('^%s(\\d+)$' % self.model.slug_prefix, slug)[0])
...
class VkontakteModel(models.Model):
    ...
    methods_access_tag = ''
    methods_namespace = ''
    fetched = models.DateTimeField('Обновлено', null=True, blank=True, db_index=True)
    ...
    def refresh(self):
        ...
        if len(objects) == 1:
            ...
            raise VkontakteContentError('Remote server returned more objects, than expected - %d instead of one. Object details: %s, request details: %s' % (
             len(objects), self.__dict__, self.refresh_kwargs))
...
class VkontakteIDModel(RemoteIdModelMixin, VkontakteModel):
    remote_id = models.BigIntegerField('ID', help_text='Уникальный идентификатор', unique=True)
