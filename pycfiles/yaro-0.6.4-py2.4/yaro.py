...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/yaro.py
# Compiled at: 2008-06-27 22:05:12
...
"""
import urllib, cgi, mimetypes, Cookie
...
class Request(object):
    ...
    __module__ = __name__
    _volitile_attrs = [
     '__class__', '__delattr__', '__dict__', '__doc__', '__getattr__', '__getattribute__', '__hash__', '__init__', '__module__', '__new__', '__reduce__', '__reduce_ex__', '__repr__', '__setattr__', '__str__', '__weakref__', '_load_body', '_load_cookie', '_parse_form', '_parse_query', '_start_response', 'content_length', 'content_type', 'environ', 'exc_info', 'extra_props', 'forward', 'load_prop', 'method', 'query', 'redirect', 'start_response', 'uri', 'wsgi_forward']
    ...
    def save_to_environ(self):
        ...
        for attr in dir(self):
            if attr not in self._volitile_attrs + [ p[0] for p in self.extra_props or [] ]:
    ...
    def __getattr__(self, attr):
        ...
        if attr == 'form' and 'form' not in self.__dict__:
            self._parse_form()
        elif attr == 'body' and 'body' not in self.__dict__:
            self._load_body()
        elif attr == 'cookie' and 'cookie' not in self.__dict__:
            self._load_cookie()
    ...
    def _parse_form(self):
        ...
        if clen:
            ...
            fs = cgi.FieldStorage(fp=sio, environ=self.environ, keep_blank_values=True)
            sio.seek(0)
