...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/test_stog.py
# Compiled at: 2018-12-13 08:22:07
import numpy as np, pandas as pd, os, sys
from tests.utils import get_data_path, load_data, get_index_of_function
...
class TestStogBase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.kwargs_for_files = {'Files': [
                   {'Filename': get_data_path(filename), 'ReciprocalFunction': 'S(Q)', 
                      'Qmin': 0.02, 
                      'Qmax': 15.0, 
                      'Y': {'Offset': 0.0, 'Scale': 1.0}, 
                      'X': {'Offset': 0.0}},
                   {'Filename': get_data_path(self.material.reciprocal_space_filename), 'ReciprocalFunction': 'S(Q)', 
                      'Qmin': 1.9, 
                      'Qmax': 35.2, 
                      'Y': {'Offset': 0.0, 'Scale': 1.0}, 
                      'X': {'Offset': 0.0}}]}
        self.kwargs_for_stog_input = {'NumberDensity': self.material.kwargs['rho'], 
           '<b_coh>^2': self.material.kwargs['<b_coh>^2'], 
           '<b_tot^2>': self.material.kwargs['<b_tot^2>'], 
           'FourierFilter': {'Cutoff': self.fourier_filter_cutoff}, 'OmittedXrangeCorrection': False, 
...
class TestStogInit(TestStogBase):
    ...
    def test_stog_init(self):
        ...
        self.assertEqual(stog.plotting_kwargs, {'figsize': (16, 8), 'style': '-', 
           'ms': 1, 
           'lw': 1})
        self.assertEqual(stog.merged_opts, {'Y': {'Offset': 0.0, 
                 'Scale': 1.0}})
...
class TestStogAttributes(TestStogBase):
    ...
    def test_stog_plotting_kwargs_setter(self):
        stog = StoG()
        new_kwargs = {'figsize': (4, 4), 'style': 'o', 
           'ms': 2, 
           'lw': 2}
...
class TestStogDataFrameAttributes(TestStogBase):
    ...
    def setUp(self):
        ...
        self.df_target = pd.DataFrame(np.random.randn(10, 2), index=np.arange(10), columns=list('XY'))
...
class TestStogDatasetSpecificMethods(TestStogBase):
    ...
    def test_stog_add_dataset(self):
        places = 5
        stog = StoG(**{'<b_coh>^2': self.kwargs['<b_coh>^2'], '<b_tot^2>': self.kwargs['<b_tot^2>']})
        index = 0
        info = {'data': pd.DataFrame({'x': self.q, 'y': self.sq}), 'ReciprocalFunction': 'S(Q)'}
        stog.add_dataset(info, index=index)
        self.assertEqual(stog.df_individuals.iloc[self.first].name, self.reciprocal_xtarget)
        self.assertAlmostEqual(stog.df_individuals.iloc[self.first][('S(Q)_%d' % index)], self.sq_target[0], places=places)
        self.assertAlmostEqual(stog.df_sq_individuals.iloc[self.first][('S(Q)_%d' % index)], self.sq_target[0], places=places)
        index = 1
        info = {'data': pd.DataFrame({'x': self.q, 'y': self.fq}), 'ReciprocalFunction': 'Q[S(Q)-1]'}
        stog.add_dataset(info, index=index)
        self.assertAlmostEqual(stog.df_individuals.iloc[self.first][('Q[S(Q)-1]_%d' % index)], self.fq_target[0], places=places)
        self.assertAlmostEqual(stog.df_sq_individuals.iloc[self.first][('S(Q)_%d' % index)], self.sq_target[0], places=places)
        index = 2
        info = {'data': pd.DataFrame({'x': self.q, 'y': self.fq_keen}), 'ReciprocalFunction': 'FK(Q)'}
        stog.add_dataset(info, index=index)
        self.assertAlmostEqual(stog.df_individuals.iloc[self.first][('FK(Q)_%d' % index)], self.fq_keen_target[0], places=places)
        self.assertAlmostEqual(stog.df_sq_individuals.iloc[self.first][('S(Q)_%d' % index)], self.sq_target[0], places=places)
        index = 3
        info = {'data': pd.DataFrame({'x': self.q, 'y': self.dcs}), 'ReciprocalFunction': 'DCS(Q)'}
        stog.add_dataset(info, index=index)
        self.assertAlmostEqual(stog.df_individuals.iloc[self.first][('DCS(Q)_%d' % index)], self.dcs_target[0], places=places)
        self.assertAlmostEqual(stog.df_sq_individuals.iloc[self.first][('S(Q)_%d' % index)], self.sq_target[0], places=places)
    ...
    def test_stog_add_dataset_yscale(self):
        ...
        info = {'data': pd.DataFrame({'x': self.q, 'y': self.sq}), 'ReciprocalFunction': 'S(Q)', 
           'Y': {'Scale': 2.0}}
        stog.add_dataset(info, index=index)
        self.assertNotEqual(stog.df_individuals.iloc[self.first][('S(Q)_%d' % index)], self.sq_target[0])
        self.assertNotEqual(stog.df_sq_individuals.iloc[self.first][('S(Q)_%d' % index)], self.sq_target[0])
    ...
    def test_stog_add_dataset_wrong_reciprocal_space_function_exception(self):
        stog = StoG()
        index = 0
        info = {'data': pd.DataFrame({'x': self.q, 'y': self.sq}), 'ReciprocalFunction': 'ABCDEFG(Q)'}
        with self.assertRaises(ValueError):
            stog.add_dataset(info, index=index)
    ...
    def test_stog_read_dataset(self):
        places = 5
        stog = StoG(**{'<b_coh>^2': self.kwargs['<b_coh>^2'], '<b_tot^2>': self.kwargs['<b_tot^2>']})
        info = {'Filename': get_data_path(self.material.reciprocal_space_filename), 
        ...
        self.assertEqual(stog.df_individuals.iloc[self.first].name, self.reciprocal_xtarget)
        self.assertAlmostEqual(stog.df_individuals.iloc[self.first][('S(Q)_%d' % info['index'])], self.sq_target[0], places=places)
        self.assertAlmostEqual(stog.df_sq_individuals.iloc[self.first][('S(Q)_%d' % info['index'])], self.sq_target[0], places=places)
    ...
    def test_stog_merge_data_qsq_opts_scale(self):
        ...
        stog.merge_data()
        self.assertAlmostEqual(stog.df_sq_master.iloc[self.first][stog.qsq_minus_one_title], qsq_opts['Y']['Scale'] * self.fq_target[0], places=places)
    ...
    def test_stog_merge_data_qsq_opts_offset(self):
        ...
        stog.merged_opts['Q[S(Q)-1]'] = qsq_opts
        stog.merge_data()
        self.assertAlmostEqual(stog.df_sq_master.iloc[self.first][stog.qsq_minus_one_title], qsq_opts['Y']['Offset'] + self.fq_target[0], places=places)
...
class TestStogTransformSpecificMethods(TestStogDatasetSpecificMethods):
    ...
    def test_stog_transform_merged_default(self):
        ...
        self.assertAlmostEqual(stog.df_gr_master.iloc[self.real_space_first].name, self.real_xtarget, places=places)
        self.assertAlmostEqual(stog.df_gr_master.iloc[self.real_space_first][stog.gr_title], self.gofr_target[0], places=places)
        stog.dr = None
        stog.transform_merged()
        self.assertAlmostEqual(stog.df_gr_master.iloc[self.real_space_first].name, self.real_xtarget, places=places)
        self.assertAlmostEqual(stog.df_gr_master.iloc[self.real_space_first][stog.gr_title], self.gofr_target[0], places=places)
    ...
    def test_stog_transform_merged_GKofR(self):
        ...
        stog.transform_merged()
        self.assertAlmostEqual(stog.df_gr_master.iloc[self.real_space_first].name, self.real_xtarget, places=places)
        self.assertAlmostEqual(stog.df_gr_master.iloc[self.real_space_first][stog.gr_title], self.GKofR_target[0], places=places)
    ...
    def test_stog_fourier_filter(self):
        ...
        stog.fourier_filter()
        self.assertAlmostEqual(stog.df_gr_master.iloc[self.real_space_first].name, self.real_xtarget, places=places)
        self.assertAlmostEqual(stog.df_gr_master.iloc[self.real_space_first][stog.gr_ft_title], self.gofr_ff_target[0], places=places)
    ...
    def test_stog_fourier_filter_GKofR(self):
        ...
        stog.fourier_filter()
        self.assertAlmostEqual(stog.df_gr_master.iloc[self.real_space_first].name, self.real_xtarget, places=places)
        self.assertAlmostEqual(stog.df_gr_master.iloc[self.real_space_first][stog.gr_ft_title], self.GKofR_ff_target[0], places=places)
    ...
    def test_stog_apply_lorch_default(self):
        ...
        self.assertAlmostEqual(stog.df_gr_master.iloc[self.real_space_first][stog.gr_lorch_title], self.gofr_lorch_target[0], places=places)
    ...
    def test_stog_apply_lorch_GKofR(self):
        ...
        q, sq, r, gr = stog.fourier_filter()
        stog.apply_lorch(q, sq, r)
        self.assertAlmostEqual(stog.df_gr_master.iloc[self.real_space_first][stog.gr_lorch_title], self.GKofR_lorch_target[0], places=places)
...
class TestStogPlottingDataFrameMethods(TestStogDatasetSpecificMethods):
    ...
    def test_stog_plot_df(self, mock_show):
        df = pd.DataFrame(np.random.randn(10, 2), index=np.arange(10), columns=list('XY'))
        stog = StoG()
    ...
    def data_provider(self, stog, df, title, filename):
        ...
        def write_out_decorator(write_out_func):
            ...
            def wrap_function(*args):
                ...
                data = pd.read_csv(outfile_path, sep='\\s+', usecols=[
                 0, 1], names=[
                 'x', 'y'], skiprows=2, engine='python')
                self.assertTrue(np.allclose(data['x'], x))
                self.assertTrue(np.allclose(data['y'] - y, np.zeros(len(y)), rtol=2.0, atol=2.0, equal_nan=True))
