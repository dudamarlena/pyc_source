...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pystog/stog.py
...
class StoG(object):
    ...
    **stog** Fortran program behavior. This class is meant to
    ...
    def __init__(self, **kwargs):
        ...
        self.__plotting_kwargs = {'figsize': (16, 8), 'style': '-', 
           'ms': 1, 
           'lw': 1}
    ...
    def __update_dr(self):
        r"""Uses **rdelta** and **rmax** attributes (:math:`\Delta r` and
    ...
    @property
    def rdelta(self):
        r"""The :math:`\Delta r` for the :math:`r`-space vector
    ...
    @property
    def density(self):
        r"""The number density used (atoms/:math:`\AA^{3}`) used for the
    ...
    @property
    def bcoh_sqrd(self):
        r"""The average coherent scattering length, squared:
    ...
    @property
    def lorch_flag(self):
        r"""This sets the option to perform the Lorch dampening correction
    ...
    def read_dataset(self, info, xcol=0, ycol=1, sep='\\s+', skiprows=2, **kwargs):
        ...
        :type skiprows: int
        """
        data = pd.read_csv(info['Filename'], sep=sep, usecols=[
         xcol, ycol], names=[
         'x', 'y'], skiprows=skiprows, engine='python', **kwargs)
        info['data'] = data
        self.add_dataset(info, index=info['index'], **kwargs)

    def add_dataset(self, info, index=0, yscale=1.0, yoffset=0.0, xoffset=0.0, ydecimals=16, **kwargs):
        ...
        if info['ReciprocalFunction'] not in ReciprocalSpaceChoices:
            ...
            error += 'ReciprocalFunction must be one of the folloing %s' % json.dumps(ReciprocalSpaceChoices)
    ...
    def merge_data(self):
        ...
        **Converter** class and applies any modification
        ...
        q, sq = self._apply_scales_and_offset(q, sq, self.merged_opts['Y']['Scale'], self.merged_opts['Y']['Offset'], 0.0)
    ...
    def transform_merged(self):
        ...
        q = self.df_sq_master[self.sq_title].index.values
        sq = self.df_sq_master[self.sq_title].values
        transform_kwargs = {'lorch': False, 'rho': self.density, 
        ...
        if self.real_space_function == 'g(r)':
            ...
            r, gofr = self.transformer.S_to_GK(q, sq, self.dr, **transform_kwargs)
    ...
    def fourier_filter(self):
        ...
        self.df_sq_master = self.add_to_dataframe(q_ft, sq_ft, self.df_sq_master, self._ft_title)
        self.write_out_ft()
        ...
        if self.plot_flag:
            exclude_list = [
             self.qsq_minus_one_title, self.sq_ft_title]
            self.plot_sq(ylabel='FourierFilter(Q)', title='Fourier Transform of the low-r region below cutoff', exclude_list=exclude_list)
    ...
    def apply_lorch(self, q, sq, r):
        ...
        if self.real_space_function == 'g(r)':
            ...
            r, gr_lorch = self.transformer.S_to_GK(q, sq, r, **{'lorch': True, 'rho': self.density, '<b_coh>^2': self.bcoh_sqrd})
        self.df_gr_master = self.add_to_dataframe(r, gr_lorch, self.df_gr_master, self.gr_lorch_title)
        self.write_out_lorched_gr()
        if self.plot_flag:
            self.plot_gr(title='Lorched %s' % self.real_space_function)
        return (r, gr_lorch)
    ...
    def _add_keen_fq(self, q, sq):
        ...
        self.df_sq_master = self.add_to_dataframe(q, fq, self.df_sq_master, self.fq_title)
    ...
    def _add_keen_gr(self, r, gr):
        ...
        if self.real_space_function == 'g(r)':
            ...
            GKofR = gr
        self.df_gr_master = self.add_to_dataframe(r, GKofR, self.df_gr_master, self.GKofR_title)
