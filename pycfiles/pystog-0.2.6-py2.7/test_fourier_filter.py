...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/test_fourier_filter.py
...
class TestFourierFilterBase(unittest.TestCase):
    ...
    def g_using_F(self):
        q_ft, fq_ft, q, fq, r, gr = self.ff.g_using_F(self.r, self.gofr, self.q, self.fq, self.cutoff, **self.kwargs)
        first, last = self.real_space_first, self.real_space_last
        self.assertTrue(numpy.allclose(gr[first:last], self.gofr_ff_target, rtol=self.rtol, atol=self.atol))

    def g_using_S(self):
        q_ft, sq_ft, q, sq, r, gr = self.ff.g_using_S(self.r, self.gofr, self.q, self.sq, self.cutoff, **self.kwargs)
        first, last = self.real_space_first, self.real_space_last
        self.assertTrue(numpy.allclose(gr[first:last], self.gofr_ff_target, rtol=self.rtol, atol=self.atol))

    def g_using_FK(self):
        q_ft, fq_ft, q, fq, r, gr = self.ff.g_using_FK(self.r, self.gofr, self.q, self.fq_keen, self.cutoff, **self.kwargs)
        first, last = self.real_space_first, self.real_space_last
        self.assertTrue(numpy.allclose(gr[first:last], self.gofr_ff_target, rtol=self.rtol, atol=self.atol))

    def g_using_DCS(self):
        q_ft, dcs_ft, q, dcs, r, gr = self.ff.g_using_DCS(self.r, self.gofr, self.q, self.dcs, self.cutoff, **self.kwargs)
        first, last = self.real_space_first, self.real_space_last
        self.assertTrue(numpy.allclose(gr[first:last], self.gofr_ff_target, rtol=self.rtol, atol=self.atol))
    ...
    def GK_using_F(self):
        q_ft, fq_ft, q, fq, r, gr = self.ff.GK_using_F(self.r, self.GKofR, self.q, self.fq, self.cutoff, **self.kwargs)
        first, last = self.real_space_first, self.real_space_last
        self.assertTrue(numpy.allclose(gr[first:last], self.GKofR_ff_target, rtol=self.rtol, atol=self.atol))

    def GK_using_S(self):
        q_ft, sq_ft, q, sq, r, gr = self.ff.GK_using_S(self.r, self.GKofR, self.q, self.sq, self.cutoff, **self.kwargs)
        first, last = self.real_space_first, self.real_space_last
        self.assertTrue(numpy.allclose(gr[first:last], self.GKofR_ff_target, rtol=self.rtol, atol=self.atol))

    def GK_using_FK(self):
        q_ft, fq_ft, q, fq, r, gr = self.ff.GK_using_FK(self.r, self.GKofR, self.q, self.fq_keen, self.cutoff, **self.kwargs)
        first, last = self.real_space_first, self.real_space_last
        self.assertTrue(numpy.allclose(gr[first:last], self.GKofR_ff_target, rtol=self.rtol, atol=self.atol))

    def GK_using_DCS(self):
        q_ft, dcs_ft, q, dcs, r, gr = self.ff.GK_using_DCS(self.r, self.GKofR, self.q, self.dcs, self.cutoff, **self.kwargs)
        first, last = self.real_space_first, self.real_space_last
        self.assertTrue(numpy.allclose(gr[first:last], self.GKofR_ff_target, rtol=self.rtol, atol=self.atol))
