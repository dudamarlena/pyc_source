...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/formconvert/__init__.py
# Compiled at: 2009-08-22 10:43:41
import datetime
...
def multiDictToDict(encoding='utf8'):
    ...
    def multiDictToDict_converter(conversion, state=None):
        ...
        if isinstance(conversion.value, cgi.FieldStorage):
        ...
        elif conversion.value.__class__.__name__ in ('NestedMultiDict', 'UnicodeMultiDict',
                                                     'MultiDict'):
            for k in conversion.value.keys():
            ...
            raise FormConvertError('Cannot convert a %r object' % conversion.value.__class__.__name__)
        conversion.result = result

    return multiDictToDict_converter


submit_image_to_record = toListOfRecords(toRecord(converters=dict(x=unicodeToInteger(), y=unicodeToInteger(), name=unicodeToUnicode())))
...
def handleSubmitImage(name, strip=False, raise_on_error=False):
    ...
    def handleSubmitImage_converter(conversion, state=None):
        ...
        if not found and raise_on_error:
            raise ConversionKitError('The expected image button field %r was not submitted' % name)
        conversion.result = result
            ...
            if k == name:
                for item in v:
                    ...
                    if result.has_key(new_key):
                        raise Exception('Cannot refactor duplicate fields, key %r already present' % new_key)
                    result[new_key] = item
                    counter += 1

            else:
                if result.has_key(k):
                    raise Exception('Cannot refactor duplicate fields, key %r already present' % k)
                result[k] = v
...
def sameValue(field1, field2):
    ...
    def sameValue_post_converter(conversion, state=None):
        ...
        if value[field1] != value[field2]:
            set_error(conversion.children[field2], 'The fields %s and %s have different values' % (
             field1,
             field2))
...
def removeFieldsIfOtherFieldResultIs(present, value, fields, raise_if_field_to_remove_is_not_present=True):
    ...
    def removeFieldsIfOtherFieldResultIs_post_converter(conversion, state=None):
        ...
        if present in result.keys() and result[present] == value:
            for field in our_fields:
                if field not in conversion.children.keys():
                    if raise_if_field_to_remove_is_not_present:
                        raise FormConvertError('Could not remove the key %r because it is not present' % field)
                else:
...
def exacltyOneFieldFrom(*fields):
    ...
    def exacltyOneFieldFrom_post_converter(conversion, state=None):
        ...
        if len(found) == 2:
            set_error(conversion, 'You should only specify %r or %r, not both' % (
             found[0],
             found[1]))
        elif not found:
            set_error(conversion, 'You must specify one of the fields %r' % (
             fields,))
...
def requireIfPresent(present, fields):
    ...
    """
    if not isinstance(fields, (list, tuple)):
        raise ConversionKitError('The ``fields`` argument should be a list or a tuple, not %r' % type(fields))

    def requireIfPresent_post_converter(conversion, state=None):
        if present in conversion.children.keys():
            for field in fields:
                if field not in conversion.children.keys():
                    set_error(conversion, 'The field %r, required if %r is present, could not be found' % (
                     field, present))
                    return
