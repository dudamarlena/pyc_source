...
# [GCC 8.4.0]
# Embedded file name: F:\sources\python_projects\fsbackup\tests\test_basic.py
# Compiled at: 2017-12-04 03:49:16
...
class TestFsbackup(unittest.TestCase):
    ...
    def testBasicBackup(self):
        ...
        self.assertEqual(self.db['files'].count(), 0)
        self.assertEqual(self.db['volumes'].count(), 0)
        fsbck_wrapper([
         'refreshHashes',
         '-db=%s' % self.conn_testing,
         '--loglevel=CRITICAL'])
        self.assertEqual(self.db['files'].count(), self.nFiles)
        if os.name == 'nt':
            ...
            with MountPathInDrive(path=vol_path, driveLetter=avLetter):
                ...
                fsbck_wrapper([
                 ...
                 '--volumeid=999999',
                 '--loglevel=CRITICAL'])
                self.assertTrue((checkFiletreesIdentical(fs_path, checkout_path)), msg='The checkout tree is not exactly equal to the original filesystem.')
                nDeleted = 0
                ...
                infoAux = fsbck_wrapper([
                 ...
                 '--volumeid=999999',
                 '--loglevel=CRITICAL'])
                self.assertEqual(infoAux['nDeleted'], 20)
