...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/slab/utils.py
# Compiled at: 2016-01-07 05:48:00
# Size of source mod 2**32: 3755 bytes
import ast, os, fnmatch, argparse
from .config import INITPY_FILENAME, SOURCE_SUFFIXES
__all__ = ('is_package', 'is_source', 'get_node_extension', 'get_node_name', 'get_node_qualname',
           'listcontent', 'is_excluded', 'get_module_imports', 'argtype_dir_input',
           'argtype_dir_output')
...
def get_module_imports(module_file):
    ...
    class __visitor(ast.NodeVisitor):
        ...
        def repr_alias_list(cls, node):
            return ''.join(', ' + cls.repr_alias(alias) for alias in node.names).lstrip(', ')

        def visit_Import(self, node):
            imports.append('import {alias_list}'.format(alias_list=self.repr_alias_list(node)))

        def visit_ImportFrom(self, node):
            imports.append('from {level}{module} import {alias_list}'.format(level='.' * node.level, module=node.module, alias_list=self.repr_alias_list(node)))
