...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/archipelagentvirtualmachinestorage/__init__.py
...
def make_archipel_plugin(configuration, entity, group):
    ...
    """
    return [
     {'info': storage.TNStorageManagement.plugin_info(), 'plugin': storage.TNStorageManagement(configuration, entity, group)}]
...
def version():
    ...
    return (
     __name__, pkg_resources.get_distribution('archipel-agent-virtualmachine-storage').version, [storage.TNStorageManagement.plugin_info()])