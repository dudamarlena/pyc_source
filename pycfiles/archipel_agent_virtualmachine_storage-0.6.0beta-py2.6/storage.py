...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/archipelagentvirtualmachinestorage/storage.py
# Compiled at: 2013-03-20 13:50:16
import magic, os, subprocess, xmpp
...
class TNStorageManagement(TNArchipelPlugin):
    ...
    def _is_file_a_drive(self, path):
        ...
        """
        return self._is_file_a_cow(path) or self._is_file_a_qcow(path) or self._is_file_a_qcow2(path) or self._is_file_a_raw(path) or self._is_file_a_vmdk(path) or self._is_file_an_iso(path)
    ...
    def process_iq(self, conn, iq):
        ...
        if self.entity.is_migrating and action not in ('get', 'getiso'):
        ...
        try:
            ...
            if prealloc and prealloc == 'metadata' and format == 'qcow2' and self.entity.configuration.getboolean('STORAGE', 'use_metadata_preallocation'):
                ...
                if format == 'qcow2':
                    ...
                    raise os.path.exists(os.path.join(self.golden_drives_dir, golden)) or Exception('The requested golden image %s has not been found in the golden folder. Cannot create drive')
                else:
        ...
        try:
            ...
            for disk in disks:
                if self._is_file_a_drive(os.path.join(self.entity.folder, disk)):
                    ...
                    currentAttributes = {'name': os.path.basename(os.path.splitext(disk)[0]), 
                       'path': diskPath, 
    ...
    def iq_getgolden(self, iq):
        ...
        try:
            ...
            for golden in goldens:
                if self._is_file_a_qcow(os.path.join(self.golden_drives_dir, golden)) or self._is_file_a_qcow2(os.path.join(self.golden_drives_dir, golden)):
