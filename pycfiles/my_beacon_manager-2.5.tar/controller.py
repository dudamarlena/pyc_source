...
# [GCC 8.4.0]
# Embedded file name: /Users/rajatg/pyProjects/beacons/beacons/portal/controller/controller.py
...
from config import REGISTER_BEACONS, ERROR, SUCCESS, LIST_BEACONS, USER_INFO, ESTIMOTE_CMD, NAMESPACE
...
def register_beacon(beacon, credentials):
    ...
    header = Header(credentials.access_token)
    response = requests.post(REGISTER_BEACONS, data=json.dumps(request_body), headers=header.get_header_body())
    return json.loads(response.content)
...
def attach_data_to_beacon(beacon_details, credentials):
    ...
    if status.status_code is 200:
        response = requests.post(url, data=json.dumps(request_body), headers=header.get_header_body())
    return response.content
...
def modify_beacon(beacon, credentials):
    ...
    response = requests.put(url, data=json.dumps(request_body), headers=header.get_header_body())
    return json.loads(response.content)
