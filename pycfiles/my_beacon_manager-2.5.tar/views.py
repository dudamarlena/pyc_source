...
# [GCC 8.4.0]
# Embedded file name: /Users/rajatg/pyProjects/beacons/beacons/portal/view/views.py
# Compiled at: 2015-12-29 22:53:01
"""
Web handler for beacon manager
"""
import base64, json, flask, requests
...
def oauth2callback():
    ...
    OAuth2.0 Callback
    """
    flow = client.flow_from_clientsecrets('client_secrets.json', scope=SCOPE, redirect_uri=flask.url_for('portal.oauth2callback', _external=True))
...
def edit_beacon():
    ...
    """
    return render_template('edit_beacon.jinja', beacon=request.args.get('name'), advid=request.args.get('advid'))
...
def edit_beacon_status():
    ...
    if 'credentials' not in flask.session:
        ...
        return render_template('edit_beacon_status.jinja', status=status)
