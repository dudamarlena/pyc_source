...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/dbtexmf/dblatex/grubber/latex.py
...
class Latex(Depend):
    ...
    def compile_needed(self):
        ...
        if not os.path.exists(self.logfile):
            ...
            return 1
        if not self.draft_support and os.path.getmtime(self.outfile) < os.path.getmtime(self.srcfile):
    ...
    def recompile_needed(self):
        ...
        if self.log.run_needed():
            ...
            return 1
        if changed:
            msg.debug(_('the %s file has changed but no re-run required?') % changed[0])
