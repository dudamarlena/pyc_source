...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/dbtexmf/xslt/xsltconf.py
...
class XsltEngine:
    ...
    def _run(self, xslfile, xmlfile, outfile):
        rc = self.command.run(kw={'stylesheet': xslfile, 'xmlfile': xmlfile, 
           'output': outfile})
        if rc != 0:
            signal_error(self, '')
    ...
    def run(self, xslfile, xmlfile, outfile, opts=None, params=None):
        if not self.subcommand_with_params:
            ...
            self.subcommand_with_params.arguments = self.param_args_format(args_pattern, params)
            self._run(xslfile, xmlfile, outfile)
    ...
    def param_args_format(self, arguments, params):
        ...
        for param, value in params.items():
            param_args.append(self.param_format % {'param_name': param, 'param_value': value})
...
class XsltConfigRunner:
    ...
    def run(self, xslfile, xmlfile, outfile, opts=None, params=None):
        ...
        self.command.run(xslfile, xmlfile, outfile, opts=opts, params=params)
