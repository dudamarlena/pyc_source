...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/dbtexmf/dblatex/grubber/index.py
...
import os
from os.path import *
import re, string, subprocess, xml.dom.minidom
...
class Xindy:
    ...
    def __init__(self, doc, idxfile, target, transcript='', opts=None, modules=None, index_lang='', style=''):
        self.doc = doc
        ...
        self._re_hyperindex = re.compile('hyperindexformat{\\\\(.*?)}}{', re.M | re.DOTALL)
    ...
    def command(self):
        ...
        if self.index_lang:
            ...
            if self.doc.lang:
                ...
                if not lang:
                    msg.warn(_("xindy: lang '%s' not found" % self.doc.lang), pkg='index')
                else:
    ...
    def _sanitize_idxfile(self):
        ...
        data = f.read()
        f.close()
        data, nsub = self._re_hyperindex.subn('\\1}{', data)
        if not nsub:
            return
    ...
    def _fix_invalid_ranges(self):
        if not self.invalid_index_ranges:
            return
        f = open(self.idxfile)
        lines = f.readlines()
        f.close()
    ...
    def _detect_invalid_ranges(self, data):
        ...
        for block in blocks:
            if 'WARNING' in block:
                ...
                m = re.search('Found.*?-range .*Location-reference is \\d+ in keyword \\((.*)\\)', block, re.M | re.DOTALL)
                if m:
                    self.invalid_index_ranges.append(Indexentry(m.group(1)))
                check_next_block = False
    ...
    def run(self):
        ...
        if cmd[0] == 'texindy' and self.doc.encoding == 'utf8':
            if not self._index_is_unicode():
                ...
                if rc == 0:
                    os.rename(tmpindex, self.target)
        return rc
...
class Makeindex:
    ...
    def command(self):
        cmd = [
         'makeindex', '-o', self.target] + self.opts
        if self.transcript:
            cmd.extend(['-t', self.transcript])
        if self.style:
    ...
    def do_order(self, *args):
        for opt in args:
            if opt == 'standard':
                ...
                msg.warn(_("unknown option '%s' for 'makeidx.order'") % opt)
    ...
    def post_compile(self):
        ...
        if not self.tool_obj:
            if self.tool == 'makeindex':
                ...
                index_cls = Xindy
            self.tool_obj = index_cls(self.doc, self.source, self.target, transcript=self.transcript, opts=self.opts, modules=self.modules, index_lang=self.lang, style=self.style)
