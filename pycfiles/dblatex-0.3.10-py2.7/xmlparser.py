...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/dbtexmf/core/xmlparser.py
...
class FilePath(BaseOption):
    ...
    def fromnode(self, xmlnode):
        ...
        if not os.path.isabs(filepath):
            filepath = os.path.normpath(os.path.join(self.config.basedir, filepath))
...
class ModuleConfig(BaseOption):
    ...
    def fromnode(self, xmlnode):
        ...
        elif xmlcmds is not None:
            args = CommandConfig(self.config)
            args.fromnode(xmlcmds)
            self.commands.append(args)
        elif xmlopts is not None:
...
class LatexConfig(XmlConfigGroup):
    node_parsers = {'texinputs': (
       ...
       'indexstyle': (
                    '--indexstyle', FilePath), 
       'backend': (
                 '--backend', ModuleConfig), 
       'texpost': (
                 '--texpost', ModuleConfig)}


class XsltConfig(XmlConfigGroup):
    node_parsers = {'stylesheet': (
                    '--xsl-user', FilePath), 
       'engine': (
                '--xslt', XsltEngineConfig)}


class ImageConfig(XmlConfigGroup):
    node_parsers = {'figpath': (
                 '--fig-path', FilePath), 
       'figformat': (
                   '--fig-format', BaseOption), 
       'converter': (
...
class XmlConfig:
    ...
    """
    node_parsers = {'latex': LatexConfig, 
       'xslt': XsltConfig, 
    ...
    def _check_root(self, root):
        ...
        if tag != self.root_tag:
            raise ValueError("Expect the XML config root element being '%s'" % self.root_tag)
