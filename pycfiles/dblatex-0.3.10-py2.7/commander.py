...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/dbtexmf/core/commander.py
    ...
    def run(self, kw=None):
        if not self.commands:
            ...
            for cmd in self.commands:
                ...
                if kw:
                    args = [ a % kw for a in cmd.arguments ]
                else:
                    args = cmd.arguments
                self.info((' ').join(args))
                if cmd.shell:
                    p = Popen(self.shcmd(args), stdin=stdin, stdout=stdout, shell=True)
                else:
