...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/dbtexmf/dblatex/xetex/fontspec.py
...
"""
import re, xml.dom.minidom, logging
...
class UnicodeInterval:
    ...
    """
    _re_codepoint = re.compile('^[Uu]\\+?([0-9A-Fa-f]+)$')
    ...
    def from_interval(self, codepoint1, codepoint2):
        ...
        if self._min_boundary > self._max_boundary:
            self._min_boundary, self._max_boundary = self._max_boundary, self._min_boundary
        return self
    ...
    def contains(self, char):
        ...
        instance's interval.
        """
        return ord(char) >= self._min_boundary and ord(char) <= self._max_boundary
...
class FontSpec:
    ...
    transition_types = [
     'enter', 'inter', 'exit']
    _re_interval = re.compile('^([Uu][0-9A-Fa-f]+)-([Uu][0-9A-Fa-f]+)$')
    _re_codepoint = re.compile('^([Uu][0-9A-Fa-f]+)$')
    ...
    def mainfont(self):
        return self.transitions['enter'].get('main') or self.transitions['enter'].get('sans')
    ...
    def _parse_range(self, range):
        ...
        for chunk in chunks:
            ...
            if m:
                ...
                if m:
                    intervals.append(UnicodeInterval().from_codepoint(m.group(1)))
                else:
