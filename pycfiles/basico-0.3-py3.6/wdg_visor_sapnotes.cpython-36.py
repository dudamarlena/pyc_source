...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/basico/widgets/wdg_visor_sapnotes.py
# Compiled at: 2019-03-31 12:04:48
# Size of source mod 2**32: 25833 bytes
"""
# File: wdg_visor.py
    ...
    def populate_sapnotes(self, bag=None, cid=None):
        ...
        for sid in self.bag:
            ...
            if metadata is not None:
                ...
                node = self.get_node(int(metadata['id']), icon, False, '<b>%s</b>' % sid, title, escape(metadata['componentkey']), escape(metadata['category']), escape(metadata['type']), escape(metadata['priority']), self.srvutl.fuzzy_date_from_timestamp(timestamp), '', timestamp)
                ...
                for fname in files:
                    with open(fname, 'r') as (fa):
                        ...
                        node = self.get_node(0, icon, False, '', annotation['Title'], annotation['Component'], '', annotation['Type'], '', self.srvutl.fuzzy_date_from_timestamp(annotation['Timestamp']), annotation['AID'], annotation['Timestamp'])
