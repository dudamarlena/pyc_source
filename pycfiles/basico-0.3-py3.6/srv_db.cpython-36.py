...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/basico/services/srv_db.py
# Compiled at: 2019-04-01 17:01:50
# Size of source mod 2**32: 12665 bytes
"""
# File: srv_db.py
...
class Database(Service):
    ...
    def initialize(self):
        ...
        self._Database__init_config_section()
        self.get_services()
        self.load_notes()
    ...
    def add_list(self, sapnotes, overwrite=False):
        ...
        n = 0
        for sid in sapnotes:
            if overwrite and self.add((sapnotes[sid]), overwrite, batch=True):
                n += 1
    ...
    def get_notes_by_node(self, key, value):
        ...
        for sapnote in sapnotes:
            if key.startswith('component') or key in ('category', 'priority', 'type'):
                ...
                if key in ('date-year', 'date-month', 'date-day'):
                    if len(value) == 6:
