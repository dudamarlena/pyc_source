...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/interactive_judger/conf_generator.py
...
def main():
    ...
    config_dict[ConfigKeys.UNIQUE_PROGRAM_LIST] = list(filter(lambda x: len(x) > 0, input('Any other programs to test(prefix only, use space to split): ').split(' ')))
    ...
    config_dict[ConfigKeys.DATA_DIR] = _alternative_input('Custom data directory({}/data): '.format(os.environ['HOME']), os.path.join(os.environ['HOME'], 'data'))
    config_dict[ConfigKeys.SRC_DIR] = _alternative_input('Custom source file directory({}/src)'.format(os.environ['HOME']), os.path.join(os.environ['HOME'], 'src'))
    config_dict[ConfigKeys.RESULT_DIR] = _alternative_input('Custom result output directory({}/result):'.format(os.environ['HOME']), os.path.join(os.environ['HOME'], 'result'))
    conf_path = get_conf_path()
