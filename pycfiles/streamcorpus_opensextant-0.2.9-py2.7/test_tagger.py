...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/streamcorpus_opensextant/tests/test_tagger.py
# Compiled at: 2015-11-16 15:31:38
from __future__ import absolute_import
from copy import deepcopy
import json, logging, os, pytest, sys, geojson, requests
from streamcorpus import make_stream_item, Chunk, EntityType, OffsetType
...
texts = [
 (
  ...
  'query-26.json'),
 (
  '☂ on the John Smith traveling in Liberia. It continues with a second sentence about Paris.',
  [
   [
    ('☂', None),
    ('on', None),
  ...
  [
   ...
   [
    ...
    (
     'the', EntityType.LOC),
    (
     'cities', EntityType.LOC),
    ('of', None),
    (
     'Montreal', EntityType.LOC),
    ('and', None),
    (
     'Québec,', EntityType.LOC),
    (
...
def use_live_service(request):
    if request.param:
        try:
            ...
            rest_url = config['scheme'] + '://' + config['network_address'] + '/opensextant/extract/'
            resp = requests.post(rest_url, timeout=10)
...
def main():
    ...
    for url in sys.stdin:
        ...
        for si in Chunk(path):
            if '<div' in si.body.raw or '<a href' in si.body.raw or '<span' in si.body.raw:
