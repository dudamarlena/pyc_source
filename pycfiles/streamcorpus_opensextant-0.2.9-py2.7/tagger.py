...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/streamcorpus_opensextant/tagger.py
...
import itertools, json, logging, time, geojson
from geojson import Point, Feature
import requests
from requests.auth import HTTPBasicAuth
from sortedcollection import SortedCollection
from streamcorpus import Tagging, make_stream_time, OffsetType, EntityType, MentionType
...
class OpenSextantTagger(IncrementalTransform):
    ...
    def __init__(self, config, *args, **kwargs):
        ...
        kwargs = {}
        self.rest_url = config.get('scheme', 'http') + '://' + config.get('network_address', 'localhost:8182') + config.get('service_path', '/opensextant/extract/')
    ...
    def request_json(self, si):
        logger.debug('POST %d bytes of clean_visible to %s', len(si.body.clean_visible), self.rest_url)
        ...
        while tries < retries:
            tries += 1
            try:
                response = self.session.post(self.rest_url, data=si.body.clean_visible, verify=self.verify_ssl, headers=headers, timeout=int(self.config.get('timeout', 10)))
                break
            except Exception as exc:
                if str(exc).startswith('ReadTimeout'):
                    logger.info('retrying OpenSextant connection: %d of %d', tries, retries)
                    time.sleep(2 ** tries)
                    continue
                raise
    ...
    def get_geo_selectors(self, results):
        ...
        for feature in features:
            if feature['type'] == 'PLACE':
                ...
                point = Point((lng, lat))
                properties = {'name': raw}
                feature = Feature(geometry=point, properties=properties, id=pid)
                o = Offset(type=OffsetType.CHARS, content_form='clean_visible', first=span[0], length=span[1] - span[0])
                yield Selector(selector_type='GEOJSON', raw_selector=raw.encode('utf-8'), canonical_selector=geojson.dumps(feature), offsets={OffsetType.CHARS: o})
    ...
    def process_item(self, si, context=None):
        ...
        if si.body and si.body.clean_visible:
            ...
            tagging = Tagging(tagger_id=self.tagger_id, tagger_version='2.1', generation_time=make_stream_time(time.time()), raw_tagging=response.content)
    ...
    def annotate_sentences(self, si, result):
        ...
        toks = SortedCollection(itertools.chain(*[ sent.tokens for sent in sentences ]), key=lambda tok: tok.offsets[OffsetType.CHARS].first)
        ...
        for anno in result.get('annoList', []):
            ...
            if not cv[start:end] == anno['matchText']:
                pre = 30
                post = 30
                logger.debug('alignment failure:\n\t%s\n\t%s%s%s', si.body.clean_visible.decode('utf8')[start - pre:end + post], ' ' * pre, anno['matchText'], ' ' * post)
...
entity_types = {'Action': (
            EntityType.EVENT, MentionType.NOM), 
   'Attribute.attribute.measurableCharacteristic': None, 
   'Attribute.weight': None, 
   'Geo.area': (
              EntityType.LOC, MentionType.NOM), 
   'Geo.distance': (
                  EntityType.LOC, MentionType.NOM), 
   'Geo.weather': (
                 EntityType.LOC, MentionType.NOM), 
   'Geo.featureType.AdminRegion': (
                                 EntityType.LOC, MentionType.NAME), 
   'Geo.featureType.Area': (
                          EntityType.LOC, MentionType.NAME), 
   'Geo.featureType.Hydro': (
                           EntityType.LOC, MentionType.NAME), 
   'Geo.featureType.Hypso': (
                           EntityType.LOC, MentionType.NAME), 
   'Geo.featureType.Misc': (
                          EntityType.LOC, MentionType.NAME), 
   'Geo.featureType.PopulatedPlace': (
                                    EntityType.LOC, MentionType.NAME), 
   'Geo.featureType.Street': (
                            EntityType.LOC, MentionType.NAME), 
   'Geo.featureType.Undersea': (
                              EntityType.LOC, MentionType.NAME), 
   'Geo.featureType.Vegetation': (
                                EntityType.LOC, MentionType.NAME), 
   'Geo.place.geocoordinate': (
                             EntityType.LOC, MentionType.NAME), 
   'Geo.place.namedPlace': (
                          EntityType.LOC, MentionType.NAME), 
   'Geo.featureType.SpotFeature': (
                                 EntityType.FAC, MentionType.NAME), 
   'Geo.facilityComponents': (
                            EntityType.FAC, MentionType.NAME), 
   'Idea': None, 
   'Information': None, 
   'Object': None, 
   'Organization': (
                  EntityType.ORG, MentionType.NAME), 
   'Person': (
            EntityType.PER, MentionType.NAME), 
   ...
   'Person.name.personName': (
                            EntityType.PER, MentionType.NAME), 
   'Person.name.title.corporateTitle': (
                                      EntityType.PER, MentionType.NOM), 
   'Person.name.title.governmentTitle': (
                                       EntityType.PER, MentionType.NOM), 
   'Person.name.title.hereditaryTitle': (
                                       EntityType.PER, MentionType.NOM), 
   'Person.name.title.militaryTitle': (
                                     EntityType.PER, MentionType.NOM), 
   'Person.name.title.personalTitle': (
                                     EntityType.PER, MentionType.NOM), 
   'Person.name.title.religiousTitle': (
                                      EntityType.PER, MentionType.NOM), 
   'Person.relative': None, 
   'Substance': None, 
   'Time': None}
entity_hierarchy = {'Action.event': None, 
