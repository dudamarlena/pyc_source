...
# Embedded file name: build/bdist.linux-x86_64/egg/pyage_forams/solutions/statistics.py
# Compiled at: 2015-01-26 17:19:48
...
class CsvStatistics(Statistics):
    ...
    def _get_entry(self, agents, step_count):
        ...
        if forams_count < 1:
            entry = [
             step_count, 0, self._get_algae_sum(), 0, 0, 0]
        return entry
    ...
    def _get_entry(self, x, y, z, step):
        ...
        if cell.is_empty() and cell.get_algae() == 0:
            ...
            if len(cell.forams) <= 8:
                ...
                return entries
            return [
             map(float, [x, y, z] + [len(cell.forams),
              cell.get_algae(),
              self.insolation_meter.get_insolation(cell, step)])]
