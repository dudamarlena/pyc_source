...
# Embedded file name: build/bdist.linux-x86_64/egg/pyage_forams/solutions/foram.py
# Compiled at: 2014-10-31 18:45:58
import logging
...
class Foram(Addressable):
    ...
    @Inject('genom_factory', 'reproduction_minimum', 'movement_energy', 'growth_minimum', 'energy_need', 'newborn_limit', 'reproduction_probability', 'growth_probability', 'growth_cost_factor', 'capacity_factor')
    ...
    def _can_reproduce(self):
        return self.energy > self.reproduction_minimum and self.genom.chambers_limit <= self.chambers and random() < self.reproduction_probability
    ...
    def _move(self):
        try:
            ...
            cell = max(empty_neighbours, key=lambda c: random() + c.available_food() + sum(s.available_food() for s in c.get_neighbours()))
            if cell:
    ...
    def _can_create_chamber(self):
        return self.energy > self.growth_minimum and self.genom.chambers_limit > self.chambers and random() > self.growth_probability
