...
# Embedded file name: build/bdist.linux-x86_64/egg/pyage_forams/solutions/distributed/neighbour_matcher.py
# Compiled at: 2014-10-28 18:05:21
...
class Neighbour2dMatcher(NeighbourMatcher):
    ...
    def _join(self, remote_agent, agent, side):
        try:
            ...
            self.request_dispatcher.submit_request(Match2dRequest(remote_address, agent.get_shadows(side), AGENT + '.' + agent.get_address(), opposite(side), agent.get_steps()))
        except Exception as e:
            logger.exception('could not join: %s', e.message)

    @staticmethod
    def create_shadow_cells(remote_address, shadows):
        shadow_cells = {address:ShadowCell(address, available_food, algae, empty, remote_address) for address, available_food, algae, empty, _ in shadows}
...
class Neighbour3dMatcher(NeighbourMatcher):
    ...
    def _join(self, remote_agent, agent, side):
        try:
            ...
            shadow_cells = self.create_shadow_cells(remote_address, shadows)
            agent.join(remote_address, shadow_cells, side, remote_agent.get_steps())
            self.request_dispatcher.submit_request(Match3dRequest(remote_address, agent.environment.get_shadows(side), AGENT + '.' + agent.get_address(), opposite(side), agent.get_steps()))
        except Exception as e:
            logger.exception('could not join: %s', e.message)

    @staticmethod
    def create_shadow_cells(remote_address, shadows):
        shadow_cells = [ [ ShadowCell(address, available_food, algae, empty, remote_address) for address, available_food, algae, empty, _ in row ] for row in shadows
                       ]
