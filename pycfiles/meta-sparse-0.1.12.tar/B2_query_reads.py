...
# [GCC 8.4.0]
# Embedded file name: /home/zhemin/software/SPARSE_wrapper/SPARSE/modules/B2_query_reads.py
# Compiled at: 2018-05-09 09:40:59
import sys, subprocess, os, gzip, re, shutil, numpy as np, pandas as pd, utils, time, glob
...
def parse_mapping(database, seqinfo, maps, mismatch=0.05, HGT_prior=[[0.05, 0.99, 0.1], [0.02, 0.99, 0.2], [0.01, 0.99, 0.5]], UCE_prior=[487, 2000], **args):
    ln_fact = np.log(factorial(np.arange(1707, dtype=float) / 10.0))
    ...
    def dpois(freq, n):
        ...
        x = (f2 > 0) & (f2 <= 20)
        res[x], f2[x] = np.log(f2[x]) * n[x] - ln_fact[(n[x] * 10).astype(int)] - (np.log(f2[x]) * np.ceil(f2[x]) - ln_fact[(np.ceil(f2[x]).astype(int) * 10)]), -1
        res[f2 > 0] = dnorm(f2[(f2 > 0)], n[(f2 > 0)]) - dnorm(f2[(f2 > 0)], np.ceil(f2[(f2 > 0)]))
        x = (f1 > 0) & (f1 <= 170)
        res[x], f1[x] = np.log(f1[x]) * n[x] - ln_fact[(n[x] * 10).astype(int)] - (np.log(f1[x]) * np.floor(f1[x]) - ln_fact[(np.floor(f1[x]).astype(int) * 10)]), -1
    ...
    m2[:, 5] += 0.9 * m2[:, 6]
    maps = m2
    maps[:, 6] = maps[:, 4] * np.log(mismatch) + maps[:, 5] * np.log(1 - mismatch) - np.sum(maps[:, 4:6], 1) * (mismatch * np.log(mismatch) + (1 - mismatch) * np.log(1 - mismatch))
    ...
    for fid, fragment_len in enumerate(UCE_prior):
        ...
        regions[:(np.max(cov_score).astype(int) + 1)] = np.bincount(cov_score.astype(int))
        ...
        for tid, (tax_id, s, e) in enumerate(tax_list.astype(int)):
            reg = regions[seq_info[s][2]:seq_info[(e - 1)][3]]
            mean_cov = 0.5
            if np.max(reg) > 1:
...
def bowtie2matrix(bowtie_db, MapDB, workspace, mismatch, r1, r2=None, n_thread=10, **params):
    ...
    for db_prefix in db_list:
        ...
        if os.path.isfile(db_prefix + '.4.bt2'):
            cmd_bt2 = ('{0} -k 300 --no-unal --ignore-quals --score-min L,20,1.1 --mp 4,4 --np 4 --sensitive-local -q -p {2} -I 25 -X 800 -x {3} {1}').format(params['bowtie2'], read_info, n_thread, db_prefix)
            ...
            for read in read_info.split()[1::2]:
                cmd_malt = ('{0} -m BlastN -i {1} -a {4} -t {2} -mq 200 -e 0.00001 -id 70 -mem load -d {3}.malt').format(params['malt_run'], read, n_thread, db_prefix, out_prefix + '.tmp.gz')
...
def ipopt(workspace, least_amount=[
 1e-06, 5], bootstrap=100, **params):
...
def match_group(data, match_ref, **params):
    ...
    for idx, assign in sorted(match_ref.iteritems(), key=lambda x: np.sum(x[1].T[1]), reverse=True):
        level_abundance = [
         np.sum(assign[:, 1]),
         np.sum(assign[(assign.T[0] <= 10.0, 1)]),
         np.sum(assign[(assign.T[0] <= 5.0, 1)]),
         np.sum(assign[(assign.T[0] <= 2.0, 1)]),
         np.sum(assign[(assign.T[0] <= 1.0, 1)])]
        ...
        for grp, abnd in zip(['~' + barcodes[0][1:]] + barcodes[:4], level_abundance):
            ...
            if grp not in results:
                results[grp] = [
                 abnd, [idx], [ {label:cnt * abnd for label, cnt in lvl.iteritems()} for lvl in groups[barcodes[3]] ]]
            else:
                results[grp][0] += abnd
                results[grp][1].append(idx)
                for res, lvl in zip(results[grp][2], groups[barcodes[3]]):
                    for label, cnt in lvl.iteritems():
                        res[label] = res.get(label, 0.0) + cnt * abnd
...
if __name__ == '__main__':
    query_read(dict([ [ k.strip() for k in arg.split('=', 1) ] for arg in sys.argv[1:] ]))