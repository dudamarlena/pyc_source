...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\kog_lib\__init__.py
# Compiled at: 2020-01-07 03:35:09
...
def help(ftype=''):
    ...
    if 'excel' in ftype:
        ...
        if 'textgrid' in ftype:
            SFTextGrid.help()
        else:
            if 'metadata' in ftype:
                SFMetadata.help()
            else:
                if 'wav' in ftype:
                    SFWav.help()
                else:
                    if 'fs' in ftype or 'filesystem' in ftype:
                        SFFS.help()
                    else:
                        if 'util' in ftype:
                            SFUtils.help()
