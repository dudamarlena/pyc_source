...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\kog_lib\wav\sf_wav.py
# Compiled at: 2020-01-07 05:05:42
...
class SFWav:
    ...
    def readframes(self, nframes, start_frame):
        ...
        start_pos = self._SFWav__get_pos_byframe(start_frame)
    ...
    def readframes_bytime(self, start_time, end_time=0):
        ...
        self._SFWav__validate_get_peak_params(1, start_time, end_time)
        start_frame = self._SFWav__get_frame_bytime(start_time)
        end_frame = self._SFWav__get_frame_bytime(end_time)
    ...
    def get_samples_bytime(self, start_time, end_time):
        ...
        return self._SFWav__parse_samples(raw_data)
    ...
    def get_peak(self, selectedchannel=1, start_time=0, end_time=0):
        if end_time == 0:
            end_time = self.wav_head.get_duration()
        self._SFWav__validate_get_peak_params(selectedchannel, start_time, end_time)
    ...
    def cut_wav(self, file_path, start_time, end_time=0, selectedchannel=1):
        ...
        self._SFWav__write_wav_file(file_path, selected_channel_samples, self.wav_head.get_samplewidth(), self.wav_head.get_framerate())
    ...
    def __write_wav_file(self, file_path, dataset, samplewidth, frame_rate, nchannels=1):
        ...
        try:
            unpack_filter = self._SFWav__get_unpack_filter(samplewidth)
    ...
    def __validate_dataset(self):
        ...
        framewidth = self.wav_head.get_framewidth()
        if self._SFWav__get_unpack_filter(samplewidth) == '':
...
class SFWavHeader:
    ...
    def open(self, wav_path):
        ...
        self._SFWavHeader__parse_params()
    ...
    def __parse_params(self):
        ...
        self.unpack_dict['pcmtype'], self.unpack_dict['channels'], self.unpack_dict['samplerate'], self.unpack_dict['bytespersec'], self.unpack_dict['alignment'], self.unpack_dict['bits'] = struct.unpack('<HHLLHH', self.raw_data[20:36])
        self.head_head = {}
        self.head_head['RIFF'], self.head_head['chunksize'], self.head_head['WAVE'], self.head_head['fmt '], self.head_head['fmtsize'] = struct.unpack('<4sI4s4sI', self.raw_data[:20])
...
class SFWavFileHandler:
    ...
    def read_file(self, file_path, start_pos, data_size, end_pos=0):
        ...
        size, end = self._SFWavFileHandler__parse_params(file_path, start_pos, data_size, end_pos)
        self._SFWavFileHandler__validate_params(start_pos, size, end)
        return self._SFWavFileHandler__read_by_size(file_path, start_pos, size)
    ...
    def __validate_params(self, start_pos, data_size, end_pos):
        if type(start_pos) is not int or type(data_size) is not int or type(end_pos) is not int:
