...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/IpLocate/run.py
# Compiled at: 2019-11-24 09:37:02
import sys, platform
...
def generate(target):
    ...
    ip = '     ' + result.pop('ip', '0.0.0.0')
    result.pop('readme', '')
    if not disable_color():
        ip = ('\x1b[01;32m{}\x1b[00m').format(ip)
    title = ('\n{}\n\n').format(ip)
    container = ('\n').join([ ('{}{}{}').format(k.title(), ' ' * left_space(k, 20), v) for k, v in sorted(result.items())
                            ])
