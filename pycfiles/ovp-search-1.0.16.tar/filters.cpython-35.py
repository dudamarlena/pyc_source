...
# [GCC 8.4.0]
# Embedded file name: /home/arroyo/ovp/suzano-ovp/django-ovp-search/ovp_search/filters.py
# Compiled at: 2017-05-08 18:11:32
...
class ProjectRelevanceOrderingFilter(OrderingFilter):
    ...
    def annotate_queryset(self, queryset, request):
        ...
        queryset = queryset.annotate(cause_relevance=Count(Case(When(causes__pk__in=[1, 3], then=1), output_field=IntegerField()), distinct=True), skill_relevance=Count(Case(When(skills__pk__in=[1, 2, 4], then=1), output_field=IntegerField()), distinct=True)).annotate(relevance=F('cause_relevance') + F('skill_relevance'))
        return queryset

    def filter_queryset(self, request, queryset, view):
        ordering = self.get_ordering(request, queryset, view)
        if ordering and ('relevance' in ordering or '-relevance' in ordering):
            ...
            if len(address['address_components']):
                for component in address['address_components']:
                    ...
                    for component_type in component['types']:
                        type_string = helpers.whoosh_raw('{}-{}'.format(component['long_name'], component_type).strip())
