...
# Embedded file name: /Users/dan/dev/pyCast/shakecast/sc/web_server.py
...
app = Flask(__name__, template_folder=BASE_DIR, static_folder=os.path.join(sc_dir(), 'view', 'static'))
...
def login():
    if request.method == 'GET':
        ...
        registered_user = session.query(User).filter(and_(User.username == username)).first()
...
def logged_in():
    ...
    return jsonify(success=True, loggedIn=bool(current_user.is_authenticated), isAdmin=bool(is_admin))
...
def get_eq_data():
    ...
    if filter_:
        ...
        if filter_.get('facility', None):
            query = query.filter(ShakeMap.facility_shaking.any(Facility_Shaking.facility_id == filter_['facility']['shakecast_id']))
    eq_time = float(request.args.get('time', 0))
    if eq_time < 1:
        eq_time = time.time()
    eqs = query.filter(Event.time < eq_time).filter(Event.event_id != 'heartbeat').order_by(Event.time.desc()).limit(50).all()
    eq_dicts = []
    for eq in eqs:
...
def get_fac_data():
    ...
    if filter_:
        ...
        if filter_.get('keywords', None) is not None:
            ...
            key_filter = [ or_(literal(key).contains(func.lower(Facility.name)), func.lower(Facility.name).contains(key), func.lower(Facility.description).contains(key)) for key in keys
                         ]
...
def get_shaking_data(facility_id, eq_id):
    session = Session()
    shaking = session.query(Facility_Shaking).filter(Facility_Shaking.shakemap.has(ShakeMap.shakemap_id == eq_id)).first()
    shaking_dict = None
    if shaking:
...
def delete_inventory():
    ...
    if len(inv_ids) > 0 and inv_type is not None:
        ui.send("{'delete_inventory: %s': {'func': f.delete_inventory_by_id,                         'args_in': {'ids': %s, 'inventory_type': '%s'},                         'db_use': True,                         'loop': False}}" % (inv_type, inv_ids, inv_type))
    return jsonify(success=True)
...
def get_users():
    if request.method == 'GET':
        ...
        if filter_:
            if filter_.get('group', None):
                users = session.query(User).filter(User.shakecast_id > request.args.get('last_id', 0)).filter(User.groups.any(Group.name.like(filter_['group']))).limit(50).all()
            else:
        ...
        if users is not None:
            ui.send("{'import_user_dicts': {'func': f.import_user_dicts,                                            'args_in': {'users': %s},                                            'db_use': True, 'loop': False}}" % str(users))
...
def get_shakemaps():
    session = Session()
    sms = session.query(ShakeMap).order_by(ShakeMap.recieve_timestamp.desc()).all()
    sm_dicts = []
    for sm in sms:
...
def get_shakemap(shakemap_id):
    session = Session()
    sms = session.query(ShakeMap).filter(ShakeMap.shakemap_id == shakemap_id).order_by(ShakeMap.shakemap_version.desc()).all()
    sm_dicts = []
    for sm in sms:
        sm_dict = sm.__dict__.copy()
        sm_dict.pop('_sa_instance_state', None)
...
def shakemap_overlay(shakemap_id):
    session = Session()
    shakemap = session.query(ShakeMap).filter(ShakeMap.shakemap_id == shakemap_id).order_by(desc(ShakeMap.shakemap_version)).limit(1).first()
    if shakemap is not None:
        img = os.path.join(app.config['EARTHQUAKES'], shakemap_id, shakemap_id + '-' + str(shakemap.shakemap_version), 'ii_overlay.png')
    else:
...
def event_image(event_id):
    session = Session()
    event = session.query(Event).filter(Event.event_id == event_id).limit(1).first()
    if event is not None:
        img = os.path.join(app.config['EARTHQUAKES'], event_id, 'image.png')
    else:
        img = app.send_static_file('sc_logo.png')
    Session.remove()
    return send_file(img, mimetype='image/gif')
...
def get_notification(event_id):
    session = Session()
    nots = session.query(Notification).filter(or_(Notification.event_id == event_id, Notification.shakemap_id == event_id)).all()
    dicts = []
    for obj in nots:
...
def software_update():
    ...
    return json.dumps({'required': update_required, 'update_info': [ info for info in update_info ], 'notify': notify})
...
def upload():
    if request.method == 'GET':
        ...
        xml_file = os.path.join(app.config['UPLOADED_XMLFILES_DEST'], file_name)
...
def get_group_specs(group_id):
    session = Session()
    group = session.query(Group).filter(Group.shakecast_id == group_id).first()
    group_specs = {'inspection': [], 'new_event': [], 'heartbeat': [], 'scenario_inspection': [], 'scenario_new_event': []}
...
def get_inventory():
    ...
    if filter_:
        if filter_.get('group', None) is not None:
            facilities = session.query(Facility).filter(Facility.shakecast_id > request.args.get('last_id', 0)).filter(Facility.lat_min > float(filter_['lat']) - float(filter_['lat_pm'])).filter(Facility.lat_max < float(filter_['lat']) + float(filter_['lat_pm'])).filter(Facility.lon_min > float(filter_['lon']) - float(filter_['lon_pm'])).filter(Facility.lon_max < float(filter_['lon']) + float(filter_['lon_pm'])).filter(Facility.groups.any(Group.name.like(filter_['group']))).limit(50).all()
        else:
            facilities = session.query(Facility).filter(Facility.shakecast_id > request.args.get('last_id', 0)).filter(Facility.lat_min > float(filter_['lat']) - float(filter_['lat_pm'])).filter(Facility.lat_max < float(filter_['lat']) + float(filter_['lat_pm'])).filter(Facility.lon_min > float(filter_['lon']) - float(filter_['lon_pm'])).filter(Facility.lon_max < float(filter_['lon']) - float(filter_['lon_pm'])).limit(50).all()
    else:
