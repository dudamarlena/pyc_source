...
# Embedded file name: /Users/dan/dev/pyCast/shakecast/sc/app/objects.py
...
import urllib2, ssl, json, os, sys, time, xml.etree.ElementTree as ET, smtplib, datetime
...
class ProductGrabber(object):
    ...
    def get_new_events(self, scenario=False):
        ...
        for eq_id in self.earthquakes.keys():
            ...
            if eq['properties']['net'] in self.ignore_nets or eq['properties']['mag'] < sc.new_eq_mag_cutoff:
                continue
            event = Event()
            ...
            if event.is_new() is False:
                ...
                old_events = [ session.query(Event).filter(Event.event_id == each_id).first() for each_id in ids
                             ]
    ...
    def get_new_shakemaps(self, scenario=False):
        ...
        for eq_id in self.earthquakes.keys():
            ...
            if scenario is False:
                ...
                eq_info = eq
            if 'shakemap' not in eq_info['properties']['products'].keys() and 'shakemap-scenario' not in eq_info['properties']['products'].keys():
                continue
            shakemap = ShakeMap()
            if scenario is False:
            ...
            if shakemap.is_new() is False:
                shakemap = session.query(ShakeMap).filter(ShakeMap.shakemap_id == shakemap.shakemap_id).filter(ShakeMap.shakemap_version == shakemap.shakemap_version).first()
            shakemap.json = eq_info['properties']['products'][sm_str][0]
            if shakemap.has_products(self.req_products) and scenario is False:
                continue
            dep_shakemaps = session.query(ShakeMap).filter(ShakeMap.shakemap_id == shakemap.shakemap_id).filter(ShakeMap.status == 'new')
            ...
            shakemap.directory_name = os.path.join(self.data_dir, shakemap.shakemap_id, shakemap.shakemap_id + '-' + str(shakemap.shakemap_version))
    ...
    def make_heartbeat(self):
        ...
        make_hb = False
        if last_hb:
            if time.time() > last_hb[(-1)].time + 86400:
                make_hb = True
        else:
            make_hb = True
        if make_hb is True:
...
class ShakeMapGrid(object):
    ...
    def __init__(self, lon_min=0, lon_max=0, lat_min=0, lat_max=0, nom_lon_spacing=0, nom_lat_spacing=0, num_lon=0, num_lat=0, event_id='', magnitude=0, depth=0, lat=0, lon=0, description='', directory_name='', xml_file=''):
    ...
    def load(self, file_=''):
        ...
        try:
            ...
            self.sorted_by = ''
            self.fields = [ child.attrib['name'] for child in root if 'grid_field' in child.tag
                          ]
            grid_str = [ child.text for child in root if 'grid_data' in child.tag
                       ][0]
    ...
    def in_grid(self, lon_min=0, lon_max=0, lat_min=0, lat_max=0):
        ...
        """
        return lon_min > self.lon_min and lon_min < self.lon_max and lat_min > self.lat_min and lat_min < self.lat_max or lon_min > self.lon_min and lon_min < self.lon_max and lat_max > self.lat_min and lat_max < self.lat_max or lon_max > self.lon_min and lon_max < self.lon_max and lat_min > self.lat_min and lat_min < self.lat_max or lon_max > self.lon_min and lon_max < self.lon_max and lat_max > self.lat_min and lat_max < self.lat_max

    def max_shaking(self, lon_min=0, lon_max=0, lat_min=0, lat_max=0, metric=None, facility=None):
        """
        ...
        if not self.grid:
            ...
            shaking = []
            while not shaking:
                shaking = [ point for point in self.grid[start:end] if point.info['LAT'] > lat_min and point.info['LAT'] < lat_max
                          ]
                lon_min -= 0.01
                lon_max += 0.01
                lat_min -= 0.01
    ...
    def __init__(self):
        ...
        if sc.use_proxy is True:
            if sc.proxy_username and sc.proxy_password:
                try:
                    socks.set_default_proxy('socks.PROXY_TYPE_SOCKS4', sc.proxy_server, sc.proxy_port, username=sc.proxy_username, password=sc.proxy_password)
                    socks.wrap_module(smtplib)
                except:
                    try:
                        socks.set_default_proxy('socks.PROXY_TYPE_SOCKS5', sc.proxy_server, sc.proxy_port, username=sc.proxy_username, password=sc.proxy_password)
                        socks.wrap_module(smtplib)
                    except:
                        try:
                            socks.set_default_proxy('socks.PROXY_TYPE_SOCKS4', sc.proxy_server, sc.proxy_port)
                            socks.wrap_module(smtplib)
                        except:
                            try:
                                socks.set_default_proxy('socks.PROXY_TYPE_SOCKS5', sc.proxy_server, sc.proxy_port)
                                socks.wrap_module(smtplib)
                            except:
...
class SC(object):
    ...
    def make_backup(self):
        conf_dir = self.get_conf_dir()
        copyfile(os.path.join(conf_dir, 'sc.json'), os.path.join(conf_dir, 'sc_back.json'))

    def revert(self):
        conf_dir = self.get_conf_dir()
        copyfile(os.path.join(conf_dir, 'sc_back.json'), os.path.join(conf_dir, 'sc.json'))
        self.load()
...
class NotificationBuilder(object):
    ...
    def build_new_event_html(events=None, notification=None, group=None, name=None, web=False, config=None):
        ...
        if not config:
            if name is None and notification is not None:
                config = temp_manager.get_configs('new_event', name=notification.group.template)
            else:
                config = temp_manager.get_configs('new_event', name=name)
        if name is None and notification is not None:
            template = temp_manager.get_template('new_event', name=notification.group.template)
        else:
            template = temp_manager.get_template('new_event', name=name)
        return template.render(events=events, group=group, notification=notification, sc=SC(), config=config, web=web)
    ...
    def build_insp_html(shakemap, name=None, web=False, config=None):
        ...
        fac_details = {'all': 0, 'gray': 0, 'green': 0, 'yellow': 0, 
           'orange': 0, 'red': 0}
        for fs in facility_shaking:
            fac_details['all'] += 1
            fac_details[fs.alert_level] += 1

        return template.render(shakemap=shakemap, facility_shaking=facility_shaking, fac_details=fac_details, sc=SC(), config=config, web=web)
...
class TemplateManager(object):
    ...
    def get_configs(not_type, name=None):
        ...
        try:
            conf_file = os.path.join(sc_dir(), 'templates', not_type, temp_name)
    ...
    def save_configs(not_type, name, config):
        if isinstance(config, dict):
            conf_file = os.path.join(sc_dir(), 'templates', not_type, name + '.json')
    ...
    def get_template(not_type, name=None):
        if name is None:
            ...
            temp_name = name + '.html'
        temp_file = os.path.join(sc_dir(), 'templates', not_type, temp_name)
        try:
    ...
    def save_template(not_type, name, template_str):
        temp_file = os.path.join(sc_dir(), 'templates', not_type, name + '.html')
    ...
    def get_template_names():
        ...
        """
        temp_folder = os.path.join(sc_dir(), 'templates', 'new_event')
...
class URLOpener(object):
    ...
    def open(url):
        ...
        try:
            ...
            if sc.use_proxy is True:
                if sc.proxy_username and sc.proxy_password:
                    proxy = urllib2.ProxyHandler({'http': ('http://{0}:{1}@{2}:{3}').format(sc.proxy_username, sc.proxy_password, sc.proxy_server, sc.proxy_port), 
                       'https': ('http://{0}:{1}@{2}:{3}').format(sc.proxy_username, sc.proxy_password, sc.proxy_server, sc.proxy_port)})
                    ...
                    proxy = urllib2.ProxyHandler({'http': ('http://{0}:{1}').format(sc.proxy_server, sc.proxy_port), 'https': ('https://{0}:{1}').format(sc.proxy_server, sc.proxy_port)})
            ...
            raise Exception(('URLOpener Error({}: {}, url: {})').format(type(e), e, url))
...
class Clock(object):
    ...
    def nighttime(self):
        ...
        return bool(hour >= sc.nighttime or hour < sc.morning)
...
class SoftwareUpdater(object):
    ...
    def check_update(self, testing=False):
        ...
        return (
         update_required, notify, update_info)
    ...
    def check_new_update(new, existing):
        ...
        return new_split[0] > existing_split[0] or new_split[0] == existing_split[0] and new_split[1] > existing_split[1] or new_split[0] == existing_split[0] and new_split[1] == existing_split[1] and new_split[2] > existing_split[2]
    ...
    def condense_files(self, update_list):
        ...
        for update in update_list:
            for file_ in update['files']:
                ...
                elif self.check_new_update(file_['version'], files[file_['path']]['version']):
                    files[file_['path']] = file_
