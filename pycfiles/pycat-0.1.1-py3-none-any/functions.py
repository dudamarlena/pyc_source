...
# Embedded file name: /Users/dan/dev/pyCast/shakecast/sc/app/functions.py
# Compiled at: 2017-04-21 05:20:51
import re, sys, itertools, xml.etree.ElementTree as ET
...
def geo_json(query_period='day'):
    ...
    log_message = pg.log
    data = {'status': status, 'message': 'Check for new earthquakes', 
       'log': log_message, 
       'error': error, 
...
def check_new():
    ...
    try:
        session = Session()
        new_events = session.query(Event).filter(Event.status == 'new').all()
        new_shakemaps = session.query(ShakeMap).filter(ShakeMap.status == 'new').all()
    except Exception as e:
        error = str(e)
    ...
    Session.remove()
    data = {'status': 'finished', 'message': 'Check for new earthquakes', 
       'log': log_message, 
       'error': error}
    return data
...
def process_events(events=None, session=None, scenario=False):
    ...
    for event in events:
        if clock.nighttime() is True and scenario is False:
            if event.magnitude < sc.night_eq_mag_cutoff:
                continue
        if scenario is True:
            in_region = session.query(Group).filter(Group.point_inside(event)).all()
            groups_affected = [ group for group in in_region if group.has_spec(not_type='scenario')
                              ]
            all_groups_affected.update(groups_affected)
        else:
            if event.event_id != 'heartbeat':
                groups_affected = session.query(Group).filter(Group.point_inside(event)).all()
                filtered_groups = [ group for group in groups_affected if group.has_spec(not_type='scenario') is False
                                  ]
                all_groups_affected.update(filtered_groups)
            else:
                all_groups = session.query(Group).all()
                groups_affected = [ group for group in all_groups if group.has_spec(not_type='heartbeat')
                                  ]
                all_groups_affected.update(groups_affected)
            ...
            for group in all_groups_affected:
                if group.has_spec(not_type='NEW_EVENT'):
                    event_spec = [ s for s in group.specs if s.notification_type == 'NEW_EVENT'
                                 ][0]
                    if event_spec.minimum_magnitude > event.magnitude:
                        continue
                    notification = Notification(group=group, event=event, notification_type='NEW_EVENT', status='created')
    ...
    if all_groups_affected:
        for group in all_groups_affected:
            nots = session.query(Notification).filter(Notification.notification_type == 'NEW_EVENT').filter(Notification.status == 'created').filter(Notification.group_name == group.name).all()
            filter_nots = filter(lambda x: x.event is not None, nots)
            new_event_notification(notifications=filter_nots, scenario=scenario)
...
def process_shakemaps(shakemaps=None, session=None, scenario=False):
    ...
    for shakemap in shakemaps:
        ...
        if scenario is True:
            in_region = session.query(Group).filter(Group.in_grid(grid)).all()
            groups_affected = [ group for group in in_region if group.has_spec(not_type='scenario')
                              ]
        else:
        ...
        for group in groups_affected:
            if group.has_spec(not_type='DAMAGE'):
                if shakemap.old_maps():
                    ...
                    if not specs:
                        continue
                notification = Notification(group=group, shakemap=shakemap, event=shakemap.event, notification_type='DAMAGE', status='created')
                session.add(notification)

        session.commit()
        notifications = session.query(Notification).filter(Notification.shakemap == shakemap).filter(Notification.notification_type == 'DAMAGE').filter(Notification.status != 'sent').all()
        affected_facilities = set(itertools.chain.from_iterable([ session.query(Facility).filter(Facility.in_grid(grid)).filter(Facility.groups.any(Group.shakecast_id == group.shakecast_id)).all() for g in groups_affected
                                                                ]))
        if affected_facilities:
            shaking_id = session.query(Facility_Shaking.shakecast_id, func.max(Facility_Shaking.shakecast_id)).first()[0]
            if shaking_id:
                shaking_id += 1
            else:
                shaking_id = 1
            ...
            for facility in affected_facilities:
                fac_shaking = make_inspection_prios(facility=facility, shakemap=shakemap, grid=grid, notifications=notifications)
                ...
                for n in fac_shaking['notifications']:
                    if n:
                        relationships[r_count] = {'notification': n.shakecast_id, 'facility_shaking': fac_shaking['_shakecast_id']}
                        r_count += 1

                f_count += 1

            relationships = filter(None, relationships)
            stmt = Facility_Shaking.__table__.insert().values(gray=bindparam('gray'), green=bindparam('green'), yellow=bindparam('yellow'), orange=bindparam('orange'), red=bindparam('red'), alert_level=bindparam('alert_level'), weight=bindparam('weight'), facility_id=bindparam('facility_id'), shakemap_id=bindparam('shakemap_id'), metric=bindparam('metric'), mmi=bindparam('MMI'), pga=bindparam('PGA'), psa03=bindparam('PSA03'), psa10=bindparam('PSA10'), psa30=bindparam('PSA30'), pgv=bindparam('PGV'), shakecast_id=bindparam('_shakecast_id'))
            rel_stmt = shaking_notification_connection.insert().values(notification=bindparam('notification'), facility_shaking=bindparam('facility_shaking'))
        ...
        if notifications:
            for n in notifications:
                inspection_notification(notification=n, grid=grid, scenario=scenario)
...
def make_inspection_prios(facility=None, shakemap=None, grid=None, notifications=None):
    """
    ...
    if shaking_point is None:
        ...
        fac_shaking = facility.make_alert_level(shaking_point=shaking_point, shakemap=shakemap, notifications=notifications)
        return fac_shaking
...
def new_event_notification(notifications=None, scenario=False):
    ...
    if len(you) > 0:
        if len(events) == 1:
            ...
            msg['Subject'] = ('{0} New Events -- Magnitudes: {1}').format(len(events), str(mags).replace("'", ''))
        if scenario is True:
...
def inspection_notification(notification=Notification(), grid=ShakeMapGrid(), scenario=False):
    """
    ...
    if notification.status != 'file failed':
        ...
        return {'status': notification.status, 'error': error}
...
def download_scenario(shakemap_id=None, scenario=False):
    if shakemap_id is not None:
        ...
        if success is True:
            ...
            success = False
    return {'status': status, 'message': {'from': 'scenario_download', 'title': 'Scenario Download Finished', 
                   'message': message, 
                   'success': success}, 
...
def delete_scenario(shakemap_id=None):
    ...
    session.commit()
    Session.remove()
    return {'status': 'finished', 'message': {'message': 'Successfully removed scenario: ' + shakemap_id, 'title': 'Scenario Deleted', 
                   'success': True}, 
...
def run_scenario(shakemap_id=None):
    ...
    if event:
        try:
            process_events(events=[event[0]], session=session, scenario=True)
    ...
    if shakemap:
        try:
            process_shakemaps(shakemaps=[shakemap[0]], session=session, scenario=True)
    ...
    return {'status': 'finished', 'message': {'from': 'scenario_run', 'title': ('Scenario: {}').format(shakemap_id), 
                   'message': message, 
...
def import_facility_xml(xml_file=''):
    ...
    for fac in facs:
        ...
        existing = session.query(Facility).filter(Facility.facility_id == facility_id).filter(Facility.component == component).filter(Facility.component_class == component_class).all()
        if existing:
            for f in existing:
                session.delete(f)
    ...
    log_message = ''
    status = 'finished'
    data = {'status': status, 'message': {'from': 'facility_import', 'title': 'Imported Facilities', 
                   'message': message, 
                   'success': True}, 
       'log': log_message}
    return data
...
def import_group_xml(xml_file=''):
    ...
    for group in groups:
        ...
        if g:
            ...
            g = Group()
            g.name = name
            if name == '' or len(split_poly) % 2 != 0 or len(split_poly) < 6:
                continue
            session.add(g)
        ...
        for child in group:
            if child.tag == 'NOTIFICATION':
                ...
                if notification_type == 'NEW_EVENT':
                    spec = session.query(Group_Specification).filter(Group_Specification.notification_type == notification_type).filter(Group_Specification.group == g).all()
                else:
                    spec = session.query(Group_Specification).filter(Group_Specification.notification_type == notification_type).filter(Group_Specification.inspection_priority == damage_level).filter(Group_Specification.group == g).all()
                if spec:
                    spec = spec[0]
                else:
    ...
    log_message = ''
    status = 'finished'
    data = {'status': status, 'message': {'title': 'Group Upload', 'message': imported_groups}, 
       'log': log_message}
    return data
...
def import_user_dicts(users=None):
    ...
    log_message = ''
    status = 'finished'
    data = {'status': status, 'message': {'from': 'import_user_dicts', 'title': 'User Upload', 
...
def add_facs_to_groups(session=None):
    ...
    for group in groups:
        group.facilities = session.query(Facility).filter(Facility.in_grid(group)).all()
        ...
        if user.group_string:
            ...
            if group_lst != ['']:
                for group_name in group_lst:
                    group = session.query(Group).filter(Group.name == group_name).all()
                    if group:
    ...
    if inventory_type is not None and ids is not None:
        if inventory_type == 'facility':
            ...
            if inventory_type == 'group':
                ...
                inv_table = User
            session = Session()
            inventory = session.query(inv_table).filter(inv_table.shakecast_id.in_(ids)).all()
