...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/whotracksme/website/templates.py
...
from whotracksme.website.plotting.colors import tracker_category_colors, site_category_colors
...
def site_to_json(data_source, blog_posts):
    ...
    for tracker in data_source.trackers.get_snapshot().itertuples():
        submit_key(name=(data_source.trackers.get_name(tracker.tracker)),
          type='tracker',
...
PATHS = {'_site':SITE_PATH, 
 'static':'/static', 
 'trackers':'/trackers', 
 'websites':'/websites', 
 'companies':'/companies', 
 'blog':'/blog'}
CATEGORY_DESC = {'advertising':'Provides advertising or advertising-related services such as data collection, behavioral analysis or retargeting.', 
...
def copy_custom_error_pages(data):
    error_pages = {'not-found':get_template(data, 'not-found.html', path_to_root=''), 
...
def generate_sitemap(data, blog_posts):
    with open('_site/sitemap.json', 'w') as (output):
        json.dump(site_to_json(data_source=data,
          blog_posts=blog_posts), output)
...
def get_template(data_source, name, render_markdown=False, path_to_root='.'):
    ...
    env.filters['prettify_label'] = lambda text: ' '.join(map(lambda x: x.capitalize(), text.split('_'))) if text not in (None, 'None',
                                                                         '') else ''
    ...
    env.filters['b_to_mb'] = lambda b: round(b / 1000000, 1)
...
def render_template(template, path_to_root='.', **context):
    ...
    return (template.render)(PATHS=paths, 
     ...
     TODAY=date.today().strftime('%d.%m.%Y'), **context)
