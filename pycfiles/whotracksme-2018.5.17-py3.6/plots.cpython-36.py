...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/whotracksme/website/plotting/plots.py
...
def doughnut_chart(values, labels, name, color_palette=False):
    ...
    trace = go.Pie(values=values,
      ...
      textinfo='label',
      textfont=dict(family=(CliqzFonts.regular),
      color=(cliqz_colors['white'])),
      marker=dict(colors=(palette(cliqz_colors['blue'], cliqz_colors['purple'], len(labels)) if color_palette else [
...
def scatter(x, y, name, color, fill=True, line_style='solid'):
    return go.Scatter(x=x,
      y=y,
      name=name,
      line=set_line_style(color=color,
      width=4,
...
def treemap():
    ...
    trace0 = go.Scatter(x=[r['x'] + r['dx'] / 2 for r in rects],
