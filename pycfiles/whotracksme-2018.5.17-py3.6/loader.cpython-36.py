...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/whotracksme/data/loader.py
# Compiled at: 2018-05-17 05:30:15
# Size of source mod 2**32: 13761 bytes
from datetime import datetime
from urllib.parse import quote_plus
import io, sqlite3, pkg_resources, pandas as pd

def asset_string(name):
    return pkg_resources.resource_string('whotracksme.data', f"assets/{name}").decode('utf-8')


def asset_stream(name):
    stream = pkg_resources.resource_stream('whotracksme.data', f"assets/{name}")
...
class DataSource:
    ...
    def __init__(self):
        ...
        self.sites_trackers = SitesTrackers(data_months=[
         max(self.data_months)],
          tracker_info=(self.app_info))
        self.trackers = Trackers(data_months=(self.data_months),
          tracker_info=(self.app_info),
          sites=(self.sites_trackers))
        self.companies = Companies(data_months=(self.data_months),
          company_info=(self.company_info),
          tracker_info=(self.app_info))
        self.sites = Sites(data_months=(self.data_months),
    ...
    def load_app_info(self, connection):
        col_names = [
         ...
         'company_id']
        cur = connection.execute('SELECT trackers.id, trackers.name,\n            categories.name, website_url, company_id\n            FROM trackers\n            LEFT JOIN categories ON categories.id = category_id')
        app_info = {}
        for row in cur:
    ...
    def load_company_info(self, connection):
        ...
        cur = connection.execute('SELECT {} FROM companies'.format(','.join(columns)))
        company_info = {}
        for row in cur:
            company_info[row[0]] = {col:row[i] for i, col in enumerate(columns)}
            company_info[row[0]]['apps'] = {}

        cur = connection.execute('SELECT company_id, id, name\n            FROM trackers\n            WHERE company_id IS NOT NULL')
...
class PandasDataLoader:
    ...
    def __init__(self, data_months, name, region='global', id_column=None):
        ...
        self.df = pd.concat([pd.read_csv((asset_stream(f"{month}/{region}/{name}.csv")), parse_dates=['month']) for month in data_months])
...
class Trackers(PandasDataLoader):
    ...
    def __init__(self, data_months, tracker_info, sites, region='global'):
        ...
        self.df['company_id'] = pd.Series([tracker_info.get(tracker, {}).get('company_id', tracker) for tracker in self.df.tracker],
          index=(self.df.index))
        self.df['category'] = pd.Series([tracker_info.get(tracker, {}).get('cat', 'unknown') for tracker in self.df.tracker],
          index=(self.df.index))
        self.sites = sites
        for row in self.get_snapshot().itertuples():
            if row.tracker in self.info:
                ...
                self.info[row.tracker] = {'id':row.tracker, 
                 'name':row.tracker, 
    ...
    def get_name(self, id):
        if id in self.info:
            return self.info.get(id).get('name')
        else:
            return id
    ...
    def get_reach(self, id):
        ...
        return {'page':[reach for reach in tr_df.reach], 
    ...
    def similar_trackers(self, id, n=4):
        ...
        st = snapshot[((snapshot.category == tracker.get('cat', 'unknown')) & (snapshot.id != id))].sort_values('reach',
          ascending=False)
        return [{'id':t.tracker,  'company_id':t.company_id} for t in st[:n].itertuples()]
...
class Sites(PandasDataLoader):
    ...
    def __init__(self, data_months, trackers, region='global'):
        ...
        self.site_category = {row.id:row.category for row in self.get_snapshot().itertuples()}
...
class Companies(PandasDataLoader):
    ...
    def __init__(self, data_months, company_info, tracker_info, region='global'):
        ...
        self.df['name'] = pd.Series([company_info.get(row.company, tracker_info.get(row.company, {})).get('name', row.company) for row in self.df.itertuples()],
          index=(self.df.index))