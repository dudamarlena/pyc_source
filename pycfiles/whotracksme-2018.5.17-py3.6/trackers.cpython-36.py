...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/whotracksme/website/plotting/trackers.py
# Compiled at: 2018-05-17 05:30:15
# Size of source mod 2**32: 5978 bytes
from statistics import mean
import plotly.graph_objs as go, squarify
...
def tracker_cfh(https, fingerprinting, cookies):
    ...
    """
    trace1 = hbar(label='Yes',
      https=https,
      fingerprinting=fingerprinting,
      cookies=cookies)
    trace2 = hbar(label='No',
    ...
    data = [
     trace1, trace2]
    layout = go.Layout(paper_bgcolor=(cliqz_colors['transparent']),
      ...
      tickfont=dict(family=(CliqzFonts.regular),
      size=13,
...
def tracker_reach_ts(ts1, ts2, t):
    ...
    layout = go.Layout(margin=set_margins(t=30),
      height=300)
...
def ts_trend(ts, t):
    ...
    trace1 = line(x=[
     ...
     y[(-1)]],
      color=(cliqz_colors['purple']),
      mode='markers')
    layout = go.Layout(dict(showlegend=False,
      margin=set_margins(l=10, t=30, r=10),
      height=100,
      width=153,
      hoverlabel=dict(bgcolor=(cliqz_colors['black']),
      bordercolor=(cliqz_colors['transparent']),
      font=dict(family=(CliqzFonts.mono),
      size=13,
      color=(cliqz_colors['bright_gray']))),
      ...
      yaxis=dict(range=[
     min(y) * 0.9, max(y) * 1.05 if max(y) != y[(-1)] else max(y) * 1.15],
      showgrid=False,
      zeroline=False,
      showline=False,
      autotick=True,
...
def tracker_map(app, site_values, rectangles):
    ...
    for r in rectangles:
        shapes.append(dict(type='rect',
          ...
          x1=(r['x'] + r['dx']),
          y1=(r['y'] + r['dy']),
          line=dict(color=(cliqz_colors['white']),
          width=0.5),
    ...
    trace0 = go.Scatter(x=[r['x'] + r['dx'] / 2 for r in rectangles],
