...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\fragmap\generate_matrix.py
# Compiled at: 2019-09-05 14:04:47
# Size of source mod 2**32: 16458 bytes
            ...
            if node_line == group[0]:
                ...
                for member in group:
                    if member._startdiff_i == node_line._startdiff_i:
                        if member._kind != node_line._kind:
    ...
    def group_by_patch_connection(fragmap):
        ...
        def patch_connection_key(column):
            ...
            if inside == [False] * fragmap.get_n_patches():
                ...
                return (
                 True, ''.join(['1' if b else '0' for b in inside]))
...
class ConnectedFragmap(object):
    ...
    def generate_matrix(self):
        ...
        def create_cell(matrix, r, c):
            ...
            change_neigh = Status9Neighborhood(up_left=status(infill=infill_up_left), up=status(connection=(change_up and change_center)),
    ...
    def render_for_console(self, colorize):
        ...
        def create_cell_description(cell):
                ...
                if status == ConnectionStatus.EMPTY:
                    ...
                    if status == ConnectionStatus.INFILL:
                        if colorize:
                            ...
                            return '^'
                    elif position in ('up_left', 'up_right', 'down_left', 'down_right'):
            ...
            return [
             [
              character('up_left', cell.changes.up_left),
        ...
        return flatten([[flatten(v) for v in lzip(*[create_cell_description(cell) for cell in row])] for row in connection_matrix])