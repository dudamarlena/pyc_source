...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\tests\infrastructure.py
...
def create_bundles(test_dir):
    test_subdirs = [subdir for subdir in subdirs(test_dir) if basename(subdir).startswith('test_')]
    print(test_subdirs)
...
def unbundle_execution_test_repos(test_dir):
    test_bundles = [filename for filename in files(test_dir) if basename(filename).startswith('test_') if filename.endswith('.bundle')]
...
def remove_execution_test_repos(test_dir):
    test_subdirs = [subdir for subdir in subdirs(os.path.join(test_dir, 'build')) if basename(subdir).startswith('test_')]
