...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\tests\test_general.py
# Compiled at: 2019-09-05 14:04:55
# Size of source mod 2**32: 20694 bytes
...
import fragmap.debug as debug, unittest
...
def MockDiffDelta(old_file_path, new_file_path):
    return Mock(old_file=(MockDiffFile(old_file_path)), new_file=(MockDiffFile(new_file_path)),
      is_binary=False)
...
def MockDiffHunk(old_start_and_lines, new_start_and_lines, mocklines):
    ...
    return Mock(lines=mocklines, old_start=old_start,
      old_lines=old_lines,
      new_start=new_start,
      new_lines=new_lines)
...
class Test(unittest.TestCase):
    ...
    def test_update_positions(self):
        filepatch = MockPatch(MockDiffDelta('dummy', 'dummy'), [
         MockDiffHunk((4, 2), (4, 4), [])])
        patch = CommitDiff(MockCommit('aaabbaaabbaaabbaaabbaaabbaaabbaaabbaaabb', 'dummy message'), [
         filepatch])
    ...
    def test_003_004(self):
        files = [
         '003-add-one-line-to-empty-file',
         '004-remove-one-line-empty-file']
    ...
    def test_003_004_groups(self):
        files = [
         '003-add-one-line-to-empty-file',
         '004-remove-one-line-empty-file']
        self.check_node_group_kinds(files, [[START, START], [END, END]])
    ...
    def get_node_lines(self, diff_filenames):
        test_name = self.id().split('.')[(-1)]
        repo_path = os.path.join(DIFF_DIR, 'build', test_name)
        commit_hexes = [find_commit_with_message(repo_path, diff_filename) for diff_filename in diff_filenames]
        cl = CommitLoader()
        ...
        for group in grouped_node_lines:
            ...
            for node_line in group:
                if node_line._kind == FragmentBoundNode.START:
                    ...
                    if node_line._kind == FragmentBoundNode.END:
                        n_end += 1
