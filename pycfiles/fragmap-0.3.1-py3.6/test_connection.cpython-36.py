...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\tests\test_connection.py
# Compiled at: 2019-09-05 14:04:48
# Size of source mod 2**32: 7363 bytes
...
assert [[Cell(Cell.CHANGE, '3'), Cell(Cell.NO_CHANGE)]] == create_node_matrix_from_description(['3 '])
...
def create_connection_matrix_from_description(desc_matrix):
    ...
    def create_connected_cell(up_row, mid_row, down_row):
        ...
        return ConnectedCell(create_base_cell(center), Status9Neighborhood(up_left=(connection_status(up_left)), up=(connection_status(up)),
    ...
    return [[create_connected_cell(desc_matrix[(3 * r)][c], desc_matrix[(3 * r + 1)][c], desc_matrix[(3 * r + 2)][c]) for c in range(n_cols)] for r in range(n_rows)]
