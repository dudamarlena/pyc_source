...
# Embedded file name: build/bdist.linux-x86_64/egg/py3plex/visualization/drawing_machinery.py
# Compiled at: 2019-02-26 09:54:34
# Size of source mod 2**32: 36735 bytes
import networkx as nx
from networkx.utils import is_string_like
from networkx.drawing.layout import shell_layout, circular_layout, kamada_kawai_layout, spectral_layout, spring_layout, random_layout
__all__ = [
 'draw',
 'draw_networkx',
 'draw_networkx_nodes',
...
def draw_networkx_nodes(G, pos, nodelist=None, node_size=300, node_color='r', node_shape='o', alpha=1.0, cmap=None, vmin=None, vmax=None, ax=None, linewidths=None, edgecolors=None, label=None, **kwds):
    ...
    if isinstance(alpha, collections.Iterable):
        ...
        alpha = None
    node_collection = ax.scatter((xy[:, 0]), (xy[:, 1]), s=node_size,
      c=node_color,
      marker=node_shape,
      cmap=cmap,
      vmin=vmin,
...
def draw_networkx_edges(G, pos, edgelist=None, width=1.0, edge_color='k', style='solid', alpha=1.0, arrowstyle='-|>', arrowsize=10, edge_cmap=None, edge_vmin=None, edge_vmax=None, ax=None, arrows=True, label=None, node_size=300, nodelist=None, node_shape='o', **kwds):
    ...
    elif is_string_like(edge_color) or len(edge_color) == 1:
        edge_colors = (
         colorConverter.to_rgba(edge_color, alpha),)
    else:
        msg = 'edge_color must be a color or list of one color per edge'
        raise ValueError(msg)
    edge_collection = G.is_directed() and arrows or 
    ...
    if edge_colors is None:
        ...
        if G.is_directed() and arrows:
            ...
            if arrow_colors is None:
                if edge_cmap is not None and not isinstance(edge_cmap, Colormap):
            ...
            for i, (src, dst) in enumerate(edge_pos):
                ...
                if len(lw) > 1:
                    ...
                    line_width = lw[0]
                arrow = FancyArrowPatch((x1, y1), (x2, y2), arrowstyle=arrowstyle,
...
def draw_networkx_labels(G, pos, labels=None, font_size=1, font_color='k', font_family='sans-serif', font_weight='normal', alpha=1.0, bbox=None, ax=None, **kwds):
    ...
    for (n1, n2), label in labels.items():
        ...
        if rotate:
            ...
            if angle < -90:
                angle += 180
            xy = np.array((x, y))
            trans_angle = ax.transData.transform_angles(np.array((angle,)), xy.reshape((1,
                                                                                        2)))[0]
        else:
            trans_angle = 0.0
        if bbox is None:
