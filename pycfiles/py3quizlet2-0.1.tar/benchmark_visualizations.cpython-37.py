...
# Embedded file name: build/bdist.linux-x86_64/egg/py3plex/visualization/benchmark_visualizations.py
# Compiled at: 2019-02-24 13:10:35
# Size of source mod 2**32: 8556 bytes
import pandas as pd
...
def generic_grouping(fname, score_name, threshold=1, percentages=True):
    ...
    if percentages:
        ...
        grouped = grouped.drop([('percent_train', 'mean')], axis=1).drop([('percent_train', 'std')], axis=1).drop([(score_name, 'std')], axis=1)
