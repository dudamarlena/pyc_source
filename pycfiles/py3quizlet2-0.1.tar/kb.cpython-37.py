...
# Embedded file name: build/bdist.linux-x86_64/egg/py3plex/algorithms/hedwig/core/kb.py
...
class ExperimentKB:
    ...
    def _build_examples(self, g):
        ...
        for i, ex_uri in enumerate(self.examples_uris):
            annot_objects = g.objects(subject=ex_uri, predicate=(HEDWIG.annotated_with))
            annotation_links = [annot for annot in annot_objects]
            annotations = []
            weights = {}
            for link in annotation_links:
                annot_objects = g.objects(subject=link, predicate=(HEDWIG.annotation))
                annotation = [one for one in annot_objects][0]
                weight_objects = g.objects(subject=link, predicate=(HEDWIG.weight))
            ...
            score_list = list(g.objects(subject=ex_uri, predicate=(HEDWIG.score)))
            if score_list:
                score = float(score_list[0])
            else:
                score_list = list(g.objects(subject=ex_uri, predicate=(HEDWIG.class_label)))
                if not score_list:
                    raise Exception('No example labels or scores found! Examples should be ' + 'instances of %s, with %s or %s provided.' % (HEDWIG.Example, HEDWIG.score, HEDWIG.class_label))
                score = str(score_list[0])
                self.class_values.add(score)
            self.uri_to_idx[ex_uri] = i
            examples.append(Example(i, (str(ex_uri)), score, annotations=annotations,
              weights=weights))

        if not examples:
            raise Exception('No examples provided! Examples should be ' + 'instances of %s.' % HEDWIG.Example)
        return (
         examples, all_annotations)

    def _build_subclassof(self, g):
        for predicate in g.subjects(predicate=(RDF.type), object=(HEDWIG.GeneralizationPredicate)):
            for sub, obj in g.subject_objects(predicate=predicate):
                if user_defined(sub) and user_defined(obj):
                    self.add_sub_class(sub, obj)

        for predicate in g.subjects(predicate=(RDF.type), object=(HEDWIG.SpecializationPredicate)):
            for sub, obj in g.subject_objects(predicate=predicate):
                if user_defined(sub) and user_defined(obj):
        ...
        examples_as_domain = set(g.subjects(object=(HEDWIG.Example), predicate=(RDFS.domain)))
        examples_as_range = set(g.subjects(object=(HEDWIG.Example), predicate=(RDFS.range)))
        ...
        for ex in self.examples:
            for inst in ex.annotations:
                if self.instances_as_leaves:
                    ...
                    inst_parents = list(g.objects(subject=(URIRef(inst)), predicate=(RDF.type)))
                    inst_parents += list(g.objects(subject=(URIRef(inst)), predicate=(RDFS.subClassOf)))
                    for obj in inst_parents:
                        self.members[str(obj)].add(ex.id)

    def _find_roots(self, all_annotations):
        roots = list(filter(lambda pred: not self.sub_class_of[pred], self.super_class_of.keys()))
