...
# Embedded file name: build/bdist.linux-x86_64/egg/py3plex/algorithms/hedwig/core/rule.py
# Compiled at: 2019-02-24 13:10:35
# Size of source mod 2**32: 14726 bytes
...
class Rule:
    ...
    def __init__(self, kb, predicates=[], target=None):
        ...
        self.sd = -1
        self.distribution = {}
        self._Rule__refresh_coverage()
        self._Rule__refresh_statistics()
        self.pval = -1
    ...
    def clone_negate(self, target_pred):
        ...
        members.invert()
        neg_pred = UnaryPredicate((target_pred.label), members,
          (self.kb),
    ...
    def clone_swap_with_subclass(self, target_pred, child_pred_label):
        ...
        producer_pred = target_pred.producer_predicate
        var_name = target_pred.input_var
        child_pred = UnaryPredicate(child_pred_label, (self.kb.get_members(child_pred_label)),
          (self.kb),
          producer_pred=producer_pred,
          custom_var_name=var_name)
    ...
    def clone_append(self, predicate_label, producer_pred, bin=False):
        ...
        if not bin:
            new_rule = self.clone()
            predicate = UnaryPredicate(predicate_label, (self.kb.get_members(predicate_label)),
              (self.kb),
              producer_pred=producer_pred)
            ...
            new_rule = self.clone()
            predicate = BinaryPredicate(predicate_label, (self.kb.get_members(predicate_label)),
              (self.kb),
              producer_pred=producer_pred)
            new_rule.predicates.append(predicate)
            new_rule.shared_var[predicate.output_var] = [
             predicate]
            new_rule.shared_var[predicate.input_var].append(predicate)
            new_rule.latest_var = predicate.output_var
        new_rule._Rule__refresh_coverage()
        new_rule._Rule__refresh_statistics()
        return new_rule
    ...
    def _replace_predicate(self, target, replacement):
        ...
        """
        Rule._Rule__replace(self.predicates, target, replacement)
        self.covered_examples = self.covered_examples & replacement.domain[replacement.input_var]
        ...
        Rule._Rule__replace(shared_list, target, replacement)
        self._Rule__refresh_coverage()
        self._Rule__refresh_statistics()
    ...
    def __refresh_coverage(self):
        ...
        self.covered_examples = self._Rule__covered_examples(var)
    ...
    def __covered_examples(self, predicates):
        ...
        for pred in predicates:
            if isinstance(pred, BinaryPredicate):
                ...
                shared.remove(pred)
                existential_cov_examples = self._Rule__covered_examples(shared)
    ...
    def _plain_report(self, show_uris=False, human=lambda label, rule: label):
        ...
        """
        s = self._plain_conjunctions(show_uris=show_uris, human=human) + ' ' + self._plain_statistics()
        return s

    def _plain_conjunctions(self, show_uris=False, human=lambda label, rule: label):
        conjuncts = []
        for pred in self.predicates:
            ...
            elif isinstance(pred, UnaryPredicate):
    ...
    @staticmethod
    def ruleset_report(rules, show_uris=False, latex=False, human=lambda label, rule: label):
        if latex:
            return Rule._latex_ruleset_report(rules)
        return Rule._plain_ruleset_report(rules, show_uris=show_uris, human=human)
    ...
    def _latex_ruleset_report(rules):
        ...
        if target:
            ...
            head = ''
        _tex_report = '\\begin{tabular}{clccccc}\\hline\n\\textbf{\\#} & \\textbf{Rule} & \\textbf{TP} & \\textbf{FP} & \\textbf{Precision} & \\textbf{Lift} & \\textbf{p-value}\\\\\\hline\n'
        for i, rule in enumerate(sorted(rules, key=(lambda r: r.score),
          reverse=True)):
            ...
            stats = (i + 1,
             ...
             rule.score,
             rule.pval)
            _tex_report += '%d & \\texttt{%s} & %d & %d & %.2f & %.2f & %.3f\\\\' % stats
            _tex_report += '\n'
    ...
    @staticmethod
    def _plain_ruleset_report(rules, show_uris=False, human=lambda label, rule: label):
        target, var = list(rules)[0].target, list(rules)[0].head_var
        if target:
            head = "'%s'(%s) <--\n\t" % (target, var)
        else:
