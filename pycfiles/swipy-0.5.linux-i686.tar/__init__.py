...
# [GCC 8.4.0]
# Embedded file name: /home/ww/pl/lib/python2.6/site-packages/swirdf/__init__.py
# Compiled at: 2010-02-09 18:28:45
...
class SWIStore(Store):
    ...
    def triples(self, statement, context=None):
        ...
        for i in Query(query, module=module):
            row = [ self._toNode(X.value) if isinstance(X, Variable) else self._toNode(X) for X in statement
                  ]
            yield (row, context)
    ...
    def _toNode(self, term):
        ...
        if isinstance(term, Term):
            ...
            if name == 'literal':
                ...
                args = term.args
                return (
                 URIRef(str(args[0].value)), args[1].value)
