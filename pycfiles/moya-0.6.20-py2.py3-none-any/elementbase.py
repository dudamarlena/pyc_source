...
# [GCC 8.4.0]
# Embedded file name: /Users/will/projects/moya/moya/elements/elementbase.py
# Compiled at: 2017-09-02 09:28:49
...
from ..compat import implements_to_string, text_type, string_types, with_metaclass, iteritems, iterkeys
import inspect, logging, weakref
from textwrap import dedent
from collections import deque, namedtuple
Closure = namedtuple(b'Closure', [b'element', b'data'])
...
class Attribute(object):
    ...
    def __init__(self, doc, name=None, map_to=None, type=None, required=False, default=None, example=None, metavar=None, context=True, evaldefault=False, oneof=None, synopsis=None, choices=None, translate=None, missing=True, empty=True):
        self.doc = doc
        self.name = name
        ...
        try:
            ...
            raise MoyaAttributeError((b"Unable to create an attribute of type '{}'").format(type))
    ...
    def type_display(self):
        if hasattr(self.type, b'type_display'):
            return self.type.get_type_display()
        return getattr(self.type, b'name', text_type(self.type.__name__))

    def default_display(self, value):
        if self.evaldefault:
            return value
        return self.type.display(value) or b''

    def __repr__(self):
        return b'Attribute(name=%r)' % self.name

    def get_param_info(self):
        param_info = {b'name': self.name, 
           b'doc': self.doc, 
           b'type': self.type_display, 
           b'required': bool(self.required), 
           b'default': self.default, 
           b'default_display': self.default_display(self.default), 
           b'metavar': self.metavar, 
           b'choices': self.choices, 
           b'missing': self.missing, 
           b'empty': self.empty}
        return param_info
...
class _Parameters(object):
    __slots__ = [
     b'__attr_values', b'__context', b'__cache']
...
def _open_tag(tag_name, attrs):
    if attrs:
        a = (b' ').join(b'%s="%s"' % (k, v) for k, v in sorted(attrs.items())).strip()
        return b'<%s>' % (b' ').join((tag_name, a))
    return b'</%s>' % tag_name
...
class _Eval(object):
    __slots__ = [
     b'code', b'filename', b'compiled_code']

    def __init__(self, code, filename):
        self.code = code
        self.filename = filename
        self.compiled_code = compile(code, filename, b'eval')
...
def make_eval(s, filename=b'unknown'):
...
class ChoicesChecker(object):
    __slots__ = [
     b'name', b'element', b'value_callable', b'choices']
    ...
    def __call__(self, context):
        ...
        if value is not None and value not in self.choices:
            ...
            raise errors.ElementError((b"attribute '{}' must be {} (not '{}') ").format(self.name, valid_choices, value), element=self.element)
        return value


class MissingChecker(object):
    __slots__ = [
     b'value_callable', b'name', b'element']
    ...
    def __call__(self, context):
        value = self.value_callable(context)
        if is_missing(value):
            raise errors.ElementError((b"attribute '{}' must not be missing (it is {})").format(self.name, context.to_expr(value)), diagnosis=b"The expression has referenced a value on the context which doesn't exist. Check the expression for typos.", element=self.element)
        return value
...
class EmptyChecker(object):
    ...
    def __call__(self, context):
        value = self.value_callable(context)
        if not value:
            raise errors.ElementError((b"attribute '{}' must not be empty or evaluate to false (it is {})").format(self.name, context.to_expr(value)), diagnosis=b'Check the expression returns a non-empty result.', element=self.element)
        return value
...
class ElementBaseType(object):
    ...
    _element_class = b'data'
    _lib_long_name = None
    _ignore_skip = False
    element_class = b'default'
    xmlns = b'http://moyaproject.com'
    preserve_attributes = []
    ...
    def _get_base_attributes(cls):
        ...
        for el in cls.__mro__[1:]:
            if hasattr(el, b'_get_tag_attributes'):
    ...
    def extract_doc_info(cls):
        ...
        doc = {}
        doc[b'namespace'] = cls.xmlns
        doc[b'tag_name'] = cls._tag_name
        doc[b'doc'] = dedent(cls._tag_doc) if cls._tag_doc else cls._tag_doc
        doc[b'defined'] = getattr(cls, b'_definition', b'?')
        if hasattr(cls, b'Help'):
            example = getattr(cls.Help, b'example', None)
            doc[b'example'] = example
            doc[b'synopsis'] = getattr(cls.Help, b'synopsis', None)
        ...
        doc[b'params'] = params
        doc[b'inherited_params'] = inherited_params
        return doc

    @classmethod
    def _get_help(cls, attribute_name, default):
        help = getattr(cls, b'Help', None)
        if help is None:
            return default
        else:
    ...
    def moya_name(self):
        ns = self.xmlns
        if ns.startswith(b'http://moyaproject.com/'):
            ns = ns[len(b'http://moyaproject.com/'):]
        return (b'{{{}}}{}').format(ns, self.tag_name)

    def get_app(self, context, app_attribute=b'from', check=True):
        app = None
        ...
        if check:
            ...
            if not isinstance(app, Application):
                raise errors.AppError((b'expected an application object here, not {!r}').format(app))
        return app
            ...
            if check_missing:
                for k, v in iteritems(let_map):
                    if getattr(v, b'moya_missing', False):
                        raise errors.ElementError((b'let:{} must not be missing (it is {!r})').format(k, v))
    ...
    def compile_expressions(self):
        ...
        if getattr(self, b'_attrs', None):
            for k, v in self._attrs.items():
                ...
                if b'${' in v and b'}' in v:
                    Expression.extract(v)

        if getattr(self, b'_let', None):
    ...
    def get_parameters_map(self, context, *names):
        if not names:
            raise ValueError(b'One or more attribute names must be supplied')
    ...
    def get_all_data_parameters(self, context):
        return {k:v(context) for k, v in self._attr_values.items() if not k.startswith(b'_')}
    ...
    def auto_build(self, context, text, attrs, translatable_attrs):
        ...
        if self._required_tag_attributes and not self._required_tag_attributes.issubset(attrs_keys_set):
            ...
            if len(missing) == 1:
                raise errors.ElementError(b"'%s' is a required attribute" % missing[0], element=self)
            else:
                raise errors.ElementError(b'%s are required attributes' % (b', ').join(b"'%s'" % m for m in missing), element=self)
        if hasattr(self._meta, b'one_of'):
            for group in self._meta.one_of:
                if not any(name in attrs for name in group):
                    raise errors.ElementError((b'one of {} is required').format(textual_list(group)), element=self)

        if not getattr(self._meta, b'all_attributes', False) and not self._tag_attributes_set.issuperset(attrs_keys_set):
            unknown_attrs = sorted(attrs_keys_set - self._tag_attributes_set)
            diagnosis = b''
            if len(unknown_attrs) == 1:
                msg = b'{} is not a valid attribute on this tag'
                nearest = nearest_word(unknown_attrs[0], self._tag_attributes_set)
                if nearest is not None:
                    diagnosis = (b"Did you mean '{}'?").format(nearest)
                else:
                    diagnosis = (b'Valid attributes on this tag are {}').format(textual_list(sorted(self._tag_attributes_set)))
            else:
                msg = b'Attributes {} are not valid on this tag'
            diagnosis += (b'\n\nrun the following for more information:\n\n**$ moya help {}**').format(self.moya_name)
            raise errors.ElementError(msg.format(textual_list(unknown_attrs, b'and')), element=self, diagnosis=diagnosis)
    ...
    def __init__(self, document, xmlns, tag_name, parent_docid, docid, source_line=None):
        ...
        if not hasattr(self.__class__, b'_definition'):
    ...
    def get_ancestor(self, element_type):
        ...
        raise errors.ElementNotFoundError(element_type, msg=(b'{} has no ancestor of type {}').format(self, format_element_type(element_type)))
        return
    ...
    def libid(self):
        if self._libid is not None:
            ...
            if not hasattr(self, b'libname'):
                return
            if not self.document.lib:
                return
            return b'%s#%s' % (self.document.lib.long_name or b'', self.libname)

    def get_appid(self, app=None):
        if app is None:
            return self.libid
        else:
            return b'%s#%s' % (app.name or b'', self.libname)
    ...
    def process_attrs(self, attrs, attr_types):
        ...
        def asint(name, s):
            if s.isdigit() or s.startswith(b'-') and s[1:].isdigit():
                return int(s)
            raise errors.AttributeTypeError(self, name, s, b'int')

        def asfloat(name, s):
            try:
                return float(s)
            except:
                raise errors.AttributeTypeError(self, name, s, b'float')

        for k in attrs.keys():
            if k not in attr_types:
                continue
            attr_type = attr_types[k]
            if attr_type in (bool, b'bool'):
                attrs[k] = attrs[k].strip().lower() in ('yes', 'true')
            elif attr_type in (int, b'int'):
                attrs[k] = asint(k, attrs[k])
            elif attr_type in (float, b'float'):
    ...
    def younger_siblings(self, element_type=None, element_class=None, **attrs):
        ...
        while next(iter_siblings) is self:
            break
    ...
    def get_closure(self, context, element=None, extra=None):
        ...
        if element is None:
            element = self
        data = {k:v for k, v in context.items(b'') if not k.startswith(b'_') if not k.startswith(b'_')}
...
class FunctionCallParams(object):
    ...
    def __str__(self):
        args_str = (b', ').join(repr(a) for a in self._args)
        kwargs_str = (b', ').join(b'%s=%r' % (k.encode(b'ascii', b'replace'), repr(v)) for k, v in self._kwargs.items())
        return b'(%s)' % (b', ').join((args_str.strip(), kwargs_str.strip()))

    def __repr__(self):
        return b'FunctionCallParams%s' % text_type(self)
...
class ReturnContainer(object):
    ...
    def __moyarepr__(self, context):
        return (b'<return {}>').format(self.get_return_value())
...
class CallStackEntry(dict):
    __slots__ = [
     b'element', b'app', b'yield_element', b'yield_frame']
...
class _CallContext(object):
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        elif b'_return' in call:
            self.has_return = True
            return_value = call.get(b'_return')
            if hasattr(return_value, b'get_return_value'):
...
class LogicElement(ElementBase):
    ...
    _if = Attribute(b'Conditional expression', type=b'expression', map_to=b'_if', default=True)
    ...
    def logic(self, context):
        yield iter(self.children(element_class=b'logic'))
    ...
    def pop_call(self, context):
        ...
        context.pop_frame()
        if callstack:
            context.root[b'call'] = callstack[(-1)]
        else:
            del context[b'.call']
        return call
    ...
    def pop_defer(self, context):
        ...
        if callstack:
            ...
            del context.root[b'.call']
        return call

    def push_funccall(self, context):
        funccalls = context.set_new_call(b'._funccalls', list)
        params = FunctionCallParams()
        funccalls.append(params)
        context.push_scope(makeindex(b'._funccalls', len(funccalls) - 1))
        return params

    def pop_funccall(self, context):
        funccalls = context[b'._funccalls']
        context.pop_scope()
...
if __name__ == b'__main__':
    ...
    document = Document()
    t = TestElement(document, None, b'tag', None, b'foo')
    ElementBase._build(t, b'', dict(apples=1))
    t2 = TestElement(document, None, b'tag2', b'foo', b'bar')
    ElementBase._build(t2, b'', dict(p=1))
    t3 = TestElement(document, None, b'tag2', b'foo', b'baz')
    ElementBase._build(t3, b'', dict(p=2))
