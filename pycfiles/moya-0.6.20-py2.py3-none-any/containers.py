...
# [GCC 8.4.0]
# Embedded file name: /Users/will/projects/moya/moya/containers.py
# Compiled at: 2015-10-30 17:14:09
...
class LRUCache(OrderedDict):
    ...
    def __reduce__(self):
        return (
         self.__class__, (self.cache_size,))
...
class QueryData(OrderedDict):
    ...
    def update(self, d):
        ...
        for k, v in d.items():
            if v is None:
                if k in self:
                    del self[k]
            elif isinstance(v, (list, set, tuple, dict)) or hasattr(v, b'items'):
                self[k] = list(v)
            else:
                if v is None:
                    v = b''
    ...
    def __repr__(self):
        return (b'<querydata "{}">').format(urlencode(self))
...
if __name__ == b'__main__':
    qd = QueryData.from_qs(b'foo=bar&a=1&b=2&hobbit=frodo&hobbit=sam')
    print(qd.items())
    qd.update({b'foo': None})
    print(qd.items())