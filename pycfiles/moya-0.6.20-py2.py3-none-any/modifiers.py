...
# [GCC 8.4.0]
# Embedded file name: /Users/will/projects/moya/moya/context/modifiers.py
# Compiled at: 2017-08-21 13:25:21
from __future__ import absolute_import
from __future__ import print_function
from __future__ import unicode_literals
from ..versioning import Version, VersionSpec
from ..url import URL, get_domain, urlencode
from ..compat import text_type, py2bytes, string_types, int_types, number_types, unichr, implements_to_string
...
from ..context.expressiontime import TimeSpan, ExpressionDateTime, ExpressionDate, ExpressionTime, epoch_to_datetime
...
from .. import compat
from fs.path import abspath, basename, join, normpath, relativefrom, dirname, splitext, split
import uuid, hashlib, copy, collections
...
class Sentinel(object):
    ...
    def __repr__(self):
        return (b"Sentinel('{s}')").format(self.name)

    def __moyarepr__(self, context):
        return (b"sentinel:'{}'").format(self.name)
...
def _slashjoin(paths):
    ...
    _paths = [
     paths.pop(0).rstrip(b'/')]
    append = _paths.append
    for p in paths:
        append(b'/')
        append(p.lstrip(b'/'))

    return (b'').join(_paths)


def make_uuid(context, version, nstype=b'url', nsname=None):
    ...
    if nsname is None and namespace == uuid.NAMESPACE_URL:
        namespace = context[b'.request.host_url']
    if version == 1:
        _uuid = uuid.uuid1()
    elif version == 3:
...
class ExpressionModifiersBase(object):
    ...
    def is_missing(cls, val):
        ...
        return getattr(val, b'moya_missing', False)
    ...
    def _keys(cls, context, obj):
        ...
        if hasattr(obj, b'__getitem__'):
            if hasattr(obj, b'keys'):
                return list(obj.keys())
            else:
                return [ i for i, _v in enumerate(obj) ]

        else:
            return [ k for k in dir(obj) if not k.startswith(b'_') ]
    ...
    def _values(cls, context, obj):
        if cls.is_missing(obj):
            return []
        if hasattr(obj, b'__getitem__'):
            if hasattr(obj, b'values'):
                return [ get_moya_interface(context, v) for v in obj.values() ]
            else:
                return obj[:]

        else:
            return [ get_moya_attribute(context, obj, k) for k in dir(obj) if not k.startswith(b'_') ]
    ...
    def _urlencode(cls, data):
        if not hasattr(data, b'items'):
            raise ValueError((b"Can't urlencode {}").format(data))
        return urlencode(data)

    @classmethod
    def _qsupdate(cls, context, data, base_qs=None):
        if base_qs is None:
            base_qs = context.get(b'.request.query_string', b'')
    ...
    def _filesize(cls, size):
        ...
        for i, suffix in enumerate(suffixes):
            unit = base ** (i + 2)
            if size < unit:
                return (b'{:,.1f} {}').format(base * size / unit, suffix)
    ...
    def _anypermission(cls, context, v):
        if not v:
            ...
            permissions = context[b'.permissions']
...
class ExpressionModifiers(ExpressionModifiersBase):
    ...
    def app(self, context, v):
        ...
        app_name = v
        if hasattr(v, b'__moya_application__'):
            return v.__moya_application__()
        app_name = text_type(v)
        archive = context[b'.app.archive']
    ...
    def choices(self, context, v):
        return getattr(v, b'choices', [])

    def intchoices(self, context, v):
        return getattr(v, b'intchoices', [])
    ...
    def collectids(self, context, v, _lookup_key=ExpressionModifiersBase._lookup_key):
        if v:
            return [ _item for _item in (_lookup_key(item, b'id', Ellipsis) for item in v) if _item is not Ellipsis ]
        return []
    ...
    def commalist(self, context, v):
        return (b',').join(text_type(s) for s in v)

    def commaspacelist(self, context, v):
        return (b', ').join(text_type(s) for s in v)

    def commasplit(self, context, v):
        return [ t for t in text_type(v).split(b',') if t ]
    ...
    def count(self, context, v):
        try:
            ...
            raise ValueError(b'count: modifier expects [<seq>, <expression>]')

        if not hasattr(predicate, b'__moyacall__'):
            raise ValueError(b'count: requires a sub expression, e.g. map:[students, `grade lt "A"`]')
        return sum(1 for item in seq if predicate.__moyacall__(item))

    def csrf(self, context, v):
        user_id = text_type(context[b'.session_key'] or b'')
        form_id = text_type(v)
        secret = text_type(context[b'.secret'])
        raw_token = (b'{}{}{}').format(user_id, secret, form_id).encode(b'utf-8', b'ignore')
        m = hashlib.md5()
        m.update(raw_token)
    ...
    def data(self, context, v):
        if isinstance(v, list):
            try:
                ...
                raise ValueError(b'data: requires list [<app>, <path]')
            ...
            path = text_type(v)
        try:
            data_fs = context[b'.fs'][b'data']
        except KeyError:
            raise ValueError(b"missing 'data' filesystem")
    ...
    def enum(self, context, v):
        key = v
        if b'#' in key:
            app, el = context[b'.app'].get_element(key)
            return context[b'.enum'].get(el.libid, None)
        else:
            return context[b'.app.lib.enum'].get(key, None)
            return
    ...
    def epoch(self, context, v):
        try:
            ...
            raise ValueError((b'unable to convert {} to a number').format(context.to_expr(v)))
    ...
    def eval(self, context, v):
        ...
        if not isinstance(v, text_type):
            raise ValueError((b'eval: modifier expects a string (not {})').format(context.to_expr(v)))
        return Expression(v).eval(context)

    def exists(self, context, v):
        return not getattr(v, b'moya_missing', False)

    def ext(self, context, v):
        return splitext(v)[1].lstrip(b'.')
    ...
    def filter(self, context, v):
        try:
            ...
            raise ValueError(b'filter: modifier expects [<seq>, <expression>]')

        if not hasattr(predicate, b'__moyacall__'):
            raise ValueError(b'filter: requires an expression, e.g. map:[students, {grade gt "A"}]')
    ...
    def group(self, context, v):
        try:
            ...
            raise ValueError(b'group: modifier expects [<sequence>, <expression]')
    ...
    def counts(self, context, v):
        try:
            ...
            raise ValueError(b'counts: modifier expects [<sequence>, <expression]')
    ...
    def groupsof(self, context, v):
        try:
            ...
            raise ValueError(b'groupsof: operator requires pair of [<sequence>, <group size>')

        seq = list(seq)
        try:
            group_size = int(group_size)
        except:
            raise ValueError((b"group size must be an integer (not '{}')").format(context.to_expr(group_size)))

        if group_size <= 0:
            raise ValueError(b'group size must be a positive integer')
    ...
    def ids(self, context, v):
        try:
            return [ item.id for item in v if hasattr(item, b'id') ]
        except:
            return []
    ...
    def isemail(self, context, v):
        email = text_type(v)
        return b'@' in email and b'.' in email
    ...
    def join(self, context, v):
        return (b'').join(text_type(i) for i in v)

    def joinspace(self, context, v):
        return (b' ').join(text_type(i) for i in v if i)

    def joinwith(self, context, v):
        try:
            join, char = v
        except:
            raise ValueError(b"joinwith: expects two values, e.g, joinwith:[filenames, ', ']")
    ...
    def map(self, context, v):
        try:
            ...
            raise ValueError(b'map: modifier expects [<seq>, <expression>]')

        if not hasattr(key, b'__moyacall__'):
            raise ValueError(b'map: requires an expression, e.g. map:[sequence, {name}]')
    ...
    def missing(self, context, v):
        return getattr(v, b'moya_missing', False)
    ...
    def partition(self, context, v, _as_text=as_text):
        ...
        if isinstance(v, list):
            try:
                ...
                raise ValueError(b'partition expects string or [<string>, <partition>]')
    ...
    def parsedatetime(self, context, v):
        try:
            ...
            raise ValueError(b'parsedatetime: modifier requires [<datestring>, <dateformat>]')
    ...
    def replace(self, context, v):
        try:
            ...
            raise ValueError(b'replace: expects [<text>, <replace dict>]')
    ...
    def rpartition(self, context, v, _as_text=as_text):
        ...
        if isinstance(v, list):
            try:
                ...
                raise ValueError(b'rpartition expects string or [<string>, <partition>]')
    ...
    def prettylist(self, context, v):
        return (b', ').join((b"'{}'").format(text_type(s)) for s in v)

    def qsupdate(self, context, v):
        return self._qsupdate(context, v)

    def quote(self, context, v):
        return (b'"{}"').format(text_type(v))

    def relto(self, context, v):
        base = dirname(context.get(b'.request.path', b'/'))
    ...
    def render(self, context, v):
        ...
        if archive is None:
            ...
            html = render_object(v, archive, context, b'html')
            return html

    def renderable(self, context, v):
        return getattr(v, b'__moyarenderable__', lambda c: v)(context)
    ...
    def rsortedby(self, context, v):
        try:
            ...
            raise ValueError(b'rsortedby: requires two arguments [<sequence>, <key>]')
    ...
    def slug(self, context, v):
        if getattr(v, b'moya_missing', False):
            raise ValueError((b"slug: can't generate a slug from {}").format(v))
        return slugify(v)
    ...
    def sortedby(self, context, v):
        try:
            ...
            raise ValueError(b'sortedby: requires two arguments [<sequence>, <key>]')
    ...
    def stripall(self, context, v):
        try:
            ...
            raise ValueError(b'stripall: takes a sequence of strings')
    ...
    def sumexp(self, context, v):
        try:
            ...
            raise ValueError(b'sumexp: modifier expects [<seq>, <expression>]')

        if not hasattr(exp, b'__moyacall__'):
            raise ValueError(b'sumexp: requires a sub expression, e.g. map:[students, `len:grades`]')
    ...
    def trim(self, context, v):
        try:
            ...
            raise ValueError(b'trim: requires a collection of [<string>, <max length>]')
    ...
    def title(self, context, v):
        return text_type(v).replace(b'_', b' ').replace(b'-', b' ').title()

    def token(self, context, v):
        try:
            size = int(v)
        except:
            raise ValueError(b'token: modifier requires an integer')

        if size < 1:
            raise ValueError(b'token: modifer requires a size >= 1')
        return (b'').join(random.choice(b'abcdefghijklmnopqrstuvwxyz0123456789') for _ in range(size))
    ...
    def update(self, context, v):
        try:
            ...
            raise ValueError(b'update: required a sequence of [<dict>, <dict>]')
    ...
    def urlupdate(self, context, v):
        if isinstance(v, list):
            ...
            url = URL(context[b'.request.path_qs'])
            data = v
        ...
        if v in (1, 4):
            ...
            try:
                if len(v) == 2:
                    ...
                    raise ValueError(b'uuid: modifier requires 1-3 values')
            except (TypeError, ValueError):
                raise ValueError(b'invalid params for uuid: modifier')

            if version not in (3, 5):
                raise ValueError(b'uuid: modifier uuid type must be 1, 2, 4, or 5')
