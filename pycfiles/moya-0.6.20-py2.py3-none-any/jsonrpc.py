# uncompyle6 version 3.7.4
# Python bytecode 2.7 (62211)
# Decompiled from: Python 3.6.9 (default, Apr 18 2020, 01:56:04) 
# [GCC 8.4.0]
# Embedded file name: /Users/will/projects/moya/moya/jsonrpc.py
# Compiled at: 2015-09-01 07:17:44
from __future__ import unicode_literals
from __future__ import print_function
import json, requests

class ProtocolError(Exception):
    """Errors where the server didn't return the correct response"""
    pass


class JSONRPCError(Exception):
    """Base class for exceptions returned from the server"""

    def __init__(self, method, code, data, message):
        self.method = method
        self.code = code
        self.data = data
        self.message = message
        super(JSONRPCError, self).__init__(message)


class RemoteError(JSONRPCError):
    """One of the generic error types defined in ErrorCode"""
    pass


class RemoteMethodError(JSONRPCError):
    """An error returned from the server"""
    pass


class ErrorCode(object):
    """Enumeration of JSONRPC error codes"""
    parse_error = -32700
    invalid_request = -32600
    method_not_found = -32601
    invalid_params = -32602
    internal_error = -32603
    to_str = {-32700: b'Parse error', -32600: b'Invalid Request', 
       -32601: b'Method not found', 
       -32602: b'Invalid params', 
       -32603: b'Internal error'}


class Batch(object):
    """An object that stores a batch of rpc calls

    May be used as a context manager

        with client.batch() as batch:
            batch.call("method1", foo="bar")
            batch.call("method2", foo="baz")

    Method call results are stored in `results` which maps the call id on to results.
    Errors are contained in `errors` which maps the call id on to a dictionary with error code / message

    """

    def __init__(self, client):
        self.client = client
        self.calls = []
        self.sent = False
        self.responses = None
        self.errors = None
        self.results = None
        self.ids_used = set()
        self.methods = {}
        super(Batch, self).__init__()
        return

    def __enter__(self):
        return self

    def __exit__(self, exc_type, exc_value, traceback):
        if not exc_type:
            self.send()

    def call(self, method, **params):
        """Add a call to the batch, using a default id"""
        call = {b'jsonrpc': b'2.0', 
           b'method': method, 
           b'params': params, 
           b'id': self.client.new_call_id()}
        self.calls.append(call)
        self.methods[call[b'id']] = method

    def call_with_id(self, call_id, method, **params):
        """Add a call to the batch with a supplied id"""
        if call_id in self.ids_used:
            raise ValueError(b'duplicate call id in batch')
        call = {b'jsonrpc': b'2.0', b'method': method, 
           b'params': params, 
           b'id': call_id}
        self.calls.append(call)
        self.ids_used.add(call_id)
        self.methods[call[b'id']] = method

    def notify(self, method, **params):
        call = {b'jsonrpc': b'2.0', 
           b'method': method, 
           b'params': params}
        self.calls.append(call)

    def send(self):
        response_json = self.client._send(self.calls)
        self.sent = True
        response = json.loads(response_json)
        if not isinstance(response, list):
            raise ProtocolError(b'Expected a list of response from the server')
        self.responses = [ (r.get(b'error', None), r.get(b'result', None)) for r in response
                         ]
        self.errors = {r[b'id']:r[b'error'] for r in response if b'error' in r if b'error' in r}
        self.results = {r[b'id']:r for r in response if b'id' in r and b'error' not in r if b'id' in r and b'error' not in r}
        return

    def get_result(self, call_id, default=Ellipsis):
        """Get a result from the batch, potentially raising rpc errors"""
        if call_id in self.results:
            return self.results[call_id].get(b'result', None)
        else:
            if call_id in self.errors:
                if default is Ellipsis:
                    self.client._handle_error(self.methods[call_id], self.errors[call_id])
                else:
                    return default
            else:
                raise KeyError(b'No such call_id in response')
            return


class JSONRPC(object):
    """A client for a JSONRPC server"""
    unknown_error_msg = b'the server did not supply further information'

    def __init__(self, url, ssl_verify=True):
        self.url = url
        self.call_id = 1
        self.ssl_verify = ssl_verify

    def new_call_id(self):
        self.call_id += 1
        return self.call_id

    def _send(self, call):
        call_json = json.dumps(call).encode(b'utf-8')
        response_json = requests.post(self.url, call_json, verify=self.ssl_verify).json()
        return response_json

    def call(self, method, **params):
        """Call a remote method"""
        call_id = self.new_call_id()
        call = {b'jsonrpc': b'2.0', 
           b'method': method, 
           b'params': params, 
           b'id': call_id}
        response = self._send(call)
        if b'jsonrpc' not in response or b'id' not in response:
            raise ProtocolError(b'Invalid response from server')
        if response[b'jsonrpc'] != b'2.0':
            raise ProtocolError(b'Client only understands JSONRPC v2.0')
        if response[b'id'] != call_id:
            raise ProtocolError(b"Invalid response from the server, 'id' field does not match")
        if b'error' in response:
            error = response[b'error']
            self._handle_error(method, error)
        return response.get(b'result', None)

    def notify(self, method, **params):
        """Send a notification to the server"""
        notify = {b'jsonrpc': b'2.0', 
           b'method': method, 
           b'params': params}
        self._send(notify)

    def batch(self):
        """Create a batch object that can be used to send multiple calls / notifications"""
        return Batch(self)

    def _handle_error(self, method, error):
        code = error.get(b'code')
        if code in ErrorCode.to_str:
            raise RemoteError(method, code, error.get(b'data', None), error.get(b'message', ErrorCode.to_str[code]))
        raise RemoteMethodError(method, code, error.get(b'data', None), error.get(b'message', self.unknown_error_msg))
        return