...
# [GCC 8.4.0]
# Embedded file name: /Users/will/projects/moya/moya/elements/registry.py
...
class ElementRegistry(object):
    ...
    def register_element(self, xmlns, name, element):
        ...
        if name in self._registry[xmlns]:
            ...
            definition = getattr(element_class, b'_location', None)
            if definition is None:
                definition = inspect.getfile(element_class)
            if xmlns:
                raise errors.ElementError((b'<{}> already registered in "{}" for xmlns "{}"').format(name, definition, xmlns), element=getattr(element, b'element', element))
            else:
                raise errors.ElementError((b'<{}/> already registered in "{}"').format(name, definition), element=element)
    ...
    def get_elements_in_lib(self, long_name):
        ...
        for namespace in itervalues(self._registry):
            lib_elements.extend(element for element in itervalues(namespace) if element._lib_long_name == long_name)
