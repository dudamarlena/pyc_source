...
# [GCC 8.4.0]
# Embedded file name: /Users/will/projects/moya/moya/trace.py
# Compiled at: 2015-10-30 19:52:54
        ...
        for frame in stack:
            ...
            if frame.one_line:
                ...
                if frame.format == 'xml':
                    ...
                    console.templatesnippet(frame.code, lineno=frame.lineno, colno=start, endcolno=end)
                else:
...
def build(context, stack, node, exc, exc_info, request, py_traceback=True):
    ...
    for s in stack:
        ...
        if e and e._code:
            frame = Frame(e._code, e._location, e.source_line or 1, obj=text_type(e), libid=e.libid)
            traceback.add_frame(frame)

    element = getattr(exc, 'element', None)
    if element is not None and hasattr(element.document, 'structure'):
        frame = Frame(element.document.structure.xml, element._location, element.source_line or 1, obj=text_type(element), libid=element.libid)
        traceback.add_frame(frame)
        add_pytraceback = False
    elif hasattr(node, '_location') and hasattr(node, 'source_line'):
        if node._code:
            frame = Frame(node._code, node._location, node.source_line or 1, obj=text_type(node), libid=node.libid)
            traceback.add_frame(frame)
    ...
    if add_pytraceback and exc_info and py_traceback:
        ...
        for filename, line_number, function_name, text in pyframes:
            ...
            frame = Frame(code, code_path, line_number, one_line=one_line, obj=function_name, format='python')
            traceback.add_frame(frame)
            traceback.msg = text_type(exc)
