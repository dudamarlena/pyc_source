...
# [GCC 8.4.0]
# Embedded file name: /Users/will/projects/moya/moya/docgen/extracter.py
    ...
    def extract_tags(self, elements, const_data=None):
        ...
        with pilot.console.progress('extracting tags', len(elements) + 1) as (progress):
            for element in progress(elements):
                ...
                doc.data.update(namespace=element.xmlns, namespace_slug=self.slugify_namespace(element.xmlns), name=element_name, lib=element._lib_long_name)
            ...
            for ns in tag_namespaces:
                ...
                ns_index.data.update(tags=sorted(tags_index.data['by_namespace'][ns], key=lambda t: t['tag_name']), namespace=ns, namespace_doc=namespaces.namespace_docs.get(ns))
    ...
    def extract_commands(self, elements, const_data=None):
        ...
        with pilot.console.progress('extracting commands', len(elements) + 1) as (progress):
            for element in progress(elements):
                ...
                doc.data.update(const_data or {})
                doc.data.update(element.extract_doc_info())
                doc.data.update(namespace=element.xmlns, namespace_slug=self.slugify_namespace(element.xmlns), name=element.libname, doc=element._doc or '', signature=element._signature, synopsis=element._synopsis)
