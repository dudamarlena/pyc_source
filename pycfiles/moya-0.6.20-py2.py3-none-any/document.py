...
# [GCC 8.4.0]
# Embedded file name: /Users/will/projects/moya/moya/document.py
# Compiled at: 2016-12-21 12:13:43
...
class DocumentStructure(object):
    ...
    def load(cls, doc_dump, document, library):
        ...
        structure = DocumentStructure(document, library, serialized[b'xml'])
        nodes = serialized[b'nodes']
    ...
    def render(self):
        ...
        def write_node(node, level=0):
            ...
            if isinstance(node, DocumentTextNode):
                print((b'{}{!r}').format(tab, node.text))
            else:
                print((b'{}{}').format(tab, node))
...
class DocumentNode(object):
    ...
    def __init__(self, xmlns, tag_name, parent_doc_id, attrs, translatable_attrs, text, source_line, translate_text=False):
    ...
    def build(self, archive, context):
        ...
        docname = attrs.get(b'docname', None)
        libname = attrs.get(b'libname', None)
        structure = self.structure
        document = structure.document
        tag_name = self.tag_name
        structure.docid_counts[tag_name] += 1
        docid = (b'{}.{}').format(tag_name, structure.docid_counts[tag_name])
        if libname is None and structure.library is not None:
            libname = structure.library.allocate_libname(docid)
        replace_lib = False
        if libname is not None and b'#' in libname:
            lib_, _, libname = libname.partition(b'#')
        ...
        if element_type is None:
            ...
            if self.parent and self.parent.element:
                ...
                parent_docid = None
            element = element_type(structure.document, self.xmlns, tag_name, parent_docid, docid, source_line=self.source_line)
            element.libname = libname
            element.docname = docname
            element._code = structure.xml
            element._build(context, self.text or b'', attrs, self.translatable_attrs)
            element._document = weakref.ref(document)
            element._let_exp = None
            ...
            if lib:
                element._libid = (b'{}#{}').format(lib.long_name, libname)
                if replace_lib:
                    lib.add_replacement_node(element)
                else:
                    lib.register_named_element(libname, element, priority=structure.library.priority)
            return element
    ...
    def xmlns_name(self):
        if self.xmlns == namespaces.default:
            ...
            return (b'{{{}}}{}').format(self.xmlns, self.tag_name)

    @property
    def tag_display_name(self):
        return (b'<{}>').format(self.xmlns_name)
...
class Document(object):
    ...
    def __repr__(self):
        if self.lib:
            return b'<document %s:"%s">' % (self.lib.long_name, self.path)
        else:
            return b'<document "%s">' % self.path
    ...
    def dumps(self, expressions=None):
        if expressions is None:
            expressions = []
        doc = {b'expressions': expressions, b'root': self.root_element.dumps()}
        return doc

    def loads(self, data):
        from .elements.elementbase import ElementBase
        Expression.insert_expressions(data[b'expressions'])
        self.root_element = ElementBase.loads(data[b'root'], self)
        return self
    ...
    def get_app_element(self, element_ref, app=None):
        if b'#' in element_ref:
            ...
            if lib_id is not None:
                ...
                element_ref = (b'{}#{}').format(app.name, name)
    ...
    def qualify_element_ref(self, element_ref, app=None, lib=None):
        if element_ref.find(b'#', 1) != -1:
            ...
            if self.lib:
                ...
                return b'%s#%s' % (qname, element.libname)
            return element_ref
    ...
    def register_named_element(self, docname, element):
        if docname in self.named_elements:
            raise ElementError((b"docname '{}' is already present in this document").format(docname), element=element)
    ...
    def finalize(self, context):
        if self.finalized:
            return
        ignore_errors = bool(context[b'._ignore_finalize_errors'])
