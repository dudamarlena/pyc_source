...
# [GCC 8.4.0]
# Embedded file name: /Users/will/projects/moya/moya/interface.py
    ...
    def deco(cls):
        ...
        class ContextClass(base_class):
            ...
            def __moyacontext__(self, context):
                if not hasattr(self, b'_moya_context'):
...
class Proxy(object):
    ...
    @property
    def moya_proxy(self):
        if not hasattr(self, b'_moya_proxy'):
...
class AttributeExposer(object):
    ...
    def keys(self):
        return [ k.lstrip(b'_') for k in self.__moya_exposed_attributes__ ]

    def iterkeys(self):
        return iter(k.lstrip(b'_') for k in self.__moya_exposed_attributes__)
    ...
    def values(self):
        return [ getattr(self, k) for k in self.__moya_exposed_attributes__
               ]

    def items(self):
        return [ (k.lstrip(b'_'), getattr(self, k)) for k in self.__moya_exposed_attributes__
               ]

    def iteritems(self):
        for k in self.__moya_exposed_attributes__:
            yield (k.lstrip(b'_'), getattr(self, k))

    def __getitem__(self, k):
        if k in self.__moya_exposed_attributes__:
            return getattr(self, k)
        if b'_' + k in self.__moya_exposed_attributes__:
            return getattr(self, b'_' + k)
        raise KeyError(k)
    ...
    def __setitem__(self, k, v):
        ...
        if b'_' + k in self.__moya_exposed_attributes__:
            return setattr(self, b'_' + k, v)
        raise KeyError(k)

    def __contains__(self, k):
        return k in self.__moya_exposed_attributes__ or b'_' + k in self.__moya_exposed_attributes__

    def get(self, k, default=None):
        if k in self.__moya_exposed_attributes__:
            return getattr(self, k, default)
        if b'_' + k in self.__moya_exposed_attributes__:
            return getattr(self, b'_' + k, default)
        raise KeyError(k)
...
class ObjectExposer(AttributeExposer):
    ...
    def __moya_exposed_attributes__(self):
        if not hasattr(self, b'_exposed_attributes'):
            self._exposed_attributes = sorted([ k for k in dir(self) if not k.startswith(b'_') and not callable(getattr(self, k))
                                              ])
...
if __name__ == b'__main__':
    ...
    class A(AttributeExposer):
        __moya_exposed_attributes__ = [b'a', b'b']
