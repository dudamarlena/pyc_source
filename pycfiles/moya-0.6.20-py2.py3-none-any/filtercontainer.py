...
# [GCC 8.4.0]
# Embedded file name: /Users/will/projects/moya/moya/filtercontainer.py
...
class FilterKeyError(KeyError):
    ...
    def __repr__(self):
        return (b'FilterKeyError({!r})').format(self.msg)
...
class FilterContainer(object):
    ...
    def _lookup(self, app, name):
        ...
        if b' ' in name:
            tokens = name.split()
            if len(tokens) != 3 or tokens[1] != b'from':
                raise ValueError(b'filter strings should be in the format "<NAME> from <APP or LIB>"')
    ...
    def lookup(self, app, name):
        ...
        filters = [ filter_app.filters[name] for filter_app in apps if name in filter_app.filters ]
        if not filters:
            raise FilterKeyError((b"no filter called '{}'").format(name))
        if len(filters) != 1:
            raise ValueError((b'filter is ambiguous, specify "{} from <APP or LIB>"').format(name))
        return filters[0]
