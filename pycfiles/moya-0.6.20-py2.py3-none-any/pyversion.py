...
# [GCC 8.4.0]
# Embedded file name: /Users/will/projects/moya/moya/pyversion.py
...
def check(version_spec, py_version=None, _re_version=re.compile(b'^(py)$|^py(\\d)$|^py(\\d)\\.(\\d)$|^py(\\d)\\.(\\d)\\+$')):
    ...
    tokens = [ token.strip() for token in version_spec.split(b',') if token ]
    for version in tokens:
        match = _re_version.match(version)
        if not match:
            raise VersionFormatError((b'{} is not a valid Py version spec').format(version))
        all_py, absolute_major_version, major_version, minor_version, major_version_plus, minor_version_plus = match.groups()
        if all_py or absolute_major_version and int(absolute_major_version) == major or major_version and int(major_version) == major and int(minor_version) == minor or major_version_plus and int(major_version_plus) == major and minor >= int(minor_version_plus):
            return True
...
if __name__ == b'__main__':
    print(b'Python 2?', check(b'py2'))
    print(b'Python 3?', check(b'py3'))
    tests = [
     b'py', b'py2', b'py2.6', b'py2.6+', b'py3']
    for test in tests:
        print(test, check(test, (2, 7)))

    print(check(b'py2.5+,py2.6', (2, 7)))
    print(check(b'py2.7,py3.2+', (3, 3)))
    print(list_compatible(b'py'))
    print(list_compatible(b'py2'))
    print(list_compatible(b'py2.7+,py3'))
    print(list_compatible(b'py3.2+'))
    print(list_compatible(b'py2.7,py3.2+'))