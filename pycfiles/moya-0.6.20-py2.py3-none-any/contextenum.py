...
# [GCC 8.4.0]
# Embedded file name: /Users/will/projects/moya/moya/contextenum.py
# Compiled at: 2016-02-09 16:36:08
...
class ContextEnumValue(object):
    ...
    def __repr__(self):
        return (b'<enumvalue {}.{} ({})>').format(self.enum.name, self.name, self.id)
    ...
    def __moyaconsole__(self, console):
        console((b"<enumvalue '{}.{}' ({}))>").format(self.enum.name, self.name, self.id), bold=True, fg=b'magenta').nl()
...
class ContextEnum(object):
    ...
    def __repr__(self):
        return (b'<enum "{}">').format(self.name)

    def __moyaconsole__(self, console):
        console.text(repr(self), fg=b'green', bold=True)
        table = []
        for value in sorted(self._values, key=int):
            table.append([value.name, value.id, value.description or b''])
    ...
    def __getitem__(self, key):
        ...
        if enum_value is None:
            raise KeyError((b'no enum value {!r} in {!r}').format(key, self))
        return enum_value
...
if __name__ == b'__main__':
    enum = ContextEnum(b'moya.admin#enum.hobbits')
    enum.add_value(b'bilbo', description=b'Bilbo Baggins')
    enum.add_value(b'sam', description=b'Sam')
    enum.add_value(b'isembard', description=b'Isembard Took')
