...
# [GCC 8.4.0]
# Embedded file name: /Users/will/projects/moya/moya/context/expressionrange.py
# Compiled at: 2015-09-01 07:17:44
from __future__ import unicode_literals
from __future__ import print_function
from ..context.tools import to_expression
from ..compat import number_types, implements_to_string, xrange, implements_bool, string_types, unichr
__all__ = [
 b'ExpressionRange',
 b'ExclusiveIntegerRange',
 b'InclusiveIntegerRange',
 b'ExclusiveCharacterRange',
 b'InclusiveCharacterRange']
...
class ExpressionRange(object):
    ...
    def create(cls, context, start, end, inclusive=True):
        if not isinstance(start, number_types + string_types):
            if hasattr(start, b'__moyarange__'):
                moyarange = start.__moyarange__(context, end, inclusive=inclusive)
            else:
                raise ValueError((b"Can't create range from {!r} to {!r}").format(start, end))
            return moyarange
        ...
        raise ValueError((b"Can't create range from {} to {}").format(to_expression(context, start), to_expression(context, end)))
    ...
    def __init__(self, context, start, end):
        ...
        try:
            ...
            raise ValueError((b"Can't create range between {} and {}").format(to_expression(context, start), to_expression(context, end)))

    def __str__(self):
        return (b'<range {} to {} inclusive>').format(self.start, self.end)
    ...
    def __moyarepr__(self, context):
        if self.inclusive:
            return (b'({})..({})').format(to_expression(context, self.start), to_expression(context, self.end))
        else:
            return (b'({})...({})').format(to_expression(context, self.start), to_expression(context, self.end))
...
class ExclusiveIntegerRange(ExpressionRange):
    ...
    def __str__(self):
        return (b'<range {} to {} exclusive>').format(self.start, self.end)
