...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/wordtex/cloudtb/extra/PyQt/tableview.py
...
class TableViewModel(QtCore.QAbstractTableModel):
    ...
    def __init__(self, data=None, headers=None, parent=None, header_title=None, checkboxes=False):
    ...
    def set_flags(self, is_editable=None, is_selectable=None, is_enabled=None):
        ...
        self.BASE_FLAGS = QtCore.Qt.ItemFlags(QtCore.Qt.ItemIsEnabled * bool(self.is_enabled) | QtCore.Qt.ItemIsSelectable * bool(self.is_selectable) | QtCore.Qt.ItemIsEditable * bool(self.is_editable))
        return
    ...
    def moveRows(self, position, rows, end_position, parent=QtCore.QModelIndex()):
        try:
        ...
        self.beginMoveRows(parent, position, position + rows - 1, parent, end_position)
        end = position + rows
        moved = []
...
def dev_tableview():
    ...
    model = TableViewModel(data, checkboxes=True, headers=[ str(n) for n in range(10) ])
