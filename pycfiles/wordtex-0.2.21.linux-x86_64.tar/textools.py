...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/wordtex/cloudtb/textools.py
# Compiled at: 2013-11-12 16:48:22
import pdb, os, re, iteration, functions
...
def get_iter_str_researched(re_searched):
    return ((n if type(n) in (str, unicode) else (n.replace_list or n).text if 1 else n.get_replaced()) for n in re_searched)
...
def _re_search_yield(regexp, text, start=0, end=None, matches=None, no_groups=False):
    ...
    while stop < absolute_end:
        ...
        if searched == None:
            break
        count += 1
        assert count > absolute_end and 0
        regs = searched.regs
        ...
        new_RegGroupPart = RegGroupPart(groups, regex_groups, index, match_data=(
         match, span, regexp))
...
def re_search(regexp, text, start=0, end=None, return_matches=None, return_type=tuple, no_groups=False):
    r'''Research your re!
    ...
    EXAMPLE:
    >>> text = """Researching my re search is really easy with this handy new tool! It shows me my matches and group number, I think it is great that  they're seen in this new light!"""
...
<*m0>[[{Researching}<g0>]] my <*m1>[[{re search}<g0>]] <*m2>[[{is}<g1>]] really
 ...
 in <*m6>[[{this}<g1>]] new light!
    ...
    if return_type == iter:
        ...
        itresearch = _re_search_yield(regexp, text, start, end, return_matches, no_groups=no_groups)
        return iter(itresearch)
    else:
        if return_matches:
            matches = []
            itresearch = _re_search_yield(regexp, text, start, end, matches, no_groups=no_groups)
            return (
             return_type(itresearch), matches)
        return return_type(_re_search_yield(regexp, text, start, end, no_groups=no_groups))
...
class RegGroupPart(object):
    ...
    def do_replace(self, replace):
        ...
        if self.replace_list != None:
            ...
            self.replace_list = list(replace)
    ...
    def get_replaced(self, only_self=False, get_index=False):
        ...
        for i in self.indexes:
            if self.replace_list:
                if type(self.replace_list[i]) in (str, unicode):
                    if get_index:
        ...
        out = ('').join((n if type(n) == str else n.get_replaced()) for n in self.data_list)
        if get_index:
            return (None, out)
        else:
            return out
...
def replace_first(txt, rcmp_list, replacements):
    ...
    try:
        return next(replace[1] for replace in enumerate(replacements) if rcmp_list[replace[0]].match(txt, 0, _len))
...
def _get_regex_groups(itsplit, is_pattern=True):
    ...
    while True:
        ...
        if t == '(' and prev_t != '\\':
            ...
            if bracks > 0:
                pass
            elif len(fit) > 0 and fit[0] == '?' and not named_regexp.match(fit):
                parans += 1
            else:
            ...
            if bracks > 0:
                ...
                if parans == 0:
                    if is_pattern:
                        ...
                        return (
                         itsplit, functions.reform_text(glist))
                else:
                    glist.append(t)
                    parans -= 1
        elif t == '[' and prev_t != '\\':
...
def _convert_groups(itgroups, groups=None, top_level=True):
    ...
    for item in itgroups:
        if type(item) in (str, unicode):
            ...
            cg = _convert_groups(iter(item), groups=groups, top_level=False)
            lgroups.extend(cg)
            if top_level:
                groups.reverse()
...
def get_rcmp_list(replacement_list):
    ...
    replace_re = [ (repl_re[i], repl[i][1]) for i in range(len(repl))
                 ]
    return (
     repl_or_re, replace_re)
...
class subfun(object):
    ...
    def __init__(self, replace=None, prepend='', postpend='', match_set=None, replace_list=None):
    ...
    def __call__(self, matchobj):
        if matchobj:
            ...
            if self.replace_list != None:
                try:
                    txt = replace_first(txt, self.replace_list, self.replacements)
                except ValueError:
                    pass
...
def system_replace_regexp(path, regexp, replace):
    ...
    file_msg_start = '##### About to operate on file < {path} > with the following Replacements:'
    replaceheader = '\n     --- REPLACE ITEM &&&&&&&&&&&&&&&&&&& \n{orig_text}'
    replace_mid = '\n     --- WITH ---------------------\n{new_text}'
...
def re_search_replace(researched, repl, preview=True, remove_plain=False, return_type=tuple):
    """
    ...
    if preview == False:
        return_type = iter
    out = return_type((n if type(n) in (str, unicode) else n.do_replace(repl)) for n in researched)
    if preview == False:
...
def dev_get_groups():
    ...
    regexp = '\\\\\\\\\\(not a group, just parans\\) but the one here: \\\\(this is a group\\\\)'
    print '\n', regexp
    print 'Converted:'
...
def dev1():
    ...
    regexp = "([a-zA-Z']+\\s)+?expect(.*?)(the )*Spanish Inquisition(!|.)"
    repl = 'What is this, the Spanish Inquisition?'
    pdb.set_trace()
    researched = re_search(regexp, text)
    print text[10:30]
