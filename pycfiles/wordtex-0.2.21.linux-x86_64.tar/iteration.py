...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/wordtex/cloudtb/iteration.py
...
"""
import itertools as itools, math, sys
...
class biter(object):
    ...
    def __getitem__(self, item):
        if type(item) == int:
            if item < 0:
                raise IndexError('Cannot address biter with negative index: ' + repr(item))
            return next(itools.islice(self._iter, item, item + 1))
        if type(item) == slice:
            self._iter = itools.islice(self._iter, item.start, item.stop, item.step)
            return self._iter
...
class soliditer(object):
    ...
    def __init__(self, iterable, default_buf=10, request_extend_multiply=1, request_soft_limit=1000, request_hard_limit=None, slicetype=tuple):
    ...
    def internal_extend(self, need_length, want_length=None):
        ...
        wanted = needed if want_length == None else want_length - self.buffer_size()
...
def first_index_et(data_list, value, start=0, stop=None, step=1):
    ...
    try:
        if type(value) == float and math.isnan(value):
            ...
            isnan = math.isnan
            return next(data[0] for data in enumerate(data_list) if type(data[1]) in floats and isnan(data[1])) + start
        else:
            return next(data[0] for data in enumerate(data_list) if data[1] == value) + start
...
def first_index_in(data_list, in_set, start=0, stop=None, step=1):
    ...
    if step == None or step > 0:
        ...
        data_list = bslice(data_list, start, stop, step)
    try:
        index = next(data[0] for data in enumerate(data_list) if data[1] in in_set)
        return index + start
    except StopIteration:
        return
...
def first_index_nin(data_list, notin_set, start=0, stop=None, step=1):
    ...
    if step == None or step > 0:
        ...
        data_list = bslice(data_list, start, stop, step)
    try:
        index = next(data[0] for data in enumerate(data_list) if data[1] not in notin_set)
        return index + start
    except StopIteration:
        return
...
def first_index_is(data_list, identity, start=0, stop=None, step=1):
    if step == None or step > 0:
        ...
        data_list = bslice(data_list, start, stop, step)
    try:
        index = next(data[0] for data in enumerate(data_list) if data[1] is identity)
        return index + start
    except StopIteration:
        return
...
def first_index_nis(data_list, identity, start=0, stop=None, step=1):
    if step == None or step > 0:
        ...
        data_list = bslice(data_list, start, stop, step)
    try:
        index = next(data[0] for data in enumerate(data_list) if data[1] is not identity)
        return index + start
    except StopIteration:
        return
