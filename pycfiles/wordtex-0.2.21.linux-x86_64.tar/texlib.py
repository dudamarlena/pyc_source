...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/wordtex/texlib.py
...
class LatexError(ValueError):
    ...
    def __str__(self):
        msg = ('LaTeX LaTeX Error at line {0}: ').format(self.line) + repr(self.msg)
        return msg
...
def get_text_data(text_objects, texpart_constructor, return_first=False):
    ...
    return convert_inout(inout, texpart_constructor, return_first=return_first)
...
def print_tex_tree(texpart, tabs=0):
    ...
    if type(texpart) == str:
        ...
        print (' ||start:{0} |end:{1}').format(repr(texpart.start_txt), repr(texpart.end_txt))
        tabs += 1
    ...
    if len(inout) == 1:
        ...
        def get_processed(was_in, processing):
            if was_in > 0:
                ...
                converted = convert_processed(processing[0], processing[1:-1], processing[(-1)])
                return [
                 converted]
            else:
        ...
        for i, item in enumerate(inout):
            ...
            if indicator == 2 and prev_indicator == 0:
                ...
                next_processing = [
                 item]
            elif indicator == 2 and prev_indicator != 2:
...
class TexPart(object):
    ...
    def __init__(self, label=None, no_update_text=None, format_call=None, call_first=None, call_last=None, no_std_format=None, no_one_spaces=None, no_paragraphs=False, add_outside=None, no_outer_pgraphs=None, no_final_subs=None):
        r"""
    ...
    def update_match_re(self, match_re):
        ...
        self.cmp_inside, self.cmp_starters, self.cmp_end = [ [ re.compile(n) for n in c ] for c in self.match_re ]
    ...
    def reset_text(self):
        self.text_data = [
         self.get_original_text(no_start_stop=True)]
    ...
    def update_text(self, use_dict=None):
        ...
        if self.no_update_text:
            ...
            if not type(self.text_data) == list:
                raise AssertionError
                return self.text_data or None
