...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/topp/build/lib/create_skeleton.py
# Compiled at: 2007-09-27 13:07:36
import os, sys, StringIO, re, optparse, pkg_resources, subprocess
...
def create_skeleton(app_name=None, ignore_exists=False):
    ...
    reinterpretables = [
     'user', 'group', 'piddir', 'exec']
    for i in reinterpretables:
        context.globals[i] = '<<%s>>' % i

    context.globals['builder'] = 'TOPPbuild'
    context.globals['apps'] = 'app'
    task = Task('write the files for the skeleton directory', workdir='${cwd}', commands=[
     InFileWriterTree(fromdir=join(module_directory(appname('lib')), 'skel'), todir=skel_dir)])
    Software(task, context).install()
    return


dist = pkg_resources.get_distribution('topp.build.lib')
parser = optparse.OptionParser(usage='%prog APP_NAME', version='%s from %s' % (dist, dist.location), description='Create a new distribution with the given name')
parser.add_option('--add-to-svn', dest='add_to_svn', action='store_true', help='Add to the topp subversion repository')
TOPP_BASE_SVN = 'https://svn.openplans.org/svn/build/'
app_name_re = re.compile('^[a-z][a-z0-9_\\-.]+$')
...
def add_to_svn(build_name):
    ...
    trunk = base + '/trunk'
    dirs = [base, trunk, base + '/tags', base + '/branches']
    for dir in dirs:
        command = [
         'svn', 'mkdir', '-m', 'New build', dir]
        print (' ').join(command)
        subprocess.call(command)

    command = [
     'svn', 'co', trunk, build_name]
    print (' ').join(command)
    subprocess.call(command)
...
def add_all_to_svn(build_name):
    command = [
     'svn', 'add'] + [ os.path.join(build_name, fn) for fn in os.listdir(build_name) ]
    print (' ').join(command)
    subprocess.call(command)