...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/topp/build/lib/commands.py
...
class WorkingEnv(ShellCommand):
    ...
    def execute(self, task):
        ...
        command = [
         'workingenv', wenv_location]
        if self.options:
            for i in self.options:
                command.append(task.interpolate(i))

        if self.log_file:
            command.extend(['--log-file', task.interpolate(self.log_file)])
        process = subprocess.Popen(command)
        process.wait()
        if process.returncode:
            raise OSError('Command %r failed with exit code %s' % (command, process.returncode))
...
class TarballExtractor(ShellCommand):
    ...
    def execute(self, task):
        ...
        if is_url(tarfile):
            ...
            download = Download(filename=short_name, url=self.tarfile)
            if download.check(task):
                download.execute(task)
                tarfile = short_name
        if not os.path.exists(tarfile):
            return 1
        extdict = {'.tar.gz': 'zcat', '.tgz': 'zcat', '.tar.bz2': 'bunzip2 --stdout', '.tar.Z': 'uncompress -c'}
        for i in extdict:
            if tarfile.endswith(i):
                cmdline = extdict[i].split()
                cmdline.append(tarfile)
                decompress = subprocess.Popen(cmdline, stdout=subprocess.PIPE)
                stdin = decompress.stdout
                break
        else:
            cat = subproces.Popen(['cat', tarfile], stdout=subprocess.PIPE)
            stdin = cat.stdout

        cmdline = ['tar', 'xv']
        if self.path:
            cmdline += ['-C', self.path]
        untar = subprocess.Popen(cmdline, stdin=stdin, stdout=subprocess.PIPE)
        self.output = untar.communicate()[0]
        print self.output
        return untar.poll()
...
class EasyInstall(ShellCommand):
    ...
    def __init__(self, package, options=None, version=None):
        if options:
            opts = [ '--%s=%s' % (opt, val) for (opt, val) in options.items() ]
...
class InstallRequirements(ShellCommand):
    ...
    def execute(self, task):
        ...
        requirements_lines = workingenv.read_requirements(logger, requirements)
...
class InFileWriterTree(ShellCommand):
    ...
    def check(self, task):
        (fromdir, todir) = [ task.interpolate(i) for i in (self.fromdir, self.todir) ]
    ...
    def execute(self, task):
        ...
        for i in targets:
            ...
            filewriter = InFileWriter(infile=join(fromdir, i), outfile=join(todir, outfile))
