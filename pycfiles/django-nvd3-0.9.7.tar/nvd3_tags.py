...
# [GCC 8.4.0]
# Embedded file name: /home/areski/projects/nvd3/django-nvd3/django_nvd3/templatetags/nvd3_tags.py
# Compiled at: 2015-10-27 18:50:23
import collections
from django.template.defaultfilters import register
from django.utils.safestring import mark_safe
from django.conf import settings
from nvd3.NVD3Chart import NVD3Chart
from nvd3 import lineWithFocusChart, lineChart, multiBarChart, pieChart, stackedAreaChart, multiBarHorizontalChart, linePlusBarChart, cumulativeLineChart, discreteBarChart, scatterChart
...
def include_chart_jscss(static_dir='', css_dir='', js_dir=''):
    ...
    **Arguments**:
    ...
    chart = NVD3Chart()
    chart.header_css = [ '<link media="all" href="%s" type="text/css" rel="stylesheet" />\n' % h for h in ('%s%s' % (path, css_file) for css_file, path in css_files_dirs.items()) ]
    chart.header_js = [ '<script src="%s" type="text/javascript" charset="utf-8"></script>\n' % h for h in ('%s%s' % (path, js_file) for js_file, path in js_files_dirs.items()) ]
