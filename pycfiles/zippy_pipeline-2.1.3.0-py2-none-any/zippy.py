...
# [GCC 8.4.0]
# Embedded file name: zippy.py
# Compiled at: 2018-04-11 20:07:59
from __future__ import print_function
import re, json, copy, sys, subprocess, imp, os, types
...
def docker_task_reformat(params, current_stage_setup, command):
    ...
    if hasattr(stage_params, 'use_docker'):
        ...
        for m in docker_config.mount_points:
            mount_points.append(('{prefix} {mount_dir}').format(prefix=_get_docker_mount_prefix(docker_config), mount_dir=m))
            if getattr(docker_config, 'intercept_mounts', False):
                from_path, to_path = m.split(':')
                command = re.sub('(\\s+)' + from_path, '\\1' + to_path, command)
...
def _get_docker_run_command(docker_config, mount_points, command):
    if docker_config.engine == 'singularity':
        return ('singularity exec {mount_points} docker://{docker_image} {command}').format(mount_points=mount_points, command=command, docker_image=docker_config.image)
    else:
        if hasattr(docker_config, 'uid'):
            uid = docker_config.uid
        else:
            uid = subprocess.check_output('id -u', shell=True).strip()
        return ('docker run --rm {mount_points} --user={uid} --entrypoint=/bin/bash {docker_image} -c "{command}"').format(mount_points=mount_points, command=command, docker_image=docker_config.image, uid=uid)
