...
# [GCC 8.4.0]
# Embedded file name: /home/quejebo/zippy/zippy/zippy/star.py
# Compiled at: 2018-04-11 20:07:59
import os, re, random, string, subprocess
...
class starFlow(WorkflowRunner):
    ...
    def workflow(self):
        ...
        for sample in fastq_by_sample_1.keys():
            ...
            self.star_command = ('{star_path} --genomeDir {star_index}  --readFilesIn {read_files_string} --runThreadN {max_job_cores}  --outFileNamePrefix {out_dir}  --outTmpDir {tmp_dir} --readFilesCommand zcat --outSAMtype BAM SortedByCoordinate --outSAMunmapped Within').format(star_path=self.star_path, star_index=self.star_index, read_files_string=read_files_string, max_job_cores=self.max_job_cores, out_dir=sample_path + '/', tmp_dir=os.path.join(self.out_dir, 'tmp' + str(sample_id)))
...
class SingleStarFlow(WorkflowRunner):
    ...
    def workflow(self):
        ...
        self.flowLog(self.star_command)
        star_task = self.addTask('star', self.star_command, nCores=self.max_job_cores, memMb=102400, dependencies=None)
