...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/nlp/lang.py
# Compiled at: 2020-04-17 19:37:40
# Size of source mod 2**32: 8054 bytes
...
from zensols.actioncli import SingleClassConfigManager, Config, DelegateStash, StashFactory
from zensols.nlp import TokenFeatures, TokenNormalizer, SpacyFeatureNormalizer, NamedEntityRecognitionFeatureNormalizer, DependencyFeatureNormalizer, PartOfSpeechFeatureNormalizer
logger = logging.getLogger(__name__)

class DocUtil(object):
    __doc__ = 'Utility class to pretty print and serialize Spacy documents.\n\n    '
    ...
    def write(doc, writer=sys.stdout):
        ...
        writer.write('tokens:\n')
        for t in doc:
            writer.write(f"  {t}: tag={t.tag_}, pos={t.pos_}, " + f"stop={t.is_stop}, lemma={t.lemma_}, dep={t.dep_}\n")
    ...
    def to_json(doc):
        ...
        def tok_json(t):
            return {'tag':t.tag_, 
             'pos':t.pos_,  'is_stop':t.is_stop, 
             'lemma':t.lemma_,  'dep':t.dep_,  'text':t.text, 
             'idx':t.idx,  'ent':None if t.i not in em else em[t.i], 
...
class LanguageResource(object):
    __doc__ = "This langauge resource parses text in to Spacy documents.  It also uses the\n    textacy library to normalize text white sapce to generate better Spacy\n    document parses.\n\n    Don't create instances of this directly.  Instead use\n    ``LanguageResourceFactory`` created with application contexts with entries\n    like this:\n\n    [default_langres]\n    lang = en\n    model_name = ${lang}_core_web_sm\n\n    "

    def __init__(self, config: Config, model_name: str, lang: str='en', components: list=None, disable_components: list=None, token_normalizer: TokenNormalizer=None):
    ...
    def parse(self, text: str, normalize=False) -> Doc:
        ...
        """
        logger.debug(f"creating document with model: {self.model_name}, " + f"disable components: {self.disable_components}")
        if normalize:
            text = self.normalize(text)
        elif self.disable_components is None:
            doc = self.model(text)
        else:
    ...
    def parse_tokens(self, tokens: List[str]) -> Doc:
        ...
        for pipe in map(lambda n: self.model.get_pipe(n), self.model.pipe_names):
            pipe(doc)
    ...
    def features(self, doc: Doc):
        ...
        """
        return map(lambda t: TokenFeatures(doc, *t), self.token_normalizer.normalize(doc))
...
class DocStash(DelegateStash):
    __doc__ = 'A stash that transforms loaded items in to a SpaCy document.\n\n    All items returned from the delegate must have a ``text`` attribute or\n    override ``item_to_text``.\n\n    '

    def __init__(self, delegate, lang_res):
        """Initialize.
...
class LanguageResourceFactory(SingleClassConfigManager):
    __doc__ = 'Creates instances of ``LanguageResource`` from application context\n    configuration.  See that class for details.\n\n    '
    INSTANCE_CLASSES = {}

    def __init__(self, config, *args, **kwargs):
        (super(LanguageResourceFactory, self).__init__)(
 config, *args, cls=LanguageResource, pattern='{name}_langres', stash=None, **kwargs)