...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/nlp/feature.py
...
import logging, sys, inspect, re
from spacy.tokens.token import Token
logger = logging.getLogger(__name__)

class TokenAttributes(object):
    __doc__ = 'Contains token properties and a few utility methods.\n\n    '
    ...
    def write(self, writer=sys.stdout, level=0):
        ...
        sp = ' ' * level * 2
        fmt = '\n'.join(map(lambda x: '{}{}: {{{}}}'.format(sp, x, x), self.FIELDS))
        fmt += '\n'
...
class TokenFeatures(TokenAttributes):
    __doc__ = 'Convenience class to create features from text.  The features are derived\n    from parsed Spacy artifacts.\n\n    '
    NONE = '<none>'
    PROP_REGEX = re.compile('^[a-z][a-z_-]*')
    ...
    def detach(self, keeps=None) -> TokenAttributes:
        ...
        for p, v in inspect.getmembers(self, lambda x: not callable(x)):
            if self.PROP_REGEX.match(p):
                if not p not in skips or keeps is None or p in keeps:
                    attrs[p] = v
    ...
    def lemma(self) -> str:
        ...
        """
        if self.is_ent:
            return self.tok_or_ent.orth_
        return self.tok_or_ent.lemma_
