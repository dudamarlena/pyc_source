...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/marrow/util/compat.py
...
__all__ = [
 'formatdate', 'unquote', 'range', 'execfile', 'exception', 'binary',
...
if sys.version_info < (3, 0):
    ...
    def execfile(filename, globals_=None, locals_=None):
        ...
        if locals_ is None:
            locals_ = globals_
        exec (
         open(filename).read(), globals_, locals_)
        return
...
def exception(maxTBlevel=None):
    ...
    * args: arguments to the exception instance
    ...
    try:
        ...
        return Bunch(name=excName, cls=cls, exception=exc, trace=trbk, formatted=excTb, args=excArgs)
    finally:
        del cls
        del exc
        del trbk
...
def uvalues(a, encoding='utf-8', fallback='iso-8859-1'):
    ...
    try:
        return (
         encoding, [ s.decode(encoding) for s in a ])
    except UnicodeError:
        return (
         fallback, [ s.decode(fallback) for s in a ])
