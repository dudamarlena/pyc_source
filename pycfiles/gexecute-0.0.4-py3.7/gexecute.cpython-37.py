...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\gexecute.py
# Compiled at: 2020-01-21 09:18:01
...
def __get_module(module_name, package_path):
    if isinstance(module_name, str):
        if exists(join(package_path, module_name + '.py')):
    ...
    if not isinstance(module_name, str):
        ...
        if not isinstance(package_path, str):
            ...
            if not exists(join(package_path, module_name + '.py')):
...
def __subset_dictionary(dict_, subset_keys):
    if isinstance(dict_, dict) and not isinstance(subset_keys, list):
        if isinstance(subset_keys, set):
    ...
    if not isinstance(module, str):
        ...
        if not isinstance(package_path, str):
            ...
            if not exists(package_path):
                ...
                if not exists(join(package_path, module + '.py')):
    ...
    if isinstance(func, str):
        if not ismodule(module):
            if isinstance(package_path, str):
                ...
                if hasattr(__get_cur_module(), func):
...
def gexec(func, params, module=None, package_path=None):
    ...
    >>> gexec(func='__subset_dictionary', params={'dict_': {'a': 1, 'b': 2}, 'subset_keys': {'a'}},             module='gexecute', package_path=getcwd())
    {'a': 1}

    >>> gexec(func='__subset_dictionary',             params={'dict_': {'a': 1, 'b': 2}, 'subset_keys': {'a'}, 'junk': 'shouldn\'t show up'},             module='gexecute', package_path=getcwd())
    {'a': 1}
    """
    if not isinstance(params, dict):
        raise TypeError('{0} not supported for <params>! Needs to be a dictionary!'.format(type(params)))
    else:
        if (isinstance(module, str) or ismodule(module) or module) is not None:
            ...
            if not isinstance(package_path, str):
                ...
                func = isfunction(func) or __get_function_obj(func=func, module=module, package_path=package_path)
