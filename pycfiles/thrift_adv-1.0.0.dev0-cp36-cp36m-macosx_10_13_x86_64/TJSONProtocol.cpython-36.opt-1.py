...
# [GCC 8.4.0]
# Embedded file name: /thrift/protocol/TJSONProtocol.py
# Compiled at: 2018-09-11 21:54:05
# Size of source mod 2**32: 19365 bytes
...
class TJSONProtocolBase(TProtocolBase):
            ...
            if ord(character) == ESCSEQ0:
                ...
                if ord(character) == ESCSEQ1:
                    ...
                    if self._isHighSurrogate(codeunit):
                        if highSurrogate:
                            raise TProtocolException(TProtocolException.INVALID_DATA, 'Expected low surrogate char')
                        highSurrogate = codeunit
                        continue
                    else:
                        if self._isLowSurrogate(codeunit):
                            if not highSurrogate:
                                raise TProtocolException(TProtocolException.INVALID_DATA, 'Expected high surrogate char')
                ...
                if character in ESCAPE_CHAR_VALS:
                    raise TProtocolException(TProtocolException.INVALID_DATA, 'Unescaped control char')
                else:
                    if sys.version_info[0] > 2:
    ...
    def isJSONNumeric(self, character):
        if NUMERIC_CHAR.find(character) != -1:
            return True
        else:
            return False
    ...
    def readJSONDouble(self):
        ...
        if self.reader.peek() == QUOTE:
            ...
            try:
                double = float(string)
                if self.context.escapeNum is False:
                    if not math.isinf(double):
                        if not math.isnan(double):
...
class TJSONProtocol(TJSONProtocolBase):
    ...
    def readMessageBegin(self):
        ...
        if self.readJSONInteger() != VERSION:
            raise TProtocolException(TProtocolException.BAD_VERSION, 'Message contained bad version.')
...
class TSimpleJSONProtocol(TJSONProtocolBase):
    __doc__ = 'Simple, readable, write-only JSON protocol.\n\n    Useful for interacting with scripting languages.\n    '
