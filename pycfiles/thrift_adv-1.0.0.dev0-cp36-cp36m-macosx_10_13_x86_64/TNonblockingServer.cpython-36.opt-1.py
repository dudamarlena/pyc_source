...
# [GCC 8.4.0]
# Embedded file name: /thrift/server/TNonblockingServer.py
...
maximum connections
"""
import logging, select, socket, struct, threading
...
class Worker(threading.Thread):
    __doc__ = 'Worker is a small helper to process incoming connection.'
...
class Connection(object):
    ...
    def read(self):
        ...
        if not self.status in (WAIT_LEN, WAIT_MESSAGE):
            raise AssertionError
        elif not not self.received:
            raise AssertionError
        buf_size = 8192
        first = True
        done = False
    ...
    def write(self):
        ...
        if not self.status == SEND_ANSWER:
            raise AssertionError
        else:
    ...
    def ready(self, all_ok, message):
        ...
        """
        if not self.status == WAIT_PROCESS:
            raise AssertionError
        else:
            if not all_ok:
                self.close()
...
class TNonblockingServer(object):
    ...
    def _select(self):
        ...
        readable = [
         self.socket.handle.fileno(), self._read.fileno()]
        writable = []
        remaining = []
        ...
        for readable in rset:
            if readable == self._read.fileno():
                ...
                try:
                    ...
                    if client:
                        self.clients[client.handle.fileno()] = Connection(client.handle, self.wake_up)
                except socket.error:
