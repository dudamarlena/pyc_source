...
# [GCC 8.4.0]
# Embedded file name: /thrift/transport/TSSLSocket.py
# Compiled at: 2018-09-11 21:54:05
# Size of source mod 2**32: 15861 bytes
import logging, os, socket, ssl, sys, warnings
...
class TSSLBase(object):
    _has_ssl_context = sys.hexversion >= 34015728
    _has_ciphers = sys.hexversion >= 34013424
    ...
    def _deprecated_arg(self, args, kwargs, pos, key):
        if len(args) <= pos:
            return
        real_pos = pos + 3
        warnings.warn(('%dth positional argument is deprecated.please use keyward argument insteand.' % real_pos),
          DeprecationWarning, stacklevel=3)
        if key in kwargs:
            raise TypeError('Duplicate argument: %dth argument and %s keyward argument.' % (
             real_pos, key))
        kwargs[key] = args[pos]
    ...
    def __getattr__(self, key):
        if key == 'SSL_VERSION':
            warnings.warn('SSL_VERSION is deprecated.please use ssl_version attribute instead.',
              DeprecationWarning,
              stacklevel=2)
    ...
    def __init__(self, server_side, host, ssl_opts):
        ...
        if TSSLBase.SSL_VERSION != self._default_protocol:
            warnings.warn('SSL_VERSION is deprecated.please use ssl_version keyward argument instead.',
              DeprecationWarning,
              stacklevel=2)
        ...
        if self._should_verify:
            ...
            if not os.access(self.ca_certs, os.R_OK):
                raise IOError('Certificate Authority ca_certs file "%s" is not readable, cannot validate SSL certificates.' % self.ca_certs)
    ...
    def _wrap_socket(self, sock):
        if self._has_ssl_context:
            ...
            return self.ssl_context.wrap_socket(sock,
              server_side=(self._server_side), server_hostname=(self._server_hostname))
        else:
            ssl_opts = {'ssl_version':self._ssl_version, 
...
class TSSLSocket(TSocket.TSocket, TSSLBase):
    ...
    def __init__(self, host='localhost', port=9090, *args, **kwargs):
        ...
        if validate is not None:
            ...
            warnings.warn(('validate is deprecated. please use cert_reqs=ssl.%s instead' % cert_reqs_name),
              DeprecationWarning,
              stacklevel=2)
    ...
    @property
    def validate(self):
        warnings.warn('validate is deprecated. please use cert_reqs instead', DeprecationWarning,
          stacklevel=2)
        return self.cert_reqs != ssl.CERT_NONE
...
class TSSLServerSocket(TSocket.TServerSocket, TSSLBase):
    __doc__ = "SSL implementation of TServerSocket\n\n    This uses the ssl module's wrap_socket() method to provide SSL\n    negotiated encryption.\n    "
        ...
        if args:
            ...
            if self._should_verify:
                if not _match_has_ipaddress:
                    raise ValueError('Need ipaddress and backports.ssl_match_hostname module to verify client certificate')
    ...
    def setCertfile(self, certfile):
        ...
        """
        warnings.warn('setCertfile is deprecated. please use certfile property instead.',
          DeprecationWarning,
          stacklevel=2)
        ...
        try:
            ...
            if self._should_verify:
                ...
                try:
                    ...
                    logger.warn('Failed to validate client certificate address: %s', (addr[0]),
                      exc_info=True)
                    client.close()
                    plain_client.close()
                    return
