...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.5/site-packages/pyVC/VirtualMachines/QEMU.py
...
class VM(Base.VM):
    ...
    macprefix = '52:54:00'
    macs = MacGenerator(macprefix)
    errors = ('no_qemu_executable', 'no_qemu_lib_path')
    platforms = ()

    def __init__(self, realmachine, name, graphic=False, snapshot=True, **keywords):
        Base.VM.__init__(self, realmachine, name, **keywords)
        if graphic == 'false' or graphic == '0' or not graphic:
            self.__graphic = False
        else:
            self.__graphic = True
        if snapshot == 'false' or snapshot == '0' or not snapshot:
            self.__snapshot = False
        else:
            self.__snapshot = True
        self.__pid = None
        from pyVC.errors import VMError
        from pyVC.Disks import ISO, Floppy
        if len([ disk for disk in self.disks if not (isinstance(disk, ISO.Disk) or isinstance(disk, Floppy.Disk))
               ]) > 4:
            raise VMError, (
             'Error: Too many disks for QEMU VM,                     QEMU can only access 4 disk images concurrently.',
             2,
             self.realmachine.hostname,
             self.vmname)
        if len([ disk for disk in self.disks if isinstance(disk, ISO.Disk) ]) > 1:
            raise VMError, (
             'Error: Too many ISO images for QEMU VM,                     QEMU can only access 1 cd image concurrently.',
             2,
             self.realmachine.hostname,
             self.vmname)
        if len([ disk for disk in self.disks if isinstance(disk, Floppy.Disk) ]) > 2:
            raise VMError, (
             'Error: Too many floppy images for QEMU VM,                     QEMU can only access 2 floppy images concurrently.',
             2,
             self.realmachine.hostname,
             self.vmname)
        return
    ...
    def start(self):
        ...
        available_disks = [
         'hda', 'hdb', 'hdc', 'hdd']
        available_floppies = ['fda', 'fdb']
        for disk in self.disks:
            if isinstance(disk, Floppy.Disk):
                disk_command += disk.qemu(available_floppies) + ' '
            else:
        ...
        if self.kernel:
            ...
            if isinstance(self.bootdisk, Floppy.Disk):
                kernel_command = '-boot a'
            elif isinstance(self.bootdisk, ISO.Disk):
                kernel_command = '-boot d'
            else:
        ...
        if self.networks:
            for network in self.networks:
                ...
                if tmp_pre_command not in pre_command:
                    pre_command.append('%s ' % tmp_pre_command)
                if isinstance(network, VDE1.Network) and not pre_vde_network_command:
    ...
    def console(self, socket=False):
        ...
        if self.__pid:
            if socket:
                return (
                 self.realmachine.hostname, self.realmachine.term_socket(self.__pid))
            else:
