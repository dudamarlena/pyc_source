...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.5/site-packages/pyVC/VirtualMachines/UML.py
...
class VM(Base.VM):
    ...
    def __init__(self, realmachine, name, kernel=None, **keywords):
        Base.VM.__init__(self, realmachine, name, kernel=kernel, **keywords)
        self.__pid = None
        self.__interfaces = [ 'eth%s' % ifid for ifid in range(0, 10) ]
        if len(self.disks) > 8:
            raise VMError, (
             'Error: Too many disks for UML VM,                     UML can only access 8 disk images concurrently.',
             3,
