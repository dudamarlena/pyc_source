...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.5/site-packages/pyVC/Disks/ISO.py
...
class Disk(Base.Disk):
        ...
        for realmachine in realmachines:
            if not realmachine.isfile(self.path):
                if 'images_path' in realmachine.config['global'] and not realmachine.isfile(realmachine.config['global']['images_path'] + '/' + self.path):
                    raise DiskError, ('Could not open ISO image %s.' % self.path,
                     0,
    ...
    def qemu(self, disks):
        return '-cdrom %s' % self.path

    def uml(self, disks):
        return '%sr=%s' % (disks.pop(0), self.path)

    def xen(self, disks):
        from lxml.etree import Element, SubElement
        disk = Element('disk', type='file')
        SubElement(disk, 'source', file=str(self.path))
        SubElement(disk, 'target', dev='hdc:cdrom')
        return disk
