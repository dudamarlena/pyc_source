...
# [GCC 8.4.0]
# Embedded file name: /home/cw12401/code/work/isambard/src/isambard/modelling/extract_loop_data.py
# Compiled at: 2018-06-29 06:01:24
# Size of source mod 2**32: 9557 bytes
...
def gather_loops_from_pdb(path: str) -> List[dict]:
    ...
    resolution_match = re.search('RESOLUTION\\.\\s+([0-9\\.]+)', pdb_str)
    ...
    for polypeptide in assembly:
        combined_loop_data.extend(extract_data_for_loops(polypeptide, pdb_code, resolution))

    return combined_loop_data


def extract_data_for_loops(polypeptide: ampal.Polypeptide, pdb_code: str, resolution: float, create_geometry_path: Optional[str]=None) -> List[dict]:
    ...
    loop_regions = [r for p, r in zip(pattern_groups, region_groups) if p[1] == 'l']
    ...
    for entering_reg, loop_reg, exiting_reg in loop_regions:
        loop_data, geometry = create_loop_dict(polypeptide, pp_primitive, entering_reg, loop_reg, exiting_reg)
        loop_data['pdb_code'] = pdb_code
        loop_data['resolution'] = resolution
        loops.append(loop_data)
        if create_geometry_path:
            loop_geometry.append(loop_geometry_ampal(geometry['first_primitive'], geometry['entering_primitive'], geometry['exiting_primitive'], geometry['last_primitive']))
...
def create_loop_dict(polypeptide: ampal.Polypeptide, pp_primitive: ampal.Primitive, entering_reg: str, loop_reg: str, exiting_reg: str):
    ...
    loop_data = {'loop_type':entering_reg[2] + exiting_reg[2], 
    ...
    return (
     loop_data, loop_geometry)


def calculate_loop_geometry(entering_prims: ampal.Primitive, exiting_prims: ampal.Primitive) -> dict:
    ...
    loop_geometry = {'first_primitive':entering_prims[0]['CA']._vector, 
...
def loop_geometry_ampal(first_primitive: numpy.array, entering_primitive: numpy.array, exiting_primitive: numpy.array, last_primitive: numpy.array) -> ampal.Polypeptide:
