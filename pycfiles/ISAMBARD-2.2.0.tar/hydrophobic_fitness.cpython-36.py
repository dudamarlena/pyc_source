...
# [GCC 8.4.0]
# Embedded file name: /home/cw12401/code/work/isambard/src/isambard/evaluation/hydrophobic_fitness.py
# Compiled at: 2018-05-03 11:09:05
...
def calculate_hydrophobic_fitness(assembly):
    ...
    tyrosine_centroids = []
    polar_centroids = []
    for residue in [r for r in assembly.get_monomers() if isinstance(r, ampal.Residue)]:
        centroid_list = None
        centroid = residue.centroid
        if residue.mol_letter in HYDROPHOBIC:
            centroid_list = hydrophobic_centroids
        else:
            if residue.mol_letter == 'Y':
    ...
    hf = run_hf_loop(hydrophobic_centroids, tyrosine_centroids, polar_centroids)
    return hf
...
def run_hf_loop(hydrophobic_centroids, tyrosine_centroids, polar_centroids):
    ...
    for reference in hydrophobic_centroids:
        h_centroids_in_7_3, h_centroids_in_10, h_neighbours = get_number_within(reference, hydrophobic_centroids)
        y_centroids_in_7_3, y_centroids_in_10, y_neighbours = get_number_within(reference, tyrosine_centroids)
        p_centroids_in_7_3, p_centroids_in_10, p_neighbours = get_number_within(reference, polar_centroids)
        all_neighbours = h_neighbours + y_neighbours + p_neighbours
        ci = h_centroids_in_7_3 + y_centroids_in_7_3 + p_centroids_in_7_3 - all_neighbours
        hi = n_hydrophobic - h_neighbours + (n_tyrosine - y_neighbours)
        Ni = hi + (n_polar - p_neighbours)
        Hic = ci * (hi / Ni)
        Hi = h_centroids_in_7_3 + y_centroids_in_7_3 - (h_neighbours + y_neighbours)
        hydrophobic_scores.append(Hi - Hic)
        burial_scores.append(h_centroids_in_10 + y_centroids_in_10 + p_centroids_in_10)

    hydrophobic_fitness = -1 * (sum(burial_scores) * sum(hydrophobic_scores) / (n_hydrophobic + n_tyrosine) ** 2)
...
def get_number_within(reference, target_points):
    ...
    for target in target_points:
        if target is reference:
            ...
            if reference[0] == target[0]:
                if reference[1] - 1 == target[1] or reference[1] + 1 == target[1]:
                    neighbours += 1
    ...
    return (
     centroids_in_7_3, centroids_in_10, neighbours)
