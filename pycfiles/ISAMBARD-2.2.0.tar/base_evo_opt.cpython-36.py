...
# [GCC 8.4.0]
# Embedded file name: /home/cw12401/code/work/isambard/src/isambard/optimisation/base_evo_opt.py
# Compiled at: 2018-04-19 09:01:18
# Size of source mod 2**32: 15811 bytes
"""Base class for bio-inspired optimizers."""
from concurrent import futures
import datetime, enum, os, sys, matplotlib.pylab as plt, numpy
...
class BaseOptimizer:
    __doc__ = 'Abstract base class for the evolutionary optimizers.\n\n    Notes\n    -----\n    Not intended to be used directly, see the evolutionary\n    optimizers for full documentation.\n    '

    def __init__(self, specification, sequences, parameters, build_fn=None, eval_fn=None, **kwargs):
        ...
        if sys.platform == 'win32':
            ...
            print('Multiprocessing for this module is currently unavailableon Windows, only a single process will be used.')
        else:
    ...
    def run_opt(self, pop_size, generations, cores=1, plot=False, log=False, log_path=None, run_id=None, store_params=True, **kwargs):
        ...
        for g in range(generations):
            ...
            (self.logbook.record)(gen=g, evals=self._evals, **self.stats.compile(self.population))
            print(self.logbook.stream)

        end_time = datetime.datetime.now()
        time_taken = end_time - start_time
        print('Evaluated {} models in total in {}'.format(self._model_count, time_taken))
    ...
    def _make_parameters(self):
        ...
        for parameter in self.parameters:
            if parameter.type == ParameterType.DYNAMIC:
                ...
                if parameter.type == ParameterType.STATIC:
    ...
    def assign_fitnesses(self, targets):
        ...
        px_parameters = zip([self.specification] * len(targets), [
         self.sequences] * len(targets), [self.parse_individual(x) for x in targets])
        if self._cores == 1 or self.mp_disabled:
            models = map(self.build_fn, px_parameters)
            fitnesses = map(self.eval_fn, models)
        else:
            with futures.ProcessPoolExecutor(max_workers=(self._cores)) as (executor):
    ...
    def log_results(self, output_path=None, run_id=None):
        ...
        model_params = self.parse_individual(best_ind)
        if output_path is None:
            output_path = os.getcwd()
        if run_id is None:
            run_id = '{:%Y%m%d-%H%M%S}'.format(datetime.datetime.now())
        with open('{0}/{1}_opt_log.txt'.format(output_path, run_id), 'w') as (log_file):
            log_file.write('\nEvaluated {0} models in total\n'.format(self._model_count))
        ...
        with open('{0}/{1}_opt_best_model.pdb'.format(output_path, run_id), 'w') as (output_file):
    ...
    def make_energy_funnel_data(self, cores=1):
        ...
        if not self.parameter_log:
            ...
            if cores == 1 or sys.platform == 'win32':
                energy_rmsd_gen = map(self.funnel_rebuild, [(x, top_result_model, self.specification) for x in gen_tagged])
            else:
                with futures.ProcessPoolExecutor(max_workers=cores) as (executor):
                    energy_rmsd_gen = executor.map(self.funnel_rebuild, [(x, top_result_model, self.specification) for x in gen_tagged])
...
class ParameterType(enum.IntEnum):
    __doc__ = 'Type of parameter used in evolutionary optimizers.'
    STATIC = 1
    DYNAMIC = 2


class Parameter:
    __doc__ = 'Defines a parameter used in the evolutionary optimizers.'
    ...
    def default_value(self):
        if self.type == ParameterType.STATIC:
            return self.value
        if self.type == ParameterType.DYNAMIC:
            return self.value[0]
