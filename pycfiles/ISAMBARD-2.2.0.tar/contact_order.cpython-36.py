...
# [GCC 8.4.0]
# Embedded file name: /home/cw12401/code/work/isambard/src/isambard/evaluation/contact_order.py
# Compiled at: 2018-04-26 05:23:20
...
def calculate_contact_order(polypeptide):
    ...
    if not isinstance(polypeptide, ampal.Polypeptide):
        raise ValueError('Contact order can only be calculated for a Polypeptide chain.')
    budeff.assign_force_field(polypeptide, budeff.FORCE_FIELDS['bude_2016v1'])
    contacts = {(int(a.parent.id), int(b.parent.id)) for a, b in budeff.find_intra_ampal(polypeptide, 18.0) if ampal.geometry.distance(a, b) <= 4.0 if ampal.geometry.distance(a, b) <= 4.0}
    d_z_values = map(lambda x: x[1] - x[0] - 1, contacts)
    try:
        contact_order = 1 / (len(contacts) * len(polypeptide)) * sum(d_z_values)
    except ZeroDivisionError:
        raise ValueError('Number of contacts ({}) or length of polypeptide ({}) is 0 so contact order cannot be calculated.'.format(len(contacts), len(polypeptide)))
