...
# [GCC 8.4.0]
# Embedded file name: /home/cw12401/code/work/isambard/src/isambard/modelling/scwrl.py
# Compiled at: 2018-06-29 04:07:38
# Size of source mod 2**32: 6170 bytes
...
.. [1] Krivov GG, Shapovalov MV, and Dunbrack Jr RL (2009) "Improved
   ...
   Proteins.
"""
import os, subprocess, tempfile, re, ampal
...
def scwrl_available():
    ...
    try:
        ...
        print('Scwrl4 has not been found on your path. If you have already installed Scwrl but are unsure how to add it to your path, check out this: https://stackoverflow.com/a/14638025')
...
def run_scwrl(pdb, sequence, path=True, rigid_rotamer_model=True, hydrogens=False):
    ...
    try:
        with tempfile.NamedTemporaryFile(delete=False) as (scwrl_tmp):
            with tempfile.NamedTemporaryFile(delete=False) as (scwrl_seq):
                with tempfile.NamedTemporaryFile(delete=False) as (scwrl_out):
                    scwrl_tmp.write(pdb)
                    scwrl_tmp.seek(0)
    ...
    return (
     scwrl_std_out.decode(), scwrl_pdb.decode())
...
def pack_side_chains_scwrl(assembly, sequences, rigid_rotamer_model=True, hydrogens=False):
    ...
    if not scwrl_available():
        ...
        if total_seq_len != total_aa_len:
            raise ValueError('Total sequence length ({}) does not match total Polypeptide length ({}).'.format(total_seq_len, total_aa_len))
        if len(protein) != len(sequences):
            raise ValueError('Number of sequences ({}) does not match number of Polypeptides ({}).'.format(len(sequences), len(protein)))
    scwrl_std_out, scwrl_pdb = run_scwrl((assembly.pdb),
      (''.join(sequences)), path=False, rigid_rotamer_model=rigid_rotamer_model,
      hydrogens=hydrogens)
