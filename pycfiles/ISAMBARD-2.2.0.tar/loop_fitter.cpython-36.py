...
# [GCC 8.4.0]
# Embedded file name: /home/cw12401/code/work/isambard/src/isambard/modelling/loop_fitter.py
# Compiled at: 2018-06-29 05:37:14
# Size of source mod 2**32: 12307 bytes
"""A module for finding and fitting loops regions to a model."""
import copy, pathlib
from typing import List
import numpy
from ampal import geometry, Polypeptide, Residue, load_pdb
...
def align_loop(entering_residue: Residue, exiting_residue: Residue, loop: Loop) -> Polypeptide:
    ...
    translation, angle, axis, point = geometry.find_transformations(loop.get_entering_primitive(), loop.get_exiting_primitive(), loop_geometry['entering_primitive'], loop_geometry['exiting_primitive'])
    quaterion = geometry.Quaternion.angle_and_axis(angle=angle,
      axis=axis,
      radians=False)
    ...
    rot_dihedral = geometry.dihedral(loop_geometry['first_primitive'], loop_geometry['entering_primitive'], loop_geometry['exiting_primitive'], first_prim)
    loop_pp.rotate(rot_dihedral, loop_geometry['exiting_primitive'] - loop_geometry['entering_primitive'], loop_geometry['entering_primitive'])
...
def fit_loop(entering_residue: Residue, exiting_residue: Residue, loop_pp: Polypeptide, fitting_rounds: int=1000, temp: float=298.15, max_dist_step: float=1.0, max_angle_step: float=10.0, verbose=True) -> Polypeptide:
    ...
    fitter.start_optimisation(fitting_rounds, max_angle_step, max_dist_step, temp=temp,
      verbose=verbose)
...
def merge_loop(entering_residue: Residue, exiting_residue: Residue, loop: Polypeptide, use_loop_flanking: bool=False) -> Polypeptide:
    ...
    merged_polypeptide.tags = {**(loop.tags), **(entering_pp.tags), **(exiting_pp.tags)}
