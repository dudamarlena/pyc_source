...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\sy85\bulktypes.py
...
class SY85BulkDump(object):
    ...
    def dump(self, device_id=0):
        ...
        datalen_lsb = chr(datalen & 127)
        datalen_msb = chr(datalen >> 7 & 127)
        checksum = chr(self._create_checksum(self.data))
        return SOX + MANUFACTURER_ID + chr(device_id) + BULKDATATYPES[self.type]['msg_fmt_id'] + datalen_msb + datalen_lsb + self.data + checksum + EOX
    ...
    def data(self):
        if self._name_offset:
            ...
            return self._data[:name_offset] + name + self._data[name_offset + self._name_len:]
        else:
            return self._data
...
class SY85AbstractVoice(SY85Patch):
    ...
    def data(self):
        ...
        return data[:30] + chr(self.bank_number) + chr(self.program_number) + data[32:]
...
class SY85Multi(SY85Patch):
    ...
    def __repr__(self):
        return super(SY85Multi, self).__repr__() + ' Slot: %i' % self.program_number
...
class SY85Sample(SY85Patch):
    ...
    def __repr__(self):
        return super(SY85Sample, self).__repr__() + ' Slot: %i' % self.program_number