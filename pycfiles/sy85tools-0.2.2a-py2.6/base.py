...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\sy85\converters\base.py
...
def read_bew(value):
    u"""Read string as big endian word.
...
def s_signed(value, minval=-127, maxval=127, *args):
    value = read_midi_short(value)
    value = -(value & 127) if value & 128 else value & 127
    if minval < value > maxval:
        raise ValueError('Value not in range: %i' % value)
    return value
