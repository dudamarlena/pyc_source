...
def save_chunk_to_file(chunk, filename, overwrite=False):
    ...
    if not os.path.exists(filename) or overwrite:
        try:
            ...
            logging.error("Could not open file '%s' for writing: %s", filename, exc)
        else:
            fo.write(chunk.dump())
            fo.close()
    else:
...
def do_help(args, options=None):
    ...
    try:
        ...
        print '\nUse option --help for general help on available options and their explanation.'
    except IndexError:
...
def do_list(args, options):
    ...
    if options.outfile:
        if not os.path.exists(options.outfile) or options.overwrite:
            try:
                ...
                logging.error("Could not open file '%s' for writing.", options.outfile)
                return 1

        else:
            logging.error("File '%s' already exists. Use -f to overwrite.", options.outfile)
            return 1
    else:
        outfile = sys.stdout
    if options.voices:
    ...
    for fn in args:
        ...
        for (i, chunk) in enumerate(syx.chunks):
            ...
            if options.csv:
                csv_values = [
                 bn, i, len(chunk)] + [ getattr(chunk, n, None) for n in [ x[0] for x in csv_fields ] if n ]
...
def do_rename(args, options):
    ...
    sy85sxtool [-f|--force] [--strict] [-o|--output=FILE] rename                                         FILE [CHUNK-NUMBER|OLDNAME] NEWNAME
    ...
    if not len(args) >= 2:
        ...
        if syx:
            ...
            if options.outfile and os.path.exists(options.outfile) and not options.overwrite:
                loggin.error("File '%s' already exists. Not overwriting.", options.outfile)
            else:
...
ACTIONS = dict(explode=do_explode, help=do_help, list=do_list, rename=do_rename)

def main(args):
    optparser = OptionParser(prog=__program__, usage=__usage__, version=__version__, description=__doc__)
    optparser.add_option('-v', '--verbose', action='store_true', dest='verbose', default=False, help="Print what's going on to stdout.")
    optparser.add_option('--strict', action='store_true', dest='strict', help='Use strict sysex parsing and abort on parsing errors.')
    optparser.add_option('-o', '--output', metavar='PATH', dest='outfile', help='Write output to PATH.')
    optparser.add_option('-f', '--force', action='store_true', dest='overwrite', help='Overwrite existing files.')
    optparser.add_option('--csv', action='store_true', dest='csv', help='Format chunk list as comma-separated values (CVS).')
    optparser.add_option('--voices', action='store_true', dest='voices', help='List only voices.')
