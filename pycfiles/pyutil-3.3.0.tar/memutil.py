...
# [GCC 8.4.0]
# Embedded file name: /home/user/code/pyutil/pyutil/memutil.py
...
def estimate_mem_of_obj(o):
    ...
    if hasattr(o, '__len__'):
        ...
        if isinstance(o, (dict, set)):
            return PY_STRUCT_HEAD_LEN + o.__len__() * 4 * 2 * 2
    if isinstance(o, int):
    ...
    if isinstance(o, long):
        ...
        return PY_STRUCT_HEAD_LEN + math.log(o) / 5
...
def measure_obj_leakage(f, numsamples=128, iterspersample=16, *args, **kwargs):
    """
...
def measure_ref_leakage(f, numsamples=128, iterspersample=16, *args, **kwargs):
    """
...
def get_mem_used():
    ...
    if not a:
        raise NotSupportedException
    return (
     int(a[1]) * resource.getpagesize(), int(a[0]) * resource.getpagesize())
...
def get_mem_usage_virt_and_res():
    ...
    if not len(a) > 1:
        raise NotSupportedException
    return (
     int(a[0]) * resource.getpagesize(), int(a[1]) * resource.getpagesize())


class Measurer(object):

    def __init__(self, f, numsamples=128, iterspersample=16, *args, **kwargs):
        """
    ...
    def start(self):
        self.resiters = [
         None] * self.numsamples
        self.resmemusage = [None] * self.numsamples
        self.totaliters = 0
        self.i = 0
        self.j = 0
        self._invoke()
...
def measure_mem_leakage(f, numsamples=128, iterspersample=16, *args, **kwargs):
    """
