...
# [GCC 8.4.0]
# Embedded file name: /home/user/code/pyutil/pyutil/test/out_of_shape/test_cache.py
...
class Bencher:
    ...
    def __init__(self, klass, MAXREPS=256, MAXTIME=5):
        print klass
        self.klass = klass
 ...
 128, 250, 2048, 5000, 8192, 1048576)):
        from pyutil import benchutil
        funcs = ('_benchmark_insert', '_benchmark_init_and_has_key_and_del', '_benchmark_init_and_remove',
                 '_benchmark_init_and_popitem', '_benchmark_update', '_benchmark_init')
        max = 0
        for func in funcs:
            if len(func) > max:
                max = len(func)
...
def quick_bench():
    Bencher(cache.LRUCache, MAXTIME=2).bench(BSIZES=(128, 4096, 16384, 32768, 65536))
    Bencher(cache.LinkedListLRUCache, MAXTIME=2).bench(BSIZES=(128, 4096, 16384, 32768))
    Bencher(cache.SmallLRUCache, MAXTIME=2).bench(BSIZES=(128, 4096, 16384, 32768))
...
class Testy(unittest.TestCase):
    ...
    def _test_LRU_has_key(self, C):
        ...
        self.failUnless(2 not in c.values())
    ...
    del test_mem_leakage

    def _test_mem_leakage(self):
        memutil.measure_mem_leakage(self.test_em, max(8, SAMPLES / 8), iterspersample=1)
        slope = memutil.measure_mem_leakage(self.test_em, max(8, SAMPLES / 8), iterspersample=1)
    ...
    del test_mem_leakage_much_adding_some_removing

    def _test_mem_leakage_much_adding_some_removing(self):
        memutil.measure_mem_leakage(self._mem_test_LRU_much_adding_some_removing, SAMPLES, iterspersample=1)
        slope = memutil.measure_mem_leakage(self._mem_test_LRU_much_adding_some_removing, SAMPLES, iterspersample=1)
    ...
    del test_obj_leakage

    def _test_obj_leakage(self):
        memutil.measure_obj_leakage(self.test_em, max(8, SAMPLES / 8), iterspersample=1)
        slope = memutil.measure_obj_leakage(self.test_em, max(8, SAMPLES / 8), iterspersample=1)
    ...
    del test_obj_leakage_much_adding_some_removing

    def _test_obj_leakage_much_adding_some_removing(self):
        memutil.measure_obj_leakage(self._mem_test_LRU_much_adding_some_removing, SAMPLES, iterspersample=1)
        slope = memutil.measure_obj_leakage(self._mem_test_LRU_much_adding_some_removing, SAMPLES, iterspersample=1)
