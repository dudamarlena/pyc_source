...
# [GCC 8.4.0]
# Embedded file name: /home/user/code/pyutil/pyutil/cache.py
...
class LRUCache():
    ...
    def freshen(self, k, strictkey=False):
        if not self._assert_invariants():
            ...
            if self.d.has_key(k) or strictkey:
                raise KeyError, k
            return
        node = self.d[k]
...
class SmallLRUCache(dict):
    ...
    def refresh(self, key, strictkey=True):
        ...
        if not self._assert_invariants():
            raise AssertionError
            if dict.has_key(self, key) or strictkey:
                raise KeyError, key
            return
        self._lru.remove(key)
        self._lru.append(key)
