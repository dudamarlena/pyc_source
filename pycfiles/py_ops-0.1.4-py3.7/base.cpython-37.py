...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pyops\base.py
# Compiled at: 2020-04-17 12:12:26
...
def cls_verify(cls, module, req):
    ...
    if cls_info.get('name') != cls:
        ...
        if req.config.getoption('--force_run', None):
            return cls_info
        if cls_info.get('disable', False):
...
def get_func(main, key):
    if not main:
        ...
        keys = [
         'setup', 'teardown', 'data', 'flow', 'check', 'setup_class', 'teardown_class']
        if key not in keys:
