...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pyops\libs.py
...
def collect_func(case_py, package=None):
    ...
    for m in case_py:
        ...
        for mem in all_mem:
            ...
            if isfunction(func):
                if hasattr(func, 'ah_type') and set(func.ah_type) & {'data', 'flow', 'check', 'setup', 'teardown', 'setup_class', 'teardown_class'}:
...
def collect_json(target_dir=None):
    ...
    if not case_json:
        raise ValueError('EEEEEEEEE..there is no json file found..EEEEEEEEE')
    return case_json


def collect_all():
    return {'case_json':collect_json(), 
