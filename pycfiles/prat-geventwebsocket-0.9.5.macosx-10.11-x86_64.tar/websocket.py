...
# [GCC 8.4.0]
# Embedded file name: /usr/local/Cellar/python/2.7.10_2/Frameworks/Python.framework/Versions/2.7/lib/python2.7/site-packages/geventwebsocket/websocket.py
...
class WebSocket(object):
    ...
    """
    __slots__ = ('utf8validator', 'utf8validate_last', 'environ', 'closed', 'stream',
                 'raw_write', 'raw_read', 'handler')
    ...
    def validate_utf8(self, payload):
        ...
        if not self.utf8validate_last[0]:
            raise UnicodeError(('Encountered invalid UTF-8 while processing text message at payload octet index {0:d}').format(self.utf8validate_last[3]))
    ...
    def read_message(self):
        ...
        while True:
            ...
            if f_opcode in (self.OPCODE_TEXT, self.OPCODE_BINARY):
                if opcode:
                    raise ProtocolError(('The opcode in non-fin frame is expected to be zero, got {0!r}').format(f_opcode))
    ...
    def close(self, code=1000, message=''):
        ...
        try:
            try:
                message = self._encode_bytes(message)
                self.send_frame(struct.pack(six.b('!H%ds' % len(message)), code, message), opcode=self.OPCODE_CLOSE)
            except WebSocketError:
...
class Header(object):
    ...
    def __repr__(self):
        return ('<Header fin={0} opcode={1} length={2} flags={3} at 0x{4:x}>').format(self.fin, self.opcode, self.length, self.flags, id(self))
