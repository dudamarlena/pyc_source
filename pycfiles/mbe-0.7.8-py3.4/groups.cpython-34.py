...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mbe/groups.py
...
def aop_has_right(_rights_function):
    """

    A decorator for to supply the has_right functionality. Takes a function pointer.
    It is necessary to make it able to get the data in runtime.
    TODO: This is quite ugly.

    :param _rights_function: A pointer to a function that returns the right to check for
    ...
    def wrapper(func):
        ...
        def wrapped_f(*args, **kwargs):
            ...
            try:
                ...
                raise Exception('Authentication aspect for "' + func.__name__ + '": No _user parameter in function.')

            if user_idx > len(args) - 1:
                raise Exception('Authentication aspect for "' + func.__name__ + '": The _user parameter isn\'t supplied.')
            has_right(_rights_function(), args[user_idx])
...
class Groups:
    ...
    def reload_all(self):
        ...
        """
        self._groups.clear()
        _cursor = self._database_access.find({'collection': 'node',  'conditions': {'schemaId': '5d3b8e21-adf3-4005-b73c-1fb63a46f399'}})
