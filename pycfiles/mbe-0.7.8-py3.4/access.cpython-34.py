...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mbe/access.py
# Compiled at: 2015-09-01 20:16:32
# Size of source mod 2**32: 11062 bytes
"""
...
class DatabaseAccess:
    ...
    def __init__(self, _database, _json_schema_folders=None, _uri_handlers=None):
        """
    ...
    def verify_document(self, _data, _collection_name, _caller_name):
        ...
        if 'schemaId' in _data:
            try:
                ...
                raise Exception(_caller_name + ': Error - invalid data structure, schemaId not found: ' + _data['schemaId'])

            if _curr_schema['collection'] != _collection_name:
                raise Exception(_caller_name + ': Invalid schemaId. ' + _caller_name + ' is restricted to the   "' + _collection_name + '"-collection.')
        else:
    ...
    def verify_condition(_data, _collection_name, _caller_name):
        ...
        if 'collection' in _data:
            if _data['collection'] != _collection_name:
                raise Exception(_caller_name + ': Error - collection omitted or wrong, only queries against the ' + _collection_name + ' collection is permitted.')

    def manage_input(self, _input, _schema_id=None):
        """
        Validate, convert _id to objectId instances and parse collection from input, whether it is data to save or a        condition.
        ...
        try:
            ...
            raise Exception('handle_input: Non-validation error from validation: Type: ' + e.__class__.__name__ + ' Message:' + str(e) + '. Data: \n' + str(_input))
    ...
    def load_exactly_one_document(_collection, _id, _zero_error=None, _duplicate_error=None):
        ...
        if _document_cursor.count() == 0:
            if _zero_error is not None:
                raise Exception(_zero_error + '_id: ' + str(_id) + ' could not be found.')
            else:
                return
        else:
            if _document_cursor.count() > 1:
                if _duplicate_error is not None:
                    raise Exception('Access.save: Tried saving existing, but found duplicate documents with(try to remove either) _id: ' + str(_id) + '.')
                else:
                    return
    ...
    def save(self, _document, _user, _old_document=None, _allow_save_id=False):
        ...
        if _old_document is None:
            if '_id' in _document:
                if not _allow_save_id:
                    _zero_error = "Access.save: Tried to save data over existing but didn't find an existing node."
                else:
                    _zero_error = None
                _old_document = self.load_exactly_one_document(_collection, _document['_id'], _zero_error=_zero_error, _duplicate_error='Access.save: Tried to save data over existing but found duplicate nodes')
                if _old_document is not None and str(_old_document['schemaId']) != str(_document['schemaId']):
                    raise Exception('Access.save: Cannot change schema of an existing node, remove and add. _id: ' + str(_document['_id']) + ', new:' + str(_old_document['schemaId']) + ', old:' + str(_old_document['schemaId']))
