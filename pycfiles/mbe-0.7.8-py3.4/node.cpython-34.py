...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mbe/node.py
# Compiled at: 2015-10-16 07:12:31
# Size of source mod 2**32: 9873 bytes
"""
...
from mbe.groups import aop_has_right, init_groups
from mbe.misc.schema_mongodb import mbe_object_id
from mbe.permission import filter_by_group
__author__ = 'nibo'
node_rights = None
...
class Node:
    ...
    def __init__(self, _database_access, _rights=None):
        ...
        if _rights is not None:
            node_rights = [
             object_id_right_admin_everything] + _rights
        else:
            node_rights = [
             object_id_right_admin_everything]
    ...
    def save(self, _document, _user):
        ...
        if '_id' in _document:
            _old_document_result = filter_by_group(self.database_access.find({'conditions': {'_id': mbe_object_id(_document['_id'])},  'collection': 'node'}), 'canWrite', _user, 'Node.save(existing node)')
            ...
            if 'parent_id' in _document:
                filter_by_group(self.database_access.find({'conditions': {'_id': mbe_object_id(_document['parent_id'])},  'collection': 'node'}), 'canWrite', _user, 'Node.save(new node)')
                _old_document = None
            else:
    ...
    def load_children(self, _parent_id, _user):
        ...
        """
        return filter_by_group(self.database_access.find({'conditions': {'parent_id': mbe_object_id(_parent_id['parent_id'])},  'collection': 'node'}), 'canRead', _user, self.database_access)
    ...
    def load_node(self, _id, _user):
        ...
        """
        return filter_by_group(self.database_access.find({'conditions': {'_id': mbe_object_id(_id['_id'])},  'collection': 'node'}), 'canRead', _user, self.database_access, _error_prefix_if_not_allowed='Node.load_node: Not permissioned to load this node. _id: ' + _id['_id'])[0]
    ...
    def remove(self, _id, _user):
        ...
        _node = self.database_access.find({'conditions': {'_id': mbe_object_id(_id['_id'])},  'collection': 'node'}, _do_not_fix_object_ids=True)[0]
