...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/pathogenseq/qc.py
...
from .fasta import *
from .fastq import *
import numpy as np, gzip, matplotlib as mpl
mpl.use('Agg')
...
def gsize_convert(x):
    d = {'G': 1000000000.0, 'M': 1000000.0, 'K': 1000.0}
    num = float(x[:-1])
    char = x[(-1)]
    if char not in d:
        log('%s not a valid value')
        quit()
    return num * d[char]
...
def get_genome_cov(bam_file, ref_file, min_dp):
    ...
    for l in subprocess.Popen(samtools_cmd, shell=True, stdout=subprocess.PIPE).stdout:
        ...
        if arr[0] not in ref_cov:
            log("Can't find %s in FASTA...Have you used the correct reference sequence?")
            quit()
...
class qc_fastq:
    ...
    def __init__(self, prefix, fq1, fq2=None, optimise=True, threads=4):
        ...
        FQ = gzip.open(fq1)
        i = int(self.read_pairs * 0.1) if optimise else self.read_pairs
        for j in range(i):
            FQ.readline()
    ...
    def run_centrifuge(self, centrifuge_db, filter_fastq=None, threads=4):
        ...
        if filter_fastq:
            tmp = [
             top_hit, num_mtb / self.read_num]
            return (
             self.params['cf_filt_fq_1'], self.params['cf_filt_fq_2'], tmp)
        else:
            return (
             top_hit, top_num_reads)
...
class qc_bam:
    ...
    def __init__(self, bam, ref, cov_thresholds=[
 1, 5, 10], threads=4):
        self.bam = None
        self.ref = None
