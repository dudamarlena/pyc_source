...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/pathogenseq/bam.py
# Compiled at: 2018-07-14 07:06:01
...
import re, numpy as np, vcf, pysam
...
class bam:
    ...
    def gbcf(self, call_method='optimise', max_dp=None, min_dp=10, threads=4, vtype='snps', bed_file=None, platform='illumina', primers=None, overlap_search=True, chunk_size=50000, mpileup_options=None, low_dp_as_missing=False):
        ...
        if primers:
            ...
            TMP.close()
        if vtype == 'snps':
            self.params['vtype'] = '-V indels'
        elif vtype == 'indels':
            self.params['vtype'] = '-V snps'
        elif vtype == 'both':
            self.params['vtype'] = ''
        else:
            sys.stderr.write('Please provide valid vtype: [snps|indels|both]...Exiting!')
            quit(1)
        self.params['primer_cmd'] = ' -T ^%(primer_bed_file)s' % self.params if primers else ''
        if call_method == 'optimise' and platform == 'illumina':
    ...
    def get_bam_qc(self, cov_thresholds=[
 1, 5, 10, 20]):
        """
                Get a qc_bam object
    ...
    def htsbox_calls(self, bed_file=None):
        ...
        if self.params['platform'] == 'Illumina':
            for l in open(self.params['temp_pileup']):
                arr = l.rstrip().split()
                if bed_file and (arr[0], arr[1]) not in bed_pos:
                    continue
                calls = arr[3].split(',')
    ...
    def pileup2vcf(self, min_het_frac=0.3, min_hom_frac=0.6, min_dp=10, bed_file=None, indels=True):
        ...
        for l in open(self.params['temp_pileup']):
            ...
            call = max_allele
            if not indels and (len(max_allele) > 1 or len(ref) > 1):
                ref_run_end_pos = arr[1]
                if tot_dp < ref_run_min_dp:
                    ref_run_min_dp = tot_dp
            elif tot_dp < min_dp:
