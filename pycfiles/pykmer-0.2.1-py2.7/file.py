...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/pykmer/file.py
# Compiled at: 2017-01-24 20:01:37
"""
...
import os, os.path, subprocess, uuid
...
def openFile(fn, mode='r'):
    ...
    if mode == 'w':
        if fn == '-':
            return sys.stdout
        if fn.endswith('.gz'):
            p = subprocess.Popen(['gzip', '-9', '-', '>', fn], stdin=subprocess.PIPE, shell=True)
            return p.stdin
        if fn.endswith('.bz2'):
            p = subprocess.Popen(['bzip2', '-9', '-', '>', fn], stdout=subprocess.PIPE, shell=True)
        return p.stdin
    return open(fn, mode)
