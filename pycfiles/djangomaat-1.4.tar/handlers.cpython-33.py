...
# [GCC 8.4.0]
# Embedded file name: /Users/germanoguerrini/Developer/github/django-maat/djangomaat/handlers.py
# Compiled at: 2015-02-03 06:23:12
...
class MaatHandler(object):
    ...
    def _get_model_manager(self):
        ...
        if not hasattr(self.model_class, self.manager):
            raise ManagerDoesNotExist("Model {} doesn't have a manager {}".format(self.model_class, self.manager))
    ...
    def _validate_typology(self, typology):
        ...
        if typology not in self._get_valid_typologies():
    ...
    def _typology_getters_iterator(self):
        ...
        return ((k[getter_prefix_length:], v) for k, v in inspect.getmembers(self, predicate=inspect.ismethod) if k.startswith(GETTER_PREFIX))
    ...
    def _get_content_type(self):
        ...
        if not hasattr(self, '_content_type'):
            ct = ContentType.objects.get_for_model(self.model_class, self.use_concrete_model)
        ...
        for typology, getter in self._typology_getters_iterator():
            ...
            if not simulate:
                with atomic():
                    ...
                    objects = (MaatRanking(content_type_id=self._get_content_type().pk, object_id=object_id, typology=typology, usable=False, position=next(current_position)) for object_id in getter())
                    ...
                    if logger:
                        logger.write('Delete...')
                        start = time()
                    MaatRanking.objects.filter(content_type=self._get_content_type(), typology=typology, usable=True).delete()
                    if logger:
                        end = time()
                        duration = end - start
                        logger.write(' done ({:.3f} sec)\n'.format(duration))
                    if logger:
                        logger.write('Update...')
                        start = time()
                    MaatRanking.objects.filter(content_type=self._get_content_type(), typology=typology, usable=False).update(usable=True)
                    if logger:
                        end = time()
                        duration = end - start
                        logger.write(' done ({:.3f} sec)\n'.format(duration))
    ...
    def ordered_by(self, typology):
        ...
        *typology* with a minus sign.
        ...
        return self._get_model_manager().filter(maat_ranking__content_type=self._get_content_type(), maat_ranking__typology=typology, maat_ranking__usable=True).order_by('{}maat_ranking__position'.format(ordering_prefix))


class HandlerDescriptor(object):
    __doc__ = ' Makes the handle accessible only by the class, not its instances. '
