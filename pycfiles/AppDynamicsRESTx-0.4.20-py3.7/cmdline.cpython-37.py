...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/appd/cmdline.py
...
def parse_argv(app_desc=None):
    ...
    parser.add_argument('-c', '--url', dest='url', required=True, help='set the controller URL')
    parser.add_argument('-u', '--username', dest='username', required=True, help='set the username for authentication')
    parser.add_argument('-p', '--password', dest='password', required=True, help='set the password for authentication')
    parser.add_argument('-a', '--account', default='customer1', dest='account', help='set the controller account (default: customer1)')
