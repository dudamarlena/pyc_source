...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/appd/request.py
# Compiled at: 2020-03-11 17:54:36
# Size of source mod 2**32: 44378 bytes
"""
...
class AppDynamicsClient(object):
    ...
    DEEP_DIVE_POLICIES = ('SLA_FAILURE', 'TIME_SAMPLING', 'ERROR_SAMPLING', 'OCCURRENCE_SAMPLING',
                          'ON_DEMAND', 'HOTSPOT', 'HOTSPOT_LEARN', 'APPLICATION_STARTUP',
                          'DIAGNOSTIC_SESSION', 'SLOW_DIAGNOSTIC_SESSION', 'ERROR_DIAGNOSTIC_SESSION',
                          'POLICY_FAILURE_DIAGNOSTIC_SESSION', 'INFLIGHT_SLOW_SESSION')
    USER_EXPERIENCES = ('NORMAL', 'SLOW', 'VERY_SLOW', 'STALL', 'BUSINESS_ERROR')
    COLLECTOR_TYPES = ('ERROR_IDS', 'STACK_TRACES', 'ERROR_DETAIL', 'HTTP_PARAMETER',
                       'BUSINESS_DATA', 'COOKIE', 'HTTP_HEADER', 'SESSION_KEY', 'RESPONSE_HEADER',
                       'LOG_MESSAGE', 'TRANSACTION_PROPERTY', 'TRANSACTION_EVENT',
                       'DOTNET_PROPS', 'DOTNET_SET')
    SNAPSHOT_REQUEST_PARAMS = ('guids', 'archived', 'deep-dive-policy', 'application-component-ids',
                               'application-component-node-ids', 'business-transaction-ids',
                               'user-experience', 'first-in-chain', 'need-props',
                               'need-exit-calls', 'execution-time-in-millis', 'session-id',
                               'user-principal-id', 'error-ids', 'error-occurred',
                               'bad-request', 'diagnostic-snapshot', 'diagnostic-session-guid',
                               'starting-request-id', 'ending-request-id', 'data-collector-name',
                               'data-collector-type', 'data-collector-value')
    SNAPSHOT_REQUEST_LISTS = ('business-transaction-ids', 'user-experience', 'error-ids',
                              'guids', 'deep-dive-policy', 'application-component-ids',
                              'application-component-node-ids', 'diagnostic-session-guid')
    ...
    def __init__(self, base_url='http://localhost:8090', username='user1', password='welcome', account='customer1', debug=False, verify=True):
        """
        ...
        :type debug: bool.
        """
        self._username, self._password, self._account, self._app_id, self._session = ('',
                                                                                      '',
                                                                                      '',
                                                                                      None,
                                                                                      None)
    ...
    def username(self, new_username):
        ...
        self._AppDynamicsClient__make_auth()
    ...
    def set_controller_url(self, controllerURL):
        ...
        return self._top_request(SetControllerUrlResponse, ('/accounts/%s/update-controller-url' % self.account),
          param, 'POST', query=False)
    ...
    def get_audit_history(self, start_time, end_time, time_zone_id=None):
        ...
        return AuditHistory.from_json(self.request('/controller/ControllerAuditHistory', params,
          'GET', query=True, use_json=True))

    def create_user(self, user_name, user_display_name, user_email, user_password, user_roles=None):
        """
        Creates a user
    ...
    def export_health_rules(self, application_id, name=None):
        ...
        return self.request(('/controller/healthrules/{0}'.format(application_id)), params,
    ...
    def export_analytics_dynamic_service_configs(self, application_id):
        ...
        :returns: JSON string
        """
        return self.request(('/controller/analyticsdynamicservice/{0}'.format(application_id)), {}, 'GET', query=True, use_json=False)
    ...
    def _validate_time_range(self, time_range_type, duration_in_mins, start_time, end_time):
        ...
        if time_range_type and time_range_type not in self.TIME_RANGE_TYPES:
            ...
            if time_range_type == 'BEFORE_NOW' and not duration_in_mins:
                raise ValueError('when using BEFORE_NOW, you must specify duration_in_mins')
            else:
                if time_range_type == 'BEFORE_TIME':
                    raise end_time and duration_in_mins or ValueError('when using BEFORE_TIME, you must specify duration_in_mins and end_time')
                else:
                    if time_range_type == 'AFTER_TIME':
                        raise start_time and duration_in_mins or ValueError('when using AFTER_TIME, you must specify duration_in_mins and start_time')
                    else:
                        if time_range_type == 'BETWEEN_TIMES':
                            raise start_time and end_time or ValueError('when using BETWEEN_TIMES, you must specify start_time and end_time')
        return {'time-range-type':time_range_type, 
         'duration-in-mins':duration_in_mins, 
         'start-time':start_time, 
         'end-time':end_time}

    def get_metrics(self, metric_path, app_id=None, time_range_type='BEFORE_NOW', duration_in_mins=15, start_time=None, end_time=None, rollup=True):
        """
        ...
        params.update({'metric-path':metric_path,  'rollup':rollup, 
         'output':'JSON'})
        return self._app_request(MetricData, '/metric-data', app_id, params)

    def get_events(self, app_id=None, event_types='ALL', severities='ERROR', time_range_type='BEFORE_NOW', duration_in_mins=15, start_time=None, end_time=None):
        """
        Retrieves events.
        ...
        """
        params = self._validate_time_range(time_range_type, duration_in_mins, start_time, end_time)
        params.update({'event-types':event_types,  'severities':severities, 
         'output':'JSON'})
        return self._app_request(Events, '/events', app_id, params)

    def create_event(self, app_id=None, summary='', comment='', eventtype='APPLICATION_CONFIG_CHANGE', severity='INFO', customeventtype=None, node=None, tier=None, bt=None):
        """
        Creates an event.
        ...
        params = {'summary':summary, 
         ...
         'severity':severity}
        if eventtype == 'CUSTOM':
            params.update({'customeventtype':customeventtype,  'node':node, 
             'tier':tier, 
             'bt':bt})
    ...
    def get_snapshots(self, app_id=None, time_range_type=None, duration_in_mins=None, start_time=None, end_time=None, **kwargs):
        """
    ...
    def get_policy_violations(self, app_id=None, time_range_type='BEFORE_NOW', duration_in_mins=15, start_time=None, end_time=None):
        """
    ...
    def get_healthrule_violations(self, app_id=None, time_range_type='BEFORE_NOW', duration_in_mins=15, start_time=None, end_time=None):
        """
    ...
    def get_action_suppressions(self, account_id, app_id):
        return self._v2_request(ActionSuppressionsResponse, '/accounts/{0}/applications/{1}/actionsuppressions'.format(account_id, app_id))

    def get_action_suppression(self, account_id, app_id, action_suppression_id):
        return self._v2_request(ActionSuppression, '/accounts/{0}/applications/{1}/actionsuppressions/{2}'.format(account_id, app_id, action_suppression_id))

    def delete_action_suppression(self, account_id, app_id, action_suppression_id):
        return self.request(('/api/accounts/{0}/applications/{1}/actionsuppressions/{2}'.format(account_id, app_id, action_suppression_id)),
          method='DELETE',
          use_json=False)

    def create_action_suppression(self, account_id, app_id, params):
        return self.request(('/api/accounts/{0}/applications/{1}/actionsuppressions'.format(account_id, app_id)), method='POST',
          use_json=False,
          query=False,
          params=params,
    ...
    def get_license_usage(self, account_id, license_module=None, start_time=None, end_time=None):
        ...
        params = {'licensemodule':license_module, 
