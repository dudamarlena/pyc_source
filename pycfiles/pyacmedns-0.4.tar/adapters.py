...
# Embedded file name: build/bdist.linux-i686/egg/acl/testutils/adapters.py
# Compiled at: 2009-04-27 10:46:59
...
__all__ = [
 'GroupsAdapterTester', 'ReadOnlyGroupsAdapterTester']
...
class _ReadOnlyBaseAdapterTester(object):
    ...
    def test_getting_group_users(self):
        for (group_name, users) in self.all_groups.items():
            self.assertEqual(self.adapter._get_group_users(group_name), users)

    def test_checking_existing_group(self):
        for group_name in self.all_groups.keys():
            assert self.adapter._group_exists(group_name), 'group "%s" does NOT exist' % group_name

    def test_checking_non_existing_group(self):
        group_name = 'i_dont_exist'
        assert not self.adapter._group_exists(group_name), 'group "%s" DOES exist' % group_name

    def test_checking_user_inclusion(self):
        for (group_name, users) in self.all_groups.items():
            for user in self.adapter._get_group_users(group_name):
                assert self.adapter._user_is_included(group_name, user), 'User "%s" must be included in group "%s"' % (
                 user, group_name)

    def test_checking_excluded_user_inclusion(self):
        excluded_user = self.new_users.pop()
        for (group_name, users) in self.all_groups.items():
            assert not self.adapter._user_is_included(group_name, excluded_user), 'User "%s" must not included in group "%s"' % (
             user, group_name)

    def test_checking_group_existence(self):
        for group_name in self.all_groups.keys():
            assert self.adapter._group_exists(group_name), 'group "%s" must exist' % group_name

    def test_checking_non_existing_group_existence(self):
        invalid_group = 'designers'
        assert not self.adapter._group_exists(invalid_group), 'group "%s" must not exist' % invalid_group

    def test_sets_if_it_writable(self):
        assert hasattr(self.adapter, 'writable'), "The adapter doesn't have the 'writable' attribute; please call its parent's constructor too"
...
class _BaseAdapterTester(_ReadOnlyBaseAdapterTester):
    ...
    def test_adding_many_users_to_group(self):
        for (group_name, users) in self.all_groups.items():
            ...
            assert self.adapter._get_group_users(group_name) == final_users, '"%s" does not include %s' % (group_name, self.new_users)

    def test_creating_group(self):
        group = 'cool-group'
        self.adapter._create_group(group)
        assert group in self.adapter._get_all_groups().keys(), 'group "%s" could not be added' % group

    def test_editing_group(self):
        old_group = self.all_groups.keys()[0]
        new_group = 'cool-group'
        self.adapter._rename_group(old_group, new_group)
        assert new_group in self.adapter._get_all_groups().keys() and old_group not in self.adapter._get_all_groups().keys(), 'group "%s" was not renamed to "%s"' % (old_group,
         new_group)

    def test_deleting_group(self):
        group = self.all_groups.keys()[0]
        self.adapter._delete_group(group)
        assert group not in self.adapter._get_all_groups().keys(), 'group "%s" was not deleted' % group
...
class ReadOnlyGroupsAdapterTester(_ReadOnlyBaseAdapterTester):
    ...
    * php
    ...
    """
    new_users = set(('guido', 'rasmus'))

    def setUp(self):
        self.all_groups = {'admins': set(('rms', )), 
    ...
    def test_finding_groups_of_authenticated_user(self):
        for userid in self._get_all_users():
            self.assertEqual(self.adapter._find_groups(userid), self._get_user_groups(userid))
...
class GroupsAdapterTester(ReadOnlyGroupsAdapterTester, _BaseAdapterTester):
    ...
    def test_removing_many_users_from_group(self):
        ...
        assert self.adapter._get_group_users(group) == set(), '"%s" still includes %s' % (group, users)