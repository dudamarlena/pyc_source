...
# Embedded file name: build/bdist.macosx-10.4-x86_64/egg/airbrakepy/logging/handlers.py
# Compiled at: 2011-10-26 03:40:04
import inspect, logging, traceback, multiprocessing, urllib2, sys, xmlbuilder
...
class AirbrakeSender(multiprocessing.Process):
    ...
    def _handle_error(self):
        ...
        try:
            try:
                traceback.print_exception(ei[0], ei[1], ei[2], file=sys.stderr)
            except IOError:
                pass
...
class AirbrakeHandler(logging.Handler):
    ...
    def __init__(self, api_key, environment=None, component_name=None, node_name=None, use_ssl=False, timeout_in_ms=30000, airbrake_url=_DEFAULT_AIRBRAKE_URL):
            ...
            with xml.error:
                ...
                with xml.backtrace:
                    if trace is None:
                        ...
                        [ xml << ('line', {'file': filename, 'number': line_number, 'method': ('{0}: {1}').format(function_name, text)}) for filename, line_number, function_name, text in traceback.extract_tb(trace)
                        ]
        return str(xml)