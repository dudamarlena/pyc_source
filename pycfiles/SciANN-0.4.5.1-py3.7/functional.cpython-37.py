...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/sciann/functionals/functional.py
# Compiled at: 2020-04-12 05:20:01
# Size of source mod 2**32: 13541 bytes
...
class Functional(object):
                ...
                if isinstance(kernel_initializer, (float, int)):
                    ...
                    if isinstance(bias_initializer, (float, int)):
                        ...
                        if all([isinstance(fld, str) for fld in fields]):
                            output_fileds = [Field(name=fld, dtype=dtype, kernel_initializer=kernel_initializer, bias_initializer=bias_initializer, trainable=trainable) for fld in fields]
                        else:
        ...
        if len(kwargs) == 1:
            ...
            if len(kwargs) == 2:
                if validations.is_scimodel(kwargs[0]):
                    ...
                    raise ValueError('Expected a SciModel object for the first arg. ')
                mesh = kwargs[1]
            else:
    ...
    def _set_model(self):
        if hasattr(self, '_model'):
            if is_same_tensor(self._inputs, self._model.inputs):
                if is_same_tensor(self._outputs, self._model.outputs):
                    return
        self._model = K.function(unique_tensors(self._inputs), self._outputs)
    ...
    def set_weights(self, weights):
        try:
            ...
            raise ValueError('Provide data exactly the same as .get_weights() outputs. ')

    def count_params(self):
        return sum([l.count_params() for l in self.layers])

    def copy(self):
        return Functional(inputs=(self.inputs),
    ...
    def split(self):
        ...
        for out, lay in zip(self._outputs, self._layers[-nr:]):
            f = Functional(inputs=(to_list(self.inputs)),
              outputs=(to_list(out)),
