...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\kg_downloader\api.py
# Compiled at: 2019-11-25 18:30:14
...
query = {'jsonpCallback':'callback_1', 
 'g_tk':'5381', 
 'outCharset':'utf-8', 
 'format':'jsonp', 
 'type':'get_ugc', 
...
def analyse(url):
    user_pattern = 'https?://node.kg.qq.com/(?P<flag>\\w+)\\?uid=(?P<uid>\\w+)\\&?.*'
    song_pattern = 'https?://node.kg.qq.com/(?P<flag>\\w+)\\?s=(?P<sid>\\w+)\\&?.*'
...
def fetch_data(url, info):
    if info['flag'] == 'personal':
        ...
        for song, sid in result_set.items():
            result_set.update(get_real_pair(song, concat_song_url(sid)))
