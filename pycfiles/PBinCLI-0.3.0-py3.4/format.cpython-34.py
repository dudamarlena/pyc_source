...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pbincli/format.py
# Compiled at: 2019-09-18 20:10:24
# Size of source mod 2**32: 10073 bytes
...
class Paste:
    ...
    def setVersion(self, version):
        if self._debug:
            print('Set paste version to {}'.format(version))
    ...
    def __deriveKey(self, salt):
        ...
        return PBKDF2(self._key + self._password.encode(), salt, dkLen=CIPHER_BLOCK_BYTES, count=CIPHER_ITERATION_COUNT, prf=lambda password, salt: HMAC.new(password, salt, SHA256).digest())
    ...
    def __decompress(self, s):
        ...
        if self._version == 2 and self._compression == 'none':
            return s
        if self._version == 1:
    ...
    def _decryptV2(self):
        ...
        key = self._Paste__deriveKey(salt)
        self._compression = self._data['adata'][0][7]
        cipher = self._Paste__initializeCipher(key, iv, self._data['adata'])
        ...
        cipher_message = json_decode(self._Paste__decompress(cipher.decrypt_and_verify(cipher_text, cipher_tag)).decode())
        self._text = cipher_message['paste'].encode()
        if 'attachment' in cipher_message:
            if 'attachment_name' in cipher_message:
    ...
    def _decryptV1(self):
        ...
        from json import loads as json_decode
        password = self._Paste__preparePassKey()
        ...
        if len(text):
            if self._debug:
                print('Decoded Text:\t{}\n'.format(text))
            self._text = self._Paste__decompress(text.decode())
        if 'attachment' in self._data:
            if 'attachmentname' in self._data:
                ...
                self._attachment = self._Paste__decompress(attachment.decode('utf-8')).decode('utf-8')
                self._attachment_name = self._Paste__decompress(attachmentname.decode('utf-8')).decode('utf-8')
    ...
    def _encryptV2(self):
        ...
        cipher = self._Paste__initializeCipher(key, iv, adata)
        ciphertext, tag = cipher.encrypt_and_digest(self._Paste__compress(json_encode(cipher_message)))
    ...
    def _encryptV1(self):
        ...
        cipher = SJCL().encrypt(self._Paste__compress(self._text.encode('utf-8')), password, mode='gcm')
        for k in ['salt', 'iv', 'ct']:
            cipher[k] = cipher[k].decode()

        self._data['data'] = json_encode(cipher)
        if self._attachment:
            cipherfile = SJCL().encrypt(self._Paste__compress(self._attachment.encode('utf-8')), password, mode='gcm')
            for k in ['salt', 'iv', 'ct']:
                cipherfile[k] = cipherfile[k].decode()

            cipherfilename = SJCL().encrypt(self._Paste__compress(self._attachment_name.encode('utf-8')), password, mode='gcm')
            for k in ['salt', 'iv', 'ct']:
