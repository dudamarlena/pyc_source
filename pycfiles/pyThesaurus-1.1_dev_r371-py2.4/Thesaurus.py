...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyThesaurus/Thesaurus.py
...
class Thesaurus:
    ...
    def _previous_concept_to_join(self, append_concept, M=matrix_comparation, S=minimum_score):
        ...
        for idx in self._concept:
            if append_concept.could_be_joined_to(self[idx], M, S):
                return idx
    ...
    def append_concept(self, c):
        ...
        for context in c.contexts():
            if context not in self._context:
                self._context[context] = self._list()
    ...
    def query(self, search_expression, narrowerthan=None, broaderthan=None, contexts=[], languages=[], inbranch=None, hidden=None, max_results=5, first_result=0):
        """
        ...
        return {'concepts': [ (conceptsResults[index][1].get_prefered(languages)[0], conceptsResults[index][0]) for index in range(first_result, min(len(conceptsResults), first_result + max_results)) ], 'concepts_count': len(conceptsResults)}

    def refine_search(self, conceptsResults, narrowerthan, broaderthan, contexts, languages, inbranch):
        return [ (cid, concept) for (cid, concept) in conceptsResults if self.is_in_all_concepts_transitive_relation('<', concept, narrowerthan, contexts) if self.is_in_all_concepts_transitive_relation('>', concept, broaderthan, contexts) if self.in_contexts(cid, contexts) ]
