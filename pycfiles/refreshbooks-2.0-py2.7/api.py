...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-intel/egg/refreshbooks/api.py
# Compiled at: 2014-01-20 11:54:38
from __future__ import print_function
import decimal, sys, functools
...
decimal_type = objectify.PyType('decimal', check_decimal_element, DecimalElement)
decimal_type.register(before='float')
default_request_encoder = adapters.xml_request

def default_response_decoder(*args, **kwargs):
    return adapters.fail_to_exception_response(objectify.fromstring(*args, **kwargs))
...
def AuthorizingClient(domain, auth, request_encoder, response_decoder, user_agent=None):
    """Creates a Freshbooks client for a freshbooks domain, using
    an auth object.
    """
    http_transport = transport.HttpTransport(api_url(domain), build_headers(auth, user_agent))
    return client.Client(request_encoder, http_transport, response_decoder)
