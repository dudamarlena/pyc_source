...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-intel/egg/refreshbooks/optional/oauth.py
...
def OAuthClient(AuthorizingClient, domain, consumer_key, consumer_secret, token, token_secret, user_agent, request_encoder, response_decoder):
    consumer = oauth.OAuthConsumer(consumer_key, consumer_secret)
    token = oauth.OAuthToken(token, token_secret)
    return AuthorizingClient(domain, OAuthAuthorization(consumer, token), request_encoder, response_decoder, user_agent=user_agent)