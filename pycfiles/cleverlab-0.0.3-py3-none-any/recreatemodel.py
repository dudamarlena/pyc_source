...
# Embedded file name: build/bdist.linux-i686/egg/harold/plugins/recreatemodel.py
...
class RecreateModels(HaroldCommandType):
    ...
    description = 'This command creates or recreates the model specified in the configuration file.'
    parser = HaroldCommandType.standard_parser(quiet=True, drop_first=True, ini_file=True, ignore_exc=True, list_sections=True, models_section=True)
    parser.add_option('-n', '--no-populate', dest='populate', default=True, action='store_false', help='Skip populating models.')
    ...
    def command(self):
        ...
        try:
            ...
            if not quiet:
                args = (
                 handler.__class__.__name__, use)
                print
