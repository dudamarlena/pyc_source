...
# Embedded file name: build/bdist.linux-i686/egg/harold/tests/test_codepub.py
...
class FsPublisher_FsCodeModule__Test(unittest.TestCase):
    ...
    def setUp(self):
        self.headers = []
        self.publisher = code_publisher(app=test_app, dirs=mod_template_dirs, debug=True)
    ...
    def test_path_class_callable_context_app_mixed(self):
        ...
        expected = [
         (
          wsgi_env()['wsgi.version'], self.publisher)]
    ...
    def __test__1__index(self):
        ...
        self.assertEqual(sorted(pos), pos, 'index')

    def __test__2__alternate_index(self):
        results = self.run_publisher('/altindex')
        keys = ['default D', 'default C', 'default B', 'alternate A']
        pos = [ results.index(key) for key in keys ]
        self.assertEqual(sorted(pos), pos, 'alt index')

    def __test__3__subdirectory_index(self):
        results = self.run_publisher('/sub')
        keys = ['default D', 'default C', 'subdir B', 'subdir A']
        pos = [ results.index(key) for key in keys ]
        self.assertEqual(sorted(pos), pos, 'subdir index')

    def __test__4__subdirectory_empty(self):
        results = self.run_publisher('/sub/empty')
        keys = ['default D', 'default C', 'default B', 'default A']
        pos = [ results.index(key) for key in keys ]
        self.assertEqual(sorted(pos), pos, 'subdir empty template')

    def __test__5__named_template_whole_file(self):
        results = self.run_publisher('/funcs')
        keys = ['default D', 'default C', 'default B', 'ntf A']
        pos = [ results.index(key) for key in keys ]
        self.assertEqual(sorted(pos), pos, 'template with ' + ntf)

    def __test__6__ntf_no_args(self):
        results = self.run_publisher('/funcs/E')
        self.assertEqual(results, '<div>ntf E</div>', 'no-arg ' + ntf)

    def __test__7__ntf_args_query_posargs(self):
        results = self.run_publisher('/funcs/F', 'a=1&b=2')
        self.assertEqual(results, '<div>ntf F 1 2</div>', 'query posarg ' + ntf)

    def __test__8__ntf_query_keyargs(self):
        results = self.run_publisher('/funcs/G', 'a=1&b=2')
        self.assertEqual(results, '<div>ntf G 1 2</div>', 'query+keyarg named template function ')

    def __test__9__ntf_args_query_posargs_keyargs(self):
        results = self.run_publisher('/funcs/H', 'a=1&b=2')
        self.assertEqual(results, '<div>ntf H 1 2</div>', 'query+posarg+defaults ' + ntf)

    def __test__10__ntf_path_only(self):
        results = self.run_publisher('/funcs/I/2/3')
        self.assertEqual(results, '<div>ntf I 2 3</div>', 'path ' + ntf)

    def __test__11__ntf_path_query_mix(self):
        results = self.run_publisher('/funcs/J/0', 'b=1')
        self.assertEqual(results, '<div>ntf J 0 1</div>', 'path+query ' + ntf)

    def __test__12__ntf_path_query_mix(self):
        results = self.run_publisher('/funcs/K/0', 'b=1')
        self.assertEqual(results, '<div>ntf K 0 1</div>', 'path+query+defaults ' + ntf)

    def __test__13__ntf_keyargs(self):
        results = self.run_publisher('/funcs/L', 'a=slug')
        for key in ['slug', 'cookies', 'environ', 'context', 'form']:
            self.failUnless(key in results, 'kwarg ' + ntf)

    def __test__14__ntf_posargs_keyargs_query(self):
        results = self.run_publisher('/funcs/M', 'a=slug')
        self.failUnless('slug' in results, 'query+posarg+kwarg ' + ntf)
        for key in ['cookies', 'environ', 'context', 'form']:
            self.failUnless(key in results, 'query+posarg+kwarg ' + ntf)

    def __test__15__ntf_posargs_keyargs_defaults_query(self):
        results = [
         self.run_publisher('/funcs/N/snail/slug'), self.run_publisher('/funcs/N/slug', 'b=snail'), self.run_publisher('/funcs/N/snail', 'b=slug'), self.run_publisher('/funcs/N', 'a=snail&b=slug')]
        keys = [
         'juice', 'snail', 'slug', 'cookies', 'environ', 'context', 'form']
        for key in keys:
            for result in results:
                self.failUnless(key in result, 'query+posarg+kwarg ' + ntf)

    def __test__16__nmf_noargs(self):
        results = self.run_publisher('/funcs/P')
        self.assertEqual(results, 'P', 'no arg ' + nmf)

    def __test__17__nmf_posarg(self):
        results = [
         self.run_publisher('/funcs/Q', 'a=1'), self.run_publisher('/funcs/Q/1')]
        for result in results:
            self.assertEqual(result, 'Q 1', ' pos arg ' + nmf)

    def __test__18__nmf_posargs(self):
        results = [
         self.run_publisher('/funcs/R', 'a=1&b=2'), self.run_publisher('/funcs/R/1', 'b=2'), self.run_publisher('/funcs/R/1/2')]
        for result in results:
            self.assertEqual(result, 'R 1 2', ' pos args ' + nmf)

    def __test__19__nmf_defaults(self):
        results = [self.run_publisher('/funcs/S/1'), self.run_publisher('/funcs/S', 'a=1')]
        for result in results:
            self.assertEqual(result, 'S 1 0', ' default args ' + nmf)

        results = [self.run_publisher('/funcs/S', 'a=1&b=2'), self.run_publisher('/funcs/S/1', 'b=2'), self.run_publisher('/funcs/S/1/2')]
        for result in results:
            self.assertEqual(result, 'S 1 2', ' default args ' + nmf)

    def __test__20__nmf_kwargs(self):
        results = [
         self.run_publisher('/funcs/T', 'a=slug&b=snail'), self.run_publisher('/funcs/T/slug', 'b=snail'), self.run_publisher('/funcs/T/snail', 'b=slug')]
        for result in results:
            self.failUnless('slug' in result, ' kw args ' + nmf)
            self.failUnless('snail' in result, ' kw args ' + nmf)

    def __test__21__nmf_extraposargs(self):
        results = [
         self.run_publisher('/funcs/U', 'b=slug&b=snail')]
        for result in results:
            self.failUnless('slug' in result, ' extra pos args ' + nmf)
            self.failUnless('snail' in result, ' extra pos args ' + nmf)

    def __test__z__22__nmf_posargs_extraposargs(self):
        results = [
         self.run_publisher('/funcs/V/1', 'b=slug&b=snail')]
        for result in results:
            self.failUnless('slug' in result, ' pos args + extra pos args ' + nmf)
            self.failUnless('snail' in result, ' pos args + extra pos args ' + nmf)
