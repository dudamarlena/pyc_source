...
# Embedded file name: build/bdist.linux-i686/egg/harold/publishers/common.py
...
class TemplateMixin:
    ...
    def __call__(self):
        ...
        if match:
            self.log.debug('matched %s to %s', env.get('SCRIPT_NAME', '') + path_info or '/', match[0])
    ...
    def context(self):
        ...
        def opener():
            ...
            ctx = dict(application=self.app, cookies=self.cookies(), publisher=self, environ=self.env, form=self.form(), request=self.request(), session=self.session())
            return ctx
...
def extend_view(view, layout, defaults, encoding, cache, key='py:extends'):
...
def publishable_items(module):
    ...
    """
    items = [ (item, getattr(module, item)) for item in dir(module) if not item.startswith('_') ]
    items = [ (item, obj) for (item, obj) in items if getattr(obj, 'expose', False) ]
    defaults = [ (item, obj) for (item, obj) in items if getattr(obj, 'default', None) ]
    return (
     items, defaults)


magic_arg_names = [
 'application', 'environ', 'publisher', 'request']
...
class RequestParameterParser:
    ...
    def __init__(self, func, path, query, context=None, ignore_additional_args=False, ignore_self_arg=True):
        self.path = path[:]
        ...
        self.arg_sig = (
         bool(posarg_names), bool(vararg_name), bool(kwarg_name))
    ...
    @classmethod
    def parse(cls, func, path, query, context=None, ignore_additional_args=False, ignore_self_arg=True):
        ...
        @return two-tuple of (arguments, keyword arguments)    
        """
        obj = cls(func, path, query, context, ignore_additional_args, ignore_self_arg)
        return obj()
    ...
    arg_sig_map = {(True, True, True): pos_args_extra_args_keyword_args, (True, True, False): pos_args_extra_args, (True, False, True): pos_args_keyword_args, (True, False, False): pos_args, (False, True, True): extra_args_keyword_args, (False, True, False): extra_args, (False, False, True): keyword_args, (False, False, False): no_args}
