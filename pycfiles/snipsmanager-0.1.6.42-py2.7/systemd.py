...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-intel/egg/snipsmanager/utils/systemd.py
...
class Systemd:
    ...
    def setup(use_default_values=None):
        if ask_yes_no('Would you like Snips to start on boot (using systemd)?', use_default_values) == False:
            return
        username, snips_home_path, snipsmanager_path, snips_path = Systemd.get_snipsmanager_params(use_default_values=use_default_values)
        Systemd.write_snipsmanager_file(username, snips_home_path, snipsmanager_path)
    ...
    def get_snipsmanager_params(use_default_values=None):
        ...
        return (
         username, snips_home_path, snipsmanager_path, snips_path)
    ...
    def write_snipsmanager_file(username, snips_home_path, snipsmanager_path):
        ...
        if contents is None:
            ...
            contents = contents.replace('{{SNIPS_HOME_PATH}}', snips_home_path).replace('{{SNIPSMANAGER_PATH}}', snipsmanager_path)
            Systemd.write_systemd_file(SNIPSMANAGER_SERVICE_NAME, username, contents)
            return

    @staticmethod
    def get_template(service_name):
        this_dir, this_filename = os.path.split(__file__)
        template_filename = os.path.join(this_dir, ('../config/systemd/{}.service').format(service_name))
    ...
    def write_systemd_file(service_name, username, contents):
        if username is None:
            output_filename = ('/etc/systemd/system/{}.service').format(service_name)
        else:
            output_filename = ('/etc/systemd/system/{}@{}.service').format(service_name, username)
