...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-intel/egg/snipsmanager/commands/run.py
...
class Runner(Base):
    ...
    def run_from_snipsfile(snipsfile, mqtt_hostname=None, mqtt_port=None, tts_service_id=None, locale=None):
        Runner.run_with_params(mqtt_hostname=mqtt_hostname or snipsfile.mqtt_hostname, mqtt_port=mqtt_port or snipsfile.mqtt_port, tts_service_id=tts_service_id or snipsfile.tts_service, locale=locale or snipsfile.locale, skilldefs=snipsfile.skilldefs)
    ...
    def __init__(self, mqtt_hostname, mqtt_port, tts_service_id, locale, skilldefs=[]):
        ...
        for skilldef in self.skilldefs:
            try:
                ...
                logger.error(('Error loading skill {}: {}').format(skilldef.package_name, str(e)))
        ...
        for skilldef in self.skilldefs:
            ...
            if intent_def.action is not None:
                if intent_def.action.startswith('{%'):
                    action = intent_def.action.replace('{%', '').replace('%}', '').strip()
                    dialog_object = SnipsDialogueAPI(self.server.client, self.server.tts_service_id, self.server.locale)
                    action_scope = {'snips': ObjectFromDict({'dialogue': dialog_object, 
                                 'session_id': sessionId, 
                                 'site_id': siteId, 
                                 'skill': skill, 
                                 'intent': intent})}
                    exec action in action_scope
                else:
    ...
    def handle_dialogue_events(self, name, sessionId, siteId):
        ...
        for skilldef in self.skilldefs:
            ...
            if dialogue_events_def.action.startswith('{%'):
                action = dialogue_events_def.action.replace('{%', '').replace('%}', '').strip()
                dialog_object = SnipsDialogueAPI(self.server.client, self.server.tts_service_id, self.server.locale)
                action_scope = {'snips': ObjectFromDict({'dialogue': dialog_object, 
                             'session_id': sessionId, 
                             'site_id': siteId, 
    ...
    def handle_notification(self, name):
        ...
        for skilldef in self.skilldefs:
            ...
            if notification_def.action.startswith('{%'):
                action = notification_def.action.replace('{%', '').replace('%}', '').strip()
                action_scope = {'snips': ObjectFromDict({'skill': skill})}
                exec action in action_scope
            else:
