...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-intel/egg/snipsmanager/utils/snipsfile.py
# Compiled at: 2017-12-13 13:37:31
""" Utilities for handline Snipsfiles. """
import pkgutil, os, yaml
...
class Snipsfile:
    ...
    def __init__(self, snipsfile='Snipsfile'):
        ...
        if not os.path.isfile(snipsfile):
            raise SnipsfileNotFoundError(('No Snipsfile found at path {}.').format(snipsfile))
        yaml_config = None
        with open(snipsfile, 'r') as (yaml_file):
            try:
                yaml_config = yaml.load(yaml_file)
            except yaml.scanner.ScannerError as err:
                raise SnipsfileParseException('Error parsing Snipsfile: ' + str(err))
        ...
        if not yaml_config:
            ...
            self.default_location = get(yaml_config, ['default_location'], 'Paris,fr')
            self.mqtt_hostname = get(yaml_config, ['mqtt_broker', 'hostname'], 'localhost')
            self.mqtt_port = get(yaml_config, ['mqtt_broker', 'port'], 1883)
            self.modify_asoundconf = get(yaml_config, ['modify_asoundconf'], True)
            ...
            for skill in get(yaml_config, ['skills'], []):
                ...
                self.skilldefs.append(SkillDef(name, package_name, class_name, url, params, intent_defs, dialogue_events_defs, notification_defs, requires_tts, addons))
