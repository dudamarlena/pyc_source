...
# [GCC 8.4.0]
# Embedded file name: /Library/Frameworks/Python.framework/Versions/2.7/lib/python2.7/site-packages/email_extractor.py
# Compiled at: 2011-12-31 03:27:19
__thisfile__ = 'http://www.jaist.ac.jp/~s1010205/email_extractor/email_extractor.py'
import sys, re, commands
from urllib import urlopen
from collections import defaultdict
import argparse, string, urllib2, urlparse
from HTMLParser import HTMLParser
from HTMLParser import HTMLParseError
import robotparser, httplib
...
class MyHTMLParser(HTMLParser):
    ...
    def __init__(self, pageMap, redirects, baseUrl, maxUrls, blockExtensions, robotParser):
        ...
        self.server = urlparse.urlsplit(baseUrl)[1]
            ...
            if tag.upper() == 'A':
                ...
                for attr in attrs:
                    if attr[0].upper() == 'REL' and attr[1].upper().find('NOFOLLOW') != -1:
                        return
                    if attr[0].upper() == 'HREF' and attr[1].upper().find('MAILTO:') == -1:
...
def grab_email(text):
    found = []
    mailsrch = re.compile('[\\w\\-][\\w\\-\\.]+@[\\w\\-][\\w\\-\\.]+[a-zA-Z]{1,4}')
    for line in text:
