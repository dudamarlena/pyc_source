...
# Compiled at: 2017-10-09 17:13:19
"""
...
class FloatCtrl(wx.TextCtrl):
    ...
    def __GetMark(self):
        ...
        try:
            self.__mark = min(wx.TextCtrl.GetSelection(self)[0], len(wx.TextCtrl.GetValue(self).strip()))
        except:
            self.__mark = 0
    ...
    def OnChar(self, event):
        ...
        has_minus = '-' in entry
        ckey = chr(key)
        if ckey == '.' and (self.__prec == 0 or '.' in entry) or ckey == '-' and (has_minus or pos[0] != 0) or ckey != '-' and has_minus and pos[0] == 0:
            return
