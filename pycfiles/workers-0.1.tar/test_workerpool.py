...
# [GCC 8.4.0]
# Embedded file name: /Users/jesse/nasuni/workerpool/tests/test_workerpool.py
# Compiled at: 2011-05-17 15:10:42
"""
workers unit tests
...
"""
import Queue, threading, time, unittest, workers
...
class TestWorker(unittest.TestCase):
    ...
    def test_worker_basic(self):
        x = workers.Worker(self.run, self.stop, self.inbox, self.outbox, self.errbox, name='flob', stagger=0.1)
        x.start()
        self.run.set()
    ...
    def test_worker_err(self):
        x = workers.Worker(self.run, self.stop, self.inbox, self.outbox, self.errbox, name='flob')
        x.start()
        self.run.set()
    ...
    def test_worker_local_data(self):
        ...
        x = workers.Worker(self.run, self.stop, self.inbox, self.outbox, self.errbox, name='worker_with_data', worker_setup=setup)
        x.start()
        self.run.set()
