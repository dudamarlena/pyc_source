...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/limiter/managers.py
# Compiled at: 2019-12-30 13:06:36
# Size of source mod 2**32: 20669 bytes
import logging, uuid
...
from limiter.exceptions import CapacityExhaustedException, ReservationNotFoundException, ThrottlingException, RateLimiterException
...
class BaseTokenManager:
    ...
    def _get_account_resource_limit(self, account_id):
        ...
        try:
            response = self.limit_table.query(KeyConditionExpression=(Key(RESOURCE_NAME).eq(self.resource_name) & Key(ACCOUNT_ID).eq(account_id)))
            if response['Count']:
...
class FungibleTokenManager(BaseTokenManager):
    ...
    def get_token(self, account_id):
        ...
        token_ms = float(limit) / window_ms
        ms_token = int(max(1, float(window_ms) / limit))
        bucket = self._get_bucket_token(account_id, exec_time, ms_token)
        current_tokens = bucket[TOKENS]
        last_refill = int(bucket.get('last_refill', 0))
    ...
    def _get_bucket_token(self, account_id, exec_time, ms_token):
        ...
        try:
            ...
            return self.token_table.update_item(Key={RESOURCE_NAME: self.resource_name, 
    ...
    def _refill_bucket_tokens(self, account_id, tokens, refill_time):
        ...
        try:
            ...
            self.token_table.update_item(Key={RESOURCE_NAME: self.resource_name, 
             ACCOUNT_ID: account_id},
              UpdateExpression=update_exp,
              ConditionExpression=condition_exp,
            ...
            if isinstance(e, ClientError):
                if e.response['Error']['Code'] == 'ConditionalCheckFailedException':
                    logger.warning('Failed to refill tokens for %s:%s, already refilled with more current state', self.resource_name, account_id)
            else:
...
class NonFungibleTokenManager(BaseTokenManager):
    ...
    def _get_token_count(self, account_id, exec_time):
        ...
        return self.token_table.query(Select='COUNT',
          ConsistentRead=True,
          KeyConditionExpression=(Key(RESOURCE_COORDINATE).eq(coordinate)),
          FilterExpression=(Attr(EXPIRATION_TIME).gt(exec_time)))['Count']
    ...
    def _build_reservation(self, account_id, exec_time):
        ...
        self.token_table.put_item(Item={RESOURCE_COORDINATE: coordinate, 
         RESERVATION_ID: id, 
...
class TokenReservation:
    ...
    def create_token(self, resource_id, expiration=28800):
        ...
        if self.is_token_created:
            ...
            try:
                ...
                self.table.update_item(Key={RESOURCE_COORDINATE: self.coordinate, 
    ...
    def delete(self):
        ...
        if self.is_deleted:
            ...
            return
        self.table.delete_item(Key={RESOURCE_COORDINATE: self.coordinate, 
         RESERVATION_ID: self.id},
          ReturnValues='NONE')
