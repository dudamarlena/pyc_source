...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/test/test_event_processors.py
# Compiled at: 2019-12-30 13:04:31
# Size of source mod 2**32: 15443 bytes
...
class EventProcessorManagerTest(TestCase):
    ...
    def test_no_event_source(self):
        ...
        manager = EventProcessorManager(table_name=(self.table_name), index_name=(self.index_name),
          processors=[
         mock_processor])
    ...
    def test_env_params(self):
        env_vars = {'NON_FUNGIBLE_TABLE':self.table_name, 
    ...
    def test_delete_on_type(self):
        ...
        manager = EventProcessorManager(table_name=(self.table_name), index_name=(self.index_name),
          processors=[
         mock_default_processor, mock_type_processor])
        manager._table = mock_table
        manager.process_event(event)
        self.assertEqual(0, self._get_resource_id_count(mock_table))
    ...
    def _get_resource_id_count(self, mock_table):
        response = mock_table.query(IndexName=(self.index_name), KeyConditionExpression=(Key('resourceId').eq(self.resource_id)))
        return response['Count']

    def _insert_token(self, mock_table):
        token_item = {'resourceCoordinate':self.coordinate, 
