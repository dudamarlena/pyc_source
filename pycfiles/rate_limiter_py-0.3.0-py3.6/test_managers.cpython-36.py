...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/test/test_managers.py
# Compiled at: 2019-12-30 13:04:32
# Size of source mod 2**32: 13760 bytes
import uuid, random
...
class FungibleTokenManagerTest(TestCase):
    ...
    def setUp(self):
        ...
        self.manager = FungibleTokenManager(self.token_table, self.limit_table, self.resource_name, self.limit, self.window)
    ...
    def test_get_bucket_token(self):
        ...
        expected_args = [
         {'Key':{'resourceName':self.resource_name, 
    ...
    def test_get_bucket_token_exhausted(self):
        ...
        self.manager._token_table = mock_token_table
        self.assertRaises(CapacityExhaustedException, self.manager._get_bucket_token, account_id, exec_time, self.ms_token)
...
class NonFungibleTokenManagerTest(TestCase):
    ...
    def test_get_reservation_exhausted(self):
        ...
        for i in range(0, self.limit):
            token = {'resourceCoordinate':coordinate, 
    ...
    def test_get_token_count(self):
        ...
        _insert_limit(mock_limit_table, self.resource_name, account_id, self.limit)
        expected_count = 1
        expired_token_1 = {'resourceCoordinate':coordinate, 
         'resourceName':self.resource_name, 
         'accountId':account_id, 
         'resourceId':random_string(), 
         'expirationTime':now, 
         'reservationId':random_string()}
        expired_token_2 = {'resourceCoordinate':coordinate, 
         'resourceName':self.resource_name, 
         'accountId':account_id, 
         'resourceId':random_string(), 
         'expirationTime':now - 1000, 
         'reservationId':random_string()}
        valid_token = {'resourceCoordinate':coordinate, 
         'resourceName':self.resource_name, 
         'accountId':account_id, 
         'resourceId':random_string(), 
...
def _insert_reservation(mock_token_table, reservation):
    reservation_item = {'resourceCoordinate':reservation.coordinate, 
...
def _insert_limit(mock_limit_table, resource_name, account_id, limit, window_sec=0):
    limit_item = {'resourceName':resource_name, 
     'accountId':account_id, 
     'limit':limit, 
