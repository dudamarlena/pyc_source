...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/test/test_limiters.py
...
class FungibleTokenLimiterDecoratorTest(BaseLimiterTest):
    ...
    def test_call_account_id_pos(self):
        ...
        account_id_pos = 2
        func_to_limit = Mock()
        limiter = rate_limit(self.resource_name, self.limit, self.window, self.token_table_name, self.limit_table_name, account_id_pos)
    ...
    def test_manager_config_ctor_params(self):
        limiter = rate_limit(self.resource_name, self.limit, self.window, self.token_table_name, self.limit_table_name)
    ...
    def test_manager_config_env_params(self):
        env_vars = {'FUNGIBLE_TABLE':str(self.token_table_name), 
...
class NonFungibleTokenLimiterContextManagerTest(BaseLimiterTest):
    ...
    def test_get_token_ctor_params(self, mock_manager_delegate):
        ...
        mock_manager_delegate.return_value = mock_manager
        with non_fungible_limiter(self.resource_name, self.account_id, self.limit, self.token_table_name, self.limit_table_name) as (reservation):
            mock_manager.get_reservation.assert_called_with(self.account_id)
            self.assertIs(empty_reservation, reservation)

    @patch('limiter.limiters.NonFungibleTokenManager')
    def test_get_token_env_params(self, mock_manager_delegate):
        env_vars = {'NON_FUNGIBLE_TABLE':self.token_table_name, 
