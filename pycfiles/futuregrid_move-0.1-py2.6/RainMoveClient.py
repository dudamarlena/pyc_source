...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/futuregrid_move/rain/move/RainMoveClient.py
# Compiled at: 2012-07-23 13:37:11
"""
...
import hashlib, logging, logging.handlers, socket, ssl, sys, time
...
class RainMoveClient(object):
    ...
    def connection(self):
        ...
        try:
            self._connMoveServer = ssl.wrap_socket(s, ca_certs=self._ca_certs, certfile=self._certfile, keyfile=self._keyfile, cert_reqs=ssl.CERT_REQUIRED, ssl_version=ssl.PROTOCOL_TLSv1)
...
def main():
    verbose = True
    validTypes = [
     'hpc', 'eucalyptus', 'openstack', 'nimbus', 'opennebula']
    rainmoveclient = RainMoveClient(verbose)
    parser = argparse.ArgumentParser(prog='fg-move', formatter_class=argparse.RawDescriptionHelpFormatter, description='FutureGrid Image Registration Help ')
    parser.add_argument('-u', '--user', dest='user', required=True, metavar='user', help='FutureGrid User name.')
    subparsers = parser.add_subparsers(dest='subparser_name', help='Positional arguments group different options that can be displayed by specifying <positional_argument> -h')
    ...
    group_node.add_argument('-a', '--add', nargs=4, metavar=('nodeId', 'hostname',
                                                             'ip', 'cluster'), help='Add new node to a cluster.')
    ...
    group_service.add_argument('-r', '--remove', nargs='+', metavar=('nodeId/s...',
                                                                     'nodeId/s... serviceId'), help='Remove node from a service.')
