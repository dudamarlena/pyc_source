...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-intel/egg/pg_python/_update.py
# Compiled at: 2019-10-12 05:49:41
import logging
...
def make_postgres_update_statement(table, kv_map, where_kv_map, clause, debug=True):
    ...
    return (
     statement, list(kv_map.values()) + list(where_kv_map.values()))
...
def make_postgres_update_multiple_statement(table, column_to_update, columns_to_query_lst, query_values_dict_lst, print_debug_log=True, typecast_suffix=''):
    """
    ...
    for dict_val in query_values_dict_lst:
        ...
        for key in dict_val['update']:
            if key not in columns_to_update_lst:
                logging.error("'%s' attribute to update isn't present in update list: %s" % (key, columns_to_update_lst))
                return False
...
def make_postgres_update_multiple_column_statement(table, columns_to_update_lst, columns_to_query_lst, query_values_dict_lst, print_debug_log=True):
    """
