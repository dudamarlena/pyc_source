...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-intel/egg/pg_python/pg_python.py
# Compiled at: 2019-10-12 05:53:19
...
def read(table, keys_to_get, kv_map, limit=None, order_by=None, order_type=None, clause='=', group_by=None, join_clause=' AND ', server='default', cols_keep_raw_type=[]):
    """
...
def update_multiple(table, column_to_update, columns_to_query_lst, query_values_dict_lst, server='default', typecast=''):
    """
    ...
    command, values = make_postgres_update_multiple_statement(table, column_to_update, columns_to_query_lst, query_values_dict_lst, print_debug_log, typecast_suffix=typecast)
    try:
        cursor.execute(command, values)
        return_dict = {'Status': True}
...
def update_multiple_col(table, columns_to_update_lst, columns_to_query_lst, query_values_dict_lst, server='default'):
    ...
    connection = db_obj.get_connection()
    cursor = db_obj.get_cursor()
    is_parameters_correct = check_parameters_multicol(columns_to_update_lst, columns_to_query_lst, query_values_dict_lst)
    if not is_parameters_correct:
        logging.error('ERROR in parameters passed')
        return {'status': False}
    command, values = make_postgres_update_multiple_column_statement(table, columns_to_update_lst, columns_to_query_lst, query_values_dict_lst, print_debug_log)
    try:
        cursor.execute(command, values)
        count = cursor.rowcount
        connection.commit()
