...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/coils/logic/task/get_todo.py
...
class GetToDoList(Command):
    ...
    def run(self, **params):
        ...
        query = db.query(Task).filter(and_(Task.executor_id.in_(self._ctx.context_ids), Task.state != '30_archived'))
        data = query.all()
