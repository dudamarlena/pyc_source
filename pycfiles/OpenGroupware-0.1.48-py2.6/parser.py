...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/coils/net/foundation/reports/parser.py
...
class Parser(object):
    ...
    def property_method_name(namespace, propname):
        return ('get_property_{0}_{1}').format(Parser.domain_from_namespace(namespace), propname.lower().replace('-', '_'))

    @staticmethod
    def property_set_method_name(namespace, propname):
        return ('set_property_{0}_{1}').format(Parser.domain_from_namespace(namespace), propname.lower().replace('-', '_'))
                ...
                for element in node.childNodes:
                    if element.nodeType == minidom.Node.ELEMENT_NODE:
                        ...
                        if namespace.upper() == 'DAV:':
                            ...
                            nm_ordinal += 1
                        set_props.append((Parser.property_set_method_name(namespace, propname),
                         namespace,
                         propname,
                         Parser.domain_from_namespace(namespace),
                         ('{0}:{1}').format(prefix, propname),
    ...
    def properties(document, user_agent_description):
        ...
        return (
         properties, dict(izip(namespaces.itervalues(), namespaces.iterkeys())))
    ...
    def lockinfo(payload, object_id, context_id):
        lockinfo = {'token': str(uuid.uuid4()), 'objectId': object_id, 
           'ownerId': context_id, 
           'expires': 0, 
           'granted': 0, 
           'kind': 'W', 
