...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/coils/logic/workflow/actions/xml/extentions.py
...
class OIEXSLTExtensionPoints:
    ...
    def _make_date(self, value, method, param):
        ...
        if len(value) == 10:
            ...
            raise CoilsException(('{0} received value {1} which is not a StandardXML formatted date: "{2}"').format(method, param, value))
        return value
    ...
    def messagetext(self, _, label):
        data = self.context.run_command('message::get-text', process=self.process, scope=self.scope, label=label)
        if not data:
            return ''
        return data
    ...
    def _search_for_objects(self, domain, args):
        ...
        result = self.context.run_command(('{0}::search').format(domain), criteria=criteria, contexts=self.ctxids)
        return result
    ...
    def tablelookup(self, _, name, *values):
        ...
        table.setup(context=self.context, process=self.process, scope=self.scope)
        return unicode(table.lookup_value(values))
    ...
    def xattrvalue(self, _, name):
        ...
        prop = self.context.property_manager.get_property(self.process, 'http://www.opengroupware.us/oie', ('xattr_{0}').format(name))
        if prop:
    ...
    def monthstart(self, _, year, month):
        try:
            ...
            raise CoilsException('Non-numeric year provided to oie:monthstart')

        try:
            month = int(month)
        except:
            raise CoilsException('Non-numeric month provided to oie:monthstart')
    ...
    def weekdays(self, _, start, end):
        ...
        rule = rrule(DAILY, byweekday=(MO, TU, WE, TH, FR), dtstart=start, until=end)
