...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/coils/core/service.py
# Compiled at: 2012-10-12 07:02:39
import sys, os, socket, base64, logging, yaml, traceback, gc
...
class Service(object):
    ...
    def send_administrative_notice(self, subject=None, message=None, urgency=9, category='unspecified', attachments=[]):
        try:
            self.send(Packet(None, 'coils.administrator/notify', {'urgency': urgency, 'category': category, 
               'subject': subject, 
               'message': message}))
        except Exception, e:
    ...
    def prepare(self):
        ...
        packet = Packet(('{0}/__hello_ack').format(self.__service__), ('coils.master.{0}/__hello').format(self.bus_host_name), self.__service__)
    ...
    def do___hello_ack(self, parameter, packet):
        if self.hello_uuid == packet.reply_to and str(packet.data) == self.__service__:
            if self.amq_debug or self.service_debug:
    ...
    def process(self, method, parameter, packet):
        ...
        if hasattr(self, method):
            ...
            try:
                ...
                self.send_administrative_notice(subject=('Message processing xception in component {0}').format(self.__service__), message=message, urgency=8, category='bus')
                raise e
            else:
                if self.amq_debug or self.service_debug:
