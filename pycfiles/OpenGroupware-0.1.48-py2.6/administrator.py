...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/coils/logic/foundation/services/administrator/administrator.py
...
class AdministratorService(Service):
    ...
    def send_notice(self, subject, message, urgency=5, category='unspecified', alert_id=None):
        if subject is None:
            subject = ('[OpenGrouware] Administrative Notice {0}').format(packet.uuid)
        else:
            subject = ('[OpenGrouware] {0}').format(subject)
    ...
    def do_notify(self, parameter, packet):
        try:
            ...
            self.send(Packet.Reply(packet, {'status': 500, 'text': 'Failure'}))
        else:
