...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/coils/protocol/bebrpc/api/customer.py
...
def render_contact(context, contact):
    response = {'objectid': contact.object_id, 'firstname': contact.first_name, 
    ...
    for telephone in contact.telephones:
        if telephone.kind == '01_tel':
            key = 'main'
        elif telephone.kind == '10_fax':
            key = 'fax'
        elif telephone.kind == '03_tel_func':
            key = 'mobile'
        else:
            continue
...
def render_enterprise(context, enterprise, contacts):
    response = {'objectid': enterprise.object_id, 'name': enterprise.name, 
...
class CustomerAPI(BEBAPI):
                ...
                if key in ('name', 'city', 'name1', 'street'):
                    ...
                    if value:
                        ...
                        if key == 'city':
                            ...
                            key = 'address.street'
                        criteria.append({'key': key, 'value': value, 
                           'expression': 'ILIKE', 
                           'conjunction': 'OR'})

            if criteria:
                enterprises = self.context.run_command('enterprise::search', criteria=criteria, contexts=context_ids, limit=50)
    ...
    def api_customer_fetch(self, args):
        if len(args) == 2:
            ...
            context_ids = self.translate_context_specification(args[1])
            if len(object_ids) > 0:
                enterprises = self.context.run_command('enterprise::get', ids=object_ids, contexts=context_ids, limit=50)
                contacts = self.context.run_command('enterprise::get-contacts', enterprises=enterprises)
                response = [ render_enterprise(self.context, e, contacts[e.object_id]) for e in enterprises ]
                return response
