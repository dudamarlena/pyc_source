...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/coils/protocol/horde/apis/turba.py
...
def render_contact(context, contact):
    ...
    response = {'key': key, 'birthday': contact.birth_date, 'department': contact.department, 
       'url': contact.URL, 
...
def translate_contact(context, contact, key=None):
    ...
    addresses = None
    for key in contact:
        if key in ('fullname', 'lastname', 'firstname', 'keywords', 'url', 'department',
                   'manager', 'gender', 'birthday', 'deathday', 'keywords', 'assistant',
                   'citizenship', 'birthplace', 'birthname'):
    ...
    if addresses is not None:
        ...
        for key in addresses:
            address = {'kind': key}
            if 'name1' in addresses[key]:
                address['name1'] = addresses[key]['name1']
            if 'name2' in addresses[key]:
                address['name2'] = addresses[key]['name2']
            if 'name2' in addresses[key]:
                address['name3'] = addresses[key]['name3']
            if 'street' in addresses[key]:
                address['street'] = addresses[key]['street']
            if 'city' in addresses[key]:
                address['city'] = addresses[key]['city']
            if 'province' in addresses[key]:
                address['province'] = addresses[key]['province']
            if 'postalcode' in addresses[key]:
                address['postalcode'] = addresses[key]['postalcode']
            if 'district' in addresses[key]:
                address['district'] = addresses[key]['district']
            if 'country' in addresses[key]:
        ...
        if isinstance(search_criteria, dict):
            ...
            if conjunction is not None:
                for criterion in search_criteria[conjunction]:
                    ...
                    criteria.append({'key': key, 'expression': 'ILIKE', 
