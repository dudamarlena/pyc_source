...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/coils/logic/twitter/api/core.py
...
class setup:
    ...
    def getUserTimeline(self, id=None, version=None, **kwargs):
        ...
        try:
            ...
            raise TwythonError("Failed with a %s error code. Does this user hide/protect their updates? If so, you'll need to authenticate and be their friend to get their timeline." % `(e.code)`, e.code)
    ...
    def showStatus(self, id, version=None):
        ...
        try:
            ...
            raise TwythonError("Failed with a %s error code. Does this user hide/protect their updates? You'll need to authenticate and be friends to get their timeline." % `(e.code)`, e.code)
    ...
    def destroyFriendship(self, id=None, user_id=None, screen_name=None, version=None):
        ...
        if self.authenticated is True:
            ...
            try:
                if id is not None:
                    return simplejson.load(self.opener.open('http://api.twitter.com/%d/friendships/destroy/%s.json' % (version, `id`), 'lol=1'))
                else:
    ...
    def showFriendship(self, source_id=None, source_screen_name=None, target_id=None, target_screen_name=None, version=None):
        ...
        """
        version = version or self.apiVersion
        apiURL = 'http://api.twitter.com/%d/friendships/show.json?lol=1' % version
    ...
    def createList(self, name, mode='public', description='', version=None):
        ...
        if self.authenticated is True:
            try:
                return simplejson.load(self.opener.open('http://api.twitter.com/%d/%s/lists.json' % (version, self.username), urllib.urlencode({'name': name, 'mode': mode, 'description': description})))
            except HTTPError, e:
    ...
    def updateList(self, list_id, name, mode='public', description='', version=None):
        ...
        if self.authenticated is True:
            try:
                return simplejson.load(self.opener.open('http://api.twitter.com/%d/%s/lists/%s.json' % (version, self.username, list_id), urllib.urlencode({'name': name, 'mode': mode, 'description': description})))
            except HTTPError, e:
    ...
    def updateProfileBackgroundImage(self, filename, tile='true', version=None):
        ...
        if self.authenticated is True:
            try:
                files = [
                 (
                  'image', filename, open(filename, 'rb').read())]
                fields = []
