...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/coils/foundation/api/elementflow/elementflow.py
...
def attr_str(attrs):
    if not attrs:
        return ''
    return ('').join([ ' %s=%s' % (k, quoteattr(v)) for (k, v) in attrs.iteritems() ])
...
class XMLGenerator(object):
    ...
    def container(self, name, attrs={}):
        ...
        """
        self.file.write('<%s%s>' % (name, attr_str(attrs)))
        self.stack.append(name)
        return self

    def element(self, name, attrs={}, text='', do_escape=True):
        """
        ...
        if text:
            if do_escape:
                text = escape(text)
            self.file.write('<%s%s>%s</%s>' % (name, attr_str(attrs), text, name))
        else:
            self.file.write('<%s%s/>' % (name, attr_str(attrs)))
...
class NamespacedGenerator(XMLGenerator):
    ...
    def _process_namespaces(self, name, attrs, namespaces):
        ...
        if namespaces:
            namespaces = dict([ ('xmlns:%s' % k if k else 'xmlns', v) for (k, v) in namespaces.iteritems()
                              ])
    ...
    def element(self, name, attrs={}, namespaces={}, text='', escape=True):
        (attrs, prefixes) = self._process_namespaces(name, attrs, namespaces)
        super(NamespacedGenerator, self).element(name, attrs, text, escape)
...
class IndentingGenerator(NamespacedGenerator):
    ...
    def element(self, name, attrs={}, namespaces={}, text='', escape=True):
        ...
        if escape:
            if len(text) > 70:
                fill = self._fill(text, indent + '  ')
                text = '%s\n%s' % (fill, indent)
        return super(IndentingGenerator, self).element(name, attrs, namespaces, text, escape)
