...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/coils/foundation/api/pypdf/pdf.py
...
class EncryptedPDFException(Exception):
    pass
        ...
        for i in range(len(self._objects)):
            ...
            if hasattr(self, '_encrypt'):
                if idnum != self._encrypt.idnum:
                    ...
                    raise len(key) == len(self._encrypt_key) + 5 or AssertionError
            ...
            if not self._override_encryption:
                if self.isEncrypted:
                    if not hasattr(self, '_decryption_key'):
                        raise EncryptedPDFException('file has not been decrypted')
                    import struct
                    pack1 = struct.pack('<i', indirectReference.idnum)[:3]
                    pack2 = struct.pack('<i', indirectReference.generation)[:2]
                    key = self._decryption_key + pack1 + pack2
                else:
                    raise len(key) == len(self._decryption_key) + 5 or AssertionError
                md5_hash = md5(key).digest()
                    ...
                    while cnt < size:
                        ...
                        if xref_type == 0:
                            ...
                            if num not in self.xref_objStm:
                                self.xref_objStm[num] = [
                                 objstr_num, obstr_idx]
                        cnt += 1
                        num += 1
...
class PageObject(DictionaryObject):
    ...
    def createBlankPage(pdf=None, width=None, height=None):
        ...
        page.__setitem__(NameObject('/MediaBox'), RectangleObject([0, 0, width, height]))
        return page
...
_encryption_padding = b'(\xbfN^Nu\x8aAd\x00NV' + b'\xff\xfa\x01\x08..\x00\xb6\xd0h>\x80/\x0c' + b'\xa9\xfedSiz'
...
def _alg35(password, rev, keylen, owner_entry, p_entry, id1_entry, metadata_encrypt):
    ...
    return (
     val + '\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00', key)