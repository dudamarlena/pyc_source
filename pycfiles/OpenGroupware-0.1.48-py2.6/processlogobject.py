...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/coils/protocol/dav/workflow/processlogobject.py
    ...
    def write_message_table(self, stream):
        ...
        for message in self.messages:
            ...
            if message.label is None:
                ...
                label = message.label
            stream.write(('<tr><td align="center"><a href="Messages/{0}">{1}</a></td><td align="center">{2}</td><td align="center">{3}</td><td align="center">{4}</td><td align="right">{5}</td><td align="center">{6}</td></tr>\n').format(message.uuid[1:-1], message.uuid, label, scope, message.version, message.size, message.mimetype))
