...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/coils/logic/workflow/services/manager.py
...
class ManagerService(Service):
            ...
            if process.route:
                ...
                if prop:
                    ...
                    if isinstance(value, basestring):
                        if value.upper() == 'YES':
                            singleton = True
            self.ps[process.object_id] = {'processId': process.object_id, 'contextId': process.owner_id, 
               'status': '?', 
    ...
    def manager_request_process_start(self, pid, cid):
        ...
        self.send(Packet('coils.workflow.manager/is_running', 'coils.workflow.executor/start', {'processId': pid, 'contextId': cid}))
    ...
    def manager_scan_running_processes(self):
        ...
        if running_pids:
            ...
            for pid in running_pids:
                self.send(Packet('coils.workflow.manager/is_running', ('coils.workflow.executor/is_running:{0}').format(pid), None))
                if pid not in self.ps:
    ...
    def manager_start_queued_processes(self):
        ...
        try:
            ...
            query = db.query(Process, op1, op2).join(Route, Route.object_id == Process.route_id).outerjoin(op1, and_(op1.parent_id == Route.object_id, op1.namespace == 'http://www.opengroupware.us/oie', op1.name == 'singleton')).outerjoin(op2, and_(op2.parent_id == Route.object_id, op2.namespace == 'http://www.opengroupware.us/oie', op2.name == 'routeGroup')).filter(and_(Process.state.in_(['Q']), Process.status != 'archived')).order_by(Process.priority.desc(), Process.object_id).limit(150)
    ...
    def manager_detect_zombie(self, process_id):
        ...
        if process:
            if process.state == 'R':
                ...
                process_messages = [ ('    UUID#{0} \n      Version:{1} Size:{2} Label:{3}').format(x.uuid, x.version, x.size, x.label) for x in process_messages
                                   ]
                message = ('objectId#{0} [Process] determined to be in zombie state.\n  Route: {1} "{2}"\n  Owner: {3} "{4}"\n  Version: {5}\n  Messages:\n{6}').format(process.object_id, route_id, route_name, process.owner_id, owners_name, process.version, ('\n').join(process_messages))
                self.send_administrative_notice(category='workflow', urgency=3, subject=('OGo#{0} [Process] Flagged As Zombie').format(process_id), message=message)
                process.state = 'Z'
    ...
    def do_failed(self, parameter, packet):
        ...
        if process:
            if process.state == 'R':
                ...
                self.send_administrative_notice(category='workflow', urgency=4, subject='Defunct OIE Worker Detected', message=('Detected OGo#{0} [Process] in defunct state, process will be failed.').format(process_id))
                process.state = 'F'
                self._ctx.commit()
                self.send(Packet(None, 'coils.workflow.logger/log', {'process_id': process_id, 'message': 'Manager set state of process to failed.'}))
        self.manager_deregister_process(process_id)
        return
    ...
    def do_queue(self, parameter, packet):
        process_id = int(parameter)
        self.send(Packet(None, 'coils.workflow.logger/log', {'process_id': process_id, 'message': 'Request to place in queued state.'}))
        process = self._ctx.run_command('process::get', id=process_id)
        if process:
            if process.state in ('I', 'H', 'P'):
                ...
                self.send(Packet(None, 'coils.workflow.logger/log', {'process_id': process_id, 'message': 'Process is already in queued state'}))
                self.send(Packet.Reply(packet, {'status': 201, 'text': 'OK, No action.'}))
            else:
                self._ctx.rollback()
                message = ('OGo#{0} [Process] cannot be queued from state "{1}"').format(process_id, process.state)
                self.send(Packet(None, 'coils.workflow.logger/log', {'process_id': process_id, 'message': message}))
