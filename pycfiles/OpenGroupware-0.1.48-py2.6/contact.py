...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/coils/foundation/alchemy/contact.py
# Compiled at: 2012-10-12 07:02:39
from sqlalchemy import *
from base import Base, KVC
from company import CompanyInfo, Telephone
...
class Contact(Base, KVC):
    ...
    object_id = Column('company_id', ForeignKey('log.object_id'), ForeignKey('object_acl.object_id'), primary_key=True)
    ...
    _info = relation('CompanyInfo', uselist=False, backref=backref('company_info_contact'), primaryjoin='CompanyInfo.parent_id==Contact.object_id')
    ...
    def __repr__(self):
        return ('<Contact objectId="{0}" version="{1}" displayName="{2}", login="{3}"/>').format(self.object_id, self.version, self.get_display_name(), self.login)