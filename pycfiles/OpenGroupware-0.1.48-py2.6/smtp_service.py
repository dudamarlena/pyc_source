...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/coils/net/smtp_service.py
    ...
    def send_to_collection(self, object_id, message):
        ...
        def authenticate_collection(object_id, message):
            ...
            if collection is None:
                ...
                if collection.auth_token == 'published':
                    return collection
        ...
        def retrieve_route():
            ...
            if route is None:
                ...
                self.send_administrative_notice(subject='Unable to marshall route requested via SMTP', message=message, urgency=7, category='workflow', attachments=[])
                return
            else:
                return route
        ...
        if collection is not None:
            ...
            if route is not None:
                ...
                g.flatten(message)
                tmp.seek(0)
                process = self._ctx.run_command('process::new', values={'route_id': route.object_id, 'handle': tmp, 
                   'priority': 210})
                process.owner_id = collection.owner_id
                self._ctx.property_manager.set_property(process, 'http://www.opengroupware.us/oie', 'xattr_collectionid', str(object_id))
                self.ctx.commit()
    ...
    def create_process(self, from_, to_, message):
        ...
        if route:
            ...
            if message.is_multipart():
                ...
                self.send_administrative_notice(subject='Multipart message submitted to workflow', message=message, urgency=8, category='workflow', attachments=[])
            else:
                try:
                    ...
                    process = self._ctx.run_command('process::new', values={'route_id': route.object_id, 'data': payload, 
                       'priority': 210})
                    self._ctx.property_manager.set_property(process, 'http://www.opengroupware.us/oie', 'xattr_from', from_)
                    self._ctx.commit()
