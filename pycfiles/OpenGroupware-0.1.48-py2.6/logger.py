...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/coils/logic/workflow/services/logger.py
...
from coils.logic.workflow.utility import filename_for_process_log, read_cached_process_log, delete_cached_process_logs, cache_process_log
...
class LoggerService(Service):
    ...
    def prepare(self):
        ...
        self.send(Packet('coils.workflow.logger/ticktock', 'coils.clock/subscribe', None))
        return

    def _log_message(self, source, process_id, message, stanza=None, timestamp=None, category='undefined', uuid=None):
        try:
            if timestamp is None:
                timestamp = float(time())
            self._log_queue[uuid4().hex] = (
             source, process_id, message.strip(), stanza, timestamp, category, uuid)
    ...
    def _empty_queue(self):
        ...
        try:
            for key in self._log_queue:
                ...
                entry = ProcessLogEntry(source, process_id, message, stanza=stanza, timestamp=timestamp, category=category, uuid=uuid)
            ...
            self.log.exception(e)
            self.send_administrative_notice(subject=subject, message=message, urgency=9, category='workflow')
        else:
            try:
    ...
    def do_log(self, parameter, packet):
        try:
            ...
            if self._log_message(source, process_id, message, stanza=stanza, timestamp=timestamp, category=category, uuid=uuid):
    ...
    def do_reap(self, parameter, packet):
        ...
        try:
            db.query(ProcessLogEntry).filter(ProcessLogEntry.process_id == process_id).delete(synchronize_session='fetch')
        except Exception, e:
            self.log.exception(e)
