...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/coils/logic/task/get_archived.py
...
class GetArchivedList(Command):
    ...
    def run(self, **params):
        ...
        query = db.query(Task).filter(and_(Task.owner_id.in_(self._ctx.context_ids), Task.state == '30_archived'))
        data = query.all()
