...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/coils/logic/schedular/get_conflicts.py
...
class GetConflicts(Command):
    ...
    def parse_parameters(self, **params):
        ...
        if params.has_key('appointment'):
            ...
            for p in x.participants:
                if p.participant_status != 'DECLINED' or p.participant_role != 'NON-PARTICIPANT':
    ...
    def participant_conflict_query(self):
        ...
        query = db.query(Appointment, Participant).filter(Appointment.object_id == Participant.appointment_id).filter(and_(Appointment.object_id != self.objid, Appointment.status != 'archived', Appointment.end > self.start, Appointment.start < self.end, or_(Appointment.conflict_disable is None, Appointment.conflict_disable == 0), and_(Participant.participant_id.in_(self.parts), Participant.participant_status != 'DECLINED', Participant.participant_role != 'NON-PARTICIPANT')))
        return query
