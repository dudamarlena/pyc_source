...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/coils/logic/workflow/services/scheduler.py
...
class SchedulerService(Service):
    ...
    def service_start_process(self, uuid, route_id, context_id, attachment_id, xattr_dict):
        ...
        try:
            try:
                ...
                if route:
                    ...
                    if attachment_id:
                        ...
                        if attachment:
                            ...
                            message = ('Scheduling indicates that a process should be created but the attachment containting the input data could not be found.\n RouteId#{0}\n ContextId#{1}\n UUID:{2}\n Attachment UUID: {3}\n').format(route_id, context_id, uuid, attachment_id)
                            self.send_administrative_notice(category='workflow', urgency=7, subject='Unable to queue scheduled process; no attachment', message=message)
                    ...
                    if handle:
                        ...
                        process = ctx.run_command('process::new', values={'route_id': route.object_id, 'priority': 200}, handle=handle, mimetype=attachment.mimetype)
                    else:
                        self.log.debug('Creating scheduled process with no input data.')
                        process = ctx.run_command('process::new', values={'route_id': route.object_id, 'priority': 200}, data='', mimetype='text/plain')
                    ...
                    for (key, value) in xattr_dict.items():
                        ...
                        if value is None:
                            ...
                            value = str(value)
                        ctx.property_manager.set_property(process, 'http://www.opengroupware.us/oie', key, value)
                    ...
                    message = ('Scheduling indicates that a process should be created but the specified route could not be found.\n RouteId#{0}\n ContextId#{1}\n UUID:{2}\n Length of Input Message: {3}\n').format(route_id, context_id, uuid, len(input_data))
                    self.send_administrative_notice(subject='Route not available to create scheduled process', message=message, urgency=8, category='workflow')
                ...
                message = ('An exception occurred in executing a scheduler entry.\n Exception: {0}\n RouteId#{1}\n ContextId#{2}\n UUID:{3}\n Attachment UUID: {4}\n').format(e, route_id, context_id, uuid, attachment_id)
                self.send_administrative_notice(category='workflow', urgency=9, subject='Exception creating workflow process from schedule', message=message)
    ...
    def prepare(self):
        ...
        self.scheduler.add_jobstore(CoilsAlchemyJobStore(engine=self._ctx.db_session().connection().engine), 'backend')
    ...
    def do_list_jobs(self, parameter, packet):
        ...
        for job in self.service_get_jobs(context_id, route_id):
            ...
            if max_runs:
                ...
                remaining_runs = -1
            entry = {'UUID': uuid, 'routeId': route_id, 'contextId': context_id, 
            ...
            if isinstance(job.trigger, SimpleTrigger):
                entry.update({'type': 'simple', 'date': job.trigger.run_date})
            elif isinstance(job.trigger, IntervalTrigger):
                entry.update({'type': 'interval', 'interval': job.trigger.interval.seconds, 
        ...
        self.send(Packet.Reply(packet, {'status': 200, 'schedule': result}))
    ...
    def do_schedule_job(self, parameter, packet):
        ...
        try:
            ...
            if trigger == 'simple':
                ...
                self.scheduler.add_date_job(func=enqueue_process, date=packet.data.get('date'), max_runs=repeats, name=packet.uuid, jobstore='backend', args=(
                 packet.uuid,
                 route_id,
                 context_id,
                 attachment_id,
                ...
                self.scheduler.add_interval_job(func=enqueue_process, name=packet.uuid, max_runs=repeats, weeks=int(packet.data.get('weeks', 0)), days=int(packet.data.get('days', 0)), hours=int(packet.data.get('hours', 0)), minutes=int(packet.data.get('minutes', 0)), seconds=int(packet.data.get('seconds', 0)), start_date=packet.data.get('start', None), jobstore='backend', args=(
                 packet.uuid,
                 route_id,
                 context_id,
                 attachment_id,
                ...
                self.scheduler.add_cron_job(func=enqueue_process, name=packet.uuid, max_runs=repeats, year=str(packet.data.get('year', '*')), month=str(packet.data.get('month', '*')), day=str(packet.data.get('day', '*')), day_of_week=str(packet.data.get('dayOfWeek', '*')), hour=str(packet.data.get('hour', '*')), minute=str(packet.data.get('minute', '*')), jobstore='backend', args=(
                 packet.uuid,
                 route_id,
                 context_id,
                 attachment_id,
            ...
            self.log.exception(e)
            self.send(Packet.Reply(packet, {'status': 500, 'text': unicode(e), 
               'UUID': None}))
        else:
            self.send(Packet.Reply(packet, {'status': 200, 'text': 'Process scheduled OK', 
               'UUID': packet.uuid}))
    ...
    def do_unschedule_job(self, parameter, packet):
        ...
        if not job_id:
            self.send(Packet.Reply(packet, {'status': 500, 'text': 'No UUID specified in unschedule_job message'}))
            return
        context_id = packet.data.get('contextId', 8999)
        if self.service_cancel_job(job_id=job_id, context_id=context_id):
            self._ctx.db_session().commit()
            self.send(Packet.Reply(packet, {'status': 200, 'UUID': job_id, 
               'text': 'Job cancelled.'}))
        else:
            self.send(Packet.Reply(packet, {'status': 404, 'UUID': job_id, 
               'text': ('No such job as "{0}"').format(job_id)}))