...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/coils/foundation/defaultsmanager.py
...
class UserDefaultsManager(DefaultsManager):
    ...
    def _read_user_defaults(self):
        plist = BLOBManager.Open(('documents/{0}.defaults').format(self._account_id), 'rb', encoding=DefaultsManager.__encoding__)
    ...
    def _write_user_defaults(self):
        ...
        plist = BLOBManager.Open(('documents/{0}.defaults').format(self._account_id), 'wb', encoding=DefaultsManager.__encoding__, create=True)
        plist.write(data)
        plist.close()
        return old_defaults
...
class ServerDefaultsManager(DefaultsManager):
    ...
    def _read_server_defaults(self):
        blob = BLOBManager.Open('.server_defaults.pickle', 'rb', encoding='binary')
        if blob is not None:
            data = blob.read()
            blob.close()
            defaults = PickleParser().propertyListFromString(data)
        else:
            blob = BLOBManager.Open('.libFoundation/Defaults/NSGlobalDomain.plist', 'rb', encoding=DefaultsManager.__encoding__)
            if blob is not None:
                data = blob.read()
                blob.close()
                defaults = PListParser().propertyListFromString(data)
            else:
                raise Exception('Unable to load server defaults; file {0} does not exist.')
    ...
    def orm_dsn(self):
        ...
        return ('postgresql://{0}@{1}:{2}/{3}').format(conndict.get('userName'), conndict.get('hostName'), conndict.get('port'), conndict.get('databaseName'))
