...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/coils/logic/vista/services/index.py
...
def parse_keywords(value, delimiter=' '):
    ...
    if value:
        if isinstance(keywords, basestring):
            keywords = [ x.strip().lower() for x in value.split(delimiter) if len(x) < 128
                       ]
        elif isinstance(value, list):
            keywords = [ x.strip().lower() for x in value if len(x) < 128
                       ]
    return keywords
...
class IndexService(Service):
    ...
    def do_log_scan(self, parameter, packet):
        ...
        query = db.query(AuditEntry.context_id, func.max(AuditEntry.datetime)).filter(AuditEntry.datetime > floor).group_by(AuditEntry.context_id)
    ...
    def do_vacuum(self, parameter, packet):
        db = self._ctx.db_session()
        query = db.query(SearchVector.object_id).outerjoin(ObjectInfo, ObjectInfo.object_id == SearchVector.object_id).filter(ObjectInfo.object_id == None)
        result = query.all()
        count = len(result)
    ...
    def do_index(self, parameter, packet):
        try:
            ...
            self.send(Packet.Reply(packet, {'status': 500, 'text': 'Invald objectId specified.'}))
            return
        else:
            ...
            if entity:
                if not hasattr(self, ('_index_{0}').format(entity.__entityName__.lower())):
                    message = ('Entity type {0} not supported by index.').format(entity.__entityName__)
                    self.log.debug(message)
                    self.send(Packet.Reply(packet, {'status': 403, 'text': message}))
                    return
            else:
                message = ('Cannot marshal objectId#{0}').format(object_id)
                self.log.debug(message)
                self.send(Packet.Reply(packet, {'status': 403, 'text': message}))
                return
            ...
            try:
                (vector_entry.keywords, vector_entry.archived, vector_entry.event_date, text) = index_function(entity)
            except UnicodeEncodeError, e:
                self.send(Packet.Reply(packet, {'status': 500, 'text': 'Unicode error encountered indexing object.'}))
                self._ctx.db_session().rollback()
                self.send_administrative_notice(subject=('Unicode error encountered indexing OGo#{0} [{1}]').format(entity.object_id, entity.__entityName__), message=traceback.format_exc(), urgency=5, category='data')
                return
            except AttributeError, e:
                self.send(Packet.Reply(packet, {'status': 500, 'text': 'Attribute error encountered indexing object.'}))
                self._ctx.db_session().rollback()
                self.send_administrative_notice(subject=('Attribute access error encountered indexing OGo#{0} [{1}]').format(entity.object_id, entity.__entityName__), message=traceback.format_exc(), urgency=7, category='model')
            except Exception, e:
                self.send(Packet.Reply(packet, {'status': 500, 'text': 'General exception encountered indexing object.'}))
                self._ctx.db_session().rollback()
                self.send_administrative_notice(subject=('A general exception occurred while indexing OGo#{0} [{1}]').format(entity.object_id, entity.__entityName__), message=traceback.format_exc(), urgency=9, category='vista')
            else:
    ...
    def _index_task(self, task):
        ...
        return (
         keywords, archived, event_date, stream.getvalue())
    ...
    def _index_note(self, note):
        ...
        try:
            ...
            self.send_administrative_notice(subject=('Content encoding error indexing noteId#{0}').format(note.object_id), message=('{0}\n{1}').format(e, traceback.format_exc()), urgency=4, category='document')
