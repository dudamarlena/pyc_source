...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/scrumtools/trello.py
...
class TrelloController(controller.CementBaseController):
    ...
    class Meta:
        ...
        config_defaults = dict(auth_key=None, auth_token=None, organization='example.org', board_admins='example', board_pattern='example.g%02d', board_admins_group=0, board_lists=[
         'Product Backlog', 'To Do', 'Doing', 'Done'])
        arguments = [
         (
          [
           '-U', '--users-file'],
          dict(action='store', metavar='FILE', dest='users_file', help='a CSV file listing all users')),
         (
          [
           '-O', '--organization'],
          dict(action='store', metavar='NAME', dest='organization', help='the organization managing the Trello repositories')),
         (
          [
           '-C', '--card-name'],
          dict(action='store', metavar='NAME', dest='card_name', help='name of a card to be added')),
         (
          [
           '-D', '--card-description'],
          dict(action='store', metavar='NAME', dest='card_description', default=None, help='description of a card to be added')),
         (
          [
           '-L', '--card-list'],
          dict(action='store', metavar='NAME', dest='card_list', default='Product Backlog', help='name of the list (in all boards) to add this card to'))]
    ...
    def authorize(self):
        ...
        if not self.app.config.get('trello', 'auth_key'):
            raise error.ConfigError("Missing config parameter 'trello.auth_key'! Please run 'scrum-tools trello authorize' first! ")
        try:
            tl = TrelloApi(self.app.config.get('trello', 'auth_key'))
            url = tl.get_token_url('scrum-tools', expires='30days', write_access=True)
            cprint(os.linesep.join(['Please follow the link below and update these entries in the [trello] section of your scrum-tools config: ',
    ...
    def validate_users(self):
        ...
        if not self.app.config.get('trello', 'auth_key') or not self.app.config.get('trello', 'auth_token'):
            raise error.ConfigError("Missing config parameter 'trello.auth_key' and/or 'trello.auth_token'! Please run 'scrum-tools trello authorize' first! ")
    ...
    def create_card(self):
        ...
        for board in boards:
            ...
            if card_list not in board_lists:
    ...
    def __add_board_member(self, board, member, member_type):
        ...
        print colored("Adding '%s' as %s member of '%s'..." % (member, member_type, board['name']), 'green', attrs=['bold']),
        try:
