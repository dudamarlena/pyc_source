...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/scrumtools/github.py
...
class GitHubController(controller.CementBaseController):
    ...
    class Meta:
        ...
        config_defaults = dict(auth_id=None, auth_token=None, organization='example.org', team_admins='example.admins', team_admins_group=-1, team_users='example.users', team_pattern='example.g%02d', repo_admins='example', repo_users='example', repo_pattern='example.g%02d')
        arguments = [
         (
          [
           '-U', '--users-file'],
         ...
         (
          [
           '-O', '--organization'],
          dict(action='store', metavar='NAME', dest='organization', help='the organization managing the GitHub repositories'))]
    ...
    def authorize(self):
        ...
        try:
            ...
            au = gh.authorize(username, password, scopes=[
             'repo', 'delete_repo', 'admin:org'], note='Scrum-tools on %s' % socket.gethostname())
    ...
    def validate_users(self):
        ...
        if not self.app.config.get('github', 'auth_token') or not self.app.config.get('github', 'auth_id'):
            raise error.ConfigError("Missing config parameter 'github.auth_id' and/or 'github.auth_token'! Please run 'scrum-tools github authorize' first! ")
    ...
    def create_teams(self):
        ...
        repo_names = [
         '%s/%s' % (organization, repo_admins)] + ['%s/%s' % (organization, repo_users)] + [ '%s/%s' % (organization, repo_pattern % int(group)) for group in user_repository.groups() ]
