...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\sc2gameLobby\cli.py
# Compiled at: 2018-09-27 00:44:14
...
def optionsParser(passedParser=None):
    ...
    startOption.add_argument('-l', '--ladder', default='versentiedge', choices=ALLOWED_LADDERS, help=('the ladder where the game is run.  Allowed values: ' + ', '.join(ALLOWED_LADDERS)),
      metavar='')
    startOption.add_argument('-p', '--player', default='defaulthuman', choices=ALLOWED_PLAYERS, help=('the player profile the ladder identifies you with.  Allowed values: ' + ', '.join(ALLOWED_PLAYERS)),
      metavar='')
    startOption.add_argument('-v', '--version', type=int, help='the specific game version to play.', metavar='')
    startOption.add_argument('-e', '--exp', default='lotv', choices=(c.EXPO_SELECT.keys()), metavar='', help=('the specific game expansion version to play.  Allowed values: ' + ', '.join(c.EXPO_SELECT.keys())))
    startOption.add_argument('-r', '--race', default=(c.RANDOM), choices=(t.SelectRaces.ALLOWED_TYPES), metavar='', help=('selected race to play. Allowed values: ' + ', '.join(t.SelectRaces.ALLOWED_TYPES)))
    startOption.add_argument('-m', '--mode', default=(c.MODE_1V1), choices=(t.GameModes.ALLOWED_TYPES), metavar='', help=('game mode to play. DEFAULT: %s' % c.MODE_1V1))
