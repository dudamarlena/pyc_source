...
# [GCC 8.4.0]
# Embedded file name: /home/viator/coding/code/microservices/microservices/http/renderers.py
# Compiled at: 2017-03-07 08:46:10
# Size of source mod 2**32: 7276 bytes
import json, six
from flask import current_app, request
from flask.json import JSONEncoder
from flask_api.renderers import BrowsableAPIRenderer, JSONRenderer
from microservices.http.helpers import url_resource, get_url_rule, get_rule_resource
...
class SchemaRenderer(object):
    ...
    def update_by_request(self, response):
        ...
        if request_name is not None and request.data:
    ...
    def update_by_resources(self, response):
        ...
        if resources_name is not None:
            ...
            for another_resource in [value for value in current_app.resources.values() if value.get('in_resources') is not None and value['rule'] != self.resource['rule']]:
                ...
                if resource_url:
                    ...
                    another_resources_info[another_resource['rule']] = {field:another_resource[field] for field in another_resource.get('in_resources', []) if field in another_resource}
