...
# [GCC 8.4.0]
# Embedded file name: /home/viator/coding/code/microservices/microservices/queues/client.py
# Compiled at: 2017-03-07 09:44:21
# Size of source mod 2**32: 8369 bytes
...
class _exchange(object):
    ...
    def __init__(self, client, name, routing_key=None, logger=None):
        ...
        if logger is None:
            logger = _logger
        self.logger = logger
        self.logger.debug('Exchange "%s" built, routing_key: %s', self.name, self.routing_key if self.routing_key is not None else '')
...
class Client(object):
    __doc__ = 'Client for queue brokers, kombu based'
    ...
    def publish_to_exchange(self, name, routing_key, message, **properties):
        ...
        with self.connections[self.connection].acquire() as (conn):
            ...
            self.logger.info('Message (len: %s) was published to exchange "%s" with routing_key "%s"', len(message), name, routing_key if routing_key else '')
            return result
