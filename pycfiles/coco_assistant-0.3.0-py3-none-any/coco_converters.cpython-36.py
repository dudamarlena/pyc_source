...
# [GCC 8.4.0]
# Embedded file name: /home/ashwin/Desktop/Projects/COCO-Assistant/coco_assistant/coco_converters.py
# Compiled at: 2019-11-27 12:19:53
# Size of source mod 2**32: 5345 bytes
import logging, os
...
def load_coco_detection_dataset(imgs_dir, annotations, shuffle_img=True):
    ...
    for index, img_id in enumerate(img_ids):
        ...
        try:
            ...
            logging.warning('Image dimension is missing from the image field. Proceeding to read it manually')
        ...
        for ann in anns:
            ...
            bboxes_data = [
             bboxes_data[0] / float(pic_width), bboxes_data[1] / float(pic_height),
...
def dict_to_coco_example(img_data):
    ...
    Args:
        img_data: infomation of one image, inclue bounding box, labels of bounding box,            height, width, encoded pixel data.
    Returns:
    ...
    example = tf.train.Example(features=tf.train.Features(feature={'image/height':dataset_util.int64_feature(img_data['height']), 
...
def convert(ann, img_dir, _format):
    dst = os.path.join(os.path.dirname(os.path.dirname(img_dir)), 'annotations', os.path.basename(img_dir) + '.tfrecord')
    if _format == 'TFRecord':
