...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gmalthgtparser/parser.py
# Compiled at: 2017-06-04 13:16:59
""" Provide classes to parse HGT file or iterate over the values in these files """
import os, re, struct, math, fractions
...
class HgtParser(object):
    ...
    def _get_corners_from_filename(self, bottom_left_corner):
        ...
        """
        bottom_left = (
         bottom_left_corner[0] - self.square_height / 2, bottom_left_corner[1] - self.square_width / 2)
    ...
    def is_inside(self, point):
        ...
        :rtype: bool
        """
        return self.corners[0][0] < point[0] and self.corners[0][1] < point[1] and point[0] < self.corners[2][0] and point[1] < self.corners[2][1]
...
class HgtValueIterator(HgtBaseIterator):
    ...
    def next(self):
        ...
        if current_idx < self.parser.sample_lat * self.parser.sample_lng:
            ...
            self.idx += 1
            return (
             line, col, current_idx, self.format_corners(square), self.parser.get_value(current_idx))
        raise StopIteration()
