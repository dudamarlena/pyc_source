...
def gdb_launch(gdbname, port, fname, run_timeout=30, post_commands=[], pre_commands=[]):
    ...
    gdb.expect('Reading symbols.*\\n')
    gdb.expect('.*\\(gdb\\) ')
    gdb.sendline('load')
    while True:
        a = gdb.expect(['Loading section.*\\n', 'Start address.*\\n', 'Error finishing flash operation.*\\n'])
        if a == 1:
            break
        if a == 2:
            gdb.expect('.*\\(gdb\\) ')
...
def pic32mx250f128b(fname, doMeasure=True):
    ...
    tf.write(':00000001FF')
    tf.close()
    foreground_proc(('{} -S -p {}').format(tool_config['tools']['pic32prog'], tf.name))
    sleep(0.2)
    if len(fname) > 4 and fname[-4:] == '.hex':
        tfname = fname
    else:
        foreground_proc(('{} -O ihex {} {}').format(tool_config['tools']['pic32_objcopy'], fname, tf.name))
        tfname = tf.name
    em = setupMeasurement('pic32mx250f128b')
    foreground_proc(('{} -S -p {}').format(tool_config['tools']['pic32prog'], tfname))
    os.unlink(tf.name)
...
def main():
    ...
    if arguments['--verbose'] == 1:
        ...
        try:
            ...
            for i in range(int(arguments['--repeat'])):
                m = run(arguments['PLATFORM'], arguments['EXECUTABLE'], arguments['--no-measure'] is False)
                ms.append(m)
    ...
    if arguments['--no-measure'] is False:
        if arguments['--csv']:
        ...
        elif arguments['-s'] and hasattr(m, 'measurements'):
            s = '       '
            ...
            print s
            names = [
             'Energy', 'Time', 'Power', 'Average current', 'Average voltage']
