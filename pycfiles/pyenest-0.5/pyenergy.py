...
# Embedded file name: build/bdist.linux-x86_64/egg/pyenergy/pyenergy.py
# Compiled at: 2015-02-04 07:41:55
...
class MeasurementSet(object):
    ...
    def __init__(self, measurements):
        ...
        self.peak_power = sum(self.measurements).peak_power
    ...
    def _op_self(self, other, fn):
        params = [
         'energy', 'time', 'peak_power', 'peak_voltage', 'peak_current',
    ...
    def __repr__(self):
        keys = [
         'energy', 'time', 'peak_power', 'peak_voltage', 'peak_current', 'n_samples', 'avg_voltage', 'avg_current', 'avg_power']
...
class Measurement(object):
    ...
    def __init__(self, energy=0, time=0, peak_power=0, peak_voltage=0, peak_current=0, n_samples=0, avg_voltage=0, avg_current=0, avg_power=0):
...
class EnergyMonitor(object):
    ...
    def getBoards():
        ...
        devs = list(usb.core.find(idVendor=62777, idProduct=62777, find_all=True))
        return devs
    ...
    def getVersion(dev):
        ...
        try:
            b = dev.ctrl_transfer(195, 12, 0, 0, 4)
        except usb.core.USBError as e:
            try:
                b = dev.ctrl_transfer(193, 12, 0, 0, 4)
            except usb.core.USBError as e:
                if e.errno == 32:
                    return 0
                raise
    ...
    def getSerial(dev):
        ...
        b = dev.ctrl_transfer(195, 13, 0, 0, 4)
        serial = unpack('=4s', b)[0]
        return serial

    def toggleLEDs(self):
        """Toggle some of the LEDs on the board."""
        self.dev.ctrl_transfer(67, 0, 0, 0, None)
        return
    ...
    def getNumberOfRuns(self, m_point=1):
        ...
        b = self.dev.ctrl_transfer(195, 9, int(m_point), 0, 4)
        runs = unpack('=L', b)
    ...
    def clearNumberOfRuns(self, m_point=1):
        ...
        self.dev.ctrl_transfer(67, 10, int(m_point), 0, None)
        return
    ...
    def setSerial(self, ser):
        ...
        if len(ser) != 4:
            ...
            ser = (ser + '0000')[:4]
        self.dev.ctrl_transfer(67, 3, ord(ser[0]) | ord(ser[1]) << 8, ord(ser[2]) | ord(ser[3]) << 8, None)
        return
    ...
    def setTrigger(self, port, m_point=1):
        ...
        self.dev.ctrl_transfer(67, 4, ord(port[1]) | m_point << 8, int(port[2]), None)
        return
    ...
    def enableMeasurementPoint(self, m_point, adc=None):
        ...
        if m_point in self.adcMpoint:
            ...
            self.dev.ctrl_transfer(67, 7, int(m_point), adc, None)
            return
    ...
    def convertData(self, md, resistor, gain, vref, samplePeriod):
        ...
        """
        try:
            en = float(vref) ** 2 / gain / resistor / 16777216 * 2 * samplePeriod * 2 / 168000000.0 * md.energy_accum * 2
            el = md.elapsed_time * 2.0 / 168000000 * 2
            pp = float(vref) ** 2 / gain / resistor / 16777216 * md.peak_power * 2
            pv = float(vref) / 4096.0 * md.peak_voltage * 2
            pi = float(vref) / gain / resistor / 4096.0 * md.peak_current
            av = float(vref) / 4096.0 * md.avg_voltage / md.n_samples * 2
            ai = float(vref) / gain / resistor / 4096.0 * md.avg_current / md.n_samples
            ap = en / el
    ...
    def getMeasurement(self, m_point=1):
        ...
        """
        info('getMeasurement')
        b = self.dev.ctrl_transfer(195, 6, int(m_point), 0, calcsize(EnergyMonitor.MeasurementData_packing))
    ...
    def getInstantaneous(self, m_point=1):
        ...
        """
        b = self.dev.ctrl_transfer(195, 11, int(m_point), 0, calcsize(EnergyMonitor.InstantaneousData_packing))
    ...
    def debugInstantaneous(self, v):
        ...
        print 'Timestamp:', v[4] * 2.0 / 168000000 * 2
        print ('Current:  Raw={:4d}  Voltage@{}={:1.3f}V  Res Vdrop={:1.5f}V  Current={:1.5f}A').format(mp, EnergyMonitor.port_mappings[mp][1], v[3] / 4096.0 * vref, float(vref) / gain / 4096.0 * v[3], float(vref) / gain / resistor / 4096.0 * v[3])
        print ('Voltage:  Raw={:4d}  Voltage@{}={:1.3f}V                      Voltage={:1.5f}V').format(v[2], EnergyMonitor.port_mappings[mp][0], v[2] / 4096.0 * vref, float(vref) / 4096.0 * v[2] * 2)
        print ''
