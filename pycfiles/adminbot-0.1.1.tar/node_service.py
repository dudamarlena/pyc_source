...
# Embedded file name: build/bdist.linux-x86_64/egg/cloud_admin/services/node_service.py
    ...
    if not nodes:
        ...
        for node in nodes:
            if isinstance(node, EucaNodeService):
                ...
                raise ValueError(('show_nodes: Unknown type for node: "{0}:{1}"').format(node, type(node)))
        ...
        for node in nodes_list:
            instances = ('').join(('{0}({1}{2}{3})').format(str(x.id).ljust(ins_id_len), str(x.state + ',').ljust(ins_st_len), str(x.instance_type + ',').ljust(ins_type_len), str(x.root_device_type).ljust(ins_dev_len)).ljust(inst_hdr[1]) for x in node.instances)
            instances.strip()
