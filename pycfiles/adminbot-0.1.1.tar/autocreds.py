...
# Embedded file name: build/bdist.linux-x86_64/egg/cloud_admin/access/autocreds.py
...
import errno, os.path, re, zipfile
...
eucarc_to_service_map = {'euare_url': 'euare', 
   'ec2_url': 'compute', 
   'token_url': 'tokens', 
...
aws_to_euca_service_map = {'ec2': ServiceMapping('ec2', 'compute', 'ec2_url', 'ec2-url'), 
   'iam': ServiceMapping('iam', 'euare', 'euare_url', 'iam-url'), 
   'bootstrap': ServiceMapping('bootstrap', 'bootstrap', 'bootstrap_url', 'bootstrap-url'), 
   'elasticloadbalancing': ServiceMapping('elasticloadbalancing', 'loadbalancing', 'aws_elb_url', 'elasticloadbalancing-url'), 
   'cloudformation': ServiceMapping('cloudformation', 'cloudformation', 'aws_cloudformation_url', 'cloudformation-url'), 
   'autoscaling': ServiceMapping('autoscaling', 'autoscaling', 'aws_auto_scaling_url', 'autoscaling-url'), 
   's3': ServiceMapping('s3', 'objectstorage', 's3_url', 's3-url'), 
   'monitoring': ServiceMapping('monitoring', 'cloudwatch', 'aws_cloudwatch_url', 'monitoring-url'), 
...
class AutoCreds(Eucarc):
    ...
    def __init__(self, auto_create=True, aws_access_key=None, aws_secret_key=None, aws_account_name=None, region=None, domain=None, service_port=8773, https=False, aws_user_name=None, machine=None, hostname=None, username='root', password=None, keypath=None, credpath=None, proxy_hostname=None, proxy_username='root', proxy_password=None, proxy_keypath=None, logger=None, log_level='INFO', eucarc_obj=None, existing_certs=None, service_connection=None, properties_connection=None, keysdir=None, string=None, clc_connect_kwargs=None):
        ...
        self._service_port = service_port
        self._has_updated_connect_args = False
        if username != 'root' or proxy_username != 'root' or password or keypath or proxy_hostname or proxy_keypath or proxy_password:
            self._has_updated_connect_args = True
        clc_connect_kwargs = clc_connect_kwargs or {}
        self._clc_connect_kwargs = {'hostname': hostname, 
           'username': username, 
           'password': password, 
           'keypath': keypath, 
    ...
    def is_https(self, value):
        assert isinstance(value, bool), ('is_https must be of type bool, got:{0}/{1}').format(value, type(value))
        ...
        if self._domain is None:
            try:
                if self.propertiesconnection:
                    ...
                    self.log.debug(('Attempting to fetch service domain from property: {0}').format(prop_name))
                ...
                self.log.warning(('{0}\nError fetching domain from cloud:{1}').format(get_traceback(), E))
    ...
    def service_port(self):
        if not self._service_port and self.propertiesconnection:
            try:
                ...
                self.log.warning(('{0}\nError fetching service port from properties connection:{1}, err:{2}').format(get_traceback(), self.propertiesconnection, E))
    ...
    def _connect_services(self):
        if self.aws_secret_key and self.aws_access_key and self._clc_ip:
            try:
                self._serviceconnection = ServiceConnection(hostname=self._clc_ip, aws_access_key=self.aws_access_key, aws_secret_key=self.aws_secret_key)
            except Exception as E:
                self.log.error(('{0}\nError creating Euca Service Connection:{1}').format(get_traceback(), E))
    ...
    def _connect_properties(self):
        if self._serviceconnection:
            try:
                self._propertiesconnection = PropertiesConnection(hostname=self._serviceconnection.host, aws_access_key=self._serviceconnection.aws_access_key_id, aws_secret_key=self._serviceconnection.aws_secret_access_key)
            except Exception as E:
                self.log.error(('{0}\nError creating Euca Properties Connection from serviceconnection:{1}').format(get_traceback(), E))

        elif self.aws_secret_key and self.aws_access_key and self._clc_ip:
            try:
                self._propertiesconnection = PropertiesConnection(hostname=self._clc_ip, aws_access_key=self.aws_access_key, aws_secret_key=self.aws_secret_key)
            except Exception as E:
                self.log.error(('{0}\nError creating Euca Properties Connection:{1}').format(get_traceback(), E))
    ...
    def update_attrs_from_cloud_services(self):
        ...
        if not self.serviceconnection and not self.region_domain:
            raise RuntimeError('Can not fetch service paths from cloud without a "region_domain"and/or a ServiceConnection\n A ServiceConnection requires: clc_ip, or aws_access_key + aws_secret_key and bootstrap endpoint')
        if self.region_domain:
            path_dict = self._get_service_paths_from_domain(domain=self.domain, region_domain=self.region_domain, port=self.service_port, secure=self.is_https)
        else:
            if self.region_domain is not None:
                path_dict = self._get_service_paths_from_service_host_urls(self.serviceconnection)
            else:
                path_dict = self._get_service_paths_from_serviceconnection(self.serviceconnection, self.propertiesconnection, logger=self.log)
    ...
    def _get_service_paths_from_domain(self, region_domain=None, domain=None, port=8773, secure=False, verbose=False):
        ...
        for service_prefix, mapping in aws_to_euca_service_map.iteritems():
            ...
            if verbose:
                self.log.debug(('Setting {0} to {1} for prefix:{2}').format(mapping.eucarc, url, service_prefix))
            ret_dict[mapping.eucarc] = url

        return ret_dict

    @classmethod
    def _get_service_paths_from_serviceconnection(cls, serviceconnection, propertiesconnection, try_domain=True, secure=False, logger=None):
        """
        ...
        if not isinstance(propertiesconnection, PropertiesConnection):
            raise ValueError(('Unknown type for properties connection, got: "{0}/{1}"').format(propertiesconnection, type(propertiesconnection)))
        if try_domain:
            try:
                ...
                if logger:
                    logger.warn(('Could not fetch domain and port info from service connection, err: "{0}"').format(DE))

        if not isinstance(serviceconnection, ServiceConnection):
            raise ValueError(('Unknown type for service connection, got: "{0}/{1}"').format(serviceconnection, type(serviceconnection)))
    ...
    def auto_find_credentials(self, keys_and_region=True, from_file=True, assume_admin=True, service_connection=True, from_remote=True, from_db=True):
        """
        ...
        def try_keys_and_region(self):
            ...
            if self.aws_secret_key and self.aws_access_key and self.region_domain:
                try:
                    ...
                    self.log.debug(('{0}\nFailed to update creds using serviceconnection, err:"{1}"').format(get_traceback(), str(RE)))
                    self._close_adminpi()
        ...
        def try_from_file(self):
            ...
            if self._credpath:
                try:
                    res = self.get_local_eucarc(credpath=self._credpath) and self.update_attrs_from_cloud_services()
                    if res:
        ...
        def try_serviceconnection(self):
            ...
            if self.aws_secret_key and self.aws_access_key and self._clc_ip:
                ...
                try:
                    ...
                    self.log.debug(('{0}\nFailed to update creds from keys + region domain, err:"{1}"').format(get_traceback(), str(RE)))
        ...
        def try_assume_admin_on_clc(self):
            ...
            if not self.aws_secret_key and not self.aws_access_key:
                try:
                    ...
                    self.log.debug(('{0}\nFailed to update creds using "clcadmin-assume-system-credentials", err:"{1}"').format(get_traceback(), str(AE)))
            ...
            if self.creds_machine and self._credpath:
                try:
                    ...
                    if machine:
                        ...
                        res = self.get_remote_eucarc(credpath=self._credpath, machine=machine) and self.update_attrs_from_cloud_services()
                        if res:
                            self.log.debug(('Found remote creds on:"{0}", at path:"{1}"').format(self.creds_machine.ssh.host, self._credpath))
                            return res
                except Exception as e:
                    self.log.debug(("{0}\nFailed to fetch creds remotely, err:'{1}'").format(get_traceback(), str(e)))
        ...
        def try_clc_db(self):
            ...
            if self.creds_machine and self.aws_account_name and self.aws_user_name:
                ...
                if machine:
                    try:
                        res = self.get_existing_keys_from_clc(account=self.aws_account_name, user=self.aws_user_name, machine=machine)
                        try:
                            res = self.update_attrs_from_cloud_services()
                        except:
                            pass
                        ...
                        self.log.debug(('{0}\nFailed to fetch creds from clc db, err:{1}').format(get_traceback(), str(RE)))
        ...
        if self._clc_ip and self._credpath and (self._has_updated_connect_args or self._sshconnection):
            if try_remote(self):
                return
    ...
    def get_existing_keys_from_clc(self, account, user, machine=None, eucahome=None, port=8777, dbuser='eucalyptus', p12file=None, pkfile=None, passphrase=None, db=None, pargs=None, do_cert=None, verbose=False):
        ret = {}
        ...
        cmd = ("echo -n '{0}' | openssl SHA256  -sign {1} | sha256sum").format(passphrase, CloudPKFile)
        out = machine.sys(cmd, code=0, verbose=verbose)
        if out:
            dbpass = str(out[0]).split()[0]
        if do_cert:
            dbsel = ('"select k.auth_access_key_query_id, k.auth_access_key_key, a.auth_account_number, a.auth_account_name, c.auth_certificate_pem from eucalyptus_auth.auth_access_key k join eucalyptus_auth.auth_user u on k.auth_access_key_owning_user=u.id join eucalyptus_auth.auth_cert c on c.auth_certificate_owning_user=u.id join eucalyptus_auth.auth_group_has_users gu on gu.auth_user_id = u.id join eucalyptus_auth.auth_group g on gu.auth_group_id=g.id join eucalyptus_auth.auth_account a on g.auth_group_owning_account=a.id where a.auth_account_name = \'{0}\' and g.auth_group_name = \'{1}\'";').format(account, '_' + user)
        else:
            dbsel = ('"select k.auth_access_key_query_id, k.auth_access_key_key, a.auth_account_number, a.auth_account_name from eucalyptus_auth.auth_access_key k join eucalyptus_auth.auth_user u on k.auth_access_key_owning_user=u.id join eucalyptus_auth.auth_group_has_users gu on gu.auth_user_id = u.id join eucalyptus_auth.auth_group g on gu.auth_group_id=g.id join eucalyptus_auth.auth_account a on g.auth_group_owning_account=a.id where a.auth_account_name = \'{0}\' and g.auth_group_name = \'{1}\'";').format(account, '_' + user)
        dbcmd = ('export PGPASSWORD={0}; psql {1} -A -F "," -h 127.0.0.1 -p {2} -U {3} -d {4} -c {5}').format(dbpass, pargs, port, dbuser, db, dbsel)
        ...
        if qout:
            try:
                ...
                self.log.error(('{0}\nOutput:\nFailed fetching existing creds for account:"{1}", user:"{2}".\nRemote Command Output:"{3}"').format(get_traceback(), account, user, ('\n').join(qout), str(PE)))
                raise PE

        return ret

    def create_ini_file(self, filepath, update_user=True, update_region=True, region=None, user_string=None, machine=None, overwrite=True):
        """
        ...
        if not update_region and not update_user:
            self.log.warning('update_user and update_region were both false, not creating ini file ')
        if update_region:
            region = region or self.region or self.region_domain
            if not region:
                raise ValueError('Region was not found or provided and update_region is set to True. Please provide the region')
        else:
            region = None
        if update_user:
            if not user_string:
                if not self.account_id or not self.user_name:
                    raise ValueError(('Missing values needed o update user ini file info: account_id:"{0}", user_name:"{1}". Please set these or provide the user_string with: "account_id:user_name_format').format(self.account_id, self.user_name))
    ...
    def create_local_creds(self, local_destdir, machine=None, keydir=None, overwrite=False, zipfilename=None, ziponly=False):
        """
        ...
        if zipfilename:
            ...
            if os.path.exists(zip_path):
                raise ValueError(('Will not overwrite existing credentials archive at:"{0}"').format(zip_path))
        ...
        if has_sftp_items:
            if not machine:
                if not self._has_updated_connect_args:
                    self.log.info('Remote machine info has not been provided, skipping remote creds download')
                else:
                    machine = self.connect_to_creds_machine()
            if zip_file and ziponly:
                filepaths = self._download_remote_artifacts(local_destdir=None, machine=machine, overwrite=overwrite, zip_file=zip_file)
            else:
                filepaths = self._download_remote_artifacts(local_destdir=local_destdir, machine=machine, overwrite=overwrite)
        if zip_file and ziponly:
    ...
    def _download_remote_artifacts(self, local_destdir, machine, zip_file=None, sftp_prefix='^sftp://', overwrite=False, maxlen=5000000):
        """
        ...
        if not isinstance(machine, Machine):
            raise ValueError(('_download_remote_artifacts requires Machine() type. Got:"{0}/{1}"').format(machine, type(machine)))
        if zip_file is not None:
            if not isinstance(zip_file, zipfile.ZipFile):
                raise TypeError(('zip_file must be of type ZipFile, got:"{0}/{1}"').format(zipfile, type(zipfile)))
            ...
            raise ValueError(('_download_remote_artifacts requires string for local_destdir(). Got:"{0}/{1}"').format(local_destdir, type(local_destdir)))
        if not os.path.exists(local_destdir):
            os.makedirs(local_destdir)
        else:
            if not os.path.isdir(local_destdir):
                raise ValueError(('Provided local_destdir exists and is not a directory:"{0}"').format(local_destdir))
            if not overwrite:
                raise ValueError(('local_destdir exists. set "overwrite=True" to write over existing contents: {0}').format(local_destdir))
            if local_destdir is not None:
            ...
            for key, path in self.get_eucarc_attrs().iteritems():
                if not key.startswith('_') and re.search(sftp_prefix, str(path)):
                    ...
                    if not self.creds_machine.hostname == urlp.hostname:
                        raise ValueError(('sftp uri hostname:{0} does not match current Machines:{1}').format(urlp.hostname, machine.hostname))
