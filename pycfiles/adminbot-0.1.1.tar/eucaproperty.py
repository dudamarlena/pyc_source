...
# Embedded file name: build/bdist.linux-x86_64/egg/cloud_admin/services/eucaproperty.py
...
def SHOW_PROPERTIES(connection, properties=None, description=True, defaults=True, readonly=True, grid=ALL, print_method=None, print_table=True, search=None, *nameprefix):
    """
    ...
    if not pt._rows:
        err_row = [
         markup('NO PROPERTIES RETURNED', [1, 91])]
...
def SHOW_PROPERTIES_NARROW(connection, properties=None, verbose=True, print_method=None, print_table=True, *prop_names):
    """
    ...
    """
    if not verbose:
        return connection.show_properties(properties=properties, description=False, print_method=print_method, print_table=print_table)
    print_method = print_method or connection._show_method
    info_len = 60
    desc_len = 40
    ...
    for prop in properties:
        ...
        for p in props:
            ...
            for i in xrange(0, len(p.name), line_len):
                if i:
                    prefix = '      '
                info_buf += str(prefix + markup(p.name[i:i + line_len], [1, 94])).ljust(info_len - 2) + '\n'
...
class EucaProperty(EucaBaseObj):
        ...
        if not new_prop:
            try:
                ...
                if silent:
                    ...
                    self.connection.err_method(('{0}Update failed for property:{1}').format(errmsg, self.name))
                    return
                raise

        if not isinstance(new_prop, EucaProperty):
            raise ValueError(('"{0}" update error. Non EucaProperty type for new_prop. Found: "{1}/{2}"').format(self.name, new_prop, type(new_prop)))
        if new_prop:
    ...
    def modify_value(self, value, regex=None):
        ...
        try:
            ...
            self.connection.err_method(('Trying string comparison due to regex error:"{0}"').format(constants_error))
            if str(value).strip() == self.value.strip():
                match = True

        if not match:
            raise ValueError(('Modified property value does not match expected regex:{0}, current:{1}').format(regex, self.value))
        return self