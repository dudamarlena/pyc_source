...
# Embedded file name: build/bdist.linux-x86_64/egg/cloud_utils/net_utils/sshconnection.py
...
import re, select, socket, time, types, sys
...
def get_ipv4_lookup(hostname, port=22, debug_method=None, verbose=False):
    ...
    if get_ipv4_ip:
        try:
            ...
            debug_method('Resolved hostname:' + str(hostname) + ' to IP(s):' + (',').join(iplist), verbose=verbose)
        except Exception as de:
            debug_method('Error looking up DNS ip for hostname:' + str(hostname) + ', err:' + str(de))
...
class SFTPifc(SFTPClient):
    ...
    def get(self, remotepath, localpath, callback=None):
        try:
            ...
            self.debug(('Error during sftp get. Remote:"{0}", Local:"{1}"').format(remotepath, localpath))
            raise type(ge)(('Error during sftp get. Remotepath:"{0}", Localpath:"{1}".\n Err:{2}').format(remotepath, localpath, str(ge)))

    def put(self, localpath, remotepath, callback=None, confirm=True):
        try:
            super(SFTPifc, self).put(localpath=localpath, remotepath=remotepath, callback=callback, confirm=confirm)
        except Exception as pe:
            raise type(pe)(('Error during sftp put. Remotepath:"{0}", Localpath:"{1}".\n Err:{2}').format(remotepath, localpath, str(pe)))
...
class SshConnection():
    ...
    def __init__(self, host, username='root', password=None, keypair=None, keypath=None, proxy=None, proxy_username='root', proxy_password=None, proxy_keyname=None, proxy_keypath=None, key_files=None, find_keys=True, enable_ipv6_dns=False, timeout=60, banner_timeout=None, retry=1, logger=None, verbose=False, debug_connect=False):
        """
    ...
    def get_proxy_transport(self, proxy_host=None, dest_host=None, port=22, proxy_username='root', proxy_password=None, proxy_keypath=None, key_files=None, verbose=True):
        """
        ...
        if proxy_username is not None and (key_files or self.find_keys or proxy_keypath is not None or proxy_password is not None):
            ...
            channel = p_transport.open_channel('direct-tcpip', dest_host, ('127.0.0.1',
                                                                           0))
    ...
    def ssh_sys_timeout(self, chan, start, cmd):
        ...
        raise CommandTimeoutException('SSH Command timer fired after ' + str(int(elapsed)) + " seconds. Cmd:'" + str(cmd) + "'", elapsed=elapsed)

    def sys(self, cmd, verbose=False, timeout=120, listformat=True, enable_debug=False, code=None, check_alive=True, invoke_shell=False, get_pty=True):
        """
        ...
        """
        out = self.cmd(cmd, verbose=verbose, timeout=timeout, listformat=listformat, enable_debug=enable_debug, get_pty=get_pty, invoke_shell=invoke_shell, check_alive=check_alive)
        output = out['output']
        if code is not None and out['status'] != code:
            if verbose:
                self.debug(output)
            raise CommandExitCodeException('Cmd:' + str(cmd) + ' failed with status code:' + str(out['status']) + ', output:' + str(output), status=out['status'])
        return output

    def cmd(self, cmd, verbose=None, timeout=120, listformat=False, enable_debug=False, cb=None, cbargs=[], invoke_shell=False, get_pty=True, shell_delay=2, check_alive=True, shell_return='\r'):
        """
        ...
        try:
            if check_alive and not self.is_alive():
                self.debug('SSH transport was not alive, attempting to restablish ssh to: ' + str(self.host))
            ...
            while not attempt:
                try:
                    ...
                    if not attempt:
                        self.log.warning(('CMD:"{0}", Error while opening channel:{0}').format(cmd, E))
    ...
    def get_ssh_connection(self, hostname, username='root', password=None, keypath=None, proxy=None, proxy_username=None, proxy_password=None, proxy_keypath=None, key_files=None, enable_ipv6_dns=None, port=22, timeout=60, banner_timeout=60, retry=1, verbose=False):
        """
        ...
        while attempt < retry and not connected:
            ...
            for ip in iplist:
                if self.proxy:
                    if not enable_ipv6_dns:
                        proxy_ip = get_ipv4_lookup(self.proxy, debug_method=self.debug, verbose=verbose)[0]
                ...
                ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
                try:
                    self.debug('SSH connection attempt(' + str(attempt) + ' of ' + str(retry) + "), host:'" + str(username) + '@' + str(hostname) + "', using ipv4:" + str(ip) + ", thru proxy:'" + str(proxy_ip) + "'")
                    if keypath is None and password:
                        ...
                        ssh.connect(ip, port=port, username=username, key_filename=keypath, timeout=timeout, banner_timeout=banner_timeout)
                        connected = True
    ...
    def sftp(self, sftp):
        if isinstance(sftp, SFTPifc) or sftp is None:
            ...
            raise ValueError(('sftp must be of types; None or {0}, got:"{1}/{2}"').format(SFTPifc.__class__.__name__, sftp, type(sftp)))
        return
    ...
    def _get_local_unused_port(self, start=8500, checklimit=100):
        ...
        raise ValueError(('Could not find an available local port in range: {0} - {1}').format(start, start + checklimit))
    ...
    def _get_dict_id(self, dict_to_hash):
        ...
        :returns int
        """
        return int(abs(reduce(lambda x, y: x ^ y, [ hash(item) for item in dict_to_hash.items() ])))

    def create_http_fwd_connection(self, destport, dest_addr='127.0.0.1', peer=None, localport=None, trans=None, httpaddr='127.0.0.1', do_cache=False, **connection_kwargs):
        """
        ...
        local_arg_dict = {'destport': destport, 'dest_addr': dest_addr, 
           'peer': peer, 
           'localport': localport, 
           'trans': trans, 
           'httpaddr': httpaddr}
        ...
        if connection_id in self._http_connections.keys():
            ...
            if http.sock is not None:
                return {'connection': self._http_connections[connection_id], 'id': connection_id}
            del self._http_connections[connection_id]
        if localport is None:
            localport = self._get_local_unused_port(start=9000)
        self.debug(('Making forwarded request from "localhost:{0}" to "{1}:{2}"').format(localport, peer, destport))
        self.debug(('open_channel(kind="{0}", dest_addr=("{1}", {2}), src_addr=("{3}", {4})').format('direct-tcpip', dest_addr, destport, peer, localport))
        chan = trans.open_channel(kind='direct-tcpip', dest_addr=(dest_addr, destport), src_addr=(
         peer, localport))
    ...
    def http_fwd_request(self, url, body=None, headers={}, method='GET', trans=None, localport=9797, destport=None, cache_connnection=False):
        """
        ...
        conn_dict = self.create_http_fwd_connection(destport=destport, dest_addr='127.0.0.1', httpaddr='127.0.0.1', localport=localport, do_cache=cache_connnection)
