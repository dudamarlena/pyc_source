...
# Embedded file name: build/bdist.linux-x86_64/egg/cloud_admin/services/services.py
...
import copy, inspect, sys, time

def SHOW_SERVICES(connection, services=None, service_type=None, show_part=False, grid=False, partition=None, print_table=True, print_method=None, do_html=False):
    """
    ...
    if not service_type and not partition:
        ...
        if service_type:
            ...
            if not service_types:
                raise ValueError(('show_services, unknown type provided for service_type: "{0}"({1})').format(service_type, type(service_type)))
        else:
            service_types = all_service_types
        if partition:
            try:
                if partition in connection.get_all_cluster_names():
                    ...
                    if not service_types:
                        raise ValueError(('No partitioned services found using filter service_type:"{0}"').format(service_type))
        ...
        for service in services:
            ...
            if service.partition == 'eucalyptus' and service.type != 'eucalyptus' or service.partition == 'bootstrap':
                markups = [
                 1, 2]
            uri = service.uri
            if state != 'ENABLED':
    ...
    if print_table:
        if do_html:
            html_string = pt.get_html_string(sortby=part_hdr, fields=fields, sort_key=itemgetter(3, 2), reversesort=True, format=True, hrules=1)
            html_string = html_string.replace(html_open, '<')
            html_string = html_string.replace(html_close, '>')
            print_method(html_string)
        else:
            print_method('\n' + pt.get_string(sortby=part_hdr, fields=fields, sort_key=itemgetter(3, 2), reversesort=True))
    else:
        return pt
    return


def SHOW_SERVICE_TYPES(connection, service_types=None, verbose=False, printmethod=None, do_html=False, print_table=True):
    """
    ...
    for service in service_types:
        if not isinstance(service, EucaServiceType):
            raise ValueError(('Service not of EucaServiceType: {0}:{1}').format(service, type(service)))

    if verbose:
        return connection.show_service_types_verbose(service_types=service_types, printmethod=printmethod, print_table=print_table)
    else:
...
def SHOW_COMPONENTS(connection, components=None, get_method=None, print_method=None, print_table=True):
    """
...
class EucaService(EucaBaseObj):
    ...
    def getAllServicePeers(self):
        ...
        if not self.type or self.partition:
            raise ValueError(('({0}). Can not request active active peers w/o both type:("{0}") and partition populated:("{1}")').format(self.name, self.type, self.partition))
    ...
    def update(self, new_service=None, silent=True):
        ...
        if not isinstance(new_service, EucaService):
            raise ValueError(('"{0}" update error. Non EucaService type for new_prop. Found: "{1}/{2}"').format(self.name, new_service, type(new_service)))
        if new_service:
    ...
    def _update_ip_addr(self):
        if self.hostname:
            ips = get_ipv4_lookup(hostname=self.hostname, port=22, debug_method=self.debug_method, verbose=False)
            if ips:
                self._ip_addr = ips[0]
        return self._ip_addr
    ...
    def _find_service_class(self, service_name):
        ...
        for name, service_class in inspect.getmembers(sys.modules[self.__module__], inspect.isclass):
            name = name.lower()
