...
# Embedded file name: build/bdist.linux-x86_64/egg/cloud_admin/access/accessconnection.py
...
class AccessConnection(IAMConnection):
    ...
    def __init__(self, creds=None, host=None, aws_access_key=None, aws_secret_key=None, is_secure=None, port=None, path=None, logger=None, boto_debug=0, **kwargs):
        ...
        if creds and not isinstance(creds, Eucarc):
            ...
            if not isinstance(credsattr, Eucarc):
                raise ValueError(('Unknown type passed for creds arg: "{0}/{1}"').format(creds, type(creds)))
            creds = credsattr
        self._eucarc = creds
        ...
        if creds:
            ...
            assert isinstance(creds, Eucarc), ('UserAdmin. eucarc not type Eucarc(), got:"{0}/{1}"').format(creds, type(creds))
        ...
        ac_kwargs = {'host': host, 'aws_access_key_id': aws_access_key, 'aws_secret_access_key': aws_secret_key, 
           'is_secure': is_secure, 
           'port': port, 
           'path': path}
    ...
    def get_all_accounts(self, account_id=None, account_name=None, search=False):
        ...
        if search:
            ...
            re_meth = re.match
        self.log.debug(('Attempting to fetch all accounts matching- account_id:{0} account_name:{1}').format(str(account_id), str(account_name)))
        params = {}
        accounts = self.get_list(action='ListAccounts', params=params, markers=[
         (
          'member', IamAccount)], verb='GET')
        retlist = []
    ...
    def get_account(self, account):
        if not account:
            raise ValueError(('get_account got unknown type for account:"{0}/{1}"').format(account, type(account)))
    ...
    def get_users_from_account(self, path=None, user_name=None, user_id=None, delegate_account=None, search=False):
        ...
        :return:
        """
        self.log.debug(('Attempting to fetch all access matching- user_id:{0} user_name:{1} acct_name:{2}').format(str(user_id), str(user_name), str(delegate_account)))
        retlist = []
        params = {}
        account = None
        if search:
    ...
    def get_all_users(self, account_name=None, account_id=None, path=None, user_name=None, user_id=None, search=False):
        ...
        for account in accounts:
            users = self.get_users_from_account(path=path, user_name=user_name, user_id=user_id, delegate_account=account.name, search=search)
            for user in users:
                user.account = account
    ...
    def show_all_accounts(self, account=None, account_name=None, account_id=None, search=False, print_method=None, print_table=True):
        """
        ...
        if account:
            ...
            alist = self.get_all_accounts(account_name=account_name, account_id=account_id, search=search)
    ...
    def show_all_groups(self, account_name=None, account_id=None, path=None, group_name=None, group_id=None, search=False, print_method=None, print_table=True):
        """
        ...
        pt.hrules = 1
        pt.align = 'l'
        glist = self.get_all_groups(account_name=account_name, account_id=account_id, path=path, group_name=group_name, group_id=group_id, search=search)
        for group in glist:
    ...
    def show_all_users(self, users=None, account_name=None, account_id=None, path=None, user_name=None, user_id=None, search=False, print_method=None, print_table=True):
        """
        ...
        if users:
            ...
            for user in ulist:
                if not isinstance(user, IamUser):
                    raise ValueError(('show_all_users got non IAMUser type: "{0}/{1}"').format(user, type(user)))

        else:
            ulist = self.get_all_users(account_name=account_name, account_id=account_id, path=path, user_name=user_name, user_id=user_id, search=search)
        for user in ulist:
