...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/listing/tests/test_api.py
...
class APITestCase(TestCase):
        ...
        data = {'title': 'title', 
           ...
           'content': [
                     {'modelbase_obj': 'http://testserver/api/v1/jmbo-modelbase/%s/' % self.model_a.pk, 
                        'position': 0}]}
        response = self.client.post('/api/v1/listing-listing/', json.dumps(data), content_type='application/json')
    ...
    def test_listing_patch(self):
        ...
        data = {'content': [
                     {'modelbase_obj': 'http://testserver/api/v1/jmbo-modelbase/%s/' % self.model_b.pk, 
                        'position': 0}]}
        response = self.client.patch('/api/v1/listing-listing/%s/' % self.listing.pk, json.dumps(data), content_type='application/json')
        as_json = json.loads(response.content)
    ...
    def test_listing_content(self):
        response = self.client.get('/api/v1/listing-listing/%s/' % self.listing.pk)
        as_json = json.loads(response.content)
        self.failUnless('http://testserver/api/v1/jmbo-modelbase/%s/' % self.model_a.pk in as_json['content'])
        self.failUnless('http://testserver/api/v1/jmbo-modelbase/%s/' % self.model_a_published.pk in as_json['content'])

    def test_listing_queryset_objects(self):
        response = self.client.get('/api/v1/listing-listing/%s/queryset_objects/' % self.listing.pk)
        as_json = json.loads(response.content)
        self.failUnless('http://testserver/api/v1/jmbo-modelbase/%s/' % self.model_a.pk in as_json)
        self.failUnless('http://testserver/api/v1/jmbo-modelbase/%s/' % self.model_a_published.pk in as_json)