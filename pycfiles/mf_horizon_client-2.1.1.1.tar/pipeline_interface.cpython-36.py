...
# [GCC 8.4.0]
# Embedded file name: /Users/stanley/PycharmProjects/horizon-python-client/src/mf_horizon_client/client/pipelines/pipeline_interface.py
# Compiled at: 2020-04-13 12:55:36
# Size of source mod 2**32: 33224 bytes
import json
...
class PipelineInterface:
    ...
    def run_multitarget_forecast(self, pipeline_template: Pipeline, *, column_names: List=-1, column_ids: List=-1, n_training_rows_for_one_point_backtest=None, one_point_backtests=False) -> Dict[(str, pd.DataFrame)]:
        ...
        if not column_names == -1:
            ...
            terminal_messages.print_expert_message('This feature is experimental and can be very computationally intensive.Please ensure that you have no queued or pending pipelines prior to running this method.')
            if column_ids == -1:
                column_ids = [column.id_ for column in pipeline_template.dataset.columns if column.name in column_names]
            if column_names == -1:
        ...
        for column_id, column_name in zip(column_ids, column_names):
            ...
            for stage, template_stage in zip(pipeline.stages, pipeline_template.stages):
                ...
                self.update_config(pipeline_id=(pipeline_template.summary.id_),
                  stage_id=(template_stage.id_),
                  config=(template_stage.config))
        ...
        return {'backtests':all_backtests, 
    ...
    def run_multitarget_forecast_with_target_specific_feature_set(self, pipeline_template: Pipeline, *, column_names: List=-1, column_ids: List=-1, n_training_rows_for_one_point_backtest=None, one_point_backtests=False) -> Dict[(str, pd.DataFrame)]:
        ...
        """
        if not column_names == -1:
            assert column_ids == -1, 'Please only specify one of column_ids or column_names'
            terminal_messages.print_expert_message('This feature is experimental and can be very computationally intensive. Please ensure that you have no queued or pending pipelines prior to running this method.')
            pipeline_columns = pipeline_template.dataset.columns
            if column_ids == -1:
                column_ids = [column.id_ for column in pipeline_columns if column.name in column_names]
        else:
            if column_names == -1:
        ...
        augmented_dataset = data_interface.upload_data(data=augmented_features_no_duplicates,
          name=f"OUTPUT_FEATURES::{pipeline.summary.name}")
        pipeline_ids = []
        pbar = tqdm(total=(len(column_ids)))
        pbar.set_description('\nRunning Multitarget Forecast')
        forecasts = []
        ...
        for column_name in column_names:
            pbar.update()
            pipeline = self.create_pipeline(dataset_id=(augmented_dataset.summary.id_),
            ...
            new_problem_spec_config.target_feature = [column.id_ for column in augmented_dataset.summary.columns if column.name == column_name][0]
            ...
            forecast['Series'] = column_name
            forecasts.append(forecast)
            backtest = self.run_backtesting_for_multitarget(n_training_rows_for_one_point_backtest or 30, one_point_backtests, pipeline, new_problem_spec_config)
            backtest.columns = backtest.columns + '::' + column_name
            backtests.append(backtest)
