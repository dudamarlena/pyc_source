...
# [GCC 8.4.0]
# Embedded file name: /Users/stanley/PycharmProjects/horizon-python-client/src/mf_horizon_client/data_structures/configs/stage_config.py
# Compiled at: 2020-04-13 11:00:26
# Size of source mod 2**32: 12736 bytes
import abc, json
from typing import List
from mf_horizon_client.data_structures.configs.stage_config_enums import CorrelationMethod, FeatureGeneratorType, RegressorType, StationarisationStrategy, TargetTransformType
...
class StageConfig(abc.ABC):
    __doc__ = ' Properties for a stage '
...
class RefinementStageConfig(StageConfig):
    ...
    def __init__(self, min_features: int, max_features: int, early_stopping_sensitivity: float, deep_search: bool, regressor: RegressorType):
        """
        ...
        if not self.max_features >= self.min_features:
            raise AssertionError('Max features must be < min in RFE')
        else:
    ...
    def as_json(self) -> str:
        return json.dumps({'min_features':self.min_features, 
    ...
    def from_json(cls, json_config: str) -> 'RefinementStageConfig':
        config = json.loads(json_config)
        return cls(min_features=(config['min_features']),
    ...
    def get_default(cls) -> 'RefinementStageConfig':
        return RefinementStageConfig(min_features=5,
          max_features=20,
...
class StationarisationStageConfig(StageConfig):
    ...
    def __init__(self, adf_threshold: float, strategy: StationarisationStrategy, target_transform: TargetTransformType):
        self.adf_threshold = adf_threshold
        self.strategy = strategy
        self.target_transform = target_transform

    def as_json(self) -> str:
        return json.dumps({'adf_threshold':self.adf_threshold, 
    ...
    def from_json(cls, json_config: str) -> 'StationarisationStageConfig':
        config = json.loads(json_config)
        return cls(adf_threshold=(config['adf_threshold']),
    ...
    def get_default(cls) -> 'StationarisationStageConfig':
        return StationarisationStageConfig(adf_threshold=0.03,
...
class ProblemSpecificationConfig(StageConfig):
    ...
    def __init__(self, target_feature: FeatureId, horizons: List[int], data_split: float, active_columns: List[int], scale_factor_multiplier=1):
    ...
    def as_json(self) -> str:
        return json.dumps({'target_feature':str(self.target_feature), 
    ...
    def get_default(cls) -> 'ProblemSpecificationConfig':
        return ProblemSpecificationConfig(target_feature=(FeatureId('unspecified')),
          horizons=[1, 2, 3, 4, 5],
          data_split=0.75,
          active_columns=[])
...
class FeatureGenerationStageConfig(StageConfig):
    ...
    def get_default(cls) -> 'FeatureGenerationStageConfig':
        return FeatureGenerationStageConfig(max_n_features=5000,
          feature_generators=[
    ...
    def as_json(self) -> str:
        return json.dumps({'max_n_features':self.max_n_features, 
...
class BacktestStageConfig(StageConfig):
    ...
    def __init__(self, n_backtests: int, fold_train_frac: float, gapping_factor: float, regressor: RegressorType):
    ...
    def as_json(self) -> str:
        return json.dumps({'n_backtests':self.n_backtests, 
