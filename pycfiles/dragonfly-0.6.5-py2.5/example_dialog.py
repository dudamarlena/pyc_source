...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\dragonfly\windows\example_dialog.py
# Compiled at: 2009-01-19 04:06:55
import sys, ctypes, struct, winxpgui as win32gui, win32api, win32con, threading, Queue, os
...
class DragonflyUpdaterWindow(BasicDialog):
    ...
    def __init__(self, modal=False):
        BasicDialog.__init__(self, 'Dragonfly updater', (300, 250), min_size=(500,
                                                                              250), modal=modal)

    def _dialog_build_controls(self):
        self.close_control = Button(self, 'Close', lambda w, h: (
         w - 405, h - 35, 125, 29), on_command=self.on_button_close)
        self.update_control = Control(self, 128, 'Update dragonfly', lambda w, h: (
         w - 270, h - 35, 125, 29), style=win32con.WS_CHILD | win32con.WS_VISIBLE | win32con.WS_TABSTOP | win32con.BS_PUSHBUTTON, on_command=self.on_button_update)
        self.copy_control = Control(self, 128, 'Copy to clipboard', lambda w, h: (
         w - 135, h - 35, 125, 29), style=win32con.WS_CHILD | win32con.WS_VISIBLE | win32con.WS_TABSTOP | win32con.BS_PUSHBUTTON, on_command=self.on_button_copy)
        self.label_control = Control(self, 130, 'Output:', lambda w, h: (
         9, 5, w - 10, 19), style=win32con.WS_CHILD | win32con.WS_VISIBLE | win32con.ES_LEFT)
        self.output_control = OutputText(self, lambda w, h: (
         5, 28, w - 10, h - 68))
...
class UpdateThread(threading.Thread):
    ...
    def run(self):
        ...
        print
        try:
            entry_point = pkg_resources.load_entry_point('setuptools', 'console_scripts', 'easy_install')
