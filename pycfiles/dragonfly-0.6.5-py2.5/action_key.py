...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\dragonfly\actions\action_key.py
# Compiled at: 2009-03-09 03:10:08
"""
.. _RefKey:
...
class Key(DynStrActionBase):
    ...
    _modifier_prefix_characters = {'a': typeables['alt'], 
       'c': typeables['control'], 
       's': typeables['shift'], 
       'w': typeables['win']}
    _pause_factor = 0.02
    ...
    def _parse_single(self, spec):
        ...
        if index != -1:
            ...
            for c in s:
                if c not in self._modifier_prefix_characters:
                    raise ActionError('Invalid modifier prefix character: %r' % c)
                m = self._modifier_prefix_characters[c]
                if m in modifiers:
                    raise ActionError('Double modifier prefix character: %r' % c)
                modifiers.append(m)
        ...
        special = None
        outer_pause = None
        delimiters = [ (c, i + index) for (i, c) in enumerate(spec[index:]) if c in self._delimiter_characters
                     ]
        ...
        if inner_pause is not None:
            ...
            try:
                ...
                raise ActionError('Invalid inner pause value: %r, should be a positive number.' % s)
        ...
        if outer_pause is not None:
            ...
            try:
                ...
                raise ActionError('Invalid outer pause value: %r, should be a positive number.' % s)
        ...
        if special is not None:
            if special == 'down':
                ...
                try:
                    ...
                    raise ActionError('Invalid repeat value: %r, should be a positive integer.' % s)
