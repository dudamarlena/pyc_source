...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\dragonfly\config.py
# Compiled at: 2009-04-01 06:10:23
"""
...
"""
import sys, os.path, inspect, textwrap, traceback, dragonfly.log as log_
...
class Config(object):
  ...
  """
    ...
    def __setattr__(self, name, value):
        if self._mode == _init:
            if isinstance(value, Section):
                ...
                raise TypeError('Invalid type %s, expecting Section.' % type(value))
        else:
            raise AttributeError(name)
    ...
    _comment_indent = '                    '
    _comment_wrapper = textwrap.TextWrapper(width=70, break_long_words=False, initial_indent=_comment_indent + '# ', subsequent_indent=_comment_indent + '#  ')
    ...
    def __setattr__(self, name, value):
        if self._mode == _init:
            if isinstance(value, Item):
                ...
                raise TypeError('Invalid type %s, expecting Item or Section.' % type(value))
