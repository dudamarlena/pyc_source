...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\dragonfly\grammar\state.py
...
class State(object):
    ...
    def build_parse_tree(self):
        ...
        for frame in self._stack:
            ...
            parent = node
            node = Node(parent, frame.actor, self._results, frame.begin, frame.end, frame.depth, self._engine)
            if parent:
...
class Node(object):
    __slots__ = ('parent', 'children', 'actor', 'results', 'begin', 'end', 'depth',
                 'engine')
