...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\dragonfly\actions\action_mouse.py
...
class _Move(_EventBase):
    ...
    def _move_relative(self, rectangle):
        if self.from_left:
            horizontal = rectangle.x1
        else:
            horizontal = rectangle.x2
        ...
        horizontal += distance
        if self.from_top:
            vertical = rectangle.y1
        else:
            vertical = rectangle.y2
...
class _Button(_EventBase):
    ...
    def execute(self, window):
        ...
        inputs = [ MouseInput(0, 0, 0, flag, 0, zero) for flag in self._flags
                 ]
...
class Mouse(DynStrActionBase):
    ...
    def _parse_position_pair(self, spec):
        ...
        return (
         h_origin, h_value, v_origin, v_value)
