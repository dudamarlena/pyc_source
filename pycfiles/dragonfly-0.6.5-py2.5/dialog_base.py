...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\dragonfly\windows\dialog_base.py
...
class DialogBase(object):
    ...
    def __init__(self, classname, title, initial_size, initial_position, min_size, max_size, style, font, modal=False):
    ...
    def _dialog_create(self, template, message_map):
        if self._dialog_modal:
            function = win32gui.DialogBoxIndirect
        else:
            function = win32gui.CreateDialogIndirect
        function(self._hinst, template, 0, message_map)

    def _dialog_build_template(self):
        dialog_template = [
         self._dialog_build_header()]
    ...
    def _dialog_build_header(self):
        ...
        return [
         self._dialog_title,
         tuple(self._dialog_initial_position) + tuple(self._dialog_initial_size),
         self._dialog_style,
         None,
         self._dialog_font,
         None,
    ...
    def _dialog_build_message_map(self):
        ...
        map.update({win32con.WM_SIZE: self.on_size, 
    ...
    def close(self):
        if self._dialog_modal:
            win32gui.EndDialog(self._hwnd, 0)
        else:
...
class BasicDialog(DialogBase):
    ...
    def __init__(self, title, size, min_size=None, max_size=None, modal=False):
        ...
        DialogBase.__init__(self, classname=classname, title=title, initial_size=size, initial_position=position, min_size=min_size, max_size=max_size, style=style, font=font, modal=modal)
