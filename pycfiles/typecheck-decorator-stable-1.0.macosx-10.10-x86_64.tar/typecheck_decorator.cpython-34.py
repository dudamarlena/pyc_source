...
# [GCC 8.4.0]
# Embedded file name: /Users/bkeroack/.virtualenvs/conductor/lib/python3.4/site-packages/typecheck/typecheck_decorator.py
# Compiled at: 2015-04-02 13:34:36
# Size of source mod 2**32: 12456 bytes
__all__ = [
 ...
 'disable']
import builtins, collections, inspect, functools, random, re as regex_module
_enabled = True
...
class TypeCheckSpecificationError(Exception):
    pass


class InputParameterError(TypeCheckError):
    pass
    ...
    def check(self, values):
        ...
        if not issequence(values):
            ...
            if self._cls == tuple or isinstance(values, self._cls):
                if len(values) != len(self._checks):
                    return False
                for thischeck, thisvalue in zip(self._checks, values):
                    if not thischeck(thisvalue):
                        return False
...
class FixedMappingChecker(Checker):
    ...
    def __init__(self, the_mapping):
        self._checks = {key:Checker.create(val) for key, val in the_mapping.items()}
    ...
    def check(self, themap):
        ...
        for key, value in themap.items():
            if key not in self._checks or not self._checks[key](value):
                return False
...
class re(Checker):
    ...
    def check(self, value):
        return type(value) is self._regex_t and (not self._regex_eol or not value.endswith(self._value_eol)) and self._regex.search(value) is not None
...
class sequence_of(Checker):
    ...
    def check(self, value):
        ...
        if len(value) <= self._checkonly:
            ...
            checkhere = random.sample(builtins.range(1, len(value) - 1), self._checkonly - 2)
            checkhere += [0, len(value) - 1]
...
class seq_of(sequence_of):
    ...
    def check(self, value):
        return isinstance(value, collections.Sequence) and not isinstance(value, str) and super().check(value)


class list_of(sequence_of):

    def check(self, value):
        return isinstance(value, collections.MutableSequence) and super().check(value)
...
class map_of(Checker):
    ...
    def check(self, value):
        ...
        for mykey, myvalue in value.items():
            if not self._key_check.check(mykey) or not self._value_check.check(myvalue):
                return False
            count += 1
...
class range(Checker):
    ...
    def check(self, value):
        return type(value) == self._rangetype and value >= self._low and value <= self._high
...
def typecheck(method, *, input_parameter_error=InputParameterError, return_value_error=ReturnValueError):
    ...
    for n, v in argspec.annotations.items():
        ...
        if n in argspec.kwonlyargs:
            if n in kwarg_defaults:
                if not checker.check(kwarg_defaults[n]):
                    raise TypeCheckSpecificationError('the default value for {0} is incompatible with its typecheck'.format(n))
            kwarg_checkers[n] = checker
        elif n == 'return':
            return_checker = checker
        else:
            i = argspec.args.index(n)
            if i >= non_default_arg_count:
                if not checker.check(argspec.defaults[(i - non_default_arg_count)]):
                    raise TypeCheckSpecificationError('the default value for {0} is incompatible with its typecheck'.format(n))
            arg_checkers[i] = (
             n, checker)
    ...
    def typecheck_invocation_proxy(*args, **kwargs):
        for check, arg in zip(arg_checkers, args):
            if check is not None:
                ...
                if not checker.check(arg):
                    raise input_parameter_error('{0}() has got an incompatible value for {1}: {2}'.format(method_name, arg_name, str(arg) == '' and "''" or arg))
                else:
                    continue
        ...
        for arg_name, checker in kwarg_checkers.items():
            ...
            if not checker.check(kwarg):
                raise input_parameter_error('{0}() has got an incompatible value for {1}: {2}'.format(method_name, arg_name, str(kwarg) == '' and "''" or kwarg))
                continue

        result = method(*args, **kwargs)
        if return_checker is not None:
            if not return_checker.check(result):
                raise return_value_error('{0}() has returned an incompatible value: {1}'.format(method_name, str(result) == '' and "''" or result))
        return result

    return functools.update_wrapper(typecheck_invocation_proxy, method, assigned=('__name__',
                                                                                  '__module__',
                                                                                  '__doc__'))
...
@typecheck
def typecheck_with_exceptions(*, input_parameter_error: optional(_exception_class)=InputParameterError, return_value_error: optional(_exception_class)=ReturnValueError):
    return lambda method: typecheck(method, input_parameter_error=input_parameter_error, return_value_error=return_value_error)