...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/multitude/graph.py
# Compiled at: 2016-07-24 01:39:33
"""
...
NO_SUCH_VERTEX_EXC_FMT = b'Graph contains no vertex with key "{}"'
NO_SUCH_EDGE_EXC_FMT = b'Graph contains no edge from key "{}" to "{}"'
DUPLICATE_VERTEX_EXC_FMT = b'Graph already contains vertex with key "{}"'
DUPLICATE_EDGE_EXC_FMT = b'Graph already contains edge from key "{}" to "{}"'
...
class DiGraph(object):
    ...
    def remove_vertex(self, key, remove_edges=False):
        ...
        if not remove_edges and self.vertex_has_edges(key):
            msg = (b'{} has edges and remove_edges is False').format(key)
