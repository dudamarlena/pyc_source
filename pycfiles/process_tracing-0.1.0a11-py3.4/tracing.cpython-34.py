...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/process_tracing/tracing.py
# Compiled at: 2017-01-23 10:17:04
...
class Tracing:
    ...
    def __init__(self, process, stop=False, recording_mode=TRACING_RECORD_MODE_MEMORY, log_filename=None, log_callback=None):
        ...
        self.file_access_filter = ['open', 'stat', 'lstat', 'access', 'connect', 'chdir', 'rename', 'mkdir', 'rmdir',
         'creat', 'link', 'unlink', 'symlink', 'readlink', 'chmod', 'chown', 'lchown', 'utimemknod',
         'statfs', 'chroot', 'pivot_root']
        ...
        if self._recording_mode & TRACING_RECORD_MODE_FILE:
            if not self._log_filename:
                raise AttributeError('File recording requested but no log file specified')
        if self._recording_mode & TRACING_RECORD_MODE_CALLBACK:
    ...
    def start(self):
        ...
        processes = [
         self.process] + self.process.children(recursive=True)
        for p in processes:
    ...
    def should_record_syscall(self, s):
        ...
        if self.syscall_filter and s.syscall not in self.syscall_filter and s.name not in self.syscall_filter:
            return False
        if self.syscall_filter_exclude and (s.syscall in self.syscall_filter_exclude or s.name in self.syscall_filter_exclude):
            return False
        return True
    ...
    def should_record_file_access(self, syscall):
        ...
        if not self.is_file_access_tracing_enabled():
            return False
        if self.file_access_filter and syscall.syscall not in self.file_access_filter and syscall.name not in self.file_access_filter:
            return False
        if self.file_access_filter_exclude and (syscall.syscall in self.file_access_filter_exclude or syscall.name in self.file_access_filter_exclude):
            return False
        return True
    ...
    file_access_detailed_tracing = property(is_file_access_detailed_tracing_enabled, set_file_access_detailed_tracing_enabled)
...
class TracingThread(Thread):
    ...
    def __init__(self, manager, pid, record, old_debugger=None, is_thread=False):
        ...
        self._debugger_options = FunctionCallOptions(replace_socketcall=True)
        self.is_thread = is_thread
        self.process = psutil.Process(pid)
        self.debugger = None
        self.traced_item = None
        self.stopped = False
        if old_debugger is not None:
            if old_debugger.dict.get(self.process.pid):
