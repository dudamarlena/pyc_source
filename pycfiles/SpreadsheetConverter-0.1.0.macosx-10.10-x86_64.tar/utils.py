...
# [GCC 8.4.0]
# Embedded file name: /Users/yugo/.virtualenvs/ssc/lib/python2.7/site-packages/spreadsheetconverter/utils.py
...
def search_path(filename, path_env=None, recursive_env=None):
    ...
    if path_env:
        ...
        if search_path_env:
            search_paths += search_path_env.split(b':')
    search_path_env = os.environ.get(b'SSC_SEARCH_PATH')
    if search_path_env:
        search_paths += search_path_env.split(b':')
    if not search_paths:
    ...
    raise IOError(b'File does not exist.', filename, search_paths, recursive)