...
# [GCC 8.4.0]
# Embedded file name: /Users/yugo/.virtualenvs/ssc/lib/python2.7/site-packages/spreadsheetconverter/loader/xls.py
...
class XlsDatetimeValueConverter(BaseDatetimeConverter):
    ...
    def _to_python(self, value):
        if isinstance(value, float):
            return self._localize(xlrd.xldate.xldate_as_datetime(value, self.date_mode))
...
class Loader(BaseLoader):
    ...
    def __init__(self, params):
        ...
        abs_path = search_path(self._params.path[1:], path_env=b'SSC_XLS_SEARCH_PATH', recursive_env=b'SSC_XLS_SEARCH_RECURSIVE')
...
class Book(BaseBook):
    ...
    def get_sheet(self, name):
        ...
        raise ValueError((b'sheet does not exist {} in {}').format(name, self._file_path))
...
class Sheet(BaseSheet):
    ...
    def rows(self):
        for row in range(self.sheet.nrows):
            yield [ self.sheet.cell(row, col).value for col in range(self.sheet.ncols) ]