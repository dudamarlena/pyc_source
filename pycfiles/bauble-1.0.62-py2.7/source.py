...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bauble/plugins/garden/source.py
...
import gtk, gobject
from sqlalchemy import Column, Unicode, Integer, ForeignKey, Float, UnicodeText, select
from sqlalchemy.exc import DBAPIError
from sqlalchemy.orm import relation, backref
from bauble.i18n import _
import bauble.db as db, bauble.editor as editor
from bauble.plugins.plants.geography import Geography, GeographyMenu
import bauble.utils as utils, bauble.btypes as types, bauble.view as view, bauble.paths as paths
...
collection_edit_action = view.Action('collection_edit', _('_Edit'), callback=collection_edit_callback, accelerator='<ctrl>e')
collection_add_plant_action = view.Action('collection_add', _('_Add plants'), callback=collection_add_plants_callback, accelerator='<ctrl>k')
collection_remove_action = view.Action('collection_remove', _('_Delete'), callback=collection_remove_callback, accelerator='<ctrl>Delete')
collection_context_menu = [
 collection_edit_action, collection_add_plant_action,
...
source_detail_edit_action = view.Action('source_detail_edit', _('_Edit'), callback=source_detail_edit_callback, accelerator='<ctrl>e')
source_detail_remove_action = view.Action('source_detail_remove', _('_Delete'), callback=source_detail_remove_callback, accelerator='<ctrl>Delete', multiselect=True)
...
class Source(db.Base):
    ...
    source_detail = relation('SourceDetail', uselist=False, backref=backref('sources', cascade='all, delete-orphan'))
    collection = relation('Collection', uselist=False, cascade='all, delete-orphan', backref=backref('source', uselist=False))
    propagation_id = Column(Integer, ForeignKey('propagation.id'))
    propagation = relation('Propagation', uselist=False, single_parent=True, primaryjoin='Source.propagation_id==Propagation.id', cascade='all, delete-orphan', backref=backref('source', uselist=False))
    plant_propagation_id = Column(Integer, ForeignKey('propagation.id'))
    plant_propagation = relation('Propagation', uselist=False, primaryjoin='Source.plant_propagation_id==Propagation.id')
...
class SourceDetail(db.Base):
    ...
    source_type = Column(types.Enum(values=[ i[0] for i in source_type_values ], translations=dict(source_type_values)), default=None)
...
class SourceDetailEditorView(editor.GenericEditorView):
    _tooltips = {'source_name_entry': 'ITF2 - E.6 - <b>Donor</b> - don', 
...
class SourceDetailEditorPresenter(editor.GenericEditorPresenter):
    widget_to_field_map = {'source_name_entry': 'name', 'source_type_combo': 'source_type', 
    ...
    def set_model_attr(self, field, value, validator=None):
        super(SourceDetailEditorPresenter, self).set_model_attr(field, value, validator)
        self._dirty = True
    ...
    def refresh_view(self):
        ...
        self.view.widget_set_value('source_type_combo', dict(source_type_values)[self.model.source_type], index=1)
...
class CollectionPresenter(editor.ChildPresenter):
    ...
    """
    widget_to_field_map = {'collector_entry': 'collector', 'coll_date_entry': 'date', 
...
class PropagationChooserPresenter(editor.ChildPresenter):
    ...
    def __init__(self, parent, model, view, session):
        ...
        self.view.widgets.prop_toggle_column.set_cell_data_func(cell, self.toggle_cell_data_func)
        ...
        self.view.widgets.prop_summary_column.set_cell_data_func(self.view.widgets.prop_summary_cell, self.summary_cell_data_func)
        ...
        self.view.attach_completion('source_prop_plant_entry', plant_cell_data_func, minimum_key_length=1)

        def plant_get_completions(text):
            from bauble.plugins.garden.accession import Accession
            from bauble.plugins.garden.plant import Plant
            query = self.session.query(Plant).join('accession').filter(utils.ilike(Accession.code, '%s%%' % text)).filter(Accession.id != self.model.accession.id).order_by(Accession.code, Plant.code)
            return query
        ...
        self.assign_completions_handler('source_prop_plant_entry', plant_get_completions, on_select=on_select)
        return
...
class GeneralSourceDetailExpander(InfoExpander):
    ...
    def update(self, row):
        self.widget_set_value('sd_name_data', '<big>%s</big>' % utils.xml_safe(row.name), markup=True)
        source_type = ''
        if row.source_type:
        ...
        nacc = select([source.c.id], source.c.source_detail_id == row.id).count().execute().fetchone()[0]
...
class SourceDetailInfoBox(InfoBox):
    ...
    def __init__(self):
        ...
        filename = os.path.join(paths.lib_dir(), 'plugins', 'garden', 'source_detail_infobox.glade')
