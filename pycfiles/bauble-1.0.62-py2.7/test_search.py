...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bauble/test/test_search.py
...
import bauble.db as db, bauble.search as search
...
class SearchTests(BaubleTestCase):
    ...
    def test_search_by_query12(self):
        ...
        self.assertEquals(sorted([ r.id for r in results ]), [ g.id for g in (self.genus, g2, g3) ])
    ...
    def test_search_by_query21(self):
        ...
        mapper_search = search.get_strategy('MapperSearch')
        self.assertTrue(isinstance(mapper_search, search.MapperSearch))
        results = mapper_search.search('genus where family.family=family1', self.session)
        self.assertEquals(len(results), 1)
        g0 = list(results)[0]
...
class BinomialSearchTests(BaubleTestCase):
    ...
    def test_cultivar_also_matched(self):
        ...
        sp5 = Species(sp='coccinea', genus=g3, infrasp1_rank='cv.', infrasp1='Nora Grant')
...
class BuildingSQLStatements(BaubleTestCase):
    ...
    def test_between_just_parse(self):
        ...
        self.assertEqual(str(results.statement), 'SELECT * FROM species WHERE (BETWEEN id 0.0 1.0)')
...
class AggregatingFunctions(BaubleTestCase):
    ...
    def test_count_just_parse(self):
        ...
        self.assertEqual(str(results.statement), 'SELECT * FROM genus WHERE ((count species.id) == 2.0)')