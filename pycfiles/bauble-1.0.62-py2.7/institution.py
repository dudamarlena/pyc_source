...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bauble/plugins/garden/institution.py
# Compiled at: 2016-10-03 09:39:22
import os, gtk, logging
logger = logging.getLogger(__name__)
import re, bauble.editor as editor, bauble.meta as meta, bauble.paths as paths, bauble.pluginmgr as pluginmgr, bauble.utils as utils
...
class Institution(object):
    ...
    its own table
    """
    __properties = ('name', 'abbreviation', 'code', 'contact', 'technical_contact',
...
class InstitutionPresenter(editor.GenericEditorPresenter):
    widget_to_field_map = {'inst_name': 'name', 'inst_abbr': 'abbreviation', 
       'inst_code': 'code', 
    ...
    def __init__(self, model, view):
        ...
        self.email_regexp = re.compile('.+@.+\\..+')
        super(InstitutionPresenter, self).__init__(model, view, refresh_view=True)
    ...
    def on_non_empty_text_entry_changed(self, widget, value=None):
        value = super(InstitutionPresenter, self).on_non_empty_text_entry_changed(widget, value)
    ...
    def on_email_text_entry_changed(self, widget, value=None):
        value = super(InstitutionPresenter, self).on_text_entry_changed(widget, value)
        self.view.widget_set_sensitive('inst_register', self.email_regexp.match(value or ''))
    ...
    def on_inst_register_clicked(self, *args, **kwargs):
        ...
        registrations.info([ (key, getattr(self.model, key)) for key in self.widget_to_field_map.values()
                           ])
...
def start_institution_editor():
    ...
    view._tooltips = {'inst_name': _('The full name of the institution.'), 'inst_abbr': _('The standard abbreviation of the institution.'), 
       ...
       'inst_tech': _('The email address or phone number of the person to contact for technical information related to the institution.'), 
