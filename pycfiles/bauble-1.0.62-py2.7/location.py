...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bauble/plugins/garden/location.py
# Compiled at: 2016-10-03 09:39:22
import os, traceback, gtk, logging
...
from bauble.editor import GenericModelViewPresenterEditor, GenericEditorView, GenericEditorPresenter, UnicodeOrNoneValidator
import bauble.utils as utils, bauble.paths as paths
...
edit_action = Action('loc_edit', _('_Edit'), callback=edit_callback, accelerator='<ctrl>e')
add_plant_action = Action('loc_add_plant', _('_Add plants'), callback=add_plants_callback, accelerator='<ctrl>k')
remove_action = Action('loc_remove', _('_Delete'), callback=remove_callback, accelerator='<ctrl>Delete', multiselect=True)
loc_context_menu = [
...
class Location(db.Base, db.Serializable):
    ...
    def top_level_count(self):
        ...
        return {(1, 'Locations'): 1, (2, 'Plantings'): len(self.plants), 
...
class LocationEditorView(GenericEditorView):
    _tooltips = {'loc_name_entry': _('The name that you will use later to refer to this location.'), 
       'loc_desc_textview': _("Any information that might be relevant to the location such as where it is or what's its purpose")}

    def __init__(self, parent=None):
        GenericEditorView.__init__(self, os.path.join(paths.lib_dir(), 'plugins', 'garden', 'loc_editor.glade'), parent=parent)
    ...
    def set_accept_buttons_sensitive(self, sensitive):
        ...
        self.widgets.loc_ok_and_add_button.set_sensitive(self.use_ok_and_add and sensitive)
...
class LocationEditorPresenter(GenericEditorPresenter):
    widget_to_field_map = {'loc_name_entry': 'name', 'loc_code_entry': 'code', 
    ...
    def __init__(self, model, view):
        ...
        self.assign_simple_handler('loc_desc_textview', 'description', UnicodeOrNoneValidator())
        ...
        init_location_comboentry(self, self.view.widgets.loc_merge_comboentry, on_location_select)
        self.view.connect('loc_merge_button', 'clicked', self.on_loc_merge_button_clicked)
        return
    ...
    def on_loc_merge_button_clicked(self, entry, *args):
        ...
        if self.has_problems(entry_widget):
            logger.warning("'%s' does not identify a valid location" % entry_widget.get_text())
            return
        logger.debug('request to merge %s into %s' % (
         self.model, self.merger_candidate))
        md = gtk.MessageDialog(self.view.get_window(), gtk.DIALOG_DESTROY_WITH_PARENT, gtk.MESSAGE_QUESTION, gtk.BUTTONS_YES_NO, _('please confirm merging %(1)s into %(2)s') % {'1': self.model, '2': self.merger_candidate})
        confirm = md.run()
        md.destroy()
        if not confirm:
            return
        ...
        for p in self.session.query(Plant).filter(Plant.location == self.merger_candidate).all():
            p.location = self.model

        for p in self.session.query(PlantChange).filter(PlantChange.from_location == self.merger_candidate).all():
            p.from_location = self.model

        for p in self.session.query(PlantChange).filter(PlantChange.to_location == self.merger_candidate).all():
        ...
        self.view.widget_set_value('loc_desc_textview', mergevalues(buf.get_text(*buf.get_bounds()), getattr(self.merger_candidate, 'description'), '%s\n---------\n%s'))
        self.view.widget_set_value('loc_name_entry', mergevalues(self.view.widgets.loc_name_entry.get_text(), getattr(self.merger_candidate, 'name'), '%s\n---------\n%s'))
    ...
    def set_model_attr(self, attr, value, validator=None):
        super(LocationEditorPresenter, self).set_model_attr(attr, value, validator)
        self._dirty = True
        self.refresh_sensitivity()
...
class GeneralLocationExpander(InfoExpander):
    ...
    def update(self, row):
        ...
        self.widget_set_value('loc_name_data', '<big>%s</big>' % utils.xml_safe(str(row)), markup=True)
