...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bauble/plugins/plants/genus.py
# Compiled at: 2016-10-03 09:39:22
import os, traceback, weakref, xml, gtk, logging
logger = logging.getLogger(__name__)
from sqlalchemy import Column, Unicode, Integer, ForeignKey, UnicodeText, String, UniqueConstraint, func, and_
from sqlalchemy.orm import relation, backref, validates
from sqlalchemy.orm.session import object_session
from sqlalchemy.exc import DBAPIError
from sqlalchemy.ext.associationproxy import association_proxy
from bauble.i18n import _
import bauble, bauble.db as db, bauble.error as error, bauble.pluginmgr as pluginmgr, bauble.editor as editor, bauble.utils as utils, bauble.btypes as types, bauble.paths as paths
...
def remove_callback(genera):
    ...
    if nsp > 0:
        msg = _('The genus <i>%(genus)s</i> has %(num_species)s species.  Are you sure you want to remove it?') % dict(genus=safe_str, num_species=nsp)
    else:
...
edit_action = Action('genus_edit', _('_Edit'), callback=edit_callback, accelerator='<ctrl>e')
add_species_action = Action('genus_sp_add', _('_Add species'), callback=add_species_callback, accelerator='<ctrl>k')
remove_action = Action('genus_remove', _('_Delete'), callback=remove_callback, accelerator='<ctrl>Delete', multiselect=True)
genus_context_menu = [
...
class Genus(db.Base, db.Serializable, db.WithNotes):
    ...
    def search_view_markup_pair(self):
        ...
        """
        return (
         utils.xml_safe(self), utils.xml_safe(self.family))

    @property
    def cites(self):
        """the cites status of this taxon, or None
        """
        cites_notes = [ i.note for i in self.notes if i.category and i.category.upper() == 'CITES'
                      ]
        if not cites_notes:
    ...
    _synonyms = relation('GenusSynonym', primaryjoin='Genus.id==GenusSynonym.genus_id', cascade='all, delete-orphan', uselist=True, backref='genus')
    __syn = relation('GenusSynonym', primaryjoin='Genus.id==GenusSynonym.synonym_id', cascade='all, delete-orphan', uselist=True)
    ...
    def accepted(self):
        ...
        if not session:
            ...
            syn = session.query(GenusSynonym).filter(GenusSynonym.synonym_id == self.id).first()
    ...
    def accepted(self, value):
        ...
        if not isinstance(value, self.__class__):
            ...
            if self in value.synonyms:
                return
            session = object_session(self)
            session or logger.warn('genus:accepted.setter - object not in session')
            return
        session.query(GenusSynonym).filter(GenusSynonym.synonym_id == self.id).delete()
        session.commit()
    ...
    def str(genus, author=False):
        if genus.genus is None:
            ...
            if not author or genus.author is None:
                return (' ').join([ s for s in [genus.genus, genus.qualifier] if s not in ('',
                                                                                           None)
                                  ])
    ...
    def retrieve(cls, session, keys):
        try:
            return session.query(cls).filter(cls.genus == keys['epithet']).one()
        except:
            if 'author' not in keys:
                return

        try:
            return session.query(cls).filter(cls.genus == keys['epithet'], cls.author == keys['author']).one()
        except:
            return
    ...
    def compute_serializable_fields(cls, session, keys):
        ...
        if keys.get('ht-epithet'):
            result['family'] = Family.retrieve_or_create(session, {'epithet': keys['ht-epithet']}, create=True)
    ...
    def top_level_count(self):
        ...
        return {(1, 'Genera'): set([self.id]), (2, 'Families'): set([self.family.id]), 
           ...
           (8, 'Sources'): set([ a.source.source_detail.id for a in accessions if a.source and a.source.source_detail
                         ])}
...
class GenusNote(db.Base):
    ...
    genus = relation('Genus', uselist=False, backref=backref('notes', cascade='all, delete-orphan'))
...
class GenusSynonym(db.Base):
    ...
    synonym = relation('Genus', uselist=False, primaryjoin='GenusSynonym.synonym_id==Genus.id')
...
from bauble.plugins.plants.species_model import Species
from bauble.plugins.plants.species_editor import edit_species
Genus.species = relation('Species', cascade='all, delete-orphan', order_by=[
 Species.sp], backref=backref('genus', uselist=False))

class GenusEditorView(editor.GenericEditorView):
    syn_expanded_pref = 'editor.genus.synonyms.expanded'
    _tooltips = {'gen_family_entry': _('The family name'), 
       'gen_genus_entry': _('The genus name'), 
       'gen_author_entry': _('The name or abbreviation of the author that published this genus'), 
       'gen_syn_frame': _('A list of synonyms for this genus.\n\nTo add a synonym enter a genus name and select one from the list of completions.  Then click Add to add it to the list of synonyms.'), 
       'gen_cancel_button': _('Cancel your changes.'), 
       'gen_ok_button': _('Save your changes.'), 
       'gen_ok_and_add_button': _('Save your changes changes and add a species to this genus.'), 
       'gen_next_button': _('Save your changes changes and add another genus.')}

    def __init__(self, parent=None):
        filename = os.path.join(paths.lib_dir(), 'plugins', 'plants', 'genus_editor.glade')
...
class GenusEditorPresenter(editor.GenericEditorPresenter):
    widget_to_field_map = {'gen_family_entry': 'family', 'gen_genus_entry': 'genus', 
    ...
    def __init__(self, model, view):
        ...
        def fam_get_completions(text):
            ...
            return query.filter(Family.family.like('%s%%' % text)).order_by(Family.family)
        ...
        def on_select(value):
            ...
            if not value:
                ...
                syn = self.session.query(FamilySynonym).filter(FamilySynonym.synonym_id == value.id).first()
                if not syn:
                    self.set_model_attr('family', value)
                    return
                msg = _('The family <b>%(synonym)s</b> is a synonym of <b>%(family)s</b>.\n\nWould you like to choose <b>%(family)s</b> instead?') % {'synonym': syn.synonym, 'family': syn.family}
                box = None
                ...
                def on_response(button, response):
                    ...
                    if response:
                        ...
                        self.view.widgets.gen_family_entry.set_text(utils.utf8(syn.family))
        ...
        self.assign_completions_handler('gen_family_entry', fam_get_completions, on_select=on_select)
        self.assign_simple_handler('gen_genus_entry', 'genus', editor.UnicodeOrNoneValidator())
        self.assign_simple_handler('gen_author_entry', 'author', editor.UnicodeOrNoneValidator())
    ...
    def set_model_attr(self, field, value, validator=None):
        super(GenusEditorPresenter, self).set_model_attr(field, value, validator)
        self._dirty = True
        self.refresh_sensitivity()

    def dirty(self):
        return self._dirty or self.synonyms_presenter.dirty() or self.notes_presenter.dirty()
...
class SynonymsPresenter(editor.GenericEditorPresenter):
    ...
    def __init__(self, parent):
        ...
        super(SynonymsPresenter, self).__init__(self.parent_ref().model, self.parent_ref().view)
        ...
        def gen_get_completions(text):
            ...
            return query.filter(and_(Genus.genus.like('%s%%' % text), Genus.id != self.model.id)).order_by(Genus.genus)
        ...
        self.assign_completions_handler('gen_syn_entry', gen_get_completions, on_select=on_select)
        self.view.connect('gen_syn_add_button', 'clicked', self.on_add_button_clicked)
        self.view.connect('gen_syn_remove_button', 'clicked', self.on_remove_button_clicked)
        self._dirty = False
        return
    ...
    def on_remove_button_clicked(self, button, data=None):
        ...
        msg = _('Are you sure you want to remove %(genus)s as a synonym to the current genus?\n\n<i>Note: This will not remove the genus from the database.</i>') % {'genus': s}
...
class GenusEditor(editor.GenericModelViewPresenterEditor):
    ...
    def start(self):
        if self.session.query(Family).count() == 0:
            msg = _('You must first add or import at least one Family into the database before you can add plants.')
...
class GeneralGenusExpander(InfoExpander):
    ...
    def __init__(self, widgets):
        ...
        def on_nacc_clicked(*args):
            g = self.current_obj
            cmd = 'accession where species.genus.genus="%s" and species.genus.qualifier="%s"' % (
             g.genus, g.qualifier)
            bauble.gui.send_command(cmd)
        ...
        def on_nplants_clicked(*args):
            g = self.current_obj
            cmd = 'plant where accession.species.genus.genus="%s" and accession.species.genus.qualifier="%s"' % (
             g.genus, g.qualifier)
            bauble.gui.send_command(cmd)
    ...
    def update(self, row):
        ...
        self.widget_set_value('gen_name_data', '<big>%s</big> %s' % (
         row, utils.xml_safe(unicode(row.author))), markup=True)
        self.widget_set_value('gen_fam_data', utils.xml_safe(unicode(row.family)))
        ...
        nacc = session.query(Accession).join('species', 'genus').filter_by(id=row.id).count()
        if nacc == 0:
            self.widget_set_value('gen_nacc_data', nacc)
        else:
            nsp_in_acc = session.query(Accession.species_id).join('species', 'genus').filter_by(id=row.id).distinct().count()
            self.widget_set_value('gen_nacc_data', '%s in %s species' % (
             nacc, nsp_in_acc))
        nplants = session.query(Plant).join('accession', 'species', 'genus').filter_by(id=row.id).count()
        if nplants == 0:
            self.widget_set_value('gen_nplants_data', nplants)
        else:
            nacc_in_plants = session.query(Plant.accession_id).join('accession', 'species', 'genus').filter_by(id=row.id).distinct().count()
