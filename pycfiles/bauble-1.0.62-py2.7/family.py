...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bauble/plugins/plants/family.py
# Compiled at: 2016-10-03 09:39:22
import os, traceback, weakref, gtk, logging
logger = logging.getLogger(__name__)
from sqlalchemy import Column, Unicode, Integer, ForeignKey, UnicodeText, func, and_, UniqueConstraint, String
from sqlalchemy.orm import relation, backref, validates
from sqlalchemy.orm.session import object_session
from sqlalchemy.exc import DBAPIError
from sqlalchemy.ext.associationproxy import association_proxy
import bauble, bauble.db as db, bauble.pluginmgr as pluginmgr, bauble.editor as editor, bauble.utils as utils, bauble.utils.web as web, bauble.btypes as types
...
def remove_callback(families):
    ...
    if ngen > 0:
        msg = _('The family <i>%(family)s</i> has %(num_genera)s genera.  Are you sure you want to remove it?') % dict(family=safe_str, num_genera=ngen)
    else:
...
edit_action = view.Action('family_edit', _('_Edit'), callback=edit_callback, accelerator='<ctrl>e')
add_species_action = view.Action('family_genus_add', _('_Add genus'), callback=add_genera_callback, accelerator='<ctrl>k')
remove_action = view.Action('family_remove', _('_Delete'), callback=remove_callback, accelerator='<ctrl>Delete', multiselect=True)
family_context_menu = [
...
class Family(db.Base, db.Serializable, db.WithNotes):
    ...
    _synonyms = relation('FamilySynonym', primaryjoin='Family.id==FamilySynonym.family_id', cascade='all, delete-orphan', uselist=True, backref='family')
    __syn = relation('FamilySynonym', primaryjoin='Family.id==FamilySynonym.synonym_id', cascade='all, delete-orphan', uselist=True)
    ...
    def accepted(self):
        ...
        if not session:
            ...
            syn = session.query(FamilySynonym).filter(FamilySynonym.synonym_id == self.id).first()
    ...
    def accepted(self, value):
        ...
        if not isinstance(value, self.__class__):
            ...
            if self in value.synonyms:
                return
            session = object_session(self)
            session or logger.warn('family:accepted.setter - object not in session')
            return
        session.query(FamilySynonym).filter(FamilySynonym.synonym_id == self.id).delete()
        session.commit()
    ...
    def retrieve(cls, session, keys):
        try:
            return session.query(cls).filter(cls.family == keys['epithet']).one()
        except:
            return
    ...
    def top_level_count(self):
        ...
        return {(1, 'Families'): set([self.id]), (2, 'Genera'): genera, 
...
class FamilyNote(db.Base):
    ...
    family = relation('Family', uselist=False, backref=backref('notes', cascade='all, delete-orphan'))
...
class FamilySynonym(db.Base):
    ...
    synonym = relation('Family', uselist=False, primaryjoin='FamilySynonym.synonym_id==Family.id')
...
Family.genera = relation('Genus', order_by=[
 Genus.genus], backref='family', cascade='all, delete-orphan')

class FamilyEditorView(editor.GenericEditorView):
    syn_expanded_pref = 'editor.family.synonyms.expanded'
    _tooltips = {'fam_family_entry': _('The family name.'), 
       'fam_qualifier_combo': _('The family qualifier helps to remove ambiguities that might be associated with this family name.'), 
       ...
       'fam_ok_and_add_button': _('Save your changes changes and add a genus to this family.'), 
       'fam_next_button': _('Save your changes changes and add another family.')}

    def __init__(self, parent=None):
        filename = os.path.join(paths.lib_dir(), 'plugins', 'plants', 'family_editor.glade')
...
class FamilyEditorPresenter(editor.GenericEditorPresenter):
    widget_to_field_map = {'fam_family_entry': 'family', 'fam_qualifier_combo': 'qualifier'}
    ...
    def __init__(self, model, view):
        ...
        self.refresh_view()
        self.assign_simple_handler('fam_family_entry', 'family', editor.UnicodeOrNoneValidator())
        self.assign_simple_handler('fam_qualifier_combo', 'qualifier', editor.UnicodeOrEmptyValidator())
    ...
    def set_model_attr(self, field, value, validator=None):
        super(FamilyEditorPresenter, self).set_model_attr(field, value, validator)
        self._dirty = True
...
class SynonymsPresenter(editor.GenericEditorPresenter):
    ...
    def __init__(self, parent):
        ...
        def fam_get_completions(text):
            ...
            return query.filter(and_(Family.family.like('%s%%' % text), Family.id != self.model.id)).order_by(Family.family)
        ...
        self.assign_completions_handler('fam_syn_entry', fam_get_completions, on_select=on_select)
        self.view.connect('fam_syn_add_button', 'clicked', self.on_add_button_clicked)
        self.view.connect('fam_syn_remove_button', 'clicked', self.on_remove_button_clicked)
        self._dirty = False
        return
    ...
    def on_remove_button_clicked(self, button, data=None):
        ...
        msg = 'Are you sure you want to remove %s as a synonym to the current family?\n\n<i>Note: This will not remove the family %s from the database.</i>' % (
         s, s)
...
from bauble.view import InfoBox, InfoExpander, PropertiesExpander, select_in_search_results
...
class GeneralFamilyExpander(InfoExpander):
    ...
    def __init__(self, widgets):
        ...
        def on_nsp_clicked(*args):
            f = self.current_obj
            cmd = 'species where genus.family.family="%s" and genus.family.qualifier="%s"' % (
             f.family, f.qualifier)
            bauble.gui.send_command(cmd)
        ...
        def on_nacc_clicked(*args):
            f = self.current_obj
            cmd = 'accession where species.genus.family.family="%s" and species.genus.family.qualifier="%s"' % (
             f.family, f.qualifier)
            bauble.gui.send_command(cmd)
        ...
        def on_nplants_clicked(*args):
            f = self.current_obj
            cmd = 'plant where accession.species.genus.family.family="%s" and accession.species.genus.family.qualifier="%s"' % (
             f.family, f.qualifier)
            bauble.gui.send_command(cmd)
    ...
    def update(self, row):
        ...
        nsp = session.query(Species).join('genus').filter_by(family_id=row.id).count()
        if nsp == 0:
            self.widget_set_value('fam_nsp_data', 0)
        else:
            ngen_in_sp = session.query(Species.genus_id).join('genus', 'family').filter_by(id=row.id).distinct().count()
        ...
        nacc = session.query(Accession).join('species', 'genus', 'family').filter_by(id=row.id).count()
        if nacc == 0:
            self.widget_set_value('fam_nacc_data', nacc)
        else:
            nsp_in_acc = session.query(Accession.species_id).join('species', 'genus', 'family').filter_by(id=row.id).distinct().count()
            self.widget_set_value('fam_nacc_data', '%s in %s species' % (
             nacc, nsp_in_acc))
        nplants = session.query(Plant).join('accession', 'species', 'genus', 'family').filter_by(id=row.id).count()
        if nplants == 0:
            self.widget_set_value('fam_nplants_data', nplants)
        else:
            nacc_in_plants = session.query(Plant.accession_id).join('accession', 'species', 'genus', 'family').filter_by(id=row.id).distinct().count()
