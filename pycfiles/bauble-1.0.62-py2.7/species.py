...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bauble/plugins/plants/species.py
# Compiled at: 2016-10-03 09:39:22
import gtk, logging
logger = logging.getLogger(__name__)
import os, traceback
from sqlalchemy.orm.session import object_session
import bauble, bauble.paths as paths, bauble.db as db
from bauble.i18n import _
import bauble.pluginmgr as pluginmgr
from bauble.prefs import prefs
import bauble.utils as utils
from bauble.plugins.plants.species_editor import SpeciesDistribution, SpeciesEditorPresenter, SpeciesEditorView, SpeciesEditorMenuItem, edit_species
from bauble.plugins.plants.species_model import Species, SpeciesNote, VernacularName, SpeciesSynonym, DefaultVernacularName
...
def remove_callback(values):
    ...
    if nacc > 0:
        msg = _('The species <i>%(species)s</i> has %(num_accessions)s accessions.  Are you sure you want remove it?') % dict(species=safe_str, num_accessions=nacc)
    else:
...
edit_action = Action('species_edit', _('_Edit'), callback=edit_callback, accelerator='<ctrl>e')
add_accession_action = Action('species_acc_add', _('_Add accession'), callback=add_accession_callback, accelerator='<ctrl>k')
remove_action = Action('species_remove', _('_Delete'), callback=remove_callback, accelerator='<ctrl>Delete', multiselect=True)
    ...
    def search(self, text, session):
        ...
        for result in r1:
            if isinstance(result, Species):
                ...
                q = session.query(SpeciesSynonym).filter_by(synonym_id=result.species.id)
                results.extend([ syn.species for syn in q ])
    ...
    def update(self, row):
        ...
        if len(row.vernacular_names) == 0:
            ...
            for vn in row.vernacular_names:
                if row.default_vernacular_name is not None and vn == row.default_vernacular_name:
...
class SynonymsExpander(InfoExpander):
    ...
    def update(self, row):
        ...
        syn = session.query(SpeciesSynonym).filter(SpeciesSynonym.synonym_id == row.id).first()
...
class GeneralSpeciesExpander(InfoExpander):
    ...
    def update(self, row):
        ...
        self.widget_set_value('sp_fam_data', '<small>(%s)</small>' % row.genus.family.family, markup=True)
        utils.make_label_clickable(self.widgets.sp_fam_data, on_label_clicked, row.genus.family)
        self.widget_set_value('sp_gen_data', '<big><i>%s</i></big>' % row.genus.genus, markup=True)
        utils.make_label_clickable(self.widgets.sp_gen_data, on_label_clicked, row.genus)
        self.widget_set_value('sp_epithet_data', '<big>%s</big>' % row.markup(authors=True, genus=False), markup=True)
        awards = ''
        if row.awards:
        ...
        nacc = session.query(Accession).join('species').filter_by(id=row.id).count()
        self.widget_set_value('sp_nacc_data', nacc)
        nplants = session.query(Plant).join('accession', 'species').filter_by(id=row.id).count()
        if nplants == 0:
            self.widget_set_value('sp_nplants_data', nplants)
        else:
            nacc_in_plants = session.query(Plant.accession_id).join('accession', 'species').filter_by(id=row.id).distinct().count()
