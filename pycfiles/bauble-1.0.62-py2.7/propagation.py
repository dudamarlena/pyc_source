...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bauble/plugins/garden/propagation.py
# Compiled at: 2016-10-03 09:39:22
import datetime, os, weakref, traceback, gtk, logging
logger = logging.getLogger(__name__)
from sqlalchemy import Column, Integer, ForeignKey, UnicodeText, Unicode
from sqlalchemy.orm import backref, relation
from sqlalchemy.orm.session import object_session
from sqlalchemy.exc import DBAPIError
import bauble, bauble.db as db, bauble.utils as utils, bauble.paths as paths, bauble.editor as editor, bauble.prefs as prefs, bauble.btypes as types
from bauble.i18n import _
prop_type_values = {'Seed': _('Seed'), 'UnrootedCutting': _('Unrooted cutting'), 
   'Other': _('Other')}
...
class Propagation(db.Base):
    ...
    prop_type = Column(types.Enum(values=prop_type_values.keys(), translations=prop_type_values), nullable=False)
    notes = Column(UnicodeText)
    date = Column(types.Date)
    _cutting = relation('PropCutting', primaryjoin='Propagation.id==PropCutting.propagation_id', cascade='all,delete-orphan', uselist=False, backref=backref('propagation', uselist=False))
    _seed = relation('PropSeed', primaryjoin='Propagation.id==PropSeed.propagation_id', cascade='all,delete-orphan', uselist=False, backref=backref('propagation', uselist=False))
    ...
    def get_summary(self):
        ...
        if self.prop_type == 'UnrootedCutting':
            ...
            if c.cutting_type is not None:
                values.append(_('Cutting type: %s') % cutting_type_values[c.cutting_type])
            if c.length:
                values.append(_('Length: %(length)s%(unit)s') % dict(length=c.length, unit=length_unit_values[c.length_unit]))
            if c.tip:
            ...
            if c.leaves:
                ...
                values.append(s)
            if c.flower_buds:
                values.append(_('Flower buds: %s') % flower_buds_values[c.flower_buds])
            ...
            if c.bottom_heat_temp:
                values.append(_('Bottom heat: %(temp)s%(unit)s') % dict(temp=c.bottom_heat_temp, unit=bottom_heat_unit_values[c.bottom_heat_unit]))
            if c.container:
...
cutting_type_values = {'Nodal': _('Nodal'), 'InterNodal': _('Internodal'), 
   'Other': _('Other')}
tip_values = {'Intact': _('Intact'), 'Removed': _('Removed'), 
   'None': _('None'), 
   None: ''}
leaves_values = {'Intact': _('Intact'), 'Removed': _('Removed'), 
   'None': _('None'), 
   None: ''}
flower_buds_values = {'Removed': _('Removed'), 'None': _('None'), 
   None: ''}
wound_values = {'No': _('No'), 'Single': _('Singled'), 
   'Double': _('Double'), 
   'Slice': _('Slice'), 
   None: ''}
hormone_values = {'Liquid': _('Liquid'), 'Powder': _('Powder'), 
   'No': _('No')}
bottom_heat_unit_values = {'F': _('°F'), 'C': _('°C'), 
   None: ''}
length_unit_values = {'mm': _('mm'), 'cm': _('cm'), 
   'in': _('in'), 
   None: ''}
...
class PropCutting(db.Base):
    ...
    cutting_type = Column(types.Enum(values=cutting_type_values.keys(), translations=cutting_type_values), default='Other')
    tip = Column(types.Enum(values=tip_values.keys(), translations=tip_values))
    leaves = Column(types.Enum(values=leaves_values.keys(), translations=leaves_values))
    leaves_reduced_pct = Column(Integer, autoincrement=False)
    length = Column(Integer, autoincrement=False)
    length_unit = Column(types.Enum(values=length_unit_values.keys(), translations=length_unit_values))
    wound = Column(types.Enum(values=wound_values.keys(), translations=wound_values))
    flower_buds = Column(types.Enum(values=flower_buds_values.keys(), translations=flower_buds_values))
    fungicide = Column(UnicodeText)
    hormone = Column(UnicodeText)
    media = Column(UnicodeText)
    container = Column(UnicodeText)
    location = Column(UnicodeText)
    cover = Column(UnicodeText)
    bottom_heat_temp = Column(Integer, autoincrement=False)
    bottom_heat_unit = Column(types.Enum(values=bottom_heat_unit_values.keys(), translations=bottom_heat_unit_values), nullable=True)
    rooted_pct = Column(Integer, autoincrement=False)
    propagation_id = Column(Integer, ForeignKey('propagation.id'), nullable=False)
    rooted = relation('PropRooted', cascade='all,delete-orphan', backref=backref('cutting', uselist=False))
...
class PropSeed(db.Base):
    ...
    nseedlings = Column(Integer, autoincrement=False)
    germ_pct = Column(Integer, autoincrement=False)
...
class PropagationTabPresenter(editor.GenericEditorPresenter):
    ...
    def create_propagation_box(self, propagation):
        ...
        title = '%(prop_type)s on %(prop_date)s' % dict(prop_type=prop_type, prop_date=date_str)
...
class PropagationEditorView(editor.GenericEditorView):
    ...
    def __init__(self, parent=None):
        """
        """
        super(PropagationEditorView, self).__init__(os.path.join(paths.lib_dir(), 'plugins', 'garden', 'prop_editor.glade'), parent=parent)
...
class CuttingPresenter(editor.GenericEditorPresenter):
    widget_to_field_map = {'cutting_type_combo': 'cutting_type', 'cutting_length_entry': 'length', 
...
class SeedPresenter(editor.GenericEditorPresenter):
    widget_to_field_map = {'seed_pretreatment_textview': 'pretreatment', 'seed_nseeds_entry': 'nseeds', 
    ...
    def __init__(self, parent, model, view, session):
        ...
        widgets = self.view.widgets
        distinct = lambda c: utils.get_distinct_values(c, self.session)
        utils.setup_text_combobox(self.view.widgets.seed_media_comboentry, distinct(PropSeed.media))
        utils.setup_text_combobox(self.view.widgets.seed_container_comboentry, distinct(PropSeed.container))
        utils.setup_text_combobox(self.view.widgets.seed_location_comboentry, distinct(PropSeed.location))
        self.refresh_view()
        self.assign_simple_handler('seed_pretreatment_textview', 'pretreatment', editor.UnicodeOrNoneValidator())
        self.assign_simple_handler('seed_nseeds_entry', 'nseeds', editor.UnicodeOrNoneValidator())
        self.assign_simple_handler('seed_sown_entry', 'date_sown', editor.DateValidator())
        utils.setup_date_button(self.view, 'seed_sown_entry', 'seed_sown_button')
        self.assign_simple_handler('seed_container_comboentry', 'container', editor.UnicodeOrNoneValidator())
        self.assign_simple_handler('seed_media_comboentry', 'media', editor.UnicodeOrNoneValidator())
        self.assign_simple_handler('seed_location_comboentry', 'location', editor.UnicodeOrNoneValidator())
        self.assign_simple_handler('seed_mvdfrom_entry', 'moved_from', editor.UnicodeOrNoneValidator())
        self.assign_simple_handler('seed_mvdto_entry', 'moved_to', editor.UnicodeOrNoneValidator())
        self.assign_simple_handler('seed_germdate_entry', 'germ_date', editor.DateValidator())
        utils.setup_date_button(self.view, 'seed_germdate_entry', 'seed_germdate_button')
        self.assign_simple_handler('seed_ngerm_entry', 'nseedlings')
        self.assign_simple_handler('seed_pctgerm_entry', 'germ_pct')
        self.assign_simple_handler('seed_date_planted_entry', 'date_planted', editor.DateValidator())
...
class PropagationPresenter(editor.ChildPresenter):
    ...
    """
    widget_to_field_map = {'prop_type_combo': 'prop_type', 'prop_date_entry': 'date', 
    ...
    def __init__(self, model, view):
        ...
        self._cutting_presenter = CuttingPresenter(self, self.model, self.view, self.session)
        ...
        if self.model.date:
            ...
            self.view.widget_set_value(self.view.widgets.prop_date_entry, utils.today_str())
        self.view.widget_set_value(self.view.widgets.notes_textview, self.model.notes)
        self._dirty = False
        ...
        self.assign_simple_handler('notes_textview', 'notes', editor.UnicodeOrNoneValidator())
    ...
    def on_prop_type_changed(self, combo, *args):
        ...
        prop_box_map = {'Seed': self.view.widgets.seed_box, 'UnrootedCutting': self.view.widgets.cutting_box}
    ...
    def set_model_attr(self, field, value, validator=None):
        ...
        """
        super(PropagationPresenter, self).set_model_attr(field, value, validator)
        self._dirty = True
...
class SourcePropagationPresenter(PropagationPresenter):
    ...
    def __init__(self, parent, model, view, session):
        ...
        try:
            ...
            filename = os.path.join(paths.lib_dir(), 'plugins', 'garden', 'prop_editor.glade')
    ...
    def on_prop_type_changed(self, combo, *args):
        ...
        if not prop_type:
            ...
            super(SourcePropagationPresenter, self).on_prop_type_changed(combo, *args)
        return
