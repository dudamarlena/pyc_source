...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bauble/plugins/plants/taxonomy_check.py
...
def start_taxonomy_check():
    ...
    """
    view = GenericEditorView(os.path.join(paths.lib_dir(), 'plugins', 'plants', 'taxonomy_check.glade'), parent=None, root_widget_name='dialog1')
...
def species_to_fix(ssn, binomial, author, create=False):
    if binomial.find(' ') == -1:
        ...
        return Species.retrieve_or_create(ssn, {'object': 'taxon', 'rank': 'species', 
           'ht-epithet': gen_epithet, 
           'epithet': sp_epithet, 
           'ht-rank': 'genus', 
           'author': author}, create=create)
...
class BatchTaxonomicCheckPresenter(GenericEditorPresenter):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.binomials = [ item.str(item, remove_zws=True) for item in self.model.selection if isinstance(item, Species) and item.sp != ''
                         ]
    ...
    def on_frame1_next(self, *args):
        ...
        for binomial, response in zip(self.binomials, responses):
            ...
            if response['Taxonomic_status'] == 'Synonym':
                row = [
                 True, YES_ICON, '', response['Accepted_name'],
        ...
        for row in self.tick_off_list:
            ...
            if row[OLD_BINOMIAL] == '':
                ...
                if row[TAXON_STATUS] == 'Synonym':
                    accepted = species_to_fix(self.session, row[ACCEPTED_BINOMIAL], row[ACCEPTED_AUTHORSHIP], create=True)
                else:
                    accepted = None
    ...
    def on_toggle_all_clicked(self, *args):
        all_active = reduce(lambda a, b: a and b, [ row[TO_PROCESS] for row in self.tick_off_list if row[ACCEPTABLE]
                                                  ])
    ...
    def on_filebtnbrowse_clicked(self, *args):
        ...
        self.view.run_file_chooser_dialog(_('Choose a file...'), None, action=gtk.FILE_CHOOSER_ACTION_SAVE, buttons=(
         ...
         gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL), last_folder=last_folder, target='file_path_entry')
        return
