...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bauble/plugins/garden/accession.py
...
import lxml.etree as etree, pango
from sqlalchemy import and_, or_, func
from sqlalchemy import ForeignKey, Column, Unicode, Integer, Boolean, UnicodeText
from sqlalchemy.orm import EXT_CONTINUE, MapperExtension, backref, relation, reconstructor, validates
from sqlalchemy.orm.session import object_session
from sqlalchemy.exc import DBAPIError
import bauble, bauble.db as db, bauble.editor as editor
from bauble import meta
from bauble.error import check
import bauble.paths as paths
from bauble.plugins.garden.propagation import SourcePropagationPresenter, Propagation
from bauble.plugins.garden.source import SourceDetail, SourceDetailEditor, Source, Collection, CollectionPresenter, PropagationChooserPresenter
import bauble.prefs as prefs, bauble.btypes as types, bauble.utils as utils
...
def generic_taxon_add_action(model, view, presenter, top_presenter, button, taxon_entry):
...
def remove_callback(accessions):
    ...
    if len(acc.plants) > 0:
        ...
        values = dict(num_plants=len(acc.plants), plant_codes=safe((', ').join(plants)), acc_code=safe(acc))
        msg = _('%(num_plants)s plants depend on this accession: <b>%(plant_codes)s</b>\n\nAre you sure you want to remove accession <b>%(acc_code)s</b>?') % values
    else:
        msg = _('Are you sure you want to remove accession <b>%s</b>?') % utils.xml_safe(unicode(acc))
...
edit_action = Action('acc_edit', _('_Edit'), callback=edit_callback, accelerator='<ctrl>e')
add_plant_action = Action('acc_add', _('_Add plants'), callback=add_plants_callback, accelerator='<ctrl>k')
remove_action = Action('acc_remove', _('_Delete'), callback=remove_callback, accelerator='<ctrl>Delete')
acc_context_menu = [
 edit_action, add_plant_action, remove_action]
ver_level_descriptions = {0: _('The name of the record has not been checked by any authority.'), 1: _('The name of the record determined by comparison with other named plants.'), 
   2: _('The name of the record determined by a taxonomist or by other competent persons using herbarium and/or library and/or  documented living material.'), 
   3: _('The name of the plant determined by taxonomist engaged in systematic revision of the group.'), 
   4: _('The record is part of type gathering or propagated from type material by asexual methods.')}
...
class Verification(db.Base):
    ...
    species = relation('Species', primaryjoin='Verification.species_id==Species.id')
    prev_species = relation('Species', primaryjoin='Verification.prev_species_id==Species.id')
...
purchase_prov_status_values = [
 ...
 (
  None, '')]
recvd_type_values = {'ALAY': _('Air layer'), 
...
class AccessionNote(db.Base, db.Serializable):
    ...
    accession_id = Column(Integer, ForeignKey('accession.id'), nullable=False)
    accession = relation('Accession', uselist=False, backref=backref('notes', cascade='all, delete-orphan'))
    ...
    def retrieve(cls, session, keys):
        ...
        if 'accession' in keys:
            q = q.join(Accession).filter(Accession.code == keys['accession'])
        if 'date' in keys:
...
class Accession(db.Base, db.Serializable, db.WithNotes):
    ...
    __mapper_args__ = {'order_by': 'accession.code', 'extension': AccessionMapperExtension()}
    ...
    prov_type = Column(types.Enum(values=[ i[0] for i in prov_type_values ], translations=dict(prov_type_values)), default=None)
    wild_prov_status = Column(types.Enum(values=[ i[0] for i in wild_prov_status_values ], translations=dict(wild_prov_status_values)), default=None)
    date_accd = Column(types.Date)
    date_recvd = Column(types.Date)
    quantity_recvd = Column(Integer, autoincrement=False)
    recvd_type = Column(types.Enum(values=recvd_type_values.keys(), translations=recvd_type_values), default=None)
    id_qual_rank = Column(Unicode(10))
    id_qual = Column(types.Enum(values=['aff.', 'cf.', 'incorrect',
     'forsan', 'near', '?', None]), default=None)
    ...
    source = relation('Source', uselist=False, cascade='all, delete-orphan', backref=backref('accession', uselist=False))
    species = relation('Species', uselist=False, backref=backref('accessions', cascade='all, delete-orphan'))
    plants = relation('Plant', cascade='all, delete-orphan', backref=backref('accession', uselist=False))
    verifications = relation('Verification', cascade='all, delete-orphan', backref=backref('accession', uselist=False))
    vouchers = relation('Voucher', cascade='all, delete-orphan', backref=backref('accession', uselist=False))
    intended_location = relation('Location', primaryjoin='Accession.intended_location_id==Location.id')
    intended2_location = relation('Location', primaryjoin='Accession.intended2_location_id==Location.id')
    ...
    def search_view_markup_pair(self):
        ...
        """
        first, second = utils.xml_safe(unicode(self)), self.species_str(markup=True)
        suffix = _('%(1)s plant groups in %(2)s location(s)') % {'1': len(set(self.plants)), 
           '2': len(set(p.location for p in self.plants))}
        suffix = '<span foreground="#555555" size="small" weight="light"> - %s</span>' % suffix
    ...
    def species_str(self, authors=False, markup=False):
        ...
        if not self.species:
            ...
            if self.id_qual in ('aff.', 'cf.') and not self.id_qual_rank and not self.__warned_about_id_qual:
                msg = _('If the id_qual is aff. or cf. then id_qual_rank is required. %s ') % self.code
                logger.warning(msg)
                self.__warned_about_id_qual = True
            if self.id_qual:
                sp_str = self.species.str(authors, markup, remove_zws=True, qualification=(
                 self.id_qual_rank, self.id_qual))
            else:
    ...
    def compute_serializable_fields(cls, session, keys):
        ...
        if 'rank' in keys and 'taxon' in keys:
            if keys['rank'] == 'species':
                ...
                result['species'] = Species.retrieve_or_create(session, {'ht-epithet': keys['taxon'], 'epithet': 'sp'})
            elif keys['rank'] == 'familia':
                unknown_genus = 'Zzz-' + keys['taxon'][:-1]
                Genus.retrieve_or_create(session, {'ht-epithet': keys['taxon'], 'epithet': unknown_genus})
                result['species'] = Species.retrieve_or_create(session, {'ht-epithet': unknown_genus, 'epithet': 'sp'})
        return result
    ...
    def top_level_count(self):
        ...
        return {(1, 'Accessions'): 1, (2, 'Species'): set([self.species.id]), 
...
class AccessionEditorView(editor.GenericEditorView):
    ...
    """
    expanders_pref_map = {}
    _tooltips = {'acc_species_entry': _('The species must be selected from the list of completions. To add a species use the Species editor.'), 
       'acc_code_entry': _('The accession ID must be a unique code'), 
       'acc_id_qual_combo': _('The ID Qualifier\n\nPossible values: %s') % utils.enum_values_str('accession.id_qual'), 
       'acc_id_qual_rank_combo': _('The part of the taxon name that the id qualifier refers to.'), 
       ...
       'acc_quantity_recvd_entry': _('The amount of plant material at the time it was accessioned.'), 
       'intended_loc_comboentry': _('The intended location for plant material being accessioned.'), 
       'intended2_loc_comboentry': _('The intended location for plant material being accessioned.'), 
       'acc_prov_combo': _('The origin or source of this accession.\n\nPossible values: %s') % (', ').join(i[1] for i in prov_type_values), 
       'acc_wild_prov_combo': _('The wild status is used to clarify the provenance.\n\nPossible values: %s') % (', ').join(i[1] for i in wild_prov_status_values), 
       'acc_private_check': _('Indicates whether this accession record should be considered private.'), 
       'acc_cancel_button': _('Cancel your changes.'), 
       'acc_ok_button': _('Save your changes.'), 
       'acc_ok_and_add_button': _('Save your changes changes and add a plant to this accession.'), 
       'acc_next_button': _('Save your changes changes and add another accession.'), 
    ...
    def __init__(self, parent=None):
        ...
        """
        super(AccessionEditorView, self).__init__(os.path.join(paths.lib_dir(), 'plugins', 'garden', 'acc_editor.glade'), parent=parent)
        self.attach_completion('acc_species_entry', cell_data_func=self.species_cell_data_func, match_func=self.species_match_func)
        self.set_accept_buttons_sensitive(False)
        self.restore_state()
        completion = self.attach_completion('datum_entry', minimum_key_length=1, match_func=self.datum_match, text_column=0)
    ...
    def species_match_func(completion, key, treeiter, data=None):
        ...
        if str(species).lower().startswith(key.lower()) or str(species.genus.genus).lower().startswith(key.lower()):
            return True
        return False
    ...
    def species_cell_data_func(column, renderer, model, treeiter, data=None):
        ...
        """
        v = model[treeiter][0]
        renderer.set_property('text', '%s (%s)' % (v.str(authors=True), v.genus.family))
...
class VoucherPresenter(editor.GenericEditorPresenter):
    ...
    def __init__(self, parent, model, view, session):
        ...
        self.view.connect('parent_voucher_add_button', 'clicked', self.on_add_clicked, True)
        self.view.connect('parent_voucher_remove_button', 'clicked', self.on_remove_clicked, True)
        ...
        setup_column('voucher_treeview', 'voucher_herb_column', 'voucher_herb_cell', 'herbarium')
        setup_column('voucher_treeview', 'voucher_code_column', 'voucher_code_cell', 'code')
        setup_column('parent_voucher_treeview', 'parent_voucher_herb_column', 'parent_voucher_herb_cell', 'herbarium')
        setup_column('parent_voucher_treeview', 'parent_voucher_code_column', 'parent_voucher_code_cell', 'code')
...
class VerificationPresenter(editor.GenericEditorPresenter):
    ...
    def add_verification_box(self, model=None):
        ...
        self.view.widgets.verifications_parent_box.pack_start(box, expand=False, fill=False)
    ...
    class VerificationBox(gtk.HBox):
        ...
        def __init__(self, parent, model):
            ...
            filename = os.path.join(paths.lib_dir(), 'plugins', 'garden', 'acc_editor.glade')
            ...
            self.presenter().view.connect(entry, 'changed', self.on_entry_changed, 'verifier')
            ...
            self.presenter().view.connect(self.date_entry, 'changed', self.on_date_entry_changed)
            ref_entry = self.widgets.ver_ref_entry
            if self.model.reference:
                ref_entry.props.text = self.model.reference
            self.presenter().view.connect(ref_entry, 'changed', self.on_entry_changed, 'reference')

            def sp_get_completions(text):
                query = self.presenter().session.query(Species).join('genus').filter(utils.ilike(Genus.genus, '%s%%' % text)).filter(Species.id != self.model.id).order_by(Species.sp)
                return query
            ...
            self.presenter().assign_completions_handler(ver_prev_taxon_entry, sp_get_completions, on_prevsp_select)
            ...
            self.presenter().assign_completions_handler(ver_new_taxon_entry, sp_get_completions, on_sp_select)
            self.presenter().view.connect(self.widgets.ver_taxon_add_button, 'clicked', self.on_taxon_add_button_clicked, ver_new_taxon_entry)
            ...
            self._sid = self.presenter().view.connect(button, 'clicked', self.on_remove_button_clicked)
            self.update_label()
            return
        ...
        def update_label(self):
            ...
            label = (' ').join(parts) % dict(date=self.model.date, species=self.model.species, verifier=self.model.verifier)
        ...
        def on_taxon_add_button_clicked(self, button, taxon_entry):
            generic_taxon_add_action(self.model, self.presenter().view, self.presenter(), self.presenter().parent_ref(), button, taxon_entry)
...
class SourcePresenter(editor.GenericEditorPresenter):
    ...
    def __init__(self, parent, model, view, session):
        ...
        self.session = session
        self._dirty = False
        self.view.connect('new_source_button', 'clicked', self.on_new_source_button_clicked)
        ...
        if self.model.source:
            ...
            self.view.widgets.sources_code_entry.props.text = self.source.sources_code
        else:
            self.source = Source()
            self.model.source = self.source
        ...
        self.source_prop_presenter = SourcePropagationPresenter(self.parent_ref(), self.propagation, view, session)
        self.prop_chooser_presenter = PropagationChooserPresenter(self.parent_ref(), self.source, view, session)
        self.collection_presenter = CollectionPresenter(self.parent_ref(), self.collection, view, session)
        ...
        self.view.connect('source_coll_add_button', 'clicked', self.on_coll_add_button_clicked)
        self.view.connect('source_coll_remove_button', 'clicked', self.on_coll_remove_button_clicked)
        self.view.connect('source_prop_add_button', 'clicked', self.on_prop_add_button_clicked)
        self.view.connect('source_prop_remove_button', 'clicked', self.on_prop_remove_button_clicked)
    ...
    def all_problems(self):
        ...
        child presenters
        """
        return self.problems | self.collection_presenter.problems | self.prop_chooser_presenter.problems | self.source_prop_presenter.problems
    ...
    def is_dirty(self):
        return self._dirty or self.source_prop_presenter.is_dirty() or self.prop_chooser_presenter.is_dirty() or self.collection_presenter.is_dirty()
    ...
    def init_source_comboentry(self, on_select):
        ...
        def match_func(completion, key, treeiter, data=None):
            ...
            if utils.utf8(value).lower().startswith(key.lower()) or isinstance(value, SourceDetail) and str(value.id).startswith(key):
                return True
            return False
        ...
        def update_visible():
            visible = dict(source_sw=False, source_garden_prop_box=False, source_none_label=False)
            if entry.props.text == self.garden_prop_str:
                visible['source_garden_prop_box'] = True
            else:
                if not self.model.source or not self.model.source.source_detail:
        ...
        def on_entry_changed(entry, data=None):
            ...
            def _cmp(row, data):
                val = row[0]
                if utils.utf8(val) == data or isinstance(val, SourceDetail) and val.id == data:
                    return True
                return False
...
class AccessionEditorPresenter(editor.GenericEditorPresenter):
    widget_to_field_map = {'acc_code_entry': 'code', 'acc_id_qual_combo': 'id_qual', 
    ...
    def __init__(self, model, view):
        ...
        if not model.code:
            ...
            if self.model.species:
                self._dirty = True
        self.ver_presenter = VerificationPresenter(self, self.model, self.view, self.session)
        self.voucher_presenter = VoucherPresenter(self, self.model, self.view, self.session)
        self.source_presenter = SourcePresenter(self, self.model, self.view, self.session)
        ...
        def sp_get_completions(text):
            ...
            return query.filter(and_(Species.genus_id == Genus.id, or_(ilike(Genus.genus, '%s%%' % text), ilike(Genus.genus, '%s%%' % genus)))).order_by(Species.sp)
        ...
        def on_select(value):
            ...
            if not value:
                ...
                if not syn:
                    set_model(value)
                    return
                msg = _('The species <b>%(synonym)s</b> is a synonym of <b>%(species)s</b>.\n\nWould you like to choose <b>%(species)s</b> instead?') % {'synonym': syn.synonym, 'species': syn.species}
                box = None
                ...
                def on_response(button, response):
                    ...
                    if response:
                        ...
                        completion.set_model(model)
                        self.view.widgets.acc_species_entry.set_text(utils.utf8(syn.species))
        ...
        self.assign_completions_handler('acc_species_entry', sp_get_completions, on_select=on_select)
        self.assign_simple_handler('acc_prov_combo', 'prov_type')
        self.assign_simple_handler('acc_wild_prov_combo', 'wild_prov_status')
        self.view.connect('acc_recvd_type_comboentry', 'changed', self.on_recvd_type_comboentry_changed)
        self.view.connect(self.view.widgets.acc_recvd_type_comboentry.child, 'changed', self.on_recvd_type_entry_changed)
        self.view.connect('acc_code_entry', 'changed', self.on_acc_code_entry_changed)
        self.view.connect('acc_date_recvd_entry', 'changed', self.on_date_entry_changed, 'date_recvd')
        utils.setup_date_button(self.view, 'acc_date_recvd_entry', 'acc_date_recvd_button')
        self.view.connect('acc_date_accd_entry', 'changed', self.on_date_entry_changed, 'date_accd')
        utils.setup_date_button(self.view, 'acc_date_accd_entry', 'acc_date_accd_button')
        self.view.connect(self.view.widgets.intended_loc_add_button, 'clicked', self.on_loc_button_clicked, self.view.widgets.intended_loc_comboentry, 'intended_location')
        self.view.connect(self.view.widgets.intended2_loc_add_button, 'clicked', self.on_loc_button_clicked, self.view.widgets.intended2_loc_comboentry, 'intended2_location')
        self.view.connect(self.view.widgets.acc_taxon_add_button, 'clicked', lambda b, w: generic_taxon_add_action(self.model, self.view, self, self, b, w), self.view.widgets.acc_species_entry)
        self.assign_simple_handler('acc_quantity_recvd_entry', 'quantity_recvd')
        self.assign_simple_handler('acc_id_qual_combo', 'id_qual', editor.UnicodeOrNoneValidator())
        ...
        init_location_comboentry(self, self.view.widgets.intended_loc_comboentry, on_loc1_select, required=False)

        def on_loc2_select(value):
            self.set_model_attr('intended2_location', value)

        init_location_comboentry(self, self.view.widgets.intended2_loc_comboentry, on_loc2_select, required=False)
        self.refresh_sensitivity()
    ...
    def populate_code_formats(self, entry_one=None, values=None):
        ...
        ls.append([entry_one])
        if values is None:
            query = self.session.query(meta.BaubleMeta).filter(meta.BaubleMeta.name.like('acidf_%')).order_by(meta.BaubleMeta.name)
            if query.count():
    ...
    def on_acc_code_format_edit_btn_clicked(self, widget, *args):
        view = editor.GenericEditorView(os.path.join(paths.lib_dir(), 'plugins', 'garden', 'acc_editor.glade'), root_widget_name='acc_codes_dialog')
        ...
        if presenter.start() > 0:
            presenter.session.query(meta.BaubleMeta).filter(meta.BaubleMeta.name.like('acidf_%')).delete(synchronize_session=False)
            i = 1
    ...
    def is_dirty(self):
        presenters = [
         self.ver_presenter, self.voucher_presenter,
    ...
    def on_recvd_type_entry_changed(self, entry, *args):
        ...
        if not text.strip():
            ...
            def match_func(row, data):
                return str(row[0]).lower() == str(data).lower() or str(row[1]).lower() == str(data).lower()
    ...
    def on_acc_code_entry_changed(self, entry, data=None):
        ...
        if text != self._original_code and query.filter_by(code=unicode(text)).count() > 0:
            self.add_problem(self.PROBLEM_DUPLICATE_ACCESSION, self.view.widgets.acc_code_entry)
            self.set_model_attr('code', None)
            return
        else:
            self.remove_problem(self.PROBLEM_DUPLICATE_ACCESSION, self.view.widgets.acc_code_entry)
            if text is '':
                self.set_model_attr('code', None)
            else:
    ...
    def set_model_attr(self, field, value, validator=None):
        ...
        """
        super(AccessionEditorPresenter, self).set_model_attr(field, value, validator)
        self._dirty = True
        prov_sensitive = True
        ...
        if field == 'id_qual' and not self.model.id_qual_rank:
            self.add_problem(self.PROBLEM_ID_QUAL_RANK_REQUIRED, self.view.widgets.acc_id_qual_rank_combo)
        else:
    ...
    def validate(self, add_problems=False):
        ...
        for ver in self.model.verifications:
            ...
            if utils.get_invalid_columns(ver, ignore_columns=ignore) or not ver.species or not ver.prev_species:
                return False
        ...
        if self.model.source:
            ...
            if prop and prop.prop_type == 'Seed':
                ...
                if prop and prop.prop_type == 'UnrootedCutting':
    ...
    def refresh_sensitivity(self):
        ...
        sensitive = self.is_dirty() and self.validate() and not self.problems and not self.source_presenter.all_problems() and not self.ver_presenter.problems and not self.voucher_presenter.problems
    ...
    def refresh_view(self):
        ...
        self.view.widget_set_value('acc_wild_prov_combo', dict(wild_prov_status_values)[self.model.wild_prov_status], index=1)
        self.view.widget_set_value('acc_prov_combo', dict(prov_type_values)[self.model.prov_type], index=1)
        self.view.widget_set_value('acc_recvd_type_comboentry', recvd_type_values[self.model.recvd_type], index=1)
...
class AccessionEditor(editor.GenericModelViewPresenterEditor):
    ...
    def handle_response(self, response):
        ...
        if response == gtk.RESPONSE_OK or response in self.ok_responses:
            try:
                ...
                msg = _('Error committing changes.\n\n%s') % utils.xml_safe(unicode(e.orig))
    ...
    def start(self):
        ...
        if self.session.query(Species).count() == 0:
            msg = _('You must first add or import at least one species into the database before you can add accessions.')
    ...
    def _cleanup_collection(model):
        ...
        if not model:
            ...
            if model.latitude is not None or model.longitude is not None:
                if model.latitude is not None and model.longitude is None or model.longitude is not None and model.latitude is None:
    ...
    def _cleanup_propagation(self, propagation):
        if propagation.prop_type == 'UnrootedCutting':
        ...
        elif propagation.prop_type == 'Seed' and propagation._cutting is not None:
...
class GeneralAccessionExpander(InfoExpander):
    ...
    def update(self, row):
        ...
        self.widget_set_value('acc_code_data', '<big>%s</big>' % utils.xml_safe(unicode(row.code)), markup=True)
        ...
        self.widget_set_value('name_data', row.species_str(markup=True), markup=True)
        ...
        if plant_locations:
            ...
            for location, quantity in plant_locations.iteritems():
                strs.append(_('%(quantity)s in %(location)s') % dict(location=str(location), quantity=quantity))
        ...
        loc_map = (
         ('intended_loc_data', 'intended_location'),
...
class SourceExpander(InfoExpander):
    ...
    def update(self, row):
        ...
        if row.source.source_detail:
            ...
            self.widget_set_value('source_name_data', utils.utf8(row.source.source_detail))

            def on_source_clicked(w, e, x):
                select_in_search_results(x)

            utils.make_label_clickable(self.widgets.source_name_data, on_source_clicked, row.source.source_detail)
        else:
        ...
        if row.source.plant_propagation:
            ...
            self.widget_set_value('parent_plant_data', str(row.source.plant_propagation.plant))
            self.widget_set_value('propagation_data', row.source.plant_propagation.get_summary())
        else:
...
class AccessionInfoBox(InfoBox):
    ...
    def __init__(self):
        ...
        filename = os.path.join(paths.lib_dir(), 'plugins', 'garden', 'acc_infobox.glade')
...
datums = {'Adindan': 'Adindan- Ethiopia, Mali, Senegal, Sudan', 'Afgooye': 'Afgooye- Somalia', 
