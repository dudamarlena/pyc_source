...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bauble/plugins/garden/correct_taxon.py
...
def species_to_fix(s, create=False):
    ...
    return plants.Species.retrieve_or_create(ssn, {'object': 'taxon', 'rank': 'species', 
       'ht-epithet': gen_epithet, 
       'epithet': sp_epithet, 
       'ht-rank': 'genus'}, create=create)
...
with codecs.open('/tmp/complete.csv', 'r', 'utf16') as (f):
    ...
    for l in f.readlines():
        ...
        print 'corrected %(Name_submitted)s to %(Name_matched)s %(Name_matched_author)s' % fields
        if fields['Taxonomic_status'] == 'Synonym' and fields['Accepted_name']:
            ...
            print 'set %(Name_matched)s %(Name_matched_author)s as synonym of %(Accepted_name)s %(Accepted_name_author)s' % fields
