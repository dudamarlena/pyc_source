...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bauble/plugins/plants/species_model.py
# Compiled at: 2016-10-03 09:39:22
from itertools import chain
import logging
logger = logging.getLogger(__name__)
from sqlalchemy.ext.associationproxy import association_proxy
from sqlalchemy import Column, Boolean, Unicode, Integer, ForeignKey, UnicodeText, func, UniqueConstraint
from sqlalchemy.orm import relation, backref
import bauble.db as db, bauble.error as error, bauble.utils as utils, bauble.btypes as types
...
infrasp_rank_values = {'subsp.': _('subsp.'), 'var.': _('var.'), 
   'subvar.': _('subvar'), 
   'f.': _('f.'), 
   'subf.': _('subf.'), 
   'cv.': _('cv.'), 
...
class Species(db.Base, db.Serializable, db.DefiningPictures, db.WithNotes):
    ...
    def search_view_markup_pair(self):
        ...
        try:
            ...
            trail = self.sp_author and ' <span weight="light">%s</span>' % utils.xml_safe(self.sp_author) or ''
            if self.accepted:
                trail += ('<span foreground="#555555" size="small" weight="light"> - ' + _('synonym of %s') + '</span>') % self.accepted.markup(authors=True)
            return (
             self.markup(authors=False) + trail, substring)
        except:
            return ('...', '...')
    ...
    def conservation(self):
        ...
        not enforced by the software in v1.0.x
        """
        {'EX': _('Extinct (EX)'), 'EW': _('Extinct Wild (EW)'), 
        ...
        notes = [ i.note for i in self.notes if i.category and i.category.upper() == 'IUCN'
                ]
        return (notes + ['DD'])[0]
    ...
    sp2 = Column(Unicode(64), index=True)
    sp_author = Column(Unicode(128))
    hybrid = Column(Boolean, default=False)
    sp_qual = Column(types.Enum(values=['agg.', 's. lat.', 's. str.', None]), default=None)
    cv_group = Column(Unicode(50))
    trade_name = Column(Unicode(64))
    infrasp1 = Column(Unicode(64))
    infrasp1_rank = Column(types.Enum(values=infrasp_rank_values.keys(), translations=infrasp_rank_values))
    infrasp1_author = Column(Unicode(64))
    infrasp2 = Column(Unicode(64))
    infrasp2_rank = Column(types.Enum(values=infrasp_rank_values.keys(), translations=infrasp_rank_values))
    infrasp2_author = Column(Unicode(64))
    infrasp3 = Column(Unicode(64))
    infrasp3_rank = Column(types.Enum(values=infrasp_rank_values.keys(), translations=infrasp_rank_values))
    infrasp3_author = Column(Unicode(64))
    infrasp4 = Column(Unicode(64))
    infrasp4_rank = Column(types.Enum(values=infrasp_rank_values.keys(), translations=infrasp_rank_values))
    ...
    _synonyms = relation('SpeciesSynonym', primaryjoin='Species.id==SpeciesSynonym.species_id', cascade='all, delete-orphan', uselist=True, backref='species')
    _syn = relation('SpeciesSynonym', primaryjoin='Species.id==SpeciesSynonym.synonym_id', cascade='all, delete-orphan', uselist=True)
    vernacular_names = relation('VernacularName', cascade='all, delete-orphan', collection_class=VNList, backref=backref('species', uselist=False))
    _default_vernacular_name = relation('DefaultVernacularName', uselist=False, cascade='all, delete-orphan', backref=backref('species', uselist=False))
    distribution = relation('SpeciesDistribution', cascade='all, delete-orphan', backref=backref('species', uselist=False))
    ...
    default_vernacular_name = property(_get_default_vernacular_name, _set_default_vernacular_name, _del_default_vernacular_name)
    ...
    def str(self, authors=False, markup=False, remove_zws=False, genus=True, qualification=None):
        """
        ...
        if markup:
            ...
            italicize = lambda s: '<i>%s</i>' % escape(s).replace('×', '</i>×<i>')
        ...
        if self.cv_group and not group_added:
            infrasp_parts.append(_('%(group)s Group') % dict(group=self.cv_group))
        binomial = [
         genus, self.hybrid and self.hybrid_char, sp, author]
        tail = []
        if self.sp_qual:
            tail = [
             self.sp_qual]
    ...
    def accepted(self):
        ...
        if not session:
            ...
            syn = session.query(SpeciesSynonym).filter(SpeciesSynonym.synonym_id == self.id).first()
    ...
    def accepted(self, value):
        ...
        if not isinstance(value, self.__class__):
            ...
            from sqlalchemy.orm.session import object_session
            session = object_session(self)
            session or logger.warn('species:accepted.setter - object not in session')
            return
        session.query(SpeciesSynonym).filter(SpeciesSynonym.synonym_id == self.id).delete()
        session.commit()
    ...
    infrasp_attr = {1: {'rank': 'infrasp1_rank', 'epithet': 'infrasp1', 
           'author': 'infrasp1_author'}, 
       2: {'rank': 'infrasp2_rank', 'epithet': 'infrasp2', 
           'author': 'infrasp2_author'}, 
       3: {'rank': 'infrasp3_rank', 'epithet': 'infrasp3', 
           'author': 'infrasp3_author'}, 
       4: {'rank': 'infrasp4_rank', 'epithet': 'infrasp4', 
    ...
    def as_dict(self, recurse=True):
        result = dict((col, getattr(self, col)) for col in self.__table__.columns.keys() if col not in ('id',
                                                                                                        'sp') and col[0] != '_' and getattr(self, col) is not None and not col.endswith('_id'))
        result['object'] = 'taxon'
        result['rank'] = 'species'
    ...
    def retrieve(cls, session, keys):
        ...
        try:
            return session.query(cls).filter(cls.sp == keys['epithet']).join(Genus).filter(Genus.genus == keys['ht-epithet']).one()
        except:
            return
    ...
    def compute_serializable_fields(cls, session, keys):
        ...
        result['genus'] = Genus.retrieve_or_create(session, {'epithet': keys['ht-epithet'], 'ht-epithet': specifies_family}, create=specifies_family is not None)
    ...
    def top_level_count(self):
        ...
        return {(1, 'Species'): 1, (2, 'Genera'): set([self.genus.id]), 
           ...
           (8, 'Sources'): set([ a.source.source_detail.id for a in self.accessions if a.source and a.source.source_detail
                         ])}
...
class SpeciesNote(db.Base, db.Serializable):
    ...
    species = relation('Species', uselist=False, backref=backref('notes', cascade='all, delete-orphan'))
    ...
    def retrieve(cls, session, keys):
        ...
        try:
            return session.query(cls).filter(cls.category == keys['category']).join(Species).filter(Species.sp == epithet).join(Genus).filter(Genus.genus == genus).one()
        except:
            return
...
class SpeciesSynonym(db.Base):
    ...
    synonym = relation('Species', uselist=False, primaryjoin='SpeciesSynonym.synonym_id==Species.id')
...
class VernacularName(db.Base, db.Serializable):
    ...
    def search_view_markup_pair(self):
        ...
        """
        return (
         str(self), self.species.markup(authors=False))
    ...
    def retrieve(cls, session, keys):
        ...
        sp = session.query(Species).filter(Species.sp == s_epithet).join(Genus).filter(Genus.genus == g_epithet).first()
        try:
            return session.query(cls).filter(cls.species == sp, cls.language == keys['language']).one()
        except:
            return
...
class DefaultVernacularName(db.Base):
    ...
    species_id = Column(Integer, ForeignKey('species.id'), nullable=False)
    vernacular_name_id = Column(Integer, ForeignKey('vernacular_name.id'), nullable=False)
...
SpeciesDistribution.geography = relation('Geography', primaryjoin='SpeciesDistribution.geography_id==Geography.id', uselist=False)
