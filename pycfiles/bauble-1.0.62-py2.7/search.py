...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bauble/search.py
# Compiled at: 2016-10-03 09:39:22
import weakref, gtk, logging
...
class TypedValueToken(ValueABC):
    ...
    constructor = {'datetime': (datetime, int), 'now': (
             datetime.now, id)}
...
class IdentifierToken(object):
    ...
    def evaluate(self, env):
        ...
        if len(self.value) == 1:
            ...
            query = query.join(aliased=True, *self.value[:-1])
...
class IdentExpression(object):
    ...
    def __init__(self, t):
        ...
        self.operation = {'=': lambda x, y: x == y, 
           ...
           'ihas': lambda x, y: utils.ilike(x, '%%%s%%' % y)}.get(self.op)
    ...
    def needs_join(self, env):
        return [
         self.operands[0].needs_join(env)]
...
class BetweenExpressionAction(object):
    ...
    def evaluate(self, env):
        ...
        return q.filter(and_(clause_low(self.operands[1].express()), clause_high(self.operands[2].express())))
...
class QueryAction(object):
    ...
    def invoke(self, search_strategy):
        ...
        domain = self.domain
        check(domain in search_strategy._domains or domain in search_strategy._shorthand, 'Unknown search domain: %s' % domain)
...
class BinomialNameAction(object):
    ...
    def invoke(self, search_strategy):
        ...
        result = search_strategy._session.query(Species).filter(Species.sp.startswith(self.species_epithet)).join(Genus).filter(Genus.genus.startswith(self.genus_epithet)).all()
        result = set(result)
        if None in result:
            logger.warn('removing None from result set')
            result = set(i for i in result if i is not None)
    ...
    def invoke(self, search_strategy):
        ...
        if self.values == '*':
            ...
            if self.cond in ('like', 'ilike'):
                ...
                if self.cond in ('contains', 'icontains', 'has', 'ihas'):
                    condition = lambda col: lambda val: utils.ilike(mapper.c[col], '%%%s%%' % val)
                elif self.cond == '=':
...
class AggregatingAction(object):
    ...
    def needs_join(self, env):
        return [
         self.identifier.needs_join(env)]
...
class ValueListAction(object):
    ...
    def invoke(self, search_strategy):
        ...
        for cls, columns in search_strategy._properties.iteritems():
            ...
            table = class_mapper(cls)
            q = search_strategy._session.query(cls)
            q = q.filter(or_(*[ like(table, c, unicol(c, v)) for c, v in column_cross_value
                              ]))
            result.update(q.all())
...
from pyparsing import Word, alphas8bit, removeQuotes, delimitedList, Regex, OneOrMore, oneOf, alphas, alphanums, Group, Literal, CaselessLiteral, WordStart, WordEnd, srange, stringEnd, Keyword, quotedString, infixNotation, opAssoc, Forward
wordStart, wordEnd = WordStart(), WordEnd()

class SearchParser(object):
    """The parser for bauble.search.MapperSearch
    """
    numeric_value = Regex('[-]?\\d+(\\.\\d*)?([eE]\\d+)?').setParseAction(NumericToken)('number')
    unquoted_string = Word(alphanums + alphas8bit + '%.-_*;:')
    string_value = (quotedString.setParseAction(removeQuotes) | unquoted_string).setParseAction(StringToken)('string')
    none_token = Literal('None').setParseAction(NoneToken)
    empty_token = Literal('Empty').setParseAction(EmptyToken)
    value_list = Forward()
    typed_value = (Literal('|') + unquoted_string + Literal('|') + value_list + Literal('|')).setParseAction(TypedValueToken)
    value = (typed_value | WordStart('0123456789.-e') + numeric_value + WordEnd('0123456789.-e') | none_token | empty_token | string_value).setParseAction(ValueToken)('value')
    value_list << Group(OneOrMore(value) ^ delimitedList(value)).setParseAction(ValueListAction)('value_list')
    domain = Word(alphas, alphanums)
    binop = oneOf('= == != <> < <= > >= not like contains has ilike icontains ihas is')
    binop_set = oneOf('in')
    equals = Literal('=')
    star_value = Literal('*')
    domain_values = value_list.copy()('domain_values')
    domain_expression = (domain + equals + star_value + stringEnd | domain + binop + domain_values + stringEnd).setParseAction(DomainExpressionAction)('domain_expression')
    caps = srange('[A-Z]')
    lowers = caps.lower()
    binomial_name = (Word(caps, lowers) + Word(lowers)).setParseAction(BinomialNameAction)('binomial_name')
    ...
    aggregating_func = Literal('sum') | Literal('min') | Literal('max') | Literal('count')
    query_expression = Forward()('filter')
    identifier = Group(delimitedList(Word(alphas + '_', alphanums + '_'), '.')).setParseAction(IdentifierToken)
    aggregated = (aggregating_func + Literal('(') + identifier + Literal(')')).setParseAction(AggregatingAction)
    ident_expression = Group(identifier + binop + value).setParseAction(IdentExpression) | Group(identifier + binop_set + value_list).setParseAction(ElementSetExpression) | Group(aggregated + binop + value).setParseAction(AggregatedExpression) | (Literal('(') + query_expression + Literal(')')).setParseAction(ParenthesisedQuery)
    between_expression = Group(identifier + BETWEEN_ + value + AND_ + value).setParseAction(BetweenExpressionAction)
    query_expression << infixNotation(ident_expression | between_expression, [
     (
    ...
    query = (domain + Keyword('where', caseless=True).suppress() + Group(query_expression) + stringEnd).setParseAction(QueryAction)
    statement = (query('query') | domain_expression('domain') | binomial_name('binomial') | value_list('value_list')).setParseAction(StatementAction)('statement')
...
class MapperSearch(SearchStrategy):
    ...
    def add_meta(self, domain, cls, properties):
        ...
        check(isinstance(properties, list), _('MapperSearch.add_meta(): default_columns argument must be list'))
        check(len(properties) > 0, _('MapperSearch.add_meta(): default_columns argument cannot be empty'))
...
class SchemaBrowser(gtk.VBox):
    ...
    def _insert_props(self, mapper, model, treeiter):
        ...
        """
        column_properties = sorted(filter(lambda x: isinstance(x, ColumnProperty) and not x.key.startswith('_'), mapper.iterate_properties), key=lambda k: k.key)
        for prop in column_properties:
            model.append(treeiter, [prop.key, prop])

        relation_properties = sorted(filter(lambda x: isinstance(x, RelationProperty) and not x.key.startswith('_'), mapper.iterate_properties), key=lambda k: k.key)
...
class SchemaMenu(gtk.Menu):
    ...
    def on_activate(self, menuitem, prop):
        ...
        path = []
        path = [
         menuitem.get_child().props.label]
...
class ExpressionRow(object):
    ...
    def __init__(self, query_builder, remove_callback, row_number=None):
        ...
        self.schema_menu = SchemaMenu(self.mapper, self.on_schema_menu_activated, self.relation_filter)
        self.prop_button.connect('button-press-event', on_prop_button_clicked, self.schema_menu)
    ...
    def on_schema_menu_activated(self, menuitem, path, prop):
        ...
        bottom = self.table.child_get_property(self.value_widget, 'bottom-attach')
    ...
    def relation_filter(self, prop):
        if isinstance(prop, ColumnProperty) and isinstance(prop.columns[0].type, bauble.btypes.Date):
            return False
        return True
    ...
    def get_widgets(self):
        ...
        """
        return (
         self.and_or_combo, self.prop_button, self.cond_combo,
...
class QueryBuilder(gtk.Dialog):
    ...
    def __init__(self, parent=None):
        """
        """
        super(QueryBuilder, self).__init__(title=_('Query Builder'), parent=parent, flags=gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT, buttons=(
