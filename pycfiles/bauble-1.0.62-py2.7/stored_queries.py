...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bauble/plugins/plants/stored_queries.py
# Compiled at: 2016-10-03 09:39:22
import gtk, pango, logging
...
class StoredQueriesPresenter(editor.GenericEditorPresenter):
    widget_to_field_map = {'stqr_label_entry': 'label', 
...
def edit_callback():
    session = db.Session()
    view = editor.GenericEditorView(os.path.join(paths.lib_dir(), 'plugins', 'plants', 'stored_queries.glade'), parent=None, root_widget_name='stqr_dialog')
    stored_queries = StoredQueriesModel()
    presenter = StoredQueriesPresenter(stored_queries, view, session=session, refresh_view=True)
