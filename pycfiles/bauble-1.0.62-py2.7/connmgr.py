...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bauble/connmgr.py
...
def populate_dbtypes(package_list):
    ...
    """
    global dbtypes
    global working_dbtypes
    dbtypes = [ second for first, second in package_list ]
    working_dbtypes = [ second for first, second in package_list if is_package_name(first)
                      ]
...
def check_and_notify_new_version(view):
    version_on_github = ('https://raw.githubusercontent.com/Ghini/ghini' + '.desktop/ghini-%s.%s/bauble/version.py') % bauble.version_tuple[:2]
    try:
        import urllib2, ssl
        ...
        logger.warning('unhandled %s(%s) while checking for newer version' % type(e), e)
...
class ConnMgrPresenter(GenericEditorPresenter):
    ...
    :param default: the name of the connection to select from the list
      of connection names
    """
    widget_to_field_map = {'name_combo': 'connection_name', 
    ...
    def __init__(self, view=None):
        self.filename = self.database = self.host = self.user = self.pictureroot = self.connection_name = self.prev_connection_name = ''
        ...
        if self.connection_names:
            ...
            if self.connection_name not in self.connections:
                self.prev_connection_name = self.connection_name = self.connection_names[0]
            self.dbtype = None
            self.set_params()
        else:
            self.dbtype = ''
        ...
        self.start_thread(Thread(target=check_and_notify_new_version, args=[
         self.view]))
        return
    ...
    def on_file_btnbrowse_clicked(self, *args):
        ...
        self.view.run_file_chooser_dialog(_('Choose a file...'), None, action=gtk.FILE_CHOOSER_ACTION_SAVE, buttons=(
         ...
         gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL), last_folder=last_folder, target='file_entry')
        return

    def on_pictureroot_btnbrowse_clicked(self, *args):
        previously = self.view.widget_get_value('pictureroot_entry')
        last_folder, bn = os.path.split(previously)
        self.view.run_file_chooser_dialog(_('Choose a file...'), None, action=gtk.FILE_CHOOSER_ACTION_SELECT_FOLDER, buttons=(
         gtk.STOCK_OK, gtk.RESPONSE_ACCEPT,
         gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL), last_folder=last_folder, target='pictureroot_entry')
        return
    ...
    def on_pictureroot2_btnbrowse_clicked(self, *args):
        ...
        self.view.run_file_chooser_dialog(_('Choose a file...'), None, action=gtk.FILE_CHOOSER_ACTION_SELECT_FOLDER, buttons=(
         gtk.STOCK_OK, gtk.RESPONSE_ACCEPT,
         gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL), last_folder=last_folder, target='pictureroot2_entry')
        return
    ...
    def on_remove_button_clicked(self, button, data=None):
        ...
        """
        msg = _('Are you sure you want to remove "%s"?\n\n<i>Note: This only removes the connection to the database and does not affect the database or its data</i>') % self.connection_name
    ...
    def on_add_button_clicked(self, *args):
        if not self.compare_prefs_to_saved(self.prev_connection_name):
            msg = _('Do you want to save your changes to %s ?') % self.prev_connection_name
            if self.view.run_yes_no_dialog(msg):
                self.save_current_to_prefs()
        self.prev_connection_name = None
        name = self.view.run_entry_dialog(_('Enter a connection name'), self.view.get_window(), gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT, (
         gtk.STOCK_OK, gtk.RESPONSE_ACCEPT))
        if name is not '':
    ...
    def on_name_combo_changed(self, combo, data=None):
        ...
        conn_dict = self.connections
        if self.prev_connection_name is not None and self.prev_connection_name in self.connection_names:
            if self.prev_connection_name not in conn_dict:
                ...
                if self.view.run_yes_no_dialog(msg):
                    self.save_current_to_prefs()
        if self.connection_names:
            self.on_combo_changed(combo, data)
        ...
        if self.connection_name in conn_dict:
            if conn_dict[self.connection_name]['type'] not in dbtypes:
                ...
                index = dbtypes.index(conn_dict[self.connection_name]['type'])
    ...
    def get_passwd(self, title=_('Enter your password'), before_main=False):
        ...
        """
        passwd = self.view.run_entry_dialog(title, self.view.get_window(), gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT, (
         gtk.STOCK_OK, gtk.RESPONSE_ACCEPT))
        return passwd
            ...
            if params['type'] == 'SQLite':
                ...
                if not os.path.exists(filename):
                    ...
                    if not os.access(path, os.R_OK):
                        valid = False
                        msg = _('Ghini does not have permission to read the directory:\n\n%s') % path
                    elif not os.access(path, os.W_OK):
                        valid = False
                        msg = _('Ghini does not have permission to write to the directory:\n\n%s') % path
                elif not os.access(filename, os.R_OK):
                    valid = False
                    msg = _('Ghini does not have permission to read the database file:\n\n%s') % filename
                elif not os.access(filename, os.W_OK):
                    valid = False
                    msg = _('Ghini does not have permission to write to the database file:\n\n%s') % filename
            else:
                fields = []
                if params['user'] == '':
                    valid = False
    ...
    def get_params(self, new=None):
        ...
        if self.dbtype == 'SQLite':
            if self.use_defaults is True:
                ...
                self.filename = os.path.join(paths.appdata_dir(), name + '.db')
                self.pictureroot = os.path.join(paths.appdata_dir(), name)
            result = {'file': self.filename, 'default': self.use_defaults, 'pictures': self.pictureroot}
        else:
            result = {'db': self.database, 'host': self.host, 
               'user': self.user, 
