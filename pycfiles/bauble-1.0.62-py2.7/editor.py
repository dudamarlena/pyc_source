...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bauble/editor.py
# Compiled at: 2016-10-03 09:39:22
import datetime, os, sys, weakref, logging
logger = logging.getLogger(__name__)
logger.setLevel(logging.DEBUG)
import glib, gtk, gobject
from random import random
import dateutil.parser as date_parser, lxml.etree as etree, pango
from sqlalchemy.orm import object_mapper, object_session
from sqlalchemy.orm.exc import UnmappedInstanceError
from bauble.i18n import _
import bauble, bauble.db as db
from bauble.error import check
import bauble.paths as paths, bauble.prefs as prefs, bauble.utils as utils
...
class DateValidator(Validator):
    ...
    def to_python(self, value):
        if not value:
            ...
            try:
                date = date_parser.parse(value, dayfirst=dayfirst, yearfirst=yearfirst, default=default)
...
class GenericEditorView(object):
        ...
        if parent:
            ...
            for widget_name, markup in self._tooltips.iteritems():
                try:
                    ...
                    logger.debug(_("Couldn't set the tooltip on widget %(widget_name)s\n\n%(exception)s") % values)
    ...
    def run_file_chooser_dialog(self, text, parent, action, buttons, last_folder, target):
    ...
    def run_message_dialog(self, msg, type=gtk.MESSAGE_INFO, buttons=gtk.BUTTONS_OK, parent=None):
    ...
    def connect_after(self, obj, signal, callback, *args):
        """
        Attach a signal handler for signal on obj.  For more
        information see :meth:`gobject.connect_after`
    ...
    def widget_set_value(self, widget, value, markup=False, default=None, index=0):
        ...
        if isinstance(widget, gtk.Widget):
            ...
            utils.set_widget_value(self.widgets[widget], value, markup, default, index)
    ...
    def attach_completion(self, entry, cell_data_func=default_completion_cell_data_func, match_func=default_completion_match_func, minimum_key_length=2, text_column=-1):
        """
    ...
    def init_translatable_combo(self, combo, translations, default=None, cmp=None):
        ...
        if cmp is not None:
            translations = sorted(translations, cmp=lambda a, b: cmp(a[0], b[0]))
...
class GenericEditorPresenter(object):
    ...
    def __get_widget_name(self, widget):
        from types import StringTypes
        return isinstance(widget, StringTypes) and widget or gtk.Buildable.get_name(widget)
    ...
    def on_relation_entry_changed(self, widget, value=None):
        attr = self.__get_widget_attr(widget)
        logger.debug('calling unimplemented on_relation_entry_changed(%s, %s, %s(%s))' % (
    ...
    def remove_problem(self, problem_id, widget=None):
        ...
        if problem_id is None and widget is None:
            ...
            tmp = self.problems.copy()
            for p, w in tmp:
                if w == widget and p == problem_id or widget is None and p == problem_id or w == widget and problem_id is None:
    ...
    def assign_simple_handler(self, widget_name, model_attr, validator=None):
        ...
        class ProblemValidator(Validator):
            ...
            def to_python(self, value):
                try:
                    ...
                    logger.debug('GenericEditorPresenter.ProblemValidator.to_python %s' % e)
        ...
        if isinstance(widget, gtk.Entry):
         ...
         gtk.RadioButton)):
            ...
            raise ValueError('assign_simple_handler() -- widget type not supported: %s' % type(widget))
        return

    def assign_completions_handler(self, widget, get_completions, on_select=lambda v: v):
        """
        ...
        def on_changed(entry, *args):
            ...
            if text != '' and not found and PROBLEM not in self.problems:
                ...
                on_select(None)
            key_length = widget.get_completion().props.minimum_key_length
            if not comp_model and len(text) > key_length or len(text) == key_length:
        ...
        completion = widget.get_completion()
        check(completion is not None, "the gtk.Entry %s doesn't have a completion attached to it" % widget.get_name())
...
class GenericModelViewPresenterEditor(object):
    ...
    def attach_response(self, dialog, response, keyname, mask):
        ...
        def callback(widget, event, key, mask):
            if event.keyval == gtk.gdk.keyval_from_name(key) and event.state & mask:
...
class NoteBox(gtk.HBox):
    ...
    def set_content(self, text):
        ...
        utils.set_widget_value(self.widgets.note_textview, text or '')
    ...
    def __init__(self, presenter, model=None):
        ...
        self.widgets.notes_expander.props.label_widget.ellipsize = pango.ELLIPSIZE_END
        mapper = object_mapper(self.model)
        ...
        utils.setup_date_button(None, self.widgets.date_entry, self.widgets.date_button)
        ...
        self.widgets.date_entry.connect('changed', self.on_date_entry_changed)
        self.widgets.user_entry.connect('changed', self.on_user_entry_changed)
        self.widgets.category_comboentry.connect('changed', self.on_category_combo_changed)
        self.widgets.category_comboentry.child.connect('changed', self.on_category_entry_changed)
        self.widgets.notes_remove_button.connect('clicked', self.on_notes_remove_button)
        self.update_label()
        self.show_all()
        return
    ...
    def on_category_combo_changed(self, combo, *args):
        ...
        if treeiter:
            ...
            return
        self.widgets.category_comboentry.child.props.text = utils.utf8(text)
    ...
    def update_label(self):
        ...
        if self.model.user and date_str:
            label.append(_('%(user)s on %(date)s') % dict(user=utils.xml_safe(self.model.user), date=date_str))
        elif date_str:
...
class PictureBox(NoteBox):
    ...
    def set_content(self, basename):
        ...
        if basename is not None:
            im = gtk.Image()
            try:
                thumbname = os.path.join(prefs.prefs[prefs.picture_root_pref], 'thumbs', basename)
                ...
                if os.path.isfile(thumbname):
                    ...
                    pixbuf = fullbuf.scale_simple(x, y, gtk.gdk.INTERP_BILINEAR)
    ...
    def on_activate_browse_button(self, widget, data=None):
        fileChooserDialog = gtk.FileChooserDialog(_('Choose a file...'), None, buttons=(
        ...
        try:
            ...
            if filename:
                ...
                full_dest_path = os.path.join(prefs.prefs[prefs.picture_root_pref], 'thumbs', basename)
                try:
                    ...
                    logger.warning('unexpected exception making thumbnail: (%s)%s' % (
                     type(e), e))
            ...
            logger.warning('unhandled exception in editor.py: (%s)%s' % (
             type(e), e))
...
class NotesPresenter(GenericEditorPresenter):
    ...
    def __init__(self, presenter, notes_property, parent_container):
        ...
        self.note_cls = object_mapper(presenter.model).get_property(notes_property).mapper.class_
        ...
        self.box.get_children()[0].set_expanded(True)
...
class PicturesPresenter(NotesPresenter):
    ...
    def __init__(self, presenter, notes_property, parent_container):
        super(PicturesPresenter, self).__init__(presenter, notes_property, parent_container)
        self.box.get_children()[0].set_expanded(False)