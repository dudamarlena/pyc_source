...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/ofxclient/config.py
...
try:
    import keyring
    keyring.get_password(b'is-backend', b'configured?')
...
try:
    DEFAULT_CONFIG = os.path.expanduser(os.path.join(b'~', b'ofxclient.ini'))
except:
    DEFAULT_CONFIG = None
...
class SecurableConfigParser(ConfigParser):
    ...
    Example::
      ...
      with open('config.ini','wb') as fp:
        c.write(fp)
    """
    _secure_placeholder = b'%{secured}'

    def __init__(self, keyring_name=b'ofxclient', keyring_available=KEYRING_AVAILABLE, **kwargs):
    ...
    def set_secure(self, section, option, value):
        ...
        if self.keyring_available:
            s_option = b'%s%s' % (section, option)
            self._unsaved[s_option] = (b'set', value)
    ...
    def get(self, section, option, *args):
        ...
        if self.is_secure_option(section, option) and self.keyring_available:
            s_option = b'%s%s' % (section, option)
            if self._unsaved.get(s_option, [b''])[0] == b'set':
...
class OfxConfig(object):
    ...
    def __init__(self, file_name=None):
        self.secured_field_names = [
         b'institution.username',
         b'institution.password']
        f = file_name or DEFAULT_CONFIG
        if f is None:
            raise ValueError(b'file_name is required')
        self._load(f)
        return
    ...
    def add_account(self, account):
        ...
        section_id = section_items[b'local_id']
    ...
    def save(self):
        ...
        with open(self.file_name, b'w') as (fp):
    ...
    def _load(self, file_name=None):
        ...
        if not os.path.exists(file_name):
            with open(file_name, b'a'):
                os.utime(file_name, None)
        self.file_name = file_name
        conf = SecurableConfigParser()
        with open(self.file_name) as (f):
            if hasattr(conf, b'read_file'):
                conf.read_file(f)
            else:
                conf.readfp(f)
        self.parser = conf
...
def unflatten_dict(dict, prefix=None, separator=b'.'):
    ret = {}
...
def flatten_dict(dict_, prefix=None, separator=b'.'):
    ret = {}
