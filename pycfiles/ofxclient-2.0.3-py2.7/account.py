...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/ofxclient/account.py
...
class Account(object):
    ...
    def long_description(self):
        ...
        :rtype: string
        """
        return b'%s: %s' % (self.institution.description, self.description)
    ...
    def download(self, days=60):
        ...
        as_of = time.strftime(b'%Y%m%d', days_ago.timetuple())
    ...
    def serialize(self):
        ...
        """
        data = {b'local_id': self.local_id(), 
           b'institution': self.institution.serialize(), 
           b'number': self.number, 
           b'description': self.description}
        if hasattr(self, b'broker_id'):
            data[b'broker_id'] = self.broker_id
        elif hasattr(self, b'routing_number'):
            data[b'routing_number'] = self.routing_number
            data[b'account_type'] = self.account_type
        return data
    ...
    def deserialize(raw):
        ...
        institution = Institution.deserialize(raw[b'institution'])
        del raw[b'institution']
        del raw[b'local_id']
        if b'broker_id' in raw:
            a = BrokerageAccount(institution=institution, **raw)
        elif b'routing_number' in raw:
    ...
    def from_ofxparse(data, institution):
        ...
        :type institution: :py:class:`ofxclient.Institution` object
        """
        description = data.desc if hasattr(data, b'desc') else None
