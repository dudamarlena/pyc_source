...
# Embedded file name: build/bdist.linux-x86_64/egg/chisubmit/repos/local.py
...
class LocalGitRepo(object):
    ...
    def reset_branch(self, remote_name, branch):
        ...
        branch_head = self._LocalGitRepo__get_head(branch_refpath)
        remote_branch = self._LocalGitRepo__get_ref(remote_branch_refpath)
    ...
    def has_branch(self, branch):
        return self._LocalGitRepo__get_branch(branch) is not None

    def has_remote_branch(self, remote_name, branch):
        return self._LocalGitRepo__get_ref('refs/remotes/%s/%s' % (remote_name, branch)) is not None
    ...
    def get_commit(self, commit_sha):
        try:
            ...
            commit_obj = self._LocalGitRepo__create_commit_object(commit)
            return commit_obj
    ...
    def get_tag(self, tag):
        ...
        if len(tags) == 0:
            ...
            return self._LocalGitRepo__create_tag_object(tags[0])
    ...
    def __create_tag_object(self, tag):
        return GitTag(name=tag.name, commit=self._LocalGitRepo__create_commit_object(tag.commit))
    ...
    def __create_commit_object(self, commit):
        ...
        return GitCommit(commit.hexsha, commit.message, commit.author.name, commit.author.email, authored_date, commit.committer.name, commit.committer.email, committed_date)
