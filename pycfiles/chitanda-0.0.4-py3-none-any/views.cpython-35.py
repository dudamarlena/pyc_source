...
# Embedded file name: build/bdist.linux-x86_64/egg/chisubmit/backend/api/views.py
# Compiled at: 2018-10-02 19:35:34
# Size of source mod 2**32: 51921 bytes
from builtins import str
from django.http import Http404
from rest_framework.views import APIView
from rest_framework.response import Response
from rest_framework import status
from chisubmit.backend.api.models import Course, Student, Instructor, Grader, Assignment, Team, RubricComponent, get_user_by_username, TeamMember, Registration, Submission, Grade, CourseRoles, SubmissionValidationException
from chisubmit.backend.api.serializers import CourseSerializer, StudentSerializer, InstructorSerializer, GraderSerializer, AssignmentSerializer, TeamSerializer, UserSerializer, RubricComponentSerializer, RegistrationRequestSerializer, RegistrationSerializer, TeamMemberSerializer, RegistrationResponseSerializer, SubmissionSerializer, SubmissionRequestSerializer, SubmissionResponseSerializer, GradeSerializer
...
from chisubmit.backend.api.helpers import get_course_person, get_assignment, get_team, get_course, get_rubric_component, get_team_member, get_registration, get_submission, get_grade

class CourseList(APIView):

    def get(self, request, format=None):
        include_archived = request.query_params.get('include_archived', 'false') in ('true',
                                                                                     'True')
        if include_archived:
...
class CourseDetail(APIView):
    ...
    def delete(self, request, course_id, format=None):
        course_obj, roles = get_course(request, course_id)
        if CourseRoles.ADMIN not in roles:
            raise PermissionDenied
        course_obj.delete()
    ...
    def post(self, request, course_id, assignment_id, format=None):
        ...
        if len(teams) > 0:
                ...
                if len(team_student_usernames) == len(students_usernames):
                    ...
                    if match:
                        if perfect_match is None:
                            ...
                            error_msg = 'There is more than one team with the exact same students in it.Please notify your instructor.'
                            return Response({'fatal': [msg]}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
                    elif t.is_registered_for_assignment(assignment_obj):
            ...
            if perfect_match is not None:
                ...
                if perfect_match.is_registered_for_assignment(assignment_obj):
                    ...
                    registration = Registration.objects.create(team=perfect_match, assignment=assignment_obj)
        ...
        if create_team:
            ...
            if course_obj.extension_policy == Course.EXT_PER_TEAM:
                ...
                extensions = 0
            team = Team.objects.create(course=course_obj, team_id=team_id, extensions=extensions)
            for student_obj in student_objs:
                if student_obj.user == request.user or not is_student:
                    confirmed = True
                else:
                    confirmed = False
                team_member = TeamMember.objects.create(student=student_obj, team=team, confirmed=confirmed)

            registration = Registration.objects.create(team=team, assignment=assignment_obj)
            create_registration = True
    ...
    def post(self, request, course_id, team_id, assignment_id, format=None):
        ...
        if not (CourseRoles.ADMIN in roles or CourseRoles.INSTRUCTOR in roles):
            ...
            if extensions_override is None and registration_obj.grading_started:
                ...
                return Response({'errors': [msg]}, status=status.HTTP_400_BAD_REQUEST)
                try:
                    submission, extensions = Submission.create(registration=registration_obj, commit_sha=commit_sha, submitted_at=now, submitted_by=request.user, extensions_override=extensions_override)
