...
# Embedded file name: build/bdist.linux-x86_64/egg/chisubmit/tests/common.py
# Compiled at: 2020-01-15 11:47:40
# Size of source mod 2**32: 18100 bytes
from __future__ import print_function
from builtins import zip
from builtins import object
import os, yaml, sys, colorama, git, functools, re
...
class ChisubmitCLITestClient(object):
    ...
    def __init__(self, api_url, user_id, api_key, runner, course=None, git_credentials={}, gradescope_api_key=None, verbose=False):
...
class ChisubmitCLITestCase(APILiveServerTestCase):
    ...
    def create_clients(self, runner, admin_id, instructor_ids=[], grader_ids=[], student_ids=[], course_id=None, gradescope_api_key=None, verbose=False):
        ...
        return (
         admin, instructors, graders, students)
    ...
    def create_team_repos(self, admin, course_id, teams, students_team):
        ...
        for team, students in zip(teams, students_team):
            ...
            self.assertEqual(result.exit_code, 0)
            r = re.findall('^Repository URL: (.*)$', result.output, flags=re.MULTILINE)
        ...
        return (
         team_git_paths, team_git_repos, team_commits)
    ...
    def register_team(self, student_clients, team_name, assignment_id, course_id):
        ...
        self.assertListEqual(sorted([s.id for s in students_in_team]), sorted([s.id for s in t.students]))
