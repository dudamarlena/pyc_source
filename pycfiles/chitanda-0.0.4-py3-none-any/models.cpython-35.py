...
# Embedded file name: build/bdist.linux-x86_64/egg/chisubmit/backend/api/models.py
# Compiled at: 2020-01-15 11:47:40
# Size of source mod 2**32: 17535 bytes
...
from chisubmit.common.utils import compute_extensions_needed, is_submission_ready_for_grading
...
class Course(models.Model):
    ...
    def __unicode__(self):
        return '%s: %s' % (self.course_id, self.name)
...
class Instructor(models.Model):
    ...
    def __unicode__(self):
        return 'Instructor %s of %s' % (self.user.username, self.course.course_id)
...
class Grader(models.Model):
    ...
    def __unicode__(self):
        return 'Grader %s of %s' % (self.user.username, self.course.course_id)
...
class Student(models.Model):
    ...
    def __unicode__(self):
        return 'Student %s of %s' % (self.user.username, self.course.course_id)
...
class Assignment(models.Model):
    ...
    def __unicode__(self):
        return 'Assignment %s of %s' % (self.assignment_id, self.course.course_id)
...
class Team(models.Model):
    ...
    def __unicode__(self):
        return 'Team %s in %s' % (self.team_id, self.course.course_id)
    ...
    def get_extensions_available(self):
        ...
        if self.course.extension_policy == Course.EXT_PER_STUDENT:
...
class Registration(models.Model):
    ...
    def is_ready_for_grading(self):
        if self.final_submission is None:
            ...
            return is_submission_ready_for_grading(assignment_deadline=self.assignment.deadline, submission_date=self.final_submission.submitted_at, extensions_used=self.final_submission.extensions_used, assignment_grace_period=self.assignment.grace_period)
...
class Submission(models.Model):
    ...
    def create(cls, registration, commit_sha, submitted_at, submitted_by, extensions_override):
        ...
        extensions_needed = compute_extensions_needed(submission_time=submitted_at, deadline=effective_deadline)
        extensions_needed_without_grace_period = compute_extensions_needed(submission_time=submitted_at, deadline=deadline)
        ...
        if extensions_available < 0:
            ...
            return (
             False, Response({'fatal': [msg]}, status=status.HTTP_500_INTERNAL_SERVER_ERROR), None)
        if registration.final_submission is not None:
        ...
        if extensions_override is not None and extensions_available + extensions_used_in_existing_submission - extensions_override < 0:
            ...
            submission = cls(registration=registration, extensions_used=extensions_used, commit_sha=commit_sha, submitted_at=submitted_at, submitted_by=submitted_by, in_grace_period=in_grace_period)
            return (
