...
# Embedded file name: build/bdist.linux-x86_64/egg/chisubmit/repos/github.py
# Compiled at: 2017-04-13 14:51:46
# Size of source mod 2**32: 16144 bytes
...
class GitHubConnection(RemoteRepositoryConnectionBase):
    ...
    def init_course(self, course, fail_if_exists=True):
        instructors_ghteam = self._GitHubConnection__create_ghteam(self._GitHubConnection__get_instructors_ghteam_name(course), [], 'admin', fail_if_exists=fail_if_exists)
        graders_ghteam = self._GitHubConnection__create_ghteam(self._GitHubConnection__get_graders_ghteam_name(course), [], 'push', fail_if_exists=fail_if_exists)

    def deinit_course(self, course):
        instructors_ghteam = self._GitHubConnection__get_ghteam_by_name(self._GitHubConnection__get_instructors_ghteam_name(course))
        graders_ghteam = self._GitHubConnection__get_ghteam_by_name(self._GitHubConnection__get_graders_ghteam_name(course))
    ...
    def update_instructors(self, course):
        ...
        for instructor in course.get_instructors():
            self._GitHubConnection__add_user_to_ghteam(self._get_user_git_username(instructor), instructors_ghteam)

    def update_graders(self, course):
        graders_ghteam = self._GitHubConnection__get_ghteam_by_name(self._GitHubConnection__get_graders_ghteam_name(course))
        for grader in course.get_graders():
            self._GitHubConnection__add_user_to_ghteam(self._get_user_git_username(grader), graders_ghteam)

    def create_team_repository(self, course, team, fail_if_exists=True, private=True):
        repo_name = self._GitHubConnection__get_team_ghrepo_name(course, team)
        ...
        github_instructors = self._GitHubConnection__get_ghteam_by_name(self._GitHubConnection__get_instructors_ghteam_name(course))
        github_graders = self._GitHubConnection__get_ghteam_by_name(self._GitHubConnection__get_graders_ghteam_name(course))
        ...
        if not self.staging:
            ...
            for s in students:
                ...
                github_student = self._GitHubConnection__get_user(username)
        ...
        github_repo = self._GitHubConnection__get_repository(repo_name)
        ...
        if not self.staging:
            team_name = self._GitHubConnection__get_team_ghteam_name(course, team)
            github_team = self._GitHubConnection__create_ghteam(team_name, [github_repo], 'push', fail_if_exists)
            for github_student in github_students:
                try:
                    self._GitHubConnection__pygithub_add_membership(github_team, github_student)
                except GithubException as ge:
    ...
    def update_team_repository(self, course, team):
        github_team = self._GitHubConnection__get_ghteam_by_name(self._GitHubConnection__get_team_ghteam_name(course, team))
        students = [s for s in course.students if s.user.id in [ts.user.id for ts in team.students]]
        for s in students:
            username = self._get_user_git_username(s)
            github_student = self._GitHubConnection__get_user(username)
    ...
    def exists_team_repository(self, course, team):
        r = self._GitHubConnection__get_repository(self._GitHubConnection__get_team_ghrepo_name(course, team))
        return r is not None

    def get_repository_git_url(self, course, team):
        orgname = self.github_organization
        reponame = self._GitHubConnection__get_team_ghrepo_name(course, team)
    ...
    def get_commit(self, course, team, commit_sha):
        try:
            github_repo = self.organization.get_repo(self._GitHubConnection__get_team_ghrepo_name(course, team))
            gh_commit = github_repo.get_commit(commit_sha)
            commit = GitCommit(gh_commit.commit.sha, gh_commit.commit.message, gh_commit.commit.author.name, gh_commit.commit.author.email, gh_commit.commit.author.date, gh_commit.commit.committer.name, gh_commit.commit.committer.email, gh_commit.commit.committer.date)
            return commit
        except GithubException as ge:
            if ge.status == 404:
                return
    ...
    def update_submission_tag(self, course, team, tag_name, tag_message, commit_sha):
        submission_tag_ref = self._GitHubConnection__get_submission_tag_ref(course, team, tag_name)
    ...
    def get_submission_tag(self, course, team, tag_name):
        ...
        tag = GitTag(name=gh_tag.tag, commit=self.get_commit(course, team, gh_tag.object.sha))
        return tag

    def delete_team_repository(self, course, team, fail_if_not_exists=True):
        ghrepo_name = self._GitHubConnection__get_team_ghrepo_name(course, team)
        github_repo = self._GitHubConnection__get_repository(ghrepo_name)
        if github_repo is None:
        ...
        github_team_name = self._GitHubConnection__get_team_ghteam_name(course, team)
        github_team = self._GitHubConnection__get_ghteam_by_name(github_team_name)
        try:
            github_team.delete()
        except GithubException as ge:
    ...
    def __create_ghteam(self, team_name, repos, permissions, fail_if_exists=True):
        github_team = self._GitHubConnection__get_ghteam_by_name(team_name)
    ...
    def __add_user_to_ghteam(self, github_id, ghteam):
        github_user = self._GitHubConnection__get_user(github_id)
        if github_user is None:
            raise ChisubmitException("GitHub user '%s' does not exist " % github_id)
        try:
            self._GitHubConnection__pygithub_add_membership(ghteam, github_user)
        except GithubException as ge:
    ...
    def __pygithub_add_membership(self, team, member):
        ...
        headers, data = team._requester.requestJsonAndCheck('PUT', team.url + '/memberships/' + member._identity)

    def __add_user_to_ghteam_by_name(self, github_id, ghteam_name):
        ghteam = self._GitHubConnection__get_ghteam_by_name(ghteam_name)
        if ghteam is None:
            raise ChisubmitException("GitHub team '%s' does not exist " % ghteam_name)
        self._GitHubConnection__add_user_to_ghteam(github_id, ghteam)
