...
# Embedded file name: build/bdist.linux-x86_64/egg/chisubmit/client/types.py
# Compiled at: 2020-01-15 11:47:40
# Size of source mod 2**32: 11781 bytes
...
class ChisubmitAPIException(Exception):
    ...
    def __init__(self, status, data):
        ...
        self._ChisubmitAPIException__status = status
        self._ChisubmitAPIException__data = data

    @property
    def status(self):
        return self._ChisubmitAPIException__status

    @property
    def data(self):
        return self._ChisubmitAPIException__data
...
class AttributeType(object):
    ...
    DICT = 9
    primitive_types = [
     STRING, INTEGER, DECIMAL, DATETIME, TIMEDELTA, BOOLEAN]
    composite_types = [OBJECT, LIST, DICT]

    def __init__(self, attrtype, subtype=None):
        if not (attrtype in self.primitive_types and subtype is None):
            assert attrtype in self.composite_types and subtype is not None
            if not subtype is None:
                if not (attrtype == self.LIST and isinstance(subtype, AttributeType)):
                    if not (attrtype == self.DICT and isinstance(subtype, AttributeType)):
                        if not (attrtype == self.OBJECT and isinstance(subtype, string_types)):
                            assert attrtype == self.OBJECT and issubclass(subtype, ChisubmitAPIObject)
    ...
    def to_python(self, value, headers, api_client):
        if self.attrtype == AttributeType.STRING:
            ...
            return value
        if self.attrtype == AttributeType.INTEGER:
            if not isinstance(value, (int, int)):
                raise AttributeTypeException(value, self)
            return value
        if self.attrtype == AttributeType.DECIMAL:
            try:
                return float(value)
            except ValueError:
                raise AttributeTypeException(value, self)
            ...
            if self.attrtype == AttributeType.BOOLEAN:
                if not isinstance(value, bool):
                    raise AttributeTypeException(value, self)
                return value
            if self.attrtype == AttributeType.DATETIME:
                if not isinstance(value, string_types):
                    raise AttributeTypeException(value, self)
                try:
                    if value[19] == '.':
                ...
                return dt
            if self.attrtype == AttributeType.TIMEDELTA:
                if not isinstance(value, string_types):
                    raise AttributeTypeException(value, self)
                try:
                ...
                return dt
            if self.attrtype == AttributeType.LIST:
                ...
                return rvalue
            if self.attrtype == AttributeType.DICT:
                ...
                return rvalue
            if self.attrtype == AttributeType.OBJECT:
                if not isinstance(value, dict):
                    raise AttributeTypeException(value, self)
...
class ChisubmitAPIObject(object):
    ...
    def _updateAttributes(self, attributes):
        for attrname, attrvalue in list(attributes.items()):
            if attrname == 'url':
            ...
            elif self._ChisubmitAPIObject__is_relationship_attr(attrname):
                if attrname.endswith('_url'):
                    object.__setattr__(self, attrname, attrvalue)
                else:
                ...
                api_attr = self._ChisubmitAPIObject__get_api_attr(attrname)
                if api_attr is None:
    ...
    def __setattr__(self, name, value):
        if self._ChisubmitAPIObject__is_relationship_attr('_rel_' + name):
            raise AttributeNotEditableException(name, value)
        api_attr = self._ChisubmitAPIObject__get_api_attr(name)
        if api_attr is None:
    ...
    def get_related(self, name, force_request=False, params=None):
        ...
        if not force_request and params is None and hasattr(self, '_rel_' + name):
            cached_rel = getattr(self, '_rel_' + name)
            if cached_rel is not None:
                pass
            return cached_rel
        rel_url = getattr(self, name + '_url')
        headers, data = self._api_client._requester.request('GET', rel_url, params=params)
    ...
    def edit(self, **kwargs):
        ...
        headers, data = self._api_client._requester.request('PATCH', self.url, data=patch_data)
    ...
    def delete(self):
        ...
        :rtype: None
        """
        _ = self._api_client._requester.request('DELETE', self.url)