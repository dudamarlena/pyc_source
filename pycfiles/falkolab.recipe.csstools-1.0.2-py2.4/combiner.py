...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/falkolab/recipe/csstools/combiner.py
# Compiled at: 2009-06-04 07:03:48
from ConfigParser import ConfigParser
import logging, sys, os.path
from string import join
import cssutils, codecs
...
class Combiner(ConfigParser):
    ...
    def assemble(self, cfg, depmap=None):
        ...
        if cfg['include']:
            ...
            cfg['last'] = [ filepath for filepath in cfg['last'] if filepath not in cfg['first'] if filepath not in exclude ]
            cfg['first'] = [ filepath for filepath in cfg['first'] if filepath not in cfg['last'] if filepath not in exclude ]
        first = cfg['first']
        last = cfg['last']
        include = cfg['include'] = [ filepath for filepath in cfg['include'] if filepath not in exclude ]
        allIncludes = first + include + last
        filesList = [ filepath for filepath in getCSSFilesList(sourcedir) if filepath not in exclude ]
        ...
        if include:
            ...
            order = first + [ filepath for filepath in filesList if filepath not in allIncludes ] + last
    ...
    def run(self, minify=False, section=None, sourceencoding=None, targetencoding=None):
        ...
        for section in sections:
            ...
            result = csscombine(files, sourceencoding=sourceencoding, targetencoding=targetencoding, minify=minify, license=cfg['license'], logger=self.printer)
...
def resolveImports(sheet, target=None, baseUrl=None):
    ...
    if sheetBaseUrl.startswith(baseUrl):
        ...
        path = sheet.href
    target.add(cssutils.css.CSSComment(cssText='/* START %s */' % path))
    for rule in sheet.cssRules:
        if rule.type == rule.CHARSET_RULE:
            pass
        elif rule.type == rule.IMPORT_RULE:
            cssutils.log.info('Processing @import %r' % rule.href, neverraise=True)
            if rule.styleSheet:
                resolveImports(rule.styleSheet, target, baseUrl)
            else:
                cssutils.log.error('Cannot get referenced stylesheet %r' % rule.href, neverraise=True)
                target.add(rule)
        else:
            target.add(rule)

    target.add(cssutils.css.CSSComment(cssText='/* END %s */' % path))
    return target


def csscombine(files, sourceencoding=None, targetencoding=None, minify=True, license=None, logger=logger.info):
    ...
    for path in files:
        ...
        logger('Using target encoding: %r' % targetencoding)
        if minify:
            oldser = cssutils.ser
