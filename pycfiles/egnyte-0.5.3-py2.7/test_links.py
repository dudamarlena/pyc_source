...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/egnyte/tests/test_links.py
# Compiled at: 2017-05-23 09:50:27
...
class TestLinks(EgnyteTestCase):
    ...
    def test_several_links(self):
        links = self.folder.link(ACCESSIBILITY_ANYONE, recipients=[
         RECIPIENT_EMAIL_1, RECIPIENT_EMAIL_2], send_email=False)
        ...
        links[0].delete()
        self.assertIsNone(links[1].check())
        self.assertRaises(exc.NotFound, links[0].check)
    ...
    def test_list_links(self):
        ...
        self.assertEqual(tuple(all_links), tuple(past), 'List of links created before tomorrow should include all links')
