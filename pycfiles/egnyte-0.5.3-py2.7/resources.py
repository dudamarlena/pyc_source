...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/egnyte/resources.py
# Compiled at: 2017-03-01 11:03:54
...
class FileOrFolder(base.Resource):
    ...
    _url_template = b'pubapi/v1/fs%(path)s'
    _lazy_attributes = {b'name', b'folder_id', b'is_folder'}
    ...
    def copy(self, destination):
        ...
        return self._action(b'copy', destination)

    def move(self, destination):
        """Move this to another path. Destination path should have all segments (including the last one)."""
        return self._action(b'move', destination)

    def link(self, accessibility, recipients=None, send_email=None, message=None, copy_me=None, notify=None, link_to_current=None, expiry_date=None, expiry_clicks=None, add_filename=None):
        """
        Create a link.
        ...
        * add_filename: If True then the filename will be appended to the end of the link. Only applies to file links, not folder links.
        ...
        """
        return Links(self._client).create(path=self.path, type=self._link_kind, accessibility=accessibility, recipients=recipients, send_email=send_email, message=message, copy_me=copy_me, notify=notify, link_to_current=link_to_current, expiry_date=expiry_date, expiry_clicks=expiry_clicks, add_filename=add_filename)
    ...
    def _get(self):
        ...
        if json[b'is_folder'] and not isinstance(self, Folder):
            instance = Folder(self._client, path=self.path)
        elif not json[b'is_folder'] and not isinstance(self, File):
            instance = File(self._client, path=self.path)
        else:
            instance = self
        instance._update_attributes(json)
        if instance.is_folder:
            instance.folders = [ Folder(self._client, **folder_data) for folder_data in json.get(b'folders', ()) ]
            instance.files = [ File(self._client, **file_data) for file_data in json.get(b'files', ()) ]
        return instance
...
class File(FileOrFolder):
    ...
    path - file path
    """
    _upload_chunk_size = 100 * 1048576
    _upload_retries = 3
    _link_kind = b'file'
    _lazy_attributes = {b'num_versions', b'name', b'checksum', b'last_modified', b'entry_id',
     b'uploaded_by', b'size', b'is_folder'}
    _url_template_content = b'pubapi/v1/fs-content%(path)s'
    _url_template_content_chunked = b'pubapi/v1/fs-content-chunked%(path)s'
    ...
    def upload(self, fp, size=None, progress_callback=None):
        ...
        if size < self._upload_chunk_size:
            ...
            while retries > 0:
                ...
                r = self._client.POST(url, data=chunk, headers={b'Content-length': str(size)})
                exc.default.check_response(r)
                server_sha = r.headers[b'X-Sha512-Checksum']
            ...
            if retries == 0:
                raise exc.ChecksumError(b'Failed to upload file', {})
        else:
    ...
    def download(self, download_range=None):
        ...
        if download_range is None:
            ...
            if len(download_range) != 2:
                raise exc.InvalidParameters(b'Download range needs to be None or a 2 element integer sequence')
            r = exc.partial.check_response(self._client.GET(url, stream=True, headers={b'Range': b'bytes=%d-%d' % download_range}))
    ...
    def _chunked_upload(self, fp, size, progress_callback):
        ...
        chunks = list(base.split_file_into_chunks(fp, size, self._upload_chunk_size))
        chunk_count = len(chunks)
        headers = {}
        for chunk_number, chunk in enumerate(chunks, 1):
            headers[b'x-egnyte-chunk-num'] = b'%d' % chunk_number
            headers[b'content-length'] = str(chunk.size)
            if chunk_number == chunk_count:
                headers[b'x-egnyte-last-chunk'] = b'true'
            retries = max(self._upload_retries, 1)
            while retries > 0:
                r = self._client.POST(url, data=chunk, headers=headers)
                server_sha = r.headers[b'x-egnyte-chunk-sha512-checksum']
                our_sha = chunk.sha.hexdigest()
                if server_sha == our_sha:
                    break
                retries -= 1
            ...
            if retries == 0:
                raise exc.ChecksumError(b'Failed to upload file chunk', {b'chunk_number': chunk_number, b'start_position': chunk.position})
            exc.default.check_response(r)
            if chunk_number == 1:
                headers[b'x-egnyte-upload-id'] = r.headers[b'x-egnyte-upload-id']
...
class Folder(FileOrFolder):
    ...
    """
    _url_template = b'pubapi/v1/fs%(path)s'
    _url_template_permissions = b'pubapi/v1/perms/folder/%(path)s'
    _url_template_effective_permissions = b'pubapi/v1/perms/user/%(username)s'
    _lazy_attributes = {b'name', b'folder_id', b'is_folder'}
    _link_kind = b'folder'
    folders = None
    files = None
    ...
    def folder(self, path, **kwargs):
        ...
        return Folder(self._client, path=(self.path + b'/' + path), **kwargs)

    def file(self, filename, **kwargs):
        """Return a file in this folder."""
        return File(self._client, folder=self, filename=filename, path=(self.path + b'/' + filename), **kwargs)
    ...
    def create(self, ignore_if_exists=True):
        ...
        """
        r = self._client.POST(self._url, dict(action=b'add_folder'))
    ...
    def get_permissions(self, users=None, groups=None):
        ...
        if users is not None:
            query_params[b'users'] = (b'|').join(six.text_type(x) for x in users)
        if groups is not None:
            query_params[b'groups'] = (b'|').join(six.text_type(x) for x in users)
...
class Link(base.Resource):
    ...
    _url_template = b'pubapi/v1/links/%(id)s'
    _lazy_attributes = {b'copy_me', b'link_to_current', b'accessibility', b'notify',
     b'path', b'creation_date', b'type', b'send_mail'}
...
class User(base.Resource):
    ...
    """
    _url_template = b'pubapi/v2/users/%(id)s'
    _url_template_effective_permissions = b'pubabi/v1/perms/user/%(userName)s'
    _lazy_attributes = {b'userName', b'externalId', b'email', b'name', b'active', b'locked', b'authType',
     b'role', b'userType', b'idpUserId'}

    def delete(self):
        """Delete this user account."""
        base.Resource.delete(self)

    def update(self, email=None, familyName=None, givenName=None, active=None, sendInvite=None, authType=None, userType=None, idpUserId=None, userPrincipalName=None):
        """
        ...
        * userPrincipalName: Do not specify if user is not AD authenticated. Used to bind child authentication policies to a user when using Active Directory authentication in a multi-domain setup (e.g. jmiller@example.com)
        ...
        data = base.filter_none_values(dict(email=email, active=active, name=name, sendInvite=sendInvite, authType=authType, userType=userType, idpUserId=idpUserId, userPrincipalName=userPrincipalName))
...
class Note(base.Resource):
    ...
    _url_template = b'pubapi/v1/notes/%(id)s'
    _lazy_attributes = {b'file_id', b'file_path', b'can_delete', b'creation_time', b'message', b'username', b'formatted_name'}
...
class Group(base.Resource):
    """Group of users."""
    _url_template = b'pubapi/v2/groups/%(id)s'
    _lazy_attributes = {b'displayName', b'members'}
    ...
    def full_update(self, displayName, members=None):
        ...
        * members: List of members to be added to the new group (user ids or User objects). Optional.
        ...
        if members is not None:
            data[b'members'] = [ dict(value=x.id if isinstance(x, User) else x) for x in members ]
        json = exc.default.check_json_response(self._client.PATCH(url, data))
        self._update_attributes(json)
        return
...
class Links(base.HasClient):
    """Link management API"""
    _url_template = b'pubapi/v1/links'

    def create(self, path, type, accessibility, recipients=None, send_email=None, message=None, copy_me=None, notify=None, link_to_current=None, expiry_date=None, expiry_clicks=None, add_filename=None):
        """
        Create links.
        ...
        * add_filename: If True then the filename will be appended to the end of the link. Only applies to file links, not folder links.
        ...
        data = base.filter_none_values(dict(path=path, type=type, accessibility=accessibility, send_email=send_email, copy_me=copy_me, notify=notify, add_filename=add_filename, link_to_current=link_to_current, expiry_clicks=expiry_clicks, expiry_date=base.date_format(expiry_date), recipients=recipients, message=message))
        response = exc.default.check_json_response(self._client.POST(url, data))
        links = response.pop(b'links')
        result = []
        for l in links:
            l.update(response)
    ...
    def list(self, path=None, username=None, created_before=None, created_after=None, type=None, accessibility=None, offset=None, count=None):
        """
        ...
        * count: Send this number of links. If not specified, all links will be sent.
        ...
        """
        url = self._client.get_url(self._url_template)
        params = base.filter_none_values(dict(path=path, username=username, created_before=base.date_format(created_before), created_after=base.date_format(created_after), type=type, accessibility=accessibility, offset=offset, count=count))
        json = exc.default.check_json_response(self._client.GET(url, params=params))
        return base.ResultList((Link(self._client, id=id) for id in json.get(b'ids', ())), json[b'total_count'], json[b'offset'])


class Users(base.HasClient):
    """User management API"""
    _url_template = b'pubapi/v2/users'
    ...
    def list(self, email=None, externalId=None, userName=None, startIndex=None, count=None):
        ...
        params[b'filter'] = [ b'%s eq "%s"' % (k, v) for k, v in filters.items() ]
        json = exc.default.check_json_response(self._client.GET(url, params=params))
        return base.ResultList((User(self._client, **d) for d in json.get(b'resources', ())), json[b'totalResults'], json[b'startIndex'] - 1)
    ...
    def create(self, userName, externalId, email, familyName, givenName, active=True, sendInvite=True, authType=b'egnyte', userType=b'power', role=None, idpUserId=None, userPrincipalName=None):
        """
        ...
        * userPrincipalName: Do not specify if user is not AD authenticated. Used to bind child authentication policies to a user when using Active Directory authentication in a multi-domain setup (e.g. jmiller@example.com)
        ...
        """
        url = self._client.get_url(self._url_template)
        data = base.filter_none_values(dict(userName=userName, externalId=externalId, email=email, name=dict(familyName=familyName, givenName=givenName), active=active, sendInvite=sendInvite, authType=authType, userType=userType, role=role, idpUserId=idpUserId, userPrincipalName=userPrincipalName))
...
class PermissionSet(object):
    ...
    def __init__(self, json):
        self._users = json.get(b'users', ())
        self._groups = json.get(b'groups', ())
        self._unpack()
    ...
    def _unpack(self):
        ...
        for d in self._users:
            self.user_to_permission[d[b'subject']] = d[b'permission']
            self.permission_to_owner[d[b'permission']][b'users'].add(d[b'subject'])

        for d in self._groups:
            self.group_to_permission[d[b'subject']] = d[b'permission']
            self.permission_to_owner[d[b'permission']][b'groups'].add(d[b'subject'])
...
class Notes(base.HasClient):
    ...
    """
    _url_template = b'pubapi/v1/notes'
    ...
    def list(self, file=None, folder=None, start_time=None, end_time=None):
        ...
        * end_time: Get notes created before end_time (datetime.date or string in 'YYYY-MM-DD' format)
        ...
        """
        url = self._client.get_url(self._url_template)
        params = base.filter_none_values(dict(file=file, folder=folder, start_time=base.date_format(start_time), end_time=base.date_format(end_time)))
        json = exc.default.check_json_response(self._client.GET(url, params=params))
        return base.ResultList((Note(self._client, **d) for d in json.pop(b'notes', ())), json[b'total_results'], json[b'offset'])
...
class Groups(base.HasClient):
    ...
    """
    _url_template = b'pubapi/v2/groups'
        ...
        * displayName: Filter by name of the group. This may contain '*' wildcards at beginning for prefix search or both at beginning and end for contains search.
        ...
        if displayName:
            if displayName.startswith(b'*'):
                op = b'co' if displayName.endswith(b'*') else b'sw'
            else:
                op = b'eq'
            params[b'filter'] = [
             b'displayName %s "%s"' % (op, displayName.strip(b'*'))]
        url = self._client.get_url(self._url_template)
        json = exc.default.check_json_response(self._client.GET(url, params=params))
        return base.ResultList((Group(self._client, **d) for d in json.pop(b'resources', ())), json[b'totalResults'], json[b'startIndex'] - 1)
    ...
    * is_folder A boolean value stating if the object is a file or folder. Please note that, currently, this API only returns file objects.
    ...
    def file(self):
        ...
        if not self.is_folder:
            return File(self._client, name=self.name, path=self.path, is_folder=self.is_folder, num_versions=self.num_versions, entry_id=self.entry_id, uploaded_by=self.uploaded_by, size=self.size, last_modified=self.last_modified)


class Search(base.HasClient):
    """Search API"""
    _url_template = b'pubapi/v1/search'
    ...
    def files(self, query, offset=None, count=None, folder=None, modified_after=None, modified_before=None):
        ...
        * modified_after Limit to results after the specified ISO-8601 timestamp (datetime.date object or string).
        ...
        """
        url = self._client.get_url(self._url_template)
        params = base.filter_none_values(dict(query=query, offset=offset, count=count, folder=folder, modified_after=base.date_format(modified_after), modified_before=base.date_format(modified_before)))
        json = exc.default.check_json_response(self._client.GET(url, params=params))
        return base.ResultList((SearchMatch(self._client, **d) for d in json.get(b'results', ())), json[b'total_count'], json[b'offset'])