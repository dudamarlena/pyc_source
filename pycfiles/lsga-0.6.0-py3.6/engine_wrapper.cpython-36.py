...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/lsga/engine_wrapper.py
# Compiled at: 2019-02-10 16:37:44
...
class EngineWrapper:
    ...
    def create_engine(self, ranges, n_jobs=1, eps=0.001, population_size=50, encoding='binary', selection='tournament', crossover='uniform', mutation='flip_bit'):
        """
        ...
        """
        if not encoding in ('binary', 'decimal'):
            raise AssertionError
        else:
            if not selection in ('tournament', 'roulette_wheel', 'linear_ranking',
                                 'exponential_ranking'):
                raise AssertionError
            else:
                if not crossover in ('uniform', ):
                    raise AssertionError
                elif not mutation in ('flip_bit', ):
                    raise AssertionError
                else:
                    if encoding == 'binary':
        ...
        engine = GAEngine(population=population, selection=lsga_selection, crossover=lsga_crossover,
          mutation=lsga_mutation,
          analysis=[
         ConsoleOutput, FitnessStore])
        return engine
