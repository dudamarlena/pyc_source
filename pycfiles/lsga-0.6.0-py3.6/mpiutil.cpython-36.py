...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/lsga/mpiutil.py
...
class MPIUtil(object):
    ...
    def split_size(self, size):
        ...
        """
        if size < self.size:
            warn_msg = 'Splitting size({}) is smaller than process number({}), more processor would be superflous'.format(size, self.size)
