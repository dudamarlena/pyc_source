...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/multipath_v4_ll.py
...
class MultipathDevices(CommandParser):
    ...
    def parse_content(self, content):
        ...
        path_group = []
        path_group_attr = {}
        MPATH_WWID_REG = re.compile('\\(?([A-Za-z0-9_\\s]+)\\)?\\s+dm-')
        PROPERTY_SQBRKT_REG = re.compile('\\[(?P<key>\\w+)=(?P<value>[^\\]]+)\\]')
        PATHGROUP_POLICY_STR = "(?:policy=')?(?P<policy>(?:round-robin|queue-length|service-time) \\d)" + "(?:' | \\[)prio=(?P<priority>\\d+)(?:\\]\\[| status=)" + '(?P<status>\\w+)(?:\\]|)'
        PATHGROUP_POLICY_REG = re.compile(PATHGROUP_POLICY_STR)
        HCTL_REG = re.compile('(?:[`|]-(?:\\+-)?|\\\\_) (\\d+:){3}\\d+')
        for line in content:
            ...
            if m:
                ...
                dm, venprod = re.findall('.*(dm-\\S+)\\s+(.*)', line)[0]
                if not no_alias:
                    dm, venprod = re.findall('\\w+\\s+\\(.*\\)\\s+(dm-\\S+)\\s+(.*)', line)[0]
