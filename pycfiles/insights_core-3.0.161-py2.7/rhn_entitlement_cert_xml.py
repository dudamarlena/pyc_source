...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/rhn_entitlement_cert_xml.py
...
class RHNCertConf(XMLParser):
    ...
    def parse_dom(self):
        ...
        for field in self.dom.findall('.//rhn-cert-field'):
            ...
            if family:
                channel_familes[family] = dict((k, v) for k, v in field.items() if k not in ('name',
                                                                                             'family'))
            elif field.text:
