...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/nfs_exports.py
...
class NFSExportsBase(Parser):
        ...
        for line in get_active_lines(content):
            ...
            if path not in self.data:
                ...
                for host, flags in hosts.items():
                    ...
                    elif path not in self.ignored_exports:
    ...
    def reconstitute(path, d):
        ...
        Returns:
            ...
            those hosts.
        """
        deprecated(NFSExportsBase.reconstitute, 'Please use the `raw_lines` dictionary property of the parser instance')
        return ('  ').join([path] + [ '%s(%s)' % (host, (',').join(options)) for host, options in d.items()
                                    ])
