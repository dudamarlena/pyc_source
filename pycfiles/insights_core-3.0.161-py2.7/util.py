...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/client/util.py
# Compiled at: 2019-05-16 13:41:33
import sys, json, subprocess, collections
from fnmatch import fnmatch as matches
from docker.utils import kwargs_from_env
import docker, selinux
...
def image_by_name(img_name, images=None):
    ...
    for i in images:
        for t in i['RepoTags']:
            ...
            if matches(reg, i_reg) and matches(rep, i_rep) and matches(tag, i_tag):
...
def print_scan_summary(json_data, names=None):
    ...
    def _max_width(data):
        max_name = 0
        for name in data:
            max_name = len(data[name]) if len(data[name]) > max_name else max_name
    ...
    for image in res_summary.keys():
        ...
        if 'msg' in image_res.keys():
            ...
            tmp_tuple = tuple([image_name] + [ str(image_res[sev]) for sev in sevs ])
            sev_results = [ image_res[sev] for sev in sevs if image_res[sev] > 0
                          ]
...
def print_detail_scan_summary(json_data, names=None):
    ...
    for image in cve_summary.keys():
        ...
        if not image_res['isRHEL']:
            writeOut(image_template.format('Result', 'Not based on Red HatEnterprise Linux'))
            continue
        else:
        ...
        for sev in sevs:
            if sev in scan_results:
                clean = False
                writeOut(image_template.format(sev, str(scan_results[sev]['num'])))
                for cve in scan_results[sev]['cves']:
                    writeOut(cve_template.format('CVE', cve['cve_title']))
                    writeOut(cve_template.format('CVE URL', cve['cve_ref_url']))
                    writeOut(cve_template.format('RHSA ID', cve['rhsa_ref_id']))
                    writeOut(cve_template.format('RHSA URL', cve['rhsa_ref_url']))
                    writeOut('')
...
def get_mounts_by_path():
    ...
    for line in f:
        _tmp = line.split(' ')
        mount_info.append({'path': _tmp[1], 'device': _tmp[0], 
           'type': _tmp[2], 
           'options': _tmp[3]})
