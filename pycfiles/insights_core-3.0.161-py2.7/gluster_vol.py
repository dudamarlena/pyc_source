...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/gluster_vol.py
...
class GlusterVolStatus(LegacyItemAccess, CommandParser):
    ...
    def parse_content(self, content):
        ...
        for i, idx in enumerate(idxs):
            ...
            body = parse_fixed_table(content[start:end], header_substitute=[
             ('Gluster process', 'Gluster_process'),
             ('TCP Port', 'TCP_Port'),
             ('RDMA Port', 'RDMA_Port')], heading_ignore=[
             'Gluster process'], trailing_ignore=[
             'Task Status of Volume', 'There are no active volume tasks'])
