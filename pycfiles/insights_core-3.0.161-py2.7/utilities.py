...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/client/utilities.py
# Compiled at: 2020-04-23 14:49:03
"""
Utility functions
"""
from __future__ import absolute_import
import glob, socket, os, logging, uuid, datetime, shlex, re, sys
...
def write_data_to_file(data, filepath):
    ...
    """
    try:
        os.makedirs(os.path.dirname(filepath), 448)
    except OSError:
        pass
        ...
        for egg in eggs:
            ...
            try:
                proc = Popen([sys.executable, '-c',
                 "from insights import package_info; print('%s-%s' % (package_info['VERSION'], package_info['RELEASE']))"], env={'PYTHONPATH': egg, 'PATH': os.getenv('PATH')}, stdout=PIPE, stderr=STDOUT)
            except OSError:
...
def migrate_tags():
    ...
    if os.path.exists(tags_conf):
        logger.info('Tags file %s detected. This filename is deprecated; please use %s. The file will be renamed automatically.', tags_conf, tags_yaml)
        try:
