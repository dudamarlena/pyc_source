...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/core/plugins.py
...
class parser(PluginType):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(parser, self).__init__(group=group, *args)
...
class Response(dict):
    ...
    def _log_length_error(self, key, length):
        ...
        extra = {'max_detail_length': settings.defaults['max_detail_length'], 
           'len': length}
        if self.key_name:
...
class _make_skip(Response):
    ...
    def __init__(self, rule_fqdn, missing):
        ...
        super(_make_skip, self).__init__(None, rule_fqdn=rule_fqdn, reason='MISSING_REQUIREMENTS', details=details)
        return