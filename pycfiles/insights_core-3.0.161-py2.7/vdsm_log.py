...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/vdsm_log.py
...
class VDSMLog(LogFileOutput):
    r"""Logs from the Virtual Desktop and Server Manager.
    ...
    def parse_lines(self, lines):
        ...
        re_datetime_obj = re.compile('^(\\d{4}-\\d{2}-\\d{2}\\s+[\\d{2}:]+\\d{2},\\d{3})')
        re_name_obj = re.compile('\\[(\\w.+)\\]')
        re_thread_and_module = re.compile('\\((.*?)\\)')
        re_message_obj = re.compile('\\[\\w.+\\](.*?)\\(\\w+\\:\\w+\\)$')
        re_level_obj = re.compile('INFO|ERROR|DEBUG|WARN')
        for line in lines:
            ...
            if re_datetime_obj.match(line):
                ...
                if 'timestamp' in fields:
                    try:
                        fields['asctime'] = datetime.strptime(fields['timestamp'], time_format)
...
class VDSMImportLog(LogFileOutput):
    ...
    """
    _line_re = re.compile('^(?:\\[\\s+(?P<timestamp>\\d+\\.\\d+)\\]\\s+)?(?P<message>.*)$')
    ...
    def get_after(self, timestamp, s=None):
        ...
        if not isinstance(timestamp, float):
            raise TypeError(("get_after needs a float type timestamp, but get '{c}'").format(c=timestamp))
