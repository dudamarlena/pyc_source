...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/examples/cluster_rules/ntp_compare.py
...
repo using the following command line::
    ...
    $ insights-run -p examples.cluster_rules.ntp_compare             -i topology_example             examples/cluster_rules/cluster_hosts.tar.gz
...
SUCCESS_TEMPLATE = ('\nServers:\n{% for n in servers %}\n    {{ n }}\n{% endfor %}\n\nNodes:\n{% for n in nodes %}\n    {{ n }}\n{% endfor %}').strip()
CONTENT = {'DISTINCT_NTP_CONFS': '{{confs}} distinct NTP configurations of {{nodes}} members.', 
