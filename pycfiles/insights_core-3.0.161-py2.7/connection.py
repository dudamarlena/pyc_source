...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/client/connection.py
...
import requests, os, six, json, logging, pkg_resources, platform, xml.etree.ElementTree as ET, warnings
...
from .utilities import determine_hostname, generate_machine_id, write_unregistered_file, write_registered_file
...
class InsightsConnection(object):
    ...
    def __init__(self, config):
        ...
        elif isinstance(self.cert_verify, six.string_types):
    ...
    def _init_session(self):
        ...
        if self.proxy_auth:
            try:
                ...
                session.request('GET', self.base_url, timeout=self.config.http_timeout)
    ...
    def user_agent(self):
        ...
        ua = ('{client_version} ({core_version}; {requests_version}) {os_family} {os_release} ({python_version}; {kernel_version})').format(client_version=client_version, core_version=core_version, python_version=python_version, os_family=os_family, os_release=os_release, kernel_version=kernel_version, requests_version=requests_version)
        return ua
    ...
    def get_proxies(self):
        ...
        if conf_proxy is not None and conf_proxy.lower() != ('None').lower() and conf_proxy != '':
            if '@' in conf_proxy:
                ...
                username = conf_proxy.split('@')[0].split(':')[1].replace('/', '')
        ...
        if no_proxy and conf_proxy:
            logger.debug("You have environment variable NO_PROXY set as well as 'proxy' set in your configuration file. NO_PROXY environment variable will be ignored.")
        if proxies is None:
    ...
    def _legacy_test_urls(self, url, method):
        ...
        for ext in paths:
            try:
                ...
                if method is 'POST':
                    test_req = self.session.post(test_url + ext, timeout=self.config.http_timeout, data=test_flag)
                else:
                    if method is 'GET':
    ...
    def _test_urls(self, url, method):
        ...
        try:
            ...
            if method is 'POST':
                ...
                test_files = {'file': (
                          'test.tar.gz', test_tar, 'application/vnd.redhat.advisor.collection+tgz'), 
                   'metadata': '{"test": "test"}'}
    ...
    def test_connection(self, rc=0):
        ...
        try:
            ...
            logger.error('Additional information may be in /var/log/' + APP_NAME + '/' + APP_NAME + '.log')
            return 1
    ...
    def handle_fail_rcs(self, req):
        ...
        if req.status_code >= 400:
            ...
            if req.status_code == 401:
                ...
                logger.error('Please ensure correct credentials in ' + constants.default_conf_file)
            ...
            if req.status_code == 403 and self.auto_config:
                ...
                if rhsm_hostname != 'subscription.rhn.redhat.com' and rhsm_hostname != 'subscription.rhsm.redhat.com':
                    logger.error('Please enable Insights on Satellite server %s to continue.', rhsm_hostname)
    ...
    def get_branch_info(self):
        ...
        logger.log(NETWORK, 'GET %s', self.branch_info_url)
        response = self.session.get(self.branch_info_url, timeout=self.config.http_timeout)
        logger.log(NETWORK, 'GET branch_info status: %s', response.status_code)
        if response.status_code != 200:
            logger.debug('There was an error obtaining branch information.')
            logger.debug('Bad status from server: %s', response.status_code)
        ...
        if branch_info['remote_branch'] is not -1 and branch_info['remote_leaf'] is -1:
    ...
    def create_system(self, new_machine_id=False):
        ...
        if not branch_info:
            ...
            data = {'machine_id': machine_id, 'remote_branch': remote_branch, 
    ...
    def _legacy_upload_archive(self, data_collected, duration):
        ...
        files = {'file': (
                  file_name, open(data_collected, 'rb'), mime_type)}
        ...
        logger.log(NETWORK, 'Upload status: %s %s %s', upload.status_code, upload.reason, upload.text)
    ...
    def upload_archive(self, data_collected, content_type, duration):
        ...
        if self.config.legacy_upload:
            ...
            files = {'file': (
                      file_name, open(data_collected, 'rb'), content_type), 
               'metadata': c_facts}
            logger.debug('Uploading %s to %s', data_collected, upload_url)
            logger.log(NETWORK, 'POST %s', upload_url)
    ...
    def _legacy_set_display_name(self, display_name):
        ...
        try:
            ...
            res = self.session.put(url, timeout=self.config.http_timeout, headers={'Content-Type': 'application/json'}, data=json.dumps({'display_name': display_name}))
            if res.status_code == 200:
                logger.info('System display name changed from %s to %s', old_display_name, display_name)
                return True
    ...
    def get_advisor_report(self):
        ...
        if content is None:
            ...
            if not os.path.exists('/var/lib/insights'):
                os.makedirs('/var/lib/insights', mode=493)
