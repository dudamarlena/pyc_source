...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/smbstatus.py
...
class Smbstatusp(Statuslist):
    ...
    def parse_content(self, content):
        ...
        if not any(l.startswith('PID ') for l in content):
            raise ParseException('Cannot find the header line.')
        self.data = parse_fixed_table(content, heading_ignore=['PID     Username'], header_substitute=[
