...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/core/dr.py
...
import inspect, logging, json, os, pkgutil, re, six, sys, time, traceback
...
def get_metadata(component):
    ...
    an empty dictionary.
    """
    if component in DELEGATES:
        return get_delegate(component).metadata
    return {}
...
def get_tags(component):
    ...
    """
    if component in DELEGATES:
        return get_delegate(component).tags
    return set()
...
def get_links(component):
    ...
    """
    if component in DELEGATES:
        return get_delegate(component).links
    return dict()
...
def _load_components(path, include='.*', exclude='test', continue_on_error=True):
    ...
    for _, name, is_pkg in pkgutil.iter_modules(path=package.__path__, prefix=prefix):
        ...
        elif do_include(name) and not do_exclude(name):
...
class Broker(object):
    ...
    def print_component(self, component_type):
        print(json.dumps(dict((get_name(c), self[c]) for c in sorted(self.get_by_type(component_type), key=get_name))))
...
def run(components=None, broker=None):
    ...
    for component in run_order(components):
        ...
        try:
            try:
                if component not in broker and component in components and component in DELEGATES and is_enabled(component):
