...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/client/auto_config.py
...
def _is_rhn_or_rhsm(hostname):
    return hostname == 'subscription.rhn.redhat.com' or hostname == 'subscription.rhsm.redhat.com'
...
def _try_satellite6_configuration(config):
    ...
    try:
        ...
        logger.debug('Found %sHost: %s, Port: %s', '' if _is_rhn_or_rhsm(rhsm_hostname) or _is_staging_rhsm(rhsm_hostname) else 'Satellite 6 Server ', rhsm_hostname, rhsm_hostport)
