...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/krb5kdc_log.py
...
class KerberosKDCLog(LogFileOutput):
    ...
    _LINE_RE = re.compile('(?P<timestamp>\\w{3} \\d\\d \\d\\d:\\d\\d:\\d\\d) ' + '(?P<system>\\w\\S+) ' + '(?P<service>\\w+)\\[(?P<pid>\\d+)\\]\\((?P<level>\\w+)\\): ' + '(?P<message>.*)')
