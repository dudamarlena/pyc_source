...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/pam.py
...
class PamConfEntry(object):
    ...
    type_re = '(?P<type>-?(?:account|auth|password|session))'
    control_re = '(?P<control>required|requisite|sufficient|optional|' + 'include|substack|\\[\\w+=\\w+(?:\\s+\\w+=\\w+)*\\])'
    mod_path_re = '(?P<module>[\\w.-]+)'
    mod_args_re = '(?P<mod_args>\\S.*)'
    line_re = ('\\s+').join([type_re, control_re, mod_path_re]) + '(?:\\s+' + mod_args_re + ')?'
    ...
    def __init__(self, line, pamd_conf=False, service=None):
        ...
        if match:
            ...
            if self._control_raw.startswith('['):
                ...
                self.control_flags = [
                 self.ControlFlag(self._control_raw, None)]
            self.module_name = match.group('module')
            self.module_args = match.group('mod_args') if 'mod_args' in match.groupdict() else None
            self.module_args_dict = optlist_to_dict(self.module_args, opt_sep=' ') if self.module_args is not None else {}
        else:
            self._errors.append(("Cannot parse line '{l}' as a valid pam.d entry").format(l=self._full_line))
        return

    def __repr__(self):
        return ('<PamConfEntry for {svc}: {typ} {ctl} {name}{args}>').format(svc=self.service, typ=self._type_raw, ctl=self._control_raw, name=self.module_name, args=' ' + self.module_args if self.module_args else '')
