...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/nmcli.py
...
class NmcliDevShow(CommandParser, dict):
    ...
    def parse_content(self, content):
        ...
        for line in get_active_lines(content):
            if not ('not found' in line or 'Error' in line or 'No such file' in line or 'Warning' in line):
                if len(line.split(': ')) >= 2:
                    key, val = line.split(': ')
                    if 'IP' in key:
                        proto = re.sub('\\[|\\]', '', key.split('.')[1])
                        key = key.split('.')[0] + '_' + proto
                    else:
                        key = key.split('.')[1]
                    val = re.sub('\\d+\\s|\\(|\\)', '', val.strip())
        ...
        if not data:
            raise SkipException()
        self.update(data)
        self._con_dev = [ k for k, v in data.items() if 'STATE' in v and v['STATE'] == 'connected'
                        ]
