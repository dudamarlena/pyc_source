...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/jboss_domain_log.py
...
class JbossDomainLog(LogFileOutput):
    ...
    """
    time_format = '%H:%M:%S'
    _line_re = re.compile('^(?P<timestamp>\\d+:\\d+:\\d+)(,\\d+)(?P<messages>.*)$')
    ...
    def get_after(self, timestamp, s=None):
        ...
        if not isinstance(timestamp, time):
            raise TypeError(("get_after needs a time type timestamp, but get '{c}'").format(c=timestamp))
