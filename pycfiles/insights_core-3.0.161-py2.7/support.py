...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/client/support.py
# Compiled at: 2019-11-14 13:57:46
"""
Module responsible for running the --support option for collecting debug information
"""
from __future__ import absolute_import
import logging, shlex, re, os, requests, tempfile, time, subprocess
...
def _legacy_registration_check(pconn):
    ...
    if type(api_reg_status) is bool:
        ...
        api_reg_status = False
    return {'messages': [local_record, api_record], 'status': api_reg_status, 
...
class InsightsSupport(object):
    ...
    def collect_support_info(self):
        ...
        tar_file = os.path.join(log_archive_dir, 'insights-client-logs-' + time.strftime('%Y%m%d%H%M%S') + '.tar.gz')
    ...
    def _support_diag_dump(self):
        ...
        if self.config.offline:
            ...
            if self.config.proxy:
                obfuscated_proxy = re.sub('(.*)(:)(.*)(@.*)', '\\1\\2********\\4', self.config.proxy)
            else:
