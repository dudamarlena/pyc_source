...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/ps.py
...
class Ps(CommandParser):
    ...
    def parse_content(self, content):
        ...
        if header_line is not None:
            self.data = [ row for row in parse_delimited_table(content, heading_ignore=[header_line], max_splits=self.max_splits, raw_line_key=raw_line_key) if self.command_name in row
                        ]
            ...
            raise ParseException(('{0}: Cannot find ps header line containing {1} and {2} in output').format(self.__class__.__name__, self.user_name, self.command_name))
        return
