...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/p4a/plonetagging/browser/tagging.py
# Compiled at: 2007-10-12 18:11:48
import Acquisition, urllib, os, lovely.tag.browser.tag
...
class TaggingView(BrowserView):
    ...
    def tagging_url(self):
        portal = cmfutils.getToolByName(self.context, 'portal_url').getPortalObject()
...
class TagsWidget(zope.app.form.browser.textwidgets.TextWidget):
    ...
    def getInputValue(self):
        return super(TagsWidget, self).getInputValue() or ''


class UserTagForm(formbase.EditForm, lovely.tag.browser.tag.UserTagForm, TaggingView):
    """Edit user-defined tags.
    """
    __module__ = __name__
    template = ViewPageTemplateFile('content-tags.pt')
    actions = lovely.tag.browser.tag.UserTagForm.actions
    form_fields = form.Fields(schema.TextLine(__name__='tags', title='Tags', description='Enter as many tags as you like, separated by spaces', required=False))
    form_fields['tags'].custom_widget = TagsWidget

    def update(self):
        satisfy_zope3_request(self.context, self.request)
        super(UserTagForm, self).update()
        if self.actions[(self.prefix + '.actions.apply')].submitted() and not self.errors:
            event.notify(objectevent.ObjectModifiedEvent(self.context))

    def setUpWidgets(self, *args, **kwargs):
        lovely.tag.browser.tag.UserTagForm.setUpWidgets(self, *args, **kwargs)
    ...
    def items_iter(self):
        ...
        for x in engine.getItems(tags=tags):
            try:
                ...
                logger.exception('whoops, an intid points to an invalid object: %s' % str(x))
...
class RootTagsView(TaggingView):
    ...
    """
    __module__ = __name__
    listing_name = 'tagged_item_listing'
    ...
    def __bobo_traverse__(self, REQUEST, name):
        ...
        obj = component.getMultiAdapter((self.context, self.request), name=self.listing_name, interface=interface.Interface)
