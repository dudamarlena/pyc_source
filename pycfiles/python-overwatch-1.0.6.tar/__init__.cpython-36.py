...
# [GCC 8.4.0]
# Embedded file name: /home/alexbotello/Code/python-overwatch/overwatch/__init__.py
# Compiled at: 2018-04-02 18:49:35
# Size of source mod 2**32: 3783 bytes
from requests_html import HTMLSession
from selections import heroes, compare
from errors import InvalidBattletag, InvalidCombination, InvalidFilter, InvalidHero, NotFound
session = HTMLSession()
...
class Overwatch:
    ...
    def initial_error_check(self):
        if self.filter == 'Hero Specific':
            if self.hero == 'all':
                raise InvalidCombination(f"'{self.filter}' and '{self.hero}' are not valid filter combinations")
        else:
            if self.filter == 'Miscellaneous':
                if self.hero != 'all':
                    raise InvalidCombination(f"'{self.filter}' and '{self.hero}' are not valid filter combinations")
    ...
    def error_handler(func):
        ...
        def decorator(self, *args):
            try:
                ...
                raise NotFound(f"No results were found for {self._hero} in this mode")
            except KeyError:
