...
# Embedded file name: build/bdist.linux-i686/egg/pyhacc/PyHaccSchema.py
# Compiled at: 2015-01-16 12:37:48
...
import sqlalchemy.sql.functions as func, sqlalchemy.sql.expression as expr, datetime, decimal, uuid
...
class Options(Base):
    __table__ = Table('options', metadata, Column('id', UUID, primary_key=True, default=lambda : uuid.uuid1()), Column('data_name', String(50)))
...
class AccountTypes(Base):
    __table__ = Table('accounttypes', metadata, Column('id', UUID, primary_key=True, default=lambda : uuid.uuid1()), Column('name', String(20)), Column('description', Text), Column('balance_sheet', Boolean, default=True), Column('debit', Boolean, default=True), Column('retained_earnings', Boolean, default=False), Column('sort', Integer))
    accounts = relationship('Accounts', backref='AccountType')


def AccountTypeReferral(label, backref):
    return ForeignKeyReferral(str, label, backref=backref, class_=AccountTypes, userkey='name')
...
class AccountTypeEntity(EntityHelper):
    ...
    def list_query_converter(self):
        ...
        return (
         Query(queryCols).order_by(AccountTypes.sort, AccountTypes.name, self.key_column), lambda x: x._hidden_id)
...
class Accounts(Base):
    __table__ = Table('accounts', metadata, Column('id', UUID, primary_key=True, default=lambda : uuid.uuid1()), Column('type_id', UUID, ForeignKey('accounttypes.id'), nullable=False), Column('journal_id', UUID, ForeignKey('journals.id', ondelete='RESTRICT'), nullable=False), Column('name', String(20)), Column('description', Text), Column('rec_note', Text, info={'label': 'Reconciliation Note'}), Column('retearn_id', UUID, ForeignKey('accounts.id'), default=None), Column('instname', Text), Column('instaddr1', Text), Column('instaddr2', Text), Column('instcity', Text), Column('inststate', Text), Column('instzip', Text))
...
def AccountReferral(label, backref):
    return ForeignKeyReferral(str, label, backref=backref, class_=Accounts, userkey='name', entity=AccountEntity)


def CapitalAccountReferral(label, backref):
    return ForeignKeyReferral(str, label, backref=backref, class_=Accounts, userkey='name', filter_query=lambda q: q.join(AccountTypes).filter(AccountTypes.retained_earnings == True))
...
class Journals(Base):
    __table__ = Table('journals', metadata, Column('id', UUID, primary_key=True, default=lambda : uuid.uuid1()), Column('name', String(20)), Column('description', Text))
    accounts = relationship('Accounts', backref='Journal')


def JournalReferral(label, backref):
    return ForeignKeyReferral(str, label, backref=backref, class_=Journals, userkey='name')
...
class Transactions(Base):
    __table__ = Table('transactions', metadata, Column('tid', UUID, primary_key=True, default=lambda : uuid.uuid1()), Column('date', Date), Column('reference', String(15)), Column('payee', Text, info={'yoke': 'line'}), Column('memo', Text, info={'yoke': 'line'}), Column('receipt', Text, info={'yoke': 'formatted'}))
...
class TransactionEntity(EntityHelper):
    ...
    def list_query_converter(self):
        ...
        queryCols = tuple([self.key_column.label('_hidden_id')] + self.list_display_columns)
        return (
         Query(queryCols).order_by(expr.desc(self.list_display_columns[0]), self.key_column), lambda x: x._hidden_id)
...
class Splits(Base):
    __table__ = Table('splits', metadata, Column('sid', UUID, primary_key=True, default=lambda : uuid.uuid1()), Column('stid', UUID, ForeignKey('transactions.tid'), nullable=False), Column('account_id', UUID, ForeignKey('accounts.id'), nullable=False), Column('sum', Numeric(precision=10, scale=2, asdecimal=True)))
...
class Tagsplits(Base):
    __table__ = Table('tagsplits', metadata, Column('tag_id', UUID, ForeignKey('tags.id', ondelete='RESTRICT'), primary_key=True, nullable=False), Column('split_id', UUID, ForeignKey('splits.sid'), primary_key=True, nullable=False))


class Tags(Base):
    __table__ = Table('tags', metadata, Column('id', UUID, primary_key=True, default=lambda : uuid.uuid1()), Column('name', String(20)), Column('description', Text))
...
def TagReferral(label, backref):
    return ForeignKeyReferral(str, label, backref=backref, class_=Tags, userkey='name')
...
def Utilities(session_source):
    ...
    for account in s.query(Accounts).join(AccountTypes).filter(expr.and_(AccountTypes.balance_sheet == False, Accounts.retearn_id == None)).all():
        print (
         account.id, account.name, account.AccountTypes.balance_sheet, account.AccountTypes.name)
...
class TransactionTemplate:
    ...
    def __cmp__(self, other):
        if self.payee == other.payee and self.memo == other.memo and self.accounts == other.accounts:
            return 0
        return 1
