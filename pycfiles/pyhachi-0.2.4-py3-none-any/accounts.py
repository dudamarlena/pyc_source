...
# Embedded file name: build/bdist.linux-i686/egg/pyhacc/accounts.py
# Compiled at: 2013-09-21 11:16:00
...
class AccountEditor(BoundDialog):
    ...
    def __init__(self, parent, row=None, Session=None, row_id=None, flush=True):
        ...
        self.transactions_tab = PBTableTab(self, Session, TransactionEntity, [
         (
          Splits.account_id, lambda dataContext: dataContext.id)], Query((Transactions.tid.label('id'), Transactions.date, Transactions.reference, Transactions.payee, Transactions.memo, Splits.sum)).join(Splits).order_by(Transactions.date.desc()), extensionId=suffixExtId(self, 'Transactions'))
...
class AccountReconcile(BoundDialog):
    ...
    def load(self):
        ...
        self.outstanding_split_list = self.session.query(Splits).outerjoin(q).join(Accounts).join(Transactions).filter(Accounts.id == self.main_row.id).filter(q.c.tag_name_s == None).order_by(Transactions.date).all()
        ...
        self.reconciled_split_list = self.session.query(Splits).outerjoin(q).join(Accounts).filter(Accounts.id == self.main_row.id).filter(q.c.tag_name_s == Tags.Names.BankReconciled).all()
