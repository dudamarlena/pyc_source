...
# Embedded file name: build/bdist.linux-i686/egg/pyhacc/PyHaccLib.py
...
def gui_app(conn=None):
    ...
    if conn is None:
        ...
        if multi_auth_dialog(parent=None, init_session_maker=SessionHolder, init_db=make_db) != QtGui.QDialog.Accepted:
            return
    else:
        SessionHolder(conn)
...
def fiscal_year_prior():
    ...
    return (
     datetime.date(d.year - 1, 1, 1), datetime.date(d.year - 1, 12, 31))


def fiscal_year_current():
    d = datetime.date.today()
    return (
     datetime.date(d.year, 1, 1), datetime.date(d.year, 12, 31))
...
def fiscal_month_prior():
    ...
    d = prior_month_end(d)
    return (
     datetime.date(d.year, d.month, 1), d)


def fiscal_month_current():
    d = datetime.date.today()
    return (
     datetime.date(d.year, d.month, 1), month_end(d.year, d.month))
...
fiscal_date_range = {'Last Year': fiscal_year_prior, 
