...
# Embedded file name: build/bdist.linux-i686/egg/pyhacc/reports.py
# Compiled at: 2014-02-05 08:16:17
from qtalchemy import *
from qtalchemy.widgets import *
from sqlalchemy.orm.exc import UnmappedInstanceError
from PySide import QtCore, QtGui
import qtalchemy.ext.reporttools as rpttools, collections, datetime, csv, re, cgi
...
class ReportChooser(QtCore.QObject):
    ...
    def __call__(self):
        ...
        report.construct()
        self.mainWindow.addWorkspaceWindow(report.unified_tab(self.mainWindow), title=self.prompt_callable.name, settingsKey=self.prompt_callable.name.replace(' ', '_'))
...
def GeraldoTemplate(rpt, pagesize):
    ...
    class Template(Report):
        ...
        class band_page_header(ReportBand):
            elements = [
             SystemField(expression='%(report_title)s', left=0, width=BAND_WIDTH, style={'fontName': 'Helvetica-Bold', 'fontSize': 14, 'alignment': TA_CENTER})]
            borders = {'bottom': True}

        class band_page_footer(ReportBand):
            elements = [
             SystemField(expression='Printed %(now:%Y, %b %d)s at %(now:%H:%M)s', width=BAND_WIDTH, style={'alignment': TA_LEFT}),
             SystemField(expression='Page %(page_number)d of %(page_count)d', width=BAND_WIDTH, style={'alignment': TA_RIGHT})]
            borders = {'top': True}
...
class PyHaccReport(ModelObject):
        ...
        class PromptWidget(QtGui.QWidget, MapperMixin):
            ...
            def refreshModel(self):
                ...
                if self.table.model() is None or self.report.refresh_model_dimensions:
                    self.table.setModel(PBTableModel(self.report.columns(), objectConverter=self.report.objectConverter()), extensionId=suffixExtId(self, self.report.tableExtensionId()))
            ...
            def pdf_button(self):
                import tempfile, os, qtalchemy.xplatform as xplatform
    ...
    def html(self, stream):
        ...
        header = [
         bracket_list([ c.label for c in cols ], 'th')]
        header += [ bracket_list([ getattr(p, c.attr) for c in cols ], 'td') for p in lines ]
        table = bracket_list(header, 'tr', join='\n')
        import pyhacc
        stream.write(html_template.format(title=self.report_title, version=pyhacc.__version__, rptclass=self.__class__.__name__, table='<table>' + table + '</table>'))
    ...
    def geraldoDetailObjects(self, pagesize, detailRatios, leftIndent=None):
        ...
        if detailRatios is None:
            detailRatios = [
             (pagesize[0] - 1.5 * inch - leftIndent) / len(cols)] * len(cols)
        else:
            ...
            for i in range(len(cols)):
                ...
                if cols[i].type_ is decimal.Decimal:
                    ...
                    style['wordWrap'] = True
                det_elements.append(ObjectValue(attribute_name=cols[i].attr, get_value=wf(cols[i].attr), top=0, left=leftIndent + sum([ detailRatios[j] for j in range(i) ]), width=detailRatios[i], style=style))
...
class PyHaccReportGrouped(PyHaccReport):
    ...
    def geraldo(self, outfile, pagesize, detailRatios=None, **kwargs):
        ...
        elts_header = [ ObjectValue(attribute_name=header_items[i], left=i * width, width=width, style=header_style) for i in range(len(header_items)) ] + [
        ...
        class GeraldoBS(GeraldoTemplate(self, pagesize)):
            ...
            groups = [
             ReportGroup(attribute_name=key, band_header=ReportBand(height=head_height * inch, borders={'bottom': True}, elements=elts_header), band_footer=ReportBand(borders={'top': True}, elements=elts_footer))]
...
class AccountBalanceListReport(PyHaccReportGrouped):
    ...
    def post_process_data(self, d):
        ...
        for l in lines:
            ...
            if self.group_by_journal:
                l.group_by = (
                 l.journals_id, l.accounttypes_id)
            else:
    ...
    def columns(self):
        return [
         ModelColumn('accounttypes_name', str, label='Account Type', width=25),
...
class BalanceSheet(AccountBalanceListReport):
    ...
    def data(self):
        ...
        if self.group_by_journal:
            orderby = (
             Journals.name, AccountTypes.sort, Accounts.name)
        else:
            orderby = (
             AccountTypes.sort, Accounts.name)
        qsub = s.query(Splits.account_id.label('account_id'), func.sum(Splits.sum).label('sum')).join(Accounts).join(AccountTypes).join(Transactions).filter(tranCriteria).group_by(Splits.account_id).subquery()
        t = s.query(Accounts.id.label('accounts_id'), Accounts.name.label('accounts_name'), Accounts.retearn_id, Journals.id.label('journals_id'), Journals.name.label('journals_name'), AccountTypes.id.label('accounttypes_id'), AccountTypes.name.label('accounttypes_name'), AccountTypes.balance_sheet, AccountTypes.retained_earnings, qsub.c.sum.label('sum')).join(Journals).outerjoin((qsub, Accounts.id == qsub.c.account_id)).join(AccountTypes).order_by(*orderby)
        d = t.all()
...
class ProfitAndLoss(AccountBalanceListReport):
    ...
    def data(self):
        ...
        qsub = s.query(Splits.account_id.label('account_id'), func.sum(Splits.sum).label('sum')).join(Accounts).join(AccountTypes).join(Transactions).filter(tranCriteria).filter(expr.not_(AccountTypes.balance_sheet)).group_by(Splits.account_id).subquery()
        t = s.query(Accounts.id.label('accounts_id'), Accounts.name.label('accounts_name'), Journals.id.label('journals_id'), Journals.name.label('journals_name'), AccountTypes.id.label('accounttypes_id'), AccountTypes.name.label('accounttypes_name'), qsub.c.sum).join((
         qsub, qsub.c.account_id == Accounts.id)).join(Journals).join(AccountTypes).order_by(*orderby).filter(expr.not_(AccountTypes.balance_sheet))
        d = t.all()
        s.close()
        return self.post_process_data(d)
...
class TransactionsByAccount(PyHaccReport):
    ...
    prompt_order = ('fiscal_range', 'begin_date', 'end_date', 'account', 'group_payee',
                    'group_memo')
    ...
    def data(self):
        ...
        if self.group_payee and self.group_memo:
            ...
            if not self.group_payee and self.group_memo:
        ...
        if self.group_payee and self.group_memo:
            ...
            if not self.group_payee and self.group_memo:
                return [ModelColumn('memo', str, label='Memo'),
                 ModelColumn('debit', decimal.Decimal, label='Debit'),
                 ModelColumn('credit', decimal.Decimal, label='Crebit')]
            if self.group_payee and not self.group_memo:
                return [ModelColumn('payee', str, label='Payee'),
                 ModelColumn('debit', decimal.Decimal, label='Debit'),
                 ModelColumn('credit', decimal.Decimal, label='Crebit')]
            return [
             ModelColumn('date', datetime.date, label='Date'),
...
class TransactionList(PyHaccReport):
    ...
    def columns(self):
        return [
         ModelColumn('account', str, label='Account', width=20),
...
class TransactionTagged(PyHaccReportGrouped):
    ...
    def data(self):
        ...
        if self.tag_obj:
            ttt = self.tag_obj.id
        q = s.query(Splits.sum, Transactions.tid, Transactions.date, Transactions.reference, Transactions.payee, Transactions.memo, Tagsplits.tag_id, Accounts.name.label('account')).join(Transactions).join(Accounts).outerjoin(Tagsplits).filter(expr.or_(Tagsplits.tag_id == None, Tagsplits.tag_id == ttt))
        if self.account_obj is not None:
            q = q.filter(Accounts.id == self.account_obj.id)
        if self.begin_date is not None:
            q = q.filter(Transactions.date >= self.begin_date)
...
class DetailedProfitAndLoss(PyHaccReportGrouped):
    ...
    def columns(self):
        return [
         ModelColumn('journals_name', str, label='Journal', width=20),
         ModelColumn('accounts_name', str, label='Account', width=25),
...
class IntervalPL(PyHaccReportGrouped):
    ...
    def data(self):
        ...
        for d1, d2 in dates:
            ...
            q = s.query(Splits.account_id.label('account_id'), func.sum(Splits.sum).label('sum')).join(Accounts).join(AccountTypes).join(Transactions).filter(tranCriteria).filter(expr.not_(AccountTypes.balance_sheet)).group_by(Splits.account_id)
        ...
        t = t.filter(expr.not_(AccountTypes.balance_sheet)).order_by(AccountTypes.sort, Accounts.name)
        lines = t.all()
    ...
    def columns(self):
        cols = [
         ModelColumn('accounttypes_name', str, label='Account Type'),
    ...
    def groupColumns(self):
        return (
         [
          'accounttypes_name'], 'accounttypes_id')
    ...
    def geraldo(self, outfile, pagesize, detailRatios=None, **kwargs):
        ...
        elts_detail = self.geraldoDetailObjects(pagesize, detailRatios, leftIndent=0.25 * inch)
        elts_header = [
         ObjectValue(attribute_name='accounttypes_name', left=0, style={'fontName': 'Helvetica-Bold', 'fontSize': 12})] + [ Label(text=('{0:%x}').format(dates[i][1]), top=0, left=elts_detail[(2 * i + 1)].left, width=elts_detail[(2 * i + 1)].width + elts_detail[(2 * i + 2)].width, style={'alignment': TA_CENTER, 'fontName': 'Helvetica-Bold', 'fontSize': 12}) for i in range(self.interval_count)
                                                                                                                          ]
        elts_footer = [ ObjectValue(get_value=lambda obj, last_row, xx=i: summary_item(xx, obj, last_row), top=0, left=elts_detail[i].left if i > 0 else 0.0, width=elts_detail[i].width, style={'alignment': TA_RIGHT if i > 0 else TA_LEFT, 'fontName': 'Helvetica-Bold'})
         for i in range(self.interval_count * 2 + 1)
                      ]
        ...
        class GeraldoBS(GeraldoTemplate(self, pagesize)):
            ...
            groups = [
             ReportGroup(attribute_name='accounttypes_name', band_header=ReportBand(height=head_height * inch, borders={'bottom': True}, elements=elts_header), band_footer=ReportBand(borders={'top': True}, elements=elts_footer))]
