...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_lib/tools/preprocessing.py
...
def run_cutadapt(job, r1_id, r2_id, fwd_3pr_adapter, rev_3pr_adapter):
    ...
    docker_call(tool='quay.io/ucsc_cgl/cutadapt:1.9--6bd44edd2b8f8f17e25c5a268fedaab65fa851d2', work_dir=work_dir, parameters=parameters)
    if r1_id and r2_id:
...
def run_samtools_faidx(job, ref_id):
    ...
    docker_call(work_dir=work_dir, parameters=command, tool='quay.io/ucsc_cgl/samtools:0.1.19--dd5ac549b95eb3e5d166a5e310417ef13651994e')
...
def run_samtools_index(job, bam):
    ...
    parameters = [
     'index', '/data/sample.bam']
    docker_call(work_dir=work_dir, parameters=parameters, tool='quay.io/ucsc_cgl/samtools:0.1.19--dd5ac549b95eb3e5d166a5e310417ef13651994e')
...
def run_samtools_sort(job, bam):
    ...
    command = ['sort',
     ...
     '-o', '/data/output.bam',
     '/data/input.bam']
    docker_call(work_dir=work_dir, parameters=command, tool='quay.io/ucsc_cgl/samtools:1.3--256539928ea162949d8a65ca5c79a72ef557ce7c', outputs={'output.bam': None})
...
def run_picard_create_sequence_dictionary(job, ref_id):
    ...
    docker_call(work_dir=work_dir, parameters=command, tool='quay.io/ucsc_cgl/picardtools:1.95--dd5ac549b95eb3e5d166a5e310417ef13651994e')
...
def picard_mark_duplicates(job, bam, bai, validation_stringency='LENIENT'):
    ...
    docker_call(work_dir=work_dir, parameters=command, env={'_JAVA_OPTIONS': ('-Djava.io.tmpdir=/data/ -Xmx{}').format(job.memory)}, tool='quay.io/ucsc_cgl/picardtools:1.95--dd5ac549b95eb3e5d166a5e310417ef13651994e', outputs={'mkdups.bam': None, 'mkdups.bai': None})
...
def run_gatk_preprocessing(job, bam, bai, ref, ref_dict, fai, g1k, mills, dbsnp, unsafe=False):
    ...
    mdups = job.wrapJobFn(picard_mark_duplicates, bam, bai, cores=job.cores, disk=mdups_disk, memory=job.memory)
    genome_ref_size = ref.size + ref_dict.size + fai.size
    indel_ref_size = mills.size + g1k.size + genome_ref_size
    realigner_target_disk = PromisedRequirement(lambda bam_, bai_, ref_size: bam_.size + bai_.size + 2 * ref_size, mdups.rv(0), mdups.rv(1), indel_ref_size)
    realigner_target = job.wrapJobFn(run_realigner_target_creator, mdups.rv(0), mdups.rv(1), ref, ref_dict, fai, g1k, mills, unsafe=unsafe, cores=1, disk=realigner_target_disk, memory=job.memory)
    indel_realign_disk = PromisedRequirement(lambda bam_, bai_, intervals, ref_size: 2 * (bam_.size + bai_.size) + intervals.size + ref_size, mdups.rv(0), mdups.rv(1), realigner_target.rv(), indel_ref_size)
    indel_realign = job.wrapJobFn(run_indel_realignment, realigner_target.rv(), mdups.rv(0), mdups.rv(1), ref, ref_dict, fai, g1k, mills, unsafe=unsafe, cores=1, disk=indel_realign_disk, memory=job.memory)
    bqsr_ref_size = dbsnp.size + mills.size + genome_ref_size
    base_recal_disk = PromisedRequirement(lambda bam_, bai_, ref_size: bam_.size + bai_.size + 2 * ref_size, indel_realign.rv(0), indel_realign.rv(1), bqsr_ref_size)
    base_recal = job.wrapJobFn(run_base_recalibration, indel_realign.rv(0), indel_realign.rv(1), ref, ref_dict, fai, dbsnp, mills, unsafe=unsafe, cores=job.cores, disk=base_recal_disk, memory=job.memory)
    recalibrate_reads_disk = PromisedRequirement(lambda bam_, bai_, recal, ref_size: 2 * (bam_.size + bai_.size) + recal.size + ref_size, indel_realign.rv(0), indel_realign.rv(1), base_recal.rv(), genome_ref_size)
    recalibrate_reads = job.wrapJobFn(apply_bqsr_recalibration, base_recal.rv(), indel_realign.rv(0), indel_realign.rv(1), ref, ref_dict, fai, unsafe=unsafe, cores=job.cores, disk=recalibrate_reads_disk, memory=job.memory)
    job.addChild(mdups)
...
def run_realigner_target_creator(job, bam, bai, ref, ref_dict, fai, g1k, mills, unsafe=False):
    ...
    :rtype: str
    """
    inputs = {'ref.fasta': ref, 'ref.fasta.fai': fai, 
       'ref.dict': ref_dict, 
       'input.bam': bam, 
       'input.bai': bai, 
       '1000G.vcf': g1k, 
    ...
    docker_call(tool='quay.io/ucsc_cgl/gatk:3.5--dba6dae49156168a909c43330350c6161dc7ecc2', inputs=inputs.keys(), outputs={'sample.intervals': None}, work_dir=work_dir, parameters=parameters, env=dict(JAVA_OPTS=('-Djava.io.tmpdir=/data/ -Xmx{}').format(job.memory)))
...
def run_indel_realignment(job, intervals, bam, bai, ref, ref_dict, fai, g1k, mills, unsafe=False):
    ...
    if unsafe:
        parameters.extend(['-U', 'ALLOW_SEQ_DICT_INCOMPATIBILITY'])
    docker_call(tool='quay.io/ucsc_cgl/gatk:3.5--dba6dae49156168a909c43330350c6161dc7ecc2', inputs=inputs.keys(), work_dir=work_dir, parameters=parameters, env=dict(JAVA_OPTS=('-Djava.io.tmpdir=/data/ -Xmx{}').format(job.memory)), outputs={'output.bam': None, 'output.bai': None})
...
def run_base_recalibration(job, bam, bai, ref, ref_dict, fai, dbsnp, mills, unsafe=False):
    ...
    if unsafe:
        parameters.extend(['-U', 'ALLOW_SEQ_DICT_INCOMPATIBILITY'])
    docker_call(tool='quay.io/ucsc_cgl/gatk:3.5--dba6dae49156168a909c43330350c6161dc7ecc2', inputs=inputs.keys(), outputs={'recal_data.table': None}, work_dir=work_dir, parameters=parameters, env=dict(JAVA_OPTS=('-Djava.io.tmpdir=/data/ -Xmx{}').format(job.memory)))
...
def apply_bqsr_recalibration(job, table, bam, bai, ref, ref_dict, fai, unsafe=False):
    ...
    if unsafe:
        parameters.extend(['-U', 'ALLOW_SEQ_DICT_INCOMPATIBILITY'])
    docker_call(tool='quay.io/ucsc_cgl/gatk:3.5--dba6dae49156168a909c43330350c6161dc7ecc2', inputs=inputs.keys(), outputs={'bqsr.bam': None, 'bqsr.bai': None}, work_dir=work_dir, parameters=parameters, env=dict(JAVA_OPTS=('-Djava.io.tmpdir=/data/ -Xmx{}').format(job.memory)))
