...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_lib/tools/variant_manipulation.py
...
def gatk_select_variants(job, mode, vcf_id, ref_fasta, ref_fai, ref_dict):
    ...
    inputs = {'genome.fa': ref_fasta, 'genome.fa.fai': ref_fai, 
    ...
    command = ['-T', 'SelectVariants',
     ...
     '-o', 'output.vcf',
     '-selectType', mode]
    docker_call(work_dir=work_dir, env={'JAVA_OPTS': ('-Djava.io.tmpdir=/data/ -Xmx{}').format(job.memory)}, parameters=command, tool='quay.io/ucsc_cgl/gatk:3.5--dba6dae49156168a909c43330350c6161dc7ecc2', inputs=inputs.keys(), outputs={'output.vcf': None})
...
def gatk_variant_filtration(job, vcf_id, filter_name, filter_expression, ref_fasta, ref_fai, ref_dict):
    ...
    job.fileStore.logToMaster(('Running GATK VariantFiltration using {name}: {expression}').format(name=filter_name, expression=filter_expression))
    docker_call(work_dir=work_dir, env={'JAVA_OPTS': ('-Djava.io.tmpdir=/data/ -Xmx{}').format(job.memory)}, parameters=command, tool='quay.io/ucsc_cgl/gatk:3.5--dba6dae49156168a909c43330350c6161dc7ecc2', inputs=inputs.keys(), outputs={'filtered_variants.vcf': None})
    ...
    with open(malformed_header, 'r') as (f):
        with open(fixed_header, 'w') as (g):
            for line in f:
...
def gatk_variant_recalibrator(job, mode, vcf, ref_fasta, ref_fai, ref_dict, annotations, hapmap=None, omni=None, phase=None, dbsnp=None, mills=None, max_gaussians=4, unsafe_mode=False):
    ...
    docker_call(work_dir=work_dir, env={'JAVA_OPTS': ('-Djava.io.tmpdir=/data/ -Xmx{}').format(job.memory)}, parameters=command, tool='quay.io/ucsc_cgl/gatk:3.5--dba6dae49156168a909c43330350c6161dc7ecc2', inputs=inputs.keys(), outputs={'output.recal': None, 'output.tranches': None, 'output.plots.R': None})
...
def gatk_apply_variant_recalibration(job, mode, vcf, recal_table, tranches, ref_fasta, ref_fai, ref_dict, ts_filter_level=99.0, unsafe_mode=False):
    """
    ...
    docker_call(work_dir=work_dir, env={'JAVA_OPTS': ('-Djava.io.tmpdir=/data/ -Xmx{}').format(job.memory)}, parameters=command, tool='quay.io/ucsc_cgl/gatk:3.5--dba6dae49156168a909c43330350c6161dc7ecc2', inputs=inputs.keys(), outputs={'vqsr.vcf': None})
...
def gatk_combine_variants(job, vcfs, ref_fasta, ref_fai, ref_dict, merge_option='UNIQUIFY'):
    ...
    docker_call(work_dir=work_dir, env={'JAVA_OPTS': ('-Djava.io.tmpdir=/data/ -Xmx{}').format(job.memory)}, parameters=command, tool='quay.io/ucsc_cgl/gatk:3.5--dba6dae49156168a909c43330350c6161dc7ecc2', inputs=inputs.keys(), outputs={'merged.vcf': None})
