...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_lib/tools/spark_tools.py
...
def _make_parameters(master_ip, default_parameters, memory, arguments, override_parameters):
    ...
    if memory is not None:
        parameters = [
         '--master', 'spark://%s:%s' % (master_ip, SPARK_MASTER_PORT),
...
def call_conductor(master_ip, src, dst, memory=None, override_parameters=None):
    ...
    arguments = [
     '-C', src, dst]
    docker_call(rm=False, tool='quay.io/ucsc_cgl/conductor', docker_parameters=master_ip.docker_parameters(['--net=host']), parameters=_make_parameters(master_ip, [], memory, arguments, override_parameters), mock=False)


def call_adam(master_ip, arguments, memory=None, override_parameters=None, run_local=False, native_adam_path=None):
    """
    ...
    if native_adam_path is None:
        docker_call(rm=False, tool='quay.io/ucsc_cgl/adam:962-ehf--6e7085f8cac4b9a927dc9fb06b48007957256b80', docker_parameters=master_ip.docker_parameters(['--net=host']), parameters=_make_parameters(master_ip, default_params, memory, arguments, override_parameters), mock=False)
    else:
        check_call([
