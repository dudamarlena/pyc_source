...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.4/site-packages/beehive/formatter/html.py
# Compiled at: 2014-11-03 06:20:31
# Size of source mod 2**32: 21370 bytes
"""
...
class HTMLFormatter(Formatter):
    __doc__ = 'Provides a single-page HTML formatter\n    that writes the result of a  test run.\n    '
    name = 'html'
    ...
    def background(self, background):
        ...
        ET.SubElement(h3, 'span', {'class': 'val'}).text = '%s: %s' % (background.keyword, background.name)
    ...
    def scenario(self, scenario):
        ...
        self.steps = ET.SubElement(self.scenario_el, 'ol', {'class': 'scenario_steps',  'id': 'scenario_%s' % self.scenario_id})
        self.scenario_name.set('onclick', "Collapsible_toggle('scenario_%s')" % self.scenario_id)
        self.scenario_id += 1
    ...
    def result(self, result):
        ...
        if result.error_message:
            ...
            link.text = 'Error message'
            embed = ET.SubElement(step, 'pre', {'id': 'embed_%s' % self.embed_id,  'style': 'display: none'})
            cleaned_error_message = ''.join(c for c in result.error_message if _valid_XML_char_ordinal(ord(c)))
        ...
        if hasattr(self, 'embed_in_this_step'):
            if self.embed_in_this_step:
                self._doEmbed(self.last_step_embed_span, self.embed_mime_type, self.embed_data, self.embed_caption)
                self.embed_in_this_step = None
    ...
    def _doEmbed(self, span, mime_type, data, caption):
        ...
        if 'video/' in mime_type:
            ...
            embed = ET.SubElement(span, 'video', {'id': 'embed_%s' % self.embed_id,  'style': 'display: none', 
             'width': '320', 
             'controls': ''})
            embed.tail = '    '
            ET.SubElement(embed, 'source', {'src': 'data:%s;base64,%s' % (mime_type, base64.b64encode(data)), 
             'type': '%s; codecs="vp8 vorbis"' % mime_type})
        if 'image/' in mime_type:
            if not caption:
                caption = 'Screenshot'
            link.text = unicode(caption)
            embed = ET.SubElement(span, 'img', {'id': 'embed_%s' % self.embed_id, 
             'style': 'display: none', 
             'src': 'data:%s;base64,%s' % (
        ...
        if 'text/' in mime_type:
            ...
            embed = ET.SubElement(span, 'pre', {'id': 'embed_%s' % self.embed_id,  'style': 'display: none'})
    ...
    def close(self):
        if not hasattr(self, 'all_features'):
            self.all_features = []
        self.duration.text = 'Finished in %0.1f seconds' % sum([x.duration for x in self.all_features])
        result = []
