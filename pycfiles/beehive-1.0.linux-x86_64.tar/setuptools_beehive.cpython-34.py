...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.4/site-packages/setuptools_beehive.py
# Compiled at: 2014-10-30 08:03:31
# Size of source mod 2**32: 4478 bytes
"""
Setuptools command for beehive.

.. code-block:: console

    python setup.py beehive_test

.. seealso::

    * http://pypi.python.org/pypi/beehive
    * http://github.com/vrutkovs/beehive
"""
from setuptools import Command
from distutils import dir_util
from fnmatch import fnmatch
import os.path, sys, shlex, subprocess

class beehive_test(Command):
    __doc__ = '\n    Simple test runner that runs \'beehive\' via a "setup.py" file.\n    This ensures that all requirements are provided before the tests are run.\n    '
    ...
    def beehive(self, path):
        beehive = os.path.join('bin', 'beehive')
        if not os.path.exists(beehive):
            beehive = 'beehive'
        cmd_options = ''
        if self.tags:
        ...
        self.announce('CMDLINE: %s %s' % (beehive, cmd_options), level=3)
        return subprocess.call([sys.executable, beehive] + shlex.split(cmd_options))