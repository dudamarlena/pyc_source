...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.4/site-packages/beehive/formatter/filter_formatter.py
...
class FilterFormatter(object):
    ...
    def examples(self, examples):
        ...
        if len(examples.rows) == 0:
            ...
            if len(examples.rows) == 1:
                table_body_range = (
                 examples.rows[0].line, examples.rows[0].line)
            else:
                table_body_range = (
                 examples.rows[1].line, examples.rows[(-1)].line)
        self._examples_range = [
         examples.line_range()[0], table_body_range[1]]
    ...
    def detect_filters(self, filter_list):
        ...
        if len(filter_classes) > 1:
            if filter_classes != set([str, unicode]):
    ...
    def replay(self):
        ...
        feature_element_ok = self.filter.eval(tags, [n for n in names if n is not None], [r for r in ranges if r is not None])
        examples_ok = self.filter.eval(tags + self._examples_tags, [n for n in names + [self._examples_name] if n is not None], [r for r in ranges + [self._examples_range] if r is not None])
