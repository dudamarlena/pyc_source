...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.4/site-packages/beehive/formatter/tags.py
...
class TagsFormatter(AbstractTagsFormatter):
    ...
    def report_tag_counts_by_usage(self):
        ordered_tags = sorted(list(self.tag_counts.keys()), key=lambda x: len(self.tag_counts[x], reverse=True))
        tag_maxsize = compute_words_maxsize(ordered_tags)
        schema = '  @%-' + str(tag_maxsize) + 's %4d    (used for %s)\n'
