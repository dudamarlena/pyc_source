...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.4/site-packages/beehive/reporter/junit.py
...
if hasattr(ElementTree, '_serialize'):
    if sys.version_info[0] == 2:
        ...
        def _serialize_xml(write, elem, encoding, qnames, namespaces, orig=ElementTree._serialize_xml):
            if elem.tag == '![CDATA[':
        ...
        def _serialize_xml(write, elem, qnames, namespaces, orig=ElementTree._serialize_xml, **kwargs):
            if elem.tag == '![CDATA[':
                write('\n<%s%s]]>\n' % (elem.tag, elem.text))
                return
