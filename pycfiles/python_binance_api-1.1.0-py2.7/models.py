...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/binance/models.py
# Compiled at: 2017-11-02 09:55:33
import six
...
class Depth(ApiModel):
    ...
    def __init__(self, last_update_id=None, bids=None, asks=None):
        ...
        if bids != None:
            self.bids = sort_dict_in_list(bids, 'price', reverse=True)
        if asks != None:
            self.asks = sort_dict_in_list(asks, 'price')
        return
...
class UserDataEvent(object):
    ...
    def object_from_dictionary(cls, entry):
        ...
        if entry['e'] == ExecutionReportEvent.EVENT_TYPE:
