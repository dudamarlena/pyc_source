...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/binance/bind.py
# Compiled at: 2017-11-02 09:29:36
import re, six, hmac, time
    ...
    class BinanceAPIMethod(object):
        ...
        def _build_parameters(self, args, kwargs):
            ...
            if self.signature and self.api.api_key != None:
                signature_parameters_string = ('&').join([ '%s=%s' % (key, value) for key, value in self.parameters.items() ])
        ...
        def execute(self):
            url, method, body, headers = Request(self.api).prepare_request(self.method, self.path, self.parameters)
