...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fever/evidence/retrieval_methods/top_docs.py
...
class TopNDocsTopNSents(RetrievalMethod):
    ...
    def get_sentences_for_claim(self, claim_text, include_text=False):
        ...
        for page in pages:
            ...
            lines = [line.split('\t')[1] if len(line.split('\t')[1]) > 1 else '' for line in lines]
            p_lines.extend(zip(lines, [page] * len(lines), range(len(lines))))

        lines = []
        for p_line in p_lines:
            lines.append({'sentence':p_line[0], 
             'page':p_line[1], 
