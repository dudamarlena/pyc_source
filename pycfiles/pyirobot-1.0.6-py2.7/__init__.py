...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/pyirobot/__init__.py
# Compiled at: 2017-01-18 23:18:59
"""
...
import json, requests, socket, struct
...
_ErrorMessages = {1: 'Roomba is stuck with its left or right wheel hanging down.', 
...
_MissionCycleToCleaningPasses = {'quick': CleaningPasses.One, 
   'clean': CleaningPasses.Two}
_PostErrorCodes = {-32600: 'The parameter order is incorrect'}
...
class RobotError(Exception):
    ...
    def __str__(self):
        if self.message:
            return self.message
        if self.errorCode in _PostErrorCodes:
            return ('Error [{}]: {}').format(self.errorCode, _PostErrorCodes[self.errorCode])
...
class Robot(object):
    ...
    def GetPassword(robotIP):
        ...
        """
        result = requests.post(('https://{}/umi').format(robotIP), data=json.dumps(collections.OrderedDict([
         ('do', 'get'),
         (
          'args', ['passwd']),
         ('id', 0)])), headers={'Content-Type': 'application/json'}, verify=False)
        if not result.text.strip():
            raise RobotError(-1, 'Empty response from robot - did you hold the home button before calling?')
        res = result.json()
        if 'err' in res:
    ...
    def GetBLID(robotIP, password):
        ...
        result = requests.post(('https://{}/umi').format(robotIP), data=json.dumps(collections.OrderedDict([
         ...
         (
          'args', ['sys']),
         ('id', 0)])), auth=(
         'user', password), headers={'Content-Type': 'application/json'}, verify=False)
        res = result.json()
        if 'err' in res:
            raise RobotError(res['err'])
    ...
    def _PostToRobot(self, cmd, args):
        ...
        if isinstance(args, str) or not isinstance(args, collections.Iterable):
            args = [
             args]
        post_data = json.dumps(collections.OrderedDict([
         (
          'do', cmd),
         (
          'args', args),
        ...
        result = requests.post(('https://{}/umi').format(self.ip), data=post_data, auth=(
         'user', self.password), headers={'Content-Type': 'application/json'}, verify=False)
        res = result.json()
        if 'err' in res:
            raise RobotError(res['err'])
        return res['ok']
    ...
    def GetTime(self):
        ...
        return {'time': datetime.time(result['h'], result['m']), 
    ...
    def GetSchedule(self):
        ...
        for idx in range(7):
            ...
            if cal_day_idx < 0:
                cal_day_idx = 6
            schedule[calendar.day_name[cal_day_idx]] = {'clean': True if res['cycle'][idx] == 'start' else False, 'startTime': datetime.time(res['h'][idx], res['m'][idx])}
    ...
    def GetStatus(self):
        ...
        """
        return {'cleaningPreferences': self.GetCleaningPreferences(), 
