...
# [GCC 8.4.0]
# Embedded file name: /home/opt/mortoray.com/master/src/shelljob/shelljob/proc.py
# Compiled at: 2014-04-30 23:01:19
"""
        A mechanism to run subprocesses asynchronously and with non-blocking read.
"""
import atexit, multiprocessing, Queue, shlex, subprocess, sys, threading, time

class WrapException(Exception):

    def __init__(self, exc, msg):
        """
                        Assumes being created within an exception handler.
                        Try to adopt Python 3 names for the bits
                """
        self.__traceback__ = sys.exc_info()[2]
        self.__cause__ = exc
        self.message = msg
        super(WrapException, self).__init__(msg, exc)

    def do_raise(self):
        raise self, None, self.__traceback__
        return


class CommandException(WrapException):

    def __init__(self, exc, msg):
        super(CommandException, self).__init__(exc, msg)
...
class Group:
    ...
    def _run_impl(self, cmd, shell):
        cmd = _expand_cmd(cmd)
        handle = subprocess.Popen(cmd, shell=shell, bufsize=1, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, stdin=subprocess.PIPE)
...
def call(cmd, encoding='utf-8', shell=False, check_exit_code=True, timeout=None):
    """
        ...
        """
    cmd = _expand_cmd(cmd)
    proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, stdin=subprocess.PIPE, shell=shell)
