...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pre_csv/pd_csv.py
...
def read_csv(file, **kwargs):
    ...
    csv_df = pd.read_csv(file, sep=sep, header=header, na_values=na_values, usecols=usecols, parse_dates=parse_dates, converters=converters, dtypes=dtypes, **kwargs)
    return csv_df
...
def write_csv(df, **kwargs):
    ...
    df.to_csv(path_or_buf=output_path, sep=sep, header=header, index=index, encoding='utf-8')
