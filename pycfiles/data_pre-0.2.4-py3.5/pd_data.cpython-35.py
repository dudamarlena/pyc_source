...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pre/pd_data.py
...
class PdData(object):
    ...
    def df_style(df, **kwargs):
        ...
        if is_style_func:
            ...
            new_df = df.style.set_properties(**{'background-color': background_color, 
             'color': color, 
    ...
    def df_pivot(df, **kwargs):
        ...
        new_df = pd.pivot_table(df, values=values, index=index, columns=columns, fill_value=fill_value, aggfunc=aggfunc, dropna=dropna)
        return new_df
    ...
    def df_total(df, **kwargs):
        ...
        new_df = pd.concat([d.append(d.sum().rename((k, subtotal))) for k, d in df.groupby(level=group_level)]).append(df.sum().rename(total_list))
        return new_df
