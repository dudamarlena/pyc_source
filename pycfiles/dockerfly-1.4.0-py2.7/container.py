...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dockerfly/runtime/container.py
# Compiled at: 2016-07-11 02:26:51
import socket
...
def update_status(containers, key='id'):
    ...
    for curr_container in curr_containers:
        for updating_container in updating_containers:
            if updating_container.get(key, None) and curr_container.get(key, None) and updating_container[key] == curr_container[key]:
...
def remove_status(container_ids, key='id'):
    ...
    for index, container in enumerate(curr_containers):
        if container.get(key, None) not in container_ids or container.get(key, None) is None:
