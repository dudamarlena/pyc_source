...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dockerfly/dockernet/veth.py
# Compiled at: 2016-07-01 01:54:46
import random, docker as dockerpy
from sh import docker, ip
from os.path import join
from dockerfly.errors import VEthCreateException, VEthUpException, VEthDownException, VEthAttachException, VEthDeleteException
...
class MacvlanEth(VEth):
    ...
    def create(self):
        try:
            ...
            with FileLock(veth_lockfile).acquire(timeout=settings.LOCK_TIMEOUT):
                ip('link', 'add', self._veth_name, 'link', self._link_to, 'address', self.gen_mac_address(), 'type', 'macvlan', 'mode', 'bridge')
    ...
    def up(self):
        try:
            if self._is_attach_to_container:
                docker('exec', self._attach_to_container_id, 'ip', 'link', 'del', self._veth_name)
            else:
    ...
    def down(self):
        try:
            if self._is_attach_to_container:
                docker('exec', self._attach_to_container_id, 'ip', 'link', self._veth_name, 'down')
            else:
    ...
    def attach_to_container(self, container_id, is_route=False, gateway=None):
        ...
        try:
            ...
            docker('exec', self._attach_to_container_id, 'ip', 'link', 'set', self._veth_name, 'up')
            if '0.0.0.0' not in self._ip_netmask:
                docker('exec', self._attach_to_container_id, 'ip', 'addr', 'add', self._ip_netmask, 'dev', self._veth_name)
        except Exception as e:
        ...
        if is_route:
            if not gateway:
                ...
                docker('exec', self._attach_to_container_id, 'ip', 'route', 'del', 'default')
                docker('exec', self._attach_to_container_id, 'ip', 'route', 'add', 'default', 'via', gateway, 'dev', self._veth_name)
                try:
                    docker('exec', self._attach_to_container_id, 'ping', '-c', '1', gateway)
                except Exception as e:
