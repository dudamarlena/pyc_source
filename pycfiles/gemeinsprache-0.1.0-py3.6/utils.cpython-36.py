...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gemeinsprache/utils.py
# Compiled at: 2019-03-06 21:14:14
# Size of source mod 2**32: 7847 bytes
from __future__ import division
import re, bz2, fnmatch, gzip, json, os, shlex, subprocess, ast
...
def run_shell_command(cmd):
    ...
    return (
     ok, decoded, exit_code, error_msg)
...
def humanize_bytes(bytes, precision=1):
    ...
    '1.3 GB'
    """
    abbrevs = ((1125899906842624, 'PB'), (1099511627776, 'TB'), (1073741824, 'GB'),
               (1048576, 'MB'), (1024, 'kB'), (1, 'bytes'))
    if bytes == 1:
        return '1 byte'
    else:
...
class Pretty:
    ...
    def __init__(self, obj):
        ...
        try:
            self.serialized = json.dumps((self.data),
              default=(lambda x: str(x)), sort_keys=True)
        except:
            self.serialized = None

        cmd = f"""echo '{self.serialized}\' | jq "." | pygmentize -l json -O style=paraiso-dark > /tmp/pretty.out"""
        self.ok, self.stdout, self.exit_code, self.error_msg = run_shell_command(cmd)
...
class REPLEntry:
    ...
    def load_session(self):
        if self.has_session:
            if os.path.isfile(self.path_to_session):
