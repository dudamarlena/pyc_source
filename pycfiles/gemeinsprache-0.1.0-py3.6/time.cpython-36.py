...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gemeinsprache/time.py
# Compiled at: 2019-03-06 21:28:56
...
app_state = {'timestamps': {'human':'%A, %B %-d, %Y at %-I:%M%p', 
...
class Kairos:
    ...
    def __init__(self):
        config_file = os.path.join(os.path.dirname(__file__), '.kairos', 'config.json')
        ...
        if not isfile(config_file):
            ...
            reinitializing = input(green('Do you want me to create it? [y/N] '))
            if reinitializing:
                if reinitializing.strip():
                    if reinitializing.strip()[0].lower() == 'y':
        ...
        self.available_timestamps = [ts_name for ts_name, ts_format in self.data['timestamps'].items()]
        self.last_used_format = self.available_timestamps[0] if self.is_loaded else None
    ...
    def create_timestamp(self, dt, name=None):
        if name is None:
            ...
            if name in self.available_timestamps:
                self.last_used_timestamp = name
        if name not in self.available_timestamps:
            print(f"""{red("I don't have a timestamp called")} {yellow(name)}.""")
