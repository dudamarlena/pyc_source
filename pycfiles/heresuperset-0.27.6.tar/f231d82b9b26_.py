...
# [GCC 8.4.0]
# Embedded file name: /Users/moretti/PycharmProjects/incubator-superset/superset/migrations/versions/f231d82b9b26_.py
...
def upgrade():
    ...
    for table, column in names.items():
        with op.batch_alter_table(table, naming_convention=conv) as (batch_op):
            batch_op.create_unique_constraint(('uq_{}_{}').format(table, column), [
...
def downgrade():
    ...
    for table, column in names.items():
        with op.batch_alter_table(table, naming_convention=conv) as (batch_op):
            batch_op.drop_constraint(generic_find_uq_constraint_name(table, {
             column, 'datasource_id'}, insp) or ('uq_{}_{}').format(table, column), type_='unique')