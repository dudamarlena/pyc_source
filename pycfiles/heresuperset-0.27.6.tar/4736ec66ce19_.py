...
# [GCC 8.4.0]
# Embedded file name: /Users/moretti/PycharmProjects/incubator-superset/superset/migrations/versions/4736ec66ce19_.py
...
from superset.utils import generic_find_fk_constraint_name, generic_find_fk_constraint_names, generic_find_uq_constraint_name
...
def upgrade():
    ...
    for foreign in ['columns', 'metrics']:
        ...
        with op.batch_alter_table(foreign, naming_convention=conv) as (batch_op):
            ...
            for name in names:
                batch_op.drop_constraint(name or ('fk_{}_datasource_name_datasources').format(foreign), type_='foreignkey')

            batch_op.drop_column('datasource_name')

    with op.batch_alter_table('datasources', naming_convention=conv) as (batch_op):
        batch_op.drop_constraint(generic_find_uq_constraint_name('datasources', {
         'datasource_name'}, insp) or 'uq_datasources_datasource_name', type_='unique')
...
def downgrade():
    ...
    for foreign in ['columns', 'metrics']:
        ...
        with op.batch_alter_table(foreign, naming_convention=conv) as (batch_op):
            batch_op.drop_constraint(('fk_{}_datasource_id_datasources').format(foreign), type_='foreignkey')
            batch_op.drop_column('datasource_id')

    with op.batch_alter_table('datasources', naming_convention=conv) as (batch_op):
        batch_op.drop_constraint(generic_find_fk_constraint_name('datasources', {
         'cluster_name'}, 'clusters', insp) or 'fk_datasources_cluster_name_clusters', type_='foreignkey')
        batch_op.drop_constraint(generic_find_uq_constraint_name('datasources', {
         'cluster_name', 'datasource_name'}, insp) or 'uq_datasources_cluster_name', type_='unique')
