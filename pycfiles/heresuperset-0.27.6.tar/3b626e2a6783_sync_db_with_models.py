...
# [GCC 8.4.0]
# Embedded file name: /Users/moretti/PycharmProjects/incubator-superset/superset/migrations/versions/3b626e2a6783_sync_db_with_models.py
...
def upgrade():
    try:
        slices_ibfk_1 = generic_find_constraint_name(table='slices', columns={'druid_datasource_id'}, referenced='datasources', db=db) or 'slices_ibfk_1'
        slices_ibfk_2 = generic_find_constraint_name(table='slices', columns={'table_id'}, referenced='tables', db=db) or 'slices_ibfk_2'
    ...
    try:
        if not table_has_constraint('tables', '_customer_location_uc', db):
            with op.batch_alter_table('tables') as (batch_op):
                batch_op.create_unique_constraint('_customer_location_uc', [
...
def downgrade():
    ...
    try:
        fk_columns = generic_find_constraint_name(table='columns', columns={'datasource_name'}, referenced='datasources', db=db)
        with op.batch_alter_table('columns') as (batch_op):
