...
# [GCC 8.4.0]
# Embedded file name: /home/jdunham/projects/didata_cli_jdunham/didata_cli/venv/lib/python2.7/site-packages/didata_cli/commands/cmd_network.py
# Compiled at: 2016-05-02 19:35:07
import click
...
def list_vlans(client, datacenterid, networkdomainid, query):
    try:
        ...
        vlans = client.node.ex_list_vlans(location=datacenterid, network_domain=networkdomainid)
...
def delete_vlan(client, vlanid):
    try:
        client.node.ex_delete_vlan(DimensionDataVlan(vlanid, None, None, None, None, None, None, None, None, None, None, None))
...
def delete_network_domain(client, networkdomainid):
    try:
        client.node.ex_delete_network_domain(DimensionDataNetworkDomain(networkdomainid, None, None, None, None, None))
...
@click.option('--networkDomainId', type=click.UNPROCESSED, required=True, help='The network domain to apply the rule to')
...
@pass_client
def create_firewall_rule(client, name, action, networkdomainid, ipversion, protocol, sourceip, sourceip_prefix_size, sourcestartport, sourceendport, destinationip, destinationip_prefix_size, destinationstartport, destinationendport, position):
    try:
        ...
        source_address = DimensionDataFirewallAddress(source_any, sourceip, sourceip_prefix_size, sourcestartport, sourceendport, None, None)
        dest_address = DimensionDataFirewallAddress(dest_any, destinationip, destinationip_prefix_size, destinationstartport, destinationendport, None, None)
        rule = DimensionDataFirewallRule(id=None, name=name, action=action, location=network_domain.location, network_domain=network_domain, status=None, ip_version=ipversion, protocol=protocol, source=source_address, destination=dest_address, enabled=True)
...
@cli.command()
@click.option('--networkDomainId', type=click.UNPROCESSED, required=True, help='Network ID where the firewall rule lives')
...
@cli.command()
@click.option('--networkDomainId', required=True, type=click.UNPROCESSED, help='ID of the network to add the public IP block')
@pass_client
def add_public_ip_block(client, networkdomainid):
    try:
        ...
        click.secho(('Public IP Block with base IP of {0} and block size of {1} added to {2}').format(ip_block.base_ip, ip_block.size, networkdomainid), fg='green', bold=True)
    except DimensionDataAPIException as e:
        handle_dd_api_exception(e)
...
class ParseNetworkLocation(object):
    ...
    def ports(self):
        if self._location.port_begin is None:
            ports = 'ANY'
        elif self._location.port_end is None:
