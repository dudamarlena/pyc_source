...
# [GCC 8.4.0]
# Embedded file name: /home/jdunham/projects/didata_cli_jdunham/didata_cli/venv/lib/python2.7/site-packages/didata_cli/commands/cmd_server.py
# Compiled at: 2016-05-06 13:48:05
import click
...
@pass_client
def list(client, datacenterid, networkdomainid, networkid, vlanid, sourceimageid, deployed, name, state, started, ipv6, privateipv4, idsonly, query):
    node_list = client.node.list_nodes(ex_location=datacenterid, ex_name=name, ex_network=networkid, ex_network_domain=networkdomainid, ex_vlan=vlanid, ex_image=sourceimageid, ex_deployed=deployed, ex_started=started, ex_state=state, ex_ipv6=ipv6, ex_ipv4=privateipv4)
    response = DiDataCLIFilterableResponse()
...
def remove_disk(client, serverid, serverfilteripv6, diskid):
    ...
    try:
        ...
        if response is True:
            ...
            click.secho(('Something went wrong attempting to remove disk {0} from {1}').format(disk_to_remove.id, serverid), fg='red', bold=True)
            exit(1)
    except DimensionDataAPIException as e:
        handle_dd_api_exception(e)
...
def modify_disk(client, serverid, serverfilteripv6, diskid, size, speed):
    if size is not None and speed is not None:
        click.secho('Only one modify disk operation can happen at a time.  Please choose either --speed or --size', fg='red', bold=True)
        exit(1)
    else:
        if size is None and speed is None:
        ...
        try:
            ...
            if response is True:
                click.secho(('Successfully modified disk {0} from {1}').format(disk_to_modify.scsi_id, serverid), fg='green', bold=True)
            else:
                click.secho(('Something went wrong attempting to modify disk {0} from {1}').format(disk_to_modify.id, serverid), fg='red', bold=True)
                exit(1)
        except DimensionDataAPIException as e:
            handle_dd_api_exception(e)
