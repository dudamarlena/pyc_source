...
# [GCC 8.4.0]
# Embedded file name: /home/jdunham/projects/didata_cli_jdunham/didata_cli/venv/lib/python2.7/site-packages/didata_cli/commands/cmd_tag.py
# Compiled at: 2016-05-06 14:07:26
import click
...
@click.option('--displayOnReport/--no-displayOnReport', is_flag=True, default=None, help='Filter if tag key should display on report')
@pass_client
def modify_key(client, tagkeyid, name, description, valuerequired, displayonreport):
    try:
        response = client.node.ex_modify_tag_key(tagkeyid, name=name, description=description, value_required=valuerequired, display_on_report=displayonreport)
        if response is True:
...
def list_keys(client, id, name, valuerequired, displayonreport, query):
    tag_key_list = client.node.ex_list_tag_keys(id=id, name=name, value_required=valuerequired, display_on_report=displayonreport)
...
@click.option('--assetType', required=True, type=click.Choice(['SERVER', 'VLAN']), help='The type of asset to tag')
...
@click.option('--query', type=click.UNPROCESSED, help='The query to pass to the filterable response')
@pass_client
def list(client, assetid, assettype, datacenter, tagkeyid, tagkeyname, tagkeyvalue, valuerequired, displayonreport, query):
    tag_list = client.node.ex_list_tags(asset_id=assetid, asset_type=assettype, location=datacenter, tag_key_name=tagkeyname, tag_key_id=tagkeyid, value=tagkeyvalue, value_required=valuerequired, display_on_report=displayonreport)
    response = DiDataCLIFilterableResponse()
    for tag in tag_list:
