...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pumblr/api.py
# Compiled at: 2010-09-14 04:39:06
import utils, urllib2, functools
...
class API(object):
    ...
    def _check_we_ll_be_back(self, text):
    ...
    def _write(f):
        ...
        - `slug`: (optional) A custom string to appear in the post's URL
            ...
            if group is not None:
                group = group if '.' in group else '%s.tumblr.com' % group
            query = dict(email=self._email, password=self._password, generator=generator, group=group, date=date, private=private, tags=tags, slug=slug)
    ...
    def write_conversation(self, conversation, title=None):
        ...
        - `title`:(optional)
        """
        return dict(type='conversation', conversation=conversation, title=title)
    ...
    def write_video(self, embed, caption):
        ...
        - `caption`: (optional, HTML allowed)
        """
        return dict(type='video', embed=embed, caption=caption)