...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/solstice/tagger/editors/highproxyeditor.py
...
class HighProxyEditor(taggereditor.TaggerEditor, object):
    ...
    def update_tag_buttons_state(self, sel=None):
        ...
        if attr_exists:
            ...
            if proxy_group is not None:
                if tp.Dcc.object_exists(proxy_group):
        ...
        if attr_exists:
            ...
            if hires_group is not None:
                if tp.Dcc.object_exists(hires_group):
    ...
    def update_proxy_group(tag_data=None):
        ...
        if tp.Dcc.object_exists(sel_name):
            ...
            children = tp.Dcc.list_relatives(node=sel_name,
              all_hierarchy=True,
              full_path=True,
            ...
            for obj in children:
                ...
                if base_name == proxy_name:
                    if proxy_path is None:
                        ...
                        LOGGER.error('Multiple proxy groups in the asset. Asset only can have one proxy group: {}'.format(proxy_name))
                        return False
            ...
            for obj in children:
                ...
                if base_name == hires_name:
                    if hires_path is None:
                        ...
                        LOGGER.error('Multiple hires groups in the asset. Asset only can have one hires group: {}'.format(hires_name))
                        return False
