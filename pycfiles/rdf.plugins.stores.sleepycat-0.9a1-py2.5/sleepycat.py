...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/sleepycat.py
# Compiled at: 2008-04-06 11:25:27
"""A BerkleyDB backed store plugin for `rdf`.

This plugin is intended to be installed into rdf.plugins.stores.
...
from rdf.store import Store, VALID_STORE, CORRUPTED_STORE, NO_STORE, UNKNOWN
...
class Sleepycat(Store):
    ...
    def open(self, path, create=True):
        ...
        for i in xrange(0, 8):
            ...
            lookup[i] = (
             self.__indicies[start], get_prefix_func(start, start + len), from_key_func(start), results_from_key_func(start, self._from_string))
    ...
    def bind(self, prefix, namespace):
        ...
        namespace = self._to_string(namespace)
...
def results_from_key_func(i, from_string):
    ...
    def from_key(key, subject, predicate, object, contexts_value):
        ...
        return (
         (
          s, p, o), (from_string(c) for c in contexts_value.split('^') if c))
