...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/django_debug_html_store/views.py
# Compiled at: 2013-02-23 01:27:41
"""
...
def read_response(request, **kwargs):
    ...
    if not getattr(settings, 'DEBUG', False):
        ...
        if debug_store_type not in ('file', 'cache'):
            raise Warning('DEBUG_STORE_HTML_STORE_TYPE should be file or cache')
        if debug_store_type == 'file':
            fname = ('{}_{}').format(getattr(settings, 'DEBUG_STORE_FILE_PREFIX', 'debug_store'), ip_addr.replace('.', '_'))
            try:
                store_response = open(fname, 'r').read()
            except Exception as e:
                error_res = (
                 'django_debug_html_store Error: %s' % e,)
