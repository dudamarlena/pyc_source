        ...
        for k, v in kwargs.items():
            if k in self.ALLOWED_OPTIONS:
                if k in PlotOptions.ALLOWED_OPTIONS.keys():
                    ...
                    if isinstance(self.ALLOWED_OPTIONS[k][0](), ArrayObject):
                        if self.__getattr__(k):
                            if isinstance(v, dict):
                                self.__dict__[k].update(v)
                            else:
                                if isinstance(v, list):
                                    ...
                                    OptionTypeError('Not An Accepted Input Type: %s, must be list or dictionary' % type(v))
                        else:
                            if isinstance(v, dict):
                                self.__dict__.update({k: (self.ALLOWED_OPTIONS[k][0])(**v)})
                            else:
                    ...
                    if isinstance(self.ALLOWED_OPTIONS[k][0](), CSSObject) or isinstance(self.ALLOWED_OPTIONS[k][0](), SVGObject):
                        ...
                        if isinstance(v, dict):
                            ...
                            self.__dict__.update({k: self.ALLOWED_OPTIONS[k][0](v)})
                elif isinstance(self.ALLOWED_OPTIONS[k], tuple) and (isinstance(self.ALLOWED_OPTIONS[k][0](), JSfunction) or isinstance(self.ALLOWED_OPTIONS[k][0](), Formatter) or isinstance(self.ALLOWED_OPTIONS[k][0](), ColorObject)):
                    if isinstance(v, dict):
                        self.__dict__.update({k: (self.ALLOWED_OPTIONS[k][0])(**v)})
                    else:
                        self.__dict__.update({k: self.ALLOWED_OPTIONS[k][0](v)})
