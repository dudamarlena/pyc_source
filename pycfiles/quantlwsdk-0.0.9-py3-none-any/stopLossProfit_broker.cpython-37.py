...
# [GCC 8.4.0]
# Embedded file name: ..\indicatorModule\pyalgotrade\broker\stopLossProfit_broker.py
# Compiled at: 2019-11-15 10:29:52
# Size of source mod 2**32: 15220 bytes
"""
...
class SimulationStopLossProfitBroker:
    ...
    def _create_clear_order_fromGmOrders(self, gmOrderObj):
        tradeo = tradeOrder(gmOrderObj.cl_ord_id, gmOrderObj.symbol, gmOrderObj.volume, gmOrderObj.side, ORDER_TYPE.MARKET, gmOrderObj.position_effect, 'sbname')
    ...
    def _create_trade_order_fromGmOrders(self, gmOrderObj, signalName):
        tradeo = tradeOrder(gmOrderObj.cl_ord_id, gmOrderObj.symbol, gmOrderObj.volume, gmOrderObj.side, ORDER_TYPE.MARKET, gmOrderObj.position_effect, signalName)
        if gmOrderObj.status == 3:
            tradeo.gm_filled_volume = gmOrderObj.filled_volume
            tradeo.gm_filled_vwap = gmOrderObj.filled_vwap
    ...
    def create_stop_order(self, aOrderPosition, stopThresh, stopCommand='stoploss'):
        ...
        orderLog = loggerHelpbylw.getFileLogger((self.bTestID + '-' + underLySym + '-orderlog'), ('log\\' + self.bTestID + '\\' + underLySym + '-orderRecord.txt'),
          mode_='a')
