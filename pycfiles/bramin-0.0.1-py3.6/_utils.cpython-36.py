...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bramin/_utils.py
# Compiled at: 2019-12-24 06:55:37
...
class SpecialMethods(object):
    __doc__ = "Traversal object's special methods.\n    see: https://docs.python.org/3/reference/datamodel.html#special-method-names\n    "
    groups = {'basic':{'initial':[
       'new', 'init'], 
      'format':[
     ...
     'attribute_access':{'attr':[
      ...
      'other':[
       'slots']}, 
     'class_creation':[
      'init_subclass', 'mro_entries', 'prepare'], 
     'check':[
    ...
    def __init__(self, types: Optional[Iterable[str]]=None, filter_func: Optional[Callable[([str, str], bool)]]=None, with_underline: bool=False, with_impl: bool=False, grp_sep: str='/'):
...
def is_partial_like(func: Callable):
    ...
    return hasattr(func, 'func') and hasattr(func, 'args') and hasattr(func, 'keywords') and isinstance(func.args, tuple)


def replace_partial_args(func: Callable, target: Any, repl: Any, match_func: Optional[Callable]=None, replace_func: Optional[Callable]=None) -> Callable:
    """Replace partial-like object's argument,
    return a new callable"""
    if match_func is None:

        def match_func(i, v, t):
            return v is t

    if replace_func is None:

        def replace_func(v, r):
            return r
...
class Singleton(type):
    __doc__ = 'Let class has only one instance.'
...
def type_error(func_name, expect_tp, got_tp, arg_name=None, ret=False) -> TypeError:
...
class type_guard(object):
    ...
    def _check_input_args(self, *args, **kwargs):
        ...
        for name, value in bound_values.arguments.items():
            if name not in self._input_checkers:
                pass
            else:
        ...
        if self.check_ret:
            if self._output_checker:
                if not self._output_checker(res):
                    raise type_error((self._func_name), expect_tp, (type(res)),
                      ret=True)
    ...
    def _dispatch_checkers(self):
        ...
        for name, p in sig.parameters.items():
            ...
            if expect_tp is inspect._empty:
                continue
    ...
    def _get_checker(self, expect_tp):
        if expect_tp is t.Any:
            ...
            if ti.is_union_type(expect_tp):
                return lambda v: isinstance(v, expect_tp.__args__)
            else:
                if ti.is_callable_type(expect_tp):
                    return lambda v: callable(v)
                if isinstance(expect_tp, str):
    ...
    def _get_str_checker(cls, expect_tp):
        ...
        def checker(v):
            ...
            if not args or expect_tp != type(args[0]).__name__:
                raise TypeError('string annotation can only be used for annotate class it self.')
            tp = type(args[0])
