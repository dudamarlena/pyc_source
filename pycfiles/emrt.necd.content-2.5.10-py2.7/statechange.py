...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/emrt/necd/content/browser/statechange.py
...
RE_EXTRACT_SECTOR_COUNTRY = re.compile('(sector\\d)?-([a-z]{2})$')
...
def filter_groups_for_context(context, names):
    validate_group = partial(group_matches_sector_and_country, pass_sector=context.ghg_source_category_value(), pass_country=context.country)
...
class FinishObservationReasonForm(Form):
    ...
    def finish_observation(self, action):
        ...
        with api.env.adopt_roles(['Manager']):
            ...
            return self.context.content_status_modify(workflow_action='finish-observation')
    ...
    def finish_observation(self, action):
        ...
        with api.env.adopt_roles(['Manager']):
            if api.content.get_state(self.context) == 'close-requested':
                ...
                return self.context.content_status_modify(workflow_action='deny-finishing-observation')
...
class RecallObservation(BrowserView):
    ...
    def __call__(self):
        ...
        with api.env.adopt_roles(['Manager']):
            return self.context.content_status_modify(workflow_action='recall-lr')
        return self.response.redirect(self.context.absolute_url())
...
class AssignFormMixin(BrowserView):
    ...
    def show_test_users(self):
        ...
        return any(usr in user.getUserName() for usr in TEST_USERS) or 'Manager' in user.getRoles()
    ...
    def _is_managed_role(self, username):
        return self._managed_role in api.user.get_roles(username=username, obj=self._assignation_target(), inherit=False)
            ...
            for userId, username, cp in self.get_counterpart_users(False):
                if cp:
                    revoke_roles(username=userId, obj=target, roles=[
                     self._managed_role], inherit=False)
    ...
    def get_counterpart_users(self, exclude_test=True):
        ...
        groups = filter(bool, _groups)
        for res in map(self.get_users_from_group, groups):
            matched = [ (user_id, user_name, self._is_managed_role(user_id)) for user_id, user_name in res if user_id != current_user_id
                      ]
...
class AssignAnswererForm(AssignFormMixin):
    ...
    _msg_no_usernames = _('You need to select at least one expert for discussion')
    ...
    def _get_wf_action(self):
        if api.content.get_state(self.context) in ('pending', 'recalled-msa', 'pending-answer-drafting'):
...
class AssignCounterPartForm(AssignFormMixin):
    ...
    def _target_groupnames(self):
        ...
        validated_groups = filter_groups_for_context(self.context, from_reviewfolder)
    ...
    def get_current_counterparters(self):
        ...
        return [ api.user.get(user) for user in users if user not in skipped_extra_users
               ]
...
class ReAssignCounterPartForm(AssignCounterPartForm):
    ...
    def __call__(self):
        ...
        if self.request.form.get('send', None):
            ...
            url = self.context.absolute_url()
            question_to_counterpart.notification_cp(self.context, reassign=True)
            return self.request.response.redirect(url)
        else:
            return self.index()
            return
...
class AssignConclusionReviewerForm(AssignFormMixin):
    ...
    _msg_no_usernames = _('You need to select at least one reviewer for conclusions')
