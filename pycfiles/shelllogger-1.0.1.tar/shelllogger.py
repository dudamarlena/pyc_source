...
# [GCC 8.4.0]
# Embedded file name: /Users/lorin/shelllogger/trunk/sl/shelllogger.py
# Compiled at: 2011-10-11 09:46:46
r"""
...
"""
import fcntl, sys, os, pty, re, select, signal, socket, struct, termios, time, errno, codecs, datetime
isFirst = True
try:
    True
except NameError:
...
def sanitize(buf, backspaces=[
 '\x08\x1b[K', '\x08 \x08'], escape_regex=re.compile('\\x1b(\\[|\\]|\\(|\\))[;?0-9]*[0-9A-Za-z](.*\\x07)?')):
...
class TTY:
    ...
    def __init__(self):
        self.iflag, self.oflag, self.cflag, self.lflag, self.ispeed, self.ospeed, self.cc = termios.tcgetattr(0)
...
re_prompt = re.compile('(.*)^\\[SL (.*)\\]\\$ $', re.MULTILINE | re.DOTALL | re.IGNORECASE)
mac_prompt = re.compile(re.compile('(?:.*)\\[SL (.*)\\](.*)(\\$)?', re.MULTILINE | re.DOTALL | re.IGNORECASE))
linux_prompt = re.compile('(?:.*)\\[SL (.*)\\]\\$', re.MULTILINE | re.DOTALL | re.IGNORECASE)
...
class UserTypingState:
    ...
    def input_from_shell(self, buf):
        if buf.startswith('\r') or buf.startswith('\r'):
