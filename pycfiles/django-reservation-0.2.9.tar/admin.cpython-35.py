...
# [GCC 8.4.0]
# Embedded file name: /home/luisza/Escritorio/desarrollo/djreservation/djreservation/admin.py
...
class ReservationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
     (
      None,
      {'fields': (('user', 'status', 'updated_datetime'), ('reserved_start_date', 'reserved_end_date'),
 'list_of_products')}),)

    def get_readonly_fields(self, request, obj=None):
        readonly = admin.ModelAdmin.get_readonly_fields(self, request, obj=obj)
        if obj and obj.status == obj.RETURNED:
            readonly = ('user', 'status', 'updated_datetime', 'reserved_start_date',
                        'reserved_end_date', 'list_of_products')
        return readonly
    ...
    def list_of_products(self, instance):
        return mark_safe(render_to_string('djreservation/product_admin_reservation.html', {'instance': instance}))
    ...
    def save_model(self, request, obj, form, change):
        ...
        if change:
            old_status = obj.__class__.objects.filter(pk=obj.pk).values_list('status')[0][0]
        ...
        if 'djreservation_product_list' in request.POST:
            ...
            old_pks = list(map(lambda x: str(x[0]), obj.product_set.all().filter(borrowed=True).values_list('pk')))
            differ_obj = different(product_pks, old_pks)
            if any(differ_obj):
                obj.product_set.all().exclude(pk__in=product_pks).update(borrowed=False)
                obj.product_set.all().filter(pk__in=product_pks).update(borrowed=True)
                product_change = True
