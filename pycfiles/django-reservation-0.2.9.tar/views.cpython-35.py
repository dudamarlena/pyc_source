...
# [GCC 8.4.0]
# Embedded file name: /home/luisza/Escritorio/desarrollo/djreservation/djreservation/views.py
# Compiled at: 2019-02-20 00:29:40
# Size of source mod 2**32: 9726 bytes
...
class ReservationList(ListView):
    ...
    def get_queryset(self):
        ...
        queryset = queryset.filter(user=self.request.user).exclude(status=Reservation.BUILDING).order_by('-updated_datetime', 'status')
        return queryset
...
class CreateReservation(CreateView):
    ...
    def form_valid(self, form):
        ...
        self.object.save()
        if TOKENIZE:
            ReservationToken.objects.create(reservation=self.object, base_url=get_base_url(self.request))
...
def finish_reservation(request):
    ...
    if request.method == 'GET':
        response = render(request, 'djreservation/reservation_confirm.html', {'reservation': request.reservation})
        ...
        response = render(request, 'djreservation/reservation_finished.html')
...
class SimpleProductReservation(CreateView):
    ...
    def get_product_form(self, instance, post_context=False):
        ...
        if self.max_amount_field and hasattr(instance, self.max_amount_field):
            self.product_form.initial['available_amount'] = getattr(instance, self.max_amount_field)
            if post_context:
                self.product_form.fields['available_amount'].value = getattr(instance, self.max_amount_field)
    ...
    def get_form_kwargs(self):
        ...
        if START_RESERVATION_DATETIME:
            kwargs['initial']['reserved_start_date'] = START_RESERVATION_DATETIME
        return kwargs
    ...
    def form_valid(self, form):
        ...
        reservation.status = Reservation.REQUESTED
        reservation.save()
        self.object = Product(amount=self.product_form.cleaned_data['amount'], amount_field=self.amount_field, reservation=reservation, content_object=self.base_instance)
        self.object.save()
        if TOKENIZE:
            ReservationToken.objects.create(reservation=reservation, base_url=get_base_url(self.request))
    ...
    def get(self, request, *args, **kwargs):
        ...
        return super(SimpleProductReservation, self).get(request, *args, **kwargs)
...
class ProductReservationView(CreateView):
    ...
    def get_extra_fields(self):
        ...
        for field in self.extra_display_field:
            ...
            extra_fields.append({'label': self.instance._meta.get_field(field).verbose_name, 
    ...
    def form_valid(self, form):
        self.object = Product(amount=form.cleaned_data['amount'], amount_field=self.amount_field, reservation=self.request.reservation, content_object=self.instance)
        self.object.save()
    ...
    def post(self, request, *args, **kwargs):
        if 'modelpk' in kwargs:
            ...
            self.instance = self.base_model.objects.get(pk=request.POST.get('model_instance'))
        return CreateView.post(self, request, *args, **kwargs)
...
def update_reservation_by_token(request, pk, token, status):
    token_reservation = get_object_or_404(ReservationToken, reservation=pk, token=token)
