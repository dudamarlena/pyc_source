...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/healsparse/visu_func.py
# Compiled at: 2019-06-18 09:26:52
# Size of source mod 2**32: 9675 bytes
...
def get_projection(projection_name, central_lon=0):
    ...
    if have_cartopy:
        ...
        for obj_name, o in vars(cartopy.crs).copy().items():
            if isinstance(o, type) and issubclass(o, cartopy.crs.Projection) and not obj_name.startswith('_'):
                if obj_name not in ('Projection', ):
...
def find_side(ls, side):
    ...
    points = {'left':[(minx, miny), (minx, maxy)],  'right':[
      (
       maxx, miny), (maxx, maxy)], 
     'bottom':[
      (
...
def hsp_view_map(HealSparseMap, projection='Robinson', show_coverage=True, central_lon=0, extent=[], cmap_name='viridis', figsize=(10, 5), savename=None, title=None, colorlabel=None, vmin=None, vmax=None, nx=10, ny=10, **kwargs):
    """
    ...
    **kwargs: `**kwargs`, arguments for `matplotlib.pyplot.subplots`
    ...
    if (len(extent) > 1) & (len(extent) != 4):
        raise ValueError('If extent is specified it needs to have the format             ra_min, ra_max, dec_min, dec_max')
    else:
        if len(extent) == 4:
            ...
            ax.set_global()
    corner_ra, corner_dec = hp.vec2ang((np.transpose(hp.boundaries((HealSparseMap.nsideSparse), (HealSparseMap.validPixels), nest=True),
      axes=[0, 2, 1]).reshape(-1, 3)),
      lonlat=True)
    ...
    if vmin is None:
        ...
        if have_cartopy:
            polycoll = PolyCollection(uv_verts, edgecolor='none', array=data, norm=norm,
    ...
    if projection in ('PlateCarree', 'Mercator'):
        ...
        if show_coverage:
            plt.axes(ax_cov)
            hp.mollview((HealSparseMap.coverageMask), title='Coverage Mask', hold=True)
