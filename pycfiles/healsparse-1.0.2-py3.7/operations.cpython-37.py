...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/healsparse/operations.py
# Compiled at: 2020-04-09 13:59:38
# Size of source mod 2**32: 13163 bytes
...
def _apply_operation(map_list, func, filler_value, union=False, int_only=False):
    ...
    if len(map_list) < 2:
        ...
        for m in map_list:
            ...
            if not nside_coverage != m._nside_coverage:
                if nside_sparse != m._nside_sparse:
            ...
            for m in map_list:
                ...
                if is_wide_mask:
                    ...
                    for i in range(wide_mask_width):
                        combined_sparse_map[(combined_pixel_index, i)] = func(combined_sparse_map[(combined_pixel_index, i)], values[:, i])

                else:
                    combined_sparse_map[combined_pixel_index] = func(combined_sparse_map[combined_pixel_index], m.get_values_pix(m_valid_pixels))
                if union:
    ...
    return HealSparseMap(cov_index_map=cov_index_map, sparse_map=combined_sparse_map, nside_sparse=nside_sparse,
      sentinel=sentinel)