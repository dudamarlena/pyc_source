...
# [GCC 8.4.0]
# Embedded file name: domain_models/collections.py
# Compiled at: 2016-04-12 17:21:44
"""Collections module."""
import six
...
class Collection(list):
    ...
    def insert(self, index, value):
        ...
        return super(Collection, self).insert(index, self._ensure_value_is_valid(value))

    def __setitem__(self, index, value):
        """Set an item at a given position."""
        if isinstance(index, slice):
            return super(Collection, self).__setitem__(index, self.__class__(value))
        else:
            return super(Collection, self).__setitem__(index, self._ensure_value_is_valid(value))
    ...
    if six.PY2:
        ...
        def __getslice__(self, start, stop):
            ...
            return self.__class__(super(Collection, self).__getslice__(start, stop), type_check=False)

        def __setslice__(self, start, stop, iterable):
            """Set slice of values."""
            super(Collection, self).__setslice__(start, stop, self.__class__(iterable))
    ...
    def _ensure_value_is_valid(self, value):
        ...
        if not isinstance(value, self.__class__.value_type):
            raise TypeError(('{0} is not valid collection value, instance of {1} required').format(value, self.__class__.value_type))
        return value