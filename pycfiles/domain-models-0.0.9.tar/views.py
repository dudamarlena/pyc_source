...
# [GCC 8.4.0]
# Embedded file name: domain_models/views.py
# Compiled at: 2016-10-25 17:59:40
...
class ContextViewMetaClass(type):
    ...
    def check_model_cls(attributes):
        ...
        if not issubclass(model_cls, models.DomainModel):
            raise TypeError('Attribute __model_cls__ must be subclass of DomainModel.')
        return
    ...
    def get_prepared_include_exclude(attributes):
        ...
        for attr in ('__include__', '__exclude__'):
            attrs[attr] = tuple([ item.name for item in attributes.get(attr, tuple())
                                ])

        return (
         attrs['__include__'], attrs['__exclude__'])
    ...
    def check_include_exclude(attributes):
        ...
        if all((not include, not exclude)):
            ...
            if all((include, exclude)):
                raise AttributeError('Usage of __include__ and __exclude__ at the same time is prohibited.')
            return
    ...
    def get_properties(attributes):
        ...
        :rtype: list
        """
        return [ key for key, value in six.iteritems(attributes) if isinstance(value, property)
               ]
    ...
    def check_properties(mcs, attributes):
        ...
        intersections = list(set(properties).intersection(include if include else exclude))
        if not intersections:
            return
        else:
            attr_name = '__include__' if include else '__exclude__'
            raise AttributeError(('It is not allowed to mention already defined properties: {0} in {1} attributes.').format((', ').join(intersections), attr_name))
            return
...
class ContextView(object):
    ...
    def __init__(self, model):
        ...
        if not isinstance(model, self.__model_cls__):
            raise TypeError(('"{0}" is not an instance of {1}').format(model, self.__model_cls__))
        self._model = model
        if self.__include__:
