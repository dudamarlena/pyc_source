...
# [GCC 8.4.0]
# Embedded file name: domain_models/models.py
# Compiled at: 2016-10-06 15:13:04
"""Models module."""
from __future__ import absolute_import
import collections as std_collections, six
from . import fields
...
class DomainModelMetaClass(type):
    ...
    def __new__(mcs, class_name, bases, attributes):
        ...
        cls.__unique_key__ = mcs.prepare_fields_attribute(attribute_name='__unique_key__', attributes=attributes, class_name=class_name)
        cls.__view_key__ = mcs.prepare_fields_attribute(attribute_name='__view_key__', attributes=attributes, class_name=class_name)
    ...
    def prepare_fields_attribute(attribute_name, attributes, class_name):
        ...
        if not attribute:
            ...
            raise errors.Error('{0}.{1} is supposed to be a list of {2}, instead {3} given', class_name, attribute_name, fields.Field, attribute)
        return attribute

    @staticmethod
    def bind_fields_to_model_cls(cls, model_fields):
        """Bind fields to model class."""
        return dict((field.name, field.bind_model_cls(cls)) for field in model_fields)
...
class DomainModel(object):
    ...
    def __hash__(self):
        ...
        if self.__class__.__unique_key__:
            return hash(tuple(field.get_value(self) for field in self.__class__.__unique_key__))
        return super(DomainModel, self).__hash__()

    def __repr__(self):
        """Return Pythonic representation of domain model."""
        return ('{module}.{cls}({fields_values})').format(module=self.__class__.__module__, cls=self.__class__.__name__, fields_values=(', ').join(('=').join((name, repr(field.get_value(self)))) for name, field in six.iteritems(self.__class__.__fields__)))

    def __str__(self):
        """Return string representation of domain model."""
        if not self.__class__.__view_key__:
            return self.__repr__()
        return ('{module}.{cls}({fields_values})').format(module=self.__class__.__module__, cls=self.__class__.__name__, fields_values=(', ').join(('=').join((field.name, str(field.get_value(self)))) for field in self.__class__.__view_key__))
    ...
    def get_data(self):
        ...
        :rtype: dict
        """
        return dict((name, field.get_builtin_type(self)) for name, field in six.iteritems(self.__class__.__fields__))
