...
# [GCC 8.4.0]
# Embedded file name: /Users/eytan/Envs/docstring/lib/python2.7/site-packages/docstring/utils.py
# Compiled at: 2012-11-28 17:37:25
import os, re, urllib
...
def _parse_params(url):
    ...
    params = dict([ part.split('=') for part in url_parts[4].split('&') ]) if url_parts[4] else {}
    return params
...
def _append_params(url, params):
    ...
    if isinstance(params, dict):
        params = params.items()
    url_parts = list(urlparse(url))
    query = [ part.split('=') for part in url_parts[4].split('&') ] if url_parts[4] else []
    params = query + params
    to_append = []
    for key, value in params:
        if type(value) == unicode:
            value = value.encode('utf-8')
        if key not in [ k for k, _ in to_append ]:
...
class Endpoint(object):
    ...
    def _get_path(self, request_path=None, clean=False, params=None):
        ...
        if use_relative:
            if not clean:
                return _append_params('./', params)
            return './'
        path = self._mount_regex.replace('$', '').replace('.*?', '').replace('.*', '')
        if path.startswith('/'):
            path = path[1:]
        path = './' + path
        if not clean:
    ...
    def get_link_path(self, request_path, clean=False, params=None):
        ...
        """
        params = params or {}
        link_path = self._get_path(request_path=request_path, clean=clean, params=params)
        return link_path
    ...
    def get_display_path(self, request_path, clean=False, params=None):
        ...
        """
        params = params or {}
        display_path = self._get_path(request_path=request_path, clean=clean, params=params)
        return display_path
    ...
    def get_doc(self, request_path, request_params):
        ...
        if not self._docstring:
            return ''
        link_path = self.get_link_path(request_path, params=dict(request_params, doc=1))
...
def get_api_doc(endpoints, title, request_url, is_root=False):
    ...
    out = base_template % {'title': title or 'no title', 
       'rows': rows, 
       'style': style, 
       'back': back}
    return out