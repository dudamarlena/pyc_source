...
# [GCC 8.4.0]
# Embedded file name: /home/Okada/github/paplot/scripts/paplot/prep.py
...
def copy_dir_style(dst, config):
    import paplot.subcode.tools as tools, shutil, os, glob
...
def _load_metadata(output_dir, output_html, project_name, name, overview, sub_text, composite, exists):
    import json
    arg_data = {'proj': project_name, 'graphs': [
...
def create_index(config, output_dir, output_html, project_name, name, overview='', sub_text='', composite=False, remarks=''):
    import paplot.subcode.tools as tools, os
    ...
    f_html.write(html_template.format(version=version_text(), date=tools.now_string(), remarks=remarks, link=link_text))
    f_html.close()
