...
# [GCC 8.4.0]
# Embedded file name: c:\cygwin\home\okada\github\paplot\scripts\paplot\mutation.py
# Compiled at: 2017-11-21 04:12:28
# Size of source mod 2**32: 17156 bytes
"""
...
def genes_list(colmun, colmun_f, colmun_id, funcs, id_list, config):
    import paplot.subcode.tools as tools, paplot.convert as convert
    sept = tools.config_getstr(config, 'result_format_mutation', 'sept_gene')
    genes_di = {}
    ids_di = {}
    for i in range(len(colmun)):
        if (colmun_f[i] in funcs) == False:
            pass
        else:
            ...
            for gene in splt:
                if gene == '':
                    ...
                    if gene in ids_di.keys():
                        if (colmun_id[i] in ids_di[gene]) == True:
                            continue
                        else:
...
def convert_tojs(input_file, output_file, positions, config):
    import os, paplot.subcode.data_frame as data_frame, paplot.subcode.merge as merge, paplot.subcode.tools as tools, paplot.convert as convert
    cols_di = merge.position_to_dict(positions)
    try:
        df = data_frame.load_file(input_file, header=1, sept=tools.config_getstr(config, 'result_format_mutation', 'sept'), comment=tools.config_getstr(config, 'result_format_mutation', 'comment'))
    ...
    id_list.sort()
    genes = genes_list(df.column(cols_di['gene']), df.column(cols_di['group']), df.column(cols_di['id']), funcs, id_list, config)
    ...
    for row in df.data:
        ...
        if iid == '':
            ...
            if (iid in mutations) == False:
                mutations[iid] = {}
                tooltips[iid] = {}
            func_split = convert.text_to_list(row[df.name_to_index(cols_di['group'])], tools.config_getstr(config, 'result_format_mutation', 'sept_group'))
            tooltip_items = []
            ...
            for func in func_split:
                if (func in mutations[iid]) == False:
                    mutations[iid][func] = {}
                    tooltips[iid][func] = {}
                gene_split = convert.text_to_list(row[df.name_to_index(cols_di['gene'])], tools.config_getstr(config, 'result_format_mutation', 'sept_gene'))
    ...
    for sec in config.sections():
        if sec.startswith('mutation_subplot_type1_'):
            ...
            continue
        f.write(subdata_template.format(name=name, title=title, type=tools.config_getstr(config, sec, 'mode'), item=convert.list_to_text(item), label=convert.list_to_text(label), colors_n=convert.list_to_text(colors_n), data=data_text))
...
def load_subdata(ids, sec, config):
    import os, paplot.subcode.tools as tools, paplot.convert as convert, paplot.color as color
...
def create_html(dataset, output_di, config):
    ...
    f_html.write(html_template.format(project=output_di['project'], title=output_di['title'], data_js=output_di['js'], version=prep.version_text(), date=tools.now_string(), sub1=sub1_text, sub2=sub2_text, set_sub_add=set_sub_add_text, set_sub_select=set_sub_select_text, style='../style/%s' % os.path.basename(tools.config_getpath(config, 'style', 'path', 'default.js'))))
    f_html.close()