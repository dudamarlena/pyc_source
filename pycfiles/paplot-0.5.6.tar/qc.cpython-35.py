...
# [GCC 8.4.0]
# Embedded file name: c:\cygwin\home\okada\github\paplot\scripts\paplot\qc.py
...
def convert_tojs(input_file, output_file, positions, config):
    import os, paplot.subcode.data_frame as data_frame, paplot.subcode.merge as merge, paplot.subcode.tools as tools, paplot.convert as convert, paplot.color as color
    cols_di = merge.position_to_dict(positions)
    try:
        df = data_frame.load_file(input_file, header=1, sept=tools.config_getstr(config, 'result_format_qc', 'sept'), comment=tools.config_getstr(config, 'result_format_qc', 'comment'))
    ...
    f.write(js_header)
    f.write(js_dataset.format(IDs=convert.list_to_text(id_list), header=convert.list_to_text(headers), plots=plots_text))
    f.write(js_data1)
    for row in df.data:
        iid = row[df.name_to_index(cols_di['id'])]
        if iid == '':
...
def create_html(dataset, output_di, config):
    import paplot.subcode.tools as tools, paplot.prep as prep, os
    chart_txt = ''
    js_txt = ''
    ...
    f_html.write(html_template.format(project=output_di['project'], title=output_di['title'], data_js=output_di['js'], version=prep.version_text(), date=tools.now_string(), charts=chart_txt, js_add_div=js_txt, style='../style/%s' % os.path.basename(tools.config_getpath(config, 'style', 'path', 'default.js'))))
    f_html.close()