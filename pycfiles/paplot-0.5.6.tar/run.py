...
# [GCC 8.4.0]
# Embedded file name: /home/Okada/github/paplot/scripts/paplot/run.py
...
"""
import os, paplot.subcode.tools as tools, paplot.subcode.merge as merge, paplot.prep as prep
...
def qc_main(args):
    ...
    params_html = {'dir': output_html_dir, 'data': output_data, 
       'js': 'data_%s.js' % args.ellipsis, 
       'html': html_name, 
       'project': args.project_name, 
       'title': args.title}
    qc.output_html(params_html, positions, config)
    prep.create_index(config, tools.win_to_unix(args.output_dir), html_name, args.project_name, args.title, overview=args.overview, remarks=args.remarks)
...
def signature_main(args):
    ...
    if len(input_list) == 0:
        ...
        for input_file in input_list:
            params_in = {'dir': output_html_dir, 'data': input_file, 
               'project': args.project_name, 
               'title': args.title, 
            ...
            if params_out == None:
                continue
            if params_out == {}:
                prep.create_index(config, tools.win_to_unix(args.output_dir), '', args.project_name, args.title, overview=args.overview, sub_text='', composite=True, remarks=args.remarks)
                continue
            sig_num_sift = 0
            if tools.config_getboolean(config, 'result_format_signature', 'background'):
                sig_num_sift = 1
            prep.create_index(config, tools.win_to_unix(args.output_dir), params_out['html'], args.project_name, args.title, overview=args.overview, sub_text='#sig %d' % (params_out['sig_num'] + sig_num_sift), composite=True, remarks=args.remarks)
