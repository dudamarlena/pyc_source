...
# [GCC 8.4.0]
# Embedded file name: /home/byt3bl33d3r/.virtualenvs/CME_old/lib/python2.7/site-packages/cme/enum/rpcquery.py
...
class RPCQUERY:
    ...
    def connect(self, service):
        if service == 'wkssvc':
            stringBinding = ('ncacn_np:{}[\\PIPE\\wkssvc]').format(self.host)
        elif service == 'srvsvc':
            stringBinding = ('ncacn_np:{}[\\PIPE\\srvsvc]').format(self.host)
    ...
    def enum_lusers(self):
        ...
        for user in lusers:
            self.logger.highlight(('Username: {}\\{} {}').format(user['wkui1_logon_domain'], user['wkui1_username'], ('LogonServer: {}').format(user['wkui1_logon_server']) if user['wkui1_logon_server'] != '\x00' else ''))
        ...
        for session in sessions:
            if level == 502:
                if session['sesi502_cname'][:-1] != self.local_ip:
                    self.logger.highlight(('\\\\{} {} [opens:{} time:{} idle:{}]').format(session['sesi502_cname'], session['sesi502_username'], session['sesi502_num_opens'], session['sesi502_time'], session['sesi502_idle_time']))
            elif level == 0:
                if session['sesi0_cname'][:-1] != self.local_ip:
                    self.logger.highlight(('\\\\{}').format(session['sesi0_cname']))
