...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/coccigrep/coccigrep.py
# Compiled at: 2014-02-10 05:25:41
...
_CONF_FILES = tuple(path.join(dirname, name_format % 'coccigrep') for dirname, name_format in (
 (
...
def _raise_run_err(err, cmd):
    if err.errno in (errno.ENOENT, errno.ENOEXEC):
        raise CocciConfigException("Unable to run spatch command '%s': %s." % (
         cmd[0], err.strerror))
...
class CocciPatch:
    ...
    """
    keywords = [
     'Name', 'Author', 'Desc', 'Confidence', 'File', 'Revision', 'Arguments']
...
class CocciGrep:
    ...
    def run(self, files):
        ...
        cocci_file.close()
        cocci_smpl = cocci_smpl_tmpl.substitute(type=self.type, attribute=self.attribute, cocci_regexp_equal=cocci_op)
    ...
    def display(self, mode='raw', before=0, after=0, oformat='term'):
        ...
        output = ('').join(match.display(self.type, mode=mode, oformat=oformat) for match in self.matches)
