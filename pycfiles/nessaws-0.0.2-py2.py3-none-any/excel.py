...
# Compiled at: 2017-08-15 11:56:24
...
def write_excel_output(results):
    ...
    for scan in results['scans']:
        ...
        scan_targets = WriteOnlyCell(summary_sheet, value=(', ').join(scan['target_names']))
    ...
    for scan in results['scans']:
        if scan['status'] == 'completed' and scan.get('result_file') is not None:
            ...
            try:
                ...
                logger.warning(('Could not find result file "{}" for scan name "{}", these results will not be included in the report').format(result_filename, scan['scan_name']))
