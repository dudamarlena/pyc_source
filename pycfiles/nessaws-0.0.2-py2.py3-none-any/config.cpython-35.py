...
# [GCC 8.4.0]
# Embedded file name: /Users/adamstauffer/Desktop/TerbiumCode/Github/nessaws/src/nessaws/config.py
# Compiled at: 2017-06-27 11:23:39
# Size of source mod 2**32: 5895 bytes
"""Configuration file parsing and data validation."""
import logging, sys, botocore.session, colander
    ...
    try:
        with open('.nessaws_state', 'r') as (state_file):
            try:
                ...
                logger.critical('State file could not be parsed! ({})'.format(e))
                sys.exit(-1)
        ...
        logger.critical('State could not be loaded, did you forget to run "pentest-request"?'.format(e))
        sys.exit(-1)
...
def parse_config(location):
    ...
    with open(location, 'r') as (config_file):
        try:
            ...
            logger.critical('Configuration file could not be parsed! ({})'.format(e))
            sys.exit(-1)
...
class AWSAccount(colander.MappingSchema):
    __doc__ = 'Colander Mapping Schema for AWS Account configuration.'
    aws_access_key_id = colander.SchemaNode(colander.String(), missing=None)
    aws_secret_access_key = colander.SchemaNode(colander.String(), missing=None)
    region = colander.SchemaNode(colander.String(), validator=colander.OneOf(get_aws_regions()), default='us-east-1')
    root_email = colander.SchemaNode(colander.String(), validator=colander.Email())
    tag_key = colander.SchemaNode(colander.String(), default='NessAWS')
    account_name = colander.SchemaNode(colander.String())
    account_number = colander.SchemaNode(colander.Int(), validator=colander.Range(1, 999999999999))


class AWSAccounts(colander.SequenceSchema):
    __doc__ = 'Colander Sequence Schema for multiple AWS Accounts.'
    aws_account = AWSAccount()


class AppConfig(colander.MappingSchema):
    __doc__ = 'Colander Mapping Schema for the overall configuration object.'
    nessus_url = colander.SchemaNode(colander.String(), missing='https://localhost:8834')
    ...
    smtp_port = colander.SchemaNode(colander.Int(), validator=colander.Range(1, 65535), missing=25)
    ...
    smtp_to = colander.SchemaNode(colander.String(), missing='aws-security-cust-pen-test@amazon.com')
    smtp_cc = colander.SchemaNode(colander.String(), missing=None)
    smtp_sendas = colander.SchemaNode(colander.String())
    smtp_subject = colander.SchemaNode(colander.String(), missing='AWS Pentest Request')
    ...
    output = colander.SchemaNode(colander.String(), validator=colander.OneOf([
     'excel', 'raw_csv', 'none']), missing='excel')


class Target(colander.MappingSchema):
    __doc__ = 'Colander Mapping Schema for the scan target object.'
...
class Targets(colander.SequenceSchema):
    __doc__ = 'Colander Sequence Schema for scan targets.'
    targets = Target()


class Scan(colander.MappingSchema):
    __doc__ = 'Colander Mapping Schema for a Nessus scan.'
    scan_name = colander.SchemaNode(colander.String())
    targets = Targets()


class Scans(colander.SequenceSchema):
    __doc__ = 'Colander Sequence Schema for Nessus scans.'
    scan = Scan()


class AccountNames(colander.SequenceSchema):
    __doc__ = 'Colander Sequence Schema for AWS account names.'
    account_name = colander.SchemaNode(colander.String())


class TagValues(colander.SequenceSchema):
    __doc__ = 'Colander Sequence Schema for tag values.'
    value = colander.SchemaNode(colander.String())


class StateConfig(colander.MappingSchema):
    __doc__ = 'Colander Mapping Schema for the state file object.'
    start_date = colander.SchemaNode(colander.String())
    end_date = colander.SchemaNode(colander.String())
