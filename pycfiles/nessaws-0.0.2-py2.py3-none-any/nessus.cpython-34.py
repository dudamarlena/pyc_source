...
# [GCC 8.4.0]
# Embedded file name: /Users/adamstauffer/Desktop/TerbiumCode/Github/nessaws/src/nessaws/nessus.py
# Compiled at: 2017-06-23 11:14:06
# Size of source mod 2**32: 8674 bytes
...
class NessusConnection(object):
    __doc__ = 'Nessus connection object.'
    ...
    def _login(self):
        ...
        token = self._action('POST', 'session', {'username': self.username, 
    ...
    def _action(self, method, endpoint, parameters=None, download=False):
        ...
        while retry_count <= retry_total:
            response = requests.request(method, '{}/{}'.format(self.url, endpoint), data=json.dumps(parameters), verify=self.secure, headers=self.headers)
            if response.status_code == 401 and endpoint != 'session':
                self._login()
            elif response.status_code != 200 and response.status_code != 401:
                logger.warning('Received a non-success status code ({code}) when accessing the "{endpoint}" Nessus API endpoint (retry {retry}/{retry_total}).\n\nResponse: {response}'.format(code=response.status_code, endpoint=endpoint, retry=retry_count, retry_total=retry_total, response=response.text))
                retry_count += 1
                time.sleep(5)
            else:
                if download:
    ...
    def launch_scan(self, scan_name, scan_targets):
        ...
        if scan_id:
            scan_uuid = self._action('POST', 'scans/{}/launch'.format(scan_id), {'alt_targets': scan_targets}).get('scan_uuid')
            return scan_uuid
    ...
    def export_scan_csv(self, scan_name, scan_uuid):
        ...
        """
        scan_id = self.get_scan_id(scan_name)
        scan_history = self._action('GET', 'scans/{}'.format(scan_id)).get('history', []) or []
        ...
        if scan_history_id:
            file_id = self._action('POST', 'scans/{}/export?history_id={}'.format(scan_id, scan_history_id), {'format': 'csv'})['file']
            while True:
                file_status = self._action('GET', 'scans/{}/export/{}/status'.format(scan_id, file_id))['status']
                if file_status == 'ready':
                    break
                time.sleep(5)

            csv_output = self._action('GET', 'scans/{}/export/{}/download'.format(scan_id, file_id), download=True)
            filename = '{}_{}.csv'.format(slugify(scan_name, max_length=15), scan_uuid)
    ...
    def get_credentialed_hosts(self, scan_name, scan_uuid):
        ...
        """
        result_dict = {'uncredentialed_hosts': [],  'credentialed_hosts': []}
        scan_id = self.get_scan_id(scan_name)
        scan_history = self._action('GET', 'scans/{}'.format(scan_id)).get('history', [])
        for scan in scan_history:
            if scan['uuid'] == scan_uuid:
                scan_history_id = scan['history_id']
                scan_hosts = self._action('GET', 'scans/{}?history_id={}'.format(scan_id, scan_history_id)).get('hosts', [])
                for scan_host in scan_hosts:
                    plugin_output = self._action('GET', 'scans/{}/hosts/{}/plugins/19506?history_id={}'.format(scan_id, scan_host['host_id'], scan_history_id)).get('outputs', [{}])[0].get('plugin_output', '')
                    if 'Credentialed checks : yes' in plugin_output:
                        result_dict['credentialed_hosts'].append(scan_host['hostname'])
                    else:
                        result_dict['uncredentialed_hosts'].append(scan_host['hostname'])

                continue

        return result_dict

    def wait_for_scan_completion(self, scan_uuid):
        """Poll a Nessus scan for status until it is completed.
    ...
    def __repr__(self):
        return '<NessusConnection: {}>'.format(self.url)