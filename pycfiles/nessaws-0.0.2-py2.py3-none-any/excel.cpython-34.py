...
# [GCC 8.4.0]
# Embedded file name: /Users/adamstauffer/Desktop/TerbiumCode/Github/nessaws/src/nessaws/excel.py
# Compiled at: 2017-06-23 14:02:24
...
def write_excel_output(results):
    ...
    scan_accounts = WriteOnlyCell(summary_sheet, value=', '.join(results['account_names']))
    ...
    tag_values = WriteOnlyCell(summary_sheet, value=', '.join(results['tag_values']))
    ...
    credentialed_targets = WriteOnlyCell(summary_sheet, value='Credentialed Targets')
    credentialed_targets.font = Font(size=12, bold=True)
    uncredentialed_targets = WriteOnlyCell(summary_sheet, value='Uncredentialed Targets')
    ...
    for scan in results['scans']:
        ...
        for target in scan['targets']:
            scan_target_names.append(target['name'] if target.get('name') else target['id'])

        scan_targets = WriteOnlyCell(summary_sheet, value=', '.join(scan_target_names))
        scan_credentialed_targets = WriteOnlyCell(summary_sheet, value=len(scan.get('credentialed_hosts', [])))
        scan_uncredentialed_targets = WriteOnlyCell(summary_sheet, value=len(scan.get('uncredentialed_hosts', [])))
        summary_sheet.append([
    ...
    for scan in results['scans']:
        if scan['status'] == 'completed' and scan.get('result_file') is not None:
            ...
            try:
                    ...
                    for row in reader:
                        ...
                        for c, column in enumerate(row):
                            ...
                            if c == 3:
                                ...
                                if c == 4:
                                    for target in scan['targets']:
                                        if str(target.get('public_ip')) == column or str(target.get('private_ip')) == column or target.get('endpoint') == column:
                                            column = '{}\n({})'.format(target['name'] if target.get('name') else target['id'], column)
                                            break

                                data_cell = WriteOnlyCell(results_sheet, value=column)
                                if fill:
                                    data_cell.fill = fill
                ...
                logger.warning('Could not find result file "{}" for scan name "{}", these results will not be included in the report'.format(scan, result_filename))

            continue

    results_sheet.auto_filter.ref = 'A1:M1'
    output_filename = 'nessaws_{}.xlsx'.format(datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%s'))
