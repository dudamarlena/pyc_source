...
# Compiled at: 2017-08-15 12:43:07
# Size of source mod 2**32: 10110 bytes
"""Provides the nessaws command line interface."""
from builtins import input
import datetime, logging, sys, time, click
...
def pen_test_request(ctx, options, tags, dry_run):
    ...
    logger.info('Performing pentest-request with following tag values "{}" and dry-run={}'.format(', '.join(tags), dry_run))
...
def perform_scan(ctx, options):
    ...
    nessus_conn = NessusConnection(configuration['nessus_url'], configuration['nessus_username'], configuration['nessus_password'], configuration['nessus_secure'])
    for scan in state_object['scans']:
        ...
        if scan_uuid:
            ...
            logger.warning('Unable to find a Nessus scan with name "{}", the following instances will not be scanned:\n\n {}'.format(scan_name, ', '.join(scan_target_names)))
    ...
    while done_count != scans_count:
        ...
        for scan in state_object['scans']:
            ...
            logger.debug('Got status "{}" from scan "{}"'.format(scan_status, scan_name))
            if scan_status != scan.get('status'):
                scan['status'] = scan_status
                if scan_status != 'running' and scan_status != 'pending':
                    done_count += 1
                    logger.debug('Scan {} is no longer running'.format(scan_name))
                    if scan_status == 'completed':
                        successful_scan = True
                        logger.info('Scan "{}" completed successfully'.format(scan_name))
                        logger.info('Updating credentialed/uncredentialed hosts in "{}" ...'.format(scan_name))
                        ...
                        logger.warning('Nessus scan "{}" was not completed successfully, the following instances may not have been scanned:\n\n {}'.format(scan_name, ', '.join(scan['target_names'])))
                else:
                    logger.info('Waiting for scan "{}" to be completed...'.format(scan_name))
    ...
    if configuration['output'] == 'excel':
        ...
        logger.info('Exported result file is named: "{}"'.format(output_filename))
    else:
        if configuration['output'] == 'raw_csv':
            logger.info('Raw CSV output configured, Nessus report CSVs are located in the current directory.')
        else:
            logger.info('No output configured, exiting successfully...')