...
# Compiled at: 2020-04-19 15:35:17
# Size of source mod 2**32: 8102 bytes
...
class CRISPRCas(object):
    ...
    def crisprcas(self):
        ...
        def dist(x, y, ss, co):
            ...
            if co:
                return (
                 min(y[0] - x[1], x[0] - y[1]), cc_circ)
                if ss > 0:
                    if y[0] > x[1]:
                        if x[0] + ss - y[1] < y[0] - x[1]:
                            cc_circ = 'crispr_end'
                        return (
                         min(y[0] - x[1], x[0] + ss - y[1]), cc_circ)
                    if y[0] + ss - x[1] < x[0] - y[1]:
                        cc_circ = 'crispr_start'
                    return (
                     min(x[0] - y[1], y[0] + ss - x[1]), cc_circ)
            else:
                return (
