...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mittepro/tests.py
# Compiled at: 2020-01-28 09:11:55
...
class TestMittePro(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.variables = {'recipients': [
                        'Foo Bar <foo.bar@gmail.com>',
                        'Fulano <fulano@gmail.com>',
                        '<ciclano@gmail.com>'], 
         
         'context_per_recipient': {'foo.bar@gmail.com': {'foo': True}, 
         ...
         'files_names': []}
        self.search_variables = {'app_ids': '1001', 
         'start': '2017-10-26', 
         'end': '2017-10-27', 
         'uuids': [
                   '21da05e09a214bf',
        ...
        self.mittepro = MittePro(key=self.variables['key'], secret=self.variables['secret'], fail_silently=False, timeout_read=20)

    def get_attachments(self):
        attachments = []
        files = self.variables['files_names']
        for dfile in files:
            content = base64.encodebytes(open(os.path.join(os.path.expanduser('~'), 'test_files', dfile), 'rb').read())
            attachments.append({'file': content, 'name': dfile})

        return attachments

    def test_method_post_text(self):
        mail = Mail(track_open=False, track_html_link=False, track_text_link=True, activate_tracking=False, subject='Mittepro-py client test', message_text=self.variables['message_text'], message_html=self.variables['message_html'], from_=self.variables['from_'], recipient_list=self.variables['recipients'])
        try:
    ...
    def t2est_method_post_template(self):
        mail = Mail(track_open=True, track_html_link=False, track_text_link=False, activate_tracking=True, subject='Mittepro-py client test', send_at='2019-05-04 08:00:00', from_=self.variables['from_'], recipient_list=self.variables['recipients'], template_slug='tpl-teste', use_tpl_default_name=True, use_tpl_default_email=True, use_tpl_default_subject=True, context={'GMERGE': 'Mah oia Soh'}, context_per_recipient=self.variables['context_per_recipient'])
    ...
    def t2est_method_get_mail_search(self):
        search_args = SearchMailArgs(end=self.search_variables['end'], start=self.search_variables['start'], app_cods=self.search_variables['app_ids'])
