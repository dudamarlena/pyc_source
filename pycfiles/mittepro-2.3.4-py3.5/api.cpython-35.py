...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mittepro/api.py
# Compiled at: 2019-11-06 06:41:48
# Size of source mod 2**32: 4839 bytes
import json, arrow, logging, requests
...
class Api(object):
    ...
    def send_request(self, url, payload):
        if payload and self.http_method == 'GET':
            ...
            request = Request(self.http_method, url, json=payload, headers=self.headers)
    ...
    def request(self, payload=None, auth_timestamp=None):
        ...
        try:
            ...
            if not self.fail_silently:
                ...
                return {'error': 'The server did not respond within the time you stipulated. The time was {0} second(s)'.format(str(self.timeout_read))}
