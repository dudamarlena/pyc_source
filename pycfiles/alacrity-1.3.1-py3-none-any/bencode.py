...
# Embedded file name: build/bdist.macosx-10.5-fat3/egg/alacarte/serialize/bencode.py
# Compiled at: 2010-03-18 05:47:02
__all__ = [
 'render', 'CodecError', 'EncodeError', 'DecodeError', 'Encoding', 'ChunkedEncoder', 'Bencode', 'EnhancedBencode']
...
def render(data, template=None, kind='enhanced'):
    ...
    return (
     'application/x-bencode', codecs[kind].encode(data))
...
class Bencode(ChunkedEncoder):
    ...
    def decode(self, data):
        length = len(data)
        if length == 0:
            raise DecodeError, 'Can not decode an empty string.'
        data, processed = self._decode(data)
        if processed != length:
            raise DecodeError, 'Did not fully decode input. %d of %d processed, %d bytes remaining.' % (processed, length, length - processed)
        return data
    ...
    def decode_str(self, data, o):
        ...
        offset = index + 1
        return (
         data[offset:offset + length], offset + length)
    ...
    def encode_dict(self, data):
        return 'd%se' % (('').join([ self.encode(key) + self.encode(data[key]) for key in sorted(data.keys()) ]),)
...
class EnhancedBencode(Bencode):
    ...
    def decode_u(self, data, o):
        ...
        offset = index + 1
        return (
         data[offset:offset + length].decode('utf-8'), offset + length)