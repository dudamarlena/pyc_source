...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/buzz/corpus.py
# Compiled at: 2020-05-03 14:01:08
# Size of source mod 2**32: 10939 bytes
import json, os
...
class Corpus(MutableSequence):
    ...
    def __init__(self, path=None):
        ...
        self.path = path
        meta = '.metadata.json'
        self._metadata_path = os.path.join(self.path, meta) if not self.is_feather else os.path.join(os.path.dirname(self.path), meta)
        ...
        self.filepaths = Contents([i.path for i in self.files],
          is_parsed=(self.is_parsed), name=(self.name))
        self.nlp = None
 ...
 'w'], subcorpora=['file'], **kwargs):
    ...
    def _get_subcorpora_and_files(self):
        ...
        if self.is_feather:
            return (
             Contents([], **info), Contents([File(self.path)], **info), True)
