...
# Size of source mod 2**32: 10277 bytes
...
def _process_string(plain, path, save_as, corpus_name, language, speakers):
    ...
    for sent_index, sent in enumerate((doc.sents), start=1):
        sstr = _process_sent(sent_index, sent, file_meta, plain, stripped_data, language, nlp, speakers)
        output.append(sstr)
...
class Parser:
    ...
    def _spacy_parse(self):
        ...
        delay = (multi.parse(x, i, self.save_as, self.corpus_name, self.language, self.speakers) for i, x in enumerate(chunks))
