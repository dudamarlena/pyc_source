...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/flexidate/test_flexidate.py
...
class TestDateParsers(object):
    ...
    def test_parse_wildcards(self):
        fd = parse('198?')
        assert fd.year == '', fd.year

    def test_parse_with_qualifiers(self):
        fd = parse('1985?')
        assert fd.year == '1985', fd
        assert fd.qualifier == 'Uncertainty : 1985?', fd.qualifier
        fd = parse('c.1780')
        assert fd.year == '1780', fd
        assert fd.qualifier == "Note 'circa' : c.1780", fd
        fd = parse('c. 1780')
        assert fd.year == '1780', fd
        assert fd.qualifier.startswith("Note 'circa'"), fd
