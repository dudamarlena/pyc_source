...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/spm1d/stats/anova/designs.py
# Compiled at: 2019-08-22 04:37:04
# Size of source mod 2**32: 25111 bytes
"""
High-level ANOVA designs.
"""
import warnings, numpy as np
from matplotlib import pyplot
from .factors import Factor, FactorNested, FactorNested2, FactorNestedTwoWay
...
class ANOVA2rm(ANOVA2):
    __doc__ = 'Both A and B are RM factors.'
    ...
    def _check_balanced(self):
        if not (self.A.balanced and self.B.balanced and self.S.balanced):
            ...
            if not self.S.check_balanced(self.A):
                raise ValueError('Design must be balanced.')
            raise self.S.check_balanced(self.B) or ValueError('Design must be balanced.')
...
class ANOVA2onerm(ANOVA2rm):
    __doc__ = 'Only B is an RM factor.'
    ...
    def _check_balanced(self):
        if not (self.A.balanced and self.B.balanced and self.S.balanced):
            ...
            if not self.S.check_balanced_rm(self.A):
                raise ValueError('Design must be balanced.')
            raise self.S.check_balanced_rm(self.B) or ValueError('Design must be balanced.')


class ANOVA3(_Design):
    effect_labels = ('Main A', 'Main B', 'Main C', 'Interaction AB', 'Interaction AC',
                     'Interaction BC', 'Interaction ABC')
    nFactors = 3
    ...
    def _check_balanced(self):
        if not (self.A.balanced and self.B.balanced and self.C.balanced):
            ...
            if not self.A.check_balanced(self.C):
                raise ValueError('Design must be balanced.')
            raise self.B.check_balanced(self.C) or ValueError('Design must be balanced.')
...
class ANOVA3rm(ANOVA3):
    __doc__ = 'A, B and C are all RM factors.'
        ...
        if not (self.A.balanced and self.B.balanced and self.C.balanced and self.S.balanced):
            ...
            if not self.A.check_balanced(self.B):
                ...
                if not self.A.check_balanced(self.C):
                    raise ValueError('Design must be balanced.')
                raise self.B.check_balanced(self.C) or ValueError('Design must be balanced.')
            raise self.S.check_balanced_rm(self.C) or ValueError('Design must be balanced.')

    def check_for_single_responses(self, dim=1):
        A, B, C, S = (
         self.A.A, self.B.A, self.C.A, self.S.A)
        only_single = False
        for a in self.A.u:
            for b in self.B.u:
                for c in self.C.u:
...
class ANOVA3onerm(ANOVA3rm):
    __doc__ = 'Only C is an RM factor.'
...
class ANOVA3tworm(ANOVA3rm):
    __doc__ = 'Both B and C are RM factors.'
