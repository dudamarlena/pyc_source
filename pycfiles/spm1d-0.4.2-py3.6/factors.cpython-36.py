...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/spm1d/stats/anova/factors.py
...
class Factor(object):
    ...
    def get_design_interaction_4way(self, other, another, yetanother):
        X = []
        S, A, B, C = (
         self.A, other.A, another.A, yetanother.A)
...
class FactorNested(Factor):
    ...
    def get_design_interaction_3way(self, other, another):
        A, B, C, S = (
         self.NEST.A, other.A, another.A, self.A)
        X = []
        for uA, US in zip(self.NEST.u, self.u):
            for uB in other.u[1:]:
                for uC in another.u[1:]:
...
class FactorNestedTwoWay(Factor):
    ...
    def get_design_interaction(self, other):
        A, B, C, S = (
         self.NEST0.A, self.NEST1.A, other.A, self.A)
        X = []
