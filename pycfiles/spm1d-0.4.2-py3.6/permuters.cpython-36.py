...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/spm1d/stats/nonparam/permuters.py
...
class _Permuter(object):
    ...
    def get_z_critical(self, alpha=0.05, two_tailed=False):
        perc = [50.0 * alpha, 100 * (1 - 0.5 * alpha)] if two_tailed else 100 * (1 - alpha)
...
class _PermuterOneSample0D(_PermuterOneSample, _Permuter0D):
    ...
    def __init__(self, y, mu=None):
        ...
        self.labels0 = np.array([0] * self.N)
...
class _PermuterOneSample1D(_PermuterOneSample, _Permuter1D):
    ...
    def __init__(self, y, mu=None, roi=None):
        ...
        self.J = y.shape[0]
        self.Q = y.shape[1]
        self.I = 1 if y.ndim == 2 else y.shape[2]
...
class _PermuterRegress1D(_PermuterRegress, _Permuter1D):
    ...
    def __init__(self, y, x, roi=None):
        ...
        self.x = x
        self.J = x.size
        self.I = y.shape[2] if self.ismultivariate else 1
        self.labels0 = np.arange(self.J)
        self.nPermTotal = int(factorial(self.J))
        self.calc = None
        self.ZZ = None
...
class _PermuterTwoSample(object):
    ...
    def __init__(self, yA, yB):
        ...
        self.labels0 = np.array([0] * self.JA + [1] * self.JB)
        self.labelsZeros = np.array([0] * self.J)
...
class _PermuterTwoSample1D(_PermuterTwoSample, _Permuter1D):
    ...
    def __init__(self, yA, yB, roi=None):
        ...
        self.roi = None
        self.I = yA.shape[2] if self.ismultivariate else 1
        self._set_roi(roi)
