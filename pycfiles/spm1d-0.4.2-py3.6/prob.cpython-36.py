...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/spm1d/rft1d/prob.py
...
def ec_density_T(z, df):
    ...
    c = (1 + z ** 2 / v) ** ((1 - v) / 2)
    EC = []
    EC.append(1 - stats.t.cdf(z, v))
    EC.append(a ** 0.5 / TWO_PI * c)
    return EC
...
def ec_density_F(z, df):
    if z < 0:
        ...
        EC.append(a ** 0.5 * np.exp(gammaln((v + k - 1) / 2) - b) * 1.4142135623730951 * (k * z / v) ** (0.5 * (k - 1)) * (1 + k * z / v) ** (-0.5 * (v + k - 2)))
        return EC
            ...
            if STAT == 'X2':
                ...
                if STAT == 'F':
                    ...
                    if STAT == 'T2':
                        ...
                        raise (
                         ValueError, 'Statistic must be one of: "Z", "T", "X2", "F", "T2"')
    return zstar
...
def isf(STAT, alpha, df, resels, n, Q=None, version='spm12'):
    ...
    for aaa in alpha:
        ...
        zzz = optimize.fmin(fn, z0, xtol=1e-09, disp=0)[0]
        zstar.append(zzz)
