...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/spm1d/stats/nonparam/_snpm.py
# Compiled at: 2019-08-22 04:37:04
# Size of source mod 2**32: 12244 bytes
...
class _SnPM(object):
    __doc__ = 'Parent class for all non-parametric SPM classes.'
    STAT = 'Z'
    isanova = False
    isparametric = False
    dim = 0
    ...
    def _check_iterations(self, iterations, alpha, force_iterations):
        if iterations > self.nPermUnique:
            ...
            if iterations == -1 and self.nPermUnique > 10000 or iterations > 10000:
        ...
        if iterations != -1 and iterations < 10:
            raise ValueError('\nNumber of specified iterations (%d) must be at least 10\n' % iterations)
        elif iterations > 0:
            if iterations < 1 / alpha:
    ...
    def get_nPermUnique_asstr(self):
        ...
        if n is None:
            ...
            s = str(n) if n < 1000000.0 else '%.4g' % n
            s += ' permutations possible'
        return s


class _SnPM0D(_SnPM):
    __doc__ = 'Parent class for all 0D non-parametric SPM classes.'

    def __init__(self, z, perm=None, isinlist=False):
        z = 0 if np.isnan(z) else z
        self.permuter = perm
        self.nPermUnique = None if isinlist else perm.nPermTotal
        self.z = z
...
class SnPM0Dinference(_SnPM0D):
    ...
    def __init__(self, spm, alpha, zstar, p, two_tailed=False, isinlist=False):
        ...
        self.STAT = spm.STAT
        self.PDF = None if isinlist else self.permuter.Z
        self.alpha = alpha
        self.nPerm = None if isinlist else self.permuter.Z.size
        self.p = p
...
class _SnPM1D(_SnPM, _spm._SPM):
    __doc__ = 'Parent class for all 1D non-parametric SPM classes.'
    dim = 1
...
class SnPMinference(_SnPM1D, _spm._SPMinference):
    ...
    def __init__(self, spm, alpha, zstar, two_tailed, clusters):
        ...
        self.STAT = spm.STAT
        self.PDF0 = self.permuter.Z
        self.PDF1 = self.permuter.Z2
        self.alpha = alpha
        self.clusters = clusters
        self.nClusters = len(clusters)
        self.p = [c.P for c in clusters]
        self.two_tailed = two_tailed
        self.zstar = zstar
