...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/spm1d/stats/ci.py
# Compiled at: 2019-08-22 04:37:04
# Size of source mod 2**32: 12671 bytes
"""
...
class _CI(object):
    ...
    def _assemble_datum_criterion_pairs(self):
        self.datum = (
         self.datum_type, self.datum_value)
    ...
    def _get_repr_ci(self):
        ...
        if self.datum_type == 'mean':
            ...
            if self.criterion_type == 'meanB':
                s += '      ciA        :  %s\n' % self.get_ciA(asstr=True)
            else:
                if self.criterion_type == 'tailB':
                    s += '      ciA        :  %s\n' % self.get_ciA(asstr=True)
...
class _CIMultiMean(_CI):
    ...
    def __init__(self, spmi, meanA, meanB, hstar, criterion='meanB'):
        ...
        if criterion == 'meanB':
            self.ciA = (
             self.meanA - self.hstar, self.meanA + self.hstar)
            if self.dim == 1:
                self.ciA = np.asarray(self.ciA)
        elif criterion == 'tailB':
            self.ciA = (
             self.meanA - 0.5 * self.hstar, self.meanA + 0.5 * self.hstar)
...
def ci_pairedsample(yA, yB, alpha=0.05, datum='difference', mu=None):
    ...
    if datum == 'difference':
        ...
        s = spmi.sigma2 ** 0.5
        hstar = spmi.zstar * s / yA.shape[0] ** 0.5
        if datum == 'difference':
...
def ci_twosample(yA, yB, alpha=0.05, equal_var=True, datum='difference', mu=None):
    if equal_var is not True:
        ...
        mA += muvalue
        s = spmi.sigma2 ** 0.5
        hstar = spmi.zstar * s * (1.0 / JA + 1.0 / JB) ** 0.5
        if datum == 'difference':
