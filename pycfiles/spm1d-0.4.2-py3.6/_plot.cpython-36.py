...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/spm1d/_plot.py
...
class DataPlotter(object):
    ...
    def _set_ylim(self, pad=0.075):
        ...
        def minmax(x):
            return (
             np.ma.min(x), np.ma.max(x))

        ax = self.ax
        ymin, ymax = (10000000000.0, -10000000000.0)
...
class SPMiPlotter(SPMPlotter):
    ...
    def plot_threshold_label(self, lower=False, pos=None, **kwdargs):
        ...
        s = '$\\alpha$=%.2f:  $%s^*$=%.3f' % (spmi.alpha, self._get_statstr(), spmi.zstar)
