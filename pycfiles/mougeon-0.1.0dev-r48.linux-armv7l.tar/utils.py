...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.5/site-packages/mougeon/gui/hildon/utils.py
...
@author: maemo
"""
import hildon, gobject, dbus, gtk, webbrowser, sys, traceback, logging, gdata.projecthosting.client, gdata.projecthosting.data, gdata.gauth, gdata.client, gdata.data, atom.http_core, atom.core, threading
from Queue import *
from mougeon.gui.gtk.utils import LOGO_PIXBUF
...
class MougeonStackableWindow(hildon.StackableWindow):
    """
    ...
    def __init__(self, title='Mougeon', async_init=False, raise_stop_flag=None):
        u"""
        Paremeter:
            - title : window title
        ...
        if async_init:
            async_thread = AsyncTask(self, raise_stop_flag, self.init_center_view, self.centerview)
            async_thread.start()
        else:
...
class AboutView(MougeonStackableWindow):
    """
    ...
    def init_center_view(self, centerview):
        ...
        centerview.add(gtk.Label('Mougeon - ' + version.getInstance().getVersion()))
        centerview.add(gtk.Label('Free Mobile Cell Tower usage tracker'))
        centerview.add(gtk.Label('by Thierry Bressure - http://blog.mougeon.bressure.net'))
    ...
    def on_blog_clicked_event(self, widget, data):
        call_handled_method(open_browser_on, 'http://blog.mougeon.bressure.net')

    def on_site_clicked_event(self, widget, data):
        call_handled_method(open_browser_on, 'http://mougeon.bressure.net')

    def on_group_clicked_event(self, widget, data):
        call_handled_method(open_browser_on, 'http://groupe.google.com/mougeon.')


class BugReportView(MougeonStackableWindow):
    """
    ...
    def __init__(self, type, value, traceback):
        ...
        MougeonStackableWindow.__init__(self, title='Bug reporting')
    ...
    def init_center_view(self, centerview):
        ...
        return MougeonStackableWindow.init_center_view(self, centerview)

    def init_bottom_button(self, bottomButtons):
        post = self.create_button('Post issue', None)
        post.connect('clicked', self.on_post_button_clicked, self)
        self.add_button(post)
        return MougeonStackableWindow.init_bottom_button(self, bottomButtons)
        ...
        try:
            ...
            if response == gtk.RESPONSE_OK:
                try:
                    ...
                    issues_client.client_login(username, password, 'mouegon', 'code')
                    ...
                    issues_client.add_issue('pyjama', title, description, 'tbressure', labels=labels)
                except:
