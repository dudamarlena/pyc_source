...
# [GCC 8.4.0]
# Embedded file name: /opt/anaconda3/lib/python3.7/site-packages/reademptionlib/vizdeseq.py
...
class DESeqViz(object):
    ...
    def _create_scatter_plots(self, condition_1, condition_2):
        ...
        deseq_result = pd.read_table(self._deseq_path_template % (condition_1, condition_2))
        deseq_result = deseq_result[(deseq_result.baseMean > 0)]
        fig = plt.figure()
        significant_deseq_result = deseq_result[(deseq_result.padj < self._p_value_significance_limit)]
        non_significant_deseq_result = deseq_result[(deseq_result.padj >= self._p_value_significance_limit)]
        plt.plot((np.log10(non_significant_deseq_result.baseMean)),
          (non_significant_deseq_result.log2FoldChange),
          '.k', alpha=0.5)
        plt.plot((np.log10(significant_deseq_result.baseMean)),
    ...
    def _create_volcano_plots(self, condition_1, condition_2):
        ...
        basemean, log2_fold_changes, p_values, adj_p_values = self._parse_deseq_file(deseq_path)
        cleaned_p_values = []
        ...
        self._create_volcano_plot(cleaned_log2_fold_changes, cleaned_p_values, condition_1, condition_2, self._pp_raw)
        ...
        self._create_volcano_plot(cleaned_log2_fold_changes,
          cleaned_adj_p_values, condition_1,
          condition_2, (self._pp_adj), pvalue_string_mod='(adjusted)')

    def _create_volcano_plot(self, log2_fold_changes, p_values, condition_1, condition_2, pp, pvalue_string_mod=''):
        fig = plt.figure()
        max_log_2_fold_change = max([
         abs(min(log2_fold_changes)),
         abs(max(log2_fold_changes))])
        p_values = np.array(p_values)
        p_values[p_values == 0.0] = 1e-100
    ...
    def _parse_deseq_file(self, deseq_path):
        ...
        return (
         basemeans, log_2_fold_changes, p_values, adj_p_values)