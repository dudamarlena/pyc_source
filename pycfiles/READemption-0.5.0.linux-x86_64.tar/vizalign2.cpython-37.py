...
# [GCC 8.4.0]
# Embedded file name: /opt/anaconda3/lib/python3.7/site-packages/reademptionlib/vizalign2.py
# Compiled at: 2019-07-15 11:56:07
# Size of source mod 2**32: 4963 bytes
import pandas as pd
from bokeh.charts import Bar, output_file, show
from bokeh.layouts import column
from bokeh.models import HoverTool, BoxSelectTool, BoxZoomTool, ResetTool, PanTool, WheelZoomTool, ResizeTool

class AlignViz2(object):

    def alignment_stats(self, input_path_file, output_path_file_csv, output_path_file_html):
        df = pd.read_json(input_path_file, orient='index')
        df.to_csv((output_path_file_csv + '.csv'), index=True,
          sep='\t')
        samples = []
        alignment_length = []
        alignment_freq = []
        ...
        bar1 = Bar(data1,
          values='frequency', label='aligned read length', stack='samples',
          agg='sum',
    ...
    def process_stats(self, input_path_file, output_path_file_csv, output_path_file_html):
        df = pd.read_json(input_path_file, orient='index')
        ...
        bar1 = Bar(data1,
          values='Nr. of reads', label='sample', stack='condition',
          agg='sum',
          title='Input read types',
          legend='top_right',
          palette=[
         'darkseagreen', 'salmon', 'darkslateblue'],
          tools=[
         HoverTool(tooltips=[
        ...
        frequency = []
        for index, row in df.iterrows():
            for key, value in row['read_length_after_processing_and_freq'].items():
        ...
        bar2 = Bar(data2,
          values='frequency', label='read length', stack='samples',
          agg='sum',
