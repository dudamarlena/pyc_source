...
# [GCC 8.4.0]
# Embedded file name: /opt/anaconda3/lib/python3.7/site-packages/reademptionlib/readalignerstatstable.py
...
class ReadAlignerStatsTable(object):
    ...
    def __init__(self, read_processing_stats, alignment_stats, primary_read_aligner_stats, realigner_stats, libs, output_path, paired_end):
        self._table = []
    ...
    def _add_global_countings(self):
        for title, data in [
         ...
         (
          ...
          self._total_alignment_stat_numbers('no_of_uniquely_aligned_reads')),
         (
    ...
    def _add_reference_wise_coutings(self):
        ref_ids = sorted(list(list(self._alignment_stats.values())[0]['stats_per_reference'].keys()))
        for ref_id in ref_ids:
            for title_template, data in [
             ('%s - No. of aligned reads',
              self._alignment_number_per_ref_seq(ref_id, 'no_of_aligned_reads')),
             (
              '%s - No. of uniquely aligned reads',
              self._alignment_number_per_ref_seq(ref_id, 'no_of_uniquely_aligned_reads')),
             (
              '%s - No. of alignments',
              self._alignment_number_per_ref_seq(ref_id, 'no_of_alignments')),
             (
              '%s - No. of split alignments',
              self._alignment_number_per_ref_seq(ref_id, 'no_of_split_alignments'))]:
                self._table.append([title_template % ref_id] + data)

    def _alignment_number_per_ref_seq(self, ref_id, attribute):
        return [round(self._alignment_stats[lib]['stats_per_reference'][ref_id].get(attribute, 0)) for lib in self._libs]

    def _total_alignment_stat_numbers(self, attribute, round_nums=True):
        countings = [self._alignment_stats[lib]['stats_total'].get(attribute, 0) for lib in self._libs]
    ...
    def _get_read_process_numbers(self, attribute):
        ...
        if self._paired_end:
            factor = 2
        return [self._read_processing_stats[lib][attribute] * factor for lib in self._libs]

    def _perc_aligned_reads_all_input(self):
        return [round(self._calc_percentage(aligned_reads, total_reads), 2) for aligned_reads, total_reads in zip(self._total_alignment_stat_numbers('no_of_aligned_reads',
          round_nums=False), self._get_read_process_numbers('total_no_of_reads'))]

    def _perc_aligned_reads_all_long_enough(self):
        return [round(self._calc_percentage(aligned_reads, total_reads), 2) for aligned_reads, total_reads in zip(self._total_alignment_stat_numbers('no_of_aligned_reads',
          round_nums=False), self._get_read_process_numbers('long_enough'))]

    def _perc_uniquely_aligned_reads(self):
        return [round(self._calc_percentage(uniquely_aligned_reads, aligned_reads), 2) for uniquely_aligned_reads, aligned_reads in zip(self._total_alignment_stat_numbers('no_of_uniquely_aligned_reads'), self._total_alignment_stat_numbers('no_of_aligned_reads'))]
