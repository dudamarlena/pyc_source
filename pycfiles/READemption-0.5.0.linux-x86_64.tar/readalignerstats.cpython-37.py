...
# [GCC 8.4.0]
# Embedded file name: /opt/anaconda3/lib/python3.7/site-packages/reademptionlib/readalignerstats.py
...
class ReadAlignerStats(object):
    ...
    def _count_unaligned_reads(self, unaligned_read_paths):
        with open(unaligned_read_paths) as (fasta_fh):
            self._stats['stats_total']['no_of_unaligned_reads'] = self._count_fasta_entries(fasta_fh)

    def _count_fasta_entries(self, fasta_fh):
        return reduce(lambda x, y: x + 1, self.fasta_parser.entries(fasta_fh), 0)
    ...
    def _count_aligned_reads_and_alignments(self, read_alignment_result_bam_path):
        ...
        for entry in bam.fetch():
            ...
            try:
                self._count_alignment(entry, ref_id, stats_per_ref, no_of_hits_per_read_freq)
            except KeyError:
                sys.stderr.write('SAM entry with unspecified reference found! Stoping\n')
                sys.exit(2)

        self._stats['stats_per_reference'] = stats_per_ref
        for ref_id, stats in stats_per_ref.items():
            stats_per_ref[ref_id]['no_of_hits_per_read_and_freqs'] = self._calc_down_to_read(stats_per_ref[ref_id]['no_of_hits_per_read_and_freqs'])
    ...
    def _calc_down_to_read(self, no_of_hits_per_read_freq):
        ...
        """
        return dict(((no_of_hits_per_read, freq / no_of_hits_per_read) for no_of_hits_per_read, freq in no_of_hits_per_read_freq.items()))
    ...
    def _init_counting_dict(self, stats_per_ref, ref_id):
        ...
        stats_per_ref[ref_id]['no_of_hits_per_read_and_freqs'] = defaultdict(int)

    def _count_alignment(self, entry, ref_id, stats_per_ref, no_of_hits_per_read_freq):
        ...
        stats_per_ref[ref_id]['no_of_aligned_reads'] += 1.0 / (float(no_of_hits) * no_of_splits)
        if no_of_hits == 1:
            stats_per_ref[ref_id]['no_of_uniquely_aligned_reads'] += 1.0 / no_of_splits
