...
# [GCC 8.4.0]
# Embedded file name: /opt/anaconda3/lib/python3.7/site-packages/reademptionlib/vizalign.py
...
class AlignViz(object):
    ...
    def __init__(self, lib_names, read_processing_stats_path, read_aligner_stats_path):
    ...
    def read_stat_files(self):
        with open(self._read_processing_stats_path) as (read_processing_stats_fh):
            self._read_processing_stats = json.loads(read_processing_stats_fh.read())
    ...
    def plot_input_read_length(self, plot_path):
        self._plot_read_lengths('read_length_before_processing_and_freq', 'Length distribution of the input reads - %s', plot_path)

    def plot_processed_read_length(self, plot_path):
        self._plot_read_lengths('read_length_after_processing_and_freq', 'Length distribution of the processed reads - %s', plot_path)
