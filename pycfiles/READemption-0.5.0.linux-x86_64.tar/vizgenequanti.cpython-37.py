...
# [GCC 8.4.0]
# Embedded file name: /opt/anaconda3/lib/python3.7/site-packages/reademptionlib/vizgenequanti.py
# Compiled at: 2019-07-15 11:59:22
# Size of source mod 2**32: 5429 bytes
import csv
...
class GeneQuantiViz(object):
    ...
    def __init__(self, gene_wise_quanti_combined_path, lib_names, use_antisene=True, axis_min=None, axis_max=None):
    ...
    def parse_input_table(self):
        ...
        self._lib_names_and_class_quanti = defaultdict(lambda : defaultdict(lambda : defaultdict(float)))
        for row in csv.reader((open(self._gene_wise_quanti_combined_path)),
          delimiter='\t'):
            if row[0].startswith('Orientation'):
                continue
            for index, cell in enumerate(row[10:]):
                self._lib_names_and_countings[self._lib_names[index]].append(float(cell))
                self._lib_names_and_class_quanti[self._lib_names[index]][row[0]][row[3]] += float(cell)
    ...
    def _plot_correlation(self, lib_1, lib_2):
        ...
        plt.plot([self._axis_min, self._axis_max], [
         self._axis_min, self._axis_max],
          ...
          color='green',
          alpha=0.4)
        corr_coeff = np.corrcoef(self._lib_names_and_countings[lib_1], self._lib_names_and_countings[lib_2])[0][1]
        plt.axis([self._axis_min, self._axis_max,
         self._axis_min, self._axis_max])
        plt.title('%s vs. %s\n(r = %s)' % (lib_1, lib_2, corr_coeff))
        plt.plot((self._lib_names_and_countings[lib_1]), (self._lib_names_and_countings[lib_2]),
          'k.',
          alpha=0.2)
    ...
    def _set_axis_max(self):
        self._axis_max = max([max(counting) for counting in self._lib_names_and_countings.values()])
    ...
    def plot_annotation_class_quantification(self, plot_path):
        ...
        cNorm = colors.Normalize(vmin=0, vmax=(len(all_classes_sorted) * len(self._lib_names_and_class_quanti[self._lib_names[0]].keys()) - 1))
        scalarMap = cm.ScalarMappable(norm=cNorm, cmap=color_map)
        color_index = 0
        for direction in self._lib_names_and_class_quanti[self._lib_names[0]].keys():
            for anno_class in all_classes_sorted:
                countings = [self._lib_names_and_class_quanti[lib][direction][anno_class] for lib in self._lib_names]
                color = scalarMap.to_rgba(color_index)
                plt.bar((range(no_of_libs)), countings, align='center', bottom=bottom,
                  linewidth=0,
                  color=color,
                  width=0.5,
        ...
        plt.xticks((np.array(range(no_of_libs))), (self._lib_names), rotation=45, ha='right')
        plt.tight_layout()
