...
# [GCC 8.4.0]
# Embedded file name: /opt/anaconda3/lib/python3.7/site-packages/reademptionlib/readprocessor.py
# Compiled at: 2019-07-15 11:59:22
# Size of source mod 2**32: 6914 bytes
import gzip, bz2
...
class ReadProcessor(object):
    ...
    def __init__(self, poly_a_clipping=False, min_read_length=12, paired_end=False, fastq=False, min_phred_score=None, adapter=None, reverse_complement=False):
    ...
    def process_paired_end(self, input_path_pair, output_path_pair):
        ...
        with gzip.open(output_path_pair[0], 'wb') as (output_p1_fh):
            with gzip.open(output_path_pair[1], 'wb') as (output_p2_fh):
                input_p1_fh = self._input_fh(input_path_pair[0])
                input_p2_fh = self._input_fh(input_path_pair[1])
                self._process_paired_end(input_p1_fh, input_p2_fh, output_p1_fh, output_p2_fh)
        return self._stats
    ...
    def _process_single_end(self, input_fh, output_fh):
        for header, seq, qualities in self._parse_sequences(input_fh):
            ...
            if self._fastq:
                if self._min_phred_score is not None:
                    seq = self._trim_by_quality(seq, qualities)
            elif self._reverse_complement:
                seq = Seq(seq)
            ...
            self._stats['read_length_before_processing_and_freq'][raw_seq_len] += 1
            self._stats['read_length_after_processing_and_freq'][clipped_seq_len] += 1
    ...
    def _parse_sequences(self, input_fh):
        if self._fastq:
            ...
            for seq_record in SeqIO.parse(input_fh, 'fasta'):
                yield (
                 seq_record.description, str(seq_record.seq), None)

    def _process_paired_end(self, input_p1_fh, input_p2_fh, output_p1_fh, output_p2_fh):
        for fasta_entry_p1, fasta_entry_p2 in zip(self._parse_sequences(input_p1_fh), self._parse_sequences(input_p2_fh)):
            ...
            if not raw_seq_p1_len < self._min_read_length:
                if raw_seq_p2_len < self._min_read_length:
                    self._stats['too_short'] += 1
                    continue
                self._stats['long_enough'] += 1
                self._stats['read_length_before_processing_and_freq'][raw_seq_p1_len] += 1
                self._stats['read_length_after_processing_and_freq'][raw_seq_p1_len] += 1
                self._stats['read_length_before_processing_and_freq'][raw_seq_p2_len] += 1
                self._stats['read_length_after_processing_and_freq'][raw_seq_p2_len] += 1
