...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\jobspider\baseclass\database.py
# Compiled at: 2016-04-13 21:04:26
import MySQLdb
from .utils.joblog import MyLogger

class Database:
    config = {'Job': {'host': 'localhost', 
               'user': 'admin', 
               'passwd': 'admin', 
               'db': 'Job', 
               'charset': 'utf8'}, 
    ...
    def update_by_dic(self, table, ref_key, ref_value, data):
        ...
        valstr = [ "'" + x + "'" if isinstance(x, unicode) else "'" + str(x).decode('utf8') + "'" for x in values ]
        obj = (',').join(str(keystr[index]) + '=%s' % valstr[index] for index in range(0, len(keys)))
    ...
    def load_file_to_db(self, table, file_name):
        sql_load = "LOAD DATA LOCAL INFILE '%s' INTO TABLE %s                 FIELDS TERMINATED BY ','                 ENCLOSED BY '' LINES TERMINATED BY '\n' " % (
         file_name, table)
