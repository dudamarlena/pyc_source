...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\jobspider\job51.py
...
class Job51_Spider(Base_Spider):
    ...
    def parse(self, url):
        headers = {'Host': 'www.search.51job.com', 
    ...
    def pages_parse(self, keyword):
        for page in xrange(1, 2):
            url = 'http://search.51job.com/jobsearch/search_result.php?fromJs=1&jobarea=010000%2C00&district=000000&funtype=0000&industrytype=00&issuedate=9&providesalary=99&' + 'keyword=%s&keywordtype=0&curr_page=%d&lang=c&stype=2' % (keyword, page) + '&postchannel=0000&workyear=99&cotype=99&degreefrom=99&jobterm=01&companysize=99&lonlat=0%2C0&radius=-1&ord_field=0&list_type=0&fromType=14&dibiaoid=0'
            data = self.parse(url)
            yield data