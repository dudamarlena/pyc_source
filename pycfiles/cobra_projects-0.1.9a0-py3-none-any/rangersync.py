...
# Embedded file name: build/bdist.linux-x86_64/egg/policytool/rangersync.py
...
def apply_tag_row_rule_command(filters, policy_template, context):
    ...
    for table in tables:
        ...
        for filter_ in filters:
            tag_filter_exprs = [ tagFilterExpr['filterExpr'] for tagFilterExpr in filter_['tagFilterExprs'] if set(tagFilterExpr['tags']) <= tags
                               ]
            tag_filter_exprs_str = (' and ').join(tag_filter_exprs)
            if len(tag_filter_exprs) > 0:
                row_filters.append({'groups': filter_['groups'], 
                   'users': filter_['users'], 
                   'conditions': [], 'accesses': [
                              {'isAllowed': True, 
                                 'type': 'select'}], 
                   'rowFilterInfo': {'filterExpr': tag_filter_exprs_str}, 
                   'delegateAdmin': False})

        if len(row_filters) > 0:
            end_date_columns = tag_columns['end_date']
            env = {'schema': table['schema'], 
               'table': table['table'], 
...
def _convert_hive_resource_accesses_to_path_resource_accesses(hive_accesses):
    ...
    for elem in hive_accesses:
        ...
        if elem['isAllowed'] and elem['type'] in ('update', 'insert', 'create', 'drop',
                                                  'alter', 'write'):
...
def _convert_hive_resource_policy_to_hdfs_policy(policy_template, context, options):
    ...
    if not options.has_key('hdfsService'):
        raise RangerSyncError('Option hdfsService must be set if expandHiveResourceToHdfs is true on a policy with database resource.')
    ...
    policy_template_hdfs['resources'] = {'path': {'values': hdfs_paths, 
                'isRecursive': True, 
                'isExcludes': False}}
