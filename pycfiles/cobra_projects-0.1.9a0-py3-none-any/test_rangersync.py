...
# Embedded file name: build/bdist.linux-x86_64/egg/tests/test_rangersync.py
...
class TestSync(unittest.TestCase):
    ...
    def test__convert_hive_access_rule_no_hdfs_service(self):
        ...
        self.assertEqual('Option hdfsService must be set if expandHiveResourceToHdfs is true on a policy with database resource.', e.exception.message)
    ...
    def test__convert_hive_access_rule_for_database_to_hdfs(self):
        policy_template_input = {'service': 'service_hive', 
           ...
           'policyItems': [
                         {'accesses': [
                            ...
                            'users': [
                                    'myuser'], 
                            'delegateAdmin': False}]}
        policy_template_expected = {'service': 'service_hdfs', 
           ...
           'resources': {'path': {'values': [
                                           '/apps/hive/warehouse/my_database.db'], 
                                  'isRecursive': True, 
                                  'isExcludes': False}}, 
           'policyItems': [
                         {'accesses': [
    ...
    def test__convert_tag_access_rule_to_both_hive_and_hdfs(self):
        policy_template_input = {'service': 'service_tag', 
           'name': 'test_policy_rule', 
           'policyType': 0, 
           'description': 'Test rule', 
           'resources': {'tag': {'values': [
                                          'visible'], 
                                 'isExcludes': False, 
                                 'isRecursive': True}}, 
           'policyItems': [
                         {'accesses': [
                            ...
                            'users': [
                                    'myuser'], 
                            'delegateAdmin': False}]}
        policy_template_expected = {'service': 'service_tag', 
           'name': 'test_policy_rule', 
           'policyType': 0, 
           'description': 'Test rule', 
           'resources': {'tag': {'values': [
