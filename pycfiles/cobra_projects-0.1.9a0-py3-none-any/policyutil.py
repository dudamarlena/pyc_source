...
# Embedded file name: build/bdist.linux-x86_64/egg/policytool/policyutil.py
...
def get_resource_type(policy):
    ...
    if policy['policyType'] != 0:
        raise AttributeError(('PolicyType must be 0 to support option expandHiveResourceToHdfs. Policy: {}').format(policy['name']))
...
def _expand_hive_tag_accesses_to_file_accesses(hive_tag_accesses):
    ...
    for elem in hive_tag_accesses:
        ...
        if elem['isAllowed'] and elem['type'] in ('hive:update', 'hive:insert', 'hive:create',
                                                  'hive:drop', 'hive:alter', 'hive:write'):
