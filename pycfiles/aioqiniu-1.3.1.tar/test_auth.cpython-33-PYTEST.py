...
class TestAuthentication:
    ...
    def call_authn_policy_methods(self, policy, request):
        ...
        if not @py_assert6:
            @py_format9 = @pytest_ar._call_reprcompare(('==', ), (@py_assert6,), ('%(py5)s\n{%(py5)s = %(py2)s\n{%(py2)s = %(py0)s.unauthenticated_userid\n}(%(py3)s)\n} == %(py8)s', ), (@py_assert4, @py_assert7)) % {'py0': @pytest_ar._saferepr(policy) if 'policy' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(policy) else 'policy',  'py3': @pytest_ar._saferepr(request) if 'request' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(request) else 'request',  'py2': @pytest_ar._saferepr(@py_assert1),  'py5': @pytest_ar._saferepr(@py_assert4),  'py8': @pytest_ar._saferepr(@py_assert7)}
        ...
        @py_assert4 = @py_assert1(request)
        @py_assert7 = ['system.Everyone', 'system.Authenticated', 'theone', 'test_user']
        @py_assert6 = @py_assert4 == @py_assert7
        if not @py_assert6:
    ...
    def yield_from_authn_policy_methods(self, policy, request):
        ...
        @py_assert3 = ['system.Everyone', 'system.Authenticated', 'theone', 'test_user']
        @py_assert2 = @py_assert0 == @py_assert3
        if not @py_assert2:
            @py_format5 = @pytest_ar._call_reprcompare(('==', ), (@py_assert2,), ('%(py1)s == %(py4)s', ), (@py_assert0, @py_assert3)) % {'py1': @pytest_ar._saferepr(@py_assert0),  'py4': @pytest_ar._saferepr(@py_assert3)}
            @py_format7 = 'assert %(py6)s' % {'py6': @py_format5}
