...
# [GCC 8.4.0]
# Embedded file name: /Users/mjjoyce/Coding/AIT/GUI/AIT-GUI/ait/gui/__init__.py
# Compiled at: 2020-04-27 18:14:26
# Size of source mod 2**32: 37624 bytes
import gevent, gevent.event, gevent.util, gevent.lock, gevent.monkey
gevent.monkey.patch_all()
import geventwebsocket, bdb
from collections import defaultdict
import pickle, importlib, json, os, struct, sys, time, urllib, webbrowser, re, bottle, pkg_resources, ait.core
...
class Playback(object):
    ...
    def _db_connect(self):
        ...
        if datastore:
            ...
            msg = '[GUI Playback Configuration]Unable to locate DataArchive plugin configuration for historical data queries. Historical telemetry playback will be disabled in monitoring UI and server endpoints.'
            log.warn(msg)
...
if ScriptRoot:
    if not os.path.isdir(ScriptRoot):
        msg = 'script.directory points to a directory that does not exist. Script loads may fail.'
        ait.core.log.warn(msg)
App = bottle.Bottle()
Servers = []
Greenlets = []
    ...
    def process(self, input_data, topic=None):
        ...
        if hasattr(self, 'telem_stream_names'):
            if topic in self.telem_stream_names:
                ...
                processed = True
        elif (processed or 'telem_stream') in topic:
            self.process_telem_msg(input_data)
            processed = True
        else:
            if topic == 'log_stream':
    ...
    try:
        with pcap.open(CMD_API.CMD_HIST_FILE, 'r') as (stream):
            if 'detailed' in bottle.request.query:
                cmds = [{'timestamp':str(header.timestamp),  'command':cmdname.decode('utf-8')} for header, cmdname in stream]
...
def handle():
    ...
    bottle.response.content_type = 'application/json'
    return json.dumps({'msgs':[str(m) for m in msgs], 
    ...
    try:
        ...
        while not wsock.closed:
            try:
                ...
                wsock.send(json.dumps({'packet':pkt_defn.name, 
...
def handle():
    ...
    tlm_query_proc = gevent.subprocess.call([
     ...
     packet,
     '--csv',
     os.path.join(HTMLRoot.Static, 'query_out.csv')] + ['{}'.format(p) for p in pcaps])
...
def bgExecSeq(bn_seqfile):
    ...
    try:
        seq_p = gevent.subprocess.Popen(['ait-seq-send', seqfile], stdout=(gevent.subprocess.PIPE))
...
def handle():
    ...
    if _RUNNING_SCRIPT is None:
        ...
        msg = 'Attempted to execute script while another script is running. Please wait until the previous script completes and try again'
        log.warn(msg)
...
def bgExecScript(script_path):
    ...
    try:
        try:
            ...
            try:
                ait.core.log.error('Script execution error: {}: {}'.format(sys.exc_info()[0].__name__, e))
...
class UIAbortException(Exception):
    __doc__ = ' Raised when user aborts script execution via GUI controls '
