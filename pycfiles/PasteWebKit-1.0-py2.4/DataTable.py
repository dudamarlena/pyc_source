...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/paste/webkit/FakeWebware/MiscUtils/DataTable.py
...
_types = {'string': StringType, 'int': IntType, 'long': LongType, 'float': FloatType, 'datetime': DateTimeType, 'object': ObjectType}
...
class TableColumn:
    ...
    def __repr__(self):
        return '<%s %s with %s at %x>' % (self.__class__.__name__, repr(self._name), repr(self._type), id(self))
...
class DataTable:
    ...
    def __init__(self, filenameOrHeadings=None, delimiter=',', allowComments=1, stripWhite=1, defaultType=None, usePickleCache=None):
        if usePickleCache is None:
            self.usePickleCache = self.usePickleCache
        else:
...
BlankValues = {StringType: '', IntType: 0, FloatType: 0.0, DateTimeType: '', None: None}
