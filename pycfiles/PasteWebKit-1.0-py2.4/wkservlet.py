...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/paste/webkit/wkservlet.py
...
class Servlet(object):
    ...
    wsgi_application = classinstance.classinstancemethod(wsgi_application)
    __traceback_supplement_hide_vars__ = [
     'config', '_session', '_request', '_response', '_methodForRequestType', '_actionDict', '_title', '_transaction']
...
class Page(HTTPServlet):
    ...
    def _respond(self, transaction):
        ...
        for action in self.actions():
            if req.hasField('_action_%s' % action) or req.field('_action_', None) == action or req.hasField('_action_%s.x' % action) and req.hasField('_action_%s.y' % action):
