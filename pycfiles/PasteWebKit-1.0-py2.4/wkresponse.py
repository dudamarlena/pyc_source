...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/paste/webkit/wkresponse.py
...
class HTTPResponse(object):
    ...
    def setHeader(self, name, value):
        ...
        assert type(name) is str, 'Header %r name is not string' % name
        assert type(value) is str, 'Header %s value is not string (%r)' % (name, value)
    ...
    def setCookie(self, name, value, path='/', expires='ONCLOSE', secure=False):
        """
    ...
    def sendRedirect(self, url):
        ...
        """
        url = resolve_relative_url(url, self._transaction.request().environ())
