...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/paste/webkit/wsgiapp.py
...
def make_webkit_app(global_conf, servlet_directory=None, package_name=None, complete_stack=True, debug=None, cookie_name='_SID_', session_file_path='/tmp', session_chmod=None, error_email=None, error_log=None, show_exceptions_in_wsgi_errors=False, from_address=None, smtp_server=None, error_subject_prefix=None, error_message=None, profile=False, profile_limit=40, **app_conf):
    ...
    if package_name:
        ...
        package = None
        package_dir = ''
        sys.stderr.write('You did not give a package or package name argument to paste.webkit.wsgiapp:make_webkit_app\n')
    ...
    app = urlparser.URLParser(global_conf, servlet_directory, url_package_name)
    ...
    if error_email is None:
        error_email = global_conf.get('error_email') or global_conf.get('sysadmin_email')
    ...
    if asbool(complete_stack):
        ...
        app = recursive.RecursiveMiddleware(app, global_conf)
        app = session.SessionMiddleware(app, global_conf, cookie_name=cookie_name, session_file_path=session_file_path, chmod=session_chmod)
        app = errormiddleware.ErrorMiddleware(app, global_conf, debug=debug, error_email=error_email, show_exceptions_in_wsgi_errors=show_exceptions_in_wsgi_errors, from_address=from_address, smtp_server=smtp_server, error_subject_prefix=error_subject_prefix, error_message=error_message)
        if debug:
            from paste.debug import prints
            app = prints.PrintDebugMiddleware(app, global_conf)
    if asbool(profile):
        from paste import profilemiddleware
        app = profilemiddleware.ProfileMiddleware(app, global_conf, limit=int(profile_limit))
    return app