...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\wlgen\__init__.py
# Compiled at: 2018-04-06 19:55:08
...
def gen_words(charset, positions=None, prev_iter=None):
    ...
    for idx, _ in enumerate(charset[iter]):
        ...
        if iter == len(charset) - 1:
            yield ''.join([charset[idx][val] for idx, val in enumerate(positions)])
        else:
