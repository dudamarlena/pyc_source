...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/chessmind/core/position.py
...
def parseCol(input):
    ...
    if not 0 <= parsed < CHESS_COLS:
        raise PositionException, 'Col out of range; %s parsed as %d.' % (input, parsed)
    return parsed
...
def parseRow(input):
    ...
    if not 0 <= parsed < CHESS_ROWS:
        raise PositionException, 'Row out of range; %s parsed as %d.' % (input, parsed)
    return parsed
...
def reverseCol(input):
    ...
    if not 0 <= input < CHESS_COLS:
        raise PositionException, 'Col out of range; %d parsed as %s.' % (input, parsed)
    return parsed
