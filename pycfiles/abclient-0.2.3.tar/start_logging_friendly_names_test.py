...
# Embedded file name: /Users/taghawi/Dropbox/workspace/abce/unittest/start_logging_friendly_names_test.py
...
class Agent(abce.Agent):
    ...
    def go(self):
        ...
        self.i = self.id
        self.r = self.round[0] + 10 * self.round[1]
        self.log('li', self.i)
        self.log('lr', self.r)
...
def main(processes, rounds):
    ...
    agents = simulation.build_agents(Agent, 'friendly_agent', agent_parameters=[{'name': 'A'}, {'name': 'Davoud'}, {'name': 'fred'}, {'name': 'F12'}])
    for rnd in range(10):
        for r in range(10):
    ...
    simulation.finalize()
    compare('aggregated_friendly_agent.csv', simulation.path, 'aggregated logging test\t\t', processes)
    compare('aggregate_friendly_agent.csv', simulation.path, 'aggregate logging test\t\t', processes)
    compare('panel_friendly_agent.csv', simulation.path, 'aggregate logging test mean\t', processes)
