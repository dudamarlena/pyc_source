...
# Embedded file name: build/bdist.linux-x86_64/egg/cloud_utils/system_utils/machine.py
# Compiled at: 2018-01-31 15:44:20
...
from cloud_utils.net_utils.sshconnection import CommandExitCodeException, SshCbReturn, SshConnection
import re, os
...
class Machine(object):
    ...
    def __init__(self, hostname, username='root', password=None, keypath=None, sshconnection=None, proxy_hostname=None, proxy_username='root', proxy_password=None, proxy_keypath=None, do_ssh_connect=True, distro=None, distro_ver=None, arch=None, timeout=120, ssh_retry=2, logger=None, log_level='INFO', verbose=True):
        """
        ...
        self.log = logger
        self.ssh_connect_kwargs = {'host': self.hostname, 'username': self.username, 
    ...
    def hostname(self, hostname):
        if self.hostname and self.hostname != hostname:
            if self.ssh and self.ssh.host != hostname:
                self.log.debug(('Hostname change from {0} -> {1}, setting self.ssh to None').format(self.hostname, hostname))
                self.ssh = None
        self._hostname = hostname
        return
    ...
    def dump_netfail_info(self, ip=None, mac=None, pass1=None, pass2=None, showpass=True, taillength=50, net_namespace=None, loglevel='debug'):
        """
        ...
        """
        buf = 'Attempting to dump network information, args: ip:' + str(ip) + ' mac:' + str(mac) + ' pass1:' + self.get_masked_pass(pass1, show=showpass) + ' pass2:' + self.get_masked_pass(pass2, show=showpass)
        if ip:
            try:
    ...
    def _get_distro_info_from_machine(self, verbose=False):
        ...
        if self.is_file('/etc/os-release'):
            try:
                ...
                return (
                 self.distro, self.distro_ver)
            except CommandExitCodeException as CE:
                self.log.warning(('Failed to fetch distro info from /etc/os-release, err:"{0}"').format(CE))

        try:
            out = self.sys('cat /etc/issue', listformat=False, code=0, verbose=verbose)
        except CommandExitCodeException as CE:
            self.log.warning(('Failed to fetch /etc/issue from machine:"{0}", err:"{1}"').format(self.hostname, str(CE)))
            out = None
    ...
    def _get_package_manager(self, verbose=False):
        ...
        raise RuntimeError(('Unable to determine package manager for machine:{0}').format(self.hostname))
            ...
            if self._do_ssh_connect:
                ...
                if port_status:
                    try:
                        ...
                        self.log.warning(('Failed to establish ssh connection with args:"{0}"').format(self.ssh_connect_kwargs))
                        raise
    ...
    def interrupt_network(self, time=120, interface=None):
        ...
        if not interface:
            raise ValueError(('interrupt_network. No interface provided, and defaults({0}) not found on system').format((', ').join(str(x) for x in defaults)))
        try:
    ...
    def sys(self, cmd, verbose=True, timeout=120, listformat=True, code=None, get_pty=True, invoke_shell=False, net_namespace=None):
        """
        ...
        return self.ssh.sys(cmd, verbose=verbose, timeout=timeout, listformat=listformat, get_pty=get_pty, invoke_shell=invoke_shell, code=code)

    def cmd(self, cmd, verbose=True, timeout=120, listformat=False, net_namespace=None, cb=None, cbargs=[]):
        """
        Issues a command against the ssh connection to this instance
    ...
    def sys_until_found(self, cmd, regex, verbose=True, timeout=120, listformat=True, net_namespace=None):
        """
        ...
        listformat -optional - specifies returned output in list of lines, or single string buffer
        """
        return self.cmd(cmd, verbose=verbose, timeout=timeout, listformat=listformat, net_namespace=net_namespace, cb=self.str_found_cb, cbargs=[regex, verbose])
    ...
    def wget_remote_image(self, url, path=None, dest_file_name=None, user=None, password=None, retryconn=True, timeout=300):
        """
    ...
    def ping_check(self, host, verbose=True, net_namespace=None):
        ...
        if out['status'] != 0:
            raise RuntimeError(('{0}\nPing to host:"{1}" returned status:{2}').format(out.get('output', None), host, out.get('status', None)))
        return out

    def ping_cmd(self, host, count=2, pingtimeout=10, commandtimeout=120, listformat=False, verbose=True, net_namespace=None):
        """
        ...
        out = self.cmd(cmd, verbose=verbose, timeout=commandtimeout, listformat=listformat, net_namespace=net_namespace)
    ...
    def show_network_ipv4_info(self, search_name=None, info_dict=None, printmethod=None, printme=True):
        if search_name and info_dict:
            self.log.warning('show_network_ipv4_info method only supports search_name or info_dict, not both')
        info_dict = info_dict or self.get_network_ipv4_info(search_name=search_name)
        header = [
         'INTERFACE', 'CIDR', 'IP', 'MASK', 'BROADCAST', 'SCOPE']
    ...
    def get_network_interfaces(self, search_name=None, proc='/proc/net/dev', verbose=False):
        ...
        for retry in xrange(0, 5):
            if retry:
                verbose = True
                self.log.debug(('Retry:{0}, attempting to fetch data from:"{1}"').format(retry, proc))
        ...
        if not out:
            raise ValueError(('Failed to fetch net interface info from "{0}", output:"{1}", retries:"{2}"').format(proc, out, retry))
        assert isinstance(out, list)
        header_line = out[0]
        headers = []
    ...
    def show_network_interfaces_table(self, search_name=None, interfaces=None, empty_header_min=4, header_min=6, printmethod=None, printme=True):
    ...
    def show_network_interfaces_delta(self, search_name=None, printmethod=None, printme=True):
        ...
        pt = self.show_network_interfaces_table(interfaces=delta.get('interfaces', {}), printme=False)
    ...
    @property
    def _mem_info(self):
        if not self._free_stats.get('stats', None) or time.time() - self._free_stats.get('last_updated', 0) > self._sys_stats_interval:
            header = []
                    ...
                    if not header:
                        ...
                        for memtype in ['mem', 'swap']:
                            if ('{0}:').format(memtype) in line:
                                free_stats[memtype] = newstats(total=line[header.index('total')], used=line[header.index('used')], free=line[header.index('free')])
                                break

                free_stats['last_updated'] = time.time()
            except Exception as CE:
                self.log.warn(('{0}\n{1}\nFailed to update mem stats:"{2}"').format(('\n').join(out or []), get_traceback(), CE))
    ...
    def _get_arch_info_from_machine(self, verbose=False):
        ...
        try:
            ...
            self.log.debug(('Failed to get arch info from:"{0}", err:"{1}"').format(self.hostname, str(UE)))
    ...
    def show_sys_info(self, mem=True, cpu=True, disk=True, print_method=None, print_table=True):
        ...
        if disk:
            ...
            for fs, info in disk_info.iteritems():
                sys_pt.add_row([os.path.basename(fs) + ' ', ('{0}/{1}').format(info.get('used'), info.get('size')), info.get('capacity')])
    ...
    @property
    def cpu_info(self):
        if not self._cpu_stats.get('stats', None) or time.time() - self._cpu_stats.get('last_updated', 0) > self._sys_stats_interval:
            header = []

            def newstats(usr, nice, sys, iowait, idle):
                return {'usr': usr, 'nice': nice, 
                   'sys': sys, 
                   'iowait': iowait, 
                   'idle': idle, 
                   'used': 100 - float(idle)}
            ...
            try:
                ...
                for line in out:
                    ...
                    if not header:
                        ...
                        if cpu != 'all':
                            cpu = int(cpu)
                        cpu_stats[cpu] = newstats(usr=line[header.index('%usr')], nice=line[header.index('%nice')], sys=line[header.index('%sys')], iowait=line[header.index('%iowait')], idle=line[header.index('%idle')])

                self._cpu_stats['last_updated'] = time.time()
            except Exception as CE:
                self.log.warn(('{0}\n{1}\nFailed to update cpu stats:"{2}"').format(('\n').join(out or []), get_traceback(), CE))
    ...
    def get_service_is_running_status(self, service, code=0):
        ...
        try:
            ...
            self.log.debug(('Could not get "{0}" service state from machine: {1}, err:{2}').format(service, self.hostname, str(e)))
    ...
    def get_elapsed_seconds_since_pid_started(self, pid):
        ...
        try:
            ...
            self.log.debug(('{0}\n"get_elapsed_seconds_since_pid_started" error: "{1}"').format(get_traceback(), str(ES)))
    ...
    def get_abs_path(self, path):
        out = self.sys(('echo "$(cd "$(dirname "{0}")" && pwd)/$(basename "{0}")"').format(path), code=0)
        if out:
    ...
    @printinfo
    def dd_monitor(self, ddif=None, ddof=None, ddcount=None, ddbs=1024, ddbytes=None, ddcmd=None, ddseek=None, timeout=300, poll_interval=1, tmpfile=None, sync=True):
        """
        ...
        if ddcmd:
            ...
            try:
                ...
                if not done:
                    ...
                    raise RuntimeError(('dd_monitor timed out before dd cmd completed, elapsed:{0}/{1}').format(str(elapsed), str(timeout)))
                elif sync:
                ...
                if not ret['dd_full_rec_out'] and not ret['dd_partial_rec_out']:
                    raise CommandExitCodeException('Did not transfer any data using dd cmd:' + str(ddcmd) + '\nstderr: ' + str(outbuf))
                if abs(int(ret['dd_full_rec_in']) - int(ret['dd_full_rec_out'])) > 1 or abs(int(ret['dd_partial_rec_out']) - int(ret['dd_partial_rec_in'])) > 1:
                    raise CommandExitCodeException(('dd in records do not match out records in transfer. full_in:{0}, part_in:{1} != full_out:{2}, part_out:{3}').format(ret['dd_full_rec_in'], ret['dd_partial_rec_in'], ret['dd_full_rec_out'], ret['dd_partial_rec_out']))
                self.log.debug(('Done with dd, copied:{0} bytes, {1} fullrecords, {2} partrecords - over elapsed:{3}').format(ret['dd_bytes'], ret['dd_full_rec_out'], ret['dd_partial_rec_out'], elapsed))
    ...
    def get_df_info(self, path=None, dfargs=None, verbose=False):
        ...
        if path is None:
            path = '${PWD}'
        elif not (self.is_dir(path) or self.is_file(path)):
            raise ValueError(('Provided path: "{0}" not found on system: "{1}"').format(path, self.hostname))
        if dfargs is None:
            dfargs = ''
        else:
    ...
    def get_available(self, path, unit=1):
        ...
        """
        if not path:
            raise ValueError(('Must supply path to get available disk. Got:"{0}/{1}"').format(path, type(path)))
