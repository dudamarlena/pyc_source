...
# Embedded file name: build/bdist.linux-x86_64/egg/cloud_admin/services/serviceconnection.py
...
import copy, errno, os, re, socket, struct, time
...
from cloud_admin.services.cluster_controller_service import EucaClusterControllerService, SHOW_CLUSTER_CONTROLLER_SERVICES
...
from cloud_admin.services.services import EucaService, EucaServiceList, EucaServiceRegResponse, EucaServiceType, SHOW_COMPONENTS, SHOW_SERVICES, SHOW_SERVICE_TYPES, SHOW_SERVICE_TYPES_VERBOSE
...
class ServiceConnection(AWSQueryConnection):
    ...
    def __init__(self, hostname, aws_access_key, aws_secret_key, path='/services/Empyrean', port=8773, is_secure=False, ec2_connection=None, boto_debug_level=0, err_method=None, logger=None, show_method=None, **kwargs):
        """
        ...
        """
        self.host = hostname
        if not isinstance(self.host, basestring) or not isinstance(aws_access_key, basestring) or not isinstance(aws_secret_key, basestring):
            raise ValueError(('Missing or invalide type for required arg. host:"{0}", aws_access_key_id:"{1}", aws_secret_access_key:"{2}"').format(self.host, aws_access_key, aws_secret_key))
        ...
        super(ServiceConnection, self).__init__(path=self.path, aws_access_key_id=aws_access_key, aws_secret_access_key=aws_secret_key, port=self.port, is_secure=self.is_secure, host=self.host, debug=self.debug, **kwargs)
    ...
    def _get_ec2_connection(self, endpoint=None, access_key=None, secret_key=None, port=None, APIVersion='2013-10-15', path=None, is_secure=None, debug_level=None, **kwargs):
        """
        ...
        if not endpoint:
            try:
                ...
                self.log.warn(('Failed to discover compute service, trying host @ "{0}"').format(self.host))
                host = self.host
        ...
        connection_args = {'aws_access_key_id': access_key, 'aws_secret_access_key': secret_key, 
           'is_secure': is_secure, 
           'debug': debug_level, 
           'port': port, 
           'path': path, 
    ...
    def _get_list_request(self, action, service=EucaService, params={}, markers=[
 'item', 'euca:item'], verb='GET'):
        """
    ...
    def get_services(self, service_type=None, show_event_stacks=None, show_events=True, list_user_services=None, listall=True, list_internal=None, service_names=None, markers=None, partition=None, service_class=EucaServiceList):
        """
        ...
        if markers is None:
            markers = [
             (
              'euca:serviceStatuses', service_class), ('serviceStatuses', service_class)]
        params = {}
        x = 0
        ...
        if service_type:
            assert isinstance(service_type, basestring), ('get_services: service_type not type basestring:{0}{1}').format(service_type, type(service_type))
            params['ByServiceType'] = str(service_type)
        if show_event_stacks:
            assert isinstance(show_event_stacks, bool), ('get_services: show_event_stacks not bool:{0}{1}').format(show_event_stacks, type(show_event_stacks))
            params['ShowEventStacks'] = str(show_event_stacks).lower()
        if show_events:
            assert isinstance(show_events, bool), ('get_services: show_events not type bool:{0}{1}').format(show_events, type(show_events))
            params['ShowEvents'] = str(show_events).lower()
        if list_user_services:
            assert isinstance(list_user_services, bool), ('get_services: list_user_services not type bool:{0}{1}').format(list_user_services, type(list_user_services))
            params['ListUserServices'] = str(list_user_services).lower()
        if listall:
            assert isinstance(listall, bool), ('get_services: listall not type bool:{0}{1}').format(listall, type(listall))
            params['ListAll'] = str(listall).lower()
        if list_internal:
            assert isinstance(list_internal, bool), ('get_services: list_internal not type bool:{0}{1}').format(list_internal, type(list_internal))
            params['ListInternal'] = str(list_internal).lower()
        if partition:
            assert isinstance(partition, basestring), ('get_services: partition not string type:{0}{1}').format(partition, type(partition))
            params['ByPartition'] = str(partition)
        service_list = self.get_list('DescribeServices', params, markers=markers, verb='GET')
        if service_list:
    ...
    def modify_service(self, service, state, verbose=True):
        ...
        if not service_name:
            raise ValueError(('modify_service: invalid service_name:"{0}/{1}"').format(service, type(service)))
        if not isinstance(state, basestring):
            raise ValueError(('modify_service: Unknown type for "state": "{0}/{1}').format(state, type(state)))
        state = str(state)
        params = {'Name': service_name, 'State': state}
        cmd_string = str(('ModifyService({0})').format((', ').join(('{0}="{1}"').format(x, y) for x, y in params.iteritems())))
        if verbose:
            self.debug_method(cmd_string)
        response = self._get_list_request(action='ModifyService', markers=markers, params=params, service=EucaServiceRegResponse)
        ...
        if response:
            ...
            raise EucaResponseException(('ERROR: {0}:\n\tStatusMessages:{1}').format(cmd_string, response.statusmessages), respobj=response)
        else:
            if verbose:
                self.debug_method(('ModifyService: Failed to parse response for: "{0}:{1}"').format(service_name, state))
            if not modified_service:
                raise RuntimeError(('Failed to fetch service: "{0}" after modify').format(service_name))
            if str(modified_service.state).lower() != str(state).lower():
                raise RuntimeError(('Modified service:"{0}" did not transition to desired state:"{1}", got:"{2}"').format(service_name, state, service.state))
        return modified_service

    def register_service(self, unique_name, service_type, service_host, partition, port='8773', verbose=True):
        """
        ...
        params = {'Type': service_type, 'Host': service_host, 'Partition': partition, 'Name': unique_name, 
           'Port': port}
        cmd_string = str(('RegisterService({0})').format((', ').join(('{0}="{1}"').format(x, y) for x, y in params.iteritems())))
        self.debug_method(cmd_string)
        response = self._get_list_request(action='RegisterService', params=params, markers=markers, service=EucaServiceRegResponse)
        if response:
            response = response[0]
            ...
            if not registered_service:
                raise EucaResponseException(('ERROR: {0}:\n\tStatusMessages:{1}').format(cmd_string, response.statusmessages), respobj=response)
        if not registered_service:
            self.log.error(('RegisterService: Failed to parse response for:"{0}"').format(cmd_string))
    ...
    def deregister_service(self, unique_name, service_type=None, verbose=True):
        ...
        cmd_string = str(('DeregisterService({0})').format((', ').join(('{0}="{1}"').format(x, y) for x, y in params.iteritems())))
        self.debug_method(cmd_string)
        response = self._get_list_request(action='DeregisterService', params=params, markers=markers, service=EucaServiceRegResponse)
        if response:
            response = response[0]
            if response.services:
            ...
            if not deregistered_service:
                raise EucaResponseException(('ERROR: {0}:\n\tStatusMessages:{1}').format(cmd_string, response.statusmessages), respobj=response)
        if not deregistered_service:
            self.log.error(('DeregisterService: Failed to parse response for:"{0}"').format(cmd_string))
    ...
    def get_cloud_controller_service(self, name):
        ...
        raise EucaNotFoundException('get_cloud_controller_service. CLC not found for args:', notfounddict={'name': name})
    ...
    def get_cluster_controller_service(self, name):
        ...
        raise EucaNotFoundException('get_cluster_controller_service. CC not found for args:', notfounddict={'name': name})
    ...
    def get_object_storage_gateway_service(self, name):
        ...
        raise EucaNotFoundException('get_object_storage_gateway_service. OSG not found for args:', notfounddict={'name': name})
    ...
    def get_storage_controller_service(self, name):
        ...
        raise EucaNotFoundException('get_storage_controller_service. SC not found for args:', notfounddict={'name': name})
    ...
    def get_walrus_backend_service(self, name):
        ...
        raise EucaNotFoundException('get_walrus_backend_service. WS not found for args:', notfounddict={'name': name})
    ...
    def get_unified_frontend_service(self, name):
        ...
        if ufss:
            ...
            if ufs.name == name:
                return ufs
        raise EucaNotFoundException('get_unified_frontend_service. UFS not found for args:', notfounddict={'name': name})
    ...
    def get_dns_service(self, name):
        ...
        raise EucaNotFoundException('get_dns_service. DNS not found for args:', notfounddict={'name': name})

    def get_all_node_controller_services(self, get_instances=True, fail_on_instance_fetch=False, filter_name=None, filter_fullname=None, partition=None):
        r"""
...
\       """
        services = self.get_services(service_type='node', listall=True, list_internal=False, partition=partition, show_event_stacks=False, show_events=True, list_user_services=False, service_class=EucaServiceList)
        nodes = []
        for service in services:
        ...
        if get_instances:
            try:
                reservations = self.ec2_connection.get_all_instances(instance_ids=[
                 'verbose'], filters={'tag-key': 'euca:node'})
            ...
            for reservation in reservations:
                for vm in reservation.instances:
                    try:
                        ...
                        self.log.debug(get_traceback())
                        self.log.warn(('Failed to fetch instances for node:{0}, err:{1}').format(node.name, str(NE)))
                        if fail_on_instance_fetch:
                            raise NE

        return nodes

    def get_node_controller_service(self, name=None, fullname=None, partition=None, get_instances=True, fail_on_instance_fetch=False):
        """
        ...
        if not (name or fullname):
            raise ValueError(('No filters provided. name="{0}", fullname="{1}", partition="{2}"').format(name, fullname, partition))
        nodes = self.get_all_node_controller_services(filter_name=name, filter_fullname=fullname, partition=partition, get_instances=get_instances, fail_on_instance_fetch=fail_on_instance_fetch)
        if not nodes:
            raise EucaNotFoundException('get_node_controller_service: Not Found, args:', notfounddict={'name': name, 'fullname': fullname, 'partition': partition})
        ...
        if node_count != 1:
            ...
            raise EucaNotFoundException('get_all_node_controller_services: Multiple nodes Matchfor filter args:', notfounddict={'name': name, 'fullname': fullname, 
               'partition': partition})
        return nodes[0]
    ...
    def show_cluster_controllers(self, ccs=None, print_method=None, print_table=True):
        ...
        """
        return SHOW_CLUSTER_CONTROLLER_SERVICES(self, ccs=None, print_method=print_method, print_table=True)
    ...
    def show_storage_controllers(self, scs=None, print_method=None, print_table=True):
        ...
        :param print_table: bool, if True will print table to connection.debug_method()
                           if False will return table object
        """
        return SHOW_COMPONENTS(self, scs, self.get_all_storage_controller_services, print_method=print_method, print_table=print_table)
    ...
    def show_objectstorage_gateways(self, osgs=None, print_method=None, print_table=True):
        ...
        :param print_table: bool, if True will print table to connection.debug_method()
                           if False will return table object
        """
        return SHOW_COMPONENTS(self, osgs, self.get_all_object_storage_gateway_services, print_method=print_method, print_table=print_table)
    ...
    def show_cloud_controllers(self, clcs=None, print_method=None, print_table=True):
        ...
        :param print_table: bool, if True will print table to connection.debug_method()
                           if False will return table object
        """
        return SHOW_COMPONENTS(self, clcs, self.get_all_cloud_controller_services, print_method=print_method, print_table=print_table)
    ...
    def show_walrus_backends(self, walruses=None, print_method=None, print_table=True):
        ...
        :param print_table: bool, if True will print table to connection.debug_method()
                           if False will return table object
        """
        return SHOW_COMPONENTS(self, walruses, self.get_all_walrus_backend_services, print_method=print_method, print_table=print_table)
    ...
    def show_components_summary(self, print_method=None, print_table=True):
        ...
        return SHOW_COMPONENTS(self, components=components, print_method=print_method, print_table=print_table)
    ...
    def migrate_instances(self, instance_id=None, source_host=None, include_dest=None, exclude_dest=None):
        ...
        if include_dest:
            ...
            markers = [
             'euca:MigrateInstancesResponseType', 'MigrateInstancesResponseType']
    ...
    def get_service_certs(self, version='eucalyptus', digest='SHA-256', certformat='pem'):
        ...
        return self._get_list_request(action='DescribeServiceCertificates', service=ServiceCertificate, params=params)
    ...
    def show_machine_mappings(self, machine_dict=None, partition=None, service_type=None, columns=4, print_method=None, print_table=True):
        ...
        total = []
        machines = machine_dict or self.get_all_machine_mappings(partition=partition, service_type=service_type)
        if not isinstance(machines, dict):
            raise ValueError(('show_machine_mappings requires dict example: {"host ip":[services]}, got:"{0}/{1}"').format(machines, type(machines)))
        ...
        spt_lines = servpt.get_string(border=0, padding_width=2, fields=servpt._field_names[0:columns]).splitlines()
        serv_lines = []
            ...
            for serv in mservices:
                for line in serv_lines:
                    ...
                    if splitline and re.match(('^{0}$').format(serv.type), line_type) and re.match(('^{0}$').format(serv.name), line_name):
                        if line not in servbuf:
                            servbuf += line + '\n'

                if serv.type == 'node' and getattr(serv, 'instances', None):
                    servbuf += '\n' + markup('INSTANCES', [1, 4]) + ' \n'
                    for x in serv.instances:
                        servbuf += ('{0}{1}{2}{3}').format(str(x.id).ljust(ins_id_len), str('(' + x.state + '),').ljust(ins_st_len), str(x.instance_type + ',').ljust(ins_type_len), str(x.root_device_type).ljust(ins_dev_len)).ljust(ins_total).strip() + '\n'

            pt.add_row([
             ('{0}\n{1}').format(str('').rjust(machine_hdr[1]), markup(machine, [1, 4, 94])),
             servbuf])
    ...
    def get_all_clusters(self, cluster_name=None):
        ...
        if cluster_name:
            if cluster_name not in cluster_names:
                raise ValueError(('Cluster name:{0} not in cluster names:{1}').format(cluster_name, (', ').join(str(x) for x in cluster_names)))
            cluster_names = [
             cluster_name]
    ...
    def wait_for_service(self, service, states=None, partition=None, attempt_both=True, interval=20, timeout=600):
        """
        ...
        if not states:
            ...
            if not service_type:
                raise ValueError(('wait_for_service. Unknown service type for:"{0}:{1}"').format(service, type(service)))
            self.debug_method(('Waiting for service type:{0} to go to States:{1}').format(service_type, state_info))
            start = time.time()
            elapsed = 0
            while elapsed < timeout:
                elapsed = int(time.time() - start)
                try:
                    matching_services = self.get_services(service_type=service_type, partition=partition) or []
                    if matching_services:
                        self.show_services(services=matching_services)
                    else:
                        err_msg = ('No service registered of type:"{0}", partition:"{1}", elapsed:{2}/{3}').format(service_type, partition, elapsed, timeout)
                    ...
                    for service in matching_services:
                        ...
                        elif re.search(state, service.state):
                            return service

                except Exception as E:
                    err_msg = ('Error while fetching services:"{0}:{1}", elapsed:{2}/{3}\nRetrying in "{4}" seconds').format(type(E), str(E), elapsed, timeout, interval)
        ...
        msg = ("{0}\nERROR: Service_type:'{1}', partition:{2} did not enter state(s):'{3}'").format(err_msg, service_type, partition, state_info)
