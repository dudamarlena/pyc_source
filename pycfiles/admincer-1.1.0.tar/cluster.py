...
# Embedded file name: build/bdist.linux-x86_64/egg/cloud_admin/services/cluster.py
...
class Cluster(object):
    ...
    def __init__(self, connection, name):
        ...
        if name not in cluster_names:
            raise ValueError(('Cluster Name:{0} was not in system:"{1}"').format(name, (', ').join(str(x) for x in cluster_names)))
        self.name = name
        self._machines_dict = {}
    ...
    def show_machine_mappings(self, printmethod=None, print_table=True):
        ...
        if print_table:
            printmethod(('\n{0}\n').format(pt.get_string(sortby=pt.field_names[1], reversesort=True)))
        else:
            return pt

    def show_properties(self, prefix=None, show_description=True, grid=True, printmethod=None, print_table=True):
        printmethod = printmethod or self.connection._show_method
        show_list = []
        if prefix:
        ...
        print ''
        self.connection.show_properties(show_list, description=show_description, grid=grid, printmethod=printmethod, print_table=print_table)
