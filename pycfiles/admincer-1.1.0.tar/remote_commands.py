...
# Embedded file name: build/bdist.linux-x86_64/egg/cloud_utils/net_utils/remote_commands.py
...
class RemoteCommands(object):
    ...
    def __init__(self, hostfile=None, ips=None, password=None, keypath=None, username='root', command='echo "ALIVE', timeout=5, no_pty=False, thread_count=20, log_level='debug'):
        self.parser = argparse.ArgumentParser(description='Run a command on list of remote hosts', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
        self.parser.add_argument('-f', '--hostfile', default=hostfile, help='file with list of ips and/or hostnames')
        self.parser.add_argument('-i', '--ips', default=ips, help='comma or space separated list of ips and/or hostnames')
        self.parser.add_argument('-p', '--password', default=password, help='Ssh password used to connect to hosts')
        self.parser.add_argument('-k', '--keypath', default=keypath, help='Local path to specific ssh key used to connect to hosts')
        self.parser.add_argument('-u', '--username', default=username, help='Ssh username used to connect to hosts')
        self.parser.add_argument('-c', '--command', default=command, help='file with list of ips and/or hostnames')
        self.parser.add_argument('-t', '--timeout', default=timeout, type=int, help='Ssh connection timeout in seconds')
        self.parser.add_argument('-b', '--batch-timeout', default=0, type=int, help='Timeout for sum of all tasks to complete in seconds. This includes time to create all remote connections + execute commands')
        self.parser.add_argument('--thread-count', default=thread_count, type=int, help='Number of threads used to run commands on hosts')
        self.parser.add_argument('--no-pty', default=no_pty, action='store_false', help='Do not request a pseudo-terminal from the server.')
        self.parser.add_argument('-l', '--log-level', default=log_level, help='Loglevel')
        if ips or hostfile:
            args = ''
        else:
            args = None
        ...
        if not self.ips:
            raise ValueError('No hosts provided. Use --hostfile or --ips to provide hosts to run command against')
        return
                ...
                try:
                    ...
                    try:
                        try:
                            ...
                            ssh = SshConnection(host=host, username=self.username, password=self.password, keypath=self.keypath, debug_connect=True, timeout=self.args.timeout, verbose=True, logger=logger)
                            logger.debug(('host: {0} running command:{1} ').format(host, command))
                            out = ssh.cmd(str(command), listformat=True, timeout=self.args.timeout, get_pty=not self.args.no_pty)
                            logger.debug(('Done with host: {0}').format(host))
                            with lock:
                                self.results[host] = {'status': out.get('status'), 'output': out.get('output'), 'elapsed': int(time.time() - start)}
                        except Exception as E:
                            err = ('{0}\n{1}').format(get_traceback(), E)
                            with lock:
                                self.results[host] = {'status': -1, 'output': [err], 'elapsed': int(time.time() - start)}
    ...
    def run_remote_commands(self, ips=None, command=None):
        ...
        if iq.unfinished_tasks:
            self.logger.warning(red(('Possible unfinished tasks detected after elapsed:{0}. Queue:{1}').format(time.time() - start, iq.queue)))
            time.sleep(0.1 * len(ips))
            for ip in ips:
                with tlock:
                    if ip not in self.results.keys():
                        self.results[ip] = {'status': -1, 'output': [
                                    ('Timed out after {0} seconds').format(int(self.args.batch_timeout))], 
                           'elapsed': int(self.args.batch_timeout)}

        self.logger.debug('Done with join')
        time.sleep(self.maxwait + 0.1)
        return self.results
        ...
        for host in sorted(results, key=sort_meth):
            ...
            for line in result.get('output'):
                ...
                for x in xrange(0, len(line), max_width - 1):
                    part = str(('{output: <{length}}').format(output=line[x:x + max_width - 1], length=max_width))
                    output += part
