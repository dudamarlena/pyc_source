...
# Embedded file name: build/bdist.linux-x86_64/egg/cloud_admin/backends/network/midget.py
...
import json, pkg_resources, socket, time
...
class Midget(object):
    ...
    def __init__(self, midonet_api_host, midonet_api_port='8080', midonet_username=None, midonet_password=None, clc_ip=None, clc_password=None, systemconnection=None, clc_tunnel=True, clc_tunnel_host='127.0.0.1', mido_log_level='INFO', euca_log_level='INFO'):
        """
        ...
        self.log = Eulogger(identifier=('MidoDebug:{0}').format(self.midonet_api_host), stdout_level=mido_log_level, parent_logger_name=self.__class__.__name__)
        if clc_tunnel:
            clc_ip = clc_ip or midonet_api_host
            self.midonet_api_host = clc_tunnel_host
            api_lib.do_request = self.tunneled_request
        self.mapi = MidonetApi(base_uri=('http://{0}:{1}/midonet-api').format(self.midonet_api_host, self.midonet_api_port), username=self.midonet_username, password=self.midonet_password)
        self.eucaconnection = systemconnection
        if not self.eucaconnection:
            clc_ip = clc_ip
            self.eucaconnection = SystemConnection(hostname=clc_ip, password=clc_password, log_level=euca_log_level)
        try:
    ...
    def check_mido_client_to_server_versions(self, rpm_string=None):
        mido_rpm = rpm_string
        if not mido_rpm:
            mido_rpm = self.eucaconnection.clc_machine.sys('rpm -qa midonet-cluster', listformat=False).strip()
        if not mido_rpm:
            mido_rpm = self.eucaconnection.clc_machine.sys('rpm -qa midolman', listformat=False).strip()
        if not mido_rpm:
            self.log.warning(red(('Midonet may not be installed on the expected host:{0}').format(self.eucaconnection.clc_machine.hostname)))
            return
        else:
            client_versions = versions('midonetclient')
            if not client_versions:
                self.log.warning(red('No midonetclient python client versions found to check againstinstalled pkgs?'))
                return
            current_client_ver = pkg_resources.get_distribution('midonetclient').version
            matches = []
            client_nums = []
            max = 0
            self.log.debug(('Looking for best client fit. Client vers:"{0}", pkg ver:"{1}"').format((', ').join(client_versions), mido_rpm))
            ...
            self.log.debug(('Pkg:"{0}", possible clients matches:"{1}"').format(mido_rpm, (',').join(matches)))
            if current_client_ver in matches:
                self.log.debug(('Current midonet client ver:{0} seems to be a good fit for:{1}').format(current_client_ver, mido_rpm))
            else:
                self.log.warning(red(('Mido version:"{0}" may not be compatible with Midolman pkg version found:{1}.\nSee available versions using: "pip search midonetclient"\n"Install with:\n"pip install midonetclient==<correct version>"').format(current_client_ver, mido_rpm)))
            return
    ...
    def tunneled_request(self, uri, method, body=None, query=None, headers=None, ssh_host=None, depth=0, max_redirects=5, mido_json_ver=None, mido_api_request_type=None, *args, **kwargs):
        """
        ...
        if header_value:
            if uri and not mido_api_request_type:
                uri_match = re.search('(http.*midonet-api)/(\\w+)/*(.*$)', uri)
                if uri_match:
                    uri_type = uri_match.groups()[1]
                    h_match = re.search('\\.(\\w+)-v\\d\\+json$', header_value)
                    if h_match:
            ...
            if mido_api_request_type:
                ...
                if match:
                    ...
                    if mido_json_ver is not None and mido_json_ver != original_version:
                        new_string = header_value.replace(('v{0}+json').format(original_version), ('v{0}+json').format(mido_json_ver))
                        headers[header_key] = new_string
                        trying_version = mido_json_ver
        if depth > max_redirects:
            raise ValueError(('Request detph:{0} has exceed max_redirects:{1}, uri:{2}').format(depth, max_redirects, uri))
        ssh_host = ssh_host or self.eucaconnection.clc_machine.ssh
        if not ssh_host:
            raise ValueError('tunneled request requires an SshConnection. None provided and could not find clc ssh host in self.eucaconnection')
        else:
            if not isinstance(ssh_host, sshconnection.SshConnection):
                raise ValueError(('ssh_host must be of type SshConnection, got: "{0}/{1}').format(ssh_host, type(ssh_host)))
            query = query or dict()
            ...
            self.log.debug(('tunneled request:mido_request_type:{0}, original ver:{1}, trying ver:{2}').format(mido_api_request_type, original_version, trying_version))
            if query:
                uri += '?' + urlencode(query)
            data = json.dumps(body) if body is not None else '{}'
            try:
                response = ssh_host.http_fwd_request(url=uri, method=method, body=data, headers=headers)
                ...
                self.log.error(('{0}\nCould not send request to midonet-api, err:"{1}"').format(get_traceback(), CSE))
                raise mido_exc.MidoApiConnectionRefused()
        ...
        if int(status == 302):
            ...
            if location and location != uri:
                ...
                depth += 1
                return self.tunneled_request(uri=location, method=method, body=body, query=query, headers=headers, ssh_host=ssh_host, depth=depth, max_redirects=max_redirects)
        if int(status) > 400:
            error = http_errors.get(str(status), None)
            if not error:
                error = RuntimeError
            self.log.error('Got http error(response=%r, content=%r) for request(uri=%r, method=%r, body=%r, query=%r,headers=%r). Exception type=%r' % (
             response, content, uri, method, body,
             query, headers, error))
            if int(status) == 406:
                if trying_version and trying_version > 0:
                    new_version = int(trying_version) - 1
                    self.log.error(red(('ErrorCode:406.  Request type:{0}, Current Ver:{1}, Retrying with version:{2}').format(mido_api_request_type, trying_version, new_version)))
                    return self.tunneled_request(uri=uri, method=method, body=body, query=query, headers=headers, ssh_host=ssh_host, depth=depth, max_redirects=max_redirects, mido_api_request_type=mido_api_request_type, mido_json_ver=new_version)
            raise error
        if trying_version:
            self._api_version_cache[mido_api_request_type] = trying_version
        return (response, from_json(content))

    def mido_cli_cmd(self, cmd, ssh=None, midonet_url='http://127.0.0.1:8080/midonet-api', listformat=True, verbose=True, no_auth=True, username=None, password=None, tenant=None):
        """
        ...
        try:
            ...
            raise ValueError('sshconnection object was not provided and not found in thelocal eucaconnection object')
    ...
    def _highlight_buf_for_instance(self, buf, instance):
        ...
        for line in str(buf).splitlines():
            searchstring = ('{0}|{1}|{2}').format(instance.id, instance.private_ip_address, instance.ip_address)
            try:
                searchstring = ('{0}|{1}').format(searchstring, self.eucaconnection.ec2_connection.get_all_subnets(instance.subnet_id)[0].cidr_block)
            except:
                pass
    ...
    def _get_instance(self, instance):
        ...
        if not isinstance(instance, Instance):
            if isinstance(instance, basestring):
                ...
                raise ValueError(('instance not type boto Instance nor instance id string. instance: "{0}:{1}"').format(instance, type(instance)))
            if not fetched_ins:
                raise ValueError(('Could not find instance {0} on system').format(instance))
            instance = fetched_ins
        return instance

    def ping_instance_private_ip_from_euca_internal(self, instance, proxy_machine=None, net_namespace=None, verbose=False, ping_timeout=5):
        """
        ...
        out = proxy_machine.ping_check(instance.private_ip_address, net_namespace=net_namespace, verbose=verbose)
        status = out.get('status', None)
        if status == 0:
            statusmsg = 'Success(0)'
        else:
            statusmsg = ('Failed({0})').format(status)
        self.info(('\nCmd:"{0}", output:\n{1}\n{2} pinging instance: {3},  private ip:{4} from internal host: {5}').format(out.get('cmd', None), out.get('output', None), statusmsg, instance.id, instance.private_ip_address, proxy_machine.hostname))
        return status

    def dump_euca_gateway_info_for_instance(self, instance, proxy_machine=None, net_namespace=None, loglevel='debug'):
        """
        ...
        if not proxy_machine:
            proxy_machine = self.eucaconnection.clc_machine
        net_namespace = net_namespace or instance.vpc_id
        buf = ('Dumping info for instance: "{0}",  private ip:"{1}" from Mido machine: "{2}"').format(instance.id, instance.private_ip_address, proxy_machine.hostname)
        buf += 'Fetching network debug info from internal host...'
        buf += proxy_machine.dump_netfail_info(ip=instance.private_ip_address, net_namespace=net_namespace, loglevel=None)
        buf += ('Done fetching/logging network debug info from internal euca proxy hostfor instance {0}, private ip: {1}, from internal host: {2}').format(instance.id, instance.private_ip_address, proxy_machine.hostname)
        if loglevel:
            ...
            for router in routers:
                if hasattr(router, key):
                    try:
                        ...
                        self.info(('Error while evaluating -> {0}("{1}","{2}")').format(('{0}.{1}').format(getattr(eval_op, '__module__', ''), getattr(eval_op, '__name__', '')), str(search_dict[key]), str(getattr(router, key))))
                        raise
    ...
    def get_router_for_instance(self, instance):
        ...
        if len(routers) != 1:
            raise ValueError(('Expected to find 1 matching router for instance:{0}, found:{1}').format(instance.id, routers))
        router = routers[0]
    ...
    def show_routers_brief(self, routers=None, showchains=False, printme=True):
        ...
        for router in routers:
            row = [
             router.get_name(), router.get_admin_state_up(), router.get_id(),
    ...
    def show_routes(self, routes, printme=True):
        ...
        for route in routes:
            pt.add_row([
             ('{0}/{1}').format(route.get_dst_network_addr(), route.get_dst_network_length()),
             ('{0}/{1}').format(route.get_src_network_addr(), route.get_src_network_length()),
    ...
    def show_routers(self, routers=None, loglevel='info', printme=True):
        ...
        if routers is None:
            ...
            if isinstance(routers, basestring):
            ...
            for router in routers:
                if router:
                    buf += ('\n{0}\n').format(self.show_router_summary(router, showchains=False, printme=False))
    ...
    def get_device_by_peer_id(self, peerid):
        ...
        if not device:
            raise ValueError(('Unknown device type for peerid:{0}, port:{1}, type:{2}').format(peerid, port.get_id(), port.get_type()))
        return device
    ...
    def get_bridge_for_instance(self, instance):
        ...
        if bridge:
            return bridge
        self.log.debug(('Did not find bridge by name:"{0}", trying by port, cidr...').format(bridge_name))
        router = self.get_router_for_instance(instance)
        if not router:
            raise ValueError(('Did not find router for instance:{0}').format(instance.id))
        subnet = self.eucaconnection.ec2_connection.get_all_subnets(subnet_ids=[
         'verbose', instance.subnet_id])[0]
        if not subnet:
            raise ValueError(('Did not find subnet for instance:{0}, subnet id:{1}').format(instance.id, instance.subnet_id))
        port = self.get_router_port_for_subnet(router, subnet.cidr_block)
        if not port:
            raise ValueError(('Did not find router port for instance:{0}, subnet:{1}').format(instance.id, subnet.cidr_block))
        bridge = self.get_device_by_peer_id(port.get_peer_id())
        if not isinstance(bridge, Bridge):
            raise ValueError('peer device for instance router is not a bridge, fix the assumptions made in this method!')
        return bridge
    ...
    def get_bgps_for_port(self, port):
        ...
        if hasattr(port, 'get_bgps'):
            ...
            header = getattr(vendor_media_type, 'APPLICATION_BGP_COLLECTION_JSON', None) or 'application/vnd.org.midonet.collection.Bgp-v1+json'
            bgps = port.auth.do_request(headers={'Accept': header}, method='GET', uri=port.dto.get('bgps'))
        return bgps
    ...
    def show_port_summary(self, port, showchains=True, showbgp=True, indent=None, printme=True):
        ...
        if port.dto.get('networkAddress', None) is not None:
            netaddr = ('{0}/{1}').format(port.dto.get('networkAddress', None), port.dto.get('networkLength', None))
        else:
            netaddr = None
        ...
        if showbgp and bgps:
            if port.dto.get('bgpStatus', None):
                ...
                buf += self._indent_table_buf(str(self.show_bgps(self.get_bgps_for_port(port), printme=False)))
        if showchains:
        ...
        if ports:
            ...
            for port in ports:
                ...
                pt.add_row([port.get_id(),
                 bgps,
                 port.dto.get('portAddress', None),
                 ('{0}/{1}').format(port.dto.get('networkAddress', None), port.dto.get('networkLength', None)),
                 port.dto.get('portMac', None),
                 port.get_type(),
                 port.get_admin_state_up(),
                 port.get_peer_id()])
                ...
                if bgps and bgps != 'ERROR' or verbose and (outbound_filter_id or inbound_filter_id):
                    lines = []
                    ...
                    if outbound_filter_id:
                        buf += self._link_table_buf(('PORT:{0} OUTBOUND FILTER:\n{1}').format(port.get_id(), self.show_chain(self.mapi.get_chain(outbound_filter_id), printme=False)))
                    if inbound_filter_id:
                        buf += self._link_table_buf(('PORT:{0} INBOUND FILTER:\n{1}').format(port.get_id(), self.show_chain(self.mapi.get_chain(inbound_filter_id), printme=False)))
                    if bgps:
                        if port.dto.get('bgpStatus', None):
                            buf += self._link_table_buf(str(port.dto.get('bgpStatus')))
                        else:
                            buf += self._link_table_buf(self.show_bgps(self.get_bgps_for_port(port), printme=False))
    ...
    def show_bgp_hosts_for_euca_router(self, router_name='eucart', printmethod=None, printme=True):
        ...
        for port in bgp_ports:
            ...
            if host:
                ...
                self.log.error(('No host found for port:{0} using host_id:{1}').format(port.get_id(), port.get_host_id()))
    ...
    def show_bgps(self, bgps, printme=True):
        ...
        for bgp in bgps:
            ...
            status_pt = PrettyTable([
             ('PORT HOST:{0} ({1} : {2} : {3}/{4})').format(hostname, interface_name, port.dto.get('portAddress', None), port.dto.get('networkAddress', None), port.dto.get('networkLength', None))])
            status_pt.align = 'l'
            status_pt.vrules = 2
            status_pt.hrules = 3
    ...
    def _update_euca_instances(self):
        instances = []
        reservations = self.eucaconnection.ec2_connection.get_all_instances(instance_ids=[
         'verbose'])
    ...
    def show_bridge_for_euca_artifact(self, instance=None, subnet=None, eni=None, indent=None, printme=True):
        if instance:
    ...
    def get_bridge_arp_table(self, bridge):
        table = bridge.get_children(bridge.dto['arpTable'], query=None, headers={'Accept': ''}, clazz=ArpTable)
        return table

    def get_bridge_mac_table(self, bridge):
        table = bridge.get_children(bridge.dto['macTable'], query=None, headers={'Accept': ''}, clazz=MacTable)
        return table
    ...
    def get_bridge_port_for_instance_learned(self, instance):
        ...
        if arp_entry:
            ...
            self.info('ARP entry for instance found, but mac has not been learned on a port yet, try pinging it?   ')
        return
    ...
    def show_bridge_port_for_instance(self, instance, showchains=True, indent=None, printme=True):
        ...
        title = self._bold(('BRIDGE PORT FOR INSTANCE:{0}, (BRIDGE:{1}), {2}').format(instance.id, bridge.get_name() or bridge.get_id(), learned), 94)
        pt = PrettyTable([title])
        pt.align[title] = 'l'
        buf = ''
        if port:
    ...
    def show_chain(self, chain, printme=True):
        ...
        if not isinstance(chain, Chain):
            raise ValueError(("Unsupported type passed to show_chain, chain:'{0}:{1}'").format(chain, type(chain)))
        title = ('CHAIN NAME:{0}, ID:{1} TENANT ID:{2}').format(chain.dto.get('name', 'NA'), self._bold(chain.get_id(), self._CHAIN_JUMP), chain.dto.get('tenantId', ''))
        pt = PrettyTable([title])
        pt.align[title] = 'l'
    ...
    def show_chains_for_instance_security_groups(self, instance, printme=True):
        ...
        mainbuf = self._bold(("\nSECURITY GROUP/MIDO CHAIN RULE MAPPING for INSTANCE: '{0}'").format(instance.id))
        ...
        if isinstance(group, SecurityGroup) or isinstance(group, BotoGroup):
            ...
            if isinstance(group, str) or isinstance(group, unicode):
                if re.match('^sg-\\w{8}$', group):
                    ...
                    self._errmsg(('Could not find security group:"{0}" on cloud? Trying to lookupmidonet chain anyways...').format(group))
                    group_id = str(group)
                else:
                    group = self.get_security_group(name=group)
                    if not group:
                        raise ValueError(('Group not found on system and not could not performa chain lookup because group was not provided in id format, ie:"sg-XXXXXXXX", group:"{0}"').format(group))
        if not group_id:
            raise RuntimeError(('Group id is none, lookup failed for provided group arg:"{0}"').format(group))
        chain_name = 'sg_ingress_' + str(group_id)
        chain = self.get_chain_by_name(name=chain_name)
        if not chain:
            self._errmsg('Chain lookup failed, this could be expected if security group is extant and no running are referencing it')
        else:
            return chain
        return
    ...
    def does_chain_allow(self, chain, src_addr, protocol, port):
        if not isinstance(chain, Chain):
            raise ValueError(('does_chain_allow passed non Chain type:"{0}:{1}"').format(chain, type(chain)))
        if not src_addr or not protocol or not port:
            raise ValueError(('Missing or empty arg not provided; src_addr:"{0}", protocol:"{1}", port:"{2}"').format(src_addr, protocol, port))
            ...
            if protocol == str(rule_protocol).upper().strip():
                if port >= start_port and port <= end_port:
                    if rule_cidr == '0.0.0.0/0' or is_address_in_network(src_addr, rule_cidr):
                        self.info(('Found rule which allows src_addr:"{0}", protocol:"{1}", port:"{2}"').format(src_addr, protocol, port))
                        self.show_rules(rules=[rule])
                        return True

        self.info(('Chain does not allow: src_addr:"{0}", protocol:"{1}", port:"{2}"').format(src_addr, protocol, port))
        return False
    ...
    def get_unsynced_rules_for_security_group(self, group, show_rules=True):
        ...
        self.info(('{0} unsynced rules out of {1} total rules found for group:"{2}"').format(len(unsynced_rules), len(group.rules), group.name))
        if unsynced_rules and show_rules:
            title = markup(('The following rules for group:"{0}" were not found) on backend').format(group.name), [1, 91, 7])
    ...
    def get_unsynced_rules_for_instance(self, instance, show_rules=True):
        ...
        for group in instance.groups:
            ...
            unsynced_rules.extend(self.get_unsynced_rules_for_security_group(group, show_rules=show_rules))

        self.info(('Number of unsynced rules found for instance ({0}):{1}').format(instance.id, len(unsynced_rules)))
        return unsynced_rules
    ...
    def get_security_group_rule_mapping_from_backend(self, group, security_group_rule):
        ...
        """
        ret_rules = []
        assert isinstance(security_group_rule, IPPermissions), ('security_group_rule arg must be of type boto.IPPermissions, got:"{0}:{1}').format(security_group_rule, type(security_group_rule))
        chain = self.get_chain_for_security_group(group)
        ip_grants = copy.copy(security_group_rule.grants)
        protocol = security_group_rule.ip_protocol and str(security_group_rule.ip_protocol).upper() or None
        ...
        for grant in security_group_rule.grants:
            self.info(self._bold(('Looking for chain rule against grant: cidr_ip:"{0}", srg_grp:"{1}", proto:"{2}", start_port:"{3}", end_port:"{4}"').format(grant.cidr_ip, grant.group_id, security_group_rule.ip_protocol, security_group_rule.from_port, security_group_rule.to_port)))
            for rule in chain.get_rules():
                match = False
                ...
                self.info(('checking protocol:"{0}" vs rule_proto:"{1}"').format(protocol, r_protocol))
                if not protocol == r_protocol:
                    continue
                port_dict = rule.get_tp_dst() or {}
                start_port = port_dict.get('start')
                end_port = port_dict.get('end')
                self.info(('Protocol matched, checking fport:{0} vs sport:{1}, and tport:{2} vs eport:{3}').format(from_port, start_port, to_port, end_port))
                ...
                if ip_addr_grp_id:
                    ...
                    if ip_addr_grp:
                        ...
                        self.info(('This rule has ipaddr group name:"{0}"').format(ip_addr_grp_name))
                self.info(('checking grant.cidr_ip:"{0}" vs rule_cidr:"{1}"').format(grant.cidr_ip, rule_cidr))
                if grant.cidr_ip and str(grant.cidr_ip) == rule_cidr:
                    match = True
                elif grant.group_id and str(grant.group_id) in ip_addr_grp_name:
                    match = True
                if match:
                    self.info(('Found rule for cidr_ip:"{0}", srg_grp:"{1}", proto:"{2}", start_port:"{3}", end_port:"{4}"').format(grant.cidr_ip, grant.group_id, protocol, from_port, to_port))
        ...
        self.info(('Found "{0}" rules for; Group:"{1}", ip_grants:"{2}", proto:"{3}", start_port:"{4}", end_port:"{5}"').format(len(ret_rules), group, security_group_rule.grants, protocol, from_port, to_port))
        if ret_rules:
    ...
    def do_instance_rules_allow(self, instance, src_addr, protocol, port):
        for group in instance.groups:
            chain = self.get_chain_for_security_group(group)
            self.info(('Checking midonet chain:"{0}" for instance:"{1}", security group "{2}"').format(chain.get_name(), instance.id, group.name))
    ...
    def show_rules(self, rules, jump=False, printme=True):
        ...
        for rule in rules:
            if pt is None:
                ...
                title = ('RULES FOR CHAIN:{0}').format(('..{0}{1}').format(chain_id[-5:-1], chain_id[(-1)]))
            ...
            pt.add_row([
             ('{0} {1}').format(self._bold('RULE#:' + str(rules.index(rule) + 1)).ljust(title_width), rule.get_id()),
             ('{0}{1}/{2}').format(invert(rule.is_inv_nw_src), rule.get_nw_src_address(), rule.get_nw_src_length()),
             ('{0}{1}/{2}').format(invert(rule.is_inv_nw_dst), rule.get_nw_dst_address(), rule.get_nw_dst_length()),
             ('{0}{1}').format(invert(rule.is_inv_nw_proto), self._get_protocol_name_by_number(rule.get_nw_proto())),
    ...
    def show_router_for_instance(self, instance, printme=True):
        instance = self._get_instance(instance)
        ret_buf = self._highlight_buf_for_instance(buf=self.show_router_summary(router=self.get_router_for_instance(instance=instance), printme=False), instance=instance)
        if printme:
            self.info(('\n{0}\n').format(ret_buf))
            return
        else:
    ...
    def show_bridge_for_instance(self, instance, printme=True):
        instance = self._get_instance(instance)
        ret_buf = self._highlight_buf_for_instance(buf=self.show_bridges(bridges=self.get_bridge_for_instance(instance=instance), printme=False), instance=instance)
        if printme:
            self.info(('\n{0}\n').format(ret_buf))
            return
        else:
    ...
    def show_instance_network_summary(self, instance, printme=True):
        ...
        title = ('NETWORK SUMMARY FOR INSTANCE:{0}, (PRIVIP:{1}, PUBIP:{2})').format(instance.id, instance.private_ip_address, instance.ip_address)
        pt = PrettyTable([title])
        pt.align[title] = 'l'
        pt.border = 0
        pt.padding_width = 0
    ...
    def show_ip_addr_group_addrs(self, ipgroup, printme=True):
        ...
        addrs = [
         ('({0})').format(ipgroup.get_name()).ljust(self._ADDR_SPACING)]
    ...
    def show_hosts(self, hosts=None, printme=True):
        ...
        for host in sorted(hosts, key=lambda host: host.get_name()):
            ...
            hostbuf += self._indent_table_buf(str(self.show_hosts_summary(hosts=host, printme=False)))
    ...
    def reset_midolman_service_on_hosts(self, hosts=None, username=None, password=None, keypath=None):
        if hosts and not isinstance(hosts, list):
            if not isinstance(hosts, Host):
            ...
            raise (hosts or ValueError)('No hosts provided or found from zookeeper or mido api?')
            ...
            for host in hosts:
                ...
                try:
                    ...
                    try:
                        ...
                        if machine.distro[0] in ('rhel', 'centos') and int(machine.distro_ver[0]) > 6:
                            machine.sys('systemctl restart midolman')
                            continue
                    except Exception as SE:
                        self.log.warn(('systemctl failed on host:"{0}", err:{1}\nTrying init.d now...').format(host, SE))

                    self.info(('Attempting to {0} host:{1} ({2})').format(status, host, ip))
                    ...
                    success = False
                    error = str(SE)
                    self.log.warning(('{0}({1}), midolman service failed to:{2}. Err:"{3}"').format(host, ip, status, str(SE)))
    ...
    def get_host_ssh(self, host, username=None, password=None, keypath=None, timeout=10):
        ...
        if not euca_host:
            ...
            ssh = sshconnection.SshConnection(host=ip, username=username, password=password, keypath=keypath, timeout=timeout, banner_timeout=10)
        else:
            ssh = euca_host.ssh
    ...
    def show_host_interfaces(self, host, printme=True):
        ...
        table
        """
        assert isinstance(host, Host), ('host type ({0}) is not of midonet Host type').format(type(host))
        interfaces = host.get_interfaces()
        pt = PrettyTable(['NAME', 'TYPE', 'MAC', 'STATUS', 'MTU', 'ENDPOINT', 'ADDRESSES'])
        for hi in interfaces:
            assert isinstance(hi, HostInterface), ('host interface type({0}) is not of midonet Host Interface type ').format(type(hi))
    ...
    def get_host_for_instance(self, instance):
        ...
        if not node:
            if instance.state != 'running':
                self.log.error(('Node, not found. Try again when instance is running, current state:{0}').format(instance.state))
    ...
    def get_instance_learned_port_by_ping(self, instance):
        ...
        if not port:
            try:
                ...
                self.info(self._bold(("Trying to ping the instance private addr('{0}') now...?)").format(instance.private_ip_address), 91))
    ...
    def show_tunnel_zones(self, tzones=None, highlight_ip=None, printme=True):
        ...
        main_pt.align = 'l'
        for tz in tzones:
            title = markup(('Name:{0}, {1}, type:{2}').format(tz.get_name(), tz.get_id(), tz.get_type()), [1, 94])
            mediapt = ('HostListMediaType:{0}, HostMediaType:{1}').format(tz.__dict__.get('tunnel_zone_host_list_media_type'), tz.__dict__.get('tunnel_zone_host_media_type'))
    ...
    def get_security_group(self, id=None, name=None):
        ...
        if name:
            names.append(name)
        groups = self.eucaconnection.ec2_connection.get_all_security_groups(groupnames=names, group_ids=ids)
        for group in groups:
    ...
    def show_security_group(self, group, printme=True):
        ...
        title = markup(('Security Group: {0}/{1}, VPC: {2}').format(group.name, group.id, group.vpc_id))
    ...
    def get_euca_mido_config(self):
        ...
        if not mido_config:
            self.log.warning('Mido config section not found in euca nework_configuration property')
        return mido_config
    ...
    def show_mido_conf_for_hosts(self, hosts=None, path='/etc/midolman.conf', username=None, password=None, keypath=None, sshtimeout=10, printmethod=None):
        printmethod = printmethod or self.log.info
        if not hosts:
            try:
                hosts = self.get_midolman_hosts_from_zk()
        ...
        for host in hosts:
            ...
            try:
                if isinstance(host, Host):
                    host = host.get_name()
                ssh = self.get_host_ssh(host, username=username, password=password, keypath=keypath, timeout=sshtimeout)
                ssh_host = ssh.host
    ...
    def show_zk_hosts(self, hosts=None, printmethod=None, printme=True):
        ...
        for host in hosts:
            ...
            try:
                ...
                status = ('Error creating and starting zookeeper client for host: "{0}", err:"{1}"').format(host, E)
