...
# Embedded file name: build/bdist.linux-x86_64/egg/cloud_admin/services/propertiesconnection.py
# Compiled at: 2018-01-31 14:44:08
import json, re, boto
from boto.connection import AWSQueryConnection
from cloud_admin.services import EucaNotFoundException
from cloud_admin.services.eucaproperty import EucaProperty, SHOW_PROPERTIES, SHOW_PROPERTIES_NARROW
from cloud_utils.log_utils import eulogger

class PropertiesConnection(AWSQueryConnection):
    APIVersion = 'eucalyptus'

    def __init__(self, hostname, aws_access_key, aws_secret_key, path='/services/Properties', port=8773, is_secure=False, boto_debug_level=0, err_method=None, logger=None, **kwargs):
        """
        ...
        if not isinstance(self.host, basestring) or not isinstance(aws_access_key, basestring) or not isinstance(aws_secret_key, basestring):
            raise ValueError(('Missing or invalid type for required arg. host:"{0}", aws_access_key_id:"{1}", aws_secret_access_key:"{2}"').format(self.host, aws_access_key, aws_secret_key))
        ...
        super(PropertiesConnection, self).__init__(path=self.path, aws_access_key_id=aws_access_key, aws_secret_access_key=aws_secret_key, port=self.port, is_secure=self.is_secure, host=self.host, debug=self.debug, **kwargs)
    ...
    def _get_list_request(self, action, service=EucaProperty, params={}, markers=[
 'item', 'euca:item'], verb='GET'):
    ...
    def get_property(self, property):
        ...
        if not property:
            raise ValueError(('Unknown type provided for property lookup: "{0}/{1}"').format(property, type(property)))
        ...
        if prop_count > 1:
            ...
            raise ValueError(('get_property: Multiple matches for property name:{0}, found {1} matches:\n{2}').format(property_name, prop_count, prop_string))
        return keep[0]
    ...
    def modify_property(self, prop, value, verbose=True):
        ...
        if not isinstance(prop, EucaProperty):
            ...
            if props:
                ...
                raise ValueError(('modify_property err. Property: "{0}" was not found on system?').format(prop))
        ...
        if ret_prop_list:
            ...
            self.debug_method(('Could not parse EucaProperty from ModifyPropertyValue response:"{0}"').format(prop))
        return ret_prop
