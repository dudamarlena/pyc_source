...
# [GCC 8.4.0]
# Embedded file name: /home/fraoustin/Téléchargements/ocglances/tmp/ocglances/plugins/glances_plugin.py
...
from ocglances.compat import iterkeys, itervalues, listkeys, map
from ocglances.actions import GlancesActions
from ocglances.history import GlancesHistory
...
class GlancesPlugin(object):
        ...
        if self.stats and self._history_enable():
            for i in self.get_items_history_list():
                if isinstance(self.stats, list):
                    for l in self.stats:
                        self.stats_history.add(l[item_name] + '_' + i['name'], l[i['name']], description=i['description'], history_max_size=self._limits['history_size'])

                else:
                    self.stats_history.add(i['name'], self.stats[i['name']], description=i['description'], history_max_size=self._limits['history_size'])
    ...
    def get_stats_snmp(self, bulk=False, snmp_oid=None):
        ...
        from ocglances.snmp import GlancesSNMPClient
            ...
            if len(snmp_oid) == 1:
                for item in snmpresult:
                    if iterkeys(item)[0].startswith(itervalues(snmp_oid)[0]):
                        ret[iterkeys(snmp_oid)[0] + iterkeys(item)[0].split(itervalues(snmp_oid)[0])[1]] = itervalues(item)[0]
        ...
        if isinstance(self.get_raw(), list) and self.get_raw() is not None and self.get_key() is not None:
            for i in self.get_raw():
                ...
                for key in listkeys(i):
                    value = {'decoration': 'DEFAULT', 'optional': False, 
                       'additional': False, 
                       'splittable': False}
    ...
    def get_alert(self, current=0, minimum=0, maximum=100, highlight_zero=True, is_max=False, header='', action_key=None, log=False):
    ...
    def manage_action(self, stat_name, trigger, header, action_key):
        ...
        try:
            ...
            self.actions.run(stat_name, trigger, command, mustache_dict=mustache_dict)
    ...
    def msg_curse(self, args=None, max_width=None):
        ...
        return [
         self.curse_add_line(str(self.stats))]
    ...
    def get_stats_display(self, args=None, max_width=None):
        ...
        if max_width is not None:
            ...
            ret = {'display': display_curse, 'msgdict': self.msg_curse(args), 
               'align': align_curse}
        return ret
    ...
    def auto_unit(self, number, low_precision=False):
        ...
        prefix = {'Y': 1208925819614629174706176, 
