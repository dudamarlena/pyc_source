...
# [GCC 8.4.0]
# Embedded file name: /home/sbo/lib/python3.4/site-packages/vaitk/gui/VPainter.py
# Compiled at: 2015-05-02 14:14:14
# Size of source mod 2**32: 8213 bytes
...
class VPainter:
    ...
    def drawText(self, pos_or_rect, string, align=0):
        ...
        if align & Alignment.AlignLeft:
            ...
            if align & Alignment.AlignRight:
                line_formatted = string.rjust(rect[Index.RECT_WIDTH])
            else:
                if align & Alignment.AlignHCenter:
                    line_formatted = string.center(rect[Index.RECT_WIDTH])
                else:
                    line_formatted = string.ljust(rect[Index.RECT_WIDTH])
        for h in range(rect[Index.RECT_HEIGHT]):
            if align & Alignment.AlignTop and h == 0 or align & Alignment.AlignVCenter and h == int(rect[Index.RECT_HEIGHT] / 2) or align & Alignment.AlignBottom and h == rect[Index.RECT_HEIGHT] - 1:
                self._widget.screenArea().write((rect[Index.RECT_X], rect[Index.RECT_Y] + h), line_formatted, self.fg_color, self.bg_color)
            else:
                self._widget.screenArea().write((rect[Index.RECT_X], rect[Index.RECT_Y] + h), ' ' * rect[Index.RECT_WIDTH], self.fg_color, self.bg_color)
    ...
    def eraseRect(self, rect):
        ...
        for h_idx in range(rect[Index.RECT_HEIGHT]):
            screen_area.write((top_left[Index.X], top_left[Index.Y] + h_idx), string, widget_colors[Index.FG_COLOR], widget_colors[Index.BG_COLOR])
