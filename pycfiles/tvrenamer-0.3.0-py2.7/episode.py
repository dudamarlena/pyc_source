...
# [GCC 8.4.0]
# Embedded file name: build/bdist.cygwin-2.2.1-i686/egg/tvrenamer/core/episode.py
...
class Episode(object):
    ...
    def as_dict(self):
        return {'original': self.original, 'name': self.name, 
    ...
    def __str__(self):
        return ('{0} => [{1} {2}|{3} {4}] meta: [{5} S{6} E{7}] formatted: {8}/{9}').format(self.original, self.location, self.name, self.clean_name, self.extension, self.series_name or '', self.season_number or '', list(zip(self.episode_numbers or [], self.episode_names or [])), self.formatted_dirname or '', self.formatted_filename or '')
    ...
    def status(self):
        ...
        :rtype: dict
        """
        return {self.original: {'formatted_filename': self.out_location, 
                           'state': self.state, 
                           'messages': ('\n\t').join(self.messages)}}
    ...
    def parse(self):
        ...
        """
        self.clean_name = formatter.apply_replacements(self.name, cfg.CONF.input_filename_replacements)
        output = parser.parse_filename(self.clean_name)
        if output is None:
            self.messages.append(('Invalid filename: unable to parse {0}').format(self.clean_name))
        ...
        if self.episode_numbers is None:
            self.messages.append('Regex does not contain episode number group, should contain episodenumber, episodenumber1-9, or episodenumberstart and episodenumberend\n\nPattern was:\n' + output.get('pattern'))
            LOG.info(self.messages[(-1)])
            raise exc.ConfigValueError(self.messages[(-1)])
        self.series_name = output.get('series_name')
        if self.series_name is None:
            self.messages.append('Regex must contain seriesname. Pattern was:\n' + output.get('pattern'))
            LOG.info(self.messages[(-1)])
            raise exc.ConfigValueError(self.messages[(-1)])
    ...
    def enhance(self):
        ...
        self.episode_names, error = self.api.get_episode_name(series, self.episode_numbers, self.season_number)
