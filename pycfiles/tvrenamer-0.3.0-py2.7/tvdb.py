...
# [GCC 8.4.0]
# Embedded file name: build/bdist.cygwin-2.2.1-i686/egg/tvrenamer/services/tvdb.py
...
class TvdbService(base.Service):
    ...
    def get_series_by_name(self, series_name):
        ...
        try:
            return (
             self.api.search_series(name=series_name), None)
    ...
    def get_series_by_id(self, series_id):
        ...
        try:
            return (
             self.api.get_series(series_id), None)
        except exceptions.TVDBRequestException as err:
    ...
    def get_episode_name(self, series, episode_numbers, season_number):
        ...
        """
        try:
            episodes = self.api.get_episodes(series.get('id'), airedSeason=season_number)
        except exceptions.TVDBRequestException as err:
            LOG.exception('episodes for series %s season no %s failed', series.get('id'), season_number)
            return (None, _as_str(err))
