...
# [GCC 8.4.0]
# Embedded file name: build/bdist.cygwin-2.2.1-i686/egg/tvrenamer/tests/core/test_watcher.py
...
class WatcherTests(base.BaseTest):
    ...
    def test_is_blacklisted_filename(self):
        ...
        blacklist = [
         {'full_path': '', 'exclude_extension': False, 
            'is_regex': False, 
            'match': '.DS_File'}]
        self.CONF.set_override('filename_blacklist', blacklist)
        self.assertTrue(watcher._is_blacklisted_filename('/tmp/.DS_File'))
        blacklist = [
         {'full_path': '', 'exclude_extension': True, 
            'is_regex': False, 
            'match': '.DS_File'}]
        self.CONF.set_override('filename_blacklist', blacklist)
        self.assertTrue(watcher._is_blacklisted_filename('/tmp/.DS_File'))
        ...
        blacklist = [
         {'full_path': True, 'exclude_extension': True, 
            'is_regex': False, 
            'match': '.DS_File'}]
        self.CONF.set_override('filename_blacklist', blacklist)
        self.assertFalse(watcher._is_blacklisted_filename('/tmp/sample.avi'))
