...
# [GCC 8.4.0]
# Embedded file name: build/bdist.cygwin-2.2.1-i686/egg/tvrenamer/services/trakt_service.py
...
OPTS = [
 cfg.StrOpt('client_id', secret=True, default=os.environ.get('TRAKT_CLIENT_ID'), help="client id from your trakt account ENV['TRAKT_CLIENT_ID']"),
 cfg.StrOpt('client_secret', secret=True, default=os.environ.get('TRAKT_CLIENT_SECRET'), help="client secret from your trakt account ENV['TRAKT_CLIENT_SECRET']")]
...
class TraktService(base.Service):
    ...
    def __init__(self):
        ...
        trakt.Trakt.configuration.defaults.client(id=cfg.CONF.trakt.client_id, secret=cfg.CONF.trakt.client_secret)
    ...
    def get_episode_name(self, series, episode_numbers, season_number):
        ...
        for epno in episode_numbers:
            try:
                episode = trakt.Trakt['shows'].episode(series_id, season_number, epno, exceptions=True)
            except exceptions.RequestError as err:
                LOG.exception('fetch episode %s S%sE%s failed', series_id, season_number, epno)
                return (None, str(err))
