...
# [GCC 8.4.0]
# Embedded file name: build/bdist.cygwin-2.2.1-i686/egg/tvrenamer/tests/test_service.py
# Compiled at: 2015-11-08 18:31:47
import logging, logging.handlers, os, tempfile, fixtures
...
class ServiceTest(base.BaseTest):
    ...
    cfg_data.append('#filename_format_ep = %(seriesname)s - %(seasonnumber)02dx%(episode)s - %(episodename)s%(ext)s\n')
    ...
    cfg_data.append('#multiep_format = %(epname)s (%(episodemin)s-%(episodemax)s)\n')
    ...
    def test_setup_logging(self):
        ...
        self.assertEqual(logging.getLogger('tvdbapi_client').getEffectiveLevel(), logging.WARNING)
    ...
    def test_configure_without_venv(self):
        with fixtures.EnvironmentVariable('VIRTUAL_ENV'):
            cfgfile = self.create_tempfiles([
             (
              os.path.join(os.path.expanduser('~'), 'tvrenamer'),
              ('').join(self.cfg_data))])[0]
