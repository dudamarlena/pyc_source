...
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/TAMO/MD/Meme.py
...
class Meme:
    ...
    def __init__(self, file='', width='', extra_args='', genome='YEAST', bfile=None):
        ...
        if not bfile:
            if genome == 'YEAST':
                ...
                print 'Perhaps something like -bfile %s' % (TAMO.paths.MEMEdir + 'tests/yeast.nc.6.freq')
                sys.exit(1)
        ...
        if bfile:
            self.args = self.args + '-bfile %s ' % bfile
    ...
    def _parse(self):
        ...
        for i in range(len(self.lines)):
            ...
            if toks[0] == 'MOTIF':
            ...
            elif toks[0] == 'Motif' and int(toks[1]) == num and toks[3] == 'BLOCKS':
                offset = 3
                j = 0
            ...
            elif toks[0] == 'Motif' and int(toks[1]) == num and toks[3] == 'probability':
                offset = 3
                j = 0
                counts = []
                ...
                m.compute_from_counts(counts, 1e-05)
                self.motifs.append(m)
            elif toks[0] == 'Sequence' and toks[1] == 'name' and toks[2] == 'Weight':
                offset = 2
                j = 0
