...
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/TAMO/MotifMetrics.py
...
import sys, os, re, string, shelve, pickle, math, TAMO.paths
...
def main():
    ...
    if '-bysitef' in sys.argv:
        ...
        if sys.argv[idx].isdigit():
            ...
            del sys.argv[idx]
    for _method in ['spec', 'chi2', 'E_seq', 'MNCP', 'ROC_AUC', 'GO', 'feature', 'dist', 'binomial', 'overrep']:
        _flag = '-%s' % _method
    ...
    if TFMAT:
        if 'metaTF' not in dir():
            import metaTF
        motifs.append(metaTF.matid2newmotif(TFMAT, bg={'A': 0.31, 'C': 0.19, 'G': 0.19, 'T': 0.31}))
    if TRANSFAC:
        tf_motifD = tfmats()
        for key, m in tf_motifD.items():
            if 1 or key.find('human') >= 0:
                m.source = key
                motifs.append(m)
    ...
    if find_best:
        ...
        for motif in motifs:
            if method == 'E':
                p = theProbeSet.p_value(motif, ids, 'verbose', omitlist=omits)
            elif method == 'spec':
                p = theProbeSet.church(motif, ids, 'verbose')
            elif method == 'bysite':
                p = theProbeSet.E_site(motif, ids, bysitek, 'verbose')
            elif method == 'byfact':
                p = theProbeSet.E_sitef(motif, ids, byfactk, 'verbose')
            elif method == 'overrep':
                p = theProbeSet.binomial(motif, ids, 'verbose')
            elif method == 'binomial':
                p = theProbeSet.binomial(motif, ids, 'verbose')
            elif method == 'chi2':
                p = theProbeSet.E_chi2(motif, ids, None, 'verbose')
            elif method == 'byseq':
                p = theProbeSet.E_seq(motif, ids, 'verbose')
            elif method == 'MNCP':
                p = theProbeSet.MNCP(motif, ids, 'verbose')
            elif method == 'ROC_AUC':
                p = theProbeSet.ROC_AUC(motif, ids, 'verbose')
            elif method == 'GO':
                p = theProbeSet.GO(motif, ids, 'verbose')
            elif method == 'feature':
                p = theProbeSet.feature_check(motif, ids, 'vebose')
            elif method == 'dist':
                p = theProbeSet.distribution(motif, ids)
            else:
...
class ProbeSet():
    ...
    def _loadfiles(self):
        ...
        if os.path.exists(self.fastafile + '.pickle'):
            ...
            if os.path.exists(self.fastafile + '.shelf'):
    ...
    def _count_matching_probes_motif_fast(self, motif, NMlist=[], factor=None):
        ...
        if NMlist == []:
            NMlist = self.probes.keys()
        count = 0
        if factor != None:
            threshold = motif.maxscore * factor
        else:
            if motif.threshold == None:
    ...
    def p_value(self, ambig, NMlist, verbose='', factor=None, omitlist=[]):
        ...
        """
        if omitlist:
            subset = [ x for x in NMlist if x not in omitlist ]
            all = [ x for x in self.probes.keys() if x not in omitlist ]
        else:
            subset = NMlist
    ...
    def church(self, ambig, preNMlist, verbose='', topcount=100, omitlist=[]):
        ...
        """
        if omitlist:
            NMlist = [ x for x in preNMlist if x not in omitlist ]
            all = [ x for x in self.probes.keys() if x not in omitlist ]
        else:
            NMlist = preNMlist
            all = self.probes.keys()
        ...
        scoresT.reverse()
        if topcount < 0:
            topcount = int(-topcount * len(NMlist))
        if len(scoresT) > topcount:
            for i in range(topcount, len(scoresT)):
                if not scoresT[i][0] == scoresT[topcount][0]:
                    break

            scoresT = scoresT[0:i]
        total = len(all)
        numinteresting = len(NMlist)
        numpicked = len(scoresT)
        numfound = 0
        for i in range(len(scoresT)):
            if scoresT[i][1] in NMlist:
                numfound = numfound + 1
    ...
    def E_chi2(self, ambig, NMlist, bins=None, verbose='', factor=None, omitlist=[]):
        ...
        if boundD:
            ...
            for count in boundbins:
                boundtot = boundtot + count

            for count in genomebins:
                genometot = genometot + count
    ...
    def matches_with_features(self, ambig, NMlist, featurelist, featurename='fE', verbose='', factor=None):
        ...
        if submatches:
            p = Arith.hypgeomsummore(len(matches), len(all), len(subset), len(submatches))
            pG = Arith.hypgeomsummore(len(all), len(self.probes.keys()), len(NMlist), len(subset))
        else:
            p = 1
            pG = 1
        if verbose:
    ...
    def binomial(self, ambig, NMlist, verbose='', factor=None, omitlist=[], MAXVALUE=4):
        ...
        if countDsub:
            ...
            for value, count in countDall.items():
                numinteresting = numinteresting + min(value, MAXVALUE) * count

            for value, count in countDsub.items():
                numfound = numfound + min(value, MAXVALUE) * count
    ...
    def ids_from_file(self, filename, QUIET=''):
        ...
        for line in FID.readlines():
            ...
            if self.probes.has_key(gene):
                ...
                print '# No Entry in genome for %s' % gene
...
def permute(letters, depth, seqs=[
 ''], curdepth=0):
    """
