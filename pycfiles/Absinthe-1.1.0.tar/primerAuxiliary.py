...
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/abseqPy/IgRepAuxiliary/primerAuxiliary.py
# Compiled at: 2019-04-23 02:08:32
__doc__ = '\n    Short description: Quality Control Analysis of Immunoglobulin Repertoire NGS (Paired-End MiSeq)    \n    Author: Monther Alhamdoosh    \n    Python Version: 2.7\n    Changes log: check git commits. \n'
import os, gc, sys, numpy as np
...
def addPrimerData(cloneAnnot, readFile, format, fr4cut, trim5end, trim3end, actualQstart, end5, end3, end5offset, threads, stream=None):
    ...
    try:
        try:
            ...
            for _ in range(threads):
                w = PrimerWorker(procCounter, fr4cut, trim5end, trim3end, actualQstart, end5, end3, end5offset, tasks, exitQueue, resultsQueue, stream=stream)
                workers.append(w)
                w.start()
...
def _collectPrimerResults(columns, queue, totalTasks, noSeqs, stream=None):
    ...
    printto(stream, ('\t{:,}/{:,} records have been collected ... ').format(processed, noSeqs))
    printto(stream, ("\tThere were {} unexpected 5' alignment and {} unexpected 3' alignment").format(totalUnexpected5, totalUnexpected3), LEVEL.WARN)
    return cloneAnnot
...
def writePrimerStats(end, name, cloneAnnot, fileprefix, category='All', stream=None):
    ...
    plotDist(integrity, name, fileprefix + 'integrity_dist.csv', title=("Integrity of {}'-end Primer Sequence (%s)").format(end) % category, proportion=True, rotateLabels=False)
    ...
    plotDist(c1, name, fileprefix + 'indelled_dist.csv', title=("Abundance of Indelled {}'-end Primers ({})").format(end, category), proportion=False, rotateLabels=False, vertical=False, top=50)
    c = Counter(known[(known[INDEL] != 0)][INDEL].tolist())
    plotDist(c, name, fileprefix + 'indel_pos_dist.csv', title=("Abundance of Indel Positions in {}'-end Primers ({})").format(end, category), proportion=False, rotateLabels=False, vertical=True, sortValues=False, top=50)
    ...
    for primer in primers:
        ...
        plotDist(germLineDist, name, fileprefix + primer + '_igv_dist.csv', title=('IGV Abundance of indelled {} ({})').format(primer, category), proportion=False, vertical=False, top=20, rotateLabels=False)
...
def generatePrimerPlots(cloneAnnot, outDir, name, end5, end3, stream=None):
    ...
    if end5:
        ...
        allInvalid5Clones = cloneAnnot.index[((cloneAnnot[PRIMER5] != NA) & (cloneAnnot[INDEL5] != 0))].tolist()
        printto(stream, '5-end analysis of out-of-frame clones ... ')
        writePrimerStats('5', name, outOfFrameClones, os.path.join(outDir, name + '_outframe_5end_'), 'Out-of-frame', stream=stream)
        outFrameInvalid5Clones = outOfFrameClones.index[((outOfFrameClones[PRIMER5] != NA) & (outOfFrameClones[INDEL5] != 0))].tolist()
        printto(stream, '5-end analysis of productive clones ... ')
        writePrimerStats('5', name, productiveClones, os.path.join(outDir, name + '_productive_5end_'), 'Productive', stream=stream)
        productiveInvalid5Clones = productiveClones.index[((productiveClones[PRIMER5] != NA) & (productiveClones[INDEL5] != 0))].tolist()
    if end3:
        ...
        writePrimerStats('3', name, outOfFrameClones, os.path.join(outDir, name + '_outframe_3end_'), 'Out-of-frame', stream=stream)
        printto(stream, '3-end analysis of productive clones ... ')
        writePrimerStats('3', name, productiveClones, os.path.join(outDir, name + '_productive_3end_'), 'Productive', stream=stream)
        if end5:
            invalid3Clones = cloneAnnot.index[((cloneAnnot[PRIMER3] != NA) & (cloneAnnot[INDEL3] != 0))].tolist()
            plotVenn({"5'-end": set(allInvalid5Clones), "3'-end": set(invalid3Clones)}, os.path.join(outDir, name + '_all_invalid_primers.png'), "Intersection of indelled 5' and 3' sequences (All)", stream=stream)
            del invalid3Clones
            del allInvalid5Clones
            outFrameInvalid3Clones = outOfFrameClones.index[((outOfFrameClones[PRIMER3] != NA) & (outOfFrameClones[INDEL3] != 0))].tolist()
            plotVenn({"5'-end": set(outFrameInvalid5Clones), "3'-end": set(outFrameInvalid3Clones)}, os.path.join(outDir, name + '_outframe_invalid_primers.png'), "Intersection of indelled 5' and 3' sequences (Out-of-frame)", stream=stream)
            del outFrameInvalid3Clones
            del outFrameInvalid5Clones
            productiveInvalid3Clones = productiveClones.index[((productiveClones[PRIMER3] != NA) & (productiveClones[INDEL3] != 0))].tolist()
            plotVenn({"5'-end": set(productiveInvalid5Clones), "3'-end": set(productiveInvalid3Clones)}, os.path.join(outDir, name + '_productive_invalid_primers.png'), "Intersection of indelled 5' and 3' sequences (productive)", stream=stream)
