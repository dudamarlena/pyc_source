...
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/TAMO/util/util/Arith.py
...
def stirhypgeom(numinteresting, total, numpicked, numfound):
    ...
    """
    ans = stircomb(numinteresting, numfound) + stircomb(total - numinteresting, numpicked - numfound) - stircomb(total, numpicked)
    return math.exp(ans)
...
def binomial(frac_exp, draws, hits):
    ...
    pre_ans = stircomb(draws, hits) + hits * math.log(frac_exp) + (draws - hits) * math.log(frac_miss)
...
def binomialsumtail(frac_exp, draws, hits):
    ...
    for h in range(hits, draws + 1):
        try:
            ...
            if 10 * h > draws and (newtot - tot) / (tot + epsilon) < 1e-05:
                break
        except:
