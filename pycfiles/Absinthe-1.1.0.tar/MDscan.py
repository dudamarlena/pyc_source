...
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/TAMO/MD/MDscan.py
...
import sys, re, os, math, tempfile, getopt, TAMO.paths
...
def usage(txt=''):
    if txt:
        print 'Error: %s' % txt
    print 'Usage: %s -f fasta_file' % re.sub('^.*/', '', sys.argv[0])
...
class metaMDscan:
    ...
    def _execute(self):
        ...
        for width in range(self.width_start, self.width_stop):
            ...
            sys.stdout.flush()
            result = MDscan(self.file, width, self.extra_args + ' -t %d' % self.top_count, bgfile=self.bgfile)
