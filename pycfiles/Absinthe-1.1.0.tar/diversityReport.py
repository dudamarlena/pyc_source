...
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/abseqPy/IgRepReporting/diversityReport.py
# Compiled at: 2019-04-25 19:45:24
...
from abseqPy.IgRepReporting.igRepPlots import plotSeqLenDist, generateCumulativeLogo, plotSeqDuplication, plotSeqRarefaction, plotSeqRecaptureNew
from abseqPy.logger import LEVEL, printto
from abseqPy.utilities import hasLargeMem, requires

def generateDiversityReport(spectraTypes, clonoTypes, name, outDir, topClonotypes, threads=2, segregate=False, stream=None):
...
def generateSpectraTypePlots(spectraTypes, name, outDir, stream=None):
    ...
    for k in spectraTypes.keys():
        ...
        plotSeqLenDist(spectraTypes[k], name, filename, dna=False, seqName=k.upper(), normed=True, maxbins=40, stream=stream)
        if k == 'cdr3':
            filename = os.path.join(specFolder, name + ('_{}_spectratype_no_outliers.csv').format(k))
            plotSeqLenDist(spectraTypes[k], name, filename, dna=False, seqName=k.upper(), normed=True, maxbins=40, removeOutliers=True, stream=stream)
...
def generateRarefactionPlots(clonoTypes, name, outDir, threads=2, stream=None):
    ...
    plotSeqDuplication(cdrWeights, cdrRegions, filename, 'Duplication of CDR Sequences', stream=stream)
    printto(stream, '\tThe rarefaction is being generated for CDRs .... ')
    filename = os.path.join(outDir, name + '_cdr_rarefaction.csv')
    plotSeqRarefaction(cdrSeqs, cdrRegions, filename, cdrWeights, 'Rarefaction of CDR Sequences', threads=threads, stream=stream)
    printto(stream, ' \tThe percent recapture is being generated for CDRs .... ')
    filename = os.path.join(outDir, name + '_cdr_recapture.csv')
    plotSeqRecaptureNew(cdrSeqs, cdrRegions, filename, 'Percent Recapture of CDR Sequences', threads=threads, stream=stream)
    frWeights = []
    frSeqs = []
    frRegions = []
    for region in regions:
    ...
    plotSeqDuplication(frWeights, frRegions, filename, 'Duplication of FR Sequences', stream=stream)
    printto(stream, '\tThe rarefaction is being generated for FRs .... ')
    filename = os.path.join(outDir, name + '_fr_rarefaction.csv')
    plotSeqRarefaction(frSeqs, frRegions, filename, frWeights, 'Rarefaction of FR Sequences', threads=threads, stream=stream)
    printto(stream, '\tThe percent recapture is being generated for FRs .... ')
    filename = os.path.join(outDir, name + '_fr_recapture.csv')
    plotSeqRecaptureNew(frSeqs, frRegions, filename, 'Percent Recapture of FR Sequences', threads=threads, stream=stream)
    cdrWeights = []
    cdrSeqs = []
    cdrRegions = []
    for region in regions:
    ...
    plotSeqDuplication(cdrWeights, cdrRegions, filename, 'Duplication of CDRs and V Domains', stream=stream)
    ...
    plotSeqRarefaction(cdrSeqs, cdrRegions, filename, cdrWeights, 'Rarefaction of CDRs and V Domains', threads=threads, stream=stream)
    ...
    plotSeqRecaptureNew(cdrSeqs, cdrRegions, filename, 'Percent Recapture of CDRs and V Domains', threads=threads, stream=stream)
    ...
    if detailed:
        ...
        for vgerm in clonoTypes:
            ...
            for region in regions:
                ...
                filename = os.path.join(regionDirectory, name + ('_{}_cumulative_logo.csv').format(vgerm.replace(os.path.sep, '_')))
                if hasLargeMem():
...
def generateSeqMotifs(flatClonoTypes, name, outDir, threads=2, stream=None):
    ...
    for region in regions:
        ...
        generateMotif(seqs, region, alphabet, filename, align=False, protein=True, weights=weights, outDir=outDir, threads=threads, stream=stream)
        ...
        generateMotif(seqs, region, alphabet, filename, align=True, protein=True, weights=weights, outDir=outDir, threads=threads, stream=stream)
...
def writeClonotypeDiversityRegionAnalysis(clonoTypes, sampleName, outDir, stream=None):
    ...
    with gzip.open(fname, 'wb') as (fp):
        ...
        for cdr3 in cdr3s:
            ...
            if len(writeBuffer) > 4000000000.0:
