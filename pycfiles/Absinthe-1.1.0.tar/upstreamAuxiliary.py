...
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/abseqPy/IgRepAuxiliary/upstreamAuxiliary.py
...
from abseqPy.IgRepertoire.igRepUtils import gunzip, compressCountsFamilyLevel, compressCountsGeneLevel, safeOpen, compressSeqGeneLevel, compressSeqFamilyLevel
...
def extractUpstreamSeqs(cloneAnnot, recordFile, upstream, upstreamFile, stream=None):
    ...
    with open(upstreamFile, 'w') as (fp):
        ...
        if len(recordsBuffer) > 0:
            ...
            SeqIO.write(recordsBuffer, fp, 'fasta')
    if revAlign > 0:
        printto(stream, ('\t\t\t{} sequences are in reversed alignment ... ').format(revAlign), LEVEL.INFO)
    if trimmedBegin > 0:
        printto(stream, ('\t\t\tThe query sequence is not aligned within 3bp of the IGV start position ... {} found and excluded!').format(trimmedBegin), LEVEL.WARN)
    if trimmedUpstream > 0:
        printto(stream, ('\t\t\tUpstream sequences shorter than the expected length are detected ... {} found').format(trimmedUpstream), LEVEL.WARN)
    if noSeq > 0:
        printto(stream, ('\t\t\tNo upstream sequence can be extracted (too short) for {} sequences.').format(noSeq), LEVEL.WARN)
    gc.collect()


def collectUpstreamSeqs(upstreamFile, sampleName, expectLength, outResDir, outHdfDir, startCodon=True, type='secsig', plotDist=True, stream=None):
    """
    ...
    if ignoredSeqs:
        printto(stream, ('\tThere are {} sequences that were ignored because the length of the provided upstreamsequences were not {} <= length(upstream_seqs) <= {}').format(ignoredSeqs, *expectLength), LEVEL.WARN)
    if sum(ighvSignalsCounts.values()):
        ...
        printto(stream, ('\tThere are {} {} within expected length ({} to {}) and startCodon={}').format(sum(ighvSignalsCounts.values()), title, expectLength[0], expectLength[1], startCodon), LEVEL.INFO)
        validSeqFile = os.path.join(outHdfDir, _VALID_SEQ_FASTA_TEMPLATE.format(sampleName, type, *expectLength))
        SeqIO.write(flattenRecs, validSeqFile, 'fasta')
        if plotDist:
            writeCountsCategoriesToFile(ighvSignalsCounts, sampleName, os.path.join(outResDir, ('{}_{}_{:.0f}_{:.0f}_valid_').format(sampleName, type, expectLength[0], expectLength[1])), title)
    if sum(faultyTransCounts.values()):
        ...
        faultySeqFile = os.path.join(outHdfDir, _FAULTY_SEQ_FASTA_TEMPLATE.format(sampleName, type, *expectLength))
        SeqIO.write(flattenRecs, faultySeqFile, 'fasta')
        if plotDist:
            writeCountsCategoriesToFile(faultyTransCounts, sampleName, os.path.join(outResDir, ('{}_{}_{:.0f}_{:.0f}_faulty_').format(sampleName, type, *expectLength)), 'Faulty Translations')
        ...
        for i in random.choice(range(len(flattenRecs)), min(5, len(flattenRecs)), replace=False):
            ...
            printto(stream, ('\t{}\n\tTranslated:{}').format(sequence, sequence[:len(sequence) - len(sequence) % 3].translate()))
    ...
    if sum(noStartCodonCounts.values()):
        ...
        noStartCodonFile = os.path.join(outHdfDir, _STARTCOD_SEQ_FASTA_TEMPLATE.format(sampleName, type, *expectLength))
        SeqIO.write(flattenRecs, noStartCodonFile, 'fasta')
        if plotDist:
            writeCountsCategoriesToFile(noStartCodonCounts, sampleName, os.path.join(outResDir, ('{}_{}_{:.0f}_{:.0f}_no_atg_').format(sampleName, type, *expectLength)), 'Upstream sequences without start codon')
        printto(stream, ('\tThere is no ATG codon in {} sequences (excluded)').format(len(flattenRecs)), LEVEL.INFO)
        for i in random.choice(range(len(flattenRecs)), min(5, len(flattenRecs)), replace=False):
    ...
    return (
     ighvSignals, faultyTrans, ighvSignalsNoATG)


@requires('TAMO', fatal=False)
def findUpstreamMotifs(upstreamFile, sampleName, outHdfDir, outResDir, expectLength, level, startCodon=True, type='secsig', clusterMotifs=False, threads=2, stream=None):
    """
    ...
    allFiles = [
     validSeqFile, faultySeqFile, noStartCodonFile]
    if all(map(lambda x: os.path.exists(x), allFiles)):
        printto(stream, 'Sequences were already analyzed at {}, loading from files instead ... ' + (' ').join(allFiles), LEVEL.WARN)
        ighvSignals, faultySeq, noStartCodonSeq = _loadIGVSeqsFromFasta(validSeqFile), _loadIGVSeqsFromFasta(faultySeqFile), _loadIGVSeqsFromFasta(noStartCodonFile)
    else:
        printto(stream, 'Sequences are being analyzed ... ')
        ighvSignals, faultySeq, noStartCodonSeq = collectUpstreamSeqs(upstreamFile, sampleName, expectLength, outResDir, outHdfDir, startCodon, type, stream=stream)
    ighvSignals = compressor(ighvSignals)
    generateMotifs(ighvSignals, align=expectLength[0] < expectLength[1], outputPrefix=os.path.join(outResDir, ('{}_{}_{:.0f}_{:.0f}_dna_' + level).format(*OUTPUT_FILE_PACKET)), clusterMotifs=clusterMotifs, threads=threads, stream=stream)
    if EXACT_LENGTH and type == 'secsig':
        faultySeq = compressor(faultySeq)
        generateMotifs(faultySeq, align=True, outputPrefix=os.path.join(outResDir, ('{}_{}_{:.0f}_{:.0f}_faulty_' + level).format(*OUTPUT_FILE_PACKET)), transSeq=False, extendAlphabet=True, clusterMotifs=clusterMotifs, threads=threads, stream=stream)
        noStartCodonSeq = compressor(noStartCodonSeq)
        generateMotifs(noStartCodonSeq, align=True, outputPrefix=os.path.join(outResDir, ('{}_{}_{:.0f}_{:.0f}_untranslated_' + level).format(*OUTPUT_FILE_PACKET)), transSeq=False, extendAlphabet=True, clusterMotifs=clusterMotifs, threads=threads, stream=stream)
        generateMotifs(ighvSignals, align=False, outputPrefix=os.path.join(outResDir, ('{}_{}_{:.0f}_{:.0f}_protein_' + level).format(*OUTPUT_FILE_PACKET)), transSeq=True, clusterMotifs=clusterMotifs, threads=threads, stream=stream)
