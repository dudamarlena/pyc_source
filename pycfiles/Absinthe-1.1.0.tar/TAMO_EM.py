...
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/TAMO/MD/TAMO_EM.py
...
def main():
    ...
    for tok, i in zip(sys.argv, xrange(len(sys.argv))):
        if tok == '-top':
            seed_count = int(sys.argv[(i + 1)])
        elif tok == '-greedy':
            algorithm = 'GREEDY'
        elif tok == '-prior':
            seed_s.append(sys.argv[(i + 1)])
        elif tok == '-beta':
            beta = float(sys.argv[(i + 1)])
        elif tok == '-seedbeta':
            seedbeta = float(sys.argv[(i + 1)])
        elif tok == '-gamma':
    ...
    if infofile and width == 'info':
        ...
        if beta:
            theEM.beta = beta
        if deltamin:
            theEM.deltamin = deltamin
        if seedbeta:
            theEM.seedbeta = seedbeta
        ...
        for C, i in zip(theEM.candidates, range(len(theEM.candidates))):
            ...
            if m.threshold:
                print 'Threshold: %5.2f' % m.threshold
...
def print_motifs(motifs, kmer_count=20, istart=0):
    ...
    for m in motifs:
        ...
        if not m.__dict__.has_key('gamma'):
            m.gamma = None
        if not m.__dict__.has_key('church'):
            m.church = None
        if not m.__dict__.has_key('realpvalue'):
            m.realpvalue = None
        if m.seedtxt:
            print 'Seed: %3d %s' % (i, m.seedtxt)
        if m.gamma:
        ...
        print 'Motif %3d %-25s (Bits: %5.2f  MAP: %5.2f   D: %5.3f  %2d) Enr: %6.3f' % (
         i, m, m.totalbits, m.MAP, m.seeddist, m.seednum, math.fabs(math.log(m.pvalue) / math.log(10.0))),
        if m.church != None:
            print ' ch: %5.2f' % math.fabs(math.log(m.church) / math.log(10.0)),
...
def load(filename):
    return tamofile2motifs(filename)
