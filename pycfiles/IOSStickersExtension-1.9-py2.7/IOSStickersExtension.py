...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/IOSStickersExtension/IOSStickersExtension.py
# Compiled at: 2018-01-25 08:53:37
import copy, fnmatch, json, os, plistlib, re, shutil, uuid
...
class IOSStickersExtension:
    ...
    def init_pbxproj_project_plist(self):
        if self.pbxproj_project_plist is not None:
            ...
            convert_plist_to_xml_cmd = [
             'plutil', '-convert', 'xml1', self.pbxproj_file_path]
    ...
    def add_sticker_icons(self):
        icon_file_paths = []
        assetcatalog_compiler_appicon_name = self.get_value_by_key_from_xc_build_configuration('ASSETCATALOG_COMPILER_APPICON_NAME')
        ...
        if not icon_file_paths:
            ...
            icon_file_paths = [ path.join(self.proj_dir_path, icon_name) for icon_name in info_plist['CFBundleIconFiles']
                              ]
        ...
        for icon_file_path in icon_file_paths:
            ...
            if icon_image_max_resolution is None:
                ...
                continue
            if icon_image.height * icon_image.width > icon_image_max_resolution.height * icon_image_max_resolution.width:
                icon_image_max_resolution = icon_image
        ...
        if icon_image_max_resolution is None:
            ...
            stickers_icon_set_dir_path = path.join(self.proj_dir_path, 'tt-stickers', 'TT-Stickers.xcassets', 'iMessage App Icon.stickersiconset')
            ...
            for sticker_icon_size in sticker_icon_sizes:
                sticker_icon_file_name = 'sticker_icon-' + str(sticker_icon_size[0]) + 'X' + str(sticker_icon_size[1]) + '.' + icon_image_max_resolution_file_extension
    ...
    def update_pbxproj_file(self):
        ...
        if pbxproj_project_plist is None:
            ...
            for build_configuration in build_configurations:
                ...
                xc_build_configuration_additions_plist['buildSettings']['PRODUCT_BUNDLE_IDENTIFIER'] = bundle_id + '.stickers'
                id = self.generate_id()
            ...
            for key, xc_build_configuration in xc_build_configurations.iteritems():
                if key not in xc_configuration_list_additions_plist['buildConfigurations'] and 'PRODUCT_NAME' in xc_build_configuration['buildSettings'] and xc_build_configuration['buildSettings']['PRODUCT_NAME'] == 'tt-stickers':
                    print 'remove' + key
    ...
    def get_value_by_key_from_xc_build_configuration(self, key):
        ...
        for xc_build_configuration in xc_build_configurations.itervalues():
            if xc_build_configuration['name'] == build_configuration and key in xc_build_configuration['buildSettings']:
                if key == 'INFOPLIST_FILE' and basename(path.dirname(xc_build_configuration['buildSettings']['INFOPLIST_FILE'])) == 'tt-stickers':
                    continue
