...
# [GCC 8.4.0]
# Embedded file name: /home/stephan/.virtualenvs/drf_amsterdam/lib/python3.7/site-packages/tests/test_serializers.py
import json
...
class SerializerTest(TestCase):
    ...
    def setUp(self):
        ...
        records = [
         {'station':ws, 
          'date':date(1901, 1, 1),  'temperature':'10.0'},
         {'station':ws, 
          'date':date(1901, 2, 1),  'temperature':'11.0'},
         {'station':ws, 
          'date':date(1901, 3, 1),  'temperature':'20.0'}]
        for rs in records:
            (TemperatureRecord.objects.create)(**rs)

    def valid_response(self, url, response, content_type='text/html; charset=utf-8'):
        """Help method to check common status/json."""
        self.assertEqual(200, response.status_code, 'Wrong response code for {}'.format(url))
        self.assertEqual(f"{content_type}", response['Content-Type'], 'Wrong Content-Type for {}'.format(url))
    ...
    def test_weatherstation_endpoint(self):
        ...
        self.assertEqual(payload['_links']['self']['href'], 'http://testserver/tests/weatherstation/?format=json')
        detail_page = payload['results'][0]['_links']['self']['href']
        response = c.get(detail_page, format='json')
        self.assertEqual(response.status_code, 200)
        payload = json.loads(response.content)
        self.assertEqual(payload['_links']['self']['href'], detail_page)
    ...
    def test_hal_style_pagination(self):
        ...
        self.assertEqual(payload['_links']['self']['href'], 'http://testserver/tests/weatherstation/')
    ...
    def test_display_field(self):
        ...
        self.assertEquals(serializer.data['_display'], 'DISPLAY FIELD CONTENT')