...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bdgt/commands/categories.py
    ...
    def __call__(self):
        if self.parent_name is not None:
            with session_scope() as (session):
                try:
                    parent = session.query(Category).filter_by(name=self.parent_name).one()
                except NoResultFound:
                    raise ValueError(("Category '{}' does not exist.").format(self.parent_name))
...
class CmdDelete(object):
    ...
    def __call__(self):
        with session_scope() as (session):
            try:
                category = session.query(Category).filter_by(name=self.name).one()
            except NoResultFound:
                raise ValueError(("Category '{}' does not exist.").format(self.name))
...
class CmdRename(object):
    ...
    def __call__(self):
        ...
        save_object(category)
        return ("Category '{}' renamed to '{}'.").format(self.name, self.new_name)