...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bdgt/commands/transactions.py
...
class CmdListTx(object):
    ...
    def __call__(self):
        with session_scope() as (session):
            txs = session.query(Transaction).filter(Transaction.account.has(name=self.account_name)).order_by(Transaction.date).all()
        output = defaultdict(list)
        for tx in txs:
            output['id'].append(tx.id)
            output['date'].append(str(tx.date))
            output['description'].append(tx.description[:130].replace('\n', ' '))
        ...
        output_io = StringIO()
        asciitable.write(output, output_io, Writer=asciitable.FixedWidthNoHeader, names=[
         'id', 'date', 'description', 'category',
         'reconciled', 'amount'], formats={'amount': lambda x: format_amount(x), 
    ...
    def __call__(self):
        with session_scope() as (session):
            try:
                ...
                raise ValueError(("Category '{}' not found.").format(self.category_name))

        with session_scope() as (session):
            txs = session.query(Transaction).filter(Transaction.id.in_(self.tx_ids)).all()
        for tx in txs:
            tx.category = category

        save_objects(txs)
        return ('Assigned {} transactions to the {} ' + 'category').format(len(txs), self.category_name)
...
class CmdUnassignTx(ParseIdMixin):
    ...
    def __call__(self):
        ...
        save_objects(txs)
        return ('Unassigned {} transactions from their ' + 'category').format(len(txs))
