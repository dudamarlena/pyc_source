...
# Embedded file name: build/bdist.macosx-10.9-intel/egg/cpip/plot/SVGWriter.py
# Compiled at: 2017-10-03 13:07:16
__doc__ = 'An SVG writer.'
...
class SVGWriter(XmlWrite.XmlStream):
    ...
    def __enter__(self):
        ...
        _attrs = {'xmlns': 'http://www.w3.org/2000/svg', 
           'version': '1.1', 
...
class SVGPointList(XmlWrite.Element):
    ...
    def __init__(self, theXmlStream, name, pointS, attrs):
        _attrs = {'points': (' ').join([ '%s,%s' % (p.x.value, p.y.value) for p in pointS ])}
        if attrs:
            _attrs.update(attrs)
