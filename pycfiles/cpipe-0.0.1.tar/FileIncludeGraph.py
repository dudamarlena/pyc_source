...
# Embedded file name: build/bdist.macosx-10.9-intel/egg/cpip/core/FileIncludeGraph.py
# Compiled at: 2017-10-03 13:07:16
__doc__ = 'Captures the ``#include`` graph of a preprocessed file.'
...
class FileIncludeGraph(object):
    ...
    """
    LINE_SEPERATOR = '#'
    RE_SPLIT_LINENUM = re.compile('^(.+?)%s([-+]?\\d+)$' % LINE_SEPERATOR)
        ...
        if self._tokCntr is not None:
            ...
            for aG in self._graph.values():
                ...
                if myChildTokCount is None:
                    raise ExceptionFileIncludeGraphTokenCounter('I have a token count but my child, %s, does not' % aG)
                else:
    ...
    def setTokenCounter(self, theTokCounter):
        ...
        if self._tokCntr is not None:
            raise ExceptionFileIncludeGraph('Calling setTokenCounter() when token counter already set.')
    ...
    def addBranch(self, theFileS, theLine, theIncFile, theState, theCondition, theLogic):
        ...
        if len(theFileS) == 1:
            if theLine in self._graph:
                ...
                assert len(self._graph) == 0 or theLine > max(self._graph.keys()), 'File=%s line=%d maxLine=%d' % (theIncFile, theLine, max(self._graph.keys()))
                self._graph[theLine] = FileIncludeGraph(theIncFile, theState, theCondition, theLogic)
        else:
            if len(self._graph) == 0:
                raise ExceptionFileIncludeGraph('FileIncludeGraph.addBranch() "%s" has no includes.' % self._fileName)
            myLineNum = max(self._graph.keys())
            self._graph[myLineNum].addBranch(theFileS[1:], theLine, theIncFile, theState, theCondition, theLogic)
