...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mimseq/tRNAtools.py
# Compiled at: 2020-04-07 06:40:34
# Size of source mod 2**32: 45067 bytes
...
def modsToSNPIndex(gtRNAdb, tRNAscan_out, mitotRNAs, modifications_table, experiment_name, out_dir, snp_tolerance=False, cluster=False, cluster_id=0.95, posttrans_mod_off=False, pretrnas=False):
    ...
    for seq in tRNA_dict:
        ...
        if anticodon not in anticodon_list:
            anticodon_list.append(anticodon)
        match = {k:v for k, v in modomics_dict.items() if tRNA_dict[seq]['type'] == v['type']}
        if len(match) >= 1:
    ...
    if not cluster:
        ...
        with open(out_dir + experiment_name + '_tRNA.gff', 'w') as (tRNAgff):
            with open(out_dir + experiment_name + 'isoacceptorInfo.txt', 'w') as (isoacceptorInfo):
        ...
        for seq in mod_lists:
            additionalMods_sub = {k:v for k, v in additionalMods.items() if tRNA_dict[seq]['species'] in v['species']}
        ...
        for seq in Inosine_lists:
            additionalInosines_sub = {k:v for k, v in additionalInosines.items() if tRNA_dict[seq]['species'] in v['species']}
        ...
        if cluster:
            ...
            for anticodon in anticodon_list:
                ...
                cluster_cmd = [
                 'usearch', '-cluster_fast', temp_dir + anticodon + '_allseqs.fa', '-id', str(cluster_id), '-sizeout', '-centroids', temp_dir + anticodon + '_centroids.fa', '-uc', temp_dir + anticodon + '_clusters.uc']
                subprocess.check_call(cluster_cmd, stdout=(subprocess.DEVNULL), stderr=(subprocess.DEVNULL))
                sort_cmd = [
                 'usearch', '-sortbysize', temp_dir + anticodon + '_centroids.fa', '-fastaout', temp_dir + anticodon + '_centroids_sort.fa']
                subprocess.check_call(sort_cmd, stdout=(subprocess.DEVNULL), stderr=(subprocess.DEVNULL))
                final_cluster_cmd = [
                 'usearch', '-cluster_smallmem', temp_dir + anticodon + '_centroids_sort.fa', '-sortedby', 'size', '-id', str(cluster_id), '-centroids', temp_dir + anticodon + '_centroidsFinal.fa']
            ...
            for cluster in mod_lists:
                additionalMods_sub = {k:v for k, v in additionalMods.items() if tRNA_dict[cluster]['species'] in v['species']}
                if additionalMods_sub:
            ...
            for cluster in Inosine_lists:
                additionalInosines_sub = {k:v for k, v in additionalInosines.items() if tRNA_dict[cluster]['species'] in v['species']}
                if additionalInosines_sub:
...
def newModsParser(out_dir, experiment_name, new_mods_list, new_Inosines, mod_lists, Inosine_lists, tRNA_dict, clustering):
    ...
    for l in new_Inosines:
        for cluster, inosines in l.items():
            ...
            newInosine_lists[cluster] = list(set(newInosine_lists[cluster] + l[cluster]))
...
def additionalModsParser(input_species, out_dir):
    ...
    for isodecoder, data in additionalMods.items():
        if data['type'] == 'mito':
            clusters = [key for key, value in tRNA_struct_nogap.items() if isodecoder in key if 'nmt' not in key if 'mito' in key]
        else:
            clusters = [key for key, value in tRNA_struct_nogap.items() if isodecoder in key if 'nmt' not in key if 'mito' not in key]
        for cluster in clusters:
            no_gap_struct = [value for key, value in tRNA_struct_nogap.items() if key == cluster if 'nmt' not in key]
            if not no_gap_struct:
                pass
            else:
                gap_struct = [value for key, value in tRNA_struct.items() if key == cluster if 'nmt' not in key]
    ...
    return (
     additionalMods_parse, additionalInosines)
...
def generateGSNAPIndices(species, name, out_dir, map_round, snp_tolerance=False, cluster=False):
    ...
    if snp_tolerance:
        ...
        index_cmd = ['snpindex', '-q', '1', '-D', genome_index_path, '-d', species + '_tRNAgenome', '-V', snp_index_path,
         '-v', name + '_modificationSNPs',
         snp_index_path + '/' + name + '_modificationSNPs.iit']
...
def generateSNPIndex(experiment_name, out_dir, snp_tolerance=False):
    ...
    if snp_tolerance:
        ...
        index_cmd = ['snpindex', '-q', '1', '-D', genome_index_path, '-d', experiment_name + '_tRNAgenome', '-V', snp_index_path,
         '-v', experiment_name + '_modificationSNPs',
         snp_index_path + '/' + experiment_name + '_modificationSNPs.iit']
    ...
    for line in mods:
        if not line.startswith('#'):
            ...
            if mod and not mod.isspace():
                modifications[mod.strip()] = {'name':name.strip(), 
                 'abbr':abbr.strip(),  'ref':ref.strip()}
