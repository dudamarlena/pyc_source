...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/asyncluster/master/control.py
...
class SessionManager(pb.Viewable):
    ...
    def _update(self):
        ...
        for ID in self.sessionMap.keys():
            ...
            if hoursLeft >= 0.0:
                ...
                d = self.end(ID)
            d.addCallback(lambda _: log('Done updating client', ID, userID))
            dList.append(d)
    ...
    def end(self, ID, callClient=True):
        ...
        if ID not in self.sessionMap:
            ...
            if callClient:
                d.addCallback(lambda _: self.ctl.nodeRemote(ID, 'setTimeLeft', 0.0))
        return d
...
class Controller(object):
    ...
    def nodeRemote(self, nodeID, called, *args, **kw):
        ...
        if nodeID not in self.nodes:
            return defer.fail(Failure(RemoteCallError("Invalid node '%s'" % nodeID)))
    ...
    def userRemote(self, userID, called, *args, **kw):
        ...
        return defer.fail(Failure(RemoteCallError("Invalid user '%s'" % userID)))
