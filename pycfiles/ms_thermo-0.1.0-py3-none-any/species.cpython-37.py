...
# [GCC 8.4.0]
# Embedded file name: /Users/defontaine/Nitrox/ms_thermo/src/ms_thermo/species.py
# Compiled at: 2020-02-18 07:52:52
# Size of source mod 2**32: 17774 bytes
"""Module to build thermodynamic properties of species."""
import numpy as np
from scipy.interpolate import interp1d
from .constants import GAS_CST, ATOMIC_WEIGHTS
__all__ = [
...
class _NasaPolys:
    ...
    def get_total_enthalpy(self, temp):
        ...
        condlist = [_temp < self.t_range['mid'],
    ...
    def get_total_energy(self, temp):
        """Computing total mass energy as : $e_k(T) = h_k(T) -RT=W_k$
...
def _get_species_indexes(database_file, db_format):
    ...
    for i, line in enumerate(db_stream, 0):
        line = line.strip()
        cond = any([db_format == 'avbp' and line.startswith('species_name ='),
...
def _make_props_avbp(species_description):
    ...
    props = dict()
    props['species_name'] = [line.split()[(-1)] for line in species_description if line.startswith('species_name =')][0]
...
def _make_props_cantera(species_description):
    ...
    """
    props = dict()
    props['species_name'] = [line for line in species_description if line.startswith('species(name')][0].strip()
    ...
    props['molecular_weight'] = _get_mol_weight_from_elements(elements)
    line = [k for k in ''.join([' '.join(k.strip().split()) for k in species_description]).split('=') if 'NASA' in k][0]
...
class Species:
    ...
    def __init__(self, mol_weight, enthalpy_func, energy_func):
        """
...
def build_thermo_from_avbp(database_file):
    ...
    *Reading all AVBP database species and storing in a dict( )     whose keys are species names*
    ...
    for i, _ in enumerate(indexes, 0):
        ...
        species[props['species_name']] = Species(props['molecular_weight'], props['total_enthalpy'], props['chemical_energy'])
