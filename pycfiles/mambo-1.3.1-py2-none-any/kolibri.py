...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/Yass/kolibri/kolibri.py
# Compiled at: 2019-06-02 22:44:06
"""
~ Mambo ~
...
class Mambo(object):
    ...
    def setup_jinja(self):
        ...
        global_context = {'page_url': self._fn_page_url, 
           ...
           'data': {}, '__info__': {'name': __title__, 
                        'version': __version__, 
                        'url': __uri__, 
                        'generator': '%s %s' % (__title__, __version__), 
                        'timestamp': int(time.time())}}
        env_extensions = [
         'kolibri.md_ext.MarkdownExtension',
         'kolibri.md_ext.MarkdownTagExtension']
        content_loaders = [
    ...
    def _fn_page_link(self, filename, text=None, title=None, _class='', id='', alt='', **kwargs):
        ...
        if '#' in filename:
            ...
            anchor = '#' + anchor
        return ('<a href=\'{url}\' class=\'{_class}\' id=\'{id}\'  title="{title}">{text}</a>').format(url=self._fn_page_url(filename, '/') + anchor, text=text or title or self._fn_page_info(filename, 'title', title), title=title or text or '', _class=_class, id=id)
    ...
    def aggregate_pages_data(self):
        ...
        for root, _, files in os.walk(self.pages_dir):
            ...
            if base_dir.startswith('_'):
                continue
            for f in files:
                if f.startswith(('_', '.')) or not f.endswith(PAGE_FORMAT):
                    continue
                ...
                self.pages.update({filename: read_markup_file(filepath, root=self.pages_dir)})
        ...
        if sfc[0] is True:
            ...
            for o in sfc_o:
                if sfc_c.get(o):
                    ...
                    assets[assets_key].append({'url': _sff, 
