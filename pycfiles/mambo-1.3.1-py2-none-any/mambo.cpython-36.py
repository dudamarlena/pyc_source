...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/Yass/mambo/mambo.py
# Compiled at: 2019-06-30 17:25:57
# Size of source mod 2**32: 27971 bytes
"""
~ Mambo ~
An elegant static site generator
"""
import os, re, sys, copy, json, time, yaml, arrow, shutil, jinja2, htmlmin, logging, functools, frontmatter, pkg_resources
...
STATIC_CACHE_BUST_CHECKSUM_FILE_TYPES = ('.js', '.css', '.svg', '.png', '.gif', '.jpg',
                                         '.jpeg')
...
RE_BLOCK_BODY = re.compile('{%\\s*block\\s+__PAGE_CONTENT__\\s*%}')
RE_BLOCK_BODY_PARSED = re.compile('{%\\s*block\\s+__PAGE_CONTENT__\\s*%}(.*?){%\\s*endblock\\s*%}')
...
def read_markup_file(filepath, root=''):
  ...
  """
    ...
    with open(filepath) as (f):
        ...
        if meta.get('url'):
            url = meta.get('url')
        meta.update({'url':url.lstrip('/').rstrip('/'), 
         'filepath':dest_file, 
         'filedest':dest_file, 
         'basefile':basefile, 
         'markup':markup, 
...
def get_content_files_collection(dir):
  ...
  """
    for root, _, files in os.walk(dir):
        for fname in files:
            if not fname.endswith(PAGE_FORMAT):
                pass
            else:
                fname = os.path.join(root, fname)
                if os.path.isfile(fname):
...
class Mambo(object):
    ...
    def __init__(self, root_dir, options={}):
        ...
        if self.config.get('globals.assets'):
            ...
            DEFAULT_PAGE_META['assets'] = {'scripts':_scripts, 
    ...
    def setup_jinja(self):
        ...
        site_config.update({'__env__':self.site_env if self.site_env else DEFAULT_ENVIRONMENT, 
         '__generator__':{'name':__title__, 
          'version':__version__, 
          'url':__uri__, 
          'generator':'%s %s' % (__title__, __version__), 
          'timestamp':int(time.time())}})
        global_context = {'page_url':self._fn_page_url, 
    ...
    def _fn_page_link(self, filename, text=None, title=None, _class='', id='', alt='', **kwargs):
        ...
        if '#' in filename:
            ...
            anchor = '#' + anchor
        return '<a href=\'{url}\' class=\'{_class}\' id=\'{id}\'  title="{title}">{text}</a>'.format(url=(self._fn_page_url(filename, '/') + anchor),
    ...
    def _fn_static_url(self, url):
        ...
        if utils.is_https_string(url):
            ...
            if self.enable_cache_busting:
                if self.cache_busting_ignores:
                    if url in self.cache_busting_ignores:
                        url = url
                if self.cache_busting_checksum:
                    if len(self.cache_busting_checksum) > 0:
    ...
    def build_static(self):
        ...
        if not os.path.isdir(self.build_static_dir):
            ...
            _assets = utils.copy_static_dir(self.static_dir, self.build_static_dir, self.cache_busting_checksum, cache_busting_extensions, self.cache_busting_ignores)
    ...
    def aggregate_pages_data(self):
        ...
        for root, _, files in os.walk(self.pages_dir):
            ...
            if base_dir.startswith('_'):
                pass
            else:
                for f in files:
                    if not f.startswith(('_', '.')):
                        if not f.endswith(PAGE_FORMAT):
                            pass
                        else:
                            ...
                            if markup_file.get('meta').get('publish') is False:
                                ...
                                self.pages.update({filename: markup_file})
    ...
    def _build_page(self, filename):
        ...
        if meta.get('collections'):
            if meta.get('collections').get('data_file'):
                ...
                if meta.get('collections').get('content_dir'):
    ...
    def create_page(self, filepath, context={}, content=None, layout=None):
        ...
        self.manifest.append({'filepath':filepath, 
