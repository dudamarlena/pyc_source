...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/Yass/yass/publisher.py
# Compiled at: 2017-03-26 09:29:52
import botocore, boto3, json, os, threading, uuid, tempfile
from . import utils
...
class S3Website(object):
    ...
    def __init__(self, sitename, region='us-east-1', aws_access_key_id=None, aws_secret_access_key=None):
        """
    ...
    def setup_dns(self):
        ...
        if not hosted_zone_id:
            ...
            response = route53.create_hosted_zone(Name=self.sitename, CallerReference=caller_reference_uuid, HostedZoneConfig={'Comment': 'HostedZone created by YASS!', 'PrivateZone': False})
        ...
        change_batch_payload = {'Changes': [
                     {'Action': 'UPSERT', 
                        'ResourceRecordSet': {'Name': self.sitename, 
                                              'Type': 'A', 
                                              'AliasTarget': {'HostedZoneId': self.S3_HOSTED_ZONE_IDS[self.region], 
                                                              'DNSName': website_dns_name, 
                                                              'EvaluateTargetHealth': False}}},
                     {'Action': 'UPSERT', 
                        'ResourceRecordSet': {'Name': self.www_sitename, 
                                              'Type': 'A', 
                                              'AliasTarget': {'HostedZoneId': self.S3_HOSTED_ZONE_IDS[self.region], 
                                                              'DNSName': redirect_dns_name, 
                                                              'EvaluateTargetHealth': False}}}]}
        response = route53.change_resource_record_sets(HostedZoneId=hosted_zone_id, ChangeBatch=change_batch_payload)
    ...
    def create_website(self):
        ...
        if not exists:
            if error_code == '404':
                ...
                website_payload = {'ErrorDocument': {'Key': 'error.html'}, 
                   'IndexDocument': {'Suffix': 'index.html'}}
                self.s3.create_bucket(Bucket=self.sitename)
                self.s3.put_bucket_policy(Bucket=self.sitename, Policy=json.dumps(policy_payload))
                self.s3.put_bucket_website(Bucket=self.sitename, WebsiteConfiguration=website_payload)
                return True
            raise Exception("Can't create website's bucket '%s' on AWS S3. Error: %s" % (
             self.sitename, error_message))
        return False
    ...
    def create_www_website(self):
        ...
        if not exists:
            if error_code == '404':
                ...
                redirect_payload = {'RedirectAllRequestsTo': {'HostName': self.sitename, 
                                             'Protocol': 'http'}}
                ...
                return True
            raise Exception("Can't create website's bucket '%s' on AWS S3. Error: %s" % (
             self.www_sitename, error_message))
        return False
    ...
    def upload(self, build_dir):
        ...
        for root, dirs, files in os.walk(build_dir):
            for filename in files:
                ...
                kwargs = dict(aws_params=self.aws_params, bucket_name=self.sitename, local_path=local_path, s3_path=s3_path, mimetype=mimetype)
                files_list.append(s3_path)
                threading.Thread(target=self._upload_file, kwargs=kwargs).start()

        self._set_manifest_data(files_list)

    def purge_files(self, exclude_files=[
 'index.html', 'error.html']):
        ...
        for chunk in utils.chunk_list(self._get_manifest_data(), 1000):
            try:
                self.s3.delete_objects(Bucket=self.sitename, Delete={'Objects': [ {'Key': f} for f in chunk if f not in exclude_files
                            ]})
    ...
    def _set_manifest_data(self, files_list):
        ...
        if files_list:
            ...
            self.s3.put_object(Bucket=self.sitename, Key=self.manifest_file, Body=data, ACL='private')
