...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/Yass/mambo/utils.py
# Compiled at: 2019-06-12 00:31:29
# Size of source mod 2**32: 5997 bytes
import os, re, uuid, yaml, mimetypes
...
RE_IS_URL = re.compile('^(http|https)://')
        ...
        if not n.startswith(('.', '_')):
            if os.path.islink(src_name):
                ...
                if cb_checksum is not None:
                    if len(cb_checksum) > 0:
                        if os.path.isfile(src_name):
                            if n.endswith(cb_extensions):
                                if base_src_name not in cb_ignores:
