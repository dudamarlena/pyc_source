...
# [GCC 8.4.0]
# Embedded file name: /Users/shin4590/src/forks/blueflood-carbon-forwarder/twisted/plugins/graphite_blueflood_plugin.py
# Compiled at: 2016-01-19 14:24:56
import logging
...
class MetricService(Service):
    ...
    def _setup_blueflood(self, factory, agent):
        log.msg(('Send metrics to {} as tenant {} with {} sec interval').format(self.blueflood_url, self.tenant, self.flush_interval))
        log.msg(('Limit is {} bytes').format(self.limit))
        client = BluefloodEndpoint(ingest_url=self.blueflood_url, tenant=self.tenant, agent=agent, limit=int(self.limit))
...
class MetricServiceMaker(object):
    ...
    def makeService(self, options):
        return MetricService(protocol_cls=MetricPickleReceiver, endpoint=options['endpoint'], interval=float(options['interval']), blueflood_url=options['blueflood'], tenant=options['tenant'], ttl=int(options['ttl']), user=options['user'], key=options['key'], auth_url=options['auth_url'], limit=options['limit'], metric_prefix=options['metric_prefix'])
