...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.4/dist-packages/gpotato/selector_events.py
# Compiled at: 2015-05-03 15:01:52
# Size of source mod 2**32: 31620 bytes
...
import collections, errno, socket
try:
    import ssl
except ImportError:
    ssl = None
        ...
        try:
            ...
            if exc.errno in (errno.EMFILE, errno.ENFILE,
             errno.ENOBUFS, errno.ENOMEM):
                ...
                self.call_later(constants.ACCEPT_RETRY_DELAY, self._start_serving, protocol_factory, sock, sslcontext, server)
            else:
                raise
        else:
            if sslcontext:
    ...
    def __init__(self, loop, rawsock, protocol, sslcontext, waiter=None, server_side=False, server_hostname=None, extra=None, server=None):
        ...
        if server_side:
            ...
            if hasattr(ssl, '_create_stdlib_context'):
                sslcontext = ssl._create_stdlib_context(cert_reqs=ssl.CERT_REQUIRED, check_hostname=bool(server_hostname))
            else:
        ...
        if server_hostname:
            if not server_side and ssl.HAS_SNI:
