...
# [GCC 8.4.0]
# Embedded file name: /root/works/github/depthwise-conv-pytorch/torch_dwconv/dwconv.py
# Compiled at: 2020-02-20 01:59:33
...
def use_dwconv2d_small(input, weight, up, stride, padding):
    return torch_dwconv._C.use_dwconv2d_small(input, weight, up[0], up[1], stride[0], stride[1], padding[0], padding[1])
...
class DepthwiseConv2dFunction(Function):
    ...
    def forward(ctx, input, kernel, stride, pad, small):
        ...
        if ctx.use_small:
            out = (torch_dwconv._C.dwconv2d_small)(input, kernel, 1, 1, *stride, *pad, *(True, ))
        else:
            out = (torch_dwconv._C.dwconv2d)(input, kernel, 1, 1, *stride, *(pad[0], pad[0], pad[1], pad[1], True))
    ...
    def backward(ctx, grad_output):
        ...
        g_pad = ctx.g_pad
        if ctx.use_small:
            grad_input = (torch_dwconv._C.dwconv2d_small)(grad_output, kernel, *stride, *(1, 1, g_pad[0], g_pad[2], False))
        else:
            grad_input = (torch_dwconv._C.dwconv2d)(grad_output, kernel, *stride, *(1,
                                                                                    1), *g_pad, *(False, ))
        grad_kernel = (torch_dwconv._C.dwconv2d_backward_kernel)(input, grad_output, kernel, 1, 1, *stride, *pad)
        return (
         grad_input, grad_kernel, None, None, None, None)
...
def make_tuple(value, n_value):
    if not isinstance(value, (list, tuple)):
        ...
        n_item = len(value)
        if n_item > n_value:
            raise ValueError(f"Number items does not match with requirements: {n_item}, expected: {n_value}")
        if len(value) == n_value:
            return value
        return value * n_value


def check_options(in_channels, out_channels, dilation, groups, bias, padding_mode='zeros'):
...
def depthwise_conv2d(input, weight, bias=None, stride=1, padding=0, dilation=1, groups=None):
        ...
        if input.shape[2] <= 32:
            if input.shape[3] <= 32:
                return F.conv2d(input,
                  weight, stride=stride, padding=padding, groups=(input.shape[1]))
    return depthwise_conv2d_fn(input, weight, stride, padding, small)


class DepthwiseConv2d(nn.Module):

    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=None, bias=False, padding_mode='zeros', custom=True):
        super().__init__()
