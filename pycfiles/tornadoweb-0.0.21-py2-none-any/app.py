...
# [GCC 8.4.0]
# Embedded file name: /app/insight2/tornadoweb/app.py
# Compiled at: 2019-10-30 04:34:48
...
class Application(object):
    ...
    def _get_handlers(self):
        ...
        for d in __conf__.ACTION_DIR_NAME:
            members.update(get_members(d, None, lambda m: isclass(m) and issubclass(m, RequestHandler) and hasattr(m, '__urls__') and m.__urls__))
        ...
        handlers.append(('^/upload/(.*?)$', tornado.web.StaticFileHandler, {'path': 'upload', 'default_filename': 'index.html'}))
        from .web import BaseHandler
        handlers.append(('^(.*?)$', BaseHandler))
        return handlers
