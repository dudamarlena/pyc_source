...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/metaq/producer.py
# Compiled at: 2012-05-10 00:57:27
...
def get_round_robin_selector():
    ...
    def round_robin_select(topic, partitions, msg):
        ...
        return partitions[(ji.increase_and_get() % plen)]
...
class Message:
    ...
    def encode(self, partition, opaque):
        """ 
         ...
         """
        vlen = len(self.data)
        return 'put %s %d %d %d %d\r\n%s' % (self.topic, partition, vlen, self.flag, opaque, self.data)
...
class MessageProducer:
    ...
    def __init__(self, topic, zk_servers='localhost:2181', partition_selector=get_round_robin_selector(), zk_timeout=10000, zk_root='/meta', dead_retry=_DEAD_RETRY, socket_timeout=_SOCKET_TIMEOUT, debug=True):
        """
    ...
    def send(self, msg):
        ...
        if partition == None:
            raise _Error('There is no aviable partition for topic %s' % topic)
        conn = self._conn_dict.get(partition.broker_id)
        if conn == None:
            raise _Error('There is no aviable server right now for topic % and partition %d' % (topic, partition.partition))
        ...
        def _unsafe_send(cmd, message):
            try:
                ...
                if len(body) != bodylen:
                    conn.mark_dead('reponse format error,expect body length is %s,but is %s' % (bodylen, len(body)))
                    return SendResult(False, None, -1, error='network error')
                if resp_opaque != opaque:
                    conn.mark_dead('reponse opaque is not equals to request opaque')
                    return SendResult(False, None, -1, error='network error')
