...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/sql_extract/__init__.py
# Compiled at: 2020-03-27 10:19:40
# Size of source mod 2**32: 12673 bytes
"""
Utility functions and class for sql-extract
"""
import sys, os, re, csv, openpyxl, logging, argparse
...
class SqlExtractHandler(object):
    ...
    def write_results(self):
        ...
        with open(self.csv_file, 'w') as (csv_handler):
            writer = csv.DictWriter(csv_handler, fieldnames=(self.columns), quoting=(self.quoting), dialect='local_custom')
            writer.writeheader()
        ...
        if in_binds:
            ...
            for param_input in in_binds:
                if not pattern.match(param_input):
                    ...
                    out_binds[parts[0]] = '='.join(parts[1:])
...
def main(cmd_line_args):
    ...
    """
    if cmd_line_args.login:
        if not cmd_line_args.password:
            logging.error('Missing Oracle credentials. You must set "full_login" and "db_password". Alternatively, you can pass your credentials to this script with the --login and --password parameters.')
    else:
        conn = get_cx_oracle_connection(login=(cmd_line_args.login), password=(cmd_line_args.password))
        try:
            try:
                logging.basicConfig(level=(os.environ.get('LOGLEVEL', 'INFO').upper()))
                handler = SqlExtractHandler(conn, (cmd_line_args.filename), (cmd_line_args.outfile), in_delim=(cmd_line_args.delimiter),
                  in_quot=(cmd_line_args.quotechar),
                ...
                logging.info('{0} {1} {2}'.format(in_display_path, str(handler.row_count).rjust(7), 'records processed'))
...
def csv2xlsx(cmd_line_args):
    if not cmd_line_args.filename:
        ...
        logging.info(f"{os.path.splitext(cmd_line_args.filename)[0].ljust(35)} " + f"{str(handler.row_count).rjust(7)} records processed")
