...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/bs4/element.py
# Compiled at: 2013-03-18 16:15:18
import collections, re, sys, warnings
...
class PageElement(object):
    ...
    FORMATTERS = {'html': EntitySubstitution.substitute_html, 
    ...
    def format_string(self, s, formatter='minimal'):
        ...
        if not callable(formatter):
            formatter = self.FORMATTERS.get(formatter, EntitySubstitution.substitute_xml)
    ...
    def insert(self, position, new_child):
        ...
        if isinstance(new_child, basestring) and not isinstance(new_child, NavigableString):
        ...
        if operator == '=':
            ...
            if operator == '|':
                ...
                def _is_or_starts_with_dash(element):
                    ...
                    return attribute_value == value or attribute_value.startswith(value + '-')
...
class Tag(PageElement):
    ...
    def __getattr__(self, tag):
        if len(tag) > 3 and tag.endswith('Tag'):
            tag_name = tag[:-3]
            warnings.warn('.%sTag is deprecated, use .find("%s") instead.' % (
             tag_name, tag_name))
        ...
        if self.attrs:
            for key, val in sorted(self.attrs.items()):
                if val is None:
                    ...
                    decoded = unicode(key) + '=' + EntitySubstitution.quoted_attribute_value(text)
...
class SoupStrainer(object):
    ...
    def _normalize_search_value(self, value):
        if isinstance(value, unicode) or callable(value) or hasattr(value, 'match') or isinstance(value, bool) or value is None:
            return value
        if isinstance(value, bytes):
            ...
            if hasattr(value, '__iter__'):
                new_value = []
                for v in value:
                    if hasattr(v, '__iter__') and not isinstance(v, bytes) and not isinstance(v, unicode):
                        new_value.append(v)
                    else:
    ...
    def search_tag(self, markup_name=None, markup_attrs={}):
        ...
        if isinstance(markup_name, Tag):
            markup = markup_name
            markup_attrs = markup
        call_function_with_tag_data = isinstance(self.name, collections.Callable) and not isinstance(markup_name, Tag)
    ...
    def _matches(self, markup, match_against):
        ...
        if isinstance(markup, list) or isinstance(markup, tuple):
            if isinstance(match_against, unicode) and ' ' in match_against:
