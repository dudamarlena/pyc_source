...
# Embedded file name: C:/Work/Python/workplace/pypsd/pypsd\ps_parser.py
# Compiled at: 2009-07-14 11:36:19
...
class PSParser(object):
    ...
    def getDictKey(self, line):
        ...
        (empty, key, rest_line) = re.compile('^/(\\S+)\\s?').split(line)
        return (
         key, rest_line.strip())
...
class _PSParserTest(unittest.TestCase):
    ...
    def setUp(self):
        self.file_stream = open('../samples/ps_example.txt', 'rb')
    ...
    def test_array_parse(self):
        ...
        obj = ps.parse()
        assert obj == [-10.12, -10, 20.12, [0.19, -0.2], 30, 20]
