...
# Compiled at: 2020-03-14 08:50:48
...
class Youtube2Zim(object):
    ...
    def download_video_files(self, max_concurrency):
        ...
        options = {'cachedir':self.videos_dir, 
         ...
         'outtmpl':str(self.videos_dir.joinpath('%(id)s', f"video.{vidext}")), 
        ...
        with youtube_dl.YoutubeDL(options) as (ydl):
            for video_id in videos_ids:
                try:
                    ...
                    post_process_video(options['y2z_videos_dir'].joinpath(video_id), video_id, options['y2z_video_format'], options['y2z_low_quality'])
    ...
    def make_test_html_file(self):
        ...
        with open((self.assets_dir.joinpath('data.js')), 'w', encoding='utf-8') as (fp):
            videos = [{'id':video_id,  'title':'A Sample Title',  'slug':'a-sample-title',  'description':'A sample description of video',  'subtitles':[],  'thumbnail':str(Path('videos').joinpath(video_id, 'video.jpg'))} for _ in range(0, 6)]
