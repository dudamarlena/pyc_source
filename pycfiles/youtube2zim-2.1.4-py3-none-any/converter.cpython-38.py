...
# Compiled at: 2020-04-21 13:06:33
...
def post_process_video(video_dir, video_id, video_format, low_quality, skip_recompress=False):
    ...
    if len(files) == 0:
        ...
        if skip_recompress or (low_quality or src_path.suffix[1:]) == video_format:
            return
...
def recompress_video(src_path, dst_path, video_format):
    ...
    logger.info(f"recompress {src_path} -> {dst_path} video_format={video_format!r}")
