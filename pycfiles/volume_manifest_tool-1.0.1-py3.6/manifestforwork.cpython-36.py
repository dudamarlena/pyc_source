...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/v_m_t/manifestforwork.py
# Compiled at: 2020-03-26 02:20:46
# Size of source mod 2**32: 15161 bytes
import argparse, csv, gzip, io, json, logging, os
...
csvlock: Lock = Lock()
s3_work_manager: S3WorkFileManager = S3WorkFileManager(S3_MANIFEST_WORK_LIST_BUCKET, todo_prefix, processing_prefix, done_prefix)
...
def parse_args(arg_namespace: object) -> None:
    ...
    _parser.add_argument('-l', '--logLevel', dest='log_level', action='store', choices=[
     'info', 'warning', 'error', 'debug', 'critical'],
      default='info',
      help='choice values are from python logging module')
    _parser.add_argument('-d', '--logParent', dest='log_parent', action='store', default='/tmp', help='Path to log file directory')
    _parser.add_argument('sourceFile', help='File containing one RID per line.', nargs='?', type=(argparse.FileType('r')))
    _parser.add_argument('-i', '--interval', dest='poll_interval', help='Seconds between alerts for file.', required=False,
      default=60,
      type=int)
...
def exception_handler(exception_type, exception, traceback):
    ...
    your format
    """
    error_string = f"{exception_type.__name__}: {exception}"
...
def manifestForVolume(client, workRID, vi, csvwriter):
    ...
    """
    s3_folder_prefix = get_s3_folder_prefix(workRID, vi.imageGroupID)
    if manifestExists(client, s3_folder_prefix):
        shell_logger.info('manifest exists: ' + workRID + '-' + vi.imageGroupID)
...
def uploadManifest(bucket, s3folderPrefix, manifestObject):
    ...
    bucket.put_object(Key=key, Body=manifest_gzip, Metadata={'ContentType':'application/json', 
     'ContentEncoding':'gzip'},
...
def fillData(transfer, s3imageKey, csvwriter, imgdata, workRID, imageGroupID):
    ...
    filename = io.BytesIO()
    try:
        transfer.download_file(S3_DEST_BUCKET, s3imageKey, filename, callback=(DoneCallback(filename, imgdata, csvwriter, s3imageKey, workRID, imageGroupID)))
    except botocore.exceptions.ClientError as e:
        if e.response['Error']['Code'] == '404':
            csvline = [
             s3imageKey, workRID, imageGroupID, '', '', '', '', '', '', '', 'keydoesnotexist']
...
def fillDataWithBlobImage(blob, data, csvwriter, s3imageKey, workRID, imageGroupID):
    ...
    if errors:
        csvline = [
         s3imageKey, workRID, imageGroupID, size, im.width, im.height, im.mode, im.format, im.palette,
...
def buildWorkListFromS3(client: object) -> (str, []):
    ...
    """
    page_iterator = client.get_paginator('list_objects_v2').paginate(Bucket=S3_MANIFEST_WORK_LIST_BUCKET, Prefix=todo_prefix)
    file_list = []
