...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/v_m_t/detectgraynb.py
# Compiled at: 2019-10-02 14:32:30
...
def detect_color_image(file, thumb_size=100, MSE_cutoff=50, MSE_bw_cutoff=30, adjust_color_bias=True):
    ...
    if bands == ('R', 'G', 'B') or bands == ('R', 'G', 'B', 'A'):
        ...
        for pixel in thumb.getdata():
            mu = sum(pixel) / 3
            SSE_gs += sum((pixel[i] - mu - bias[i]) * (pixel[i] - mu - bias[i]) for i in (0,
                                                                                          1,
                                                                                          2))
...
files = [
 'peydurma.jpeg', 'peydurma-color.jpeg', 'smallcolor.jpeg', 'manuscript-1.jpeg']
for file in files:
    print(file)
