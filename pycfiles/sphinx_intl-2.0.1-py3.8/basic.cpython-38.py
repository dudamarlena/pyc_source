...
# [GCC 8.4.0]
# Embedded file name: _build/bdist.macosx-10.15-x86_64/egg/sphinx_intl/basic.py
                ...
                for lang in languages:
                    ...
                    if os.path.exists(po_file):
                        ...
                        if msgids != new_msgids:
                            ...
                            status['update'] += 1
                            click.echo('Update: {0} +{1}, -{2}'.format(po_file, len(added), len(deleted)))
        ...
        for dirpath, dirnames, filenames in os.walk(lang_dir):
            for filename in filenames:
                ...
                if ext != '.po':
                    ...
                    cat = c.load_po(po_file)
                    r = result[po_file.replace('\\', '/')] = {'translated':len(c.translated_entries(cat)), 
                     'fuzzy':len(c.fuzzy_entries(cat)), 
                     'untranslated':len(c.untranslated_entries(cat))}
                    click.echo('{0}: {1} translated, {2} fuzzy, {3} untranslated.'.format(po_file, r['translated'], r['fuzzy'], r['untranslated']))
