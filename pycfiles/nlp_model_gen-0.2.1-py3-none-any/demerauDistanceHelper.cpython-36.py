...
# [GCC 8.4.0]
# Embedded file name: /home/gerardo/Projects/nlp_model_gen/nlp_model_gen/packages/wordProcessor/spanishFuzzyTermsGenerator/helpers/demerauDistanceHelper.py
...
def get_accent_adjustment(s1, s2, ignore_case):
    ...
    for special_char, non_special_char in WORD_PROCESSOR_SPECIAL_CHARS:
        if special_char in lower_s1:
            if special_char not in lower_s2:
    ...
    for i in range(lenstr1):
        for j in range(lenstr2):
            if str1[i] == str2[j]:
                ...
                cost = 1
            dict[(i, j)] = min(dict[(i - 1, j)] + 1, dict[(i, j - 1)] + 1, dict[(i - 1, j - 1)] + cost)
            if i and j and str1[i] == str2[(j - 1)] and str1[(i - 1)] == str2[j]:
                dict[(i, j)] = min(dict[(i, j)], dict[(i - 2, j - 2)] + cost)
