...
# [GCC 8.4.0]
# Embedded file name: /home/gerardo/Projects/nlp_model_gen/nlp_model_gen/packages/logger/Logger.py
...
from .assets.logColors import CLASS_COLOR, ERROR_COLOR, INFO_COLOR, METHOD_COLOR, SUCCESS_COLOR, WARNING_COLOR
from .assets.logTexts import LOG_DATA, TYPE_INFO, TYPE_ERR, TYPE_WRN, TYPE_SUCCESS

class EventLogger(metaclass=Singleton):

    def __init__(self):
        self._EventLogger__lock = Lock()
    ...
    def write_log(self, log_entry):
        ...
        """
        self._EventLogger__lock.acquire()
        try:
            try:
                if not DEBUG_MODE:
            ...
            self._EventLogger__lock.release()
    ...
    def log(self, code, inserted_texts=None):
        ...
        """
        if code not in LOG_DATA.keys():
            return
        if LOG_DATA[code]['type'] not in LOG_LEVEL:
            return
