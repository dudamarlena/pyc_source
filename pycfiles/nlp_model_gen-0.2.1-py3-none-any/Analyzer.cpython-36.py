...
# [GCC 8.4.0]
# Embedded file name: /home/gerardo/Projects/nlp_model_gen/nlp_model_gen/packages/modelManager/analyzer/Analyzer.py
...
class Analyzer:
    ...
    def __init__(self, rule_set, exceptions_set):
        self._Analyzer__rule_set = rule_set
        self._Analyzer__exceptions_set = exceptions_set
    ...
    def validate_exception(self, token):
        ...
        """
        for analyzer_exception in self._Analyzer__exceptions_set:
    ...
    def analyze_token(self, token):
        ...
        """
        if self._Analyzer__rule_set is None:
            return
        for category in self._Analyzer__rule_set:
            if token.get_base_form() in category['lemma_list']:
                if self.validate_exception(token):
