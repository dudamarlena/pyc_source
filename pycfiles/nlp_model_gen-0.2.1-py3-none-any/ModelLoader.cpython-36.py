...
# [GCC 8.4.0]
# Embedded file name: /home/gerardo/Projects/nlp_model_gen/nlp_model_gen/packages/modelManager/modelLoader/ModelLoader.py
...
from nlp_model_gen.utils.fileUtils import build_path, check_dir_existence, copy_file, create_dir_if_not_exist, dictionary_to_disk, get_absoulute_path, load_json_file, remove_dir
from nlp_model_gen.base import CURRENT_BASE_PATH
from nlp_model_gen.constants.constants import COMPOUNDING_COMPOUND_DEFAULT, COMPOUNDING_START_DEFAULT, COMPOUNDING_STOP_DEFAULT, DIR_PATH_SEPARATOR, MODEL_CONFIG_FILE_NAME, MODEL_IMPORT_EXT, MODEL_MANAGER_CUSTOM_FILES_DIR, MODEL_MANAGER_DEFAULT_BASE_MODEL, MODEL_MANAGER_ROOT_DIR, MODEL_NER, MODEL_PACKAGING_EXTENSION, MODEL_TMP_JOINT_FILE_NAME, MODEL_TRAINING_DROP_RATE, MODEL_TRAINING_ITER_AMOUNT, TOKEN_RULES_GEN_MODEL_SEED_FILENAME
...
class ModelLoader:
    ...
    def save_model(model, path, tmp_files_path, new_model):
        ...
        dictionary_to_disk(cfg_file_path, {'model_name':new_model.get_model_name(), 
         'description':new_model.get_description(), 
         'author':new_model.get_author(), 
         'analyzer_rule_set':new_model.get_analyser_rules_set()}, True)
        Logger.log('L-0032')
    ...
    def apply_training_loop(model_ref, training_data):
        ...
        for iter_number in range(1, MODEL_TRAINING_ITER_AMOUNT):
            ...
            for batch in batches:
                ...
                model_ref.update(texts,
                  annotations,
                  drop=MODEL_TRAINING_DROP_RATE,
                  losses=losses)
