...
# [GCC 8.4.0]
# Embedded file name: /home/gerardo/Projects/nlp_model_gen/nlp_model_gen/packages/adminModule/AdminModuleController.py
...
from nlp_model_gen.constants.constants import TRAIN_EXAMPLE_STATUS_APPROVED, TRAIN_EXAMPLE_STATUS_HISTORIC, TRAIN_EXAMPLE_STATUS_SUBMITTED, WORD_PROCESSOR_MODULE_KEY_CONJUGATOR, WORD_PROCESSOR_MODULE_KEY_FUZZY_GEN, WORD_PROCESSOR_MODULE_KEY_NOUN_CONV
...
class AdminModuleController(metaclass=Singleton):
    ...
    def __init__(self):
        self._AdminModuleController__analyzer_rules_generator = None
        self._AdminModuleController__model_manager = None
        self._AdminModuleController__tokenizer_rules_generator = None
        self._AdminModuleController__train_manager = None
        self._AdminModuleController__word_processor = None
        self._AdminModuleController__init_success = False
        self._AdminModuleController__initialize()

    def is_ready(self):
        return self._AdminModuleController__init_success
    ...
    def __initialize(self, is_retry=False):
        ...
        Logger.log('L-0036')
        self._AdminModuleController__init_success = False
        self._AdminModuleController__word_processor = WordProcessorController()
        if is_retry:
            self._AdminModuleController__word_processor.retry_initialization()
        self._AdminModuleController__tokenizer_rules_generator = TokenizerRulesGenerator()
        self._AdminModuleController__model_manager = ModelManagerController()
        self._AdminModuleController__train_manager = ModelTrainingController()
        self._AdminModuleController__analyzer_rules_generator = AnalyzerRulesGerator()
        self._AdminModuleController__init_success = True
        Logger.log('L-0037')
    ...
    def retry_initialization(self):
        ...
        """
        self._AdminModuleController__initialize(True)
    ...
    def generate_model(self, model_id, model_name, description, author, tokenizer_exceptions, max_dist):
        ...
        """
        Logger.log('L-0001')
        if self._AdminModuleController__model_manager.get_model(model_id):
            ErrorHandler.raise_error('E-0025')
        if not validate_model_seed(tokenizer_exceptions):
            ErrorHandler.raise_error('E-0026')
        tokenizer_exceptions_path = self._AdminModuleController__tokenizer_rules_generator.generate_model_data(tokenizer_exceptions, model_id, max_dist)
        analyzer_rule_set = self._AdminModuleController__analyzer_rules_generator.create_analyzer_rule_set(tokenizer_exceptions)
        self._AdminModuleController__model_manager.create_model(model_id, model_name, description, author, tokenizer_exceptions_path, analyzer_rule_set)
        Logger.log('L-0034')
    ...
    def get_available_models(self):
        ...
        """
        if not self._AdminModuleController__init_success:
            Logger.log('L-0095')
            return
        else:
            return self._AdminModuleController__model_manager.get_available_models_dict()
    ...
    def load_model(self, model_id):
        ...
        if not self._AdminModuleController__init_success:
            ...
            return self._AdminModuleController__model_manager.load_model(model_id)
    ...
    def edit_model_data(self, model_id, new_model_name=None, new_description=None):
        ...
        """
        Logger.log('L-0074')
        current_model = self._AdminModuleController__model_manager.get_model(model_id)
        ...
        if edited_model_name == current_model_name:
            if edited_description == current_description:
                ErrorHandler.raise_error('E-0075')
        self._AdminModuleController__model_manager.edit_model(model_id, edited_model_name, edited_description)
    ...
    def delete_model_data(self, model_id):
        ...
        :model_id: [String] - Id del modelo.
        """
        self._AdminModuleController__model_manager.remove_model(model_id)
    ...
    def get_word_processor_available_configs(self, module_key):
        ...
        if module_key == WORD_PROCESSOR_MODULE_KEY_CONJUGATOR:
            return self._AdminModuleController__word_processor.get_available_conjugator_configs()
        else:
            if module_key == WORD_PROCESSOR_MODULE_KEY_FUZZY_GEN:
                return self._AdminModuleController__word_processor.get_available_fuzzy_gen_configs()
            if module_key == WORD_PROCESSOR_MODULE_KEY_NOUN_CONV:
                return self._AdminModuleController__word_processor.get_available_conversor_configs()
            return []
    ...
    def add_word_processor_config_theme(self, module_key, theme_name, configs, irregular_groups=None):
        ...
        """
        if module_key == WORD_PROCESSOR_MODULE_KEY_CONJUGATOR:
            self._AdminModuleController__word_processor.add_conjugator_config(theme_name, configs, irregular_groups)
            return
        else:
            if module_key == WORD_PROCESSOR_MODULE_KEY_FUZZY_GEN:
                self._AdminModuleController__word_processor.add_fuzzy_gen_config(theme_name, configs)
                return
            if module_key == WORD_PROCESSOR_MODULE_KEY_NOUN_CONV:
                self._AdminModuleController__word_processor.add_noun_conversor_config(theme_name, configs)
                return
    ...
    def add_theme_conjugator_exceptions(self, theme_name, exceptions):
        ...
        """
        self._AdminModuleController__word_processor.add_conjugator_exceptions(theme_name, exceptions)
    ...
    def get_word_processor_active_themes(self):
        ...
        """
        return {'conjugator':self._AdminModuleController__word_processor.get_conjugator_active_theme(), 
         'fuzzy_generator':self._AdminModuleController__word_processor.get_fuzzy_gen_active_theme(), 
         'noun_conversor':self._AdminModuleController__word_processor.get_noun_converseor_active_theme()}
    ...
    def set_word_processor_active_theme(self, module_key, theme_name):
        ...
        """
        if module_key == WORD_PROCESSOR_MODULE_KEY_CONJUGATOR:
            self._AdminModuleController__word_processor.set_conjugator_active_theme(theme_name)
            return
        else:
            if module_key == WORD_PROCESSOR_MODULE_KEY_FUZZY_GEN:
                self._AdminModuleController__word_processor.set_fuzzy_generator_active_theme(theme_name)
                return
            if module_key == WORD_PROCESSOR_MODULE_KEY_NOUN_CONV:
                self._AdminModuleController__word_processor.set_noun_conversor_active_theme(theme_name)
                return
    ...
    def update_word_processor_config_theme(self, module_key, theme_name, config_mod, irregular_groups_mod=None):
        ...
        """
        if module_key == WORD_PROCESSOR_MODULE_KEY_CONJUGATOR:
            self._AdminModuleController__word_processor.update_conjugator_configs(theme_name, config_mod, irregular_groups_mod)
            return
        else:
            if module_key == WORD_PROCESSOR_MODULE_KEY_FUZZY_GEN:
                self._AdminModuleController__word_processor.update_fuzzy_gen_config(theme_name, config_mod)
                return
            if module_key == WORD_PROCESSOR_MODULE_KEY_NOUN_CONV:
                self._AdminModuleController__word_processor.update_noun_conversor_config(theme_name, config_mod)
                return
    ...
    def update_theme_conjugator_exceptions(self, theme_name, exception_key, exception_data):
        ...
        """
        self._AdminModuleController__word_processor.update_conjugator_exception(theme_name, exception_key, exception_data)
    ...
    def delete_word_processor_theme(self, module_key, theme_name):
        ...
        """
        if module_key == WORD_PROCESSOR_MODULE_KEY_CONJUGATOR:
            self._AdminModuleController__word_processor.remove_conjugator_theme(theme_name)
            return
        else:
            if module_key == WORD_PROCESSOR_MODULE_KEY_FUZZY_GEN:
                self._AdminModuleController__word_processor.remove_fuzzy_gen_theme(theme_name)
                return
            if module_key == WORD_PROCESSOR_MODULE_KEY_NOUN_CONV:
                self._AdminModuleController__word_processor.remove_noun_conversor_theme(theme_name)
                return
    ...
    def submit_training_examples(self, model_id, training_examples_list):
        ...
        """
        return self._AdminModuleController__train_manager.add_training_examples(model_id, training_examples_list)
    ...
    def get_submitted_training_examples(self, model_id, status):
        ...
        if status == TRAIN_EXAMPLE_STATUS_APPROVED:
            return self._AdminModuleController__train_manager.get_approved_training_examples(model_id)
        else:
            if status == TRAIN_EXAMPLE_STATUS_HISTORIC:
                return self._AdminModuleController__train_manager.get_training_examples_history(model_id)
            if status == TRAIN_EXAMPLE_STATUS_SUBMITTED:
                return self._AdminModuleController__train_manager.get_pending_training_examples(model_id)
    ...
    def approve_training_examples(self, training_examples_list):
        ...
        """
        return self._AdminModuleController__train_manager.approve_traning_examples(training_examples_list)
    ...
    def discard_training_examples(self, training_examples_list):
        ...
        :return: [List(Dict)] - Lista con el resultado para cada ejemplo particular.
        """
        return self._AdminModuleController__train_manager.discard_training_examples(training_examples_list)
    ...
    def apply_approved_examples(self, model_id):
        ...
        :model_id: [String] - Id del modelo.
        """
        self._AdminModuleController__train_manager.apply_training_approved_examples(model_id)
    ...
    def get_available_entities(self):
        ...
        available_entities = self._AdminModuleController__train_manager.get_available_entities()
    ...
    def add_custom_entity(self, name, description):
        ...
        """
        self._AdminModuleController__train_manager.add_custom_entity(name, description)
    ...
    def edit_custom_entity(self, name, description):
        ...
        """
        self._AdminModuleController__train_manager.edit_custom_entity(name, description)
    ...
    def add_analyzer_exception(self, model_id, base_form, token_text, enabled=True):
        ...
        :base_form:\xa0[String] - Forma base del token.
        ...
        True)
        """
        self._AdminModuleController__model_manager.add_analyzer_exception(model_id, base_form, token_text, enabled)
    ...
    def enable_analyzer_exception(self, model_id, base_form, token_text):
        ...
        :token_text: [String] - Forma especifica en la que detectar el token.
        """
        self._AdminModuleController__model_manager.enable_analyzer_exception(model_id, base_form, token_text)
    ...
    def disable_analyzer_exception(self, model_id, base_form, token_text):
        ...
        :token_text: [String] - Forma especifica en la que detectar el token.
        """
        self._AdminModuleController__model_manager.disable_analyzer_exception(model_id, base_form, token_text)
    ...
    def get_analyzer_exceptions(self, model_id):
        ...
        modelo y su detalle.
        """
        return self._AdminModuleController__model_manager.get_analyzer_exceptions(model_id)
    ...
    def import_model(self, model_id, source=None):
        ...
        """
        self._AdminModuleController__model_manager.import_model(model_id, source)
    ...
    def export_model(self, model_id, output_path, split=True):
        ...
        """
        self._AdminModuleController__model_manager.export_model(model_id, output_path, split)