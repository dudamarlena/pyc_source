...
# [GCC 8.4.0]
# Embedded file name: /home/gerardo/Projects/nlp_model_gen/nlp_model_gen/packages/systemController/SystemController.py
# Compiled at: 2019-07-04 19:59:46
# Size of source mod 2**32: 24013 bytes
...
class SystemController:
    ...
    def __init__(self):
        self._SystemController__admin_module = None
        self._SystemController__application_module = None
        self._SystemController__task_manager = None
        self._SystemController__ready = False
        self._SystemController__init()
    ...
    def __init(self):
        ...
        """
        try:
            self._SystemController__admin_module = AdminModuleController()
            self._SystemController__application_module = ApplicationModuleController()
            self._SystemController__task_manager = TaskManager()
            self._SystemController__ready = True
        except:
            self._SystemController__ready = False
    ...
    def __process_incoming_request(self, action, task_check=None):
        ...
        if not self.is_ready():
            return self._SystemController__build_response_object(False, error=(ErrorHandler.get_error('E-0073', [])))
        else:
            if task_check:
                if self._SystemController__task_manager.check_model_creation_tasks([TASK_KEYS_WORD_PROCESSOR]):
                    return self._SystemController__build_response_object(False, error=(ErrorHandler.get_error('E-0031', [])))
            try:
                return action()
            except Exception as e:
                return self._SystemController__build_response_object(False, error=(ErrorHandler.get_error_dict(e)))
    ...
    def is_ready(self):
        return self._SystemController__ready
    ...
    def generate_model(self, model_id, model_name, description, author, tokenizer_exceptions, max_dist):
        ...
        def action():
            task_id = self._SystemController__task_manager.create_model_creation_task(model_id, model_name, description, author, tokenizer_exceptions, max_dist)
            return self._SystemController__build_response_object(True, {'task_id': task_id})

        return self._SystemController__process_incoming_request(action)
    ...
    def delete_word_processor_theme(self, module_key, theme_name):
        ...
        def action():
            self._SystemController__admin_module.delete_word_processor_theme(module_key, theme_name)
            return self._SystemController__build_response_object(True)

        return self._SystemController__process_incoming_request(action, [TASK_KEYS_WORD_PROCESSOR])
    ...
    def update_theme_conjugator_exceptions(self, theme_name, exception_key, exception_data):
        ...
        def action():
            self._SystemController__admin_module.update_theme_conjugator_exceptions(theme_name, exception_key, exception_data)
            return self._SystemController__build_response_object(True)
    ...
    def update_word_processor_config_theme(self, module_key, theme_name, config_mod, irregular_groups_mod=None):
        ...
        def action():
            self._SystemController__admin_module.update_word_processor_config_theme(module_key, theme_name, config_mod, irregular_groups_mod)
            return self._SystemController__build_response_object(True)
    ...
    def set_word_processor_active_theme(self, module_key, theme_name):
        ...
        def action():
            self._SystemController__admin_module.set_word_processor_active_theme(module_key, theme_name)
            return self._SystemController__build_response_object(True)
    ...
    def get_word_processor_active_themes(self):
        ...
        def action():
            active_themes = self._SystemController__admin_module.get_word_processor_active_themes()
            return self._SystemController__build_response_object(True, {'active_themes': active_themes})
    ...
    def add_theme_conjugator_exceptions(self, theme_name, exceptions):
        ...
        def action():
            self._SystemController__admin_module.add_theme_conjugator_exceptions(theme_name, exceptions)
            return self._SystemController__build_response_object(True)
    ...
    def add_word_processor_config_theme(self, module_key, theme_name, configs, irregular_groups=None):
        ...
        def action():
            self._SystemController__admin_module.add_word_processor_config_theme(module_key, theme_name, configs, irregular_groups)
            return self._SystemController__build_response_object(True)
    ...
    def get_word_processor_available_configs(self, module_key):
        ...
        :module_key:\xa0[String] - Clave del submodulo.

        :return: [Dict] - Resultados de la tarea.
        """

        def action():
            available_configs = self._SystemController__admin_module.get_word_processor_available_configs(module_key)
            return self._SystemController__build_response_object(True, {'available_configs': available_configs})
    ...
    def delete_model(self, model_id):
        ...
        def action():
            self._SystemController__admin_module.delete_model_data(model_id)
            return self._SystemController__build_response_object(True)

        return self._SystemController__process_incoming_request(action, [TASK_KEYS_MODEL_UPDATE])
    ...
    def edit_model_data(self, model_id, new_model_name=None, new_description=None):
        ...
        def action():
            self._SystemController__admin_module.edit_model_data(model_id, new_model_name, new_description)
            return self._SystemController__build_response_object(True)
    ...
    def get_available_models(self):
        ...
        def action():
            available_models = self._SystemController__admin_module.get_available_models()
            return self._SystemController__build_response_object(True, available_models)
    ...
    def approve_training_examples(self, training_examples_list):
        ...
        def action():
            results = self._SystemController__admin_module.approve_training_examples(training_examples_list)
            return self._SystemController__build_response_object(True, {'results': results})
    ...
    def discard_training_examples(self, training_examples_list):
        ...
        def action():
            results = self._SystemController__admin_module.discard_training_examples(training_examples_list)
            return self._SystemController__build_response_object(True, {'results': results})
    ...
    def get_submitted_training_examples(self, model_id, status):
        ...
        def action():
            submitted_training_examples = self._SystemController__admin_module.get_submitted_training_examples(model_id, status)
            return self._SystemController__build_response_object(True, {'status':status,  'examples':submitted_training_examples})
    ...
    def submit_training_examples(self, model_id, examples):
        ...
        def action():
            self._SystemController__admin_module.submit_training_examples(model_id, examples)
            return self._SystemController__build_response_object(True)
    ...
    def submit_single_training_example(self, model_id, example):
        ...
        def action():
            self._SystemController__application_module.submit_training_example(model_id, example)
            return self._SystemController__build_response_object(True)
    ...
    def apply_approved_training_examples(self, model_id):
        ...
        def action():
            task_id = self._SystemController__task_manager.create_model_training_task(model_id)
            return self._SystemController__build_response_object(True, {'task_id': task_id})
    ...
    def analyze_text(self, model_id, text, only_positives=False):
        ...
        def action():
            task_id = self._SystemController__task_manager.create_text_analysis_task(model_id, text, only_positives)
            return self._SystemController__build_response_object(True, {'task_id': task_id})
    ...
    def get_task_status(self, task_id):
        ...
        def action():
            task_status = self._SystemController__task_manager.get_task_status(task_id)
            return self._SystemController__build_response_object(True, task_status)
    ...
    def get_available_tagging_entities(self):
        ...
        def action():
            tagging_entities = self._SystemController__application_module.get_available_tagging_entities()
            return self._SystemController__build_response_object(True, {'custom_entities': tagging_entities})
    ...
    def analyze_files(self, model_id, files, only_positives):
        ...
        def action():
            task_id = self._SystemController__task_manager.create_files_analysis_task(model_id, files, only_positives)
            return self._SystemController__build_response_object(True, {'task_id': task_id})
    ...
    def add_custom_entity(self, name, description):
        ...
        def action():
            self._SystemController__admin_module.add_custom_entity(name, description)
            return self._SystemController__build_response_object(True)
    ...
    def edit_custom_entity(self, name, description):
        ...
        def action():
            self._SystemController__admin_module.edit_custom_entity(name, description)
            return self._SystemController__build_response_object(True)
    ...
    def add_analyzer_exception(self, model_id, base_form, token_text, enabled=True):
        ...
        def action():
            self._SystemController__admin_module.add_analyzer_exception(model_id, base_form, token_text, enabled)
            return self._SystemController__build_response_object(True)
    ...
    def enable_analyzer_exception(self, model_id, base_form, token_text):
        ...
        def action():
            self._SystemController__admin_module.enable_analyzer_exception(model_id, base_form, token_text)
            return self._SystemController__build_response_object(True)
    ...
    def disable_analyzer_exception(self, model_id, base_form, token_text):
        ...
        def action():
            self._SystemController__admin_module.disable_analyzer_exception(model_id, base_form, token_text)
            return self._SystemController__build_response_object(True)
    ...
    def get_analyzer_exceptions(self, model_id):
        ...
        def action():
            results = self._SystemController__admin_module.get_analyzer_exceptions(model_id)
            return self._SystemController__build_response_object(True, results)
    ...
    def import_model(self, model_id, source=None):
        ...
        def action():
            self._SystemController__admin_module.import_model(model_id, source)
            return self._SystemController__build_response_object(True)
    ...
    def export_model(self, model_id, output_path, split=True):
        ...
        def action():
            self._SystemController__admin_module.export_model(model_id, output_path, split)
            return self._SystemController__build_response_object(True)
