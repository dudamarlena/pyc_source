...
# [GCC 8.4.0]
# Embedded file name: /home/gerardo/Projects/nlp_model_gen/nlp_model_gen/packages/adminModule/tokenGenerator/TokenGenerator.py
# Compiled at: 2019-06-14 18:07:58
# Size of source mod 2**32: 8589 bytes
from spacy.symbols import ORTH, LEMMA, POS, TAG, SHAPE
from nlp_model_gen.constants.constants import TOKEN_RULES_GEN_NOUN_SING_TAG, TOKEN_RULES_GEN_NOUN_PLUR_TAG, DEFAULT_REPLACE_WILDCARD, TOKEN_RULES_GEN_NOUN, TOKEN_RULES_GEN_VERB, TOKEN_RULES_GEN_VERB_CFG
...
class TokenGenerator:
    _TokenGenerator__word_processor = None

    def __init__(self):
        self._TokenGenerator__word_processor = WordProcessorController()
    ...
    def __get_verb_token_rule(self, verb, base_verb, person, time_key):
        ...
        return {verb: [
                {ORTH: verb, 
                 ...
                 TAG: self._TokenGenerator__get_tag_key(person, time_key), 
                 SHAPE: self._TokenGenerator__get_shape(verb)}]}
    ...
    def __get_noun_token_rule(self, noun, base_noun, tag):
        ...
        return {noun: [
                {ORTH: noun, 
                 ...
                 TAG: tag, 
                 SHAPE: self._TokenGenerator__get_shape(noun)}]}
    ...
    def __create_custom_verb_token_rules(self, verb, base_verb, person, time_key, max_dist):
        ...
        fuzzy_token_set = self._TokenGenerator__word_processor.get_fuzzy_set(verb, max_dist)
        tokenizer_exceptions_set = list([])
        for fuzzy_token in fuzzy_token_set:
            token_exception = self._TokenGenerator__get_verb_token_rule(fuzzy_token, base_verb, person, time_key)
    ...
    def __create_custom_noun_token_rules(self, noun, base_noun, max_dist, is_plural=False):
        ...
        fuzzy_set = self._TokenGenerator__word_processor.get_fuzzy_set(noun, max_dist)
        tag = TOKEN_RULES_GEN_NOUN_SING_TAG if not is_plural else TOKEN_RULES_GEN_NOUN_PLUR_TAG
        noun_rule_set = list([])
        for fuzzy_token in fuzzy_set:
            noun_rule_set.append(self._TokenGenerator__get_noun_token_rule(fuzzy_token, base_noun, tag))
    ...
    def generate_verb_rules_set(self, infinitive, max_dist):
        ...
        """
        conj_dict = self._TokenGenerator__word_processor.conjugate_verb(infinitive)
        verb_rules_dict = dict({})
        for key in conj_dict.keys():
            for index, verb in enumerate(conj_dict[key]):
                if verb != '':
                    verb_rules_dict[verb] = self._TokenGenerator__create_custom_verb_token_rules(verb, infinitive, index, key, max_dist)
    ...
    def generate_noun_rules_set(self, singular, max_dist):
        ...
        noun_rules_dict[singular] = self._TokenGenerator__create_custom_noun_token_rules(singular, singular, max_dist, False)
        plural_form = self._TokenGenerator__word_processor.get_plural(singular)
        if plural_form != '':
            noun_rules_dict[plural_form] = self._TokenGenerator__create_custom_noun_token_rules(plural_form, singular, max_dist, True)
