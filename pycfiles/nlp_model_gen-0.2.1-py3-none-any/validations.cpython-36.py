...
# [GCC 8.4.0]
# Embedded file name: /home/gerardo/Projects/nlp_model_gen/nlp_model_gen/packages/wordProcessor/packageUtils/validations.py
...
conjugator_general_config_schema = Schema({'singular':[
  And(Use(int))], 
 'plural':[
  And(Use(int))], 
 'primera_persona':[
 ...
 'tercera_persona':[
  And(Use(int))], 
 'verb_suffixes':[
  And(str, Use(str.lower), len)], 
 'irregular_suffix_exceptions':[
  And(str, Use(str.lower), len)], 
 'empty_conj_dict':{'inf':And([And(lambda s: s is '')], lambda arr: len(arr) == 1), 
...
conjugator_verb_exceptions_schema = Schema({'key':And(str, Use(str.lower), len), 
 'exceptions':{Optional('inf'): [And(str, Use(str.lower))], 
...
conjugator_verb_groups_schema = Schema({'irregular_verbs_grupo_01_ar':[
  And(str, len)], 
 'irregular_verbs_grupo_01_er':[
  And(str, len)], 
 'irregular_verbs_grupo_01_ir':[
  And(str, len)], 
 'irregular_verbs_grupo_02_ar':[
  And(str, len)], 
 'irregular_verbs_grupo_02_er':[
  And(str, len)], 
 'irregular_verbs_grupo_03_acer':[
  And(str, len)], 
 'irregular_verbs_grupo_03_ecer':[
  And(str, len)], 
 'irregular_verbs_grupo_03_ocer':[
  And(str, len)], 
 'irregular_verbs_grupo_03_ucer':[
  And(str, len)], 
 'irregular_verbs_grupo_04_ducir':[
  And(str, len)], 
 'irregular_verbs_grupo_05_er':[
  And(str, len)], 
 'irregular_verbs_grupo_05_nir':[
  And(str, len)], 
 'irregular_verbs_grupo_05_ullir':[
  And(str, len)], 
 'irregular_verbs_grupo_06_ir':[
  And(str, len)], 
 'irregular_verbs_grupo_07_enir':[
  And(str, len)], 
 'irregular_verbs_grupo_07_eir':[
  And(str, len)], 
 'irregular_verbs_grupo_08_ir':[
  And(str, len)], 
 'irregular_verbs_grupo_09_u':[
  And(str, len)], 
 'irregular_verbs_grupo_10_irir':[
  And(str, len)], 
 'irregular_verbs_grupo_11_uir':[
  And(str, len)], 
 'irregular_verbs_grupo_12_o':[
  And(str, len)], 
 'irregular_verbs_grupo_13_aler':[
  And(str, len)], 
 'irregular_verbs_grupo_13_alir':[
  And(str, len)]})
fuzzy_genetator_config_schema = Schema({'char_confusions':{str: [And(str)]}, 
 'transformations':[
  And(str, lambda s: s in ('rnd_confuse_char', 'rnd_char_del', 'rnd_char_change', 'rnd_duplicate_char'))], 
 'char_confusions_max_length':And(Use(int), lambda n: n > 0)})
noun_conversor_config_schema = Schema({Optional('nacionalidades'): [And(str, len)], 
 'groups': [
...
def validate_config(config_type, config):
    if config_type not in schemas.keys():
        return False
    else:
