...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/xrit/packetmanager.py
# Compiled at: 2017-01-28 00:18:23
import os, struct, datetime
NOAA_PRODUCT_ID = {1: {'name': 'NOAA Text', 
       'sub': {0: 'None'}}, 
   4: {'name': 'Other Satellite', 
       'sub': {0: 'None', 
               1: 'Infrared Full Disk', 
               3: 'Visible Full Disk'}}, 
   3: {'name': 'Other Satellite', 
       'sub': {1: 'Infrared Full Disk', 
               3: 'Visible Full Disk'}}, 
   6: {'name': 'Weather Data', 
       'sub': {0: 'None'}}, 
   8: {'name': 'DCS', 
       'sub': {0: 'None'}}, 
   13: {'name': 'Scanner Image', 
        'sub': {1: 'Infrared Full Disk', 
...
FILE_TYPE_CODE_NAME = {0: 'Image', 
   ...
   130: 'DCS', 
   214: 'EMWIN'}
COMPRESSION_TYPE_NAME = {0: 'Not Compressed', 
   1: 'LRIT Rice', 
   2: 'JPEG', 
   5: 'GIF'}
...
def readHeader(f):
  ...
  """
    ...
    if type == 0:
        filetypecode, headerlength, datalength = struct.unpack('>BIQ', data)
        return (
         type, filetypecode, headerlength, datalength)
    else:
        if type == 1:
            bitsperpixel, columns, lines, compression = struct.unpack('>BHHB', data)
            return (
             type, bitsperpixel, columns, lines, compression)
        if type == 2:
            projname, cfac, lfac, coff, loff = struct.unpack('>32sIIII', data)
            return (
             type, projname, cfac, lfac, coff, loff)
        if type == 3:
            return (type, data)
        if type == 4:
            return (type, data)
        ...
        if type == 128:
            imageid, sequence, startcol, startline, maxseg, maxcol, maxrow = struct.unpack('>7H', data)
            return (
             type, imageid, sequence, startcol, startline, maxseg, maxcol, maxrow)
        if type == 129:
            signature, productId, productSubId, parameter, compression = struct.unpack('>4sHHHB', data)
            return (
             type, signature, productId, productSubId, parameter, compression)
        if type == 130:
            return (type, data)
        if type == 131:
            flags, pixel, line = struct.unpack('>HBB', data)
            return (
             type, flags, pixel, line)
        if type == 132:
            return (type, data)
        return type
