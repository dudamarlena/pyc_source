...
# [GCC 8.4.0]
# Embedded file name: /opt/brew/lib/python2.7/site-packages/sentry_notify_github_issues/plugin.py
# Compiled at: 2016-06-27 03:57:46
...
import urllib2, sentry_notify_github_issues

class NotifyGitHubIssuesForm(forms.Form):
    repo = forms.CharField(label=_('Repository Name'), widget=forms.TextInput(attrs={'placeholder': 'e.g. getsentry/sentry'}), help_text=_('Enter your repository name, including the owner.'))
    access_token = forms.CharField(label=_('access token'), widget=forms.TextInput(attrs={'placeholder': 'e.g. xxxxxxxxxxxxxxxxxxxxxxxx'}), help_text=_('Enter your personal access token, create here https://github.com/settings/tokens/new'))
    api_endpoint = forms.CharField(label=_('API endpoint'), widget=forms.TextInput(attrs={'placeholder': 'e.g. https://github.example.com/api/v3/'}), help_text=_('if Github Enterprise, Enter api endpoint. '), required=False)
    label = forms.CharField(label=_('label'), widget=forms.TextInput(attrs={'placeholder': 'e.g. sentry'}), help_text=_('Enter label text.'), required=False)
...
class NotifyGitHubIssuesPlugin(NotificationPlugin):
    ...
    def create_sentry_issue(self, group, title, location, label):
        issue_information = {'title': title, 
           'provider': self.get_title(), 
           'location': location, 
           'label': label}
        Activity.objects.create(project=group.project, group=group, type=Activity.CREATE_ISSUE, data=issue_information)