...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/deebotozmo/cli.py
# Compiled at: 2020-04-03 15:32:49
# Size of source mod 2**32: 12517 bytes
import configparser, itertools, os, platform, random, re, base64, lzma, json, click
...
@click.option('--continent-code', prompt='your two-letter continent code', default=(lambda : continent_for_country(click.get_current_context().params['country_code'])))
...
def run(actions, debug):
    ...
    if actions:
        config = read_config()
        api = EcoVacsAPI((config['device_id']), (config['email']), (config['password_hash']), (config['country']),
          (config['continent']), verify_ssl=(config['verify_ssl']))
