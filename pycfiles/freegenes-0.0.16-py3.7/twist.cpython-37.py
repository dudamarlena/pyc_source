...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/freegenes/main/twist.py
...
class Client(object):
    ...
    def __init__(self, email=None, token=None, eutoken=None, base='https://twist-api.twistbioscience-staging.com/', version='v1'):
    ...
    def _set_tokens(self, token, eutoken):
        ...
        if self.token:
            username = self.eutoken or os.environ.get('FREEGENES_TWIST_LOGIN')
            password = os.environ.get('FREEGENES_TWIST_PASSWORD')
            username and password or bot.exit('You must export FREEGENES_TWIST_TOKEN or FREEGENES_TWIST_LOGIN and FREEGENES_TWIST_PASSWORD')
        else:
    ...
    def _test_token(self):
        ...
        """
        if requests.head(('%s' % self.base), headers=(self.headers)).status_code not in (200,
                                                                                         302):
    ...
    def order_platemaps_by_barcode(self, sfdc_id, barcode, email=None, return_download=False):
        ...
        if 'platemaps_file_url' in result:
            if not return_download:
    ...
    def order_platemaps(self, sfdc_id, email=None):
        ...
        rows = []
        for shipment in items['shipments']:
            for container in shipment['containers']:
                new_rows = self.order_platemaps_by_barcode(sfdc_id=sfdc_id,
