...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/mongua/__init__.py
# Compiled at: 2017-06-19 14:08:00
# Size of source mod 2**32: 12435 bytes
"""
    mongua
    ~~~~~~~~~~~~~~~~~~~
...
from .errors import MonguaLocked, MonguaNotFound, MonguaKeyFrozen, MonguaKeyUndefined, MonguaFieldUnmovable
env = os.environ.get
...
class MongoMixin(object):
    ...
    @classmethod
    def _fields(cls):
        fs = cls._MongoMixin__fixed_fields() + cls.__fields__
        if cls._valid_fields(fs):
            return fs

    @classmethod
    def _frozen_keys(cls):
        ks = cls.__frozen_keys__ + cls._MongoMixin__immutable_keys()
        ks = list(set(ks))
        return ks
    ...
    def json(self):
        ...
        """
        td = self.to_dict()
        d = {k:v for k, v in td.items() if not k.startswith('_') if not k.startswith('_')}
    ...
    def paging(cls, skip=0, limit=20, **kwargs):
        ...
        :return:
        """
        sort_key = kwargs.pop('__sort_key', 'id')
