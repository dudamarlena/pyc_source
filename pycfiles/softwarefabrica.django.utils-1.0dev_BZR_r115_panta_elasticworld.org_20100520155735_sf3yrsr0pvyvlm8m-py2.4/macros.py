...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/softwarefabrica/django/utils/templatetags/macros.py
# Compiled at: 2009-03-04 04:23:40
"""
...
def do_usemacro(parser, token):
    ...
    if len(values) != len(macro.args):
        raise template.TemplateSyntaxError, "Macro '%s' was declared with %d parameters and used with %d parameter" % (macro_name, len(macro.args), len(values))
    filter_expressions = []
    for val in values:
