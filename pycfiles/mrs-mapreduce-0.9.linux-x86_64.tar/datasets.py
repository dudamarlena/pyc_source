...
# [GCC 8.4.0]
# Embedded file name: /home/amcnabb/python/mrs/datasets.py
# Compiled at: 2012-11-13 17:39:02
...
from .serializers import dumps_functions, loads_functions, raw_serializer, Serializers
from . import util
...
class BaseDataset(object):
    ...
    def __init__(self, splits=0, dir=None, format=None, permanent=True, serializers=None):
        self.splits = splits
        self.dir = dir
        self.format = format
    ...
    def _assert_open(self, _called_in_runner):
        ...
        """
        assert _called_in_runner or not self.closed, 'Invalid operation on a closed dataset.'
    ...
    def __getitem__(self, key_pair):
        ...
        if not source_key_is_slice and not split_key_is_slice:
            ...
            return bucket
        if source_key_is_slice and (source_key.start != None or source_key.stop != None or source_key.step != None) or split_key_is_slice and (split_key.start != None or split_key.stop != None or split_key.step != None):
...
class LocalData(BaseDataset):
    ...
    def __init__(self, itr, splits=None, source=0, parter=None, write_only=False, **kwds):
        if parter is not None and splits is None:
            raise RuntimeError('The splits parameter is required when parter is specified.')
    ...
    def _make_bucket(self, source, split):
        ...
        return bucket.WriteBucket(source, split, self.dir, self.format, serializers=self.serializers)
...
class RemoteData(BaseDataset):
    ...
    def fetchall(self, _called_in_runner=False):
        ...
        if self._fetched:
            return
        self._assert_open(_called_in_runner)
        assert self._urls_known, 'Invalid fetchall on a dataset with unknown urls.'
        kwds = {}
        if self.serializers:
...
class MergeSortData(BaseDataset):
    ...
    def __init__(self, input, input_split, max_sort_size, splits=None, source=None, parter=None, _called_in_runner=False, **kwds):
    ...
    def _collect(self, input, input_split, max_sort_size, _called_in_runner):
        ...
        raw_serializers = Serializers(raw_serializer, 'raw_serializer', raw_serializer, 'raw_serializer')
        max_ram_bytes = 1048576 * max_sort_size
        current_bytes = 0
        total_bytes = 0
        data_list = []
        for raw_key, raw_value in input.stream_split(input_split, serializers=raw_serializers, _called_in_runner=_called_in_runner):
        ...
        if self._data:
            ...
            data_itr = self._iter_deserialized(data_list, loads_key, loads_value)
            b.collect(data_itr)
    ...
    def _flush_data(self, data_list, serializers, input_serializers):
        if not data_list:
            return
        b = bucket.WriteBucket(len(self._data), self.fixed_split, self.dir, serializers=serializers)
...
class FileData(RemoteData):
    r"""A list of static files or urls to be used as input to an operation.
    ...
    def __init__(self, urls, sources=None, splits=None, first_source=0, first_split=0, **kwds):
        n = len(urls)
        if splits is None:
            if sources is None:
                splits = n
