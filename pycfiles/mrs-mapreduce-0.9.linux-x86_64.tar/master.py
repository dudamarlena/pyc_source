...
# [GCC 8.4.0]
# Embedded file name: /home/amcnabb/python/mrs/master.py
# Compiled at: 2012-11-13 16:10:00
"""Mrs Master"""
from __future__ import division, print_function
import collections, os, socket, sys, threading, time
from . import http
...
class MasterRunner(runner.TaskRunner):
    ...
    def run(self):
        ...
        self.slaves = Slaves(sched_write_pipe, self.chore_queue, self.opts.mrs__timeout, self.opts.mrs__pingdelay)
        try:
    ...
    def start_rpc_server(self):
        ...
        self.rpc_interface = MasterInterface(self.slaves, program_hash, self.opts, self.args, self.jobdir)
        ...
        self.rpc_thread = threading.Thread(target=rpc_server.serve_forever, name='RPC Server')
    ...
    def remove_sources(self, dataset_id, slave_source_list, delete):
        ...
        removed from disk.
        """
        items = [ (slave.remove, (dataset_id, source, delete)) for slave, source in slave_source_list
                ]
        self.chore_queue.do_many(items)

    def sched_timing_stats(self):
        if self.opts.mrs__timing_interval > 0:
            self.chore_queue.do(self.do_timing_stats, delay=self.opts.mrs__timing_interval)
    ...
    def debug_status(self):
        ...
        print('Idle slaves:', (', ').join(str(slave.id) for slave in self.idle_slaves._all_slaves), file=sys.stderr)
        print('Dead slaves:', (', ').join(str(slave.id) for slave in self.dead_slaves), file=sys.stderr)
...
class MasterInterface(object):
    ...
    @http.uses_host
    def xmlrpc_signin(self, version, cookie, slave_port, program_hash, host=None):
        ...
        if version != __version__:
            ...
            return (
             slave.id, host, self.jobdir, self.opts_dict, self.args)
    ...
    @http.uses_host
    def xmlrpc_done(self, slave_id, dataset_id, source, urls, cookie, host=None):
    ...
    @http.uses_host
    def xmlrpc_failed(self, slave_id, dataset_id, task_index, cookie, host=None):
...
class RemoteSlave(object):
    ...
    def send_assignment(self):
        with self._rpc_lock:
            if not self.alive():
                logger.warning('Canceling RPC call because slave %s is no longer alive.' % self.id)
                return
    ...
    def remove(self, dataset_id, source, delete):
        with self._rpc_lock:
            ...
            try:
                ...
                logger.error('Connection failed in remove call to slave %s' % self.id)
                success = False
    ...
    def update_timestamp(self):
        ...
        if self._state == 'failed':
            logger.warning('Updating timestamp of the failed slave %s.' % self.id)
            self.resurrect()
    ...
    def slave_ready(self, slave):
        if not slave.alive():
            if not slave.resurrect():
                return
        if slave.busy():
            logger.error('Slave %s reported ready but has an assignment; check the slave logs for errors.' % slave.id)
