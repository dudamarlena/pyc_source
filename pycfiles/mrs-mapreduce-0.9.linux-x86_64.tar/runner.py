...
# [GCC 8.4.0]
# Embedded file name: /home/amcnabb/python/mrs/runner.py
# Compiled at: 2012-11-13 17:31:05
"""Mrs Runner"""
...
class BaseRunner(object):
    ...
    def __init__(self, program_class, opts, args, job_conn, jobdir, default_dir, worker_pipe):
    ...
    def read_job_conn(self):
        ...
        if isinstance(message, job.DatasetSubmission):
            ...
            if ds.serializers is not None:
                ds.serializers = serializers.from_names(ds.serializers, self.program_class)
...
class TaskRunner(BaseRunner):
    ...
    def __init__(self, *args):
        ...
        self.event_loop.register_fd(self.chore_queue_pipe, self.read_chore_queue_pipe)
    ...
    def task_done(self, dataset_id, task_index, outurls, backlinked=False):
        ...
        if tasklist.time_to_report_progress():
            response = job.ProgressUpdate(dataset_id, tasklist.fraction_complete())
            self.job_conn.send(response)
            ...
            if forward_link_ids:
                for forward_ds_id in forward_link_ids.copy():
                    if forward_ds_id not in self.tasklists:
                        continue
                    self.task_done(forward_ds_id, task_index, outurls, backlinked=True)
    ...
    def task_lost(self, dataset_id, task_index):
        ...
        if tasklist is not None:
            ...
            if failure_count > self.opts.mrs__max_failures:
                msg = 'Task failed too many times (see slave logs for details): (%s, %s)' % (
    ...
    def _wakeup_dependents(self, dataset_id):
        ...
        for dependent_id in self.data_dependents[dataset_id]:
            ...
            if dep_ds in self.pending_datasets and (fraction_complete == 1 or dep_ds.async_start):
                wakeup_count += 1
    ...
    def timing_stats(self):
        ...
        print('TIMING: Completed tasks (since last):', num_tasks, file=sys.stderr)
        print('TIMING: Elapsed time (since last):', elapsed_time, file=sys.stderr)

    def debug_status(self):
        super(TaskRunner, self).debug_status()
        self.timing_stats()
        print('Runnable datasets:', (', ').join(ds.id for ds in self.runnable_datasets), file=sys.stderr)
        print('Pending datasets:', (', ').join(ds.id for ds in self.pending_datasets), file=sys.stderr)
