...
# [GCC 8.4.0]
# Embedded file name: /home/amcnabb/python/mrs/worker.py
# Compiled at: 2012-11-07 13:30:14
"""Mrs Worker
...
class WorkerFailure(object):
    ...
    def __init__(self, dataset_id, task_index, exception, traceback, request_id):
    ...
    def run_once(self):
        ...
        try:
            ...
            if isinstance(request, WorkerSetupRequest):
                ...
                if isinstance(request, WorkerQuitRequest):
                    return False
                if isinstance(request, WorkerRemoveRequest):
                    ...
                    t = tasks.Task.from_args(program=self.program, *request.args)
                    t.run(self.program, self.default_dir, max_sort_size=max_sort_size)
                    response = WorkerSuccess(request.dataset_id, request.task_index, t.outdir, t.outurls(), request.id())
            ...
            response = WorkerFailure(request.dataset_id, request.task_index, e, tb, request_id)
...
class WorkerManager(object):
    ...
    def read_worker_pipe(self):
        ...
        assert isinstance(r, WorkerSuccess) or isinstance(r, WorkerFailure) or False, 'Unexpected response type'
