...
# [GCC 8.4.0]
# Embedded file name: /home/amcnabb/python/mrs/job.py
# Compiled at: 2012-11-05 18:28:17
from __future__ import division, print_function
import multiprocessing, os, select, threading, time, traceback, weakref
from . import bucket
...
class Job(object):
    ...
    def __init__(self, manager, program, opts, default_dir=None, url_converter=None):
    ...
    def local_data(self, itr, splits=None, outdir=None, parter=None, **kwds):
        ...
        ds = datasets.LocalData(itr, splits, dir=outdir, parter=parter, permanent=permanent, **kwds)
    ...
    def map_data(self, input, mapper, splits=None, outdir=None, combiner=None, parter=None, **kwds):
        ...
        ds = computed_data.ComputedData(op, input, splits=splits, dir=outdir, permanent=permanent, **kwds)
        self._manager.submit(ds)
        ds._close_callback = self._manager.close_dataset
        return ds

    def reduce_data(self, input, reducer, splits=None, outdir=None, parter=None, **kwds):
    ...
    def reducemap_data(self, input, reducer, mapper, splits=None, outdir=None, combiner=None, parter=None, **kwds):
        ...
        if combiner is not None:
            ...
            combine_name = ''
        part_name, _ = self._named_attr(parter)
        op = tasks.ReduceMapOperation(reduce_name, map_name, combine_name, part_name)
        ds = computed_data.ComputedData(op, input, splits=splits, dir=outdir, permanent=permanent, **kwds)
        self._manager.submit(ds)
        ds._close_callback = self._manager.close_dataset
        return ds
    ...
    def _named_attr(self, value):
        if isinstance(value, str):
            ...
            return (
             self._registry[value], value)


def job_process(program_class, opts, args, default_dir, pipe, quit_pipe, use_bucket_server):
    ...
    if use_bucket_server:
        ...
        bucket_proc = multiprocessing.Process(target=bucket_server.serve_forever, name='Bucket Server')
    ...
    user_thread = threading.Thread(target=run_user_thread, args=(
     program_class, opts, args, default_dir, manager,
     url_converter), name='User Thread')
...
def run_user_thread(program_class, opts, args, default_dir, manager, url_converter):
    try:
    ...
    try:
        if opts.mrs__profile:
            exitcode = util.profile_call(program.run, (job,), {}, 'mrs-run-user.prof')
        else:
            exitcode = program.run(job)
    except Exception as e:
        exitcode = 1
        logger.critical('Exception raised in the run function: %s' % traceback.format_exc())
...
class DataManager(object):
    ...
    def handle_message(self, message):
        if isinstance(message, BucketReady):
            ...
            if isinstance(message, QuitJobProcess):
                return
