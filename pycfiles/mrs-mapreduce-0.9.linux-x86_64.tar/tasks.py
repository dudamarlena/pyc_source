...
# [GCC 8.4.0]
# Embedded file name: /home/amcnabb/python/mrs/tasks.py
# Compiled at: 2012-11-07 19:35:13
"""MapReduce Tasks.
...
class Task(object):
    ...
    def __init__(self, op, input_ds, dataset_id, task_index, splits, storage, ext, serializers):
        self.op = op
    ...
    @staticmethod
    def from_args(op_args, urls, dataset_id, task_index, splits, storage, ext, input_ser_names, ser_names, program):
        ...
        input_ds = datasets.FileData(urls, program, splits=1, first_split=task_index, serializers=input_serializers)
        return Task.from_op(op, input_ds, dataset_id, task_index, splits, storage, ext, output_serializers)
    ...
    def to_args(self):
        ...
        if input_serializers:
            input_ser_names = (
             input_serializers.key_s_name,
             input_serializers.value_s_name)
        else:
            input_ser_names = ''
        if self.serializers:
            ser_names = (
             self.serializers.key_s_name,
    ...
    def _get_all_input(self, serial, sort=False, default_dir=None, max_sort_size=None):
        ...
        if serial:
            ...
            sorted_ds = datasets.MergeSortData(self.input_ds, self.task_index, max_sort_size, dir=tmpdir, _called_in_runner=True)
            data = sorted_ds.stream_data(_called_in_runner=True)
            self.sorted_ds = sorted_ds
        else:
            data = self.input_ds.stream_split(self.task_index, _called_in_runner=True)
        return data

    def _outdata_kwds(self, program, permanent, serial):
        """Returns arguments for the output dataset (common to all task types).
        """
        kwds = {'source': self.task_index, 'parter': self.op.parter(program), 
           'dir': self.outdir, 
           'format': self.format(), 
...
class ReduceTask(Task):
    ...
    def run(self, program, default_dir, serial=False, max_sort_size=None):
        ...
        all_input = self._get_all_input(serial, sort=True, default_dir=default_dir, max_sort_size=max_sort_size)
        permanent = self.make_outdir(default_dir)
        kwds = self._outdata_kwds(program, permanent, serial)
        reduce_itr = self.op.reduce(program, all_input)
        self.output = datasets.LocalData(reduce_itr, permanent=permanent, **kwds)
...
class MapOperation(Operation):
    ...
    def to_args(self):
        return (
         self.op_name, self.map_name, self.combine_name,
         self.part_name)
...
class ReduceOperation(Operation):
    ...
    def reduce(self, program, input):
        ...
        grouped_input = ((k, (pair[1] for pair in v)) for k, v in itertools.groupby(input, key=itemgetter(0)))
    ...
    def to_args(self):
        return (
         self.op_name, self.reduce_name, self.part_name)
...
class ReduceMapOperation(MapOperation, ReduceOperation):
    ...
    def to_args(self):
        return (
         self.op_name, self.reduce_name, self.map_name,
