...
# [GCC 8.4.0]
# Embedded file name: /home/amcnabb/python/mrs/main.py
# Compiled at: 2012-11-07 14:53:00
...
import binascii, logging, multiprocessing, os, random, signal, sys, threading, time, traceback, warnings
from . import master
from . import param
...
def main(program_class, update_parser=None, args=None):
    ...
    if update_parser:
        warnings.warn('The update_parser argument is deprecated.', DeprecationWarning)
...
def option_parser():
    ...
    parser.usage = USAGE
    parser.add_option('-I', '--mrs', dest='mrs', metavar='IMPLEMENTATION', action='extend', search=['mrs.main'], default='Serial', help='Mrs Implementation (Serial, Master, Slave, Bypass, etc.)')
    return parser
...
class BaseImplementation(ParamObj):
    ...
    """
    _params = dict(verbose=Param(type='bool', doc='Verbose mode (set log level to INFO)'), debug=Param(type='bool', doc='Debug mode (set log level to DEBUG)'), profile=Param(type='bool', doc='Run the python profiler'), seed=Param(default=str(DEFAULT_SEED), doc='Random seed, default changes each run'), timing_file=Param(doc='Name of a file to write timing data to'))
    ...
    def make_job_process(self, opts, args, jobdir=None):
        ...
        job_proc = multiprocessing.Process(target=job.job_process, name='Job Process', args=(
         self.program_class, opts, args, jobdir, child_job_conn,
...
class Implementation(BaseImplementation):
    ...
    def _main(self, opts, args):
        ...
        if self.shared:
            ...
            jobdir = None
            default_dir = None
        job_proc, job_conn, job_quit_pipe = self.make_job_process(opts, args, default_dir)
        try:
            try:
                job_proc.start()
                ...
                self.runner = self.runner_class(self.program_class, opts, args, job_conn, jobdir, default_dir, self.worker_pipe)
                if opts.mrs__profile:
                    exitcode = util.profile_call(self.runner.run, (), {}, 'mrs-runner.prof')
                else:
...
class NetworkParams(ParamObj):
    _params = dict(port=Param(default=0, type='int', shortopt='-P', doc='RPC Port for incoming requests'), timeout=Param(default=120, type='float', doc='Timeout for RPC calls (incl. pings)'), pingdelay=Param(default=120, type='float', doc='Interval between pings'))


class Master(Implementation, FileParams, NetworkParams, TaskRunnerParams):
    _params = dict(runfile=Param(default='', doc="Server's RPC port will be written here"))
...
class Slave(BaseImplementation, FileParams, NetworkParams):
    _params = dict(master=Param(shortopt='-M', doc='URL of the Master RPC server'))
    ...
    def _main(self, opts, args):
        ...
        if not self.master:
            ...
            return 1
        self.start_worker_process(opts.mrs__profile)
        s = slave.Slave(self.program_class, self.master, self.tmpdir, self.pingdelay, self.timeout, self.worker_pipe)
        try:
            exitcode = s.run()
        finally:
            self.stop_worker_process()
