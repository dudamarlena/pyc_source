...
# [GCC 8.4.0]
# Embedded file name: /home/amcnabb/python/mrs/slave.py
# Compiled at: 2012-10-25 12:19:24
"""Mrs Slave
...
PING_ATTEMPTS = 50
COOKIE_LEN = 8
import datetime, multiprocessing, optparse, socket, threading
from . import bucket
from . import http
...
class Slave(worker.WorkerManager):
    ...
    def __init__(self, program_class, master_url, tmpdir, pingdelay, timeout, worker_pipe):
        ...
        self.event_loop.register_fd(self.worker_pipe.fileno(), self.read_worker_pipe)
        self.event_loop.register_fd(self.exit_pipe_recv.fileno(), self.read_exit_pipe)
        return
    ...
    def run(self):
        ...
        if not jobdir:
            ...
            self.url_converter = bucket.URLConverter(addr, self.bucket_port, default_dir)
    ...
    def start_rpc_server_thread(self):
        ...
        rpc_thread = threading.Thread(target=rpc_server.serve_forever, name='RPC Server')
    ...
    def start_bucket_server_thread(self, default_dir):
        ...
        bucket_thread = threading.Thread(target=bucket_server.serve_forever, name='Bucket Server')
    ...
    def signin(self):
        ...
        try:
            slave_id, addr, jobdir, optdict, args = self.master_rpc.signin(__version__, cookie, self.rpc_port, program_hash)
        ...
        if slave_id < 0:
            ...
            return (
             slave_id, addr, jobdir, opts, args)
    ...
    def worker_success(self, r):
        ...
        self.master_rpc.done(self.id, r.dataset_id, r.task_index, outurls, self.cookie)

    def worker_failure(self, r):
        """Called when a worker sends a WorkerFailure."""
        try:
            self.master_rpc.failed(self.id, r.dataset_id, r.task_index, self.cookie)
        except socket.error as e:
            msg = e.args[0]
            logger.critical('Failed to report due to network error: %s' % msg)
...
class SlaveInterface(object):
    ...
    @http.uses_host
    def xmlrpc_start_task(self, op_args, urls, dataset_id, task_index, splits, storage, ext, input_ser_names, ser_names, cookie, host=None):
        ...
        request = worker.WorkerTaskRequest(op_args, urls, dataset_id, task_index, splits, storage, ext, input_ser_names, ser_names)
