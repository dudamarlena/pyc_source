...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sorno/consoleutil.py
# Compiled at: 2019-08-09 12:21:44
# Size of source mod 2**32: 13810 bytes
        ...
        if options is None:
            ...
            if style == DataPrinter.PRINT_STYLE_R:
                ...
                for header, t in zip(self.headers, self.header_types):
                    h = {'name':header, 
                     'type':t}
                    json_headers.append(json.dumps(h).replace('\t', '').replace('\n', ''))
