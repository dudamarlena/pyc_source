...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/logger/raven_logger.py
# Compiled at: 2018-04-03 10:33:18
...
class LoggerClient:
            ...
            for i, line in enumerate(f):
                ...
                if current_line < start:
                    ...
                    if current_line > stop:
                        break
                var = re.match('^(?P<name>[a-z]{1}\\w*)=\\S', line, re.IGNORECASE)
                if var:
    ...
    def _get_declares(self, declare_output, local_vars):
        ...
        for var in local_vars:
            m = re.search('^(?P<name>' + re.escape(var) + ')=(?P<value>\\S.*)', declare_output, re.MULTILINE)
            if m:
    ...
    def capture(self, shell_args):
        frame = {'filename':shell_args.script, 
        ...
        if shell_args.pwd:
            ...
            try:
                ...
                frame.update(filename=filename,
                  abs_path=abspath,
        ...
        exc_info = (
         RavenBashException, RavenBashException(exc_text), None)
