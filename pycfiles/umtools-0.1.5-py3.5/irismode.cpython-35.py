...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/umtools/irismode.py
# Compiled at: 2016-01-29 18:16:12
# Size of source mod 2**32: 11358 bytes
import copy
from datetime import datetime
import glob, numpy as np, os, scipy.interpolate as scinter, cf_units, iris
from . import utils

def interp_cubes_to_points(cubelist, cube_name_and_axes, verbose=0, extrapolation_mode='linear'):
    """ """
    ...
    for k, icube in enumerate(cubelist):
        if verbose > 1:
            utils.tic()
        new_cube = iris.analysis.interpolate.linear(icube, pnts, extrapolation_mode=extrapolation_mode)
        if verbose > 1:
...
def gather_cube(cubelists, cubenames, add_coord=None, remove_aux_fac=False, remove_fcst_period=False, remove_aux_z=False, time_var_name='time_0'):
        ...
        for icubelist in cubelists:
            ...
            if remove_fcst_period:
                for i_auxcoord in icube.aux_coords:
                    if i_auxcoord.name() == 'forecast_period':
                        icube.remove_coord(i_auxcoord)

            if remove_aux_z:
                for i_auxcoord in icube.aux_coords:
                    if i_auxcoord.name() in ('surface_altitude', 'level_height', 'atmosphere_hybrid_height_coordinate'):
                        icube.remove_coord(i_auxcoord)
...
def get_model_real_coords(vrbl, dims='tzyx'):
    ...
    pref_coords = dict(x=('x', 'grid_longitude', 'longitude'), y=('y', 'grid_latitude',
                                                                  'latitude'), z=('height',
                                                                                  'level_height',
                                                                                  'pressure',
                                                                                  'atmosphere_hybrid_height_coordinate'), t='time')
    model_coords = []
    for iax in dims:
...
def regrid_model_to_obs(datacontainer, obs_coord, model_coords=None, obs_time_convert=True, rot_ll=True, shift=None, dims='tzyx', return_cube=False):
    if isinstance(obs_coord, tuple):
        ...
        if isinstance(datacontainer, list) or isinstance(datacontainer, tuple):
            ...
            if isinstance(datacontainer, dict):
                ivar = get_cube(datacontainer['dataset'], datacontainer['varname'])
            else:
                if isinstance(datacontainer, iris.cube.Cube):
                    ivar = datacontainer
                else:
                    if isinstance(datacontainer, np.ndarray):
            ...
            if rot_ll:
                ...
                obs_coord_dict['x'], obs_coord_dict['y'] = iris.analysis.cartography.rotate_pole(obs_coord_dict['x'], obs_coord_dict['y'], nplon, nplat)
    ...
    if return_cube:
        ...
        interp_cube.rename('interpolated_' + ivar.name())
...
def unrotate_wind(cubelist, uwind_name='x_wind', vwind_name='y_wind', newcs=iris.coord_systems.GeogCS(iris.fileformats.pp.EARTH_RADIUS), replace=False, verbose=0):
    ...
    if u is not None or v is not None:
        ...
        if get_cube(cubelist, 'transformed_x_wind', lazy=False) is not None and get_cube(cubelist, 'transformed_y_wind', lazy=False) is not None:
