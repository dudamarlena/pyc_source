...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/maxthreads/maxthreads.py
# Compiled at: 2016-04-26 17:13:45
...
class MaxThreads:
    ...
    def add_task(self, target, args=(), kwargs=None, priority=None):
        ...
        new_thread = None
        if (self.threads_active() < self._max_threads or not self._limit) and self._threads_waiting == 0 and self._queue.qsize() > 0:
            new_thread = self._start_loop_thread()
        self._queue.put(SetPrio(target=target, args=args, kwargs=kwargs or {}, priority=priority or 0))
        return new_thread
