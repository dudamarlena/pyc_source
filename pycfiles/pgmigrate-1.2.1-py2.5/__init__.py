...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pgmigrate/__init__.py
# Compiled at: 2008-05-28 08:25:38
import sys, uuid
...
class SqlDbPatch(object):
    ...
    def __repr__(self):
        return 'SqlPatch("%s", "%s", "%s")' % (self.id, self.author, self.comment)
...
class PythonDbPatch(object):
    ...
    def __repr__(self):
        return 'PythonPatch("%s", "%s", "%s")' % (self.id, self.author, self.comment)


PATCH_FORMAT_MAP = {'sql': SqlDbPatch, 
...
class DbPatcher(object):
    ...
    def __init__(self, database, host, port, user, password, verbose, patch_file=None):
        ...
        conn_args = zip(['dbname', 'host', 'port', 'user', 'password'], [database, host, port, user, password])
        conn_string = (' ').join(('%s=%s' % x for x in conn_args if x[1]))
        import psycopg2
    ...
    def test(self):
        ...
        if len(patches) == 0:
            ...
            return 1
        if self.verbose >= 1:
            print ngettext('Testing %d patch', 'Testing %d patches', len(patches)) % len(patches)
        for p in patches:
            self._apply_patch(p)
...
def main():
    ...
    usage = 'usage: %prog [options] init|verify|test|patch|uuid [patchfile.dbp]'
