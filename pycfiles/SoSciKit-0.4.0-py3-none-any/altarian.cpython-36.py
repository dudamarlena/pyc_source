...
# [GCC 8.4.0]
# Embedded file name: C:\Users\scars\Desktop\soscikit_github\soscikit\plotting\altarian.py
# Compiled at: 2019-11-28 01:56:15
...
def altair_monovariate_hist(data, column):
    ...
    histogram = alt.Chart(data).mark_bar().properties(width=400, height=300).encode(x=alt.X((column + ':Q'), bin=True),
      y='count()')
    chart = histogram
    return chart
...
def altair_bivariate_hue(dataset, g_x, g_y, g_hue):
    ...
    points = alt.Chart(dataset).mark_point().encode(x=g_x,
      y=g_y,
      color=g_hue).interactive()
