...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/repoze/dbbrowser/tests.py
# Compiled at: 2010-03-03 04:48:03
import unittest
...
def setup_db():
    ...
    session = DBSession()
    for i in range(21):
        model = MyModel(name='test name %d' % i, value=i * 5)
        session.add(model)
        session.commit()
...
class TestViews(unittest.TestCase):
    ...
    def test_tabledata_view_default(self):
        ...
        self.assertEqual(response['rows'][0]['cell'], (1, 'test name 0', 0))
        self.assertEqual(response['rows'][9]['cell'], (10, 'test name 9', 45))
    ...
    def test_tabledata_view_page(self):
        ...
        self.assertEqual(response['total'], 3)
        self.assertEqual(response['records'], 21)
        self.assertEqual(response['rows'][0]['cell'], (11, 'test name 10', 50))
        self.assertEqual(response['rows'][9]['cell'], (20, 'test name 19', 95))
        self.assertEqual(len(response['rows']), 10)
    ...
    def test_tabledata_view_last(self):
        ...
        self.assertEqual(response['total'], 3)
        self.assertEqual(response['records'], 21)
        self.assertEqual(response['rows'][0]['cell'], (21, 'test name 20', 100))
    ...
    def test_tabledata_view_sort(self):
        ...
        self.assertEqual(response['total'], 3)
        self.assertEqual(response['records'], 21)
        self.assertEqual(response['rows'][0]['cell'], (10, 'test name 9', 45))
        self.assertEqual(response['rows'][9]['cell'], (20, 'test name 19', 95))
        self.assertEqual(len(response['rows']), 10)
    ...
    def test_tabledata_view_search(self):
        ...
        self.assertEqual(response['rows'][0]['cell'], (10, 'test name 9', 45))
        self.assertEqual(response['rows'][1]['cell'], (20, 'test name 19', 95))
    ...
    def test_tableedit_view_add(self):
        ...
        request.params['name'] = 'add test name'
        request.params['value'] = '88'
        ...
        self.assertEqual(new_entry[0].name, 'add test name')
    ...
    def test_tableedit_view_edit(self):
        ...
        request.params['name'] = 'edit test name'
        request.params['value'] = '55'
        request.params['id'] = '2'
        response = tableedit_view(request)
        entries = self.session.query(MyModel).count()
        ...
        self.assertEqual(len(new_entry), 1)
        self.assertEqual(new_entry[0].name, 'edit test name')
