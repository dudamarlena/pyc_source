...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/sahriswiki/macros/wiki.py
# Compiled at: 2010-07-22 07:07:54
"""Wiki Macros
...
def SetTitle(macro, environ, data, *args, **kwargs):
    ...
    **Example(s):**
    ...
    """
    title = kwargs.get('title', args and args[0] or '')
...
def AddComment(macro, environ, data, *args, **kwargs):
    ...
    **Example(s):**
    ...
    if action == 'preview':
        ...
        the_preview += tag.div(parser.generate(comment, context='inline', environ=(
         environ, data)), class_='article')
    if comment and action == 'save':
        ...
        storage.save_text(page_name, new_text, author, 'Comment added by %s' % author)
        search.update_page(environ.get_page(page_name), page_name, text=new_text)
        the_comment = parser.generate(comment_text, context='inline', environ=(
         environ, data))
    the_form = tag.form(tag.input(type='hidden', name='parent', value=page['node']), tag.fieldset(tag.legend('Add Comment'), tag.p(tag.textarea(action not in ('cancel',
                                                                                                                                                               'save') and comment or '', id='comment', name='comment', cols=80, rows=5), class_='text'), tag.h4(tag.label('Your email or username:', for_='author')), tag.p(tag.input(id='author', name='author', type='text'), class_='input'), tag.p(tag.button('Preview', type='submit', name='action', value='preview'), tag.button('Save', type='submit', name='action', value='save'), tag.button('Cancel', type='submit', name='action', value='cancel'), class_='button')), method='post', action='')
...
def source(macro, environ, data, *args, **kwargs):
    ...
    **Example(s):**
    ...
    if not macro.body:
        ...
        contents = environ.parser.generate(macro.body, context='inline', environ=(
         environ, data))
