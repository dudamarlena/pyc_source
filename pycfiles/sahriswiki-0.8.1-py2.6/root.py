...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/sahriswiki/root.py
# Compiled at: 2010-07-24 13:36:59
"""Root Controller
...
class Root(BaseController):
    ...
    def index(self, *args, **kwargs):
        ...
        try:
            ...
            data = {'title': name}
            data['results'] = sorted(self.search.find((name,)), key=itemgetter(0), reverse=True)[:5]
        ...
        if action == 'upload':
            ...
            if file is not None:
                ...
                if author:
                    ...
                    author = self.request.headers.get('X-Forwarded-For', self.request.remote.ip or 'AnonymousUser')
                self.storage.reopen()
    ...
    def login(self):
        ...
        users = dict((user.username, user.password) for user in db.query(schema.User).all())
        ...
        if not check_auth(self.request, self.response, realm, users):
            ...
            if self.request.login not in users:
                return basic_auth(self.request, self.response, realm, users)
            ...
            data = {'title': 'Login', 'html': Markup('Login successful.')}
    ...
    def search(self, *args, **kwargs):
        ...
        def snippet(name, words):
            ...
            try:
                text = unicode(self.storage.open_page(name).read(), 'utf-8', errors='replace')
            except NotFoundErr:
                return ''
        ...
        def search(words):
            ...
            for (score, name) in results:
                yield (
                 score, name, Markup(snippet(name, words)))

        query = kwargs.get('q', None)
        if query is not None:
            query = query.strip()
        if not query:
            data = {'title': 'Page Index', 'ctxnav': self.environ._ctxnav('index')}
            ...
            if not words:
                words = (
                 query,)
            data = {'title': 'Search results for "%s"' % query, 
               'name': 'Search', 
               'query': query, 
    ...
    def backlinks(self, *args, **kwargs):
        ...
        self.storage.reopen()
        self.search.update(self.environ)
        data = {'title': 'BackLinks for "%s"' % name, 
    ...
    def feed(self, *args, **kwargs):
        ...
        if format not in ('rss1', 'rss2', 'atom'):
    ...
    def wanted(self, *args, **kwargs):
        self.storage.reopen()
        self.search.update(self.environ)
        data = {'title': 'Wanted Pages', 
           'pages': sorted(self.search.wanted_pages(), key=itemgetter(0), reverse=True), 
           'ctxnav': self.environ._ctxnav('index')}
    ...
    def favicon(self, *args, **kwargs):
        if 'favicon.ico' in self.storage:
            ...
            return self.serve_file(os.path.join(self.config.get('theme'), 'htdocs', 'favicon.ico'))
    ...
    def robots(self, *args, **kwargs):
        ...
        return ('\r\n').join(('User-agent: *', 'Disallow: /+', 'Disallow: /%2b', 'Disallow: /%2B'))
        ...
        if name not in self.storage:
            ...
            if action == 'rename':
                ...
                if newname and newname not in self.storage:
                    ...
                    data = {'success': False, 'message': '%s already exists' % newname}
                else:
                    data = {'success': False, 'message': 'A new name is requried'}
                return self.render('rename.html', **data)
            raise Exception('Invalid action %r' % action)
            return
    ...
    def diff(self, *args, **kwargs):
        ...
        if not from_date:
            from_date = to_date
        diff = ('\n').join(unified_diff(text, other, '%s@%d' % (name, from_rev), '%s@%d' % (name, to_rev), strftime(date_format, gmtime(from_date)), strftime(date_format, gmtime(to_date))))
        data = {'title': 'diff of %s from %s to %s' % (name, from_rev, to_rev), 
