...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/sahriswiki/creoleparser/elements.py
# Compiled at: 2010-07-22 04:45:57
import re, urlparse, urllib, keyword, sys, genshi.builder as bldr
...
MACRO_NAME = '(?P<name>[a-zA-Z]+([-.]?[a-zA-Z0-9]+)*)'
try:
...
class WikiElement(object):
    ...
    def _build(self, mo, element_store, environ):
        ...
        """
        return bldr.tag.__getattr__(self.tag)(fragmentize(mo.group(1), self.child_elements, element_store, environ))
    ...
    def _process(self, mos, text, wiki_elements, element_store, environ):
        ...
        for mo in mos:
            if end != mo.start():
                frags.extend(fragmentize(text[end:mo.start()], wiki_elements[1:], element_store, environ))
        ...
        if end < len(text):
            ...
            frags.extend(fragmentize(text[end:], wiki_elements, element_store, environ))
        return frags
...
class InlineElement(WikiElement):
    ...
    def re_string(self):
        if isinstance(self.token, str):
            content = '(.+?)'
            end = '(' + esc_neg_look + re.escape(self.token) + '|$)'
...
class SimpleElement(InlineElement):
    ...
    def _build(self, mo, element_store, environ):
        return bldr.tag.__getattr__(self.token_dict[mo.group(1)])(fragmentize(mo.group(2), self.child_elements, element_store, environ))
...
class LinkElement(InlineElement):
    ...
    def __init__(self, tag, token, delimiter, interwiki_delimiter, base_urls, links_funcs, default_space_char, space_chars, base_url, space_char, class_func, path_func):
    ...
    def content_re_string(self):
        return '(?P<body>.*?)(' + re.escape(self.delimiter) + '(?P<arg_string>.*?))?$'

    def interwikilink_re_string(self):
        all_wikis = set(self.links_funcs.keys() + self.base_urls.keys())
        wiki_id = '(?P<wiki_id>' + ('|').join(all_wikis) + ')'
        optional_spaces = ' *'
        page_name = '(?P<page_name>\\S+?( \\S+?)*)'
        return '^' + optional_spaces + wiki_id + re.escape(self.interwiki_delimiter) + ' *' + page_name + optional_spaces + '$'
...
class AnchorElement(LinkElement):
    ...
    def emit(self, element_store, environ, link_type, body, url, the_class, alias=None):
        if alias:
            ...
            alias = body.strip()
        return bldr.tag.__getattr__(self.tag)(alias, href=url, class_=the_class)
...
class Macro(WikiElement):
    ...
    def re_string(self):
        content = '(.*?)'
        return esc_neg_look + re.escape(self.token[0]) + '(' + MACRO_NAME + content + ')' + esc_neg_look + re.escape(self.token[1])
    ...
    def _build(self, mo, element_store, environ):
        ...
        if self.func:
            ...
            value = None
        if value is None:
            return bldr.tag.code(self.token[0], bldr.tag.span(mo.group('name'), class_='macro_name'), bldr.tag.span(arg_string, class_='macro_arg_string'), self.token[1], class_='unknown_macro')
        else:
            if isinstance(value, (basestring, bldr.Fragment, bldr.Element, Stream)):
                return value
...
class BodiedMacro(Macro):
    ...
    def re_string(self):
        content = '(?P<arg_string>[ \\S]*?)'
        body = '(?P<body>.+)'
        return esc_neg_look + re.escape(self.token[0]) + MACRO_NAME + content + '(?<!/)' + re.escape(self.token[1]) + body + esc_neg_look + re.escape(self.token[0]) + '/(?P=name)' + '(?<!/)' + re.escape(self.token[1])

    def _build(self, mo, element_store, environ):
        start = ('').join([esc_neg_look, re.escape(self.token[0]), re.escape(mo.group('name')),
         '(?P<arg_string>[ \\S]*?)', re.escape(self.token[1])])
        ...
        if value is None:
            content_out = [
             self.token[0], bldr.tag.span(mo.group('name'), class_='macro_name'),
            ...
            return [
             bldr.tag.code(content_out, class_='unknown_macro', style='white-space:pre-wrap'), tail]
        else:
            if isinstance(value, (basestring, bldr.Fragment, Stream)):
                return [value, tail]
            raise Exception('macros can only return strings and genshi objects')
            return
...
class BodiedBlockMacro(WikiElement):
    ...
    def re_string(self):
        ...
        body = '(?P<body>.*\\n)'
        end = re.escape(self.token[0]) + '/(?P=name)' + '(?<!/)' + re.escape(self.token[1]) + '\\s*?$'
        return start + '(' + MACRO_NAME + arg_string + ')' + '(?<!/)' + re.escape(self.token[1]) + '\\s*?\\n' + body + end
    ...
    def _process(self, mos, text, wiki_elements, element_store, environ):
        ...
        if isinstance(processed, basestring) and not isinstance(processed, Markup):
            ...
            frags = []
            if mo.start():
                frags.extend(fragmentize(text[:mo.start()], wiki_elements[1:], element_store, environ))
            frags.append(processed)
            if self.append_newline:
                frags.append('\n')
            if tail or mo.end() < len(text):
                frags.extend(fragmentize(tail + text[mo.end():], wiki_elements, element_store, environ))
        return frags

    def _build(self, mo, element_store, environ):
        start = ('').join(['^', re.escape(self.token[0]), re.escape(mo.group('name')),
         '(?P<arg_string>(?![^\\n]*>>[^\\n]*>>)[ \\S]*?)', re.escape(self.token[1]), '\\s*?\\n'])
        end = ('').join(['^', re.escape(self.token[0]), '/', re.escape(mo.group('name')),
         re.escape(self.token[1]), '\\s*?$'])
        count = 0
        ...
        if value is None:
            return [
             bldr.tag.pre(self.token[0], bldr.tag.span(mo.group('name'), class_='macro_name'), bldr.tag.span(mo.group('arg_string'), class_='macro_arg_string'), self.token[1], '\n', bldr.tag.span(mo.group('body'), class_='macro_body'), self.token[0] + '/' + mo.group('name') + self.token[1], class_='unknown_macro'), tail]
        else:
            if isinstance(value, (Stream, basestring)) or isinstance(value, bldr.Element) and value.tag in BLOCK_TAGS:
                return [value, tail]
            if isinstance(value, bldr.Fragment):
...
class RawLink(InlineElement):
    ...
    def re_string(self):
        ...
        protocol = '((https?)://'
        rest_of_url = '\\S+?)'
        look_ahead = '(?=([>)}\\]]?[,.?!:;"\\\']?(([^a-zA-Z0-9])\\6)?(\\s|$))|<<<)'
        return escape + protocol + rest_of_url + look_ahead

    def _build(self, mo, element_store, environ):
        if not mo.group(1) and mo.group(3) in self.linking_protocols:
            return bldr.tag.__getattr__(self.tag)(self.alias(mo, element_store), href=self.href(mo))
        else:
            return self.href(mo)
...
class URLLink(WikiElement):
    ...
    def re_string(self):
        protocol = '^\\s*((\\w+?:|/)'
        rest_of_url = '[\\S\\n]*?)\\s*'
        alias = '(' + re.escape(self.delimiter) + ' *(.*?))? *$'
    ...
    def _build(self, mo, element_store, environ):
        if not self.href(mo):
            ...
            return bldr.tag.__getattr__(self.tag)(self.alias(mo, element_store, environ), href=self.href(mo))
...
class InterWikiLink(WikiElement):
    ...
    def __init__(self, tag, delimiter1, delimiter2, base_urls, links_funcs, default_space_char, space_chars):
    ...
    def re_string(self):
        ...
        optional_spaces = ' *'
        page_name = '(\\S+?( \\S+?)*)'
        alias = '(' + re.escape(self.delimiter2) + ' *(.*?))? *$'
        return '^' + optional_spaces + wiki_id + optional_spaces + re.escape(self.delimiter1) + optional_spaces + page_name + optional_spaces + alias
...
class WikiLink(WikiElement):
    ...
    def __init__(self, tag, delimiter, base_url, space_char, class_func, path_func):
    ...
    def re_string(self):
        ...
        page_name = '(\\S+?( \\S+?)*?)'
        alias = '(' + re.escape(self.delimiter) + ' *(.*?))? *$'
        return '^' + optional_spaces + page_name + optional_spaces + alias
    ...
    def _build(self, mo, element_store, environ):
        if self.class_func:
            ...
            the_class = None
        return bldr.tag.__getattr__(self.tag)(self.alias(mo, element_store, environ), href=self.href(mo, environ), class_=the_class)
...
class List(BlockElement):
    ...
    def re_string(self):
        ...
        rest_of_list = '.*?(?:\\n|\\Z))'
        only_one_stop_token = '([' + re.escape(self.stop_tokens) + '])(?!\\3)'
        look_ahead = '(?=([ \t]*' + only_one_stop_token + '|$))'
        return leading_whitespace + only_one_token + rest_of_list + look_ahead
...
class ListItem(BlockElement):
    ...
    def re_string(self):
        whitespace = '[ \\t]*'
        item_start = '(([' + self.list_tokens + '])\\2*)'
        rest_of_item = '(.*?(?:\\n|\\Z))'
        start_of_same_level_item = '\\1(?!\\2)'
        look_ahead = '(?=(' + whitespace + start_of_same_level_item + '|$))'
        return whitespace + item_start + whitespace + rest_of_item + look_ahead

    def _build(self, mo, element_store, environ):
        return bldr.tag.__getattr__(self.tag)(fragmentize(mo.group(3), self.child_elements, element_store, environ))
...
class NestedList(WikiElement):
    ...
    def re_string(self):
        ...
        whitespace = '(\\s*'
        rest_of_list = '.*$)'
        return look_behind + '^' + whitespace + re.escape(self.token) + rest_of_list
...
class DefinitionTerm(BlockElement):
    ...
    def re_string(self):
        look_ahead = '(\\n|(?=(' + esc_neg_look + re.escape(self.stop_token) + '|$)))'
        return '^[ \\t]*' + re.escape(self.token) + '[ \\t]*(.*?' + re.escape(self.stop_token) + '?)\\s*' + look_ahead
...
class DefinitionDef(BlockElement):
    ...
    def re_string(self):
        look_ahead = '(?=(^[ \\t]*' + re.escape(self.token) + ')|\\Z)'
        return '^[ \\t]*' + re.escape(self.token) + '?[ \\t]*(.+?)\\s*' + look_ahead
...
class Paragraph(BlockElement):
    ...
    def _build(self, mo, element_store, environ):
        ...
        for (i, element) in enumerate(content):
            if isinstance(element, bldr.Element) and element.tag in BLOCK_ONLY_TAGS or isinstance(element, (Stream, Markup)):
        ...
        if block_only_frags:
            ...
            for i in block_only_frags:
                if content[last_i + 1:i]:
                    if not (len(content[last_i + 1:i]) == 1 and content[(last_i + 1)] == '\n'):
...
class Heading(BlockElement):
    ...
    def re_string(self):
        ...
        content = '(.*?)'
        trailing_markup = '(' + re.escape(self.token) + '+[ \\t]*)?(\\n|\\Z)'
        return '^' + whitespace + tokens + whitespace + content + whitespace + trailing_markup

    def _build(self, mo, element_store, environ):
        heading_tag = self.tags[(len(mo.group(1)) - 1)]
        return bldr.tag.__getattr__(heading_tag)(fragmentize(mo.group(2), self.child_elements, element_store, environ))
...
class Table(BlockElement):
    ...
    def re_string(self):
        whitespace = '[ \\t]*'
        rest_of_line = '.*?(\\n|\\Z)'
        return '^((' + whitespace + re.escape(self.token) + rest_of_line + ')+)'
...
class TableRow(BlockElement):
    ...
    def re_string(self):
        ...
        return '^' + whitespace + content + trailing_token + whitespace + '(\\n|\\Z)'
...
class TableCell(WikiElement):
    ...
    def re_string(self):
        ...
        return esc_neg_look + re.escape(self.token) + whitespace + content + whitespace + look_ahead
...
class Image(InlineElement):
    ...
    def __init__(self, tag, token, delimiter):
        ...
        self.regexp = re.compile(self.re_string())
        self.delimiter = delimiter
        self.src_regexp = re.compile('^\\s*(\\S+)\\s*$')
...
class NoWikiElement(InlineElement):
    ...
    def _build(self, mo, element_store, environ):
        if self.tag:
            return bldr.tag.__getattr__(self.tag)(fragmentize(mo.group(1), self.child_elements, element_store, environ, remove_escapes=False))
        else:
            return bldr.tag(fragmentize(mo.group(1), self.child_elements, element_store, environ, remove_escapes=False))

    def re_string(self):
        if isinstance(self.token, str):
            content = '(.+?' + re.escape(self.token[(-1)]) + '*)'
            return esc_neg_look + re.escape(self.token) + content + re.escape(self.token)
        else:
            content = '(.+?' + re.escape(self.token[1][(-1)]) + '*)'
            return esc_neg_look + re.escape(self.token[0]) + content + re.escape(self.token[1])
...
class PreBlock(BlockElement):
    ...
    def re_string(self):
        if isinstance(self.token, str):
            return '^' + re.escape(self.token) + '\\s*?\\n(.*?\\n)' + re.escape(self.token) + '\\s*?\\n'
        else:
            start = '^' + re.escape(self.token[0]) + '\\s*?\\n'
            content = '(.+?\\n)'
            end = re.escape(self.token[1]) + '\\s*?$'
            return start + content + end

    def re_string2(self):
        """Finds a closing token with a space at the start of the line."""
        if isinstance(self.token, str):
            return '^ (\\s*?' + re.escape(self.token) + '\\s*?\\n)'
        else:
            return '^ (\\s*?' + re.escape(self.token[1]) + '\\s*?\\n)'

    def _build(self, mo, element_store, environ):
        match = self.regexp2.sub('\\1', mo.group(1))
        return bldr.tag.__getattr__(self.tag)(fragmentize(match, self.child_elements, element_store, environ, remove_escapes=False))
...
class IndentedBlock(BlockElement):
    ...
    def _build(self, mo, element_store, environ):
        match = self.regexp2.sub('', mo.group(1))
        return bldr.tag.__getattr__(self.tag)(fragmentize(match, self.child_elements, element_store, environ))
...
class GenericElement(InlineElement):
    ...
    def _build(self, mo, element_store, environ):
        ...
        if d.has_key('all') or d.has_key('environ'):
            raise Exception('GenericElement patterns cannot have "all" or                             "environ" as group names.')
        d['all'] = mo.group(0)
        d['environ'] = environ
        if mo.group(1):
            return mo.group(0)[1:]
...
class KeywordArg(ArgString):
    ...
    def re_string(self):
        return '(?P<key>\\w[\\w0-9]*) *' + re.escape(self.token) + ' *(?P<body>.*?) *(?=\\w[\\w0-9]* *' + re.escape(self.token) + '|$)'
...
class QuotedArg(InlineElement):
    ...
    def re_string(self):
        return esc_neg_look + '(?P<quote>[' + re.escape(self.token) + '])(?P<body>.*?)' + esc_neg_look + '(?P=quote)'
...
class ListArg(ArgString):
   ...
   """
    ...
    def re_string(self):
        return esc_neg_look + re.escape(self.token[0]) + '(?P<body>.*?)' + esc_neg_look + re.escape(self.token[1])
