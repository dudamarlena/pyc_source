...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/sahriswiki/storage.py
# Compiled at: 2010-07-21 23:00:36
"""Storage Classes

...
"""
import re, os, thread, tempfile, mercurial.hg, mercurial.ui, mercurial.util, mercurial.revlog, mercurial.context
...
class WikiStorage(object):
    ...
    def __init__(self, path, charset=None):
        ...
        try:
            self.ui = mercurial.ui.ui(report_untrusted=False, interactive=False, quiet=True)
        except TypeError:
    ...
    def _check_path(self, path):
        ...
        if os.path.islink(path) or os.path.isdir(path):
            raise ForbiddenErr(_("Can't use symbolic links or directories as pages"))
        if not abspath.startswith(self.path):
            raise ForbiddenErr(_("Can't read or write outside of the pages repository"))
    ...
    def _title_to_file(self, title):
        ...
        _windows_device_files = ('CON', 'AUX', 'COM1', 'COM2', 'COM3', 'COM4', 'LPT1',
                                 'LPT2', 'LPT3', 'PRN', 'NUL')
        if filename.split('.')[0].upper() in _windows_device_files or filename.startswith('_') or filename.startswith('.'):
            filename = '_' + filename
    ...
    def merge_changes(self, changectx, repo_file, text, user, parent):
        ...
        try:
            ...
            msg = _('merge of edit conflict')
        except mercurial.util.Abort:
            msg = _('failed merge of edit conflict')
    ...
    def _commit(self, files, text, user):
        try:
            return self.repo.commit(files=files, text=text, user=user, force=True, empty_ok=True)
        except TypeError:
            match = mercurial.match.exact(self.repo_path, '', list(files))
            return self.repo.commit(match=match, text=text, user=user, force=True)

    def save_data(self, title, data, author='', comment='', parent=None):
    ...
    def page_file_meta(self, title):
        ...
        try:
            (st_mode, st_ino, st_dev, st_nlink, st_uid, st_gid, st_size, st_atime, st_mtime, st_ctime) = os.stat(self._file_path(title))
        except OSError:
            return (0, 0, 0)
    ...
    def page_meta(self, title):
        ...
        date = filectx.date()[0]
        author = unicode(filectx.user(), 'utf-8', 'replace').split('<')[0].strip()
    ...
    def history(self):
        ...
        for wiki_rev in range(maxrev, minrev - 1, -1):
            ...
            date = change.date()[0]
            author = unicode(change.user(), 'utf-8', 'replace').split('<')[0].strip()
    ...
    def all_pages(self):
        ...
        for filename in os.listdir(self.path):
            ...
            if os.path.isfile(file_path) and not os.path.islink(file_path) and not filename.startswith('.'):
    ...
    def save_file(self, title, file_name, author='', comment='', parent=None):
        ...
        try:
            ...
            if e.errno == 17 and not os.path.isdir(dir_path):
                raise ForbiddenErr(_("Can't make subpages of existing pages"))
            elif e.errno != 17:
                raise

        super(WikiSubdirectoryStorage, self).save_file(title, file_name, author, comment, parent)
    ...
    def all_pages(self):
        ...
        for (dirpath, dirnames, filenames) in os.walk(self.path):
            ...
            for name in filenames:
                ...
                if os.path.isfile(os.path.join(self.path, filename)) and not filename.startswith('.'):
                    yield url_unquote(filename)
...
class WikiSubdirectoryIndexesStorage(WikiSubdirectoryStorage):
    ...
    def all_pages_tree(self):
        ...
        def generate(root):
            for name in os.listdir(root):
                ...
                if os.path.isdir(path):
                    has_index = any([ os.path.join(base, name, index) in self for index in self.indexes
                                    ])
                    yield {(url_unquote(rel), url_unquote(name), has_index): sorted(generate(path))}
                elif os.path.isfile(path) and not os.path.islink(path) and name not in self.indexes:
                    yield (
                     url_unquote(rel), url_unquote(name))
    ...
    def save_file(self, title, file_name, author='', comment='', parent=None):
        ...
        Save the file and make the subdirectories if needed.
        """
        super(WikiSubdirectoryIndexesStorage, self).save_file(title, file_name, author, comment, parent)