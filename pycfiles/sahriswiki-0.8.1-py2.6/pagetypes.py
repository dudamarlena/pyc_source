...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/sahriswiki/pagetypes.py
# Compiled at: 2010-07-24 11:07:22
...
class WikiPage(object):
    ...
    def download(self):
        ...
        expires(self.request, self.response, 2592000, force=True)
...
class WikiPageText(WikiPage):
    ...
    def edit(self):
        ...
        if action == 'cancel':
            ...
            if action == 'preview':
                data = {'page': {'name': self.name, 'text': text}, 'author': self.environ._user(), 
                   'comment': comment, 
                   'preview': True}
            ...
            if action == 'save':
                ...
                self.storage.save_text(self.name, text, self.environ._user(), comment, parent=parent)
...
class WikiPageWiki(WikiPageText):
    ...
    def _render(self, text=None, data={}):
        if text is None:
            text = self._get_text()
        return self.environ.parser.generate(text, context='block', environ=(
         self.environ, data))
...
class WikiPageImage(WikiPageFile):
    ...
    def view(self):
        ...
        data = {'title': self.name, 
           'name': self.name, 
           'html': tag.img(alt=self.name, src=self.url('/+download/%s' % self.name)), 
           'ctxnav': list(self.environ._ctxnav('view', self.name))}
        return self.render('view.html', **data)
...
class WikiPageCSV(WikiPageFile):
    ...
    def edit(self):
        ...
        if action == 'cancel':
            ...
            if action == 'preview':
                data = {'page': {'name': self.name, 'text': text}, 'html': self._render(), 
...
class WikiPageRST(WikiPageText):
    ...
    def _render(self, text=None):
        ...
        return Markup(docutils.core.publish_parts(text, writer_name='html', settings_overrides=SAFE_DOCUTILS)['html_body'])
    ...
    def edit(self):
        ...
        if action == 'cancel':
            ...
            if action == 'preview':
                data = {'page': {'name': self.name, 'text': text}, 'html': self._render(text), 
                   'author': self.environ._user(), 
                   'comment': comment, 
                   'preview': True}
                return self.render('edit.html', **data)
