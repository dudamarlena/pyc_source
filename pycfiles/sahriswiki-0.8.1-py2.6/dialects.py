...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/sahriswiki/creoleparser/dialects.py
# Compiled at: 2010-07-22 04:44:52
...
def creole10_base(wiki_links_base_url='', wiki_links_space_char='_', interwiki_links_base_urls={}, no_wiki_monospace=True, wiki_links_class_func=None, wiki_links_path_func=None, interwiki_links_funcs={}, interwiki_links_space_chars={}, blog_style_endings=False):
    ...
    url_end_pattern = '\\S+?(?=([>)}\\]]?[,.?!:;"\\\']?(([^a-zA-Z0-9])\\5)?(\\s|$))|<<<)'
    ...
    class Base(Dialect):
        ...
        p = Paragraph('p')
        pre = PreBlock('pre', ['{{{', '}}}'])
        br = GenericElement(blog_style_endings and '\\\\\\\\\\n?|\\n(?!$)' or '\\\\\\\\', 'br')
        raw_link = GenericElement('(?P<protocol>https?|ftp)://' + url_end_pattern, raw_link_tag, '{all}', dict(href='{all}'))
        link = AnchorElement('a', ('[[', ']]'), delimiter='|', interwiki_delimiter=':', base_urls=interwiki_links_base_urls, links_funcs=interwiki_links_funcs, default_space_char=wiki_links_space_char, space_chars=interwiki_links_space_chars, base_url=wiki_links_base_url, space_char=wiki_links_space_char, class_func=wiki_links_class_func, path_func=wiki_links_path_func)
        img = ImageElement('img', ('{{', '}}'), delimiter='|', interwiki_delimiter=':', base_urls=interwiki_links_base_urls, links_funcs=interwiki_links_funcs, default_space_char=embed_space_char, space_chars=interwiki_links_space_chars, base_url=embed_base_url, space_char=embed_space_char, class_func=wiki_links_class_func, path_func=embed_path_func)
        td = TableCell('td', '|')
        th = TableCell('th', '|=')
        tr = TableRow('tr', '|')
        table = Table('table', '|')
        ...
        def __init__(self):
            ...
            self.td.child_elements = self.inline_elements[3:]
            self.th.child_elements = self.inline_elements[3:]
...
def creole11_base(macro_func=None, indent_class=None, indent_style='margin-left:2em', **kwargs):
  ...
  in many cases, like this one, class attributes can be altered directly instead:
   ...
   .. note::
    ...
    class Base(Creole10Base):
        simple_element = SimpleElement(token_dict={'**': 'strong', '//': 'em', ',,': 'sub', '^^': 'sup', 
           '__': 'u', '##': 'code'})
