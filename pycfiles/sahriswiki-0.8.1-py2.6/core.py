...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/sahriswiki/creoleparser/core.py
# Compiled at: 2010-07-22 00:31:56
import re, string, warnings, genshi.builder as bldr
__docformat__ = 'restructuredtext en'
escape_char = '~'
esc_neg_look = '(?<!' + re.escape(escape_char) + ')'
esc_to_remove = re.compile(('').join(['(?<!', re.escape(escape_char), ')', re.escape(escape_char), '(?!([ \\n]|$))']))
place_holder_re = re.compile('<<<(-?\\d+?)>>>')
...
class ArgParser(object):
    ...
    def __init__(self, dialect, convert_implicit_lists=True, arg_func=None, key_func=None, illegal_keys=(), convert_unicode_keys=True):
    ...
    def _parse(self, arg_string, convert_implicit_lists, arg_func, key_func, illegal_keys, convert_unicode_keys):
        frags = fragmentize(arg_string, self.dialect.top_elements, {}, {}, remove_escapes=False)
        positional_args = []
        kw_args = {}
        for arg in frags:
...
def fill_from_store(text, element_store):
    ...
    for mo in mos:
        ...
        frags.append(element_store.get(mo.group(1), mo.group(1).join(['<<<', '>>>'])))
        start = mo.end()
