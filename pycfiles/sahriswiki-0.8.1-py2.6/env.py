...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/sahriswiki/env.py
# Compiled at: 2011-02-12 01:25:41
...
class Environment(BaseComponent):
    filename_map = {'README': (
       ...
       'ChangeLog': (
                   WikiPageText, 'text/plain'), 
       'favicon.ico': (
                     WikiPageImage, 'image/x-icon')}
    mime_map = {'text': WikiPageText, 
    ...
    def __init__(self, config):
        ...
        self.config = config
        self.dbm = DatabaseManager(self.config.get('db'), echo=self.config.get('debug') and self.config.get('verbose')).register(self)
        self.storage = DefaultStorage(self.config.get('repo'), self.config.get('encoding'), index=self.config.get('index'), indexes=self.config.get('indexes'))
        self.search = WikiSearch(self.dbm.session, self.config.get('language'), self.storage)
        self.parser = Parser(create_dialect(creole11_base, macro_func=macros.dispatcher, wiki_links_base_url='/', wiki_links_class_func=self._wiki_links_class_func, wiki_links_path_func=self._wiki_links_path_func), method='xhtml')
        template_config = {'allow_exec': False, 
           'auto_reload': True, 
        ...
        self.site = {'name': self.config.get('name'), 
    ...
    def _user(self):
        return self._login() or self.request.headers.get('X-Forwarded-For', self.request.remote.ip)
    ...
    def _ctxnav(self, type='view', name=None):
        ...
        if name and type == 'view':
            ...
            if 'PAGE_EDIT' in permissions:
                yield (
                 'Edit', self.url('/+edit/%s' % name))
            if 'PAGE_DELETE' in permissions:
                yield (
                 'Delete', self.url('/+delete/%s' % name))
            if 'PAGE_RENAME' in permissions:
                yield (
                 'Rename', self.url('/+rename/%s' % name))
        elif name and type == 'info':
            yield (
             'History', self.url('/+history/%s' % name))
            yield ('Feeds', self._ctxnav('history', name))
        elif name and type == 'misc':
            yield (
             'Download', self.url('/+download/%s' % name))
            if 'PAGE_UPLOAD' in permissions:
                yield (
                 'Upload', self.url('/+upload/%s' % name))
    ...
    def include(self, name, parse=True, context='block', data=None):
        if name in self.storage:
            ...
            if parse:
                return self.parser.generate(text, context=context, environ=(
                 self, data))
            return tag.pre(text)
        else:
            return tag.div(tag.p('Page %s Not Found' % name), class_='error')

    def render(self, template, **data):
        data.update({'sahriswiki': {'version': sahriswiki.__version__}, 
           'url': self.url, 
           'site': self.site, 
