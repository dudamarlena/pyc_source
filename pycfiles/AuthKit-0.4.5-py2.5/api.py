...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/authkit/authenticate/sso/api.py
...
class RedirectingAuthMiddleware(object):
    ...
    def __call__(self, environ, start_response):
        ...
        if path in self.dispatch:
            log.debug('Found %s in dispatch path, calling method %s', path, self.dispath[path])
            return getattr(self, self.path + self.dispatch[path])(environ, start_response)
        for route in self.protect:
            if path.startswith(route) and 'REMOTE_USER' not in environ and 'type=' + self.type in environ['QUERY_STRING']:
                log.debug('Found %s in protection paths. No REMOTE_USER set, running auth verify.', path)
