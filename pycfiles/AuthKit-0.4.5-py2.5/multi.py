...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/authkit/authenticate/multi.py
    ...
    def __call__(self, environ, start_response):
        ...
        def app(environ, start_response):
            ...
            def find(status, headers, exc_info=None):
                ...
                if result is None:
                    ...
                    writable = start_response(status_[(-1)], headers_ and headers_[(-1)] or [], exc_info_[(-1)])
                    return writable
                else:

                    class NotWritableShouldntBeUsed:
                        pass
        ...
        def check():
            for (checker, binding) in self.predicate:
                if checker(environ):
                    log.debug('MultMiddleware self.predicate check() returning %r', binding)
            ...
            for (checker, binding) in self.checker:
                ...
                if checker(environ, status_[(-1)], headers_ and headers_[(-1)] or []):
                    log.debug('MultiMiddleware self.checker check() returning %r', binding)
                    environ['authkit.multi'] = True
...
def status_checker(environ, status, headers):
    ...
    """
    log.debug('Status checker recieved status %r, headers %r, intecept %r', status, headers, environ['authkit.intercept'])
