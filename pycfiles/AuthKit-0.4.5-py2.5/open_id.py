...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/authkit/authenticate/open_id.py
...
"""
import cgi, paste.request, string, sys
...
from authkit.authenticate import get_template, valid_password, get_authenticate_function, strip_base, RequireEnvironKey, AuthKitUserSetter, AuthKitAuthHandler
...
def render(template, **p):
    if sys.version_info >= (2, 4):
        return string.Template(template()).substitute(**p)
    else:
        for (k, v) in p.items():
...
class OpenIDAuthHandler(object):
    ...
    def __call__(self, environ, start_response):
        ...
        content = render(self.template, message='', value='', css_class='', action=baseurl + self.path_verify)
...
class AuthOpenIDHandler():
    ...
    def __init__(self, app, store_type, store_config, baseurl, path_signedin, template=None, session_middleware='beaker.session', path_verify='/verify', path_process='/process', urltouser=None, charset=None, sreg_required=None, sreg_optional=None, sreg_policyurl=None, ax_attr=None):
    ...
    def __call__(self, environ, start_response):
        if not environ.has_key(self.session_middleware):
            raise AuthKitConfigError('The session middleware %r is not present. Have you set up the session middleware?' % self.session_middleware)
    ...
    def verify(self, environ, start_response):
        ...
        if not openid_url:
            response = render(self.template, message='Enter an identity URL to verify.', value='', css_class='', action=baseurl + self.path_verify)
            start_response('200 OK', [
             (
              'Content-type', 'text/html' + self.charset),
             (
        ...
        try:
            ...
            response = render(self.template, message='Error retrieving identity URL: %s' % cgi.escape(str(exc[0])), value=self._quoteattr(openid_url), css_class='error', action=baseurl + self.path_verify)
            start_response('200 OK', [
             (
              'Content-type', 'text/html' + self.charset),
             (
            ...
            if request_ is None:
                response = render(self.template, message='No OpenID services found for <code>%s</code>' % (
                 cgi.escape(openid_url),), value=self._quoteattr(openid_url), css_class='error', action=baseurl + self.path_verify)
                start_response('200 OK', [
                 (
                  'Content-type', 'text/html' + self.charset),
                 (
                ...
                if 'HTTP_REFERER' in environ and not environ['HTTP_REFERER'].endswith(self.path_verify) and not environ['HTTP_REFERER'].endswith(self.path_process):
                ...
                if self.sreg_required or self.sreg_optional or self.sreg_policyurl:
                    ...
                    sreg_request = sreg.SRegRequest(required=required_list, optional=optional_list, policy_url=self.sreg_policyurl)
                ...
                if request_.shouldSendRedirect():
                    ...
                    form_html = request_.formMarkup(trust_root, return_to, form_tag_attrs={'id': 'openid_message'})
    ...
    def process(self, environ, start_response):
        ...
        if environ['REQUEST_METHOD'] == 'GET':
            ...
            params = dict(paste.request.parse_formvars(environ))
        oidconsumer = self._get_consumer(environ)
        info = oidconsumer.complete(dict(params), params.get('openid.return_to'))
        ...
        response = render(self.template, message=message, value=value, css_class=css_class, action=baseurl + self.path_verify)
        start_response('200 OK', [
         (
          'Content-type', 'text/html' + self.charset),
         (
...
class OpenIDUserSetter(AuthKitUserSetter):
    ...
    def __init__(self, app, **options):
        app = AuthOpenIDHandler(app, store_type=options['store_type'], store_config=options['store_config'], baseurl=options.get('baseurl', ''), path_signedin=options['path_signedin'], path_process=options.get('path_process', '/process'), template=options['template'], urltouser=options['urltouser'], charset=options['charset'], sreg_required=options['sreg_required'], sreg_optional=options['sreg_optional'], sreg_policyurl=options['sreg_policyurl'], session_middleware=options['session_middleware'], ax_attr=options['ax_attr'])
        self.app = app
...
def load_openid_config(app, auth_conf, app_conf=None, global_conf=None, prefix='authkit.openid'):
    ...
    user_setter_params = {'store_type': auth_conf['store.type'], 'store_config': auth_conf['store.config'], 
    ...
    auth_handler_params = {'template': user_setter_params['template'], 
       'path_verify': auth_conf.get('path.verify', '/verify'), 
       'baseurl': user_setter_params['baseurl'], 
       'charset': user_setter_params['charset']}
    return (
     app, auth_handler_params, user_setter_params)


def _load_ax_config(auth_conf):
    """Read configuration for attribute exchange"""
    ax_params = {'ax_attr': []}
    ax_attr_list = [ k.replace('ax.typeuri.', '') for k in auth_conf.iterkeys() if k.startswith('ax.typeuri.')
                   ]
    for i in ax_attr_list:
        ...
        attrInfo = ax.AttrInfo(typeURI, count=count, required=required, alias=alias)
        ax_params['ax_attr'] += [attrInfo]

    return ax_params


def make_passurl_handler(app, auth_conf, app_conf=None, global_conf=None, prefix='authkit.openid'):
    (app, auth_handler_params, user_setter_params) = load_openid_config(app, auth_conf, app_conf=None, global_conf=None, prefix='authkit.openid')
    app = MultiHandler(app)
    app.add_method('openid', OpenIDAuthHandler, template=auth_handler_params['template'], path_verify=auth_handler_params['path_verify'], baseurl=auth_handler_params['baseurl'], charset=auth_handler_params['charset'])
