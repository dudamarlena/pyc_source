...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/authkit/users/sqlalchemy_driver/sqlalchemy_044.py
...
class UsersFromDatabase(Users):
    ...
    def update_model(self, model):
        ...
        groups_table = Table('groups', metadata, Column('uid', Integer, primary_key=True), Column('name', String(255), unique=True, nullable=False))
        roles_table = Table('roles', metadata, Column('uid', Integer, primary_key=True), Column('name', String(255), unique=True, nullable=False))
        users_table = Table('users', metadata, Column('uid', Integer, primary_key=True), Column('username', String(255), unique=True, nullable=False), Column('password', String(255), nullable=False), Column('group_uid', Integer, ForeignKey('groups.uid')))
        users_roles_table = Table('users_roles', metadata, Column('user_uid', Integer, ForeignKey('users.uid')), Column('role_uid', Integer, ForeignKey('roles.uid')))
        mapper(Group, groups_table, properties={'users': relation(User)})
        mapper(User, users_table, properties={'roles': relation(Role, lazy=True, secondary=users_roles_table), 
           'group': relation(Group)})
        mapper(Role, roles_table, properties={'users': relation(User, lazy=True, secondary=users_roles_table)})
        model.User = User
        model.Group = Group
        model.Role = Role
        return model
    ...
    def user_create(self, username, password, group=None):
        ...
        if group is None:
            ...
            new_user = self.model.User(username=username.lower(), password=self.encrypt(password), group_uid=self.meta.Session.query(self.model.Group).filter_by(name=group.lower()).first().uid)
    ...
    def list_roles(self):
        ...
        """
        return [ r.name for r in self.meta.Session.query(self.model.Role).order_by(self.model.Role.name)
               ]
    ...
    def list_users(self):
        ...
        """
        return [ r.username for r in self.meta.Session.query(self.model.User).order_by(self.model.User.username)
               ]
    ...
    def list_groups(self):
        ...
        """
        return [ r.name for r in self.meta.Session.query(self.model.Group).order_by(self.model.Group.name)
               ]
    ...
    def user_roles(self, username):
        ...
        if not self.user_exists(username.lower()):
            raise AuthKitNoSuchUserError('No such user %r' % username.lower())
        roles = [ r.name for r in self.meta.Session.query(self.model.User).filter_by(username=username.lower()).first().roles
                ]
        roles.sort()
        return roles
    ...
    def user_group(self, username):
        ...
        if not self.user_exists(username.lower()):
            raise AuthKitNoSuchUserError('No such user %r' % username.lower())
        return self.meta.Session.query(self.model.User).filter_by(username=username.lower()).first().group.name
    ...
    def user_password(self, username):
        ...
        if not self.user_exists(username.lower()):
            raise AuthKitNoSuchUserError('No such user %r' % username.lower())
        return self.meta.Session.query(self.model.User).filter_by(username=username.lower()).first().password
    ...
    def user_has_role(self, username, role):
        ...
        for role_ in self.meta.Session.query(self.model.User).filter_by(username=username.lower()).first().roles:
    ...
    def user_set_group(self, username, group, auto_add_group=False):
        ...
        if not self.group_exists(group.lower()):
            if auto_add_group:
                ...
                raise AuthKitNoSuchGroupError('No such group %r' % group.lower())
        user = self.meta.Session.query(self.model.User).filter_by(username=username.lower()).first()
        user.group = self.meta.Session.query(self.model.Group).filter_by(name=group.lower()).first()
        self.meta.Session.flush()
