...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/authkit/permissions.py
...
no_authkit_users_in_environ = AuthKitConfigError('No authkit.users object exists in the environment. You may have forgotton to specify a Users object or are using the the default valid_password() method in the authenticate middleware when you may have meant to specify your own.')
    ...
    def check(self, app, environ, start_response):
        ...
        if self.end > self.start:
            if now >= self.start and now < self.end:
                ...
                raise error
        elif now < datetime.time(23, 59, 59, 999999) and now >= self.start:
            return app(environ, start_response)
