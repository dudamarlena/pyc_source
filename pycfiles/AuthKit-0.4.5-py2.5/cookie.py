...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/authkit/authenticate/cookie.py
...
import md5, os, inspect, types, time, logging
from paste.deploy.converters import asbool
...
class AuthKitTicket(AuthTicket):
    ...
    def __init__(self, secret, userid, ip, tokens=(), user_data='', time=None, cookie_name='authkit', cookie_params=None, nouserincookie=False):
        self.nouserincookie = nouserincookie
        secure = False
        if cookie_params is None:
            self.cookie_params = {}
        elif cookie_params.has_key('secure'):
        ...
        AuthTicket.__init__(self, secret, userid, ip, tokens=tokens, user_data=user_data, time=time, cookie_name=cookie_name, secure=secure)
        return

    def digest(self):
        digest_ = calculate_digest(self.ip, self.time, self.secret, self.userid, self.tokens, self.user_data)
        log.debug('Calculating the digest ip %r, time %r, secret %r, userid %r, tokens %r, user_data %r, digest %r', self.ip, self.time, self.secret, self.userid, self.tokens, self.user_data, digest_)
        return digest_
...
def calculate_digest(ip, timestamp, secret, userid, tokens, user_data):
    log.debug('calculate_digest(ip=%r, timestamp=%r, secret=%r, userid=%r, tokens=%r, user_data=%r)', ip, timestamp, secret, userid, tokens, user_data)
    digest0 = md5.new(encode_ip_timestamp(ip, timestamp) + secret + userid.encode('utf-8') + '\x00' + tokens + '\x00' + user_data).hexdigest()
...
def encode_ip_timestamp(ip, timestamp):
    ...
    t = int(timestamp)
    ts = ((t & 4278190080) >> 24, (t & 16711680) >> 16, (t & 65280) >> 8,
     t & 255)
...
class CookieUserSetter(AuthKitUserSetter, AuthTKTMiddleware):
    ...
    def __init__(self, app, secret, name='authkit', params=None, includeip=True, signoutpath=None, enforce=False, ticket_class=AuthKitTicket, nouserincookie=False, session_middleware='beaker.session', badcookiepage=True, badcookietemplate=None):
        log.debug('Setting up the cookie middleware')
        secure = False
        if params.has_key('secure') and asbool(params['secure']) == True:
            secure = True
        AuthTKTMiddleware.__init__(self, app, secret, cookie_name=name, secure=secure, include_ip=asbool(includeip), logout_path=signoutpath)
        ...
        if self.cookie_enforce and not self.cookie_params.has_key('expires'):
            raise AuthKitConfigError("Cannot enforce cookie expiration since no cookie_params expires' has been set")
        self.nouserincookie = nouserincookie
        ...
        if self.badcookiepage and not badcookietemplate:
            raise AuthKitConfigError('No badcookiepage.template option was specified for the cookie middleware')
    ...
    def __call__(self, environ, start_response):
        ...
        if cookies.has_key(self.cookie_name):
            ...
            cookie_value = ''
        log.debug('Our cookie %r value is therefore %r', self.cookie_name, cookie_value)
        remote_addr = environ.get('HTTP_X_FORWARDED_FOR', environ.get('REMOTE_ADDR', '0.0.0.0'))
        remote_addr = remote_addr.split(',')[0]
        log.debug('Remote addr %r, value %r, include_ip %r', remote_addr, cookie_value, self.include_ip)
        if cookie_value:
            if self.include_ip:
                pass
            else:
                remote_addr = '0.0.0.0'
            try:
                log.debug('Parsing ticket secret %r, cookie value %r, remote address %s', self.secret, cookie_value, remote_addr)
                (timestamp, userid, tokens, user_data) = parse_ticket(self.secret, cookie_value, remote_addr, session)
            except BadTicket, e:
                if e.expected:
        ...
        if self.logout_path and environ.get('PATH_INFO') == self.logout_path or environ.get('authkit.cookie.error', False):
            logout_user()
    ...
    def set_user_cookie(self, environ, userid, tokens, user_data):
        if self.include_ip:
            ...
            remote_addr = '0.0.0.0'
        ticket = self.ticket_class(self.secret, userid, remote_addr, tokens=tokens, user_data=user_data, cookie_name=self.cookie_name, cookie_params=self.cookie_params, nouserincookie=self.nouserincookie)
        ...
        if self.nouserincookie:
            if self.cookie_name == environ[self.session_middleware].key:
                raise AuthKitConfigError('The session cookie name %r is the same as the AuthKit cookie name. Please change the session cookie name.' % environ[self.session_middleware].key)
...
def load_cookie_config(app, auth_conf, app_conf=None, global_conf=None, prefix='authkit.cookie.'):
    ...
    if template_conf:
        ...
        template_ = template
    user_setter_params = {'params': strip_base(auth_conf, 'params.'), 'ticket_class': AuthKitTicket, 
    ...
    if not user_setter_params.has_key('secret'):
        raise AuthKitConfigError('No cookie secret specified under %r' % (prefix + 'secret'))
    if user_setter_params.has_key('signout'):
        raise AuthKitConfigError('The authkit.cookie.signout option should now be named signoutpath')
    return (app, None, user_setter_params)


def make_cookie_user_setter(app, auth_conf, app_conf=None, global_conf=None, prefix='authkit.cookie.'):
    (app, auth_handler_params, user_setter_params) = load_cookie_config(app, auth_conf, app_conf=None, global_conf=None, prefix='authkit.cookie.')
