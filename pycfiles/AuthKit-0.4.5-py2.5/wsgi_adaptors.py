...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/authkit/authorize/wsgi_adaptors.py
        ...
        if all_conf.get('setup.enable', True) is True:
            try:
                ...
                if environ.has_key('REMOTE_USER'):
                    raise NonConformingPermissionError('Faulty permission: NotAuthenticatedError raised but REMOTE_USER key is present.')
                else:
                    raise
...
def authorize_request(environ, permission):
    ...
    if all_conf is None:
        raise Exception('Authentication middleware not present')
    if all_conf.get('setup.enable', True) is True:
        error = PermissionSetupError('The permissions being authorized require access to a response and so cannot be used to authorize based on a request alone. Try using the authkit.authorize.middleware or the authorize decorator.')
        try:

            def dummy_app(environ, start_response):
                if not start_response == _PermissionStartResponse:
                    raise _FiddledWith('Fiddled with start_response %r' % start_response)
                start_response('1000 Test Response For Permissions Check', [
                 ('Content-type', 'text/plain')])
                return _PermissionList('Dummy response from permission check.')

            if not isinstance(permission.check(dummy_app, environ, _PermissionStartResponse), _PermissionList):
