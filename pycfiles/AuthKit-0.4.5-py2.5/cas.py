...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/authkit/authenticate/sso/cas.py
...
class AuthCASMiddleware(RedirectingAuthMiddleware):
    ...
    def verify(self, environ, start_response):
        ...
        if 'ticket' not in req.GET:
            log.debug('No ticket found in request, unable to verify, returning404 Not Found.')
        ...
        if self._cas2:
            ...
            if valid:
                ...
                log.info('Authentication failed for auth: %s, ticket %s, response: %s', self._authtype, ticket, tree[0].attrib['code'])
        else:
            ...
            if valid:
                ...
                log.info('Authentication failed for auth: %s, ticket %s, response: %s', self._authtype, ticket, result[0])
        if not valid:
...
def make_cas_handler(app, auth_conf, app_conf=None, global_conf=None, prefix='authkit.sso.cas'):
