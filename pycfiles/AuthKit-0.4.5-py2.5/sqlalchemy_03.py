...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/authkit/users/sqlalchemy_driver/sqlalchemy_03.py
...
class UsersFromDatabase(Users):
    ...
    def update_model(self, model):
        ...
        groups_mapper = assign_mapper(ctx, Group, groups_table, properties={'users': relation(User)})
        users_mapper = assign_mapper(ctx, User, users_table, properties={'roles': relation(Role, lazy=True, secondary=users_roles_table), 
           'group': relation(Group)})
        roles_mapper = assign_mapper(ctx, Role, roles_table, properties={'users': relation(User, lazy=True, secondary=users_roles_table)})
        model.User = User
        model.Group = Group
        model.Role = Role
        return model
    ...
    def list_roles(self):
        ...
        """
        return [ r.name for r in self.model.Role.select(order_by=self.model.Role.c.name)
               ]
    ...
    def list_users(self):
        ...
        """
        return [ u.username for u in self.model.User.select(order_by=self.model.User.c.username)
               ]
    ...
    def list_groups(self):
        ...
        """
        return [ g.name for g in self.model.Group.select(order_by=self.model.Group.c.name)
               ]
    ...
    def user_roles(self, username):
        ...
        if not self.user_exists(username.lower()):
            raise AuthKitNoSuchUserError('No such user %r' % username.lower())
        roles = [ r.name for r in self.model.User.get_by(username=username.lower()).roles ]
        roles.sort()
        return roles
