...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/authkit/users/sqlalchemy_driver/sqlalchemy_04.py
# Compiled at: 2009-10-05 09:07:44
import warnings
warnings.warn('This module is untested and unmaintained, please upgrade to SQLAlchemy  0.4.4 or above', DeprecationWarning, 2)
...
class UsersFromDatabase(Users):
    ...
    def __init__(self, model, encrypt=None):
        ...
        if hasattr(model, 'authkit_initialized'):
            raise AuthKitError('The AuthKit database model has already been setup')
        else:
    ...
    def update_model(self, model):
        ...
        groups_table = Table('groups', meta, Column('uid', Integer, primary_key=True), Column('name', String(255), unique=True, nullable=False))
        roles_table = Table('roles', meta, Column('uid', Integer, primary_key=True), Column('name', String(255), unique=True, nullable=False))
        users_table = Table('users', meta, Column('uid', Integer, primary_key=True), Column('username', String(255), unique=True, nullable=False), Column('password', String(255), nullable=False), Column('group_uid', Integer, ForeignKey('groups.uid')))
        users_roles_table = Table('users_roles', meta, Column('user_uid', Integer, ForeignKey('users.uid')), Column('role_uid', Integer, ForeignKey('roles.uid')))
        groups_mapper = mapper(Group, groups_table, properties={'users': relation(User)})
        users_mapper = mapper(User, users_table, properties={'roles': relation(Role, lazy=True, secondary=users_roles_table), 
           'group': relation(Group)})
        roles_mapper = mapper(Role, roles_table, properties={'users': relation(User, lazy=True, secondary=users_roles_table)})
        model.User = User
        model.Group = Group
        model.Role = Role
        return model
    ...
    def user_create(self, username, password, group=None):
        ...
        if group is None:
            new_user = self.model.User(username=username.lower(), password=self.encrypt(password))
        else:
            if not self.group_exists(group):
                raise AuthKitNoSuchGroupError('There is no such group %r' % group)
            new_user = self.model.User(username=username.lower(), password=self.encrypt(password), group_uid=self.model.Group.get_by(name=group.lower()).uid)
    ...
    def user_exists(self, username):
        ...
        """
        user = self.model.Session.query(self.model.User).filter_by(username=username.lower()).first()
        if user:
            return True
        return False
    ...
    def role_exists(self, role):
        ...
        case insensitive.
        """
        role = self.model.Session.query(self.model.Role).filter_by(name=role.lower()).first()
        if role:
            return True
        return False
    ...
    def group_exists(self, group):
        ...
        """
        group = self.model.Session.query(self.model.Group).filter_by(name=group.lower()).first()
        if group:
            return True
        return False
    ...
    def list_roles(self):
        ...
        """
        return [ r.name for r in self.model.Session.query(self.model.Role).order_by(self.model.Role.name)
               ]
    ...
    def list_users(self):
        ...
        """
        return [ r.name for r in self.model.Session.query(self.model.User).order_by(self.model.User.username)
               ]
    ...
    def list_groups(self):
        ...
        """
        return [ r.name for r in self.model.Session.query(self.model.Group).order_by(self.model.Group.name)
               ]
    ...
    def user(self, username):
        ...
        user = self.model.Session.query(self.model.User).filter_by(username=username.lower()).one()
    ...
    def user_roles(self, username):
        ...
        if not self.user_exists(username.lower()):
            raise AuthKitNoSuchUserError('No such user %r' % username.lower())
        roles = [ r.name for r in self.model.Session.query(self.model.User).filter_by(username=username.lower()).one().roles
                ]
        roles.sort()
        return roles
    ...
    def user_group(self, username):
        ...
        if not self.user_exists(username.lower()):
            raise AuthKitNoSuchUserError('No such user %r' % username.lower())
        return self.model.Session.query(self.model.User).filter_by(username=username.lower()).one().group.name
    ...
    def user_password(self, username):
        ...
        if not self.user_exists(username.lower()):
            raise AuthKitNoSuchUserError('No such user %r' % username.lower())
        return self.model.Session.query(self.model.User).filter_by(username=username.lower()).one().password
    ...
    def user_has_role(self, username, role):
        ...
        for role_ in self.model.Session.query(self.model.User).filter_by(username=username.lower()).one().roles:
    ...
    def user_has_group(self, username, group):
        ...
        if user.group is None:
            if group == None:
                return True
        elif group is not None and user.group.name == group.lower():
            return True
        return False
    ...
    def user_set_group(self, username, group, auto_add_group=False):
        ...
        if not self.group_exists(group.lower()):
            if auto_add_group:
                ...
                raise AuthKitNoSuchGroupError('No such group %r' % group.lower())
        user = self.model.Session.query(self.model.User).filter_by(username=username.lower()).one()
        user.group = self.model.Session.query(self.model.Group).filter_by(name=group.lower()).one()
        self.model.Session.save(user)
        self.model.Session.flush()
    ...
    def user_add_role(self, username, role, auto_add_role=False):
        ...
        if not self.role_exists(role.lower()):
            if auto_add_role:
                ...
                raise AuthKitNoSuchRoleError('No such role %r' % role.lower())
        user = self.model.Session.query(self.model.User).filter_by(username=username.lower()).one()
        role = self.model.Session.query(self.model.Role).filter_by(name=role.lower()).one()
