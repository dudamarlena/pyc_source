...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/scythe/helpers/scythecore.py
# Compiled at: 2014-06-14 11:13:40
            ...
            for e in GFFParser().parse(gff):
                if e.type == 'mRNA':
                    if e.attrib_dct['Parent'] not in self._mapping:
                        self._mapping[e.attrib_dct['Parent']] = [
                         e.attrib_dct['ID']]
...
class ScytheFrame(object):
    ...
    def callNeedleAll(self, asequence, bsequence, outfile, gapOpen, gapExtend, aformat='score', stype1='-sprotein1', stype2='-sprotein2', stdout=False):
        ...
        """
        cmd = 'needleall -auto -asequence {0} -bsequence {1} -aformat {2} {3} {4} -outfile {5} -gapopen {6} -gapextend {7}'.format(asequence, bsequence, aformat, stype1, stype2, outfile, gapOpen, gapExtend)
        if stdout:
            cmd = 'needleall -auto -asequence {0} -bsequence {1} -aformat {2} {3} {4} -gapopen {5} -gapextend {6} -stdout'.format(asequence, bsequence, aformat, stype1, stype2, gapOpen, gapExtend)
