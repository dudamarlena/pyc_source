...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/scythe/scythe-gui.py
# Compiled at: 2014-06-16 04:34:26
import os, sys, re, tkinter as tk, configparser, multiprocessing
...
import scythe, helpers.mergeSubsets as mergeSubsets, helpers.ensembl_ortho_mysql as ensembl_ortho_mysql, helpers.ensembl2grp as ensembl2grp, helpers.ensembl as ensembl, multiprocessing, queue
...
SECTIONS = [
 CF_MODE, CF_PATHS, CF_CLEANUP, CF_RUN, CF_PENALTIES, CF_ALGORITHM, CF_FASTAHEADER]
            ...
            for u in self.txt_subsec[t]:
                ...
                try:
                    ...
                    try:
                        ...
                        if u in OPTIONS:
                            dd_subsec[t] = [
                             OptionMenu(fr, self.var_subsec[t][(-1)], *OPTIONS[u])]
                        else:
                            dd_subsec[t] = [
                             '']
                    except KeyError as e:
...
class ScytheWizard(tk.Tk):
    ...
    def prepRun(self, reloadFields=True):
        ...
        if useEnsembl == 'yes' and reloadFields:
    ...
    def initWizard(self):
        ...
        self.st_outDir.set('')
        self.ent_fastaDir = tk.Entry(root, width=30, textvariable=self.st_fastaDir, state=tk.DISABLED)
        self.ent_locDir = tk.Entry(root, width=30, textvariable=self.st_locDir, state=tk.DISABLED)
        self.ent_grpFile = tk.Entry(root, width=30, textvariable=self.st_grpFile, state=tk.DISABLED)
        self.ent_outDir = tk.Entry(root, width=30, textvariable=self.st_outDir, state=tk.DISABLED)
        self.b_loadConfig = tk.Button()
        self.b_saveConfig = tk.Button()
        self.b_fastaDir = tk.Button(text='open...', command=self.askFastaDir, state=tk.DISABLED)
        self.b_locDir = tk.Button(text='open...', command=self.askLocDir, state=tk.DISABLED)
        self.b_grpFile = tk.Button(text='open...', command=self.askGrpFile, state=tk.DISABLED)
        self.b_outDir = tk.Button(text='open...', command=self.askOutDir, state=tk.DISABLED)
        self.b_next = tk.Button(root, text='Next...', command=self.prepRun)
        self.b_quit = tk.Button(root, text='Quit', command=self.quit)
        self.cb_use_ensembl = tk.Checkbutton(root, text='use ENSEMBL', variable=self.int_ensembl, command=self.useEnsembl)
        self.cb_use_local = tk.Checkbutton(root, text='use local files', variable=self.int_local, command=self.useLocal, state=tk.NORMAL)
