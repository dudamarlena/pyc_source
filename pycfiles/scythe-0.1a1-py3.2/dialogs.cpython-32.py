...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/scythe/gui/dialogs.py
# Compiled at: 2014-06-14 10:56:16
import tkinter as tk
from tkinter import filedialog
import convert.scythe_loc_gff, os, random, string
...
class ScytheConvertDialogGrp:
    ...
    def __init__(self):
        ...
        self.st_dir.set('')
        self.ent_dir = tk.Entry(self.root, width=30, textvariable=self.st_dir, state=tk.NORMAL)
        self.lab_file = tk.Label(self.root, text='gff/tsv file')
        self.st_file = tk.StringVar()
        self.st_file.set('')
        self.ent_file = tk.Entry(self.root, width=30, textvariable=self.st_file, state=tk.NORMAL)
        ...
        self.cb_use_tsv = tk.Checkbutton(root, text='.tsv (eg ENSEMBL)', variable=self.int_tsv, command=self.useEnsembl)
        self.cb_use_proteinortho = tk.Checkbutton(root, text='proteinortho output', variable=self.int_proteinortho, command=self.useProteinortho, state=tk.NORMAL)
        self.cb_use_orthomcl = tk.Checkbutton(root, text='orthomcl output ', variable=self.int_orthomcl, command=self.useOrthomcl, state=tk.NORMAL)
    ...
    def askFile(self):
        formats = [
         ('GFF', '*.gff'), ('GFF', '*.gff3'), ('tab separated', '*.tsv')]
