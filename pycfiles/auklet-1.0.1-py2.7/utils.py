...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/auklet/utils.py
# Compiled at: 2018-10-16 12:42:27
import os, sys, uuid, hashlib, requests, tempfile
from auklet.__about__ import __version__ as auklet_version
from auklet.errors import AukletConfigurationError
__all__ = [
 'open_auklet_url', 'post_auklet_url', 'create_file', 'clear_file',
...
def open_auklet_url(url, apikey):
    ...
    try:
        ...
        if e.code == 401:
            raise AukletConfigurationError('Invalid configuration of Auklet Monitoring, ensure proper API key and app ID passed to Monitoring class')
        raise e
    except URLError:
        return
...
def post_auklet_url(url, apikey, data):
    try:
        res = requests.post(url, json=data, headers={'Authorization': 'JWT %s' % apikey, 'Content-Type': 'application/json'})
