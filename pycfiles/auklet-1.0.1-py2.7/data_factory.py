...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/data_factory.py
...
class MonitoringDataGenerator(object):
    ...
    def __str__(self):
        return '{"commitHash": "%s", "id": "%s", "publicIP": "%s", "timestamp": %d, "application": "%s", "macAddressHash": "%s", "callees": [{"lineNumber": %d, "nSamples": %d, "functionName": "%s", "nCalls": %d}]}' % (
...
class ConfigGenerator(object):
    ...
    def __str__(self):
        return '{"brokers": ["%s"], "prof_topic": "%s", "event_topic": "%s", "log_topic": "%s", "user_metrics_topic": "%s"}' % (
...
class LimitsGenerator(object):
    ...
    def __init__(self, storage_limit='null', emission_period=60, performance_metrics='true', user_metrics='false', cellular_data_limit='null', normalized_cell_plan_date=1):
    ...
    def __str__(self):
        return '{"storage": {"storage_limit": ' + self.storage_limit + '}, "emission_period": ' + self.emission_period + ', "features": {"performance_metrics": ' + self.performance_metrics + ', "user_metrics": ' + self.user_metrics + '}, "data": {"cellular_data_limit": ' + self.cellular_data_limit + ', "normalized_cell_plan_date": ' + self.normalized_cell_plan_data + '}}'