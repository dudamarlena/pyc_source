...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/src/benchmark/run_tests.py
...
def with_auklet_and_mqtt(get_certs_mock, update_limits_mock):
    ...
    conf_patcher = patch('auklet.broker.MQTTClient._read_from_conf', new=_load_conf)
    producer_patcher = patch('auklet.broker.MQTTClient.create_producer', new=create_producer)
    get_conf_patcher = patch('auklet.broker.MQTTClient._get_conf', new=_get_conf)
    register_device_patcher = patch('auklet.monitoring.processing.Client._register_device', new=register_device)
    conf_patcher.start()
...
def display_complete_results():
    ...
    try:
        print('\n\nTests comparison.')
        print(my_list[0][0].split('_')[0], my_list[number_of_tests][0].split('_')[0], sep='\t')
