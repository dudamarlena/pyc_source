...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/test_processing.py
# Compiled at: 2018-10-16 12:42:27
import os, ast, json, unittest
...
class TestClient(unittest.TestCase):
    ...
    def setUp(self):
        with patch('auklet.monitoring.processing.Client._register_device', new=self.__register_device):
            self.broker_username = 'test-username'
            self.broker_password = 'test-password'
            self.client = Client(api_key='', app_id='', base_url='https://api-staging.auklet.io/', auklet_dir='.auklet')
    ...
    def test_register_device(self):
        with patch('json.loads', return_value=False):
            with patch('auklet.monitoring.processing.Client.create_device') as (create_mock):
                create_mock.return_value = {'client_password': 'test-pass', 'client_id': '12345', 
                   'id': '12345', 
                   'organization': '12345'}
                self.assertTrue(self.client._register_device())
        with patch('json.loads') as (loads_mock):
            with patch('auklet.monitoring.processing.Client.check_device') as (check_mock):
                loads_mock.return_value = {'id': '12345'}
                check_mock.return_value = (
                 {'client_password': 'test-pass', 'client_id': '12345', 
                    'id': '12345', 
                    'organization': '12345'},
                 True)
                self.assertTrue(self.client._register_device())
    ...
    def test_check_device(self):
        with patch('auklet.monitoring.processing.open_auklet_url', return_value=self.MockResult()):
            res = self.client.check_device('123')
            self.assertFalse(res[1])
        with patch('auklet.monitoring.processing.open_auklet_url', side_effect=HTTPError('url', 404, 'failed post', {'test': 'header'}, None)):
            res = self.client.check_device('123')
            self.assertTrue(res[1])
        return

    def test_create_device(self):
        with patch('auklet.monitoring.processing.post_auklet_url', return_value=self.MockResult()):
    ...
    def test_load_limits(self):
        ...
        data = data_factory.LimitsGenerator(cellular_data_limit='null', storage_limit=10)
        self.write_load_limits_test(data)
        self.client._load_limits()
        ...
        self.base_patch_side_effect_with_none('auklet.monitoring.processing.open', IOError, self.client._load_limits)
        return
    ...
    def test_check_data_limit(self):
        ...
        self.assertTrue(self.client.check_data_limit(data=self.data, current_use=0, offline=True))
        self.client.data_limit = None
        self.assertTrue(self.client.check_data_limit(data=self.data, current_use=0))
        self.client.offline_limit = self.client.data_limit = 1
        self.assertFalse(self.client.check_data_limit(data=self.data, current_use=0))
        self.client.data_limit = 1000
        self.client.check_data_limit(data=self.data, current_use=0, offline=True)
    ...
    def test_update_limits(self):
        with patch('auklet.monitoring.processing.Client._get_config', new=self._get_config):
    ...
    def test_build_event_data(self):
        with patch('auklet.monitoring.processing.traceback.format_exc', new=self.mock_format_exc):
            self.assertBuildEventData(self.client.build_event_data)

    def test_build_log_data(self):
        self.assertBuildLogData(self.client.build_log_data(msg='msg', data_type='data_type', level='level'))
    ...
    def test_build_msgpack_log_data(self):
        self.assertBuildLogData(self.client.build_msgpack_log_data(msg='msg', data_type='data_type', level='level'))
    ...
    def assertBuildEventData(self, function):
        ...
        with patch('auklet.monitoring.processing.Event', new=self.get_mock_event):
            self.monitoring_tree.cached_filenames['file_name'] = 'file_name'
            self.assertNotEqual(function(type=Exception, tb=traceback(), tree=self.monitoring_tree), None)
        return
    ...
    def _get_config(self):
        if self.none:
            ...
            return {'storage': {'storage_limit': self.storage_limit}, 'emission_period': 60, 'features': {'performance_metrics': True, 'user_metrics': False}, 
               'data': {'cellular_data_limit': self.cellular_data_limit, 'normalized_cell_plan_date': self.cell_plan_date}}
            return
