...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/auklet/monitoring/processing.py
# Compiled at: 2018-10-16 12:42:27
import json, msgpack, traceback
from time import time
from uuid import uuid4
from datetime import datetime
from auklet.stats import Event, SystemMetrics
from auklet.utils import create_file, get_abs_path, get_device_ip, open_auklet_url, build_url, get_agent_version, post_auklet_url, u
try:
...
class Client(object):
    ...
    def __init__(self, api_key=None, app_id=None, release=None, base_url='https://api.auklet.io/', mac_hash=None, version='', auklet_dir=''):
        self.apikey = api_key
        self.app_id = app_id
    ...
    def _set_filenames(self):
        ...
        self.com_config_filename = self.com_config_filename.format(self.auklet_dir)
        self.identification_filename = self.identification_filename.format(self.auklet_dir)

    def _register_device(self):
        try:
            read_id = json.loads(open(self.identification_filename, 'r').read())
            if not read_id:
                raise IOError
        ...
        self._write_identification({'id': self.broker_username, 'client_password': self.broker_password, 
    ...
    def check_device(self, device_id):
        try:
            opened = open_auklet_url(build_url(self.base_url, ('private/devices/{}/').format(device_id)), self.apikey)
    ...
    def create_device(self):
        return post_auklet_url(build_url(self.base_url, 'private/devices/'), self.apikey, {'mac_address_hash': self.mac_hash, 'application': self.app_id})
    ...
    def _get_config(self):
        res = open_auklet_url(build_url(self.base_url, ('private/devices/{}/app_config/').format(self.app_id)), self.apikey)
        if res is not None:
            ...
            with open(self.limits_filename, 'r') as (limits):
                ...
                if limits_str:
                    ...
                    if temp_limit is not None:
                        self.data_limit = data['data']['cellular_data_limit'] * MB_TO_B
                    else:
                        self.data_limit = temp_limit
                    temp_offline = data['storage']['storage_limit']
                    if temp_offline is not None:
                        self.offline_limit = data['storage']['storage_limit'] * MB_TO_B
                    else:
