...
class TestUtils(unittest.TestCase):
    ...
    def test_open_auklet_url(self):
        ...
        with patch('auklet.utils.urlopen') as (url_open):
            ...
            url_open.side_effect = HTTPError(url=None, code=401, msg=None, hdrs=None, fp=None)
            self.assertRaises(AukletConfigurationError, lambda : open_auklet_url(url, self.client.apikey))
            url_open.side_effect = HTTPError(url=None, code=None, msg=None, hdrs=None, fp=None)
            self.assertRaises(HTTPError, lambda : open_auklet_url(url, self.client.apikey))
    ...
    def test_create_file(self):
        files = [
         '.auklet/local.txt', '.auklet/limits',
    ...
    def test_build_url(self):
        ...
        self.assertEqual(build_url(self.client.base_url, extension), self.client.base_url + extension)
    ...
    def test_setup_thread_excepthook(self):
        print '\nDue to the nature of this test and of the function itself, the two Stack Traces, Exception and KeyboardInterrupt, must print in order to prove validity of test.'
        from threading import Thread
        self.assertIsNone(setup_thread_excepthook())
        thread_except = Thread(target=self.throw_exception)
        thread_keyboard_interrupt = Thread(target=self.throw_keyboard_interrupt)
        thread_except.start()
