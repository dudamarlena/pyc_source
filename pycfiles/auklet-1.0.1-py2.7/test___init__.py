...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/test___init__.py
...
class TestMonitoring(unittest.TestCase):
        ...
        with patch('auklet.broker.MQTTClient._get_conf') as (_get_conf):
            with patch('auklet.monitoring.processing.Client._register_device', new=self.__register_device):
                with patch('os.path.isfile') as (is_file_mock):
                    ...
                    self.monitoring = Monitoring(api_key='', app_id='', release='', base_url='https://api-staging.io', monitoring=True)
    ...
    def test_initialize_without_release(self):
        self.assertRaises(AukletConfigurationError, Monitoring, api_key='', app_id='', base_url='https://api.auklet.io/')
    ...
    def test_process_periodic(self):
        ...
        with patch('auklet.broker.MQTTClient.produce', new=produce):
            with patch('auklet.monitoring.processing.Client.check_date', new=check_date):
    ...
    def test_handle_exc(self):
        with patch('auklet.broker.MQTTClient.produce') as (_produce):
            with patch('auklet.monitoring.processing.Client.build_msgpack_event_data') as (_build_msgpack_event_data):
