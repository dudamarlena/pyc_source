...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/src/benchmark/statprof/statprof.py
...
def display(fp=None, format=DisplayFormat.BY_LINE, path_format=PathFormat.FULL_PATH, order=DisplayOrder.LOCAL):
    ...
    if state.sample_count == 0:
        p(b'Test not long enough to create a statistical analysis...')
        with open(b'/tmp/benchmark_results', b'a') as (file):
            file.write(b'0\n')
        file.close()
        return
    else:
        ...
        with open(b'/tmp/benchmark_results', b'a') as (file):
            file.write(str(state.accumulated_time) + b'\n')
        file.close()
        return
