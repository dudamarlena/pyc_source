...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/test_stats.py
...
class TestEvent(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patcher.start()
        self.event = Event(exc_type=str, tb=self.get_traceback(), tree=self.tree, abs_path='/')
    ...
    def test_convert_locals_to_string(self):
        ...
        self.assertNotEqual(self.event._convert_locals_to_string(local_vars={'key': True}), None)
        return
    ...
    def build_patcher(self, patch_location, return_value, expected=[
 {'functionName': None, 'filePath': '', 
    'lineNumber': 0, 
    'locals': {'key': 'value'}}]):
        with patch(patch_location) as (mock_patcher):
            mock_patcher.return_value = return_value
            self.event._build_traceback(trace=self.get_traceback(), tree=self.tree)
...
class TestMonitoringTree(unittest.TestCase):
    ...
    def test_get_filename(self):
        ...
        self.assertIsNone(self.monitoring_tree.get_filename(code=self.get_code(), frame='frame'))
        with patch('inspect.getsourcefile') as (_get_source_file):
            _get_source_file.return_value = 'file_name'
            self.assertIsNotNone(self.monitoring_tree.get_filename(code=self.get_code(), frame='frame'))
        return
    ...
    def test_create_frame_function(self):
        ...
        self.assertIsNotNone(self.monitoring_tree._create_frame_func('', root=True))
        with patch('auklet.stats.MonitoringTree.get_filename') as (_get_filename):
            _get_filename.return_value = None
            self.assertIsNotNone(self.monitoring_tree._create_frame_func(Frame))
    ...
    def test__build_tree(self):
        ...
        with patch('auklet.stats.MonitoringTree._create_frame_func') as (create_frame_func):
    ...
    def test_update_sample_count(self):
        ...
        if sys.version_info < (3, ):
            self.build_assert_raises("class NewChild has no attribute 'children'", True)
            self.build_assert_raises("Parent instance has no attribute 'children'", False)
        else:
    ...
    def test_update_hash(self):
        ...
        with patch('auklet.stats.MonitoringTree._update_sample_count', new=_update_sample_count):
    ...
    def test_build_msgpack_tree(self):
        self.monitoring_tree.root_func = self.get_root_function()
        self.assertNotEqual(self.monitoring_tree.build_msgpack_tree(self.Client()), None)
        return
    ...
    def build_assert_raises(self, expected, child_state):
        ...
        with self.assertRaises(AttributeError) as (error):
            self.monitoring_tree._update_sample_count(parent=self.Parent(), new_parent=self.NewParent())
