...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/oyProjectManager/ui/version_updater.py
# Compiled at: 2013-02-05 13:32:41
import os, sys
...
class MainDialog(QtGui.QDialog, version_updater_UI.Ui_Dialog):
    ...
    def _fill_UI(self):
        ...
        for i, version_info in enumerate(self._version_tuple_list):
            ...
            latest_published_version_number = str(version.latest_published_version().version_number)
            item = QtGui.QTableWidgetItem(latest_published_version_number)
            item.setTextAlignment(QtCore.Qt.AlignHCenter | QtCore.Qt.AlignVCenter)
        ...
        if len(unpublished_versions):
            QtGui.QMessageBox.warning(self, 'Warning', 'The following references have no published versions:\n\n' + ('\n').join([ vers.filename for vers in unpublished_versions ]) + '\n\nPlease publish them and re-open the current file.', QtGui.QMessageBox.Ok)
        return
