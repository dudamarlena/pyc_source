...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/oyProjectManager/models/repository.py
...
class Repository(object):
    ...
    def __init__(self):
        ...
        try:
            self._windows_path = self.conf.repository['windows_path'].replace('\\', '/')
    ...
    def _validate_repository_env_key(self):
        ...
        if not os.environ.has_key(self.conf.repository_env_key):
            raise RuntimeError('Please set an environment variable with the name %s and set it to your repository path' % self.conf.repository_env_key)
        if os.environ[self.conf.repository_env_key] == '':
            raise ValueError('The %s environment variable can not be an empty string' % self.conf.repository_env_key)
    ...
    def update_project_list(self):
        ...
        try:
            ...
            for folder in child_folders:
                if os.path.exists(os.path.join(self.server_path, folder, self.conf.database_file_name)):
    ...
    def server_path(self):
        """The server path
        """
        return os.path.expandvars(os.path.expandvars(os.path.expanduser(os.environ[self.conf.repository_env_key])))
    ...
    def get_project_name(self, file_path):
        ...
        if file_path is None:
            ...
            file_path = os.path.expandvars(os.path.expanduser(os.path.normpath(file_path))).replace('\\', '/')
    ...
    def relative_path(self, path):
        ...
        """
        return path.replace(self.server_path, '$' + self.conf.repository_env_key)