...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/oyProjectManager/models/sequence.py
...
class Sequence(Base):
    r"""Sequence object to help manage sequence related data.
    ...
    __table_args__ = (
     ...
     UniqueConstraint('name', 'project_id'), {'extend_existing': True})
    ...
    shots = relationship('Shot', cascade='all, delete-orphan')
    ...
    def __new__(cls, project=None, name=None, code=None):
        ...
        if project and name:
            ...
            seq_db = db.session.query(Sequence).filter_by(name=name).first()
            if seq_db is not None:
                logger.debug('found the sequence in the database')
                logger.debug('returning the Sequence instance from the database')
                seq_db.__skip_init__ = None
                return seq_db
            logger.debug('the Sequence should be new, there is no such Sequence in the database')
    ...
    def add_shots(self, shot_range_formula):
        ...
        for shot_number in new_shot_numbers:
            ...
            if self.project.shot_number_prefix != '' and self.project.shot_number_prefix is not None:
                if shot_number.startswith(self.project.shot_number_prefix):
                    shot_number = shot_number[len(self.project.shot_number_prefix):]
                    shot_number = re.sub('^[0]+', '', shot_number)
    ...
    def add_alternative_shot(self, shot_number):
        ...
        alternative_shot_number = self.get_next_alternate_shot_number(shot_number)
    ...
    def get_next_alternate_shot_number(self, shot_number):
        ...
        for letter in alternate_letters:
            ...
            shot_from_db = Shot.query().filter(Shot.sequence_id == self.id).filter(Shot.number == new_shot_number).first()
            if not shot_from_db:
    ...
    def __eq__(self, other):
        ...
        """
        return isinstance(other, Sequence) and other.name == self.name and other.project == self.project
    ...
    def _check_project(cls, project):
        ...
        if not isinstance(project, Project):
            raise TypeError('The project should be and instance of oyProjectManager.models.project.Project')
        return project
    ...
    def _validate_code(self, key, code):
        ...
        if not isinstance(code, (str, unicode)):
            raise TypeError('Sequence.code should be an instance of str or unicode, not %s' % type(code))
    ...
    def _condition_name(cls, name):
        ...
        if not isinstance(name, (str, unicode)):
            raise TypeError('Sequence.name should be an instance of string or unicode not %s' % type(name))
        if name is '':
