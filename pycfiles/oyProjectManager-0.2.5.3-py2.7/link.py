...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/oyProjectManager/models/link.py
...
class FileLink(Base):
    ...
    def _validate_filename(self, key, filename):
        ...
        if not isinstance(filename, (str, unicode)):
            raise TypeError('FileLink.filename should be a str or unicode, not %s' % filename.__class__.__name__)
        return filename
    ...
    def _validate_path(self, key, path):
        ...
        if not isinstance(path, (str, unicode)):
            raise TypeError('FileLink.path should be a str or unicode, not %s' % path.__class__.__name__)
        return path
    ...
    def _validate_type(self, key, type_):
        ...
        if not isinstance(type_, (str, unicode)):
            raise TypeError('FileLink.type should be a str or unicode, not %s' % type_.__class__.__name__)
        return type_