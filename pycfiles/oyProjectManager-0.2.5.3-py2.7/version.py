...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/oyProjectManager/models/version.py
# Compiled at: 2012-11-07 17:58:21
import os, re
from copy import copy
import jinja2
from sqlalchemy import UniqueConstraint, Column, Integer, ForeignKey, String, Boolean, Enum, Table
...
class Version(Base, IOMixin):
    r"""Holds versions of assets or shots.
    ...
    __table_args__ = (
     UniqueConstraint('version_of_id', 'take_name', '_version_number', 'type_id'), {'extend_existing': True})
    id = Column(Integer, primary_key=True)
    version_of_id = Column(Integer, ForeignKey('Versionables.id'), nullable=False)
    ...
    references = relationship('Version', secondary='Version_References', primaryjoin='Versions.c.id==Version_References.c.referencer_id', secondaryjoin='Version_References.c.reference_id==Versions.c.id', backref='referenced_by')
    is_published = Column(Boolean, default=False)
    _status = Column(Enum(name='StatusNames', *conf.status_list))

    def __init__(self, version_of, base_name, type, created_by, take_name=conf.default_take_name, version_number=1, note='', extension='', is_published=False, status=None, inputs=[], outputs=[]):
    ...
    def __eq__(self, other):
        ...
        """
        return isinstance(other, Version) and self.base_name == other.base_name and self.version_of == other.version_of and self.type == other.type and self.take_name == other.take_name and self.version_number == other.version_number
    ...
    def update_paths(self):
        ...
        self._output_path = jinja2.Template(self.type.output_path).render(**kwargs)
    ...
    def _validate_version_of(self, key, version_of):
        ...
        if not isinstance(version_of, VersionableBase):
            raise TypeError('Version.version_of should be an Asset or Shot or anything derives from VersionableBase class')
        return version_of
    ...
    def _validate_type(self, key, type):
        ...
        if not isinstance(type, VersionType):
            raise TypeError('Version.type should be an instance of VersionType class')
        if self.version_of.__class__.__name__ != type.type_for:
            raise TypeError('The VersionType instance given for Version.type is not suitable for the given VersionableBase instance, the version_of is a %s and the version_type is for %s' % (
    ...
    def _validate_extension(self, extension):
        ...
        if not isinstance(extension, (str, unicode)):
            raise TypeError('Version.extension should be an instance of string or unicode')
        if extension != '':
    ...
    extension = synonym('_extension', descriptor=property(fget=_extension_getter, fset=_extension_setter), doc='The extension of this version file, updating the extension will\n        also update the filename\n        ')
    ...
    def _template_variables(self):
        ...
        kwargs = {'project': self.version_of.project, 
    ...
    def path(self):
        ...
        """
        return os.path.join(self.project.path, self._path).replace('\\', '/')

    @property
    def full_path(self):
        r"""The full_path of this version.
        
        It is the join of
        :class:`~oyProjectManager.models.repository.Repository`.\ 
        :attr:`~oyProjectManager.models.repository.Repository.server_path` and
        :class:`~oyProjectManager.models.version.Version`.        :attr:`~oyProjectManager.models.version.Version.path` and
        :class:`~oyProjectManager.models.version.Version`.        :attr:`~oyProjectManager.models.version.Version.filename` attributes.
    ...
    def output_path(self):
        r"""The output_path of this version.
        ...
        :attr:`~oyProjectManager.models.repository.Repository.server_path`.
        """
        return os.path.join(self.project.path, self._output_path).replace('\\', '/')
    ...
    def _condition_name(self, name):
        ...
        name = ('_').join([ word[0].upper() + word[1:] for word in name.split('_') if len(word)
                          ])
        name = ('-').join([ word[0].upper() + word[1:] for word in name.split('-') if len(word)
                          ])
        return name
    ...
    def _validate_base_name(self, key, base_name):
        ...
        if base_name is None:
            raise TypeError('Version.base_name can not be None, please supply a proper string or unicode value')
        if not isinstance(base_name, (str, unicode)):
            raise TypeError('Version.base_name should be an instance of string or unicode')
        base_name = self._condition_name(base_name)
        if base_name == '':
            raise ValueError('Version.base_name is either given as an empty string or it became empty after formatting')
        return base_name
    ...
    def _validate_take_name(self, key, take_name):
        ...
        if not isinstance(take_name, (str, unicode)):
            raise TypeError('Version.take_name should be an instance of string or unicode')
        take_name = self._condition_name(take_name)
        if take_name == '':
            raise ValueError('Version.take_name is either given as an empty string or it became empty after formatting')
        return take_name
    ...
    def latest_version(self):
        ...
        """
        return db.session.query(Version).filter(Version.type == self.type).filter(Version.version_of == self.version_of).filter(Version.take_name == self.take_name).order_by(Version.version_number.desc()).first()
    ...
    version_number = synonym('_version_number', descriptor=property(_version_number_getter, _version_number_setter))
    ...
    def _validate_note(self, key, note):
        ...
        if not isinstance(note, (str, unicode)):
            raise TypeError('Version.note should be an instance of string or unicode')
        return note
    ...
    def _validate_created_by(self, key, created_by):
        ...
        if created_by is None:
            raise TypeError('Version.created_by can not be None, please set it to oyProjectManager.models.auth.User instance')
        if not isinstance(created_by, User):
            raise TypeError('Version.created_by should be an instance ofoyProjectManager.models.auth.User')
        return created_by
    ...
    def _validate_references(self, key, reference):
        ...
        if reference is self:
            raise ValueError('Version.references can not have a reference to itself')
    ...
    def latest_published_version(self):
        ...
        """
        return db.session.query(Version).filter(Version.type == self.type).filter(Version.version_of == self.version_of).filter(Version.take_name == self.take_name).filter(Version.is_published == True).order_by(Version.version_number.desc()).first()
    ...
    def _validate_status(self, status):
        ...
        if not isinstance(status, (str, unicode)):
            raise TypeError('Version.status should be one an instance of string and the value should be one of of %s not %s' % (
...
class VersionType(Base):
    r"""A template for :class:`~oyProjectManager.models.version.Version` class.
    ...
    def _validate_name(self, key, name):
        ...
        """
        if name is None:
            raise TypeError('VersionType.name can not be None, please supply a string or unicode instance')
        if not isinstance(name, (str, unicode)):
            raise TypeError('VersionType.name should be an instance of string or unicode')
        return name
    ...
    def _validate_code(self, key, code):
        ...
        """
        if code is None:
            raise TypeError('VersionType.code can not be None, please specify a proper string value')
        if not isinstance(code, (str, unicode)):
            raise TypeError('VersionType.code should be an instance of string or unicode, please supply one')
        return code
    ...
    def _validate_extra_folders(self, key, extra_folders):
        ...
        if not isinstance(extra_folders, (str, unicode)):
            raise TypeError('VersionType.extra_folders should be a string or unicode value showing the extra folders those needs to be created with the Version of this type.')
    ...
    def _validate_filename(self, key, filename):
        ...
        """
        if filename is None:
            raise TypeError('VersionType.filename can not be None, please specify a valid filename template string by using Jinja2 template syntax')
        if not isinstance(filename, (str, unicode)):
            raise TypeError('VersionType.filename should be an instance ofstring or unicode')
        if filename == '':
            raise ValueError('VersionType.filename can not be an empty string, it should be a string containing a Jinja2 template code showing the file naming convention of Versions of this type.')
        return filename
    ...
    def _validate_path(self, key, path):
        ...
        """
        if path is None:
            raise TypeError('VersionType.path can not be None, please specify a valid path template string by using Jinja2 template syntax')
        if not isinstance(path, (str, unicode)):
            raise TypeError('VersionType.path should be an instance of string or unicode')
        if path == '':
            raise ValueError('VersionType.path can not be an empty string, it should be a string containing a Jinja2 template code showing the file naming convention of Versions of this type.')
        return path
    ...
    def _validate_output_path(self, key, output_path):
        ...
        if not isinstance(output_path, (str, unicode)):
            raise TypeError('VersionType.output_path should be an instance of string or unicode, not %s' % type(output_path))
        if output_path == '':
            raise ValueError('VersionType.output_path can not be an empty string')
        return output_path
    ...
    def _validate_type_for(self, key, type_for):
        ...
        """
        if type_for is None:
            raise TypeError('VersionType.type_for can not be None, it should be a string or unicode value')
        if not isinstance(type_for, (str, unicode)):
            raise TypeError('VersionType.type_for should be an instance of string or unicode, not %s' % type(type_for))
        return type_for
...
class VersionTypeEnvironments(Base):
    ...
    __table_args__ = {'extend_existing': True}
    versionType_id = Column(Integer, ForeignKey('VersionTypes.id'), primary_key=True)
    environment_name = Column(String, primary_key=True, doc='The name of the environment which the VersionType instance is\n        valid for\n        ')
    version_type = relationship('VersionType', backref=backref('version_type_environments', cascade='all, delete-orphan'))
    ...
    def _validate_environment_name(self, key, environment_name):
        ...
        """
        if environment_name is None or not isinstance(environment_name, (str, unicode)):
            raise TypeError('VersionType.environments should be a list of strings containing the environment names')
...
def _check_circular_dependency(version, check_for_version):
    ...
    for reference in version.references:
        if reference is check_for_version:
            raise CircularDependencyError('version %s can not reference %s, this creates a circular dependency' % (
             version, check_for_version))
        else:
