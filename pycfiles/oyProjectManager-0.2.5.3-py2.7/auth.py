...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/oyProjectManager/models/auth.py
# Compiled at: 2012-10-22 14:14:06
...
class Client(Base):
    ...
    def _validate_name(self, key, name):
        ...
        if name is None or not isinstance(name, (str, unicode)):
            raise TypeError('Client.name should be a string or unicode, not %s' % name.__class__.__name__)
        return name
    ...
    def _validate_code(self, key, code):
        ...
        """
        if code is None:
            code = re.sub('[^A-Z]', '', self.name.title())
            import uuid
            code = code + '_' + uuid.uuid4().get_hex()[:4]
        if not isinstance(code, (str, unicode)):
            raise TypeError('Client.code should be a string or unicode, not %s' % code.__class__.__name__)
        code = code.replace(' ', '')
        return code
