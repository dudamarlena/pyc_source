...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/oyProjectManager/utils/backup.py
...
class BackUp(object):
    ...
    def __init__(self, project=None, output=None, number_of_versions=None, extra_filter_rules=None):
    ...
    def _validate_project(self, project):
        ...
        """
        if not isinstance(project, Project) and not isinstance(project, str):
            raise TypeError('BackUp.project should be an instance ofoyProjectManager.models.project.Project instance')
        if project == '':
            raise ValueError
        if isinstance(project, (str, unicode)):
            project = Project(name=project)
        if not project.exists:
            raise RuntimeError("The project doesn't exists, so it can not be backup")
        return project
