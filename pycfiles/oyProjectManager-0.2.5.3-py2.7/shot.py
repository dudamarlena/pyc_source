...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/oyProjectManager/models/shot.py
# Compiled at: 2013-01-04 13:52:30
...
class Shot(VersionableBase):
    ...
    __tablename__ = 'Shots'
    __table_args__ = (
     UniqueConstraint('sequence_id', 'number'), {'extend_existing': True})
    ...
    _sequence = relationship('Sequence', primaryjoin='Shots.c.sequence_id==Sequences.c.id')

    def __init__(self, sequence, number, start_frame=1, end_frame=1, description=''):
    ...
    def __eq__(self, other):
        ...
        """
        return isinstance(other, Shot) and other.number == self.number and other.sequence == self.sequence
    ...
    def _validate_sequence(self, sequence):
        ...
        if not isinstance(sequence, Sequence):
            raise TypeError('Shot.sequence should be an instance of oyProjectManager.models.sequence.Sequence')
        return sequence
    ...
    def _validate_description(self, key, description):
        ...
        if not isinstance(description, (str, unicode)):
            raise TypeError('Shot.description should be an instance of str or unicode')
        return description
    ...
    def _validate_start_frame(self, key, start_frame):
        ...
        if not isinstance(start_frame, int):
            raise TypeError('Shot.start_frame should be an instance of integer')
    ...
    def _validate_end_frame(self, key, end_frame):
        ...
        if not isinstance(end_frame, int):
            raise TypeError('Shot.end_frame should be an instance of integer')
        if self.end_frame is not None:
    ...
    def _validates_number(self, key, number):
        ...
        if not isinstance(number, (int, str, unicode)):
            raise TypeError('Shot.number should be and instance of integer, string or unicode')
        number = str(number)
        number = re.sub('[^0-9a-zA-Z\\-_\\ ]+', '', number)
        number = re.sub('[\\ ]+', '_', number)
        number = number.upper()
        if number == '':
            raise ValueError('Shot.number is not in good format, please supply something like 1, 2, 3A, 10B')
        shot_instance = db.session.query(Shot).filter(Shot.number == number).filter(Shot.sequence_id == self.sequence.id).first()
        if shot_instance is not None:
            raise ValueError('Shot.number already exists for the given sequence please give a unique shot code')
        return number
    ...
    def code(self):
        ...
        if '-' in self.number or '_' in self.number:
            ...
            number = re.sub('[A-Z]+', '', self.number)
            alter = re.sub('[0-9]+', '', self.number)
            return self.project.shot_number_prefix + number.zfill(self.project.shot_number_padding) + alter
    ...
    def _validate_handles_at_start(self, key, handle_at_start):
        ...
        if not isinstance(handle_at_start, int):
            raise TypeError('Shot.handle_at_start should be an instance of integer not %s' % type(handle_at_start))
        if handle_at_start < 0:
            raise ValueError('Shot.handle_at_start can not be a negative value')
        if self.handle_at_end is not None:
            if handle_at_start + self.handle_at_end > self.duration - 1:
                raise ValueError('Shot.handle_at_start + Shot.handle_at_end can not be bigger than Shot.duration - 1')
    ...
    def _validate_handle_at_end(self, key, handle_at_end):
        ...
        if not isinstance(handle_at_end, int):
            raise TypeError('Shot.handle_at_end should be an instance of integer not %s' % type(handle_at_end))
        if handle_at_end < 0:
            raise ValueError('Shot.handle_at_end can not be a negative value')
