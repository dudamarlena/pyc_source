...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/oyProjectManager/ui/shot_editor.py
...
class MainDialog(QtGui.QDialog, shot_editor_UI.Ui_Dialog):
    ...
    def setup_signals(self):
        """set up the signals
        """
        QtCore.QObject.connect(self.buttonBox, QtCore.SIGNAL('accepted()'), self.dialog_accepted)
        QtCore.QObject.connect(self.buttonBox, QtCore.SIGNAL('rejected()'), self.dialog_rejected)
        QtCore.QObject.connect(self.upload_thumbnail_pushButton, QtCore.SIGNAL('clicked()'), self.upload_thumbnail)
    ...
    def setup_defaults(self):
        ...
        ui_utils.update_gview_with_versionable_thumbnail(self.shot, self.thumbnail_graphicsView)
    ...
    def upload_thumbnail(self):
        ...
        if not self.shot:
            return
        ui_utils.update_gview_with_image_file(thumbnail_full_path, self.thumbnail_graphicsView)
    ...
    def dialog_accepted(self):
        ...
        if self.shot:
            ...
            ui_utils.render_image_from_gview(self.thumbnail_graphicsView, self.shot.thumbnail_full_path)
            self.shot.save()
        self.close()
