...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/oyProjectManager/models/entity.py
...
class VersionableBase(Base):
    ...
    def _validate_description(self, key, description):
        ...
        if not isinstance(description, (str, unicode)):
            raise TypeError('Asset.description should be an instance of string or unicode')
        return description
    ...
    def thumbnail_full_path(self):
        ...
        if isinstance(self, Asset):
            ...
            template_vars.update({'project': self.project, 
               'asset': self, 
        ...
        thumbnail_full_path = os.path.join(os.environ[conf.repository_env_key], path, filename).replace('\\', '/')
...
class EnvironmentBase(object):
    ...
    def get_versions_from_path(self, path):
        r"""Finds Version instances from the given path value.
        ...
        return Version.query().filter(Version.path.startswith(path)).order_by(Version.id.desc()).all()
    ...
    def get_version_from_full_path(self, full_path):
        ...
        version = Version.query().filter(Version.path == path).filter(Version.filename == filename).first()
        return version
