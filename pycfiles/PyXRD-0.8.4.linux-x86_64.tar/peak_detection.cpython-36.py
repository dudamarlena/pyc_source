...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyxrd/calculations/peak_detection.py
# Compiled at: 2020-03-07 03:51:49
# Size of source mod 2**32: 11349 bytes
import numpy as np
...
def score_minerals(peak_list, minerals):
    ...
    for mineral, abbreviation, mpeaks in minerals:
        ...
        for i, (mpos, mint) in enumerate(mpeaks):
            ...
            if abs(epos - mpos) / mpos <= max_pos_dev:
                if epos not in already_matched:
        ...
        if len(p_matches) > 3:
            ...
            p_factor = p_r_value ** 2 * min(1.0 / (abs(1.0 - p_slope) + 1e-50), 1000.0) / 1000.0
            i_factor = (1.0 - min(i_std_err / 0.25, 5.0) / 5.0) * min(1.0 / (abs(1.0 - i_slope) + 1e-50), 1000.0) / 1000.0
...
def peakdetect_zero_crossing(y_axis, x_axis=None, window=49):
    ...
    if x_axis is None:
        ...
        bins = [y_axis[indice:indice + diff] for indice, diff in zip(zero_indices, period_lengths)]
        even_bins = bins[::2]
        odd_bins = bins[1::2]
