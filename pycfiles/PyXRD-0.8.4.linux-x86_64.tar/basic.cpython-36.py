...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/mvc/adapters/gtk_support/basic.py
# Compiled at: 2020-03-07 03:51:48
# Size of source mod 2**32: 5713 bytes
import logging
...
class GtkAdapter(ModelAdapter):
    ...
    def __init__(self, controller, prop, widget, prop_read=None, prop_write=None, value_error=None, spurious=False, wid_read=None, wid_write=None, signal=None, signal_args=None, update=True):
        """
        ...
        """
        super(GtkAdapter, self).__init__(controller,
          prop, widget, prop_read=prop_read,
        ...
        if self._check_widget_type is not None:
            ...
            if not isinstance(widget, self._check_widget_type):
                msg = "Property '%s' from model '%s' has a widget type %s, which can only be used for (a subclass of) a %s widget, and not for a %s widget!" % (
    ...
    def _connect_widget(self):
        ...
        if self._signal:
            try:
                self._signal_id = self._widget.connect(self._signal, self._on_wid_changed, self._signal_args)
            except TypeError:
