...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyxrd/refinement/methods/deap_utils.py
...
class FitnessMin(base.Fitness):
    ...
    def setValues(self, values):
        try:
            ...
            raise TypeError('Both weights and assigned values must be a sequence of numbers when assigning to values of %r. Currently assigning value(s) %r of %r to a fitness with weights %s.' % (
             self.__class__, values, type(values), (-1, ) * len(values))).with_traceback(traceback)

    values = property(getValues, setValues, base.Fitness.delValues, 'Fitness values. Use directly ``individual.fitness.values = values`` in order to set the fitness and ``del individual.fitness.values`` in order to clear (invalidate) the fitness. The (unweighted) fitness can be directly accessed via ``individual.fitness.values``.')
