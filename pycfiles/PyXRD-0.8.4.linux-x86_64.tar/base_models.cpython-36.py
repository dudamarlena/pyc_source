...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyxrd/probabilities/models/base_models.py
...
class _AbstractProbability(RefinementGroup, DataModel, Storable, metaclass=PyXRDRefinableMeta):
    ...
    @property
    def _flags(self):
        return [1 if getattr(self, prop.inherit_flag, False) else 0 for prop in self.Meta.all_properties if getattr(prop, 'inheritable', False)]
    ...
    def validate(self):
        ...
        def _validate_WW(W, R):
            ...
            rank = self.G ** max(R, 1)
            for i in range(rank):
                if abs(np.sum(W[(..., i)]) - self._W[(i, i)]) > 10000.0:
                    W_valid_mask[(..., i)] -= 1

            if abs(np.sum(W) - 1.0) > 10000.0:
                W_valid_mask -= 1
            for i in range(rank):
                for j in range(rank):
            ...
            return (
             W_valid, W_valid_mask)

        def _validate_W(W, R):
            W_valid_mask = np.ones_like(W)
            rank = self.G ** max(R, 1)
            if abs(np.sum(W) - 1.0) > 1000000.0:
                for i in range(rank):
                    W_valid_mask[(i, i)] -= 1
        ...
        def _validate_P(P, R):
            ...
            rank = self.G ** max(R, 1)
            for i in range(rank):
                if abs(np.sum(P[(i, ...)]) - 1.0) > 1000000.0:
                    P_valid_mask[(i, ...)] -= 1
