...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyxrd/generic/plot/event_delegator.py
...
class MPLCanvasEventDelegator(object):
    ...
    def __init__(self, canvas):
        self.canvas = canvas
        self._handler_dict = {'button_press_event':[],  'button_release_event':[],  'draw_event':[],  'key_press_event':[],  'key_release_event':[],  'motion_notify_event':[],  'pick_event':[],  'resize_event':[],  'scroll_event':[],  'figure_enter_event':[],  'figure_leave_event':[],  'axes_enter_event':[],  'axes_leave_event':[],  'close_event':[]}
    ...
    def connect(self, event_name, handler, first=False):
        ...
        handlers = self._get_handlers(event_name)
        if len(handlers) == 0:
            setattr(self, '_event_id_%s' % event_name, self.canvas.mpl_connect(event_name, self._handle_event(event_name)))
        else:
            if not first:
    ...
    def disconnect(self, event_name, handler):
        ...
        try:
            ...
            logger.warning('Tried to disconnect an unregistered handler `%r` on `%r`' % (
             handler, self))
