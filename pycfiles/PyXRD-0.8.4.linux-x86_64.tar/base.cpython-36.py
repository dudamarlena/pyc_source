...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyxrd/specimen/models/base.py
# Compiled at: 2020-03-07 03:51:50
# Size of source mod 2**32: 24114 bytes
from math import pi, log
import numpy as np
from mvc.observers import ListObserver
from mvc.models.properties import StringProperty, SignalMixin, ReadOnlyMixin, FloatProperty, LabeledProperty, ObserveMixin, ListProperty, BoolProperty
...
class Specimen(DataModel, Storable):
    ...
    def data_object(self):
        ...
        self._data_object.range_theta = self._Specimen__get_range_theta()
    ...
    display_calculated = BoolProperty(default=True,
      ...
      mix_with=(
     SignalMixin,))
    display_experimental = BoolProperty(default=True,
    ...
    goniometer = LabeledProperty(default=None,
      ...
      mix_with=(
     SignalMixin, ObserveMixin))
    statistics = LabeledProperty(default=None,
      text='Markers',
      visible=False,
      persistent=False,
      tabular=True)
 ...
 CalculatedLine, child=True, 
                 default_is_class=True, label='Calculated Profile', 
                 parent=self, **calc_pattern_old_kwargs)
 ...
 ExperimentalLine, child=True, 
                 default_is_class=True, label='Experimental Profile', 
                 parent=self, **exp_pattern_old_kwargs)
 ...
 Goniometer, child=True, default_is_class=True, parent=self, **gonio_kwargs)
                self.markers = self.get_list(kwargs, None, 'markers', 'data_markers', parent=self)
                for marker in self.markers:
                    self.observe_model(marker)

                self._specimens_observer = ListObserver((self.on_marker_inserted),
    ...
    def from_experimental_data(filename, parent, parser=xrd_parsers._group_parser, load_as_insitu=False):
        ...
        if len(xrdfiles):
            if getattr(xrdfiles[0], 'relative_humidity_data', None) is not None:
                specimen = None
                x_data = None
                y_datas = []
                rh_datas = []
    ...
    def get_export_meta_data(self):
        ...
        return dict(sample=(self.label + ' ' + self.sample_name),
    ...
    def get_exclusion_selector(self):
        ...
        """
        x = self._Specimen__get_range_theta() * 360.0 / pi
    ...
    def update_visuals(self, phases):
        ...
        if phases is not None:
            self.calculated_pattern.y_names = [phase.name if phase is not None else '' for phase in phases]
            self.calculated_pattern.phase_colors = [phase.display_color if phase is not None else '#FF00FF' for phase in phases]
    ...
    def update_pattern(self, total_intensity, phase_intensities, phases):
        ...
        if len(phases) == 0:
            ...
            for z_index in range(maxZ):
                ...
                self.calculated_pattern.set_data(self._Specimen__get_range_theta() * 360.0 / pi, new_data)
    ...
    def convert_to_fixed(self):
        ...
        otherwise) 
        """
        correction = self.goniometer.get_ADS_to_fixed_correction(self._Specimen__get_range_theta())
    ...
    def convert_to_ads(self):
        ...
        otherwise) 
        """
        correction = 1.0 / self.goniometer.get_ADS_to_fixed_correction(self._Specimen__get_range_theta())
        self.experimental_pattern.apply_correction(correction)
