...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyxrd/phases/controllers/edit_phase_controller.py
...
class EditPhaseController(BaseController):
    ...
    def custom_handler(self, prop, widget):
        if prop.label in ('CSDS_distribution', 'components', 'probabilities', 'based_on'):
            if prop.label == 'CSDS_distribution':
                ...
                if prop.label == 'components':
                    self.reset_components_controller()
                else:
                    if prop.label == 'probabilities':
                        self.reset_probabilities_controller()
                    else:
                        if prop.label == 'based_on':
                            if self.phases_treemodel is not None:
    ...
    def reset_csds_controller(self):
        if self.csds_controller is None:
            self.csds_controller = EditCSDSTypeController(model=(self.model),
              view=(self.csds_view),
              parent=self)
        else:
    ...
    def reset_components_controller(self):
        self.components_controller = ComponentsController(model=(self.model),
    ...
    def reset_probabilities_controller(self):
        if self.probabilities_controller is None:
            if self.model.G > 1:
                self.probabilities_controller = EditProbabilitiesController(model=(self.model.probabilities),
                  view=(self.probabilities_view),
                  parent=self)
        else:
    ...
    def on_based_on_changed(self, combo, user_data=None):
        ...
        if itr is not None:
            ...
            if val != self.model:
                if val.get_based_on_root() != self.model:
                    if val.G == self.model.G:
