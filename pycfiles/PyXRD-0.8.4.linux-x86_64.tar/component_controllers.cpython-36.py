...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyxrd/phases/controllers/component_controllers.py
...
class EditComponentController(BaseController):
    ...
    def components_treemodel(self):
        if self.model.phase.based_on:
            return wrap_list_property_to_treemodel(self.model.phase.based_on, Phase.components)
        else:
            return
    ...
    def update_sensitivities(self):
        ...
        for name in ('interlayer_atoms', 'layer_atoms', 'atom_relations', 'ucp_a',
                     'ucp_b'):
...
class ComponentsController(ChildObjectListStoreController):
    ...
    def load_components(self, filename):
        ...
        if old_comps:
            ...
            if num_oc != num_nc:
                DialogFactory.get_information_dialog('The number of components to import must equal the number of selected components!').run()
                return
            ...
            DialogFactory.get_information_dialog('No components selected to replace!').run()
    ...
    def on_save_object_clicked(self, event):
        ...
        DialogFactory.get_save_dialog('Export components',
    ...
    def on_load_object_clicked(self, event):
        ...
        DialogFactory.get_load_dialog('Import components',
          parent=(self.view.get_toplevel()), filters=(self.file_filters)).run(on_accept)
        return True