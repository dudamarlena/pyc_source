...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyxrd/refinement/controllers/refinement_controller.py
# Compiled at: 2020-03-07 03:51:50
# Size of source mod 2**32: 11794 bytes
import logging
...
class RefinementController(DialogController):
    ...
    def setup_refinables_tree_view(self, store, widget):
            ...
            def get_pb(column, cell, model, itr, user_data=None):
                ...
                try:
                    if not hasattr(ref_prop, 'pb') or not ref_prop.pb:
                        ref_prop.pb = create_pb_from_mathtext((ref_prop.title),
                          align='left',
                          weight='medium')
        ...
        def_float_args = {'sensitive_col':store.c_refinable, 
         'editable_col':store.c_refinable, 
         'visible_col':store.c_refinable, 
         'data_func':get_value}
        widget.append_column(new_text_column('Value', markup_col=store.c_value, edited_callback=(
 on_float_edited,
 (
  store, store.c_value)), **def_float_args))
        widget.append_column(new_text_column('Min', markup_col=store.c_value_min, edited_callback=(
 on_float_edited,
 (
  store, store.c_value_min)), **def_float_args))
        widget.append_column(new_text_column('Max', markup_col=store.c_value_max, edited_callback=(
 on_float_edited,
 (
  store, store.c_value_max)), **def_float_args))
        widget.append_column(new_toggle_column('Refine',
          toggled_callback=(
         self.refine_toggled, (store,)),
          resizable=False,
          expand=False,
    ...
    def _setup_method_options_treeview(self):
        ...
        tv.append_column(new_text_column('Value',
          text_col=0, data_func=get_value,
          edited_callback=(
         on_value_edited, (0, ))))
    ...
    def _launch_gui_updater(self, refiner):
        ...
        def _on_update_gui():
            if self.view is not None and refiner is not None:
                self.view.update_refinement_info(refiner.history.last_residual, refiner.status.message, get_status())
                return True
            else:
                return False
        ...
        with self.model.mixture.needs_update.hold():
            with self.model.mixture.data_changed.hold():
                if len(self.model.mixture.specimens) > 0:
                    with DialogFactory.error_dialog_handler('There was an error when creating the refinement setup:\n{}',
                      parent=(self.view.get_toplevel()),
                      reraise=False):
                        refiner = self.model.get_refiner()
                        self.results_view = RefinerView(parent=(self.view.parent))
                        self.results_controller = RefinerController(refiner=refiner,
                          model=(self.model),
                    ...
                    DialogFactory.get_information_dialog('Cannot refine an empty mixture!',
