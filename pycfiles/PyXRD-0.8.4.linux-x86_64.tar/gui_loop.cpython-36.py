...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/mvc/support/gui_loop.py
...
def load_toolkit_functions(add_idle_call, remove_idle_call, add_timeout_call, remove_timeout_call, start_event_loop, stop_event_loop):
    ...
    global __start_event_loop
    global __stop_event_loop
    if not callable(add_idle_call):
        raise AssertionError
    else:
        if not callable(remove_idle_call):
            raise AssertionError
        else:
            if not callable(add_timeout_call):
                raise AssertionError
            elif not callable(remove_timeout_call):
                raise AssertionError
