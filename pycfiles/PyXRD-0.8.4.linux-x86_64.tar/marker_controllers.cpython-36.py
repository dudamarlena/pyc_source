...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyxrd/specimen/controllers/marker_controllers.py
# Compiled at: 2020-03-07 03:51:50
# Size of source mod 2**32: 16714 bytes
import logging
...
from pyxrd.specimen.views import EditMarkerView, DetectPeaksView, MatchMineralsView
...
class EditMarkerController(BaseController):
    ...
    def on_sample_clicked(self, widget):
        ...
        self.edc = EyeDropper(self.parent.plot_controller, click_callback)
...
class MarkersController(ObjectListStoreController):
    ...
    file_filters = (
     (
      'Marker file', get_case_insensitive_glob('*.MRK')),)
    ...
    def setup_treeview_col_c_visible(self, treeview, name, col_descr, col_index, tv_col_nr):
        ...
        col = new_toggle_column(' ', toggled_callback=(
         self.on_marker_visible_toggled, (treeview.get_model(), col_index)),
    ...
    def on_load_object_clicked(self, event):
        ...
        DialogFactory.get_load_dialog('Import markers',
          parent=(self.view.get_top_widget()), filters=(self.file_filters)).run(on_accept)
    ...
    def on_find_peaks_clicked(self, widget):
        ...
        show_threshold_plot = DialogFactory.get_progress_dialog(action=(sel_model.update_threshold_plot_data),
        ...
        if len(self.model.markers) > 0:
            ...
            DialogFactory.get_confirmation_dialog('Do you want to clear the current markers for this pattern?',
    ...
    def on_match_minerals_clicked(self, widget):
                ...
                for name, abbreviation, peaks, matches, score in matches:
                    for marker in self.get_selected_objects():
                        for mpos, epos in matches:
                            if marker.get_nm_position() * 10.0 == epos:
        ...
        for marker in self.get_selected_objects():
            ...
            marker_peaks.append((marker.get_nm_position() * 10.0, intensity))
...
class MatchMineralController(DialogController):
    ...
    def register_view(self, view):
        ...
        if view is not None:
            ...
            setup_treeview(tv, None, reset=True,
              on_selection_changed=(self.selection_changed))
            tv.append_column(new_text_column('Name',
              markup_col=0, xalign=0))
            tv.append_column(new_text_column('Abbr.',
              markup_col=1, expand=False))
            ...
            tv.append_column(new_text_column('Score',
              markup_col=4,
              expand=False,
              data_func=get_value))
...
class ThresholdController(DialogController):
    ...
    def update_plot(self):
        if self.view is not None:
            ...
            if self.model is not None:
                if self.model.threshold_plot_data is not None:
