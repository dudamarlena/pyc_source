...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyxrd/refinement/methods/deap_pso_cma.py
...
class SwarmStrategy(Cancellable):
    ...
    def __create_strategy(self, parent, sigma, ranges, **kwargs):
        return Strategy(centroid=parent, 
         sigma=sigma, ranges=ranges, stop=self._stop, **kwargs)

    def __init__(self, parents, sigma, ranges, stop, **kwargs):
        self.nswarms = len(parents)
        self._stop = stop
        self.strategies = [(self._SwarmStrategy__create_strategy)((parents[i]), sigma, ranges, **kwargs) for i in range(self.nswarms)]
...
class SwarmAlgorithm(RefineAsyncHelper):
    ...
    def __init__(self, toolbox, halloffame, stats, ngen=NGEN, ngen_comm=NGEN_COMM, verbose=True, refiner=None, stop=None, eval_func=None, result_func=None):
        """
    ...
    def _ask(self):
        ...
        population = self.do_async_evaluation(iter_func=iter_func,
    ...
    def _setup(self, refiner, ngen=NGEN, ngen_comm=NGEN_COMM, nswarms=NSWARMS, **kwargs):
        if not self._has_been_setup:
            ...
            for i in range(1, nswarms):
                parents[i] = create_individual([random.uniform(bounds[(j, 0)], bounds[(j, 1)]) for j in range(len(refiner.history.initial_solution))])

            strategy = SwarmStrategy(parents=parents,
              sigma=0.1,
              ranges=bounds,
              stop=(self._stop))
            ...
            halloffame = HallOfFame(1, similar=(lambda a1, a2: np.all(a1 == a2)))
            stats = self._create_stats()
            self.algorithm = SwarmAlgorithm(toolbox,
              halloffame, stats, ngen=ngen, ngen_comm=ngen_comm, refiner=refiner,
              stop=(self._stop))
