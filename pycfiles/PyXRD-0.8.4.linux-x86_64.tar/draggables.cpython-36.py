...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyxrd/generic/plot/draggables.py
...
class DraggableMixin(object):
    ...
    def _is_dragging(self):
        return not bool(self._draggable_artist is None or self._first_event_data is None or self._prev_event_data is None)

    def __init__(self, artist=None, on_dragged=None, on_released=None, *args, **kwargs):
    ...
    def convert_display_to_figure(self, x, y):
        return self._convert_to_inverse_transform(x, y, self._draggable_figure.transFigure)

    def convert_display_to_axes(self, x, y):
        return self._convert_to_inverse_transform(x, y, self._draggable_axes.transAxes)

    def convert_display_to_data(self, x, y):
        return self._convert_to_inverse_transform(x, y, self._draggable_axes.transData)

    def _draggable_on_press(self, event):
        if self._draggable_figure is None:
            return False
        else:
            contains, attrd = self._draggable_artist.contains(event)
            if not contains:
                return False
    ...
    def _draggable_on_motion(self, event):
        ...
        if callable(self._on_dragged):
            self._on_dragged((
             x0, y0),
              (x1, y1), (x2, y2), event=event)
        self._prev_event_data = (
         x2, y2)
    ...
    _DraggableMixin__connected = False

    def _draggable_connect(self):
        if not self._DraggableMixin__connected:
            ...
            self._DraggableMixin__connected = True

    def _draggable_disconnect(self):
        if self._DraggableMixin__connected:
            delegator = MPLCanvasEventDelegator.wrap_canvas(self._draggable_canvas)
            ...
            self._DraggableMixin__connected = False
...
class DraggableVLine(DraggableMixin):
    ...
    def __init__(self, line, callback=None, window=None):
        super(DraggableVLine, self).__init__(artist=line,
    ...
    def _check_cursor(self, event):
        if self.window is not None:
            ...
            if not change_cursor:
                ...
                arrows = Gdk.Cursor.new(Gdk.CursorType.SB_H_DOUBLE_ARROW)
