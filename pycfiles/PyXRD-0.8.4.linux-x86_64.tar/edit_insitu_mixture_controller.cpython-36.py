...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyxrd/mixture/controllers/edit_insitu_mixture_controller.py
...
class EditInSituMixtureController(BaseController):
    ...
    def _add_phase_view(self, phase_slot):
        ...
        def on_fraction_changed(editable):
            try:
                self.model.fractions[phase_slot] = float(editable.get_text())
            except ValueError:
                return
        ...
        self.view.add_phase_slot((self.phases_treemodel),
          (self.behavs_treemodel), on_phase_delete,
          on_label_changed, on_fraction_changed, None,
          (self.on_phase_combo_changed), (self._is_behav_visible),
          (self.on_behav_combo_changed), label=(self.model.phases[phase_slot]),
          fraction=(self.model.fractions[phase_slot]),
          phases=(self.model.phase_matrix),
          behavs=(self.model.behaviour_matrix))
    ...
    def _add_specimen_view(self, specimen_slot):
        ...
        def on_scale_changed(editable):
            try:
                self.model.scales[specimen_slot] = float(editable.get_text())
            except ValueError:
                return

        def on_bgs_changed(editable):
            try:
                self.model.bgshifts[specimen_slot] = float(editable.get_text())
            except ValueError:
                return
        ...
        self.view.add_specimen_slot((self.phases_treemodel),
          (self.behavs_treemodel), (self.specimens_treemodel), on_specimen_delete,
          on_scale_changed, on_bgs_changed, on_specimen_changed, None,
          (self.on_phase_combo_changed), (self._is_behav_visible),
          (self.on_behav_combo_changed), scale=(self.model.scales[specimen_slot]),
          bgs=(self.model.bgshifts[specimen_slot]),
    ...
    def on_composition_clicked(self, widget, *args):
        ...
        for row in self.model.get_composition_matrix():
            comp += '%s %s\n' % (re.sub('(\\d+)', '<sub>\\1</sub>', row[0]), ' '.join(row[1:]))

        comp += '</span>'
        DialogFactory.get_information_dialog(comp,
