...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyxrd/refinement/refiner.py
...
class RefineSetupError(ValueError):
    __doc__ = ' Raised if an error exists in the refinement setup '
...
class Refiner(object):
    ...
    def __init__(self, method, data_callback, refinables, event_cmgr, metadata={}):
        ...
        if not method is not None:
            raise AssertionError('Cannot refine without a refinement method!')
        else:
        ...
        self.history.set_initial_solution(initial_solution, self.get_history_residual(self.get_residual(initial_solution)))
    ...
    def update(self, solution, residual=None, iteration=0):
        ...
        """
        residual = self.get_history_residual(residual if residual is not None else self.get_residual(solution))
