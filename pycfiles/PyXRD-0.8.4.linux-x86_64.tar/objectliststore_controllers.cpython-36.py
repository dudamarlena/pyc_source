...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyxrd/generic/controllers/objectliststore_controllers.py
# Compiled at: 2020-03-07 03:51:50
# Size of source mod 2**32: 20274 bytes
...
class TreeModelMixin(object):
    ...
    def _update_treemodel_property(self):
        if getattr(self, 'model', None) is not None:
            self._treemodel = wrap_list_property_to_treemodel(self.model, getattr(type(self.model), self.treemodel_property_name))
...
class TreeControllerMixin(TreeViewMixin, TreeModelMixin):
    ...
    _TreeControllerMixin__row_signal_ids = None

    def _update_treemodel_property(self):
        if self._TreeControllerMixin__row_signal_ids is not None:
            old_treemodel, deleted_id, inserted_id = self._TreeControllerMixin__row_signal_ids
            old_treemodel.disconnect(deleted_id)
            old_treemodel.disconnect(inserted_id)
            self._TreeControllerMixin__row_signal_ids = None
        if getattr(self, 'model', None) is not None:
            super(TreeControllerMixin, self)._update_treemodel_property()
            self._TreeControllerMixin__row_signal_ids = (
             self._treemodel,
    ...
    def get_new_edit_view(self, obj):
        ...
        raise NotImplementedError('Unsupported object type %s; subclasses of TreeControllerMixin need to define an obj_type_map attribute!' % obj)
    ...
    def setup_treeview(self, tv):
        ...
        setup_treeview(tv,
          (self.treemodel), sel_mode=sel_mode,
            ...
            if hasattr(self, 'setup_treeview_col_%s' % str(col_descr)):
                ...
                if not handled:
                    tv.append_column(new_text_column(name,
                      text_col=col_index, resizable=(tv_col_nr == 0),
                      expand=(tv_col_nr == 0),
    ...
    def select_objects(self, objs):
        for obj in objs:
            self.select_object(obj, unselect_all=False)
    ...
    def on_del_object_clicked(self, event, del_callback=None, callback=None):
        ...
        if selection.count_selected_rows() >= 1:
            ...
            if not isinstance(parent, Gtk.Window):
                parent = None
            DialogFactory.get_confirmation_dialog(message=(self.delete_msg),
              parent=parent).run(delete_objects)
...
class InlineObjectListStoreController(BaseController, TreeControllerMixin):
    ...
    def _edit_item(self, item):
        ...
        if self._edit_dict is None:
            self._edit_dict = {}
        if item not in self._edit_dict:
    ...
    def update_sensitivities(self):
        self.view.del_item_widget.set_sensitive(self.treeview.get_cursor() != (None,
                                                                               None))
