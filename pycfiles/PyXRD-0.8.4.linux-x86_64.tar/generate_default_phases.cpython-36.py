...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyxrd/scripts/generate_default_phases.py
# Compiled at: 2020-03-07 03:51:50
# Size of source mod 2**32: 26719 bytes
import os
from pyxrd.data import settings
from pyxrd.project.models import Project
from pyxrd.phases.models import Component, Phase

def generate_expandables(filename_format, phase_name, maxR, phase_kwargs_AD, phase_kwargs_EG, phase_kwargs_350, code_AD, code_EG, code_350, comp_kwargs_AD, comp_kwargs_EG, comp_kwargs_350):
    """
            Generates a list of phase descriptions for a combination of an
            AD, EG and 350° Ca-saturated phase linked together
        """
    return [('%s' + filename_format % R, [(dict(R=R, name=phase_name + ' R%d Ca-AD' % R, **phase_kwargs_AD), code_AD, comp_kwargs_AD), (dict(R=R, name=phase_name + ' R%d Ca-EG' % R, based_on=phase_name + ' R%d Ca-AD' % R, **phase_kwargs_EG), code_EG, comp_kwargs_EG), (dict(R=R, name=phase_name + ' R%d Ca-350' % R, based_on=phase_name + ' R%d Ca-AD' % R, **phase_kwargs_350), code_350, comp_kwargs_350)]) for R in range(maxR)]
...
def run(args=None, ui_callback=None):
    ...
    default_phases = []
    inherit_S = dict(inherit_ucp_a=True,
      inherit_ucp_b=True,
      inherit_delta_c=True,
      inherit_layer_atoms=True)
    inherit_all = dict(inherit_d001=True, 
     inherit_default_c=True, 
     inherit_interlayer_atoms=True, 
     inherit_atom_relations=True, **inherit_S)
    inherit_phase = dict(inherit_display_color=True,
    ...
    S_code_EG = 'dS2g'
    S_code_350 = 'dSht'
    S_inh_comp_args = {'dS2g':dict(linked_with='dS2w', **inherit_S), 
    ...
    SS_inh_comp_args.update({'dS1g': dict(linked_with='dS1w', **inherit_S)})
    ...
    SSS_inh_comp_args.update({'dS0w': dict(linked_with='dS0w', **inherit_S)})
    default_phases += [
     (
    ...
    default_phases += generate_expandables('Smectites/SS/Di-SS R%d Ca.phs', 'SS', 4, {}, inherit_phase, inherit_phase, SS_code_AD, SS_code_EG, SS_code_350, {}, SS_inh_comp_args, SS_inh_comp_args)
    default_phases += generate_expandables('Smectites/SSS/Di-SSS R%d Ca.phs', 'SSS', 3, {}, inherit_phase, inherit_phase, SSS_code_AD, SSS_code_EG, SSS_code_350, {}, SSS_inh_comp_args, SSS_inh_comp_args)
    tS_code_AD = 'tS2w'
    tS_code_EG = 'tS2g'
    tS_code_350 = 'tSht'
    tS_inh_comp_args = {'tS2g':dict(linked_with='tS2w', **inherit_S), 
    ...
    tSS_inh_comp_args.update({'tS1g': dict(linked_with='tS1w', **inherit_S)})
    ...
    tSSS_inh_comp_args.update({'tS0w': dict(linked_with='tS0w', **inherit_S)})
    default_phases += [
     (
    ...
    default_phases += generate_expandables('Smectites/SS/Tri-SS R%d Ca.phs', 'SS', 4, {}, inherit_phase, inherit_phase, tSS_code_AD, tSS_code_EG, tSS_code_350, {}, tSS_inh_comp_args, tSS_inh_comp_args)
    default_phases += generate_expandables('Smectites/SSS/Tri-SSS R%d Ca.phs', 'SSS', 3, {}, inherit_phase, inherit_phase, tSSS_code_AD, tSSS_code_EG, tSSS_code_350, {}, tSSS_inh_comp_args, tSSS_inh_comp_args)
    V_code_AD = 'dV2w'
    V_code_EG = 'dV2g'
    V_code_350 = 'dVht'
    V_inh_comp_args = {'dV2g':dict(linked_with='dV2w', **inherit_S), 
    ...
    VV_inh_comp_args.update({'dV1g': dict(linked_with='dV1w', **inherit_S)})
    ...
    VVV_inh_comp_args.update({'dV0w': dict(linked_with='dV0w', **inherit_S)})
    default_phases += [
     (
    ...
    default_phases += generate_expandables('Vermiculites/VV/Di-VV R%d Ca.phs', 'VV', 4, {}, inherit_phase, inherit_phase, VV_code_AD, VV_code_EG, VV_code_350, {}, VV_inh_comp_args, VV_inh_comp_args)
    default_phases += generate_expandables('Vermiculites/VVV/Di-VVV R%d Ca.phs', 'VVV', 3, {}, inherit_phase, inherit_phase, VVV_code_AD, VVV_code_EG, VVV_code_350, {}, VVV_inh_comp_args, VVV_inh_comp_args)
    K_code = 'K   '
    K_inh_comp_args = {'K   ': dict(linked_with='K   ', **inherit_all)}
    ...
    default_phases += generate_expandables('Kaolinite-Smectites/KS/KS R%d Ca.phs', 'KS', 4, {}, inherit_phase, inherit_phase, KS_code_AD, KS_code_EG, KS_code_350, {}, KS_inh_comp_args, KS_inh_comp_args)
    default_phases += generate_expandables('Kaolinite-Smectites/KSS/KSS R%d Ca.phs', 'KSS', 3, {}, inherit_phase, inherit_phase, KSS_code_AD, KSS_code_EG, KSS_code_350, {}, KSS_inh_comp_args, KSS_inh_comp_args)
    default_phases += generate_expandables('Kaolinite-Smectites/KSSS/KSSS R%d Ca.phs', 'KSSS', 2, {}, inherit_phase, inherit_phase, KSSS_code_AD, KSSS_code_EG, KSSS_code_350, {}, KSSS_inh_comp_args, KSSS_inh_comp_args)
    I_code = 'I   '
    I_inh_comp_args = {'I   ': dict(linked_with='I   ', **inherit_all)}
    ...
    default_phases += generate_expandables('Illite-Smectites/IS/IS R%d Ca.phs', 'IS', 4, {}, inherit_phase, inherit_phase, IS_code_AD, IS_code_EG, IS_code_350, {}, IS_inh_comp_args, IS_inh_comp_args)
    default_phases += generate_expandables('Illite-Smectites/ISS/ISS R%d Ca.phs', 'ISS', 3, {}, inherit_phase, inherit_phase, ISS_code_AD, ISS_code_EG, ISS_code_350, {}, ISS_inh_comp_args, ISS_inh_comp_args)
    default_phases += generate_expandables('Illite-Smectites/ISSS/ISSS R%d Ca.phs', 'ISSS', 2, {}, inherit_phase, inherit_phase, ISSS_code_AD, ISSS_code_EG, ISSS_code_350, {}, ISSS_inh_comp_args, ISSS_inh_comp_args)
    C_code = 'C   '
    C_inh_comp_args = {'C   ': dict(linked_with='C   ', **inherit_all)}
    ...
    default_phases += generate_expandables('Chlorite-Smectites/CS/CS R%d Ca.phs', 'CS', 4, {}, inherit_phase, inherit_phase, CS_code_AD, CS_code_EG, CS_code_350, {}, CS_inh_comp_args, CS_inh_comp_args)
    default_phases += generate_expandables('Chlorite-Smectites/CSS/CSS R%d Ca.phs', 'CSS', 3, {}, inherit_phase, inherit_phase, CSS_code_AD, CSS_code_EG, CSS_code_350, {}, CSS_inh_comp_args, CSS_inh_comp_args)
    default_phases += generate_expandables('Chlorite-Smectites/CSSS/CSSS R%d Ca.phs', 'CSSS', 2, {}, inherit_phase, inherit_phase, CSSS_code_AD, CSSS_code_EG, CSSS_code_350, {}, CSSS_inh_comp_args, CSSS_inh_comp_args)
    T_code = 'T   '
    T_inh_comp_args = {'T   ': dict(linked_with='T   ', **inherit_all)}
    ...
    default_phases += generate_expandables('Talc-Smectites/TS/TS R%d Ca.phs', 'TS', 4, {}, inherit_phase, inherit_phase, TS_code_AD, TS_code_EG, TS_code_350, {}, TS_inh_comp_args, TS_inh_comp_args)
    default_phases += generate_expandables('Talc-Smectites/TSS/TSS R%d Ca.phs', 'TSS', 3, {}, inherit_phase, inherit_phase, TSS_code_AD, TSS_code_EG, TSS_code_350, {}, TSS_inh_comp_args, TSS_inh_comp_args)
    default_phases += generate_expandables('Talc-Smectites/TSSS/TSSS R%d Ca.phs', 'TSSS', 2, {}, inherit_phase, inherit_phase, TSSS_code_AD, TSSS_code_EG, TSSS_code_350, {}, TSSS_inh_comp_args, TSSS_inh_comp_args)
        ...
        while True:
            try:
                ...
                for phase_kwargs, code, comp_props in phase_descr:
                    ...
                    limits = list(zip(list(range(0, len(code), code_length)), list(range(code_length, len(code) + 1, code_length))))
                    phase.components[:] = []
                    for ll, ul in limits:
                        part = code[ll:ul]
