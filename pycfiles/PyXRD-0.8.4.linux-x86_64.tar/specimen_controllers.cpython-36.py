...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyxrd/specimen/controllers/specimen_controllers.py
...
from pyxrd.generic.controllers.line_controllers import LinePropertiesController, BackgroundController, SmoothDataController, AddNoiseController, ShiftDataController, StripPeakController, CalculatePeakPropertiesController
from pyxrd.generic.views.line_views import BackgroundView, SmoothDataView, AddNoiseView, ShiftDataView, StripPeakView, CalculatePeakPropertiesView

class SpecimenController(DialogController, TreeViewMixin):
    __doc__ = '\n        Specimen controller.\n    '
    widget_handlers = {'custom': 'custom_handler'}
    ...
    def custom_handler(self, prop, widget):
        if prop.label in 'goniometer':
            ...
            ad = DummyAdapter(controller=self, prop=prop)
            return ad
    ...
    def setup_experimental_pattern_tree_view(self, store, widget):
        ...
        """
        setup_treeview(widget, store, on_cursor_changed=(self.on_exp_data_tv_cursor_changed),
          sel_mode='MULTIPLE')
    ...
    def setup_exclusion_ranges_tree_view(self, store, widget):
        ...
        """
        setup_treeview(widget, store, on_cursor_changed=(self.on_exclusion_ranges_tv_cursor_changed),
          sel_mode='MULTIPLE')

        def data_func(col, cell, model, iter, colnr):
            cell.set_property('text', '%g' % model.get(iter, colnr)[0])

        widget.append_column(new_text_column('From [°2θ]',
          text_col=(store.c_x), editable=True, data_func=(
         data_func, (store.c_x,)),
          edited_callback=(
         self.on_xy_data_cell_edited, (self.model.exclusion_ranges, 0)),
          resizable=True,
          expand=True))
        widget.append_column(new_text_column('To [°2θ]',
          text_col=(store.c_y), editable=True, data_func=(
         data_func, (store.c_y,)),
          edited_callback=(
         self.on_xy_data_cell_edited, (self.model.exclusion_ranges, 1)),
          resizable=True,
          expand=True))
    ...
    def update_calc_treeview(self, tv):
        ...
        for i in range(model.get_n_columns() - 2):
            tv.append_column(new_text_column((self.model.calculated_pattern.get_y_name(i)),
    ...
    def update_exp_treeview(self, tv):
        ...
        if n_columns > 2:
            for i in range(n_columns):
                tv.append_column(new_text_column((self.model.calculated_pattern.get_y_name(i)),
                  editable=True, edited_callback=(
                 self.on_xy_data_cell_edited, (self.model.experimental_pattern, i)),
                  data_func=(
                 get_num, (i,))))

        else:
            tv.append_column(new_text_column('°2θ',
              editable=True, data_func=(
             get_num, (model.c_x,)),
              edited_callback=(
             self.on_xy_data_cell_edited, (self.model.experimental_pattern, 0))))
            tv.append_column(new_text_column('Intensity',
              editable=True, data_func=(
             get_num, (model.c_y,)),
              edited_callback=(
             self.on_xy_data_cell_edited, (self.model.experimental_pattern, 1))))
    ...
    def on_import_exclusion_ranges_clicked(self, widget, data=None):
        ...
        def on_confirm(dialog):
            ...
            DialogFactory.get_load_dialog(title='Import exclusion ranges',
              parent=(self.view.get_top_widget()),
              filters=(exc_parsers.get_import_file_filters())).run(on_accept)

        DialogFactory.get_confirmation_dialog('Importing exclusion ranges will erase all current data.\nAre you sure you want to continue?',
    ...
    def on_export_exclusion_ranges_clicked(self, widget, data=None):
        ...
        DialogFactory.get_save_dialog('Select file for exclusion ranges export',
          parent=(self.view.get_top_widget()),
          filters=(exc_parsers.get_export_file_filters())).run(on_accept)
    ...
    def on_replace_experimental_data(self, *args, **kwargs):
        ...
        DialogFactory.get_load_dialog('Open XRD file for import',
          parent=(self.view.get_top_widget()), filters=(xrd_parsers.get_import_file_filters())).run(on_accept)
        return True
    ...
    def on_btn_import_experimental_data_clicked(self, widget, data=None):
        ...
        DialogFactory.get_confirmation_dialog('Importing a new experimental file will erase all current data.\nAre you sure you want to continue?',
          parent=(self.view.get_top_widget())).run(on_confirm)
        return True
    ...
    def _export_data(self, line):
        ...
        DialogFactory.get_save_dialog('Select file for export',
          parent=(self.view.get_top_widget()), filters=(xrd_parsers.get_export_file_filters()),
