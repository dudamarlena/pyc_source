...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyxrd/generic/plot/plotters.py
# Compiled at: 2020-03-07 03:51:50
# Size of source mod 2**32: 27321 bytes
import logging
logger = logging.getLogger(__name__)
import numpy as np, matplotlib, matplotlib.colors as mcolors, matplotlib.transforms as transforms
...
def plot_marker_text(project, marker, offset, marker_scale, base_y, axes):
    ...
    within_range = bool(project.axes_xlimit == 0 or marker.position >= project.axes_xmin and marker.position <= project.axes_xmax)
    if marker.visible and marker.style != 'offset' and within_range:
        ...
        if marker.top == 0:
            ...
            if text:
                ...
                text = Text(**kws)
        if text not in axes.get_children():
    ...
    for marker in specimen.markers:
        ...
        if marker.base == 1:
            ...
            if marker.base == 2:
                ...
                if marker.base == 3:
                    base_y = min(specimen.experimental_pattern.get_plotted_y_at_x(marker.position), specimen.calculated_pattern.get_plotted_y_at_x(marker.position))
                else:
                    if marker.base == 4:
                        base_y = max(specimen.experimental_pattern.get_plotted_y_at_x(marker.position), specimen.calculated_pattern.get_plotted_y_at_x(marker.position))
...
def plot_hatches(project, specimen, offset, scale, axes):
    ...
    leftborder, hatch, rightborder = getattr(specimen, '__plot_hatches_artists', (None,
                                                                                  None,
                                                                                  None))
    if leftborder:
        try:
...
def plot_label(specimen, labels, label_offset, plot_left, axes):
    ...
    if text:
        for key in props:
            getattr(text, 'set_%s' % key)(props[key])
...
def apply_transform(data, scale=1, offset=0, cap=0):
    ...
    if cap > 0:
        np.copyto(data_y, [cap], where=(data_y >= cap))
    data_y = data_y * scale + offset
...
def plot_pattern(pattern, axes, scale=1, offset=0, cap=0, z_data=[None], **kwargs):
    if len(z_data) > 1 and pattern.data_y.size > 0:
        ...
        X = np.ones(shape=(rows, cols), dtype=float) * angles
        Y = (np.ones(shape=(cols, rows), dtype=float) * rh).transpose()
        ...
        if kwargs:
            line.update(kwargs)
        line.update(dict(data=apply_transform((pattern.get_xy_data()), scale=scale, offset=offset, cap=cap),
          color=(pattern.color),
...
def plot_specimen(cc, project, specimen, labels, marker_lbls, label_offset, plot_left, offset, scale, marker_scale, axes):
    """
    ...
    if specimen.display_experimental:
        ...
        if pattern.bg_type == 0:
            if pattern.bg_position != 0.0:
                bg_line.update(dict(data=apply_transform(([xmin, xmax], [pattern.bg_position, pattern.bg_position]), scale=scale, offset=offset),
                  visible=True))
        elif pattern.bg_type == 1:
            if pattern.bg_pattern is not None:
                bg_line.update(dict(data=apply_transform((x_data, pattern.bg_pattern * pattern.bg_scale + pattern.bg_position), scale=scale, offset=offset),
                  visible=True))
        else:
            bg_line.update(dict(data=([], []), visible=True))
        if bg_line.get_visible():
            if bg_line not in axes.get_lines():
                axes.add_line(bg_line)
        if not bg_line.get_visible():
            try:
                bg_line.remove()
            except:
                pass
        ...
        if int(pattern.smooth_degree) > 1:
            data = (
             x_data, smooth(y_data, pattern.smooth_degree))
        else:
            data = ([], [])
        smooth_line.update(dict(data=apply_transform(data, scale=scale, offset=offset),
          visible=(bool(pattern.smooth_degree > 1))))
        if smooth_line.get_visible():
            if smooth_line not in axes.get_lines():
                axes.add_line(smooth_line)
        if not smooth_line.get_visible():
            try:
                smooth_line.remove()
            except:
                pass
        ...
        if pattern.noise_fraction > 0.0:
            data = (
             x_data, add_noise(y_data, pattern.noise_fraction))
        else:
            data = ([], [])
        noise_line.update(dict(data=apply_transform(data, scale=scale, offset=offset),
          visible=(bool(pattern.noise_fraction > 0.0))))
        if noise_line.get_visible():
            if noise_line not in axes.get_lines():
                axes.add_line(noise_line)
        if not noise_line.get_visible():
            try:
                noise_line.remove()
            except:
                pass
        ...
        if pattern.shift_value != 0.0:
            shifted_line.update(dict(data=apply_transform((x_data - pattern._shift_value, y_data.copy()), scale=scale, offset=offset),
              visible=True))
            position = specimen.goniometer.get_2t_from_nm(pattern.shift_position)
            reference_line.update(dict(data=apply_transform(([position, position], [0, ymax]), scale=scale, offset=offset),
              visible=True))
            if shifted_line not in axes.get_lines():
                axes.add_line(shifted_line)
            if reference_line not in axes.get_lines():
            ...
            if pattern.strip_startx != 0.0 and pattern.strip_endx != 0.0:
                ...
                stripped_line.update(dict(data=apply_transform((strip_xdata.copy(), strip_ydata.copy()), scale=scale, offset=offset),
                  visible=True))
                if stripped_line not in axes.get_lines():
        ...
        if peak_area is not None:
            if peak_area in axes.get_children():
                peak_area.remove()
        if peak_fwhm_line is not None:
            if peak_fwhm_line in axes.get_children():
                peak_fwhm_line.remove()
        if pattern.peak_startx != 0.0:
            if pattern.peak_endx != 0.0:
                if pattern.peak_properties_pattern is not None:
    ...
    if specimen.display_calculated:
        ...
        if specimen.display_phases:
            ...
            for i in range(2, pattern.num_columns):
                ...
                phase_line.update(dict(data=apply_transform(phase_data, scale=scale, offset=offset),
                  color=phase_color,
                  linewidth=(pattern.lw)))
                axes.add_line(phase_line)

            specimen.__plot_phase_lines = phase_lines
    if hasattr(specimen, 'mineral_preview'):
        if specimen.mineral_preview is not None:
            ...
            for position, intensity in peaks:
                position = specimen.goniometer.get_2t_from_nm(position / 10.0)
                intensity /= 100.0
                trans = transforms.blended_transform_factory(axes.transData, axes.transAxes)
                ymin, ymax = axes.get_ybound()
                style = 'solid'
...
def plot_statistics(project, specimen, spec_scale, stats_y_pos, stats_height, axes):
    ...
    if specimen.display_residuals:
        if specimen.statistics.residual_pattern is not None:
            plot_pattern_middle((specimen.statistics.residual_pattern),
              axes,
              height=stats_height, vscale=(specimen.display_residual_scale),
              offset=stats_y_pos,
              alpha=0.75)
    if specimen.display_derivatives:
        for pattern in (specimen.statistics.der_residual_pattern,
         specimen.statistics.der_exp_pattern,
         specimen.statistics.der_calc_pattern):
            if pattern is not None:
                plot_pattern_middle(pattern,
                  axes, height=stats_height, vscale=(specimen.display_residual_scale),
                  offset=stats_y_pos,
                  alpha=0.65)
...
def plot_specimens(axes, pos_setup, cc, project, specimens):
    ...
    for _, specimen in enumerate(specimens):
        ...
        if project.layout_mode == 'FULL' and (specimen.display_residuals or specimen.display_derivatives):
            ...
            plot_statistics(project, specimen, spec_scale, stats_y_pos, stats_height, axes)
        plot_specimen(cc, project, specimen, labels, marker_lbls, lbl_y_pos, pos_setup.left, spec_y_pos, spec_scale, scale_unit, axes)
        group_counter += 1
...
def plot_mixtures(axes, project, mixtures):
    ...
    for mixture in mixtures:
        ...
        for i, (phase, fraction) in enumerate(zip(mixture.phases, mixture.fractions)):
            ...
            phase_children = [create_rect_patch(fc=(phase.display_color)) for phase in mixture.phase_matrix[:, i].flat if phase is not None]
            phase_children.insert(0, label)
            legend_items.append(HPacker(children=phase_children, align='center', pad=0, sep=3))

        legends.append(VPacker(children=legend_items, align='right', pad=0, sep=3))

    if legends:
        legend = AnchoredOffsetbox(loc=1,
          pad=0.1,
          borderpad=0.1,
          frameon=False,
