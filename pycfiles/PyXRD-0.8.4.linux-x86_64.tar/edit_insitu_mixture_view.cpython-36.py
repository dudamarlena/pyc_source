...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyxrd/mixture/views/edit_insitu_mixture_view.py
...
class EditInSituMixtureView(BaseView):
    ...
    def reset_view(self):
        ...
        self.scale_inputs = []
        self.bgs_inputs = []
        self.phase_combos = np.empty(shape=(0, 0), dtype=(np.object_))
        self.behav_combos = np.empty(shape=(0, 0), dtype=(np.object_))
    ...
    def set_edit_view(self, view):
        if self._on_sr_id is not None:
            if self.child_view is not None:
    ...
    def add_phase_slot(self, phase_store, behav_store, del_phase_callback, label_callback, fraction_callback, phase_visible_callback, phase_combo_callback, behav_visible_callback, behav_combo_callback, label, fraction, phases, behavs):
        ...
        for col in range(c - self.base_width):
            ...
            self._add_new_phase_behav_combo(mrow, mcol, phase_store, phase_store.c_name, phases[(mrow, mcol)], phase_visible_callback, phase_combo_callback, behav_store, behav_store.c_name, behavs[(mrow, mcol)], behav_visible_callback, behav_combo_callback)

        self.wrapper.show_all()

    def add_specimen_slot(self, phase_store, behav_store, specimen_store, del_specimen_callback, scale_callback, bgs_callback, specimen_callback, phase_visible_callback, phase_combo_callback, behav_visible_callback, behav_combo_callback, scale, bgs, specimen, phases, behavs):
        r, c = self.matrix.get_property('n_rows'), self.matrix.get_property('n_columns')
    ...
    def _get_new_input(self, text='', width=7, callback=None):
        ...
        if callback is not None:
            new_input.connect('changed', callback)
        return new_input

    def _add_new_phase_behav_combo(self, r, c, phase_model, phase_text_column, default_phase, phase_visible_callback, phase_callback, behav_model, behav_text_column, default_behav, behav_visible_callback, behav_callback):
        """
