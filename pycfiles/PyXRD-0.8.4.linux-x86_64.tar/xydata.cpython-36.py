...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/mvc/models/xydata.py
# Compiled at: 2020-03-07 03:51:49
# Size of source mod 2**32: 10874 bytes
import logging
logger = logging.getLogger(__name__)
import types, json, numpy as np
...
class XYData(Model):
    ...
    def __init__(self, *args, **kwargs):
        ...
        my_kwargs = pop_kwargs(kwargs, 'names', 'data', *[prop.label for prop in type(self).Meta.get_local_persistent_properties()])
    ...
    def _serialize_data(self):
        ...
        return '[' + ','.join(['[' + ','.join(['%f' % val for val in row]) + ']' for row in conc]) + ']'
    ...
    def remove_from_indeces(self, *indeces):
        if indeces != []:
            ...
            for index in indeces:
                self.set_data(np.delete((self.data_x), index, axis=0), np.delete((self.data_y), index, axis=0))
    ...
    def __getitem__(self, index):
        return (
         self.data_x[index], self.data_y[index].tolist())
