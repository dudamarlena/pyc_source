...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyxrd/phases/controllers/add_phase_controller.py
...
class AddPhaseController(DialogController):
    ...
    def __init__(self, model=None, view=None, parent=None, callback=None):
        super(AddPhaseController, self).__init__(model=model,
          view=view,
          parent=parent)
    ...
    def generate_combo(self):
        ...
        add_combo_text_column((self.view.phase_combo_box),
          text_col=0, sensitive_col=2)

    def reload_combo_model(self):
        cmb_model = create_treestore_from_directory(settings.DATA_REG.get_directory_path('DEFAULT_PHASES'))
        self.view.phase_combo_box.set_model(cmb_model)

    def on_btn_ok_clicked(self, event):
        self.view.hide()
        self.callback(self.view.get_phase_type(), self.view.get_G(), self.view.get_R())
        return True
