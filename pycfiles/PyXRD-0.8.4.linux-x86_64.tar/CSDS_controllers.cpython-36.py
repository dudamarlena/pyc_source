...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyxrd/phases/controllers/CSDS_controllers.py
    ...
    def reset_distributions_controller(self):
        if self.view is not None:
            if self.distributions_controller is None:
                self.distributions_controller = EditCSDSDistributionController(model=(self.model.CSDS_distribution),
                  view=(self.view),
                  parent=self)
            else:
...
class EditCSDSDistributionController(BaseController):
        ...
        if self.view is not None:
            ...
            for prop in self.model.Meta.all_properties:
                if getattr(prop, 'refinable', False):
                    self.view.add_param_widget(self.view.widget_format % prop.label, prop.label, prop.minimum, prop.maximum)
    ...
    def notif_updated(self, model, prop_name, info):
        if self.model.distrib is not None:
            if not self.model.phase.project.before_needs_update_lock:
                try:
                    self.view.update_figure(self.model.distrib[0])
