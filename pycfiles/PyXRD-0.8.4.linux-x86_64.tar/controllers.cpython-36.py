...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyxrd/project/controllers.py
# Compiled at: 2020-03-07 03:51:50
# Size of source mod 2**32: 14593 bytes
import logging
...
class ProjectController(ObjectListStoreController):
    ...
    def setup_treeview(self, widget):
        ...
        col = new_text_column('Name', text_col=(store.c_name),
          min_width=125,
          xalign=0.0,
        ...
        def setup_check_column(title, colnr):
            col = new_toggle_column(title, toggled_callback=(
             self.specimen_tv_toggled, (store, colnr)),
    ...
    def import_multiple_specimen(self):
        ...
        def on_accept(dialog):
            ...
            window = DialogFactory.get_custom_dialog(task,
            ...
            def load_specimens(stop=None):
                for filename in filenames:
                    ...
                    with DialogFactory.error_dialog_handler(message,
                      (self.view.get_top_widget()), title='Failed to load file',
                      reraise=False):
        ...
        DialogFactory.get_load_dialog(title='Select XRD files for import', filters=(xrd_parsers.get_import_file_filters()),
          parent=(self.view.get_top_widget()),
    ...
    def delete_selected_specimens(self):
        ...
        if selection is not None:
            if len(selection) >= 1:
                ...
                DialogFactory.get_confirmation_dialog(message='Deleting a specimen is irreversible!\nAre You sure you want to continue?',
    ...
    def remove_backgrounds(self, specimens):
        ...
        def on_automated(dialog):
            for specimen in specimens:
                if specimen not in self.model.specimens:
                    raise ValueError('Specimen `%s` is not part of this Project!' % specimen)
                else:
                    specimen.experimental_pattern.bg_type = 0
        ...
        DialogFactory.get_confirmation_dialog('Do you want to perform an automated linear background subtraction?',
    ...
    def specimen_tv_button_press(self, tv, event):
        ...
        if event.button == 3:
            if specimen is not None:
                if specimen not in current_specimens:
        ...
        if event.type == Gdk.EventType._2BUTTON_PRESS:
            if specimen is not None:
                if getattr(col, 'colnr') == self.treemodel.c_name:
                    self.parent.on_edit_specimen_activate(event)
                    return True
        if (event.button == 1 or event.type == Gdk.EventType._2BUTTON_PRESS) and specimen is not None:
            ...
            if column in (self.treemodel.c_display_experimental,
             ...
             self.treemodel.c_display_phases):
                if column == self.treemodel.c_display_experimental:
                    ...
                    if column == self.treemodel.c_display_calculated:
