...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyxrd/probabilities/models/R0models.py
...
def R0_model_generator(pasG):
    ...
    class _BaseR0Model(object):
        ...
        def __init__(self, *args, **kwargs):
            ...
            (super(_BaseR0Model, self).__init__)(args, R=0, **kwargs)
    ...
    for g in range(pasG - 1):
        ...
        math_text = '$\\large\\frac{W_{%(g)d}}{\\sum_{i=%(g)d}^{%(G)d} W_i}$' % {'g':g + 1,  'G':pasG}
        inh_flag = 'inherit_F%d' % (g + 1)
        inh_from = 'parent.based_on.probabilities.F%d' % (g + 1)
        set_attribute(label, FloatProperty(default=0.8,
          text=text,
          math_text=math_text,
          refinable=True,
          persistent=True,
        ...
        set_attribute(label, BoolProperty(default=False,
          text=text,
          refinable=False,
          persistent=True,
          visible=True,
