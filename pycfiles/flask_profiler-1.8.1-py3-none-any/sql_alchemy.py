...
# [GCC 8.4.0]
# Embedded file name: /Users/muatik/projects/flask-profiler/flask_profiler/storage/sql_alchemy.py
# Compiled at: 2018-10-07 10:00:50
import json
...
class Measurements(base):
    ...
    def __repr__(self):
        return ('<Measurements {}, {}, {}, {}, {}, {}, {}, {}, {}>').format(self.id, self.startedAt, self.endedAt, self.elapsed, self.method, self.args, self.kwargs, self.name, self.context)
...
class Sqlalchemy(BaseStorage):
    ...
    def insert(self, kwds):
        ...
        session.add(Measurements(endedAt=endedAt, startedAt=startedAt, elapsed=elapsed, args=args, kwargs=kwargs, context=context, method=method, name=name))
        session.commit()
        return
    ...
    def filter(self, kwds={}):
        ...
        sql = ('SELECT * FROM flask_profiler_measurements {conditions}\n        order by {sort_field} {sort_direction}\n        limit {limit} OFFSET {skip} ').format(conditions=conditions, sort_field=f['sort'][0], sort_direction=f['sort'][1], limit=f['limit'], skip=f['skip'])
        session = sessionmaker(self.db)()
