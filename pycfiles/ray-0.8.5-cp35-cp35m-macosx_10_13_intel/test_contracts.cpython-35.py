...
# [GCC 8.4.0]
# Embedded file name: /private/var/folders/nz/vv4_9tw56nv9k3tkvyszvwg80000gn/T/pip-unpacked-wheel-c6tqtva7/psutil/tests/test_contracts.py
# Compiled at: 2020-05-06 17:45:43
# Size of source mod 2**32: 25355 bytes
"""Contracts tests. These tests mainly check API sanity in terms of
returned types and APIs availability.
Some of these are duplicates of tests test_system.py and test_process.py
"""
import errno, os, stat, time, traceback
...
class TestAvailSystemAPIs(unittest.TestCase):
    ...
    def test_cpu_freq(self):
        linux = LINUX and (os.path.exists('/sys/devices/system/cpu/cpufreq') or os.path.exists('/sys/devices/system/cpu/cpu0/cpufreq'))
        self.assertEqual(hasattr(psutil, 'cpu_freq'), linux or MACOS or WINDOWS or FREEBSD)
...
class TestAvailProcessAPIs(unittest.TestCase):
    ...
    def test_environ(self):
        self.assertEqual(hasattr(psutil.Process, 'environ'), LINUX or MACOS or WINDOWS or AIX or SUNOS)
    ...
    def test_memory_maps(self):
        ...
        self.assertEqual(hasit, False if OPENBSD or NETBSD or AIX or MACOS else True)


class TestSystemAPITypes(unittest.TestCase):
    __doc__ = 'Check the return types of system related APIs.\n    Mainly we want to test we never return unicode on Python 2, see:\n    https://github.com/giampaolo/psutil/issues/1039\n    '
    ...
    def test_net_if_addrs(self):
        for ifname, addrs in psutil.net_if_addrs().items():
            ...
            for addr in addrs:
                if enum is not None:
                    if not isinstance(addr.family, enum.IntEnum):
                        raise AssertionError(addr)
                elif not isinstance(addr.family, int):
                    raise AssertionError(addr)
...
class TestFetchAllProcesses(unittest.TestCase):
    __doc__ = "Test which iterates over all running processes and performs\n    some sanity checks against Process API's returned values.\n    "
    ...
    def test_fetch_all(self):
        ...
        for p, name in self.iter_procs():
            ...
            try:
                ...
                msg = '%r was skipped because not implemented' % (self.__class__.__name__ + '.test_' + name)
                warn(msg)
    ...
    def exe(self, ret, proc):
        ...
        if POSIX and os.path.isfile(ret) and hasattr(os, 'access') and hasattr(os, 'X_OK'):
    ...
    def cwd(self, ret, proc):
        if ret:
            ...
            try:
                ...
                if WINDOWS and err.errno in psutil._psplatform.ACCESS_DENIED_SET:
                    pass
