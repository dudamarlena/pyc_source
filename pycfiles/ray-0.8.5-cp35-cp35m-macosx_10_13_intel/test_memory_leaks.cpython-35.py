...
# [GCC 8.4.0]
# Embedded file name: /private/var/folders/nz/vv4_9tw56nv9k3tkvyszvwg80000gn/T/pip-unpacked-wheel-c6tqtva7/psutil/tests/test_memory_leaks.py
# Compiled at: 2020-05-06 17:45:43
# Size of source mod 2**32: 18499 bytes
"""
...
import functools, gc, os, sys, threading, time, psutil, psutil._common
...
def skip_if_linux():
    return unittest.skipIf(LINUX and SKIP_PYTHON_IMPL, 'worthless on LINUX (pure python)')


@unittest.skipIf(PYPY, 'unreliable on PYPY')
class TestMemLeak(unittest.TestCase):
    __doc__ = 'Base framework class which calls a function many times and\n    produces a failure if process memory usage keeps increasing\n    between calls or over time.\n    '
...
class TestProcessObjectLeaks(TestMemLeak):
    __doc__ = 'Test leaks of Process class methods.'
    proc = thisproc

    def test_coverage(self):
        skip = set(('pid', 'as_dict', 'children', 'cpu_affinity', 'cpu_percent', 'ionice',
                    'is_running', 'kill', 'memory_info_ex', 'memory_percent', 'nice',
                    'oneshot', 'parent', 'parents', 'rlimit', 'send_signal', 'suspend',
                    'terminate', 'wait'))
...
class TestTerminatedProcessLeaks(TestProcessObjectLeaks):
    __doc__ = "Repeat the tests above looking for leaks occurring when dealing\n    with terminated processes raising NoSuchProcess exception.\n    The C functions are still invoked but will follow different code\n    paths. We'll check those code paths.\n    "
...
class TestModuleFunctionsLeaks(TestMemLeak):
    __doc__ = 'Test leaks of psutil module functions.'

    def test_coverage(self):
        skip = set(('version_info', '__version__', 'process_iter', 'wait_procs', 'cpu_percent',
                    'cpu_times_percent', 'cpu_count'))
    ...
    def test_net_if_addrs(self):
        self.execute(psutil.net_if_addrs, tolerance_=81920 if WINDOWS else None)
