...
# [GCC 8.4.0]
# Embedded file name: /private/var/folders/nz/vv4_9tw56nv9k3tkvyszvwg80000gn/T/pip-unpacked-wheel-c6tqtva7/psutil/tests/test_process.py
# Compiled at: 2020-05-06 17:45:43
# Size of source mod 2**32: 63924 bytes
"""Tests for psutil.Process class."""
import collections, errno, getpass, itertools, os, signal, socket, subprocess, sys, tempfile, textwrap, time, types, psutil
...
class TestProcess(unittest.TestCase):
    __doc__ = 'Tests for psutil.Process class.'
    ...
    def test_ionice_linux(self):
        ...
        try:
            ...
            self.assertRaisesRegex(ValueError, 'ioclass accepts no value', p.ionice, psutil.IOPRIO_CLASS_NONE, 1)
            self.assertRaisesRegex(ValueError, 'ioclass accepts no value', p.ionice, psutil.IOPRIO_CLASS_IDLE, 1)
            self.assertRaisesRegex(ValueError, "'ioclass' argument must be specified", p.ionice, value=1)
        finally:
            p.ionice(psutil.IOPRIO_CLASS_BE)
    ...
    def test_ionice_win(self):
        ...
        try:
            ...
            self.assertRaisesRegex(TypeError, 'value argument not accepted on Windows', p.ionice, psutil.IOPRIO_NORMAL, value=1)
            self.assertRaisesRegex(ValueError, 'is not a valid priority', p.ionice, psutil.IOPRIO_HIGH + 1)
        finally:
    ...
    def test_rlimit(self):
        ...
        try:
            ...
            with self.assertRaises(IOError) as (exc):
                with open(TESTFN, 'wb') as (f):
                    f.write(b'X' * 1025)
            self.assertEqual(exc.exception.errno if PY3 else exc.exception[0], errno.EFBIG)
        finally:
    ...
    def test_exe(self):
        ...
        try:
            ...
            if WINDOWS and len(exe) == len(PYTHON_EXE):
                ...
                try:
                    self.assertEqual(exe.replace(ver, ''), PYTHON_EXE.replace(ver, ''))
                except AssertionError:
                    pass
    ...
    def test_cmdline(self):
        cmdline = [
         PYTHON_EXE, '-c', 'import time; time.sleep(60)']
        sproc = get_test_subprocess(cmdline)
        try:
            self.assertEqual(' '.join(psutil.Process(sproc.pid).cmdline()), ' '.join(cmdline))
        except AssertionError:
            if NETBSD or OPENBSD or AIX:
                self.assertEqual(psutil.Process(sproc.pid).cmdline()[0], PYTHON_EXE)
            else:
                raise
    ...
    def test_long_name(self):
        long_name = TESTFN + '01234567890123456789'
    ...
    def test_prog_w_funky_name(self):
        ...
        self.assertEqual(os.path.normcase(p.exe()), os.path.normcase(funky_path))
            ...
            try:
                try:
                    if hasattr(os, 'getpriority'):
                        self.assertEqual(os.getpriority(os.PRIO_PROCESS, os.getpid()), p.nice())
                    p.nice(1)
    ...
    def test_cpu_affinity(self):
        ...
        for n in all_cpus if not TRAVIS else initial:
            ...
            if hasattr(os, 'sched_getaffinity'):
                self.assertEqual(p.cpu_affinity(), list(os.sched_getaffinity(p.pid)))
    ...
    def test_parent_disappeared(self):
        sproc = get_test_subprocess()
        p = psutil.Process(sproc.pid)
        with mock.patch('psutil.Process', side_effect=psutil.NoSuchProcess(0, 'foo')):
    ...
    def test_as_dict(self):
        ...
        with mock.patch('psutil.Process.nice', create=True, side_effect=psutil.AccessDenied):
            self.assertEqual(p.as_dict(attrs=['nice'], ad_value=1), {'nice': 1})
        with mock.patch('psutil.Process.nice', create=True, side_effect=psutil.NoSuchProcess(p.pid, 'name')):
            self.assertRaises(psutil.NoSuchProcess, p.as_dict, attrs=['nice'])
        with mock.patch('psutil.Process.nice', create=True, side_effect=psutil.ZombieProcess(p.pid, 'name')):
            self.assertEqual(p.as_dict(attrs=['nice'], ad_value='foo'), {'nice': 'foo'})
    ...
    def test_halfway_terminated_process(self):
        ...
        excluded_names = [
         'pid', 'is_running', 'wait', 'create_time',
            ...
            if not name.startswith('_'):
                if name in excluded_names:
                    ...
                    try:
                        ...
                        self.fail('ZombieProcess for %r was not supposed to happen' % name)
                        ...
                        if WINDOWS and name in ('exe', 'name'):
                            ...
                            if name == 'exe':
                                ...
                                self.assertEqual(normcase(ret), normcase(os.path.basename(PYTHON_EXE)))
                                continue
                                self.fail('NoSuchProcess exception not raised for %r, retval=%s' % (
                                 name, ret))
    ...
    def test_zombie_process(self):
        ...
        if hasattr(zproc, 'rlimit'):
            ...
            succeed_or_zombie_p_exc(zproc.rlimit, psutil.RLIMIT_NOFILE, (5, 5))
    ...
    def test_zombie_process_status_w_exc(self):
        p = psutil.Process()
        with mock.patch('psutil._psplatform.Process.status', side_effect=psutil.ZombieProcess(0)) as (m):
    ...
    def test_environ(self):
        ...
        def clean_dict(d):
            ...
            return dict([(k.replace('\r', '').replace('\n', ''), v.replace('\r', '').replace('\n', '')) for k, v in d.items()])
    ...
    def test_weird_environ(self):
        ...
        sproc = get_test_subprocess([path], stdin=subprocess.PIPE, stderr=subprocess.PIPE)
        p = psutil.Process(sproc.pid)
        wait_for_pid(p.pid)
...
if POSIX and os.getuid() == 0:
    ...
    class LimitedUserTestCase(TestProcess):
        __doc__ = 'Repeat the previous tests by using a limited user.\n        Executed only on UNIX and only if the user who run the test script\n        is root.\n        '
...
class TestPopen(unittest.TestCase):
    __doc__ = 'Tests for psutil.Popen class.'
    ...
    def test_misc(self):
        cmd = [
         PYTHON_EXE, '-c', 'import time; time.sleep(60);']
        with psutil.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE) as (proc):
            proc.name()
            proc.cpu_times()
            proc.stdin
    ...
    def test_ctx_manager(self):
        with psutil.Popen([PYTHON_EXE, '-V'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.PIPE) as (proc):
            proc.communicate()
