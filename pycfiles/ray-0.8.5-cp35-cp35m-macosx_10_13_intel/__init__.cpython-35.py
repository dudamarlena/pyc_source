...
# [GCC 8.4.0]
# Embedded file name: /private/var/folders/nz/vv4_9tw56nv9k3tkvyszvwg80000gn/T/pip-unpacked-wheel-c6tqtva7/psutil/__init__.py
# Compiled at: 2020-05-06 17:45:43
# Size of source mod 2**32: 87126 bytes
...
import collections, contextlib, datetime, functools, os, signal, subprocess, sys, threading, time
try:
    import pwd
except ImportError:
    pwd = None
...
if LINUX:
    ...
    if WINDOWS:
        ...
        if int(__version__.replace('.', '')) != getattr(_psplatform.cext, 'version', None):
            msg = 'version conflict: %r C extension module was built for another version of psutil' % getattr(_psplatform.cext, '__file__')
            if hasattr(_psplatform.cext, 'version'):
                msg += ' (%s instead of %s)' % (
                 '.'.join([x for x in str(_psplatform.cext.version)]), __version__)
            else:
                msg += ' (different than %s)' % __version__
            msg += "; you may try to 'pip uninstall psutil', manually remove %s" % getattr(_psplatform.cext, '__file__', 'the existing psutil install directory')
    ...
    class Process(object):
        ...
        def _init(self, pid, _ignore_nsp=False):
            if pid is None:
                pid = os.getpid()
            elif not _PY3 and not isinstance(pid, (int, long)):
        ...
        *memtype* argument is a string that dictates what type of
        ...
        """
            ...
            fun = self.memory_info if memtype in _psplatform.pmem._fields else self.memory_full_info
            metrics = fun()
            value = getattr(metrics, memtype)
            total_phymem = _TOTAL_PHYMEM or virtual_memory().total
            if not total_phymem > 0:
                raise ValueError("can't calculate process memory percent because total physical system memory is not positive (%r)" % total_phymem)
        ...
        if POSIX:
            ...
            def _send_signal(self, sig):
                ...
                if self.pid == 0:
                    raise ValueError('preventing sending signal to process with PID 0 as it would affect every process in the process group of the calling process (os.getpid()) instead of PID 0')
                try:
    ...
    class Popen(Process):
        __doc__ = 'A more convenient interface to stdlib subprocess.Popen class.\n    It starts a sub process and deals with it exactly as when using\n    subprocess.Popen class but in addition also provides all the\n    properties and methods of psutil.Process class as a unified\n    interface:\n\n      >>> import psutil\n      >>> from subprocess import PIPE\n      >>> p = psutil.Popen(["python", "-c", "print \'hi\'"], stdout=PIPE)\n      >>> p.name()\n      \'python\'\n      >>> p.uids()\n      user(real=1000, effective=1000, saved=1000)\n      >>> p.username()\n      \'giampaolo\'\n      >>> p.communicate()\n      (\'hi\n\', None)\n      >>> p.terminate()\n      >>> p.wait(timeout=2)\n      0\n      >>>\n\n    For method names common to both classes such as kill(), terminate()\n    and wait(), psutil.Process implementation takes precedence.\n\n    Unlike subprocess.Popen this class pre-emptively checks whether PID\n    has been reused on send_signal(), terminate() and kill() so that\n    you don\'t accidentally terminate another process, fixing\n    http://bugs.python.org/issue6973.\n\n    For a complete documentation refer to:\n    http://docs.python.org/3/library/subprocess.html\n    '

        def __init__(self, *args, **kwargs):
            self._Popen__subproc = subprocess.Popen(*args, **kwargs)
            self._init(self._Popen__subproc.pid, _ignore_nsp=True)

        def __dir__(self):
            return sorted(set(dir(Popen) + dir(subprocess.Popen)))

        def __enter__(self):
            if hasattr(self._Popen__subproc, '__enter__'):
                self._Popen__subproc.__enter__()
            return self

        def __exit__(self, *args, **kwargs):
            if hasattr(self._Popen__subproc, '__exit__'):
                return self._Popen__subproc.__exit__(*args, **kwargs)
            if self.stdout:
                self.stdout.close()
            if self.stderr:
                self.stderr.close()
        ...
        def __getattribute__(self, name):
            try:
                ...
                try:
                    return object.__getattribute__(self._Popen__subproc, name)
                except AttributeError:
                    raise AttributeError("%s instance has no attribute '%s'" % (
                     self.__class__.__name__, name))

        def wait(self, timeout=None):
            if self._Popen__subproc.returncode is not None:
                return self._Popen__subproc.returncode
            ret = super(Popen, self).wait(timeout)
            self._Popen__subproc.returncode = ret
            return ret


    _as_dict_attrnames = set([x for x in dir(Process) if not x.startswith('_') and x not in ('send_signal',
                                                                                             'suspend',
                                                                                             'resume',
                                                                                             'terminate',
                                                                                             'kill',
    ...
    *attrs* and *ad_value* have the same meaning as in
    ...
    """
        ...
        with _lock:
            ls = sorted(list(_pmap.items()) + list(dict.fromkeys(new_pids).items()))
        for pid, proc in ls:
            try:
                if proc is None:
                    yield add(pid)
    ...
    *timeout* occurs.
    ...
    """
        tot = sum(times)
        if LINUX:
            tot -= getattr(times, 'guest', 0)
            tot -= getattr(times, 'guest_nice', 0)
        return tot
    ...
    *interval* and *percpu* arguments have the same meaning as in
    ...
    disk_io_counters.cache_clear = functools.partial(_wrap_numbers.cache_clear, 'psutil.disk_io_counters')
    ...
    net_io_counters.cache_clear = functools.partial(_wrap_numbers.cache_clear, 'psutil.net_io_counters')
        ...
        for name, fam, addr, mask, broadcast, ptp in rawlist:
            if has_enums:
                try:
                    ...
                    elif hasattr(_psplatform, 'AF_LINK') and _psplatform.AF_LINK == fam:
                        fam = _psplatform.AF_LINK
    ...
    def test():
        ...
        print(templ % ('USER', 'PID', '%MEM', 'VSZ', 'RSS', 'NICE', 'STATUS', 'START',
                       'TIME', 'CMDLINE'))
