...
# [GCC 8.4.0]
# Embedded file name: /private/var/folders/nz/vv4_9tw56nv9k3tkvyszvwg80000gn/T/pip-unpacked-wheel-c6tqtva7/psutil/tests/test_windows.py
# Compiled at: 2020-05-06 17:45:43
# Size of source mod 2**32: 33666 bytes
"""Windows specific tests."""
import datetime, errno, glob, os, platform, re, signal, subprocess, sys, time, warnings, psutil
...
class TestCpuAPIs(TestCase):
    ...
    @unittest.skipIf('NUMBER_OF_PROCESSORS' not in os.environ, 'NUMBER_OF_PROCESSORS env var is not available')
...
class TestSystemAPIs(TestCase):
    ...
    def test_total_phymem(self):
        ...
        self.assertEqual(int(w.TotalPhysicalMemory), psutil.virtual_memory().total)
            ...
            for wmi_part in wmi_parts:
                if ps_part.device.replace('\\', '') == wmi_part.DeviceID:
                    ...
                    if 'cdrom' in ps_part.opts:
                        ...
                        if abs(usage.free - wmi_free) > 10485760:
    ...
    def test_disk_usage(self):
        for disk in psutil.disk_partitions():
            if 'cdrom' in disk.opts:
                ...
                self.assertAlmostEqual(sys_value[0], psutil_value.free, delta=1048576)
                self.assertAlmostEqual(sys_value[1], psutil_value.total, delta=1048576)
                self.assertEqual(psutil_value.used, psutil_value.total - psutil_value.free)

    def test_disk_partitions(self):
        sys_value = [x + '\\' for x in win32api.GetLogicalDriveStrings().split('\\\x00') if x and not x.startswith('A:')]
    ...
    def test_net_if_stats(self):
        ...
        self.assertTrue(ps_names & wmi_names, 'no common entries in %s, %s' % (ps_names, wmi_names))
...
class TestSensorsBattery(TestCase):
    ...
    def test_percent(self):
        ...
        self.assertAlmostEqual(battery_psutil.percent, battery_wmi.EstimatedChargeRemaining, delta=1)
    ...
    def test_power_plugged(self):
        ...
        battery_wmi = w.query('select * from Win32_Battery')[0]
        battery_psutil = psutil.sensors_battery()
        self.assertEqual(battery_psutil.power_plugged, battery_wmi.BatteryStatus == 2)

    def test_emulate_no_battery(self):
        with mock.patch('psutil._pswindows.cext.sensors_battery', return_value=(0,
                                                                                128,
                                                                                0,
                                                                                0)) as (m):
            self.assertIsNone(psutil.sensors_battery())
    ...
    def test_emulate_power_connected(self):
        with mock.patch('psutil._pswindows.cext.sensors_battery', return_value=(1,
                                                                                0,
                                                                                0,
                                                                                0)) as (m):
    ...
    def test_emulate_secs_left_unknown(self):
        with mock.patch('psutil._pswindows.cext.sensors_battery', return_value=(0,
                                                                                ...
                                                                                0,
                                                                                -1)) as (m):
            self.assertEqual(psutil.sensors_battery().secsleft, psutil.POWER_TIME_UNKNOWN)
            assert m.called
...
class TestProcess(TestCase):
    ...
    def test_num_handles_increment(self):
        ...
        handle = win32api.OpenProcess(win32con.PROCESS_QUERY_INFORMATION, win32con.FALSE, os.getpid())
        after = p.num_handles()
    ...
    def test_handles_leak(self):
        ...
        for name in dir(psutil.Process):
            if not name.startswith('_'):
                if name in ('terminate', 'kill', 'suspend', 'resume', 'nice', 'send_signal',
                            'wait', 'children', 'as_dict', 'memory_info_ex'):
                    continue
                else:
                    try:
                        call(p, name)
    ...
    @unittest.skipIf(not sys.version_info >= (2, 7), 'CTRL_* signals not supported')
    ...
    def test_username(self):
        self.assertEqual(psutil.Process().username(), win32api.GetUserNameEx(win32con.NameSamCompatible))
    ...
    def test_memory_info(self):
        handle = win32api.OpenProcess(win32con.PROCESS_QUERY_INFORMATION, win32con.FALSE, self.pid)
        self.addCleanup(win32api.CloseHandle, handle)
        sys_value = win32process.GetProcessMemoryInfo(handle)
        psutil_value = psutil.Process(self.pid).memory_info()
        self.assertEqual(sys_value['PeakWorkingSetSize'], psutil_value.peak_wset)
        self.assertEqual(sys_value['WorkingSetSize'], psutil_value.wset)
        self.assertEqual(sys_value['QuotaPeakPagedPoolUsage'], psutil_value.peak_paged_pool)
        self.assertEqual(sys_value['QuotaPagedPoolUsage'], psutil_value.paged_pool)
        self.assertEqual(sys_value['QuotaPeakNonPagedPoolUsage'], psutil_value.peak_nonpaged_pool)
        self.assertEqual(sys_value['QuotaNonPagedPoolUsage'], psutil_value.nonpaged_pool)
        self.assertEqual(sys_value['PagefileUsage'], psutil_value.pagefile)
        self.assertEqual(sys_value['PeakPagefileUsage'], psutil_value.peak_pagefile)
    ...
    def test_io_counters(self):
        ...
        self.assertEqual(psutil_value.read_count, sys_value['ReadOperationCount'])
        self.assertEqual(psutil_value.write_count, sys_value['WriteOperationCount'])
        self.assertEqual(psutil_value.read_bytes, sys_value['ReadTransferCount'])
        self.assertEqual(psutil_value.write_bytes, sys_value['WriteTransferCount'])
        self.assertEqual(psutil_value.other_count, sys_value['OtherOperationCount'])
        self.assertEqual(psutil_value.other_bytes, sys_value['OtherTransferCount'])

    def test_num_handles(self):
        import ctypes, ctypes.wintypes
        PROCESS_QUERY_INFORMATION = 1024
        handle = ctypes.windll.kernel32.OpenProcess(PROCESS_QUERY_INFORMATION, 0, self.pid)
...
class TestProcessWMI(TestCase):
    __doc__ = 'Compare Process API results with WMI.'
...
class RemoteProcessTestCase(TestCase):
    __doc__ = 'Certain functions require calling ReadProcessMemory.\n    This trivially works when called on the current process.\n    Check that this works on other processes, especially when they\n    have a different bitness.\n    '
    ...
    def setUp(self):
        ...
        self.proc32 = get_test_subprocess([self.python32] + self.test_args, env=env, stdin=subprocess.PIPE)
        self.proc64 = get_test_subprocess([self.python64] + self.test_args, env=env, stdin=subprocess.PIPE)
...
class TestServices(TestCase):
    ...
    def test_win_service_get(self):
        ERROR_SERVICE_DOES_NOT_EXIST = psutil._psplatform.cext.ERROR_SERVICE_DOES_NOT_EXIST
        ...
        if PY3:
            args = (
             0, 'msg', 0, ERROR_SERVICE_DOES_NOT_EXIST)
        else:
            args = (
             ERROR_SERVICE_DOES_NOT_EXIST, 'msg')
        exc = WindowsError(*args)
        with mock.patch('psutil._psplatform.cext.winservice_query_status', side_effect=exc):
            self.assertRaises(psutil.NoSuchProcess, service.status)
        with mock.patch('psutil._psplatform.cext.winservice_query_config', side_effect=exc):
            self.assertRaises(psutil.NoSuchProcess, service.username)
        if PY3:
            args = (
             0, 'msg', 0, ERROR_ACCESS_DENIED)
        else:
            args = (
             ERROR_ACCESS_DENIED, 'msg')
        exc = WindowsError(*args)
        with mock.patch('psutil._psplatform.cext.winservice_query_status', side_effect=exc):
