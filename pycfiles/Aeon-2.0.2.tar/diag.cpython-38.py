...
# Embedded file name: build/bdist.linux-x86_64/egg/aeolus/calc/diag.py
# Compiled at: 2020-05-01 02:20:41
# Size of source mod 2**32: 11555 bytes
__doc__ = 'Some commonly used diagnostics in atmospheric science.'
...
__all__ = ('bond_albedo', 'ghe_norm', 'heat_redist_eff', 'precip_sum', 'sfc_net_energy',
           'sfc_water_balance', 'toa_cloud_radiative_effect', 'toa_eff_temp', 'toa_net_energy',
           'water_path')
PRECIP_MAPPING = {'total':[
  'convective_rainfall_flux',
  'convective_snowfall_flux',
  'stratiform_rainfall_flux',
  'stratiform_snowfall_flux'], 
 'conv':[
  'convective_rainfall_flux', 'convective_snowfall_flux'], 
 'stra':[
  'stratiform_rainfall_flux', 'stratiform_snowfall_flux'], 
 'rain':[
  'convective_rainfall_flux', 'stratiform_rainfall_flux'], 
 'snow':[
  'convective_snowfall_flux', 'stratiform_snowfall_flux']}
...
def toa_net_energy(cubelist):
    ...
    if len(terms) != 3:
        raise MissingCubeError(f"{varnames} required for TOA energy balance are missing from cubelist:\n{cubelist}")
    terms_ave = []
    for cube in terms:
...
def sfc_net_energy(cubelist):
    ...
    terms = cubelist.extract(varnames)
    if len(terms) != 6:
        raise MissingCubeError(f"{varnames} required for SFC energy balance are missing from cubelist:\n{cubelist}")
    terms_ave = []
    for cube in terms:
        terms_ave.append(spatial(cube, 'mean'))

    sfc_net = terms_ave[0] - terms_ave[1] + terms_ave[2] - terms_ave[3] - terms_ave[4] - terms_ave[5]
...
def precip_sum(cubelist, ptype='total', const=None):
    ...
    try:
        ...
        if len(cubelist.extract(varnames)) == 0:
            raise MissingCubeError(f"{varnames} required for ptype={ptype} are missing from cubelist:\n{cubelist}")
        precip = 0.0
