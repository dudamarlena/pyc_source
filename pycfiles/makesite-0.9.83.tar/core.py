...
# [GCC 8.4.0]
# Embedded file name: /home/klen/Projects/makesite/makesite/core.py
# Compiled at: 2012-09-07 11:36:40
import logging, sys
...
LOGFILE_HANDLER = logging.FileHandler(mktemp('.log', 'ms.%s-' % datetime.now().strftime('%d.%m')))
...
class ColoredFormater(logging.Formatter):
    ...
    def format(self, record):
        ...
        if record.levelno == logging.WARN:
            return terminal.styled_text('[WARN] ' + s, terminal.BOLD, terminal.fg(terminal.BROWN))
        return terminal.styled_text(s, terminal.BOLD, terminal.fg(terminal.RED))
...
def action(*arguments, **options):
    parser = MakesiteArgsParser(description="'Makesite' easy control of project structure.", **options)
...
def get_base_modules():
    ...
    return sorted(filter(lambda x: op.isdir(op.join(MOD_DIR, x)), listdir(MOD_DIR)))


def get_base_templates():
    """ Get list of installed templates. """
    return sorted(filter(lambda x: op.isdir(op.join(TPL_DIR, x)), listdir(TPL_DIR)))
...
def gen_template_files(path):
    ...
    for root, _, files in walk(path):
        for f in filter(lambda x: x not in (TPLNAME, CFGNAME), files):
