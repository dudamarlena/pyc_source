...
# [GCC 8.4.0]
# Embedded file name: /home/klen/Projects/makesite/makesite/main.py
# Compiled at: 2012-09-07 11:38:03
import sys
...
@action((['-p', '--path'],
 dict(help='path to makesite sites instalation dir. you can set it in $makesite_home env variable.', required=not bool(settings.MAKESITE_HOME), default=settings.MAKESITE_HOME)))
def ls(args):
    """
    List sites
    ----------
...
def module(args):
    ...
    if not op.exists(mod):
        raise AssertionError('Not found module: %s' % args.MODULE)
        args.DEST = args.DEST.startswith(op.sep) or op.join(getcwd(), args.DEST)
...
@action((
 [
  'ACTION'],
 dict(help='Choose add or remove operation', choices=('add', 'remove'))), (
 [
...
@action((
 ...
 [
  '-r', '--repeat'],
 dict(action='store_true', default=False, help='Repeat installation.')), (
 [
  '-i', '--info'],
 dict(action='store_true', default=False, help='Show project install options and exit.')), (
 [
...
@action((
 ...
 [
  '-H'], dict(help='Host for run Makesite commands by ssh.', nargs='+', dest='host')), add_help=False)
def main(args=None):
    """ Base dispather """
    try:
        start = datetime.now()
        ...
        if args.host:
            ...
            func(args.argv)
            LOGGER.info('\nOPERATION SUCCESSFUL')
    except (AssertionError, CalledProcessError, gaierror) as e:
