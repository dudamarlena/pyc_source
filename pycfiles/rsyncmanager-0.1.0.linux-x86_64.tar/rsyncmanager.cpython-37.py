...
# [GCC 8.4.0]
# Embedded file name: /home/payno/.local/share/virtualenvs/tomwer_venv/lib/python3.7/site-packages/rsyncmanager/rsyncmanager.py
# Compiled at: 2020-05-05 04:54:40
# Size of source mod 2**32: 20084 bytes
"""
...
class RSyncManager(Borg, qt.QObject):
    ...
    def sync_files(self, sources, targets, wait=False, delete=False, callback=None, callback_parameters=None, parallel=False, verbose=False, rights=None):
        """sync a list of file
        ...
        if not len(sources) == len(targets):
            ...
            callback_obj = _MergeCallback(n_call=(len(sources)), callback=callback,
              callback_parameters=callback_parameters)
        else:
            callback_obj = None
        for source, target in zip(sources, targets):
            if callback_obj is not None:
                self.sync_file(source=source, target=target, wait=wait, delete=delete,
                  callback=(callback_obj.call),
    ...
    def sync_file(self, source, target, wait=False, delete=False, callback=None, callback_parameters=None, parallel=False, verbose=False, rights=None):
        """sync a file
        ...
        return self.sync_folder_raw(source=source, target=target,
          options=options,
          block=wait,
          callback=callback,
          callback_parameters=callback_parameters,
    ...
    def remove_sync_files(self, dir, files, block=False):
        ...
        _queue.add_action(action='rm', source=dir,
          files=_filesToRemove)
        if block is True or self._force_sync is True:
            self.rsyncQueues[(dir, None)].rsyncThread.wait()

    def sync_folder_raw(self, source, target, options, block=False, callback=None, callback_parameters=None, parallel=False):
    ...
    def _managed_finished_sync(self, source, target):
        if self.rsyncQueues[(source, target)].empty() is True:
            self.rsyncQueues[(source, target)].sigQueueFinished.disconnect(self._managed_finished_sync)
            del self.rsyncQueues[(source, target)]

    def _managed_finished_rm(self, source):
        if self.rsyncQueues[(source, None)].empty() is True:
            self.rsyncQueues[(source, None)].sigQueueFinished.disconnect(self._managed_finished_rm)
...
class SetQueue(Queue):
    ...
    def _get_id(self, thread):
        ...
        if isinstance(thread, _RmThread):
            return (
             thread.source, None, thread.files_to_rm)
...
class _RSyncThread(qt.QThread):
    ...
    def run(self):
        if not os.path.exists(self._source):
            ...
            return
        command = RSyncManager().get_rsync_command(source=(self._source),
          target=(self._target),
