...
# Size of source mod 2**32: 22746 bytes
...
class SuperSourceNode(ir.Module):
    ...
    def verify_mode_depths(self) -> None:
        ...
        for node in self.tpo_valid_node_gen():
            if self in node.parents:
                ...
                latency_table[node] = pulp.LpVariable(name=f"latency_{node.name}",
                  lowBound=0,
                  cat='Integer')
                lp_problem.extend(parent.get_latency(node) + latency_table[parent] <= latency_table[node] for parent in node.parents)
                lp_problem.extend(parent.get_latency(node) + latency_table[parent] + parent.fifo(node).depth >= latency_table[node] for parent in node.parents)

        lp_status = lp_problem.solve()
        if lp_status == pulp.LpStatusOptimal:
            _logger.debug('II=1 check: PASS')
        else:
            if lp_status == pulp.LpStatusInfeasible:
        ...
        for node in self.tpo_valid_node_gen():
            if self in node.parents:
                ...
                min_capacity = min(parent.get_latency(node) + int(pulp.value(latency_table[parent])) + parent.fifo(node).depth for parent in node.parents)
            debug_enabled = _logger.isEnabledFor(logging.DEBUG)
            check_failed = int(pulp.value(latency_table[node])) > min_capacity
            if debug_enabled or check_failed:
                _logger.debug if debug_enabled else _logger.warn('II=1 check %s: %s: latency %d %s min capacity %d', '✖' if check_failed else '✔', repr(node), int(pulp.value(latency_table[node])), '>' if check_failed else '<=', min_capacity)

    def update_module_depths(self, depths: Dict[(int, int)]) -> None:
    ...
    """
        ...
        latency_table = {x:pulp.LpVariable(name=f"latency_{x.name}", lowBound=0, cat='Integer') for x in self.tpo_valid_node_gen()}
        for node in self.tpo_valid_node_gen():
            if self in node.parents:
                latency_table[node] = 0
            else:
                lp_problem.extend(parent.get_latency(node) + latency_table[parent] <= latency_table[node] for parent in node.parents)
                lp_problem.extend(parent.get_latency(node) + latency_table[parent] + lp_vars[(parent, node)] >= latency_table[node] for parent in node.parents)
