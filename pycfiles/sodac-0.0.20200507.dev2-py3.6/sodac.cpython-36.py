...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/soda/sodac.py
# Compiled at: 2020-05-08 13:24:17
# Size of source mod 2**32: 8523 bytes
import argparse, logging, os, sys, textx
...
def main():
    parser = argparse.ArgumentParser(prog='sodac',
      description='Stencil with Optimized Dataflow Architecture (SODA) compiler')
    ...
    parser.add_argument('--burst-width', type=int,
      dest='burst_width',
      help='override burst width')
    parser.add_argument('--unroll-factor', type=int,
    ...
    parser.add_argument('--dram-in', type=str,
      dest='dram_in',
      help='override DRAM configuration for input')
    parser.add_argument('--dram-out', type=str,
      dest='dram_out',
    ...
    parser.add_argument(type=str, dest='soda_src',
      metavar='file',
    ...
    parser.add_argument('--estimation-file', type=str,
    ...
    if args.recursion_limit is not None:
        ...
        if sys_recursion_limit > args.recursion_limit:
            logger.warning('Python system recursion limit (%d) > specified value (%d); the latter will be ignored', sys_recursion_limit, args.recursion_limit)
        else:
            sys.setrecursionlimit(args.recursion_limit)
            logger.warning('Python recursion limit is set to %d', sys.getrecursionlimit())
    ...
    try:
        ...
        for dim in range(soda_model.dim - 1):
            if args.tile_size is not None:
                if dim < len(args.tile_size):
                    if args.tile_size[dim] > 0:
        ...
        stencil = core.Stencil(burst_width=(args.burst_width if args.burst_width is not None else soda_model.burst_width),
        ...
        if args.estimation_file is not None:
            ...
            def print_estimation():
                ...
                def print_estimation():
                    model.print_estimation(stencil, model_file, estimation_file)
