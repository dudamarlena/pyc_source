...
# [GCC 8.4.0]
# Embedded file name: /home/mareike/work/app/pyrap-dev/examples/pyrap-admin/admin.py
...
class PyRAPAdmin:
    ...
    def update_process_list(self):
        ...
        for _, thread in threads.active().items():
            if type(thread) is threads._DummyThread:
                continue
            items['%s (%s)' % (thread.name, thread.ident)] = thread

        for thread in threading.enumerate():
            if type(thread) is threading._DummyThread:
                continue
            items['%s (%s)' % (thread.name, thread.ident)] = thread
...
if __name__ == '__main__':
    pyrap.register(clazz=PyRAPAdmin, entrypoints={'main': PyRAPAdmin.main}, path='admin', name='pyRAP Administration Tool')
    pyrap.run()