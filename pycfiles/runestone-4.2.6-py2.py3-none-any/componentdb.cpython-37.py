...
# [GCC 8.4.0]
# Embedded file name: /srv/RunestoneComponents/runestone/server/componentdb.py
# Compiled at: 2019-09-20 13:20:48
...
def get_dburl(outer={}):
    ...
    if ret:
        return re.sub('postgres:.*/', 'postgresql:/', ret)
...
def reset_questions(app, env, docnames):
    if sess:
        ...
        stmt = questions.update().where(and_(questions.c.base_course == basecourse, questions.c.question_type != 'page')).values(from_source='F')
        sess.execute(stmt)
...
def addQuestionToDB(self):
    ...
    if dburl:
        ...
        sel = select([questions]).where(and_(questions.c.name == id_, questions.c.base_course == basecourse))
        res = sess.execute(sel).first()
        try:
            if res:
                stmt = questions.update().where(questions.c.id == res['id']).values(question=(self.block_text),
                ...
                ins = questions.insert().values(base_course=basecourse,
                  name=id_,
...
def getQuestionID(base_course, name):
    sel = select([questions]).where(and_(questions.c.name == name, questions.c.base_course == base_course))
    res = sess.execute(sel).first()
    if res:
        return res['id']
    return
...
def getOrInsertQuestionForPage(base_course=None, name=None, is_private='F', question_type='page', autograde='visited', author=None, difficulty=1, chapter=None):
    ...
    if res:
        id = res['id']
        stmt = questions.update().where(questions.c.id == id).values(timestamp=last_changed,
        ...
        return id
    ins = questions.insert().values(base_course=base_course,
      name=name,
      timestamp=last_changed,
      is_private=is_private,
      question_type=question_type,
...
def addAssignmentQuestionToDB(question_id, assignment_id, points, activities_required=0, autograde=None, which_to_grade=None, reading_assignment=None, sorting_priority=0):
    sel = select([assignment_questions]).where(and_(assignment_questions.c.assignment_id == assignment_id, assignment_questions.c.question_id == question_id))
    res = sess.execute(sel).first()
    vals = dict(assignment_id=assignment_id,
...
def addAssignmentToDB(name=None, course_id=None, assignment_type_id=None, deadline=None, points=None):
    last_changed = datetime.now()
    sel = select([assignments]).where(and_(assignments.c.name == name, assignments.c.course == course_id))
    res = sess.execute(sel).first()
    if res:
        stmt = assignments.update().where(assignments.c.id == res['id']).values(assignment_type=assignment_type_id,
          duedate=deadline,
          points=points)
        sess.execute(stmt)
        a_id = res['id']
        ...
        ins = assignments.insert().values(course=course_id,
          name=name,
...
def addHTMLToDB(divid, basecourse, htmlsrc, feedback=None):
    if dburl:
        last_changed = datetime.now()
        sel = select([questions]).where(and_(questions.c.name == divid, questions.c.base_course == basecourse))
        res = sess.execute(sel).first()
        try:
            if res:
                if res['htmlsrc'] != htmlsrc or res['feedback'] != feedback:
                    stmt = questions.update().where(questions.c.id == res['id']).values(htmlsrc=htmlsrc,
                      feedback=feedback,
                      timestamp=last_changed)
                    sess.execute(stmt)
        except UnicodeEncodeError:
