...
# [GCC 8.4.0]
# Embedded file name: /home/bmiller/Runestone/RunestoneComponents/runestone/codelens/matrix.py
...
class Matrix:
    ...
    def __init__(self, nrows=1, ncols=1, data=None, dftFormat='', dftStyle='', title='', tableAttr=dftTableAttr, tableHeaders=None, Expand=True):
        self.nrows = nrows
        self.ncols = ncols
        self.values = {}
    ...
    def renderHtml(self, wrap=None):
        ...
        if self.title:
            lins[0] = '<div>%s</div>' % self.title
        headers = self.tableHeaders
        if headers:
            lins.append('<tr><th>' + '</th><th>'.join(map(str, headers)) + '</th></tr>')
        for row in range(self.nrows):
            rowLin = [
             '  <tr>']
            vals = self.getrow(row)
            if self.format:
                formats = self.format.getrow(row)
            else:
                formats = [
                 ''] * self.ncols
            if self.style:
                styles = self.style.getrow(row)
            else:
                styles = [
                 ''] * self.ncols
            for c in range(self.ncols):
                ...
                if format:
                    ...
                    if not style:
                        style = self.dftStyle
                    if style:
                        cell = '<td style="%s">%s</td>' % (style, val)
                    else:
                        cell = '<td>%s</td>' % val
                if wrap and c > 0:
                    if c % wrap == 0:
