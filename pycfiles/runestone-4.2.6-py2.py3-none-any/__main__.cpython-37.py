...
# [GCC 8.4.0]
# Embedded file name: /srv/RunestoneComponents/runestone/__main__.py
# Compiled at: 2020-04-16 12:40:55
# Size of source mod 2**32: 12933 bytes
import sys, os, shutil, getpass, six, click
...
def init():
    ...
    conf_dict['dynamic_pages'] = click.prompt('Build book for dynamic page service?',
    ...
    if conf_dict['use_services'] == 'true':
        ...
        conf_dict['dburl'] = click.prompt('DataBase Connection URL',
          default='postgresql://user:password@localhost/runestone')
        conf_dict['enable_chatcodes'] = click.prompt('Enable Enable the chatcode feature)',
          type=bool, default=False)
        if conf_dict['log_level'] and conf_dict['login_req']:
            conf_dict['server_side_grading'] = click.prompt('Grade questions on the server where possible?',
              type=bool,
              default=False)
        else:
            conf_dict['server_side_grading'] = False
        conf_dict['allow_pairs'] = click.prompt('Enable Pair Programming feature(s)',
          type=bool, default=False)
    else:
    ...
    conf_dict['downloads_enabled'] = click.prompt('Enable inline Activecode downloads by default (single activecode downloads may be enabled with the :enabledownload: flag)',
      default='false')
...
def build(all, wd):
    import paver.tasks as paver_main
    if wd:
        os.chdir(wd)
    else:
    ...
    try:
        ...
        try:
            if pavement.dynamic_pages == True:
                click.echo('Error -- dynamic_pages is True, but this server does not support templates',
                  err=True)
...
def deploy(dest):
    ...
    if not dest:
        ...
        if os.name == 'nt':
            sh(('robocopy /mir /mt /nfl {} {}'.format(pavement.serving_dir, os.path.join(dest, os.path.basename(pavement.serving_dir)))),
              ignore_error=True)
        else:
