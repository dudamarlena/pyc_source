...
# [GCC 8.4.0]
# Embedded file name: /home/bmiller/Runestone/RunestoneComponents/runestone/pretext/chapter_pop.py
# Compiled at: 2020-02-01 10:19:36
...
def manifest_data_to_db(course_name, manifest_path):
    ...
    for chapter in root.findall('./chapter'):
        ...
        ins = chapters.insert().values(chapter_name=(chapter.find('./title').text),
        ...
        for subchapter in chapter.findall('./subchapter'):
            ...
            ins = subchapters.insert().values(sub_chapter_name=(subchapter.find('./title').text),
              chapter_id=chapid,
              sub_chapter_label=(subchapter.find('./id').text),
              skipreading='F',
              sub_chapter_num=subchap)
            sess.execute(ins)
            ins = questions.insert().values(base_course=course_name,
              name=('{}/{}'.format(chapter.find('./title').text, subchapter.find('./title').text)),
            ...
            for question in subchapter.findall('./question'):
                ...
                res = sess.execute(f"select * from questions where name='{idchild}' and base_course='{course_name}'").first()
                if res:
                    continue
                ins = questions.insert().values(base_course=course_name,
                  name=idchild,
    ...
    cid = res['id']
    ins = course_attributes.insert().values(course_id=cid,
      attr='latex_macros',
      value=(latex.text))
    sess.execute(ins)
    sess.commit()
