...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/openbandparams/iii_v_zinc_blende_alloy.py
...
class IIIVZincBlendeAlloy(IIIVAlloy):
    ...
    def strained_001(self, target):
        ...
        if isinstance(target, Alloy):
            return IIIVZincBlendeStrained001(unstrained=self, substrate=target)
        else:
            return IIIVZincBlendeStrained001(unstrained=self, strain_out_of_plane=target)
    ...
    @method_parameter(dependencies=['Eg_Gamma', 'Eg_L', 'Eg_X'], units='eV', aliases=[
     'bandgap'])
    def Eg(self, **kwargs):
        """
        ...
        temperature, T, in K (default=300.).
        """
        return min(self.Eg_Gamma(**kwargs), self.Eg_L(**kwargs), self.Eg_X(**kwargs))
    ...
    @method_parameter(dependencies=['Eg_Gamma_0', 'Delta_SO', 'Ep',
     'meff_e_Gamma_0'], units='dimensionless', references=[vurgaftman_2001])
    def F(self, **kwargs):
        """
        ...
        return (1.0 / meff - 1 - Ep * (Eg + 2.0 * Delta_SO / 3.0) / (Eg * (Eg + Delta_SO))) / 2
    ...
    def meff_SO(self, **kwargs):
        ...
        return 1.0 / (luttinger1 - Ep * Delta_SO / (3 * Eg * (Eg + Delta_SO)))
    ...
    @method_parameter(dependencies=['meff_e_L_long', 'meff_e_L_trans'], units='m_e', references=[vurgaftman_2001])
    def meff_e_L_DOS(self, **kwargs):
        t = self.meff_e_L_trans(**kwargs)
        l = self.meff_e_L_long(**kwargs)
        return (t * t * l) ** (1.0 / 3.0)

    @method_parameter(dependencies=['meff_e_X_long', 'meff_e_X_trans'], units='m_e', references=[vurgaftman_2001])
    ...
    def meff_hh_100(self, **kwargs):
        ...
        """
        return 1.0 / (self.luttinger1(**kwargs) - 2 * self.luttinger2(**kwargs))
    ...
    def meff_hh_110(self, **kwargs):
        ...
        """
        return 2.0 / (2 * self.luttinger1(**kwargs) - self.luttinger2(**kwargs) - 3 * self.luttinger3(**kwargs))
    ...
    def meff_hh_111(self, **kwargs):
        ...
        """
        return 1.0 / (self.luttinger1(**kwargs) - 2 * self.luttinger3(**kwargs))
