...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/openbandparams/tests/test_iii_v_zinc_blende_quaternary.py
# Compiled at: 2015-04-09 03:43:25
import os, sys
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../..')))
from openbandparams import iii_v_zinc_blende_quaternaries, GaAs, AlAs, GaSb, AlGaInAs, AlPAsSb, AlGaAsSb, GaPAsSb, AlGaInSb, AlGaPAs, AlInAsSb
...
class TestIIIVZincBlendeQuaternary(unittest.TestCase):
    ...
    def test_repr(self):
        self.assertEqual(eval(repr(AlGaInAs(x=0, y=0))), AlGaInAs(x=0, y=0))
        for quaternary in iii_v_zinc_blende_quaternaries:
            self.assertEqual(eval(repr(quaternary)), quaternary)
            self.assertEqual(eval(repr(quaternary(x=0, y=0))), quaternary(x=0, y=0))
            self.assertEqual(eval(repr(quaternary(x=0, y=0.1))), quaternary(x=0, y=0.1))
            self.assertEqual(eval(repr(quaternary(x=0.1, y=0))), quaternary(x=0.1, y=0))
            self.assertEqual(eval(repr(quaternary(x=0.1, y=0.1))), quaternary(x=0.1, y=0.1))

    def test_quaternary1_latex(self):
        self.assertEqual(AlPAsSb.latex(), 'AlP_{x}As_{y}Sb_{1-x-y}')
        self.assertEqual(AlPAsSb(x=0, y=0).latex(), 'AlP_{0}As_{0}Sb_{1}')

    def test_quaternary2_latex(self):
        self.assertEqual(AlGaInAs.latex(), 'Al_{x}Ga_{y}In_{1-x-y}As')
        self.assertEqual(AlGaInAs(x=0, y=0).latex(), 'Al_{0}Ga_{0}In_{1}As')

    def test_quaternary3_latex(self):
        self.assertEqual(AlGaAsSb.latex(), 'Al_{x}Ga_{1-x}As_{y}Sb_{1-y}')
        self.assertEqual(AlGaAsSb(x=0, y=0).latex(), 'Al_{0}Ga_{1}As_{0}Sb_{1}')
    ...
    def test_non_instanced_lattice_matching(self):
        ...
        Eg2 = AlInAsSb(Al=0, a=GaSb.a(), T=300.0).Eg()
