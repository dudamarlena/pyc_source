...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/openbandparams/examples/Plot_Bandgap_vs_Lattice_Constant.py
 ...
 (
  arsenide_antimonide_ternaries, green_blue)]:
    for ternary in ternaries:
        ax.plot([ ternary(x=f).a(T=T_lattice) for f in fractions ], [ ternary(x=f).Eg(T=T) for f in fractions ], color=color, linewidth=1.2)
...
for binaries, color in [(phosphide_binaries, red),
 ...
 (
  antimonide_binaries, blue)]:
    ax.plot([ b.a(T=T_lattice) for b in binaries ], [ b.Eg(T=T) for b in binaries ], color=color, linestyle=' ', marker='o', markersize=4, markeredgecolor=color)
