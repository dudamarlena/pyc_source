...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/openbandparams/parameter.py
...
class ValueParameter(Parameter):
    ...
    def __init__(self, name, value, units, aliases=[], references=[]):
        """
        Parameters
        ----------
        name : string
        ...
        references : list of Reference objects (default=[])
            literature references
        """
        super(ValueParameter, self).__init__(name=name, units=units, aliases=aliases, references=references)
        self.value = value
...
class FunctionParameter(Parameter):
    ...
    def __init__(self, name, function, units, aliases=[], references=[]):
        """
        Parameters
        ----------
        name : string
        ...
        references : list of Reference objects (default=[])
            literature references
        """
        super(FunctionParameter, self).__init__(name=name, units=units, aliases=aliases, references=references)
...
class MethodParameter(Parameter):
    ...
    def __init__(self, name, method, dependencies, units, aliases=[], references=[]):
        """
        Parameters
        ----------
        name : string
        ...
        references : list of Reference objects (default=[])
            literature references
        """
        super(MethodParameter, self).__init__(name=name, units=units, aliases=aliases, references=references)
        self.alloy = None
        self.method = method
    ...
    def __call__(self, *args, **kwargs):
        if self.alloy is None:
            raise TypeError('MethodParameter must be bound to an Alloy with `bind` before calling.')
...
def method_parameter(dependencies, units, aliases=[], references=[]):
    ...
    def decorator(method):
        ...
        return MethodParameter(name, method, dependencies, units, aliases, references)

    return decorator


descriptions = {'CBO': 'conduction band offset energy relative to InSb VBO', 
