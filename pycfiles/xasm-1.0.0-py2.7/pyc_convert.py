...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/xasm/pyc_convert.py
# Compiled at: 2017-08-11 14:31:31
...
from xasm.assemble import asm_file, Assembler, create_code, Instruction, decode_lineno_tab
...
def copy_magic_into_pyc(input_pyc, output_pyc, src_version, dest_version):
    ...
    assert version == float(src_version), 'Need Python %s bytecode; got bytecode for version %s' % (
     src_version, version)
...
def transform_26_27(inst, new_inst, i, n, offset, instructions, new_asm):
    ...
    if inst.opname in ('JUMP_IF_FALSE', 'JUMP_IF_TRUE'):
        ...
        new_inst.opname = 'POP_JUMP_IF_FALSE' if inst.opname == 'JUMP_IF_FALSE' else 'POP_JUMP_IF_TRUE'
...
@click.command()
@click.option('--conversion-type', '-t', type=click.Choice([
 '20-21', '21-22', '23-24', '24-23', '24-25', '25-26',
 '26-27', '32-33', '33-32']), help='specify conversion from/to bytecode', default='26-27')
@click.argument('input_pyc', type=click.Path('r'), nargs=1)
@click.argument('output_pyc', type=click.Path('w'), required=False, nargs=1, default=None)
def main(conversion_type, input_pyc, output_pyc):
    ...
    if conversion_type in UPWARD_COMPATABLE:
        ...
        filename, co, version, timestamp, magic_int = disassemble_file(input_pyc, temp_asm, asm_format=True)
        temp_asm.close()
        assert version == float(src_version), 'Need Python %s bytecode; got bytecode for version %s' % (
         src_version, version)
