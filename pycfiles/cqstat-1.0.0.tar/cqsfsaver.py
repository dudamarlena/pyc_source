...
# Embedded file name: build/bdist.linux-i686/egg/cqsfsaver/cqsfsaver.py
# Compiled at: 2011-02-17 03:30:27
import sys, os, re, cgi, BaseHTTPServer, logging, simplejson as json, Tkinter, tkFileDialog, tkMessageBox
if os.name == 'nt':
    import ctypes
PATH_PATTERN = re.compile('^/\\?key=(\\w+)$', re.U)
PORTS_RANGE = range(30778, 31336)
SU_PATTERN = re.compile('^exec cqsfsaver start&$', re.U | re.M)
    ...
    def stop(self):
        if self.running():
            try:
                ...
                logging.warning('Cannot stop server')
...
class GUI:
    ...
    def show_source(self, source, url, action='save'):
        ...
        label.pack(side=Tkinter.TOP, expand=0, fill=Tkinter.BOTH)
        ...
        self.root.minsize(root_width, root_height)
        if 'screen_width' in CONFIG:
            root_width = CONFIG['screen_width']
        margin_v = (self.root.winfo_screenwidth() - root_width) / 2
        margin_h = (self.root.winfo_screenheight() - root_height) / 2
        geom_string = '%dx%d+%d+%d' % (root_width, root_height, margin_v, margin_h)
    ...
    def save_width(self):
        changed = False
        cwidth = self.root.winfo_width()
        if 'screen_width' in CONFIG:
            if CONFIG['screen_width'] != cwidth:
                CONFIG['screen_width'] = cwidth
                changed = True
        else:
            CONFIG['screen_width'] = cwidth
            changed = True
...
class Saver:
    ...
    def __init__(self, key, url, text, stype=None, allow=None, name=None):
        if url and text:
            ...
            self.key = key
            if stype:
                self.stype = stype[0].decode('utf-8').encode('utf-8')
            else:
                self.stype = None
            ...
            self.send_false('No "css_text" in POST ')
        elif text:
            self.send_false('No "css_url" in POST ')
        else:
            self.send_false('No "css_url" and "css_text" in POST ')
        return
...
class HttpHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path_match:
            ...
            if saver.success:
                self.send_true(saver.message)
            else:
                self.send_false(saver.message)
...
def install():
    if sys.platform == 'linux2' or sys.platform == 'darwin':
        import stat
        xi_path = os.path.join(os.environ['HOME'], '.xinitrc')
        if os.path.exists(xi_path):
            x = open(os.path.join(os.environ['HOME'], '.xinitrc'), 'r+')
            istr = '\nexec cqsfsaver start&\n'
        else:
            x = open(os.path.join(os.environ['HOME'], '.xinitrc'), 'w+')
            istr = '#!/bin/sh\nexec cqsfsaver start&\n'
        sf = x.read()
        if re.search(SU_PATTERN, sf):
            print 'Already installed to startup'
        else:
            x.write(istr)
            print 'Installed to startup'
        x.close()
        os.chmod(xi_path, stat.S_IXUSR | stat.S_IRUSR | stat.S_IWUSR)
...
def uninstall():
    if sys.platform == 'linux2' or sys.platform == 'darwin':
        ...
        if os.path.exists(xi_path):
            buf = ''
            found = False
            current = open(xi_path, 'r')
            for line in current:
                if line == 'exec cqsfsaver start&\n':
                    found = True
                    buf += ''
                else:
                    buf += line
...
def main(args=None):
    if len(sys.argv) == 2:
        ...
        if sys.argv[1] == 'stop':
            s = Httpd()
            s.stop()
        if sys.argv[1] == 'install_startup':
            install()
        if sys.argv[1] == 'uninstall_startup':
            uninstall()
    elif len(sys.argv) == 3:
        if sys.argv[1] == 'test':
            Test(cgi.parse_qs(sys.argv[2]))
    else:
        print 'Usage: cqsfsaver start|stop|install_startup|uninstall_startup|test "query"'
