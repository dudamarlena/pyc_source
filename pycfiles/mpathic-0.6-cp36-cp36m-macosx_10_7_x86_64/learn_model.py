...
# [GCC 8.4.0]
# Embedded file name: mpathic/src/learn_model.py
...
class LearnModel:
    ...
    def __init__(self, df, lm='ER', modeltype='MAT', LS_means_std=None, db=None, iteration=30000, burnin=1000, thin=10, runnum=0, initialize='LS', start=0, end=None, foreground=1, background=0, alpha=0.0, pseudocounts=1, drop_library=False, verbose=False, tm=None):
        ...
        if lm == 'ER':
            if modeltype == 'NBR':
                ...
                emat = self.Berg_von_Hippel(df, dicttype, foreground=foreground, background=background, pseudocounts=pseudocounts)
        if lm == 'PR':
            emat = self.convex_opt(df, seq_dict, inv_dict, col_headers, tm=tm, dicttype=dicttype, modeltype=modeltype)
        if lm == 'LS':
            if LS_means_std:
            ...
            print par_seq_dict
            print ('dict: ', dicttype)
            raveledmat, batch, sw = utils.genweightandmat(df, par_seq_dict, dicttype, means=means, modeltype=modeltype)
    ...
    def convex_opt(self, df, seq_dict, inv_dict, columns, tm=None, modeltype='MAT', dicttype='dna'):
        ...
        output_parameterized = self.reverse_parameterization(output, cols_for_keep, wtrow, seq_dict, bins=tm.shape[1], modeltype=modeltype)
    ...
    def Berg_von_Hippel(self, df, dicttype, foreground=1, background=0, pseudocounts=1):
        ...
        foreground_freqs[binheaders] = foreground_freqs[binheaders].div(foreground_freqs[binheaders].sum(axis=1), axis=0)
        background_freqs[binheaders] = background_freqs[binheaders].div(background_freqs[binheaders].sum(axis=1), axis=0)
    ...
    def Markov(self, df, dicttype, foreground=1, background=0, pseudocounts=1):
        ...
        foreground_freqs_neighbor[binheaders_neighbor] = foreground_freqs_neighbor[binheaders_neighbor].div(foreground_freqs_neighbor[binheaders_neighbor].sum(axis=1), axis=0)
        background_freqs_neighbor[binheaders_neighbor] = background_freqs_neighbor[binheaders_neighbor].div(background_freqs_neighbor[binheaders_neighbor].sum(axis=1), axis=0)
