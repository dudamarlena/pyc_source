...
# [GCC 8.4.0]
# Embedded file name: /Users/tareen/Desktop/Desktop_Tests/MPathic/mpathic/src/_nsb.py
...
Interface to this functionality is supplied through mpathic.info.entropy()
...
def nsb_integrand_variance_term1(beta, nxkx, N, m):
    ...
    summand1 = sp.array([nxkx[x] * (x + beta + 1) * (x + beta) / ((N + m * beta + 1) * (N + m * beta)) * psi(1, x + beta + 1) for x in nxkx])
    summand2 = sp.array([nxkx[x] * (x + beta) / ((N + m * beta + 1) * (N + m * beta)) * psi(0, x + beta + 1) ** 2 for x in nxkx])
    summand3 = sp.array([nxkx[x] * (x + beta) / (N + m * beta) * psi(0, x + beta + 1) for x in nxkx])
    return sp.sum(summand1) - psi(1, N + beta * m + 1) + sp.sum(summand2) - 1 / (N + m * beta + 1) * sp.sum(summand3) ** 2


def nsb_integrand_variance_term2(beta, nxkx, N, m):
    summand1 = sp.array([nxkx[x] * ((x + beta) / (N + m * beta) * psi(0, x + beta + 1)) for x in nxkx])
...
def _dSi(w, nxkx, N, K):
    sbeta = w / (1 - w)
    beta = sbeta * sbeta
    return _rho(beta, nxkx, N, K) * (nsb_integrand_variance_term1(beta, nxkx, N, K) + nsb_integrand_variance_term2(beta, nxkx, N, K)) * _dxi(beta, K) * 2 * sbeta / (1 - w) / (1 - w)
...
def dS(x, N, K):
  ...
  """
    ...
    g = lambda w: _measure(w, nxkx, N, K)
    return np.log2(np.exp(1)) * quadgl(f, [0, 1]) / quadgl(g, [0, 1]) - S(x, N, K) ** 2 / np.log2(np.exp(1))
