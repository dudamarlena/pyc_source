...
# [GCC 8.4.0]
# Embedded file name: /Users/tareen/Desktop/Desktop_Tests/MPathic3/mpathic/src/profile_info.py
...
class ProfileInfo:
    ...
    def __init__(self, dataset_df=None, err=False, method='naive', pseudocount=1.0, start=0, end=None):
        ...
        for i in range(end - start):
            ...
            if err:
                mi, mi_err = info.estimate_mutualinfo(nxy, err=True, method=method,
                  pseudocount=pseudocount)
                info_df.loc[(i + start, 'info')] = mi
                info_df.loc[(i + start, 'info_err')] = mi_err
            else:
                mi = info.estimate_mutualinfo(nxy, err=False, method=method,
                  pseudocount=pseudocount)
                info_df.loc[(i + start, 'info')] = mi
