...
# [GCC 8.4.0]
# Embedded file name: /Users/tareen/Desktop/Desktop_Tests/MPathic3/mpathic/src/qc.py
# Compiled at: 2018-06-22 15:44:17
# Size of source mod 2**32: 48589 bytes
import pandas as pd
from Bio.Alphabet import IUPAC
import re, pdb, numpy as np, sys
...
col_patterns = {'seqs':'^seq$|^seq_rna$|^seq_pro$', 
 ...
 'contig':'^contig$', 
 'ori':'^ori$'}
seqtype_to_alphabet_dict = {'dna':''.join(sorted(IUPAC.IUPACUnambiguousDNA.letters)), 
...
for seqtype, alphabet in seqtype_to_alphabet_dict.items():
    seqerr_re_dict[seqtype] = re.compile('[^%s]' % alphabet)
...
colname_to_seqtype_dict = {'seq':'dna', 
 'seq_rna':'rna', 
 'seq_pro':'protein', 
 'tag':'dna', 
 'wt':'dna', 
 'wt_rna':'rna', 
 'wt_pro':'protein'}
seqtype_to_wtcolname_dict = {'dna':'wt', 
 'rna':'wt_rna', 
 'protein':'wt_pro'}
seqtype_to_seqcolname_dict = {'dna':'seq', 
 'rna':'seq_rna', 
 'protein':'seq_pro'}
seqtypes = [
 'dna', 'rna', 'protein']
...
def _validate_ori_cols(df, fix=False):
    ...
    if col in df.columns:
        ...
        if not all((df[col] == '+') | (df[col] == '-')):
            raise SortSeqError('ori column contains more than just "+" and "-" characters.')
    return df
...
def _validate_pos_cols(df, fix=False):
    ...
    if col in df.columns:
        try:
            ...
            raise SortSeqError('Cannot convert values in column %s to numbers.' % col)
        ...
        if not df[col].values.dtype == int:
            if all(int_vals == float_vals):
                if fix:
                    ...
                    raise SortSeqError('Positions are not integers; set fix=True to fix.')
            else:
                raise SortSeqError('Positions cannot be interpreted as integers.')
        first = df[col].iloc[0]
        last = df[col].iloc[(-1)]
    ...
    for col in mut_cols:
        if not df[col].values.dtype == float:
            ...
            if fix:
                ...
                raise SortSeqError('Freqs are not floats; set fix=True to fix.')
    ...
    for col in err_cols:
        if not df[col].values.dtype == float:
            ...
            if fix:
                ...
                raise SortSeqError('Errs are not floats; set fix=True to fix.')
            if not all(np.isfinite(df[col])):
    ...
    if col in df.columns:
        if not df[col].values.dtype == float:
            ...
            if fix:
                ...
                raise SortSeqError('std values are not floats; set fix=True to fix.')
        else:
            if not all(np.isfinite(df[col])):
    ...
    for col in val_cols:
        if not df[col].values.dtype == float:
            try:
                ...
                raise SortSeqError('Cannot interpret values in %s as floats.' % col)

            if fix:
                df[col] = float_vals
            else:
                SortSeqError('Values in %s not floats; set fix=True to fix.' % col)
            if not all(np.isfinite(df[col])):
...
def _validate_freq_cols(df, fix=False, tol=0.01):
    """
...
def validate_sitelist(df, fix=False):
    ...
    if not df.shape[0] >= 1:
        ...
        if not len(seq_cols) == 1:
            raise SortSeqError('Invalid number=%d of "seqs" columns in sitelist.' % len(seq_cols))
        seq_col = seq_cols[0]
        ...
        if not np.array_equal(seq_lengths_obs, seq_lengths_calc):
            raise SortSeqError('%s lengths are not consistent with "left" and "right" cols.' % seq_col)
...
def validate_filelist(df, fix=False):
    ...
    if not df.shape[0] >= 1:
        raise SortSeqError('Dataframe must contain at least one row')
    elif not df.shape[1] == 2:
        raise SortSeqError('Dataframe must contain exactly two columns')
    else:
        exact_cols = [
         'bin', 'file']
        for col in df.columns:
        ...
        if all(df.columns == exact_cols) or fix:
            df = df[exact_cols]
        else:
            raise SortSeqError('Dataframe columns are in the wrong order; set fix=True to fix.')
    df = _validate_cols(df, fix=fix)
...
def validate_model(df, fix=False):
    ...
    for cols in model_parameters_dict.values():
        if len(cols) == len(val_cols) and all([a == b for a, b in zip(cols, val_cols)]):
            ok = True
...
def validate_profile_freq(df, fix=False, tol=0.01):
    ...
    if not tol >= 0.0:
        ...
        if all(df.columns == new_cols) or fix:
            df = df[new_cols]
        else:
            raise SortSeqError('Dataframe columns are in the wrong order; set fix=True to fix.')
    return df
...
def validate_input_arguments_for_learn_model(modeltype=None, learningmethod=None, start=None, end=None, iteration=None, burnin=None, thin=None, pseudocounts=None, foreground=None, background=None, alpha=None):
    ...
    if foreground == background:
        raise SortSeqError('Foreground and background bins cannot be the             same')
        ...
        if start < 0:
            ...
            if burnin is not None:
                ...
                if iteration is not None:
                    if burnin > iteration:
                        raise SortSeqError('Burnin argument must be less than the number \n                    of iterations')
            if thin is not None:
                if thin < 0:
                    raise SortSeqError('Thin argument must be positive')
                if iteration is not None:
                    if iteration < thin:
                        raise SortSeqError('Thin argument must be less than the number of iterations')
    else:
