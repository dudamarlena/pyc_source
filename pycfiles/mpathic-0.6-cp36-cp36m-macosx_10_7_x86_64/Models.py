...
# [GCC 8.4.0]
# Embedded file name: mpathic/src/Models.py
...
class LinearModel(ExpModel):
    ...
    def evaluate_on_mutarray(self, mutarray, wtrow):
        return numerics.eval_modelmatrix_on_mutarray(modelmatrix=self.matrix.T, mutarray=mutarray, wtrow=wtrow)
...
class NormalNoise(NoiseModel):
    ...
    def gennoisyexp(self, logexp):
        ...
        nexp = logexp + np.random.normal(scale=s, size=len(logexp))
        return nexp
