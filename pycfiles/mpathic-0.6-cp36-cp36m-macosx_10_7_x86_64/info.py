...
# [GCC 8.4.0]
# Embedded file name: mpathic/src/info.py
...
def estimate_mutualinfo(raw_counts, pseudocount=1, err=False, method='naive'):
    ...
    """
    if not method in ('naive', 'tpm', 'nsb'):
        raise AssertionError
        raise (pseudocount >= 0 or SortSeqError)('pseudocount is not nonnegative.')
    counts = fix_counts_2d(raw_counts)
    if method == 'naive':
        if err:
            mi, mi_err = _estimate_mutualinfo_naive(counts, pseudocount=pseudocount, err=True)
        else:
            mi = _estimate_mutualinfo_naive(counts, pseudocount=pseudocount, err=False)
    else:
        if method == 'tpm':
            if err:
                mi, mi_err = _estimate_mutualinfo_naive(counts, pseudocount=pseudocount, err=True)
