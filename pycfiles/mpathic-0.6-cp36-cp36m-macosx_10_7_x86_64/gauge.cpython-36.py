...
# [GCC 8.4.0]
# Embedded file name: /Users/tareen/Desktop/Desktop_Tests/MPathic/mpathic/src/gauge.py
# Compiled at: 2018-05-10 19:02:32
# Size of source mod 2**32: 18839 bytes
...
def load_matrix(filename, wtseq, mutrate, seqtype='DNA', normalize=False, features=False, verbose=False, rcond=1e-10):
    ...
    matrix_cg = fix_matrix(matrix_model, wtseq=wtseq, mutrate=mutrate, seqtype=seqtype,
      normalize=normalize,
      verbose=verbose,
      rcond=rcond)
    if features:
...
def load_neighbor(filename, wtseq, mutrate, seqtype='DNA', normalize=False, features=False, verbose=False, rcond=1e-10):
    ...
    neighbor_cg = fix_neighbor(neighbor_model, wtseq=wtseq, mutrate=mutrate, seqtype=seqtype,
      normalize=normalize,
      verbose=verbose,
      rcond=rcond)
    if features:
...
def fix_matrix(matrix_model, wtseq='', mutrate=1, seqtype='DNA', verbose=False, rcond=1e-10, normalize=False):
    ...
    if not len(wtseq) == P:
        raise AssertionError('len(wtseq)==%d, P==%d' % (len(wtseq), P))
    else:
...
def neighbor2matrix(neighbor_model, wtseq, mutrate, seqtype='DNA', verbose=False, rcond=1e-10):
    ...
    if not C == len(alphabets[seqtype]):
        raise AssertionError('Size conflict: seqtype==%s specifies %d characters; matrix has %d columns' % (seqtype, len(alphabets[seqtype]), C))
    else:
...
def hierarchical_gauge(neighbor_model, wtseq, mutrate, seqtype='DNA', verbose=False, rcond=1e-10, power=False):
    neighbor_cg = fix_neighbor(neighbor_model, wtseq=wtseq, mutrate=mutrate,
      rcond=rcond)
    matrix_hg = neighbor2matrix(neighbor_cg, wtseq=wtseq, mutrate=mutrate,
      rcond=rcond)
    neighbor_hg_parallel = matrix2neighbor(matrix_hg, wtseq=wtseq,
      mutrate=mutrate,
      rcond=rcond)
