...
# [GCC 8.4.0]
# Embedded file name: /Users/tareen/Desktop/Desktop_Tests/MPathic3/mpathic/src/profile_ct.py
# Compiled at: 2018-06-21 15:20:48
...
def main(dataset_df, bin=None, start=0, end=None):
    ...
    if not dataset_df.shape[0] > 1:
        ...
        if bin is None:
            ...
            ct_col = 'ct_%d' % bin
    if ct_col not in dataset_df.columns:
...
def add_subparser(subparsers):
    ...
    p.add_argument('-b',
      '--bin', type=int, default=None, help='Dataset bin to use for counts. If blank, total counts will be used')
    p.add_argument('-i',
      '--i', type=str, default=None, help='Input file, otherwise input through the standard input.')
    p.add_argument('-s',
      '--start', type=int, default=0, help='Position to start your analyzed region')
    p.add_argument('-e',
      '--end', type=int, default=None, help='Position to end your analyzed region')
    p.add_argument('-o',
      '--out', type=str, default=None, help='Output file, otherwise use standard output.')
