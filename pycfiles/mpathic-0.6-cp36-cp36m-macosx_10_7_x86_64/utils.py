...
# [GCC 8.4.0]
# Embedded file name: /Users/tareen/Desktop/Desktop_Tests/MPathic/mpathic/src/utils.py
# Compiled at: 2018-05-23 15:19:22
from __future__ import division
import sys, numpy as np, scipy as sp, scipy.ndimage, pandas as pd
...
def profile_counts(df, dicttype, wtseq=None, return_wtseq=False, bin_k=None, start=0, end=None):
    ...
    if wtseq:
        assert wtseq == wttest
        is_seq_valid(wttest, seq_dict) or sys.exit('Wild type seq is invalid')
...
def choose_dict(dicttype, modeltype='MAT'):
    ...
    allowedDicttypes = [
     'dna', 'rna', 'protein']
    check(dicttype in allowedDicttypes, " 'dicttype' must be either 'dna', 'rna', or 'protein', entered dicttype: %s " % dicttype)
    if dicttype == 'dna':
    ...
    if modeltype == 'NBR':
        seq_dict = {('').join([inv_dict[i], inv_dict[z]]):i * len(seq_dict) + z for i in range(len(seq_dict)) for z in range(len(seq_dict))}
...
def collapse(df):
    ...
    for i in range(1, df['batch'].max() + 1):
        ...
        output_df = pd.concat([
         output_df, pd.Series(thisbatch.value_counts(), name='ct_' + str(i))], axis=1)
...
def seq2mat2(seq):
    """ returns which parameters are true for the sequence, 
        where each parameter is a possible pairing"""
    pair_dict = {'AA': 0, 
       'AC': 1, 'AG': 2, 'AT': 3, 'CA': 4, 'CC': 5, 'CG': 6, 'CT': 7, 
       'GA': 8, 'GC': 9, 'GG': 10, 'GT': 11, 'TA': 12, 'TC': 13, 'TG': 14}
    pairlist = []
    lseq = len(seq)
    index = 0
    for i, bp in enumerate(seq):
...
def genlassomat(df, modeltype, seq_dict, dicttype):
    ...
    return (
     sp.sparse.csr_matrix(lasso_mat), batch)
...
def genweightandmat(weights_df, seq_dict, dicttype, means=None, modeltype='MAT'):
    ...
    return (
     sp.sparse.csr_matrix(lasso_mat), batch, sample_weights)
...
def handle_errors(func):
    ...
    def wrapped_func(*args, **kwargs):
        ...
        try:
            ...
            if should_fail is True:
                print (
                 'Error, as expected: ', e.__str__())
                mistake = False
            elif should_fail is False:
                print (
                 'MISTAKE: Failed but should have succeeded: ', e.__str__())
                mistake = True
            else:
