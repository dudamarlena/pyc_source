...
# [GCC 8.4.0]
# Embedded file name: mpathic/src/EstimateMutualInfoforMImax.py
...
def main():
    ...
    parser.add_argument('-q1', '--q1type', choices=['Continuous', 'Discrete'], default='Discrete', help='Data type for first quantity.')
    parser.add_argument('-q2', '--q2type', choices=['Continuous', 'Discrete'], default='Discrete', help='Data type for first quantity.')
    parser.add_argument('-k', '--kneig', default='6', help='If you are estimating Continuous\n        vs Continuous, you can overwrite default arguments for the Kraskov \n        estimator here. This argument is number of nearest neighbors \n        to use, with 6 as the default.')
    parser.add_argument('-td', '--timedelay', default='1', help='Kraskov Time Delay, default=1')
    parser.add_argument('-cv', '--crossvalidate', default=False, choices=[True, False], help='Cross validate Kernel Density Estimate. Default=False')
    parser.add_argument('-o', '--out', default=False, help='Output location/type, by \n        default it writes to standard output, if a file name is supplied \n        it will write to a text file')
    args = parser.parse_args()
    MI, V = EstimateMI(quant1, quant2, args.q1type, args.q2type, timedelay=args.timedelay, embedding=args.embedding, kneig=args.kneig, cv=args.crossvalidate)
    if args.out:
