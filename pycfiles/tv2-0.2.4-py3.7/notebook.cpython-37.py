...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tv2/notebook.py
# Compiled at: 2019-02-15 00:31:18
...
class NewNoteBookError(Error):
    __doc__ = 'Exception raised if initialising a new NoteBook fails.\n\n    '
...
class NewNoteError(Error):
    __doc__ = 'Exception raised if making a new Note or adding it to a NoteBook fails.\n\n    '
...
class NoteAlreadyExistsError(NewNoteError):
    __doc__ = 'Exception raised when trying to add a new note that already exists.\n\n    '


class InvalidNoteTitleError(NewNoteError):
    __doc__ = 'Exception raised when trying to add a new note with an invalid title.\n\n    '


class DelNoteError(Error):
    __doc__ = 'Exception raised if removing a Note from a NoteBook fails.\n\n    '
...
class PlainTextNote(object):
    __doc__ = 'A note, stored as a plain text file on disk.'
    ...
    def __init__(self, title, notebook, extension):
        ...
        os.path.isdir(directory) or logger.debug("'{0} doesn't exist, creating it".format(directory))
        try:
            os.makedirs(directory)
        except os.error as e:
            try:
                raise NewNoteError('{0} could not be created: {1}'.format(directory, e))
            finally:
                e = None
                del e
    ...
    def contents(self):
        ...
        if contents is None:
            logger.error('Could not decode file contents: {0}'.format(self.abspath))
            return ''
        return contents
...
class PlainTextNoteBook(object):
    __doc__ = 'A NoteBook that stores its notes as a directory of plain text files.'

    def __init__(self, path, extension, extensions, search_function=brute_force_search, exclude=None):
        ...
        if extension:
            ...
            os.path.isdir(self.path) or logger.debug("'{0} doesn't exist, creating it".format(self.path))
            try:
                os.makedirs(self.path)
            except os.error as e:
                try:
                    raise NewNoteBookError('{0} could not be created: {1}'.format(self.path, e))
                finally:
                    e = None
                    del e
            ...
            for root, dirs, files in os.walk(self.path):
                ...
                for filename in files:
                    ...
                    if not filename.startswith('.'):
                        ...
                        if relpath is None:
                            logger.error('Could not decode filename: {0}'.format(relpath))
                        else:
    ...
    def add_new(self, title, extension=None):
        ...
        if extension is None:
            ...
            title = title.strip()
            assert os.path.split(title)[1], 'Invalid note title: {0}'.format(title)
        for note in self._notes:
            if note.title == title and note.extension == extension:
                raise NoteAlreadyExistsError('Note already in NoteBook: {0}'.format(note.title))
