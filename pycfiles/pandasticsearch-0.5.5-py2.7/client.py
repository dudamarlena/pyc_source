...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/pandasticsearch/client.py
# Compiled at: 2019-02-07 07:32:07
import json, sys, base64, ssl
...
class RestClient(object):
    ...
    def post(self, data, params=None):
        ...
        try:
            ...
            if params is not None:
                url = ('{0}?{1}').format(url, urllib.parse.urlencode(params))
            req = urllib.request.Request(url=url, data=json.dumps(data).encode('utf-8'), headers={'Content-Type': 'application/json'})
            if username is not None and password is not None:
                base64creds = base64.b64encode('%s:%s' % (username, password))
                req.add_header('Authorization', 'Basic %s' % base64creds)
            if verify_ssl is False:
