...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/kend/converter/XML.py
# Compiled at: 2017-06-19 12:17:51
import collections, datetime, kend.model
...
class _PageBox:
    ...
    @staticmethod
    def _parse(input):
        return kend.model.PageBox(page=input.get('page'), rotation=input.get('rotation', 0), left=input.get('left'), top=input.get('top'), right=input.get('right'), bottom=input.get('bottom'))
...
class _Evidence:
    ...
    @staticmethod
    def _parse(input):
        return kend.model.Evidence(type=input.get('type', None), data=input.text, srctype=input.get('srctype', None), src=input.get('src', None))
...
class _Media(_XMLConverter):
    ...
    def _parse(input):
        ...
        return kend.model.Media(id=input.get('id', None), name=input.get('name', None), src=input.get('src', None), type=input.get('type', None), data=input.text, revision=revision, size=size, edit=input.get('edit', None))
...
class _Group:
    ...
    def _parse(input):
        ...
        if fnode is not None:
            ...
            return kend.model.Group(input=input_parameters, ignored=ignored_parameters, output=output_parameters, annotations=annotations, documents=documents)
            return
...
_converters = {kend.model.DocumentReference: _DocumentReference, 
...
def serialise(obj, cls=None):
    ...
    return (
     etree.tostring(output, pretty_print=True, xml_declaration=True, encoding='utf-8'),
     mime_type)
...
def convert4To6(xml, doc):
    ...
    if old_properties is not None:
        for old_property in old_properties:
            new_property_key = old_property.tag.replace('http://utopia.cs.manchester.ac.uk/utopia/annotation', 'http://utopia.cs.manchester.ac.uk/kend').replace('http://utopia.cs.man.ac.uk/utopia/annotation', 'http://utopia.cs.manchester.ac.uk/kend')
...
def convertTo7(xml, doc=None, v=None):
    ...
    if (v or element.attrib.get('version', None)) == '0.7':
        ...
        if (v or element.attrib.get('version', None)) == '0.6':
            return convert6To7(element, doc)
        if (v or element.attrib.get('version', None)) == '0.4':
