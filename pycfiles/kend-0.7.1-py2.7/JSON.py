...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/kend/converter/JSON.py
# Compiled at: 2017-04-05 11:10:18
from kend.converter.base import *
import kend.model, json
    ...
    def default(self, obj):
        ...
        if isinstance(obj, kend.model.PageBox):
            ...
            if isinstance(obj, kend.model.DocumentAnchor):
                json_obj.update({'extents': [ self.default(extent) for extent in obj.extents ], 'areas': [ self.default(area) for area in obj.areas ]})
            ...
            _apply(obj, json_obj, ('author', 'concept', 'created', 'updated', 'id',
                                   'revision', 'edit', 'status', 'context'))
            json_obj.update({'properties': dict(obj), 
               'anchors': [ self.default(anchor) for anchor in obj.anchors ], 'media': [ self.default(media) for media in obj.media ]})
            ...
            if obj.fault is not None:
                ...
                json_obj.update({'input': [ self.default(parameter) for parameter in obj.input ], 'ignored': [ self.default(parameter) for parameter in obj.ignored ], 'output': [ self.default(parameter) for parameter in obj.output ], 'annotations': [ self.default(annotation) for annotation in obj ]})
        ...
        if cls is kend.model.PageBox:
            ...
            if cls is kend.model.Extent:
                ...
                if obj.fault is not None:
                    ...
                    json_obj.update({'parameters': [ self.default(parameter) for parameter in obj.parameters ], 'annotations': [ self.default(annotation) for annotation in obj ]})
