...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyupp.py
# Compiled at: 2014-12-21 05:48:25
"""
...
import binascii, argparse, logging, six, struct, sys
...
def loads(data):
    ...
    while True:
        ...
        if valuetype == b'\xfe':
            ...
            assert strlen <= 127, repr((strlen, 127))
            value = reader.read(strlen)
            _debug(name, value, True)
            result[name] = value
...
def dumps(data):
    ...
    for k, v in data.items():
        ...
        if isinstance(v, six.string_types) or isinstance(v, bytes):
            ...
            raise AssertionError(repr(('Unknown type of data:', v)))
...
def _debug(msg, dat, is_string=False):
    return
    if not is_string:
        logging.debug('%s (%d bytes) => %s', msg, len(dat), binascii.hexlify(dat))
    else:
        logging.debug('%s (%d bytes) => %s', msg, len(dat), repr(dat))
