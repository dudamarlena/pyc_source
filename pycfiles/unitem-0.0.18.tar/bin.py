...
# [GCC 8.4.0]
# Embedded file name: /srv/whitlam/home/users/uqdparks/git/unitem/unitem/bin.py
# Compiled at: 2017-08-24 16:36:42
...
class Bin:
    ...
    def check_on_path(self, options):
        ...
        if options.mb_verysensitive or options.mb_sensitive or options.mb_specific or options.mb_veryspecific or options.mb_superspecific:
            self.logger.info('Checking MetaBAT dependencies.')
            check_on_path('metabat')
    ...
    def metabat(self, bin_file_out, preset):
        ...
        cmd = 'metabat -t %d -m %d -i %s -a %s -o %s --%s' % (self.cpus,
         self.min_contig_len,
         self.assembly_file,
         self.cov_file,
         bin_prefix,
         preset)
        self._run_method(cmd, bin_dir, bin_file_out, 'metabat')
