...
# [GCC 8.4.0]
# Embedded file name: /srv/whitlam/home/users/uqdparks/git/unitem/unitem/ensemble.py
# Compiled at: 2017-08-25 11:47:40
from __future__ import absolute_import
from __future__ import division
from __future__ import print_function
import os, sys, logging, copy, random
from shutil import copyfile
from collections import defaultdict
import biolib.seq_io as seq_io
from biolib.common import make_sure_path_exists
from unitem.defaults import *
from unitem.common import read_bins, parse_bin_stats, calculateN50L50M50
...
class Ensemble:
    ...
    def _bin_quality(self, bins, contigs, gene_tables, quality_weight):
        ...
        for bm, (bac_gene_tables, ar_gene_tables) in gene_tables.items():
            for bid in bac_gene_tables:
                domain, comp, cont = markers.evaluate(bac_gene_tables[bid], ar_gene_tables[bid])
            ...
            if not no_bin_matching and quality >= min_quality and comp >= min_comp and cont <= max_cont:
                for test_bm, test_bid, _domain, comp, cont, quality, N50, gs in bin_quality:
                    ...
                    if quality < min_quality or comp < min_comp or cont > max_cont:
                        continue
                    bp_in_common, total_bp1, total_bp2 = self._bases_in_common(bins[cur_bm][cur_bid], bins[test_bm][test_bid], contig_lens)
    ...
    def _bases_in_common(self, contig_ids1, contig_ids2, contig_lens):
        ...
        """
        bp_in_common = sum([ contig_lens[seq_id] for seq_id in contig_ids2 if seq_id in contig_ids1
                           ])
        total_bp1 = sum([ contig_lens[seq_id] for seq_id in contig_ids1 ])
        total_bp2 = sum([ contig_lens[seq_id] for seq_id in contig_ids2 ])
        return (
         bp_in_common, total_bp1, total_bp2)

    def _resolve_matched_set(self, matched_set, bins, contigs, bin_quality, remove_perc, add_perc, add_matches, sel_min_quality, sel_min_comp, sel_max_cont, greedy, unanimous):
        ...
        return (
         new_bin, removed_contigs, added_contigs)

    def _report_selection(self, bin_name, bin, markers, quality_weight, binning_method, original_bid, num_matched_bins, num_removed, num_added, greedy, unanimous):
    ...
    def _bin_summary(self, sel_bins, merged_bins, quality_weight, report_min_quality, markers, summary_file):
    ...
    def _perc_bases_in_common(self, query_bin, bins, contig_lens):
        ...
        for bm in bins:
            for bid in bins[bm]:
                bp_in_common, total_bp1, total_bp2 = self._bases_in_common(query_bin.keys(), bins[bm][bid], contig_lens)
    ...
    def run(self, profile_dir, bin_dirs, quality_weight, sel_min_quality, sel_min_comp, sel_max_cont, remove_perc, add_perc, add_matches, greedy, unanimous, merge, cov_file, report_min_quality, simple_headers, output_dir):
        """Perform ensemble binning of genomes across multiple binning methods.
        ...
        cov_file : str
          File containing coverage information for each contig.
        ...
        markers = Markers()
        if greedy:
            self.logger.info('Greedy selection with quality_weight = %.1f, sel_min_quality = %.1f, sel_min_comp = %.1f, and sel_max_cont = %.1f.' % (quality_weight, sel_min_quality, sel_min_comp, sel_max_cont))
            remove_perc = 101
            add_perc = 101
        else:
            if unanimous:
                self.logger.info('Unanimous selection with quality_weight = %.1f, sel_min_quality = %.1f, sel_min_comp = %.1f, and sel_max_cont = %.1f.' % (quality_weight, sel_min_quality, sel_min_comp, sel_max_cont))
            else:
                self.logger.info('Consensus selection with quality_weight = %.1f, sel_min_quality = %.1f, sel_min_comp = %.1f, and sel_max_cont = %.1f.' % (quality_weight, sel_min_quality, sel_min_comp, sel_max_cont))
            ...
            self.logger.info('Reporting bins with a quality >= %.1f.' % report_min_quality)
            ...
            fout_contig_init = open(os.path.join(output_dir, 'contig_info_initial.tsv'), 'w')
            fout_contig_init.write('Contig ID')
            for bm in methods_sorted:
                fout_contig_init.write('\t' + bm)

            fout_contig_init.write('\n')
            for cid in contigs_in_bins:
                fout_contig_init.write(cid)
                for bm in methods_sorted:
                    if bm not in contigs_in_bins[cid]:
                        fout_contig_init.write('\tunbinned')
                    else:
                        bid = contigs_in_bins[cid][bm]
                        domain, comp, cont = markers.bin_quality(bins[bm][bid])
                        fout_contig_init.write('\t%s,%.1f,%.1f' % (bid, comp, cont))

                fout_contig_init.write('\n')

            fout_contig_init.close()
            ...
            fout.write(header)
            fout_matched = open(os.path.join(output_dir, 'matched_set_info.tsv'), 'w')
            if greedy:
                fout_matched.write('No. Matched Bins\tBin ID\tCompleteness (%)\tContamination (%)\tQuality (%)\n')
            else:
                fout_matched.write('No. Matched Bins\tUniteM Bin ID\tCompleteness (%)\tContamination (%)\tQuality (%)')
                fout_matched.write('\tMatched Bin ID\tCompleteness (%)\tContamination (%)\tQuality (%)\n')
            fout_bin_info = open(os.path.join(output_dir, 'matched_bin_info.tsv'), 'w')
            fout_bin_info.write('UniteM Bin ID\tCompleteness (%)\tContamination (%)\tQuality (%)')
            fout_bin_info.write('\tNo. Matched Bins\tMatched Bin ID\tPercent Common Bases\tCompleteness (%)\tContamination (%)\tQuality (%)\n')
            fout_contigs = open(os.path.join(output_dir, 'contig_info.tsv'), 'w')
            fout_contigs.write('UniteM Bin ID\tContig ID\tNo. Matched Bins\tNo. Unmatched Bins\tNo. Degenerate Bins\tNo. Unbinned')
            ...
            while True:
                bin_quality = self._bin_quality(bins, contigs, gene_tables, quality_weight)
                matched_sets = self._matched_bin_sets(bins, contig_lens, bin_quality, sel_min_quality, sel_min_comp, sel_max_cont, greedy)
                if len(matched_sets) == 0:
                    break
                new_bin, removed_contigs, added_contigs = self._resolve_matched_set(matched_sets[0], bins, contigs, bin_quality, remove_perc, add_perc, add_matches, sel_min_quality, sel_min_comp, sel_max_cont, greedy, unanimous)
                ...
                if not greedy and not unanimous:
                    ...
                    if not greedy:
                        self.logger.info('-> Identified %d matched bins and removed %d contigs.' % (
                         len(matched_sets[0]),
                         len(removed_contigs)))
                    fout_matched.write('%d' % len(matched_sets[0]))
                    if not greedy:
                        fout_matched.write('\t%s\t%.1f\t%.1f\t%.1f' % (unitem_bin_id, comp, cont, quality))
                    for bm, bid, q, n50, gs in matched_sets[0]:
                        domain, comp, cont = markers.bin_quality(bins[bm][bid])
                        quality = comp - quality_weight * cont
                        fout_matched.write('\t%s\t%.1f\t%.1f\t%.1f' % (bm + '~' + bid, comp, cont, quality))

                    fout_matched.write('\n')
                    matches = self._perc_bases_in_common(new_bin, orig_bins, contig_lens)
                    fout_bin_info.write('%s\t%.1f\t%.1f\t%.1f\t%d' % (unitem_bin_id, comp, cont, quality, len(matches)))
                    for bm, bid, perc_common in matches:
                        ...
                        quality = comp - quality_weight * cont
                        fout_bin_info.write('\t%s\t%.1f\t%.1f\t%.1f\t%.1f' % (bm + '~' + bid, perc_common, comp, cont, quality))

                    fout_bin_info.write('\n')
                    output_file = os.path.join(plot_dir, unitem_bin_id + '.svg')
                    tree_common_bases.plot(unitem_bin_id, matches, output_file)
                    row = self._report_selection(unitem_bin_id, new_bin, markers, quality_weight, primary_bm, primary_bid, str(len(matched_sets[0])), str(len(removed_contigs)), str(len(added_contigs)), greedy, unanimous)
                    fout.write(row)
                    ...
                    new_bin_file = os.path.join(bin_dir, unitem_bin_id + '.fna')
                    fout_bin = open(new_bin_file, 'w')
                    for cid, seq in new_bin.items():
                        fout_contigs.write('%s\t%s' % (unitem_bin_id, cid))
                        matched = 0
                        unmatched = 0
                        unbinned = 0
                        degenerate = 0
                        ...
                        for m in methods_sorted:
                            if m in contigs_in_bins[cid]:
                                ...
                                if m in matched_bins and bid == matched_bins[m]:
                                    row += '\t%s,%s,%.1f,%.1f' % ('matched', bid, comp, cont)
                                    matched += 1
                                else:
                                    q = comp - quality_weight * cont
                                    if q < sel_min_quality or comp < sel_min_comp or cont > sel_max_cont:
                                        row += '\t%s,%s,%.1f,%.1f' % ('degenerate', bid, comp, cont)
                                        degenerate += 1
                                    else:
                                        row += '\t%s,%s,%.1f,%.1f' % ('unmatched', bid, comp, cont)
                                        unmatched += 1
                            else:
                                row += '\tunbinned'
                                unbinned += 1

                        fout_contigs.write('\t%d\t%d\t%d\t%d' % (matched, unmatched, degenerate, unbinned))
                        fout_contigs.write(row + '\n')
                        if simple_headers:
                            fout_bin.write('>%s\n' % cid)
                        else:
                            cid_info = '[matched=%d] [unmatched=%d] [degenerate=%d] [unbinned=%d]' % (matched, unmatched, degenerate, unbinned)
        ...
        self._bin_summary(sel_bins, {}, quality_weight, report_min_quality, markers, summary_file)