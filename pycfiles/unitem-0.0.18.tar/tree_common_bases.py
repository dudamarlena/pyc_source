...
# [GCC 8.4.0]
# Embedded file name: /srv/whitlam/home/users/uqdparks/git/unitem/unitem/tree_common_bases.py
# Compiled at: 2017-08-25 13:59:18
...
class TreeCommonBases:
    ...
    def _render_scale(self, dwg):
        ...
        for s in [50, 60, 70, 80, 90, 100]:
            ...
            t = dwg.text(int(s), x=[
             xpos], y=[
             scale_y_pos + 0.75 * self.row_height + self.label_offset], font_size='%fpt' % self.scale_font_size, text_anchor='middle', fill='rgb(0,0,0)')
            scale_text_group.add(t)
            if s != 50 and s != 100:
                tick = dwg.line(start=(xpos, scale_y_pos), end=(
                 xpos, scale_y_pos + 0.25 * self.row_height), fill='black', stroke_width=0.5)
    ...
    def _render_labels(self, dwg, bin_id, common_bases):
        ...
        dwg.add(label_group)
        t = dwg.text(bin_id, x=[
         self.fig_size_x + self.label_offset], y=[
         self.fig_size_y + 0.5 * self.font_size], font_size='%fpt' % self.font_size, direction='ltr', fill='rgb(0,0,0)')
        label_group.add(t)
        y = self.fig_size_y
        for bm, bid, cb in common_bases:
            ...
            t = dwg.text(bm, x=[
             self.fig_size_x + self.label_offset], y=[
             y + 0.5 * self.font_size], font_size='%fpt' % self.font_size, direction='ltr', fill='rgb(0,0,0)')
            label_group.add(t)
    ...
    def _render_tree(self, dwg, common_bases):
        ...
        root = dwg.line(start=(x_start, y_start), end=(
         0, y_start), fill='black', stroke_width=1)
    ...
    def plot(self, bin_id, common_bases, output_plot):
        ...
        if not output_plot.endswith('.svg'):
            output_plot += '.svg'
        self.fig_size_x = 5 * self.scale_interval_width
