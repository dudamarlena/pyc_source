...
# [GCC 8.4.0]
# Embedded file name: build/bdist.darwin-8.11.1-i386/egg/traccas/traccas.py
# Compiled at: 2008-05-04 19:34:16
...
class CasLoginModule(LoginModule):
    ...
    login_path = Option('cas', 'login_path', default='/login', doc='Path component for the login system')
    logout_path = Option('cas', 'logout_path', default='/logout', doc='Path component for the logout system')
    validate_path = Option('cas', 'validate_path', default='/validate', doc='Path component for the validation system')
    ...
    def get_navigation_items(self, req):
        if req.authname and req.authname != 'anonymous':
            yield (
             'metanav', 'login', 'logged in as %s' % req.authname)
            yield ('metanav', 'logout', tag.a('Logout', href=req.href.logout()))
        else:
            yield (
             'metanav', 'login', tag.a('Login', href=self.cas.login_url(req.abs_href.login())))
    ...
    def cas(self):
        paths = {'login_path': self.login_path, 'logout_path': self.logout_path, 'validate_path': self.validate_path}
