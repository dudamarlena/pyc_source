...
# [GCC 8.4.0]
# Embedded file name: /home/omerw/git/leap/leap/regression/leapUtils.py
# Compiled at: 2014-12-25 10:53:38
import numpy as np
from optparse import OptionParser
import scipy.linalg as la, scipy.linalg.blas as blas, csv, time, fastlmm.util.VertexCut as vc
from pysnptools.snpreader.bed import Bed
import pysnptools.util as pstutil, pysnptools.util.pheno as phenoUtils
...
def checkIntersection(bed, fileDict, fileStr, checkSuperSet=False):
    ...
    if checkSuperSet:
        if not fileSet.issuperset(bedSet):
            raise Exception(fileStr + ' file does not include all individuals in the bfile')
...
def findRelated(bed, cutoff):
    ...
    removeSet = set(np.sort(vc.VertexCut().work(XXT, cutoff)))
...
def getSNPCovarsMatrix(bed, resfile, pthresh, mindist):
    ...
    for i, s in enumerate(bed.sid):
        snpNameToNumDict[s] = i
    ...
    for l in csvReader:
        ...
        if pVal < lastPval:
            raise Exception('P-values are not sorted in descending order: ' + str(pVal) + '>' + str(lastPval))
        lastPval = pVal
        if pVal > pthresh:
            break
        if snpName not in snpNameToNumDict:
            continue
