...
# [GCC 8.4.0]
# Embedded file name: /home/omerw/git/leap/leap/probit.py
# Compiled at: 2015-08-01 05:40:04
import numpy as np, scipy.stats as stats, scipy.linalg as la, time, sklearn.linear_model, sys, argparse, scipy.optimize as opt, scipy.linalg.blas as blas, leapUtils, leapMain
...
def evalProbitReg(beta, X, cases, controls, thresholds, invRegParam, normPDF, h2):
    ...
    grad = X.T.dot(w)
    logLik -= 0.5 * invRegParam * beta.dot(beta)
...
def probitRegression(X, y, thresholds, numSNPs, numFixedFeatures, h2, useHess, maxFixedIters, epsilon, nofail):
    ...
    if not optObj.success:
        ...
        if nofail == 0:
            raise Exception('Probit regression failed with message: ' + optObj.message)
    beta = optObj.x
...
def probit(bed, pheno, h2, prev, eigen, outFile, keepArr, covar, thresholds, nofail, numSkipTopPCs, mineig, hess, recenter, maxFixedIters, epsilon, treatFixedAsRandom=False):
    ...
    if keepArr is None:
        keepArr = np.ones(phe.shape[0], dtype=np.bool)
    S = eigen['arr_1'] * bed.sid.shape[0]
    U = eigen['arr_0']
    S = np.sqrt(S)
    goodS = S > mineig
    if numSkipTopPCs > 0:
        goodS[(-numSkipTopPCs):] = False
    if np.sum(~goodS) > 0:
    ...
    if outFile is not None:
        ...
        if numFixedFeatures > 0:
            ...
            for ind_i, (fid, iid) in enumerate(bed.iid):
                f.write((' ').join([fid, iid, '%0.3f' % liab_nofixed[ind_i]]) + '\n')
            ...
            for ind_i, (fid, iid) in enumerate(bed.iid):
                f.write((' ').join([fid, iid, '%0.3f' % liab_nofixed2[ind_i]]) + '\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pthresh', metavar='pthresh', type=float, default=5e-08, help='p-value cutoff below which SNPs will be used as fixed effects')
    parser.add_argument('--mineig', metavar='mineig', type=float, default=0.001, help='eigenvectors with singular value below this value will not be used')
    ...
    parser.add_argument('--epsilon', metavar='epsilon', type=float, default=0.001, help='Convergence cutoff for fitting of fixed effects')
    parser.add_argument('--missingPhenotype', metavar='missingPhenotype', default='-9', help='identifier for missing values (default: -9)')
    args = parser.parse_args()
    if args.extract is not None and args.bfile is None:
        raise Exception('--extract cannot be used without --bfile')
    if args.bfile is not None and args.resfile is None:
        raise Exception('--bfile cannot be used without --resfile')
    if args.bfilesim is None:
        raise Exception('bfilesim must be supplied')
    if args.pheno is None:
        raise Exception('phenotype file must be supplied')
    if args.out is None:
        raise Exception('output file name must be supplied')
    if args.prev is None:
        raise Exception('prevlence must be supplied')
    if args.h2 is None:
        raise Exception('heritability must be supplied')
    ...
    if args.covar is not None:
        ...
        if covar is None:
            ...
            covar = np.concatenate((covar, covarsMat), axis=1)
    if args.thresholds is not None:
        thresholds = np.loadtxt(args.thresholds, usecols=[0])
    else:
        thresholds = None
    leapMain.probit(bed, phe, args.h2, args.prev, eigen, args.out, keepArr, covar, thresholds, args.nofail == 1, args.numSkipTopPCs, args.mineig, args.hess == 1, args.recenter == 1, args.maxFixedIters, args.epsilon, treatFixedAsRandom=args.treatFixedAsRandom >= 1)