...
# [GCC 8.4.0]
# Embedded file name: /home/omerw/git/leap/leap/regression/calc_h2.py
# Compiled at: 2015-04-03 04:19:49
import numpy as np, argparse, scipy.stats as stats, scipy.linalg.blas as blas, sklearn.linear_model, time, sys
np.set_printoptions(precision=3, linewidth=200)
import leapUtils, leapMain
...
def calcLiabThreholds(U, S, keepArr, phe, numRemovePCs, prev):
    ...
    thresholds[Ki >= 1.0] = -999999999
    thresholds[Ki <= 0.0] = 999999999
    return (
     Pi, thresholds)
...
def calc_h2(pheno, prev, eigen, keepArr, numRemovePCs, h2coeff, lowtail):
    ...
    if np.isnan(h2):
        raise Exception('Invalid heritability estimate. Please double-check your input for any errors.')
    print 'h2: %0.6f' % h2
    return h2
...
if __name__ == '__main__':
    ...
    if args.prev is None:
        raise Exception('--prev must be supplied')
    if args.pheno is None:
        raise Exception('--pheno must be supplied')
