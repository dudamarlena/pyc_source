...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/emrt/necd/content/utilities/roles.py
...
def context_aware_query(context):
    ...
    return ldap_utils.format_or('cn', (
...
def get_ldap_role_filters(context):
    ...
    return (
     f_start_msa, f_start_lr, f_start_se)
...
def setup_reviewfolder_roles(folder):
    ...
    grant = chain(product([ROLE_MSA], filter(f_start_msa, groups)), product([ROLE_LR], filter(f_start_lr, groups)), product([ROLE_SE], filter(f_start_se, groups)))
