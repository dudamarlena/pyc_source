...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/emrt/necd/content/reviewfolder.py
# Compiled at: 2020-04-15 11:34:39
import itertools, time, tablib
...
import plone.directives, zope.schema as schema
...
QUESTION_WORKFLOW_MAP = {'SE': 'Sector Expert', 
   'LR': 'Lead Reviewer', 
   'MSC': 'MS Coordinator', 
...
def get_highlight_vocabs(context):
    vocab_highlight = getUtility(IVocabularyFactory, name='emrt.necd.content.highlight')(context)
    ...
    vocab_description_flags = translate_highlights(vocab_highlight, vocab_highlight_values[:highlight_split])
    vocab_conclusion_flags = translate_highlights(vocab_highlight, vocab_highlight_values[highlight_split:])
    return (
...
class IReviewFolder(directives.form.Schema, IImageScaleTraversable):
    ...
    """
    type = schema.Choice(title='Type', source=REVIEWFOLDER_TYPES, required=True)
    tableau_statistics = schema.Text(title='Tableau statistics embed code', required=False)
    plone.directives.form.widget(tableau_statistics_roles=CheckBoxFieldWidget)
    tableau_statistics_roles = schema.List(title='Roles that can access the statistics', value_type=schema.Choice(vocabulary='emrt.necd.content.roles'))
...
class ReviewFolderMixin(BrowserView):
    ...
    def get_nfr_categories(self):
        vocab_factory = getUtility(IVocabularyFactory, name='emrt.necd.content.nfr_code')
        vocabulary = vocab_factory(self.context)
        return [ (x.value, x.title) for x in vocabulary ]

    def get_sector_names(self):
        vocab_factory = getUtility(IVocabularyFactory, name='emrt.necd.content.sector_names')
        vocabulary = vocab_factory(self.context)
    ...
    def is_member_state_expert(self):
        ldap_wrapper = getUtility(IGetLDAPWrapper)(self.context)
        return partial(user_has_ldap_role, LDAP_MSEXPERT, ldap_wrapper=ldap_wrapper)
...
class ReviewFolderBrowserView(ReviewFolderMixin):
    ...
    def folderitems(self, sort_on='modified', sort_order='reverse'):
        """
        """
        return [ dict(brain=brain) for brain in self.get_questions(sort_on, sort_order)
               ]
...
IS_FIELD_MAP = {'get_is_adjustment': 'Adjustment', 
...
class ExportReviewFolderForm(form.Form, ReviewFolderMixin):
    ...
    def extract_data(self, form_data):
        ...
        user_is_manager = 'Manager' in api.user.get_roles()
        skip_for_user = user_is_ms and not user_is_manager
        fields_to_export = [ name for name in form_data.get('exportFields', []) if not skip_for_user or name not in EXCLUDE_FIELDS_FOR_MS
                           ]
        ...
        base_len = 0
        rows = []
        vocab_description_flags, vocab_conclusion_flags, vocab_highlight = get_highlight_vocabs(self.context)
        for observation in observations:
            row = [
             observation.getId]
            highlights = translate_highlights(vocab_highlight, observation['get_highlight'] or [])
            for key in fields_to_export:
                if key == 'observation_is_potential_technical_correction':
                    ...
                    row.append((', ').join(get_common(highlights, vocab_description_flags)))
                elif key == 'get_conclusion_flags':
                    row.append((', ').join(get_common(highlights, vocab_conclusion_flags)))
                    ...
                    row.append(QUESTION_WORKFLOW_MAP.get(observation['observation_status'], observation['observation_status']))
                elif key == 'fuel':
                    fuel = get_vocabulary_value(self.context.aq_parent, 'emrt.necd.content.fuel', observation.getObject().fuel)
                    row.append(fuel)
                elif key == 'modified':
    ...
    def build_file(self, data):
        ...
        """
        now = datetime.now()
        filename = ('EMRT-observations-{}_{}.xls').format(self.context.getId(), now.strftime('%d-%m-%Y_%H:%M'))
        book = tablib.Databook((self.extract_data(data),))
        response = self.request.response
        response.setHeader('content-type', 'application/vnc.ms-excel')
        response.setHeader('Content-disposition', ('attachment;filename="{filename}"').format(filename=filename))
...
class RoleMapItem(object):
    ...
    def check_roles(self, rolename):
        if rolename == ROLE_CP:
            return self.isCP
        if rolename == ROLE_MSA:
            return self.isMSA
        if rolename == ROLE_MSE:
            return self.isMSE
        if rolename == ROLE_SE:
            return self.isSE
        if rolename == 'NotCounterPart':
...
class InboxReviewFolderView(BrowserView):
    ...
    def get_draft_observations(self):
        ...
        """
        return self.get_observations(rolecheck=ROLE_SE, observation_question_status=[
         'observation-draft'])
    ...
    def get_draft_questions(self):
        ...
        """
        conclusions = self.get_observations(rolecheck=ROLE_SE, observation_question_status=[
         'draft',
         'drafted'])
        no_conclusion_yet = self.get_observations(observation_question_status=[
         'closed'], observation_finalisation_reason='no-conclusion-yet')
    ...
    def get_counterpart_questions_to_comment(self):
        ...
        """
        return self.get_observations(rolecheck=ROLE_CP, observation_question_status=[
    ...
    def get_ms_answers_to_review(self):
        """
         ...
         that need review
        """
        answered = self.get_observations(rolecheck=ROLE_SE, observation_question_status=[
         'answered'])
        pending = self.get_observations(rolecheck=ROLE_SE, observation_question_status=[
         'closed'], review_state=[
         'pending'])
        return answered + pending

    def get_draft_conclusions(self):
        return self.get_observations(rolecheck=ROLE_SE, review_state=[
         'conclusions'])
    ...
    def get_denied_observations(self):
        ...
        """
        observations = self.get_observations(rolecheck=ROLE_SE, review_state=[
         'conclusions', 'conclusions-lr-denied'])
    ...
    def get_approval_questions(self):
        ...
        statuses = [
         'drafted',
         'recalled-lr']
        return self.get_observations(rolecheck=ROLE_SE, observation_question_status=statuses)
    ...
    def get_waiting_for_comment_from_counterparts_for_question(self):
        """
         Role: Sector Expert
        """
        return self.get_observations(rolecheck='NotCounterPart', observation_question_status=[
         'counterpart-comments'])
    ...
    def get_questions_to_be_sent(self):
        ...
        """
        return self.get_observations(rolecheck=ROLE_LR, observation_question_status=[
         'drafted',
         'recalled-lr'])
    ...
    def get_questions_to_be_answered(self):
        ...
        """
        return self.get_observations(rolecheck=ROLE_MSA, observation_question_status=[
         'pending',
         'recalled-msa',
         'pending-answer-drafting'])
    ...
    def get_questions_with_comments_received_from_mse(self):
        ...
        """
        return self.get_observations(rolecheck=ROLE_MSA, observation_question_status=[
         'expert-comments'], last_answer_has_replies=True)
    ...
    def get_answers_sent_to_se_re(self):
        """
         ...
         Answers sent to SE
        """
        answered = self.get_observations(rolecheck=ROLE_MSA, observation_question_status=[
         'answered'])
        ...
        not_closed = self.get_observations(rolecheck=ROLE_MSA, review_state=statuses, observation_already_replied=True)
    ...
    def get_questions_with_comments_for_answer_needed_by_msc(self):
        ...
        """
        return self.get_observations(rolecheck=ROLE_MSE, observation_question_status=[
         'expert-comments'])
    ...
    def get_observations_with_my_comments(self):
        ...
        return self.get_observations(rolecheck=ROLE_MSE, observation_question_status=[
         'expert-comments',
         'pending-answer-drafting'], reply_comments_by_mse=True)
    ...
    def get_observations_with_my_comments_sent_to_se_re(self):
        ...
        return self.get_observations(rolecheck=ROLE_MSE, observation_question_status=[
         'answered',
         'recalled-msa'], reply_comments_by_mse=True)
    ...
    def is_sector_expert(self):
        ldap_wrapper = getUtility(IGetLDAPWrapper)(self.context)
        return partial(user_has_ldap_role, LDAP_SECTOREXP, ldap_wrapper=ldap_wrapper)

    def is_lead_reviewer(self):
        ldap_wrapper = getUtility(IGetLDAPWrapper)(self.context)
        return partial(user_has_ldap_role, LDAP_LEADREVIEW, ldap_wrapper=ldap_wrapper)
...
class FinalisedFolderView(BrowserView):
    ...
    def get_reasons_with_observations(self):
        ...
        return dict(reasons=reason_obs, total_obs=sum(obs['num_obs'] for obs in reason_obs.values()))
    ...
    def get_resolved_observations(self, reason):
        ...
        return self.get_observations(observation_question_status=[
         'closed'], observation_finalisation_reason=reason)
