...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\hnc\forms\validators.py
# Compiled at: 2013-08-17 16:09:20
import re
...
class SanitizedHTMLString(formencode.validators.String):
    ...
    valid_tags = [
     'a', 'strong', 'em', 'p', 'ul', 'ol', 'li', 'br', 'b', 'i', 'u', 's', 'strike', 'font', 'pre', 'blockquote', 'div', 'span', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6']
    valid_attrs = ['size', 'color', 'face', 'title', 'align', 'style']

    def linkAttrs(self, attrs):
        return [
         (
          'href', attrs.get('href')), ('target', '_blank')]
    ...
    def _to_python(self, value, state):
        ...
        if val not in self.list:
            if self.hideList:
                ...
                raise Invalid(self.message('notIn', state, items=items, val=val), val, state)
        return val
...
class OneOfStateChoice(OneOfChoice):
    ...
    def getItems(self, request):
        obj = request
        for key in self.stateKey.split('.'):
            obj = getattr(obj, key)
    ...
    def _to_python(self, value, state):
        ...
        if len(set(values).difference(self.list)):
            if self.hideList:
                ...
                raise Invalid(self.message('notIn', state, items=items, val=values), val, state)
        return values
...
class DecimalValidator(formencode.FancyValidator):
    ...
    step = 0.01
    messages = {'invalid_amount': _('Bitte eine Zahl eingeben'), 'amount_too_high': _('Bitte eine Zahl %(max_amount)s oder kleiner eingeben'), 
...
class LessThanEach(formencode.validators.FormValidator):
    ...
    messages = dict(notLess=_('Field should be greater than %(ref)s'), notDict=_('Fields should be a dictionary'))
...
class SettlementValidator(formencode.validators.FormValidator):
    ...
    messages = {'MissingAField': _('SETTLEMENT_PAYPAL_EMAIL_MISSIN_Please enter a valid value'), 
...
class URL(formencode.FancyValidator):
    ...
    def _to_python(self, value, state):
        ...
        if self.require_tld and not match.group('domain'):
            raise Invalid(self.message('noTLD', state, domain=match.group('tld')), value, state)
        if self.check_exists and (value.startswith('http://') or value.startswith('https://')):
