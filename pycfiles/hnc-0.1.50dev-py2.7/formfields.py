...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\hnc\forms\formfields.py
# Compiled at: 2013-11-26 08:38:51
...
class DependentAttrs(HtmlAttrs):
    ...
    def getGroupClasses(self):
        return ('{} dependent-control').format(self.getClasses())
...
class TagSearchField(StringField):
    ...
    def __init__(self, name, label, api_url, api_result, **kwargs):
        ...
        self.api_url = api_url
        if kwargs.get('query_extra'):
            self.query_extra = simplejson.dumps(kwargs.get('query_extra')).replace('"', '&quot;')
        else:
