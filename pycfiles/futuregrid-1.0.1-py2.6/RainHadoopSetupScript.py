...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/futuregrid/rain/RainHadoopSetupScript.py
...
def create_hdfs_site(master_node, dfs_name_dir, dfs_data_dir, ports):
    (doc, config_element) = get_config_document()
    config_element.appendChild(create_property('dfs.http.address', master_node + ':' + str(ports[1]), doc))
...
def create_mapred_site(master_node_ip, mapred_local_dir, ports):
    ...
    config_element.appendChild(create_property('mapred.job.tracker.http.address', master_node_ip + ':' + str(ports[3]), doc))
...
def generate_hadoop_configs(nodes, local_base_dir, conf_dir, tempdir):
    ...
    return (
     hdfs_site_doc, core_site_doc, mapred_site_doc)
