...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/futuregrid/rain/One.py
...
import xmlrpclib, time, argparse
...
class OpenNebulaTest(object):
    ...
    def boot_VM(self, server, vmfile, n):
        ...
        if ok == n:
            ...
            fail = False
            allrunning = False
            while not allrunning and retry < maxretry and not fail:
                running = 0
                for i in range(n):
                    try:
                        vminfo = server.one.vm.info(self.oneauth, vm[i][1])
                ...
                if running == n:
                    ...
                    time.sleep(5)
                if retry >= maxretry:
                    self.logger.error('The VMs did not change to runn status. Please verify that the status of the OpenNebula hosts or increase the wait time in the configuration file (max_wait) \n')
                    fail = True
...
def main():
    parser = argparse.ArgumentParser(prog='One', formatter_class=argparse.RawDescriptionHelpFormatter, description='One Help ')
