...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/futuregrid/image/repository/server/IRTypes.py
...
class IRUser(object):
    ...
    def __init__(self, userId, cred=None, fsCap=8589934592, fsUsed=0, lastLogin=None, status=Status[0], role=Role[0], ownedImgs=0):
...
class ImgMeta(object):
    ...
    metaArgsIdx = {'imgId': 0, 
       ...
       'imgstatus': 9}
    VmType = [
     'none', 'xen', 'kvm', 'virtualbox', 'vmware']
    ...
    def __init__(self, imgId, os, arch, owner, description, tag, vmType='none', imgType='machine', permission='private', imgStatus='available'):
...
class ImgEntry(object):
    ...
    def __init__(self, imgId, imgMeta, imgURI, size, extension, createdDate=datetime.utcnow(), lastAccess=datetime.utcnow(), accessCount=0):
