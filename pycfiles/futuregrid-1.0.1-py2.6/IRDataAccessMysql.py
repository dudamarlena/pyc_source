...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/futuregrid/image/repository/server/IRDataAccessMysql.py
...
class ImgStoreMysql(AbstractImgStore):
    ...
    def mysqlConnection(self):
        ...
        connected = False
        try:
            self._dbConnection = MySQLdb.connect(host=self._mysqlAddress, db=self._dbName, read_default_file=self._configFile, user=self._userAdmin)
            connected = True
        except MySQLdb.Error, e:
            self._log.error('Error %d: %s' % (e.args[0], e.args[1]))
        except TypeError, detail:
...
class ImgMetaStoreMysql(AbstractImgMetaStore):
    ...
    def queryStore(self, criteria):
        ...
        if self.mysqlConnection():
            try:
                try:
                    cursor = self._dbConnection.cursor()
                    criteria = string.lower(criteria.strip())
    ...
    def convertDicToObject(self, dic, fullMode):
        ...
        """
        if fullMode:
            tmpMeta = ImgMeta(dic[0], dic[1], dic[2], dic[3], dic[4], dic[5], dic[6], dic[7], dic[8], dic[9])
        else:
            tmpMeta = ImgMeta('', '', '', '', '', '', '', 0, '', '')
        for i in dic.keys():
            if i == 'imgid':
        ...
        if self.mysqlConnection():
            try:
                try:
                    cursor = self._dbConnection.cursor()
                    for item in items:
                        sql = "INSERT INTO %s (imgId, os, arch, vmType,                     imgType, permission, owner, imgStatus, description, tag)        VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s' , '%s', '%s')" % (
...
class IRUserStoreMysql(AbstractIRUserStore):
    """
                ...
                try:
                    ...
                    if self.isAdmin(userId):
                        if userIdtoSearch == None:
                            ...
                            results = cursor.fetchall()
                            for result in results:
                                tmpUser[result[0]] = IRUser(result[0], result[1], int(result[2]), int(result[3]), result[4], result[5], result[6], result[7])
                                found = True
                            ...
                            results = cursor.fetchone()
                            if results != None:
                                tmpUser[results[0]] = IRUser(results[0], results[1], int(results[2]), int(results[3]), results[4], results[5], results[6], results[7])
                ...
                try:
                    ...
                    if authorized:
                        for user in users:
                            ...
                            sql = "INSERT INTO %s (userId, cred, fsUsed, fsCap,                         lastLogin, status, role, ownedImgs)            VALUES ('%s', '%s', '%d', '%d', '%s', '%s', '%s', %d)" % (
