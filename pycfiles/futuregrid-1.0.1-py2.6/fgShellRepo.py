...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/futuregrid/shell/fgShellRepo.py
...
class fgShellRepo(Cmd):
    ...
    def help_repohistimg(self):
        msg = 'Image Repository histimg command: Return information about the ' + ' image historical usage. \n'
    ...
    def help_repohistuser(self):
        msg = 'Image Repository histuser command: Return information about the ' + 'user historical usage.'
    ...
    def do_repouser(self, args):
        ...
        parser = argparse.ArgumentParser(prog='repouser', formatter_class=argparse.RawDescriptionHelpFormatter, description='User command ')
        ...
        group1.add_argument('-m', '--modify', dest='modify', nargs=3, metavar=('userId',
                                                                               'quota/role/status',
                                                                               'value'), help='Modify quota, role or status of an user')
        ...
        if '-m' in used_args or '--modify' in used_args:
            if args.modify[1].strip() == 'quota':
                ...
                if args.modify[2] not in IRTypes.IRUser.Role:
                ...
                if args.modify[2] not in IRTypes.IRUser.Status:
    ...
    def help_repomodify(self):
        msg = 'Image Repository modify command: Modify image metadata. Example ' + 'of all values of attributeString (you do not need to provide all of ' + 'them): vmtype=xen & imgtype=opennebula & os=linux & arch=x86_64 & ' + 'description=my image & tag=tag1,tag2 & permission=public & ' + 'imgStatus=available. Some attributes are controlled:'
    ...
    def help_reposetpermission(self):
        msg = 'Image Repository setPermission command: Change image permission.' + ' Permission= ' + str(IRTypes.ImgMeta.Permission)
    ...
    def help_repoput(self):
        msg = 'Image Repository put command: Upload a new image and its ' + 'metadata as an attributeString. If no attributeString provided some default values are ' + 'assigned. Example of all values of attributeString (you do not need to ' + 'provide all of them): vmtype=xen & imgtype=opennebula & os=linux & ' + 'arch=x86_64 & description=my image & tag=tag1,tag2 & permission=public & ' + 'imgStatus=available. Some attributes are controlled:'
