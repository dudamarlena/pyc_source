...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/futuregrid/image/management/IMRegister.py
...
import hashlib, time, boto.ec2, boto
...
class IMRegister(object):
    ...
    def iaas_justregister(self, machinename, image, image_source, ramdisk, iaas_type, varfile, wait):
        ...
        if stat == 0:
            ...
            getimg = False
            output = eval('self.' + iaas_type + '_method("' + str(realnameimg) + '","' + str(self.kernel) + '","' + str(ramdisk) + '","' + str(operatingsystem) + '","' + str(machinename) + '","' + str(varfile) + '","' + str(getimg) + '","' + str(wait) + '","' + str(delorigin) + '")')
            end = time.time()
    ...
    def iaas_generic(self, machinename, image, image_source, iaas_type, varfile, getimg, ldap, wait):
        ...
        try:
            iaasServer = ssl.wrap_socket(s, ca_certs=self._ca_certs, certfile=self._certfile, keyfile=self._keyfile, cert_reqs=ssl.CERT_REQUIRED, ssl_version=ssl.PROTOCOL_TLSv1)
            iaasServer.connect((self.iaasmachine, self._iaas_port))
            msg = str(image) + ',' + str(image_source) + ',' + str(machinename) + ',' + str(iaas_type) + ',' + str(self.kernel) + ',' + str(self.user) + ',' + str(self.passwd) + ',ldappassmd5' + ',' + str(ldap)
                ...
                if re.search('^ERROR', ret):
                    ...
                    if len(results) == 4:
                        ...
                        if imagebackpath != None:
                            start = time.time()
                            delorigin = True
                            output = eval('self.' + iaas_type + '_method("' + str(imagebackpath) + '","' + str(eki) + '","' + str(eri) + '","' + str(operatingsystem) + '","' + str(machinename) + '","' + str(varfile) + '","' + str(getimg) + '","' + str(wait) + '","' + str(delorigin) + '")')
                            end = time.time()
                            self._log.info('TIME uploading image to cloud framework:' + str(end - start))
                            if wait:
                                self.wait_available(iaas_type, varfile, output)
    ...
    def nimbus_method(self, imagebackpath, eki, eri, operatingsystem, machinename, varfile, getimg, wait, delorigin):
        if not eval(getimg):
            ...
            if isinstance(nimbusEnv, IMEc2Environ):
                ...
                try:
                    s3conn = S3Connection(nimbusEnv.getS3id(), nimbusEnv.getS3key(), host=nimbusEnv.getS3_url().lstrip('http://').split(':')[0], port=nimbusEnv.getS3_port(), is_secure=False, calling_format=cf)
                except:
    ...
    def euca_method(self, imagebackpath, eki, eri, operatingsystem, machinename, varfile, getimg, wait, delorigin):
        ...
        if not eval(getimg):
            ...
            cmd = 'euca-bundle-image --cert ' + str(os.getenv('EC2_CERT')) + ' --privatekey ' + str(os.getenv('EC2_PRIVATE_KEY')) + ' --user ' + str(os.getenv('EC2_USER_ID')) + ' --ec2cert ' + str(os.getenv('EUCALYPTUS_CERT')) + ' --url ' + str(eucaEnv.getEc2_url()) + ' -a ' + str(os.getenv('EC2_ACCESS_KEY')) + ' -s ' + str(os.getenv('EC2_SECRET_KEY')) + ' --image ' + str(imagebackpath) + ' --destination ' + tempdir
            if eki != 'None':
                cmd += ' --kernel ' + str(eki)
            if eri != 'None':
                cmd += ' --ramdisk ' + str(eri)
            ...
            if stat != 0:
                ...
                self._log.error(msg)
                return msg + '\n ' + errormsg
            cmd = 'euca-upload-bundle -a ' + os.getenv('EC2_ACCESS_KEY') + ' -s ' + os.getenv('EC2_SECRET_KEY') + ' --url ' + eucaEnv.getS3_url() + ' --ec2cert ' + os.getenv('EUCALYPTUS_CERT') + ' --bucket ' + self.user + ' --manifest ' + tempdir + filename + '.manifest.xml'
            print cmd
            self._log.debug(cmd)
            stat = os.system(cmd)
            if stat != 0:
    ...
    def openstack_method(self, imagebackpath, eki, eri, operatingsystem, machinename, varfile, getimg, wait, delorigin):
        ...
        if not eval(getimg):
            ...
            tempdir = '/tmp/' + str(randrange(999999999999999999999999)) + str(time.time()) + '/'
            os.system('mkdir -p ' + tempdir)
            cmd = 'euca-bundle-image --cert ' + os.path.expanduser(os.path.expandvars(os.getenv('EC2_CERT'))) + ' --privatekey ' + os.path.expanduser(os.path.expandvars(os.getenv('EC2_PRIVATE_KEY'))) + ' --user ' + str(os.getenv('EC2_USER_ID')) + ' --ec2cert ' + str(os.getenv('EUCALYPTUS_CERT')) + ' --url ' + str(openstackEnv.getEc2_url()) + ' -a ' + str(os.getenv('EC2_ACCESS_KEY')) + ' -s ' + str(os.getenv('EC2_SECRET_KEY')) + ' --image ' + str(imagebackpath) + ' --destination ' + tempdir
            if eki != 'None':
                cmd += ' --kernel ' + str(eki)
            if eri != 'None':
                cmd += ' --ramdisk ' + str(eri)
            ...
            if stat != 0:
                ...
                self._log.error(msg)
                return msg + '\n ' + errormsg
            cmd = 'euca-upload-bundle -a ' + os.getenv('EC2_ACCESS_KEY') + ' -s ' + os.getenv('EC2_SECRET_KEY') + ' --url ' + openstackEnv.getS3_url() + ' --ec2cert ' + os.getenv('EUCALYPTUS_CERT') + ' --bucket ' + self.user + ' --manifest ' + tempdir + filename + '.manifest.xml'
            print cmd
            self._log.debug(cmd)
            stat = os.system(cmd)
            if stat != 0:
    ...
    def xcat_method(self, xcat, image, operation):
        ...
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        try:
            xcatServer = ssl.wrap_socket(s, ca_certs=self._ca_certs, certfile=self._certfile, keyfile=self._keyfile, cert_reqs=ssl.CERT_REQUIRED, ssl_version=ssl.PROTOCOL_TLSv1)
        ...
        if operation != 'kernels':
            ...
            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            try:
                moabServer = ssl.wrap_socket(s, ca_certs=self._ca_certs, certfile=self._certfile, keyfile=self._keyfile, cert_reqs=ssl.CERT_REQUIRED, ssl_version=ssl.PROTOCOL_TLSv1)
...
def main():
    parser = argparse.ArgumentParser(prog='fg-register', formatter_class=argparse.RawDescriptionHelpFormatter, description='FutureGrid Image Registration Help ')
    ...
    parser.add_argument('-j', '--justregister', dest='justregister', action='store_true', default=False, help='It assumes that the image is ready to run in the selected infrastructure. Thus, no additional configuration will be performed. Only valid for Cloud infrastructures. (This is basically a wrapper of the tools that register images into the cloud infrastructures)')
    ...
    if args.listsites:
        ...
        if '-e' in used_args or '--euca' in used_args:
            if checkVarFile(varfile, args.getimg, args.listkernels, 'Eucalyptus'):
                if args.listkernels:
                    ...
                    if output != None:
                        print isinstance(output, list) or output
                    else:
                        print 'The list of available images on Eucalyptus ' + str(args.euca) + ' is:'
                        for i in output:
                            print i

                        print 'You can get more details by querying the image repository using fg-repo -q command and the query string: "* where tag=imagename". \n' + 'NOTE: To query the repository you need to remove the OS from the image name (centos,ubuntu,debian,rhel...). ' + 'The real name starts with the username and ends before .img.manifest.xml'
            ...
            if checkVarFile(varfile, args.getimg, args.listkernels, 'Nimbus'):
                if args.listkernels:
                    ...
                    if output != None:
                        ...
                        print 'You can get more details by querying the image repository using fg-repo -q command and the query string: "* where tag=imagename". \n' + 'NOTE: To query the repository you need to remove the OS from the image name (centos,ubuntu,debian,rhel...). ' + 'The real name starts with the username and ends before .img'
            elif args.justregister:
            ...
            if checkVarFile(varfile, args.getimg, args.listkernels, 'OpenStack'):
                if args.listkernels:
