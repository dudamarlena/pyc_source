...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/futuregrid/shell/fgShellImage.py
# Compiled at: 2012-09-11 13:01:08
"""
...
class fgShellImage(Cmd):
    ...
    def extra_help(self):
        msg = 'Useful information about the software. Currently, we do not parse the packages names provided within the -s/--software option' + "Therefore, if some package name is wrong, it won't be installed. Here we provide a list of useful packages names from the official repositories: \n" + 'CentOS: mpich2, python26, java-1.6.0-openjdk. More packages names can be found in http://mirror.centos.org/\n' + 'Ubuntu: mpich2, openjdk-6-jre, openjdk-6-jdk. More packages names can be found in http://packages.ubuntu.com/ \n\n' + 'FutureGrid Performance packages (currently only for CentOS 5): fg-intel-compilers, intel-compilerpro-common, ' + 'intel-compilerpro-devel, intel-compilerproc, intel-compilerproc-common, intel-compilerproc-devel ' + 'intel-compilerprof, intel-compilerprof-common, intel-compilerprof-devel, intel-openmp, intel-openmp-devel, openmpi-intel'
        return msg
    ...
    def do_imagegenerate(self, args):
        ...
        parser = argparse.ArgumentParser(prog='imagegenerate', formatter_class=argparse.RawDescriptionHelpFormatter, description='FutureGrid Image Generation Help', epilog=self.extra_help())
        ...
        if len(used_args) == 0:
            ...
            if status != None:
                ...
                if not args.baseimage:
                    print '\n The image and the manifest generated are packaged in a tgz file.' + '\n Please be aware that this FutureGrid image does not have kernel and fstab. Thus, ' + 'it is not built for any infrastructure. To register the new image, use the Image Management register command.'
                else:
    ...
    def do_imageregister(self, args):
        ...
        parser = argparse.ArgumentParser(prog='imageregister', formatter_class=argparse.RawDescriptionHelpFormatter, description='FutureGrid Image Registration Help ')
        parser.add_argument('-d', '--debug', dest='debug', action='store_true', help='Print logs in the screen for debug')
        ...
        parser.add_argument('-k', '--kernel', dest='kernel', metavar='Kernel version', help='Specify the desired kernel. Case a) if the image has to be adapted (any image generated with the generate command) this option can be used to select one of the available kernels. Both kernelId and ramdiskId will be selected according to the selected kernel. This case is for any infrastructure. Case b) if the image is ready to be registered, you may need to specify the id of the kernel in the infrastructure. This case is when -j/--justregister is used and only for cloud infrastructures.')
    ...
    def do_imagecloudlist(self, args):
        ...
        parser = argparse.ArgumentParser(prog='imagecloudlist', formatter_class=argparse.RawDescriptionHelpFormatter, description='FutureGrid Image Cloud List Help ')
        group1 = parser.add_mutually_exclusive_group()
    ...
    def do_imagecloudlistkernels(self, args):
        ...
        parser = argparse.ArgumentParser(prog='imagecloudlistkernels', formatter_class=argparse.RawDescriptionHelpFormatter, description='FutureGrid Image Cloud List Kernels Help ')
    ...
    def do_imagederegister(self, args):
        ...
        parser = argparse.ArgumentParser(prog='imagederegister', formatter_class=argparse.RawDescriptionHelpFormatter, description='FutureGrid Image Deregister Help ')
