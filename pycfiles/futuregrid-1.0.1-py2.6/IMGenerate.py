...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/futuregrid/image/management/IMGenerate.py
...
class IMGenerate(object):
    ...
    def generate(self):
        ...
        output = None
        checkauthstat = []
        options = str(self.user) + '|' + str(self.OS) + '|' + str(self.version) + '|' + str(self.arch) + '|' + str(self.software) + '|' + str(self.givenname) + '|' + str(self.desc) + '|' + str(self.getimg) + '|' + str(self.passwd) + '|ldappassmd5|' + str(self.baseimage) + '|' + str(self.scratch) + '|' + str(self.size)
...
def main():
    ...
    default_centos = '5.6'
    default_fedora = '13'
    parser = argparse.ArgumentParser(prog='fg-generate', formatter_class=argparse.RawDescriptionHelpFormatter, description='FutureGrid Image Generation Help', epilog=textwrap.dedent(extra_help()))
    ...
    if status != None:
        ...
        if not args.baseimage:
            print '\n The image and the manifest generated are packaged in a tgz file.' + '\n Please be aware that this FutureGrid image does not have kernel and fstab. Thus, ' + 'it is not built for any infrastructure. To register the new image, use the IMRegister command.'
        else:
