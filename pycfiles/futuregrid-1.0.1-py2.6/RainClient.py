...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/futuregrid/rain/RainClient.py
# Compiled at: 2012-09-11 12:21:20
"""
...
import argparse
from types import *
import re, logging, logging.handlers, glob, random
...
class RainClient(object):
        ...
        try:
            ...
            if not failed and allrunning:
                ...
                if allaccessible:
                    ...
                    f.write('su - ' + self.user + ' -c "cd /tmp; sshfs ' + self.user + '@' + loginnode + ':/N/u/' + self.user + ' /tmp/N/u/' + self.user + ' -o nonempty -o ssh_command=\'ssh -oStrictHostKeyChecking=no\'" \n')
                    if volume > 0:
                        f.write('\n sleep 5 \n')
                        f.write('\n mke2fs -F -j ' + device + '\n')
                        f.write('\n mount ' + device + ' /mnt \n')
                    ...
                    if jobscript != 'interactive' and jobscript != 'background':
                        ...
                        if self.verbose:
                            ...
                            for instan in reservation.instances:
                                print ' The VM id is ' + str(instan.id) + ' . The public ip of the ' + 'VM is ' + str(instan.public_dns_name) + ' and the keypair to use is ' + sshkeypair_path

                            print 'Thus, the command to ssh into a VM should be: ssh -i ' + sshkeypair_path + ' root@' + str(instan.public_dns_name) + ' .Remember to put ! before the command if you execute it from the FG shell.'
                            if hadoop:
                                print 'Hadoop is in the home directory of your user.'
    ...
    def install_sshfs_home(self, sshkeypair_path, sshkeypair_name, sshkey_name, sshkeytemp, reservation, connection, i):
        ...
        if self.verbose:
            print msg
        cmd = 'scp -i ' + sshkeypair_path + ' -q -oBatchMode=yes -oStrictHostKeyChecking=no ' + sshkeytemp + ' ' + sshkeytemp + '.pub ' + sshkeytemp + '.sh /N/u/' + self.user + '/.ssh/authorized_keys ' + sshkeytemp + '.machines root@' + str(i.public_dns_name) + ':/tmp/'
        self._log.debug(cmd)
        p = Popen(cmd.split(), stdout=PIPE, stderr=PIPE)
        std = p.communicate()
        if p.returncode != 0:
    ...
    def HadoopSetup(self, hadoop, master, jobscript):
        ...
        randhadooptempdir = '/tmp/hadoop-' + randomnum
        randhadoophdfsdir = randomnum + '-fg-hadoop/'
        ...
        if jobscript != 'interactive':
            ...
            if not hadoop.getHpc():
                ...
                f.write(msg)
                f.close()
            os.system('chmod +x  ' + start_script_name + ' ' + run_script_name + ' ' + shutdown_script_name + ' ' + randfile + 'setup.sh' + ' ' + genConf_script_name)
            cmd = hadoop.getHpc() or 'ssh -q -oStrictHostKeyChecking=no ' + str(master) + ' mkdir -p ' + randir
            self._log.debug(cmd)
            p = Popen(cmd.split(), stdout=PIPE, stderr=PIPE)
            std = p.communicate()
            if p.returncode != 0:
            ...
            if p.returncode != 0:
                ...
                if self.verbose:
                    print msg
            cmd = 'scp -q -oBatchMode=yes ' + start_script_name + ' ' + run_script_name + ' ' + shutdown_script_name + ' ' + genConf_script_name + ' ' + randfile + 'setup.sh' + ' ' + str(master) + ':' + randir + '/'
            self._log.debug(cmd)
            p = Popen(cmd.split())
            std = p.communicate()
            if p.returncode != 0:
                ...
                if self.verbose:
                    print msg
            cmd = 'rm -f ' + start_script_name + ' ' + shutdown_script_name + ' ' + run_script_name + ' ' + randfile + 'setup.sh' + ' ' + genConf_script_name
            self._log.debug(cmd)
            p = Popen(cmd.split())
            std = p.communicate()
            if p.returncode != 0:
            ...
            if os.path.expandvars(os.path.expanduser(randir)).rstrip('/') != os.getenv('HOME'):
                ...
                f.write(cmd)
                f.close()
            cmd = 'mv ' + start_script_name + ' ' + run_script_name + ' ' + shutdown_script_name + ' ' + genConf_script_name + ' ' + randfile + 'setup.sh' + ' ' + all_script_name + ' ' + randir
            self._log.debug(cmd)
            p = Popen(cmd.split())
            std = p.communicate()
            if p.returncode != 0:
...
def main():
    ...
    parser = argparse.ArgumentParser(prog='fg-rain', formatter_class=argparse.RawDescriptionHelpFormatter, description='FutureGrid Rain Help ')
    ...
    parser.add_argument('-k', '--kernel', dest='kernel', metavar='Kernel version', help='Specify the desired kernel(fg-register can list the available kernels for each infrastructure). Needed only when your image is in the Image Repository instead of in the infrastructure.')
    ...
    if args.instancetype not in instancetypelist:
