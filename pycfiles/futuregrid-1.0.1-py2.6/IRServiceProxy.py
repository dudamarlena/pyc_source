...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/futuregrid/image/repository/client/IRServiceProxy.py
...
class IRServiceProxy(object):
    ...
    def connection(self):
        ...
        try:
            self._connIrServer = ssl.wrap_socket(s, ca_certs=self._ca_certs, certfile=self._certfile, keyfile=self._keyfile, cert_reqs=ssl.CERT_REQUIRED, ssl_version=ssl.PROTOCOL_TLSv1)
    ...
    def getUserStatus(self, userId, passwd, userIdB):
        """
    ...
    def put(self, userId, passwd, userIdB, imgFile, attributeString):
        ...
        if self.checkMeta(attributeString):
            if os.path.isfile(imgFile):
                ...
                msg = userId + '|' + str(passwd) + '|' + self.passwdtype + '|put|' + userIdB + '|' + str(size) + '|' + extension + '|' + attributeString
                self._connIrServer.write(msg)
                authstatus = self.check_auth(userId, checkauthstat)
                ...
                if authstatus:
                    ...
                    output = re.search('^ERROR', output) or output.split(',')
                    imgStore = output[0]
                    imgId = output[1]
                    ...
                    if self.verbose:
                        ...
                        cmd = 'scp ' + imgFile + ' ' + self._serveraddr + ':' + fileLocation
                    else:
                        cmd = 'scp -q -oBatchMode=yes ' + imgFile + ' ' + self._serveraddr + ':' + fileLocation
                    stat = os.system(cmd)
                    if str(stat) != '0':
