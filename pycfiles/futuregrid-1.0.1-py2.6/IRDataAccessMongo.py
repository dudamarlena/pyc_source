...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/futuregrid/image/repository/server/IRDataAccessMongo.py
...
class ImgStoreMongo(AbstractImgStore):
                    ...
                    for item in items:
                        if requestInstance == None:
                            with open(item._imgURI) as (image):
                                imgId = gridfsLink.put(image, chunksize=4194304)
                        else:
                            requestInstance.file.seek(0)
                            imgId = gridfsLink.put(requestInstance.file, chunksize=4194304)
                        item._imgId = imgId.__str__().decode('utf-8')
    ...
    def mongoConnection(self):
        ...
        try:
            ...
            connected = True
        except pymongo.errors.ConnectionFailure, detail:
...
class ImgMetaStoreMongo(AbstractImgMetaStore):
            ...
            if self.existAndOwner(imgId, userId):
                try:
                    try:
                        ...
                        self._log.debug(dic)
                        if len(dic) > 0:
                            collectionMeta.update({'_id': imgId}, {'$set': dic}, safe=True)
                            imgUpdated = True
                    except pymongo.errors.AutoReconnect:
                        self._log.warning('Autoreconnected.')
                    except pymongo.errors.ConnectionFailure:
            ...
            try:
                try:
                    ...
                    if re.search('where', criteria):
                        (beforewhere, afterwhere) = criteria.split('where')
    ...
    def convertDicToObject(self, dic, fullMode):
        ...
        if fullMode:
            ...
            tmpMeta = ImgMeta(dic['_id'], dic['os'], dic['arch'], dic['owner'], dic['description'], tags, dic['vmType'], dic['imgType'], dic['permission'], dic['imgStatus'])
        else:
            tmpMeta = ImgMeta('', '', '', '', '', '', '', 0, '', '')
        for i in dic.keys():
            if i == '_id':
                ...
                try:
                    ...
                    if self.isAdmin(userId):
                        if userIdtoSearch == None:
                            ...
                            for dic in dicList:
                                tmpUser[dic['userId']] = IRUser(dic['userId'], dic['cred'], dic['fsCap'], dic['fsUsed'], dic['lastLogin'], dic['status'], dic['role'], dic['ownedImgs'])
                                found = True
        ...
        if self.mongoConnection():
            try:
                try:
                    ...
                    if total < 0:
                        total = 0
                    collection.update({'userId': userId}, {'$set': {'fsUsed': totalSize, 'ownedImgs': total}}, safe=True)
                    success = True
                except pymongo.errors.AutoReconnect:
        ...
        if self.mongoConnection():
            try:
                try:
                    dbLink = self._dbConnection[self._dbName]
                    collection = dbLink[self._usercollection]
                    collection.update({'userId': userIdtoModify}, {'$set': {'lastLogin': datetime.utcnow()}}, safe=True)
                    success = True
                except pymongo.errors.AutoReconnect:
            ...
            try:
                try:
                    if self.isAdmin(userId) and self._getUser(userIdtoModify) != None:
                        dbLink = self._dbConnection[self._dbName]
                        collection = dbLink[self._usercollection]
                        collection.update({'userId': userIdtoModify}, {'$set': {'role': role}}, safe=True)
                        success = True
                except pymongo.errors.AutoReconnect:
            ...
            try:
                try:
                    if self.isAdmin(userId) and self._getUser(userIdtoModify) != None:
                        dbLink = self._dbConnection[self._dbName]
                        collection = dbLink[self._usercollection]
                        collection.update({'userId': userIdtoModify}, {'$set': {'fsCap': quota}}, safe=True)
                        success = True
                except pymongo.errors.AutoReconnect:
                    self._log.warning('Autoreconnected in IRUserStoreMongo - setRole')
                except pymongo.errors.ConnectionFailure:
            ...
            try:
                try:
                    if self.isAdmin(userId) and self._getUser(userIdtoModify) != None:
                        dbLink = self._dbConnection[self._dbName]
                        collection = dbLink[self._usercollection]
                        collection.update({'userId': userIdtoModify}, {'$set': {'status': status}}, safe=True)
                        success = True
                except pymongo.errors.AutoReconnect:
        ...
        if self.mongoConnection():
            try:
                try:
                    ...
                    if authorized:
                        for user in users:
                            user._lastLogin = datetime.fromordinal(1)
                            meta = {'userId': user._userId, 'cred': user._cred, 
                               'fsUsed': user._fsUsed, 
                               'fsCap': user._fsCap, 
