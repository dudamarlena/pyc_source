...
# Embedded file name: /home/travis/build/quantmind/pulsar-odm/tests/base.py
# Compiled at: 2017-11-24 06:00:10
# Size of source mod 2**32: 7386 bytes
import unittest, string, inspect
...
class Task(Model):
    ...
    type = Column(ChoiceType(TaskType, impl=Integer), default=(TaskType.work))
...
class TestCase(unittest.TestCase):
    ...
    async def setUpClass(cls):
        ...
        cls.mapper = await cls.green_pool.submit(cls.init_mapper.database_create, cls.dbname)
...
class MapperMixin:
    ...
    def test_database_drop_fail(self):
        self.assertRaises(AssertionError, self.mapper.database_drop, lambda e: None)

    def test_create_task(self):
        with self.mapper.begin() as (session):
            task = self.mapper.task(id=(uuid4()), subject='simple task',
    ...
    def test_update_task(self):
        with self.mapper.begin() as (session):
            task = self.mapper.task(id=(uuid4()), subject='simple task to update')
    ...
    def test_task_employee(self):
        ...
        with mapper.begin() as (session):
            ...
            session.add(user)
        with mapper.begin() as (session):
            task = mapper.task(id=(uuid4()), employee_id=(user.id),
    ...
    def test_view(self):
        mapper = self.mapper
        with mapper.begin() as (session):
            session.add(mapper.task(id=(uuid4()), subject='simple task 1',
              type=(TaskType.personal)))
            session.add(mapper.task(id=(uuid4()), subject='simple task 2',
              type=(TaskType.personal)))
            session.add(mapper.task(id=(uuid4()), subject='simple task 3',
              type=(TaskType.work)))
        with mapper.begin() as (session):
