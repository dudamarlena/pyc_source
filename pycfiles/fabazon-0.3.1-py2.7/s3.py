...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/fabazon/s3.py
...
class S3Bucket(object):
    ...
    def __init__(self, bucket_name):
        ...
        if bucket_name not in _buckets:
            _buckets[bucket_name] = self.cnx.get_bucket(bucket_name, validate=False)
    ...
    def sign_download_request(self, key, expiration_secs=DEFAULT_SIGN_EXPIRATION):
        """Signs an S3 request.
    ...
    def download(self, key, dest_file):
        ...
        url = 'http://s3.amazonaws.com/%(bucket_name)s/%(key)s?AWSAccessKeyId=%(aws_key_id)s&Expires=%(expires)s&Signature=%(signature_quoted)s' % download_info
    ...
    def upload_directory_index(self, path, recurse=False):
        ...
        key.set_contents_from_string(self._build_directory_index(path, entries))
        key.make_public()
        if recurse:
