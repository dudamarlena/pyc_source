...
# [GCC 8.4.0]
# Embedded file name: /Users/ekeyme/git/bio-pm/tests/test_status.py
# Compiled at: 2017-08-20 03:44:31
...
class RoutineTest(unittest.TestCase):
    __doc__ = 'Routine test.'

    def test_pm_status_gt_order(self):
        """Status should have a right order when doing gt-comparison"""
        r = Y() > Conserved(aa_pm=0) > Conserved(nt_pm=8, aa_pm=0) > PM(aa_pm=0) > PM(aa_pm=5, nt_pm=10) > NA() > NA(gaps=1)
        self.assertTrue(r)
    ...
    def test_pm_status_le_order(self):
        ...
        r = Y() <= Y() and Conserved(aa_pm=0) <= Conserved(aa_pm=0) and PM() <= PM() and NA() <= NA()
        self.assertTrue(r)
...
class ErrorTest(unittest.TestCase):
    ...
    def test_raise_TypeError1(self):
        ...
        with self.assertRaises(TypeError):
            Y(stdseq='atg') > Conserved(stdseq='tga', aa_pm=0) > PM(stdseq='aaa') > NA(stdseq='tgg')
