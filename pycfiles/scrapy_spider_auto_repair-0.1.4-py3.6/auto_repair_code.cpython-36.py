...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\spider_auto_repair\auto_repair_code.py
# Compiled at: 2018-08-13 06:01:22
# Size of source mod 2**32: 64141 bytes
    ...
    def __init__(self, path_to_data, parser):
        ...
        if parser.lower() == 'xml':
            ...
            if parser.lower() == 'html':
                ...
                raise AssertionError('Invalid parser!')
    ...
    def remove_br(self, code):
        """
            ...
            Example:
                ...
                >>> 
        """
        code = sub('<\\s*br\\s*>|<\\s*br\\s*/\\s*>|<\\s*/\\s*br\\s*>', '', code)
        return code
    ...
    def remove_tag_attributes(self, code):
        ...
        for ch in code:
            if state == State.wait_for_open_angular_bracket:
                ...
                if state == State.wait_for_whitespace_or_close_angular_bracket:
                    if ch == '>':
                        state = State.wait_for_open_angular_bracket
                        lst.append(ch)
                    else:
                    ...
                    if state == State.wait_for_close_angular_bracket:
                        if ch == '>':
                            state = State.wait_for_open_angular_bracket
                            lst.append(ch)
...
class Page(ParsingAndProcessing, HungarianHelperMethods):
    ...
    def dfs(self, root, mn, str_subtree):
        """
            ...
            Example:
                ...
                >>>
        """
        edit_dis = self.get_edit_distance(str_subtree[0], tostring(root, pretty_print=False).decode('utf-8'))
        if edit_dis < mn[0]:
            mn[0] = edit_dis
    ...
    def compress_tree(self, tree, parent, idx_of_child, orig_tree, dic):
        """
            ...
            Example:
                ...
                >>> orig_tree = fromstring('<div><div><div><div>child1</div></div></div>                                      <div>child2</div></div>')
                >>> idx_of_child = 0
                >>> parent = -1
                >>> dic = {}
    ...
    def get_new_page_compressed_subtree_path(self, compressed_subtree, compressed_old_tree, compressed_new_tree):
        """
            ...
            Example:
                >>> path = 'Examples/Hello_World.html'
                >>> obj = Page(path, 'html')
                >>> compressed_old_tree = fromstring('<body>                                                        <div>                                                            <p>Username</p>                                                            <p>Password</p>                                                            <div>Submit</div>                                                        </div>                                                        <div>                                                            <div>                                                                <div>                                                                    <div>                                                                        <p>Username</p>                                                                        <p>Captcha1</p>                                                                        <p>Captcha2</p>                                                                    </div>                                                                </div>                                                            </div>                                                        </div>                                                         <p>This should not be extracted</p>                                                    </body>')
                >>> compressed_new_tree = fromstring('<body>                                                        <div>                                                            <p>Username</p>                                                            <p>email</p>                                                        </div>                                                        <p>This should not be extracted</p>                                                        <div>                                                            <p>Hello World</p>                                                            <div>                                                                <p>Username</p>                                                                <p>Password</p>                                                            </div>                                                        </div>                                                    </body>')
        ...
        features_old = self.get_k_nearest_leaves_for_all_subtrees(lst_occurences_old, k=2)
        features_new = self.get_k_nearest_leaves_for_all_subtrees(lst_occurences_new, k=2)
    ...
    def get_path_in_compressed_tree(self, subtree, old_tree, new_tree):
        """
            ...
            Example:
                >>> path = 'Examples/Hello_World.html'
                >>> obj = Page(path, 'html')
                >>> old_page = fromstring('<html>                    <body>                        <div>                            <p>Username</p>                            <p>Password</p>                            <div>Submit</div>                        </div>                        <div>                        <div>                                <div>                                    <div>                                        <p>Username</p>                                        <p>Captcha1</p>                                        <p>Captcha2</p>                                    </div>                                </div>                            </div>                        </div>                         <p>This should not be extracted</p>                    </body>                </html>')
                >>> new_page = fromstring('<html>                    <body>                        <div>                            <p>Username</p>                            <p>email</p>                        </div>                        <p>This should not be extracted</p>                        <div>                            <p>Hello World</p>                            <div>                                <p>Username</p>                                <p>Password</p>                            </div>                        </div>                    </body>                </html>')
        ...
        path_of_compressed_new_subtree = self.get_new_page_compressed_subtree_path(compressed_old_subtree, compressed_old_tree, compressed_new_tree)
    ...
    def get_path_in_uncompressed_tree_helper(self, tree, str_old_page_subtree, path_compressed, path_in_new_tree, temp_path):
        """
            ...
            Example:
                ...
                >>> 
        """
        if self.is_subsequence(path_compressed, temp_path):
            if tostring(tree).strip() == str_old_page_subtree:
    ...
    def get_path_in_uncompressed_tree(self, subtree, old_tree, new_tree):
        """
            ...
            Example:
                ...
                [2, 1, 0]
        """
        path_of_compressed_new_subtree = self.get_path_in_compressed_tree(subtree, old_tree, new_tree)
        str_old_page_subtree = tostring(subtree).strip()
        path_in_new_tree = []
        self.get_path_in_uncompressed_tree_helper(new_tree, str_old_page_subtree, path_of_compressed_new_subtree, path_in_new_tree, [])
...
def get_subtrees_to_be_extracted(rules, extracted_old_subtree, old_page):
    ...
    for path in paths:
        subtrees_to_be_extracted.append(old_page.retrieve_subtree((old_page.tree), path,
          cpy=False))
...
def auto_repair(old_page, new_page, extracted_old_subtree, rules=None):
    ...
    if rules is not None:
        repaired_subtree = new_page.get_repaired_subtree(rules, ElementTree(extracted_old_subtree), new_page.tree)
        return (
         rules, repaired_subtree)
    else:
        rules = new_page.generate_rules(extracted_old_subtree, new_page.tree_without_attr)
        subtrees_to_be_extracted = get_subtrees_to_be_extracted(rules, extracted_old_subtree, old_page)
        ...
        for subtree, rule in zip(subtrees_to_be_extracted, rules):
            final_rules.append((rule[0],
             old_page.get_path_in_uncompressed_tree(subtree, root_old_page, root_new_page)))
...
def show_demo():
    for i in range(1, 4):
        ...
        rules = tree_old_page.generate_rules(query_tree_obj.tree_without_attr, tree_old_page.tree_without_attr)
        print('rules =', rules)
...
def show_auto_repair():
    ...
    rules = tree_obj1.generate_rules(query_tree_obj.tree_without_attr, tree_obj1.tree_without_attr)
    repaired_query_tree = tree_obj2.get_repaired_subtree(rules, query_tree_obj.tree, tree_obj2.tree)
    print('rules =', rules)
