...
# Embedded file name: /usr/local/lib/python2.7/site-packages/plists/v1parser.py
# Compiled at: 2015-03-22 02:31:13
import cStringIO
...
class Parser(object):
    ...
    def parse_value(self):
        ...
        if token.toktype in (TOKEN_NUMBER, TOKEN_STRING, TOKEN_IDENTIFIER):
            return token
        if token.toktype == TOKEN_OPEN_LIST:
            return self.parse_list()
        if token.toktype == TOKEN_OPEN_DICT:
    ...
    def __repr__(self):
        if self.value:
            if self.toktype == TOKEN_NUMBER:
                ...
                if self.toktype == TOKEN_STRING:
                    return "'%s'" % self.value
                if self.toktype == TOKEN_ERROR:
