...
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pwclip/cmdline.py
# Compiled at: 2020-03-20 08:07:41
# Size of source mod 2**32: 20039 bytes
__doc__ = 'pwclip main program'
try:
    from os import fork
except ImportError:
...
from system import absrelpath, copy, paste, xgetpass, xmsgok, xyesno, xnotify, xinput, xkbid, which, whoami, dictreplace, adbout
...
def forkwaitclip(text, poclp, boclp, wait=3, out=None, enter=None):
    ...
    if out:
        ...
        if out == 'gui':
            ...
            if out == 'cli':
                stdout.write('%s' % text if not enter else '%s\\n' % text)
                stdout.flush()
            elif out == 'ano':
                adbout(text, enter)
                enter = False
...
def optpars(cfgs, mode, name):
    desc = 'pwclip - Multi functional password manager to temporarily save passphrases to your copy/paste buffers for easy and secure accessing your passwords. The following arguments might also be set by the config ~/.config/%s.yaml file.' % name
    epic = 'the yubikey mode is compatible with the challenge-response feature of yubikeys only for now.'
    ...
    pars.add_argument('-E',
      '--enter', dest='ent',
      action='store_true',
      help='also enter newline when printing password (only useful with -o/-O)')
    pars.add_argument('-O',
      '--android', dest='out',
      action='store_const',
      const='ano',
    ...
    pars.add_argument('-e',
      '--expression', dest='rex',
      default=False,
      metavar='EXPRESSION:[LEN]',
      nargs=('?' if mode == 'gui' else None),
      help='generate password using EXPRESSION to generate password of lenght LEN, either LEN is set or 24 is used - use with -g(default is "[a-zA-Z0-9\\!$%%&/\\(\\)=\\?\\+#,\\.-:]*:24")')
    pars.add_argument('-g',
      '--genpw', dest='gpw',
      action='store_true',
      help='randomly generate password, only useful with -a (uses EXPRESSION)')
    pars.add_argument('--show-clip',
      dest='scl',
      action='store_true',
      help='show current clipboard content (WARNING: insecure - if cliptime set to 0 cli mode does not clear the sreen)')
    pars.add_argument('-s',
      '--show-passwords', dest='sho',
      action='store_true',
    ...
    gpars.add_argument('-k',
      '--key', dest='gpgkey',
      metavar='ID',
      type=str,
      help='gpg-key ID(s) to use for decryption/signing')
    gpars.add_argument('-r',
      '--recipients', dest='recvs',
      metavar='"ID [ID ...]"',
      help='one ore more gpg-key ID(s) to use for encryption (strings seperated by spaces within "")')
    gpars.add_argument('-u',
      '--user', dest='usr',
      metavar='USER',
      nargs=('?' if mode == 'gui' else None),
      default=(False if mode == 'gui' else cfgs['user']),
      help=('query entrys only for USER (-A overrides, "%s" is default)' % cfgs['user']))
    gpars.add_argument('-p',
      '--password', dest='pwd',
      default=None,
      help='enter password for add/change actions(insecure & not recommended)')
    gpars.add_argument('--comment',
      dest='com',
      default=None,
      help='enter comment for add/change actions')
    gpars.add_argument('-x',
      '--x509', dest='gpv',
      action='store_const',
      const='gpgsm',
      help='force ssl compatible gpgsm mode - usually is autodetected (use --cert & --key for imports)')
    gpars.add_argument('-C',
      '--cert', dest='sslcrt',
      metavar='SSL-Certificate',
      help='one-shot setting of SSL-Certificate')
    gpars.add_argument('-K',
      '--ssl-key', dest='sslkey',
      metavar='SSL-Private-Key',
      help='one-shot setting of SSL-Private-Key')
    gpars.add_argument('--ca',
      '--ca-cert', dest='sslca',
      metavar='SSL-CA-Certificate',
      help='one-shot setting of SSL-CA-Certificate')
    gpars.add_argument('-P',
      '--passcrypt', dest='pcr',
      metavar='CRYPTFILE',
      default=(path.expanduser('~/.passcrypt')),
      help='set location of CRYPTFILE to use as password store (~/.passcrypt is default)')
    gpars.add_argument('-Y',
      '--yaml', dest='yml',
      metavar='YAMLFILE',
      default=(path.expanduser('~/.pwd.yaml')),
      help='set location of YAMLFILE to read whole sets of passwords from a yaml file (~/.pwd.yaml is default)')
    ypars = pars.add_argument_group('yubikey arguments')
    (ypars.set_defaults)(**cfgs)
    ypars.add_argument('-S', '--slot', dest='ysl', default=None, type=int, choices=(1,
                                                                                    2), help='set one of the two yubikey slots (only useful with -y)').completer = ChoicesCompleter((1,
                                                                                                                                                                                     2))
    ypars.add_argument('-y',
      '--ykserial', nargs='?',
      dest='yks',
      metavar='SERIAL',
      default=False,
      help='switch to yubikey mode and optionally set SERIAL of yubikey (autoselect serial and slot is default)')
    apars = pars.add_argument_group('action arguments')
    (apars.set_defaults)(**cfgs)
    apars.add_argument('-a',
      '--add', dest='add',
      metavar='ENTRY',
      nargs=('?' if mode == 'gui' else None),
      default=False,
      help='add ENTRY (password will be asked interactivly)')
    apars.add_argument('-c',
      '--change', dest='chg',
      metavar='ENTRY',
      nargs=('?' if mode == 'gui' else None),
      default=False,
      help='change ENTRY (password will be asked interactivly)')
    apars.add_argument('-d',
      '--delete', dest='rms',
      metavar='ENTRY',
      nargs=('?' if mode == 'gui' else '+'),
      default=False,
      help='delete ENTRY(s) from the passcrypt list')
    apars.add_argument('-l',
      '--list', dest='lst',
      metavar='PATTERN',
      nargs='?',
      default=False,
      help='pwclip an entry matching PATTERN if given - otherwise list all entrys')
    autocomplete(pars)
...
def confpars(mode):
    ...
    cfgmap = {'gpg':{'recipients':'recvs', 
      'key':'gpgkey',  'delkey':True}, 
     'yubikey':{'slot':'ykslot', 
      'seerial':'ykser',  'delkey':True}}
    envmap = {'GPGKEY':'gpgkey', 
     'RECIPIENTS':'recvs', 
     'PWCLIPTIME':'time', 
     'YKSERIAL':'ykser', 
     'USER':'usr', 
    ...
    if args.gpw:
        ...
        genpwlen = 24
        if ':' in genpwrex:
            genpwrex, genpwlen = ''.join(str(genpwrex).split(':')[:-1]), str(genpwrex).split(':')[(-1)]
    ...
    if mode != 'gui':
        if args.yks is False:
            if args.lst is False:
                if args.add is False:
                    if args.chg is False:
                        if args.rms is False:
                            if args.sslcrt is None:
...
def cli():
    ...
    if not path.isfile(args.yml):
        if not path.isfile(args.pcr):
            if args.yks is False:
    ...
    err = None
    if args.add:
        __ents = PassCrypt(*pargs, **pkwargs).adpw(args.add, args.pwd, args.com)
        if not __ents:
        ...
        if args.chg:
            ...
            __ents = PassCrypt(*pargs, **pkwargs).chpw(args.chg, args.pwd, args.com)
            if not args.aal:
                __ents[args.usr]
            if not __ents:
                if [u for u, es in __ents.items() if args.chg in es.keys()]:
                    exit(0)
                err = (
                 'could not change entry', args.chg)
        else:
            if args.rms:
                ers = []
                for r in args.rms:
                ...
                if len(ers) >= 1:
                    ewrd = 'entrys'
                err = ('deleting the following %s failed:', bred(', ').join(ers)) if ers else None
                ...
                if __ents and args.lst not in __ents.keys():
                    err = ('could not find entry', args.lst,
...
def gui(typ='pw'):
    ...
    if args.yks or args.yks is None or typ == 'yk':
        ...
        if args.add is not False:
            ...
            if args.chg is not False:
                _chg = __xdialog('as %s: enter name of the entry to change' % usr)
                if not _chg:
                    xnotify('changing entry aborted')
                    exit(1)
                if _chg:
                    __ents = PassCrypt(*pargs, **pkwargs).chpw(_chg, None, None)
                    if not __ents or usr not in __ents or _chg not in __ents[usr].keys():
                ...
                if args.rms is not False:
                    _rms = __xdialog('as %s: enter name of the entry(s) to delete' % usr)
                    if not _rms:
                        xnotify('deleting entry aborted')
                        exit(1)
                    elif _rms and ' ' in _rms:
