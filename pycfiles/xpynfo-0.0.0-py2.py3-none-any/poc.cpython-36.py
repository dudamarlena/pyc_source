...
# Compiled at: 2018-05-28 20:46:34
# Size of source mod 2**32: 9740 bytes
...
from anytree.render import ContStyle, RenderTree
CONNECTION = connect()
ATOM_CACHE = {}
...
def get_property(window_id, atom_id):
    ...
    prop_reply = CONNECTION.core.GetProperty(delete=False,
      window=window_id,
      property=atom_id,
      type=(GetPropertyType.Any),
      long_offset=LONG_OFFSET,
      long_length=LONG_LENGTH).reply()
    if Atom.ATOM == prop_reply.type:
        ...
        if prop_reply.type in STRING_ATOMS:
...
def define_attributes(window_id, discover=True):
    return define_generic_member_dict(window_id=window_id,
      discover=discover,
...
def create_node_from_window(window_id, parent=None, recurse=True, max_depth=None, use_names=True, **kwargs):
    ...
    if can_recurse:
        ...
        for child in child_windows:
            create_node_from_window(window_id=child, 
             parent=WINDOW_NODES[window_id], 
             recurse=can_recurse, 
             max_depth=max_depth, 
             use_names=use_names, **kwargs)
    ...
    for pre, fill, node in render:
        ...
        for key, value in list(PARAMETER_META_MAP.items()):
            if key in kwargs and kwargs[key] or key not in kwargs:
                lines += build_tree_member_list(fill=fill, 
                 node=node, 
...
def parse_args(args=None):
    if args is None:
        args = argv[1:]
    parser = ArgumentParser(description='A tool to examine various components of X info',
      add_help=True)
    parser.add_argument('-r',
      '--recurse', dest='recurse',
      action='store_true',
      help='Queries the given ID and its children recursively')
    parser.add_argument('-d',
      '--max-depth', dest='max_depth',
      action='store',
      type=int,
      default=None,
      help='Limit the depth of recursion')
    parser.add_argument('-n',
      '--use-names', dest='use_names',
      action='store_true',
      help='Add _NET_WM_NAME or WM_NAME to tree output')
...
def cli():
    ...
    create_args = {'window_id':config.window_id, 
     'parent':None, 
