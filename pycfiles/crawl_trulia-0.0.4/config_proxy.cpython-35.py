...
# Embedded file name: build\bdist.win-amd64\egg\crawl_requests\config_proxy.py
# Compiled at: 2018-01-31 06:58:52
...
class Gen_proxy:
    ...
    def __init__(self):
        ...
        self.headers = {'Host': 'www.xicidaili.com', 
         ...
         'Cookie': '_free_proxy_session=BAh7B0kiD3Nlc3Npb25faWQGOgZFVEkiJTRiZTUyMzBmNTg5OTM2YmJkMjUwNzI0YzA3MmZmYzhhBjsAVEkiEF9jc3JmX3Rva2VuBjsARkkiMW41d1hVT2lRbmx6SG05QWZzTGhsQnJMKzY0S0swNE5zcURzWERORi9QSG89BjsARg%3D%3D--75202b4ab0931f7023c611e51d6b3726333b1c83; Hm_lvt_0cf76c77469e965d2957f0553e6ecf59={0}; Hm_lpvt_0cf76c77469e965d2957f0553e6ecf59={1}'.format(int(time.time()), int(time.time())), 
        ...
        for item in items:
            ...
            if '天' in it[8].get_text():
                ...
                if speed_seconds < 1.0 and conn_seconds < 1.0:
                    data = {'complete_ip': it[1].get_text() + ':' + it[2].get_text(), 
    ...
    def test_pool(self, pool):
        ...
        for pxy in pool:
            ...
            N += 1
            print('Tested [{0}%] -------- Wait [{1} minutes]'.format(math.trunc(N / len(pool) * 100), math.trunc(-1 + (len(pool) * 5 - (time.time() - start)) / 60)))
