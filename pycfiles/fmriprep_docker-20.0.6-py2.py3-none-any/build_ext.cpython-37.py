...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-hbx1i2h0/setuptools/setuptools/command/build_ext.py
# Compiled at: 2020-04-16 14:32:33
# Size of source mod 2**32: 13048 bytes
import os, sys, itertools
import distutils.command.build_ext as _du_build_ext
...
import distutils.sysconfig as _CONFIG_VARS
    ...
    class build_ext(_build_ext):
        ...
        def get_ext_filename(self, fullname):
            ...
            if fullname in self.ext_map:
                ...
                use_abi3 = not six.PY2 and getattr(ext, 'py_limited_api') and get_abi3_suffix()
                if use_abi3:
        ...
        def get_outputs(self):
            return _build_ext.get_outputs(self) + self._build_ext__get_stubs_outputs()

        def __get_stubs_outputs(self):
            ns_ext_bases = ((os.path.join)(self.build_lib, *ext._full_name.split('.')) for ext in self.extensions if ext._needs_stub)
            pairs = itertools.product(ns_ext_bases, self._build_ext__get_output_extensions())
        ...
        def write_stub(self, output_dir, ext, compile=False):
            ...
            if compile:
                ...
                if optimize > 0:
                    byte_compile([stub_file], optimize=optimize, force=True,
                      dry_run=(self.dry_run))
                if os.path.exists(stub_file):
                    if not self.dry_run:
                        os.unlink(stub_file)
