...
# [GCC 8.4.0]
# Embedded file name: /Users/sogubaby/.virtualenvs/scrapy_middlewares/lib/python3.7/site-packages/scrapy_cabinet/libs/sle/extractor/URLExtractor.py
...
class URLExtractor(object):
    ...
    def _json_extractor(self, json_dict: Union[(Dict, List)]):
        ...
        if self.url_key and '$' in self.url_key:
            ...
            LOGGER.info('PATTERN_TYPE: {} || PATTERN: {} || RESULT: {}'.format('time_args', self.url_key, text))
        else:
            if self.url_key:
                r = reduce(lambda x, y: x + y, re.findall(self.url_key, json.dumps(json_dict), re.DOTALL))
                r = max(r, key=(lambda x: len(x)))
                if r:
                    LOGGER.info('PATTERN_TYPE: {} || PATTERN: {} || RESULT: {}'.format('url_key_regex', self.url_key, r))
                    return r
            else:
                ...
                if text:
                    LOGGER.info('PATTERN_TYPE: {} || PATTERN: {} || RESULT: {}'.format('url_default', 'url in dict.keys', text))
                return text
