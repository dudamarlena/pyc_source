...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/harambe/harambe/cli.py
# Compiled at: 2017-05-03 01:13:11
r"""
...
________________________________________________________________________________
"""
import os, re, sys, traceback, logging, importlib, pkg_resources, click, yaml, functools, json
from werkzeug import import_string
from .__about__ import *
from harambe import utils
import flask, sh, subprocess
from .core import db
CWD = os.getcwd()
SKELETON_DIR = 'skel'
...
def init():
    ...
    if os.path.isfile(harambepyfile):
        ...
        print ''
        print '- Harambe is still alive in our code!'
        print ''
        print "> What's next?"
        print '- Edit the config [ app/config.py ] '
        print '- If necessary edit and run the command [ happy :dbsync ]'
        print '- Launch app on development mode, run [ happy :serve ]'
        print ''
        print '*' * 80
...
def server(port):
    ...
    print ''
    cwd_to_sys_path()
    application.happy.run(debug=True, host='0.0.0.0', port=port)
...
def db_sync():
    ...
    cwd_to_sys_path()
    if application.happy.db and application.happy.db.Model:
        application.happy.db.create_all()
        for m in application.happy.db.Model.__subclasses__():
...
def _set_flask_alembic():
    ...
    application.happy.extensions['sqlalchemy'] = type('', (), {'db': db})
    alembic = Alembic()
    alembic.init_app(application.happy)
    return alembic
...
def db_migrate():
    ...
    with application.happy.app_context():
        p = application.happy.db.Model.__subclasses__()
        print p
...
def assets2s3():
    ...
    print ''
    build_assets(application.happy)
    print ''
    print 'Uploading assets files to S3 ...'
    flask_s3.create_all(application.happy)
    print ''
