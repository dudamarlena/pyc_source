...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/harambe/harambe/ext.py
# Compiled at: 2017-05-03 01:40:33
"""
Extensions
"""
import re, logging
from flask import request, current_app
import flask_cloudy, flask_recaptcha, flask_seasurf, flask_kvsession, flask_caching, ses_mailer, flask_mail, flask_s3, flask_babel
from flask_babel import lazy_gettext as _
from . import Harambe, init_app, utils, exceptions, g, get_config
...
def _session(app):
    ...
    if uri:
        ...
        if 'redis' in scheme:
            ...
            raise exceptions.HarambeError("Invalid Session Store. '%s' provided" % scheme)
    if store:
...
class _Mailer(object):
    ...
    def init_app(self, app):
        ...
        if mailer_uri:
            ...
            if 'ses' in scheme.lower():
                ...
                self.mail = ses_mailer.Mail(aws_access_key_id=access_key, aws_secret_access_key=secret_key, region=region, sender=self.config.get('MAIL_SENDER'), reply_to=self.config.get('MAIL_REPLY_TO'), template=self.config.get('MAIL_TEMPLATE'), template_context=self.config.get('MAIL_TEMPLATE_CONTEXT'))
                ...
                _ses_mailer = ses_mailer.Mail(template=self.config.get('MAIL_TEMPLATE'), template_context=self.config.get('MAIL_TEMPLATE_CONTEXT'))
    ...
    def send(self, to, subject=None, body=None, reply_to=None, template=None, **kwargs):
        ...
        if not isinstance(to, list):
            recipients = [to] if 1 else to
            kwargs.update({'subject': subject, 
               'body': body, 
               'reply_to': reply_to})
            raise (self.validated or exceptions.HarambeError)('Mail configuration error')
        if self.provider == 'SES':
            ...
            _safe_keys = [
             'recipients', 'subject', 'body', 'html', 'alts',
            ...
            raise exceptions.HarambeError("Invalid mail provider. Must be 'SES' or 'SMTP'")
...
def upload(_props_key, file, **kw):
    ...
    config file must have STORAGE_UPLOAD_PROPS where it contains k/v, ie:

    STORAGE_UPLOAD_PROPS = {
        "profile-image": {
    ...
    upload("profile-image", my_file)
    :param _props_key: (str) a key available in config.STORAGE_UPLOAD_PROPS
    :param file: FileStorage object or string location
    :param kw: extra **kw for 
    :return: Storage object
    """
    kwargs = {}
    conf = get_config('STORAGE_UPLOAD_PROPS')
    if not conf:
        raise ValueError('Missing STORAGE_UPLOAD_PROPS in config')
    if _props_key not in conf:
        raise ValueError("Missing '%s' in config STORAGE_UPLOAD_PROPS" % _props_key)
