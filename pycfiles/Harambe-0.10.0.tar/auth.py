...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/harambe/harambe/contrib/views/auth.py
# Compiled at: 2017-04-06 00:21:51
import datetime, uuid, arrow, time, exceptions
from flask_login import fresh_login_required
import logging, exceptions
from flask_login import LoginManager, login_user, current_user, fresh_login_required
from harambe import Harambe, _, g, models, page_meta, request, redirect, flash_success, flash_error, flash_info, abort, recaptcha, storage, decorators as harambe_deco, exceptions as harambe_exc, exceptions, utils, paginate, views, url_for, session
from harambe.contrib.auth import oauth, is_authenticated, not_authenticated, get_user_by_action_token, session_set_require_password_change, visible_to_managers, authenticate, authenticate_social_login, login_user, UserModel, create_user, exceptions, __options__ as auth_options, login_manager, decorators as deco
...
class Login(Harambe):
    ...
    def _register(cls, app, **kwargs):
        ...
        harambe_deco.nav_title.add(nav.pop('title'), cls, **nav)
    ...
    @harambe_deco.nav_title('Login', visible=not_authenticated)
    @harambe_deco.accept_post_get
    @deco.logout_user
    def login(self):
        ...
        if request.method == 'POST':
            ...
            try:
                if not username or not password:
                    raise harambe_exc.AppError('Email/Username or Password is empty')
                user = authenticate(username=username, password=password)
                if not user:
                    raise harambe_exc.AppError('Email or Password is invalid')
                login_user(user)
                if user.require_password_change is True:
                    flash_info('Password change is required')
                    session_set_require_password_change(True)
                    return redirect(views.auth.Account.change_password)
            ...
            except (harambe_exc.AppError, exceptions.AuthError) as ae:
                flash_error(str(ae))
        ...
        page_meta('Login')
        return {'username': request.args.get('username'), 
    ...
    @harambe_deco.nav_title('Register', visible=not_authenticated())
    @harambe_deco.accept_post_get
    @deco.logout_user
    def register(self):
        """ Registration """
        ...
        if request.method == 'POST':
            try:
                if not recaptcha.verify():
                    raise harambe_exc.AppError('Invalid Security code')
                ...
                if __options__.get('registration_username'):
                    ...
                    login_method = 'email'
                if not first_name:
                    raise harambe_exc.AppError(_('First Name or Name is required'))
                else:
                    if not password or password != password_confirm:
                        raise harambe_exc.AppError(_("Passwords don't match"))
                    if not login_method:
                        raise exceptions.AuthError(_('Registration is disabled'))
                    user = create_user(username=username, password=password, email=email, first_name=first_name, last_name=last_name, login_method=login_method)
                    if with_oauth and oauth_provider and oauth_user_id:
                        user.add_social_login(oauth_provider, oauth_user_id)
                        login_user(user)
                        return redirect(request.form.get('next') or views.auth.Account.account_info)
                ...
                return redirect(request.form.get('next') or __options__.get('login_view'))
            except (harambe_exc.AppError, exceptions.AuthError) as ex:
                flash_error(str(ex))
            except Exception as e:
                logging.exception(e)
            ...
            return {'reg_email': __options__.get('registration_email'), 
    ...
    def _register_oauth_session_user(self, user):
        ...
        if oauth_session:
            if 'provider' in oauth_session and 'user_id' in oauth_session:
                user.add_social_login(provider=oauth_session.get('provider'), social_id=oauth_session.get('user_id'))
        delete_oauth_session()

    @harambe_deco.nav_title('Lost Password')
    @harambe_deco.accept_post_get
    @deco.logout_user
    ...
    @harambe_deco.nav_title('Reset Password', visible=False)
    @harambe_deco.route('/reset-password/<action_token>/<signed_data>/', methods=['POST', 'GET'])
    @deco.logout_user
    def reset_password(self, action_token, signed_data):
        ...
        try:
            ...
            if not user or not user.signed_data_match(signed_data, action):
                raise harambe_exc.AppError('Verification Invalid!')
            if request.method == 'POST':
                password = request.form.get('password', '').strip()
                password_confirm = request.form.get('password_confirm', '').strip()
                if not password or password != password_confirm:
    ...
    @harambe_deco.route('/verify-email/<action_token>/<signed_data>/')
    @deco.logout_user
    ...
    @harambe_deco.nav_title('Confirm Email', visible=False)
    @harambe_deco.accept_post_get
    @deco.logout_user
    def request_email_verification(self):
        ...
        return {'email': request.args.get('email')}

    @harambe_deco.nav_title('Logout', visible=False)
    @harambe_deco.accept_get
    @deco.logout_user
    def logout(self):
        session_set_require_password_change(False)
        return redirect(__options__.get('logout_view') or self.login)

    @harambe_deco.render_json
    @harambe_deco.route('/oauth-connect/<provider>/<action>/', methods=['GET', 'POST'])
    def oauth_connect(self, provider, action):
        ...
        **** This methods doesn't save the user token, it only retrieves the ID
        ...
        """
        valid_actions = [
         'connect', 'authorized', 'test']
        _redirect = views.auth.Account.account_info if is_authenticated() else self.login
        if action not in valid_actions or 'oauth' not in __options__.get('registration_methods') or not __options__.get('allow_registration') or not hasattr(oauth, provider):
            ...
            oauth_email = None
            if action == 'test':
                session_data = {'provider': 'ensure', 'user_id': '1234', 
                   'name': 'Mardix', 
            ...
            if action == 'connect':
                ...
                authorized_url = url_for(self, provider=provider, action='authorized', next=_next or request.referrer or None, _external=True)
            ...
            if action == 'authorized' and oauth_user_id:
                if is_authenticated():
                    try:
                        current_user.add_social_login(provider=provider, social_id=oauth_user_id)
                        flash_success('You can now login with your %s account' % provider.upper())
                    except Exception as e:
                        logging.exception(e)

                    return redirect(views.auth.Account.account_info)
                user = authenticate_social_login(provider, oauth_user_id)
                if user:
                    login_user(user)
                    return redirect(request.args.get('next') or __options__.get('login_view'))
...
class Account(Harambe):
    ...
    def _register(cls, app, **kwargs):
        ...
        if __options__.get('account.set_custom_nav_title') and custom_nav_title:
            ...
            def custom_nav():
                if is_authenticated():
                    return custom_nav_title.format(USER_NAME=current_user.name, USER_EMAIL=current_user.login_email, USER_PROFILE_IMAGE_URL=current_user.profile_image_url)

            title = custom_nav
        harambe_deco.nav_title.add(title, cls, **nav)
    ...
    @harambe_deco.nav_title('Logout', endpoint='auth.Login:logout', order=100)
    def _(self):
        pass

    @harambe_deco.nav_title('Account Info', order=1)
    def account_info(self):
        page_meta('Account Info')
        return {'__USER_ACCOUNT_FORM__': 'auth/Account/edit-forms.jade'}

    @harambe_deco.accept_post
    ...
    def update_info(self):
        ...
        try:
            ...
            kwargs = {}
            if first_name:
                kwargs['first_name'] = first_name.strip()
            if last_name:
                kwargs['last_name'] = last_name.strip()
            if file:
                prefix = __options__.get('profile_image_dir', 'profile.imgs').replace('/', '')
                prefix += '/'
                kwargs['profile_image'] = storage.upload(file, name=utils.guid(), prefix=prefix, public=True, allowed_extensions=[
                 'jpg', 'jpeg',
                 'png', 'gif'])
            if kwargs:
    ...
    @harambe_deco.nav_title('Setup Login', visible=False)
    @harambe_deco.accept_post_get
        ...
        if request.method == 'POST':
            try:
                ...
                if not password.strip() or password.strip() != password2.strip():
                    ...
                    new_login = models.AuthUserLogin.new(login_type='email', user_id=current_user.id, email=email, password=password.strip())
                    if verify_email:
...
class Admin(Harambe):
    ...
    def _user_roles_options(cls):
        _r = models.AuthUserRole.query().filter(models.AuthUserRole.level <= current_user.role.level).order_by(models.AuthUserRole.level.desc())
        return [ (r.id, r.name.upper()) for r in _r ]

    @harambe_deco.nav_title('All Users')
    def index(self):
        page_meta('All Users')
        ...
        return dict(user_roles_options=self._user_roles_options(), sorting_options=sorting, users=users, search_query={'include-deleted': request.args.get('include-deleted', 'n'), 
    ...
    @harambe_deco.nav_title('User Info', visible=False)
    def info(self, id):
        page_meta('User Info')
    ...
    def create(self):
        try:
            ...
            if not role:
                raise exceptions.AuthError('Invalid ROLE selected')
            if not first_name:
                raise harambe_exc.AppError('First Name is required')
            else:
                if not password or password != password_confirm:
                    raise harambe_exc.AppError("Passwords don't match")
                user = create_user(username=email, password=password, first_name=first_name, last_name=last_name, login_method='email', role=role.name)
                if user:
    ...
    @harambe_deco.nav_title('User Roles', order=2)
    @harambe_deco.accept_post_get
    def roles(self):
        """
        ...
        if request.method == 'POST':
            try:
                ...
                if name and level:
                    ...
                    if level in _levels or name in _names:
                        ...
                        if role:
                            if action == 'delete':
                                ...
                                if role.level != level and models.AuthUserRole.get_by_level(level):
                                    raise exceptions.AuthError("Role Level '%s' exists already" % level)
                                elif role.name != models.AuthUserRole.slug_name(name) and models.AuthUserRole.get_by_name(name):
                    ...
                    elif models.AuthUserRole.get_by_level(level):
        ...
        roles = models.AuthUserRole.query().order_by(models.AuthUserRole.level.desc())
