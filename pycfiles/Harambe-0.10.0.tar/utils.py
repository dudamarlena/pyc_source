...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/harambe/harambe/utils.py
# Compiled at: 2017-04-14 00:01:35
"""
...
def create_uri(host, user=None, password=None, port=None, path=None, scheme='http'):
    ...
    return ('{scheme}://{user_info}{host}{port}{path}').format(scheme=scheme, host=host, user_info=user_info, port=port, path=path)
...
def sign_jwt(data, secret_key, expires_in, salt=None, **kw):
    ...
    """
    try:
        s = itsdangerous.TimedJSONWebSignatureSerializer(secret_key=secret_key, expires_in=expires_in, salt=salt, **kw)
        return s.dumps(data)
...
def sign_url_safe(data, secret_key, expires_in=None, salt=None, **kw):
    ...
    try:
        if expires_in:
            expires_in *= 60
            s = URLSafeTimedSerializer2(secret_key=secret_key, expires_in=expires_in, salt=salt, **kw)
        else:
