...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/harambe/harambe/contrib/auth/models.py
# Compiled at: 2017-02-26 06:59:25
import datetime, arrow
from . import exceptions
from harambe import db, utils, send_mail, _
from harambe.exceptions import ModelError
...
class AuthUser(db.Model):
    ...
    login_method = db.Column(db.String(75))
    ...
    @classmethod
    def new(cls, username, password, email=None, first_name=None, last_name=None, login_method=None, role='MEMBER'):
        """
        Create a new user
        :param username: str
        :param password: str
    ...
    def search_by_name(cls, query, name):
        ...
        :return:
        """
        query = query.filter(db.or_(cls.first_name.contains(name), cls.last_name.contains(name)))
        return query
    ...
    def has_any_roles(self, *roles):
        ...
        if AuthUserRole.query().join(AuthUser).filter(AuthUserRole.name.in_(roles)).filter(AuthUser.id == self.id).count():
            return True
        return False
...
class AuthUserSocialLogin(db.Model):
    ...
    def get_user(cls, provider, social_id):
        login = cls.query().filter(cls.provider == provider).filter(cls.social_id == social_id).first()
        if login:
            return login.user
        else:
            return
    ...
    def new(cls, user, provider, social_id):
        ...
        return cls.create(user_id=user.id, provider=provider, social_id=social_id)