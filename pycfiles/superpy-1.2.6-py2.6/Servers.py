...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\superpy\core\Servers.py
...
"""
import os, socket, threading, SocketServer, logging, re, time, datetime, Tasks, PicklingXMLRPC, SimpleXMLRPCServer, DataStructures
...
class Scheduler():
    ...
    def __init__(self, hostList):
        ...
        for i in range(len(hostList)):
            ...
            if isinstance(entry, (str, unicode)):
                entry = [entry]
            if len(entry) == 1:
                host, port = entry[0], BasicRPCServer.defaultPort
            elif len(entry) == 2:
                (host, port) = entry
            else:
    ...
    def SubmitTaskToBestServer(self, task, *args, **kw):
        ...
        if len(loads) < 1:
            raise Exception('No server can be reached in %s seconds, re-try later' % newtimeout)
...
class BasicRPCServer(PicklingXMLRPC.PicklingXMLRPCServer, SocketServer.ThreadingMixIn):
    ...
    """
    _RPCFunctions = [
     'Submit', 'Terminate', 'RemoveFromQueue', 'Status',
    ...
    def __init__(self, cpus=None, host=None, port=None, *args, **kw):
        ...
        PicklingXMLRPC.PicklingXMLRPCServer.__init__(self, (host, port), requestHandler=NewRequestHandler, *args, **kw)
    ...
    def Submit(self, task, emailList=None, callbacks=None):
        ...
        serverTask = Tasks.ServerSideTask(task, self._host, self._port, emailList, callbacks)
    ...
    def CPULoad(self):
        ...
        """
        count = sum([ 1 if not task.finished.isSet() else 0 for (_name, task) in self._queue.ShowItems()
                    ])
    ...
    def ShowQueue(self, regexp='.*', timeout=0):
        ...
        """
        compiledRegexp = re.compile(regexp) if isinstance(regexp, (str, unicode)) else regexp
        result = []
    ...
    def CleanOldTasks(self, allowedLag=86400):
        ...
        tasksToClean = [ (name, task) for (name, task) in self._queue.ShowItems() if task.finished.isSet() if now - task.endtime > datetime.timedelta(seconds=allowedLag)
                       ]
    ...
    def UpdateActives(self):
        ...
        try:
            ...
            while len(activeItems) < self._cpus:
                pendingItems = [ (name, task) for (name, task) in self._queue.ShowItems() if not task.started.isSet()
                               ]
