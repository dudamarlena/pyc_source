...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\superpy\demos\pyfog\fogMaker.py
...
"""
import re, urllib, os, datetime, copy, logging, codecs, feedparser, ignores
...
class FogSource():
    ...
    def _GetWordsFromList(self, config, data):
        ...
        try:
            ...
            logging.warning("Can't decode data via codecs due to exception %s; retrying" % str(e)[0:256])

        if preKillData is None:
            preKillData = unicode(data, errors='replace').encode('ascii', 'ignore')
        data = re.sub(config.session.killRegexp, '', preKillData)
        if self.traceDir is not None:
            fileName = os.path.join(self.traceDir, re.sub('[^a-zA-Z0-9.]', '_', self.Name() + '_preKill.txt'))
            open(fileName, 'wb').write(preKillData)
            fileName = os.path.join(self.traceDir, re.sub('[^a-zA-Z0-9.]', '_', self.Name() + '_postKill.txt'))
...
class FogMachine():
    ...
    def __init__(self, config, sources=()):
        ...
        self.config = config
        self.scheduler = self._MakeTaskScheduler(self.config.superpy.serverList, self.config.superpy.localServers)
    ...
    def AddSource(self, source):
        ...
        if not isinstance(source, FogSource):
            raise TypeError('FogMachine.AddSource excepts a FogSource; got %s' % str(source))
    ...
    def CountWords(self):
        ...
        """
        self.wordCount = {}
        Manager.ProcessElements(self.sources, Manager.DoNothing, lambda e: self.DispatchElement(e), lambda e, r: self.ProcessResult(e, r))
    ...
    def _MakeTaskScheduler(serverList, localServers):
        ...
        if not isinstance(serverList, (list, tuple)):
            raise TypeError('serverList must be a sequence but got %s' % str(serverList))
    ...
    def _MakeTaskToRunSource(self, source):
        ...
        task = Tasks.ImpersonatingTask(targetTask=source, workingDir=workingDir, domain=self.config.superpy.domain, password=self.config.superpy.password, user=self.config.superpy.user, prependPaths=[
         self.config.superpy.remotePyPath.replace('\\', '/')], name='RemoteTest/pyfog/%s/%s/%s' % (
         datetime.datetime.now().strftime('%Y-%m-%d_%H:%M:%S'),
         self.config.superpy.user, source.Name()), mode='createProcess', env={'USERNAME': os.getenv('USER', os.getenv('USERNAME', None))}, wrapTask=True)
    ...
    def DispatchElement(self, element):
        ...
        if element.Remoteable():
            ...
            handle = TaskInfo.StaticHandle(element.Name(), {'starttime': now, 
    ...
    def MakeRankedResultList(self, numWords):
        ...
        sortedCounts = list(reversed(sorted(self.wordCount.items(), key=lambda pair: pair[1])))
        result = [
         ('Word').ljust(33) + ('hits').ljust(13) + ('%').ljust(10)]
        result.extend([ word.ljust(33) + ('%i' % count).ljust(13) + '%.5f%%' % (count / total * 100.0) for (word, count) in sortedCounts[0:numWords]
                      ])
        return ('\n').join(result)
    ...
    def _regr_test_simple():
        r"""Simple regression test for FogMachine.
