...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\superpy\utils\WindowsSubprocess.py
...
def CreateProcessWithLogonW(lpUsername=None, lpDomain=None, lpPassword=None, dwLogonFlags=0, lpApplicationName=None, lpCommandLine=None, dwCreationFlags=0, lpEnvironment=None, lpCurrentDirectory=None, lpStartupInfo=None):
    ...
    lpApplicationName = ctypes.c_wchar_p(lpApplicationName) if lpApplicationName else NULL
    lpCommandLine = ctypes.create_unicode_buffer(lpCommandLine) if lpCommandLine else NULL
    lpEnvironment = ctypes.c_wchar_p(lpEnvironment) if lpEnvironment else NULL
    lpCurrentDirectory = ctypes.c_wchar_p(lpCurrentDirectory) if lpCurrentDirectory else NULL
    ...
    success = ctypes.windll.advapi32.CreateProcessWithLogonW(lpUsername, lpDomain, lpPassword, dwLogonFlags, lpApplicationName, ctypes.byref(lpCommandLine), ctypes.c_uint(dwCreationFlags), lpEnvironment, lpCurrentDirectory, ctypes.byref(lpStartupInfo), ctypes.byref(lpProcessInformation))
    if success == FALSE:
