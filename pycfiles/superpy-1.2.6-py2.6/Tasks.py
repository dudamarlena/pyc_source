...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\superpy\core\Tasks.py
# Compiled at: 2010-06-04 07:07:11
"""Module providing various tasks to be excuted remotely.
"""
import os, threading, logging, sys, imp, stat, time, tempfile, smtplib, datetime
...
class ServerSideTask(threading.Thread):
    ...
    def __init__(self, clientTask, host, port, emailList, callbacks, *threadInitArgs, **threadInitKW):
    ...
    def GetHandle(self):
        ...
        while self.started.isSet() and self.finished.isSet() and self.isAlive() and secsToWaitForCleanup:
            logging.debug('waiting for cleanup...')
            time.sleep(0.1)
            secsToWaitForCleanup -= 0.1

        if self.started.isSet() and self.finished.isSet() and self.isAlive():
            raise Exception('Task %s finsihed but still alive; cleanups fail' % self.clientTask.Name())
        result = TaskHandle(self.clientTask.Name(), self.started.isSet(), self.finished.isSet(), self.isAlive(), self.host, self.port, self.clientTask.result, self.starttime, self.endtime, self.clientTask.user, taskRepr=repr(self.clientTask), pids=self.clientTask.GetPids())
        return result
    ...
    def run(self):
        ...
        try:
            ...
            try:
                ...
                logging.error('Got second exception while processing first: %s' % str(doubleE))
                raise
    ...
    def Finish(self):
        ...
        if self.finished.isSet():
            logging.warning('Called Finish on %s multiple times.' % self.clientTask.Name())
        else:
            self.finished.set()
...
class EmailingCallback(BasicCallback):
    ...
    def __init__(self, emailList=None, message='', subject='Task Report', attachments=[], fromName='automated-report', smtpServer=None):
        if smtpServer is None:
            smtpServer = config.smtpServer
        if None == emailList:
            emailList = [
             os.getenv('USER', os.getenv('USERNAME', None))]
            if emailList[0] is None:
                logging.error('Could not find user to email.')
                raise Exception('Could not find user to email')
        assert list == type(emailList) or tuple == type(emailList), 'emailList must be a list or tuple not a %s' % type(emailList)
        self.emailList = emailList
    ...
    def Report(self):
        ...
        for (itemName, itemData) in self.attachments:
            attachment = MIMENonMultipart.MIMENonMultipart('application', 'octet-stream')
            attachment.add_header('Content-Disposition', 'attachment', filename=itemName)
...
class BasicTask:
    ...
    def GetPids(self):
        ...
        """
        return [
         'task %s does not support GetPids; please fix' % str(self)]
    ...
    def _MakeRepr(self, params):
        ...
        return self.__class__.__name__ + '(' + (',\n').join('%s=%s' % (n, str(getattr(self, n, 'unknown'))[0:512]) for n in params) + ')'
    ...
    def _DoKill(self, pid):
        ...
        if os.name == 'nt':
            ...
            handle = win32api.OpenProcess(1, win32con.PROCESS_TERMINATE, pid)
            win32process.TerminateProcess(handle, -1)
            win32api.CloseHandle(handle)
        elif hasattr(os, 'kill'):
            os.kill(pid, signals.SIGABRT)
        else:
            raise Exception('Do not know how to kill on os.name=%s' % str(os.name))
...
class ImpersonatingTask(BasicTask):
    ...
    def __init__(self, targetTask, workingDir, prependPaths=None, domain=None, password=None, wrapTask=False, mode='subprocess', env=None, *args, **kw):
        """
        ...
        if prependPaths is None:
            prependPaths = list(reversed(ListAllParents(workingDir) + [workingDir]))
        if not isinstance(prependPaths, (list, tuple)):
            raise Exception('prependPaths must be list or tuple not %s' % prependPaths)
        self.targetTask = targetTask if not wrapTask else Process.PickledTaskWrapper(targetTask)
    ...
    def _ValidateMode(self):
        ...
        if self.mode != 'subprocess':
            if self.credentials is None or len(self.credentials) == 0 or self.credentials.get('password', None) is None:
    ...
    def Run(self, wait=True, reRaise=False):
        ...
        try:
            ...
            class Task(Process.RemoteProcess):
                ...
                def __init__(self, myTargetTask, remotePyPath, cred, exe, myWorkingDir, myPrependPaths, *args, **kw):
                ...
                def run(self):
                    ...
                    fileInfo = Process.RemoteFileInfo(childStdin=None, childStdout=None, childStderr=None, prependPaths=self.myPrependPaths, workingDir=self.myWorkingDir, exe=self.myExe)
                    result = self.SpawnTask(target=self.myTargetTask, fileInfo=fileInfo, debug=False, credentials=self.cred, wait=wait, mode=myMode, env=myEnv)
                    logging.debug('Got result: %s from spawning task' % str(result))
                    return result

            self.remoteTask = Task(self.targetTask, os.getenv('PYTHONPATH', None), self.credentials, self.exe, self.workingDir, self.prependPaths)
            logging.debug('Running remoteTask for %s from server' % self.targetTask)
            try:
                self.result = self.remoteTask.run()
            except Exception, e:
                logging.warning('got exception from running remoteTask %s: %s; re-raise' % (
                 self.targetTask, str(e)))
                raise

            logging.debug('finished running remoteTask %s: got %s' % (
...
def ListAllParents(dirName):
    ...
    """
    result = [
     os.path.dirname(dirName).rstrip('/').rstrip('\\').rstrip(os.sep)]
    if not len(result[0]) or result[0][(-1)] in ('/', '\\', ':', ''):
        logging.debug('Not including path of %s since it looks like root' % result[0])
        return []
    else:
        if dirName == result[0] or len(dirName) == len(result[0]):
            return result
