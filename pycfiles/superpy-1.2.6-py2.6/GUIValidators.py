...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\superpy\SuperWatch\GUIValidators.py
# Compiled at: 2010-06-28 08:51:24
"""Module containing the Validator class and sub-classes.

See docs on Validator class to start.
"""
import Tkinter, datetime, os, re
...
class BooleanValidator(Validator):
    ...
    def Validate(self, arg):
        ...
        if str == type(arg):
            arg = arg.strip()
        if None == arg or 0 == arg or '' == arg or '0' == arg or False == arg or 'False' == arg:
            return 0
        else:
            return 1
            return
...
class BooleanPulldownValidator(ChooseOneFromListValidator):
    ...
    def __init__(self, default='False'):
        ChooseOneFromListValidator.__init__(self, options=['True', 'False'], default=str(default))

    def Validate(self, arg):
        """Validate boolean"""
        if arg is False or arg in (0, '0', 'False'):
            return False
        if arg is True or arg in (1, '1', 'True'):
            return True
...
class LogLevelValidator(ChooseOneFromListValidator):
    ...
    def __init__(self, default='WARNING'):
        ChooseOneFromListValidator.__init__(self, options=['CRITICAL', 'ERROR', 'WARNING', 'INFO', 'DEBUG'], default=default)
...
class YesNoValidator(ChooseOneFromListValidator):
    ...
    def __init__(self, default):
        ChooseOneFromListValidator.__init__(self, options=['yes', 'no'], default=default)
...
class FloatValidator(Validator):
    ...
    def Validate(self, arg):
        ...
        if arg is None or arg == 'None' or arg == '':
            return
        else:
            value = float(arg)
            if None != self.minVal and value < self.minVal:
                raise Exception, 'Value ' + `value` + ' too small; minimum allowed value = ' + `(self.minVal)` + '.'
            if None != self.maxVal and value > self.maxVal:
                raise Exception, 'Value ' + `value` + ' too big; maximum allowed value = ' + `(self.maxVal)` + '.'
            return value
...
class StringItemDictValidator(Validator):
    ...
    def Validate(self, arg):
        ...
        if arg is None or arg == 'None' or arg == '':
            ...
            arg = arg.split(',')
            arg = dict([ (s.split(':')[0], self._typeConverter_(s.split(':')[1])) for s in arg
                       ])
        else:
...
class PairItemListValidator(Validator):
    ...
    def Validate(self, arg):
        ...
        if arg is None or arg == 'None' or arg == '':
            ...
            arg = arg.split(',')
            arg = [ (self._typeConverters_[0](s.split(':')[0]), self._typeConverters_[1](s.split(':')[1])) for s in arg
                  ]
        else:
            raise Exception('Invalid value %s for %s.' % (
             str(arg), self.__class__.__name__))
...
class StringStringDictValidator(Validator):
    ...
    def Validate(self, arg):
        ...
        if arg is None or arg == 'None' or arg == '':
            ...
            arg = dict([ (stripQuotes(s.split(':')[0]), stripQuotes(s.split(':')[1])) for s in arg
                       ])
        else:
...
class FileNameValidator(Validator):
    ...
    def Validate(self, arg):
        ...
        if self.mustexist and None != arg and '' != arg.strip() and not os.path.exists(arg):
            raise Exception, 'No file named ' + `arg` + ' exists.'
        elif '<' in arg or '>' in arg:
            raise Exception('File name %s has forbidden characters like <,>.' % arg)
        else:
            return arg
        return
    ...
    def MakeMenuEntry(self, argFrame):
        ...
        browseButton = Tkinter.Button(browseFrame, text='(Browse)', command=lambda : BrowseForFile(browseFrame, entry, self.initialDir, self.mustexist))
...
class ExistingFileNameValidator(FileNameValidator):
    ...
    def __init__(self, initialDir=None, default=None):
        FileNameValidator.__init__(self, initialDir=initialDir, mustexist=True, default=default)
...
class ExistingOrNonExistingFileNameValidator(FileNameValidator):
    ...
    def __init__(self, initialDir=None, default=None):
        FileNameValidator.__init__(self, initialDir=initialDir, mustexist=False, default=default)
...
def BrowseForFile(parentWindow, stringVarToSet, initialDir, mustexist):
    ...
    stringVarToSet.set(parentWindow.tk.call(command, '-parent', parentWindow, '-initialdir', initialDir))
...
class DirNameValidator(Validator):
    ...
    def __init__(self, initialDir=None, mustExist=False, default=None):
        if default is not None:
            if initialDir is None:
                ...
                raise Exception('default is alias for initialDir; only one should be non-None')
        if initialDir is None:
            initialDir = os.getcwd()
    ...
    def MakeMenuEntry(self, argFrame):
        ...
        if None != self.default:
            entry.set(self.default)
        menuEntry = Tkinter.Entry(browseFrame, textvariable=entry)
        browseButton = Tkinter.Button(browseFrame, text='(Browse)', command=lambda : BrowseForDir(browseFrame, entry, self.initialDir, self.mustExist))
        menuEntry.pack(side='top', expand=1, fill='x')
        browseButton.pack(side='top', expand=1, fill='x')
        return (
         browseFrame, entry)
...
def BrowseForDir(parentWindow, stringVarToSet, initialDir, mustExist):
    ...
    """
    stringVarToSet.set(parentWindow.tk.call('tk_chooseDirectory', '-parent', parentWindow, '-initialdir', initialDir, '-mustexist', mustExist))
