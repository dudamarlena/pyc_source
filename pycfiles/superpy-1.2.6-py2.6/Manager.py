...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\superpy\core\Manager.py
# Compiled at: 2010-06-04 07:07:11
"""Module providing a client manager to help distribute tasks over superpy.
"""
import datetime, socket, logging, math, cPickle, TaskInfo, Process, Tasks
...
0.0000,1.0000,1.4142,1.7321
    ...
    def __init__(self, scheduler, elementList, workingDir, credentials=None):
        """Initializer.
    ...
    def Process(self, procParams=None):
        ...
        """
        ProcessElements(self.elementList, self._DecorateElement, self._DispatchElement, self._HandleResult, procParams)
    ...
    def _DispatchElement(self, element):
        ...
        """
        if self.scheduler:
            logging.debug('Creating impersonating task for element %s' % element.Name())
            impTask = Tasks.ImpersonatingTask(element, workingDir=self.workingDir, prependPaths=None, wrapTask=True, mode='createProcess', name=element.Name(), **self.credentials)
            handle = self.scheduler.SubmitTaskToBestServer(impTask)
            return handle
        else:
            logging.debug('Locally running element for %s' % element.Name())
            result = element.Run()
            return TaskInfo.StaticHandle(element.Name(), {'result': cPickle.dumps(result)})
            return
...
def ProcessElements(elements, decorateElement, dispatchElement, handleResult, procParams=None):
    ...
    while elementNum < numElements:
        ...
        elementNum += 1
        untilFinished = None if len(unfinishedHandles) < procParams.maxUnfinished else 1
        (finishedHandles, unfinishedHandles) = WaitForTasks(unfinishedHandles, untilFinished=untilFinished, maxTime=procParams.maxTime)
        CleanupFinishedHandles(finishedHandles, handleResult)

    (finishedHandles, unfinishedHandles) = WaitForTasks(unfinishedHandles, untilFinished=len(unfinishedHandles))
    CleanupFinishedHandles(finishedHandles, handleResult)
    return
...
def WaitForTasks(handles, untilFinished=None, maxTime=0, handleException=None):
    ...
    if handleException is None:
        handleException = lambda _h, _el, exc: logging.debug('Ignoring exception %s' % str(exc))
    ...
    while keepGoing:
        ...
        for (handle, element) in handles:
            ...
            try:
                ...
                elif maxTime and newHandle.started and info['starttime'] > threshold:
                    logging.warning('Timeout exceeded; killing task %s' % str(newHandle))
                    newHandle.Kill()
                ...
                try:
                    ...
                    logging.error('Could not show handle info due to exception:%s' % str(eAgain))
                else:
        ...
        handles = tempHandles
        keepGoing = len(handles) > 0 and untilFinished is not None and len(finishedHandles) < untilFinished
