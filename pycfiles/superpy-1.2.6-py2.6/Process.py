...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\superpy\core\Process.py
...
"""
import sys, os, random, subprocess, logging, re, copy, time, threading, cPickle, traceback, socket, PicklingXMLRPC, Forker, TaskInfo
try:
...
def ChangeUser(domain, user, password):
    ...
    handle = win32security.LogonUser(user, domain, password, win32con.LOGON32_LOGON_INTERACTIVE, win32con.LOGON32_PROVIDER_DEFAULT)
...
class RemoteFileInfo():
    ...
    def __init__(self, childStdin, childStdout, childStderr, prependPaths, appendPaths=('.', ), remotePyPath=None, workingDir=None, exe=None):
        """Initializer.
...
class RemoteProcess():
    ...
    _publicMethods = [
     'AcceptStudent']
    _masterMethods = [
     'ShowPath', 'SetCWD', 'ExtendSysPath', 'ProcessObject',
    ...
    def PrependSysPath(pathList):
        ...
        if not isinstance(pathList, (list, tuple)):
            raise Exception('Expected pathList to be a list or tuple but got %s.' % repr(pathList))
    ...
    def _GetTokenAndPeerWithMaxID(self):
        ...
        if len(self._peers) == 0:
            ...
            return max(self._peers.items(), key=lambda (token, relationship): relationship.uniqueID)
            return
    ...
    def _GetNewUniqueID(self):
        ...
        if len(self._peers) == 0:
            ...
            return self._GetTokenAndPeerWithMaxID()[1].uniqueID + 1
    ...
    def RequestMaster(self, host, port):
        ...
        newPeers = {token: PeerRelationship(host, port, self._masterMethods, uniqueID, 'master')}
    ...
    def AcceptStudent(self, host, port, token):
        ...
        if token in self._peers:
            ...
            uniqueID = self._GetNewUniqueID()
            self._peers[token] = PeerRelationship(host, port, [], uniqueID, 'student')
    ...
    def SpawnTask(self, target, fileInfo, host='localhost', wait=True, port=None, debug=False, credentials=None, priority=None, mode='subprocess', env=None):
    ...
    See the docstrings for the following methods for some examples:
      ...
      _picklingExample
        ...
        forkArgs = Forker.MakeCommandLine(fileInfo.exe, Forker.__file__, host, port)
        (childProc, self._pipePid) = self._CreateSubprocess(forkArgs, fileInfo, mode, credentials)
        ...
        try:
            connectionToChild = PicklingXMLRPC.PicklingServerProxy('http://%s:%i' % (newPeer.host, newPeer.port))
            if credentials is not None and len(credentials) == 3:
                if mode == 'subprocess':
                    logging.info('Requestig change user to %s' % credentials['user'])
                    msg = connectionToChild.SetUser(newToken, credentials['domain'], credentials['user'], credentials['password'])
            ...
            if len(fileInfo.prependPaths):
                connectionToChild.PrependSysPath(newToken, fileInfo.prependPaths)
            if len(fileInfo.appendPaths):
                connectionToChild.ExtendSysPath(newToken, fileInfo.appendPaths)
            ...
            if wait:
                ...
                try:
                    try:
                        ...
                        logging.error('Ignoring error %s when asking child to die.' % str(myException))
                ...
                result = TaskInfo.ThreadHandle(newToken, target, connectionToChild, childProc, newPeer.host, newPeer.port)
        except Exception, e:
    ...
    def _CreateSubprocess(cmdLine, fileInfo, mode, credentials):
        ...
        if mode == 'subprocess':
            ...
            if fileInfo.remotePyPath is not None:
                newEnv['PYTHONPATH'] = os.path.normpath(fileInfo.remotePyPath).replace('\\', '/')
                logging.info('Using env:\n%s\n' % str(newEnv))
            pipeToChild = subprocess.Popen(cmdLine, stdin=fileInfo.childStdin, stdout=fileInfo.childStdout, stderr=fileInfo.childStderr, env=newEnv)
            pid = pipeToChild.pid
        elif mode == 'createProcess':
            pipeToChild = winprocess.Process((' ').join(cmdLine), '%s\n%s\n%s' % (
             credentials['domain'], credentials['user'],
             credentials['password']))
            pid = pipeToChild.PId
        elif mode == 'logonW':
            pipeToChild = WindowsSubprocess.CreateProcessWithLogonW(credentials['user'], credentials['domain'], credentials['password'], 0, cmdLine[0], (' ').join(cmdLine), lpCurrentDirectory=os.path.normpath(os.getcwd()), dwCreationFlags=0)
    ...
    @staticmethod
    def _simpleExample():
        r"""Docstring illustrating a simple example of usage.
...
False
    ...
    def _exampleWithNoWait():
        r"""Docstring illustrating simple example with wait=False.
...
False
    ...
    def _picklingExample():
        r"""Docstring illustrating simple example with pickling wrapper.
...
False
    ...
    def _dispatch(self, method, params):
        ...
        if method not in allowedMethods:
            raise Exception('Invalid method name %s for token %s. Allowed methods=\n%s\n' % (
...
def GetCallMethod(obj, options=(
 'Run', 'run', '__call__')):
    ...
    raise Exception('Object %s does not have any methods in %s' % (str(obj), options))


def CallIt(obj, options=(
 'Run', 'run', '__call__')):
...
class _DoubleSpawnExample():
    ...
    def __init__(self, credentials=None, mode='createProcess', prependPaths=None):
        self.proc = None
        self.result = None
    ...
    def run(self):
        ...
        fileInfo = RemoteFileInfo(prependPaths=self.prependPaths, childStdin=None, childStdout=None, childStderr=None)
        logging.warning('Doing double spawn')
        self.result = self.proc.SpawnTask(_ExampleForTest(), fileInfo, credentials=self.credentials, mode=self.mode)
        return self.result
...
class PickledTaskWrapper():
    ...
    def __init__(self, target, pickleResult=True, pickleMode='s'):
        ...
        """
        _callMethod = GetCallMethod(target)
    ...
    def __repr__(self):
        ...
        args = (', ').join([ '%s=%s' % (name, repr(getattr(self, name))[0:128]) for name in [
         'target', 'pickleResult']
                           ])
...
def MakeErrTrace(myException, extraMsg=''):
    ...
    return "Exception of type '" + str(sys.exc_type) + "':  '" + myException.__str__() + "'.\n\n\n" + extraMsg + ('').join(traceback.format_tb(sys.exc_info()[2])) + str(sys.exc_info)
