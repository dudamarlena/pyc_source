...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\superpy\SuperWatch\GUIUtils.py
# Compiled at: 2010-06-04 07:07:10
"""Module containing utilities for running commands in GUI.
"""
import sys, logging, traceback, datetime, Tkinter, tkFont, tkMessageBox
...
def SaveTextToFile(text, parent):
    ...
    """
    fileName = parent.tk.call('tk_getSaveFile', '-parent', parent, '-initialdir', 'c:')
...
def MakeTextDialog(title, text, grab=0, extraCommands=None, fontArgs=None):
    ...
    saveMsgButton = Tkinter.Button(buttonFrame, text='Save', command=lambda : SaveTextToFile(message.get(1.0, Tkinter.END), window))
...
class GenericGUICommand:
    ...
    def ProcessCommand(self, window):
        ...
        try:
            ...
            MakeTextDialog(title='Command Output', text='Command %s completed at %s with result:\n%s\n' % (
    ...
    def MakeWindow(self, argList, window):
        ...
        """
        self.argDict = dict([ (n, {'name': n, 'validator': v, 'doc': d, 'entry': None, 'value': None}) for (n, v, d) in argList
                            ])
        if window is None:
            window = Tkinter.Toplevel()
        ...
        for (row, (name, validator, doc)) in enumerate(argList):
            ...
            (menuEntry, self.argDict[name]['entry']) = validator.MakeMenuEntry(argFrame)
        ...
        okButton = Tkinter.Button(buttonFrame, text='OK', command=lambda : self.ProcessCommand(window))
        cancelButton = Tkinter.Button(buttonFrame, text='Cancel', command=lambda : self.CancelCommand(window))
        helpButton = Tkinter.Button(buttonFrame, text='Help', command=lambda : self.HelpCommand(window))
        commandTitle = Tkinter.Label(window, text='Run ' + self.name, relief='ridge', padx=3, pady=3)
    ...
    def HelpCommand(self, window):
        """Provide help on command"""
        _ignore = window
        MakeTextDialog(title='Help on %s' % self.name, text=self.__doc__, grab=1)
...
def ComplainAboutException(e, errTrace, window=None):
    ...
    if window is None:
        window = Tkinter.Toplevel()
        killWindow = True
    showTraceBack = window.tk.call('tk_dialog', '.showTBOnErrorDialog', 'Error', errMsg, 'error', 0, 'OK', 'Show Traceback', 'Debug')
    if killWindow:
        window.destroy()
