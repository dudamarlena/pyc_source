...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\superpy\SuperWatch\SPWidgets.py
# Compiled at: 2010-06-28 08:51:54
"""Module providing widgets for SuperWatch.
"""
import Tkinter, Pmw, csv, datetime, logging, os, traceback, sys, threading, time, smtplib, GUIValidators, GUIUtils, Periodicity, SuperInfo, MsgWindow
...
class HelperPage():
    ...
    def MakeFrame(self, notebook):
        ...
        self.frame = getattr(Pmw, 'ScrolledFrame')(page, labelpos='n', horizflex='expand', vertflex='expand')
...
class ConfigPage(HelperPage):
    ...
    def _MakeConfigPage(self):
        ...
        button = Tkinter.Button(buttonFrame, text='Apply', command=GUIUtils.GenericGUICommand('ApplyConfig', lambda : [ item.Apply(self.master) for (_name, item) in confItems ]))
        ...
        for (name, value) in defaults.items():
            if name not in self.confItemDict:
                raise Exception('Invalid default item "%s". Possible values are:\n%s' % (
...
class TaskPage(HelperPage):
    ...
    def _MakeTaskFrameMenu(self):
        ...
        menu.add_command(label='ImportTasks', command=ImportTasksCmd(master, '.', '.', '.'))
        menu.add_command(label='CleanTasks', command=CleanTasksCmd(master, '.', '.', '.', master.GetParam('autoClean')))
    ...
    def MakeTaskFrameItems(self, allTasks):
        ...
        for (row, task) in enumerate(allTasks):
            ...
            info = self.master.MakeTaskItems(mainFrame, task.Name(), [server], server, None)
    ...
    def Kill(self, info):
        ...
        if info.status.get() == 'unknown':
            raise Exception('Task "%s" not connected. Try Show or Launch first.' % info.name)
        else:
...
class ScriptPage(HelperPage):
    ...
    def _ParseScriptFile(scriptFile):
        ...
        """
        varDict = {'user': os.getenv('USER', os.getenv('USERNAME', 'unknown')), 
           'cwd': os.getcwd()}
        ...
        for (num, line) in enumerate(reader):
            if len(line) == 0 or map(len, line) == [0] * len(line) or line[0][0] == '#':
                pass
            elif len(line) != len(headers):
                raise Exception('Line %i invalid: expected %i fields, got %i :\n%s\n' % (
    ...
    def _AddScriptsToNotebook(self, scripts):
        ...
        for page in pages:
            ...
            frame = getattr(Pmw, 'ScrolledFrame')(pageFrame, labelpos='n', horizflex='expand', vertflex='expand')
            frame.pack(expand=1, fill='both')
            pageFrames[page] = frame.interior()
            serverOptions = ['<best>', '<local>'] + [ '%s:%s' % (host, port) for (host, port) in self.master.helpers['Servers'].servers
                                                    ]
            for h in headers:
                Tkinter.Label(pageFrames[page], text=h).grid(row=0, column=hColDict[h])

        numItems = dict([ (n, 0) for n in pageFrames ])
        for rowData in scripts:
            scriptName = rowData[0]
            params = self.scriptDict[scriptName]
            params['period'] = Periodicity.ParsePeriod(params['autoRunAt'], failureMsgPrefix='\n                Could not parse period for script %s. Using Never.\n                ' % scriptName)
            info = self.master.MakeTaskItems(pageFrames[params['page']], scriptName, serverOptions, params['server'], params['scriptFile'], params['period'], 1 + numItems[params['page']])
        ...
        try:
            for (script, _params) in self.scriptDict.items():
                ...
                if len(theDict) == 0:
                    ...
                    raise Exception('Cannot handle script with multiple servers: %s' % str(script, sorted(theDict)))
                else:
            ...
            errTrace = (
             "Exception of type '" + `(sys.exc_type)` + "':  '" + e.__str__() + "'.\n\n\n" + ('').join(traceback.format_tb(sys.exc_info()[2])),
             sys.exc_info())
            ...
            try:
                ...
                if info.enable.get() and info.period.ActivateP():
                    ...
                    try:
                        ...
                        self.master.MakeWarning('update problem', 'Got exception when updating task %s:\n%s' % (
                         info.name, str(e)))
                    else:
                        self.master.helpers['Scripts'].Launch(info)
            except Exception, e:
                errTrace = 'Exception of type %s:\n%s\n%s\n%s\n' % (
                 str(sys.exc_type), e.__str__(),
                 ('').join(traceback.format_tb(sys.exc_info()[2])), sys.exc_info())
    ...
    def Launch(self, info, useSetServer=False):
        ...
        if info.status.get() in ('unknown', 'cleaned'):
            ...
            task = Tasks.ImpersonatingTask(targetTask=Tasks.ImportPyTask(fileName=params['scriptFile'], name=info.name), workingDir=workingDir, domain=self.master.GetParam('domain'), password=self.master.GetParam('password'), user=self.master.GetParam('user'), name=info.name, mode='createProcess')
            if useSetServer:
...
class ServerPage(HelperPage):
    ...
    def ReloadServers(self, serverData):
        ...
        for (host, port) in serverData:
            ...
            menuButtonFrame = Tkinter.Frame(itemFrame, borderwidth=2, relief='raised')
            menuButton = Tkinter.Menubutton(menuButtonFrame)
            menu = Tkinter.Menu(menuButton, tearoff=0)
            menuButton.configure(menu=menu)
            menuButton.configure(text='Actions')
            menu.add_command(label='Show', command=GUIUtils.GenericGUICommand('Show %s:%s' % (host, port), self.ShowServer, (host, port)))
            menu.add_command(label='Clean', command=CleanTasksCmd(self.master, host, port, '.', self.master.GetParam('autoClean')))
            menu.add_command(label='Import', command=ImportTasksCmd(self.master, host, port, '.'))
            menu.add_command(label='Launch', command=LaunchTasksCmd(self.master, host))
    ...
    def ShowTask(self, info, timeout=3, cache=None):
        ...
        """
        return SuperInfo.RefreshInfo(self.scheduler, info, timeout, cache, idle=self.master.notebook.update_idletasks)
...
class MasterMonitor():
            ...
            if smtp is not None and smtp.strip() not in ('', 'None'):
                if self._emailsSent[1] > 100:
                    if self._emailsSent[0] == datetime.date.today():
                        logging.warning('Already sent too many emails today. Not sending')
                        return
    ...
    def _MakeSuperWatchButtonFrame(self, rootWindow, quitCmd):
        ...
        helpButton = Tkinter.Button(buttonFrame, text='Help', command=lambda : self.MakeMainHelpDialog())
    ...
    def _MakeMainFrame(self, parent):
        ...
        buttonFrame = self._MakeSuperWatchButtonFrame(parent, lambda : parent.destroy())
        nameFrame.pack(side='top', expand=0, fill='x')
        mainFrame.pack(fill='both', expand=1, side='top')
        window = getattr(Pmw, 'PanedWidget')(mainFrame, orient='vertical', hull_height=500, hull_width=500)
        pane = window.add('Main', min=0.6, size=0.9)
        self.notebook = getattr(Pmw, 'NoteBook')(pane)
        pane = window.add('msgs', min=0.1, size=0.1)
        ...
        window.configurepane('msgs', size=0.1)
        window.configurepane('Main', size=0.8)
        window.updatelayout()
    ...
    def _MakeSuperWatchNameFrame(rootWindow):
        ...
        nLabel = Tkinter.Label(frame, foreground='blue', background='white', padx=5, pady=5, text='SuperWatch')
    ...
    def CheckMajorUpdates(self):
        ...
        if now - self._lastMajorUpdates > datetime.timedelta(seconds=self.GetParam('majorPollFrequency')):
            logging.debug('Doing major update at %s' % now)
            SuperInfo.CleanTasks(self.helpers['Servers'].scheduler, '.', '.', '.', self.GetParam('autoClean'))
    ...
    def AutoUpdateTaskStatus(self, groupName):
        ...
        keysToRemove = []
        try:
            (taskDict, _msg) = SuperInfo.MakeTaskDict(self.helpers['Servers'].scheduler, idle=self.notebook.update_idletasks)
        except Exception, e:
        ...
        for (taskName, cmdTaskDict) in commands.items():
            for (serverName, info) in cmdTaskDict.items():
                ...
                try:
                    ...
                    errTrace = ("Exception of type '" + `(sys.exc_type)` + "':  '" + e.__str__() + "'.\n\n\n" + ('').join(traceback.format_tb(sys.exc_info()[2])),
                     sys.exc_info())
    ...
    def MakeTaskItems(self, frame, taskName, serverOptions, defaultServer, scriptFile, period=None, row=None):
        ...
        """
        info = TaskInfo(taskName, defaultServer, period=period, enable=row is not None and self.GetParam('enableAutoRun'))
        ...
        if scriptFile:
            ...
            menu.add_command(label='Launch', command=GUIUtils.GenericGUICommand('Launch %s' % taskName, self.helpers['Scripts'].Launch, [info, True]))
        menu.add_command(label='Kill', command=GUIUtils.GenericGUICommand('Kill %s' % taskName, self.helpers['Tasks'].Kill, [info]))
        menu.add_command(label='KillAndReset', command=GUIUtils.GenericGUICommand('KillAndReset %s' % taskName, self.helpers['Tasks'].KillAndReset, [
         info]))
        menu.add_command(label='Show', command=GUIUtils.GenericGUICommand('Show %s' % taskName, self.helpers['Servers'].ShowTask, [info]))
        menuButton.pack(fill='x', expand=1, side='left')
        info.itemList = [
         (
    ...
    def ImportTasks(self, hostRE, portRE, taskRE):
        ...
        """
        (tasks, msg) = SuperInfo.SearchForTasks(self.helpers['Servers'].scheduler, hostRE, portRE, taskRE, idle=self.notebook.update_idletasks)
...
class ImportTasksCmd(GUIUtils.GenericGUICommand):
    ...
    def __init__(self, monitor, hostRE, portRE, taskRE):
        ...
        GUIUtils.GenericGUICommand.__init__(self, 'ImportTasks', command=None, argList=[
         (
...
class LaunchTasksCmd(GUIUtils.GenericGUICommand):
    ...
    def __init__(self, monitor, host, port=9287):
        self.monitor = monitor
        GUIUtils.GenericGUICommand.__init__(self, 'LaunchTask', command=None, argList=[
         (
    ...
    def _DoCommand(self):
        ...
        task = Tasks.ImpersonatingTask(targetTask=Tasks.ImportPyTask(fileName=taskScript, name=taskName), workingDir=os.path.dirname(taskScript), domain=self.monitor.GetParam('domain'), password=self.monitor.GetParam('password'), user=self.monitor.GetParam('user'), name=taskName, mode='createProcess')
        _handle = self.monitor.helpers['Servers'].SubmitTask(task, '%s:%i' % (host, port))
...
class CleanTasksCmd(GUIUtils.GenericGUICommand):
    ...
    def __init__(self, monitor, hostRE, portRE, taskRE, threshold):
        self.monitor = monitor
        GUIUtils.GenericGUICommand.__init__(self, 'CleanTasks', command=None, argList=[
         (
          'hostRegexp', GUIValidators.StringValidator(hostRE),
          '\n            Regular expression for hosts to search.'),
         (
         ...
         (
          'threshold',
          GUIValidators.IntValidator(default=threshold, minVal=0),
    ...
    def _DoCommand(self):
        ...
        result = SuperInfo.CleanTasks(self.monitor.helpers['Servers'].scheduler, hostRE, portRE, taskRE, threshold)
        return result
...
class ConfigItem():
    ...
    def __init__(self, frame, name, validator, doc=None):
        ...
        self.helpButton = Tkinter.Button(frame, text='?', command=lambda : GUIUtils.MakeTextDialog(title='Help on %s' % self.name, text=self.doc))
...
class PollFrequency(ConfigItem):
    ...
    def __init__(self, argFrame):
        ConfigItem.__init__(self, argFrame, 'pollFrequency', GUIValidators.IntValidator(default=60000))
...
class MajorPollFrequency(ConfigItem):
    ...
    def __init__(self, argFrame):
        ConfigItem.__init__(self, argFrame, 'majorPollFrequency', GUIValidators.IntValidator(default=3600000))
...
class EnableScripts(ConfigItem):
    ...
    def __init__(self, argFrame):
        ConfigItem.__init__(self, argFrame, 'enableAutoRun', GUIValidators.BooleanValidator(default=False))
...
class ServerConfItem(ConfigItem):
    ...
    def __init__(self, argFrame):
        ConfigItem.__init__(self, argFrame, 'servers', GUIValidators.StringIntListValidator())
...
class SMTPConfItem(ConfigItem):
    ...
    def __init__(self, argFrame):
        ConfigItem.__init__(self, argFrame, 'smtp', GUIValidators.StringValidator())
...
class ScriptConfItem(ConfigItem):
    ...
    def __init__(self, argFrame):
        ConfigItem.__init__(self, argFrame, 'scripts', GUIValidators.ExistingFileNameValidator())
    ...
    def Apply(self, master):
        ...
        if scriptName in ('None', None, ''):
            master.MakeWarning('no scripts', 'No script name given; not loading scripts')
        else:
...
class LogConfItem(ConfigItem):
    ...
    def __init__(self, argFrame):
        ConfigItem.__init__(self, argFrame, 'logLevel', GUIValidators.LogLevelValidator())
...
class AutoCleanConfItem(ConfigItem):
    ...
    def __init__(self, argFrame):
        ConfigItem.__init__(self, argFrame, 'autoClean', GUIValidators.IntValidator(default=30000))
...
class DomainConfItem(ConfigItem):
    ...
    def __init__(self, argFrame):
        ConfigItem.__init__(self, argFrame, 'domain', GUIValidators.StringValidator())
...
class UserConfItem(ConfigItem):
    ...
    def __init__(self, argFrame):
        ConfigItem.__init__(self, argFrame, 'user', GUIValidators.StringValidator())
...
class PasswordConfItem(ConfigItem):
    ...
    def __init__(self, argFrame):
        ConfigItem.__init__(self, argFrame, 'password', GUIValidators.StringValidator(meOptions={'show': '*'}))
...
class TaskInfo():
    ...
    def __init__(self, name, defaultServer, status='unknown', started='unknown', finished='unknown', enable=1, period=None, handle=None):
        """Initializer.
    ...
    def SetFromHandle(self, status=None, server=None, started=None, finished=None, handle=None):
        ...
        self.started.set('-' if started is None else started.strftime('%Y-%m-%d %H:%M:%S'))
        self.finished.set('-' if finished is None else finished.strftime('%Y-%m-%d %H:%M:%S'))
        return

    def __repr__(self):
        params = [ '%s=%s' % (name, repr(getattr(self, name))) for name in [
         'name', 'status', 'server', 'started', 'finished',
