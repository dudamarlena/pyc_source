...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\superpy\SuperWatch\SuperInfo.py
...
def SearchForTasks(scheduler, hostRE='.', portRE='.', taskRE='.', threaded=True, idle=None):
...
def MakeTaskDict(*searchArgs, **searchKW):
    ...
    for t in allTasks:
        ...
        if (t.host, t.port) in subDict:
            raise Exception('Got multiple tasks with same name, host, port:\n%s,%s,%s' % (
             name, t.host, t.port))
    ...
    def run(self):
        ...
        try:
            for i in range(self.attempts):
                ...
                candidateTasks = self.scheduler.ShowQueue(self.host, self.port, myTimeout)
            ...
            if candidateTasks is None:
                self.msg.append('Unable to contact %s:%s in %s attempts. Ignoring' % (
                 self.host, self.port, self.attempts))
                logging.warning(self.msg[(-1)])
                candidateTasks = []
            found = [ t for t in candidateTasks if self.taskRE.search(t.Name()) ]
...
def CleanTasks(scheduler, hostRE, portRE, taskRE, threshold):
    ...
    for task in allTasks:
        if task.finished and datetime.datetime.now() - task.endtime > threshold:
...
def RefreshInfo(scheduler, info, timeout=3, cache=None, idle=None):
    ...
    if isinstance(info.handle, (str, unicode)):
        raise Exception('Got unexpected string "%s" as info.handle.' % info.handle)
    if info.server.get() == '<local>' and info.handle is not None and not info.handle.StaleP():
        ...
        tasks, msg = [], ''
    if cache is None:
        (cache, _msg) = SearchForTasks(scheduler, taskRE='^%s$' % info.name, idle=idle)
    tasks.extend(cache)
    if len(tasks) == 0:
        info.Reset()
