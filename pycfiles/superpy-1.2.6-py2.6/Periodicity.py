...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\superpy\SuperWatch\Periodicity.py
...
False
    ...
    def __init__(self, periodStr):
        if periodStr is None or periodStr.strip() in ('None', 'NONE', ''):
            ...
            raise Exception('The periodStr %s is not valid for NoPeriod.' % str(periodStr))
        return
...
True
    ...
    def __init__(self, periodStr):
        ...
        if self.typeChar not in ('A', 'D'):
            raise Exception('Could not parse EveryDayAt periodicity from %s.' % periodStr)
    ...
    def ActivateP(self, now=None):
        ...
        if self.typeChar == 'A' or now.weekday() < 5:
            timeTarget = self.timeTarget.replace(year=now.year, month=now.month, day=now.day)
...
True
    ...
    def __init__(self, periodStr, now=None):
        ...
        if letter != 'M':
            raise Exception('Invalid periodStr %s for ModMinutes.' % str(periodStr))
        self.minutes = int(minutes)
        if self.minutes < 0 or self.minutes > 60:
            raise Exception('Invalid minutes in periodStr %s for ModMinutes.' % str(periodStr))
...
def ParsePeriod(periodStr, failureMsgPrefix=''):
    ...
    exceptions = []
    classes = [
     NoPeriod, ExactDateTime, EveryDayAt, SomeDaysAt, ModMinutes]
    for c in classes:
        try:
            return c(periodStr)
        except Exception, e:
    ...
    raise Exception(failureMsgPrefix + '\n    Could not parse periodStr %s. Attempts shown below:\n%s\n\n    ' % (periodStr,
     ('\n').join([ '%s : %s' % (c.__name__, str(e)) for (c, e) in zip(classes, exceptions)
            ])))
