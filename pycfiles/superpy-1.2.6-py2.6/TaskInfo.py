...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\superpy\core\TaskInfo.py
# Compiled at: 2010-06-04 07:07:11
"""Module containing various classes to provide information about tasks.
"""
import socket, xmlrpclib, logging, time, datetime, threading, copy, PicklingXMLRPC
...
class InvalidHandle(GenericProcessHandle):
    ...
    def UpdatedHandle(self, timeout=None, numAttempts=3, serverTimeout=None):
        ...
        _ignore = (
         timeout, numAttempts, serverTimeout)
        return self

    def WaitForUpdatedHandle(self, waitInSeconds=10, pollTime=3, *args, **kw):
        """Just return self since no need to update an invalid handle."""
        _ignore = (
         waitInSeconds, pollTime, args, kw)
        return self

    def Pretty(self):
        """Return pretty version of self."""
        return 'InvalidHandle:\n    ' + (',\n    ').join([ '.%s=%s' % (n, repr(getattr(self, n))) for n in [
         'name', 'infoDict']
                                                         ])
...
class StaticHandle(GenericProcessHandle):
    ...
    def __init__(self, name, infoDict=None):
        ...
        defaults = {'starttime': now, 
           'endtime': now + datetime.timedelta(seconds=1), 'alive': False, 
           'host': socket.gethostname(), 'port': 0, 'mode': 'finished', 
           'result': None, 'pids': []}
...
class TaskHandle(GenericProcessHandle):
    ...
    def __init__(self, name, started, finished, alive, host, port, result, starttime, endtime, user='unknown', taskRepr='', pids=None, tokens=None):
        GenericProcessHandle.__init__(self)
        if tokens is None:
            tokens = []
        self.name = name
        ...
        self._stale = False
        self._tokens = tokens
        assert isinstance(host, (str, unicode)), 'Expected str for host; got %s of %s' % (host, type(host))
        assert isinstance(port, int), 'Expected int for port; got %s of %s' % (port, type(port))
        return
    ...
    def UpdatedHandle(self, timeout=None, numAttempts=3, serverTimeout=None):
        ...
        for _attempt in range(numAttempts):
            try:
                try:
                    ...
                    logging.warning('Got protocol error %s while getting status' % str(exc))
                    raise
                except socket.error, exc:
                    logging.warning('Exception updating status with server %s:\n%s\n' % (
                     connString, str(exc)))
                    raise
        ...
        for name in self.__dict__.keys():
            if '_' != name[0]:
                setattr(self, name, property(self._Invalid, self._Invalid, doc='Object is stale; last valid value for %s was %s' % (
    ...
    def Kill(self):
        ...
        logging.debug('Connecting to %s' % connString)
        connection = PicklingXMLRPC.PicklingServerProxy(connString)
        result = connection.RemoveFromQueue(*(getattr(self, '_tokens', []) + [self]))
        return result
    ...
    def __str__(self):
        if self._stale:
            self._Invalid()
        result = self.__class__.__name__ + '(\n' + (',\n').join([ '    %s=%s' % (item, repr(getattr(self, item, 'unknown'))) for item in ['name', 'started', 'finished', 'alive', 'host', 'port', 'result', 'starttime', 'endtime', 'user', 'taskRepr', 'pids'] ]) + ')\n'
        return result

    def Cleanup(self):
        """Removes task represented by handle from the server
        """
        connection = PicklingXMLRPC.PicklingServerProxy('http://%s:%i' % (self.host, self.port))
        ...
        if not self.started:
            ...
            if self.alive:
                if self.finished:
                    ...
                    return 'running'
            raise self.finished or Exception('Weird status: not alive but finished')
        else:
            return 'finished'
...
class _ThreadRunner(threading.Thread):
    ...
    def __init__(self, token, target, connection, callbacks, *threadArgs, **threadKW):
        """Initializer.
...
class ThreadHandle(GenericProcessHandle):
    ...
    def __init__(self, token, target, connection, pipe, host, port, name='unnamed', extraInfo=None):
        """Initializer.
