...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\superpy\core\Forker.py
...
"""
import sys, logging, time, tempfile, Process
...
def MakeCommandLine(exe, scriptName, hostName, portNumber, logFile=None):
    ...
    cmdLine = [
     exe, scriptName, str(hostName), str(portNumber)]
    if logFile:
        cmdLine += [logFile]
    return cmdLine
...
def ProcessCommandLine(argv):
    ...
    if len(argv) == 4:
        return {'progName': argv[0], 'host': argv[1], 'port': int(argv[2]), 'logFile': argv[3]}
