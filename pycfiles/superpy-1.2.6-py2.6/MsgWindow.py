...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\superpy\SuperWatch\MsgWindow.py
# Compiled at: 2010-06-04 07:07:10
"""Module providing MsgWindow class.
"""
import Tkinter, Pmw, GUIUtils
...
class MsgWindow:
    ...
    def _MakeMsgFrame(rootWindow):
        ...
        sframe = getattr(Pmw, 'ScrolledFrame')(rootWindow, labelpos='n', horizflex='expand', vertflex='expand')
    ...
    def MakeMsg(self, title, msg):
        ...
        """
        frame = Tkinter.Frame(self.frame.interior(), relief='raised', bd=2, padx=1, pady=1)
        show = Tkinter.Button(frame, text=title, padx=2, pady=2, command=lambda : GUIUtils.MakeTextDialog(title='Details on %s' % title, text=msg))
        kill = Tkinter.Button(frame, text='Kill', padx=2, pady=2, command=lambda : self.KillWindowAndChildren(frame))
