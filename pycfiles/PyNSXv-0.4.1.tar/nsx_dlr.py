...
# [GCC 8.4.0]
# Embedded file name: /Users/yfauser/Documents/Development/vmware-org/pynsxv/pynsxv/library/nsx_dlr.py
...
def dlr_add_interface(client_session, dlr_id, interface_ls_id, interface_ip, interface_subnet):
    ...
    dlr_interface = client_session.create('interfaces', uri_parameters={'edgeId': dlr_id}, query_parameters_dict={'action': 'patch'}, request_body_dict=dlr_interface_dict)
    return dlr_interface


def _dlr_add_interface(client_session, vccontent, **kwargs):
    if not (kwargs['dlr_name'] and kwargs['interface_ls_name'] and kwargs['interface_ip'] and kwargs['interface_subnet']):
        print 'Mandatory parameters missing, [-n NAME] [--interface_ls INTERFACE_LS] [--interface_ip INTERFACE_IP] [--interface_subnet INTERFACE_SUBNET]'
        return
    else:
        ...
        if dlr_id:
            ...
            if not interface_ls_id:
                ...
                if not interface_ls_id:
                    print ('ERROR: DLR interface logical switch {} does NOT exist as VDS port group nor NSX logical switch').format(interface_ls_name)
                    return
...
def dlr_list_interfaces(client_session, dlr_id):
    ...
    return (
     dlr_int_list, dlr_int_list_verbose)
...
def dlr_create(client_session, dlr_name, dlr_pwd, dlr_size, datacentermoid, datastoremoid, resourcepoolid, ha_ls_id, uplink_ls_id, uplink_ip, uplink_subnet, uplink_dgw):
    """
    ...
    dlr_create_dict['edge']['cliSettings'] = {'password': dlr_pwd, 'remoteAccess': 'True', 'userName': 'admin'}
    ...
    dlr_create_dict['edge']['interfaces'] = {'interface': {'type': 'uplink', 'isConnected': 'True', 'connectedToId': uplink_ls_id, 
                     'addressGroups': {'addressGroup': {'primaryAddress': uplink_ip, 'subnetMask': uplink_subnet}}}}
...
def _dlr_create(client_session, vccontent, datacenter_name, edge_datastore, edge_cluster, **kwargs):
    if not (kwargs['dlr_name'] and kwargs['dlr_pwd'] and kwargs['dlr_size'] and datacenter_name and kwargs['ha_ls_name'] and kwargs['uplink_ls_name'] and kwargs['uplink_ip'] and kwargs['uplink_subnet'] and edge_datastore and edge_cluster):
        print 'You are missing a mandatory parameter, those are [-n NAME] [-p DLRPASSWORD] [-s DLRSIZE] [--ha_ls HA_LS] [--uplink_ls UPLINK_LS] [--uplink_ip UPLINK_IP] [--uplink_subnet UPLINK_SUBNET] and the ini file options datacenter_name,edge_datastore and edge_cluster'
        return
    else:
        dlr_name = kwargs['dlr_name']
        ...
        if not uplink_ls_id:
            ...
            if not uplink_ls_id:
                print ('ERROR: DLR uplink switch {} does NOT exist as VDS port group nor NSX logical switch').format(uplink_ls_name)
                return
        dlr_id, dlr_params = dlr_create(client_session, dlr_name, dlr_pwd, dlr_size, datacentermoid, datastoremoid, resourcepoolid, ha_ls_id, uplink_ls_id, uplink_ip, uplink_subnet, uplink_dgw)
        if kwargs['verbose']:
            print dlr_params
        else:
...
def dlr_set_dgw(client_session, dlr_id, uplink_dgw):
    ...
    dlr_static_route = client_session.update('routingConfig', uri_parameters={'edgeId': dlr_id}, request_body_dict=dlr_static_route_dict)
...
def dlr_list(client_session):
    ...
    return (
     dist_lr_list, dist_lr_list_verbose)
...
def contruct_parser(subparsers):
    parser = subparsers.add_parser('dlr', description="nsxv function for dlr '%(prog)s @params.conf'.", help='Functions for distributed logical routers', formatter_class=RawTextHelpFormatter)
    ...
    parser.add_argument('-n', '--name', help='dlr name')
    parser.add_argument('-p', '--dlrpassword', help='dlr admin password', default='VMware1!VMware1!')
    parser.add_argument('-s', '--dlrsize', help='dlr size (compact, large, quadlarge, xlarge)', default='compact')
    parser.add_argument('--ha_ls', help='dlr ha LS name')
    parser.add_argument('--uplink_ls', help='dlr uplink logical switch name')
    parser.add_argument('--uplink_ip', help='dlr uplink ip address')
    parser.add_argument('--uplink_subnet', help='dlr uplink subnet')
    parser.add_argument('--uplink_dgw', help='dlr uplink default gateway')
    parser.add_argument('--interface_ls', help='interface logical switch in dlr')
    parser.add_argument('--interface_ip', help='interface ip address in dlr')
    parser.add_argument('--interface_subnet', help='interface subnet in dlr')
...
def _dlr_main(args):
    ...
    try:
        command_selector = {'list': _dlr_list_print, 
           'create': _dlr_create, 
           'delete': _dlr_delete, 
           'read': _dlr_read, 
        ...
        command_selector[args.command](client_session, vccontent=vccontent, dlr_name=args.name, dlr_pwd=args.dlrpassword, dlr_size=args.dlrsize, datacenter_name=datacenter_name, edge_datastore=edge_datastore, edge_cluster=edge_cluster, ha_ls_name=args.ha_ls, uplink_ls_name=args.uplink_ls, uplink_ip=args.uplink_ip, uplink_subnet=args.uplink_subnet, uplink_dgw=args.uplink_dgw, interface_ls_name=args.interface_ls, interface_ip=args.interface_ip, interface_subnet=args.interface_subnet, verbose=args.verbose)
    except KeyError:
