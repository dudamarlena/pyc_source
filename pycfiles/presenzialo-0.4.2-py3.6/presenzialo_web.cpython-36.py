...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/presenzialo/presenzialo_web.py
...
class PRweb:
    ...
    def login(self, auth):
        ...
        cookies = r.cookies
        payload = {'shib_idp_ls_exception.shib_idp_session_ss':'', 
         ...
         '_eventId_proceed':''}
        r = s.post(('{}/idp/profile/SAML2/Redirect/SSO?execution=e1s1'.format(self.auth['idpurl'])),
          headers=headers,
          allow_redirects=True,
          cookies=cookies,
          data=payload)
        payload = {'j_username':auth['username'], 
         'j_password':auth._decode(auth['password']), 
         '_eventId_proceed':''}
        r = s.post(('{}/idp/profile/SAML2/Redirect/SSO?execution=e1s2'.format(self.auth['idpurl'])),
          headers=headers,
        ...
        if not a[0]:
            ...
            exit()
        payload = {'RelayState':a[0], 
         'SAMLResponse':a[1]}
        r = s.post(('{}/Shibboleth.sso/SAML2/POST'.format(self.url)),
          allow_redirects=False,
          cookies=(r.cookies),
          data=payload)
        ...
        headers = {'connection':'keep-alive', 
         ...
         'Accept-Language':'en-US,en;q=0.9'}
        r = s.get(('{}/LoginShib.aspx?PageMethod=EseguiLogin'.format(self.url)),
          allow_redirects=False,
          headers=headers,
          cookies=(s.cookies))
        cookies = r.cookies
        r = s.get(('{}/default.aspx'.format(self.url)),
          allow_redirects=True,
          cookies=(s.cookies),
          data=payload)
        r = s.get(('{}/rpc/Utente.aspx?PageMethod=GetCurrent'.format(self.url)),
          allow_redirects=True,
          cookies=cookies,
          data=payload)
    ...
    def timecard(self, day_from=datetime.date.today(), day_to=datetime.date.today()):
        url = '{}/rpc/Cartellino.aspx?PageMethod=DettaglioCartellino&iddip={}'.format(self.url, self.idworker)
        payload = {'_dtdatainizio':day_from.strftime('%Y%m%d%H%M%S'), 
    ...
    def workers_id(self):
        url = '{}/rpc/Rubrica.aspx?pageMethod=ElencoSottopostiSelectVisibilt%C3%A0Estesa&pattern=&page=1&pageLimit={}'.format(self.url, max_workers)
    ...
    def address_book(self, iddip=-1):
        ...
        url = '{}/rpc/Rubrica.aspx?PageMethod={}&iddip={}&data={}&_=1578658512666'.format(self.url, api_name, iddip, datetime.datetime.now().strftime('%Y%m%d%H%M%S'))
        r = self.session.post(url,
          allow_redirects=False,
          cookies=(self.cookies),
          headers=(self.headers))
