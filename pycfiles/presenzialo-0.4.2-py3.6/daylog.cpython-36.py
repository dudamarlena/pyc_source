...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/presenzialo/logs/daylog.py
...
class DayLog(dict):
    __doc__ = 'Class to management of login/logout accesses.\n   '
    ...
    def __str__(self):
        ...
        if not self['date'] or not self['logs']:
            ...
            s += '       ' + '{:.<20}[ {} ]'.format('Logs', ', '.join([i.time().strftime(dayutils.fmt_time) for i in self['logs']]))
            return s
    ...
    def _get_uptime_from(self, start, final):
        ...
        if not isinstance(start, datetime.datetime):
            raise AssertionError('@_get_uptime_from start is not datetime.datetime: %s %s' % (
             type(start), start))
        elif not isinstance(final, datetime.datetime):
            raise AssertionError('@_get_uptime_from final is not datetime.datetime: %s %s' % (
             type(final), final))
