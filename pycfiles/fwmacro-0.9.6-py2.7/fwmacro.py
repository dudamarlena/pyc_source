...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fwmacro.py
# Compiled at: 2016-11-25 00:52:55
import netaddr, netifaces, socket, os, os.path, re, sys, subprocess, time
__version__ = (
 2, 9, 6)
...
class FWUndefinedError(FWMacroException):
    ...
    def __str__(self):
        ...
        return ('Undefined {} {} at line {}').format(entity, self.name, self.lineno)
...
class FWRedefinitionError(FWMacroException):
    ...
    def __str__(self):
        ...
        if self.reflineno:
            ...
            refline = ''
        return ('Redefinition of {}{} at line {}{}').format(entity, self.name, self.lineno, refline)
...
class FWRecursionError(FWMacroException):
    ...
    def __str__(self):
        ...
        return ('Recursion detected in group definition for group {}: {}').format(self.name, (', ').join(lines))
...
class FWIPMaskBoundaryError(FWMacroException):
    ...
    def __str__(self):
        return ('IP is not on mask boundary ({}) at line {}').format(self.ip, self.lineno)
...
class FWInvalidParentChain(FWMacroException):
    ...
    def __str__(self):
        return ('Invalid parent chain {} in table {}').format(self.chain, self.table)
...
class FWReservedChainName(FWMacroException):
    ...
    def __str__(self):
        return ('Reserved chain name {} at {}:{}').format(self.chain, self.fname, self.lineno)
...
class FWOrderConflict(FWMacroException):
    ...
    def __str__(self):
        return ('Order redefinition of chain {} at {}:{} (first defined at {}:{})').format(self.chain, self.fname, self.lineno, self.origdef[1], self.origdef[2])
...
class FWInvalidIP(FWMacroException):
    ...
    def __str__(self):
        return ('Invalid IP address {} at line {}').format(self.address, self.lineno)
...
class FWInvalidPort(FWMacroException):
    ...
    def __str__(self):
        return ('Invalid port {} at line {}').format(self.port, self.lineno)
    ...
    def __str__(self):
        if not self.expected:
            ...
            if isinstance(self.expected, str):
                ...
                expected = ('one of {}').format((', ').join([ i for i in self.expected if i is not None ]))
            return ('Expected {} but got {} at line {}').format(expected, self.text, self.lineno)
...
class FWGroupNameRequired(FWMacroException):
    ...
    def __str__(self):
        return ('Group names are required. Got {} at line {}').format(self.text, self.lineno)
...
class Chain(object):
    ...
    def __str__(self):
        return ('line: {}, name: {}, condition: {}').format(self.lineno, self.local, self.direction, self.action)
...
class Rule(object):
    ...
    def __str__(self):
        return ('line: {}, local: {}, direction: {}, action: {}').format(self.lineno, self.local, self.direction, self.action)
    ...
    def chainname(self, chainnr, chainname, iface):
        ...
        if len(chainname) >= 30:
            chainname = ('{}{}:{}{}').format(iface, direction_char, chainnr, chainname)
...
class Token(object):
    ...
    def __repr__(self):
        return ('Token({}, {}, {})').format(self.lineno, self.indent, self.text)
            ...
            for obj in self:
                ...
                if isinstance(obj, Group):
                    try:
                        ...
                        raise FWRecursionError(self.lineno, self.name, e.groups + [self])
                    ...
                    for ip in ipv4:
                        if ip not in self.ipv4:
                            if isinstance(ip, netaddr.IPNetwork) and ip.network != ip.ip:
...
class FWMacro(object):
    ...
    def __init__(self):
        self.n_errors = 0
        self.n_warnings = 0
        self.interfaces = list([ i for i in netifaces.interfaces() if netifaces.AF_LINK in netifaces.ifaddresses(i) and (netifaces.AF_INET in netifaces.ifaddresses(i) or netifaces.AF_INET6 in netifaces.ifaddresses(i))
                               ])
    ...
    def exec_cmd(self, cmd):
        ...
        if stderrdata:
            self.warning(('Error while executing {}: {}').format((' ').join(cmd), stderrdata))
            stdoutdata = None
        return stdoutdata
    ...
    def get_icmp4options(self):
        ...
        cmd = [
         iptables_cmd, '-p', 'icmp', '-h']
        self.icmp4_options = self.parse_icmp_options(cmd, self.default_icmp4options)
    ...
    def get_icmp6options(self):
        ...
        cmd = [
         ip6tables_cmd, '-p', 'icmpv6', '-h']
        self.icmp6_options = self.parse_icmp_options(cmd, self.default_icmp6options)
    ...
    def get_services(self):
        ...
        with open('/etc/services', 'r') as (fp):
            for line in fp:
                ...
                if words[0] not in services:
                    services[words[0]] = number
                for word in words[2:]:
                    if word not in services:
                        services[word] = number

        sorted_services = list([ i[1] for i in sorted([ (len(key), key) for key in services.keys() ]) ])
...
class FWPreprocess(FWMacro):
    ...
    re_tokens = re.compile('((?P<q>[\'"]).*?(?P=q))|([A-Fa-f0-9]*:[A-Fa-f0-9]*:[A-Fa-f0-9:]*)|(\\d+[.]\\d+[.]\\d+[.]\\d+)|(\\d+(?!\\w))|(\\w[\\w.-]*)|(\\W)')
    ...
    def get_token(self, expect_text=False, expect_indent=None, expect_lineno=None):
        token = self._get_token(expect_text, expect_indent, expect_lineno)
        return token

    def _get_token(self, expect_text=False, expect_indent=None, expect_lineno=None):
        if self.token_stack:
        ...
        if not line:
            if expect_text is False or isinstance(expect_text, list) and None in expect_text:
                return
    ...
    def get_ports(self, token):
        ...
        if len(ports) > 1 and 'all' in ports:
            raise FWSyntaxError(lineno, "Cannot use 'all' with other ports")
        self.push_token(token)
        return ports
    ...
    def get_name_ip_net(self, token, names_only=False):
        try:
            ...
            if token.text != 'any' and token.text in self.reserved_words:
                raise FWSyntaxError(token.lineno, ('Reserved word {}').format(token.text))
            return token.text

        next_token = self.get_token()
        if not next_token or next_token.lineno != token.lineno or next_token.text != '/':
            self.push_token(next_token)
        else:
            mask_token = self.get_token(True, token.indent, token.lineno)
            try:
                ip = netaddr.IPNetwork(('{}/{}').format(token.text, mask_token.text))
            except:
    ...
    def get_name_ip_net_list(self, start_token, names_only=False):
        ...
        while token and token.text == ',' and token.lineno == lineno:
            entries.append(self.get_name_ip_net(self.get_token(True, indent, lineno), names_only))
            token = self.get_token()
    ...
    def parse(self, fp):
        ...
        unreferenced_ifaces = list([ i for i in self.interfaces if netifaces.AF_INET in netifaces.ifaddresses(i) or netifaces.AF_INET6 in netifaces.ifaddresses(i)
                                   ])
        ...
        if unreferenced_ifaces:
            self.warning(('Unreferenced interfaces: {}').format((', ').join(unreferenced_ifaces)))
        return
    ...
    def handle_ruleset_def(self, start_token):
        ...
        if name in self.rulesets:
            raise FWRedefinitionError(token.lineno, name, 'ruleset')
        self.get_token(':', token.indent, token.lineno)
    ...
    def handle_interface(self, start_token):
        ...
        if name in self.ifaces:
            raise FWRedefinitionError(token.lineno, name, 'interface')
    ...
    def handle_rule(self, start_token):
        ...
        def get_nat_ip(token):
            ...
            if text in self.groups:
                if len(self.groups[text]) != 1:
                    raise FWSyntaxError(token.lineno, 'NAT ip group can only have 1 item')
                else:
        ...
        if token.text == 'ruleset':
            ...
            if token.text not in self.rulesets:
                raise FWUndefinedError(lineno, token.text, 'ruleset')
            ...
            if token.text == 'local':
                ...
                if rule.action in ('snat', 'dnat'):
                    ...
                    if token.text != '-':
                        ...
                        token = self.get_token(True, indent, lineno)
                    rule.natports = ('-').join([ str(i) for i in self.get_port_range(token) ])
                    if rule.natports:
                        token = self.get_token(True, indent, lineno)
                states = []
                while token.text in ('NONE', 'NEW', 'ESTABLISHED', 'RELATED', 'INVALID'):
                    if token.lineno != lineno:
                        self.push_token(token)
                ...
                try:
                    ...
                    if token.text in ('ip', 'all', 'tcp', 'udp', 'icmp') or token.text in self.protocols:
                        proto = token.text
                    else:
            ...
            if proto in ('ip', 'all', 0):
                if invert:
                    raise FWSyntaxError(('Cannot invert protocol {}').format(proto))
                proto = 'all'
            ...
            if rule.action in ('dnat', ):
                if proto in ('tcp', 'udp'):
                    if not rule.natports:
                        raise FWSyntaxError('Specific ports needed in nat definition (when using tcp or udp match condition)')
                elif rule.natports:
                    raise FWSyntaxError('Ports not used in nat definition (use tcp or udp match condition)')
            if proto == 'icmp':
                icmp4 = []
                icmp6 = []
                while True:
                ...
                if token.text == 'syn':
                    ...
                    token = self.get_token([
                     '!', 'ALL', 'SYN', 'ACK', 'FIN', 'RST', 'URG', 'PSH'], indent, lineno)
                    invert = False
                    if token.text == '!':
                        invert = True
                        token = self.get_token([
            ...
            if len(addrs) > 1 and 'any' in addrs:
                raise FWSyntaxError(lineno, "Cannot mix 'any' with other source addresses")
            rule.sources = addrs
            token = self.get_token(True, indent, lineno)
            if proto in ('tcp', 'udp'):
            ...
            if len(addrs) > 1 and 'any' in addrs:
                raise FWSyntaxError(lineno, "Cannot mix 'any' with other destination addresses")
            ...
            if token and token.lineno == lineno and token.text == 'log':
                ...
                if token and token.lineno == lineno:
                    if token.text in self.logging_levels:
                        ...
                        token = self.get_token()
                    if token.lineno == lineno and token.text not in self.logging_levels:
    ...
    def resolve_ip(self, targets, rule):
        ...
        for name in targets:
            ...
            try:
                ...
                if ip.network != ip.ip:
                    self.error(FWIPMaskBoundaryError(ip, rule.lineno))
                ips = [
                 ip]
            except netaddr.core.AddrFormatError as e:
                if name in self.groups and self.groups[name].lineno is not None:
        ...
        for prefixlen, ip, invert in all_ip:
            if last_ip is not None:
                if last_ip == ip:
                    if last_invert != invert:
                        self.error('Conflicting definitions ({}, !{},{})at line {}').format(ip, invert, last_invert, rule.lineno)
                    continue
            last_ip = ip
            last_invert = invert
            for target_ip, target_invert in all_targets:
                if target_ip.size != 1 and (target_ip[0] >= ip[0] and target_ip[0] <= ip[(-1)] or target_ip[(-1)] >= ip[0] and target_ip[1] <= ip[(-1)] or ip[0] >= target_ip[0] and ip[0] <= target_ip[(-1)] or ip[(-1)] >= target_ip[0] and ip[(-1)] <= target_ip[(-1)]):
                    self.warning(('Overlapping ranges ({}, {}) at line {}').format(ip, target_ip, rule.lineno))
            ...
            if ip.version == 4:
                ...
                self.error(('Invalid ip version for {} at line {}').format(ip, rule.lineno))
    ...
    def resolve_ports(self, ports, rule):
        ...
        while len(all_raw):
            ...
            while len(all_raw):
                ...
                if p1b <= p2a + 1:
                    if pia != pib:
                        self.error(('Conflicting port definition at line {}').format(rule.lineno))
                        break
                    if p2a < p2b:
                        p2a = p2b
                    del all_raw[0]
        ...
        for p1, p2, invert in all:
            if invert and len(all) > 1:
                self.error(("Cannot use '!' on multiple port definitions at line {}").format(rule.lineno))
                return ([], [])
            if p1 == p2:
                if not invert:
                    if len(all) == 1:
    ...
    def resolve(self):
        ...
        for name, group in self.groups.items():
            if group.lineno is None and not isinstance(group, Hostname):
    ...
    def make_rule(self, chainnr, chainname, iface, rule):
        if not rule:
            if self.n_errors == 0:
                self.error(('Invalid rule in interface {}: {}').format(iface, chainname))
            return ('', '')
        ...
        if not self.allow_mixed_ipv and (srcs_ip4 and not dsts_ip4 or dsts_ip4 and not srcs_ip4 or srcs_ip6 and not dsts_ip6 or dsts_ip6 and not srcs_ip6):
            self.error(('Cannot mix IPv4 and IPv6 source and destination at line {}').format(rule.lineno))
        lines_ip4 = []
        lines_ip6 = []
        line_ipv4 = []
        line_ipv6 = []
        ...
        if rule.nat or rule.action == 'masq':
            ...
            if not srcs_ip4 or not dsts_ip4:
                self.error(('NAT rule only valid for IPv4 at line {}').format(rule.lineno))
            else:
                all = netaddr.IPNetwork('::/0')
                for src in srcs_ip6:
                    if src[1] != all:
                        self.warning(('Ignoring {} rule for IPv6 source address {} at line {}').format(rule.action, src, rule.lineno))

                for dst in dsts_ip6:
                    if dst[1] != all:
                        self.warning(('Ignoring {} rule for IPv6 destination address {} at line {}').format(rule.action, dst, rule.lineno))
        ...
        if rule.logging:
            ...
            if len(s) > 27:
                s = s[:20] + '...' + s[-5:]
            targets.append(('LOG --log-prefix "{} " --log-level {} -m limit --limit 60/minute --limit-burst 10').format(s, rule.logging))
        if rule.direction == 'in':
            ...
            if rule.direction == 'out':
                ...
                self.error(('Invalid direction {}').format(rule.direction, rule.lineno))
            ...
            if rule.nat:
                ...
                if rule.direction == 'in' and not rule.local:
            ...
            for src_invert, src_ip in sources:
                ...
                if src_ip.prefixlen == 0:
                    ...
                    src = ('--src {}{}/{}').format(src_invert, src_ip.ip, src_ip.prefixlen)
                for dst_invert, dst_ip in destinations:
                    ...
                    if dst_ip.prefixlen == 0:
                        ...
                        dst = ('--dst {}{}/{}').format(dst_invert, dst_ip.ip, dst_ip.prefixlen)
        ...
        for iface in ifaces_keys:
            ...
            for rule in self.ifaces[iface]:
                ...
                if not rule_ip4 and not rule_ip6:
                    self.warning(('Nothing to do for {} rule for IPv4 and IPv6 at line {}').format(rule.action, rule.lineno))
                lines_ip4 += rule_ip4
                lines_ip6 += rule_ip6
    ...
    def write_rules(self, chains4, chains6):
        if self.n_errors != 0:
            return
        for chainsdir, chains in [(self.chainsdir_ipv4, chains4),
         (
          ...
          self.chainsdir_ipv6, chains6)]:
            ...
            for chainname in chains_keys:
                ...
                try:
                    ...
                    self.error(('Failed to write to file {}: {}').format(fname, why))
...
class FWCompile(FWMacro):
    ...
    def parentchains(self, table, chain):
        ...
        if short == 'IN':
            ...
            parentchains = [
             'POSTROUTING']
        if len(parentchains) == 1 and parentchains[0] not in self.builtin_chains[table]:
                ...
                for line in lines:
                    ...
                    if m:
                        ...
                        if m.group(2):
                            ...
                            newchain = m.group(3)
                        line = line.replace(('-A {}').format(chain), ('-A {}').format(newchain))
                        chain = newchain
                    ...
                    if chain not in chainorder[table]:
                        ...
                        raise FWOrderConflict(chain, fname, linenr, chainorder[table][chain])
    ...
    def generate_restore_file(self, rule_file):
            ...
            for chain in self.newchains[table]:
                ...
                for pchain in parentchains:
                    ...
                    if m.group(2):
                        ...
                        rules[table].append(('-A {} -{} {} -j {}').format(pchain, direction, iface, chain))
                    else:
                        rules[table].append(('-A {} -j {}').format(pchain, chain))
                ...
                for fname in self.filechains.keys():
                    if chain in self.filechains[fname][table]:
                        for line in self.filechains[fname][table][chain]:
                            ...
                            if match:
                                line = ('{} {}').format(match.group(1).strip(), match.group(3).strip())
        ...
        if rule_file == '-' or not rule_file:
            ...
            if hasattr(rule_file, 'seek'):
                ...
                fp = open(rule_file, 'w')
            fp.write(('# Generated with {} {} at {}\n').format(self.__class__.__name__, ('.').join([ str(i) for i in __version__ ]), time.ctime()))
            for table in self.tables:
                fp.write(('*{}\n').format(table))
...
def fwmpp():
    ...
    parser.add_option('--base', dest='basedir', default='/etc/fwmacro', metavar='DIRECTORY', help=("Set the base path (default: '{}')").format(FWMacro.basedir))
    parser.add_option('--ipv4-chains', action='store', dest='chainsdir_ipv4', default=FWPreprocess.chainsdir_ipv4, metavar='DIRECTORY', help=('directory with iptables chains (default: {})').format(FWPreprocess.chainsdir_ipv4))
    parser.add_option('--ipv6-chains', action='store', dest='chainsdir_ipv6', default=FWPreprocess.chainsdir_ipv6, metavar='DIRECTORY', help=('directory with ip6tables chains (default: {})').format(FWPreprocess.chainsdir_ipv6))
    parser.add_option('--logtag', action='store', dest='logtag', default=FWPreprocess.logtag, help="log tag template (default: '%s')" % FWPreprocess.logtag)
    parser.add_option('--force-groups', action='store_true', dest='force_groups', default=False, help="Force the use of groups (default: '%s')" % False)
...
def fwmc():
    import optparse
    parser = optparse.OptionParser(usage='usage: %prog [options] start | stop\n')
    parser.add_option('-V', '--version', action='store_true', dest='version', default=False, help='show version and exit')
    parser.add_option('--verbose', action='store_true', dest='verbose', default=False, help='verbose messages')
    parser.add_option('--base', dest='basedir', default='/etc/fwmacro', metavar='DIRECTORY', help=("Set the base path (default: '{}')").format(FWMacro.basedir))
    parser.add_option('--ipv4-rules', action='store', dest='ipv4_rules', default=FWCompile.chainsfile_ipv4, help='filename of generated iptables-restore file')
    parser.add_option('--ipv6-rules', action='store', dest='ipv6_rules', default=FWCompile.chainsfile_ipv6, help='filename of generated ip6tables-restore file')
    parser.add_option('--no-ipv4', action='store_false', dest='ipv4', default=True, help='no iptables chains (ipv4)')
    parser.add_option('--no-ipv6', action='store_false', dest='ipv6', default=True, help='no ip6tables chains (ipv6)')
    parser.add_option('--ipv4-chains', action='store', dest='chainsdir_ipv4', default=FWCompile.chainsdir_ipv4, metavar='DIRECTORY', help=('directory with iptables chains (default: {})').format(FWCompile.chainsdir_ipv4))
    parser.add_option('--ipv6-chains', action='store', dest='chainsdir_ipv6', default=FWCompile.chainsdir_ipv6, metavar='DIRECTORY', help=('directory with ip6tables chains (default: {})').format(FWCompile.chainsdir_ipv6))
    parser.add_option('--reserved-target', action='append', dest='reserved_targets', default=FWCompile.reserved_targets, help='reserved target (e.g. ACCEPT) that will not be mapped to a chain')

    def no_reserved_target(option, opt_str, value, parser, *args, **kwargs):
        FWCompile.reserved_targets.remove(value)

    parser.add_option('--no-reserved-target', type='string', action='callback', callback=no_reserved_target, help='not a reserved target (remove from reserved targets list)')
    parser.add_option('--help-reserved-target', action='store_true', dest='help_reserved_target', default=False, help='show help on reserved targets')
    ...
    if args[0] in ('stop', ):
        remove_all_chains = 1
    if opts.ipv4:
        fc = FWCompileIPv4(remove_all_chains=remove_all_chains, verbose=opts.verbose)
    ...
    if opts.ipv6:
        fc = FWCompileIPv6(remove_all_chains=remove_all_chains, verbose=opts.verbose)
        fc.basedir = os.path.abspath(opts.basedir)
