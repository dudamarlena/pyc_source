...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/zif/sedna/pool.py
...
import logging, Queue, cPickle as pickle
try:
...
class _ConnectionRecord(object):
    ...
    def close(self):
        if self.connection is not None:
            ...
            try:
                ...
                if self.__pool._should_log_info:
                    self.__pool.log('Exception closing connection %r' % self.connection)
    ...
    def get_connection(self):
        if self.connection is None:
            ...
            if self.__pool._should_log_info:
                self.__pool.log('Connection %r exceeded timeout; recycling' % self.connection)
            self.__close()
            self.connection = self.__connect()
            self.info.clear()
            if self.__pool._on_connect:
    ...
    properties = property(lambda self: self.info, doc='A synonym for .info, will be removed in 0.5.')
...
class _ConnectionFairy(object):
    ...
    def __init__(self, pool):
        ...
        if self._pool._should_log_info:
            self._pool.log('Connection %r checked out from pool' % self.connection)
        return
    ...
    def checkout(self):
        ...
        while attempts > 0:
            try:
                ...
                if self._pool._should_log_info:
                    self._pool.log('Disconnection detected on checkout: %s' % e)
    ...
    def detach(self):
        ...
        if self._connection_record is not None:
            ...
            self._detached_info = self._connection_record.info.copy()
            self._connection_record = None
        return
