...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/zif/sedna/sednaobject.py
...
class SednaXQuery(object):
    ...
    def index0(self, obj):
        ...
        q = 'declare namespace se = "http://www.modis.ispras.ru/sedna";'
    ...
    def __getslice__(self, start, stop):
        ...
        q = 'declare namespace se = "http://www.modis.ispras.ru/sedna";'
        q += 'declare option se:output "indent=no";'
        q += 'for $i in subsequence(%s,%s,%s) ' % (self.path, rstart, rlen)
        q += 'return $i'
...
class SednaContainer(SednaXQuery):
    ...
    def _checkElement(self):
        ...
        if c == 1:
            ...
            raise LookupError('The path did not return an element. ([0] might need to be [1]?)')
        else:
            raise ValueError('Cannot init SednaContainer with multiple elements.')
    ...
    def append(self, obj):
        item = checkobj(obj)
        if self.count() > 0:
            q = 'update insert %s following %s/*[last()]' % (item, self.path)
        else:
            q = 'update insert %s into %s' % (item, self.path)
    ...
    def insert(self, key, item):
        ...
        item = checkobj(item)
        if count > 0:
            q = 'update insert %s preceding %s/*[%s]' % (item, self.path, local)
            self.cursor.execute(q)
        else:
            self.append(item)
        return
    ...
    def remove(self, obj):
        ...
        q = 'update delete %s/*[%s]' % (self.path, index)
        self.cursor.execute(q)
        self._count = None
        return
    ...
    def __delitem__(self, key):
        ...
        q = 'update delete %s/*[%s]' % (self.path, local)
        self.cursor.execute(q)
        self._count = None
        return
    ...
    @property
    def tag(self):
        q = 'let $i := %s return <t>{$i/name()}</t>' % self.path
    ...
    def __setitem__(self, key, value):
        item = checkobj(value)
        local = self._localKey(key)
        q = 'update replace $i in %s/*[%s] ' % (self.path, local)
        q += ' with %s' % item
        s = self.cursor.execute(q)
    ...
    def replace(self, obj):
        ...
        item = checkobj(obj)
        q = 'update replace $i in %s ' % (self.path,)
        q += ' with %s' % (item,)
        self.cursor.execute(q)
        self._attrib = None
        self._count = None
...
class SednaObjectifiedElement(object):
    ...
    def _checkElement(self):
        ...
        if c == 1:
            ...
            raise ValueError('Cannot init SednaObjectifiedElement with multiple elements.')

    def replace(self, obj):
        """ replace item at self._path with the object"""
        item = checkobj(obj)
        q = 'update replace $i in %s ' % (self._path,)
        q += ' with %s' % (item,)
    ...
    def __setattr__(self, x, value):
        if x in ('_cursor', '_path', '_element'):
            try:
                ...
                raise ValueError('Oops. _cursor, _path, and _element are used internally.')
            else:
