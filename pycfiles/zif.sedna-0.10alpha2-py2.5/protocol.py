...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/zif/sedna/protocol.py
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                logger.error('zif.sedna protocol wants an elementtree implementation for some functions.')
...
def escapeAndQuote(aDict):
    ...
    for item in aDict:
        try:
            ...
            raise ProgrammingError('expected a parameters dict. Use pyformat %(var)s for constants.')
...
class SednaProtocol(object):
    ...
    def endTransaction(self, how):
        ...
        if how == 'commit':
            ...
            raise ProgrammingError("Expected 'commit' or 'rollback', got '%s'" % how)
    ...
    def __init__(self, host='localhost', db='test', login='SYSTEM', passwd='MANAGER', port=5050, trace=False):
        self.host = host
        self.port = port
        self.username = login
        ...
        self.database = db
        self.ermsgs = []
        self.handlers = {SEDNA_SEND_SESSION_PARAMETERS: self._sendSessionParameters, 
           ...
           SEDNA_TRANSACTION_ROLLBACK_BEFORE_CLOSE: self._transactionRollbackBeforeClose, 
    ...
    def openSocket(self, host, port):
        ...
        try:
            ...
            if self.socket:
                self.socket.close()
            raise InterfaceError('Server connection failed. Is Sedna server running? %s' % e)
    ...
    def _send_string(self, data='', token=0, format=0, respond=True):
        ...
        if token in (SEDNA_EXECUTE, SEDNA_EXECUTE_LONG):
            ...
            if datalen + self.prefixLength > self.maxQueryLength:
                for split in splitString(data, LOAD_BUFFER_SIZE):
                    self._send_string(split, token=SEDNA_EXECUTE_LONG, format=format, respond=False)
        ...
        self._sendSocketData(pack(self.headerFormat, int(token), len(data)) + data)
        if self.doTrace:
            logger = logging.getLogger()
            if token in (SEDNA_EXECUTE, SEDNA_EXECUTE_LONG):
                trace = data[6:]
    ...
    def _getSocketData(self, length):
        ...
        data, self.receiveBuffer = self.receiveBuffer[:length], self.receiveBuffer[length:]
        return data

    def _sendSessionParameters(self, msg):
        token = SEDNA_SESSION_PARAMETERS
        msg = pack('!bb', SEDNA_VERSION_MAJOR, SEDNA_VERSION_MINOR) + zString(self.username.encode('utf-8')) + zString(self.database.encode('utf-8'))
