...
# [GCC 8.4.0]
# Embedded file name: /Users/jace/Dropbox/projects/hasgeek/baseframe/baseframe/forms/validators.py
# Compiled at: 2017-03-08 12:14:00
import re
...
from wtforms.validators import DataRequired, InputRequired, Optional, Length, EqualTo, URL, NumberRange, ValidationError, StopValidation
import requests
...
__all__ = [
 'OptionalIf', 'OptionalIfNot', 'ValidEmail', 'ValidEmailDomain', 'ValidUrl', 'AllUrlsValid',
...
EMAIL_RE = re.compile('\\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\\.[A-Z]{2,63}\\b', re.I)
...
class _Comparison(object):
    ...
    def __call__(self, form, field):
        ...
        if not self.compare(field.data, other.data):
            d = {'other_label': hasattr(other, 'label') and other.label.text or self.fieldname, 'other_name': self.fieldname}
...
class ValidUrl(object):
    ...
    def __init__(self, message=None, message_urltext=None, invalid_urls=[]):
        self.message = message or _('The URL “{url}” is not valid or is currently inaccessible')
        self.invalid_urls = invalid_urls
        self.message_urltext = message_urltext or _('The URL “{url}” linked from “{text}” is not valid or is currently inaccessible')
    ...
    def check_url(self, invalid_urls, url, text=None):
        ...
        if rurl is not None and code in (200, 201, 202, 203, 204, 205, 206, 207, 208,
                                         226, 999):
...
class NoObfuscatedEmail(object):
    ...
    def __init__(self, message=None):
        if not message:
            message = __('Email address identified')
        self.message = message
...
class ValidName(object):
    ...
    def __init__(self, message=None):
        if not message:
            message = __('This name contains unsupported characters. It should have letters, numbers and non-terminal hyphens only')
        self.message = message
...
class ValidCoordinates(object):
    ...
    def __init__(self, message=None, message_latitude=None, message_longitude=None):
        self.message = message or __('Valid latitude and longitude expected')
        self.message_latitude = message_latitude or __('Latitude must be within ± 90 degrees')
        self.message_longitude = message_longitude or __('Longitude must be within ± 180 degrees')
