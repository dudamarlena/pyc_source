...
# [GCC 8.4.0]
# Embedded file name: /Users/jace/Dropbox/projects/hasgeek/baseframe/baseframe/forms/form.py
# Compiled at: 2017-03-08 11:44:48
...
__all__ = [
 'field_registry', 'widget_registry', 'validator_registry', 'Form', 'FormGenerator']
field_registry = {'SelectField': bparsleyjs.SelectField, 
...
widget_registry = {}
validator_registry = {'Length': (
            wtforms.validators.Length, 'min', 'max', 'message'), 
   'NumberRange': (
                 wtforms.validators.NumberRange, 'min', 'max', 'message'), 
   'Optional': (
              wtforms.validators.Optional, 'strip_whitespace'), 
   'Required': (
              wtforms.validators.DataRequired, 'message'), 
   'AnyOf': (
           wtforms.validators.AnyOf, 'values', 'message'), 
   'NoneOf': (
            wtforms.validators.NoneOf, 'values', 'message'), 
   'ValidEmail': bvalidators.ValidEmail, 
   'ValidUrl': bvalidators.ValidUrl, 
   'AllUrlsValid': bvalidators.AllUrlsValid}
filter_registry = {'lower': (
           bfilters.lower,), 
   'upper': (
           bfilters.upper,), 
   'strip': (
           bfilters.strip, 'chars'), 
   'lstrip': (
            bfilters.lstrip, 'chars'), 
   'rstrip': (
            bfilters.rstrip, 'chars'), 
...
class Form(BaseForm):
    ...
    def errors_with_data(self):
        return {name:{'data': f.data, 'errors': [ unicode(e) if is_lazy_string(e) else e for e in f.errors ]} for name, f in iteritems(self._fields) if f.errors if f.errors}
