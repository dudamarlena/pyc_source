...
# [GCC 8.4.0]
# Embedded file name: /Users/jace/Dropbox/projects/hasgeek/baseframe/baseframe/forms/widgets.py
...
from .. import b_ as _
__all__ = [
 'TinyMce3', 'TinyMce4', 'SubmitInput', 'DateTimeInput', 'HiddenInput', 'CoordinatesInput',
...
class DateTimeInput(wtforms.widgets.Input):
    ...
    def __call__(self, field, **kwargs):
        ...
        return Markup('<input type="date" class="datetime-date %s" %s /> <input type="text" class="datetime-time %s" %s /> %s' % (
         class_,
...
class CoordinatesInput(wtforms.widgets.core.Input):
    ...
    def __call__(self, field, **kwargs):
        ...
        return Markup('<input %s> <input %s>' % (
         self.html_params(id=(id_ + '_latitude'), name=field.name, placeholder=_('Latitude'), value=value[0], **kwargs),
         self.html_params(id=(id_ + '_longitude'), name=field.name, placeholder=_('Longitude'), value=value[1], **kwargs)))
