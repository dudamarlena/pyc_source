...
# [GCC 8.4.0]
# Embedded file name: /Users/jace/Dropbox/projects/hasgeek/baseframe/baseframe/views.py
...
def asset_key(assets):
    return make_name(('-').join(assets).replace('==', '-eq-').replace('>=', '-gte-').replace('<=', '-lte-').replace('>', '-gt-').replace('<', '-lt-'), maxlength=250)
...
def gen_assets_url(assets):
    ...
    if not os.path.exists(gendir):
        os.mkdir(gendir)
    if is_js:
        bundle = Bundle(assets_repo.require(*(['!jquery.js'] + assets)), output='gen/' + output_name + '.js', filters='uglipyjs')
    elif is_css:
...
def ext_assets(assets):
    ...
    if current_app.config.get('ASSET_SERVER'):
        try:
            r = requests.get(urljoin(current_app.config['ASSET_SERVER'], 'asset'), params={'a': assets}, allow_redirects=False)
...
def baseframe_context():
    return {'networkbar_links': networkbar_links, 
...
def favicon(subdomain=None):
    ...
    return send_from_directory(app_icon_path, 'favicon.ico', mimetype='image/vnd.microsoft.icon')
...
def humans(subdomain=None):
    return send_from_directory(current_app.static_folder if os.path.exists(os.path.join(current_app.static_folder, 'humans.txt')) else baseframe.static_folder, 'humans.txt', mimetype='text/plain')
...
def robots(subdomain=None):
    return send_from_directory(current_app.static_folder if os.path.exists(os.path.join(current_app.static_folder, 'robots.txt')) else baseframe.static_folder, 'robots.txt', mimetype='text/plain')
...
def editorcss(subdomain=None):
    response = current_app.response_class(render_template('editor.css'), mimetype='text/css', headers={'Expires': (datetime.utcnow() + timedelta(minutes=60)).strftime('%a, %d %b %Y %H:%M:%S GMT')})
    return response
