...
class TestClient:
    ...
    def test_upload_file(self):
        fw_client = client.Client('dummy-api-key',
          api_url='http://api.testing.findwatt.com')
        fw_client.upload_file('dummy_file.txt', 'dummy-catalog')
        last_invocation = client.Uploads.upload_file_calls[(-1)]
        if not last_invocation.get('file_path'):
            raise AssertionError
        elif not last_invocation.get('catalog_id'):
            raise AssertionError
    ...
    def test_upload_fileobj(self):
        ...
        fw_client.upload_fileobj(fileobj,
          'dummy-file.txt', catalog_name='Dummy Catalog')
        last_invocation = client.Uploads.upload_fileobj_calls[(-1)]
        if not last_invocation.get('fileobj'):
            raise AssertionError
        elif not last_invocation.get('catalog_name'):
            raise AssertionError