...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/findwatt/uploads.py
...
class Uploads:
    ...
    def upload_fileobj(self, fileobj: io.BytesIO, file_name: str, catalog_id: Optional[str]=None, catalog_name: Optional[str]=None) -> Optional[Upload]:
        if not catalog_id:
            if not catalog_name:
                msg = 'Must provide either catalog_id or catalog_name'
                raise MissingInformation(msg)
        file_size = fileobj.getbuffer().nbytes
        upload = self.create_upload(file_name,
          file_size, catalog_id=catalog_id, catalog_name=catalog_name)
        while 1:
    ...
    def create_upload(self, file_name: str, file_size: int, catalog_id: Optional[str]=None, catalog_name: Optional[str]=None) -> Optional[Upload]:
    ...
    def upload_chunk(self, chunk: bytes, content_range: str, upload_id: str) -> Tuple[(Upload, bool)]:
