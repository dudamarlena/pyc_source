...
# Embedded file name: build/bdist.macosx-10.3-i386/egg/appy/pod/pod_parser.py
...
class OdInsert:
    ...
    def __init__(self, odtChunk, elem, nsUris={}):
        self.odtChunk = odtChunk.decode('utf-8')
        self.elem = elem
        self.nsUris = nsUris
    ...
    def resolve(self, namespaces):
        ...
        for (nsName, nsUri) in self.nsUris.iteritems():
            self.odtChunk = re.sub('@%s@' % nsName, namespaces[nsUri], self.odtChunk)
...
class PodEnvironment(OdfEnvironment):
    """Contains all elements representing the current parser state during
       parsing."""
    __module__ = __name__
    ignorableElements = None
    impactableElements = None
    ADD_IN_BUFFER = 0
    ADD_IN_SUBBUFFER = 1
    IGNORING = 0
    READING_CONTENT = 1
    ...
    def onStartElement(self):
        ...
        if elem == Table.OD.elem:
            ...
            if attrs.has_key('%s:number-columns-repeated' % tableNs):
                self.getTable().nbOfColumns += int(attrs[('%s:number-columns-repeated' % tableNs)])
            else:
    ...
    def propagateNamespaces(self):
        ...
        self.ignorableElements = (
         '%s:tracked-changes' % ns[self.NS_TEXT], '%s:change' % ns[self.NS_TEXT])
        self.impactableElements = (
         Text.OD.elem, Title.OD.elem, Table.OD.elem, Row.OD.elem, Cell.OD.elem, Section.OD.elem)
...
class PodParser(OdfParser):
    ...
    def startElement(self, elem, attrs):
        ...
        if elem in e.ignorableElements:
            ...
            if elem == '%s:span' % textNs and not e.currentContent.strip():
                e.currentBuffer.dumpStartElement(elem, attrs)
                e.exprHasStyle = True
        e.manageInserts()
    ...
    def endElement(self, elem):
        ...
        if elem in e.ignorableElements:
            ...
            if actionElemIndex != -1:
                e.currentBuffer = oldCb.transferActionIndependentContent(actionElemIndex)
