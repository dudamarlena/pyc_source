...
# Embedded file name: build/bdist.macosx-10.3-i386/egg/appy/shared/test.py
...
WRONG_TEST_PLAN = 'The test plan you specified does not correspond to an existing RTF file.'
_FLAVOUR = 'A flavour represents a test configuration.'
FLAVOURS_NOT_LIST = 'The flavours specified must be a list or tuple of string. ' + _FLAVOUR
FLAVOUR_NOT_STRING = 'Each specified flavour must be a string. ' + _FLAVOUR
WRONG_TEST_FACTORY = 'You must give a test factory that inherits from the abstract "appy.shared.test.TestFactory" class.'
CREATE_TEST_NOT_OVERRIDDEN = 'The appy.shared.test.TestFactory.createTest method must be overridden in your concrete TestFactory.'
MAIN_TABLE_NOT_FOUND = 'No table "TestSuites" found in test plan "%s".'
MAIN_TABLE_MALFORMED = 'The "TestSuites" table must have at least two columns, named "Name" and "Description".'
TEST_SUITE_NOT_FOUND = 'Table "%s.descriptions" and/or "%s.data" were not found.'
TEST_SUITE_MALFORMED = 'Tables "%s.descriptions" and "%s.data" do not have the same length. For each test in "%s.data", You should have one line in "%s.descriptions" describing the test.'
FILE_NOT_FOUND = 'File to compare "%s" was not found.'
WRONG_ARGS = 'You must specify as unique argument the configuration flavour you want, which may be one of %s.'
WRONG_FLAVOUR = 'Wrong flavour "%s". Flavour must be one of %s.'
TEST_REPORT_SINGLETON_ERROR = 'You can only use the TestReport constructor once. After that you can access the single TestReport instance via the TestReport.instance static member.'
...
class XmlHandler(ContentHandler):
    ...
    def __init__(self, xmlTagsToIgnore):
...
class Test:
    ...
    def compareFiles(self, expected, actual, areXml=False, xmlTagsToIgnore=(), encoding=None):
        ...
        if areXml:
            ...
            contentB = f.read()
            f.close()
            xmlHandler = XmlHandler(xmlTagsToIgnore)
        ...
        for line in diffResult:
            ...
            if line and line[0] != ' ':
                if not atLeastOneDiff:
                    self.report.say('Difference(s) detected between files %s and %s:' % (expected, actual), encoding='utf-8')
                    atLeastOneDiff = True
...
class Tester:
    ...
    def __init__(self, testPlan, flavours, testFactory):
        if not os.path.exists(testPlan) or not os.path.isfile(testPlan) or not testPlan.endswith('.rtf'):
        ...
        self.getOptions()
        self.report = TestReport('%s/Tester.report.txt' % self.testFolder, self.verbose)
    ...
    def getOptions(self):
        ...
        optParser.add_option('-v', '--verbose', action='store_true', help='Dumps the whole test report on stdout')
        optParser.add_option('-k', '--keepTemp', action='store_true', help='Keep the temp folder, in order to be able to copy some results and make them expected results when needed.')
    ...
    def runSuite(self, suite):
        ...
        for testData in self.tables[('%s.data' % suite['Name'])]:
            ...
            if testData['Name'].startswith('_'):
                ...
                description = self.tables[('%s.descriptions' % suite['Name'])][i]['Description']
                test = self.testFactory.createTest(testData, description, self.testFolder, self.config, self.flavour)
                testFailed = test.run()
                if not self.verbose:
                    sys.stdout.write('.')
                    sys.stdout.flush()
    ...
    def run(self):
        assert self.tables.has_key('TestSuites'), TesterError(MAIN_TABLE_NOT_FOUND % self.testPlan)
        for testSuite in self.tables['TestSuites']:
            if not testSuite.has_key('Name') or not testSuite.has_key('Description'):
            ...
            if testSuite['Name'].startswith('_'):
                ...
                tsIgnored = False
            assert self.tables.has_key('%s.descriptions' % tsName) and self.tables.has_key('%s.data' % tsName), TesterError(TEST_SUITE_NOT_FOUND % (tsName, tsName))
            assert len(self.tables[('%s.descriptions' % tsName)]) == len(self.tables[('%s.data' % tsName)]), TesterError(TEST_SUITE_MALFORMED % ((tsName,) * 4))
            if tsIgnored:
    ...
    def finalize(self):
        msg = '%d/%d successful test(s)' % (self.nbOfSuccesses, self.nbOfTests - self.nbOfIgnoredTests)
        if self.nbOfIgnoredTests > 0:
            msg += ', but %d ignored test(s) not counted' % self.nbOfIgnoredTests
        msg += '.'
