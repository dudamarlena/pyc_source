...
# [GCC 8.4.0]
# Embedded file name: /www/elorus/local/lib/python2.7/site-packages/yawdadmin/admin_options.py
...
class OptionSetBase(type):
    ...
    def __new__(self, name, bases, attrs):
        ...
        if not parents:
            ...
            if not optionset_label or not re.match('[a-zA-z-]+', optionset_label):
            ...
            new_class = super_new(self, name, bases, {'__module__': module, 
               'options': options, 
...
class OptionSetAdmin(object):
    ...
    def __init__(self, **kwargs):
        ...
        for attr, value in self.option_fields:
            if attr not in self.value_dict or self.value_dict[attr] is None:
            ...
            if value.lang_dependant:
                for lang in get_supported_languages():
                    ...
                    self.lang_options[field_name] = (
                     attr, lang)
                    if lang not in self.langformfields:
    ...
    def save(self):
        ...
        for field, value in self.form.cleaned_data.iteritems():
            ...
            if field in self.lang_options:
                ...
                if original_field not in lang_dependant_value_dict:
                    lang_dependant_value_dict[original_field] = {}
                lang_dependant_value_dict[original_field].update({lang: prep_value})
            elif self.value_dict[field] != prep_value:
