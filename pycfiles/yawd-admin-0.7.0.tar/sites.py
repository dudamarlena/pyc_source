...
# [GCC 8.4.0]
# Embedded file name: /www/elorus/local/lib/python2.7/site-packages/yawdadmin/sites.py
...
class YawdAdminDashboard(object):
    ...
    def _app_dict_sets(self):
        for app_label in self.app_dict:
            if 'linksets' not in self.app_dict[app_label]:
                if 'models' not in self.app_dict[app_label]:
                    self.app_dict[app_label]['models'] = []
                self.app_dict[app_label]['models'].sort(key=lambda x: x['name'])
                if 'extras' not in self.app_dict[app_label]:
                    ...
                    self.app_dict[app_label]['sets'] = ((
                      '',
                      self.app_dict[app_label]['models'] + self.app_dict[app_label]['extras']),)
            else:
                self.app_dict[app_label]['sets'] = []
    ...
    def _get_has_any_perms(self, data):
        for set in data['sets']:
            for m in set[1]:
                if 'perms' in m and m['perms']['change'] or 'url' in m and m['url']:
                    return True

    def _check_app_dict(self, app_label, has_module_perms):
        if app_label not in self.app_dict:
            self.app_dict[app_label] = {}
        if 'has_module_perms' not in self.app_dict[app_label]:
            self.app_dict[app_label]['has_module_perms'] = has_module_perms
        if 'name' not in self.app_dict[app_label]:
            self.app_dict[app_label]['name'] = app_label.title()
        if 'app_url' not in self.app_dict[app_label]:
            self.app_dict[app_label]['app_url'] = reverse('admin:app_list', kwargs={'app_label': app_label}, current_app=self.name)
        if 'models' not in self.app_dict[app_label]:
            self.app_dict[app_label]['models'] = []
    ...
    def get_app_list(self, request, registry, label=''):
        ...
        for model, model_admin in registry.items():
            ...
            if has_module_perms and app_label not in self.exclude:
                perms = model_admin.get_model_perms(request)
                if True in perms.values():
                    info = (
                     app_label, model._meta.module_name)
                    model_dict = {'classname': model.__name__, 
                    ...
                    if 'exclude' not in self.app_dict[app_label] or model_dict['classname'] not in self.app_dict[app_label]['exclude']:
...
class YawdAdminSite(AdminSite):
    ...
    def check_dependencies(self):
        ...
        if not AppOption._meta.installed:
            raise ImproperlyConfigured("Put 'yawdadmin' in your INSTALLED_APPS setting in order to use the yawd-admin application.")
        if settings.INSTALLED_APPS.index('yawdadmin') > settings.INSTALLED_APPS.index('django.contrib.admin'):
            raise ImproperlyConfigured("Put 'yawdadmin' before 'django.contrib.admin' in your INSTALLED_APPS setting to use the yawd-admin application")
        if 'yawdadmin.middleware.PopupMiddleware' not in settings.MIDDLEWARE_CLASSES:
            raise ImproperlyConfigured("Put 'yawdadmin.middleware.PopupMiddleware' in your MIDDLEWARE_CLASSES setting in order to use the yawd-admin application.")
    ...
    def get_urls(self):
        ...
        up = patterns('', url('^configuration-options/(?P<optionset_label>%s)/$' % ('|').join(_optionset_labels.keys()), wrap(AppOptionView.as_view()), name='optionset-label-options'), url('^oauth2callback/$', wrap(AnalyticsAuthView.as_view()), name='oauth2-callback'), url('^google-analytics/$', wrap(AnalyticsConfigView.as_view()), name='analytics'), url('^google-analytics/connect/$', wrap(AnalyticsConnectView.as_view()), name='analytics-connect'), url('^my-account/$', wrap(MyAccountView.as_view()), name='my-account'))
    ...
    def register_top_menu_item(self, item, icon_class='', children={}, perms=None):
        ...
        if isinstance(item, basestring) and item in app_labels:
            if item not in self._top_menu:
                self._top_menu[item] = icon_class
        elif item and children:
            for child in children:
                if 'name' not in child or 'admin_url' not in child:
            ...
            if item not in self._top_menu:
                self._top_menu[force_text(item)] = {'name': item, 'icon': icon_class, 'models': children}
                if perms:
        ...
        for app in self._top_menu.values():
            if isinstance(app, dict):
                for child in app['models']:
                    if 'show' not in child and 'perms' in app and hasattr(app['perms'], '__call__'):
    ...
    def index(self, request, extra_context={}):
        ...
        context = {'title': _('Site administration'), 'app_list': dashboard.get_app_list(request, self._registry), 
           'dashboard': dashboard}
    ...
    def app_index(self, request, app_label, extra_context=None):
        ...
        context = {'title': _('%s administration') % capfirst(app_label), 'app_list': [
                      app_dict]}
        context.update(extra_context or {})
        return TemplateResponse(request, self.app_index_template or [
         'admin/%s/app_index.html' % app_label,
         'admin/app_index.html'], context, current_app=self.name)
    ...
    def i18n_javascript(self, request):
        ...
        return javascript_catalog(request, packages=['django.conf',
         'django.contrib.admin',
         'yawdadmin'] + getattr(settings, 'ADMIN_JS_CATALOG', []))