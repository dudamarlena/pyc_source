...
# [GCC 8.4.0]
# Embedded file name: /www/elorus/local/lib/python2.7/site-packages/yawdadmin/admin.py
# Compiled at: 2013-10-20 14:32:03
import json
...
class PopupModelAdmin(admin.ModelAdmin):
    ...
    def add_view(self, request, form_url='', extra_context=None):
        if self.popup_only and '_popup' not in request.REQUEST:
            raise Http404
    ...
    def change_view(self, request, object_id, form_url='', extra_context=None):
        if self.popup_only and '_popup' not in request.REQUEST:
            raise Http404
        return super(PopupModelAdmin, self).change_view(request, object_id, form_url, extra_context)
    ...
    def get_urls(self):
        ...
        my_urls = patterns('', url('^(.+)/ajax/delete/$', self.admin_site.admin_view(self.ajaxdelete_view), name='%s_%s_deleteit' % info), url('^ajax/reorder/$', self.admin_site.admin_view(self.inline_reorder), name='%s_%s_inlinereorder' % info))
        return my_urls + urls

    def inline_reorder(self, request):
        if not self.linked_inline or not self.linked_inline.sortable or not self.linked_inline.sortable_order_field:
            raise Http404
    ...
    def response_add(self, request, obj, post_url_continue=None):
        ...
        if '_popup' in request.POST and request.GET.get('fk_name'):
            return HttpResponse('<!DOCTYPE html><html><head><title></title></head><body><script type="text/javascript">parent.dismissAddAnotherPopupInline(window, "%s", "%s", "%s");</script></body></html>' % (
             escape(obj.pk),
             escapejs(inline_items_for_result(self.linked_inline, obj) if self.linked_inline else obj),
    ...
    def response_change(self, request, obj):
        ...
        """
        if '_popup' in request.POST and request.GET.get('fk_name'):
            return HttpResponse('<!DOCTYPE html><html><head><title></title></head><body><script type="text/javascript">parent.dismissEditPopupInline(window, "%s", "%s");</script></body></html>' % (
             escape(obj.pk),
             escapejs(inline_items_for_result(self.linked_inline, obj) if self.linked_inline else obj)))
...
class SortableModelAdmin(admin.ModelAdmin):
    ...
    def get_urls(self):
        ...
        info = (
         self.model._meta.app_label, self.model._meta.module_name)
        my_urls = patterns('', url('^sortables/$', self.admin_site.admin_view(self.sortables), name='%s_%s_sortables' % info), url('^sortables/reorder/$', self.admin_site.admin_view(self.reorder), name='%s_%s_reorder' % info), url('^ajax/$', self.admin_site.admin_view(self.partial_changelist_view), name='%s_%s_partial_changelist' % info))
        return my_urls + urls
    ...
    def _reorder(self, data, request):
        for obj in data:
            ...
            if self.sortable_mptt:
                setattr(model_obj, model_obj._mptt_meta.parent_attr, self.model.objects.get(pk=int(obj['parent'])) if 'parent' in obj else None)
    ...
    def sortables(self, request):
        return TemplateResponse(request, 'admin/sortables/mptt_list.html' if self.sortable_mptt else 'admin/sortables/list.html', {'mptt': self.sortable_mptt, 'objects': self.sortables_ordered(self.queryset(request))})
