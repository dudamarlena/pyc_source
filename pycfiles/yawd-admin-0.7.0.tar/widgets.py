...
# [GCC 8.4.0]
# Embedded file name: /www/elorus/local/lib/python2.7/site-packages/yawdadmin/widgets.py
# Compiled at: 2013-10-21 08:46:09
...
class ContentTypeSelect(forms.Select):
    ...
    def render(self, name, value, attrs=None, choices=()):
        ...
        choiceoutput += '};'
        output += '<script>(function($) {  $(document).ready( function() {%(choiceoutput)s    $(\'#%(id)s\').change(function (){        $(\'#%(fk_id)s\').val(\'\');        $(\'#lookup_%(fk_id)s\').attr(\'href\',%(id)s_choice_urls[$(this).val()]+"&pop=1").siblings(\'strong\').html(\'\');    });  });})(yawdadmin.jQuery);</script>' % {'choiceoutput': choiceoutput, 'id': attrs['id'], 
...
class Select2MultipleWidget(forms.SelectMultiple):
    ...
    def render(self, name, value, attrs=None, choices=()):
        ...
        return result + mark_safe("<script>(function($){$('#%s').select2();})(yawdadmin.jQuery);</script>" % attrs['id'])
...
class Select2Widget(forms.Select):
    ...
    def render(self, name, value, attrs=None, choices=()):
        ...
        return result + mark_safe("<script>(function($){$('#%s').select2(%s);})(yawdadmin.jQuery);</script>" % (
         attrs['id'],
...
class SwitchWidget(forms.CheckboxInput):
    ...
    def render(self, name, value, attrs=None):
        ...
        return mark_safe('<div class="switch %s" data-on="%s" data-off="%s" data-on-label="%s" data-off-label="%s">' % (classes, data_on, data_off, data_on_label, data_off_label)) + output + mark_safe('</div>')