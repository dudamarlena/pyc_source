...
# [GCC 8.4.0]
# Embedded file name: /www/elorus/local/lib/python2.7/site-packages/yawdadmin/templatetags/yawdadmin_tags.py
# Compiled at: 2013-08-25 05:42:07
import inspect, datetime, re
...
def clean_media(media):
    if hasattr(media, '_js'):
        media._js = [ i for i in media._js if not re.match('%sadmin/js/((jquery(\\.init)?|collapse|admin/RelatedObjectLookups)(\\.min)?\\.)js' % settings.STATIC_URL, i) ]
    return media
...
def get_admin_site_meta(context):
    context['ADMIN_SITE_NAME'] = getattr(settings, 'ADMIN_SITE_NAME', _('Django Administration'))
    context['ADMIN_SITE_DESCRIPTION'] = getattr(settings, 'ADMIN_SITE_DESCRIPTION', _('Welcome to the yawd-admin administration page. Please sign in to manage your website.'))
