...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/geats/virtualmachine/abstract.py
# Compiled at: 2013-12-22 08:34:44
        ...
        if 'storage' in self.definition:
            for storage in self.definition['storage']:
                ...
                if valid_storage_types is not None and storage['type'] not in valid_storage_types:
                    raise VMException('Unsupported storage type for this VM. Must be one of: %s' % (', ').join(valid_storage_types))
                vol = self.manager._make_storage_volume(storage['type'], storage, self)
