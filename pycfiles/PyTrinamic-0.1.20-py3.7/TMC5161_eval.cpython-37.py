...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\PyTrinamic\evalboards\TMC5161_eval.py
# Compiled at: 2020-01-30 03:11:26
...
@author: JM
"""
import PyTrinamic.ic.TMC5161.TMC5161 as TMC5161
...
class TMC5161_eval(TMC5161):
    ...
    def __init__(self, connection, moduleID=1):
        ...
        self._TMC5161_eval__connection = connection
    ...
    def writeRegister(self, registerAddress, value, moduleID=None):
        ...
        return self._TMC5161_eval__connection.writeMC(registerAddress, value, moduleID)

    def readRegister(self, registerAddress, moduleID=None, signed=False):
        if not moduleID:
            moduleID = self._MODULE_ID
        return self._TMC5161_eval__connection.readMC(registerAddress, moduleID, signed)

    def getAxisParameter(self, apType, axis):
        if not 0 <= axis < self.MOTORS:
            raise ValueError('Axis index out of range')
        return self._TMC5161_eval__connection.axisParameter(apType, axis)

    def setAxisParameter(self, apType, axis, value):
        if not 0 <= axis < self.MOTORS:
            raise ValueError('Axis index out of range')
        self._TMC5161_eval__connection.setAxisParameter(apType, axis, value)

    def rotate(self, motor, value):
        if not 0 <= motor < self.MOTORS:
            raise ValueError
        self._TMC5161_eval__connection.rotate(motor, value, moduleID=(self._MODULE_ID))

    def stop(self, motor):
        self._TMC5161_eval__connection.stop(motor, moduleID=(self._MODULE_ID))

    def moveTo(self, motor, position, velocity=None):
        if velocity:
            if velocity != 0:
                self.setAxisParameter(self.APs.MaxVelocity, motor, velocity)
        self._TMC5161_eval__connection.move(0, motor, position, moduleID=(self._MODULE_ID))
