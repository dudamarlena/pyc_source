...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\PyTrinamic\TMCL.py
...
class TMCL_Request(object):
    ...
    def __init__(self, address, command, commandType, motorBank, value):
        ...
        self.value = value & 4294967295
        self.checksum = 0
        checksum_struct = struct.pack(TMCL.PACKAGE_STRUCTURE[:-1], address, command, commandType, motorBank, value & 4294967295)
    ...
    def toBuffer(self):
        return struct.pack(TMCL.PACKAGE_STRUCTURE, self.moduleAddress, self.command, self.commandType, self.motorBank, self.value, self.checksum)

    def dump(self):
        print('TMCL_Request: {0:02X},{1:02X},{2:02X},{3:02X},{4:08X},{5:02X}'.format(self.moduleAddress, self.command, self.commandType, self.motorBank, self.value, self.checksum))
...
class TMCL_Reply(object):
    ...
    def dump(self):
        print('TMCL_Reply:   {0:02X},{1:02X},{2:02X},{3:02X},{4:08X},{5:02X}'.format(self.reply_address, self.module_address, self.status, self.command, self.value, self.checksum))
