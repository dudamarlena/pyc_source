...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/asvmq/topic_communications.py
# Compiled at: 2019-01-31 09:37:46
# Size of source mod 2**32: 12586 bytes
"""
...
"""
import uuid, pika, asvprotobuf.std_pb2
...
class Channel:
    __doc__ = 'Internal class for Using Common Functionalities of RabbitMQ and Pika'
...
class Publisher(Channel):
    ...
    def __init__(self, **kwargs):
        ...
        hostname = kwargs.get('hostname', 'localhost')
        port = kwargs.get('port', 5672)
        node_name = kwargs.get('node_name', 'pub_%s' % str(object_type).split("'")[1])
        self._object_type = object_type
        self._topic = topic_name
        Channel.__init__(self, exchange_name=DEFAULT_EXCHANGE_NAME, exchange_type='topic',
          hostname=hostname,
          port=port,
          node_name=node_name)
    ...
    def create(self):
        ...
        Channel.create(self)
        self._channel.exchange_declare(exchange=LOG_EXCHANGE_NAME, exchange_type='fanout')
    ...
    def publish(self, message):
        ...
        if isinstance(message, str):
            ...
            log_success = self._channel.basic_publish(exchange=LOG_EXCHANGE_NAME, routing_key='',
              body=(MessageToJson(log_message).replace('\n', '').replace("'", "'")))
            if not log_success:
                raise RuntimeWarning('Cannot deliver message to logger')
            success = self._channel.basic_publish(exchange=(self.exchange_name), routing_key=(self.topic),
              body=message)
            if not success:
...
class Subscriber(Channel):
    ...
    def __init__(self, **kwargs):
        ...
        hostname = kwargs.get('hostname', 'localhost')
        port = kwargs.get('port', 5672)
        node_name = kwargs.get('node_name', 'sub_%s' % str(object_type).split("'")[1])
        self._topic = topic_name
        self._object_type = object_type
        self._queue = None
    ...
    def create(self):
        ...
        Channel.create(self)
        self._channel.exchange_declare(exchange=GRAPH_EXCHANGE_NAME, exchange_type='fanout')
        self._queue = self._channel.queue_declare(arguments={'x-message-ttl': self.ttl},
          exclusive=True)
        ...
        if self.type is None or self.type == str:
            ...
            if isinstance(body, str):
                ...
                if _type != str:
                    ...
                    curr_timestamp = msg.header.stamp.seconds + msg.header.stamp.nanos / 1000000000
                ...
                if graph_message.freq < 0:
                    graph_message.freq = 0
                graph_success = self._channel.basic_publish(exchange=GRAPH_EXCHANGE_NAME, routing_key='',
                  body=(MessageToJson(graph_message).replace('\n', '').replace("'", "'")))
                assert graph_success, 'The messages cannot be sent to graph.'
...
def _log(string, **kwargs):
    ...
    channel.channel.basic_publish(exchange=LOG_EXCHANGE_NAME, body=log_message,
      routing_key='')
    del channel
