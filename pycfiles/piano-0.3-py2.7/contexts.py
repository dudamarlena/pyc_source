...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\piano\resources\contexts.py
...
class Page(b.ContextBase):
    ...
    def find(cls, key, parent):
        ...
        print data
        return cls(key=key, parent=parent, id=data['_id'], title=data['title'], data=data['data'], slug=data['slug'], origin=data['parent'], views=data['views'], source=str(data['source']), date_created=data['created'])
...
class Site(Page):
    ...
    def find(cls, key, parent):
        ...
        return cls(key=key, parent=parent, id=data['_id'], title=data['title'], slug=data['slug'], views=data['views'], date_created=data['created'])
    ...
    def save(self, include_default=False):
        ...
        data.save()
        if include_default:
            Page(key='home', parent=self, title='Home', slug='home', source='sample.home').save()
        return self
