...
# [GCC 8.4.0]
# Embedded file name: C:/Users/HDi/Google Drive/ProgramCodes/Released/PyPI/cognitivegeo\cognitivegeo\src\gui\calcmathattribsingle.py
# Compiled at: 2019-12-15 18:54:42
# Size of source mod 2**32: 9553 bytes
from PyQt5 import QtCore, QtGui, QtWidgets
import numpy as np, os, sys
sys.path.append(os.path.dirname(__file__)[:-4][:-4][:-13])
import cognitivegeo.src.seismic.analysis as seis_ays
import cognitivegeo.src.seismic.attribute as seis_attrib
import cognitivegeo.src.vis.messager as vis_msg
import cognitivegeo.src.gui.calculator as gui_calculator
...
class calcmathattribsingle(object):
    ...
    dialog = None
    mathattriblist = [
     'Calculator', 'Cumulative sum', 'First derivative (with z)']
    ...
    def clickBtnCalcMathAttribSingle(self):
        ...
        if len(_propertylist) < 1:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Calculate Math Attribute from Single Property', 'No property selected for attribute analysis')
            return
        if len(self.ldtname.text()) < 1:
            vis_msg.print('ERROR in CalcMathAttribSingle: No name specified for output attribute', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Calculate Math Attribute from Single Property', 'No name specified for output attribute')
            return
        if self.ldtname.text() in self.seisdata.keys():
            if self.checkSeisData(self.ldtname.text()):
                reply = QtWidgets.QMessageBox.question(self.msgbox, 'Calculate Math Attribute from Single Property', self.ldtname.text() + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
        ...
        if self.cbbattrib.currentIndex() == 2:
            ...
            self.seisdata[self.ldtname.text()] = _attrib
        self.refreshLwgProperty()
        QtWidgets.QMessageBox.information(self.msgbox, 'Calculate Math Attribute from Single Property', self.mathattriblist[self.cbbattrib.currentIndex()] + ' calculated successfully')
