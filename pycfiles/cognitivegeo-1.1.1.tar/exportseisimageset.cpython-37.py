...
# [GCC 8.4.0]
# Embedded file name: C:/Users/HDi/Google Drive/ProgramCodes/Released/PyPI/cognitivegeo\cognitivegeo\src\gui\exportseisimageset.py
...
class exportseisimageset(object):
    ...
    dialog = None
    cmapsname = [
     'Seismic', 'Phase', 'Frequency', 'Red-White-Blue', 'Gray-Scale',
    ...
    def retranslateGUI(self, ExportSeisImageSet):
        ...
        for _i in range(len(self.cmaps)):
            self.cbbcmap.setItemIcon(_i, QtGui.QIcon(QtGui.QPixmap(os.path.join(self.iconpath, 'icons/cmap_' + self.cmaps[_i] + '.png')).scaled(80, 30)))
    ...
    def clickBtnExportSeisImageSet(self):
        ...
        if len(_attriblist) < 1:
            vis_msg.print('ERROR in ExportSeisImageSet: No property selected for export', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Export Seismic ImageSet', 'No property selected for export')
            return
        if self.rdbslicedef.isChecked():
            for s in self.ldtslicedef.text().split(','):
                if basic_data.str2int(s) is False:
                    vis_msg.print('ERROR in ExportSeisImageSet: Non-integer slice selection', type='error')
                    QtWidgets.QMessageBox.critical(self.msgbox, 'Export Seismic ImageSet', 'Non-integer slice selection')
                    return
        ...
        for i in range(len(_attriblist)):
            ...
            _imagefile = os.path.join(self.ldtsave.text(), _attriblist[i].text() + '_')
            ...
            if self.cbbtype.currentIndex() == 0:
                seis_vis.saveSeisILSliceFrom3DMat(_data, _imagefile, inlsls=_sls, seisinfo=(self.survinfo), valuemin=(np.min(_data)),
                  valuemax=(np.max(_data)),
                  colormap=(self.cmaps[_cmap]),
                  flipcmap=_flip,
                  verbose=False)
            if self.cbbtype.currentIndex() == 1:
                seis_vis.saveSeisXLSliceFrom3DMat(_data, _imagefile, xlsls=_sls, seisinfo=(self.survinfo), valuemin=(np.min(_data)),
                  valuemax=(np.max(_data)),
                  colormap=(self.cmaps[_cmap]),
                  flipcmap=_flip,
                  verbose=False,
            ...
            if self.cbbtype.currentIndex() == 2:
                seis_vis.saveSeisZSliceFrom3DMat(_data, _imagefile, zsls=_sls, seisinfo=(self.survinfo), valuemin=(np.min(_data)),
                  valuemax=(np.max(_data)),
                  colormap=(self.cmaps[_cmap]),
                  flipcmap=_flip,
                  verbose=False)

        QtWidgets.QMessageBox.information(self.msgbox, 'Export Seismic ImageSet', str(len(_attriblist)) + ' properties exported as ImageSet successfully')
