...
# [GCC 8.4.0]
# Embedded file name: C:/Users/HDi/Google Drive/ProgramCodes/Released/PyPI/cognitivegeo\cognitivegeo\src\gui\plotvis2dpsseisshot.py
# Compiled at: 2019-12-16 00:14:22
# Size of source mod 2**32: 14546 bytes
...
from cognitivegeo.src.psseismic.visualization import visualization as psseis_vis
...
class plotvis2dpsseisshot(object):
    ...
    def retranslateGUI(self, PlotVis2DPsSeisShot):
        ...
        if self.checkPsSeisData():
            ...
            if self.lwgpsseis.currentItem() is not None and self.lwgpsseis.currentItem().text() in self.psseisdata.keys():
        ...
        if self.lwgpsseis.currentItem() is not None:
            if self.lwgpsseis.currentItem().text() in self.psseisdata.keys():
    ...
    def clickBtnPlotShot(self):
        ...
        if len(_psseislist) < 1:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, '2D Window: Pre-stack Gather', 'No pre-stack selected for plot')
            return
        ...
        if _min is False or _max is False:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, '2D Window: Pre-stack Gather', 'Non-float range specified for plot')
            return
        for i in range(len(_psseislist)):
            ...
            psseis_vis.plotPsSeisShotPlayer(_data, initshot=_sls, titlesurf=(' in ' + _psseislist[i].text()),
              valuemin=_min,
              valuemax=_max,
    ...
    def getShotRange(self, f):
        _min = -1
        _max = 1
        if f in self.psseisdata.keys():
            if psseis_ays.checkPsSeis(self.psseisdata[f]):
