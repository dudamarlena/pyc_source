...
# [GCC 8.4.0]
# Embedded file name: C:/Users/HDi/Google Drive/ProgramCodes/Released/PyPI/cognitivegeo\cognitivegeo\src\gui\exportseissegy.py
# Compiled at: 2019-12-15 21:49:30
# Size of source mod 2**32: 12312 bytes
...
class exportseissegy(object):
    ...
    def clickBtnSurveyRef(self):
        ...
        _file = _dialog.getOpenFileName(None, 'Select Reference SEG-Y', (self.rootpath), filter='SEG-Y files (*.segy; *.sgy);; All files (*.*)')
        if len(_file[0]) > 0:
            self.ldtsurveyref.setText(_file[0])

    def clickBtnSave(self):
        _dialog = QtWidgets.QFileDialog()
        _file = _dialog.getExistingDirectory(None, 'Select Export Folder', (self.rootpath), options=(QtWidgets.QFileDialog.DontUseNativeDialog))
        if len(_file) > 0:
    ...
    def clickBtnExportSeisSegy(self):
        ...
        if len(_attriblist) < 1:
            vis_msg.print('ERROR in ExportSeisSegy: No property selected for export', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Export Seismic SEG-Y', 'No property selected for export')
            return
        if self.rdbsurveyref.isChecked():
            if os.path.exists(self.ldtsurveyref.text()) is False:
                vis_msg.print('ERROR in ExportSeisSegy: Reference SEG-Y not found', type='error')
                QtWidgets.QMessageBox.critical(self.msgbox, 'Export Seismic SEG-Y', 'Reference SEG-Y not found')
                return
        ...
        for i in range(len(_attriblist)):
            ...
            _segyfile = os.path.join(self.ldtsave.text(), _attriblist[i].text() + '.segy')
            print('ExportSeisSegy: Export %d of %d SEG-Y: %s' % (i + 1, len(_attriblist), _segyfile))
            _data = self.seisdata[_attriblist[i].text()]
            if self.rdbsurveynew.isChecked():
                seis_io.writeSeis3DMatToSegyNoRef(_data, _segyfile, inlstart=(self.survinfo['ILStart']),
                  inlstep=(self.survinfo['ILStep']),
                  xlstart=(self.survinfo['XLStart']),
                  xlstep=(self.survinfo['XLStep']),
                  zstart=(self.survinfo['ZStart']),
                  zstep=(self.survinfo['ZStep']),
                  xcoorstart=(self.survinfo['XLStart']),
                  ycoorstart=(self.survinfo['ILStart']),
                  inlbinsize=(self.survinfo['ILStep']),
                  xlbinsize=(self.survinfo['XLStep']),
                  orientation=90,
                  verbose=False,
                  qpgsdlg=_pgsdlg)
            else:
                _reffile = self.ldtsurveyref.text()
                seis_io.writeSeis3DMatToSegyWithRef(_data, _segyfile, _reffile, verbose=False, qpgsdlg=_pgsdlg)

        QtWidgets.QMessageBox.information(self.msgbox, 'Export Seismic SEG-Y', str(len(_attriblist)) + ' properties exported as SEG-Y successfully')
