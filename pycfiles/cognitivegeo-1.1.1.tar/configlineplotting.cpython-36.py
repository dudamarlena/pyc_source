...
# [GCC 8.4.0]
# Embedded file name: C:/Users/HDi/Google Drive/ProgramCodes/Released/PyPI/cognitivegeo\cognitivegeo\src\gui\configlineplotting.py
# Compiled at: 2019-12-15 18:54:42
# Size of source mod 2**32: 7606 bytes
from PyQt5 import QtCore, QtGui, QtWidgets
import os, sys, numpy as np
sys.path.append(os.path.dirname(__file__)[:-4][:-4][:-13])
from cognitivegeo.src.vis.color import color as vis_color
from cognitivegeo.src.vis.line import line as vis_line
from cognitivegeo.src.vis.marker import marker as vis_marker
    ...
    def retranslateGUI(self, ConfigLinePlotting):
        ...
        for line in sorted(self.lineplottingconfig.keys()):
            ...
            for _i in range(len(vis_color.ColorList)):
                item.setItemIcon(_i, QtGui.QIcon(QtGui.QPixmap(os.path.join(self.iconpath, 'icons/color_' + vis_color.ColorList[_i] + '.png')).scaled(80, 40)))

            item.setCurrentIndex(list.index(vis_color.ColorList, self.lineplottingconfig[line]['Color']))
            self.twgline.setCellWidget(_idx, 1, item)
            item = QtWidgets.QComboBox()
            item.addItems([str(_i) for _i in vis_line.LineWidthList])
            item.setCurrentIndex(list.index(vis_line.LineWidthList, self.lineplottingconfig[line]['Width']))
            self.twgline.setCellWidget(_idx, 2, item)
            item = QtWidgets.QComboBox()
            item.addItems(vis_line.LineStyleList)
            for _i in range(len(vis_line.LineStyleList)):
                item.setItemIcon(_i, QtGui.QIcon(QtGui.QPixmap(os.path.join(self.iconpath, 'icons/line_' + vis_line.LineStyleList[_i] + '.png')).scaled(80, 40)))

            item.setCurrentIndex(list.index(vis_line.LineStyleList, self.lineplottingconfig[line]['Style']))
            self.twgline.setCellWidget(_idx, 3, item)
            item = QtWidgets.QComboBox()
            item.addItems(vis_marker.MarkerStyleList)
            item.setCurrentIndex(list.index(vis_marker.MarkerStyleList, self.lineplottingconfig[line]['MarkerStyle']))
            self.twgline.setCellWidget(_idx, 4, item)
            item = QtWidgets.QComboBox()
            item.addItems([str(_i) for _i in vis_marker.MarkerSizeList])
            item.setCurrentIndex(list.index(vis_marker.MarkerSizeList, self.lineplottingconfig[line]['MarkerSize']))
