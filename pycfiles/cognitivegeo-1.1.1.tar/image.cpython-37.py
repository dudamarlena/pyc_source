...
# [GCC 8.4.0]
# Embedded file name: C:/Users/HDi/Google Drive/ProgramCodes/Released/PyPI/cognitivegeo\cognitivegeo\src\vis\image.py
...
__all__ = [
 'image']
InterpolationList = [
 'None', 'Nearest', 'Quadric', 'Bilinear', 'Bicubic', 'Kaiser']

def plotImage(image, image_height, image_width, interpolation='bicubic', index_list=[], cmapname='red_white_blue', flipcmap=False):
    """
    Plot images (by matplotlib.imshow)
    ...
    Args:
        ...
        interpolation:  Interpolation option. Default is 'bicubic'
    ...
    for i in index_list:
        if i >= 0 and i < nimage:
            ...
            plt.imshow(image_i, interpolation=interpolation, cmap=(vis_cmap.makeColorMap(cmapname, flipcmap)))
            plt.axis('off')

    plt.show()


def plotImageGallery(image, image_height, image_width, ncol=5, cmapname='red_white_blue', flipcmap=False, interpolation='bicubic', spacing=False, maxfigsize=10, valuemin=None, valuemax=None):
    """
    Plot image gallery (by matplotlib.imshow)
    ...
    Args:
        ...
        spacing:        Flag of spacing between images. Default is False
        valuemin:       Minimum value. Default is None (minimum of all images)
        valuemax:       Maximum value. Default is None (maximum of all images)
    ...
    for i in range(nrow):
        for j in range(ncol):
            ...
            plt.imshow((np.reshape(image[i * ncol + j, :], [image_height, image_width])), interpolation=interpolation,
