...
# [GCC 8.4.0]
# Embedded file name: C:/Users/HDi/Google Drive/ProgramCodes/Released/PyPI/cognitivegeo\cognitivegeo\src\gui\convertpsseis2seis.py
# Compiled at: 2019-12-15 18:54:42
# Size of source mod 2**32: 9718 bytes
...
class convertpsseis2seis(object):
    ...
    def clickBtnApply(self):
        ...
        if len(_psseislist) < 1:
            vis_msg.print('ERROR in ConvertPsSeis2Seis: No pre-stack selected for conversion', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Convert Pre-stack to Seismic', 'No pre-stack selected for conversion')
            return
        _psshotlist = self.lwgpsshot.selectedItems()
        _psshotlist = [f.text() for f in _psshotlist]
        if len(_psshotlist) < 1:
            vis_msg.print('ERROR in ConvertPsSeis2Seis: No shot selected for conversion', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Convert Pre-stack to Seismic', 'No shot selected for conversion')
            return
        ...
        if checkSurvInfo(self.survinfo) and self.survinfo['ZNum'] == np.shape(_psseisdata)[0] and self.survinfo['XLNum'] == np.shape(_psseisdata)[1] and self.survinfo['ILNum'] == np.shape(_psseisdata)[2]:
            _survinfo = self.survinfo
        else:
            _survinfo = seis_ays.createSeisInfoFrom3DMat(_psseisdata, zstart=(_psseisinfo['ZStart']),
        ...
        _seisdata = {}
        _seisdata[_psshotlist[0]] = _psseisdata
        ...
        for key in _seisdata.keys():
            if key in self.seisdata.keys():
                if checkSeisData(self.seisdata[key], self.survinfo):
                    reply = QtWidgets.QMessageBox.question(self.msgbox, 'Convert Pre-stack to Seismic', key + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
                    if reply == QtWidgets.QMessageBox.No:
                        return
            self.seisdata[key] = _seisdata[key]

        QtWidgets.QMessageBox.information(self.msgbox, 'Convert Pre-stack to Seismic', 'Pre-stack seismic converted successfully')
