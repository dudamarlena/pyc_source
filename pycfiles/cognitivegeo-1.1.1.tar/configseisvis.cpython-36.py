...
# [GCC 8.4.0]
# Embedded file name: C:/Users/HDi/Google Drive/ProgramCodes/Released/PyPI/cognitivegeo\cognitivegeo\src\gui\configseisvis.py
# Compiled at: 2019-12-13 23:25:59
# Size of source mod 2**32: 8167 bytes
...
class configseisvis(object):
    ...
    def retranslateGUI(self, ConfigSeisVis):
        ...
        for seis in sorted(self.seisvisconfig.keys()):
            ...
            for _i in range(len(vis_cmap.ColorMapList)):
                item.setItemIcon(_i, QtGui.QIcon(QtGui.QPixmap(os.path.join(self.iconpath, 'icons/cmap_' + vis_cmap.ColorMapList[_i] + '.png')).scaled(80, 40)))

            item.setCurrentIndex(list.index(vis_cmap.ColorMapList, self.seisvisconfig[seis]['Colormap']))
            ...
            item.setCurrentIndex(list.index(vis_cmap.OpacityList, self.seisvisconfig[seis]['Opacity']))
            ...
            for _i in range(len(vis_image.InterpolationList)):
                item.setItemIcon(_i, QtGui.QIcon(QtGui.QPixmap(os.path.join(self.iconpath, 'icons/interp_' + vis_image.InterpolationList[_i] + '.png')).scaled(80, 40)))

            item.setCurrentIndex(list.index(vis_image.InterpolationList, self.seisvisconfig[seis]['Interpolation']))
    ...
    def clickBtnApply(self):
        ...
        for seis in sorted(self.seisvisconfig.keys()):
            ...
            if _max is not False:
                if _min is not False:
                    if _max >= _min:
