...
# [GCC 8.4.0]
# Embedded file name: C:/Users/HDi/Google Drive/ProgramCodes/Released/PyPI/cognitivegeo\cognitivegeo\src\gui\importsurveysegy.py
...
class importsurveysegy(object):
    ...
    def clickBtnSegy(self):
        ...
        _file = _dialog.getOpenFileName(None, 'Select Seismic SEG-Y', (self.rootpath), filter='SEG-Y files (*.segy; *.sgy);;All files (*.*)')
        if len(_file[0]) > 0:
    ...
    def clickBtnTextualHeader(self):
        ...
        if os.path.exists(self.segyfile) is False or os.path.isfile(self.segyfile) is False:
            vis_msg.print('ERROR in ImportSurveySegy: No SEG-Y selected for viewing textual header', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import Survey from Seismic SEG-Y', 'No SEG-Y selected for viewing textual header')
            return
    ...
    def clickBtnBinaryHeader(self):
        self.refreshMsgBox()
        if os.path.exists(self.segyfile) is False or os.path.isfile(self.segyfile) is False:
            vis_msg.print('ERROR in ImportSurveySegy: No SEG-Y selected for viewing binary header', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import Survey from Seismic SEG-Y', 'No SEG-Y selected for viewing binary header')
            return
        _viewheader = QtWidgets.QDialog()
    ...
    def clickBtnTraceHeader(self):
        self.refreshMsgBox()
        if os.path.exists(self.segyfile) is False or os.path.isfile(self.segyfile) is False:
            vis_msg.print('ERROR in ImportSurveySegy: No SEG-Y selected for viewing trace header', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import Survey from Seismic SEG-Y', 'No SEG-Y selected for viewing trace header')
            return
        _viewheader = QtWidgets.QDialog()
    ...
    def clickBtnImport(self):
        self.refreshMsgBox()
        if os.path.exists(self.segyfile) is False or os.path.isfile(self.segyfile) is False:
            vis_msg.print('ERROR in ImportSurveySegy: No SEG-Y selected for import', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import Survey from Seismic SEG-Y', 'No SEG-Y selected for import')
            return
        ...
        _survinfo = seis_io.readSeisInfoFromSegy((self.segyfile), traceheaderformat=_traceheaderformat)
        ...
        self.checkSurvInfo()
        QtWidgets.QMessageBox.information(self.msgbox, 'Import Survey from Seismic SEG-Y', ' Survey imported successfully')
    ...
    def checkSurvInfo(self):
        ...
        if checkSurvInfo(self.survinfo) is False:
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import Seismic SEG-Y', 'No survey found')
            return
