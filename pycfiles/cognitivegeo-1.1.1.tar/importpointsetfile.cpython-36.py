...
# [GCC 8.4.0]
# Embedded file name: C:/Users/HDi/Google Drive/ProgramCodes/Released/PyPI/cognitivegeo\cognitivegeo\src\gui\importpointsetfile.py
# Compiled at: 2020-01-04 15:56:14
# Size of source mod 2**32: 15553 bytes
from PyQt5 import QtCore, QtGui, QtWidgets
import numpy as np, sys, os
sys.path.append(os.path.dirname(__file__)[:-4][:-4][:-13])
from cognitivegeo.src.pointset.inputoutput import inputoutput as point_io
...
class importpointsetfile(object):
    ...
    def clickBtnFile(self):
        ...
        _file = _dialog.getOpenFileNames(None, 'Select Point File(s)', (self.rootpath), filter='All files (*.*)')
        if len(_file[0]) > 0:
    ...
    def clickBtnImportPointSetFile(self):
        ...
        if _nfile <= 0:
            vis_msg.print('ERROR in ImagePointSetFile: No file selected for import', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import PointSet from File', 'No file selected for import')
            return
        ...
        for i in range(_nfile):
            ...
            _pointsetdata[_filenamemain] = {}
            _data = point_io.readPointFromAscii(_filename, comment=_comment, delimiter=_delimiter, inlcol=(self.cbbinl.currentIndex()),
              xlcol=(self.cbbxl.currentIndex()),
              zcol=(self.cbbz.currentIndex()))
            _pointsetdata[_filenamemain]['Inline'] = _data[:, 0:1]
            _pointsetdata[_filenamemain]['Crossline'] = _data[:, 1:2]
            _pointsetdata[_filenamemain]['Z'] = _data[:, 2:3]
            for _i in range(np.shape(_data)[1] - 3):
                _propname = 'property_' + str(_i + 1)
                _pointsetdata[_filenamemain][_propname] = _data[:, _i + 3:_i + 4]

            if 'Z' in _pointsetdata[_filenamemain].keys():
                if np.min(_pointsetdata[_filenamemain]['Z']) >= 0:
        ...
        for key in _pointsetdata.keys():
            if key in self.pointsetdata.keys():
                if checkPointData(self.pointsetdata[key]):
        ...
        QtWidgets.QMessageBox.information(self.msgbox, 'Import PointSet from File', str(_nfile) + ' file(s) imported successfully')
