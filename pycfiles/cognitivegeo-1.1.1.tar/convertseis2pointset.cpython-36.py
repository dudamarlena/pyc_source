...
# [GCC 8.4.0]
# Embedded file name: C:/Users/HDi/Google Drive/ProgramCodes/Released/PyPI/cognitivegeo\cognitivegeo\src\gui\convertseis2pointset.py
# Compiled at: 2020-01-04 23:31:17
# Size of source mod 2**32: 22831 bytes
...
class convertseis2pointset(object):
    ...
    def clickBtnApply(self):
        self.refreshMsgBox()
        if self.ldtsave.text() in self.pointsetdata.keys():
            reply = QtWidgets.QMessageBox.question(self.msgbox, 'Convert Seismic to PointSet', self.ldtsave.text() + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
            if reply == QtWidgets.QMessageBox.No:
                return
        if self.checkSurvInfo() is False:
            vis_msg.print('ERROR in ConvertSeis2PointSet: No seismic survey found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Convert Seismic to PointSet', 'No seismic survey found')
            return
        if self.rdbsrvpart.isChecked():
            ...
            if _inlstart is False or _xlstart is False or _zstart is False or _inlend is False or _xlend is False or _zend is False:
                vis_msg.print('ERROR in ConvertSeis2PointSet: Non-integer survey selection', type='error')
                QtWidgets.QMessageBox.critical(self.msgbox, 'Convert Seismic to PointSet', 'Non-integer survey selection')
                return
        _survinfo = self.survinfo
        if self.rdbsrvall.isChecked():
        ...
        if self.rdbsrvpart.isChecked():
            ...
            _inlidx = np.arange(_inlstart_idx, (_inlend_idx + 1), (self.cbbinlitvl.currentIndex() + 1),
              dtype=int)
            _xlidx = np.arange(_xlstart_idx, (_xlend_idx + 1), (self.cbbxlitvl.currentIndex() + 1),
              dtype=int)
            _zidx = np.arange(_zstart_idx, (_zend_idx + 1), (self.cbbzitvl.currentIndex() + 1),
              dtype=int)
            ...
            _pts = seis_ays.retrieveSeisILSliceFrom3DMat((np.zeros([self.survinfo['ZNum'], self.survinfo['XLNum'], self.survinfo['ILNum']])), inlsls=_inl,
              verbose=False,
              seisinfo=(self.survinfo))
            _pts = seis_ays.retrieveSeisXLSliceFrom3DMat((seis_ays.convertSeis2DMatTo3DMat(_pts)), xlsls=_xl, verbose=False, seisinfo=(seis_ays.getSeisInfoFrom2DMat(_pts)))
            _pts = seis_ays.retrieveSeisZSliceFrom3DMat((seis_ays.convertSeis2DMatTo3DMat(_pts)), zsls=_z, verbose=False, seisinfo=(seis_ays.getSeisInfoFrom2DMat(_pts)))
        self.pointsetdata[self.ldtsave.text()] = {}
        self.pointsetdata[self.ldtsave.text()]['Inline'] = _pts[:, 0:1]
        self.pointsetdata[self.ldtsave.text()]['Crossline'] = _pts[:, 1:2]
        self.pointsetdata[self.ldtsave.text()]['Z'] = _pts[:, 2:3]
        ...
        for _idx in range(len(_proplist)):
            ...
            if self.rdbsrvall.isChecked():
                self.pointsetdata[self.ldtsave.text()][_proplist[_idx]] = np.reshape(np.transpose(_data, [2, 1, 0]), [-1, 1])
            if self.rdbsrvpart.isChecked():
                self.pointsetdata[self.ldtsave.text()][_proplist[_idx]] = seis_ays.retrieveSeisSampleFrom3DMat(_data, seisinfo=(self.survinfo), verbose=False, samples=_pts)[:, 3:4]

        _pgsdlg.setValue(len(_proplist))
        QtWidgets.QMessageBox.information(self.msgbox, 'Convert Seismic to PointSet', str(len(_pts)) + ' seismic converted successfully')
