...
# [GCC 8.4.0]
# Embedded file name: C:/Users/HDi/Google Drive/ProgramCodes/Released/PyPI/cognitivegeo\cognitivegeo\src\gui\editsurvey.py
...
class editsurvey(object):
    ...
    def clickBtnApply(self):
        ...
        if self.checkSurvInfo() is False:
            vis_msg.print('ERROR in EditSurvey: No seismic survey found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Edit Survey', 'No seismic survey found')
            return
        ...
        if _inlstart is False or _inlstep is False or _inlnum is False or _xlstart is False or _xlstep is False or _xlnum is False or _zstart is False or _zstep is False or _znum is False:
            vis_msg.print('ERROR in EditSurvey: Non-integer survey selection', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Edit Survey', 'Non-integer survey selection')
            return
        _zerodata = np.zeros([_znum, _xlnum, _inlnum])
        self.survinfo = seis_ays.createSeisInfoFrom3DMat(_zerodata, inlstart=_inlstart,
          inlstep=_inlstep,
          xlstart=_xlstart,
          xlstep=_xlstep,
          zstart=_zstart,
