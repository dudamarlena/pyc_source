...
# [GCC 8.4.0]
# Embedded file name: /home/szczepan/.virtualenvs/pyh/lib/python2.7/site-packages/pyhistory/cli.py
...
line_length = click.option('--line-length', help='Formatted line length.', default=default_values.get('line_length', 79), show_default=True, type=int)

@click.group(help=__description__)
@click.pass_context
@click.version_option()
@click.help_option('-h')
@click.option('--history-dir', help='History directory location.', default=default_values.get('history_dir', 'history'), show_default=True)
@click.option('--history-file', help='History file name.', default=default_values.get('history_file', 'HISTORY.rst'), show_default=True)
def main(context, history_dir, history_file):
    ...
    context.obj = {'history_dir': history_dir, 
...
def list(context, line_length):
    ...
    formatted_lines = [ format_line(LINE_PREFIX, line, line_length) for line in lines.values()
                      ]
...
@line_length
@click.option('--at-line', help='Update file at line. (By default after first headline.)', default=default_values.get('at_line'), callback=_maybe_positive, type=int)
@click.option('--date', help='Date of update. By default today, but can be arbitrary value.')
def update(context, version, at_line, date, line_length):
    pyhistory.update(version, context.obj['history_dir'], context.obj['history_file'], at_line, date, line_length, LINE_PREFIX)
