...
# [GCC 8.4.0]
# Embedded file name: /home/szczepan/.virtualenvs/pyh/lib/python2.7/site-packages/pyhistory/pyhistory.py
...
def update(version, history_dir, history_file, at_line=None, date=None, line_length=0, prefix=''):
...
def _get_paragraph(version, history_dir, date, line_length, prefix):
    ...
    content = [
     header + '\n',
     '+' * len(header) + '\n\n']
    lines = [ format_line(prefix, line, line_length) for line in list_(history_dir).values()
            ]
    content += lines
    content.append('\n')
    return content
