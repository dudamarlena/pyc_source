...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taco/aws_wrappers/s3_wrapper/s3_wrapper.py
# Compiled at: 2019-09-05 09:49:59
import botocore.exceptions, taco.common.logger_based_object
from taco import aws_wrappers as boto3_helpers
import taco.boto3.boto_config as boto_config, taco.boto3.boto3_helpers as boto3_helpers
...
class S3Wrapper(taco.common.logger_based_object.LoggerBasedObject):
    ...
    def __init__(self, region_name=s3_consts.DEFAULT_REGION, resource_config=s3_consts.DEFAULT_RESOURCE_CONFIG, aws_access_key=None, aws_secret_key=None, logger=None):
        super().__init__(logger=logger)
        self._default_region_name = region_name
        self._s3_client, self._s3_resource = boto3_helpers.get_client_resource(aws_access_key, aws_secret_key, boto_config.Boto3Resources.s3.value, self._default_region_name, config=resource_config)
    ...
    def delete_bucket(self, bucket_name, delete_non_empty_bucket=True):
        try:
            ...
            if sum(1 for _ in bucket_objects) != 0:
                ...
                self._logger.debug('Deleted bucket objects', deletion_output=objects_deletion_output)
            bucket.delete()
        except botocore.exceptions.ClientError as exc:
    ...
    def create_bucket(self, bucket_name, region_name=None):
        ...
        try:
            bucket = self._s3_resource.create_bucket(Bucket=bucket_name, ACL='private', CreateBucketConfiguration={'LocationConstraint': region_name})
        except botocore.exceptions.ClientError as exc:
    ...
    def upload_data_to_file(self, bucket_name, file_path, data='', metadata=None, content_type=s3_consts.ContentType.default.value):
        if metadata is None:
            metadata = {}
        ...
        try:
            ...
            self._logger.log_and_raise(s3_exceptions.UploadingFileContentException, bucket_name=bucket_name, file_path=file_path, exc=exc)
    ...
    def update_file_metadata(self, bucket_name, file_path, new_metadata):
        try:
            ...
            s3_object.copy_from(CopySource={'Bucket': bucket_name, 'Key': file_path}, Metadata=original_metadata, MetadataDirective='REPLACE')
        except botocore.exceptions.ClientError as exc:
            self._logger.log_and_raise(s3_exceptions.UpdatingFileMetadataException, bucket_name=bucket_name, file_path=file_path, exc=exc)
    ...
    def delete_file(self, bucket_name, file_path):
        try:
            ...
            self._logger.log_and_raise(s3_exceptions.DeletingFileException, bucket_name=bucket_name, file_path=file_path, exc=exc)
    ...
    def get_file_metadata(self, bucket_name, file_path):
        try:
            ...
            if boto3_helpers.is_exception_type(exc, s3_consts.IgnoredErrors.object_missing.value):
                self._logger.log_and_raise(s3_exceptions.GettingFileMetadataException, bucket_name=bucket_name, file_path=file_path, exc=exc)
    ...
    def get_file_data(self, bucket_name, file_path):
        try:
            ...
            self._logger.log_and_raise(s3_exceptions.GettingFileContentException, bucket_name=bucket_name, file_path=file_path, exc=exc)
    ...
    def get_head_object(self, bucket_name, file_path):
        try:
            ...
            if boto3_helpers.is_exception_type(exc, s3_consts.IgnoredErrors.object_missing.value):
                return
            self._logger.log_and_raise(s3_exceptions.GettingHeadObjectException, bucket_name=bucket_name, file_path=file_path, exc=exc)