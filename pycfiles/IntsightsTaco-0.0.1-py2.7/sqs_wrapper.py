...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taco/aws_wrappers/sqs_wrapper/sqs_wrapper.py
# Compiled at: 2019-09-05 09:49:59
import json
from botocore.exceptions import ClientError, ParamValidationError
import taco.boto3.boto_config as boto_config, taco.boto3.boto3_helpers as boto3_helpers, taco.common.logger_based_object
...
from taco.aws_wrappers.sqs_wrapper import sqs_message_wrapper
...
class SQSWrapper(taco.common.logger_based_object.LoggerBasedObject):
    ...
    def __init__(self, region_name=sqs_consts.DEFAULT_REGION, aws_access_key=None, aws_secret_key=None, logger=None):
        ...
        self._sqs_client, self._sqs_resource = boto3_helpers.get_client_resource(aws_access_key, aws_secret_key, boto_config.Boto3Resources.sqs.value, self._default_region_name)
    ...
    def create_queue(self, queue_creation_config):
        ...
        try:
            ...
            self._logger.log_and_raise(sqs_exceptions.SQSInvalidParamsException, params=queue_creation_config.requested_data, exc=exc)
        except ClientError as exc:
            if boto3_helpers.is_exception_type(exc, sqs_consts.SpecificClientErrors.existing_queue.value):
                self._logger.log_and_raise(sqs_exceptions.SQSCreateExistingQueueParamsErrorException, params=queue_creation_config.requested_data, exc=exc)
            self._logger.log_and_raise(sqs_exceptions.CreateQueuesClientException, queue_creation_config=queue_creation_config, exc=exc)

    def get_queue_by_name(self, queue_name):
        try:
            self._logger.info('Find queue', name=queue_name)
            return self._sqs_resource.get_queue_by_name(QueueName=sqs_consts.DefaultQueueConfig.queue_name_format.value.format(queue_name))
        except ClientError as exc:
    ...
    def clear_queue(self, queue_name):
        try:
            ...
            if boto3_helpers.is_exception_type(exc, sqs_consts.IgnoredErrors.purge_queue_in_progress.value):
                self._logger.info('Client exception will be ignored', exception_type=sqs_consts.IgnoredErrors.purge_queue_in_progress.value)
                return
    ...
    def delete_queue(self, queue_name, is_dlq=False):
        ...
        """
        formatted_queue_name = sqs_consts.DefaultQueueConfig.dead_letter_queue_name_format.value.format(queue_name) if is_dlq else sqs_consts.DefaultQueueConfig.queue_name_format.value.format(queue_name)
        ...
        try:
            ...
            if boto3_helpers.is_exception_type(exc, sqs_consts.IgnoredErrors.non_existent_queue.value):
                self._logger.info('Client exception will be ignored', exception_type=sqs_consts.IgnoredErrors.non_existent_queue.value)
                return
    ...
    def _create_request_data(self, data, delay_seconds=None, message_id=None):
        requested_data = {'MessageBody': data}
    ...
    def send_message(self, queue_name, data, delay_seconds=None):
        ...
        try:
            ...
            self._logger.log_and_raise(sqs_exceptions.SendMessageClientException, queue_name=queue_name, data=data, exc=exc)
    ...
    def send_message_batch(self, queue_name, data_list, delay_seconds=None):
        ...
        while current_butch_message_index < len(data_list):
            ...
            try:
                ...
                self._logger.log_and_raise(sqs_exceptions.SendMessageClientException, queue_name=queue_name, data_messages=data_messages, exc=exc)
    ...
    def read_message(self, queue_name, delete_after_receive=True):
        ...
        try:
            ...
            try:
                sqs_message = sqs_message_wrapper.SQSMessageWrapper(message.message_id, message.receipt_handle, queue.url, message.body)
                ...
                sqs_message = sqs_message_wrapper.SQSMessageWrapper(message.message_id, message.receipt_handle, queue.url)
    ...
    def delete_messages(self, sqs_messages):
        try:
            for sqs_message in sqs_messages:
                message = self._sqs_resource.Message(sqs_message.queue_url, sqs_message.receipt_handle)
                message.delete()
