...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/txoids/ports/processors.py
# Compiled at: 2018-03-27 04:05:48
# Size of source mod 2**32: 10758 bytes
from txoids.utils import FullDict
from .oids import OIDS_MAP
from collections import OrderedDict
import re, binascii, logging, six

class PortsProcessor(object):
    main_oids = {'oper': '.1.3.6.1.2.1.2.2.1.8', 
    ...
    port_fields = ('equipment', 'index', 'speed', 'name', 'alias', 'lldp', 'oper',
                   'admin', 'copper_admin_duplex', 'copper_admin_speed', 'copper_admin_state',
                   'copper_flow_control', 'copper_mdix', 'copper_oper_duplex', 'copper_oper_speed',
                   'fiber_admin_duplex', 'fiber_admin_speed', 'fiber_admin_state',
                   'fiber_flow_control', 'fiber_mdix', 'fiber_oper_duplex', 'fiber_oper_speed')
    ...
    def convert_portname(cls, portname):
        ...
        """
        try:
            split_symbol = re.search('[/:]', portname).group()
                    ...
                    for medium_type in port['medium_types']:
                        for field, value in medium_type.items():
                            ...
                            if values and value not in values:
                                logging.warning("%s, %s has not string value for '%s' in field '%s'" % (
