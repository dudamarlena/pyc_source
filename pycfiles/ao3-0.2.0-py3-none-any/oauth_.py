...
# Embedded file name: build/bdist.linux-x86_64/egg/ao/social/oauth_.py
# Compiled at: 2010-04-14 11:39:16
import cgi
...
class OAuthClient(object):
    ...
    _default_config = {'request_token_url': '', 
       'access_token_url': '', 
       'authorize_url': '', 
    ...
    def _get_auth_token(self, callback_url):
        ...
        """
        result = self._extract_credentials(self._make_callback_request(callback_url))
        auth_token = result['token']
        auth_secret = result['secret']
        memcache.set(self._get_memcache_auth_key(auth_token), auth_secret, time=1200)
        return auth_token

    def _make_callback_request(self, callback_url):
        """Makes a generic OAuth request."""
        request = oauth.OAuthRequest.from_consumer_and_token(self._consumer, callback=callback_url, http_url=self._config['request_token_url'])
    ...
    def _make_verification_request(self, token, secret, verifier, params):
        ...
        request = oauth.OAuthRequest.from_consumer_and_token(self._consumer, token=token, verifier=verifier, http_url=self._config['access_token_url'])
    ...
    def _make_protected_request(self, token, secret, headers=False):
        ...
        token = oauth.OAuthToken(token, secret)
        request = oauth.OAuthRequest.from_consumer_and_token(self._consumer, token=token, http_url=url)
        request.sign_request(self._signature_method, self._consumer, token)
        if headers:
            return urlfetch.fetch(url, headers=request.to_header())
        return urlfetch.fetch(request.to_url(), headers={'Authorization': 'OAuth'})
    ...
    def get_user_info(self, auth_token, auth_verifier):
        ...
        response = self._make_verification_request(auth_token, auth_secret, auth_verifier, {'oauth_verifier': auth_verifier})
