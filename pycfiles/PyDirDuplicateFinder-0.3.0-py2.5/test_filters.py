...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pydirduplicatefinder/tests/test_filters.py
# Compiled at: 2009-08-15 11:16:32
...
class TestPathFilters(PyDirDuplicateFinderTestCase):
    ...
    def testDirectDirectoryNameSkippingFile1(self):
        ...
        self.addFile('a', d2)
        d2name = basename(d2)
        self.assertEquals(main(['--include-dir=%s' % d2name, d1, d2, d3]), self.wrapInNormalExecution([d2], []))
    ...
    def testDirectDirectoriesName(self):
        ...
        d1name = basename(d1)
        d2name = basename(d2)
        self.assertEquals(main(['--include-dir=%s' % d1name, '--include-dir=%s' % d2name, d1, d2, d3]), self.wrapInNormalExecution([d2, d1], []))
    ...
    def testDirectDirectoriesNameWithRecursion(self):
        ...
        b = self.addFile('a', d2)
        d1name = basename(d1)
        self.assertEquals(main(['-r', '--include-dir=%s' % d1name, d1]), self.wrapInNormalExecution([d1], []))
    ...
    def testDirectDirectoriesNameWithJollyChar(self):
        ...
        self.assertEquals(main(['--include-dir=di*', dira, dirb, cdir]), self.wrapInNormalExecution([dira, dirb], [
         interface_texts.FILE_IS_DUPLICATE % (b, a)]))
    ...
    def testIncludeEquality(self):
        ...
        self.assertEquals(main(['--include-dir=Carl', '--include-dir=Carlos', carl, carlos]), main(['--include-dir=Carl*', carl, carlos]))
    ...
    def testIncludeExcludeDirectoriesAtSametime(self):
        ...
        self.assertEquals(main(['-r', '--include-dir=Carl*', '--exclude-dir=Carles', carl, carlos]), self.wrapInNormalExecution([carl, carlos], [
         interface_texts.FILE_IS_DUPLICATE % (b, a)]))
        self.assertEquals(main(['-r', '--include-dir=Carl*', '--exclude-dir=Carles', carl, carlos, carlis]), self.wrapInNormalExecution([carl, carlis, carlos], [
    ...
    def testIncludeExcludeFiles(self):
        ...
        self.assertEquals(main(['--include-file=l?ris', '--exclude-file=li*', self.temp_dir]), self.wrapInNormalExecution([self.temp_dir], [
