...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pydirduplicatefinder/tests/test_functions.py
# Compiled at: 2009-06-20 11:12:15
...
class TestBasicOperations(PyDirDuplicateFinderTestCase):
    ...
    def testThatTooSmallDuplicatesAreIgnored(self):
        ...
        a = self.addFile('a', self.temp_dir, 200)
        b = self.addFile('b', self.temp_dir, like='a')
        self.assertEquals(main(['-ra', 'print', '--min-size=400', self.temp_dir]), self.wrapInNormalExecution([self.temp_dir], []))
