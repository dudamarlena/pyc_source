...
# [GCC 8.4.0]
# Embedded file name: /Users/joe/devel/covviz/covviz/covviz.py
# Compiled at: 2019-08-30 19:03:10
...
def parse_args():
    ...
    p.add_argument('bed',
      help='bed3+ file format with a header defining sample columns after chrom, start, and end.')
    p.add_argument('-e',
      '--exclude',
    ...
    p.add_argument('-o',
      '--output', default='covviz_report.html', help='output file path')
    p.add_argument('--skip-norm',
      action='store_true',
      help='skip normalization by global sample median if the depths in your .bed are already normalized')
    p.add_argument('--min-samples',
      default=8,
      type=int,
    ...
    meta_group.add_argument('-p',
      '--ped', help='ped file defining samples, sex, and other metadata')
    meta_group.add_argument('--sample-col',
      default='sample_id',
      help='when using --ped, this defines the sample ID column')
    meta_group.add_argument('--sex-col',
      default='sex', help='when using --ped, this defines the sex column')
    meta_group.add_argument('--sex-vals',
      default='1,2',
      help='when using --ped, this defines male,female encoding')
    annotations_group = p.add_argument_group('annotations')
    annotations_group.add_argument('--bed',
      dest='bed_track',
      action='append',
      help='.bed reference file to place an optional region track; the name field is displayed on hover and overlapping regions will be merged.')
    annotations_group.add_argument('--gff',
      action='append',
      help=".gff reference file to place an optional gene track; only rows of type 'gene' are used and annotated with gene ID where the attributes include 'Name=<symbol>;' annotation")
    annotations_group.add_argument('--gff-feature',
      default='gene',
      help='feature type within GFF upon which to search for attribute')
    annotations_group.add_argument('--gff-attr',
      default='Name=',
      help='the regex search string to grab the relevant portion of the attributes')
    annotations_group.add_argument('--vcf',
      action='append',
      help='.vcf file to place an optional variant track; info is displayed by default, but can be broken up using --vcf-info regex')
    annotations_group.add_argument('--vcf-info',
      default=None,
      help="the regex search string to grab the relevant portion of the INFO field, e.g. 'CLNDN=' for the clinical diagnosis field in ClinVar")
    return p.parse_args()
...
def cli():
    ...
    traces, samples = parse_bed(args.bed, exclude, args.ped, args.sample_col, args.sex_col, args.sex_chroms, args.z_threshold, args.distance_threshold, args.slop, args.min_samples, args.skip_norm)
    ...
    if args.gff:
        for gff in args.gff:
            ...
            traces = parse_gff(gff,
              traces,
              exclude,
              ftype=(args.gff_feature),
              regex=(args.gff_attr),
              y_offset=y_offset,
              track_color=(annotation_track_colors[(track_id % len(annotation_track_colors))]))
            y_offset += -0.15
            track_id += 1
    ...
    if args.ped:
        ...
        traces = parse_ped(args.ped, traces, args.sample_col, args.sex_chroms, args.sex_vals)
