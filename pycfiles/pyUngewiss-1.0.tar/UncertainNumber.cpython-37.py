...
# [GCC 8.4.0]
# Embedded file name: /home/wehrle/opt/pyUngewiss/pyUngewiss/UncertainNumber.py
# Compiled at: 2019-08-28 11:33:56
...
class UncertainNumber(object):
            ...
            if self.Form == 'triangle':
                ...
                if self.Form == 'trapazoid':
                    ...
                    if self.Form == 'gauss-cuttoff':
                        ...
                        A[:, 0] = np.flip(mean - np.sqrt(-2 * sigmaleft ** 2 * np.log(alpha)))
                        A[:, 1] = np.flip(mean + np.sqrt(-2 * sigmaleft ** 2 * np.log(alpha)))
                        A[A < mean - sigmatrunc * sigmaleft] = mean - sigmatrunc * sigmaleft
                        A[A > mean + sigmatrunc * sigmaright] = mean + sigmatrunc * sigmaright
                        self.Value = A
                    else:
                        if self.Form == 'empirical':
                            self.Value = values
                        else:
    ...
    def calcArea(self):
        ...
        if self.Area == 0.0:
            ...
            xValNorm = [
             self.ValueNorm[:, 0], self.ValueNorm[:, 1]]
    ...
    def plotValue(self, xlabel=[], ylabel=[], pdpi=100, fill=True, fontsize=10, xsize=2.5, ysize=2, xlimits=[], lang='EN', TextRender=True, AxisNameLong=True, frame=False, nYTicks=2, nXTicks=5, xAxisRot=True, trans=1.0, font='tex gyre pagella', Units=[], plotBuffer=0.1, color=DefaultColor, delta=0.5, padLabel=[]):
        plt.rcParams['font.family'] = font
        rcParams.update({'figure.autolayout': True})
        rcParams.update(pgf_with_pdflatex)
        if not TextRender:
            rcParams.update({'svg.fonttype': 'none'})
        elif self.Form == 'interval' or self.nalpha == 1:
            xsize = 5
            ysize = 0.25
            ...
            plt.xlim(np.min(self.Value) - (np.max(self.Value) - np.min(self.Value)) / 5, np.max(self.Value) + (np.max(self.Value) - np.min(self.Value)) / 5)
        else:
            fig = plt.figure(figsize=(xsize, ysize), dpi=pdpi)
            ax = fig.add_subplot(1, 1, 1)
            mu = np.linspace(1, 0, np.size(self.Value, 0))
            xplot = np.concatenate((np.flipud(self.Value[:, 0]),
             self.Value[:, 1]),
              axis=0)
            muplot = np.concatenate((np.flipud(mu), mu), axis=0)
            if fill:
                ax.fill_between((self.Value[:, 0]), mu, facecolor=color, alpha=trans,
                  linewidth=0.0)
                ax.fill_between([self.Value[(0, 1)], self.Value[(0, 0)]], [1, 1], facecolor=color,
                  alpha=trans,
                  linewidth=0.0)
                ax.fill_between((self.Value[:, 1]), mu, facecolor=color, alpha=trans,
                  linewidth=0.0)
...
def plotIntervals(IntervalList, labels=[], Units=[], show=False, xPlot=5, color=DefaultColor, delta=0.5, plotBuffer=0.1, xAxisRot=True, xlabel='Uncertain value', font='tex gyre pagella', padLabel=[]):
    if type(IntervalList) is list:
        ...
        for i, val in enumerate(IntervalList):
            if type(val) == np.ndarray:
                ...
                if type(val) == UncertainNumber:
                    data[i] = val.Value

    else:
        if type(IntervalList) == UncertainNumber:
...
def plotUncertainFn(IntervalList, x=[], xlabel='Dependent parameter', ylabel='Value of uncertain function', color=DefaultColor, fontsize=12, font='tex gyre pagella', pdpi=100, xlimits=[], ylimits=[], xsize=7, ysize=5, outline=False, fill=True, nYTicks=5, nXTicks=5, xAxisRot=True, frame=True):
    if type(IntervalList) is list:
    ...
    if fill:
        for ii in reversed(range(np.size(rFuzz, 1) - 1)):
            ax1.fill_between(x, (rFuzz[:, ii + 1, 1]), (rFuzz[:, ii, 1]), facecolor=color,
              alpha=(1.0 / nalpha * (nalpha - ii - 1) * 0.9),
              linewidth=0,
              edgecolor=DefaultEdgeColor)
            ax1.fill_between(x, (rFuzz[:, ii + 1, 0]), (rFuzz[:, ii, 0]), facecolor=color,
              alpha=(1.0 / nalpha * (nalpha - ii - 1) * 0.9),
              linewidth=0,
              edgecolor=DefaultEdgeColor)

        ax1.fill_between(x, (rFuzz[:, 0, 1]), (rFuzz[:, 0, 0]), facecolor=color, alpha=1.0,
          linewidth=0)
    if outline:
        ax1.plot(x, (rFuzz[:, :, 0]), x, (rFuzz[:, :, 1]), color=color, linewidth=1.0)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    if ylimits != []:
...
if __name__ == '__main__':
    ...
    plotIntervals([AInt.Value, AInt.Value / 10])
    print()
