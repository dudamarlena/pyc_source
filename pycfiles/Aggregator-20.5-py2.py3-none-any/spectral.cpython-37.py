...
# Embedded file name: build\bdist.win-amd64\egg\aggregate\spectral.py
# Compiled at: 2019-12-25 20:43:46
# Size of source mod 2**32: 14084 bytes
...
class Distortion(object):
    """Distortion"""
    _available_distortions_ = ('ph', 'wang', 'tt', 'cll', 'lep', 'ly', 'clin', 'dual',
                               'tvar', 'convex')
    _has_mass_ = ('ly', 'clin', 'lep')
    _long_names_ = ('Proportional Hazard', 'Wang-normal', 'Wang-tt', 'Capped Loglinear',
                    'Layer Equivalent Pricing', 'Linear Yield', 'Capped Linear',
                    'Dual Moment', 'Tail VaR', 'Convex Envelope')
    _eg_param_1_ = (0.9, 1, 1, 0.9, 0.25, 0.9, 1.1, 3, 0.75)
    _eg_param_2_ = (0.5, 2, 2, 0.8, 0.35, 1.5, 1.8, 6, 0.95)
    ...
    def __init__(self, name, shape, r0=0.0, df=None, col_x='', col_y=''):
        ...
        if self.name == 'ph':
            ...
            g = interp1d((df.iloc[(knots, df.columns.get_loc(col_x))]), (df.iloc[(knots, df.columns.get_loc(col_y))]),
              kind='linear')
            g_inv = interp1d((df.iloc[(knots, df.columns.get_loc(col_y))]), (df.iloc[(knots, df.columns.get_loc(col_x))]),
              kind='linear')
        else:
            raise ValueError('Incorrect spec passed to Distortion; implemented g types are ph, wang, tvar, ly (linear yield), lep (layer equivalent pricing) and clin (clipped linear)')
        self.g = g
        self.g_inv = g_inv
    ...
    def test(cls, r0=0.05):
        ...
        :return:
        """
        axiter = axiter_factory(None, 18, figsize=(10, 8.0))
