...
# [GCC 8.4.0]
# Embedded file name: /yuneta/development/yuneta/^yuneta/web-skeleton3/web_skeleton3/main.py
# Compiled at: 2018-05-01 06:53:27
# Size of source mod 2**32: 15011 bytes
"""
Utility for creating static html code.
"""
import traceback, os, argparse, sys, stat, json, shutil, datetime, os.path
...
comandos = [
 'init', 'skeleton', 'render', 'link', 'rsync', 'version']
try:
...
class WebSkeleton(object):
    ...
    parser.add_argument('command', choices=comandos, help="Available commands (WARNING: to be executed in the web-skeleton3.ini directory):\ninit {project} ==> create a .ini file and the directory structure.\nskeleton ==> create a new tag directory, copying a assets directory.\nrender ==> generate a new index.html, in tags/{version} directory.\nlink {path} ==> link file from 'code' directory to 'tag' directory.\nrsync ==> syncronize the tag version with the remote host.\n\nOnce a skeleton has been created:\n   - Create new js/css/scss files in `code` directory (for example with yuno-skeleton)\n       - Use `web-skeleton3 link` command to link new files from 'code/?' to 'tag' directory\n       - Update the `code/config.json` file with the new links in 'tag' directory\n   - Render (web-skeleton render)\n\n")
    ...
    def render(self, **kw):
        ...
        assets_env = self.get_assets_env(code_path, output_path)
    ...
    def get_assets_env(self, code_path, output_path):
        ...
        assets_env.config['compass_config'] = {'additional_import_paths': [
        ...
        if len(css_list) or len(scss_list):
            ...
            for scss_file in scss_list:
                ...
                try:
                    xxx = Bundle(scss_file, filters='compass', output=scss_file + '.css')
                    scss_bundle.append(xxx)
                except Exception as error:
                    print("ERROR processing filename '%s': %s" % (scss_file, error))

            css = Bundle(css_bundle, *scss_bundle, output='packed.css')
        ...
        if 'top_js' in self.config.data:
            ...
            if top_js_list and len(top_js_list):
                top_js = Bundle(*top_js_list, output='top.js')
        ...
        if 'bottom_js' in self.config.data:
            ...
            if bottom_js_list and len(bottom_js_list):
                bottom_js = Bundle(*self.config.data['bottom_js'], output='bottom.js')
