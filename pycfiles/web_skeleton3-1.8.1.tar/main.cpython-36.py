...
# Compiled at: 2020-01-22 05:08:41
# Size of source mod 2**32: 16836 bytes
"""
...
comandos = [
 'init', 'skeleton', 'render', 'rsync', 'version']
try:
...
class WebSkeleton(object):
    ...
    def render(self, **kw):
        ...
        assets_bottom_js_env = self.get_assets_bottom_js_env(code_path, output_path)
    ...
    def get_assets_css_env(self, code_path, output_path):
        ...
        if len(css_list) or len(scss_list):
            ...
            css = Bundle(
 css_bundle, *scss_bundle, **{'filters':'cssutils,cssrewrite', 
             'output':'packed.css'})
    ...
    def get_assets_top_js_env(self, code_path, output_path):
        ...
        if 'top_js' in self.config.data:
            ...
            if top_js_list:
                if len(top_js_list):
                    top_js = Bundle(*top_js_list, filters='rjsmin', 
                     output='top.js')
    ...
    def get_assets_bottom_js_env(self, code_path, output_path):
        ...
        if 'bottom_js' in self.config.data:
            ...
            if bottom_js_list:
                if len(bottom_js_list):
                    bottom_js = Bundle(*bottom_js_list, filters='rjsmin', 
                     output='bottom.js')
