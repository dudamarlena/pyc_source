...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/easycluster/__init__.py
...
import sys, os, os.path, re, signal, socket, select, traceback, struct, weakref, time, hmac, threading, errno, zlib, subprocess
...
if PYTHON3:
    ...
    import cPickle as pickle, Queue as queue, __builtin__ as builtins
    _hexenc = b2a_hex
    _hexdec = a2b_hex
    _str_to_bytes = str
    __metaclass__ = type
...
def make_server_class(typ, export_methods=(
 '@auto',), export_attrs=('@auto',), export_attrs_cache=(), proxy_class=None):
    ...
    """
    typedat = _make_typedat(typ, export_methods, export_attrs, export_attrs_cache, proxy_class)
    try:
...
def make_singleton(obj, export_methods=('@auto',), export_attrs=('@auto',), export_attrs_cache=(), proxy_class=None):
    ...
    typedat = _make_typedat(obj.__class__, export_methods, export_attrs, export_attrs_cache, proxy_class)
...
class RemoteProxy(_RemoteProxyBase):
    ...
    def raw_call_method(self, method, args, kwargs={}, nonblocking=False, oncomplete=None, onerror=None, threadid=None, origexc=True):
        ...
        """
        return self._connection.send_request(self._oid, method, args, kwargs, nonblocking, oncomplete, onerror, threadid, origexc)

    def raw_get_attribute(self, attr, nonblocking=False, oncomplete=None, onerror=None, threadid=None, origexc=True):
        ...
        """
        return self._connection.send_request(0, 'getattr', (self, attr), {}, nonblocking, oncomplete, onerror, threadid, origexc)

    def raw_set_attribute(self, attr, val, nonblocking=False, oncomplete=None, onerror=None, threadid=None, origexc=True):
        ...
        """
        return self._connection.send_request(0, 'setattr', (self, attr, val), {}, nonblocking, oncomplete, onerror, threadid, origexc)
...
class Connection():
    ...
    def reconnect(self):
        ...
        self.close()
        self.connect(self._remote_host, self._remote_port, self._peername, self._ssh_user, self._ssh, self._ssh_rpython, self._ssh_extra_args)
        return
    ...
    def send_message(self, typ, data, delay_hmac=False):
        ...
        with self._sendlock:
            if self._enable_compression and self._peer_enable_compression and typ in _pickle_messages and len(data) >= 32:
                typ |= MSG_COMPRESSED
                c = self._compress
            ...
            if hmac:
                ...
                if prev_hmac:
                    if typ == ERROR:
                        prev_hmac = '\x00' * len(prev_hmac)
                    self._delay_digest = ''
            if delay_hmac:
                data = ('').join((prev_hmac, hdr, data))
                self._delay_digest = digest
            else:
                data = ('').join((prev_hmac, hdr, data, digest))
        ...
        while True:
            ...
            if newlen > 0:
                if newlen > 256 and not self._verified:
                    self.send_message(ERROR, 'Initial message too long')
                    self.close()
            ...
            if recv_hmac and data != expect_hmac:
                self.send_message(ERROR, 'HMAC error')
                continue
            ...
            if rmsg in _pickle_messages:
                if not self._verified:
                    self.send_message(ERROR, 'Not verified')
                    continue
                ...
                if rdata == self._my_nonce:
                    ...
                    self.send_message(ERROR, 'Invalid nonce')
            else:
                self.send_message(ERROR, 'Invalid message type')
...
class Client(Connection):
    ...
    def __init__(self, key=None, host=None, port=None, definitions='easycluster.remote_code', enable_compression=False, ssh=None, user=None, rpython=None, extra_args=()):
    ...
    def _check_version(self, pv):
        if _version_compare(pv, VERSION) != 0:
            if _version_compare(pv, '0.09') < 0:
                raise ValueError('Incompatible EasyCluster version: client has version %s, server has version %s; client requires server to have at least version %s' % (
    ...
    def _after_connect(self):
        ...
        if self._defmod._source_code:
            self.update_definitions(oncomplete=self._on_update_complete, onerror=self._init_error)
    ...
    def send_request(self, oid, meth, args, kwargs={}, nonblocking=False, oncomplete=None, onerror=None, threadid=None, origexc=True):
    ...
    def update_definitions(self, nonblocking=False, oncomplete=None, onerror=None, threadid=None, origexc=True):
        ...
        rv = self.send_request(0, 'update_definitions', (source[current_count:],), {}, nonblocking, oncomplete, onerror, threadid, origexc)
        self._definition_count = len(source)
        return rv

    def raw_call(self, func, args, kwargs={}, nonblocking=False, oncomplete=None, onerror=None, threadid=None, origexc=True):
        ...
        """
        return self.send_request(0, 'call', (func, args, kwargs), {}, nonblocking, oncomplete, onerror, threadid, origexc)
    ...
    def eval(self, expr, nonblocking=False, oncomplete=None, onerror=None, threadid=None, origexc=True):
        ...
        """
        return self.send_request(0, 'eval', (expr, 'eval'), {}, nonblocking, oncomplete, onerror, threadid, origexc)
    ...
    def execblock(self, code, nonblocking=False, oncomplete=None, onerror=None, threadid=None, origexc=True):
        ...
        """
        return self.send_request(0, 'eval', (code, 'exec'), {}, nonblocking, oncomplete, onerror, threadid, origexc)
    ...
    def import_modules(self, names, nonblocking=False, oncomplete=None, onerror=None, threadid=None, origexc=True):
        ...
        """
        return self.send_request(0, 'import_modules', (names,), {}, nonblocking, oncomplete, onerror, threadid, origexc)
    ...
    def import_module(self, name, nonblocking=False, oncomplete=None, onerror=None, threadid=None, origexc=True):
        ...
        return self.send_request(0, 'import_modules', ([name],), {}, nonblocking, oncomplete, onerror, threadid, origexc)
        ...
        if len(l) > 1:
            ...
            if proxclas is None and len(l) > 2:
                ...
                if typedat[0] == ':':
                    ...
                    typedict = {'__module__': __name__, 'proxy_methods': export_methods, 
...
class ThreadedClient(Client):
    ...
    def __init__(self, key=None, host=None, port=None, definitions='easycluster.remote_code', enable_compression=False, ssh=False, user=None, rpython=None, extra_args=(), keep_alive_interval=10, keep_alive_timeout=60):
        ...
        self._wrthread = None
        Client.__init__(self, key, host, port, definitions, enable_compression, ssh, user, rpython, extra_args)
...
def definitions_module(name, keep_source=True):
    ...
    if len(components) > 1:
        ...
        try:
            ...
            raise ImportError('Cannot import parent package %r: Definitions module must be top-level, or in an existing package' % parent_package_name)
...
def add_key_options(options, key_option='-K', keyfile_option='-k'):
    ...
    options.add_option(key_option, '--key', help='HMAC key to use; either a %d-character hex string or a ASCII string which will be padded. This method of specifying a key is insecure because other users on the system can read it.' % (2 * HMAC_SIZE))
    options.add_option(keyfile_option, '--keyfile', metavar='FILE', help='Read HMAC key from file. This is the preferred method of specifying a key.')
...
def parse_connection_spec(spec, default_port=None, default_key='', default_enable_compression=False, custom_options=(), warn=True):
    """
    ...
    params = {'host': host, 'port': port, 'key': default_key, 'ssh': ssh, 'user': user, 
...
def _get_typedat(obj, typ):
    ...
    if type(typedat) == _TypeData:
        if typedat == '':
            typedat = _make_typedat(typ, typ.export_methods, typ.export_attrs, typ.export_attrs_cache, typ.proxy_class)
            typ._easycluster_server_object_typedat = typedat
        return typedat
...
def _make_typedat(typ, export_methods, export_attrs, export_attrs_cache, proxy_class):
    ...
    if proxy_class:
        ...
        if '@auto' in export_methods:
            ...
            functypes = (
             FunctionType, BuiltinFunctionType, staticmethod,
             classmethod, type(FunctionType.__dict__['__call__']))
            export_methods.update(name for name, val in attrs.items() if name not in _forbidden_slots and isinstance(val, functypes))
...
if sys.platform.startswith('win'):
    ...
    def _write_key_file(path, key):
        ...
        import win32api, win32file, win32security, ntsecuritycon, win32con, pywintypes
        ...
        sa.bInheritHandle = 0
        try:
            fil = win32file.CreateFile(path, win32con.GENERIC_WRITE, 0, sa, win32con.CREATE_NEW, 0, 0)
