...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/easycluster/server.py
...
import os, traceback, time, socket, errno, threading, select, easycluster as _core
...
_method_types = (
 BuiltinMethodType, MethodType, FunctionType, type(_core.set_global.__call__))
...
class Server(_core.Connection):
    ...
    def __init__(self, key, sock, peername, verbosity=0, repl=None):
    ...
    def _check_version(self, pv):
        if _core._version_compare(pv, MIN_CLIENT_VERSION) < 0:
            raise ValueError('Incompatible EasyCluster version: client has version %s, server has version %s; server requires client to have at least version %s' % (
             pv, VERSION, MIN_CLIENT_VERSION))
            ...
            while True:
                if self._keep_alive_interval:
                    ...
                    if rtime <= 0:
                        ...
                        self.send_message(_core.KEEPALIVE, '')
                        continue
...
class IPAddress(object):
    ...
    def __init__(self, txt):
        ...
        if ':' in addr:
            ...
            if '.' in components[(-1)]:
                ...
                components.append('%04x' % (v4part & 65535))
            ...
            if val & -4294967296 == 281470681743360:
                val &= 4294967295
        ...
        if maskbits is None:
            ...
            raise ValueError('Invalid value for mask length %r in address %r: must be between 0 and %d' % (
             maskbits, txt, numbits))
    ...
    def in_network(self, net):
        return self.family == net.family and self.aslong & net.mask == net.aslong
...
def spawn_local(clientclass=_core.Client, server_verbosity=0):
    ...
    spawn_runner_process(ssock, 'local:%d' % _local_id, '', server_verbosity, (csock,))
    ssock.close()
    rc = clientclass('')
...
def server_main(args=None):
    ...
    if sys.platform.startswith('win'):
        ...
        options.add_option('-a', '--allow', action='append', default=[], help='Allow only a specific address or subnet to connect (e.g. 192.168.0.0/24). Specify more than once to allow multiple addresses.')
        options.add_option('-c', '--class', dest='svrclass', default=None, help=SUPPRESS_HELP)
        if _daemonize:
            options.add_option('-d', '--daemonize', action='store_true', help='Detach from current terminal and run in the background')
            options.add_option('-P', '--pidfile', action='store', dest='pidfile', metavar='FILE', help='write daemon process ID to a file')
        if servmod:
            options.add_option('-i', '--install', action='store_true', help='Install EasyCluster as a service. The keyfile for this service is located at %s. If a keyfile is specified, it will be copied there; if no key exists currently, or if -O is given, a new key file will be written.' % servmod.KEY_PATH)
            options.add_option('-u', '--uninstall', action='store_true', help='Uninstall the service.')
            ...
            options.add_option('--query-installed', action='store_true', help='Exits with code 0 if the service is installed, or code 1 if it is not.')
        ...
        if opts.generate:
            _core.write_key_file(opts.generate, _core.generate_key(), opts.overwrite) or print('Key file already exists (use --overwrite to overwrite it)', file=sys.stderr)
        return
    if opts.serve:
        key = _core.key_from_options(opts)
        if key == '':
            print('SECURITY WARNING: running with blank key.', file=sys.stderr)
        run_server(opts.port, key, opts.bind, opts.allow, opts.verbose, getattr(opts, 'daemonize', False), getattr(opts, 'pidfile', None), opts.no_ipv6)
    else:
        options.print_help()
    return
...
def run_server(port, key, bindaddr=None, allow=(), verbosity=0, daemonize=False, pidfile=None, disable_ipv6=False):
    ...
    while _server_socket:
        ...
        try:
            try:
                ...
                spawn_runner_process(csock, _core._get_peer_name(addr), key, verbosity, (sock,))
            except Exception:
                traceback.print_exc()
...
if sys.platform.startswith('win'):
    ...
    def spawn_runner_process(csock, peername, key, verbosity, close=()):
        args = [
         _python_exe, '-m', 'easycluster.__main__', MAGIC_ARG]
        p = subprocess.Popen(args, stdin=subprocess.PIPE)
        pinfo = dupsock(csock, p.pid)
        p.stdin.write(_core.b2a_hex(_core.pickle.dumps((pinfo, csock.family, csock.type,
         csock.proto, peername, key, verbosity), protocol=_core.pickle.HIGHEST_PROTOCOL)) + '\r\n')
        p.stdin.close()
    ...
    def server_check_runner():
        if len(sys.argv) == 2 and sys.argv[1] == MAGIC_ARG:
            ...
            try:
                svr = Server(key, sock, peername, verbosity)
                while svr:
                    svr = svr.run()
    ...
    def spawn_runner_process(csock, peername, key, verbosity, close=()):
        ...
        if cpid == 0:
            ...
            try:
                svr = Server(key, csock, peername, verbosity)
                while svr:
                    svr = svr.run()
