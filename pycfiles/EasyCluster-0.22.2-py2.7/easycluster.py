...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/easycluster.py
...
import sys, os, signal, socket, select, traceback, struct, weakref, time, hmac, threading, errno
...
__all__ = [
 'AutoThreadingClient', 'Client', 'ClientGroup', 'Connection',
...
def make_server_class(typ, export_methods=(), export_attrs=(), proxy_class=None):
    ...
    try:
        ...
        _extra_base_classes[typ] = (
         export_methods, export_attrs, proxy_class)
...
class RemoteProxy(RemoteProxyBase):
    ...
    def raw_call_method(self, method, args, kwargs={}, nonblocking=False, oncomplete=None, onerror=None, threadid=None):
        ...
        """
        return self._connection.send_request(self._oid, method, args, kwargs, nonblocking, oncomplete, onerror, threadid)
    ...
    def send_message(self, typ, data):
        ...
        with self._sendlock:
            ...
            if typ == ERROR:
                ...
                digest = hmac.digest()
                hmac.update(digest)
            _sock_sendall(self._sock, ('').join((hdr, data, digest)))
...
class Client(Connection):
    ...
    def __init__(self, key, sock_or_host=None, port=DEFAULT_PORT, definitions='easycluster_code'):
    ...
    def _after_connect(self):
        ...
        if self._defmod._source_code:
            self.update_definitions(oncomplete=lambda val: None, onerror=self._init_error)
        return
    ...
    def send_request(self, oid, meth, args, kwargs={}, nonblocking=False, oncomplete=None, onerror=None, threadid=None):
    ...
    def update_definitions(self, nonblocking=False, oncomplete=None, onerror=None, threadid=None):
        ...
        rv = self.send_request(0, 'update_definitions', (source[current_count:],), {}, nonblocking, oncomplete, onerror, threadid)
        self._definition_count = len(source)
        return rv

    def raw_call(self, func, args, kwargs={}, nonblocking=False, oncomplete=None, onerror=None, threadid=None):
        ...
        'oncomplete', 'onerror', and 'threadid' arguments. 
        """
        return self.send_request(0, 'call', (func, args, kwargs), {}, nonblocking, oncomplete, onerror, threadid)
    ...
    def eval(self, expr, nonblocking=False, oncomplete=None, onerror=None, threadid=None):
        ...
        'oncomplete', 'onerror', and 'threadid' arguments. 
        """
        return self.send_request(0, 'eval', (expr, 'eval'), {}, nonblocking, oncomplete, onerror, threadid)
    ...
    def execblock(self, code, nonblocking=False, oncomplete=None, onerror=None, threadid=None):
        ...
        'oncomplete', 'onerror', and 'threadid' arguments. 
        """
        return self.send_request(0, 'eval', (code, 'exec'), {}, nonblocking, oncomplete, onerror, threadid)
    ...
    def import_modules(self, names, nonblocking=False, oncomplete=None, onerror=None, threadid=None):
        ...
        """
        return self.send_request(0, 'import_modules', (names,), {}, nonblocking, oncomplete, onerror, threadid)
    ...
    def import_module(self, name, nonblocking=False, oncomplete=None, onerror=None, threadid=None):
        ...
        return self.send_request(0, 'import_modules', ([name],), {}, nonblocking, oncomplete, onerror, threadid)
...
class AutoThreadingClient(Client, threading.Thread):
    ...
    def __init__(self, key, sock_or_host=None, port=DEFAULT_PORT, definitions='easycluster_code', keep_alive_interval=10, keep_alive_timeout=60):
...
class Server(Connection):
    ...
    def _persistent_id(self, obj):
        ...
        if oid is not None:
            ...
            if tid is not None:
                return '%d\n%d' % (oid, tid)
            if not (proxy_class or export_methods or export_attrs):
                return str(oid)
...
def server_main(args=None):
    ...
    options.add_option('-g', '--generate', metavar='FILE', help='Generate random HMAC key and write it to FILE. The file will be created with read access only for the owner.')
...
def spawn_local(clientclass=Client, svrclass=QuietServer):
    ...
    spawn_runner_process(ssock, 'local:%d' % _local_id, svrclass, '')
    ssock.close()
    rc = clientclass('')
...
def _add_extra_classes():
    _file_export = (
     (), (), FileProxy)
    _extra_base_classes[GeneratorType] = (
     (), (), GeneratorProxy)
    ...
    _extra_base_classes[ModuleType] = (
     (), (), None)
    return
...
if os.name == 'nt':
    from ctypes import windll, Structure, Union, c_size_t, c_int, py_object, POINTER, pythonapi, create_string_buffer, WinError
    import subprocess
    ...
    def spawn_runner_process(csock, peername, svrclass, key):
        ...
        p.stdin.write(b2a_hex(pickle.dumps((pinfo, csock.family, csock.type, csock.proto, peername, svrclass, key), protocol=pickle.HIGHEST_PROTOCOL)) + '\r\n')
        p.stdin.close()
