...
# [GCC 8.4.0]
# Embedded file name: /home/manish/projects/outernet/squery-pg/squery_pg/migrations.py
# Compiled at: 2016-06-15 06:55:21
"""
...
"""
import importlib, logging, os, re, sys, psycopg2, sqlize_pg
PYMOD_RE = re.compile('^((\\d{2})_(\\d{2})_[^.]+)\\.pyc?$', re.I)
VERSION_MULTIPLIER = 10000
MIGRATION_TABLE = 'migrations'
GET_VERSION_SQL = sqlize_pg.Select(what='version', sets=MIGRATION_TABLE, where='name = %(name)s')
SET_VERSION_SQL = sqlize_pg.Replace(table=MIGRATION_TABLE, constraints=('name', ), cols=('name',
                                                                                         'version'))
...
def get_new(modules, min_major_version, min_minor_version):
    ...
    for mod_data in modules:
        ...
        if mod_major_version > min_major_version or mod_major_version == min_major_version and mod_minor_version >= min_minor_version:
            yield mod_data
