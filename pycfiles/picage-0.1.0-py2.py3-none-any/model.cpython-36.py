...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/picage-project/picage/model.py
# Compiled at: 2019-10-01 11:14:29
...
class Package(BaseModuleOrPackage):
    ...
    def __init__(self, name, path=None, parent=None, is_single_file=None):
        super(Package, self).__init__(name,
          path=path, parent=parent, is_single_file=is_single_file)
            ...
            for p in Path.sort_by_abspath(self.path.iterdir()):
                if p.is_dir():
                    if Path(p, '__init__.py').exists():
                        pkg = Package(name=(name + '.' + p.basename),
                          path=p,
                          parent=self,
                    ...
                    if p.ext == '.py':
                        if p.fname != '__init__':
                            module = Module(name=(name + '.' + p.fname),
                              path=p,
                              parent=self,
                              is_single_file=True)
