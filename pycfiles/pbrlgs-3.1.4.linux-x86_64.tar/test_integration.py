...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/pbr/tests/test_integration.py
# Compiled at: 2017-12-04 07:19:32
import os.path, shlex, sys, fixtures, testtools, textwrap
...
def all_projects():
    ...
    for name in PROJECTS:
        ...
        if short_name in excludes:
            continue
        yield (
         short_name, dict(name=name, short_name=short_name))
...
class TestIntegration(base.BaseTestCase):
    ...
    def test_integration(self):
        self.useFixture(base.CapturedSubprocess('sync-req', [
         'python', 'update.py', os.path.join(REPODIR, self.short_name)], cwd=os.path.join(REPODIR, 'requirements')))
        self.useFixture(base.CapturedSubprocess('commit-requirements', 'git diff --quiet || git commit -amrequirements', cwd=os.path.join(REPODIR, self.short_name), shell=True))
        path = os.path.join(self.useFixture(fixtures.TempDir()).path, 'project')
        self.useFixture(base.CapturedSubprocess('clone', [
         'git', 'clone', os.path.join(REPODIR, self.short_name), path]))
        venv = self.useFixture(test_packaging.Venv('sdist', modules=[
         'pip', 'wheel', PBRVERSION], pip_cmd=PIP_CMD))
        python = venv.python
        self.useFixture(base.CapturedSubprocess('sdist', [python, 'setup.py', 'sdist'], cwd=path))
        venv = self.useFixture(test_packaging.Venv('tarball', modules=[
         'pip', 'wheel', PBRVERSION], pip_cmd=PIP_CMD))
        python = venv.python
        ...
        venv = self.useFixture(test_packaging.Venv('install-git', modules=[
         'pip', 'wheel', PBRVERSION], pip_cmd=PIP_CMD))
        root = venv.path
        python = venv.python
        ...
        venv = self.useFixture(test_packaging.Venv('install-e', modules=[
         'pip', 'wheel', PBRVERSION], pip_cmd=PIP_CMD))
        root = venv.path
        python = venv.python
...
class TestInstallWithoutPbr(base.BaseTestCase):
    ...
    def test_install_without_pbr(self):
        ...
        pkgs = {'pkgTest': {'setup.py': textwrap.dedent("                    #!/usr/bin/env python\n                    import setuptools\n                    setuptools.setup(\n                        name = 'pkgTest',\n                        tests_require = ['pkgReq'],\n                        test_suite='pkgReq'\n                    )\n                "), 
           ...
           'pkgReq': {'requirements.txt': textwrap.dedent('                    pbr\n                '), 
    ...
    def test_pip_versions(self):
        ...
        for module in self.modules:
            self._run_cmd(bin_python, [
             '-m', 'pip', 'install', '--upgrade', module], cwd=venv.path, allow_fail=False)
        ...
        self._run_cmd(bin_python, [
         ...
         'test_markers'], cwd=venv.path, allow_fail=False)
...
class TestLTSSupport(base.BaseTestCase):
    scenarios = [
     (
      'EL7',
      {'modules': ['pip==1.4.1', 'setuptools==0.9.8'], 'py3support': True}),
     (
      'Trusty',
      {'modules': ['pip==1.5', 'setuptools==2.2'], 'py3support': True}),
     (
      'Jessie',
      {'modules': ['pip==1.5.6', 'setuptools==5.5.1'], 'py3support': True}),
     (
      'WheezyPrecise',
      {'modules': ['pip==1.4.1', 'setuptools==0.6c11'], 'py3support': False})]

    @testtools.skipUnless(os.environ.get('PBR_INTEGRATION', None) == '1', 'integration tests not enabled')
    def test_lts_venv_default_versions(self):
        if sys.version_info[0] == 3 and not self.py3support:
            self.skipTest('This combination will not install with py3, skipping test')
