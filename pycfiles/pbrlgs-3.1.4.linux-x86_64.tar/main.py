...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/pbr/cmd/main.py
# Compiled at: 2017-12-04 07:19:32
import argparse, json, sys, pkg_resources, pbr.version

def _get_metadata(package_name):
    try:
        return json.loads(pkg_resources.get_distribution(package_name).get_metadata('pbr.json'))
...
def get_info(args):
    print ('{name}\t{version}\t{released}\t{sha}').format(**_get_info(args.name))
...
def freeze(args):
    sorted_dists = sorted(pkg_resources.working_set, key=lambda dist: dist.project_name.lower())
...
def main():
    parser = argparse.ArgumentParser(description='pbr: Python Build Reasonableness')
    parser.add_argument('-v', '--version', action='version', version=str(pbr.version.VersionInfo('pbr')))
    ...
    cmd_info = subparsers.add_parser('info', help='print version info for package')
    cmd_info.set_defaults(func=get_info)
    cmd_info.add_argument('name', help='package to print info of')
    cmd_freeze = subparsers.add_parser('freeze', help='print version info for all installed packages')
