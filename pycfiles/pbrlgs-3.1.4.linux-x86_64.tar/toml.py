...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/pbr/toml.py
# Compiled at: 2017-12-04 11:08:24
import datetime, io
...
def loads(s, decoder=None):
    ...
    for line in s:
        ...
        if line[0] == '[':
            ...
            decoder._load_inline_object(line, currentlevel, multikey, multibackslash)
        elif '=' in line:
            ret = decoder._load_line(line, currentlevel, multikey, multibackslash)
            if ret is not None:
...
def _load_date(val):
    ...
    try:
        d = datetime.datetime(int(val[:4]), int(val[5:7]), int(val[8:10]), int(val[11:13]), int(val[14:16]), int(val[17:19]), microsecond)
    except ValueError:
        return
...
class TomlDecoder(object):
    ...
    def _load_inline_object(self, line, currentlevel, multikey=False, multibackslash=False):
        ...
        while len(candidate_groups) > 0:
            ...
            value = value.strip()
            if value[0] == value[(-1)] and value[0] in ('"', "'") or value[0] in '-0123456789' or value in ('true',
                                                                                                            'false') or value[0] == '[' and value[(-1)] == ']':
                groups.append(candidate_group)
            else:
                candidate_groups[0] = candidate_group + ',' + candidate_groups[0]

        for group in groups:
            status = self._load_line(group, currentlevel, multikey, multibackslash)
    ...
    def _load_line(self, line, currentlevel, multikey, multibackslash):
        ...
        if (pair[0][0] == '"' or pair[0][0] == "'") and (pair[0][(-1)] == '"' or pair[0][(-1)] == "'"):
            pair[0] = pair[0][1:-1]
        if len(pair[1]) > 2 and (pair[1][0] == '"' or pair[1][0] == "'") and pair[1][1] == pair[1][0] and pair[1][2] == pair[1][0] and not (len(pair[1]) > 5 and pair[1][(-1)] == pair[1][0] and pair[1][(-2)] == pair[1][0] and pair[1][(-3)] == pair[1][0]):
            k = len(pair[1]) - 1
        ...
        if v == 'true':
            ...
            if v == 'false':
                ...
                if not strictly_valid:
                    raise TomlDecodeError('Weirdness with leading zeroes or underscores in your number.')
                itype = 'int'
                neg = False
                if v[0] == '-':
                    neg = True
    ...
    def _load_array(self, a):
        ...
        if '[' not in a[1:-1] or '' != a[1:-1].split('[')[0].strip():
            ...
            if strarray:
                while b < len(a) - 1:
                    ab = a[b].strip()
                    while ab[(-1)] != ab[0] or len(ab) > 2 and ab[0] == ab[1] == ab[2] and ab[(-2)] != ab[0] and ab[(-3)] != ab[0]:
                        a[b] = a[b] + ',' + a[(b + 1)]
                        ab = a[b].strip()
                        if b < len(a) - 2:
                            a = a[:b + 1] + a[b + 2:]
