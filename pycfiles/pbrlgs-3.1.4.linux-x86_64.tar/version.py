...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/pbr/version.py
...
class SemanticVersion(object):
    ...
    def __init__(self, major, minor=0, patch=0, prerelease_type=None, prerelease=None, dev_count=None):
        ...
        if self._prerelease_type and not self._prerelease:
            self._prerelease = 0
        self._dev_count = dev_count or 0
    ...
    def _from_pip_string_unsafe(klass, version_string):
        ...
        if digit_len == 0:
            ...
            if remainder_starts_with_int:
                ...
                if remainder and (remainder[0][0] == '0' or remainder[0][0] in ('a',
                                                                                'b',
                                                                                'r')):
                ...
                while remainder:
                    ...
                    if component.startswith('dev'):
                        ...
                        raise ValueError('Unknown remainder %r in %r' % (
        ...
        if post_count:
            if dev_count:
                raise ValueError('Cannot combine postN and devN - no mapping in %r' % (
                 version_string,))
    ...
    def increment(self, minor=False, major=False):
        ...
        if major:
            ...
            new_major = self._major
        return SemanticVersion(new_major, new_minor, new_patch, new_prerelease_type, new_prerelease)
    ...
    def _long_version(self, pre_separator, rc_marker=''):
        ...
        if (self._prerelease_type or self._dev_count) and pre_separator is None:
            segments = [
             self.decrement().brief_string()]
            pre_separator = '.'
        else:
            segments = [
             self.brief_string()]
        if self._prerelease_type:
            segments.append('%s%s%s%s' % (pre_separator, rc_marker, self._prerelease_type,
             self._prerelease))
        if self._dev_count:
    ...
    def version_tuple(self):
        ...
        if self._prerelease_type:
            type_map = {('a', False): 'alpha', ('b', False): 'beta', ('rc', False): 'candidate', 
               ('a', True): 'alphadev', 
               ('b', True): 'betadev', 
