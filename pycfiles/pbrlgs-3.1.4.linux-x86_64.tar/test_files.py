...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/pbr/tests/test_files.py
...
class FilesConfigTest(base.BaseTestCase):
    ...
    def setUp(self):
        ...
        pkg_fixture = fixtures.PythonPackage('fake_package', [
         ('fake_module.py', ''),
         ('other_fake_module.py', '')])
        self.useFixture(pkg_fixture)
        pkg_etc = os.path.join(pkg_fixture.base, 'etc')
        pkg_sub = os.path.join(pkg_etc, 'sub')
        subpackage = os.path.join(pkg_fixture.base, 'fake_package', 'subpackage')
        os.makedirs(pkg_sub)
