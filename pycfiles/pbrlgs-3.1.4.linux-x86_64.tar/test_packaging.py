...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/pbr/tests/test_packaging.py
# Compiled at: 2017-12-04 11:08:24
import email, email.errors, imp, os, re, sysconfig, tempfile, textwrap, fixtures, mock, pkg_resources, six, testtools
from testtools import matchers
import virtualenv, wheel.install
from pbr import git
...
class CreatePackages(fixtures.Fixture):
    ...
    """
    defaults = {'setup.py': textwrap.dedent(six.u("            #!/usr/bin/env python\n            import setuptools\n            setuptools.setup(\n                setup_requires=['pbr'],\n                pbr=True,\n            )\n        ")), 
       'setup.cfg': textwrap.dedent(six.u('            [metadata]\n            name = {pkg_name}\n        '))}
...
class TestPackagingHelpers(testtools.TestCase):
    ...
    def test_generate_script_validates_expectations(self):
        ...
        entry_point = pkg_resources.EntryPoint(name='test-ep', module_name='pbr.packaging', attrs=('attr1',
                                                                                                   'attr2',
                                                                                                   'attr3'))
        self.assertRaises(ValueError, packaging.generate_script, group, entry_point, header, template)
...
class TestPipfileRequirements(base.BaseTestCase):
    ...
    def test_pipfilerequirement(self):
        pipfile_path = os.path.join(os.path.join(os.path.dirname(__file__), 'testpipfilepackage', 'Pipfile'))
