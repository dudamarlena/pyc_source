...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/pbr/tests/test_setup.py
# Compiled at: 2017-12-04 07:19:32
from __future__ import print_function
import os, sys, tempfile, testscenarios
try:
...
class SkipFileWrites(base.BaseTestCase):
    scenarios = [
     (
      ...
      dict(option_key='skip_changelog', option_value='True', env_key='SKIP_WRITE_GIT_CHANGELOG', env_value=None, pkg_func=git.write_git_changelog, filename='ChangeLog')),
     (
      'changelog_option_false',
      dict(option_key='skip_changelog', option_value='False', env_key='SKIP_WRITE_GIT_CHANGELOG', env_value=None, pkg_func=git.write_git_changelog, filename='ChangeLog')),
     (
      'changelog_env_true',
      dict(option_key='skip_changelog', option_value='False', env_key='SKIP_WRITE_GIT_CHANGELOG', env_value='True', pkg_func=git.write_git_changelog, filename='ChangeLog')),
     (
      'changelog_both_true',
      dict(option_key='skip_changelog', option_value='True', env_key='SKIP_WRITE_GIT_CHANGELOG', env_value='True', pkg_func=git.write_git_changelog, filename='ChangeLog')),
     (
      'authors_option_true',
      dict(option_key='skip_authors', option_value='True', env_key='SKIP_GENERATE_AUTHORS', env_value=None, pkg_func=git.generate_authors, filename='AUTHORS')),
     (
      'authors_option_false',
      dict(option_key='skip_authors', option_value='False', env_key='SKIP_GENERATE_AUTHORS', env_value=None, pkg_func=git.generate_authors, filename='AUTHORS')),
     (
      'authors_env_true',
      dict(option_key='skip_authors', option_value='False', env_key='SKIP_GENERATE_AUTHORS', env_value='True', pkg_func=git.generate_authors, filename='AUTHORS')),
     (
      'authors_both_true',
      dict(option_key='skip_authors', option_value='True', env_key='SKIP_GENERATE_AUTHORS', env_value='True', pkg_func=git.generate_authors, filename='AUTHORS'))]
...
class GitLogsTest(base.BaseTestCase):
    ...
    def test_generate_authors(self):
        ...
        cmd_map = {git_log_cmd: author_new, 
...
class BaseSphinxTest(base.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.useFixture(fixtures.MonkeyPatch('sphinx.application.Sphinx.__init__', lambda *a, **kw: None))
        self.useFixture(fixtures.MonkeyPatch('sphinx.application.Sphinx.build', lambda *a, **kw: None))
        self.useFixture(fixtures.MonkeyPatch('sphinx.application.Sphinx.config', _SphinxConfig))
        self.useFixture(fixtures.MonkeyPatch('sphinx.config.Config.init_values', lambda *a: None))
        self.useFixture(fixtures.MonkeyPatch('sphinx.config.Config.__init__', lambda *a: None))
        from distutils import dist
        self.distr = dist.Distribution()
        self.distr.packages = ('fake_package', )
        self.distr.command_options['build_sphinx'] = {'source_dir': [
                        'a', '.']}
        pkg_fixture = fixtures.PythonPackage('fake_package', [('fake_module.py', ''),
         ('another_fake_module_for_testing.py', ''),
         ('fake_private_module.py', '')])
        self.useFixture(pkg_fixture)
        self.useFixture(base.DiveDir(pkg_fixture.base))
        self.distr.command_options['pbr'] = {}
        if hasattr(self, 'excludes'):
            self.distr.command_options['pbr']['autodoc_exclude_modules'] = ('setup.cfg',
...
class BuildSphinxTest(BaseSphinxTest):
    ...
    def test_build_doc(self):
        ...
        self.assertTrue(os.path.exists('api/fake_package.fake_module.rst') == self.has_autodoc)
        ...
        assertion(os.path.exists('api/fake_package.another_fake_module_for_testing.rst'))
    ...
    def test_cmd_builder_override(self):
        if self.has_opt:
            self.distr.command_options['pbr'] = {'autodoc_index_modules': ('setup.cfg', self.autodoc)}
        self.distr.command_options['build_sphinx']['builder'] = ('command line', 'non-existing-builder')
        build_doc = packaging.LocalBuildDoc(self.distr)
    ...
    def test_cmd_builder_override_multiple_builders(self):
        if self.has_opt:
            self.distr.command_options['pbr'] = {'autodoc_index_modules': ('setup.cfg', self.autodoc)}
        self.distr.command_options['build_sphinx']['builder'] = ('command line', 'builder1,builder2')
        build_doc = packaging.LocalBuildDoc(self.distr)
        build_doc.finalize_options()
...
class APIAutoDocTest(base.BaseTestCase):
    ...
    def test_default_api_build_dir(self):
        ...
        self.assertTrue(os.path.exists('api/fake_package.another_fake_module_for_testing.rst'))
    ...
    def test_different_api_build_dir(self):
        ...
        self.assertTrue(os.path.exists('contributor/api/fake_package.fake_private_module.rst'))
        return
...
class ParseRequirementsTestScenarios(base.BaseTestCase):
    ...
    scenarios = scenarios + testscenarios.multiply_scenarios([
     ...
     (
      'http_egg_url', {'url': 'https://foo.com/zipball#egg=bar'})], versioned_scenarios)
    scenarios = scenarios + testscenarios.multiply_scenarios([
     (
     ...
     (
      'editable', {'editable': True})], versioned_scenarios)
...
class ParseRequirementsTest(base.BaseTestCase):
    ...
    def setUp(self):
        ...
        fd, self.tmp_file = tempfile.mkstemp(prefix='openstack', suffix='.setup')

    def test_parse_requirements_override_with_env(self):
        with open(self.tmp_file, 'w') as (fh):
            fh.write('foo\nbar')
        self.useFixture(fixtures.EnvironmentVariable('PBR_REQUIREMENTS_FILES', self.tmp_file))
        self.assertEqual(['foo', 'bar'], packaging.parse_requirements())

    def test_parse_requirements_override_with_env_multiple_files(self):
        with open(self.tmp_file, 'w') as (fh):
            fh.write('foo\nbar')
        self.useFixture(fixtures.EnvironmentVariable('PBR_REQUIREMENTS_FILES', 'no-such-file,' + self.tmp_file))
        self.assertEqual(['foo', 'bar'], packaging.parse_requirements())
