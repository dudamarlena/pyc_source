...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/pbr/pipfile.py
# Compiled at: 2017-12-04 11:08:24
import codecs, json, hashlib, platform, sys, os
from pbr import toml
...
class PipfileParser(object):
    ...
    def parse(self):
        with open(self.filename) as (f):
            content = f.read()
        default_config = {'source': [{'url': 'https://pypi.python.org/simple', 'verify_ssl': True, 'name': 'pypi'}], 'packages': {}, 'requires': {}, 'dev-packages': {}}
        config = {}
        config.update(default_config)
        config.update(toml.loads(content))
        data = {'_meta': {'sources': config['source'], 
