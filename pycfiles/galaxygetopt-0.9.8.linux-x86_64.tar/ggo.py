...
# [GCC 8.4.0]
# Embedded file name: /home/users/cpt/cpt/esr/Projects/galaxy/galaxygetopt/env/lib/python2.7/site-packages/galaxygetopt/ggo.py
...
class GalaxyGetOpt(object):
    ...
    def __init__(self, options=[], outputs=[], defaults={}, tests=[], doc=None, argv=None):
        if argv is not None:
            self.argv = argv
        else:
            self.argv = sys.argv
        ...
        if '--generate_galaxy_xml' in self.argv[1:]:
            ...
            print ggx.gen(param_collection=parameter_collection, appinfo={'id': self.appid, 'name': self.appname, 
               'desc': self.appdesc, 'vers': self.appvers}, docstring=doc, outputs=outputs, options=options, defaults=defaults, tests=tests)
            sys.exit(1)
        ...
        if '--dump_options' in self.argv[1:]:
            print self.serialize_opt_to_yaml(options=options, outputs=outputs, defaults=defaults, tests=tests)
            sys.exit(1)
    ...
    def serialize_opt_to_yaml(self, options=[], outputs=[], defaults={}, tests=[]):
        ...
        import yaml
        return yaml.dump({'options': options, 
           'outputs': outputs, 
           'defaults': defaults, 
           'tests': tests})
    ...
    def convert_opt_arr_to_params(self, options_array, outputs):
        ...
        pC.push_param(['generate_galaxy_xml', 'Generate Galaxy XML file',
         {'hidden': True, 'validate': 'Flag', '_show_in_galaxy': False}])
        pC.push_param(['galaxy', 'Run with galaxy-specific overrides',
         {'hidden': True, 'validate': 'Flag', '_galaxy_specific': True, 
            '_show_in_galaxy': False}])
        pC.push_param(['outfile_supporting', 'File or folder to output to',
         {'hidden': True, 'validate': 'String', 'default': '__new_file_path__', 
            '_galaxy_specific': True, '_show_in_galaxy': False}])
        self.pC = pC
        return pC
    ...
    def fix_outputs(self, outputs):
        r"""Internal function to parse specified outputs and munge them into standard options
        ...
        for output in outputs:
            ...
            desc = output[1]
            opts = output[2]
            fixed_outputs[name] = {'description': desc, 
            ...
            fixed_outputs[namefp] = {'description': namefp_desc, 
               'options': copy.deepcopy(opts)}
            ...
            fixed_outputs[namefmt] = {'description': namefmt_desc, 
               'options': copy.deepcopy(opts)}
            fixed_outputs[namefmt]['options']['default'] = opts['default_format']
            ...
            fixed_outputs[nameid] = {'description': nameid_desc, 
               'options': copy.deepcopy(opts)}
