...
# Embedded file name: build/bdist.linux-x86_64/egg/plantflow/analysis.py
# Compiled at: 2020-01-27 00:39:20
# Size of source mod 2**32: 31074 bytes
import pandas as pd, numpy as np
import matplotlib.pyplot as plt
import random, datetime
from IPython.display import clear_output, display
from collections import OrderedDict
import weibull, scipy, os
...
class PlantPerformance:
    ...
    def split(self, df, event_type='offspec', quality_type='%FAT CONTENT', color_variable='PRODUCT', process_limits='Wonka', xsize=5, ysize=5):
        ...
        if event_type == 'quality':
            ...
            if type(process_limits) is str:
                try:
                    process_limits = pd.read_excel((os.path.join(os.path.dirname(__file__), '../data/{}.xlsx'.format(process_limits))), sheet_name='LIQUOR PROCESS LIMITS')
        ...
        if event_type == 'waste':
            ...
            good = dff.loc[(pd.to_numeric(dff['offspec']) <= ucl)]
        self.event = event
        self.good = df.loc[df['ID'].isin(good['ID'])]
    ...
    def predict(self, xsize=5, ysize=5):
        ...
        if event.shape[0] > 3:
            ...
            T = time_delta.total_seconds() / 60 / 60
            ...
            df_ca.Prediction = df_ca.Prediction.shift(1)
            df_ca['Prediction'][0] = 0
            df_ca['Error'] = df_ca['Prediction'] - df_ca['time to event']
            if event_type == 'offspec':
                print('{} avg prediction error in hrs: {:.2f}, (N = {})'.format(items.unique(), np.average(np.abs(df_ca['Error'][4:])), df_ca.shape[0]))
                print('{} median prediction error in hrs: {:.2f}, (N = {})'.format(items.unique(), np.median(np.abs(df_ca['Error'][4:])), df_ca.shape[0]))
            else:
                print('{} avg prediction error in hrs: {:.2f}, (N = {})'.format(item, np.average(np.abs(df_ca['Error'][4:])), df_ca.shape[0]))
                print('{} median prediction error in hrs: {:.2f}, (N = {})'.format(item, np.median(np.abs(df_ca['Error'][4:])), df_ca.shape[0]))
    ...
    def moods(self, mode='quality'):
        ...
        if mode == 'quality':
            for atr in good.loc[(good['Attribute Type'] == 'QUALITY')]['ITEM'].unique():
                try:
                    stat, p, m, table = scipy.stats.median_test(good.loc[(good['ITEM'] == atr)]['VALUE'].astype(float).reset_index(drop=True), bad.loc[(bad['ITEM'] == atr)]['VALUE'].astype(float).reset_index(drop=True))
                    print(atr, '\nstat:\t', stat, '\n p:\t', p, '\n m:\t', m, '\n table:\t', table, '\n')
...
def calc_product_metrics(product, data, verbose=0, stream_designation='PRODUCT'):
    ...
    """
    ids = data.loc[((data['Stream Type'] == stream_designation) & (data['Scheduled or Actual'] == 'A') & data['ITEM'].isin(product))]['ID'].values
    theo_yield = np.round(np.sum(data.loc[(data['ID'].isin(ids) & (data['Scheduled or Actual'] == 'S') & (data['Stream Type'] == 'PRODUCT'))]['VALUE']) / np.sum(data.loc[(data['ID'].isin(ids) & (data['Scheduled or Actual'] == 'S') & (data['Stream Type'] == 'FEED'))]['VALUE']), 5)
    rm_yield = np.round(np.sum(data.loc[(data['ID'].isin(ids) & (data['Scheduled or Actual'] == 'A') & (data['Stream Type'] == 'PRODUCT'))]['VALUE']) / np.sum(data.loc[(data['ID'].isin(ids) & (data['Scheduled or Actual'] == 'A') & (data['Stream Type'] == 'FEED'))]['VALUE']), 5)
    ...
    if end_a.shape[0] == start_b.shape[0] - 1:
        ...
        feed_rate_mean = np.round(total_kg_feed / uptime_total.total_seconds() * 60 * 60, 2)
        product_rate_mean = np.round(total_kg_product / uptime_total.total_seconds() * 60 * 60, 2)
        ...
        feed_rate = kg_feed / uptime.dt.total_seconds() * 60 * 60
        product_rate = kg_product / uptime.dt.total_seconds() * 60 * 60
        ...
        metrics.columns = [
         'PRODUCT', 'start', 'stop', 'uptime', 'theoretical yield', 'actual yield', 'actual/theoretical yield', 'total kg feed', 'total kg product',
         'feed rate (kg/hr)', 'product rate (kg/hr)', 'MAC feed (kg/hr)', 'MAC product (kg/hr)', '% rate feed',
        ...
        for line in lines:
            ...
            for i in range(1, start_end.shape[0] + 1, 2):
                ...
                if i < start_end.shape[0] - 1:
                    ...
                    if start_end.iloc[(i + 1)]['VALUE'] - start_end.iloc[i]['VALUE'] < datetime.timedelta(0):
            ...
            if verbose == 0:
                ...
                print('')
            elif data.loc[((data['Stream Type'] == 'PRODUCT') & (data['Scheduled or Actual'] == 'A'))].groupby('ID', sort=False)['ITEM'].unique()[1].shape[0] > 1:
