...
# Embedded file name: build/bdist.linux-x86_64/egg/plantflow/plantflow.py
# Compiled at: 2019-12-04 09:08:54
# Size of source mod 2**32: 9775 bytes
import random, simpy
...
class Tank(simpy.Container):
    ...
    def __init__(self, env, name, capacity=1000.0, init=0, *args, **kwargs):
        (super().__init__)(env, *args, init=init, capacity=capacity, **kwargs)
        self.env = env
        self.name = name
...
class UnitOperation(object):
    ...
    def __init__(self, env, name, operating_times, discharge_header=False, batch_size=1, route_in=None, route_out=None, route_out_type='priority', route_out_di=None, schedule=None, dump_blocked_discharge=True):
        self.env = env
        self.name = name
    ...
    def envoke_schedule(self, env):
        for i in range(0, max(self.schedule['Days Operating'].values())):
            if i not in self.schedule['Days Operating'].values():
                if env.now == i * 24 * 60:
    ...
    def charge(self, env):
        while True:
            available_tanks = [i for i, x in enumerate([i.level >= self.batch_size for i in self.route_in]) if x]
            if available_tanks:
                yield self.route_in[available_tanks[random.randint(0, len(available_tanks) - 1)]].get(self.batch_size)
                break
            else:
    ...
    def discharge(self, env):
        ...
        if self.route_out_type == 'percent':
            ...
            for i, x in enumerate(self.route_out_di):
                ...
                while not successful:
                    ...
                    if quantity > 0:
                        matching_tanks = [self.route_out[a] for a, b in enumerate([x in k.name for k in self.route_out]) if b]
                        available_tanks = [l for l, m in enumerate([l.capacity - l.level >= self.batch_size for l in matching_tanks]) if m]
                        if available_tanks:
                            self.blocked = False
                            yield matching_tanks[available_tanks[random.randint(0, len(available_tanks) - 1)]].put(self.batch_size * quantity)
                            successful = True
                        elif self.dump_blocked_discharge:
                            self.blocked = True
        ...
        if self.route_out_type == 'priority':
            successful = False
            while not successful:
                available_tanks = [i for i, x in enumerate([i.capacity - i.level >= self.batch_size for i in self.route_out]) if x]
                if available_tanks:
                    self.blocked = False
                    yield self.route_out[available_tanks[random.randint(0, len(available_tanks) - 1)]].put(self.batch_size)
                    successful = True
                elif self.dump_blocked_discharge:
                    self.blocked = True
