...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/pdfkit/pdfkit.py
# Compiled at: 2017-01-09 10:38:30
...
class PDFKit(object):
    ...
    def __init__(self, url_or_file, type_, options=None, toc=None, cover=None, css=None, configuration=None, cover_first=False):
    ...
    def _genargs(self, opts):
        ...
        for optkey, optval in self._normalize_options(opts):
            yield optkey
            if not (isinstance(optval, (list, tuple)) and len(optval) == 2 and optval[0] and optval[1]):
                raise AssertionError('Option value can only be either a string or a (tuple, list) of 2 items')
                yield optval[0]
                yield optval[1]
            else:
                yield optval
    ...
    def _command(self, path=None):
        ...
        if self.source.isString() or self.source.isFileObj():
            yield '-'
        elif isinstance(self.source.source, basestring):
    ...
    def _normalize_options(self, options):
        ...
        for key, value in list(options.items()):
            ...
            if isinstance(value, (list, tuple)):
                for optval in value:
                    yield (
                     normalized_key, optval)

            else:
                yield (
                 normalized_key, str(value) if value else value)
    ...
    def _prepend_css(self, path):
        if self.source.isUrl() or isinstance(self.source.source, list):
            raise self.ImproperSourceError('CSS files can be added only to a single file or string')
