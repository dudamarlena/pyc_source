...
# [GCC 8.4.0]
# Embedded file name: /home/zhemin/software/GrapeTree/module/MSTrees.py
# Compiled at: 2019-04-04 07:05:34
...
params = dict(method='MSTreeV2', matrix_type='symmetric', heuristic='eBurst', handle_missing='pair_delete', branch_recraft=False, wgMLST=False, n_proc=5, checkEnv=False, NJ_Windows=os.path.join(base_dir, 'binaries', 'fastme.exe'), NJ_Darwin=os.path.join(base_dir, 'binaries', 'fastme-2.1.5-osx'), NJ_Linux=os.path.join(base_dir, 'binaries', 'fastme-2.1.5-linux64'), NJ_Linux32=os.path.join(base_dir, 'binaries', 'fastme-2.1.5-linux32'), edmonds_Windows=os.path.join(base_dir, 'binaries', 'edmonds.exe'), edmonds_Darwin=os.path.join(base_dir, 'binaries', 'edmonds-osx'), edmonds_Linux=os.path.join(base_dir, 'binaries', 'edmonds-linux'), RapidNJ_Linux=os.path.join(base_dir, 'binaries', 'rapidnj'), RapidNJ_Darwin=os.path.join(base_dir, 'binaries', 'rapidnj-osx'), RapidNJ_Windows=os.path.join(base_dir, 'binaries', 'rapidnj.exe'))
...
class distance_matrix(object):
    ...
    def get_distance(func, profiles, handle_missing):
        ...
        if n_proc > 1:
            ...
            subfiles = [
             parallel_distance([func, params['prof_file'], params['dist_subfile'], handle_missing, [0, n_profile]])]
    ...
    def symmetric(profiles, handle_missing='pair_delete', index_range=None):
        ...
        if handle_missing in ('as_allele', ):
            ...
            if handle_missing in ('pair_delete', 'absolute_distance'):
                presences = profiles > 0
            else:
    ...
    def symmetric_link(profiles, links, handle_missing='pair_delete'):
        if handle_missing in ('as_allele', ):
            ...
            presences = np.repeat(np.sum(profiles > 0, 0) >= profiles.shape[0], profiles.shape[0]).reshape([profiles.shape[1], profiles.shape[0]]).T
        return [ [s, t, np.sum((profiles[s] != profiles[t]) & presences[s] & presences[t])] for s, t, d in links
               ]
...
class methods(object):
    ...
    def _blockwise(dist, weight, **params):
        x = methods._symmetric((dist * 10000.0), weight, **params)
        return [ [b[0], b[1], b[2] / 10000.0] for b in x ]
    ...
    def _asymmetric(dist, weight, **params):
        ...
        try:
            ...
            with open(wdist_file, 'w') as (fout):
                for d in wdist:
                    fout.write(('{0}\n').format(('\t').join([ str(dd) for dd in d + 1.0 ])))
    ...
    def _branch_recraft(branches, dist, weights, n_loci):
        if n_loci is None:
            n_loci = np.max(dist)
        group_id, groups, childrens = {b:b for br in branches for b in br[:2]}, {b:[b] for br in branches for b in br[:2]}, {b:[] for br in branches for b in br[:2]}
    ...
    def ninja(names, profiles, embeded, handle_missing='pair_delete', **params):
        ...
        del dist
        del d
        free_memory = int(0.9 * psutil.virtual_memory().total / 1048576.0)
    ...
    for line_id, line in enumerate(fin):
        if line.startswith('#'):
            if not line.startswith('##'):
                ...
                allele_cols = np.array([ id for id, col in enumerate(header) if id > 0 and not col.startswith('#') and col.lower() not in {'st_id', 'st'} ])
            continue
        if line.startswith('>'):
            fmt = 'fasta'
        else:
...
def estimate_Consumption(platform, method, matrix, n_proc, n_loci, n_profile):
    if method in ('MSTree', 'RapidNJ'):
        if matrix == 'asymmetric':
            if platform == 'Windows':
                time = 5.600754e-06 * n_profile * n_profile + 6.22306e-09 * n_loci * n_profile * n_profile / n_proc + 22.71
                memory = 182.16 * n_profile * n_profile + 282674000
            else:
                time = 2.431284e-06 * n_profile * n_profile + 2.701426667e-09 * n_loci * n_profile * n_profile / n_proc + 33.753
                memory = 103.77 * n_profile * n_profile + 516625000
        elif platform == 'Windows':
            time = 3.362214e-06 * n_profile * n_profile + 3.735793333e-09 * n_loci * n_profile * n_profile / n_proc + 20
            memory = 70.14 * n_profile * n_profile + 292156000
        else:
            time = 2.272428e-06 * n_profile * n_profile + 32.625 + 2.52492e-09 * n_loci * n_profile * n_profile / n_proc
            memory = 66.297 * n_profile * n_profile + 429570000
    elif method == 'NJ':
        if platform == 'Windows':
            time = 1.149e-08 * n_profile * n_profile * n_profile
            memory = max(0.058292 * n_profile * n_profile * n_profile, 1390000.0 * n_profile - 986000000.0)
        else:
            time = 1.1042e-08 * n_profile * n_profile * n_profile
            memory = max(0.058292 * n_profile * n_profile * n_profile, 1390000.0 * n_profile - 986000000.0)
    return (
     max(time, 5), max(memory, 52428800))
