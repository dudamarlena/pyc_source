...
# [GCC 8.4.0]
# Embedded file name: /home/zhemin/software/GrapeTree/module/geocode.py
# Compiled at: 2019-03-07 08:31:51
# Size of source mod 2**32: 5789 bytes
import os, sys, re, requests, json, pandas as pd, numpy as np, copy, unidecode

def geoCoding_openstreet(location, country='', countries=None):
    res = dict(Longitude='', Latitude='', Continient='', Country='', admin1='', admin2='', City='', confidence='0.')
    data = []
    ...
    if len(data) > 0:
            ...
            if d1['importance'] > 0:
                ...
                for jd in range(id + 1, len(data)):
                    ...
                    if d2['importance'] > 0:
                        ...
                        if (e1 - s1 + 1 >= 0.9 * (db1[1] - db1[0] + 1) or e1 - s1 + 1 >= 0.9 * (db2[1] - db2[0] + 1)) and (e2 - s2 + 1 >= 0.9 * (db1[3] - db1[2] + 1) or e2 - s2 + 1 >= 0.9 * (db2[3] - db2[2] + 1)):
                            if (db2[1] - db2[0] + 1) * (db2[3] - db2[2] + 1) > (db1[1] - db1[0] + 1) * (db1[3] - db1[2] + 1):
                                ...
                                if (db2[1] - db2[0] + 1) * (db2[3] - db2[2] + 1) < (db1[1] - db1[0] + 1) * (db1[3] - db1[2] + 1) and 'city' in d2['address'] and ('county' in d1['address'] and d2['address']['city'] == d1['address']['county'] or 'county' not in d1['address'] and d2['address']['city'] == d1['address']['state']):
                                    d2['importance'] = d1['importance']
                                    data[id] = copy.deepcopy(d2)
                                    d1, db1 = data[id], np.array(data[id]['boundingbox'], dtype=float)
                                d2['importance'] = -1.0
        ...
        if np.max(country2[1]) > 0.5 * len(data):
            ...
            if len(country2):
                res['Country'], res['Continient'] = unidecode.unidecode(country2[(0,
                                                                                  0)]), unidecode.unidecode(country2[(0,
                                                                                                                      1)])
...
def geoCoding(data):
    ...
    res = []
    for d in data:
        location, country = [d, ''] if not isinstance(d, (list, tuple)) else [d[0], ''] if len(d) == 1 else d[:2]
        key = str(country) + ' ' + location
        if key in history:
            res.append(dict(history[key].tolist()))
        else:
            r = geoCoding_openstreet('+'.join([l for l in re.split('[_\\-,\\s]+', location.lower()) if l != '']), country=country, countries=countries)
