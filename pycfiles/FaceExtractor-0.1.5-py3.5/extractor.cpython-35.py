...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\face_extractor\extractor.py
# Compiled at: 2017-09-03 02:03:59
...
class Extractor:
    ...
    HAARCASCADE_ALT = os.path.join(_current_dir, 'data', 'haarcascade_frontalface_alt.xml')
    HAARCASCADE_ALT2 = os.path.join(_current_dir, 'data', 'haarcascade_frontalface_alt2.xml')
    HAARCASCADE_ALT_TREE = os.path.join(_current_dir, 'data', 'haarcascade_frontalface_alt_tree.xml')
    HAARCASCADE_DEFAULT = os.path.join(_current_dir, 'data', 'haarcascade_frontalface_default.xml')
    ...
    @classmethod
    def detect(cls, image, minSize=(50, 50), scaleFactor=1.1, minNeighbors=5, cascadeFile=_current_cascade):
        ...
        return classifier.detectMultiScale(grayImage, scaleFactor=scaleFactor, minNeighbors=minNeighbors, minSize=minSize)

    @classmethod
    def extract(cls, imagePath, size=None, scaleFactor=1.1, minNeighbors=5, minSize=(50, 50), cascadeFile=_current_cascade, outputDirectory=None, outputPrefix=None, startCount=0):
        ...
        faces = cls.detect(image, scaleFactor=scaleFactor, minNeighbors=minNeighbors, minSize=minSize, cascadeFile=cascadeFile)
        idx = startCount
        if not outputPrefix:
