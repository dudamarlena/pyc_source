...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/SPARQLWrapper/Wrapper.py
...
import base64, re, sys, warnings, json
...
TSV = 'tsv'
_allowedFormats = [
 JSON, XML, TURTLE, N3, RDF, RDFXML, CSV, TSV]
GET = 'GET'
POST = 'POST'
_allowedRequests = [
 POST, GET]
...
ADD = 'ADD'
_allowedQueryTypes = [
 SELECT, CONSTRUCT, ASK, DESCRIBE, INSERT, DELETE, CREATE, CLEAR, DROP,
 LOAD, COPY, MOVE, ADD]
URLENCODED = 'urlencoded'
POSTDIRECTLY = 'postdirectly'
_REQUEST_METHODS = [
 URLENCODED, POSTDIRECTLY]
_SPARQL_DEFAULT = [
 'application/sparql-results+xml', 'application/rdf+xml', '*/*']
_SPARQL_XML = ['application/sparql-results+xml']
_SPARQL_JSON = ['application/sparql-results+json', 'application/json', 'text/javascript', 'application/javascript']
...
class SPARQLWrapper(object):
    ...
    """
    prefix_pattern = re.compile('((?P<base>(\\s*BASE\\s*<.*?>)\\s*)|(?P<prefixes>(\\s*PREFIX\\s+.+:\\s*<.*?>)\\s*))*')
    pattern = re.compile('(?P<queryType>(CONSTRUCT|SELECT|ASK|DESCRIBE|INSERT|DELETE|CREATE|CLEAR|DROP|LOAD|COPY|MOVE|ADD))', re.VERBOSE | re.IGNORECASE)
    ...
    def __init__(self, endpoint, updateEndpoint=None, returnFormat=XML, defaultGraph=None, agent=__agent__):
        ...
        :param returnFormat: The return format.        No local check is done, so the parameter is simply sent to the endpoint. Eg, if the value is set to :data:`JSON` and a construct query is issued, it is up to the endpoint to react or not, this wrapper does not check.        The possible values are :data:`JSON`, :data:`XML`, :data:`TURTLE`, :data:`N3`, :data:`RDF`, :data:`RDFXML`, :data:`CSV`, :data:`TSV`, :data:`JSONLD` (constants in this module).        The **default** value is :data:`XML`.
    ...
    def _getRequestEncodedParameters(self, query=None):
        ...
        :param query: a tuple of two items. The first item can be the string         ``query`` (for :data:`SELECT`, :data:`DESCRIBE`, :data:`ASK`, :data:`CONSTRUCT` query) or the string ``update``         (for SPARQL Update queries, like :data:`DELETE` or :data:`INSERT`). The second item of the tuple         is the query string itself.
        :type query: tuple
            ...
            if not self.onlyConneg:
                for f in _returnFormatSetting:
                    ...
                    if self.returnFormat in [TSV, JSONLD, RDFXML]:
                        acceptHeader = self._getAcceptHeader()
                        if '*/*' in acceptHeader:
                            acceptHeader = ''
                        query_parameters[f] += [acceptHeader]

        pairs = ('%s=%s' % (urllib.quote_plus(param.encode('UTF-8'), safe='/'), urllib.quote_plus(value.encode('UTF-8'), safe='/')) for param, values in query_parameters.items() for value in values)
        return ('&').join(pairs)
        ...
        if self.isSparqlUpdateRequest():
            ...
            if self.user and self.passwd:
                if self.http_auth == BASIC:
                    ...
                    valid_types = (', ').join(_allowedAuth)
                    raise NotImplementedError(('Expecting one of: {0}, but received: {1}').format(valid_types, self.http_auth))
...
class QueryResult(object):
    ...
    * ``info()`` that returns the meta-information of the HTTP result as a dictionary-like object.
    ...
    def _convertJSONLD(self):
        ...
        from rdflib import ConjunctiveGraph
        retval = ConjunctiveGraph()
        retval.load(self.response, format='json-ld')
        return retval
    ...
    def _get_responseFormat(self):
        ...
        if 'content-type' in self.info():
            ...
            if _content_type_in_list(ct, _RDF_XML):
                return RDFXML
            if _content_type_in_list(ct, _RDF_TURTLE):
                return TURTLE
            if _content_type_in_list(ct, _RDF_N3):
                return N3
            if _content_type_in_list(ct, _CSV):
