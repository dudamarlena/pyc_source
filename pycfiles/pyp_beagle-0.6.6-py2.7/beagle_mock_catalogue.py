...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyp_beagle/beagle_mock_catalogue.py
...
import bokeh.plotting as bk_plt, bokeh.models as bk_mdl
from beagle_utils import prepare_data_saving, prepare_plot_saving, BeagleDirectories, is_FITS_file, data_exists, plot_exists, set_plot_ticks, is_integer, match_ID, ID_COLUMN_LENGTH
...
def extract_err(err, data):
    ...
    if len(err) > 1:
        ...
        if not (len(err) == len(data) and not (iterable(fe) and len(fe) > 1)):
            raise ValueError('err must be a scalar, the same dimensions as x, or 2xN.')
...
class BeagleMockCatalogue(object):
    ...
    def __init__(self, params_file, ID_key='ID', file_name=None, ignore_string=None, overwrite_plots=True, plot_title=None, n_bins=10):
    ...
    def compare_hist(self, summary_catalogue, class_indices=None, class_colors=None, plot_name=None, summary_type='median', percentile=95.0, params_to_plot=None, pow=False, overwrite=False, kwargs=None):
        ...
        if plot_exists(plot_name) and not overwrite:
            ...
            for i, param in enumerate(params_to_plot):
                ...
                for i, indices in enumerate(class_indices):
                    ...
                    if 'diff_range' in self.adjust_params[param]:
                        ...
                        range = np.percentile(diff[idx], (0.5 * (100.0 - percentile), 100.0 - 0.5 * (100.0 - percentile)))
                    if class_colors is None:
                        color = None
                    else:
                        color = class_colors[i]
                    med, d0, d1 = np.percentile(diff[idx], [50.0, 0.5 * 32.0, 100.0 - 0.5 * 32.0])
                    dispersion = 0.5 * (d1 - d0)
                    diff_med.append(med)
                    diff_disp.append(dispersion)
                    ax.hist(diff[idx], bins=self.n_bins, range=range, color=color, lw=0, **kwargs)
                    ax.text(0.1 + i * 0.25, 1.05, '$\\sigma=' + ('{:.3f}').format(dispersion) + '$', fontsize=8, horizontalalignment='center', transform=ax.transAxes)
                    ax.text(0.1 + i * 0.25, 1.14, '$\\textnormal{med}=' + ('{:.3f}').format(med) + '$', fontsize=8, horizontalalignment='center', transform=ax.transAxes)
            ...
            plt.close(fig)
            return (
             diff_median, diff_dispersion)

    def compare(self, summary_catalogue, plot_name=None, summary_type='median', level=68.0, params_to_plot=None, overwrite=False, rows=None, interactive=False):
        indx1, indx2 = match_ID(self.hdulist[1].data['ID'], summary_catalogue.hdulist[1].data['ID'], ignore_string=self.ignore_string)
        if summary_type not in ('mean', 'median'):
            raise ValueError('`summary_type` con only be set to `mean` or `median`')
        if plot_name is None:
        ...
        if plot_exists(plot_name) and not overwrite:
            ...
            if interactive:
                size = 400
                name = prepare_plot_saving(os.path.splitext(plot_name)[0] + '.html', overwrite=overwrite)
                ...
                strID = [ str(ID) + '_BEAGLE_input_for_Pierre.fits_snr_PS_CLEAR_PRISM_BEAGLE_marginal_SED_spec.pdf' for ID in data['ID']
                        ]
                data['strID'] = strID
            ...
            for i, param in enumerate(params_to_plot):
                ...
                ax.errorbar(x, y, yerr=[
                 yerr_d, yerr_u], ls='', marker='o', ms=3, mew=0, elinewidth=0.6, capsize=3, alpha=0.5)
                for item in [
                 ax.title, ax.xaxis.label, ax.yaxis.label] + ax.get_xticklabels() + ax.get_yticklabels():
                    item.set_fontsize(fontsize)
                ...
                ax.set_xlim(ll)
                ax.set_ylim(ll)
                ax.plot(ax.get_xlim(), ax.get_ylim(), ls='-', color='black', c='.3')
        ...
        for i, (keyX, valueX) in enumerate(self.adjust_params.iteritems()):
            ...
            for j, (keyY, valueY) in enumerate(self.adjust_params.iteritems()):
                ...
                if j > i:
                    ...
                    if i == j:
                        ax.hist(valuesX, bins=self.n_bins, lw=0, color=color)
                    else:
                        if j < i:
                            ax.plot(valuesY, valuesX, marker='o', ms=2, color=color, ls='')
                        if i == j:
                            ax.yaxis.tick_right()
                            if i == 0:
                                ax.set_xticklabels(('', ))
                    ...
                    ax.tick_params(which='minor', axis='both', length=1.5, width=axes_linewidth)
                    ax.tick_params(which='major', axis='both', length=3, width=axes_linewidth)
