...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyp_beagle/beagle_utils.py
# Compiled at: 2019-07-16 04:25:49
import fnmatch, re, os, logging, numpy as np
from bisect import bisect_left
from scipy.integrate import simps, cumtrapz
from scipy.interpolate import interp1d
from datetime import datetime
import sys, dependencies.WeightedKDE as WeightedKDE, matplotlib.ticker as plticker, matplotlib as mpl
...
def configure_matplotlib():
    ...
    mpl.rc('lines', linewidth=1.5, linestyle='-', color='blue', markeredgewidth=0.0, markersize=6)
    mpl.rc('font', family='serif', style='normal', variant='normal', weight='normal', stretch='normal', size=16.0, serif='Times')
    mpl.rc('text', color='black', usetex=True)
    mpl.rcParams['text.latex.preamble'] = '\\usepackage{amssymb}, \\usepackage{amsmath},             \\usepackage{upgreek}, \\usepackage{txfonts}'
    mpl.rcParams['text.latex.preview'] = True
    mpl.rc('axes', facecolor='white', edgecolor='black', linewidth=1.5, grid=False, labelpad=4.0, labelcolor='black')
    ...
    mpl.rc('legend', frameon=False, numpoints=1, handletextpad=0.2)
...
def prepare_violin_plot(data, weights=None, min_x=None, max_x=None, nXgrid=100, max_interval=99.7):
    if min_x is None:
        min_x = np.min(data)
    if max_x is None:
        max_x = np.max(data)
    ...
    pdf_grid /= pdf_norm
    cumul_pdf = cumtrapz(pdf_grid, x_grid, initial=0.0)
    cumul_pdf /= cumul_pdf[(-1)]
    f_interp = interp1d(cumul_pdf, x_grid)
    intv = 0.5 * (100.0 - max_interval) / 100.0
    lims = f_interp([intv, 1.0 - intv])
    ...
    return (
     pdf, pdf_norm, median, x_violin, y_violin)