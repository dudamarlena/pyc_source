...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyp_beagle/beagle_spectra.py
...
class ObservedSpectrum(object):
    ...
    def configure(self, param_file=None, config=None):
        ...
        self.description = {'wl': {'colName': None, 'conversion': None, 'dispersion': None, 'type': None}, 'flux': {'colName': None, 'conversion': None}, 'fluxerr': {'colName': None}, 'sky': {'colName': None}, 'mask': {'colName': None}, 'redshift': {'keyword': 'redshift'}, 'min_rel_err': None}
    ...
    def load(self, file_name):
        ...
        if self.description is None:
            msg = 'The `ObservedSpectrum.description` must be set through the `configure` method!'
        ...
        try:
            ...
            msg = '`' + self.description['wl']['colName'] + '` not found in the current spectrum!'
            raise AttributeError(msg)
        ...
        try:
            ...
            msg = '`' + self.description['flux']['colName'] + '` not found in the current spectrum!'
            raise AttributeError(msg)
        ...
        if self.description['fluxerr']['colName'] is not None:
            try:
                ...
                msg = '`' + self.description['fluxerr']['colName'] + '` not found in the current spectrum!'
                raise AttributeError(msg)
...
class Spectrum(object):
    ...
    def __init__(self, params_file, **kwargs):
        ...
        if kwargs.get('line_labels_json') is None:
            self.line_labels = json.load(resource_stream(__name__, 'files/emission_lines.json'), object_pairs_hook=OrderedDict)
        else:
    ...
    def plot_marginal(self, ID, observation_name=None, max_interval=95.0, print_text=False, replot=False):
        """ 
        ...
        if self.plot_suffix is not None:
            plot_suffix = '_' + self.plot_suffix
        if self.wl_rest:
        ...
        if plot_exists(plot_name) and not replot:
            ...
            for i in range(n_wl):
                ...
                lev = (1.0 - max_interval / 100.0) / 2.0
                lower_flux[i] = f_interp(lev)
                lev = 1.0 - (1.0 - max_interval / 100.0) / 2.0
            ...
            if self.wl_range is None:
                ...
                if len(self.wl_range) == 2:
                    ...
                    n_ranges = int(1.0 * len(self.wl_range) / 2.0)
            ...
            if n_ranges == 1:
                ...
                if self.show_residual:
                    ...
                    t = residual_axs[0].get_figure().get_dpi() / (t[(1, 1)] - t[(0,
                                                                                 1)]) / 72
                    d = 0.5 * (rcParams['xtick.major.size'] * t)
                    wl_l = wl_ranges[0]
                ...
                ylabel = '$F_{\\uplambda} / (\\textnormal{erg} \\,                 \\textnormal{s}^{-1} \\, \\textnormal{cm}^{-2} \\,                 \\textnormal{\\AA}^{-1})$'
                ...
                for ax in axs:
                    ...
                    for slic, col in zip(slices, colors):
                        for s in slic:
                            if self.draw_steps:
                                ax.step(data_wl[s], data_flux[s], where='mid', color=col, linewidth=1.5, alpha=alpha_line)
                            else:
                                ax.plot(data_wl[s], data_flux[s], color=col, linewidth=2.0, alpha=alpha_line)
                            if self.draw_steps:
                                FillBetweenStep.fill_between_steps(ax, data_wl[s], data_flux[s] - data_flux_err[s], data_flux[s] + data_flux_err[s], step_where='mid', color=col, linewidth=0, interpolate=True, alpha=alpha_fill)
                            else:
                                ax.fill_between(data_wl[s], data_flux[s] - data_flux_err[s], data_flux[s] + data_flux_err[s], facecolor=col, linewidth=0, interpolate=True, alpha=alpha_fill)

                    colors = ['blue', 'deepskyblue']
                    for slic, col in zip(slices_model, colors):
                        for s in slic:
                            if self.draw_steps:
                                ax.step(model_wl[s], median_flux[s], where='mid', color=col, linewidth=1.0, alpha=alpha_line)
                            else:
                                ax.plot(model_wl[s], median_flux[s], color=col, linewidth=1.5, alpha=alpha_line)
                            if self.draw_steps:
                                FillBetweenStep.fill_between_steps(ax, model_wl[s], lower_flux[s], upper_flux[s], step_where='mid', color=col, linewidth=0, alpha=alpha_fill)
                            else:
                                ax.fill_between(model_wl[s], lower_flux[s], upper_flux[s], facecolor=col, linewidth=0, interpolate=True, alpha=alpha_fill)
                    ...
                    if 'full sed wl' in hdulist and self.plot_full_SED:
                        ...
                        if self.wl_rest:
                            ...
                            ax.plot(wl_obs, flux_obs, color='black', ls='-', lw=0.5, alpha=0.5)
                    kwargs = {'alpha': 0.8}
                ...
                if self.show_residual:
                    ...
                    residual_err = (1.0 / data_flux_ - (data_flux_ - median_flux) / data_flux_ ** 2) * data_flux_err_
                    ...
                    if has_mask:
                        colors = [
                         'darkgreen', mask_color]
                    for ax in residual_axs:
                        ax.set_ylim([-ymax, ymax])
                        kwargs = {'alpha': 0.7}
                        ax.plot(ax.get_xlim(), [0.0, 0.0], color='black', lw=0.7, zorder=5, **kwargs)
                        for i, col in enumerate(colors):
                            if i == 0:
                                ind = indices[data_mask_]
                            elif i == 1:
                                ind = indices[(~data_mask_)]
                            ax.errorbar(model_wl[ind], residual[ind], yerr=residual_err[ind], color=col, ls=' ', elinewidth=0.5, marker='o', ms=3, **kwargs)
                ...
                for ax in axs:
                    ...
                    if self.plot_line_labels:
                        for key, label in self.line_labels.iteritems():
                            ...
                            if 'ha' in label:
                                ha = label['ha']
                            ax.text(x, y, label['label'], fontsize=self.inset_fontsize, rotation=45, color=color, va='center', ha=ha)
                            y0 = mm
                            ax.plot([x, x], [y0, ydots], ls=':', color=color, lw=1.0, zorder=0)
                    ...
                    if print_text:
                        ...
                        try:
                            ...
                            y = y1 - (y1 - y0) * 0.2
                            ax.text(x, y, '$\\langle\\chi^2/(\\textnormal{N}_\\textnormal{data}-1)\\rangle=' + ('{:.2f}').format(aver_red_chi_square) + '\\;                             (\\textnormal{N}_\\textnormal{data}=' + ('{:d}').format(n_data) + ')' + '$', fontsize=self.inset_fontsize)
                        except AttributeError:
