...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyp_beagle/beagle_photometry.py
# Compiled at: 2019-07-16 04:25:49
import logging, os
from scipy.interpolate import interp1d
from collections import OrderedDict
from bisect import bisect_left
import numpy as np, matplotlib.pyplot as plt, matplotlib.ticker as plticker
...
from beagle_utils import BeagleDirectories, prepare_plot_saving, set_plot_ticks, prepare_violin_plot, plot_exists, pause, extract_row, is_FITS_file
...
class PhotometricCatalogue(ObservedCatalogue):
    ...
    def extract_fluxes(self, filters, ID, key='ID', aper_corr=1.0):
        """ 
        ...
        for j in range(filters.n_bands):
            ...
            if flux_err[j] > 0.0:
                flux_err[j] = np.sqrt((flux_err[j] / flux[j]) ** 2 + np.float32(filters.data['min_rel_err'][j]) ** 2) * abs(flux[j])
...
class Photometry:
    ...
    def plot_marginal(self, ID, max_interval=99.7, print_text=False, print_title=False, replot=False, show=False, units='nanoJy', SED_prob_log_scale=False, n_SED_to_plot=10):
        """ 
        ...
        if self.flux_units == 'milliJy':
            ...
            flux_factor = 1000000.0
            ylabel = '$f_{\\nu}/\\upmu\\textnormal{Jy}$'
        elif self.flux_units == 'nanoJy':
            flux_factor = 1000000000.0
            ylabel = '$f_{\\nu}/\\textnormal{nJy}$'
        else:
        ...
        if plot_exists(plot_name) and not replot and not show:
            ...
            if 'aper_corr' in self.observed_catalogue.data.dtype.names:
                aper_corr = 10.0 ** (-0.4 * observation[0]['aper_corr'])
            else:
                aper_corr = 1.0
            ...
            ok = np.where(obs_flux_err > 0.0)[0]
            ...
            for i in range(n_bands):
                if old_API:
                    ...
                    xdata = 10.0 ** (0.4 * (8.9 - model_sed.data[band_name])) * flux_factor
                min_x = np.min(xdata)
                max_x = np.max(xdata)
            ...
            for i in range(n_bands):
                ...
                if has_pdf[i]:
                    ...
                    ax.fill_betweenx(x_plot[i], (y_grid - y_plot[i] * w), (y_grid + y_plot[i] * w), **kwargs)
                    ax.plot([wl_eff[i] - _lim_y, wl_eff[i] + _lim_y], [
                     median_flux[i], median_flux[i]], color='black', linewidth=0.2)
                ax.plot(wl_eff[i], median_flux[i], color='black', marker='o', markersize=5, zorder=3, alpha=0.6)
            ...
            if 'full sed wl' in hdulist:
                if self.plot_full_SED or self.plot_MAP_SED:
                    ...
                    for j in range(_n_SED_to_plot):
                        ...
                        alpha = 1.0
                        ax.plot(wl_obs, flux_obs, color=color, ls='-', lw=lw, alpha=alpha * max_alpha)
            ...
            x0, x1 = ax.get_xlim()
            if yMin < 0.0:
                plt.plot([x0, x1], [0.0, 0.0], color='gray', lw=0.8)
            if self.plot_filter_labels:
                for i in range(n_bands):
                    ...
                    if len(band_name) >= 5:
                        rotation = 45
                    ax.text(x, y, band_name, fontsize=self.inset_fontsize, rotation=rotation, color='black', va='center', ha='center')
                    ydots = y - 0.025 * dy
                    y = max_flux[i]
                    ax.plot([x, x], [y, ydots], ls=':', color='darkgrey', lw=1.0, zorder=0)
            ...
            kwargs = {'alpha': 0.7}
            plt.errorbar(wl_eff[ok], obs_flux[ok], yerr=obs_flux_err[ok], color='dodgerblue', ls=' ', marker='D', markeredgewidth=0.0, markersize=8, elinewidth=1.0, capsize=3, **kwargs)
            if self.single_solutions is not None:
                ...
                ax.plot(wl_eff, solution, color='green', marker='*', ls='', markersize=10, alpha=0.7)
            if self.log_flux:
            ...
            if print_text:
                ...
                try:
                    ...
                    print '`PosteriorPredictiveChecks` not computed/loaded, hence <chi^2> for the object `' + str(ID) + '` is not available'
                ...
                try:
                    ...
                    y = y1 - (y1 - y0) * 0.2
                    ax.text(x, y, '$\\langle\\chi^2/(\\textnormal{N}_\\textnormal{data}-1)\\rangle=' + ('{:.2f}').format(aver_red_chi_square) + '\\;                         (\\textnormal{N}_\\textnormal{data}=' + ('{:d}').format(n_data) + ')' + '$', fontsize=10)
                except AttributeError:
                    print '`PosteriorPredictiveChecks` not computed/loaded, hence <chi^2_red> for the object `' + str(ID) + '` is not available'

            if y0 < 0.0:
                plt.plot([x0, x1], [0.0, 0.0], color='gray', lw=1.0)
            if show:
                plt.show()
            else:
    ...
    def plot_replicated_data(self, ID, max_interval=99.7, n_replic_to_plot=16, print_text=False, replot=False):
        """ 
        ...
        if plot_exists(plot_name) and not replot:
            ...
            obs_flux_err *= flux_factor
            ok = np.where(obs_flux_err > 0.0)[0]
            fits_file = os.path.join(BeagleDirectories.results_dir, str(ID) + '_BEAGLE.fits.gz')
            model_hdu = fits.open(fits_file)
            model_sed = model_hdu['marginal photometry']
            fits_file = os.path.join(BeagleDirectories.results_dir, BeagleDirectories.pypbeagle_data, str(ID) + '_BEAGLE_replic_data.fits.gz')
            ...
            for i, band_name in enumerate(self.filters.data['label'][sor]):
                if obs_flux_err[i] > 0.0:
                    ...
                    p_value_bands[i] = 1.0 * np.count_nonzero(repl_discr > obs_discr) / n_replicated
            ...
            p_value = 1.0 * np.count_nonzero(repl_discr > obs_discr) / n_replicated
            ...
            residual_fluxes = (replic_fluxes - obs_flux.reshape(n_bands, 1).repeat(n_replicated, 1)) / obs_flux_err.reshape(n_bands, 1).repeat(n_replicated, 1)
            ...
            if 'sns' in sys.modules:
                ...
                sns.heatmap(corr, mask=mask, cmap=cmap, vmax=0.4, square=True, annot=True, fmt='.2f', annot_kws={'size': 10}, linewidths=0.5, cbar_kws={'shrink': 0.85}, ax=ax)
                plt.setp(ax.xaxis.get_majorticklabels(), rotation=45, horizontalalignment='right')
                plt.setp(ax.yaxis.get_majorticklabels(), rotation=45, horizontalalignment='right')
            ...
            for i, ax in enumerate(np.ravel(axs)):
                ...
                if i != 0:
                    for um in unique_markers:
                        mask = markers == um
                        _, caps, _ = ax.errorbar(wl_eff[mask], diff_fluxes[mask], ls=' ', marker=um, markersize=5, color='black', markeredgewidth=0.0, elinewidth=1.0, capsize=2, **kwargs)
                        for cap in caps:
                            cap.set_color('black')
                ...
                if i == 0:
                    ...
                    for j in range(n_bands):
                        ...
                        ax.fill_betweenx(x_plot, (y_grid - y_plot * w), (y_grid + y_plot * w), **kwargs)
                        ax.plot([wl_eff[j] - _lim_y, wl_eff[j] + _lim_y], [
                         median_flux, median_flux], color='black', linewidth=0.2)
            ...
            for ax in np.ravel(axs):
                ...
                ax.tick_params(which='minor', axis='both', length=2, width=axes_linewidth)
                ax.tick_params(which='major', axis='both', length=3.5, width=axes_linewidth)
                x0, x1 = ax.get_xlim()
                if yMin < 0.0:
                    ax.plot([x0, x1], [0.0, 0.0], color='gray', lw=0.8)
