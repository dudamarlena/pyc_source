...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/swsg/templates.py
...
DEFAULT_SIMPLE_TEMPLATE = BASE_DEFAULT_TEMPLATE.format(title='${title}', head='<meta charset="utf-8">', content='${content}')
DEFAULT_MAKO_TEMPLATE = DEFAULT_SIMPLE_TEMPLATE
DEFAULT_GENSHI_TEMPLATE = BASE_DEFAULT_TEMPLATE.format(title='${title}', head='<meta charset="utf-8" />', content='${Markup(content)}')
DEFAULT_JINJA_TEMPLATE = BASE_DEFAULT_TEMPLATE.format(title='{{ title }}', head='<meta charset="utf-8">', content='{{ content }}')
...
class UnsupportedTemplate(Exception):
    ...
    def __str__(self):
        return ('the template language {0} does either not exist or is not supported.').format(self.template_language)

    def __repr__(self):
        return ('{0}({1})').format(self.__class__.__name__, self.template_language)
