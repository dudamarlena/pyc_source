...
# [GCC 8.4.0]
# Embedded file name: build/bdist.cygwin-2.6.0-i686/egg/tvdbapi_client/api.py
# Compiled at: 2016-11-05 17:23:40
"""Provide API client for TVDB API v2.0 services."""
import json, logging, cachecontrol
...
class TVDBClient(object):
    ...
    def headers(self):
        ...
        if self.__token:
            self._headers.update(**{'Authorization': 'Bearer %s' % self.__token})
    ...
    def session(self):
        ...
        if self._session is None:
            self._session = cachecontrol.CacheControl(requests.Session(), cache=caches.FileCache('.tvdb_cache'))
    ...
    def _exec_request(self, service, method=None, path_args=None, data=None, params=None):
        ...
        if path_args is None:
            path_args = []
        req = {'method': method or 'get', 'url': ('/').join(str(a).strip('/') for a in [
    ...
    def _login(self):
        data = {'apikey': self.__apikey, 'username': self.__username, 
    ...
    def search_series(self, **kwargs):
        ...
        resp = self._exec_request('search', path_args=['series'], params=params)
    ...
    def get_episodes(self, series_id, **kwargs):
        ...
        return self._exec_request('series', path_args=[
         series_id, 'episodes', 'query'], params=params)['data']
    ...
    def get_episodes_summary(self, series_id):
        ...
        :rtype: dict
        """
        return self._exec_request('series', path_args=[series_id, 'episodes', 'summary'])['data']
    ...
    def get_series_image_info(self, series_id):
        ...
        :rtype: dict
        """
        return self._exec_request('series', path_args=[series_id, 'images'])['data']
