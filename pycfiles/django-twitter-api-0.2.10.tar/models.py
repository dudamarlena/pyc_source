...
# [GCC 8.4.0]
# Embedded file name: /Users/ramusus/workspace/manufacture/env/src/django-twitter-api/twitter_api/models.py
# Compiled at: 2016-02-25 14:18:18
...
__all__ = [
 'User', 'Status', 'TwitterContentError', 'TwitterModel', 'TwitterManager', 'UserManager']
...
class TwitterManager(models.Manager):
    ...
    def get_by_url(self, url):
        ...
        """
        m = re.findall('(?:https?://)?(?:www\\.)?twitter\\.com/([^/]+)/?', url)
        if not len(m):
    ...
    def parse_response(self, response, extra_fields=None):
        ...
        if isinstance(response, tweepy.models.Model):
...
class TwitterBaseModel(TwitterModel):
    ...
    fetched = models.DateTimeField('Fetched', null=True, blank=True)
...
class User(TwitterBaseModel):
    ...
    contributors_enabled = models.BooleanField('Contributors enabled', default=False)
    default_profile = models.BooleanField('Default profile', default=False)
    default_profile_image = models.BooleanField('Default profile image', default=False)
    follow_request_sent = models.BooleanField('Follow request sent', default=False)
    following = models.BooleanField('Following', default=False)
    geo_enabled = models.BooleanField('Geo enabled', default=False)
    is_translator = models.BooleanField('Is translator', default=False)
    notifications = models.BooleanField('Notifications', default=False)
    profile_use_background_image = models.BooleanField('Profile use background image', default=False)
    protected = models.BooleanField('Protected', default=False)
    ...
    objects = models.Manager()
    remote = UserManager(methods={'get': 'get_user'})
...
class Status(TwitterBaseModel):
    ...
    objects = models.Manager()
    remote = StatusManager(methods={'get': 'get_status'})
