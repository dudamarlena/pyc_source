...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/jenkviz/crawl.py
# Compiled at: 2013-09-24 09:49:07
...
class Crawl(object):
    ...
    def get_rest_children(self, build):
        ...
        job_api_url = job_url + '/api/xml?xpath=//downstreamProject/name&wrapper=names'
        ...
        body = getText(body)
        children_names = []
        match = re.findall('matched ([0-9]*) ', body)
        if len(match):
            for i in range(1, int(match[0]) + 1):
                body = self.fetch_from_server(job_url + '/api/xml?xpath=//downstreamProject[%d]/name' % i, ok_codes=[
                 200, 404])
                body = getText(body)
            ...
            for child in children_names:
                ...
                url += '/api/xml?depth=1&xpath=//build[action/cause/upstreamProject="%s"%%20and%%20action/cause/upstreamBuild=%s]/number' % (build.name, build.build_number)
                url = getText(url)
    ...
    def parse_build(self, url, body):
        if body.startswith('ERROR'):
            ...
            name = ret[3]
            build_number = ret[4]
            return Build(url, body, name, build_number, None, None, 'Unknown', [], self.server_url, '', [])
        else:
            ...
            host = extract_token(body, ' href="/jenkins/computer/', '"')
            ...
            if 'Started by upstream project' in body:
                ...
                upstream_urls = re.findall('href="([^"]+[0-9]/)"', upstream)
                trigger = None
            ...
            if downstream_builds:
                downstream_urls = re.findall('href="([^"]+[0-9]/)"', downstream_builds)
            build = Build(url, host, name, build_number, start, duration, status, downstream_urls, self.server_url, trigger, upstream_urls)
            return build
    ...
    def stats(self):
        ...
        throughput = 0
        if elapsed.seconds:
            throughput = duration * 100.0 / elapsed.seconds
        ret = {'start': start, 'stop': stop, 'elapsed': elapsed, 
           'duration': duration, 
