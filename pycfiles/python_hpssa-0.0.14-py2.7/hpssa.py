...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hpssa/hpssa.py
# Compiled at: 2019-03-15 10:56:45
import logging, re
...
def __parse_array_name(line):
    ...
    LOG.debug(('Found {} in slot {}').format(matches.group('name'), matches.group('slot')))
...
def __parse_array_line(line):
    line = line.strip()
    array_info = {'letter': line.split()[1], 
...
def __parse_ld_line(line):
    line = line.strip()
    attributes = [ x.strip() for x in line.split('(')[1].strip(')').split(',')
                 ]
...
def __parse_pd_line(line):
    ...
    attributes = [ x.strip() for x in line.split('(')[1].strip(')').split(',')[1:]
                 ]
...
def _update_spare_list(array_info, spare_list, pd):
    for spare in spare_list:
        if spare['port'] == pd['port'] and spare['box'] == pd['box'] and spare['bay'] == pd['bay']:
...
def parse_show_config(config):
    ...
    configuration = {'arrays': arrays, 
       'unassigned': unassigned, 'spares': spares}
            ...
            if array_info:
                if pd_info:
                    if pd_info.pop('spare'):
                        ...
                        array_info['physical_drives'].append(pd_info)
                elif ld_info:
                    array_info['logical_drives'].append(ld_info)
            if unassigned_drives:
                if pd_info:
                    unassigned.append(pd_info)
...
class HPSSA(object):
    ...
    def _raw_system_info(self):
        ...
        for adapter in adapters:
            if adapter.get('error'):
                LOG.debug(('Controller {} (slot {}) is in an error state: {}').format(adapter['name'], adapter['slot'], adapter['error']))
                continue
    ...
    def get_drive_configuration(self):
        ...
        for adapter in self.adapters:
            ...
            drive_list.append({'slot': adapter['slot'], 'drives': adapter['drives'], 
    ...
    def get_all_drives(self):
        ...
        for drive_config in self.get_drive_configuration():
            for drive in drive_config['drives']:
                all_drives.append(dict(slot=drive_config['slot'], drive_id=self.assemble_id(drive), **drive))
    ...
    @update_late
    def create(self, slot, selection=None, raid=None, array_letter=None, array_type='ld', size='max', stripe_size='default', write_policy='writeback', sectors=32, caching=True, data_ld=None, parity_init_method='default'):
        """
        ...
        if adapter.get('error'):
            LOG.debug(('Controller {} (slot {}) is in an error state: {}').format(adapter['name'], adapter['slot'], adapter['error']))
            return
        create_string = ('controller slot={} {}create').format(slot, array_letter and ('array {} ').format(array_letter) or '')
        command = ('{create_string} type={type}{drives}{raid} size={size} stripesize={stripe_size} forced').format(**{'create_string': create_string, 
           'slot': slot, 
           'type': array_type, 
        ...
        standard_array_options = {'sectors': sectors, 
           'caching': caching and 'enable' or 'disable'}
        cache_array_options = {'datald': data_ld, 
           'writepolicy': write_policy}
        ssd_array_options = {'ssdoverprovisioningoptimization': 'on'}
        parity_array_options = {'parityinitializationmethod': parity_init_method}
        ...
        LOG.info(('Creating LD - slot: {}, raid: {}, type: {} array: {}, selection: {}, size={}').format(slot, raid, array_type, array_letter, selection, size))
    ...
    def add_spares(self, slot, selection, array_letters=None):
        ...
        for array_letter in array_letters:
            LOG.info(('Adding spare - slot: {}, array: {}, disks: {}').format(slot, array_letter, selection))
    ...
    def _set_hba_mode(self, slot, state):
        LOG.info('Setting %s mode on controller in Slot %s', 'HBA' if state == 'on' else 'RAID', slot)
        cmd = ('ctrl slot={} modify hbamode={} forced').format(slot, state)
        result = self.run(cmd)
        LOG.warning('Command sent. The device must be rebooted for this change to take effect')
        return result
