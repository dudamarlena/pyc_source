...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insight/tests.py
...
def create_user(username, password):
    try:
        return User.objects.create_user(username, 'user@host.com', password)
    except TypeError:
...
class AuthUserTestCase(TestCase):
    ...
    def test_querystringparams_are_recorded(self):
        ...
        self.assertEqual(QuerystringParameter.objects.get(origin=origin, identifier='pid', value=123).number_of_registrations, 1)
        self.assertEqual(QuerystringParameter.objects.get(origin=origin, identifier='oid', value=444).number_of_registrations, 1)
        ...
        self.assertEqual(QuerystringParameter.objects.get(origin=origin, identifier='pid', value='123').number_of_registrations, 2)
        self.assertEqual(QuerystringParameter.objects.get(origin=origin, identifier='oid', value='444').number_of_registrations, 1)
        self.assertFalse(QuerystringParameter.objects.filter(origin=origin, identifier='kid').exists())
        self.assertEqual(QuerystringParameter.objects.get(origin=origin, identifier='gid', value=444).number_of_registrations, 1)
    ...
    def test_redirect(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response['Location'].split('/', 3)[(-1)], reverse('stub').lstrip('/'))

    def test_origin_hit_signal(self):
        origin = create_origin()
        signal_dict = {'instance': None, 
           'request': None, 
