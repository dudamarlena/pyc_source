...
# [GCC 8.4.0]
# Embedded file name: /media/data/work/hpp2plantuml-dbg/v0.6/venv/lib/python3.7/site-packages/hpp2plantuml/hpp2plantuml.py
# Compiled at: 2019-10-26 19:27:44
# Size of source mod 2**32: 45517 bytes
import os, re, glob, argparse, CppHeaderParser, jinja2
MEMBER_PROP_MAP = {'private':'-', 
 'public':'+', 
 'protected':'#'}
LINK_TYPE_MAP = {'inherit':'<|--', 
 'aggregation':'o--', 
 'composition':'*--', 
 'dependency':'<..'}
CONTAINER_TYPE_MAP = [
...
class Container(object):
    ...
    def _do_parse_members(self, header_container):
        ...
        header_container : CppClass, CppStruct or CppEnum
            Parsed header for container
        """
        raise NotImplementedError('Derived class must implement :func:`_do_parse_members`.')
    ...
    def comparison_keys(self):
        ...
        """
        return (
         self._container_type, self._name)
...
class Class(Container):
    ...
    def __init__(self, header_class):
        ...
        if 'template' in header_class[1]:
            self._template_type = _cleanup_single_line(header_class[1]['template'])
        self._inheritance_list = [re.sub('<.*>', '', parent['class']) for parent in header_class[1]['inherits']]
        ...
        for member_type, member_type_handler in member_type_map:
            for member_prop in MEMBER_PROP_MAP.keys():
                ...
                for header_member in member_list:
                    self._member_list.append(member_type_handler(header_member, member_prop))
...
class ClassMember(ContainerMember):
    ...
    def render(self):
        ...
        if len(self._properties) > 0:
            ...
            props = ''
        vis = MEMBER_PROP_MAP[self._scope] + ('{static} ' if self._static else '')
        member_str = vis + self._render_name() + (' : ' + self._type if self._type else '') + props
        return member_str
...
class ClassVariable(ClassMember):
    ...
    def __init__(self, class_variable, member_scope='private'):
        ...
        """
        assert isinstance(class_variable, CppHeaderParser.CppHeaderParser.CppVariable)
...
class ClassMethod(ClassMember):
    ...
    def __init__(self, class_method, member_scope):
        if not isinstance(class_method, CppHeaderParser.CppHeaderParser.CppMethod):
            ...
            if class_method['returns_pointer']:
                ...
                if class_method['returns_reference']:
                    self._type += '&'
    ...
    def _render_name(self):
        ...
        method_str = ('{abstract} ' if self._abstract else '') + self._name + '(' + ', '.join((' '.join(it).strip() for it in self._param_list)) + ')'
        return method_str


class Struct(Class):
    __doc__ = 'Representation of C++ struct objects\n\n    This class derived is almost identical to `Class`, the only difference\n    being the container type name ("struct" instead of "class").\n    '
...
class ClassRelationship(object):
    ...
    def comparison_keys(self):
        ...
        """
        return (
         self._parent, self._child, self._link_type)
    ...
    def render(self):
        ...
        link_str = ''
        namespace_wrap = None
        if self._parent_namespace == self._child_namespace:
            if self._parent_namespace is not None:
                namespace_wrap = self._parent_namespace
        flag_render_namespace = self._flag_use_namespace and not namespace_wrap
        parent_str = self._render_name(self._parent, self._parent_namespace, flag_render_namespace)
        child_str = self._render_name(self._child, self._child_namespace, flag_render_namespace)
...
class Diagram(object):
    ...
    def __init__(self, template_file=None, flag_dep=False):
        ...
        if template_file is not None:
            loader_list.append(jinja2.FileSystemLoader(os.path.abspath(os.path.dirname(template_file))))
        ...
        self._env = jinja2.Environment(loader=(jinja2.ChoiceLoader(loader_list)),
    ...
    def _build_helper(self, input, build_from='string', flag_build_lists=True, flag_reset=False):
        ...
        if flag_reset:
            self.clear()
        elif build_from in ('string', 'file'):
            self.parse_objects(input, build_from)
        else:
            if build_from in ('string_list', 'file_list'):
    ...
    def create_from_file(self, header_file):
        ...
        """
        self._build_helper(header_file, build_from='file', flag_build_lists=True,
          flag_reset=True)
    ...
    def create_from_file_list(self, file_list):
        ...
        """
        self._build_helper(file_list, build_from='file_list', flag_build_lists=True,
          flag_reset=True)
    ...
    def add_from_file(self, header_file):
        ...
        """
        self._build_helper(header_file, build_from='file', flag_build_lists=False,
          flag_reset=False)
    ...
    def add_from_file_list(self, file_list):
        ...
        """
        self._build_helper(file_list, build_from='file_list', flag_build_lists=False,
          flag_reset=False)
    ...
    def create_from_string(self, header_string):
        ...
        input, building the relationship lists and with object reset.
        """
        self._build_helper(header_string, build_from='string', flag_build_lists=True,
          flag_reset=True)
    ...
    def create_from_string_list(self, string_list):
        ...
        reset.
        """
        self._build_helper(string_list, build_from='string_list', flag_build_lists=True,
          flag_reset=True)
    ...
    def add_from_string(self, header_string):
        ...
        reset (new objects are added to the object).
        """
        self._build_helper(header_string, build_from='string', flag_build_lists=False,
          flag_reset=False)
    ...
    def add_from_string_list(self, string_list):
        ...
        """
        self._build_helper(string_list, build_from='string_list', flag_build_lists=False,
          flag_reset=False)
    ...
    def parse_objects(self, header_file, arg_type='string'):
        ...
        """
        parsed_header = CppHeaderParser.CppHeader(header_file, argType=arg_type)
        for container_type, container_iterator, container_handler in CONTAINER_TYPE_MAP:
    ...
    def _make_class_list(self):
        ...
        """
        return [{'name':obj.get_name(),  'obj':obj} for obj in self._objects if isinstance(obj, Class)]
            ...
            if isinstance(obj, Class):
                for parent in obj.build_inheritance_list():
                    if parent in class_list:
                        parent_obj = class_list_obj[class_list.index(parent)]['obj']
                        self._inheritance_list.append(ClassInheritanceRelationship(parent_obj,
                          obj, flag_use_namespace=flag_use_namespace))
    ...
    def build_aggregation_list(self):
            ...
            if child_class in variable_type_list.keys():
                ...
                for var_type in var_types:
                    for parent in class_list:
                        if re.search('\\b' + parent + '\\b', var_type):
                            rel_type = 'composition'
                            if '{}*'.format(parent) in var_type:
                                rel_type = 'aggregation'
                            self._augment_comp(aggregation_counts, parent, child_class,
                              rel_type=rel_type)
        ...
        for obj_class, obj_comp_list in aggregation_counts.items():
            for comp_parent, rel_type, comp_count in obj_comp_list:
                ...
                self._aggregation_list.append(ClassAggregationRelationship(obj_class_obj,
                  comp_parent_obj, comp_count, rel_type=rel_type,
                ...
                for member in obj._member_list:
                    if isinstance(member, ClassMethod):
                        for method in member._param_list:
                            index = ValueError
                            try:
                                index = [re.search(o, method[0]) is not None for o in objects_name].index(True)
                            except ValueError:
                                pass

                            if index != ValueError and method[0] != obj.get_name():
                                depend_obj = self._objects[index]
                                self._dependency_list.append(ClassDependencyRelationship(depend_obj,
                                  obj, flag_use_namespace=flag_use_namespace))
    ...
    def render(self):
        ...
        return template.render(objects=(self._objects), inheritance_list=(self._inheritance_list),
...
def _cleanup_type(type_str):
    ...
    """
    return re.sub('[ ]+([*&])', '\\1', re.sub('(\\s)+', '\\1', type_str))
...
def main():
    ...
    parser.add_argument('-i', '--input-file', dest='input_files', action='append',
      metavar='HEADER-FILE',
      required=True,
      help='input file (must be quoted when using wildcards)')
    parser.add_argument('-o', '--output-file', dest='output_file', required=False,
      default=None,
      metavar='FILE',
      help='output file')
    parser.add_argument('-d', '--enable-dependency', dest='flag_dep', required=False,
      default=False,
      action='store_true',
      help='Extract dependency relationships from method arguments')
    parser.add_argument('-t', '--template-file', dest='template_file', required=False,
      default=None,
      metavar='JINJA-FILE',
    ...
    if len(args.input_files) > 0:
        CreatePlantUMLFile((args.input_files), (args.output_file), template_file=(args.template_file),
