...
# [GCC 8.4.0]
# Embedded file name: /home/oshane/Workspace/osoobe/packages/kotti/src/kotti_history/kotti_history/views/view.py
# Compiled at: 2017-01-06 12:46:45
"""
...
from kotti_history.decorators import login_required

@view_config(name='history-recorder', renderer='kotti_history:templates/recorder.pt')
...
class SearchHistoryView(BaseView):
    ...
    @view_config(name='save-search', root_only=True, permission='view', decorator=login_required, request_method='POST', renderer='json')
    ...
    @view_config(name='previous-searches', root_only=True, permission='view', decorator=login_required, renderer='kotti_history:templates/searches.pt')
...
class DefaultView(BaseView):
    ...
    @view_config(name='all-history', request_method='GET', root_only=True, renderer='kotti_history:templates/history.pt', permission='admin')
...
class Viewer(BaseView):
    ...
    def save_view_history(self):
        ...
        try:
            history = ViewHistory.find(content_id=self.context.id, username=self.request.user.name).one()
            ...
            if today > history.date_last_viewed:
                ...
                return dict(status='success', message='Already updated', num_views=num_views)
        except NoResultFound as e:
            history = ViewHistory.create(content=self.context, user=self.request.user)
            num_views = history.num_views
            DBSession.add(history)

        return {'status': 'success', 'message': ('View count has been changed to {}').format(num_views), 
           'num_views': num_views}