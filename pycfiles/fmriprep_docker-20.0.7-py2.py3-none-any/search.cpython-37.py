...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-aejasjrz/pip/pip/_internal/commands/search.py
...
class SearchCommand(Command, SessionCommandMixin):
    __doc__ = 'Search for PyPI packages whose name or summary contains <query>.'
    usage = '\n      %prog [options] <query>'
    ignore_require_venv = True

    def __init__(self, *args, **kw):
        (super(SearchCommand, self).__init__)(*args, **kw)
        self.cmd_opts.add_option('-i',
          '--index', dest='index',
          metavar='URL',
...
def print_results(hits, name_column_width=None, terminal_width=None):
    ...
    for hit in hits:
        ...
        line = ('{name_latest:{name_column_width}} - {summary}'.format)(name_latest=('{name} ({latest})'.format)(**locals()), **locals())
        try:
            write_output(line)
