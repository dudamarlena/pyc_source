...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-aejasjrz/setuptools/setuptools/build_meta.py
...
"""
import io, os, sys, tokenize, shutil, contextlib, setuptools, distutils
...
class _BuildMetaBackend(object):
    ...
    def prepare_metadata_for_build_wheel(self, metadata_directory, config_settings=None):
        ...
        while True:
            dist_infos = [f for f in os.listdir(dist_info_directory) if f.endswith('.dist-info')]
            if len(dist_infos) == 0:
                if len(_get_immediate_subdirectories(dist_info_directory)) == 1:
                    dist_info_directory = os.path.join(dist_info_directory, os.listdir(dist_info_directory)[0])
                    continue
            assert len(dist_infos) == 1
            break

        if dist_info_directory != metadata_directory:
            shutil.move(os.path.join(dist_info_directory, dist_infos[0]), metadata_directory)
    ...
    def _build_with_temp_dir(self, setup_command, result_extension, result_directory, config_settings):
        ...
        with TemporaryDirectory(dir=result_directory) as (tmp_dist_dir):
            sys.argv = sys.argv[:1] + setup_command + ['--dist-dir', tmp_dist_dir] + config_settings['--global-option']
            self.run_setup()
    ...
    def build_wheel(self, wheel_directory, config_settings=None, metadata_directory=None):
        return self._build_with_temp_dir(['bdist_wheel'], '.whl', wheel_directory, config_settings)
