...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-aejasjrz/pip/pip/_internal/req/req_file.py
# Compiled at: 2020-05-05 12:41:47
# Size of source mod 2**32: 19075 bytes
"""
...
if MYPY_CHECK_RUNNING:
    ...
    from typing import Any, Callable, Dict, Iterator, List, NoReturn, Optional, Text, Tuple
...
def parse_requirements(filename, session, finder=None, comes_from=None, options=None, constraint=False):
...
def handle_requirement_line(line, options=None):
    ...
    if line.is_editable:
        return ParsedRequirement(requirement=(line.requirement),
...
class RequirementsFileParser(object):
    ...
    def __init__(self, session, line_parser, comes_from):
    ...
    if scheme == 'file':
        if comes_from:
            if comes_from.startswith('http'):
                raise InstallationError('Requirements file {} references URL {}, which is local'.format(comes_from, url))
        path = url.split(':', 1)[1]
        path = path.replace('\\', '/')
