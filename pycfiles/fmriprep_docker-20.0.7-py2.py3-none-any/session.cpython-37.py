...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-aejasjrz/pip/pip/_internal/network/session.py
# Compiled at: 2020-05-05 12:41:47
# Size of source mod 2**32: 15208 bytes
...
class InsecureCacheControlAdapter(CacheControlAdapter):
    ...
    def cert_verify(self, conn, url, verify, cert):
        super(InsecureCacheControlAdapter, self).cert_verify(conn=conn,
          url=url,
          verify=False,
          cert=cert)
...
class PipSession(requests.Session):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if cache:
            secure_adapter = CacheControlAdapter(cache=(SafeFileCache(cache)),
              max_retries=retries)
            self._trusted_host_adapter = InsecureCacheControlAdapter(cache=(SafeFileCache(cache)),
              max_retries=retries)
        else:
    ...
    def add_trusted_host(self, host, source=None, suppress_logging=False):
        ...
        if not suppress_logging:
            ...
            host_port[1] or self.mount(build_url_from_netloc(host) + ':', self._trusted_host_adapter)
