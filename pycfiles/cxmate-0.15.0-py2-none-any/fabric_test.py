...
# Embedded file name: /home/travis/virtualenv/python2.7.9/lib/python2.7/site-packages/cxmanage_api/tests/fabric_test.py
# Compiled at: 2017-02-08 04:42:30
__doc__ = 'Calxeda: fabric_test.py '
...
class FabricTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.fabric._nodes = dict((i, self.nodes[i]) for i in xrange(len(self.nodes)))
    ...
    def test_get_mac_addresses(self):
        ...
        self.assertEqual(self.nodes[0].method_calls, [
    ...
    def test_set_uplink(self):
        ...
        self.assertEqual(self.nodes[0].bmc.method_calls, [
    ...
    def test_set_boot_order(self):
        ...
        for node in self.nodes:
            self.assertEqual(node.method_calls, [call.set_boot_order(boot_args)])
    ...
    def test_set_pxe_interface(self):
        ...
        for node in self.nodes:
            self.assertEqual(node.method_calls, [call.set_pxe_interface('eth0')])
    ...
    def test_ipmitool_command(self):
        ...
        for node in self.nodes:
            self.assertEqual(node.method_calls, [call.ipmitool_command(ipmitool_args)])
    ...
    def test_failed_command(self):
        ...
        self.fabric._nodes = dict((i, fail_nodes[i]) for i in xrange(len(self.nodes)))
        try:
        ...
        for node in self.fabric.nodes.values():
            if node == self.fabric.primary_node:
                self.assertEqual(node.bmc.method_calls, [
                 call.fabric_config_set_macaddr_base(macaddr='00:11:22:33:44:55')])
            else:
                self.assertEqual(node.bmc.method_calls, [])
    ...
    def test_set_macaddr_mask(self):
        ...
        for node in self.fabric.nodes.values():
            if node == self.fabric.primary_node:
                self.assertEqual(node.bmc.method_calls, [
                 call.fabric_config_set_macaddr_mask(mask='00:11:22:33:44:55')])
            else:
                self.assertEqual(node.bmc.method_calls, [])
