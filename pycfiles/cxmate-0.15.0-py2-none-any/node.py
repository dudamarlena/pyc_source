...
# Embedded file name: /home/travis/virtualenv/python2.7.9/lib/python2.7/site-packages/cxmanage_api/node.py
# Compiled at: 2017-02-08 04:42:30
__doc__ = 'Calxeda: node.py'
import os, re, time, tempfile, socket, subprocess
...
from cxmanage_api.cx_exceptions import TimeoutError, NoSensorError, SocmanVersionError, FirmwareConfigError, PriorityIncrementError, NoPartitionError, TransferFailure, ImageSizeError, PartitionInUseError, UbootenvError, EEPROMUpdateError, ParseError, NodeMismatchError
...
class Node(object):
    ...
    def __init__(self, ip_address, credentials=None, tftp=None, ecme_tftp_port=5001, verbose=False, bmc=None, image=None, ubootenv=None, ipretriever=None):
        ...
        self.bmc = make_bmc(bmc, hostname=ip_address, username=self.credentials.ecme_username, password=self.credentials.ecme_password, verbose=verbose)
        self.image = image
    ...
    def refresh(self, new_node):
        ...
        if not isinstance(new_node, Node) or not new_node.guid == self.guid:
            raise NodeMismatchError('Passed in node does not match node to be updated')
    ...
    def get_sensors(self, search=''):
        ...
        """
        sensors = [ x for x in self.bmc.sdr_list() if search.lower() in x.sensor_name.lower()
                  ]
        if len(sensors) == 0:
            if search == '':
                raise NoSensorError('No sensors were found')
            else:
                raise NoSensorError('No sensors containing "%s" were found' % search)
    ...
    def get_sensors_dict(self, search=''):
        ...
        """
        return dict((key, vars(value)) for key, value in self.get_sensors(search=search).items())
    ...
    def get_firmware_info(self):
        ...
        """
        fwinfo = [ x for x in self.bmc.get_firmware_info() if hasattr(x, 'partition')
                 ]
        for entry in fwinfo:
            if entry.version == '':
    ...
    def is_updatable(self, package, partition_arg='INACTIVE', priority=None):
        ...
        try:
            ...
            return True
        except (SocmanVersionError, FirmwareConfigError, PriorityIncrementError, NoPartitionError, ImageSizeError,
         PartitionInUseError):
            return False
    ...
    def update_firmware(self, package, partition_arg='INACTIVE', priority=None):
        ...
        logger.info('Firmware Update Log for Node %d' % self.node_id)
        logger.info('ECME IP address: ' + self.ip_address)
        version_info = self.get_versions()
        logger.info('\nOld firmware version: ' + version_info.firmware_version)
        if package.version:
            logger.info('New firmware version: ' + package.version)
        else:
            logger.warn('New firmware version name unavailable.')
        logger.info('\n[ Pre-Update Firmware Info for Node %d ]' % self.node_id)
        fwinfo = self.get_firmware_info()
        num_ubootenv_partitions = len([ x for x in fwinfo if 'UBOOTENV' in x.type
                                      ])
        ...
        for image in package.images:
            if image.type == 'UBOOTENV' and num_ubootenv_partitions >= 2:
                ...
                old_ubootenv = self.ubootenv(open(old_ubootenv_image.filename).read())
                try:
                    ...
                    ubootenv_image = self.image(filename, image.type, False, image.daddr, image.skip_crc32, image.version)
                    logger.info('Uploading %s to %s\n' % (image, running_part))
                    self._upload_image(ubootenv_image, running_part, priority)
                ...
                if partition_arg == 'BOTH':
                    partitions = [self._get_partition(fwinfo, image.type, 'FIRST'),
        ...
        for old_partition in updated_partitions:
            ...
            if new_partition.type != old_partition.type:
                logger.error('Update failed (partition %i, type changed)' % partition_id)
                raise Exception('Update failed (partition %i, type changed)' % partition_id)
            if int(new_partition.priority, 16) != priority:
                logger.error('Update failed (partition %i, wrong priority)' % partition_id)
                raise Exception('Update failed (partition %i, wrong priority)' % partition_id)
            if int(new_partition.flags, 16) & 2 != 0:
                logger.error('Update failed (partition %i, not activated)' % partition_id)
                raise Exception('Update failed (partition %i, not activated)' % partition_id)
            self.bmc.check_firmware(partition_id)
            logger.info('Check complete for partition %d' % partition_id)
    ...
    def update_node_eeprom(self, image):
        ...
        :raises EEPROMUpdateError: When an error is encountered while updating the EEPROM

        """
        if not os.path.exists(image):
            raise EEPROMUpdateError('%s does not exist' % image)
        node_hw_ver = self.get_versions().hardware_version
        if 'Dual Node' not in node_hw_ver:
            raise EEPROMUpdateError('eepromupdate is only valid on TerraNova systems')
        ...
        if image_prefix not in image:
            raise EEPROMUpdateError('%s is not a valid node EEPROM image for this node' % image)
    ...
    def update_slot_eeprom(self, image):
        ...
        if 'tn_storage.single_slot' not in image:
            raise EEPROMUpdateError('%s is an invalid image for slot EEPROM' % image)
    ...
    def get_versions(self):
        ...
        if result.firmware_version.startswith('ECX-1000'):
            ...
            if result.firmware_version.startswith('ECX-2000'):
    ...
    def ipmitool_command(self, ipmitool_args):
        r"""Send a raw ipmitool command to the node.
    ...
    def get_fabric_ipinfo(self, allow_errors=False):
        ...
        """
        contents = self.run_fabric_tftp_command(function_name='fabric_config_get_ip_info')
        results = {}
        for line in contents.splitlines():
            if line.strip():
                ...
                try:
                    ...
                    if allow_errors:
                        continue
                    raise ParseError('Invalid IP address %s\n%s' % (ip_address, contents))

                if ip_address == '0.0.0.0':
                    if allow_errors:
                        continue
                    raise ParseError('Invalid IP address 0.0.0.0\n%s' % contents)
    ...
    def get_fabric_macaddrs(self):
        ...
        """
        contents = self.run_fabric_tftp_command(function_name='fabric_config_get_mac_addresses')
        results = {}
        for line in contents.splitlines():
            if line.strip():
                elements = line.split()
    ...
    def get_fabric_uplink_info(self):
        ...
        """
        contents = self.run_fabric_tftp_command(function_name='fabric_config_get_uplink_info')
        results = {}
        for line in contents.splitlines():
    ...
    def get_link_stats(self, link=0):
        ...
        """
        contents = self.run_fabric_tftp_command(function_name='fabric_get_linkstats', link=link)
        results = {}
        for line in contents.splitlines():
            if '=' in line:
                reg_value = line.strip().split('=')
                if len(reg_value) < 2:
                    raise ValueError('Register: %s has no value!' % reg_value[0])
                else:
                    results[reg_value[0].replace('pFS_LCn', 'FS_LC%s' % link).replace('(link)', '').strip()] = reg_value[1].strip()
    ...
    def get_linkmap(self):
        ...
        """
        contents = self.run_fabric_tftp_command(function_name='fabric_info_get_link_map')
        results = {}
        for line in contents.splitlines():
    ...
    def get_routing_table(self):
        ...
        """
        contents = self.run_fabric_tftp_command(function_name='fabric_info_get_routing_table')
        results = {}
        for line in contents.splitlines():
    ...
    def get_depth_chart(self):
        ...
        """
        contents = self.run_fabric_tftp_command(function_name='fabric_info_get_depth_chart')
        results = {}
        for line in contents.splitlines():
            if line.startswith('Node'):
                elements = line.strip().split()
    ...
    def get_server_ip(self, interface=None, ipv6=False, aggressive=False):
        ...
        retriever = self.ipretriever(self.ip_address, aggressive=aggressive, verbosity=verbosity, server_user=self.credentials.linux_username, server_password=self.credentials.linux_password, interface=interface, ipv6=ipv6, bmc=self.bmc)
        retriever.run()
    ...
    def run_fabric_tftp_command(self, function_name, **kwargs):
        ...
        try:
            ...
            getattr(self.bmc, function_name)(filename=basename, tftp_addr=self.tftp_address, **kwargs)
    ...
    def _upload_image(self, image, partition, priority=None):
        ...
        for _ in xrange(2):
            ...
            result = self.bmc.update_firmware(basename, partition_id, image.type, self.tftp_address)
    ...
    def _download_image(self, partition):
        ...
        for _ in xrange(2):
            ...
            result = self.bmc.retrieve_firmware(basename, partition_id, image_type, self.tftp_address)
            self._wait_for_transfer(result.tftp_handle_id)
            self.tftp.get_file(basename, filename)

        return self.image(filename=filename, image_type=image_type, daddr=int(partition.daddr, 16), version=partition.version)
    ...
    def _check_firmware(self, package, partition_arg='INACTIVE', priority=None):
        ...
        if package.version and info.firmware_version:
            ...
            if package_match and firmware_match:
                ...
                if package_version != firmware_version:
                    raise FirmwareConfigError('Refusing to upload an %s package to an %s host' % (
        ...
        if package.required_socman_version:
            ...
            if (package.required_socman_version and LooseVersion(ecme_version)) < LooseVersion(required_version):
                raise SocmanVersionError('Update requires socman version %s (found %s)' % (
                 required_version, ecme_version))
        if package.config and info.firmware_version != 'Unknown' and len(info.firmware_version) < 32:
            firmware_config = 'default'
            if package.config != firmware_config:
                raise FirmwareConfigError("Refusing to upload a '%s' package to a '%s' host" % (
                 package.config, firmware_config))
        if priority == None:
            priority = self._get_next_priority(fwinfo, package)
        for image in package.images:
            if image.type == 'UBOOTENV' and num_ubootenv_partitions >= 2 or partition_arg == 'BOTH':
                partitions = [ self._get_partition(fwinfo, image.type, x) for x in ['FIRST', 'SECOND'] ]
            else:
                partitions = [
                 self._get_partition(fwinfo, image.type, partition_arg)]
            for partition in partitions:
                ...
                if image.type in ('CDB', 'BOOT_LOG') and partition.in_use == '1':
                    raise PartitionInUseError("Can't upload to a CDB/BOOT_LOG partition " + "that's in use")
    ...
    def _get_next_priority(fwinfo, package):
        ...
        if priority > 65535:
            raise PriorityIncrementError('Unable to increment SIMG priority, too high')
        return priority