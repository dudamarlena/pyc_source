...
# Embedded file name: /home/travis/virtualenv/python2.7.9/lib/python2.7/site-packages/cxmanage_api/ip_retriever.py
# Compiled at: 2017-02-08 04:42:30
__doc__ = 'Calxeda: ip_retriever.py'
...
class IPRetriever(threading.Thread):
    ...
    def __init__(self, ecme_ip, aggressive=False, verbosity=0, **kwargs):
        ...
        if '_inet_pattern' in kwargs and '_ip_pattern' in kwargs:
            ...
            self.set_interface(kwargs.get('interface', None), kwargs.get('ipv6', False))
        if 'bmc' in kwargs:
            self._bmc = kwargs['bmc']
        else:
            self._bmc = make_bmc(LanBMC, verbose=self.verbosity > 1, hostname=self.ecme_ip, username=self.ecme_user, password=self.ecme_password)
    ...
    def set_interface(self, interface=None, ipv6=False):
        ...
        if not ipv6:
            self._ip_pattern = re.compile('\\d+\\.\\d+\\.\\d+\\.\\d+')
            self._inet_pattern = re.compile('inet addr:(%s)' % self._ip_pattern.pattern)
        else:
            self._ip_pattern = re.compile('[0-9a-fA-F:]*:' * 2 + '[0-9a-fA-F:]+')
            self._inet_pattern = re.compile('inet6 addr: ?(%s)' % self._ip_pattern.pattern)
    ...
    def sol_find_ip(self, session):
        ...
        if index == 0:
            ...
            raise IPDiscoveryError('Interface %s does not have given address' % self.interface)
        elif index == 1:
            self._bmc.deactivate_payload()
            raise IPDiscoveryError('Could not find interface %s' % self.interface)
        else:
            return
        return
    ...
    def sol_try_command(self, command):
        ...
        while attempt < 7:
            ...
            if index == 2:
                ...
                self._bmc.deactivate_payload()
                raise IPDiscoveryError('Unable to boot linux due to an invalid boot device')
            elif index == 4:
                ...
                if attempt == 0:
                    ...
                    sleep(2)
                    self._bmc.deactivate_payload()
                    raise IPDiscoveryError("Unable to obtain the server's IP address unintrusively")
                elif attempt == 1:
    ...
    def write_config(self, path):
        ...
        config_data = {'ecme_host': self.ecme_ip, 'server_host': self.server_ip}
