...
# Embedded file name: /home/travis/virtualenv/python2.7.9/lib/python2.7/site-packages/cxmanage_api/cli/commands/fw.py
# Compiled at: 2017-02-08 04:42:30
__doc__ = 'Calxeda: fw.py '
from pkg_resources import parse_version
from cxmanage_api.cli import get_tftp, get_nodes, get_node_strings, run_command, prompt_yes
...
def fwupdate_command(args):
    ...
    def do_update():
        ...
        if not args.force:
            ...
            _, errors = run_command(args, nodes, '_check_firmware', package, args.partition, args.priority)
            if errors:
                print 'ERROR: Firmware update aborted.'
                return True
        if not args.quiet:
            print 'Updating firmware...'
        _, errors = run_command(args, nodes, 'update_firmware', package, args.partition, args.priority)
        if errors:
    ...
    if args.image_type == 'PACKAGE':
        ...
        try:
            ...
            if args.force_simg:
                ...
                simg = True
            image = Image(args.filename, args.image_type, simg, args.daddr, args.skip_crc32, args.fw_version)
    ...
    if not args.all_nodes:
        ...
        elif not prompt_yes('WARNING: Updating firmware without ' + '--all-nodes is dangerous. Continue?'):
            return 1
    tftp = get_tftp(args)
