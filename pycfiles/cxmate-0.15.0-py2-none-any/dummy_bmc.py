...
# Embedded file name: /home/travis/virtualenv/python2.7.9/lib/python2.7/site-packages/cxmanage_api/tests/dummy_bmc.py
# Compiled at: 2017-02-08 04:42:30
__doc__ = ' Module for the DummyBMC class '
...
class DummyBMC(Dummy):
    ...
    dummy_spec = LanBMC
    ip_addresses = [ '192.168.100.%i' % n for n in range(1, 5) ]
    tftp = InternalTftp()
    GUID_UNIQUE = 0
    ...
    def __init__(self, **kwargs):
        ...
        self.partitions = [
         Partition(0, 3, 0, 393216, in_use=True),
         Partition(1, 10, 393216, 196608, in_use=True),
         Partition(2, 3, 589824, 393216, in_use=False),
         Partition(3, 10, 983040, 196608, in_use=False),
         Partition(4, 10, 1179648, 196608, in_use=True),
         Partition(5, 11, 1376256, 12288),
         Partition(6, 11, 1388544, 12288)]
    ...
    def generate_sel(with_errors=False):
        ...
        if with_errors:
            return [
             ...
             '83 | 11/14/2013 | 18:01:35 | OS Stop/Shutdown OS Stop Reason | ' + 'Error during system startup | Asserted']
        else:
            return [
             '88 | 11/14/2013 | 18:02:29 | System Boot Initiated OS Boot ' + 'Reason | Initiated by power up | Asserted',
    ...
    def get_info_basic(self):
        ...
        return Result(iana=int('0x0096CD', 16), firmware_version='ECX-0000-v0.0.0', ecme_version='v0.0.0', ecme_timestamp=0)
...
class Partition(object):
    ...
    def __init__(self, partition, type_, offset=0, size=0, priority=0, daddr=0, in_use=None):
        self.updates = 0
        self.retrieves = 0
        self.checks = 0
        self.activates = 0
        self.fwinfo = FWInfoEntry(partition, type_, offset, size, priority, daddr, in_use)
...
class FWInfoEntry(object):
    ...
    def __init__(self, partition, type_, offset=0, size=0, priority=0, daddr=0, in_use=None):
        ...
        self.type = {2: '02 (S2_ELF)', 
           3: '03 (SOC_ELF)', 
           10: '0a (CDB)', 
           11: '0b (UBOOTENV)'}[type_]
