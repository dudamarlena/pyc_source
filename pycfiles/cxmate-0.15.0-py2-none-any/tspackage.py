...
# Embedded file name: /home/travis/virtualenv/python2.7.9/lib/python2.7/site-packages/cxmanage_api/cli/commands/tspackage.py
# Compiled at: 2017-02-08 04:42:30
__doc__ = 'Calxeda: tspackage.py'
import os, shutil, subprocess, sys, tarfile, tempfile, time, pyipmi, cxmanage_api
...
def write_client_info():
    ...
    with open('client.txt', 'w') as (fout):
        ...
        def write_command(command):
            ...
            try:
                fout.write(subprocess.check_output(command, stderr=subprocess.STDOUT, shell=True))
...
def write_version_info(args, nodes):
    ...
    for node in nodes:
        ...
        if node in info_results:
            ...
            lines.append('Hardware version    : %s' % info_result.hardware_version)
            lines.append('Firmware version    : %s' % info_result.firmware_version)
...
def write_mac_addrs(args, nodes):
    ...
    mac_addr_results, _ = run_command(args, nodes, 'get_fabric_macaddrs')
    for node in nodes:
        lines = []
...
def write_sensor_info(args, nodes):
    ...
    args.sensor_name = ''
    results, _ = run_command(args, nodes, 'get_sensors', args.sensor_name)
    for node in nodes:
            ...
            for key in depth_chart:
                ...
                for subkey in subchart:
                    if str(subkey) == 'shortest':
                        lines.append('  ' + str(subkey) + ' : ' + str(subchart[subkey]))
                    else:
                        for entry in subchart[subkey]:
                            lines.append('  ' + str(subkey) + '   : ' + str(entry))
