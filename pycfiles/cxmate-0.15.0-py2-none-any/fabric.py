...
# Embedded file name: /home/travis/virtualenv/python2.7.9/lib/python2.7/site-packages/cxmanage_api/fabric.py
# Compiled at: 2017-02-08 04:42:30
__doc__ = ' Calxeda: fabric.py '
import time, re
...
from cxmanage_api.cx_exceptions import CommandFailedError, IpmiError, TftpException, ParseError, TimeoutError
...
class Fabric(object):
    ...
    class CompositeBMC(object):
        ...
        def __getattr__(self, name):
            ...
            for node in nodes.values():
                if not hasattr(node.bmc, name) or not hasattr(getattr(node.bmc, name), '__call__'):
                    raise AttributeError("'CompositeBMC' object has no attribute '%s'" % name)

            def function(*args, **kwargs):
                """ Run the named BMC command in parallel across all nodes. """
                tasks = {}
                for node_id, node in nodes.iteritems():
                    tasks[node_id] = task_queue.put(getattr(node.bmc, name), *args, **kwargs)
    ...
    def __init__(self, ip_address, credentials=None, tftp=None, ecme_tftp_port=5001, task_queue=None, verbose=False, node=None):
    ...
    def refresh(self, wait=False, timeout=600):
        ...
        def get_nodes():
            ...
            new_nodes = {}
            root_node = self.node(ip_address=self.ip_address, credentials=self.credentials, tftp=self.tftp, ecme_tftp_port=self.ecme_tftp_port, verbose=self.verbose)
            ipinfo = root_node.get_fabric_ipinfo()
            for node_id, node_address in ipinfo.items():
                node = self.node(ip_address=node_address, credentials=self.credentials, tftp=self.tftp, ecme_tftp_port=self.ecme_tftp_port, verbose=self.verbose)
        ...
        if wait:
            error = TimeoutError('Timeout after %s seconds occurred.' % timeout)
    ...
    def get_networks(self):
        ...
        """
        results = {}
        contents = self.primary_node.run_fabric_tftp_command('fabric_config_get_networks')
        ...
        for line in contents.splitlines():
            try:
                ...
                raise ParseError('Unable to parse networks\n%s' % contents)
    ...
    def add_network(self, name, private=False):
        ...
        """
        self.primary_node.bmc.fabric_config_add_network(name=name, private=private)
    ...
    def assign_network_to_uplink(self, name, uplink):
        ...
        """
        self.primary_node.bmc.fabric_config_add_network(name=name, uplink=uplink)
    ...
    def unassign_network_from_uplink(self, name, uplink):
        ...
        """
        self.primary_node.bmc.fabric_config_rm_network(name=name, uplink=uplink)
    ...
    def assign_interface_to_network(self, interface, network):
        ...
        """
        self.primary_node.bmc.fabric_config_set_mac(mac=interface, network=network)
    ...
    def get_network_assignment(self, interface):
        ...
        """
        return self.primary_node.bmc.fabric_config_get_network_mac(mac=interface)
    ...
    def get_uplinks(self):
        ...
        """
        results = {}
        contents = self.primary_node.run_fabric_tftp_command('fabric_config_get_uplinks')
        current_uplink = None
        for line in contents.splitlines():
            if 'Uplink' in line:
    ...
    def is_updatable(self, package, partition_arg='INACTIVE', priority=None, async=False):
        ...
        """
        return self._run_on_all_nodes(async, 'is_updatable', package, partition_arg, priority)

    def update_firmware(self, package, partition_arg='INACTIVE', priority=None, async=False):
        ...
        :type async: boolean
        """
        self._run_on_all_nodes(async, 'update_firmware', package, partition_arg, priority)
    ...
    def ipmitool_command(self, ipmitool_args, asynchronous=False):
        ...
        """
        return self._run_on_all_nodes(asynchronous, 'ipmitool_command', ipmitool_args)
    ...
    def get_server_ip(self, interface=None, ipv6=False, aggressive=False, async=False):
        ...
        """
        return self._run_on_all_nodes(async, 'get_server_ip', interface, ipv6, aggressive)
    ...
    def add_macaddr(self, nodeid, iface, macaddr):
        ...
        """
        self.primary_node.bmc.fabric_add_macaddr(nodeid=nodeid, iface=iface, macaddr=macaddr)
    ...
    def rm_macaddr(self, nodeid, iface, macaddr):
        ...
        """
        self.primary_node.bmc.fabric_rm_macaddr(nodeid=nodeid, iface=iface, macaddr=macaddr)
    ...
    def _run_on_all_nodes(self, async, name, *args, **kwargs):
        ...
        for node_id, node in self.nodes.iteritems():
            tasks[node_id] = self.task_queue.put(getattr(node, name), *args, **kwargs)
