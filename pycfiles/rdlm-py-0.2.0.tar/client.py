...
# [GCC 8.4.0]
# Embedded file name: /Users/fab/Documents/rdlm-py/rdlmpy/client.py
# Compiled at: 2013-06-06 16:20:15
import requests, socket, getpass, os, sys
...
class RDLMClient(object):
    ...
    def resource_get_all(self, username=None, password=None):
        ...
        if r.status_code != 200:
            raise RDLMServerException('Impossible to get all resources                                      (unknown error')
        try:
            json_hal = json.loads(r.content)
        except:
            raise RDLMServerException("Impossible to get all resources                                      (can't unserialize result")

        if '_embedded' not in json_hal or 'resources' not in json_hal['_embedded']:
            return []
    ...
    def resource_get_all_locks(self, resource_name, username=None, password=None):
        ...
        if r.status_code != 200:
            raise RDLMServerException('Impossible to get all locks                                      (unknown error')
        try:
            json_hal = json.loads(r.content.decode('utf-8'))
        except:
            raise RDLMServerException("Impossible to get all locks                                      (can't unserialize result")

        if '_embedded' not in json_hal or 'locks' not in json_hal['_embedded']:
            return []
        out = []
