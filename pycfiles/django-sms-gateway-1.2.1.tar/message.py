...
# [GCC 8.4.0]
# Embedded file name: /Users/matt/Development/django-sms-gateway/sms/models/message.py
# Compiled at: 2012-11-23 02:05:21
...
import uuidfield.fields, picklefield, jsonfield
...
class MessageManager(models.Manager):
    ...
    def get_matching_message(self, datadict):
        for gateway in Gateway.objects.all():
            try:
                return Message.objects.get(gateway_message_id=datadict.get(gateway.status_msg_id), gateway=gateway)
    ...
    def get_original_for_reply(self, datadict):
        for gateway in Gateway.objects.all():
            try:
                return Message.objects.get(uuid=datadict.get(gateway.uuid_keyword), gateway=gateway)
            except Message.DoesNotExist:
                pass
    ...
    def get_last_rate_for(self, recipient_number):
        m = Message.objects.filter(recipient_number=recipient_number).exclude(gateway_charge=None).order_by('-send_date')[0]
...
class Message(models.Model):
    ...
    recipient_number = models.CharField(max_length=32, help_text=_('The international number of the recipient, without the leading +'))
    ...
    uuid = uuidfield.fields.UUIDField(auto=True, help_text=_('Used for associating replies.'))
    ...
    gateway = models.ForeignKey('sms.Gateway', null=True, blank=True, editable=False)
    gateway_message_id = models.CharField(max_length=128, blank=True, null=True, editable=False)
    reply_callback = picklefield.PickledObjectField(null=True, blank=True)
    gateway_charge = models.DecimalField(max_digits=10, decimal_places=5, null=True, blank=True)
    ...
    def local_send_time(self):
        if getattr(self.billee, 'timezone', None):
            return adjust_datetime_to_timezone(self.send_date, settings.TIME_ZONE, self.billee.timezone)
        else:
