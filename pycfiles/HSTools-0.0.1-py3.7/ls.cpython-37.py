...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/hstools/funcs/ls.py
# Compiled at: 2019-10-23 10:33:18
...
def print_resource_list(hs, username, filter_dict={}, count=1000000, long_format=False):
    qcount = 25
    ...
    for r in (hs.hs.resources)(**kwargs):
        ...
        if cnt > count:
            return
            if not long_format:
                print(f"+ {r['resource_title'][:25]:<25} {r['resource_id']} ", flush=True)
        else:
            print(f"\n+ {r['resource_id']} ")
...
def add_arguments(parser):
    ...
    parser.add_argument('-l', default=False, action='store_true', help='list in long format')
    parser.add_argument('-n', default=1000000, type=int, help='number of resources to show')
    parser.add_argument('-filter', nargs='*', help='filter resource by metadata attribute, e.g owner=<USERNAME> author=<USERNAME> text=<FULL TEXT>. Multiple filters can be applied at once by space separating them. Available filters are: CREATOR, USER, OWNER, AUTHOR, GROUP, TEXT, PUBLISHED, EDITABLE, PUBLIC')
    set_usage(parser)
...
def main(args):
    ...
    print_resource_list(hs, (userinfo['username']), filter_dict=filters, count=(args.n),
      long_format=(args.l))
