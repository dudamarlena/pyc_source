...
# [GCC 8.4.0]
# Embedded file name: /home/mad/Documents/spike/spike/File/BrukerSMX.py
...
class BrukerSMXHandler(object):
    ...
    def prepare_mat(self):
        ...
        self.long_mat = (
         self.dim_mat[0] * self.sub_per_dim[1], self.dim_sub_mat[1])
    ...
    def reorder_subm(self, data):
        ...
        rdata = np.empty((self.si1, self.si2))
        interm = data.reshape(self.long_mat)
        for sub_num, sub_idx in enumerate(np.ndindex(tuple(self.sub_per_dim))):
            zipshape = zip(sub_idx, self.dim_sub_mat)
            sub_slices = [slice(i * j, (i + 1) * j) for i, j in zipshape]
            slt2 = slice(sub_num * self.dim_sub_mat[0], (sub_num + 1) * self.dim_sub_mat[0])
            slt1 = slice(0, self.dim_sub_mat[1])
