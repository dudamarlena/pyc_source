...
# [GCC 8.4.0]
# Embedded file name: /home/mad/Documents/spike/spike/FTMS.py
# Compiled at: 2020-02-19 16:37:08
...
HighestMass = 10000000.0

class FTMSAxis(NPKData.Axis):
    __doc__ = '\n    hold information for one FT-MS axis\n    used internally\n    '

    def __init__(self, itype=0, currentunit='points', size=1024, specwidth=1000000.0, offsetfreq=0.0, left_point=0.0, highmass=10000.0, calibA=1000000.0, calibB=0.0, calibC=0.0):
        """
        ...
        self.units['sec'] = NPKData.Unit(name='sec', converter=(self.itos), bconverter=(self.stoi))
    ...
    def borders(self):
        ...
        if self.currentunit == 'm/z':
            return (
             int(self.mztoi(self.highmass)), self.size - 1)
    ...
    def extract(self, zoom):
        ...
        new_specwidth = self.itoh(end - 1) - self.offsetfreq
    ...
    def report(self):
        ...
        if self.itype == 0:
            return 'FT-MS axis at %f kHz,  %d real points,  high m/z = %8.3f  R max (M=400) = %.0f' % (
             self.specwidth / 1000, self.size, self.highmass, 400.0 / self.deltamz(400.0))
        return 'FT-MS axis at %f kHz,  %d complex pairs,  high m/z = %8.3f  R max (M=400) = %.0f' % (
         self.specwidth / 1000, self.size / 2, self.highmass, 400.0 / self.deltamz(400.0))
...
class FTMSData(NPKData._NPKData):
    ...
    def trimz(self, axis=0):
        ...
        unitaxis = []
        for i in range(self.dim):
            unitaxis.append(self.axes(i + 1).currentunit)
        ...
        self.extract(ext)
        for i in range(self.dim):
            self.axes(i + 1).currentunit = unitaxis[i]
