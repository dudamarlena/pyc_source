...
# [GCC 8.4.0]
# Embedded file name: /home/mad/Documents/ mad/CASC4DE/CODES/EUFTICR-NB/spike/Interactive/ipyfilechooser.py
# Compiled at: 2019-05-03 14:06:06
# Size of source mod 2**32: 12893 bytes
...
class FileChooser(VBox):
    ...
    def __init__(self, path=os.getcwd(), filename='', show_hidden=False, **kwargs):
        ...
        self._select = Button(description='Select',
          ...
          layout=Layout(width='auto'))
        self._pathlist.observe((self._on_pathlist_select),
          names='value')
        self._dircontent.observe((self._on_dircontent_select),
          names='value')
        self._filename.observe((self._on_filename_change),
          names='value')
        self._select.on_click(self._on_select_click)
        self._cancel.on_click(self._on_cancel_click)
        self._label = HTML(value=(self._LBL_TEMPLATE.format(self._LBL_NOFILE, 'black')),
          placeholder='',
          description='')
        self._gb = GridBox(children=[
         self._pathlist,
         self._filename,
         self._dircontent],
 ...
 self._gb,
 buttonbar], 
         layout=Layout(width='auto'), **kwargs)

    def _set_form_values(self, path, filename):
        """Set the form values"""
        self._pathlist.unobserve((self._on_pathlist_select),
          names='value')
        self._dircontent.unobserve((self._on_dircontent_select),
          names='value')
        self._filename.unobserve((self._on_filename_change),
          names='value')
    ...
    def default_filename(self, filename):
        ...
        self._default_filename = filename
        self._default = os.path.join(self._pathlist.value, self._default_filename)
    ...
    @property
    def nmrname(self):
        return os.path.join(os.path.basename(os.path.dirname(self._selected_path)), os.path.basename(self._selected_path))

    def __repr__(self):
        str_ = "FileChooser(path='{0}', filename='{1}', show_hidden='{2}')".format(self._default_path, self._default_filename, self._show_hidden)
        return str_
