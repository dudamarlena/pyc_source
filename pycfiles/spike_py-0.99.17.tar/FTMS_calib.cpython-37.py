...
# [GCC 8.4.0]
# Embedded file name: /home/mad/Documents/spike/spike/plugins/FTMS_calib.py
# Compiled at: 2020-01-31 02:51:02
...
def mzcalib(xind, ref, axis, method='l1'):
    ...
    if axfit.calibC == 0:
        Po = [
         axfit.calibA, axfit.calibB]
    else:
        Po = [
         axfit.calibA, axfit.calibB, axfit.calibC]
    if method == 'l2':
...
def icalib(npk, xind, ref, axis=1, method='l1', verbose=False):
    ...
    npk.RefAxis = npk.axes(todo).copy()
    ax2fit = npk.axes(todo).copy()
    if verbose:
...
def ppm_error(axis, xref, mzref):
    ...
    """
    return 1000000.0 * (axis.itomz(xref) - mzref) / mzref
...
def _display_icalib(axis, xref, mzref, symbol='bo'):
    ...
    """
    maxis = np.linspace(min(axis.mzref) - 10, max(axis.mzref) + 10, 10)
    plt.plot(maxis, np.zeros_like(maxis), 'k--')
...
setattr(FTMSAxis, 'mzref', np.empty(10))
setattr(FTMSAxis, 'imzmeas', np.empty(10))
...
def set_calib(npkd, mzmeas, mzref, axis=1):
    ...
    npkd.axes(todo).imzmeas = npkd.axes(todo).mztoi(np.array(mzmeas))
    return npkd
...
def recalibrate(calib):
    ...
    for mol, values in calib.items():
        ...
        if not peptide:
            ...
            diff.append(1000000.0 * abs(mass - nmass) / mass)
