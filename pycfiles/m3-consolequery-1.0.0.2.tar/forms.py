...
# [GCC 8.4.0]
# Embedded file name: /home/khalikov/projects/processing/src/processing/../env/m3_consolequery/forms.py
...
class ExtQueryConsoleWindow(ExtWindow):
    ...
    def __init__(self, window_params, *args, **kwargs):
        ...
        self.query_str = ExtTextArea(label='Текст запроса', name='query_str', anchor='100%')
        self.query_selected = ExtDictSelectField(label='Запрос', name='query_selected', ask_before_deleting=False, display_field='name', value_field='id', hide_trigger=False, hide_edit_trigger=True, width=500)
        self.query_selected.pack = 'CustomQueries_DictPack'
        load_button = ExtButton(text='Загрузить', handler='load_selected_query')
        save_button = ExtButton(text='Сохранить', handler='save_new_query')
        ...
        self.buttons.append(ExtButton(text='Сделать запрос', handler='make_query'))
        self.buttons.append(ExtButton(text='Закрыть', handler=js_close_window(self, True)))
...
class ExtNewQueryWindow(ExtWindow):
    ...
    def __init__(self, window_params, *args, **kwargs):
        ...
        self.title = 'Сохранение нового запроса'
        ...
        self.query_name = ExtStringField(label='Наименование', name='query_name', anchor='100%', value=query_name)
        self.save_button = ExtButton(text='Сохранить', handler='save_query')
        self.close_button = ExtButton(text='Закрыть', handler=js_close_window(self, True))
