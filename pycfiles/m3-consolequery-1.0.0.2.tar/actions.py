...
# [GCC 8.4.0]
# Embedded file name: /home/khalikov/projects/processing/src/processing/../env/m3_consolequery/actions.py
...
@author: Камилла
"""
from m3.ui.actions import ActionPack, Action, ExtUIScriptResult, OperationResult, ActionContextDeclaration, PreJsonResult
import forms
from m3.helpers import logger
import helpers as admin_helpers, models
...
class QueryConsoleAction(Action):
    ...
    def context_declaration(self):
        return [
         ActionContextDeclaration('query_str', default='', type=str, required=True)]
    ...
    def run(self, request, context):
        ...
        if checking[0:6] == 'select':
            check_phrase = [
             'drop', 'grant', 'alter', 'create', 'update', 'insert', 'delete']
            for wrong_word in check_phrase:
                if checking.find(' ' + wrong_word + ' ') != -1 or checking.find('\n' + wrong_word + ' ') != -1 or checking.find(' ' + wrong_word + '\n') != -1 or checking.find('\n' + wrong_word + '\n') != -1:
                    error_message = 'Запрос содержит не должен содержать слово %s' % wrong_word
            ...
            try:
                ...
                return OperationResult.by_message('' + e.args[0])

        else:
            error_message = 'Запрос должен начинаться с SELECT.'
        return OperationResult.by_message(error_message)
...
class NewQuerySaveWinAction(Action):
    ...
    def run(self, request, context):
        window_params = {'new_query_save_url': self.parent.new_query_save_action.get_absolute_url()}
        window_params.update(context.__dict__)
...
class NewQuerySaveAction(Action):
    ...
    def run(self, request, context):
        ...
        try:
            ...
            error_message = 'Не удалось сохранить запрос. Вероятно запрос неверный.'
...
class LoadSelectedQuery(Action):
    ...
    def contect_declaration(self):
        return [
         ActionContextDeclaration('query_id', default='', required=True)]
    ...
    def run(self, request, context):
        ...
        try:
            ...
            error_message = 'Не удалось загрузить запрос'
...
class CustomQueries_DictPack(BaseDictionaryModelActions):
    ...
    def get_select_window(self, win):
        u""" Возвращает настроенное окно типа "Список" справочника """
