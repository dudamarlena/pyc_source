...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/Dallinger/Dallinger/dallinger/heroku/rq_gevent_worker.py
# Compiled at: 2020-04-26 19:37:24
# Size of source mod 2**32: 7620 bytes
from __future__ import absolute_import, division, print_function, unicode_literals
from gevent import monkey
from gevent.hub import LoopExit
monkey.patch_all()
import logging, signal, gevent, gevent.pool
...
class GeventWorker(Worker):
    ...
    def _install_signal_handlers(self):
        ...
        def request_stop():
            if not self._stop_requested:
                ...
                self.log.warning('Stopping after all greenlets are finished. Press Ctrl+C again for a cold shutdown.')
    ...
    def _work(self, burst=False, logging_level=logging.INFO):
        ...
        self.register_birth()
        self.log.info('RQ GEVENT worker (Greenlet pool size={0}) {1!r} started, version {2}'.format(self.gevent_pool.size, self.key, VERSION))
    ...
    def dequeue_job_and_maintain_ttl(self, timeout):
        ...
        while True:
            ...
            try:
                result = self.queue_class.dequeue_any((self.queues),
                  timeout, connection=(self.connection))
...
def main():
    ...
    if '-w' in sys.argv or '--worker-class' in sys.argv:
        print('You cannot specify worker class when using this script,use the official rqworker instead')
        sys.exit(1)
