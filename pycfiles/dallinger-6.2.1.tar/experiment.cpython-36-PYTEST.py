...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/Dallinger/Dallinger/dallinger/experiment.py
# Compiled at: 2020-04-26 19:37:24
# Size of source mod 2**32: 34247 bytes
...
import os, random, requests, sys, time, uuid
...
class Experiment(object):
    __doc__ = 'Define the structure of an experiment.'
    app_id = None
    channel = None
    exp_config = None
    replay_path = '/'
    ...
    def __init__(self, session=None):
        ...
        self.known_classes = {'Agent':Agent, 
        ...
        if full not in ('all', True, False):
            ...
            if full == 'all':
                ...
                if role == 'all':
                    ...
                    return Network.query.filter(and_(Network.role == role, Network.full == full)).all()
    ...
    def get_network_for_participant(self, participant):
        ...
        """
        key = participant.id
        networks_with_space = Network.query.filter_by(full=False).order_by(Network.id).all()
        networks_participated_in = [node.network_id for node in Node.query.with_entities(Node.network_id).filter_by(participant_id=(participant.id)).all()]
        ...
        if not legal_networks:
            ...
            self.log('{} networks out of {} available'.format(len(legal_networks), self.practice_repeats + self.experiment_repeats), key)
    ...
    def fail_participant(self, participant):
        ...
        participant_nodes = Node.query.filter_by(participant_id=(participant.id),
          failed=False).all()
    ...
    def run(self, exp_config=None, app_id=None, bot=False, **kwargs):
        ...
        try:
            if self.exp_config.get('mode') == 'debug':
                ...
                dlgr.deployment.deploy_sandbox_shared_setup((dlgr.command_line.log),
                  app=app_id,
    ...
    def collect(self, app_id, exp_config=None, bot=False, **kwargs):
        ...
        try:
            ...
            self.log(('Could not fetch data for id: {}, checking registry'.format(app_id)),
              key='Retrieve:')

        exp_config = exp_config or {}
        if is_registered(app_id):
            raise RuntimeError('The id {} is registered, '.format(app_id) + 'but you do not have permission to access to the data')
        else:
            if kwargs.get('mode') == 'debug' or exp_config.get('mode') == 'debug':
                raise RuntimeError('No remote or local data found for id {}'.format(app_id))
        try:
            @py_assert2 = uuid.UUID
            @py_assert5 = 4
            @py_assert7 = @py_assert2(app_id, version=@py_assert5)
            @py_assert10 = uuid.UUID
            @py_assert12 = isinstance(@py_assert7, @py_assert10)
            if not @py_assert12:
                @py_format14 = 'assert %(py13)s\n{%(py13)s = %(py0)s(%(py8)s\n{%(py8)s = %(py3)s\n{%(py3)s = %(py1)s.UUID\n}(%(py4)s, version=%(py6)s)\n}, %(py11)s\n{%(py11)s = %(py9)s.UUID\n})\n}' % {'py0':@pytest_ar._saferepr(isinstance) if 'isinstance' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(isinstance) else 'isinstance',  'py1':@pytest_ar._saferepr(uuid) if 'uuid' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(uuid) else 'uuid',  'py3':@pytest_ar._saferepr(@py_assert2),  'py4':@pytest_ar._saferepr(app_id) if 'app_id' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(app_id) else 'app_id',  'py6':@pytest_ar._saferepr(@py_assert5),  'py8':@pytest_ar._saferepr(@py_assert7),  'py9':@pytest_ar._saferepr(uuid) if 'uuid' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(uuid) else 'uuid',  'py11':@pytest_ar._saferepr(@py_assert10),  'py13':@pytest_ar._saferepr(@py_assert12)}
                raise AssertionError(@pytest_ar._format_explanation(@py_format14))
            @py_assert2 = @py_assert5 = @py_assert7 = @py_assert10 = @py_assert12 = None
    ...
    def make_uuid(cls, app_id=None):
        ...
        try:
            if app_id:
                if isinstance(uuid.UUID((str(app_id)), version=4), uuid.UUID):
                    return str(app_id)
        except (ValueError, AssertionError):
            pass
    ...
    def restore_state_from_replay(self, app_id, session, zip_path=None, **configuration_options):
        ...
        self.import_session = scoped_session(sessionmaker(autocommit=False, autoflush=True, bind=import_engine))
        if zip_path is None:
        ...
        self._replay_range = tuple(self.import_session.query(func.min(Info.creation_time), func.max(Info.creation_time)))[0]
...
class Scrubber(object):
    ...
    def build_widget(self):
        ...
        scrubber = widgets.SelectionSlider(description='Current time',
          options=options,
          disabled=False,
        ...
        play_button = widgets.ToggleButton(value=False,
          description='',
          disabled=False,
        ...
        def playback(change):
            import threading
            if change['new']:
                thread = threading.Thread(target=(self.in_realtime),
                  kwargs={'callback': realtime_callback})
                thread.start()
            else:
                self.realtime = False
