...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/Dallinger/Dallinger/dallinger/recruiters.py
# Compiled at: 2020-04-26 19:37:24
# Size of source mod 2**32: 40323 bytes
"""Recruiters manage the flow of participants to the experiment."""
import builtins as @py_builtins, _pytest.assertion.rewrite as @pytest_ar, flask, json, logging, os, re, requests
...
class Recruiter(object):
    ...
    def notify_duration_exceeded(self, participants, reference_time):
        ...
        the experiment.
        """
        logger.warning('Received notification that some participants have been active for too long. No action taken.')
...
class CLIRecruiter(Recruiter):
    __doc__ = 'A recruiter which prints out /ad URLs to the console for direct\n    assigment.\n    '
    nickname = 'cli'
    ...
    def open_recruitment(self, n=1):
        ...
        message = 'Search for "{}" in the logs for subsequent recruitment URLs.\nOpen the logs for this experiment with "dallinger logs --app {}"'.format(NEW_RECRUIT_LOG_PREFIX, self.config.get('id'))
        return {'items':recruitments, 
         'message':message}
    ...
    def recruit(self, n=1):
        ...
        for i in range(n):
            ad_url = template.format(get_base_url(), self.nickname, generate_random_id(), generate_random_id(), generate_random_id(), self._get_mode())
...
class HotAirRecruiter(CLIRecruiter):
    ...
    def reward_bonus(self, assignment_id, amount, reason):
        ...
        logger.info('Were this a real Recruiter, we\'d be awarding ${} for assignment {}, with reason "{}"'.format(amount, assignment_id, reason))
...
class SimulatedRecruiter(Recruiter):
    __doc__ = 'A recruiter that recruits simulated participants.'
    nickname = 'sim'
...
class MTurkHITMessages(object):
    ...
    _templates = {'resubmitted':{'subject':'Dallinger automated email - minor error.', 
      'template':mturk_resubmit}, 
     'cancelled':{'subject':'Dallinger automated email - major error.', 
    ...
    def _build(self, category):
        ...
        return {'body':data['template'].format(s=self.summary), 
         'subject':data['subject']}


class WhimsicalMTurkHITMessages(MTurkHITMessages):
    _templates = {'resubmitted':{'subject':'A matter of minor concern.', 
      'template':mturk_resubmit_whimsical}, 
     'cancelled':{'subject':'Most troubling news.', 
      'template':mturk_cancelled_hit_whimsical}}


class MTurkRecruiterException(Exception):
    __doc__ = 'Custom exception for MTurkRecruiter'
...
class MTurkRecruiter(Recruiter):
    __doc__ = 'Recruit participants from Amazon Mechanical Turk'
    nickname = 'mturk'
    ...
    def __init__(self):
        ...
        self.mturkservice = MTurkService(aws_access_key_id=(self.config.get('aws_access_key_id')),
    ...
    def _validate_config(self):
        ...
        if mode not in ('sandbox', 'live'):
            raise MTurkRecruiterException('"{}" is not a valid mode for MTurk recruitment. The value of "mode" must be either "sandbox" or "live"'.format(mode))
    ...
    def open_recruitment(self, n=1):
        ...
        if self.is_in_progress:
            raise MTurkRecruiterException('Tried to open_recruitment on already open recruiter.')
        ...
        hit_request = {'experiment_id':self.config.get('id'), 
         'max_assignments':n, 
    ...
    def compensate_worker(self, worker_id, email, dollars, notify=True):
        ...
        """
        qualification = self.mturkservice.create_qualification_type(name=('Dallinger Compensation Qualification - {}'.format(generate_random_id())),
          description=('You have received a qualification to allow you to complete a compensation HIT from Dallinger for ${}.'.format(dollars)))
        qid = qualification['id']
        self.mturkservice.assign_qualification(qid, worker_id, 1, notify=notify)
        hit_request = {'experiment_id':'(compensation only)', 
         'max_assignments':1, 
         'title':'Dallinger Compensation HIT', 
         'description':'For compenation only; no task required.', 
         'keywords':[],  'reward':float(dollars), 
         'duration_hours':1, 
         'lifetime_days':3, 
         'question':MTurkQuestions.compensation(sandbox=self._is_sandbox), 
         'qualifications':[
          MTurkQualificationRequirements.must_have(qid)], 
         'do_subscribe':False}
        hit_info = (self.mturkservice.create_hit)(**hit_request)
        if email is not None:
            message = {'subject':'Dallinger Compensation HIT',  'sender':self.config.get('dallinger_email_address'), 
             'recipients':[
              email], 
             'body':'A special compenstation HIT is available for you to complete on MTurk.\n\nTitle: {title}\nReward: ${reward:.2f}\nURL: {worker_url}'.format(**hit_info)}
            (self.mailer.send)(**message)
        else:
            message = {}
        return {'hit':hit_info, 
         'qualification':qualification,  'email':message}
    ...
    def recruit(self, n=1):
        ...
        if hit_id is None:
            ...
            return
        try:
            return self.mturkservice.extend_hit(hit_id,
              number=n, duration_hours=(self.config.get('duration')))
    ...
    def notify_duration_exceeded(self, participants, reference_time):
        ...
        for participant in participants:
            ...
            if status == 'Approved':
                ...
                logger.warning('Error - submitted notification for participant {} missed. A replacement notification was created and sent, but proceed with caution.'.format(participant.id))
            else:
    ...
    def rejects_questionnaire_from(self, participant):
        ...
        if participant.status != 'working':
            return 'This participant has already sumbitted their HIT on MTurk and can no longer submit the questionnaire'
    ...
    def current_hit_id(self):
        any_participant_record = Participant.query.with_entities(Participant.hit_id).filter_by(recruiter_id=(self.nickname)).first()
    ...
    def _resend_submitted_rest_notification_for(self, participant):
        q = _get_queue()
        q.enqueue(worker_function, 'AssignmentSubmitted', participant.assignment_id, None)

    def _send_notification_missing_rest_notification_for(self, participant):
        q = _get_queue()
        q.enqueue(worker_function, 'NotificationMissing', participant.assignment_id, None)
...
class BotRecruiter(Recruiter):
    __doc__ = 'Recruit bot participants using a queue'
    nickname = 'bots'
    _timeout = '1h'
...
class MultiRecruiter(Recruiter):
    ...
    def recruiters(self, n=1):
        ...
        while recruit_count <= n:
            counts = dict(session.query(Recruitment.recruiter_id, func.count(Recruitment.id)).group_by(Recruitment.recruiter_id).all())
    ...
    def open_recruitment(self, n=1):
        ...
        logger.info('Multi-recruited {} out of {} participants, using {} recruiters.'.format(n - remaining, n, len(messages)))
        return {'items':recruitments, 
