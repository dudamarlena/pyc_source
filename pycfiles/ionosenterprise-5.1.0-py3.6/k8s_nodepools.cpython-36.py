...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ionosenterprise/requests/k8s_nodepools.py
...
class k8s_nodepools:
    ...
    def get_k8s_cluster_nodepool(self, k8s_cluster_id, nodepool_id):
        ...
        """
        response = self._perform_request('/k8s/%s/nodepools/%s' % (
    ...
    def delete_k8s_cluster_nodepool(self, k8s_cluster_id, nodepool_id):
        ...
        """
        response = self._perform_request(url=('/k8s/%s/nodepools/%s' % (
         k8s_cluster_id, nodepool_id)),
          method='DELETE')
        return response

    def update_k8s_cluster_nodepool(self, k8s_cluster_id, nodepool_id, name, datacenter_id, node_count, cpu_family, cores_count, ram_size, availability_zone, storage_type, storage_size):
        """
        ...
        response = self._perform_request(url=('/k8s/%s/nodepools/%s' % (k8s_cluster_id, nodepool_id)),
          method='PUT',
    ...
    def list_k8s_cluster_nodepools(self, k8s_cluster_id, depth=1):
        ...
        """
        response = self._perform_request(url=('/k8s/%s/nodepools?depth=%s' % (k8s_cluster_id, str(depth))))
        return response

    def create_k8s_cluster_nodepool(self, k8s_cluster_id, name, datacenter_id, node_count, cpu_family, cores_count, ram_size, availability_zone, storage_type, storage_size):
        """
        ...
        data = {'properties': {'name':name, 
                        ...
                        'storageType':storage_type, 
                        'storageSize':storage_size}}
        response = self._perform_request(url=('/k8s/%s/nodepools' % k8s_cluster_id),
          method='POST',
          data=(json.dumps(data)))
        return response