...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ionosenterprise/client.py
# Compiled at: 2019-10-08 04:15:23
# Size of source mod 2**32: 17005 bytes
import getpass, logging, os, sys, re, time, requests, six
try:
    import configparser
except ImportError:
...
from ionosenterprise import API_HOST, __version__
from ionosenterprise.errors import ICNotAuthorizedError, ICNotFoundError, ICValidationError, ICRateLimitExceededError, ICError, ICFailedRequest, ICTimeoutError
...
class IonosEnterpriseService(IonosEnterpriseRequests):
    ...
    def wait_for(self, fn_check, fn_request, timeout=3600, initial_wait=5, scaleup=10, console_print=None):
        """
            ...
            if console_print is not None:
                ...
                if timeout:
                    if current_time > timeout:
                        raise ICTimeoutError('Timed out waiting for request {0}.'.format(resp['requestId']), resp['requestId'])
        ...
        while True:
            ...
            if request['metadata']['status'] == 'DONE':
                ...
                if request['metadata']['status'] == 'FAILED':
                    raise ICFailedRequest('Request {0} failed to complete: {1}'.format(response['requestId'], request['metadata']['message']), response['requestId'])
            current_time = time.time()
            if timeout:
                if current_time > timeout:
                    raise ICTimeoutError('Timed out waiting for request {0}.'.format(response['requestId']), response['requestId'])
            if current_time > next_increase:
                wait_period *= 2
                next_increase = time.time() + wait_period * scaleup
                scaleup *= 2
