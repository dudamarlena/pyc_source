...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\arcadeplus\emitter.py
# Compiled at: 2020-03-29 14:04:04
...
class EmitController:
    __doc__ = 'Base class for how a client configure the rate at which an Emitter emits Particles\n\n    Subclasses allow the client to control the rate and duration of emitting'
...
class EmitBurst(EmitController):
    __doc__ = 'Used to configure an Emitter to emit particles in one burst'
...
class EmitMaintainCount(EmitController):
    __doc__ = 'Used to configure an Emitter so it emits particles so that the given count is always maintained'
...
class EmitInterval(EmitController):
    __doc__ = 'Base class used to configure an Emitter to have a constant rate of emitting. Will emit indefinitely.'
...
class EmitterIntervalWithCount(EmitInterval):
    __doc__ = 'Configure an Emitter to emit particles with given interval, ending after emitting given number of particles'

    def __init__(self, emit_interval, particle_count):
        super().__init__(emit_interval)
        self._count_remaining = particle_count

    def how_many(self, delta_time, current_particle_count):
...
class EmitterIntervalWithTime(EmitInterval):
    __doc__ = 'Configure an Emitter to emit particles with given interval, ending after given number of seconds'

    def __init__(self, emit_interval, lifetime):
        super().__init__(emit_interval)
...
class Emitter:
    __doc__ = 'Emits and manages Particles over their lifetime.  The foundational class in a particle system.'

    def __init__(self, center_xy: Point, emit_controller: EmitController, particle_factory: Callable[(['Emitter'], Particle)], change_xy: Vector=(0.0, 0.0), emit_done_cb: Callable[(['Emitter'], None)]=None, reap_cb: Callable[([], None)]=None):
        ...
        self._particles = arcadeplus.SpriteList(use_spatial_hash=False)
    ...
    def update(self):
        ...
        emit_count = self.rate_factory.how_many(0.016666666666666666, len(self._particles))
