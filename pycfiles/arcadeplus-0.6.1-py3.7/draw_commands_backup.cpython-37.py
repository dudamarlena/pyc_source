...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\arcadeplus\draw_commands_backup.py
# Compiled at: 2020-03-31 13:43:09
# Size of source mod 2**32: 36221 bytes
"""
...
"""
import math, array, PIL.Image, PIL.ImageOps, PIL.ImageDraw
...
def draw_circle_outline(center_x: float, center_y: float, radius: float, color: Color, border_width: float=1, num_segments: int=128):
    ...
    width = radius * 2
    height = radius * 2
    draw_ellipse_outline(center_x, center_y, width, height, color,
...
def draw_polygon_outline(point_list: PointList, color: Color, line_width: float=1):
    ...
    points = get_points_for_thick_line(new_point_list[0][0], new_point_list[0][1], new_point_list[1][0], new_point_list[1][1], line_width)
...
def draw_lrtb_rectangle_outline(left: float, right: float, top: float, bottom: float, color: Color, border_width: float=1):
    """
    ...
    """
    if left > right:
        raise AttributeError('Left coordinate must be less than or equal to the right coordinate')
    if bottom > top:
        raise AttributeError('Bottom coordinate must be less than or equal to the top coordinate')
    center_x = (left + right) / 2
    center_y = (top + bottom) / 2
    width = right - left
    height = top - bottom
    draw_rectangle_outline(center_x, center_y, width, height, color, border_width)


def draw_xywh_rectangle_outline(bottom_left_x: float, bottom_left_y: float, width: float, height: float, color: Color, border_width: float=1):
    """
...
def draw_lrtb_rectangle_filled(left: float, right: float, top: float, bottom: float, color: Color):
    """
    Draw a rectangle by specifying left, right, top, and bottom edges.
    ...
    """
    if left > right:
        raise AttributeError('Left coordinate {} must be less than or equal to the right coordinate {}'.format(left, right))
    if bottom > top:
        raise AttributeError('Bottom coordinate {} must be less than or equal to the top coordinate {}'.format(bottom, top))
    center_x = (left + right) / 2
    center_y = (top + bottom) / 2
    width = right - left + 1
    height = top - bottom + 1
...
def draw_xywh_rectangle_filled(bottom_left_x: float, bottom_left_y: float, width: float, height: float, color: Color):
    """
...
def draw_rectangle_filled(center_x: float, center_y: float, width: float, height: float, color: Color, tilt_angle: float=0):
    """
