...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\arcadeplus\draw_commands.py
# Compiled at: 2020-04-01 15:56:18
# Size of source mod 2**32: 48017 bytes
"""
...
"""
import math, array, itertools
from collections import defaultdict
import pyglet.gl as gl
import numpy as np, PIL.Image, PIL.ImageOps, PIL.ImageDraw
import pyglet.gl as gl
...
class Shape:
    ...
    def draw(self):
        with self.vao:
            ...
            gl.glPrimitiveRestartIndex(4294967295)
...
class ShapeElementList(Generic[TShape]):
    ...
    def _refresh_shape(self, group):
        ...
        for shape in batch.items:
            ...
            gl.glCopyBufferSubData(gl.GL_COPY_READ_BUFFER, gl.GL_COPY_WRITE_BUFFER, gl.GLintptr(0), gl.GLintptr(offset), shape.vbo.size)
            offset += shape.vbo.size

        reset_idx = 4294967295
        indices = []
...
def _create_line_generic_with_colors(point_list: PointList, color_list: Iterable[Color], shape_mode: int, line_width: float=1):
    """
    ...
    :Returns Shape:
    """
    program = shader.program(vertex_shader='\n            #version 330\n            uniform mat4 Projection;\n            in vec2 in_vert;\n            in vec4 in_color;\n            out vec4 v_color;\n            void main() {\n               gl_Position = Projection * vec4(in_vert, 0.0, 1.0);\n               v_color = in_color;\n            }\n        ',
      fragment_shader='\n            #version 330\n            in vec4 v_color;\n            out vec4 f_color;\n            void main() {\n                f_color = v_color;\n            }\n        ')
...
def _create_line_generic(point_list: PointList, color: Color, shape_mode: int, line_width: float=1):
    """
    This function is used by ``draw_line_strip`` and ``draw_line_loop``,
    just changing the OpenGL type for the line drawing.
    """
    colors = [
     get_four_byte_color(color)] * len(point_list)
    shape = _create_line_generic_with_colors(point_list, colors, shape_mode, line_width)
    return shape


def _generic_draw_line_strip(point_list: PointList, color: Color, mode: int=gl.GL_LINE_STRIP):
    """
    ...
    vbo_buf_desc = shader.BufferDescription(vbo_buf, '2f', [
     'in_vert'])
    vao_content = [
     vbo_buf_desc, color_buf_desc]
    vao = shader.vertex_array(program, vao_content)
    with vao:
        program['Projection'] = get_projection().flatten()
...
def get_rectangle_points(center_x: float, center_y: float, width: float, height: float, tilt_angle: float=0):
    """
...
def draw_line_loop(point_list: PointList, color: Color, line_width: float=1):
    """
...
def draw_polygon_outline(point_list: PointList, color: Color, line_width: float=1):
    """
...
def draw_rectangle(center_x: float, center_y: float, width: float, height: float, color: Color, border_width: float=1, tilt_angle: float=0, filled=True):
    """
    Draw a rectangle.
    ...
    """
    data = cast(List[Point], get_rectangle_points(center_x, center_y, width, height, tilt_angle))
    if filled:
        shape_mode = gl.GL_TRIANGLE_STRIP
        ...
        i_lb = (
         center_x - width / 2 + border_width / 2, center_y - height / 2 + border_width / 2)
...
def draw_rectangle_filled(center_x: float, center_y: float, width: float, height: float, color: Color, border_width: float=1, tilt_angle: float=0):
    """
    ...
    """
    draw_rectangle(center_x, center_y, width, height, color,
      border_width=border_width, tilt_angle=tilt_angle, filled=True)


def draw_rectangle_outline(center_x: float, center_y: float, width: float, height: float, color: Color, border_width: float=1, tilt_angle: float=0):
    """
    ...
    """
    draw_rectangle(center_x, center_y, width, height, color,
      border_width=border_width, tilt_angle=tilt_angle, filled=False)


def draw_lrtb_rectangle_filled(left: float, right: float, top: float, bottom: float, color: Color, tilt_angle: float=0, border_width: float=1):
    """
    ...
    height = top - bottom
    draw_rectangle(center_x, center_y, width, height, color,
      tilt_angle=tilt_angle, filled=True, border_width=border_width)


def draw_xywh_rectangle_filled(bottom_left_x: float, bottom_left_y: float, width: float, height: float, color: Color, border_width: float=1, tilt_angle: float=0):
    """
...
def draw_lrtb_rectangle_outline(left: float, right: float, top: float, bottom: float, color: Color, border_width: float=1, tilt_angle: float=0):
    """
...
def draw_xywh_rectangle_outline(bottom_left_x: float, bottom_left_y: float, width: float, height: float, color: Color, border_width: float=1, tilt_angle: float=0):
    """
...
def draw_triangle_outline(x1: float, y1: float, x2: float, y2: float, x3: float, y3: float, color: Color, border_width: float=1):
    """
...
def draw_ellipse(center_x: float, center_y: float, width: float, height: float, color: Color, border_width: float=1, tilt_angle: float=0, num_segments: int=32, filled=True):
    """
    Draw an ellipse.
    ...
    for segment in range(num_segments):
        theta = 6.2831852 * segment / num_segments
...
def draw_ellipse_filled(center_x: float, center_y: float, width: float, height: float, color: Color, tilt_angle: float=0, num_segments: int=128):
    """
    Draw a filled ellipse.
    """
    border_width = 1
    draw_ellipse(center_x, center_y, width, height, color, border_width,
      tilt_angle, num_segments, filled=True)


def draw_ellipse_outline(center_x: float, center_y: float, width: float, height: float, color: Color, border_width: float=1, tilt_angle: float=0, num_segments: int=128):
    """
    Draw an ellipse outline.
    """
    draw_ellipse(center_x, center_y, width, height, color, border_width,
      tilt_angle, num_segments, filled=False)


def draw_ellipse_filled_with_colors(center_x: float, center_y: float, width: float, height: float, outside_color: Color, inside_color: Color, tilt_angle: float=0, num_segments: int=32):
    """
    ...
    color_list = [
     inside_color] + [outside_color] * (num_segments + 1)
...
def draw_circle_filled(center_x: float, center_y: float, radius: float, color: Color, num_segments: int=128):
    """
...
def draw_circle_outline(center_x: float, center_y: float, radius: float, color: Color, border_width: float=1, num_segments: int=128):
    """
    ...
    :param int num_segments: Int of triangle segments that make up this
         circle. Higher is better quality, but slower render time.
    """
    draw_ellipse_outline(center_x, center_y, radius, radius, color,
      border_width, num_segments=num_segments)
...
def draw_triangle_filled(x1: float, y1: float, x2: float, y2: float, x3: float, y3: float, color: Color):
    """
...
def draw_arc_filled(center_x: float, center_y: float, width: float, height: float, color: Color, start_angle: float, end_angle: float, tilt_angle: float=0, num_segments: int=128):
    """
...
def draw_arc_outline(center_x: float, center_y: float, width: float, height: float, color: Color, start_angle: float, end_angle: float, border_width: float=1, tilt_angle: float=0, num_segments: int=128):
    """
...
def draw_parabola_filled(start_x: float, start_y: float, end_x: float, height: float, color: Color, tilt_angle: float=0):
    """
...
def draw_parabola_outline(start_x: float, start_y: float, end_x: float, height: float, color: Color, border_width: float=1, tilt_angle: float=0):
    """
...
def draw_scaled_texture_rectangle(center_x: float, center_y: float, texture: Texture, scale: float=1.0, angle: float=0, alpha: int=255):
    """
...
def draw_texture_rectangle(center_x: float, center_y: float, width: float, height: float, texture: Texture, angle: float=0, alpha: int=255):
    """
...
def draw_lrwh_rectangle_textured(bottom_left_x: float, bottom_left_y: float, width: float, height: float, texture: Texture, angle: float=0, alpha: int=255):
    """
