...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\arcadeplus\application.py
# Compiled at: 2020-04-11 11:02:29
# Size of source mod 2**32: 25040 bytes
"""
...
class Window(pyglet.window.Window):
    ...
    def __init__(self, width=800, height=600, title='ArcadePlus Window', fullscreen=False, resizable=False, update_rate=0.016666666666666666, antialiasing=True):
        """
        ...
        if antialiasing:
            ...
            config = pyglet.gl.Config(major_version=3, minor_version=3,
              double_buffer=True)
        try:
            super().__init__(width=width, height=height, caption=title, resizable=resizable,
              config=config,
              vsync=False)
    ...
    def test(self, frames: int=10):
        ...
        for i in range(frames):
            ...
            if elapsed_time < 0.016666666666666666:
                sleep_time = 0.016666666666666666 - elapsed_time
                time.sleep(sleep_time)
            self.update(0.016666666666666666)

    def show_view(self, new_view: 'View'):
        if not isinstance(new_view, View):
            raise ValueError('Must pass an arcadeplus.View object to Window.show_view()')
        elif new_view.window is None:
            new_view.window = self
        else:
            if new_view.window != self:
                raise RuntimeError('You are attempting to pass the same view object between multiple windows. A single view object can only be used in one window.')
        if self.current_view is not None:
...
def open_window(width: int, height: int, window_title: str, resizable: bool=False, antialiasing: bool=True) -> Window:
    """
    ...
    """
    global _window
    _window = Window(width, height, window_title, resizable=resizable, update_rate=None, antialiasing=antialiasing)
