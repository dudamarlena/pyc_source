...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\arcadeplus\examples\asteroid_smasher.py
# Compiled at: 2020-03-29 18:04:18
# Size of source mod 2**32: 15127 bytes
"""
Asteroid Smasher
...
"""
import random, math, arcadeplus, os
...
class TurningSprite(arcadeplus.Sprite):
    __doc__ = ' Sprite that sets its angle to the direction it is traveling in. '
...
class AsteroidSprite(arcadeplus.Sprite):
    __doc__ = ' Sprite that represents an asteroid. '
...
class BulletSprite(TurningSprite):
 ...
 L. 151        10  LOAD_FAST                'self'
               12  LOAD_ATTR                center_x
               14  LOAD_CONST               -100
               16  COMPARE_OP               <
               18  POP_JUMP_IF_TRUE     50  'to 50'
               20  LOAD_FAST                'self'
               22  LOAD_ATTR                center_x
               24  LOAD_CONST               1500
               26  COMPARE_OP               >
               28  POP_JUMP_IF_TRUE     50  'to 50'

 L. 152        30  LOAD_FAST                'self'
               32  LOAD_ATTR                center_y
               34  LOAD_CONST               1100
               36  COMPARE_OP               >
               38  POP_JUMP_IF_TRUE     50  'to 50'
               40  LOAD_FAST                'self'
               42  LOAD_ATTR                center_y
               44  LOAD_CONST               -100
               46  COMPARE_OP               <
 ...
 L. 153        50  LOAD_FAST                'self'
               52  LOAD_METHOD              remove_from_sprite_lists
...
class MyGame(arcadeplus.Window):
    ...
    def start_new_game(self):
        ...
        image_list = (':resources:images/space_shooter/meteorGrey_big1.png', ':resources:images/space_shooter/meteorGrey_big2.png',
                      ':resources:images/space_shooter/meteorGrey_big3.png', ':resources:images/space_shooter/meteorGrey_big4.png')
    ...
    def on_key_press(self, symbol, modifiers):
        ...
        if not self.player_sprite.respawning:
            if symbol == arcadeplus.key.SPACE:
            ...
            if symbol == arcadeplus.key.RIGHT:
                ...
                if symbol == arcadeplus.key.UP:
                    self.player_sprite.thrust = 0.15
                else:
                    if symbol == arcadeplus.key.DOWN:
    ...
    def on_update(self, x):
        """ Move everything """
        self.frame_count += 1
        self.game_over or self.all_sprites_list.update()
        ...
        asteroids = self.player_sprite.respawning or arcadeplus.check_for_collision_with_list(self.player_sprite, self.asteroid_list)
        if len(asteroids) > 0:
            if self.lives > 0:
                self.lives -= 1
