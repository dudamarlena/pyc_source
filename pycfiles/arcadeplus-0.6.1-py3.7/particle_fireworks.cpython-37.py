...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\arcadeplus\examples\particle_fireworks.py
# Compiled at: 2020-03-29 18:06:41
...
def make_spinner():
    spinner = arcadeplus.Emitter(center_xy=(
     SCREEN_WIDTH / 2, SPINNER_HEIGHT - 5),
      emit_controller=(arcadeplus.EmitterIntervalWithTime(0.025, 2.0)),
      particle_factory=(lambda emitter: arcadeplus.FadeParticle(filename_or_texture=(random.choice(STAR_TEXTURES)),
      change_xy=(0, 6.0),
      lifetime=0.2)))
...
def make_rocket(emit_done_cb):
    ...
    rocket = RocketEmitter(center_xy=(
     random.uniform(100, SCREEN_WIDTH - 100), 25),
      emit_controller=(arcadeplus.EmitterIntervalWithTime(0.04, 2.0)),
      particle_factory=(lambda emitter: arcadeplus.FadeParticle(filename_or_texture=ROCKET_SMOKE_TEXTURE,
...
def make_flash(prev_emitter):
    ...
    return arcadeplus.Emitter(center_xy=(prev_emitter.get_pos()),
      ...
      particle_factory=(lambda emitter: arcadeplus.FadeParticle(filename_or_texture=FLASH_TEXTURE,
...
def make_puff(prev_emitter):
    ...
    return arcadeplus.Emitter(center_xy=(prev_emitter.get_pos()),
      ...
      particle_factory=(lambda emitter: arcadeplus.FadeParticle(filename_or_texture=PUFF_TEXTURE,
...
class AnimatedAlphaParticle(arcadeplus.LifetimeParticle):
    __doc__ = 'A custom particle that animates between three different alpha levels'

    def __init__(self, filename_or_texture, change_xy, start_alpha=0, duration1=1.0, mid_alpha=255, duration2=1.0, end_alpha=0, center_xy=(0.0, 0.0), angle=0, change_angle=0, scale=1.0, mutation_callback=None):
...
class RocketEmitter(arcadeplus.Emitter):
    __doc__ = 'Custom emitter class to add gravity to the emitter to represent gravity on the firework shell'
...
class FireworksApp(arcadeplus.Window):
    ...
    def __init__(self):
        ...
        stars = arcadeplus.Emitter(center_xy=(0.0, 0.0),
          emit_controller=(arcadeplus.EmitMaintainCount(20)),
          particle_factory=(lambda emitter: AnimatedAlphaParticle(filename_or_texture=(random.choice(STAR_TEXTURES)),
          change_xy=(0.0, 0.0),
          start_alpha=0,
        ...
        self.cloud = arcadeplus.Emitter(center_xy=(50, 500),
          change_xy=(0.15, 0),
          emit_controller=(arcadeplus.EmitMaintainCount(60)),
          particle_factory=(lambda emitter: AnimatedAlphaParticle(filename_or_texture=(random.choice(CLOUD_TEXTURES)),
    ...
    def explode_firework(self, prev_emitter):
        ...
        sparks = arcadeplus.Emitter(center_xy=(prev_emitter.get_pos()),
          emit_controller=(arcadeplus.EmitBurst(random.randint(30, 40))),
          particle_factory=(lambda emitter: arcadeplus.FadeParticle(filename_or_texture=spark_texture,
    ...
    def explode_ringed_firework(self, prev_emitter):
        ...
        sparks = arcadeplus.Emitter(center_xy=(prev_emitter.get_pos()),
          ...
          mutation_callback=firework_spark_mutator)))
        self.emitters.append(sparks)
        ring = arcadeplus.Emitter(center_xy=(prev_emitter.get_pos()),
          emit_controller=(arcadeplus.EmitBurst(20)),
          particle_factory=(lambda emitter: arcadeplus.FadeParticle(filename_or_texture=ring_texture,
          change_xy=(arcadeplus.rand_on_circle((0.0, 0.0), 5.0) + arcadeplus.rand_in_circle((0.0,
                                                                                             0.0), 0.25)),
    ...
    def explode_sparkle_firework(self, prev_emitter):
        ...
        sparks = arcadeplus.Emitter(center_xy=(prev_emitter.get_pos()),
          emit_controller=(arcadeplus.EmitBurst(random.randint(30, 40))),
          particle_factory=(lambda emitter: AnimatedAlphaParticle(filename_or_texture=spark_texture,
          change_xy=(arcadeplus.rand_in_circle((0.0, 0.0), 9.0)),
          start_alpha=255,
