...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\arcadeplus\read_tiled_map.py
# Compiled at: 2020-04-19 09:44:05
# Size of source mod 2**32: 14096 bytes
"""
Functions and classes for managing a map created in the "Tiled Map Editor"
"""
import xml.etree.ElementTree as ElementTree
import base64, zlib, gzip, typing
...
class TiledMap:
    __doc__ = ' This class holds a tiled map, and tile set from the map. '
...
class Tile:
    __doc__ = ' This class represents an individual tile from a tileset. '
...
class GridLocation:
    __doc__ = ' This represents a location on the grid. Contains the x/y of the\n    grid location, and the tile that is on it. '
...
def read_tiled_map(tmx_file: str, scaling: float=1, tsx_file: str=None) -> TiledMap:
    ...
    my_map = TiledMap()
    if isinstance(tmx_file, str):
        if str(tmx_file).startswith(':resources:'):
            import os
                ...
                if layer_grid_ints[row_index][column_index] != 0:
                    ...
                    if key not in my_map.global_tile_set:
                        ...
                        if my_map.orientation == 'orthogonal':
                            ...
                            grid_loc.center_x, grid_loc.center_y = isometric_grid_to_screen(column_index, row_index, my_map.width, my_map.height, my_map.tilewidth, my_map.tileheight)
