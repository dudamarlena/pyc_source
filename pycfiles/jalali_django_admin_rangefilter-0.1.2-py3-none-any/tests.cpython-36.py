...
# [GCC 8.4.0]
# Embedded file name: /home/nima/Projects/Python/jalali-django-admin-rangefilter/rangefilter/tests.py
...
class DateRangeFilterTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.djangonaut_book = MyModel.objects.create(created_at=(timezone.now() - datetime.timedelta(days=7)))
        self.django_book_date = MyModelDate.objects.create(created_at=(timezone.now()))
        self.djangonaut_book_date = MyModelDate.objects.create(created_at=(timezone.now() - datetime.timedelta(days=7)))
    ...
    def get_changelist(self, request, model, modeladmin):
        if getattr(modeladmin, 'get_changelist_instance', None):
            ...
            return ChangeList(request, model, modeladmin.list_display, modeladmin.list_display_links, modeladmin.list_filter, modeladmin.date_hierarchy, modeladmin.search_fields, modeladmin.list_select_related, modeladmin.list_per_page, modeladmin.list_max_show_all, modeladmin.list_editable, modeladmin)
    ...
    def test_datefilter_filtered(self):
        self.request_factory = RequestFactory()
        modeladmin = MyModelAdmin(MyModel, site)
        request = self.request_factory.get('/', {'created_at__range__gte':self.today,  'created_at__range__lte':self.tomorrow})
        request.user = self.user
        changelist = self.get_changelist(request, MyModel, modeladmin)
        queryset = changelist.get_queryset(request)
...
class DateTimeRangeFilterTestCase(TestCase):
    ...
    def test_datetimfilter_filtered(self):
        self.request_factory = RequestFactory()
        modeladmin = MyModelTimeAdmin(MyModel, site)
        request = self.request_factory.get('/', {'created_at__range__gte_0':self.today,  'created_at__range__gte_1':self.min_time, 
...
class TemplateTagsTestCase(TestCase):
    ...
    def test_returns_static_path_to_asset_when_staticfiles_app_is_installed(self):
        with self.modify_settings(INSTALLED_APPS={'append': 'django.contrib.staticfiles'}):
