...
# [GCC 8.4.0]
# Embedded file name: /Users/jgorset/Code/python/fandjango/fandjango/utils.py
...
def cached_property(**kwargs):
    ...
    def decorator(function):
        ...
        @wraps(function)
        def wrapper(self):
            key = 'fandjango.%(model)s.%(property)s_%(pk)s' % {'model': self.__class__.__name__, 
               'pk': self.pk, 
...
def get_post_authorization_redirect_url(request, canvas=True):
    ...
    if canvas:
        ...
        redirect_uri = 'https://%(domain)s/%(namespace)s%(path)s' % {'domain': FACEBOOK_APPLICATION_DOMAIN, 
