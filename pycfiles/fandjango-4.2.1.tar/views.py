...
# [GCC 8.4.0]
# Embedded file name: /Users/jgorset/Code/python/fandjango/fandjango/views.py
...
from fandjango.settings import FACEBOOK_APPLICATION_ID, FACEBOOK_APPLICATION_DOMAIN, FACEBOOK_APPLICATION_NAMESPACE, FACEBOOK_APPLICATION_SECRET_KEY, FACEBOOK_APPLICATION_INITIAL_PERMISSIONS
...
def authorize_application(request, redirect_uri='https://%s/%s' % (FACEBOOK_APPLICATION_DOMAIN, FACEBOOK_APPLICATION_NAMESPACE), permissions=FACEBOOK_APPLICATION_INITIAL_PERMISSIONS):
    ...
    return render(request=request, template_name='fandjango/authorize_application.html', dictionary={'url': 'https://www.facebook.com/dialog/oauth?%s' % urlencode(query)}, status=401)
...
def authorization_denied(request):
    ...
    """
    return render(request=request, template_name='fandjango/authorization_denied.html', status=403)
