...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/notdot/bdbdatastore/socket_apiproxy_stub.py
# Compiled at: 2009-12-02 20:07:05
import rpc_pb2, socket, struct
...
class SocketApiProxyStub(object):
    ...
    def _sendRPC(self, service, method, request, response):
        ...
        if response_wrapper.status == APPLICATION_ERROR:
            raise apiproxy_errors.ApplicationError(response_wrapper.application_error, response_wrapper.error_detail)
...
class RecordingSocketApiProxyStub(SocketApiProxyStub):
    ...
    def __init__(self, endpoint, max_request_size=MAX_REQUEST_SIZE):
        super(RecordingSocketApiProxyStub, self).__init__(endpoint, max_request_size)
    ...
    def MakeSyncCall(self, service, call, request, response):
        ...
        super(RecordingSocketApiProxyStub, self).MakeSyncCall(service, call, request, response)
