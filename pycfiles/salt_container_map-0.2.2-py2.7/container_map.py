...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/saltcontainermap/states/container_map.py
# Compiled at: 2017-10-31 04:35:58
"""
...
def set_up(name, containers=None, volumes=None, host=None, host_root=None, repository=None, default_domain=None, check_integrity=True, ignore_existing=False):
    """
    ...
    ignore_existing : False
        ...
        maps instead.
    """
    res = __salt__[b'container_map.setup'](name, containers=containers, volumes=volumes, host=host, host_root=host_root, repository=repository, default_domain=default_domain, check_integrity=check_integrity, ignore_existing=ignore_existing)
    res[b'name'] = res[b'item_id']
    return res


def merged(name, target_map, containers=None, volumes=None, host=None, host_root=None, repository=None, default_domain=None, lists_only=False):
    """
    ...
    """
    res = __salt__[b'container_map.merge'](target_map, containers=containers, volumes=volumes, host=host, host_root=host_root, repository=repository, default_domain=default_domain, lists_only=lists_only)
    res[b'name'] = res[b'item_id']
    return res
...
def created(name, instances=None, map_name=None, extra_kwargs=None):
    ...
    res = __salt__[b'container_map.create'](name, instances=instances, map_name=map_name, **create_kwargs)
...
def started(name, instances=None, map_name=None, extra_kwargs=None):
    ...
    res = __salt__[b'container_map.start'](name, instances=instances, map_name=map_name, **start_kwargs)
    res.update(name=name, instances=instances, map_name=map_name)
    return res
...
def restarted(name, instances=None, map_name=None, extra_kwargs=None):
    ...
    res = __salt__[b'container_map.restart'](name, instances=instances, map_name=map_name, **restart_kwargs)
    res.update(name=name, instances=instances, map_name=map_name)
    return res
...
def stopped(name, instances=None, map_name=None, extra_kwargs=None):
    ...
    res = __salt__[b'container_map.stop'](name, instances=instances, map_name=map_name, **stop_kwargs)
    res.update(name=name, instances=instances, map_name=map_name)
    return res
...
def removed(name, instances=None, map_name=None, extra_kwargs=None):
    ...
    res = __salt__[b'container_map.remove'](name, instances=instances, map_name=map_name, **remove_kwargs)
    res.update(name=name, instances=instances, map_name=map_name)
    return res
...
def started_up(name, instances=None, map_name=None):
    ...
    map_name
        Container map name.
    """
    res = __salt__[b'container_map.startup'](name, instances=instances, map_name=map_name)
    res.update(name=name, instances=instances, map_name=map_name)
    return res
...
def shut_down(name, instances=None, map_name=None):
    ...
    map_name
        Container map name.
    """
    res = __salt__[b'container_map.shutdown'](name, instances=instances, map_name=map_name)
    res.update(name=name, instances=instances, map_name=map_name)
    return res
...
def updated(name, instances=None, map_name=None, reload_signal=None, send_signal=False, **kwargs):
    ...
    * command, entrypoint, or environment have not been changed.
    ...
    if send_signal:
        ...
        signal = None
    res = __salt__[b'container_map.update'](name, instances=instances, map_name=map_name, reload_signal=signal)
    res.update(name=name, instances=instances, map_name=map_name)
    return res
...
def signaled(name, instances=None, map_name=None, signal=None):
    ...
    """
    res = __salt__[b'container_map.kill'](name, instances=instances, map_name=map_name, signal=signal)
    res.update(name=name, instances=instances, map_name=map_name)
    return res
...
def all_removed(name, **kwargs):
    ...
    """
    res = __salt__[b'container_map.remove_all_containers'](**clean_kwargs(**kwargs))
    res[b'name'] = b'__all__'
    return res
...
def containers_clean(name, include_initial=False, exclude=None):
    ...
    """
    res = __salt__[b'container_map.cleanup_containers'](include_initial=include_initial, exclude=exclude)
    res[b'name'] = b'__all__'
    return res
...
def images_clean(name, remove_old=False, keep_tags=None):
    ...
    """
    res = __salt__[b'container_map.cleanup_images'](remove_old=remove_old, keep_tags=keep_tags)
    res[b'name'] = b'__all__'
    return res
...
def images_updated(name, map_name=None, utility_images=True, insecure_registry=False):
    ...
    """
    res = __salt__[b'container_map.pull_latest_images'](name, map_name=map_name, utility_images=utility_images, insecure_registry=insecure_registry)
    res[b'name'] = map_name or b'__base__'
    return res
...
def logged_in(name, username=None, password=None, email=None, reauth=False):
    ...
    """
    res = __salt__[b'container_map.login'](name, username=username, password=password, email=email, reauth=reauth)
    res[b'name'] = name
    return res
...
def built(name, **kwargs):
    ...
    """
    ignore_existing = kwargs.pop(b'ignore_existing', False)
    if not ignore_existing and __salt__[b'container_map.image_tag_exists'](name):
        return dict(result=True, name=name, changes={}, comment=b'Image exists.')
    res = __salt__[b'container_map.build'](name, **kwargs)
    res[b'name'] = name
    return res
...
def mod_watch(name, sfun=None, **kwargs):
    if sfun == b'updated':
        kwargs[b'send_signal'] = True
        return updated(name, **kwargs)
    if sfun == b'built':
        kwargs[b'ignore_existing'] = True
        return built(name, **kwargs)
    return dict(name=name, result=False, changes={}, comment=(b'watch requisite is not implemented for {0}').format(sfun))