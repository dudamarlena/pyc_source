...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/saltcontainermap/modinstall.py
# Compiled at: 2015-12-12 06:22:53
import argparse
from collections import namedtuple
import logging, os, salt.config
...
def get_env():
    ...
    parser.add_argument('-c', '--salt-config', default='/etc/salt/master', help='Path to the salt master configuration file.')
    parser.add_argument('--salt-extmods', help='Path for extension modules. If not set, the setting from the master config is used.')
    parser.add_argument('--salt-root', help='Path to the master file root, e.g. /srv/salt. If not set, looks up the base environment in the master configuration file.')
...
def remove_modules():
    ...
    if res_mod and res_state:
        log.info("Removed minion modules. 'saltutil.clear_cache' can be used for distributing the removal, but 'saltutil.sync_all' should be run immediately afterwards if you have any other custom modules.")