...
# [GCC 8.4.0]
# Embedded file name: /home/vagrant/envs/conversocial/lib/python2.7/site-packages/djangosampler/views.py
...
def queries(request, query_type, date_string, offset=0, sort='total_duration'):
    ...
    query_qs = Query.objects.filter(query_type=query_type, created_dt__gte=start_date, created_dt__lt=end_date)
    ...
    pages = list([ {'number': page, 'url': reverse('queries', kwargs={'date_string': date_string, 'query_type': query_type, 'offset': PAGE_SIZE * (page - 1), 'sort': sort})} for page in pages
                 ])

    def get_sort_url(field):
        if field == sort:
            field = '-%s' % field
        return reverse('queries', kwargs={'date_string': date_string, 
    ...
    return render_to_response('djangosampler/queries.html', {'by_count_url': by_count_url, 
       ...
       'current_query_type': query_type}, context_instance=RequestContext(request))
...
def query(request, query_hash):
    ...
    back_link = reverse('queries', kwargs={'date_string': date_string, 
       'query_type': query.query_type, 
       'sort': 'total_duration', 
       'offset': 0})
    return render_to_response('djangosampler/query.html', locals(), context_instance=RequestContext(request))
...
def index(request):
    ...
    return HttpResponseRedirect(reverse('queries', kwargs={'date_string': current_date, 
       'query_type': query_type, 
       'sort': 'total_duration', 
       'offset': 0}))
...
def _get_query_types(date_string):
    ...
    for query_type in query_type_names:
        ...
        query_obj['url'] = reverse('queries', kwargs={'date_string': date_string, 
           'query_type': query_type, 
           'sort': 'total_duration', 
           'offset': 0})
...
def _get_date_links(current_date, query_type):
    ...
    for day in xrange(-7, 8):
        ...
        date_link['url'] = reverse('queries', kwargs={'date_string': date_link['friendly_name'], 
           'query_type': query_type, 
           'sort': 'total_duration', 
           'offset': 0})
