...
# [GCC 8.4.0]
# Embedded file name: /home/vagrant/envs/conversocial/lib/python2.7/site-packages/djangosampler/plugins/mongo.py
# Compiled at: 2015-11-17 05:08:55
import time
...
class Mongo(object):
    ...
    def get_remove_query(collection, spec_or_id, safe=False, **kwargs):
        ...
        return (
         '%s.remove(%s)' % (collection.name, repr(safe_spec)), 'mongo')
    ...
    def get_refresh_query(cursor):
        ...
        if collection_name == '$cmd':
            ...
            if '$orderby' in query_son:
                ordering = (', ').join(fmt(f, d) for f, d in query_son['$orderby'].items())
    ...
    @staticmethod
    def install():
        wrapped_methods = {'insert': pymongo.collection.Collection.insert, 
        ...
        for name, method in wrapped_methods.items():
            setattr(method.im_class, method.im_func.func_name, Mongo.make_wrapper(name, method))