...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/site-packages/stock/scrape/yahoo_japan.py
# Compiled at: 2016-03-20 21:36:42
# Size of source mod 2**32: 2303 bytes
import re
from .interface import Scraper, Interface, ParseError
REG_SPLIT_STOCK_DATE = re.compile('分割\\W+(?P<from_number>\\d+)株.*?(?P<to_number>\\d+)株')
REG_DATE = re.compile('(?P<year>\\d{4})年(?P<month>\\d{1,2})月(?P<day>\\d{1,2})日')
...
class History(Scraper):
    ...
    def parse(self, soup):
        ...
        return [self._parse(tr) for tr in span.next_sibling.findAll('tr')[1:]]
...
class YahooJapan(Interface):
    scrapers = [
     SplitStockDate, History, CurrentValue]