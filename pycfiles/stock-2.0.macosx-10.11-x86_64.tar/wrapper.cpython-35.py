...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/site-packages/stock/wrapper.py
# Compiled at: 2017-01-15 03:40:49
# Size of source mod 2**32: 5547 bytes
import datetime, pandas as pd

def to_seq(dates, series):
    return list([a, b] for a, b in zip(dates.values.tolist(), series.values.tolist()) if not pd.isnull(b))
...
class DayInfoQuery(object):
    ...
    def ohlc(self):
        return [(d['date'], d['opening'], d['high'], d['low'], d['closing']) for d in self.df().to_dict(orient='records')]
...
buy_conditions = {'closing_rsi_14': lambda ins: ins.closing_rsi_14 <= 30, 
...
sell_conditions = {'closing_rsi_14': lambda ins: ins.closing_rsi_14 >= 70, 
...
class DayInfo(Wrapper):
    dict_keys = [
     'high', 'low', 'opening', 'closing', 'company_id', 'js_datetime']
