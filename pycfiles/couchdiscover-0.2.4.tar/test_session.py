...
# Embedded file name: build/bdist.linux-i686/egg/couchdbsession/tests/test_session.py
# Compiled at: 2010-02-26 07:34:24
import itertools, unittest, uuid, couchdb
...
class TestFlush(TempDatabaseMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.session = session.Session(self.db, pre_flush_hook=self._flush_hook, post_flush_hook=self._flush_hook)
...
class TestFlushHook(TempDatabaseMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.db.update([{'type': 'tag', 'name': 'foo'}, {'type': 'tag', 'name': 'bar'}, {'type': 'post', 'title': 'Post #1', 'tag': 'foo'}, {'type': 'post', 'title': 'Post #2', 'tag': 'bar'},
         {'_id': '_design/test', 'views': {'tag_by_name': {'map': "function(doc) {if(doc.type=='tag') {emit(doc.name, null);}}"}, 
                      'post_by_tag': {'map': "function(doc) {if(doc.type=='post') {emit(doc.tag, null);}}"}}}])
