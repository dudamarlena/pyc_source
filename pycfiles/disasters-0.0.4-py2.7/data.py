...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/disasters/data.py
# Compiled at: 2014-03-11 17:21:20
...
file_pattern = re.compile('(\\d\\d\\d\\d)_(\\d\\d)_(\\d\\d)_(\\d\\d)_(\\d\\d)_(\\d\\d)_(\\d+)_(.*?)\\.html')
_disaster_categories = ('hurricanes', 'typhoons', 'cyclones', 'tropicalstorms')
...
def get_html(category, num_revs=1, latest_first=False):
    ...
    for fname in listdir(d):
        ...
        if m:
            rdate = datetime(year=int(m.group(1)), month=int(m.group(2)), day=int(m.group(3)), hour=int(m.group(4)), minute=int(m.group(5)), second=int(m.group(6)))
