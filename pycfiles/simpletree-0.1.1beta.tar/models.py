...
# [GCC 8.4.0]
# Embedded file name: /home/klen/Projects/simpletree/simpletree/models.py
# Compiled at: 2012-10-14 16:20:53
import sys
...
class NodeMeta(models.Model.__metaclass__):
    ...
    def __new__(mcs, class_name, bases, class_dict):
        ...
        relation_model = type(relation_name, (
         models.Model,), dict(__module__=class_module, parent=models.ForeignKey(('{0}.{1}').format(relation_app_label, class_name), related_name='_child_links'), child=models.ForeignKey(('{0}.{1}').format(relation_app_label, class_name), related_name='_parent_links'), depth=models.PositiveIntegerField(default=0), Meta=RelationMeta, __repr__=lambda s: ('{0}/{1} [{2}]').format(s.parent_id, s.child_id, s.depth)))
        class_dict[('node_descendants').format(class_name.lower())] = models.ManyToManyField('self', through=('{0}.{1}').format(relation_app_label, relation_name), symmetrical=False, related_name=('node_ancestors').format(class_name.lower()))
...
class Node(models.Model):
    ...
    parent = models.ForeignKey('self', null=True, blank=True, verbose_name=_('Parent node'))
    ...
    def get_tree(self):
        return self.__class__.objects.filter(models.Q(_child_links__child=self) | models.Q(_parent_links__parent=self)).distinct()
