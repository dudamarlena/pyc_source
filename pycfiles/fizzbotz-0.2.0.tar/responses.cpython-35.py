...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/martensm/fizzbotz/fizzbotz/responses.py
# Compiled at: 2016-02-18 00:02:19
# Size of source mod 2**32: 3987 bytes
import html, random, re, string, bs4, fizzbotz.util as util
...
class TwitchChat:
    ...
    async def get_twitchquotes_pasta(self, from_html=None):
        ...
        text = await util.get_markup(url) if from_html is None else from_html
        raw_quote = self._twitchquotes_regex.sub('\\1', text)
    ...
    async def get(self):
        pasta_funcs = (
         self.get_bestoftwitchchat_pasta, self.get_twitchquotes_pasta)
