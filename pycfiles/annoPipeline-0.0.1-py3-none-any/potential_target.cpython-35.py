...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/potential_target.py
# Compiled at: 2019-07-31 07:09:56
...
def print_fasta(entry, seq, out, gene):
    ...
    if 'ID' in entry.attributes.keys():
        out.write('>{0}_{1}-{2}_{3}\n{4}\n'.format('_'.join([locus,
         entry.attributes['ID']]), entry.start, entry.end, entry.strand, seq))
    else:
        out.write('>{0}_{1}-{2}_{3}\n{4}\n'.format('_'.join([locus, 'NA']), entry.start, entry.end, entry.strand, seq))
...
def read_file(seq_file, gff_file, target_folder, features):
    ...
    for entry in Gff3Parser().entries(g_h):
        if os.path.exists(os.path.join(target_folder, '_'.join([entry.seq_id, 'target.fa']))):
            os.remove(os.path.join(target_folder, '_'.join([entry.seq_id, 'target.fa'])))
    ...
    if len(cdss_f) == 0 and len(cdss_r) == 0:
        print('No assigned features can be found. Please check your genome annotation.And assign correct features to --target_feature.')
        sys.exit()
    ...
    genes = sorted(genes, key=lambda k: (k.seq_id, k.start,
     k.end, k.strand))
    return (
     fasta, cdss_f, cdss_r, genes)


def check_parent_gene(cds, genes):
    target_gene = None
    for gene in genes:
        if gene.seq_id == cds.seq_id and gene.strand == cds.strand and gene.start > cds.end:
            break
        elif 'Parent' in cds.attributes.keys() and gene.attributes['ID'] in cds.attributes['Parent'].split(','):
            target_gene = gene
...
def deal_cds_forward(cdss_f, target_folder, fasta, genes, tar_start, tar_end):
    ...
    for cds in cdss_f:
        if cds.seq_id != pre_id:
            out = open(os.path.join(target_folder, '_'.join([cds.seq_id, 'target.fa'])), 'w')
            pre_id = cds.seq_id
        ...
        if cds.start + tar_end < len(fasta) and cds.end - cds.start >= tar_end:
            ...
            if cds.start + tar_end >= len(fasta):
                end = len(fasta)
...
def deal_cds_reverse(cdss_r, target_folder, fasta, genes, tar_start, tar_end):
    ...
    for cds in cdss_r:
        if cds.seq_id != pre_id:
            out = open(os.path.join(target_folder, '_'.join([cds.seq_id, 'target.fa'])), 'a')
            pre_id = cds.seq_id
...
def potential_target(gff_file, seq_file, target_folder, args_tar):
    ...
    fasta, cdss_f, cdss_r, genes = read_file(seq_file, gff_file, target_folder, args_tar.features)
    sort_cdss_f = sorted(cdss_f, key=lambda k: (k.seq_id, k.start,
     k.end, k.strand))
    deal_cds_forward(sort_cdss_f, target_folder, fasta, genes, args_tar.tar_start, args_tar.tar_end)
    sort_cdss_r = sorted(cdss_r, reverse=True, key=lambda k: (
     k.seq_id, k.start, k.end, k.strand))
    deal_cds_reverse(sort_cdss_r, target_folder, fasta, genes, args_tar.tar_start, args_tar.tar_end)