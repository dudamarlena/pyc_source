...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/validate_gene.py
...
def print_stat(feature, num, out):
    if num['_'.join(['all', feature])] != 0:
        out.write('The number of {0} which is start from TSS: {1} ({2})\n'.format(feature, num[feature], float(num[feature]) / float(num['_'.join(['all', feature])])))
    else:
        out.write('The number of {0} which is start from TSS: {1} ({2})\n'.format(feature, num[feature], 'NA'))
...
def compare_tss(tsss, gff, utr_length, num_all, num_strain, program):
    ...
    for tss in tsss:
        length = utr_length
        if gff.feature == 'CDS' or gff.feature == 'rRNA' or gff.feature == 'tRNA':
            if gff.seq_id == tss.seq_id and gff.start < tss.start and gff.strand == '+' and tss.strand == '+':
                break
        elif gff.seq_id == tss.seq_id and gff.end < tss.start - utr_length and gff.strand == '-' and tss.strand == '-':
            break
        if gff.seq_id == tss.seq_id and gff.strand == '+' and tss.strand == '+' and gff.start - tss.start <= utr_length and gff.start - tss.start >= 0:
            detect = True
            if gff.start - tss.start <= length:
                start = tss
                length = gff.start - tss.start
        elif gff.seq_id == tss.seq_id and gff.strand == '-' and tss.strand == '-' and tss.start - gff.end <= utr_length and tss.start - gff.end >= 0:
            detect = True
    ...
    if detect:
        detect = False
        gff.attributes['start_' + type_] = type_ + '_' + str(start.start) + start.strand
        if gff.feature == 'CDS':
            num_all['cds'] += 1
...
def validate_gff(tss_file, gff_file, stat_file, out_cds_file, utr_length, program):
