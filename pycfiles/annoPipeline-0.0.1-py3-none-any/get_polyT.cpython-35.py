...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/get_polyT.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 17652 bytes
import math
from annogesiclib.gff3 import Gff3Parser
from annogesiclib.helper import Helper

def import_candidate(cands, term_features, strain, start, end, ut, name, total_length, strand, parent_p, parent_m, p_pos, m_pos):
...
def filter_term(cands, terms, miss_rate):
    ...
    for cand1 in cands:
        ...
        if not cand1['print'] and float(cand1['miss']) / float(stem_len) <= cutoff_miss:
            tmp_term = cand1.copy()
            for cand2 in cands:
                if tmp_term['strain'] == cand2['strain'] and tmp_term['miss'] >= cand2['miss']:
                    if tmp_term['start'] >= cand2['start'] and tmp_term['start'] < cand2['end'] and tmp_term['end'] > cand2['end']:
                        tmp_term['start'] = cand2['start']
                        check_miss(tmp_term, cand2, cutoff_miss)
                        cand2['print'] = True
                    else:
                        if cand2['start'] > tmp_term['start'] and cand2['start'] < tmp_term['end'] and cand2['end'] >= tmp_term['end']:
                            tmp_term['end'] = cand2['end']
                            check_miss(tmp_term, cand2, cutoff_miss)
                            cand2['print'] = True
                        else:
                            if tmp_term['start'] >= cand2['start'] and tmp_term['end'] <= cand2['end']:
                                tmp_term['start'] = cand2['start']
                                tmp_term['end'] = cand2['end']
                                check_miss(tmp_term, cand2, cutoff_miss)
                                cand2['print'] = True
                            elif cand2['start'] >= tmp_term['start'] and cand2['end'] <= tmp_term['end']:
                                cand2['print'] = True
                                check_miss(tmp_term, cand2, cutoff_miss)
...
def detect_candidates(seq, sec, name, strain, start, end, parent_p, parent_m, strand, args_term, p_pos, m_pos):
    """check the criteria of sec str of terminator"""
    term_len = 2 * args_term.max_stem + 2 * (args_term.max_stem * args_term.miss_rate) + args_term.max_loop
    cands = []
            ...
            if nts['ut'] >= args_term.at_tail:
                ...
                if stop > 10:
                    ...
                    if detects['conflict'] is False:
                        ...
                        if total_length <= term_len and term_features['loop'] <= args_term.max_loop and term_features['loop'] >= args_term.min_loop and (term_features['r_stem'] + term_features['l_stem'] - term_features['real_miss']) / 2 >= args_term.min_stem and (term_features['r_stem'] + term_features['l_stem'] - term_features['real_miss']) / 2 <= args_term.max_stem:
                            nts['detect'] = True
                            if strand == '+':
                                import_candidate(cands, term_features, strain, start + (len(sec[0:stop + 1]) - term_features['st_pos']) - 1, start + stop, nts['ut'], name, total_length, strand, parent_p, parent_m, p_pos, m_pos)
                            else:
                                import_candidate(cands, term_features, strain, end - stop, end - (len(sec[0:stop + 1]) - term_features['st_pos']) + 1, nts['ut'], name, total_length, strand, parent_p, parent_m, p_pos, m_pos)
                            num_sec += 1
...
def check_parent(genes, term, detects, strand, fuzzy_up, fuzzy_down, type_):
    tmp = None
    for gene in genes:
        if term['strain'] == gene.seq_id and gene.strand == strand:
            if type_ == 'parent_p':
                if term['start'] - fuzzy_down <= gene.end and term['start'] >= gene.end:
                    detects[type_] = True
                    tmp = get_feature(gene)
            else:
                if gene.end - term['end'] <= fuzzy_up and gene.end - term['end'] >= 0:
                    detects[type_] = True
                    tmp = get_feature(gene)
                elif gene.end - term['start'] > fuzzy_up and gene.end - term['start'] >= 0:
                    break
        elif type_ == 'parent_m':
            if term['end'] + fuzzy_down >= gene.start and term['end'] <= gene.start:
                detects[type_] = True
                tmp = get_feature(gene)
            else:
                if term['start'] - gene.start <= fuzzy_up and term['start'] - gene.start >= 0:
                    detects[type_] = True
                    tmp = get_feature(gene)
...
def parents(terms, genes, args_term):
    ...
    for term in terms:
        ...
        if 'tran' in term['parent_p']:
            tmp_p = check_parent(genes, term, detects, '+', args_term.fuzzy_up_gene, args_term.fuzzy_down_gene, 'parent_p')
            pos = term['p_pos'].split('-')[(-1)]
            if term['start'] - int(pos) <= args_term.fuzzy_down_ta and term['start'] - int(pos) >= 0 or int(pos) - term['end'] <= args_term.fuzzy_up_ta and int(pos) - term['end'] >= 0:
                pass
        else:
            term['parent_p'] = ''
        if 'tran' in term['parent_m']:
            tmp_m = check_parent(genes, term, detects, '-', args_term.fuzzy_up_gene, args_term.fuzzy_down_gene, 'parent_m')
            pos = term['m_pos'].split('-')[0]
            if int(pos) - term['end'] <= args_term.fuzzy_down_ta and int(pos) - term['end'] >= 0 or term['start'] - int(pos) <= args_term.fuzzy_up_ta and term['start'] - int(pos) >= 0:
                pass
            else:
                term['parent_m'] = ''
            if detects['parent_p']:
...
def compare_anno(gffs, cands, fuzzy_up, fuzzy_down):
    ...
    for cand in cands:
        for gff in gffs:
            if gff.seq_id == cand['strain'] and gff.strand == cand['strand']:
                if cand['strand'] == '+':
                    if gff.start <= cand['start'] and gff.end >= cand['start'] and gff.end <= cand['end']:
                        detect = True
                        break
                    else:
                        if math.fabs(gff.end - cand['end']) <= fuzzy_up and gff.end >= cand['end']:
                            detect = True
                            break
                        elif math.fabs(gff.end - cand['start']) <= fuzzy_down and gff.end <= cand['start']:
                            detect = True
                            break
                else:
                    if gff.start >= cand['start'] and gff.start <= cand['end'] and gff.end >= cand['end']:
                        detect = True
                        break
                    else:
                        if math.fabs(gff.start - cand['end']) <= fuzzy_down and gff.start >= cand['end']:
                            detect = True
                            break
                        elif math.fabs(gff.start - cand['start']) <= fuzzy_up and cand['start'] >= gff.start:
                            detect = True
                            break
        ...
        for line in s_h:
            ...
            if line.startswith('>'):
                ...
                if len(sec_seq['seq']) <= 6:
                    ...
                    cands = detect_candidates(sec_seq['seq'], sec_seq['sec'], name, strain, start, end, parent_p, parent_m, strand, args_term, p_pos, m_pos)
                cands = sorted(cands, key=lambda x: (x['miss'], x['start']))
                new_cands_gene = compare_anno(genes, cands, args_term.fuzzy_up_gene, args_term.fuzzy_down_gene)
                new_cands_ta = compare_anno(trans, cands, args_term.fuzzy_up_ta, args_term.fuzzy_down_ta)
