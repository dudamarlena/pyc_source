...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/detect_coverage_term.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 24465 bytes
...
def compare_ta(terms, tas, fuzzy):
    ...
    for term in terms:
        for ta in tas:
            ...
            if ta.seq_id == term['strain'] and ta.strand == term['strand'] and (start <= term['start'] and end >= term['start'] and end <= term['end'] or start >= term['start'] and end <= term['end'] or start >= term['start'] and start <= term['end'] and end >= term['end'] or start <= term['start'] and end >= term['end']):
                term['express'] = 'True'
...
def compare_transtermhp(hps, fr_terms):
    ...
    for term in fr_terms:
        detect = False
        for hp in hps:
            if hp.seq_id == term['strain'] and hp.strand == term['strand'] and (hp.start <= term['start'] and hp.end >= term['start'] and hp.end <= term['end'] or hp.start >= term['start'] and hp.end <= term['end'] or hp.start >= term['start'] and hp.start <= term['end'] and hp.end >= term['end'] or hp.start <= term['start'] and hp.end >= term['end']):
...
def compare_replicates(term_covers, template_texs, cond, args_term):
    ...
    diff = []
    detect = False
    detect_num = check_tex(template_texs, term_covers, term_datas, None, 'terminator', None, None, None, None, 0, args_term.tex_notex)
    if 'texnotex' in cond:
...
def coverage2term(covers, term, hl_covers, hl_poss, strand, term_covers, track, args_term, start_plus, end_minus, lib_type):
    ...
    for cover in covers:
        ...
        if term['start'] <= cover_pos + args_term.fuzzy and term['end'] >= cover_pos - args_term.fuzzy:
            first = coverage_comparison(cover, hl_covers, hl_poss, first, strand, cover_pos)
        else:
            if strand == '+' and cover_pos > term['end'] + args_term.fuzzy:
                break
            elif strand == '-' and cover_pos < term['start'] - args_term.fuzzy:
                break
            if first is not True and hl_covers['high'] > 0 and hl_covers['low'] / hl_covers['high'] < args_term.decrease and hl_covers['low'] > -1:
...
def get_coverage(term, wigs, strand, template_texs, args_term):
        ...
        if wig_strain == term['strain']:
            for cond, tracks in conds.items():
                ...
                for lib_name, covers in tracks.items():
                    ...
                    if strand == '-':
                        covers = covers[::-1]
                    coverage2term(covers, term, hl_covers, hl_poss, strand, term_covers, track, args_term, c_start, c_end, lib_type)

                if len(term_covers) != 0:
                    tmp_cov, tmp_diff, term_datas[cond], detect_nums[cond] = compare_replicates(term_covers, template_texs, cond, args_term)
    ...
    return (
     diff_cover, diff, term_datas, detect_nums)
    ...
    if len(terms) != 0:
        for tmp in terms:
            if term['miss'] < tmp['miss']:
                ...
                if 'diff_cover' in term.keys() and 'diff_cover' in tmp.keys():
    ...
    if term['express'] == 'True' and term['diff_cover'] != -1:
        if term['diff']['high'] >= args_term.cutoff_coverage:
            ...
            for datas in term['datas'].values():
                for data in datas:
                    if first:
                        out_t.write('{0}(diff={1};high={2};low={3})'.format(data['track'], data['diff'], data['high'], data['low']))
                        first = False
                    else:
                        out_t.write(';{0}(diff={1};high={2};low={3})'.format(data['track'], data['diff'], data['high'], data['low']))

            else:
                out_t.write('{0}(diff={1};high={2};low={3})'.format(term['diff']['track'], term['diff_cover'], term['diff']['high'], term['diff']['low']))
...
def print2file(num, term, coverage, parent, out, out_t, method, args_term):
    ...
    if 'detect_num' in term.keys() and term['diff_cover'] != -1:
    ...
    if term['express'] == 'True' and term['diff_cover'] != -1:
        if term['diff']['high'] >= args_term.cutoff_coverage:
            diff = '{0}(high:{1},low:{2})'.format(term['diff']['track'], term['diff']['high'], term['diff']['low'])
            attribute_string = get_attribute_string(num, name, parent, diff, term, coverage, method)
        elif term['diff']['high'] < args_term.cutoff_coverage:
            attribute_string = get_attribute_string(num, name, parent, 'NA', term, 'No_coverage_decreasing', method)
    else:
        if term['express'] == 'True' and term['diff_cover'] == -1:
            attribute_string = get_attribute_string(num, name, parent, 'NA', term, 'No_coverage_decreasing', method)
        elif term['express'] == 'False':
            attribute_string = get_attribute_string(num, name, parent, 'NA', term, 'NA', method)
...
def print_detect_undetect(term, num, out, out_t, detect, args_term):
    ...
    if term['strand'] == '+':
        print2file(num, term, detect, term['parent_p'], out, out_t, term['method'], args_term)
        num += 1
    else:
        print2file(num, term, detect, term['parent_m'], out, out_t, term['method'], args_term)
        num += 1
    return num


def term_validation(pre_term, term, detect, detect_terms, out, out_t, num, args_term, final_terms):
    ...
    if pre_term['name'] != term['name'] or args_term.keep_multi:
        if detect:
            ...
            final_terms['undetect'] = final_terms['undetect'] + detect_terms['undetect']
        detect_terms['detect'] = []
        ...
        if term['strand'] == '+':
            if term['detect_p']:
                detect = True
                detect_terms['detect'] = compare_term(term, detect_terms['detect'])
            elif not detect:
                detect_terms['undetect'] = compare_term(term, detect_terms['undetect'])
        else:
            if term['detect_m']:
                detect = True
                detect_terms['detect'] = compare_term(term, detect_terms['detect'])
...
def print_term(terms, out, out_t, args_term):
    ...
    for term in terms:
        if first:
            ...
            num, detect = term_validation(pre_term, term, detect, detect_terms, out, out_t, num, args_term, final_terms)
            pre_term = term
    ...
    if detect:
        ...
        final_terms['undetect'] = final_terms['undetect'] + detect_terms['undetect']
    remove_repeat(final_terms['detect'], num, out, out_t, 'True', args_term)
    remove_repeat(final_terms['undetect'], num, out, out_t, 'False', args_term)
...
def del_repeat_term(terms):
    ...
    for term in terms:
        ...
        if first:
            first = False
            pre_term = term
        elif term['strain'] == pre_term['strain'] and term['strand'] == pre_term['strand'] and term['parent_p'] == pre_term['parent_p'] and term['parent_m'] == pre_term['parent_m']:
            if term['start'] <= pre_term['start'] and term['end'] >= pre_term['start'] and term['end'] <= pre_term['end']:
                detect = True
                pre_term['start'] = term['start']
            else:
                if term['start'] <= pre_term['start'] and term['end'] >= pre_term['end']:
                    detect = True
                    pre_term['start'] = term['start']
                    ...
                    if term['start'] >= pre_term['start'] and term['end'] <= pre_term['end']:
                        detect = True
                    elif term['start'] >= pre_term['start'] and term['start'] <= pre_term['end'] and term['end'] >= pre_term['end']:
                        detect = True
                        pre_term['end'] = term['end']
            if detect:
...
def compute_wig(wig_file, libs, terms, strand, texs, args_term):
    ...
    for term in terms:
        if term['strand'] == strand and term['express'] == 'True':
            term['diff_cover'], term['diff'], term['datas'], detect_nums = get_coverage(term, wigs, strand, texs, args_term)
            term['detect_num'] = {}
            for cond, num in detect_nums.items():
...
def remove_repeat(terms, num, out, out_t, detect, args_term):
    ...
    for term1 in terms:
        if args_term.keep_multi:
            num = print_detect_undetect(term1, num, out, out_t, detect, args_term)
        else:
            tmp_term = term1
            for term2 in terms:
                if term1['strain'] == term2['strain'] and term1['strand'] == term2['strand']:
                    if term1['strand'] == '+':
                        ...
                        if parents1.issubset(parents2):
                            if parents2.issubset(parents1):
                                if tmp_term['end'] < term2['end']:
                                    tmp_term = term2
                                elif tmp_term['end'] == term2['end'] and tmp_term['start'] > term2['start']:
                                    tmp_term = term2
                            else:
                                tmp_term = term2
                    else:
                        ...
                        if parents1.issubset(parents2):
                            if parents2.issubset(parents1):
                                if tmp_term['start'] > term2['start']:
                                    tmp_term = term2
                                elif tmp_term['start'] == term2['start'] and term1['end'] < term2['end']:
                                    tmp_term = term2
                            else:
                                tmp_term = term2

            if tmp_term not in finals:
                num = print_detect_undetect(tmp_term, num, out, out_t, detect, args_term)
                finals.append(tmp_term)

    return num


def detect_coverage(term_table, gff_file, tran_file, seq_file, wig_f_file, wig_r_file, tranterm_file, wig_folder, output_file, output_table, args_term):
    """For detecting the coverage of terminator"""
    gffs, tas, hps, fr_terms, seq = read_data(gff_file, tran_file, tranterm_file, seq_file, term_table)
