...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/compare_srna_term.py
...
def print_table(srna_table, out_t, srnas):
    ...
    for row in csv.reader(fh, delimiter='\t'):
        for srna in srnas:
            if row[0] == srna.seq_id and int(row[2]) == srna.start and int(row[3]) == srna.end and row[4] == srna.strand:
...
def compare_srna_term(srna_gff, srna_table, term_file, fuzzy_b, fuzzy_a):
    ...
    for srna in srnas:
        detect = False
        for term in terms:
            if srna.seq_id == term.seq_id and srna.strand == term.strand:
                if srna.strand == '+':
                    if srna.end - term.end <= fuzzy_b and srna.end >= term.end and srna.start < term.start or term.start - srna.end <= fuzzy_a and term.start >= srna.end or srna.end > term.start and srna.end < term.end and srna.start < term.start:
                        term_string = term.feature + ':' + str(term.start) + '-' + str(term.end) + '_' + term.strand
                        srna.attributes['with_term'] = term_string
                        detect = True
                        break
                elif term.start - srna.start <= fuzzy_b and term.start >= srna.start and term.end < srna.end or srna.start - term.end <= fuzzy_a and srna.start >= term.end or srna.start > term.start and srna.start < term.end and srna.end > term.end:
                    term_string = term.feature + ':' + str(term.start) + '-' + str(term.end) + '_' + term.strand
                    srna.attributes['with_term'] = term_string
                    detect = True
                    break
        ...
        if 'end_cleavage' in srna.attributes.keys():
            if srna.attributes['end_cleavage'] != 'NA' and 'with_term' not in srna.attributes.keys():
                srna.attributes['with_term'] = srna.attributes['end_cleavage']
            elif srna.attributes['end_cleavage'] != 'NA' and 'with_term' in srna.attributes.keys():
                srna.attributes['with_term'] = ','.join([
                 srna.attributes['with_term'],
                 srna.attributes['end_cleavage']])
            if detect:
                out_g.write(srna.info + ';with_term=' + srna.attributes['with_term'] + '\n')
            else:
