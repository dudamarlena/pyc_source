...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/filter_low_expression.py
...
def stat(tars, refs, cutoff, gene_length, cluster):
    ...
    for ref in refs:
        ...
        detect = False
        for tar in tars:
            if ref.seq_id == tar.seq_id and ref.strand == tar.strand and float(tar.attributes['coverage']) >= cutoff and tar.start <= int(gene_length) and math.fabs(ref.start - tar.start) <= cluster:
                stats['tp'] += 1
    ...
    for tar in tars:
        if not tar.attributes['print'] and float(tar.attributes['coverage']) >= cutoff and tar.start <= int(gene_length):
            stats['fp'] += 1
...
def change_best(num_ref, best, stat_value):
    ...
    if num_ref > 100:
        ...
        if best['tp'] - stat_value['tp'] >= 5:
            ...
            if best['tp'] <= stat_value['tp'] and best['fp'] >= stat_value['fp']:
                best = stat_value.copy()
                change = True
...
def filter_low_expression(gff_file, args_tss, wig_f_file, wig_r_file, out_file):
    ...
    first = True
    while True:
        stat_value, num_ref = stat(tars, refs, cutoff, args_tss.gene_length, args_tss.cluster)
        if first:
            first = False
            best = stat_value.copy()
            continue
