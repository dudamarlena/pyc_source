...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/compare_sRNA_sORF.py
...
def print_file(datas, out, feature):
    for data in datas:
        ...
        data.attribute_string = ';'.join(['='.join(items) for items in data.attributes.items()])
...
def srna_sorf_comparison(sRNA_file, sORF_file, sRNA_out, sORF_out):
    ...
    for srna in srnas:
        for sorf in sorfs:
            if srna.seq_id == sorf.seq_id and srna.strand == sorf.strand and (srna.start <= sorf.start and srna.end >= sorf.end or srna.start >= sorf.start and srna.end <= sorf.end or srna.start <= sorf.start and srna.end >= sorf.start and srna.end <= sorf.end or srna.start >= sorf.start and srna.start <= sorf.end and srna.end >= sorf.end):
