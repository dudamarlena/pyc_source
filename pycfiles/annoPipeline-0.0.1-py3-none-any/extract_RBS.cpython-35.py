...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/extract_RBS.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 8575 bytes
                ...
                for nts in range(0, len(inter['seq']) - len(ribo_seq)):
                    ...
                    for index in range(len(ribo_seq)):
                        if miss > args_ribo.fuzzy_rbs:
                            break
                        elif inter['seq'][nts:nts + len(ribo_seq)][index] != ribo_seq[index]:
                            miss += 1

                    if miss <= args_ribo.fuzzy_rbs and len(inter['seq'][nts:nts + len(ribo_seq)]) >= len(ribo_seq):
                        rbss.append(inter)
                        detect = True
                        break
...
def compare_pre_cds(first, cdss, cds, seq):
    ...
    for pre_cds in cdss:
        if pre_cds.seq_id == cds.seq_id and pre_cds.strand == cds.strand:
            if pre_cds.strand == '+':
                if first:
                    start = 1
                    end = cds.start
                    ...
                    if pre_cds.end < cds.start:
                        start = pre_cds.end
                        end = cds.start
                        detect_cds = True
                    elif pre_cds.end >= cds.start:
                        break
            elif pre_cds.start > cds.end:
                start = cds.end
                end = pre_cds.start
                detect_cds = True
                break
...
def compare_tran(cds, trans, seq, inters, fuzzy, start, end):
    ...
    detect = False
    for tran in trans:
        if tran.seq_id == cds.seq_id and tran.strand == cds.strand:
            if tran.strand == '+':
                if cds.start > tran.start and cds.start <= tran.end:
                    if start < tran.start:
                        start = tran.start
                        detect = True
            else:
                if tran.start > cds.start:
                    break
        elif cds.end > tran.start and cds.end <= tran.end:
            if end > tran.start:
                end = tran.start
                detect = True
            else:
                if tran.start > cds.end:
                    break
    ...
    if detect:
        ...
        for tran in trans:
            if tran.seq_id == cds.seq_id and tran.strand == cds.strand and (start <= tran.start and end >= tran.end or start >= tran.start and end <= tran.end or start <= tran.start and end <= tran.end and end >= tran.start or start >= tran.start and start <= tran.end and end >= tran.end):
                inter = {'start': tran.start, 'end': cds.start, 
                 'strain': cds.seq_id, 'strand': cds.strand}
                extract_inter_seq(inter, cds, seq, fuzzy, inters)
                break
...
def extract_potential_rbs(seq_file, gff_file, tss_file, tran_file, out_file, args_ribo, feature):
    """Get the potential riboswitch or RNA-thermometer"""
    out = open(out_file, 'w')
    cdss, seq, tsss, trans = read_file(seq_file, gff_file, tss_file, tran_file)
    inters = extract_seq(cdss, seq, tsss, trans, args_ribo.fuzzy, args_ribo.utr)
    rbss = detect_site(inters, args_ribo)
    num = 0
    for rbs in rbss:
        out.write('>' + feature + '_{0}\n'.format('|'.join([str(num), rbs['strain'], rbs['strand'],
