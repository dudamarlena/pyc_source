...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/stat_sublocal.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 4608 bytes
import csv, numpy as np, matplotlib as mpl
mpl.use('Agg')
...
def plot(subs, total, unknown, strain, prefix_name):
    ...
    sort_subs = sorted(subs.items(), key=lambda x: x[1], reverse=True)
    ...
    plt.xticks(ind + width, classes_no_unknown, rotation=40, fontsize=20, ha='right')
    plt.yticks(fontsize=16)
    plt.tight_layout(2, None, None, None)
...
def print_file_and_plot(sub, total_nums, unknown_nums, strain, out_stat, prefix_name):
    ...
    out_stat.write('Total including Unknown is {0}; Total excluding Unknown is {1}\n'.format(total_nums[strain], total_nums[strain] - unknown_nums[strain]))
    for local, num in sub.items():
        if local != 'Unknown':
            out_stat.write('\t{0}\t{1}(including Unknown {2}; excluding Unknonwn {3})\n'.format(local, num, float(num) / float(total_nums[strain]), float(num) / (float(total_nums[strain]) - float(unknown_nums[strain]))))
        else:
            out_stat.write('\t{0}\t{1}(including Unknown {2})\n'.format(local, num, float(num) / float(total_nums[strain])))
...
def stat_sublocal(psortb_file, prefix_name, stat_file):
    ...
    if len(subs) > 2:
        print_file_and_plot(subs['all_genome'], total_nums, unknown_nums, 'all_genome', out_stat, prefix_name)
    for strain, sub in subs.items():
        if strain != 'all_genome':
            print_file_and_plot(sub, total_nums, unknown_nums, strain, out_stat, prefix_name)