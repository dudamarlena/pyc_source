...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/gen_table_tran.py
            ...
            for cond, tracks in conds.items():
                for lib_name, covers in tracks.items():
                    ...
                    for cover in covers[c_start:c_end]:
                        ...
                        if cover_pos >= tran.start and cover_pos <= tran.end:
                            total = cover + total
                            if cover > infos[track]['high']:
                                infos[track]['high'] = cover
                            if cover < infos[track]['low'] or infos[track]['low'] == -1:
                                infos[track]['low'] = cover
                            pos += 1

                    infos[track]['avg'] = float(total) / float(tran.end - tran.start + 1)
...
def compare_ta_genes(tran, genes, out):
    ...
    if len(genes) != 0:
        for gene in genes:
            if gene.seq_id == tran.seq_id and gene.strand == tran.strand and (tran.start <= gene.start and tran.end >= gene.end or tran.start >= gene.start and tran.end <= gene.end or tran.start <= gene.start and tran.end <= gene.end and tran.end >= gene.start or tran.start >= gene.start and tran.start <= gene.end and tran.end >= gene.end):
...
def print_coverage(trans, out, out_gff, wigs_f, wigs_r, gff_file):
    ...
    for tran in trans:
        infos = {}
        tran.attributes['detect_lib'] = tran.attributes['detect_lib'].replace('tex_notex', 'TEX+/-')
        ...
        for track, cover in infos.items():
            if best != -1:
                out.write(';')
            out.write('{0}({1})'.format(track, str(cover['avg'])))
            if cover['avg'] > best:
                best = cover['avg']
                best_track = track
                best_cover = cover
        ...
        attribute_string = ';'.join(['='.join(items) for items in new_attrs.items()])
...
def gen_table_transcript(gff_folder, args_tran):
    ...
    for gff in os.listdir(gff_folder):
        if os.path.isfile(os.path.join(gff_folder, gff)):
            ...
            trans = []
            out = open(os.path.join(args_tran.out_folder, 'tables', gff.replace('.gff', '.csv')), 'w')
            ...
            if args_tran.gffs is not None:
                gff_file = os.path.join(args_tran.gffs, gff.replace('_transcript', ''))
            ...
            out.close()
            out_gff.close()
            shutil.move(os.path.join(args_tran.out_folder, 'tmp_gff'), os.path.join(gff_folder, gff))