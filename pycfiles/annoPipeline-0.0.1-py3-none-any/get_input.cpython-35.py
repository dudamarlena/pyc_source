...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/get_input.py
...
def get_file(ftp, input_folder, files_type, log):
    ...
    for file_ in os.listdir(input_folder):
        ...
        if file_[-3:] == 'fna':
            ...
            if file_[-5:] == 'fasta':
                ...
                if file_[-2:] == 'fa':
                    ...
                    if file_[-6:] == 'fna.gz' and '_genomic' in file_:
                        if '_cds_from_genomic' in file_ or '_rna_from_genomic' in file_:
                        ...
                        if file_[-6:] == 'gff.gz' or file_[-3:] == 'gff':
                            ...
                            shutil.move(input_file, os.path.join(input_folder, gff_name + '.gff'))
                            fh.close()
        ...
        if checks['detect']:
            checks['detect'] = False
            checks['change'], seq_name = deal_detect(input_file, filename, checks['change'], input_folder)