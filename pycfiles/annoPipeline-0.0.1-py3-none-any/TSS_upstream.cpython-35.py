...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/TSS_upstream.py
...
def upstream(tss_file, fasta_file, gff_file, out_class, args_pro, prefix):
    ...
    for tss in tsss:
        if 'type' not in tss.attributes.keys() and args_pro.source:
            print('Error: The TSS gff file may not generated from ANNOgesic.Please run with --tss_source!')
            sys.exit()
        if args_pro.source:
    ...
    if not args_pro.source:
        if args_pro.tex_wigs is not None:
            ...
            wigs_f = read_wig(os.path.join(args_pro.wig_path, prefix + '_forward.wig'), '+', libs)
            wigs_r = read_wig(os.path.join(args_pro.wig_path, prefix + '_reverse.wig'), '+', libs)
        else:
            wigs_f = None
            wigs_r = None
