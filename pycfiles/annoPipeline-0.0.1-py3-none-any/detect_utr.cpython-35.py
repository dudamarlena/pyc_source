...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/detect_utr.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 34299 bytes
import math, shutil, matplotlib as mpl
...
def plot(utr, utr_pri, utr_sec, filename, source, utr_type, base_5utr):
    ...
    if utr_type == '5utr':
        if source and base_5utr != 'transcript':
            n, bins, hist1 = plt.hist(utr, bin_num, color='#FF9999', label='secondary', edgecolor='black', linewidth=1)
            n, bins, hist2 = plt.hist(utr_pri, bin_num, color='#9999FF', label='primary', edgecolor='black', linewidth=1)
        ...
        n, bins, hist = plt.hist(utr, bin_num, color='#9999FF', label="3'UTR", edgecolor='black', linewidth=1)
...
def check_ta(tas, utr_start, utr_end, seq_id, strand):
    ...
    detect = False
    for ta in tas:
        if ta.seq_id == seq_id and ta.strand == strand and ta.start <= utr_start and ta.end >= utr_end:
            detect = True
            break
...
def get_print_string_5utr(num_utr, name_utr, length, tss, cds_name, locus_tag, ta, source, out, start, end, utrs_tss):
    if 'Name' not in tss.attributes.keys():
        tss.attributes['Name'] = tss.feature + ':' + str(tss.start) + '-' + str(tss.end) + '_' + tss.strand
    ...
    if ta is not None:
        ...
        attribute_string = ';'.join([
         attribute_string,
         '='.join(['Parent',
          'Transcript:' + str(ta.start) + '-' + str(ta.end) + '_' + ta.strand])])
    out.write('{0}\tANNOgesic\t5UTR\t{1}\t{2}\t.\t{3}\t.\t{4}\n'.format(tss.seq_id, start, end, tss.strand, attribute_string))
    utrs_tss.append({'strain': tss.seq_id, 'start': start, 'end': end, 
     'strand': tss.strand})


def get_5utr(tss, near_cds, utr_strain, utr_all, tas, num_utr, num, num_tss, cds_name, locus_tag, out, args_utr, utrs_tss, check_cdss, pres):
    ...
    if tss.strand == '+':
        ...
        if str(near_cds.start) + '+' not in check_cdss:
            detect, ta = import_utr(tss, utr_strain, utr_all, start, end, tas, length, args_utr)
        ...
        if str(near_cds.end) + '-' not in check_cdss or [num == num_tss]:
            ...
            if pres['tss'] is not None:
                detect, ta = import_utr(pres['tss'], utr_strain, utr_all, pres['start'], pres['end'], tas, pres['len'], args_utr)
            pres['start'] = start
            pres['end'] = end
            pres['len'] = length
            pres['tss'] = tss
    if detect:
        ...
        if length >= 0:
            get_print_string_5utr(num_utr, name_utr, length, tss, cds_name, locus_tag, ta, args_utr.source, out, start, end, utrs_tss)
            num_utr += 1
    return num_utr
...
def detect_cds(cdss, gene):
    ...
    detect = False
    for cds in cdss:
        if 'Parent' in cds.attributes.keys() and gene.attributes['ID'] in cds.attributes['Parent'].split(','):
            detect = True
            near_cds = cds
            check_utr = True
        ...
        if not detect:
            if 'locus_tag' in cds.attributes.keys():
                if gene.attributes['locus_tag'] == cds.attributes['locus_tag']:
                    ...
                    check_utr = True
                    detect = True
            elif gene.seq_id == cds.seq_id and gene.strand == cds.strand and gene.start >= cds.start and gene.end <= cds.end:
                near_cds = cds
                cds_name = get_feature(cds)
                check_utr = True
                detect = True
...
def read_file(tss_file, gff_file, ta_file, term_file):
    ...
    if tss_file is not None:
        for entry in Gff3Parser().entries(open(tss_file)):
            if 'type' in entry.attributes.keys() and entry.attributes['type'] != 'Orphan':
                source = True
            tsss.append(entry)
    ...
    return (
     genes, cdss, terms, tsss, tas, source)
...
def check_associated_TSSpredator(genes, tss, cdss, check_utr, cds_name, locus):
    ...
    near_cds = None
    for gene in genes:
        if tss.seq_id == gene.seq_id and tss.strand == gene.strand and 'locus_tag' in gene.attributes.keys() and gene.attributes['locus_tag'] == locus:
            for cds in cdss:
                if 'Parent' in cds.attributes.keys() and gene.attributes['ID'] in cds.attributes['Parent'].split(','):
                    near_cds = cds
                    check_utr = True
...
def get_5utr_from_TSSpredator(tss, genes, cdss):
    ...
    check_utr = False
    cds_name = 'NA'
    if 'Primary' in tss.attributes['type'] or 'Secondary' in tss.attributes['type']:
        ass_gene = tss.attributes['associated_gene'].split(',')
        tss_type = tss.attributes['type'].split(',')
        for index in range(len(tss_type)):
            if tss_type[index] == 'Primary' or tss_type[index] == 'Secondary':
                locus_tag = ass_gene[index]
                check_utr, cds_name, near_cds = check_associated_TSSpredator(genes, tss, cdss, check_utr, cds_name, locus_tag)
                if not check_utr:
                    for cds in cdss:
                        if cds.seq_id == tss.seq_id and cds.strand == tss.strand:
                            strand = Helper().get_strand_name(cds.strand)
                            if locus_tag == cds.feature + ':' + str(cds.start) + '-' + str(cds.end) + '_' + strand:
                                near_cds = cds
                                cds_name = get_feature(cds)
                                check_utr = True
...
def detect_feature_5utr(feas, tss, cdss, length, check_cds):
    ...
    check_utr = False
    for fea in feas:
        if tss.seq_id == fea.seq_id and tss.strand == fea.strand:
            if tss.strand == '+' and fea.start - tss.start <= length and tss.start <= fea.start:
                ...
                if check_cds:
                    ...
                    check_utr = True
                break
            elif tss.strand == '-' and tss.start - fea.end <= length and tss.start >= fea.end:
                if 'locus_tag' in fea.attributes.keys():
                    locus_tag = fea.attributes['locus_tag']
                else:
                    locus_tag = fea.attributes['ID']
    ...
    return (
     near_cds, cds_name, check_utr, locus_tag)
...
def get_5utr_from_other(tss, genes, cdss, length):
    ...
    cds_name = 'NA'
    if len(genes) != 0:
        near_cds, cds_name, check_utr, locus_tag = detect_feature_5utr(genes, tss, cdss, length, True)
    else:
        near_cds, cds_name, check_utr, locus_tag = detect_feature_5utr(cdss, tss, cdss, length, False)
    if check_utr:
        utr_datas = {'check': check_utr, 'cds_name': cds_name, 
         'near_cds': near_cds, 'locus': locus_tag}
    else:
...
def get_attribute_string(num_utr, length, cds, gene_name, ta, id_name, name, feature, feature_name):
    name_utr = '%0*d' % (5, num_utr)
    cds_name = get_feature(cds)
    attribute_string = ';'.join(['='.join(items) for items in [
     (
    ...
    if feature == 'Parent':
        if 'ID' in ta.attributes.keys():
            ...
            attribute_string = ';'.join([
             attribute_string,
             'Parent=' + (feature,
              feature_name + str(ta.start) + '-' + str(ta.end) + '_' + ta.strand)])
    elif feature == 'associated_term':
        attribute_string = ';'.join([
         attribute_string,
         'associated_term=' + (feature_name + str(ta.start) + '-' + str(ta.end) + '_' + ta.strand)])
...
def get_gene_name(genes, cds):
    gene_name = 'NA'
    for gene in genes:
        if cds.seq_id == gene.seq_id and cds.strand == gene.strand:
            if 'Parent' in cds.attributes.keys() and 'ID' in gene.attributes.keys() and gene.attributes['ID'] in cds.attributes['Parent'].split(','):
                if 'locus_tag' in gene.attributes.keys():
                    gene_name = gene.attributes['locus_tag']
                else:
                    gene_name = 'Gene:' + str(gene.start) + '-' + str(gene.end) + '_' + gene.strand
                break
...
def check_repeat(start, end, strain, strand, utrs_tss):
    for utr in utrs_tss:
        if utr['strain'] == strain and utr['strand'] == strand and (utr['start'] >= start and utr['end'] <= end or utr['start'] <= start and utr['end'] >= end or utr['start'] >= start and utr['start'] <= end and utr['end'] >= end or utr['start'] <= start and utr['end'] >= start and utr['end'] <= end):
            return True

    return False


def compare_ta(tas, genes, cdss, utr_strain, utr_all, out, args_utr, utrs_tss, num_utr):
    ...
    for ta in tas:
        ...
        for cds in cdss:
            if ta.seq_id == cds.seq_id and ta.strand == cds.strand:
                if ta.strand == '+':
                    if cds.start - ta.start <= args_utr.length and cds.start - ta.start >= 0 and (ta.start <= cds.start and ta.end >= cds.end or ta.start <= cds.start and ta.end <= cds.end and ta.end >= cds.start):
                        if not check_repeat(ta.start, cds.start, ta.seq_id, ta.strand, utrs_tss):
                            length = cds.start - ta.start
                            utr_strain['all'][ta.seq_id].append(length)
                            utr_all['all'].append(length)
                            gene_name = get_gene_name(genes, cds)
                            string = get_attribute_string(num_utr, length, cds, gene_name, ta, 'utr5', "5'UTR", 'Parent', 'Transcript:')
                            detect = True
                            start = ta.start
                            end = cds.start - 1
                            break
            elif ta.end - cds.end <= args_utr.length and ta.end - cds.end >= 0 and (ta.start <= cds.start and ta.end >= cds.end or ta.start >= cds.start and ta.start <= cds.end and ta.end >= cds.end):
                if not check_repeat(cds.end, ta.end, ta.seq_id, ta.strand, utrs_tss):
                    near_cds = cds
                    detect = True
        ...
        if ta.strand == '-' and detect:
            ...
            string = get_attribute_string(num_utr, length, near_cds, gene_name, ta, 'utr5', "5'UTR", 'Parent', 'Transcript:')
            start = near_cds.end + 1
            end = ta.end
        if detect and end - start > 0:
            string = ';'.join([string, 'associated_tss=NA', 'tss_type=NA'])
            out.write('{0}\tANNOgesic\t5UTR\t{1}\t{2}\t.\t{3}\t.\t{4}\n'.format(ta.seq_id, start, end, ta.strand, string))
            num_utr += 1
...
def detect_5utr(tss_file, gff_file, ta_file, out_file, args_utr):
    ...
    genes, cdss, terms, tsss, tas, source = read_file(tss_file, gff_file, ta_file, None)
    utrs_tss = []
    check_cdss = []
    pres = {'check': None, 'tss': None, 'start': -1, 'end': -1, 'len': -1}
    if args_utr.source and not source:
        args_utr.source = False
    if args_utr.base_5utr.upper() == 'TSS' or args_utr.base_5utr.lower() == 'both':
        num = 0
        for tss in tsss:
            num = num + 1
            if args_utr.source:
                ...
                utr_datas = get_5utr_from_other(tss, genes, cdss, args_utr.length)
            if utr_datas['check']:
                ...
                num_utr = get_5utr(tss, utr_datas['near_cds'], utr_strain, utr_all, tas, num_utr, num, len(tsss), utr_datas['cds_name'], utr_datas['locus'], out, args_utr, utrs_tss, check_cdss, pres)

    if args_utr.base_5utr.lower() == 'transcript' or args_utr.base_5utr.lower() == 'both':
        num_utr = compare_ta(tas, genes, cdss, utr_strain, utr_all, out, args_utr, utrs_tss, num_utr)
    out.close()
    ...
    if len(utr_strain['all']) > 1:
        for strain in utr_strain['all'].keys():
            plot(utr_strain['all'][strain], utr_strain['pri'][strain], utr_strain['sec'][strain], '_'.join([strain, '5utr_length.png']), args_utr.source, '5utr', args_utr.base_5utr)
...
def compare_term(ta, terms, fuzzy):
    ...
    for term in terms:
        if ta.strand == term.strand:
            if term.strand == '+':
                if math.fabs(ta.end - term.start) <= fuzzy or ta.end >= term.start and ta.end <= term.end:
                    return term
            elif math.fabs(ta.start - term.end) <= fuzzy or ta.start >= term.start and ta.start <= term.end:
                return term


def get_3utr(ta, near_cds, utr_all, utr_strain, attributes, num_utr, out, args_utr, utrs_ta):
    """print the 3UTR"""
    if ta.strand == '+':
        start = near_cds.end + 1
        end = ta.end
...
def get_gene_string(gene, attributes):
    if 'locus_tag' in gene.attributes.keys():
        ...
        gene_string = gene.feature + ':' + str(gene.start) + '-' + str(gene.end) + '_' + gene.strand
        attributes.append('='.join(['associated_gene',
         gene_string]))
...
def get_near_cds(cdss, genes, ta, attributes, utr_length):
    ...
    near_cds = None
    for cds in cdss:
        if ta.seq_id == cds.seq_id and ta.strand == cds.strand and cds.feature == 'CDS':
            if ta.strand == '+':
                if cds.end <= ta.end and ta.end - cds.end <= utr_length and ta.end - cds.end > 0:
                    if near_cds == None:
                        near_cds = cds
            else:
                if cds.end > near_cds.end:
                    near_cds = cds
                detect = True
        elif cds.start >= ta.start and cds.start - ta.start <= utr_length and cds.start - ta.start > 0:
            if near_cds == None:
                near_cds = cds
            else:
    ...
    if detect:
        ...
        if 'protein_id' in near_cds.attributes.keys():
            ...
            if 'locus_tag' in near_cds.attributes.keys():
                attributes.append('='.join(['associated_cds',
                 near_cds.attributes['locus_tag']]))
            else:
                cds_string = near_cds.feature + ':' + str(near_cds.start) + '-' + str(near_cds.end) + '_' + near_cds.strand
...
def compare_term_3utr(terms, cdss, genes, utr_all, utr_strain, args_utr, out, utrs_ta, num_utr):
    ...
    for term in terms:
        ...
        for cds in cdss:
            if cds.seq_id == term.seq_id:
                if cds.strand == term.strand and term.strand == '+' and term.end >= cds.end and (term.end - cds.end <= args_utr.length or term.start - cds.end <= args_utr.length and term.start >= cds.end):
                    if not check_repeat(cds.end, term.end, cds.seq_id, cds.strand, utrs_ta):
                        detect = True
                        near_cds = cds
                elif term.start <= cds.start and (cds.start - term.start <= args_utr.length or cds.start - term.end <= args_utr.length and cds.start >= term.end):
                    if not check_repeat(term.start, cds.start, cds.seq_id, cds.strand, utrs_ta):
                        length = term.start - cds.start
                        utr_strain[term.seq_id].append(length)
                        utr_all.append(length)
                        gene_name = get_gene_name(genes, cds)
                        string = get_attribute_string(num_utr, length, cds, gene_name, term, 'utr3', "3'UTR", 'associated_term', 'Terminator:')
                        detect = True
                        start = term.start
                        end = cds.start - 1
                        break
                    if term.strand == '+' and detect:
                        ...
                        utr_all.append(length)
                        gene_name = get_gene_name(genes, near_cds)
                        string = get_attribute_string(num_utr, length, near_cds, gene_name, term, 'utr3', "3'UTR", 'associated_term', 'Terminator:')
                        detect = True
                        start = near_cds.end + 1
                        end = term.end

        if detect and end - start > 0:
            out.write('{0}\tANNOgesic\t3UTR\t{1}\t{2}\t.\t{3}\t.\t{4}\n'.format(term.seq_id, start, end, term.strand, string))
            num_utr += 1
...
def detect_3utr(ta_file, gff_file, term_file, out_file, args_utr):
    ...
    out = open(out_file, 'w')
    out.write('##gff-version 3\n')
    genes, cdss, terms, tsss, tas, args_utr.source = read_file(None, gff_file, ta_file, term_file)
    utrs_ta = []
    if args_utr.base_3utr == 'transcript' or args_utr.base_3utr == 'both':
        for ta in tas:
            if term_file is not None:
            ...
            if term_file is not None:
                if term is not None:
                    attributes.append('='.join(['associated_term',
                     'Terminator:' + str(term.start) + '-' + str(term.end) + '_' + term.strand]))
                else:
                ...
                if near_cds is not None:
                    num_utr = get_3utr(ta, near_cds, utr_all, utr_strain, attributes, num_utr, out, args_utr, utrs_ta)

    if args_utr.base_3utr == 'terminator' or args_utr.base_3utr == 'both':
        num_utr = compare_term_3utr(terms, cdss, genes, utr_all, utr_strain, args_utr, out, utrs_ta, num_utr)
    out.close()
    Helper().sort_gff(out_file, out_file + 'sort')
    shutil.move(out_file + 'sort', out_file)
    name = gff_file.split('/')[(-1)].replace('.gff', '')
    plot(utr_all, None, None, '_'.join([name, 'all_3utr_length.png']), None, '3utr', '3utr')
    if len(utr_strain) > 1:
        for strain in utr_strain.keys():
            plot(utr_strain[strain], None, None, '_'.join([strain, '3utr_length.png']), None, '3utr', '3utr')