...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/detect_operon.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 14747 bytes
import math
...
def get_term_feature(ta, data, term_fuzzy, features, datas, ta_check_point, data_check_start, data_check_end):
    """verify and get proper terminator to operon"""
    jump = False
    if ta.strand == data.strand and ta.seq_id == data.seq_id and (math.fabs(data.start - ta_check_point) <= term_fuzzy or math.fabs(data.end - ta_check_point) <= term_fuzzy or ta_check_point >= data.start and ta_check_point <= data.end):
        features['detect'] = True
    if ta.strand == data.strand and ta.seq_id == data.seq_id:
        if ta.start <= data_check_start and ta.end >= data_check_end:
            features['num'] += 1
            datas.append(data)
    elif ta_check_point >= data.start and ta_check_point <= data.end:
        features['num'] += 1
        datas.append(data)
    if ta.seq_id == data.seq_id and data.start - term_fuzzy > ta.end:
        jump = True
    return jump


def get_tss_feature(ta, data, features, tss_fuzzy, datas, ta_check_point, data_check_start, data_check_end):
    """verify and get the proper TSS for operon"""
    jump = False
    if ta.strand == data.strand and ta.seq_id == data.seq_id and math.fabs(ta_check_point - data.start) <= tss_fuzzy:
        features['detect'] = True
    if ta.strand == data.strand and ta.seq_id == data.seq_id and ta.start <= data_check_start and ta.end >= data_check_end:
        features['num'] += 1
        datas.append(data)
    if ta.seq_id == data.seq_id and data_check_end > ta.end:
        jump = True
    return jump
...
def detect_features(ta, inputs, feature, term_fuzzy, tss_fuzzy):
    ...
    for data in inputs:
        if feature == 'term':
            if ta.strand == '+':
                jump_term = get_term_feature(ta, data, term_fuzzy, features, datas, ta.end, data.start, data.start - term_fuzzy)
            else:
                if ta.strand == '-':
                    jump_term = get_term_feature(ta, data, term_fuzzy, features, datas, ta.start, data.end + term_fuzzy, data.end)
                if jump_term:
                    break
        else:
            if feature == 'tss':
                if ta.strand == '+':
                    jump_tss = get_tss_feature(ta, data, features, tss_fuzzy, datas, ta.start, data.start + tss_fuzzy, data.end)
                else:
                    if ta.strand == '-':
                        jump_tss = get_tss_feature(ta, data, features, tss_fuzzy, datas, ta.end, data.end, data.start - tss_fuzzy)
                    if jump_tss:
                        break
            elif feature == 'gene' and ta.strand == data.strand and ta.seq_id == data.seq_id and data.feature == 'gene' and (ta.start <= data.start and ta.end >= data.end or ta.start >= data.start and ta.end <= data.end or ta.start >= data.start and ta.start <= data.end and ta.end >= data.end or ta.start <= data.start and ta.end <= data.end and ta.end >= data.start):
                features['num'] += 1
                features['detect'] = True
                datas.append(data)
...
def check_gene(tsss, genes, strand, ta_pos, first, min_length, end, operons, operon_pos):
    ...
    for tss in tsss:
        if tss not in no_count_tsss:
            end_points = [
             ta_pos]
            for pos in tsss:
                if pos not in no_count_tsss and tss.start != pos.start:
            ...
            for point in end_points:
                ...
                if tss.strand == '+':
                    for gene in genes['data_list']:
                        if gene.seq_id == tss.seq_id and gene.strand == tss.strand and gene.start >= tss.start and gene.end <= point:
                            detect_pos = True
                            break

                else:
                    for gene in genes['data_list']:
                        if gene.seq_id == tss.seq_id and gene.strand == tss.strand and gene.start >= point and gene.end <= tss.start:
                            detect_pos = True
                            break
                ...
                if not detect_pos:
                    ...
                    operon_pos, first = compute_sub_operon(strand, point, ta_pos, first, min_length, end, operons, operon_pos)
                    break


def sub_operon_gene_conflict(tsss, strand, genes, ta_pos, first, min_length, end, operons, operon_pos):
    ...
    check_gene(new_tsss, genes, strand, ta_pos, first, min_length, end, operons, operon_pos)
...
def sub_operon(strand, tsss, ta_pos, end, genes, min_length):
    ...
    if tsss['with_feature']:
        if tsss['num_feature'] == 1:
            ...
            sub_operon_gene_conflict(tsss, strand, genes, ta_pos, first, min_length, end, operons, operon_pos)
    else:
        sub_operon_gene_conflict(tsss, strand, genes, ta_pos, first, min_length, end, operons, operon_pos)
    return operons


def compute_sub_operon(strand, point, ta_pos, first, min_length, end, operons, operon_pos):
    ...
    if math.fabs(point - operon_pos) > min_length:
        if strand == '+':
            operons.append(import_to_operon(operon_pos, point - 1, strand))
            operon_pos = point
    else:
        operons.append(import_to_operon(point + 1, operon_pos, strand))
        operon_pos = point
...
def print_file(ta, operons, out, operon_id, whole_operon, tsss, terms, genes, whole_gene, out_g):
    ...
    out_g.write('{0}\tANNOgesic\toperon\t{1}\t{2}\t.\t{3}\t.\t{4}\n'.format(ta.seq_id, str(whole_operon.start), str(whole_operon.end), whole_operon.strand, attribute_string))
    if len(operons) <= 1:
        ...
        for sub in operons:
            ...
            num_sub_gene = 0
            for gene in genes['data_list']:
                if sub['strand'] == gene.strand and sub['start'] <= gene.start and sub['end'] >= gene.end:
...
def operon(ta_file, tss_file, gff_file, terminator_file, tss_fuzzy, term_fuzzy, min_length, out_file, out_gff):
    ...
    num_operon = 0
    tas, gffs, tss_gffs, term_gffs = read_gff(ta_file, gff_file, tss_file, terminator_file)
    for ta in tas:
        whole_gene = []
        check_operon = False
        ...
        if len(tss_gffs) != 0:
            if ta.strand == '+':
                operons = sub_operon(ta.strand, tsss, ta.start, ta.end, genes, min_length)
            else:
                operons = sub_operon(ta.strand, tsss, ta.end, ta.start, genes, min_length)
        else:
            operons = [
             {'start': ta.start, 'end': ta.end, 'strand': ta.strand}]
        ...
        if check_operon and whole_gene != ['NA']:
            operon_id = 'Operon' + str(num_operon)
            num_operon += 1
            if len(tss_gffs) != 0:
                print_file(ta, operons, out, operon_id, whole_operon, tsss, terms, genes, whole_gene, out_g)
            else:
                print_file(ta, operons, out, operon_id, ta, tsss, terms, genes, whole_gene, out_g)
