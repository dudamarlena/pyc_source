...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/sORF_detection.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 46472 bytes
        ...
        if wig_strain == sorf['strain']:
            for cond, tracks in conds.items():
                ...
                for lib_name, covers in tracks.items():
                    ...
                    total_cover = 0
                    first = True
                    c_start, c_end = check_start_and_end(sorf['start'], sorf['end'], covers)
                    ...
                    for cover in covers:
                        if lib_strand == strand:
                            if strand == '+':
                                ...
                                cover_pos = c_end - pos
                            if sorf['start'] <= cover_pos and sorf['end'] >= cover_pos:
                    ...
                    if medianlist is not None:
                        cutoff_cover = get_cutoff(sorf, track, coverages, medianlist, min_cutoff)
                    else:
        ...
        for nts in range(0, start):
            ...
            for index in range(len(ribo_seq)):
                if miss > args_sorf.fuzzy_rbs:
                    break
                elif fasta[nts:nts + len(ribo_seq)][index] != ribo_seq[index]:
                    miss += 1

            if miss <= args_sorf.fuzzy_rbs and len(fasta[nts:nts + len(ribo_seq)]) >= len(ribo_seq):
                get = True
                if miss <= pre_miss:
                    if miss < pre_miss:
                        detect = []
...
def check_terminal_seq(seq, start, end, args_sorf, source, inter, sorfs, rbs):
    ...
    for i in [0, 1, -1, 2, -2]:
        ...
        if fasta[:3] in args_sorf.start_codon and fasta[-3:] in args_sorf.stop_codon:
            detect = i
...
def detect_start_stop(inters, seq, args_sorf):
    ...
    for inter in inters:
        ...
        fasta = Helper().extract_gene(seq[inter.seq_id], inter.start, inter.end, inter.strand)
        starts = []
        stops = []
        ...
        for start in starts:
            get_stop = False
            for stop in stops:
                if stop - start > 0 and (stop - start) % 3 == 0:
                    if not args_sorf.multi_stop and get_stop:
                        break
                else:
                    get_stop = True
                if stop - start <= args_sorf.max_len and stop - start >= args_sorf.min_len:
                    rbs = detect_rbs_site(fasta, start, inter, args_sorf)
                    if len(rbs) == 1 and rbs[0] == 'NA':
                        pass
                    else:
                        if inter.source == 'intergenic' or inter.source == 'antisense':
                            if inter.strand == '+':
                                check_terminal_seq(seq[inter.seq_id], inter.start + start, inter.start + stop + 2, args_sorf, inter.source, inter, sorfs, rbs)
                            else:
                                check_terminal_seq(seq[inter.seq_id], inter.start + (len(fasta) - stop - 3), inter.start + (len(fasta) - start - 1), args_sorf, inter.source, inter, sorfs, rbs)
                        elif inter.source == 'UTR_derived':
                            if inter.strand == '+':
                                check_terminal_seq(seq[inter.seq_id], inter.start + start, inter.start + stop + 2, args_sorf, inter.attributes['UTR_type'], inter, sorfs, rbs)
                            else:
                                check_terminal_seq(seq[inter.seq_id], inter.start + (len(fasta) - stop - 3), inter.start + (len(fasta) - start - 1), args_sorf, inter.attributes['UTR_type'], inter, sorfs, rbs)
...
def read_data(inter_gff, tss_file, srna_gff, fasta, utr_detect):
    ...
    for entry in Gff3Parser().entries(fh):
        if entry.source == 'UTR_derived' and utr_detect or entry.source == 'intergenic' or entry.source == 'antisense':
            inters.append(entry)
    ...
    return (
     inters, tsss, srnas, seq)


def check_tss(sorf, tss, utr_fuzzy, checks):
    if sorf['start'] - tss.start <= utr_fuzzy and sorf['start'] - tss.start >= 0 and sorf['strand'] == '+' or tss.start - sorf['end'] <= utr_fuzzy and tss.start - sorf['end'] >= 0 and sorf['strand'] == '-':
...
def compare_sorf_tss(sorfs, tsss, tss_file, args_sorf):
    ...
    if tss_file is not None:
        for sorf in sorfs:
            ...
            for tss in tsss:
                checks['import'] = False
                if sorf['strain'] == tss.seq_id and sorf['strand'] == tss.strand:
                    if sorf['strand'] == '+':
                        check_tss(sorf, tss, args_sorf.utr_length, checks)
                    else:
                        check_tss(sorf, tss, args_sorf.utr_length, checks)
                    if not checks['import']:
                        if tss.start <= sorf['start'] and tss.start >= sorf['end']:
            ...
            if len(sorf['with_TSS']) == 0:
                sorf['with_TSS'] = [
                 'NA']
            if checks['rbs'] and not args_sorf.noafter_tss and not args_sorf.no_tss:
                sorf['rbs'] = checks['rbs']
                sorfs_best.append(copy.deepcopy(sorf))
            else:
                if sorf['rbs'][0] != 'NA' and args_sorf.noafter_tss and not args_sorf.no_tss and checks['start']:
                    sorfs_best.append(copy.deepcopy(sorf))
                elif sorf['rbs'][0] != 'NA' and args_sorf.noafter_tss and args_sorf.no_tss:
                    sorfs_best.append(copy.deepcopy(sorf))
...
def compare_sorf_srna(sorfs, srnas, srna_gff):
    if srna_gff is not None:
        for sorf in sorfs:
            sorf['srna'] = []
            for srna in srnas:
                if sorf['strain'] == srna.seq_id and sorf['strand'] == srna.strand and (srna.start <= sorf['start'] and srna.end >= sorf['end'] or srna.start >= sorf['start'] and srna.end <= sorf['end'] or srna.start <= sorf['start'] and srna.end >= sorf['start'] and srna.end <= sorf['end'] or srna.start >= sorf['start'] and srna.start <= sorf['end'] and srna.end >= sorf['end']):
                    sorf['srna'].append('sRNA:' + str(srna.start) + '-' + str(srna.end) + '_' + srna.strand)
...
def merge(sorfs, seq):
    ...
    for sorf1 in sorfs:
        ...
        if not sorf1['print']:
            ...
            for sorf2 in sorfs:
                overlap = False
                if final['strain'] == sorf2['strain'] and final['strand'] == sorf2['strand']:
                    if final['start'] >= sorf2['start'] and final['end'] <= sorf2['end']:
                        overlap = True
                    else:
                        if final['start'] >= sorf2['start'] and final['start'] <= sorf2['end'] and final['end'] >= sorf2['end']:
                            overlap = True
                        else:
                            if final['start'] <= sorf2['start'] and final['end'] >= sorf2['start'] and final['end'] <= sorf2['end']:
                                overlap = True
                            else:
                                if final['start'] <= sorf2['start'] and final['end'] >= sorf2['end']:
                                    overlap = True
                            ...
                            if overlap:
                                sorf2['print'] = True
                                first, final = import_overlap(sorf2, final, sorf1, first)

            final['seq'] = Helper().extract_gene(seq[final['strain']], final['start'], final['end'], final['strand'])
            new = {}
...
def assign_utr_cutoff(coverages, utr_type, medians, track, min_cutoff):
    if track in medians.keys():
        ...
        if coverages[utr_type] != 'median' and coverages[utr_type] != 'mean':
            cutoff = float(coverages[utr_type])
        else:
            cutoff = min_cutoff
    return cutoff
...
def get_cutoff(sorf, track, coverages, medians, min_cutoff):
    if sorf['type'] == 'intergenic':
        ...
        if sorf['type'] == 'antisense':
            cutoff_cover = float(coverages['anti'])
        else:
            if '5utr' in sorf['type'] and '3utr' in sorf['type']:
                cutoff_utr3 = assign_utr_cutoff(coverages, '3utr', medians[sorf['strain']]['3utr'], track, min_cutoff)
                cutoff_utr5 = assign_utr_cutoff(coverages, '5utr', medians[sorf['strain']]['5utr'], track, min_cutoff)
                cutoff_cover = min(cutoff_utr5, cutoff_utr3)
            else:
                if '5utr' in sorf['type']:
                    cutoff_cover = assign_utr_cutoff(coverages, '5utr', medians[sorf['strain']]['5utr'], track, min_cutoff)
                else:
                    if '3utr' in sorf['type']:
                        cutoff_cover = assign_utr_cutoff(coverages, '3utr', medians[sorf['strain']]['3utr'], track, min_cutoff)
                    elif 'interCDS' in sorf['type']:
                        cutoff_cover = assign_utr_cutoff(coverages, 'interCDS', medians[sorf['strain']]['interCDS'], track, min_cutoff)
    return cutoff_cover
...
def check_start_and_tss_point(sorf):
    ...
    for tss in sorf['with_TSS']:
        if tss != 'NA':
            if int(tss.replace('TSS:', '')[:-2]) >= int(sorf['start']) and int(tss.replace('TSS:', '')[:-2]) <= int(sorf['end']):
                tsss.append(tss)
            else:
                tsss.append(tss)
            ...
            if rbs != 'NA':
                if sorf['strand'] == '+':
                    for start in sorf['starts']:
                        if int(start) - int(rbs) >= min_rbs + 6 and int(start) - int(rbs) <= max_rbs + 6 and rbs not in new_rbss:
                            new_rbss.append(rbs)
                            detect = True

                else:
                    for end in sorf['ends']:
                        if int(rbs) - int(end) >= min_rbs + 6 and int(rbs) - int(end) <= max_rbs + 6 and rbs not in new_rbss:
                            new_rbss.append(rbs)
                            detect = True
    ...
    for start in sorf['starts']:
        for end in sorf['ends']:
            if (int(end) - int(start) + 1) % 3 == 0:
                for rbs in sorf['rbs']:
                    if sorf['strand'] == '+' and int(start) - rbs >= min_rbs + 6 and int(start) - rbs <= max_rbs + 6:
                        if sorf['with_TSS'] == ['NA']:
                            new_candidates.append('_'.join([
                             '-'.join([
                              start, end]), 'NA',
                             'RBS:' + str(rbs)]))
                        else:
                            ...
                            new_candidates.append('_'.join([
                             ...
                             'RBS:' + str(rbs)]))
                    elif sorf['strand'] == '-' and rbs - int(end) >= min_rbs + 6 and rbs - int(end) <= max_rbs + 6:
                        if sorf['with_TSS'] == ['NA']:
                            new_candidates.append('_'.join([
                             '-'.join([
                              start, end]), 'NA',
...
def check_candidates_srnas(sorf, min_rbs, max_rbs):
    ...
    for cand in sorf['candidate']:
        ...
        if start in sorf['starts'] and end in sorf['ends'] and rbs in sorf['rbs']:
    ...
    if len(sorf['srna']) == 1 and sorf['srna'][0] == 'NA':
        ...
        for srna in sorf['srna']:
            if srna != 'NA':
                ...
                if strand == sorf['strand'] and (srna_start <= int(sorf['start']) and srna_end >= int(sorf['end']) or srna_start >= int(sorf['start']) and srna_end <= int(sorf['end']) or srna_start <= int(sorf['start']) and srna_end >= int(sorf['start']) and srna_end <= int(sorf['end']) or srna_start >= int(sorf['start']) and srna_start <= int(sorf['end']) and srna_end >= int(sorf['end'])):
...
def assign_sorf(sorf, starts, ends, fasta):
    ...
    sorf['seq'] = Helper().extract_gene(fasta[sorf['strain']], sorf['start'], sorf['end'], sorf['strand'])
        ...
        if sorf['strand'] == '+':
            ...
            for start in sorf['starts']:
                if int(start) < min(sorf['rbs']):
                    ...
                    for rbs in sorf['rbs']:
                        if int(start) - int(rbs) >= args_sorf.min_rbs + 6 and int(start) - int(rbs) <= args_sorf.max_rbs + 6 and start not in starts:
                            starts.append(start)

            ends = []
            for end in sorf['ends']:
                for start in starts:
                    if (int(end) - int(start) + 1) % 3 == 0 and int(end) - int(start) + 1 >= args_sorf.min_len and int(end) - int(start) + 1 <= args_sorf.max_len and end not in ends and end not in ends:
                        ends.append(end)
            ...
            for end in sorf['ends']:
                if int(end) > max(sorf['rbs']):
                    ...
                    for rbs in sorf['rbs']:
                        if int(rbs) - int(end) >= args_sorf.min_rbs + 6 and int(rbs) - int(end) <= args_sorf.max_rbs + 6 and end not in ends:
                            ends.append(end)

            starts = []
            for start in sorf['starts']:
                for end in ends:
                    if (int(end) - int(start) + 1) % 3 == 0 and int(end) - int(start) + 1 >= args_sorf.min_len and int(end) - int(start) + 1 <= args_sorf.max_len and start not in starts and start not in starts:
                        starts.append(start)
...
def print_file(sorf, sorf_datas, num, out_g, out_t, file_type, args_sorf):
    ...
    if sorf['type'] == 'intergenic' or sorf['type'] == 'antisense':
        ...
        attribute_string = get_attribute(num, name, sorf['start_TSS'], sorf, type_.lower())
    else:
        if '3utr' in sorf['type'] and '5utr' in sorf['type']:
            ...
            if '3utr' in sorf['type']:
                ...
                info = '\t'.join([str(field) for field in [
                 ...
                 '.', attribute_string]])
                out_g.write(info + '\n')
                if 'frag' in ';'.join(sorf_datas['conds'].keys()) and 'tex' in ';'.join(sorf_datas['conds'].keys()):
                    lib_type = 'TEX+/-;Fragmented'
                else:
                    if 'frag' in ';'.join(sorf_datas['conds'].keys()):
                        lib_type = 'Fragmented'
                    elif 'tex' in ';'.join(sorf_datas['conds'].keys()):
                        lib_type = 'TEX+/-'
    print_table(out_t, sorf, name, type_, lib_type, sorf_datas, args_sorf)
...
def print_table(out_t, sorf, name, type_, lib_type, sorf_datas, args_sorf):
    ...
    for data in sorf_datas['detail']:
        if first:
            out_t.write('{0}({1})'.format(data['track'], data['avg']))
            first = False
        else:
            out_t.write(';{0}({1})'.format(data['track'], data['avg']))
...
def detect_utr_type(inter, utr_type, med_inters, wigs, strand, background):
    ...
    if inter.attributes['UTR_type'] == utr_type:
        ...
        inter_datas = get_coverage(inter_datas, wigs, strand, background, None, None, background)
...
def validate_tss(starts, ends, sorf, utr_fuzzy):
    ...
    if sorf['with_TSS'][0] != 'NA':
        for tss in sorf['with_TSS']:
            ...
            if sorf['strand'] == '+':
                if tss_start >= min(starts) - utr_fuzzy and tss_start <= max(ends):
                    tsss.append(tss)
                    if tss_start >= min(starts) - utr_fuzzy and tss_start <= min(starts):
                        start_pos = tss
            elif tss_start >= min(starts) and tss_start <= max(ends) + utr_fuzzy:
                tsss.append(tss)
                if tss_start <= min(ends) + utr_fuzzy and tss_start >= min(ends):
                    start_pos = tss
                    break
    ...
    for srna in sorf['srna']:
        if srna == 'NA':
            ...
            for index in range(0, len(starts)):
                if start <= starts[index] and end >= ends[index] or start >= starts[index] and end <= ends[index] or start >= starts[index] and start <= ends[index] and end >= ends[index] or start <= starts[index] and end >= starts[index] and end <= ends[index]:
                    srnas.append(srna)
                    break
            ...
            if sorf['with_TSS'][0] != 'NA' or args_sorf.no_tss:
                ...
                for candidate in sorf['candidate']:
                    ...
                    if tss != 'NA' or args_sorf.no_tss:
                        ...
                        cands.append('-'.join([str(datas[0]), str(datas[1])]) + '_TSS:' + tss + '_RBS:' + rbs)
                ...
                tsss_datas = validate_tss(starts, ends, sorf, args_sorf.utr_length)
...
def coverage_and_output(sorfs, mediandict, wigs, out_g, out_t, file_type, fasta, coverages, args_sorf, texs, run):
    ...
    if run == 'final':
        ...
        final_sorfs = []
        for sorf in sorfs:
            if compare_rbs_start(sorf, args_sorf.min_rbs, args_sorf.max_rbs) and file_type == 'best' or file_type == 'all':
                if file_type == 'best':
                    check_start_end(sorf, args_sorf, fasta, run)
                detect_frame_shift(sorf)
                cutoffs = {}
                if sorf['strand'] == '+':
                    sorf_covers = get_coverage(sorf, wigs['forward'], '+', coverages, mediandict, cutoffs, args_sorf.background)
            else:
                sorf_covers = get_coverage(sorf, wigs['reverse'], '-', coverages, mediandict, cutoffs, args_sorf.background)
            if len(sorf_covers) != 0:
                sorf_info = replicate_comparison(args_sorf, sorf_covers, sorf['strand'], 'sORF', None, cutoffs, None, cutoffs, None, texs)
                if len(sorf_info['conds'].keys()) != 0:
                    if run != 'final':
                        final_sorfs.append(sorf)
                    else:
                        print_file(sorf, sorf_info, num, out_g, out_t, file_type, args_sorf)
                        num += 1
...
def detect_inter_type(inters, wigs, background):
    ...
    for inter in inters:
        ...
        if inter.source == 'UTR_derived' and inter.strand == '+':
            detect_utr_type(inter, '5utr', med_inters, wigs['forward'], '+', background)
            detect_utr_type(inter, '3utr', med_inters, wigs['forward'], '+', background)
            detect_utr_type(inter, 'interCDS', med_inters, wigs['forward'], '+', background)
        elif inter.source == 'UTR_derived' and inter.strand == '-':
            detect_utr_type(inter, '5utr', med_inters, wigs['reverse'], '-', background)
            detect_utr_type(inter, '3utr', med_inters, wigs['reverse'], '-', background)
            detect_utr_type(inter, 'interCDS', med_inters, wigs['reverse'], '-', background)
    ...
    for strain, utrs in covers.items():
        ...
        for utr, tracks in utrs.items():
            for track, avgs in tracks.items():
                ...
                mediandict[strain][utr][track] = {'median': median_score(avgs, coverages[utr])}

    for utr, value in coverages.items():
        if type(value) is str and 'p_' in value:
...
def set_coverage(args_sorf):
    ...
    if 'n_' in args_sorf.cutoff_3utr:
        args_sorf.cutoff_3utr = float(args_sorf.cutoff_3utr.split('_')[(-1)])
    if 'n_' in args_sorf.cutoff_5utr:
        args_sorf.cutoff_5utr = float(args_sorf.cutoff_5utr.split('_')[(-1)])
    if 'n_' in args_sorf.cutoff_intercds:
        args_sorf.cutoff_intercds = float(args_sorf.cutoff_intercds.split('_')[(-1)])
...
def sorf_detection(fasta, srna_gff, inter_gff, tss_file, wig_f_file, wig_r_file, out_prefix, args_sorf):
    coverages = set_coverage(args_sorf)
    libs, texs = read_libs(args_sorf.libs, args_sorf.merge_wigs)
    inters, tsss, srnas, seq = read_data(inter_gff, tss_file, srna_gff, fasta, args_sorf.utr_detect)
    ...
    sorfs_best = sorted(sorfs_best, key=lambda k: (k['strain'], k['start'],
     k['end'], k['strand']))
    final_all = coverage_and_output(sorfs_all, mediandict, wigs, out_ag, out_at, 'all', seq, coverages, args_sorf, texs, 'first')
    final_best = coverage_and_output(sorfs_best, mediandict, wigs, out_bg, out_bt, 'best', seq, coverages, args_sorf, texs, 'first')
    final_all = merge(final_all, seq)
    final_best = merge(final_best, seq)
    final_best = get_best(final_best, tss_file, srna_gff, args_sorf)
    coverage_and_output(final_all, mediandict, wigs, out_ag, out_at, 'all', seq, coverages, args_sorf, texs, 'final')
    coverage_and_output(final_best, mediandict, wigs, out_bg, out_bt, 'best', seq, coverages, args_sorf, texs, 'final')
    out_ag.close()
    out_at.close()
    out_bg.close()
    out_bt.close()