...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/optimize_TSSpredator.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 44337 bytes
import os, shutil, sys, random, csv
...
def compute_stat(stat_value, best, best_para, cores, list_num, out_path, indexs, strain):
    if indexs['change']:
        indexs['change'] = False
        best = stat_value
        ...
        best_out.write('{0}\t{1}\tTP={2}\tTP_rate={3}\tFP={4}\tFP_rate={5}\tFN={6}\tMissing_ratio={7}\t'.format(indexs['step'] - cores + 1 + indexs['count'], para_line, best['tp'], best['tp_rate'], best['fp'], best['fp_rate'], best['fn'], best['missing_ratio']))
        best_out.close()
    print(', '.join(['Current genome={0}', 'Current Parameters:step={1}',
     ...
     'enrichment_factor={7}',
     'processing_factor={8}']).format(strain, indexs['step'] - cores + 1 + indexs['count'], list_num[(-1 * cores + indexs['count'])]['height'], list_num[(-1 * cores + indexs['count'])]['re_height'], list_num[(-1 * cores + indexs['count'])]['factor'], list_num[(-1 * cores + indexs['count'])]['re_factor'], list_num[(-1 * cores + indexs['count'])]['base_height'], list_num[(-1 * cores + indexs['count'])]['enrichment'], list_num[(-1 * cores + indexs['count'])]['processing']))
    print('Current:TP={0}\tTP_rate={1}\tFP={2}\tFP_rate={3}\tFN={4}\tMissing_ratio={5}'.format(stat_value['tp'], stat_value['tp_rate'], stat_value['fp'], stat_value['fp_rate'], stat_value['fn'], stat_value['missing_ratio']))
    print('\t'.join(['Best Parameters:height={0}', 'height_reduction={1}',
     'factor={2}', 'factor_reduction={3}', 'base_height={4}',
     'enrichment_factor={5}', 'processing_factor={6}']).format(best_para['height'], best_para['re_height'], best_para['factor'], best_para['re_factor'], best_para['base_height'], best_para['enrichment'], best_para['processing']))
    print('Best:TP={0}\tTP_rate={1}\tFP={2}\tFP_rate={3}\tFN={4}\tMissing_ratio={5}'.format(best['tp'], best['tp_rate'], best['fp'], best['fp_rate'], best['fn'], best['missing_ratio']))
    indexs['count'] += 1
    return (
     best_para, best)
...
def scoring_function(best, stat_value, indexs, num_manual):
    ...
    indexs['change'] = False
    if stat_value['tp_rate'] == best['tp_rate'] and stat_value['fp_rate'] == best['fp_rate']:
        pass
    else:
        if stat_value['tp_rate'] - best['tp_rate'] >= 0.1:
            indexs['change'] = True
        elif best['tp_rate'] - stat_value['tp_rate'] <= 0.1:
            if best['tp_rate'] <= stat_value['tp_rate'] and best['fp_rate'] >= stat_value['fp_rate']:
                indexs['change'] = True
            else:
                if num_manual > 100:
                    if stat_value['tp_rate'] - best['tp_rate'] >= 0.01 and stat_value['fp_rate'] - best['fp_rate'] <= 5e-05:
                        indexs['change'] = True
                elif best['tp_rate'] - stat_value['tp_rate'] <= 0.01 and best['fp_rate'] - stat_value['fp_rate'] >= 5e-05:
                    indexs['change'] = True
...
def comparison(manuals, predicts, nums, args_ops, length):
    ...
    for tss_m in manuals:
        ...
        for tss_p in predicts:
            if tss_p.strand == tss_m.strand and (tss_p.seq_id == tss_m.seq_id or tss_p.seq_id == tss_m.seq_id[:-2] and tss_m.seq_id[(-2)] == '.'):
                if tss_p.start == tss_m.start:
                    tss_p.attributes['print'] = True
                    overlap = True
                    pre_tss = None
                    break
                elif math.fabs(tss_p.start - tss_m.start) <= args_ops.cluster:
                    overlap = True
                    pre_tss = tss_p
    ...
    for tss_p in predicts:
        if tss_p.attributes['print'] is False and tss_p.start <= int(length):
            nums['predict'] += 1
...
def compare_manual_predict(total_step, para_list, gff_files, out_path, out, args_ops, manuals, num_manual, length):
    ...
    if num_manual != 0:
        for gff_file in gff_files:
            ...
            out.write('{0}\t{1}\tTP={2}\tTP_rate={3}\t'.format(total_step, para, nums['overlap'], float(nums['overlap']) / float(num_manual)))
            out.write('FP={0}\tFP_rate={1}\tFN={2}\tmissing_ratio={3}\n'.format(nums['predict'], float(nums['predict']) / float(int(length) - num_manual), nums['manual'], float(nums['manual']) / float(num_manual)))
            if nums['overlap'] == -1:
            ...
            stats.append({'tp': nums['overlap'], 
             ...
             'fp': nums['predict'], 
             'fp_rate': float(nums['predict']) / float(length - num_manual), 
...
def convert2gff(out_path, gff_files, args_ops, strain):
    for core in range(1, args_ops.cores + 1):
        ...
        Converter().convert_mastertable2gff(os.path.join(output_folder, 'MasterTable.tsv'), 'TSSpredator', args_ops.program, strain, gff_file)
...
def print_lib(lib_num, lib_list, out, wig_folder, prefix, rep_set):
    for num_id in range(1, lib_num + 1):
        ...
        for cond in cond_sort_list:
            out.write('{0}_{1}{2} = {3}/{4}\n'.format(prefix, cond['condition'], cond['replicate'], wig_folder, cond['wig']))
            reps.append(cond['replicate'])

        for rep in sorted(rep_set):
            if rep not in reps:
                out.write('{0}_{1}{2} = \n'.format(prefix, cond['condition'], rep))
...
def import_lib(wig_folder, rep_set, lib_dict, out, gff, list_num_id, fasta, args_ops, strain):
    lib_num = 0
    for lib in args_ops.libs:
        ...
        for wig in os.listdir(wig_folder):
            ...
            if filename[0] == lib_datas[0][:-4] and filename[1][:-4] == strain:
                lib_datas[0] = wig
            elif filename[0] == lib_datas[0][:-4] and '.' == filename[1][(-6)] and filename[1][:-6] == strain:
                lib_datas[0] = wig
        ...
        if lib_datas[1] == 'tex' and lib_datas[4] == '+':
            ...
            if lib_datas[1] == 'tex' and lib_datas[4] == '-':
                ...
                if lib_datas[1] == 'notex' and lib_datas[4] == '+':
    ...
    if args_ops.program.lower() == 'tss':
        print_lib(lib_num, lib_dict['fm'], out, wig_folder, 'fivePrimeMinus', rep_set)
        print_lib(lib_num, lib_dict['fp'], out, wig_folder, 'fivePrimePlus', rep_set)
    else:
        if args_ops.program.lower() == 'ps':
            print_lib(lib_num, lib_dict['nm'], out, wig_folder, 'fivePrimeMinus', rep_set)
            print_lib(lib_num, lib_dict['np'], out, wig_folder, 'fivePrimePlus', rep_set)
        else:
...
def gen_config(para_list, out_path, core, wig, fasta, gff, args_ops, strain):
    ...
    lib_num = import_lib(wig, rep_set, lib_dict, out, gff, list_num_id, fasta, args_ops, strain)
    out.write('idList = ')
    ...
    out.write('outputDirectory = {0}\n'.format(os.path.join(out_path, '_'.join(['MasterTable', str(core)]))))
    for prefix_id in range(len(args_ops.replicate_name)):
        out.write('outputPrefix_{0} = {1}\n'.format(prefix_id + 1, args_ops.replicate_name[prefix_id]))
...
def run_tss_and_stat(indexs, list_num, seeds, diff_h, diff_f, out_path, stat_out, best_para, current_para, wig, fasta, gff, best, num_manual, args_ops, strain, manuals, length, log, set_config, run_tss):
    """run TSS and do statistics"""
    if indexs['step'] > args_ops.steps + int(args_ops.cores):
        return (True, best_para)
    if len(list_num) == indexs['length']:
        indexs['step'] = indexs['step'] - 1
        seeds['pre_seed'].append(seeds['seed'])
    else:
        if diff_h <= 0 or diff_f <= 0:
            indexs['step'] = indexs['step'] - 1
            list_num = list_num[:-1]
        else:
            indexs['num'] += 1
    ...
    if indexs['num'] == args_ops.cores:
        ...
        for para in list_num[-1 * args_ops.cores:]:
            ...
            config_files.append(gen_config(para, out_path, index, wig, fasta, gff, args_ops, strain))
            if not set_config:
                set_config = True
                log.write('config files can be generated and stored in {0} successfully.\n'.format(out_path))
        ...
        run_TSSpredator_paralle(config_files, args_ops.tsspredator_path, processes)
        if not run_tss:
            run_tss = True
            log.write('TSSpredator is running successfully.\n')
        convert2gff(out_path, gff_files, args_ops, strain)
        stat_values = compare_manual_predict(indexs['step'], list_num[-1 * args_ops.cores:], gff_files, out_path, stat_out, args_ops, manuals, num_manual, length)
        for stat_value in stat_values:
            if indexs['first']:
                indexs['first'] = False
                best = stat_value
                best_para = copy.deepcopy(list_num[(-1 * args_ops.cores + indexs['count'])])
            else:
                scoring_function(best, stat_value, indexs, num_manual)
            datas = compute_stat(stat_value, best, best_para, args_ops.cores, list_num, out_path, indexs, strain)
            best_para = datas[0]
            best = datas[1]
    ...
    if num_type == 'base_height':
        ...
        while True:
            if new_para[num_type] <= 0.0:
                ...
                new_para[num_type] = float('%.1f' % new_para[num_type])
                continue
            elif (num_type == 'factor' or num_type == 'height') and new_para[num_type] <= compare:
                new_para[num_type] = best_num
                actions['in_or_de'] = 2
                actions['minus'] = True
                break
    ...
    if num_type == 'base_height':
        ...
        while True:
            if new_para[num_type] >= max_num:
                ...
                new_para[num_type] = float('%.1f' % new_para[num_type])
                continue
            elif (num_type == 're_factor' or num_type == 're_height') and new_para[num_type] >= compare:
                new_para[num_type] = best_num
                actions['in_or_de'] = 1
                actions['plus'] = True
                break
...
def run_small_change_part(seeds, features, indexs, current_para, best_para, list_num, max_num):
    ...
    if seeds['seed'] == 0:
        current_para['height'] = small_change(max_num['height'], 'height', best_para['re_height'], list_num, best_para['height'], best_para)
    else:
        if seeds['seed'] == 1:
            current_para['re_height'] = small_change(max_num['re_height'], 're_height', best_para['height'], list_num, best_para['re_height'], best_para)
        else:
            if seeds['seed'] == 2:
                current_para['factor'] = small_change(max_num['factor'], 'factor', best_para['re_factor'], list_num, best_para['factor'], best_para)
            else:
                if seeds['seed'] == 3:
                    current_para['re_factor'] = small_change(max_num['re_factor'], 're_factor', best_para['factor'], list_num, best_para['re_factor'], best_para)
                else:
                    if seeds['seed'] == 4:
                        current_para['base_height'] = small_change(max_num['base_height'], 'base_height', best_para['base_height'], list_num, best_para['base_height'], best_para)
                    else:
                        if seeds['seed'] == 5:
                            current_para['enrichment'] = small_change(max_num['enrichment'], 'enrichment', best_para['enrichment'], list_num, best_para['enrichment'], best_para)
                        elif seeds['seed'] == 6:
                            current_para['processing'] = small_change(max_num['processing'], 'processing', best_para['processing'], list_num, best_para['processing'], best_para)
    return current_para


def gen_large_random(max_num, num_type, compare, list_num, origin_num, best_para, index_large, indexs):
    ...
    while 1:
        ...
        if num_type == index_large[seed]:
            ...
            if num_type == 'base_height':
                number = round(random.uniform(0.0, max_num[num_type]), 3)
                number = '%.3f' % number
                number = float(number)
            else:
            ...
            if index_large[seed] == 'base_height':
                number_par = round(random.uniform(0.0, max_num[index_large[seed]]), 3)
        ...
        if new_para in list_num:
            continue
        elif not new_para['height'] <= new_para['re_height']:
            if new_para['factor'] <= new_para['re_factor']:
                continue
            else:
                list_num.append(copy.deepcopy(new_para))
                return new_para
...
def run_large_change_part(seeds, features, indexs, current_para, max_num, best_para, list_num):
    ...
    if seeds['seed'] == 0:
        current_para = gen_large_random(max_num, 'height', best_para['re_height'], list_num, best_para['height'], best_para, index_large, indexs)
    else:
        if seeds['seed'] == 1:
            current_para = gen_large_random(max_num, 're_height', best_para['height'], list_num, best_para['re_height'], best_para, index_large, indexs)
        else:
            if seeds['seed'] == 2:
                current_para = gen_large_random(max_num, 'factor', best_para['re_factor'], list_num, best_para['factor'], best_para, index_large, indexs)
            else:
                if seeds['seed'] == 3:
                    current_para = gen_large_random(max_num, 're_factor', best_para['factor'], list_num, best_para['re_factor'], best_para, index_large, indexs)
                else:
                    if seeds['seed'] == 4:
                        current_para = gen_large_random(max_num, 'base_height', best_para['base_height'], list_num, best_para['base_height'], best_para, index_large, indexs)
                    else:
                        if seeds['seed'] == 5:
                            current_para = gen_large_random(max_num, 'enrichment', best_para['enrichment'], list_num, best_para['enrichment'], best_para, index_large, indexs)
                        elif seeds['seed'] == 6:
                            current_para = gen_large_random(max_num, 'processing', best_para['processing'], list_num, best_para['processing'], best_para, index_large, indexs)
    return current_para
...
def run_random_part(current_para, list_num, max_num, steps, indexs):
    ...
    while 1:
        ...
        current_para['base_height'] = round(random.uniform(0.0, max_num['base_height']), 3)
        if current_para['height'] > current_para['re_height'] and current_para['factor'] > current_para['re_factor'] and current_para not in list_num:
...
def optimization_process(indexs, current_para, list_num, max_num, best_para, out_path, stat_out, best, wig, fasta, gff, num_manual, new, args_ops, strain, manuals, length, log):
            ...
            if indexs['switch'] % 3 == 0:
                ...
                if new:
                    ...
                    if features['feature'] != features['pre_feature']:
                        seeds['pre_seed '] = []
                    current_para = run_random_part(current_para, list_num, max_num, args_ops.steps, indexs)
                if current_para is None:
                    tmp_step += 1
            else:
                if indexs['switch'] % 3 == 1:
                    features['feature'] = 'l'
                    if features['feature'] != features['pre_feature']:
                        seeds['pre_seed'] = []
                    current_para = run_large_change_part(seeds, features, indexs, current_para, max_num, best_para, list_num)
                else:
                    features['feature'] = 's'
                    if features['feature'] != features['pre_feature']:
                        seeds['pre_seed'] = []
                    current_para = run_small_change_part(seeds, features, indexs, current_para, best_para, list_num, max_num)
                if current_para is not None:
                    ...
                    datas, set_config, run_tss = run_tss_and_stat(indexs, list_num, seeds, diff_h, diff_f, out_path, stat_out, best_para, current_para, wig, fasta, gff, best, num_manual, args_ops, strain, manuals, length, log, set_config, run_tss)
                    tmp_step = 0
                else:
                    indexs['step'] = indexs['step'] - 1
            if tmp_step >= 2:
...
def load_stat_csv(out_path, list_num, best, best_para, indexs, num_manual, stat_file):
    ...
    for row in csv.reader(f_h, delimiter='\t'):
        if line_num == 0 and len(row) < 8:
            print('Error: {0} has something wrong, please check it or remove it!!!'.format(stat_file))
            sys.exit()
        line_num += 1
        paras = row[1].split('_')
        if len(row) == 8:
    ...
    f_h.close()
    return (
     line_num, best, best_para)


def reload_data(out_path, list_num, best, best_para, indexs, num_manual, stat_file, log):
    """if is based on previous run, it is for reload the previous results"""
    indexs['switch'] = 1
    indexs['exist'] = True
    datas = load_stat_csv(out_path, list_num, best, best_para, indexs, num_manual, stat_file)
    line_num = datas[0]
    best = datas[1]
    best_para = datas[2]
    if len(list_num) > 0:
...
def initiate(args_ops):
    ...
    indexs = {'switch': 0, 'extend': False, 'exist': False, 'step': 0, 
     ...
     'count': 0}
    return (
     max_num, best_para, current_para, indexs)
    ...
    if len(os.listdir(out_path)) == 1:
        ...
        if 'stat_' + strain + '.csv' in os.listdir(out_path):
            ...
            if empty_file:
                ...
                datas = reload_data(out_path, list_num, best, best_para, indexs, num_manual, stat_file, log)
                log.write('The intermediate results of the previous optimization is loaded. The optimization will start from {0}\n'.format(indexs['step']))
                best_para = datas[0]
                best = datas[1]
                current_para = extend_data(out_path, best, best_para, indexs['step'], strain)
                stat_out = open(stat_file, 'a')
                indexs['first'] = False
        else:
            list_num = []
            stat_out = open(stat_file, 'w')
    optimization_process(indexs, current_para, list_num, max_num, best_para, out_path, stat_out, best, wig, fasta, gff, num_manual, new, args_ops, strain, manuals, length, log)
