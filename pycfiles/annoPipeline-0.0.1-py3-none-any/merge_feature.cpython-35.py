...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/merge_feature.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 10005 bytes
...
def read_gffs(gff_files, feature):
    ...
    if feature == 'transcript':
        ...
        gff_f.close()
        gffs['transcript'] = sorted(gffs['transcript'], key=lambda x: (x.seq_id, x.start,
         x.end, x.strand))
    else:
        num = 0
        for files in gff_files:
            for gff_file in glob(files):
                ...
                for entry in Gff3Parser().entries(gff_f):
                    parent = None
                    if entry.feature != 'gene' and entry.feature != 'transcript' and entry.feature != 'source' and entry.feature != 'region' and entry.feature != 'remark' and 'Parent' in entry.attributes.keys():
                ...
                gff_f.close()
                gffs[num] = sorted(gffs[num], key=lambda x: (x.seq_id, x.start,
                 x.end, x.strand))
                num += 1
...
def assign_parent(other, tran):
    ...
    elif tran.attributes['ID'] not in other.attributes['Parent'].split(','):
...
def compare_tran(tran_gffs, other_gffs, fuzzy_tss, fuzzy_term):
    for tran in tran_gffs['transcript']:
        for num, others in other_gffs.items():
            for other in others:
                if tran.seq_id == other.seq_id and tran.strand == other.strand and other.feature != 'source' and other.feature != 'region' and other.feature != 'remark':
                    if other.feature == 'TSS':
                        if other.start >= tran.start - fuzzy_tss and other.end <= tran.end + fuzzy_tss:
                            assign_parent(other, tran)
                    else:
                        if other.feature.lower() == 'terminator':
                            if tran.strand == '+':
                                if tran.end >= other.start and tran.end <= other.end or tran.end <= other.start and other.start - tran.end <= fuzzy_term or tran.end >= other.end and tran.end - other.end <= fuzzy_term or tran.start <= other.start and tran.end >= other.end:
                                    assign_parent(other, tran)
                            elif tran.start >= other.start and tran.start <= other.end or tran.start <= other.start and other.start - tran.start <= fuzzy_term or tran.start >= other.end and tran.start - other.end <= fuzzy_term or tran.start <= other.start and tran.end >= other.end:
                                assign_parent(other, tran)
                        elif tran.start <= other.start and tran.end >= other.end or tran.start >= other.start and tran.end <= other.end or tran.start <= other.start and tran.end >= other.start and tran.end <= other.end or tran.start >= other.start and tran.start <= other.end and tran.end >= other.end:
                            assign_parent(other, tran)
            ...
            for num, others in other_gffs.items():
                for other in others:
                    if not other.attributes['print']:
                        if tran.seq_id == other.seq_id and 'Parent' in other.attributes.keys():
                            attributes = {}
                            for key, value in other.attributes.items():
                                if key != 'print':
                                    attributes[key] = value

                            if tran.attributes['ID'] in other.attributes['Parent'].split(','):
                                other.attribute_string = ';'.join(['='.join(items) for items in attributes.items()])
                                other.attributes['print'] = True
                                gffs.append(other)

    for num, others in other_gffs.items():
        for other in others:
            if other.feature == 'source' or other.feature == 'region' or other.feature == 'remark':
                s_gffs.append(other)
            if not other.attributes['print']:
                attributes = {}
                for key, value in other.attributes.items():
    ...
    if len(gffs) != 0:
        ...
        for gff in gffs:
            ...
            for source in s_gffs:
                if source.seq_id == gff.seq_id and not source.attributes['print']:
