...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tina/cookbook_metadata.py
...
class CookbookMetadata:
    ...
    def parse_metadata(self):
        try:
            ...
            regex_depends = re.compile('depends\\s+[\'"](.*?)[\'"](,\\s*[\'"]([~<>=]+)\\s+([\\d\\.]+)[\'"])?')
            ...
            for line in raw:
                ...
                if matches:
                    if self.version:
                        raise Exception("Metadata file has multiple 'version' sections: '%s'" % self.filename)
    ...
    def inject_versions(self, tag, versions):
        ...
        for line in content:
            ...
            if depends_match:
                ...
                if cookbook not in versions:
                    raise Exception("Missing version number for cookbook '%s' " % cookbook)
                else:
                    version = versions[cookbook].version_str()
                    line = re.sub('[\'"]%s[\'"]' % cookbook, '"%s", "= %s"' % (cookbook, version), line)
