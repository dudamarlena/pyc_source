...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tina/tina_util.py
# Compiled at: 2013-05-30 15:51:34
import fileinput, os, re, shutil, sys
    ...
    for depends in root_repo.metadata.depends:
        ...
        for local_dir, cookbook in available_cookbooks.items():
            if cookbook.name == depends:
                found = True
                if cookbook.local_dir not in repos:
                    cookbook.checkout()
                    repos[cookbook.name] = cookbook
                    discover_cookbooks(available_cookbooks, repos, cookbook)
...
def checkout_and_parse(path, interactive):
    ...
    if check_version_conflicts(repos):
        sys.exit(1)
