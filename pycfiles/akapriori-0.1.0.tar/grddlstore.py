...
# Embedded file name: /Library/Python/2.6/site-packages/akamu/protocol/grddlstore.py
# Compiled at: 2013-06-21 16:04:16
...
def HandleGET(req, mgr, environ, root, start_response):
    ...
    if 'HTTP_ACCEPT' not in environ or requestedXML and not requestedRDF:
        ...
        return content
    if requestedRDF and requestedXML:
        doc = parse(content)
        docRoot = doc.xml_select('/*')[0]
        transformPath = os.path.join(root, mgr.findTransform(path)[1:])
        docRoot.xmlns_attributes['grddl'] = GRDDL_NS
        docRoot.xml_attributes[(GRDDL_NS, 'transformation')] = transformPath
        stream = StringIO()
    ...
    if requestedRDF:
        ...
        graphUri = URIRef(mgr.graphUriFn(path, GetFNameFromPath(path)))
...
def HandlePATCH(req, mgr, environ, root, start_response):
    ...
    if req.content_type == XSLT_MT:
        ...
        if payloadDoc.xml_select('/xu:modifications', prefixes={'xu': XUPDATE_NS}):
            from amara.xupdate import reader, XUpdateError, apply_xupdate
...
def HandlePOST(req, mgr, diglotPath, base=None):
    ...
    if req.content_type == XML_MT:
        return (
         transform(req.body, targetContent), 'application/xml+rdf')
    else:
        doc = parse(targetContent)
        payloadGraph = Graph().parse(StringIO(req.body), format=layercake_mimetypes[req.content_type], publicID=URIRef(base) if base else None)
        for revXFormSrc in doc.xml_select('/xsl:stylesheet/ggs:reverse_transform', prefixes={'ggs': 'http://code.google.com/p/akamu/wiki/DiglotFileSystemProtocol#Bidirectional_transformations', 
           'xsl': 'http://www.w3.org/1999/XSL/Transform'}):
            fnCode = revXFormSrc.xml_select('string(text())').strip()
            module = imp.new_module('inverseMap')
            exec fnCode in module.__dict__
            return (
             module.ReverseTransform(payloadGraph), XML_MT)
...
def HandleDirectoryPOST(req, start_response, mgr, newPath, newDiglotPath):
    if req.content_type == XML_MT:
        ...
        doc = parse(targetContent)
        revXForm = doc.xml_select('/xsl:stylesheet/ggs:reverse_transform', prefixes={'ggs': 'http://code.google.com/p/akamu/wiki/DiglotFileSystemProtocol#Bidirectional_transformations', 
           'xsl': 'http://www.w3.org/1999/XSL/Transform'})
        if revXForm:
...
class xforms_grddl(object):
    ...
    def __init__(self, instanceId='diglot-resource', submissionId='diglot-submission', hostDocumentBase=None, hostDocument=None, diglotRoot=None, hostDocumentParam='document', instanceAttribute='src', instanceParam='src'):
...
class grddl_graphstore_resource(object):
    ...
    def __init__(self, root, graphUriFn, caching=False, cacheability=None):
        ...
        self.cachingSetup = WheezyCachingAdapterSetup(queries=['base'], environ=['HTTP_ACCEPT'], asFn=True) if caching else None
        if cacheability:
    ...
    def handleResourceCacheability(self, mgr, diglotPath, cacheability, environ):
        ...
        updatePolicy4File(mgr.getFullPath(diglotPath), policy)
                ...
                if req.method in ('HEAD', 'GET'):
                    ...
                    if req.method == 'GET':
                        if self.cachingSetup:
                            self.handleResourceCacheability(mgr, diglotPath, self.cacheability, environ)
                        return rt
                    return ''
                else:
                    if req.method == 'PUT':
                        ...
                        if not os.path.exists(_path):
                            ...
                            if self.cachingSetup:
                                self.invalidateResourceCache(self.cacheability, environ, diglotPath)
                        return ''
                    else:
                        ...
                        if req.method == 'POST' and isXslt:
                            ...
                            if self.cachingSetup:
                                environ['wheezy.http.noCache'] = True
                            return rt
                        if req.method == 'POST' and os.path.isdir(mgr.getFullPath(diglotPath)):
                            ...
                            if self.cachingSetup:
                                environ['wheezy.http.noCache'] = True
                            return HandleDirectoryPOST(req, start_response, mgr, newPath, newDiglotPath)
