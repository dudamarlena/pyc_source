...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/emrt/necd/content/browser/observation_import.py
# Compiled at: 2020-02-21 10:42:30
import re
...
UNCOMPLETED_ERR = 'The observation on row no. {} seems to be a bit off. Please fill all the fields as shown in the import file sample. '
WRONG_DATA_ERR = 'The information you entered in the {} section of row no. {} is not correct. Please consult the columns in the sample xls file to see the correct set of data.'
DONE_MSG = 'Successfully imported {} observations!'
PROJECTION_COLS = ('nfr_code_inventory', 'year', 'reference_year', 'pollutants', 'scenario',
                   'review_year', 'activity_data_type', 'activity_data', 'ms_key_category',
                   'parameter', 'highlight')
INVENTORY_COLS = ('year', 'pollutants', 'review_year', 'fuel', 'ms_key_category', 'parameter',
                  'highlight')
...
def get_constants(context):
    ...
    next_col = 3
    iter_cols = PROJECTION_COLS if context == 'projection' else INVENTORY_COLS
...
class Entry(object):
    ...
    def activity_data(self):
        ...
        if cell_value == ('', ):
            ...
            if not self.activity_data_type:
                return False
            keys = [ key_in_vocab(activity_voc, val) for val in cell_value
                   ]
            if False in keys:
                return False
            activity_data_registry = get_registry_interface_field_data(INECDVocabularies, 'activity_data')
            if set(activity_data_registry[self.activity_data_type]).difference(cell_value):
                return False
            return keys
    ...
    def get_fields(self):
        ...
        return {name:getattr(self, name) for name in fields if name not in UNUSED_FIELDS and name != 'question'}
...
def _create_observation(entry, context, request, portal_type, obj):
    ...
    if errors:
        ...
        content = api.content.create(context, type=portal_type, title=getattr(entry, 'title'), **fields)
...
class ObservationXLSImport(BrowserView):
    ...
    def do_import(self):
        ...
        get_vocabulary = _cached_get_vocabulary(self.context, prefix='emrt.necd.content.')
        entries = []
        for row in valid_rows:
            entries.append(Entry(row, constants, self.context.type == 'projection', get_vocabulary))

        for entry in entries:
            _, errors = _create_observation(entry, self.context, self.request, PORTAL_TYPE, self)
            if errors:
                return error_status_messages(self.context, self.request, errors)
