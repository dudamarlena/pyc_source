...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/emrt/necd/content/browser/tableau.py
# Compiled at: 2020-05-07 14:24:27
...
def entry_for_cmp(entry):
    ...
    """
    return {k:v for k, v in entry.items() if k not in ('Timestamp', 'Modified') if k not in ('Timestamp',
                                                                                             'Modified')}
...
def flatten_historical_data(data):
    ...
    """
    return sorted(chain.from_iterable(data.values()), key=GET_TIMESTAMP, reverse=True)
...
def get_qa(catalog, brain):
    path = brain.getPath()
    return catalog.unrestrictedSearchResults(portal_type=[
     'Comment', 'CommentAnswer'], path=path)
...
def extract_entry(qa, timestamp, vocab_description_flags, vocab_conclusion_flags, vocab_highlight, brain):
    b_id = brain['id']
    obs_status = brain['observation_status']
    len_a = qa['CommentAnswer'][b_id]
    len_q = qa['Comment'][b_id]
    highlights = translate_highlights(vocab_highlight, brain['get_highlight'] or [])
    return {'Current status': current_status(brain), 
       ...
       'Potential technical correction': bool(brain['observation_is_potential_technical_correction'])}


def validate_token(request, token=TOKEN_VIEW):
    if token:
        return request.get('tableau_token') == token
    return False
...
def get_snapshot(context):
    ...
    qa = dict(Comment=Counter(p.split('/')[3] for p in p_comment), CommentAnswer=Counter(p.split('/')[3] for p in p_comment_answer))
    vocab_description_flags, vocab_conclusion_flags, vocab_highlight = get_highlight_vocabs(context)
    entry = partial(extract_entry, qa, timestamp, vocab_description_flags, vocab_conclusion_flags, vocab_highlight)
    brains = catalog.unrestrictedSearchResults(portal_type=[
     'Observation'], path=('/').join(context.getPhysicalPath()))
...
class TableauHistoricalView(BrowserView):
    ...
    def __call__(self):
        ...
        if validate_token(request):
            ...
            data = flatten_historical_data(update_history_with_snapshot(data, snapshot))
        else:
            request.RESPONSE.setStatus(401)
...
class ConnectorView(BrowserView):
    ...
    def __call__(self):
        ...
        if validate_token(request):
            return self.index(portal_url=getSite().absolute_url())
        request.RESPONSE.setStatus(401)
