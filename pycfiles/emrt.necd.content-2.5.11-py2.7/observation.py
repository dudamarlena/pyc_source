...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/emrt/necd/content/observation.py
# Compiled at: 2020-05-04 06:15:41
try:
    from cStringIO import StringIO
except ImportError:
    from StringIO import StringIO

import datetime, simplejson as json, re
...
def get_nfr_title_projection(fname):
    names = dict(nfr_code='Review sectors for Projections, NAPCP and PaMs', nfr_code_inventory='Review sub sectors')
    return names[fname]
...
def check_country(value):
    ...
    valid = False
    for group in groups:
        if group.startswith(('{}-').format(LDAP_SECTOREXP)) and group.endswith('-%s' % value):
            valid = True
...
class IObservation(model.Schema, IImageScaleTraversable):
    ...
    """
    text = schema.Text(title='Short description by sector expert', required=True, description='Describe the issue identified. Keep it short, you cannot change this description once you have sent it to LR. MS can only see the question once it has been approved and sent by the LR. The question to the MS should be asked in the Q&A tab, not here.')
    country = schema.Choice(title='Country', vocabulary='emrt.necd.content.eea_member_states', required=True)
    nfr_code = schema.Choice(title='NFR category codes', vocabulary='emrt.necd.content.nfr_code', required=True)
    nfr_code_inventory = schema.Choice(title='NFR inventories category code', vocabulary='emrt.necd.content.nfr_code_inventories', required=False)
    year = schema.TextLine(title='Inventory year', description='Inventory year can be a given year (2014), a range of years (2012-2014) or a list of the years (2012, 2014, 2016)', constraint=inventory_year, required=True)
    reference_year = schema.Int(title='Reference year', required=True, min=1000, max=9999)
    form.widget(pollutants=CheckBoxFieldWidget)
    pollutants = schema.List(title='Pollutants', value_type=schema.Choice(vocabulary='emrt.necd.content.pollutants'), constraint=check_pollutants, required=True)
    form.widget(scenario=CheckBoxFieldWidget)
    scenario = schema.List(title='Scenario Type', value_type=schema.Choice(vocabulary='emrt.necd.content.scenario_type'), required=False)
    review_year = schema.Int(title='Review year', description='Review year is the year in which the inventory was submitted and the review was carried out', defaultFactory=default_year, required=True)
    fuel = schema.Choice(title='Fuel', vocabulary='emrt.necd.content.fuel', required=False)
    activity_data_type = schema.Choice(title='Activity Data Type', vocabulary='emrt.necd.content.activity_data_type', required=False)
    form.widget(activity_data=CheckBoxFieldWidget)
    activity_data = schema.List(title='Activity Data', value_type=schema.Choice(vocabulary='emrt.necd.content.activity_data'), required=False)
    ms_key_category = schema.Bool(title='MS key category')
    form.widget(parameter=CheckBoxFieldWidget)
    parameter = schema.List(title='Parameter', value_type=schema.Choice(vocabulary='emrt.necd.content.parameter'), constraint=check_parameter, required=True)
    form.widget(highlight=CheckBoxFieldWidget)
    highlight = schema.List(title='Description flags', description='Description flags highlight important information that is closely related to the item.', value_type=schema.Choice(vocabulary='emrt.necd.content.highlight'), required=False, default=[])
    form.write_permission(closing_comments='cmf.ManagePortal')
    closing_comments = schema.Text(title='Finish request comments', required=False)
    form.write_permission(closing_deny_comments='cmf.ManagePortal')
    closing_deny_comments = schema.Text(title='Finish deny comments', required=False)
...
class NfrCodeContextValidator(validator.SimpleFieldValidator):
    ...
    def validate(self, value, force=False):
        ...
        if not valid:
            raise Invalid('You are not allowed to add observations for this sector category.')


validator.WidgetValidatorDiscriminators(NfrCodeContextValidator, field=IObservation['nfr_code'])
...
validator.WidgetValidatorDiscriminators(CountryContextValidator, field=IObservation['country'])
...
def set_title_to_observation(obj, event):
    ...
    obj_year = (obj.year if isinstance(obj.year, basestring) else (', ').join(obj.year)) if obj.year else ''
...
class Observation(Container):
    ...
    def country_value(self):
        return get_vocabulary_value(self, 'emrt.necd.content.eea_member_states', self.country)
    ...
    def parameter_value(self):
        return get_join_from_vocab(self.aq_parent, 'emrt.necd.content.parameter', self.parameter)

    def pollutants_value(self):
        return get_join_from_vocab(self.aq_parent, 'emrt.necd.content.pollutants', self.pollutants)

    def activity_data_value(self):
        return get_list_from_vocab(self.aq_parent, 'emrt.necd.content.activity_data', self.activity_data)

    def highlight_value(self):
        return get_join_from_vocab(self.aq_parent, 'emrt.necd.content.highlight', self.highlight)

    def scenario_type_value(self):
        return get_join_from_vocab(self.aq_parent, 'emrt.necd.content.scenario_type', self.scenario)

    def finish_reason_value(self):
        return get_vocabulary_value(self.aq_parent, 'emrt.necd.content.finishobservationreasons', self.closing_reason)

    def finish_deny_reason_value(self):
        return get_vocabulary_value(self.aq_parent, 'emrt.necd.content.finishobservationdenyreasons', self.closing_deny_reason)
    ...
    def can_draft_conclusions(self):
        ...
        if len(questions) > 0:
            q = questions[0]
            return q.get_state_api() in ('draft', 'drafted', 'recalled-lr', 'closed')
        else:
            return True
    ...
    def wf_location(self):
        ...
        if status == 'conclusion-discussion':
            return 'Counterpart'
        ...
        if questions:
            ...
            if state in ('draft', 'closed'):
                return 'Sector Expert'
            if state in ('counterpart-comments', ):
                return 'Counterparts'
            if state in ('drafted', 'recalled-lr'):
                return 'Lead reviewer'
            if state in ('pending', 'answered', 'pending-answer-drafting', 'recalled-msa'):
                return 'Member state coordinator'
            if state in ('expert-comments', ):
    ...
    def wf_status(self):
        ...
        if self.get_status() in ('closed', ):
            return ['Observation finished', 'observationIcon']
        if self.get_status() in ('close-requested', ):
        ...
        if self.get_status() in ('conclusion-discussion', ):
        ...
        if questions:
            ...
            if state in ('pending', 'pending-answer-drafting', 'recalled-msa'):
            ...
            return [
             'Observation created', 'observationIcon']
    ...
    def observation_status(self):
        ...
        if status in ('draft', 'counterpart-comments', 'observation-draft'):
            return 'SE'
        if status in ('drafted', 'recalled-lr'):
            return 'LR'
        if status in ('pending', 'recalled-msa', 'pending-answer-drafting', 'expert-comments'):
            return 'MSC'
        if status in ('answered', ):
            return 'answered'
        if status in ('conclusions', 'conclusions-lr-denied', 'conclusion-discussion'):
            return 'conclusions'
        if status in ('close-requested', ):
        ...
        if status == 'closed':
            ...
            if conclusion_reason == 'no-conclusion-yet':
                return 'SE'
            if not my_status.endswith('closed'):
                return 'answered'
            return 'finalised'
        else:
            return status
    ...
    def myHistory(self):
        ...
        for item in observation_history:
            ...
            if i_rstate == 'draft':
                ...
                item['role'] = 'Sector Expert'
                observation_wf.append(item)
            elif i_rstate in ('conclusions', 'conclusions-lr-denied') and i_action == 'deny-finishing-observation':
        ...
        if questions:
            ...
            for item in question_history:
                ...
                if i_rstate == 'draft' and i_action == 'reopen':
                    ...
                    item['role'] = 'Member state coordinator'
                    question_wf.append(item)
                elif i_action == 'validate-answer-msa' and i_action == 'validate-answer-msa':
    ...
    def reply_comments_by_mse(self):
        ...
        commentators = []
        if question:
            commentators = list(set(chain.from_iterable([ IConversation(c).commentators for c in question.values()
                                                        ])))
        return [ uid for uid in commentators if ROLE_MSE in api.user.get_roles(username=uid, obj=self)
               ]
    ...
    def observation_already_replied(self):
        ...
        if questions:
            ...
            states = [ w.get('review_state') for w in winfo.get('esd-question-review-workflow', [])
                     ]
            if states:
                sp = {s:idx for idx, s in enumerate(states)}
                return states[(-1)] not in ('recalled-msa', 'pending', 'pending-answer-drafting',
                                            'expert-comments') and sp.get('answered')
        return False
...
def set_form_widgets(form_instance):
    ...
    if _is_projection(form_instance.context):
        ...
        widgets['activity_data_type'].template = Z3ViewPageTemplateFile('templates/widget_activity_type.pt')
        w_activity_data.template = Z3ViewPageTemplateFile('templates/widget_activity.pt')
        w_activity_data.activity_data_registry = json.dumps(get_registry_interface_field_data(INECDVocabularies, 'activity_data'))
    else:
        ...
        widgets['pollutants'].template = Z3ViewPageTemplateFile('templates/widget_pollutants.pt')
        widgets['nfr_code_inventory'].mode = interfaces.HIDDEN_MODE
    widgets['text'].rows = 15
    widgets['highlight'].template = Z3ViewPageTemplateFile('templates/widget_highlight.pt')
    form_instance.groups = [ g for g in form_instance.groups if g.label == 'label_schema_default'
                           ]
...
def set_form_fields(form_instance):
    ...
    if _is_projection(form_instance.context):
        ...
        if hide_for_user:
            ...
            fields['parameter'].field = schema.List(title='Parameter', value_type=schema.Choice(vocabulary='emrt.necd.content.parameter'), required=False)
            fields['pollutants'].field = schema.List(title='Pollutants', value_type=schema.Choice(vocabulary='emrt.necd.content.pollutants'), required=False)
        else:
            fields['year'].field = schema.List(title='Projection year', description="Projection year is the year or a list of years (e.g. '2050', '2020, 2025, 2030') when the emissions had occured for which an issue was observed in the review.", value_type=schema.Choice(values=[
             '2020',
             '2025',
             '2030',
             '2040',
             '2050']), required=True, __name__='year')
...
class EditForm(edit.DefaultEditForm):
    ...
    def updateFields(self):
        ...
        if 'Manager' in roles:
            ...
            if ROLE_SE in roles:
                fields = [ f for f in field.Fields(IObservation) if f not in ('country',
                                                                              'nfr_code',
                                                                              'review_year',
...
class ObservationMixin(DefaultView):
    ...
    def can_add_conclusion(self):
        ...
        return sm.checkPermission('emrt.necd.content: Add Conclusions', self.context) and question_state in ('draft',
                                                                                                             'drafted',
                                                                                                             'pending',
                                                                                                             'closed')
    ...
    def actions(self):
        context = aq_inner(self.context)
        question = self.question()
        observation_menu_items = getMenu('plone_contentmenu_workflow', context, self.request)
        menu_items = observation_menu_items
        if question:
            question_menu_items = getMenu('plone_contentmenu_workflow', question, self.request)
            if not self.can_add_follow_up_question():
                question_menu_items = [ item for item in question_menu_items if not item['action'].endswith('add-followup-question')
                                      ]
    ...
    def get_user_name(self, userid, question=None):
        if question is not None:
            ...
            if question.portal_type == 'CommentAnswer':
    ...
    def in_conclusions(self):
        ...
        return state in ('conclusions', 'conclusions-lr-denied', 'conclusion-discussion',
        ...
        if context is not None:
            if context.can_edit():
                ...
                if hist_meta:
                    ...
                    changeset = dt.createChangeSet(self.getVersion(version2), self.getVersion(version1), id1=self.versionTitle(version2), id2=self.versionTitle(version1))
                    self.changes = [ change for change in changeset.getDiffs() if not change.same
                                   ]
        return
    ...
    def versionTitle(self, version):
        ...
        return translate(_CMFE('version ${version}', mapping=dict(version=version_name)), context=self.request)
...
class ObservationView(ObservationMixin):
    ...
    def can_export_observation(self):
        sm = getSecurityManager()
        return sm.checkPermission('emrt.necd.content: Export an Observation', self.context)
...
class ExportAsDocView(ObservationMixin):
    ...
    def build_file(self):
        ...
        row_cells[3].text = (is_projection or get_vocabulary_value)(self.context, IObservation['fuel'].vocabularyName, self.context.fuel) if 1 else str(self.context.reference_year)
        ...
        if is_projection:
            ...
            document.add_paragraph('NFR Inventories Category Code', style='Label Bold')
        ...
        if self.context.get_status() == 'close-requested':
            ...
            document.add_paragraph('SE comments on finish observation request:', style='Label Bold')
        ...
        if chats:
            ...
            for chat in chats:
                ...
                if chat.portal_type.lower() == 'comment':
                    ...
                    document.add_paragraph('From TERTs To Member State \t\t %s' % (sent_info % date.strftime('%d %b %Y, %H:%M CET')))
                if chat.portal_type.lower() == 'commentanswer':
                    document.add_paragraph('< %s' % self.strip_special_chars(chat.text))
                    document.add_paragraph('From Member State To TERTs \t\t %s' % (sent_info % date.strftime('%d %b %Y, %H:%M CET')))
    ...
    def render(self):
        ...
        response.setHeader('Content-disposition', 'attachment;filename=' + self.context.getId() + '.docx')
        f = StringIO()
        document.save(f)
        f.seek(0)
...
class AddQuestionForm(Form):
    ...
    def create_question(self, action):
        ...
        if qs:
            ...
            q_id = context.invokeFactory(type_name='Question', id='question-1', title='Question 1')
...
class AddAnswerAndRequestComments(BrowserView):
    ...
    def render(self):
        ...
        if len(comments) == len(answers):
            ...
            return self.request.response.redirect(observation.absolute_url())
        context = questions[0]
        text = 'For MS coordinator: please draft, edit and finalize here your answer AFTER CLOSING COMMENT within your member state expert.'
        id = str(int(time()))
        item_id = context.invokeFactory(type_name='CommentAnswer', id=id)
        comment = context.get(item_id)
        comment.text = text
