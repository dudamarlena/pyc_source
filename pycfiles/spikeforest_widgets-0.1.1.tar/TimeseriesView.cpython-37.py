...
# [GCC 8.4.0]
# Embedded file name: /home/magland/src/spikeforest2/spikeforest_widgets/generated/spikeforest_widgets/spikeforest_widgets/widgets/TimeseriesView/TimeseriesView.py
# Compiled at: 2019-12-03 09:52:43
# Size of source mod 2**32: 11118 bytes
from mountaintools import client as mt
from mountaintools import MountainClient
import traceback, spikeextractors as se, numpy as np, io, base64, time
...
class TimeseriesView:
    ...
    def javascript_state_changed(self, prev_state, state):
        ...
        if not self._recording:
            ...
            self.set_state(dict(num_channels=(self._recording.get_num_channels()),
    ...
    def on_message(self, msg):
        if msg['command'] == 'requestSegment':
            ...
            self.send_message(dict(command='setSegment',
              ds_factor=ds,
              segment_num=ss,
              data=data0_base64))
    ...
    def _load_data(self, ds, ss):
        ...
        if ds > 1:
            if self._multiscale_recordings is None:
                ...
                self._multiscale_recordings = _create_multiscale_recordings(recording=(self._recording),
            ...
            return X
        start_time = time.time()
        traces = self._recording.get_traces(start_frame=(ss * self._segment_size),
...
def _create_multiscale_recordings(*, recording, progressive_ds_factor, create_efficient_access=True):
    ...
    while current_ds_factor * progressive_ds_factor < N:
        current_rx = _DownsampledRecordingExtractor(recording=current_rx,
          ds_factor=progressive_ds_factor,
          input_has_minmax=recording_has_minmax)
...
class _DownsampledRecordingExtractor(se.RecordingExtractor):
    ...
    def hash(self):
        ...
        return mt.sha1OfObject(dict(name='downsampled-recording-extractor',
          version=2,
    ...
    def get_traces(self, channel_ids=None, start_frame=None, end_frame=None):
        ds_factor = self._ds_factor
        if self._input_has_minmax:
            X = self._recording.get_traces(channel_ids=channel_ids,
              start_frame=(start_frame * ds_factor),
              end_frame=(end_frame * ds_factor))
            X_mins = X[:, 0::2]
            X_maxs = X[:, 1::2]
            X_mins_reshaped = np.reshape(X_mins,
              (X_mins.shape[0], X_mins.shape[1] // ds_factor, ds_factor), order='C')
            X_maxs_reshaped = np.reshape(X_maxs,
              (X_maxs.shape[0], X_maxs.shape[1] // ds_factor, ds_factor), order='C')
            ret = np.zeros((X.shape[0], X.shape[1] // ds_factor))
            ret[:, 0::2] = np.min(X_mins_reshaped, axis=2)
            ret[:, 1::2] = np.max(X_maxs_reshaped, axis=2)
            return ret
        X = self._recording.get_traces(channel_ids=channel_ids,
          start_frame=(start_frame * self._ds_factor // 2),
          end_frame=(end_frame * self._ds_factor // 2))
        X_reshaped = np.reshape(X,
          (X.shape[0], X.shape[1] // ds_factor, ds_factor), order='C')
    ...
    def write_recording(recording, save_path):
        EfficientAccessRecordingExtractor(recording=recording,
          _dest_path=save_path)


def _samplehash(recording):
    from mountaintools import client as mt
    obj = {'channels':tuple(recording.get_channel_ids()), 
