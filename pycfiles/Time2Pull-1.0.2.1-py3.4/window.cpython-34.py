...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/time2pull/window.py
# Compiled at: 2014-06-16 15:45:18
# Size of source mod 2**32: 14609 bytes
"""
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.load_repo_from_settings()
        self.pushButtonRemove.setEnabled(bool(len(self.listWidgetRepos.selectedItems())))
    ...
    def check_git(self):
        if os.system('git --version') != 0:
            self.show()
            QtWidgets.QMessageBox.warning(self, 'Git not found', 'Cannot find git, please add it to your PATH')
            sys.exit(1)
    ...
    def setup_tray_icon_mnu(self):
        ...
        self.actionIconGroup.actions()[int(TrayIconType.light)].setChecked(Settings().tray_icon_type == TrayIconType.light)
        self.actionIconGroup.actions()[int(TrayIconType.dark)].setChecked(Settings().tray_icon_type == TrayIconType.dark)
    ...
    def setupUi(self, MainWindow):
        ...
        self.listWidgetRepos.itemSelectionChanged.connect(self.on_selection_changed)
        ...
        if not self._quitting:
            if not self._user_warned_about_tray:
                if not Settings().hide_on_startup:
                    QtWidgets.QMessageBox.information(self, 'Time2Pull', 'The program will keep running in the system tray.\nTo terminate the program, choose Quit in the context menu of the system tray entry.')
    ...
    def on_pushButtonAdd_clicked(self):
        path = QtWidgets.QFileDialog.getExistingDirectory(self, 'Select a repository')
        settings = Settings()
        if path:
            if path not in settings.repositories:
                if os.path.exists(os.path.join(path, '.git')):
                    ...
                    QtWidgets.QMessageBox.warning(self, 'Not a git repository', 'The chosen directory is not a git repository')

    @QtCore.pyqtSlot()
    def on_pushButtonRemove_clicked(self):
        repo = self.listWidgetRepos.currentItem().text()
        answer = QtWidgets.QMessageBox.question(self, 'Remove repository', 'Are you sure you want to remove repository: %s?' % repo)
    ...
    def on_selection_changed(self):
        self.pushButtonRemove.setEnabled(len(self.listWidgetRepos.selectedItems()) and self.worker_thread.is_sleeping())

    def alert(self, repo):
        if Settings().show_msg:
            repo_name = QtCore.QFileInfo(repo).fileName()
            self.tray_icon.showMessage(repo_name, "Remote repository has been updated. It's time to pull!")
    ...
    def on_status_available(self, repo, dirty, remote_status):
        for i in range(self.listWidgetRepos.count()):
            ...
            if item.text() == repo:
                ...
                old_dirty_flg, old_remote_status = item.data(QtCore.Qt.UserRole)
                item.setData(QtCore.Qt.UserRole, (dirty, remote_status))
                if remote_status == RemoteStatus.behind or remote_status == RemoteStatus.diverged:
                    if old_remote_status != RemoteStatus.behind and old_remote_status != RemoteStatus.diverged:
                        self.alert(repo)
                else:
                    continue
    ...
    def on_actionAbout_triggered(self):
        QtWidgets.QMessageBox.about(self, 'About Time2Pull', 'Time2Pull is a small application that monitors your local git repositories and warns you when a remote got updated.\n\nTime2Pull is a free open source software licensed under the GPL v3. The application is written in Python using the PyQt5 GUI toolkit.\n\nAuthor: Colin Duquesnoy\nVersion: %s' % __version__)

    def on_tray_icon_style_triggered(self, action):
        ltext = action.text().lower()
        settings = Settings()
        settings.tray_icon_type = TrayIconType.dark if 'dark' in ltext else TrayIconType.light
        self.update_tray_icon()
    ...
    def on_actionHelp_triggered(self):
        QtGui.QDesktopServices.openUrl(QtCore.QUrl('http://time2pull.readthedocs.org/en/latest/'))