...
# [GCC 8.4.0]
# Embedded file name: /home/jovyan/search-tools/search_tools/metrics.py
# Compiled at: 2019-10-08 18:12:54
...
def get_rankings(df, gt_col, est_col, query_col='query', id_col='pid'):
    ...
    """
    sorted_sdf = df.groupBy(query_col).agg(F.sort_array((F.collect_list(F.struct(F.col(gt_col), F.col(id_col)))), asc=False).alias('best'), F.sort_array((F.collect_list(F.struct(F.col(est_col), F.col(id_col)))), asc=False).alias('estimated'))
