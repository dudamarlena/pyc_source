...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/turingmarkov/turing.py
# Compiled at: 2015-06-18 05:25:18
    ...
    def add_state(self, string):
        ...
        if len(parsed_string) > 0:
            ...
            if len(rules) != len(self.alphabet):
                raise SyntaxError('Wrong count of rules ({cur}/{exp}): {string}'.format(cur=len(rules), exp=len(self.alphabet), string=string))
