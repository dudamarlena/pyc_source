...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/Products/Zpydoc/ZpyDocumentable.py
...
from AccessControl.Permissions import change_configuration, view, view_management_screens, access_contents_information
...
class ZpyDocumentable(ObjectManager, PropertyManager, SimpleItem):
    ...
    _properties = ({'id': 'title', 'mode': 'w', 'type': 'string'}, {'id': 'directory', 'mode': 'w', 'type': 'selection', 'select_variable': 'directories'}, {'id': 'renderer_id', 'mode': 'w', 'type': 'selection', 'select_variable': 'renderers'}, {'id': 'show_private', 'mode': 'w', 'type': 'boolean'})
    __ac_permissions__ = ObjectManager.__ac_permissions__ + PropertyManager.__ac_permissions__ + SimpleItem.__ac_permissions__ + (
     (
      view, ('packages', 'renderer', 'isPermitted')),
     (
      view_management_screens,
      ('manage_file_permissions', 'isChecked', 'directories', 'all_modules', 'renderers')),
     (
      access_contents_information, ('path', 'path_difference')),
     (
      change_configuration, ('manage_updateFilePermissions', )))
    manage_options = ObjectManager.manage_options + ({'label': 'View', 'action': ''}, {'label': 'Permissions', 'action': 'manage_file_permissions'}) + PropertyManager.manage_options + SimpleItem.manage_options
    ...
    def packages(self, all=False):
        ...
        if self.directory == 'builtin__':
            if all:
                return map(lambda x: {'name': x, 'file': '', 'ispackage': 0}, sys.builtin_module_names)
            else:
                return map(lambda x: {'name': x, 'file': '', 'ispackage': 0}, self._file_permissions.keys())
        ...
        for file in map(lambda x, y=self.directory: '%s/%s' % (y, x), os.listdir(self.directory)):
