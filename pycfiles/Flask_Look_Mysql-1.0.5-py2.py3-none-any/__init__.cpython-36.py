...
# [GCC 8.4.0]
# Embedded file name: C:\Users\libai\dfk-common\dfk_github\Flask-Look-Mysql\flask_look_mysql\__init__.py
# Compiled at: 2019-09-23 23:37:53
...
class FlaskLookMysql(object):
    ...
    def v2_listModel(self, name):
        if name not in self._url_dict.keys():
            ...
            dbName = name
            all_tableStr = "select table_name,table_comment,TABLE_TYPE from information_schema.tables where TABLE_TYPE in ('BASE TABLE','VIEW') and  table_schema='{}'".format(dbName)
    ...
    def v2_listModels(self, tableresultList, dbName):
        ...
        if len(resultList) == 1:
            ...
            if len(resultList) > 1:
                ...
                return
        filedStr = "select ORDINAL_POSITION,column_name,COLUMN_COMMENT,DATA_TYPE,CHARACTER_MAXIMUM_LENGTH,numeric_precision,numeric_scale,COLUMN_KEY,COLUMN_DEFAULT,IS_NULLABLE,table_name from information_schema.columns where  table_schema='{}' and table_name in {} ".format(dbName, resultList)
        ...
        if filedList:
            for index, field in enumerate(filedList):
                ...
                if field[5]:
                    ...
                    if field[9] == 'YES':
                        ...
                        IS_NULLABLE = 'N'
                _infoDict = {'ORDINAL_POSITION':field[0], 
                 'column_name':field[1], 
            ...
            for key, value in table_name_dict.items():
                infoDict = {'index':len(_infoList) + 1, 
                 'tableName':key, 
