...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/rippleTank/tank.py
...
class RippleTank:
    ...
    def __init__(self, xdim=(-15, 15), ydim=(-15, 15), deep=1.0, n_cells_x=100, n_cells_y=100, mask=1.0, bc='open', alpha=0.45, units='cm'):
        posible_bcs = ('open', 'close')
        if bc not in posible_bcs:
            raise Exception("'%s' is not a valid boundary condition." % bc)
        posible_units = ('cm', 'm')
        if units not in posible_units:
        ...
        self.masked_deep = deep * self.mask
        self.g = 9.8
        if self.units == 'cm':
            self.g = 980
        self.speed = np.sqrt(self.g * deep)
        self.dt = alpha * min(self.dx, self.dy) / self.speed
        ...
        self.masks = []
        self.amplitude = None
        self.complete_values = None
        self.forbidden_pos = None
        self.sim_duration = None
        self.animation_speed = 1.0
        self.fps = 24.0
        self.extent = [
         self.xdim[0], self.xdim[1], self.ydim[0], self.ydim[1]]
    ...
    def solveInstant(self, i):
        ...
        self.amplitude[i + 1, 1:-1, 1:-1] = 2 * self.amplitude[i, 1:-1, 1:-1] - self.amplitude[i - 1, 1:-1, 1:-1] + self.getSecondPartEquation(i)[1:-1, 1:-1]
        if self.bc == 'open':
            self.solveBorders(i)
        pos = self.speed == 0
    ...
    def getSecondPartEquation(self, i):
        ...
        temp[1:-1, 1:-1] = ratiox * (self.amplitude[i, 1:-1, :-2] - 2 * self.amplitude[i, 1:-1, 1:-1] + self.amplitude[i, 1:-1, 2:]) + ratioy * (self.amplitude[i, :-2, 1:-1] - 2 * self.amplitude[i, 1:-1, 1:-1] + self.amplitude[i, 2:, 1:-1])
        return temp
    ...
    def solvePoints(self, n_instants):
        ...
        self.amplitude[0] = self.evaluateSources(0)
        self.amplitude[1] = self.amplitude[0] + self.getSecondPartEquation(1)
    ...
    def configPlot(self, figsize=(6, 4.5), xlabel=None, ylabel=None, cmap=jet, vmin=None, vmax=None, cbar_label=None, origin='lower'):
        """
        Configures the plot.
        ...
        self.time_label = self.ax.text(self.xdim[0] + 0.1 * (self.xdim[1] - self.xdim[0]), self.ydim[1] - 0.1 * (self.ydim[1] - self.ydim[0]), '')
        self.wave_show = self.ax.imshow(np.zeros_like(self.X), cmap=cmap, vmin=vmin, vmax=vmax, origin=origin, animated=True, extent=self.extent)
    ...
    def makeAnimation(self, data=None, fig=None, fps=None, duration=None):
        ...
        if fig == None:
            if self.fig == None:
                self.configPlot()
        ani = FuncAnimation(self.fig, self.animate, frames=data.shape[0] // skip, interval=50, fargs=(data, skip), blit=True)
        return ani