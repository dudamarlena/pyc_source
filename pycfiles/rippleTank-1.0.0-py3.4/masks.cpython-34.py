...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/rippleTank/masks.py
...
class Mask:
    ...
    def __init__(self, rippletank, rel_deep=None):
        ...
        self.mask = np.ones_like(self.rippletank.X)
...
def halfCircleMask(mask, x0, y0, r, width=1, on='x', direction='upper'):
    ...
    if on not in posible_ons:
        raise Exception("'%s' is not a valid location." % on)
    if direction not in posible_directions:
