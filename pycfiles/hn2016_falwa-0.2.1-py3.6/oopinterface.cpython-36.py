...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/hn2016_falwa/oopinterface.py
# Compiled at: 2017-11-20 23:26:14
# Size of source mod 2**32: 19270 bytes
import numpy as np
from math import pi
from scipy import interpolate

def curl_2D(ufield, vfield, clat, dlambda, dphi, planet_radius=6378000.0):
    """
    ...
    ans[1:-1, 1:-1] = (vfield[1:-1, 2:] - vfield[1:-1, :-2]) / (2.0 * dlambda) - (ufield[2:, 1:-1] * clat[2:, np.newaxis] - ufield[:-2, 1:-1] * clat[:-2, np.newaxis]) / (2.0 * dphi)
    ans[0, :] = 0.0
    ans[-1, :] = 0.0
    ans[1:-1, 0] = (vfield[:, 1] - vfield[:, -1]) / (2.0 * dlambda) - (ufield[2:, 0] * clat[2:, np.newaxis] - ufield[:-2, 0] * clat[:-2, np.newaxis]) / (2.0 * dphi)
    ans[1:-1, -1] = (vfield[:, 0] - vfield[:, -2]) / (2.0 * dlambda) - (ufield[2:, -1] * clat[2:, np.newaxis] - ufield[:-2, -1] * clat[:-2, np.newaxis]) / (2.0 * dphi)
...
class BarotropicField(object):
    ...
    def __init__(self, xlon, ylat, pv_field, area=None, dphi=None, n_partitions=None, planet_radius=6378000.0):
        ...
        *pv_field*
        ...
        if dphi is None:
            ...
            self.dphi = dphi
        if area is None:
            self.area = 2.0 * pi * planet_radius ** 2 * (np.cos(ylat[:, np.newaxis] * pi / 180.0) * self.dphi[:, np.newaxis]) / float(self.nlon) * np.ones((self.nlat, self.nlon))
        else:
            self.area = area
    ...
    def equivalent_latitudes(self):
        ...
        self.eqvlat, dummy = basis.eqvlat(ylat, pv_field, area, (self.n_partitions), planet_radius=planet_radius)
        return self.eqvlat
    ...
    def lwa(self):
        ...
        lwa_ans, dummy = basis.lwa(self.nlon, self.nlat, self.pv_field, self.eqvlat, self.planet_radius * self.clat * self.dphi)
        return lwa_ans
...
class QGField(object):
    ...
    def __init__(self, xlon, ylat, zlev, u_field, v_field=None, t_field=None, qgpv_field=None, area=None, dphi=None, n_partitions=None, rkappa=0.2858565737051793, planet_radius=6378000.0, scale_height=7000.0):
        """Create a windtempfield object.
        ...
        *n_partitions*
        ...
        if (qgpv_field is None) & (v_field is None):
            ...
            if qgpv_field is None:
                print('Compute QGPV field from u and v field.')
        if t_field:
            self.pt_field = t_field[:, :, :] * np.exp(rkappa * zlev[:, np.newaxis, np.newaxis] / scale_height)
            f_Thalf = interpolate.interp1d(zlev, self.pt_field.mean(axis=(-1)), axis=0)
            zlev_half = np.array([zlev[0] + 0.5 * (zlev[1] - zlev[0])] * i for i in range(zlev.size * 2 + 1))
            self.pt_field_half = f_Thalf(zlev_half)
            print('self.pt_field_half.shape')
            print(self.pt_field_half.shape)

    def equivalent_latitudes(self, domain_size='half_globe'):
        """
        Compute equivalent latitude with the *pv_field* stored in the object.
        ...
        for k in range(self.nlev):
            ...
            if domain_size == 'half_globe':
                ...
                qref[:nlat_s] = eqv_lat_core(ylat[:nlat_s], pv_field[:nlat_s, :], area[:nlat_s, :], nlat_s)
                pv_field_inverted = -pv_field[::-1, :]
                qref2 = eqv_lat_core(ylat[:nlat_s], pv_field_inverted[:nlat_s, :], area[:nlat_s, :], nlat_s)
                qref[-nlat_s:] = -qref2[::-1]
            else:
                if domain_size == 'full_globe':
                    qref = eqv_lat_core(ylat, pv_field, area, (self.nlat), planet_radius=planet_radius)
                else:
    ...
    def lwa(self):
        ...
        for k in range(self.nlev):
            ...
            for j in np.arange(0, self.nlat - 1):
                ...
                lwact[k, j, :] = np.sum((vort_e * vort_boo * self.planet_radius * self.clat[:, np.newaxis] * self.dphi[:, np.newaxis]),
                  axis=0)
...
def main():
    ...
    if test_2D:
        ...
        xlon = np.linspace(0, 360.0, 512, endpoint=False)
        ylat = np.linspace((-90), 90.0, 256, endpoint=True)
        nlon = xlon.size
        nlat = ylat.size
        Earth_radius = 6378000.0
        dphi = (ylat[2] - ylat[1]) * pi / 180.0
        area = 2.0 * pi * Earth_radius ** 2 * (np.cos(ylat[:, np.newaxis] * pi / 180.0) * dphi) / float(nlon) * np.ones((nlat, nlon))
        cc1 = BarotropicField(xlon, ylat, pv_field=abs_vorticity)
    ...
    if test_3D:
        ...
        clat = np.abs(np.cos(ylat * pi / 180.0))
        nlon = xlon.size
        nlat = ylat.size
        u = u_QGPV_File.variables['U'][(0, Ellipsis)]
        QGPV = u_QGPV_File.variables['QGPV'][(0, Ellipsis)]
        u_QGPV_File.close()
        print(u.shape)
        print(QGPV.shape)
        cc2 = QGField(xlon, ylat, (np.array([240.0])), u, qgpv_field=QGPV)
        cc3 = cc2.lwa()
        print('cc3 shape')
        print(cc3.shape)
