...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/hn2016_falwa/wrapper_old.py
# Compiled at: 2017-09-30 18:53:20
...
def qgpv_eqlat_lwa(ylat, vort, area, dmu, nlat_s=None, n_points=None, planet_radius=6378000.0):
    ...
    qref1 = basis.eqvlat((ylat[:nlat_s]), (vort[:nlat_s, :]), (area[:nlat_s, :]), n_points,
      planet_radius=planet_radius)
    qref[:nlat_s] = qref1
    lwa_result[:nlat_s, :], dummy = basis.lwa(nlon, nlat_s, vort[:nlat_s, :], qref1, dmu[:nlat_s])
    vort2 = -vort[::-1, :]
    qref2 = basis.eqvlat((ylat[:nlat_s]), (vort2[:nlat_s, :]), (area[:nlat_s, :]), n_points,
      planet_radius=planet_radius)
    qref[-nlat_s:] = -qref2[::-1]
