...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/hn2016_falwa/qgpv_lwa_ncforce.py
# Compiled at: 2017-08-28 08:37:59
...
def qgpv_eqlat_lwa_ncforce(ylat, vort, ncforce, area, dmu, nlat_s=None, planet_radius=6378000.0):
    """
    Input variables:
    ...
    qref[:nlat_s] = qref1
    lwa_South, ncforce_South = lwa_ncforce(nlon, nlat_s, vort[:nlat_s, :], qref[:nlat_s], ncforce[:nlat_s, :], dmu[:nlat_s])
    ...
    qref[-nlat_s:] = -qref2[::-1]
    lwa_North, ncforce_North = lwa_ncforce(nlon, nlat_s, vort[-nlat_s:, :], qref[-nlat_s:], ncforce[-nlat_s:, :], dmu[-nlat_s:])
    lwa_result[-nlat_s:, :] = lwa_North
    bigsigma_result[-nlat_s:, :] = ncforce_North
    return (
     qref, lwa_result, bigsigma_result)