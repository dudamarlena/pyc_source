...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/hn2016_falwa/theta_lwa.py
...
def theta_lwa(ylat, theta, area, dmu, nlat_s=None, planet_radius=6378000.0):
    """
    Input variables:
    ...
    qref1 = eqvlat((ylat[:nlat_s]), (theta[:nlat_s, :]), (area[:nlat_s, :]), nlat_s, planet_radius=6378000.0)
    qref[:nlat_s] = qref1
    lwa_south = lwa(nlon, nlat_s, theta[:nlat_s, :], qref1, dmu[:nlat_s])
    lwa_result[:nlat_s, :] = lwa_south
    theta2 = theta[::-1, :]
    qref2 = eqvlat((ylat[:nlat_s]), (theta2[:nlat_s, :]), (area[:nlat_s, :]), nlat_s, planet_radius=6378000.0)
    qref[-nlat_s:] = qref2[::-1]
