...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/hn2016_falwa/wrapper.py
# Compiled at: 2018-02-26 01:31:24
# Size of source mod 2**32: 24112 bytes
import numpy as np

def barotropic_eqlat_lwa(ylat, vort, area, dmu, n_points, planet_radius=6378000.0):
    """
...
def eqvlat_hemispheric(ylat, vort, area, nlat_s=None, n_points=None, planet_radius=6378000.0):
    """
    ...
    if n_points is None:
        n_points = nlat_s
    qref1, dummy = basis.eqvlat((ylat[:nlat_s]), (vort[:nlat_s, :]), (area[:nlat_s, :]), n_points,
      planet_radius=planet_radius)
    qref[:nlat_s] = qref1
    vort2 = -vort[::-1, :]
    qref2, dummy = basis.eqvlat((ylat[:nlat_s]), (vort2[:nlat_s, :]), (area[:nlat_s, :]), n_points,
      planet_radius=planet_radius)
    qref[-nlat_s:] = qref2[::-1]
    return qref


def eqvlat_bracket_hemispheric(ylat, vort, area, nlat_s=None, n_points=None, planet_radius=6378000.0, vgrad=None):
    """
    ...
    if n_points is None:
        n_points = nlat_s
    qref1, brac1 = basis.eqvlat((ylat[:nlat_s]), (vort[:nlat_s, :]), (area[:nlat_s, :]), n_points,
      planet_radius=planet_radius, vgrad=vgrad)
    qref[:nlat_s] = qref1
    vort2 = -vort[::-1, :]
    qref2, brac2 = basis.eqvlat((ylat[:nlat_s]), (vort2[:nlat_s, :]), (area[:nlat_s, :]), n_points,
      planet_radius=planet_radius, vgrad=vgrad)
    qref[-nlat_s:] = qref2[::-1]
    brac[:nlat_s] = brac1
    brac[-nlat_s:] = brac2[::-1]
...
def qgpv_eqlat_lwa(ylat, vort, area, dmu, nlat_s=None, n_points=None, planet_radius=6378000.0):
    """
...
def qgpv_eqlat_lwa_ncforce(ylat, vort, ncforce, area, dmu, nlat_s=None, n_points=None, planet_radius=6378000.0):
    """
    ...
    qref1, dummy = basis.eqvlat((ylat[:nlat_s]), (vort[:nlat_s, :]), (area[:nlat_s, :]), n_points,
      planet_radius=planet_radius)
    qref[:nlat_s] = qref1
    lwa_result[:nlat_s, :], bigsigma_result[:nlat_s, :] = basis.lwa(nlon, nlat_s, (vort[:nlat_s, :]),
      qref1,
      (dmu[:nlat_s]), ncforce=(ncforce[:nlat_s, :]))
    vort2 = -vort[::-1, :]
    qref2, dummy = basis.eqvlat((ylat[:nlat_s]), (vort2[:nlat_s, :]), (area[:nlat_s, :]), n_points,
      planet_radius=planet_radius)
    qref[-nlat_s:] = -qref2[::-1]
...
def qgpv_eqlat_lwa_options(ylat, vort, area, dmu, nlat_s=None, n_points=None, vgrad=None, ncforce=None, planet_radius=6378000.0):
    """
    Compute equivalent latitutde *qref*, local wave activity *lwa_result* and
    non-conservative force on wave activity *bigsigma_result* based on Quasi-
    geostrophic potential vorticity field *vort* at a pressure level as
    ...
    if nlat_s is None:
        ...
        if vgrad is None:
            qref1, brac = basis.eqvlat((ylat[:nlat_s]), (vort[:nlat_s, :]), (area[:nlat_s, :]), n_points,
              planet_radius=planet_radius)
        else:
            qref1, brac = basis.eqvlat((ylat[:nlat_s]), (vort[:nlat_s, :]), (area[:nlat_s, :]), n_points,
              planet_radius=planet_radius, vgrad=(vgrad[:nlat_s, :]))
    qref[:nlat_s] = qref1
    if vgrad is not None:
        ...
        vort2 = -vort[::-1, :]
        if vgrad is None:
            qref2, brac = basis.eqvlat((ylat[:nlat_s]), (vort2[:nlat_s, :]), (area[:nlat_s, :]), n_points,
              planet_radius=planet_radius)
        else:
            vgrad2 = -vgrad[::-1, :]
            qref2, brac = basis.eqvlat((ylat[:nlat_s]), (vort2[:nlat_s, :]), (area[:nlat_s, :]), n_points,
              planet_radius=planet_radius, vgrad=(vgrad2[:nlat_s, :]))
        qref[-nlat_s:] = -qref2[::-1]
        if vgrad is not None:
...
def qgpv_input_qref_to_compute_lwa(ylat, qref, vort, area, dmu, nlat_s=None, planet_radius=6378000.0):
    """
    Compute equivalent latitutde *qref* and local wave activity *lwa_result* based
    on Quasi-geostrophic potential vorticity field *vort* at a pressure level as
...
def theta_lwa(ylat, theta, area, dmu, nlat_s=None, n_points=None, planet_radius=6378000.0):
    """
    ...
    theta2 = theta[::-1, :]
    qref2, brac = basis.eqvlat((ylat[:nlat_s]), (theta2[:nlat_s, :]), (area[:nlat_s, :]), n_points,
      planet_radius=planet_radius)
    qref[-nlat_s:] = qref2[::-1]
    lwa_result[-nlat_s:, :], dummy = basis.lwa(nlon, nlat_s, theta[-nlat_s:, :], qref[-nlat_s:], dmu[-nlat_s:])
    return (
