...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/hn2016_falwa/utilities.py
# Compiled at: 2017-12-21 21:42:33
# Size of source mod 2**32: 7041 bytes
import numpy as np
...
def static_stability(height, area, theta, s_et=None, n_et=None):
    ...
    return (
     t0_n, t0_s, stat_n, stat_s)


def compute_qgpv_givenvort(omega, nlat, nlon, kmax, unih, ylat, avort, potential_temp, t0_cn, t0_cs, stat_cn, stat_cs, nlat_s=None, scale_height=7000.0):
    """
    ...
    if nlat_s == None:
        nlat_s = nlat // 2
    clat = np.cos(ylat * pi / 180.0)
    clat = np.abs(clat)
    av2 = np.empty_like(potential_temp)
    av3 = np.empty_like(potential_temp)
    qgpv = np.empty_like(potential_temp)
    av1 = np.ones((kmax, nlat, nlon)) * 2 * omega * np.sin(ylat[np.newaxis, :, np.newaxis] * pi / 180.0)
    ...
    dzdiv[1:kmax - 1, :, :] = np.exp(unih[1:kmax - 1, np.newaxis, np.newaxis] / scale_height) * (zdiv[2:kmax, :, :] - zdiv[0:kmax - 2, :, :]) / (unih[2:kmax, np.newaxis, np.newaxis] - unih[0:kmax - 2, np.newaxis, np.newaxis])
    dzdiv[0, :, :] = exp(unih[0] / scale_height) * (zdiv[1, :, :] - zdiv[0, :, :]) / (unih[(1, np.newaxis, np.newaxis)] - unih[(0, np.newaxis, np.newaxis)])
    dzdiv[kmax - 1, :, :] = exp(unih[(kmax - 1)] / scale_height) * (zdiv[kmax - 1, :, :] - zdiv[kmax - 2, :, :]) / (unih[(kmax - 1, np.newaxis, np.newaxis)] - unih[(kmax - 2, np.newaxis, np.newaxis)])
