...
# Embedded file name: build/bdist.linux-x86_64/egg/pyrpl/widgets/module_widgets/lockbox_widget.py
# Compiled at: 2017-08-29 09:44:06
__doc__ = '\nThe lockbox widget is composed of all the submodules widgets\n'
from qtpy import QtCore, QtWidgets
import pyqtgraph as pg, logging, numpy as np
...
class OutputSignalWidget(ModuleWidget):
    ...
    def init_gui(self):
        ...
        self.plot_item_phase.showGrid(y=True, x=True, alpha=1.0)
    ...
    def update_transfer_function(self):
        ...
        if max(abs_curve) > 0:
            ...
            self.curve_phase.setData(freqs, 180.0 / np.pi * np.angle(curve))
...
class LockboxInputWidget(ModuleWidget):
    ...
    def show_lock(self, stage):
        ...
        self.module._logger.debug('show_lock with sp %f, signal %f', setpoint, signal)
    ...
    def input_calibrated(self, input=None):
        ...
        input._logger.debug('Updated widget for input %s to show GUI display of expected signal (min at %f)!', input.name, input.expected_signal(0))
        return
...
class AllSignalsWidget(QtWidgets.QTabWidget):
    ...
    def __init__(self, lockbox_widget):
        ...
        self.tabBar().tabButton(0, QtWidgets.QTabBar.RightSide).resize(0, 0)
        self.tab_plus = PlusTab()
        self.addTab(self.tab_plus, '+')
        self.tabBar().tabButton(self.count() - 1, QtWidgets.QTabBar.RightSide).resize(0, 0)
    ...
    def update_transfer_function(self, output):
        if output.name in self.output_widget_names():
            self.get_output_widget_by_name(output.name).update_transfer_function()
...
class StageOutputWidget(ReducedModuleWidget):
    ...
    def init_attribute_layout(self):
        ...
        ro.setToolTip('Reset output offset value at the beginning of this stage?')
...
class LockboxStageWidget(ReducedModuleWidget):
    ...
    def create_title_bar(self):
        ...
        self.add_button.clicked.connect(lambda : self.module.parent.insert(self.module.name, self.module.setup_attributes))
...
class LockboxSequenceWidget(ModuleWidget):
    ...
    def show_widget(self):
        ...
        minimumsizehint = self.minimumSizeHint().height() + self.scrollarea.horizontalScrollBar().height()
...
class LockboxWidget(ModuleWidget):
    ...
    def init_gui(self):
        ...
        self.button_is_locked.clicked.connect(lambda : self.module.is_locked(loglevel=self.module._logger.getEffectiveLevel()))
        ...
        minimumsizehint = self.sequence_widget.minimumSizeHint().height() + self.scrollarea.horizontalScrollBar().height()
        self.scrollarea.setMinimumHeight(minimumsizehint)
    ...
    def update_lockstatus(self, islockedlist=[
 None]):
    ...
    def _is_locked_display_color(self, islocked=None):
        ...
        if module.current_state == 'sweep':
            ...
            if module.current_state == 'unlock':
                return 'darkRed'
            if islocked is None:
