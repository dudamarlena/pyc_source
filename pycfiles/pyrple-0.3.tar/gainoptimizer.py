...
# Embedded file name: build/bdist.linux-x86_64/egg/pyrpl/software_modules/lockbox/gainoptimizer.py
...
class GainOptimizerLoop(LockboxPlotLoop):
    amplitude = FloatProperty(default=0.1, doc='Amplitude of gain modulation for the estimation of the dependency of lock rms')
...
class GainOptimizer(LockboxModule):
    ...
    _setup_attributes = [
     'interval', 'amplitude', 'unity_gain_frequency', 'plot', 'measurement_time']
    ...
    def start(self):
        ...
        if self.lockbox.is_locked_and_final(loglevel=0):
            self.loop = GainOptimizerLoop(parent=self, name='gainoptimizerloop', interval=self.interval, unity_gain_frequency=self.unity_gain_frequency, amplitude=self.amplitude, plot=self.plot)
        else:
