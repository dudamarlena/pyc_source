...
# Embedded file name: build/bdist.linux-x86_64/egg/pyrpl/redpitaya.py
...
import logging, os, random, socket
...
defaultparameters = dict(hostname='', port=2222, sshport=22, user='root', password='root', delay=0.05, autostart=True, reloadserver=False, reloadfpga=True, serverbinfilename='fpga.bin', serverdirname='//opt//pyrpl//', leds_off=True, frequency_correction=1.0, timeout=1, monitor_server_name='monitor_server', silence_env=False)

class RedPitaya(object):
    cls_modules = [
     rp.HK, rp.AMS, rp.Scope, rp.Sampler, rp.Asg0, rp.Asg1] + [rp.Pwm] * 2 + [rp.Iq] * 3 + [rp.Pid] * 3 + [rp.Trig] + [rp.IIR]
    ...
    def __init__(self, config=None, **kwargs):
            ...
            for k in self.parameters.keys():
                if 'REDPITAYA_' + k.upper() in os.environ:
                    ...
                    if k == 'password':
                        oldvalue = '********'
                        newvalue = '********'
                    self.logger.debug("Variable %s with value %s overwritten by environment variable REDPITAYA_%s with value %s. Use argument 'silence_env=True' if this is not desired!", k, oldvalue, k.upper(), newvalue)

        try:
            update_with_typeconversion(self.parameters, self.c._get_or_create('redpitaya')._data)
        except BaseException as e:
            self.logger.warning('An error occured during the loading of your Red Pitaya settings from the config file: %s', e)
        ...
        if self.parameters['hostname'] is None or self.parameters['hostname'] == '':
            ...
            if gui:
                self.logger.info('Please choose the hostname of your Red Pitaya in the hostname selector window!')
        ...
        if self.parameters['hostname'] in ('_FAKE_REDPITAYA_', '_FAKE_'):
            ...
            self.logger.warning('Simulating RedPitaya because (hostname==' + self.parameters['hostname'] + '). Incomplete functionality possible. ')
            return
        else:
            if self.parameters['hostname'] in ('_NONE_', ):
                self.modules = []
                self.logger.warning('No RedPitaya created (hostname==' + self.parameters['hostname'] + '). No hardware modules are available. ')
                return
            self.start_ssh()
            if self.parameters['reloadfpga']:
                self.update_fpga()
            if self.parameters['reloadserver']:
                self.installserver()
            if self.parameters['autostart']:
                self.start()
            self.logger.info('Successfully connected to Redpitaya with hostname %s.' % self.ssh.hostname)
            self.parent = self
            return
    ...
    def start_ssh(self, attempt=0):
        ...
        if self.parameters['hostname'] == '_FAKE_REDPITAYA_':
            ...
            return True
        try:
            self.ssh = SshShell(hostname=self.parameters['hostname'], sshport=self.parameters['sshport'], user=self.parameters['user'], password=self.parameters['password'], delay=self.parameters['delay'], timeout=self.parameters['timeout'])
            self.ssh.ask()
        except BaseException as e:
            if attempt < 3:
    ...
    def update_fpga(self, filename=None):
        ...
        if source is None or not os.path.isfile(source):
            if source is not None:
                self.logger.warning('Desired bitfile "%s" does not exist. Using default file.', source)
            source = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'fpga', 'red_pitaya.bin')
        if not os.path.isfile(source):
            raise IOError('Wrong filename', 'The fpga bitfile was not found at the expected location. Try passing the arguments dirname="c://github//pyrpl//pyrpl//" adapted to your installation directory of pyrpl and filename="red_pitaya.bin"! Current dirname: ' + self.parameters['dirname'] + ' current filename: ' + self.parameters['filename'])
        for i in range(3):
            try:
                self.ssh.scp.put(source, os.path.join(self.parameters['serverdirname'], self.parameters['serverbinfilename']))
        ...
        self.ssh.ask('cat ' + os.path.join(self.parameters['serverdirname'], self.parameters['serverbinfilename']) + ' > //dev//xdevcfg')
        sleep(self.parameters['delay'])
        ...
        self.ssh.ask('systemctl start redpitaya_nginx')
        sleep(self.parameters['delay'])
        self.ssh.ask('ro')
        return
    ...
    def fpgarecentlyflashed(self):
        self.ssh.ask()
        result = self.ssh.ask('echo $(($(date +%s) - $(date +%s -r "' + os.path.join(self.parameters['serverdirname'], self.parameters['serverbinfilename']) + '")))')
        age = None
        ...
        if not age:
            self.logger.debug('Could not retrieve bitfile age from: %s', result)
            return False
        else:
            if age > 10:
    ...
    def installserver(self):
        ...
        for serverfile in ['monitor_server', 'monitor_server_0.95']:
            sleep(self.parameters['delay'])
            try:
                self.ssh.scp.put(os.path.join(os.path.abspath(os.path.dirname(__file__)), 'monitor_server', serverfile), self.parameters['serverdirname'] + self.parameters['monitor_server_name'])
            except (SCPException, SSHException):
    ...
    def startserver(self):
        ...
        if self.fpgarecentlyflashed():
            ...
            sleep(2.0)
        result = self.ssh.ask(self.parameters['serverdirname'] + '/' + self.parameters['monitor_server_name'] + ' ' + str(self.parameters['port']))
        if 'sh' not in result:
            self.logger.debug('Server application started on port %d', self.parameters['port'])
    ...
    def startclient(self):
        self.client = redpitaya_client.MonitorClient(self.parameters['hostname'], self.parameters['port'], restartserver=self.restartserver)
        self.makemodules()
    ...
    def make_a_slave(self, port=None, monitor_server_name=None, gui=False):
        ...
        slaveparameters.update(dict(port=port, autostart=True, reloadfpga=False, reloadserver=False, monitor_server_name=monitor_server_name, silence_env=True))
        r = RedPitaya(**slaveparameters)
        r._master = self
