...
# Embedded file name: build/bdist.linux-x86_64/egg/pyrpl/software_modules/software_pid.py
...
class RunningProperty(LedProperty):
    ...
    def start(self, obj):
        ...
        """
        self.stop(obj)
        obj.loop = SoftwarePidLoop(parent=obj, name='loop', interval=obj.interval, plot=True, plotter='plotter')
...
class SoftwarePidController(Module):
    ...
    plotter = Plotter(legend='error (red, V) and output (green, V)')
