...
# Embedded file name: build/bdist.linux-x86_64/egg/pyrpl/widgets/pyrpl_widget.py
...
class ExceptionLauncher(QtCore.QObject):
    show_exception = QtCore.Signal(list)
...
class MyDockWidget(QtWidgets.QDockWidget):
    ...
    def __init__(self, create_widget_func, name):
        ...
        self.setFeatures(QtWidgets.QDockWidget.DockWidgetFloatable | QtWidgets.QDockWidget.DockWidgetMovable | QtWidgets.QDockWidget.DockWidgetVerticalTitleBar | QtWidgets.QDockWidget.DockWidgetClosable)
...
class PyrplWidget(QtWidgets.QMainWindow):
    ...
    def __init__(self, pyrpl_instance):
        ...
        self.centralbutton = QtWidgets.QPushButton('Click on "Modules" in the upper left corner to load a specific PyRPL module!')
    ...
    def save_window_position(self):
        if self.isVisible():
            ...
            if 'dock_positions' not in self.parent.c.pyrpl._keys() or self.parent.c.pyrpl['dock_positions'] != act_state:
    ...
    def set_window_position(self):
        if 'dock_positions' in self.parent.c.pyrpl._keys():
            try:
                ...
                self.logger.warning('Sorry, there was a problem with the restoration of Dock positions. ')
