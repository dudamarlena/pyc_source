...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpRenamer/core/renamer.py
# Compiled at: 2019-12-22 00:17:48
# Size of source mod 2**32: 17639 bytes
"""
...
class Renamer(window.MainWindow, object):
    ...
    def __init__(self):
        super(Renamer, self).__init__(name='RenamerBaseWindow',
          title='Renamer',
          size=(350, 700),
          fixed_size=False,
          auto_run=True,
...
class RenamerWidget(base.BaseWidget, object):
    ...
    def _setup_categories(self):
        ...
        if not self._config:
            tpRenamer.logger.warning('Impossible to setup categories because tpRenamer configuration file is not available!')
            return
        categories = self._config.get('categories', default=(list()))
        if not categories:
            tpRenamer.logger.warning('Impossible to setup categories because categories property is not defined in tpRenamer configuration file!')
            return
        ...
        if self.rename_tab.currentIndex() == 0:
            ...
            for item in items:
                ...
                if generate_preview_name:
                    ...
                    preview_name = self._find_manual_available_name(items, base_name, prefix=prefix, side=side, suffix=suffix, index=index,
                      padding=padding,
                    ...
                    while preview_name in generated_names:
                        duplicated_names[base_name] += 1
                        preview_name = self._find_manual_available_name(items, base_name, prefix=prefix, side=side, suffix=suffix, index=(duplicated_names[base_name]),
                          padding=padding,
                          letters=naming_method,
                          capital=upper,
                          joint_end=joint_end,
                ...
                if hasattr(item, 'handle'):
                    if tp.is_maya():
                        ...
                        try:
                            ...
                            tpRenamer.logger.warning('Impossible to rename: {} > {} | {}'.format(full_name, item.preview_name, exc))
