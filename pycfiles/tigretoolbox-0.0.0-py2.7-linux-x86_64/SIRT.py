...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tigre/Algorithms/SIRT.py
# Compiled at: 2017-06-22 08:02:00
...
def SIRT(proj, geo, alpha, niter, lmbda=1, lmbda_red=0.99, OrderStrategy=None, Quameasopts=None, init=None, verbose=True, noneg=True):
    """
    ...
    if type(init) == np.ndarray:
        ...
        for i in range(niter):
            ...
            res += lmbda * 1 / V * Atb(W * (proj - Ax(res, geo, alpha, 'ray-voxel')), geo, alpha, 'FDK')
            if noneg:
                res = res.clip(min=0)
            if Quameasopts != None:
