...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tigre/Utilities/filtering.py
...
def filtering(proj, geo, angles, parker):
    ...
    for i in range(len(angles)):
        ...
        proj[i] = np.transpose(fproj[end / 2 - geo.nDetector[0] / 2:end / 2 + geo.nDetector[0] / 2, :] / 2 / geo.dDetector[0] * (2 * np.pi / len(angles)) / 2 * (geo.DSD / geo.DSO))
