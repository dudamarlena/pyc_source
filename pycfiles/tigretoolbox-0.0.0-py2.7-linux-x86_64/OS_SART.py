...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tigre/Algorithms/OS_SART.py
# Compiled at: 2017-06-22 08:01:20
...
def OS_SART(proj, geo, alpha, niter, blocksize=20, lmbda=1, lmbda_red=0.99, OrderStrategy=None, Quameasopts=None, init=None, verbose=True, noneg=True, computel2=False):
    """
        ...
        for i in range(niter):
            ...
            for j in range(len(angleblocks)):
                if blocksize == 1:
                    ...
                    sumax = 3
                    dim_exp = False
                res += lmbda * 1 / np.array(np.sum(np.expand_dims(V[:, :, angle_index[j]], axis=0), axis=sumax), dtype=np.float32) * Atb(adddim(W[:, :, angle_index[j]], dim_exp) * (adddim(proj[:, :, angle_index[j]], dim_exp) - Ax(res, geo, angle, 'ray-voxel')), geo, angle, 'FDK')
                if noneg:
                    res = res.clip(min=0)
