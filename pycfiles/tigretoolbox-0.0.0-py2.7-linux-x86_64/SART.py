...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tigre/Algorithms/SART.py
# Compiled at: 2017-06-22 08:01:40
...
def SART(proj, geo, alpha, niter, lmbda=1, lmbda_red=0.99, OrderStrategy=None, Quameasopts=None, init=None, verbose=True, noneg=True):
    """
    ...
    if type(init) == np.ndarray:
        ...
        for i in range(niter):
            ...
            for j in range(len(alpha)):
                ...
                res += lmbda * 1 / V[:, :, angle_index[j]] * Atb(np.expand_dims(W[:, :, j], axis=2) * (np.expand_dims(proj[:, :, angle_index[j]], axis=2) - Ax(res, geo, angle, 'ray-voxel')), geo, angle, 'FDK')
                if noneg:
                    res = res.clip(min=0)
