...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/spackl/file/csv.py
# Compiled at: 2019-03-09 00:01:34
"""
    Class for reading data from a CSV or CSV-like object
"""
import io, logging, six, zipfile
...
class CSV(BaseFile):
    ...
    def _set_file(self, file_path_or_obj):
        ...
        if hasattr(file_path_or_obj, 'open') or hasattr(file_path_or_obj, 'read'):
            ...
            if not file.exists():
                ...
                file = None
            elif zipfile.is_zipfile(str(file)) and not self._use_pandas:
    ...
    def _get_dialect(self):
        dialect = Sniffer().sniff(self._data.read(1048576))
        self._data.seek(0)
        return dialect
