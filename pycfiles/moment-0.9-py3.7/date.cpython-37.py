...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/moment/date.py
...
class MutableDate(object):
    __doc__ = 'Incapsulate mutable dates in one class.'
    ...
    def add(self, key=None, amount=None, **kwds):
        ...
        if not key:
            ...
            if key == 'hours' or key == 'hour':
                ...
                if key == 'minutes' or key == 'minute':
                    self._date += timedelta(minutes=amount)
        if key == 'seconds' or key == 'second':
            self._date += timedelta(seconds=amount)
        else:
            if key == 'milliseconds' or key == 'millisecond':
                self._date += timedelta(milliseconds=amount)
            else:
                if key == 'microseconds' or key == 'microsecond':
