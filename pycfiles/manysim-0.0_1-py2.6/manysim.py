...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/manysim.py
# Compiled at: 2011-05-23 18:22:23
import os, time, urllib, itertools, tarfile, pickle, multiprocessing, boto
...
class Configuration(object):
    ...
    def __init__(self, **kwargs):
        valid_kwargs = [
         'key_name', 'region', 'job_archive', 'file_manifest',
        ...
        for (k, v) in kwargs.iteritems():
            assert k in valid_kwargs or k in required_kwargs, 'Invalid keyword argument %s' % k
            setattr(self, k, v)
...
class Store(object):
    ...
    def __init__(self, configuration):
        ...
        self._conn = boto.connect_s3(aws_access_key_id=self._config.access_key, aws_secret_access_key=self._config.secret_key)
        self._bucket = self._conn.get_bucket(self._config.bucket_name)

    def push(self, local_target, remote_target, overwrite=False):
        assert os.path.exists(local_target), 'Local target file does not exist at %s' % local_target
        key = self._bucket.get_key(remote_target)
        if key is None:
            key = self._bucket.new_key(remote_target)
        else:
            assert overwrite is True, 'Remote target file exist, overwrite=True to force.'
        key.set_contents_from_file(file(local_target, 'r'))
        return

    def pull(self, remote_target, local_target, overwrite=False):
        if overwrite is False:
            assert not os.path.exists(local_target), 'Local target file exist, overwrite=True to force.'
        key = self._bucket.get_key(remote_target)
        assert key is not None, 'Remote target does not exist at %s' % remote_target
...
class Instance(object):
    ...
    def __init__(self, configuration):
        ...
        self._conn = boto.connect_ec2(aws_access_key_id=self._config.access_key, aws_secret_access_key=self._config.secret_key)
        return
    ...
    def _spot(self):
        assert hasattr(self._config, 'price'), 'Must set maximum spot price Configuration.spot'
        self._instances = self.conn.request_spot_instances(self._config.spot_price, self._config.instance_image, count=1, key_name=self._config.key_name, user_data=self._user_data, instance_type=self._config.instance_type)

    def _on_demand(self):
        self._reservation = self._conn.run_instances(self._config.instance_image, max_count=1, key_name=self._config.key_name, user_data=self._user_data, instance_type=self._config.instance_type)
...
class JobMaster(object):
    ...
    def run(self):
        ...
        output = pool.imap_unordered(job.run, itertools.repeat(self._config.job_config, self._config.pool_size))
        pool.close()
        pool.join()
