...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\loutilities\geo.py
# Compiled at: 2017-12-24 04:46:57
...
def elevation_gain(elevations, isMiles=True, upthreshold=8, downthreshold=8, debug=False):
    ...
    if debug:
        debuginfo = [
         'ele,state,highup,lowdown,totclimb\n']
    else:
        state = 'unknown'
        thisel = elevations[0]
        highup = thisel
        ...
        for thisel in elevations[1:]:
            if state == 'unknown':
                if thisel >= highup + upthreshold:
                    ...
                    if thisel <= lowdown - downthreshold:
                        state = 'descending'
                        lowdown = thisel
                    else:
                        if state == 'climbing':
