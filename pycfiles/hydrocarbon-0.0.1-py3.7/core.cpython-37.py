...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hydrocarbon/core.py
# Compiled at: 2019-04-08 11:58:28
# Size of source mod 2**32: 13680 bytes
"""
...
"""
import base64, re, ipaddress, json, logging, hashlib, urllib.parse
...
class FeedGenerator:
    ...
    def _convert_image(self, filename, optimum_width=0, optimum_height=0):
        with Image.open(filename) as (img):
            ...
            if optimum_width:
                if width != optimum_width:
                    self._log.warning('Width of image %s does not match recommended %d', filename, optimum_width)
            if optimum_height:
                if height != optimum_height:
                    self._log.warning('Height of image %s does not match recommended %d', filename, optimum_height)
            output = BytesIO()
    ...
    def _init_regex(self):
        ...
        """
        self._regex_md5 = re.compile('[0-9a-f]{32}', re.IGNORECASE)
        self._regex_dns = re.compile('((xn--)?[a-z0-9]+(-[a-z0-9]+)*\\.)+[a-z]{2,}', re.IGNORECASE)
        ...
        if self._git_enabled:
            if repo:
                ...
                if relative_name in repo.untracked_files or relative_name in changed:
                    self._log.warning('warning: file is untracked or modified: %s', relative_name)
                    if self._git_strict:
                        self.errors.append('Untracked or modified file: {}'.format(relative_name))
                else:
        ...
        try:
            if query['type'] in valid_types:
                ...
                if search.startswith('q='):
                    self._log.warning('Search query starts with q=, this is not necessary and should be removed')
                    search = search[2:]
    ...
    def _parse_file(self, filename, current_dir, repo=None):
        ...
        if not 'query' in data:
            ...
            return data['meta'].get('enabled', True) or report
        if 'iocs' in data:
            indicators = self._extract_indicators(data['iocs'], report)
        elif 'query' in data:
            if indicators:
                self._log.error('This report already has IOCs, cannot also have a search query')
            else:
