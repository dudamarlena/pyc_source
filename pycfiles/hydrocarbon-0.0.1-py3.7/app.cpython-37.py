...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hydrocarbon/app.py
# Compiled at: 2019-04-08 11:57:40
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate Carbon Black alliance feed from a collection of YAML files')
    ...
    parser.add_argument('--output',
      ...
      default='output.json')
    parser.add_argument('--icon-large',
      type=str,
      help='large icon, recommended 370x97 (optional)',
      default=None,
      required=False)
    parser.add_argument('--icon-small',
      type=str,
      help='small icon, recommended 100x100 (optional)',
      default=None,
      required=False)
    parser.add_argument('--git-enabled',
      help='enable git support (default: on)',
      default=True,
      action='store_true')
    parser.add_argument('--git-strict',
      help='error if files in git are untracked or modified (default: off)',
      default=False,
      action='store_true')
    args = parser.parse_args()
    builder = FeedGenerator((args.config),
      use_git=(args.git_enabled), git_strict=(args.git_strict))
    if args.icon_large:
        if args.icon_small:
