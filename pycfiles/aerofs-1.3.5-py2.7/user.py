...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/aerofs/sdk/user.py
...
class User(APIObject):
    ...
    def from_json(self, json):
        ...
        if 'shares' in json:
            ...
            self._shares = frozenset([ SharedFolder(self.api).from_json(f) for f in json['shares']
                                     ])
        if 'invitations' in json:
            from .invitation import Invitation
            self._invitations = frozenset([ Invitation(self.api, self.email).from_json(f) for f in json['invitations']
                                          ])
        return self
    ...
    def load_devices(self):
        ...
        self._devices = frozenset([ Device(self.api).from_json(d) for d in data
                                  ])
    ...
    def save_two_factor(self):
        ...
        raise NoRouteException('Can not enable two-factor authentication from API.')
