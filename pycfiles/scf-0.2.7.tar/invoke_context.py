...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-MsePIp/scf/tcfcli/cmds/native/common/invoke_context.py
# Compiled at: 2019-12-02 05:04:19
import json, sys, os, click, subprocess, threading, traceback
...
class InvokeContext(object):
    BOOTSTRAP_SUFFIX = {MacroRuntime.node610: 'bootstrap.js', 
       MacroRuntime.node89: 'bootstrap.js', 
       MacroRuntime.python27: 'bootstrap.py', 
       MacroRuntime.python36: 'bootstrap.py'}
    _thread_err_msg = ''

    def __init__(self, template_file, function=None, namespace=None, env_file=None, debug_port=None, debug_args='', event='{}', is_quiet=False):
    ...
    def cmd(self):
        if self._debug_context.cmd is not None:
            ...
            if self._runtime.runtime == 'python3.6' and UserConfig().python3_path.upper() != 'NONE':
                return UserConfig().python3_path
            else:
                if self._runtime.runtime == 'python2.7' and UserConfig().python2_path.upper() != 'NONE':
    ...
    def argv(self):
        ...
        bootstrap = os.path.join(runtime_pwd, 'runtime', self._runtime.runtime, self.BOOTSTRAP_SUFFIX[self._runtime.runtime])
        ...
        code = os.path.normpath(os.path.join(os.path.dirname(os.path.abspath(self._template_file)), self._runtime.codeuri))
