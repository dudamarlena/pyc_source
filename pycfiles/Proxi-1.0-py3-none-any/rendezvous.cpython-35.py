...
# Compiled at: 2017-01-24 22:48:37
# Size of source mod 2**32: 6111 bytes
__doc__ = 'Provides node selection based on Highest Random Weight (HRW) hashing.'
from __future__ import absolute_import
import enum, hashlib, random, struct, sys, typing, siphash, six, proxenos.config, proxenos.node
try:
    import mmh3
except ImportError:
    err = RuntimeError('Please install the mmh3 package from PyPI for Murmur3 support')
...
__all__ = ('HashMethod', 'KeyType', 'select_node', 'weight')
...
def select_node(cluster, key, hash_method=None, **hash_options):
    ...
    selected_node = None
    for node in cluster:
        node_weight = weight(socket_address=node.socket_address, key=key, hash_method=hash_method, **hash_options)
...
def weight(socket_address, key, hash_method=None, **hash_options):
