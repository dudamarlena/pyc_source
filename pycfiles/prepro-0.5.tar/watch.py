...
# Embedded file name: /Users/jsick/.virtualenvs/paperweight/lib/python2.7/site-packages/preprint/watch.py
...
class Watch(Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument('--exts', nargs='*', default=self.app.confs.config('exts'), help='File extensions to look for')
        parser.add_argument('--cmd', default=self.app.confs.config('cmd'), help='Command to run on changes')
        parser.add_argument('--diff', nargs='?', const='HEAD', default=None, help='Typeset diff against git commit')
        return parser

    def take_action(self, parsed_args):
        ignore = (
         os.path.splitext(self.app.options.master)[0] + '.pdf',
         'build', '_current.tex', '_prev.tex')
        if parsed_args.diff is None:
            handler = RegularChangeHandler(parsed_args.cmd, parsed_args.exts, ignore)
        else:
            handler = DiffChangeHandler(self.app.options.master, parsed_args.diff, parsed_args.exts, ignore)
        self._watch(handler)
        return
...
class BaseChangeHandler(FileSystemEventHandler):
    ...
    def on_any_event(self, event):
        ...
        if event.is_directory:
            return
        event_ext = os.path.splitext(event.src_path)[(-1)].lower().lstrip('.')
...
class DiffChangeHandler(BaseChangeHandler):
    ...
    def __init__(self, master_path, prev_commit, exts, ignores):
        ...
        self._output_name = ('{0}_diff').format(os.path.splitext(self._master)[0])
        self._ignores = list(ignores)
        self._ignores.append(self._output_name)

    def run_compile(self):
        """Run a latexdiff+compile."""
        git_diff_pipeline(self._output_name, self._master, self._prev_commit)