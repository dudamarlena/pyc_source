...
# Embedded file name: /Library/Python/2.6/site-packages/akamu/demo/web_acl.py
# Compiled at: 2013-03-20 13:07:58
import sys
...
def CreateRepozeMiddleWare(app):
    ...
    return PluggableAuthenticationMiddleware(app, identifiers, authenticators, challengers, mdproviders, default_request_classifier, default_challenge_decider, log_stream=log_stream, log_level=logging.DEBUG)
...
def service_1(environ, start_response):
    return service_stub(environ, start_response, [
     'POST', 'GET'])


@service(SERVICE_ID_PREFIX + '2', 'service.2', wsgi_wrapper=CreateRepozeMiddleWare)
@web_acl('mysqlDataset', accessMap={'POST': WEB_ACL_NS.Append}, simple_service=False)
def service_2(environ, start_response):
    return service_stub(environ, start_response, [
     'POST', 'GET', 'PUT'])


@simple_service('GET', SERVICE_ID_PREFIX + '3', 'service.3', 'text/plain', wsgi_wrapper=CreateRepozeMiddleWare)
