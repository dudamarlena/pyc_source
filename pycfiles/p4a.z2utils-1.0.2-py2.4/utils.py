...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/p4a/z2utils/utils.py
...
def persist_five_components(context, product_name):
    ...
    """
    portal_url = cmfutils.getToolByName(Acquisition.aq_inner(context), 'portal_url')
    portal = portal_url.getPortalObject()
    qi = portal.portal_quickinstaller
    try:
        ip = qi[product_name]
    except KeyError, e:
        raise KeyError('No product installed with the name "%s"' % str(product_name))

    if 'utilities' in getattr(ip, 'portalobjects', []):
        portalobjects = [ x for x in ip.portalobjects if x != 'utilities' ]
