...
# [GCC 8.4.0]
# Embedded file name: /home/leaf/working/apocrypha/test/test_server.py
# Compiled at: 2018-07-24 00:15:01
# Size of source mod 2**32: 13046 bytes
import threading, unittest, apocrypha.client
...
class TestServerBase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        """
        TestServerBase.database = ServerDatabase('test/test-db.json', stateless=True)
        host, port = '0.0.0.0', PORT
        TestServerBase.server = Server((
         host, port), ServerHandler, TestServerBase.database, quiet=True)
        TestServerBase.server_thread = threading.Thread(target=TestServerBase.server.serve_forever)
...
class TestServer(TestServerBase):
    ...
    def test_cache_deep_hit(self):
        ...
        query(['a', 'b', 'c', 'd', 'e', '=', 'f'])
        query(['a', 'b', 'c', 'd', 'e'])
        self.assertIn(('a', 'b', 'c', 'd', 'e'), TestServer.database.cache)
    ...
    def test_cache_doesnt_effect_sibling(self):
        ...
        self.assertEqual(client.get('one layer', 'two layer'), 'cake')
        self.assertEqual(client.get('one layer', 'apple layer'), 'sauce')
        self.assertEqual(client.get('one layer'), {'two layer': 'cake', 'apple layer': 'sauce'})
    ...
    def test_get_string(self):
        self.assertEqual(TestServer.db.get('green'), 'nice')
        self.assertEqual(TestServer.db.get('octopus', 'legs'), 8)

    def test_get_list(self):
        self.assertEqual(TestServer.db.get('animals'), [
         'wolf', 'octopus', 'bird'])
    ...
    def test_get_non_existant(self):
        self.assertEqual(TestServer.db.get('yahoo', 'foobar'), None)
    ...
    def test_get_default(self):
        ...
        respond with
        """
        self.assertEqual(TestServer.db.get('yahoo', 'foobar', default={}), {})
        self.assertEqual(TestServer.db.get('yahoo', 'foobar', default=[]), [])
        self.assertEqual(TestServer.db.get('yahoo', 'foobar', default='abc'), 'abc')
    ...
    def test_get_cast_to_list(self):
        self.assertEqual(TestServer.db.get('green', cast=list), [
         'nice'])

    def test_get_cast_to_str(self):
        self.assertEqual(TestServer.db.get('animals', cast=str), "['wolf', 'octopus', 'bird']")

    def test_get_cast_to_set(self):
        self.assertEqual(TestServer.db.get('animals', cast=set), {
         'wolf', 'octopus', 'bird'})
    ...
    def test_keys(self):
        self.assertEqual(TestServer.db.keys('octopus'), ['legs'])

    def test_keys_non_existant(self):
        self.assertEqual(TestServer.db.keys('does not exist', 'foobar'), [])
    ...
    def test_remove(self):
        ...
        self.assertEqual(TestServer.db.get('test list'), [
         'b', 'c'])

    def test_remove_list(self):
        TestServer.db.set('test list', value=['a', 'b', 'c'])
        TestServer.db.remove('test list', value=['a', 'b'])
        self.assertEqual(TestServer.db.get('test list'), 'c')
    ...
    def test_append(self):
        ...
        self.assertEqual(TestServer.db.get('test list'), 'apple')
    ...
    def test_delete(self):
        TestServer.db.set('test item', value='hello')
        self.assertEqual(TestServer.db.get('test item'), 'hello')
        TestServer.db.delete('test item')
        self.assertEqual(TestServer.db.get('test item'), None)
    ...
    def test_apply(self):
        ...
        self.assertEqual(sorted(TestServer.db.get('list')), sorted(['a', 'b', 'c']))

    def test_query(self):
        self.assertEqual(apocrypha.client.query(['non', 'existant', '--keys']), [])
