...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/navhip/navhipComm.py
# Compiled at: 2020-02-12 12:43:49
"""
*******************************************************************************
...
class HIDDongleHIDAPI(Dongle, DongleWait):
    ...
    def exchange(self, apdu, timeout=20000):
        ...
        if self.ledger:
            apdu = wrapCommandAPDU(257, apdu, 64)
        padSize = len(apdu) % 64
        tmp = apdu
        if padSize != 0:
        ...
        if not self.ledger:
            ...
            self.device.set_nonblocking(False)
            while True:
                response = unwrapResponseAPDU(257, result, 64)
...
def getDongle(debug=False):
    ...
    if HID:
        for hidDevice in hid.enumerate(0, 0):
            if hidDevice['vendor_id'] == 9601 and hidDevice['product_id'] == 11132:
                hidDevicePath = hidDevice['path']
            if hidDevice['vendor_id'] == 9601 and hidDevice['product_id'] == 15228:
                hidDevicePath = hidDevice['path']
                ledger = True
            if hidDevice['vendor_id'] == 9601 and hidDevice['product_id'] == 19324:
                hidDevicePath = hidDevice['path']
                ledger = True
            if hidDevice['vendor_id'] == 11415:
                if 'interface_number' in hidDevice and hidDevice['interface_number'] == 0 or 'usage_page' in hidDevice and hidDevice['usage_page'] == 65440:
                    hidDevicePath = hidDevice['path']
                    ledger = True
            if hidDevice['vendor_id'] == 9601 and hidDevice['product_id'] == 6151:
                hidDevicePath = hidDevice['path']
