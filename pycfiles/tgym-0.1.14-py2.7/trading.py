...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/tgym/envs/trading.py
# Compiled at: 2017-07-10 04:06:18
...
class SpreadTrading(Env):
    ...
    """
    _actions = {'hold': np.array([1, 0, 0]), 
       'buy': np.array([0, 1, 0]), 
       'sell': np.array([0, 0, 1])}
    _positions = {'flat': np.array([1, 0, 0]), 
       'long': np.array([0, 1, 0]), 
    ...
    def step(self, action):
        ...
        if all(action == self._actions['buy']):
            ...
            if all(self._position == self._positions['flat']):
                ...
                self._entry_price = calc_spread(self._prices_history[(-1)], self._spread_coefficients)[1]
            elif all(self._position == self._positions['short']):
                self._exit_price = calc_spread(self._prices_history[(-1)], self._spread_coefficients)[1]
                instant_pnl = self._entry_price - self._exit_price
                self._position = self._positions['flat']
                self._entry_price = 0
        else:
            if all(action == self._actions['sell']):
                reward -= self._trading_fee
                if all(self._position == self._positions['flat']):
                    self._position = self._positions['short']
                    self._entry_price = calc_spread(self._prices_history[(-1)], self._spread_coefficients)[0]
                elif all(self._position == self._positions['long']):
                    self._exit_price = calc_spread(self._prices_history[(-1)], self._spread_coefficients)[0]
    ...
    def render(self, savefig=False, filename='myfig'):
        ...
        plt.suptitle('Cumulated Reward: ' + '%.2f' % self._total_reward + ' ~ ' + 'Cumulated PnL: ' + '%.2f' % self._total_pnl + ' ~ ' + 'Position: ' + ['flat', 'long', 'short'][list(self._position).index(1)] + ' ~ ' + 'Entry Price: ' + '%.2f' % self._entry_price)
    ...
    def _get_observation(self):
        ...
        """
        return np.concatenate([ prices for prices in self._prices_history[-self._history_length:] ] + [
