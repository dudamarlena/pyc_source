...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cuteSV/cuteSV_resolveINV.py
...
def resolution_INV(path, chr, svtype, read_count, max_cluster_bias, sv_size, bam_path, action, MaxSize, gt_round):
    """
        cluster INV
        ************************************************************************
        path:   INV.sigs
        ...
        if breakpoint_1_in_read - semi_inv_cluster[(-1)][0] > max_cluster_bias or breakpoint_2_in_read - semi_inv_cluster[(-1)][1] > max_cluster_bias or strand != semi_inv_cluster[(-1)][(-1)]:
            if len(semi_inv_cluster) >= read_count:
                if semi_inv_cluster[(-1)][0] == semi_inv_cluster[(-1)][1] == 0:
                    ...
                    generate_semi_inv_cluster(semi_inv_cluster, chr, svtype, read_count, sv_size, candidate_single_SV, max_cluster_bias, bam_path, action, MaxSize, gt_round)
            semi_inv_cluster = []
...
def generate_semi_inv_cluster(semi_inv_cluster, chr, svtype, read_count, sv_size, candidate_single_SV, max_cluster_bias, bam_path, action, MaxSize, gt_round):
            ...
            if temp_count >= read_count:
                ...
                if inv_len >= sv_size and max_count_id >= read_count:
                    if inv_len <= MaxSize:
                        if action:
                            import time
                            DV, DR, GT, GL, GQ, QUAL = call_gt(bam_path, int(breakpoint_1), int(breakpoint_2), chr, list(temp_id.keys()), max_cluster_bias, gt_round)
                        else:
                            DR = '.'
                            GT = './.'
                            GL = '.,.,.'
