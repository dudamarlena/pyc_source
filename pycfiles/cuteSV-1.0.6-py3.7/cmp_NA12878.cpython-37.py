...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/benchmarks/cmp_NA12878.py
        ...
        for ele in hc['DEL'][chr]:
            if chr in pac['DEL']:
                for i in pac['DEL'][chr]:
                    if not ele[0] - offect <= i[0] <= ele[1] + offect:
                        if not ele[0] - offect <= i[2] <= ele[1] + offect:
                            if i[0] - offect <= ele[0] <= i[2] + offect:
                                pass
