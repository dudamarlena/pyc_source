...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/benchmarks/eva_trio.py
# Compiled at: 2020-04-17 02:15:05
# Size of source mod 2**32: 15021 bytes
...
def parse_BND(tag):
    if len(tag.split('[')) == 1:
        return (
         tag.split(']')[1].split(':')[0], int(tag.split(']')[1].split(':')[1]))
                ...
                if chr not in call_B[svtype]:
                    ...
                    for i in call_A[svtype][chr]:
                        for j in call_B[svtype][chr]:
                            if not i[0] - offect <= j[0] <= i[2] + offect:
                                if not i[0] - offect <= j[2] <= i[2] + offect:
                                    if j[0] - offect <= i[0] <= j[2] + offect:
                                        pass
...
USAGE = '\tEvaluate SV callset generated by cuteSV\n'
