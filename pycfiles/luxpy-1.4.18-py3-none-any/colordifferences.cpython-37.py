...
# [GCC 8.4.0]
# Embedded file name: D:\Documents\GitHub\luxpy\luxpy\color\deltaE\colordifferences.py
# Compiled at: 2019-10-24 08:26:38
# Size of source mod 2**32: 17409 bytes
"""
...
__all__ = [
 'deltaH', 'DE_camucs', 'DE2000', 'DE_cspace']
...
def _process_DEi(DEi, DEtype='jab', avg=None, avg_axis=0, out='DEi'):
    ...
    if DEtype == 'jab':
        ...
        if DEtype == 'ab':
            ...
            if DEtype == 'j':
                DEi = np.sqrt(DEi[0])
            elif (avg is not None) & ('DEa' in out.split(',')):
                if isinstance(DEi, tuple):
                    DEa = (
                     avg((DEi[0]), axis=avg_axis, keepdims=True), avg((DEi[1]), axis=avg_axis, keepdims=True))
                else:
...
def DE_camucs(xyzt, xyzr, DEtype='jab', avg=None, avg_axis=0, out='DEi', xyzwt=cam._CAM_02_X_DEFAULT_WHITE_POINT, xyzwr=cam._CAM_02_X_DEFAULT_WHITE_POINT, Ywt=None, conditionst=cam._CAM_02_X_DEFAULT_CONDITIONS, Ywr=None, conditionsr=cam._CAM_02_X_DEFAULT_CONDITIONS, camtype=cam._CAM_02_X_DEFAULT_TYPE, ucstype='ucs', mcat=None, outin='J,aM,bM', yellowbluepurplecorrect=False, **kwargs):
    """
    ...
    Returns:
        :returns: 
            | ndarray with DEi [, DEa] or other as specified by :out:
    """
    jabt = cam.camucs_structure(xyzt, xyzw=xyzwt, Yw=Ywt, conditions=conditionst, camtype=camtype,
      ucstype=ucstype,
      mcat=mcat,
      direction='forward',
      outin=outin,
      yellowbluepurplecorrect=yellowbluepurplecorrect)
    jabr = cam.camucs_structure(xyzr, xyzw=xyzwr, Yw=Ywr, conditions=conditionsr, camtype=camtype,
      ucstype=ucstype,
      mcat=mcat,
      direction='forward',
      outin=outin,
    ...
    DEi = (
     (((jabt[..., 0:1] - jabr[..., 0:1]) / KL) ** 2).sum(axis=(jabt[..., 0:1].ndim - 1), keepdims=True),
...
def DE2000(xyzt, xyzr, dtype='xyz', DEtype='jab', avg=None, avg_axis=0, out='DEi', xyzwt=None, xyzwr=None, KLCH=None):
    """
    ...
    Cavg = (Ct + Cr) / 2
    G = 0.5 * (1 - np.sqrt(Cavg ** 7.0 / (Cavg ** 7.0 + 6103515625.0)))
    apt = (1 + G) * at
    apr = (1 + G) * ar
    ...
    T = 1 - 0.17 * np.cos((hp - 30) * np.pi / 180) + 0.24 * np.cos(2 * hp * np.pi / 180) + 0.32 * np.cos((3 * hp + 6) * np.pi / 180) - 0.2 * np.cos((4 * hp - 63) * np.pi / 180)
    dtheta = 30 * np.exp(-((hp - 275) / 25) ** 2)
    RC = 2 * np.sqrt(Cp ** 7 / (Cp ** 7 + 6103515625))
    ...
    kL, kC, kH = KLCH
    DEi = (
     (dL / (kL * SL)) ** 2, (dCp / (kC * SC)) ** 2 + (dH / (kH * SH)) ** 2 + RT * (dCp / (kC * SC)) * (dH / (kH * SH)))
    return _process_DEi(DEi, DEtype=DEtype, avg=avg, avg_axis=avg_axis, out=out)


def DE_cspace(xyzt, xyzr, dtype='xyz', tf=_CSPACE, DEtype='jab', avg=None, avg_axis=0, out='DEi', xyzwt=None, xyzwr=None, fwtft={}, fwtfr={}, KLCH=None, camtype=cam._CAM_02_X_DEFAULT_TYPE, ucstype='ucs'):
    """
    ...
    if xyzwr is not None:
        ...
        if tf == 'camucs':
            ...
            if (tf == 'DE2000') | (tf == 'DE00'):
                return DE2000(xyzt, xyzr, dtype='xyz', DEtype=DEtype, avg=avg, avg_axis=avg_axis,
                  out=out,
                  xyzwt=xyzwt,
                  xyzwr=xyzwr,
                  KLCH=KLCH)
            ...
            if (KLCH == None) | (KLCH == [1, 1, 1]):
                ...
                DEi = (
                 (dJ / KLCH[0]) ** 2, (dC / KLCH[1]) ** 2 + (dH / KLCH[2]) ** 2)
        return _process_DEi(DEi, DEtype=DEtype, avg=avg, avg_axis=avg_axis, out=out)