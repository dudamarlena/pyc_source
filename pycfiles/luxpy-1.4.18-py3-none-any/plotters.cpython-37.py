...
# [GCC 8.4.0]
# Embedded file name: D:\Documents\GitHub\luxpy\luxpy\color\utils\plotters.py
# Compiled at: 2020-02-12 02:06:12
# Size of source mod 2**32: 36426 bytes
"""
...
__all__ = [
 'get_subplot_layout', 'plotSL', 'plotDL', 'plotBB', 'plot_color_data', 'plotceruleanline', 'plotUH', 'plotcircle', 'plotellipse', 'plot_chromaticity_diagram_colors', 'plot_spectrum_colors']
...
def plotDL(ccts=None, cieobs=_CIEOBS, cspace=_CSPACE, axh=None, show=True, force_daylight_below4000K=False, cspace_pars={}, formatstr='k-', **kwargs):
    ...
    """
    if ccts is None:
        ccts = 10 ** np.linspace(np.log10(4000.0), np.log10(1e+19), 100)
...
def plotBB(ccts=None, cieobs=_CIEOBS, cspace=_CSPACE, axh=None, cctlabels=True, show=True, cspace_pars={}, formatstr='k-', **kwargs):
    ...
    if ccts is None:
        ...
        ccts2 = 10 ** np.linspace(np.log10(15000.0), np.log10(1e+19), 100)
...
def plotSL(cieobs=_CIEOBS, cspace=_CSPACE, DL=True, BBL=True, D65=False, EEW=False, cctlabels=False, axh=None, show=True, cspace_pars={}, formatstr='k-', diagram_colors=False, diagram_samples=100, diagram_opacity=1.0, diagram_lightness=0.25, **kwargs):
    """
    ...
    if np.any([DL, BBL, D65, EEW]):
        show = False
    elif diagram_colors == True:
        axh_ = plot_chromaticity_diagram_colors(axh=axh, show=diagram_colors, cspace=cspace, cieobs=cieobs, cspace_pars=cspace_pars,
    ...
    if D65 == True:
        ...
        plt.plot(YxyD65[(Ellipsis, 1)], YxyD65[(Ellipsis, 2)], 'bo')
    if EEW == True:
        YxyEEW = colortf((spd_to_xyz(_CIE_ILLUMINANTS['E'])), tf=cspace, tfa0=cspace_pars)
        plt.plot(YxyEEW[(Ellipsis, 1)], YxyEEW[(Ellipsis, 2)], 'ko')
    if showcopy == False:
        return axh_
    plt.show()
...
def plot_chromaticity_diagram_colors(diagram_samples=256, diagram_opacity=1.0, diagram_lightness=0.25, cieobs=_CIEOBS, cspace='Yxy', cspace_pars={}, show=True, axh=None, show_grid=True, label_fontname='Times New Roman', label_fontsize=12, **kwargs):
    """
...
def plot_spectrum_colors(spd=None, spdmax=None, wavelength_height=-0.05, wavelength_opacity=1.0, wavelength_lightness=1.0, cieobs=_CIEOBS, show=True, axh=None, show_grid=True, ylabel='Spectral intensity (a.u.)', xlim=None, **kwargs):
    """
    ...
    if show == True:
        ...
        if (wavelength_height == 'spd') & (spd is not None):
            ...
            if (spdmax is None) & (spd is not None):
                ...
                if (spdmax is None) & (spd is None):
                    spdmax = 1
                    y_min, y_max = wavelength_height, 0
                else:
                    if spdmax is not None:
        ...
        axh.add_patch(polygon)
        padding = 0.1
        axh.bar(x=(wavs - padding), height=(wavelength_height * spdmax),
          width=(1 + padding),
          color=srgb,
          align='edge',
          linewidth=0,
