...
# [GCC 8.4.0]
# Embedded file name: D:\Documents\GitHub\luxpy\luxpy\toolboxes\indvcmf\individual_observer_cmf_model.py
# Compiled at: 2020-02-06 15:35:07
# Size of source mod 2**32: 99340 bytes
"""
...
import warnings
__all__ = [
 '_DATA', '_DSRC_STD_DEF', '_DSRC_LMS_ODENS_DEF', '_LMS_TO_XYZ_METHOD']
...
_WL_CRIT = 620
_WL_ASANO = getwlr([390, 780, 5])
_WL_CIETC197 = getwlr([390, 830, 0.1])
if _DSRC_LMS_ODENS_DEF == 'asano':
    _WL = _WL_ASANO
else:
    if _DSRC_LMS_ODENS_DEF == 'cietc197':
        _WL = _WL_CIETC197
    _USE_MY_ROUND = True
    _USE_SIGN_FIGS = True
    _USE_CHOP = True
    ...
    """
        ...
        data['M']['10d'] = np.array([[0.4499, -0.263, 0.046],
         [
    ...
    """
        ...
        if dsrc == 'germany':
            scale_factors = [
             0.98, 0.98, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5]
    ...
    """
        ...
        isnan = np.isnan(tmp[3, :])
        first_isnan_wl = tmp[(0, np.where(isnan)[0][0])]
        tmp = cie_interp(tmp, wl, kind='linear', negative_values_allowed=True)
        absorbance = tmp[[0, 1, 2, 3], :]
        absorbance[(-1, tmp[0] > first_isnan_wl)] = -np.inf
        macula_rel = tmp[[0, 5], :]
        macula_rel[1, :] /= 0.35
        docul2 = pd.read_csv((path + 'cietc197_docul2.dat'), header=None)
        ocular_sum_32 = tmp[[0, 4], :].T
    ...
    def init(wl=None, dsrc_std=None, dsrc_lms_odens=None, lms_to_xyz_method=None, use_sign_figs=True, use_my_round=True, use_chop=True, path=None, out=None, verbosity=1):
        """
    ...
    """
        ...
        corrected_lomd = corrected_lomd * (1 + var_od_lens / 100)
    ...
    """
        if var_od < -100:
            var_od = -100
        return my_round((0.3 + 0.45 * np.exp(-fieldsize / 1.333)) * (1 + var_od / 100), 3)
    ...
    """
        ...
        if var_shft_LMS[2] == 0:
            ...
            pkOd_L = _d_LM_max(fieldsize, var_od_LMS[0])
            pkOd_M = _d_LM_max(fieldsize, var_od_LMS[1])
            pkOd_S = _d_S_max(fieldsize, var_od_LMS[2])
            alpha_lms = 1.0 * LMSa_shft
    ...
    def _LMS_quantal(fieldsize=10, age=32, var_od_lens=0, var_od_mac=0, var_shft_LMS=[
 0, 0, 0], var_od_LMS=[0, 0, 0], norm_type='max', out='LMSq', odata0=None):
        """
    ...
    """
        ...
        if out == 'LMSq':
            return LMSq
        if out == 'LMSq,rmd,docul,alpha_lms':
            return (
             LMSq, rmd, docul, alpha_lms)
        return eval(out)


    def _LMS_energy(fieldsize=10, age=32, var_od_lens=0, var_od_mac=0, var_shft_LMS=[
 0, 0, 0], var_od_LMS=[0, 0, 0], norm_type='max', out='LMSe', base=False, odata0=None):
        """
    ...
    Returns:
        :LMSe: 
            | ndarray with the calculated quantum_based LMS cone fundamentals; first row are wavelengths.
    """
        LMSq, rmd, docul, alpha_lms = _LMS_quantal(fieldsize=fieldsize, age=age, var_od_lens=var_od_lens,
          var_od_mac=var_od_mac,
          var_shft_LMS=var_shft_LMS,
          var_od_LMS=var_od_LMS,
          norm_type='max',
          out='LMSq,rmd,docul,alpha_lms',
          odata0=odata0)
        wls = LMSq[:1, :]
        LMSe = LMSq.copy()
        LMSe[1:, :] = LMSe[1:, :] * wls
        ...
        if norm_type == 'max':
            ...
            if norm_type == 'area':
                ...
                if out == 'LMSe':
                    return LMSe
                if out == 'LMSe,LMSq,alpha_lms,LMSe_o_max,rmd,docul':
                    return (
                     LMSe, LMSq, alpha_lms, LMSe_o_max, rmd, docul)
                if out == 'LMSe,LMSq,alpha_lms,LMSe_o_max':
                    return (
                     LMSe, LMSq, alpha_lms, LMSe_o_max)
                if out == 'LMSe,LMSe_o_max':
                    return (
                     LMSe, LMSe_o_max)
                if out == 'LMSe,LMSq':
                    return (
                     LMSe, LMSq)
                if out == 'LMSe,LMSq,alpha_lms':
                    return (
                     LMSe, LMSq, alpha_lms)
                if out == 'LMSe,rmd,docul,alpha_lms':
                    return (
                     LMSe, rmd, docul, alpha_lms)
                if out == 'LMSe,LMSq,rmd,docul,alpha_lms':
                    return (
                     LMSe, LMSq, rmd, docul, alpha_lms)
                return eval(out)


    def _relative_L_cone_weight_Vl_quantal(fieldsize=10, age=32, strategy_2=True, LMSa=None, LMSq=None, var_od_lens=0, var_od_mac=0, var_shft_LMS=[
 0, 0, 0], var_od_LMS=[0, 0, 0], out='kLq', odata0=None):
        """
    ...
    """
        ...
        if (field_size != fieldsize) | (LMSa is None) | (LMSq is None):
            _, LMSq_fs_age, LMSa_fs, _ = _LMS_energy(fieldsize=field_size, age=age, var_od_lens=var_od_lens,
              var_od_mac=var_od_mac,
              var_shft_LMS=var_shft_LMS,
              var_od_LMS=var_od_LMS,
              norm_type='max',
              out='LMSe,LMSq,alpha_lms,LMSe_o_max',
              base=True,
              odata0=odata)
        else:
            LMSa_fs = LMSa
        ...
        const_fs_age = LMSa_fs[(1, 0)] * LMSq_fs_age[(2, 0)] / (LMSa_fs[(2, 0)] * LMSq_fs_age[(1,
                                                                                               0)])
        const_2_32 = LMSa_2[(1, 0)] * LMSq_2_32[(2, 0)] / (LMSa_2[(2, 0)] * LMSq_2_32[(1,
                                                                                       0)])
    ...
    def _Vl_energy_and_LM_weights(fieldsize=10, age=32, strategy_2=True, LMSa=None, LMSq=None, LMSe=None, LMSe_o_max=None, var_od_lens=0, var_od_mac=0, var_shft_LMS=[
 0, 0, 0], var_od_LMS=[0, 0, 0], odata0=None):
        """
    ...
    """
        kLq_rel = _relative_L_cone_weight_Vl_quantal(fieldsize=fieldsize, age=age, strategy_2=strategy_2, LMSa=LMSa,
          LMSq=LMSq,
          var_od_lens=var_od_lens,
          var_od_mac=var_od_mac,
          var_shft_LMS=var_shft_LMS,
          var_od_LMS=var_od_LMS,
          out='kLq',
    ...
    def _square_sum(a13, a21, a22, a33, xyz_ref_trunk, x_ref_min, L_wl_sum, M_wl_sum, S_wl_sum, V_wl_sum, L_wl_ref_min, M_wl_ref_min, S_wl_ref_min, V_wl_ref_min, LMS_390_830, wl_390_830, wl_ref_min, full_results=False):
        """
    ...
    """
        a11 = (a13 * (1 - x_ref_min) * (M_wl_ref_min * S_wl_sum - S_wl_ref_min * M_wl_sum) + x_ref_min * (a21 * L_wl_ref_min + a22 * M_wl_ref_min + a33 * S_wl_ref_min) * M_wl_sum - (1 - x_ref_min) * M_wl_ref_min * V_wl_sum) / ((1 - x_ref_min) * (L_wl_ref_min * M_wl_sum - M_wl_ref_min * L_wl_sum))
        a12 = (a13 * (1 - x_ref_min) * (L_wl_ref_min * S_wl_sum - S_wl_ref_min * L_wl_sum) + x_ref_min * (a21 * L_wl_ref_min + a22 * M_wl_ref_min + a33 * S_wl_ref_min) * L_wl_sum - (1 - x_ref_min) * L_wl_ref_min * V_wl_sum) / ((1 - x_ref_min) * (M_wl_ref_min * L_wl_sum - L_wl_ref_min * M_wl_sum))
        a11 = my_round(a11[0], 8)
        a12 = my_round(a12[0], 8)
        a13 = my_round(a13[0], 8)
        trans_mat = np.array([[a11, a12, a13], [a21, a22, 0.0], [0.0, 0.0, a33]], dtype=(np.float))
    ...
    def _compute_XYZ(L_spline, M_spline, S_spline, V_spline, LMS_spec, LMS_all, LM_weights, xyz_reference):
        """
            ...
            with warnings.catch_warnings():
                ...
                a13 = sp.optimize.fmin(_square_sum, 0.39, (
                 ...
                 LMS_390_830, wl_390_830, wl_x_min_ref, False),
                  xtol=1e-10,
                  disp=False)
            trans_mat, wl_x_min_ref, ok = _square_sum(a13, a21, a22, a33, xyz_ref_trunk, x_ref_min, L_wl_sum, M_wl_sum, S_wl_sum, V_wl_sum, L_wl_ref_min, M_wl_ref_min, S_wl_ref_min, V_wl_ref_min, LMS_390_830, wl_390_830, wl_x_min_ref, True)[1:]

        wl_spec = LMS_spec[0, :]
        X_exact_spec, Y_exact_spec, Z_exact_spec = np.dot(trans_mat, LMS_spec[1:, :])
        if wl_spec[0] == 390.0 and wl_spec[(-1)] == 830.0 and my_round(wl_spec[1] - wl_spec[0], 1) == 1.0:
    ...
    def compute_cmfs(fieldsize=10, age=32, wl=None, var_od_lens=0, var_od_macula=0, var_shft_LMS=[
 0, 0, 0], var_od_LMS=[0, 0, 0], norm_type=None, out='lms', base=False, strategy_2=True, odata0=None, lms_to_xyz_method=None, allow_negative_values=False, normalize_lms_to_xyz_matrix=False):
        """
    ...
    """
        ...
        if (fieldsize_tmp == 2) | (fieldsize_tmp == 10):
            ...
            wl_all = odata['wls']
            wl_spec = wl
            LMS_base_all, LMSq_, LMSa_, LMSe_o_max, rmd, docul = _LMS_energy(fieldsize=fieldsize, age=age, var_od_lens=var_od_lens,
              var_od_mac=var_od_macula,
              var_shft_LMS=var_shft_LMS,
              var_od_LMS=var_od_LMS,
              norm_type='max',
              out='LMSe,LMSq,alpha_lms,LMSe_o_max,rmd,docul',
              base=True,
              odata0=odata)
        ...
        if ('xyz' in out_list) | ('XYZ' in out_list) | ('M' in out_list):
            if lms_to_xyz_method == 'asano':
                ...
                if lms_to_xyz_method == 'cietc197':
                    V_std_all, LM_weights = _Vl_energy_and_LM_weights(fieldsize=fieldsize, age=age, strategy_2=strategy_2,
                      LMSa=LMSa_,
                      LMSq=LMSq_,
                      LMSe=LMS_base_all,
                      LMSe_o_max=LMSe_o_max,
                      odata0=odata0)
                    ...
                    trans_mat_std, XYZ_std_spec, trans_mat_std_N, XYZ_std_spec_N = _compute_XYZ(L_base_spline, M_base_spline, S_base_spline, V_std_spline, LMS_base_spec, LMS_base_all, LM_weights, xyz_reference)
                    M = trans_mat_std_N
                    XYZ = XYZ_std_spec_N
        ...
        if ('M' in out_list) & (normalize_lms_to_xyz_matrix == True):
            Mi = np.linalg.inv(M)
            Min = math.normalize_3x3_matrix(Mi, xyz0=(np.array([[1, 1, 1]])))
            M = np.linalg.inv(Min)
            LMS[1:, :] = np.dot(Min, XYZ[1:, :])
        ...
        if (out == 'XYZ') | (out == 'xyz'):
            return XYZ
        if (out == 'LMS,M') | (out == 'lms,M'):
            return (
             LMS, M)
        if (out == 'XYZ,M') | (out == 'xyz,M'):
            return (
             XYZ, M)
        if (out == 'LMS,XYZ,M') | (out == 'lms,xyz,M'):
            return (
             LMS, XYZ, M)
        if out == 'M':
            return M
        if out == 'LMS,trans_lens,trans_macula,sens_photopig':
            return (
             LMS, trans_lens, trans_macula, sens_photopig)
        if out == 'LMS,trans_lens,trans_macula,sens_photopig,LMSa':
            return (
             LMS, trans_lens, trans_macula, sens_photopig, odata['LMSa'].copy())
        return eval(out)


    def cie2006cmfsEx(age=32, fieldsize=10, wl=None, var_od_lens=0, var_od_macula=0, var_od_L=0, var_od_M=0, var_od_S=0, var_shft_L=0, var_shft_M=0, var_shft_S=0, norm_type=None, out='lms', base=False, strategy_2=True, odata0=None, lms_to_xyz_method=None, allow_negative_values=False, normalize_lms_to_xyz_matrix=False):
        """
    Generate Individual Observer CMFs (cone fundamentals) 
    based on CIE2006 cone fundamentals and published literature 
    on observer variability in color matching and in physiological parameters.
    ...
    References:
         ...
         <http://github.com/ifarup/ciefunctions>`_
    """
        return compute_cmfs(fieldsize=fieldsize, age=age, wl=wl, var_od_lens=var_od_lens,
          var_od_macula=var_od_macula,
          var_shft_LMS=[
         var_shft_L, var_shft_M, var_shft_S],
          var_od_LMS=[
         var_od_L, var_od_M, var_od_S],
          norm_type=norm_type,
          out=out,
          base=base,
    ...
    """
        ...
        freq_AgeCensus = np.round(t_num[1] / 1000)
        freq_AgeCensus[:10] = 0
        freq_AgeCensus[71:] = 0
        list_Age = []
    ...
    def genMonteCarloObs(n_obs=1, fieldsize=10, list_Age=[32], wl=None, norm_type=None, out='lms', base=False, strategy_2=True, odata0=None, lms_to_xyz_method=None, allow_negative_values=False):
        """
    ...
    """
        ...
        if list_Age is 'us_census':
            ...
            p = h / h.sum()
            var_age = np.random.choice((np.unique(list_AgeRound)), size=n_obs,
              replace=True,
              p=p)
            if odata0 is None:
                odata = _DATA['odata']
        ...
        LMS_All = np.nan * np.ones((4, wl.shape[0], n_obs))
        for k in range(n_obs):
            t_LMS, t_trans_lens, t_trans_macula, t_sens_photopig = cie2006cmfsEx(age=(var_age[k]), fieldsize=fieldsize, wl=wl, var_od_lens=(vAll['od_lens'][k]),
        ...
        if out == 'LMS':
            return LMS_All
        if out == 'LMS,var_age,vAll':
            return (
             LMS_All, var_age, vAll)
        return eval(out)


    def getCatObs(n_cat=10, fieldsize=2, wl=None, norm_type=None, out='lms', base=False, strategy_2=True, odata0=None, lms_to_xyz_method=None, allow_negative_values=False):
        """
    ...
    """
        ...
        if 'lms' in out.lower().split(','):
            ...
            if 'xyz' in out.lower().split(','):
                ...
                raise Exception("Must request either 'lms' or 'xyz' in :out:.")
        LMS_All = np.nan * np.ones((4, wl.shape[0], n_cat))
        for k in range(n_cat):
            t_LMS = cie2006cmfsEx(age=(var_age[k]), fieldsize=fieldsize, wl=wl, var_od_lens=(vAll['od_lens'][k]),
              var_od_macula=(vAll['od_macula'][k]),
              var_od_L=(vAll['od_L'][k]),
              var_od_M=(vAll['od_M'][k]),
              var_od_S=(vAll['od_S'][k]),
    ...
    if __name__ == 'x__main__':
        ...
        print('cct,duv using 1931_2: {:1.0f} K, {:1.4f}'.format(cct2[(0, 0)], duv2[(0,
                                                                                    0)]))
        print('cct,duv using CatObs1: {:1.0f} K, {:1.4f}'.format(cct1[(0, 0)], duv1[(0,
                                                                                     0)]))