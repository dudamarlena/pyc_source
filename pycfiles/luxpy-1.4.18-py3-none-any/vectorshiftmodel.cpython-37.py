...
# [GCC 8.4.0]
# Embedded file name: D:\Documents\GitHub\luxpy\luxpy\color\cri\VFPX\vectorshiftmodel.py
# Compiled at: 2019-09-04 12:27:50
# Size of source mod 2**32: 39895 bytes
"""
...
__all__ = [
 '_VF_CRI_DEFAULT', '_VF_CSPACE', '_VF_CSPACE_EXAMPLE', '_VF_CIEOBS', '_VF_MAXR', '_VF_DELTAR', '_VF_MODEL_TYPE', '_VF_SIG', '_VF_PCOLORSHIFT']
...
def get_poly_model(jabt, jabr, modeltype=_VF_MODEL_TYPE):
    ...
    return (
     poly_model, pmodel, dab_model, dab_res, dCHoverC_res, dab_std, dCHoverC_std)
...
def apply_poly_model_at_x(poly_model, pmodel, axr, bxr):
    ...
    Cxt = np.sqrt(axt ** 2 + bxt ** 2)
    hxt = np.arctan(bxt / (axt + _EPS))
    return (
     axt, bxt, Cxt, hxt, axr, bxr, Cxr, hxr)


def apply_poly_model_at_hue_x(poly_model, pmodel, dCHoverC_res, hx=None, Cxr=40, sig=_VF_SIG):
    """
    ...
    return (
     dCoverC_x, dCoverC_x_sig, dH_x, dH_x_sig)


def generate_vector_field(poly_model, pmodel, axr=np.arange(-_VF_MAXR, _VF_MAXR + _VF_DELTAR, _VF_DELTAR), bxr=np.arange(-_VF_MAXR, _VF_MAXR + _VF_DELTAR, _VF_DELTAR), make_grid=True, limit_grid_radius=0, color='k'):
    """
...
def VF_colorshift_model(S, cri_type=_VF_CRI_DEFAULT, model_type=_VF_MODEL_TYPE, cspace=_VF_CSPACE, sampleset=None, pool=False, pcolorshift={'href':np.arange(np.pi / 10, 2 * np.pi, 2 * np.pi / 10), 
 'Cref':_VF_MAXR,  'sig':_VF_SIG}, vfcolor='k', verbosity=0):
    """
    ...
    if (len(Jabr.shape) == 3) & (Jabr.shape[1] > 1) & (pool == True):
        Jabr = np.transpose(Jabr, (1, 0, 2))
        Jabt = np.transpose(Jabt, (1, 0, 2))
        Jabr = Jabr.reshape(Jabr.shape[0] * Jabr.shape[1], 3)
        Jabt = Jabt.reshape(Jabt.shape[0] * Jabt.shape[1], 3)
        Jabt = Jabt[:, None, :]
        Jabr = Jabr[:, None, :]
    else:
        out = [{} for _ in range(Jabr.shape[1])]
        if pool == False:
            N = Jabr.shape[1]
        else:
            N = 1
    for i in range(N):
        ...
        Jabt_i = Jabt[:, i, :].copy()
        Jabt_i = Jabt_i[:, None, :]
        DEi = np.sqrt((Jabr_i[(Ellipsis, 0)] - Jabt_i[(Ellipsis, 0)]) ** 2 + (Jabr_i[(Ellipsis,
                                                                                      1)] - Jabt_i[(Ellipsis,
                                                                                                    1)]) ** 2 + (Jabr_i[(Ellipsis,
                                                                                                                         2)] - Jabt_i[(Ellipsis,
                                                                                                                                       2)]) ** 2)
        ...
        Jt = Jabt_i[(Ellipsis, 0)].copy()
        at = Jabt_i[(Ellipsis, 1)].copy()
        bt = Jabt_i[(Ellipsis, 2)].copy()
        Jr = Jabr_i[(Ellipsis, 0)].copy()
        ar = Jabr_i[(Ellipsis, 1)].copy()
        br = Jabr_i[(Ellipsis, 2)].copy()
        ar = ar + dab_model[:, 0:1]
        br = br + dab_model[:, 1:2]
        Jabtm = np.hstack((Jt, at, bt))
        Jabrm = np.hstack((Jr, ar, br))
        DEim = np.sqrt(0 * (Jr - Jt) ** 2 + (at - ar) ** 2 + (bt - br) ** 2)
        ...
        out[i] = {'Source':{'S':S, 
          'cct':cct[i],  'duv':duv[i]}, 
         'metrics':{'Rf':Rf[:, i], 
          'Rt':Rf_deshifted,  'Rt_rms':Rf_deshifted_rms,  'Rfi':Rfi[:, i],  'Rti':Rfi_deshifted,  'cri_type':cri_type_str}, 
         'Jab':{'Jabt':Jabt_i, 
          'Jabr':Jabr_i,  'DEi':DEi}, 
         'dC/C_dH_x_sig':np.vstack((dCoverC_x, dCoverC_x_sig, dH_x, dH_x_sig)).T, 
         'fielddata':{'vectorfield':{'axt':vfaxt, 
           'bxt':vfbxt,  'axr':vfaxr,  'bxr':vfbxr}, 
          'circlefield':{'axt':cfaxt, 
           'bxt':cfbxt,  'axr':cfaxr,  'bxr':cfbxr}}, 
         'modeldata':{'pmodel':pmodel, 
          'pcolorshift':pcolorshift,  'dab_model':dab_model, 
          'dab_res':dab_res,  'dab_std':dab_std,  'model_type':model_type, 
          'fmodel':poly_model,  'Jabtm':Jabtm, 
          'Jabrm':Jabrm,  'DEim':DEim}, 
...
def generate_grid(jab_ranges=None, out='grid', ax=np.arange(-_VF_MAXR, _VF_MAXR + _VF_DELTAR, _VF_DELTAR), bx=np.arange(-_VF_MAXR, _VF_MAXR + _VF_DELTAR, _VF_DELTAR), jx=None, limit_grid_radius=0):
    """
...
def plot_shift_data(data, fieldtype='vectorfield', scalef=_VF_MAXR, color='k', axtype='polar', ax=None, hbins=10, start_hue=0.0, bin_labels='#', plot_center_lines=True, plot_axis_labels=False, plot_edge_lines=False, plot_bin_colors=True, force_CVG_layout=True):
    """
        ...
        if fieldtype is not None:
            ...
            if axtype == 'polar':
                if fieldtype == 'vectorfield':
                    ...
                    dh = math.angle_v1v2((np.hstack((vf['axt'], vf['bxt']))), (np.hstack((vf['axr'], vf['bxr']))), htype='deg')[:, None]
                    dh = dh / np.nanmax(dh)
                    plt.set_cmap('jet')
...
def plotcircle(center=np.array([0.0, 0.0]), radii=np.arange(0, 60, 10), angles=np.arange(0, 350, 10), color='k', linestyle='--', out=None):
    """
...
def initialize_VF_hue_angles(hx=None, Cxr=_VF_MAXR, cri_type=_VF_CRI_DEFAULT, modeltype=_VF_MODEL_TYPE, determine_hue_angles=_DETERMINE_HUE_ANGLES):
    ...
    rflM = _MUNSELL['R']
    hn = _MUNSELL['H']
    rH5 = np.where([_MUNSELL['H'][:, 0][x][0] == '5' for x in range(_MUNSELL['H'][:, 0].shape[0])])[0]
    hns5 = np.unique(_MUNSELL['H'][rH5])
    pool = False
    IllC = _CIE_ILLUMINANTS['C']
    ...
    pcolorshift = {'href':hx, 
     'Cref':Cxr,  'sig':_VF_SIG,  'labels':hns5}
    return pcolorshift
