...
_WL3 = [
 360.0, 830.0, 1.0]
_BB = {'c1':3.74183e-16, 
 'c2':0.014388000000000001,  'n':1.0,  'na':1.00028,  'c':299792458,  'h':6.62607004e-34,  'k':1.38064852e-23}
        ...
        if (not np.array_equal(data[0], wl_new)) | np.isnan(data).any():
            ...
            if kind in _INTERP_TYPES['linear']:
                ...
                if kind in _INTERP_TYPES['cubic']:
                    ...
                    for i in range(N):
                        ...
                        if nan_indices.any():
                            ...
                            if extrap_values[0] is None:
                                ...
                                if (type(extrap_values[0]) == np.str_) | (type(extrap_values[0]) == str) and extrap_values[0][:3] == 'ext':
                                    pass
                                else:
                    ...
                    if negative_values_allowed == False and np.any(Si):
                        Si[Si < 0.0] = 0.0
...
def daylightphase(cct, wl3=None, force_daylight_below4000K=False, verbosity=None):
    ...
    if wl3 is None:
        wl3 = _WL3
    elif (cct < 4000.0) & (force_daylight_below4000K == False):
...
def cri_ref(ccts, wl3=None, ref_type=_CRI_REF_TYPE, mix_range=None, cieobs=_CIEOBS, norm_type=None, norm_f=None, force_daylight_below4000K=False):
    ...
    if ref_type == 'spd':
        ...
        ref_type = isinstance(ref_type, list) | isinstance(ref_type, dict) or [
         ref_type]
