...
# [GCC 8.4.0]
# Embedded file name: D:\Documents\GitHub\luxpy\luxpy\color\cri\indices\cqs.py
# Compiled at: 2018-05-09 08:27:18
...
_CQS_DEFAULTS['cqs-v7.5'] = {'sampleset':"_CRI_RFL['cqs']['v7.5']", 
 ...
 'avg':math.rms, 
 'scale':{'fcn':log_scale, 
  'cfactor':[2.93, 3.1, 3.78]}, 
 'cspace':{'type':'lab', 
  'xyzw':None}, 
 'catf':{'xyzw':None, 
  'mcat':'cmc',  'D':None,  'La':[1000.0, 1000.0],  'cattype':'vonkries',  'Dtype':'cmc',  'catmode':'1>2'}, 
 'rg_pars':{'nhbins':None, 
...
_CQS_DEFAULTS['cqs-v9.0'] = {'sampleset':"_CRI_RFL['cqs']['v9.0']", 
 'ref_type':'ciera', 
 'cieobs':{'xyz':'1931_2',  'cct':'1931_2'},  'avg':math.rms, 
 'scale':{'fcn':log_scale, 
  'cfactor':[3.03, 3.2, 3.88]}, 
 'cspace':{'type':'lab', 
  'xyzw':None}, 
 'catf':{'xyzw':None, 
  'mcat':'cmc',  'D':None,  'La':[1000.0, 1000.0],  'cattype':'vonkries',  'Dtype':'cmc',  'catmode':'1>2'}, 
...
def spd_to_cqs(SPD, version='v9.0', out='Qa', wl=None):
    ...
    if len(test_original_shape) < 3:
        ...
        if version == 'v7.5':
            GA = 9.2672e-11 * cct ** 3.0 - 8.395899999999999e-07 * cct ** 2.0 + 0.00255 * cct - 1.612
        else:
            if version == 'v9.0':
    ...
    if ('Qa' in outlist) | ('Qai' in outlist) | ('Qp' in outlist) | ('Qpi' in outlist):
        ...
        for ii in range(labti.shape[1]):
            ...
            DEi_Climited[p_deltaC_pos] = np.sqrt(DEi_Climited[p_deltaC_pos] ** 2.0 - deltaC_Climited[p_deltaC_pos] ** 2.0)
            ...
            if ('Qp' in outlist) | ('Qpi' in outlist):
                ...
                Qpi[:, ii, None] = GA[ii] * scale_fcn(DEi_Climited - deltaC_pos, [scale_factor[2]])
                Qp[:, ii] = GA[ii] * scale_fcn(avg(DEi_Climited, axis=0) - deltaCmu, [scale_factor[2]])

    if 'Qg' in outlist:
        Qg = Qf.copy()
        for ii in range(labti.shape[1]):
            Qg[:, ii] = 100.0 * math.polyarea(labti[:, ii, 1], labti[:, ii, 2]) / math.polyarea(labri[:, ii, 1], labri[:, ii, 2])
