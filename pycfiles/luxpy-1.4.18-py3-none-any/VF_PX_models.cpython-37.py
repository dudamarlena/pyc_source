...
# [GCC 8.4.0]
# Embedded file name: D:\Documents\GitHub\luxpy\luxpy\color\cri\VFPX\VF_PX_models.py
# Compiled at: 2019-01-25 08:10:26
# Size of source mod 2**32: 23426 bytes
"""
Module with functions related to CRI Vector Field and Pixel models
==================================================================
...
--------------------------
 ...
 :subsample_RFL_set(): Sub samples a set of spectral reflectance functions 
                       by pixelization of color space.
...
def calculate_VF_PX_models(S, cri_type=_VF_CRI_DEFAULT, sampleset=None, pool=False, pcolorshift={'href':np.arange(np.pi / 10, 2 * np.pi, 2 * np.pi / 10), 
 'Cref':_VF_MAXR, 
 'sig':_VF_SIG,  'labels':'#'}, vfcolor='k', verbosity=0):
    """
    ...
    PX_jab_ranges = np.vstack(([0, 100, _VF_DELTAR], [-_VF_MAXR, _VF_MAXR + _VF_DELTAR, _VF_DELTAR], [-_VF_MAXR, _VF_MAXR + _VF_DELTAR, _VF_DELTAR]))
...
def subsample_RFL_set(rfl, rflpath='', samplefcn='rand', S=_CIE_ILLUMINANTS['E'], jab_ranges=None, jab_deltas=None, cieobs=_VF_CIEOBS, cspace=_VF_CSPACE, ax=np.arange(-_VF_MAXR, _VF_MAXR + _VF_DELTAR, _VF_DELTAR), bx=np.arange(-_VF_MAXR, _VF_MAXR + _VF_DELTAR, _VF_DELTAR), jx=None, limit_grid_radius=0):
    """
...
def plot_VF_PX_models(dataVF=None, dataPX=None, plot_VF=True, plot_PX=True, axtype='polar', ax='new', plot_circle_field=True, plot_sample_shifts=False, plot_samples_shifts_at_pixel_center=False, jabp_sampled=None, plot_VF_colors=[
 'g'], plot_PX_colors=['r'], hbin_cmap=None, bin_labels=None, plot_bin_colors=True, force_CVG_layout=False):
    """
    ...
    for Snr in range(len(dataVF)):
        ...
        if (plot_sample_shifts == True) & (dataVF is not None):
            ...
            dataS['fielddata']['vectorfield']['axr'] = dataVF[Snr]['Jab']['Jabr'][(Ellipsis,
                                                                                   1)][:, 0]
            dataS['fielddata']['vectorfield']['bxr'] = dataVF[Snr]['Jab']['Jabr'][(Ellipsis,
                                                                                   2)][:, 0]
            dataS['fielddata']['vectorfield']['axt'] = dataVF[Snr]['Jab']['Jabt'][(Ellipsis,
                                                                                   1)][:, 0]
            dataS['fielddata']['vectorfield']['bxt'] = dataVF[Snr]['Jab']['Jabt'][(Ellipsis,
                                                                                   2)][:, 0]
            if plot_samples_shifts_at_pixel_center == True:
                if jabp_sampled is not None:
                    dataS['fielddata']['vectorfield']['axr'] = jabp_sampled[:, 1].copy()
                    dataS['fielddata']['vectorfield']['bxr'] = jabp_sampled[:, 2].copy()
                    dataS['fielddata']['vectorfield']['axt'] = jabp_sampled[:, 1] + dataVF[Snr]['Jab']['Jabt'][(Ellipsis,
                                                                                                                1)][:, 0] - dataVF[Snr]['Jab']['Jabr'][(Ellipsis,
                                                                                                                                                        1)][:, 0]
                    dataS['fielddata']['vectorfield']['bxt'] = jabp_sampled[:, 2] + dataVF[Snr]['Jab']['Jabt'][(Ellipsis,
                                                                                                                2)][:, 0] - dataVF[Snr]['Jab']['Jabr'][(Ellipsis,
                                                                                                                                                        2)][:, 0]
                if (Snr == 0) & (ax == 'new') & (plot_VF is False):
                    ...
                    force_CVG_layout = False
            elif (plot_circle_field == True) & (dataVF is not None):
                if cmap is None:
