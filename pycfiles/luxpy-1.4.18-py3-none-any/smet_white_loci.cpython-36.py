...
# [GCC 8.4.0]
# Embedded file name: D:\Documents\GitHub\luxpy\luxpy\color\whiteness\smet_white_loci.py
# Compiled at: 2019-08-02 07:20:38
# Size of source mod 2**32: 6619 bytes
"""
...
__all__ = [
 '_UW_NEUTRALITY_PARAMETERS_SMET2014', 'xyz_to_neutrality_smet2018', 'cct_to_neutral_loci_smet2018']
...
def cct_to_neutral_loci_smet2018(cct, nlocitype='uw', out='duv,D'):
    ...
    if nlocitype == 'uw':
        ...
        D = np.exp(-(6368 * (1 / cct - 0.00015600624024960998)) ** 2)
    else:
        if nlocitype == 'ca':
            duv = 0.0382 * np.log(cct / 2194) * np.exp(-0.679 * np.log(cct / 2194) ** 2) - 0.0172
            D = np.exp(-(3912 * (1 / cct - 0.00014716703458425313)) ** 2)
        else:
            raise Exception('Unrecognized nlocitype')
        if out == 'duv,D':
            return (
