...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/canteen/util/cli.py
...
class Tool(object):
        ...
        def __new__(mcs, name, bases, properties):
          ...
          :returns: Initialized class, transformed into additional objects
            ...
            if name == 'Tool':
                ...
                for key, value in properties.viewitems():
                    if isinstance(value, (list, tuple)) and key is 'arguments':
                        ...
                        def _add_subparser(root, obj, subparsers):
                            sub = subparsers.add_parser((getattr(obj, 'name') if hasattr(obj, 'name') else obj.__name__).lower(), **{'conflict_handler': 'resolve', 
                               'help': textwrap.dedent(getattr(obj, '__doc__').strip()) if hasattr(obj, '__doc__') and getattr(obj, '__doc__') is not None else None})
                ...
                mcs.parsers[name] = {'name': (properties['name'] if 'name' in properties else name).lower(), 
                   'description': textwrap.dedent(properties['__doc__']) if '__doc__' in properties else None, 
                   'implementation': klass, 
                   'objects': {'subtools': _subtools, 
    ...
    def __init__(self, parser=None, autorun=False, safe=False):
        ...
        if not parser:
            parser = argparse.ArgumentParser(prog=(self.name if hasattr(self, 'name') else self.__class__.__name__).lower(), description=(sys.flags.optimize > 1 or textwrap.dedent)(self.__doc__.strip()) if 1 else '')
            if not _root_tool:
                _root_tool = parser
        self.parser = parser
        for _callable, flag, _config in config.get('objects', {}).get('arguments', []):
            _callable(parser, flag, _config)
        ...
        if config.get('objects', {}).get('subtools', []):
            ...
            for impl, _callable in config.get('objects', {}).get('subtools', []):
                subparser = _callable(_root_tool, impl, commands)
    ...
    def execute(cls, arguments):
        ...
        raise NotImplementedError('Command line tool "%s" is not implemented.' % repr(cls))
