...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/canteen/core/meta.py
...
class MetaFactory(type):
    ...
    def __new__(mcs, name=None, bases=None, properties=None):
        ...
        if '__root__' in properties and properties['__root__']:
            del properties['__root__']
            return construct(mcs, name, bases, properties)
        name = name if name != '__metaclass__' or '__owner__' not in properties else properties['__owner__']
        if 'initialize' in properties or any(hasattr(b, 'initialize') for b in bases):
            return grab(properties['initialize'] if 'initialize' in properties else getattr((x for x in bases if hasattr(x, 'initialize')).next(), 'initialize'))(*(mcs, name, bases, properties))
        return metachain(mcs, name, bases, properties)
    ...
    def mro(cls):
        ...
        for group in ([order[0]], order[1:-2], cls.__metachain__, order[-2:]):
            for base in group:
                if base not in seen:
                    (seen.add(base), tree.append(base))
    ...
    class Factory(Base):
        ...
        def initialize(cls, name, bases, properties):
          ...
          :returns: Prepared ``Factory`` initializor class. """
            ...
            if cls.__metachain__:
                properties['__new__'] = metanew
          ...
          :returns: ``target`` that was registered. """
            ...
            for base in target.__bases__:
                if base not in (object, type):
                    if _owner not in meta.__chain__:
                        meta.__chain__[_owner] = []
    ...
    class Component(Registry):
        ...
        def inject(cls):
          ...
          :returns: Two-item tuple of 1) the final concrete class (after
            ...
            if concrete is None:
                        ...
                        for prop, value in iterator:
                            if cls.__bindings__:
                                if prop in cls.__bindings__:
                                    func = cls.__dict__[prop] if not isinstance(cls.__dict__[prop], (
                                     staticmethod, classmethod)) else cls.__dict__[prop].__func__
                ...
                return (
                 concrete,
                 _injectable or set(filter(lambda x: not x.startswith('__'), concrete.__dict__.iterkeys())))
        ...
        def collapse(cls, spec=None):
          ...
          :returns: Collapsed DI pool, ready to be used for resolving dependency
            ...
            paths. """
            if (
             cls, spec) not in Proxy.Component.__injector_cache__:
                property_bucket = {}
                for metabucket in Proxy.Registry.__chain__.iterkeys():
                    for concrete in filter(is_component, Proxy.Component.__chain__[metabucket]):
                        namespace = ''
