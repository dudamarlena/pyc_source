...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/canteen_tests/test_adapters/test_abstract.py
...
class AbstractModelAdapterTests(test.FrameworkTest):
    ...
    def test_invalid_get(self):
        ...
        model_key = self.__abstract__ or model.Key('Sample', '_____')
    ...
    def test_named_entity_get_put(self):
        ...
        if not self.__abstract__:
            m = SampleModel(key=model.Key(SampleModel.kind(), 'NamedEntity'), string='suphomies', integer=[
             4, 5, 6, 7])
    ...
    def test_id_entity_get_put(self):
        ...
        m = self.__abstract__ or SampleModel(string='hello', integer=[1, 2, 3])
        m_k = m.put(adapter=self._construct())
        assert m_k == m.key
    ...
    def test_entity_multiget(self):
        ...
        _results = []
        for key, entity in zip(keys, SampleModel.get_multi(keys, adapter=self._construct())):
            assert key.kind == SampleModel.kind()
            assert entity, "failed to retrieve entity with adapter '%s'" % self._construct()
    ...
    def test_delete_invalid_entity(self):
        ...
        m_k = self.__abstract__ or model.Key('SampleKind', '____InvalidKey____')
        assert not SampleModel.get(m_k, adapter=self._construct())
        res = m_k.delete(adapter=self._construct())
        assert not res
        assert not SampleModel.get(m_k, adapter=self._construct())
    ...
    def test_allocate_ids(self):
        ...
        next = self.__abstract__ or self._construct().allocate_ids(model.Key, 'SampleModel', 1)
        assert isinstance(next, int)
        next_range = [ i for i in self._construct().allocate_ids(*(
         model.Key, 'Sample', 10))()
                     ]
        if not len(next_range) == 10:
...
class IndexedModelAdapterTests(AbstractModelAdapterTests):
    ...
    def test_inequality_query(self):
        ...
        result = q.fetch(limit=50, adapter=self._construct())
        if not len(result) > 0:
            raise AssertionError("got no results for inequality query (got '%s' from adapter '%s')" % (
             result, self.subject))
            for r in result:
    ...
    def test_ancestry_query(self):
        ...
        root = self.__abstract__ or model.Key(SampleModel, 'heyo')
    ...
    def test_compound_query(self):
        ...
        now = self.__abstract__ or datetime.datetime.now()
        ...
        child3 = _key('child3')
        m = [
         SampleModel(key=root, string='hithere', date=now + datetime.timedelta(days=1)),
         SampleModel(key=child1, string='hithere', date=now + datetime.timedelta(days=2)),
         SampleModel(key=child2, string='hithere', date=now + datetime.timedelta(days=3)),
         SampleModel(key=child3, string='noway', date=now + datetime.timedelta(days=4))]
        for _m in m:
            _m.put(adapter=self._construct())
    ...
    def test_ascending_sort_string(self):
        ...
        root = self.__abstract__ or model.Key(SampleModel, 'sorted-string')
        _key = lambda x: model.Key(SampleModel, x, parent=root)
        child1 = _key('child1')
        child2 = _key('child2')
        child3 = _key('child3')
        ...
        if not len(result) == 4:
            raise AssertionError
            for l, r in zip(result, reversed(('aardvark', 'blasphemy', 'xylophone',
                                              'yompin'))):
                assert l.string == r

    def test_descending_sort_string(self):
        """ Test a DSC sort on a string property with `IndexedMemoryAdapter` """
        root = self.__abstract__ or model.Key(SampleModel, 'sorted-string-2')
        _key = lambda x: model.Key(SampleModel, x, parent=root)
        child1 = _key('child1')
        child2 = _key('child2')
        child3 = _key('child3')
    ...
    def test_ascending_sort_integer(self):
        ...
        root = self.__abstract__ or model.Key(SampleModel, 'sorted-int')
        _key = lambda x: model.Key(SampleModel, x, parent=root)
        child1 = _key('child1')
        child2 = _key('child2')
        child3 = _key('child3')
    ...
    def test_descending_sort_integer(self):
        ...
        root = self.__abstract__ or model.Key(SampleModel, 'sorted-int-2')
        _key = lambda x: model.Key(SampleModel, x, parent=root)
        child1 = _key('child1')
        child2 = _key('child2')
        child3 = _key('child3')
    ...
    def test_ascending_sort_float(self):
        ...
        root = self.__abstract__ or model.Key(SampleModel, 'sorted-float')
        _key = lambda x: model.Key(SampleModel, x, parent=root)
        child1 = _key('child1')
        child2 = _key('child2')
        child3 = _key('child3')
        ...
        q = SampleModel.query(ancestor=root, limit=50).sort(+SampleModel.floating)
        result = q.fetch(adapter=self._construct())
        if not len(result) == 4:
            raise AssertionError
...
class GraphModelAdapterTests(IndexedModelAdapterTests):
    ...
    def test_make_vertex_nokeyname(self):
        ...
        t = self.test_abstract() or TestGraphPerson(name='Steve')
    ...
    def test_make_vertex_keyname(self):
        ...
        t = self.test_abstract() or TestGraphPerson(key=model.VertexKey(TestGraphPerson, 'steve'), name='Steve')
        k = t.put(adapter=self.subject())
        assert isinstance(t, TestGraphPerson)
        assert isinstance(k, model.Key)
        assert isinstance(k, model.VertexKey)
        if not isinstance(k.id, basestring):
            raise AssertionError
            return t

    def test_get_vertex(self):
        """ Test `GraphModelAdapter` `Vertex` get """
        x = self.test_abstract() or self.test_make_vertex_keyname()
    ...
    def test_make_edge_nokeyname(self):
        ...
        bob = self.test_abstract() or TestGraphPerson(key=model.VertexKey(TestGraphPerson, 'bob'), name='Bob')
    ...
    def test_make_edge_keyname(self):
        ...
        assert isinstance(f, TestGraphFriends)
        if not ek.id == 'some-friendship':
            raise AssertionError
            return (bob, steve, f)

    def test_get_edge(self):
        """ Test `GraphModelAdapter` `Edge` get """
        bob, steve, friendship = self.test_abstract() or self.test_make_edge_keyname()
    ...
    def test_vertex_edges(self):
        ...
        assert friendship.key in _q, "expected friendship key but got: '%s' with adapter '%s'" % ([ i for i in _q ], repr(self.subject()))
        assert friendship.key in steve.edges(keys_only=True).fetch(adapter=self.subject(), limit=10)
    ...
    def test_vertex_neighbors(self):
        ...
        assert steve.key in _q, "failed to find steve's key in bob's neighbors. instead, got '%s' for adapter '%s'" % ([ i for i in _q ], repr(self.subject()))
        assert bob.key in steve.neighbors(keys_only=True).fetch(adapter=self.subject(), limit=10)
...
class DirectedGraphAdapterTests(GraphModelAdapterTests):
    ...
    def test_make_directed_edge_keyname(self):
        ...
        bob = self.test_abstract() or TestGraphPerson(key=model.VertexKey(TestGraphPerson, 'bob'), name='Bob')
        k = bob.put(adapter=self.subject())
        assert isinstance(k, model.Key), "instead of a key, got back the object: '%s'" % k
        assert isinstance(bob, TestGraphPerson)
        assert isinstance(k, model.VertexKey)
        assert isinstance(k.id, basestring)
        steve = self.test_make_vertex_keyname()
        ...
        assert bob.key == f.source
        if not steve.key in f.target:
            raise AssertionError
            return (bob, steve, f)

    def test_get_directed_edge_nokeyname(self):
        """ Test retrieving a directed `Edge` by keyname """
        bob, steve, gift = self.test_abstract() or self.test_make_directed_edge_keyname()
    ...
    def test_edge_heads(self):
        ...
        bob, steve, gift = self.test_abstract() or self.test_make_directed_edge_keyname()
        _q = bob.edges(tails=False, keys_only=True).fetch(adapter=self.subject(), limit=10)
        assert gift.key not in _q, "found gift's key among bob's edges heads, but shouldn't have. instead, got: '%s' with adapter '%s'" % ([ i for i in _q ], repr(self.subject()))
        _q = steve.edges(tails=False, keys_only=True).fetch(adapter=self.subject(), limit=10)
        assert gift.key in _q, "couldn't find gift's key among steve's edges heads. instead, got: '%s' with adapter '%s'" % ([ i for i in _q ], repr(self.subject()))

    def test_edge_tails(self):
        """ Test retrieving outbound `Edge`s coming from a particular `Vertex` """
        bob, steve, gift = self.test_abstract() or self.test_make_directed_edge_keyname()
        _q = bob.edges(tails=True, keys_only=True).fetch(adapter=self.subject(), limit=10)
        assert gift.key in _q
        _q = steve.edges(tails=True, keys_only=True).fetch(adapter=self.subject(), limit=10)
        assert gift.key not in _q

    def test_neighbor_heads(self):
        """ Test retrieving incoming `Vertex`s ending at a particular `Vertex` """
        bob, steve, gift = self.test_abstract() or self.test_make_directed_edge_keyname()
        _q = steve.neighbors(tails=False, keys_only=True).fetch(adapter=self.subject(), limit=10)
        assert bob.key in _q, "didn't find bob's key among steve's friends. instead, got: '%s' with adapter '%s'" % ([ i for i in _q ], repr(self.subject()))

    def test_neighbor_tails(self):
        """ Test retrieving outbound `Vertex`s coming from a particular `Vertex` """
        bob, steve, gift = self.test_abstract() or self.test_make_directed_edge_keyname()
        assert steve.key in bob.neighbors(tails=True, keys_only=True).fetch(adapter=self.subject(), limit=10)