...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/canteen/logic/http/semantics.py
...
def url(name_or_route, route=None, **kwargs):
    ...
    if not route:
        name, route = (None, name_or_route) if 1 else (name_or_route, route)
        raise (route or ValueError)('Cannot bind to a URL with an empty route.')
    ...
    def bind_route(target):
        ...
        :returns: Decorated ``target``, unchanged, after registration. """
        wrap = kwargs.get('wrap') or (lambda x: x)
        if 'wrap' in kwargs:
            del kwargs['wrap']
        for entry in (isinstance(route, tuple) or route,) if 1 else route:
...
with runtime.Library('werkzeug', strict=True) as (library, werkzeug):
    wsgi, utils, routing, wrappers, exceptions = library.load('wsgi', 'utils', 'routing', 'wrappers', 'exceptions')
    ...
    class HTTPSemantics(logic.Logic):
        ...
        __aliases__, __map__, __router__ = {}, None, None
        HTTPException = exceptions.HTTPException

        class HTTPRequest(wrappers.BaseRequest, wrappers.AcceptMixin, wrappers.UserAgentMixin, wrappers.AuthorizationMixin, wrappers.CommonRequestDescriptorsMixin):
          ...
          - ``CommonRequestDescriptorsMixin`` - extra utility attributes """
            ...
            :returns: ``self``, for chainability. """
                ...
                return setattr(self, '__session__', (session_obj, engine)) or self.__session__

        class HTTPResponse(wrappers.BaseResponse, wrappers.ETagResponseMixin, wrappers.ResponseStreamMixin, wrappers.WWWAuthenticateMixin, wrappers.CommonResponseDescriptorsMixin):
        ...
        def redirect(cls, url=None, name=None, permanent=False, code=None, *args, **kwargs):
          ...
          :returns: Prepared HTTP redirect, ready to be returned from a handler
            ...
            if not url and not name or url and name:
                raise TypeError('Must provide either a URL or name to redirect to - not both and at least one or the other. Got "%s" and "%s".' % (
                 url, name))
