...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/doapfiend/plugins/fields.py
# Compiled at: 2008-06-10 00:38:17
"""
...
class OutputPlugin(Plugin):
    ...
    def add_options(self, parser, output, search):
        ...
        output.add_option('--%s' % self.name, action='store', dest=self.enable_opt, help='Output specific DOAP fields as plain text')
    ...
    def serialize(self, doap_xml, color=False):
        ...
        for field in fields:
            ...
            elif field in ('maintainer', 'developer', 'documenter', 'helper', 'tester',
                           'translator'):
...
def get_releases(doap, brief=False):
    ...
    if hasattr(doap, 'releases') and len(doap.releases) != 0:
        ...
        for release in doap.releases:
            if release.name:
                out += COLOR['bold'] + COLOR['cyan'] + release.name + COLOR['normal'] + '\n'
            if hasattr(release, 'created') and release.created is not None:
                created = release.created
            else:
                created = ''
            out += COLOR['cyan'] + '  ' + release.revision + ' ' + COLOR['normal'] + created + '\n'
            if not brief:
                if hasattr(release, 'changelog'):
                    if release.changelog:
                        out += COLOR['yellow'] + release.changelog + COLOR['normal'] + '\n'
