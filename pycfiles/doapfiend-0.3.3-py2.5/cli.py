...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/doapfiend/cli.py
# Compiled at: 2008-06-09 17:42:23
"""
...
class DoapFiend(object):
    ...
    def print_doap(self, doap_xml):
        ...
        if self.options.write:
            ...
            filename = None
        print_doap(doap_xml, serializer=self.serializer, filename=filename, color=not self.options.no_color)
        return
    ...
    def setup_opt_parser(self):
        ...
        group_search = optparse.OptionGroup(opt_parser, 'Search options', 'Options for searching for DOAP')
        opt_parser.add_option('--version', action='store_true', dest='doapfiend_version', default=False, help='Show doapfiend version and exit.')
        opt_parser.add_option('-P', '--http-proxy', action='store', dest='proxy', default=False, help='Specify http proxy URL if you use one.')
        group_output = optparse.OptionGroup(opt_parser, 'Output options', 'Choose these options to change default output behavior')
        group_output.add_option('--debug', action='store_true', dest='debug', default=False, help='Show debugging information')
        group_output.add_option('-f', '--follow-links', action='store_true', dest='follow', default=False, help='Search for and show additional DOAP.', metavar='FILENAME')
        group_output.add_option('-s', '--show-links', action='store_true', dest='show_links', default=False, help='Search for and show links to additional DOAP.', metavar='FILENAME')
        group_output.add_option('-w', '--write', action='store', dest='write', default=False, help='Write DOAP to a file instead of displaying it.', metavar='FILENAME')
        group_output.add_option('-C', '--no-color', action='store_true', dest='no_color', default=False, help="Don't use color in output")
        group_output.add_option('-q', '--quiet', action='store_true', dest='quiet', default=False, help='Show less output')
        group_output.add_option('-v', '--verbose', action='store_true', dest='verbose', default=False, help='Show more output')
