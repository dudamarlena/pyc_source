...
# [GCC 8.4.0]
# Embedded file name: /mnt/c/Users/TimHe/OneDrive/Programming/LookingGlass/looking_glass/lib/tables.py
...
class Node(Base):
    ...
    def from_dict(node, workspace_id):
        return Node(id=node.get('id'), workspace_id=workspace_id, version_number=node.get('version_number'), hostname=node.get('hostname'), ip=node.get('ip'), x_coordinate=node.get('x'), y_coordinate=node.get('y'), group=node.get('group'), active=True)
...
def setup_tables(engine):
    ...
    if session.query(ChangeLog).count() == 0:
        first_changelog_row = ChangeLog(version_number=0, date_time=datetime.datetime.utcnow())
