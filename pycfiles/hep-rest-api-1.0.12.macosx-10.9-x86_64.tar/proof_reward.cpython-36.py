...
# [GCC 8.4.0]
# Embedded file name: /Users/erhu/project/gitrepo/hep-sdk/venv/lib/python3.6/site-packages/hep_rest_api/models/proof_reward.py
# Compiled at: 2019-07-15 09:20:14
# Size of source mod 2**32: 12502 bytes
"""
    HEP REST API
...
class ProofReward(object):
    ...
    def __init__(self, proof_hash=None, proof_item_id=None, proof_subitem_id=None, reward_tokens=None, newid=None, newforce=None, action=None, issue_timestamp=None, issue_status=None):
        """ProofReward - a model defined in Swagger"""
    ...
    def proof_subitem_id(self, proof_subitem_id):
        ...
        if proof_subitem_id is None:
            raise ValueError('Invalid value for `proof_subitem_id`, must not be `None`')
        else:
            ...
            if proof_subitem_id is not None:
                if len(proof_subitem_id) < 1:
                    raise ValueError('Invalid value for `proof_subitem_id`, length must be greater than or equal to `1`')
    ...
    def reward_tokens(self, reward_tokens):
        ...
        if reward_tokens is None:
            raise ValueError('Invalid value for `reward_tokens`, must not be `None`')
        else:
            if reward_tokens is not None:
                if len(reward_tokens) > 64:
                    raise ValueError('Invalid value for `reward_tokens`, length must be less than or equal to `64`')
            if reward_tokens is not None:
                if len(reward_tokens) < 1:
                    raise ValueError('Invalid value for `reward_tokens`, length must be greater than or equal to `1`')
    ...
    def newforce(self, newforce):
        ...
        """
        if newforce is None:
            raise ValueError('Invalid value for `newforce`, must not be `None`')
        else:
            if newforce is not None:
                if len(newforce) > 64:
                    raise ValueError('Invalid value for `newforce`, length must be less than or equal to `64`')
            if newforce is not None:
                if len(newforce) < 1:
                    raise ValueError('Invalid value for `newforce`, length must be greater than or equal to `1`')
    ...
    def issue_timestamp(self, issue_timestamp):
        ...
        if issue_timestamp is None:
            raise ValueError('Invalid value for `issue_timestamp`, must not be `None`')
    ...
    def issue_status(self, issue_status):
        ...
        if issue_status is None:
            raise ValueError('Invalid value for `issue_status`, must not be `None`')
        else:
            if issue_status is not None:
                if len(issue_status) > 64:
                    raise ValueError('Invalid value for `issue_status`, length must be less than or equal to `64`')
            if issue_status is not None:
                if len(issue_status) < 1:
                    raise ValueError('Invalid value for `issue_status`, length must be greater than or equal to `1`')
