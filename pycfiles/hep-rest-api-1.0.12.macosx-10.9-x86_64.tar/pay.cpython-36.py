...
# [GCC 8.4.0]
# Embedded file name: /Users/erhu/project/gitrepo/hep-sdk/venv/lib/python3.6/site-packages/hep_rest_api/scenarios/pay.py
# Compiled at: 2019-07-18 03:05:52
...
class PayHelper(BaseHelper):
    ...
    def generate_pay_request(self, order_number, price_currency, total_price, description, seller, customer, broker='', expired=300, uuid=None):
        ...
        """
        if not uuid:
            raise AttributeError('uuid can not be None')
        if not order_number:
            raise AttributeError('order number can not be None')
        data = {'action':self.action_auth_pay, 
    ...
    def get_client_pay_params(self, order_number, price_currency, total_price, description, seller, customer, broker='', uuid=None):
        """Generate the payment request
