...
# [GCC 8.4.0]
# Embedded file name: /Users/erhu/project/gitrepo/hep-sdk/venv/lib/python3.6/site-packages/hep_rest_api/utils.py
# Compiled at: 2019-07-10 22:06:38
...
import json, hashlib, collections, base58, binascii, logging
    ...
    for k, v in ordered_data.items():
        if k in sign_fields:
            ...
            if isinstance(v, list) or isinstance(v, dict):
                sign_string += '%s=%s' % (k, json.dumps(v, sort_keys=True, separators=(',', ':'), ensure_ascii=False))
            else:
                sign_string += '%s=%s' % (k, v)
...
def newid_encode_by_public_key(public_key, chain_id):
    ...
    data = base58.b58encode_check(b'\x00' + binascii.a2b_hex(num_sum))
...
def split_signature(signature):
    ...
    """
    if not signature:
        raise ValueError("Missing the required parameter 'signature' when calling split_signature")
