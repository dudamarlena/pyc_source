...
# [GCC 8.4.0]
# Embedded file name: /Users/erhu/project/gitrepo/hep-sdk/venv/lib/python3.6/site-packages/hep_rest_api/models/newforce_daily_stats_response.py
# Compiled at: 2019-07-15 09:20:14
# Size of source mod 2**32: 7261 bytes
"""
    HEP REST API
...
class NewforceDailyStatsResponse(object):
    ...
    swagger_types = {'nf_new':'str', 
     'nf_cny':'str', 
     'nf_usd':'str', 
     'gnf':'str'}
    attribute_map = {'nf_new':'nf_new', 
     'nf_cny':'nf_cny', 
     'nf_usd':'nf_usd', 
     'gnf':'gnf'}

    def __init__(self, nf_new=None, nf_cny=None, nf_usd=None, gnf=None):
        """NewforceDailyStatsResponse - a model defined in Swagger"""
        self._nf_new = None
        self._nf_cny = None
        self._nf_usd = None
        self._gnf = None
    ...
    def nf_new(self, nf_new):
        ...
        """
        if nf_new is None:
            raise ValueError('Invalid value for `nf_new`, must not be `None`')
        else:
            if nf_new is not None:
                if len(nf_new) > 256:
                    raise ValueError('Invalid value for `nf_new`, length must be less than or equal to `256`')
            if nf_new is not None:
                if len(nf_new) < 1:
                    raise ValueError('Invalid value for `nf_new`, length must be greater than or equal to `1`')
        self._nf_new = nf_new
    ...
    def nf_cny(self, nf_cny):
        ...
        """
        if nf_cny is None:
            raise ValueError('Invalid value for `nf_cny`, must not be `None`')
        else:
            if nf_cny is not None:
                if len(nf_cny) > 64:
                    raise ValueError('Invalid value for `nf_cny`, length must be less than or equal to `64`')
            if nf_cny is not None:
                if len(nf_cny) < 1:
                    raise ValueError('Invalid value for `nf_cny`, length must be greater than or equal to `1`')
        self._nf_cny = nf_cny
    ...
    def nf_usd(self, nf_usd):
        ...
        """
        if nf_usd is None:
            raise ValueError('Invalid value for `nf_usd`, must not be `None`')
        else:
            if nf_usd is not None:
                if len(nf_usd) > 64:
                    raise ValueError('Invalid value for `nf_usd`, length must be less than or equal to `64`')
            if nf_usd is not None:
                if len(nf_usd) < 1:
                    raise ValueError('Invalid value for `nf_usd`, length must be greater than or equal to `1`')
        self._nf_usd = nf_usd
    ...
    def gnf(self, gnf):
        ...
        """
        if gnf is None:
            raise ValueError('Invalid value for `gnf`, must not be `None`')
        else:
            if gnf is not None:
                if len(gnf) > 64:
                    raise ValueError('Invalid value for `gnf`, length must be less than or equal to `64`')
            if gnf is not None:
                if len(gnf) < 1:
                    raise ValueError('Invalid value for `gnf`, length must be greater than or equal to `1`')
        self._gnf = gnf
