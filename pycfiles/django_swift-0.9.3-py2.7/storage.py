...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/django_swift/storage.py
# Compiled at: 2013-06-14 05:31:15
...
class SwiftclientStorageFile(File):
    ...
    def __init__(self, connection, container_name, name, *args, **kwargs):
        ...
        self._pos = 0
        File.__init__(self, file=None, name=name, *args, **kwargs)
        return

    def size(self):
        try:
            head = self.connection.head_object(self.container_name, self.name)
            return int(head['content-length'])
        except BaseException:
            return 0

    def __iter__(self, chunk_size=1024):
        _, data = self.connection.get_object(self.container_name, self.name, resp_chunk_size=chunk_size)
        return data
    ...
    def read(self):
        ...
        if self.pos > 0:
            return ''
        _, data = self.connection.get_object(self.container_name, self.name)
        self._pos += len(data)
        return data
...
class SwiftStorage(Storage):
    ...
    def connection(self):
        if self._connection is None:
            self._connection = swiftclient.Connection(authurl=self.authurl, user=self.user, key=self.key, retries=self.retries, preauthurl=self.preauthurl, preauthtoken=self.preauthurl, snet=self.snet, starting_backoff=self.starting_backoff, tenant_name=self.tenant_name, os_options=self.os_options, auth_version=self.auth_version, cacert=self.cacert, insecure=self.insecure)
        return self._connection

    def _open(self, name, mode):
        return SwiftclientStorageFile(self.connection, self.container_name, name)
    ...
    def size(self, name):
        try:
            head = self.connection.head_object(self.container_name, name)
            return int(head['content-length'])
        except BaseException:
            return 0
