...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-uunam8sj/setuptools/setuptools/wheel.py
...
class Wheel:
    ...
    def get_dist_info(self, zf):
        for member in zf.namelist():
            ...
            if dirname.endswith('.dist-info'):
                if canonicalize_name(dirname).startswith(canonicalize_name(self.project_name)):
                    return dirname
    ...
    def _convert_metadata(zf, destination_eggdir, dist_info, egg_info):
        ...
        dist = pkg_resources.Distribution.from_location(destination_eggdir,
          dist_info, metadata=(pkg_resources.PathMetadata(destination_eggdir, dist_info)))
        ...
        extras_require = {extra:sorted(req for req in map(raw_req, dist.requires((extra,))) if req not in install_requires) for extra in dist.extras}
        os.rename(dist_info, egg_info)
        os.rename(os.path.join(egg_info, 'METADATA'), os.path.join(egg_info, 'PKG-INFO'))
        setup_dist = setuptools.Distribution(attrs=dict(install_requires=install_requires,
