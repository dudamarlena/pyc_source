...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-uunam8sj/pip/pip/_internal/req/req_file.py
# Compiled at: 2020-03-25 22:23:37
# Size of source mod 2**32: 18485 bytes
"""
Requirements file parsing
"""
from __future__ import absolute_import
import optparse, os, re, shlex, sys
...
if MYPY_CHECK_RUNNING:
    ...
    from typing import Any, Callable, Iterator, List, NoReturn, Optional, Text, Tuple
...
__all__ = [
 'parse_requirements']
SCHEME_RE = re.compile('^(http|https|file):', re.I)
COMMENT_RE = re.compile('(^|\\s+)#.*$')
ENV_VAR_RE = re.compile('(?P<var>\\$\\{(?P<name>[A-Z0-9_]+)\\})')
SUPPORTED_OPTIONS = [
...
class ParsedLine(object):
    ...
    def __init__(self, filename, lineno, comes_from, args, opts, constraint):
...
def parse_requirements(filename, session, finder=None, comes_from=None, options=None, constraint=False, wheel_cache=None, use_pep517=None):
    ...
    """
    skip_requirements_regex = options.skip_requirements_regex if options else None
    line_parser = get_line_parser(finder)
    parser = RequirementsFileParser(session, line_parser, comes_from, skip_requirements_regex)
    for parsed_line in parser.parse(filename, constraint):
        req = handle_line(parsed_line, finder, options, session, wheel_cache, use_pep517)
        if req is not None:
            yield req
...
def handle_line(line, finder=None, options=None, session=None, wheel_cache=None, use_pep517=None):
    ...
    if line.args:
        ...
        return install_req_from_line((line.args),
    ...
    if line.opts.editables:
        isolated = options.isolated_mode if options else False
        return install_req_from_editable((line.opts.editables[0]),
          comes_from=line_comes_from, use_pep517=use_pep517,
          constraint=(line.constraint),
          isolated=isolated,
          wheel_cache=wheel_cache)
    if line.opts.require_hashes:
        ...
        search_scope = SearchScope(find_links=find_links,
...
class RequirementsFileParser(object):
    ...
    def __init__(self, session, line_parser, comes_from, skip_requirements_regex):
    ...
    def _parse_and_recurse(self, filename, constraint):
        for line in self._parse_file(filename, constraint):
            if not line.args and not line.opts.editables and (line.opts.requirements or line.opts.constraints):
                ...
                if SCHEME_RE.search(filename):
                    ...
                    if not SCHEME_RE.search(req_path):
                        req_path = os.path.join(os.path.dirname(filename), req_path)
                for inner_line in self._parse_and_recurse(req_path, nested_constraint):
                    yield inner_line
    ...
    def _parse_file(self, filename, constraint):
        ...
        for line_number, line in lines_enum:
            ...
            yield ParsedLine(filename, line_number, self._comes_from, args_str, opts, constraint)
...
def join_lines(lines_enum):
    ...
    if new_line:
        yield (
         primary_line_number, ''.join(new_line))
