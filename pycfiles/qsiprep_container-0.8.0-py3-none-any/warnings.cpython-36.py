...
# [GCC 8.4.0]
# Embedded file name: /tmp/src/qsiprep/wrapper/build/lib/python3.6/warnings.py
# Compiled at: 2019-03-22 00:31:36
# Size of source mod 2**32: 18488 bytes
...
def filterwarnings(action, message='', category=Warning, module='', lineno=0, append=False):
    ...
    _add_filter(action, (re.compile(message, re.I)), category, (re.compile(module)),
...
def simplefilter(action, category=Warning, lineno=0, append=False):
    ...
    if not action in ('error', 'ignore', 'always', 'default', 'module', 'once'):
        raise AssertionError('invalid action: %r' % (action,))
    elif not (isinstance(lineno, int) and lineno >= 0):
