...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-uunam8sj/setuptools/setuptools/_vendor/packaging/markers.py
# Compiled at: 2020-03-25 22:23:37
# Size of source mod 2**32: 8268 bytes
from __future__ import absolute_import, division, print_function
import operator, os, platform, sys
...
def _format_marker(marker, first=True):
    if not isinstance(marker, (list, tuple, string_types)):
        raise AssertionError
    else:
        if isinstance(marker, list):
            if len(marker) == 1:
                if isinstance(marker[0], (list, tuple)):
...
_operators = {'in':lambda lhs, rhs: lhs in rhs,  'not in':lambda lhs, rhs: lhs not in rhs, 
 '<':operator.lt, 
 '<=':operator.le, 
 '==':operator.eq, 
 '!=':operator.ne, 
