...
# [GCC 8.4.0]
# Embedded file name: /tmp/src/qsiprep/wrapper/build/lib/python3.6/base64.py
# Compiled at: 2019-03-22 00:31:36
# Size of source mod 2**32: 20487 bytes
...
def b32encode(s):
    ...
    if _b32tab2 is None:
        ...
        b32tab = None
    elif not isinstance(s, bytes_types):
...
def _85encode(b, chars, chars2, pad=False, foldnuls=False, foldspaces=False):
    if not isinstance(b, bytes_types):
        ...
        chunks = [b'z' if not word else b'y' if (foldspaces and word == 538976288) else (chars2[(word // 614125)] + chars2[(word // 85 % 7225)] + chars[(word % 85)]) for word in words if foldnuls]
        if padding:
            if not pad:
                if chunks[(-1)] == b'z':
                    chunks[-1] = chars[0] * 5
