...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-uunam8sj/pip/pip/_internal/req/constructors.py
# Compiled at: 2020-03-25 22:23:37
# Size of source mod 2**32: 14388 bytes
...
def _get_url_from_path(path, name):
    ...
    if _looks_like_path(name):
        ...
        return is_archive_file(path) or None
    else:
        if os.path.isfile(path):
            return path_to_url(path)
        urlreq_parts = name.split('@', 1)
        if len(urlreq_parts) >= 2:
            if not _looks_like_path(urlreq_parts[0]):
                return
