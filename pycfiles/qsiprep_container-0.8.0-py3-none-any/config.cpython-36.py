...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-uunam8sj/setuptools/setuptools/config.py
# Compiled at: 2020-03-25 22:23:37
# Size of source mod 2**32: 20575 bytes
...
class ConfigHandler:
    ...
    def _parse_file(cls, value):
        ...
        if not isinstance(value, string_types):
            ...
            return '\n'.join(cls._read_file(path) for path in filepaths if cls._assert_local(path) or True if os.path.isfile(path))
...
class ConfigOptionsHandler(ConfigHandler):
    ...
    def parse_section_packages__find(self, section_options):
        ...
        find_kwargs = dict([(k, v) for k, v in section_data.items() if k in valid_keys if v])
