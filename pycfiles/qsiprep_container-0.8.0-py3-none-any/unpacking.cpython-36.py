...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-uunam8sj/pip/pip/_internal/utils/unpacking.py
...
def split_leading_dir(path):
    ...
    if '/' in path:
        if '\\' in path and path.find('/') < path.find('\\') or '\\' not in path:
            return path.split('/', 1)
    if '\\' in path:
        return path.split('\\', 1)
    else:
...
def unpack_file(filename, location, content_type=None):
    ...
    if content_type == 'application/zip' or filename.lower().endswith(ZIP_EXTENSIONS) or zipfile.is_zipfile(filename):
        ...
        if content_type == 'application/x-gzip' or tarfile.is_tarfile(filename) or filename.lower().endswith(TAR_EXTENSIONS + BZ2_EXTENSIONS + XZ_EXTENSIONS):
