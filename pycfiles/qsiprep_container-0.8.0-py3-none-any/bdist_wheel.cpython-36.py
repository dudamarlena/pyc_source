...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-uunam8sj/wheel/wheel/bdist_wheel.py
...
class bdist_wheel(Command):
    ...
    user_options = [
     ...
     (
      'plat-name=', 'p',
      'platform name to embed in generated filenames (default: %s)' % get_platform(None)),
     ...
     (
      'python-tag=', None,
      'Python implementation compatibility tag (default: py%s)' % get_impl_ver()[0]),
    ...
    boolean_options = [
     'keep-temp', 'skip-build', 'relative', 'universal']
    ...
    def finalize_options(self):
        if self.bdist_dir is None:
            ...
            if self.build_number is not None:
                if not self.build_number[:1].isdigit():
    ...
    def get_tag(self):
        if self.plat_name_supplied:
            ...
            if self.root_is_pure:
                ...
                if self.plat_name:
                    if not self.plat_name.startswith('macosx'):
                        plat_name = self.plat_name
                else:
                ...
                if self.root_is_pure:
                    ...
                    if self.py_limited_api:
                        if (impl_name + impl_ver).startswith('cp3'):
                    ...
                    supported_tags = pep425tags.get_supported((self.bdist_dir),
                      supplied_platform=(plat_name if self.plat_name_supplied else None))
                    if not self.py_limited_api:
                        if not tag == supported_tags[0]:
                            raise AssertionError('%s != %s' % (tag, supported_tags[0]))
    ...
    def run(self):
        ...
        distinfo_dirname = '{}-{}.dist-info'.format(safer_name(self.distribution.get_name()), safer_version(self.distribution.get_version()))
    ...
    def license_paths(self):
        ...
        files = set()
        patterns = sorted({option for option in metadata.get('license_files', ('',
                                                                               ''))[1].split()})
        if 'license_file' in metadata:
            warn('The "license_file" option is deprecated. Use "license_files" instead.', DeprecationWarning)
            files.add(metadata['license_file'][1])
        if 'license_file' not in metadata:
            if 'license_files' not in metadata:
