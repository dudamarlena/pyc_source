...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/robograph/datamodel/base/graph.py
# Compiled at: 2016-07-13 17:51:17
import logging, traceback, networkx as nx
...
class Graph:
    ...
    def edges(self):
        ...
        result = []
        for e in nx_edges:
            result.append(dict(node_from=e[0], node_to=e[1], output_label=e[2]['name']))
    ...
    def connect(self, node_from, node_to, output_label):
        ...
        if not self._nxgraph.has_node(node_from):
            raise exceptions.NodeConnectionError('Graph does not contain node_from: %s' % (
             node_from,))
        if not self._nxgraph.has_node(node_to):
            raise exceptions.NodeConnectionError('Graph does not contain node_to: %s' % (
             node_to,))
    ...
    def execute(self, result_label='result'):
        ...
        """
        if self.has_isles():
            raise exceptions.GraphExecutionError('Cannot execute graphs with isolated nodes')
