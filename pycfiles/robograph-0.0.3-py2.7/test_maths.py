...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/robograph/datamodel/tests/test_maths.py
...
def test_sin():
    pi = maths.Pi().output()
    instance = maths.Sin(argument=pi / 2.0)
    instance.set_output_label('any')
    assert 1 - instance.output() < 1e-05


def test_cos():
    pi = maths.Pi().output()
    instance = maths.Cos(argument=pi / 2.0)
    instance.set_output_label('any')
    assert instance.output() < 1e-05
