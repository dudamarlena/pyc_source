...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/robograph/datamodel/nodes/lib/functional_operators.py
...
class Sorter(node.Node):
    ...
    """
    _reqs = [
     'sorting_function', 'sequence']

    def output(self):
        if self._params['sorting_function'] is None:
            return sorted(self._params['sequence'])
        else:
            return sorted(self._params['sequence'], cmp=self._params['sorting_function'])
...
class ReverseSorter(Sorter):
    ...
    """
    _reqs = [
     'reverse_sorting_function', 'sequence']

    def output(self):
        if self._params['reverse_sorting_function'] is None:
            return sorted(self._params['sequence'], reverse=True)
        else:
            return sorted(self._params['sequence'], cmp=self._params['reverse_sorting_function'])
