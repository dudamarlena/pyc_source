...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/robograph/datamodel/tests/test_ifthenreturn.py
...
def test_output():
    ...
    function_true = lambda x: math.sqrt(x)
    function_false = lambda x: 0
    instance = branching.IfThenReturn(data=data, condition=condition, function_true=function_true, function_false=function_false)
    instance.set_output_label('any')
    assert instance.output() == function_true
    data = -34
    instance = branching.IfThenReturn(data=data, condition=condition, function_true=function_true, function_false=function_false)
