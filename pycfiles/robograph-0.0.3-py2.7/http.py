...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/robograph/datamodel/nodes/lib/http.py
...
class HttpClient(node.Node):
    ...
    """
    _reqs = [
     'url', 'headers', 'auth', 'timeout_secs', 'verify_ssl']
...
class RawGet(HttpClient):
    ...
    def output(self):
        r = requests.get(self._params['url'], auth=self._params['auth'], params=self._params['query'], headers=self._params['headers'], timeout=self._params['timeout_secs'] or self.DEFAULT_TIMEOUT_SECS, verify=self._params['verify_ssl'] if self._params['verify_ssl'] is not None else True)
        return r
...
class Get(RawGet):
    ...
    def output(self):
        ...
        payload = self._encode_payload(raw_response, self._params['mime_type'] or 'application/json')
        return payload
...
class RawPost(HttpClient):
    ...
    def output(self):
        r = requests.post(self._params['url'], data=self._params['post_data'], auth=self._params['auth'], files=self._params['files'], headers=self._params['headers'], timeout=self._params['timeout_secs'] or self.DEFAULT_TIMEOUT_SECS, verify=self._params['verify_ssl'] if self._params['verify_ssl'] is not None else True)
        return r
...
class RawPut(HttpClient):
    ...
    def output(self):
        r = requests.put(self._params['url'], data=self._params['put_data'], auth=self._params['auth'], headers=self._params['headers'], timeout=self._params['timeout_secs'] or self.DEFAULT_TIMEOUT_SECS, verify=self._params['verify_ssl'] if self._params['verify_ssl'] is not None else True)
        return r
...
class RawDelete(HttpClient):
    ...
    def output(self):
        r = requests.put(self._params['url'], data=self._params['delete_data'], auth=self._params['auth'], headers=self._params['headers'], timeout=self._params['timeout_secs'] or self.DEFAULT_TIMEOUT_SECS, verify=self._params['verify_ssl'] if self._params['verify_ssl'] is not None else True)
        return r
