...
# [GCC 8.4.0]
# Embedded file name: distributions/vendor/stats.py
# Compiled at: 2017-10-28 18:53:45
from __future__ import division
import numpy as np
from numpy.random import random
na = np.newaxis
import scipy.stats as stats, scipy.special as special, scipy.linalg
...
def mask_data(data):
    return np.ma.masked_array(np.nan_to_num(data), np.isnan(data), fill_value=0.0, hard_mask=True)
...
def sample_discrete_from_log(p_log, axis=0, dtype=np.int32):
    ...
    thesize[axis] = 1
    randvals = random(size=thesize) * np.reshape(cumvals[[ slice(None) if i is not axis else -1 for i in range(p_log.ndim)
                                                         ]], thesize)
...
def sample_wishart(sigma, nu):
    ...
    if nu <= 81 + n and nu == round(nu):
        ...
        A[np.tri(n, k=-1, dtype=bool)] = np.random.normal(size=n * (n - 1) / 2.0)
        X = np.dot(chol, A)
    return np.dot(X, X.T)
...
def sample_mniw(nu, S, M, K=None, Kinv=None):
    ...
    if K is not None:
        ...
        return (
         sample_mn(M=M, U=Sigma, Vinv=Kinv), Sigma)
        return
...
def invwishart_log_partitionfunction(sigma, nu, chol=None):
    D = sigma.shape[0]
    chol = np.linalg.cholesky(sigma) if chol is None else chol
    return -1 * (nu * np.log(chol.diagonal()).sum() - (nu * D / 2 * np.log(2) + D * (D - 1) / 4 * np.log(np.pi) + special.gammaln((nu - np.arange(D)) / 2).sum()))
...
def multivariate_t_loglik(y, nu, mu, lmbda):
    ...
    return scipy.special.gammaln((nu + d) / 2.0) - scipy.special.gammaln(nu / 2.0) - d / 2.0 * np.log(nu * np.pi) - np.log(L.diagonal()).sum() - (nu + d) / 2.0 * np.log1p(1.0 / nu * inner1d(ys.T, ys.T))
...
def f_statistic(pop1, pop2):
    pop1, pop2 = (flattendata(p) for p in (pop1, pop2))
    var1, var2 = pop1.var(0), pop2.var(0)
    n1, n2 = np.where(var1 >= var2, pop1.shape[0], pop2.shape[0]), np.where(var1 >= var2, pop2.shape[0], pop1.shape[0])
