...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/key/tests/test_key.py
...
import logging, hashlib, test_urls
...
class ApiKeyTest(TestCase):
    ...
    def setUp(self):
        cache.clear()
        create_group()
        self.user = User.objects.create_user(username='ApiKeyTest', email='ApiKeyTest@example.com', password='ApiKeyTestPassword')
        if USE_API_GROUP:
        ...
        kstr = k.key
        self.unauthorized_user = User.objects.create_user(username='NonAuthorized', email='NonAuthorized@example.com', password='NonAuthorizedPassword')
    ...
    def test_key_profile(self):
        ...
        post_save.connect(check_signal, sender=ApiKeyProfile, dispatch_uid='test_check_signal')
    ...
    def test_perm_check(self):
        ...
        self.assertRaises(PermissionDenied, generate_unique_api_key, self.unauthorized_user)
    ...
    def test_views(self):
        ...
        self.assertEquals(rv.status_code, 302)
        client = Client()
        self.assertTrue(client.login(username='ApiKeyTest', password='ApiKeyTestPassword'))
        ...
        self.assertEquals(rv.status_code, 302)
        self.assertEquals(rv['Location'], 'http://testserver' + reverse('key.list'))
