...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/key/models/actions.py
...
def perm_check(user):
    try:
        ...
        if user.has_perm('key.has_api_key_profile'):
            ...
            if created:
                api_user_created.send(sender=user.__class__, instance=user)
            else:
...
def generate_unique_key_code(user):
    ...
    while True:
        ...
        key, created = ApiKey.objects.get_or_create(profile=user.key_profile, key=kstr, created=now)
        if created:
            try:
                key.save()
                return key
...
def send_login_logout_signals(sender, instance, created, *args, **kwargs):
    if instance.logged_ip:
        api_user_logged_in.send(sender=instance.profile.user.__class__, user=instance.profile.user)
    else:
        api_user_logged_out.send(sender=instance.profile.user.__class__, user=instance.profile.user)
