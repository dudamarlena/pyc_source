...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/key/models/models.py
...
def assign_api_key_permissions(user_or_group):
    ...
    can_gen, pc = Permission.objects.get_or_create(name='Can generate an API key', codename='can_make_api_key', content_type=ct)
    ct = ContentType.objects.get(app_label='key', model='apikeyprofile')
    has_prof, pc = Permission.objects.get_or_create(name='Has an API key profile', codename='has_api_key_profile', content_type=ct)
    can_login, pc = Permission.objects.get_or_create(name='Can use the API', codename='can_use_api', content_type=ct)
    perm_list = getattr(user_or_group, 'permissions', getattr(user_or_group, 'user_permissions'))
