...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hubspot/contacts/testing.py
...
from hubspot.contacts.generic_utils import convert_date_to_timestamp_in_milliseconds
from hubspot.contacts.generic_utils import convert_timestamp_in_milliseconds_to_datetime
...
from hubspot.contacts.request_data_formatters.contacts import format_contacts_data_for_saving
from hubspot.contacts.request_data_formatters.properties import format_data_for_property
from hubspot.contacts.request_data_formatters.property_groups import format_data_for_property_group as format_request_data_for_property_group
...
class _PaginatedObjectsRetriever(object):
    ...
    def _get_api_call_for_page(self, page_objects):
        ...
        response_body_deserialization = self._get_response_body_deserialization(page_objects)
        api_call = SuccessfulAPICall(CONTACTS_API_SCRIPT_NAME + self._API_CALL_PATH_INFO, 'GET', query_string_args, response_body_deserialization=response_body_deserialization)
        return api_call
    ...
    def _get_query_string_args(self, page_objects):
        ...
        if 1 < page_number:
            query_string_args_for_page = self._get_query_string_args_for_page(page_number)
    ...
    def _get_response_body_deserialization(self, page_objects):
        ...
        response_body_deserialization = {'has-more': page_has_successors, 
           self._OBJECT_DATA_KEY: page_objects_data}
        response_body_deserialization.update(self._get_response_body_deserialization_for_page(page_objects))
...
class GetAllContacts(_PaginatedObjectsRetriever):
    ...
    _LAST_MODIFIED_DATE_PROPERTY = DatetimeProperty('lastmodifieddate', 'label', 'description', 'group_name', 'text')
    _STUB_LAST_MODIFIED_TIMESTAMP = convert_date_to_timestamp_in_milliseconds(STUB_LAST_MODIFIED_DATETIME)
    ...
    def _get_query_string_args(self, page_contacts):
        query_string_args = super(GetAllContacts, self)._get_query_string_args(page_contacts)
    ...
    def _get_query_string_args_for_page(self, page_number):
        ...
        query_string_args_for_page = {'vidOffset': previous_page_last_contact.vid}
    ...
    def _get_response_body_deserialization_for_page(self, page_contacts):
        ...
        response_body_deserialization_for_page = {'vid-offset': page_last_contact_vid}
        return response_body_deserialization_for_page

    def _get_objects_data(self, contacts):
        contacts_data = []
        for contact in contacts:
            contact_properties_data = self._get_contact_properties_data(contact)
            contact_profiles_data = self._get_contact_profiles_data(contact)
            contact_data = {'vid': contact.vid, 
    ...
    def _get_contact_properties_data(self, contact):
        ...
        stub_last_modified_timestamp_string = str(self._STUB_LAST_MODIFIED_TIMESTAMP)
        stub_lastmodifieddate_property_value_data = self._get_contact_property_value_data(stub_last_modified_timestamp_string)
        contact_properties_data = {self._LAST_MODIFIED_DATE_PROPERTY.name: stub_lastmodifieddate_property_value_data}
        for property_name in self._property_names:
            if property_name == 'email' and contact.email_address:
                assert 'email' in contact_properties and contact.email_address == contact_properties['email']
                property_value = contact.email_address
            elif property_name not in contact_properties:
                continue
            else:
                property_value = self._get_property_value(property_name, contact_properties)
            contact_properties_data[property_name] = self._get_contact_property_value_data(property_value)
    ...
    def _get_contact_profiles_data(contact):
        contact_profile_data = {'vid': contact.vid, 
           'identities': [{'type': 'LEAD_GUID', 'value': get_uuid4_str()}, {'type': 'EMAIL', 'value': contact.email_address}]}
...
class _UnsucessfulContactRetrievalSimulator(object):
    ...
    def __init__(self, successful_simulator_class, contacts, exception, *args, **kwargs):
        super(_UnsucessfulContactRetrievalSimulator, self).__init__()
        self._successful_api_call_simulator = successful_simulator_class(contacts, *args, **kwargs)
    ...
    def __call__(self):
        ...
        if contacts_by_page:
            ...
            unsuccessful_api_call_query_string_args = last_api_call.query_string_args.copy()
            unsuccessful_api_call_pagination_query_string_args = successful_api_call_simulator._get_query_string_args_for_page(unsuccessful_api_call_number)
            unsuccessful_api_call_query_string_args.update(unsuccessful_api_call_pagination_query_string_args)
            unsuccessful_api_call = UnsuccessfulAPICall(last_api_call.url_path, last_api_call.http_method, unsuccessful_api_call_query_string_args, exception=self._exception)
        else:
            last_api_call = api_calls.pop()
            unsuccessful_api_call = UnsuccessfulAPICall(last_api_call.url_path, last_api_call.http_method, last_api_call.query_string_args, exception=self._exception)
        api_calls.append(unsuccessful_api_call)
        return api_calls


UnsuccessfulGetAllContacts = partial(_UnsucessfulContactRetrievalSimulator, GetAllContacts)
...
class GetAllContactsByLastUpdate(GetAllContacts):
    ...
    _MOST_RECENT_CONTACT_UPDATE_TIMESTAMP = convert_date_to_timestamp_in_milliseconds(MOST_RECENT_CONTACT_UPDATE_DATETIME)

    def __init__(self, contacts, available_properties, property_names=(), cutoff_datetime=None):
        r"""
        ...
        """
        filtered_contacts = self._exclude_contacts_pages_after_cutoff_datetime(contacts, cutoff_datetime)
        super(GetAllContactsByLastUpdate, self).__init__(filtered_contacts, available_properties, property_names=property_names)
    ...
    def _get_query_string_args_for_page(self, page_number):
        ...
        query_string_args_for_page = super_._get_query_string_args_for_page(page_number)
        page_index = page_number - 1
        previous_page_contacts = self._objects_by_page[(page_index - 1)]
        previous_page_last_contact = previous_page_contacts[(-1)]
        query_string_args_for_page['timeOffset'] = self._get_contact_added_at_timestamp(previous_page_last_contact)
        return query_string_args_for_page

    def _get_response_body_deserialization_for_page(self, page_contacts):
        super_ = super(GetAllContactsByLastUpdate, self)
        response_body_deserialization_for_page = super_._get_response_body_deserialization_for_page(page_contacts)
        if page_contacts:
            page_last_contact = page_contacts[(-1)]
            time_offset = self._get_contact_added_at_timestamp(page_last_contact)
        else:
            time_offset = 0
    ...
    def _get_objects_data(self, contacts):
        contacts_data = super(GetAllContactsByLastUpdate, self)._get_objects_data(contacts)
        for contact, contact_data in zip(contacts, contacts_data):
            contact_data['addedAt'] = self._get_contact_added_at_timestamp(contact)
    ...
    def get_contact_added_at_datetime(cls, contact, contacts):
        ...
        contact_added_at_timestamp = cls._MOST_RECENT_CONTACT_UPDATE_TIMESTAMP - contact_index
        contact_added_at_datetime = convert_timestamp_in_milliseconds_to_datetime(contact_added_at_timestamp)
        return contact_added_at_datetime

    def _get_contact_added_at_timestamp(self, contact):
        contact_added_at_datetime = self.get_contact_added_at_datetime(contact, self._contacts)
        contact_added_at_timestamp = convert_date_to_timestamp_in_milliseconds(contact_added_at_datetime)
        return contact_added_at_timestamp


UnsuccessfulGetAllContactsByLastUpdate = partial(_UnsucessfulContactRetrievalSimulator, GetAllContactsByLastUpdate)
    ...
    def __call__(self):
        if not self._contacts_by_page:
            ...
            for batch_contacts in self._contacts_by_page:
                request_body_deserialization = format_contacts_data_for_saving(batch_contacts, self._property_type_by_property_name)
                api_call = SuccessfulAPICall(CONTACTS_API_SCRIPT_NAME + '/contact/batch/', 'POST', request_body_deserialization=request_body_deserialization, response_body_deserialization=None)
                api_calls.append(api_call)
...
class UnsuccessfulSaveContacts(SaveContacts):
    ...
    def __init__(self, contacts, exception, available_properties):
        ...
        """
        super(UnsuccessfulSaveContacts, self).__init__(contacts, available_properties)
        self._exception = exception
    ...
    def __call__(self):
        ...
        if api_calls:
            last_api_call = api_calls.pop()
            unsuccessful_api_call = UnsuccessfulAPICall(last_api_call.url_path, last_api_call.http_method, last_api_call.query_string_args, last_api_call.request_body_deserialization, exception=self._exception)
            api_calls.append(unsuccessful_api_call)
        return api_calls
...
class GetAllProperties(object):
    ...
    def __init__(self, properties):
        ...
        :param iterable properties:             :class:`~hubspot.contacts.properties.Property` instances             for all the properties supposedly defined in the portal.
    ...
    def __call__(self):
        response_body_deserialization = _format_response_data_for_properties(self._properties)
        get_all_properties_api_call = SuccessfulAPICall(CONTACTS_API_SCRIPT_NAME + '/properties', 'GET', response_body_deserialization=response_body_deserialization)
        return [
         get_all_properties_api_call]
...
class _BaseCreateProperty(object):
    ...
    def __init__(self, property_):
        ...
        :param hubspot.contacts.properties.Property property_: The property             to be supposedly created.
    ...
    def _get_api_call(self):
        url_path = CONTACTS_API_SCRIPT_NAME + '/properties/' + self._property.name
        property_data = format_data_for_property(self._property)
        api_call = APICall(url_path, 'PUT', request_body_deserialization=property_data)
        return api_call
...
class CreateProperty(_BaseCreateProperty):
    ...
    def _get_api_call(self):
        ...
        api_call = SuccessfulAPICall.init_from_generalization(super_, response_body_deserialization=super_.request_body_deserialization)
        return api_call
...
class UnsuccessfulCreateProperty(_BaseCreateProperty):
    ...
    def _get_api_call(self):
        super_ = super(UnsuccessfulCreateProperty, self)._get_api_call()
        api_call = UnsuccessfulAPICall.init_from_generalization(super_, exception=self._exception)
        return api_call
...
class DeleteProperty(object):
    ...
    def __init__(self, property_name):
        ...
        :param basestring property_name: The name of the property to be             supposedly deleted.
    ...
    def __call__(self):
        url_path = CONTACTS_API_SCRIPT_NAME + '/properties/' + self._property_name
        api_call = SuccessfulAPICall(url_path, 'DELETE', response_body_deserialization=None)
        return [
         api_call]
...
class GetAllPropertyGroups(object):
    ...
    def __init__(self, property_groups):
        ...
        :param iterable property_groups:             :class:`~hubspot.contacts.property_groups.PropertyGroup` instances             for all the property groups supposedly defined in the portal.
    ...
    def __call__(self):
        ...
        for property_group in self._property_groups:
            property_group_data = _format_response_data_for_property_group(property_group)
            property_groups_data.append(property_group_data)

        api_call = SuccessfulAPICall(CONTACTS_API_SCRIPT_NAME + '/groups', 'GET', response_body_deserialization=property_groups_data)
        return [
         api_call]
...
class _BaseCreatePropertyGroup(object):
    ...
    def __init__(self, property_group):
        ...
        :param hubspot.contacts.property_groups.PropertyGroup property_group:             Property group to be supposedly created.
    ...
    @abstractmethod
    def _get_api_call(self):
        url_path = CONTACTS_API_SCRIPT_NAME + '/groups/' + self._property_group.name
        request_body_deserialization = format_request_data_for_property_group(self._property_group)
        api_call = APICall(url_path, 'PUT', request_body_deserialization=request_body_deserialization)
        return api_call
...
class CreatePropertyGroup(_BaseCreatePropertyGroup):
    ...
    def _get_api_call(self):
        ...
        api_call = SuccessfulAPICall.init_from_generalization(super_, response_body_deserialization=response_body_deserialization)
        return api_call
...
class UnsuccessfulCreatePropertyGroup(_BaseCreatePropertyGroup):
    ...
    def _get_api_call(self):
        super_ = super(UnsuccessfulCreatePropertyGroup, self)._get_api_call()
        api_call = UnsuccessfulAPICall.init_from_generalization(super_, exception=self._exception)
        return api_call


def _format_response_data_for_property_group(property_group):
    property_group_data = {'name': property_group.name, 
       'displayName': property_group.display_name or '', 
       'displayOrder': 1, 
       'portalId': 1}
    if property_group.properties:
        property_group_data['properties'] = _format_response_data_for_properties(property_group.properties)
...
class DeletePropertyGroup(object):
    ...
    def __init__(self, property_group_name):
        ...
        :param basestring property_group_name: The name of the property group             to be supposedly deleted.
    ...
    def __call__(self):
        url_path = CONTACTS_API_SCRIPT_NAME + '/groups/' + self._property_group_name
        api_call = SuccessfulAPICall(url_path, 'DELETE', response_body_deserialization=None)
        return [
         api_call]
...
class GetAllContactLists(_PaginatedObjectsRetriever):
    ...
    def _get_query_string_args_for_page(self, page_number):
        query_string_args_for_page = {'offset': BATCH_RETRIEVAL_SIZE_LIMIT * (page_number - 1)}
        return query_string_args_for_page
    ...
    def _get_response_body_deserialization_for_page(self, page_contact_lists):
        ...
        response_body_deserialization_for_page = {'offset': BATCH_RETRIEVAL_SIZE_LIMIT * page_number}
        return response_body_deserialization_for_page
    ...
    def _get_objects_data(contact_lists):
        ...
        for contact_list in contact_lists:
            contact_list_data = {'listId': contact_list.id, 'name': contact_list.name, 
...
class _BaseCreateStaticContactList(object):
    ...
    @abstractmethod
    def _get_api_call(self):
        request_body_deserialization = {'name': self._contact_list_name, 
           'dynamic': False}
        api_call = APICall(CONTACTS_API_SCRIPT_NAME + '/lists', 'POST', request_body_deserialization=request_body_deserialization)
        return api_call
...
class UnsuccessfulCreateStaticContactList(_BaseCreateStaticContactList):
    ...
    def _get_api_call(self):
        super_ = super(UnsuccessfulCreateStaticContactList, self)._get_api_call()
        api_call = UnsuccessfulAPICall.init_from_generalization(super_, exception=self._exception)
        return api_call
...
class DeleteContactList(object):
    ...
    def __init__(self, contact_list_id):
        ...
        :param basestring contact_list_id: The id of the static list to be             supposedly deleted
    ...
    def __call__(self):
        url_path = ('{}/lists/{}').format(CONTACTS_API_SCRIPT_NAME, self._contact_list_id)
        api_call = SuccessfulAPICall(url_path, 'DELETE', response_body_deserialization=None)
        return [
         api_call]
...
class _UpdateContactListMembership(object):
    ...
    def __init__(self, contact_list, contacts_to_update, updated_contacts):
        ...
        self._contacts_by_page = paginate(contacts_to_update, BATCH_SAVING_SIZE_LIMIT)
        self._updated_contacts = updated_contacts

    def __call__(self):
        api_calls = []
        for contacts_page in self._contacts_by_page:
            request_body_deserialization = {'vids': self._get_contact_vids(contacts_page)}
            updated_contacts_in_page = [ c for c in self._updated_contacts if c in contacts_page ]
            response_body_deserialization = {'updated': self._get_contact_vids(updated_contacts_in_page)}
            path_info = ('/lists/{}/{}').format(self._contact_list.id, self.url_path_list_action)
            api_call = SuccessfulAPICall(CONTACTS_API_SCRIPT_NAME + path_info, 'POST', request_body_deserialization=request_body_deserialization, response_body_deserialization=response_body_deserialization)
            api_calls.append(api_call)
...
class AddContactsToList(_UpdateContactListMembership):
    ...
    def __init__(self, contact_list, contacts_to_add, updated_contacts):
        ...
        """
        super(AddContactsToList, self).__init__(contact_list, contacts_to_add, updated_contacts)
...
class RemoveContactsFromList(_UpdateContactListMembership):
    ...
    def __init__(self, contact_list, contacts_to_remove, updated_contacts):
        ...
        """
        super(RemoveContactsFromList, self).__init__(contact_list, contacts_to_remove, updated_contacts)
...
class GetContactsFromList(GetAllContacts):
    ...
    def __init__(self, contact_list, contacts, available_properties, property_names=()):
