...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hubspot/contacts/_data_retrieval.py
# Compiled at: 2017-11-13 03:36:59
import re
from voluptuous import Schema
from hubspot.contacts._constants import BATCH_RETRIEVAL_SIZE_LIMIT
_CAMEL_CASE_CONVERSION_RE = re.compile('\\-(\\w)')

class PaginatedDataRetriever(object):

    def __init__(self, response_data_key, response_offset_keys, page_size=BATCH_RETRIEVAL_SIZE_LIMIT):
        self._response_data_key = response_data_key
        self._response_offset_keys = response_offset_keys
        self._page_size = page_size
        self._offset_url_param_name_by_response_key = {k:_convert_to_camel_case(k) for k in self._response_offset_keys}
        self._schema = self._get_response_data_schema()

    def get_data(self, connection, path_info, query_string_args=None):
        data_by_page = self._get_data_by_page(path_info, query_string_args, connection)
    ...
    def _get_data_by_page(self, path_info, query_string_args, connection):
        ...
        while has_more_pages:
            ...
            yield response_data
            next_request_offset = _filter_dict(response, self._response_offset_keys)
            next_request_offset_query_string_args = _translate_dict_keys(next_request_offset, self._offset_url_param_name_by_response_key)
