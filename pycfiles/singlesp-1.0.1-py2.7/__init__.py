...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/singlesp/__init__.py
# Compiled at: 2016-08-16 13:00:29
"""
singlesp.py
===========
...
class Proc(object):
    ...
    def run(self):
        ...
        if self.cb_err:
            self.callbacks.append(lambda self: self.cb_err(Reader(self, self.stderr)))
        if self.cb_out:
            self.callbacks.append(lambda self: self.cb_out(Reader(self, self.stdout)))
    ...
    def wait(self, check=False):
        ...
        if check and self.status:
            raise ProcFailedException(self.status, self.args, self.p.stderr.read())
        return self
...
class ProcFactory(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.env:
            ...
            kwargs['env'] = env
        if args:
            assert isinstance(args[0], (list, tuple)), 'Args[0] is not a list or tuple instance: %s:%r' % (
    ...
    def new(self, env=None, mgr=None, options=None, **kwargs):
        ...
        if env:
            env_.update(env)
        return self.__class__(self.name, cmd=cmd, env=env_, mgr=mgr or self.mgr)
...
class SSHProcFactory(ProcFactory):
    ...
    def __init__(self, name, cmd=None, **kwargs):
        if not cmd:
            cmd = "ssh-keygen -R %(host)s >/dev/null 2>&1 || true;sshpass -e ssh -oStrictHostKeyChecking=no '%(user)s@%(host)s' -p %(port)s '{cmd};exit $?'"
    ...
    def connector(self, host, user, password, port=22):
        return self.new(env={'SSHPASS': password}, options=dict(user=user, host=host, port=port))
