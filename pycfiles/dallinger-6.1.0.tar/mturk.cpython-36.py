...
# Size of source mod 2**32: 24535 bytes
...
class MTurkService(object):
    ...
    def __init__(self, aws_access_key_id, aws_secret_access_key, region_name, sandbox=True, max_wait_secs=0, subscribe=True):
        self.aws_key = aws_access_key_id
        self.aws_secret = aws_secret_access_key
        self.region_name = region_name
    ...
    def build_hit_qualifications(self, approve_requirement, restrict_to_usa, blacklist):
        ...
        if restrict_to_usa:
            quals.append({'QualificationTypeId':LOCALE_REQUIREMENT_ID, 
             'Comparator':'EqualTo', 
             'LocaleValues':[
              {'Country': 'US'}], 
             'RequiredToPreview':True})
        if blacklist is not None:
            for item in blacklist:
                ...
                if qtype:
                    quals.append({'QualificationTypeId':qtype['id'], 
    ...
    def create_hit(self, experiment_id, title, description, keywords, reward, duration_hours, lifetime_days, ad_url, notification_url, approve_requirement, max_assignments, us_only, blacklist=None, annotation=None):
