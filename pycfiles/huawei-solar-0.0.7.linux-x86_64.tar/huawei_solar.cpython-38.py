...
# Compiled at: 2020-05-11 09:18:26
# Size of source mod 2**32: 30549 bytes
"""
            ...
            if reg.type == 'u16':
                if reg.unit == 'status_enum':
                    ...
                    if reg.type == 'u16' and reg.unit == 'grid_enum':
                        ...
                        result = GRID_CODES.get(tmp, GridCode('unknown/not implemented', 'unknown/not implemented'))
                    else:
...
class AsyncHuaweiSolar:
 ...
 L. 284        14  LOAD_FAST                'client'
               16  LOAD_ATTR                connected
               18  POP_JUMP_IF_TRUE     42  'to 42'

 L. 285        20  LOAD_STR                 'failed to connect to device, is the host correct?'
               22  STORE_FAST               'message'

 L. 286        24  LOAD_GLOBAL              LOGGER
               26  LOAD_METHOD              exception
               28  LOAD_FAST                'message'
               30  CALL_METHOD_1         1  ''
               32  POP_TOP          

 L. 287        34  LOAD_GLOBAL              ConnectionException
               36  LOAD_FAST                'message'
               38  CALL_FUNCTION_1       1  ''
               40  RAISE_VARARGS_1       1  'exception instance'
             42_0  COME_FROM            18  '18'

 L. 288        42  SETUP_FINALLY        98  'to 98'

 L. 289        44  LOAD_GLOBAL              asyncio
               46  LOAD_ATTR                wait_for

 L. 290        48  LOAD_FAST                'client'
 ...
 L. 291        60  LOAD_FAST                'self'
               62  LOAD_ATTR                timeout

 L. 289        64  LOAD_CONST               ('timeout',)
 ...
 L. 298       144  LOAD_STR                 'could not read register value, is an other device already connected?'

 L. 297       146  STORE_FAST               'message'

 L. 301       148  LOAD_GLOBAL              LOGGER
              150  LOAD_METHOD              error
              152  LOAD_FAST                'message'
              154  CALL_METHOD_1         1  ''
              156  POP_TOP          

 L. 302       158  LOAD_GLOBAL              ReadException
 ...
 L. 305       174  LOAD_STR                 'could not read register value for unknown reason'
              176  STORE_FAST               'message'

 L. 306       178  LOAD_GLOBAL              LOGGER
              180  LOAD_METHOD              error
              182  LOAD_FAST                'message'
              184  CALL_METHOD_1         1  ''
              186  POP_TOP          

 L. 307       188  LOAD_GLOBAL              ReadException
