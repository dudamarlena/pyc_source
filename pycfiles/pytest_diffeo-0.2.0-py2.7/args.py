...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pytest_diffeo/args.py
...
import logging, os.path, pstats, time
try:
...
def pytest_addoption(parser):
    ...
    group.addoption('--runslow', action='store_true', help='run known-slow tests')
    group.addoption('--runperf', action='store_true', help='run performance tests')
    group.addoption('--runload', action='store_true', help='run load tests')
    group.addoption('--run-integration', action='store_true', help='run integration tests')
    group = parser.getgroup('external systems')
    group.addoption('--ingest-v2', metavar='URL', help='URL for Streamcorpus v2 ingest service')
    group.addoption('--elastic-address', metavar='HOST:PORT', help='location of an ElasticSearch database server')
    group.addoption('--redis-address', metavar='HOST:PORT', help='location of a Redis database server')
    group.addoption('--third-dir', metavar='THIRD-DIR', help='location of a third party software')
    group.addoption('--external-data', metavar='EXTERNAL_DATA', help='location of a external data resources')
    group = parser.getgroup('general')
    group.addoption('--profile', metavar='path', help='run tests with profiling, write results to file')
    group.addoption('--profile-truncate', action='store_true', default=False, help='when profiling, truncate output file at start')
    group.addoption('--log-level', metavar='DEBUG|INFO|WARNING|ERROR|FATAL', default='DEBUG', help='Control logging level of tests.')


def pytest_configure(config):
    config.addinivalue_line('markers', 'slow: mark tests as taking longer than your average unit test')
    config.addinivalue_line('markers', 'performance: mark tests as performance tests')
    config.addinivalue_line('markers', 'load: mark tests as load tests')
...
def redis_address(request):
    ...
    if addr is None:
        ...
        if host and port:
            addr = host + ':' + port
    assert addr is not None, 'this test requires --redis-address on the command line'
    return addr
...
def elastic_address(request):
    ...
    if addr is None:
        ...
        if host and port:
            addr = host + ':' + port
    assert addr is not None, 'this test requires --elastic-address on the command line'
    return addr
...
def third_dir(request):
    ...
    assert third_dir is not None, 'this test requires --third_dir on the command line'
...
def external_data(request):
    ...
    if external_data is None:
        ...
        assert os.path.exists(external_data), 'Could not find external_data=%r' % external_data
