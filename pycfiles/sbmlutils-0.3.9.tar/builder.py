...
import inspect, libsbml
MODEL_FRAMEWORK_FBA = 'fba'
MODEL_FRAMEWORK_ODE = 'ode'
MODEL_FRAMEWORKS = {MODEL_FRAMEWORK_FBA: [
                       'SBO:0000624'], 
   MODEL_FRAMEWORK_ODE: [
                       'SBO:0000293']}
DT_ID = 'dt'
...
def create_dfba_compartment(model, compartment_id, unit_volume=None, create_port=True):
    ...
    """
    objects = [
     fac.Compartment(sid=compartment_id, value=1.0, unit=unit_volume, constant=True, name=compartment_id, spatialDimensions=3)]
...
def create_dfba_species(model, model_fba, compartment_id, hasOnlySubstanceUnits=False, unit=None, create_port=True):
    ...
    for ex_rid in ex_rids:
        ...
        objects.append(fac.Species(sid=sid, name=s.getName(), value=1.0, unit=unit, hasOnlySubstanceUnits=hasOnlySubstanceUnits, compartment=compartment_id))
...
def create_exchange_reaction(model, species_id, exchange_type=EXCHANGE, flux_unit=None):
    ...
    if exchange_type == EXCHANGE_EXPORT:
        lb_value = ZERO_BOUND
    parameters = [
     fac.Parameter(sid=lb_id, value=lb_value, unit=flux_unit, constant=True, sboTerm=FLUX_BOUND_SBO),
     fac.Parameter(sid=ub_id, value=ub_value, unit=flux_unit, constant=True, sboTerm=FLUX_BOUND_SBO)]
    fac.create_objects(model, parameters)
    ex_r = fac.create_reaction(model, rid=ex_rid, reversible=True, reactants={species_id: 1}, sboTerm=EXCHANGE_REACTION_SBO)
...
def create_update_reaction(model, sid, modifiers=[], formula='-{}'):
    ...
    fac.create_reaction(model, rid=rid_update, sboTerm=UPDATE_REACTION_SBO, reactants={sid: 1}, modifiers=modifiers, formula=formula)
...
def create_dummy_species(model, compartment_id, unit=None, hasOnlySubstanceUnits=False):
    ...
    fac.create_objects(model, [
     fac.Species(sid=DUMMY_SPECIES_ID, name=DUMMY_SPECIES_ID, value=0, unit=unit, hasOnlySubstanceUnits=hasOnlySubstanceUnits, compartment=compartment_id, sboTerm=DUMMY_SPECIES_SBO)])
...
def create_dummy_reactions(model, model_fba, unit_flux=None):
    ...
    for ex_rid, sid in iteritems(ex_rids):
        ...
        objects.append(fac.Parameter(sid=pid_flux, value=1.0, constant=True, unit=unit_flux, sboTerm=FLUX_PARAMETER_SBO))
        fac.create_reaction(model, rid=rid_flux, reversible=False, products={DUMMY_SPECIES_ID: 1}, sboTerm=DUMMY_REACTION_SBO, formula=('0 {}').format(unit_flux))
        objects.append(fac.AssignmentRule(pid_flux, value=rid_flux, sboTerm=FLUX_ASSIGNMENTRULE_SBO))
...
def create_top_replacedBy(model, model_fba):
    ...
    ex_rids = utils.find_exchange_reactions(model_fba)
    for ex_rid, sid in iteritems(ex_rids):
        comp.replaced_by(model, DUMMY_REACTION_PREFIX + sid, ref_type=comp.SBASE_REF_TYPE_PORT, submodel='fba', replaced_by=('{}_port').format(EXCHANGE_REACTION_PREFIX + sid))
