...
# [GCC 8.4.0]
# Embedded file name: /home/mkoenig/git/sbmlutils/sbmlutils/dfba/simulator.py
# Compiled at: 2019-10-27 06:58:29
# Size of source mod 2**32: 17466 bytes
"""
...
def simulate_dfba(sbml_path, tstart=0.0, tend=10.0, dt=0.1, pfba=True, abs_tol=1e-06, rel_tol=1e-06, lp_solver='glpk', ode_integrator='cvode', **kwargs):
    ...
    dfba_simulator = DFBASimulator(dfba_model, pfba=pfba, abs_tol=abs_tol,
      rel_tol=rel_tol,
      lp_solver=lp_solver,
...
class DFBASimulator(object):
    ...
    def __init__(self, dfba_model, abs_tol=1e-06, rel_tol=1e-06, lp_solver='glpk', ode_integrator='cvode', pfba=True, check_uniqueness=True):
    ...
    def _set_timecourse_selections(self):
        ...
        sel = [
         'time'] + sorted([''.join(['[', item, ']']) for item in rr.getFloatingSpeciesIds()]) + sorted([''.join(['[', item, ']']) for item in rr.getBoundarySpeciesIds()]) + sorted(rr.getReactionIds()) + sorted(rr.getGlobalParameterIds())
        self.ode_model.timeCourseSelections = sel
        self.columns = dict(zip(sel, range(len(sel))))

    def simulate(self, tstart=0.0, tend=10.0, dt=0.1, absTol=1e-06, relTol=1e-06, reset=True, show_settings=True):
        ...
        try:
            ...
            df_results = pd.DataFrame(index=all_time, columns=(self.ode_model.timeCourseSelections), data=all_results)
            ...
            import traceback
            traceback.print_exc()
            df_results = pd.DataFrame(columns=(self.ode_model.timeCourseSelections), data=all_results)
            df_results.time = all_time[:len(all_results)]

        self.solution = df_results
        self.simulation_time = timeit.default_timer() - start_time
        self.all_fva = all_fva
        self.unique = pd.DataFrame(data=[np.sum(df.maximum - df.minimum) / len(df) < 1e-06 for df in all_fva], index=(self.solution.time), columns=['unique'])
    ...
    def _is_fba_unique(self, tol=1e-06):
