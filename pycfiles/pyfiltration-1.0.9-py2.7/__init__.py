...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyfiltration/__init__.py
# Compiled at: 2016-03-29 04:35:43
__version__ = '1.0.9'
import logging, datetime, functools, re, types, urlparse
_logger = logging.getLogger('pyfiltration')
__all__ = [
 'FiltrationException', 'Item', 'filtration', 'Helper']
...
class _Filtration(object):
    ...
    def get_request_detail(self):
        ...
        return ('<{f_module}.{f_class}>({method})').format(f_module=f_module, f_class=f_class, method=method)
    ...
    def convert_data_type(self, item, value):
        try:
            ...
            if item.type_ == str:
                return value
            raise FiltrationException(('{} {}: type({}) not support').format(self.get_request_detail(), item.name, item.type_), item.err_message)
        except:
            raise FiltrationException(('{} {}: type error({} required)').format(self.get_request_detail(), item.name, item.type_), item.err_message)
    ...
    def _parse_item(self, item, value):
        ...
        for funcs in item.validates:
            try:
                ...
                raise FiltrationException(('{} {}: validate failed: {}({}) {}').format(self.get_request_detail(), item.name, funcs.f_, value, e), funcs.err_message)

            if not result:
                raise FiltrationException(('{} {}: validate failed: {}({})').format(self.get_request_detail(), item.name, funcs.f_, value), funcs.err_message)
        ...
        for funcs in item.uses:
            try:
                ...
                raise FiltrationException(('{} {}: uses failed: {}({}) {}').format(self.get_request_detail(), item.name, funcs.f_, value, e), funcs.err_message)
...
class Item(object):
    ...
    def get_func_details(self, funcs):
        ...
        assert isinstance(funcs[0], types.FunctionType) or isinstance(funcs[0], types.LambdaType) or isinstance(funcs[0], types.MethodType)
        ...
        if len(funcs) == 3:
            err_message = funcs[2]
        return _ObjectDict(func=funcs[0], args=args, err_message=err_message, f_=('<{f_module}.{f_class}.{f_name}>({args})').format(f_module=f_module, f_class=f_class, f_name=f_name, args=args))


class Helper(object):
    EMAIL_ADDRESS_REG = re.compile('(^[-!#$%&\'*+/=?^_`{}|~0-9A-Z]+(\\.[-!#$%&\'*+/=?^_`{}|~0-9A-Z]+)*|^"([\\001-\\010\\013\\014\\016-\\037!#-\\[\\]-\\177]|\\\\[\\001-\\011\\013\\014\\016-\\177])*")@(?:[A-Z0-9](?:[A-Z0-9-]{0,247}[A-Z0-9])?\\.)+(?:[A-Z]{2,6}|[A-Z0-9-]{2,}(?<!-))$', re.IGNORECASE)
    EMAIL_ADDRESS_SUFFIX_REG = re.compile('^@(?:[A-Z0-9](?:[A-Z0-9-]{0,247}[A-Z0-9])?\\.)+(?:[A-Z]{2,6}|[A-Z0-9-]{2,}(?<!-))$', re.IGNORECASE)
    URL_ADDRESS_REG = re.compile("^(https?|ftp):\\/\\/(((([a-z]|\\d|-|\\.|_|~|[\\u00A0-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFEF])|(%[\\da-f]{2})|[!\\$&'\\(\\)\\*\\+,;=]|:)*@)?(((\\d|[1-9]\\d|1\\d\\d|2[0-4]\\d|25[0-5])\\.(\\d|[1-9]\\d|1\\d\\d|2[0-4]\\d|25[0-5])\\.(\\d|[1-9]\\d|1\\d\\d|2[0-4]\\d|25[0-5])\\.(\\d|[1-9]\\d|1\\d\\d|2[0-4]\\d|25[0-5]))|((([a-z]|\\d|[\\u00A0-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFEF])|(([a-z]|\\d|[\\u00A0-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFEF])([a-z]|\\d|-|\\.|_|~|[\\u00A0-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFEF])*([a-z]|\\d|[\\u00A0-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFEF])))\\.)+(([a-z]|[\\u00A0-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFEF])|(([a-z]|[\\u00A0-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFEF])([a-z]|\\d|-|\\.|_|~|[\\u00A0-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFEF])*([a-z]|[\\u00A0-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFEF])))\\.?)(:\\d*)?)(\\/((([a-z]|\\d|-|\\.|_|~|[\\u00A0-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFEF])|(%[\\da-f]{2})|[!\\$&'\\(\\)\\*\\+,;=]|:|@)+(\\/(([a-z]|\\d|-|\\.|_|~|[\\u00A0-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFEF])|(%[\\da-f]{2})|[!\\$&'\\(\\)\\*\\+,;=]|:|@)*)*)?)?(\\?((([a-z]|\\d|-|\\.|_|~|[\\u00A0-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFEF])|(%[\\da-f]{2})|[!\\$&'\\(\\)\\*\\+,;=]|:|@)|[\\uE000-\\uF8FF]|\\/|\\?)*)?(\\#((([a-z]|\\d|-|\\.|_|~|[\\u00A0-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFEF])|(%[\\da-f]{2})|[!\\$&'\\(\\)\\*\\+,;=]|:|@)|\\/|\\?)*)?$")
    STRING_HAS_NUMBER = 1
    STRING_HAS_LETTER = 2
    ...
    def is_valid_string(cls, val, exists=None):
        if not exists:
            ...
            for cond in set(exists):
                if cond == cls.STRING_HAS_NUMBER and re.search('\\d', val) is None:
                    return False
                if cond == cls.STRING_HAS_LETTER and re.search('[a-zA-Z]', val) is None:
                    return False
    ...
    def is_valid_random_token(cls, val, length=32):
        return re.match('^([0-9a-z]){' + ('{}').format(length) + '}$', val) is not None

    @classmethod
    def is_valid_telephone(cls, val):
        return re.match('^1[3,4,5,7,8]{1}[0-9]{9}$', val) is not None
