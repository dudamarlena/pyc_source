...
class ShotList(object):
    ...
    def get_shots_and_disruption_times(shots_and_disruption_times_path):
        data = np.loadtxt(shots_and_disruption_times_path, ndmin=1, dtype={'names': ('num', 'disrupt_times'), 'formats': ('i4', 'f4')})
    ...
    def split_train_test(self, conf):
        ...
        return (
         shots_train.random_sublist(use_shots_train), shots_test.random_sublist(use_shots_test))

    def split_direct(self, frac, do_shuffle=True):
        shot_list_one, shot_list_two = train_test_split(self.shots, frac, do_shuffle)
        return (
         ShotList(shot_list_one), ShotList(shot_list_two))
...
class Shot(object):
    ...
    def save(self, prepath):
        ...
        np.savez(save_path, number=self.number, valid=self.valid, is_disruptive=self.is_disruptive, signals=self.signals, ttd=self.ttd)
