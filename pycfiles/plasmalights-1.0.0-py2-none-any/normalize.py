...
# Embedded file name: build/bdist.linux-x86_64/egg/plasma/preprocessor/normalize.py
# Compiled at: 2017-02-17 21:50:27
__doc__ = '\n#########################################################\nThis file containts classes to handle data processing\n\nAuthor: Julian Kates-Harbeck, jkatesharbeck@g.harvard.edu\n\nThis work was supported by the DOE CSGF program.\n#########################################################\n'
from __future__ import print_function
import os, time, sys, abc, numpy as np
...
class MeanVarNormalizer(Normalizer):
    ...
    def save_stats(self):
        np.savez(self.path, means=self.means, stds=self.stds, num_processed=self.num_processed, num_disruptive=self.num_disruptive)
...
class MinMaxNormalizer(Normalizer):
    ...
    def incorporate_stats(self, stats):
        if stats.valid:
            ...
            if self.num_processed == 0:
                ...
                self.minimums = (self.num_processed * self.minimums + minimums) / (self.num_processed + 1.0)
                self.maximums = (self.num_processed * self.maximums + maximums) / (self.num_processed + 1.0)
            self.num_processed = self.num_processed + 1
            self.num_disruptive = self.num_disruptive + (1 if stats.is_disruptive else 0)
    ...
    def save_stats(self):
        np.savez(self.path, minimums=self.minimums, maximums=self.maximums, num_processed=self.num_processed, num_disruptive=self.num_disruptive)
        print(('saved normalization data from {} shots ( {} disruptive )').format(self.num_processed, self.num_disruptive))
