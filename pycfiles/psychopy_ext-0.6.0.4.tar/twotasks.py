...
# [GCC 8.4.0]
# Embedded file name: /Users/qbilius/Dropbox (MIT)/psychopy_ext/psychopy_ext/demos/scripts/twotasks.py
# Compiled at: 2015-12-11 19:53:08
"""
...
class TwoTasks(exp.Experiment):
    ...
    *(Use 'Left Shift + Esc' to exit.)*
    ...
    def __init__(self, name='exp', info=('subjid', 'twotasks_'), rp=(
 'phase', ('both', 'train', 'test'))):
        super(TwoTasks, self).__init__(name=name, info=info, rp=rp, actions='run', paths=PATHS, computer=computer)
        self.nsessions = 3
...
class _Test(exp.Task):
    ...
    *(Use 'Left Shift + Esc' to exit.)*
    ...
    def __init__(self, parent, session=1):
        ...
        super(_Test, self).__init__(parent, method='random', blockcol='pos', data_fname=data_fname)
        self.session = session
        self.oris = parent.oris
        self.stim_size = parent.stim_size
        self.stim_dist = parent.stim_dist
...
class Analysis(object):
    ...
    def __init__(self, name='analysis', info={'subjid': 'twotasks_'}, rp={'all': False}, actions='test'):
        self.name = name
        self.info = info
        self.rp = rp
        self.paths = PATHS
