...
# [GCC 8.4.0]
# Embedded file name: /mindhive/dicarlolab/u/qbilius/Dropbox (MIT)/psychopy_ext/psychopy_ext/stats.py
# Compiled at: 2016-07-22 06:52:32
"""
...
import numpy as np, scipy.stats, pandas
...
def _signal_detection(df, values, signal_noise_col, groupby=None, signal='signal', noise='noise', hits='correct', fas='incorrect'):
    if len(df[signal_noise_col].unique()) != 2:
        raise ValueError('There must be two values in the %s column.' % signal_noise_col)
    if groupby is None:
        groupby = [
         signal_noise_col]
    elif isinstance(groupby, (str, unicode)):
        groupby = [
...
def d_prime(df, values, signal_noise_col, groupby=None, signal='signal', noise='noise', hits='correct', fas='incorrect'):
    """
    ...
    :Kwargs:
      ...
      :Returns:
          A pandas.DataFrame.
    """
    agg = _signal_detection(df, values, signal_noise_col, groupby=groupby, signal=signal, noise=noise, hits=hits, fas=fas)
...
def a_prime(df, values=None, hits='correct', fas='incorrect', **kwargs):
    ...
    ap = 0.5 + np.sign(d) * (d ** 2 + np.abs(d)) / (4 * np.max(agg[signal], agg[noise]) - 4 * agg[signal] * agg[noise])
    return ap


def confidence(agg, kind='sem', within=None, alpha=0.05, nsamples=None, skipna=True):
    """
    ...
    if isinstance(agg, pandas.DataFrame):
        ...
        if within is not None:
            ...
            if len(levels) > 0:
                try:
                    subj_mean = agg.mean(level=levels, skipna=skipna, axis=1)
                except:
...
def nan_outliers(df, values=None, group=None):
    ...
    df[values] = np.select([tdf < -3, tdf > 3], [np.nan, np.nan], default=df[values])
    return df
...
def stats_test(agg, test='ttest'):
    ...
    if test == 'ttest':
        ...
        ttestPrint(title='**** 2-tail T-Test of related samples ****', values=ttest, plotOpt=plotOpt, type=2)
    elif test == 'ttest_1samp':
        m = 0.5
        oneSample = stats.ttest_1samp(agg, m, axis=0)
        ttestPrint(title='**** One-sample t-test: difference from %.2f ****' % m, values=oneSample, plotOpt=plotOpt, type=1)
    elif test == 'binomial':
...
def ttestPrint(title='****', values=None, xticklabels=None, legend=None, bon=None):
    d = 8
    if np.any([ np.any(val < 0) for val in values ]):
        neg = True
    else:
        neg = False
    print('\n', title)
    for xi, xticklabel in enumerate(xticklabels):
        ...
        for yi, legendname in enumerate(legendnames):
            ...
            if neg and t > 0:
                ...
                outputStr = '    %(s)s: t(%(d)d) = %(t).3f, p = %(p).3f %(star)s'
            print(outputStr % {'s': legendname, 'd': d - 1, 't': t, 'p': p, 
               'star': star})
...
def pearson_corr(df1, df2=None):
    ...
    for colname, col in corrs.iteritems():
        for rowname, corr in col.iteritems():
            ...
            ps.loc[(rowname, colname)] = scipy.stats.t.sf(abs(t), N - 2)
...
def reliability_splithalf_orig(df, level=1, niter=100, func='mean'):
    ...
    if func == 'corr':
        ...
        for n in range(niter):
            ...
            split1 = pandas.concat([ panel[item] for item in panel.items if item in split1inds
                                   ])
            split2 = pandas.concat([ panel[item] for item in panel.items if item in split2inds
                                   ])
    ...
    corr = np.mean(corrs)
    t = corr * np.sqrt((N - 2) / (1 - corr ** 2))
    p = 1 - scipy.stats.t.cdf(abs(t), N - 2)
    p = 2 * p / (1.0 + p)
    return (corr, t, p)
...
def mds(mean, ndim=2):
    if mean.ndim != 2:
        raise Exception('Similarity matrix for MDS must have exactly 2 dimensions but only %d were found.' % mean.ndim)
...
def classical_mds(d, ndim=2):
    ...
    B = -0.5 * (oo - rowmean - colmean + np.mean(oo))
...
def bootstrap_permutation(m1, m2=None, func=np.mean, niter=10000, ci=95, *func_args, **func_kwargs):
    m1 = np.array(m1)
    if m2 is not None:
        m2 = np.array(m2)
    df = []
    ...
    if ci is not None:
        return (np.percentile(df, 50 - ci / 2.0), np.percentile(df, 50 + ci / 2.0))
    else:
        return df
        return


def bootstrap_resample(data1, data2=None, func=np.mean, niter=1000, ci=95, struct=None, seed=None, *func_args, **func_kwargs):
    np.random.seed(seed)
...
def bootstrap_ttest(data1, data2=None, kind='ind', tails='one', struct=None, *func_args, **func_kwargs):
    if data2 is None:
        return bootstrap_permutation(data1, func=np.mean, niter=niter, ci=None, *func_args, **func_kwargs)
    else:
        b1 = bootstrap_resample(data1, func=np.mean, ci=None, struct=struct, *func_args, **func_kwargs)
        b2 = bootstrap_resample(data2, func=np.mean, ci=None, struct=struct, *func_args, **func_kwargs)
        pct = scipy.stats.percentileofscore(b1 - b2, 0, kind='mean') / 100.0
        p = min(pct, 1 - pct)
        if tails == 'one':
            p *= 2
