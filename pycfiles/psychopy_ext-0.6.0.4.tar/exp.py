...
# [GCC 8.4.0]
# Embedded file name: /Users/qbilius/Dropbox (MIT)/psychopy_ext/psychopy_ext/exp.py
# Compiled at: 2016-09-23 15:42:30
"""
...
import numpy as np, wx, pyglet, textwrap
...
def set_paths(exp_root='.', computer=default_computer, fmri_rel=''):
    ...
    if exp_root != '':
        exp_root += '/'
    paths = {'root': computer.root, 'exp_root': exp_root, 
       'fmri_root': fmri_root, 
       'analysis': os.path.join(exp_root, 'analysis/'), 
       'logs': os.path.join(exp_root, 'logs/'), 
       'data': os.path.join(exp_root, 'data/'), 
       'report': 'report/', 
       'data_behav': os.path.join(fmri_root, 'data_behav/'), 
       'data_fmri': os.path.join(fmri_root, 'data_fmri/'), 
       'data_struct': os.path.join(fmri_root, 'data_struct/'), 
       'spm_analysis': os.path.join(fmri_root, 'analysis/'), 
       'rec': os.path.join(fmri_root, 'reconstruction/'), 
       'rois': os.path.join(fmri_root, 'rois/'), 
       'data_rois': os.path.join(fmri_root, 'data_rois/'), 
       'sim': exp_root}
    return paths
...
class Task(TrialHandler):
    ...
    def __init__(self, parent, name='', version='0.1', method='random', data_fname=None, blockcol=None):
        """
    ...
    def setup_task(self):
        ...
        if not hasattr(self, 'trial'):
            raise Exception('self.trial variable must be created with the self.create_trial() method')
        ...
        if not hasattr(self, 'exp_plan'):
            raise Exception('self.exp_plan variable must be created with the self.create_exp_plan() method')
    ...
    def _parse_instructions(self, text):
        ...
        instructions = visual.TextStim(self.win, units='pix', wrapWidth=width)
        instructions._pygletTextObj = pyglet.text.HTMLLabel(html, width=width, multiline=multiline, x=0, anchor_x='left', anchor_y='center')
        return instructions

    def create_fixation(self, shape='complex', color='black', size=0.2):
        ...
        if shape == 'complex':
            ...
            oval = []
            for i in range(4):
                oval.append(visual.ShapeStim(self.win, name='oval', fillColor=color, lineColor=None, vertices=verts, ori=90 * i, pos=oval_pos[i]))

            center = visual.Circle(self.win, name='center', fillColor=color, lineColor=None, radius=r2)
            fixation = GroupStim(stimuli=oval + [center], name='fixation')
            fixation.color = color
            self.fixation = fixation
        elif shape == 'dot':
            self.fixation = GroupStim(stimuli=visual.PatchStim(self.win, name='fixation', color='red', tex=None, mask='circle', size=size), name='fixation')
        return
    ...
    def last_keypress(self, keyList=None, timeStamped=False):
        ...
        this_keylist = self.get_resp(keyList=keyList + self.keylist_flat, timeStamped=timeStamped)
        keys = []
    ...
    def wait_until_response(self, draw_stim=True):
        ...
        event_keys = []
        event.clearEvents()
        while len(event_keys) == 0:
            if 'autort' in self.this_trial:
                if self.trial_clock.getTime() > self.this_trial['autort']:
                    event_keys = [
                     (
                      self.this_trial['autoresp'], self.this_trial['autort'])]
            else:
                event_keys = self.last_keypress(keyList=self.computer.valid_responses.keys(), timeStamped=self.trial_clock)
    ...
    def feedback(self):
        ...
        if isinstance(self.this_event.display, (list, tuple)):
            ...
            if fix is not None:
                orig_color = fix.color
                if self.this_trial['corr_resp'] == subj_resp:
                    fix.setFillColor('DarkGreen')
                else:
                    fix.setFillColor('DarkRed')
            for stim in self.this_event.display:
                stim.draw()
    ...
    def set_TrialHandler(self, trial_list, trialmap=None):
        ...
        if len(self.blocks) > 1:
            self.set_seed()
        TrialHandler.__init__(self, trial_list, nReps=self.nReps, method=self.method, extraInfo=self.info, name=self.name, seed=self.seed)
        if trialmap is None:
    ...
    def get_blocks(self):
        ...
        if self.blockcol is not None:
            ...
            blocks = [
             [
              self.exp_plan, range(len(self.exp_plan))]]
        self.blocks = blocks
        return

    def before_task(self, text=None, wait=0.5, wait_stim=None, **kwargs):
        r"""Shows text from docstring explaining the task.
        ...
        if len(self.parent.tasks) > 1:
            if wait_stim is None:
                if len(self.blocks) <= 1:
                    ...
                    wait = 0
            if text is None:
                self.show_text(text=self.__doc__, wait=wait, wait_stim=wait_stim, **kwargs)
            else:
                self.show_text(text=text, wait=wait, wait_stim=wait_stim, **kwargs)
        return
    ...
    def after_task(self, text=None, auto=1, **kwargs):
        r"""Useful for showing feedback after a task is done.
    ...
    def before_block(self, text=None, auto=1, wait=0.5, wait_stim=None):
        ...
        if len(self.blocks) > 1:
            ...
            if text is None:
                self.show_text(text='Block %d' % (self.this_blockn + 1), auto=auto, wait=wait, wait_stim=wait_stim)
            else:
    ...
    def after_block(self, text=None, **kwargs):
        r"""Show text at the end of a block.
        ...
        sys.stdout.flush()
        if text is None:
            text = 'Pause. Hit %s to continue.' % self.computer.default_keys['trigger']
    ...
    def run_event(self):
        ...
        self.all_keys += self.last_keypress(keyList=self.computer.valid_responses.keys(), timeStamped=self.trial_clock)
        return
...
class SVG(object):
    ...
    def open(self, filename):
        ...
        self.svgfile = svgwrite.Drawing(profile='tiny', filename='%s.svg' % filename, size=(
         '%dpx' % self.win.size[0],
         '%dpx' % self.win.size[1]), viewBox='%d %d %d %d' % (
         0, 0,
         self.win.size[0],
         self.win.size[1]))
        bkgr = self.svgfile.rect(insert=(0, 0), size=('100%', '100%'), fill=self.color2rgb255(self.win))
    ...
    def write(self, stim):
        if 'Circle' in str(stim):
            color_kw = self.color2attr(stim, 'stroke', color=stim.lineColor, colorSpace=stim.lineColorSpace)
            color_kw = self.color2attr(stim, 'fill', color=stim.fillColor, colorSpace=stim.fillColorSpace, kwargs=color_kw)
            svgstim = self.svgfile.circle(center=self.get_pos(stim), r=self.get_size(stim, stim.radius), stroke_width=stim.lineWidth, opacity=stim.opacity, **color_kw)
        else:
            if 'ImageStim' in str(stim):
                raise NotImplemented
            elif 'Line' in str(stim):
                color_kw = self.color2attr(stim, 'stroke', color=stim.lineColor, colorSpace=stim.lineColorSpace)
                svgstim = self.svgfile.line(start=self.get_pos(stim, stim.start), end=self.get_pos(stim, stim.end), stroke_width=stim.lineWidth, opacity=stim.opacity, **color_kw)
            elif 'Polygon' in str(stim):
                raise NotImplemented
            elif 'Rect' in str(stim):
                color_kw = self.color2attr(stim, 'stroke', color=stim.lineColor, colorSpace=stim.lineColorSpace)
                color_kw = self.color2attr(stim, 'fill', color=stim.fillColor, colorSpace=stim.fillColorSpace, kwargs=color_kw)
                svgstim = self.svgfile.rect(insert=self.get_pos(stim, offset=(-stim.width / 2.0, -stim.height / 2.0)), size=(
                 self.get_size(stim, stim.width), self.get_size(stim, stim.height)), stroke_width=stim.lineWidth, opacity=stim.opacity, **color_kw)
                ...
                color_kw = self.color2attr(stim, 'fill', color=stim.fillColor, colorSpace=stim.fillColorSpace, kwargs=color_kw)
                if stim.closeShape:
                    svgstim = self.svgfile.polygon(points=points, stroke_width=stim.lineWidth, opacity=stim.opacity, **color_kw)
                else:
                    svgstim = self.svgfile.polyline(points=points, stroke_width=stim.lineWidth, opacity=stim.opacity, **color_kw)
                ...
                if stim.font == '':
                    ...
                    font = stim.font
                svgstim = self.svgfile.text(text=stim.text, insert=self.get_pos(stim) + np.array([0, stim.height / 2.0]), fill=self.color2rgb255(stim), font_family=font, font_size=self._calc_attr(stim, stim.height), text_anchor='middle', opacity=stim.opacity)
            else:
                svgstim = stim.to_svg(self)
    ...
    def _calc_attr(self, stim, attr):
        if stim.units == 'height':
            try:
                ...
                out = attr * stim.win.size * np.array([1.0 / self.aspect, 1])
...
class Experiment(ExperimentHandler, Task):
    ...
    def __init__(self, name='', version='0.1', info=None, rp=None, actions=None, computer=default_computer, paths=None, data_fname=None, **kwargs):
        r"""
        ...
        """
        ExperimentHandler.__init__(self, name=name, version=version, extraInfo=info, dataFileName='.data')
        ...
        self.rp = OrderedDict([
         ...
         (
          'unittest', False),
         (
          'repository', ('do nothing', 'commit & push', 'only commit'))])
        if rp is not None:
        ...
        if self.rp['unittest']:
            self.rp['autorun'] = 100
        self.tasks = []
        Task.__init__(self, self, version=version, **kwargs)
        return
    ...
    def _guess_participant(self, data_path, default_subjid='01'):
        ...
        if len(partids) > 0:
            return '%02d' % (max(partids) + 1)
        else:
            return default_subjid
    ...
    def get_mon_sizes(self, screen=None):
        warnings.warn('get_mon_sizes is deprecated; use exp.get_mon_sizes instead')
        return get_mon_sizes(screen=screen)

    def create_win(self, debug=False, color='DimGray', units='deg', winType='pyglet', **kwargs):
        ...
        monitor = monitors.Monitor(self.computer.name, distance=self.computer.distance, width=self.computer.width)
        ...
        self.win = visual.Window(monitor=monitor, units=units, fullscr=(not debug), allowGUI=debug, color=color, winType=winType, screen=self.computer.screen, viewScale=self.computer.view_scale, **kwargs)
    ...
    def setup(self):
        ...
        self.runtime_info = psychopy.info.RunTimeInfo(author=author, version=version, verbose=True, win=False)
        key, value = get_version()
        self.runtime_info[key] = value
        self._set_keys_flat()
    ...
    def before_exp(self, text=None, wait=0.5, wait_stim=None, **kwargs):
        r"""
    ...
    def after_exp(self, text=None, auto=1, **kwargs):
        r"""Text after the experiment is over.
        ...
        """
        if text is None:
            self.show_text(text='End of Experiment. Thank you!', auto=auto, **kwargs)
        else:
    ...
    def repo_action(self):
        ...
        if self.rp['repository'] != 'do nothing':
            textstim = visual.TextStim(self.win, text=text, height=0.3)
            textstim.draw()
...
class Event(object):
    ...
    def __init__(self, parent, name='', dur=0.3, durcol=None, display=None, func=None):
        """
...
class ThickShapeStim(ShapeStim):
    ...
    def __init__(self, win, lineWidth=0.01, **kwargs):
        ...
        if self.units == 'pix':
            ...
            for vertices in self.vertices_all:
                ...
                for i in range(numPairs):
                    ...
                    lh = np.sqrt(edges[0] ** 2 + edges[1] ** 2) / 2.0
                    rend_vert = [[-lh, -wh], [-lh, wh], [lh, wh], [lh, -wh]]
                    line = visual.ShapeStim(self.win, lineWidth=1, lineColor=self.lineColor, interpolate=True, fillColor=self.lineColor, ori=-np.arctan2(edges[1], edges[0]) * 180 / np.pi, pos=np.mean(thisPair_rot, 0) + self.pos, vertices=rend_vert)
    ...
    def to_svg(self, svg):
        ...
        for stim, vertices in zip(self.stimulus, self.vertices_all):
            ...
            points[:, 1] *= -1
            rect = svg.svgfile.polyline(points=points, stroke_width=svg._calc_attr(self, self.lineWidth), stroke=svg.color2rgb255(self, color=self.lineColor, colorSpace=self.lineColorSpace), fill_opacity=0)
            tr = svg.get_pos(self)
...
class MouseRespGroup(object):
    ...
    def __init__(self, win, stimuli, respmap=None, multisel=False, on_color='#ec6b00', off_color='white', pos=(0, 0), height=0.2, name=''):
        self.win = win
        ...
        for i, stim in enumerate(stimuli):
            if isinstance(stim, str):
                ...
                stim = visual.TextStim(self.win, text=stim, height=height, pos=pos + add)
            ...
            stim.respbox = visual.Rect(self.win, name=stim.name, lineColor=None, fillColor=None, pos=stim._posRendered, height=size[1], width=size[0], units='pix')
...
def get_version():
    ...
    githash = psychopy.info._getHashGitHead(gdir=d)
    if not githash:
...
def weighted_sample(probs):
    warnings.warn('weighted_sample is deprecated; use weighted_choice instead')
