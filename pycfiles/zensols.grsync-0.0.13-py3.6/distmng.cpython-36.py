...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/zensols/grsync/distmng.py
# Compiled at: 2019-07-10 22:08:35
# Size of source mod 2**32: 5241 bytes
import logging, json, zipfile
from pathlib import Path
from zensols.actioncli import YamlConfig, persisted
from zensols.grsync import Discoverer, Distribution, FreezeManager, ThawManager, PathTranslator, DistributionMover
logger = logging.getLogger(__name__)

class DistManager(object):
    __doc__ = 'The main entry point that supports saving user home directory information\n    (freezing) so that the data can later be restored (thawed).  It does this\n    by finding git repositories and saving the remotes.  It also finds symbolic\n    links, files and empty directories specified in the configuration.\n\n    '

    def __init__(self, config: YamlConfig, dist_dir: Path=None, target_dir: Path=None, profiles: list=None, repo_preference: str=None, dry_run: bool=False):
        """Initialize.
    ...
    def discoverer(self):
        return Discoverer(self.config, self.profiles, self.path_translator, self.repo_preference)

    @property
    def distribution(self):
        return Distribution(self.dist_file, self.defs_file, self.target_dir, self.path_translator)
    ...
    def freeze(self, wheel_dependency=None):
        ...
        """
        fmng = FreezeManager(self.config, self.dist_file, self.defs_file, self.discoverer, self.app_version)
    ...
    def thaw(self):
        ...
        """
        tmng = ThawManager(self.distribution, self.defs_file, self.path_translator, self.app_version, self.dry_run)
        tmng.thaw()
    ...
    def move(self, destination_path, dir_reduce=True):
        ...
        mv = DistributionMover((self.distribution),
          (self.target_dir), destination_path,
