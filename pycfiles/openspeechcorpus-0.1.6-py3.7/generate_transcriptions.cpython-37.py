...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/openspeechcorpus_cli/cmu_sphinx/generate_transcriptions.py
# Compiled at: 2020-01-11 17:45:27
...
def execute_script(etc_folder_name, name, transcription_file, test_samples_coefficient, ignore_wav_missing=False):
    ...
    for line in lines:
        ...
        line_transcription_content = '<s> {} </s>'.format(allow_characters(filter_special_characters(clean_accents(remove_jump_characters(raw_trasncription.lower())))))
        line_id_content = '{}'.format(remove_jump_characters(remove_extensions(file_name)))
