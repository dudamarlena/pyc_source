...
# Embedded file name: /home/ax/Workspace/norm/norm/models/norm.py
# Compiled at: 2019-05-19 18:32:04
# Size of source mod 2**32: 26937 bytes
__doc__ = 'A collection of ORM sqlalchemy models for Lambda'
import zlib, norm.config as config
...
lambda_variable = Table('lambda_variable', metadata, Column('id', Integer, primary_key=True), Column('lambda_id', Integer, ForeignKey('lambdas.id')), Column('variable_id', Integer, ForeignKey('variables.id')))
...
class Lambda(Model, ParametrizedMixin):
    ...
    variables = relationship(Variable, order_by=(Variable.position), collection_class=(ordering_list('position')), secondary=lambda_variable)
    ...
    __mapper_args__ = {'polymorphic_identity':'lambda', 
    ...
    def exists(cls, session, obj):
        ...
        :return: List
        """
        in_stores = session.query(Lambda).filter(Lambda.name == obj.name, Lambda.description == obj.description, Lambda.namespace == obj.namespace).all()
    ...
    def get_type(self, variable_name):
        ...
        :rtype: Lambda
        """
        if variable_name == self.VAR_OUTPUT:
            if not self.is_functional:
                return self
            else:
                if variable_name == self.VAR_OID:
                    return self.VAR_OID_T
                if variable_name == self.VAR_TIMESTAMP:
                    return self.VAR_TIMESTAMP_T
            if variable_name == self.VAR_PROB:
                return self.VAR_PROB_T
        else:
            if variable_name == self.VAR_LABEL:
                return self.VAR_LABEL_T
            if variable_name == self.VAR_TOMBSTONE:
    ...
    def clone(self):
        ...
        :rtype: Lambda
        """
        lam = self.__class__(namespace=(self.namespace), name=(self.name), description=(self.description), params=(self.params),
    ...
    def _add_optional_variables(self, df):
        ...
        vars_to_add = [Variable(col, get_type_by_dtype(dtype)) for col, dtype in zip(df.columns, df.dtypes) if col not in current_variable_names]
    ...
    def revise(self, query, description, df, op):
        ...
        if any(rev.query == query for rev in self.revisions):
            if df is not None:
                return self.data.loc[df.index]
            else:
                return
        else:
            if df is not None:
                if isinstance(df, DataFrame):
                    if not df.index.name == self.VAR_OID:
                        raise AssertionError
                    elif not self.VAR_TIMESTAMP in df.columns:
                        raise AssertionError
            ...
            if op == RevisionType.DISJUNCTION:
                ...
                if op == RevisionType.CONJUNCTION:
    ...
    def fill_oid(self, df):
        if self.VAR_OID in df.columns or df.index.name == self.VAR_OID:
            ...
            cols = [v.name for v in self.variables if v.primary if v.name in df.columns]
            c = None
            for col in cols:
                if c is None:
                    c = df[col].astype(str)
    ...
    @property
    def folder(self):
        return '{}/{}/{}/{}'.format(config.DATA_STORAGE_ROOT, self.namespace.replace('.', '/'), self.name, self.version)
    ...
    def _all_columns(self):
        return [
         self.VAR_OID, self.VAR_PROB, self.VAR_LABEL, self.VAR_TIMESTAMP,
         self.VAR_TOMBSTONE] + [v.name for v in self.variables]

    @property
    def _all_column_types(self):
        return [
         self.VAR_OID_T, self.VAR_PROB_T, self.VAR_LABEL_T, self.VAR_TIMESTAMP_T,
...
class GroupLambda(Lambda):
    __mapper_args__ = {'polymorphic_identity': 'lambda_group'}
...
def retrieve_type(namespaces, name, version=None, status=None):
    ...
    lam = (session.query(Lambda).filter)(*queries).order_by(desc(Lambda.created_on)).first()
    return lam