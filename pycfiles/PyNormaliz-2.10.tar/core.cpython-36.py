...
# Embedded file name: /home/ax/Workspace/norm/norm/models/core.py
# Compiled at: 2019-04-26 20:11:01
# Size of source mod 2**32: 7301 bytes
__doc__ = 'A collection of ORM sqlalchemy models for CoreLambda'
...
class CoreLambda(Lambda):
    """CoreLambda"""
    __mapper_args__ = {'polymorphic_identity': 'lambda_core'}
    NAMESPACE = 'norm.core'
...
class ReadFileLambda(CoreLambda):
    __mapper_args__ = {'polymorphic_identity': 'lambda_core_read_file'}
    VAR_PATH = 'path'
    VAR_PARM = 'parameters'
    VAR_EXT = 'ext'
    EXT_CSV = 'csv'
    ...
    def __init__(self):
        ...
        super().__init__(name='read', description='Read data from files [.csv, .tsv, .parq, .jsonl], default to csv fileyour_lambda.read("path_to_the_file.csv", (sep="\t", skiprows=3))read(your_lambda, "path_to_the_file.csv", (sep="\t", skiprows=3))',
          variables=[
    ...
    def __call__(self, **inputs):
                ...
                if not isinstance(path, str):
                    ...
                    if ext is None:
                        ...
                        if last_path == self.EXT_CSV:
                            ...
                            if last_path == self.EXT_PAR:
                                ext = self.EXT_PAR
                            else:
                                if last_path == self.EXT_TSV:
                                    ext = self.EXT_TSV
                                else:
                                    if last_path == self.EXT_JSL:
                                        ext = self.EXT_JSL
                                    else:
                                        ext = self.EXT_CSV
        ...
        msg = 'Currently supported file formats: CSV (.csv), TSV (.tsv), Parquet (.parq) and JSONL (.jsonl)'
        logger.error(msg)
        raise TypeError(msg)
...
class StringFormatterLambda(CoreLambda):
    __mapper_args__ = {'polymorphic_identity': 'lambda_core_string_formatter'}
    ...
    def __init__(self):
        ...
        super().__init__(name='format', description='Format the strings with given inputs, the semantic is the same as Python format',
          variables=[
         formatter, variables, output])
    ...
    def __call__(self, **inputs):
        ...
        if variables is None or formatter is None:
            ...
            if isinstance(formatter, DataFrame):
                ...
                data = DataFrame(df.apply((lambda x: (x[f_col].format)(*[x[c] for c in v_cols])), axis=1), columns=[
                 self.VAR_OUTPUT])
            else:
                assert isinstance(formatter, str)
                data = DataFrame(variables.apply((lambda x: (formatter.format)(*[x[c] for c in v_cols])), axis=1), columns=[
                 self.VAR_OUTPUT])
            return data
...
class ExtractPatternLambda(CoreLambda):
    __mapper_args__ = {'polymorphic_identity': 'lambda_core_extract_pattern'}
    VAR_PATTERN = 'pattern'
    VAR_STRING = 'string'
    VAR_FILLNA = 'fillna'
    ...
    def __init__(self):
        from norm.models import lambdas
        super().__init__(name='extract', description='Extract patterns from a string"(2014).*(6)".extract("2014-06") --> (2014, 6)/None"2014.*6".extract(s, fillna=False) --> True/False',
          variables=[
