...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/praekelt/recipe/deploy.py
# Compiled at: 2011-06-29 06:48:12
from datetime import datetime
from optparse import OptionParser
import os, re, sys, time, zc.recipe.egg
...
class Deploy:
    ...
    def __init__(self, buildout, name, options):
        ...
        self.script_args['supervisorctl_commands'] = []
        for supervisorctl_command in options.get('supervisorctl_commands', '').split('\n'):
            if supervisorctl_command:
                self.script_args['supervisorctl_commands'].append(supervisorctl_command)
    ...
    def create_manage_script(self, ws):
        ...
        return zc.buildout.easy_install.scripts([
         (
          self.name, 'praekelt.recipe.deploy', 'script')], ws, sys.executable, self.options['bin-directory'], arguments=(', ').join(args))
...
def construct_git_url(git_url, username, password):
...
def script(host, root_path, git_url, as_user='www-data', python_exec='python', deploy_latest_tag=False, git_branch=None, conf_file='buildout.cfg', deploy_key_path=None, shared_resources=[], supervisorctl_commands=[], initd_commands=[], puppet_manifest=None, update_supervisor=False, cron_commands=[]):
    """
    Args:
    ...
    update_supervisor: Whether or not to update supervisor configurations. Defaults to False.
    ...
    The script creates a new deploy as follows:
        ...
        * Each initd command specified in initd_commands is run in order.
    """
    release_timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    parser = OptionParser()
    parser.add_option('-H', '--host', dest='host', default=None, help='Hostname on which to perform deploy.', metavar='<[user@]hostname>')
    parser.add_option('-s', '--ssh-user', dest='ssh_user', default=None, help='SSH username to use during deploy.', metavar='<username>')
    parser.add_option('-i', '--ignore-deploy-key', action='store_true', dest='ignore_deploy_key', default=False, help="Don't use deploy key specified in buildout configuration's 'deploy_key_path' parameter.")
    parser.add_option('-n', '--newest', action='store_true', dest='newest', default=False, help='New distributions will be sought to meet Buildout requirements.')
    parser.add_option('-v', '--verbose', action='store_true', dest='verbose', default=False, help='Run Buildout in verbose mode.')
    parser.add_option('-p', '--password', dest='password', default=None, help='Sudo password to use during deploy.', metavar='<password>')
    parser.add_option('-f', '--force', action='store_true', dest='force', default=False, help="Don't prompt for confirmations. Deploy will proceed without any prompts.")
    parser.add_option('-g', '--git-user', dest='git_user', default=None, help='Git username to use during git clone.', metavar='<username>')
    parser.add_option('-w', '--git-password', dest='git_password', default=None, help='Git password to use during git clone.', metavar='<password>')
    ...
    git_url = construct_git_url(git_url, username=options.git_user, password=options.git_password)
    if options.verbose:
        hide_args = ()
    else:
    ...
    with settings(hide(*hide_args), host_string=host):
        ...
        if options.force or confirm("This will buildout %s's %s branch in %s's %s path using %s and run the following commands:\n%s\nContinue?" % (
         ...
         new_release_path,
         conf_file,
         ('\n').join((['supervisorctl update'] if update_supervisor else []) + [ 'supervisorctl %s' % supervisorctl_command for supervisorctl_command in supervisorctl_commands ] + [ '/etc/init.d/%s' % initd_command for initd_command in initd_commands ]))):
            if not current_release_path:
                if shared_resources:
                    if not options.force:
                        confirm('It looks like there is no current release. This means no shared resources can be copied to this new release. Continue anyway?') or abort('No current release found. Aborting at user request.')
            ...
            if update_supervisor:
                ...
                stop_wait()
            for supervisorctl_command in supervisorctl_commands:
                print_wait("Running supervisorctl command '%s'" % supervisorctl_command)
                sudo('supervisorctl %s' % supervisorctl_command)
                stop_wait()
