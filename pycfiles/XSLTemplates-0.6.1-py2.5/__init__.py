...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/xsltemplates/__init__.py
# Compiled at: 2008-04-03 02:13:58
import os
...
class LocalTemplateResolver(Uri.FtUriResolver):
    ...
    def resolve(self, uri, base_uri=None):
        ...
        if uri.startswith('local:'):
            ...
            raise UriException(UriException.RESOURCE_ERROR, uri=uri, loc=uri, msg="The file did not exist in '%s'" % self.templates)
        elif uri.startswith('pkg:'):
            usage = 'usage: package_name#path/to/file'
            uri = uri[4:]
            (package, sep, path) = uri.partition('#')
            if not package or path:
                raise UriException(UriException.RESOURCE_ERROR, uri=uri, loc=uri, msg='Invalid pkg_resources uri. \n %s' % usage)
            if pkg_resources.resource_exists(package, path):
                return pkg_resources.resource_stream(package, path)
            raise UriException(UriException.RESOURCE_ERROR, uri=uri, loc=uri, msg="'%s' was not found in the python package '%s'" % (path, package))
        else:
...
class TemplateMiddleware(object):
    ...
    def __init__(self, app_conf, app, **kw):
        self.ns = unicode(app_conf.get('xsltemplate_namespace', 'http://ionrock.org/ns/xsltemplate'))
    ...
    def get(self, fn):
        ...
        try:
            ...
            xslt = self.xslt_factory.fromString(fn, Uri.OsPathToUri(os.path.abspath('.')))
    ...
    def run(self, xml, xslt, params):
        ...
        content_type = str(proc.outputParams.mediaType)
        if content_type:
    ...
    def setup_xslt_params(self, params):
        xsltparams = dict([ ((self.ns, k), params[k]) for k in params if xsltize(params[k]) is not None
                          ])
        return xsltparams
...
class TemplateConstants(object):
    ...
    def __call__(self, environ, start_response):
        if self.use_environ:
            set_params(environ, dict([ (k, v) for (k, v) in environ.items() if not k.startswith(XParams)
                                     ]))
