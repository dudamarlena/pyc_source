...
# Embedded file name: build/bdist.linux-x86_64/egg/cary_perdiemcommand/perdiem_scraper.py
# Compiled at: 2015-08-04 07:27:58
# Size of source mod 2**32: 7866 bytes
...
def conus_files(q, base_url):
    ...
    """
    result = []
    conus_locations_list = q('p b:contains("CONUS Locations")').parent().next().children('li')
    for i in range(0, len(conus_locations_list)):
        item = conus_locations_list.eq(i)
        date = item('b:contains("Effective")').text()
        date_text = re.search('(\\d+\\s\\w+\\s\\d+)', date).group(0)
...
def oconus_files(q, base_url):
    result = []
    oconus_locations_list = q('p b:contains("OCONUS and Foreign")').parent().next().children('li')
...
def row_from_record(r):
    ...
    fieldmap = dict(location='location_name', country='country_name', country_code='country_code', eff_date='eff_date', exp_date='exp_date', lodging='lodging_rate', meals='local_meals', prop_meals='proportional_meals_rate', incidentals='incidentals', state_alpha='state_alpha', state_name='state_name')
    result = {}
...
def dict_from_query(q):
    ...
    def season_from_row(row):
        return dict(eff_date=row['eff_date'], exp_date=row['exp_date'], lodging=row['lodging'], meals=row['meals'], prop_meals=row['prop_meals'], incidentals=row['incidentals'])

    def loc_record_from_row(row):
        return dict(location=row['location'], country=row['country'], country_code=row['country_code'], state_alpha=row['state_alpha'], state_name=row['state_name'], seasons=[
         season_from_row(row)], locstrings=row['locstrings'])
...
def update_database_metadata(database_filename, last_conus_entry, last_oconus_entry, download_date):
    db = TinyDB(database_filename)
    table = db.table('metadata')
    table.insert(dict(conus_url=last_conus_entry[1], conus_date=last_conus_entry[0], oconus_url=last_oconus_entry[1], oconus_date=last_oconus_entry[0], download_date=download_date))


def create_locstrings_from_dict(filename, record_dict):
    with open(filename, 'w') as (csvfile):
        w = csv.writer(csvfile)
        for row in itertools.chain.from_iterable(((l, x[0]) for l in x[1]['locstrings']) for x in record_dict.items()):
            w.writerow(row)


def build_perdiem_database(locstrings_filename, database_filename, working_directory):
    q = perdiem_query()
    ...
    update_database_metadata(database_filename, conus_urls[0], oconus_urls[0], today)
