...
def add_resource(config, resource_class):
    ...
    if not issubclass(resource_class, Resource):
        raise Exception('The resource cannot be added, as it is not a subclass of spynl.main.routing.Resource')
    resource_name = resource_class.__name__
    resources = config.get_settings()['spynl.resources']
    added_paths = []
    if 'paths' in resource_class.__dict__:
        for path in resource_class.paths:
            added_paths.append(path)
            for ex_res in resources.values():
                if path in ex_res.paths:
                    logger.warn('Cannot add path "%s" for resource %s, as it was already added by resource %s!' % (
            ...
            if path in added_paths:
                ...
                for rpattern, route_info in route_info.items():
                    if issubclass(resource_class, route_info['resources']):
                        ...
                        rf(config, route_name=rpattern.replace('{path}', path), resource_class=resource_class, path=path)
                        logger.info("Adding routes for resource: '%s', path '%s', pattern '%s'", resource_name, path, rpattern)
...
def add_resource_routes(config, route_name, resource_class, path):
    ...
    """
    config.add_route(name=route_name, pattern='/%s/{method}' % path, factory=resource_class)
    config.add_route(name=route_name + '.nomethod', pattern='/%s' % path, factory=resource_class)


def add_endpoint(config, func, endpoint_name, permission=NO_PERMISSION_REQUIRED, context=None, available_in_production=True, **kw):
    """
    ...
    """
    logger = get_logger('spynl.main.routing')
    if is_production_environment(or_test=True) and not available_in_production:
        logger.warn("Skipping endpoint '%s' - should not be available in production environments", endpoint_name)
        return
    if 'renderer' not in kw:
        kw['renderer'] = 'spynls-renderer'
    if context:
        context_name = context.__name__
        if is_production_environment(or_test=True) and not context.available_in_production:
            logger.warn('Not adding endpoint %s, as its resource (%s) is not to be used in a production-like environment.' % (
        ...
        if 'paths' not in context.__dict__.keys() or len(context.paths) == 0:
            logger.warn('Resource %s defines no paths, cannot add endpoint.' % context_name)
            return
        ...
        for path in context.paths:
            ...
            for route_name, rinfo in rroutes.items():
                if issubclass(context, rinfo['resources']):
                    ...
                    config.add_view(func, context=context, route_name=route_name, match_param=match_param, permission=permission, **kw)
                    logger.info("Added endpoint '%s' for route '%s', resource '%s'", endpoint_name, route_name, context_name)
                    document_endpoint(config, func, has_method and path + '/' + endpoint_name or endpoint_name, resource=path)
...
def main(config):
    ...
    resource_routes_info = {'spynl.{path}': {'resources': (Resource,), 
