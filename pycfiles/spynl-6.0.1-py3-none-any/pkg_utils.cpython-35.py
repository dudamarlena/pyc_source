...
# [GCC 8.4.0]
# Embedded file name: /home/nicolas/workspace/spynl-git/venv/src/spynl/spynl/main/pkg_utils.py
...
def get_spynl_packages(include_scm_urls=False):
    ...
    Package = namedtuple('Package', ['project_name', 'version', 'location',
     'scm_url'])
    installed_packages = []
    plist = subprocess.run('pip list -l --format columns', shell=True, stdout=subprocess.PIPE, universal_newlines=True)
    ...
    with chdir(package_location):
        if os.path.exists('.git'):
            ...
            if os.path.exists('.hg'):
                ...
                return
        cmd_result = subprocess.run(cmd, shell=True, stdout=subprocess.PIPE, universal_newlines=True)
    ...
    for package in packages:
        ...
        if 'development.ini' in plisting and 'production.ini' in plisting:
            if config_package is not None:
                emsg = 'Two packages have configurations (development.ini and production.ini): %s and %s. unsure which to use!' % (
