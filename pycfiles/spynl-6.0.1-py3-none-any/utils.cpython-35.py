...
# [GCC 8.4.0]
# Embedded file name: /home/nicolas/workspace/spynl-git/venv/src/spynl/spynl/main/utils.py
# Compiled at: 2017-02-02 10:07:08
# Size of source mod 2**32: 18182 bytes
"""Helper functions and view derivers for spynl.main."""
import json, logging, traceback, sys, os, contextlib
...
def check_origin(endpoint, info):
    ...
    def wrapper_view(context, request):
        ...
        if not is_origin_allowed(origin):
            raise Forbidden(detail=BadOrigin(origin).message.translate(request.localizer))
...
def get_header_args(request):
    ...
    headers = {key:value for key, value in request.headers.items() if key.lower().startswith('x-spynl-') if key.lower().startswith('x-spynl-')}
...
def unify_args(request):
    ...
    args.update(json.loads(json.dumps(urlson.loads_dict(request.GET)), object_hook=objects.SpynlDecoder(context=context)))
...
def find_view_name(request):
    ...
    """
    name = None
    if request.matchdict and 'method' in request.matchdict:
        name = request.matchdict['method']
    else:
        name = request.path_info
    if name.startswith('/'):
        name = name[1:]
    if hasattr(request, 'matched_route') and request.matched_route and name in request.matched_route.name:
...
def get_err_source(original_traceback=None):
    ...
    try:
        ...
        return dict(module=first_call[0], linenr=first_call[1], method=first_call[2], src_code=first_call[3])
    except Exception:
...
def parse_value(value, class_info):
    ...
    if isclass(class_info):
        try:
            ...
            raise SpynlException(_('parse-value-exception-as-class', default='"${value}" could not be parsed as ${class}', mapping={'value': value, 'class': class_info.__name__}))

        if hasattr(class_info, '__iter__'):
            for cl in class_info:
                if not isclass(cl):
                    raise SpynlException(_('parse-value-exception-not-class', default='${class} is not a class. Cannot parse value "${value}".', mapping={'class': cl, 'value': value}))
                try:
                    return cl(value)
                except Exception:
                    pass

        raise SpynlException(_('parse-value-exception-any-class', default='"${value}" could not be parsed into any class in ${classes}', mapping={'value': value, 
...
def send_exception_to_external_monitoring(user_info=None, exc_info=None, metadata=None, endpoint=None):
    ...
    try:
        if is_package_installed('raven') and settings.get('spynl.sentry_key') and settings.get('spynl.sentry_project'):
            import raven
            spynl_env = settings.get('spynl.spynl_environment', 'dev')
            spynl_function = settings.get('spynl.function', 'all')
            client = raven.Client(dsn='https://{}@app.getsentry.com/{}'.format(settings.get('spynl.sentry_key'), settings.get('spynl.sentry_project')), release=spynl_version, site='Spynl [{}-{}]'.format(spynl_env, spynl_function), processors=('raven.processors.SanitizePasswordsProcessor', ))
...
def log_error(exc, request, top_msg, error_type, error_msg):
    ...
    if hasattr(exc, '__cause__') and exc.__cause__ is not None:
        exc_info = (
         exc.__cause__.__class__, exc.__cause__, sys.exc_info()[2])
    metadata = dict(user=user_info, url=request.path_url, err_source=get_err_source(sys.exc_info()[2]), detail=getattr(exc, 'detail', None))
    log.error(top_msg, error_type, str(error_msg), exc_info=exc_info, extra=dict(meta=metadata, payload=str(request.body)[:500]))
    send_exception_to_external_monitoring(user_info=user_info, exc_info=exc_info, metadata=metadata)
