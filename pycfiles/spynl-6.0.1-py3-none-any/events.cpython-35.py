...
# [GCC 8.4.0]
# Embedded file name: /home/nicolas/workspace/spynl-git/venv/src/spynl/spynl/main/events.py
...
from spynl.main.utils import unify_args, get_logger, is_origin_allowed, get_settings
...
def prepare_content_types(event):
    ...
    if request.response.content_type == 'text/html' and '/static' not in request.path_url:
...
def store_accepted_lang(event):
    ...
    supported_languages = [langcode(lang.strip()) for lang in settings.get('spynl.languages', '').split(',')]
    preferred_language = supported_languages[0]
    if 'lang' in request.cookies:
        language = langcode(request.cookies['lang'])
    elif request.accept_language:
        language = langcode(request.accept_language.best_match(supported_languages, preferred_language))
...
def parse_args_and_log_request(event):
    ...
    log.info('New request for URL path "%s" from %s', request.path_url, origin_txt, extra=dict(meta=dict(url=request.path_url), payload=args))
...
def enforce_response_type(event):
    ...
    if 'force_download' in args:
        ...
        event.response.headers.update({'Content-Description': 'File Transfer', 
