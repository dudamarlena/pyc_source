...
# [GCC 8.4.0]
# Embedded file name: /home/nicolas/workspace/spynl-git/venv/src/spynl/spynl/main/serial/objects.py
...
from spynl.main.dateutils import date_format_str, localize_date, date_to_str, date_from_str
    ...
    def __call__(self, dic):
        ...
        for fieldname in dic:
            if fieldname in decode_functions:
                decode_functions[fieldname](dic, fieldname=fieldname, context=self.context)
...
def add_decode_function(config, function, fields):
    ...
    for field in fields:
        if decode_functions.get(field) is not None:
            log.warning('A custom decoding function for field %s is being overwritten.', field)
...
def decode_date(dic, fieldname, context):
    ...
    try:
        ...
        raise ValueError(_('date-decode-value-error', default="The value '${value}' for key '${key}' does not seem to be a valid date string that conforms to ${format}.", mapping={'value': dic[fieldname], 'key': fieldname, 
