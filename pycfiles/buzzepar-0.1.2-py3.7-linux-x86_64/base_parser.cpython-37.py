...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/benepar/base_parser.py
# Compiled at: 2020-05-05 12:30:15
# Size of source mod 2**32: 11423 bytes
import tensorflow as tf, numpy as np, os, sys, json, codecs
try:
...
BERT_TOKEN_MAPPING = {'-LRB-':'(', 
 ...
 '–':'--', 
 '—':'--'}
LABEL_VOCAB = ((), ('S',), ('PP',), ('NP',), ('PRN',), ('VP',), ('ADVP',), ('SBAR', 'S'),
               ('ADJP',), ('QP',), ('UCP',), ('S', 'VP'), ('SBAR',), ('WHNP',), ('SINV',),
               ('FRAG',), ('NAC',), ('WHADVP',), ('NP', 'QP'), ('PRT',), ('S', 'PP'),
               ('S', 'NP'), ('NX',), ('S', 'ADJP'), ('WHPP',), ('SBAR', 'S', 'VP'),
               ('SBAR', 'SINV'), ('SQ',), ('NP', 'NP'), ('SBARQ',), ('SQ', 'VP'),
               ('CONJP',), ('ADJP', 'QP'), ('FRAG', 'NP'), ('FRAG', 'ADJP'), ('WHADJP',),
               ('ADJP', 'ADJP'), ('FRAG', 'PP'), ('S', 'ADVP'), ('FRAG', 'SBAR'),
               ('PRN', 'S'), ('PRN', 'S', 'VP'), ('INTJ',), ('X',), ('NP', 'NP', 'NP'),
               ('FRAG', 'S', 'VP'), ('ADVP', 'ADVP'), ('RRC',), ('VP', 'PP'), ('VP', 'VP'),
               ('SBAR', 'FRAG'), ('ADVP', 'ADJP'), ('LST',), ('NP', 'NP', 'QP'),
               ('PRN', 'SBAR'), ('VP', 'S', 'VP'), ('S', 'UCP'), ('FRAG', 'WHNP'),
               ('NP', 'PP'), ('NP', 'SBAR', 'S', 'VP'), ('WHNP', 'QP'), ('VP', 'FRAG', 'ADJP'),
               ('FRAG', 'WHADVP'), ('NP', 'ADJP'), ('VP', 'SBAR'), ('NP', 'S', 'VP'),
               ('X', 'PP'), ('S', 'VP', 'VP'), ('S', 'VP', 'ADVP'), ('WHNP', 'WHNP'),
               ('NX', 'NX'), ('FRAG', 'ADVP'), ('FRAG', 'VP'), ('VP', 'ADVP'), ('SBAR', 'WHNP'),
               ('FRAG', 'SBARQ'), ('PP', 'PP'), ('PRN', 'PP'), ('VP', 'NP'), ('X', 'NP'),
               ('PRN', 'SINV'), ('NP', 'SBAR'), ('PP', 'NP'), ('NP', 'INTJ'), ('FRAG', 'INTJ'),
               ('X', 'VP'), ('PRN', 'NP'), ('FRAG', 'UCP'), ('NP', 'ADVP'), ('SBAR', 'SBARQ'),
               ('SBAR', 'SBAR', 'S'), ('SBARQ', 'WHADVP'), ('ADVP', 'PRT'), ('UCP', 'ADJP'),
               ('PRN', 'FRAG', 'WHADJP'), ('FRAG', 'S'), ('S', 'S'), ('FRAG', 'S', 'ADJP'),
               ('INTJ', 'S'), ('ADJP', 'NP'), ('X', 'ADVP'), ('FRAG', 'WHPP'), ('NP', 'FRAG'),
               ('NX', 'QP'), ('NP', 'S'), ('SBAR', 'WHADVP'), ('X', 'SBARQ'), ('NP', 'PRN'),
               ('NX', 'S', 'VP'), ('NX', 'S'), ('UCP', 'PP'), ('RRC', 'VP'), ('ADJP', 'ADVP'))
...
class BaseParser(object):
    ...
    def __init__(self, name, batch_size=64):
        ...
        if not isinstance(model, dict):
            ...
            self._bert_tokenizer = BertTokenizer((model['vocab']),
              do_lower_case=(meta['bert_do_lower_case']))
    ...
    def _make_feed_dict_elmo(self, sentences):
        ...
        if padded_len > SENTENCE_MAX_LEN:
            raise ValueError('Sentence of length {} exceeds the maximum supported length of {}'.format(padded_len - 2, SENTENCE_MAX_LEN - 2))
        ...
        for snum, sentence in enumerate(sentences):
            ...
            for i, word in enumerate(sentence):
                ...
                if IS_PY2:
                    chars = [
                     ELMO_START_WORD] + [ord(char) for char in word.encode('utf-8', 'ignore')[:48]] + [ELMO_STOP_WORD]
                else:
                    chars = [
                     ELMO_START_WORD] + list(word.encode('utf-8', 'ignore')[:48]) + [ELMO_STOP_WORD]
    ...
    def _make_feed_dict_bert(self, sentences):
        ...
        for snum, sentence in enumerate(sentences):
            ...
            if not len(sentence) + 2 > SENTENCE_MAX_LEN:
                if len(input_ids) > BERT_MAX_LEN:
                    raise ValueError('Sentence of length {} is too long to be parsed'.format(len(sentence)))
        ...
        return {self._input_ids: all_input_ids, 
        ...
        for sentence, datum in sentence_data_pairs:
            ...
            if len(batch_sentences) >= self.batch_size:
                for (chart_np, tags_np), datum in zip(self._make_charts_and_tags(batch_sentences), batch_data):
                    yield (
                     chart_decoder.decode(chart_np), tags_np, datum)
