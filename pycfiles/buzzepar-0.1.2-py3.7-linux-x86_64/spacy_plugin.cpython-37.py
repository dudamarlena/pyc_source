...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/benepar/spacy_plugin.py
# Compiled at: 2020-05-05 12:30:15
# Size of source mod 2**32: 8399 bytes
import numpy as np, spacy
from spacy.tokens import Doc, Span, Token
from .base_parser import BaseParser, PTB_TOKEN_ESCAPE
__all__ = [
 'BeneparComponent', 'NonConstituentException']
...
def get_constituent(span):
    ...
    return (
     constituent_data, found_position)
...
def parse_string(span):
    ...
    def make_str():
        ...
        if i + 1 >= j:
            ...
            children = []
            while idx_cell[0] + 1 < len(constituent_data.starts) and i <= constituent_data.starts[(idx_cell[0] + 1)] and constituent_data.ends[(idx_cell[0] + 1)] <= j:
