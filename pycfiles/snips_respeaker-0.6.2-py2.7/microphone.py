...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/respeaker/microphone.py
...
"""
import os, wave, types, collections, random, string, logging
...
class Microphone:
    ...
    def __init__(self, pyaudio_instance=None, quit_event=None, decoder=None):
        pixel_ring.set_color(rgb=4194304)
        ...
        self.stream = self.pyaudio_instance.open(input=True, start=False, format=pyaudio.paInt16, channels=1, rate=self.sample_rate, frames_per_buffer=self.frames_per_buffer, stream_callback=self._callback, input_device_index=self.device_index)
        ...
        while not self.quit_event.is_set():
            ...
            if hypothesis:
                ...
                if collecting_audio != 'no':
                    ...
                    save_as_wav(('').join(self.detect_history), hypothesis.hypstr)
                self.detect_history.clear()
                if keyword:
