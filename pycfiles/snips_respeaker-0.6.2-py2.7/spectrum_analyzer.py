...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/respeaker/spectrum_analyzer.py
...
class SpectrumAnalyzer:
    ...
    def __init__(self, size, sample_rate=16000, band_number=12, window=[50, 8000]):
        ...
        self.resolution = self.sample_rate / self.size
        self.set_band(band_number, window)
        self.fft = FFT(self.size)

    def set_band(self, n, window=[
 50, 8000]):
        self.band = n
    ...
    def analyze(self, data):
        ...
        for i in range(self.band):
            self.strength[i] = sum(amplitude[self.breakpoints[i]:self.breakpoints[(i + 1)]])
