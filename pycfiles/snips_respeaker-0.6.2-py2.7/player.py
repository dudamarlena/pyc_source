...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/respeaker/player.py
...
"""
import audioop, threading, platform, subprocess, tempfile, types, wave
try:
    import Queue
except:
...
class Player:
    ...
    def __init__(self, pyaudio_instance=None):
        ...
        def ignite(queue):
            ...
            while True:
                ...
                spi.write(address=160, data=level)
                data = queue.get()
    ...
    def _play(self, data, rate=16000, channels=1, width=2, spectrum=True):
        stream = self.pyaudio_instance.open(format=self.pyaudio_instance.get_format_from_width(width), channels=channels, rate=rate, output=True, frames_per_buffer=CHUNK_SIZE)
