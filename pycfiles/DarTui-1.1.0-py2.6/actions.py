...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/dartui/actions.py
# Compiled at: 2012-04-16 18:25:37
import os, time, rtorrent, formatters, common, utils, sql
...
def start_stop_rehash_torrents(mode, rpc_ids):
    ...
    for rpc_id in rpc_ids:
        ...
        if t is not None:
            if mode == 'start':
                t.multicall_add(m, 'd.try_start')
            elif mode == 'stop':
                t.multicall_add(m, 'd.try_stop')
            elif mode == 'rehash':
                t.multicall_add(m, 'd.check_hash')
    ...
    for rpc_id in rpc_ids:
        t = get_torrent(rpc_id)
        if t is not None:
            torrents.append(build_torrent_info(t))
    ...
    for torrent in common.conf.torrent_cache:
        if torrent.rpc_id not in old_rpcids:
            for t in torrent.get_trackers():
                ...
                if url is not None:
                    if url not in common.conf.tracker_cache:
                        common.conf.tracker_cache[url] = []
    ...
    for torrent in common.conf.old_torrent_cache:
        if torrent.rpc_id not in new_rpcids:
            for t in torrent.trackers:
                url = formatters.strip_url(t.url)
                if url is not None:
                    common.conf.tracker_cache[url].remove(torrent.rpc_id)
...
def save_torrent_file(tf):
    ...
    torrent_filename = ('{0}.{1}.torrent').format(os.path.splitext(utils.safe_filename(tf.name))[0], tf.info_hash)
