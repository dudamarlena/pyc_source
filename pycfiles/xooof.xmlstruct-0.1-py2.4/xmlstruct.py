...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/xooof/xmlstruct/xmlstruct.py
...
from xml.sax.handler import ContentHandler, feature_namespaces, property_lexical_handler
...
class XMLStructHandler(ContentHandler, LexicalHandler):
    ...
    def startElementNS(self, name, qname, atts):
        ...
        context = utils.FromSAXContext(self.__structFactory, self.__reader, self.__eb, self.__nsmgr)
...
class _XMLStructFactoryInstance(IXMLStructFactory):
    ...
    def create(self, name):
        if self.__firstTime:
            ...
            if name[0] == self.__instance.xsNamespaceURI() and className == self.__instance.__class__.__name__:
                self.__firstTime = 0
                return self.__instance
            else:
                raise XMLStructFactoryError, 'Name and/or URI do not match for instance'
        elif self.__chainedStructFactory is not None:
            return self.__chainedStructFactory.create(name)
        else:
            raise XMLStructFactoryError, 'No chained struct factory'
        return
...
class _XMLStructCommonBase(IXMLStruct):
    ...
    def xsFromSAX(self, reader, ctx='', structFactory=None):
        eb = ErrorBag()
        context = utils.FromSAXContext(structFactory, reader, eb, utils.NamespacesManager())
...
class XMLStructBase(_XMLStructCommonBase):
    ...
    def xsToSAX(self, handler, nsContext=None):
        self._xsToSAX(handler, (
         self.xsNamespaceURI(), self.__class__.__name__), utils.NamespacesManager(nsContext), self.__class__)
    ...
    def _xsToSAX(self, handler, wrapperName, nsContext, baseKlass):
        ...
        if self.__class__ != baseKlass:
            ...
            atts = AttributesNSImpl({utils.typeAttName: self.__class__.__name__}, {utils.typeAttName: typeAttQName})
        else:
            newContext2 = 0
            atts = utils.noatts
    ...
    def _xsFromSAXNoRaise(self, context, ctx):
        h = handlers.GFieldHandler(context, context.reader.getContentHandler(), context.reader.getProperty(property_lexical_handler), None, None, ctx, self)
    ...
    def _xsValidateNoRaise(self, eb, ctx):
        ok = 1
        for (fieldName, fieldMeta) in self._getMetaFields():
            ok = fieldMeta.validate(getattr(self, fieldName), eb, utils.makeCtx(ctx, fieldName)) and ok
...
class XMLStructBase_C(_XMLStructCommonBase, UserList):
    ...
    def xsToSAX(self, handler, nsContext=None):
        self._xsToSAX(handler, (
         self.xsNamespaceURI(), self._itemKlass.__name__), utils.NamespacesManager(nsContext), self._itemKlass)

    def _xsToSAX(self, handler, wrapperName, nsContext, baseKlass):
        listWrapperName = (
         wrapperName[0], wrapperName[1] + '-list')
    ...
    def _xsFromSAXNoRaise(self, context, ctx):
        h = handlers.GLFieldHandler(context, context.reader.getContentHandler(), context.reader.getProperty(property_lexical_handler), None, None, ctx, self.xsNamespaceURI(), self)
        context.reader.setContentHandler(h)
        context.reader.setProperty(property_lexical_handler, h)
        return
    ...
    def _xsValidateNoRaise(self, eb, ctx):
        ...
        for item in self.data:
            if isinstance(item, self._itemKlass):
                ...
                eb.addError(_('The object must be an instance of class %s') % self._itemKlass, '%s[%d]' % (ctx, index))
                ok = 0
            index += 1
