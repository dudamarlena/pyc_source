...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\cube\network.py
# Compiled at: 2007-04-12 00:21:17
...
import pcapy, socket, struct, re, string
protocols = {socket.IPPROTO_TCP: 'tcp', socket.IPPROTO_UDP: 'udp', socket.IPPROTO_ICMP: 'icmp'}
tcp_flags = {1: 'fin', 2: 'syn', 4: 'rst', 8: 'push', 16: 'ack', 32: 'urg', 64: 'ece', 128: 'cwr'}
...
def decode_tcp_packet(s):
    ...
    """
    d = {}
    d['source_port'] = socket.ntohs(struct.unpack('H', s[0:2])[0]) & 65535
    d['destination_port'] = socket.ntohs(struct.unpack('H', s[2:4])[0]) & 65535
    d['sequence_number'] = struct.unpack('I', s[4:8])[0]
    d['acknowledgement_number'] = struct.unpack('I', s[8:12])[0]
    d['data_offset'] = ord(s[12]) & 240 >> 4
    d['reserved'] = socket.ntohs(struct.unpack('H', s[12:14])[0]) & 4032 >> 6
    d['flags'] = ord(s[13]) & 63
    ...
    if len(s) > 20:
        tempstring = s[20:24]
        if len(tempstring) == 4:
            d['options'] = struct.unpack('I', s[20:24])[0] & 4095
    if len(s) > 24:
        d['padding'] = ord(s[24])
        d['data'] = s[25:]
    return d
