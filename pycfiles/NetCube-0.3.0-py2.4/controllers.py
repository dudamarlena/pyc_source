...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\cube\controllers.py
# Compiled at: 2007-04-12 00:59:44
import config, network, pkg_resources, optparse, scenegraph, display, graphics, struct, socket, time, pygame
...
class GraphicsController(Controller):
    ...
    def __init__(self, config, section='graphics'):
        ...
        for entry in colorlist.split():
            ...
            self.grapher.colors[float(y)] = (float(r), float(g), float(b), float(a))
...
class ConfigurationController(Controller):
    ...
    def __init__(self, argv=None):
        ...
        files = [
         pkg_resources.resource_stream(__name__, 'default.cfg')]
        f = None
        ...
        op.add_option('-i', '--interface', help='read packets from INTERFACE', metavar='INTERFACE', type='string', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'capture', 'option': 'interface'})
        op.add_option('-l', '--list', action='callback', callback=self.listinterfaces)
        op.add_option('-f', '--fullscreen', help='display in fullscreen mode', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'display', 'option': 'fullscreen', 'value': True})
        op.add_option('-p', '--promisc', help='put the interface into promiscuous mode', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'capture', 'option': 'promisc', 'value': True})
        op.add_option('-P', '--nopromisc', dest='promisc', help='DO NOT put the interface into promiscuous mode', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'capture', 'option': 'promisc', 'value': False})
        op.add_option('-a', '--axislabels', help='show axis labels', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'graphics', 'option': 'axislabels', 'value': 1})
        op.add_option('-A', '--noaxislabels', dest='axislabels', help='DO NOT show axis labels', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'graphics', 'option': 'axislabels', 'value': 0})
        op.add_option('-r', '--read', dest='readfile', help='read packets previously captured to FILE', metavar='FILE', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'capture', 'option': 'read'})
        op.add_option('-w', '--write', dest='writefile', help='write packets to FILE', metavar='FILE', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'capture', 'option': 'write'})
        op.add_option('--filter', dest='netfilter', help='filter captured packets using FILTER', metavar='FILTER', default='tcp[tcpflags] == tcp-syn', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'capture', 'option': 'filter'})
        op.add_option('--pointsize', dest='pointsize', help='set the point size to SIZE pixels', metavar='SIZE', default=1.0, type='float', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'display', 'option': 'pointsize'})
        op.add_option('--linewidth', dest='linewidth', help='set the line width to WIDTH pixels', metavar='WIDTH', default=1.0, type='float', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'display', 'option': 'linewidth'})
        op.add_option('--fps', dest='fps', help='attempt to display FRAMES frames per second', metavar='FRAMES', default=30, type='int', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'display', 'option': 'fps'})
        op.add_option('--width', dest='width', help='set display width to WIDTH pixels', metavar='WIDTH', default=320, type='int', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'display', 'option': 'width'})
        op.add_option('--height', dest='height', help='set display height to HEIGHT pixels', metavar='HEIGHT', default=240, type='int', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'display', 'option': 'height'})
        op.add_option('-c', '--colors', dest='colors', help='set the vertical gradient colors', metavar="'<y,r,g,b,a> [ ...]'", default='0.0,1.0,0.0,0.0,1.0 0.5,0.0,1.0,0.0,1.0 1.0,0.0,0.0,1.0,1.0', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'graphics', 'option': 'colors'})
        op.add_option('-t', '--ttl', dest='ttl', help='set the lifetime of packet points (in seconds)', default=None, type='float', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'graphics', 'option': 'ttl'})
        op.add_option('--xcolor', dest='xcolor', help='set the x-axis color', metavar='r,g,b,a', default='0.5,0.0,0.0,0.5', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'graphics', 'option': 'xcolor'})
        op.add_option('--ycolor', dest='ycolor', help='set the y-axis color', metavar='r,g,b,a', default='0.0,0.5,0.0,0.5', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'graphics', 'option': 'ycolor'})
        op.add_option('--zcolor', dest='zcolor', help='set the z-axis color', metavar='r,g,b,a', default='0.0,0.0,0.5,0.5', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'graphics', 'option': 'zcolor'})
        op.add_option('-s', '--spin', dest='spin', help='set the spin rate in degrees / second', default=10.0, type='float', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'graphics', 'option': 'spin'})
        op.add_option('--xmin', dest='xmin', help='x axis minimum address', metavar='a.b.c.d', default='0.0.0.0', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'graphics', 'option': 'xmin'})
        op.add_option('--xmax', dest='xmax', help='x axis maximum address', metavar='a.b.c.d', default='255.255.255.255', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'graphics', 'option': 'xmax'})
        op.add_option('--ymin', dest='ymin', help='y axis minimum port', metavar='x', default=0, type='int', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'graphics', 'option': 'ymin'})
        op.add_option('--ymax', dest='ymax', help='y axis maximum port', metavar='x', default=65535, type='int', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'graphics', 'option': 'ymax'})
        op.add_option('--zmin', dest='zmin', help='z axis minimum address', metavar='a.b.c.d', default='0.0.0.0', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'graphics', 'option': 'zmin'})
        op.add_option('--zmax', dest='zmax', help='z axis maximum address', metavar='a.b.c.d', default='255.255.255.255', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'graphics', 'option': 'zmax'})
        op.add_option('--xcidrip', dest='xcidrip', help='x axis network range in CIDR (IP)', metavar='a.b.c.d', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'graphics', 'option': 'xcidrip'})
        op.add_option('--xcidrmask', dest='xcidrmask', help='x axis network range in CIDR (mask)', metavar='x', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'graphics', 'option': 'xcidrmask'})
        op.add_option('--zcidrip', dest='zcidrip', help='z axis network range in CIDR (IP)', metavar='a.b.c.d', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'graphics', 'option': 'zcidrip'})
        op.add_option('--zcidrmask', dest='zcidrmask', help='z axis network range in CIDR (mask)', metavar='z', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'graphics', 'option': 'zcidrmask'})
        op.add_option('--xbounds', dest='xbounds', help='x axis bounds type (range, cidr, device)', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'graphics', 'option': 'xbounds'})
        op.add_option('--zbounds', dest='xbounds', help='z axis bounds type (range, cidr, device)', action='callback', callback=self.setconfigoption_cb, callback_kwargs={'config': self.parser, 'section': 'graphics', 'option': 'zbounds'})
    ...
    def setconfigoption_cb(self, longopt, shortopt, cmdvalue, parser, config=None, section=None, option=None, value=None):
        ...
        if event is not None:
            if event.type == pygame.VIDEORESIZE:
                ...
                if event.key == K_s:
                    ...
                    elif self.proproot.state() == 'withdrawn':
                        self.proproot.deiconify()
                    else:
                        self.proproot.withdraw()
                        self.saveproprect()
                    ...
                    if saverate:
                        ...
                        rate += mod
            if event.type == QUIT or event.type == KEYDOWN and (event.key == K_ESCAPE or event.key == K_F4 and event.mod & KMOD_ALT):
                if self.proproot is not None:
                    self.saveproprect()
                raise QuitEvent()
        return
