...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.7/site-packages/pynexus/pynexus.py
# Compiled at: 2019-05-22 10:42:38
# Size of source mod 2**32: 17276 bytes
import atexit
...
class NexusConn(object):
    ...
    def cancelChannels(self):
        with self.resTableLock:
            for channel in self.resTable.values():
                channel.put({'jsonrpc':'2.0',  'id':None,  'error':{'code':ErrConnClosed,  'message':ErrStr[ErrConnClosed]}})
    ...
    def executeNoWait(self, method, params, taskId=None):
        with self._stoppingLock:
            if self._stopping:
                return (
                 0, None, {'code':ErrConnClosed,  'message':ErrStr[ErrConnClosed]})
            task_id, channel = self.newId(taskId=taskId)
        req = {'id':task_id,  'method':method, 
         'params':params}
    ...
    def taskPull(self, prefix, timeout=0, taskId=None):
        ...
        if err:
            return (
             None, err)
        task = Task(self, res['taskid'], res['path'], res['method'], res['params'], res['tags'], res['prio'], res['detach'], res['user'])
        return (
         task, None)
...
class Pipe(object):
    ...
    def read(self, mx, timeout=0):
        par = {'pipeid':self.pipeId, 
         'max':mx,  'timeout':timeout}
        ...
        try:
            ...
            return (
             None, {'code':ErrInternal,  'message':ErrStr[ErrInternal]})
            return (
             PipeData(msgres, res['waiting'], res['drops']), None)
