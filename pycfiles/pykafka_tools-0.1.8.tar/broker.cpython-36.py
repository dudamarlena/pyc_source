...
# Embedded file name: build/bdist.linux-x86_64/egg/pykafka/broker.py
# Compiled at: 2018-08-22 17:17:16
# Size of source mod 2**32: 26090 bytes
...
from .protocol import FetchRequest, FetchResponse, ListOffsetRequest, ListOffsetResponse, MetadataRequest, MetadataResponse, OffsetCommitRequest, OffsetCommitResponse, OffsetFetchRequest, OffsetFetchResponse, ProduceResponse, JoinGroupRequest, JoinGroupResponse, SyncGroupRequest, SyncGroupResponse, HeartbeatRequest, HeartbeatResponse, LeaveGroupRequest, LeaveGroupResponse, ListGroupsRequest, ListGroupsResponse, DescribeGroupsRequest, DescribeGroupsResponse, ApiVersionsRequest, ApiVersionsResponse, CreateTopicsRequest, CreateTopicsResponse, DeleteTopicsRequest, DeleteTopicsResponse
...
class Broker(object):
    ...
    def __init__(self, id_, host, port, handler, socket_timeout_ms, offsets_channel_socket_timeout_ms, buffer_size=1048576, source_host='', source_port=0, ssl_config=None, broker_version='0.9.0', api_versions=None):
        ...
        try:
            ...
            log.warning('Failed to connect to broker at {host}:{port}. Check the `listeners` property in server.config.'.format(host=(self._host),
              port=(self._port)))

    def __repr__(self):
        return '<{module}.{name} at {id_} (host={host}, port={port}, id={my_id})>'.format(module=(self.__class__.__module__),
    ...
    @classmethod
    def from_metadata(cls, metadata, handler, socket_timeout_ms, offsets_channel_socket_timeout_ms, buffer_size=65536, source_host='', source_port=0, ssl_config=None, broker_version='0.9.0', api_versions=None):
        ...
        :type api_versions: Iterable of :class:`pykafka.protocol.ApiVersionsSpec`
        """
        return cls((metadata.id), (metadata.host), (metadata.port),
          handler, socket_timeout_ms, offsets_channel_socket_timeout_ms,
    ...
    def connect(self, attempts=3):
        ...
        """
        self._connection = BrokerConnection((self.host), (self.port), (self._handler),
    ...
    def connect_offsets_channel(self, attempts=3):
        ...
        channel
        """
        self._offsets_channel_connection = BrokerConnection((self.host),
          (self.port), (self._handler), buffer_size=(self._buffer_size),
          source_host=(self._source_host),
          source_port=(self._source_port),
          ssl_config=(self._ssl_config))
        self._offsets_channel_connection.connect((self._offsets_channel_socket_timeout_ms), attempts=attempts)
        self._offsets_channel_req_handler = RequestHandler(self._handler, self._offsets_channel_connection)
    ...
    def _get_unique_req_handler(self, connection_id):
        ...
        if len(self._req_handlers) == 0:
            ...
            if connection_id not in self._req_handlers:
                conn = BrokerConnection((self.host),
                  (self.port), (self._handler), buffer_size=(self._buffer_size), source_host=(self._source_host),
                  source_port=(self._source_port))
    ...
    @_check_handler
    def fetch_messages(self, partition_requests, timeout=30000, min_bytes=1):
        ...
        future = self._req_handler.request(request_class(partition_requests=partition_requests,
          timeout=timeout,
          min_bytes=min_bytes,
    ...
    def request_metadata(self, topics=None):
        ...
        for i in range(max_retries):
            ...
            try:
                ...
                log.warning('Encountered SocketDisconnectedError while requesting metadata from broker %s:%s. Continuing.', self.host, self.port)
                continue
            ...
            for name, topic_metadata in iteritems(response.topics):
                ...
                for pid, partition_metadata in iteritems(topic_metadata.partitions):
                    if partition_metadata.err == LeaderNotAvailable.ERROR_CODE:
                        log.warning("Leader not available for topic '%s' partition %d.", name, pid)

            return response

    def commit_consumer_group_offsets(self, consumer_group, consumer_group_generation_id, consumer_id, preqs):
        ...
        req = OffsetCommitRequest(consumer_group, (get_bytes(consumer_group_generation_id)),
          consumer_id,
    ...
    def join_group(self, connection_id, consumer_group, member_id, topic_name, membership_protocol):
        ...
        if handler is None:
            raise SocketDisconnectedError
        future = handler.request(JoinGroupRequest(consumer_group, member_id, topic_name, membership_protocol))
        self._handler.sleep()
    ...
    def sync_group(self, connection_id, consumer_group, generation_id, member_id, group_assignment):
        ...
        if handler is None:
            raise SocketDisconnectedError
        future = handler.request(SyncGroupRequest(consumer_group, generation_id, member_id, group_assignment))
    ...
    def heartbeat(self, connection_id, consumer_group, generation_id, member_id):
        ...
        if handler is None:
            raise SocketDisconnectedError
        future = handler.request(HeartbeatRequest(consumer_group, generation_id, member_id))
        self._handler.sleep()
    ...
    def create_topics(self, topic_reqs, timeout=0):
        ...
        :type timeout: int
        """
        future = self._req_handler.request(CreateTopicsRequest(topic_reqs, timeout=timeout))
