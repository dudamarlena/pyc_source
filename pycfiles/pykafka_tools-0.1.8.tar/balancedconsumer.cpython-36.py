...
# Embedded file name: build/bdist.linux-x86_64/egg/pykafka/balancedconsumer.py
...
from .exceptions import KafkaException, PartitionOwnedError, ConsumerStoppedException, UnicodeException
...
class BalancedConsumer(object):
    ...
    def __init__(self, topic, cluster, consumer_group, fetch_message_max_bytes=1048576, num_consumer_fetchers=1, auto_commit_enable=False, auto_commit_interval_ms=60000, queued_max_messages=2000, fetch_min_bytes=1, fetch_error_backoff_ms=500, fetch_wait_max_ms=100, offsets_channel_backoff_ms=1000, offsets_commit_max_retries=5, auto_offset_reset=OffsetType.EARLIEST, consumer_timeout_ms=-1, rebalance_max_retries=5, rebalance_backoff_ms=2000, zookeeper_connection_timeout_ms=6000, zookeeper_connect=None, zookeeper_hosts='127.0.0.1:2181', zookeeper=None, auto_start=True, reset_offset_on_start=False, post_rebalance_callback=None, use_rdkafka=False, compacted_topic=False, membership_protocol=RangeProtocol, deserializer=None, reset_offset_on_fetch=True):
        ...
        self._offsets_commit_max_retries = valid_int(offsets_commit_max_retries, allow_zero=True)
        self._auto_offset_reset = auto_offset_reset
        self._zookeeper_connect = zookeeper_connect or zookeeper_hosts
        self._zookeeper_connection_timeout_ms = valid_int(zookeeper_connection_timeout_ms, allow_zero=True)
        ...
        self._consumer = None
        self._consumer_id = get_bytes('{hostname}:{uuid}'.format(hostname=(socket.gethostname()),
          uuid=(uuid4())))
        self._setting_watches = True
        self._topic_path = '/consumers/{group}/owners/{topic}'.format(group=(get_string(self._consumer_group)),
          topic=(self._topic.name))
        self._consumer_id_path = '/consumers/{group}/ids'.format(group=(get_string(self._consumer_group)))
    ...
    def _partitions(self):
        ...
        return set([] if self.partitions is None else itervalues(self.partitions))
    ...
    def start(self):
        ...
        try:
            if self._zookeeper is None:
                self._setup_zookeeper(self._zookeeper_connect, self._zookeeper_connection_timeout_ms)
        ...
        if partitions is None:
            ...
            if partitions != self._partitions:
                ...
                if self._post_rebalance_callback is not None:
                    old_offsets = self._consumer.held_offsets if self._consumer else dict()
                    new_offsets = cns.held_offsets
                    try:
                        reset_offsets = self._post_rebalance_callback(self, old_offsets, new_offsets)
                    except Exception:
                        ...
                        if reset_offsets:
                            cns.reset_offsets(partition_offsets=[(cns.partitions[id_], offset) for id_, offset in iteritems(reset_offsets)])
                self._consumer = cns
            ...
            elif self._internal_consumer_running.is_set():
    ...
    def _get_internal_consumer(self, partitions=None, start=True):
        ...
        if partitions is None:
            ...
            Cls = rdkafka.RdKafkaSimpleConsumer if self._use_rdkafka else SimpleConsumer
            cns = Cls((self._topic),
              (self._cluster),
    ...
    def _get_participants(self):
        ...
        try:
            ...
            log.debug("Consumer group doesn't exist. No participants to find")
            return []
        else:
            participants = []
    ...
    def _set_watches(self):
        ...
        try:
            self._broker_watcher = ChildrenWatch(self._zookeeper, broker_path, _brokers_changed)
        except NoNodeException:
            raise Exception('The broker_path "%s" does not exist in your ZooKeeper cluster -- is your Kafka cluster running?' % broker_path)

        self._topics_watcher = ChildrenWatch(self._zookeeper, '/brokers/topics', _topics_changed)
        self._consumer_watcher = ChildrenWatch(self._zookeeper, self._consumer_id_path, _consumers_changed)
        self._setting_watches = False

    def _add_self(self):
        """Register this consumer in zookeeper."""
        self._zookeeper.create((self._path_self),
          (self._topic.name), ephemeral=True, makepath=True)

    @property
    def _path_self(self):
        """Path where this consumer should be registered in zookeeper"""
        return '{path}/{id_}'.format(path=(self._consumer_id_path),
    ...
    def _update_member_assignment(self):
        ...
        for i in range(self._rebalance_max_retries):
            try:
                ...
                new_partitions = self._membership_protocol.decide_partitions(participants, self._topic.partitions, self._consumer_id)
                if not new_partitions:
                    log.warning('No partitions assigned to consumer %s', self._consumer_id)
                ...
                if i == self._rebalance_max_retries - 1:
                    log.warning('Failed to acquire partition %s after %d retries.', ex.partition, i)
                    raise
    ...
    def _add_partitions(self, partitions):
        ...
        for p in partitions:
            try:
                self._zookeeper.create((self._path_from_partition(p)),
    ...
    def _get_held_partitions(self):
        ...
        for partition_slug in all_partitions:
            try:
                owner_id, stat = self._zookeeper.get('{path}/{slug}'.format(path=(self._topic_path),
                  slug=partition_slug))
    ...
    def _brokers_changed(self, brokers):
        ...
        if self._setting_watches:
            return
        log.debug('Rebalance triggered by broker change ({})'.format(self._consumer_id))
        self._rebalance()
    ...
    def _consumers_changed(self, consumers):
        ...
        if self._setting_watches:
            return
        log.debug('Rebalance triggered by consumer change ({})'.format(self._consumer_id))
        self._rebalance()
    ...
    def _topics_changed(self, topics):
        ...
        if self._setting_watches:
            return
        log.debug('Rebalance triggered by topic change ({})'.format(self._consumer_id))
        self._rebalance()
