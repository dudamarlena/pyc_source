...
# Embedded file name: build/bdist.linux-x86_64/egg/tests/pykafka/test_balancedconsumer.py
# Compiled at: 2018-07-24 15:14:34
# Size of source mod 2**32: 21457 bytes
from __future__ import division
import math, mock, os, pkg_resources, platform, pytest, time, threading, unittest2
...
from pykafka.membershipprotocol import GroupMembershipProtocol, RoundRobinProtocol, RangeProtocol
...
class TestBalancedConsumer(unittest2.TestCase):
    ...
    @classmethod
    def buildMockConsumer(self, consumer_group='testgroup', num_partitions=10, num_participants=1, timeout=2000):
        topic = mock.Mock()
        ...
        zk = mock.MagicMock()
        return (
         BalancedConsumer(topic, cluster, consumer_group, zookeeper=zk,
           auto_start=False,
           use_rdkafka=False,
           consumer_timeout_ms=timeout), topic)

    def test_unicode_consumer_group(self):
        consumer, _ = self.buildMockConsumer(consumer_group='testgroup')
    ...
    def _test_decide_partitions(self, membership_protocol):
        for i in range(100):
            ...
            participants = sorted(['test-debian:{p}'.format(p=p) for p in range(num_participants)])
            cns, topic = self.buildMockConsumer(num_partitions=num_partitions, num_participants=num_participants)
            cns._membership_protocol = membership_protocol
            assigned_parts = []
            for consumer_id in participants:
                partitions = cns._membership_protocol.decide_partitions(participants, topic.partitions, consumer_id)
...
class TestManagedBalancedConsumer(TestBalancedConsumer):
    ...
    def buildMockConsumer(self, consumer_group='testgroup', num_partitions=10, num_participants=1, timeout=2000):
        ...
        cluster = mock.MagicMock()
        cns = ManagedBalancedConsumer(topic, cluster, consumer_group, auto_start=False,
          use_rdkafka=False,
...
class BalancedConsumerIntegrationTests(unittest2.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.prod = cls.client.topics[cls.topic_name].get_producer(min_queued_messages=1)
 ...
 consumer_group, managed=self.MANAGED_CONSUMER, **kwargs)
    ...
    def test_extra_consumer(self):
        ...
        try:
            consumers = [self.get_balanced_consumer(group, consumer_timeout_ms=5000) for i in range(self.n_partitions + extras)]
    ...
    def test_rebalance_callbacks(self):
        ...
        try:
            consumer_group = 'test_rebalance_callbacks'
            consumer_a = self.get_balanced_consumer(consumer_group,
              zookeeper_connect=(self.kafka.zookeeper),
              auto_offset_reset=(OffsetType.EARLIEST),
              post_rebalance_callback=on_rebalance,
              use_rdkafka=(self.USE_RDKAFKA))
            consumer_b = self.get_balanced_consumer(consumer_group,
              zookeeper_connect=(self.kafka.zookeeper),
              auto_offset_reset=(OffsetType.EARLIEST),
              use_rdkafka=(self.USE_RDKAFKA))
    ...
    def test_rebalance_callbacks_surfaces_errors(self):
        ...
        self.offset_reset = 50
        try:
            consumer_group = 'test_rebalance_callbacks_error'
            consumer_a = self.get_balanced_consumer(consumer_group,
              zookeeper_connect=(self.kafka.zookeeper),
              auto_offset_reset=(OffsetType.EARLIEST),
              post_rebalance_callback=on_rebalance,
    ...
    def test_consume_earliest(self):
        try:
            consumer_a = self.get_balanced_consumer('test_consume_earliest',
              zookeeper_connect=(self.kafka.zookeeper),
              auto_offset_reset=(OffsetType.EARLIEST),
              use_rdkafka=(self.USE_RDKAFKA))
            consumer_b = self.get_balanced_consumer('test_consume_earliest',
              zookeeper_connect=(self.kafka.zookeeper),
              auto_offset_reset=(OffsetType.EARLIEST),
              use_rdkafka=(self.USE_RDKAFKA))
            ...
            self.assertTrue(len(messages) == 1)
            self.assertSetEqual(consumer_a._partitions & consumer_b._partitions, set())
            self.assertSetEqual(consumer_a._partitions | consumer_b._partitions, set(self.client.topics[self.topic_name].partitions.values()))
        finally:
            try:
                consumer_a.stop()
                consumer_b.stop()
    ...
    def test_consume_latest(self):
        try:
            consumer_a = self.get_balanced_consumer('test_consume_latest',
              zookeeper_connect=(self.kafka.zookeeper),
              auto_offset_reset=(OffsetType.LATEST),
              use_rdkafka=(self.USE_RDKAFKA))
            consumer_b = self.get_balanced_consumer('test_consume_latest',
              zookeeper_connect=(self.kafka.zookeeper),
              auto_offset_reset=(OffsetType.LATEST),
              use_rdkafka=(self.USE_RDKAFKA))
            self.wait_for_rebalancing(consumer_a, consumer_b)
    ...
    def test_external_kazoo_client(self):
        ...
        zk.start()
        consumer = self.get_balanced_consumer('test_external_kazoo_client',
          zookeeper=zk,
    ...
    def test_no_partitions(self):
        ...
        consumer = self.get_balanced_consumer('test_no_partitions',
          zookeeper_connect=(self.kafka.zookeeper),
          auto_start=False,
          consumer_timeout_ms=50,
          use_rdkafka=(self.USE_RDKAFKA))
        consumer._membership_protocol = GroupMembershipProtocol(consumer._membership_protocol.protocol_type, consumer._membership_protocol.protocol_name, consumer._membership_protocol.metadata, _decide_dummy)
        consumer.start()
    ...
    def test_zk_conn_lost(self):
        ...
        zk.start()
        try:
            consumer_group = 'test_zk_conn_lost'
            consumer = self.get_balanced_consumer(consumer_group, zookeeper=zk,
              use_rdkafka=(self.USE_RDKAFKA))
            self.assertTrue(check_partitions(consumer))
            with consumer._rebalancing_lock:
                zk.stop()
            other_consumer = self.get_balanced_consumer(consumer_group,
              use_rdkafka=(self.USE_RDKAFKA))
            with consumer._rebalancing_lock:
                zk.start()
    ...
    def wait_for_rebalancing(self, *balanced_consumers):
        ...
        for _ in range(500):
            ...
            if max(n_parts) - min(n_parts) <= 1:
                if sum(n_parts) == self.n_partitions:
                    break
            else:
                balanced_consumers[0]._cluster.handler.sleep(0.2)
...
@pytest.mark.skipif((kafka_version < version_09), reason='Managed consumer unsupported until 0.9')
class ManagedBalancedConsumerIntegrationTests(BalancedConsumerIntegrationTests):
    MANAGED_CONSUMER = True


@pytest.mark.skipif((platform.python_implementation() == 'PyPy' or kafka_version < version_09 or gevent is None),
