...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/tests/test_cli.py
# Compiled at: 2018-03-25 12:03:13
...
class TestCli(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.env = {'CLEANRC_PATH': str(self.test_cleanrc_path)}
        self.initialPath = {'glob': 'fuga', 
         'path': 'hoge', 
         'use_meta_tag': False}
    ...
    def test_move(self):
        """Run cleaning."""
        test_from_dir = self.current_dir / 'test_from_dir'
        ...
        test_value = {'glob': str(test_from_dir) + '/*', 
         'path': str(test_to_dir)}
        runner = CliRunner()
        runner.invoke(cli.add, [test_value['glob'], test_value['path']], env=self.env)
    ...
    def test_meta_tag(self):
        ...
        self.assertEqual({
         ...
         str(test_to_dir) + '/20180315000000/bar'}, {str(x) for x in files})
