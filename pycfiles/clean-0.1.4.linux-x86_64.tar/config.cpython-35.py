...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/clean/config.py
# Compiled at: 2018-03-24 06:51:27
...
class NoConfigFileException(Exception):
    __doc__ = 'this exception throws If the config file found.'
...
class Config(object):
    __doc__ = 'Config file manager class.\n\n    Returns:\n        Config -- config file instance\n\n    '
    ...
    def __init__(self, config_path: Path=None):
        ...
        if not self.config_path.is_file():
            if self.config_path.exists():
                click.echo("Can't create file. Same name something is exist. " + "Please check your home's {}.".format(str(config_path)))
                exit(1)
            self._create_new_config_file()
        self._load_file()

    def add_glob_path(self, glob: str, path: str, enable_meta_tag: bool=True) -> bool:
        """Add new glob path to config file."""
        if self._is_contain_same_config(glob, path):
            return False
        self.config['path'].append({'glob': glob, 
         'path': path, 
    ...
    def _is_contain_same_config(self, glob: str, path: str) -> bool:
        return any(x['path'] == path and x['glob'] == glob for x in self.config['path'])
    ...
    def delete_glob_path(self, id: int) -> dict:
        ...
        if not self.config['path']:
            click.echo('There is no path settings. ' + 'Please add a path setting by "add" command.')
            return
        if 0 > id:
            click.echo('Please input 0 or positive id. The max id is {}.'.format(len(self.config['path'])))
        if len(self.config['path']) <= id:
            click.echo('The id is too big. Please input 0 <= id < {}.'.format(len(self.config['path'])))
            return
    ...
    def _back_up_file(self):
        with self.config_path.open(encoding='utf_8') as (f):
            with (self.config_path.parent / (self.config_path.name + '.bk')).open(mode='w', encoding='utf_8') as (g):
                g.write(f.read())
