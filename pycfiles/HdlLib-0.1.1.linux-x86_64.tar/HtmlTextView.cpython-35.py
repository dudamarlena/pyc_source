...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/HdlLib/Utilities/HtmlTextView.py
# Compiled at: 2017-07-08 08:29:58
# Size of source mod 2**32: 22150 bytes
"""
...
import xml.sax, xml.sax.handler, re, warnings
from io import StringIO
import urllib.request, urllib.error, urllib.parse, operator, webbrowser
__all__ = [
 'HtmlTextView']
whitespace_rx = re.compile('\\s+')
allwhitespace_rx = re.compile('^\\s*$')
Screen = Gdk.Screen.get_default()
display_resolution = 0.3514598 * (Screen.get_height() / float(Screen.get_height_mm()))
...
class HtmlHandler(xml.sax.handler.ContentHandler):
    ...
    def _parse_style_background_color(self, tag, value):
        ...
        def _get_current_attributes(self):
            ...
            attrs.font_scale = self._get_current_style_attr('scale', operator.mul)
    ...
    def __parse_length_frac_size_allocate(self, textview, allocation, frac, callback, args):
    ...
    def _parse_length(self, value, font_relative, callback, *args):
        ...
        if value.endswith('%'):
            ...
            if font_relative:
                ...
                self._HtmlHandler__parse_length_frac_size_allocate(self.textview, alloc, frac, callback, args)
                self.textview.connect('size-allocate', self._HtmlHandler__parse_length_frac_size_allocate, frac, callback, args)
        else:
    ...
    _HtmlHandler__parse_font_size_cb = staticmethod(_HtmlHandler__parse_font_size_cb)

    def _parse_style_font_size(self, tag, value):
        try:
            scale = {'xx-small': pango.SCALE_XX_SMALL, 
             'x-small': pango.SCALE_X_SMALL, 
             'small': pango.SCALE_SMALL, 
             'medium': pango.SCALE_MEDIUM, 
             'large': pango.SCALE_LARGE, 
             'x-large': pango.SCALE_X_LARGE, 
             'xx-large': pango.SCALE_XX_LARGE}[value]
        except KeyError:
            pass
        else:
            attrs = self._get_current_attributes()
        ...
        if value == 'larger':
            ...
            return
        self._parse_length(value, True, self._HtmlHandler__parse_font_size_cb, tag)

    def _parse_style_font_style(self, tag, value):
        try:
            style = {'normal': pango.STYLE_NORMAL, 
             'italic': pango.STYLE_ITALIC, 
             'oblique': pango.STYLE_OBLIQUE}[value]
        except KeyError:
    ...
    _HtmlHandler__frac_length_tag_cb = staticmethod(_HtmlHandler__frac_length_tag_cb)

    def _parse_style_margin_left(self, tag, value):
        self._parse_length(value, False, self._HtmlHandler__frac_length_tag_cb, tag, 'left-margin')

    def _parse_style_margin_right(self, tag, value):
        self._parse_length(value, False, self._HtmlHandler__frac_length_tag_cb, tag, 'right-margin')

    def _parse_style_font_weight(self, tag, value):
        try:
            weight = {'100': pango.WEIGHT_ULTRALIGHT, 
    ...
    def _parse_style_text_align(self, tag, value):
        try:
            align = {'left': Gtk.JUSTIFY_LEFT, 
             'right': Gtk.JUSTIFY_RIGHT, 
             'center': Gtk.JUSTIFY_CENTER, 
             'justify': Gtk.JUSTIFY_FILL}[value]
        except KeyError:
    ...
    _HtmlHandler__style_methods = dict()
    for style in ['background-color', 'color', 'font-family', 'font-size',
     ...
     'text-align', 'text-decoration']:
        try:
            ...
            _HtmlHandler__style_methods[style] = method
    ...
    def _begin_span(self, style, tag=None):
        ...
        for attr, val in [item.split(':', 1) for item in style.split(';')]:
            ...
            val = val.strip()
            try:
                method = self._HtmlHandler__style_methods[attr]
            except KeyError:
                warnings.warn("Style attribute '%s' requested but not yet implemented" % attr)
            else:
                method(self, tag, val)
                        ...
                        if name == 'ul':
                            ...
                            if name == 'ol':
                                ...
                                if name == 'li':
                                    ...
                                    if name == 'img':
                                        try:
                                            mem = urllib.request.urlopen(attrs['src']).read(10485760)
...
class HtmlTextView(Gtk.TextView):
    ...
    __gsignals__ = {'url-clicked': (GObject.SIGNAL_RUN_LAST, None, (str, str))}
    ...
    def __init__(self):
        ...
        self.connect('motion-notify-event', self._HtmlTextView__motion_notify_event)
        self.connect('leave-notify-event', self._HtmlTextView__leave_event)
        self.connect('enter-notify-event', self._HtmlTextView__motion_notify_event)
        self.connect('event', self._HtmlTextView__on_event)
    ...
    def __on_event(self, Widget, Event):
        if Event.type == Gdk.BUTTON_PRESS and Event.button == 1:
            x, y, _ = Widget.window.get_pointer()
            x, y = Widget.window_to_buffer_coords(Gtk.TEXT_WINDOW_TEXT, x, y)
            StartIter = Widget.get_iter_at_location(x, y)
            tags = StartIter.get_tags()
            for tag in tags:
                if getattr(tag, 'is_anchor', False):
                    is_over_anchor = True
                    ...
                    webbrowser.open_new_tab(URLText)
                    break
    ...
    def display_html(self, html):
        ...
        parser.parseString(StringIO(html))
