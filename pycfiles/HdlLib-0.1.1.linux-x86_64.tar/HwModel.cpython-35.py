...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/HdlLib/SysGen/HW/HwModel.py
# Compiled at: 2017-07-08 08:29:58
# Size of source mod 2**32: 21147 bytes
__all__ = ()
import os, sys, logging, shutil, tempfile, re, collections
...
class HwModel:
    ...
    def __init__(self, Name):
        """
                ...
                """
        self.Name = Name
        self.Config = HwLib.ArchiConfig(self.Name)
        self.InterfaceDict = self.Config.GetInterface()
        self.AvailableRsc = HwResources(RscDict=self.Config.GetAvailableRsc(), FPGAModel=self)
        self.XMLElmt = None
    ...
    def ModuleResources(self, Mod, Library, ParamVal={}, RemoteHost=None):
        ...
        for PName in PNames:
            if PName in ExternalPads:
                continue
            elif PName in ClockResets:
                continue
        ...
        if 'RouteReport' in ResultFilesDict:
            ...
            if 'PlaceReport' in ResultFilesDict:
    ...
    def GetPadConstraints(self, Module, Library, OutputPath='./Constraints', ControlHW=None):
        """
                ...
                """
        ConstraintPath, ModuleWrapper, NewClockSigs = HwConstraints.GenConstraintsFile(Module=Module, HwModel=self, ControlHW=ControlHW, Library=Library, OutputPath=OutputPath)
        return (
         ConstraintPath, ModuleWrapper, NewClockSigs)
    ...
    def Upload(self, BinaryPath, ScanChainPos=1, Remote=False):
        """
                ...
                """
        return FpgaOperations.Upload(HwModel=self, BinaryPath=BinaryPath, ScanChainPos=ScanChainPos, Remote=Remote)
...
def GetUsedResourcesDict(MapFilePath):
    ...
    with open(MapFilePath, 'r') as (RsrcFile):
        for Line in RsrcFile.readlines():
            Matched = re.match('^\\s*\\|\\s*(Slice|Block)\\s*(?P<Resource>[\\w\\d]+).*\\s*\\|\\s*(?P<Used>[<.,\\d]+)\\s*\\|\\s*([<,.\\d]+)\\s*\\|\\s*(?P<Available>[<,.\\d]+)\\s*\\|\\s*(?P<Percentage>[<,.\\d]+)\\s*\\|\\s*.*$', Line)
            if Matched:
