...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/HdlLib/SysGen/HW/HwResources.py
...
class HwResources:
    ...
    def CompleteRscInfo(self, FPGAModel, RscName, Used, Available, Percentage):
        ...
        if Available == -1:
            ...
            if RscName in AvRsc:
                Available = AvRsc[RscName][0]
            if Percentage == -1:
                Percentage = float(Used) / float(Available)
            return (
    ...
    def AddResources(self, Name, Used, Available, Percentage):
        ...
        Name = Name.upper()
        if self.FPGAModel:
            Available, Percentage = self.CompleteRscInfo(FPGAModel, Name, Used, Available, Percentage)
        self.RscDict[Name] = (
         int(Used), int(Available), int(Percentage))
        return True
    ...
    def SetFromSynthesisDir(self, SynthesisDir):
        ...
        for RName, (Used, Available, Percentage) in Rsc.items():
            self.RscDict[RName.upper().replace(' ', '_')] = (
    ...
    def __iadd__(self, Other):
        ...
        if isinstance(Other, HwResources):
            for RName, (Used, Available, Percentage) in Other.RscDict.items():
                if self.FPGAModel:
                    Available, Percentage = self.CompleteRscInfo(self.FPGAModel, RName, Used, Available, Percentage)
                if RName not in self.RscDict:
                    self.RscDict[RName] = (
                     Used, Available, Percentage)
                else:
