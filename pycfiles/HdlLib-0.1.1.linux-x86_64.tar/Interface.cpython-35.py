...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/HdlLib/SysGen/Interface.py
# Compiled at: 2017-07-08 08:29:58
# Size of source mod 2**32: 23786 bytes
...
from HdlLib.SysGen.Protocol import DirectedProtocol
...
class Interface:
    ...
    def __init__(self, Service, XMLElmt=None, Infos={}):
        ...
        if XMLElmt is not None:
            ...
            self.SerializationFactors = {D.Name:1 for D in self.DataList if D.Direction == 'IN' if D.Direction == 'IN'}
        self.SignalDict = {}
        for S in self.Signals:
            self.SignalDict[S.Name] = S

        try:
            int(eval(str(self.Size), self.ParamVars))
        except:
    ...
    def GetSize(self, Eval=True):
        ...
        if self.Size is None:
            ...
            if Eval is True:
                Size = int(eval(self.Size, self.ParamVars))
            else:
                Size = self.Size
        return Size
    ...
    def SetFromXML(self, XMLElmt):
        ...
        self.Size = XMLElmt.attrib.get('number')
        ...
        for CtrlElmt in XMLElmt.iterchildren('datawidth'):
            ...
            N = Attr.get('name')
            if N not in self.Service.Params:
        ...
        for CtrlElmt in XMLElmt.iterchildren('control'):
            Attr = CtrlElmt.attrib
            N = Attr.get('name')
            if N not in self.Service.Ports:
        ...
        try:
            self.Protocol.SetFromXML(XMLElements=list(XMLElmt.iterchildren('protocol')))
        except:
    ...
    def AddSignal(self, Signal, Target=None, Ctrl=True):
        """
                ...
                """
        if Ctrl is True:
            self.CtrlList.append(Signal)
        else:
            self.DataList.append(Signal)
        if Target:
            self.Mapping[Signal.GetName()] = Target
    ...
    def GetXMLElmt(self):
        """
                ...
                """
        ItfElmt = etree.Element('interface', name=self.Name, direction=self.Direction, number=str(self.Size))
        for C in self.CtrlList:
            CtrlElmt = etree.SubElement(ItfElmt, 'control', name=C.Name, target=self.Mapping[C.Name] if C.Name in self.Mapping else C.Name)

        for C in self.DataList:
            DataElmt = etree.SubElement(ItfElmt, 'data', name=C.Name, target=self.Mapping[C.Name] if C.Name in self.Mapping else C.Name)
            if C.Name in self.Registers:
                for RDict in self.Registers[C.Name]:
                    ...
                    if Value is None:
                        RegElmt = etree.SubElement(DataElmt, 'sets', register=Register, position=Position)
                    else:
                        RegElmt = etree.SubElement(DataElmt, 'sets', register=Register, position=Position, value=Value)
    ...
    def SetupRegisters(self, Signal, Register, Value):
        ...
        if SignalName not in self.Registers:
        ...
        for RegDict in self.Registers[SignalName]:
            if RegDict['Name'] == Register:
                ...
                if 'Position' not in RegDict:
                    RegDict['Position'] = len(self.Registers[SignalName])
                RegDict['Value'] = Value
        ...
        self.Registers[SignalName].append({'Data': Signal, 
    ...
    def Connect(self, ITF, XMLReqServ=None, Ref=False, KeepNames=False):
        ...
        if self.CurIndex != None:
            FormalIDX = ':{0}'.format(self.CurIndex)
        else:
            FormalIDX = ''
        IntSignalCreated = []
        Mapping = {}
        if Ref:
            for Sig in self.Signals:
                if self.CurIndex != None:
                    LocalIndex = ':' + str(self.CurIndex)
                else:
                    LocalIndex = ''
                IntSignal = '.'.join([self.Service.Alias, str(Sig.Name) + LocalIndex])
                if XMLReqServ:
                    MAP = etree.SubElement(XMLReqServ, 'map', formal=Sig.Name + FormalIDX, actual=IntSignal)
                Mapping[str(Sig) + LocalIndex] = (
                 [
                  [
                   self.Service.Alias, str(Sig.Name), self.CurIndex]], True, {})
                S = Sig.Copy()
                S.Name = IntSignal
            ...
            for Sig in self.Signals:
                ...
                if XMLReqServ:
                    MAP = etree.SubElement(XMLReqServ, 'map', formal=Sig.Name + FormalIDX, actual='.'.join([ITF.Service.Alias, ActualName]))
                Mapping[str(Sig) + FormalIDX] = (
                 [
                  [
                   ITF.Service.Alias, ActualName, ITF.CurIndex]], True, {})
                S = Sig.Copy()
                S.Name = IntSignal
                IntSignalCreated.append(S)

            return (
             Mapping, IntSignalCreated)
    ...
    def GetSignals(self, Ctrl=True, Data=True, Directions=['IN', 'OUT']):
        ...
        Sigs = []
        if Data == True:
            Sigs += [x for x in self.DataList if x.Direction in Directions]
        if Ctrl == True:
            Sigs += [x for x in self.CtrlList if x.Direction in Directions]
    ...
    def __getitem__(self, index):
        ...
        if isinstance(index, int):
            if int(eval(self.Size, self.ParamVars)) > 1:
                self.CurIndex = index
            else:
                self.CurIndex = None
