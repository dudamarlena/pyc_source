...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/HdlLib/GUIGeneric.py
# Compiled at: 2017-07-08 08:29:58
# Size of source mod 2**32: 10712 bytes
...
def Display(VHDLFilePath, ParentWindow=None):
    ...
    if not os.path.isfile(VHDLFilePath):
        logging.error("[Display] Given VHDL file '{0}' does not exist or is not a file.".format(VHDLFilePath))
        return False
    else:
        if not VHDLFilePath.lower().endswith('.vhd'):
            logging.error('[Display] Given file does not have a VHDL extension.'.format(VHDLFilePath))
            return False
...
class Interface:
    __doc__ = 'This is the GUI builder for Generic GUI'
    ...
    def InitGUI(self, ParentWindow=None):
        ...
        if self.IconPath:
            self.MainWindow.set_icon_from_file(IconPath)
    ...
    def Popup(self, title, text, dialog_type='info', ErrorMsg=None):
        ...
        if dialog_type == 'check':
            ...
            dialog = Gtk.MessageDialog(self.MainWindow, modal=True, message_type=Gtk.MessageType.INFO, buttons=Gtk.ButtonsType.CANCEL, text=text)
