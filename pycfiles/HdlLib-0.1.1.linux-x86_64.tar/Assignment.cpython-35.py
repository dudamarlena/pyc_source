...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/HdlLib/SysGen/Assignment.py
# Compiled at: 2017-07-08 08:29:58
# Size of source mod 2**32: 10315 bytes
...
class AssignmentSignal:
    ...
    def AssignTo(self, Other):
        ...
        if Other.NonAssignedBits == 0:
            ...
            return
        if Other.NonAssignedBits > self.NonAssignedBits:
            BitsToBeAssign = self.NonAssignedBits
            logging.debug('Full space for {0} in {1} - Assign {2} bits'.format(self, Other, BitsToBeAssign))
            Assignments.Add(Assignee=Other.Sig[Other.IMin:Other.IMin + self.NonAssignedBits], Assignor=self.Sig[self.IMin:self.IMin + self.NonAssignedBits], Cond=None)
            ...
            if Other.NonAssignedBits < self.NonAssignedBits:
                ...
                Assignments.Add(Assignee=Other.Sig[Other.IMin:Other.IMin + Other.NonAssignedBits], Assignor=self.Sig[self.IMin:self.IMin + Other.NonAssignedBits], Cond=None)
                ...
                BitsToBeAssign = self.NonAssignedBits
                logging.debug('Full space for {0} in {1} - Assign {2} bits'.format(self, Other, BitsToBeAssign))
                Assignments.Add(Assignee=Other.Sig, Assignor=self.Sig, Cond=None)
...
class AssignmentStatement:
    ...
    def __init__(self, Assignee=None, Assignor=None, Cond=None, Desc=''):
        ...
        if Assignee is None:
            ...
            self.Add(Assignee=Assignee, Assignor=Assignor, Cond=Cond, Desc=Desc)
    ...
    def Add(self, Assignee, Assignor=None, Cond=None, Desc=''):
        ...
        if isinstance(Assignee, AssignmentStatement):
            ...
            if isinstance(Assignee, list):
                ...
                if len(Statements) != 0:
                    ADict = {'Condition': Cond, 'Statements': Statements}
            ...
            if isinstance(Assignee, dict):
                ...
                self._Hierarchy.append(ADict)
                return True
            if hasattr(Assignee, 'GenericSize'):
    ...
    def __iter__(self):
        ...
        for ADict in self._Hierarchy:
            yield (
             ADict['Condition'], ADict['Statements'])
    ...
    def Pop(self, Index=0):
        ...
        if Index < len(self._Hierarchy):
            ...
            return False
        return self._Hierarchy.pop(Index)
