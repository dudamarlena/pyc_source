...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\pySPM\Bruker.py
# Compiled at: 2019-05-21 08:54:40
# Size of source mod 2**32: 6102 bytes
"""
Module to handle SPM images recorded by a Bruker AFM
"""
import struct, re, numpy as np, pySPM, warnings
...
class Bruker:
    ...
    def _get_raw_layer(self, i):
        ...
        self.file.seek(off)
        return np.array((struct.unpack('<' + str(length) + {2:'h',  4:'i',  8:'q'}[bpp], self.file.read(byte_length))),
    ...
    def get_channel(self, channel='Height Sensor', backward=False, corr=None, debug=False, encoding='latin1', lazy=True):
        ...
        for i in range(len(self.layers)):
            ...
            result = re.match('([^ ]+) \\[([^\\]]*)\\] "([^"]*)"', layer_name).groups()
            if result[2] == channel:
                if debug:
                ...
                if bck == backward:
                    if debug:
                        ...
                        if debug:
                            print('@2:Z scale', var)
                        if '[' in var:
                            result = re.match('[A-Z]+\\s+\\[([^\\]]+)\\]\\s+\\(-?[0-9\\.]+ .*?\\)\\s+(-?[0-9\\.]+)\\s+(.*?)$', var).groups()
                            if debug:
                                print(result)
                            ...
                            result = re.match('[A-Z]+ \\(-?[0-9\\.]+ [^\\)]+\\)\\s+(-?[0-9\\.]+) [\\w]+', var).groups()
                        scale = float(result[0]) / 65536.0
                        scale2 = 1
                        zscale = b'V'
                        result = re.match('[A-Z]+ \\(-?[0-9\\.]+ .*?\\)\\s+(-?[0-9\\.]+) .*?', self.layers[i][b'@2:Z offset'][0].decode(encoding)).groups()
                        offset = float(result[0])
                    ...
                    size = {'x':float(scan_size[0]),  'y':float(scan_size[1]), 
                     'unit':scan_size[2].decode(encoding)}
                    image = pySPM.SPM_image(channel=([
                     channel, 'Topography'][(channel == 'Height Sensor')]),
                      BIN=data,
                      real=size,
                      _type='Bruker AFM',
