...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\pySPM\utils\fit.py
# Compiled at: 2019-05-21 08:54:40
# Size of source mod 2**32: 11123 bytes
"""
...
def lgCDF_fit(x, y, p0, dic=False):
    ...
    if dic:
        d = np.diag(pcov)
        return (
         dict(bg=(popt[0]), lg=(popt[1]), A=(popt[2::3]), x0=(popt[3::3]), sig=(popt[4::3])), dict(bg=(d[0]), lg=(d[1]), A=(d[2::3]), x0=(d[3::3]), sig=(d[4::3])))
    else:
        return (
         popt, pcov)
...
def CDF_fit(x, y, p0, dic=False):
    ...
    if dic:
        d = np.diag(pcov)
        return (
         dict(bg=(popt[0]), A=(popt[1::3]), x0=(popt[2::3]), sig=(popt[3::3])), dict(bg=(d[0]), A=(d[2::3]), x0=(d[3::3]), sig=(d[4::3])))
    else:
        return (
         popt, pcov)
...
def LG2Dr(A, ratio=np.sqrt(2), Rweight=None, sigma=None, dic=False, **kargs):
    ...
    from scipy.optimize import curve_fit
    params = [
     'amplitude', 'angle', 'sig_y', 'x0', 'y0', 'LG_x', 'LG_y', 'bg']
    ...
    bounds = [[bounds_def[0][i] for i, x in enumerate(params) if x not in kargs], [bounds_def[1][i] for i, x in enumerate(params) if x not in kargs]]
    ...
    if dic:
        ...
        return (
         dict(zip(params2, pout)), dict(zip(params2, dpout)))
    else:
        return (
         pout, dpout)
...
def LG2D(A, Rweight=None, sigma=None, dic=False, **kargs):
    ...
    from scipy.optimize import curve_fit
    params = [
     'amplitude', 'angle', 'sig_x', 'sig_y', 'x0', 'y0', 'LG_x', 'LG_y', 'assym_x', 'assym_y', 'bg']
