...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\pySPM\ToF.py
# Compiled at: 2019-05-21 08:54:40
# Size of source mod 2**32: 7576 bytes
"""
This is an old module which is deprecated now. It is kept in the project as it can be used to read BIF6 and BIF3D images.
"""
import numpy as np, struct, os, re, matplotlib.pyplot as plt, pySPM
Elements = {'H':{1:(1.00782503223, 0.999885), 
  2:(2.01410177812, 0.000115)}, 
 'Li':{6:(6.0151228874, 0.0759), 
  7:(7.0160034366, 0.9241)}, 
 'Be':{9: (9.012183065, 1)}, 
 'B':{10:(10.01293695, 0.199), 
  11:(11.00930536, 0.801)}, 
 'C':{12:(12, 0.9893), 
  13:(13.00335483507, 0.01107)}, 
 'N':{14:(14.00307400443, 0.99636), 
  15:(15.00010889888, 0.00364)}, 
 'O':{16:(15.99491461957, 0.99757), 
  17:(16.9991317565, 0.00038),  18:(17.99915961286, 0.00205)}, 
 'F':{19: (18.9984036273, 1)}, 
 'Ag':{107:(106.9050916, 0.51839), 
  109:(108.9047553, 0.48161)}, 
 'Au':{197: (196.96656879, 1)}, 
 'Si':{28:(27.97692653465, 0.92223), 
  29:(28.9764946649, 0.04685),  30:(29.973770136, 0.3092)}, 
 'Ti':{46:(45.95262772, 0.0825), 
  47:(46.95175879, 0.0744),  48:(47.94794198, 0.7372),  49:(48.94786568, 0.0541),  50:(49.94478689, 0.0518)}}
...
class BIF6:
    ...
    def showImgElt(self, elt, size=10, abundCorr=False, tot=True):
        ...
        fig, ax = plt.subplots(((len(ks) + 1) // 4 + 1), 4, figsize=(
         10 * ((len(ks) + 1) // 4 + 1), 10))
        ...
        for i, k in enumerate(ks):
            if abundCorr:
                ax[((i + di) // 4)][((i + di) % 4)].imshow((A[k]['data'].pixels / A[k]['abund']),
                  vmin=0, vmax=ma)
            else:
                A[k]['data'].show(ax=(ax[((i + di) // 4)][((i + di) % 4)]),
                  vmin=0,
                  vmax=ma)
...
class BIF3D:
    ...
    def __init__(self, path):
        ...
        for x in os.listdir(self.Path):
            if x[:len(self.Basename)] == self.Basename and x[-6:] == '.BIF3D':
                s = self.Basename + ' \\(([0-9]+)\\)(?: - (.*?))?\\.BIF3D'
                r = re.search(s, x)
                ID = int(r.group(1))
    ...
    def getIDs(self, channels):
        if not type(channels) in [str, int, list, tuple]:
    ...
    def getChannel(self, channel):
        ...
        if k in self.data:
            ...
            f = open(path.format(path=(self.Path), sep=(os.sep), basename=(self.Basename),
              ID=k,
              Peak=v), 'rb')
            A = f.read()
            f.close()
