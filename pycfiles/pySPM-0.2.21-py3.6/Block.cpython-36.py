...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\pySPM\Block.py
# Compiled at: 2019-05-21 08:54:40
# Size of source mod 2**32: 33284 bytes
"""
Module to handle block type used in iontof file formats ITA,ITM,ITS, etc...
"""
from __future__ import absolute_import
import sys, binascii, struct, os
...
class Block:
    ...
    def __init__(self, fp, parent=None):
        ...
        Blocks of type \x80\x19\x00\x00\x00 have it's values compressed with zlib (notice the x^ at the beginning which is typical for zlib encoded data)
        ...
        if self.Type[1:] != b'\x19\x00\x00\x00':
            raise ValueError('Wrong block type ({Type}) found @{pos}'.format(pos=(self.offset),
              Type=(binascii.hexlify(self.Type[1:]))))
        if len(self.Type) < 5:
            raise ValueError('EOF reached. Block cannot be read')
        self.head = dict(zip(['name_length', 'ID', 'N', 'length1', 'length2'], struct.unpack('<5I', self.f.read(20))))
    ...
    def add_child(self, blk):
        ...
        if NextBlock != 0:
            ...
            self.f.write(struct.pack('<B4I2Q', blk.Type[0], index, len(blk.name), blk.head['ID'], [0, 1][(blk.Type[0] in (0,
                                                                                                                          128))], blk.head['length1'], blk.offset))
    ...
    def get_list(self):
        ...
        """
        if self.Type[0:1] not in (b'\x01', b'\x03'):
            return []
        else:
    ...
    def create_list(self, limit=None, debug=False):
        ...
        while True:
            ...
            head = dict(zip(['name_length', 'ID', 'N', 'length1', 'length2'], struct.unpack('<5x5I', data)))
            name = self.f.read(head['name_length'])
            data = self.f.tell()
            ...
            for i in range(N):
                ...
                S = dict(zip(['index', 'slen', 'id', 'blen', 'bidx'], struct.unpack('<III4xQQ', self.f.read(32))))
                ...
                if child.Type[0:1] == b'\x00':
                    if len(child.value) == 4:
                        ...
                        if len(child.value) % 2 == 0:
                            ...
                            if len(vS) > 20:
                                vS = vS[:20] + '...'
                            print(('{name} ({id}) <{blen}> @{bidx}, value = {value} (hex) = "{vS}" (UTF-16)= {vL} ({Dtype}){other}'.format)(value=value, 
                             vL=vL, Dtype=Dtype, other=other, vS=vS, **l))
                        else:
                            print(('{name} ({id}) <{blen}> @{bidx}, value = {value} (hex) = {vL} ({Dtype}){other}'.format)(value=value, 
                             vL=vL, Dtype=Dtype, other=other, **l))
                else:
                ...
                if l['id'] == 0 or all:
                    if digraph:
                        out.write('"{parent}-{name}" [label="{name}"]\n"{parent}" -> "{parent}-{name}"\n'.format(parent=parent,
                          name=(l['name'].decode('utf8'))))
                    else:
                        if ex is None:
                            out.write(('{tab}{name} ({id}) @{bidx}\n'.format)(tab='\t' * level, **l))
                    if level < maxlevel:
                        try:
                            self.goto_item(l['name'], l['id']).show(maxlevel,
                              (level + 1), all=all, out=out, digraph=digraph, parent=(parent + '-' + l['name']),
                              ex=ex)
                        except:
                            pass
    ...
    def modify_block_and_export(self, path, new_data, output, debug=False, prog=False, lazy=False):
        if not not os.path.exists(output):
            ...
            while self.f.tell() < FILE_SIZE:
                ...
                if current.offset == block_offset:
                    debug_msg.append('Block to change FOUND!')
                    out.write(self.f.read(5))
                    ...
                    self.f.read(20)
                    out.write(self.f.read(block.head['name_length']))
                    self.f.read(block.head['length1'])
                    out.write(new_data)
                elif current.Type[0] in (1, 3):
                    debug_msg.append('Block container found. Checking children...')
                    out.write(self.f.read(25))
                    out.write(self.f.read(current.head['name_length']))
                    SubHeader = list(struct.unpack('<2I5s5IQ', self.f.read(41)))
                    if SubHeader[8] > block_offset:
                        SubHeader[8] += length_diff
                    out.write((struct.pack)(*('<2I5s5IQ', ), *SubHeader))
                    N = current.head['N']
                    for i in range(N):
                        X, index, slen, id, Y, blen, bidx = struct.unpack('<B4I2Q', self.f.read(33))
                        if bidx == block_offset:
                            blen = len(new_data)
                        else:
                            if bidx > block_offset:
                                bidx += length_diff
                        out.write(struct.pack('<B4I2Q', X, index, slen, id, Y, blen, bidx))

                    delta = curr_block_length - (self.f.tell() - current.offset)
