...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\pySPM\ITM.py
# Compiled at: 2019-05-21 08:54:40
# Size of source mod 2**32: 58889 bytes
from __future__ import absolute_import
from . import Block
import numpy as np, struct, os.path, zlib, re, os
...
class ITM:
    ...
    def __init__(self, filename, debug=False, readonly=True, precond=False, label=None):
        ...
        end with À, the second with Ð, the third to @, the a suite
        of uint32 which don't end with À and the sequence starts again. Each
        sequence tell: The first uint32 terminating by À tells the x
        coordinates of the pixel location (just replace the À byte by 0 and
        read it as a uint32) The second uint32 terminating by\cD0 tells the y
        coord.  of the pixel. The third terminating by @ tells the pixel
        ...
        try:
            ...
            self.size = {'pixels':{'x':d['res_x']['long'], 
              'y':d['res_y']['long']}, 
             'real':{'x':d['fieldofview']['float'], 
        ...
        try:
            self.size['Scans'] = self.root.goto('filterdata/TofCorrection/ImageStack/Reduced Data/ImageStackScans/Image.NumberOfScans').get_ulong()
        except:
            pass
    ...
    def show_summary(self, fig=None, plot=True, **kargs):
        ...
        if plot:
            ...
            if SI is not None:
                ...
                (SI.show)(ax=ax, **{k:kargs[k] for k in kargs if k not in ('high',
                                                                           'low')})
                index += 1
            ...
            if EC is not None or Supp is not None or Press is not None:
                ...
                if Press is not None:
                    ...
                    ax.plot(t, Press[:, 1] * 1000000.0, 'C2')
    ...
    def get_values(self, prog=False, start=False, end=True, names=[], startsWith='', nest=False, hidePrefix=True, numeric=False):
        ...
        for start in startEnd:
            ...
            if prog:
                List = PB(List)
            for l in List:
                Node = self.root.goto(['propend', 'propstart'][start]).goto_item(l['name'], l['id'])
    ...
    def auto_mass_cal(self, t=None, S=None, pos=True, debug=False, error=False, Range=5000, fitting_peaks=['C', 'CH', 'CH2', 'CH3', 'Na'], apply=False, **kargs):
        ...
        if 'FittingPeaks' in kargs:
            ...
            if type(fitting_peaks) is str:
                ...
                time_width = 10000000000.0 * self.root.goto('propend/Instrument.LMIG.Chopper.Width').get_key_value()['float']
    ...
    def precond(self, amp=10000, do_mass_cal=False):
        ...
        amp0 = np.max(s[mask])
        mH = get_mass('H' + '+-'[(self.polarity == 'Negative')])
        if do_mass_cal or amp0 < max(100, 0.05 * self.Nscan * self.size['pixels']['x'] * self.size['pixels']['y']):
    ...
    def get_spectrum(self, sf=None, k0=None, scale=None, time=False, error=False, **kargs):
        ...
        """
        RAW = zlib.decompress(self.root.goto('filterdata/TofCorrection/Spectrum/Reduced Data/IITFSpecArray/' + ['CorrectedData', 'Data'][kargs.get('uncorrected', False)]).value)
        if scale is None:
            scale = self.scale
    ...
    def show_stability(self, ax=None, prog=False):
        ...
        if ax is None:
            ...
            ax = plt.gca()
        self.show_meas_data(ax=ax, scans=3, mul=1000000.0, prog=prog)
        axb = dual_plot(ax)
    ...
    def spectra_per_pixel(self, pixel_aggregation=None, peak_lim=0, scans=None, prog=False, safe=True, FOVcorr=True, smooth=False):
        ...
        if pixel_aggregation is None:
            ...
            gun = self.root.goto('propend/Instrument.PrimaryGun.Species').get_key_value()['string']
            if gun[(-1)] != '+':
                gun += '+'
            Q = gun.count('+')
            nrj = self.root.goto('propend/Instrument.PrimaryGun.Energy').get_key_value()['float']
            dx = self.size['real']['x'] / self.size['pixels']['x']
            mp = get_mass(gun)
            if FOVcorr:
                DT = dx * 20000000000.0 * 0.5 * np.sqrt(2) * np.sqrt(0.001 * mp / const.NA / (Q * 2 * nrj * const.qe))
            else:
                DT = 0
            if peak_lim is None:
                peak_lim = 0
            ...
            dts = DT * (self.size['pixels']['x'] / 2 - np.arange(self.size['pixels']['x']))
    ...
    def show_spectrum(self, low=0, high=None, sf=None, k0=None, ax=None, log=False, show_peaks=False, **kargs):
        ...
        if 'showPeaks' in kargs:
            ...
            if show_peaks:
                ...
                pos = []
                index = 0
                for P in [x for x in self.peaks if self.peaks[x]['desc']['utf16'] not in ('total',
                                                                                          'sum of rest')]:
                    p = self.peaks[P]
                    c = p['cmass']['float']
    ...
    def get_raw_spectrum(self, scans=None, ROI=None, FOVcorr=True, deadTimeCorr=True, **kargs):
        ...
        if gun[(-1)] != '+':
            ...
            number_channels = int(round(self.root.goto('propend/Measurement.CycleTime').get_key_value()['float'] / self.root.goto('propend/Registration.TimeResolution').get_key_value()['float']))
        ...
        if deadTimeCorr:
            dt = 1300
            N = self.Nscan * self.size['pixels']['x'] * self.size['pixels']['y']
    ...
    def show_stage(self, ax=None, markers=False, plist=False):
        ...
        """
        import pickle, matplotlib as mpl
        ...
        if markers:
            ...
            ll = toXY((X - self.fov * 500.0, Y - self.fov * 5000.0), W, H)
            ur = toXY((X + self.fov * 500.0, Y + self.fov * 5000.0), W, H)
    ...
    def create_new_miblock(self, assign, lmass=None, umass=None, cmass=None, desc='', _uuid=None, **kargs):
        ...
        defaults = {'clsid':('raw', b'\x85\x1b\xa9\xe4hZ\xc8M\x93\xe0\x7f\xc0\xf8\xe2\xbb\xd9'), 
