...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\pySPM\tools\win32_helper.py
# Compiled at: 2019-05-21 08:54:40
# Size of source mod 2**32: 8021 bytes
from ctypes import wintypes
import win32gui, win32api, win32con, ctypes, struct, array, os, sys, psutil
...
def _getMultipleWindowValues(hwnd, getCountMessage, getValueMessage):
    ...
    for itemIndex in range(valuecount):
        ...
        valueLength = win32gui.SendMessage(hwnd, getValueMessage, itemIndex, valuebuffer)
...
def getListboxItems(hwnd):
    return _getMultipleWindowValues(hwnd, getCountMessage=(win32con.LB_GETCOUNT),
...
def findTopWindow(wantedText=None, wantedClass=None, selectionFunction=None):
    ...
    if topWindows:
        return topWindows[0]
    raise WinGuiAutoError('No top level window found for wantedText=' + repr(wantedText) + ', wantedClass=' + repr(wantedClass) + ', selectionFunction=' + repr(selectionFunction))
...
def findTopWindows(wantedText=None, wantedClass=None, selectionFunction=None):
    ...
    for hwnd, windowText, windowClass in topWindows:
        if wantedText:
            if _normaliseText(wantedText) not in _normaliseText(windowText):
                continue
        else:
            if wantedClass:
                if not windowClass == wantedClass:
                    continue
            if selectionFunction:
                if not selectionFunction(hwnd):
                    continue
        results.append(hwnd)
...
def findWindow(name='', C='', parent=0):
    ...
    def callbck(hwnd, lParam):
        ...
        if name == '' or title.startswith(name):
            if C == '' or c == C:
                R.append(hwnd)
        return True
...
def findControl(topHwnd, wantedText=None, wantedClass=None, selectionFunction=None):
    controls = findControls(topHwnd, wantedText=wantedText,
      wantedClass=wantedClass,
      selectionFunction=selectionFunction)
    if controls:
        return controls[0]
    raise WinGuiAutoError('No control found for topHwnd=' + repr(topHwnd) + ', wantedText=' + repr(wantedText) + ', wantedClass=' + repr(wantedClass) + ', selectionFunction=' + repr(selectionFunction))
            ...
            for childHwnd, windowText, windowClass in childWindows:
                ...
                if wantedClass:
                    if not windowClass == wantedClass:
                        continue
                if selectionFunction:
                    if not selectionFunction(childHwnd):
                        continue
...
def _sendNotifyMessage(hwnd, nofifyMessage):
    ...
    win32gui.SendMessage(win32gui.GetParent(hwnd), win32con.WM_COMMAND, _buildWinLong(nofifyMessage, win32api.GetWindowLong(hwnd, win32con.GWL_ID)), hwnd)
