...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\pySPM\ITA.py
# Compiled at: 2019-05-21 08:54:40
# Size of source mod 2**32: 41678 bytes
"""
...
import numpy as np, struct, os.path, zlib, re, copy
from .ITM import ITM
...
class ITA(ITM):
    ...
    def __init__(self, filename, *args, **kargs):
        ...
        try:
            self.Nimg = int(self.root.goto('filterdata/TofCorrection/ImageStack/Reduced Data/ImageStackScans/Image.NumberOfImages').get_ulong())
        except:
            self.Nimg = 0
    ...
    def show_channels(self, ch):
        ...
        """
        for z in ch:
            print('\t{name} ({desc}), mass: {lower:.2f} - {upper:.2f}'.format(desc=(z['desc']['utf16']),
              name=(z['assign']['utf16']),
              lower=(z['lmass']['float']),
              upper=(z['umass']['float'])))
    ...
    def get_channel_by_mass(self, mass, full=False):
        ...
        for P in self.peaks:
            p = self.peaks[P]
            if p['id']['long'] > 1:
                if p['lmass']['float'] <= mass:
                    if mass <= p['umass']['float']:
                        if full:
                            return p
                        else:
                            return p['id']['long']
    ...
    def get_sum_image_by_name(self, names, scans=None, strict=False, prog=False, raw=False, **kargs):
        ...
        if scans is None:
            ...
            if prog:
                scans = PB(scans)
        Z = self._ITA__get_sum_image(scans, channels)
        if raw:
            return (Z, channels)
        else:
    ...
    def show(self, ax=None):
        ...
        ax.imshow((self.img), extent=(0, self.fov * 1000000.0, 0, self.fov * 1000000.0))
    ...
    def get_added_image_by_name(self, names, strict=False, raw=False, **kargs):
        ...
        **kargs: supplementary arguments
        ...
        if raw:
            ...
            return (
             self.image((np.flipud(Z)), channel=(','.join([z['assign'] for z in ch]))), ch)
    ...
    def get_sum_image_by_mass(self, masses, scans=None, prog=False, raw=False, **kargs):
        ...
        Z = (self._ITA__get_sum_image)(scans, channels, **kargs)
        if raw:
            return (Z, channels)
        else:
    ...
    def get_added_image(self, channel, **kargs):
        ...
        """
        if not type(channel) is int:
            raise AssertionError
        elif not (channel >= 0 and channel < self.Nimg):
            raise AssertionError
    ...
    def get_image(self, channel, scan, shifts=None, shift_mode='roll', const=0, **kargs):
        ...
        if 'Shifts' in kargs:
            shifts = kargs.pop('Shifts')
        elif 'ShiftMode' in kargs:
            shift_mode = kargs.pop('ShiftMode')
        elif not type(channel) is int:
            raise AssertionError
        elif not type(scan) is int:
            raise AssertionError
        else:
            if not (channel >= 0 and channel < self.Nimg):
                raise AssertionError
            elif not (scan >= 0 and scan < self.Nscan):
                raise AssertionError
        ...
        for i, it in enumerate(items):
            ...
            while OPTYPE != 3:
                ...
                if OPTYPE == 4:
                    ...
                    palette = np.array(blk.goto_item('BMP-Palette').get_data('B')).reshape((256,
                                                                                            4))
...
class ITA_collection(Collection):
    ...
    def __init__(self, filename, channels1=None, channels2=None, name=None, mass=False, strict=False):
        ...
        self.name = name
        Collection.__init__(self, sx=(self.ita.fov), sy=(self.ita.fov * self.ita.sy / self.ita.sx), unit='m',
          name=name)
        self.msg = ''
                ...
                if type(channels) is dict:
                    for x in channels:
                        if mass:
                            ...
                            Z, ch = self.ita.get_added_image_by_name(channels[x], strict)
                            self.msg += '{0}\n'.format(x)
                            for z in ch:
                                self.msg += '\t{name} ({desc}), mass: {lower:.2f} - {upper:.2f}\n'.format(desc=(z['desc']),
                                  name=(z['assign']),
    ...
    def stitch_correction(self, channel, stitches, gauss=0, debug=False):
        ...
        for x in self.channels:
            ...
            for i in range(stitches[0]):
                for j in range(stitches[1]):
                    F[sy * i:sy * (i + 1), sx * j:sx * (j + 1)] = self.channels[x].pixels[sy * i:sy * (i + 1), sx * j:sx * (j + 1)] / S
