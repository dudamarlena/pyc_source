...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\pySPM\utils\misc.py
...
def adaptive(img):
    import skimage
    mi = np.min(img)
    img = np.asarray((65536 * (img - mi) / (np.max(img) - mi)), dtype=(np.uint16))
    img = skimage.exposure.equalize_adapthist(img, clip_limit=0.03)
    return img


def smiley(width, height=None, ratio=0.9, eye=0.15, thick=0.1, eye_sep=0.35, eye_height=0.3, mouth_rad=0.5, mouth_thick=0.1):
    if height is None:
        height = width
...
def getBAM(x, x0, N=10, least_one=False):
    ...
    for key in sorted(pos):
        width = [
         d[key], d[key] // 2][(key[0] == 'P')]
        lines.append((pos[key], width))
        if key[0] == 'P':
            lines.append((pos[key] + d[key], width))

    for pos, width in lines:
        P[(x - x0 >= pos) * (x - x0 <= pos + width)] = 1
        if least_one:
            P[np.argmin(abs(x - x0 - pos))] = 1
