...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\pySPM\collection.py
# Compiled at: 2019-05-21 08:54:40
# Size of source mod 2**32: 9621 bytes
"""
...
"""
import copy, re, numpy as np, matplotlib.pyplot as plt
...
class Collection:
                ...
                if channels_number == 4:
                    if ncols == 3:
                        fig, ax = plt.subplots(2, 2, figsize=(width,
                         self[channels[0]].pixels.shape[0] * width / self[channels[0]].pixels.shape[1]))
                else:
                    Ny = (channels_number - 1) // ncols + 1
                    Nx = min(ncols, channels_number)
                    fig, ax = plt.subplots(Ny, Nx, figsize=(
                     width, ((channels_number - 1) // ncols + 1) * width / Nx))
    ...
    def overlay(self, channel_names, colors=[
 [
  1, 0, 0], [0, 1, 0], [0, 0, 1]], sig=None, vmin=None, vmax=None, **kargs):
        """
        ...
        """
        if not len(channel_names) >= 2:
            raise AssertionError
        else:
            assert len(colors) >= len(channel_names)
            data = [SPM.normalize((self[ch].pixels), sig=sig, vmin=vmin, vmax=vmax) for ch in channel_names]
            layers = [np.stack([data[i] * colors[i][j] for j in range(3)], axis=2) for i in range(len(channel_names))]
    ...
    def stitch_correction(self, channel, stitches):
        ...
        for x in self.channels:
            ...
            for i in range(stitches[0]):
                for j in range(stitches[1]):
                    F[sy * i:sy * (i + 1), sx * j:sx * (j + 1)] = self.channels[x][sy * i:sy * (i + 1), sx * j:sx * (j + 1)] / S
...
def overlay_triangle(channel_names, colors=[
 [
  1, 0, 0], [0, 1, 0], [0, 0, 1]], radius=0.8, proportion=0.8, ax=None, size=512, bgcolor=[0, 0, 0], textcolor='white', fontsize=20):
    """
    ...
    for j in range(3):
        ...
        ax.annotate((channel_names[j]), (radius * np.cos(np.radians(120 * j)), radius * np.sin(np.radians(120 * j))), color=textcolor,
          fontsize=fontsize,
          va='center',
          ha='center')
