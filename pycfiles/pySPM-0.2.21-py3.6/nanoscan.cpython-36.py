...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\pySPM\nanoscan.py
# Compiled at: 2019-05-21 08:54:40
# Size of source mod 2**32: 8251 bytes
import os, base64, xml.etree.ElementTree as ET, struct, numpy as np, pySPM.SPM
...
def get_curve(filename, channel='Normal Deflection', backward=False):
    ...
    RAW = root.findall("spm:vector/spm:contents/spm:direction/spm:vector/spm:contents/spm:name[spm:v='{direction}']/../spm:channel/spm:vector/spm:contents/spm:name[spm:v='{channel}']/../spm:data/spm:v".format(direction=([
...
class Nanoscan:
    ...
    def __init__(self, filename=None):
        if not os.path.exists(filename):
            ...
            if self.root.tag == '{http://www.nanoscan.ch/SPM}scan':
                ...
                self.pixel_size = [int(z) for z in self._Nanoscan__grab('spm:vector/spm:contents/spm:size/spm:contents//spm:v')]
                uval = float(self._Nanoscan__grab('.//spm:area//spm:contents/spm:size/spm:contents/spm:fast_axis/spm:v'))
                udispu = self._Nanoscan__grab('.//spm:area//spm:contents/spm:display_unit/spm:v')
                udisps = float(self._Nanoscan__grab('.//spm:area/spm:contents/spm:display_scale/spm:v'))
                uname = self._Nanoscan__grab('.//spm:area/spm:contents/spm:unit/spm:v')
                x = funit(uval * udisps, udispu)
                uval = float(self._Nanoscan__grab('.//spm:area//spm:contents/spm:size/spm:contents/spm:slow_axis/spm:v'))
                y = funit(uval * udisps, udispu)
                self.size = {'unit':x['unit'], 
                 'x':x['value'], 
                 'y':y['value']}
                try:
                    self.feedback = {'channel': self._Nanoscan__grab('{0}/spm:z_feedback_channel/spm:v'.format(self.fbPath))}
                    self.feedback['P'] = {'value':float(self._Nanoscan__grab('{0}/spm:proportional_z_gain/spm:v'.format(self.fbPath))), 
                     'unit':self._Nanoscan__grab('{0}/spm:proportional_z_gain_unit/spm:v'.format(self.fbPath))}
                    self.feedback['I'] = {'value':float(self._Nanoscan__grab('{0}/spm:integral_z_time/spm:v'.format(self.fbPath))), 
                     'unit':self._Nanoscan__grab('{0}/spm:integral_z_time_unit/spm:v'.format(self.fbPath))}
                    if self.feedback['channel'] == 'df':
                        self.feedback['channel'] = 'Δf'
                except:
                    self.feedback = {}

                self.scan_speed = {z:{'value':float(self._Nanoscan__grab("spm:vector//spm:direction/spm:vector/spm:contents/spm:name[spm:v='{dir}']/../spm:point_interval/spm:v".format(dir=z))) * self.pixel_size[0],  'unit':self._Nanoscan__grab("spm:vector//spm:direction/spm:vector/spm:contents/spm:name[spm:v='{dir}']/../spm:point_interval_unit/spm:v".format(dir=z))} for z in ('forward',
                                                                                                                                                                                                                                                                                                                                                                                           'backward')}
            else:
    ...
    def get_channel(self, channel='Topography', backward=False, corr=None):
        try:
            RAW = self._Nanoscan__grab("spm:vector//spm:direction/spm:vector/spm:contents/spm:name[spm:v='{direction}']/../spm:channel//spm:contents/spm:name[spm:v='{channel}']/../spm:data/spm:v".format(direction=([
             'forward', 'backward'][backward]),
              channel=channel))
        except:
    ...
    def array_summary(self):
        ...
        res = [(y.format)(**self.__dict__) for y in ('{filename}', '{pixel_size[0]}×{pixel_size[1]}',
                                                     '{size[x][value]}×{size[y][value]} {size[x][unit]}',
                                                     '{scan_speed[forward][value]} {scan_speed[forward][unit]}',
                                                     '{feedback[channel]}', '{P[value]:.2f} {P[unit]}',
                                                     '{I[value]:.2f} {I[unit]}')]
    ...
    def get_summary(self):
        ...
        return 'Feedback: {feedback[channel]} : P:{P[value]}{P[unit]} : I:{I[value]}{I[unit]}\nSize: {pixel_size[0]}×{pixel_size[1]} pixels = {x[value]:.3} {x[unit]}×{y[value]:.3} {y[unit]}\nScan Speed: {scanSpeed[value]}{scanSpeed[unit]}/line'.format(x=x,
          y=y,
          P=P,
          I=I,
    ...
    def show_dir_summary(path):
        from pySPM.utils import htmlTable
        res = [
         ['Filename', 'pixel size', 'real size',
          'scan_speed', 'feedback', 'P', 'I']]
        for x in os.listdir(path):
            try:
                A = pySPM.Nanoscan(path + x)
                res.append([(y.format)(f=os.path.basename(A.filename), **A.__dict__) for y in ('{f}',
                                                                                               '{pixel_size[0]}×{pixel_size[1]}',
                                                                                               '{size[x]}×{size[y]} {size[unit]}',
                                                                                               '{scan_speed[forward][value]} {scan_speed[forward][unit]}',
                                                                                               '{feedback[channel]}',
                                                                                               '{feedback[P][value]:.2f} {feedback[P][unit]}',
