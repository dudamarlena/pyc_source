...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\pySPM\utils\elts.py
# Compiled at: 2019-05-21 08:54:40
# Size of source mod 2**32: 13517 bytes
"""
...
def formulafy(x):
    ...
    """
    import re
    return '$' + re.sub('([a-zA-Z])_?([0-9]+)', '\\1_{\\2}', re.sub('\\^([0-9]+)', '^{\\1}', re.sub('([\\+-]+)$', '^{\\1}', x))) + '$'
...
def get_peaklist(nominal_mass, negative=False):
    ...
    if type(nominal_mass) in [float, int]:
        ...
        if type(nominal_mass) in [tuple, list]:
...
def _dict2formula(elts, debug=False):
    ...
    for A, sym in sorted([x for x in elts if x[1] not in order],
      key=(lambda x: x[1])):
        n = elts[(A, sym)]
        if n == 0:
            pass
...
def elts_nm(elts, NM):
    ...
    res = []
    if type(elts) is str:
        elts = re.compile('((?:^[0-9]+)?[A-Z][a-z]?(?:_[0-9]+)?)').findall(elts)
    me = [get_mass(x) for x in elts]
    while r:
        rnew = _elts_add(r, list(zip([_formula2dict(x) for x in elts], me)))
        r = [x for x in rnew if x[1] < NM + 0.5]
        res += [_dict2formula(x[0]) for x in r if x[1] > NM - 0.5 if _dict2formula(x[0]) not in res]
        res = list(set(res))
