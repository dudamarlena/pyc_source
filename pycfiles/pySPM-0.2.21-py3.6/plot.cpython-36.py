...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\pySPM\utils\plot.py
...
def Xdist(ax, left, right, y, color='r', linestyle=':', fmt='{dist:.1f}{unit}', xtransf=lambda x: x, va='bottom', ha='center', offset=(0, 2), **kargs):
    ...
    **kargs: additional arguments.
    ...
    (ax.annotate)(fmt.format(dist=(xtransf(right - left)), unit=(kargs.get('unit', ''))), ({'center':0.5 * left + right,  'left':right,  'right':left}[ann['ha']], y), offset, textcoords='offset pixels', **ann)
...
def Ydist(ax, down, up, x, color='r', linestyle=':', fmt='{dist:.2f}{unit}', ytransf=lambda y: y, rotation=90, va='center', ha='right', offset=(-2, 0), **kargs):
    ...
    **kargs: additional arguments.
    ...
    ann = dict(va=va, ha=ha, color=color)
    ann.update({k[3:]:kargs[k] for k in kargs if k.startswith('an_')})
    (ax.annotate)(fmt.format(dist=(ytransf(up - down)), unit=(kargs.get('unit', ''))), (x, {'center':0.5 * down + up,  'top':up,  'bottom':down}[ann['va']]), offset, textcoords='offset pixels', rotation=rotation, **ann)
    arr = dict(arrowstyle='<->', color=color)
    arr.update({k[4:]:kargs[k] for k in kargs if k.startswith('arr_')})
...
def sublegend(*ax, **kargs):
    ...
    if not hasattr(margin, '__getitem__'):
...
def _overlap(ax, obj, objs, debug=False):
    ...
    tr = ax.transData
    r = obj.get_window_extent(renderer)
    for o in objs:
        if type(o) is mpl.lines.Line2D:
            ...
            if isin:
                if debug:
                    ax.plot(xyd[:, 0][ins], xyd[:, 1][ins], 'rx')
                return o
        else:
...
def put_Xlabels(ax, pos, labels, colors='rgb', debug=False, save=False, bbox=False, **kargs):
    ...
    for i, (x, lab) in enumerate(P):
        ...
        ax.axvline(x, color=col, linestyle=(kargs.get('linestyle', '-')), alpha=(kargs.get('line_alpha', 0.5)))
        ax.draw(renderer)
        r = labs[(-1)].get_window_extent(renderer)
        ...
        while ov:
            it += 1
            if debug:
                print('Label "{}" overlap with {}'.format(labs[(-1)].get_text(), repr(ov)))
            ...
            new_xy = ax.transData.inverted().transform(new_xy_px)
            ...
            if save:
                ...
                if ov:
                    r2 = ov.get_window_extent(renderer)
    ...
    if bbox:
        ...
        for o in objs:
            if type(o) is not mpl.lines.Line2D:
                ...
                ax.add_patch(mpl.patches.Rectangle((r.xmin, r.ymin), (r.xmax - r.xmin), (r.ymax - r.ymin), ec='r', fill=False))

        for l in labs:
            r = l.get_window_extent(renderer).transformed(ax.transData.inverted())
            ax.add_patch(mpl.patches.Rectangle((r.xmin, r.ymin), (r.xmax - r.xmin), (r.ymax - r.ymin), ec='b', fill=False))
...
def stdplot(x, y, Nsig=2, ax=None, color='b', axis=1, **kargs):
    ...
    **kargs: arguments passed to plot
    ...
    for i in range(1, Nsig + 1):
        ax.fill_between(x, (mean - i * dy), (mean + i * dy), color=color, alpha=0.2)
...
def pixel2img(xy, ax=None):
    ...
    if hasattr(ax, 'isPixel'):
        pixel_shape = ax.isPixel or ax.images[0].get_array().shape
        extent = ax.images[0].get_extent()
        eW = extent[1] - extent[0]
        eH = extent[3] - extent[2]
        return (
         xy[0] * eW / pixel_shape[1], xy[1] * eH / pixel_shape[0])
    else:
        return xy