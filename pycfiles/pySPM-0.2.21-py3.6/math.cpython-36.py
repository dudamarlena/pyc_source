...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\pySPM\utils\math.py
...
def CDF(x, mu, sig, amp=1, lg=0, **kargs):
    ...
    return amp * lg * (0.5 + np.arctan2(x - mu, g) / np.pi) + (1 - lg) * amp * 0.5 * (1 + erf((x - mu) / (sig * np.sqrt(2))))
...
def fitCDF1line(A):
    ...
    for x in range(A.shape[1]):
        popt, pcov = opt.curve_fit(CDF, (np.arange(A.shape[0])),
          (A[:, x]),
          (
         A.shape[0] / 2, 1, np.max(A[:, x])),
          bounds=(
         0, (A.shape[1], np.inf, np.inf)))
        line[x] = popt[0]
...
def asymm_ellipse(left, right, upper, lower, phi):
    phi = np.divmod(phi + 2 * np.pi, 2 * np.pi)[1]
