...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\pySPM\utils\units.py
...
_SI_units = [
 'kg', 'm', 's', 'A', 'K', 'cd', 'mol']
_units = {'V':{'kg':1, 
  'm':2,  's':-3,  'A':-1,  'K':0,  'cd':0,  'mol':0}, 
 'C':{'kg':0, 
  'm':0,  's':1,  'A':1,  'K':0,  'cd':0,  'mol':0}, 
 'N':{'kg':1, 
  'm':1,  's':-2,  'A':0,  'K':0,  'cd':0,  'mol':0}, 
 'J':{'kg':1, 
  'm':2,  's':-2,  'A':0,  'K':0,  'cd':0,  'mol':0}, 
 'W':{'kg':1, 
  'm':2,  's':-3,  'A':0,  'K':0,  'cd':0,  'mol':0}, 
 'Pa':{'kg':1, 
  'm':-1,  's':-2,  'A':0,  'K':0,  'cd':0,  'mol':0}, 
 'Ω':{'kg':1, 
  'm':2,  's':-3,  'A':-2,  'K':0,  'cd':0,  'mol':0}}
_units_scale_conversion = {'eV':(
  const.qe, 'V'), 
 'bar':(100000.0, 'Pa'), 
 'atm':(101325, 'Pa')}
_unit_shift_conversion = {'°C': (273.15, 'K')}
_unit_scale = {'y':-24, 
 'z':-21,  'a':-18,  'f':-15,  'p':-12,  'n':-9,  'u':-6,  'm':-3,  'c':-2,  'd':-1,  'h':2, 
 'k':3,  'M':6,  'G':9,  'T':12,  'P':15,  'E':18,  'Z':21,  'Y':24,  '':0}
_unit_revert_scale = {}
import re
_SI_units = [
 'kg', 'm', 's', 'A', 'K', 'cd', 'mol']
...
def op_parse(s):
    ...
    for x in s:
        if type(x) is list:
            ...
            r += [x for x in re.split('(\\*|/|\\^)', x) if len(x) > 0]
            ...
            if s in _SI_units:
                ...
                if s in _units:
                    ...
                    if s[0] in _unit_scale:
                        ...
                        if s == 'g':
                            result = unit('kg', 0.001)
                        else:
                            if s in _units_scale_conversion:
