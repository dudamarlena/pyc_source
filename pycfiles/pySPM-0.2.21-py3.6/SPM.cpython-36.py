...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\pySPM\SPM.py
# Compiled at: 2019-05-21 08:54:40
# Size of source mod 2**32: 68220 bytes
"""
Library to handle SPM data.
This is the core module of all images retrieved by SPM and ToF-SIMS.
"""
import numpy as np, matplotlib.pyplot as plt, scipy, scipy.ndimage, scipy.optimize, skimage, skimage.exposure, skimage.filters, scipy.interpolate
from skimage import transform as tf
import copy
from .utils import CDF, funit
import sys, matplotlib as mpl, warnings
...
class SPM_image:
    ...
    def __init__(self, BIN, channel='Topography', corr=None, real=None, zscale='?', _type='Unknown'):
        """
    ...
    def add_scale(self, length, ax=None, height=20, margin=5, color='w', loc=4, text=True, pixels=None, fontsize=20, edge_color='k', edge_width=3):
        ...
        """
        import matplotlib.patches, matplotlib.patheffects as PathEffects
        ...
        if hasattr(ax, 'flipped'):
            ...
            if type(loc) is int:
                ...
                if not type(loc) in [tuple, list]:
                    raise AssertionError
                elif not len(loc) == 2:
                    raise AssertionError
                ref = ax.transData.transform(loc) + ax.transAxes.transform((-fL / 2, -fH / 2)) - ax.transAxes.transform((0,
                                                                                                                         0))
        inv = ax.transData.inverted()
        ref = inv.transform(ref)
        WH = inv.transform(ax.transAxes.transform((fL, fH))) - inv.transform(ax.transAxes.transform((0,
                                                                                                     0)))
        ...
        if text:
            ...
            if loc in (3, 4):
                label_ref = [
                 ref[0] + WH[0] / 2, ref[1]]
                ann = ax.annotate(('{value:.01f} {unit}'.format)(**r), label_ref,
                  color=color, fontsize=fontsize,
                  va='top',
                  ha='center')
            else:
                label_ref = [
                 ref[0] + WH[0] / 2, ref[1] + WH[1]]
                ann = ax.annotate(('{value:.01f} {unit}'.format)(**r), label_ref,
                  color=color, fontsize=fontsize,
                  va='bottom',
                  ha='center')
    ...
    def get_row_profile(self, x1, y1, x2, y2, width=1, col='C1', ax=None, alpha=0, **kargs):
        ...
        **kargs:
        ...
        if ax is not None:
            ...
            if kargs.get('axPixels', False):
                ...
                if kargs.get('label', False):
                    ax.annotate((kargs.get('label')), (0.5 * (x1 + x2), 0.5 * (y1 + y2)), color=col)
                if alpha > 0:
                ...
                if kargs.get('label', False):
                    ax.annotate((kargs.get('label')), (0.5 * (x1 + x2) * pxs, 0.5 * (2 * h - y1 - y2) * pys), color=col)
                if alpha > 0:
                    import matplotlib.patches
        ...
        for w in np.arange(width):
            xl = np.linspace(x1 - (width - 1) / 2.0 + w, x2 - (width - 1) / 2.0 + w, len(Y))
    ...
    def show(self, ax=None, sig=None, cmap=None, title=None, adaptive=False, dmin=0, dmax=0, pixels=False, flip=False, wrap=None, mul=1, symmetric=False, **kargs):
        """
        ...
        if cmap == None:
            cmap = 'gray'
            if unit == 'm':
                if self.channel == 'Topography':
                    cmap = 'hot'
            mi, ma = np.nanmin(self.pixels), np.nanmax(self.pixels)
            if adaptive:
                img = np.asarray((65536 * (self.pixels - mi) / (ma - mi)), dtype=(np.uint16))
                mi, ma = (0, 1)
            ...
            if pixels:
                ...
                if not pixels:
                    if isunit != 6:
                        ...
                        if u == 'u':
                            u = '$\\mu$'
                        ax.set_xlabel('x [{0}{1}]'.format(u, unit))
                        ax.set_ylabel('y [{0}{1}]'.format(u, unit))
                    else:
    ...
    def circular_profile(self, x0, y0, Ra=1, Rn=0, width=1, N=20, A=0, B=360, cmap='jet', axImg=None, axPolar=None, axProfile=None, plotProfileEvery=1, xtransf=lambda x: x * 1000000000.0, ytransf=lambda x: x * 1000000000.0, ToFcorr=False, fit=lambda x, *p: p[3] + p[2] * CDF(x, *p[:2]), p0=None, errors=False, bounds=(-np.inf, np.inf), fakefit=False, **kargs):
        """
 ...
 y0 + Rn * np.sin(a),
 x0 + Ra * np.cos(a),
 y0 - Ra * np.sin(a), ax=axImg, 
             width=width, color=scalarMap.to_rgba(i), **kargs)
            if width == 0:
                profile = p
            else:
            ...
            if p0 is None:
                ...
                if AC < AE:
                    p0 = [
                     l[(len(l) // 2)], 5 * (l[1] - l[0]), np.max(profile) - np.min(profile), np.min(profile)]
                else:
                    p0 = [
                     l[(len(l) // 2)], 5 * (l[1] - l[0]), -np.max(profile) + np.min(profile), np.max(profile)]
            else:
        ...
        if axPolar:
            ...
            if errors:
                axPolar.fill_between(angles, (ytransf(res[:, 1] - cov[:, 1])), (ytransf(res[:, 1] + cov[:, 1])), color=(kargs.get('sig_color', 'C0')), alpha=(kargs.get('fillalpha', 0.5)))
                axPolar.fill_between(angles, (fact * ytransf(res[:, 1] - cov[:, 1])), (ytransf(res[:, 1] + cov[:, 1])), color=(kargs.get('fwhm_color', 'C1')), alpha=(kargs.get('fillalpha', 0.5)))
        return (
         angles, res, cov)
    ...
    def get_profile(self, x1, y1, x2, y2, width=0, ax=None, pixels=True, color='w', axPixels=None, **kargs):
        ...
        if ax is not None:
            ...
            if not pixels:
                ...
                xvalues, p = axPixels or get_profile(np.flipud(self.pixels), x1, y1, x2, y2, ax=ax, width=width, color=color, transx=lambda x: x * (self.size['real']['x'] / 10 ** fact) / self.pixels.shape[1], 
                 transy=lambda x: (self.pixels.shape[0] - x) * (self.size['real']['y'] / 10 ** fact) / self.pixels.shape[0], **kargs)
            else:
            ...
            if axPixels:
                ...
                values, p = get_profile(np.flipud(self.pixels), x1, y1, x2, y2, ax=ax, width=width, color=color, transx=lambda x: x * (self.size['real']['x'] / 10 ** fact) / self.pixels.shape[1], 
                 transy=lambda x: (self.pixels.shape[0] - x) * (self.size['real']['y'] / 10 ** fact) / self.pixels.shape[0], **kargs)
    ...
    def plot_profile(self, x1, y1, x2, y2, width=0, ax=None, pixels=True, img=None, imgColor='w', ztransf=lambda x: x, zunit=None, **kargs):
        ...
        if kargs.get('stdplot', False):
            for ns in range(1, kargs.get('sig', 2) + 1):
                ax.fill_between(xvalues, (profile - ns * s), (profile + ns * s), color=col, alpha=0.2, label=([lab + ' ($\\sigma,\\ldots {}\\sigma$)'.format(kargs.get('sig', 2)), None][(ns > 1)]))
                ...
                if kargs['num']:
                    ...
                    for i in range(len(X)):
                        ...
                        ax.annotate((str(i)), (X[i], Y[i]), (
                         [
                          5, -5][l], 0),
                          textcoords='offset pixels', va='center', ha=(['left', 'right'][l]), color=text_color)
    ...
    def filter_scars_removal(self, thresh=0.5, inline=True):
        """
    ...
    def cut(self, c, inline=False, pixels=True, **kargs):
        ...
        if not pixels:
            ...
            new = inline or copy.deepcopy(self)
...
def cut(img, c, **kargs):
    ...
    if c[2] < c[0]:
        c = [
         c[2], c[1], c[0], c[3]]
    if c[2] - c[0] == img.shape[1]:
        if c[3] - c[1] == img.shape[0]:
...
def tukeywin(window_length, alpha=0.5):
    """The Tukey window, also known as the tapered cosine window, can be regarded as a cosine lobe of width \x07lpha * N / 2
    that is convolved with a rectangle window of width (1 - \x07lpha / 2). At \x07lpha = 1 it becomes rectangular, and
    at \x07lpha = 0 it becomes a Hann window.
...
def beam_profile(target, source, mu=1e-06, tukey=0, meanCorr=False, source_tukey=None, real=np.abs, **kargs):
    """
...
def beam_profile1d(target, source, mu=1e-06, real=np.abs):
    source = source
...
def get_profile(I, x1, y1, x2, y2, width=0, ax=None, color='w', alpha=0, N=None, transx=lambda x: x, transy=lambda x: x, interp_order=1, **kargs):
    """
    ...
    if len(P) == 1:
        ...
        return (
         np.linspace(0, d, N), np.vstack(P).T)
    ...
    if A == 0:
        if transform != 0:
            if not theta is not None:
                raise AssertionError
            elif not phi is not None:
                raise AssertionError
            tf_out *= ((np.cos(theta) + complex(0.0, 1.0) * (np.cos(phi) * np.sin(-theta) * kx + np.sin(phi) * np.sin(-theta) * ky)) / k) ** transform
        if derivative == 1:
            tf_out *= k
    return tf_out * tf_in


def mfm_inv_calc_flat(img, z, tf_in, thickness=None, delta_w=None, amplitude=0, derivative=0, transform=0, mu=1e-08):
    """
