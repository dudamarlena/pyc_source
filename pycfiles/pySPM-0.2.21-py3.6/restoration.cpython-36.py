...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\pySPM\utils\restoration.py
...
def _rl(x, image, psf, type='default', extend=True, damping=0, ndamp=10):
    ...
    """
    I = strictly_positify(convolve(x, psf, type=type, extend=extend))
    if damping != 0:
        ratio = _rl_damped(I, image, damping=damping, ndamp=ndamp)
    else:
        ratio = image / I
    return x * convolve(ratio, (psf[::-1, ::-1]), type=type, extend=extend)
...
def _rl_accelerate(x, x1, x2, g1=None, g2=None, order=1):
    ...
    if g2 is None:
        ...
        alpha = np.sum(g1 * g2) / strictly_positify(np.sum(g2 ** 2))
        alpha = clip01(alpha)
    if alpha == 0:
        return x
    else:
...
def richardson_lucy(image, psf, iterations, damping=0, ndamp=10, core='default', acceleration=2, init='mean', extend=True, clip=False, **kargs):
    """
    ...
    """
    if not core in ('default', 'fft', 'accurate'):
