...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\pySPM\PCA.py
...
class PCA:
    ...
    def hinton(self, max_weight=None, ax=None, matrix=None, xlabel=None, ylabel=None):
        ...
        if matrix is None:
            ...
            for (x, y), w in np.ndenumerate(matrix):
                ...
                rect = plt.Rectangle([y - size / 2, x - size / 2], size, size, facecolor=color,
                  edgecolor=color)
                ax.add_patch(rect)
    ...
    def standardized(self, meanCentering=True):
        self.standX = pd.DataFrame(scale((self.data), with_mean=meanCentering),
...
class ITA_PCA(PCA):
    ...
    def get_pca_col(self, num=None, pn=False):
        ...
        elif not num <= self.data.shape[1]:
            raise AssertionError
        PCA_col = collection.Collection(cls=(self.col),
          name=(self.col.name + '[PCA]'))
        for i in range(num):
            PC = self.get_pca(i)
            if pn:
