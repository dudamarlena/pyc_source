...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/slacksound/spotifyclient.py
# Compiled at: 2017-10-27 07:55:59
"""
...
class SpotifyClient(object):
    ...
    def __init__(self, client_id, client_secret, username, password, callback, scope):
        ...
        self._spotify = Spotify(client_id=client_id, client_secret=client_secret, username=username, password=password, callback=callback, scope=scope)
    ...
    def playlists(self):
        ...
        if not self._playlists:
            ...
            self._playlists = [ Playlist(self._username, self._spotify, playlist) for playlist in raw_playlists.get('items')
                              ]
    ...
    def get_playlist_by_name(self, playlist_name):
        ...
        """
        playlist = next((plist for plist in self.playlists if plist.name == playlist_name), None)
        return playlist
...
class Playlist(object):
    ...
    def tracks(self):
        ...
        """
        songs_playlist = self._spotify.user_playlist_tracks(user=self._username, playlist_id=self.playlist_id)
        return [ Track(track.get('track')) for track in songs_playlist.get('items')
               ]
    ...
    def delete_all_tracks(self):
        ...
        return self._spotify.user_playlist_remove_all_occurrences_of_tracks(self._username, self.playlist_id, track_ids)
    ...
    def add_track(self, track_id):
        ...
        self._spotify.user_playlist_add_tracks(user=self._username, playlist_id=self.uri, tracks=[
         track_id])
        return True
