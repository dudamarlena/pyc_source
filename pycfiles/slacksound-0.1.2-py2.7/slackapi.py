...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/slacksound/slackapi.py
# Compiled at: 2017-10-27 05:05:52
"""
...
class Group(object):
    ...
    def created(self):
        ...
        date = datetime.fromtimestamp(float(unix_timestamp), tzlocal.get_localzone())
        return date
    ...
    def history(self):
        ...
        """
        ch_history = self._slack_instance.client.api_call(method='groups.history', channel=self.group_id)
        return [ Message(history_message) for history_message in ch_history.get('messages')
               ]
...
class Channel(object):
    ...
    def history(self):
        ...
        """
        ch_history = self.__slack_instance.client.api_call(method='channels.history', channel=self.channel_id)
        return [ Message(history_message) for history_message in ch_history.get('messages')
               ]
...
class Message(object):
    ...
    def attachments(self):
        ...
        """
        return [ Attachment(m_attachment) for m_attachment in self._message_details.get('attachments', [])
               ]
    ...
    def reaction(self):
        ...
        """
        return [ Reaction(reaction) for reaction in self._message_details.get('reactions', [])
               ]
