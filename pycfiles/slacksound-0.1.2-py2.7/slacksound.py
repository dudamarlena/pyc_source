...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/slacksound/slacksound.py
# Compiled at: 2017-10-27 10:12:57
"""
...
"""
import logging, os, json, argparse, time
...
def get_arguments():
    ...
    parser.add_argument('--log-level', '-L', help='Provide the log level. Defaults to INFO.', dest='log_level', action='store', default='INFO', choices=[
     ...
     'ERROR',
     'CRITICAL'])
    parser.add_argument('--credentials', dest='credentials', action='store', default=False, required=False)
...
def connect_spotify(credentials):
    spotify = SpotifyClient(client_id=credentials.get('spotify', 'client_id'), client_secret=credentials.get('spotify', 'client_secret'), username=credentials.get('spotify', 'username'), password=credentials.get('spotify', 'password'), callback=credentials.get('spotify', 'callback_url'), scope=credentials.get('spotify', 'scope'))
    return spotify
...
def get_config_details(credentials):
    config = SlackSound(playlist=credentials.get('spotify', 'playlist'), reaction=credentials.get('slack', 'reaction'), channel=credentials.get('slack', 'channel'), count=int(credentials.get('slack', 'count')))
    return config
...
def main():
    ...
    slack.post_message(('SlackSound started! Add your :{}: reaction to the link. The minimum votes are: {}').format(config_details.reaction, config_details.count), config_details.channel)
    blacklisted = []
    while True:
        time.sleep(1)
                ...
                for reaction in message.reaction:
                    for attachment in message.attachments:
                        ...
                        if not tracks and sanitized_title not in blacklisted:
                            ...
                            slack.post_message("Couldn't find the song", config_details.channel)
                        ...
                        if track:
                            if reaction.count >= config_details.count and reaction.name == config_details.reaction:
                                ...
                                if track.uri not in track_uris:
                                    ...
                                    slack.post_message(('Song {} added').format(sanitized_title), config_details.channel)
