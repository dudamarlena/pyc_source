...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ielu/color_utils.py
...
def traits2mayavi_color(traits_color):
    ...
    rgb_col = (
     rgba_col[0], rgba_col[1], rgba_col[2])
    return rgb_col
...
def set_binary_lut(mayavi_obj, color1, color2, use_vector_lut=False):
    ...
    if use_vector_lut:
        ...
        lut_mgr = mayavi_obj.module_manager.scalar_lut_manager
    lut_mgr.lut.table = map_to_table(LinearSegmentedColormap.from_list('ign', [color1, color2]))
    lut_mgr.data_range = [
     0, 1]
