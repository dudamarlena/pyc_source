...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ielu/geometry.py
...
def get_vox2rasxfm(volume, stem='vox2ras'):
    import subprocess
    ps = subprocess.Popen(['mri_info', '--%s' % stem, volume], stdout=subprocess.PIPE)
    ps.wait()
...
def apply_affine(locs, affine):
    ...
    for loc in locs:
        ...
        new_locs.append([ np.around(new_loc[i], decimals=4) for i in xrange(3)
                        ])
