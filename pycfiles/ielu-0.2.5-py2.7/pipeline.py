...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ielu/pipeline.py
# Compiled at: 2016-04-05 15:45:09
from __future__ import division
import os, numpy as np, nibabel as nib, geometry as geo, grid as gl
from utils import SortingLabelingError
from electrode import Electrode
from scipy.spatial.distance import cdist

def create_brainmask_in_ctspace(ct, subjects_dir=None, subject=None, overwrite=False):
    """
...
def identify_electrodes_in_ctspace(ct, mask=None, threshold=2500, use_erosion=True, isotropization_type=None, iso_vector_override=None):
    """
    ...
    def get_centerofmass(isotropize=None):
        ...
        if isotropization_type == 'By voxel':
            ...
            new_shape = ctd.shape
            print ('INITIAL SHAPE: {0}, NEW SHAPE {1}').format(initial_shape, new_shape)
        elif isotropization_type == 'By header':
            initial_shape = ctd.shape
            vox2ras = cti.get_affine()
            rd, ad, sd = geo.get_std_orientation(vox2ras)
            vox2ras_rstd = np.array(map(lambda ix: np.squeeze(vox2ras[ix, :3]), (
             rd, ad, sd)))
        ...
        class Component:
            ...
            def center_of_mass(self):
                ...
                return (
                 round(Rx / M), round(Ry / M), round(Rz / M))
    ...
    if isotropization_type != 'Isotropization off':
        return [ Electrode(iso_coords=i) for i in get_centerofmass(isotropize=isotropization_type)
               ]
    else:
        return [ Electrode(ct_coords=c) for c in get_centerofmass(isotropize=isotropization_type)
               ]
        return


def linearly_transform_electrodes_to_isotropic_coordinate_space(electrodes, ct, isotropization_direction_on=None, isotropization_direction_off=None, isotropization_strategy=None, iso_vector_override=None):
    """
    ...
    for elec in electrodes:
        if isotropization_strategy == 'Isotropization off':
            ...
            aff = cti.get_affine()
            aff_rstd = np.array(map(lambda ix: aff[ix, :3], geo.get_std_orientation(aff)))
...
def identify_extracranial_electrodes_in_freesurfer_space(electrodes, dilation_iterations=5, subjects_dir=None, subject=None):
    """
    ...
    for e in electrodes:
        ...
        if not np.all([ 0 <= p <= 255 for p in (nx, ny, nz) ]):
            removals.append(e)
            print 'Warning: Removed some extremely far outlying noise, likely sign of registration errors'
            continue
        if maskd[(nx, ny, nz)] == 0:
            removals.append(e)
...
def classify_electrodes(electrodes, known_geometry, delta=0.35, rho=35, rho_strict=20, rho_loose=50, color_scheme=None, epsilon=10, mindist=0, maxdist=36, crit_pct=0.75):
    """
    ...
    for dims in known_geometry:
        new_elecs = geo.rm_pts(np.reshape(used_points, (-1, 3)), np.array(electrode_arr))
        angles, _, neighbs = gl.find_init_angles(new_elecs, mindist=mindist, maxdist=maxdist)
        ba = np.squeeze(sorted(zip(*np.where(np.abs(90 - angles) < epsilon)), key=lambda v: np.abs(90 - angles[v])))
        if ba.shape == ():
            ba = [
             ba]
        else:
            ...
            for j, k in enumerate(ba):
                p0, p1, p2 = neighbs[k]
                pog = gl.Grid(p0, p1, p2, new_elecs, delta=delta, rho=rho, rho_strict=rho_strict, rho_loose=rho_loose, name=names.next(), critical_percentage=crit_pct)
                ...
                try:
                    ...
                    if j == len(ba) - 1:
                        print 'No suitable strip found. Returning an empty strip in its place'
                ...
                for p in sp:
                    ...
                    if tuple(p.tolist()) in electrode_arr:
                        ix, = np.where(np.logical_and(np.logical_and(np.array(electrode_arr)[:, 0] == p[0], np.array(electrode_arr)[:, 1] == p[1]), np.array(electrode_arr)[:, 2] == p[2]))
                        try:
                            elec = electrodes[ix]
                            ...
                            print ix
                            print p
                            raise SortingLabelingError('multiple electrodes at same point')

                    else:
                        elec = Electrode(iso_coords=tuple(p), is_interpolation=True)
                        found_grids[pog.name].append(elec)
                    ...
                    try:
                        elec.geom_coords = list(final_connectivity[elec.asiso()])
                    except KeyError:
                        pass
    ...
    return (
     grid_colors, grid_geom, found_grids, colors)


def remove_large_negative_values_from_ct(ct, subjects_dir=None, subject=None, threshold=-200):
    """
    ...
    if np.min(ctd) > threshold:
        ...
        ct_unaltered = os.path.join(subjects_dir, subject, 'mri', 'ct_unaltered.nii.gz')
        if os.path.exists(ct_unaltered):
            return
        nib.save(cti, ct_unaltered)
        ctd[ctd < threshold] = threshold
        ct_new = nib.Nifti1Image(ctd, header=cti.get_header(), affine=cti.get_affine())
        nib.save(ct_new, ct)
        return


def register_ct_to_mr_using_mutual_information(ct, subjects_dir=None, subject=None, overwrite=False):
    """
    ...
    if os.path.exists(lta) and not overwrite:
        ...
        mri_robustreg_cmd = [
         'mri_robust_register', '--mov', ct, '--dst', rawavg,
...
def register_ct_using_zoom_correction(ct, subjects_dir=None, subject=None, cm_dist=5, overwrite=False, zf_override=None):
    """
    ...
    if os.path.exists(lta) and not overwrite:
        ...
        if zf_override == None:
            ...
            center_img = nib.Nifti1Image(center_slice, affine=cti.get_affine(), header=hdr)
            upper_img = nib.Nifti1Image(upper_slice, affine=cti.get_affine(), header=hdr)
...
def create_dural_surface(subjects_dir=None, subject=None):
    ...
    print scripts_dir
    if os.path.exists(os.path.join(subjects_dir, subject, 'surf', 'lh.dural')) and os.path.exists(os.path.join(subjects_dir, subject, 'surf', 'rh.dural')):
        ...
        for hemi in ('lh', 'rh'):
            make_dural_surface_cmd = [
             os.path.join(scripts_dir, 'make_dural_surface.csh'), '-i', '%s.pial' % hemi]
...
def get_rawavg_to_orig_xfm(subjects_dir=None, subject=None, skip_rawavg_to_orig=False):
    """
    ...
    rawavg = os.path.join(subjects_dir, subject, 'mri', 'rawavg.mgz')
    orig = os.path.join(subjects_dir, subject, 'mri', 'orig.mgz')
    lta = os.path.join(subjects_dir, subject, 'mri', 'transforms', 'raw2orig.lta')
    if not os.path.exists(lta) and not skip_rawavg_to_orig:
        import subprocess
        mri_robustreg_cmd = [
         'mri_robust_register', '--mov', rawavg, '--dst',
...
def translate_electrodes_to_surface_space(electrodes, ct2mr, subjects_dir=None, subject=None, affine=None):
    """
    ...
    orig = os.path.join(subjects_dir, subject, 'mri', 'orig.mgz')
    if True:
        nas2ras = get_rawavg_to_orig_xfm(subject=subject, subjects_dir=subjects_dir)
...
def snap_electrodes_to_surface(electrodes, subjects_dir=None, subject=None, max_steps=40000, giveup_steps=10000, init_temp=0.001, temperature_exponent=1, deformation_constant=1.0):
    """
    ...
    for i in xrange(n):
        ...
        h5, = np.where(np.logical_and(neighbor_vec < np.sort(neighbor_vec)[5], neighbor_vec != 0))
        neighbors.append(h5)
    ...
    lh_dura, _ = nib.freesurfer.read_geometry(os.path.join(subjects_dir, subject, 'surf', 'lh.dural'))
    rh_dura, _ = nib.freesurfer.read_geometry(os.path.join(subjects_dir, subject, 'surf', 'rh.dural'))
    ...
    lh_pia, _ = nib.freesurfer.read_geometry(os.path.join(subjects_dir, subject, 'surf', 'lh.pial'))
    rh_pia, _ = nib.freesurfer.read_geometry(os.path.join(subjects_dir, subject, 'surf', 'rh.pial'))
...
def fit_grid_to_line(electrodes, mindist=0, maxdist=36, epsilon=30, delta=0.5, rho=35, rho_strict=20, rho_loose=50):
    """
    ...
    next_isol = np.argmin(np.ma.masked_array(eadist[isol], mask=eadist[isol] == 0))
    c1 = np.array(electrodes[isol].asiso())
    c2 = np.array(electrodes[next_isol].asiso())
    pog = gl.Grid(c1, c2, None, np.array(electrode_arr), delta=delta, rho=rho, rho_strict=rho_strict, rho_loose=rho_loose, is_line=True)
    pog.extend_grid_arbitrarily()
...
def fit_grid_by_fixed_points(electrodes, geom, delta=0.35, rho=35, rho_strict=20, rho_loose=50, epsilon=10, mindist=0, maxdist=36):
    """
    Sort the given electrodes (generally in the space of the CT scan) into
    grids and strips matching the specified geometry.
    ...
    angles, _, neighbs = gl.find_init_angles(elecs, mindist=mindist, maxdist=maxdist)
    ba = np.squeeze(sorted(zip(*np.where(np.abs(90 - angles) < epsilon)), key=lambda v: np.abs(90 - angles[v])))
    if ba.shape == ():
        ba = [
         ba]
        ...
        for j, k in enumerate(ba):
            p0, p1, p2 = neighbs[k]
            pog = gl.Grid(p0, p1, p2, elecs, delta=delta, rho=rho, rho_strict=rho_strict, rho_loose=rho_loose)
            pog.critical_percentage = 1.0
            try:
            ...
            for p in sp:
                if tuple(p.tolist()) in electrode_arr:
                    ...
                    raise SortingLabelingError('Electrodes in not same as electrodes out')
                for corner in corners:
                    if np.all(corner == np.array(elec.asiso())):
                        elec.corner = [
                         'corner 1']
...
def fit_grid_to_plane(electrodes, c1, c2, c3, geom, reverse_grid='check'):
    ...
    if reverse_grid == True:
        ...
        if reverse_grid == False:
            transpix = [
             0, 1]
        else:
            if reverse_grid == 'check':
                max_x = 0
                max_y = 0
                for elec in electrodes:
            ...
            for elec in electrodes:
                if len(elec.geom_coords) == 0:
                    continue
                e_init_choice = reverse_plane[tuple(np.array(elec.geom_coords)[transpix])]
...
def identify_roi_from_atlas(pos, approx=4, atlas=None, subjects_dir=None, subject=None):
    """
    ...
    if atlas is None or atlas in ('', 'aparc'):
        return identify_roi_from_aparc(pos, approx=approx, subjects_dir=subjects_dir, subject=subject)
    else:
        from scipy.spatial.distance import cdist
        lh_pia, _ = nib.freesurfer.read_geometry(os.path.join(subjects_dir, subject, 'surf', 'lh.pial'))
        rh_pia, _ = nib.freesurfer.read_geometry(os.path.join(subjects_dir, subject, 'surf', 'rh.pial'))
        ...
        radius_label, = mne.grow_labels(subject, closest_vert, approx, hemi_code, subjects_dir=subjects_dir, surface='pial')
        parcels = mne.read_labels_from_annot(subject, parc=atlas, hemi=hemi_str, subjects_dir=subjects_dir, surf_name='pial')
        regions = []
        for parcel in parcels:
            if len(np.intersect1d(parcel.vertices, radius_label.vertices)) > 0:
                regions.append(str(parcel.name))

        subcortical_regions = identify_roi_from_aparc(pos, approx=approx, subjects_dir=subjects_dir, subject=subject, subcortical_only=True)
        regions.extend(subcortical_regions)
        return regions


def identify_roi_from_aparc(pos, approx=4, subjects_dir=None, subject=None, subcortical_only=False):
    """
    ...
    def find_neighboring_regions(pos, mri_dat, region, approx, excludes):
        ...
        for p in xrange(neighb.shape[0]):
            cx, cy, cz = pos[0] + neighb[(p, 0)], pos[1] + neighb[(p, 1)], pos[2] + neighb[(p, 2)]
    ...
    excludes = [
     'white', 'WM', 'Unknown', 'White', 'unknown']
    if subcortical_only:
        excludes.append('ctx')
