...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ielu/plotting_utils.py
# Compiled at: 2016-03-10 13:38:23
...
def coronal_slice(elecs, start=None, end=None, outfile=None, subjects_dir=None, subject=None, reorient2std=True, dpi=150, size=(200, 200), title=None):
    """
    ...
    subjdir_subj = get_subjects_dir(subjects_dir=subjects_dir, subject=subject)
    ...
    r_size = [
     x_size, y_size, z_size][rd]
    a_size = [x_size, y_size, z_size][ad]
    s_size = [x_size, y_size, z_size][sd]
    electrodes = np.squeeze([ apply_affine([e.asras()], ras2vox) for e in elecs
                            ])
    ...
    if start is not None and end is not None:
        ...
        try:
            for rvox in rnew:
                slice[:, rvox] = vol[rvox, alower[rvox], :] * (1 - afrac[rvox]) + vol[rvox, alower[rvox] + 1, :] * afrac[rvox]
    ...
    pix = np.dot(ras2pix, np.transpose([ np.append(e.asras(), 1) for e in elecs ]))
    import pylab as pl
    fig = pl.figure()
    pl.imshow(slice, cmap='gray')
    pl.scatter(pix[0, :], pix[1, :], s=10, c='red', edgecolor='yellow', linewidths=0.4)
...
def sequence_3d_images(figure):
    from mayavi import mlab
    views = [
     lambda : mlab.view(azimuth=0, elevation=90, figure=figure),
