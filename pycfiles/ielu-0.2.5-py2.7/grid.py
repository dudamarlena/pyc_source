...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ielu/grid.py
# Compiled at: 2016-04-01 16:04:09
from __future__ import division
import numpy as np
from numpy.linalg import norm
from geometry import angle, is_parallel, is_perpend, within_distance, rm_pts, find_nearest_pt, find_neighbors, binarize
...
class Grid():
    ...
    def __init__(self, p0, p1, p2, all_elecs, delta=0.35, rho=35, rho_strict=20, rho_loose=50, max_cost=0.4, name='', critical_percentage=0.75, is_line=False):
        self.name = name
        ...
        self.distances = [
         norm(p0 - p1)]
        if is_line:
            self.connectivity = {GridPoint(p0): (0, 0), GridPoint(p1): (0, 1)}
            self.reverse_connectivity = {(0, 0): p0, (0, 1): p1}
            self.is_line = True
        else:
            self.connectivity = {GridPoint(p0): (0, 0), GridPoint(p1): (0, 1), GridPoint(p2): (1, 0)}
            self.reverse_connectivity = {(0, 0): p0, (0, 1): p1, 
               (1, 0): p2}
            self.is_line = False
        self.marked = {}
    ...
    def nearest(self, p0, allow_self=True):
        try:
            p, _ = find_nearest_pt(p0, self.remaining_points(), allow_self=allow_self)
            return p
    ...
    def get_local_connectivity_2d(self, coord_2d):
        ...
        if nr_neighbors == 4:
            ...
            orientation = 'north'
        return (
         connectivity_profile, orientation)
    ...
    def extract_strip(self, N, M):
        ...
        if not fit_ok:
            raise SortingLabelingError('No strip had a sufficiently good fit,  best fit was %i' % int(best_fit))
        ...
        print 'Decided that the %i by %i strip at %s is the best fit' % (
         M, N, best_loc)
        return (
         points, corners, final_connectivity)
    ...
    def disambiguate_best_fit_strips(self, potential_strip_locs, M, N):
        ...
        if len(potential_strip_locs) == 1:
            print 'Only one strip location possible, possibly the result of user intervention, returning it'
        else:
        ...
        for r, c, orient in potential_strip_locs:
            ...
            while len(interpolated_points) < M * N - len(cur_points):
                ...
                for x, y in zip(*np.where(strip_graph == 0)):
                    ...
                    if (
                     i, j) in interpolated_gridpoints:
                        continue
                    connectivity, orientation = self.get_local_connectivity_2d((
                     i, j))
                    ...
                    if connectivity == 'FULL':
                        ...
                        if pNN is not None and (orientation == 'north' or connectivity == 'MOTIF' and orientation == 'east'):
                            pInterp = 2 * pN - pNN
                        elif pWW is not None and (orientation == 'west' or connectivity == 'MOTIF' and orientation == 'north'):
                            pInterp = 2 * pW - pWW
                        elif pSS is not None and (orientation == 'south' or connectivity == 'MOTIF' and orientation == 'west'):
                            pInterp = 2 * pS - pSS
                        elif pEE is not None and (orientation == 'east' or connectivity == 'MOTIF' and orientation == 'south'):
                            pInterp = 2 * pE - pEE
                        else:
                            continue
                    ...
                    if pInterp is None:
                        ...
                        if (
                         i, j) in interpolated_gridpoints:
                            raise ValueError('Internal error: should never beadding a point that already exists')
                    else:
...
def find_init_angles(all_elecs, mindist=10, maxdist=25):
    ...
    for k in xrange(n):
        ...
        if mindist < norm(p1 - p0) < maxdist and mindist < norm(p2 - p0) < maxdist:
