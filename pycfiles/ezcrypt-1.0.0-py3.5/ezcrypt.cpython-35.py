...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ezcrypt/ezcrypt.py
# Compiled at: 2016-04-17 08:39:18
# Size of source mod 2**32: 5500 bytes
...
def _int2bytes(data, size=None):
    if size is None:
        return data.to_bytes((data.bit_length() + 7) // 8, 'big') or b'\x00'
...
class Crypt:
    ...
    def __init__(self, key=Random.get_random_bytes(_DEFAULT_KEY_LENGTH), iv=Random.get_random_bytes(_DEFAULT_IV_LENGTH), mode=_DEFAULT_MODE, segment_size=_DEFAULT_SEGMENT_SIZE, use_padding=_DEFAULT_PADDING):
        self.key = key
        self.iv = iv
        self.mode = mode
...
class CryptString(str):
    ...
    def decrypt(self, key, iv, mode=_DEFAULT_MODE, segment_size=_DEFAULT_SEGMENT_SIZE, unpad=_DEFAULT_PADDING, encoding='utf-8'):
...
class CryptBytes(bytes):
    ...
    def decrypt(self, key, iv, mode=_DEFAULT_MODE, segment_size=_DEFAULT_SEGMENT_SIZE, unpad=_DEFAULT_PADDING):
