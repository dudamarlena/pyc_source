...
# [GCC 8.4.0]
# Embedded file name: /Users/nailixing/PyProjects/nusdb_rafiki/singa_auto/container/docker_swarm.py
# Compiled at: 2020-04-15 05:36:06
# Size of source mod 2**32: 8577 bytes
import os, time, docker, logging, traceback
...
class DockerSwarmContainerManager(ContainerManager):
    ...
    def __init__(self, network=os.environ.get('DOCKER_NETWORK', 'singa_auto'), label_num_services=os.environ.get('DOCKER_NODE_LABEL_NUM_SERVICES', 'num_services'), label_available_gpus=os.environ.get('DOCKER_NODE_LABEL_AVAILABLE_GPUS', 'available_gpus')):
    ...
    def create_service(self, service_name, docker_image, replicas, args, environment_vars, mounts={}, publish_port=None, gpus=0) -> ContainerService:
        ...
        service_id, hostname, port = self._create_service(deployment, service_name, docker_image, replicas, args, environment_vars, mounts, publish_port)
        info = {'node_id':deployment.node_id, 
    ...
    def _create_service(self, deployment, service_name, docker_image, replicas, args, environment_vars, mounts, publish_port):
        ...
        if publish_port is not None:
            ...
            constraints.append('node.id=={}'.format(deployment.node_id))
            if len(deployment.gpu_nos) > 0:
                env.append('CUDA_VISIBLE_DEVICES={}'.format(','.join([str(x) for x in deployment.gpu_nos])))
            else:
                env.append('CUDA_VISIBLE_DEVICES=-1')
        docker_service = _retry(self._client.services.create)(image=docker_image,
          args=args,
          networks=[
         self._network],
          name=service_name,
          ...
          endpoint_spec={'Ports': ports_list},
          mode={'Replicated': {'Replicas': replicas}})
        return (
         docker_service.id, hostname, container_port)
    ...
    def _update_node(self, node_id, num_services, available_gpus):
        ...
        spec = docker_node.attrs.get('Spec', {})
        spec_labels = spec.get('Labels', {})
        _retry(docker_node.update)({**spec, **{'Labels': {**spec_labels, **{self._label_num_services: str(num_services), 
