...
# [GCC 8.4.0]
# Embedded file name: /Users/nailixing/PyProjects/nusdb_rafiki/singa_auto/meta_store/meta_store.py
# Compiled at: 2020-04-15 05:36:06
# Size of source mod 2**32: 22430 bytes
from datetime import datetime
import os
from sqlalchemy import create_engine, distinct, or_
from sqlalchemy.orm import sessionmaker
from singa_auto.constants import TrainJobStatus, UserType, TrialStatus, ServiceStatus, InferenceJobStatus, ModelAccessRight
from singa_auto.meta_store.schema import Base, TrainJob, SubTrainJob, TrainJobWorker, InferenceJob, Trial, Model, User, Service, InferenceJobWorker, TrialLog, Dataset
...
class MetaStore(object):
    ...
    def get_datasets(self, user_id, task=None):
        query = self._session.query(Dataset).filter(Dataset.owner_id == user_id)
        if task is not None:
            query = query.filter(Dataset.task == task)
        datasets = query.all()
        return datasets

    def create_train_job(self, user_id, app, app_version, task, budget, train_dataset_id, val_dataset_id, train_args):
    ...
    def get_train_jobs_by_app(self, user_id, app):
        train_jobs = self._session.query(TrainJob).filter(TrainJob.app == app).filter(TrainJob.user_id == user_id).order_by(TrainJob.app_version.desc()).all()
        return train_jobs

    def get_train_jobs_by_user(self, user_id):
        train_jobs = self._session.query(TrainJob).filter(TrainJob.user_id == user_id).order_by(TrainJob.datetime_started.desc()).all()
        return train_jobs
    ...
    def get_train_jobs_by_statuses(self, statuses):
        train_jobs = self._session.query(TrainJob).filter(TrainJob.status.in_(statuses)).all()
        return train_jobs

    def get_train_job_by_app_version(self, user_id, app, app_version=-1):
        query = self._session.query(TrainJob).filter(TrainJob.user_id == user_id).filter(TrainJob.app == app)
        if app_version == -1:
    ...
    def get_sub_train_jobs_of_train_job(self, train_job_id):
        sub_train_jobs = self._session.query(SubTrainJob).filter(SubTrainJob.train_job_id == train_job_id).all()
    ...
    def get_sub_train_job_by_advisor(self, advisor_service_id):
        sub_train_job = self._session.query(SubTrainJob).filter(SubTrainJob.advisor_service_id == advisor_service_id).first()
        return sub_train_job
    ...
    def get_train_job_worker(self, service_id):
        worker = self._session.query(TrainJobWorker).filter(TrainJobWorker.service_id == service_id).first()
        return worker

    def get_workers_of_sub_train_job(self, sub_train_job_id):
        workers = self._session.query(TrainJobWorker).filter(TrainJobWorker.sub_train_job_id == sub_train_job_id).all()
        return workers
    ...
    def get_inference_job_by_predictor(self, predictor_service_id):
        inference_job = self._session.query(InferenceJob).filter(InferenceJob.predictor_service_id == predictor_service_id).first()
        return inference_job

    def get_deployed_inference_job_by_train_job(self, train_job_id):
        inference_job = self._session.query(InferenceJob).filter(InferenceJob.train_job_id == train_job_id).filter(InferenceJob.status.in_([InferenceJobStatus.RUNNING, InferenceJobStatus.STARTED])).first()
        return inference_job

    def get_inference_jobs_by_user(self, user_id):
        inference_jobs = self._session.query(InferenceJob).filter(InferenceJob.user_id == user_id).all()
    ...
    def get_inference_jobs_of_app(self, user_id, app):
        inference_jobs = self._session.query(InferenceJob).join(TrainJob, InferenceJob.train_job_id == TrainJob.id).filter(TrainJob.user_id == user_id).filter(TrainJob.app == app).order_by(InferenceJob.datetime_started.desc()).all()
        return inference_jobs

    def get_inference_jobs_by_statuses(self, statuses):
        inference_jobs = self._session.query(InferenceJob).filter(InferenceJob.status.in_(statuses)).all()
        return inference_jobs
    ...
    def get_inference_job_worker(self, service_id):
        worker = self._session.query(InferenceJobWorker).filter(InferenceJobWorker.service_id == service_id).first()
        return worker

    def get_workers_of_inference_job(self, inference_job_id):
        workers = self._session.query(InferenceJobWorker).filter(InferenceJobWorker.inference_job_id == inference_job_id).all()
        return workers

    def create_service(self, service_type, container_manager_type, docker_image, replicas, gpus):
    ...
    def mark_service_as_deploying(self, service, container_service_name, container_service_id, hostname, port, ext_hostname, ext_port, container_service_info):
    ...
    def create_model(self, user_id, name, task, model_file_bytes, model_class, docker_image, dependencies, access_right, checkpoint_id):
    ...
    def delete_model(self, model):
        sub_train_job = self._session.query(SubTrainJob).filter(SubTrainJob.model_id == model.id).first()
        if sub_train_job is not None:
            raise ModelUsedError()
        self._session.delete(model)

    def get_available_models(self, user_id, task=None):
        query = self._session.query(Model).filter(or_(Model.access_right == ModelAccessRight.PUBLIC, Model.user_id == user_id))
        if task is not None:
    ...
    def get_model_by_name(self, user_id, name):
        model = self._session.query(Model).filter(Model.user_id == user_id).filter(Model.name == name).first()
        return model
    ...
    def get_trial(self, id):
        trial = self._session.query(Trial).join(SubTrainJob, Trial.sub_train_job_id == SubTrainJob.id).filter(Trial.id == id).first()
        return trial

    def get_trial_logs(self, id):
        trial_logs = self._session.query(TrialLog).filter(TrialLog.trial_id == id).all()
        return trial_logs

    def get_best_trials_of_train_job(self, train_job_id, max_count=3):
        trials = self._session.query(Trial).join(SubTrainJob, Trial.sub_train_job_id == SubTrainJob.id).filter(SubTrainJob.train_job_id == train_job_id).filter(Trial.status == TrialStatus.COMPLETED).filter(Trial.is_params_saved == True).order_by(Trial.score.desc()).limit(max_count).all()
        return trials

    def get_best_trials_of_sub_train_job(self, sub_train_job_id, max_count=3):
        trials = self._session.query(Trial).filter(Trial.sub_train_job_id == sub_train_job_id).filter(Trial.status == TrialStatus.COMPLETED).filter(Trial.is_params_saved == True).order_by(Trial.score.desc()).limit(max_count).all()
        return trials

    def get_trials_of_train_job(self, train_job_id, limit=1000, offset=0):
        trials = self._session.query(Trial).join(SubTrainJob, Trial.sub_train_job_id == SubTrainJob.id).filter(SubTrainJob.train_job_id == train_job_id).order_by(Trial.datetime_stopped.desc(), Trial.datetime_started.desc()).offset(offset).limit(limit).all()
        return trials

    def get_trials_of_sub_train_job(self, sub_train_job_id, min_trial_no=None):
        query = self._session.query(Trial).filter(Trial.sub_train_job_id == sub_train_job_id)
