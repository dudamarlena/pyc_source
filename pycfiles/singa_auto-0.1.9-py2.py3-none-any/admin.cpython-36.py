...
# [GCC 8.4.0]
# Embedded file name: /Users/nailixing/PyProjects/nusdb_rafiki/singa_auto/admin/admin.py
# Compiled at: 2020-04-15 15:16:04
# Size of source mod 2**32: 37487 bytes
import os, logging, bcrypt, pandas as pd, zipfile, tempfile
...
class Admin(object):
    ...
    def __init__(self, meta_store=None, container_manager=None, data_store=None, param_store=None):
        ...
        self._data_store = data_store or FileDataStore()
        self._param_store = param_store or FileParamStore()
        self._base_worker_image = '{}:{}'.format(os.environ['SINGA_AUTO_IMAGE_WORKER'], os.environ['SINGA_AUTO_VERSION'])
    ...
    def get_users(self):
        ...
        return [{'id':user.id,  'email':user.email,  'user_type':user.user_type,  'banned_date':user.banned_date} for user in users]
        ...
        if 'images.csv' in dataset_zipfile.namelist():
            ...
            with tempfile.TemporaryDirectory() as (dir_path):
                ...
                if task == 'IMAGE_CLASSIFICATION':
                    ...
                    img_size = str(img.size)
        if task == 'IMAGE_CLASSIFICATION':
            stat = {'num_labeled_samples':num_labeled_samples, 
             'num_unlabeled_samples':num_unlabeled_samples,  'class_count':class_count.to_json(), 
             'ratio':ratio.to_json(),  'img_size':img_size}
        else:
            stat = {'num_labeled_samples':num_labeled_samples, 
             'num_unlabeled_samples':num_unlabeled_samples,  'class_count':class_count.to_json(), 
             'ratio':ratio.to_json()}
    ...
    def create_train_job(self, user_id, app, task, train_dataset_id, val_dataset_id, budget, model_ids=None, train_args=None):
        """
        ...
        train_job = self._meta_store.create_train_job(user_id=user_id,
          app=app,
    ...
    def get_train_jobs_by_app(self, user_id, app):
        ...
        """
        train_jobs = self._meta_store.get_train_jobs_by_app(user_id, app)
        return [{'id':x.id,  'status':x.status,  'app':x.app,  'app_version':x.app_version,  'task':x.task,  'train_dataset_id':x.train_dataset_id,  'val_dataset_id':x.val_dataset_id,  'train_args':x.train_args,  'datetime_started':x.datetime_started,  'datetime_stopped':x.datetime_stopped,  'budget':x.budget} for x in train_jobs]
    ...
    def get_best_trials_of_train_job(self, user_id, app, app_version=-1, max_count=2):
        ...
        return [{'id':trial.id,  'proposal':trial.proposal,  'datetime_started':trial.datetime_started,  'status':trial.status,  'datetime_stopped':trial.datetime_stopped,  'model_name':model.name,  'score':trial.score,  'is_params_saved':trial.is_params_saved} for trial, model in zip(best_trials, trials_models)]
    ...
    def get_trials_of_train_job(self, user_id, app, app_version=-1, limit=1000, offset=0):
        ...
        return [{'id':trial.id,  'no':trial.no,  'worker_id':trial.worker_id,  'proposal':trial.proposal,  'datetime_started':trial.datetime_started,  'status':trial.status,  'datetime_stopped':trial.datetime_stopped,  'model_name':model.name,  'score':trial.score,  'is_params_saved':trial.is_params_saved} for trial, model in zip(trials, trials_models)]
    ...
    def create_inference_job_by_checkpoint(self, user_id, budget, model_name=None):
        ...
        inference_job = self._meta_store.create_inference_job(user_id=user_id,
          ...
          budget=budget)
        self._meta_store.commit()
        inference_job, predictor_service = self._services_manager.create_inference_services((inference_job.id), use_checkpoint=True)
        return {'id':inference_job.id, 
         'model_id':model.id, 
    ...
    def create_inference_job(self, user_id, app, app_version, budget):
        ...
        inference_job = self._meta_store.create_inference_job(user_id=user_id,
          ...
          budget=budget)
        self._meta_store.commit()
        inference_job, predictor_service = self._services_manager.create_inference_services(inference_job.id)
        return {'id':inference_job.id, 
         'train_job_id':train_job.id, 
         'app':train_job.app, 
         'app_version':train_job.app_version, 
    ...
    def get_running_inference_job(self, user_id, app, app_version=-1):
        ...
        if inference_job is None:
            raise InvalidRunningInferenceJobError()
        predictor_service = self._meta_store.get_service(inference_job.predictor_service_id) if inference_job.predictor_service_id is not None else None
        return {'id':inference_job.id, 
    ...
    def get_inference_jobs_of_app(self, user_id, app):
        ...
        return [{'id':inference_job.id,  'status':inference_job.status,  'train_job_id':train_job.id,  'app':train_job.app,  'app_version':train_job.app_version,  'datetime_started':inference_job.datetime_started,  'datetime_stopped':inference_job.datetime_stopped} for inference_job, train_job in zip(inference_jobs, train_jobs)]
        ...
        for inference_job in inference_jobs:
            if inference_job.status in ('RUNNING', 'STARTED', 'ERRORED'):
                if inference_job.train_job_id:
                    ...
                    res.append({'id':inference_job.id, 
                     'status':inference_job.status, 
                     'train_job_id':train_job.id, 
                     'app':train_job.app, 
                     'app_version':train_job.app_version, 
    ...
    def create_model(self, user_id, name, task, model_file_bytes, model_class, docker_image=None, dependencies=None, access_right=ModelAccessRight.PRIVATE, checkpoint_id=None):
        if dependencies is None:
            dependencies = {}
        model = self._meta_store.create_model(user_id=user_id,
          name=name,
          task=task,
    ...
    def get_available_models(self, user_id, task=None):
        ...
        return [{'id':model.id,  'user_id':model.user_id,  'name':model.name,  'task':model.task,  'datetime_created':model.datetime_created,  'dependencies':model.dependencies,  'access_right':model.access_right} for model in models]
