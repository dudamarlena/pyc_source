...
# Embedded file name: /home/ryan/anaconda3/envs/wsl2/lib/python3.8/site-packages/pyEQL/activity_correction.py
# Compiled at: 2020-04-22 01:04:47
# Size of source mod 2**32: 37965 bytes
...
def _debye_parameter_B(temperature='25 degC'):
    ...
    """
    param_B = (8 * math.pi * unit.avogadro_constant * unit.elementary_charge ** 2 / (h2o.water_density(unit(temperature)) * unit.epsilon_0 * h2o.water_dielectric_constant(unit(temperature)) * unit.boltzmann_constant * unit(temperature))) ** 0.5
...
def _debye_parameter_activity(temperature='25 degC'):
    ...
    """
    debyeparam = unit.elementary_charge ** 3 * (2 * math.pi * unit.avogadro_constant * h2o.water_density(unit(temperature))) ** 0.5 / (4 * math.pi * unit.epsilon_0 * h2o.water_dielectric_constant(unit(temperature)) * unit.boltzmann_constant * unit(temperature)) ** 1.5
    logger.info('Computed Debye-Huckel Limiting Law Constant A^{\\gamma} = %s at %s' % (
...
def _debye_parameter_osmotic(temperature='25 degC'):
    ...
    """
    output = 0.3333333333333333 * _debye_parameter_activity(temperature)
    logger.info('Computed Debye-Huckel Limiting slope for osmotic coefficient A^{\\phi} = %s at %s' % (
     output, temperature))
...
def _debye_parameter_volume(temperature='25 degC'):
    ...
    This constant is calculated according to: [#]_
     ...
     .. math:: A_V = -2 A_{\phi} R T [ {3 \over \epsilon} {{\partial \epsilon \over \partial p}      } - {{1 \over \rho}{\partial \rho \over \partial p} }]
    ...
    result = -2 * _debye_parameter_osmotic(temperature) * unit.R * unit(temperature) * (3 / epsilon * dedp - 1 / unit('2.2 GPa'))
    ...
    logger.info('Computed Debye-Huckel Limiting Slope for volume A^V = %s at %s' % (
     result, temperature))
...
def get_activity_coefficient_davies(ionic_strength, formal_charge=1, temperature='25 degC'):
    ...
    """
    if not ionic_strength.magnitude <= 0.5:
        if ionic_strength.magnitude >= 0.1:
...
def get_apparent_volume_pitzer(ionic_strength, molality, alpha1, alpha2, beta0, beta1, beta2, C_phi, V_o, z_cation, z_anion, nu_cation, nu_anion, temperature='25 degC', b=1.2):
    ...
    third_term = nu_cation * nu_anion * unit.R * unit(temperature) * (2 * molality * BMX + molality ** 2 * C_phi * (nu_cation * nu_anion) ** 0.5)
...
def _pitzer_f1(x):
    """
    The function of ionic strength used to calculate \x08eta_MX in the Pitzer ion intercation model.
...
def _pitzer_f2(x):
    """
    The function of ionic strength used to calculate \x08eta_\\gamma in the Pitzer ion intercation model.
...
def _pitzer_log_gamma(ionic_strength, molality, B_MX, B_phi, C_phi, z_cation, z_anion, nu_cation, nu_anion, temperature='25 degC', b=unit('1.2 kg**0.5/mol**0.5')):
    ...
    .. math:: \ln \gamma_{MX} = -{|z_+ z_-| A^{Phi} ( I ^ {0.5} \over (1 + b I ^ {0.5})} + {2 \over b }\ln (1 + b I ^ {0.5}) )+    + {m (2 \nu_+ \nu_-) \over (\nu_+ + \nu_-)} (B_{MX} + B_{MX}^\Phi) + {m^2(3 (\nu_+ \nu_-)^{1.5} \over (\nu_+ + \nu_-))} C_{MX}^\Phi
