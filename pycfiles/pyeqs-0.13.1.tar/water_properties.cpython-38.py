...
# Embedded file name: /home/ryan/anaconda3/envs/wsl2/lib/python3.8/site-packages/pyEQL/water_properties.py
...
def water_density(temperature=25 * unit('degC'), pressure=1 * unit('atm')):
    ...
    """
    density = 999.65 + 0.20438 * temperature.to('degC').magnitude - 0.061744 * temperature.to('degC').magnitude ** 1.5
    density = density * unit('kg/m**3')
    logger.info('Computed density of water as %s at T= %s and P = %s' % (
...
def water_specific_weight(temperature=25 * unit('degC'), pressure=1 * unit('atm')):
    ...
    logger.info('Computed specific weight of water as %s at T=%s and P = %s' % (
...
def water_viscosity_dynamic(temperature=25 * unit('degC'), pressure=1 * unit('atm')):
    ...
    for i in range(len(b)):
        ...
        logger.info('Computed dynamic (absolute) viscosity of water as %s at T=%s and P = %s' % (
...
def water_viscosity_kinematic(temperature=25 * unit('degC'), pressure=1 * unit('atm')):
    ...
    logger.info('Computed kinematic viscosity of water as %s at T=%s and P = %s ' % (
