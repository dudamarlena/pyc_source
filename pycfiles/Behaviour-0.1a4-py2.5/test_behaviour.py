...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/test/test_behaviour.py
# Compiled at: 2007-12-13 19:26:11
"""
Unit tests for behaviour.py
"""
__docformat__ = 'restructuredtext en'
import sys, types, unittest, re, decimal, behaviour
...
def _testForFailure(parent, wantException, wantMessage, executable, *args, **kwargs):
    ...
    except wantException, ex:
        if not isRE and str(ex) == wantMessage:
            return True
        elif isRE and re.match(wantMessage, str(ex)) != None:
            return True
        else:
...
class TestShouldEqual(unittest.TestCase):
    ...
    def test_shouldEqual_rejects_notEqual(self):
        _testForFailure(self.behave, AssertionError, 'Behaviour.shouldEqual: Actual value (1) should equal expected value (2).', self.behave.shouldEqual, 1, 2)
...
class TestShouldNotEqual(unittest.TestCase):
    ...
    def test_shouldNotEqual_rejects_equal(self):
        _testForFailure(self.behave, AssertionError, 'Behaviour.shouldNotEqual: Actual value (1) should not equal unexpected value (1).', self.behave.shouldNotEqual, 1, 1)
...
class TestShouldApproximatelyEqual(unittest.TestCase):
    ...
    def test_shouldApproximatelyEqual_rejects_notQuiteEqual(self):
        _testForFailure(self.behave, AssertionError, 'Behaviour.shouldApproximatelyEqual: Actual value (1.00000000011) should approximately equal expected value (1) to 10 significant digits. Allowable difference is 1e-10, actual difference is 1.10000009101e-10.', self.behave.shouldApproximatelyEqual, 1.00000000011, 1, significantDigits=10)

    def test_shouldApproximatelyEqual_rejects_nonNumeric_actual(self):
        _testForFailure(self.behave, TypeError, "Behaviour.shouldApproximatelyEqual: Type (<type 'str'>) value (spam) cannot be used, must be a number.", self.behave.shouldApproximatelyEqual, 1, 'spam')

    def test_shouldApproximatelyEqual_rejects_nonNumeric_expected(self):
        _testForFailure(self.behave, TypeError, "Behaviour.shouldApproximatelyEqual: Type (<type 'tuple'>) value ((1, 2)) cannot be used, must be a number.", self.behave.shouldApproximatelyEqual, (1,
                                                                                                                                                                                                     2), 1)
...
class TestShouldNotApproximatelyEqual(unittest.TestCase):
    ...
    def test_shouldNotApproximatelyEqual_rejects_almostEqual(self):
        _testForFailure(self.behave, AssertionError, 'Behaviour.shouldNotApproximatelyEqual: Actual value (1.00000000009) should not approximately equal unexpected value (1) to 10 significant digits. Allowable difference is 1e-10, actual difference is 9.00000074466e-11.', self.behave.shouldNotApproximatelyEqual, 1.00000000009, 1, significantDigits=10)

    def test_shouldNotApproximatelyEqual_rejects_nonNumeric_actual(self):
        _testForFailure(self.behave, TypeError, "Behaviour.shouldNotApproximatelyEqual: Type (<type 'list'>) value ([1, 2]) cannot be used, must be a number.", self.behave.shouldNotApproximatelyEqual, 1, [1, 2])

    def test_shouldNotApproximatelyEqual_rejects_nonNumeric_expected(self):
        _testForFailure(self.behave, TypeError, "Behaviour.shouldNotApproximatelyEqual: Type (<type 'dict'>) value ({1: 'spam'}) cannot be used, must be a number.", self.behave.shouldNotApproximatelyEqual, {1: 'spam'}, 1)
...
class TestShouldBeSameAs(unittest.TestCase):
    ...
    def test_shouldBeSameAs_rejects_different(self):
        self.same = self.object
        _testForFailure(self.behave, AssertionError, 'Behaviour\\.shouldBeSameAs\\: Actual object id \\(.+\\) value \\(\\[\\]\\) should be the expected object id \\(.+\\) value \\(\\[\\]\\)\\.', self.behave.shouldBeSameAs, self.object, [], isRE=True)
...
class TestShouldNotBeSameAs(unittest.TestCase):
    ...
    def test_shouldNotBeSameAs_rejects_same(self):
        self.same = self.object
        _testForFailure(self.behave, AssertionError, 'Behaviour\\.shouldNotBeSameAs: Actual object id \\(.+\\) value \\(\\[\\]\\) should not be the unexpected object id \\(.+\\) value \\(\\[\\]\\).', self.behave.shouldNotBeSameAs, self.object, self.same, isRE=True)
...
class TestShouldMatch(unittest.TestCase):
    ...
    def test_shouldMatch_rejects_mismatch(self):
        _testForFailure(self.behave, AssertionError, 'Behaviour.shouldMatch: Actual value (spam and eggs) should match the regular expression pattern (.+but.+).', self.behave.shouldMatch, 'spam and eggs', '.+but.+')

    def test_shouldMatch_rejects_nonStringActual(self):
        _testForFailure(self.behave, TypeError, "Behaviour.shouldMatch: Type (<type 'int'>) value (12345) cannot be used, must be a string.", self.behave.shouldMatch, 12345, '.+but.+')

    def test_shouldMatch_rejects_nonStringPattern(self):
        _testForFailure(self.behave, TypeError, "Behaviour.shouldMatch: Type (<type 'int'>) value (12345) cannot be used, must be a string.", self.behave.shouldMatch, 'spam and eggs', 12345)
...
class TestShouldNotMatch(unittest.TestCase):
    ...
    def test_shouldNotMatch_rejects_match(self):
        _testForFailure(self.behave, AssertionError, 'Behaviour.shouldNotMatch: Actual value (spam and eggs) should not match the regular expression pattern (.+and.+).', self.behave.shouldNotMatch, 'spam and eggs', '.+and.+')

    def test_shouldNotMatch_rejects_nonStringActual(self):
        _testForFailure(self.behave, TypeError, "Behaviour.shouldNotMatch: Type (<type 'int'>) value (12345) cannot be used, must be a string.", self.behave.shouldNotMatch, 12345, '.+but.+')

    def test_shouldNotMatch_rejects_nonStringPattern(self):
        _testForFailure(self.behave, TypeError, "Behaviour.shouldNotMatch: Type (<type 'int'>) value (12345) cannot be used, must be a string.", self.behave.shouldNotMatch, 'spam and eggs', 12345)
...
class TestShouldBeNone(unittest.TestCase):
    ...
    def test_shouldBeNone_rejects_notNone(self):
        _testForFailure(self.behave, AssertionError, 'Behaviour.shouldBeNone: Actual value (0) should be None.', self.behave.shouldBeNone, 0)
...
class TestShouldNotBeNone(unittest.TestCase):
    ...
    def test_shouldNotBeNone_rejects_None(self):
        _testForFailure(self.behave, AssertionError, 'Behaviour.shouldNotBeNone: Actual value (None) should not be None.', self.behave.shouldNotBeNone, None)
        return
...
class TestShouldBeZero(unittest.TestCase):
    ...
    def test_shouldBeZero_rejects_nonZero(self):
        _testForFailure(self.behave, AssertionError, 'Behaviour.shouldBeZero: Actual value (1) should be Zero.', self.behave.shouldBeZero, 1)

    def test_shouldBeZero_rejects_notNumber(self):
        _testForFailure(self.behave, TypeError, "Behaviour.shouldBeZero: Type (<type 'str'>) value (spam) cannot be used, must be a number.", self.behave.shouldBeZero, 'spam')
...
class TestShouldNotBeZero(unittest.TestCase):
    ...
    def test_shouldNotBeZero_rejects_zero_integer(self):
        _testForFailure(self.behave, AssertionError, 'Behaviour.shouldNotBeZero: Actual value (0) should not be Zero.', self.behave.shouldNotBeZero, 0)

    def test_shouldNotBeZero_rejects_zero_float(self):
        _testForFailure(self.behave, AssertionError, 'Behaviour.shouldNotBeZero: Actual value (0.0) should not be Zero.', self.behave.shouldNotBeZero, 0.0)

    def test_shouldNotBeZero_rejects_zero_decimal(self):
        _testForFailure(self.behave, AssertionError, 'Behaviour.shouldNotBeZero: Actual value (0) should not be Zero.', self.behave.shouldNotBeZero, decimal.Decimal(0))

    def test_shouldNotBeZero_rejects_notNumber(self):
        _testForFailure(self.behave, TypeError, "Behaviour.shouldNotBeZero: Type (<type 'str'>) value (spam) cannot be used, must be a number.", self.behave.shouldNotBeZero, 'spam')
...
class TestShouldBeTrue(unittest.TestCase):
    ...
    def test_shouldBeTrue_rejects_False(self):
        _testForFailure(self.behave, AssertionError, 'Behaviour.shouldBeTrue: Actual value (False) should be True.', self.behave.shouldBeTrue, False)

    def test_shouldBeTrue_rejects_notBoolean(self):
        _testForFailure(self.behave, TypeError, "Behaviour.shouldBeTrue: Type (<type 'str'>) value (spam) cannot be used, must be a boolean (True or False).", self.behave.shouldBeTrue, 'spam')
...
class TestShouldBeFalse(unittest.TestCase):
    ...
    def test_shouldBeFalse_rejects_True(self):
        _testForFailure(self.behave, AssertionError, 'Behaviour.shouldBeFalse: Actual value (True) should be False.', self.behave.shouldBeFalse, True)

    def test_shouldBeTrue_rejects_notBoolean(self):
        _testForFailure(self.behave, TypeError, "Behaviour.shouldBeFalse: Type (<type 'int'>) value (12345) cannot be used, must be a boolean (True or False).", self.behave.shouldBeFalse, 12345)
...
class TestShouldBeEmpty(unittest.TestCase):
    ...
    def test_shouldBeEmpty_rejects_nonEmpty_string(self):
        _testForFailure(self.behave, AssertionError, 'Behaviour.shouldBeEmpty: Actual value (spam) should be empty.', self.behave.shouldBeEmpty, 'spam')
    ...
    def test_shouldBeEmpty_rejects_nonEmpty_dict(self):
        _testForFailure(self.behave, AssertionError, "Behaviour.shouldBeEmpty: Actual value ({1: 'spam'}) should be empty.", self.behave.shouldBeEmpty, {1: 'spam'})

    def test_shouldBeEmpty_rejects_notSequenceOrMapping(self):
        _testForFailure(self.behave, TypeError, "Behaviour.shouldBeEmpty: Type (<type 'int'>) value (12345) cannot be used, must be a sequence or mapping.", self.behave.shouldBeEmpty, 12345)
...
class TestShouldNotBeEmpty(unittest.TestCase):
    ...
    def test_shouldNotBeEmpty_rejects_empty_string(self):
        _testForFailure(self.behave, AssertionError, 'Behaviour.shouldNotBeEmpty: Actual value () should be not empty.', self.behave.shouldNotBeEmpty, '')
    ...
    def test_shouldNotBeEmpty_rejects_notSequenceOrMapping(self):
        _testForFailure(self.behave, TypeError, "Behaviour.shouldNotBeEmpty: Type (<type 'int'>) value (12345) cannot be used, must be a sequence or mapping.", self.behave.shouldNotBeEmpty, 12345)
...
class TestShouldInclude(unittest.TestCase):
    ...
    def test_shouldInclude_accepts_member_dict_subclass(self):
        self.behave.shouldInclude(MyDict({1: 'spam', 2: 'eggs', 3: 'sausage'}), (2,
                                                                                 'eggs'))

    def test_shouldInclude_rejects_nonMember_string(self):
        _testForFailure(self.behave, AssertionError, 'Behaviour.shouldInclude: String (spam, eggs, sausage) should include item (bacon).', self.behave.shouldInclude, 'spam, eggs, sausage', 'bacon')

    def test_shouldInclude_rejects_nonMember_tuple(self):
        _testForFailure(self.behave, AssertionError, "Behaviour.shouldInclude: Tuple (('spam', 'eggs', 'sausage')) should include item (bacon).", self.behave.shouldInclude, ('spam',
                                                                                                                                                                              'eggs',
                                                                                                                                                                              'sausage'), 'bacon')

    def test_shouldInclude_rejects_nonMember_list(self):
        _testForFailure(self.behave, AssertionError, "Behaviour.shouldInclude: List (['spam', 'eggs', 'sausage']) should include item (bacon).", self.behave.shouldInclude, ['spam', 'eggs', 'sausage'], 'bacon')

    def test_shouldInclude_rejects_nonMember_dict_missingKey(self):
        _testForFailure(self.behave, AssertionError, "Behaviour.shouldInclude: Dictionary ({1: 'spam', 2: 'eggs', 3: 'sausage'}) should include key (4).", self.behave.shouldInclude, {1: 'spam', 2: 'eggs', 3: 'sausage'}, (4,
                                                                                                                                                                                                                             'eggs'))

    def test_shouldInclude_rejects_nonMember_dict_hasKeyButDifferentData(self):
        _testForFailure(self.behave, AssertionError, "Behaviour.shouldInclude: Dictionary ({1: 'spam', 2: 'eggs', 3: 'sausage'}) should include key (2) data (bacon).", self.behave.shouldInclude, {1: 'spam', 2: 'eggs', 3: 'sausage'}, (2,
                                                                                                                                                                                                                                          'bacon'))

    def test_shouldInclude_rejects_notSequenceorMapping(self):
        _testForFailure(self.behave, TypeError, "Behaviour.shouldInclude: Type (<type 'int'>) value (12345) cannot be used, must be a sequence or mapping.", self.behave.shouldInclude, 12345, 234)

    def test_shouldInclude_rejects_dict_itemNot2Tuple(self):
        _testForFailure(self.behave, TypeError, "Behaviour.shouldInclude: Item ((2, 'eggs', 'junk')) must be a 2-tuple ( key, data ).", self.behave.shouldInclude, {1: 'spam', 2: 'eggs', 3: 'sausage'}, (2,
                                                                                                                                                                                                          'eggs',
                                                                                                                                                                                                          'junk'))

    def test_shouldInclude_rejects_dict_itemNotATuple(self):
        _testForFailure(self.behave, TypeError, "Behaviour.shouldInclude: Item (2) type (<type 'int'>) must be a 2-tuple ( key, data ).", self.behave.shouldInclude, {1: 'spam', 2: 'eggs', 3: 'sausage'}, 2)
...
class TestShouldNotInclude(unittest.TestCase):
    ...
    def test_shouldNotInclude_rejects_member_string(self):
        _testForFailure(self.behave, AssertionError, 'Behaviour.shouldNotInclude: String (spam, eggs, sausage) should not include item (eggs).', self.behave.shouldNotInclude, 'spam, eggs, sausage', 'eggs')

    def test_shouldNotInclude_rejects_member_tuple(self):
        _testForFailure(self.behave, AssertionError, "Behaviour.shouldNotInclude: Tuple (('spam', 'eggs', 'sausage')) should not include item (eggs).", self.behave.shouldNotInclude, ('spam',
                                                                                                                                                                                       'eggs',
                                                                                                                                                                                       'sausage'), 'eggs')

    def test_shouldNotInclude_rejects_member_list(self):
        _testForFailure(self.behave, AssertionError, "Behaviour.shouldNotInclude: List (['spam', 'eggs', 'sausage']) should not include item (eggs).", self.behave.shouldNotInclude, ['spam', 'eggs', 'sausage'], 'eggs')

    def test_shouldNotInclude_rejects_member_dict(self):
        _testForFailure(self.behave, AssertionError, "Behaviour.shouldInclude: Dictionary ({1: 'spam', 2: 'eggs', 3: 'sausage'}) should not include key (2) data (eggs).", self.behave.shouldNotInclude, {1: 'spam', 2: 'eggs', 3: 'sausage'}, (2,
                                                                                                                                                                                                                                                'eggs'))

    def test_shouldNotInclude_rejects_notSequenceorMapping(self):
        _testForFailure(self.behave, TypeError, "Behaviour.shouldNotInclude: Type (<type 'float'>) value (12345.6789) cannot be used, must be a sequence or mapping.", self.behave.shouldNotInclude, 12345.6789, 234)

    def test_shouldNotInclude_rejects_dict_itemNot2Tuple(self):
        _testForFailure(self.behave, TypeError, "Behaviour.shouldNotInclude: Item ((2, 'eggs', 'junk')) must be a 2-tuple ( key, data ).", self.behave.shouldNotInclude, {1: 'spam', 2: 'eggs', 3: 'sausage'}, (2,
                                                                                                                                                                                                                'eggs',
                                                                                                                                                                                                                'junk'))

    def test_shouldNotInclude_rejects_dict_itemNotATuple(self):
        _testForFailure(self.behave, TypeError, "Behaviour.shouldNotInclude: Item (2) type (<type 'int'>) must be a 2-tuple ( key, data ).", self.behave.shouldNotInclude, {1: 'spam', 2: 'eggs', 3: 'sausage'}, 2)
...
class TestShouldRaiseException(unittest.TestCase):
    ...
    def test_ShouldRaiseException_rejects_noException(self):
        try:
            ...
            self.fail('Behaviour.shouldRaiseException: Executable (_divideByZero) raised error class (%s), expected an AssertionError' % str(ex))
        else:
    ...
    def test_ShouldRaiseException_rejects_wrongException(self):
        _testForFailure(self.behave, AssertionError, "Behaviour.shouldRaiseException: Executable (_divideByZero) should raise exception (<type 'exceptions.ZeroDivisionError'>), got exception (<type 'exceptions.TypeError'>).", self.behave.shouldRaiseException, ZeroDivisionError, self._divideByZero, 1, second='spam')

    def test_ShouldRaiseException_rejects_wrongPattern(self):
        _testForFailure(self.behave, AssertionError, "Behaviour.shouldRaiseException: Executable (_divideByZero) raised exception (<type 'exceptions.ZeroDivisionError'>), pattern (integer division or modulo by zero) should be pattern (Won't match!).", self.behave.shouldRaiseException, ZeroDivisionError, self._divideByZero, 1, second=0, exceptionPattern="Won't match!")

    def test_ShouldRaiseException_rejects_wrongArgs(self):
        _testForFailure(self.behave, AssertionError, "Behaviour.shouldRaiseException: Executable (_divideByZero) raised exception (<type 'exceptions.ZeroDivisionError'>), args (('integer division or modulo by zero',)) should be args (123450).", self.behave.shouldRaiseException, ZeroDivisionError, self._divideByZero, 1, second=0, exceptionArgs=123450)
...
class TestReturnsGivenMessage(unittest.TestCase):
    ...
    def test_shouldEqual_rejects_notEqual_returnsGivenMessage(self):
        _testForFailure(self.behave, AssertionError, 'Actual should equal expected.', self.behave.shouldEqual, 1, 2, passMessage=True)

    def test_shouldNotEqual_rejects_equal_returnsGivenMessage(self):
        _testForFailure(self.behave, AssertionError, 'Actual should not equal unexpected.', self.behave.shouldNotEqual, 1, 1, passMessage=True)

    def test_shouldApproximatelyEqual_rejects_notQuiteEqual_returnsGivenMessage(self):
        _testForFailure(self.behave, AssertionError, 'Actual should approximtely equal expected.', self.behave.shouldApproximatelyEqual, 1.00000000011, 1, significantDigits=10, passMessage=True)

    def test_shouldNotApproximatelyEqual_rejects_almostEqual_returnsGivenMessage(self):
        _testForFailure(self.behave, AssertionError, 'Actual should not approximtely equal unexpected.', self.behave.shouldNotApproximatelyEqual, 1.00000000009, 1, significantDigits=10, passMessage=True)

    def test_shouldBeSameAs_rejects_different_returnsGivenMessage(self):
        self.object = []
        _testForFailure(self.behave, AssertionError, 'Actual should be expected.', self.behave.shouldBeSameAs, self.object, [], passMessage=True)

    def test_shouldNotBeSameAs_rejects_same_returnsGivenMessage(self):
        self.object = []
        self.same = self.object
        _testForFailure(self.behave, AssertionError, 'Actual should not be unexpected.', self.behave.shouldNotBeSameAs, self.object, self.same, passMessage=True)

    def test_shouldMatch_rejects_mismatch_returnsGivenMessage(self):
        _testForFailure(self.behave, AssertionError, 'Actual should match pattern.', self.behave.shouldMatch, 'spam and eggs', '.+but.+', passMessage=True)

    def test_shouldNotMatch_rejects_match_returnsGivenMessage(self):
        _testForFailure(self.behave, AssertionError, 'Actual should not match pattern.', self.behave.shouldNotMatch, 'spam and eggs', '.+and.+', passMessage=True)

    def test_shouldBeNone_rejects_notNone_returnsGivenMessage(self):
        _testForFailure(self.behave, AssertionError, 'Actual should be None.', self.behave.shouldBeNone, 0, passMessage=True)

    def test_shouldNotBeNone_rejects_None_returnsGivenMessage(self):
        _testForFailure(self.behave, AssertionError, 'Actual should not be None.', self.behave.shouldNotBeNone, None, passMessage=True)
        return

    def test_shouldBeZero_rejects_nonZero_returnsGivenMessage(self):
        _testForFailure(self.behave, AssertionError, 'Actual should be Zero.', self.behave.shouldBeZero, 1, passMessage=True)

    def test_shouldNotBeZero_rejects_zero_returnsGivenMessage(self):
        _testForFailure(self.behave, AssertionError, 'Actual should not be Zero.', self.behave.shouldNotBeZero, 0, passMessage=True)

    def test_shouldBeTrue_rejects_False_returnsGivenMessage(self):
        _testForFailure(self.behave, AssertionError, 'Actual should be True.', self.behave.shouldBeTrue, False, passMessage=True)

    def test_shouldBeFalse_rejects_True_returnsGivenMessage(self):
        _testForFailure(self.behave, AssertionError, 'Actual should be False.', self.behave.shouldBeFalse, True, passMessage=True)

    def test_shouldBeEmpty_rejects_nonEmpty_list_returnsGivenMessage(self):
        _testForFailure(self.behave, AssertionError, 'Actual should be empty.', self.behave.shouldBeEmpty, ['spam'], passMessage=True)

    def test_shouldBeEmpty_rejects_nonEmpty_dict_returnsGivenMessage(self):
        _testForFailure(self.behave, AssertionError, 'Actual should be empty.', self.behave.shouldBeEmpty, {1: 'spam'}, passMessage=True)

    def test_shouldNotBeEmpty_rejects_empty_tuple_returnsGivenMessage(self):
        _testForFailure(self.behave, AssertionError, 'Actual should be not empty.', self.behave.shouldNotBeEmpty, (), passMessage=True)
    ...
    def test_shouldInclude_rejects_nonMember_string_returnsGivenMessage(self):
        _testForFailure(self.behave, AssertionError, 'Actual should include item.', self.behave.shouldInclude, 'spam, eggs, sausage', 'bacon', passMessage=True)

    def test_shouldInclude_rejects_nonMember_dict_hasKeyButDifferentData_returnsGivenMessage(self):
        _testForFailure(self.behave, AssertionError, 'Actual should include item.', self.behave.shouldInclude, {1: 'spam', 2: 'eggs', 3: 'sausage'}, (2,
                                                                                                                                                      'bacon'), passMessage=True)

    def test_shouldNotInclude_rejects_member_list_returnsGivenMessage(self):
        _testForFailure(self.behave, AssertionError, 'Actual should not include item.', self.behave.shouldNotInclude, ['spam', 'eggs', 'sausage'], 'eggs', passMessage=True)

    def test_shouldNotInclude_rejects_member_dict_returnsGivenMessage(self):
        _testForFailure(self.behave, AssertionError, 'Actual should not include item.', self.behave.shouldNotInclude, {1: 'spam', 2: 'eggs', 3: 'sausage'}, (2,
                                                                                                                                                             'eggs'), passMessage=True)
