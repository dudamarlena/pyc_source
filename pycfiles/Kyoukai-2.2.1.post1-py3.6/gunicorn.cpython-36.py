...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kyoukai/backends/gunicorn.py
# Compiled at: 2018-02-07 20:02:41
...
class GunicornAdapter(object):
    ...
    async def _run_application(self, environ, start_response) -> ClosingIterator:
        ...
        c = response(environ, start_response)
        return c
    ...
    def run_application(self, environment: dict, start_response: typing.Callable) -> asyncio.Task:
        ...
        :return: A new :class:`asyncio.Task` that can be awaited on to get a response from the         application.
        """
        is_async = environment.get('wsgi.async', False)
        if not is_async:
            r = Response('<h1>Error</h1><br/>You did not use the <code>gaiohttp</code> gunicorn worker. This is an error! Please switch to the gaiohttp worker instead.')
