...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sql/connection.py
...
class Connection(object):
        ...
        try:
            if creator:
                engine = sqlalchemy.create_engine(connect_str,
                  connect_args=connect_args, creator=creator)
            else:
                engine = sqlalchemy.create_engine(connect_str,
    ...
    def connection_list(cls):
        ...
        for key in sorted(cls.connections):
            engine_url = cls.connections[key].metadata.bind.url
    ...
    def _close(cls, descriptor):
        ...
        if not conn:
            raise Exception('Could not close connection because it was not found amongst these: %s' % str(cls.connections.keys()))
