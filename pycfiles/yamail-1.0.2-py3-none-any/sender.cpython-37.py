...
# [GCC 8.4.0]
# Embedded file name: /Users/didi/PycharmProjects/nnmail/yamail/sender.py
# Compiled at: 2019-11-25 23:08:21
# Size of source mod 2**32: 8290 bytes
import time, logging, smtplib
from .log import get_logger
from .utils import find_user_home_path
from .oauth2 import get_oauth2_info, get_oauth_string
from .headers import resolve_addresses
from .validate import validate_email_with_regex
from .password import handle_password
from .message import prepare_message
from .headers import make_addr_alias_user

class SMTPBase:
    __doc__ = " :class:`yagmail.SMTP` is a magic wrapper around\n    ``smtplib``'s SMTP connection, and allows messages to be sent."

    def __init__(self, user=None, password=None, host='smtp.gmail.com', port=None, smtp_starttls=None, smtp_ssl=True, smtp_set_debuglevel=0, smtp_skip_login=False, encoding='utf-8', oauth2_file=None, soft_email_validation=True, **kwargs):
        self.log = get_logger()
        self.set_logging()
    ...
    def prepare_send(self, to=None, subject=None, contents=None, attachments=None, cc=None, bcc=None, headers=None, newline_to_break=True):
        ...
        msg = prepare_message(self.user, self.useralias, addresses, subject, contents, attachments, headers, self.encoding, newline_to_break)
        recipients = addresses['recipients']
        msg_string = msg.as_string()
        return (recipients, msg_string)

    def send(self, to=None, subject=None, contents=None, attachments=None, cc=None, bcc=None, preview_only=False, headers=None, newline_to_break=True):
        """ Use this to send an email with gmail"""
        self.login()
        recipients, msg_string = self.prepare_send(to, subject, contents, attachments, cc, bcc, headers, newline_to_break)
        if preview_only:
            return (
             recipients, msg_string)
