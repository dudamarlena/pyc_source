...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/reactionrnn/reactionrnn.py
# Compiled at: 2017-08-19 19:04:28
...
class reactionrnn:
    ...
    def __init__(self, weights_path=None, vocab_path=None):
        if weights_path is None:
            weights_path = resource_filename(__name__, 'reactionrnn_weights.hdf5')
        if vocab_path is None:
            vocab_path = resource_filename(__name__, 'reactionrnn_vocab.json')
        ...
        self.model_enc = Model(inputs=self.model.input, outputs=self.model.get_layer('rnn').output)
        return
    ...
    def predict(self, texts, **kwargs):
        ...
        if len(texts_enc) == 1:
            predicts_dict = {react:round(float(predicts[0][i]), 4) for i, react in enumerate(self.REACTIONS)}
            predicts_dict = OrderedDict(sorted(predicts_dict.items(), key=lambda t: -t[1]))
...
def reactionrnn_model(weights_path, num_classes, maxlen=140):
    ...
    embedded = Embedding(num_classes, 100, input_length=maxlen, name='embedding')(input)
    rnn = GRU(256, return_sequences=False, name='rnn')(embedded)
    output = Dense(5, name='output', activation=lambda x: K.relu(x) / K.sum(K.relu(x), axis=-1))(rnn)
