...
# [GCC 8.4.0]
# Embedded file name: /Users/jennyq/.pyenv/versions/venv_t12/lib/python3.7/site-packages/tendenci/apps/base/templatetags/base_filters.py
# Compiled at: 2020-03-30 17:48:03
# Size of source mod 2**32: 13710 bytes
from builtins import str
import re, os, pytz, codecs
...
def obfuscate_email(email, linktext=None, autoescape=None):
    ...
    if linktext:
        ...
        linktext = email
    rotten_link = '<script type="text/javascript">document.write         ("<n uers=\\"znvygb:%s\\">%s<\\057n>".replace(/[a-zA-Z]/g,         function(c){return String.fromCharCode((c<="Z"?90:122)>=        (c=c.charCodeAt(0)+13)?c:c-26);}));</script>' % (email, linktext)
...
def twitterize(value, autoescape=None):
    ...
    value = re.sub('(^[^:]+)', '<a href="http://twitter.com/\\1">\\1</a>', value)
    value = re.sub('(\\s+|\\A)@([a-zA-Z0-9\\-_]*)\\b', '\\1<a href="http://twitter.com/\\2">@\\2</a>', value)
    value = re.sub('(\\s+|\\A)#([a-zA-Z0-9\\-_]*)\\b', '\\1<a href="http://search.twitter.com/search?q=%23\\2">#\\2</a>', value)
...
def thumbnail(file, size='200x200'):
    ...
    if default_storage.exists(miniature_filename):
        ...
        mt_miniature_filename = default_storage.get_modified_time(miniature_filename)
