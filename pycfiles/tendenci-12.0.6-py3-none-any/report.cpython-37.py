...
# [GCC 8.4.0]
# Embedded file name: /Users/jennyq/.pyenv/versions/venv_t12/lib/python3.7/site-packages/tendenci/libs/model_report/report.py
# Compiled at: 2020-02-11 12:52:19
# Size of source mod 2**32: 45073 bytes
...
def fields_for_model(model, fields=None, exclude=None, widgets=None, formfield_callback=None, localized_fields=None, labels=None, help_texts=None, error_messages=None, field_classes=None, apply_limit_choices_to=True):
    ...
    ignored = []
    opts = model._meta
    import django.db.models.fields as ModelField
    ...
    for f in sorted(chain(opts.concrete_fields, sortable_private_fields, opts.many_to_many)):
        ...
        if not localized_fields == ALL_FIELDS:
            if localized_fields:
                if f.name in localized_fields:
                ...
                if help_texts:
                    if f.name in help_texts:
                        kwargs['help_text'] = help_texts[f.name]
                if error_messages and f.name in error_messages:
                    kwargs['error_messages'] = error_messages[f.name]
            if field_classes:
                if f.name in field_classes:
    ...
    if fields:
        field_dict = OrderedDict([(f, field_dict.get(f)) for f in fields if exclude if exclude and f not in exclude if f not in ignored])
    return field_dict
...
class ReportAdmin(object):
    ...
    def __init__(self, parent_report=None, request=None):
        ...
        if parent_report:
            self.related_inline_field = [f for f in self.model._meta.get_fields() if f.remote_field if f.remote_field.model is self.parent_report.model][0]
            self.related_inline_accessor = self.related_inline_field.remote_field.get_accessor_name()
            self.related_fields = ['%s__%s' % (get_model_name(pfield.model), attname) for pfield, attname in self.parent_report.model_fields if not isinstance(pfield, str) if pfield.model == self.related_inline_field.remote_field.model]
    ...
    def get_query_set(self, filter_kwargs):
        ...
        elif self.model._meta.verbose_name.lower() == 'membership':
    ...
    def get_render_context(self, request, extra_context={}, by_row=None):
        ...
        try:
            ...
            context = {'render_report':render_report,  'is_inline':is_inline, 
    ...
    def render(self, request, extra_context={}):
        ...
        return render_to_resp(request=request, template_name=(self.template_name), context=context_or_response)
    ...
    def get_form_groupby(self, request):
        ...
        if hasattr(self, 'hide_show_only_totals'):
            if self.hide_show_only_totals:
        ...
        if widget:
            for field_to_set_widget, widget in widget.items():
                if field_to_set_widget == field:
                    return (
                     True, widget, MultipleChoiceField().__class__)
    ...
    def get_form_filter(self, request):
        ...
        if not form_fields:
            ...
            for k, v in dict(form_fields).items():
                if v is None:
                    ...
                    if isinstance(model_field, (DateField, DateTimeField)):
                        ...
                        if not hasattr(model_field, 'formfield'):
                            ...
                            if isinstance(model_field, ForeignKey):
                                ...
                                if self.always_show_full_username:
                                    if model_field.remote_field.model == User:
                ...
                elif isinstance(v, forms.BooleanField):
                    form_fields.pop(k)
                    ...
                    if isinstance(v, (forms.DateField, forms.DateTimeField)):
                        field_name = k.split('__')[0]
                        model_field = opts.get_field(field_name)
                        form_fields.pop(k)
                        field = RangeField(model_field.formfield)
                    else:
                        if opts.verbose_name.lower() == 'invoice':
                            if k == 'status_detail':
                                form_fields.pop(k)
                                field = forms.ChoiceField()
                                field.label = v.label
                            ...
                            if opts.verbose_name.lower() == 'membership' and k == 'status_detail':
                                form_fields.pop(k)
                                field = forms.ChoiceField()
                                field.label = v.label
                                field.help_text = v.help_text
                    ...
                    for index, value in enumerate(resource):
                        if ffields_include_self[index] in self.group_totals:
                            ...
                            if ffields[index] in self.group_totals:
                                ...
                                if ffields[index] in self.report_totals:
