...
# [GCC 8.4.0]
# Embedded file name: /Users/jennyq/.pyenv/versions/venv_t12/lib/python3.7/site-packages/tendenci/apps/articles/templatetags/article_tags.py
...
def article_options(context, user, article):
    context.update({'opt_object':article, 
     'user':user})
    return context
...
def article_nav(context, user, article=None):
    context.update({'nav_object':article, 
     'user':user})
    return context
...
def article_current_app(context, user, article=None):
    context.update({'app_object':article, 
     'user':user})
    return context
