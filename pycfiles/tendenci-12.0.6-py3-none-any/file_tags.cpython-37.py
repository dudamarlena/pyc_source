...
# [GCC 8.4.0]
# Embedded file name: /Users/jennyq/.pyenv/versions/venv_t12/lib/python3.7/site-packages/tendenci/apps/files/templatetags/file_tags.py
...
def file_options(context, user, file):
    context.update({'opt_object':file, 
     'user':user})
    return context
...
def file_nav(context, user, file=None):
    context.update({'nav_object':file, 
     'user':user})
    return context
...
def file_current_app(context, user, file=None):
    context.update({'app_object':file, 
     'user':user})
    return context
...
def file_thumbnail(context, layout, file_obj):
    ...
    if layout == 'grid':
        if file_obj.type() == 'image':
            use_image = True
    context.update({'use_image': use_image})
    return context
