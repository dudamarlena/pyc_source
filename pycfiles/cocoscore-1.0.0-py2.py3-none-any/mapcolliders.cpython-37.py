...
__doc__ = 'Support for handling collisions between an actor and a container of objects'
...
class RectMapCollider(object):
    ...
    def collide_map(self, maplayer, last, new, vx, vy):
        ...
        for obj in objects:
            if not obj is None:
                if not (obj.tile is None or obj.intersects(new)):
                    continue
    ...
    def detect_collision(self, obj, last, new):
        ...
        if last.bottom >= obj.top > new.bottom:
            ...
            elif last.right <= obj.left < new.right:
