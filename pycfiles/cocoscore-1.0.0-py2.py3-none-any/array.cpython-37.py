...
# Embedded file name: d:\dev\cocos2020\cocos\audio\SDL\array.py
...
class SDL_array:
    ...
    def as_int16(self):
        ...
        :rtype: SDL_array
        """
        return SDL_array(self.ptr, self.count * sizeof(self.ctype) / 2, c_ushort)
    ...
    def as_int32(self):
        ...
        :rtype: SDL_array
        """
        return SDL_array(self.ptr, self.count * sizeof(self.ctype) / 4, c_uint)
    ...
    def as_ctypes(self):
        ...
        if not self._ctypes_array:
            self._ctypes_array = cast(self.ptr, POINTER(self.ctype * self.count)).contents
...
def to_ctypes(values, count, ctype):
    ...
    if isinstance(values, Array):
        if values._type_ is ctype:
            return (ref, values)
        return (
         ref, cast(values, POINTER(ctype * count)).contents)
    if type(values) == str:
        ref = create_string_buffer(values)
        return (
         ref, cast(ref, POINTER(ctype * count)).contents)
