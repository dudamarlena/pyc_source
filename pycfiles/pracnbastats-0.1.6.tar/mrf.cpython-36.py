...
# Embedded file name: /data/code/pracmln/python3/pracmln/mln/mrf.py
# Compiled at: 2019-05-17 05:25:59
# Size of source mod 2**32: 31797 bytes
import re, sys, time
from math import *
...
class MRF(object):
    ...
    def set_evidence(self, atomvalues, erase=False, cw=False):
        ...
        for key, value in dict(atomvalues).items():
            ...
            if isinstance(self.mln.logic, FuzzyLogic) and (isinstance(var, MutexVariable) or isinstance(var, SoftMutexVariable) or isinstance(var, BinaryVariable)) and value is not None and value in Interval(']0,1['):
    ...
    def apply_cw(self, *prednames):
        ...
        for i, v in enumerate(self._evidence):
            if prednames:
                if self.gndatom(i).predname not in prednames:
                    continue
                if v is None:
                    self._evidence[i] = 0
    ...
    def gndatom(self, identifier, *args):
        ...
        foo(x,y)
        """
        if args or isinstance(identifier, str):
        ...
        if not constraints:
            ...
            for req in constraints:
                if 'gndFormula' not in req:
                    ...
                    groundVars = {}
                    for varName, domName in variables.items():
            ...
            return (
             results[len(constraints):], {'steps':min(step, steps),  'fittingSteps':fittingStep,  'maxdiff':maxdiff,  'meandiff':meandiff,  'time':time.time() - t_start})
