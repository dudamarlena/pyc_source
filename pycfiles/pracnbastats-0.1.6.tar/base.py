...
# Embedded file name: /home/nyga/work/code/pracmln/python2/pracmln/mln/base.py
...
from pracmln.mln.mlnpreds import Predicate, FuzzyPredicate, SoftFunctionalPredicate, FunctionalPredicate
    ...
    def materialize(self, *dbs):
        ...
        for i, template in mln_.iterformulas():
            for variant in template.template_variants():
                ...
                f = mln__.formula(variant, weight=template.weight if isinstance(template.weight, basestring) else template.weight, fixweight=mln_.fixweights[i])
                f.idx = idx
        ...
        try:
            ...
            if line == '#group':
                ...
                if line == '#group.':
                    ...
                    parse_mln(text=mlnpath(includefilename).content, searchpaths=[p.resolve_path()] + dirs, projectpath=ifnone(p.project, projectpath, lambda x: ('/').join(p.path + [x])), logic=logic, grammar=grammar, mln=mln)
                    continue
                ...
                if isPredDecl:
                    ...
                    try:
                        ...
                        for variant in formula.expandgrouplits():
                            mln.formula(variant, weight=weight, fixweight=fixweight, unique_templvars=uniquevars)
