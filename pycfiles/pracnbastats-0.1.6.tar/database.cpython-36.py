...
# Embedded file name: /data/code/pracmln/python3/pracmln/mln/database.py
        ...
        if type(gndatom) is str:
            ...
            if isinstance(gndatom, Logic.GroundAtom):
                ...
                if isinstance(gndatom, Logic.GroundLit):
                    ...
                    if isinstance(gndatom, Logic.Lit):
    ...
    for line, l in enumerate(content.split('\n')):
        ...
        if '{' in l:
            ...
            if l.startswith('#include'):
                ...
                dbs.extend(parse_db(content=(mlnpath(includefilename).content), ignore_unknown_preds=ignore_unknown_preds, dirs=([p.resolve_path()] + dirs), projectpath=(ifnone(p.project, projectpath, lambda x: '/'.join(p.path + [x]))),
                  mln=mln))
                continue
            else:
