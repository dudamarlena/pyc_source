...
# Embedded file name: /data/code/pracmln/python3/pracmln/utils/widgets.py
...
class SyntaxHighlightingText(ScrolledText2):
    ...
    def key_release(self, key):
        if key.char in ' :[(]),"\'':
            ...
            if key.char != '' and not key.state & 4 or key.keysym in ('BackSpace',
                                                                      'Delete'):
                self.onChange()
    ...
    def colorize(self, cline):
        ...
        for i in range(len(buffer)):
            ...
            if in_quote or buffer[i:i + 2] == '//':
                ...
                if buffer[i] in self.highlighter.close_brackets:
                    if self.index(here + ' + 1 char') == cursorPos:
...
class FilePickEdit(Frame):
    ...
    def onChangeRename(self):
        if self.rename_on_edit.get() == 1:
            if not self.unmodified:
                if self.save_name.get() == self.picked_name.get():
                    self.autoRename()
        else:
    ...
    def select(self, filename, notify=True):
        ...
        if filename in self.files:
            havePMW or self.picked_name.set(filename)
        else:
    ...
    def makelist(self):
        if havePMW:
            self.list = ComboBox((self.list_frame), selectioncommand=(self.onSelChange),
...
class FilePick(Frame):
    ...
    def _makelist(self):
        if havePMW:
            self.list = ComboBox(self, selectioncommand=(self.onSelChange),
              scrolledlist_items=(self.files))
