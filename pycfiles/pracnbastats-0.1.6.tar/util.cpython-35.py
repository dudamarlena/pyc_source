...
# Embedded file name: /home/nyga/work/code/pracmln/python3/pracmln/mln/util.py
# Compiled at: 2018-04-24 04:48:32
# Size of source mod 2**32: 15174 bytes
import re, time, logging, traceback, colored
...
class Interval:
    ...
    def __init__(self, interval):
        tokens = re.findall('(\\(|\\[|\\])([-+]?\\d*\\.\\d+|\\d+),([-+]?\\d*\\.\\d+|\\d+)(\\)|\\]|\\[)', interval.strip())[0]
        if tokens[0] in ('(', ']'):
            self.left = EXC
        else:
            if tokens[0] == '[':
...
def cumsum(i, upto=None):
    if not i or upto == 0:
        return 0
    return reduce(int.__add__, i[:ifnone(upto, len(i))])
    ...
    def printSteps(self):
        for tag in sorted(list(self.tags.values()), key=lambda ta: ta.starttime):
            if tag.finished:
                ...
                print('{} is running for {} now...'.format(colorize(tag.label, (None,
                                                                                None,
                                                                                True), True), elapsed_time_str(tag.elapsedtime)))
