...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kutana/context.py
...
class Context:
    ...
    async def request(self, method, **kwargs):
        ...
        return await self.backend.perform_api_call(method, kwargs)
    ...
    async def send_message(self, target_id, message, attachments=(), **kwargs):
        ...
        for part in parts[:-1]:
            responses.append(await self.backend.perform_send(target_id, part, (), {}))

        responses.append(await self.backend.perform_send(target_id, parts[(-1)], attachments, kwargs))
        return responses
