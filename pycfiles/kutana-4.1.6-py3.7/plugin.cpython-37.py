...
from .routers import CommandsRouter, AttachmentsRouter, ListRouter, AnyMessageRouter, AnyUnprocessedMessageRouter
...
class Plugin:
    ...
    def on_commands(self, commands, group_state='*', user_state='*', priority=0):
        """
        ...
        def decorator(func):
            ...
            for command in commands:
                router.add_handler(Handler(func, group_state, user_state, priority), command)

            return func

        return decorator

    def on_attachments(self, types, group_state='*', user_state='*', priority=0):
        """
        Decorator for registering coroutine to be called when
        ...
        def decorator(func):
            ...
            for _type in types:
                router.add_handler(Handler(func, group_state, user_state, priority), _type)
    ...
    def on_any_message(self, group_state='*', user_state='*', priority=0):
        ...
        def decorator(func):
            ...
            router.add_handler(Handler(func, group_state, user_state, priority))
            return func

        return decorator

    def on_any_unprocessed_message(self, group_state='*', user_state='*', priority=0):
        """
        Decorator for registering coroutine to be called when
    ...
    def on_match(self, pattern, group_state='*', user_state='*', priority=0):
        ...
        def decorator(func):
            ...
            router.add_handler(Handler(wrapper, group_state, user_state, priority))
            return wrapper
