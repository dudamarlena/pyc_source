...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kutana/router.py
...
class Router:
    ...
    def _check(self, handler, update, ctx):
        ...
        return handler.group_state in ('*', ctx.group_state) and handler.user_state in ('*', ctx.user_state)
...
class ListRouter(Router):
    ...
    async def handle(self, update, ctx):
        ...
        for handler in self._handlers:
            if self._check(handler, update, ctx) and await handler.handle(update, ctx) != hr.SKIPPED:
                return hr.COMPLETE
