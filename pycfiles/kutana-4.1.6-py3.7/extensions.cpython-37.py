...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kutana/backends/vkontakte/extensions.py
...
class PayloadRouter(MapRouter):
    ...
    def _to_hashable(self, obj):
        if isinstance(obj, dict):
            return tuple(((k, self._to_hashable(v)) for k, v in sorted(obj.items())))
...
class VkontaktePluginExtension:
    ...
    def on_payload(self, payloads, group_state='*', user_state='*', priority=0):
        """
        ...
        def decorator(func):
            ...
            for payload in payloads:
                router.add_handler(Handler(func, group_state, user_state, priority), payload)
