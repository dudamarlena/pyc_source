...
# [GCC 8.4.0]
# Embedded file name: /home/mfi/mytools/muteria/muteria/repositoryandcode/repository_manager.py
# Compiled at: 2019-12-05 11:58:16
# Size of source mod 2**32: 26659 bytes
...
import muteria.common.mix as common_mix, muteria.common.fs as common_fs
...
class RepositoryManager(object):
    ...
    def __init__(self, repository_rootdir, repo_executables_relpaths=None, dev_test_runner_func=None, code_builder_func=None, dev_test_program_wrapper=None, source_files_to_objects=None, dev_tests_list=None, delete_created_on_revert_as_initial=False, test_branch_name=DEFAULT_TESTS_BRANCH_NAME):
        ...
        self.delete_created_on_revert_as_initial = delete_created_on_revert_as_initial
        ...
        if self.repository_rootdir is None:
            ERROR_HANDLER.error_exit('repository rootdir cannot be None', __file__)
        self.lock = threading.RLock()
        self.muteria_metadir = os.path.join(self.repository_rootdir, self.DEFAULT_MUTERIA_REPO_META_FOLDER)
        self.muteria_metadir_info_file = os.path.join(self.muteria_metadir, 'src_infos.json')
    ...
    def has_dev_tests(self):
        return self.dev_test_runner_func is not None and self.dev_tests_list is not None
    ...
    def _set_callback_basics(self, callback_object):
        if callback_object is not None:
            ...
            callback_object.set_repo_executables_relpaths(self.repo_executables_relpaths)
            callback_object.set_source_files_to_objects(self.source_files_to_objects)
            callback_object.set_dev_tests_list(self.dev_tests_list)

    def run_dev_test(self, dev_test_name, exe_path_map=None, env_vars=None, timeout=None, collected_output=None, callback_object=None):
        if self.dev_test_runner_func is None:
            ERROR_HANDLER.error_exit('dev_test_runner_func cannot be none when called', __file__)
        ...
        try:
            ...
            if pre_ret == common_mix.GlobalConstants.COMMAND_SUCCESS:
                ret = self.dev_test_runner_func(dev_test_name, self.repository_rootdir, exe_path_map=exe_path_map, env_vars=env_vars if env_vars is not None else {}, timeout=timeout, collected_output=collected_output, using_wrapper=self.get_wrapper_object() is not None)
                if callback_object is not None:
        ...
        return (
         pre_ret, ret, post_ret)

    def build_code(self, compiler=None, flags_list=None, clean_tmp=False, reconfigure=False, callback_object=None):
...
econfigure: enable reconfigure the project repository before
        ...
        if self.code_builder_func is None:
            ERROR_HANDLER.error_exit('code_builder_func cannot be none when called', __file__)
        pre_ret = common_mix.GlobalConstants.COMMAND_SUCCESS
        post_ret = common_mix.GlobalConstants.COMMAND_UNCERTAIN
        ret = common_mix.GlobalConstants.COMMAND_UNCERTAIN
        self._set_callback_basics(callback_object)
        ...
        try:
            if callback_object is not None:
                pre_ret = callback_object.before_command()
            if pre_ret == common_mix.GlobalConstants.COMMAND_SUCCESS:
                ret = self.code_builder_func(self.repository_rootdir, self.repo_executables_relpaths, compiler, flags_list, clean_tmp, reconfigure)
                if callback_object is not None:
                    callback_object.set_op_retval(ret)
                    post_ret = callback_object.after_command()
        finally:
    ...
    def custom_read_access(self, callback_object):
        ...
        try:
            if callback_object is None:
                ERROR_HANDLER.error_exit('{} {}'.format('callback object must', 'not be None in custom_read_access call'), __file__)
            post_ret = common_mix.GlobalConstants.COMMAND_UNCERTAIN
            pre_ret = callback_object.before_command()
            if pre_ret == common_mix.GlobalConstants.COMMAND_SUCCESS:
    ...
    def get_exe_path_map(self):
        ...
        for src, dest in list(self.source_files_to_objects.items()):
            src_obj_map[self.repo_abs_path(src)] = dest if dest is None else self.repo_abs_path(dest)
    ...
    def revert_src_list_files(self):
        ...
        try:
            repo = git_repo(self.repository_rootdir)
            gitobj = repo.git
            self._unlocked_revert_repository_file(self.source_files_list, gitobj=gitobj)
        finally:
            self.lock.release()
        ...
        try:
            if as_initial:
                if self.delete_created_on_revert_as_initial:
                    self._delete_testing_branch(self.repository_rootdir, self.test_branch_name)
                else:
                    self.lock.release()
                    ERROR_HANDLER.error_exit('{} {}'.format('revert_repository called with', "'delete_created_on_revert_as_initial' disabled"), __file__)
            else:
                self.revert_src_list_files()
    ...
    def _setup_repository(self):
        ERROR_HANDLER.assert_true(os.path.isdir(self.repository_rootdir), 'given repository dir is not existing: {}'.format(self.repository_rootdir), __file__)
        try:
            repo = git_repo(self.repository_rootdir)
            gitobj = repo.git
        except git_exc.InvalidGitRepositoryError:
            make_it_git = common_mix.confirm_execution('{} {} {} {}'.format('The given repository directory is not', 'a git repository, this must be a git', 'repository to proceed.\n Do you want to', 'set is as git repository?'))
            if make_it_git:
                repo = git_repo.init(self.repository_rootdir)
                gitobj = repo.git
            else:
                ERROR_HANDLER.error_exit('{} {}'.format('Must make the repository as git repository,', 'then re-run'), __file__)

        if self.delete_created_on_revert_as_initial:
            if self.test_branch_name not in self._get_branches_list(self.repository_rootdir):
                self._make_testing_branch(self.repository_rootdir, self.test_branch_name)
            gitobj.checkout(self.test_branch_name)
        src_in_prev = set()
        if os.path.isdir(self.muteria_metadir):
            prev_src_list, prev_use_branch = common_fs.loadJSON(self.muteria_metadir_info_file)
            ERROR_HANDLER.assert_true(prev_use_branch == self.delete_created_on_revert_as_initial, '{} {} {} {} {}'.format('unmatching repo backup type.', 'previously, use branch was', prev_use_branch, ', now it is', self.delete_created_on_revert_as_initial), __file__)
            src_in_prev = set(prev_src_list) & set(self.source_files_list)
            prev_src_list = set(prev_src_list) - set(self.source_files_list)
            remain_prev_src_set = {src for src in prev_src_list if os.path.isfile(self.repo_abs_path(src))}
            ...
            if len(prev_untracked_diff) > 0:
                bypass = common_mix.confirm_execution('{} {} {} {} {}'.format('the following files were previously used as', 'src files by muteria and are now untracked:', prev_untracked_diff, '\nDo you want to handle it or bypass it?', 'Choose yes to bypass: '))
                if not bypass:
                    revert_them = common_mix.confirm_execution('{} {}'.format('Do you want to automatically restore the', 'The untracked previous source and continue?'))
                    if revert_them:
                        for src in prev_untracked_diff:
                            self.revert_repository_file(src, gitobj=gitobj)

            else:
                ERROR_HANDLER.error_exit('Handle it manually and restart the execution')
            if set(prev_src_list) != set(self.source_files_list):
                common_fs.dumpJSON([self.source_files_list,
                 self.delete_created_on_revert_as_initial], self.muteria_metadir_info_file)
        else:
        ...
        if len(untracked_diff_src_in_prev) > 0:
            if common_mix.confirm_execution('{} {} {} {} {}'.format('The following source', 'files of interest are untracked', 'and will be reverted (previous execution unfinished):', src_in_prev, 'do you want to revert them?')):
                for src in src_in_prev:
                    self.revert_repository_file(src, gitobj=gitobj)

        else:
            ERROR_HANDLER.error_exit('{} {}'.format('Handle untracked source files manually', 'then restart the execution'))
        untracked_diff_files = self._get_untracked_and_diffed(repo)
        untracked_diff_src_files = set(self.source_files_list) & untracked_diff_files
        if len(untracked_diff_src_files) > 0:
            if common_mix.confirm_execution('{} {} {} {}'.format('The following source', 'files of interest are untracked:', untracked_diff_src_files, 'do you want to track them?')):
