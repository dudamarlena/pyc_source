...
# [GCC 8.4.0]
# Embedded file name: /home/mfi/mytools/muteria/muteria/common/matrices.py
# Compiled at: 2020-02-26 05:06:55
# Size of source mod 2**32: 37013 bytes
from __future__ import print_function
import os, sys, itertools, copy, pandas as pd, muteria.common.mix as common_mix, muteria.common.fs as common_fs
...
class RawExecutionMatrix(object):
    ...
    def __init__(self, filename=None, key_column_name=DEFAULT_KEY_COLUMN_NAME, non_key_col_list=None, active_cell_default_val=[1], inactive_cell_vals=[
 0], uncertain_cell_default_val=[-1], is_active_cell_func=lambda x: x > 0, is_inactive_cell_func=lambda x: x == 0, is_uncertain_cell_func=lambda x: x < 0, cell_dtype=int):
        ...
        ERROR_HANDLER.assert_true(self.is_active_cell_func(self.active_cell_default_val[0]), '', __file__)
        ERROR_HANDLER.assert_true(self.is_inactive_cell_func(self.inactive_cell_vals[0]), '', __file__)
        ERROR_HANDLER.assert_true(self.is_uncertain_cell_func(self.uncertain_cell_default_val[0]), '', __file__)
        for v in self.inactive_cell_vals:
            ERROR_HANDLER.assert_true(not self.is_active_cell_func(v), '', __file__)

        for v in self.inactive_cell_vals:
            ERROR_HANDLER.assert_true(not self.is_uncertain_cell_func(v), '', __file__)

        if self.filename is None or not os.path.isfile(self.filename):
            ERROR_HANDLER.assert_true(self.non_key_col_list is not None, "Must specify 'non_key_col_list' when " + 'filename inexistant. filename is ' + str(self.filename), __file__)
            ...
            ERROR_HANDLER.assert_true(self.key_column_name == list(self.dataframe)[0], 'key_column name missing or not first column in dataframe', __file__)
            if self.non_key_col_list is None:
                self.non_key_col_list = list(self.dataframe)[1:]
            else:
                ERROR_HANDLER.assert_true(self.non_key_col_list == list(self.dataframe)[1:], 'non key mismatch', __file__)
    ...
    def add_row_by_key(self, key, values, serialize=True):
        ...
        2
        """
        ERROR_HANDLER.assert_true(key not in self.keys_set, "adding an existing key: '" + str(key) + "', to matrix: " + str(self.filename), __file__)
        ...
        if type(values) in (list, tuple):
            ...
            if type(values) == dict:
                ERROR_HANDLER.assert_true(self.key_column_name not in values, 'key column name in values', __file__)
    ...
    def delete_rows_by_key(self, key_list, serialize=True):
        ...
        True
        """
        self.dataframe = self.dataframe.loc[(~self.dataframe[self.key_column_name].isin(set(key_list)))]
    ...
    def extract_by_rowkey(self, row_key_list, out_filename=None):
        ...
        True
        """
        ERROR_HANDLER.assert_true(len(row_key_list) > 0, 'key list should not be empty', __file__)
        ERROR_HANDLER.assert_true(len(set(row_key_list) - self.keys_set) == 0, 'invalid row key passed to extract row', __file__)
        ret = self.get_a_deepcopy(new_filename=out_filename, serialize=True)
        ret.dataframe = ret.dataframe.loc[ret.dataframe[ret.key_column_name].isin(row_key_list)]
    ...
    def extract_by_column(self, non_key_col_list, out_filename=None):
        ...
        True
        """
        ERROR_HANDLER.assert_true(len(non_key_col_list) > 0, 'col list should not be empty', __file__)
        ERROR_HANDLER.assert_true(len(set(non_key_col_list) - set(self.non_key_col_list)) == 0, 'invalid column passed to extract col', __file__)
    ...
    def query_active_columns_of_rows(self, row_key_list=None):
        ...
        if len(row_key_list) > 0:
            ...
            for _, row in small_df.iterrows():
                result[row[self.key_column_name]] = [x for x in self.non_key_col_list if self.is_active_cell_func(row[x])]
    ...
    def query_active_rows_of_columns(self, non_key_col_list=None):
        ...
        if len(non_key_col_list) > 0:
            ...
            for col in non_key_col_list:
                result[col] = list(small_df.loc[small_df[col].apply(self.is_active_cell_func)][self.key_column_name])
    ...
    def query_inactive_columns_of_rows(self, row_key_list=None):
        ...
        if len(row_key_list) > 0:
            small_df = self.extract_by_rowkey(row_key_list).dataframe
            for _, row in small_df.iterrows():
                result[row[self.key_column_name]] = [x for x in self.non_key_col_list if self.is_inactive_cell_func(row[x])]
    ...
    def query_inactive_rows_of_columns(self, non_key_col_list=None):
        ...
        if len(non_key_col_list) > 0:
            small_df = self.extract_by_column(non_key_col_list).dataframe
            for col in non_key_col_list:
                result[col] = list(small_df.loc[small_df[col].apply(self.is_inactive_cell_func)][self.key_column_name])
    ...
    def query_uncertain_columns_of_rows(self, row_key_list=None):
        ...
        if len(row_key_list) > 0:
            small_df = self.extract_by_rowkey(row_key_list).dataframe
            for _, row in small_df.iterrows():
                result[row[self.key_column_name]] = [x for x in self.non_key_col_list if self.is_uncertain_cell_func(row[x])]
    ...
    def query_uncertain_rows_of_columns(self, non_key_col_list=None):
        ...
        if len(non_key_col_list) > 0:
            small_df = self.extract_by_column(non_key_col_list).dataframe
            for col in non_key_col_list:
                result[col] = list(small_df.loc[small_df[col].apply(self.is_uncertain_cell_func)][self.key_column_name])
    ...
    def update_cells(self, key, values):
        ...
        True
        """
        key_pos = self.dataframe.index[(self.dataframe[self.get_key_colname()] == key)].tolist()
        ERROR_HANDLER.assert_true(len(key_pos) == 1)
        key_pos = key_pos[0]
        for col, cval in list(values.items()):
            self.dataframe.at[(key_pos, col)] = cval

    def update_with_other_matrix(self, other_matrix, override_existing=False, allow_missing=False, ask_confirmation_with_exist_missing=False, serialize=False):
        ...
        col_existing = set(self.get_nonkey_colname_list()) & set(other_matrix.get_nonkey_colname_list())
        if len(row_existing) > 0 and len(col_existing) > 0 and ask_confirmation_with_exist_missing and override_existing:
            override_existing = common_mix.confirm_execution('Some cells are existing, do you confirm their override?')
            ERROR_HANDLER.assert_true(override_existing, 'Override_existing not set but there is overlap', __file__)
        if len(row_existing) < len(other_matrix.get_keys()) and (len(col_existing) != len(self.get_nonkey_colname_list()) or len(col_existing) != len(other_matrix.get_nonkey_colname_list())):
            if ask_confirmation_with_exist_missing and allow_missing:
                allow_missing = common_mix.confirm_execution('Some values are missing, do you confirm their presence?')
            ERROR_HANDLER.assert_true(allow_missing, 'allow_missing disable but there are missing', __file__)
        col_to_add = set(other_matrix.get_nonkey_colname_list()) - set(self.get_nonkey_colname_list())
        for col in col_to_add:
            self.non_key_col_list.append(col)
            self.dataframe[col] = [self.getUncertainCellDefaultVal()] * len(self.get_keys())

        extra_cols = set(self.get_nonkey_colname_list()) - set(other_matrix.get_nonkey_colname_list())
        missing_extracol_vals = {e_c:self.getUncertainCellDefaultVal() for e_c in extra_cols}
...
class OutputLogData(object):
    ...
    TIMEDOUT = 'TIMEDOUT'
    Dat_Keys = {
     OUTLOG_LEN, OUTLOG_HASH, RETURN_CODE, TIMEDOUT}
    UNCERTAIN_TEST_OUTLOGDATA = {OUTLOG_LEN: common_mix.GlobalConstants.COMMAND_UNCERTAIN, 
    ...
    def outlogdata_equiv(cls, outlogdata1, outlogdata2):
        if outlogdata1 == cls.UNCERTAIN_TEST_OUTLOGDATA or outlogdata2 == cls.UNCERTAIN_TEST_OUTLOGDATA:
            return
        if outlogdata1 != outlogdata2 and not (outlogdata1[cls.TIMEDOUT] and outlogdata2[cls.TIMEDOUT]):
            return False
        return True
    ...
    def add_data(self, data_dict, check_all=True, override_existing=False, ask_confirmation_with_exist_missing=False, serialize=False):
        if check_all:
            ERROR_HANDLER.assert_true(type(data_dict) == dict and len(data_dict) > 0, 'expecting a non empty dict', __file__)
            for o, o_obj in data_dict.items():
                if len(o_obj) == 0:
                    pass
                else:
                    ERROR_HANDLER.assert_true(type(o_obj) == dict, 'expecting dict for value data: objective is ' + o, __file__)
                    for t, t_obj in o_obj.items():
                        ERROR_HANDLER.assert_true(set(t_obj) == self.Dat_Keys, 'Invalid data for o ' + o + ' and t ' + t, __file__)
        ...
        if not override_existing:
            for objective in intersect_objective:
                ERROR_HANDLER.assert_true(len(set(self.data[objective]) & set(data_dict[objective])) == 0, 'Override_existing not set but there is overlap', __file__)

        for objective in intersect_objective:
            if ask_confirmation_with_exist_missing and len(set(self.data[objective]) & set(data_dict[objective])) != 0:
                ERROR_HANDLER.assert_true(common_mix.confirm_execution('Some values are existing, do you confirm their override?'), 'Existing values were not overriden', __file__)
    ...
    def update_with_other(self, other_execoutput, override_existing=False, ask_confirmation_with_exist_missing=False, serialize=False):
        self.add_data(other_execoutput.data, check_all=False, override_existing=override_existing, ask_confirmation_with_exist_missing=ask_confirmation_with_exist_missing, serialize=serialize)
