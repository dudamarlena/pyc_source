...
# [GCC 8.4.0]
# Embedded file name: /home/mfi/mytools/muteria/muteria/drivers/optimizers/criteriatestexecution/tools/strongmutation_by_weakmutation.py
# Compiled at: 2019-11-07 05:53:41
...
from muteria.drivers.optimizers.criteriatestexecution.base_criteria_test_execution_optimizer import BaseCriteriaTestExecutionOptimizer
...
class CriteriaTestExecutionOptimizer(BaseCriteriaTestExecutionOptimizer):
    ...
    def reset(self, toolalias, test_objective_list, test_list, **kwargs):
        ...
        self.pointer = 0
        self.test_objective_to_test_execution_optimizer = {to:TestExecutionOptimizer(self.config, self.explorer, disable_reset=True) for to in self.test_objective_ordered_list}
        opt_by_criterion = self._get_optimizing_criterion()
        matrix_file = self.explorer.get_existing_file_pathname(explorer.TMP_CRITERIA_MATRIX[opt_by_criterion])
        test_list_per_test_objective = self._get_test_objective_tests_from_matrix(matrix_file=matrix_file)
        for to, teo in self.test_objective_to_test_execution_optimizer.items():
            alias_to = DriversUtils.make_meta_element(to, toolalias)
            ERROR_HANDLER.assert_true(alias_to in test_list_per_test_objective, 'Bug: test objective missing(' + str(to) + ')')
            teo.reset(None, test_list_per_test_objective[alias_to], disable_reset=True)
