...
# [GCC 8.4.0]
# Embedded file name: /home/mfi/mytools/muteria/muteria/drivers/criteria/tools_by_languages/c/gcov/gcov.py
# Compiled at: 2019-12-05 12:06:26
# Size of source mod 2**32: 16763 bytes
""" TODO: call coverage
"""
...
class CriteriaToolGCov(BaseCriteriaTool):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.gc_files_dir = os.path.join(self.criteria_working_dir, 'gcno_gcda')
        self.gcov_gdb_wrapper_template = os.path.join(os.path.dirname(__file__), 'gcov-gdb_call_wrapper.sh.in')
        self.gcov_gdb_wrapper_sh = os.path.join(self.instrumented_code_storage_dir, 'tmp_gcov_gdb_wrapper.sh')
    ...
    class InstrumentCallbackObject(DefaultCallbackObject):
        ...
        def after_command(self):
            ...
            for _, obj in list(self.source_files_to_objects.items()):
                ...
                ERROR_HANDLER.assert_true(os.path.isfile(abs_gcno), 'gcno file missing after build', __file__)
    ...
    def get_instrumented_executable_paths_map(self, enabled_criteria):
        ...
        for name in obj:
            obj[name] = os.path.join(self.instrumented_code_storage_dir, obj[name])

        if using_gdb_wrapper:
            with open(self.gcov_gdb_wrapper_template) as (f):
                template_str = f.read()
            single_exe = obj[list(obj)[0]]
            template_str = template_str.replace('MUTERIA_GCOV_PROGRAMEXE_PATHNAME', single_exe)
    ...
    def _collect_temporary_coverage_data(self, criteria_name_list, test_execution_verdict, used_environment_vars, result_dir_tmp, testcase):
        ...
        cov2flags = {TestCriteria.STATEMENT_COVERAGE: [], 
        ...
        if len(gcda_files) > 0:
            ...
            r, _, _ = DriversUtils.execute_and_get_retcode_out_err(prog=prog, args_list=args_list, out_on=False, err_on=False)
            os.chdir(cwd)
            if r != 0:
                ERROR_HANDLER.error_exit('Program {} {}.'.format(prog, 'error collecting coverage is problematic'), __file__)
            for gcda_f in gcda_files:
                os.remove(gcda_f)

        common_fs.dumpJSON(self._get_gcov_list(), os.path.join(result_dir_tmp, self.gcov_files_list_filename))
    ...
    def _extract_coverage_data_of_a_test(self, enabled_criteria, test_execution_verdict, result_dir_tmp):
        ...
        """
        gcov_list = common_fs.loadJSON(os.path.join(result_dir_tmp, self.gcov_files_list_filename))
        ...
        _, src_map = self.code_builds_factory.repository_manager.get_relative_exe_path_map()
                    ...
                    if len(col_split) > 2 and col_split[1] == '0':
                        if col_split[2] == 'Source':
                            ...
                            if line.startswith('function '):
                                parts = line.split()
                                ident = DriversUtils.make_meta_element(parts[1], src_file)
                                func_cov[ident] = int(parts[3])
                            else:
                                if line.startswith('branch '):
                                    parts = line.split()
                                    ident = DriversUtils.make_meta_element(parts[1], last_line)
                                    if parts[2:4] == ['never', 'executed']:
                                        branch_cov[ident] = 0
                                    else:
                                        branch_cov[ident] = int(parts[3])
                                elif len(col_split) > 2 and re.match('^\\d+$', col_split[1]):
                                    if col_split[0] == '-':
                                        pass
                                    else:
                                        last_line = DriversUtils.make_meta_element(col_split[1], src_file)
                                        if col_split[0] in ('#####', '====='):
                                            exec_count = 0
                                        else:
                                            exec_count = int(re.findall('^\\d+', col_split[0])[0])
    ...
    def _do_instrument_code(self, exe_path_map, code_builds_factory, enabled_criteria, parallel_count=1):
        ...
        os.mkdir(self.gc_files_dir, mode=511)
        prog = 'gcc'
        if self.custom_binary_dir is not None:
            prog = os.path.join(self.custom_binary_dir, prog)
            ERROR_HANDLER.assert_true(os.path.isfile(prog), 'The tool {} is missing from the specified dir {}'.format(os.path.basename(prog), self.custom_binary_dir), __file__)
        ...
        ret, out, err = DriversUtils.execute_and_get_retcode_out_err(prog, ['-dumpversion'])
        ERROR_HANDLER.assert_true(ret == 0, "'gcc -dumpversion' failed'")
        if int(out.split('.')[0]) >= 6 and (int(out.split('.')[0]) > 6 or int(out.split('.')[1]) > 5):
            additionals += ['-fkeep-static-functions']
        flags += additionals
        rel_rel_path_map = {}
        rel_abs_path_map = {}
        exes, _ = code_builds_factory.repository_manager.get_relative_exe_path_map()
        ERROR_HANDLER.assert_true(len(exes) == 1, 'Only single exe supported in GCOV', __file__)
        for exe in exes:
            filename = os.path.basename(exe)
            rel_rel_path_map[exe] = filename
            rel_abs_path_map[exe] = os.path.join(self.instrumented_code_storage_dir, filename)
        ...
        pre_ret, ret, post_ret = code_builds_factory.transform_src_into_dest(src_fmt=CodeFormats.C_SOURCE, dest_fmt=CodeFormats.NATIVE_CODE, src_dest_files_paths_map=None, compiler=prog, flags_list=flags, clean_tmp=True, reconfigure=True, callback_object=self.instrument_callback_obj)
        if ret == common_mix.GlobalConstants.COMMAND_FAILURE:
            ERROR_HANDLER.error_exit('Program {} {}.'.format(prog, 'built problematic'), __file__)
