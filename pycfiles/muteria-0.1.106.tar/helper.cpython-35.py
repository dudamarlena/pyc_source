...
# [GCC 8.4.0]
# Embedded file name: /home/mfi/mytools/muteria/muteria/configmanager/helper.py
# Compiled at: 2019-12-05 09:47:28
# Size of source mod 2**32: 16433 bytes
"""
...
import os, sys, argparse, logging, importlib, copy, muteria.common.mix as common_mix, muteria.drivers.criteria as criteria, muteria.drivers.testgeneration as testgeneration, muteria.drivers.optimizers.criteriatestexecution.optimizerdefs as crit_opt_module, muteria.configmanager.configurations as configurations
...
class ConfigurationHelper(object):
    ...
    def _get_default_raw_conf(cls, language):
        ...
        com_default_conf = cls._load_raw_conf_from_file(module_str='.'.join([config_default, 'common_defaults']), info='common config module', must_exist=True)
        ERROR_HANDLER.assert_true(com_default_conf is not None, 'invalid for comm', __file__)
        lang_default_conf = cls._load_raw_conf_from_file(module_str='.'.join([config_default, 'languages', language]), info='language ' + language + ' config module', must_exist=False)
        ...
        ERROR_HANDLER.assert_true(len(useless) + len(c_tmp) == len(com_conf_dict), 'Some Configs params not in common default', __file__)
        for u in useless:
            del com_conf_dict[u]

        res = cls._get_update_left_with_right_raw_conf(com_conf_dict, lang_conf_dict, same_key=False)
        return res
    ...
    def _load_raw_conf_from_file(module_str, info='module', must_exist=True):
        ...
        try:
            ...
            if must_exist:
                ERROR_HANDLER.error_exit('{} {} {}. \n {}.'.format('Failed to load', info, module_str, str(ie)), __file__)
        except SyntaxError as se:
            ERROR_HANDLER.error_exit('{} {} {}. \n {}.'.format('Syntax error in', info, module_str, str(se)), __file__)
    ...
    def _get_update_rawconf_with_file(cls, raw_conf, raw_conf_filename):
        ...
        """
        path, filename = os.path.split(os.path.normpath(os.path.abspath(raw_conf_filename)))
        if path:
            sys.path.insert(0, path)
        ERROR_HANDLER.assert_true(filename.endswith('.py'), '{}{}{}'.format('invalid conf file (', filename, '). must be python source file'), __file__)
        mod_name = filename[:-len('.py')]
        fconf = cls._load_raw_conf_from_file(mod_name, info='conf file', must_exist=True)
        if path:
        ...
        res = cls._get_update_left_with_right_raw_conf(raw_conf, fconf_dict, same_key=False)
        return res
    ...
    def _get_update_left_with_right_raw_conf(left, right, same_key=True):
        ...
        """
        if same_key:
            ERROR_HANDLER.assert_true(set(left) == set(right), 'different keys', __file__)
        res = dict(left)
    ...
    def _make_conf_class_from_dict(cls, dict_obj):
        ...
        if set(dict_obj) != set(cls._get_object_params_vals_as_dict(CompleteConfiguration)):
            in_obj_only = set(dict_obj) - set(cls._get_object_params_vals_as_dict(CompleteConfiguration))
            final_templ_only = set(cls._get_object_params_vals_as_dict(CompleteConfiguration)) - set(dict_obj)
            ERROR_HANDLER.error_exit('config missmatch: {} {}. {} {}'.format('in_obj_only', in_obj_only, 'final template only', final_templ_only), __file__)
    ...
    def _make_tool_conf_from_raw(raw_dict_conf, tool_type_enum, default_tool_type, target_tool_class):
        ...
        if ttype in raw_dict_conf:
            if raw_dict_conf[ttype]:
                ERROR_HANDLER.assert_true(hasattr(tool_type_enum, raw_dict_conf[ttype]), 'Invalid test tool type: ' + raw_dict_conf[ttype], __file__)
        ...
        if c_on in raw_dict_conf:
            if raw_dict_conf[c_on]:
                raw_dict_conf[c_on] = [getattr(criteria.TestCriteria, c) for c in raw_dict_conf[c_on]]
        else:
            raw_dict_conf[c_on] = None
        if tuc in raw_dict_conf:
            if raw_dict_conf[tuc]:
                raw_dict_conf[tuc] = configurations.ToolUserCustom(**raw_dict_conf[tuc])
        else:
    ...
    def get_extend_raw_conf(cls, raw_conf, language):
        ...
        """
        def_conf = cls._get_default_raw_conf(language)
        res = cls._get_update_left_with_right_raw_conf(def_conf, raw_conf, same_key=False)
        return res
    ...
    def get_finalconf_from_rawconf(cls, raw_conf):
        ...
        if conf.ENABLED_CRITERIA is None:
            conf.ENABLED_CRITERIA = copy.deepcopy(conf.CRITERIA_TOOLS_CONFIGS_BY_CRITERIA)
        tmp = []
        for c in conf.ENABLED_CRITERIA:
            if not isinstance(c, criteria.TestCriteria):
                ERROR_HANDLER.assert_true(criteria.TestCriteria.has_element_named(c), 'invalid test criterion: ' + c)
        ...
        for c in conf.CRITERIA_WITH_OUTPUT_SUMMARY:
            if not isinstance(c, criteria.TestCriteria):
                ERROR_HANDLER.assert_true(criteria.TestCriteria.has_element_named(c), 'invalid test criterion in out sum: ' + c, __file__)
                c = criteria.TestCriteria[c]
        ...
        for pos, group in enumerate(conf.CRITERIA_SEQUENCE):
            ...
            for c in group:
                if not isinstance(c, criteria.TestCriteria):
                    ERROR_HANDLER.assert_true(criteria.TestCriteria.has_element_named(c), 'invalid test criterion in seq: ' + c, __file__)
                    c = criteria.TestCriteria[c]
                if c not in conf.ENABLED_CRITERIA:
                    pass
                else:
        ...
        if conf.CRITERIA_REQUIRING_OUTDIFF_WITH_PROGRAM is None:
            conf.CRITERIA_REQUIRING_OUTDIFF_WITH_PROGRAM = copy.deepcopy(criteria.CRITERIA_REQUIRING_OUTDIFF_WITH_PROGRAM)
        tmp = []
        for c in conf.CRITERIA_REQUIRING_OUTDIFF_WITH_PROGRAM:
            if not isinstance(c, criteria.TestCriteria):
                ERROR_HANDLER.assert_true(criteria.TestCriteria.has_element_named(c), 'invalid test criterion in outdiff: ' + c, __file__)
                c = criteria.TestCriteria[c]
            if c not in conf.ENABLED_CRITERIA:
                pass
            else:
        ...
        for c, sel_tech in conf.CRITERIA_ELEM_SELECTIONS.items():
            if not isinstance(c, criteria.TestCriteria):
                ERROR_HANDLER.assert_true(criteria.TestCriteria.has_element_named(c), 'invalid test criterion in crit elem selection: ' + c, __file__)
                c = criteria.TestCriteria[c]
            if c not in conf.ENABLED_CRITERIA:
                continue
            tmp[c] = sel_tech
        ...
        for tc in conf.TESTCASE_TOOLS_CONFIGS:
            if not isinstance(tc, configurations.TestcaseToolsConfig):
                tc = cls._make_tool_conf_from_raw(tc, testgeneration.TestToolType, testgeneration.TEST_TOOL_TYPES_SCHEDULING[0][0], configurations.TestcaseToolsConfig)
            tmp.append(tc)

        conf.TESTCASE_TOOLS_CONFIGS = tmp
        tmp = {}
        for crit, cc_list in list(conf.CRITERIA_TOOLS_CONFIGS_BY_CRITERIA.items()):
            for cc in cc_list:
                if not isinstance(cc, configurations.CriteriaToolsConfig):
                    cc = cls._make_tool_conf_from_raw(cc, criteria.CriteriaToolType, criteria.CRITERIA_TOOL_TYPES_SCHEDULING[0][0], configurations.CriteriaToolsConfig)
        ...
        for c, opt in conf.CRITERIA_EXECUTION_OPTIMIZERS.items():
            if not isinstance(c, criteria.TestCriteria):
                ERROR_HANDLER.assert_true(criteria.TestCriteria.has_element_named(c), 'invalid test criterion in opt: ' + c, __file__)
                c = criteria.TestCriteria[c]
            if c not in conf.ENABLED_CRITERIA:
                pass
            else:
                if not isinstance(opt, crit_opt_module.CriteriaOptimizers):
                    ERROR_HANDLER.assert_true(crit_opt_module.CriteriaOptimizers.has_element_named(opt), 'Invalid criterion Optimizer: ' + opt)
                    ERROR_HANDLER.assert_true(crit_opt_module.CriteriaOptimizers.has_element_named(opt), 'invalid test criterion: ' + opt)
                    opt = crit_opt_module.CriteriaOptimizers[opt]
                ERROR_HANDLER.assert_true(crit_opt_module.check_is_right_optimizer(c, opt), 'Wrong optimizer for test criterion')
            tmp[c] = opt
        ...
        for ct in conf.RE_EXECUTE_FROM_CHECKPOINT_META_TASKS:
            if not isinstance(ct, checkpoint_tasks.Tasks):
                ERROR_HANDLER.assert_true(checkpoint_tasks.Tasks.has_element_named(ct), 'invalid checkpoint task: ' + ct)
