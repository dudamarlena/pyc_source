...
# [GCC 8.4.0]
# Embedded file name: /home/mfi/mytools/muteria/muteria/drivers/checkpoint_handler.py
# Compiled at: 2019-09-19 05:11:43
# Size of source mod 2**32: 3004 bytes
import logging, muteria.common.mix as common_mix
...
class CheckPointHandler(object):
    ...
    def get_current_data(self):
        if self.current_data is None:
            self.current_data = self.used_checkpointer.load_checkpoint_or_start()
            if self.current_data is not None:
                if type(self.current_data) != dict:
                    ERROR_HANDLER.error_exit('Invalid checkpoint data. Must be a dict', __file__)
                if set(self.current_data) != {self.FUNC_NAME_KEY,
                 self.TASK_ID_KEY, self.TOOLS_KEY,
                 self.OPT_PAYLOAD_KEY}:
                    ERROR_HANDLER.error_exit('Problem with checkpoint data dict key', __file__)
    ...
    def is_to_execute(self, func_name, taskid, tool=None):
        ...
        if func_name != self.current_data[self.FUNC_NAME_KEY]:
            return True
        if taskid >= self.current_data[self.TASK_ID_KEY] and (taskid > self.current_data[self.TASK_ID_KEY] or tool not in self.current_data[self.TOOLS_KEY]):
            return True
        return False

    def do_checkpoint(self, func_name, taskid, tool=None, opt_payload=None):
        if self.current_data is None:
            self.current_data = {self.FUNC_NAME_KEY: None, 
             self.TASK_ID_KEY: -1, 
             self.TOOLS_KEY: [], 
