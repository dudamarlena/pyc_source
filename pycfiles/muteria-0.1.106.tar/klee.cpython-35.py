...
# [GCC 8.4.0]
# Embedded file name: /home/mfi/mytools/muteria/muteria/drivers/testgeneration/tools_by_languages/c/klee/klee.py
# Compiled at: 2019-10-09 05:07:28
# Size of source mod 2**32: 11005 bytes
from __future__ import print_function
import os, sys, glob, shutil, logging, re, muteria.common.fs as common_fs, muteria.common.mix as common_mix
...
class TestcasesToolKlee(BaseTestcaseTool):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.test_details_file = os.path.join(self.tests_working_dir, 'test_details.json')
        self.klee_used_tmp_build_dir = os.path.join(self.tests_working_dir, 'klee_used_tmp_build_dir')
    ...
    def _execute_a_test(self, testcase, exe_path_map, env_vars, callback_object=None, timeout=None, collect_output=False):
        ...
        ERROR_HANDLER.assert_true(len(exe_path_map) == 1, 'support a single exe for now', __file__)
        ERROR_HANDLER.assert_true(callback_object is None, 'TODO: handle callback_obj', __file__)
        ...
        if remote_exe != self.repo_exe_to_local_to_remote[repo_exe][local_exe]:
            if remote_exe == repo_exe:
                self.code_builds_factory.set_repo_to_build_default(also_copy_to_map={repo_exe: local_exe})
            else:
    ...
    def _remove_output_noise(self, out):
        try:
            ...
            self.grep_regex = re.compile('(' + '|'.join([
             ...
             '^warning: check_file .*: .* ' + 'mismatch: [0-9]+ [vV][sS] [0-9]+$' + ')']))
            self.sed_regex1 = re.compile(' \\([0-9]+\\s+seconds\\)')
            self.sed_regex2 = re.compile('RUNNING GDB: /usr/bin/gdb --pid [0-9]+ -q --batch')
        ...
        res = self.sed_regex2.sub('RUNNING GDB: /usr/bin/gdb --pid PID -q --batch', res)
        return res
    ...
    def _do_generate_tests(self, exe_path_map, outputdir, code_builds_factory, max_time=None):
        ...
        pre_ret, ret, post_ret = code_builds_factory.transform_src_into_dest(src_fmt=CodeFormats.C_SOURCE, dest_fmt=CodeFormats.LLVM_BITCODE, src_dest_files_paths_map=rel_path_map, compiler=back_llvm_compiler, clean_tmp=True, reconfigure=True)
        if ret == common_mix.GlobalConstants.TEST_EXECUTION_ERROR:
            ERROR_HANDLER.error_exit('Program {}.'.format('LLVM built problematic'), __file__)
        rel2bitcode = {}
