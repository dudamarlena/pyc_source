...
# [GCC 8.4.0]
# Embedded file name: /home/mfi/mytools/muteria/muteria/controller/explorer.py
# Compiled at: 2019-12-11 09:57:17
# Size of source mod 2**32: 13085 bytes
from __future__ import print_function
import os, logging, shutil, muteria.common.mix as common_mix, muteria.common.fs as common_fs
...
for criterion in TestCriteria:
    PARTIAL_TMP_CRITERIA_MATRIX[criterion] = 'partial_tmp_' + criterion.get_str() + '.csv'
...
for criterion in TestCriteria:
    TMP_CRITERIA_EXECUTION_OUTPUT[criterion] = 'tmp_' + criterion.get_str() + '_output.json'

PARTIAL_TMP_CRITERIA_EXECUTION_OUTPUT = {}
for criterion in TestCriteria:
    PARTIAL_TMP_CRITERIA_EXECUTION_OUTPUT[criterion] = 'partial_tmp_' + criterion.get_str() + '_output.json'
...
def get_outputdir_structure_by_filesdirs():
    ...
    """
    TopExecutionDir = {CRITERIA_WORKDIR: [CRITERIA_WORKDIR], 
     TESTSCASES_WORKDIR: [TESTSCASES_WORKDIR], 
    ...
    TopExecutionDir[SAVED_CONF] = TopExecutionDir[CTRL_CHECKPOINT_DIR] + [
     SAVED_CONF]
    TopExecutionDir[EXECUTION_STATE] = TopExecutionDir[CTRL_CHECKPOINT_DIR] + [
     EXECUTION_STATE]
    TopExecutionDir[EXECUTION_STATE_BAKUP] = TopExecutionDir[CTRL_CHECKPOINT_DIR] + [
    ...
    TopExecutionDir[MAIN_LOG_FILE] = TopExecutionDir[CTRL_LOGS_DIR] + [
     MAIN_LOG_FILE]
    TopExecutionDir[TEST_PASS_FAIL_MATRIX] = TopExecutionDir[RESULTS_MATRICES_DIR] + [TEST_PASS_FAIL_MATRIX]
    TopExecutionDir[TMP_TEST_PASS_FAIL_MATRIX] = TopExecutionDir[RESULTS_MATRICES_DIR] + [
     TMP_TEST_PASS_FAIL_MATRIX]
    TopExecutionDir[PARTIAL_TMP_TEST_PASS_FAIL_MATRIX] = TopExecutionDir[RESULTS_MATRICES_DIR] + [
     PARTIAL_TMP_TEST_PASS_FAIL_MATRIX]
    for criterion in TestCriteria:
        TopExecutionDir[CRITERIA_MATRIX[criterion]] = TopExecutionDir[RESULTS_MATRICES_DIR] + [
         CRITERIA_MATRIX[criterion]]
        TopExecutionDir[TMP_CRITERIA_MATRIX[criterion]] = TopExecutionDir[EXECUTION_TMP_DIR] + [
         TMP_CRITERIA_MATRIX[criterion]]
        TopExecutionDir[PARTIAL_TMP_CRITERIA_MATRIX[criterion]] = TopExecutionDir[EXECUTION_TMP_DIR] + [
         PARTIAL_TMP_CRITERIA_MATRIX[criterion]]

    TopExecutionDir[PROGRAM_TESTEXECUTION_OUTPUT] = TopExecutionDir[RESULTS_TESTEXECUTION_OUTPUTS_DIR] + [
     PROGRAM_TESTEXECUTION_OUTPUT]
    TopExecutionDir[TMP_PROGRAM_TESTEXECUTION_OUTPUT] = TopExecutionDir[RESULTS_TESTEXECUTION_OUTPUTS_DIR] + [
     TMP_PROGRAM_TESTEXECUTION_OUTPUT]
    TopExecutionDir[PARTIAL_TMP_PROGRAM_TESTEXECUTION_OUTPUT] = TopExecutionDir[RESULTS_TESTEXECUTION_OUTPUTS_DIR] + [
     PARTIAL_TMP_PROGRAM_TESTEXECUTION_OUTPUT]
    for criterion in TestCriteria:
        TopExecutionDir[CRITERIA_EXECUTION_OUTPUT[criterion]] = TopExecutionDir[RESULTS_TESTEXECUTION_OUTPUTS_DIR] + [
         CRITERIA_EXECUTION_OUTPUT[criterion]]
        TopExecutionDir[TMP_CRITERIA_EXECUTION_OUTPUT[criterion]] = TopExecutionDir[EXECUTION_TMP_DIR] + [
         TMP_CRITERIA_EXECUTION_OUTPUT[criterion]]
        TopExecutionDir[PARTIAL_TMP_CRITERIA_EXECUTION_OUTPUT[criterion]] = TopExecutionDir[EXECUTION_TMP_DIR] + [
         PARTIAL_TMP_CRITERIA_EXECUTION_OUTPUT[criterion]]

    TopExecutionDir[TMP_SELECTED_TESTS_LIST] = TopExecutionDir[EXECUTION_TMP_DIR] + [TMP_SELECTED_TESTS_LIST]
    TopExecutionDir[TMP_SELECTED_CRITERIA_OBJECTIVES_LIST] = TopExecutionDir[EXECUTION_TMP_DIR] + [
...
class Explorer(common_fs.FileDirStructureHandling):
    ...
    def __init__(self, outdir):
        common_fs.FileDirStructureHandling.__init__(self, top_dir=os.path.abspath(outdir), top_dir_key=TOP_OUTPUT_DIR_KEY, file_dir_dict=get_outputdir_structure_by_filesdirs())
...
class TopExplorer(object):
    ...
    def __init__(self, root_outdir):
        ...
        self.timeline_outdirs.append(os.path.join(self.root_outdir, self.LATEST_NAME))
        tmp_outdirs = []
        ...
        for ind in range(len(tmp_histories)):
            ...
            if tmp_histories[ind] != ind + 1:
                if not fix_missing_history:
                    fix_missing_history = common_mix.confirm_execution('Missing history: {}. {}'.format(ind + 1, 'Do you want to automatically fix it?'))
                    ERROR_HANDLER.assert_true(fix_missing_history, 'No history should be missing {}'.format('when executing. Fix manually.'), __file__)
                else:
    ...
    def get_explorer_list(self, history):
        ERROR_HANDLER.assert_true(history >= 0, 'Invalid History, must be >=0', __file__)
        ERROR_HANDLER.assert_true(history < len(self.explorer_list), 'Invalid History, must be <= max history', __file__)
    ...
    def archive_latest(self):
        new_timeline_outdirs = [
         self._make_history_name(1)]
        for history in range(1, len(self.timeline_outdirs)):
            new_timeline_outdirs.append(self._change_history(self.timeline_outdirs[history], history + 1))
    ...
    def _create_explorers(self, outdir_list):
        ...
        for outdir in outdir_list:
            ERROR_HANDLER.assert_true(outdir.startswith(self.root_outdir), 'outdir {} not in rootdir'.format(outdir), __file__)
    ...
    def _change_history(self, pathname, new_history):
        ...
        new_base_name = base_name.replace(str(self._history_of_outdir(pathname)), str(new_history), 1)
