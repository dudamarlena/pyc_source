...
# [GCC 8.4.0]
# Embedded file name: /home/mfi/mytools/muteria/muteria/repositoryandcode/code_transformations/c_cpp.py
# Compiled at: 2019-11-25 05:18:29
...
class FromC(ccs.BaseCodeFormatConverter):
    ...
    def convert_code(self, src_fmt, dest_fmt, file_src_dest_map, repository_manager, **kwargs):
        ERROR_HANDLER.assert_true(src_fmt in self.src_formats, 'Unsupported src format', __file__)
            ...
            def after_command(self):
                ...
                for sf, df in list(file_src_dest_map.items()):
                    ...
                    if not os.path.isfile(abs_sf):
                        ERROR_HANDLER.error_exit('an expected file missing after build: ' + abs_sf, __file__)
                    if df is not None:
                        shutil.copy2(abs_sf, df)

        callbak_obj_key = 'callback_object'
        if callbak_obj_key in kwargs:
            ERROR_HANDLER.assert_true(file_src_dest_map is None, 'file_src_dest_map must be None ' + 'if callback_object is passed', __file__)
        else:
            if file_src_dest_map is not None and len(file_src_dest_map) > 0:
        ...
        if dest_fmt == ccs.CodeFormats.C_PREPROCESSED_SOURCE:
            ...
            if dest_fmt == ccs.CodeFormats.LLVM_BITCODE:
                spec_compiler = kwargs['compiler'] if 'compiler' in kwargs else None
            ...
            ERROR_HANDLER.assert_true(has_wllvm, 'wllvm not found (To install please visit https://github.com/travitch/whole-program-llvm)', __file__)
            ...
            ERROR_HANDLER.assert_true(ret != common_mix.GlobalConstants.COMMAND_FAILURE and pre_ret != common_mix.GlobalConstants.COMMAND_FAILURE and post_ret != common_mix.GlobalConstants.COMMAND_FAILURE, 'Build LLVM bitcode failed!', __file__)
            if file_src_dest_map is not None:
                for src, dest in list(file_src_dest_map.items()):
                    ret, out, err = DriversUtils.execute_and_get_retcode_out_err('extract-bc', [dest])
                    ERROR_HANDLER.assert_true(ret == 0, '{}. \n# OUT: {}\n# ERR: {}'.format('extract-bc failed', out, err), __file__)
                    os.remove(dest)
