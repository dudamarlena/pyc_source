...
# [GCC 8.4.0]
# Embedded file name: /home/mfi/mytools/muteria/muteria/common/mix.py
# Compiled at: 2019-10-26 20:10:40
# Size of source mod 2**32: 6319 bytes
...
import sys, shutil, logging, inspect, traceback, enum
...
class ErrorHandler(object):
    ...
    def set_corresponding_repos_manager(cls, repos_dir_manager):
        cls.assert_true(cls.repos_dir_manager is None, err_string='the repo dir manager is already set', call_location=__file__)
        cls.repos_dir_manager = repos_dir_manager

    @classmethod
    def error_exit(cls, err_string=None, call_location=None, error_code=1, ask_revert=True, from_assert=False):
        logging.error('\n' + ''.join(traceback.format_stack()))
        if call_location is not None:
            logging.error('# Error happened in location {}'.format(call_location))
        if from_assert:
        ...
        if ask_revert and cls.repos_dir_manager is not None:
            if cls.error_exit_revert_repo_called:
                logging.error('@ post error: Failed to revert repository.DO IT MANUALLY!!')
    ...
    @classmethod
    def assert_true(cls, condition, err_string=None, call_location=None, ask_revert=True):
        """
        Call this function with the parameter *__file__*
        as location_called_from
        """
        if not condition:
            cls.error_exit(err_string=err_string, call_location=call_location, ask_revert=ask_revert, from_assert=True)
