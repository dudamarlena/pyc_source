...
# [GCC 8.4.0]
# Embedded file name: /home/mfi/mytools/muteria/muteria/drivers/criteria/tools_by_languages/c/mart/mart.py
# Compiled at: 2019-12-13 09:25:31
# Size of source mod 2**32: 19881 bytes
from __future__ import print_function
import os, sys, re, shutil, shlex, logging, subprocess, muteria.common.mix as common_mix, muteria.common.fs as common_fs
...
class CriteriaToolMart(BaseCriteriaTool):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.wm_res_log_file = 'label_log.' + TestCriteria.WEAK_MUTATION.get_field_value()
        self.mcov_res_log_file = 'label_log.' + TestCriteria.MUTANT_COVERAGE.get_field_value()
        self.mutant_data = os.path.join(self.instrumented_code_storage_dir, 'mutant_data')
        self.mart_out = os.path.join(self.mutant_data, 'mart-out-0')
        self.separate_muts_folder_name = 'mutants.out'
        self.separate_muts_dir = os.path.join(self.mart_out, self.separate_muts_folder_name)
        self.archive_separated = True

    def _get_default_params(self):
        bool_params = {'-keep-mutants-bc': None, 
         '-no-COV': None, 
         '-no-Meta': None, 
         '-no-WM': None, 
         '-no-mutant-info': None, 
         ...
         '-write-mutants': None}
        key_val_params = {'-linking-flags': None, 
         '-mutant-config': None, 
         '-mutant-scope': None}
        return (
    ...
    def get_instrumented_executable_paths_map(self, enabled_criteria):
        ...
        for criterion in enabled_criteria:
            ERROR_HANDLER.assert_true(criterion.get_str() in obj, 'criterion was not enabled during instrumentation with Mart: {}. Eneble it and run again'.format(criterion.get_str()), __file__)
    ...
    def get_criterion_info_object(self, criterion):
        try:
            ...
            for mid, info in list(mart_inf_obj.items()):
                minf_obj.add_element(mid, mutant_type=info['Type'], mutant_locs=info['SrcLoc'], mutant_function_name=info['FuncName'], IRPosInFunc=info['IRPosInFunc'])
    ...
    def _get_single_exe_filename(self, criterion):
        try:
            ...
            inst_path_map = self.get_instrumented_executable_paths_map([criterion])
    ...
    def _get_criterion_element_executable_path(self, criterion, element_id):
        ERROR_HANDLER.assert_true(self.get_criterion_info_object(criterion).has_element(element_id), 'Inexistant mutant id: ' + element_id, __file__)
        rel_names = []
        mut_code = {}
        for k, v in self._get_single_exe_filename(criterion).items():
            mut_code[k] = os.path.join(self.separate_muts_dir, element_id, v)
            rel_names.append(os.path.join(self.separate_muts_folder_name, element_id, v))

        if self.archive_separated:
            archive_path = common_fs.TarGz.get_archive_filename_of(self.separate_muts_dir)
            ERROR_HANDLER.assert_true(os.path.isfile(archive_path), 'Archived separated mutant file missing', __file__)
            if os.path.isdir(self.separate_muts_dir):
                shutil.rmtree(self.separate_muts_dir)
            for arch_name in rel_names:
                err_msg = common_fs.TarGz.extractFromArchive(archive_path, arch_name)
                ERROR_HANDLER.assert_true(err_msg is None, 'failed to extract, err: ' + str(err_msg), __file__)
    ...
    def _get_criteria_environment_vars(self, result_dir_tmp, enabled_criteria):
        ...
        for c in enabled_criteria:
            if c == TestCriteria.WEAK_MUTATION:
                res[c] = {'MART_WM_LOG_OUTPUT': os.path.join(result_dir_tmp, self.wm_res_log_file)}
            else:
                if c == TestCriteria.MUTANT_COVERAGE:
                    res[c] = {'MART_WM_LOG_OUTPUT': os.path.join(result_dir_tmp, self.mcov_res_log_file)}
                else:
                    res[c] = None
    ...
    def _extract_coverage_data_of_a_test(self, enabled_criteria, test_execution_verdict, result_dir_tmp):
        ...
        """
        ERROR_HANDLER.assert_true(TestCriteria.STRONG_MUTATION not in enabled_criteria, 'SM metamutant run not yet supported', __file__)

        def extract_covered(filename, criterion):
            mutant_id_set = set(self.get_criterion_info_object(criterion).get_elements_list())
            cov_res = {m:common_mix.GlobalConstants.ELEMENT_NOTCOVERED_VERDICT for m in mutant_id_set}
            with open(filename) as (f):
                for line in f:
                    mut_id = line.strip()
                    if mut_id in mutant_id_set:
                        cov_res[mut_id] = common_mix.GlobalConstants.ELEMENT_COVERED_VERDICT

            return cov_res

        res = {}
        if TestCriteria.WEAK_MUTATION in enabled_criteria:
            res[TestCriteria.WEAK_MUTATION] = extract_covered(os.path.join(result_dir_tmp, self.wm_res_log_file), TestCriteria.WEAK_MUTATION)
        if TestCriteria.MUTANT_COVERAGE in enabled_criteria:
            res[TestCriteria.MUTANT_COVERAGE] = extract_covered(os.path.join(result_dir_tmp, self.mcov_res_log_file), TestCriteria.MUTANT_COVERAGE)
        return res
    ...
    def _do_instrument_code(self, exe_path_map, code_builds_factory, enabled_criteria, parallel_count=1):
        ...
        if self.custom_binary_dir is not None:
            ...
            ERROR_HANDLER.assert_true(os.path.isfile(prog), 'The tool {} is missing from the specified dir {}'.format(os.path.basename(prog), self.custom_binary_dir), __file__)
        ret, out, err = DriversUtils.execute_and_get_retcode_out_err(prog, ['--version'])
        llvm_compiler_path = None
        for line in out.splitlines():
            line = line.strip()
            if line.startswith('LLVM tools dir:'):
                llvm_compiler_path = line.split()[3]
                break

        ERROR_HANDLER.assert_true(llvm_compiler_path is not None, 'Problem getting llvm path for mart', __file__)
        back_llvm_compiler = 'clang'
        rel_path_map = {}
        exes, _ = code_builds_factory.repository_manager.get_relative_exe_path_map()
        for exe in exes:
            filename = os.path.basename(exe)
            rel_path_map[exe] = os.path.join(self.mutant_data, filename)

        pre_ret, ret, post_ret = code_builds_factory.transform_src_into_dest(src_fmt=CodeFormats.C_SOURCE, dest_fmt=CodeFormats.LLVM_BITCODE, src_dest_files_paths_map=rel_path_map, compiler=back_llvm_compiler, flags_list=['-g'], clean_tmp=True, reconfigure=True, llvm_compiler_path=llvm_compiler_path)
        if ret == common_mix.GlobalConstants.TEST_EXECUTION_ERROR:
            ERROR_HANDLER.error_exit('Program {}.'.format('LLVM (clang) built problematic'), __file__)
        rel2bitcode = {}
        for r_file, b_file in list(rel_path_map.items()):
            bc = b_file + '.bc'
            ERROR_HANDLER.assert_true(os.path.isfile(bc), 'Bitcode file not existing: ' + bc, __file__)
            rel2bitcode[r_file] = bc

        ERROR_HANDLER.assert_true(len(rel_path_map) == 1, 'Support single bitcode module for now', __file__)
        ...
        if ret not in (0, ):
            logging.error(out)
            ERROR_HANDLER.error_exit("'Mart' cannot compile bitcode" + 'into native code, you need to specify' + 'linking flags (based on the above error).', __file__)
        ...
        if k_v_params['-mutant-scope'] is None:
            src_list_scope_file = os.path.join(self.mutant_data, 'mutation-scope.json')
            scope_obj = {'Source-Files': [], 'Functions': []}
            _, src2obj = self.code_builds_factory.repository_manager.get_relative_exe_path_map()
        ...
        ret, out, err = DriversUtils.execute_and_get_retcode_out_err(prog, args)
        os.chdir(cwd)
        if ret != 0:
            logging.error(out)
            logging.error(err)
            logging.error('\n>> CMD: ' + ' '.join([prog] + args) + '\n')
            ERROR_HANDLER.error_exit('mart failed (ran from {})'.format(self.mutant_data), __file__)
        ...
        if self.archive_separated:
            err_msg = common_fs.TarGz.compressDir(self.separate_muts_dir, remove_in_directory=True)
            ERROR_HANDLER.assert_true(err_msg is None, 'Compression failed: ' + str(err_msg), __file__)

    def get_test_gen_metamutant_bc(self):
        ERROR_HANDLER.assert_true(os.path.isfile(self.instrumentation_details), 'Are you sure mutant generation with Mart was ran?This is needed before meta mu bc is called', __file__)
        crit2file = self.get_instrumented_executable_paths_map((
         TestCriteria.STRONG_MUTATION,))
        sm_map = list(crit2file[TestCriteria.STRONG_MUTATION].items())
        ERROR_HANDLER.assert_true(len(sm_map) == 1, 'Expects exactly 1 file for Strong Mutation')
