...
# [GCC 8.4.0]
# Embedded file name: /home/mfi/mytools/muteria/muteria/common/fs.py
# Compiled at: 2019-11-08 07:08:04
# Size of source mod 2**32: 27709 bytes
"""
    fs
    --
...
import os, json, tarfile, zipfile, time, shutil, logging, pandas as pd, muteria.common.mix as common_mix
...
class TarGz:
    ...
    @classmethod
    def compressDir(cls, in_directory, out_archive_pathname=None, remove_in_directory=False):
        ERROR_HANDLER.assert_true(os.path.isdir(in_directory), 'invalid in_directory: ' + in_directory)
    ...
    @classmethod
    def compressFile(cls, in_file, out_archive_pathname=None, remove_in_file=False):
        ERROR_HANDLER.assert_true(os.path.isfile(in_file), 'invalid in_file: ' + in_file)
    ...
    def _compressFileOrDir(cls, in_directory, out_archive_pathname=None):
        ...
        with cls.opening_function(out_archive_pathname, cls.open_write_flag) as (handle):
            push_dir = os.getcwd()
    ...
    @classmethod
    def decompressDir(cls, in_archive_pathname, out_directory=None, remove_in_archive=False):
        err_msg = cls._decompressFileOrDir(in_archive_pathname, out_directory, remove_in_archive)
        return err_msg

    @classmethod
    def decompressFile(cls, in_archive_pathname, out_directory=None, remove_in_archive=False):
        err_msg = cls._decompressFileOrDir(in_archive_pathname, out_directory, remove_in_archive)
        return err_msg

    @classmethod
    def _decompressFileOrDir(cls, in_archive_pathname, out_directory=None, remove_in_archive=False):
        """
        ...
        if in_archive_pathname.endswith(cls.archive_ext):
            ...
            ERROR_HANDLER.assert_true(os.path.isdir(out_directory), 'Extract location is missing', __file__)
            with cls.opening_function(in_archive_pathname, cls.open_read_flag) as (handle):
    ...
    @classmethod
    def extractFromArchive(cls, in_archive_pathname, extract_pathname, out_location=None, is_folder=False):
        if in_archive_pathname.endswith(cls.archive_ext):
    ...
    @classmethod
    def removeFromArchive(cls, in_archive_pathname, rem_pathname, is_folder=False):
        tmp_dir = in_archive_pathname + '.tmp_dir'
        if os.path.isdir(tmp_dir):
            shutil.rmtree(tmp_dir)
        os.mkdir(tmp_dir)
        errmsg = cls.decompressDir(in_archive_pathname, out_directory=tmp_dir, remove_in_archive=True)
        ...
        if is_folder:
            ...
            os.remove(rem)
        errmsg = cls.compressDir(tmp_dir, in_archive_pathname, remove_in_directory=True)
        if errmsg is not None:
            return errmsg + ' -- (removeFromArchive)'
...
class Zip(TarGz):
    ...
    @classmethod
    def addToArchive(cls, archive_pathname, added_pathname, in_archive_name=None, is_folder=False):
        ...
        if archive_pathname.endswith(cls.archive_ext):
            with cls.opening_function(archive_pathname, 'a') as (handle):
                if not is_folder:
                    ...
                    cls._add_folder_function(handle, added_pathname)
                try:
...
class FileDirStructureHandling(object):
    ...
    def __init__(self, top_dir, top_dir_key, file_dir_dict):
        ...
        for fd in file_dir_dict:
            if type(file_dir_dict[fd]) not in (list, tuple):
                self.error_module.error_exit(err_string='%s %s' % ('Each value in file_dir_dict',
                                                                   'must be an ordered list of strings'), call_location=__file__)
            if len(file_dir_dict[fd]) < 1:
                self.error_module.error_exit(err_string='%s %s' % (
                 'empty path elements for file/dir:', fd), call_location=__file__)
            if fd in self.file_dir_to_path_dict:
                self.error_module.error_exit(err_string='%s %s %s' % (
                 'file or directory already appears in',
                 'self.file_dir_to_path_dict:', fd), call_location=__file__)
    ...
    def resolve(self, name):
        ...
        if name not in self.file_dir_to_path_dict:
            self.error_module.error_exit(err_string='%s %s' % (
             name, 'not in file_dir_to_path_dict'), call_location=__file__)
        return self.file_dir_to_path_dict[name]

    def get_file_pathname(self, filename, rel_path=False):
        if rel_path:
            return self.resolve(filename)
        return os.path.normpath(os.path.join(self.top_dir, self.resolve(filename)))
    ...
    def get_existing_file_pathname(self, filename, rel_path=False):
        ...
        if not os.path.isfile(fullpathstring):
            self.error_module.error_exit(err_string='%s %s' % (
             'getting a file non existing', fullpathstring), call_location=__file__)
    ...
    def get_existing_dir_pathname(self, dirname, rel_path=False):
        ...
        if not os.path.isdir(fullpathstring):
            self.error_module.error_exit(err_string='%s %s' % (
             'getting a directory non existing', fullpathstring), call_location=__file__)
        return retpathstring
...
class CheckpointState(object):
    ...
    def set_finished(self, detailed_exectime_obj=None):
        if not self.started:
            ERROR_HANDLER.error_exit('%s' % 'finishing checkpointed task while not started', __file__)
        self.started = False
        self.finished = True
        self.write_checkpoint(self.EXEC_COMPLETED, detailed_exectime_obj=detailed_exectime_obj)
    ...
    def is_destroyed(self):
        ...
        no_files &= not os.path.isfile(self.store_filepath) and not os.path.isfile(self.backup_filepath)
        return no_files
    ...
    def load_checkpoint_or_start(self, ret_detailed_exectime_obj=False):
        ...
        if ret_detailed_exectime_obj:
            res = (
             res, raw_obj[self.DETAILED_TIME_KEY])
        return res
    ...
    def write_checkpoint(self, json_obj, detailed_exectime_obj=None):
        ...
        if os.path.isfile(self.store_filepath):
            ...
            remove_back = True
        cur_agg_time = self.prev_aggregated_time + (time.time() - self.starttime)
        raw_obj = {self.AGG_TIME_KEY: cur_agg_time, 
    ...
    def get_execution_time(self):
        ERROR_HANDLER.assert_true(not self.is_destroyed(), 'Trying to get time for destroyed checkpoint', __file__)
    ...
    def get_detailed_execution_time(self):
        ERROR_HANDLER.assert_true(not self.is_destroyed(), 'Trying to get detailed time for destroyed checkpoint', __file__)
        raw_obj = self._get_from_file()
    ...
    def _get_from_file(self):
        ...
        if trybackup and os.path.isfile(self.backup_filepath):
            ...
            if not common_mix.confirm_execution('%s %s' % ('The checkpoint store_file is invalid but backup',
                                                           'is valid. Do you want to use backup?')):
                ERROR_HANDLER.error_exit('%s %s' % ('Execution terminated due to',
                                                    'invalid Checkpoint store_file'), __file__)
        if contain is not None:
            for key in [self.DETAILED_TIME_KEY, self.AGG_TIME_KEY, self.CHECKPOINT_DATA_KEY]:
                if key not in contain:
                    file_used = self.backup_filepath if trybackup else self.store_filepath
    ...
    def _update_this_object(self, raw_obj):
        if raw_obj is None:
            ...
            if checkpoint_data == self.EXEC_STARTING:
                ...
                if checkpoint_data == self.EXEC_COMPLETED:
                    self.started = False
                    self.finished = True
                    self.starttime = None
                else:
