...
# [GCC 8.4.0]
# Embedded file name: /home/mfi/mytools/muteria/muteria/drivers/criteria/meta_testcriteriatool.py
# Compiled at: 2019-11-29 11:36:46
# Size of source mod 2**32: 36916 bytes
"""
...
import os, sys, glob, logging, shutil, muteria.common.fs as common_fs, muteria.common.matrices as common_matrices, muteria.common.mix as common_mix
...
class MetaCriteriaTool(object):
    ...
    def get_toolnames_by_types_by_criteria_by_language(cls):
        ...
        """
        modules_dict = ToolsModulesLoader.get_tools_modules(ToolsModulesLoader.CRITERIA_TOOLS)
        res = {}
        for language in modules_dict:
            ...
            for toolname in modules_dict[language]:
                for tooltype in CriteriaToolType:
                    ...
                    try:
                        CriteriaTool = getattr(modules_dict[language][toolname], tooltype_name)
                        ...
                        ERROR_HANDLER.error_exit('{} {} {} {}'.format('(REPORT BUG) The criteria tool of type', tooltype_name, 'is not present for criteria tool', toolname), __file__)

        return res

    def __init__(self, language, meta_test_generation_obj, criteria_working_dir, code_builds_factory, tools_config_by_criterion_dict):
        self.modules_dict = ToolsModulesLoader.get_tools_modules(ToolsModulesLoader.CRITERIA_TOOLS)
        ...
        for criterion in self.tools_config_by_criterion_dict:
            ERROR_HANDLER.assert_true(len(self.tools_config_by_criterion_dict[criterion]) == len(set([c.get_tool_config_alias() for c in self.tools_config_by_criterion_dict[criterion]])), 'some tool configs appear multiple times for {}'.format(criterion), __file__)

        self.checkpoints_dir = os.path.join(self.criteria_working_dir, '_checkpoints_')
        self.criteria_info_file_by_criteria = {}
        for criterion in self.tools_config_by_criterion_dict:
            self.criteria_info_file_by_criteria[criterion] = os.path.join(self.criteria_working_dir, criterion.get_str() + '_info_file.json')
        ...
        for criterion in self.tools_config_by_criterion_dict:
            for config in self.tools_config_by_criterion_dict[criterion]:
                ...
                if toolalias in tmp_alias2conf:
                    ERROR_HANDLER.assert_true(config == tmp_alias2conf[toolalias], '{} {} {}'.format('same tool alias but different', 'config for alias', toolalias), __file__)
                else:
                    tmp_alias2conf[toolalias] = config
                    tool_working_dir = self._get_criteria_tool_out_folder(toolalias)
                    tool_checkpointer = common_fs.CheckpointState(*self._get_criteria_tool_checkpoint_files(toolalias))
                    self.checkpointer.add_dep_checkpoint_state(tool_checkpointer)
                    self.criteria_configured_tools[toolalias] = {self.TOOL_OBJ_KEY: self._create_criteria_tool(toolname, tool_working_dir, config, tool_checkpointer), 
        ...
        for criterion in self.tools_config_by_criterion_dict:
            for toolconf in self.tools_config_by_criterion_dict[criterion]:
                ...
                if criterion not in self.criteria_configured_tools[toolalias][self.TOOL_OBJ_KEY].get_supported_criteria():
                    ERROR_HANDLER.error_exit('tool {} specified for unsupported criterion {}'.format(toolalias, criterion), __file__)

    def _create_criteria_tool(self, toolname, tool_working_dir, config, tool_checkpointer):
        """

        """
        ERROR_HANDLER.assert_true(toolname in self.modules_dict[self.language], 'Invalid toolname given: {}'.format(toolname), __file__)
        try:
            CriteriaTool = getattr(self.modules_dict[self.language][toolname], config.get_tool_type().get_field_value())
        except AttributeError:
            ERROR_HANDLER.error_exit('{} {} {} {}'.format('(REPORT BUG) The Criteria tool of type', config.get_tool_type().get_field_value(), 'is not present for test tool', toolname), __file__)

        ERROR_HANDLER.assert_true(CriteriaTool is not None, "The {} language's tool {} {} {}.".format(self.language, toolname, 'does not implement the criteria tool type', config.get_tool_type().get_str()), __file__)
        crit_tool = CriteriaTool(self.meta_test_generation_obj, tool_working_dir, self.code_builds_factory, config, tool_checkpointer)
        return crit_tool
    ...
    def check_tools_installed(self):
        ...
        if len(non_installed) > 0:
            ERROR_HANDLER.error_exit('{}: {}'.format('The following Criteria tools are not installed', str(non_installed)))
...
estart_checkpointer:
        ...
        """
        ERROR_HANDLER.assert_true(parallel_count <= 1, 'Must implement parallel execution support here', __file__)
        ERROR_HANDLER.assert_true(parallel_criteria_test_scheduler is None, 'Must implement parallel codes tests execution support here', __file__)
        ERROR_HANDLER.assert_true(parallel_count > 0, 'invalid parallel  execution count: {}. {}'.format(parallel_count, 'must be >= 1'))
        ...
        if checkpoint_handler.is_finished():
            return
        ERROR_HANDLER.assert_true(len(criterion_to_matrix) > 0, 'no criterion is enabled', __file__)
        ERROR_HANDLER.assert_true(len(set(criterion_to_matrix) - set(self.tools_config_by_criterion_dict)) == 0, 'Passed matrices output are more than tool specified', __file__)
        if criterion_to_executionoutput is not None:
            ERROR_HANDLER.assert_true(set(criterion_to_matrix) == set(criterion_to_executionoutput), 'criteria mismatch between matrix and output', __file__)
        tool2criteria = self._get_tool2criteria(criterion_to_matrix.keys())
        matrices_dir_tmp = os.path.join(self.criteria_working_dir, 'codecov_dir.tmp')
        ...
        if criteria_element_list_by_criteria is None:
            criteria_element_list_by_criteria = {c:None for c in criterion_to_matrix}
        ...
        for criterion in criteria_element_list_by_criteria:
            if criteria_element_list_by_criteria[criterion] is None:
                ...
                ERROR_HANDLER.assert_true(len(set(criteria_elem_list_by_tool) - set(self.criteria_configured_tools)) == 0, 'some tool in data not registered', __file__)
        ...
        for ctoolalias in tool2criteria:
            ...
            for criterion in tool2criteria[ctoolalias]:
                _criteria2matrix[criterion] = os.path.join(matrices_dir_tmp, criterion.get_field_value() + '-' + ctoolalias + '.csv')
                if criterion_to_executionoutput is None or criterion_to_executionoutput[criterion] is None:
                    _criteria2outhash[criterion] = None
                else:
                    _criteria2outhash[criterion] = os.path.join(matrices_dir_tmp, criterion.get_field_value() + '-' + ctoolalias + '.outloghash.json')
                crit2tool2matrixfile[criterion][ctoolalias] = _criteria2matrix[criterion]
                crit2tool2outhashfile[criterion][ctoolalias] = _criteria2outhash[criterion]

            if checkpoint_handler.is_to_execute(func_name=cp_func_name, taskid=cp_task_id, tool=ctoolalias):
                for criterion in _criteria2matrix:
                    _criteria2matrix[criterion] = common_matrices.ExecutionMatrix(filename=_criteria2matrix[criterion], non_key_col_list=testcases)
                    if _criteria2outhash[criterion] is not None:
                        _criteria2outhash[criterion] = common_matrices.OutputLogData(filename=_criteria2outhash[criterion])

                ctool = self.criteria_configured_tools[ctoolalias][self.TOOL_OBJ_KEY]
                ctool.runtests_criteria_coverage(testcases, criteria_element_list_by_criteria=criteria_elem_list_by_tool[toolalias], criterion_to_matrix=_criteria2matrix, criterion_to_executionoutput=_criteria2outhash, re_instrument_code=re_instrument_code, cover_criteria_elements_once=cover_criteria_elements_once, prioritization_module_by_criteria=prioritization_module_by_criteria)
                checkpoint_handler.do_checkpoint(func_name=cp_func_name, taskid=cp_task_id, tool=ctoolalias)

        result_matrices = {}
        result_outloghashes = {}
        for criterion in criterion_to_matrix:
            result_matrices[criterion] = common_matrices.ExecutionMatrix(filename=criterion_to_matrix[criterion], non_key_col_list=testcases)
            if criterion_to_executionoutput[criterion] is None:
                result_outloghashes[criterion] = None
            else:
                result_outloghashes[criterion] = common_matrices.OutputLogData(filename=criterion_to_executionoutput[criterion])
                ERROR_HANDLER.assert_true(crit2tool2outhashfile[criterion] is not None, 'Bug: log enabled but hidden from tool', __file__)
        ...
        for criterion in result_matrices:
            ...
            for mtoolalias in crit2tool2matrixfile[criterion]:
                tool_matrix = common_matrices.ExecutionMatrix(filename=crit2tool2matrixfile[criterion][mtoolalias])
                ERROR_HANDLER.assert_true(tool_matrix.get_key_colname() == result_matrix.get_key_colname(), 'mismatch on key column name', __file__)
                ERROR_HANDLER.assert_true(set(tool_matrix.get_nonkey_colname_list()) == set(result_matrix.get_nonkey_colname_list()), 'mismatch on non key column names', __file__)
                key2nonkeydict = tool_matrix.to_pandas_df().set_index(tool_matrix.get_key_colname(), drop=True).to_dict(orient='index')
                for c_key in key2nonkeydict:
                    meta_c_key = DriversUtils.make_meta_element(str(c_key), mtoolalias)
                    result_matrix.add_row_by_key(meta_c_key, key2nonkeydict[c_key], serialize=False)

                if crit2tool2outhashfile[criterion] is not None:
                    tool_outloghash = common_matrices.OutputLogData(filename=crit2tool2outhashfile[criterion][mtoolalias])
                    for objective, objective_data in tool_outloghash.get_zip_objective_and_data():
                        meta_objective = DriversUtils.make_meta_element(str(objective), mtoolalias)
                        result_outloghash.add_data({meta_objective: objective_data}, serialize=False)

            if checkpoint_handler.is_to_execute(func_name=cp_func_name, taskid=cp_task_id + 1, tool=criterion.get_str()):
                result_matrix.serialize()
                if result_outloghash is not None:
                    result_outloghash.serialize()
                checkpoint_handler.do_checkpoint(func_name=cp_func_name, taskid=cp_task_id + 1, tool=criterion.get_str())
    ...
    def instrument_code(self, criteria_enabled_list=None, exe_path_map=None, parallel_count=1, restart_checkpointer=False, finish_destroy_checkpointer=True):
...
estart_checkpointer:
        ...
        :type \x0cinish_destroy_checkpointer:
        :param \x0cinish_destroy_checkpointer:
    
        :raises:
    
        :rtype:
        """
        ERROR_HANDLER.assert_true(parallel_count <= 1, 'FIXME: Must first implement support for parallel')
        ERROR_HANDLER.assert_true(parallel_count > 0, 'invalid parallel  execution count: {}. {}'.format(parallel_count, 'must be >= 1'), __file__)
        ...
        if criteria_enabled_list is None:
            ...
            ERROR_HANDLER.assert_true(len(criteria_enabled_list) > 0, 'no criterion is enabled', __file__)
            ERROR_HANDLER.assert_true(len(set(criteria_enabled_list) - set(self.tools_config_by_criterion_dict)) == 0, 'Passed matrice output are more than toll specified', __file__)
        tool2criteria = self._get_tool2criteria(criteria_enabled_list)
        for ctoolalias in tool2criteria:
            if checkpoint_handler.is_to_execute(func_name=cp_func_name, taskid=cp_task_id, tool=ctoolalias):
                ctool = self.criteria_configured_tools[ctoolalias][self.TOOL_OBJ_KEY]
                ctool.instrument_code(enabled_criteria=tool2criteria[ctoolalias], exe_path_map=exe_path_map)
                self.code_builds_factory.set_repo_to_build_default()
                checkpoint_handler.do_checkpoint(func_name=cp_func_name, taskid=cp_task_id, tool=ctoolalias)

        self._invalidate_criteria_info(enabled_criteria=tool2criteria[ctoolalias])
        detailed_exectime = {}
        for ctoolalias in tool2criteria:
            ct = self.criteria_configured_tools[ctoolalias][self.TOOL_OBJ_KEY]
            detailed_exectime[ctoolalias] = (
    ...
    def _compute_criterion_info(self, criterion, candidate_tool_aliases=None):
        ...
        for ctoolalias in candidate_tool_aliases:
            ...
            if tool_element_info is not None:
                ...
                meta_criterion_info_obj.update_using(ctoolalias, old2new_tests, tool_element_info)
    ...
    def get_criterion_info_object(self, criterion):
        if criterion not in CriteriaToInfoObject:
            return
        return CriteriaToInfoObject[criterion]().load_from_file(self.get_criterion_info_file(criterion))
    ...
    def _criterion_info_is_invalidated(self, criterion):
        ERROR_HANDLER.assert_true(criterion in CriteriaToInfoObject, 'The criterion {} {}'.format(criterion.get_str(), 'must have info Class registered for this'), __file__)
        return not os.path.isfile(self._unchecked_get_criterion_info_file(criterion))
    ...
    def _get_criteria_tool_checkpoint_files(self, criteria_toolname, top_checkpoint_dir=None):
        if top_checkpoint_dir is None:
            top_checkpoint_dir = self.checkpoints_dir
        return [os.path.join(top_checkpoint_dir, criteria_toolname + '_checkpoint.state' + suffix) for suffix in ('',
                                                                                                                  '.backup')]
