...
# [GCC 8.4.0]
# Embedded file name: /home/mfi/mytools/muteria/muteria/drivers/testgeneration/meta_testcasetool.py
# Compiled at: 2019-12-17 11:27:13
# Size of source mod 2**32: 41085 bytes
"""
...
import os, sys, glob, shutil, copy, logging, multiprocessing, joblib, muteria.common.fs as common_fs, muteria.common.matrices as common_matrices, muteria.common.mix as common_mix
...
from muteria.drivers.testgeneration.custom_dev_testcase.custom_dev_testcase import CustomTestcases
...
class MetaTestcaseTool(object):
    ...
    def get_toolnames_by_types_by_language(cls):
        ...
        """
        modules_dict = ToolsModulesLoader.get_tools_modules(ToolsModulesLoader.TESTCASES_TOOLS)
        res = {}
        for language in modules_dict:
            ...
            for toolname in modules_dict[language]:
                for tooltype in TestToolType:
                    ...
                    try:
                        TestcaseTool = getattr(modules_dict[language][toolname], tooltype_name)
                        ...
                        ERROR_HANDLER.error_exit('{} {} {} {}'.format('(REPORT BUG) The test case tool of type', tooltype_name, 'is not present for test tool', toolname), __file__)
    ...
    def __init__(self, language, tests_working_dir, code_builds_factory, test_tool_config_list, head_explorer):
        ...
        :rtype:
        """
        self.modules_dict = ToolsModulesLoader.get_tools_modules(ToolsModulesLoader.TESTCASES_TOOLS)
        ...
        ERROR_HANDLER.assert_true(len(self.test_tool_config_list) == len(set([c.get_tool_config_alias() for c in self.test_tool_config_list])), 'some tool configs appear multiple times', __file__)
        self.checkpoints_dir = os.path.join(self.tests_working_dir, '_checkpoints_')
        self.testcases_info_file = os.path.join(self.tests_working_dir, 'testcasesInfos.json')
        self.flakiness_workdir = os.path.join(self.tests_working_dir, 'Flakiness')
        ...
        for idx in range(len(test_tool_config_list)):
            ...
            tool_checkpointer = common_fs.CheckpointState(*self._get_test_tool_checkpoint_files(toolalias))
            self.checkpointer.add_dep_checkpoint_state(tool_checkpointer)
            self.testcases_configured_tools[toolalias] = {self.TOOL_OBJ_KEY: self._create_testcase_tool(toolname, tool_working_dir, config, tool_checkpointer), 
             
             self.TOOL_WORKDIR_KEY: tool_working_dir, 
             self.TOOL_TYPE_KEY: config.get_tool_type()}

    def _create_testcase_tool(self, toolname, tool_working_dir, config, tool_checkpointer):
        """
            
        """
        if toolname == CustomTestcases.CUSTOM_TEST_TOOLNAME:
            TestcaseTool = CustomTestcases
            ERROR_HANDLER.assert_true(self.custom_devtest_toolalias is None, 'Must specify custom dev tests only once', __file__)
            self.custom_devtest_toolalias = config.get_tool_config_alias()
        else:
            ERROR_HANDLER.assert_true(toolname in self.modules_dict[self.language], 'Invalid toolname given: {}'.format(toolname), __file__)
            try:
                TestcaseTool = getattr(self.modules_dict[self.language][toolname], config.get_tool_type().get_field_value())
            except AttributeError:
                ERROR_HANDLER.error_exit('{} {} {} {}'.format('(REPORT BUG) The test case tool of type', config.get_tool_type().get_field_value(), 'is not present for test tool', toolname), __file__)

            ERROR_HANDLER.assert_true(TestcaseTool is not None, "The {} language's tool {} {} {}.".format(self.language, toolname, 'does not implement the test tool type', config.get_tool_type().get_str()), __file__)
        testcase_tool = TestcaseTool(tool_working_dir, self.code_builds_factory, config, self.head_explorer, tool_checkpointer, parent_meta_tool=self)
    ...
    def check_tools_installed(self):
        ...
        if len(non_installed) > 0:
            ERROR_HANDLER.error_exit('{}: {}'.format('The following Testcase tools are not installed', str(non_installed)))
    ...
    def execute_testcase(self, meta_testcase, exe_path_map, env_vars, timeout=None, use_recorded_timeout_times=None, recalculate_execution_times=False, with_output_summary=True, hash_outlog=True):
        """
        ...
        ERROR_HANDLER.assert_true(ttoolalias in self.testcases_configured_tools, 'Test tool {} not registered'.format(ttoolalias), __file__)
        ttool = self.testcases_configured_tools[ttoolalias][self.TOOL_OBJ_KEY]
        return ttool.execute_testcase(testcase, exe_path_map, env_vars, timeout=timeout, use_recorded_timeout_times=use_recorded_timeout_times, recalculate_execution_times=recalculate_execution_times, with_output_summary=with_output_summary, hash_outlog=hash_outlog)
    ...
    def runtests(self, meta_testcases=None, exe_path_map=None, env_vars=None, stop_on_failure=False, per_test_timeout=None, use_recorded_timeout_times=None, recalculate_execution_times=False, fault_test_execution_matrix_file=None, fault_test_execution_execoutput_file=None, with_output_summary=True, hash_outlog=True, test_prioritization_module=None, parallel_test_count=1, parallel_test_scheduler=None, restart_checkpointer=False, finish_destroy_checkpointer=True):
        ...
        """
        ERROR_HANDLER.assert_true(meta_testcases is not None, 'Must specify testcases', __file__)
        ERROR_HANDLER.assert_true(test_prioritization_module is None, 'Must implement test prioritization support here', __file__)
        ERROR_HANDLER.assert_true(parallel_test_scheduler is None, 'Must implement parallel tests execution support here', __file__)
        if exe_path_map is None:
            exe_path_map = self._get_default_exe_path_map()
        ERROR_HANDLER.assert_true(parallel_test_count is None or parallel_test_count >= 1, 'invalid parallel tests count ({})'.format(parallel_test_count), __file__)
        cp_func_name = 'runtests'
        cp_task_id = 1
        checkpoint_handler = CheckPointHandler(self.get_checkpoint_state_object())
        if restart_checkpointer:
            checkpoint_handler.restart()
        if checkpoint_handler.is_finished():
            logging.warning('%s %s' % ("The function 'runtests' is finished", 'according to checkpoint, but called again. None returned'))
            if common_mix.confirm_execution('%s %s' % ("Function 'runtests' is already",
                                                       'finished, do you want to restart?')):
                checkpoint_handler.restart()
                logging.info("Restarting the finished 'runtests'")
            else:
                ERROR_HANDLER.error_exit(err_string='%s %s %s' % ('Execution halted. Cannot continue because no value',
                                                                  ' can be returned. Check the results of the',
                                                                  'finished execution'), call_location=__file__)
            meta_test_failedverdicts_outlog = checkpoint_handler.get_optional_payload()
            if meta_test_failedverdicts_outlog is None:
                meta_test_failedverdicts_outlog = [{}, {}]
            ERROR_HANDLER.assert_true(len(meta_testcases) == len(set(meta_testcases)), 'not all tests are unique', __file__)
            testcases_by_tool = {}
            ...
            for tpos, ttoolalias in enumerate(testcases_by_tool.keys()):
                if not checkpoint_handler.is_to_execute(func_name=cp_func_name, taskid=cp_task_id, tool=ttoolalias):
                    pass
                else:
            ...
            def tool_parallel_test_exec(ttoolalias):
                ...
                ttool = self.testcases_configured_tools[ttoolalias][self.TOOL_OBJ_KEY]
                test_failed_verdicts, test_execoutput = ttool.runtests(testcases_by_tool[ttoolalias], exe_path_map, env_vars, stop_on_failure, per_test_timeout=per_test_timeout, use_recorded_timeout_times=use_recorded_timeout_times, recalculate_execution_times=recalculate_execution_times, with_output_summary=with_output_summary, hash_outlog=hash_outlog, parallel_count=next_parallel_count)
                with shared_loc:
                    for testcase in test_failed_verdicts:
                        meta_testcase = DriversUtils.make_meta_element(testcase, ttoolalias)
                        meta_test_failedverdicts_outlog[0][meta_testcase] = test_failed_verdicts[testcase]
                        meta_test_failedverdicts_outlog[1][meta_testcase] = test_execoutput[testcase]
                        if not found_a_failure and test_failed_verdicts[testcase] == common_mix.GlobalConstants.COMMAND_UNCERTAIN:
                            found_a_failure = True

                    checkpoint_handler.do_checkpoint(func_name=cp_func_name, taskid=cp_task_id, tool=ttoolalias, opt_payload=meta_test_failedverdicts_outlog)
                return found_a_failure

            ptest_tresh = 5
            if len(candidate_aliases) > 1 and len(meta_testcases) >= ptest_tresh and (parallel_test_count is None or parallel_test_count > 1):
                if parallel_test_count is None:
                    paralle_count = min(len(candidate_aliases), multiprocessing.cpu_count())
                else:
                    paralle_count = min(len(candidate_aliases), parallel_test_count)
                joblib.Parallel(n_jobs=paralle_count, require='sharedmem')(joblib.delayed(tool_parallel_test_exec)(ttoolalias) for ttoolalias in candidate_aliases)
        else:
            for tpos, ttoolalias in enumerate(candidate_aliases):
                ...
                if stop_on_failure and found_a_failure:
                    for rem_tool in list(testcases_by_tool.keys())[tpos + 1:]:
                        checkpoint_handler.do_checkpoint(func_name=cp_func_name, taskid=cp_task_id, tool=rem_tool, opt_payload=meta_test_failedverdicts_outlog)

                    break

        if stop_on_failure and len(meta_test_failedverdicts_outlog[0]) < len(meta_testcases):
            for meta_testcase in set(meta_testcases) - set(meta_test_failedverdicts_outlog[0]):
                meta_test_failedverdicts_outlog[0][meta_testcase] = common_mix.GlobalConstants.UNCERTAIN_TEST_VERDICT
                meta_test_failedverdicts_outlog[1][meta_testcase] = common_matrices.OutputLogData.UNCERTAIN_TEST_OUTLOGDATA

        ERROR_HANDLER.assert_true(len(meta_test_failedverdicts_outlog[0]) == len(meta_testcases), 'Not all tests have a verdict reported', __file__)
        if fault_test_execution_matrix_file is not None:
            fault_test_execution_matrix = common_matrices.ExecutionMatrix(filename=fault_test_execution_matrix_file, non_key_col_list=meta_testcases)
            ERROR_HANDLER.assert_true(fault_test_execution_matrix.is_empty(), 'matrix must be empty. Filename is: ' + fault_test_execution_matrix_file, __file__)
            failverdict2val = {common_mix.GlobalConstants.FAIL_TEST_VERDICT: fault_test_execution_matrix.getActiveCellDefaultVal(), 
             
             common_mix.GlobalConstants.PASS_TEST_VERDICT: fault_test_execution_matrix.getInactiveCellVal(), 
             
             common_mix.GlobalConstants.UNCERTAIN_TEST_VERDICT: fault_test_execution_matrix.getUncertainCellDefaultVal()}
            cells_dict = {}
            for meta_testcase in meta_test_failedverdicts_outlog[0]:
                cells_dict[meta_testcase] = failverdict2val[meta_test_failedverdicts_outlog[0][meta_testcase]]

            fault_test_execution_matrix.add_row_by_key(self.FAULT_MATRIX_KEY, cells_dict, serialize=True)
        if fault_test_execution_execoutput_file is not None:
            fault_test_execution_execoutput = common_matrices.OutputLogData(filename=fault_test_execution_execoutput_file)
            ERROR_HANDLER.assert_true(fault_test_execution_execoutput.is_empty(), 'outlog data must be empty', __file__)
            fault_test_execution_execoutput.add_data({self.PROGRAM_EXECOUTPUT_KEY: meta_test_failedverdicts_outlog[1]}, serialize=True)
        detailed_exectime = {}
    ...
    def get_candidate_tools_aliases(self, test_tool_type_list):
        ...
        for test_tool_type in test_tool_type_list:
            ERROR_HANDLER.assert_true(TestToolType.is_valid(test_tool_type), 'Invalid test tool type passed to test generation', __file__)
            for ttoolalias in self.testcases_configured_tools:
                if self.testcases_configured_tools[ttoolalias][self.TOOL_TYPE_KEY] == test_tool_type:
                    candidate_tools_aliases.append(ttoolalias)

        return candidate_tools_aliases

    def generate_tests(self, meta_criteria_tool_obj=None, exe_path_map=None, test_tool_type_list=None, max_time=None, test_generation_guidance_obj=None, parallel_testgen_count=1, restart_checkpointer=False, finish_destroy_checkpointer=True):
        r""" This method should be used to generate the tests and must 
        ...
        :rtype:
        """
        ERROR_HANDLER.assert_true(test_generation_guidance_obj is None, 'FIXME: Must first implement support for test gen guidance')
        ERROR_HANDLER.assert_true(parallel_testgen_count <= 1, 'FIXME: Must first implement support for parallel test gen')
        if exe_path_map is None:
            exe_path_map = self._get_default_exe_path_map()
        ERROR_HANDLER.assert_true(parallel_testgen_count > 0, 'invalid parallel test generation count: {}. {}'.format(parallel_testgen_count, 'must be >= 1'))
        if test_tool_type_list is None:
            candidate_tools_aliases = self.testcases_configured_tools.keys()
        else:
            ERROR_HANDLER.assert_true(len(test_tool_type_list) > 0, 'Invalid test_tool_type_list passed (empty)', __file__)
            candidate_tools_aliases = self.get_candidate_tools_aliases(test_tool_type_list)
            ...
            if meta_criteria_tool_obj is None:
                if ttool.requires_criteria_instrumented():
                    ...
                    if not ttool.requires_criteria_instrumented():
                        continue
                    if checkpoint_handler.is_to_execute(func_name=cp_func_name, taskid=cp_task_id, tool=ttoolalias):
                        ttool.generate_tests(exe_path_map, meta_criteria_tool_obj=meta_criteria_tool_obj, max_time=max_time)
                        checkpoint_handler.do_checkpoint(func_name=cp_func_name, taskid=cp_task_id, tool=ttoolalias)
    ...
    def _compute_testcases_info(self, candidate_tool_aliases=None):
        ...
        for ttoolalias in candidate_tool_aliases:
            ...
            meta_testcase_info_obj.update_using(ttoolalias, old2new_tests, tool_testcase_info)

        return meta_testcase_info_obj

    def get_testcase_info_object(self, candidate_tool_aliases=None):
        tc_info = TestcasesInfoObject()
        tc_info.load_from_file(self.get_testcase_info_file(candidate_tool_aliases))
        return tc_info

    def get_testcase_info_file(self, candidate_tool_aliases=None):
        self._compute_testcases_info(candidate_tool_aliases).write_to_file(self._unchecked_get_testcase_info_file())
    ...
    def _get_test_tool_checkpoint_files(self, test_toolname, top_checkpoint_dir=None):
        if top_checkpoint_dir is None:
            top_checkpoint_dir = self.checkpoints_dir
        return [os.path.join(top_checkpoint_dir, test_toolname + '_checkpoint.state' + suffix) for suffix in ('',
                                                                                                              '.backup')]

    def _get_checkpoint_files(self):
        top_checkpoint_dir = self.checkpoints_dir
        return [os.path.join(top_checkpoint_dir, 'checkpoint.state' + suffix) for suffix in ('',
                                                                                             '.backup')]
    ...
    def check_get_flakiness(self, meta_testcases, repeat_count=2):
        ...
        """
        ERROR_HANDLER.assert_true(repeat_count > 1, 'Cannot check flakiness with less than on repetition', __file__)
        if os.path.isdir(self.flakiness_workdir):
            shutil.rmtree(self.flakiness_workdir)
        os.mkdir(self.flakiness_workdir)
        outlog_files = [os.path.join(self.flakiness_workdir, str(of) + '-out.json') for of in range(repeat_count)]
        matrix_files = [os.path.join(self.flakiness_workdir, str(of) + '-mat.csv') for of in range(repeat_count)]

        def run(rep, test_list, hash_outlog):
            self.runtests(test_list, fault_test_execution_matrix_file=matrix_files[rep], fault_test_execution_execoutput_file=outlog_files[rep], with_output_summary=True, hash_outlog=hash_outlog)
        ...
        for i in range(1, repeat_count):
            other_outdata = common_matrices.OutputLogData(filename=outlog_files[i])
            other_outdata = list(other_outdata.get_zip_objective_and_data())[0][1]
            for test, t_dat_fix in fix_outdata.items():
                if test in flaky_tests:
                    pass
                else:
                    t_dat_other = other_outdata[test]
                    if not common_matrices.OutputLogData.outlogdata_equiv(t_dat_fix, t_dat_other):
        ...
        if len(flaky_tests) > 0:
            ...
            flaky_test_list_file = os.path.join(self.flakiness_workdir, 'flaky_test_list.json')
            logging.warning('There were some flaky tests (see file {})'.format(flaky_test_list_file))
            common_fs.dumpJSON(list(flaky_tests), flaky_test_list_file, pretty=True)
