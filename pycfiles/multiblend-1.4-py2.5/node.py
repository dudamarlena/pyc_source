...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/multiblend/node.py
# Compiled at: 2009-04-08 10:17:54
"""Represents a single rendering node."""
import os.path, random, threading, StringIO, time
try:
...
class Node(object):
        ...
        def thread():
            ...
            while self.rendering:
                ...
                if time.time() - start_time < 2:
                    self.log.error('Rendering takes too little time, something must be wrong. Aborted this node.')
                    self.rendering = False
    ...
    def fetch_saved_file(self, filename):
        ...
        if not os.WIFEXITED(result):
            msg = 'scp process did not finish properly while fetching %s' % filename
            self.log.critical(msg)
            raise RuntimeError(msg)
        status = os.WEXITSTATUS(result)
        if status > 0:
            msg = 'scp process exited with error status %i while fetching %s' % (
             status, filename)
            self.log.critical(msg)
            raise RuntimeError(msg)
    ...
    def send_blenderstarter(self):
        ...
        if self.config['scene']:
            ...
            scene = ''
        starter = starter % {'nice': self.config['nice'], 
           'scene': scene}
    ...
    def send_file(self, fileobj, remotename):
        ...
        if os.WCOREDUMP(result):
            ...
            raise RuntimeError(msg)
        if not os.WIFEXITED(result):
            msg = 'scp process did not finish properly while sending %s' % remotename
            self.log.critical(msg)
            raise RuntimeError(msg)
        status = os.WEXITSTATUS(result)
        if status > 0:
            msg = 'scp process exited with error status %i while sending %s' % (
             status, remotename)
            self.log.critical(msg)
            raise RuntimeError(msg)
        try:
