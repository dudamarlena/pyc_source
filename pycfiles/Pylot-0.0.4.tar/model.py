...
# [GCC 8.4.0]
# Embedded file name: /Users/mardochee.macxis/Projects/Python/flask-pilot/pylot/component/model.py
# Compiled at: 2015-05-26 23:23:05
import datetime
...
def user_struct(db):
    ...
    class User(db.Model):
        ...
        def get_by_token(cls, token):
            ...
            if user:
                ...
                if user.require_password_change is True and user.reset_password_token_expiration > now:
                    return user
        ...
        def set_reset_password_token(self, expiration=24):
            ...
            self.update(require_password_change=True, reset_password_token=token, reset_password_token_expiration=expiration)
            return token

        def clear_reset_password_token(self):
            self.update(require_password_change=False, reset_password_token=None, reset_password_token_expiration=None)
            return
        ...
        @classmethod
        def oauth_register(cls, provider, provider_user_id=None, email=None, name=None, image_url=None, **kwargs):
            """
            Register
            :param provider:
            ...
            """
            oal = UserOauthLogin
            oauthuser = oal.all().filter(oal.provider == provider).filter(oal.provider_user_id == provider_user_id).first()
            if oauthuser:
                ...
                if user:
                    ...
                    oal.create(**data)
                    return user
                user = cls.new(email=email, name=name, profile_pic_url=image_url, signin_method=provider)
                data.update({'user_id': user.id})
                oal.create(**data)
                return user

        def oauth_connect(self, provider, provider_user_id=None, email=None, name=None, image_url=None, **kwargs):
            """
    ...
    class UserRoleRole(db.Model):
        ...
        def add(cls, user_id, role_id):
            r = cls.all().filter(cls.user_id == user_id).filter(cls.role_id == role_id).first()
            if not r:
    ...
    return utils.to_struct(User=User, Role=UserRole, RoleRole=UserRoleRole, OauthLogin=UserOauthLogin)


def cloud_storage_struct(db):

    class CloudStorageObject(db.Model):
        parent_id = db.Column(db.Integer, index=True)
        user_id = db.Column(db.Integer, index=True)
        provider = db.Column(db.String(250))
        container = db.Column(db.String(250))
        local_path = db.Column(db.String(250))
        name = db.Column(db.String(250))
        description = db.Column(db.String(250))
        extension = db.Column(db.String(10), index=True)
        type = db.Column(db.String(250), index=True)
        size = db.Column(db.Integer, default=None)
        width = db.Column(db.Integer, default=None)
        object_hash = db.Column(db.String(250))
        object_url = db.Column(db.String(250))
        is_private = db.Column(db.Boolean, index=True, default=False)

    return utils.to_struct(StorageObject=CloudStorageObject)
...
def post_struct(UserStruct):
    ...
    class PostPostTag(db.Model):
        ...
        def add(cls, post_id, tag_id):
            c = cls.all().filter(cls.post_id == post_id).filter(cls.tag_id == tag_id).first()
            if not c:
    ...
    class Post(db.Model):
        ...
        tags = db.relationship(PostTag, secondary=PostPostTag.__table__.name)
        ...
        def get_published_by_category_slug(cls, slug):
            ...
            :return SQLA :
            """
            return cls.all().join(PostPostCategory).join(PostCategory).filter(PostCategory.slug.in_(slug)).filter(cls.is_published == True)
        ...
        def get_published_by_type_slug(cls, slug):
            ...
            :return SQLA :
            """
            return cls.all().join(PostType).filter(PostType.slug.in_(slug)).filter(cls.is_published == True)
        ...
        def publish(self, published_date=None):
            if self.is_draft:
                data = {'is_draft': False, 'is_published': True, 
    ...
    return utils.to_struct(Post=Post, Category=PostCategory, Type=PostType, PostCategory=PostPostCategory, Tag=PostTag, PostTag=PostPostTag, File=PostFile)