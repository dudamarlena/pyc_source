...
# [GCC 8.4.0]
# Embedded file name: /mnt/c/Users/steve/Documents/devel/XKCD-password-generator/tests/test_xkcdpass.py
# Compiled at: 2018-11-02 14:35:23
...
class XkcdPasswordTests(unittest.TestCase):
    ...
    def setUp(self):
        self.wordlist_full = xkcd_password.generate_wordlist(wordfile=WORDFILE, min_length=5, max_length=8)
        self.wordlist_small = xkcd_password.generate_wordlist(wordfile='tests/test_list.txt', valid_chars='[a-z]')
    ...
    def test_acrostic(self):
        word = 'face'
        result = xkcd_password.generate_xkcdpassword(self.wordlist_small, acrostic=word)
        self.assertEqual(('').join(map(lambda x: x[0], result.split())), word)

    def test_delim(self):
        tdelim = '_'
        result = xkcd_password.generate_xkcdpassword(self.wordlist_small, delimiter=tdelim)
    ...
    def test_set_case(self):
        ...
        results['first'] = xkcd_password.set_case(words, method='capitalize')
        ...
        observed_random_result_2 = xkcd_password.set_case(words_extra, method='random', testing=True)
...
class TestEmitPasswords(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.wordlist_small = xkcd_password.generate_wordlist(wordfile='tests/test_list.txt', valid_chars='[a-z]')
        self.options = argparse.Namespace(interactive=False, numwords=6, count=1, acrostic=False, delimiter=' ', separator='\n', case='lower')
    ...
    def test_emits_specified_count_of_passwords(self):
        ...
        with self.stdout_patcher as (mock_stdout):
            xkcd_password.emit_passwords(wordlist=self.wordlist_small, options=self.options)
...
if __name__ == '__main__':
    test_cases = [
     XkcdPasswordTests, TestEmitPasswords, TestEntropyInformation]
