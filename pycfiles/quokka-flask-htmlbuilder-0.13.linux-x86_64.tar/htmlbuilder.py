...
# [GCC 8.4.0]
# Embedded file name: /home/brocha/.virtualenvs/htmlbuilder/lib/python2.7/site-packages/flask_htmlbuilder/htmlbuilder.py
# Compiled at: 2016-06-26 14:26:55
"""
...
__all__ = [
 b'init_htmlbuilder', b'html', b'render', b'render_template', b'root_block',
 b'block', b'RootBlock', b'Block', b'Context', b'Attrs']
...
class BaseElement(object):
    ...
    def render(self, level):
        raise NotImplementedError(b'render() method has not been implemented')


class Element(BaseElement):
    __slots__ = [
     b'_name', b'_children', b'_attributes']
    ...
    def __call__(self, *children, **attributes):
        if attributes:
            self._attributes = collections.OrderedDict(sorted(attributes.items(), key=lambda a: a[0].endswith(b'_')))
        elif children:
    ...
    def __repr__(self):
        result = b'<' + type(self).__name__ + b' ' + self._name
    ...
    def render(self, level):
        result = _indent(level) + b'<' + self._name
        if self._attributes is not None:
            result += _serialize_attributes(self._attributes)
...
class Comment(BaseElement):
    ...
    def render(self, level):
        result = _indent(level) + b'<!--'
        if self._comment is not None:
            result += self._comment
        result += b'-->' + _new_line(level)
        return result
...
class Doctype(BaseElement):
    ...
    def render(self, level):
        return _indent(level) + b'<!doctype ' + self._doctype + b'>' + _new_line(level)
...
class Join(BaseElement):
    ...
    def render(self, level):
        return _indent(level) + _render_iteratable(self._children, None) + _new_line(level)
...
class HasAttr(BaseHasElement):
    ...
    Is equivalent to the following::
        ...
        def meta_description():
            return 'description' in html.keys()(
...
special_elements = {b'comment': Comment, 
   b'doctype': Doctype, 
   b'safe': Safe, 
   b'join': Join, 
   b'newline': NewLine, 
   b'has_block': HasBlock, 
   b'block': BlockElement, 
   b'has_attr': HasAttr}
...
def _render_iteratable(iteratable, level):
    ...
    return (b'').join([ render(element, level) for element in iteratable ])
...
def _serialize_attributes(attributes):
    ...
    for name, value in attributes.items():
        if not value:
            continue
        result += b' ' + _unmangle_attribute_name(name)
        result += b'="' + escape(value, True) + b'"'

    return result


_PYTHON_KEYWORD_MAP = dict((reserved + b'_', reserved) for reserved in kwlist)
...
def _unmangle_attribute_name(name):
    ...
    name = name.replace(b'__', b':').replace(b'_', b'-')
    return name
...
DEFAULT_TEMPLATE_NAME = b'default'
...
class Block(object):
    ...
    def __repr__(self):
        result = b'<' + self.__class__.__name__ + b' ' + self.block_func.__name__
        if self.endpoint is not None:
            result += b' => ' + repr(self.endpoint)
        result += b'>'
        return result
...
class Context(object):
    ...
    def __repr__(self):
        return b'<' + self.__class__.__name__ + b' ' + repr(self.name) + b'>'
