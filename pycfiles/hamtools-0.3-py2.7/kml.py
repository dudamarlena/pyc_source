...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hamtools/kml.py
# Compiled at: 2014-01-07 01:28:50
"""
...
class KML(object):
    ...
    def createPlacemark(self, name, lat=None, lon=None, desc=None, style=None, range=None, tilt=None, alt=None, heading=None, visible=True, altMode=None, timeStamp=None):
    ...
    def createLink(self, url, refreshMode=None, refreshInterval=None):
        ln = self.xml.createElement('Link')
        elt = self.xml.createElement('href')
        elt.appendChild(self.xml.createTextNode(url))
        ln.appendChild(elt)
        if refreshMode != None:
            if refreshMode not in ('onChange', 'onInterval', 'onExpire'):
    ...
    def createNetworkLink(self, url, name=None, desc=None, visible=True, isOpen=True, refreshMode=None, refreshInterval=None):
