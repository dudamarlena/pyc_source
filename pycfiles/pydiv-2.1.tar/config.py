...
# Embedded file name: build/bdist.linux-x86_64/egg/ditz/config.py
# Compiled at: 2016-03-07 14:40:46
__doc__ = '\nConfiguration file.\n'
import os, re, warnings, pkg_resources as pkg
...
class Config(object):
    ...
    def set_option(self, setting):
        m = re.match('(\\w+)\\.(\\w+)=(.*)', setting)
        if m:
            section, option, value = m.groups()
        else:
            raise ValueError("'%s': expected 'section.option=value'" % setting)
        try:
