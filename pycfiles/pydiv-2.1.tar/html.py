...
# Embedded file name: build/bdist.linux-x86_64/egg/ditz/plugins/html.py
...
class HTMLExporter(Exporter):
    ...
    def setup(self):
        ...
        def progressmeter(value, size=50):
            ...
            return "<span class='progress-meter'>" + "<span class='progress-meter-done'>" + '&nbsp;' * done + "</span><span class='progress-meter-undone'>" + '&nbsp;' * undone + '</span></span>'
    ...
    def write(self, dirname):
        ...
        events = sorted(self.db.issue_events, key=lambda x: x[0], reverse=True)
        count = self.config.getint('index_events')
        self.render(dirname, 'index.html', project=project, issues=self.db.issues, activity=events[:count])
        ...
        for rel in project.releases:
            ...
            self.render(dirname, 'release.html', targetfile=filename, project=project, release=rel, issues=issues, activity=relevents[:count], show_components=showcomp)
        ...
        for comp in project.components:
            ...
            self.render(dirname, 'component.html', targetfile=filename, project=project, component=comp, issues=issues)

        for issue in self.db.issues:
            filename = self.export_filename(issue)
            self.render(dirname, 'issue.html', targetfile=filename, project=project, issue=issue)

        issues = [ i for i in self.db.issues if not i.release ]
        issues = sorted(issues, key=lambda x: x.creation_time)
        issues = list(reversed(issues))
        self.render(dirname, 'unassigned.html', project=project, issues=issues, show_components=showcomp)