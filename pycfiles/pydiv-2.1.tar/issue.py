...
# Embedded file name: build/bdist.linux-x86_64/egg/ditz/command/issue.py
...
class CmdIssue(object):
    ...
    def do_add(self, arg):
        ...
        issue = self.db.add_issue(title, desc, type=issuetype, reporter=reporter, release=release, component=comp, comment=comment)
    ...
    def do_edit(self, arg):
        ...
        while True:
            ...
            if not editor.error:
                break
            reply = self.getyesno('%s\n\nRe-edit to fix problem?' % editor.error, True)
            if not reply:
                return
    ...
    def do_claim(self, arg):
        ...
        if issue.claimer:
            ...
            if not self.getyesno('Issue already claimed by %s.  Claim it anyway?' % issue.claimer):
                return
        comment = self.getcomment()
        self.db.claim_issue(issue, claimer=claimer, comment=comment, force=True)
    ...
    def do_unclaim(self, arg):
        ...
        if issue.claimer != claimer:
            if not self.getyesno('Issue can only be unclaimed by %s.  Unclaim it anyway?' % issue.claimer):
                return
        comment = self.getcomment()
        self.db.unclaim_issue(issue, claimer=claimer, comment=comment, force=True)
    ...
    def do_close(self, arg):
        ...
        disp = revdisp[choice]
        comment = self.getcomment()
        self.db.set_status(issue, flags.CLOSED, disposition=disp, comment=comment)
