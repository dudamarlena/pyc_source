...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/ks/smartimage/smartimageadapter/smartimagecached.py
# Compiled at: 2008-12-23 17:55:56
"""SiteUrl adapters for the Zope 3 based issue package

$Id: smartimagecached.py 12476 2007-10-26 21:06:39Z anton $
"""
__author__ = 'Andrey Orlov'
__license__ = 'ZPL'
__version__ = '$Revision: 12476 $'
__date__ = '$Date: 2007-10-27 00:06:39 +0300 (Sat, 27 Oct 2007) $'
...
class SmartImageCached(Contained):
    ...
    def tag(self, **kw):
        if self.getImage.getSize() > 0:
            return '<img src="%s" width="%s" height="%s" alt="%s" %s/>' % (self.url, self.width, self.height, self.title, (' ').join([ '%s="%s"' % (x, y) for (x, y) in kw.items() ]))
        return ''