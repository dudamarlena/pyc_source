...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/ks/smartimage/smartimageadapter/smartimageadapter.py
# Compiled at: 2008-12-23 17:55:56
"""SiteUrl adapters for the Zope 3 based issue package

$Id: smartimageadapter.py 35338 2008-06-12 18:42:18Z anatoly $
"""
...
class SmartImageViewGetBase(BrowserView):
    ...
    def __call__(self):
        try:
            ...
            if cache.iscached:
                self.request.response.setHeader('Expires', time.strftime('%a, %d %b %Y %H:%M:%S GMT', time.gmtime(time.time() + cache.interval)))
                self.request.response.setHeader('Last-Modified', time.strftime('%a, %d %b %Y %H:%M:%S GMT', time.gmtime(time.time() - cache.interval)))
            else:
                self.request.response.setHeader('Expires', time.strftime('%a, %d %b %Y %H:%M:%S GMT', time.gmtime(time.time())))
