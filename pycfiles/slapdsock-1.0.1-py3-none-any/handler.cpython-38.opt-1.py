...
# Compiled at: 2020-03-28 13:06:37
# Size of source mod 2**32: 14032 bytes
"""
    ...
    def log(self, logger):
        ...
        if logger.getEffectiveLevel() <= logging.DEBUG:
            for var_name, var_value in self.log_vars.items():
                logger.debug('%s=%s', var_name, repr(var_value))
            ...
            if self.peer_gid in self.server._allowed_gids:
                ...
                if uid != None:
                    try:
                        ...
                        self._log(logging.DEBUG, 'peer_pwd.pw_name = %s', repr(peer_pwd.pw_name))
        ...
        if self.server.logger.getEffectiveLevel() <= logging.DEBUG:
            ...
            for var_name in self.server._log_vars:
                if var_name in all_vars and var_name not in self._logged_vars:
                    ...
                    self._log(logging.DEBUG, '%s=%s', var_name, repr(all_vars[var_name]))
 ...
 ' '.join((self.log_prefix, args[0])), *(args[1:]), **kwargs)
    ...
    def do_monitor(self, request):
        ...
        if not self._check_access():
            error_message = 'Peer %d/%d not authorized to query monitor' % (
             self.peer_uid, self.peer_gid)
            self._log(logging.ERROR, error_message)
            return RESULTResponse((request.msgid),
              'insufficientAccessRights',
              info=error_message)
        monitor_entry = self.server.monitor_entry()
        self._log(logging.DEBUG, 'self.server.monitor_dn=%s', repr(self.server.monitor_dn))
        self._log(logging.DEBUG, 'monitor_entry=%s', repr(monitor_entry))
    ...
    def handle(self):
        ...
        try:
            ...
            self._log(logging.DEBUG, '----- incoming request via %s from pid=%s uid=%s gid=%s -----', repr(self.request.getsockname()), self.peer_pid, self.peer_uid, self.peer_gid)
            request_data = self.request.recv(500000)
            self._log(logging.DEBUG, 'request_data=%s', repr(request_data))
            ...
            self._log(logging.DEBUG, 'sock_req=%s // %s', repr(sock_req), sock_req.__dict__)
            ...
            try:
                try:
                    ...
                    self._log(logging.DEBUG, 'Request not cached: cache_key=%s', repr(cache_key))
                    ...
                    if cache_key:
                        if reqtype in self.server.req_cache:
                            ...
                            self._log(logging.DEBUG, 'Response stored in cache: cache_key=%s', repr(cache_key))
                else:
                    self._log(logging.DEBUG, 'Response retrieved from cache: cache_key=%s', repr(cache_key))
            ...
            try:
                ...
                self._log(logging.DEBUG, 'response_str=%s', repr(response_str))
                if response_str:
