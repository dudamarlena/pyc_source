...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.8/site-packages/hbmqtt/mqtt/publish.py
...
class PublishVariableHeader(MQTTVariableHeader):
    ...
    def __init__(self, topic_name, packet_id=None):
        super().__init__()
        if '*' in topic_name:
    ...
    def from_stream(cls, reader: asyncio.StreamReader, fixed_header: MQTTFixedHeader):
        ...
        has_qos = fixed_header.flags >> 1 & 3
        if has_qos:
...
class PublishPacket(MQTTPacket):
    ...
    def __init__(self, fixed=None, variable_header=None, payload=None):
        if fixed is None:
            header = MQTTFixedHeader(PUBLISH, 0)
        else:
