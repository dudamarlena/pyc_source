...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.8/site-packages/scripts/pub_script.py
...
"""
import sys, logging, asyncio, os, json
...
def _get_qos--- This code section failed: ---

 L.  60         0  SETUP_FINALLY        20  'to 20'

 L.  61         2  LOAD_GLOBAL              int
                4  LOAD_FAST                'arguments'
                6  LOAD_STR                 '--qos'
                8  BINARY_SUBSCR    
               10  LOAD_CONST               0
               12  BINARY_SUBSCR    
...
Parse error at or near `POP_EXCEPT' instruction at offset 26
...
def do_pub(client, arguments):
    ...
    try:
        ...
        (yield from client.connect(uri=(arguments['--url']), cleansession=(arguments['--clean-session']),
...
def main(*args, **kwargs):
    if sys.version_info[:2] < (3, 4):
        ...
        if arguments['--will-topic']:
            if arguments['--will-message'] and arguments['--will-qos']:
                config['will'] = dict()
