...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.8/site-packages/hbmqtt/mqtt/connect.py
# Compiled at: 2020-01-25 09:04:28
# Size of source mod 2**32: 10775 bytes
import asyncio
...
class ConnectVariableHeader(MQTTVariableHeader):
    ...
    WILL_FLAG = 4
    WILL_QOS_MASK = 24
    CLEAN_SESSION_FLAG = 2
    RESERVED_FLAG = 1

    def __init__(self, connect_flags=0, keep_alive=0, proto_name='MQTT', proto_level=4):
        super().__init__()
...
class ConnectPayload(MQTTPayload):
    __slots__ = ('client_id', 'will_topic', 'will_message', 'username', 'password',
                 'client_id_is_random')
    ...
    @classmethod
    @asyncio.coroutine
    def from_stream(cls, reader: ReaderAdapter, fixed_header: MQTTFixedHeader, variable_header: ConnectVariableHeader):
        payload = cls()
        try:
            ...
            if not payload.client_id is None:
            ...
            elif variable_header.password_flag:
                try:
...
class ConnectPacket(MQTTPacket):
    ...
    def __init__(self, fixed=None, vh=None, payload=None):
        if fixed is None:
            header = MQTTFixedHeader(CONNECT, 0)
        else:
