...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.8/site-packages/tests/mqtt/test_connect.py
# Compiled at: 2020-01-25 09:04:28
# Size of source mod 2**32: 6778 bytes
...
class ConnectPacketTest(unittest.TestCase):
    ...
    def test_decode_ok_will_flag(self):
        data = b'\x10&\x00\x04MQTT\x04\xca\x00\x00\x00\n0123456789\x00\x04user\x00\x08password'
        stream = BufferReader(data)
        message = self.loop.run_until_complete(ConnectPacket.from_stream(stream))
        self.assertEqual(message.variable_header.proto_name, 'MQTT')
        self.assertEqual(message.variable_header.proto_level, 4)
    ...
    def test_encode(self):
        ...
        variable_header = ConnectVariableHeader(206, 0, 'MQTT', 4)
        ...
        self.assertEqual(encoded, b'\x10>\x00\x04MQTT\x04\xce\x00\x00\x00\n0123456789\x00\tWillTopic\x00\x0bWillMessage\x00\x04user\x00\x08password')
