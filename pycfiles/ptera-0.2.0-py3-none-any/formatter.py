    ...
    def formatsection(self, tokens, data, scopes=[]):
        ...
        for token in tokens:
            ...
            if text:
                result.append(text)
            if token.marker == 'startsection':
                section = Section(name=token.field, tokens=[], data=data, scopes=scopes, conversion=token.conversion, format=token.spec)
