...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/simply/backends/docker.py
# Compiled at: 2016-09-29 12:16:42
import os
...
')
    ...
    def image_exist(self):
        return [
         self.image] == self.get_real_images()
...
def docker_exec(cmd, container, user=None, shell=False, daemon=False, raises=False, status_only=False, stdout_only=True):
    ...
    docker_cmd = ('docker exec -{}i {} {} {}').format('d' if daemon else '', ('-u {}').format(user) if user else '', container, cmd)
    dock = utils.Command(docker_cmd)
    if raises and dock.returncode:
        raise RuntimeError(('Error while executing <{}> on {}: [{}]').format(docker_cmd, container, dock.stderr.strip() or dock.returncode))
    if status_only:
