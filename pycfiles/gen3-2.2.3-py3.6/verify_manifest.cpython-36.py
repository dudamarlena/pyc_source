...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gen3/tools/indexing/verify_manifest.py
# Compiled at: 2020-03-14 11:10:26
# Size of source mod 2**32: 13500 bytes
"""
...
"""
import csv, glob, logging, fnmatch
from multiprocessing import Pool, Process, Manager, Queue
import multiprocessing, os, sys, shutil, math, time, urllib.parse
...
def verify_object_manifest(commons_url, manifest_file, num_processes=20, manifest_row_parsers=manifest_row_parsers, manifest_file_delimiter=',', log_output_filename=f"verify-manifest-errors-{time.time()}.log"):
    """
    ...
    _verify_all_index_records_in_file(commons_url, log_output_filename, num_processes, manifest_file, manifest_row_parsers, manifest_file_delimiter)
    end_time = time.time()
    logging.info(f"end time: {end_time}")
    logging.info(f"run time: {end_time - start_time}")


def _verify_all_index_records_in_file(commons_url, log_output_filename, num_processes, manifest_file, manifest_row_parsers, manifest_file_delimiter):
    """
    ...
    logging.info(f"done adding to queue, sending {num_processes} STOP messages b/c {num_processes} processes")
    for x in range(num_processes):
        queue.put('STOP')

    _start_processes_and_process_queue(queue, commons_url, num_processes, manifest_row_parsers)
    logging.info(f"done processing queue, combining outputs to single file {log_output_filename}")
...
def _start_processes_and_process_queue(queue, commons_url, num_processes, manifest_row_parsers):
    """
    ...
    processes = []
    for x in range(num_processes):
        p = Process(target=_verify_records_in_indexd,
          args=(
         queue, commons_url, manifest_row_parsers))
        p.start()
        processes.append(p)

    logging.info('waiting for processes to finish processing queue...')
        ...
        while row != 'STOP':
            ...
            try:
                ...
                if not actual_record:
                    raise Exception(f"Index client could not find record for GUID: {guid}")
            ...
            if md5 != actual_record['hashes'].get('md5'):
                if not md5:
                    if md5 != 0:
                        if not actual_record['hashes'].get('md5'):
                            if actual_record['hashes'].get('md5') != 0:
                                pass
                else:
            ...
            if sorted(urls) != sorted(actual_record['urls']):
                ...
                file.write(output)
                logging.error(output)
            if not actual_record['file_name']:
                if file_name:
