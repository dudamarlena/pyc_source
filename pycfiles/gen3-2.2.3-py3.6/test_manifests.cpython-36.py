...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/test_manifests.py
# Compiled at: 2020-03-14 11:10:26
# Size of source mod 2**32: 14128 bytes
import asyncio, os, glob, sys, shutil, logging
...
from gen3.tools.indexing.index_manifest import index_object_manifest, _get_and_verify_fileinfos_from_tsv_manifest
...
def test_verify_manifest(mock_index):
    ...
    verify_object_manifest('http://localhost',
    ...
    try:
        with open('test.log') as (file):
            for line in file:
                ...
                logs.setdefault(guid, {})[error] = {'expected':expected.split('expected ')[1], 
    ...
    if not 'dg.TEST/f2a39f98-6ae1-48a5-8d48-825a0c52a22b' not in logs:
        raise AssertionError
    else:
        if not 'dg.TEST/1e9d3103-cbe2-4c39-917c-b3abad4750d2' in logs:
            raise AssertionError
        else:
            if not 'dg.TEST/9c205cd7-c399-4503-9f49-5647188bde66' in logs:
                raise AssertionError
            else:
                if not '/programs/DEV/projects/test2' in logs['dg.TEST/1e9d3103-cbe2-4c39-917c-b3abad4750d2'].get('authz', {}).get('expected'):
                    raise AssertionError
                else:
                    assert 'DEV' in logs['dg.TEST/1e9d3103-cbe2-4c39-917c-b3abad4750d2'].get('acl', {}).get('expected')
                    assert '235' in logs['dg.TEST/1e9d3103-cbe2-4c39-917c-b3abad4750d2'].get('file_size', {}).get('expected')
                assert 'c1234567891234567890123456789012' in logs['dg.TEST/1e9d3103-cbe2-4c39-917c-b3abad4750d2'].get('md5', {}).get('expected')
            assert 'gs://test/test3.txt' in logs['dg.TEST/1e9d3103-cbe2-4c39-917c-b3abad4750d2'].get('urls', {}).get('expected')
...
def test_download_manifest(monkeypatch, gen3_index):
    ...
    """
    rec1 = gen3_index.create_record(did='dg.TEST/f2a39f98-6ae1-48a5-8d48-825a0c52a22b',
     ...
     '/programs/DEV/projects/test'],
      urls=[
     's3://testaws/aws/test.txt', 'gs://test/test.txt'])
    rec2 = gen3_index.create_record(did='dg.TEST/1e9d3103-cbe2-4c39-917c-b3abad4750d2',
      hashes={'md5': 'b1234567891234567890123456789012'},
      size=234,
      acl=[
     'DEV', 'test2'],
      authz=[
     '/programs/DEV/projects/test2', '/programs/DEV/projects/test2bak'],
      urls=[
     'gs://test/test.txt'],
      file_name='test.txt')
    rec3 = gen3_index.create_record(did='dg.TEST/ed8f4658-6acd-4f96-9dd8-3709890c959e',
      hashes={'md5': 'e1234567891234567890123456789012'},
      size=345,
      acl=[
     'DEV', 'test3'],
      authz=[
     '/programs/DEV/projects/test3', '/programs/DEV/projects/test3bak'],
      urls=[
     'gs://test/test3.txt'])
    monkeypatch.setattr(download_manifest, 'INDEXD_RECORD_PAGE_SIZE', 2)
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    loop.run_until_complete(async_download_object_manifest('http://localhost:8001',
    ...
    if not 'gs://test/test.txt' in records.get('dg.TEST/f2a39f98-6ae1-48a5-8d48-825a0c52a22b', {}).get('urls', []):
        raise AssertionError
    else:
        if not 's3://testaws/aws/test.txt' in records.get('dg.TEST/f2a39f98-6ae1-48a5-8d48-825a0c52a22b', {}).get('urls', []):
            raise AssertionError
        else:
            if not '/programs/DEV/projects/test' in records.get('dg.TEST/f2a39f98-6ae1-48a5-8d48-825a0c52a22b', {}).get('authz', []):
                raise AssertionError
            else:
                if not 'DEV' in records.get('dg.TEST/f2a39f98-6ae1-48a5-8d48-825a0c52a22b', {}).get('acl', []):
                    raise AssertionError
                else:
                    if not 'test' in records.get('dg.TEST/f2a39f98-6ae1-48a5-8d48-825a0c52a22b', {}).get('acl', []):
                        raise AssertionError
                    else:
                        assert '123' in records.get('dg.TEST/f2a39f98-6ae1-48a5-8d48-825a0c52a22b', {}).get('file_size')
                        assert 'a1234567891234567890123456789012' in records.get('dg.TEST/f2a39f98-6ae1-48a5-8d48-825a0c52a22b', {}).get('md5')
                    assert not records.get('dg.TEST/f2a39f98-6ae1-48a5-8d48-825a0c52a22b', {}).get('file_name')
        ...
        assert 'test.txt' == records.get('dg.TEST/1e9d3103-cbe2-4c39-917c-b3abad4750d2', {}).get('file_name')
...
def _mock_get_guid(guid, **kwargs):
    if guid == 'dg.TEST/f2a39f98-6ae1-48a5-8d48-825a0c52a22b':
        return {'acl':[
         ...
         'urls':[
          's3://testaws/aws/test.txt', 'gs://test/test.txt'], 
         'urls_metadata':{'gs://test/test.txt':{},  's3://testaws/aws/test.txt':{}},  'version':None}
    else:
        if guid == 'dg.TEST/1e9d3103-cbe2-4c39-917c-b3abad4750d2':
            return {'acl':['DEV', 'test2'],  'authz':[
...
def test_read_manifest():
    ...
    if not headers.index('GUID') >= 0:
        raise AssertionError
    else:
        if not headers.index('md5') >= 0:
            raise AssertionError
        else:
            if not headers.index('url') >= 0:
                raise AssertionError
            else:
...
def test_index_manifest(gen3_index, indexd_server):
    rec1 = gen3_index.create_record(did='255e396f-f1f8-11e9-9a07-0a80fada099c',
     ...
     'DEV', 'test'],
      ...
      urls=[
     's3://testaws/aws/test.txt', 'gs://test/test.txt'])
    index_object_manifest((indexd_server.baseurl),
      './test.tsv', 1, ('admin', 'admin'), replace_urls=False)
    ...
    if not set(rec1['urls']) == set([
     ...
     's3://pdcdatastore/test1.raw']):
        ...
        if not rec1['authz'] == []:
            ...
            if not rec2['hashes']['md5'] == '473d83400bc1bc9dc635e334fadde33c':
                raise AssertionError
            else:
                if not rec2['size'] == 363455714:
                    raise AssertionError
                else:
                    if not rec2['authz'] == ['/program/DEV/project/test']:
                        raise AssertionError
                    else:
                        if not rec2['urls'] == ['s3://pdcdatastore/test5.raw']:
                            raise AssertionError
                        else:
...
def test_index_manifest_with_replace_urls(gen3_index, indexd_server):
    rec1 = gen3_index.create_record(did='255e396f-f1f8-11e9-9a07-0a80fada099c',
     ...
     's3://testaws/aws/test.txt', 'gs://test/test.txt'])
    index_object_manifest((indexd_server.baseurl),
      './test.tsv', 1, ('admin', 'admin'), replace_urls=True)
    rec1 = gen3_index.get('255e396f-f1f8-11e9-9a07-0a80fada099c')
    assert rec1['urls'] == ['s3://pdcdatastore/test1.raw']


def test_index_non_guid_manifest(gen3_index, indexd_server):
    files, _ = index_object_manifest((indexd_server.baseurl),
      './test2.tsv', 1, ('admin', 'admin'), replace_urls=True)
    if not 'testprefix' in files[0]['GUID']:
        raise AssertionError
    else:
