...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gen3/submission.py
# Compiled at: 2020-03-14 11:10:26
# Size of source mod 2**32: 22935 bytes
...
class Gen3Submission:
    ...
    def export_record(self, program, project, uuid, fileformat, filename=None):
        ...
        api_url = '{}/api/v0/submission/{}/{}/export?ids={}&format={}'.format(self._endpoint, program, project, uuid, fileformat)
        ...
        if filename is None:
            ...
            self._Gen3Submission__export_file(filename, output)
            return output
    ...
    def export_node(self, program, project, node_type, fileformat, filename=None):
        ...
        """
        assert fileformat in ('json', 'tsv'), "File format must be either 'json' or 'tsv'"
        api_url = '{}/api/v0/submission/{}/{}/export/?node_label={}&format={}'.format(self._endpoint, program, project, node_type, fileformat)
        output = requests.get(api_url, auth=(self._auth_provider)).text
        if filename is None:
            if fileformat == 'json':
                output = json.loads(output)
    ...
    def get_dictionary_node(self, node_type):
        ...
        """
        api_url = '{}/api/v0/submission/_dictionary/{}'.format(self._endpoint, node_type)
        output = requests.get(api_url).text
        data = json.loads(output)
        return data
    ...
    def submit_file(self, project_id, filename, chunk_size=30, row_offset=0):
        ...
        df.rename(columns={c:c.lstrip('*') for c in df.columns},
          inplace=True)
        ...
        while start + len(chunk) <= len(df):
            ...
            if 'Request Timeout' in response or '413 Request Entity Too Large' in response or 'Connection aborted.' in response or 'service failure - try again later' in response:
            ...
            if 'code' not in json_res:
                ...
                if json_res['code'] == 200:
                    ...
                    if json_res['code'] == 400 or json_res['code'] == 403 or json_res['code'] == 404:
                        entities = json_res.get('entities', [])
                    ...
                    elif json_res['code'] == 500:
                        print('\t Internal Server Error: {}'.format(response))
                        results['responses'].append('Internal Server Error: {}'.format(response))
                    elif len(valid_but_failed) > 0 and len(invalid) > 0:
                        chunk = chunk.loc[df['submitter_id'].isin(valid_but_failed)]
