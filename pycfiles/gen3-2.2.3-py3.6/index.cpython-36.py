...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gen3/index.py
# Compiled at: 2020-03-14 11:10:26
# Size of source mod 2**32: 17611 bytes
import aiohttp, backoff, requests, urllib.parse, logging, sys, indexclient.client as client
...
def __log_backoff_retry(details):
    ...
    logging.warning(('backoff: call {func_call} delay {wait:0.1f} seconds after {tries} tries'.format)(func_call=func_call_log, **details))
...
def __log_backoff_giveup(details):
    ...
    kwargs_str = ', ' + _print_kwargs(details['kwargs']) if details.get('kwargs') else ''
    func_call_log = '{}({}{})'.format(_print_func_name(details['target']), args_str, kwargs_str)
    logging.error(('backoff: gave up call {func_call} after {tries} tries; exception: {exc}'.format)(func_call=func_call_log, 
     exc=sys.exc_info(), **details))


BACKOFF_SETTINGS = {'on_backoff':__log_backoff_retry, 
...
class Gen3Index:
    ...
    @(backoff.on_exception)((backoff.expo), Exception, **BACKOFF_SETTINGS)
    def create_record(self, hashes, size, did=None, urls=None, file_name=None, metadata=None, baseid=None, acl=None, urls_metadata=None, version=None, authz=None):
        """
        ...
        """
        rec = self.client.create(hashes, size, did, urls, file_name, metadata, baseid, acl, urls_metadata, version, authz)
        return rec.to_json()
    ...
    def create_blank(self, uploader, file_name=None):
        ...
        json = {'uploader':uploader, 
         'file_name':file_name}
        response = self.client._post('index/blank',
    ...
    @(backoff.on_exception)((backoff.expo), Exception, **BACKOFF_SETTINGS)
    def create_new_version(self, guid, hashes, size, did=None, urls=None, file_name=None, metadata=None, acl=None, urls_metadata=None, version=None, authz=None):
        """
        ...
        if urls is None:
            ...
            if did:
                json['did'] = did
            response = self.client._post('index',
              guid,
              headers={'content-type': 'application/json'},
              data=(client.json_dumps(json)),
              auth=(self.client.auth))
    ...
    def get_records(self, dids):
        ...
        """
        try:
            response = self.client._post('bulk/documents',
              json=dids, auth=(self.client.auth))
    ...
    def update_blank(self, guid, rev, hashes, size):
        ...
        response = self.client._put('index/blank/',
          guid,
          headers={'content-type': 'application/json'},
          params=p,
          auth=(self.client.auth),
    ...
    @(backoff.on_exception)((backoff.expo), Exception, **BACKOFF_SETTINGS)
    def update_record(self, guid, file_name=None, urls=None, version=None, metadata=None, acl=None, authz=None, urls_metadata=None):
        """
        ...
        """
        updatable_attrs = {'file_name':file_name, 
         'urls':urls, 
         'version':version, 
         'metadata':metadata, 
         'acl':acl, 
