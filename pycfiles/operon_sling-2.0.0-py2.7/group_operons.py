...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/sling/group_operons.py
# Compiled at: 2018-12-03 09:57:07
import utils, os, networkx as nx, csv, numpy as np, pandas
...
class GroupHits():
    ...
    def __init__(self, order, filter_id, group_id, out_dir='.', min_identity=30, save_to_ITOL=False, sep=',', report_unfit=True):
        ...
        for file in os.listdir(results_dir):
            ...
            with open(os.path.join(results_dir, file)) as (f):
                ...
                for line in f:
                    ...
                    if dir_name == '':
                        self.hits[ID] = Hit(ID, results_table[line_num - 1:line_num])
                    else:
            ...
            if network_type == 'hits':
                ...
                for n in curr_nodes:
                    ...
                    if network_type == 'unfit':
                        ...
                        reason2 = str(curr_hit.toks['Reason2'].tolist()[0])
                        if not reason2 == 'nan':
                            reasons.add(reason2)
    ...
    def _write_graph_to_matrix(self, G, network_type, unfit_dict={}):
        ...
        binary = [
         [
          'Strain'] + map(str, range(1, 1 + len(components)))]
            ...
            if network_type == 'hits':
                ...
                for n in curr_nodes:
                    ...
                    if network_type == 'unfit':
                        ...
                        hits = self.hits
                    curr_hit = hits[hit_ID]
                    curr_hit.clusters[hit_type] = label
                    if network_type == 'unfit' and n in unfit_dict:
    ...
    def _read_blast_to_network(self, network_type):
        ...
        with open(results) as (f):
            for line in f:
                ...
                score = float(line[2])
                if score >= self.min_identity and node_type_1 != 'unfit' and node_type_2 != 'unfit':
                    G.add_edge(id1, id2)
