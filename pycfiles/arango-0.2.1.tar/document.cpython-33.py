...
# [GCC 8.4.0]
# Embedded file name: /Users/maxk/Projects/OpenSource/arango-python/arango/document.py
# Compiled at: 2013-12-07 05:02:27
# Size of source mod 2**32: 12984 bytes
import logging
from .mixins import ComparsionMixin, LazyLoadMixin
from .exceptions import DocumentAlreadyCreated, DocumentIncompatibleDataType, DocumentNotFound, DocuemntUpdateError
...
class Documents(object):
    __doc__ = 'Proxy object to handle work with\n    documents within collection instance\n    '
    ...
    def count(self):
        ...
        """
        cursor = self.connection.query('FOR d IN {} RETURN d'.format(self.collection.cid), count=True)
        return len(cursor)

    def _cursor(self, rs):
        return self.connection.query('FOR d IN {} {} RETURN d'.format(self.collection.cid, self._limits(rs)), count=True)
    ...
    def _limits(self, rs):
        ...
        if rs._limit:
            ...
            if rs._offset:
                limit = 'LIMIT {}, {}'.format(rs._offset, rs._limit)
        return limit
    ...
    def create(self, *args, **kwargs):
        ...
        """
        doc = Document(collection=self.collection, connection=self.connection)
    ...
    def create_bulk(self, docs, batch=100):
        ...
        if not docs:
            return False
        qs_args = {'createCollection': 'true', 
         'collection': self.collection.cid}
        if not isinstance(docs[0], (list, tuple)):
            qs_args['type'] = 'documents'
        response = self.connection.post(self.connection.qs(self.BULK_INSERT_PATH, **qs_args), data='\n'.join(json.dumps(doc) for doc in docs), ignore_request_args=True)
    ...
    def delete(self, ref_or_document):
        ...
        """
        doc = Document(collection=self.collection, connection=self.connection, id=proxied_document_ref(ref_or_document))
        return doc.delete()
...
class Document(ComparsionMixin, LazyLoadMixin):
    __doc__ = 'Particular instance of Document'
    ...
    def __init__(self, collection=None, id=None, rev=None, resource_url=None, connection=None):
    ...
    def wrap(cls, connection, item):
        doc = cls(connection=connection, id=item.get('_id'), rev=item.get('_rev'))
        doc._body = item
    ...
    def load(cls, connection, meta=None, id=None):
        ...
        response = connection.get(cls.READ_DOCUMENT_PATH.format(id), _expect_raw=True)
        if response.status != 200:
            raise DocumentNotFound('Sorry, document with handle `{0}` not exist in database'.format(id))
    ...
    def __repr__(self):
        self._handle_lazy()
        return '<ArangoDB Document: Reference {0}, Rev: {1}>'.format(self._id, self._rev)
    ...
    def get(self, name=None, default=None):
        ...
        *(fallback)* value ``0``
        ...
        if not self._body:
            ...
            if isinstance(self._body, (list, tuple)) and isinstance(name, int):
    ...
    def create(self, body, createCollection=False, **kwargs):
        ...
        if self._id is not None:
            raise DocumentAlreadyCreated('This document already created with id {0}'.format(self.id))
        params = {'collection': self.collection.cid}
        if createCollection is True:
            params.update({'createCollection': True})
        params.update(kwargs)
        response = self.connection.post(self.connection.qs(self.DOCUMENT_PATH, **params), data=body)
    ...
    def update(self, newData, save=True, **kwargs):
        ...
        """
        if issubclass(type(self._body), dict) and issubclass(type(newData), dict):
            self._body.update(newData)
        else:
            if issubclass(type(self._body), list) and issubclass(type(newData), list):
                self._body.extend(newData)
            else:
                raise DocumentIncompatibleDataType('You trying to update document `{0}` with incompatible datat type {1}'.format(self.id, repr(newData)))
        if save is True:
    ...
    def save(self, **kwargs):
        ...
        arguments.
        """
        response = self.connection.put(self.UPDATE_DOCUMENT_PATH.format(self.id), data=self.body, **kwargs)
        if response.status in (200, 201, 202):
            self._rev = response.data.get('_rev')
            return self
        raise DocuemntUpdateError(response.get('errorMessage', 'Unknown error'))
    ...
    def delete(self):
        ...
        """
        response = self.connection.delete(self.DELETE_DOCUMENT_PATH.format(self.id))
