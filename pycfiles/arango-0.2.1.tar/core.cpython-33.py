...
# [GCC 8.4.0]
# Embedded file name: /Users/maxk/Projects/OpenSource/arango-python/arango/core.py
# Compiled at: 2013-12-06 03:31:37
...
class ArangoVersion(object):
    ...
    def __repr__(self):
        return '<{0} {1}>'.format(self.server.title(), self.version)


class Connection(object):
    __doc__ = 'Connetion to ArangoDB\n    '
    VERSION_PATH = '/_api/version'
    _prefix = 'http://'
    _pass_args = ('get', 'put', 'post', 'delete')

    def __init__(self, host='localhost', port=8529, is_https=False, client=None, db=None, **kwargs):
        """
    ...
    def __getattr__(self, name):
        ...
        raise AttributeError("{cls} object has no attribute '{attr}'".format(cls=self.__class__, attr=name))
    ...
    def requests_factory(self, method='get'):
        ...
        def requests_factory_wrapper(path, **kwargs):
            ...
            logger.debug("'{method}' request to '{url}'".format(method=method, url=url))
            ...
            if 'data' in kw:
                if isinstance(kw.get('data'), (dict, list)) and not kw.pop('rawData', False):
                    kw['data'] = json.dumps(kw.get('data'))
            return Response(url, req(url, **kw), args=kw if ignore_request_args is False else None, expect_raw=expect_raw)
    ...
    def url(self, db_prefix=True):
        ...
        if self.is_https:
            self._prefix = 'https://'
        return '{prefix}{host}:{port}{db_prefix}'.format(prefix=self.prefix, host=self.host, port=self.port, db_prefix=self.database.prefix if db_prefix else '')
    ...
    def version(self):
        ...
        ArangoDB Server.
        """
        data = self.get(self.qs(self.VERSION_PATH, details='true'), ignore_request_args=True).data
    ...
    def __init__(self, url, response, args=None, expect_raw=False):
        ...
        try:
            if expect_raw is False:
                self.update({k:v for k, v in json.loads(response.text).items()})
        except (TypeError, ValueError) as e:
            msg = "Can't parse response from ArangoDB: {0} (URL: {1}, Response: {2})".format(str(e), url, repr(response))
            logger.error(msg)
            self.status = 500
            self.message = msg
...
class Resultset(object):
    ...
    def _prepare(self):
        """Prepare data"""
        if not self.data:
            if hasattr(self.base, 'prepare_resultset'):
                self.base.prepare_resultset(self, args=self._args, kwargs=self._kwargs)
    ...
    def __repr__(self):
        ...
        return '<Resultset: {0}{1}>'.format(', '.join(items), suff)
...
class RequestChunk(object):
    ...
    def __init__(self, url, body, method=None, headers=None, boundary=None, part_num=1):
    ...
    def build(self):
        self.headers.append(('Content-Type', 'application/x-arango-batchpart'), (
         'Content-Id', self.part_num))
        headers = self.CRLF.join('{0}: {1}'.format(name, value) for name, value in self.headers)
        request = '{headers}{crlf}{crlf}{method} {url} HTTP/1.1{crlf}{crlf}{body}{crlf}'
        return request.format(crlf=self.CRLF, headers=headers, url=self.url, method=self.method, body=self.body)