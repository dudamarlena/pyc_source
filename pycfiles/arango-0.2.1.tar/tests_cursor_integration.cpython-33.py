...
# [GCC 8.4.0]
# Embedded file name: /Users/maxk/Projects/OpenSource/arango-python/arango/tests/tests_cursor_integration.py
...
class TestsCursor(TestsIntegration):
    ...
    def setUp(self):
        ...
        self.cursor = lambda *args**args: Cursor(self.conn, *args, **kwargs)
    ...
    def test_bind(self):
        [self.c.documents.create({'num': n}) for n in range(10)]
        cursor = self.cursor('FOR d IN test FILTER d.num == @num RETURN d', count=True)
        cursor.bind({'num': 2})
...
class TestQueries(TestsIntegration):
    ...
    def test_query(self):
        ...
        assert_equal(total, 2, 'One of docs are not added to database')