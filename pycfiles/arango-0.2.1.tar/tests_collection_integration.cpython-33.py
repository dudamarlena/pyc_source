...
# [GCC 8.4.0]
# Embedded file name: /Users/maxk/Projects/OpenSource/arango-python/arango/tests/tests_collection_integration.py
# Compiled at: 2013-12-14 12:04:39
# Size of source mod 2**32: 5670 bytes
import logging
...
class TestsCollection(TestsIntegration):
    ...
    def test_proxy_repr(self):
        ...
        assert_equal(repr(self.conn.collection.test.documents), '<ArangoDB Documents Proxy Object>')
    ...
    def test_limit_documents(self):
        ...
        c.test.docs.create({'doc': 1})
        c.test.docs.create({'doc': 2})
        assert_not_equal(self.conn.collection.test.documents().count, 1)
        assert_equal(len(self.conn.collection.test.documents()), self.conn.collection.test.documents().count)
        assert_equal(len(self.conn.collection.test.documents().limit(1)), 1)
        assert_equal(len(self.conn.collection.test.documents().offset(2).limit(1)), 0)