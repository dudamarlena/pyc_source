...
# [GCC 8.4.0]
# Embedded file name: /Users/maxk/Projects/OpenSource/arango-python/arango/index.py
...
class Index(object):
    __doc__ = 'Interface to work with Indexes'
    ...
    def __call__(self):
        ...
        """
        response = self.connection.get(self.INDEXES.format(self.collection.cid))
    ...
    def create(self, fields, index_type='hash', unique=False):
        ...
        if index_type not in self.INDEX_TYPES:
            raise WrongIndexType('The type you provided `{0}` is undefined. Possible values are: {1}'.format(repr(type), ', '.join(self.INDEX_TYPES)))
        if len(fields) == 0:
            raise EmptyFields("It's required to provide at least one field to index")
        response = self.connection.post(self.CREATE.format(self.collection.cid), data={'fields': fields, 
         'type': index_type, 
         'unique': unique})
    ...
    def delete(self, field_id):
        """Return tuple of two values:
            ...
            - original response
        """
        response = self.connection.delete(self.DELETE.format(self.collection.cid, field_id))
    ...
    def get(self, field_id, force_read=False):
        ...
        response = self.connection.get(self.READ.format(self.collection.cid, field_id))
