...
# [GCC 8.4.0]
# Embedded file name: /Users/maxk/Projects/OpenSource/arango-python/arango/collection.py
# Compiled at: 2013-12-14 11:58:58
# Size of source mod 2**32: 10696 bytes
import logging
from .document import Documents
from .edge import Edges
from .index import Index
from .exceptions import InvalidCollectionId, CollectionIdAlreadyExist, InvalidCollection
from .aql import AQLQuery
logger = logging.getLogger(__name__)
__all__ = ('Collection', 'Collections')

class Collections(object):
    __doc__ = 'connection) for Collections'
    ...
    def _collection(self, name):
        ...
        self.collections[name] = self.collections.get(name, Collection(connection=self.connection, name=name))
        return self.collections.get(name)
    ...
    def rename_collection(self, collection, new_name):
        ...
        instance itself.
        """
        if collection is None or not issubclass(collection.__class__, Collection):
            raise InvalidCollection("Object '{0}' is not subclass of Collection or is None".format(repr(collection)))
        if new_name in self.collections:
            raise CollectionIdAlreadyExist("Collection with name '{0}' already exist".format(new_name))
        if collection.cid not in self.collections:
...
class Collection(object):
    __doc__ = 'Represent single collection with certain name'
    ...
    INFO_ALLOWED_RESOURCES = [
     'count', 'figures']

    def __init__(self, connection=None, name=None, id=None, createCollection=True, response=None):
        self.connection = connection
        self.name = name
        self.id = id
    ...
    def query(self):
        ...
        """
        return AQLQuery(connection=self.connection, collection=self.cid)
    ...
    def info(self, resource=''):
        ...
        if resource not in self.INFO_ALLOWED_RESOURCES:
            resource = ''
        return self.connection.get(self.COLLECTION_DETAILS_PATH.format(self.name, resource)).data
    ...
    def create(self, waitForSync=False, type=Collections.COLLECTION_DOCUMENTS, **kwargs):
        """
        Create new **Collection**. You can specify
        ``waitForSync`` argument (boolean) to wait until
        collection will be synced to disk
        """
        params = {'waitForSync': waitForSync,  'name': self.name, 
         'type': type}
        params.update(kwargs)
        response = self.connection.post(self.CREATE_COLLECTION_PATH, data=params)
    ...
    def load(self):
        ...
        """
        return self.connection.put(self.LOAD_COLLECTION_PATH.format(self.name))

    def unload(self):
        """
        Unload collection from memory
        """
        return self.connection.put(self.UNLOAD_COLLECTION_PATH.format(self.name))

    def delete(self):
        """
        Delete collection
        """
        response = self.connection.delete(self.DELETE_COLLECTION_PATH.format(self.name))
        if response.status == 200:
            return True
        return False
    ...
    def rename(self, name=None):
        ...
        if name is None or name == '':
            raise InvalidCollectionId('Please, provide correct collection name')
        response = self.connection.post(self.RENAME_COLLECTION_PATH.format(self.name), data={'name': name})
    ...
    def truncate(self):
        ...
        """
        return self.connection.put(self.TRUNCATE_COLLECTION_PATH.format(self.name))