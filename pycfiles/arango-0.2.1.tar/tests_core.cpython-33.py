...
# [GCC 8.4.0]
# Embedded file name: /Users/maxk/Projects/OpenSource/arango-python/arango/tests/tests_core.py
...
class TestConnectionInit(TestsBase):
    ...
    def test_repr(self):
        conn = Connection()
        assert_equal(str(conn), '<Connection to ArangoDB (http://localhost:8529)>')
...
class TestConnectionRequestsFactory(TestsBase):
    ...
    def test_http_methods_factory(self):
        ...
        for method in self.methods:
            assert_equal(getattr(conn, method)('/'), conn.requests_factory(method=method)('/'))

    def test_http_methods_execution(self):
        conn = Connection()
        url = '{0}{1}'.format(conn.url, '/')
        for method in self.methods:
            assert_equal(getattr(conn, method)('/'), Response(url, getattr(Client, method)(url)))
...
class TestResponse(TestsBase):
    ...
    def test_unparseable_response(self):
        ...
        assert_true("Can't parse response from ArangoDB: " in response.message)

    def test_repr(self):
        response = self.response()
        assert_equal(str(response), '<Response for {0}: {1}>'.format(repr(response.__dict__), self.url))

    def test_parse_response(self):
        response = self.response(status=200, text='{"status": 200, "message": "test", "value": 1}')
        assert_false(response.is_error)
        assert_equal(response.status, 200)
        assert_equal(response.get('message'), 'test')
        assert_equal(response.get('value'), 1)
...
class TestResultset(TestsBase):
    ...
    def setUp(self):
        ...
        self.rs.base._cursor = lambda *a**a: list(range(3))
        return
    ...
    def test_repr(self):
        assert_equal(str(self.rs), '<Resultset: {0}>'.format(', '.join([str(i) for i in self.data])))
    ...
    def test_repr_large_resultset(self):
        ...
        assert_equal(str(Resultset(base=custom_base)), '<Resultset: {0}... more>'.format(', '.join([str(i) for i in dataset[:self.rs.max_repr_items + 1]])))