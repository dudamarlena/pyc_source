...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/collective/multisitepanel/browser/msp_products.py
# Compiled at: 2010-08-23 11:43:39
...
class ProductsPanelView(BrowserView, MultisiteBase):
    ...
    def __call__(self):
        if self.request.form.has_key('reinstall_pid') and self.request.form.has_key('siteid'):
            if self.request.form['siteid'] == 'allSites':
                ...
                siteApps = [
                 self.context.restrictedTraverse('/' + self.request.form['siteid'])]
    ...
    def pypiSearch(self, pid):
        ...
        if pid is None:
            return pid
        return dict(label=_('Search on Pypy'), url='http://pypi.python.org/pypi?%3Aaction=search&term=' + self.productCompleteId(pid) + '&submit=search')
    ...
    def countInstallation(self, pid):
        tot = 0
        for (siteId, siteApp) in self.sitesList:
            isInstalled = self.multisite_productInstalledVersion(siteApp, pid)
            if isInstalled:
                tot = tot + 1
    ...
    def multisite_reinstallProducts(self, siteApps, product):
        ...
        message = _('feedback', default=_('Reinstalled ${product}'), mapping={'product': product})
