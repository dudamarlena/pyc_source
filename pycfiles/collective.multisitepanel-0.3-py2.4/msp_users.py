...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/collective/multisitepanel/browser/msp_users.py
...
class UsersPanelView(BrowserView):
    ...
    def __call__(self):
        ...
        self.searchstring = form.get('searchstring', None)
        self.submitted = form.get('submitted', '')
        self.userid = form.get('userid', '')
        if not self.submitted:
            return self.template()
        elif not self.searchstring:
            message = _('feedback', default=_('No user found'))
            self.messages.addStatusMessage(message, type='info')
            return self.template()
        self.distinctUsers = self.multisite_searchUser(self.searchstring)
        if len(self.distinctUsers) == 0:
            message = _('feedback', default=_('No user found'))
            self.messages.addStatusMessage(message, type='info')
        elif len(self.distinctUsers) == 1:
            self.userid = self.distinctUsers[0][0].getId()
        return self.template()
    ...
    def multisite_searchUser(self, searchstring, this_site='all'):
        ...
        return [ (distinctUsers[key][0], distinctUsers[key][1]) for key in distinctUsers ]
