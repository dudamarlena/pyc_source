...
# [GCC 8.4.0]
# Embedded file name: /Users/mdiazmel/code/aramis/clinica/clinica/iotools/converters/nifd_to_bids/utils/parse_clinical.py
# Compiled at: 2019-10-10 04:46:11
# Size of source mod 2**32: 15305 bytes
...
class Parse_clinical:
    __doc__ = 'Creates the clinical files for the BIDS directory'
    ...
    def merge_clinical_scans(self, pat_name):
        ...
        bloc_ida['examination_date'] = bloc_ida['examination_date'].apply(lambda x: '0' + str(x).split(' ')[0] if str(x).split(' ')[0][0] != '0' else str(x).split(' ')[0])
    ...
    def get_clinical_ida(self):
        ...
        dfSol = pd.merge(df_clinical, df_ida, how='left', left_on=['LONI_ID', 'CLINICAL_LINKDATE'], right_on=[
         'Subject ID', 'Study Date'])
        dfSol.insert(0, 'session_id', '')
        dfSol['session_id'] = dfSol['Session_number'].apply(--- This code section failed: ---

 L. 151         0  LOAD_DEREF               'math'
                2  LOAD_ATTR                isnan
                4  LOAD_FAST                'x'
...
Parse error at or near `None' instruction at offset -1
    ...
    def make_scans(self, path_scans):
        ...
        for sub in subs:
            ...
            name = [i for i in name if i != '.DS_Store' if i.endswith('.nii.gz') or i.endswith('.nii')]
            for n in name:
                s += sub + '/' + n + '\n'
        ...
        def make_dic_tuples(to_convert, root):
            ...
            for tuple in to_convert:
                ...
                if s_path1[(-4)] not in dic_pat_sess:
                    dic_pat_sess[s_path1[(-4)]] = [
                     s_path1[(-3)]]
                else:
                    if s_path1[(-3)] not in dic_pat_sess[s_path1[(-4)]]:
        ...
        for sub_id in dic_pat_sess:
            for ses_num in dic_pat_sess[sub_id]:
                ...
                for tuple in dic_tuples[os.path.join(root, sub_id, ses_num)]:
                    ...
                    df_line_ida = self.df_ida[((self.df_ida['Subject ID'] == s_path1[(-1)][8] + '_S_' + s_path1[(-1)][10:14]) & (self.df_ida['Visit'] == 'Month ' + str(int(ses_num.split('M')[(-1)]))) & (self.df_ida['Description'] == s_path0[(-3)]))]
                    if df_line_ida.empty:
                        df_line_ida = self.df_ida[((self.df_ida['Subject ID'] == s_path1[(-1)][8] + '_S_' + s_path1[(-1)][10:14]) & (self.df_ida['Visit'] == 'Month ' + str(int(ses_num.split('M')[(-1)]))) & (self.df_ida['Description'] == s_path0[(-3)].replace('_', ' ')))]
                    if df_line_ida.empty:
                        ...
                        df_line_ida = self.df_ida[((self.df_ida['Subject ID'] == s_path1[(-1)][8] + '_S_' + s_path1[(-1)][10:14]) & (self.df_ida['Visit'] == 'Month ' + str(int(ses_num.split('M')[(-1)]))) & (self.df_ida['Description'] == des))]
