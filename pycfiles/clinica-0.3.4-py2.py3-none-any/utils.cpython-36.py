...
# [GCC 8.4.0]
# Embedded file name: /Users/mdiazmel/code/aramis/clinica/clinica/lib/nipype/interfaces/mrtrix3/utils.py
...
class TensorMetricsInputSpec(CommandLineInputSpec):
    ...
    component = traits.List([
     1],
      ...
      argstr='-num %s',
      sep=',',
      desc='specify the desired eigenvalue/eigenvector(s). Note that several eigenvalues can be specified as a number sequence')
    in_mask = File(exists=True,
      argstr='-mask %s',
      desc='only perform computation within the specified binary brain mask image')
    modulate = traits.Enum('FA',
      'none',
      'eval',
      argstr='-modulate %s',
