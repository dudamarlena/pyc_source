...
# [GCC 8.4.0]
# Embedded file name: /Users/mdiazmel/code/aramis/clinica/clinica/iotools/converters/nifd_to_bids/utils/patient.py
...
class Patient(object):
    __doc__ = 'Class that handles sessions ordering, image quality hierarchy and final BIDS structure'
    ...
    def order_sessions(self, equivalences, descriptors, folders):
        ...
        same_dates = [(self.get_sesName(date),
         [fold for fold in folders if date in fold]) for date in dates]
        ...
        for ses_id in sessions:
            ...
            for path in sessions[ses_id]:
                ...
                if final_name not in sol[ses_id][dataType][priority]:
                    sol[ses_id][dataType][priority][final_name] = [
                     path]
                else:
                    if path not in sol[ses_id][dataType][priority][final_name]:
                            ...
                            if name not in name_encountered:
                                ...
                                if len(ordered_bids[ses][dataType][highest_priority][name2]) > 1:
                                    if name not in conflicts:
                                        conflicts[name] = [
                                         ordered_bids[ses][dataType][highest_priority][name2]]
                                    else:
