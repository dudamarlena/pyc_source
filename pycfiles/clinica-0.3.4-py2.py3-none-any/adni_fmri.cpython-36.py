...
# [GCC 8.4.0]
# Embedded file name: /Users/mdiazmel/code/aramis/clinica/clinica/iotools/converters/adni_to_bids/adni_modalities/adni_fmri.py
# Compiled at: 2019-10-10 04:46:11
# Size of source mod 2**32: 11904 bytes
"""
...
def compute_fmri_path(source_dir, csv_dir, dest_dir, subjs_list):
    ...
    for conv_error in conversion_errors:
        error_indices.append((fmri_df.Subject_ID == conv_error[0]) & (fmri_df.VISCODE == conv_error[1]))
...
def fmri_paths_to_bids(dest_dir, fmri_paths, mod_to_update=False):
    ...
    counter = Value('i', 0)
    partial_generate_subject_files = partial(generate_subject_files, fmri_paths=fmri_paths,
      dest_dir=dest_dir,
...
def generate_subject_files(subj, fmri_paths, dest_dir, mod_to_update):
    import clinica.iotools.converters.adni_to_bids.adni_utils as adni_utils, clinica.iotools.bids_utils as bids_utils
    from clinica.utils.stream import cprint
    import os
    from os import path
    ...
    for ses in sess_list:
        with counter.get_lock():
            counter.value += 1
        cprint('[fMRI] Processing subject ' + str(subj) + ' - session ' + ses + ', ' + str(counter.value) + ' / ' + str(len(fmri_paths)))
        ...
        if not fmri_info['Path'].values[0] == '':
            if type(fmri_info['Path'].values[0]) != float:
                if not os.path.exists(path.join(ses_path, 'func')):
                    ...
                    dcm_to_convert = adni_utils.check_two_dcm_folder(fmri_path, dest_dir, fmri_info['Image_ID'].values[0])
    ...
    for r in fmri_paths.iterrows():
        ...
        if not files_list:
            ...
            if len(files_list) != 2:
                ...
                if sum([not f.endswith(('_task-rest_bold.json', '_task-rest_bold.nii.gz')) for f in files_list]) > 0:
                    name_wrong += 1
                    print(files_list)
