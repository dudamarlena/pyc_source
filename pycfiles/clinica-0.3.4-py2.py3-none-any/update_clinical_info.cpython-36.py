...
# [GCC 8.4.0]
# Embedded file name: /Users/mdiazmel/code/aramis/clinica/clinica/iotools/converters/nifd_to_bids/preprocessing/update_clinical_info.py
...
def update_info_clinical(path_data_dict, path_clinicals, path_to_clinical, path_preprocessing):
    ...
    dfParticipant = pd.read_csv((os.path.join(path_clinicals, 'clinical_data_bids_correspondence_participant.tsv')), sep='\t')
    dfSessions = pd.read_csv((os.path.join(path_clinicals, 'clinical_data_bids_correspondence_sessions.tsv')),
      sep='\t')
