...
# [GCC 8.4.0]
# Embedded file name: /Users/mdiazmel/code/aramis/clinica/clinica/iotools/converter_utils.py
...
def check_bin(bin_name):
    ...
    if returncode == 127:
        cprint(Fore.RED + bin_name + ' not found in your system. ' + 'Have you installed the corresponding software ?' + Fore.RESET)
        res = 1
    else:
        res = 0
    return res
...
class MissingModsTracker:
    ...
    def __init__(self, ses, mod_list=False):
        ...
        if mod_list:
            ...
            for s in ses:
                self.missing.update({s: {'session':0,  'dwi':0, 
                     'func':0, 
                     'fieldmap':0, 
                     'flair':0, 
                     't1w':0}})
