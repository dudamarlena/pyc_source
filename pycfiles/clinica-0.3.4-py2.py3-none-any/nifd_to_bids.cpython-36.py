...
# [GCC 8.4.0]
# Embedded file name: /Users/mdiazmel/code/aramis/clinica/clinica/iotools/converters/nifd_to_bids/nifd_to_bids.py
# Compiled at: 2019-10-10 04:46:11
# Size of source mod 2**32: 6739 bytes
"""
...
def convert_images(path_to_dataset, bids_dir, path_to_clinical):
    ...
    import os
    from clinica.iotools.converters.nifd_to_bids.utils.conv_image_folders import get_all_med_name, dict_conversion, get_descriptors
    from clinica.iotools.converters.nifd_to_bids.nifd_utils import get_patients_source_files, filter_patients_source_files, collect_conversion_tuples, convert
    ...
    if not os.path.isfile(path_to_ida):
        if os.path.isfile(path_idaSearch):
            cprint('ida.tsv was not found in the clinical data directory, ida.tsv will be created from idaSearch_<date_of_download>_all.csv')
        else:
            cprint('\nida.tsv does not exist and idaSearch_<date_of_download>_all.csv was not found in the clinical data directory, to create it please enter path/to/idaSearch_all.csv :')
        ...
        if not os.path.isfile(path_to_clinical_info):
            if os.path.isfile(path_idaSearch):
                ...
                cprint('\nclinical_info.tsv does not exist and DataDictionary_NIFD_2017.10.18.xlsx was not found in the clinical data directory, to create it please enter path/to/DataDictionary_NIFD_2017.10.18.xlsx :')
        ...
        patients_source_files = {pat:patients_source_files[pat] for pat in patients_source_files if patients_source_files[pat] != []}
