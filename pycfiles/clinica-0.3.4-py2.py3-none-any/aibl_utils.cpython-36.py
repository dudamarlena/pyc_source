...
# [GCC 8.4.0]
# Embedded file name: /Users/mdiazmel/code/aramis/clinica/clinica/iotools/converters/aibl_to_bids/aibl_utils.py
# Compiled at: 2019-10-10 04:46:11
# Size of source mod 2**32: 35461 bytes
"""
...
def find_T1_folder(subdirectory, path_to_T1_1):
    ...
    import os
    path_to_convert = {
     'MPRAGE_ADNI_confirmed', 'MPRAGE', 'MPRAGE_ADNI_confirmed_RPT', 'MPRAGE_ADNI_confirmed_REPEATX2', 'MPRAGE_ADNI_confirmed_repeat', 'MPRAGE_ADNI_confirmed_REPEAT',
...
def match_data(exame_date, i, csv_file):
    ...
    for xx in index:
        if str(csv_date[xx]) != '-4':
            m = re.search('([0-9].*)-(.*)-(.*)_(.*)_(.*)_(.*)', str(exame_date))
            p = re.search('(.*)/(.*)/(.*)', str(csv_date[xx]))
...
def list_of_paths():
    ...
    """
    return [
     '.DS_Store', 'localizer', 'Space_3D_T2_FLAIR_sag_p2', 'AXIAL_FLAIR', 'MPRAGE_ADNI_confirmed_REPEATX2', 'Axial_PD-T2_TSE',
...
def dicom_to_nii(subject, output_path, output_filename, image_path):
    ...
    if not exists(nifti_file):
        ...
        try:
            ...
            cprint(Fore.RED + 'We did not found the dicom files associated with the following directory: ' + image_path + Fore.RESET)
        ...
        if exists(os.path.expandvars('$FREESURFER_HOME/bin/mri_convert')):
            ...
            cprint('mri_convert (from Freesurfer) not detected. ' + nifti_file + ' not created...')
    if not exists(nifti_file):
...
def find_path_to_pet_modality(path_to_dataset, csv_file):
    ...
    data = pandas.DataFrame({'Subjects_ID':sub_ID,  'Session_ID':ses_ID, 
...
def paths_to_bids(path_to_dataset, path_to_csv, bids_dir, modality):
    ...
    def create_file(image):
        ...
        name_of_path = {'t1':'Path_to_T1',  'av45':'Path_to_pet', 
         'flute':'Path_to_pet', 
         'pib':'Path_to_pet'}
        ...
        if image_path is nan:
            ...
            cprint('[' + modality.upper() + '] Processing subject ' + str(subject) + ' - session ' + session + ', ' + str(counter.value) + ' / ' + str(total))
            ...
            if modality == 't1':
                ...
                if modality in ('flute', 'pib', 'av45'):
                    output_path = join(bids_dir, 'sub-AIBL' + subject, 'ses-' + session, 'pet')
                    output_filename = 'sub-AIBL' + subject + '_ses-' + session + '_task-rest_acq-' + modality + '_pet'
    ...
    if modality == 't1':
        ...
        path_to_csv_pet_modality = join(path_to_csv, 'aibl_' + modality + 'meta_28-Apr-2015.csv')
        if not exists(path_to_csv_pet_modality):
            raise FileNotFoundError(path_to_csv_pet_modality + ' file not found in clinical data folder')
        df_pet = pds.read_csv(path_to_csv_pet_modality, sep=',|;')
        images = find_path_to_pet_modality(path_to_dataset, df_pet)
    images.to_csv((join(bids_dir, modality + '_paths_aibl.tsv')), index=False,
      sep='\t',
      encoding='utf-8')
    counter = Value('i', 0)
        ...
        if not pd.isnull(participant_fields_db[i]):
            ...
            if location == prev_location and sheet == prev_sheet:
                ...
                for j in range(0, len(file_to_read)):
                    if participant_fields_bids[i] == 'alternative_id_1':
                        if file_to_read[participant_fields_db[i]].dtype == np.float64 or file_to_read[participant_fields_db[i]].dtype == np.int64:
...
def create_sessions_dict_AIBL(input_path, clinical_data_dir, clinical_spec_path):
    ...
    for r in rid:
        ...
        for i in files_to_read:
            ...
            for j in sessions_fields_to_read:
                if j in list(file_to_read.columns.values) and j == 'MMSCORE':
                    ...
                    if j in list(file_to_read.columns.values):
                        if j == 'CDGLOBAL':
                            CDGLOBAL = file_to_read.loc[(file_to_read['RID'] == r, j)]
                            CDGLOBAL[CDGLOBAL == -4] = np.nan
                        if j in list(file_to_read.columns.values) and j == 'DXCURREN':
        ...
        dict = pd.DataFrame({'session_id':'ses-' + viscode,  'age':age, 
         'MMS':MMSCORE, 
         'cdr_global':CDGLOBAL, 
         'diagnosis':DXCURREN, 
        ...
        if path.exists(bids_paths):
            dict.to_csv((path.join(input_path, 'sub-AIBL' + str(r), 'sub-AIBL' + str(r) + '_sessions.tsv')), sep='\t', index=False,
              encoding='utf8')
...
def get_examdates(rid, examdates, viscodes, clinical_data_dir):
    ...
    import pandas as pd
    res_examdates = []
    csv_list = ('aibl_mri3meta_28-Apr-2015.csv', 'aibl_mrimeta_28-Apr-2015.csv', 'aibl_cdr_28-Apr-2015.csv',
                'aibl_flutemeta_28-Apr-2015.csv', 'aibl_mmse_28-Apr-2015.csv', 'aibl_pibmeta_28-Apr-2015.csv')
    for e in range(len(examdates)):
        ...
        if exam != '-4':
            ...
            for csv_file in csv_list:
                ...
                if exam_date.shape[0] > 0:
                    if exam_date.iloc[0].EXAMDATE != '-4':
