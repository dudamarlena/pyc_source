...
# [GCC 8.4.0]
# Embedded file name: /home/pwittek/wrk/python/happycowler/happycowler/file_io.py
# Compiled at: 2016-09-24 12:02:02
...
def append_results_to_file_gpx(target_file, coordinates, names, tags, ratings, addresses, phone_numbers, opening_hours, cuisines, descriptions):
    f = open(target_file, 'a')
    for i in range(len(names)):
        f.write('<wpt lat="' + coordinates[i][0] + '" lon="' + coordinates[i][1] + '">\n')
        f.write('    <name>' + names[i].replace('&', 'and') + '</name>\n')
        if 'Vegan' in tags[i]:
            f.write('    <extensions><color>#b421880d</color></extensions>\n')
        else:
            if 'Vegetarian' in tags[i]:
                f.write('    <extensions><color>#b48a2091</color></extensions>\n')
            else:
                if 'Veg-friendly' in tags[i]:
            ...
            if opening_hours[i] != '' and 'Call for hours' not in opening_hours[i]:
...
def append_results_to_file_kml(target_file, coordinates, names, tags, ratings, addresses, phone_numbers, opening_hours, cuisines, descriptions):
    f = open(target_file, 'a')
    for i in range(len(names)):
        f.write('  <Placemark>\n')
        f.write('    <name>' + names[i] + '</name>\n')
        f.write('    <Point>\n')
        f.write('      <coordinates>' + coordinates[i][1] + ',' + coordinates[i][0] + ',0</coordinates>\n')
        f.write('    </Point>\n')
        f.write('   <description>\n')
        f.write('     <![CDATA[ ')
...
def append_results_to_file(target_file, coordinates, names, tags, ratings, addresses, phone_numbers, opening_hours, cuisines, descriptions):
    if target_file.endswith('.kml'):
        append_results_to_file_kml(target_file, coordinates, names, tags, ratings, addresses, phone_numbers, opening_hours, cuisines, descriptions)
    else:
        if target_file.endswith('.gpx'):
            append_results_to_file_gpx(target_file, coordinates, names, tags, ratings, addresses, phone_numbers, opening_hours, cuisines, descriptions)
        else:
            raise Exception('Unknown filetype')