...
# [GCC 8.4.0]
# Embedded file name: /Users/nakagawa.masaki/.pyenv/versions/2.7.11/lib/python2.7/site-packages/builder/slave.py
# Compiled at: 2016-05-23 03:45:17
import logging, json, requests, xml.etree.ElementTree as ET
...
class Slave:
    ...
    fields = [
     'name', 'description', 'executors', 'remote_fs', 'labels', 'mode']
    create_params_mapping = {'name': [
              'name', lambda x: x], 
       'description': [
                     'nodeDescription', lambda x: x], 
       'executors': [
                   'numExecutors', lambda x: x], 
       'remote_fs': [
                   'remoteFS', lambda x: x], 
       'labels': [
                'labelString', lambda x: (' ').join(x)], 
       'mode': [
              'mode', lambda x: x.upper()]}
    update_params_mapping = {'name': [
              'name', lambda x: x], 
       'description': [
       ...
       'mode': [
              'mode', lambda x: x.upper()]}
    launcher_mapping = {'jnlp': {'class': 'hudson.slaves.JNLPLauncher', 
                'keys': [
                       'tunnel', 'vmargs']}, 
       'command': {'class': 'hudson.slaves.CommandLauncher', 
                   'keys': [
                          'command']}, 
       'ssh': {'class': 'hudson.plugins.sshslaves.SSHLauncher', 
               'keys': [
                      'host',
                      'port',
                      'private_key',
                      ...
                      'max_num_retries',
                      'retry_wait_time']}, 
       'windows': {'class': 'hudson.os.windows.ManagedWindowsServiceLauncher', 
                   'keys': [
                          'user_name', 'password', 'host', 'java_path', 'vmargs']}}
    ...
    def construct_params(self, params={}):
        default_params = {'description': 'Automatically created through jenkins-slave-builder', 
           'executors': 2, 
           'remote_fs': '/tmp', 
           'labels': [], 'mode': 'normal'}
    ...
    def authinfo(self):
        return (
         self.jenkins['user'], self.jenkins['password'])
    ...
    def create(self):
        ...
        json_params = {'type': 'hudson.slaves.DumbSlave$DescriptorImpl', 
        ...
        payload = {'name': self.params['name'], 'type': 'hudson.slaves.DumbSlave$DescriptorImpl', 
