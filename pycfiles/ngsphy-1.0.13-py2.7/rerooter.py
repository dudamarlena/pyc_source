...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ngsphy/rerooter.py
# Compiled at: 2017-12-14 13:13:35
import dendropy, logging, os, shutil, sys, settings as sp
...
class Rerooter:
    ...
    def __init__(self, settings):
        ...
        self.outputFilePath = os.path.join(self.settings.basepath, '1', self.outputFilename)
    ...
    def run(self):
        ...
        try:
            self.tree = dendropy.Tree.get(path=self.settings.geneTreeFile, schema='newick', rooting='force-rooted', preserve_underscores=True)
        ...
        if newroot:
            ...
            return (
             False,
             ('{0}\n\t{1}\n\t{2}').format('Rerooting problem.', 'Something might be wrong with the reference label.', 'Please Verify. Exiting'))
    ...
    def writeTreeIntoFile(self):
        ...
        self.appLogger.debug('Writing tree')
        self.tree.write(path=self.outputFilePath, schema='newick', suppress_rooting=True)
    ...
    def recheckPloidyAfterRerooting(self):
        ...
        try:
            ...
            if fail:
                ...
                return (
                 status,
                 ('\n\t{0}\n\t{1}\n\t{2}').format('Problems for the individual assignment.', 'Number of gene-tree tips per species is not valid for the given ploidy.', 'Please verify. Exiting.'))
        except Exception as ex:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
            message = ('\n\t{0} {1} | {2} - File: {3} - Line:{4}\n\t{5}').format('Rerooter: Something is wrong.', ex, exc_type, fname, exc_tb.tb_lineno, 'Please verify. Exiting.')
            status = False
