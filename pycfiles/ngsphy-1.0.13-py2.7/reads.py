...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ngsphy/reads.py
# Compiled at: 2017-12-14 13:13:35
import csv, logging, os, subprocess, sys, threading, time, numpy as np, random as rnd
...
class ARTIllumina:
    """
        ...
        """
    __SHORT_NAMES = [
     'sf', 'dp', 'ploidy', 'ofn', '1', '2', 'amp', 'c', 'd', 'ef', 'f', 'h', 'i',
    ...
    def __init__(self, settings):
        ...
        for p in settingsParams:
            if p[0] in self.__dSHORT_NAMES.keys():
                dash = '-'
            if p[0] in self.__dLONG_NAMES.keys():
                dash = '--'
            if dash == '-':
                if p[0] == 'm' and (par[1].lower() in ('true', 'false', 'on', 'off') or par[1] in (0,
                                                                                                   1)):
                    par = [
                     'M']
                elif p[0] == 'm':
                    ...
                    par = [
                     self.__dSHORT_NAMES[p[0]]]
            if dash == '--':
                par = [
                 self.__dLONG_NAMES[p[0]]]
            par += [p[1]]
    ...
    def writeSGEScript(self):
        ...
        jobfile = os.path.join(self.settings.scriptsFolderPath, ('{0}.job.sge.sh').format(self.settings.projectName))
        j = open(jobfile, 'w')
        seedfile = os.path.join(self.settings.scriptsFolderPath, ('{0}.sh').format(self.settings.projectName))
    ...
    def writeSLURMScript(self):
        """
                ...
                """
        self.appLogger.debug('Start')
        jobfile = os.path.join(self.settings.scriptsFolderPath, ('{0}.job.slurm.sh').format(self.settings.projectName))
        j = open(jobfile, 'w')
        seedfile = os.path.join(self.settings.scriptsFolderPath, ('{0}.sh').format(self.settings.projectName))
    ...
    def retrieveCoverageMatrix(self, indexREP):
        ...
        coverageMatrixFilename = os.path.join(self.settings.coverageFolderPath, ('{0}.{1:0{2}d}.coverage.csv').format(self.settings.projectName, self.filteredReplicates[index], self.numReplicateDigits))
        ...
        with open(coverageMatrixFilename, 'rb') as (csvfile):
            ...
            for row in coveragereader:
                if firstLine:
                    ...
                    counter += 1
                if not counter < self.numIndividualsPerReplicate[index]:
                    break
    ...
    def getCommands(self):
        ...
        for index in range(0, len(self.filteredReplicates)):
            csvfilename = os.path.join(self.settings.tablesFolderPath, ('{0}.{1:0{2}d}.{3}.csv').format(self.settings.projectName, self.filteredReplicates[index], self.numReplicateDigits, self.individualsFileNameSuffix))
            with open(csvfilename) as (csvfile):
                ...
                for indexLOC in range(1, self.numLociPerReplicate[index] + 1):
                    for row in self.matingDict:
                        inputFile = os.path.join(self.settings.individualsFolderPath, ('REPLICATE_{0:0{1}d}').format(int(row['repID']), self.numReplicateDigits), ('LOCUS_{0:0{1}d}').format(indexLOC, self.numLociPerReplicateDigits[index]), ('{0}_{1:0{2}d}_{3:0{4}d}_{5}_{6}.fasta').format(self.settings.projectName, int(row['repID']), self.numReplicateDigits, indexLOC, self.numLociPerReplicateDigits[index], self.settings.simphyDataPrefix, int(row['indID'])))
                        outputFile = os.path.join(self.settings.readsFolderPath, ('REPLICATE_{0:0{1}d}').format(int(row['repID']), self.numReplicateDigits), ('LOCUS_{0:0{1}d}').format(indexLOC, self.numLociPerReplicateDigits[index]), ('{0}_{1:0{2}d}_{3:0{4}d}_{5}_{6}_R').format(self.settings.projectName, int(row['repID']), self.numReplicateDigits, indexLOC, self.numLociPerReplicateDigits[index], self.settings.simphyDataPrefix, int(row['indID'])))
    ...
    def writeBashScript(self):
        """
                ...
                """
        self.appLogger.debug('Start')
        bashfile = os.path.join(self.settings.scriptsFolderPath, ('{0}.sh').format(self.settings.projectName))
        j = open(bashfile, 'w')
    ...
    def run(self):
        ...
        try:
            ...
            status = False
            message = ('\t{0}\n\t{1}\n\t{2}').format('Distribution parameter error:', verror, 'Please verify. Exciting.')
        except RuntimeError as rte:
            status = False
            message = ('\t{0}\n\t{1}\n\t{2}').format('Execution error.', rte, 'Please verify. Exciting.')
    ...
    def writeRunningInfoIntoFile(self):
        ...
        for item in self.runningInfo.value:
            f.write(str(item[0]) + ',' + str(item[1]) + ',' + str(item[2]) + ',' + str(item[3]) + ',' + str(item[4]) + ',' + item[5] + '\n')
