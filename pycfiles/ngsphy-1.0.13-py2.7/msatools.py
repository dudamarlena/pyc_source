...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ngsphy/msatools.py
...
def parseMSAFile(fastapath):
    ...
    for line in seqdata:
        if count % 2 == 0:
            ...
            try:
                seqDict[tag].update({tmp[2]: {'description': description, 
                            'sequence': seq}})
            except:
                seqDict[tag][tmp[2]] = {}
                seqDict[tag].update({tmp[2]: {'description': description, 
