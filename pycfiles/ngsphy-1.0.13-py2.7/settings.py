...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ngsphy/settings.py
# Compiled at: 2018-02-13 07:25:30
import argparse, datetime, dendropy, glob, logging, msatools, os, re, random, sys, numpy as np
...
class Settings:
    ...
    def checkArgs(self):
        ...
        statusGeneral, messageGeneral = self.checkBlockGeneral(parserMessageCorrect, parserMessageWrong)
        if not statusGeneral:
            return (statusGeneral, messageGeneral)
        statusData, messageData = self.checkBlockData(parserMessageCorrect, parserMessageWrong)
        if not statusData:
            return (statusData, messageData)
        if self.parser.has_section('ngs-reads-art'):
            self.ngsmode = 1
            statusNGSArt, messageNGSArt = self.checkBlockNGSReadsArt(parserMessageCorrect, parserMessageWrong)
            if statusNGSArt:
            ...
            if self.parser.has_section('coverage'):
                self.parser.remove_section('coverage')
    ...
    def checkBlockGeneral(self, parserMessageCorrect, parserMessageWrong):
        ...
        if not self.parser.has_section('general'):
            parserMessageWrong = ('\n\t{0}\n\t{1}').format('[general] block missing and required.', 'Please verify. Exiting.')
            return (
             False, parserMessageWrong)
        if self.parser.has_option('general', 'seed'):
            self.seed = self.parser.getint('general', 'seed')
            if self.seed > 4294967295 or self.seed < 0:
                self.seed = np.random.random_integers(0, 4294967295)
        else:
            self.seed = np.random.random_integers(0, 4294967295)
        ...
        if not self.parser.has_option('general', 'ploidy'):
            ...
            if p in (1, 2):
                ...
                parserMessageWrong = ('\n\t{0}\n\t{1}\n\t{2}').format('Ploidy value is out of range.', 'Value must be in [1,2].', 'Please verify. Exiting.')
                return (
                 False, parserMessageWrong)
        if self.parser.has_option('general', 'path'):
            self.path = os.path.abspath(self.parser.get('general', 'path'))
        else:
            parserMessageWrong = ('\n\t{0}\n\t{1}').format('Path option is needed.', 'Please verify. Exiting.')
            return (
             False, parserMessageWrong)
        ...
        self.alignmentsFolderPath = os.path.join(self.outputFolderPath, 'alignments')
        self.coverageFolderPath = os.path.join(self.outputFolderPath, 'coverage')
        self.individualsFolderPath = os.path.join(self.outputFolderPath, 'individuals')
        self.readsFolderPath = os.path.join(self.outputFolderPath, 'reads')
        self.refAllelesFolderPath = os.path.join(self.outputFolderPath, 'ref_alleles')
        self.scriptsFolderPath = os.path.join(self.outputFolderPath, 'scripts')
        self.tablesFolderPath = os.path.join(self.outputFolderPath, 'ind_labels')
        return (
         True, parserMessageCorrect)
        ...
        if self.parser.has_section('data'):
            if self.parser.has_option('data', 'inputmode'):
                try:
                    ...
                    parserMessageWrong = ('\n\t\n\t{0}\n\t{1}\n\t{2}').format('[data] block: Value introduced in the inputmode option is not valid.', 'Value should be an integer.', 'Please verify. Exiting.')
                    return (
                     False, parserMessageWrong)

            else:
                parserMessageWrong = ('\n\t\n\t{0}\n\t{1}\n\t{2}').format('[data] block: inputmode option value is missing. This option is required.', 'Please verify. Exiting.')
                return (
                 False, parserMessageWrong)
            if self.inputmode not in (1, 2, 3, 4):
                parserMessageWrong = ('\n\t\n\t{0}\n\t{1}').format('[data] block: inputmode option value is invalid', 'Value is out of range. Possible range [1,3]', 'Please verify. Exiting.')
                return (
                 False, parserMessageWrong)
            if self.inputmode < 4:
                self.basepath = self.alignmentsFolderPath
                if self.parser.has_option('data', 'indelible_control_file'):
                    self.indelibleControlFile = os.path.abspath(self.parser.get('data', 'indelible_control_file'))
                else:
                    parserMessageWrong = ('\n\t{0}{1}{2}\n\t{3}\n\t{4}').format('[data] block: Input mode (', self.inputmode, ') selected but invalid option.', 'INDELible  control file is required', 'Please verify. Exiting.')
                    return (
                     False, parserMessageWrong)
                if os.path.exists(self.indelibleControlFile) and os.path.isfile(self.indelibleControlFile):
                    ...
                    parserMessageWrong = ('\n\t{0}{1}{2}\n\t{3}\n\t{4}').format('[data] block: Input mode (', self.inputmode, ') selected but invalid option.', 'INDELible control file does not exist.', 'Please verify. Exiting.')
                    return (
                     False, parserMessageWrong)
                if self.parser.has_option('data', 'gene_tree_file'):
                    ...
                    if not filesOk:
                        parserMessageWrong = ('\n\t{0}{1}{2}\n\t{3}\n\t{4}').format('[data] block: Input mode (', self.inputmode, ') selected but invalid option.', 'Gene tree file does not exist.', 'Please verify. Exiting.')
                        return (
                         False, parserMessageWrong)
                else:
                    parserMessageWrong = ('\n\t{0}{1}{2}\n\t{3}\n\t{4}').format('[data] block: Input mode (', self.inputmode, ') selected but invalid option.', 'Gene tree file does not exist.', 'Please verify. Exiting.')
                ...
                if self.parser.has_option('data', 'simphy_data_prefix'):
                    self.parser.remove('data', 'simphy_data_prefix')
                if self.parser.has_option('data', 'simphy_folder_path'):
                    self.parser.remove('data', 'simphy_folder_path')
                if self.parser.has_option('data', 'simphy_filter'):
                    self.parser.remove('data', 'simphy_filter')
            if self.inputmode == 1:
                ...
                if stream1:
                    ...
                    parserMessageWrong = ('\n\t{0}{1}{2}\n\t{3}\n\t{4}').format('[data] block: Input mode (', self.inputmode, ') selected but invalid option.', 'indelible nor indelible-ngsphy found. Programs either not installed or not in the current path.', 'Please verify the installation. Exiting.')
                    return (
                     False, parserMessageWrong)
                ...
                if self.parser.has_option('data', 'anchor_sequence_file'):
                    self.parser.remove_option('data', 'anchor_sequence_file')
            ...
            if self.inputmode in (2, 3):
                ...
                if stream:
                    ...
                    parserMessageWrong = ('\n\t{0}{1}{2}\n\t{3}\n\t{4}').format('[data] block: Input mode (', self.inputmode, ') selected but invalid option.', 'indelible-ngsphy not found. Program either not installed or not in the current path.', 'Please verify the installation. Exiting.')
                    return (
                     False, parserMessageWrong)
            if self.inputmode == 2:
                if self.parser.has_option('data', 'ancestral_sequence_file'):
                    ...
                    if not filesOk:
                        parserMessageWrong = ('\n\t{0}{1}{2}\n\t{3}\n\t{4}').format('[data] block: Input mode (', self.inputmode, ') selected but invalid option.', 'Ancestral sequence file does not exist or the given path does not belong to a file.', 'Please verify. Exiting.')
                        return (
                         False, parserMessageWrong)
                    statusSeq, messageSeq = self.correctContentReferenceSequence()
                    if not statusSeq:
                        return (statusSeq, messageSeq)
                else:
                    parserMessageWrong = ('\n\t{0}{1}{2}\n\t{3}\n\t{4}').format('[data] block: Input mode (', self.inputmode, ') selected but invalid option.', 'Ancestral sequence file is required', 'Please verify. Exiting.')
                    return (
                     False, parserMessageWrong)
                if self.parser.has_option('data', 'anchor_sequence_file'):
                    self.parser.remove_option('data', 'anchor_sequence_file')
            if self.inputmode == 3:
                if self.parser.has_option('data', 'anchor_sequence_file'):
                    ...
                    filesOk = os.path.exists(self.ancestralSequenceFilePath) and os.path.isfile(self.ancestralSequenceFilePath)
                    if not filesOk:
                        parserMessageWrong = ('\n\t{0}{1}{2}\n\t{3}\n\t{4}').format('[data] block: Input mode (', self.inputmode, ') selected but invalid option.', 'Anchor sequence file does not exist or the given path does not belong to a file.', 'Please verify. Exiting.')
                        return (
                         False, parserMessageWrong)
                    statusSeq, messageSeq = self.correctContentReferenceSequence()
                    if not statusSeq:
                    ...
                    parserMessageWrong = ('\n\t{0}{1}{2}\n\t{3}\n\t{4}').format('[data] block: Input mode (', self.inputmode, ') selected but invalid option.', 'Anchor sequence file is required', 'Please verify. Exiting.')
                    return (
                     False, parserMessageWrong)
                if self.parser.has_option('data', 'anchor_tip_label'):
                    ...
                    parserMessageWrong = ('\n\t{0}{1}{2}\n\t{3}\n\t{4}').format('[data] block: Input mode (', self.inputmode, ') selected but invalid option.', 'Anchor tip label is required', 'Please verify. Exiting.')
                    return (
                     False, parserMessageWrong)
                if self.parser.has_option('data', 'ancestral_sequence_file'):
                    self.parser.remove_option('data', 'ancestral_sequence_file')
            if self.inputmode == 4:
                if self.parser.has_option('data', 'simphy_folder_path'):
                    ...
                    parserMessageWrong = ('\n\t{0}{1}{2}\n\t{3}\n\t{4}').format('[data] block: Input mode (', self.inputmode, ') selected but invalid option.', 'SimPhy folder does not exist, or the given path does not belong to a directory.', 'Please verify. Exiting.')
                    return (
                     False, parserMessageWrong)
                ...
                if self.parser.has_option('data', 'simphy_data_prefix'):
                    ...
                    parserMessageWrong = ('\n\t{0}{1}{2}\n\t{3}\n\t{4}').format('[data] block: Input mode (', self.inputmode, ') selected but invalid option.', '<simphy_data_prefix> field is missing. This prefix correponds to the name of the file sequences that are going to be processed.', 'Please verify. Exiting.')
                    return (
                     False, parserMessageWrong)
                if self.parser.has_option('data', 'simphy_filter'):
            ...
            parserMessageWrong = ('\n\t\n\t{0}\n\t{1}\n\t{2}').format('[data] block is missing. This block is required.', 'Please verify. Exiting.')
            return (
             False, parserMessageWrong)
        return (
         True, parserMessageCorrect)
    ...
    def checkBlockNGSReadsArt(self, parserMessageCorrect, parserMessageWrong):
        ...
        if stream:
            ...
            if self.parser.has_option('ngs-reads-art', 'o'):
                self.parser.remove_option('ngs-reads-art', 'o')
            if self.parser.has_option('ngs-reads-art', 'out'):
                self.parser.remove_option('ngs-reads-art', 'out')
            if self.parser.has_option('ngs-reads-art', 'i'):
                self.parser.remove_option('ngs-reads-art', 'i')
            if self.parser.has_option('ngs-reads-art', 'in'):
                self.parser.remove_option('ngs-reads-art', 'in')
            self.appLogger.warning('Removing I/O options. Be aware: I/O naming is auto-generated.')
            if not self.parser.has_option('ngs-reads-art', 'fcov') and not self.parser.has_option('ngs-reads-art', 'f') and not self.parser.has_option('ngs-reads-art', 'rcount') and not self.parser.has_option('ngs-reads-art', 'c'):
                parserMessageWrong = ('\n\t{0}\n\t{1}\n\t{2}').format('There has been a problem with the coverage process parameterization.', 'One of the f/fcov or c/rcount parameteres needed.', 'Please verify. Exiting.')
                return (
                 False, parserMessageWrong)
            ...
            parserMessageWrong = ('\n\t{0}\n\t{1}').format('art_illumina not found. Program either not installed or not in your current path.', 'Please verify the installation. Exiting.')
            return (
             False, parserMessageWrong)
        return (
         True, parserMessageCorrect)
        ...
        if self.parser.has_section('ngs-read-counts'):
            if not self.parser.has_option('ngs-read-counts', 'read_counts_error'):
                ...
                try:
                    ...
                    message = ('\n\t{0}\n\t{1}').format('[ngs-read-counts] block.')
                    return (
                     False, message)
            ...
            if not self.parser.has_option('ngs-read-counts', 'reference_alleles_file'):
                ...
                if not fileOk:
                    status = False
                    message = ('\n\t{0}\n\t{1}\n\t{2}').format('[ngs-read-counts] block.', 'Reference file does not exist or path is incorrect.', 'Please verify. Exiting.')
        else:
            status = False
            message = ('\n\t{0}\n\t{1}').format('[ngs-read-counts] block is missing.', 'Please verify. Exiting.')
        return (status, message)
        ...
        if self.parser.has_section('coverage'):
            if self.inputmode == 4:
                ...
                if self.parser.has_option('coverage', 'offtarget'):
                    ...
                    if not len(offTargetValues) == 2:
                        return (False,
                         ('\n\t{0}\n\t{1}\n\t{2}\n\t{3}').format(parserMessageWrong, '[coverage] block: offtarget option invalid.', 'Incorrect number of parameters.', 'Please verify. Exiting'))
                    try:
                        ...
                        return (
                         False,
                         ('\n\t{0}\n\t{1}\n\t{2}\n\t{3}').format(parserMessageWrong, '[coverage] block: offtarget option invalid.', 'One or both values are incorrect.', 'Please verify. Exiting'))

                    if (self.offtarget['loci'] < 0 or self.offtarget['loci'] > 1) and (self.offtarget['coverage'] < 0 or self.offtarget['coverage'] > 1):
                        return (False,
                         ('\n\t{0}\n\t{1}\n\t{2}\n\t{3}').format('[coverage] block: offtarget option invalid.', 'One or both values are out of range', 'Value should be in the interval [0,1]', 'Please verify. Exiting'))
                ...
                if self.parser.has_option('coverage', 'notcaptured'):
                    ...
                    if not self.notcaptured <= 1:
                        return (False,
                         ('\n\t{0}\n\t{1}\n\t{2}\n\t{3}').format(parserMessageWrong, '[coverage] block: notcaptured option invalid.', 'Value out of range, should be within the half-closed interval [0,1)', 'Please verify. Exiting'))
            else:
                if self.parser.has_option('coverage', 'ontarget'):
                    self.parser.remove('coverage', 'ontarget')
                if self.parser.has_option('coverage', 'offtarget'):
                    self.parser.remove('coverage', 'offtarget')
                if self.parser.has_option('coverage', 'notcaptured'):
                    self.parser.remove('coverage', 'notcaptured')
                ...
                if self.parser.has_option('coverage', 'experiment'):
                    ...
                    parserMessageWrong = ('\n\t{0}\n\t{1}').format('[coverage] block:  experiment option is required.', 'Please verify. Exiting.')
                    return (
                     False, parserMessageWrong)
            ...
            if self.parser.has_option('coverage', 'taxon'):
                ...
                if not len(values) % 2 == 0:
                    return (False,
                     ('\n\t{0}\n\t{1}\n\t{2}\n\t{3}').format(parserMessageWrong, '[coverage] block: taxon option invalid.', 'Incorrect number of values.', 'Please verify. Exiting'))
                values = sum(values, [])
                taxonOk = True
                for item in xrange(0, len(values), 2):
                    try:
                        ...
                        return (
                         False,
                         ('\n\t{0}\n\t{1}\n\t{2}\n\t{3}').format(parserMessageWrong, '[coverage] block: taxon option invalid.', 'One or many values are invalid.', 'Please verify. Exiting'))
                ...
                if not taxonOk:
                    return (False,
                     ('\n\t{0}\n\t{1}\n\t{2}\n\t{3}').format(parserMessageWrong, '[coverage] block: taxon option invalid.', 'One or many values are out of range. Value should be in the interval [0,1].', 'Please verify. Exiting'))
                self.appLogger.info('Applying taxon-specific coverage')
        else:
            message = ('\n\t{0}\n\t{1}\n\t{2}\n\t').format('Settings: [coverage] block', 'When using [ngs-reads-art] or [ngs-read-counts] block. Coverage is required.', 'Please verify. Exiting.')
            return (
             False, message)
        return (True, message)
        ...
        if not self.parser.has_section('execution'):
            ...
            if self.parser.has_option('execution', 'environment'):
                ...
                if value in ('sge', 'slurm', 'bash'):
                    ...
                    if value == 'sge':
                        self.executionMode = 2
                    if value == 'slurm':
                        self.executionMode = 3
                    if self.executionMode > 1:
                        self.runART = False
                else:
                    message = ('\n\t{0}\n\t{1}\n\t{2}').format('Settings: Execution block', 'Environment variable is incorrect or unavailable.', 'Please check the settings file and rerun. Exiting.')
                    return (
                     False, message)
            else:
                message = ('\n\t{0}\n\t{1}\n\t{2}').format('Settings: Execution block', 'Environment variable is incorrect or unavailable.', 'Please check the settings file and rerun. Exiting.')
            ...
            if self.parser.has_option('execution', 'runART'):
                try:
                    ...
                    message = ('\n\t{0}\n\t{1}\n\t{2}').format('Settings: Execution block', 'runART variable is incorrect.', 'Please check the settings file and rerun. Exiting.')
                    return (
                     False, message)
        ...
        for index in xrange(0, len(newlines)):
            if newlines[index].startswith('[MODEL]') and model == 0:
                if model == 0:
                    ...
                    parserMessageWrong = ('Validating INDELible Control file: \n\t{0}\n\t{1}').format('There is more than one [MODEL] block.', 'Please verify. Exiting')
                    return (
                     False, parserMessageWrong)
            if newlines[index].startswith('[NGSPHYPARTITION]') and partition == 0:
                if partition == 0:
                    ...
                    parserMessageWrong = ('Validating INDELible Control file: \n\t{0}\n\t{1}').format('There is more than one [NGSPHYPARTITION] block.', 'Please verify. Exiting')
                    return (
                     False, parserMessageWrong)

        self.partition = newlines[partition].split()
        if len(self.partition) != 4:
            parserMessageWrong = ('Validating INDELible Control file: \n\t{0}\n\t{1}\n\t{2}').format('[NGSPHYPARTITION] block has the wrong number of parameters.', '[NGSPHYPARTITION] <tree_filename_basename> <model_name> <sequence_length>', 'Please verify. Exiting')
            return (
             False, parserMessageWrong)
        else:
            newickBasename, _ = os.path.splitext(os.path.basename(self.geneTreeFile))
            if not self.partition[1] == newickBasename:
                parserMessageWrong = ('Validating INDELible Control file: \n\t{0}\n\t{1}\n\t{2}\n\t{3}').format('[NGSPHYPARTITION] block, tree name does not correspond with the Newick File introduced.', 'Remember! Newick filename: newick.tree.', '[NGSPHYPARTITION] newick model1 200', 'Please verify. Exiting')
                return (
                 False, parserMessageWrong)
            modelname = newlines[model].strip().split()[1]
            if not self.partition[2] == modelname:
                parserMessageWrong = ('Validating INDELible Control file: \n\t{0}\n\t{1}\n\t{2}\n\t{3}').format('[NGSPHYPARTITION] block, model name does not correspond to the model defined.', '[MODEL] modelname\n...', '[NGSPHYPARTITION] tree modelname 200', 'Please verify. Exiting')
                return (
                 False, parserMessageWrong)
            if not self.partition[3].isdigit():
                parserMessageWrong = ('Validating INDELible Control file: \n\t{0}\n\t{1}').format('[NGSPHYPARTITION] block, sequence length is not valid.', 'Please verify. Exiting')
                return (
                 False, parserMessageWrong)
    ...
    def checkSimPhyProjectValid(self):
        ...
        if not simphyfiles:
            message = ('\n\t{0}\n\t{1}').format('One of the mandatory SimPhy files does not exist.', 'Please verify. Exiting.')
            return (
             False, message)
        ...
        if not self.numReplicates > 0:
            message = ('\n\t{0}\n\t{1}').format('Not enough number of species tree replicates (at least 1 required)', 'Please verify. Exiting.')
            return (
             False, message)
        return (True, message)
    ...
    def checkBranchLengthsInTree(self):
        ...
        if leafedge == None:
            message = ('\n\t{0}\n\t{1}\n\t{2}').format("INDELible control file - Something's wrong!", 'Branch lengths are not specified in the gene tree file.', 'Please verify. Exiting.')
            status = False
        return (
         status, message)
    ...
    def checkLabelFormatInTree(self):
        ...
        if not bool(re.match('^([1-9]+_[0-9]+_[0-9]+){1}', item)):
            message = ('\n\t{0}\n\t{1}\n\t{2}\n\t{3}\n\t{4}\n\t{5}\n\t').format("INDELible control file - Something's wrong!", 'Labels chosen for the tips of the tree are not correct.', 'Labels should follow this pattern: SpeciesID_LocusID_IndividualID', 'Where SpeciesID,LocusID,IndividualID are numbers.', 'Where SpeciesID > 1, LocusID > 0, IndividualID > 0', 'Please verify. Exiting.')
            return (
             False, message)
        return (
         True, message)
    ...
    def checkAnchorTipLabelInTree(self):
        ...
        if not node:
            messageWrong += ('\n\t{0}\n\t{1}').format('Tip label for reference sequence is not in the Newick file.', 'Please verify. Exiting.')
            return (
             False, messageWrong)
        return (
         True, messageCorrect)
        ...
        for sec in sections:
            ...
            for param in items:
                ...
                if param[0] == 'inputmode':
                    if int(param[1]) == 1:
                        extra = '(Single gene tree)'
                    if int(param[1]) == 2:
                        extra = '(Single gene tree with ancestral sequence)'
                    if int(param[1]) == 3:
                        extra = '(Single gene tree with anchor sequence)'
                    if int(param[1]) == 4:
                        extra = '(Gene-tree distribution - SimPhy output)'
                if param[0] == 'ploidy':
                    if int(param[1]) == 1:
                        extra = '(Haploid individuals)'
                    if int(param[1]) == 2:
                        extra = '(Diploid individuals)'
    ...
    def correctContentReferenceSequence(self):
        ...
        for item in reference:
            if item.upper() not in self.__NUCLEOTIDES:
                status = False
                message = ('\n\t{0}{1}{2}\n\t{3}\n\t{4}').format('[data] block: Input mode (', self.inputmode, ') selected but invalid option.', 'Reference sequence should be a nucleotidic sequence, but some other characters exist..', 'Please verify. Exiting.')
                break
