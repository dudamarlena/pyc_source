...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ngsphy/coverage.py
# Compiled at: 2017-12-17 03:57:10
import csv, datetime, logging, os, subprocess, sys, threading, time, numpy as np, random as rnd
...
class NGSPhyDistributionParser:
    ...
    def validate(self):
        ...
        status = True
        if self.__name not in NGSAvailableDistributions.distributions:
            message = ('{0}\n\t{1}\n\t{2}').format('Problem parsing coverage distribution.', 'Distribution selected invalid.Please verify. Exiting')
            status = False
        if NGSAvailableDistributions.relationNumParams[self.__name] == len(self.__params) or NGSAvailableDistributions.relationNumParams[self.__name] - 1 == len(self.__params) and self.__dependency:
            for index in range(0, len(self.__params)):
                try:
                    ...
                    message = ('{0}\n\t{1}\n\t{2}').format('Problem parsing coverage distribution.', 'Not a correct parameter value.', 'Please verify. Exiting')
                    status = False
                    break

        else:
            message = ('{0}\n\t{1}\n\t{2}\t{3} ({4}) - Num params: {5}\n\t{6}\t{7}\n\t{8}').format('Problem parsing coverage distribution.', 'Not the correct number of parameters for the selected distribution', 'Given: ', NGSAvailableDistributions.names[self.__name], self.__name, len(self.__params), 'Required: ', NGSAvailableDistributions.relationNumParams[self.__name], 'Please verify. Exiting')
            status = False
    ...
    def value(self, samples=1):
        ...
        try:
            ...
            if item == 0:
                ...
                if self.__name == 'b':
                    value = self.binom(samples)
                if self.__name == 'e':
                    value = self.exponential(samples)
                if self.__name == 'f':
                    value = self.fixed(samples)
                if self.__name == 'g':
                    value = self.gamma(samples)
                if self.__name == 'g1':
                    value = self.gamma1(samples)
                if self.__name == 'ln':
                    value = self.lognormal(samples)
                if self.__name == 'n':
                    value = self.normal(samples)
                if self.__name == 'nb':
                    value = self.nbinom(samples)
                if self.__name == 'p':
                    value = self.poisson(samples)
                if self.__name == 'u':
                    value = self.uniform(samples)
...
class CoverageMatrixGenerator:
    ...
    def calculate(self):
        ...
        for index in range(0, len(self.filteredReplicates)):
            ...
            self.appLogger.info(('ReplicateID {0} - {1}/{2} ({3},{4})').format(self.filteredReplicates[index], index + 1, len(self.filteredReplicates), nInds, nLoci))
            if self.settings.locus:
                try:
                    ...
                    exc_type, exc_obj, exc_tb = sys.exc_info()
                    fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
                    message = ('{0}\n\t{1}\n\t{2} | File: {3} - Line:{4}').format('Unexpected. Coverage computation. Locus-wide multipliers.', ex, exc_type, fname, exc_tb.tb_lineno)
            ...
            if self.settings.individual:
                try:
                    ...
                    exc_type, exc_obj, exc_tb = sys.exc_info()
                    fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
                    message = ('{0}\n\t{1}\n\t{2} | File: {3} - Line:{4}').format('Unexpected. Coverage computation. Individual-wide multipliers.', ex, exc_type, fname, exc_tb.tb_lineno)
                    return (False, message)

            if self.settings.taxon:
                self.appLogger.debug('Taxon-specific multipliers')
                individualsTableFilename = os.path.join(self.settings.tablesFolderPath, ('{0}.{1:0{2}d}.individuals.csv').format(self.settings.projectName, self.filteredReplicates[index], self.numReplicateDigits))
            ...
            try:
                ...
                return (
                 False,
                 ('\n\t{0}\n\t{1}\n\t{2}\n\t{3}').format(ex, 'Coverage Matrix Generator', 'An unexpected error occurr with the on/off target parameter. ', 'Please verify. Exiting.'))
            ...
            try:
                ...
                return (
                 False,
                 ('\n\t{0}\n\t{1}\n\t{2}\n\t{3}').format(ex, 'Coverage Matrix Generator', 'An unexpected error occurr with the not captured parameter. ', 'Please verify. Exiting.'))
    ...
    def write(self, coverageMatrix, indexRep):
        ...
        self.appLogger.debug(('Writing coverage matrix for replicate: {0:0{1}d}').format(self.filteredReplicates[index], self.numReplicateDigits))
        filename = os.path.join(self.settings.coverageFolderPath, ('{0}.{1:0{2}d}.coverage.csv').format(self.settings.projectName, self.filteredReplicates[index], self.numReplicateDigits))
        filepath = os.path.abspath(filename)
        header = ['indID'] + [ ('L.{0:0{1}d}').format(loc + 1, self.numLociPerReplicateDigits[index]) for loc in range(0, self.numLociPerReplicate[index])
                             ]
        nInds = self.numIndividualsPerReplicate[index]
        ...
        f.close()
        filename = os.path.join(self.settings.coverageFolderPath, ('{0}.{1:0{2}d}.multipliers.txt').format(self.settings.projectName, self.filteredReplicates[index], self.numReplicateDigits))
        filepath = os.path.abspath(filename)
        f = open(filepath, 'w')
