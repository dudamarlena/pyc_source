...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ngsphy/sequence.py
# Compiled at: 2018-02-15 07:27:05
import argparse, copy, datetime, dendropy, logging, os, re, sys, multiprocessing, msatools, subprocess, platform, stat, numpy as np, random as rnd, settings as sp
...
class SequenceGenerator:
    ...
    def __init__(self, settings):
        ...
        self.newIndelibleControlFilePath = os.path.join(self.settings.alignmentsFolderPath, 'control.txt')
        self.newIndelibleAncestralSequence = os.path.join(self.settings.alignmentsFolderPath, 'ancestral.fasta')
        if self.settings.inputmode == 3:
            self.geneTreeFile = os.path.join(self.settings.alignmentsFolderPath, 'ngsphy.tree')
    ...
    def generateFolderStructure(self):
        ...
        try:
            ...
            self.appLogger.info(('Generating data folder ({0})').format(os.path.join(self.settings.alignmentsFolderPath)))
        except:
            self.appLogger.debug(('Data folder exists ({0})').format(os.path.join(self.settings.alignmentsFolderPath)))
    ...
    def copyAncestralSequenceToOutputFolder(self):
        ...
        self.appLogger.info(('Copying reference sequence file to: {}').format(self.newIndelibleAncestralSequence))
        description = ''
        try:
            with open(self.settings.ancestralSequenceFilePath, 'r') as (f):
                description = f.readline().strip()
        except Exception as ex:
            message = ('\n\t{0}\n\t{1}\n\t{2}\n\t{3}\n').format('I/O problem.', ex, 'Stopped while reading the ancestral sequence file.', 'Please verify and rerun. Exiting.')
            status = False
        ...
        try:
            ...
            message = ('\n\t{0}\n\t{1}\n\t{2}\n\t{3}\n').format('I/O problem.', ex, 'Stopped while copying the ancestral sequence file.', 'Please verify and rerun. Exiting.')
            status = False
            return (status, message)
    ...
    def writeIndelibleControlFile(self):
        ...
        self.appLogger.debug(('Given INDELible control file: ').format(self.settings.indelibleControlFile))
        lines = None
        ...
        statesFreqPresent = -1
        stateFreqEntry = [
         ('[statefreq]\t{0} {1} {2} {3}').format(self.ancestralFreq['T'], self.ancestralFreq['C'], self.ancestralFreq['A'], self.ancestralFreq['G'])]
        ...
        if geneTreeFile[(-1)] != ';':
            geneTreeFile += ';'
        controllines += [
         ('{0} {1} {2}').format('[TREE]', 'ngsphytree', geneTreeFile)]
        controllines += [
         ('{0} {1} [{2} {3} {4}]').format('[PARTITIONS]', 'ngsphypartition', 'ngsphytree', modelname, self.partition[3])]
        controllines += ['[EVOLVE]']
        controllines += [
         (' {0} {1} {2}').format('ngsphypartition', self.evolve[0], self.evolve[1])]
    ...
    def indelibleLauncher(self):
        ...
        try:
            ...
            message = ('{1} | File: {2} - Line:{3}\n\t{0}').format(ex, exc_type, fname, exc_tb.tb_lineno)
            raise RuntimeError(message)

        if self.settings.runningTimes:
            self.writeRunningInfoIntoFile(lines)

    def writeRunningInfoIntoFile(self, lines):
        """
                Writes the information about timing into a file.
                """
        outputFile = os.path.join(self.settings.alignmentsFolderPath, ('{}.indelible.time').format(self.settings.projectName))
        f = open(outputFile, 'w')
