...
def channel_val(fid: int, cid: int, val: int):
    ...
    if not this_fixture:
        return (
         jsonify({'error': 'Fixture {} not found'.format(fid)}), 404)
    chan = this_fixture.get_channel_id(cid)
    if chan == -1:
        return (
         jsonify({'error': 'Channel {} not found'.format(cid)}), 404)
    if val < 0 or val > 255:
        return (jsonify({'error': 'Value {} is invalid'.format(val)}), 400)
    else:
...
def color(fid: int, val: str):
    ...
    if not this_fixture:
        ...
        if not match:
            return (
             jsonify({'error': 'Invalid color {} supplied'.format(val)}), 400)
...
def intensity(fid: int, val: int):
    ...
    if chan == -1:
        ...
        if val < 0 or val > 255:
            return (
             jsonify({'error': 'Value {} is invalid'.format(val)}), 400)
        this_fixture.set_channel(chan, val)
        val = fixture_channel_value(this_fixture, chan)
        return (
...
def helper(fid: int, val: str):
    ...
    if val not in this_helpers.keys():
        return (
         jsonify({'error': 'Helper {} not found'.format(val)}), 404)
    try:
        this_helpers[val]()
    except Exception:
        return (
        ...
        return (
         jsonify({'message':'Helper {} executed'.format(val),  'elements':dict(
...
def callback(cb: str):
    if cb not in current_app.parent.callbacks.keys():
        ...
        return (
         jsonify({'message': 'Callback {} executed'.format(cb)}), 200)
...
def run_timed_event(te: str):
    if te not in current_app.parent.timed_events.keys():
        ...
        return (
         jsonify({'message':'Timed Event {} fired'.format(te),  'elements':{te + '-state': 'Running'}}), 200)
...
def stop_timed_event(te: str):
    if te not in current_app.parent.timed_events.keys():
        ...
        return (
         jsonify({'message':'Timed Event {} stopped'.format(te),  'elements':{te + '-state': 'Stopped'}}), 200)