...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/snipsfakeweather/snipsfakeweather.py
# Compiled at: 2017-10-06 10:20:54
...
ANSWERS = {WeatherCondition.rain: {True: [
...
class SnipsFakeWeather(object):
    ...
    def speak_forecast(self, locality, date, granularity=0):
        ...
        """
        response = SnipsFakeWeather.generate_forecast_sentence(locality, date, granularity)
    ...
    def speak_condition(self, condition, locality, date, granularity=0):
        ...
        :return: A random response for a given weather condition
                 at a specified locality and datetime.
        """
        response = self.generate_condition_sentence(condition, locality, date, granularity)
        if self.tts_service is not None:
            self.tts_service.speak(response)
        return
    ...
    def generate_condition_sentence(condition, locality, date, granularity=0):
        ...
        if condition is None:
            ...
            if locality and date:
                sentence = _('{} {} in {}').format(answer, SnipsFakeWeather.date_to_string(date, granularity), locality)
            elif locality:
                sentence = _('{} in {}').format(answer, locality)
            elif date:
                sentence = _('{} {}').format(answer, SnipsFakeWeather.date_to_string(date, granularity))
            else:
                sentence = answer
            return sentence
    ...
    def generate_forecast_sentence(locality, date, granularity=0):
        ...
        if locality and date:
            ...
            description = _('Weather conditions for ' + '{} for {}').format(locality, date_s)
        elif locality:
            description = _('Current weather conditions for {}').format(locality)
        elif date:
            description = _('Weather conditions for {}').format(SnipsFakeWeather.date_to_string(date, granularity))
        else:
            description = _('Current weather conditions')
        return ('{0}: {1}').format(description, SnipsFakeWeather.generate_random_forecast())
    ...
    def generate_random_forecast(use_celcius=True):
        ...
        conditions = random.choice([_('cloudy'), _('rainy'),
         _('thunder storms'), _('windy'), _('clear sky'), _('light wind')])
        if use_celcius:
    ...
    def generate_temperature(locality, date, granularity=0, use_celcius=True):
        ...
        if use_celcius:
            ...
            degrees_sentence = _('{} degrees Fahrenheit').format(degrees)
        if locality and date:
            sentence = _("On {} in {}, it's going to be {}").format(SnipsFakeWeather.date_to_string(date, granularity), locality, degrees_sentence)
        elif locality:
            sentence = _("In {}, it's going to be {}").format(locality, degrees_sentence)
        elif date:
            sentence = _("On {}, it's going to be {}").format(SnipsFakeWeather.date_to_string(date, granularity), degrees_sentence)
        else:
