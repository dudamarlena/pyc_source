...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/softwarefabrica/django/appserver/djangoserve.py
# Compiled at: 2009-03-16 09:15:25
...
__version__ = '0.1'
import os, sys, getopt, glob, re, string, time, optparse, datetime, logging, signal
...
class Application(object):
    ...
    def Run(self):
        ...
        self.server = Server((self.options.IP_ADDRESS, self.options.PORT), logged_app, self.options.SERVER_THREADS, self.options.SERVER_NAME)
        if self.options.SSL:
...
def main():
    ...
    Program entry point
    """
    parser = optparse.OptionParser(version='Django App Server (%%prog) version %s' % __version__, usage='%prog [options]\n\nServe a Django application.\n')
    parser.add_option('-c', '--conf', '--config', '--configfile', help='use FILE as the configuration file (default: %s)' % DEFAULT_CONFIG_FILE, action='store', dest='configfile', default=DEFAULT_CONFIG_FILE, metavar='FILE')
    parser.add_option('-l', '--logfile', help='use FILE as log file (default: %s)' % DEFAULT_LOGFILE, action='store', dest='LOGFILE', default=DEFAULT_LOGFILE, metavar='FILE')
    parser.add_option('-D', '--deamonize', action='store_true', dest='RUN_AS_DAEMON', default=False, help='become a daemon')
    parser.add_option('-p', '--pidfile', help='save the daemon PID into PIDFILE (default: %s)' % DEFAULT_PIDFILE, action='store', dest='PIDFILE', default=DEFAULT_PIDFILE, metavar='PIDFILE')
    parser.add_option('-L', '--lockfile', help='use LOCKFILE as the daemon lock file (default: %s)' % DEFAULT_LOCKFILE, action='store', dest='LOCKFILE', default=DEFAULT_LOCKFILE, metavar='LOCKFILE')
    parser.add_option('-u', '--user', '--serveruser', '--server_user', help='run server as USER (default: %s)' % DEFAULT_DAEMON_USER, action='store', dest='SERVER_USER', default=DEFAULT_DAEMON_USER, metavar='USER')
    parser.add_option('-g', '--group', '--servergroup', '--server_group', help='run server as group GROUP (default: %s)' % DEFAULT_DAEMON_GROUP, action='store', dest='SERVER_GROUP', default=DEFAULT_DAEMON_GROUP, metavar='GROUP')
    ...
    if options.RUN_AS_DAEMON:
        ...
        daemonize.daemonize(pidfile=options.PIDFILE, lockfile=options.LOCKFILE, run_as_user=run_as_user, run_as_group=run_as_group, workdir=options.DAEMON_RUN_DIR, log=log, loggers=[log, dbg, logging.getLogger()], stdout_log=log, stderr_log=log)
        log.info('Daemonized.')
