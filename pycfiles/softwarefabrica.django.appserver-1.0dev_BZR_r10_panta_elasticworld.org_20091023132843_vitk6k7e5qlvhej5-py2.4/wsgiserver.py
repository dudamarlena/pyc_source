...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/softwarefabrica/django/appserver/wsgiserver.py
...
class HTTPRequest(object):
    ...
    def _parse_request(self):
        ...
        elif environ.get('HTTP_CONNECTION', '') != 'Keep-Alive':
            self.close_connection = True
        te = None
        if self.response_protocol == 'HTTP/1.1':
    ...
    def send_headers(self):
        ...
        if status == 413:
            ...
            if status < 200 or status in (204, 205, 304):
                pass
            elif self.response_protocol == 'HTTP/1.1' and self.environ['REQUEST_METHOD'] != 'HEAD':
...
class HTTPConnection(object):
    ...
    def communicate(self):
        ...
        try:
            while True:
                req = None
                req = self.RequestHandlerClass(self.wfile, self.environ, self.wsgi_app)
                req.parse_request()
                if not req.ready:
                    return
                req.respond()
                ...
                try:
                    if timeout is None or timeout < 0:
                        ...
                        if worker.isAlive():
                            ...
                            worker.join()
                except (AssertionError, KeyboardInterrupt), exc1:
                    pass
...
class CherryPyWSGIServer(object):
    ...
    def __init__(self, bind_addr, wsgi_app, numthreads=10, server_name=None, max=-1, request_queue_size=5, timeout=10, shutdown_timeout=5):
        ...
        if callable(wsgi_app):
            ...
            warnings.warn('The ability to pass multiple apps is deprecated and will be removed in 3.2. You should explicitly include a WSGIPathInfoDispatcher instead.', DeprecationWarning)
    ...
    def bind(self, family, type, proto=0):
        ...
        if self.ssl_certificate and self.ssl_private_key:
            ...
            if not isinstance(self.bind_addr, basestring) and self.bind_addr[0] == '::' and family == socket.AF_INET6:
                try:
    ...
    def populate_ssl_environ(self):
        ...
        ssl_environ = {'wsgi.url_scheme': 'https', 'HTTPS': 'on'}
