...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/lightbox/widgets.py
# Compiled at: 2008-04-28 15:02:59
__all__ = [
 'default_config', 'Lightbox', 'LightboxConfig', 'lightbox_js', 'lightbox_css']
...
class LightboxConfig(JSSource):
    ...
    params = [
     'overlay_opacity', 'animate', 'resize_speed', 'border_size', 'label_image', 'label_of']
    overlay_opacity = 0.8
    animate = 'true'
    resize_speed = 7
    border_size = 10
    label_image = 'Image'
    label_of = 'of'
    params_doc = {'overlay_opacity': 'Controls transparency of shadow overlay (0.0 - 1.0, default: 0.8)', 'animate': 'Toggles resizing animations("true" or "false", default: "true")', 'resize_speed': 'Controls the speed of the image resizing animations (1=slowest and 10=fastest, default: 7)', 'border_size': 'If you adjust the padding in the CSS, you will also need to specify this setting (default: 10)', 'label_image': 'Term to use for "Image" part of "Image # of #" label. Change it for non-english localization.', 'label_of': 'Term to use for "of" part of "Image # of #" label. Change it for non-english localization.'}
...
class Lightbox(Widget):
    ...
    """
    __module__ = __name__
    template = '<a xmlns:py="http://purl.org/kid/ns#" href="${value}" rel="${rel}" py:attrs="dict(title=title)"><img src="${thumb_url}" width="${thumb_width}" height="${thumb_height}" border="0"/></a>\n'
    params = [
     'group', 'rel', 'thumb_url', 'thumb_width', 'thumb_height', 'title']
    thumb_url = ''
    thumb_width = 80
    thumb_height = 60
    title = None
    group = None
    rel = 'lightbox'
    params_doc = {'thumb_url': 'URL of thumbnail image (required)', 'thumb_width': 'Width of thumbnail image (required)', 'thumb_height': 'Height of thumbnail image (required)', 'title': 'Caption to show below image (optional)', 'group': 'Name of group of related images. You can browse through all Lightbox images in the same group in one popup (optional).'}
