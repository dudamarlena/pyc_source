...
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/boto/mws/response.py
# Compiled at: 2015-11-24 05:02:18
# Size of source mod 2**32: 23191 bytes
...
class ListInventorySupplyResult(ResponseElement):
    InventorySupplyList = MemberList(EarliestAvailability=Element(), SupplyDetail=MemberList(EarliestAvailableToPick=Element(), LatestAvailableToPick=Element()))
...
class ItemAttributes(AttributeSet):
    ...
    def __init__(self, *args, **kw):
        names = ('Actor', 'Artist', 'Author', 'Creator', 'Director', 'Feature', 'Format',
                 'GemType', 'MaterialType', 'MediaType', 'OperatingSystem', 'Platform')
        for name in names:
...
class Product(ResponseElement):
    _namespace = 'ns2'
    Identifiers = Element(MarketplaceASIN=Element(), SKUIdentifier=Element())
    AttributeSets = Element(ItemAttributes=ElementList(ItemAttributes))
    Relationships = Element(VariationParent=ElementList(VariationRelationship))
    CompetitivePricing = ElementList(CompetitivePricing)
    SalesRankings = Element(SalesRank=ElementList(SalesRank))
    LowestOfferListings = Element(LowestOfferListing=ElementList(LowestOfferListing))
    Offers = Element(Offer=ElementList(Offer))
...
class Order(ResponseElement):
    ...
    PaymentExecutionDetail = Element(PaymentExecutionDetailItem=ElementList(PaymentExecutionDetailItem=Element(Payment=Element(ComplexMoney))))
