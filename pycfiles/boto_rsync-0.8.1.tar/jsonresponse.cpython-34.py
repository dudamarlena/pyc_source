...
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/boto/jsonresponse.py
...
class Element(dict):
    ...
    def __init__(self, connection=None, element_name=None, stack=None, parent=None, list_marker=('Set', ), item_marker=('member', 'item'), pythonize_name=False):
        dict.__init__(self)
        self.connection = connection
    ...
    def startElement(self, name, attrs, connection):
        ...
        for lm in self.list_marker:
            if name.endswith(lm):
                l = ListElement(self.connection, name, self.list_marker, self.item_marker, self.pythonize_name)
                self[self.get_name(name)] = l
                return l

        if len(self.stack) > 0:
            element_name = self.stack[(-1)]
            e = Element(self.connection, element_name, self.stack, self, self.list_marker, self.item_marker, self.pythonize_name)
...
class ListElement(list):
    ...
    def __init__(self, connection=None, element_name=None, list_marker=[
 'Set'], item_marker=('member', 'item'), pythonize_name=False):
        list.__init__(self)
        self.connection = connection
        self.element_name = element_name
    ...
    def startElement(self, name, attrs, connection):
        ...
        if name in self.item_marker:
            e = Element(self.connection, name, parent=self, list_marker=self.list_marker, item_marker=self.item_marker, pythonize_name=self.pythonize_name)
            self.append(e)
            return e
        else:
            return
