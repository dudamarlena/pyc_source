...
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/boto/services/bs.py
...
class BS(object):
    ...
    Commands = {'reset': 'Clear input queue and output bucket',  'submit': 'Submit local files to the service', 
    ...
    def __init__(self):
        ...
        self.parser.add_option('--help-commands', action='store_true', dest='help_commands', help='provides help on the available commands')
        self.parser.add_option('-a', '--access-key', action='store', type='string', help='your AWS Access Key')
        self.parser.add_option('-s', '--secret-key', action='store', type='string', help='your AWS Secret Access Key')
        self.parser.add_option('-p', '--path', action='store', type='string', dest='path', help='the path to local directory for submit and retrieve')
        self.parser.add_option('-k', '--keypair', action='store', type='string', dest='keypair', help='the SSH keypair used with launched instance(s)')
        self.parser.add_option('-l', '--leave', action='store_true', dest='leave', help="leave the files (don't retrieve) files during retrieve command")
        self.parser.set_defaults(leave=False)
        self.parser.add_option('-n', '--num-instances', action='store', type='string', dest='num_instances', help='the number of launched instance(s)')
        self.parser.set_defaults(num_instances=1)
        self.parser.add_option('-i', '--ignore-dirs', action='append', type='string', dest='ignore', help='directories that should be ignored by submit command')
        self.parser.add_option('-b', '--batch-id', action='store', type='string', dest='batch', help='batch identifier required by the retrieve command')
    ...
    def do_start(self):
        ...
        img = rs[0]
        r = img.run(user_data=s.getvalue(), key_name=self.options.keypair, max_count=self.options.num_instances, instance_type=instance_type, security_groups=[
         security_group])
