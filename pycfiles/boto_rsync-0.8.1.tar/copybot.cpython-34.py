...
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/boto/pyami/copybot.py
...
class CopyBot(ScriptBase):
    ...
    def main(self):
        ...
        if self.dst:
            self.copy_log()
        self.notify('%s (%s) Stopping' % (self.name, self.instance_id), 'Copy Operation Complete')
