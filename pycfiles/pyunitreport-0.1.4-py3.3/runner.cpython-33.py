...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/pyunitreport/runner.py
# Compiled at: 2017-07-31 05:23:59
...
class HTMLTestRunner(TextTestRunner):
    __doc__ = '" A test runner class that output the results. '

    def __init__(self, output, report_name=None, verbosity=2, stream=sys.stderr, descriptions=True, failfast=False, buffer=False, report_title=None, template=None, resultclass=None):
        self.verbosity = verbosity
        self.output = output
        self.report_name = report_name
        self.encoding = UTF8
        TextTestRunner.__init__(self, stream, descriptions, verbosity, failfast=failfast, buffer=buffer)
    ...
    def run(self, test):
        ...
        if reports_path_list:
            self.stream.writeln('Reports generated: {}'.format(', '.join(reports_path_list)))
        return result