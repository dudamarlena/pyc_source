...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/pyunitreport/result.py
# Compiled at: 2017-09-04 04:21:04
...
class _TestInfo(object):
    __doc__ = '" Keeps information about the execution of a test method. '
    ...
    def __init__(self, test_result, test_method, outcome=SUCCESS, err=None, subTest=None):
        ...
        self.test_exception_info = '' if outcome in (self.SUCCESS, self.SKIP) else self.test_result._exc_info_to_string(self.err, test_method)
...
class HtmlTestResult(TextTestResult):
    __doc__ = ' A test runner class that displays results in Html form. '
    ...
    def addSubTest(self, testcase, test, err):
        ...
        if err is not None:
            self._save_output_data()
            testinfo = self.infoclass(self, testcase, self.infoclass.ERROR, err, subTest=test)
            self.errors.append((
             testinfo,
    ...
    def printErrorList(self, flavour, errors):
        ...
        for test_info, dummy in errors:
            ...
            self.stream.writeln('{} [{:3f}s]: {}'.format(flavour, test_info.elapsed_time, test_info.get_description()))
    ...
    def _get_report_attributes(self, tests, start_time, time_taken):
        ...
        hearders = {'start_time': start_time, 
         'duration': time_taken, 
         'status': result}
    ...
    def _render_html_report(self, testRunner, tests):
        ...
        html_report_content = render_html(testRunner.template, title=testRunner.report_title, headers=report_headers, testcase_name=testRunner.output, tests_results=test_cases_list, total_tests=total_test)
