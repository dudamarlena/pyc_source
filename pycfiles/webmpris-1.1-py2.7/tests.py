...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/webmpris/tests.py
# Compiled at: 2013-11-14 22:46:20
from django.test import TestCase
from django.test.client import Client
import json
from contextlib import nested
import mock, webmpris
...
class BaseTestCase(object):
    ...
    def test_get(self):
        ...
        with nested(mock.patch('webmpris.views.check_player_id', return_value=True)):
            mymock = mock.Mock(**spec)
            setattr(getattr(webmpris.views, self.CLS_VIEW), 'CLS_MPRIS', mock.Mock(return_value=mymock))
    ...
    def test_post(self):
        ...
        with nested(mock.patch('webmpris.views.check_player_id', return_value=True)):
            ...
            for method in methods:
                ...
                self.assertEqual(json.loads(response.content).get('status'), 'success')
    ...
    def test_post_fail(self):
        ...
        with nested(mock.patch('webmpris.views.check_player_id', return_value=True)):
            ...
            for method in methods:
                url = '/webmpris/players/:TestPlayer/%s/%s' % (
                 self.PATH, method)
                response = self.client.post(url, data=json.dumps(args), content_type='application/json')
                self.assertEqual(json.loads(response.content).get('status'), 'fail')
...
class RootTestCase(TestCase, BaseTestCase):
    spec_props = {'CanQuit': True, 
       ...
       'SupportedMimeTypes': [
                            'mime1', 'mime2']}
    spec_props_changed = {'CanQuit': False, 
       'HasTrackList': False, 
...
class PlayerTestCase(TestCase, BaseTestCase):
    spec_props = {'PlaybackStatus': 'Stopped', 
       'LoopStatus': 'Track', 
       'Rate': 1, 
       'Shuffle': True, 
       'Metadata': {'mpris:trackid': '/org/mpris/MediaPlayer2/Track/1', 'xesam:url': 'jazzradio://vocallegends'}, 
       'Volume': 0.2, 
       'Position': 15000, 
       'Minimumrate': 0, 
       'MaximumRate': 1, 
       ...
       'CanSeek': True, 
       'CanControl': True}
    spec_props_changed = {'PlaybackStatus': 'Paused', 
       'LoopStatus': 'Playlist', 
       'Rate': 1, 
       'Shuffle': True, 
       'Metadata': {'mpris:trackid': '/org/mpris/MediaPlayer2/Track/3', 'xesam:url': 'jazzradio://blues'}, 
       'Volume': 0.7, 
       'CanGoNext': False}
    CLS_VIEW = 'Player'
    PATH = 'Player'
...
class TrackListTestCase(TestCase, BaseTestCase):
    spec_props = {'Tracks': [
                '/org/videolan/vlc/playlist/5'], 
       'CanEditTracks': True}
    spec_props_changed = {'Tracks': [
...
class PlaylistsTestCase(TestCase, BaseTestCase):
    spec_props = {'PlaylistCount': 7, 
       'Orderings': [
                   'User'], 
       'ActivePlaylist': [
                        '/org/mpris/MediaPlayer2/Playlists/12',
                        'jazzradio',
                        '']}
    spec_props_changed = {'PlaylistCount': 9, 
       'Orderings': [
                   'Alphabetical'], 
       'ActivePlaylist': [
                        '/org/mpris/MediaPlayer2/Playlists/10',
                        'newradio',
                        '']}
    CLS_VIEW = 'Playlists'
    PATH = 'Playlists'