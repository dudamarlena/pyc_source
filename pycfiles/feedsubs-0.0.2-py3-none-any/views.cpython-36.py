...
# [GCC 8.4.0]
# Embedded file name: /home/nicolas/dev/feedpubsub/um/views.py
...
class AccountSettings(CurrentPageSettings, LoginRequiredMixin, SuccessMessageMixin, FormView):
    ...
    success_message = 'The account will be permanently deleted in {} hours, sign in again to reactivate it.'.format(int(UM_DELETE_ACCOUNT_AFTER.total_seconds() / 3600))

    def get_form_kwargs(self):
        kwargs = super().get_form_kwargs()
        kwargs.update({'current_username': self.request.user.username})
        return kwargs

    def form_valid(self, form):
        self.request.user.um_profile.deletion_pending = True
        self.request.user.um_profile.save()
        tasks.tasks.schedule_at('um:delete_user', now() + UM_DELETE_ACCOUNT_AFTER, self.request.user.id)
        logout(self.request)
...
class SecuritySettings(CurrentPageSettings, LoginRequiredMixin, PasswordChangeView):
