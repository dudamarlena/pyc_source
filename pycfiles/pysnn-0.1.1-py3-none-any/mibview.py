...
# Embedded file name: build/bdist.linux-x86_64/egg/pysnmp_apps/cli/mibview.py
    ...
    def n_OutputOption(self, cbCtx, node):
        ...
        for c in opt:
            if c == 'q':
                ...
                raise error.PySnmpError('Unknown output option %s at %s' % (c, self))
    ...
    def n_InputOption(self, cbCtx, node):
        ...
        for c in opt:
            if c == 'R':
                ...
                mibViewProxy.defaultOidPrefix = ('iso', 'org', 'dod', 'internet', 'mgmt',
                                                 'mib-2')
            elif c == 'r':
                pass
            elif c == 'h':
                ...
                raise error.PySnmpError('Unknown input option %s at %s' % (c, self))
...
def generator(cbCtx, ast):
    ...
    compiler.addMibCompiler(snmpEngine.getMibBuilder(), sources=ctx['MibDir'], borrowers=ctx['MibBorrowers'])
    return (
     snmpEngine, ctx)
...
class MibViewProxy:
    __module__ = __name__
    defaultOidPrefix = ('iso', 'org', 'dod', 'internet', 'mgmt', 'mib-2', 'system')
    ...
    def __init__(self, mibViewController):
        ...
        if self.defaultMibDirs:
            mibViewController.mibBuilder.setMibSources(*(mibViewController.mibBuilder.getMibSources() + tuple([ builder.ZipMibSource(m).init() for m in self.defaultMibDirs ])))
        if self.defaultMibs:
    ...
    def getPrettyOidVal(self, mibViewController, oid, val):
        ...
        if self.buildValue:
            ...
            elif not self.buildEnums and (self.__intValue.isSuperTypeOf(val) or self.__bitsValue.isSuperTypeOf(val)):
