...
# [GCC 8.4.0]
# Embedded file name: ../multi_job/models/jobs.py
# Compiled at: 2020-02-14 16:09:50
...
class Job(ABC):
    ...
    def resolve_targets(self, projects: List[Project]) -> List[Project]:
        matches = []
        if self.targets:
            matches = [p for p in projects if p.name in self.targets or self.targets == ['all']]
        else:
            if self.skips:
                matches = [p for p in projects if not (p.name in self.skips or self.skips == ['all'])]
        if not matches:
    ...
    @abstractmethod
    def make_process(self, context: dict, path: str, alias: str, config_path: str) -> Process:
        pass

    @staticmethod
    def from_config(dct: dict) -> List[Any]:
        return [CommandJob(name=k, **v) if 'command' in v else FunctionJob(name=k, **v) if 'function' in v else ScriptJob(name=k, **v) for k, v in dct.items()]
