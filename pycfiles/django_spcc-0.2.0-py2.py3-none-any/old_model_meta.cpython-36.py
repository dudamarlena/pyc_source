...
# [GCC 8.4.0]
# Embedded file name: D:\SM\CODE\sfsm\venv\Lib\site-packages\spcc\models\old_model_meta.py
...
def get_concrete_fields_with_model(model=None):
    fields = []
    if model:
        fields = [(f, f.model if f.model != model else None) for f in model._meta.get_fields() if f.concrete if not f.is_relation or f.one_to_one or f.many_to_one and f.related_model]
    return fields
...
def get_m2m_with_model(model=None):
    fields = []
    if model:
        fields = [(f, f.model if f.model != model else None) for f in model._meta.get_fields() if f.many_to_many if not f.auto_created]
    return fields
...
def get_all_related_objects(model=None):
    fields = []
    if model:
        fields = [f for f in model._meta.get_fields() if f.one_to_many or f.one_to_one if f.auto_created if not f.concrete]
    return fields
...
def get_all_related_objects_with_model(model=None):
    fields = []
    if model:
        fields = [(f, f.model if f.model != model else None) for f in model._meta.get_fields() if f.one_to_many or f.one_to_one if f.auto_created if not f.concrete]
    return fields
...
def get_all_related_many_to_many_objects(model=None):
    fields = []
    if model:
        fields = [f for f in model._meta.get_fields(include_hidden=True) if f.many_to_many if f.auto_created]
    return fields
...
def get_all_related_m2m_objects_with_model(model=None):
    fields = []
    if model:
        fields = [(f, f.model if f.model != model else None) for f in model._meta.get_fields(include_hidden=True) if f.many_to_many if f.auto_created]
    return fields
