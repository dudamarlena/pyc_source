...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/printflow2/NooshMonitor_django.py
# Compiled at: 2014-08-13 04:35:49
# Size of source mod 2**32: 33435 bytes
"""
Created on Sep 19, 2013

@author: "Colin Manning"
"""
import requests, datetime, time, os, json, logging, smtplib, traceback, pyinotify
...
class NooshMonitor_django(pyinotify.ProcessEvent):
    ...
    def ensureClientFoldersForUser(self, workgroup):
        ...
        for client in clients:
            ...
            if not os.path.exists(clientFolder):
                os.makedirs(clientFolder, mode=511)
        ...
        while self.ready:
            try:
                try:
                    ...
                    ok = self.checkForProjects(self.db_workgroup, 'create_date_from', True)
                    if ok:
                        ok = self.checkForProjects(self.db_workgroup, 'last_spec_update_from', False)
                    else:
    ...
    def createDamProjectCategory(self, workgroup, project, client):
        ...
        utils.ensureDirectoryExistsForUser(result['dropbox_path'], self.os_userid, self.os_groupid, 511)
        if 'project_folders' in self.theWorkgroup:
            for f in self.theWorkgroup['project_folders']:
                fp = os.path.join(result['dropbox_path'], f)
                utils.ensureDirectoryExistsForUser(fp, workgroup.os_userid, workgroup.os_groupid, 511)
    ...
    def createDamSpecificationCategory(self, client, project, spec):
        try:
            ...
            if 'default_dropbox_folder' not in client:
                ...
                utils.ensureDirectoryExistsForUser(project_folder, self.os_userid, self.os_groupid, 511)
                result['dropbox_path'] = project_folder + '/' + str(spec['reference_number'])
                utils.ensureDirectoryExistsForUser(result['dropbox_path'], self.db_workgroup.os_userid, self.db_workgroup.os_groupid, 511)
            else:
                default_dropbox_folder = project['dropbox_path'] + '/' + client['default_dropbox_folder']
                utils.ensureDirectoryExistsForUser(default_dropbox_folder, self.os_userid, self.os_groupid, 511)
                project_folder = default_dropbox_folder + '/Project-' + str(result['id_project'])
                utils.ensureDirectoryExistsForUser(project_folder, self.db_workgroup.os_userid, self.db_workgroup.os_groupid, 511)
                result['dropbox_path'] = project_folder + '/' + str(spec['reference_number'])
                utils.ensureDirectoryExistsForUser(result['dropbox_path'], self.os_userid, self.db_workgroup.os_groupid, 511)
            if 'account_managers' in client:
                if 'users' in client:
                    ...
                    for am_id in client['account_managers']:
                        if am_id in users:
                            am_project_path = os.path.join(project['dropbox_path'], users[am_id]['dropbox_folder'] + '/Project-' + str(result['id_project']))
                            utils.ensureDirectoryExistsForUser(am_project_path, self.os_userid, self.os_groupid, 511)
                            am_spec_path = am_project_path + '/' + str(spec['reference_number'])
                            utils.ensureDirectoryExistsForUser(am_spec_path, self.os_userid, self.os_groupid, 511)
                            continue
    ...
    def inform_flowdock(self, accesstoken, proj, specs):
        ...
        message = '<div><h3>A new project has been created:</h3> <table> <tr><td>Project Id:</td><td>%s</td></tr> <tr><td>Project Name:</td><td>%s</td></tr> <tr><td>Description:</td><td>%s</td></tr><tr><td>Comments:</td><td>%s</td></tr> </table></div><br /><div>Specifications:</div><hr />' % (
        ...
        data = {'source': 'PrintFlow 2 Service',  'from_address': 'colin@printoutsource.com',  'subject': subject, 
         ...
         'tags': [
                  '@all', '#project', str(proj['project_id'])]}
        d = json.dumps(data)
    ...
    def get_email_project_create_body(self, proj):
        return '<div><h3>A new project has been created:</h3> <table> <tr><td>Project Id:</td><td>%s</td></tr> <tr><td>Project Name:</td><td>%s</td></tr> <tr><td>Description:</td><td>%s</td></tr><tr><td>Comments:</td><td>%s</td></tr> </table></div><br /><div>Specifications:</div><hr />' % (
         proj['project_id'], proj['project_name'], proj['project_description'], proj['comments'])

    def get_email_specification_create_body(self, proj, spec):
        return '<div> <h3>A new specification has been created:</h3> <table> <tr><td>Project Id:</td><td>%s</td></tr> <tr><td>Project Name:</td><td>%s</td></tr> <tr><td>Specification Id:</td><td>%s</td></tr> <tr><td>Specification Name:</td><td>%s</td></tr> <tr><td>Reference No:</td><td> %s</td></tr> </table> <hr /></div>' % (
    ...
    def get_email_specification_update_body(self, proj, spec):
        return '<div> <h3>A specification has been updated:</h3> <table> <tr><td>Project Id:</td><td>%s</td></tr> <tr><td>Project Name:</td><td>%s</td></tr> <tr><td>Specification Id:</td><td>%s</td></tr> <tr><td>Specification Name:</td><td>%s</td></tr> <tr><td>Reference No:</td><td> %s</td></tr> </table> <hr /></div>' % (
         proj['project_id'], proj['project_name'], spec['spec_id'], spec['spec_name'], spec['reference_number'])

    def get_email_specification_body(self, proj, spec):
        return '<div> <table> <tr><td>Specification Id:</td><td>%s</td></tr> <tr><td>Specification Name:</td><td>%s</td></tr> <tr><td>Reference No:</td><td> %s</td></tr> </table> <hr /></div>' % (
    ...
    def processSpecification(self, workgroup, client, project, spec, proj_is_new, db_spec):
        if db_spec is None:
            ...
            if not proj_is_new:
                subject = 'Specification created for project: ' + str(project['noosh_project']['project_id']) + ' - ' + project['noosh_project']['project_name']
                ...
                self.send_html_mail(mail_list, subject, message)
        elif db_spec['noosh_specification']['spec_name'] != spec['spec_name']:
            ...
            try:
                ...
                with open(name_file, 'w') as (f):
                    ...
                    if not proj_is_new:
                        subject = 'Specification updated for project: ' + str(project['noosh_project']['project_id']) + ' - ' + project['noosh_project']['project_name']
