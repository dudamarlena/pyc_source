...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/stackoverflow/base.py
# Compiled at: 2014-02-23 09:50:30
import re, os, sys, time, pprint, pickle, requests, requests.utils
...
class StackOverflowAPI:
    ...
    def query(self, target, **parameters):
        while time.time() - self.last_req < 0.37:
            pass
        ...
        r = self.session.get(('{api}/{target}').format(api=self.api, target=target), params=parameters)
        r = r.json()
        if 'error_id' in r.keys():
            raise StackOverflowException(('Error #{error_id}: {error_name} ({error_message})').format(**r))
        return r
...
class StackOverflowQNA_Soup:
    ...
    def get_questions(self, sort='added', number='50'):
        if sort == 'added':
            sort = 'newest'
        r = self.session.get('http://%s/questions' % (self.site,), params={'sort': sort, 'pagesize': number})
        ql = BeautifulSoup(r.text).findAll(attrs={'class': 'question-summary'})
        for q in ql:
            yield {'id': q['id'].split('-')[(-1)], 'url': q.find('h3').find('a', href=True)['href'], 
               'title': q.find('h3').text, 
               'views': q.find(attrs={'class': 'views '}).text.split(' ')[0], 
               'votes': q.find(attrs={'class': 'vote-count-post '}).text, 
               'answs': (q.find(attrs={'class': 'status unanswered'}) or q.find(attrs={'class': 'status answered'})).find('strong').text, 
               'summary': q.find(attrs={'class': 'excerpt'}).text, 
               'tags': [ t.text for t in q.findAll(attrs={'class': 'post-tag'}) ], 'user': q.find(attrs={'class': 'user-details'}).find('a').text}
    ...
    def get_answers(self, question):
        ...
        for u in q.findAll(attrs={'class': 'post-signature'}):
            for u in u.findAll(attrs={'class': 'user-info '}):
                ...
                username = u.find(attrs={'class': 'user-details'}).find('a')
                reputation = u.find(attrs={'class': 'reputation-score'})
                qd['users'].append({'last_edit': u.find(attrs={'class': 'user-action-time'}).find('span')['title'], 
                   'avatar': avatar['src'] if avatar else '', 
                   'username': username.text if username else '', 
                   'reputation': reputation.text if reputation else ''})
        ...
        for c in q.findAll(attrs={'class': 'comment'}):
            qd['comments'].append({'id': c['id'].split('-')[(-1)], 
        ...
        for aa in a.findAll(attrs={'class': re.compile('^answer( accepted-answer)?$')}):
            aad = {'id': aa['id'].split('-')[(-1)], 
            ...
            for u in aa.findAll(attrs={'class': 'post-signature'}):
                ...
                username = u.find(attrs={'class': 'user-details'}).find('a')
                reputation = u.find(attrs={'class': 'reputation-score'})
                aad['users'].append({'last_edit': u.find(attrs={'class': 'user-action-time'}).find('span')['title'], 
                   'avatar': avatar['src'] if avatar else '', 
                   'username': username.text if username else '', 
                   'reputation': reputation.text if reputation else ''})
            ...
            for c in aa.findAll(attrs={'class': 'comment'}):
                ...
                aad['comments'].append({'id': c['id'].split('-')[(-1)], 
                   'score': c.find(attrs={'class': 'comment-score'}).text, 
                   'text': c.find(attrs={'class': 'comment-copy'}).text, 
...
class StackOverflowQNA(StackOverflowAPI):
    ...
    def get_questions(self, sort='activity', number='50'):
        if sort == 'active':
            ...
            if sort == 'newest':
                ...
                sort == 'votes'
            r = self.query('questions', order='desc', sort=sort, site=self.site)
            for item in r['items']:
                yield {'id': item['question_id'], 'url': item['link'], 
                   'title': item['title'], 
    ...
    def get_answers(self, question, sort='votes'):
        ...
        r = self.query(('questions/{}/answers').format(question, sort=sort))
        return r
...
class StackOverflowChat:
    ...
    def get_my_info(self):
        ...
        return dict(uid=unescape(u_id), name=unescape(u_name))
    ...
    def get_room_info(self, room):
        try:
            ...
            return dict(name=unescape(r_name), topic=unescape(r_topic), tags=r_tags, users=r_users)
...
class StackOverflowBase(StackOverflowChat, StackOverflowInbox_Soup, StackOverflowQNA_Soup):
