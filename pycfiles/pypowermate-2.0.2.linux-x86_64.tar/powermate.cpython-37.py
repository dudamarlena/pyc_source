...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.7/site-packages/pypowermate/powermate.py
# Compiled at: 2018-12-13 14:33:39
...
class Powermate(object):
    ...
    """
        if timeout == 0:
            return self._Powermate__poll_event()
        t0 = time.time()
        while 1:
            if timeout is None:
                event = self._Powermate__read_event(None)
            else:
                elapsed = time.time() - t0
                if elapsed >= timeout:
                    raise PowermateTimeoutException()
                event = self._Powermate__read_event(max(timeout - elapsed, 0))
            if event is None:
                continue
            if event.type == ecodes.EV_REL:
                return (
                 event.timestamp(), self.EVENT_ROTATE, event.value)
            if event.type == ecodes.EV_KEY:
                return (
                 event.timestamp(), self.EVENT_BUTTON, event.value)
    ...
    """
        value = brightness | pulse_speed << MAGIC_SPEED_SHIFT | pulse_table << MAGIC_PULSE_SHIFT | int(asleep) << MAGIC_ASLEEP_SHIFT | int(awake) << MAGIC_AWAKE_SHIFT
