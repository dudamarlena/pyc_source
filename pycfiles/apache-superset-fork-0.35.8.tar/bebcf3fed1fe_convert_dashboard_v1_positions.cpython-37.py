...
def get_chart_holder(position):
    ...
    if code is not None:
        ...
        return {'type':MARKDOWN_TYPE,  'id':'DASHBOARD_MARKDOWN_TYPE-{}'.format(generate_id()), 
...
def convert(positions, level, parent, root):
    ...
    for layer in layers:
        if len(layer) == 0:
            ...
            sorted_by_col = has_overlap(current_positions) or sorted(current_positions, key=(lambda pos: pos['col']))
