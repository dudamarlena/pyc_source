...
# [GCC 8.4.0]
# Embedded file name: /Users/bogdankyryliuk/src/oss/incubator-superset/superset/migrations/versions/4736ec66ce19_.py
...
def upgrade():
    ...
    try:
        with op.batch_alter_table('datasources', naming_convention=conv) as (batch_op):
            batch_op.drop_constraint((generic_find_uq_constraint_name('datasources', {'datasource_name'}, insp) or 'uq_datasources_datasource_name'),
              type_='unique')
...
def downgrade():
    ...
    with op.batch_alter_table('datasources', naming_convention=conv) as (batch_op):
        batch_op.drop_constraint((generic_find_fk_constraint_name('datasources', {'cluster_name'}, 'clusters', insp) or 'fk_datasources_cluster_name_clusters'),
          type_='foreignkey')
        batch_op.drop_constraint((generic_find_uq_constraint_name('datasources', {'cluster_name', 'datasource_name'}, insp) or 'uq_datasources_cluster_name'),
          type_='unique')
