...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\osuapi\osu.py
# Compiled at: 2016-03-31 16:39:54
...
class OsuApi:
    ...
    def get_user(self, username, *, type_='string', mode=0):
        return self._process(endpoints.USER, dict(k=self.key, u=username, type=type_, m=mode), JsonList(User))

    def get_user_best(self, username, *, type_='string', mode=0):
        return self._process(endpoints.USER_BEST, dict(k=self.key, u=username, type=type_, m=mode), JsonList(Score))