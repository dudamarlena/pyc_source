...
# [GCC 8.4.0]
# Embedded file name: D:\PROJECT_HOME\bets-cli\src\bets\model\stats\stats_collection.py
# Compiled at: 2019-05-20 03:53:22
# Size of source mod 2**32: 11354 bytes
import json
...
class StatsCollection(AbstractStats):
    ...
    def read_json(cls, file=None) -> 'StatsCollection':
        if file is None:
            file = 'D:\\PROJECT_HOME\\f_stats\\src\\f_stats\\storage\\matches.json'
        path = Path(file)
        matches = []
        with path.open('rb') as (fp):
            matches_dicts = json.loads(fp.read().decode('utf-8'))
            for m in matches_dicts:
                matches.append(ScoreStats(m['ratio_1'], m['ratio_x'], m['ratio_2'], m['host_score'], m['guest_score'], m['host_team'], m['guest_team'], m['date'], m['country'], m['tournament']))
    ...
    def get_counters(self) -> Dict[(str, int)]:
        return {'Countries':len(self.unique_countries), 
    ...
    def tournaments_by_countries(self) -> Dict[(str, List[str])]:
        return {country:list(sorted(set([m.country for m in stats if m.country]))) for country, stats in self.by_country().items()}

    def agg_date(self) -> Dict[(str, List[Dict[(str, Union[(int, float, str)])]])]:
        return {date:date_stats.get_matches_dicts() for date, date_stats in self.by_date().items()}

    def agg_rank(self) -> Dict[(str, List[Dict[(str, Union[(int, float, str)])]])]:
        return {rank:rank_stats.get_matches_dicts() for rank, rank_stats in self.by_rank().items()}

    def agg_ratio(self: 'StatsCollection') -> Dict[(str, List[Dict[(str, Union[(int, float, str)])]])]:
        ratio_stats = self.by_ratio()
        return {f"{ratio:.02f}":ratio_stats[ratio].get_matches_dicts() for ratio in sorted((ratio_stats.keys()), reverse=True)}
    ...
    def summary_by_rank(self) -> List[Dict[(str, Union[(int, float, str)])]]:
        stats_by_rank = [dict(rank=rank, **stats.as_dict()) for rank, stats in self.by_rank().items()]
        stats_by_rank.sort(key=(lambda s: s['size']))
        return stats_by_rank

    def summary_by_ratio(self) -> List[Dict[(str, Union[(int, float, str)])]]:
        stats_by_ratio = [dict(ratio=ratio, won='', **stats.as_dict()) for ratio, stats in self.by_ratio().items()]
