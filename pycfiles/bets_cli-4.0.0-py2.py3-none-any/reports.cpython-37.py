...
# [GCC 8.4.0]
# Embedded file name: D:\PROJECT_HOME\bets-cli\src\bets\model\stats\reports.py
...
TABLE_FMT = {True:'simple', 
 False:'fancy_grid',  None:'fancy_grid'}

def _fmt_table(table: List[dict], simple=True, showindex=False):
    return tabulate(tabular_data=table,
      headers='keys',
...
def prepare_daily_report(stats: StatsCollection, simple=True) -> str:
    ...
    daily = '\n\n'.join([f"{date}:\n{_fmt_table(date_stats, simple)}" for date, date_stats in stats.agg_date().items()])
    return f"Summary:\n{summary}\n\n\n{daily}"
...
def prepare_ranks_report(stats: StatsCollection, simple=True) -> str:
    ...
    ranked = '\n\n'.join([f"{rank}:\n{_fmt_table(rank_stats, simple)}" for rank, rank_stats in stats.agg_rank().items()])
    return f"Summary:\n{summary}\n\n{ranked}"
...
def prepare_ratios_report(stats: StatsCollection, simple=True) -> str:
    ...
    ratios_tables = '\n\n\n'.join([f"[{ratio}]\n{_fmt_table(ratio_stats, simple)}" for ratio, ratio_stats in stats.agg_ratio().items()])
    return f"Summary:\n{summary_table}\n\n{ratios_tables}"
