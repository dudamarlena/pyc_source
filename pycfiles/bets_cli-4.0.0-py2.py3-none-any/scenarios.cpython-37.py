...
# [GCC 8.4.0]
# Embedded file name: D:\PROJECT_HOME\bets-cli\src\bets\model\scenarios.py
...
class Scenario:
    ...
    def from_outcomes(cls, outcomes, matches: Matches):
        ...
        s.tuple = s.outcomes + tuple(s.ratios) + tuple(s.ranks) + tuple(s.ranks_counts.values()) + tuple(s.outcome_counts.values()) + (s.total_ratio,)
        return s
...
class Scenarios:
    ...
    def from_matches(cls, matches: Matches):
        scenarios = [Scenario.from_outcomes(outcomes, matches) for outcomes in product(OUTCOMES, repeat=(len(matches)))]
    ...
    @property
    def table(self) -> str:
        return tabulate(tabular_data=[s.tuple for s in self.scenarios],
          headers=([
         'ID'] + self.columns),
          tablefmt='fancy_grid',
          stralign='center',
    ...
    def filter_by_total_occurrences(self, value, min_count, max_count):
        ...
        def is_matching_counts(scenario):
            count = 0
    ...
    def filter_by_sequential_occurrences(self, value, max_count):
        ...
        def is_matching_count(scenario):
