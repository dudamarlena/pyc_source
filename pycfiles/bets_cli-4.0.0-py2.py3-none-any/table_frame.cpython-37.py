...
# [GCC 8.4.0]
# Embedded file name: D:\PROJECT_HOME\bets-cli\src\bets\ui\table_frame.py
# Compiled at: 2019-03-24 19:21:36
...
def _setup_canvas(table_frame, data: List[List[str]], column_widths: List[int]=None, height=200, width=None):
    ...
    if not width:
        cells_width = sum(column_widths) * 8
        cells_ipadx = len(column_widths) * 8
...
def _fill_data(frame, data: List[List[str]], column_widths: List[int]=None, font=('Consolas', 10, 'normal')):
    column_widths = column_widths or _get_columns_widths(data)
    for row_idx, row in enumerate(data):
        for col_idx, cell_text in enumerate(row):
            cell_label = Label(frame, text=cell_text,
...
def _create_table_frame(parent, columns: List[str], rows: List[List[str]], height=200, width: int=None):
    ...
    return (
     header_frame, body_frame, footer_frame)
