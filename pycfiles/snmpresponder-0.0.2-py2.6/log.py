...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/snmpresponder/log.py
...
class SyslogLogger(AbstractLogger):
    ...
    def init(self, *priv):
        if len(priv) < 1:
            raise SnmpResponderError('Bad syslog params, need at least facility, also accept host, port, socktype (tcp|udp)')
        if len(priv) < 2:
            priv = [
             priv[0], 'debug']
        if len(priv) < 3:
        ...
        try:
            ...
            raise SnmpResponderError('Bad syslog option(s): %s' % sys.exc_info()[1])
    ...
    def init(self, *priv):
        ...
        if len(priv) > 1 and priv[1]:
            try:
                ...
                raise SnmpResponderError('Error in timed log rotation specification. Use <NNN>k,m,g for size or <NNN>S,M,H,D for time limits')
        ...
        try:
            ...
            raise SnmpResponderError('Failure configure logging: %s' % sys.exc_info()[1])
...
class StreamLogger(AbstractLogger):
    ...
    def init(self, *priv):
        try:
            ...
            raise SnmpResponderError('Stream logger failure: %s' % sys.exc_info()[1])
...
methodsMap = {'syslog': SyslogLogger, 
   'file': FileLogger, 
   'stdout': StdoutLogger, 
   'stderr': StderrLogger, 
   'null': NullLogger}
levelsMap = {'debug': LOG_DEBUG, 
   'info': LOG_INFO, 
   'error': LOG_ERROR}
msg = lambda x: None
logLevel = LOG_INFO
...
def setLevel(level):
    ...
    try:
        ...
        raise SnmpResponderError('Unknown log level "%s", known levels are: %s' % (level, (', ').join(levelsMap)))
...
def setLogger(progId, *priv, **options):
    ...
    try:
        ...
        raise SnmpResponderError('Unknown logging method "%s", known methods are: %s' % (priv[0], (', ').join(methodsMap)))