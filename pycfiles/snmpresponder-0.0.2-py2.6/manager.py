...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/snmpresponder/plugins/manager.py
    ...
    def loadPlugin(self, pluginId, pluginModuleName, pluginOptions):
        ...
        for pluginModulesDir in self.__path:
            ...
            if not os.path.exists(modPath):
                ...
                continue
            ctx = {'modulePath': modPath, 'moduleContext': {}, 'moduleOptions': pluginOptions}
            modData = open(modPath).read()
            try:
                exec (
                 compile(modData, modPath, 'exec'), ctx)
            except Exception:
