...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/snmpresponder/snmpresponderd.py
# Compiled at: 2019-01-31 03:27:43
import os, sys, getopt, traceback, random, re, socket, pkg_resources
...
authProtocols = {'MD5': config.usmHMACMD5AuthProtocol, 
...
privProtocols = {'DES': config.usmDESPrivProtocol, 
...
snmpPduTypesMap = {rfc1905.GetRequestPDU.tagSet: 'GET', 
...
def main():
    ...
    class MibTreeProxyMixIn(object):
        ...
        def _routeToMibTree(self, *varBinds, **context):
            ...
            for (pluginNum, pluginId) in enumerate(pluginIdList):
                (st, pdu) = pluginManager.processCommandRequest(pluginId, snmpEngine, pdu, mibTreeReq, reqCtx)
                if st == status.BREAK:
            ...
            if not mibTreeId:
                ...
                cbCtx = (
                 pluginIdList, mibTreeId, mibTreeReq, snmpEngine, reqCtx, context['cbFun'])
    ...
    class LogString(LazyLogString):
        GROUPINGS = [
         ...
         [
          'client-snmp-pdu']]
        FORMATTERS = {'client-snmp-pdu': LazyLogString.prettyVarBinds, 
    ...
    def requestObserver(snmpEngine, execpoint, variables, cbCtx):
        mibTreeReq = {'callflow-id': '%10.10x' % random.randint(0, 1099511627775), 
        ...
        mibTreeReq['snmp-credentials-id'] = macro.expandMacro(credIdMap.get((
    ...
    if params:
        ...
        with daemon.PrivilegesOf(procUser, procGroup):
            try:
                log.setLogger(PROGRAM_NAME, force=True, *loggingMethod)
                if loggingLevel:
        ...
        try:
            ...
            pluginManager = PluginManager(macro.expandMacros(cfgTree.getAttrValue('plugin-modules-path-list', '', default=[], vector=True), {'config-dir': os.path.dirname(cfgFile)}), progId=PROGRAM_NAME, apiVer=PLUGIN_API_VERSION)
            for pluginCfgPath in cfgTree.getPathsToAttr('plugin-id'):
                ...
                pluginOptions = macro.expandMacros(cfgTree.getAttrValue('plugin-options', default=[], vector=True, *pluginCfgPath), {'config-dir': os.path.dirname(cfgFile)})
            ...
            for configEntryPath in cfgTree.getPathsToAttr('snmp-credentials-id'):
                ...
                if engineId in engineIdMap:
                    ...
                    snmpEngine.observer.registerObserver(securityAuditObserver, 'rfc2576.prepareDataElements:sm-failure', 'rfc3412.prepareDataElements:sm-failure', cbCtx=gCurrentRequestContext)
                    snmpEngine.observer.registerObserver(requestObserver, 'rfc3412.receiveMessage:request', cbCtx=gCurrentRequestContext)
                    snmpEngine.observer.registerObserver(usmRequestObserver, 'rfc3414.processIncomingMsg', cbCtx=gCurrentRequestContext)
                    ...
                    engineIdMap[engineId] = (
                     snmpEngine, snmpContext, snmpEngineMap)
                ...
                if transportDomain[:len(udp.domainName)] != udp.domainName and udp6 and transportDomain[:len(udp6.domainName)] != udp6.domainName:
                ...
                if transportDomain in snmpEngineMap['transportDomain']:
                    ...
                    transportOptions = cfgTree.getAttrValue('snmp-transport-options', default=[], vector=True, *configEntryPath)
                    try:
                        (bindAddr, bindAddrMacro) = endpoint.parseTransportAddress(transportDomain, bindAddr, transportOptions)
                    except SnmpResponderError:
                        ...
                        snmpEngineMap['transportDomain'][transportDomain] = (
                         bindAddr, transportDomain)
                    ...
                    if securityModel in (1, 2):
                        if securityName in snmpEngineMap['securityName']:
                            ...
                            config.addV1System(snmpEngine, securityName, communityName, securityName=securityName)
                        ...
                        if securityName in snmpEngineMap['securityName']:
                            ...
                            securityEngineId = cfgTree.getAttrValue('snmp-security-engine-id', default=None, *configEntryPath)
                            if securityEngineId:
                                securityEngineId = rfc1902.OctetString(securityEngineId)
                            log.info('new USM user %s, security-model %s, security-level %s, security-name %s, security-engine-id %s' % (
                             usmUser, securityModel, securityLevel,
                             securityName, securityEngineId and securityEngineId.prettyPrint() or '<none>'))
                            if securityLevel in (2, 3):
                                usmAuthProto = cfgTree.getAttrValue('snmp-usm-auth-protocol', default=config.usmHMACMD5AuthProtocol, *configEntryPath)
                                try:
                                    ...
                                    if securityLevel == 3:
                                        usmPrivProto = cfgTree.getAttrValue('snmp-usm-priv-protocol', default=config.usmDESPrivProtocol, *configEntryPath)
                                        try:
                                            ...
                                            usmPrivKey = cfgTree.getAttrValue('snmp-usm-priv-key', default=None, *configEntryPath)
                                            log.info('new USM encryption key: %s, encryption protocol: %s' % (usmPrivKey, usmPrivProto))
                                            config.addV3User(snmpEngine, usmUser, usmAuthProto, usmAuthKey, usmPrivProto, usmPrivKey, securityEngineId=securityEngineId)
                                    else:
                                        config.addV3User(snmpEngine, usmUser, usmAuthProto, usmAuthKey, securityEngineId=securityEngineId)
                            else:
            ...
            for peerCfgPath in cfgTree.getPathsToAttr('snmp-peer-id'):
                ...
                for peerAddress in cfgTree.getAttrValue('snmp-peer-address-pattern-list', vector=True, *peerCfgPath):
                    for bindAddress in cfgTree.getAttrValue('snmp-bind-address-pattern-list', vector=True, *peerCfgPath):
            ...
            for contentCfgPath in cfgTree.getPathsToAttr('snmp-content-id'):
                ...
                for x in cfgTree.getAttrValue('snmp-pdu-oid-prefix-pattern-list', vector=True, *contentCfgPath):
            ...
            for pluginCfgPath in cfgTree.getPathsToAttr('using-plugin-id-list'):
                pluginIdList = cfgTree.getAttrValue('using-plugin-id-list', vector=True, *pluginCfgPath)
                log.info('configuring plugin ID(s) %s (at %s)...' % ((',').join(pluginIdList), ('.').join(pluginCfgPath)))
                for credId in cfgTree.getAttrValue('matching-snmp-credentials-id-list', vector=True, *pluginCfgPath):
                    for peerId in cfgTree.getAttrValue('matching-snmp-peer-id-list', vector=True, *pluginCfgPath):
                        for contextId in cfgTree.getAttrValue('matching-snmp-context-id-list', vector=True, *pluginCfgPath):
                            for contentId in cfgTree.getAttrValue('matching-snmp-content-id-list', vector=True, *pluginCfgPath):
                                k = (
            ...
            for routeCfgPath in cfgTree.getPathsToAttr('using-mib-tree-id'):
                ...
                for credId in cfgTree.getAttrValue('matching-snmp-credentials-id-list', vector=True, *routeCfgPath):
                    for peerId in cfgTree.getAttrValue('matching-snmp-peer-id-list', vector=True, *routeCfgPath):
                        for contextId in cfgTree.getAttrValue('matching-snmp-context-id-list', vector=True, *routeCfgPath):
                            for contentId in cfgTree.getAttrValue('matching-snmp-content-id-list', vector=True, *routeCfgPath):
                                k = (
                                 credId, contextId, peerId, contentId)
                                if k in routingMap:
            ...
            for mibTreeCfgPath in cfgTree.getPathsToAttr('mib-tree-id'):
                ...
                mibTextPaths = cfgTree.getAttrValue('mib-text-search-path-list', default=[], vector=True, *mibTreeCfgPath)
                mibCodePatternPaths = macro.expandMacros(cfgTree.getAttrValue('mib-code-modules-pattern-list', default=[], vector=True, *mibTreeCfgPath), {'config-dir': os.path.dirname(cfgFile)})
                ...
                for topDir in mibCodePatternPaths:
                    ...
                    for (root, dirs, files) in os.walk(topDir):
                        if not files or root.endswith('__pycache__'):
                            continue
                        mibBuilder.setMibSources(builder.DirMibSource(root), *mibBuilder.getMibSources())
                        for filename in files:
                            if not filenameRegExp.match(filename):
                                log.debug('skipping non-matching file %s while loading MIB tree ID %s' % (
                                 filename, mibTreeId))
                                continue
                            ...
                            try:
                                ...
                                log.error('fail to load MIB implementation from file %s into MIB tree ID %s' % (
                                 os.path.join(root, filename), mibTreeId))
                                raise SnmpResponderError(str(ex))

                            log.info('loaded MIB implementation file %s into MIB tree ID %s' % (
                             os.path.join(root, filename), mibTreeId))

                mibCodePackages = macro.expandMacros(cfgTree.getAttrValue('mib-code-packages-pattern-list', default=[], vector=True, *mibTreeCfgPath), {'config-dir': os.path.dirname(cfgFile)})
                    ...
                    for entryPoint in pkg_resources.iter_entry_points('snmpresponder.mibs'):
                        ...
                        for filename in os.listdir(root):
                            ...
                            if not mibCodePackageRegExp.match(mibPath):
                                log.debug('extension MIB %s from %s is NOT configured, skipping' % (
                                 mibPath, entryPoint.name))
                                continue
                            if not mibPathSet:
                                mibBuilder.setMibSources(builder.DirMibSource(root), *mibBuilder.getMibSources())
                                mibPathSet = True
                            log.debug('loading extension MIB %s from %s into MIB tree ID %s' % (
                             mibPath, entryPoint.name, mibTreeId))
                            (module, _) = os.path.splitext(filename)
                            try:
                                mibBuilder.loadModule(module)
                            except PySnmpError, ex:
                                log.error('fail to load MIB implementation %s from %s into MIB tree ID %s' % (
                                 mibPath, entryPoint.name,
                                 mibTreeId))
                                raise SnmpResponderError(str(ex))

                            log.info('loaded MIB implementation %s from %s into MIB tree ID %s' % (
                             mibPath, entryPoint.name, mibTreeId))
