...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/snmpresponder/cparser.py
...
class Scanner(object):
    ...
    def load(self, filename):
        try:
            ...
            raise error.SnmpResponderError('cant open config file %s: %s' % (filename, sys.exc_info()[1]))
        ...
        while 1:
            if state == 'FSM_START':
                ...
                if symbol != SYMBOL_SECTION_BEGIN:
                    raise error.SnmpResponderError('%s missing object beginning sign: %s' % (self, token))
                ...
                (token, symbol) = self.scanner.get_token()
                if symbol != SYMBOL_SECTION_END:
                    raise error.SnmpResponderError('%s missing object closure sign: %s' % (self, token))
                state = 'FSM_START'
                ...
                (token, symbol) = self.scanner.get_token()
                if token in obj:
                    raise error.SnmpResponderError('%s multiple option occurrence: %s' % (self, token))
                obj[token] = []
                self.scanner.unget_token()
                ...
                if state == 'FSM_STOP':
                    return obj
                raise error.SnmpResponderError('%s unknown FSM state: %s' % (self, state))

    def parse(self):
        try:
            return self.load_section()
        except error.EofError:
            raise error.SnmpResponderError('%s premature EOF while reading config file' % self)
...
class Config(object):
    ...
    def getAttrValue(self, attr, *nodes, **kwargs):
                ...
                if 'vector' in kwargs:
                    if expect:
                        try:
                            ...
                            raise error.SnmpResponderError('%s value casting error at scope "%s" attribute "%s"' % (self, ('.').join(nodes), attr))
        ...
        raise error.SnmpResponderError('%s non-existing attribute "%s" at scope "%s"' % (self, attr, ('.').join(nodes)))