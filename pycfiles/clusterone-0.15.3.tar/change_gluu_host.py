...
# Embedded file name: /mnt/data/projects/gluu/cluster-mgr/clustermgr/core/change_gluu_host.py
# Compiled at: 2020-05-09 08:47:08
import sys, os, json, argparse
from clustermgr.core.remote import RemoteClient
from clustermgr.core.clustermgr_installer import Installer
from ldap3 import Server, Connection, SUBTREE, BASE, LEVEL, MODIFY_REPLACE, MODIFY_ADD, MODIFY_DELETE
    ...
    for l in old_hosts:
        ...
        if ls:
            if not ls[0] == '#':
                ...
                lss = ls.split()
                ip_addr = lss[0]
                if ip_addr not in hosts[h_type]:
                    hosts[h_type][ip_addr] = []
                for h in lss[1:]:
                    if h not in hosts[h_type][ip_addr] and h != old_host:
...
class ChangeGluuHostname:
    ...
    def __init__(self, old_host, new_host, cert_city, cert_mail, cert_state, cert_country, ldap_password, os_type, ip_address, gluu_version, server='localhost', local=False):
    ...
    def startup(self):
        ...
        self.c.startup()
        self.installer = Installer(self.c, self.gluu_version, self.os_type, self.logger_tid)
        self.installer.hostname = self.server
        return True

    def change_ldap_entries(self):
        print 'Changing LDAP Entries'
        self.conn.modify('ou=configuration,o=gluu', {'gluuHostname': [MODIFY_REPLACE, self.new_host]})
        sdns = [
        ...
        for sdn in sdns:
            ...
            for entry in self.conn.response:
                for field in entry['attributes']:
                    ...
                    if changeAttr:
                        self.conn.modify(entry['dn'], {field: [MODIFY_REPLACE, entry['attributes'][field]]})
    ...
    def create_new_certs(self):
        ...
        cmd_list = [
         ...
         ('/usr/bin/openssl req -new -key /etc/certs/{0}.key -out /etc/certs/{0}.csr -subj "/C={4}/ST={5}/L={1}/O=Gluu/CN={2}/emailAddress={3}"').format('{0}', self.cert_city, self.new_host, self.cert_mail, self.cert_country, self.cert_state),
        ...
        cert_list = [
         'httpd', 'asimba', 'idp-encryption', 'idp-signing', 'shibIDP', 'saml.pem']
...
if __name__ == '__main__':
    ...
    name_changer = ChangeGluuHostname(old_host=args.old, new_host=args.new, cert_city=args.city, cert_mail=args.mail, cert_state=args.state, cert_country=args.country, server=args.server, ldap_password=args.password, os_type=args.os)
