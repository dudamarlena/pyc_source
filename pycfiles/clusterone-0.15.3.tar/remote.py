...
# Embedded file name: /mnt/data/projects/gluu/cluster-mgr/clustermgr/core/remote.py
# Compiled at: 2020-05-09 08:47:08
import StringIO, socket, os, base64, logging
...
class RemoteClient(object):
    ...
    def __init__(self, host, ip=None, user='root', passphrase=None):
        ...
        if not passphrase:
            ...
            if os.path.exists(pw_file):
                ...
                passphrase = decode(os.getenv('NEW_UUID'), encoded_passphrase)
    ...
    def startup(self):
        ...
        try:
            ...
            self.client.connect(self.host, port=22, username=self.user, passphrase=self.passphrase)
            ...
            if self.ip:
                logger.warning('Connection with hostname failed. Retrying with IP')
                self._try_with_ip()
            else:
    ...
    def _try_with_ip(self):
        try:
            ...
            self.client.connect(self.ip, port=22, username=self.user, passphrase=self.passphrase)
            self.sftpclient = self.client.open_sftp()
        except PasswordRequiredException:
            raise ClientNotSetupException('Pubkey is encrypted.')
        except SSHException as e:
    ...
    def download(self, remote, local):
        ...
        if not self.sftpclient:
            raise ClientNotSetupException('Cannot download file. Client not initialized')
        ...
        try:
            ...
            return (
             False, "Error: Local file %s doesn't exist." % local)
        except IOError:
            logger.debug('[%s] ERROR %s', self.host, IOError)
            return (
             False, "Error: Remote location %s doesn't exist." % remote)
    ...
    def upload(self, local, remote):
        ...
        """
        if not self.sftpclient:
            raise ClientNotSetupException('Cannot upload file. Client not initialized')
    ...
    def exists(self, filepath):
        ...
        """
        if not self.client:
            raise ClientNotSetupException('Cannot run procedure. Client not initialized')
    ...
    def __repr__(self):
        return ('RemoteClient({0}, ip={1}, user={2})').format(self.host, self.ip, self.user)


def get_connection(server, task_id):
    wlogger.log(task_id, ('Making SSH connection to {} ...').format(server.hostname), 'action', server.id)
