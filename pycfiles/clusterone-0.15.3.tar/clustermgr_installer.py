...
# Embedded file name: /mnt/data/projects/gluu/cluster-mgr/clustermgr/core/clustermgr_installer.py
# Compiled at: 2020-05-09 08:47:08
import sys, os, re, select
...
class Installer:
    ...
    def __init__(self, conn, gluu_version, server_os=None, logger_task_id=None, server_id=None):
        ...
        if conn.__class__.__name__ != 'RemoteClient' and conn.__class__.__name__ != 'FakeRemote':
            ...
            self.ip = conn.ip
            wlogger.log(self.logger_task_id, ('Making SSH connection to {} ...').format(conn.hostname), 'action', server_id=self.server_id)
            try:
                print ('Installer> Establishing SSH connection to host {}').format(conn.hostname)
                self.conn.startup()
                wlogger.log(self.logger_task_id, ('SSH connection to {} was successful.').format(conn.hostname), 'success', server_id=self.server_id)
            except Exception as e:
                print e
                self.conn = None
                if str(e) == 'Pubkey is encrypted.':
                    error_msg = 'Pubkey seems to password protected. After setting your passphrase re-submit this form.'
                elif str(e) == 'Could not deserialize key data.':
                    error_msg = 'Password you provided for pubkey did not work. After setting your passphrase re-submit this form.'
                else:
                    error_msg = str(e)
                wlogger.log(self.logger_task_id, ("Can't make SSH connection to {}. Reason: {}").format(conn.hostname, error_msg), 'fail', server_id=self.server_id)
    ...
    def settings(self):
        ...
        if self.conn.__class__.__name__ != 'FakeRemote':
            ...
            if self.server_os in ('CentOS 7', 'RHEL 7', 'Ubuntu 18'):
                self.run_command = 'ssh -q -o IdentityFile=/etc/gluu/keys/gluu-console -o UserKnownHostsFile=/dev/null -o Port=60022 -o StrictHostKeyChecking=no -o PubkeyAuthentication=yes root@localhost  "{0}"'
            else:
    ...
    def is_gluu_installed(self):
        check_file = '/opt/gluu-server/install/community-edition-setup/setup.properties.last'
        check_file_enc = '/opt/gluu-server/install/community-edition-setup/setup.properties.last.enc'
