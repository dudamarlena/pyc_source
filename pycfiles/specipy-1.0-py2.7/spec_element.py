...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/specipy/spec_element.py
# Compiled at: 2015-06-07 18:01:13
...
class TestElement:
    ...
    def elements_description_r(self, level, acc):
        prefix = [
         ' '] * (level * 4)
        prefix = ('').join(prefix)
        string = self.decorator.before_decoration() + prefix + self.decorator.name_decoration(self.name) + self.decorator.after_decoration()
        acc.append(string)
        if self.children:
