...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/syzygy/EMFreqAssocStrandNew.py
...
def main(argv=None):
    ...
    parser.add_option('--snplist', default='snplist.alleles.readyannot')
    parser.add_option('--iterations', default=10)
    parser.add_option('--outputdir')
    parser.add_option('--out', default='EMFreqAssoc.out')
    ...
    if not options.force:
        ...
        for key in snplist.keys():
            ...
            populationfrequency[(key, 0)] = 0.4
            populationfrequencyfwd[(key, 0)] = 0.4
            populationfrequencyrev[(key, 0)] = 0.4
            for iter in range(1, iterations):
                ...
                for k in range(0, len(casemap)):
                    ...
                    if (
                     key, k, 'coverage') in snpscases and snpscases[(key, k, 'coverage')] > 0:
                        ...
                        for l in range(0, int(chromcasefile[casemapbwd[k]]) + 1):
                            if l == 0:
                                postprobarray.append(r.dbinom(snpscases[(key, k, 'nonref')], snpscases[(key, k, 'coverage')], 0.001) * r.dbinom(l, int(chromcasefile[casemapbwd[k]]), populationfrequency[(key, iter - 1)]))
                                postprobarrayfwd.append(r.dbinom(snpscases[(key, k, 'nonreffwd')], snpscases[(key, k, 'coveragefwd')], 0.001) * r.dbinom(l, int(chromcasefile[casemapbwd[k]]), populationfrequencyfwd[(key, iter - 1)]))
                                postprobarrayrev.append(r.dbinom(snpscases[(key, k, 'nonrefrev')], snpscases[(key, k, 'coveragerev')], 0.001) * r.dbinom(l, int(chromcasefile[casemapbwd[k]]), populationfrequencyrev[(key, iter - 1)]))
                            else:
                                postprobarray.append(r.dbinom(snpscases[(key, k, 'nonref')], snpscases[(key, k, 'coverage')], l / int(chromcasefile[casemapbwd[k]])) * r.dbinom(l, int(chromcasefile[casemapbwd[k]]), populationfrequency[(key, iter - 1)]))
                                postprobarrayfwd.append(r.dbinom(snpscases[(key, k, 'nonreffwd')], snpscases[(key, k, 'coveragefwd')], l / int(chromcasefile[casemapbwd[k]])) * r.dbinom(l, int(chromcasefile[casemapbwd[k]]), populationfrequencyfwd[(key, iter - 1)]))
                                postprobarrayrev.append(r.dbinom(snpscases[(key, k, 'nonrefrev')], snpscases[(key, k, 'coveragerev')], l / int(chromcasefile[casemapbwd[k]])) * r.dbinom(l, int(chromcasefile[casemapbwd[k]]), populationfrequencyrev[(key, iter - 1)]))
                        ...
                        variancealtcnts += 0.01 * variancepostprobinit
                ...
                for k in range(0, len(controlmap)):
                    ...
                    postprobarraynormrev = []
                    if (
                     key, k, 'coverage') in snpscontrols and snpscontrols[(key, k, 'coverage')] > 0:
                        ...
                        for l in range(0, int(chromcontrolfile[controlmapbwd[k]]) + 1):
                            if l == 0:
                                postprobarray.append(r.dbinom(snpscontrols[(key, k, 'nonref')], snpscontrols[(key, k, 'coverage')], 0.001) * r.dbinom(l, int(chromcontrolfile[controlmapbwd[k]]), populationfrequency[(key, iter - 1)]))
                                postprobarrayfwd.append(r.dbinom(snpscontrols[(key, k, 'nonreffwd')], snpscontrols[(key, k, 'coveragefwd')], 0.001) * r.dbinom(l, int(chromcontrolfile[controlmapbwd[k]]), populationfrequencyfwd[(key, iter - 1)]))
                                postprobarrayrev.append(r.dbinom(snpscontrols[(key, k, 'nonrefrev')], snpscontrols[(key, k, 'coveragerev')], 0.001) * r.dbinom(l, int(chromcontrolfile[controlmapbwd[k]]), populationfrequencyrev[(key, iter - 1)]))
                            else:
                                postprobarray.append(r.dbinom(snpscontrols[(key, k, 'nonref')], snpscontrols[(key, k, 'coverage')], l / int(chromcontrolfile[controlmapbwd[k]])) * r.dbinom(l, int(chromcontrolfile[controlmapbwd[k]]), populationfrequency[(key, iter - 1)]))
                                postprobarrayfwd.append(r.dbinom(snpscontrols[(key, k, 'nonreffwd')], snpscontrols[(key, k, 'coveragefwd')], l / int(chromcontrolfile[controlmapbwd[k]])) * r.dbinom(l, int(chromcontrolfile[controlmapbwd[k]]), populationfrequencyfwd[(key, iter - 1)]))
                                postprobarrayrev.append(r.dbinom(snpscontrols[(key, k, 'nonrefrev')], snpscontrols[(key, k, 'coveragerev')], l / int(chromcontrolfile[controlmapbwd[k]])) * r.dbinom(l, int(chromcontrolfile[controlmapbwd[k]]), populationfrequencyrev[(key, iter - 1)]))
        ...
        for key in snplist.keys():
            ...
            if key in observedallelescontrolsdict:
                ...
                controlchromtot = 0
            if weightscasetmp == 1 and weightscontrolstmp == 1 and not isinstance(weightscase, dict) and not isinstance(weightscontrols, dict):
            ...
            for j in range(0, len(casemap)):
                if (
                 key, j, 'nonref') in snpscases and snpscases[(key, j, 'coverage')] > 0:
                    for m in range(0, int(chromcasefile[casemapbwd[j]]) + 1):
                        if m == 0:
                            factor = 0.002
                        else:
                            factor = 0
                        likelihoodfinal += r.log10(r.dbinom(snpscases[(key, j, 'nonref')], snpscases[(key, j, 'coverage')], m / chromcasefile[casemapbwd[j]] + factor) * r.dbinom(m, chromcasefile[casemapbwd[j]], casefrequency[(key, iterations - 1)]))
                        likelihoodfwd += r.log10(r.dbinom(snpscases[(key, j, 'nonreffwd')], snpscases[(key, j, 'coveragefwd')], m / chromcasefile[casemapbwd[j]] + factor) * r.dbinom(m, chromcasefile[casemapbwd[j]], populationfrequency[(key, iterations - 1)]))
                        likelihoodrev += r.log10(r.dbinom(snpscases[(key, j, 'nonrefrev')], snpscases[(key, j, 'coveragerev')], m / chromcasefile[casemapbwd[j]] + factor) * r.dbinom(m, chromcasefile[casemapbwd[j]], populationfrequency[(key, iterations - 1)]))
                        likelihoodnull += r.log10(r.dbinom(snpscases[(key, j, 'nonrefrev')], snpscases[(key, j, 'coveragerev')], m / chromcasefile[casemapbwd[j]] + factor) * r.dbinom(m, chromcasefile[casemapbwd[j]], populationfrequency[(key, iterations - 1)]))
                        likelihoodnull += r.log10(r.dbinom(snpscases[(key, j, 'nonreffwd')], snpscases[(key, j, 'coveragefwd')], m / chromcasefile[casemapbwd[j]] + factor) * r.dbinom(m, chromcasefile[casemapbwd[j]], populationfrequency[(key, iterations - 1)]))

            for k in range(0, len(controlmap)):
                if (
                 key, k, 'nonref') in snpscontrols and snpscontrols[(key, k, 'coverage')] > 0:
                    for m in range(0, int(chromcontrolfile[controlmapbwd[k]]) + 1):
                        if m == 0:
                            factor = 0.002
                        else:
                            factor = 0
                        likelihoodfinal += r.log10(r.dbinom(snpscontrols[(key, k, 'nonref')], snpscontrols[(key, k, 'coverage')], m / chromcontrolfile[controlmapbwd[k]] + factor) * r.dbinom(m, chromcontrolfile[controlmapbwd[k]], controlfrequency[(key, iterations - 1)]))
                        likelihoodfwd += r.log10(r.dbinom(snpscontrols[(key, k, 'nonreffwd')], snpscontrols[(key, k, 'coveragefwd')], m / chromcontrolfile[controlmapbwd[k]] + factor) * r.dbinom(m, chromcontrolfile[controlmapbwd[k]], populationfrequency[(key, iterations - 1)]))
                        likelihoodrev += r.log10(r.dbinom(snpscontrols[(key, k, 'nonrefrev')], snpscontrols[(key, k, 'coveragerev')], m / chromcontrolfile[controlmapbwd[k]] + factor) * r.dbinom(m, chromcontrolfile[controlmapbwd[k]], populationfrequency[(key, iterations - 1)]))
                        likelihoodnull += r.log10(r.dbinom(snpscontrols[(key, k, 'nonrefrev')], snpscontrols[(key, k, 'coveragerev')], m / chromcontrolfile[controlmapbwd[k]] + factor) * r.dbinom(m, chromcontrolfile[controlmapbwd[k]], populationfrequency[(key, iterations - 1)]))
                        likelihoodnull += r.log10(r.dbinom(snpscontrols[(key, k, 'nonreffwd')], snpscontrols[(key, k, 'coveragefwd')], m / chromcontrolfile[controlmapbwd[k]] + factor) * r.dbinom(m, chromcontrolfile[controlmapbwd[k]], populationfrequency[(key, iterations - 1)]))

            for j in range(0, len(casemap)):
                if (
                 key, j, 'nonref') in snpscases and snpscases[(key, j, 'coverage')]:
                    for m in range(0, int(chromcasefile[casemapbwd[j]]) + 1):
                        if m == 0:
                            factor = 0.002
                        else:
                            factor = 0
                        likelihoodfinal -= r.log10(r.dbinom(snpscases[(key, j, 'nonref')], snpscases[(key, j, 'coverage')], m / chromcasefile[casemapbwd[j]] + factor) * r.dbinom(m, chromcasefile[casemapbwd[j]], populationfrequency[(key, iterations - 1)]))
                        likelihoodalt1 += r.log10(r.dbinom(snpscases[(key, j, 'nonreffwd')], snpscases[(key, j, 'coveragefwd')], m / chromcasefile[casemapbwd[j]] + factor) * r.dbinom(m, chromcasefile[casemapbwd[j]], populationfrequencyfwd[(key, iterations - 1)]))
                        likelihoodalt2 += r.log10(r.dbinom(snpscases[(key, j, 'nonrefrev')], snpscases[(key, j, 'coveragerev')], m / chromcasefile[casemapbwd[j]] + factor) * r.dbinom(m, chromcasefile[casemapbwd[j]], populationfrequencyrev[(key, iterations - 1)]))
                ...
                if (key, k, 'nonref') in snpscontrols and snpscontrols[(key, k, 'coverage')]:
                    for m in range(0, int(chromcontrolfile[controlmapbwd[k]]) + 1):
                        if m == 0:
                            ...
                            factor = 0
                        likelihoodfinal -= r.log10(r.dbinom(snpscontrols[(key, k, 'nonref')], snpscontrols[(key, k, 'coverage')], m / chromcontrolfile[controlmapbwd[j]] + factor) * r.dbinom(m, chromcontrolfile[controlmapbwd[j]], populationfrequency[(key, iterations - 1)]))
                        likelihoodalt1 += r.log10(r.dbinom(snpscontrols[(key, k, 'nonreffwd')], snpscontrols[(key, k, 'coveragefwd')], m / chromcontrolfile[controlmapbwd[j]] + factor) * r.dbinom(m, chromcontrolfile[controlmapbwd[j]], populationfrequencyfwd[(key, iterations - 1)]))
                        likelihoodalt2 += r.log10(r.dbinom(snpscontrols[(key, k, 'nonrefrev')], snpscontrols[(key, k, 'coveragerev')], m / chromcontrolfile[controlmapbwd[j]] + factor) * r.dbinom(m, chromcontrolfile[controlmapbwd[j]], populationfrequencyrev[(key, iterations - 1)]))
            ...
            if '-inf' in str(likelihoodalt2):
                ...
                likelihoodalt2 = 10000
            strandLOD1 = likelihoodalt1 - likelihoodnull
            strandLOD2 = likelihoodalt2 - likelihoodnull
            ...
            summarytest.write(str(key) + ' ' + str(chisqstat2) + ' ' + str(LRTassoc) + ' ' + popfreqstr + ' ' + casefreqstr + ' ' + controlfreqstr + ' ' + str(strandLOD) + ' ' + str(populationfrequencyfwd[(key, iterations - 1)]) + ' ' + str(populationfrequencyrev[(key, iterations - 1)]) + '\n')
