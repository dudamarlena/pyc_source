...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/syzygy/PlotVisualizations.py
...
def main(argv=None):
    ...
    parser.add_option('--heatmap', default='heatmap.exon.matrix')
    parser.add_option('--pf', default='PooledExperiment.pf')
    ...
    if not options.force:
        ...
        for line in poolfile[1:]:
            ...
            for line in filenameread[1:]:
                ...
                if chroffset in exonmap:
                    if (
                     exonmap[chroffset], bampool) in coverageexon:
        ...
        for keyexon in exons.keys():
            ...
            for keymap in poolmap.keys():
                if (
                 keyexon, poolmap[keymap]) in coverageexon:
