...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/syzygy/FinalAnnotation.py
...
def main(argv=None):
    ...
    parser.add_option('--sndb', default='false')
    parser.add_option('--ncpu')
    parser.add_option('--dbsnp')
    parser.add_option('--bqthr')
    parser.add_option('--sndout', default='sndlod.2b')
    parser.add_option('--nonsynonymous', default='snplist.alleles.ns')
    parser.add_option('--synonymous', default='snplist.alleles.syn')
    parser.add_option('--noncoding', default='snplist.alleles.nc')
    parser.add_option('--utrs', default='snplist.alleles.utr')
    parser.add_option('--utrsncnt', default='snplist.alleles.utrncnt')
    parser.add_option('--emsummary', default='EMFreqAssoc.out')
    parser.add_option('--snplist', default='snplist.alleles.readyannot')
    parser.add_option('--fishertablehigh', default='snplist.alleles.high')
    parser.add_option('--module')
    parser.add_option('--fishertablepoor', default='snplist.alleles.poor')
    parser.add_option('--out', default='PooledExperiment.summary')
    ...
    if not options.force:
        ...
        if options.skipannot == 'true':
            ...
            for snpline in snpfileread[1:]:
                ...
                if float(fisherdict[chroffsetsnp][0]) >= 0.1:
            ...
            for snpline in snpfileread[1:]:
                ...
                if chroffsetsnp in exptsummary:
                    ...
                    if float(exptsummary[chroffsetsnp]) > 5 or float(fisherdict[chroffsetsnp][0]) < 0.1:
                        ...
                        if (chroffsetsnp, 'rs') in snpsdict:
                            if 'rs' in snpsdict[(chroffsetsnp, 'rs')]:
                                low2bdbS += 1
                        if (
                         refallele, 'low2b') in propN:
                            propN[(refallele, 'low2b')] += 1
                        else:
                            propN[(refallele, 'low2b')] = 0
                            propN[(refallele, 'low2b')] += 1
                        if (
                         transition, 'low2b') in propN:
                        ...
                        if (chroffsetsnp, 'rs') in snpsdict:
                            if 'rs' in snpsdict[(chroffsetsnp, 'rs')]:
                                medCdbS += 1
                        if (
                         refallele, 'medC') in propN:
                            propN[(refallele, 'medC')] += 1
                        else:
                            propN[(refallele, 'medC')] = 0
                            propN[(refallele, 'medC')] += 1
                        if (
                         transition, 'medC') in propN:
                        ...
                        if (
                         ...
                         chroffsetsnp, 'chroffset') in syn:
                            medCSyn += 1
                    elif float(exptsummary[chroffsetsnp]) < 0 and float(fisherdict[chroffsetsnp][0]) >= 0.1:
                        highS += 1
                        ...
                        if (chroffsetsnp, 'rs') in snpsdict:
                            if 'rs' in snpsdict[(chroffsetsnp, 'rs')]:
                                highdbS += 1
                        if (
                         refallele, 'high') in propN:
                            propN[(refallele, 'high')] += 1
                        else:
                            propN[(refallele, 'high')] = 0
                            propN[(refallele, 'high')] += 1
                        if (
                         transition, 'high') in propN:
                        ...
                        if (
                         ...
                         chroffsetsnp, 'chroffset') in syn:
                            highSyn += 1
                    elif float(exptsummary[chroffsetsnp]) >= 0 and float(exptsummary[chroffsetsnp]) <= 5 and float(fisherdict[chroffsetsnp][0]) >= 0.1:
                        ...
                        if (chroffsetsnp, 'rs') in snpsdict:
                            if 'rs' in snpsdict[(chroffsetsnp, 'rs')]:
                                meddbS += 1
                        if (
                         refallele, 'med') in propN:
                            propN[(refallele, 'med')] += 1
                        else:
                            propN[(refallele, 'med')] = 0
                            propN[(refallele, 'med')] += 1
                        if (
                         transition, 'med') in propN:
