...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/syzygy/SyzygyWrapper.py
# Compiled at: 2010-10-12 17:55:40
import GenerateCIFFile, FinalAnnotation, CleanUpSNPCalls, EMFreqAssocStrand, CalphaRareTest, runsamtools, runsamtoolsparallel, SAMPileupAnalysis, SAMpileuphelper, ErrorModel, PlotVisualizations, IndividualPoolSummary, Learn2ndbestIntensity, AnnotateSNP
from mpgutils import utils
from mpgutils.RUtils import RscriptToPython
import runrscript, runrscriptparallel, SyzygyPower
...
def main(argv=None):
    ...
    parser.add_option('--tgf', help='Experimental Target File')
    parser.add_option('--pif', help='Pool/Sample Info File')
    parser.add_option('--rarethr', help='Frequency Threshold for classifying variant as rare (default = .03)')
    parser.add_option('--hg', help='Specify Human Genome build being used (Currently 17 or 18)')
    parser.add_option('--chr', default='T')
    parser.add_option('--samtoolspath', help='Specify SAMTools Path')
    parser.add_option('--sndb', help='2nd best base annotation with Quality Scores (true or false)', default='false')
    parser.add_option('--threaded', help='Allow Threading (default=false)', default='false')
    parser.add_option('--ncpu', help='Number of CPUs', default=1)
    parser.add_option('--bqthr', help='Base Quality Threshold (default Q = 22)', default=22)
    parser.add_option('--mqthr', help='Mapping Quality Threshold (default Q = 1)', default=1)
    parser.add_option('--dbsnp', help='dbSNP File')
    parser.add_option('--plots', help='Generates Plots for Error Modeling, Frequency Estimation, Power Computations, and Calpha Diagnostics (true or false)')
    parser.add_option('--bds', help='Bounds for Calpha Test (default = 5)', default=5)
    parser.add_option('--module', default='false')
    parser.add_option('--outputdir', help='Define directory for output files')
    parser.add_option('--ref', help='FASTA for Reference Genome')
    parser.add_option('--cif', help=' Coordinate File for SAMTools (default: generate from target file)', default='')
    parser.add_option('--skipannot', help='Skip Annotation', default='false')
    (dctOptions, lstArgs) = parser.parse_args(argv)
    if dctOptions.module == 'false':
        lstValidbuild = [
         '17', '18']
        if dctOptions.hg not in lstValidbuild:
        ...
        if not dctOptions.cif:
            possible_args = [
             'tgf', 'outputdir', 'samtoolspath', 'pif']
        ...
        if dctOptions.samtoolspath and dctOptions.ncpu == 1:
            if dctOptions.cif:
                possible_args = [
                 'cif', 'ref', 'pif', 'outputdir', 'samtoolspath', 'sndb']
                current_args = MakeArguments(possible_args, dctOptions)
                print '\n------Running SAMTools------------------'
                runsamtools.main(current_args)
            else:
                possible_args = [
                 'tgf', 'ref', 'pif', 'outputdir', 'samtoolspath', 'sndb']
                current_args = MakeArguments(possible_args, dctOptions)
                print '\n------Running SAMTools------------------'
                runsamtools.main(current_args)
            ...
            if dctOptions.cif:
                possible_args = [
                 'cif', 'ref', 'pif', 'outputdir', 'samtoolspath', 'sndb', 'ncpu']
                current_args = MakeArguments(possible_args, dctOptions)
                print '\n------Running SAMTools Parallel------------------'
                runsamtoolsparallel.main(current_args)
            else:
                possible_args = [
                 'tgf', 'ref', 'pif', 'outputdir', 'samtoolspath', 'sndb', 'ncpu']
                current_args = MakeArguments(possible_args, dctOptions)
                print '\n------Running SAMTools Parallel------------------'
                runsamtoolsparallel.main(current_args)
        else:
