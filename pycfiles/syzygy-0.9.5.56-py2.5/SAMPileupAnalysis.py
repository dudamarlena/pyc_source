...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/syzygy/SAMPileupAnalysis.py
# Compiled at: 2010-10-26 06:53:48
from __future__ import division
from optparse import OptionParser
import array, numpy, sys, re
...
def main(argv=None):
    ...
    parser.add_option('--bqthr', default=22, help='Base Quality Threshold Parameter')
    parser.add_option('--mqthr', help='Read Mapping Quality Threshold Parameter', default=1)
    ...
    parser.add_option('--dbsnp', help='dbSNP File', default='/seq/dirseq/rivas/Rivasglioma/dbsnp/basepos.snpshg18.dbsnp')
    ...
    if not options.force:
        ...
        for linef in poolfile[1:]:
            ...
            if len(miscallratefwdthresholdeddbsnp) > 0:
                r.points(miscallratefwdthresholdeddbsnp, miscallraterevthresholdeddbsnp, col='blue', pch=18)
            r.plot(miscallratefwdthresholded, miscallraterevthresholded, xlim=r.c(0, 0.1), ylim=r.c(0, 0.1), main='%s' % fname, xlab='Forward Miscall Rate', ylab='Reverse Miscall Rate')
            if len(miscallratefwdthresholdeddbsnp) > 0:
                r.points(miscallratefwdthresholdeddbsnp, miscallraterevthresholdeddbsnp, col='blue', pch=18)
            r.plot(miscallratefwdthresholded, miscallraterevthresholded, xlim=r.c(0, 0.01), ylim=r.c(0, 0.01), main='%s' % fname, xlab='Forward Miscall Rate', ylab='Reverse Miscall Rate')
            if len(miscallratefwdthresholdeddbsnp) > 0:
                r.points(miscallratefwdthresholdeddbsnp, miscallraterevthresholdeddbsnp, col='blue', pch=18)
            r.dev_off()
            ...
            outsummary.write(fname + ' has' + '\t' + str(len([ i for i in coverageproportion if i >= 0.8 ]) / len(coverageproportion) * 100) + '\t' + '% of bases recovered with greater than 80% of original coverage' + '\n')
            outsummary.write(fname + ' has' + '\t' + str(len([ i for i in coverageproportion if i < 0.8 and i > 0.6 ]) / len(coverageproportion) * 100) + '\t' + '% of bases recovered with greater than 60% of bases but less than 80% of original coverage' + '\n')
            outsummary.write(fname + ' has' + '\t' + str(len([ i for i in coverageproportion if i < 0.4 ]) / len(coverageproportion) * 100) + '\t' + '% of bases recovered with less than 40% of original coverage' + '\n')
        ...
        if len(mcrateuniversalfwddb) > 0:
            r.points(mcrateuniversalfwddb, mcrateuniversalrevdb, col='blue', pch=18)
        r.plot(mcrateuniversalfwd, mcrateuniversalrev, xlim=r.c(0, 0.1), ylim=r.c(0, 0.1), main='Miscall Rate %s' % qthr, xlab='Forward miscall rate', ylab='Reverse miscall rate')
        if len(mcrateuniversalfwddb) > 0:
            r.points(mcrateuniversalfwddb, mcrateuniversalrevdb, col='blue', pch=18)
        r.plot(mcrateuniversalfwd, mcrateuniversalrev, xlim=r.c(0, 0.01), ylim=r.c(0, 0.01), main='Miscall Rate %s' % qthr, xlab='Forward miscall rate', ylab='Reverse miscall rate')
        if len(mcrateuniversalfwddb) > 0:
            r.points(mcrateuniversalfwddb, mcrateuniversalrevdb, col='blue', pch=18)
        r.dev_off()
