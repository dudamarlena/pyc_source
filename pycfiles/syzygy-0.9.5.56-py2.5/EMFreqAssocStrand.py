...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/syzygy/EMFreqAssocStrand.py
...
def main(argv=None):
    ...
    parser.add_option('--skipdosage', default='FALSE')
                    ...
                    if (
                     key, k, 'coverage') in snpscases and snpscases[(key, k, 'coverage')] > 0:
                        ...
                        for l in range(0, int(chromcasefile[casemapbwd[k]]) + 1):
                            if l == 0:
                                postprobarray.append(r.dbinom(snpscases[(key, k, 'nonref')], snpscases[(key, k, 'coverage')], 0.001)[0] * r.dbinom(l, int(chromcasefile[casemapbwd[k]]), populationfrequency[(key, iter - 1)])[0])
                                postprobarrayfwd.append(r.dbinom(snpscases[(key, k, 'nonreffwd')], snpscases[(key, k, 'coveragefwd')], 0.001)[0] * r.dbinom(l, int(chromcasefile[casemapbwd[k]]), populationfrequencyfwd[(key, iter - 1)])[0])
                                postprobarrayrev.append(r.dbinom(snpscases[(key, k, 'nonrefrev')], snpscases[(key, k, 'coveragerev')], 0.001)[0] * r.dbinom(l, int(chromcasefile[casemapbwd[k]]), populationfrequencyrev[(key, iter - 1)])[0])
                            else:
                        ...
                        if iter == iterations - 1:
                            ...
                            if str(options.skipdosage) == 'FALSE':
                    ...
                    if (
                     key, k, 'coverage') in snpscontrols and snpscontrols[(key, k, 'coverage')] > 0:
                        ...
                        for l in range(0, int(chromcontrolfile[controlmapbwd[k]]) + 1):
                            if l == 0:
                                postprobarray.append(r.dbinom(snpscontrols[(key, k, 'nonref')], snpscontrols[(key, k, 'coverage')], 0.001)[0] * r.dbinom(l, int(chromcontrolfile[controlmapbwd[k]]), populationfrequency[(key, iter - 1)])[0])
                                postprobarrayfwd.append(r.dbinom(snpscontrols[(key, k, 'nonreffwd')], snpscontrols[(key, k, 'coveragefwd')], 0.001)[0] * r.dbinom(l, int(chromcontrolfile[controlmapbwd[k]]), populationfrequencyfwd[(key, iter - 1)])[0])
                                postprobarrayrev.append(r.dbinom(snpscontrols[(key, k, 'nonrefrev')], snpscontrols[(key, k, 'coveragerev')], 0.001)[0] * r.dbinom(l, int(chromcontrolfile[controlmapbwd[k]]), populationfrequencyrev[(key, iter - 1)])[0])
                            else:
        ...
        for key in snplist.keys():
            ...
            for j in range(0, len(casemap)):
                if (
                 key, j, 'nonref') in snpscases and snpscases[(key, j, 'coverage')]:
                    ...
                    likelihoodalt1fwd[j] = likelihoodalt1fwd[j] * r.dbinom(snpscases[(key, j, 'nonrefrev')], snpscases[(key, j, 'coveragerev')], 0.002)[0]
                    likelihoodalt2rev[j] = likelihoodalt2rev[j] * r.dbinom(snpscases[(key, j, 'nonreffwd')], snpscases[(key, j, 'coveragefwd')], 0.002)[0]
            ...
            for k in range(0, len(controlmap)):
                if (key, k, 'nonref') in snpscontrols and snpscontrols[(key, k, 'coverage')]:
                    ...
                    likelihoodalt1fwd[k + len(casemap)] = likelihoodalt1fwd[(k + len(casemap))] * r.dbinom(snpscontrols[(key, k, 'nonrefrev')], snpscontrols[(key, k, 'coveragerev')], 0.002)[0]
                    likelihoodalt2rev[k + len(casemap)] = likelihoodalt2rev[(k + len(casemap))] * r.dbinom(snpscontrols[(key, k, 'nonreffwd')], snpscontrols[(key, k, 'coveragefwd')], 0.002)[0]
