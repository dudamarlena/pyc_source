...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/syzygy/IndividualPoolSummary.py
...
def main(argv=None):
    ...
    parser.add_option('--dosage', default='snps.dosage.pool')
    parser.add_option('--summary', default='PooledExperiment.summary')
    parser.add_option('--snpcalls', default='PooledExperiment.snpcalls')
    parser.add_option('--out', default='PooledExperiment.pbp')
    ...
    if not options.force:
        ...
        for pool in poollist.keys():
            ...
            for key in snps.keys():
                if (
                 key, pool, 'lod') in pooldict:
                    if float(pooldict[(key, pool, 'lod')]) >= 3:
                        poolcnt += 1
                        if snpsdict[(key, 'S')] < 0 and snpsdict[(key, 'fisher')] >= 0.1:
                            poolcntq[0] += 1
                            ...
                            if snpsdict[(key, 'type')] == 'ns':
                                ...
                                poolsyn[0] += 1
                        elif snpsdict[(key, 'S')] >= 0 and snpsdict[(key, 'S')] <= 5 and snpsdict[(key, 'fisher')] >= 0.1:
                            poolcntq[1] += 1
                            if 'rs' in snpsdict[(key, 'rs')]:
                                pooldb[1] += 1
                            else:
                            ...
                            if snpsdict[(key, 'type')] == 'ns':
                                ...
                                poolsyn[1] += 1
                        elif snpsdict[(key, 'S')] > 5 or snpsdict[(key, 'fisher')] > 0.1:
                            poolcntq[2] += 1
                            if 'rs' in snpsdict[(key, 'rs')]:
                                pooldb[2] += 1
                            else:
            ...
            dbratio = [ pooldb[i] / (poolnondb[i] + pooldb[i] + 1e-06) for i in range(0, len(pooldb)) ]
            nssratio = [ poolns[i] / (poolsyn[i] + 1e-06) for i in range(0, len(poolns)) ]
            tstratio = [ transitions[i] / (transversions[i] + 1e-06) for i in range(0, len(transitions)) ]
