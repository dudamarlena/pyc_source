...
# [GCC 8.4.0]
# Embedded file name: /home/sushi/Projects/CTU/MI-PYT_B171/labelord/tests/tests_cli/test_run_replace.py
# Compiled at: 2017-11-22 03:33:06
# Size of source mod 2**32: 6636 bytes
import builtins as @py_builtins, _pytest.assertion.rewrite as @pytest_ar

def test_replace_empty_repos(invoker, utils):
    invocation = invoker('-c', (utils.config('config_emptyrepos')), 'run',
      'replace', session_expectations={'get':0, 
     'post':0, 
     'patch':0, 
     'delete':0})
...
def test_replace_empty_labels(invoker, utils):
    invocation = invoker('--config', (utils.config('config_emptylabels')), 'run',
      'replace', session_expectations={'get':2, 
     'post':0, 
     'patch':0, 
     'delete':4})
    lines = invocation.result.output.split('\n')
...
def test_replace_all_repos(invoker, utils):
    invocation = invoker('--config', (utils.config('config_normal')), 'run',
      'replace', '--all-repos', session_expectations={'get':5, 
     'post':9, 
     'patch':1, 
     'delete':3})
    lines = invocation.result.output.split('\n')
...
def test_replace_template_repo(invoker, utils):
    invocation = invoker('-c', (utils.config('config_templaterepo')), 'run',
      'replace', session_expectations={'get':3, 
     'post':3, 
     'patch':1, 
     'delete':3})
    lines = invocation.result.output.split('\n')
...
def test_replace_dry(invoker, utils):
    invocation = invoker('--config', (utils.config('config_normal')), 'run',
      'replace', '--dry-run', session_expectations={'get':2, 
     'post':0, 
     'patch':0, 
     'delete':0})
    lines = invocation.result.output.split('\n')
...
def test_replace_dry_with_errors(invoker, utils):
    invocation = invoker('-c', (utils.config('config_errors')), 'run',
      'replace', '-d', session_expectations={'get':3, 
     'post':0, 
     'patch':0, 
     'delete':0})
    lines = invocation.result.output.split('\n')
