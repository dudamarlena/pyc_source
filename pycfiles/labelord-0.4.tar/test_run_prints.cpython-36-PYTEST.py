...
# [GCC 8.4.0]
# Embedded file name: /home/sushi/Projects/CTU/MI-PYT_B171/labelord/tests/tests_cli/test_run_prints.py
# Compiled at: 2017-11-22 03:33:06
# Size of source mod 2**32: 10248 bytes
import builtins as @py_builtins, _pytest.assertion.rewrite as @pytest_ar

def test_update_verbose(invoker, utils):
    invocation = invoker('--config', (utils.config('config_normal')), 'run',
      'update', '--verbose', session_expectations={'get':2, 
     'post':3, 
     'patch':1, 
     'delete':0})
    ...
    @py_assert0 = '[ADD][SUC] MarekSuchanek/repo1; label2; CCAAFF'
    ...
    @py_assert0 = '[UPD][SUC] MarekSuchanek/repo1; label3; 00FF00'
    @py_assert2 = @py_assert0 in lines
    if not @py_assert2:
        @py_format4 = @pytest_ar._call_reprcompare(('in', ), (@py_assert2,), ('%(py1)s in %(py3)s', ), (@py_assert0, lines)) % {'py1':@pytest_ar._saferepr(@py_assert0),  'py3':@pytest_ar._saferepr(lines) if 'lines' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(lines) else 'lines'}
        @py_format6 = 'assert %(py5)s' % {'py5': @py_format4}
        raise AssertionError(@pytest_ar._format_explanation(@py_format6))
    @py_assert0 = @py_assert2 = None
    @py_assert0 = '[ADD][SUC] MarekSuchanek/repo2; label1; FFAA00'
    @py_assert2 = @py_assert0 in lines
    if not @py_assert2:
        @py_format4 = @pytest_ar._call_reprcompare(('in', ), (@py_assert2,), ('%(py1)s in %(py3)s', ), (@py_assert0, lines)) % {'py1':@pytest_ar._saferepr(@py_assert0),  'py3':@pytest_ar._saferepr(lines) if 'lines' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(lines) else 'lines'}
        @py_format6 = 'assert %(py5)s' % {'py5': @py_format4}
        raise AssertionError(@pytest_ar._format_explanation(@py_format6))
    @py_assert0 = @py_assert2 = None
    @py_assert0 = '[ADD][SUC] MarekSuchanek/repo2; label3; 00FF00'
    @py_assert2 = @py_assert0 in lines
    if not @py_assert2:
        @py_format4 = @pytest_ar._call_reprcompare(('in', ), (@py_assert2,), ('%(py1)s in %(py3)s', ), (@py_assert0, lines)) % {'py1':@pytest_ar._saferepr(@py_assert0),  'py3':@pytest_ar._saferepr(lines) if 'lines' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(lines) else 'lines'}
        @py_format6 = 'assert %(py5)s' % {'py5': @py_format4}
...
def test_replace_verbose(invoker, utils):
    invocation = invoker('-c', (utils.config('config_normal')), 'run',
      'replace', '-v', session_expectations={'get':2, 
     'post':3, 
     'patch':1, 
     'delete':1})
    lines = invocation.result.output.split('\n')
    ...
    if not @py_assert2:
        ...
        raise AssertionError(@pytest_ar._format_explanation(@py_format6))
    @py_assert0 = @py_assert2 = None
    @py_assert0 = '[DEL][SUC] MarekSuchanek/repo1; label4; 771077'
    @py_assert2 = @py_assert0 in lines
    if not @py_assert2:
        @py_format4 = @pytest_ar._call_reprcompare(('in', ), (@py_assert2,), ('%(py1)s in %(py3)s', ), (@py_assert0, lines)) % {'py1':@pytest_ar._saferepr(@py_assert0),  'py3':@pytest_ar._saferepr(lines) if 'lines' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(lines) else 'lines'}
        @py_format6 = 'assert %(py5)s' % {'py5': @py_format4}
...
def test_update_normal(invoker, utils):
    invocation = invoker('--config', (utils.config('config_normal')), 'run',
      'update', '--quiet', '--verbose', session_expectations={'get':2, 
     'post':3, 
     'patch':1, 
     'delete':0})
    lines = invocation.result.output.split('\n')
...
def test_replace_normal(invoker, utils):
    invocation = invoker('-c', (utils.config('config_normal')), 'run',
      'replace', '-q', '-v', session_expectations={'get':2, 
     'post':3, 
     'patch':1, 
     'delete':1})
    lines = invocation.result.output.split('\n')
...
def test_update_quiet(invoker, utils):
    invocation = invoker('-c', (utils.config('config_normal')), 'run',
      'update', '-q', session_expectations={'get':2, 
     'post':3, 
     'patch':1, 
     'delete':0})
    lines = invocation.result.output.split('\n')
...
def test_replace_quiet(invoker, utils):
    invocation = invoker('--config', (utils.config('config_normal')), 'run',
      'replace', '--quiet', session_expectations={'get':2, 
     'post':3, 
     'patch':1, 
     'delete':1})
    lines = invocation.result.output.split('\n')
...
def test_update_verbose_with_errors(invoker, utils):
    invocation = invoker('-c', (utils.config('config_errors')), 'run',
      'update', '-v', session_expectations={'get':3, 
     'post':5, 
     'patch':1, 
     'delete':0})
    lines = invocation.result.output.split('\n')
    ...
    if not @py_assert2:
        ...
        raise AssertionError(@pytest_ar._format_explanation(@py_format6))
    @py_assert0 = @py_assert2 = None
    @py_assert0 = '[ADD][ERR] MarekSuchanek/repo1; label7; 00FFCCAA; 422 - Validation Failed'
    @py_assert2 = @py_assert0 in lines
    if not @py_assert2:
        @py_format4 = @pytest_ar._call_reprcompare(('in', ), (@py_assert2,), ('%(py1)s in %(py3)s', ), (@py_assert0, lines)) % {'py1':@pytest_ar._saferepr(@py_assert0),  'py3':@pytest_ar._saferepr(lines) if 'lines' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(lines) else 'lines'}
        @py_format6 = 'assert %(py5)s' % {'py5': @py_format4}
        raise AssertionError(@pytest_ar._format_explanation(@py_format6))
    @py_assert0 = @py_assert2 = None
    @py_assert0 = '[UPD][ERR] MarekSuchanek/repo1; label3; 00FFXX; 422 - Validation Failed'
    @py_assert2 = @py_assert0 in lines
    if not @py_assert2:
        @py_format4 = @pytest_ar._call_reprcompare(('in', ), (@py_assert2,), ('%(py1)s in %(py3)s', ), (@py_assert0, lines)) % {'py1':@pytest_ar._saferepr(@py_assert0),  'py3':@pytest_ar._saferepr(lines) if 'lines' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(lines) else 'lines'}
        @py_format6 = 'assert %(py5)s' % {'py5': @py_format4}
    ...
    if not @py_assert2:
        ...
        raise AssertionError(@pytest_ar._format_explanation(@py_format6))
    @py_assert0 = @py_assert2 = None
    @py_assert0 = '[ADD][ERR] MarekSuchanek/repo2; label3; 00FFXX; 422 - Validation Failed'
    @py_assert2 = @py_assert0 in lines
    if not @py_assert2:
        @py_format4 = @pytest_ar._call_reprcompare(('in', ), (@py_assert2,), ('%(py1)s in %(py3)s', ), (@py_assert0, lines)) % {'py1':@pytest_ar._saferepr(@py_assert0),  'py3':@pytest_ar._saferepr(lines) if 'lines' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(lines) else 'lines'}
        @py_format6 = 'assert %(py5)s' % {'py5': @py_format4}
        raise AssertionError(@pytest_ar._format_explanation(@py_format6))
    @py_assert0 = @py_assert2 = None
    @py_assert0 = '[ADD][ERR] MarekSuchanek/repo2; label7; 00FFCCAA; 422 - Validation Failed'
    @py_assert2 = @py_assert0 in lines
    if not @py_assert2:
        @py_format4 = @pytest_ar._call_reprcompare(('in', ), (@py_assert2,), ('%(py1)s in %(py3)s', ), (@py_assert0, lines)) % {'py1':@pytest_ar._saferepr(@py_assert0),  'py3':@pytest_ar._saferepr(lines) if 'lines' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(lines) else 'lines'}
        @py_format6 = 'assert %(py5)s' % {'py5': @py_format4}
        raise AssertionError(@pytest_ar._format_explanation(@py_format6))
    @py_assert0 = @py_assert2 = None
    @py_assert0 = '[LBL][ERR] MarekSuchanek/repo7; 404 - Not Found'
    @py_assert2 = @py_assert0 in lines
    if not @py_assert2:
        @py_format4 = @pytest_ar._call_reprcompare(('in', ), (@py_assert2,), ('%(py1)s in %(py3)s', ), (@py_assert0, lines)) % {'py1':@pytest_ar._saferepr(@py_assert0),  'py3':@pytest_ar._saferepr(lines) if 'lines' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(lines) else 'lines'}
        @py_format6 = 'assert %(py5)s' % {'py5': @py_format4}
...
def test_replace_verbose_with_errors(invoker, utils):
    invocation = invoker('--config', (utils.config('config_errors')), 'run',
      'replace', '--verbose', session_expectations={'get':3, 
     'post':5, 
     'patch':1, 
     'delete':1})
    lines = invocation.result.output.split('\n')
...
def test_update_quiet_with_errors(invoker, utils):
    invocation = invoker('-c', (utils.config('config_errors')), 'run',
      'update', '-q', session_expectations={'get':3, 
     'post':5, 
     'patch':1, 
     'delete':0})
    lines = invocation.result.output.split('\n')
...
def test_replace_quiet_with_errors(invoker, utils):
    invocation = invoker('--config', (utils.config('config_errors')), 'run',
      'replace', '--quiet', session_expectations={'get':3, 
     'post':5, 
     'patch':1, 
     'delete':1})
    lines = invocation.result.output.split('\n')
...
def test_update_dry_verbose_with_errors(invoker, utils):
    invocation = invoker('-c', (utils.config('config_errors')), 'run',
      'update', '-v', '-d', session_expectations={'get':3, 
     'post':0, 
     'patch':0, 
     'delete':0})
    lines = invocation.result.output.split('\n')
    ...
    if not @py_assert0:
        ...
        raise AssertionError(@pytest_ar._format_explanation(@py_format22))
    @py_assert0 = @py_assert1 = @py_assert4 = @py_assert6 = @py_assert7 = @py_assert12 = @py_assert14 = @py_assert15 = None
    @py_assert0 = '[ADD][DRY] MarekSuchanek/repo1; label2; CCAAFF'
    @py_assert2 = @py_assert0 in lines
    if not @py_assert2:
        @py_format4 = @pytest_ar._call_reprcompare(('in', ), (@py_assert2,), ('%(py1)s in %(py3)s', ), (@py_assert0, lines)) % {'py1':@pytest_ar._saferepr(@py_assert0),  'py3':@pytest_ar._saferepr(lines) if 'lines' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(lines) else 'lines'}
        @py_format6 = 'assert %(py5)s' % {'py5': @py_format4}
        raise AssertionError(@pytest_ar._format_explanation(@py_format6))
    @py_assert0 = @py_assert2 = None
    @py_assert0 = '[ADD][DRY] MarekSuchanek/repo1; label7; 00FFCCAA'
    @py_assert2 = @py_assert0 in lines
    if not @py_assert2:
        @py_format4 = @pytest_ar._call_reprcompare(('in', ), (@py_assert2,), ('%(py1)s in %(py3)s', ), (@py_assert0, lines)) % {'py1':@pytest_ar._saferepr(@py_assert0),  'py3':@pytest_ar._saferepr(lines) if 'lines' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(lines) else 'lines'}
        @py_format6 = 'assert %(py5)s' % {'py5': @py_format4}
        raise AssertionError(@pytest_ar._format_explanation(@py_format6))
    @py_assert0 = @py_assert2 = None
    @py_assert0 = '[UPD][DRY] MarekSuchanek/repo1; label3; 00FFXX'
    @py_assert2 = @py_assert0 in lines
    if not @py_assert2:
        @py_format4 = @pytest_ar._call_reprcompare(('in', ), (@py_assert2,), ('%(py1)s in %(py3)s', ), (@py_assert0, lines)) % {'py1':@pytest_ar._saferepr(@py_assert0),  'py3':@pytest_ar._saferepr(lines) if 'lines' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(lines) else 'lines'}
        @py_format6 = 'assert %(py5)s' % {'py5': @py_format4}
        raise AssertionError(@pytest_ar._format_explanation(@py_format6))
    @py_assert0 = @py_assert2 = None
    @py_assert0 = '[ADD][DRY] MarekSuchanek/repo2; label1; FFAA00'
    @py_assert2 = @py_assert0 in lines
    if not @py_assert2:
        @py_format4 = @pytest_ar._call_reprcompare(('in', ), (@py_assert2,), ('%(py1)s in %(py3)s', ), (@py_assert0, lines)) % {'py1':@pytest_ar._saferepr(@py_assert0),  'py3':@pytest_ar._saferepr(lines) if 'lines' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(lines) else 'lines'}
        @py_format6 = 'assert %(py5)s' % {'py5': @py_format4}
        raise AssertionError(@pytest_ar._format_explanation(@py_format6))
    @py_assert0 = @py_assert2 = None
    @py_assert0 = '[ADD][DRY] MarekSuchanek/repo2; label3; 00FFXX'
    @py_assert2 = @py_assert0 in lines
    if not @py_assert2:
        @py_format4 = @pytest_ar._call_reprcompare(('in', ), (@py_assert2,), ('%(py1)s in %(py3)s', ), (@py_assert0, lines)) % {'py1':@pytest_ar._saferepr(@py_assert0),  'py3':@pytest_ar._saferepr(lines) if 'lines' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(lines) else 'lines'}
        @py_format6 = 'assert %(py5)s' % {'py5': @py_format4}
        raise AssertionError(@pytest_ar._format_explanation(@py_format6))
    @py_assert0 = @py_assert2 = None
    @py_assert0 = '[ADD][DRY] MarekSuchanek/repo2; label7; 00FFCCAA'
    @py_assert2 = @py_assert0 in lines
    if not @py_assert2:
        @py_format4 = @pytest_ar._call_reprcompare(('in', ), (@py_assert2,), ('%(py1)s in %(py3)s', ), (@py_assert0, lines)) % {'py1':@pytest_ar._saferepr(@py_assert0),  'py3':@pytest_ar._saferepr(lines) if 'lines' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(lines) else 'lines'}
        @py_format6 = 'assert %(py5)s' % {'py5': @py_format4}
...
def test_replace_dry_verbose_with_errors(invoker, utils):
    invocation = invoker('--config', (utils.config('config_errors')), 'run',
      'replace', '--verbose', '--dry-run', session_expectations={'get':3, 
     'post':0, 
     'patch':0, 
     'delete':0})
    lines = invocation.result.output.split('\n')
    ...
    if not @py_assert2:
        ...
        raise AssertionError(@pytest_ar._format_explanation(@py_format6))
    @py_assert0 = @py_assert2 = None
    @py_assert0 = '[DEL][DRY] MarekSuchanek/repo1; label4; 771077'
    @py_assert2 = @py_assert0 in lines
    if not @py_assert2:
        @py_format4 = @pytest_ar._call_reprcompare(('in', ), (@py_assert2,), ('%(py1)s in %(py3)s', ), (@py_assert0, lines)) % {'py1':@pytest_ar._saferepr(@py_assert0),  'py3':@pytest_ar._saferepr(lines) if 'lines' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(lines) else 'lines'}
        @py_format6 = 'assert %(py5)s' % {'py5': @py_format4}
