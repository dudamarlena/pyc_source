        ...
        if self.qs:
            ...
            if self.lookups:
                for field_name, value in self.lookups.items():
                    ...
                    try:
                        results.append(LOGIC_FUNCTIONS[logic_function](obj[field_name], value))
                    except TypeError:
    ...
    def __repr__(self, pretty=True, recursion_stack=None):
        ...
        for k, v in self.data.items():
            if k in QUOTE_KEYS:
                ...
                if isinstance(v, (Content, ContentSet)):
...
class ContentSet:
    ...
    def order_by(self, field_name):
        ...
        if field_name.startswith('-'):
            ...
            reverse = True
        return self.__class__(contents=sorted(self.contents, key=lambda x: (
         x[field_name] is None, x[field_name]), reverse=reverse))
