...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/ipauth/views.py
# Compiled at: 2012-12-03 17:19:57
...
def login(request, redirect_field_name=REDIRECT_FIELD_NAME, **kwargs):
    ...
    if user is None:
        return base_login_view(request, redirect_field_name=redirect_field_name, **kwargs)
    else:
        auth_login(request, user)
        messages.add_message(request, messages.INFO, 'You are now logged in as %s' % (user.get_full_name(),))
