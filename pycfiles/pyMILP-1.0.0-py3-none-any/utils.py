...
# Embedded file name: build/bdist.linux-x86_64/egg/pymills/utils.py
# Compiled at: 2014-09-16 08:15:24
...
class State(object):
    ...
    def __lt__(self, s):
        return s in self._states and self._state == s and self._states[s] < self._states[self._state]
...
def validateEmail(email):
    ...
    otehrwise.
    """
    return len(email) > 7 and re.match('^.+\\@(\\[?)[a-zA-Z0-9\\-\\.]+\\.([a-zA-Z]{2,3}|[0-9]{1,3})(\\]?)$', email) is not None
...
def safe__import__(moduleName, globals=globals(), locals=locals(), fromlist=[]):
    ...
    try:
        ...
        for name in sys.modules.copy():
            if name not in alreadyImported:
...
class MemoryStats(object):
    ...
    def __call__(self):
        return (
         self['VmSize'], self['VmRSS'], self['VmStk'])
