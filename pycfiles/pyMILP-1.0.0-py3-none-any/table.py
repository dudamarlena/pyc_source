...
# Embedded file name: build/bdist.linux-x86_64/egg/pymills/table.py
# Compiled at: 2013-11-20 07:00:27
__doc__ = 'Table generation, display and manipulation module.\n\nClasses to generate ASCII and HTML tables and traverse\ndata.\n'
    ...
    * cstyle - cell style attribute to use for each cell used by toHTML
    ...
    def getWidth(self):
        if self.hidden:
            ...
            if self.table:
                ...
                return max(max([ len(str(x[hIndex])) for x in rows ]) + 2, len(self.name) + 2)
    ...
    def toHTML(self):
        if self.hidden:
            ...
            align, cls, style = (None, None, None)
            if self.align:
