...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/alexandria/dsl/validators.py
# Compiled at: 2010-04-20 16:31:56
...
def integer(input, options):
    ...
    if not input.isdigit():
        raise InvalidInputException, 'contains not only digits'
    return int(input)
...
def pick_one(input, options):
    ...
    if not isinstance(input, str):
        raise InvalidInputException, 'not a string'
    if input.isdigit():
