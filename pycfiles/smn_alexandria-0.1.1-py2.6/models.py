...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/alexandria/sessions/db/models.py
...
class ClientTimeLimitManager(models.Manager):
    ...
    def recent(self, uuid, client_type):
        ...
        """
        try:
            client = super(ClientTimeLimitManager, self).get_query_set().get(uuid=uuid, client_type=client_type, updated_at__gte=datetime.now() - self.TIME_LIMIT)
...
class Item(models.Model):
    ...
    def __unicode__(self):
        ...
        >>>
        """
        return '%s (%s)' % (self.key, self.value_type)