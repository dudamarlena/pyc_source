...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/alexandria/tests.py
# Compiled at: 2011-04-12 08:16:41
from unittest import TestCase
from alexandria.dsl.core import MenuSystem, prompt, end, question, pick_first_unanswered, case
...
class MenuSystemTestCase(TestCase):
    ...
    def setUp(self):
        self.menu = MenuSystem(prompt('Prompt 1'), prompt('Prompt 2'), prompt('Prompt 3'))
    ...
    def test_appending(self):
        ...
        """
        menu = self.menu.append(prompt('Prompt 4'), prompt('Prompt 5'))
    ...
    def test_next_after(self):
        ...
        _test_coroutine(prompt)
        _test_coroutine(menu.stack[1])
...
class ClientTestCase(TestCase):
    ...
    def test_stepping_through_system(self):
        ...
        """
        client = TestingClient('test_client')
        menu = MenuSystem(prompt('What is your name?'), prompt('What is your favorite color?', validator=pick_one, options=('red',
                                                                                                                            'white',
                                                                                                                            'blue')), prompt('How old are you?', validator=integer), end('Thanks & goodbye!'))
    ...
    def test_coroutines_returning_nothing(self):
        ...
        """
        client = TestingClient('test_client')
        menu = MenuSystem(prompt('What is your age?', validator=integer), case((
         lambda ms, session: False, prompt('This should never be displayed'))), end('Goodbye!'))
        client.answer('*120*USSD_SHORTCODE#', menu)
        client.answer('29', menu)
        self.assertEquals(client.outbox, [
         (
    ...
    def test_validation(self):
        ...
        """
        client = TestingClient('test_client')
        menu = MenuSystem(prompt('What is your favorite color?', validator=pick_one, options=('red',
                                                                                              'white',
                                                                                              'blue')), prompt('How old are you?', validator=integer), end('Thanks & goodbye!'))
        client.answer('*120*USSD_SHORTCODE#', menu)
...
class PromptTestCase(TestCase):
    ...
    def test_numeric_response(self):
        ...
        menu items
        """
        p = prompt(self.question_text, validator=pick_one, options=self.question_options)
        p.next()
        (question, end_of_session) = p.send((MenuSystem(), self.session_store))
        self.assertEquals(question, msg(self.question_text, self.question_options))
        self.assertFalse(end_of_session)
        p.next()
        validated_answer = p.send('1')
        self.assertEquals(validated_answer, 'red')
        self.assertEquals(self.session_store['What is your favorite color?'], 'red')
    ...
    def test_character_response(self):
        ...
        self.assertEquals(self.session_store['What is your favorite color?'], 'blue')
...
class QuestionTestCase(TestCase):
    ...
    def test_generated_stack(self):
        ...
        """
        question_stack = question('Is your right thumb on your left hand?', {'no': 'Please see a doctor and press 1 to continue.', 
           'yes': 'Correct! Please press 1 to continue'})
        (prompt, case) = question_stack
        prompt.next()
        (question_text, end_of_session) = prompt.send((MenuSystem(), {}))
        self.assertEquals(question_text, msg('Is your right thumb on your left hand?', options=('yes',
                                                                                                'no')))
        case.next()
        (response_text, end_of_session) = case.send((MenuSystem(),
         {'Is your right thumb on your left hand?': 'no'}))
        self.assertEquals(response_text, 'Please see a doctor and press 1 to continue.')
        self.assertFalse(end_of_session)
...
class CombinedCoroutineTestCase(TestCase):
    ...
    def test_pick_first_unanswered(self):
        ...
        """
        pfu = pick_first_unanswered(prompt('What is your name?'), prompt('What is your age?'), prompt('What is your gender?'))
        session_store = {'What is your name?': 'Simon', 
...
class SessionManagerTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.original_data = self.session.data = {'string': 'bar', 
           'int': '1', 
           'list': [
                  1, 2, 3, 4, 5], 
           'dict': {'di': 'ct'}}
