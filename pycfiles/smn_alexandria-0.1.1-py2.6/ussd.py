...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/alexandria/transport/ussd.py
# Compiled at: 2010-05-07 16:19:20
...
import logging
from ssmi.client import SSMI_USSD_TYPE_NEW, SSMI_USSD_TYPE_EXISTING, SSMI_USSD_TYPE_END, SSMI_USSD_TYPE_TIMEOUT
...
class SSMIService(object):
    ...
    def register_ssmi(self, ssmi_protocol):
        ...
        self.ssmi_client.app_setup(username=self.username, password=self.password, ussd_callback=self.process_ussd, sms_callback=self.process_sms)
    ...
    def process_ussd(self, msisdn, ussd_type, ussd_phase, message):
        if self.ssmi_client is None:
            ...
            routes = {SSMI_USSD_TYPE_NEW: self.new_ussd_session, SSMI_USSD_TYPE_EXISTING: self.existing_ussd_session, 
