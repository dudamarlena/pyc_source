...
class MockGitlabServer(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.headers['PRIVATE-TOKEN'] == 'wrong token':
            ...
            if re.search(self.PIPELINE, self.path) and re.search(self.PER_PAGE, self.path):
                self.send_all_pipeline()
            else:
                if re.search(self.PROJECTS, self.path) and re.search(self.PER_PAGE, self.path):
                    self.send_all_projects()
                else:
                    if re.search(self.PIPELINE, self.path) and re.search(self.THE_PIPELINE, self.path):
                        self.send_pipeline()
                    else:
