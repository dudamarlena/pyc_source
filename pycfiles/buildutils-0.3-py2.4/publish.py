...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/buildutils/publish_command/publish.py
# Compiled at: 2007-08-08 19:58:57
"""
Publish distributables using ``sftp``/``scp``.
"""
import sys, os, fnmatch, md5
...
def filter_files(file_list, file_sigs):
    ...
    return (
     result, file_sigs.keys(), new_sigs)
...
def scp_handler(url, src_dir, spawn, make_dirs=0, verbose=0, match=None, force=False):
    ...
    if not all_files:
        raise DistutilsFileError('No files found in %s (matching pattern(s): %s)' % (src_dir, match and (',').join(match) or '*'))
    if not files:
    ...
    if make_dirs:
        for dir in dirlist:
            args = [
             'ssh', host, 'mkdir -p %r' % (path + dir,)]
            spawn(args)

    for (dest_dir, file_list) in files_by_dir.items():
        args = [
         'scp', '-pC'] + file_list + ['%s:%s%s' % (host, path, dest_dir)]
        spawn(args)
...
def sftp_handler(url, src_dir, spawn, make_dirs=0, verbose=0, match=None, force=False):
    import os
...
publish_handlers = {'scp': scp_handler, 'sftp': sftp_handler}

class publish(Command):
    __module__ = __name__
    description = 'push distributables to a remote site.'
    user_options = [
     ('dist-dir=', 'd', 'directory containing distributables [default: dist].'), ('dist-dest=', 't', 'destination directory URL for distributables.'), ('doc-dir=', None, 'directory containing documentation to publish.'), ('doc-dest=', None, 'destination directory URL for documentation.'), ('make-dirs', 'm', 'try to create remote directories'), ('protocol-list', None, 'show the list of available URL types available for the dest arg.'), ('match=', None, 'Publish only files that match the given wildcard pattern'), ('force', 'f', 'Force files to be published, even if they should be up-to-date on server')]
    boolean_options = [
     'protocol-list', 'make-dirs']
    ...
    def finalize_options(self):
        ...
        if not self.dist_dest and not self.doc_dest:
            raise DistutilsOptionError, 'Must specify a one of `dist-dest` or `doc-dest` options (where to publish files).'
        self.expand_options()
        if self.dist_dest:
    ...
    def list_protocols(self):
        if self.protocol_list:
            log.info('supported destination URL schemes: %s', (', ').join(publish_handlers.keys()))
            return 1

    def run(self):
        if self.list_protocols():
            return
        for (src, dest) in [(self.dist_dir, self.dist_dest), (self.doc_dir, self.doc_dest)]:
            if not dest:
                continue
            handler = handler_for_url(dest)
            if not handler:
                raise DistutilsOptionError, 'No handler available for %r URLs' % self.scheme
            self.execute(handler, (dest, src, self.spawn, self.make_dirs, 0, self.match, self.force))