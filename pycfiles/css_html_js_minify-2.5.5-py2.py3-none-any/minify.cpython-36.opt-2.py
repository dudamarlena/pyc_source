...
# [GCC 8.4.0]
# Embedded file name: /css_html_js_minify/minify.py
# Compiled at: 2018-04-14 10:27:57
# Size of source mod 2**32: 13534 bytes
"""CSS-HTML-JS-Minify.

Minifier for the Web.
"""
import atexit, os, sys, gzip, logging as log
...
__all__ = ('process_multiple_files', 'prefixer_extensioner', 'process_single_css_file',
           'process_single_html_file', 'process_single_js_file', 'make_arguments_parser',
           'main')

def process_multiple_files(file_path, watch=False, wrap=False, timestamp=False, comments=False, sort=False, overwrite=False, zipy=False, prefix='', add_hash=False):
            ...
            if previous == actual:
                ...
                if file_path.endswith('.css'):
                    process_single_css_file(file_path,
                      wrap=wrap, timestamp=timestamp, comments=comments,
                      sort=sort,
                      overwrite=overwrite,
                      zipy=zipy,
                      prefix=prefix,
                    ...
                    if file_path.endswith('.js'):
                        process_single_js_file(file_path,
                          timestamp=timestamp, overwrite=overwrite,
                          zipy=zipy)
                    else:
                        process_single_html_file(file_path,
                          comments=comments, overwrite=overwrite,
                          prefix=prefix,
                          add_hash=add_hash)
...
def process_single_css_file(css_file_path, wrap=False, timestamp=False, comments=False, sort=False, overwrite=False, zipy=False, prefix='', add_hash=False, output_path=None):
    ...
    minified_css = css_minify(original_css, wrap=wrap, comments=comments,
      sort=sort)
    if timestamp:
        ...
        if output_path is None:
            min_css_file_path = prefixer_extensioner(css_file_path,
              '.css', ('.css' if overwrite else '.min.css'), original_css,
              prefix=prefix, add_hash=add_hash)
            if zipy:
                gz_file_path = prefixer_extensioner(css_file_path,
                  '.css', ('.css.gz' if overwrite else '.min.css.gz'),
                  original_css, prefix=prefix,
                  add_hash=add_hash)
...
def process_single_html_file(html_file_path, comments=False, overwrite=False, prefix='', add_hash=False, output_path=None):
    ...
    if output_path is None:
        html_file_path = prefixer_extensioner(html_file_path,
          ('.html' if overwrite else '.htm'), '.html', prefix=prefix,
          add_hash=add_hash)
    else:
...
def process_single_js_file(js_file_path, timestamp=False, overwrite=False, zipy=False, output_path=None):
    ...
    if timestamp:
        ...
        if output_path is None:
            min_js_file_path = prefixer_extensioner(js_file_path, '.js', '.js' if overwrite else '.min.js', original_js)
            if zipy:
                gz_file_path = prefixer_extensioner(js_file_path, '.js', '.js.gz' if overwrite else '.min.js.gz', original_js)
...
def make_arguments_parser():
    ...
    parser.add_argument('--wrap', action='store_true', help='Wrap output to ~80 chars per line, CSS only.')
    parser.add_argument('--prefix', type=str, help='Prefix string to prepend on output filenames.')
    parser.add_argument('--timestamp', action='store_true', help='Add a Time Stamp on all CSS/JS output files.')
    parser.add_argument('--quiet', action='store_true', help='Quiet, Silent.')
    parser.add_argument('--hash', action='store_true', help='Add SHA1 HEX-Digest 11chars Hash to Filenames.')
    parser.add_argument('--zipy', action='store_true', help="GZIP Minified files as '*.gz', CSS/JS only.")
    parser.add_argument('--sort', action='store_true', help='Alphabetically Sort CSS Properties, CSS only.')
    parser.add_argument('--comments', action='store_true', help='Keep comments, CSS/HTML only (Not Recommended)')
    parser.add_argument('--overwrite', action='store_true', help='Force overwrite all in-place (Not Recommended)')
    ...
    parser.add_argument('--multiple', action='store_true', help='Allow Multiple instances (Not Recommended).')
    return parser.parse_args()


def walk2list(folder: str, target: tuple, omit: tuple=(), showhidden: bool=False, topdown: bool=True, onerror: object=None, followlinks: bool=False) -> tuple:
    """Perform full walk, gather full path of all files."""
    oswalk = os.walk(folder, topdown=topdown, onerror=onerror,
      followlinks=followlinks)
    return [os.path.abspath(os.path.join(r, f)) for r, d, fs in oswalk if not f.startswith(() if showhidden else '.') if not f.endswith(omit) if f.endswith(target) for f in fs if not f.endswith(omit) if f.endswith(target)]
...
def main():
    ...
    if os.path.isfile(args.fullpath):
        if args.fullpath.endswith('.css'):
            print('Target is a CSS File.')
            ...
            process_single_css_file((args.fullpath),
              ...
              prefix=(args.prefix),
              add_hash=(args.hash))
    if os.path.isfile(args.fullpath) and args.fullpath.endswith('.html' if args.overwrite else '.htm'):
        print('Target is HTML File.')
        list_of_files = str(args.fullpath)
        process_single_html_file((args.fullpath),
          comments=(args.comments), overwrite=(args.overwrite),
          prefix=(args.prefix),
          add_hash=(args.hash))
        ...
        process_single_js_file((args.fullpath),
          timestamp=(args.timestamp), overwrite=(args.overwrite),
          zipy=(args.zipy))
    else:
        if os.path.isdir(args.fullpath):
            ...
            pool = Pool(cpu_count())
            pool.map_async(partial(process_multiple_files,
              watch=(args.watch), wrap=(args.wrap),
              ...
              zipy=(args.zipy),
              prefix=(args.prefix),
              add_hash=(args.hash)), list_of_files)
            pool.close()
            pool.join()
        else:
