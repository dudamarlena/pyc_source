...
# [GCC 8.4.0]
# Embedded file name: /opt/hostedtoolcache/Python/3.7.5/x64/lib/python3.7/site-packages/torchsnooper/__init__.py
# Compiled at: 2019-12-31 00:12:40
# Size of source mod 2**32: 7208 bytes
import torch, pysnooper, pysnooper.utils, warnings, numpy
...
class TensorFormat:
    ...
    def repr_shape(self, tensor):
        ret = ''
        if hasattr(tensor, 'names'):
            if not tensor.has_names():
...
class TorchSnooper(pysnooper.tracer.Tracer):
    ...
    def return_types_repr(self, x):
        if type(x).__name__ in {'kthvalue', 'mode', 'sort', 'median', 'max', 'min', 'topk'}:
...
def register_snoop(verbose=False, tensor_format=default_format, numpy_format=default_numpy_format):
    import snoop, cheap_repr, snoop.configuration
    ...
    if verbose:
        ...
        snoop.config.watch_extras += (
         lambda source, value: (
          '{}.data'.format(source), TensorWrap(value.data)),)