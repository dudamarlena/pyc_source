...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv5tejl/egg/ev3/event_loop.py
...
class EventLoop(object):
    ...
    def register_value_change(self, getter, startvalue, target, count=-1):
        ...
        """
        self._events.append(ValueChangeEvent(getter, startvalue, target, count))
        return len(self._events) - 1
...
class Event(object):
    ...
    def poll(self):
        ...
        return self._evaluation and (self._repeat or not self._previous_evaluation)
    ...
    evaluation = property(lambda self: self._evaluation, doc='The value from the most resent evaluation of the condition.')
    previous_evaluation = property(lambda self: self._previous_evaluation, doc='The value from the previous evaluation of the condition.')
    repeat = property(lambda self: self._repeat, lambda self, value: setattr(self, '_repeat', bool(value)), doc="Whether to call target every time `condition` is true, even if it haven't changed since last poll.")
    count = property(lambda self: self._count, lambda self, value: setattr(self, '_count', int(value)), doc='Remaining number of times this event can be fired before it is automatically unregistered.')
