...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/ore/xd/_xd.py
# Compiled at: 2008-12-11 14:13:41
"""
...
class ImportReader(ContentHandler):
    _types = {'string': str, 
       ...
       'datetime': parseDate}
    subnode_types = (
     types.ListType, types.DictType, types.TupleType, struct)
    ...
    def endElement(self, element_name):
        ...
        if self.buf and self._types[self.stack[(-1)]] not in self.subnode_types:
            data = ('').join(self.buf)
            data = data.strip()
...
class ExportWriter(object):
    _values = {types.StringType: 'dumpEntry', 
       unicode: 'dumpEntry', 
       float: 'dumpEntry', 
    ...
    _attrs = {types.StringType: 'convertValue', 
    ...
    _typecode = {types.StringType: 'string', 
       unicode: 'unicode', 
       float: 'float', 
       types.IntType: 'int', 
    ...
    subnode_types = (
     types.ListType, types.DictType, types.TupleType)
    policy_id = 'policy'
    entry_key = 'entry'
    ...
    def dumpDictionary(self, key, value):
        ...
        attr_values = {(None, 'type'): type_code}
    ...
    def dumpList(self, key, value):
        type_code = self.getTypeCode(value)
        attrs = AttributesNSImpl({(None, 'type'): type_code}, {(None, 'type'): 'type'})
        self._logger.startElementNS((None, key), key, attrs)
        for entry in value:
...
if __name__ == '__main__':
    d = {'identification': {'filters': [{'attributes': {'id': 'no_members', 'expr': 'python: not 1'}}]}, 'bd': True, 
       'ef': 'ste', 
       'mo': None, 
       'ab': 'abc', 
