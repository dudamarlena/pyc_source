...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/trec_dd/scorer/run.py
...
import json, kvlayer, logging, os, sys, yakonfig
...
of (subtopic_id, rating)
    ...
    for line in fh:
        ...
        parts = line.split()
        if len(parts) != 6:
            sys.exit('Your run file is invalid, because line %d has %d parts instead of 5' % (
             line_idx, len(parts)))
        ...
        results_by_topic['results'][topic_id].append(dict(rank=rank, stream_id=stream_id, confidence=confidence, on_topic=on_topic, subtopics=subtopics))
        rank += 1
...
def main():
    ...
    parser.add_argument('scored_run_file_output_path', help='path to file to create with scores insertedinto run file.')
    parser.add_argument('--overwrite', action='store_true', default=False, help='overwrite any existing run file.')
    parser.add_argument('--verbose', action='store_true', default=False, help='display verbose log messages.')
    parser.add_argument('--scorer', action='append', default=[], dest='scorers', help='names of scorer functions to run; if none are provided, it runs all of them')
    modules = [
     yakonfig, kvlayer]
    ...
    open(args.scored_run_file_output_path, 'wb').write(json.dumps(run, indent=4))
