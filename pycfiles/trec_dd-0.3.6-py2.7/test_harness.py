...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/trec_dd/harness/tests/test_harness.py
# Compiled at: 2015-07-22 16:10:03
from __future__ import absolute_import
from ..run import Harness
from dossier.label import LabelStore, Label, CorefValue
import csv, kvlayer, os, pytest

@pytest.fixture
def local_kvl():
    kvl = kvlayer.client(config={}, storage_type='local', namespace='test', app_name='test')
    build_test_data(kvl)
    return kvl
...
def build_test_data(kvl):
    ...
    for t_idx, topic in enumerate(topics):
        for s_idx, subtopic in enumerate(subtopics):
            meta = dict(topic_name=topic, topic_id=str(t_idx), passage_text='howdy', subtopic_name='bye bye')
            label = Label(str(t_idx), 'doc' + str(t_idx) + str(s_idx), 'me', CorefValue.Positive, subtopic_id1=subtopic, subtopic_id2=offset, rating=ratings[t_idx][s_idx], meta=meta)
