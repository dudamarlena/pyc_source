...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/trec_dd/harness/truth_data.py
...
def label_from_truth_data_file_line(line_data):
    ...
    if not doc_id.strip():
        logger.warn('dropping invalid truth data line: bad docno: %r: %r' % (
         doc_id, line_data))
        return None
    else:
        ...
        try:
            ...
            logger.warn('replacing bogus grade with zero = %r', line_data['grade'])
            rating = 0

        if rating < 0:
            value = CorefValue.Negative
            rating = 0
        meta = {'domain_name': line_data['domain_name'], 'domain_id': line_data['domain_id'], 
        ...
        label = Label(topic_id, doc_id, annotator, value, subtopic_id1=subtopic_id, subtopic_id2=passage_id, rating=rating, meta=meta)
        return label
...
def main():
    parser = argparse.ArgumentParser('test tool for checking that we can load the truth data as distributed by NIST for TREC 2015')
    ...
    logger.debug('Done!  The truth data was loaded into this kvlayer backend: %r', json.dumps(yakonfig.get_global_config('kvlayer'), indent=4, sort_keys=True))
