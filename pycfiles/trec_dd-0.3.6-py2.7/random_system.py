...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/trec_dd/system/random_system.py
...
import logging, os, random, sys, yaml
...
class RandomSystem(object):
    ...
    def search(self, query, page_number):
        ...
        if page_number >= 5:
            ...
            doc_ids = list(self.doc_store.scan_ids(query, ignore=self.submitted_docs))
            if len(doc_ids) < 5:
