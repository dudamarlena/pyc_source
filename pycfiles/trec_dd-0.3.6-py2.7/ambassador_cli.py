...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/trec_dd/system/ambassador_cli.py
...
import json, logging, time, subprocess, sys
...
class HarnessAmbassadorCLI(object):
    ...
    def stop(self, out=None):
        ...
        logger.info('%.1f seconds spent so far, %.1f in search, %.1f in generating feedback, %.1f in processing feedback', total_elapsed, self.search_elapsed, self.feedback_elapsed, self.process_elapsed)
        return
    ...
    def step(self):
        ...
        """
        self.num_steps += 1
        logger.info('Doing step %d for topic %s: %r', self.num_steps, self.query, self.topic_id)
        start_time = time.time()
        results = self.system.search(self.query, self.num_steps)
        self.search_elapsed += time.time() - start_time
        logger.info('got %d results for %r page %d', len(results), self.query, self.num_steps)
        if not results:
            return
    ...
    def run(self):
        ...
        while 1:
            ...
            while 1:
                feedback = self.step()
                if feedback is None or len(feedback) < self.batch_size:
                    break
