...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/libparsing/__init__.py
# Compiled at: 2017-03-27 15:15:59
...
class ParsingElement(CObject):
    ...
    def __repr__(self):
        ...
        if not self._cobject:
            ...
            return ('<{0} {1}@{2}#{3}>').format(classname, ensure_str(self.type), ensure_str(self.name), self.id)
...
class Condition(ParsingElement):
    ...
    def _new(self, callback):
        self._callback = (
         self.WrapCallback(callback), callback)
...
class Reference(CObject):
    ...
    def __repr__(self):
        return ('<{0} {1}@{2}→{3}>').format(self.__class__.__name__.rsplit('.', 1)[(-1)], self.id, ensure_str(self.name), self.element)
...
class Match(CObject):
    ...
    def name(self):
        ...
        if name:
            return ensure_str(ffi.string(name))
        else:
            return
    ...
    def __repr__(self):
        if not self._cobject:
            return ('<{0} NULL {1}>').format(self.__class__.__name__.rsplit('.', 1)[(-1)], id(self))
        else:
            return ('<{0} {1}:{2}@{3} {4}-{5}>').format(self.__class__.__name__.rsplit('.', 1)[(-1)], ensure_str(self.type), self.id, ensure_str(self.name) or '_', self.offset, self.offset + self.length)
...
class MatchResult(object):
    ...
    def __repr__(self):
        return ('<{0} {1}={2}>').format(self.__class__.__name__.rsplit('.', 1)[(-1)], self.match, self.value)
...
class ParsingResult(CObject):
    ...
    @property
    def lastMatch(self):
        return MatchTuple(offset=self.lastMatchOffset, length=self.lastMatchLength, id=self.lastMatchElementID, element=self._grammar.symbol(self.lastMatchElementID) if self.lastMatchElementID != -1 else None)
    ...
    def isSuccess(self):
        if lib.ParsingResult_isSuccess(self._cobject) != 0:
            return True
        return False

    def isFailure(self):
        if lib.ParsingResult_isFailure(self._cobject) != 0:
            return True
        return False

    def isPartial(self):
        if lib.ParsingResult_isPartial(self._cobject) != 0:
            return True
        return False
    ...
    def getContext(self, start=None, end=None, before=3, after=3):
        ...
        at = t[e:].split('\n', after + 1)[:after + 1]
        c = bt[(-1)] + t[s:e] + at[0]
        return dict(before=bt[:-1], line=c, after=at[1:], start=start, end=end, lineBefore=bt[(-1)], lineMatch=t[s:e], lineAfter=at[0], lineOffset=len(bt[(-1)]), lineLength=max(0, e - s))
    ...
    def formatContext(self, context):
        red = '\x1b[0m\x1b[01;31m'
        yellow = '\x1b[0m\x1b[01;33m'
        orange = '\x1b[0m\x1b[00;33m'
        reset = '\x1b[0m\x1b[00;00m'
        before = '\n│ ' + ('\n│ ').join(context['before'])
        after = '\n│ ' + ('\n│ ').join(context['after'])
        line = '\n└┐' + yellow + context['line'] + reset
        error = '\n┌┘' + (orange + self._asSpaces(context['lineBefore'], '▸') + red + self._asSpaces(context['lineMatch'], '▲') + reset + self._asSpaces(context['lineAfter']))
    ...
    def describe(self, context=3, color=True):
        ...
        if self.isSuccess():
            ...
            return ('Parsing failed at line {0} character {1}, offset {2}→{3}{4}:{5}').format(line, char, s, e, sym, ('\n').join([ _ for _ in t.split('\n') ]))
    ...
    def __repr__(self):
        return ('<{0}(status={2}, line={3}, char={4}, offset={5}, remaining={6}) at {1:02x}>').format(self.__class__.__name__, id(self), self.status, self.line, -1, self.offset, 0)
...
class ParsingStats(CObject):
    ...
    def symbols(self):
        return [ (i, self._cobject.successBySymbol[i], self._cobject.failureBySymbol[i]) for i in range(self.symbolsCount()) ]
...
class Processor:
    ...
    def __init__(self, grammar=None, strict=True):
        ...
        self._defaults = dict((k, getattr(self, v) if hasattr(self, v) else None) for k, v in {TYPE_WORD: 'processWord', 
           TYPE_TOKEN: 'processToken', 
           TYPE_GROUP: 'processGroup', 
           TYPE_RULE: 'processRule', 
           TYPE_CONDITION: 'processCondition', 
           TYPE_PROCEDURE: 'processProcedure'}.items())
        return
