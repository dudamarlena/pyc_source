...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-ppc/egg/test/SceneTest.py
...
def mmm_node_to_str(node, level):
    ...
    if len(node.child_nodes) > 0:
        ret += ' ' * (level + SHIFT_WIDTH) + ' "' + node.entity.name + '" childs {\n'
...
def mmm_to_str(mmm, level=0):
    ...
    ret = ''
    if mmm.primitive is not dasty.core.MMM.EMPTY_PRIMITIVE:
