...
# [GCC 8.4.0]
# Embedded file name: /Users/naphatkrit/Dropbox/Documents/code/easyci/easyci/commands/test.py
# Compiled at: 2015-09-08 20:06:30
import click, subprocess32 as subprocess
from easyci import exit_codes, locking
from easyci.history import commit_signature, get_committed_signatures, get_staged_signatures, stage_signature, unstage_signature
...
from easyci.user_config import load_user_config, ConfigFormatError, ConfigNotFoundError, _default_config
...
def test(ctx, staged_only, head_only):
    ...
    with git.temp_copy() as (copy):
        ...
        with locking.lock(git, locking.Lock.tests_history):
            ...
            if in_committed:
                click.echo('')
                click.echo(click.style('OK', bg='green', fg='black') + ' Tests already ran.')
                try:
            ...
            if in_staged:
                click.echo('')
                click.echo(click.style('In Progress', bg='yellow', fg='black') + ' Tests already running.')
        ...
        with locking.lock(git, locking.Lock.tests_history):
            if len(config['collect_results']) > 0:
                ...
                save_results(git, new_signature, copy.path, config['collect_results'])
                click.echo('Done')
                click.echo('Syncing test results...', nl=False)
                sync_results(git, new_signature)
                click.echo('Done')
