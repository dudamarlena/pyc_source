...
# [GCC 8.4.0]
# Embedded file name: /home/ganow/.pyenv/versions/gq-test/lib/python2.7/site-packages/gq/core.py
# Compiled at: 2015-03-13 09:46:10
import re, json, os
...
class Selector(ValMissing):
    ...
    def where(self, expr):
        ...
        if self.filter:
            if self.next_is_or:
                return Selector(self.rootdir, params, curdir=self.curdir, filter=self.filter | expr, onGet=self.onGet)
            else:
                return Selector(self.rootdir, params, curdir=self.curdir, filter=self.filter & expr, onGet=self.onGet)

        else:
            return Selector(self.rootdir, params, curdir=self.curdir, filter=expr, onGet=self.onGet)
    ...
    def sortby(self, *keys):
        ...
        return Selector(self.rootdir, params, curdir=self.curdir, filter=self.filter, onGet=self.onGet)
