...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\jld\backup\delicious_db.py
...
import sqlite3 as sql, jld.api as api, jld.tools.mos as mos, jld.tools.db as db
...
class Posts(SQLObject):
    ...
    changed = DateTimeCol()
    _attributesToVerify = [
     'href', 'hash', 'description', 'time', 'tag']
    ...
    def getAll(cls, tag=None):
        ...
        list = []
        if tag:
            all = cls.select(OR(Posts.q.tag1 == tag, Posts.q.tag2 == tag, Posts.q.tag3 == tag), orderBy=DESC(Posts.q.changed))
        else:
    ...
    def updateFromList(cls, list):
        ...
        for entry in list:
            ...
            elif cls._updateOne(entry, post):
                updated = updated + 1
    ...
    def _createOne(cls, entry):
        ...
        Posts(hash=entry['hash'], href=entry['href'], description=entry['description'], tag=entry['tag'], tag1=tag1, tag2=tag2, tag3=tag3, time=entry['time'], changed=datetime.datetime.now())
    ...
    def _updateOne(cls, entry, post):
        ...
        if needsUpdate:
            (tag1, tag2, tag3) = cls._extractTags(entry['tag'])
            post.set(href=entry['href'], hash=entry['hash'], tag=entry['tag'], tag1=tag1, tag2=tag2, tag3=tag3, description=entry['description'], time=entry['time'], changed=datetime.datetime.now())
        return needsUpdate
