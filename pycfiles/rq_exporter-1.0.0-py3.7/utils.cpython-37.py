...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/rq_exporter/utils.py
# Compiled at: 2020-04-23 10:04:22
...
def get_workers_stats(connection=None):
    ...
    return [{'name':w.name,  'queues':[q.name for q in w.queues], 
     'state':w.get_state()} for w in workers]
...
def get_queue_jobs(queue_name, connection=None):
    ...
    return {JobStatus.QUEUED: queue.count, 
...
def get_jobs_by_queue(connection=None):
    ...
    return {q.name:get_queue_jobs(q.name, connection) for q in queues}