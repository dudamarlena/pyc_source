...
# Embedded file name: build/bdist.macosx-10.9-intel/egg/cili/cleanup.py
# Compiled at: 2014-06-10 20:14:39
from models import *
import pandas as pd
...
def mask_eyelink_blinks(samples, events, mask_fields=[
 'pup_l'], find_recovery=True):
...
def mask_zeros(samples, mask_fields=[
 'pup_l']):
...
def interp_zeros(samples, interp_fields=[
 'pup_l']):
...
def adjust_eyelink_recov_idxs(samples, events, z_thresh=0.1, window=1000, kernel_size=100):
    ...
    for idx, dur in events.duration.iteritems():
        ...
        if s_pos == e_pos:
            new_durs.append(dur)
            continue
        e_dpos = np.argmax(dfs_ravg[s_pos:e_pos] < z_thresh)
...
def butterworth_series(samples, fields=[
 'pup_l'], filt_order=5, cutoff_freq=0.01, inplace=False):
