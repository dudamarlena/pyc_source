...
# [GCC 8.4.0]
# Embedded file name: /Users/beazley/Projects/bitey/examples/mandel/png.py
# Compiled at: 2012-08-08 22:41:57
"""
...
__version__ = '$URL: http://pypng.googlecode.com/svn/trunk/code/png.py $ $Rev: 183 $'
...
import math, operator, struct, sys, zlib, warnings
__all__ = [
 'Reader', 'Writer', 'write_chunks']
    ...
    if palette is None:
        ...
        for i, t in enumerate(p):
            ...
            if seen_triple and len(t) == 4:
                raise ValueError('palette entry %d: all 4-tuples must precede all 3-tuples' % i)
            for x in t:
                if int(x) != x or not 0 <= x <= 255:
                    raise ValueError('palette entry %d: values must be integer: 0 <= x <= 255' % i)
...
class Writer():
    ...
    def __init__(self, width=None, height=None, size=None, greyscale=False, alpha=False, bitdepth=8, palette=None, transparent=None, background=None, gamma=None, compression=None, interlace=False, bytes_per_sample=None, planes=None, colormap=None, maxval=None, chunk_limit=2 ** 20):
        """
        ...
        *height*).
        ...
        if size:
            ...
            if width is not None and width != size[0]:
                raise ValueError('size[0] (%r) and width (%r) should match when both are used.' % (
                 size[0], width))
            if height is not None and height != size[1]:
                raise ValueError('size[1] (%r) and height (%r) should match when both are used.' % (
                 size[1], height))
            width, height = size
        del size
        ...
        if width > 4294967295 or height > 4294967295:
    ...
    def write(self, outfile, rows):
        ...
        if nrows != self.height:
            raise ValueError('rows supplied (%d) does not match height (%d)' % (
             nrows, self.height))
    ...
    def write_passes(self, outfile, rows, packed=False):
        ...
        write_chunk(outfile, 'IHDR', struct.pack('!2I5B', self.width, self.height, self.bitdepth, self.color_type, 0, 0, self.interlace))
        if self.gamma is not None:
            write_chunk(outfile, 'gAMA', struct.pack('!L', int(round(self.gamma * 100000.0))))
        if self.rescale:
            write_chunk(outfile, 'sBIT', struct.pack(('%dB' % self.planes), *([
...
class Reader():
    ...
    def chunk(self, seek=None):
        ...
        while 1:
            if True:
                self.atchunk = self.atchunk or self.chunklentype()
            ...
            if checksum != verify:
                ...
                raise ChunkError('Checksum error in %s chunk: 0x%08X != 0x%08X.' % (
                 type, a, b))
            return (
             type, data)
    ...
    def process_chunk(self):
        ...
        if type == 'IHDR':
            ...
            self.width, self.height, self.bitdepth, self.color_type, self.compression, self.filter, self.interlace = struct.unpack('!2I5B', data)
            ...
            if self.color_type & 1 and self.bitdepth > 8 or self.bitdepth < 8 and self.color_type not in (0,
                                                                                                          3):
                raise FormatError('Illegal combination of bit depth (%d) and colour type (%d). See http://www.w3.org/TR/2003/REC-PNG-20031110/#table111 .' % (
                 self.bitdepth, self.color_type))
            if self.compression != 0:
                raise Error('unknown compression method %d' % self.compression)
            if self.filter != 0:
                raise FormatError('Unknown filter method %d, see http://www.w3.org/TR/2003/REC-PNG-20031110/#9Filters .' % self.filter)
            if self.interlace not in (0, 1):
                raise FormatError('Unknown interlace method %d, see http://www.w3.org/TR/2003/REC-PNG-20031110/#8InterlaceMethods .' % self.interlace)
    ...
    def read(self):
        ...
        return (
         self.width, self.height, pixels, meta)
...
class Test(unittest.TestCase):
    ...
    def helperLN(self, n):
        ...
        self.assertEqual(list(itertools.chain(*pixels)), map(mask.__and__, range(1, 256)))
    ...
    def testRGBtoRGBA(self):
        ...
        self.assertEqual(row9[0:8], [
         255, 223, 255, 255, 255, 222, 255, 255])
    ...
    def testAdam7write(self):
        ...
        for name, bytes in _pngsuite.items():
            ...
            pngi = topngbytes('adam7wn' + name + '.png', pixels, x=x, y=y, bitdepth=it.bitdepth, greyscale=it.greyscale, alpha=it.alpha, transparent=it.transparent, interlace=False)
            x, y, ps, meta = Reader(bytes=pngi).read()
            it = Reader(bytes=bytes)
            x, y, pixels, meta = it.read()
            pngs = topngbytes('adam7wi' + name + '.png', pixels, x=x, y=y, bitdepth=it.bitdepth, greyscale=it.greyscale, alpha=it.alpha, transparent=it.transparent, interlace=True)
    ...
    def testPAMin(self):
        ...
        s.write('P7\nWIDTH 3\nHEIGHT 1\nDEPTH 4\nMAXVAL 255\nTUPLTYPE RGB_ALPHA\nENDHDR\n')
        flat = [
         255, 0, 0, 255, 0, 255, 0, 120, 0, 0, 255, 30]
    ...
    def testPackedIter(self):
        ...
        o = StringIO()
        w.write_packed(o, [itertools.chain([10], [170]),
         itertools.chain([15], [255])])
        r = Reader(bytes=o.getvalue())
...
def test_suite(options, args):
    ...
    writer = Writer(size, size, bitdepth=options.bitdepth, transparent=options.transparent, background=options.background, gamma=options.gamma, greyscale=options.greyscale, alpha=options.alpha, compression=options.compression, interlace=options.interlace)
...
def read_pam_header(infile):
    ...
    if 'WIDTH' not in header or 'HEIGHT' not in header or 'DEPTH' not in header or 'MAXVAL' not in header:
...
def _main(argv):
    ...
    import re
    version = '%prog ' + re.sub('( ?\\$|URL: |Rev:)', '', __version__)
    parser = OptionParser(version=version)
    parser.set_usage('%prog [options] [imagefile]')
    parser.add_option('-r', '--read-png', default=False, action='store_true', help='Read PNG, write PNM')
    parser.add_option('-i', '--interlace', default=False, action='store_true', help='create an interlaced PNG file (Adam7)')
    parser.add_option('-t', '--transparent', action='store', type='string', metavar='color', help='mark the specified colour (#RRGGBB) as transparent')
    parser.add_option('-b', '--background', action='store', type='string', metavar='color', help='save the specified background colour')
    ...
    parser.add_option('-T', '--test', default=False, action='store_true', help='create a test image (a named PngSuite image if an argument is supplied)')
    parser.add_option('-L', '--list', default=False, action='store_true', help='print list of named test images')
    ...
    parser.add_option('-K', '--test-black', action='store', type='string', metavar='pattern', help='test pattern for greyscale image')
    ...
    if options.test:
        ...
        if len(args) == 0:
            ...
            if options.read_png:
                ...
                format, width, height, depth, maxval = read_pnm_header(infile, ('P5',
                                                                                'P6',
                                                                                'P7'))
                greyscale = depth <= 2
                ...
                try:
                    ...
                    raise NotImplementedError('your maxval (%s) not in supported list %s' % (
                     maxval, str(supported)))

            bitdepth = mi + 1
            writer = Writer(width, height, greyscale=greyscale, bitdepth=bitdepth, interlace=options.interlace, transparent=options.transparent, background=options.background, alpha=bool(pamalpha or options.alpha), gamma=options.gamma, compression=options.compression)
            if options.alpha:
