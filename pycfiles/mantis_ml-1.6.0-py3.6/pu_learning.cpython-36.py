...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mantis_ml/modules/supervised_learn/pu_learn/pu_learning.py
# Compiled at: 2019-11-19 09:10:27
# Size of source mod 2**32: 19391 bytes
...
class PULearning:
    ...
    def __init__(self, cfg, data, clf_id, final_level_classifier=None):
        ...
        if self.clf_id == 'Stacking':
            if self.final_level_classifier is None:
    ...
    def train_stacking_on_subset(self, selected_base_classifiers, final_level_classifier, train_data, test_data, pos_genes_dict, neg_genes_dict, feature_dataframe_list, train_acc_list, test_acc_list, auc_score_list, pos_y_prob_dict):
        set_generator = PrepareTrainTestSets(self.cfg)
        X_train, y_train, train_gene_names, X_test, y_test, test_gene_names = set_generator.prepare_train_test_tables(train_data, test_data)
        if self.cfg.feature_selection == 'boruta':
            try:
    ...
    def train_extended_sklearn_clf_on_subset(self, train_data, test_data, pos_genes_dict, neg_genes_dict, feature_dataframe_list, train_acc_list, test_acc_list, auc_score_list, pos_y_prob_dict):
        ...
        if self.cfg.feature_selection == 'boruta':
            ...
            model = SklearnExtendedClassifier(self.cfg, X_train, y_train, X_test, y_test, train_gene_names, test_gene_names, self.clf_id, clf_params, make_plots, verbose)
    ...
    def run_pu_learning(self, selected_base_classifiers=None, final_level_classifier=None):
        ...
        neg_genes_df = pd.DataFrame((list(neg_genes_dict.values())), index=(neg_genes_dict.keys()), columns=[
         'negative_genes'])
        neg_genes_df.sort_index(inplace=True)
        print(neg_genes_df.shape)
        pos_genes_df = pd.DataFrame((list(pos_genes_dict.values())), index=(pos_genes_dict.keys()), columns=[
         'positive_genes'])
    ...
    def run(self):
        if self.clf_id == 'Stacking':
            ...
            self.run_pu_learning(selected_base_classifiers=selected_base_classifiers, final_level_classifier=(self.final_level_classifier))
        else:
