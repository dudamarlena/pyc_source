...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mantis_ml/modules/supervised_learn/core/balanced_datasets_random_generator.py
# Compiled at: 2019-11-29 07:32:55
...
class BalancedDatasetRandomGenerator:
    ...
    def get_balanced_datasets(self, verbose=False):
        ...
        while unrelated_df.shape[0] >= neg_sample_size:
            positive_df = all_positive_df.sample(n=(round(len(all_positive_df) * self.cfg.seed_pos_ratio)), random_state=(random.randint(0, 1000000)))
        ...
        if self.cfg.seed_pos_ratio == 1:
            ...
            assert verif_rows == total_rows, 'Validation Error: [verif_rows != total_rows] - not all negative data points have been included in a balanced subsample.'
            print('Passed.')
        if verbose:
    ...
    def split_into_train_and_test_sets(self, verbose=False):
        ...
        if self.stratified_kfold:
            for bal_df in self.balanced_dfs_list:
                ...
                skf = StratifiedKFold(n_splits=(self.kfold), random_state=k_fold_random_state, shuffle=False)
            ...
            for bal_df in self.balanced_dfs_list:
                for iter in range(self.cfg.kfold):
                    ...
                    train_df, test_df = train_test_split(bal_df, test_size=(self.cfg.test_size), random_state=fold_random_state,
