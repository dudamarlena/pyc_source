...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mantis_ml/modules/unsupervised_learn/dimens_reduction.py
...
import random, sys, seaborn as sns
...
class DimensionalityReduction:
    ...
    def get_clustering_from_tsne(self, X_tsne, n_clusters=15, perplexity=30):
        ...
        return (
         agglom_cl, tsne_repr, gene_names)
    ...
    def plot_embedding_w_clusters(self, agglom_cl, tsne_repr, gene_list=[], gene_names=None, filename_prefix='embedding_w_clusters', figsize=(16, 16)):
        ...
        for i in range(agglom_cl.n_clusters):
            _ = ax.scatter(x=(tsne_repr.loc[(tsne_repr.cluster == i, 'x')]), y=(tsne_repr.loc[(tsne_repr.cluster == i, 'y')]),
              color=(palette[i]),
              label=i,
              s=40,
              marker='.')
    ...
    def plot_embedding_w_labels(self, df, highlighted_genes, x, y, plot_title, filename_prefix, figsize=(10, 10)):
        ...
        for target, color in zip(targets, colors):
            ...
            ax.scatter((df.loc[(indicesToKeep, x)]), (df.loc[(indicesToKeep, y)]),
              c=color,
              s=20)
    ...
    def plot_interactive_viz(self, data, highlighted_genes, method, pos_label, neg_label, show_plot=False, save_plot=False):
        ...
        source = ColumnDataSource(dict(x=(data['x']),
          y=(data['y']),
          color=(data['colors']),
          content=(data['Gene_Name']),
          annot=(data['annotation'])))
        interact_viz = figure(plot_width=900, plot_height=900, title=method,
          tools='pan,wheel_zoom,box_zoom,reset,hover,previewsave',
          x_axis_type=None,
          y_axis_type=None,
          min_border=1)
        interact_viz.scatter(x='x', y='y', source=source,
          color='color',
          alpha=0.8,
          size=10,
          legend=method)
