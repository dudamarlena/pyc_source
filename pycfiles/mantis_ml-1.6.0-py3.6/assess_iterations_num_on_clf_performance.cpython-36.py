...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mantis_ml/modules/benchmarking/assess_iterations_num_on_clf_performance.py
...
class IterationsBenchmark:
    ...
    def get_definitive_gene_predictions(self, pos_ratio_thres=0.99):
        ...
        self.gene_pred_df = pd.merge((self.gene_pred_df), (self.known_genes_df), left_on='Gene_Name', right_on='Gene_Name', how='outer')
        self.gene_pred_df['pos_ratio'] = self.gene_pred_df['positive_genes'] / (self.gene_pred_df['positive_genes'] + self.gene_pred_df['negative_genes'])
        ...
        self.predicted_known_genes_df = self.gene_pred_df.loc[((self.gene_pred_df.pos_ratio >= pos_ratio_thres) & (self.gene_pred_df.known_gene == 1))]
        self.predicted_novel_genes_df = self.gene_pred_df.loc[((self.gene_pred_df.pos_ratio >= pos_ratio_thres) & (self.gene_pred_df.known_gene == 0))]
...
if __name__ == '__main__':
    ...
    ax.set_title('Correlation of gene probability predictions\nfor different numbers of stochastic iterations', fontsize=20,
      y=1.06)
    ax.set_xlabel('Number of stochastic iterations', fontsize=16)
    ax.set_ylabel('Number of stochastic iterations', fontsize=16)
    plt.show()
    ax.get_figure().savefig((str(cfg.out_root / '../../benchmarking-for-iterations/Gene_proba_correlation-Per_Iterations.pdf')), bbox_inches='tight')
    print(corr)
    print(type(corr))
    fig, ax = plt.subplots(figsize=(10, 10))
