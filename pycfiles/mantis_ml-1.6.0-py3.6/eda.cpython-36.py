...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mantis_ml/modules/pre_processing/eda.py
# Compiled at: 2019-11-14 14:47:10
# Size of source mod 2**32: 17618 bytes
import matplotlib
matplotlib.use('agg')
import matplotlib.pyplot as plt, pandas as pd, numpy as np, sys, seaborn as sns, warnings
...
class EDA:
    ...
    def drop_uninformative_features(self, df):
        ...
        if self.cfg.drop_gene_len_features:
            drop_elements = [
             'GeneSize', 'ExAC_cds_len', 'ExAC_gene_length']
    ...
    def plot_feature_corr_map(self, df, filename_suffix, annot=True):
        ...
        sns_plot = sns.heatmap((df.astype(float).corr()), linewidths=0.1, vmax=1.0, vmin=(-1.0), square=True,
          cmap=colormap,
          linecolor='white',
          annot=annot,
          annot_kws={'size': 7})
    ...
    def fix_feature_data_types(self, df):
        ...
        cols_with_pvals = []
        proba_cols = [
         'ExAC_mu_lof', 'ExAC_pLI', 'ExAC_pNull', 'LoF_FDR_ExAC']
    ...
    def plot_hist_for_numeric_features(self, df, pos_label='known_genes', neg_label='unrelated'):
        ...
        cols_for_log_plus_one = [
         'GTEx_Kidney_TPM_expression']
    ...
    def select_features_manually(self, df):
        ...
        """
        cols_to_keep = [
         'RVIS', 'RVIS_ExAC', 'MTR_ExACv2', 'ExAC_pREC', 'ExAC_pLI', 'LoF_FDR_ExAC',
        ...
        cols_to_keep = [
         'GO_renal', 'ExAC_mis_z', 'GO_kidney', 'LoF_FDR_ExAC', 'glomerular_expr_flag',
    ...
    def fix_classes_imbalance(self, df, gene_names, balancing_ratio=1.5, random_state=0):
        ...
        print('Balanced feature table: {0} data points x {1} features.'.format(str(balanced_df.shape[0]), str(balanced_df.shape[1])))
        return balanced_df
