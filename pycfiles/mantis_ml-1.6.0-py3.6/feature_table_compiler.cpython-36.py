...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mantis_ml/modules/pre_processing/feature_table_compiler.py
# Compiled at: 2020-02-27 11:46:08
# Size of source mod 2**32: 13535 bytes
import sys, re, pandas as pd
from mantis_ml.modules.pre_processing.data_compilation import process_generic_features, process_features_filtered_by_disease, process_ckd_specific_features, process_cardiov_specific_features
...
class FeatureTableCompiler:
    ...
    def __init__(self, cfg):
        self.cfg = cfg
        self.disease_specific_compiler = {'CardioV': process_cardiov_specific_features.ProcessCardiovascularSpecificFeatures}
        self.disease_specific_feature_files = {'CardioV': self.cfg.cardiov_specific_feature_table}
    ...
    def compile_feature_tables_per_class(self):
        ...
        if self.cfg.include_disease_features:
            ...
            if any(re.findall('CKD|chronic kidney disease', self.cfg.phenotype, re.IGNORECASE)):
                self.simplified_phenotype = 'CKD'
            if any(re.findall('Heart|Cardio|Stroke', self.cfg.phenotype, re.IGNORECASE)):
    ...
    def combine_all_feature_tables(self):
        ...
        if self.cfg.include_disease_features:
            try:
                ...
                for feature in ('CKDdb_Disease', 'CKDdb_num_of_studies', 'glom_FDR',
                                'glom_Pr_of_no_eQTL', 'glom_Exp_num_of_eQTLs', 'tub_FDR',
                                'tub_Pr_of_no_eQTL', 'tub_Exp_num_of_eQTLs', 'GOA_Kidney_Research_Priority'):
    ...
    def inspect_missing_data(self, df, verbose=False):
        ...
        if self.cfg.create_plots and missing_data.shape[0] > 0:
            ...
            ax = missing_data.reset_index().plot.barh(x='index', y='Percent', align='center',
              color='#4292c6',
              ecolor='black',
              fontsize=8,
              figsize=(10, 15))
    ...
    def drop_features_w_missing_data(self, df, missing_data, missing_data_thres=0.99):
        ...
        print('Dropped {0} features with more than {1}% missing values'.format(str(len(missing_data_elements)), str(missing_data_thres)))
        print(elems_to_drop)
        return df

    def impute_nas_w_zeros(self, df):
        valid_feature_substrings = [
         'GO_', 'ProteinAtlas_', 'GTEx_', 'GWAS_']
        go_cols = [col for col in df.columns if any(s in col for s in valid_feature_substrings)]
        replace_with_zero_elements = [
         'known_gene', 'glom_FDR', 'glom_Pr_of_no_eQTL', 'glom_Exp_num_of_eQTLs',
    ...
    def impute_nas_w_median(self, df):
        ...
        replace_with_median_elements = [
         'ExAC_dup.score', 'ExAC_del.score', 'ExAC_dup.sing.score', 'ExAC_del.sing.score',
