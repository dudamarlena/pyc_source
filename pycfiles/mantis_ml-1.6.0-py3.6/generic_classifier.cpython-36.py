...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mantis_ml/modules/supervised_learn/classifiers/generic_classifier.py
...
class GenericClassifier:
    ...
    def __init__(self, cfg, X_train, y_train, X_test, y_test, train_gene_names, test_gene_names, clf_id, verbose=False):
        self.cfg = cfg
    ...
    def plot_confusion_matrix(self, verbose=True):
        ...
        plot_confusion_matrix(confusion, classes=['unrelated', 'known CKD genes'], title=(self.clf_id.replace('Classifier', '')),
          pdf_filepath=(str(self.cfg.superv_figs_out / (self.clf_id + '_confusion_matrix.pdf'))))
    ...
    def plot_roc_curve(self, make_plot=False):
        ...
        if make_plot:
            ...
            _ = plt.plot([0, 1], [0, 1], '--', linewidth=0.5)
            _ = plt.xlim([0.0, 1.0])
            _ = plt.ylim([0.0, 1.0])
    ...
    def aggregate_predictions(self):
        TP_df = self.conc_pred_df.loc[((self.conc_pred_df['pred_1'] >= self.conc_pred_df['pred_0']) & (self.conc_pred_df['test_1'] == 1.0))]
        FP_df = self.conc_pred_df.loc[((self.conc_pred_df['pred_1'] >= self.conc_pred_df['pred_0']) & (self.conc_pred_df['test_1'] == 0.0))]
        TN_df = self.conc_pred_df.loc[((self.conc_pred_df['pred_0'] > self.conc_pred_df['pred_1']) & (self.conc_pred_df['test_0'] == 1.0))]
        FN_df = self.conc_pred_df.loc[((self.conc_pred_df['pred_0'] > self.conc_pred_df['pred_1']) & (self.conc_pred_df['test_0'] == 0.0))]
