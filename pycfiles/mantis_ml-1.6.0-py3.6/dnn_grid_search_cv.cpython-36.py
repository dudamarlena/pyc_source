...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mantis_ml/modules/supervised_learn/model_selection/dnn_grid_search_cv.py
# Compiled at: 2019-11-14 14:47:12
# Size of source mod 2**32: 7905 bytes
import pandas as pd, numpy as np, random, operator, itertools, sys
...
def run_kfold_dnn_model(train_test_indexes, params_to_tune, tmp_dnn_params):
    ...
    for i in train_test_indexes:
        ...
        dnn_model = DnnClassifier(cfg, X_train, y_train, X_test, y_test, train_gene_names, test_gene_names, **tmp_dnn_params)
        dnn_model.run()
...
def update_optimal_val_for_cur_param(gridsearch_results, params_to_tune):
    ...
    for p in range(len(params_to_tune)):
        ...
        if param == 'hidden_layer_nodes':
            optimal_val = [int(s) for s in optimal_val.split(layers_size_delim)]
...
if __name__ == '__main__':
    ...
    parameter_space = {'hidden_layer_nodes':[
      [
       32], [64], [128], [256], [32, 32], [64, 64], [128, 128], [64, 64, 64]], 
     'dropout_ratio':[
      0.1, 0.2, 0.3], 
     'optimizer':[
     ...
     'epochs':[
      10, 20, 30, 50, 70, 100]}
    params_to_tune = [
     'dropout_ratio', 'hidden_layer_nodes']
