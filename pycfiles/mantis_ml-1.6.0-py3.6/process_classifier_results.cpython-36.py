...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mantis_ml/modules/post_processing/process_classifier_results.py
# Compiled at: 2019-11-19 09:10:27
# Size of source mod 2**32: 9824 bytes
import matplotlib
matplotlib.use('agg')
import matplotlib.pyplot as plt, pandas as pd, numpy as np, pickle
    ...
    def get_density_and_cdf_plots(self, all_clf, proba_means, gene_class='Known'):
        ...
        for clf_id, clf_eval in all_clf.items():
            ...
            if clf_id in ('ExtraTreesClassifier', 'RandomForestClassifier'):
                linewidth = 3
            sns.distplot((getattr(clf_eval, proba_means)), hist=False, kde=True, bins=(int(36.0)),
              color=(boxplot_pallete[plot_cnt]),
              label=clf_id,
