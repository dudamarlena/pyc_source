...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mantis_ml/modules/supervised_learn/classifiers/dnn.py
...
class DnnClassifier(GenericClassifier):
    ...
    def __init__(self, cfg, X_train, y_train, X_test, y_test, train_gene_names, test_gene_names, clf_id, regl, hidden_layer_nodes, add_dropout, dropout_ratio, optimizer, epochs, batch_size, verbose, make_plots):
    ...
    def build_model(self):
        for l in range(self.num_hidden_layers):
            self.model.add(Dense((self.hidden_layer_nodes[l]), activation='relu', input_shape=(
    ...
    def train_model(self):
        self.callbacks = [
         EarlyStopping(monitor='acc', patience=4)]
        self.out = self.model.fit((self.X_train), (self.y_train), epochs=(self.epochs), batch_size=(self.batch_size), verbose=(self.verbose),
