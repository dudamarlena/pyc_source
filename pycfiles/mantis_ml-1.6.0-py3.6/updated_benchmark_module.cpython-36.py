...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mantis_ml/modules/hypergeom_enrichment/updated_benchmark_module.py
# Compiled at: 2020-02-28 06:42:54
# Size of source mod 2**32: 17899 bytes
import matplotlib
matplotlib.use('agg')
...
class ExternalRankingOverlap:
    ...
    def read_external_ranked_gene_list(self, external_ranked_file):
        """
                ...
                """
        self.external_ranked_df = pd.read_csv(external_ranked_file, sep=',', header=None)
        if self.external_ranked_df.shape[1] > 1:
            self.external_ranked_df.columns = [
             'Gene_Name', 'p-val']
    ...
    def calc_stepwise_hypergeometric(self, all_clf, pval_cutoff=0.05, collapsing_top_ratio=-1, genes_to_remove=[]):
        ...
        if run_for_novel_genes:
            ...
            linewidth = 1
            ax.plot(hypergeom_pvals, color='#33a02c', label=(self.clf_str),
              linewidth=linewidth)
            ...
            merged_results_df = pd.merge(merged_results_df, (pd.DataFrame({'Gene_Name':self.seed_genes,  'Known_gene':1})), how='left',
              left_on='Gene_Name',
              right_on='Gene_Name')
...
def main():
    ...
    parser.add_argument('-e', dest='external_ranked_file', required=True, help='Input file with external ranked gene list;\neither 1-column or 2-columns (with p-values in the 2nd column) [Required]\n\n')
    ...
    for clf_str in classifiers:
        ...
        rank_overlap.calc_stepwise_hypergeometric(all_clf, pval_cutoff=pval_cutoff, collapsing_top_ratio=collapsing_top_ratio)
