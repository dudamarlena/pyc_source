...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mantis_ml/modules/pre_processing/data_compilation/process_features_filtered_by_disease.py
# Compiled at: 2019-11-28 05:26:30
# Size of source mod 2**32: 28138 bytes
import pandas as pd
pd.set_option('display.max_columns', None)
import numpy as np, re, os, sys, random
...
class ProcessFeaturesFilteredByDisease(ProcessGenericFeatures):
    ...
    def process_hpo(self, include_terms, exclude_terms, annot_descr, conservative=True, save_to_file=False):
        ...
        if self.cfg.custom_known_genes_file is None:
            df = None
            df = conservative or pd.read_csv((self.cfg.data_dir / 'HPO/ALL_SOURCES_ALL_FREQUENCIES_genes_to_phenotype.txt'), sep='\t')
        else:
        ...
        return (
         known_genes_df, hpo_selected_terms)
    ...
    def process_gtex_features(self, save_to_file=False):
        ...
        return (
         return_gtex_df, all_selected_tissue_cols, all_tissue_cols)
    ...
    def process_protein_atlas_features(self, include_terms, exclude_terms, verbose=False, save_to_file=False):
        ...
        if not self.cfg.generic_classifier:
            ...
            def generic_collapse_rna_expression(df, target_col):
                ...
                if save_to_file:
                    tissue_str = 'generic'
                    tmp_df.to_csv((self.cfg.data_dir / ('human_protein_atlas/human_protein_atlas_' + tissue_str + '_rna_expression_tpm.tsv')), sep='\t',
                      index=None)
                return tmp_df
        ...
        return (
         prot_atlas_df, selected_normal_tissues, all_normal_tissues, selected_rna_samples, all_rna_samples)
    ...
    def process_msigdb_go_features(self, include_go_terms, exclude_go_terms, min_go_set_length=150, verbose=False, save_to_file=False):
        ...
        if os.path.exists(generic_go_file) and self.cfg.generic_classifier:
            ...
            return (
             msigdb_go_df, selected_go_terms)
    ...
    def process_mgi_features(self, include_terms, exclude_terms, verbose=False, save_to_file=False):
        ...
        include_pattern = re.compile('|'.join(include_terms), re.IGNORECASE)
        query_human_pheno_df = query_human_pheno_df.loc[query_human_pheno_df['human_phenotypes'].str.contains(include_pattern)]
        ...
        return (
         mgi_pheno_df, selected_mgi_phenotypes)
