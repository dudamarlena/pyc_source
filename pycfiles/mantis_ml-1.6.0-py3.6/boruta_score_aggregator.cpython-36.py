...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mantis_ml/modules/supervised_learn/feature_selection/boruta_score_aggregator.py
# Compiled at: 2019-11-14 14:47:11
# Size of source mod 2**32: 6820 bytes
import matplotlib
matplotlib.use('agg')
import matplotlib.pyplot as plt, pandas as pd, numpy as np, glob, os, sys
...
class BorutaScoreAggregator:
    ...
    def get_final_decisions(self):
        ...
        final_tentative_features = np.setdiff1d(feat_df.index.values, np.union1d(final_confirmed_features, final_rejected_features))
        ...
        self.final_decision_colors = pd.Series([self.feat_colors[feat_df.loc[(i, 'Final_Decision')]] for i in feat_df.index.values], index=(feat_df.index.values))
    ...
    def get_boruta_boxplots(self):
        ...
        for position, name in enumerate(final_decision_colors.index.values):
            ...
            bp = ax.boxplot(cur_feature_series, positions=[position], patch_artist=True, notch=True, widths=0.4, flierprops=dict(marker='o', markerfacecolor='black', markersize=3, linestyle='dotted'))
