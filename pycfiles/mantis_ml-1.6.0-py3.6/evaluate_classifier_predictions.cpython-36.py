...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mantis_ml/modules/post_processing/evaluate_classifier_predictions.py
# Compiled at: 2019-11-19 16:52:48
# Size of source mod 2**32: 14034 bytes
import matplotlib
matplotlib.use('agg')
...
feature_imp_classifiers = [
 'RandomForestClassifier', 'ExtraTreesClassifier', 'GradientBoostingClassifier', 'XGBoost']
...
class ClassifierEvaluator:
    ...
    def __init__(self, cfg, clf_id):
        ...
        try:
            ...
            self.ordered_features = [f for f in proc_feat_df.columns.values if f not in ('known_gene',
                                                                                         'Gene_Name')]
    ...
    def plot_feat_imp_distribustion(self):
        ...
        for position, name in enumerate(pivot_feat_df.columns.values):
            ...
            bp = ax.boxplot(cur_feature_series, positions=[position], patch_artist=True, notch=True, widths=0.6, flierprops=dict(marker='o', markerfacecolor='#bdbdbd', markersize=1, linestyle='dotted'))
    ...
    def plot_evaluation_metrics(self, make_plots=False):
        ...
        if make_plots:
            ...
            _ = ax.set_title('AUC score distribution across ' + str(self.total_runs) + ' runs\nAverage AUC: ' + str(self.avg_auc))
            _ = ax.set_xlabel(self.clf_id)
            _ = ax.set_ylabel('AUC')
            plt.boxplot((self.eval_metrics_df['AUC']), patch_artist=True, notch=True, widths=0.4, flierprops=dict(marker='o', markerfacecolor='black', markersize=3,
              linestyle='dotted'))
    ...
    def get_definitive_gene_predictions(self, pos_ratio_thres=0.6):
        ...
        gene_pred_df = pd.merge(gene_pred_df, (self.known_genes_df), left_on='Gene_Name', right_on='Gene_Name', how='outer')
        gene_pred_df['pos_ratio'] = gene_pred_df['positive_genes'] / (gene_pred_df['positive_genes'] + gene_pred_df['negative_genes'])
        ...
        predicted_known_genes_df = gene_pred_df.loc[((gene_pred_df.pos_ratio >= pos_ratio_thres) & (gene_pred_df.known_gene == 1))]
        predicted_novel_genes_df = gene_pred_df.loc[((gene_pred_df.pos_ratio >= pos_ratio_thres) & (gene_pred_df.known_gene == 0))]
    ...
    def gene_ranking_boxplot(self, genelist, list_id):
        ...
        for gene in genelist:
            ...
            bp = ax.boxplot(cur_gene_proba, positions=[position], patch_artist=True, notch=True, widths=0.4, flierprops=dict(marker='o', markerfacecolor='black', markersize=3, linestyle='dotted'))
        ...
        custom_legend_elements = [
         Patch(facecolor=(gene_colors['Known']), edgecolor=None, label='Known genes'),
        ...
        fig.savefig((str(self.cfg.superv_figs_gene_proba / (self.clf_id + '.' + list_id + '_genes.top_' + str(top_hits) + '.pdf'))),
          bbox_inches='tight')
        plt.close()

    def process_gene_proba_predictions(self, top_hits=50, save_to_file=False, top_hits_to_save=1000, make_plots=True, pos_decision_thres=0.5):
        ...
        self.known_gene_proba_df = self.known_gene_proba_df.reindex((self.known_gene_proba_df.mean().sort_values(ascending=False).index),
          axis=1)
        if save_to_file:
            self.known_gene_proba_df.to_hdf(self.cfg.superv_proba_pred / (self.clf_id + '.known_genes.predicted_proba.h5'))
        ...
        self.unlabelled_gene_proba_df = self.unlabelled_gene_proba_df.reindex((self.unlabelled_gene_proba_df.mean().sort_values(ascending=False).index),
          axis=1)
        if save_to_file:
            self.unlabelled_gene_proba_df.to_hdf(self.cfg.superv_proba_pred / (self.clf_id + '.unlabelled_genes.predicted_proba.h5'))
