...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mantis_ml/modules/benchmarking/estimate_predictive_power_of_GMS_score.py
# Compiled at: 2019-11-19 09:10:26
# Size of source mod 2**32: 12042 bytes
import matplotlib, matplotlib.pyplot as plt, pandas as pd, numpy as np, sys
...
config_file = '../../input_configs/validation.Generic_config.yaml'
cfg = Config(config_file)
color_palette = {'OMIM_MGI_union':'black', 
 'seed':'#33a02c',  'hidden_seed':'#fb9a99',  'unlabelled_sample':'#1f78b4',  'unlabelled':'#969696',  'omim_recessive':'#33a02c', 
 'omim_haploinsufficient':'#b2df8a',  'omim_dom_neg':'#1f78b4',  'omim_de_novo':'#a6cee3',  'omim_de_novo_and_haploinsuf':'#e31a1c', 
 'mgi_lethal_orthologs':'#fdbf6f',  'mgi_seizure_orthologs':'#cab2d6'}
...
def plot_roc_curve(fpr, tpr, roc_auc, gene_class):
    if gene_class in color_palette:
        ...
        plot_color = 'black'
    plt.plot(fpr, tpr, color=plot_color, label=(gene_class + ' (AUC = %0.4f)' % roc_auc))
...
lr = LogisticRegression(C=1000000000.0, solver='lbfgs')
...
intolerance_scores = [
 'GnomAD_mis_z', 'GnomAD_pLI', 'ExAC_cnv.score', 'LoF_FDR_ExAC', 'RVIS_ExACv2']
...
full_feature_df.shape
mantis_ml_score = 'mantis_ml_perc'
...
def get_predictive_power_for_intol_score(intol_score):
    ...
    def run_logist_regression(intol_score, positive_genes, negative_genes, model, gene_class):
        ...
        tmp_merged_df['Class'] = tmp_merged_df['Class'].map({'omim_recessive':1,  'omim_haploinsufficient':1, 
         'omim_dom_neg':1, 
         'omim_de_novo':1, 
