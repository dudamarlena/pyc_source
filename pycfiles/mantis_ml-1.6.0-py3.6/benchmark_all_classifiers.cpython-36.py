...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mantis_ml/modules/supervised_learn/model_selection/benchmark_all_classifiers.py
# Compiled at: 2019-11-14 14:47:12
# Size of source mod 2**32: 24881 bytes
import matplotlib
matplotlib.use('agg')
import matplotlib.pyplot as plt
from scipy import interp
from sklearn.metrics import auc
import pandas as pd, numpy as np, random, sys, os
...
sklearn_extended_classifiers = [
 'RandomForestClassifier', 'ExtraTreesClassifier', 'GradientBoostingClassifier', 'SVC', 'XGBoost']
...
def train_stacking_on_subset(selected_base_classifiers, final_level_classifier, train_data, test_data, feature_dataframe_list, train_acc_list, test_acc_list, auc_score_list, fpr_tpr_lists):
    set_generator = PrepareTrainTestSets(cfg)
    X_train, y_train, train_gene_names, X_test, y_test, test_gene_names = set_generator.prepare_train_test_tables(train_data, test_data)
    if cfg.feature_selection == 'boruta':
        important_features = pd.read_csv((str(cfg.boruta_tables_dir / 'Confirmed.boruta_features.csv')), header=None)
...
def train_extended_sklearn_clf_on_subset(clf_id, train_data, test_data, feature_dataframe_list, train_acc_list, test_acc_list, auc_score_list, fpr_tpr_lists):
    set_generator = PrepareTrainTestSets(cfg)
    X_train, y_train, train_gene_names, X_test, y_test, test_gene_names = set_generator.prepare_train_test_tables(train_data, test_data)
    ...
    if cfg.feature_selection == 'boruta':
        ...
        model = SklearnExtendedClassifier(cfg, X_train, y_train, X_test, y_test, train_gene_names, test_gene_names, clf_id, clf_params, make_plots, verbose)
        if clf_id == 'XGBoost':
...
def run_pu_learning(train_dfs, test_dfs, train_test_indexes, clf_id, max_workers, iterations, selected_base_classifiers=None, final_level_classifier=None):
    ...
    for i in train_test_indexes:
        ...
        if clf_id == 'DNN':
            ...
            if clf_id.startswith('Stacking'):
                ...
                if clf_id in sklearn_extended_classifiers:
    ...
    return (
     auc_score_list, fpr_tpr_lists, all_roc_data)
...
def plot_aggregate_roc_curves(all_clf, fpr_tpr_dict, auc_scores_dict, overlap_plots=True):
    if overlap_plots:
        ...
        for clf_id in all_clf:
            ...
            std_auc = np.std(aucs)
            plt.plot(mean_fpr, mean_tpr, color=(clf_colors[clf_id]), label=('%s (AUC = %0.3f $\\pm$ %0.3f)' % (clf_id, mean_auc, std_auc)),
              lw=1,
              alpha=0.8)
            plt.xlim([0.0, 1.0])
            plt.ylim([0.0, 1.0])
            ...
            plt.legend(loc='lower right', fontsize='x-small', bbox_to_anchor=(1.5,
                                                                              0))
            if not overlap_plots:
                ...
                plt.fill_between(mean_fpr, tprs_lower, tprs_upper, color='grey', alpha=0.2,
                  label='$\\pm$ 1 std. dev.')
                plt.plot([0, 1], [0, 1], linestyle='--', color='r', label='Chance', alpha=0.8, linewidth=1)
                f.savefig((str(cfg.benchmark_out / (clf_id + '.ROC_curve.pdf'))), bbox_inches='tight')

        if overlap_plots:
            plt.plot([0, 1], [0, 1], linestyle='--', color='r', label='Chance', alpha=0.8, linewidth=0.7)
        ...
        if not overlap_plots:
            ...
            for clf_id in avg_auc_distr_per_clf.columns.values:
                ...
                bp = ax.boxplot(cur_clf_series, positions=[position], patch_artist=True, notch=False, widths=0.4, flierprops=dict(marker='o', markerfacecolor='black', markersize=3, linestyle='dotted'))
...
if __name__ == '__main__':
    ...
    all_roc_data_dict = {}
    all_clf = [
     'Stacking_DNN', 'DNN', 'ExtraTreesClassifier', 'RandomForestClassifier', 'GradientBoostingClassifier', 'SVC', 'XGBoost']
    clf_colors = {'ExtraTreesClassifier':'#33a02c', 
     'RandomForestClassifier':'#1f78b4',  'GradientBoostingClassifier':'#e31a1c', 
     'SVC':'#6a3d9a',  'XGBoost':'#ff7f00',  'DNN':'#67001f', 
    ...
    for clf_id in all_clf:
        ...
        if clf_id.startswith('Stacking'):
            ...
            train_dfs, test_dfs, train_test_indexes = stacking_train_dfs, stacking_test_dfs, stacking_train_test_indexes
            auc_score_list, fpr_tpr_lists, all_roc_data = run_pu_learning(train_dfs, test_dfs, train_test_indexes, clf_id,
              max_workers, iterations, selected_base_classifiers=selected_base_classifiers,
