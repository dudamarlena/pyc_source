...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mantis_ml/modules/post_processing/pyupset.py
# Compiled at: 2019-11-14 14:47:12
# Size of source mod 2**32: 29348 bytes
...
def plot(data_dict, *, unique_keys=None, sort_by='size', inters_size_bounds=(0, np.inf), inters_degree_bounds=(
 1, np.inf), additional_plots=None, query=None):
    """
    ...
    ordered_inters_sizes, ordered_in_sets, ordered_out_sets = plot_data.get_filtered_intersections(sort_by, inters_size_bounds, inters_degree_bounds)
    ...
    fig_dict = upset.main_plot(ordered_dfs, ordered_df_names, ordered_in_sets, ordered_out_sets, ordered_inters_sizes)
    fig_dict['additional'] = []
...
def __get_all_common_columns(data_dict):
    ...
    if len(common_columns.values) == 0:
        raise ValueError('Data frames should have homogeneous columns with the same name to use for computing intersections')
...
class UpSetPlot:
    ...
    def __init__(self, rows, cols, additional_plots, query):
        ...
        """
        self.greys = plt.cm.Greys([0.22, 0.8])
        self.query = query
        qu_col = plt.cm.rainbow(np.linspace(0.01, 0.99, len(self.query)))
        ...
        self.fig, self.ax_intbars, self.ax_intmatrix, self.ax_setsize, self.ax_tablenames, self.additional_plots_axes = self._prepare_figure(additional_plots)
        self.standard_graph_settings = {'scatter':{'alpha':0.3, 
          'edgecolor':None}, 
    ...
    def _prepare_figure(self, additional_plots):
        ...
        if additional_plots:
            main_gs = gridspec.GridSpec(3, 1, hspace=0.4)
            topgs = main_gs[:2, 0]
            botgs = main_gs[(2, 0)]
        else:
        ...
        gs_top = gridspec.GridSpecFromSubplotSpec(fig_rows, fig_cols, subplot_spec=topgs, wspace=0.1, hspace=0.2)
        ...
        if additional_plots:
            ...
            gs_bottom = gridspec.GridSpecFromSubplotSpec(num_bot_rows, num_bot_cols, subplot_spec=botgs,
              wspace=0.15,
              hspace=0.2)
        ...
        return (
         fig, ax_intbars, ax_intmatrix, ax_setsize, ax_tablenames, tuple(add_ax))
    ...
    def main_plot(self, ordered_dfs, ordered_df_names, ordered_in_sets, ordered_out_sets, ordered_inters_sizes):
        ...
        return {'figure':self.fig,  'intersection_bars':self.ax_intbars, 
    ...
    def _inters_sizes_plot(self, ordered_in_sets, inters_sizes):
        ...
        for x, y in zip(self.x_values, inters_sizes):
            ax.text(x, (y + label_vertical_gap), ('%.2g' % y), rotation=90,
              ha='center',
              va='bottom')
        ...
        ax.yaxis.grid(True, lw=0.25, color='grey', ls=':')
    ...
    def _inters_matrix(self, ordered_in_sets, ordered_out_sets, xlims, ylims, set_row_map):
        ...
        self._strip_axes(ax)
        background = plt.cm.Greys([0.09])[0]
        for r, y in enumerate(self.y_values):
            if r % 2 == 0:
                ax.add_patch(Rectangle((xlims[0], y - row_width / 2), height=row_width, width=(xlims[1]),
                  color=background,
                  zorder=0))
        ...
        for col_num, (in_sets, out_sets) in enumerate(zip(ordered_in_sets, ordered_out_sets)):
            ...
            ax.scatter((np.repeat(self.x_values[col_num], len(in_y))), in_y, color=(np.tile(self._color_for_query(frozenset(in_sets)), (len(in_y), 1))),
              s=300)
    ...
    def additional_plot(self, ax_index, kind, data_values, graph_args, *, labels=None):
        ...
        for query, data_item in data_values.items():
            ...
            plot_method(color=self._color_for_query(frozenset(query)), zorder=self._zorder_for_query(frozenset(query)), **data_item)
        ...
        for l, text in labels.items():
            getattr(ax, 'set_%slabel' % l)(text, labelpad=3, fontweight='bold', fontsize=13) if l in ('x',
                                                                                                      'y') else None
...
class DataExtractor:
    ...
    def __init__(self, data_dict, unique_keys):
        ...
        self.ordered_dfs, self.ordered_df_names, self.df_dict = self.extract_base_sets_data(data_dict, unique_keys)
        self.in_sets_list, self.inters_degrees, self.out_sets_list, self.inters_df_dict = self.extract_intersection_data()
    ...
    def extract_base_sets_data(self, data_dict, unique_keys):
        ...
        return (
         ordered_base_sets, ordered_base_set_names, set_dict)
    ...
    def extract_intersection_data(self):
        ...
        inters_dict = {}
        inters_degrees = []
        for col_num, in_sets in enumerate(chain.from_iterable(combinations(self.ordered_df_names, i) for i in np.arange(1, len(self.ordered_dfs) + 1))):
            ...
            for s in in_sets_l:
                exclusive_intersection = exclusive_intersection.intersection(pd.Index(self.df_dict[s][self.unique_keys]))

            for s in out_sets:
                exclusive_intersection = exclusive_intersection.difference(pd.Index(self.df_dict[s][self.unique_keys]))

            final_df = self.df_dict[seed].set_index(pd.Index(self.df_dict[seed][self.unique_keys])).ix[exclusive_intersection].reset_index(drop=True)
    ...
    def get_filtered_intersections(self, sort_by, inters_size_bounds, inters_degree_bounds):
        ...
        size_clip = (inters_sizes <= inters_size_bounds[1]) & (inters_sizes >= inters_size_bounds[0]) & (inters_degrees >= inters_degree_bounds[0]) & (inters_degrees <= inters_degree_bounds[1])
        ...
        return (
         self.filtered_inters_sizes, self.filtered_in_sets, self.filtered_out_sets)
    ...
    def extract_data_for(self, var_dict, queries):
        ...
        for q in poss_queries:
            data_values[q] = dict(zip(var_dict.keys(), [self.inters_df_dict[frozenset(q)][v].values for k, v in var_dict.items()]))

        data_values['others'] = dict(zip(var_dict.keys(), [chain(*[self.inters_df_dict[frozenset(q)][v].values for q in self.filtered_in_sets if q not in poss_queries]) for k, v in var_dict.items()]))
...
if __name__ == '__main__':
    ...
    data_dict = load(f)
    f.close()
    d = plot(data_dict, unique_keys=['title'], inters_size_bounds=(2, 20),
      query=[
     ('action', ), ('romance', 'action', 'war', 'adventure'), ('romance', 'adventure')],
      additional_plots=[
     {'kind':'scatter', 
      'data_quantities':{'x':'rating_std', 
       'y':'rating_avg'}, 
      'graph_properties':{'alpha':1, 
       'edgecolor':'w',  'lw':0.3}},
     {'kind':'hist', 
      'data_quantities':{'x': 'views'}, 
      'graph_properties':{'bins':10, 
       'alpha':0.6}}])