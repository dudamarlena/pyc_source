...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mantis_ml/modules/benchmarking/benchmarking_collapsing_hypergeom_enrichment.py
# Compiled at: 2019-11-19 09:10:26
# Size of source mod 2**32: 20552 bytes
import matplotlib
matplotlib.use('agg')
...
clf_alias = {'ExtraTreesClassifier':'ET', 
 'SVC':'SVC',  'DNN':'DNN',  'RandomForestClassifier':'RF',  'XGBoost':'XGB', 
 'GradientBoostingClassifier':'GB',  'Stacking':'Stacking'}
primary_analysis_types = [
 'primary', 'URV_AC_eq_1', 'v-AURORA-CUMC-all_dom_ultrarare_OO', 'Dom_LoF']
primary_analyses_dict = {'CKD':'v-AURORA-CUMC-all_dom_ultrarare_OO', 
 'GGE':'primary',  'ALS':'Dom_LoF'}
synonymous_analyses_dict = {'CKD':'v-AURORA-CUMC-all_dom_rare_syn',  'GGE':'synonymous',  'ALS':'Dom_not_benign'}
color_pallete = {'primary':'#33a02c', 
 'lof':'#fb9a99',  'common':'#a6cee3',  'synonymous':'#000000',  'Dom_LoF':'#33a02c', 
 'Dom_coding':'#fb9a99',  'Dom_not_benign':'#000000',  'Rec_LoF':'#feb24c', 
 'Rec_coding':'#a1d99b',  'Rec_not_benign':'#3182bd',  'URV_AC_eq_1':'#33a02c', 
 'URV_AC_lteq_3':'#000000',  'v-AURORA-CUMC-all_dom_ultrarare_OO':'#33a02c', 
 'v-AURORA-CUMC-all_dom_rare_LOF':'#33a02c',  'piv-mendelian-all_dom_rare_LOF':'#33a02c', 
 'i-all_AURORA_dom_rare_missense':'#ff7f00',  'viii-CUMC-all_dom_rare_mtr50':'#33a02c', 
 'v-AURORA-CUMC-all_dom_rare_missense_mtr50':'#000000',  'i-all_AURORA_dom_rare_syn':'#a6cee3', 
 'v-AURORA-CUMC-all_dom_rare_syn':'#3182bd',  'piv-mendelian-all_dom_rare_syn':'#756bb1', 
 'v-AURORA-CUMC-all-rec_syn':'#d95f0e',  'iii-CUMC-all_dom_rare_syn':'#c51b8a',  'shuffle':'#1f78b4'}
dataset_ylim = {'CKD':60, 
 'GGE':60,  'ALS':60}
...
class CollapsingMantisMlOverlap:
    ...
    def read_collapsing_ranking(self, analysis_type='primary', disease='GGE', pval_cutoff=0.05, collapsing_top_ratio=-1, genes_to_remove=[]):
        ...
        if analysis_type == 'shuffle':
            ...
            collapsing_df = pd.read_csv((str(self.cfg.out_root / ('../../' + input_dir + '/' + disease + '/' + analysis_type + '_collapsing_ranking.' + disease + '.csv'))), header=None,
              quotechar="'")
            collapsing_df.columns = [
             'Gene_Name', 'p-val']
            if disease in ('GGE', 'NAFE', 'ALS', 'IPF') and analysis_type != 'primary':
                tmp_tuples = [(collapsing_df.loc[(i, 'Gene_Name')], collapsing_df.loc[(i, 'p-val')]) for i in range(len(collapsing_df))]
    ...
    def calc_stepwise_hypergeometric(self, clf_str, all_clf, top_ratio, disease='GGE', pval_cutoff=0.05, collapsing_top_ratio=-1, show_plots=False, genes_to_remove=[]):
        ...
        for analysis_type in analysis_types:
            ...
            if analysis_type in primary_analysis_types:
                linewidth = 2
            ax.plot(hypergeom_pvals, color=(color_pallete[analysis_type]), label=analysis_type,
              linewidth=linewidth)
            ...
            if not run_external_benchmark:
                ...
                if analysis_type in primary_analysis_types:
                    ...
                    merged_results_df = pd.merge(merged_results_df, (pd.DataFrame({'Gene_Name':seed_genes,  'Known_gene':1})), how='left',
                      left_on='Gene_Name',
                      right_on='Gene_Name')
        ...
        with open(Path(str(cfg.out_root) + '/../../' + input_dir + '/Hypergeometric_results/' + disease + '/' + method_identifier + '.top' + str(top_ratio) + '_genes.Mann_Whitney_U.txt'), 'w') as (text_file):
...
if __name__ == '__main__':
    ...
    if 'CKD' in cfg.phenotype:
        analysis_types = [
         'v-AURORA-CUMC-all_dom_ultrarare_OO',
    ...
    for clf_str in classifiers:
        ...
        overlap_obj.calc_stepwise_hypergeometric(clf_str, all_clf, top_ratio, disease=disease, pval_cutoff=pval_cutoff, collapsing_top_ratio=collapsing_top_ratio,
