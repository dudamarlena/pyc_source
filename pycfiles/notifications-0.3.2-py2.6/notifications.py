...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/notifications.py
# Compiled at: 2011-11-16 19:57:40
...
def get_credentials(email, password, debug=False):
    ...
    if cred_key in resp_dict.keys():
        return resp_dict[cred_key]
    raise ValueError(('Unknown response received: {0}').format(resp_dict))


def send(credentials, message, title=None, subtitle=None, long_message=None, long_message_preview=None, icon_url=None, message_level=0, silent=False, action_loc_key=None, run_command=None, sound=1, debug=False):
    u"""Send a notification, waiting for the message to be sent.
    ...
    for key in ('title', 'subtitle', 'long_message', 'long_message_preview', 'icon_url',
