...
# [GCC 8.4.0]
# Embedded file name: /app/django_sloop/handlers.py
# Compiled at: 2019-07-23 08:57:06
...
class SNSHandler(object):
    ...
    def get_client(self):
        if self.client:
            return self.client
        client = boto3.client('sns', region_name=DJANGO_SLOOP_SETTINGS.get('AWS_REGION_NAME') or None, aws_access_key_id=DJANGO_SLOOP_SETTINGS.get('AWS_ACCESS_KEY_ID') or None, aws_secret_access_key=DJANGO_SLOOP_SETTINGS.get('AWS_SECRET_ACCESS_KEY') or None)
        return client
    ...
    def application_arn(self):
        if self.device.platform == AbstractSNSDevice.PLATFORM_IOS:
            ...
            if self.device.platform == AbstractSNSDevice.PLATFORM_ANDROID:
    ...
    def get_or_create_platform_endpoint_arn(self):
        if self.device.sns_platform_endpoint_arn:
            ...
            endpoint_response = self.client.create_platform_endpoint(PlatformApplicationArn=self.application_arn, Token=self.device.push_token)
    ...
    def _send_payload(self, data):
        ...
        if settings.DEBUG:
            ...
            print(message)
        try:
            publish_result = self.client.publish(TargetArn=endpoint_arn, Message=message, MessageStructure='json')
