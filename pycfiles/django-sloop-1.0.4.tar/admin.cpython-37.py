...
# [GCC 8.4.0]
# Embedded file name: /app/django_sloop/admin.py
# Compiled at: 2019-06-28 12:43:45
# Size of source mod 2**32: 4664 bytes
...
class PushNotificationView(FormView):
    ...
    def get_initial(self):
        ...
        if not initial.get('receivers'):
            if self.request.GET.get('receivers'):
    ...
    def get_success_url(self):
        ...
        url += '?' + urlencode({'receivers': json.dumps(self.receivers)})
        return url
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({'admin_site':admin_site.name, 
...
class SloopAdminMixin(object):
    ...
    def get_push_notification_urls(self):
        custom_urls = [
         url('^send-push-notification/$', (self.admin_site.admin_view(self.push_notification_view)), name=('%s_%s_send_push_notification' % (self.model._meta.app_label, self.model._meta.model_name)))]
        return custom_urls
