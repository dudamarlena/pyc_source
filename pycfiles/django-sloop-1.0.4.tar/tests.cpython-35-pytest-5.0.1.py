...
# [GCC 8.4.0]
# Embedded file name: /app/django_sloop/tests.py
# Compiled at: 2019-07-23 09:16:11
# Size of source mod 2**32: 12974 bytes
import builtins as @py_builtins, _pytest.assertion.rewrite as @pytest_ar, json, time
...
class SNSHandlerTests(TestCase):
    ...
    def test_create_platform_endpoint(self):
        ...
        sns_client.create_platform_endpoint.assert_called_once_with(PlatformApplicationArn=DJANGO_SLOOP_SETTINGS['SNS_IOS_APPLICATION_ARN'], Token=self.ios_device.push_token)
...
class DeviceTests(TestCase):
    ...
    def test_send_ios_push_notification(self):
        ...
        sns_client.publish.return_value = {'MessageId': sns_test_message_id}
        SNSHandler.client = sns_client
        ...
        expected_message = {'APNS': {'aps': {'alert': 'test_message', 
                          'sound': 'test_sound', 
                          'badge': 3, 
        ...
        self.assertEqual(push_message.sns_response, str({'MessageId': sns_test_message_id}))
    ...
    def test_send_android_push_notification(self):
        ...
        self.assertEqual(call_kwargs['MessageStructure'], 'json')
        expected_message = {'GCM': {'data': {'alert': 'test_message', 
                          'sound': 'test_sound', 
                          'badge': 3, 
                          'category': 'test_category', 
                          'custom': {'foo': 'bar', 'url': 'test_url'}, 
    ...
    def test_send_ios_silent_push_notification(self):
        ...
        self.assertEqual(call_kwargs['TargetArn'], self.ios_device.sns_platform_endpoint_arn)
        self.assertEqual(call_kwargs['MessageStructure'], 'json')
        expected_message = {'APNS': {'aps': {'content-available': True, 
                          'sound': '', 
                          'badge': 0, 
                          'custom': {'foo': 'bar'}, 
                          'foo': 'bar'}}}
    ...
    def test_send_android_silent_push_notification(self):
        ...
        self.assertEqual(call_kwargs['TargetArn'], self.android_device.sns_platform_endpoint_arn)
        self.assertEqual(call_kwargs['MessageStructure'], 'json')
        expected_message = {'GCM': {'data': {'content-available': True, 
                          'sound': '', 
                          'badge': 0, 
                          'custom': {'foo': 'bar'}, 
                          'foo': 'bar'}}}
    ...
    def test_invalidate_device_if_push_message_fails(self):
        ...
        self.assertEqual(push_message.sns_response, str({'Error': {'Code': 'EndpointDisabled'}}))
