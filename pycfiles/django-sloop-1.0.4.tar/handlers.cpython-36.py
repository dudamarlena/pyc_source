...
# Size of source mod 2**32: 6142 bytes
import json, boto3
...
class SNSHandler(object):
    ...
    def get_or_create_platform_endpoint_arn(self):
        if self.device.sns_platform_endpoint_arn:
            ...
            endpoint_response = self.client.create_platform_endpoint(PlatformApplicationArn=(self.application_arn),
    ...
    def _send_payload(self, data):
        ...
        if settings.DEBUG:
            ...
            print(message)
        try:
            publish_result = self.client.publish(TargetArn=endpoint_arn,
              Message=message,
