...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hisensetv/hisensetv.py
# Compiled at: 2020-02-29 14:06:09
# Size of source mod 2**32: 12704 bytes
...
import posixpath, queue, ssl, time, uuid

class HisenseTvError(Exception):
    __doc__ = ' Base exception for all exceptions raise by this API. '


class HisenseTvNotConnectedError(HisenseTvError):
    __doc__ = ' Raised when an API function is called without a connection. '


class HisenseTvAuthorizationError(HisenseTvError):
    __doc__ = ' Raised upon authorization failures. '


class HisenseTvTimeoutError(HisenseTvError):
    __doc__ = ' Raised upon a failure to receive a response in the timeout period. '
...
def _check_connected(func: Callable):
    ...
    def wrapper(self, *args, **kwargs):
        if not self.connected:
            raise HisenseTvNotConnectedError(f"you must be connected to call {func.__name__}")
...
class HisenseTv:
    ...
    _VALID_SERVICES = {
     'platform_service', 'remote_service', 'ui_service'}

    def __init__(self, hostname: str, *, port: int=36669, username: str='hisenseservice', password: str='multimqttservice', timeout: Union[(int, float)]=10.0, enable_client_logger: bool=False):
    ...
    def __enter__(self):
        ...
        self._mqtt_client.username_pw_set(username=(self.username),
    ...
    def _on_connect(self, client: mqtt.Client, userdata: Optional[Any], flags: Dict[(str, int)], rc: int):
    ...
    def _on_message(self, client: mqtt.Client, userdata: Optional[Any], msg: mqtt.MQTTMessage):
        ...
        if msg.payload:
            try:
                ...
                try:
                    ...
                    self.logger.debug(f"Recieved message on topic {msg.topic} with payload: {payload}")
        else:
            payload = msg.payload
    ...
    def _wait_for_response(self) -> Optional[dict]:
        ...
        try:
            ...
            try:
                raise HisenseTvTimeoutError(f"failed to recieve a response in {self.timeout:.3f}s") from e
            finally:
                e = None
                del e

    def _call_service(self, *, service: str, action: str, payload: Optional[Union[(str, dict)]]=None):
        """
        ...
        if service not in self._VALID_SERVICES:
            raise ValueError(f"service of {service!r} is invalid, service must be one of {self._VALID_SERVICES!r}")
        if isinstance(payload, dict):
            payload = json.dumps(payload)
        full_topic = posixpath.join('/', 'remoteapp', 'tv', service, self._device_topic, 'actions', action)
    ...
    def set_source(self, sourceid: Union[(int, str)], sourcename: str):
        ...
        self._call_service(service='ui_service',
          action='changesource',
    ...
    def set_volume(self, volume: int):
        ...
        if volume < 0 or volume > 100:
            raise ValueError(f"volume of {volume!r} is invalid, volume must be between 0 and 100")
        self._call_service(service='platform_service',
          action='changevolume',
          payload=(str(volume)))
    ...
    def send_authorization_code(self, code: Union[(int, str)]):
        ...
        if result != 1:
            raise HisenseTvAuthorizationError(f"authorization failed with code {result}")