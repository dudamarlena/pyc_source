...
# [GCC 8.4.0]
# Embedded file name: /opt/hostedtoolcache/Python/3.8.2/x64/lib/python3.8/site-packages/wimsapi/user.py
...
class User(ClassItemABC):
    ...
    def infos(self):
        ...
        status, user_info = self._class._api.getuser((self._class.qclass),
          (self._class.rclass), (self.quser), verbose=True)
        if not status:
    ...
    def _to_payload(self):
        return {v:k for k, v in self.__dict__.items() if k not in ('quser', '_class',
                                                                   '_saved') if k not in ('quser',
                                                                                          '_class',
                                                                                          '_saved')}
    ...
    def save(self, wclass=None, check_exists=True):
        ...
        if not wclass:
            if not self._class:
                raise NotSavedError('wclass must be provided if this user has neither been imported from a WIMS class nor saved once yet')
        else:
            ...
            if self._saved:
                status, response = wclass._api.moduser((wclass.qclass),
                  (wclass.rclass), (self.quser), (self._to_payload()), verbose=True)
            else:
                status, response = wclass._api.adduser((wclass.qclass),
                  (wclass.rclass), (self.quser), (self._to_payload()), verbose=True)
        if not status:
    ...
    def delete(self):
        ...
        status, response = self._class._api.deluser((self._class.qclass), (self._class.rclass), (self.quser),
          verbose=True)
        if not status:
            raise AdmRawError(response['message'])
        self.wclass = False
    ...
    def check(cls, wclass, user):
        ...
        if not wclass._saved:
            raise NotSavedError('Class must be saved before being able to check whether an user exists')
    ...
    def get(cls, wclass, quser):
        ...
        if not wclass._saved:
            ...
            status, user_password = wclass._api.getuser((wclass.qclass), (wclass.rclass), quser, [
             'password'],
              verbose=True)
            assert status, user_password['message']
