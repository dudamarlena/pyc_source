...
# [GCC 8.4.0]
# Embedded file name: /opt/hostedtoolcache/Python/3.8.2/x64/lib/python3.8/site-packages/wimsapi/exam.py
...
class Exam(ClassItemABC):
    ...
    def infos(self):
        ...
        status, exam_info = self._class._api.getexam((self._class.qclass),
          (self._class.rclass), (self.qexam), verbose=True)
        if not status:
    ...
    def _to_payload(self):
        return {v:k for k, v in self.__dict__.items() if k not in ('qexam', '_saved',
                                                                   '_class') if k not in ('qexam',
                                                                                          '_saved',
                                                                                          '_class')}
    ...
    def save(self, wclass=None, check_exists=True):
        ...
        if not wclass:
            if not self._class:
                raise NotSavedError('wclass must be provided if this exam has neither been imported from a WIMS class nor saved once yet')
        else:
            ...
            if self._saved:
                status, response = wclass._api.modexam((wclass.qclass),
                  (wclass.rclass), (self.qexam), (self._to_payload()), verbose=True)
            else:
                status, response = wclass._api.addexam((wclass.qclass),
    ...
    def delete(self):
        ...
        status, response = self._class._api.delexam((self._class.qclass), (self._class.rclass), (self.qexam),
          verbose=True)
        if not status:
            raise AdmRawError(response['message'])
        self.wclass = False
    ...
    def check(cls, wclass, exam):
        ...
        if not wclass._saved:
            raise NotSavedError('Class must be saved before being able to check whether a exam exists')
        exam = exam.qexam if isinstance(exam, cls) else exam
        status, response = wclass._api.checkexam((wclass.qclass), (wclass.rclass), exam, verbose=True)
    ...
    def get(cls, wclass, qexam):
        ...
        status, exam_info = wclass._api.getexam((wclass.qclass), (wclass.rclass), qexam, verbose=True)
        if not status:
            raise AdmRawError(exam_info['message'])
    ...
    def scores(self, user=None):
        ...
        if not self.wclass:
            ...
            status, response = self._class._api.getexamscores((self._class.qclass), (self._class.rclass), (self.qexam),
              verbose=True)
