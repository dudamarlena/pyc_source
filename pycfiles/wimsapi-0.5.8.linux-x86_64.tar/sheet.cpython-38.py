...
# [GCC 8.4.0]
# Embedded file name: /opt/hostedtoolcache/Python/3.8.2/x64/lib/python3.8/site-packages/wimsapi/sheet.py
# Compiled at: 2020-05-04 16:09:45
# Size of source mod 2**32: 13576 bytes
import datetime, sys
...
class Sheet(ClassItemABC):
    ...
    def infos(self):
        ...
        status, sheet_info = self._class._api.getsheet((self._class.qclass),
          (self._class.rclass), (self.qsheet), verbose=True)
        if not status:
    ...
    def __eq__(self, other):
        ...
        if isinstance(other, self.__class__):
            if not (self.wclass and other.wclass):
    ...
    def _to_payload(self):
        return {v:k for k, v in self.__dict__.items() if k not in ('qsheet', '_saved',
                                                                   '_class') if k not in ('qsheet',
                                                                                          '_saved',
                                                                                          '_class')}
    ...
    def save(self, wclass=None, check_exists=True):
        ...
        if not wclass:
            if not self._class:
                raise NotSavedError('wclass must be provided if this sheet has neither been imported from a WIMS class nor saved once yet')
        else:
            ...
            if self._saved:
                status, response = wclass._api.modsheet((wclass.qclass),
                  (wclass.rclass), (self.qsheet), (self._to_payload()), verbose=True)
            else:
                status, response = wclass._api.addsheet((wclass.qclass),
                  (wclass.rclass), (self._to_payload()), verbose=True)
        if not status:
    ...
    def delete(self):
        ...
        status, response = self._class._api.delsheet((self._class.qclass), (self._class.rclass), (self.qsheet),
          verbose=True)
        if not status:
            raise AdmRawError(response['message'])
        self.wclass = False
    ...
    def check(cls, wclass, sheet):
        ...
        if not wclass._saved:
            raise NotSavedError('Class must be saved before being able to check whether a sheet exists')
        sheet = sheet.qsheet if isinstance(sheet, cls) else sheet
        status, response = wclass._api.checksheet((wclass.qclass), (wclass.rclass), sheet, verbose=True)
        msg = 'element #%s of type sheet does not exist in this class (%s)' % (
         str(sheet), str(wclass.qclass))
        if not status:
            if msg not in response['message']:
                raise AdmRawError(response['message'])
        return status
    ...
    def get(cls, wclass, qsheet):
        ...
        status, sheet_info = wclass._api.getsheet((wclass.qclass), (wclass.rclass), qsheet, verbose=True)
        if not status:
            raise AdmRawError(sheet_info['message'])
    ...
    def scores(self, user=None):
        ...
        if not self.wclass:
            ...
            if quser is not None:
                if not self._class.checkitem(quser, User):
                    raise ValueError("User '%s' does not exists in class '%s'" % (user, self._class.qclass))
            status, response = self._class._api.getsheetscores((self._class.qclass), (self._class.rclass), (self.qsheet),
              verbose=True)
            assert status, response['message']
        scores = []
        ...
        for data in response['data_scores']:
            if quser is not None and data['id'] != quser:
                ...
                try:
                    score = self._compute_grade(response['sheet_formula']['formula'], response['sheet_formula']['I'], data['user_quality'], data['user_percent'], data['user_best'], data['user_level'])
                except Exception:
                    score = -1
                else:
                    sheet_params = {'sheet':self, 
                     'user':user, 
                     'score':score, 
