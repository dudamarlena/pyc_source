...
# [GCC 8.4.0]
# Embedded file name: /opt/hostedtoolcache/Python/3.8.2/x64/lib/python3.8/site-packages/wimsapi/wclass.py
# Compiled at: 2020-05-04 16:09:45
# Size of source mod 2**32: 15784 bytes
import datetime
...
class Class:
    ...
    def __eq__(self, other):
        ...
        if isinstance(other, self.__class__):
            if not (self._api and other._api):
    ...
    def _to_payload(self):
        ...
        d = {v:k for k, v in self.__dict__.items() if k not in ('qclass', 'rclass',
                                                                'supervisor', '_api',
                                                                'wclass') if k not in ('qclass',
                                                                                       'rclass',
                                                                                       'supervisor',
                                                                                       '_api',
                                                                                       'wclass')}
    ...
    def check(cls, url, ident, passwd, qclass, rclass):
        ...
        if not status:
            if response['message'] not in (msg1, msg2):
    ...
    def save(self, url=None, ident=None, passwd=None):
        ...
        if url:
            if ident:
                ...
                assert self._api, 'url, ident and passwd must be provided when saving for the first time.'
            ...
            status, response = self._api.addclass((self.rclass),
              payload, (self.supervisor._to_payload()), (self.qclass), verbose=True)
            if not status:
                raise AdmRawError(response['message'])
            self._saved = True
    ...
    def get(cls, url, ident, passwd, qclass, rclass):
        ...
        status, password_info = api.getuser(qclass, rclass, 'supervisor', ['password'], verbose=True)
        if not status:
    ...
    def delitem(self, item, cls=None):
        ...
        test = not isinstance(item, str) and not issubclass(type(item), ClassItemABC) or cls is not None and not issubclass(cls, ClassItemABC)
        if test:
            raise InvalidItemTypeError('Item of type %s cannot be deleted from the WIMS class' % str(type(item) if not isinstance(item, str) else cls))
        if not self._saved:
    ...
    def checkitem(self, item, cls=None):
        ...
        test = not isinstance(item, str) and not issubclass(type(item), ClassItemABC) or cls is not None and not issubclass(cls, ClassItemABC)
        if test:
            raise InvalidItemTypeError('Cannot check if an item of type %s is in a WIMS class' % str(type(item) if not isinstance(item, str) else cls))
        if not self._saved:
            raise NotSavedError('Class must be saved before being able to check whether an item exists')
        cls = cls or type(item)
    ...
    def listitem(self, cls):
        ...
        if not issubclass(cls, ClassItemABC):
            raise InvalidItemTypeError('Cannot list element of type %s from a WIMS class' % str(cls))
        if not self._saved:
