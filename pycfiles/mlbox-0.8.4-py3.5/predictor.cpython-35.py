...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mlbox/prediction/predictor.py
# Compiled at: 2020-04-13 16:34:29
# Size of source mod 2**32: 15970 bytes
import pandas as pd, numpy as np, os, pickle, warnings, time, operator, matplotlib
try:
                    ...
                    if set_params:
                        try:
                            ...
                            try:
                                ...
                                self._Predictor__save_feature_importances(importance, self.to_path + '/' + est.get_params()['strategy'] + '_feature_importance.png')
                                if self.verbose:
                                    self._Predictor__plot_feature_importances(importance, 10)
                                    print('')
        ...
        if df['target'].dtype == 'int':
            ...
            if df['target'].dtype == 'float':
                pred = pd.DataFrame([], columns=[
                 df['target'].name + '_predicted'], index=df['test'].index)
                try:
                    if self.verbose:
                        print('')
                        print('predicting...')
