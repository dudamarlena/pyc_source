...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/easyshop/checkout/browser/thank_you.py
...
class ThankYouPageView(BrowserView):
    ...
    def getLatestOrder(self):
        ...
        transaction = {'order_id': order.getId(), 'affiliation': '', 'total': prices.getPriceForCustomer(), 'tax': prices.getPriceForCustomer() - prices.getPriceNet(), 'shipping': order.getShippingPriceGross(), 'city': address.city, 'state': '', 'country': address.country}
        items = []
        for item in IItemManagement(order).getItems():
            ...
            items.append({'order_id': order.getId(), 'sku': product.getArticleId(), 'productname': product.Title(), 'category': category_title, 'price': item.getProductPriceGross(), 'quantity': item.getProductQuantity()})

        result = {'id': order.getId(), 'url': order.absolute_url(), 'google_transaction': transaction, 'google_items': items}
        return result
    ...
    def getProducts(self):
        ...
        for product in selector.getRefs():
            ...
            result.append({'title': product.Title(), 'short_title': product.getShortTitle() or product.Title(), 'url': product.absolute_url(), 'price': price, 'image': image})
    ...
    def sendRecommendation(self):
        ...
        if self.request.get('email', '') == '':
            ...
            text = template()
            sendMultipartMail(context=self.context, from_=self.context.getMailFrom(), to=self.request.get('email'), subject='Empfehlung Demmelhuber Holz und Raum GmbH', text=text, charset=charset)
            utool = getToolByName(self.context, 'plone_utils')
        ...
        for selector in self.context.objectValues('ProductSelector'):
            ...
            for (index, product) in enumerate(selector.getRefs()):
                ...
                if index == n - 1 and n > 1 and products_per_line > 1:
                    ...
                    klass = 'notlast'
                products.append({'title': product.Title(), 'short_title': product.getShortTitle() or product.Title(), 'url': product.absolute_url(), 'price': price, 'image': image, 'text': text, 'class': klass})
                if (index + 1) % products_per_line == 0:
                    lines.append(products)
                    products = []

            lines.append(products)
            selectors.append({'edit_url': '%s/base_edit' % selector.absolute_url(), 'show_title': selector.getShowTitle(), 'title': selector.Title(), 'lines': lines, 'products_per_line': products_per_line, 'td_width': '%s%%' % (100 / products_per_line)})
