...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/motif/motif.py
# Compiled at: 2015-09-14 01:27:57
"""10 Digit Phone Numbers Question
...
Please write a program in the language of your choice to solve the above problem. Submit all source files and any test
...
class Motif(object):
    ...
    def __init__(self):
        self.keypad = [
         ...
         [
          '*', 0, '#']]
        self.NUM_OF_POSSIBLE_NEXT_DIGIT = {0: 3, 
           1: 2, 2: 3, 3: 2, 4: 3, 5: 5, 6: 3, 7: 3, 8: 3, 9: 3}
        self.DIGIT_MAP = {0: (0, 7, 9), 1: (1, 5), 
           2: (2, 4, 6), 
           3: (3, 5), 
           4: (2, 4, 8), 
           5: (1, 3, 7, 9, 5), 
    ...
    def set_keypad(self, k):
        """ interface for customize keypad """
        self.keypad = k

    def construct_map(self):
        """ construct_map meta data for current keypad """
        self.DIGIT_MAP = {}
            ...
            for j in range(N):
                ...
                if isinstance(k, int):
                    tmp = [
                     k]
                    for x in [1, -1]:
                        if i + x < M and i + x >= 0 and j + x >= 0 and j + x < N and isinstance(self.keypad[(i + x)][(j + x)], int):
                            tmp.append(self.keypad[(i + x)][(j + x)])
                        if i - x < M and i - x >= 0 and j + x >= 0 and j + x < N and isinstance(self.keypad[(i - x)][(j + x)], int):
                            tmp.append(self.keypad[(i - x)][(j + x)])

                    self.DIGIT_MAP[k] = set(tmp)

        for k, v in self.DIGIT_MAP.items():
            self.NUM_OF_POSSIBLE_NEXT_DIGIT[k] = len(v)
