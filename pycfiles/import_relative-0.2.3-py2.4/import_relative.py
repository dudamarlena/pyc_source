...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/import_relative.py
# Compiled at: 2013-03-24 10:56:42
import imp, os, sys
...
def path2abspath(path, call_level):
    ...
    if path is None:
        ...
        if alldots:
            srcdir = os.path.abspath(os.path.join(get_srcdir(call_level), pardir))
        else:
            srcdir = os.path.abspath(os.path.join(get_srcdir(call_level), pardir, path))
    else:
        srcdir = os.path.abspath(os.path.join(get_srcdir(call_level), '.', path))
    return srcdir
...
def import_relative(import_name, path=None, top_name=None):
    ...
    if not mod or not mod.__file__.startswith(top_file_prefix):
        try:
            ...
            raise ImportError('No module %s found in %s' % (top_module, srcdir))
        else:
            module_save = None
            ...
            try:
                try:
                    mod = imp.load_module(namespaced_top_module, fp, pathname, description)
...
if __name__ == '__main__':
    ...
    os2_path = Mimport_relative.import_relative('os2.path', 'test', 'import_relative')
    print os2_path
