...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/gerritcheck/check.py
# Compiled at: 2016-04-05 02:49:22
from collections import defaultdict
from plumbum import local, SshMachine
from flake8.engine import get_style_guide
import argparse, exceptions, json, os, sys
...
def extract_files_for_commit(rev):
    ...
    rc = result.wait()
    if rc != 0:
        raise GerritCheckExecption('Could not run diff on current revision. Make sure that the current revision has a parent.')
        ...
        if report.total_errors:
            ...
            for line_number, offset, code, text, doc in report._deferred_print:
                if not line_part_of_commit(file, line_number, commit):
                    continue
                review['comments'][file].append({'path': file, 
                   'line': line_number, 
...
def cppcheck_on_files(files, commit):
    ...
    to submit a review.
    """
    cppcheck_cmd = local['cppcheck'][('--quiet', '-j 5', '--template={file}###{line}###{severity}###{message}')]
    review = {}
    ...
    if len(err) > 0:
        ...
        for c in err.split('\n'):
            if len(c.strip()) == 0:
                continue
            parts = c.split('###')
            if not line_part_of_commit(parts[0], parts[1], commit):
                continue
            review['comments'][parts[0]].append({'path': parts[0], 
               'line': parts[1], 
...
def cpplint_on_files(files, commit, filters=DEFAULT_CPPLINT_FILTER_OPTIONS):
    ...
    if len(err) > 0 and len(files):
        ...
        review['comments'] = defaultdict(list)
        for c in err.split('\n'):
            if len(c.strip()) == 0 or c.strip().startswith('Done') or c.strip().startswith('Total') or c.strip().startswith('Ignoring'):
                continue
            location, rest = c.split('  ', 1)
            message, category = rest.rsplit('  ', 1)
            file, line, _ = location.split(':', 2)
            if not line_part_of_commit(file, line, commit):
                continue
            review['comments'][file].append({'path': file, 
               'line': line, 
...
CHECKER_MAPPING = {'cppcheck': cppcheck_on_files, 
   'cpplint': cpplint_on_files, 
   'flake8': flake8_on_files}

def main():
    parser = argparse.ArgumentParser(description='Execute code analysis and report results locally or to gerrit', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('-g', '--gerrit-host', help='Gerrit host')
    parser.add_argument('-u', '--user', help='Username', default='jenkins')
    parser.add_argument('-p', '--port', help='SSH port Gerrit listens on', default=22)
    parser.add_argument('-c', '--commit', help='Git Hash of the commit to check', default='HEAD')
    parser.add_argument('-t', '--tool', help='Which validation to run', choices=CHECKER_MAPPING.keys(), action='append', required=True)
    parser.add_argument('-l', '--local', action='store_true', default=False, help='Display output locally instead of submitting it to Gerrit')
    ...
    for t in args.tool:
        ...
        if args.local:
            ...
            submit_review(args.commit, args.user, args.gerrit_host, result, args.port)
