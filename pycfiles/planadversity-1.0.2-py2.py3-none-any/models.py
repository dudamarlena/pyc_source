...
# [GCC 8.4.0]
# Embedded file name: /home/powellc/projects/volunteer-coordination/volunteerhub/apps/volunteers/models.py
# Compiled at: 2014-07-14 11:21:51
...
class Organization(TimeStampedModel, TitleSlugDescriptionModel):
    ...
    managers = models.ManyToManyField(get_user_model(), blank=True, null=True)
...
class Project(TimeStampedModel, TitleSlugDescriptionModel):
    lead_volunteers = models.ManyToManyField(get_user_model(), blank=True, null=True)
    ...
    def __unicode__(self):
        return ('{0} at {1}').format(self.title, self.organization)


class OpenOpportunityManager(models.Manager):

    def get_queryset(self):
        return super(OpenOpportunityManager, self).get_queryset().filter(fulfilled=False)
...
class Opportunity(TimeStampedModel, TitleSlugDescriptionModel):
    ...
    max_applicants = models.IntegerField(_('Max applicants'), blank=True, null=True)
    ...
    def get_absolute_url(self):
        return (
         'opportunity-detail',
         None, {'project_slug': self.project.slug, 'slug': self.slug})

    def __unicode__(self):
        return ('{0} for {1}').format(self.title, self.project)
    ...
    @property
    def app_count(self):
        return len(VolunteerApplication.objects.filter(opportunity=self))
...
class Volunteer(TimeStampedModel):
    ...
    opportunities_completed = models.ManyToManyField(Opportunity, blank=True, null=True)
...
class VolunteerApplication(TimeStampedModel):
    ...
    status = models.CharField(_('Status'), choices=APP_STATUSES, default='pending', max_length=15)

    def __unicode__(self):
        return ('Application for {0}: {1}').format(self.opportunity, self.user)