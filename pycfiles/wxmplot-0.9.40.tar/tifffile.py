...
class TIFFfile(object):
    ...
    def _fromfile(self):
        ...
        self._fd.seek(0)
        try:
            self.byte_order = {'II': '<', 'MM': '>'}[self._fd.read(2)]
        except KeyError:
    ...
    def series(self):
        ...
        if self.is_ome:
            ...
            series = [
             Record(axes=('').join(dims.get(i[0].strip().upper(), 'O') for i in mmhd if i[1] > 1), shape=tuple(int(i[1]) for i in mmhd if i[1] > 1), pages=self.pages, dtype=numpy.dtype(self.pages[0].dtype))]
        elif self.is_lsm:
            ...
            series = [
             Record(pages=self.pages, shape=shape, axes='O' * len(shape), dtype=numpy.dtype(self.pages[0].dtype))]
        else:
            shapes = []
            pages = {}
    ...
    def __str__(self):
        ...
        result = [
         ...
         '%.2f MB' % (self.fstat[6] / 1048576), {'<': 'little endian', '>': 'big endian'}[self.byte_order]]
        if self.is_bigtiff:
