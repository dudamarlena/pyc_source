...
# Compiled at: 2020-02-12 21:01:00
...
def imsave(filename, data, photometric=None, planarconfig=None, resolution=None, description=None, software='tifffile.py', byteorder=None, bigtiff=False):
    ...
    if not photometric in (None, 'minisblack', 'miniswhite', 'rgb'):
    ...
    elif not planarconfig in (None, 'contig', 'planar'):
        raise AssertionError
    elif not byteorder in (None, '<', '>'):
        raise AssertionError
    else:
        ...
        if not bigtiff:
            ...
            if photometric is None:
                if not data.ndim > 2 or shape[(-3)] in (3, 4) or shape[(-1)] in (3,
                                                                                 4):
                    photometric = 'rgb'
                else:
        ...
        if samplesperpixel == 4:
            ...
            if planarconfig and len(shape) > 2:
                if planarconfig == 'contig':
                    data = data.reshape((-1, 1) + shape[-3:])
                    samplesperpixel = shape[(-1)]
                else:
...
class TIFFpage(object):
    ...
    def _process_tags--- This code section failed: ---

 L. 880         0  LOAD_DEREF               'self'
                2  LOAD_ATTR                tags
                4  STORE_FAST               'tags'

 L. 881         6  SETUP_LOOP          210  'to 210'
                8  LOAD_GLOBAL              TIFF_TAGS
               10  LOAD_METHOD              items
               12  CALL_METHOD_0         0  '0 positional arguments'
               14  GET_ITER         
             16_0  COME_FROM            82  '82'
             16_1  COME_FROM            78  '78'
               16  FOR_ITER            208  'to 208'
               18  UNPACK_SEQUENCE_2     2 
               20  STORE_FAST               'code'
               22  UNPACK_SEQUENCE_5     5 
               24  STORE_FAST               'name'
               26  STORE_FAST               'default'
               28  STORE_FAST               'dtype'
               30  STORE_FAST               'count'
               32  STORE_DEREF              'validate'

 L. 882        34  LOAD_FAST                'name'
               36  LOAD_FAST                'tags'
               38  COMPARE_OP               in
               ...
               44  LOAD_CONST               None
               46  COMPARE_OP               is
               48  POP_JUMP_IF_TRUE     72  'to 72'

 L. 883        50  LOAD_GLOBAL              TIFFtag
               52  LOAD_FAST                'code'
               54  LOAD_FAST                'dtype'
               56  LOAD_FAST                'count'

 L. 884        58  LOAD_FAST                'default'
               60  LOAD_FAST                'name'
               62  LOAD_CONST               ('dtype', 'count', 'value', 'name')
               64  CALL_FUNCTION_KW_5     5  '5 total positional and keyword args'
               66  LOAD_FAST                'tags'
               68  LOAD_FAST                'name'
               70  STORE_SUBSCR     
             72_0  COME_FROM            48  '48'
             72_1  COME_FROM            40  '40'

 L. 885        72  LOAD_FAST                'name'
               74  LOAD_FAST                'tags'
               76  COMPARE_OP               in
               78  POP_JUMP_IF_FALSE    16  'to 16'
               80  LOAD_DEREF               'validate'
               82  POP_JUMP_IF_FALSE    16  'to 16'

 L. 886        84  SETUP_EXCEPT        164  'to 164'

 L. 887        86  LOAD_FAST                'tags'
               88  LOAD_FAST                'name'
               90  BINARY_SUBSCR    
               92  LOAD_ATTR                count
               94  LOAD_CONST               1
 ...
 L. 888       100  LOAD_GLOBAL              setattr
              102  LOAD_DEREF               'self'
              104  LOAD_FAST                'name'
              106  LOAD_DEREF               'validate'
              108  LOAD_FAST                'tags'
              110  LOAD_FAST                'name'
              112  BINARY_SUBSCR    
              114  LOAD_ATTR                value
              116  BINARY_SUBSCR    
              118  CALL_FUNCTION_3       3  '3 positional arguments'
              120  POP_TOP          
              122  JUMP_FORWARD        160  'to 160'
            124_0  COME_FROM            98  '98'

 L. 890       124  LOAD_GLOBAL              setattr
              126  LOAD_DEREF               'self'
              128  LOAD_FAST                'name'
              130  LOAD_GLOBAL              tuple
              132  LOAD_CLOSURE             'validate'
              134  BUILD_TUPLE_1         1 
              136  LOAD_GENEXPR             '<code_object <genexpr>>'
              138  LOAD_STR                 'TIFFpage._process_tags.<locals>.<genexpr>'
              140  MAKE_FUNCTION_8          'closure'

 L. 891       142  LOAD_FAST                'tags'
              144  LOAD_FAST                'name'
              146  BINARY_SUBSCR    
              148  LOAD_ATTR                value
              150  GET_ITER         
            ...
            160_0  COME_FROM           122  '122'
              160  POP_BLOCK        
              162  JUMP_BACK            16  'to 16'
            164_0  COME_FROM_EXCEPT     84  '84'
 ...
 L. 893       178  LOAD_GLOBAL              ValueError
              180  LOAD_STR                 '%s.value (%s) not supported'

 L. 894       182  LOAD_FAST                'name'
              184  LOAD_FAST                'tags'
              186  LOAD_FAST                'name'
              188  BINARY_SUBSCR    
              190  LOAD_ATTR                value
            ...
            204_0  COME_FROM           170  '170'
              204  END_FINALLY      
              206  JUMP_BACK            16  'to 16'
              208  POP_BLOCK        
            210_0  COME_FROM_LOOP        6  '6'

 L. 896       210  LOAD_FAST                'tags'
              212  LOAD_STR                 'bits_per_sample'
              214  BINARY_SUBSCR    
              216  STORE_FAST               'tag'

 L. 897       218  LOAD_FAST                'tag'
              220  LOAD_ATTR                count
              222  LOAD_CONST               1
              224  COMPARE_OP               ==
              226  POP_JUMP_IF_FALSE   238  'to 238'

 L. 898       228  LOAD_FAST                'tag'
              230  LOAD_ATTR                value
              232  LOAD_DEREF               'self'
              234  STORE_ATTR               bits_per_sample
              236  JUMP_FORWARD        296  'to 296'
            238_0  COME_FROM           226  '226'

 L. 900       238  LOAD_FAST                'tag'
              240  LOAD_ATTR                value
              242  LOAD_CONST               None
              244  LOAD_DEREF               'self'
              246  LOAD_ATTR                samples_per_pixel
              248  BUILD_SLICE_2         2 
              250  BINARY_SUBSCR    
              252  STORE_DEREF              'value'

 L. 901       254  LOAD_GLOBAL              any
              256  LOAD_CLOSURE             'value'
              258  BUILD_TUPLE_1         1 
              260  LOAD_GENEXPR             '<code_object <genexpr>>'
              262  LOAD_STR                 'TIFFpage._process_tags.<locals>.<genexpr>'
              264  MAKE_FUNCTION_8          'closure'
              266  LOAD_DEREF               'value'
              268  GET_ITER         
 ...
 L. 902       278  LOAD_DEREF               'value'
              280  LOAD_DEREF               'self'
              282  STORE_ATTR               bits_per_sample
              284  JUMP_FORWARD        296  'to 296'
            286_0  COME_FROM           274  '274'

 L. 904       286  LOAD_DEREF               'value'
              288  LOAD_CONST               0
              290  BINARY_SUBSCR    
              292  LOAD_DEREF               'self'
              294  STORE_ATTR               bits_per_sample
            296_0  COME_FROM           284  '284'
            296_1  COME_FROM           236  '236'

 L. 906       296  LOAD_FAST                'tags'
              298  LOAD_STR                 'sample_format'
              300  BINARY_SUBSCR    
              302  STORE_FAST               'tag'

 L. 907       304  LOAD_FAST                'tag'
              306  LOAD_ATTR                count
              308  LOAD_CONST               1
              310  COMPARE_OP               ==
          312_314  POP_JUMP_IF_FALSE   330  'to 330'

 L. 908       316  LOAD_GLOBAL              TIFF_SAMPLE_FORMATS
              318  LOAD_FAST                'tag'
              320  LOAD_ATTR                value
              322  BINARY_SUBSCR    
              324  LOAD_DEREF               'self'
              326  STORE_ATTR               sample_format
              328  JUMP_FORWARD        402  'to 402'
            330_0  COME_FROM           312  '312'

 L. 910       330  LOAD_FAST                'tag'
              332  LOAD_ATTR                value
              334  LOAD_CONST               None
              336  LOAD_DEREF               'self'
              338  LOAD_ATTR                samples_per_pixel
              340  BUILD_SLICE_2         2 
              342  BINARY_SUBSCR    
              344  STORE_DEREF              'value'

 L. 911       346  LOAD_GLOBAL              any
              348  LOAD_CLOSURE             'value'
              350  BUILD_TUPLE_1         1 
              352  LOAD_GENEXPR             '<code_object <genexpr>>'
              354  LOAD_STR                 'TIFFpage._process_tags.<locals>.<genexpr>'
              356  MAKE_FUNCTION_8          'closure'
              358  LOAD_DEREF               'value'
              360  GET_ITER         
 ...
 L. 912       370  LOAD_LISTCOMP            '<code_object <listcomp>>'
              372  LOAD_STR                 'TIFFpage._process_tags.<locals>.<listcomp>'
              374  MAKE_FUNCTION_0          'Neither defaults, keyword-only args, annotations, nor closures'
              376  LOAD_DEREF               'value'
              378  GET_ITER         
              380  CALL_FUNCTION_1       1  '1 positional argument'
              382  LOAD_DEREF               'self'
              384  STORE_ATTR               sample_format
              386  JUMP_FORWARD        402  'to 402'
            388_0  COME_FROM           366  '366'

 L. 914       388  LOAD_GLOBAL              TIFF_SAMPLE_FORMATS
              390  LOAD_DEREF               'value'
              392  LOAD_CONST               0
              394  BINARY_SUBSCR    
              396  BINARY_SUBSCR    
              398  LOAD_DEREF               'self'
              400  STORE_ATTR               sample_format
            402_0  COME_FROM           386  '386'
            402_1  COME_FROM           328  '328'

 L. 916       402  LOAD_GLOBAL              int
              404  LOAD_GLOBAL              math
              406  LOAD_METHOD              floor
              408  LOAD_GLOBAL              float

 L. 917       410  LOAD_DEREF               'self'
              412  LOAD_ATTR                image_length
              414  LOAD_DEREF               'self'
              416  LOAD_ATTR                rows_per_strip
              418  BINARY_ADD       
              420  LOAD_CONST               1
              422  BINARY_SUBTRACT  
              424  CALL_FUNCTION_1       1  '1 positional argument'
              426  LOAD_DEREF               'self'
              428  LOAD_ATTR                rows_per_strip
              430  BINARY_TRUE_DIVIDE
              432  CALL_METHOD_1         1  '1 positional argument'
              434  CALL_FUNCTION_1       1  '1 positional argument'
              436  LOAD_DEREF               'self'
              438  STORE_ATTR               strips_per_image

 L. 919       440  LOAD_DEREF               'self'
              442  LOAD_ATTR                sample_format
              444  LOAD_DEREF               'self'
              446  LOAD_ATTR                bits_per_sample
              448  BUILD_TUPLE_2         2 
              450  STORE_FAST               'key'

 L. 920       452  LOAD_GLOBAL              TIFF_SAMPLE_DTYPES
              454  LOAD_METHOD              get
              456  LOAD_FAST                'key'
              458  LOAD_CONST               None
              ...
              462  DUP_TOP          
              464  LOAD_DEREF               'self'
              466  STORE_ATTR               dtype
              468  LOAD_DEREF               'self'
              470  STORE_ATTR               _dtype

 L. 922       472  LOAD_DEREF               'self'
              474  LOAD_ATTR                is_stk
          476_478  POP_JUMP_IF_FALSE   664  'to 664'

 L. 923       480  LOAD_FAST                'tags'
              482  LOAD_STR                 'mm_uic2'
              484  BINARY_SUBSCR    
              486  LOAD_ATTR                count
              488  STORE_FAST               'planes'

 L. 925       490  LOAD_GLOBAL              Record
              492  LOAD_FAST                'tags'
              494  LOAD_STR                 'mm_uic2'
              496  BINARY_SUBSCR    
              498  LOAD_ATTR                value
              500  CALL_FUNCTION_1       1  '1 positional argument'
              502  LOAD_DEREF               'self'
              504  STORE_ATTR               mm_uic_tags

 L. 926       506  SETUP_LOOP          550  'to 550'
              508  LOAD_CONST               ('mm_uic3', 'mm_uic4', 'mm_uic1')
              510  GET_ITER         
            512_0  COME_FROM           522  '522'
              512  FOR_ITER            548  'to 548'
              514  STORE_FAST               'key'

 L. 927       516  LOAD_FAST                'key'
              518  LOAD_FAST                'tags'
              520  COMPARE_OP               in
          522_524  POP_JUMP_IF_FALSE   512  'to 512'

 L. 928       526  LOAD_DEREF               'self'
              528  LOAD_ATTR                mm_uic_tags
              530  LOAD_METHOD              update
              532  LOAD_FAST                'tags'
              534  LOAD_FAST                'key'
              536  BINARY_SUBSCR    
              538  LOAD_ATTR                value
              ...
              542  POP_TOP          
          544_546  JUMP_BACK           512  'to 512'
              548  POP_BLOCK        
            550_0  COME_FROM_LOOP      506  '506'

 L. 929       550  LOAD_DEREF               'self'
              552  LOAD_ATTR                planar_configuration
              554  LOAD_STR                 'contig'
              556  COMPARE_OP               ==
          558_560  POP_JUMP_IF_FALSE   616  'to 616'

 L. 930       562  LOAD_FAST                'planes'
              564  LOAD_CONST               1
              566  LOAD_DEREF               'self'
              568  LOAD_ATTR                image_length

 L. 931       570  LOAD_DEREF               'self'
              572  LOAD_ATTR                image_width
              574  LOAD_DEREF               'self'
              576  LOAD_ATTR                samples_per_pixel
              578  BUILD_TUPLE_5         5 
              580  LOAD_DEREF               'self'
              582  STORE_ATTR               _shape

 L. 932       584  LOAD_GLOBAL              tuple
              586  LOAD_CLOSURE             'self'
              588  BUILD_TUPLE_1         1 
              590  LOAD_GENEXPR             '<code_object <genexpr>>'
              592  LOAD_STR                 'TIFFpage._process_tags.<locals>.<genexpr>'
              594  MAKE_FUNCTION_8          'closure'
              596  LOAD_CONST               (0, 2, 3, 4)
              598  GET_ITER         
              600  CALL_FUNCTION_1       1  '1 positional argument'
              602  CALL_FUNCTION_1       1  '1 positional argument'
              604  LOAD_DEREF               'self'
              606  STORE_ATTR               shape

 L. 933       608  LOAD_STR                 'PYXS'
              610  LOAD_DEREF               'self'
              612  STORE_ATTR               axes
              614  JUMP_FORWARD       1152  'to 1152'
            616_0  COME_FROM           558  '558'

 L. 935       616  LOAD_FAST                'planes'
              618  LOAD_DEREF               'self'
              620  LOAD_ATTR                samples_per_pixel

 L. 936       622  LOAD_DEREF               'self'
              624  LOAD_ATTR                image_length
              626  LOAD_DEREF               'self'
              628  LOAD_ATTR                image_width
              630  LOAD_CONST               1
              632  BUILD_TUPLE_5         5 
              634  LOAD_DEREF               'self'
              636  STORE_ATTR               _shape

 L. 937       638  LOAD_DEREF               'self'
              640  LOAD_ATTR                _shape
              642  LOAD_CONST               None
              644  LOAD_CONST               4
              646  BUILD_SLICE_2         2 
              648  BINARY_SUBSCR    
              650  LOAD_DEREF               'self'
              652  STORE_ATTR               shape

 L. 938       654  LOAD_STR                 'PSYX'
              656  LOAD_DEREF               'self'
              658  STORE_ATTR               axes
          660_662  JUMP_FORWARD       1152  'to 1152'
            664_0  COME_FROM           476  '476'

 L. 939       664  LOAD_DEREF               'self'
              666  LOAD_ATTR                is_palette
          668_670  POP_JUMP_IF_FALSE   852  'to 852'

 L. 940       672  LOAD_DEREF               'self'
              674  LOAD_ATTR                tags
              676  LOAD_STR                 'color_map'
              678  BINARY_SUBSCR    
              680  LOAD_ATTR                dtype
              682  LOAD_CONST               1
              684  BINARY_SUBSCR    
              686  LOAD_DEREF               'self'
              688  STORE_ATTR               dtype

 L. 941       690  LOAD_GLOBAL              numpy
              692  LOAD_METHOD              array
              694  LOAD_DEREF               'self'
              696  LOAD_ATTR                color_map
              698  LOAD_DEREF               'self'
              700  LOAD_ATTR                dtype
              702  CALL_METHOD_2         2  '2 positional arguments'
              704  LOAD_DEREF               'self'
              706  STORE_ATTR               color_map

 L. 942       708  LOAD_DEREF               'self'
              710  LOAD_ATTR                color_map
              712  LOAD_METHOD              max
              714  CALL_METHOD_0         0  '0 positional arguments'
              716  STORE_FAST               'dmax'

 L. 943       718  LOAD_FAST                'dmax'
              720  LOAD_CONST               256
              722  COMPARE_OP               <
          724_726  POP_JUMP_IF_FALSE   752  'to 752'

 L. 944       728  LOAD_GLOBAL              numpy
              730  LOAD_ATTR                uint8
              732  LOAD_DEREF               'self'
              734  STORE_ATTR               dtype

 L. 945       736  LOAD_DEREF               'self'
              738  LOAD_ATTR                color_map
              740  LOAD_METHOD              astype
              742  LOAD_DEREF               'self'
              744  LOAD_ATTR                dtype
              746  CALL_METHOD_1         1  '1 positional argument'
              748  LOAD_DEREF               'self'
              750  STORE_ATTR               color_map
            752_0  COME_FROM           724  '724'

 L. 950       752  LOAD_CONST               (3, -1)
              754  LOAD_DEREF               'self'
              756  LOAD_ATTR                color_map
              758  STORE_ATTR               shape

 L. 951       760  LOAD_CONST               1
              762  LOAD_CONST               1
              764  LOAD_DEREF               'self'
              766  LOAD_ATTR                image_length
              768  LOAD_DEREF               'self'
              770  LOAD_ATTR                image_width
              772  LOAD_CONST               1
              774  BUILD_TUPLE_5         5 
              776  LOAD_DEREF               'self'
              778  STORE_ATTR               _shape

 L. 952       780  LOAD_DEREF               'self'
              782  LOAD_ATTR                color_map
              784  LOAD_ATTR                shape
              786  LOAD_CONST               1
              788  BINARY_SUBSCR    
              790  LOAD_CONST               2
              792  LOAD_DEREF               'self'
              794  LOAD_ATTR                bits_per_sample
              796  BINARY_POWER     
              798  COMPARE_OP               >=
          800_802  POP_JUMP_IF_FALSE   828  'to 828'

 L. 953       804  LOAD_CONST               3
              806  LOAD_DEREF               'self'
              808  LOAD_ATTR                image_length
              810  LOAD_DEREF               'self'
              812  LOAD_ATTR                image_width
              814  BUILD_TUPLE_3         3 
              816  LOAD_DEREF               'self'
              818  STORE_ATTR               shape

 L. 954       820  LOAD_STR                 'SYX'
              822  LOAD_DEREF               'self'
              824  STORE_ATTR               axes
              826  JUMP_FORWARD       1152  'to 1152'
            828_0  COME_FROM           800  '800'

 L. 957       828  LOAD_DEREF               'self'
              830  LOAD_ATTR                image_length
              832  LOAD_DEREF               'self'
              834  LOAD_ATTR                image_width
              836  BUILD_TUPLE_2         2 
              838  LOAD_DEREF               'self'
              840  STORE_ATTR               shape

 L. 958       842  LOAD_STR                 'YX'
              844  LOAD_DEREF               'self'
              846  STORE_ATTR               axes
          848_850  JUMP_FORWARD       1152  'to 1152'
            852_0  COME_FROM           668  '668'

 L. 959       852  LOAD_DEREF               'self'
              854  LOAD_ATTR                is_rgb
          856_858  POP_JUMP_IF_TRUE    872  'to 872'
              860  LOAD_DEREF               'self'
              862  LOAD_ATTR                samples_per_pixel
              864  LOAD_CONST               1
              866  COMPARE_OP               >
          868_870  POP_JUMP_IF_FALSE  1110  'to 1110'
            872_0  COME_FROM           856  '856'

 L. 960       872  LOAD_DEREF               'self'
              874  LOAD_ATTR                planar_configuration
              876  LOAD_STR                 'contig'
              878  COMPARE_OP               ==
          880_882  POP_JUMP_IF_FALSE   932  'to 932'

 L. 961       884  LOAD_CONST               1
              886  LOAD_CONST               1
              888  LOAD_DEREF               'self'
              890  LOAD_ATTR                image_length
              892  LOAD_DEREF               'self'
              894  LOAD_ATTR                image_width

 L. 962       896  LOAD_DEREF               'self'
              898  LOAD_ATTR                samples_per_pixel
              900  BUILD_TUPLE_5         5 
              902  LOAD_DEREF               'self'
              904  STORE_ATTR               _shape

 L. 963       906  LOAD_DEREF               'self'
              908  LOAD_ATTR                image_length
              910  LOAD_DEREF               'self'
              912  LOAD_ATTR                image_width

 L. 964       914  LOAD_DEREF               'self'
              916  LOAD_ATTR                samples_per_pixel
              918  BUILD_TUPLE_3         3 
              920  LOAD_DEREF               'self'
              922  STORE_ATTR               shape

 L. 965       924  LOAD_STR                 'YXS'
              926  LOAD_DEREF               'self'
              928  STORE_ATTR               axes
              930  JUMP_FORWARD        976  'to 976'
            932_0  COME_FROM           880  '880'

 L. 967       932  LOAD_CONST               1
              934  LOAD_DEREF               'self'
              936  LOAD_ATTR                samples_per_pixel
              938  LOAD_DEREF               'self'
              940  LOAD_ATTR                image_length

 L. 968       942  LOAD_DEREF               'self'
              944  LOAD_ATTR                image_width
              946  LOAD_CONST               1
              948  BUILD_TUPLE_5         5 
              950  LOAD_DEREF               'self'
              952  STORE_ATTR               _shape

 L. 969       954  LOAD_DEREF               'self'
              956  LOAD_ATTR                _shape
              958  LOAD_CONST               1
              960  LOAD_CONST               -1
              962  BUILD_SLICE_2         2 
              964  BINARY_SUBSCR    
              966  LOAD_DEREF               'self'
              968  STORE_ATTR               shape

 L. 970       970  LOAD_STR                 'SYX'
              972  LOAD_DEREF               'self'
              974  STORE_ATTR               axes
            976_0  COME_FROM           930  '930'

 L. 971       976  LOAD_DEREF               'self'
              978  LOAD_ATTR                is_rgb
          980_982  POP_JUMP_IF_FALSE  1152  'to 1152'
              984  LOAD_STR                 'extra_samples'
              986  LOAD_DEREF               'self'
              988  LOAD_ATTR                tags
              990  COMPARE_OP               in
          992_994  POP_JUMP_IF_FALSE  1152  'to 1152'

 L. 972       996  LOAD_DEREF               'self'
              998  LOAD_ATTR                extra_samples
             1000  STORE_FAST               'extra_samples'

 L. 973      1002  LOAD_DEREF               'self'
             1004  LOAD_ATTR                tags
             1006  LOAD_STR                 'extra_samples'
             1008  BINARY_SUBSCR    
             1010  LOAD_ATTR                count
             1012  LOAD_CONST               1
             1014  COMPARE_OP               ==
         1016_1018  POP_JUMP_IF_FALSE  1026  'to 1026'

 L. 974      1020  LOAD_FAST                'extra_samples'
             1022  BUILD_TUPLE_1         1 
             1024  STORE_FAST               'extra_samples'
           1026_0  COME_FROM          1016  '1016'

 L. 975      1026  SETUP_LOOP         1152  'to 1152'
             1028  LOAD_FAST                'extra_samples'
             1030  GET_ITER         
           1032_0  COME_FROM          1042  '1042'
             1032  FOR_ITER           1106  'to 1106'
             1034  STORE_FAST               'exs'

 L. 976      1036  LOAD_FAST                'exs'
             1038  LOAD_CONST               ('unassalpha', 'assocalpha')
             1040  COMPARE_OP               in
         1042_1044  POP_JUMP_IF_FALSE  1032  'to 1032'

 L. 977      1046  LOAD_DEREF               'self'
             1048  LOAD_ATTR                planar_configuration
             1050  LOAD_STR                 'contig'
             1052  COMPARE_OP               ==
         1054_1056  POP_JUMP_IF_FALSE  1080  'to 1080'

 L. 978      1058  LOAD_DEREF               'self'
             1060  LOAD_ATTR                shape
             1062  LOAD_CONST               None
             1064  LOAD_CONST               2
             1066  BUILD_SLICE_2         2 
             1068  BINARY_SUBSCR    
             1070  LOAD_CONST               (4,)
             1072  BINARY_ADD       
             1074  LOAD_DEREF               'self'
             1076  STORE_ATTR               shape
             1078  JUMP_FORWARD       1100  'to 1100'
           1080_0  COME_FROM          1054  '1054'

 L. 980      1080  LOAD_CONST               (4,)
             1082  LOAD_DEREF               'self'
             1084  LOAD_ATTR                shape
             1086  LOAD_CONST               1
             ...
             1098  STORE_ATTR               shape
           1100_0  COME_FROM          1078  '1078'

 L. 981      1100  BREAK_LOOP       
         1102_1104  JUMP_BACK          1032  'to 1032'
             1106  POP_BLOCK        
             1108  JUMP_FORWARD       1152  'to 1152'
           1110_0  COME_FROM           868  '868'

 L. 983      1110  LOAD_CONST               1
             1112  LOAD_CONST               1
             1114  LOAD_DEREF               'self'
             1116  LOAD_ATTR                image_length
             1118  LOAD_DEREF               'self'
             1120  LOAD_ATTR                image_width
             1122  LOAD_CONST               1
             1124  BUILD_TUPLE_5         5 
             1126  LOAD_DEREF               'self'
           1128_0  COME_FROM           826  '826'
             1128  STORE_ATTR               _shape

 L. 984      1130  LOAD_DEREF               'self'
             1132  LOAD_ATTR                _shape
             1134  LOAD_CONST               2
             1136  LOAD_CONST               4
             1138  BUILD_SLICE_2         2 
             1140  BINARY_SUBSCR    
             1142  LOAD_DEREF               'self'
             1144  STORE_ATTR               shape

 L. 985      1146  LOAD_STR                 'YX'
             1148  LOAD_DEREF               'self'
             1150  STORE_ATTR               axes
           1152_0  COME_FROM          1108  '1108'
           1152_1  COME_FROM_LOOP     1026  '1026'
           1152_2  COME_FROM           992  '992'
           1152_3  COME_FROM           980  '980'
           1152_4  COME_FROM           848  '848'
           1152_5  COME_FROM           660  '660'

 L. 987      1152  LOAD_DEREF               'self'
             1154  LOAD_ATTR                compression
         1156_1158  POP_JUMP_IF_TRUE   1194  'to 1194'
             1160  LOAD_STR                 'strip_byte_counts'
             1162  LOAD_FAST                'tags'
             1164  COMPARE_OP               not-in
         1166_1168  POP_JUMP_IF_FALSE  1194  'to 1194'

 L. 988      1170  LOAD_GLOBAL              numpy
             1172  LOAD_METHOD              prod
             1174  LOAD_DEREF               'self'
             1176  LOAD_ATTR                shape
             1178  CALL_METHOD_1         1  '1 positional argument'

 L. 989      1180  LOAD_DEREF               'self'
             1182  LOAD_ATTR                bits_per_sample
             1184  LOAD_CONST               8
             1186  BINARY_FLOOR_DIVIDE
             1188  BINARY_MULTIPLY  
             1190  LOAD_DEREF               'self'
             1192  STORE_ATTR               strip_byte_counts
           1194_0  COME_FROM          1166  '1166'
...
Parse error at or near `COME_FROM' instruction at offset 1128_0
    ...
    def asarray(self, squeeze=True, colormapped=True, rgbonly=True):
        ...
        if any((o < 2 for o in offsets)):
        ...
        elif not (self.is_tiled or self.is_stk or self.compression):
            if not bits_per_sample in (8, 16, 32, 64) or all((offsets[i] == offsets[(i + 1)] - byte_counts[i] for i in range(len(offsets) - 1))):
                fd.seek(offsets[0])
            ...
            if self.predictor == 'horizontal':
                if not self.parent.is_lsm or self.compression:
...
def decodelzw(encoded):
    ...
    switchbitch = {255:(
      9, 23, int('11111111100000000000000000000000', 2)), 
     511:(
      10, 22, int('11111111110000000000000000000000', 2)), 
     1023:(
      11, 21, int('11111111111000000000000000000000', 2)), 
     2047:(
    ...
    if photometric == 'palette':
        ...
        if data.dtype.kind in 'ui':
            if not isrgb or bitspersample is None:
                ...
                if not isinstance(bitspersample, int):
...
def main(argv=None):
    ...
    if float(sys.version[0:3]) < 2.6:
        ...
        if argv is None:
            argv = sys.argv
        import re, optparse
        search_doc = lambda r, d:         if __doc__:
re.search(r, __doc__).group(1) # Avoid dead code: d
        ...
        images = [
         (
          None, tif[(0 if settings.page < 0 else settings.page)])]
        settings.noplot or print('Reading image data... ', end=' ')
