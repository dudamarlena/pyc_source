...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\chatette_qiu\parsing\parser_utils.py
# Compiled at: 2019-03-27 03:00:31
# Size of source mod 2**32: 37884 bytes
"""
Module `chatette_qiu.parsing.parser_utils`
...
INCLUDE_FILE_SYM = '|'
RESERVED_VARIATION_NAMES = [
 'all-variations-aggregation', 'rules',
 'nb-gen-asked', 'arg']
PATTERN_COMMENT_DEPRECATED = re.compile('(?<!\\\\)' + COMMENT_SYM_DEPRECATED)
PATTERN_COMMENT = re.compile('(?<!\\\\)' + COMMENT_MARKER)
PATTERN_NB_TRAIN_EX_KEY = re.compile("'?train(ing)?'?")
PATTERN_NB_TEST_EX_KEY = re.compile("'?test(ing)?'?")

class UnitType(Enum):
    __doc__ = 'Enumeration of all possible types of unit declarations.'
    alias = 1
    slot = 2
    intent = 3
...
def add_escapement_back_in_group(text):
    ...
    escaped_text = ''
    for c in text:
        if c == ESCAPE_SYM or is_boundary_sym(c) or is_group_modifier_sym(c) or is_comment_sym(c):
            escaped_text += ESCAPE_SYM + c
        else:
            escaped_text += c
...
def add_escapement_back_in_unit_ref(unit_name):
    ...
    escaped_text = ''
    for c in unit_name:
        if c == ESCAPE_SYM or is_boundary_sym(c) or is_comment_sym(c) or is_unit_ref_modifier_sym(c):
            escaped_text += ESCAPE_SYM + c
        else:
            escaped_text += c
...
def add_escapement_back_in_unit_decl(unit_name):
    ...
    for c in unit_name:
        print(c)
        if c == ESCAPE_SYM or is_boundary_sym(c) or is_unit_decl_modifier_sym(c) or is_arg_sym(c) or is_comment_sym(c):
            escaped_text += ESCAPE_SYM + c
        else:
            escaped_text += c
...
def should_be_escaped_char(text):
    ...
    """
    return is_special_sym(text) or text == COMMENT_MARKER or text == COMMENT_SYM_DEPRECATED or text == ALT_SLOT_VALUE_NAME_SYM


def is_special_sym(text):
    """Returns `True` if `text` is a string made of only 1 special character."""
    return text == ALIAS_SYM or text == SLOT_SYM or text == INTENT_SYM or text == UNIT_OPEN_SYM or text == UNIT_CLOSE_SYM or text == VARIATION_SYM or text == RAND_GEN_SYM or text == PERCENT_GEN_SYM or text == CASE_GEN_SYM or text == ARG_SYM or text == CHOICE_OPEN_SYM or text == CHOICE_CLOSE_SYM
...
def is_start_unit_sym(char):
    ...
    return char == UNIT_OPEN_SYM or char == ALIAS_SYM or char == SLOT_SYM or char == INTENT_SYM
    ...
    if starting_index >= length:
        ...
        while end_index < length and nb_closing_brackets_expected > 0:
            if tokens[end_index] == UNIT_OPEN_SYM:
                ...
                if tokens[end_index] == UNIT_CLOSE_SYM:
                    nb_closing_brackets_expected -= 1
            end_index += 1
    ...
    if starting_index >= length:
        ...
        while end_index < length and nb_closing_brackets_expected > 0:
            if tokens[end_index] == ANNOTATION_OPEN_SYM:
                ...
                if tokens[end_index] == ANNOTATION_CLOSE_SYM:
                    nb_closing_brackets_expected -= 1
            end_index += 1
...
def check_declaration_validity(tokens_unit_inside):
    ...
    if casegen_count > 1:
        raise SyntaxError('There can be only one case generation modifier ' + 'in a unit declaration.')
    if casegen_count == 1:
        if tokens_unit_inside.index(CASE_GEN_SYM) != 0:
            raise SyntaxError('Case generation modifiers have to be at the start ' + 'of a unit declaration.')
    if casegen_count == 0:
        if is_special_sym(tokens_unit_inside[0]):
            raise SyntaxError('Unit declarations must be named.')
    if casegen_count == 1 and len(tokens_unit_inside) <= 1:
        raise SyntaxError('Unit declarations must be named.')
    else:
        if casegen_count == 1:
            if is_special_sym(tokens_unit_inside[1]):
                raise SyntaxError('Unit declarations must be named.')
    variation_count = tokens_unit_inside.count(VARIATION_SYM)
    if variation_count > 1:
        raise SyntaxError('There can be only one variation modifier ' + 'in a unit declaration.')
    if variation_count == 1:
        variation_name_index = tokens_unit_inside.index(VARIATION_SYM) + 1
        if variation_name_index >= len(tokens_unit_inside) or is_special_sym(tokens_unit_inside[variation_name_index]):
        ...
        if variation_name in RESERVED_VARIATION_NAMES:
            raise SyntaxError('The following variation names are reserved: ' + str(RESERVED_VARIATION_NAMES) + ". Please don't " + 'use them.')
    argument_count = tokens_unit_inside.count(ARG_SYM)
    if argument_count > 1:
        raise SyntaxError('There can be only one argument modifier ' + 'per unit declaration.')
    if argument_count == 1:
        argument_name_index = tokens_unit_inside.index(ARG_SYM) + 1
        if argument_name_index >= len(tokens_unit_inside) or is_special_sym(tokens_unit_inside[argument_name_index]):
            raise SyntaxError('Arguments must be named.')
    randgen_count = tokens_unit_inside.count(RAND_GEN_SYM)
    if randgen_count > 0:
        raise SyntaxError('Unit declarations cannot take a random generation ' + 'modifier.')
    percentgen_count = tokens_unit_inside.count(PERCENT_GEN_SYM)
    if percentgen_count > 0:
        raise SyntaxError('Unit declarations cannot take a percentage for ' + 'the random generation modifier.')
...
def check_reference_validity(tokens_unit_inside):
    ...
    casegen_count = tokens_unit_inside.count(CASE_GEN_SYM)
    if casegen_count > 1:
        raise SyntaxError('There can be only one case generation modifier ' + 'in a unit reference.')
    if casegen_count == 1:
        if tokens_unit_inside.index(CASE_GEN_SYM) != 0:
            raise SyntaxError('Case generation modifiers have to be at the start ' + 'of a unit reference.')
    if casegen_count == 0:
        if is_special_sym(tokens_unit_inside[0]):
    ...
    variation_count = tokens_unit_inside.count(VARIATION_SYM)
    if variation_count > 1:
        raise SyntaxError('There can be only one variation modifier ' + 'in a unit reference.')
    if variation_count == 1:
        variation_name_index = tokens_unit_inside.index(VARIATION_SYM) + 1
        if variation_name_index >= len(tokens_unit_inside) or is_special_sym(tokens_unit_inside[variation_name_index]):
            raise SyntaxError('Variations must be named.')
    ...
    argument_count = tokens_unit_inside.count(ARG_SYM)
    if argument_count > 1:
        raise SyntaxError('There can be only one argument modifier ' + 'per unit reference.')
    randgen_count = tokens_unit_inside.count(RAND_GEN_SYM)
    if randgen_count > 1:
        raise SyntaxError('There can be only one random generation modifier ' + 'per unit reference.')
    percentgen_count = tokens_unit_inside.count(PERCENT_GEN_SYM)
    if percentgen_count > 1:
        raise SyntaxError('There can be only one percentage for generation ' + 'modifier per unit reference.')
    if percentgen_count == 1:
        if randgen_count == 0:
            raise SyntaxError('There cannot be a percentage for generation ' + 'modifier if there is no random generation modifier ' + "(did you mean to escape '" + PERCENT_GEN_SYM + "'?)")
    if percentgen_count == 1:
        ...
        if index_randgen > index_percentgen:
            raise SyntaxError('A percentage for generation modifier must ' + 'always be right after the random generation ' + 'modifier.')
        if index_percentgen == len(tokens_unit_inside) - 1:
            raise SyntaxError('No percentage found after the special symbol ' + 'for percentage modifier.')
        try:
            percentgen = int(tokens_unit_inside[(index_percentgen + 1)])
        except ValueError:
            raise SyntaxError('Percentage for generation modifiers need to be ' + 'an integer.')

        if percentgen < 0 or percentgen > 100:
            raise SyntaxError('Percentage for generation modifiers need to be ' + 'between 0 and 100.')
...
def check_choice_validity(tokens_choice_inside):
    ...
    if len(tokens_choice_inside) > 0:
        if tokens_choice_inside[(-1)] == CHOICE_SEP:
            raise SyntaxError('Choice cannot end with a choice separator. ' + 'Did you forget to escape the last character?')
        if len(tokens_choice_inside) > 1:
            if tokens_choice_inside[(-1)] == RAND_GEN_SYM:
                if tokens_choice_inside[(-2)] == CHOICE_SEP:
                    raise SyntaxError('Choice ends with an empty choice item. ' + 'Did you forget to escape the choice separator?')
...
def check_word_group_validity(tokens_word_group_inside):
    ...
    if casegen_count > 1:
        raise SyntaxError('There can be only one case generation modifier ' + 'in a word group.')
    if casegen_count == 1:
        if tokens_word_group_inside.index(CASE_GEN_SYM) != 0:
            raise SyntaxError('Case generation modifiers have to be at the start ' + 'of a word group.')
    ...
    if randgen_count > 1:
        raise SyntaxError('There can be only one random generation modifier ' + 'per word group.')
    percentgen_count = tokens_word_group_inside.count(PERCENT_GEN_SYM)
    if percentgen_count > 1:
        raise SyntaxError('There can be only one percentage for generation ' + 'modifier per word group.')
    if percentgen_count == 1:
        if randgen_count == 0:
            raise SyntaxError('There cannot be a percentage for generation ' + 'modifier if there is no random generation modifier ' + "(did you mean to escape '" + PERCENT_GEN_SYM + "'?)")
    if percentgen_count == 1:
...
def find_name(tokens_inside_unit):
    ...
    if tokens_inside_unit[0] == CASE_GEN_SYM:
        start_index = 1
    name = ''
    while start_index < len(tokens_inside_unit) and not is_special_sym(tokens_inside_unit[start_index]):
...
def find_modifiers_decl(tokens_inside_decl):
    ...
    while i < len(tokens_inside_decl):
        if tokens_inside_decl[i] == VARIATION_SYM:
            ...
            if tokens_inside_decl[i] == ARG_SYM:
                modifiers.argument_name = ''
                expecting_variation = False
                expecting_argument = True
            else:
                if expecting_variation:
...
def find_modifiers_reference(tokens_inside_reference):
    ...
    while i < len(tokens_inside_reference):
        if tokens_inside_reference[i] == RAND_GEN_SYM:
            ...
            if tokens_inside_reference[i] == PERCENT_GEN_SYM:
                expecting_randgen_name = False
                expecting_percentgen = True
                expecting_variation = False
                expecting_argument = False
            else:
                if tokens_inside_reference[i] == VARIATION_SYM:
                    modifiers.variation_name = ''
                    expecting_randgen_name = False
                    expecting_percentgen = False
                    expecting_variation = True
                    ...
                    if tokens_inside_reference[i] == ARG_SYM:
                        ...
                        if expecting_randgen_name:
                            modifiers.randgen_name += tokens_inside_reference[i]
                        else:
                            if expecting_percentgen:
...
def find_modifiers_word_group(tokens_inside_word_group):
    ...
    while i < len(tokens_inside_word_group):
        if tokens_inside_word_group[i] == RAND_GEN_SYM:
            ...
            if tokens_inside_word_group[i] == PERCENT_GEN_SYM:
                expecting_percentgen = True
                expecting_randgen_name = False
            else:
                if expecting_randgen_name:
    ...
    if len(annotation_interior) == 0:
        ...
        if len(annotation_interior) == 1:
            ...
            for token in annotation_interior:
                if token not in (ANNOTATION_ASSIGNMENT_SYM, ANNOTATION_SEP) and not token.isspace():
                    if PATTERN_NB_TRAIN_EX_KEY.match(token):
                        expecting_train = True
                    else:
                        if PATTERN_NB_TEST_EX_KEY.match(token):
                            expecting_test = True
                        else:
                            if expecting_train:
                                nb_train = token
...
def find_alt_slot_and_index(slot_rule_tokens):
    ...
    try:
        ...
        if index + 1 < len(slot_rule_tokens):
            ...
            return (
             index, remove_escapement(alt_slot_val))
...
def next_choice_tokens(choice_interior_tokens):
    ...
    for i, token in enumerate(choice_interior_tokens):
        if token == CASE_GEN_SYM:
            ...
            if token == RAND_GEN_SYM:
                if i == len(choice_interior_tokens) - 1:
                    break
                else:
    ...
    for token in tokens:
        if reading_sub_rule:
            if token == stop_with_char:
                ...
                current_sub_rule.append(token)
        elif is_start_unit_sym(token):
            current_sub_rule.append(token)
            reading_sub_rule = True
            stop_with_char = UNIT_CLOSE_SYM
        else:
            if token == UNIT_OPEN_SYM:
                current_sub_rule.append(token)
                reading_sub_rule = True
                stop_with_char = UNIT_CLOSE_SYM
            else:
                if token == CHOICE_OPEN_SYM:
                    current_sub_rule.append(token)
                    reading_sub_rule = True
