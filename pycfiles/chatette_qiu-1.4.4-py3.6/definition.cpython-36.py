...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\chatette_qiu\units\slot\definition.py
# Compiled at: 2019-03-27 03:00:31
# Size of source mod 2**32: 12043 bytes
from copy import deepcopy
from chatette_qiu.units import Example, UnitDefinition, randomly_change_case, ENTITY_MARKER, with_leading_lower, with_leading_upper
from chatette_qiu.utils import choose
...
class SlotDefinition(UnitDefinition):
    ...
    def generate_random(self, variation_name=None, arg_value=None):
        ...
        if arg_value is not None:
            if arg_value not in self.arg_values_encountered:
            ...
            if variation_name is None:
                ...
                if variation_name not in self.variations:
                    raise SyntaxError("Couldn't find a variation named '" + variation_name + "' for " + self.type + " '" + self.name + "'")
            chosen_rule = choose(self.variations[variation_name])
        if chosen_rule is None:
            return Example()
        else:
            if len(chosen_rule) <= 0:
                raise ValueError('Tried to generate an entity using an empty rule ' + "for slot named '" + self.name + "'")
            else:
                ...
                if self.modifiers.casegen:
                    if self.can_have_casegen():
                        generated_example.text = randomly_change_case(generated_example.text)
                generated_example.text = ENTITY_MARKER + self._replace_arg(generated_example.text, arg_value).strip()
                slot_value = chosen_rule[0].name
                slot_value = isinstance(chosen_rule[0], DummySlotValRuleContent) or generated_example.text[len(ENTITY_MARKER):]
            slot_value = self._replace_arg(slot_value, arg_value)
            generated_example.entities.append({'slot-name':self.name, 
        ...
        if arg_value is not None:
            ...
            if variation_name is not None:
                if variation_name in self.variations:
                    ...
                    raise SyntaxError("Couldn't find variation '" + str(variation_name) + "' for slot '" + str(self.name) + "'")
            if relevant_rules or variation_name is None:
                raise SyntaxError('No rules could be found for ' + self.type + " '" + self.name + "'")
        else:
            raise SyntaxError('No rules could be found for ' + self.type + " '" + self.name + "' (variation: '" + variation_name + "'")
        for rule in relevant_rules:
            examples_from_current_rule = []
            for sub_unit_rule in rule:
                sub_unit_possibilities = sub_unit_rule.generate_all()
                if len(examples_from_current_rule) <= 0:
                    ...
                    for ex in examples_from_current_rule:
                        for possibility in sub_unit_possibilities:
                            tmp_buffer.append(Example(ex.text + possibility.text, ex.entities + possibility.entities))
                ...
                if self.modifiers.argument_name is not None:
                    for ex in examples_from_current_rule:
                        ...
                        for entity in ex.entities:
                            entity['text'] = self._replace_arg(entity['text'], arg_value)
                            entity['value'] = self._replace_arg(entity['value'], arg_value)
            ...
            if self.modifiers.casegen:
                ...
                slot_value = isinstance(rule[0], DummySlotValRuleContent) or None
            else:
                slot_value = self._replace_arg(slot_value, arg_value)
            for ex in examples_from_current_rule:
                if slot_value is not None:
                    ex.entities.append({'slot-name':self.name, 
                     'text':ex.text[:], 
                     'value':slot_value})
                else:
                    ex.entities.append({'slot-name':self.name, 
        ...
        for rule in self.rules:
            ...
            for token in rule:
                ...
                if len(current_examples) <= 0:
                    current_examples = [gen.text for gen in current_token_all_generations]
                else:
                    current_examples = [example_part + gen.text for example_part in current_examples for gen in current_token_all_generations]
            ...
            for slot_value in unprocessed_synonyms:
                if self._contains_arg(slot_value):
                    for arg_value in self.arg_values_encountered:
                        processed_slot_val = self._replace_arg(slot_value, arg_value)
                        synonyms[processed_slot_val] = []
                        for ex in unprocessed_synonyms[slot_value]:
                            if self._contains_arg(ex):
                                for arg_value in self.arg_values_encountered:
                                    synonyms[processed_slot_val].append(self._replace_arg(ex, arg_value))
                    ...
                    for ex in unprocessed_synonyms[slot_value]:
                        if self._contains_arg(ex):
                            for arg_value in self.arg_values_encountered:
                                synonyms[slot_value].append(self._replace_arg(ex, arg_value))
    ...
    def _get_template_rules(self, variation=None):
        ...
        for rule in rules:
            ...
            rule_templates.append(''.join([sub_rule.as_string() for sub_rule in rule]))
