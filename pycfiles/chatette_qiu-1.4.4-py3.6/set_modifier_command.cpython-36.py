...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\chatette_qiu\cli\interactive_commands\set_modifier_command.py
# Compiled at: 2019-03-27 03:00:31
# Size of source mod 2**32: 3883 bytes
"""
Module `chatette_qiu.cli.interactive_commands.set_modifier_command`.
...
from chatette_qiu.utils import str_to_bool
from chatette_qiu.parsing.parser_utils import CASE_GEN_SYM, ARG_SYM

class SetModifierCommand(CommandStrategy):
    usage_str = 'set-modifier <unit-type> "<unit-type>" ' + '<modifer-name> "<value>"'
            ...
            if unit_regex is None:
                try:
                    ...
                    if variation_name is not None:
                        self.print_wrapper.error_log('Cannot set a modifier for the ' + 'variation of a unit.')
                        return
                    self._set_modifier(facade.parser, unit_type, unit_name, modifier_name, modifier_value)
            else:
                count = 0
    ...
    def _set_modifier(self, parser, unit_type, unit_name, modifier_name, value):
        ...
        if modifier_name in ('casegen', CASE_GEN_SYM):
            try:
                ...
                self.print_wrapper.write('Invalid value for case generation ' + 'modifier (True or False).')
                return

        else:
            if modifier_name in ('arg', ARG_SYM):
                unit.set_arg(value)
            else:
                self.print_wrapper.write('Invalid modifier selected (can be ' + "'casegen' or 'arg').")
                return
            self.print_wrapper.write('Modifier for ' + unit_type.name + " '" + unit_name + "' successfully changed.")
