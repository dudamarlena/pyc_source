...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\chatette_qiu\cli\interactive_commands\execute_command.py
# Compiled at: 2019-03-27 03:00:31
# Size of source mod 2**32: 2053 bytes
"""
Module `chatette_qiu.cli.interactive_commands.execute_command`.
Contains the strategy class that represents the interactive mode command
`execute` which opens a file containing a bunch of commands and executes them.
"""
import io
from chatette_qiu.cli.interactive_commands.command_strategy import CommandStrategy, REDIRECTION_SYM, REDIRECTION_APPEND_SYM
...
class ExecuteCommand(CommandStrategy):
    ...
    def execute(self, facade):
        ...
        if len(self.command_tokens) < 2:
            ...
            with io.open(self.split_exact_unit_name(self.command_tokens[1]), 'r') as (command_file):
                commands = [line.rstrip() for line in command_file.readlines() if not line.lstrip().startswith('//')]
            ...
            if redirection_tuple is not None:
                ...
                if redirection_filepath is not None:
                    ...
                    for i, cmd in enumerate(commands):
                        if REDIRECTION_APPEND_SYM not in cmd and REDIRECTION_SYM not in cmd:
