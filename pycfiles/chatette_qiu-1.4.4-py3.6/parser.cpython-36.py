...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\chatette_qiu\parsing\parser.py
# Compiled at: 2019-03-27 03:03:24
# Size of source mod 2**32: 20625 bytes
"""
Module `chatette_qiu.parsing.new_parser`
...
import chatette_qiu.parsing.parser_utils as pu
from chatette_qiu.parsing.tokenizer import Tokenizer
from chatette_qiu.units.word.rule_content import WordRuleContent
from chatette_qiu.units.word.group_rule_content import GroupWordRuleContent
from chatette_qiu.units.choice import ChoiceRuleContent
from chatette_qiu.units.alias import AliasDefinition, AliasRuleContent
from chatette_qiu.units.slot import SlotDefinition, SlotRuleContent, DummySlotValRuleContent
from chatette_qiu.units.intent import IntentDefinition, IntentRuleContent
import os
...
class Parser(object):
    ...
    def __init__(self, master_filename):
        if not isinstance(master_filename, string_types):
            raise ValueError('Since v1.4.0, the parser takes as an argument ' + 'the path of the master file directly, rather ' + 'than the file itself as before.')
        else:
            self.all_files = []
    ...
    def get_definition(self, definition_name, unit_type):
        ...
        if unit_type == pu.UnitType.alias:
            ...
            if unit_type == pu.UnitType.slot:
                relevant_dict = self.slot_definitions
            else:
                if unit_type == pu.UnitType.intent:
                    relevant_dict = self.intent_definitions
                else:
                    raise ValueError('Tried to get a definition with wrong type ' + '(expected alias, slot or intent)')
        if definition_name not in relevant_dict:
            raise KeyError("Couldn't find a definition for " + unit_type.name + " '" + definition_name + "' (did you mean to use " + "the word group '[" + definition_name + "]'?)")
    ...
    def _parse_declaration_initiator(self, token_line):
        ...
        if self._expecting_rule:
            self.tokenizer.syntax_error('Expected a generation rule, got a ' + 'unit declaration instead.')
        else:
        ...
        if unit_type == pu.UnitType.intent:
            ...
            if annotation_interior is not None:
    ...
    def create_unit(self, unit_type, unit_name, modifiers, nb_examples_asked=None):
        """
    ...
    def _parse_rule(self, tokens):
        ...
        if self._currently_parsed_declaration is None:
            self.tokenizer.syntax_error('Got a rule outside of ' + 'a unit declaration.')
        else:
    ...
    def tokens_to_sub_rules(self, tokens):
        ...
        for sub_rule_tokens in pu.next_sub_rule_tokens(tokens):
            if len(sub_rule_tokens) == 1:
                if sub_rule_tokens[0] == ' ':
                    leading_space = True
                    continue
            sub_rule = self._make_sub_rule_from_tokens(sub_rule_tokens, leading_space)
            if alt_slot_value is not None:
                if alt_slot_value != pu.ALT_SLOT_VALUE_FIRST_SYM:
                    sub_rules = [
                     DummySlotValRuleContent(alt_slot_value, sub_rule)]
                else:
                    sub_rules = [
                     DummySlotValRuleContent(sub_rule.name, sub_rule)]
                alt_slot_value = None
    ...
    def _make_sub_rule_from_tokens(self, sub_rule_tokens, leading_space):
        if pu.is_sub_rule_word(sub_rule_tokens):
            ...
            if pu.is_sub_rule_word_group(sub_rule_tokens):
                ...
                self._check_sub_rule_validity(group_interior_tokens, pu.SubRuleType.word_group)
                ...
                word_group = GroupWordRuleContent(words_str, leading_space, casegen=(modifiers.casegen),
                ...
                if pu.is_sub_rule_choice(sub_rule_tokens):
                    ...
                    self._check_sub_rule_validity(choice_interior_tokens, pu.SubRuleType.choice)
                    modifiers = pu.find_modifiers_choice(choice_interior_tokens)
                    choice = ChoiceRuleContent((''.join(choice_interior_tokens)), leading_space,
                    ...
                    for choice_tokens in pu.next_choice_tokens(choice_interior_tokens):
                        ...
                        for choice_sub_rule_tokens in pu.next_sub_rule_tokens(choice_tokens):
                            if choice_sub_rule_tokens == ' ':
                                ...
                                continue
                            choice_sub_rule = self._make_sub_rule_from_tokens(choice_sub_rule_tokens, current_leading_space)
                    ...
                    if len(sub_rule_tokens) == 0 or not pu.is_unit_type_sym(sub_rule_tokens[0]):
                        self.tokenizer.syntax_error('Invalid type of sub-rule.', word_to_find=(sub_rule_tokens[0]))
                    interior_tokens = sub_rule_tokens[2:-1]
                    name = pu.find_name(interior_tokens)
                    if pu.is_sub_rule_alias_ref(sub_rule_tokens):
                        self._check_sub_rule_validity(interior_tokens, pu.SubRuleType.alias)
                        modifiers = pu.find_modifiers_reference(interior_tokens)
                        alias = AliasRuleContent(name, leading_space, modifiers.variation_name, modifiers.argument_value, modifiers.casegen, modifiers.randgen_name, modifiers.percentage_randgen, self)
                        return alias
                if pu.is_sub_rule_slot_ref(sub_rule_tokens):
                    self._check_sub_rule_validity(interior_tokens, pu.SubRuleType.slot)
                    modifiers = pu.find_modifiers_reference(interior_tokens)
                    slot = SlotRuleContent(name, leading_space, modifiers.variation_name, modifiers.argument_value, modifiers.casegen, modifiers.randgen_name, modifiers.percentage_randgen, self)
                    return slot
            if pu.is_sub_rule_intent_ref(sub_rule_tokens):
                self._check_sub_rule_validity(interior_tokens, pu.SubRuleType.intent)
                modifiers = pu.find_modifiers_reference(interior_tokens)
                intent = IntentRuleContent(name, leading_space, modifiers.variation_name, modifiers.argument_value, modifiers.casegen, modifiers.randgen_name, modifiers.percentage_randgen, self)
                return intent
    ...
    def _check_sub_rule_validity(self, interior_tokens, sub_rule_type):
        ...
        try:
            if sub_rule_type == pu.SubRuleType.word_group:
                ...
                if sub_rule_type == pu.SubRuleType.choice:
                    pu.check_choice_validity(interior_tokens)
                else:
                    if sub_rule_type in (pu.SubRuleType.alias,
                     pu.SubRuleType.slot,
    ...
    def rename_unit(self, unit_type, old_name, new_name):
        ...
        if unit_type == pu.UnitType.alias:
            ...
            if unit_type == pu.UnitType.slot:
                ...
                if unit_type == pu.UnitType:
                    relevant_dict = self.intent_definitions
                else:
                    raise ValueError('Tried to rename a definition with wrong type ' + '(expected alias, slot or intent)')
            if old_name in relevant_dict:
                if new_name in relevant_dict:
                    raise ValueError('Tried to rename a definition to a name that ' + "was already in use ('" + new_name + "').")
        ...
        if unit_type == pu.UnitType.alias:
            ...
            if unit_type == pu.UnitType.slot:
                ...
                if unit_type == pu.UnitType.intent:
                    ...
                    raise ValueError('Tried to delete a definition with wrong type ' + '(expected alias, slot or intent)')
            if unit_name not in relevant_dict:
                raise KeyError("Couldn't find a definition for " + unit_type.name + " '" + unit_name + "'.")
    ...
    def add_definition(self, unit_type, unit_name, definition):
        ...
        if unit_name in relevant_dict:
            raise ValueError(unit_type.name.capitalize() + " '" + unit_name + "' " + 'is already defined. Tried to add a definition ' + 'for it again.')
