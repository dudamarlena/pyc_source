...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\chatette_qiu\cli\interactive_commands\command_strategy.py
# Compiled at: 2019-03-27 03:00:31
# Size of source mod 2**32: 13452 bytes
"""
Module `chatette_qiu.cli.interactive_commands.command_strategy`.
...
"""
import re
from chatette_qiu.utils import rchop
from chatette_qiu.parsing.parser_utils import UnitType, ALIAS_SYM, SLOT_SYM, INTENT_SYM, VARIATION_SYM, ESCAPE_SYM
from chatette_qiu.cli.terminal_writer import TerminalWriter, RedirectionType
REDIRECTION_SYM = '>'
...
class CommandStrategy(object):
    ...
    def __init__(self, command_str, quiet=False):
        ...
        redirection_tuple = CommandStrategy.find_redirection_file_path(self.command_tokens)
        if redirection_tuple is not None:
            self.remove_redirection_tokens()
            redirection_type, redirection_filepath = redirection_tuple
            self.print_wrapper = TerminalWriter(redirection_type, redirection_filepath)
        else:
            if quiet:
                self.print_wrapper = TerminalWriter(redirection_type=(RedirectionType.quiet))
            else:
            ...
            for word in splitted:
                if inside_token:
                    ...
                    if CommandStrategy._is_end_regex(word):
                        ...
                        current_token = ''
                elif word.startswith('"') and word.endswith('"') and (len(word) < 2 or word[(-2)] != '\\'):
                    tokens.append(word)
    ...
    def _is_end_regex(word):
        ...
        return word.endswith('/') or word.endswith('/g') or word.endswith('/i') or word.endswith('/ig') or word.endswith('/gi')
    ...
    def find_redirection_file_path(tokens):
        ...
        if len(tokens) < 2:
            ...
            if tokens[(-2)] == REDIRECTION_APPEND_SYM:
                return (
                 RedirectionType.append, tokens[(-1)])
            if tokens[(-2)] == REDIRECTION_SYM:
                return (
                 RedirectionType.truncate, tokens[(-1)])
            if tokens[(-1)] == REDIRECTION_APPEND_SYM or tokens[(-1)] == REDIRECTION_SYM:
                return (
    ...
    def get_regex_name(self, name):
        ...
        if name.startswith(REGEX_SYM):
            if name.endswith(REGEX_SYM) or name[(-2)] == REGEX_SYM or name[(-3)] == REGEX_SYM:
                ...
                if 'i' in flags:
                    return re.compile(text_without_flags[1:-1].replace('\\' + REGEX_SYM, REGEX_SYM), re.IGNORECASE)
        ...
        if unit_type == UnitType.alias:
            ...
            if unit_type == UnitType.slot:
                ...
                if unit_type == UnitType.intent:
                    ...
                    raise ValueError('Unexpected unit type when matching regex: ' + str(unit_type))
    ...
    def remove_redirection_tokens(self):
        ...
        """
        if self.command_tokens[(-2)] == REDIRECTION_APPEND_SYM or self.command_tokens[(-2)] == REDIRECTION_SYM:
