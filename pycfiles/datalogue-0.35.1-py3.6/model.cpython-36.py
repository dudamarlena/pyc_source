...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/datalogue/models/model.py
# Compiled at: 2020-05-12 19:42:15
# Size of source mod 2**32: 52121 bytes
...
class Deployment:
    ...
    def __repr__(self):
        return f"Deployment(id: {self.id}, ontology_id: {self.ontology_id}, model_id: {self.model_id},model_type: {str(self.model_type)}, status: {str(self.status)}, created_at: {self.created_at})"
    ...
    def _from_payload(json: dict) -> Union[(DtlError, 'Deployment')]:
        ...
        if isinstance(model_id, DtlError):
            return model_id
        model_type = json.get('modelType')
...
class OriginalSection:
    ...
    def __init__(self, section_id: UUID, datastore_id: UUID, source_path: List[str], tokens: List[str], token_types: List[str], rejected: bool, tags: List[List[NerTag]]):
    ...
    def replace_tag_by(self, test: Callable[([NerTag], bool)], generate_tokens: Callable[([], List[Token])]) -> List['SynthesizedSection']:
        """
        Create new sections
    ...
    def replace_tag_by_many(self, test: Callable[([NerTag], bool)], generate_tokens: Callable[([], List[List[Token]])]) -> List['SynthesizedSection']:
        """
        Create new sections
        replace tag when 'test' is true using 'generate_tokens' method to replace original tokens in the section
        Returned list of NewSections to be created as augmented data.
        ...
        def replace_tag(section, tag, all_tags):
            ...
            for token_definition in new_tokens_definition:
                ...
                tag_sections.append(SynthesizedSection(None, section.section_id, section.datastore_id, section.source_path, tokens, token_types, section.rejected, before_tags + [NerTag(tag.start_token, tag.start_token + len(new_tokens), tag.tag)] + after_tags))
                return tag_sections
...
class SynthesizedSection:
    ...
    def __init__(self, section_id: Optional[UUID], original_id: UUID, datastore_id: UUID, source_path: List[str], tokens: List[str], token_types: List[str], rejected: bool, tags: List[NerTag]):
        self.section_id = section_id
