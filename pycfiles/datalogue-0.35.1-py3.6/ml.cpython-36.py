...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/datalogue/clients/ml.py
# Compiled at: 2020-05-12 19:42:15
# Size of source mod 2**32: 27885 bytes
from typing import Optional, Union, List, Dict, Tuple, Iterator
from itertools import chain
from typing import AnyStr, Callable
import statistics, pandas as pd, numpy as np
...
from datalogue.models.model import Model, ModelType, TrainingStatusType, OrderList, TrainingState, training_status_type_from_str, Deployment, Section, SynthesizedSection, OriginalSection, SynthesisStats, TrainingData
...
class _SynthesisClient:
    ...
    def _get_sections(self, endpoint: str, datastore_ids: List[Union[(str, UUID)]], page: int, size: int) -> Union[(DtlError, dict)]:
        params = {'datastoreId':datastore_ids, 
         'page':page, 
         'size':size}
    ...
    def get(self, datastore_ids: List[Union[(str, UUID)]], ontology_id: Union[(str, UUID)], page: int, page_size: int) -> Union[(DtlError, List[SynthesizedSection])]:
        """
    ...
    def _create(self, ontology_id: Union[(str, UUID)], section: SynthesizedSection) -> Union[(DtlError, None)]:
        ...
        return self.http_client.make_authed_request(f"/themis/sections/ontology/{ontology_id}/synthesized", (HttpMethod.POST),
          body=body)
    ...
    def synthesize(self, datastore_ids: List[Union[(str, UUID)]], ontology_id: Union[(str, UUID)], synthesis_algorithm: Callable[([OriginalSection], List[SynthesizedSection])], overwrite_synthesis=False) -> Union[(DtlError, SynthesisStats)]:
        """
...
class TrainingCoach:
    ...
    def check(ontology: Ontology) -> pd.DataFrame:
        ...
        return pd.DataFrame(rows, columns=[
         'Class', 'Id', 'WARNING: No data', 'WARNING: <2 sources', 'WARNING: <2000 data points'])
...
class _MLBase:
    ...
    def get_model_history(self, ontology_id: UUID, limit: Optional[int]=20) -> Union[(DtlError, List[Model])]:
        ...
        if limit is not None:
            ...
            params['limit'] = limit
        res = self.http_client.make_authed_request(f"/yggy/ontology/{str(ontology_id)}/trainings/history",
          (HttpMethod.GET),
          params=params)
        if isinstance(res, DtlError):
            return res
...
class _CBCDataClient:
    ...
    def add(self, store_id: UUID, store_name: str, refs: List[DataRef], count: Optional[int]=None) -> Union[(DtlError, List[UUID])]:
        ...
        for dataRef in refs:
            for path in dataRef.path_list:
                dataset_id = self._CBCDataClient__create_dataset(store_name)
                if isinstance(dataset_id, DtlError):
                    return dataset_id
                stream_id = self._CBCDataClient__transfer_data_from_datastore(store_id, dataset_id, dataRef.node_id, path, count)
                if isinstance(stream_id, DtlError):
                    return stream_id
                self._CBCDataClient__update_node(dataRef.node_id, path, dataset_id, stream_id)
    ...
    def __update_node(self, node_id: UUID, path: List[str], dataset_id: UUID, stream_id: UUID) -> List[str]:
        ...
        if training_data_list is None:
            training_data_list = []
        training_data_list.append({'datasetId':str(dataset_id), 
         'nodePath':path, 
         'streamId':str(stream_id)})
        self.http_client.make_authed_request(f"/yggy/entity/{node_id}", (HttpMethod.POST), body={'trainingDataInfo': training_data_list})
        dataset_ids = []
...
class _CBCClient(_MLBase):
    ...
    def get_deployments(self, page: int=1, item_per_page: int=25) -> Union[(DtlError, List[TrainingState])]:
        ...
        params = {'from':from_item, 
         'limit':item_per_page, 
         'status':'Successful', 
         'model-type':str(ModelType.Cbc)}
        res = self.http_client.make_authed_request('/argus/deployments',
    ...
    def deploy(self, model_id: UUID, ontology_id: UUID) -> Union[(DtlError, UUID)]:
        ...
        """
        res = self.http_client.make_authed_request(f"/yggy/ontology/{str(ontology_id)}/trainings/{str(model_id)}/deploy", HttpMethod.POST, {})
        if isinstance(res, DtlError):
            return res
        else:
    ...
    def get_models(self, ontology_id: UUID, status_type: Optional[TrainingStatusType]=None, sort: Optional[OrderList]=OrderList.desc, limit: Optional[int]=20) -> Union[(DtlError, List[TrainingState])]:
        ...
        res = self.http_client.make_authed_request(f"/yggy/ontology/{str(ontology_id)}/trainings",
          (HttpMethod.GET), params=params)
        if isinstance(res, DtlError):
            return res
        else:
...
class _NERDataClient:
    ...
    def delete_section(self, datastore_id: Union[(str, UUID)], path: List[str]) -> Union[(DtlError, None)]:
        ...
        if isinstance(datastore_id, DtlError):
            ...
            res = self.http_client.make_authed_request('/themis/sections', (HttpMethod.DELETE), body={'datastore_id':str(datastore_id), 
             'path':path})
            if isinstance(res, DtlError):
                return res
            return

    def get_annotations(self, ontology_id: Union[(str, UUID)], datastore_ids: List[Union[(str, UUID)]], items_per_page: int=25, page_after_id: Union[(str, UUID, None)]=None) -> Union[(DtlError, Tuple[(List[Section], Optional[UUID])])]:
        """
        ...
        if isinstance(sections_statistics, DtlError):
            ...
            if isinstance(next_token, DtlError):
                return next_token
            return (
             sections_statistics, next_token)
...
class _NERClient(_MLBase):
    ...
    def train(self, ontology_id: UUID, training_data: TrainingData=TrainingData.Original, force_run: bool=True) -> Union[(DtlError, UUID, List)]:
        ...
        params['force-train'] = force_run
        res = self.http_client.make_authed_request(f"/yggy/ontology/{ontology_id}/train",
