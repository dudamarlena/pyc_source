...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/datalogue/models/graph.py
...
class Path(object):
    ...
    def __init__(self, prev_full_path: Optional['Path']):
...
class Graph:
    ...
    def get_node_at_path(self, paths: List[str]) -> Union[(None, Node)]:
        ...
        def _get_node_at_path(paths, cur_node):
            if len(paths) == 0:
                return
            else:
                cur_path = paths[0]
                ...
                if len(updated_paths) == 0:
                    if cur_path == cur_node.label:
                        return cur_node
                siblings = self.id_to_node[str(cur_node.id)].siblings
    ...
    def _from_payload(ads_json: dict, node_decoder) -> Union[(DtlError, 'Graph')]:
        ...
        def _parse_list_schema_nodes(json):
        ...
        def _build_full_path(graph: Graph) -> Graph:
            ...
            root_node = graph.id_to_node[graph.root].node
            ...
            while len(next_nodes) > 0:
                ...
                while len(to_process_nodes) > 0:
                    ...
                    cur_node = graph.id_to_node[str(cur_node_id)]
        ...
        if isinstance(parsed_schema_nodes, DtlError):
            ...
            for edge in parsed_edges:
                ...
                target_node_def = graph.id_to_node[target_node_id]
                existing_source_node_def = graph.id_to_node[source_node_id]
