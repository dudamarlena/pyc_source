...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/ycyc/tests/base/test_txtutils.py
...
class TestCoding(TestCase):
    ...
    def test_strcat(self):
        ...
        gbk_ch = ('测试').encode('gbk')
        self.assertEqual(txtutils.strcat(gbk_ch, self.b_ch, 'gbk', 'utf-8'), target)
        self.assertEqual(txtutils.strcat(gbk_ch, self.u_ch, 'gbk', 'utf-8'), target)
        self.assertEqual(txtutils.strcat(self.b_ch, gbk_ch, 'utf-8', 'gbk'), target)
        self.assertEqual(txtutils.strcat(self.u_ch, gbk_ch, 'utf-8', 'gbk'), target)


class TestStringUtils(TestCase):

    def test_drop_prefix(self):
        self.assertEqual(txtutils.drop_prefix('root/file.txt', 'root/'), 'file.txt')
        self.assertEqual(txtutils.drop_prefix('dir/file.txt', 'root/'), 'dir/file.txt')

    def test_drop_postfix(self):
        self.assertEqual(txtutils.drop_postfix('file.txt', '.txt'), 'file')
        self.assertEqual(txtutils.drop_postfix('file.tmp', '.txt'), 'file.tmp')

    def test_split_and_strip(self):
        self.assertListEqual(txtutils.split_and_strip('a,b ,c, d , e'), [
         'a', 'b', 'c', 'd', 'e'])
...
class TestTxtDistance(TestCase):
    ...
    def test_hamming_distance(self):
        ...
        for case, result in test_cases.items():
            s1, s2 = case
            self.assertEqual(txtutils.TxtDistance.hamming_distance(s1, s2), result)


class TestSepJoin(TestCase):

    def test_usage(self):
        self.assertEqual(txtutils.sep_join('|', map(str, range(3))), '0|1|2')
        self.assertEqual(txtutils.sep_join('|', map(str, range(3)), True), '|0|1|2')
        self.assertEqual(txtutils.sep_join('|', map(str, range(3)), True, True), '|0|1|2|')
        self.assertEqual(txtutils.sep_join(' | ', map(str, range(3)), '| ', ' |'), '| 0 | 1 | 2 |')
        self.assertEqual(txtutils.sep_join(',', map(str, range(3)), 'x='), 'x=0,1,2')