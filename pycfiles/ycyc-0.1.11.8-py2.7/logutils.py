...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/ycyc/base/logutils.py
# Compiled at: 2016-02-25 04:17:16
import contextlib, logging.config, logging, sys, thread, inspect
...
class LoggerInfo(object):
    ...
    def func_args(self):
        ...
        result = {i:code_locals[i] for i in args.args}
        if args.varargs:
    ...
    config = {'version': 1, 
       ...
       'handlers': {'console': {'level': 'DEBUG', 
                    ...
                    'file': {'level': 'INFO', 
                             ...
                             'filename': log_file, 
                             'formatter': 'verbose'}}, 
       'loggers': {name:{'handlers': ['console', 'file'], 'level': 'DEBUG'} for name in loggers or ['']}}
...
def console_only_config(level='DEBUG', format='%(message)s'):
    config = {'version': 1, 
       'disable_existing_loggers': True, 
       'formatters': {'simple': {'format': format}}, 
       'handlers': {'console': {'level': 'DEBUG', 
                                'class': 'logging.StreamHandler', 
                                'formatter': 'simple'}}, 
       'loggers': {'': {'handlers': [
...
def log_disable(level=logging.CRITICAL):
    ...
    logger.info('with logger disabled from %s to %s in %s[%s], id: %s', logging.root.manager.disable, level, pre_frame.f_code.co_name, pre_frame.f_lineno, id(cur_frame))
...
class LogFunctionCall(object):
    ...
    def __str__(self):
        s_returned = '' if self.returned is None else '-> %s' % str(self.returned)
        args = map(str, self.args or ())
        args.extend(('{k}={v}').format(k=str(k), v=str(v)) for k, v in self.kwargs.items() or ())
        return ('{f}({args}){returned}').format(f=self.func.__name__ if self.func else '', args=(', ').join(args), returned='' if self.returned is None else '-> %s' % str(self.returned))