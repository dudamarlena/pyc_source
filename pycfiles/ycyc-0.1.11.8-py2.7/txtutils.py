...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/ycyc/base/txtutils.py
...
def template_render(template, model):
    ...
    """
    parts = []
    for part in re.split('{{([\\.\\w]+?)}}', template):
        parts.append(str(part if len(parts) & 1 == 0 else reduce(lambda m, k: m[k], part.split('.'), model)))
...
def split_and_strip(val_str, sep=','):
    ...
    """
    return [ i for i in (i.strip() for i in val_str.split(sep)) if i
           ]
    ...
    def edit_distance2(cls, str1, str2):
        ...
        def edit_distance(s1, s2):
            ...
            if len1 <= 0 or len2 <= 0:
                ...
                result = 1 + min(edit_distance(ss1, s2), edit_distance(s1, ss2), edit_distance(ss1, ss2))
...
def look_like(target, candidates):
    ...
    """
    results = [ (TxtDistance.edit_distance(target, i), i) for i in candidates
              ]
    result = min(key=(lambda x: x[0]), *results)
    return result[1]
