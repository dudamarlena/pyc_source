...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/tools/inline_style.py
# Compiled at: 2016-05-23 10:53:29
import sys
...
class InlineStyleConvertor(HTMLParser):
    ...
    def format_style(self, style_txt):
        style_txt = re.sub('/\\*(\\s|.)*?\\*/', '', style_txt, re.M)
        styles = [ i.split('{') for i in style_txt.split('}') if i.strip()
                 ]
        results = {}
    ...
    def handle_starttag(self, tag, attrs):
        ...
        self.output.write(('<{tag}{attrs}>').format(tag=tag, attrs=' %s' % attr_str if attr_str else ''))
...
def main():
    ...
    arg_parser.add_argument('-s', '--style', help='css file')
    arg_parser.add_argument('-o', '--output', default=None, help='output file')
    arg_parser.add_argument('-i', '--ignore', action='store_true', default=False, help='ignore inline style')
