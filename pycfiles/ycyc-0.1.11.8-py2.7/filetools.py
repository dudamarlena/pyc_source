...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/ycyc/base/filetools.py
# Compiled at: 2016-02-25 04:17:16
import os
from os import path as os_path
import codecs, tempfile, shutil
...
def oserror_format(func):
    ...
    @wraps(func)
    def foo(*args, **kwargs):
        with contextutils.catch(getattr(exceptions, 'WindowsError', OSError), reraise=OSError):
...
class PathInfo(object):
    ...
    def children(self):
        if not self.is_dir:
            return []
        return [ PathInfo(os_path.join(self.path, i)) for i in os.listdir(self.path)
               ]
