...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/ycyc/ycollections/namedict.py
...
class NamedDict(six.with_metaclass(NamedDictMeta, dict)):
    ...
    def __init__(self, *args, **kwg):
        ...
        if kwg:
            ...
            fields.update({k:kwg[k] for k in real_keys & kwg_keys})
        if args:
            i_args = iter(args)
            fields.update({k:v for k, v in zip(chain(requires, field_keys), i_args)})
        ...
        if missed_fields:
            ...
            raise RequireFieldsMissError('fields: [%s]' % (', ').join(missed_fields))
...
def namedict(name, requires=(), fields=None):
    ...
    """
    return type(name, (NamedDict,), {'__Fields__': fields or {}, 
