...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/ycyc/ycollections/rangecls.py
...
class ARange(object):
    ...
    def __str__(self):
        return ('{lb}{start}..{end}{rb}').format(lb='[' if self.include_start else '(', start=self.start, end=self.end, rb=']' if self.include_end else ')')

    def __repr__(self):
        return ('{clsname}({start}, {end}, include_start={include_start}, include_end={include_end})').format(clsname=self.__class__.__name__, start=repr(self.start), end=repr(self.end), include_start=repr(self.include_start), include_end=repr(self.include_end))
    ...
    def __add__(self, other):
        return ARange(self.start + other.start, self.end + other.end, include_start=self.include_start and other.include_start, include_end=self.include_end and other.include_end)
    ...
    def __mul__(self, other):
        vals = [
         ...
         self.end * other.end]
        return ARange(min(vals), max(vals), include_start=self.include_start and other.include_start, include_end=self.include_end and other.include_end)
