...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/ycyc/base/funcutils.py
# Compiled at: 2016-03-29 10:38:39
"""
funcutils provided some useful functions.
"""
import inspect, sys, thread
...
def iter_attrs(obj, include_fields=(), exclude_fields=(), public_only=True, exclude_methods=True):
    """
    ...
    for attr, val in inspect.getmembers(obj):
        if attr not in include_fields:
            if attr in exclude_fields or attr.startswith('__') or attr.startswith('_%s__' % obj_type_name) or public_only and attr.startswith('_') or exclude_methods and inspect.ismethod(val):
                continue
        yield (
         attr, val)
...
def export_module(name, env_locals=None):
    ...
    all_attrs = list(getattr(module, '__all__', (i for i in dir(module) if not i.startswith('_'))))
    for i in all_attrs:
