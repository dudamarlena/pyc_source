...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/tools/mk_unittest.py
...
def mk_unittest_script(script_relative_path, root):
    ...
    with open(real_path, 'wt') as (fp):
        fp.write(TeseCaseTemplate.format(test_name='Test%s' % module_name.title().replace('_', '')))
...
def main():
    ...
    arg_parser.add_argument('-r', '--recursion', default=0, type=int)
    arg_parser.add_argument('-a', '--anchor', default=working_dir)
    arg_parser.add_argument('-t', '--target', default=os.path.join(working_dir, 'tests'))
