...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/tools/analysis_tools.py
...
class Analysis(object):
    ...
    def visit_assign(self, node):
        ...
        for target in node.targets:
            if isinstance(target, ast.Attribute):
                continue
            flag = True
            self.analysis_result.writeline('var: {name}', lineno=target.lineno, name=self.var_name(target))
    ...
    def visit_functiondef(self, node):
        ...
        decorators = [ '%s(*)' % d.attr if isinstance(d, ast.Call) else self.var_name(d) for d in node.decorator_list
                     ]
        decorators.reverse()
        self.analysis_result.writeline('function: {name}({args}){decorators}', lineno=node.lineno, name=node.name, args=(', ').join(args_lst), decorators=' @%s' % decorators if decorators else '')
        docstring = self.get_docstring(node)
        if docstring:
            self.analysis_result.writeline('{docstring}\n--- docstring ---', docstring=docstring)

    @NodeAnalysisMaps.register('ClassDef')
    def visit_classdef(self, node):
        basestr = ''
        if node.bases:
            basestr = '(%s)' % (', ').join([ self.var_name(b) for b in node.bases ])
        self.analysis_result.writeline('class: {name}{basestr}', name=node.name, basestr=basestr)
        docstring = self.get_docstring(node)
        if docstring:
            self.analysis_result.writeline('{docstring}\n--- docstring ---', docstring=docstring)
