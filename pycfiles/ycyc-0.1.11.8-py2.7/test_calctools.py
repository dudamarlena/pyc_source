...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/ycyc/tests/base/test_calctools.py
...
class TestSafeCalc(TestCase):
    ...
    def test_usage(self):
        calc = calctools.SafeCalc({'vint': 1, 
           'vfloat': 2.3, 'vstr': '456', 'vlist': [7, 8.9, '10'], 'vdict': {'key1': 11, 'key2': ObjModel(key=12.13)}})
    ...
    def test_func(self):
        import operator
        calc = calctools.SafeCalc({'add': operator.add, 
           'sub': operator.sub})
    ...
    def test_evil(self):
        forbidden_exprs = ('import os', "open('test.txt', 'w')", 'def x(): pass; print x.func_globals',
    ...
    def test_timeout(self):
        import time
        calc = calctools.SafeCalc({'sleep': time.sleep}, timeout=0.05, interval=0.001)
        try:
            calc('sleep(1)')
    ...
    def test_allow_attr(self):
        ...
        calc = calctools.SafeCalc({'model': m, 'value': m.value}, allow_attr=False)
...
class TestSelect(TestCase):
    ...
    def test_usage(self):
        obj = {'key': {'name': 'test'}, 'value': [
                   123]}
        self.assertDictEqual(calctools.select(obj, ('key', 'name'), ('value', 0)), {0: obj['key']['name'], 1: obj['value'][0]})
        self.assertDictEqual(calctools.select(obj, key=('key', 'name'), value=('value',
                                                                               0)), {'key': obj['key']['name'], 'value': obj['value'][0]})
        self.assertDictEqual(calctools.select(obj, noting=('key', 'noting')), {'noting': None})
        return