...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/ycyc/base/contextutils.py
# Compiled at: 2016-02-25 04:17:16
from contextlib import contextmanager
import six, logging, threading, time
...
def catch(errors=Exception, reraise=None, callback=logger.warning):
    ...
    """
    exec_info = {'callback_returned': None, 
       'exception': None, 
       'ok': True}
    try:
        yield exec_info
...
@contextmanager
def companion(target, auto_start=True, auto_join=True, process=threading.Thread):
    """
...
def heartbeat(target, interval, auto_start=True, auto_join=True):
    ...
    with companion(check, auto_start=auto_start, auto_join=auto_join) as (processor):
        try:
            yield processor
        finally:
            enable = False