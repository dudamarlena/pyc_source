...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/ycyc/ycollections/heap.py
...
class Heap(object):
    ...
    def __init__(self, iterable=(), cmp_attrs=(), reverse=False):
        self.data = list(HeapItem(i, self) for i in iterable)
        self.cmp_attrs = cmp_attrs
        self.reverse = reverse
        heapq_op.heapify(self.data)

    def compare_items(self, item1, item2):
        value = cmp(getattrs(item1.value, self.cmp_attrs), getattrs(item2.value, self.cmp_attrs))
        if self.reverse:
            return value * -1
        return value
    ...
    def headn(self, n):
        return [ i.value for i in heapq_op.nsmallest(n, self.data) ]

    def tailn(self, n):
        values = [ i.value for i in heapq_op.nlargest(n, self.data) ]
        return values[::-1]
