...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/ycyc/base/typeutils.py
...
class TypeFactory(object):
    ...
    def __getattr__(self, name):
        return lambda bases=None, attrs=None: subtype(name, bases, attrs)
...
class ExceptionFactory(object):
    ...
    def __getattr__(self, name):
        return lambda bases=None, attrs=None: subexception(name, bases, attrs)
...
def freezed_attrs(attrs):
    ...
    def setattr_hook(cls):
        ...
        return subtype(cls.__name__, cls, {'__doc__': cls.__doc__, 
...
class Constants(object):
    ...
    def __new__(cls, mappings, attrs):
        consts_index = {val:tuple(i[0] for i in item) for val, item in groupby(mappings.items(), itemgetter(1))}
        ...
        constants = super(Constants, cls).__new__(subtype(__name__, cls, {'__doc__': cls.__doc__, 
           '__iter__': __iter__, 
...
def enums(*values):
    ...
    """
    return Constants({k:i for i, k in enumerate(values)}, {'name': 'EnumerationSet', 'getitem_hook': _enums_getitem_hook})