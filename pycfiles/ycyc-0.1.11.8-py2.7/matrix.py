...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/ycyc/libs/mathutils/matrix.py
# Compiled at: 2016-05-23 10:53:43
...
class Matrix(object):
    ...
    def __init__(self, row_n, col_n, matrix=None, init_val=0):
        ...
        matrix = matrix or (repeat(init_val, col_n) for r in range(row_n))
        self.data = deque((deque((c for c in r), maxlen=col_n) for r in matrix), maxlen=row_n)
    ...
    def make_sure_matrix_size(self):
        ...
        data = self.data
        self.check_size(self, lambda x: x > 0, lambda x: x > 0)
    ...
    def __str__(self):
        ...
        return os.linesep.join((',').join(template % c for c in r) for r in output)
    ...
    def __add__(self, other):
        self.check_size(other, lambda x: self.row_n == x, lambda x: self.col_n == x)
    ...
    def __mul__(self, other):
        self.check_size(other, lambda x: self.col_n == x)
