...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/ycyc/tests/base/test_iterutils.py
# Compiled at: 2016-07-19 10:55:32
from unittest import TestCase
from collections import namedtuple
import operator
from ycyc.base.iterutils import getitems, getattrs, iterable, getnext, getfirst, groupby, mkparts, get_single_item, dict_merge, flatten, find_peak_item, filter_n, every_n, iter_chunk, safe_max, safe_min
import mock
...
class Testgetattrs(TestCase):
    ...
    def test_usage(self):
        Pair = self.Pair
        model = Pair(key='test', value=Pair(key='a', value=1))
...
class TestGroupBy(TestCase):
    ...
    def test_usage(self):
        data = [{'name': 'int', 'val': 1}, {'name': 'float', 'val': 2.3}, {'name': 'float', 'val': 4.5}, {'name': 'int', 'val': 6}, {'name': 'str', 'val': '7'}]
        self.assertDictEqual(groupby(data, operator.itemgetter('name')), {'int': [{'name': 'int', 'val': 1}, {'name': 'int', 'val': 6}], 'float': [{'name': 'float', 'val': 2.3}, {'name': 'float', 'val': 4.5}], 'str': [{'name': 'str', 'val': '7'}]})
...
class TestDictMerge(TestCase):
    ...
    def test_usage(self):
        self.assertDictEqual(dict_merge([{1: 2, 2: 3}, {2: 4, 3: 4}, {4: 5}]), {1: 2, 2: 3, 3: 4, 4: 5})
...
class TestFlatten(TestCase):
    ...
    def test_usage(self):
        seq_list = [
         range(0, 3), range(3, 7), (i for i in range(7, 9))]
...
class TestFindPeakItem(TestCase):
    ...
    def test_usage(self):
        self.assertListEqual(list(find_peak_item([13, 4, 5, 3, 6, 9, 4])), [
         (1, 4), (2, 5), (3, 3), (5, 9)])
        ...
        self.assertListEqual(list(find_peak_item([1, 2], True)), [(0, 1), (1, 2)])
        self.assertListEqual(list(find_peak_item([1, 2, 3], True)), [(0, 1), (2, 3)])
...
class TestFilterN(TestCase):
    ...
    def test_filter_n(self):
        self.assertListEqual([
         1, 3], list(filter_n(lambda x: x % 2, range(10), 2)))
        self.assertListEqual([
         1], list(filter_n(None, range(10), 1)))
        self.assertListEqual([
         0, 4, 8], list(filter_n(lambda x: x % 4 == 0, range(10), 20)))
        return
...
class TestEveryN(TestCase):
    ...
    def test_every_n(self):
        self.assertListEqual([
         (0, 1), (2, 3)], list(every_n(range(4), 2)))
        self.assertListEqual([
         (0, 1, 2)], list(every_n(range(4), 3)))
