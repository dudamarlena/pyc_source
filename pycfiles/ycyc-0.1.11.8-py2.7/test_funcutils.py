...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/ycyc/tests/base/test_funcutils.py
...
class TestObjectHelper(TestCase):
    ...
    def test_set_default_attr(self):
        ...
        self.assertTrue(funcutils.set_default_attr(foo, 'foobarbaz', True))
        self.assertTrue(foo.foobarbaz)
        self.assertTrue(funcutils.set_default_attr(foo, 'foobarbaz', False))
        self.assertTrue(foo.foobarbaz)
    ...
    def test_iter_attrs(self):
        ...
        foo = Foo()
        self.assertDictEqual(dict(funcutils.iter_attrs(foo)), {'Public': 0, 
           'public': 4, 'protected': 5})
        self.assertDictEqual(dict(funcutils.iter_attrs(foo, public_only=False)), {'Public': 0, 
           '_Protected': 1, 'public': 4, 'protected': 5, 
           '_protected': 5})
        self.assertDictEqual(dict(funcutils.iter_attrs(foo, include_fields={'_Protected'})), {'Public': 0, 
           '_Protected': 1, 'public': 4, 'protected': 5})
        self.assertDictEqual(dict(funcutils.iter_attrs(foo, exclude_fields={'protected'})), {'Public': 0, 
           'public': 4})
        result = dict(funcutils.iter_attrs(foo, exclude_methods=False))
        self.assertEqual(result['get_privated'], foo.get_privated)
        self.assertIn('_Protected', dict(funcutils.iter_attrs(foo, include_fields={
         '_Protected'}, exclude_fields={
         '_Protected'})))
