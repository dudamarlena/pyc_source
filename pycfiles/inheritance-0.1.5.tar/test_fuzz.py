...
# [GCC 8.4.0]
# Embedded file name: /home/brentp/src/inheritance/inheritance/tests/test_fuzz.py
# Compiled at: 2019-01-10 11:53:26
...
def make_fam(n_affecteds, n_unaffecteds, n_unknowns, id='xxx'):
    ...
    for i in range(n_affecteds):
        samples.append(Sample('affected_%d' % i, affected=True, sex=random.randint(1, 2), name='affected_%d' % i))

    for i in range(n_unaffecteds):
        samples.append(Sample('unaffected_%d' % i, affected=False, sex=random.randint(1, 2), name='affected_%d' % i))

    for i in range(n_unknowns):
        samples.append(Sample('unknown_%d' % i, affected=None, sex=random.randint(1, 2), name='affected_%d' % i))
...
def test_fuzz():
    ...
    for i in range(n):
        ...
        if n_affecteds + n_unaffecteds + n_unknowns == 0:
            continue
