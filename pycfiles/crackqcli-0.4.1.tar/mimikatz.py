...
# Embedded file name: /home/byt3bl33d3r/.virtualenvs/CME_old/lib/python2.7/site-packages/cme/modules/mimikatz.py
...
class CMEModule:
    ...
    def parse_mimikatz(self, data):
        ...
        creds = []
        regexes = [
         '(?s)(?<=msv :).*?(?=tspkg :)', '(?s)(?<=tspkg :).*?(?=wdigest :)', '(?s)(?<=wdigest :).*?(?=kerberos :)', '(?s)(?<=kerberos :).*?(?=ssp :)', '(?s)(?<=ssp :).*?(?=credman :)', '(?s)(?<=credman :).*?(?=Authentication Id :)', '(?s)(?<=credman :).*?(?=mimikatz)']
        hostDomain = ''
        domainSid = ''
        hostName = ''
        lines = data.split('\n')
