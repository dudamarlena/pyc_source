...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/RESTinpy/resources.py
...
class Resource:
    ...
    def get_urlpatterns(self):
        return patterns('', ('^' + self.get_url_regex() + '$', self.front_controller))
    ...
    def get_relative_link_to_parent(self):
        return '..' if self.is_directory_resource() else '.'
    ...
    def as_resource_dict(self):
        somekeys = [
         'slug_field', 'form_class', 'extra_context', 'login_required', 'template_name']
        return dict([ (k, self.__dict__.get(k)) for k in somekeys if k in self.__dict__ if self.__dict__.get(k) ])
    ...
    def apply_reverse_adapters(self, dict_n):
