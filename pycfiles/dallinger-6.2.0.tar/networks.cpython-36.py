...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/Dallinger/Dallinger/dallinger/networks.py
# Compiled at: 2020-04-24 19:15:49
# Size of source mod 2**32: 10249 bytes
...
class DelayedChain(Network):
    ...
    def add_node(self, node):
        ...
        if len(self.nodes()) > 11:
            parents = [
             max(other_nodes, key=(attrgetter('creation_time')))]
        else:
...
class FullyConnected(Network):
    __doc__ = 'A fully-connected network (complete graph) with all possible vectors.'
...
class Empty(Network):
    __doc__ = 'An empty network with no vectors.'
...
class DiscreteGenerational(Network):
    ...
    def add_node(self, node):
        ...
        if curr_generation == 0:
            ...
            parent = self._select_fit_node_from_generation(node_type=(type(node)),
    ...
    def _select_fit_node_from_generation(self, node_type, generation):
        prev_agents = node_type.query.filter_by(failed=False,
          network_id=(self.id),
    ...
    def add_node(self, node):
        ...
        if len(nodes) <= self.m0:
            ...
            for idx_newvector in range(self.m):
                these_nodes = [n for n in nodes if n.id != node.id if not n.is_connected(direction='either', whom=node)]
...
class SequentialMicrosociety(Network):
    ...
    def _most_recent_predecessors_to(self, node):
        other_nodes = [n for n in self.nodes() if n.id != node.id]
        other_nodes_newest_first = sorted(other_nodes,
