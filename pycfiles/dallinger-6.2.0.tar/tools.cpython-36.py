...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/Dallinger/Dallinger/dallinger/heroku/tools.py
# Compiled at: 2020-04-24 19:15:49
# Size of source mod 2**32: 17198 bytes
"""Miscellaneous tools for Heroku."""
from __future__ import unicode_literals
import json
from six.moves import shlex_quote as quote
import signal, os, psutil, re, six, sys, subprocess, time, traceback
...
class HerokuCommandRunner(object):
    __doc__ = 'Heroku command runner base class'
...
class HerokuInfo(HerokuCommandRunner):
    __doc__ = 'Methods for getting information about current heroku status'
    ...
    def my_apps(self):
        ...
        for app in self.all_apps():
            ...
            creator = self._result([
             'heroku', 'config:get', 'CREATOR', '--app', name]).strip()
            if creator == my_login:
                uid = self._result([
                 'heroku', 'config:get', 'DALLINGER_UID', '--app', name]).strip()
...
class HerokuApp(HerokuCommandRunner):
    __doc__ = 'Representation of a Heroku app'
    ...
    def bootstrap(self):
        ...
        """
        cmd = [
         'heroku', 'apps:create', self.name, '--buildpack', 'heroku/python']
        if self.team:
            cmd.extend(['--team', self.team])
        self._run(cmd)
        self.set_multiple(HOST=(self.url),
    ...
    def addon(self, name):
        """Set up an addon"""
        cmd = [
         'heroku', 'addons:create', name, '--app', self.name]
        self._run(cmd)
    ...
    def buildpack(self, url):
        ...
        cmd = [
         'heroku', 'buildpacks:add', url, '--app', self.name]
        self._run(cmd)
    ...
    def db_uri(self):
        ...
        if match is None:
            raise NameError('Could not retrieve the DB URI. Check for error output from heroku above the stack trace.')
    ...
    def get(self, key, subcommand='config:get'):
        ...
        cmd = [
         'heroku', subcommand, key, '--app', self.name]
        return self._result(cmd)

    def open_logs(self):
        """Show the logs."""
        cmd = [
         'heroku', 'addons:open', 'papertrail', '--app', self.name]
        self._run(cmd)
...
class HerokuLocalWrapper(object):
    ...
    def start(self, timeout_secs=60):
        ...
        def _handle_timeout(signum, frame):
            raise HerokuTimeoutError('Failed to start after {} seconds.'.format(timeout_secs, self._record))
        ...
        if self.is_running:
            ...
            if not success:
                ...
                raise HerokuStartupError('Failed to start for unknown reason: {}'.format(self._record))
            return True
    ...
    def _verify_startup(self):
        for line in self._stream():
            ...
            if self._redis_not_running(line):
                self.out.error('Could not connect to redis instance, experiment may not behave correctly.')
            if self._worker_error(line) or self._startup_error(line):
                if not self.verbose:
                    self.out.error('There was an error while starting the server. Run with --verbose for details.')
        ...
        if not self.is_running:
            ...
            for child in psutil.Process(self._process.pid).children(recursive=True):
                if 'python' in child.name():
                    ...
                    name = child.name()
                reprs.append("<Process pid='{}', name='{}', status='{}'>".format(child.pid, name, child.status()))
...
def sanity_check(config):
    ...
    if team:
        if 'free' in sizes:
            raise RuntimeError('Heroku "free" dyno type not compatible with team/org deployment. Please use a different "dyno_type" or unset the "heroku_team" configuration.')