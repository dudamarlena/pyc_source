...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/Dallinger/Dallinger/dallinger/mturk.py
# Compiled at: 2020-04-24 19:15:49
# Size of source mod 2**32: 26328 bytes
import builtins as @py_builtins, _pytest.assertion.rewrite as @pytest_ar, boto3, datetime, logging, time
...
class MTurkServiceException(Exception):
    __doc__ = 'Custom exception type'


class RemoteAPICallTimedOut(MTurkServiceException):
    __doc__ = 'A call to a remote API took too long.'


class DuplicateQualificationNameError(MTurkServiceException):
    __doc__ = 'A Qualification with the given name already exists.'


class QualificationNotFoundException(MTurkServiceException):
    __doc__ = 'A Qualification searched for by name does not exist.'


class WorkerLacksQualification(MTurkServiceException):
    __doc__ = 'The worker has not been granted a given Qualification.'


class RevokedQualification(MTurkServiceException):
    __doc__ = 'The Qualification has been revoked for this worker.'


class NonExistentSubscription(MTurkServiceException):
    __doc__ = 'The SNS subscription does not exist.'


class SNSService(object):
    __doc__ = 'Handles AWS SNS subscriptions'
    max_wait_secs = 12

    def __init__(self, aws_access_key_id, aws_secret_access_key, region_name, confirm=True):
    ...
    @cached_property
    def _sns(self):
        session = boto3.session.Session(aws_access_key_id=(self.aws_key),
    ...
    def confirm_subscription(self, token, topic):
        ...
        self._sns.confirm_subscription(Token=token,
          TopicArn=topic)

    def create_subscription(self, experiment_id, notification_url):
        logger.warning('Creating new SNS subscription for {}...'.format(notification_url))
        protocol = 'https' if notification_url.startswith('https') else 'http'
        topic = self._sns.create_topic(Name=experiment_id)
        subscription = self._sns.subscribe(TopicArn=(topic['TopicArn']),
          Protocol=protocol,
    ...
    def cancel_subscription(self, experiment_id):
        ...
        if sns_topic is None:
            raise NonExistentSubscription('No SNS subscription found for {}'.format(experiment_id))
    ...
    def _awaiting_confirmation(self, subscription):
        if not self.do_confirm_subscription:
            ...
            report = self._sns.get_subscription_attributes(SubscriptionArn=(subscription['SubscriptionArn']))
    ...
    def _get_sns_topic_for_experiment(self, experiment_id):
        ...
        experiment_topics = (t for t in all_topics if t['TopicArn'].endswith(experiment_id))
        try:
...
class MTurkQuestions(object):
    __doc__ = 'Creates MTurk HIT Question definitions:\n    https://docs.aws.amazon.com/AWSMechTurk/latest/AWSMturkAPI/ApiReference_QuestionAnswerDataArticle.html\n    '

    @staticmethod
    def external(ad_url, frame_height=600):
        q = '<ExternalQuestion xmlns="http://mechanicalturk.amazonaws.com/AWSMechanicalTurkDataSchemas/2006-07-14/ExternalQuestion.xsd"><ExternalURL>{}</ExternalURL><FrameHeight>{}</FrameHeight></ExternalQuestion>'
...
class MTurkQualificationRequirements(object):
    ...
    def restrict_to_countries(countries):
        return {'QualificationTypeId':LOCALE_REQUIREMENT_ID, 
         'Comparator':'EqualTo', 
    ...
    def must_have(qualification_id):
        return {'QualificationTypeId':qualification_id, 
         'Comparator':'Exists', 
         'RequiredToPreview':True}
...
class MTurkService(object):
    __doc__ = 'Facade for Amazon Mechanical Turk services provided via the boto3\n       library.\n    '

    def __init__(self, aws_access_key_id, aws_secret_access_key, region_name, sandbox=True, max_wait_secs=0):
        self.aws_key = aws_access_key_id
        self.aws_secret = aws_secret_access_key
        self.region_name = region_name
    ...
    def mturk(self):
        ...
        return session.client('mturk',
          endpoint_url=(self.host), region_name=(self.region_name))

    @cached_property
    def sns(self):
        return SNSService(aws_access_key_id=(self.aws_key),
          aws_secret_access_key=(self.aws_secret),
          region_name=(self.region_name))
    ...
    def check_credentials(self):
        ...
        try:
            ...
            raise MTurkServiceException('Error checking credentials: {}'.format(str(ex)))
    ...
    def create_qualification_type(self, name, description, status='Active'):
        ...
        """
        try:
            response = self.mturk.create_qualification_type(Name=name,
    ...
    def assign_qualification(self, qualification_id, worker_id, score, notify=False):
        ...
        return self._is_ok(self.mturk.associate_qualification_with_worker(QualificationTypeId=qualification_id,
          WorkerId=worker_id,
          IntegerValue=score,
    ...
    def increment_qualification_score(self, name, worker_id, notify=False):
        ...
        return {'qtype':result['qtype'], 
         'score':new_score}

    def revoke_qualification(self, qualification_id, worker_id, reason=''):
        return self._is_ok(self.mturk.disassociate_qualification_from_worker(QualificationTypeId=qualification_id,
          WorkerId=worker_id,
          Reason=reason))
    ...
    def get_qualification_score(self, qualification_id, worker_id):
        ...
        """
        try:
            response = self.mturk.get_qualification_score(QualificationTypeId=qualification_id,
              WorkerId=worker_id)
        except ClientError as ex:
            error = str(ex)
            if 'does not exist' in error:
                raise WorkerLacksQualification('Worker {} does not have qualification {}.'.format(worker_id, qualification_id))
            if 'operation can be called with a status of: Granted' in error:
                raise RevokedQualification('Worker {} has had qualification {} revoked.'.format(worker_id, qualification_id))
    ...
    def get_current_qualification_score(self, name, worker_id):
        ...
        if qtype is None:
            raise QualificationNotFoundException('No Qualification exists with name "{}"'.format(name))
        try:
    ...
    def dispose_qualification_type(self, qualification_id):
        ...
        return self._is_ok(self.mturk.delete_qualification_type(QualificationTypeId=qualification_id))
    ...
    def get_workers_with_qualification(self, qualification_id):
        ...
        while not done:
            if next_token is not None:
                response = self.mturk.list_workers_with_qualification_type(QualificationTypeId=qualification_id,
    ...
    def create_hit(self, experiment_id, title, description, keywords, reward, duration_hours, lifetime_days, question, max_assignments, notification_url=None, annotation=None, qualifications=(), do_subscribe=True):
        """Create the actual HIT and return a dict with its useful properties.
        """
        hit_type_id = self._register_hit_type(title, description, reward, duration_hours, keywords, qualifications)
        if do_subscribe:
            self._create_notification_subscription(experiment_id, notification_url, hit_type_id)
    ...
    def create_additional_assignments_for_hit(self, hit_id, number):
        try:
            response = self.mturk.create_additional_assignments_for_hit(HITId=hit_id,
              NumberOfAdditionalAssignments=number,
              UniqueRequestToken=(self._request_token()))
        except Exception as ex:
            raise MTurkServiceException('Error: failed to add {} assignments to HIT: {}'.format(number, str(ex)))
    ...
    def update_expiration_for_hit(self, hit_id, hours):
        ...
        try:
            response = self.mturk.update_expiration_for_hit(HITId=hit_id,
              ExpireAt=expiration)
        except Exception as ex:
            raise MTurkServiceException('Failed to extend time until expiration of HIT: {}'.format(expiration, str(ex)))
    ...
    def grant_bonus(self, assignment_id, amount, reason):
        ...
        try:
            return self._is_ok(self.mturk.send_bonus(WorkerId=worker_id,
              BonusAmount=amount_str,
              AssignmentId=assignment_id,
              Reason=reason,
              UniqueRequestToken=(self._request_token())))
        except ClientError as ex:
            error = 'Failed to pay assignment {} bonus of {}: {}'.format(assignment_id, amount_str, str(ex))
            raise MTurkServiceException(error)
    ...
    def approve_assignment(self, assignment_id):
        ...
        """
        try:
            return self._is_ok(self.mturk.approve_assignment(AssignmentId=assignment_id))
        except ClientError as ex:
            assignment = self.get_assignment(assignment_id)
            raise MTurkServiceException('Failed to approve assignment {}, {}: {}'.format(assignment_id, str(assignment), str(ex)))

    def _create_notification_subscription(self, experiment_id, notification_url, hit_type_id):
        ...
        self.mturk.update_notification_settings(HITTypeId=hit_type_id,
          Notification={'Destination':topic_arn, 
         'Transport':'SNS', 
         'Version':'2014-08-15', 
         'EventTypes':[
          'AssignmentAccepted',
    ...
    def _register_hit_type(self, title, description, reward, duration_hours, keywords, qualifications):
        ...
        hit_type = self.mturk.create_hit_type(Title=title,
          Description=description,
          Reward=reward,
    ...
    def _translate_assignment(self, assignment):
        translated = {'id':assignment['AssignmentId'], 
    ...
    def _translate_hit(self, hit):
        ...
        translated = {'id':hit['HITId'], 
         ...
         'keywords':keywords, 
         'qualification_type_ids':[q['QualificationTypeId'] for q in hit['QualificationRequirements']], 
    ...
    def _translate_qtype(self, qtype):
        return {'id':qtype['QualificationTypeId'], 
