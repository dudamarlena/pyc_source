...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/Dallinger/Dallinger/dallinger/data.py
# Compiled at: 2020-04-24 19:15:49
# Size of source mod 2**32: 12799 bytes
"""Data-handling tools."""
from .config import get_config
import csv, errno, io, logging, os, shutil, six, subprocess, tempfile, warnings
from zipfile import ZipFile, ZIP_DEFLATED
import botocore, boto3, hashlib, postgres_copy, psycopg2
...
with warnings.catch_warnings():
    ...
    try:
        import odo, pandas as pd, tablib
    except ImportError:
...
def find_experiment_export(app_id):
    ...
    if os.path.exists(path_to_data):
        try:
            ...
            logger.exception('Error reading local data file {}, checking remote.'.format(path_to_data))
        else:
            return path_to_data
    path_to_data = os.path.join(tempfile.mkdtemp(), data_filename)
    buckets = [
     user_s3_bucket(), dallinger_s3_bucket()]
...
def _scrub_participant_table(path_to_data):
    ...
    with open_for_csv(path, 'r') as (input):
        with open('{}.0'.format(path), 'w') as (output):
...
def fix_autoincrement(table_name):
    ...
    """
    db.engine.execute("select setval('{0}_id_seq', max(id)) from {0}".format(table_name))
...
def ingest_to_model(file, model, engine=None):
    ...
    postgres_copy.copy_from(file,
      model, engine, columns=columns, format='csv', HEADER=False)
...
def user_s3_bucket(canonical_user_id=None):
    ...
    s3_bucket_name = 'dallinger-{}'.format(hashlib.sha256(canonical_user_id.encode('utf8')).hexdigest()[0:8])
    ...
    def __init__(self, URL):
        ...
        if self.source.endswith('.zip'):
            ...
            for tab in table_names:
                setattr(self, '{}s'.format(tab), Table(os.path.join(tmp_dir, 'data', '{}.csv'.format(tab))))


class Table(object):
    __doc__ = 'Dallinger data-table object.'
