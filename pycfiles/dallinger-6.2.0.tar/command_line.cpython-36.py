...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/Dallinger/Dallinger/dallinger/command_line.py
# Compiled at: 2020-04-24 19:15:49
# Size of source mod 2**32: 29938 bytes
"""The Dallinger command-line utility."""
from __future__ import print_function
from __future__ import unicode_literals
from collections import Counter
from functools import wraps
import inspect, os, shutil, signal, sys, tabulate, tempfile, time, webbrowser, click, requests
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            def _handle_timeout(signum, frame):
                ...
                if not config.ready:
                    config.load()
                message = {'subject':'Idle Experiment.',  'body':idle_template.format(app_id=config.get('id'),
...
def verify_directory(verbose=True, max_size_mb=50):
    ...
    if size > max_size:
        ...
        log(('✗ {}MB is TOO BIG (greater than {}MB)\n\tIncluded files:\n\t{}'.format(size_in_mb, max_size_mb, '\n\t'.join(file_source.files))),
          chevrons=False,
          verbose=verbose)
        ok = False
    return ok
...
def verify_config(verbose=True):
    ...
    try:
        ...
        if float(dollarFormat) != float(base_pay):
            log(('✗ base_payment must be in [dollars].[cents] format in config.txt. Try changing {0} to {1}.'.format(base_pay, dollarFormat)),
              delay=0,
              chevrons=False,
              verbose=verbose)
            ok = False
...
def setup():
    ...
    if os.path.isfile(config_path):
        ...
        src = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'default_configs', config_name)
...
@click.option('--bot',
  is_flag=True, flag_value=True, help='Use bot to complete experiment')
@click.option('--proxy',
  default=None, help='Alternate port when opening browser windows')
@click.option('--no-browsers',
  is_flag=True,
  flag_value=True,
  default=False,
...
def _mturk_service_from_config(sandbox):
    config = get_config()
    config.load()
    return MTurkService(aws_access_key_id=(config.get('aws_access_key_id')),
...
def qualify(workers, qualification, value, by_name, notify, sandbox):
    ...
    if not (workers and qualification and value):
        raise click.BadParameter('Must specify a qualification ID, value/score, and at least one worker ID')
    else:
        mturk = _mturk_service_from_config(sandbox)
        if by_name:
            result = mturk.get_qualification_type_by_name(qualification)
            if result is None:
                raise click.BadParameter('No qualification with name "{}" exists.'.format(qualification))
            qid = result['id']
        else:
            qid = qualification
    click.echo('Assigning qualification {} with value {} to {} worker{}...'.format(qid, value, len(workers), 's' if len(workers) > 1 else ''))
...
def email_test():
    ...
    if problems:
        out.error('✗ There are mail configuration problems. Fix these first:\n{}'.format(problems))
        return
    out.log('✓ email config looks good!')
    mailer = SMTPMailer(config.get('smtp_host'), config.get('smtp_username'), config.get('smtp_password'))
    msg = {'subject':'Test message from Dallinger', 
...
def compensate(recruiter, worker_id, email, dollars, sandbox):
    ...
    with config.override({'mode': mode}):
        ...
        if not click.confirm('\n\nYou are about to pay worker "{}" ${:.2f} in "{}" mode using the "{}" recruiter.\nThe worker will{} be notified by email. Continue?'.format(worker_id, dollars, mode, recruiter, no_email_str)):
            out.log('Aborting...')
            return
        try:
            ...
            out.error(('Compensation failed. The recruiter reports the following error:\n{}'.format(ex)),
              delay=0)
            return
...
def revoke(workers, qualification, by_name, reason, sandbox):
    ...
    for worker in workers:
        if mturk.revoke_qualification(qid, worker, reason):
            click.echo('Revoked qualification "{}" from worker "{}"'.format(qid, worker))

    results = list(mturk.get_workers_with_qualification(qid))
    click.echo('There are now {} workers with qualification "{}"'.format(len(results), qid))
...
def hits(app, sandbox):
    ...
    out = Output()
    out.log('Found {} hits for this experiment id: {}'.format(len(hit_list), ', '.join(h['id'] for h in hit_list)))
...
def expire(app, sandbox, exit=True):
    ...
    if failures:
        out.log('Could not expire {} hits: {}'.format(len(failures), ', '.join(failures)))
    if not success:
        if not failures:
            out.log('No hits found for this application.')
            if not sandbox:
                out.log('If this experiment was run in the MTurk sandbox, use: `dallinger expire --sandbox --app {}`'.format(app))
    if exit:
        if not success:
            sys.exit(1)
...
@click.option('--expire-hit/--no-expire-hit',
  flag_value=True,
  default=True,
...
def verify():
    ...
    verbose = True
    log('Verifying current directory as a Dallinger experiment...',
      delay=0,
      verbose=verbose)
...
def apps():
    ...
    if not config.ready:
        ...
        if listing:
            out.log('Found {} heroku apps running for user {}'.format(len(listing), my_user))
