...
# [GCC 8.4.0]
# Embedded file name: F:\pythonAPP\yxspkg_songzgif_module\yxspkg_songzgif\gif.py
# Compiled at: 2018-08-03 07:44:51
# Size of source mod 2**32: 52763 bytes
import os, sys, tempfile
from io import StringIO
from os import path
import cv2, imageio, numpy as np, pip, yxspkg_songzviewer
from numpy import fromfile, fromstring, hstack, stack
...
from PyQt5.QtWidgets import QAction, QApplication, QCheckBox, QColorDialog, QDialog, QFileDialog, QFontDialog, QHBoxLayout, QInputDialog, QLabel, QLineEdit, QMenu, QMessageBox, QProgressBar, QPushButton, QRadioButton, QScrollArea, QSizePolicy, QVBoxLayout, QWidget
import asyncio
...
class SaveThread(QThread):
    ...
    def generate_image(self, size):
        ...
        if outimg.shape[(-1)] == 4:
            if path.splitext(self.filename)[1].lower() in ('.jpg', '.jpeg'):
                outimg = outimg[:, :, :3]
    ...
    def run(self):
        ...
        try:
            if self.filename[-4:] == '.gif':
                ...
                if path.splitext(self.filename)[1] in self.parent.image_files:
                    self.generate_image(size)
                else:
                    if self.filename[-3:] == '%6d':
...
class VideoCutter(QDialog):
    ...
    def accept(self):
        ...
        try:
            ...
            QMessageBox.information(self, 'Input error', '输入有误\n{0}'.format(e))
            return
...
class FontEditor(QDialog):
    ...
    def initUI(self):
        ...
        self.size_up.clicked.connect(lambda : self._FontEditor__changeSize(1))
        self.size_down.clicked.connect(lambda : self._FontEditor__changeSize(-1))
        ...
        self.weight_up.clicked.connect(lambda : self._FontEditor__setFontWeight(10))
        self.weight_down.clicked.connect(lambda : self._FontEditor__setFontWeight(-10))
        ...
        self.rotate_up.clicked.connect(lambda : self._FontEditor__setFontrotate(10))
        self.rotate_down.clicked.connect(lambda : self._FontEditor__setFontrotate(-10))
    ...
    def accept(self):
        self.info = (
         self.font_input, self.color_input, self.position_input, self.rotate_input, self.get_text_input())
        super().accept()
    ...
    def get_info(self, parent):
        ...
        self.p_rate = self.ndimg.shape[0] / ImageLabel.global_height
        self.initUI()
        self.exec_()
        return self.info
...
class ImageBoard(QWidget):
    ...
    def scaleImage(self, image):
        ...
        self.image_list.append([image, 0, w, right])
        image.move(right, 0)
        right += w
        self.resize(right, self.height())


class DrawImageWidget(QLabel):
...
class GifMaker(QWidget):
    video_files = [
     '.mp4', '.mkv', '.ts', '.avi', '.flv', '.mov', '.mpg', '.m4v', '.wmv']
    ...
    def __init__(self):
        ...
        self.text_h_label.connect(self._GifMaker__check_height)
        ...
        self.text_w_label.connect(self._GifMaker__check_width)
        ...
        self.grabButton = QPushButton(self)
        self.grabButton.setGeometry(701, h_p + 30, 65, 30)
        self.grabButton.setText('前景提取')
        ...
        self.textbutton.clicked.connect(self._GifMaker__drawText)
        ...
        self.musicbutton.clicked.connect(self._GifMaker__insert_music)
    ...
    def check_ffmpeg(self):
        ...
        reply = ffmpeg or QMessageBox.question(self, 'Download ffmpeg', '你的系统中没有安装ffmpeg。点击确定自行安装ffmpeg，或者你也可以点击取消，自己安装ffmpeg（需要放到环境变量中）')
    ...
    def create_right_key_menu(self):
        ...
        self.deleteCurrentAct = QAction('删除该帧', self, triggered=(self._GifMaker__delete_image_current))
        self.rightMenu.addAction(self.deleteCurrentAct)
        self.deleteBeforeAct = QAction('删除该帧及之前所有帧', self, triggered=(self._GifMaker__delete_image_before))
        self.rightMenu.addAction(self.deleteBeforeAct)
        self.deleteBehindAct = QAction('删除该处及之后所有帧', self, triggered=(self._GifMaker__delete_image_behind))
        self.rightMenu.addAction(self.deleteBehindAct)
        self.deleteSpecialAct = QAction('删除指定帧', self, triggered=(self._GifMaker__delete_image_special))
        ...
        self.changefpsAct = QAction('改变帧率', self, triggered=(self._GifMaker__changefps))
        self.rightMenu.addAction(self.changefpsAct)
        self.rightMenu.addSeparator()
        self.insertimageAct = QAction('插入文件', self, triggered=(self._GifMaker__insertfile))
        self.rightMenu.addAction(self.insertimageAct)
        self.reverseAct = QAction('逆序', self, triggered=(self._GifMaker__reverseimage))
        self.rightMenu.addAction(self.reverseAct)
        self.rotateAct = QAction('逆时针旋转90度', self, triggered=(lambda : self._GifMaker__rotate(90)))
        self.rightMenu.addAction(self.rotateAct)
        self.rotate270Act = QAction('顺时针旋转90度', self, triggered=(lambda : self._GifMaker__rotate(270)))
        self.rightMenu.addAction(self.rotate270Act)
        self.rotate180Act = QAction('旋转180度', self, triggered=(lambda : self._GifMaker__rotate(180)))
        self.rightMenu.addAction(self.rotate180Act)
        self.rotate_1Act = QAction('左右镜像对称', self, triggered=(lambda : self._GifMaker__rotate(-1)))
        self.rightMenu.addAction(self.rotate_1Act)
        self.rotate_2Act = QAction('上下镜像对称', self, triggered=(lambda : self._GifMaker__rotate(-2)))
        self.rightMenu.addAction(self.rotate_2Act)
        self.mergeAct = QAction('合成一张图', self, triggered=(self._GifMaker__mergeimage))
        self.rightMenu.addAction(self.mergeAct)
        self.rightMenu.addSeparator()
        self.saveimageAct = QAction('图片另存为', self, triggered=(self._GifMaker__saveimage))
        self.rightMenu.addAction(self.saveimageAct)
        self.saveimageSeriesAct = QAction('导出图片序列', self, triggered=(self._GifMaker__saveimageSeries))
        self.rightMenu.addAction(self.saveimageSeriesAct)
        self.rightMenu.addSeparator()
        self.setupAct = QAction('设置', self, triggered=(self._GifMaker__setup))
        self.rightMenu.addAction(self.setupAct)
        self.rightMenu.addSeparator()
        self.setupAct = QAction(('关于' + software_name), self, triggered=(self._GifMaker__about))
        self.rightMenu.addAction(self.setupAct)
    ...
    def __check_height(self, s):
        if not s.isdigit() or self.ratio_state is False or self.ratio_value is None:
            return
        s = int(s)
        self.text_w_label.setText(str(int(s / self.ratio_value)))

    def __check_width(self, s):
        if not s.isdigit() or self.ratio_state is False or self.ratio_value is None:
            return
        s = int(s)
        self.text_h_label.setText(str(int(s * self.ratio_value)))

    def __delete_image_current(self):
        n = self.board.current_image.number
        self.delete_image(n - 1, n)

    def __delete_image_behind(self):
    ...
    def delete_image(self, start, end):
        print(start, end)
        if end == -1:
            end = len(self.board.image_list)
        else:
            if end > len(self.board.image_list) or end < start:
                return False
    ...
    def release_label_useless(self):
            ...
            if len(files) == 1:
                if ext.lower() in self.video_files:
                    ...
                    if ext.lower() == '.gif':
                        ...
                        if ext.lower() in self.image_files:
    ...
    def grab_image_run(self, img, mask, bgdModel=None, fgdModel=None):
        if img.shape[(-1)] == 4:
            ...
            fgdModel = fgdModel or np.zeros((1, 65), np.float64)
    ...
    def get_cutsize(self, info):
        ...
        if img_geometry == cutpart_geometry:
            return
        ...
        yy = y + h2 / h1
        x, y, xx, yy = (
         int(x * w), int(y * h), int(xx * w), int(yy * h))
