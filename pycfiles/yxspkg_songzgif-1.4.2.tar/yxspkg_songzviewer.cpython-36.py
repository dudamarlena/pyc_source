...
# [GCC 8.4.0]
# Embedded file name: F:\pythonAPP\yxspkg_songzgif_module\yxspkg_songzgif\yxspkg_songzviewer.py
# Compiled at: 2017-11-29 05:43:03
# Size of source mod 2**32: 19414 bytes
...
def setDefault():
    ...
    if sys.platform.startswith('win'):
        ...
        home_path = path.join(environ['HOMEPATH'], '.songzgifrc')
...
class YTitleLabel(QLabel):
    ...
    def mousePressEvent(self, e):
        if self.parent.resizeWindow:
            ...
            return
        self.xt, self.yt = self.parent.x(), self.parent.y()
    ...
    def enterEvent(self, e):
        if not self.parent.source_name:
            return
...
class YDesignButton(QLabel):
    position_dict = {'center':Qt.AlignCenter, 
     'left':Qt.AlignLeft,  'hcenter':Qt.AlignHCenter,  'vcenter':Qt.AlignVCenter,  'justify':Qt.AlignJustify}
...
class GifPreview(QWidget):
    ...
    def __init__(self, s='SongZ Viewer', name=None):
        ...
        self.CloseButton.setNormalQimg([(30, 30, 70, 70), (30, 70, 70, 30)], (0, 0,
                                                                              0,
                                                                              0), (0,
                                                                                   0,
                                                                                   0,
                                                                                   0), (100,
                                                                                        100), 4)
        self.CloseButton.setFocusQimg([(30, 30, 70, 70), (30, 70, 70, 30)], (255, 0,
                                                                             0, 255), (255,
                                                                                       255,
                                                                                       255), (100,
                                                                                              100), 4)
    ...
    def scaleImage(self, factor):
        ...
        if tt > self.factor_max or tt < self.factor_min:
            return
    ...
    def preview(self, parent):
        ...
        if self.first_window:
            ...
            shape0 = (shape[0] / t, shape[1] / t)
            self.resize(shape0[1] + self.border * 2, shape0[0] + self.border * 2 + self.bottom_height)
            self.first_window = False
        else:
            w, h = self.width(), self.height()
        ...
        if self.timer:
            self.timer.stop()
        self.label.resize(shape0[1], shape0[0])
        self.factor = shape0[1] / shape[1]
        self.setPosition()
        self.update_image()
        if self.fps != 0:
    ...
    def isresizeMouse(self, x, y):
        ...
        distance = 8
        if x < x0 + width:
            if y0 + h0 - distance > y > y0 + distance:
                self.setCursor(QCursor(Qt.SizeHorCursor))
                self.resizeWindow = 'Left'
        if x > x0 + w0 - width and y0 + h0 - distance > y > y0 + distance:
            self.setCursor(QCursor(Qt.SizeHorCursor))
            self.resizeWindow = 'Right'
        else:
            if y < y0 + width and x0 + w0 - distance > x > x0 + distance:
                self.setCursor(QCursor(Qt.SizeVerCursor))
                self.resizeWindow = 'Top'
            elif y > y0 + h0 - width:
                if x0 + w0 - distance > x > x0 + distance:
                    self.setCursor(QCursor(Qt.SizeVerCursor))
