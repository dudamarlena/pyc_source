...
def rotation_matrix(angle, direction, point=None):
    ...
    R += numpy.array(((0.0, -direction[2], direction[1]),
     ...
     (
      -direction[1], direction[0], 0.0)), dtype=numpy.float64)
    M = numpy.identity(4)
    M[:3, :3] = R
...
def projection_from_matrix(matrix, pseudo=False):
    ...
    if not pseudo and len(i):
        ...
        if len(i):
            ...
            return (
             point, normal, direction, None, False)
        return (
         point, direction, None, None, False)
    else:
        i = numpy.where(abs(numpy.real(l)) > 1e-08)[0]
        if not len(i):
...
class Arcball(object):
    ...
    def drag(self, point):
        ...
        if numpy.dot(t, t) < _EPS:
            ...
            q = [
             t[0], t[1], t[2], numpy.dot(self._vdown, vnow)]
...
_AXES2TUPLE = {'sxyz': (0, 0, 0, 0), 
   'sxyx': (0, 0, 1, 0), 'sxzy': (0, 1, 0, 0), 'sxzx': (0, 1, 1, 0), 
   'syzx': (1, 0, 0, 0), 'syzy': (1, 0, 1, 0), 'syxz': (1, 1, 0, 0), 
   'syxy': (1, 1, 1, 0), 'szxy': (2, 0, 0, 0), 'szxz': (2, 0, 1, 0), 
   'szyx': (2, 1, 0, 0), 'szyz': (2, 1, 1, 0), 'rzyx': (0, 0, 0, 1), 
   'rxyx': (0, 0, 1, 1), 'ryzx': (0, 1, 0, 1), 'rxzx': (0, 1, 1, 1), 
   'rxzy': (1, 0, 0, 1), 'ryzy': (1, 0, 1, 1), 'rzxy': (1, 1, 0, 1), 
   'ryxy': (1, 1, 1, 1), 'ryxz': (2, 0, 0, 1), 'rzxz': (2, 0, 1, 1), 
   'rxyz': (2, 1, 0, 1), 'rzyz': (2, 1, 1, 1)}
