...
# Embedded file name: build/bdist.macosx-10.5-i386/egg/alchemist/security/permission.py
...
class LocalRolePermissionMap(object):
    ...
    def getPermissionsForRole(self, role_id):
        ...
        """
        s = select([rpm.c.permission_id, rpm.c.setting], and_(rpm.c.role_id == role_id, rpm.c.object_id == self.oid, rpm.c.object_type == self.object_type))
        for o in s.execute():
    ...
    def getRolesForPermission(self, permission_id):
        ...
        returned.
        """
        s = select([rpm.c.role_id, rpm.c.setting], and_(rpm.c.permission_id == permission_id, rpm.c.object_id == self.oid, rpm.c.object_type == self.object_type))
        for o in s.execute():
            yield (o[0], BooleanAsSetting[o[1]])
    ...
    def getSetting(self, permission_id, role_id):
        ...
        """
        s = select([rpm.c.setting], and_(rpm.c.permission_id == permission_id, rpm.c.role_id == role_id, rpm.c.object_id == self.oid, rpm.c.object_type == self.object_type))
    ...
    def getRolesAndPermissions(self):
        ...
        list is returned.
        """
        s = select([rpm.c.permission_id, rpm.c.role_id, rpm.c.setting], and_(rpm.c.object_id == self.oid, rpm.c.object_type == self.object_type))
        for o in s.execute():
    ...
    def grantPermissionToRole(self, permission_id, role_id):
        ...
        """
        rpm.insert(values=dict(permission_id=permission_id, role_id=role_id, object_id=self.oid, object_type=self.object_type)).execute()

    def denyPermissionToRole(self, permission_id, role_id):
        """Deny the permission to the role
        """
        s = select([rpm.c.permission_id], and_(rpm.c.object_id == self.oid, rpm.c.permission_id == permission_id, rpm.c.role_id == role_id, rpm.c.object_type == self.object_type))
        if s.execute().fetchone():
            self.unsetPermissionFromRole(permission_id, role_id)
        rpm.insert(values=dict(permission_id=permission_id, role_id=role_id, setting=False, object_id=self.oid, object_type=self.object_type)).execute()

    def unsetPermissionFromRole(self, permission_id, role_id):
        """Clear the setting of the permission to the role.
        """
        rpm.delete(and_(rpm.c.role_id == role_id, rpm.c.permission_id == permission_id, rpm.c.object_type == self.object_type, rpm.c.object_id == self.oid)).execute()
