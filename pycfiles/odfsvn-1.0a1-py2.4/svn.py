...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/odfsvn/svn.py
...
class SVNRepository:
    ...
    def _getInfoFromPath(self, path):
        command = [
         'svn', 'info', '--non-interactive', '--xml', path]
        process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        ...
        root = entries[0]
        try:
            self._info = dict(type='svn', url=self.url, root=getXmlInfo(root, 'root'), uuid=getXmlInfo(root, 'uuid'), author=getXmlInfo(root, 'author'), revision=int(getXmlInfo(root, 'commit', 'revision')))
        except KeyError:
    ...
    def retrieve(self, odf, revision=None, tag=None, tempdir=None):
        ...
        if revision is None:
            ...
            run('svn', 'co', '--quiet', '--non-interactive', '--revision', revision, url, tempdir)
    ...
    def store(self, odf, odf_update=True, message='ODFSVN commit', tempdir=None):
        ...
        parent = ('/').join(parts[:-1])
        filename = parts[(-1)]
        run('svn', 'co', '--quiet', '--non-interactive', '--non-recursive', parent, tempdir)
        ...
        run('svn', 'ci', '--quiet', '--non-interactive', '--message', message, tempdir)
    ...
    def update(self, odf, tempdir=None):
        ...
        run('svn', 'co', '--quiet', '--non-interactive', '--revision', info['revision'], self.url, tempdir)
        ...
        output = [ (action[0], path.replace(prefix, '')) for (action, path) in output ]
