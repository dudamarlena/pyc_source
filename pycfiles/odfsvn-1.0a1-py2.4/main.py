...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/odfsvn/scripts/main.py
...
import sys, optparse
aliases = {'ci': 'commit', 'co': 'checkout', 'up': 'update'}
options = None
...
def main():
    global options
    parser = optparse.OptionParser(formatter=HelpFormatter(), usage='%prog [options] <action> [<parameters>]', description='Manage ODF documents maintained in a subversion repository.\nAvailable actions are:\n\n   help        Show detailed information about an action\n   info        Show repository information for a document\n   import      Import a document into a repository\n   checkout    Retrieve a document from a repository\n   commit      Commit changes to the repository\n   update      Update a document to the latest repository version\n')
    parser.add_option('-t', '--tag', action='store', dest='tag', help='Use this tag instead of trunk')
    parser.add_option('-r', '--revision', action='store', dest='revision', help='Checkout a specific revision')
    parser.add_option('-m', '--message', action='store', dest='message', help='Log message for this action')
    parser.add_option('-v', '--verbose', action='count', dest='verbose', help='Be more verbose about what is happening')
    parser.set_defaults(verbose=0, message='ODSVN commit')
