...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/odfsvn/utils.py
# Compiled at: 2008-01-17 07:36:18
import errno, os, shutil, tempfile, zipfile
...
def getXmlInfo(node, tag, attr=None):
    ...
    element = elements[0]
    if attr is None:
        return ('').join([ child.data for child in element.childNodes if child.nodeType == node.TEXT_NODE ])
    else:
