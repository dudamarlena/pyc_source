...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/odfsvn/package.py
# Compiled at: 2008-01-17 07:36:18
import errno, md5, os, shutil, tempfile, zipfile
...
metadata_names = [
 (
  'type', 'Type', unicode), ('uuid', 'UUID', unicode), ('revision', 'Revision', int), ('url', 'URL', unicode)]
...
class ODFPackage(object):
    ...
    def getRepositoryInfo(self):
        ...
        for element in dom.getElementsByTagNameNS(NS_META, 'user-defined'):
            name = element.getAttributeNS(NS_META, 'name')
            if not name.startswith('Repository-'):
                continue
            name = name[11:]
            text = ('').join([ child.data for child in element.childNodes if child.nodeType == element.TEXT_NODE ])
            text = text.strip()
            try:
    ...
    def setRepositoryInfo(self, info):
        ...
        def addNode(dom, parent, name, value):
            node = dom.createElementNS(NS_META, 'meta:user-defined')
            node.setAttributeNS(NS_META, 'meta:name', 'Repository-' + name)
        ...
        meta = root.getElementsByTagNameNS(NS_OFFICE, 'meta')
        if not meta:
            meta = dom.createElementNS(NS_OFFICE, 'office:meta')
            root.appendChild(meta)
        else:
            meta = meta[0]
        for element in meta.getElementsByTagNameNS(NS_META, 'user-defined'):
            name = element.getAttributeNS(NS_META, 'name')
...
class ZipODFPackage(ODFPackage):
    ...
    def _tempPath(self, key):
        ...
        return os.path.join(self.tempdir, md5.new(key).hexdigest())
    ...
    def close(self):
        ...
        if not changed:
            for key in self.changed:
                self.zip.write(self._tempPath(key), compress_type=zipfile.ZIP_DEFLATED)
...
__all__ = [
 'ZipODFPackage', 'UnpackedODFPackage']