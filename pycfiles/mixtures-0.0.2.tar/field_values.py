...
# [GCC 8.4.0]
# Embedded file name: /home/agamdua/Developer/sandbox/mixtures/mixtures/field_values.py
# Compiled at: 2016-08-18 01:44:00
...
def get_factory_func(field):
    return getattr(FieldValue, (b'make_{}_field_value').format(field.__class__.__name__.split(b'Field')[0].lower()))
...
class StringFieldMixin(object):
    ...
    def get_random_string(cls, string_range):
        return (b'').join(random.choice(ascii_uppercase) for i in string_range)
...
class FieldHelperMixin(StringFieldMixin):
    ...
    def get_value_based_inclusive_interval(cls, field, max_value=None):
        ...
        if field.min_value is None:
            field.min_value = 0
        Interval = namedtuple(b'interval', [b'start', b'stop'])
...
class FieldValue(FieldHelperMixin):
    ...
    def make_email_field_value(cls, field):
        ...
        range_length = length - 12
        return (b'{}@example.com').format(cls.get_random_string(range(range_length)))
