...
# [GCC 8.4.0]
# Embedded file name: /home/agamdua/Developer/sandbox/mixtures/mixtures/mixtures.py
...
def make_fixture(model_class, **kwargs):
    ...
    assert len(all_fields) == len(values), (b'Mismatch in values, {} != {}').format(len(all_fields), len(values))
...
def get_fields(model_class):
    ...
    """
    return [ attr for attr, value in model_class.__dict__.items() if issubclass(type(value), (mongo.base.BaseField, mongo.EmbeddedDocumentField))
           ]
