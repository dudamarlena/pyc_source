...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/turnit/turnit.py
# Compiled at: 2019-10-14 21:07:23
# Size of source mod 2**32: 24081 bytes
...
import threading, numpy as np, scipy.optimize
...
def turnIt():
    ...
    def get_facet_XY_bounding_box(obj, i_facet):
        return (
         obj.vectors[i_facet, :, 0].min(), obj.vectors[i_facet, :, 1].min(),
    ...
    def get_support_height(**kw):
        ...
        for possible_support_facet_i in r_tree.intersection(tuple(pt[0:2])):
            is_in, height = is_XY_pt_in_XY_projection(pt, (kw['obj']), possible_support_facet_i, dot00=(kw['dot00']), dot11=(kw['dot11']),
              dot01=(kw['dot01']),
              invDenom=(kw['invDenom']))
            if is_in:
                if is_in and height > support_bottom and height < pt[2] - 0.0001:
    ...
    def get_amount_support(obj):
        ...
        normals = normals / (np.sqrt((obj.normals ** 2).sum(axis=1)) + 1e-09)[:, np.newaxis]
        up_facets = np.argwhere(np.logical_and(normals[:, 2] > 0.0, np.abs(denoms) > 1e-08))
        denoms = np.where(np.abs(denoms) > 1e-08, denoms, 1.0)
        ...
        for i_facet in need_support_facets:
            ...
            for ipt, pt in enumerate(pts):
                height = get_support_height(obj=obj, point=pt, rtree=up_index, minZ=minZ, dot00=dot00s, dot11=dot11s,
                  dot01=dot01s,
                  invDenom=invDenoms)
                support_heights.append(height)

            if sum(support_heights) > 0.0001:
    ...
    class Optimizer:
        ...
        def optimize(self):
            ...
            for ivec, vec in enumerate(test_vecs):
                ...
                try:
                    v, cost, rotobj, supports = self.cost_func([0, 0.0])
            ...
            try:
                result = scipy.optimize.minimize((self.min_func), (np.array([0.0, 0.0])), method='Nelder-Mead',
        ...
        def cost_func(self, x):
            ...
            if self.opt_state.get() != OPTIMIZE_STATE_RUNNING:
                ...
                raise OptException()
            return (
             vec, amount_support, rot_obj, supports)
    ...
    class App(tk.Frame):
                ...
                while not self.msg_queue.empty():
                    ...
                    if cmd == WRITE_TEXT_CMD:
                        ...
                        if self.best_obj is not None:
                            if self.out_STL_file.get().strip() != '':
        ...
        def initUI(self):
            ...
            in_button = tk.Button(self, text='Input STL File',
              width=16,
            ...
            out_button = tk.Button(self, text='Output STL File',
              width=16,
            ...
            self.text_output.grid(row=4, column=3, rowspan=3, columnspan=3, padx=(10,
                                                                                  10))

        def get_in_file(self):
            self.in_STL_file.set(askopenfilename(filetypes=(('STL File', '*.stl'),
                                                            ('STL File', '*.STL')), title='Choose an STL file.'))
            self.load_STL()
            self.read_crit_ang()
        ...
        def get_out_file(self):
            self.out_STL_file.set(asksaveasfilename(filetypes=(('STL File', '*.stl'),
                                                               ('STL File', '*.STL')), title='Save Oriented STL as...'))
