...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/django_evolution/mutations.py
...
from django_evolution.errors import CannotSimulate, SimulationFailure, EvolutionNotImplementedError
from django_evolution.signature import ATTRIBUTE_DEFAULTS, record_unique_together_applied
...
def create_field(proj_sig, field_name, field_type, field_attrs, parent_model):
    ...
    if field_type is ManyToManyField and parent_model is not None:
        ...
        if through_model:
            ...
            if field.rel.to == RECURSIVE_RELATIONSHIP_CONSTANT or to == parent_model._meta.object_name.lower():
                from_ = 'from_%s' % to
                to = 'to_%s' % to
            else:
            ...
            fields = SortedDict()
            fields['id'] = {'field_type': AutoField, 
               'primary_key': True}
            fields[from_] = {'field_type': ForeignKey, 
               'related_model': '%s.%s' % (parent_model.app_name,
                               parent_model._meta.object_name), 
               'related_name': '%s+' % through_model_name}
            fields[to] = {'field_type': ForeignKey, 
               'related_model': related_model, 
               'related_name': '%s+' % through_model_name}
            through_model_sig = {'meta': {'db_table': field._get_m2m_db_table(parent_model._meta), 
                        'managed': True, 
                        'auto_created': True, 
        ...
        if through_model_sig:
            through = MockModel(proj_sig, through_app_name, through_model_name, through_model_sig)
...
class MockMeta(object):
    ...
    def __init__(self, proj_sig, app_name, model_name, model_sig):
        ...
        self.meta = {'order_with_respect_to': None, 
           'has_auto_field': None, 
           'db_tablespace': None, 
           'swapped': False}
    ...
    def setup_fields(self, model, stub=False):
        for field_name, field_sig in self._model_sig['fields'].items():
            if not stub or field_sig.get('primary_key', False):
                ...
                field = create_field(self._proj_sig, field_name, field_type, field_sig, model)
    ...
    def get_field_by_name(self, name):
        return (
         self.get_field(name), None, True, None)
...
class MockModel(object):
    ...
    def __init__(self, proj_sig, app_name, model_name, model_sig, stub=False, db_name=None):
    ...
    def __eq__(self, other):
        return isinstance(other, MockModel) and self.app_name == other.app_name and self.model_name == other.model_name
...
class MonoBaseMutation(BaseMutation):
    ...
    def evolver(self, model, database_sig, database=None):
        if database is None:
            database = get_database_for_model_name(model.app_label, model.model_name)
        return EvolutionOperationsMulti(database, database_sig).get_evolver()

    def is_mutable(self, app_label, proj_sig, database_sig, database):
        db_name = database or get_database_for_model_name(app_label, self.model_name)
...
class DeleteField(MutateModelField):
    ...
    def mutate(self, mutator, model):
        ...
        field_type = field_sig.pop('field_type')
        field = create_field(mutator.proj_sig, self.field_name, field_type, field_sig, model)
        field_sig['field_type'] = field_type
        if field_type is models.ManyToManyField:
            mutator.add_sql(self, mutator.evolver.delete_table(field._get_m2m_db_table(model._meta)))
        else:
...
class AddField(MutateModelField):
    ...
    def __str__(self):
        if self.field_type.__module__.startswith('django.db.models'):
            ...
            field_prefix = ''
        str_output = [
         "'%(model_name)s', '%(field_name)s', %(field_type)s" % {'model_name': self.model_name, 
    ...
    def simulate(self, app_label, proj_sig, database_sig, database=None):
        ...
        if self.field_type is not models.ManyToManyField and not self.field_attrs.get('null', ATTRIBUTE_DEFAULTS['null']) and self.initial is None:
        ...
        model_sig['fields'][self.field_name] = {'field_type': self.field_type}
    ...
    def add_column(self, mutator, model):
        field = create_field(mutator.proj_sig, self.field_name, self.field_type, self.field_attrs, model)
    ...
    def add_m2m_table(self, mutator, model):
        ...
        related_model = MockModel(mutator.proj_sig, related_app_label, related_model_name, related_sig, mutator.database)
...
class RenameField(MutateModelField):
    ...
    def __init__(self, model_name, old_field_name, new_field_name, db_column=None, db_table=None):
    ...
    def mutate(self, mutator, model):
        ...
        old_field = create_field(mutator.proj_sig, self.old_field_name, field_type, old_field_sig, None)
        new_field = create_field(mutator.proj_sig, self.new_field_name, field_type, new_field_sig, None)
        old_field_sig['field_type'] = field_type
        new_model = MockModel(mutator.proj_sig, mutator.app_label, self.model_name, mutator.model_sig, db_name=mutator.database)
...
class ChangeField(MutateModelField):
    ...
    def simulate(self, app_label, proj_sig, database_sig, database=None):
        ...
        if 'null' in self.field_attrs and field_sig['field_type'] != models.ManyToManyField and not self.field_attrs['null'] and self.initial is None:
    ...
    def mutate(self, mutator, model):
        ...
        for attr_name in self.field_attrs.iterkeys():
            if attr_name not in mutator.evolver.supported_change_attrs:
                raise EvolutionNotImplementedError("ChangeField does not support modifying the '%s' attribute on '%s.%s'." % (
                 attr_name, self.model_name, self.field_name))

        new_field_attrs = {}
        for attr_name, attr_value in self.field_attrs.iteritems():
            old_attr_value = field_sig.get(attr_name, ATTRIBUTE_DEFAULTS[attr_name])
            if old_attr_value != attr_value:
                new_field_attrs[attr_name] = {'old_value': old_attr_value, 'new_value': attr_value}
...
class RenameModel(MutateModelField):
        ...
        for app_sig in proj_sig.itervalues():
            ...
            for model_sig in app_sig.itervalues():
                for field in model_sig['fields'].itervalues():
                    if 'related_model' in field and field['related_model'] == old_related_model:
    ...
    def mutate(self, mutator, model):
        ...
        new_model = MockModel(mutator.proj_sig, mutator.app_label, self.new_model_name, new_model_sig, db_name=mutator.database)
        evolver = mutator.evolver
        sql = evolver.rename_table(new_model, old_model_sig['meta']['db_table'], new_model_sig['meta']['db_table'])
        mutator.add_sql(self, sql)
...
class DeleteApplication(BaseMutation):
    ...
    def simulate(self, app_label, proj_sig, database_sig, database=None):
        if database:
            ...
            for model_name in app_sig.keys():
                ...
                if mutation.is_mutable(app_label, proj_sig, database_sig, database):
    ...
    def mutate(self, mutator):
        if mutator.database:
            ...
            for model_name in app_sig.keys():
                mutation = DeleteModel(model_name)
                if mutation.is_mutable(mutator.app_label, mutator.proj_sig, mutator.database_sig, mutator.database):
...
class ChangeMeta(MutateModelField):
    ...
    def mutate(self, mutator, model):
        if self.prop_name not in mutator.evolver.supported_change_meta:
            raise EvolutionNotImplementedError("ChangeMeta does not support modifying the '%s' attribute on '%s'." % (
