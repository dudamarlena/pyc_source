...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/django_evolution/diff.py
# Compiled at: 2018-06-14 23:17:51
from django.db import models
from django_evolution.errors import EvolutionException
from django_evolution.mutations import DeleteField, AddField, DeleteModel, ChangeField, ChangeMeta
from django_evolution.signature import ATTRIBUTE_DEFAULTS, has_index_together_changed, has_unique_together_changed
...
class Diff(object):
    ...
    def __init__(self, original, current):
        ...
        if self.original_sig.get('__version__', 1) != 1:
            raise EvolutionException('Unknown version identifier in original signature: %s', self.original_sig['__version__'])
        if self.current_sig.get('__version__', 1) != 1:
            raise EvolutionException('Unknown version identifier in target signature: %s', self.current_sig['__version__'])
        for app_name, old_app_sig in original.items():
            ...
            for model_name, old_model_sig in old_app_sig.items():
                ...
                if new_model_sig is None:
                    items = self.chain_set_default(self.changed, app_name, 'deleted')
                ...
                for field_name, old_field_data in old_fields.items():
                    ...
                    if new_field_data is None:
                        items = self.chain_set_default(self.changed, app_name, 'changed', model_name, 'deleted')
                        items.append(field_name)
                        continue
                    properties = set(old_field_data.keys())
                    properties.update(new_field_data.keys())
                    for prop in properties:
                        old_value = old_field_data.get(prop, ATTRIBUTE_DEFAULTS.get(prop, None))
                        new_value = new_field_data.get(prop, ATTRIBUTE_DEFAULTS.get(prop, None))
                        if old_value != new_value:
                            try:
                                if prop == 'field_type' and old_value().get_internal_type() == new_value().get_internal_type():
                                    continue
                            except TypeError:
                                pass

                            items = self.chain_set_default(self.changed, app_name, 'changed', model_name, 'changed', field_name)
                            items.append(prop)
                ...
                for field_name, new_field_data in new_fields.items():
                    ...
                    if old_field_data is None:
                        items = self.chain_set_default(self.changed, app_name, 'changed', model_name, 'added')
                        items.append(field_name)

                if has_unique_together_changed(old_model_sig, new_model_sig):
                    items = self.chain_set_default(self.changed, app_name, 'changed', model_name, 'meta_changed')
        ...
        for app_label, app_changes in self.changed.items():
            ...
            for model_name, change in app_changed.iteritems():
                ...
                for prop_name in meta_changed:
                    lines.append("    Meta property '%s' has changed" % prop_name)
        ...
        for app_label, app_changes in self.changed.items():
            for model_name, change in app_changes.get('changed', {}).items():
                ...
                for field_name in change.get('added', {}):
                    ...
                    if field_type is not models.ManyToManyField and not field_sig.get('null', ATTRIBUTE_DEFAULTS['null']):
                        add_params.append((
                         'initial',
                    ...
                    mutations.setdefault(app_label, []).append(AddField(model_name, field_name, **dict(add_params)))

                for field_name in change.get('deleted', []):
                    mutations.setdefault(app_label, []).append(DeleteField(model_name, field_name))
                ...
                for field_name, field_change in changed.iteritems():
                    ...
                    if 'null' in changed_attrs and current_field_sig['field_type'] != models.ManyToManyField and not current_field_sig.get('null', ATTRIBUTE_DEFAULTS['null']):
                ...
                for prop_name in ('unique_together', 'index_together'):
                    if prop_name in meta_changed:
                        mutations.setdefault(app_label, []).append(ChangeMeta(model_name, prop_name, model_sig['meta'][prop_name]))
