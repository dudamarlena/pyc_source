...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/django_evolution/db/common.py
...
from django_evolution.signature import add_index_to_database_sig, remove_index_from_database_sig
...
class BaseEvolutionOperations(object):
    ...
    def generate_table_ops_sql(self, mutator, ops):
        ...
        prev_op = None
        for op in ops:
            sql_result = self.generate_table_op_sql(mutator, op, prev_sql_result, prev_op)
    ...
    def generate_table_op_sql(self, mutator, op, prev_sql_result, prev_op):
        ...
        if op_type == 'add_column':
            ...
            sql_result.add(self.change_column_attrs(model, mutation, op['field'].name, op['new_attrs']))
            ...
            sql_result.add(evolve_func(model, op['old_value'], op['new_value']))
        elif op_type == 'sql':
            sql_result.add(op['sql'])
        else:
            raise EvolutionNotImplementedError('Unknown mutation operation "%s"' % op_type)
    ...
    def quote_sql_param(self, param):
        ...
        if isinstance(param, basestring):
            return "'%s'" % unicode(param).replace("'", "\\'")
        else:
            return param
    ...
    def rename_table(self, model, old_db_tablename, db_tablename):
        ...
        for field in model._meta.local_many_to_many:
            if field.rel and field.rel.through and field.rel.through._meta.db_table == old_db_tablename:
        ...
        for relto in models:
            sql_result.add_pre_sql(creation.sql_remove_table_constraints(relto, remove_refs, style))

        sql_result.add(self.get_rename_table_sql(model, old_db_tablename, db_tablename))
        for relto in models:
            for rel_class, f in refs[relto]:
                if rel_class._meta.db_table == old_db_tablename:
                    rel_class._meta.db_table = db_tablename
                rel_class._meta.db_table = truncate_name(rel_class._meta.db_table, max_name_length)

            sql_result.add_post_sql(creation.sql_for_pending_references(relto, style, refs))

        return sql_result

    def delete_column(self, model, f):
        return AlterTableSQLResult(self, model, [
         {'op': 'DROP COLUMN', 
            'column': f.column, 
            'params': [
                     'CASCADE']}])
    ...
    def add_m2m_table(self, model, f):
        ...
        if f.rel.through:
            ...
            for refto, refs in sorted(references.iteritems(), key=lambda i: repr(i)):
                pending_references.setdefault(refto, []).extend(refs)
                sql.extend(creation.sql_for_pending_references(refto, style, pending_references))
    ...
    def add_column(self, model, f, initial):
                ...
                if callable(initial):
                    ...
                    sql_result.add_sql([
                     'UPDATE %s SET %s = %s WHERE %s IS NULL;' % (
                      ...
                      initial(), qn(f.column))])
                    if not f.null:
                        sql_result.add_sql(self.set_field_null(model, f, f.null))
                else:
                    sql_result.add_alter_table([
                     {'op': 'ADD COLUMN', 
                        'column': f.column, 
        ...
        if f.unique or f.primary_key:
            self.record_index(model, [f], use_constraint_name=True, unique=True)
        return sql_result
    ...
    def create_index(self, model, f):
        ...
        return SQLResult(self.connection.creation.sql_indexes_for_field(model, f, style))
    ...
    def get_new_index_name(self, model, fields, unique=False):
        ...
        return truncate_name('%s_%s' % (model._meta.db_table, colname), self.connection.ops.max_name_length())
    ...
    def change_column_attrs(self, model, mutation, field_name, new_attrs):
        ...
        for attr_name, attr_info in new_attrs.iteritems():
            ...
            try:
                sql_result = evolve_func(model, mutation, field, attr_info['old_value'], attr_info['new_value'])
                assert not sql_result or isinstance(sql_result, SQLResult)
            except Exception as e:
                logging.critical('Error running database evolver function %s: %s', method_name, e, exc_info=1)
                raise
    ...
    def change_column_attr_null(self, model, mutation, field, old_value, new_value):
        ...
        if not new_value and initial is not None:
            sql_prefix = 'UPDATE %(table_name)s SET %(column_name)s = %%s WHERE %(column_name)s IS NULL;' % {'table_name': qn(opts.db_table), 
    ...
    def change_column_attr_max_length(self, model, mutation, field, old_value, new_value):
    ...
    def change_column_attr_db_column(self, model, mutation, field, old_value, new_value):
    ...
    def change_column_attr_db_table(self, model, mutation, field, old_value, new_value):
        """Returns the SQL for changing the table for a ManyToManyField."""
        return self.rename_table(model, old_value, new_value)

    def change_column_attr_db_index(self, model, mutation, field, old_value, new_value):
    ...
    def change_column_attr_unique(self, model, mutation, field, old_value, new_value):
        ...
        """
        if new_value:
            constraint_name = self.get_new_index_name(model, [field], unique=True)
            self.record_index(model, [field], index_name=constraint_name, unique=True)
        else:
            constraint_name = self.find_index_name(model, [field.column], unique=True)
            self.remove_recorded_index(model, constraint_name, unique=True)
        return self.get_change_unique_sql(model, field, new_value, constraint_name, mutation.initial)

    def get_change_unique_sql(self, model, field, new_unique_value, constraint_name, initial):
        ...
        qn = self.connection.ops.quote_name
        if new_unique_value:
            alter_table_item = {'sql': 'ADD CONSTRAINT %s UNIQUE(%s)' % (
                     qn(constraint_name), qn(field.column))}
        else:
            alter_table_item = {'sql': 'DROP CONSTRAINT %s' % qn(constraint_name)}
        return AlterTableSQLResult(self, model, [alter_table_item])

    def change_meta_unique_together(self, model, old_unique_together, new_unique_together):
        ...
        for field_names in to_remove:
            ...
            if index_name:
                ...
                sql_result.add_sql(self.get_drop_unique_constraint_sql(model, index_name))
        ...
        for field_names in new_unique_together:
            ...
            index_name = self.find_index_name(model, columns, unique=True)
            if not index_name:
                index_name = self.get_new_index_name(model, fields, unique=True)
                sql_result.add_sql(self.create_unique_index(model, index_name, fields))
    ...
    def change_meta_index_together(self, model, old_index_together, new_index_together):
        ...
        for field_names in new_index_together:
            ...
            if not index_name:
                ...
                sql.extend(self.connection.creation.sql_indexes_for_fields(model, fields, style))
    ...
    def find_index_name(self, model, column_names, unique=False):
        ...
        if table_name in self.database_sig:
            ...
            for index_name, index_info in indexes.iteritems():
                if index_info['columns'] == column_names and index_info['unique'] == unique:
                    return index_name
    ...
    def remove_field_constraints(self, field, opts, models, refs):
        ...
        if field.primary_key:
            ...
            for f in opts.local_many_to_many:
                if f.rel and f.rel.through:
                    ...
                    for m2m_f in through._meta.local_fields:
                        if m2m_f.rel and m2m_f.rel.to._meta.db_table == opts.db_table and m2m_f.rel.field_name == field.column:
            ...
            style = color.no_style()
            for relto in models:
                sql.extend(creation.sql_remove_table_constraints(relto, remove_refs, style))

        return sql

    def add_primary_key_field_constraints(self, old_field, new_field, models, refs):
        sql = []
    ...
    def record_index(self, model, fields, use_constraint_name=False, index_name=None, unique=False):
        ...
        if not index_name and use_constraint_name:
            index_name = truncate_name('%s_%s_key' % (model._meta.db_table, fields[0].column), self.connection.ops.max_name_length())
        assert index_name or not unique
        add_index_to_database_sig(self, self.database_sig, model, fields, index_name=index_name, unique=unique)

    def remove_recorded_index(self, model, index_name, unique=False):
        """Removes an index from the database signature."""
        remove_index_from_database_sig(self.database_sig, model, index_name, unique=unique)
    ...
    def _are_ops_mergeable(self, op1, op2):
        ...
        """
        return op1['type'] in self.mergeable_ops and op2['type'] in self.mergeable_ops