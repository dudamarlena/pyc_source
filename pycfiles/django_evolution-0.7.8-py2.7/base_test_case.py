...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/django_evolution/tests/base_test_case.py
...
from django_evolution.tests.utils import create_test_proj_sig, deregister_models, execute_test_sql, register_models_multi, test_sql_mapping
...
class EvolutionTestCase(TransactionTestCase):
    ...
    def setUp(self):
        ...
        if self.default_base_model:
            self.set_base_model(self.default_base_model, pre_extra_models=self.default_pre_extra_models, extra_models=self.default_extra_models)
        return
    ...
    def set_base_model(self, model, name=None, extra_models=[], pre_extra_models=[], db_name=None):
        ...
        self.start = self.register_model(model, name, register_indexes=True, db_name=db_name)
    ...
    def perform_evolution_tests(self, model, evolutions, diff_text=None, expected_hint=None, sql_name=None, model_name=None, end=None, end_sig=None, expect_noop=False, rescan_indexes=True, use_hinted_evolutions=False, perform_simulations=True, perform_mutations=True, db_name=None):
        model_name = model_name or self.default_model_name
        db_name = db_name or self.default_database_name
        if end is None or end_sig is None:
            end, end_sig = self.make_end_signatures(model, model_name, db_name)
        d = self.perform_diff_test(end_sig, diff_text, expected_hint, expect_empty=expect_noop)
        ...
        if perform_mutations:
            self.perform_mutations(evolutions, end, end_sig, sql_name, rescan_indexes=rescan_indexes, db_name=db_name)
        return

    def perform_diff_test(self, end_sig, diff_text, expected_hint, expect_empty=False):
        ...
        if not expect_empty:
            ...
            if expected_hint is not None:
                self.assertEqual([ str(e) for e in d.evolution()['tests'] ], expected_hint)
        return d

    def perform_simulations(self, evolutions, end_sig, ignore_apps=False, db_name=None):
        db_name = db_name or self.default_database_name
        self.test_database_sig = self.copy_sig(self.database_sig)
        test_sig = self.copy_sig(self.start_sig)
        for mutation in evolutions:
            mutation.simulate('tests', test_sig, self.test_database_sig, database=db_name)

        d = Diff(test_sig, end_sig)
        self.assertTrue(d.is_empty(ignore_apps=ignore_apps))

    def perform_mutations(self, evolutions, end, end_sig, sql_name, rescan_indexes=True, db_name=None):

        def run_mutations():
            app_mutator = AppMutator('tests', test_sig, self.test_database_sig, db_name)
        ...
        self.test_database_sig = self.copy_sig(self.database_sig)
        test_sig = self.copy_sig(self.start_sig)
        sql = execute_test_sql(self.start, end, run_mutations, database_sig=self.test_database_sig, rescan_indexes=rescan_indexes, return_sql=True, database=db_name)
        if sql_name is not None:
            print "** Comparing SQL against '%s'" % sql_name
            self.assertEqual(('\n').join(sql), self.get_sql_mapping(sql_name, db_name))
        return
    ...
    def register_model(self, model, name, db_name=None, **kwargs):
        ...
        return register_models_multi(self.database_sig, 'tests', db_name, *models, **kwargs)

    def create_test_proj_sig(self, model, name, extra_models=[], pre_extra_models=[]):
        return create_test_proj_sig(*(self.pre_extra_models + pre_extra_models + [(name, model)] + extra_models + self.extra_models))
