...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/django_evolution/management/commands/evolve.py
...
from django_evolution.utils import execute_sql, get_app_label, get_evolutions_path, write_sql
...
class Command(BaseCommand):
    option_list = BaseCommand.option_list + (
     ...
     make_option('--hint', action='store_true', dest='hint', default=False, help='Generate an evolution script that would update the app.'),
     make_option('--purge', action='store_true', dest='purge', default=False, help='Generate evolutions to delete stale applications.'),
     make_option('--sql', action='store_true', dest='compile_sql', default=False, help='Compile a Django evolution script into SQL.'),
     make_option('-w', '--write', metavar='EVOLUTION_NAME', action='store', dest='write_evolution_name', default=None, help='Write the generated evolutions to files with the given evolution name in each affected app\'s "evolutions" paths.'),
     make_option('-x', '--execute', action='store_true', dest='execute', default=False, help='Apply the evolution to the database.'),
     make_option('--database', action='store', dest='database', help='Nominates a database to synchronize.'))
    if '--verbosity' not in [ opt.get_opt_string() for opt in BaseCommand.option_list
                            ]:
        option_list += (
         make_option('-v', '--verbosity', action='store', dest='verbosity', default='1', type='choice', choices=['0', '1', '2'], help='Verbosity level; 0=minimal output, 1=normal output, 2=all output'),)
    ...
    def evolve(self, *app_labels, **options):
        ...
        sql = []
        try:
            latest_version = Version.objects.current_version(using=self.database)
    ...
    def evolve_app(self, app):
        ...
        mutations = [ mutation for mutation in temp_mutations if mutation.is_mutable(app_label, self.old_proj_sig, self.database_sig, self.database)
                    ]
        if mutations:
            app_sql = [
             '', '-- Evolve application %s' % app_label]
            self.evolution_required = True
            app_mutator = AppMutator(app_label, self.old_proj_sig, self.database_sig, self.database)
            ...
            self.simulated = app_mutator.can_simulate or False
        self.new_evolutions.extend(Evolution(app_label=app_label, label=label) for label in evolutions)
        if not self.execute:
            if self.compile_sql:
                ...
                if self.hint and self.write_evolution_name:
                    evolutions_filename = os.path.join(get_evolutions_path(app), self.write_evolution_name + '.py')
                    if not evolutions_filename:
    ...
    def purge_apps(self):
        ...
        if self.diff.deleted:
            ...
            for app_label in self.diff.deleted:
                if delete_app.is_mutable(app_label, self.old_proj_sig, self.database_sig, self.database):
                    app_mutator = AppMutator(app_label, self.old_proj_sig, self.database_sig, self.database)
            ...
            if not self.execute:
                if self.compile_sql:
                    ...
                    self.stdout.write('The following application(s) can be purged:\n')
                    for app_label in self.diff.deleted:
                        self.stdout.write('    %s\n' % app_label)
    ...
    def check_simulation(self):
        if self.simulated:
            ...
            if not diff.is_empty(not self.purge):
                if self.hint:
                    self.stdout.write(self.style.ERROR('Your models contain changes that Django Evolution cannot resolve automatically.\n'))
                    self.stdout.write('This is probably due to a currently unimplemented mutation type.\n')
                    self.stdout.write('You will need to manually construct a mutation to resolve the remaining changes.\n')
                else:
                    self.stdout.write(self.style.ERROR('The stored evolutions do not completely resolve all model changes.\n'))
                    self.stdout.write('Run `./manage.py evolve --hint` to see a suggestion for the changes required.\n')
                self.stdout.write('\nThe following are the changes that could not be resolved:\n%s\n' % diff)
                raise CommandError('Your models contain changes that Django Evolution cannot resolve automatically.')
        else:
            self.stdout.write(self.style.NOTICE('Evolution could not be simulated, possibly due to raw SQL mutations\n'))
        ...
        if self.execute:
                ...
                try:
                    for app_label, app_sql in sql:
                        if app_label and self.verbosity > 0:
                            self.stdout.write('Applying database evolutions for %s...\n' % app_label)
                    ...
                    self.stdout.write(self.style.ERROR('Database evolutions for %s failed!' % app_label))
                    if hasattr(e, 'last_sql_statement'):
                        self.stdout.write(self.style.ERROR('The SQL statement was: %s' % e.last_sql_statement))
                    self.stdout.write(self.style.ERROR('The database error was: %s\n' % e))
            ...
            if self.hint:
                if self.write_evolution_name:
                    self.stdout.write('\nThe following evolution files were written. Verify the contents and add them\nto the SEQUENCE lists in each __init__.py.\n\n')
                    for filename, hinted_evolution in self.written_hint_files:
                        ...
                        if not os.path.exists(evolutions_dir):
                            os.mkdir(evolutions_dir, 493)
                        with open(filename, 'w') as (fp):
                            fp.write(hinted_evolution)
                        self.stdout.write('  * %s\n' % os.path.relpath(filename))

            else:
                self.stdout.write("Run './manage.py evolve --execute' to apply evolution.\n")
        return
    ...
    def generate_hint(self, app, app_label, mutations):
        ...
        lines += [
         ...
         '']
        lines.append('from django_evolution.mutations import %s' % (', ').join(sorted(mutation_types)))
        lines += sorted(imports)
        if project_imports:
            lines += [''] + sorted(project_imports)
        lines += [
         '', '', 'MUTATIONS = ['] + [ '    %s,' % mutation for mutation in mutations ] + [
         ']']
        if not self.write_evolution_name:
            lines.append('#----------------------')
        return ('\n').join(lines)