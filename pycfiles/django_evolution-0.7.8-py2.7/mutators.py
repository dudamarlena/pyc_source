...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/django_evolution/mutators.py
# Compiled at: 2018-06-14 23:17:51
import copy, logging
from django_evolution.db import EvolutionOperationsMulti
from django_evolution.errors import CannotSimulate
from django_evolution.mutations import AddField, ChangeField, ChangeMeta, DeleteField, DeleteModel, MockModel, MonoBaseMutation, MutateModelField, RenameField, RenameModel
...
class ModelMutator(object):
    ...
    def __init__(self, app_mutator, model_name, app_label, proj_sig, database_sig, database):
        ...
        self.database = database or get_database_for_model_name(app_label, model_name)
        self.can_simulate = True
        self._ops = []
        self._finalized = False
        assert self.database
        evolution_ops = EvolutionOperationsMulti(self.database, self.database_sig)
    ...
    def create_model(self):
        ...
        """
        return MockModel(self.proj_sig, self.app_label, self.model_name, self.model_sig, db_name=self.database)
    ...
    def add_column(self, mutation, field, initial):
        ...
        self._ops.append({'type': 'add_column', 
           'mutation': mutation, 
           'field': field, 
           'initial': initial})
    ...
    def change_column(self, mutation, field, new_attrs):
        ...
        """
        assert not self._finalized
        self._ops.append({'type': 'change_column', 
           'mutation': mutation, 
           'field': field, 
           'new_attrs': new_attrs})
    ...
    def delete_column(self, mutation, field):
        ...
        """
        assert not self._finalized
        self._ops.append({'type': 'delete_column', 
           'mutation': mutation, 
           'field': field})
    ...
    def delete_model(self, mutation):
        ...
        """
        assert not self._finalized
        self._ops.append({'type': 'delete_model', 
           'mutation': mutation})
    ...
    def change_meta(self, mutation, prop_name, new_value):
        ...
        """
        assert not self._finalized
        self._ops.append({'type': 'change_meta', 
           'mutation': mutation, 
           'prop_name': prop_name, 
    ...
    def run_simulation(self, mutation):
        try:
            mutation.simulate(self.app_label, self.proj_sig, self.database_sig, self.database)
...
class AppMutator(object):
    ...
    def run_mutation(self, mutation):
        ...
        if isinstance(mutation, MutateModelField):
            if self._last_model_mutator and mutation.model_name == self._last_model_mutator.model_name:
                model_mutator = self._last_model_mutator
            else:
                self._finalize_model_mutator()
                model_mutator = ModelMutator(self, mutation.model_name, self.app_label, self.proj_sig, self.database_sig, self.database)
    ...
    def _preprocess_mutations(self, mutations):
        ...
        try:
            for mutation_batch in mutation_batches:
                result_mutations.extend(self._process_mutation_batch(mutation_batch))

        except CannotSimulate:
            logging.warning('Unable to pre-process mutations for optimization. %s contains a mutation that cannot be smimulated.', self.app_label)
    ...
    def _process_mutation_batch(self, mutation_batch):
        ...
        if can_process:
            ...
            for mutation in reversed(mutations):
                ...
                if isinstance(mutation, AddField):
                    ...
                    if mutation_id in deleted_fields:
                        ...
                        last_change_mutation = last_change_mutations[mutation_id]
                        self._copy_change_attrs(last_change_mutation, mutation)
                    ...
                    if mutation_id in deleted_fields:
                        ...
                        last_change_mutation = last_change_mutations.get(mutation_id)
                    ...
                    old_mutation_id = self._get_mutation_id(mutation, mutation.old_field_name)
                    new_mutation_id = self._get_mutation_id(mutation, mutation.new_field_name)
                    ...
                    if new_mutation_id in renames:
                        self._rename_dict_key(renames, new_mutation_id, old_mutation_id)
                    else:
                        renames[old_mutation_id] = {'can_process': False, 
                           'mutations': []}
                    renames[old_mutation_id]['mutations'].append(mutation)
                    if new_mutation_id in last_change_mutations:
                        self._rename_dict_key(last_change_mutations, new_mutation_id, old_mutation_id)
                    ...
                    if new_model_name in model_renames:
                        self._rename_dict_key(model_renames, new_model_name, old_model_name)
                    else:
                        model_renames[old_model_name] = {'can_process': False, 
                           'mutations': []}
                    model_renames[old_model_name]['mutations'].append(mutation)
                elif isinstance(mutation, ChangeMeta) and mutation.prop_name == 'unique_together':
                    if mutation.model_name not in unique_together:
                        unique_together[mutation.model_name] = mutation.new_value
                if remove_mutation:
                ...
                for mutation in mutations:
                    ...
                    if isinstance(mutation, AddField):
                        ...
                        if mutation_id in renames:
                            ...
                            mutation.field_name = rename_mutation.new_field_name
                            if rename_mutation.db_column:
                                mutation.field_attrs['db_column'] = rename_mutation.db_column
                            removed_mutations.update(rename_mutations)
                        related_model = mutation.field_attrs.get('related_model')
                        if related_model:
                            ...
                            if rename_info:
                                ...
                                rename_mutations = rename_info['mutations']
                                rename_mutation = rename_mutations[0]
                                mutation.field_attrs['related_model'] = ('.').join([
                                 app_label,
                        ...
                        if mutation_id in noop_fields:
                            ...
                            if rename_info['can_process']:
                                rename_mutation = rename_info['mutations'][0]
                                mutation.field_name = rename_mutation.old_field_name
                    elif isinstance(mutation, RenameField):
                        old_mutation_id = self._get_mutation_id(mutation, mutation.old_field_name)
                        new_mutation_id = self._get_mutation_id(mutation, mutation.new_field_name)
                        ...
                        if old_mutation_id in renames:
                            ...
                            rename_info['can_process'] = True
                            rename_mutations = rename_info['mutations']
                            self._rename_dict_key(renames, old_mutation_id, new_mutation_id)
                            mutation.new_field_name = rename_mutations[0].new_field_name
                        ...
                        if rename_info and rename_info['can_process']:
                            rename_mutation = rename_info['mutations'][0]
                            mutation.model_name = rename_mutation.old_model_name
                    elif isinstance(mutation, RenameModel):
                        old_model_name = mutation.old_model_name
                        new_model_name = mutation.new_model_name
                        if old_model_name in model_renames:
                            ...
                            rename_mutations = rename_info['mutations']
                            rename_mutation = rename_mutations[0]
                            self._rename_dict_key(model_renames, old_model_name, new_model_name)
                            mutation.new_model_name = rename_mutation.new_model_name
                            if rename_mutation.db_table:
                                mutation.db_table = rename_mutation.db_table
                            removed_mutations.update(rename_mutations[:-1])
                            rename_info['mutations'] = [rename_mutations[(-1)]]
                            app_sig = self.proj_sig[self.app_label]
                            if new_model_name in app_sig and old_model_name not in app_sig:
                                remove_mutation = True
                    elif isinstance(mutation, ChangeMeta) and mutation.prop_name == 'unique_together':
                        if mutation.model_name in unique_together:
            ...
            mutations = [ mutation for mutation in mutations if mutation not in removed_mutations
                        ]
            mutations_by_model = dict([ (model_name, []) for model_name in model_names
                                      ])
            for mutation in mutations:
                mutations_by_model[mutation.model_name].append(mutation)

            mutations = [ mutation for model_name in sorted(model_names) for mutation in mutations_by_model[model_name]
                        ]
        return mutations
    ...
    def _get_mutation_id(self, mutation, field_name=None):
        ...
        return (
         mutation.model_name, field_name or mutation.field_name)