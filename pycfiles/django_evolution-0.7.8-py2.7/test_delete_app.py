...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/django_evolution/tests/test_delete_app.py
...
class DeleteAppTests(EvolutionTestCase):
    ...
    def _perform_delete_app_test(self, sql_name, database=None):
        self.set_base_model(self.default_base_model, extra_models=self.default_extra_models, db_name=database)
        ...
        self.perform_simulations([mutation], end_sig, ignore_apps=True, db_name=database)
        test_database_sig = self.copy_sig(self.database_sig)
        test_sig = self.copy_sig(self.start_sig)
        app_mutator = AppMutator('tests', test_sig, test_database_sig, database)
