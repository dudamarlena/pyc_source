...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/django_evolution/tests/test_sql_mutation.py
...
class OrderingTests(EvolutionTestCase):
    ...
    def test_add_fields_cannot_simulate(self):
        ...
        self.assertRaisesMessage(CannotSimulate, 'Cannot simulate SQLMutations', lambda : self.perform_evolution_tests(AddFieldsModel, [
    ...
    def test_add_fields_simulation_functions(self):
        ...
        def update_first_two(app_label, proj_sig):
            ...
            model_sig['fields']['added_field1'] = {'field_type': models.IntegerField, 
               'null': True}
            model_sig['fields']['added_field2'] = {'field_type': models.IntegerField, 
               'null': True}

        def update_third(app_label, proj_sig):
            app_sig = proj_sig[app_label]
            model_sig = app_sig['TestModel']
            model_sig['fields']['added_field3'] = {'field_type': models.IntegerField, 
               'null': True}

        self.perform_evolution_tests(AddFieldsModel, [
         SQLMutation('first-two-fields', self.get_sql_mapping('AddFirstTwoFields').split('\n'), update_first_two),
         SQLMutation('third-field', self.get_sql_mapping('AddThirdField').split('\n'), update_third)], "In model tests.TestModel:\n    Field 'added_field1' has been added\n    Field 'added_field3' has been added\n    Field 'added_field2' has been added", sql_name='SQLMutationOutput')