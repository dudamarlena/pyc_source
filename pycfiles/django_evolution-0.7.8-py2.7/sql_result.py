...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/django_evolution/db/sql_result.py
...
class AlterTableSQLResult(SQLResult):
    ...
    def to_sql(self):
        ...
        if self.alter_table:
            ...
            for statements, sql_params in alter_table_batches:
                alter_table_sql = 'ALTER TABLE %s %s;' % (
    ...
    def _preprocess_alter_table_ops(self):
        ...
        for item in self.alter_table:
            alter_table_attrs = []
            op = item.get('op', 'sql')
            if op == 'MODIFY COLUMN' and prev_op == op and prev_item['column'] == item['column']:
                if prev_item['db_type'] == item['db_type']:
                    if 'params' in item:
                    ...
                    if 'sql_params' in item:
                        prev_item.setdefault('sql_params', []).extend(item['sql_params'])
                    continue
        ...
        for item in new_alter_table_items:
            ...
            if op == 'sql':
                ...
                if 'params' in item:
                    alter_table_attrs.extend([ param for param in item['params'] if param
                                             ])
        ...
        return [ alter_table_batch for alter_table_batch in alter_table_batches if alter_table_batch[0]
               ]

    def __repr__(self):
        return '<AlterTableSQLResult: pre_sql=%r, sql=%r, post_sql=%r, alter_table=%r>' % (
