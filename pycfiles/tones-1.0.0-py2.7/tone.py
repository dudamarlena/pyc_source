...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tones/tone.py
# Compiled at: 2018-11-18 03:32:32
import struct, tones, tones._utils as utils
...
class Samples(list):
    ...
    def serialize(self):
        ...
        :rtype: bytes
        """
        return bytes('').join([ bytes(self._pack_sample(s)) for s in self ])
...
class Tone(object):
    ...
    _table_generators = {tones.SINE_WAVE: _sine_wave_table, 
    ...
    def __init__(self, rate, amplitude, wavetype):
        ...
        :param float amplitude: Tone amplitude, where 1.0 is the max. sample             value and 0.0 is total silence
        """
        try:
    ...
    def samples(self, num, frequency, endfrequency=None, attack=0.05, decay=0.05, phase=0.0, vphase=0.0, vibrato_frequency=None, vibrato_variance=20.0):
        """
        ...
        :param float endfrequency: If not None, the tone frequency will change             between 'frequency' and 'endfrequency' in increments of 1ms over             all samples
        ...
        """
        points = None
        if endfrequency is not None:
            points = self._linear_pitch_change(num, frequency, endfrequency)
        if vibrato_frequency is not None:
            vpoints, vphase = self._vibrato_pitch_change(num, vibrato_frequency, vibrato_variance, vphase)
            if points is None:
