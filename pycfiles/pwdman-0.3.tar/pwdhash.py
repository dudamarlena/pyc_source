...
# Embedded file name: build/bdist.linux-x86_64/egg/pwdhash.py
# Compiled at: 2016-11-13 16:00:14
import base64, hmac, itertools, re, sys
try:
    import hashlib
    _md5 = hashlib.md5
except ImportError:
...
_domains = [
 'ab.ca', 'ac.ac', 'ac.at', 'ac.be', 'ac.cn', 'ac.il',
...
def generate(password, uri):
    ...
    nonalphanumeric = len(re.findall('\\W', password)) != 0
...
def apply_constraints(phash, size, nonalphanumeric):
    ...
    extras = itertools.chain((ord(ch) for ch in phash[starting_size:]), itertools.repeat(0))
...
def console_main():
    ...
    if sys.platform == 'darwin':
        import subprocess
        try:
            pb = subprocess.Popen('pbcopy', stdin=subprocess.PIPE, stdout=open('/dev/null', 'w'), stderr=open('/dev/null', 'w'))
