...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bagit_profile.py
# Compiled at: 2020-03-24 08:55:44
# Size of source mod 2**32: 24238 bytes
"""
...
"""
import json, logging, mimetypes, sys
...
class Profile(object):
        ...
        for fn, msg, min_version in [
         ...
         (
          self.validate_tag_files_allowed, 'Tag files not allowed', (1, 2, 0))]:
            try:
                if min_version:
                    if self.profile_version_info < min_version:
        ...
        if 'Accept-BagIt-Version' in profile:
            for version_number in profile['Accept-BagIt-Version']:
                if not isinstance(version_number, basestring):
                    raise ProfileValidationError('Version number "%s" in "Accept-BagIt-Version" is not a string!' % version_number)
    ...
    def validate_bagit_profile_bag_info(self, profile):
        if 'Bag-Info' in profile:
            for tag in profile['Bag-Info']:
                ...
                if self.profile_version_info >= (1, 3, 0) and 'description' in config:
                    isinstance(config['description'], basestring) or self._fail("%s: Profile Bag-Info '%s' tag 'description' property, when present, must be a string." % (
                     profile, tag))
    ...
    def validate_bag_info(self, bag):
        ...
        if not exists(path_to_baginfotxt):
            ...
            if profile_id_tag not in bag_info:
                self._fail(("%s: Required '%s' tag is not in bag-info.txt." + ignore_tag_case_help) % (
                 bag, self._baginfo_profile_id_tag))
            else:
                if bag_info[profile_id_tag] != self.url:
                    self._fail("%s: '%s' tag does not contain this profile's URI: <%s> != <%s>" % (
            ...
            if 'required' in config:
                if config['required'] is True:
                    if normalized_tag not in bag_info:
                        self._fail(("%s: Required tag '%s' is not present in bag-info.txt." + ignore_tag_case_help) % (
                         bag, tag))
            if 'values' in config and normalized_tag in bag_info:
                if bag_info[normalized_tag] not in config['values']:
                    self._fail("%s: Required tag '%s' is present in bag-info.txt but does not have an allowed value ('%s')." % (
                ...
                if 'repeatable' in config and config['repeatable'] is False:
                    ...
                    if isinstance(value, list):
                        self._fail("%s: Nonrepeatable tag '%s' occurs %s times in bag-info.txt." % (
                         bag, tag, len(value)))

        return True

    def normalize_tag(self, tag):
        if not self.ignore_baginfo_tag_case:
            return tag
        return tag.lower()
    ...
    def validate_manifests_required(self, bag):
        for manifest_type in self.profile['Manifests-Required']:
            ...
            if not exists(path_to_manifest):
                self._fail("%s: Required manifest type '%s' is not present in Bag." % (
                 bag, manifest_type))
    ...
    def validate_tag_manifests_required(self, bag):
        ...
        for tag_manifest_type in self.profile['Tag-Manifests-Required']:
            ...
            if not exists(path_to_tag_manifest):
                self._fail("%s: Required tag manifest type '%s' is not present in Bag." % (
    ...
    def validate_tag_manifests_allowed(self, bag):
        return self._validate_allowed_manifests(bag, manifest_type='tag', manifests_present=(self.manifest_algorithms(bag.tagmanifest_files())),
    ...
    def validate_payload_manifests_allowed(self, bag):
        return self._validate_allowed_manifests(bag, manifest_type='payload', manifests_present=(self.manifest_algorithms(bag.manifest_files())),
          allowed_attribute='Manifests-Allowed',
          required_attribute='Manifests-Required')

    def _validate_allowed_manifests(self, bag, manifest_type=None, manifests_present=None, allowed_attribute=None, required_attribute=None):
    ...
    def validate_tag_files_allowed(self, bag):
        ...
        if required_but_not_allowed:
            self._fail("%s: Required tag files '%s' not listed in Tag-Files-Allowed" % (
        ...
        for tag_file in find_tag_files(bag.path):
            ...
            if not fnmatch_any(tag_file, allowed):
                self._fail("%s: Existing tag file '%s' is not listed in Tag-Files-Allowed." % (
                 bag, tag_file))
    ...
    def validate_tag_files_required(self, bag):
        ...
        for tag_file in self.profile['Tag-Files-Required']:
            ...
            if not exists(path_to_tag_file):
                self._fail("%s: Required tag file '%s' is not present in Bag." % (
    ...
    def validate_accept_bagit_version(self, bag):
        ...
        if actual not in allowed:
            self._fail("%s: Bag version '%s' is not in list of allowed values: %s" % (
             bag, actual, allowed))
        return True
    ...
    def validate_serialization(self, path_to_bag):
        ...
        elif self.profile['Serialization'] == 'required':
            if isdir(path_to_bag):
                self._fail('%s: Bag serialization is required but Bag is a directory.' % path_to_bag)
        else:
            if self.profile['Serialization'] == 'forbidden':
                if isfile(path_to_bag):
...
def find_tag_files(bag_dir):
    for root, _, basenames in walk(bag_dir):
        ...
        for basename in basenames:
            if fnmatch(reldir, 'data*') or reldir == '.':
                if fnmatch_any(basename, [
                 'manifest-*.txt',
                 'bag-info.txt',
                 'tagmanifest-*.txt',
                 'bagit.txt',
...
def _main():
    ...
    parser.add_argument('-i',
      '--ignore-baginfo-tag-case', dest='ignore_baginfo_tag_case',
      action='store_true',
      help='Ignore capitalization for Bag-Info tag names. Default: %(default)s')
    parser.add_argument('--log',
      dest='logdir', help='Log directory. Default: %(default)s')
    ...
    parser.add_argument('--file',
      help='Load profile from FILE, not by URL. Default: %(default)s.')
    ...
    if args.file:
        with open(args.file, 'r') as (local_file):
            profile = Profile(profile_url, profile=(local_file.read()), ignore_baginfo_tag_case=(args.ignore_baginfo_tag_case))
    else:
