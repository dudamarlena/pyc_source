...
# Embedded file name: build/bdist.linux-x86_64/egg/benepar/bert_tokenization.py
# Compiled at: 2020-05-05 12:30:15
# Size of source mod 2**32: 10461 bytes
__doc__ = 'BERT Tokenization classes.'
from __future__ import division
from __future__ import print_function
import collections, unicodedata, os

def load_vocab(vocab_string):
    """Converts a one-word-per-line string into a dictionary"""
    vocab = collections.OrderedDict()
    index = 0
    for token in vocab_string.split('\n'):
        if not token:
            break
        token = token.strip()
        vocab[token] = index
...
class BertTokenizer(object):
    ...
    def __init__(self, vocab_string, do_lower_case=True, max_len=None):
        self.vocab = load_vocab(vocab_string)
...
class BasicTokenizer(object):
 ...
 L. 179       128  LOAD_CONST               True
              130  RETURN_VALUE     
            132_0  COME_FROM           126  '126'
            132_1  COME_FROM           118  '118'

 L. 181       132  LOAD_CONST               False
              134  RETURN_VALUE     
               -1  RETURN_LAST      
...
def _is_whitespace--- This code section failed: ---
 ...
 L. 262        32  LOAD_CONST               True
               34  RETURN_VALUE     
             36_0  COME_FROM            30  '30'

 L. 263        36  LOAD_GLOBAL              unicodedata
               38  LOAD_METHOD              category
               40  LOAD_FAST                'char'
               42  CALL_METHOD_1         1  ''
               44  STORE_FAST               'cat'

 L. 264        46  LOAD_FAST                'cat'
               48  LOAD_STR                 'Zs'
               50  COMPARE_OP               ==
               52  POP_JUMP_IF_FALSE    58  'to 58'

 L. 265        54  LOAD_CONST               True
               56  RETURN_VALUE     
             58_0  COME_FROM            52  '52'

 L. 266        58  LOAD_CONST               False
               60  RETURN_VALUE     
               -1  RETURN_LAST      
...
def _is_punctuation--- This code section failed: ---
 ...
 L. 283         0  LOAD_GLOBAL              ord
                2  LOAD_FAST                'char'
                4  CALL_FUNCTION_1       1  ''
                6  STORE_FAST               'cp'

 L. 288         8  LOAD_FAST                'cp'
               10  LOAD_CONST               33
               12  COMPARE_OP               >=
