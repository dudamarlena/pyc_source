...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/virtback.py
# Compiled at: 2013-06-27 18:54:29
...
class Domfetcher(object):
    ...
    def get_running_doms(self):
        ...
        doms = []
        for id in self.c.listDomainsID():
            dom = self.c.lookupByID(id)
            if 'Domain-' not in dom.name():
                doms.append(dom)
...
def invoke(doms, method):
    ...
    for dom in doms:
        try:
            ...
            retcode = f(dom)
            if retcode:
                logit(method, ('{0} returned {1} on {2}').format(method, retcode, dom.name()))
        except libvirt.libvirtError:
            pass
...
def backup(doms):
    ...
    for dom in doms:
        ...
        if dom.isActive():
            shutdown([dom])
        if dom.isActive():
            logit('error', 'unable to shutdown or destroy %s and BACKUP FAILED!' % dom.name())
            continue
        xml = dom.XMLDesc(0)
        ...
        for disk_source in disklist:
            ...
            logit('backup', 'copying %s to %s for %s' % (disk_source, disk_dest, dom.name()))
...
def shutdown(doms, wait=180):
    ...
    for i in range(0, wait + 1):
        if check_all_shutoff(doms):
            ...
            logit('shutdown', 'waited ' + str(i * secs) + ' seconds for ' + (', ').join(dom.name() for dom in get_all_running(doms)) + ' to shut off')
        if i == wait:
...
def rotate(target, retention=3):
    ...
    for i in range(retention - 2, 0, -1):
...
def getoptions():
    ...
    p.add_option('-q', '--quiet', dest='quiet', action='store_true', default=False, help='prevent output to stdout')
    p.add_option('-d', '--date', dest='tardate', action='store_true', default=False, help='append date to tar filename [default: no date]')
    p.add_option('-g', '--no-gzip', dest='nogzip', action='store_true', default=False, help='do not gzip the resulting tar file')
    p.add_option('-a', '--retention', dest='retention', metavar='amount', default=3, type='int', help='backups to retain [default: 3]')
    p.add_option('-p', '--path', dest='backpath', metavar="'PATH'", default='/KVMBACK', help="backup path [default: '/KVMBACK']")
    p.add_option('-u', '--uri', dest='uri', metavar="'URI'", help='optional hypervisor uri')
    g0 = OptionGroup(p, 'Actions for info testing', 'These options display info or test a list of guests.')
    g0.add_option('-i', '--info', dest='info', action='store_true', default=False, help='info/test a list of guests (space delimited dom names)')
    g0.add_option('--info-all', dest='infoall', action='store_true', default=False, help='attempt to show info on ALL guests')
    g1 = OptionGroup(p, 'Actions for a list of dom names', 'WARNING:  These options WILL bring down guests!')
    g1.add_option('-b', '--backup', dest='backup', action='store_true', default=False, help='backup a list of guests (space delimited dom names)')
    g1.add_option('-r', '--reboot', dest='reboot', action='store_true', default=False, help='reboot a list of guests (space delimited dom names)')
    g1.add_option('-s', '--shutdown', dest='shutdown', action='store_true', default=False, help='shutdown a list of guests (space delimited dom names)')
    g1.add_option('-c', '--create', dest='create', action='store_true', default=False, help='start a list of guests (space delimited dom names)')
    g2 = OptionGroup(p, 'Actions for all doms', 'WARNING:  These options WILL bring down ALL guests!')
    g2.add_option('--backup-all', dest='backupall', action='store_true', default=False, help='attempt to shutdown, backup, and start ALL guests')
    g2.add_option('--reboot-all', dest='rebootall', action='store_true', default=False, help='attempt to shutdown and then start ALL guests')
    g2.add_option('--shutdown-all', dest='shutdownall', action='store_true', default=False, help='attempt to shutdown ALL guests')
    g2.add_option('--create-all', dest='createall', action='store_true', default=False, help='attempt to start ALL guests')
    p.add_option_group(g0)
    p.add_option_group(g1)
    p.add_option_group(g2)
    options, guest_names = p.parse_args()
    actions = [
     options.backup, options.reboot, options.shutdown,
...
if __name__ == '__main__':
    ...
    if options.backup or options.reboot or options.shutdown or options.create or options.info:
