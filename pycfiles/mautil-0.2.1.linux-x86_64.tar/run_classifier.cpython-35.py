...
# [GCC 8.4.0]
# Embedded file name: /home/tom/pyenv/py3/lib/python3.5/site-packages/mautil/tf_net/bert/run_classifier.py
...
import collections, csv, os, modeling, optimization, tokenization, tensorflow as tf
flags = tf.flags
FLAGS = flags.FLAGS
...
class PaddingInputExample(object):
    __doc__ = "Fake example so the num input examples is a multiple of the batch size.\n\n  When running eval/predict on the TPU, we need to pad the number of examples\n  to be a multiple of the batch size, because the TPU requires a fixed batch\n  size. The alternative is to drop the last batch, which is bad because it means\n  the entire output data won't be generated.\n\n  We use this class instead of `None` because treating `None` as padding\n  battches could cause silent errors.\n  "
...
class DataProcessor(object):
    __doc__ = 'Base class for data converters for sequence classification data sets.'
...
class XnliProcessor(DataProcessor):
    __doc__ = 'Processor for the XNLI data set.'
...
class MnliProcessor(DataProcessor):
    __doc__ = 'Processor for the MultiNLI data set (GLUE version).'
...
class MrpcProcessor(DataProcessor):
    __doc__ = 'Processor for the MRPC data set (GLUE version).'
...
class ColaProcessor(DataProcessor):
    __doc__ = 'Processor for the CoLA data set (GLUE version).'
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer):
    ...
    if isinstance(example, PaddingInputExample):
        return InputFeatures(input_ids=[
         0] * max_seq_length, input_mask=[
         0] * max_seq_length, segment_ids=[
         0] * max_seq_length, label_id=0, is_real_example=False)
    label_map = {}
...
def input_fn_builder(features, seq_length, is_training, drop_remainder):
    ...
    def input_fn(params):
        ...
        d = tf.data.Dataset.from_tensor_slices({'input_ids': tf.constant(all_input_ids, shape=[num_examples, seq_length], dtype=tf.int32), 
         ...
         'input_mask': tf.constant(all_input_mask, shape=[
                        num_examples, seq_length], dtype=tf.int32), 
         
         'segment_ids': tf.constant(all_segment_ids, shape=[
                         num_examples, seq_length], dtype=tf.int32), 
