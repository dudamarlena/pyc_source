...
# [GCC 8.4.0]
# Embedded file name: /home/tom/pyenv/py3/lib/python3.5/site-packages/mautil/tf_models.py
# Compiled at: 2019-06-28 22:27:22
# Size of source mod 2**32: 27506 bytes
...
class TF(BasicModel, tf.train.SessionRunHook):
    ...
    def before_run(self, run_context):
        ...
        if self._run_phase == 'train':
            ...
            if self._run_phase == 'validate':
                fetches = self.validate_nodes
            else:
                if self._run_phase == 'predict':
    ...
    def _add_opt(self, loss, var_list, name='train_op', clip=None, lr_node=None, global_step=None, opt=None, opt_name='adam'):
        if global_step is None:
            ...
            if self.cfg.warmup_steps > 0:
                warmup_lr = tf.to_float(global_step) / tf.to_float(self.cfg.warmup_steps) * self.cfg.lr
            else:
                warmup_lr = 0.0
            if lr_node is None:
    ...
    def _add_train_op(self, loss, var_list, name='train_op', clip=None, lr_node=None, global_step=None, opt=None, opt_name='adam'):
        ...
        return (
         train_op, grads, global_step, lr_node, opt, gnorm)
    ...
    def pre_run_batch(self, batch, epoch=0, itr=0, global_step=0, is_training=True):
        ...
        elif self.cfg.dropout is not None:
            batch['dropout'] = 1.0
        return batch
    ...
    def _get_input_fn(self, x, y, shuffle, data_type):
        ...
        if self.cfg.use_tpu:
            ...
            if not os.path.exists(fpath) or self.cfg.regen_tfrecord:
                ds = self._create_dataset(x, y, self.cfg.batch_size, shuffle=shuffle, data_type=data_type)
    ...
    def _create_dataset(self, x, y, batch_size, data_type, shuffle=False):
        dtypes = {}
        shapes = {}
        record_spec = self._get_tfrecord_spec(self._input_features)
        for fea in self._input_features:
    ...
    def _get_estimator_spec(self, mode, batch_size, hooks=None):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            output_spec = tf.contrib.tpu.TPUEstimatorSpec(mode=mode, loss=self._loss, train_op=self._train_op, training_hooks=hooks)
        elif mode == tf.estimator.ModeKeys.EVAL:
            loss = self._loss
            if self.cfg.use_tpu:
                with tf.colocate_with(loss):
                    loss = tf.contrib.tpu.cross_replica_sum(loss) / self.cfg.num_hosts / self.cfg.num_core_per_host
            metric_loss = tf.tile(tf.reshape(loss, [1, 1]), [batch_size, 1])
            output_spec = tf.contrib.tpu.TPUEstimatorSpec(mode=mode, loss=loss, evaluation_hooks=hooks, eval_metrics=(
             self._metric_fn, [metric_loss]))
        return output_spec
    ...
    def _get_estimator(self, model_fn, run_config, warm_start_from):
        estimator = tf.contrib.tpu.TPUEstimator(use_tpu=self.cfg.use_tpu, model_fn=model_fn, config=run_config, train_batch_size=self.cfg.batch_size, eval_batch_size=self.cfg.val_batch_size, warm_start_from=warm_start_from)
        return estimator
    ...
    def fit_estimator(self, x, y, xV=None, yV=None, save=False, restore=False, xTest=None):
        ...
        if self.cfg.use_tpu:
            tpu_cluster_resolver = tf.contrib.cluster_resolver.TPUClusterResolver(self.cfg.tpu_name, zone=self.cfg.tpu_zone, project=self.cfg.gcp_project)
            model_dir = os.path.join(self.cfg.gsurl, os.path.basename(self.gen_fname('')))
        else:
            model_dir = self.gen_fname('')
        is_per_host = tf.contrib.tpu.InputPipelineConfig.PER_HOST_V2
        run_config = tf.contrib.tpu.RunConfig(cluster=tpu_cluster_resolver, model_dir=model_dir, session_config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True), save_checkpoints_steps=self.cfg.save_step, save_checkpoints_secs=None, keep_checkpoint_max=self.cfg.save_keep, tpu_config=tf.contrib.tpu.TPUConfig(num_shards=self.cfg.num_core_per_host * self.cfg.num_hosts, iterations_per_loop=self.cfg.tpu_loop_iterations, per_host_input_for_training=is_per_host))
