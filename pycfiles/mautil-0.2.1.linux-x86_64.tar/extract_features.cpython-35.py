...
# [GCC 8.4.0]
# Embedded file name: /home/tom/pyenv/py3/lib/python3.5/site-packages/mautil/tf_net/bert/extract_features.py
...
import codecs, collections, json, re, modeling, tokenization, tensorflow as tf
flags = tf.flags
FLAGS = flags.FLAGS
...
flags.DEFINE_bool('use_one_hot_embeddings', False, 'If True, tf.one_hot will be used for embedding lookups, otherwise tf.nn.embedding_lookup will be used. On TPUs, this should be True since it is much faster.')
...
def input_fn_builder(features, seq_length):
    ...
    def input_fn(params):
        ...
        d = tf.data.Dataset.from_tensor_slices({'unique_ids': tf.constant(all_unique_ids, shape=[num_examples], dtype=tf.int32), 
         ...
         'input_type_ids': tf.constant(all_input_type_ids, shape=[
                            num_examples, seq_length], dtype=tf.int32)})
...
def main(_):
    ...
    estimator = tf.contrib.tpu.TPUEstimator(use_tpu=FLAGS.use_tpu, model_fn=model_fn, config=run_config, predict_batch_size=FLAGS.batch_size)
