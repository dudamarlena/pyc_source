...
# [GCC 8.4.0]
# Embedded file name: /home/tom/pyenv/py3/lib/python3.5/site-packages/mautil/tf_layers.py
# Compiled at: 2019-04-15 09:18:19
# Size of source mod 2**32: 13969 bytes
"""
    ...
    def _create_graph(self, inputs, inputs_lens=None, initial_state=None, rnn_func=None):
        if rnn_func is None:
            if self.bidirection:
                ...
                outs, stats = tf.nn.bidirectional_dynamic_rnn(self.cell, cell_bw, inputs, sequence_length=inputs_lens, swap_memory=self.swap_memory, dtype=self.dtype, initial_state_fw=initial_state_fw, initial_state_bw=initial_state_bw, scope=self.scope)
            else:
    ...
    def next_inputs(self, time, outputs, state, sample_outputs, name=None, **unused_kwargs):
        ...
        with ops.name_scope(name, 'TrainingHelperNextInputs', [
         time, outputs, state]):
            ...
            next_inputs = control_flow_ops.cond(all_finished, lambda : self._zero_inputs, lambda : next_inputs)
        return (
         finished, next_inputs, state)
...
class BasicDecoder(tf.contrib.seq2seq.BasicDecoder):
    ...
    def output_size(self):
        ...
        return self.BasicDecoderOutput(rnn_output=self._rnn_output_size(), sample_output=sample_output)
    ...
    def output_dtype(self):
        ...
        return self.BasicDecoderOutput(nest.map_structure(lambda _: dtype, self._rnn_output_size()), sample_output)
    ...
    def step(self, time, inputs, state, name=None):
        ...
        with ops.name_scope(name, 'BasicDecoderStep', (time, inputs, state)):
            ...
            sample_outputs = self._helper.sample(time=time, outputs=cell_outputs, state=cell_state)
            if 'sample_outputs' in inspect.signature(self._helper.next_inputs).parameters:
                finished, next_inputs, next_state = self._helper.next_inputs(time=time, outputs=cell_outputs, state=cell_state, sample_outputs=sample_outputs)
            else:
                finished, next_inputs, next_state = self._helper.next_inputs(time=time, outputs=cell_outputs, state=cell_state, sample_ids=sample_outputs)
