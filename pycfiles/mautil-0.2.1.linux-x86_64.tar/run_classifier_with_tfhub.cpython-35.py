...
# [GCC 8.4.0]
# Embedded file name: /home/tom/pyenv/py3/lib/python3.5/site-packages/mautil/tf_net/bert/run_classifier_with_tfhub.py
...
import os, optimization, run_classifier, tokenization, tensorflow as tf, tensorflow_hub as hub
flags = tf.flags
FLAGS = flags.FLAGS
flags.DEFINE_string('bert_hub_module_handle', None, 'Handle for the BERT TF-Hub module.')

def create_model(is_training, input_ids, input_mask, segment_ids, labels, num_labels, bert_hub_module_handle):
...
def model_fn_builder(num_labels, learning_rate, num_train_steps, num_warmup_steps, use_tpu, bert_hub_module_handle):
    ...
    def model_fn(features, labels, mode, params):
        ...
        total_loss, per_example_loss, logits, probabilities = create_model(is_training, input_ids, input_mask, segment_ids, label_ids, num_labels, bert_hub_module_handle)
        output_spec = None
...
def main(_):
    ...
    processors = {'cola': run_classifier.ColaProcessor, 
    ...
    model_fn = model_fn_builder(num_labels=len(label_list), learning_rate=FLAGS.learning_rate, num_train_steps=num_train_steps, num_warmup_steps=num_warmup_steps, use_tpu=FLAGS.use_tpu, bert_hub_module_handle=FLAGS.bert_hub_module_handle)
    ...
    if FLAGS.do_train:
        train_features = run_classifier.convert_examples_to_features(train_examples, label_list, FLAGS.max_seq_length, tokenizer)
        tf.logging.info('***** Running training *****')
        tf.logging.info('  Num examples = %d', len(train_examples))
        tf.logging.info('  Batch size = %d', FLAGS.train_batch_size)
        tf.logging.info('  Num steps = %d', num_train_steps)
        train_input_fn = run_classifier.input_fn_builder(features=train_features, seq_length=FLAGS.max_seq_length, is_training=True, drop_remainder=True)
        estimator.train(input_fn=train_input_fn, max_steps=num_train_steps)
    if FLAGS.do_eval:
        eval_examples = processor.get_dev_examples(FLAGS.data_dir)
        eval_features = run_classifier.convert_examples_to_features(eval_examples, label_list, FLAGS.max_seq_length, tokenizer)
        ...
        eval_input_fn = run_classifier.input_fn_builder(features=eval_features, seq_length=FLAGS.max_seq_length, is_training=False, drop_remainder=eval_drop_remainder)
    ...
    if FLAGS.do_predict:
        ...
        run_classifier.file_based_convert_examples_to_features(predict_examples, label_list, FLAGS.max_seq_length, tokenizer, predict_file)
        tf.logging.info('***** Running prediction*****')
        tf.logging.info('  Num examples = %d', len(predict_examples))
        tf.logging.info('  Batch size = %d', FLAGS.predict_batch_size)
        predict_input_fn = run_classifier.file_based_input_fn_builder(input_file=predict_file, seq_length=FLAGS.max_seq_length, is_training=False, drop_remainder=FLAGS.use_tpu)
