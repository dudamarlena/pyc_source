...
class NamedParser(object):
    ...
    def _get_subparsers(self, dest):
        ...
        if not self._subparsers:
            ...
            raise KeyError('Subparser names mismatch. You can only create one subcommand.')
...
class Subcommand(object):
    ...
    def __enter__(self):
        ...
        Subcommand.CURRENT_PARSER = self._old_parser.get_subparser(self.subcmd, dest=self.dest)
        return self
...
def DEFINE_boolean(flag_name, default_value, docstring):
    ...
    get_context_parser().add_argument('--' + flag_name, nargs='?', const=True, help=docstring, default=default_value, type=str2bool)
    get_context_parser().add_argument('--no' + flag_name, action='store_false', dest=flag_name.replace('-', '_'))
