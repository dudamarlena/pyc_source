...
# Embedded file name: /home/olek/Wszystkie_aggrescamy_swiata/aggrescan3d/a3d_gui/create_config.py
# Compiled at: 2018-12-17 08:36:03
import os, json, re, sqlite3, numpy as np
...
def generate_config(jid):
    ...
    try:
        project_settings = query_db('SELECT dynamic, mutate, mutation, chains,                    foldx, distance, auto_mutation FROM project_details WHERE jid=?', [jid], one=True)
    ...
    with open(join(app.config['USERJOB_DIRECTORY'], jid, 'config.ini'), 'w') as (f):
        ...
        if project_settings['chains']:
            if len(project_settings['chains']) == 1:
...
def reverse_config(filepath):
    ...
    final_options = {'dynamic': options.dynamic, 
...
def prepare_data(jid):
    ...
    """
    system_info = query_db('SELECT chain_sequence, distance, mutt_energy_diff, dynamic,             mutation, mutate, chains, foldx, auto_mutation FROM  project_details WHERE jid=?', [jid], one=True)
    basic_info = query_db('SELECT started, project_name, status , working_dir FROM  user_queue WHERE jid=?', [
     jid], one=True)
    ...
    if system_info['mutate'] == 1:
        for mutation in system_info['mutation'].strip().replace(' ', '').split(','):
            k = mutation[(-1)] + mutation[2:-1]
            mut[k] = mutation

    if system_info['foldx']:
    ...
    project_info['error'] = ''
    project_info['table'] = ''
    project_info['avg_scores'] = {'dummy': 'dummy'}
    ...
    if project_info['status'] == 'done':
        if system_info['auto_mutation']:
            project_info['autom_data'] = _parse_auto_mut_info(join(project_info['working_dir'], 'Mutations_summary.csv'))
        ...
        with open(os.path.join(project_info['working_dir'], 'A3D.csv')) as (fw):
            rec = re.compile('^(.*),(.*),(.*),(.*),(.*)$', re.M)
            ...
            sum3d = np.sum(dat)
            avg3d = sum3d / len(dat)
            a3dtable = {'min': min3d, 'avg': avg3d, 'max': max3d, 'sum': sum3d, 
               'tab': a3d_table}
