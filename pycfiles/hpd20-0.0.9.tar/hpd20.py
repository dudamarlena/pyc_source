...
# [GCC 8.4.0]
# Embedded file name: /Users/scjurgen/Projects/hpd-20/hpd20/hpd20.py
# Compiled at: 2017-01-13 06:59:11
...
class hpd:
    ...
    def __init__(self, file_name):
        ...
        self.memoryBlock = memory_block[0:-16]
    ...
    def digest_single_kit(self, kit):
        ...
        for i in range(0, hpd.PADS_PER_KIT):
            ...
            result += ('{0}: {1} {2} {3} {4}\n').format(self.pads.get_pad_name(i), real_note, self.pads.get_pad(kit * hpd.PADS_PER_KIT + i).get_volume(0), get_instrument_name(self.pads.get_pad(kit * hpd.PADS_PER_KIT + i).get_patch(0)), self.pads.get_pad(kit * hpd.PADS_PER_KIT + i).get_pitch(0))
    ...
    def create_kit_filename(self, kit_index):
        ...
        kit_as_file_name = 'kits/' + ('').join([ x if x.isalnum() or x in ('!', '&',
                                                                           '(', ')',
                                                                           '_', '.',
                                                                           '{', '}',
                                                                           ' ') else '_' for x in kit_name ])
    ...
    def apply_pad(self, pad_index):
        ...
        self.memoryBlock[(hpd.PAD_MEMINDEX + hpd.PAD_MEMSIZE * pad_index):(hpd.PAD_MEMINDEX + hpd.PAD_MEMSIZE * (pad_index + 1))] = pad.memory_block
