...
# [GCC 8.4.0]
# Embedded file name: hpd20/hpd20wx.py
# Compiled at: 2017-01-05 08:24:10
import wx, wx.grid as gridlib, hpd20
...
class MyForm(wx.Frame):
    ...
    def layout_kit_grid(self):
        self.columns = [
         'Layer', 'fade', 'trigger', 'velo', 'mutgrp', 'mn/ply', 'Instrument', 'nr', 'vol', 'cents', 'note', 'muff', 'color', 'swp', 'ambsend']
    ...
    def load_kit(self, event):
        openFileDialog = wx.FileDialog(self, 'Load Kit to current set', '', '', 'hpd-20 Kit (*.kit)|*.kit', wx.FD_OPEN | wx.FD_FILE_MUST_EXIST)
    ...
    def save_kit(self, event):
        openFileDialog = wx.FileDialog(self, 'Save Kit', '', '', 'hpd-20 Kit (*.kit)|*.kit', wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT)
        openFileDialog.ShowModal()
        openFileDialog.GetPath()
        openFileDialog.Destroy()
        self.fill_kit(3)
    ...
    def load_memory_backup(self, event):
        openFileDialog = wx.FileDialog(self, 'Open a memory dump', '', '', 'hpd-20 Dump (*.HS0)|*.HS0', wx.FD_OPEN | wx.FD_FILE_MUST_EXIST)
        openFileDialog.ShowModal()
        openFileDialog.GetPath()
        openFileDialog.Destroy()

    def save_memory_backup(self, event):
        saveFileDialog = wx.FileDialog(self, 'Save memory dump as', '', '', 'hpd-20 Dump (*.HS0)|*.HS0', wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT)
