...
# [GCC 8.4.0]
# Embedded file name: /Users/sravan953/Documents/CU/Projects/virtual-scanner/virtualscanner/coms/coms_ui/coms_server_flask.py
# Compiled at: 2019-10-16 15:15:18
# Size of source mod 2**32: 18861 bytes
import os
from pathlib import Path
from flask import Flask, render_template, request, redirect, session
from werkzeug.utils import secure_filename
from virtualscanner.server.ana import T2_mapping, T1_mapping, ROI_analysis
...
from virtualscanner.server.rf.tx.SAR_calc import SAR_calc_main
...
def on_acq():
    ...
    """
    if 'acq' in session:
        return render_template('acquire.html', success=(session['acq']), axial=(session['acq_out_axial']), sagittal=(session['acq_out_sagittal']),
          coronal=(session['acq_out_coronal']),
    ...
    if 'ana_load' in session:
        if 'ana_map' in session:
            if 'ana_roi' in session:
                return render_template('analyze.html', roi_success=(session['ana_roi']), payload3=(session['ana_payload3']), map_success=(session['ana_map']),
                  load_success=(session['ana_load']),
                  payload1=(session['ana_payload1']),
                  payload2=(session['ana_payload2']))
            else:
                return render_template('analyze.html', map_success=(session['ana_map']), load_success=(session['ana_load']), payload1=(session['ana_payload1']),
                  payload2=(session['ana_payload2']))
        else:
    ...
    if request.method == 'POST':
        ...
        if request.form['formName'] == 'reg':
            if payload['subjecttype'] == 'Subject':
                ...
                return redirect('register_success')
        if request.form['formName'] == 'new-reg':
            session.pop('reg_success')
            return redirect('register')
        if request.form['formName'] == 'acq':
            session['acq'] = 0
            if 'patid' not in session:
                return redirect('register')
            ...
            print(payload)
            progress = bsim.run_blochsim(seqinfo=payload, phtinfo=(rows[0][0]), pat_id=pat_id)
            ...
            if request.form['formName'] == 'ana':
                if 'original-data-opt' in payload:
                    ...
                    original_data_path = [str(STATIC_ANALYZE_PATH_REL / 'inputs' / (payload['original-data-opt'] + '_original_data') / iname) for iname in filenames_in_path]
                    payload['data-path'] = original_data_path
                    session['ana_payload1'] = payload
                else:
                    if 'map-form' in payload:
                        STATIC_ANALYZE_PATH_REL = constants.COMS_UI_STATIC_ANALYZE_PATH.relative_to(CURRENT_PATH)
                        session['ana_map'] = 1
                        if payload['TI'] == '':
        ...
        if request.form['formName'] == 'tx':
            ...
            if request.form['formName'] == 'rx':
                ...
                return redirect('rx')
            if request.form['formName'] == 'recon':
                file = request.files['file']
                filename = secure_filename(file.filename)
