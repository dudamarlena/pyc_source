...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/sdag2/core.py
# Compiled at: 2016-08-02 07:46:59
"""
sdag2 - Python Simple Directed Acyclic Graph whith Cicle Detector and
...
class DAG:
    ...
    def add_edge(self, from_vertex, to_vertex):
        ...
        if cycle is not None:
            ...
            msg = "Edge between '%s' and '%s' introduces a cycle in the graph" % (
...
class order(decorator):
    ...
    def __init__(self, before=None, after=None, target=None, called=None):
        decorator.__init__(self, target, called)
        self.before = before or ()
        self.after = after or ()


def sort_items(clslist, gemkey=None, orderfrom=None):
    if not gemkey:
        gemkey = lambda item: '%s.%s' % (item.__module__, item.__name__)
    if not orderfrom:
        orderfrom = lambda item: Decs.get_dec(item, order) or order()
    dag = DAG()
    imap = dict()
    vmap = dict()
    items = set(clslist)
    ...
    for k in imap:
        _ = imap[k]
        v = vmap[k]
        orderdec = orderfrom(_)
        if not orderdec:
            orderdec = order()
        for b in orderdec.before:
            try:
                vb = vmap[gemkey(b)]
                dag.add_edge(v, vb)
            except KeyError:
