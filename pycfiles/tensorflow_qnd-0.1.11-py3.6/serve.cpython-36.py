...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/qnd/serve.py
# Compiled at: 2017-06-30 10:37:41
# Size of source mod 2**32: 2875 bytes
import http.server, json, logging, queue, threading, numpy as np, tensorflow as tf
...
def def_serve():
    ...
    def serve(model_fn, preprocess_fn=None, postprocess_fn=None):
        ...
        """
        server = EstimatorServer(estimator(model_fn, FLAGS.output_dir), preprocess_fn, postprocess_fn)
        ...
        class Handler(http.server.BaseHTTPRequestHandler):
            ...
            def do_POST(self):
                ...
                self.end_headers()
                inputs = json.loads(self.rfile.read(int(self.headers['Content-Length'])))
                outputs = server.predict(inputs)
                logging.info('Prediction results: {}'.format(outputs))
                self.wfile.write(json.dumps(outputs).encode())

        http.server.HTTPServer((FLAGS.ip_address, FLAGS.port), Handler).serve_forever()
...
class EstimatorServer:
    ...
    def __init__(self, estimator, preprocess_fn=None, postprocess_fn=None):
        ...
        def input_fn():
            return (
             tf.train.batch((preprocess_fn(self._input_queue.get)), 1,
               dynamic_pad=True),
             None)
