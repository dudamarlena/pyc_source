...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/qnd/train_and_evaluate.py
...
def def_train_and_evaluate(batch_inputs=True, prepare_filename_queues=True, distributed=False):
    ...
    """
    add_output_dir_flag()
    def_experiment_fn = def_def_experiment_fn(batch_inputs, prepare_filename_queues, distributed)

    def train_and_evaluate(model_fn, train_input_fn, eval_input_fn=None, serving_input_fn=None):
        ...
        """
        return run(def_experiment_fn(model_fn, train_input_fn, eval_input_fn, serving_input_fn), FLAGS.output_dir)
