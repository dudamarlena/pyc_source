...
# Embedded file name: build/bdist.linux-x86_64/egg/pymodelfit/builtins.py
# Compiled at: 2011-05-10 05:46:57
__doc__ = '\nThis module contains builtin function models that use the framework of the\n:mod:`core` module.\n'
from __future__ import division, with_statement
import numpy as np
from core import ParametricModel, FunctionModel, FunctionModel1DAuto, DatacentricModel1DAuto, FunctionModel2DScalarAuto, register_model
    ...
    * 'fiterrxy': 
    ...
    def _linearFit(self, x, y, fixedpars=(), weights=None, **kwargs):
        ...
        if self.fittype == 'basic':
            if weights is None:
                ((m, b), merr, berr, dy) = self.fitBasic(x, y, self.m if 'm' in fixedpars else False, self.b if 'b' in fixedpars else False)
            else:
            ...
            if weights is None:
                ...
                if len(weights.shape) == 1:
                    xerr = yerr = 1 / (weights * 0.7071067811865476)
                else:
                    (xerr, yerr) = 1 / weights
    ...
    def fitBasic(x, y, fixslope=False, fixint=False):
        ...
        """
        N = len(x)
        if (fixint is False or fixint is None) and (fixslope is False or fixslope is None):
            if len(y) != N:
    ...
    def fitWeighted(x, y, sigmay=None, doplot=False, fixslope=None, fixint=None):
        ...
        x, y = array(x), array(y)
        w = 1.0 / sigmay / sigmay
        if (fixint is None or fixint is False) and (fixslope is None or fixslope is False):
            delta = sum(w) * sum(w * x * x) - sum(w * x) ** 2
            A = (sum(w * x * x) * sum(w * y) - sum(w * x) * sum(w * x * y)) / delta
            B = (sum(w) * sum(w * x * y) - sum(w * x) * sum(w * y)) / delta
            diff = y - A - B * x
            sigmaysq = sum(w * diff * diff) / (sum(w) * (N - 2) / N)
...
class GaussianModel(FunctionModel1DAuto):
    ...
    def f(self, x, A=1, sig=1, mu=0):
        tsq = (x - mu) * 0.7071067811865476 / sig
    ...
    def integrate(self, lower, upper, method=None, **kwargs):
        if method is not None:
            ...
            if len(kwargs) > 0:
                ...
                uval = erf(u * 0.7071067811865476)
                lval = erf(-l * 0.7071067811865476)
...
class DoubleGaussianModel(FunctionModel1DAuto):
    ...
    def f(self, x, A=1, B=1, sig1=1, sig2=1, mu1=-0.5, mu2=0.5):
        tsq1 = (x - mu1) * 0.7071067811865476 / abs(sig1)
        tsq2 = (x - mu2) * 0.7071067811865476 / abs(sig2)
...
class LognormalModel(FunctionModel1DAuto):
    ...
    def rangehint(self):
        return (
         np.exp(self.mulog - self.siglog * 4), np.exp(self.mulog + self.siglog * 4))
...
class LorentzianModel(FunctionModel1DAuto):
    ...
    def rangehint(self):
        return (
         self.mu - self.gamma * 6, self.mu + self.gamma * 6)
...
class VoigtModel(GaussianModel, LorentzianModel):
    ...
    def f(self, x, A=1, sig=0.5, gamma=0.5, mu=0):
        ...
        if sig == 0:
            ...
            w = wofz((x - mu + complex(0.0, 1.0) * gamma) * 0.7071067811865476 / sig)
    ...
    def rangehint(self):
        ...
        return (
         self.mu - halfwidth, self.mu + halfwidth)
...
class TwoSlopeATanModel(FunctionModel1DAuto):
    ...
    def rangehint(self):
        return (
         self.x0 - 3 * self.w, self.x0 + 3 * self.w)
...
class _KnotSplineModel(DatacentricModel1DAuto):
    ...
    * 'even'
    ...
    def __init__(self):
        ...
        self._ws = None
        self.data = (
         np.arange(self.degree + self.nknots + 1), np.arange(self.degree + self.nknots + 1), self._ws)
        return
...
class MaxwellBoltzmannModel(FunctionModel1DAuto):
    ...
    def rangehint(self):
        ...
        return (
         0, min(3 * (2 * kb * self.T / self.m) ** 0.5, c))
...
class Gaussian2DModel(FunctionModel2DScalarAuto):
    ...
    def rangehint(self):
        ...
        return (
         mux - 4 * sigx, mux + 4 * sigx, muy - 4 * sigy, muy + 4 * sigy)
...
class Plane(FunctionModel):
    ...
    def plot3d(self, data=np.array([(-1, 1), (-1, 1), (-1, 1)]), n=10, showdata=False, clf=True, **kwargs):
        """
