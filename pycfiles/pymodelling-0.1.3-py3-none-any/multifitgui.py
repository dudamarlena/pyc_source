...
# Embedded file name: build/bdist.linux-x86_64/egg/pymodelfit/multifitgui.py
# Compiled at: 2011-04-26 06:15:06
__doc__ = '\nThis module contains the internals for the FitGui gui.\n'
from __future__ import division, with_statement
import numpy as np
from enthought.traits.api import HasTraits, Bool, Button, Float, Int, Color, Instance, Tuple, Array, List, Dict, Str, Property, on_trait_change, cached_property
from enthought.traits.ui.api import View, VGroup, HGroup, Item, TupleEditor, ListEditor
...
class MultiFitGui(HasTraits):
    ...
    traits_view = View(VGroup(Item('fgs', editor=ListEditor(use_notebook=True, page_name='.plotname'), style='custom', show_label=False), Item('show3d', show_label=False)), resizable=True, height=900, buttons=['OK', 'Cancel'], title='Multiple Model Data Fitters')
    plot3d_view = View(VGroup(Item('scene3d', editor=SceneEditor(scene_class=MayaviScene), show_label=False, resizable=True), Item('plot3daxes', editor=TupleEditor(cols=3, labels=['x', 'y', 'z']), label='Axes'), HGroup(Item('do3dscale', label='Scale by weight?'), Item('scalefactor3d', label='Point scale'), Item('nmodel3d', label='Nmodel')), HGroup(Item('usecolor3d', label='Use color?'), Item('color3d', label='Relation Color', enabled_when='usecolor3d')), Item('replot3d', show_label=False), springy=True), resizable=True, height=800, width=800, title='Multiple Model3D Plot')
    ...
    def _axisnames_changed(self):
        ...
        self.plot3daxes = (
         self.axisnames[0], self.axisnames[1], self.axisnames[2] if len(self.axisnames) > 2 else self.axisnames[1])
        ...
        if self.doplot3d:
            ...
            try:
                ...
                try:
                    ...
                    if self.usecolor3d:
                        c = (
                         self.color3d[0] / 255, self.color3d[1] / 255, self.color3d[2] / 255)
