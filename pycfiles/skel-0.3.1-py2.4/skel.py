...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/skel.py
# Compiled at: 2007-01-24 21:42:40
...
class Builder(object):
    ...
    __module__ = __name__
    default_regex = '\\+(?P<key>\\w+)\\+'

    def __init__(self, data, ignore=None, filters=None, overwrite=False, regex=None):
    ...
    def find_filter(self, to_path):
        ...
        for filter in self.filters:
            if filter.applies(to_path):
                return (
                 filter, filter.new_path(to_path))
        else:
            return (
             None, to_path)
...
def command():
    ...
    parser.add_option('-i', '--ignore', dest='ignore', default='', help='comma separated list of patterns to ignore')
    parser.add_option('-o', '--overwrite', dest='overwrite', action='store_true', default='PROMPT', help='overwrite existing files')
    parser.add_option('-k', '--keep', dest='overwrite', action='store_false', default='PROMPT', help='keep existing files')
    parser.add_option('-d', '--data', dest='data', help='vars for replacement like foo=bar,spam=eggs')
    parser.add_option('-f', '--file', dest='file', help='file containing vars whitespace separated like foo=bar spam=eggs')
    parser.add_option('-p', '--prompt', dest='prompt', action='store_true', help='prompt for all variables')
    parser.add_option('-r', '--regex', dest='regex', default=Builder.default_regex, help='regex for identifying variables in paths (must have named group "key")')
    ...
    try:
        if opts.file:
            data.update((d.split('=') for d in open(opts.file, 'rb').read().split()))
    except:
    ...
    builder = Builder(data, ignore, [StringTemplateFilter()], opts.overwrite, opts.regex)
...
def test():
    ...
    builder = Builder(dct, ignore=['dontcopy'], filters=[StringTemplateFilter()])
