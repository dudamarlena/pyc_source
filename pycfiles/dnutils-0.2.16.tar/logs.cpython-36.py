...
# [GCC 8.4.0]
# Embedded file name: /data/code/dnutils/python3.5/dnutils/logs.py
# Compiled at: 2019-05-09 07:17:25
# Size of source mod 2**32: 17661 bytes
import json, logging, os, re, tempfile, atexit, warnings, colored, datetime
...
class FileHandler(logging.FileHandler):
    ...
    def format(self, record):
        return '{} - {} - {}'.format(datetime.datetime.fromtimestamp(record.created).strftime(self.timeformatstr), record.levelname, ' '.join(' '.join(map(str, record.msg)).split('\n')))
...
class ExposureEmptyError(Exception):
    pass


class ExposureLockedError(Exception):
    pass
            ...
            if re.match('\\.\\w+\\.lock', f):
                ...
                try:
                    ...
                    for idx, pat in enumerate(patterns):
                        try:
                            repattern = '^%s$' % re.escape(pat).replace('\\*', '.*?')
...
class ColoredFormatter(logging.Formatter):
    fmap = {logging.DEBUG: colored.fg('cyan') + colored.attr('bold'), 
     ...
     logging.CRITICAL: colored.bg('dark_red_2') + colored.fg('white') + colored.attr('underlined') + colored.attr('bold')}
    msgmap = {logging.DEBUG: colored.fg('cyan'), 
    ...
    class MongoFormatter(logging.Formatter):
        ...
        def format(self, record):
            return {'message':record.msg, 
             'timestamp':datetime.datetime.utcfromtimestamp(record.created),  'module':record.module, 
             'lineno':record.lineno,  'name':record.name,  'level':record.levelname}
