...
# [GCC 8.4.0]
# Embedded file name: /data/code/dnutils/python3.5/dnutils/debug.py
...
def out(*args, file=sys.stdout, sep=' ', end='\n', flush=False, tb=1):
    ...
    rv = _caller(tb)
    print('{}: l.{}: {}'.format(os.path.basename(rv[0]), rv[1], args[0] if args else ''), *(args[1:] if len(args) > 1 else []), **{'file':file,  'sep':sep,  'end':end,  'flush':flush})
