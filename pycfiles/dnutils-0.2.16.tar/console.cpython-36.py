...
# [GCC 8.4.0]
# Embedded file name: /data/code/dnutils/python3.5/dnutils/console.py
# Compiled at: 2019-05-09 07:17:25
# Size of source mod 2**32: 12146 bytes
import os, platform, shlex, struct, subprocess, colored, sys
...
def barstr(width, percent, color=None, inf=False):
    ...
    if inf:
        ...
        return '{0} {1: >7.3f} %'.format(bar, percent * 100.0)
...
class ProgressBar:
    ...
    def finish(self, erase=True, msg='', end='\n'):
        ...
        """
        if erase:
            sys.stdout.write('\r' + msg.ljust(self.lblwidth + self.barwidth, ' '))
    ...
    def __str__(self):
        return '\r' + barstr((self.barwidth), (self.value), color=(self.color), inf=(self.inf)) + ' ' + self._label[:self.lblwidth].ljust(self.lblwidth, ' ')
...
class StatusMsg(object):
    __doc__ = 'Print a Linux-style status message to the console.rst.'
    ...
    def __init__(self, message='', status=None, width='100%', stati=None):
        if stati is None:
            self.stati = {
             StatusMsg.ERROR, StatusMsg.OK, StatusMsg.WARNING, StatusMsg.FAILED, StatusMsg.PASSED}
        else:
            self.stati = stati
        self.widthstr = width
    ...
    def setwidth(self, width):
        ...
        if '%' in width:
            ...
            self.width = int(round(consolewidth * float(width.strip('%')) * 0.01))
        else:
