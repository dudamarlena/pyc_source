...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/pyUSID/io/hdf_utils/simple.py
# Compiled at: 2020-04-03 17:46:59
# Size of source mod 2**32: 61657 bytes
"""
...
import sys, h5py, numpy as np, dask.array as da
...
from ..reg_ref import write_region_references, simple_region_ref_copy, copy_reg_ref_reduced_dim, create_region_reference, copy_all_region_refs
from ..write_utils import clean_string_att, build_ind_val_matrices, get_aux_dset_slicing, INDICES_DTYPE, VALUES_DTYPE, Dimension, DimType
from .base import get_auxiliary_datasets, link_h5_obj_as_alias, get_attr, link_h5_objects_as_attrs, write_book_keeping_attrs, write_simple_attrs, is_editable_h5, validate_h5_objs_in_same_h5_file
...
def find_results_groups(h5_main, tool_name, h5_parent_group=None):
    ...
    if h5_parent_group is not None:
        if not isinstance(h5_parent_group, (h5py.File, h5py.Group)):
            raise TypeError("'h5_parent_group' should either be a h5py.File or h5py.Group object")
    else:
...
def check_and_link_ancillary(h5_dset, anc_names, h5_main=None, anc_refs=None):
    ...
    def __check_and_link_single(h5_obj_ref, target_ref_name):
        if isinstance(h5_obj_ref, h5py.Reference):
            ...
            if isinstance(h5_obj_ref, (h5py.Dataset, h5py.Group, h5py.File)):
    ...
    if bool(np.iterable(anc_refs) and not isinstance(anc_refs, h5py.Dataset)):
        ...
        if anc_refs is not None:
            ...
            if isinstance(anc_names, str) or isinstance(anc_names, unicode):
...
def copy_attributes(source, dest, skip_refs=True, verbose=False):
    ...
    try:
        ...
        if not skip_refs:
            warn('Dataset references will not be copied since {} and {} are in different files'.format(source, dest))
        skip_dset_refs = True
    ...
    for att_name in source.attrs.keys():
        ...
        if isinstance(att_val, h5py.Reference) and not isinstance(att_val, h5py.RegionReference):
            if not skip_dset_refs:
                ...
                if isinstance(att_val, h5py.RegionReference):
                    continue
                else:
                    if verbose:
...
def validate_main_dset(h5_main, must_be_h5):
    ...
    elif not isinstance(h5_main, (np.ndarray, da.core.Array)):
...
def validate_anc_h5_dsets(h5_inds, h5_vals, main_shape, is_spectroscopic=True):
    ...
    if isinstance(main_shape, (list, tuple)):
        if not contains_integers(main_shape, min_val=1) or len(main_shape) != 2:
            raise ValueError("'main_shape' must be a valid HDF5 dataset shape")
    else:
        raise TypeError('main_shape should be of the following types:h5py.Dataset, tuple, or list. {} provided'.format(type(main_shape)))
...
def validate_dims_against_main(main_shape, dims, is_spectroscopic=True):
    ...
    if not isinstance(main_shape, (list, tuple)):
        raise TypeError('main_shape should be a list or tuple. Provided object was of type: {}'.format(type(main_shape)))
    if len(main_shape) != 2:
    ...
    if isinstance(dims, Dimension):
        ...
        if not isinstance(dims, (list, tuple)):
            raise TypeError('"dims" must be a list or tuple of usid.Dimension objects. Provided object was of type: {}'.format(type(dims)))
    ...
    if lhs != rhs:
        raise ValueError(dim_category + ' dimensions in main data of size: {} do not match with product of values in provided Dimension objects: {}'.format(lhs, rhs))
...
def check_if_main(h5_main, verbose=False):
    ...
    * units
    ...
    if not np.all(anc_shape_match):
        if verbose:
            print('The shapes of the Position indices:{}, values:{} datasets did not match with that of the main dataset: {}'.format(h5_pos_inds.shape, h5_pos_vals.shape, h5_main.shape))
        return False
    anc_shape_match = list()
    ...
    if not np.all(anc_shape_match):
        if verbose:
            print('The shapes of the Spectroscopic indices:{}, values:{} datasets did not match with that of the main dataset: {}'.format(h5_spec_inds.shape, h5_spec_vals.shape, h5_main.shape))
        return False
    return success
...
def check_for_old(h5_base, tool_name, new_parms=None, target_dset=None, h5_parent_goup=None, verbose=False):
    """
    ...
    if new_parms is None:
        new_parms = dict()
    elif not isinstance(new_parms, dict):
    ...
    matching_groups = []
    groups = find_results_groups(h5_base, tool_name, h5_parent_group=h5_parent_goup)
    for group in groups:
        if verbose:
        ...
        if target_dset is not None:
            if target_dset in group.keys():
                ...
                if verbose:
                    print('{} did not contain the target dataset: {}'.format(group.name.split('/')[(-1)], target_dset))
                continue
...
def get_source_dataset(h5_group):
    ...
    if len(name_split) != 2:
        raise ValueError("The provided group's name could not be split by '-' as expected in SourceDataset-ProcessName_000")
...
def assign_group_index(h5_parent_group, base_name, verbose=False):
    ...
    if verbose:
        print('Looking for group names starting with {} in parent containing items: {}'.format(base_name, temp))
    previous_indices = []
...
def create_results_group(h5_main, tool_name, h5_parent_group=None):
    ...
    tool_name = validate_single_string_arg(tool_name, 'tool_name')
    if '-' in tool_name:
        warn('tool_name should not contain the "-" character. Reformatted name from:{} to {}'.format(tool_name, tool_name.replace('-', '_')))
...
def create_empty_dataset(source_dset, dtype, dset_name, h5_group=None, new_attrs=None, skip_refs=False):
    ...
    if h5_group is None:
        ...
        if source_dset.file != h5_group.file and not skip_refs:
            warn('H5 object references will not be copied over since {} is in a different HDF5 file as {}'.format(h5_group, source_dset))
            skip_refs = True
        dset_name = validate_single_string_arg(dset_name, 'dset_name')
        if '-' in dset_name:
            warn('dset_name should not contain the "-" character. Reformatted name from:{} to {}'.format(dset_name, dset_name.replace('-', '_')))
        ...
        if source_dset.file.driver == 'mpio' and kwargs.pop('compression', None) is not None:
            warn('This HDF5 file has been opened wth the "mpio" communicator. mpi4py does not allow creation of compressed datasets. Compression kwarg has been removed')
        if dset_name in h5_group.keys():
            if isinstance(h5_group[dset_name], h5py.Dataset):
                ...
                if any((source_dset.shape != h5_new_dset.shape, dtype != h5_new_dset.dtype)):
                    warn('Either the shape (existing: {} desired: {}) or dtype (existing: {} desired: {}) of the dataset did not match with expectations. Deleting and creating a new one.'.format(h5_new_dset.shape, source_dset.shape, h5_new_dset.dtype, dtype))
                    del h5_new_dset
...
def write_ind_val_dsets(h5_parent_group, dimensions, is_spectral=True, verbose=False, base_name=None, slow_to_fast=False):
    """
    ...
    if base_name is not None:
        ...
        for sub_name in ['Indices', 'Values']:
            if base_name + sub_name in h5_parent_group.keys():
                raise KeyError('Dataset: {} already exists in provided group: {}'.format(base_name + sub_name, h5_parent_group.name))
        ...
        if sing_mode == DimType.DEFAULT:
            ...
            indices, values = build_ind_val_matrices([dim.values for dim in dimensions], is_spectral=is_spectral)
            rev_func = np.flipud if is_spectral else np.fliplr
            dimensions = dimensions[::-1]
            indices = rev_func(indices)
            values = rev_func(values)
        else:
            if sing_mode == DimType.INCOMPLETE:
...
def copy_dataset(h5_orig_dset, h5_dest_grp, alias=None, verbose=False):
    ...
    if not isinstance(h5_dest_grp, (h5py.File, h5py.Group)):
        raise TypeError("'h5_dest_grp' should either be a h5py.File or h5py.Group object")
    ...
    if alias in h5_dest_grp.keys():
        if verbose:
            warn('{} already contains an object with the same name: {}'.format(h5_dest_grp, alias))
        h5_new_dset = h5_dest_grp[alias]
        if not isinstance(h5_new_dset, h5py.Dataset):
            raise TypeError('{} already contains an object: {} with the desired name which is not a dataset'.format(h5_dest_grp, h5_new_dset))
        da_source = lazy_load_array(h5_orig_dset)
        da_dest = lazy_load_array(h5_new_dset)
        if da_source.shape != da_dest.shape:
            raise ValueError('Existing dataset: {} has a different shape compared to the original dataset: {}'.format(h5_new_dset, h5_orig_dset))
        if not da.allclose(da_source, da_dest):
            raise ValueError('Existing dataset: {} has different contentscompared to the original dataset: {}'.format(h5_new_dset, h5_orig_dset))
    else:
        kwargs = {'shape': h5_orig_dset.shape, 
         'dtype': h5_orig_dset.dtype, 
         'compression': h5_orig_dset.compression, 
         'chunks': h5_orig_dset.chunks}
        if h5_orig_dset.file.driver == 'mpio' and kwargs.pop('compression', None) is not None:
            warn('This HDF5 file has been opened wth the "mpio" communicator. mpi4py does not allow creation of compressed datasets. Compression kwarg has been removed')
        if verbose:
            print('Creating new HDF5 dataset named: {} at: {} with kwargs: {}'.format(alias, h5_dest_grp, kwargs))
        h5_new_dset = h5_dest_grp.create_dataset(alias, **kwargs)
        if verbose:
            print('dask.array will copy data from source dataset to new dataset')
        da.to_hdf5(h5_new_dset.file.filename, {h5_new_dset.name: lazy_load_array(h5_orig_dset)})
    if verbose:
        print('Copying simple attributes of original dataset: {} to destination dataset: {}'.format(h5_orig_dset, h5_new_dset))
...
def copy_linked_objects(h5_source, h5_dest, verbose=False):
    ...
    if same_file:
        warn('{} and {} are in the same HDF5 file. Consider copying references instead of copying linked objects'.format(h5_source, h5_dest))
        return
    h5_dest_grp = h5_dest.parent
    for link_obj_name in h5_source.attrs.keys():
        h5_orig_obj = get_attr(h5_source, link_obj_name)
        if isinstance(h5_orig_obj, h5py.Reference) and not isinstance(h5_orig_obj, h5py.RegionReference):
            h5_orig_obj = h5_source.file[h5_orig_obj]
            if verbose:
                print('Attempting to copy object linked to source: {} as {}'.format(h5_orig_obj, link_obj_name))
            if link_obj_name in h5_dest_grp.keys():
                h5_new_obj = h5_dest_grp[link_obj_name]
                warn('An object with the same name: {} already exists in the destination group: {}'.format(h5_new_obj, h5_dest_grp.name))
                if type(h5_dest_grp[link_obj_name]) != type(h5_orig_obj):
                    mesg = 'Destination parent: {} already has a child named {} that is of type: {} which does not match with that of the object linked with the source dataset: {}'.format(h5_dest_grp, link_obj_name, type(h5_orig_obj), type(h5_new_obj))
                    raise TypeError(mesg)
                else:
                    if isinstance(h5_new_obj, h5py.Dataset):
                        _ = copy_dataset(h5_orig_obj, h5_dest_grp, alias=link_obj_name, verbose=verbose)
                        h5_dest.attrs[link_obj_name] = h5_new_obj.ref
                        continue
                    else:
                        if isinstance(h5_new_obj, h5py.Group):
                            raise ValueError('Destination already contains another HDF5 group: {} with the same name as the source: {}'.format(h5_new_obj, h5_orig_obj))
                        else:
                            raise NotImplementedError('Unable to copy {} objects yet. Contact developer if you need this'.format(type(h5_orig_obj)))
            else:
                if isinstance(h5_orig_obj, h5py.Dataset):
                    h5_new_obj = copy_dataset(h5_orig_obj, h5_dest_grp, alias=link_obj_name, verbose=verbose)
                    h5_dest.attrs[link_obj_name] = h5_new_obj.ref
                else:
                    raise NotImplementedError('Unable to copy {} objects yet. Contact developer if you need this'.format(type(h5_orig_obj)))
...
def write_reduced_anc_dsets(h5_parent_group, h5_inds, h5_vals, dim_name, basename=None, is_spec=None, verbose=False):
    """
    ...
    if is_spec is None:
        ...
        if ind_mat.shape[0] == ind_mat.shape[1]:
            raise ValueError('Unable automatically guess whether the provided datasets are position or spectroscopic. Please explicitely specify via the "is_spec" boolean kwarg')
        if ind_mat.shape[0] < ind_mat.shape[1]:
            is_spec = True
    else:
        ...
        for sub_name in ['_Indices', '_Values']:
            if basename + sub_name in h5_parent_group.keys():
                raise KeyError('Dataset: {} already exists in provided group: {}'.format(basename + sub_name, h5_parent_group.name))
        ...
        if set(dim_name) != set(all_dim_names):
            ...
            for row_ind, row_name in enumerate(labels):
                if is_spec:
                    reg_ref_slices[row_name] = (
                     slice(row_ind, row_ind + 1), slice(None))
                else:
                    reg_ref_slices[row_name] = (
                     slice(None), slice(row_ind, row_ind + 1))
                if verbose:
