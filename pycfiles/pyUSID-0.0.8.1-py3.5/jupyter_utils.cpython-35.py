...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/pyUSID/viz/jupyter_utils.py
# Compiled at: 2019-04-03 21:18:58
# Size of source mod 2**32: 18187 bytes
"""
...
import os, matplotlib.pyplot as plt, ipywidgets as widgets, numpy as np, sys
...
def simple_ndim_visualizer(data_mat, pos_dims, spec_dims, spec_xdim=None, pos_xdim=None, verbose=False):
    ...
    for parm, parm_name in zip([pos_dims, spec_dims], ['pos_dims', 'spec_dims']):
        ...
        if len(parm) > 2:
            raise NotImplementedError('Currently not able to handle more than 2 position or spectroscopic dimensions. {} contains {} dimensions'.format(parm_name, len(parm)))
        elif len(parm) < 1:
            raise ValueError('{} contains too few ({}) dimensions'.format(parm_name, len(parm)))

    if len(pos_dims) + len(spec_dims) != data_mat.ndim:
        raise ValueError('Lengths of pos_dims: {} and spec_dims: {} not matching with that of the dimensions of data_mat: {}'.format(len(pos_dims), len(spec_dims), data_mat.ndim))
            ...
            if item.values is None:
                ...
                if verbose:
                    ...
                    if len(item.values) != data_mat.shape[actual_ind]:
                        raise ValueError('{} dimension {} of size {} in the dataset does not have values of the same length: {}.'.format(dim_type, item.name, data_mat.shape[actual_ind], len(item.values)))
    ...
    if pos_size == 1 or spec_size == 1:
        raise ValueError('Too few position: {} or spectroscopic: {} values to visualize interactively. Consider using alternate visualization approaches'.format(pos_size, spec_size))
    pos_dims_dict = {}
    ...
    def get_slice_string(slice_dict, dim_list):
        ...
        for dimension in dim_list:
            ...
            if dimension.name in slice_dict.keys():
                slice_str += '{} = {} {}\n'.format(dimension.name, dimension.values[slice_dict[dimension.name]], dimension.units)
    ...
    def plot_1d(axis, image_mat, dim_name, dim_dict, component_title):
        ...
        if image_mat.ndim > 1:
            ...
            axis.legend(dim_dict[other_dims].values)
        return img_handle

    def plot_2d(axis, image_mat, clims, dim_list):
        if verbose:
            print('image shape: {}, x_vec: {}, y_vec: {}'.format(image_mat.shape, dim_list[1].values.shape, dim_list[0].values.shape))
        img, cbar = plot_map(axis, image_mat, aspect='auto', clim=clims, x_vec=dim_list[1].values, y_vec=dim_list[0].values)
    ...
    def update_image(axis, img_handle, data_mat, slice_dict, twoD=True):
        if twoD:
            ...
            if y_mat.ndim > 1 and y_mat.shape[0] != len(img_handle):
                y_mat = y_mat.T
    ...
    if data_type == 1:
        ...
        if data_type == 2:
            ...
            if verbose:
                print('Spatial map data shape: {}, Spectrogram data shape: {}'.format(current_spatmap.shape, current_spgram.shape))
    ...
    for dim_ind, dim_name in enumerate([item.name for item in pos_dims]):
        slice_dict[dim_name] = (
         0, sub_data.shape[dim_ind] - 1, 1)

    for dim_ind, dim_name in enumerate([item.name for item in spec_dims]):
        slice_dict[dim_name] = (
         0, sub_data.shape[(dim_ind + len(pos_dims))] - 1, 1)
    ...
    def update_plots(**kwargs):
        ...
        if component_name is not None and component_name != slice_dict['component']:
            if data_type == 1:
...
def save_fig_filebox_button(fig, filename):
    ...
    name_box = widgets.Text(value=filename, placeholder='Type something', description='Output Filename:', disabled=False, layout={'width': '50%'})
