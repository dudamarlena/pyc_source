...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/tests/io/test_write_utils.py
# Compiled at: 2019-12-15 11:40:18
...
class TestMakeIndicesMatrix(unittest.TestCase):
    ...
    def test_3_dims(self):
        expected = np.vstack((np.tile(np.arange(2), 12),
...
class TestDimension(unittest.TestCase):
    ...
    def test_inequality(self):
        name = 'Bias'
        units = 'V'
        self.assertNotEqual(write_utils.Dimension(name, units, [0, 1, 2, 3]), write_utils.Dimension(name, units, [0, 1, 2, 4]))
        self.assertNotEqual(write_utils.Dimension('fdfd', units, [0, 1, 2, 3]), write_utils.Dimension(name, units, [0, 1, 2, 3]))
        self.assertNotEqual(write_utils.Dimension(name, 'fdfd', [0, 1, 2, 3]), write_utils.Dimension(name, units, [0, 1, 2, 3]))
        self.assertNotEqual(write_utils.Dimension(name, units, [0, 1, 2, 3], mode=write_utils.DimType.DEPENDENT), write_utils.Dimension(name, units, [0, 1, 2, 3], mode=write_utils.DimType.INCOMPLETE))
        self.assertNotEqual(write_utils.Dimension(name, units, [0, 1, 2]), write_utils.Dimension(name, units, [0, 1, 2, 3]))
...
class TestCalcChunks(unittest.TestCase):
    ...
    def test_invalid_types(self):
        ...
        with self.assertRaises(TypeError):
            _ = write_utils.calc_chunks((16384, 65536), 2.124)
...
class TestGetSlope(unittest.TestCase):
    ...
    def test_linear_dirty(self):
        expected = 2.5e-10
        vector = np.arange(-1e-09, 1e-09, expected)
        round_error = np.random.rand(vector.size) * 1e-14
        vector += round_error
        actual = write_utils.get_slope(vector, tol=0.001)
