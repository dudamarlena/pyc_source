...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/tests/processing/test_comp_utils.py
# Compiled at: 2020-04-03 18:05:53
# Size of source mod 2**32: 10223 bytes
"""
...
class TestParallelCompute(unittest.TestCase):
    ...
    def test_invalid_func_type(self):
        with self.assertRaises(TypeError):
            _ = comp_utils.parallel_compute(np.random.rand(2, 5), 'not callable')
        with self.assertRaises(TypeError):
            _ = comp_utils.parallel_compute(np.random.rand(2, 5), {'a': 1})

    def test_invalid_data_type(self):
        with self.assertRaises(TypeError):
            _ = comp_utils.parallel_compute([[1, 2, 3], [4, 5, 6]], np.mean)

    def test_invalid_func_args_type(self):
        with self.assertRaises(TypeError):
            _ = comp_utils.parallel_compute(np.random.rand(2, 5), np.mean, func_args={'should be': 'a list'})

    def test_invalid_func_kwargs_type(self):
        with self.assertRaises(TypeError):
            _ = comp_utils.parallel_compute(np.random.rand(2, 5), np.mean, func_kwargs=[
             'not', 'a', 'dict'])

    def base_parallel_compute(self, num_job_scaler=50, cores=None, lengthy_computation=False, func_args=None, expected_cores=None, func_kwargs=None):
        ...
        exp_log = 'Rank 0 starting computing on {} cores (requested {} cores)'.format(expected_cores, cores)
        with data_utils.capture_stdout() as (get_stdout):
            result = comp_utils.parallel_compute(data, np.mean, cores=cores, lengthy_computation=lengthy_computation, func_args=func_args, func_kwargs=func_kwargs, verbose=True)
            logs = get_stdout()
    ...
    def test_respect_requested_cores(self):
        self.base_parallel_compute(num_job_scaler=30, cores=2, expected_cores=2)

    def base_with_custom_func(self, use_args=False, use_kwargs=False, cores=None, expected_cores=None):
        ...
        if use_args and not use_kwargs:
            ...
            if not use_args and use_kwargs:
                ...
                func_args = [0.5]
                func_kwargs = {'arg_2': 3}
        print('func: {}, args: {}, kwargs: {}'.format(func, func_args, func_kwargs))
        ...
        exp_log = 'Rank 0 starting computing on {} cores (requested {} cores)'.format(expected_cores, cores)
        with data_utils.capture_stdout() as (get_stdout):
            result = comp_utils.parallel_compute(data, func, cores=cores, lengthy_computation=False, func_args=func_args, func_kwargs=func_kwargs, verbose=True)
            logs = get_stdout()
        self.assertIsInstance(result, list)
        self.assertEqual(data.shape[0], len(result))
        self.assertTrue(np.allclose(expected, np.array(result)))
    ...
    def test_func_args_only_parallel(self):
        self.base_with_custom_func(use_args=True, use_kwargs=False, cores=None, expected_cores=None)

    def test_func_kwargs_only_parallel(self):
        self.base_with_custom_func(use_args=False, use_kwargs=True, cores=None, expected_cores=None)

    def test_func_args_and_kwargs_parallel(self):
        self.base_with_custom_func(use_args=True, use_kwargs=True, cores=None, expected_cores=None)

    def test_func_args_only_serial(self):
        self.base_with_custom_func(use_args=True, use_kwargs=False, cores=1, expected_cores=1)

    def test_func_kwargs_only_serial(self):
        self.base_with_custom_func(use_args=False, use_kwargs=True, cores=1, expected_cores=1)

    def test_func_args_and_kwargs_serial(self):
        self.base_with_custom_func(use_args=True, use_kwargs=True, cores=1, expected_cores=1)
