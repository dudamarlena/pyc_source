...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/tests/io/test_usi_dataset.py
# Compiled at: 2020-01-13 10:05:53
# Size of source mod 2**32: 38137 bytes
"""
...
import unittest, os, sys, h5py, numpy as np, dask.array as da, matplotlib as mpl
mpl.use('Agg')
...
class TestBEPS(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.pos_dims = []
        self.spec_dims = []
        for dim_name, dim_units in zip(self.h5_source.pos_dim_labels, hdf_utils.get_attr(self.h5_source.h5_pos_inds, 'units')):
            self.pos_dims.append(Dimension(dim_name, dim_units, h5_grp[dim_name][()]))

        for dim_name, dim_units in zip(self.h5_source.spec_dim_labels, hdf_utils.get_attr(self.h5_source.h5_spec_inds, 'units')):
            self.spec_dims.append(Dimension(dim_name, dim_units, h5_grp[dim_name][()]))
...
class TestUSIDatasetReal(unittest.TestCase):
    ...
    def get_expected_n_dim(self, h5_f):
        ...
        return (
         nd_slow_to_fast, nd_fast_to_slow)
...
class TestStringRepr(TestBEPS):
    ...
    def test_string_representation(self):
        ...
        for h5_inds in [usi_dset.h5_pos_inds, usi_dset.h5_spec_inds]:
            for dim_name, dim_size in zip(hdf_utils.get_attr(h5_inds, 'labels'), hdf_utils.get_dimensionality(h5_inds)):
...
class TestPosSpecSlicesReal(TestUSIDatasetReal):
    ...
    def test_both_pos_removed(self):
        with h5py.File(test_h5_file_path, mode='r') as (h5_f):
            ...
            expected_pos = np.expand_dims([8], axis=1)
            expected_spec = np.expand_dims(np.arange(14), axis=1)
            self.assertTrue(np.allclose(expected_spec, actual_spec))
            self.assertTrue(np.allclose(expected_pos, actual_pos))
...
class TestSliceReal(TestUSIDatasetReal):
    ...
    def base(self, slice_dict, f2s_slice_list, result_as_nd, lazy_result, verbose=False):
        with h5py.File(test_h5_file_path, mode='r') as (h5_f):
            usi_main = USIDataset(h5_f['/Raw_Measurement/source_main'])
            actual, success = usi_main.slice(slice_dict, ndim_form=result_as_nd, lazy=lazy_result)
            if verbose:
                print('Status: {}, actual.shape: {}, actual.dtype: {}, type(actual): {}'.format(success, actual.shape, actual.dtype, type(actual)))
            ...
            if result_as_nd:
                ...
                s2f_slice_list = f2s_slice_list[:2][::-1] + f2s_slice_list[2:][::-1]
                if verbose:
                    print('Slice list converted from: {} to {}'.format(f2s_slice_list, s2f_slice_list))
                expected = n_dim_s2f[tuple(s2f_slice_list)]
                if verbose:
                    print('Expected in N-dim form: {}'.format(expected.shape))
                expected = expected.reshape(np.prod(expected.shape[:2]), np.prod(expected.shape[2:]))
                if verbose:
    ...
    def test_negative_index_nd_numpy(self):
        self.base({'X': -2, 'Y': 1}, [
         slice(-2, -1), slice(1, 2)] + [slice(None) for _ in range(2)], True, False)

    def test_negative_index_nd_dask(self):
        self.base({'X': -2, 'Y': 1}, [
         slice(-2, -1), slice(1, 2)] + [slice(None) for _ in range(2)], True, True)
    ...
    def test_one_pos_dim_removed_2d_numpy(self):
        self.base({'X': 3}, [
         slice(3, 4)] + [slice(None) for _ in range(3)], False, False)

    def test_one_pos_dim_removed_2d_dask(self):
        self.base({'X': 3}, [
         slice(3, 4)] + [slice(None) for _ in range(3)], False, True)

    def test_one_pos_dim_sliced_nd_numpy(self):
        self.base({'X': slice(1, 5, 2)}, [
         slice(1, 5, 2)] + [slice(None) for _ in range(3)], True, False)

    def test_one_pos_dim_sliced_nd_dask(self):
        self.base({'X': slice(1, 5, 2)}, [
         slice(1, 5, 2)] + [slice(None) for _ in range(3)], True, True)

    def test_one_pos_dim_sliced_2d_numpy(self):
        self.base({'X': slice(1, 5, 2)}, [
         slice(1, 5, 2)] + [slice(None) for _ in range(3)], False, False)

    def test_one_pos_dim_sliced_2d_dask(self):
        self.base({'X': slice(1, 5, 2)}, [
         slice(1, 5, 2)] + [slice(None) for _ in range(3)], False, True)

    def test_two_pos_dim_sliced_nd_numpy(self):
        self.base({'X': slice(1, 5, 2), 'Y': 1}, [
         slice(1, 5, 2), slice(1, 2)] + [slice(None) for _ in range(2)], True, False)

    def test_two_pos_dim_sliced_nd_dask(self):
        self.base({'X': slice(1, 5, 2), 'Y': 1}, [
         slice(1, 5, 2), slice(1, 2)] + [slice(None) for _ in range(2)], True, True)

    def test_two_pos_dim_sliced_2d_numpy(self):
        self.base({'X': slice(1, 5, 2), 'Y': 1}, [
         slice(1, 5, 2), slice(1, 2)] + [slice(None) for _ in range(2)], False, False)

    def test_two_pos_dim_sliced_2d_dask(self):
        self.base({'X': slice(1, 5, 2), 'Y': 1}, [
         slice(1, 5, 2), slice(1, 2)] + [slice(None) for _ in range(2)], False, True)

    def test_two_pos_dim_sliced_list_nd_numpy(self):
        self.base({'X': [1, 2, 4], 'Y': 1}, [
         [
          1, 2, 4], slice(1, 2)] + [slice(None) for _ in range(2)], True, False)

    def test_two_pos_dim_sliced_list_nd_dask(self):
        self.base({'X': [1, 2, 4], 'Y': 1}, [
         [
          1, 2, 4], slice(1, 2)] + [slice(None) for _ in range(2)], True, True)

    def test_two_pos_dim_sliced_list_2d_numpy(self):
        self.base({'X': [1, 2, 4], 'Y': 1}, [
         [
          1, 2, 4], slice(1, 2)] + [slice(None) for _ in range(2)], False, False)

    def test_two_pos_dim_sliced_list_2d_dask(self):
        self.base({'X': [1, 2, 4], 'Y': 1}, [
         [
          1, 2, 4], slice(1, 2)] + [slice(None) for _ in range(2)], False, True)

    def test_both_pos_removed_nd_numpy(self):
        self.base({'X': 3, 'Y': 1}, [
         slice(3, 4), slice(1, 2)] + [slice(None) for _ in range(2)], True, False)

    def test_both_pos_removed_nd_dask(self):
        self.base({'X': 3, 'Y': 1}, [
         slice(3, 4), slice(1, 2)] + [slice(None) for _ in range(2)], True, True)

    def test_both_pos_removed_2d_numpy(self):
        self.base({'X': 3, 'Y': 1}, [
         slice(3, 4), slice(1, 2)] + [slice(None) for _ in range(2)], False, False)

    def test_both_pos_removed_2d_dask(self):
        self.base({'X': 3, 'Y': 1}, [
         slice(3, 4), slice(1, 2)] + [slice(None) for _ in range(2)], False, True)

    def test_pos_and_spec_sliced_list_nd_numpy(self):
        self.base({'X': [1, 2, 4], 'Bias': slice(1, 7, 3)}, [
         [
          1, 2, 4], slice(None), slice(1, 7, 3), slice(None)], True, False)

    def test_pos_and_spec_sliced_list_nd_dask(self):
        self.base({'X': [1, 2, 4], 'Bias': slice(1, 7, 3)}, [
         [
          1, 2, 4], slice(None), slice(1, 7, 3), slice(None)], True, True)

    def test_pos_and_spec_sliced_list_2d_numpy(self):
        self.base({'X': [1, 2, 4], 'Bias': slice(1, 7, 3)}, [
         [
          1, 2, 4], slice(None), slice(1, 7, 3), slice(None)], False, False)

    def test_pos_and_spec_sliced_list_2d_dask(self):
        self.base({'X': [1, 2, 4], 'Bias': slice(1, 7, 3)}, [
         [
          1, 2, 4], slice(None), slice(1, 7, 3), slice(None)], False, True)

    def test_all_dims_sliced_nd_numpy(self):
        self.base({'X': [1, 2, 4], 'Y': 2, 'Bias': slice(1, 7, 3), 'Cycle': 1}, [
         [
          1, 2, 4], slice(2, 3), slice(1, 7, 3), slice(1, 2)], True, False)

    def test_all_dims_sliced_nd_dask(self):
        self.base({'X': [1, 2, 4], 'Y': 2, 'Bias': slice(1, 7, 3), 'Cycle': 1}, [
         [
          1, 2, 4], slice(2, 3), slice(1, 7, 3), slice(1, 2)], True, True)

    def test_all_dims_sliced_2d_numpy(self):
        self.base({'X': [1, 2, 4], 'Y': 2, 'Bias': slice(1, 7, 3), 'Cycle': 1}, [
         [
          1, 2, 4], slice(2, 3), slice(1, 7, 3), slice(1, 2)], False, False)

    def test_all_dims_sliced_2d_dask(self):
        self.base({'X': [1, 2, 4], 'Y': 2, 'Bias': slice(1, 7, 3), 'Cycle': 1}, [
         [
          1, 2, 4], slice(2, 3), slice(1, 7, 3), slice(1, 2)], False, True)

    def test_all_but_one_dims_sliced_nd_numpy(self):
        self.base({'X': 1, 'Y': 2, 'Bias': slice(1, 7, 3), 'Cycle': 1}, [
         slice(1, 2), slice(2, 3), slice(1, 7, 3), slice(1, 2)], True, False)

    def test_all_but_one_dims_sliced_nd_dask(self):
        self.base({'X': 1, 'Y': 2, 'Bias': slice(1, 7, 3), 'Cycle': 1}, [
         slice(1, 2), slice(2, 3), slice(1, 7, 3), slice(1, 2)], True, True)

    def test_all_but_one_dims_sliced_2d_numpy(self):
        self.base({'X': 1, 'Y': 2, 'Bias': slice(1, 7, 3), 'Cycle': 1}, [
         slice(1, 2), slice(2, 3), slice(1, 7, 3), slice(1, 2)], False, False)

    def test_all_but_one_dims_sliced_2d_dask(self):
        self.base({'X': 1, 'Y': 2, 'Bias': slice(1, 7, 3), 'Cycle': 1}, [
         slice(1, 2), slice(2, 3), slice(1, 7, 3), slice(1, 2)], False, True)

    def test_all_dims_sliced_nd_numpy(self):
        self.base({'X': 1, 'Y': 2, 'Bias': 4, 'Cycle': 1}, [
         slice(1, 2), slice(2, 3), slice(4, 5), slice(1, 2)], True, False)

    def test_all_dims_sliced_nd_dask(self):
        self.base({'X': 1, 'Y': 2, 'Bias': 4, 'Cycle': 1}, [
         slice(1, 2), slice(2, 3), slice(4, 5), slice(1, 2)], True, True)

    def test_all_dims_sliced_2d_dask(self):
        self.base({'X': 1, 'Y': 2, 'Bias': 4, 'Cycle': 1}, [
         slice(1, 2), slice(2, 3), slice(4, 5), slice(1, 2)], False, False)

    def test_all_dims_sliced_2d_dask(self):
        self.base({'X': 1, 'Y': 2, 'Bias': 4, 'Cycle': 1}, [
         slice(1, 2), slice(2, 3), slice(4, 5), slice(1, 2)], False, True)
...
class TestGetDimsForSliceReal(TestUSIDatasetReal):
    ...
    def get_all_dimensions():
        ...
        with h5py.File(test_h5_file_path, mode='r') as (h5_f):
            ...
            for dim_name, dim_units in zip(usi_main.pos_dim_labels, hdf_utils.get_attr(usi_main.h5_pos_inds, 'units')):
                pos_dims.append(Dimension(dim_name, dim_units, h5_raw_grp[dim_name][()]))

            for dim_name, dim_units in zip(usi_main.spec_dim_labels, hdf_utils.get_attr(usi_main.h5_spec_inds, 'units')):
    ...
    def base(self, slice_dict, pos_exp, spec_exp, verbose=False):
        with h5py.File(test_h5_file_path, mode='r') as (h5_f):
            usi_main = USIDataset(h5_f['/Raw_Measurement/source_main'])
            pos_act, spec_act = usi_main._get_dims_for_slice(slice_dict=slice_dict, verbose=verbose)
        if verbose:
            print(pos_act)
            print(spec_act)
        self._TestGetDimsForSliceReal__validate_dim_list(pos_act, pos_exp)
        self._TestGetDimsForSliceReal__validate_dim_list(spec_act, spec_exp)
    ...
    def test_both_spec_dim_sliced(self):
        self.base({'Bias': 4, 'Cycle': 1}, self.pos_dims, [self.default_dimension], verbose=False)
...
def validate_imshow(self, axis, exp_data, title=None, x_vec=None, y_vec=None, x_label=None, y_label=None, verbose=False):
    self.assertIsInstance(axis, mpl.axes.Axes)
    im_handles = [obj for obj in axis.get_children() if isinstance(obj, mpl.image.AxesImage)]
...
def validate_single_curve(self, axis, x_vec, y_vec, title=None, x_label=None, y_label=None):
    self.assertIsInstance(axis, mpl.axes.Axes)
    line_handles = [obj for obj in axis.get_children() if isinstance(obj, mpl.lines.Line2D)]
...
class TestSimpleStaticVisualizationReal(TestUSIDatasetReal):
    ...
    def test_two_pos_simple(self):
        ...
        with h5py.File(test_h5_file_path, mode='r') as (h5_f):
            ...
            validate_imshow(self, axis, exp_data, title=dset_path, x_vec=h5_f[('/Raw_Measurement/' + usi_main.pos_dim_labels[1])], y_vec=h5_f[('/Raw_Measurement/' + usi_main.pos_dim_labels[0])], x_label=usi_main.pos_dim_descriptors[1], y_label=usi_main.pos_dim_descriptors[0])
    ...
    def test_two_spec(self):
        ...
        with h5py.File(test_h5_file_path, mode='r') as (h5_f):
            ...
            self.assertTrue(success)
            fig, axis = usi_main.visualize(slice_dict=slice_dict)
            validate_imshow(self, axis, exp_data, title=dset_path, x_vec=h5_f[('/Raw_Measurement/' + usi_main.spec_dim_labels[1])], y_vec=h5_f[('/Raw_Measurement/' + usi_main.spec_dim_labels[0])], x_label=usi_main.spec_dim_descriptors[1], y_label=usi_main.spec_dim_descriptors[0])
    ...
    def test_one_pos_one_spec(self):
        ...
        with h5py.File(test_h5_file_path, mode='r') as (h5_f):
            ...
            validate_imshow(self, axis, exp_data, title=dset_path, x_vec=h5_f[('/Raw_Measurement/' + usi_main.spec_dim_labels[spec_ind])], y_vec=h5_f[('/Raw_Measurement/' + usi_main.pos_dim_labels[pos_ind])], x_label=usi_main.spec_dim_descriptors[spec_ind], y_label=usi_main.pos_dim_descriptors[pos_ind])
    ...
    def test_one_pos(self):
        ...
        with h5py.File(test_h5_file_path, mode='r') as (h5_f):
            ...
            self.assertTrue(success)
            fig, axis = usi_main.visualize(slice_dict=slice_dict)
            validate_single_curve(self, axis, h5_f[('/Raw_Measurement/' + rem_dim_name)], exp_data, title=dset_path, x_label=usi_main.pos_dim_descriptors[pos_ind], y_label=usi_main.data_descriptor)
    ...
    def test_one_spec(self):
        ...
        with h5py.File(test_h5_file_path, mode='r') as (h5_f):
            ...
            self.assertTrue(success)
            fig, axis = usi_main.visualize(slice_dict=slice_dict)
            validate_single_curve(self, axis, h5_f[('/Raw_Measurement/' + rem_dim_name)], exp_data, title=dset_path, x_label=usi_main.spec_dim_descriptors[spec_ind], y_label=usi_main.data_descriptor)
