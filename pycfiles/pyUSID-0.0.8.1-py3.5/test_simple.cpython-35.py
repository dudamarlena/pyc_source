...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/tests/io/hdf_utils/test_simple.py
# Compiled at: 2020-04-03 17:46:27
# Size of source mod 2**32: 101554 bytes
"""
...
import unittest, os, sys, h5py, numpy as np, shutil
...
class TestSimple(unittest.TestCase):
    ...
    def test_copy_region_refs(self):
        ...
        with h5py.File(file_path, mode='w') as (h5_f):
            ...
            self.assertTrue(np.allclose(h5_dset_source[h5_dset_source.attrs['regref']], h5_dset_dest[h5_dset_dest.attrs['regref']]))
        os.remove(file_path)
...
class TestCheckIfMain(TestSimple):
    ...
    def test_legal(self):
        with h5py.File(data_utils.std_beps_path, mode='r') as (h5_f):
            expected_dsets = [
             h5_f['/Raw_Measurement/source_main'],
...
class TestWriteIndValDsets(TestSimple):
    ...
    def base_bare_minimum_inputs(self, slow_to_fast, is_spectral):
        ...
        data_utils.delete_existing_file(file_path)
        with h5py.File(file_path, mode='w') as (h5_f):
            h5_inds, h5_vals = hdf_utils.write_ind_val_dsets(h5_f, descriptor, is_spectral=is_spectral, slow_to_fast=slow_to_fast)
            data_utils.validate_aux_dset_pair(self, h5_f, h5_inds, h5_vals, dim_names, dim_units, inds_data, is_spectral=is_spectral, slow_to_fast=slow_to_fast)
        os.remove(file_path)
    ...
    def test_legal_override_steps_offsets_base_name(self):
        ...
        with h5py.File(file_path, mode='w') as (h5_f):
            ...
            h5_inds, h5_vals = hdf_utils.write_ind_val_dsets(h5_group, descriptor, is_spectral=True, base_name=new_base_name, slow_to_fast=False)
            data_utils.validate_aux_dset_pair(self, h5_group, h5_inds, h5_vals, dim_names, dim_units, spec_inds, vals_matrix=spec_vals, base_name=new_base_name, is_spectral=True, slow_to_fast=False)
        os.remove(file_path)
...
class TestWriteReducedAncDsets(TestSimple):
    ...
    def test_spec_2d_to_1d(self):
        ...
        with h5py.File(duplicate_path, mode='r+') as (h5_f):
            ...
            new_base_name = 'Blah'
            h5_spec_inds_new, h5_spec_vals_new = hdf_utils.write_reduced_anc_dsets(h5_spec_inds_orig.parent, h5_spec_inds_orig, h5_spec_vals_orig, 'Bias', basename=new_base_name)
            dim_names = [
             'Cycle']
            dim_units = ['']
            ...
            for h5_dset, exp_dtype, exp_name in zip([h5_spec_inds_new, h5_spec_vals_new], [
             ...
             new_base_name + '_Indices', new_base_name + '_Values']):
                ...
                for dim_ind, curr_name in enumerate(dim_names):
                    self.assertTrue(np.allclose(np.squeeze(ref_data[dim_ind]), np.squeeze(h5_dset[h5_dset.attrs[curr_name]])))
    ...
    def test_spec_2d_to_1d_new_file(self):
        ...
        with h5py.File(data_utils.std_beps_path, mode='r') as (h5_f):
            ...
            with h5py.File(new_file_path, mode='w') as (h5_f_new):
                h5_spec_inds_new, h5_spec_vals_new = hdf_utils.write_reduced_anc_dsets(h5_f_new, h5_spec_inds_orig, h5_spec_vals_orig, 'Bias', basename=new_base_name)
                dim_names = [
                 'Cycle']
                dim_units = ['']
                ref_data = np.expand_dims(np.arange(2), axis=0)
    ...
    def test_spec_1d_to_0d(self):
        ...
        with h5py.File(duplicate_path, mode='w') as (h5_f):
            h5_spec_inds_orig, h5_spec_vals_orig = hdf_utils.write_ind_val_dsets(h5_f, write_utils.Dimension('Bias', 'V', 10), is_spectral=True)
            new_base_name = 'Blah'
            h5_spec_inds_new, h5_spec_vals_new = hdf_utils.write_reduced_anc_dsets(h5_f, h5_spec_inds_orig, h5_spec_vals_orig, 'Bias', basename=new_base_name)
            dim_names = [
             'Single_Step']
            dim_units = ['a. u.']
    ...
    def test_3d_to_1d_pos_fastest_n_slowest(self):
        ...
        with h5py.File(duplicate_path, mode='w') as (h5_f):
            ...
            h5_spec_inds_new, h5_spec_vals_new = hdf_utils.write_reduced_anc_dsets(h5_grp, h5_spec_inds_orig, h5_spec_vals_orig, ['X', 'Z'])
            dim_names = [
             'Y']
            dim_units = ['um']
    ...
    def test_3d_to_1d_spec_fastest_n_slowest(self):
        ...
        with h5py.File(duplicate_path, mode='w') as (h5_f):
            ...
            new_base_name = 'Blah'
            h5_spec_inds_new, h5_spec_vals_new = hdf_utils.write_reduced_anc_dsets(h5_f, h5_spec_inds_orig, h5_spec_vals_orig, [
             'Freq', 'Cycle'], basename=new_base_name)
            dim_names = [
             'Bias']
            dim_units = ['V']
    ...
    def test_3d_to_1d_spec_fastest(self):
        ...
        with h5py.File(duplicate_path, mode='w') as (h5_f):
            ...
            new_base_name = 'Blah'
            h5_spec_inds_new, h5_spec_vals_new = hdf_utils.write_reduced_anc_dsets(h5_f, h5_spec_inds_orig, h5_spec_vals_orig, [
             'Freq', 'Bias'], basename=new_base_name)
            dim_names = [
             'Cycle']
            dim_units = ['a.u.']
    ...
    def test_3d_to_1d_spec_slowest(self):
        ...
        with h5py.File(duplicate_path, mode='w') as (h5_f):
            ...
            new_base_name = 'Blah'
            h5_spec_inds_new, h5_spec_vals_new = hdf_utils.write_reduced_anc_dsets(h5_f, h5_spec_inds_orig, h5_spec_vals_orig, [
             'Cycle', 'Bias'], basename=new_base_name)
            dim_names = [
             'Freq']
            dim_units = ['Hz']
    ...
    def test_3d_to_2d_spec_fastest_n_slowest(self):
        ...
        with h5py.File(duplicate_path, mode='w') as (h5_f):
            dims = [write_utils.Dimension('Freq', 'Hz', np.linspace(300, 350, 5)),
             write_utils.Dimension('Bias', 'V', [-2, 4, 10]),
             write_utils.Dimension('Cycle', 'a.u.', 2)]
            h5_spec_inds_orig, h5_spec_vals_orig = hdf_utils.write_ind_val_dsets(h5_f, dims, is_spectral=True, slow_to_fast=False)
            new_base_name = 'Blah'
            h5_spec_inds_new, h5_spec_vals_new = hdf_utils.write_reduced_anc_dsets(h5_f, h5_spec_inds_orig, h5_spec_vals_orig, [
...
class TestFindResultsGroup(TestSimple):
    ...
    def test_results_in_diff_file(self):
        ...
        with h5py.File(file_path, mode='w') as (h5_f):
            ...
            with h5py.File(new_path, mode='w') as (h5_f_2):
                ...
                grps = hdf_utils.find_results_groups(h5_main, 'Tool', h5_parent_group=h5_f_2)
    ...
    def test_results_in_diff_file_invalid_type(self):
        ...
        with h5py.File(file_path, mode='w') as (h5_f):
            h5_main = h5_f.create_dataset('Main', data=[1, 2, 3])
            with self.assertRaises(TypeError):
                _ = hdf_utils.find_results_groups(h5_main, 'Tool', h5_parent_group=h5_main)
        os.remove(file_path)
...
class TestCreateResultsGroup(unittest.TestCase):
    ...
    def test_invalid_types(self):
        ...
        with h5py.File(file_path, mode='w') as (h5_f):
            ...
            h5_dset = h5_f.create_dataset('Main', data=[1, 2, 3])
            with self.assertRaises(TypeError):
                _ = hdf_utils.create_results_group(h5_dset, 'Tool', h5_parent_group='not_group')
        os.remove(file_path)
    ...
    def test_different_file(self):
        ...
        with h5py.File(file_path, mode='w') as (h5_f):
            ...
            with h5py.File(new_path, mode='w') as (h5_f_new):
                ...
                h5_group = hdf_utils.create_results_group(h5_dset, 'Tool', h5_parent_group=h5_f_new)
                self.assertIsInstance(h5_group, h5py.Group)
...
class TestLinkAsMain(unittest.TestCase):
    ...
    def helper_test(self, quant_units_specified):
        ...
        with h5py.File(file_path, mode='w') as (h5_f):
            ...
            h5_source_spec_inds = h5_raw_grp.create_dataset('Spectroscopic_Indices', data=source_spec_data, dtype=np.uint16)
            ...
            h5_source_spec_vals = h5_raw_grp.create_dataset('Spectroscopic_Values', data=source_spec_data, dtype=np.float32)
            ...
            usid_source = hdf_utils.link_as_main(h5_source_main, h5_pos_inds, h5_pos_vals, h5_source_spec_inds, h5_source_spec_vals)
            if quant_units_specified:
...
class TestCreateEmptyDataset(unittest.TestCase):
    ...
    def test_invalid_types(self):
        ...
        with h5py.File(file_path, mode='w') as (h5_f):
            ...
            with self.assertRaises(TypeError):
                _ = hdf_utils.create_empty_dataset(h5_dset_source, np.float16, 'Duplicate', new_attrs='not_a_dictionary')
            with self.assertRaises(TypeError):
                _ = hdf_utils.create_empty_dataset(h5_dset_source, np.float16, 'Duplicate', h5_group=h5_dset_source)
        os.remove(file_path)
    ...
    def validate_copied_dataset(self, h5_f_new, h5_dest, dset_new_name, dset_data, dset_attrs):
        ...
        self.assertEqual(len(dset_attrs), len(h5_anc_dest.attrs.keys()))
        ...
        self.assertEqual(h5_f_new[h5_dest.attrs[dset_new_name]], h5_anc_dest)
    ...
    def test_diff_file_new_attrs_linked_dsets(self):
        ...
        with h5py.File(file_path, mode='w') as (h5_f):
            ...
            with h5py.File(new_path, mode='w') as (h5_f_new):
                ...
                if sys.version_info.major == 3:
                    with self.assertWarns(UserWarning):
                        h5_duplicate = func(h5_dset_source, np.float16, 'Duplicate', h5_group=h5_f_new, new_attrs=easy_attrs, skip_refs=False)
                else:
                    h5_duplicate = func(h5_dset_source, np.float16, 'Duplicate', h5_group=h5_f_new, new_attrs=easy_attrs, skip_refs=False)
                self.assertIsInstance(h5_duplicate, h5py.Dataset)
                ...
                self.validate_copied_dataset(h5_f_new, h5_duplicate, 'dset_ref', h5_other[()], anc_attrs)
        os.remove(file_path)
    ...
    def test_diff_groups(self):
        ...
        with h5py.File(file_path, mode='w') as (h5_f):
            ...
            h5_dset_source.attrs.update(existing_attrs)
            h5_group = h5_f.create_group('Group')
            h5_duplicate = hdf_utils.create_empty_dataset(h5_dset_source, np.float16, 'Duplicate', h5_group=h5_group, new_attrs=easy_attrs)
            self.assertIsInstance(h5_duplicate, h5py.Dataset)
...
class TestCheckAndLinkAncillary(TestSimple):
    ...
    def test_one_dset_to_name(self):
        ...
        with h5py.File(file_path, mode='w') as (h5_f):
            ...
            att_name = 'root'
            expected = h5_dset_2
            hdf_utils.check_and_link_ancillary(h5_dset_source, att_name, h5_main=None, anc_refs=expected)
    ...
    def test_many_objs_to_many_names(self):
        ...
        with h5py.File(file_path, mode='w') as (h5_f):
            ...
            hdf_utils.check_and_link_ancillary(h5_dset_source, att_names, h5_main=None, anc_refs=expected)
    ...
    def test_no_anc_refs_provided(self):
        ...
        with h5py.File(file_path, mode='w') as (h5_f):
            h5_dset_source = h5_f.create_dataset('Source', data=[1, 2, 3])
            with self.assertRaises(ValueError):
                hdf_utils.check_and_link_ancillary(h5_dset_source, ['M', 'Wa'], h5_main=None, anc_refs=None)
        os.remove(file_path)
    ...
    def test_linking_main_legit_anc_names(self):
        ...
        with h5py.File(file_path, mode='r+') as (h5_f):
            ...
            hdf_utils.check_and_link_ancillary(h5_dset_source, att_names, h5_main=h5_main, anc_refs=None)
            self.assertEqual(len(h5_dset_source.attrs.keys()), len(att_names))
            self.assertEqual(set(att_names), set(h5_dset_source.attrs.keys()))
            for name, exp_val in zip(att_names, expected):
                actual = h5_dset_source.attrs[name]
...
class TestValidateAncH5Dsets(TestSimple):
    ...
    def test_valid_shapes(self):
        with h5py.File(data_utils.std_beps_path, mode='r') as (h5_f):
            ...
            hdf_utils.validate_anc_h5_dsets(h5_pos_inds, h5_pos_vals, h5_main.shape, is_spectroscopic=False)
            hdf_utils.validate_anc_h5_dsets(h5_pos_inds, h5_pos_vals, list(h5_main.shape), is_spectroscopic=False)
            h5_spec_inds = h5_grp['Spectroscopic_Indices']
            h5_spec_vals = h5_grp['Spectroscopic_Values']
            hdf_utils.validate_anc_h5_dsets(h5_spec_inds, h5_spec_vals, h5_main.shape, is_spectroscopic=True)
            hdf_utils.validate_anc_h5_dsets(h5_spec_inds, h5_spec_vals, list(h5_main.shape), is_spectroscopic=True)
    ...
    def test_invalid_spec_pos_bool(self):
        with h5py.File(data_utils.std_beps_path, mode='r') as (h5_f):
            ...
            h5_pos_vals = h5_grp['Position_Values']
            with self.assertRaises(ValueError):
                hdf_utils.validate_anc_h5_dsets(h5_pos_inds, h5_pos_vals, h5_main.shape, is_spectroscopic=True)
            h5_spec_inds = h5_grp['Spectroscopic_Indices']
            h5_spec_vals = h5_grp['Spectroscopic_Values']
            with self.assertRaises(ValueError):
                hdf_utils.validate_anc_h5_dsets(h5_spec_inds, h5_spec_vals, h5_main.shape, is_spectroscopic=False)
    ...
    def test_invalid_dtypes(self):
        with h5py.File(data_utils.std_beps_path, mode='r') as (h5_f):
            ...
            with self.assertRaises(TypeError):
                hdf_utils.validate_anc_h5_dsets('h5_pos_inds', h5_pos_vals, h5_main.shape, is_spectroscopic=False)
            with self.assertRaises(TypeError):
                hdf_utils.validate_anc_h5_dsets(h5_pos_inds, h5_pos_vals, np.arange(3), is_spectroscopic=False)
...
class TestValidateDimsAgainstMAin(unittest.TestCase):
    ...
    def test_single_dim(self):
        ...
        func((1, 5), write_utils.Dimension('blah', 'meh', np.arange(5)), is_spectroscopic=True)
        func((5, 1), [write_utils.Dimension('blah', 'meh', np.arange(5))], is_spectroscopic=False)

    def test_multi_dims(self):
        func = hdf_utils.validate_dims_against_main
        func((1, 15), [write_utils.Dimension('a', 'b', 5),
         write_utils.Dimension('a', 'b', 3)], is_spectroscopic=True)
        func((15, 5), [write_utils.Dimension('a', 'b', 5),
         write_utils.Dimension('a', 'b', 3)], is_spectroscopic=False)
    ...
    def test_invalid_dims(self):
        ...
        with self.assertRaises(ValueError):
            func((1, 25), [write_utils.Dimension('a', 'b', 5),
             write_utils.Dimension('a', 'b', 3)], is_spectroscopic=True)
        with self.assertRaises(ValueError):
            func((25, 5), [write_utils.Dimension('a', 'b', 5)], is_spectroscopic=False)
...
class TestCopyLinkedObjects(TestSimple):
    ...
    def base_two_dsets_simple_attrs(self, exist_dset_same_data=False, exist_dset_diff_data_shape=False, exist_dset_diff_data=False, exist_grp_inst_dset=False):
        file_path = 'test.h5'
        new_path = 'new.h5'
        data_utils.delete_existing_file(file_path)
        data_utils.delete_existing_file(new_path)
        with h5py.File(file_path, mode='w') as (h5_f):
            ...
            with h5py.File(new_path, mode='w') as (h5_f_new):
                ...
                if exist_dset_same_data:
                    ...
                    if exist_dset_diff_data:
                        _ = h5_f_new.create_dataset('Pos_Vals', data=[8, 1, 3])
                    else:
                        if exist_dset_diff_data_shape:
                            _ = h5_f_new.create_dataset('Pos_Vals', data=np.random.rand(5, 3))
                ...
                if exist_dset_diff_data or exist_dset_diff_data_shape:
                    ...
                    if exist_grp_inst_dset:
                        with self.assertRaises(TypeError):
                            hdf_utils.copy_linked_objects(h5_source, h5_dest, verbose=False)
                    else:
                        hdf_utils.copy_linked_objects(h5_source, h5_dest, verbose=False)
                    if not exist_dset_diff_data_shape and not exist_dset_diff_data and not exist_grp_inst_dset:
                        self.assertEqual(len(h5_f_new.keys()), 3)
                        self.validate_copied_dataset(h5_f_new, h5_dest, 'Pos_Inds', h5_anc_1[()], anc_1_attrs)
                        self.validate_copied_dataset(h5_f_new, h5_dest, 'Pos_Vals', h5_anc_2[()], anc_2_attrs)
        os.remove(file_path)
        os.remove(new_path)

    def test_two_dsets_simple_attrs_empty_dest(self):
        self.base_two_dsets_simple_attrs(exist_dset_same_data=False, exist_dset_diff_data_shape=False, exist_dset_diff_data=False, exist_grp_inst_dset=False)

    def test_existing_anc_dset_same_data_no_attrs(self):
        self.base_two_dsets_simple_attrs(exist_dset_same_data=True, exist_dset_diff_data_shape=False, exist_dset_diff_data=False, exist_grp_inst_dset=False)

    def test_existing_anc_dset_diff_data(self):
        self.base_two_dsets_simple_attrs(exist_dset_same_data=False, exist_dset_diff_data_shape=False, exist_dset_diff_data=True, exist_grp_inst_dset=False)

    def test_existing_anc_dset_diff_data_shape(self):
        self.base_two_dsets_simple_attrs(exist_dset_same_data=False, exist_dset_diff_data_shape=True, exist_dset_diff_data=False, exist_grp_inst_dset=False)

    def test_existing_group_instead_of_det(self):
        self.base_two_dsets_simple_attrs(exist_dset_same_data=False, exist_dset_diff_data_shape=False, exist_dset_diff_data=False, exist_grp_inst_dset=True)


class TestCopyDataset(TestSimple):

    def validate_copied_dataset(self, h5_f_new, dset_new_name, dset_data, dset_attrs):
        self.assertTrue(dset_new_name in h5_f_new.keys())
        h5_anc_dest = h5_f_new[dset_new_name]
        self.assertIsInstance(h5_anc_dest, h5py.Dataset)
        self.assertTrue(np.allclose(dset_data, h5_anc_dest[()]))
    ...
    def base_test(self, exist_dset_same_data=False, use_alias=False, exist_dset_diff_data_shape=False, exist_dset_diff_data=False, exist_grp_inst_dset=False):
        file_path = 'test.h5'
        new_path = 'new.h5'
        data_utils.delete_existing_file(file_path)
        data_utils.delete_existing_file(new_path)
            ...
            with h5py.File(new_path, mode='w') as (h5_f_new):
                ...
                if exist_dset_same_data:
                    ...
                    if exist_dset_diff_data:
                        ...
                        if exist_dset_diff_data_shape:
                            _ = h5_f_new.create_dataset(alias, data=np.random.rand(5, 3))
                        elif exist_grp_inst_dset:
                ...
                if exist_dset_diff_data or exist_dset_diff_data_shape:
                    ...
                    if not exist_dset_diff_data_shape and not exist_dset_diff_data and not exist_grp_inst_dset:
                        ...
                        self.validate_copied_dataset(h5_f_new, alias, h5_source[()], simple_attrs)
        os.remove(file_path)
        os.remove(new_path)

    def test_exact_copy(self):
        self.base_test(exist_dset_same_data=False, use_alias=False, exist_dset_diff_data_shape=False, exist_dset_diff_data=False, exist_grp_inst_dset=False)

    def test_copy_w_alias(self):
        self.base_test(exist_dset_same_data=False, use_alias=True, exist_dset_diff_data_shape=False, exist_dset_diff_data=False, exist_grp_inst_dset=False)

    def test_existing_group_same_name(self):
        self.base_test(exist_dset_same_data=False, use_alias=False, exist_dset_diff_data_shape=False, exist_dset_diff_data=False, exist_grp_inst_dset=True)

    def test_existing_dset_same_name_data(self):
        self.base_test(exist_dset_same_data=True, use_alias=False, exist_dset_diff_data_shape=False, exist_dset_diff_data=False, exist_grp_inst_dset=False)

    def test_existing_dset_same_name_diff_data_shape(self):
        self.base_test(exist_dset_same_data=False, use_alias=False, exist_dset_diff_data_shape=True, exist_dset_diff_data=False, exist_grp_inst_dset=False)

    def test_existing_dset_same_name_diff_data(self):
        self.base_test(exist_dset_same_data=False, use_alias=False, exist_dset_diff_data_shape=False, exist_dset_diff_data=True, exist_grp_inst_dset=False)
