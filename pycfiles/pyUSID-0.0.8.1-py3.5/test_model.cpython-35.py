...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/tests/io/hdf_utils/test_model.py
# Compiled at: 2020-03-29 19:00:10
# Size of source mod 2**32: 48035 bytes
"""
...
import unittest, os, sys, h5py, numpy as np, dask.array as da, shutil
...
class TestGetDimensionality(TestModel):
    ...
    def test_legal_no_sort(self):
        self._TestGetDimensionality__helper_no_sort(hdf_dsets=True)
        self._TestGetDimensionality__helper_no_sort(hdf_dsets=False)
...
class TestGetSortOrder(TestModel):
    ...
    def test_reversed(self):
        with h5py.File(data_utils.std_beps_path, mode='r') as (h5_f):
            h5_dsets = [
             np.flipud(h5_f['/Raw_Measurement/Spectroscopic_Indices']),
             h5_f['/Raw_Measurement/source_main-Fitter_000/Spectroscopic_Indices'],
...
class TestReshapeToNDims(TestModel):
    ...
    def test_h5_already_sorted(self):
        with h5py.File(data_utils.std_beps_path, mode='r') as (h5_f):
            ...
            n_dim, success, labels = hdf_utils.reshape_to_n_dims(h5_main, get_labels=True, sort_dims=False, lazy=False, verbose=True)
            ...
            n_dim, success, labels = hdf_utils.reshape_to_n_dims(h5_main, get_labels=True, sort_dims=True, lazy=False, verbose=True)
            self.assertTrue(success)
    ...
    def test_h5_manually_provided_anc_dsets_h5(self):
        with h5py.File(data_utils.std_beps_path, mode='r') as (h5_f):
            ...
            n_dim, success, labels = hdf_utils.reshape_to_n_dims(h5_main, h5_pos=h5_pos_inds, h5_spec=h5_spec_inds, get_labels=True, sort_dims=False, lazy=False, verbose=True)
            self.assertTrue(np.all([x == y for x, y in zip(labels, exp_labs)]))
            self.assertTrue(success)
            self.assertTrue(np.allclose(nd_fast_to_slow, n_dim))
            n_dim, success, labels = hdf_utils.reshape_to_n_dims(h5_main, h5_pos=h5_pos_inds, h5_spec=None, get_labels=True, sort_dims=False, lazy=False, verbose=True)
            self.assertTrue(np.all([x == y for x, y in zip(labels, exp_labs)]))
            self.assertTrue(success)
            self.assertTrue(np.allclose(nd_fast_to_slow, n_dim))
            n_dim, success, labels = hdf_utils.reshape_to_n_dims(h5_main, h5_pos=None, h5_spec=h5_spec_inds, get_labels=True, sort_dims=False, lazy=False, verbose=True)
            self.assertTrue(np.all([x == y for x, y in zip(labels, exp_labs)]))
            self.assertTrue(success)
            self.assertTrue(np.allclose(nd_fast_to_slow, n_dim))
        ...
        for row_ind in range(num_rows):
            for col_ind in range(num_cols):
                for cycle_ind in range(num_cycles):
                    val = 1000.0 * row_ind + 100.0 * col_ind + 10.0 * cycle_ind + np.arange(num_cycle_pts)
                    main_nd[(row_ind, col_ind, cycle_ind)] = val

        return (
         main_nd, pos_inds, spec_inds)

    def base_comparison_4d(self, flip_pos_inds, flip_spec_inds, lazy_in=False, lazy_out=False, verbose=False):
        exp_nd_s2f, pos_inds, spec_inds = self.build_main_anc_4d()
        main_2d = exp_nd_s2f.reshape(np.prod(exp_nd_s2f.shape[:2]), np.prod(exp_nd_s2f.shape[2:]))
        labs_s2f = [
         'Position Dimension 1', 'Position Dimension 0',
        ...
        n_dim, suc, labs = hdf_utils.reshape_to_n_dims(main_2d, h5_pos=pos_inds, h5_spec=spec_inds, sort_dims=True, get_labels=True, lazy=lazy_out, verbose=verbose)
        if lazy_out:
        ...
        if verbose:
            print('~~~~~~~~~~~~~~~~~~~~~~ UNSORTED ~~~~~~~~~~~~~~~~~~~~~~~~~')
        n_dim, suc, labs = hdf_utils.reshape_to_n_dims(main_2d, h5_pos=pos_inds, h5_spec=spec_inds, sort_dims=False, get_labels=True, lazy=lazy_out, verbose=verbose)
        if lazy_out:
            self.assertIsInstance(n_dim, da.core.Array)
        if verbose:
        ...
        if verbose:
            print('Expected N-dim shape: {} and labels: {}'.format(exp_nd.shape, exp_labs))
    ...
    def build_main_anc_1_2d(self, is_2d=True, is_spec=False):
        ...
        pos_inds = np.vstack((np.tile(np.arange(num_cols), num_rows),
         np.repeat(np.arange(num_rows), num_cols))).T
        main_nd = np.random.randint(0, high=255, size=(num_rows, num_cols), dtype=np.uint8)
        if not is_2d:
            pos_inds = np.expand_dims(np.arange(num_rows), axis=1)
            main_nd = np.random.randint(0, high=255, size=num_rows, dtype=np.uint8)
        spec_inds = np.expand_dims([0], axis=0)
        if is_spec:
            return (main_nd, spec_inds, pos_inds.T)
        return (main_nd, pos_inds, spec_inds)

    def base_comparison_1_2d(self, is_2d, is_spec, flip_inds, lazy_in=False, lazy_out=False):
        main_nd, pos_inds, spec_inds = self.build_main_anc_1_2d(is_2d=is_2d, is_spec=is_spec)
        ...
        if lazy_in:
            main_2d = da.from_array(main_2d, chunks=main_2d.shape)
        n_dim, success = hdf_utils.reshape_to_n_dims(main_2d, h5_pos=pos_inds, h5_spec=spec_inds, sort_dims=True, get_labels=False, lazy=lazy_out, verbose=True)
        if lazy_out:
            self.assertIsInstance(n_dim, da.core.Array)
        self.assertTrue(np.allclose(main_nd_w_sing, n_dim))
        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        n_dim, success = hdf_utils.reshape_to_n_dims(main_2d, h5_pos=pos_inds, h5_spec=spec_inds, sort_dims=False, get_labels=False, lazy=lazy_out, verbose=True)
        if lazy_out:
            self.assertIsInstance(n_dim, da.core.Array)
        if is_2d:
    ...
    def test_dask_in_ordinary_1d_pos(self):
        self.base_comparison_1_2d(False, False, False, lazy_in=True, lazy_out=False)

    def test_dask_out_ordinary_1d_pos(self):
        self.base_comparison_1_2d(False, False, False, lazy_in=False, lazy_out=True)

    def test_dask_all_ordinary_1d_pos(self):
        self.base_comparison_1_2d(False, False, False, lazy_in=True, lazy_out=True)

    def test_numpy_ordinary_1d_spec(self):
        self.base_comparison_1_2d(False, True, False)

    def test_dask_in_ordinary_1d_spec(self):
        self.base_comparison_1_2d(False, True, False, lazy_in=True, lazy_out=False)

    def test_dask_out_ordinary_1d_spec(self):
        self.base_comparison_1_2d(False, True, False, lazy_in=False, lazy_out=True)

    def test_dask_all_ordinary_1d_spec(self):
        self.base_comparison_1_2d(False, True, False, lazy_in=True, lazy_out=True)
    ...
    def test_h5_both_inds_flipped(self):
        ...
        with h5py.File(file_path, mode='w') as (h5_f):
            ...
            source_main_data = main_nd.reshape(np.prod(main_nd.shape[:2]), np.prod(main_nd.shape[2:]))
            ...
            n_dim, success, labels = hdf_utils.reshape_to_n_dims(h5_source_main, get_labels=True, sort_dims=True, lazy=False, verbose=False)
            self.assertTrue(np.all([x == y for x, y in zip(labels, ['Y', 'X', 'Cycle', 'Bias'])]))
            self.assertTrue(np.allclose(main_nd, n_dim))
            expected_n_dim = main_nd
            n_dim, success, labels = hdf_utils.reshape_to_n_dims(h5_source_main, get_labels=True, sort_dims=False, lazy=False, verbose=False)
            self.assertTrue(np.all([x == y for x, y in zip(labels, ['Y', 'X', 'Cycle', 'Bias'])]))
    ...
    def test_h5_beps_field(self):
        ...
        with h5py.File(file_path, mode='w') as (h5_f):
            ...
            main_nd = np.zeros(shape=(
             num_rows, num_cols, num_fields, num_cycle_pts), dtype=np.float16)
            for row_ind in range(num_rows):
                for col_ind in range(num_cols):
                    main_nd[(row_ind, col_ind)] = 1000.0 * row_ind + 100.0 * col_ind + test
            ...
            n_dim, success, labels = hdf_utils.reshape_to_n_dims(h5_source_main, get_labels=True, sort_dims=True, lazy=False, verbose=False)
            self.assertTrue(np.all([x == y for x, y in zip(labels, ['Y', 'X', 'Bias', 'Field'])]))
            self.assertTrue(np.allclose(main_nd, n_dim))
            expected_n_dim = np.transpose(main_nd, [1, 0, 3, 2])
            n_dim, success, labels = hdf_utils.reshape_to_n_dims(h5_source_main, get_labels=True, sort_dims=False, lazy=False, verbose=False)
            self.assertTrue(np.all([x == y for x, y in zip(labels, ['X', 'Y', 'Field', 'Bias'])]))
            self.assertTrue(np.allclose(expected_n_dim, n_dim))
        os.remove(file_path)
...
class TestReshapeFromNDims(TestModel):
    ...
    def test_pos_and_spec_provided(self):
            ...
            for col_ind in range(num_cols):
                for cycle_ind in range(num_cycles):
                    for bias_ind in range(num_cycle_pts):
                        val = 1000.0 * row_ind + 100.0 * col_ind + 10.0 * cycle_ind + bias_ind
        ...
        source_spec_data = np.vstack((np.tile(np.arange(num_cycle_pts), num_cycles),
         np.repeat(np.arange(num_cycles), num_cycle_pts)))
        ret_2d, success = hdf_utils.reshape_from_n_dims(main_pos_sorted, h5_pos=source_pos_data, h5_spec=source_spec_data)
        self.assertTrue(success)
        self.assertTrue(np.allclose(ret_2d, expected_2d))
        ...
        source_spec_data = np.vstack((np.repeat(np.arange(num_cycles), num_cycle_pts),
         np.tile(np.arange(num_cycle_pts), num_cycles)))
        ret_2d, success = hdf_utils.reshape_from_n_dims(main_spec_sorted, h5_pos=source_pos_data, h5_spec=source_spec_data)
        self.assertTrue(success)
        self.assertTrue(np.allclose(ret_2d, expected_2d))
        ...
        source_spec_data = np.vstack((np.tile(np.arange(num_cycle_pts), num_cycles),
         np.repeat(np.arange(num_cycles), num_cycle_pts)))
        ret_2d, success = hdf_utils.reshape_from_n_dims(main_not_sorted, h5_pos=source_pos_data, h5_spec=source_spec_data)
        self.assertTrue(success)
        self.assertTrue(np.allclose(ret_2d, expected_2d))
...
class TestWriteMainDataset(TestModel):
    ...
    def base_write(self, lazy_main=False, empty_main=False, pre_pos=False, pre_spec=False, to_new_file=False):
        file_path = 'test.h5'
        ...
        with h5py.File(file_path, mode='w') as (h5_f):
            ...
            if to_new_file:
                ...
                targ_loc = h5_f_2
            usid_main = hdf_utils.write_main_dataset(targ_loc, input_data, main_data_name, quantity, dset_units, pos_dims, spec_dims, main_dset_attrs=None, slow_to_fast=False, verbose=True, **kwargs)
            ...
            data_utils.validate_aux_dset_pair(self, targ_loc, usid_main.h5_pos_inds, usid_main.h5_pos_vals, pos_names, pos_units, pos_data, h5_main=usid_main, is_spectral=False, slow_to_fast=False)
            data_utils.validate_aux_dset_pair(self, targ_loc, usid_main.h5_spec_inds, usid_main.h5_spec_vals, spec_names, spec_units, spec_data, h5_main=usid_main, is_spectral=True, slow_to_fast=False)
        if to_new_file:
    ...
    def test_prod_sizes_mismatch(self):
        ...
        with h5py.File(file_path, mode='w') as (h5_f):
            with self.assertRaises(ValueError):
                _ = hdf_utils.write_main_dataset(h5_f, main_data, main_data_name, quantity, dset_units, pos_dims, spec_dims)
        os.remove(file_path)
