...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/tests/io/data_utils.py
# Compiled at: 2020-03-29 18:50:49
# Size of source mod 2**32: 25526 bytes
...
def write_aux_reg_ref(h5_dset, labels, is_spec=True):
    for index, reg_ref_name in enumerate(labels):
        if is_spec:
            reg_ref_tuple = (
             slice(index, index + 1), slice(None))
        else:
            reg_ref_tuple = (
...
def validate_aux_dset_pair(test_class, h5_group, h5_inds, h5_vals, dim_names, dim_units, inds_matrix, vals_matrix=None, base_name=None, h5_main=None, is_spectral=True, slow_to_fast=False):
...
def make_sparse_sampling_file():
    ...
    with h5py.File(sparse_sampling_path, mode='w') as (h5_f):
        ...
        for h5_chan_grp, add_attribute in zip([h5_chan_grp_1, h5_chan_grp_2], [False, True]):
            ...
            h5_main = h5_chan_grp.create_dataset('Raw_Data', data=np.random.rand(len(chosen_pos), freq_pts), dtype=np.float32)
...
def make_incomplete_measurement_file():
    ...
    with h5py.File(incomplete_measurement_path, mode='w') as (h5_f):
        ...
        def _create_pos_and_main(h5_group, add_attribute):
            h5_pos_inds = h5_group.create_dataset('Position_Indices', data=pos_inds[:tot_positions], dtype=np.uint16)
            h5_pos_vals = h5_group.create_dataset('Position_Values', data=pos_vals[:tot_positions], dtype=np.float32)
            this_pos_attrs = pos_attrs.copy()
            if add_attribute:
            ...
            h5_main = h5_group.create_dataset('Raw_Data', data=np.random.rand(tot_positions, freq_pts * dc_offsets), dtype=np.float32)
            write_safe_attrs(h5_main, {'units': 'V', 'quantity': 'Cantilever Deflection'})
            for dset in [h5_pos_inds, h5_pos_vals, h5_spec_inds, h5_spec_vals]:
                h5_main.attrs[dset.name.split('/')[(-1)]] = dset.ref
...
def make_relaxation_file():
    ...
    with h5py.File(relaxation_path, mode='w') as (h5_f):
        ...
        for h5_chan_grp, add_attribute in zip([h5_chan_grp_1, h5_chan_grp_2], [False, True]):
            ...
            h5_main = h5_chan_grp.create_dataset('Raw_Data', data=np.random.rand(num_rows * num_cols, freq_pts * repeats * dc_offsets), dtype=np.float32)
            write_safe_attrs(h5_main, {'units': 'V', 'quantity': 'Cantilever Deflection'})
            for dset in [h5_pos_inds, h5_pos_vals, h5_spec_inds, h5_spec_vals]:
                h5_main.attrs[dset.name.split('/')[(-1)]] = dset.ref
...
def make_beps_file(rev_spec=False):
    ...
    with h5py.File(std_beps_path, mode='w') as (h5_f):
        ...
        write_safe_attrs(h5_results_grp_1, {'att_1': 'string_val', 'att_2': 1.2345, 'att_3': [1, 2, 3, 4]})
        ...
        h5_results_1_spec_inds = h5_results_grp_1.create_dataset('Spectroscopic_Indices', data=results_spec_inds, dtype=np.uint16)
        ...
        h5_results_1_spec_vals = h5_results_grp_1.create_dataset('Spectroscopic_Values', data=results_spec_vals, dtype=np.float32)
        ...
        h5_results_1_nd = h5_results_grp_1.create_dataset('n_dim_form', data=results_1_nd)
        write_string_list_as_attr(h5_results_1_nd, {'dims': ['Y', 'X', 'Cycle', 'Bias']})
        results_1_main_data = results_1_nd.reshape(num_rows * num_cols, num_cycle_pts * num_cycles)
        ...
        write_safe_attrs(h5_results_grp_2, {'att_1': 'other_string_val', 'att_2': 5.4321, 'att_3': [4, 1, 3]})
        write_string_list_as_attr(h5_results_grp_2, {'att_4': ['s', 'str_2', 'str_3']})
        results_2_nd = np.random.random(size=num_elems) + complex(0.0, 1.0) * np.random.random(size=num_elems)
        h5_results_2_nd = h5_results_grp_2.create_dataset('n_dim_form', data=results_2_nd)
        write_string_list_as_attr(h5_results_2_nd, {'dims': ['Y', 'X', 'Cycle', 'Bias']})
        results_2_main_data = results_2_nd.reshape(num_rows * num_cols, num_cycle_pts * num_cycles)
        ...
        h5_results_2_spec_inds = h5_results_grp_2.create_dataset('Spectroscopic_Indices', data=results_spec_inds, dtype=np.uint16)
        ...
        h5_results_2_spec_vals = h5_results_grp_2.create_dataset('Spectroscopic_Values', data=results_spec_vals, dtype=np.float32)
