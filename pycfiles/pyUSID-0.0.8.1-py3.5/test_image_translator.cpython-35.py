...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/tests/io/test_image_translator.py
...
class TestReadImage(TestImage):
    ...
    def test_text_complex_to_pillow(self):
        ...
        txt_kwargs = {'delimiter': ',', 
         ...
         'header': 'cat, dog, cow'}
        np.savetxt(img_path, img_data, **txt_kwargs)
        pillow_obj = read_image(img_path, as_grayscale=True, as_numpy_array=False, delimiter=',', skiprows=1)
...
class TestImageTranslator(TestImage):
    ...
    def main_translate(self, **kwargs):
        ...
        if 'bin_factor' in kwargs.keys():
            ...
            img_obj = img_obj.resize((int(vsize / bin_factor[1]), int(usize / bin_factor[0])), resample=interp_func)
