...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/tests/io/test_dtype_utils.py
# Compiled at: 2019-11-30 21:58:33
# Size of source mod 2**32: 33210 bytes
"""
...
import unittest, sys, os, numpy as np, dask.array as da, h5py
    ...
    def setUp(self):
        if not os.path.exists(file_path):
            with h5py.File(file_path, mode='w') as (h5_f):
                ...
                _ = h5_f.create_dataset('real2', data=450 * np.random.random(size=(5,
                                                                                   7,
                                                                                   6)))
                _ = h5_f.create_dataset('complex', data=np.random.random(size=num_elems) + complex(0.0, 1.0) * np.random.random(size=num_elems), dtype=np.complex64)
                h5_f.flush()
...
class TestIntegersToSlices(unittest.TestCase):
    ...
    def test_illegal_inputs(self):
        ...
        with self.assertRaises(ValueError):
            dtype_utils.integers_to_slices([-1.43, 34.6565, 45.344, complex(5.0, 6.0)])
        with self.assertRaises(ValueError):
            dtype_utils.integers_to_slices(['asdds', None, True, 45.344, complex(5.0, 6.0)])
    ...
    def test_negative(self):
        expected = [
         slice(-7, -4), slice(-2, 3), slice(14, 18), slice(22, 23), slice(27, 28), slice(29, 30)]
        inputs = np.hstack([range(item.start, item.stop) for item in expected])
        ret_val = dtype_utils.integers_to_slices(inputs)
        self.assertEqual(expected, ret_val)
...
class TestStackRealToComplex(unittest.TestCase):
    ...
    def test_single(self):
        expected = complex(4.32, 5.67)
        real_val = [np.real(expected), np.imag(expected)]
        actual = dtype_utils.stack_real_to_complex(real_val)
        self.assertTrue(np.allclose(actual, expected))

    def test_1d(self):
        expected = 5 * np.random.rand(6) + complex(0.0, 7.0) * np.random.rand(6)
        real_val = np.hstack([np.real(expected), np.imag(expected)])
        actual = dtype_utils.stack_real_to_complex(real_val)
        self.assertTrue(np.allclose(actual, expected))

    def test_2d(self):
        expected = 5 * np.random.rand(2, 8) + complex(0.0, 7.0) * np.random.rand(2, 8)
        real_val = np.hstack([np.real(expected), np.imag(expected)])
        actual = dtype_utils.stack_real_to_complex(real_val)
        self.assertTrue(np.allclose(actual, expected))

    def base_nd(self, lazy_in, lazy):
        expected = 5 * np.random.rand(2, 3, 5, 8) + complex(0.0, 7.0) * np.random.rand(2, 3, 5, 8)
...
class TestStackRealToComplexHDF5(TestDtypeUtils):
    ...
    def base_h5_legal(self, lazy):
        with h5py.File(file_path, mode='r') as (h5_f):
            h5_real = h5_f['real2']
            expected = h5_real[:, :, :3] + complex(0.0, 1.0) * h5_real[:, :, 3:]
    ...
    def test_illegal_odd_last_dim(self):
        expected = 5 * np.random.rand(2, 3, 5, 7) + complex(0.0, 7.0) * np.random.rand(2, 3, 5, 7)
...
class TestFlattenComplexToReal(unittest.TestCase):
    ...
    def test_1d_array(self):
        complex_array = 5 * np.random.rand(5) + complex(0.0, 7.0) * np.random.rand(5)
    ...
    def test_2d_array(self):
        complex_array = 5 * np.random.rand(2, 3) + complex(0.0, 7.0) * np.random.rand(2, 3)
        actual = dtype_utils.flatten_complex_to_real(complex_array)
        expected = np.hstack([np.real(complex_array), np.imag(complex_array)])
        self.assertTrue(np.allclose(actual, expected))

    def test_nd_array_numpy(self):
        complex_array = 5 * np.random.rand(2, 3, 5, 7) + complex(0.0, 7.0) * np.random.rand(2, 3, 5, 7)
        actual = dtype_utils.flatten_complex_to_real(complex_array)
...
class TestGetCompoundSubTypes(unittest.TestCase):
    ...
    def test_legal(self):
        self.assertEqual({'r': np.float32, 'g': np.uint16, 'b': np.float64}, dtype_utils.get_compound_sub_dtypes(struc_dtype))
...
class TestStackRealToCompoundHDF5(TestDtypeUtils):
    ...
    def base_nd_h5_legal(self, lazy):
        with h5py.File(file_path, mode='r') as (h5_f):
            h5_real = h5_f['real2']
            structured_array = np.zeros(shape=list(h5_real.shape)[:-1] + [h5_real.shape[(-1)] // len(struc_dtype.names)], dtype=struc_dtype)
...
class TestGetExponent(unittest.TestCase):
    ...
    def test_negative_small(self):
        expected = -7
        self.assertEqual(expected, dtype_utils.get_exponent(np.arange(5) * -10 ** expected))

    def test_positive_large(self):
        expected = 4
        self.assertEqual(expected, dtype_utils.get_exponent(np.arange(6) * 10 ** expected))

    def test_mixed_large(self):
        expected = 4
        self.assertEqual(expected, dtype_utils.get_exponent(np.random.randint(-8, high=3, size=(5,
                                                                                                5)) * 10 ** expected))
