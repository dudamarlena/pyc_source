...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/pyUSID/io/hdf_utils/model.py
# Compiled at: 2020-03-29 18:17:20
# Size of source mod 2**32: 46369 bytes
"""
...
from ..dtype_utils import contains_integers, validate_dtype, validate_single_string_arg, validate_string_args, validate_list_of_strings, lazy_load_array
...
def reshape_to_n_dims(h5_main, h5_pos=None, h5_spec=None, get_labels=False, verbose=False, sort_dims=False, lazy=False):
    """
    ...
    if h5_pos is None and h5_spec is None:
        ...
        if h5_pos is not None:
            ...
            if h5_pos.shape[0] != h5_main.shape[0]:
                raise ValueError('The size of h5_pos: {} does not match with h5_main: {}'.format(h5_pos.shape, h5_main.shape))
        if h5_spec is not None:
            ...
            if h5_spec.shape[1] != h5_main.shape[1]:
                raise ValueError('The size of h5_spec: {} does not match with h5_main: {}'.format(h5_spec.shape, h5_main.shape))
            ...
            if h5_pos is None:
                ...
                if isinstance(h5_pos, h5py.Dataset):
                    ...
                    if isinstance(h5_pos, (np.ndarray, da.core.Array)):
            ...
            if h5_spec is None:
                ...
                if isinstance(h5_spec, h5py.Dataset):
                    ...
                    if isinstance(h5_spec, (np.ndarray, da.core.Array)):
                        ds_spec = h5_spec
                        spec_labs = np.array(['Spectral Dimension {}'.format(ispec) for ispec in range(ds_spec.shape[0])])
                    else:
                    ...
                    if np.prod(pos_dims) != h5_main.shape[0]:
                        mesg = 'Product of position dimension sizes: {} = {} not matching with size of first axis of main dataset: {}. One or more dimensions are dependent dimensions and not marked as such.'.format(pos_dims, np.prod(pos_dims), h5_main.shape[0])
                        raise ValueError(mesg)
                    if np.prod(spec_dims) != h5_main.shape[1]:
                        mesg = 'Product of spectroscopic dimension sizes: {} = {} not matching with size of second axis of main dataset: {}. One or more dimensions are dependent dimensions and not marked as such.'.format(spec_dims, np.prod(spec_dims), h5_main.shape[1])
                        raise ValueError(mesg)
                    if verbose:
...
def reshape_from_n_dims(data_n_dim, h5_pos=None, h5_spec=None, verbose=False):
    ...
    if h5_pos is None:
        ...
        if isinstance(h5_pos, h5py.Dataset):
            ...
            if isinstance(h5_pos, da.core.Array):
                ds_pos = h5_pos.compute()
            else:
                if isinstance(h5_pos, np.ndarray):
                    ds_pos = h5_pos
                else:
                    raise TypeError('Position Indices must be either h5py.Dataset or None')
        if h5_spec is None:
            ...
            if isinstance(h5_spec, h5py.Dataset):
                ...
                if isinstance(h5_spec, da.core.Array):
                    ds_spec = h5_spec.compute()
                else:
                    if isinstance(h5_spec, np.ndarray):
                        ds_spec = h5_spec
                    else:
                        raise TypeError('Spectroscopic Indices must be either h5py.Dataset or None')
            if h5_spec is None and h5_pos is not None:
                ...
                if not np.all([x in data_n_dim.shape for x in pos_dims]):
                    raise ValueError('Dimension sizes in pos_dims: {} do not exist in data_n_dim shape: {}'.format(pos_dims, data_n_dim.shape))
                spec_dims = [col for col in list(data_n_dim.shape[len(pos_dims):])]
                if verbose:
                    print('data has dimensions: {}. Provided position indices had dimensions of size: {}. Spectral dimensions will built with dimensions: {}'.format(data_n_dim.shape, pos_dims, spec_dims))
                ds_spec = make_indices_matrix(spec_dims, is_position=False)
            else:
                if h5_pos is None and h5_spec is not None:
                    if verbose:
                    ...
                    if not np.all([x in data_n_dim.shape for x in spec_dims]):
                        raise ValueError('Dimension sizes in spec_dims: {} do not exist in data_n_dim shape: {}'.format(spec_dims, data_n_dim.shape))
                    pos_dims = [col for col in list(data_n_dim.shape[:data_n_dim.ndim - len(spec_dims)])]
                    if verbose:
                        print('data has dimensions: {}. Spectroscopic position indices had dimensions of size: {}. Position dimensions will built with dimensions: {}'.format(data_n_dim.shape, spec_dims, pos_dims))
                    ds_pos = make_indices_matrix(pos_dims, is_position=True)
                else:
                    if h5_spec is not None and h5_pos is not None:
                        if ds_pos.shape[0] * ds_spec.shape[1] != np.product(data_n_dim.shape):
                            raise ValueError('The product ({}) of the number of positions ({}) and spectroscopic ({}) observations is not equal to the product ({}) of the data shape ({}).'.format(ds_pos.shape[0] * ds_spec.shape[1], ds_pos.shape[0], ds_spec.shape[1], np.product(data_n_dim.shape), data_n_dim.shape))
                        if ds_pos.shape[1] + ds_spec.shape[0] != data_n_dim.ndim:
                            if (ds_pos.size == 1 or ds_spec.size == 1) and verbose:
                                print('ALL Position dimensions squeezed: {}. ALL Spectroscopic dimensions squeezed: {}.'.format(ds_pos.size == 1, ds_spec.size == 1))
                        else:
                            raise ValueError('The number of position ({}) and spectroscopic ({}) dimensions do not match with the dimensionality of the N-dimensional dataset: {}.'.format(ds_pos.shape[1], ds_spec.shape[0], data_n_dim.ndim))
                        if ds_pos.size == 1:
                            pos_sort = []
                        else:
                            pos_sort = get_sort_order(np.transpose(ds_pos))
...
def get_dimensionality(ds_index, index_sort=None):
    ...
    if len(np.unique(index_sort)) > ds_index.shape[0]:
        raise ValueError('length of index_sort ({}) should be smaller than number of dimensions in provided dataset ({}'.format(len(np.unique(index_sort)), ds_index.shape[0]))
    if set(np.arange(ds_index.shape[0])) != set(index_sort):
        raise ValueError('Sort order of dimensions ({}) not  matching  with number of dimensions ({})'.format(index_sort, ds_index.shape[0]))
...
def get_unit_values(ds_inds, ds_vals, dim_names=None, all_dim_names=None, is_spec=None, verbose=False):
    ...
    if isinstance(ds_inds, h5py.Dataset) and np.any(['incomplete_dimensions' in dset.attrs.keys() for dset in [ds_inds, ds_vals]]):
        try:
        ...
        if incomp_dims_inds is None and incomp_dims_vals is not None:
            ...
            if incomp_dims_inds is not None and incomp_dims_vals is None:
                incomp_dims = incomp_dims_inds
            else:
                if incomp_dims_vals != incomp_dims_inds:
                    raise ValueError('Provided indices ({}) and values ({}) datasets were marked with different values for incomplete_datasets.'.format(incomp_dims_inds, incomp_dims_vals))
                incomp_dims = incomp_dims_vals
            all_dim_names = get_attr(ds_inds, 'labels')
            raise ValueError('Among all dimensions: {}, These dimensions were marked as incomplete dimensions: {}. You are recommended to find unit values manually'.format(all_dim_names, incomp_dims))
        ...
        if inds_mat.shape[0] < inds_mat.shape[1]:
            is_spec = True
    elif not isinstance(is_spec, bool):
        raise TypeError('is_spec should be a boolean. Provided object is of type: {}'.format(type(is_spec)))
    if verbose:
        print('Ancillary matrices of shape: {}, hence determined to be Spectroscopic:{}'.format(inds_mat.shape, is_spec))
    if not is_spec:
        inds_mat = np.transpose(inds_mat)
        vals_mat = np.transpose(vals_mat)
    if len(all_dim_names) != inds_mat.shape[0]:
        raise ValueError('Length of dimension names list: {} not matching with shape of dataset: {}.'.format(len(all_dim_names), inds_mat.shape[0]))
    if dim_names is None:
        ...
        if verbose:
            print('Checking to make sure that the target dimension names: {} exist in the datasets attributes: {}.'.format(dim_names, all_dim_names))
    ...
    for dim_name in all_dim_names:
        ...
        if starts[0] != 0:
            raise ValueError('Spectroscopic Indices for dimension: "{}" not starting with 0. Please fix this and try again.'.format(dim_name))
...
def write_main_dataset(h5_parent_group, main_data, main_data_name, quantity, units, pos_dims, spec_dims, main_dset_attrs=None, h5_pos_inds=None, h5_pos_vals=None, h5_spec_inds=None, h5_spec_vals=None, aux_spec_prefix='Spectroscopic_', aux_pos_prefix='Position_', verbose=False, slow_to_fast=False, **kwargs):
    """
    ...
    def __check_anc_before_creation(aux_prefix, dim_type='pos'):
        ...
        if not aux_prefix.endswith('_'):
            aux_prefix += '_'
        if '-' in aux_prefix:
            warn('aux_' + dim_type + ' should not contain the "-" character. Reformatted name from:{} to {}'.format(aux_prefix, aux_prefix.replace('-', '_')))
        aux_prefix = aux_prefix.replace('-', '_')
        for dset_name in [aux_prefix + 'Indices', aux_prefix + 'Values']:
            if dset_name in h5_parent_group.keys():
                raise KeyError('Dataset named: ' + dset_name + ' already exists in group: {}. Consider passing these datasets using kwargs (if they are correct) instead of providing the pos_dims and spec_dims arguments'.format(h5_parent_group.name))
    ...
    def __ensure_anc_in_correct_file(h5_inds, h5_vals, prefix):
        ...
        if h5_inds.file != h5_parent_group.file:
            if verbose:
                print('Need to copy over ancillary datasets: {} and {} to destination group: {} which is in a different HDF5 file'.format(h5_inds, h5_vals, h5_parent_group))
    ...
    if '-' in main_data_name:
        warn('main_data_name should not contain the "-" character. Reformatted name from:{} to {}'.format(main_data_name, main_data_name.replace('-', '_')))
    ...
    if isinstance(main_data, (list, tuple)):
        ...
        if isinstance(main_data, (np.ndarray, da.core.Array)):
            if main_data.ndim != 2:
    ...
    if h5_pos_inds is not None and h5_pos_vals is not None:
        ...
        h5_pos_inds, h5_pos_vals = write_ind_val_dsets(h5_parent_group, pos_dims, is_spectral=False, verbose=verbose, slow_to_fast=slow_to_fast, base_name=aux_pos_prefix)
    if verbose:
    ...
    if h5_spec_inds is not None and h5_spec_vals is not None:
        ...
        if verbose:
            print('The shapes of the provided h5 position indices and values are OK')
        h5_spec_inds, h5_spec_vals = __ensure_anc_in_correct_file(h5_spec_inds, h5_spec_vals, 'Spectroscopic')
    else:
        ...
        h5_spec_inds, h5_spec_vals = write_ind_val_dsets(h5_parent_group, spec_dims, is_spectral=True, verbose=verbose, slow_to_fast=slow_to_fast, base_name=aux_spec_prefix)
    if verbose:
        print('Created Spectroscopic datasets')
    if h5_parent_group.file.driver == 'mpio' and kwargs.pop('compression', None) is not None:
    ...
    if isinstance(main_data, np.ndarray):
        ...
        if isinstance(main_data, da.core.Array):
            _ = kwargs.pop('dtype', None)
            h5_main = h5_parent_group.create_dataset(main_data_name, shape=main_data.shape, dtype=main_data.dtype, **kwargs)
            if verbose:
                print('Created empty dataset: {} for writing Dask dataset: {}'.format(h5_main, main_data))
                print('Dask array will be written to HDF5 dataset: "{}" in file: "{}"'.format(h5_main.name, h5_main.file.filename))
...
def map_grid_to_cartesian(h5_main, grid_shape, mode='histogram', **kwargs):
    ...
    def interpolate(points, values, grid_shape, method):
        grid_shape = list(map((complex(0.0, 1.0)).__mul__, grid_shape))
    ...
    if mode == 'histogram':
        histogram_weighted, _, _ = np.histogram2d(*ds_pos_vals.T, weights=ds_main)
