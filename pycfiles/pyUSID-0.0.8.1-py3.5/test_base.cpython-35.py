...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/tests/io/hdf_utils/test_base.py
# Compiled at: 2020-03-26 17:52:41
# Size of source mod 2**32: 36682 bytes
"""
...
import unittest, os, sys, h5py, numpy as np
...
class TestGetH5ObjRefs(TestHDFUtilsBase):
    ...
    def test_many(self):
        with h5py.File(data_utils.std_beps_path, mode='r') as (h5_f):
            ...
            chosen_objs = [h5_f['/Raw_Measurement/Position_Indices'],
             ...
             h5_f['/Raw_Measurement/source_main-Fitter_000/Spectroscopic_Indices']]
            target_ref_names = [
             'Position_Indices', 'source_main-Fitter_000', 'Spectroscopic_Indices']
    ...
    def test_non_string_names(self):
        with h5py.File(data_utils.std_beps_path, mode='r') as (h5_f):
            h5_obj_refs = [
             h5_f, 4.123, np.arange(6),
             h5_f['/Raw_Measurement/Position_Indices']]
            target_ref_names = [
             'Position_Indices', np.arange(6), 4.123]
            with self.assertRaises(TypeError):
...
class TestPrintTreeNoMain(unittest.TestCase):
    ...
    def test_not_a_group(self):
        ...
        with h5py.File(file_path, mode='w') as (h5_f):
            ...
            with self.assertRaises(TypeError):
                hdf_utils.print_tree(dset, rel_paths=False, main_dsets_only=False)
    ...
    def test_single_level_tree(self):
        ...
        with h5py.File(file_path, mode='w') as (h5_f):
            ...
            obj_name = 'B_Group'
            expected.append('├ ' + obj_name)
            expected.append('  ' + len(obj_name) * '-')
            _ = h5_f.create_group(obj_name)
            with data_utils.capture_stdout() as (get_value):
                hdf_utils.print_tree(h5_f, rel_paths=False, main_dsets_only=False)
                actual = get_value()
        expected = '\n'.join(expected) + '\n'
    ...
    def test_single_level_rel_paths(self):
        ...
        with h5py.File(file_path, mode='w') as (h5_f):
            ...
            _ = h5_f.create_group(obj_name)
            with data_utils.capture_stdout() as (get_value):
                hdf_utils.print_tree(h5_f, rel_paths=True, main_dsets_only=False)
                actual = get_value()
        expected = '\n'.join(expected) + '\n'
        self.assertEqual(expected, actual)
        os.remove(file_path)
    ...
    def test_multi_level_tree_main_dsets_only(self):
        ...
        with h5py.File(file_path, mode='w') as (h5_f):
            ...
            _ = h5_f.create_dataset(obj_name, data=[1, 2, 3])
            with data_utils.capture_stdout() as (get_value):
                hdf_utils.print_tree(h5_f, rel_paths=False, main_dsets_only=True)
                actual = get_value()
        expected = '\n'.join(expected) + '\n'
        self.assertEqual(expected, actual)
        os.remove(file_path)
    ...
    def test_multi_level_tree_grp_a(self):
        ...
        with h5py.File(file_path, mode='w') as (h5_f):
            ...
            _ = h5_f.create_dataset(obj_name, data=[1, 2, 3])
            with data_utils.capture_stdout() as (get_value):
                hdf_utils.print_tree(grp_1, rel_paths=False, main_dsets_only=False)
                actual = get_value()
        expected = '\n'.join(expected) + '\n'
        self.assertEqual(expected, actual)
        os.remove(file_path)
    ...
    def test_multi_level_tree_grp_b(self):
        ...
        with h5py.File(file_path, mode='w') as (h5_f):
            ...
            _ = h5_f.create_dataset(obj_name, data=[1, 2, 3])
            with data_utils.capture_stdout() as (get_value):
                hdf_utils.print_tree(grp_2, rel_paths=False, main_dsets_only=False)
                actual = get_value()
        expected = '\n'.join(expected) + '\n'
        self.assertEqual(expected, actual)
        os.remove(file_path)
    ...
    def test_multi_level_rel_paths_grp_b(self):
        ...
        with h5py.File(file_path, mode='w') as (h5_f):
            ...
            _ = h5_f.create_dataset(obj_name, data=[1, 2, 3])
            with data_utils.capture_stdout() as (get_value):
                hdf_utils.print_tree(grp_2, rel_paths=True, main_dsets_only=False)
                actual = get_value()
        expected = '\n'.join(expected) + '\n'
        self.assertEqual(expected, actual)
        os.remove(file_path)
