...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/pyUSID/io/dtype_utils.py
...
__all__ = [
 'flatten_complex_to_real', 'get_compound_sub_dtypes', 'flatten_compound_to_real', 'check_dtype',
...
def lazy_load_array(dataset):
    ...
    if not isinstance(dataset, (h5py.Dataset, np.ndarray)):
        raise TypeError('Expected one of h5py.Dataset, dask.array.core.Array, or numpy.ndarrayobjects. Provided object was of type: {}'.format(type(dataset)))
...
def flatten_compound_to_real(dataset, lazy=False):
    ...
    if isinstance(dataset, (np.ndarray, da.core.Array)):
        if isinstance(dataset, da.core.Array):
            lazy = True
        xp = np
        if lazy:
...
def check_dtype(h5_dset):
    ...
    if is_complex_dtype(h5_dset.dtype):
        ...
        if len(h5_dset.dtype) > 0:
            ...
            func = new_dtype
    return (
     func, is_complex, is_compound, n_features, type_mult)
...
def stack_real_to_complex(ds_real, lazy=False):
    ...
    return ds_real[..., :half_point] + complex(0.0, 1.0) * ds_real[..., half_point:]
...
def validate_dtype(dtype):
    ...
    if isinstance(dtype, (h5py.Datatype, np.dtype)):
        ...
        if isinstance(np.dtype(dtype), np.dtype):
            pass
        else:
...
def integers_to_slices(int_array):
    ...
    def integers_to_consecutive_sections(integer_array):
        ...
        for key, group in groupby(enumerate(integer_array), lambda t: t[1] - t[0]):
            group = list(group)
