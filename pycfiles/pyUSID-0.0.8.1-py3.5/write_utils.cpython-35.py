...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/pyUSID/io/write_utils.py
# Compiled at: 2019-12-17 21:54:37
...
__all__ = [
 'clean_string_att', 'get_aux_dset_slicing', 'make_indices_matrix', 'INDICES_DTYPE', 'VALUES_DTYPE', 'get_slope',
...
class DimType(Enum):
    ...
    def __lt__(self, other):
        self._DimType__check_other_type(other)
    ...
    def __eq__(self, other):
        if isinstance(other, Dimension):
            ...
            if len(self.values) != len(other.values):
                return False
            return np.allclose(self.values, other.values) or False
        return True
...
def validate_dimensions(dimensions, dim_type='Position'):
    ...
    if isinstance(dimensions, Dimension):
        dimensions = [
         dimensions]
    if isinstance(dimensions, np.ndarray) and dimensions.ndim > 1:
...
def get_aux_dset_slicing(dim_names, last_ind=None, is_spectroscopic=False):
    ...
    for spat_ind, curr_dim_name in enumerate(dim_names):
        val = (
         slice(last_ind), slice(spat_ind, spat_ind + 1))
        if is_spectroscopic:
            val = val[::-1]
    ...
    try:
        if isinstance(att_val, Iterable):
            if type(att_val) in [unicode, str]:
                return att_val
            if np.any([type(x) in [str, unicode, bytes, np.str_] for x in att_val]):
                pass
...
def build_ind_val_matrices(unit_values, is_spectral=True):
    ...
    if not is_spectral:
        ...
        ind_mat = ind_mat.T
    return (
     INDICES_DTYPE(ind_mat), VALUES_DTYPE(val_mat))
...
def get_slope(values, tol=0.001):
    """
