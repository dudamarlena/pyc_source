...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/tests/processing/test_process.py
# Compiled at: 2020-04-03 17:16:43
...
def _create_results_grp_dsets(h5_main, process_name, parms_dict, h5_parent_group=None):
    h5_results_grp = usid.hdf_utils.create_results_group(h5_main, process_name, h5_parent_group=h5_parent_group)
    usid.hdf_utils.write_simple_attrs(h5_results_grp, parms_dict)
    spec_dims = usid.write_utils.Dimension('Empty', 'a. u.', 1)
    h5_results = usid.hdf_utils.write_main_dataset(h5_results_grp, (h5_main.shape[0], 1), 'Results', 'quantity', 'units', None, spec_dims, dtype=np.float32, h5_pos_inds=h5_main.h5_pos_inds, h5_pos_vals=h5_main.h5_pos_vals)
    return (
     h5_results_grp, h5_results)
...
class NoMapFunc(usid.Process):
    ...
    def __init__(self, h5_main, **kwargs):
        ...
        super(NoMapFunc, self).__init__(h5_main, 'Mean_Val', parms_dict=parms_dict, **kwargs)
...
class AvgSpecUltraBasic(usid.Process):
    ...
    def __init__(self, h5_main, *args, **kwargs):
        parms_dict = {'parm_1': 1, 'parm_2': [1, 2, 3]}
        super(AvgSpecUltraBasic, self).__init__(h5_main, 'Mean_Val', parms_dict=parms_dict, *args, **kwargs)
...
class TestInvalidInitialization(unittest.TestCase):
    ...
    def test_invalid_process_name(self):
        ...
        class TempProc(usid.Process):
            ...
            def __init__(self, h5_main, *args, **kwargs):
                parms_dict = {'parm_1': 1, 'parm_2': [1, 2, 3]}
                super(TempProc, self).__init__(h5_main, {'a': 1}, parms_dict=parms_dict, *args, **kwargs)
    ...
    def test_invalid_parms_dict(self):
        ...
        class TempProc(usid.Process):
            ...
            def __init__(self, h5_main, *args, **kwargs):
                super(TempProc, self).__init__(h5_main, 'Proc', parms_dict='Parms', *args, **kwargs)
    ...
    def test_none_parms_dict(self):
        ...
        class TempProc(usid.Process):
            ...
            def __init__(self, h5_main, *args, **kwargs):
                super(TempProc, self).__init__(h5_main, 'Proc', parms_dict=None, *args, **kwargs)
...
class TestCoreProcessNoTest(unittest.TestCase):
    ...
    def setUp(self, proc_class=AvgSpecUltraBasic, **proc_kwargs):
        ...
        self.h5_main = self.h5_file['Raw_Measurement/source_main']
        self.h5_main = usid.USIDataset(self.h5_main)
        self.exp_result = np.expand_dims(np.mean(self.h5_main[()], axis=1), axis=1)
...
class TestCoreProcessWTest(TestCoreProcessNoTest):
    ...
    def setUp(self, proc_class=AvgSpecUltraBasicWTest, **proc_kwargs):
        super(TestCoreProcessWTest, self).setUp(proc_class=proc_class, **proc_kwargs)
...
class TestWriteResultsToNewH5File(TestCoreProcessWTest):
    ...
    def setUp(self, proc_class=AvgSpecUltraBasicWTest, **proc_kwargs):
        ...
        super(TestWriteResultsToNewH5File, self).setUp(proc_class=proc_class, h5_target_group=self.h5_f_new, **proc_kwargs)
...
class TestCoreProcessWExistingResults(unittest.TestCase):
    ...
    def __create_fake_result(self, percent_complete=100, parms_dict=None, status_dset=True, status_attr=False, verbose=False, h5_parent_group=None):
        if parms_dict is None:
            parms_dict = {'parm_1': 1, 'parm_2': [1, 2, 3]}
        if verbose:
            print('Using parms_dict: {}'.format(parms_dict))
        results_grp, h5_results_dset = _create_results_grp_dsets(self.h5_main, 'Mean_Val', parms_dict, h5_parent_group=h5_parent_group)
        exp_result = np.expand_dims(np.random.rand(h5_results_dset.shape[0]), axis=1)
        ...
        if verbose:
            print('Positions up to {} of {} will be marked as complete.'.format(complete_index, self.h5_main.shape[0]))
    ...
    def setUp(self, proc_class=AvgSpecUltraBasicWGetPrevResults, percent_complete=100, parms_dict=None, status_dset=True, status_attr=False, verbose=False, h5_target_group=None):
        delete_existing_file(data_utils.std_beps_path)
        data_utils.make_beps_file()
        self.h5_file = h5py.File(data_utils.std_beps_path, mode='r+')
        self.h5_main = self.h5_file['Raw_Measurement/source_main']
        ...
        if any([isinstance(item, (list, tuple)) for item in [percent_complete,
         ...
         parms_dict]]):
            ...
            self.h5_results = []
            self.exp_result = []
            for this_per, this_parms, has_status_dset, has_status_attr in zip(percent_complete, parms_dict, status_dset, status_attr):
                ret_vals = self._TestCoreProcessWExistingResults__create_fake_result(percent_complete=this_per, parms_dict=this_parms, status_dset=has_status_dset, status_attr=has_status_attr, h5_parent_group=h5_target_group, verbose=verbose)
                self.fake_results_grp.append(ret_vals[0])
                self.h5_results.append(ret_vals[1])
                self.exp_result.append(ret_vals[2])

        else:
            ret_vals = self._TestCoreProcessWExistingResults__create_fake_result(percent_complete=percent_complete, parms_dict=parms_dict, status_dset=status_dset, status_attr=status_attr, h5_parent_group=h5_target_group, verbose=verbose)
            self.fake_results_grp, self.h5_results, self.exp_result = ret_vals
        self.proc = AvgSpecUltraBasicWGetPrevResults(self.h5_main, h5_target_group=h5_target_group, verbose=verbose)
...
class TestProcWLegacyResultsComplete(TestCoreProcessWExistingResults):
    ...
    def setUp(self, percent_complete=100):
        super(TestProcWLegacyResultsComplete, self).setUp(percent_complete=percent_complete, status_dset=False, status_attr=True)

    def test_compute(self):
        super(TestProcWLegacyResultsComplete, self).test_compute()
...
class TestCoreProcessWDuplicateResultsOverride(TestCoreProcessWExistingResults):
    ...
    def test_compute(self, override=True):
        ...
        self.assertEqual(h5_grp, self.proc.h5_results_grp)
        results_dset = h5_grp['Results']
        self.assertTrue(np.allclose(results_dset[()], np.expand_dims(np.mean(self.h5_main[()], axis=1), axis=1)))
        self.assertTrue('completed_positions' in list(h5_grp.keys()))
        h5_status_dset = h5_grp['completed_positions']
        self.assertIsInstance(h5_status_dset, h5py.Dataset)
        self.assertEqual(h5_status_dset.shape, (self.h5_main.shape[0],))
...
class TestCoreProcessWExistResultsDiffParms(TestCoreProcessWDuplicateResultsOverride):
    ...
    def setUp(self, proc_class=AvgSpecUltraBasicWGetPrevResults, percent_complete=100, parms_dict={'parm_1': 'Decoy', 'parm_2': 14.56}):
        super(TestCoreProcessWExistResultsDiffParms, self).setUp(percent_complete=percent_complete, parms_dict=parms_dict)
    ...
    def test_compute(self, override=False):
        super(TestCoreProcessWExistResultsDiffParms, self).test_compute(override=override)


class TestCoreProcessWExistingPartResults(TestCoreProcessWExistingResults):

    def setUp(self, proc_class=AvgSpecUltraBasicWGetPrevResults, percent_complete=50):
        super(TestCoreProcessWExistingPartResults, self).setUp(percent_complete=percent_complete)


class TestProcWLegacyResultsPartial(TestCoreProcessWExistingResults):

    def setUp(self, percent_complete=50):
        super(TestProcWLegacyResultsPartial, self).setUp(percent_complete=percent_complete, status_dset=False, status_attr=True)

    def test_compute(self):
        super(TestProcWLegacyResultsPartial, self).test_compute()
        items_in_grp = list(self.proc.h5_results_grp.keys())
        self.assertEqual(len(items_in_grp), 4)
        self.assertTrue(self.proc._status_dset_name in items_in_grp)
...
class TestProcWoStatus(TestCoreProcessWDuplicateResultsOverride):
    ...
    def setUp(self, percent_complete=50):
        super(TestProcWoStatus, self).setUp(percent_complete=percent_complete, status_dset=False, status_attr=False)
...
class TestProcReturnCompletedNotPartial(TestCoreProcessWExistingResults):
    ...
    def setUp(self):
        super(TestProcReturnCompletedNotPartial, self).setUp(percent_complete=[100, 50], parms_dict=[
         None, None], status_dset=[
         True, False], status_attr=[
         False, True], verbose=False)

    def test_compute(self):
        self.assertEqual(len(self.proc.duplicate_h5_groups), 1)
        self.assertEqual(self.proc.duplicate_h5_groups[0], self.fake_results_grp[0])
        self.assertEqual(len(self.proc.partial_h5_groups), 1)
        self.assertEqual(self.proc.partial_h5_groups[0], self.fake_results_grp[1])
        h5_results_grp = self.proc.compute(override=False)
...
class TestProcLastPartialResult(TestCoreProcessWExistingResults):
    ...
    def setUp(self, **kwargs):
        super(TestProcLastPartialResult, self).setUp(percent_complete=[75, 50], parms_dict=[
         None, None], status_dset=[
         True, False], status_attr=[
         False, True], verbose=False, **kwargs)
...
class TestProcessWExistingResultsDiffFile(TestProcLastPartialResult):
    ...
    def setUp(self):
        self.results_h5_file_path = 'sep_results.h5'
        self.h5_f_new = h5py.File(self.results_h5_file_path, mode='w')
        super(TestProcessWExistingResultsDiffFile, self).setUp(h5_target_group=self.h5_f_new)
