...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/pyUSID/io/image.py
# Compiled at: 2019-05-15 16:00:45
# Size of source mod 2**32: 9008 bytes
"""
...
class ImageTranslator(ArrayTranslator):
    ...
    def _parse_file_path(image_path, h5_path=None):
        ...
        if os.path.exists(os.path.abspath(h5_path)):
            raise FileExistsError('ImageTranslator: There is already a valid (output HDF5) file at:\n{}\nPlease consider providing an alternate path or deleting the specified file'.format(h5_path))
        return (image_path, h5_path)

    def translate(self, image_path, h5_path=None, bin_factor=None, interp_func=Image.BICUBIC, normalize=False, **image_args):
        """
        ...
        if bin_factor is not None:
            ...
            if interp_func not in [Image.NEAREST, Image.BILINEAR, Image.BICUBIC, Image.LANCZOS]:
                raise ValueError("'interp_func' argument for ImageTranslator.translate must be one of PIL.Image.NEAREST, PIL.Image.BILINEAR, PIL.Image.BICUBIC, PIL.Image.LANCZOS")
        ...
        pos_dims = [
         Dimension('Y', 'a.u.', np.arange(usize)), Dimension('X', 'a.u.', np.arange(vsize))]
        spec_dims = Dimension('arb', 'a.u.', 1)
        image = image.transpose()
        h5_path = super(ImageTranslator, self).translate(h5_path, 'Raw_Data', image.reshape((-1,
                                                                                             1)), 'Intensity', 'a.u.', pos_dims, spec_dims, translator_name='ImageTranslator', parm_dict=image_parms)
