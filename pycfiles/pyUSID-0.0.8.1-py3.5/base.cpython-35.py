...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/pyUSID/io/hdf_utils/base.py
# Compiled at: 2020-03-26 17:43:38
...
def get_auxiliary_datasets(h5_object, aux_dset_name=None):
    ...
    try:
        ...
        for curr_name in aux_dset_name:
            ...
            if isinstance(h5_ref, h5py.Reference) and isinstance(h5_file[h5_ref], h5py.Dataset) and not isinstance(h5_ref, h5py.RegionReference):
...
def get_attr(h5_object, attr_name):
    ...
    elif type(att_val) == np.ndarray and sys.version_info.major == 3 and att_val.dtype.type in [np.bytes_, np.object_]:
    ...
    for target_name in obj_names:
        for h5_object in h5_refs:
            if not isinstance(h5_object, (h5py.File, h5py.Group, h5py.Dataset)):
                pass
            elif h5_object.name.split('/')[(-1)] == target_name:
                try:
...
def validate_h5_objs_in_same_h5_file(h5_src, h5_other):
    ...
    if not isinstance(h5_src, (h5py.Dataset, h5py.File, h5py.Group)):
        raise TypeError('h5_src should either be a h5py Dataset, File, or Group')
    if not isinstance(h5_other, (h5py.Dataset, h5py.File, h5py.Group)):
        raise TypeError('h5_other should either be a h5py Dataset, File, or Group')
    if h5_src.file != h5_other.file:
        raise ValueError('Cannot link h5 objects across files. {} is present in file: {}, while {} is in file :{}'.format(h5_src.name, h5_src.file, h5_other.name, h5_other.file))
...
def is_editable_h5(h5_obj):
    ...
    if not isinstance(h5_obj, (h5py.File, h5py.Group, h5py.Dataset)):
        raise TypeError('h5_obj should be a h5py File, Group or Dataset object but is instead of type {}t'.format(type(h5_obj)))
    try:
