...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/tests/io/test_reg_ref.py
...
class TestRegRef(unittest.TestCase):
    ...
    def __write_main_reg_refs(h5_dset, attrs):
        ...
        TestRegRef._TestRegRef__write_string_list_as_attr(h5_dset, {'labels': list(attrs.keys())})
    ...
    def test_simple_region_ref_copy(self):
        ...
        with h5py.File(file_path, mode='w') as (h5_f):
            ...
            self.assertTrue(np.all([x in list(attrs['labels'].keys()) for x in get_attr(h5_orig_dset, 'labels')]))
            expected_data = [
             data[::2], data[1::2]]
            ...
            written_data = [
             h5_new_dset[h5_new_dset.attrs['even_rows']], h5_new_dset[h5_new_dset.attrs['odd_rows']]]
            for exp, act in zip(expected_data, written_data):
                self.assertTrue(np.allclose(exp, act))
