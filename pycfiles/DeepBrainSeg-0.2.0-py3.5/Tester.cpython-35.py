...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/DeepBrainSeg/tumor/Tester.py
# Compiled at: 2019-11-11 08:44:10
# Size of source mod 2**32: 21122 bytes
import torch, SimpleITK as sitk, numpy as np, nibabel as nib
...
class tumorSeg:
    ...
    def __init__(self, quick=False, ants_path=bin_path):
        ...
        self.ABLnclasses = 3
        self.ABLnet = FCDenseNet103(n_classes=self.ABLnclasses)
        saved_parms = torch.load(ckpt_ABL, map_location=map_location)
        self.ABLnet.load_state_dict(saved_parms['state_dict'])
    ...
    def get_localization(self, t1_v, t1c_v, t2_v, flair_v, brain_mask):
        ...
        for slices in tqdm(range(flair_v.shape[2])):
            ...
            logits = self.ABLnet(transformed_array).detach().cpu().numpy()
    ...
    def inner_class_classification_with_logits_NCube(self, t1, t1ce, t2, flair, brain_mask, mask, N=64):
        """
        ...
        x_min, x_max, y_min, y_max, z_min, z_max = (
         x_min, min(shape[0] - N, x_max), y_min, min(shape[1] - N, y_max), z_min, min(shape[2] - N, z_max))
        with torch.no_grad():
    ...
    def inner_class_classification_with_logits_DualPath(self, t1, t1ce, t2, flair, brain_mask, mask=None, prediction_size=9):
        """
                output of BNet3D 
        ...
        for x in tqdm(range(x_min, x_max - prediction_size, prediction_size)):
            for y in range(y_min, y_max - prediction_size, prediction_size):
                for z in range(z_min, z_max - prediction_size, prediction_size):
                    ...
                    high[(0, 0)], high[(0, 1)], high[(0, 2)], high[(0, 3)] = (
                     high[(0, 0)] + flair[(0, 0, 0)], high[(0, 1)] + t2[(0, 0, 0)], high[(0, 2)] + t1[(0, 0, 0)], high[(0, 2)] + t1ce[(0, 0, 0)])
    ...
    def get_segmentation(self, t1_path, t2_path, t1ce_path, flair_path, save_path=None):
        """
    ...
    def get_segmentation_brats(self, path, save=True):
        """
