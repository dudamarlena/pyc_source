...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/DeepBrainSeg/helpers/helper.py
# Compiled at: 2019-11-11 08:44:10
# Size of source mod 2**32: 6459 bytes
from scipy.ndimage import uniform_filter, maximum_filter
import pydensecrf.densecrf as dcrf
from pydensecrf.utils import unary_from_softmax, create_pairwise_bilateral, create_pairwise_gaussian
import SimpleITK as sitk, numpy as np, nibabel as nib
...
def get_dice_score(prediction, ground_truth):
    masks = (
     get_whole_tumor, get_tumor_core, get_enhancing_tumor)
    p = np.uint8(prediction)
    gt = np.uint8(ground_truth)
    wt, tc, et = [2 * np.sum(func(p) * func(gt)) / (np.sum(func(p)) + np.sum(func(gt)) + 1e-06) for func in masks]
    return (wt, tc, et)
...
def combine_logits_GM(x):
    ...
    return final ** (1.0 / x.shape[0]) / 10.0
