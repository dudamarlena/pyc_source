...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/composer/templatetags/composer_tags.py
...
def composer(parser, token):
    try:
        ...
        raise template.TemplateSyntaxError('composer tag requires argument slot_name')
...
class ComposerNode(template.Node):
    ...
    def render(self, context):
        ...
        rows = slot.rows
        if rows:
            return render_to_string('composer/inclusion_tags/composer.html', context={'rows': rows}, request=request)
        return render_to_string('composer/inclusion_tags/%s.html' % self.slot_name, context={'object': slot}, request=request)
    ...
    def render(self, context):
        ...
        if tile.view_name:
            ...
            with context.push():
                ...
                try:
                    return render_to_string('composer/inclusion_tags/%s.html' % tile.style or 'tile', context.flatten())
