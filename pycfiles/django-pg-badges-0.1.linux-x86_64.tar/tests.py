...
# [GCC 8.4.0]
# Embedded file name: /home/yohann/Dev/django-pg-badges/demo/venv/lib/python2.7/site-packages/badges/tests.py
# Compiled at: 2016-01-24 18:12:17
import datetime
...
from badges.management.commands.create_badge_triggers import create_trigger, drop_existing_triggers
...
class CollectorTest(TriggerMixin, TestCase):
    ...
    def test_trigger_create(self):
        ...
        users = User.objects.all()[:10]
        for user in users:
            fixture = AutoFixture(Sketch, field_values={'user': user})
            fixture.create(5)
...
class StarTest(TriggerMixin, TestCase):
    ...
    def test_trigger_create(self):
        ...
        users = User.objects.all()[:10]
        for user in users:
            fixture = AutoFixture(Sketch, field_values={'user': user, 'hit_views': 0})
