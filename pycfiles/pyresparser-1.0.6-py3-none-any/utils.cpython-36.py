...
# [GCC 8.4.0]
# Embedded file name: /home/omkarpathak27/Documents/GITS/py-resume-parser/pyresparser/utils.py
# Compiled at: 2019-11-30 21:44:54
# Size of source mod 2**32: 15102 bytes
import io, os, re, nltk, pandas as pd, docx2txt
...
def extract_mobile_number(text, custom_regex=None):
    ...
    """
    if not custom_regex:
        mob_num_regex = '(\\d{3}[-\\.\\s]??\\d{3}[-\\.\\s]??\\d{4}|\\(\\d{3}\\)\n                        [-\\.\\s]*\\d{3}[-\\.\\s]??\\d{4}|\\d{3}[-\\.\\s]??\\d{4})'
...
def extract_experience(resume_text):
    ...
    filtered_sentence = [w for w in word_tokens if w not in stop_words if wordnet_lemmatizer.lemmatize(w) not in stop_words]
    ...
    x = [x[x.lower().index('experience') + 10:] for i, x in enumerate(test) if x if 'experience' in x.lower()]
    return x