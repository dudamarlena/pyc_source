...
def is_object(obj):
    ...
    False
    """
    return isinstance(obj, object) and type(obj) is not type and type(obj) is not types.FunctionType
...
def is_collection_subclass(obj):
    ...
    *obj* must be a subclass and not the actual builtin, such
    ...
    True
    """
    return (issubclass(obj.__class__, COLLECTIONS) or is_list_like(obj)) and not is_collection(obj)
...
def is_function(obj):
    ...
    return module == '__builtin__' and name in ('function', 'builtin_function_or_method',
...
def get_public_variables(t):
    ...
    return [ i[0] for i in inspect.getmembers(t, lambda i: not inspect.isroutine(i)) if not i[0].startswith('__')
           ]