...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.5/site-packages/bankpassweb/apiresults.py
# Compiled at: 2007-12-03 16:57:10
...
class Result(DataTree):
    ...
    def add_data(self, el):
        ...
        if isinstance(node, Request):
            ...
            self.op_list = node
            self.ops = []
        elif isinstance(node, AuthListTag) or isinstance(node, OrderStatus):
            self.auth_list = node
            self.auths = []
        else:
    ...
    def check_mac(self):
        """
                ...
                """
        self.engine.mac_ok('%s&%s' % (str(self.xml.Timestamp), str(self.xml.Esito)), mac=str(self.xml.MAC))
...
class Authorization(DataTree):
    ...
    macfields = ('Tautor', 'IDtrans', 'Circuito', 'NumOrdine', 'ImportoTrans', 'ImportoAutor',
                 'Valuta', 'ImportoContab', 'EsitoTrans', 'Timestamp', 'NumAut',
                 'AcqBIN', 'CodiceEsercente', 'Stato')
...
class AccountingOp(DataTree):
    ...
    macfields = ('IDtrans', 'TimestampRic', 'TimestampElab', 'TipoOp', 'Importo', 'Esito',
                 'Stato')
...
class FormerRequestType(DataTree):
    ...
    @classmethod
    def parse(cls, xml, **kwargs):
        from apirequests import Authorize, CloseAuth, WriteOff, Account, CancelAccounting, Split
        return {'01': Authorize, '02': CloseAuth, '03': WriteOff, '04': Account, 
           '05': CancelAccounting, '06': Split}[str(xml)]
