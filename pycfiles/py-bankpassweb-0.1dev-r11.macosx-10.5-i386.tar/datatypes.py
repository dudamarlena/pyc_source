...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.5/site-packages/bankpassweb/datatypes.py
# Compiled at: 2008-01-28 09:44:47
...
def general_parser(cls, value, attr=None, builder='parse_init', *args, **kwargs):
    """
    ...
    while len(class_queue) > 0:
        ...
        if match == value or isinstance(match, tuple) and value in match:
            if builder:
    ...
    raise BPWParmException('Missing default option for class %s, and cannot find subclass with value %s' % (
     cls, value))
    return
...
class DataTree(Parsable):
    ...
    def __init__(self, xml, engine):
        ...
        if hasattr(self, 'macfields'):
            self.engine.mac_ok([ getattr(self.xml, field) for field in self.macfields
    ...
    def _debugging_dict(self):
        ...
        try:
            ...
            if not isinstance(self, Request):
                raise TypeError('xml attribute missing in %s instance' % self.__class__.__name__)
