...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/dccs/maya/core/render.py
# Compiled at: 2020-04-11 22:28:15
# Size of source mod 2**32: 6317 bytes
"""
...
def get_render_globals_attribute(attribute_name, default_value=None):
    ...
    if not tp.Dcc.attribute_exists(render_globals_node, attribute_name):
        LOGGER.warning('Attribute "{}" does not exists in RenderGlobals node "{}"!'.format(attribute_name, render_globals_node))
...
def set_render_globals_attribute(attribute_name, attribute_value):
    ...
    render_globals_node = get_render_globals_node_name()
    if not tp.Dcc.attribute_exists(render_globals_node, attribute_name):
        LOGGER.warning('Attribute "{}" does not exists in RenderGlobasls node "{}"!'.format(attribute_name, render_globals_node))
        return False
    try:
        return tp.Dcc.set_attribute_value(render_globals_node, attribute_name, attribute_value)
    except Exception as exc:
        LOGGER.error('Was impossible to set attribute "{}" in RenderGlobals node "{}" with value "{}" | "{}"'.format(attribute_name, render_globals_node, attribute_value, exc))
        return False
...
def get_default_resolution_attribute(attribute_name, default_value=None):
    ...
    if not tp.Dcc.attribute_exists(default_resolution_node, attribute_name):
        LOGGER.warning('Attribute "{}" does not exists in DefaultResolution node "{}"!'.format(attribute_name, default_resolution_node))
        return default_value
    else:
...
def set_default_resolution_attribute(attribute_name, attribute_value):
    ...
    default_resolution_node = get_default_resolution_node_name()
    if not tp.Dcc.attribute_exists(default_resolution_node, attribute_name):
        LOGGER.warning('Attribute "{}" does not exists in DefafultResolution node "{}"!'.format(attribute_name, default_resolution_node))
        return False
    try:
        return tp.Dcc.set_attribute_value(default_resolution_node, attribute_name, attribute_value)
    except Exception as exc:
        LOGGER.error('Was impossible to set attribute "{}" in DefafultResolution node "{}" with value "{}" | "{}"'.format(attribute_name, default_resolution_node, attribute_value, exc))
        return False
...
def get_default_resolution_size_units():
    ...
    """
    return [
     'pixels', 'inches', 'cm', 'mm', 'points', 'picas']
