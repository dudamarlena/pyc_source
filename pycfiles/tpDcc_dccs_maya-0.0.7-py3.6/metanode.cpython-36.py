...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/dccs/maya/meta/metanode.py
# Compiled at: 2020-04-11 22:28:15
# Size of source mod 2**32: 89971 bytes
"""
MetaNode class implementation for Maya
"""
from __future__ import print_function, division, absolute_import
import sys, json, time, types, logging, traceback
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            try:
                locked = False
                meta_node = args[0]
                LOGGER.debug('NodeLockManager > fn : {0} : MetaNode / self: {1}'.format(fn.__name__, meta_node.meta_node))
                if meta_node.meta_node:
                    if meta_node._lockState:
                        locked = True
...
class MetaNode(object):
        ...
        if args:
            ...
            if meta_class:
                ...
                if meta_class in metadatamanager.METANODE_CLASSES_REGISTER:
                    ...
                    try:
                        LOGGER.debug('### Instantiating existing MetaClass : {0} >> {1} ###'.format(meta_class, registered_meta_class))
    ...
    def __init__(self, node=None, name=None, name_args=None, name_kwargs=None, node_type='network', *args, **kwargs):
        LOGGER.debug('Meta => __init__ => main args :: node={0}, name={1}, node_type={2}'.format(node, name, node_type))
        auto_rename = kwargs.get('auto_rename', True)
        name_lib = kwargs.get('name_lib', None)
        if name_args is None:
            name_args = list()
        elif name_kwargs is None:
            name_kwargs = dict()
        else:
            if auto_rename:
                if name_lib:
                    if name_args or name_kwargs:
                        ...
                        if node is not None:
                            if maya.cmds.objExists(node):
                                LOGGER.debug('|MayaMetaNode| >> Node {} already exists in scene!'.format(node))
                                created_state = False
                        if name is not None:
                            if maya.cmds.objExists(name):
                                LOGGER.debug('|MayaMetaNode| >> Node {} already exists in scene!'.format(name))
                                created_state = False
                        component_mode = False
                        component = False
                        if node is not None:
                            if metautils.MetaAttributeValidator.is_component(node):
                                component_mode = True
                    ...
                    object.__setattr__(self, 'logger', kwargs.pop('logger', LOGGER))
                    name = node or name or self.__class__.__name__
                else:
                    if not node_type == 'network':
                        if node_type not in metadatamanager.METANODE_TYPES_REGISTER:
        ...
        if self.cached:
            LOGGER.debug('>> Using Cache: {}'.format(self._last_dag_path))
            return
        ...
        try:
            if attr in MetaNode.UNMANAGED:
                ...
                try:
                    ...
                    if object_attr:
                        return data
                    raise AttributeError('Object instance has no attribute : {}'.format(attr))
    ...
    def __get_message_attr__(self, attr):
        msg_links = maya.cmds.listConnections(('{0}.{1}'.format(self.meta_node, attr)),
          destination=True, source=True, sh=True)
        if msg_links:
                ...
                if attr_type == 'string':
                    if value_type == 'string' or value_type == 'unicode':
                        ...
                        if value_type == 'complex':
                            LOGGER.debug('setAttr : {0} : type : "complex_string" to value : {1}'.format(attr, serialize_json_attr(value)[0]))
    ...
    def __repr__(self):
        try:
            if self.has_attr('meta_class'):
                return '{0}(meta_class: "{1}", node: "{2}")'.format(self.__class__, self.meta_class, self.meta_node.split('|')[(-1)])
            else:
                return '{0}(Wrapped Standard MayaNode, node: "{1}")'.format(self.__class__, self.meta_node.split('|')[(-1)])
        except Exception:
            try:
                metadatamanager.METANODES_CACHE.pop(object.__getattribute__(self, '_lastUUID'))
                LOGGER.debug('Dead MetaNode {0} removed from cache ...'.format(object.__getattribute__(self, '_lastDagPath')))
            except Exception:
                pass

            try:
                return 'Dead MetaNode : Last good Dag Path was: {0}'.format(object.__getattribute__(self, '_lastDagPath'))
            except Exception:
        ...
        if mobj_handle:
            try:
                if not mobj_handle.isValid():
                    LOGGER.info('MetaNode : MObject is no longer valid - {0} - object may have been deleted or the scene reloaded?'.format(object.__getattribute__(self, 'meta_node_id')))
                    return
        ...
        if not node:
            ...
            if check_instance:
                ...
                if meta_class:
                    if meta_class in metadatamanager.METANODE_CLASSES_REGISTER:
                        if meta_types:
                            if meta_class in metadatamanager.MetaDataManager.meta_types_to_registry_key(meta_types=meta_types):
    ...
    def is_meta_node_inherited(cls, node, meta_instances=[], mode='short'):
        ...
        if not node:
            ...
            if meta_class:
                if meta_class in metadatamanager.METANODE_CLASSES_REGISTER:
                    for inst in metadatamanager.MetaDataManager.meta_types_to_registry_key(meta_types=meta_instances):
                        if mode == 'full':
                            if metadatamanager.METANODE_CLASSES_REGISTER[inst] in metadatamanager.METANODE_CLASSES_INHERITANCE_MAP[meta_class]['full']:
                                LOGGER.debug('MetaNode {0} is subclass >> {1}'.format(meta_class, inst))
                                return True
                        else:
                            if issubclass(metadatamanager.METANODE_CLASSES_REGISTER[meta_class], metadatamanager.METANODE_CLASSES_REGISTER[inst]):
                                LOGGER.debug('MetaNode {0} is subclass >> {1}'.format(meta_class, inst))
                                return True
    ...
    def register_metanode_to_cache(meta_node):
        ...
        """
        metadatamanager.MetaDataManager.register_metanode_to_cache(meta_node=meta_node)
    ...
    def is_valid(self):
        ...
        try:
            if not self.is_valid_mobject():
                return False
            if self.has_attr('meta_class'):
                if not maya.cmds.listRelatives(self.meta_node):
                    return False
        except Exception:
    ...
    def add_attribute(self, attr, value=None, attr_type=None, hidden=False, **kwargs):
        ...
        """
        LOGGER.debug('|Adding Attribute| >> node: {0} | attr: {1} | attrType: {2}'.format(self.meta_node, attr, attr_type))
        added = False
        if attr_type:
            if attr_type == 'enum':
                if 'enumName' not in kwargs:
                    raise ValueError('enum attribute type must be passed with "enumName" keyword in args')
        DataTypeKwargs = {'string':{'longName':attr, 
          'dt':'string'}, 
         'unicode':{'longName':attr, 
          'dt':'string'}, 
         'int':{'longName':attr, 
          'at':'long'}, 
         'bool':{'longName':attr, 
         ...
         'float':{'longName':attr, 
          'at':'double'}, 
         'float3':{'longName':attr, 
          'at':'float3'}, 
         'double3':{'longName':attr, 
          'at':'double3'}, 
         'doubleArray':{'longName':attr, 
          'dt':'doubleArray'}, 
         'enum':{'longName':attr, 
          'at':'enum'}, 
         'complex':{'longName':attr, 
          'dt':'string'}, 
         'message':{'longName':attr, 
          'at':'message',  'm':True,  'im':True}, 
         'messageSimple':{'longName':attr, 
          'at':'message',  'm':False}}
        keyable = [
         'int', 'float', 'bool', 'enum', 'double3']
        ...
        if self.has_attr(attr):
                ...
                if kwargs:
                    ...
                    if set_kwargs_to_edit:
                        try:
                            ...
                            LOGGER.debug('MetaNode is referenced and the setEditFlags are therefore invalid (lock, keyable, channelBox)')
            ...
            try:
                if not attr_type:
                    ...
                    LOGGER.debug('addAttr : {0} : value_type : {1} > data_type keywords: {2}'.format(attr, attr_type, DataTypeKwargs[attr_type]))
    ...
    def attr_is_locked(self, attr):
        ...
        if hasattr(attr, '__iter__'):
            locked = False
            for a in attr:
                if maya.cmds.getAttr(('{0}.{1}'.format(self.meta_node, attr)), long=True):
                    locked = True
                    break

            return locked
        else:
            return maya.cmds.getAttr(('{0}.{1}'.format(self.meta_node, attr)), long=True)
    ...
    def attr_set_locked(self, attr, state):
        ...
        try:
            ...
            if not self.is_referenced():
                for a in attr:
                    maya.cmds.setAttr(('{0}.{1}'.format(self.meta_node, a)), long=state)
    ...
    def convert_meta_class_to_type(self, node_to_convert, new_meta_class):
        ...
        :param kwargs:
        """
        new_meta_class = metadatamanager.MetaDataManager.meta_types_to_registry_key(new_meta_class)[0]
            ...
            for plug in plugs:
                ...
                if attr == current_name:
                    try:
                        ...
                        LOGGER.debug('Renamed Child attribute to match new MetaNode name: {}.{}'.format(child.meta_node, attr))
                    except Exception:
            ...
            if uuid in metadatamanager.METANODES_CACHE.keys():
                try:
                    if MetaNode.check_metanode_validity(metadatamanager.METANODES_CACHE[uuid]):
                        LOGGER.debug('CACHE: Cached MetaNode {} has a valid MObject'.format(metadatamanager.METANODES_CACHE[uuid]))
                        if not metadatamanager.METANODES_CACHE[uuid]._MObject == get_mobject(meta_node):
                            LOGGER.debug('CACHE: {} : UUID is already registered but to a different node: {}'.format(uuid, meta_node))
                            return
                        else:
                            LOGGER.debug('CACHE : {} : Returning MetaNode from UUID cache! => {}'.format(meta_node, uuid))
    ...
    def is_child_node(self, node, attr=None, source_attr=None):
        ...
        if issubclass(type(node), MetaNode):
            ...
            if attr:
                connections = maya.cmds.ls(maya.cmds.listConnections(('{0}.{1}'.format(self.meta_node, attr)), source=False, destination=True, plugs=True),
                  long=True)
            else:
                connections = maya.cmds.ls(maya.cmds.listConnections((self.meta_node), source=False, destination=True, plugs=True),
                  long=True)
        if connections:
            for cnt in connections:
                if source_attr:
                ...
                if not self.is_child_node(node, attr, source_attr):
                    try:
                        ...
                        maya.cmds.connectAttr(('{0}.{1}'.format(self.meta_node, attr)),
                          ('%s.%s[%i]' % (
                         node, source_attr, self.get_next_array_index(node, source_attr))),
                          f=force)
                    except Exception:
    ...
    def connect_children(self, nodes, attr, source_attr=None, clean_current=False, force=True, allow_incest=True, source_simple=False, **kwargs):
        ...
        if not issubclass(type(nodes), list):
            nodes = [
             nodes]
        if clean_current:
            self.disconnect_current_attr_plugs(attr)
        if not source_attr:
            source_attr = self.meta_node_id
        if not nodes:
            return
        for node in nodes:
            ...
            if self.is_meta_node(node=node):
                ...
                if maya.cmds.objExists('{0}.{1}'.format(node, source_attr)) or allow_incest:
                    MetaNode(node).add_attribute(source_attr, attr_type='message')
                else:
                ...
                if not self.is_child_node(node=node, attr=attr, source_attr=source_attr):
                    try:
                        if is_meta or allow_incest:
                            if is_meta:
                                LOGGER.debug('Connecting MeatNode nodes via indices: {0}.{1} >> {2}.{3}'.format(self.meta_node, attr, node, source_attr))
                            else:
                                if allow_incest:
                                    LOGGER.debug('Connecting Standard Maya nodes via indices: {0}.{1} >> {2}.{3}'.format(self.meta_node, attr, node, source_attr))
                                if not source_simple:
                                    ...
                                    maya.cmds.connectAttr(('%s.%s[%i]' % (
                                     self.meta_node, attr,
                                     self.get_next_array_index(self.meta_node, attr))),
                                      ('%s.%s' % (node, source_attr)), f=force)
                        else:
                            LOGGER.debug('Connecting {0}.{1} >> {2}.{3}'.format(self.meta_node, attr, node, source_attr))
                            maya.cmds.connectAttr(('{0}.{1}'.format(node, source_attr)), f=force)
                    except Exception:
                        maya.cmds.connectAttr(('{0}.{1}'.format(self.meta_node, attr)), ('{0}.{1}'.format(node, source_attr)),
                          f=force)
    ...
    def disconnect_child(self, node, attr=None, delete_source_plug=True, delete_dst_plug=True):
        ...
        for source_plug, dst_plug in zip(connections[0::2], connections[1::2]):
            ...
            if attr and search_connection == dst_plug.split('[')[0] or not attr and search_connection in dst_plug:
                LOGGER.debug('Disconnecting {0} >> {1} as {2} found in destination plug'.format(dst_plug, source_plug, search_connection))
        ...
        if delete_source_plug:
            try:
                allow_delete = True
                attr = source_plug.split('[')[0]
                if maya.cmds.listConnections(attr):
                    allow_delete = False
                    LOGGER.debug('SourceAttr connections remaining: {0}'.format(','.join(maya.cmds.listConnections(attr))))
                if allow_delete:
        ...
        if delete_dst_plug:
            try:
                ...
                if maya.cmds.listConnections(attr):
                    allow_delete = False
                    LOGGER.debug('DstPlug connections remaining: {0}'.format(','.join(maya.cmds.listConnections(attr))))
                if allow_delete:
    ...
    def get_message(self, attr, full_path=True, as_meta=False, data_attr=None, data_key=None, simple=True, *args, **kwargs):
        """
        ...
        :return:
        """
        result = metautils.MetaAttributeUtils.get_message((self.meta_node),
          attr, data_attr=data_attr, data_key=data_key, simple=simple)
        if as_meta:
            if result:
...
def validate_obj_arg(node, meta_class, none_valid=False, default_meta_type=None, maya_type=None, update_class=False):
    ...
    if meta_class is not None:
        t1 = time.clock()
        if type(meta_class) not in [unicode, str]:
            try:
    ...
    if not none_valid:
        ...
        if node in (None, False):
            ...
            if uuid in current_metaclases_keys:
                ...
                if unicode_node in current_metaclases_keys:
            ...
            if cached is not None:
                ...
                if _node != cached.meta_node:
                    ...
                    if cached_type == new_meta_class:
                        LOGGER.debug('Cached Type ({0}) match with new given MetaClass ({1})'.format(cached_type, new_meta_class))
                        if update_class:
                            if not cached_meta_class:
                    ...
                    if cached_meta_class:
                        if meta_class is not None:
                            if cached_type != new_meta_class:
                                ...
                                if cached_meta_class != meta_class:
                                    LOGGER.debug('Cached Type ({0}) does not match with new given MetaClass ({1})'.format(cached_meta_class, meta_class))
                                    change = True
                    else:
                        if new_meta_class:
                ...
                if change:
                    try:
                        if issubclass(cached_type, new_meta_class):
                            if not update_class:
                                change = False
                    except Exception as e:
        ...
        if meta_class:
            if update_class or was_cached:
                LOGGER.debug('Updating current MetaClass ({0}) to new MetaClass ({1})'.format(node_meta_class, new_meta_class.__name__))
                try:
                    metautils.MetaAttributeUtils.add(_node_short, 'meta_class', 'string')
                except Exception:
                    pass
            ...
            if node_meta_class:
                ...
                if default_meta_type:
                    if type(default_meta_type) not in [unicode, str]:
                        try:
                            default_meta_type = default_meta_type.__name__
                        except Exception as e:
                            raise ValueError('Meta Type is not a string and is not a usable class name. Default MetaType: {}'.format(default_meta_type))
    ...
    if metautils.MetaAttributeValidator.is_transform(_node_short):
        try:
...
def validate_obj_list_arg(list_arg=None, meta_class=None, none_valid=False, default_meta_type=None, maya_type=None, update_class=False):
    if type(list_arg) not in [list, tuple]:
        list_arg = [
         list_arg]
    return_list = list()
    kwargs = {'meta_class':meta_class,  'none_valid':none_valid,  'default_meta_type':default_meta_type,  'maya_type':maya_type, 
