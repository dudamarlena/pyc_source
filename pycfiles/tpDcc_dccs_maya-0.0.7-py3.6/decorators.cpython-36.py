...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/dccs/maya/core/decorators.py
# Compiled at: 2020-04-11 22:28:15
# Size of source mod 2**32: 11008 bytes
"""
...
class ShowMayaProgress(object):
    ...
    def start(self):
        ...
        maya.cmds.progressBar((self._main_progressbar), edit=True, beginProgress=True, isInterruptable=(self._interruptable),
          status=(self._status),
          minValue=(self._start_value),
          maxValue=(self._end_value))
        maya.cmds.refresh()
...
def TryExcept(fn):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            print(fn.__doc__, 'n')
            print('=================== ERROR ===================')
            maya.cmds.inViewMessage(amg=('<span style="color:#F05A5A;">Error: </span>' + str(e) + ' <span style="color:#FAA300;">Look at the script editor for more info about the error.</span>'),
              pos='topCenter',
              fade=True,
              fst=4000,
              dk=True)
