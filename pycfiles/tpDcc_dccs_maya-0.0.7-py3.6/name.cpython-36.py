...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/dccs/maya/core/name.py
# Compiled at: 2020-04-11 22:28:15
# Size of source mod 2**32: 42138 bytes
"""
...
def get_short_name(obj):
    ...
    if node_names:
        ...
        for i, o in enumerate(node_names):
            maya.logger.warning('    ' + '{0}: "{1}"'.format(i, o))
...
def get_long_name(obj):
    ...
    if node_names:
        ...
        for i, o in enumerate(node_names):
            maya.logger.error('    ' + '{0}: "{1}"'.format(i, o))
...
def find_unique_name_by_filter(filter_type, include_last_number=True, do_rename=False, rename_shape=True, search_hierarchy=False, selection_only=True, dag=False, remove_maya_defaults=True, transforms_only=True):
    """
    ...
    filtered_obj_list = filtertypes.filter_by_type(filter_type=filter_type,
    ...
    if not filtered_obj_list:
        ...
        return find_unique_name(obj_names=filtered_obj_list,
...
def add_prefix(prefix, obj_names=None, uuid=None, add_underscore=False, rename_shape=True, check_existing_prefix=True):
    ...
    :param filter_type: str, If given, only nodes with given filter type will be processed
...
def add_prefix_by_filter(prefix, filter_type, add_underscore=False, rename_shape=True, search_hierarchy=False, selection_only=True, dag=False, remove_maya_defaults=True, transforms_only=True):
    """
    ...
    if not filtered_obj_list:
        ...
        return add_prefix(prefix=prefix,
          obj_names=filtered_obj_list,
          add_underscore=add_underscore,
          rename_shape=rename_shape)


def add_suffix_by_filter(suffix, filter_type, add_underscore=False, rename_shape=True, search_hierarchy=False, selection_only=True, dag=False, remove_maya_defaults=True, transforms_only=True):
    """
    Adds a prefix to filtered by types nodes.
    :param suffix: str, suffix to add
    ...
    if not filtered_obj_list:
        ...
        return add_suffix(suffix=suffix,
          obj_names=filtered_obj_list,
          add_underscore=add_underscore,
          rename_shape=rename_shape)
...
def change_suffix_padding_by_filter(filter_type, padding=2, add_underscore=True, rename_shape=True, search_hierarchy=False, selection_only=True, dag=False, remove_maya_defaults=True, transforms_only=True):
    """
    ...
    if not filtered_obj_list:
        ...
        return change_suffix_padding(obj_names=filtered_obj_list,
          padding=padding,
          add_underscore=add_underscore,
          rename_shape=rename_shape)
...
def edit_item_index(obj_names, index, text='', mode=EditIndexModes.INSERT, separator='_', rename_shape=True, uuid=None):
    ...
    def _edit_item_index(obj_name, index, obj_uuid=None):
        long_prefix, namespace, base_name = get_node_name_parts(obj_name)
        base_name_list = base_name.split(separator)
        if not check_index_in_list(base_name_list, index):
            return obj_name
        else:
            if mode == EditIndexModes.REMOVE:
                if len(base_name_list) == 1:
                    maya.logger.warning('Not enough name parts to rename: {}'.format(base_name_list[0]))
                    return obj_name
                del base_name_list[index]
            else:
                if mode == EditIndexModes.REPLACE:
                    if not text:
                        del base_name_list[index]
                    else:
                        base_name_list[index] = text
                else:
                    if mode == EditIndexModes.INSERT:
                        if not text:
                            return obj_name
                        else:
                            neg = False
...
def edit_item_index_by_filter(index, filter_type, text='', mode=EditIndexModes.INSERT, separator='_', rename_shape=True, search_hierarchy=False, selection_only=True, dag=False, remove_maya_defaults=True, transforms_only=True):
    """
    Splits object node name by given separator and edits the position based on the given index
    :param index: int, positive or negative index number (['pSphere', 'grp'] -> 0='pSphere'; 1='grp'; -1='grp')
    :param filter_type: str, filter used to filter nodes to edit index of
    ...
    if not filtered_obj_list:
        ...
        return edit_item_index(obj_names=filtered_obj_list,
          index=index,
          text=text,
          mode=mode,
          separator=separator,
...
def auto_suffix_object(obj_names, uuid=None, rename_shape=True):
    ...
    def _auto_suffix_obj(obj_name, obj_uuid=None):
        ...
        if obj_type == 'transform':
            ...
            if obj_type == 'joint':
                shape_nodes = maya.cmds.listRelatives(obj_name, shapes=True, fullPath=True)
                if shape_nodes:
                    if maya.cmds.objectType(shape_nodes[0]) == 'nurbsCurve':
                        obj_type = 'controller'
        ...
        if obj_type not in auto_suffix:
            ...
            if existing_suffix == suffix:
                return obj_name
            return add_suffix(obj_names=obj_name,
              suffix=suffix,
              uuid=obj_uuid,
              add_underscore=True,
              rename_shape=rename_shape)
    ...
    if not auto_suffix:
        maya.logger.warning('Impossible to launch auto suffix functionality because no auto suffixes are defined for Maya!')
        return
    else:
        if isinstance(obj_names, (tuple, list)):
            uuid_list = maya.cmds.ls(obj_names, uuid=True)
...
def auto_suffix_object_by_type(filter_type, rename_shape=True, search_hierarchy=False, selection_only=True, dag=False, remove_maya_defaults=True, transforms_only=True):
    """
...
def remove_numbers_from_object_by_filter(filter_type, trailing_only=False, rename_shape=True, remove_underscores=True, search_hierarchy=False, selection_only=True, dag=False, remove_maya_defaults=True, transforms_only=True):
    """
    ...
    if not filtered_obj_list:
        ...
        return remove_numbers_from_object(obj_names=filtered_obj_list,
...
def renumber_objects_by_filter(filter_type, remove_trailing_numbers=True, add_underscore=True, padding=2, rename_shape=True, search_hierarchy=False, selection_only=True, dag=False, remove_maya_defaults=True, transforms_only=True):
    """
    ...
    if not filtered_obj_list:
        ...
        return renumber_objects(obj_names=filtered_obj_list,
