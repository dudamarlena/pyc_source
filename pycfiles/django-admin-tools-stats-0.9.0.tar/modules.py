...
# [GCC 8.4.0]
# Embedded file name: /home/areski/projects/django/django-admin-tools-stats/admin_tools_stats/modules.py
# Compiled at: 2015-12-11 12:40:07
...
class DashboardChart(modules.DashboardModule):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.days is None:
            self.days = {'hours': 24, 'days': 7, 'weeks': 7, 'months': 60}[self.interval]
        self.data = self.get_registrations(self.interval, self.days, self.graph_key, self.select_box_value)
    ...
    def get_registrations(self, interval, days, graph_key, select_box_value):
        ...
        try:
            ...
            if conf_data.type_operation_field_name and conf_data.operation_field_name:
                operation = {'Sum': Sum(conf_data.operation_field_name), 'Avg': Avg(conf_data.operation_field_name), 
            ...
            stats = QuerySetStats(model_name.objects.filter(**kwargs), conf_data.date_field_name, aggregate)
            today = now()
            if days == 24:
    ...
    def prepare_template_data(self, data, graph_key, select_box_value):
        ...
        extra_serie = {'tooltip': {'y_start': '', 'y_end': ''}, 'date_format': self.tooltip_date_format}
        self.values = {'x': xdata, 
           'name1': self.interval, 
