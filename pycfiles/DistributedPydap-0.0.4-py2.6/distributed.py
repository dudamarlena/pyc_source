...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pydap/wsgi/distributed.py
# Compiled at: 2011-06-20 14:30:50
import os, hashlib
from sha import sha
    ...
    def __call__(self, environ, start_response):
        ...
        if req.path_info == '/jsonp/catalog':
            ...
            if etag in req.if_none_match:
                ...
                res = Response(body=body, content_type='text/javascript', charset='utf-8')
        elif req.path_info == '/jsonp/ping':
            body = jsonp + '(' + dumps('pong') + ')'
            res = Response(body=body, content_type='text/javascript', cache_control='no-cache, no-store', charset='utf-8')
        else:
            latencies = []
...
def md5sum(filepath, block_size=1048576):
    f = open(filepath)
    md5 = hashlib.md5()
    while True:
