...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/kuaipandriver/kuaipanfuse.py
# Compiled at: 2014-08-07 03:37:54
import os, sys, time, signal, pycurl
...
from kuaipandriver.common import timestamp, to_timestamp, define, SafeLRUCache, DiskCacheable, gethomedir, config, logger, CopyOnWriteBuffer
TYPE_DIR = S_IFDIR | 420
TYPE_FILE = S_IFREG | 420
BLOCK_SIZE = 4096
CACHE_PATH = '/dev/shm/'
ROOT_ST_INFO = {'st_mtime': timestamp(), 
   'st_mode': TYPE_DIR, 
   'st_size': BLOCK_SIZE, 
   'st_gid': os.getgid(), 
   'st_uid': os.getuid(), 
...
class TaskPool(object):
    ...
    def __getlock(self, tasktype):
        if tasktype == 'upload':
            return self.uploadlock
        return self.downlock

    def __getpool(self, tasktype):
        if tasktype == 'upload':
            return self.uploadpool
        return self.downloadpool
...
class WalkAroundTreeTask(Future):
    ...
    def listdir(self, path, allfilesinfo):
        ...
        for finfo in result['files']:
            ...
            st_info = {'st_mtime': to_timestamp(finfo['modify_time']), 
    ...
    def readfromserver(self, offset, size):
        while 1:
            with self.condition:
                ...
                if cachefilesize == self.filesize:
...
class AfterUploadHandler(object):
    ...
    def __call__(self, *args):
        ...
        viewtype = 'normal'
        if extname in ('.pdf', '.doc', '.wps', '.csv', '.prn', '.xls', '.et', '.ppt',
                       '.dps', '.txt', '.rtf'):
...
class KuaiPanFuse(LoggingMixIn, Operations):
    ...
    def initkuaipan(self):
        ...
        self.mkdir(config.get_virtual_convert_dir(), 420)
    ...
    def read(self, path, size, offset, fh):
        ...
        except OpenAPIException as apiex:
            raise FuseOSError(EIO)
    ...
    def write(self, path, data, offset, ph):
        ...
        if writetask:
            ...
            task = self.taskpool.upload_file(hashpath, self.api, path, filename, self.uploadqueue, handler=AfterUploadHandler(self.api, path, self))
    ...
    def statfs(self, path):
        ...
        return dict(f_bsize=BLOCK_SIZE, f_frsize=BLOCK_SIZE, f_blocks=self.quota['f_blocks'], f_bfree=self.quota['f_bavail'], f_bavail=self.quota['f_blocks'])
...
def main():
    ...
    try:
        ...
        FUSE(KuaiPanFuse(), mntpoint, foreground=options.foreground, nothreads=False, debug=False, big_writes=True, gid=os.getgid(), uid=os.getuid(), umask='0133')
