...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/kuaipandriver/kuaipanapi.py
# Compiled at: 2014-08-07 02:54:17
import sys, urllib, signal, common, requests, lxml.html as html
...
class KuaipanAPI(Singleton):
    ...
    def request_token(self, callback=None):
        ...
        try:
            ...
            if result.status_code == 200:
                msg = result.json()
                for k in ('oauth_token', 'oauth_token_secret', 'oauth_callback_confirmed'):
                    if msg.has_key(k):
                        v = msg.get(k)
    ...
    def __get_sig_url(self, method, urlsuffix=None, attachdata=None, httpmethod='get', has_oauth_token=True):
        method_url = {'request_token_base_url': config.get_request_token_base_url(), 
    ...
    def _oauth_parameter(self, has_token=True):
        parameters = {'oauth_consumer_key': self.consumer_key, 
