...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/lazyscraper/htmltools.py
# Compiled at: 2018-01-14 15:49:41
from .consts import *
...
def taglist_to_dict(tags, fields, strip_lf=True):
    ...
    data = []
    if has_text:
        finfields.remove(TEXT_FIELD)
    if has_tag:
        finfields.remove(TAG_FIELD)
    for t in tags:
        item = {}
        if has_tag:
