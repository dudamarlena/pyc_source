...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpRigToolkit/managers/tools.py
# Compiled at: 2020-02-05 21:34:07
# Size of source mod 2**32: 13749 bytes
"""
...
import os, logging, inspect, traceback, logging.config
from tpPyUtils import importer, decorators, python
import tpRigToolkit
from tpRigToolkit.core import tool, resource
...
class ToolsManager(object):
    ...
    def register_tool(self, pkg_loaders, environment):
        """
        Register tool given path
        :param str tool_path: path to the module (for example, tpRigToolkit.tools.rignode)
        """
        ...
        if pkg_loader in self._tools:
            ...
            return False
        tool_config = config.YAMLConfiguration(config_name=(tool_path.replace('.', '-')),
          environment=environment,
          config_dict={'join':os.path.join, 
        ...
        if not tool_id:
            LOGGER.warning('Impossible to register tool "{}" because its ID is not defined'.format(tool_path))
            return False
        if not tool_name:
            LOGGER.warning('Impossible to register tool "{}" because its ID is not defined'.format(tool_path))
            return False
        if tool_id in self._tools:
            LOGGER.warning('Impossible to register tool "{}" because its ID "{}" is already defined.'.format(tool_path, tool_id))
            return False
        tools_found = list()
        version_found = None
        for sub_module in loader.walk_packages([pkg_loader.filename]):
            ...
            try:
                ...
                msg = 'Impossible to register Tool: {}. Exception raised: {} | {}'.format(tool_path, exc, traceback.format_exc())
                LOGGER.warning(msg)
                continue

            if qname.endswith('__version__'):
            ...
            for cname, obj in inspect.getmembers(mod, inspect.isclass):
                if issubclass(obj, tpRigToolkit.Tool):
        ...
        if len(tools_found) > 1:
            ...
            self._tools[tool_id] = {'name':tool_name, 
             'icon':tool_icon, 
             'loader':pkg_loader, 
             'config':tool_config, 
    ...
    def run_tool(self, tool_path, do_reload=False, debug=False, extra_args=None):
        """
        ...
        default_logger_dir = os.path.normpath(os.path.join(os.path.expanduser('~'), 'tpRigToolkit', 'logs', 'tools'))
        ...
        logging.config.fileConfig((tool_config.data.get('logging_file', default_logging_config)),
        ...
        try:
            ...
            if not tool_found:
                ...
                return
            if extra_args:
                tool_widget = tool_found(config=tool_config, **extra_args)
            else:
                tool_widget = tool_found(config=tool_config, **extra_args)
            tool_name = tool_config.data.get('name', 'Tool')
            tool_logo = tool_config.data.get('logo', 'tpRigToolkit')
            tool_url = tool_config.data.get('url', None)
            if tool_found.ATTACHER_TYPE == tool.ToolAttacher.Window:
                win_name = '{}_Window'.format(tool_name)
                tool_win = tpRigToolkit.Window(name=win_name, tool=tool_widget)
                ...
                if tool_found.ATTACHER_TYPE == tool.ToolAttacher.Dialog:
    ...
    def get_config(self, tool_id):
        ...
        LOGGER.warning('Impossible to retrieve configuration of tool: "{}" because tool is not registered'.format(tool_id))
...
tpRigToolkit.register.register_class('ToolsMgr', ToolsManagerSingleton)