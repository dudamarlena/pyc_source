...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpRigToolkit/core/config.py
# Compiled at: 2020-02-05 21:34:07
# Size of source mod 2**32: 8425 bytes
"""
...
import os, logging, metayaml, tpDccLib as tp
...
class YAMLConfigurationAttribute(dict, object):
    ...
    def from_nested_dict(data):
        ...
        if not isinstance(data, dict):
            ...
            return YAMLConfigurationAttribute({key:YAMLConfigurationAttribute.from_nested_dict(data[key]) for key in data})
...
class YAMLConfiguration(object):
    ...
    def __init__(self, config_name, environment, config_dict=None, parser_class=YAMLConfigurationParser):
    ...
    def get(self, attr_section, attr_name=None, default=None):
        ...
        """
        if not self._parsed_data:
            LOGGER.warning('Configuration "{}" is empty for "{}"'.format(self._config_name, self._environment))
            return default
        else:
            if attr_section:
                if attr_name:
                    ...
                    if not attr_section:
                        LOGGER.warning('Configuration "{}" has no attribute "{}" in section "{}" for "{}"'.format(self._config_name, attr_name, orig_section, self._environment))
                        return default
                    else:
                        attr_value = attr_section.get(attr_name, None)
                        if attr_value is None:
                            LOGGER.warning('Configuration "{}" has no attribute "{}" in section "{}" for "{}"'.format(self._config_name, attr_name, attr_section, self._environment))
                            return default
                        return attr_value
                ...
                if attr_value is None:
                    LOGGER.warning('Configuration "{}" has no attribute "{}" for "{}"'.format(self._config_name, attr_to_use, self._environment))
                    return default
            return attr_value
    ...
    def _get_tprigtoolkit_config_paths(self, module_config_name):
        ...
        if not os.path.isdir(config_env_dir):
            LOGGER.warning('Configuration Folder for Environment "{}" does not exists: "{}"'.format(self._environment, config_env_dir))
        ...
        dcc_version_config_path = os.path.join(config_env_dir, tp.Dcc.get_name(), tp.Dcc.get_version_name(), module_config_name)
        return [
         config_path, dcc_config_path, dcc_version_config_path]
    ...
    def _get_config_data(self, config_name, config_dict):
        ...
        """
        if not config_name:
            tp.Dcc.error('Configuration File for tpRigToolkit not found! {}'.format(self, config_name))
            return
        else:
            ...
            if not valid_config_paths:
                raise RuntimeError('Impossible to load configuration "{}"  for tpRigToolkit because it does not exists in any of the configuration folders: {}'.format(config_name, ''.join(all_config_paths)))
            config_path = valid_config_paths[(-1)]
            config_data = metayaml.read(valid_config_paths, config_dict)
            if not config_data:
                raise RuntimeError('Configuration File is empty! {}'.format(self, config_path))
            if 'config' in config_data:
                if 'path' in config_data['config']:
                    raise RuntimeError('Configuration File for cannot contains config section with path attribute! {}'.format(self, config_path))
            if 'config' in config_data:
...
def get_config(config_name, config_dict=None, parser_class=YAMLConfigurationParser):
    ...
    """
    new_cfg = YAMLConfiguration(config_name=config_name,
