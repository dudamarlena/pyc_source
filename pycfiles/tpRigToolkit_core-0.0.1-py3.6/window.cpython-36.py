...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpRigToolkit/widgets/window.py
...
class MainWindow(tpQtLib.Window, object):
    ...
    def __init__(self, tool=None, name='MainWindow', title='MainWindow', size=(800, 535), fixed_size=False, parent=None, *args, **kwargs):
        self._tool = tool
        (super(MainWindow, self).__init__)(args, name=name, title=title, size=size, fixed_size=fixed_size, auto_run=True, frame_less=True, use_style=False, parent=parent, **kwargs)
        if self.parent():
            for widget in self.parent().findChildren(QMainWindow):
                if widget is not self and widget.objectName() == self.objectName():
                    widget.close()
