...
# Embedded file name: /home/zhanghui/py35/lib/python3.5/site-packages/mypythonhelper/pyspark_helper/call_detail_handler.py
# Compiled at: 2018-02-01 22:38:29
# Size of source mod 2**32: 9351 bytes
import json
...
class InfoParser(object):
    ...
    def PaymentInfoParser(paymentInfoList_Result):
        ...
        return_value = {'monthOfPayment': len(previous_result)}
    ...
    def BillInfoParser(billInfoList_Result):
        ...
        if previous_status == StatusField.failed:
            return Result({}, previous_status)
        return_value = {'monthOfBill': str(len(previous_result))}
        return Result(return_value, StatusField.succeed)
    ...
    def AccountInfoParser(AccountInfo_Result):
        ...
        if previous_status == StatusField.failed:
            return Result({}, previous_status)
        return_value = {'net_age': previous_result.get('net_age', StatusField.getNothingDefaultValue)}
        return Result(return_value, StatusField.succeed)
    ...
    def CallInfoParser(CallInfoList_Result):
        ...
        for call_detail_monthly in previous_result:
            ...
            if call_cycle is None:
                continue
            call_count_dict[call_cycle] = {'callsMonthly': call_detail_monthly.get('total_call_count', StatusField.getNothingDefaultValue), 
    ...
    def OtherNodesAndRelationshipsFromCallInfoParser(CallInfoList_Result):
        ...
        for call_records_monthly in previous_result:
            ...
            for record in each_call_records:
                ...
                if not otherNumber is None:
                    if otherNumber == 'UNKONWN':
                        pass
                    else:
                        OtherNumbersSetCollector.add(otherNumber)
                        call_type_name = record.get('call_type_name', 'UNKONWN')
                        one_relationships_dict = {'call_address': record.get('call_address', 'UNKONWN'), 
        ...
        return_value = (
         calling_relationships_dict, called_relationships_dict, OtherNumbersSetCollector)
        return Result(return_value, StatusField.succeed)


class GetInfoFromCalldetail(object):
    """GetInfoFromCalldetail"""
    ...
    def __init__(self, call_detail_str):
        ...
        for name_str, type_class, pLevel_str in structureList:
            setattr(self, name_str, type(name_str, (
