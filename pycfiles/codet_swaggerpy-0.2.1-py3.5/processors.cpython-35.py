...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/swaggerpy/processors.py
...
class SwaggerError(Exception):
    __doc__ = 'Raised when an error is encountered mapping the JSON objects into the\n    model.\n    '
...
class WebsocketProcessor(SwaggerProcessor):
    __doc__ = 'Process the WebSocket extension for Swagger\n    '
    ...
    def process_operation(self, resources, resource, api, operation, context):
        ...
        if operation['is_websocket']:
            ...
            if operation['httpMethod'] != 'GET':
                raise SwaggerError('upgrade: websocket is only valid on GET operations', context)


class FlatenningProcessor(SwaggerProcessor):
    __doc__ = 'Flattens model and property dictionaries into lists.\n\n    Mustache requires a regular schema.\n    '
