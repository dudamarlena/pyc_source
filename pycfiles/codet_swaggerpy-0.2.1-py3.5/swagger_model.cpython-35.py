...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/swaggerpy/swagger_model.py
# Compiled at: 2016-11-21 09:37:14
# Size of source mod 2**32: 10151 bytes
"""Code for handling the base Swagger API model.
"""
import json, os, urllib.request, urllib.parse, urllib.error, urllib.parse, codecs
...
class ValidationProcessor(SwaggerProcessor):
    __doc__ = 'A processor that validates the Swagger model.\n    '

    def process_resource_listing(self, resources, context):
        required_fields = [
         'basePath', 'apis', 'swaggerVersion']
        validate_required_fields(resources, required_fields, context)
        if resources['swaggerVersion'] not in SWAGGER_VERSIONS:
            raise SwaggerError('Unsupported Swagger version %s' % resources.swaggerVersion, context)
    ...
    def process_parameter(self, resources, resource, api, operation, parameter, context):
        ...
        if 'allowedValues' in parameter:
            raise SwaggerError("Field 'allowedValues' invalid; use 'allowableValues'", context)

    def process_error_response(self, resources, resource, api, operation, error_response, context):
        required_fields = [
         'code', 'reason']
...
class Loader(object):
    ...
    def load_api_declaration(self, base_url, api_dict):
        ...
        api_dict['api_declaration'] = json_load_url(self.http_client, api_dict['url'])
...
def validate_required_fields(json, required_fields, context):
    ...
    if missing_fields:
        raise SwaggerError('Missing fields: %s' % ', '.join(missing_fields), context)
...
def load_file(resource_listing_file, http_client=None, processors=None):
    ...
    return load_url(url, http_client=http_client, processors=processors, base_url=base_url)


def load_url(resource_listing_url, http_client=None, processors=None, base_url=None):
    ...
    return loader.load_resource_listing(resource_listing_url, base_url=base_url)
