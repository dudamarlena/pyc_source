...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/libs/kitsu/widgets/logindialog.py
...
class KitsuLoginDialog(base.BaseWidget, object):
    ...
    def _on_kitsu_worker_completed(self, uid, valid_login):
        ...
        """
        if valid_login:
            artellapipe.Tracker().is_logged() or LOGGER.warning('Something went wrong during Kitsu login')
            return False
        else:
            if artellapipe.Tracker().email:
                if artellapipe.Tracker().password:
