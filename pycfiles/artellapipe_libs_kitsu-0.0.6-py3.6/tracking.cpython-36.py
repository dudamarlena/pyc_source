...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/libs/kitsu/core/tracking.py
# Compiled at: 2020-04-15 11:57:47
# Size of source mod 2**32: 17615 bytes
"""
...
class KitsuTrackingManager(tracking.TrackingManager, object):
    ...
    def login(self, *args, **kwargs):
        ...
        if not kitsulib.host_is_up():
            ...
            qtutils.show_warning(None, 'Kitsu server is down!', 'Was not possible to retrieve Gazu API. This usually happens when Kitsu server is down. Please contact TD!')
            return False
        try:
    ...
    def all_project_assets(self):
        ...
        if not self.is_logged():
            ...
            for kitsu_asset in kitsu_assets:
                ...
                if not entity_type:
                    LOGGER.warning('Entity Type {} for Asset {} is not valid! Skipping ...'.format(entity_type, kitsu_asset.name))
                else:
    ...
    def all_project_sequences(self):
        ...
        if not self.is_logged():
            ...
            for kitsu_sequence in kitsu_sequences:
                ...
                if not entity_type:
                    LOGGER.warning('Entity Type {} for Sequence {} is not valid! Skipping ...'.format(entity_type, kitsu_sequence.name))
                else:
                    sequences_data.append({'sequence':kitsu_sequence, 
    ...
    def all_project_shots(self):
        ...
        if not self.is_logged():
            ...
            for kitsu_shot in kitsu_shots:
                ...
                if not entity_type:
                    LOGGER.warning('Entity Type {} for Shot {} is not valid! Skipping ...'.format(entity_type, kitsu_shot.name))
                else:
                    shots_data.append({'shot':kitsu_shot, 
    ...
    def all_assets_in_shot(self, shot_id, force_update=False):
        ...
        :return: list
        """
        if shot_id in self._shots_data:
            if self._shots_data[shot_id]:
                if not force_update:
    ...
    def casting_in_shot(self, shot, force_update=False):
        ...
        :return:
        """
        if shot in self._shots_data:
            if self._shots_data[shot]:
                if not force_update:
                    if 'casting' in self._shots_data[shot]:
                        return self._shots_data[shot]['casting']
            project_id = kitsu_lib.config.get('project_id', default=None)
            project_id or LOGGER.warning('Impossible to retrieve assets because does not defines a valid Kitsu ID')
            return
        else:
    ...
    def all_task_types_for_assets(self):
        ...
        for task_type in all_task_types:
            if not not task_type.for_entity == 'Asset':
    ...
    def _load_user_settings(self):
        ...
        if not self._project:
            return
        self._email = self._project.settings.get('kitsu_email') if self._project.settings.has_setting('kitsu_email') else None
        self._password = self._project.settings.get('kitsu_password') if self._project.settings.has_setting('kitsu_password') else None
        self._store_credentials = self._project.settings.get('kitsu_store_credentials') if self._project.settings.has_setting('kitsu_store_credentials') else False
