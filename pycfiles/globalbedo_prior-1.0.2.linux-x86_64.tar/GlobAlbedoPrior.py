...
# [GCC 8.4.0]
# Embedded file name: /opt/anaconda/lib/python2.7/site-packages/globalbedo_prior/GlobAlbedoPrior.py
# Compiled at: 2014-03-24 07:43:04
"""
...
"""
import argparse, sys, os, calendar, logging, time
try:
    import numpy as np
except ImportError:
...
class GlobAlbedoPrior(object):
    ...
    def __init__(self, tile, data_dir, output_dir, bands=[
 1, 2, 3, 4, 5, 6, 7], no_snow=True):
    ...
    def get_modis_fnames(self):
        ...
        for doy in xrange(1, 367, 8):
            ...
            self.fnames_mcd43a1[doy] = [ f for f in locate(pattern, root=self.data_dir)
                                       ]
            pattern = 'MCD43A2.A????%03d.%s.005.*.hdf' % (doy, self.tile)
            self.fnames_mcd43a2[doy] = [ f for f in locate(pattern, root=self.data_dir)
                                       ]
    ...
    def _interpret_qa(self, qa_data):
        ...
        """
        qa = np.bitwise_and(qa_data, 7)
        weight = np.where(qa < 4, MAGIC ** qa, 0.0)
        return weight
    ...
    def _interpret_snowmask(self, snow_data):
        ...
        """
        if self.no_snow:
            snow = np.where(snow_data == 0, True, False)
        else:
            snow = np.where(snow_data == 1, True, False)
        return snow
        ...
        for band in self.bands:
            for doy in self.fnames_mcd43a1.iterkeys():
                for kernel in ['iso', 'volu', 'geo']:
                    output_fname = os.path.join(self.output_dir, 'MCD43P.%03d.%s.%s.b%d.tif' % (
                    ...
                    self.output_ptrs[output_prod] = drv.Create(output_fname, 2400, 2400, 2, gdal.GDT_Float32, options=gdal_opts)
                    LOG.debug('\t... Created!')
    ...
    def stage1_prior(self):
        ...
        for band in self.bands:
            for doy in xrange(1, 367, 8):
                ...
                obs_fnames = [ 'HDF4_EOS:EOS_GRID:"%s":MOD_Grid_BRDF:BRDF_Albedo_Parameters_Band%d' % (f, band) for f in self.fnames_mcd43a1[doy]
                             ]
                qa_fnames = [ 'HDF4_EOS:EOS_GRID:"%s":MOD_Grid_BRDF:BRDF_Albedo_Band_Quality' % f for f in self.fnames_mcd43a2[doy]
                            ]
                snow_fnames = [ 'HDF4_EOS:EOS_GRID:"%s":MOD_Grid_BRDF:Snow_BRDF_Albedo' % f for f in self.fnames_mcd43a2[doy]
                              ]
                land_fnames = [ 'HDF4_EOS:EOS_GRID:"%s":MOD_Grid_BRDF:BRDF_Albedo_Ancillary' % f for f in self.fnames_mcd43a2[doy]
                              ]
                all_files = obs_fnames + qa_fnames + snow_fnames + land_fnames
                for ds_config, this_X, this_Y, nx_valid, ny_valid, data_in in extract_chunks(all_files):
                    LOG.info('\t\tRead a %d x %d pixel chunk...' % (ny_valid, nx_valid))
                    n_years = len(data_in) / 4
                    ...
                    for i, kernel in enumerate(['iso', 'volu', 'geo']):
                        ...
                        bout.WriteArray(prior_mean[i, :, :].astype(np.float32), xoff=this_X, yoff=this_Y)
                        bout = self.output_ptrs[output_prod].GetRasterBand(2)
                        bout.WriteArray(prior_var[i, :, :].astype(np.float32), xoff=this_X, yoff=this_Y)
    ...
    def stage2_prior(self):
        ...
        for band in self.bands:
            for kernel in ['iso', 'volu', 'geo']:
                drv = gdal.GetDriverByName('GTiff')
                output_fname = os.path.join(self.output_dir, 'MD43P.%s.%s.%b%d.tif' % (kernel, self.tile, band))
                dst_ds = drv.Create(output_fname, len(t), 2400, 2400, gdal.GDT_Float32, options=gdal_opts)
                mean_vals = np.empty((len(ts), 2400, 2400))
                i = 0
                for d in ts:
                    g = gdal.Open(os.path.join(self.output_dir, 'MCD43P.%03d.%s.%s.b%d.tif' % (
                ...
                for this_doy in t:
                    ...
                    mean_vals = np.ma.array(mean_vals, mask=mean_vals == 0.0)
                    mean_iterp = np.ma.average(mean_vals, axis=0, weights=w)
