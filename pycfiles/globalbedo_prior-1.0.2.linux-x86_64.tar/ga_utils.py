...
# [GCC 8.4.0]
# Embedded file name: /opt/anaconda/lib/python2.7/site-packages/globalbedo_prior/ga_utils.py
# Compiled at: 2014-03-23 18:29:35
"""
...
GDAL2NUMPY = {gdal.GDT_Byte: np.uint8, gdal.GDT_UInt16: np.uint16, 
...
def calculate_prior(brdf_data, mask):
    ...
    for i in xrange(3):
        A = np.ma.array(brdf_data[:, i, :, :] * 0.001, mask=np.logical_or(brdf_data[:, i, :, :] == 32767, mask == 0))
        kw_mean = np.ma.average(A, axis=0, weights=mask)
        v1 = np.ma.sum(mask, axis=0)
        v2 = np.ma.sum(mask ** 2, axis=0)
        kw_weight = np.sum(mask * (A - kw_mean) ** 2, axis=0) * (v1 / (v1 * v1 - v2))
    ...
    for X in xrange(nx_blocks):
        ...
        for Y in xrange(ny_blocks):
            ...
            for ig, ptr in enumerate(gdal_ptrs):
                ...
                buf = ptr.ReadRaster(this_X, this_Y, nx_valid, ny_valid, buf_xsize=nx_valid, buf_ysize=ny_valid, band_list=the_bands)
