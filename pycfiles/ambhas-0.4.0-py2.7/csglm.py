...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ambhas/csglm.py
# Compiled at: 2012-05-18 10:29:18
"""
...
import numpy as np, xlrd, xlwt, os, gdal
...
class CSGLM():
    ...
    def _read_input(self):
        ...
        """
        input_sheets = [
         'ind', 'forcing', 'initial_condition', 'gw_par',
    ...
    def _read_spatial(self):
        ...
        if no_layer != len(z):
            raise ValueError('The length of the thickness_layers            should be equal to the No_layer')
    ...
    def _read_initial_condition(self):
        ...
        try:
            ...
            raise ValueError('The length of the theta_0 should be             equal to the no_layer')
    ...
    def _read_shp(self):
        ...
        m = 1 - 1 / soil_par['n']
        soil_par['evap_fc'] = self.psi2theta(-0.33, soil_par['qr'], soil_par['f'], soil_par['a'], m, soil_par['n'])
        soil_par['evap_wp'] = self.psi2theta(-15, soil_par['qr'], soil_par['f'], soil_par['a'], m, soil_par['n'])
    ...
    def _colored_output(self, output_message, color):
        ...
        """
        print ('\x1b[31m' + output_message + '\x1b[0m').replace('31', str(color))
    ...
    def _soil_fun(self):
        ...
        for i in range(self.no_layer):
            if i == 0:
                ...
                U[0] = (-AT[0] - K[0] + self.net_rain_cur - AE - self.runoff_cur + self.pumping_cur) / z[0]
            else:
                if i == self.no_layer - 1:
    ...
    def _smi_fun(self):
        ...
        stress index
        """
        SSMI = (self.sm[(0, self.t)] - self.soil_par['evap_wp']) / (self.soil_par['evap_fc'] - self.soil_par['evap_wp'])
        if SSMI > 1:
            SSMI = 1
        else:
            if SSMI < 0:
    ...
    def _surface_storage_fun(self):
        ...
        """
        surface_storage = self.surface_storage[self.t] + self.runoff_cur + self.horton_runoff[self.t]
...
class CSGLM_ENKF(CSGLM):
    ...
    def _read_shp_ens(self):
        ...
        m = 1 - 1 / shp_ens['n']
        shp_ens['evap_fc'] = self.psi2theta(-0.33, shp_ens['qr'], shp_ens['f'], shp_ens['a'], m, shp_ens['n'])
        shp_ens['evap_wp'] = self.psi2theta(-15, shp_ens['qr'], shp_ens['f'], shp_ens['a'], m, shp_ens['n'])
        ...
        self.Ks_min, self.Ks_max = (1e-06, 1e-05)
    ...
    def _smi_fun(self):
        ...
        ens = self.ens
        sm = self.sm_ens[ens]
        SSMI = (sm[0] - self.soil_par_ens['evap_wp'][ens]) / (self.soil_par_ens['evap_fc'][ens] - self.soil_par_ens['evap_wp'][ens])
        if SSMI > 1:
            SSMI = 1
        else:
            if SSMI < 0:
    ...
    def _surface_storage_ens_fun(self):
        ...
        """
        ens = self.ens
        surface_storage = self.surface_storage_ens[(ens, self.t)] + self.runoff_cur + self.horton_runoff
    ...
    def _enkf_par(self):
        ...
        X_X_bar = X - X_bar
        cov_XX = np.dot(X_X_bar.T, X_X_bar) + 1e-06 * np.eye(self.no_layer + 6)
        ...
        v = v - np.tile(v.mean(axis=0), (self.n_ens, 1))
        ev = e + v
        cov_ee = np.dot(ev.T, ev) + 1e-06 * np.eye(self.no_layer + 6)
