...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-ppc/egg/garbas/forum/content/base.py
# Compiled at: 2008-10-07 13:03:03
...
class ForumFolder(ATBTreeFolder):
    ...
    def _getLastPost(self):
        ...
        result = catalog(object_provides=IForumPost.__identifier__, path=dict(query=('/').join(self.getPhysicalPath())), sort_on='created', sort_order='reverse', sort_limit=1)
        if result:
            return result[0].getObject()

    def _getLastTopic(self):
        catalog = getToolByName(self, 'portal_catalog')
        result = catalog(object_provides=IForumTopic.__identifier__, path=dict(query=('/').join(self.getPhysicalPath())), sort_on='created', sort_order='reverse', sort_limit=1)
        if result:
            return result[0].getObject()
    ...
    def forum_topics(self):
        ...
        catalog = getToolByName(self, 'portal_catalog')
        return len(catalog(object_provides=IForumTopic.__identifier__, path=dict(query=('/').join(self.getPhysicalPath()))))

    def forum_posts(self):
        """number of all posts"""
        catalog = getToolByName(self, 'portal_catalog')
        return len(catalog(object_provides=IForumPost.__identifier__, path=dict(query=('/').join(self.getPhysicalPath()))))
    ...
    def forum_info(self):
        ...
        if not last_post:
            last_post = self._getLastTopic()
        return [
         last_post != None and last_post.Creator() or '', last_post != None and last_post.absolute_url()[len(self.absolute_url()):] or '']
...
def reindex_for_topics(context, event):
    ...
    def reindex_parent(context):
        ...
        if IForum.providedBy(parent) or IForumTopic.providedBy(parent):
            parent.reindexObject(['forum_topics', 'modified', 'forum_info'])
...
def reindex_for_posts(context, event):
    ...
    def reindex_parent(context):
        parent = aq_inner(context.getParentNode())
        if IForum.providedBy(parent) or IForumTopic.providedBy(parent):
            parent.reindexObject(['forum_posts', 'modified', 'forum_info'])
            reindex_parent(parent)
