...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/deeplearn/conv.py
...
class Im2DimCalculator(object):
    ...
    def validate(self):
        W, H, F, P, S = (
         self.W, self.H, self.F, self.P, self.S)
        if (W - F[0] + 2 * P) % S:
        ...
        if self.W_row[1] != self.X_col[0]:
            raise ValueError(f"columns of W_row {self.W_row} do not match " + f"rows of X_col {self.X_col}")
    ...
    def X_col(self):
        return (
         self.F[0] ** 2 * self.D, self.W_out * self.H_out)

    @property
    @persisted('_W_col')
    def W_row(self):
        return (
         self.K, self.F[0] ** 2 * self.D)
...
class ConvolutionLayerFactory(object):
    __doc__ = 'Create convolution layers.\n    '
