...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/deeplearn/result.py
...
from zensols.persist import persisted, PersistableContainer, DirectoryStash
from zensols.deeplearn import Batch, ModelSettings, NetworkSettings
...
class ResultsContainer(PersistableContainer, Writable, metaclass=ABCMeta):
    __doc__ = 'Container class for results while training, testing and validating a model.\n\n    '
    PREDICTIONS_INDEX = 0
    LABELS_INDEX = 1
    ...
    @staticmethod
    def compute_metrics(average: str, y_true: np.ndarray, y_pred: np.ndarray) -> Dict[(str, float)]:
        scores = tuple(map(lambda f: f(y_true, y_pred, average=average), (
         mt.f1_score, mt.precision_score, mt.recall_score)))
        return {'f1':scores[0],  'precision':scores[1], 
         'recall':scores[2]}
    ...
    def dataframe(self) -> pd.DataFrame:
        ...
        """
        self._assert_results()
        return pd.DataFrame({'id':self.ids,  'label':self.labels, 
    ...
    def write(self, depth: int=0, writer: TextIOWrapper=sys.stdout):
        ...
        writer.write(f"{sp}micro: F1: {micro['f1']:.3f}, " + f"precision: {micro['precision']:.2f}, " + f"recall: {micro['recall']:.2f}\n")
        writer.write(f"{sp}macro: F1: {macro['f1']:.3f}, " + f"precision: {macro['precision']:.2f}, " + f"recall: {macro['recall']:.2f}\n")


@dataclass
class EpochResult(ResultsContainer):
    __doc__ = 'Contains results recorded from an epoch of a neural network model.  This is\n    during a training/validation or test cycle.\n\n    :param loss_updates: the losses generated from each iteration of the epoch\n    :param id_updates: the IDs of the data points from each iteration of the\n                       epoch\n    :param prediction_updates: the predictions generated by the model from each\n                               iteration of the epoch\n\n    '
    index: int
    split_type: str
    loss_updates = field(default_factory=list)
    loss_updates: List[float]
    id_updates = field(default_factory=list)
    id_updates: List[int]
    prediction_updates = field(default_factory=list)
    prediction_updates: List[np.ndarray]
    n_data_points = field(default_factory=list)
    n_data_points: List[int]

    def update(self, batch: Batch, loss: torch.Tensor, labels: torch.Tensor, preds: torch.Tensor, label_shape: List[tuple]):
        logger.debug(f"{self.index}:{self.split_type}: " + f"update batch: {batch.id}, label_shape: {label_shape}")
...
class DatasetResult(ResultsContainer):
    __doc__ = 'Contains results from training/validating or test cycle.\n\n    :param results: the results generated from the iterations of the epoch\n    '
    results = field(default_factory=list)
    results: List[EpochResult]
    start_time = field(default=None)
    start_time: datetime
    end_time = field(default=None)
    end_time: datetime
...
class ModelResult(ResultsContainer, Writable):
    ...
    def write_result_statistics(self, result_name: str, depth: int=0, writer=sys.stdout):
        sp = self._sp(depth)
    ...
    def write(self, depth: int=0, writer: TextIOWrapper=sys.stdout, verbose=False):
        ...
        self._write_line(f"Learning rate: {self.model_settings.learning_rate}", depth, writer)
        sp = self._sp(depth + 1)
...
class ModelResultGrapher(object):
    ...
    def __post_init__(self):
        ...
        if self.title is None:
            self.title = 'Figure {r.index} (lr={r.model_settings.learning_rate:.5f}, F1={r.micro_metrics[f1]:.3f})'
    ...
    def plot(self, containers: List[ModelResult], show: bool=False):
        ...
        for i, cont in enumerate(containers):
            ...
            es = tuple(map(lambda n: (n.capitalize(), cont.dataset_result[n]), self.split_types))
