...
# Embedded file name: /Library/Python/2.7/site-packages/bqlib.py
# Compiled at: 2014-03-18 10:32:07
__doc__ = ' bqlib - BigQuery python library '
import logging, urllib2, time, sys, datetime, os, re
...
class BaseBQ(object):
    ...
    def __init__(self, http, project_id=None, discovery_document_storage=None, bq_client=None):
        """Initialize BaseBQ.
        ...
        if bq_client is None:
            ...
            discovery_document = BQHelper.retrieve_discovery_document(discovery_document_storage)
            bq_client = BigqueryClient(api=_API, api_version=_API_VERSION, project_id=project_id, discovery_document=discovery_document, wait_printer_factory=BigqueryClient.QuietWaitPrinter)
            bq_client._apiclient = BQHelper.build_apiclient(discovery_document, http)
...
class BQJob(BaseBQ):
    ...
    def __init__(self, http, project_id, discovery_document_storage=None, bq_client=None, query=None, verbose=True, **kwargs):
        """Initialize BQJob.
        ...
        """
        super(BQJob, self).__init__(http, project_id=project_id, discovery_document_storage=discovery_document_storage, bq_client=bq_client)
    ...
    def run_async(self, **kwargs):
        try:
            job = run_func_with_backoff(self.bq_client.Query, self.query, sync=False, **kwargs)
    ...
    def get_result(self, timeout=sys.maxint):
        ...
        job = bq_client.WaitJob(job_reference, wait=timeout, wait_printer_factory=bq_client.wait_printer_factory)
        if job['status'].get('errorResult') is not None:
            raise BQError(message=job['status']['errorResult']['message'], error=None)
        if self.verbose:
            self._print_verbose(job)
        bqtable = BQTable(self.http, bq_client=bq_client, table_dict=job['configuration']['query']['destinationTable'])
    ...
    def _print_verbose(self, job_dict):
        ...
        logging.info(log_format.format(project_id=job_dict['jobReference']['projectId'], job_id=job_dict['jobReference']['jobId'], query=job_dict['configuration']['query']['query'], total_bytes=job_dict['statistics']['query']['totalBytesProcessed'], cache_hit=job_dict['statistics']['query'].get('cacheHit', 'False')))
...
class BQTable(BaseBQ):
    ...
    def __init__(self, http, project_id=None, dataset_id=None, table_id=None, table_dict=None, discovery_document_storage=None, bq_client=None, **kwargs):
        """Initialize BQTable.

        Required keywords:
            http: oauth2-authorized HTTP object
        """
        super(BQTable, self).__init__(http, discovery_document_storage=discovery_document_storage, bq_client=bq_client)
        if table_dict is None:
            table_dict = {'projectId': project_id, 'datasetId': dataset_id, 
               'tableId': table_id}
    ...
    def get_info(self):
        ...
        fqtn = BQHelper.build_fully_qualified_table_name(table_dict=self.table_dict, with_bracket=False)
    ...
    def read_rows(self):
        ...
        rows = self.bq_client.ReadTableRows(table_dict)
        results = []
        for row in rows:
            result = {}
...
class BQHelper(object):
    ...
    def is_gae_runtime():
        ...
        if server_software is not None:
            num = '\\d+'
            gae = 'Google App Engine/%s\\.%s\\.%s' % (num, num, num)
            gae_dev = 'Development/%s\\.%s' % (num, num)
            return bool(re.match(gae, server_software) or re.match(gae_dev, server_software))
        return False
        return

    @staticmethod
    def retrieve_discovery_document(storage=None):
        """ Retrieve discovery document for BigQuery
        ...
        if storage is not None and hasattr(storage, 'set'):
            storage.set(document_key, body, time=86400)
        return body
    ...
    def build_apiclient(discovery_document, http):
        ...
        return discovery.build_from_document(discovery_document, http=http, model=bigquery_model, requestBuilder=BigqueryHttp.Factory(bigquery_model))
    ...
    def convert_type(field_type, value):
        ...
        supported_field_type = ('STRING', 'INTEGER', 'FLOAT', 'BOOLEAN', 'TIMESTAMP')
        if field_type not in supported_field_type:
            raise ValueError
    ...
    @staticmethod
    def build_fully_qualified_table_name(project_id=None, dataset_id=None, table_id=None, table_dict=None, with_bracket=True):
