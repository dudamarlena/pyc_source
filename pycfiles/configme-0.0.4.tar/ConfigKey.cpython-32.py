...
# Embedded file name: build/bdist.linux-x86_64/egg/configmaster/ConfigKey.py
    ...
    def load_from_dict(self, data: dict, overwrite: bool=True):
        ...
        if data is None or data == {}:
            ...
            if isinstance(data, list) or isinstance(data, tuple):
                ...
                if isinstance(data, dict) or isinstance(data, self.__class__):
                    items = data.items()
                else:
            ...
            for key, item in items:
                ...
                if self.safe_load:
                    if key.startswith('__') or key in ('dump', 'items', 'keys', 'values',
                                                       'iter_list', 'load_from_dict',
                                                       'iter_list_dump', 'parsed',
                                                       'safe_load'):
                        key = 'unsafe_' + key
                if '.' in key:
                    key = key.replace('.', '_')
