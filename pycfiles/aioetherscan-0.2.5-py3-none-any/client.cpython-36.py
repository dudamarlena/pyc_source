...
# Embedded file name: build/bdist.linux-x86_64/egg/aioetcd3/client.py
...
class Client(KV, Lease, Auth, Watch, Maintenance, Cluster):
    ...
    def __init__(self, endpoint, ssl=False, ca_cert=None, cert_key=None, cert_cert=None, default_ca=False, grpc_options=None, timeout=5, *, loop=None, executor=None):
        channel = self._create_grpc_channel(endpoint=endpoint, ssl=ssl, ca_cert=ca_cert,
          cert_key=cert_key,
          cert_cert=cert_cert,
    ...
    def _create_grpc_channel(self, endpoint, ssl=False, ca_cert=None, cert_key=None, cert_cert=None, default_ca=False, options=None, *, loop=None, executor=None):
        credentials = None
        if not ssl:
            channel = aiogrpc.insecure_channel(endpoint, options=options, loop=loop, executor=executor, standalone_pool_for_streaming=True)
        else:
            if default_ca:
                ca_cert = None
            else:
                ...
                channel = aiogrpc.secure_channel(endpoint, credentials, options=options, loop=loop,
                  executor=executor,
    ...
    def _recreate_grpc_channel(self, endpoint):
        if self._credentials:
            channel = aiogrpc.secure_channel(endpoint, (self._credentials), options=(self._options), loop=(self._loop),
              executor=(self._executor),
              standalone_pool_for_streaming=True)
        else:
            channel = aiogrpc.insecure_channel(endpoint, options=(self._options), loop=(self._loop), executor=(self._executor),
              standalone_pool_for_streaming=True)
        return channel
...
def ssl_client(endpoint, ca_file=None, cert_file=None, key_file=None, default_ca=False, grpc_options=None, timeout=None):
    ca, key, cert = get_secure_creds(ca_cert=ca_file, cert_cert=cert_file, cert_key=key_file)
    return Client(endpoint, ssl=True, ca_cert=ca, cert_key=key, cert_cert=cert, default_ca=default_ca,
...
def set_grpc_cipher(enable_rsa=True, enable_ecdsa=True, ciphers=None):
    ...
    if ciphers:
        ...
        rsa_ciphers = 'ECDHE-RSA-AES128-GCM-SHA256:ECDHE-RSA-AES128-SHA256:ECDHE-RSA-AES256-SHA384:ECDHE-RSA-AES256-GCM-SHA384'
