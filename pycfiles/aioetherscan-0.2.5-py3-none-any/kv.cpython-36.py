...
# Embedded file name: build/bdist.linux-x86_64/egg/aioetcd3/kv.py
...
import functools, aioetcd3._etcdv3.rpc_pb2_grpc as stub
...
def _kv(request_builder, response_builder, method):
    ...
    def _decorator(f):
        ...
        def txn(*args, timeout=_default_timeout, **kwargs):
            ...
            return (
             request_builder(**call_args), response_builder(**call_args))
...
def _create_txn_response_builder(success, fail, **kwargs):
    ...
    def _response_builder(response):
        if response.succeeded:
            ...
            return (
             False, [t[1](_get_op_response(r)) for t, r in zip(fail, response.responses)])
...
def _put_request(key, value, lease=None, prev_kv=False, ignore_value=False, ignore_lease=False):
    if lease is None:
        ...
        if hasattr(lease, 'id'):
            lease = lease.id
        put_request = rpc.PutRequest(key=(to_bytes(key)), prev_kv=prev_kv,
...
def _put_response(response, prev_kv=False, **kwargs):
    if prev_kv:
        if response.HasField('prev_kv'):
            return (
             response.prev_kv.value, KVMetadata(response.prev_kv))
    return (None, None)
...
def _create_op_request(request):
    if isinstance(request, rpc.PutRequest):
        ...
        if isinstance(request, rpc.TxnRequest):
...
class KV(StubMixin):
    ...
    async def range(self, key_range, limit=None, revision=None, timeout=_default_timeout, sort_order=None, sort_target='key', serializable=None, keys_only=None, count_only=None, min_mod_revision=None, max_mod_revision=None, min_create_revision=None, max_create_revision=None):
        pass

    @_kv(functools.partial(_range_request, count_only=True), _static_builder(lambda r: r.count), lambda x: x._kv_stub.Range)
    async def count(self, key_range, revision=None, timeout=_default_timeout, min_mod_revision=None, max_mod_revision=None, min_create_revision=None, max_create_revision=None):
        pass

    @_kv(functools.partial(_range_request, keys_only=True), _static_builder(_range_keys_response), lambda x: x._kv_stub.Range)
    async def range_keys(self, key_range, limit=None, revison=None, sort_order=None, sort_target='key', timeout=_default_timeout, serializable=None, count_only=None, min_mod_revision=None, max_mod_revision=None, min_create_revision=None, max_create_revision=None):
        pass

    @_kv(_range_request, _static_builder(_get_response), lambda x: x._kv_stub.Range)
    async def get(self, key_range, revision=None, timeout=_default_timeout, serializable=None, min_mod_revision=None, max_mod_revision=None, min_create_revision=None, max_create_revision=None):
        pass
    ...
    @_kv(functools.partial(_delete_request, prev_kv=True), _partial_builder(functools.partial(_delete_response, prev_kv=True)), lambda x: x._kv_stub.DeleteRange)
    ...
    async def compact(self, revision, physical=False, *, timeout=_default_timeout):
        ...
        """
        await self.grpc_call((self._kv_stub.Compact), rpc.CompactionRequest(revision=revision, physical=physical),
          timeout=timeout)