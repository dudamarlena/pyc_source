...
# [GCC 8.4.0]
# Embedded file name: /home/machin/.virtualenvs/twine/lib/python2.7/site-packages/gerritclient/tests/unit/cli/test_server.py
# Compiled at: 2017-07-26 02:15:48
import mock
...
class TestConfigServerCommand(clibase.BaseCLITest):
    ...
    def test_server_configuration_download_json(self, m_dump):
        ...
        args = ('server configuration download -f {} -d {}').format(file_format, directory)
        expected_path = ('{directory}/configuration.{file_format}').format(directory=directory, file_format=file_format)
    ...
    def test_server_configuration_download_yaml(self, m_safe_dump):
        ...
        with mock.patch('gerritclient.commands.server.open', m_open, create=True):
            self.exec_command(args)
        m_open.assert_called_once_with(expected_path, 'w')
        m_safe_dump.assert_called_once_with(test_data, mock.ANY, default_flow_style=False)
        self.m_get_client.assert_called_once_with('server', mock.ANY)
        self.m_client.get_config.assert_called_once_with()
    ...
    def test_capabilities_download_json(self, m_dump):
        ...
        args = ('server capabilities download -f {} -d {}').format(file_format, directory)
        expected_path = ('{directory}/capabilities.{file_format}').format(directory=directory, file_format=file_format)
    ...
    def test_server_cache_flush_several(self):
        ...
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('server', mock.ANY)
        self.m_client.flush_caches.assert_called_once_with(is_all=False, names=fake_caches)
    ...
    def test_server_cache_flush_wo_params_fail(self, m_stderr):
        ...
        self.assertRaises(SystemExit, self.exec_command, args)
        self.assertIn('error: one of the arguments', m_stderr.write.call_args_list[(-1)][0][0])
