...
# [GCC 8.4.0]
# Embedded file name: /home/machin/.virtualenvs/twine/lib/python2.7/site-packages/gerritclient/tests/utils/fake_server.py
# Compiled at: 2017-07-25 02:27:41
...
def get_fake_config():
    ...
    return {'auth': {'auth_type': 'LDAP', 
                ...
                'editable_account_fields': [
                                          'FULL_NAME',
                                          'REGISTER_NEW_EMAIL']}, 
       'download': {'schemes': {'anonymous http': {'url': 'http://gerrithost:8080/${project}', 
                                                   'commands': {'Checkout': 'git fetch http://gerrithost:8080/${project} ${ref} \\u0026\\u0026 git checkout FETCH_HEAD', 
                                                   ...
                                                   'clone_commands': {'Clone': 'git clone http://gerrithost:8080/${project}', 
                                ...
                                'http': {'url': 'http://jdoe@gerrithost:8080/${project}', 
                                         'is_auth_required': True, 
                                         'is_auth_supported': True, 
                                         'commands': {'Checkout': 'git fetch http://jdoe@gerrithost:8080/${project} ${ref} \\u0026\\u0026 git checkout FETCH_HEAD', 
                                         ...
                                         'clone_commands': {'Clone': 'git clone http://jdoe@gerrithost:8080/${project}', 
                                ...
                                'ssh': {'url': 'ssh://jdoe@gerrithost:29418/${project}', 
                                        'is_auth_required': True, 
                                        'is_auth_supported': True, 
                                        'commands': {'Checkout': 'git fetch ssh://jdoe@gerrithost:29418/${project} ${ref} \\u0026\\u0026 git checkout FETCH_HEAD', 
                                        ...
                                        'clone_commands': {'Clone': 'git clone ssh://jdoe@gerrithost:29418/${project}', 
                    ...
                    'archives': [
                               ...
                               'tbz2',
                               'txz']}, 
       'gerrit': {'all_projects': 'All-Projects', 
                  'all_users': 'All-Users', 
                  'doc_search': True, 
                  'web_uis': [
                            'gwt']}, 
...
def get_fake_capabilities():
    ...
    Gerrit Code Review environment.
    """
    return {'accessDatabase': {'id': 'accessDatabase', 
                          'name': 'Access Database'}, 
       'administrateServer': {'id': 'administrateServer', 
       ...
       'createProject': {'id': 'createProject', 
                         'name': 'Create Project'}, 
       'emailReviewers': {'id': 'emailReviewers', 
                          'name': 'Email Reviewers'}, 
       'flushCaches': {'id': 'flushCaches', 
                       'name': 'Flush Caches'}, 
       'killTask': {'id': 'killTask', 
                    'name': 'Kill Task'}, 
       'priority': {'id': 'priority', 
                    'name': 'Priority'}, 
       'queryLimit': {'id': 'queryLimit', 
                      'name': 'Query Limit'}, 
       'runGC': {'id': 'runGC', 
                 'name': 'Run Garbage Collection'}, 
       'streamEvents': {'id': 'streamEvents', 
                        'name': 'Stream Events'}, 
       'viewCaches': {'id': 'viewCaches', 
                      'name': 'View Caches'}, 
       'viewConnections': {'id': 'viewConnections', 
                           'name': 'View Connections'}, 
       'viewPlugins': {'id': 'viewPlugins', 
                       'name': 'View Plugins'}, 
       'viewQueue': {'id': 'viewQueue', 
                     'name': 'View Queue'}}


def get_fake_cache_info(name='fake_cache', cache_type=None, is_single_item=True):
    """Creates a fake cache info data."""
    fake_cache = {'type': cache_type or 'MEM', 
       'entries': {'mem': 4}, 
       'average_get': '2.5ms', 
       'hit_ratio': {'mem': 94}}
    if is_single_item:
...
def get_fake_caches_info(caches_count):
    ...
    for i in range(1, caches_count + 1):
        fake_caches.update(get_fake_cache_info(name=('fake-cache-{}').format(i), is_single_item=False))
...
def get_fake_summary_state():
    ...
    return {'task_summary': {'total': 1, 
                        'sleeping': 1}, 
       'mem_summary': {'total': '495.50m', 
                       'used': '220.50m', 
                       'free': '275.00m', 
                       'buffers': '0.00k', 
                       'max': '3.45g'}, 
       'thread_summary': {'cpus': 2, 
                          'threads': 60, 
                          'counts': {'SshCommandStart': {'WAITING': 2}, 
                                     'sshd-SshServer': {'TIMED_WAITING': 1, 
                                                        'WAITING': 3}, 
                                     'HTTP': {'TIMED_WAITING': 2, 
                                              'RUNNABLE': 3}, 
                                     'Other': {'TIMED_WAITING': 17, 
                                               'WAITING': 10, 
                                               'RUNNABLE': 2}}}, 
