...
# [GCC 8.4.0]
# Embedded file name: /home/machin/.virtualenvs/twine/lib/python2.7/site-packages/gerritclient/tests/unit/cli/test_change.py
# Compiled at: 2017-08-16 05:55:35
import json, mock
...
class TestChangeCommand(clibase.BaseCLITest):
    ...
    def test_change_list_w_single_query(self):
        ...
        self.m_client.get_all.assert_called_once_with(query=query, options=None, limit=None, skip=None)
        return
    ...
    def test_change_list_w_skip(self):
        ...
        args = ('change list {query} --skip {skip} --max-width 110').format(query=('').join(query), skip=skip)
        self.m_client.get_all.return_value = fake_change.get_fake_changes(5)
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('change', mock.ANY)
        self.m_client.get_all.assert_called_once_with(query=query, options=None, limit=None, skip=skip)
        return

    def test_change_list_w_limit(self):
        limit = 2
        query = ['status:open+is:watched']
        args = ('change list {query} --limit {limit} --max-width 110').format(query=('').join(query), limit=limit)
        self.m_client.get_all.return_value = fake_change.get_fake_changes(2)
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('change', mock.ANY)
        self.m_client.get_all.assert_called_once_with(query=query, options=None, limit=limit, skip=None)
        return

    def test_change_list_w_options(self):
        options = ['LABELS', 'MESSAGES', 'REVIEWED']
        query = ['status:open+is:watched']
        args = ('change list {query} --option {option} --max-width 110').format(query=('').join(query), option=(' ').join(options))
        self.m_client.get_all.return_value = fake_change.get_fake_changes(2)
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('change', mock.ANY)
        self.m_client.get_all.assert_called_once_with(query=query, options=options, limit=None, skip=None)
        return

    def test_change_show_wo_details(self):
        change_id = 'I8473b95934b5732ac55d26311a706c9c2bde9940'
        args = ('change show {change_id} --max-width 110').format(change_id=change_id)
        self.m_client.get_by_id.return_value = fake_change.get_fake_change(identifier=change_id)
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('change', mock.ANY)
        self.m_client.get_by_id.assert_called_once_with(change_id=change_id, detailed=False, options=None)
        return

    def test_change_show_w_details(self):
        change_id = 'I8473b95934b5732ac55d26311a706c9c2bde9940'
        args = ('change show {change_id} --all --max-width 110').format(change_id=change_id)
        self.m_client.get_by_id.return_value = fake_change.get_fake_change(identifier=change_id)
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('change', mock.ANY)
        self.m_client.get_by_id.assert_called_once_with(change_id=change_id, detailed=True, options=None)
        return

    def test_change_show_w_options(self):
        change_id = 'I8473b95934b5732ac55d26311a706c9c2bde9940'
        options = ['LABELS', 'MESSAGES', 'REVIEWED']
        args = ('change show {change_id} --option {options} --max-width 110').format(change_id=change_id, options=(' ').join(options))
        self.m_client.get_by_id.return_value = fake_change.get_fake_change(identifier=change_id)
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('change', mock.ANY)
        self.m_client.get_by_id.assert_called_once_with(change_id=change_id, detailed=False, options=options)

    @mock.patch('gerritclient.common.utils.file_exists', mock.Mock(return_value=True))
    def test_change_create(self):
        test_data = {'project': 'myProject', 'subject': 'Fake subject', 
           'branch': 'master', 
    ...
    def test_change_revert(self):
        ...
        change_id = 'I8473b95934b5732ac55d26311a706c9c2bde9940'
        args = ('change revert {change_id} --message "{message}"').format(change_id=change_id, message=message)
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('change', mock.ANY)
    ...
    def test_change_rebase_w_parent(self):
        ...
        change_id = 'I8473b95934b5732ac55d26311a706c9c2bde9940'
        args = ('change rebase {change_id} --parent {base} ').format(change_id=change_id, base=base)
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('change', mock.ANY)
    ...
    def test_change_move(self):
        ...
        branch = 'fake-branch'
        change_id = 'I8473b95934b5732ac55d26311a706c9c2bde9940'
        args = ('change move {change_id} -b {branch} -m "{message}"').format(change_id=change_id, branch=branch, message=message)
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('change', mock.ANY)
    ...
    def test_change_submit_wo_parameters(self):
        ...
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('change', mock.ANY)
        self.m_client.submit.assert_called_once_with(change_id, on_behalf_of=None, notify='ALL')
        return
    ...
    def test_change_submit_w_parameters(self):
        ...
        username = 'jdoe'
        change_id = 'I8473b95934b5732ac55d26311a706c9c2bde9940'
        args = ('change submit {change_id} --on-behalf-of {username} --notify {notify}').format(change_id=change_id, username=username, notify=notify)
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('change', mock.ANY)
        self.m_client.submit.assert_called_once_with(change_id, on_behalf_of=username, notify=notify)
    ...
    def test_change_topic_set(self):
        ...
        change_id = 'I8473b95934b5732ac55d26311a706c9c2bde9940'
        args = ('change topic set {change_id} --topic "{topic}"').format(change_id=change_id, topic=topic)
    ...
    def test_change_assignee_history_show(self):
        change_id = 'I8473b95934b5732ac55d26311a706c9c2bde9940'
        args = ('change assignee history show {change_id}').format(change_id=change_id)
    ...
    def test_change_assignee_set(self):
        account = {'_account_id': 26071983, 'name': 'John Doe', 
           'username': 'jdoe', 
           'email': 'jdoe@example.com'}
        change_id = 'I8473b95934b5732ac55d26311a706c9c2bde9940'
        args = ('change assignee set {change_id} --account {account}').format(change_id=change_id, account=account['username'])
        account = fake_account.get_fake_account(**account)
        self.m_client.set_assignee.return_value = account
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('change', mock.ANY)
        self.m_client.set_assignee.assert_called_once_with(change_id, account['username'])
    ...
    def test_change_included_get(self):
        change_id = 'I8473b95934b5732ac55d26311a706c9c2bde9940'
        args = ('change included-in show {change_id}').format(change_id=change_id)
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('change', mock.ANY)
    ...
    def test_change_comments_list(self):
        change_id = 'I8473b95934b5732ac55d26311a706c9c2bde9940'
        args = ('change comment list {change_id} --max-width 110').format(change_id=change_id)
    ...
    def test_change_draft_comments_list(self):
        change_id = 'I8473b95934b5732ac55d26311a706c9c2bde9940'
        args = ('change comment list {change_id} --type drafts --max-width 110').format(change_id=change_id)
        fake_comments = fake_comment.get_fake_comments_in_change(3)
        self.m_client.get_comments.return_value = fake_comments
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('change', mock.ANY)
        self.m_client.get_comments.assert_called_once_with(change_id, comment_type='drafts')

    def test_change_robotcomments_list(self):
        change_id = 'I8473b95934b5732ac55d26311a706c9c2bde9940'
        args = ('change comment list {change_id} --type robotcomments --max-width 110').format(change_id=change_id)
        fake_comments = fake_comment.get_fake_comments_in_change(3)
        self.m_client.get_comments.return_value = fake_comments
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('change', mock.ANY)
        self.m_client.get_comments.assert_called_once_with(change_id, comment_type='robotcomments')

    @mock.patch('sys.stderr')
    def test_change_comments_list_w_wrong_type_fail(self, mocked_stderr):
        change_id = 'I8473b95934b5732ac55d26311a706c9c2bde9940'
        args = ('change comment list {change_id} --type bad_comment').format(change_id=change_id)
        self.assertRaises(SystemExit, self.exec_command, args)
        self.assertIn("invalid choice: 'bad_comment'", mocked_stderr.write.call_args_list[(-1)][0][0])

    def test_change_consistency_check(self):
        change_id = 'I8473b95934b5732ac55d26311a706c9c2bde9940'
        args = ('change check {change_id} --max-width 110').format(change_id=change_id)
    ...
    def test_change_consistency_check_and_fix(self):
        change_id = 'I8473b95934b5732ac55d26311a706c9c2bde9940'
        args = ('change fix {change_id} --max-width 110').format(change_id=change_id)
        change = fake_change.get_fake_change(identifier=change_id)
        self.m_client.fix_consistency.return_value = change
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('change', mock.ANY)
        self.m_client.fix_consistency.assert_called_once_with(change_id, is_delete=False, expect_merged_as=False)

    def test_change_consistency_check_and_fix_w_parameters(self):
        change_id = 'I8473b95934b5732ac55d26311a706c9c2bde9940'
        args = ('change fix {change_id} --delete-patchset --expect-merged-as --max-width 110').format(change_id=change_id)
        change = fake_change.get_fake_change(identifier=change_id)
        self.m_client.fix_consistency.return_value = change
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('change', mock.ANY)
        self.m_client.fix_consistency.assert_called_once_with(change_id, is_delete=True, expect_merged_as=True)