...
# [GCC 8.4.0]
# Embedded file name: /home/machin/.virtualenvs/twine/lib/python2.7/site-packages/gerritclient/v1/account.py
...
class AccountClient(base.BaseV1ClientCreateEntity):
    ...
    def get_all(self, query, suggested=False, limit=None, skip=None, detailed=False, all_emails=False):
        ...
        params = {k:v for k, v in (('n', limit),
         (
          'S', skip),
         (
          'o', option)) if v is not None if v is not None}
        request_path = ('{api_path}{suggest}{query}').format(api_path=self.api_path, suggest='?suggest&' if suggested else '?', query=('q={query}').format(query=query))
    ...
    def get_by_id(self, account_id, detailed=False):
        ...
        """
        request_path = ('{api_path}{account_id}/{detail}').format(api_path=self.api_path, account_id=account_id, detail='detail' if detailed else '')
    ...
    def set_name(self, account_id, name):
        ...
        """
        data = {'name': name}
        request_path = ('{api_path}{account_id}/name').format(api_path=self.api_path, account_id=account_id)
    ...
    def set_username(self, account_id, username):
        ...
        request_path = ('{api_path}{account_id}/username').format(api_path=self.api_path, account_id=account_id)
        return self.connection.put_request(request_path, json_data=data)

    def is_active(self, account_id):
        """Check the status of an account in Gerrit."""
        request_path = ('{api_path}{account_id}/active').format(api_path=self.api_path, account_id=account_id)
    ...
    def set_password(self, account_id, password=None, generate=False):
        ...
        data = {k:v for k, v in (
         (
          'generate', generate),
         (
          'http_password', password)) if v is not None if v is not None}
        request_path = ('{api_path}{account_id}/password.http').format(api_path=self.api_path, account_id=account_id)
        return self.connection.put_request(request_path, json_data=data)
    ...
    def get_ssh_keys(self, account_id):
        ...
        request_path = ('{api_path}{account_id}/sshkeys').format(api_path=self.api_path, account_id=account_id)
        return self.connection.get_request(request_path)
    ...
    def get_ssh_key(self, account_id, sequence_id):
        ...
        """
        request_path = ('{api_path}{account_id}/sshkeys/{sequence_id}').format(api_path=self.api_path, account_id=account_id, sequence_id=sequence_id)
        return self.connection.get_request(request_path)

    def add_ssh_key(self, account_id, ssh_key):
        """Add an SSH key for a user."""
        request_path = ('{api_path}{account_id}/sshkeys').format(api_path=self.api_path, account_id=account_id)
        return self.connection.post_request(request_path, data=ssh_key, content_type='plain/text')

    def delete_ssh_key(self, account_id, ssh_key_id):
        """Delete an SSH key of a user."""
        request_path = ('{api_path}{account_id}/sshkeys/{ssh_key_id}').format(api_path=self.api_path, account_id=account_id, ssh_key_id=ssh_key_id)
        return self.connection.delete_request(request_path)

    def get_membership(self, account_id):
        """Lists all groups that contain the specified user as a member."""
        request_path = ('{api_path}{account_id}/groups').format(api_path=self.api_path, account_id=account_id)
        return self.connection.get_request(request_path)
    ...
    def add_email(self, account_id, email, preferred=False, no_confirmation=False):
        ...
        """
        data = {'email': email, 'preferred': preferred, 
           'no_confirmation': no_confirmation}
        request_path = ('{api_path}{account_id}/emails/{email}').format(api_path=self.api_path, account_id=account_id, email=email)
        return self.connection.put_request(request_path, json_data=data)
    ...
    def set_preferred_email(self, account_id, email):
        ...
        request_path = ('{api}{account_id}/emails/{email}/preferred').format(api=self.api_path, account_id=account_id, email=email)
        return self.connection.put_request(request_path, json_data={})
