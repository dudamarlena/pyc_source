...
# [GCC 8.4.0]
# Embedded file name: /home/machin/.virtualenvs/twine/lib/python2.7/site-packages/gerritclient/tests/unit/cli/test_account.py
# Compiled at: 2017-06-02 15:59:48
import json, mock
...
class TestAccountCommand(clibase.BaseCLITest):
    ...
    def exec_list_command(self, cmd, **kwargs):
        ...
        self.m_client.get_all.assert_called_once_with(query, suggested=kwargs.get('suggested', False), limit=kwargs.get('limit'), skip=kwargs.get('skip'), detailed=kwargs.get('detailed', False), all_emails=kwargs.get('all_emails', False))
    ...
    def test_account_list_w_details_all_emails_limit_skip(self):
        skip = 5
        limit = 10
        args = ('account list --all --all-emails --limit {0} --skip {1}').format(limit, skip)
        self.exec_list_command(args, detailed=True, all_emails=True, limit=limit, skip=skip)
    ...
    def test_account_show_fail(self, mocked_stderr):
        ...
        self.assertIn('account show: error:', mocked_stderr.write.call_args_list[(-1)][0][0])
    ...
    def test_account_create_w_parameters_from_file(self):
        username = 'fake-user'
        test_data = {'username': username, 'name': 'Fake User', 
           'groups': [
                    'Fake Group']}
        expected_path = '/tmp/fakes/fake-account.yaml'
        args = ('account create {0} --file {1}').format(username, expected_path)
    ...
    def test_account_create_fail(self, mocked_stderr):
        args = 'account create'
        self.assertRaises(SystemExit, self.exec_command, args)
        self.assertIn('account create: error:', mocked_stderr.write.call_args_list[(-1)][0][0])
    ...
    def test_account_set_username(self):
        ...
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('account', mock.ANY)
        self.m_client.set_username.assert_called_once_with(account_id, username=username)
    ...
    def test_account_set_password(self):
        ...
        args = ('account password set {0} --password {1}').format(account_id, password)
    ...
    def test_account_set_empty_password(self):
        account_id = '69'
        empty_password = ''
        args = ('account password set {0} --password "{1}"').format(account_id, empty_password)
        self.m_client.set_password.return_value = empty_password
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('account', mock.ANY)
        self.m_client.set_password.assert_called_once_with(account_id, empty_password, False)
    ...
    def test_account_ssh_key_show(self):
        account_id = '69'
        sequence_id = 71
        args = ('account ssh-key show {0} --sequence-id {1}').format(account_id, sequence_id)
    ...
    def test_account_ssh_key_show_fail(self, mocked_stderr):
        ...
        self.assertRaises(SystemExit, self.exec_command, args)
        self.assertIn('ssh-key show: error:', mocked_stderr.write.call_args_list[(-1)][0][0])

    def test_account_ssh_key_add(self):
        account_id = '69'
        ssh_key = 'ssh-rsa AAAAB3NzaC1yc2EAAAABIwAAAQEA0T...YImydZAw\\u003d\\u003d john.doe@example.com'
        args = ('account ssh-key add {0} --ssh-key "{1}"').format(account_id, ssh_key)
    ...
    def test_account_ssh_key_add_from_file(self):
        account_id = '69'
        test_data = 'ssh-rsa AAAAB3NzaC1yc2EAAAABIwAAAQEA0T...YImydZAw\\u003d\\u003d john.doe@example.com'
        expected_path = '/tmp/fakes/fake-ssh-key.pub'
        args = ('account ssh-key add {0} --file {1}').format(account_id, expected_path)
        fake_ssh_key_info = fake_sshkeyinfo.get_fake_ssh_key_info(1)
        self.m_client.add_ssh_key.return_value = fake_ssh_key_info
    ...
    @mock.patch('gerritclient.common.utils.file_exists', mock.Mock(return_value=True))
    @mock.patch('sys.stderr')
    def test_account_ssh_key_add_fail_with_mutually_exclusive_params(self, mocked_stderr):
        account_id = '69'
        expected_path = '/tmp/fakes/fake-ssh-key.pub'
        ssh_key = 'ssh-rsa AAAAB3NzaC1yc2EAAAABIwAAAQEA0T...YImydZAw\\u003d\\u003d john.doe@example.com'
        args = ('account ssh-key add {0} --ssh-key "{1}" --file {2} ').format(account_id, ssh_key, expected_path)
        self.assertRaises(SystemExit, self.exec_command, args)
    ...
    def test_account_ssh_key_delete(self):
        account_id = '69'
        sequence_id = 71
        args = ('account ssh-key delete {0} --sequence-id {1}').format(account_id, sequence_id)
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('account', mock.ANY)
    ...
    def test_account_email_add(self):
        ...
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('account', mock.ANY)
        self.m_client.add_email.assert_called_once_with(account_id, email, no_confirmation=False, preferred=False)
    ...
    def test_account_email_set_as_preferred(self):
        ...
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('account', mock.ANY)
        self.m_client.set_preferred_email.assert_called_once_with(account_id, email=email)