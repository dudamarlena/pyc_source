...
# [GCC 8.4.0]
# Embedded file name: /home/machin/.virtualenvs/twine/lib/python2.7/site-packages/gerritclient/tests/unit/cli/test_group.py
# Compiled at: 2017-06-30 13:28:31
import json, mock
...
class TestGroupCommand(clibase.BaseCLITest):
    ...
    def test_group_create_w_parameters_from_file(self):
        group_name = 'Fake-Group'
        test_data = {'name': group_name, 'description': 'Fake Group description', 
           'owner_id': 'Administrators', 
           'owners': 'admin'}
        expected_path = '/tmp/fakes/fake-group.yaml'
        args = ('group create {0} --file {1}').format(group_name, expected_path)
    ...
    def test_group_project_fail(self, mocked_stderr):
        ...
        self.assertIn('group create: error:', mocked_stderr.write.call_args_list[(-1)][0][0])

    def test_group_rename(self):
        group_id = '69'
        new_name = 'New-Fake-Group'
        args = ('group rename {group_id} {new_name}').format(group_id=group_id, new_name=new_name)
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('group', mock.ANY)
    ...
    def test_group_set_options(self):
        group_id = '69'
        args = ('group options set {group_id} --visible').format(group_id=group_id)
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('group', mock.ANY)
    ...
    def test_group_set_options_fail(self, mocked_stderr):
        group_id = '69'
        args = ('group options set {group_id} --visible --no-visible').format(group_id=group_id)
        self.assertRaises(SystemExit, self.exec_command, args)
        self.assertIn('not allowed with argument', mocked_stderr.write.call_args_list[(-1)][0][0])

    def test_group_set_owner_group(self):
        group_id = '69'
        owner_group = 'Administrators'
        args = ('group owner set {group_id} {owner_group}').format(group_id=group_id, owner_group=owner_group)
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('group', mock.ANY)
    ...
    def test_group_members_add(self):
        ...
        args = ('group member add {group_id} --account {account_id}').format(group_id=group_id, account_id=(' ').join(accounts_ids))
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('group', mock.ANY)
    ...
    def test_group_members_delete(self):
        group_id = '69'
        accounts_ids = ['1013', '1014', '1015']
        args = ('group member delete {group_id} --account {account_id}').format(group_id=group_id, account_id=(' ').join(accounts_ids))
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('group', mock.ANY)
    ...
    def test_group_include_groups(self):
        ...
        args = ('group include {group_id} --group {groups_ids}').format(group_id=group_id, groups_ids=(' ').join(groups_ids))
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('group', mock.ANY)
    ...
    def test_group_exclude_groups(self):
        group_id = '69'
        groups_ids = ['1013', '1014', '1015']
        args = ('group exclude {group_id} --group {groups_ids}').format(group_id=group_id, groups_ids=(' ').join(groups_ids))
        self.exec_command(args)
        self.m_get_client.assert_called_once_with('group', mock.ANY)
