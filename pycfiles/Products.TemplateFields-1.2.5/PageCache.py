...
from Products.CMFCore.utils import _checkConditionalGET as CMFCheckConditionalGET, _setCacheHeaders
_marker = []
import logging
...
class PageCache:
    ...
    def getCacheReport(self):
        ...
        for oc in self.cache.values():
            ...
            views.sort()
            info = {'template_path': oc.template_path, 'context_path': oc.context_path, 'hits': oc.hits, 'misses': oc.misses, 'size': size, 'counter': ac, 'views': views, 'entries': len(oc.entries)}
            rval.append(info)
    ...
    def get(self, ob, REQUEST, etag, default):
        ...
        try:
            ...
            self.writelock.release()
        return (
         entry.data, copy.copy(entry.headers))
