...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/Polygon/Utils.py
...
__left = lambda p: p[1][0] * p[2][1] + p[0][0] * p[1][1] + p[2][0] * p[0][1] - p[1][0] * p[0][1] - p[2][0] * p[1][1] - p[0][0] * p[2][1] >= 0
...
__vImp = lambda p: (sqrt((p[1][0] - p[0][0]) ** 2 + (p[1][1] - p[0][1]) ** 2) + sqrt((p[2][0] - p[1][0]) ** 2 + (p[2][1] - p[1][1]) ** 2)) * fabs(p[1][0] * p[2][1] + p[0][0] * p[1][1] + p[2][0] * p[0][1] - p[1][0] * p[0][1] - p[2][0] * p[1][1] - p[0][0] * p[2][1])
...
def _find_split(count_dict):
    ...
    return (
     by_index[(index + 1)][0], totals[index])
    ...
    while work:
        ...
        if countx < county and not _single_poly(polygon, 0, minx + 1.0) or _single_poly(polygon, 1, miny + 1.0):
            work.append(polygon & Polygon([(minx, miny), (minx, maxy + 1),
             (
              spx, maxy + 1), (spx, miny)]))
            work.append(polygon & Polygon([(spx, miny), (spx, maxy + 1),
             (
            ...
            work.append(polygon & Polygon([(minx, spy), (minx, maxy + 1),
             (
...
def gpfInfo(fileName):
    ...
    f.close()
    return (
     contours, holes, points, withHoles)