...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/Polygon/IO.py
...
def getWritableObject(ofile):
    ...
    if ofile is None:
        ...
        if type(ofile) in (file, StringIO):
            return (ofile, False)
...
def getReadableObject(ifile):
    ...
    if type(ifile) in StringTypes:
        ...
        if type(ifile) in (file, StringIO):
            return (ifile, False)
...
def writeSVG(ofile, polylist, width=None, height=None, fill_color=None, fill_opacity=None, stroke_color=None, stroke_width=None):
    """
    ...
    """
    f, cl = getWritableObject(ofile)
    pp = [ Polygon(p) for p in polylist ]
    [ p.flop(0.0) for p in pp ]
    ...
    for i in range(npoly):
        p = pp[i]
        bb = bbs[i]
        p.warpToBox(width * (bb[0] - minx) / xdim, width * (bb[1] - minx) / xdim, height * (bb[2] - miny) / ydim, height * (bb[3] - miny) / ydim)
        subl = [
...
def writeXML(ofile, polylist, withHeader=False):
    ...
    for p in polylist:
        l = [
         '<polygon contours="%d" area="%g" xMin="%g" xMax="%g" yMin="%g" yMax="%g">' % ((len(p), p.area()) + p.boundingBox())]
        for i, c in enumerate(p):
