...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pgp/packets/packets.py
...
from pgp.packets.user_attribute_subpackets import user_attribute_subpacket_from_data
from pgp import s2k
from pgp import utils
SYM_ENC_ID_PROTECTED_DATA_PACKET_TYPE = constants.SYMMETRICALLY_ENCRYPTED_AND_INTEGRITY_PROTECTED_DATA_PACKET_TYPE
...
class Packet(object):
    ...
    def __eq__(self, other):
        return self.__class__ == other.__class__ and self.header_format == other.header_format and self.type == other.type

    def __repr__(self):
        return '<{0} at 0x{1:x}>'.format(self.__class__.__name__, id(self))
    ...
    def __bytes__(self):
        ...
        if self.header_format == constants.NEW_PACKET_HEADER_TYPE:
            ...
            while remaining:
                ...
                packet_length_bytes, remaining = utils.new_packet_length_to_bytes(data_length, allow_partial)
            ...
            tag += packet_type << 2
            length_type, length_bytes = utils.old_packet_length_to_bytes(data_length)
            tag += length_type
            result = bytearray([tag])
...
class PublicKeyEncryptedSessionKeyPacket(Packet):
    ...
    def from_packet_content(cls, header_format, type_, data):
        ...
        if key_id == b'0000000000000000':
            key_id = None
        offset += 8
        public_key_algorithm = int(data[offset])
        offset += 1
        encrypted_session_key = data[offset:]
        result = cls(header_format, version, key_id, public_key_algorithm, encrypted_session_key)
        result._content = data
        return result

    def __init__(self, header_format, version, key_id, public_key_algorithm, encrypted_session_key):
        Packet.__init__(self, header_format, constants.PUBLIC_KEY_ENCRYPTED_SESSION_KEY_PACKET_TYPE)
    ...
    def __eq__(self, other):
        return super(PublicKeyEncryptedSessionKeyPacket, self).__eq__(other) and self.version == other.version and self.key_id == other.key_id and self.public_key_algorithm == other.public_key_algorithm and self.encrypted_session_key == other.encrypted_session_key

    @classmethod
    def _get_key_and_cipher_algo(cls, public_key_algorithm, secret_key_obj, encrypted_session_key):
        ...
        while offset < encrypted_session_key_length - 2:
            ...
            offset += mpilen
            em = b'\x00' * (len(sentinel) - len(em)) + em
    ...
    @classmethod
    def _get_encrypted_key(cls, public_key_algorithm, key_obj, symmetric_algorithm, session_key):
        if public_key_algorithm in (1, 2, 3):
        ...
        if public_key_algorithm in (1, 2, 3):
            ...
            values = cipher.encrypt(utils.bytes_to_int(session_key, 0, len(session_key)), k)
...
class SignaturePacket(Packet):
    ...
    def from_packet_content(cls, header_format, type_, data):
        ...
        if version in (2, 3):
            ...
            if version == 4:
                ...
                while offset < subpacket_end:
                    hashed_subpacket, offset = signature_subpacket_from_data(data, offset)
                ...
                unhashed_subpackets = []
                while offset < subpacket_end:
                    unhashed_subpacket, offset = signature_subpacket_from_data(data, offset)
                    unhashed_subpackets.append(unhashed_subpacket)

                hash2 = data[offset:offset + 2]
                offset += 2
            else:
                raise ValueError('Unsupported signature packet, version %d' % version)
        signature_values = []
        data_len = len(data)
        ...
        result = cls(header_format, version, signature_type, public_key_algorithm, hash_algorithm, hash2, signature_values, creation_time, key_id, hashed_subpackets, unhashed_subpackets)
        result._content = data
        return result
    ...
    def __repr__(self):
        return '<{0} 0x{1:02x} ({2}) at 0x{3:x}>'.format(self.__class__.__name__, self.signature_type, self.human_signature_type, id(self))

    def __init__(self, header_format, version, signature_type, public_key_algorithm, hash_algorithm, hash2, signature_values, creation_time=None, key_id=None, hashed_subpackets=None, unhashed_subpackets=None):
    ...
    def __eq__(self, other):
        return super(SignaturePacket, self).__eq__(other) and self.version == other.version and self.signature_type == other.signature_type and self.public_key_algorithm == other.public_key_algorithm and self.hash_algorithm == other.hash_algorithm and self.hash2 == other.hash2 and self.signature_values == other.signature_values and set(self.hashed_subpackets) == set(other.hashed_subpackets) and set(self.unhashed_subpackets) == set(other.unhashed_subpackets)
...
class SymmetricKeyEncryptedSessionKeyPacket(Packet):
    ...
    def from_packet_content(cls, header_format, type_, data):
        ...
        offset += 1
        s2k_specification, offset = s2k.parse_s2k_bytes(symmetric_algorithm, data, offset)
        encrypted_session_key_length = utils.symmetric_cipher_key_lengths.get(symmetric_algorithm)
        encrypted_session_key = data[offset:offset + encrypted_session_key_length + 1]
        result = cls(header_format, version, symmetric_algorithm, s2k_specification, encrypted_session_key)
        result._content = data
        return result

    def __init__(self, header_format, version, symmetric_algorithm, s2k_specification, encrypted_session_key=None):
        Packet.__init__(self, header_format, constants.SYMMETRIC_KEY_ENCRYPTED_SESSION_KEY_PACKET_TYPE)
        self.version = version
    ...
    def __eq__(self, other):
        return super(SymmetricKeyEncryptedSessionKeyPacket, self).__eq__(other) and self.version == other.version and self.symmetric_algorithm == other.symmetric_algorithm and self.s2k_specification == other.s2k_specification and self.encrypted_session_key == other.encrypted_session_key

    def get_key_and_cipher_algo(self, passphrase):
        return self._get_key_and_cipher_algo(self.s2k_specification, self.symmetric_algorithm, passphrase, self.encrypted_session_key)

    @classmethod
    def _get_key_and_cipher_algo(cls, s2k_specification, symmetric_algorithm, passphrase, encrypted_session_key=None):
        key = s2k_specification.to_key(passphrase)
        symmetric_algorithm = symmetric_algorithm
        if encrypted_session_key:
            block_size = utils.symmetric_cipher_block_lengths.get(symmetric_algorithm)
            iv = bytearray([0] * block_size)
            cipher = utils.get_symmetric_cipher(symmetric_algorithm, key, utils.CFB, iv)
    ...
    @classmethod
    def _get_encrypted_key(cls, s2k_specification, symmetric_algo, passphrase, session_key):
        s2k_sym_algo = s2k_specification.symmetric_algorithm
        key = s2k_specification.to_key(passphrase)
        block_size = utils.symmetric_cipher_block_lengths.get(s2k_sym_algo)
        iv = bytearray([0] * block_size)
        cipher = utils.get_symmetric_cipher(s2k_sym_algo, key, utils.CFB, iv)
...
class OnePassSignaturePacket(Packet):
    ...
    def from_packet_content(cls, header_format, type_, data):
        ...
        nested = data[offset]
        offset += 1
        result = cls(header_format, version, signature_type, hash_algorithm, public_key_algorithm, key_id, nested)
        result._content = data
        return result

    def __init__(self, header_format, version, signature_type, hash_algorithm, public_key_algorithm, key_id, nested):
        Packet.__init__(self, header_format, constants.ONE_PASS_SIGNATURE_PACKET_TYPE)
        self.version = version
        self.signature_type = signature_type
        self.hash_algorithm = hash_algorithm
        self.public_key_algorithm = public_key_algorithm
    ...
    def __eq__(self, other):
        return super(OnePassSignaturePacket, self).__eq__(other) and self.signature_type == other.signature_type and self.hash_algorithm == other.hash_algorithm and self.public_key_algorithm == other.public_key_algorithm and self.key_id == other.key_id and self.nested == other.nested
...
class PublicKeyPacket(Packet):
    ...
    def _values_from_packet_contents(cls, data):
        ...
        if version in (2, 3):
            ...
            if public_key_algorithm in (1, 2, 3):
                ...
                if public_key_algorithm in (16, 20):
    ...
    def __init__(self, header_type, version, creation_time, public_key_algorithm, expiration_days=None, modulus=None, exponent=None, prime=None, group_order=None, group_generator=None, key_value=None):
    ...
    def __eq__(self, other):
        return super(PublicKeyPacket, self).__eq__(other) and self.version == other.version and self.creation_time == other.creation_time and self.public_key_algorithm == other.public_key_algorithm and self.expiration_days == other.expiration_days and self.modulus == other.modulus and self.exponent == other.exponent and self.prime == other.prime and self.group_generator == other.group_generator and self.group_order == other.group_order and self.key_value == other.key_value
    ...
    def public_content(self):
        ...
        if self.public_key_algorithm in (1, 2, 3):
            ...
            if self.public_key_algorithm in (16, 20):
                if self.public_key_algorithm == 20:
                    warnings.warn('These are no longer permitted. An implementation MUST NOT generate such keys. An implementation MUST NOT generate Elgamal signatures. These must only be used for test purposes.')
...
class PublicSubkeyPacket(PublicKeyPacket):
    ...
    def __init__(self, header_type, version, creation_time, public_key_algorithm, expiration_days=None, modulus=None, exponent=None, prime=None, group_order=None, group_generator=None, key_value=None):
        Packet.__init__(self, header_type, constants.PUBLIC_SUBKEY_PACKET_TYPE)
        self.version = version
        self.creation_time = creation_time
        self.public_key_algorithm = public_key_algorithm
        self.expiration_days = expiration_days
...
class SecretKeyPacket(PublicKeyPacket):
    ...
    def _values_from_packet_contents(cls, data):
        ...
        if s2k_usage == 0:
            ...
            if s2k_usage not in (0, 101):
                block_length = utils.symmetric_cipher_block_lengths.get(symmetric_algorithm, None)
    ...
    def __init__(self, header_type, version, creation_time, public_key_algorithm, expiration_days=None, modulus=None, exponent=None, prime=None, group_order=None, group_generator=None, key_value=None, s2k_specification=None, symmetric_algorithm=None, iv=None, encrypted_portion=None, checksum=None, hash_=None, passphrase=None, exponent_d=None, prime_p=None, prime_q=None, multiplicative_inverse_u=None, exponent_x=None):
    ...
    def __eq__(self, other):
        return super(SecretKeyPacket, self).__eq__(other) and self.s2k_specification == other.s2k_specification and self.symmetric_algorithm == other.symmetric_algorithm and self.iv == other.iv and self.encrypted_portion == other.encrypted_portion and self.checksum == other.checksum and self.hash == other.hash
    ...
    def decrypt(self, passphrase):
        if self.s2k_specification is not None:
            ...
            key = passphrase
        values = self.decrypt_encrypted_key_portion(self.version, key, self.symmetric_algorithm, self.iv, self.encrypted_portion, self.checksum, self.hash)
        if self.public_key_algorithm in (1, 2, 3):
            ...
            if self.public_key_algorithm in (16, 17, 20):
    ...
    @classmethod
    def encrypt_key_values(cls, version, values, key, symmetric_algorithm, iv, checksum=False, hash_=False):
        cipher = utils.get_symmetric_cipher(symmetric_algorithm, key, utils.OPENPGP, iv, syncable=True)
            ...
            for i in values:
                ...
                if len(mpi_value_bytes) < bytes_required:
                    mpi_value_bytes = bytearray([
                     0] * (bytes_required - len(mpi_value_bytes))) + mpi_value_bytes
                encrypted_data.extend(cipher.encrypt(mpi_value_bytes))

        if checksum:
            checksum = utils.int_to_2byte(sum(unencrypted_data) & 65535)
            if version >= 4:
    ...
    @classmethod
    def decrypt_encrypted_key_portion(cls, version, key, symmetric_algorithm, iv, encrypted_data, checksum=False, hash_=False):
        sym_block_size = utils.symmetric_cipher_block_lengths.get(symmetric_algorithm, 8)
        if version >= 4:
            cipher = utils.get_symmetric_cipher(symmetric_algorithm, key, utils.CFB, iv)
            padding = bytearray([
             0] * (len(encrypted_data) % sym_block_size))
            decrypted_data = cipher.decrypt(bytes(encrypted_data + padding))[:-len(padding)]
        else:
            if version in (2, 3):
                cipher = utils.get_symmetric_cipher(symmetric_algorithm, key, utils.CFB, iv, syncable=True)
                offset = 0
                decrypted_data = bytearray()
                while offset < len(encrypted_data):
                    cipher.sync()
                    decrypted_data.extend(encrypted_data[offset:offset + 2])
                    mpi_length = utils.mpi_length(encrypted_data, offset)
                    offset += 2
                    encrypted_mpi_content = encrypted_data[offset:offset + mpi_length]
                    offset += mpi_length
        ...
        if checksum:
            data_length -= 2
            if version >= 4:
                expected_checksum = utils.short_to_int(cipher.decrypt(utils.int_to_2byte(decrypted_data[-2:])))
            actual_checksum = sum(decrypted_data[:-2]) & 65535
        ...
        if self.encrypted_portion:
            ...
            if self.public_key_algorithm in (1, 2, 3):
                ...
                if self.s2k_specification is not None:
                    ...
                    key = self.passphrase
                encrypted_data = self.encrypt_key_values(self.version, values, key, self.symmetric_algorithm, self.iv, s2k_usage in (0,
                                                                                                                                     255), s2k_usage == 254)
...
class SecretSubkeyPacket(SecretKeyPacket):
    ...
    def __init__(self, header_type, version, creation_time, public_key_algorithm, expiration_days=None, modulus=None, exponent=None, prime=None, group_order=None, group_generator=None, key_value=None, s2k_specification=None, symmetric_algorithm=None, iv=None, encrypted_portion=None, checksum=False, hash_=False, passphrase=None, exponent_d=None, prime_p=None, prime_q=None, multiplicative_inverse_u=None, exponent_x=None):
        if None in (s2k_specification, iv, encrypted_portion):
            raise ValueError
        Packet.__init__(self, header_type, constants.SECRET_SUBKEY_PACKET_TYPE)
        self.version = version
        self.creation_time = creation_time
        self.public_key_algorithm = public_key_algorithm
        self.expiration_days = expiration_days
...
class CompressedDataPacket(Packet):
    ...
    def __init__(self, header_format, compression_algorithm, compressed_data):
        super(CompressedDataPacket, self).__init__(header_format, constants.COMPRESSED_DATA_PACKET_TYPE)
    ...
    def __eq__(self, other):
        return super(CompressedDataPacket, self).__eq__(other) and self.compression_algorithm == other.compression_algorithm and self.compressed_data == other.compressed_data
...
class SymmetricallyEncryptedDataPacket(Packet):
    ...
    def __init__(self, header_format, data):
        Packet.__init__(self, header_format, constants.SYMMETRICALLY_ENCRYPTED_DATA_PACKET_TYPE)
        self.data = data

    def __eq__(self, other):
        return super(SymmetricallyEncryptedDataPacket, self).__eq__(other) and self.data == other.data
...
class MarkerPacket(Packet):
    ...
    def __eq__(self, other):
        return super(MarkerPacket, self).__eq__(other) and self.old_literal == other.old_literal and self.content == other.content
...
class LiteralDataPacket(Packet):
    ...
    def __init__(self, header_format, data_format, filename, time, data):
        Packet.__init__(self, header_format, constants.LITERAL_DATA_PACKET_TYPE)
    ...
    def __eq__(self, other):
        return super(LiteralDataPacket, self).__eq__(other) and self.data_format == other.data_format and self.filename == other.filename and self.time == other.time and self.data == other.data
    ...
    def content(self):
        if self.data_format in (b'l', b'1'):
            ...
            if self.data_format not in (b'b', b't', b'u'):
                raise ValueError
...
class TrustPacket(Packet):
    ...
    def __eq__(self, other):
        return super(TrustPacket, self).__eq__(other) and self.trust_value == other.trust_value and self.sig_cache == other.sig_cache
...
class UserIDPacket(Packet):
    ...
    def __repr__(self):
        return '<{0} {1} at 0x{2:x}>'.format(self.__class__.__name__, repr(self.user_id), id(self))

    def __eq__(self, other):
        return super(UserIDPacket, self).__eq__(other) and self.user_id == other.user_id
...
class OldCommentPacket(Packet):
    ...
    def __init__(self, header_format, comment):
        Packet.__init__(self, header_format, constants.OLD_COMMENT_PACKET_TYPE)
        self.comment = comment

    def __eq__(self, other):
        return super(OldCommentPacket, self).__eq__(other) and self.comment == other.comment

    @property
    def content(self):
        warnings.warn('The comment packet type, 16, only appears in an OpenPGP draft.')
...
class UserAttributePacket(Packet):
    ...
    def from_packet_content(cls, header_format, type_, data):
        ...
        subpackets = []
        while offset < data_len:
            sp, offset = user_attribute_subpacket_from_data(data, offset)
    ...
    def __init__(self, header_format, subpackets):
        Packet.__init__(self, header_format, constants.USER_ATTRIBUTE_PACKET_TYPE)
        self.subpackets = subpackets

    def __eq__(self, other):
        return super(UserAttributePacket, self).__eq__(other) and self.subpackets == other.subpackets
...
class SymmetricallyEncryptedAndIntegrityProtectedDataPacket(Packet):
    ...
    def __init__(self, header_format, version, encrypted_data):
        Packet.__init__(self, header_format, SYM_ENC_ID_PROTECTED_DATA_PACKET_TYPE)
        self.version = version
        self.encrypted_data = encrypted_data

    def __eq__(self, other):
        return super(self.__class__, self).__eq__(other) and self.version == other.version and self.encrypted_data == encrypted_data
...
class ModificationDetectionCodePacket(Packet):
    ...
    def __init__(self, header_format, data):
        Packet.__init__(self, header_format, constants.MODIFICATION_DETECTION_CODE_PACKET_TYPE)
        self.data = data

    def __eq__(self, other):
        return super(ModificationDetectionCodePacket, self).__eq__(other) and self.data == other.data
...
class GpgCommentPacket(OldCommentPacket):
    ...
    def __init__(self, header_format, comment):
        Packet.__init__(self, header_format, constants.GPG_COMMENT_PACKET_TYPE)
        self.comment = comment
...
class GpgControlPacket(Packet):
    ...
    def __init__(self, header_format, session_marker, control, content_data):
        Packet.__init__(self, header_format, constants.GPG_CONTROL_PACKET_TYPE)
        self.session_marker = session_marker
        self.control = control
        self.content_data = content_data

    def __eq__(self, other):
        return super(GpgControlPacket, self).__eq__(other) and self.session_marker == other.session_marker and self.control == other.control and self.content_data == other.content_data
...
PACKET_TYPES = {constants.PUBLIC_KEY_ENCRYPTED_SESSION_KEY_PACKET_TYPE: PublicKeyEncryptedSessionKeyPacket, 
 constants.SIGNATURE_PACKET_TYPE: SignaturePacket, 
 constants.SYMMETRIC_KEY_ENCRYPTED_SESSION_KEY_PACKET_TYPE: SymmetricKeyEncryptedSessionKeyPacket, 
 ...
 constants.SYMMETRICALLY_ENCRYPTED_DATA_PACKET_TYPE: SymmetricallyEncryptedDataPacket, 
 ...
 SYM_ENC_ID_PROTECTED_DATA_PACKET_TYPE: SymmetricallyEncryptedAndIntegrityProtectedDataPacket, 
 constants.MODIFICATION_DETECTION_CODE_PACKET_TYPE: ModificationDetectionCodePacket, 
    ...
    while incomplete:
        ...
        if previous_tag is not None:
            if tag != previous_tag:
                raise ValueError()
        previous_tag = tag
        header_type = constants.NEW_PACKET_HEADER_TYPE if bool(first & 64) else constants.OLD_PACKET_HEADER_TYPE
        if previous_header_type is not None:
            if header_type != previous_header_type:
                raise ValueError()
        if header_type == constants.NEW_PACKET_HEADER_TYPE:
            data_length, incomplete = utils.new_packet_length_from_stream(fh)
        else:
            tag >>= 2
            fh.seek(fh.tell() - 1)
...
def packet_from_packet_data(data, offset=0):
    ...
    while incomplete:
        if not tag:
            tag = data[offset] & 63
            header_type = constants.NEW_PACKET_HEADER_TYPE if bool(data[offset] & 64) else constants.OLD_PACKET_HEADER_TYPE
            if header_type == constants.NEW_PACKET_HEADER_TYPE:
                offset += 1
            else:
                tag >>= 2
        if header_type == constants.NEW_PACKET_HEADER_TYPE:
            offset, data_length, incomplete = utils.new_packet_length(data, offset)
        else:
