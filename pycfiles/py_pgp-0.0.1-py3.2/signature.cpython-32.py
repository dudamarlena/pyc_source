...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pgp/signature.py
# Compiled at: 2015-08-31 08:17:33
import datetime, re, time, weakref
...
class Notation(object):
    ...
    def from_subpacket(cls, subpacket):
        ...
        return cls(subpacket.namespace, subpacket.name, subpacket.value, subpacket.human_readable)

    def to_subpacket(self):
        return signature_subpackets.NotationSubpacket(critical=False, name=self.name, namespace=self.namespace, human_readable=self.is_human_readable(), value=self.value)
...
class RevocationKeyInfo(object):
    ...
    @classmethod
    def from_subpacket(cls, subpacket):
        return cls(subpacket.public_key_algorithm, subpacket.fingerprint, subpacket.sensitive)

    def to_subpacket(self, critical=False):
        return signature_subpackets.RevocationKeySubpacket(critical, self.fingerprint, self.public_key_algorithm, self.sensitive)
...
class BaseSignature(object):
    ...
    def from_packet(cls, packet, target):
        ...
        if version in (2, 3):
            ...
            if version >= 4:
                ...
                raise ValueError
        sig = cls(target, version, signature_type, public_key_algorithm, hash_algorithm, hash2, signature_values, creation_time, issuer_key_id, hashed_subpackets, unhashed_subpackets)
    ...
    def to_packet(self, header_format=None):
        ...
        if self.version >= 4:
            ...
            packet = packets.SignaturePacket(header_format, self.version, self.signature_type, self.public_key_algorithm, self.hash_algorithm, self.hash2, self.signature_values, hashed_subpackets=hashed_subpackets, unhashed_subpackets=unhashed_subpackets)
        elif self.version in (2, 3):
            creation_time = int(time.mktime(self.creation_time.timetuple()))
            packet = packets.SignaturePacket(header_format, self.version, self.signature_type, self.public_key_algorithm, self.hash_algorithm, self.hash2, self.signature_values, creation_time, self.issuer_key_ids[0])
        return packet
    ...
    def to_signable_data(self, signature_type, signature_version=3):
        ...
        if self.version < 4:
            result.extend(utils.int_to_4byte(time.mktime(self.creation_time.timetuple())))
        else:
    ...
    def __repr__(self):
        ...
        if self.validated is True:
            ...
            validation = 'invalid '
        return '<{0} {1} by {2} {3}at 0x{4:x}>'.format(self.__class__.__name__, C.human_signature_types.get(self.signature_type, 'Unknown'), self.issuer_key_ids[0], validation, id(self))

    def __init__(self, target, version, signature_type, public_key_algorithm, hash_algorithm, hash2, signature_values, creation_time=None, issuer_key_id=None, hashed_subpackets=None, unhashed_subpackets=None, current_time_fn=None):
    ...
    def _add_subpacket_values(self, type_, attr, value, hashed=True, critical=False):
        sub = signature_subpackets.SIGNATURE_SUBPACKET_TYPES[type_](**{attr: value,  'critical': critical})
    ...
    def _get_creation_time(self):
        if self.version in (2, 3):
            ...
            value = self._get_subpacket_values(C.CREATION_TIME_SUBPACKET_TYPE, 'time')
    ...
    def _set_creation_time(self, dt):
        ...
        if self.version in (2, 3):
            ...
            self._update_subpacket_values(C.CREATION_TIME_SUBPACKET_TYPE, 'time', value)
    ...
    def _get_issuer_key_ids(self):
        ...
        if self.version >= 4:
            return self._get_subpacket_values(C.ISSUER_KEY_ID_SUBPACKET_TYPE, 'key_id', as_list=True)
    ...
    def _set_issuer_key_ids(self, key_ids):
        ...
        for k in set(key_ids) | set(existing):
            if k in existing and key_ids:
                ...
                self._remove_subpacket_values(C.ISSUER_KEY_ID_SUBPACKET_TYPE, 'key_id', k)
            else:
                self._add_subpacket_values(C.ISSUER_KEY_ID_SUBPACKET_TYPE, 'key_id', k)

    issuer_key_ids = property(_get_issuer_key_ids, _set_issuer_key_ids)

    def _get_signature_expiration_time(self):
        seconds = self._get_subpacket_values(C.EXPIRATION_SECONDS_SUBPACKET_TYPE, 'time')
        if not seconds:
            return None
        else:
    ...
    def _set_signature_expiration_time(self, dt):
        if dt is None:
            ...
            if dt < self.creation_time:
                ...
                self._update_subpacket_values(C.EXPIRATION_SECONDS_SUBPACKET_TYPE, 'time', value)
        return

    signature_expiration_time = property(_get_signature_expiration_time, _set_signature_expiration_time)

    def _get_exportable(self):
        return self._get_subpacket_values(C.EXPORTABLE_SUBPACKET_TYPE, 'exportable')

    def _set_exportable(self, value):
        self._update_subpacket_values(C.EXPORTABLE_SUBPACKET_TYPE, 'exportable', value)

    exportable = property(_get_exportable, _set_exportable)

    def _get_trust_amount(self):
        return self._get_subpacket_values(C.TRUST_SUBPACKET_TYPE, 'trust_amount')

    def _set_trust_amount(self, value):
        self._update_subpacket_values(C.TRUST_SUBPACKET_TYPE, 'trust_amount', value)

    trust_amount = property(_get_trust_amount, _set_trust_amount)

    def _get_trust_depth(self):
        return self._get_subpacket_values(C.TRUST_SUBPACKET_TYPE, 'trust_depth')

    def _set_trust_depth(self, value):
        self._update_subpacket_values(C.TRUST_SUBPACKET_TYPE, 'trust_depth', value)

    trust_depth = property(_get_trust_depth, _set_trust_depth)

    def _get_regular_expressions(self):
        patterns = self._get_subpacket_values(C.REGULAR_EXPRESSION_SUBPACKET_TYPE, 'pattern', as_list=True)
    ...
    def _set_regular_expressions(self, regexes):
        ...
        for k in set(regexes) | set(existing):
            if k in existing and regexes:
                ...
                self._remove_subpacket_values(C.REGULAR_EXPRESSION_SUBPACKET_TYPE, 'pattern', k)
            else:
                self._add_subpacket_values(C.REGULAR_EXPRESSION_SUBPACKET_TYPE, 'pattern', k)

    regular_expressions = property(_get_regular_expressions, _set_regular_expressions)

    def _get_revocable(self):
        return self._get_subpacket_values(C.REVOCABLE_SUBPACKET_TYPE, 'revocable')

    def _set_revocable(self, value):
        self._update_subpacket_values(C.REVOCABLE_SUBPACKET_TYPE, 'revocable', value)
    ...
    def _get_notations(self):
        ...
        return [self.NotationClass.from_subpacket(s) for s in subpackets]
    ...
    def _set_notations(self, notations):
        ...
        for k in set(notations) | set(existing):
            if k in existing and notations:
                ...
                self._remove_subpacket(k.to_subpacket())
            else:
                self._add_subpacket(k.to_subpacket())

    notations = property(_get_notations, _set_notations)

    def _get_issuer_user_id(self):
        return self._get_subpacket_values(C.ISSUERS_USER_ID_SUBPACKET_TYPE, 'user_id')

    def _set_issuer_user_id(self, value):
        self._update_subpacket_values(C.ISSUERS_USER_ID_SUBPACKET_TYPE, 'user_id', value)

    issuer_user_id = property(_get_issuer_user_id, _set_issuer_user_id)

    def _get_revocation_reason(self):
        return self._get_subpacket_values(C.TRUST_SUBPACKET_TYPE, 'revocation_reason')
    ...
    def _get_revocation_code(self):
        return self._get_subpacket_values(C.REVOCATION_REASON_SUBPACKET_TYPE, 'revocation_code')

    def _set_revocation_code(self, value):
        self._update_subpacket_values(C.REVOCATION_REASON_SUBPACKET_TYPE, 'revocation_code', value)
    ...
    def _get_embedded_signatures(self):
        ...
        return [self.__class__.from_packet(s.signature, self) for s in subpackets]
    ...
    def _set_embedded_signatures(self, embedded_signatures):
        ...
        for k in set(embedded_signatures) | set(existing):
            if k in existing and embedded_signatures:
                ...
                self._remove_subpacket(k.to_packet().to_embedded_subpacket())
            else:
                self._add_subpacket(k.to_packet().to_embedded_subpacket())

    embedded_signatures = property(_get_embedded_signatures, _set_embedded_signatures)

    def _get_key_expiration_time(self):
        seconds = self._get_subpacket_values(C.KEY_EXPIRATION_TIME_SUBPACKET_TYPE, 'time')
        if not seconds:
            return seconds
    ...
    def _set_key_expiration_time(self, value):
        ...
        self._update_subpacket_values(C.KEY_EXPIRATION_TIME_SUBPACKET_TYPE, 'time', seconds)

    key_expiration_time = property(_get_key_expiration_time, _set_key_expiration_time)

    def _get_preferred_compression_algorithms(self):
        return self._get_subpacket_values(C.PREFERRED_COMPRESSION_ALGORITHMS_SUBPACKET_TYPE, 'preferred_algorithms')

    def _set_preferred_compression_algorithms(self, value):
        self._update_subpacket_values(C.PREFERRED_COMPRESSION_ALGORITHMS_SUBPACKET_TYPE, 'preferred_algorithms', value)

    preferred_compression_algorithms = property(_get_preferred_compression_algorithms, _set_preferred_compression_algorithms)

    def _get_preferred_hash_algorithms(self):
        return self._get_subpacket_values(C.PREFERRED_HASH_ALGORITHMS_SUBPACKET_TYPE, 'preferred_algorithms')

    def _set_preferred_hash_algorithms(self, value):
        self._update_subpacket_values(C.PREFERRED_HASH_ALGORITHMS_SUBPACKET_TYPE, 'preferred_algorithms', value)

    preferred_hash_algorithms = property(_get_preferred_hash_algorithms, _set_preferred_hash_algorithms)

    def _get_preferred_symmetric_algorithms(self):
        return self._get_subpacket_values(C.PREFERRED_SYMMETRIC_ALGORITHMS_SUBPACKET_TYPE, 'preferred_algorithms')

    def _set_preferred_symmetric_algorithms(self, value):
        self._update_subpacket_values(C.PREFERRED_SYMMETRIC_ALGORITHMS_SUBPACKET_TYPE, 'preferred_algorithms', value)

    preferred_symmetric_algorithms = property(_get_preferred_symmetric_algorithms, _set_preferred_symmetric_algorithms)

    def _get_revocation_keys(self):
        subpackets = self._get_subpackets(C.REVOCATION_KEY_SUBPACKET_TYPE)
        return [self.RevocationKeyInfoClass.from_subpacket(s) for s in subpackets]
    ...
    def _get_key_server_should_not_modify(self):
        return self._get_subpacket_values(C.KEY_SERVER_PREFERENCES_SUBPACKET_TYPE, 'no_modify')

    def _set_key_server_should_not_modify(self, value):
        self._update_subpacket_values(C.KEY_SERVER_PREFERENCES_SUBPACKET_TYPE, 'no_modify', value)

    key_server_should_not_modify = property(_get_key_server_should_not_modify, _set_key_server_should_not_modify)

    def _get_preferred_key_server(self):
        return self._get_subpacket_values(C.PREFERRED_KEY_SERVER_SUBPACKET_TYPE, 'uri')

    def _set_preferred_key_server(self, value):
        self._update_subpacket_values(C.PREFERRED_KEY_SERVER_SUBPACKET_TYPE, 'uri', value)

    preferred_key_server = property(_get_preferred_key_server, _set_preferred_key_server)

    def _get_primary_user_id(self):
        return self._get_subpacket_values(C.PRIMARY_USER_ID_SUBPACKET_TYPE, 'primary')

    def _set_primary_user_id(self, value):
        self._update_subpacket_values(C.PRIMARY_USER_ID_SUBPACKET_TYPE, 'primary', value)

    primary_user_id = property(_get_primary_user_id, _set_primary_user_id)

    def _get_policy_uri(self):
        return self._get_subpacket_values(C.POLICY_URI_SUBPACKET_TYPE, 'uri')

    def _set_policy_uri(self, value):
        self._update_subpacket_values(C.POLICY_URI_SUBPACKET_TYPE, 'uri', value)

    policy_uri = property(_get_policy_uri, _set_policy_uri)

    def _get_supports_modification_detection(self):
        return self._get_subpacket_values(C.FEATURES_SUBPACKET_TYPE, 'supports_modification_detection')

    def _set_supports_modification_detection(self, value):
        self._update_subpacket_values(C.FEATURES_SUBPACKET_TYPE, 'supports_modification_detection', value)

    supports_modification_detection = property(_get_supports_modification_detection, _set_supports_modification_detection)

    def _get_may_certify_others(self):
        return self._get_subpacket_values(C.KEY_FLAGS_SUBPACKET_TYPE, 'may_certify_others')

    def _set_may_certify_others(self, value):
        self._update_subpacket_values(C.KEY_FLAGS_SUBPACKET_TYPE, 'may_certify_others', value)

    may_certify_others = property(_get_may_certify_others, _set_may_certify_others)

    def _get_may_sign_data(self):
        return self._get_subpacket_values(C.KEY_FLAGS_SUBPACKET_TYPE, 'may_sign_data')

    def _set_may_sign_data(self, value):
        self._update_subpacket_values(C.KEY_FLAGS_SUBPACKET_TYPE, 'may_sign_data', value)

    may_sign_data = property(_get_may_sign_data, _set_may_sign_data)

    def _get_may_encrypt_comms(self):
        return self._get_subpacket_values(C.KEY_FLAGS_SUBPACKET_TYPE, 'may_encrypt_comms')

    def _set_may_encrypt_comms(self, value):
        self._update_subpacket_values(C.KEY_FLAGS_SUBPACKET_TYPE, 'may_encrypt_comms', value)

    may_encrypt_comms = property(_get_may_encrypt_comms, _set_may_encrypt_comms)

    def _get_may_encrypt_storage(self):
        return self._get_subpacket_values(C.KEY_FLAGS_SUBPACKET_TYPE, 'may_encrypt_storage')

    def _set_may_encrypt_storage(self, value):
        self._update_subpacket_values(C.KEY_FLAGS_SUBPACKET_TYPE, 'may_encrypt_storage', value)

    may_encrypt_storage = property(_get_may_encrypt_storage, _set_may_encrypt_storage)

    def _get_may_be_used_for_auth(self):
        return self._get_subpacket_values(C.KEY_FLAGS_SUBPACKET_TYPE, 'may_be_used_for_auth')

    def _set_may_be_used_for_auth(self, value):
        self._update_subpacket_values(C.KEY_FLAGS_SUBPACKET_TYPE, 'may_be_used_for_auth', value)

    may_be_used_for_auth = property(_get_may_be_used_for_auth, _set_may_be_used_for_auth)

    def _get_may_have_been_split(self):
        return self._get_subpacket_values(C.KEY_FLAGS_SUBPACKET_TYPE, 'may_have_been_split')

    def _set_may_have_been_split(self, value):
        self._update_subpacket_values(C.KEY_FLAGS_SUBPACKET_TYPE, 'may_have_been_split', value)

    may_have_been_split = property(_get_may_have_been_split, _set_may_have_been_split)

    def _get_may_have_multiple_owners(self):
        return self._get_subpacket_values(C.KEY_FLAGS_SUBPACKET_TYPE, 'may_have_multiple_owners')

    def _set_may_have_multiple_owners(self, value):
        self._update_subpacket_values(C.KEY_FLAGS_SUBPACKET_TYPE, 'may_have_multiple_owners', value)

    may_have_multiple_owners = property(_get_may_have_multiple_owners, _set_may_have_multiple_owners)
    ...
    def target(self):
        ...
        for sub in subpackets:
            ...
            for sig in parent.signatures:
                digest = utils.hash_packet_for_signature(sig.to_packet(), self.signature_type, self.signature_version, hash_algorithm, self.creation_time, pub_algorithm)
