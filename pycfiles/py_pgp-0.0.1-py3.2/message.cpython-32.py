...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pgp/message.py
# Compiled at: 2015-08-31 08:17:33
import abc, collections, datetime, re
...
class BaseMessage(object):
    ...
    def sign(self, secret_key, signature_version=4, signature_type=C.SIGNATURE_OF_A_BINARY_DOCUMENT, hash_algorithm=None, one_pass=True):
        if hash_algorithm is None:
            ...
            if hash_algorithms:
                ...
                hash_algorithm = 1
        signature = secret_key.sign(self, signature_version, signature_type, hash_algorithm)
        return SignedMessageWrapper([signature], self, one_pass)

    def compress(self, compression_algorithm, compression_level):
        return CompressedMessageWrapper(compression_algorithm, compression_level, self)

    def public_key_encrypt(self, symmetric_algorithm, public_key=None, public_keys=None, hidden_public_keys=None, session_key=None, integrity_protect=True):
        if public_keys is not None and public_key is not None:
            raise TypeError('Must be called with one of `public_key` or `public_keys`.')
        ...
        for public_key in public_keys:
            session_key_objs.append(PublicKeySessionKey(3, public_key=public_key, symmetric_algorithm=symmetric_algorithm, session_key=session_key))

        for public_key in hidden_public_keys:
            session_key_objs.append(PublicKeySessionKey(3, public_key=public_key, symmetric_algorithm=symmetric_algorithm, session_key=session_key, hide_key_id=True))

        packet_data = (b'').join(map(bytes, self.to_packets()))
        message_data_obj = SymmetricallyEncryptedMessageData(symmetric_algorithm, session_key, packet_data, integrity_protected=integrity_protect, version=1)
        return EncryptedMessageWrapper(session_key_objs, message_data_obj)

    def symmetric_encrypt(self, symmetric_algorithm, passphrase, session_key=None, integrity_protect=True, s2k_type=None, s2k_hash_algorithm=None, s2k_symmetric_algorithm=None, s2k_salt=None, s2k_count=None):
        if s2k_type is None:
            s2k_type = 3
        ...
        s2k_specification = S2KCls(symmetric_algorithm=s2k_symmetric_algorithm, hash_algorithm=s2k_hash_algorithm, salt=s2k_salt, count=s2k_count)
        if session_key is None:
            key_len = utils.symmetric_cipher_key_lengths[symmetric_algorithm]
            session_key = Random.new().read(key_len)
        session_key_obj = SymmetricSessionKey(symmetric_algorithm=symmetric_algorithm, s2k_specification=s2k_specification, passphrase=passphrase, session_key=session_key)
        packet_data = (b'').join(map(bytes, self.to_packets()))
        message_data_obj = SymmetricallyEncryptedMessageData(symmetric_algorithm, session_key, packet_data, integrity_protected=integrity_protect, version=1)
...
class TextMessage(LiteralMessage):
    ...
    def to_packet(self, header_format=None):
        ...
        return packets.LiteralDataPacket(header_format, b'u', self.filename, self.timestamp, self.data)
...
class BinaryMessage(LiteralMessage):
    ...
    def to_packet(self, header_format=None):
        if header_format is None:
            header_format = self.packet_header_type
        return packets.LiteralDataPacket(header_format, b'b', self.filename, self.timestamp, self.data)
...
encrypted_message_packet_types = encrypted_message_data_packet_types + encrypted_message_session_key_packet_types
...
class SymmetricSessionKey(object):
    ...
    @classmethod
    def from_packet(cls, packet):
        return cls(packet.symmetric_algorithm, packet.s2k_specification, encrypted_key=packet.encrypted_session_key)

    def __init__(self, symmetric_algorithm, s2k_specification, passphrase=None, session_key=None, encrypted_key=None):
    ...
    def get_algorithm_and_session_key(self, passphrase):
        ...
        if isinstance(passphrase, str):
            passphrase = passphrase.encode('utf8')
        return PacketCls._get_key_and_cipher_algo(self.s2k_specification, self.symmetric_algorithm, passphrase, self.encrypted_key)
    ...
    def set_session_key(self, passphrase, session_key=None):
        if not session_key:
            ...
            self.encrypted_key = PacketCls._get_encrypted_key(self.s2k_specification, self.symmetric_algorithm, passphrase, session_key)
        return

    def to_packet(self, header_format=None):
        if header_format is None:
            header_format = self.packet_header_type
        return packets.SymmetricKeyEncryptedSessionKeyPacket(header_format, 4, self.symmetric_algorithm, self.s2k_specification, self.encrypted_key)
...
class PublicKeySessionKey(object):
    packet_header_type = C.NEW_PACKET_HEADER_TYPE
    symmetric_algorithm = 1
    key_hash_algorithm = 1

    @classmethod
    def from_packet(cls, packet):
        result = cls(packet.version, public_key_algorithm=packet.public_key_algorithm, key_id=packet.key_id, encrypted_key=packet.encrypted_session_key)
    ...
    def to_packet(self, header_format=None):
        if header_format is None:
            header_format = self.packet_header_type
        result = packets.PublicKeyEncryptedSessionKeyPacket(header_format, self.version, key_id=self.key_id, public_key_algorithm=self.public_key_algorithm, encrypted_session_key=self.encrypted_key)
        return result

    def __init__(self, version, public_key=None, symmetric_algorithm=None, session_key=None, public_key_algorithm=None, key_id=None, encrypted_key=None, hide_key_id=False):
        ...
        if encrypted_key:
            self.encrypted_key = encrypted_key
        if session_key:
            self.set_algorithm_and_session_key(public_key, symmetric_algorithm, session_key)
    ...
    def get_algorithm_and_session_key(self, secret_key):
        ...
        return PktCls._get_key_and_cipher_algo(self.public_key_algorithm, ko, self.encrypted_key)

    def set_algorithm_and_session_key(self, public_key, sym_algorithm, session_key):
        ko = public_key._get_key_obj()
        PktCls = packets.PublicKeyEncryptedSessionKeyPacket
        self.encrypted_key = PktCls._get_encrypted_key(public_key.public_key_algorithm, ko, sym_algorithm, session_key)
...
SymEncAndIPDPacket = packets.SymmetricallyEncryptedAndIntegrityProtectedDataPacket
...
class SymmetricallyEncryptedMessageData(object):
    ...
    @classmethod
    def from_packet(cls, packet):
        integrity_protected = packet.type == C.SYMMETRICALLY_ENCRYPTED_AND_INTEGRITY_PROTECTED_DATA_PACKET_TYPE
        version = None
        if integrity_protected:
            version = packet.version
        return cls(encrypted_data=packet.encrypted_data, integrity_protected=integrity_protected, version=version)

    def __init__(self, symmetric_algorithm=None, session_key=None, data=None, encrypted_data=None, integrity_protected=False, version=None):
    ...
    def encrypt(self, symmetric_algo, key, message):
        ...
        data = bytearray()
        cipher = utils.get_symmetric_cipher(symmetric_algo, key, utils.CFB, b'\x00' * block_len)
        padding = 0
        offset = 0
        if not self.integrity_protected:
            ...
            cipher = utils.get_symmetric_cipher(symmetric_algo, key, utils.CFB, iv)
    ...
    def decrypt(self, symmetric_algo, key):
        ...
        if self.integrity_protected:
            ...
            cipher = utils.get_symmetric_cipher(symmetric_algo, key, utils.OPENPGP, encrypted_iv)
    ...
    def to_packet(self, header_format=None):
        if header_format is None:
            header_format = self.packet_header_type
        if self.integrity_protected:
            return SymEncAndIPDPacket(header_format, self.version, self.data)
        else:
            return packets.SymmetricallyEncryptedDataPacket(header_format, self.data)
            return
...
def parse_session_key_packet(packet):
    ...
    if packet.type == C.SYMMETRIC_KEY_ENCRYPTED_SESSION_KEY_PACKET_TYPE:
    ...
    def get_message(self, secret_key_or_passphrase=None):
        ...
        for sess_key_obj in self.session_keys:
            if isinstance(sess_key_obj, PublicKeySessionKey):
                try:
                    sym_algo, sess_key = sess_key_obj.get_algorithm_and_session_key(secret_key_or_passphrase)
                except ValueError:
                    continue

            else:
                if isinstance(sess_key_obj, SymmetricSessionKey):
                    sym_algo, sess_key = sess_key_obj.get_algorithm_and_session_key(secret_key_or_passphrase)
    ...
    def from_packets(cls, packets):
        ...
        for packet in packets:
            if packet.type == C.ONE_PASS_SIGNATURE_PACKET_TYPE:
                ...
                if message:
                    signature = parse_signature_from_packet(packet, message, one_pass=one_pass)
    ...
    def to_packets(self, header_format=None):
        ...
        if self.one_pass:
            ...
            for sig in self.signatures:
                ...
                pkts.append(packets.OnePassSignaturePacket(op_header_format, 3, sig.signature_type, sig.hash_algorithm, sig.public_key_algorithm, sig.issuer_key_ids[0], nested=i != len(self.signatures)))
...
class CompressedMessageWrapper(BaseMessage):
    ...
    @classmethod
    def from_packet(cls, packet):
        return cls(packet.compression_algorithm, compressed_data=packet.compressed_data)

    def __init__(self, compression_algorithm, compression_level=None, message=None, compressed_data=None):
    ...
    def get_message(self):
        pkts = packets.CompressedDataPacket.decompress_data(self.compression_algorithm, self.data)
        return open_pgp_message_from_packets(pkts)

    def set_message(self, message):
        pkts = message.to_packets()
        self.data = packets.CompressedDataPacket.compress_packets(self.compression_algorithm, self.compression_level, pkts)

    def to_packet(self, header_format=None):
        if header_format is None:
            header_format = self.packet_header_type
        return packets.CompressedDataPacket(header_format, self.compression_algorithm, self.data)
...
def open_pgp_message_from_packets(pkts):
    ...
    if packet.type in encrypted_message_packet_types:
        ...
        if packet.type in (C.ONE_PASS_SIGNATURE_PACKET_TYPE,
         C.SIGNATURE_PACKET_TYPE):
            pkts.appendleft(packet)
            result = SignedMessageWrapper.from_packets(pkts)
        else:
            if packet.type == C.COMPRESSED_DATA_PACKET_TYPE:
