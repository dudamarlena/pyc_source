...
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/appd/model/policy_violation.py
...
class PolicyViolation(JsonObject):
    ...
    def __init__(self, pv_id=0, pv_name='', description='', status='OPEN', severity='INFO', start_time_ms=0, end_time_ms=0, detected_time_ms=0, deep_link_url='', affected_entity=None, triggered_entity=None):
        self._severity = None
        self._status = None
        self.id, self.name, self.description, self.status, self.severity, self.start_time_ms, self.end_time_ms, self.detected_time_ms, self.deep_link_url, self.affected_entity, self.triggered_entity = (
