...
# [GCC 8.4.0]
# Embedded file name: /Users/Mark/PycharmProjects/multi_label_classification/mlclas/neural/bpmll.py
# Compiled at: 2016-06-17 11:33:22
...
class BPMLL:
    ...
    def __init__(self, neural=0.2, epoch=20, weight_decay=1e-05, regularization=0.1, normalize=False, axis=0, print_procedure=False):
        self.features = 0
        self.classes = 0
        self.samples = 0
        self.neural_num = 0
    ...
    def global_error(self):
        global_error = 0
        weights_square_sum = np.sum(np.square(self.wsj_matrix)) + np.sum(np.square(self.vhs_matrix)) + np.sum(np.square(self.bias_b)) + np.sum(np.square(self.bias_a))
        for i in range(self.samples):
