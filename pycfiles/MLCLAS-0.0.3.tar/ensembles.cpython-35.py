...
# [GCC 8.4.0]
# Embedded file name: /Users/Mark/PycharmProjects/multi_label_classification/mlclas/ensemble/ensembles.py
# Compiled at: 2016-06-17 11:32:42
...
"""
import numpy as np, copy, math, random
    ...
    def fit(self, X, y):
        ...
        for i in range(0, self.classes_):
            for j in range(i + 1, self.classes_):
                ...
                if not target.count(target[0]) == len(target):
                    if len(target) == 0:
                        pass
                    else:
    ...
    def predict(self, X):
        ...
        for index in range(test_samples_length):
            ...
            for each_label in range(self.classes):
                pch = (self.s + self.kj[each_label][neighbor_label_count[each_label]]) / (self.s * (self.k + 1) + sum(self.kj[each_label]))
                pcnh = (self.s + self.knj[each_label][neighbor_label_count[each_label]]) / (self.s * (self.k + 1) + sum(self.knj[each_label]))
