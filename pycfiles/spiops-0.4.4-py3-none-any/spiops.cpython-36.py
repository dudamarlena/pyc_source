...
# Compiled at: 2019-02-26 09:15:32
# Size of source mod 2**32: 78780 bytes
import math, numpy as np, spiceypy, logging, os, numpy as np
...
def adcsng_hk_quaternions2ck_reader(tm_file, input_time_format='UTC', input_time_field_number='1', delimiter=',', input_processing=False, qs_col=1, qx_col=2, qy_col=3, qz_col=4):
    input_data_field_numbers = [
     qx_col, qy_col, qz_col, qs_col]
    tm_list = []
    previous_row_time = ''
    sclk_partition = '1'
    sclk_delimiter = '.'
        ...
        for line in t:
            if '#' not in line and 'Date' not in line and input_time_format not in line:
                    ...
                    if ',' in delimiter:
                        if input_time_format == 'SCLK':
                            if ',' in input_time_field_number:
                                row_time = sclk_partition + '/' + str(line.split(delimiter)[(int(input_time_field_number[0]) - 1)]) + sclk_delimiter + str(line.split(delimiter)[(int(input_time_field_number[2]) - 1)])
                            else:
                ...
                if row_time == previous_row_time:
                    logging.info('   HM TM Processing: Found duplicate time at {}'.format(row_time))
                else:
                    append_bool = True
                    for quaternion in row_data:
                        if quaternion >= 1.0:
                            append_bool = False
                            logging.info('   HM TM Processing: Found quaternion GT 1 on input line {}:'.format(row_data))
                ...
                if input_processing:
                    ...
                    if filter_flag:
                        if sclk_fraction == sclk_fraction_prev:
                            ...
                            if sclk_fraction == sclk_initial:
                                logging.info('   HM TM Processing: Coarse quaternion: Spurious SCLK fractions before input line {}:'.format(index))
...
def fov_illum(mk, sensor, time=None, angle='DEGREES', abcorr='LT+S', report=False, unload=False):
    """
    ...
    if report:
        print('Illumination angle of {} is {} [{}]'.format(sensor, fov_illumination, angle))
...
def cov_spk_obj(mk, object, time_format='TDB', global_boundary=False, report=False, unload=False):
    ...
    while spk_count >= 0:
        ...
        for id in spk_ids:
            if id == object_id:
                ...
                boundaries = time.cov_int(object_cov=object_cov, object_id=object_id,
                  kernel=(spk_kernel[0]),
                ...
                if global_boundary:
                    et_boundaries_list.append(time.cov_int(object_cov=object_cov,
                      object_id=object_id,
                      kernel=(spk_kernel[0]),
    ...
    if global_boundary:
        ...
        if report:
            print('Global Coverage for {} [{}]: {} - {}'.format(str(spiceypy.bodc2n(object_id)), time_format, boundaries_list[0], boundaries_list[1]))
    if unload:
        spiceypy.unload(mk)
    return boundaries_list


def cov_spk_ker(spk, object=False, time_format='TDB', support_ker='', report=False, unload=True):
    """
    ...
    for id in object_id:
        if id in spk_ids:
            ...
            cov = time.cov_int(object_cov=object_cov, object_id=id,
              kernel=spk,
              time_format=time_format,
              report=report)
        else:
            if report:
                print('{} with ID {} is not present in {}.'.format(object, id, spk))
            return False
    ...
    while ck_count >= 0:
        ...
        for id in ck_ids:
            if id == object_id:
                ...
                object_cov = spiceypy.ckcov(ck=(ck_kernel[0]), idcode=object_id, needav=False,
                  level='SEGMENT',
                  tol=0.0,
                  timsys='TDB',
                  cover=object_cov)
...
def cov_ck_ker(ck, object, support_ker=list(), time_format='UTC', report=False, unload=True):
    """
    ...
    if support_ker:
        ...
        if object_id in ck_ids:
            ...
            object_cov = spiceypy.ckcov(ck=ck, idcode=object_id, needav=False,
              level='INTERVAL',
              tol=0.0,
              timsys='TDB',
              cover=object_cov)
...
def fk_body_ifj2000(mission, body, pck, body_spk, frame_id, report=False, unload=False, file=True):
    """
    ...
    if report:
        ...
        if file:
            ...
            return '{}_IF->J2000 (3-2-3): {} - {} - {}'.format(body, r3 * spiceypy.dpr(), r2 * spiceypy.dpr(), r1 * spiceypy.dpr())
    if unload:
        spiceypy.unload(pck)
        ...
        for element in eul_error:
            if element >= tolerance:
                ...
                if interval_bool:
                    ...
                    f.write('TOLERANCE of ' + str(tolerance) + ' DEG exceeded from ' + utc_start + ' until ' + utc_finish + ' with an average angle of ' + str(numpy.mean(eul_tol_list)) + ' DEG \n')
                interval_bool = False
            count += 1
...
def state_report(et_list, pos_spk1, pos_spk2, vel_spk1, vel_spk2, pos_tolerance, vel_tolerance, name=''):
    ...
    with open('state_{}_report.txt'.format(name), 'w+') as (f):
        ...
        for element in pos_error:
            if element >= pos_tolerance:
                ...
                if interval_bool:
                    utc_finish = spiceypy.et2utc(et_list[count], 'ISOC', 2)
                    f.write('TOLERANCE of ' + str(pos_tolerance) + ' KM exceeded from ' + utc_start + ' until ' + utc_finish + ' with an average distance of ' + str(numpy.mean(pos_tol_list)) + ' KM \n')
                interval_bool = False
            count += 1
        ...
        for element in vel_error:
            if element >= vel_tolerance:
                ...
                if interval_bool:
                    utc_finish = spiceypy.et2utc(et_list[count], 'ISOC', 2)
                    f.write('TOLERANCE of ' + str(vel_tolerance) + ' KM/S exceeded from ' + utc_start + ' until ' + utc_finish + ' with an average velocity of ' + str(numpy.mean(vel_tol_list)) + ' KM/S \n')
                count += 1
...
def ckdiff_euler(mk, ck1, ck2, spacecraft_frame, target_frame, resolution, tolerance, utc_start='', utc_finish='', plot_style='line', report=True, notebook=False):
    """
    Provides time coverage summary for a given object for a given CK file.
    Several options are available. This function is based on the following
    SPICE API:
...
def ckdiff(ck1, ck2, spacecraft_frame, target_frame, resolution, tolerance, utc_start='', utc_finish='', mk='', output='boresight', boresight=[0, 0, 1], plot_style='line', report=False, notebook=False):
    """
    Provides time coverage summary for a given object for a given CK file.
    Several options are available. This function is based on the following
    SPICE API:
    ...
    if mk:
        ...
        if output == 'euler_angles':
            plot(et_list, [eul1_ck1, eul1_ck2], yaxis_name=['Degrees', 'Degrees'], title=('Euler Angle 1 {}'.format(title_name)),
              format=plot_style,
              notebook=notebook)
            plot(et_list, [eul2_ck1, eul2_ck2], yaxis_name=['Degrees', 'Degrees'], title=('Euler Angle 2 {}'.format(title_name)),
              format=plot_style,
              notebook=notebook)
            plot(et_list, [eul3_ck1, eul3_ck2], yaxis_name=['Degrees', 'Degrees'], title=('Euler Angle 3 {}'.format(title_name)),
              format=plot_style,
              notebook=notebook)
        else:
            plot(et_list, [bsight_ck], yaxis_name=['Degrees', 'Degrees'], title=('+Z Axis Angle Difference {}'.format(title_name)),
              format=plot_style,
              notebook=notebook)
        if report:
...
def ckdiff_error(ck1, ck2, spacecraft_frame, target_frame, resolution, tolerance, mk='', utc_start='', utc_finish='', output='', boresight=[
 0, 0, 1], plot_style='line', report=False, notebook=False):
    """
    Provides time coverage summary for a given object for a given CK file.
    Several options are available. This function is based on the following
    SPICE API:
    ...
    if mk:
        ...
        eul3_name = '{}_{}'.format(ck1_filename, ck2_filename)
        if output == 'euler_angles':
            plot(et_list, [eul1_diff, eul2_diff, eul3_diff], yaxis_name=['Degrees', 'Degrees', 'Degrees'], title='Euler Angle Differences',
              format=plot_style,
              notebook=notebook)
        else:
            if output == 'boresight':
                plot(et_list, bsight_diff, yaxis_name='Degrees', title='Boresight Angle Difference',
                  format=plot_style,
                  notebook=notebook)
            else:
                plot(et_list, angle_diff, yaxis_name='Milidegrees', title='Attitude Error',
                  format=plot_style,
                  notebook=notebook)
        if report:
            eul_angle_report(et_list, eul1_ck1, eul1_ck2, 1, tolerance, name=eul1_name)
...
def ckplot(ck1, spacecraft_frame, target_frame, resolution, mk='', utc_start='', utc_finish='', notebook=False, plot_style='circle'):
    """
    Provides time coverage summary for a given object for a given CK file.
    Several options are available. This function is based on the following
    SPICE API:
    ...
    if mk:
        ...
        spiceypy.furnsh(ck1)
        et_boundaries_list = cov_ck_ker(ck1, support_ker=mk, object=spacecraft_frame, time_format='TDB')
        start = True
        for et_start, et_finish in zip(et_boundaries_list[0::2], et_boundaries_list[1::2]):
            if start:
                et_list = numpy.arange(et_start, et_finish, resolution)
    ...
    spiceypy.unload(ck1)
    plot(et_list, [eul1, eul2, eul3], yaxis_name=[
     'Euler Angle 1', 'Euler Angle 2', 'Euler Angle 3'],
      title=('Euler Angles for {}'.format(ck1.split('/')[(-1)])),
      notebook=notebook,
      format=plot_style)


def spkdiff(mk, spk1, spk2, spacecraft, target, resolution, pos_tolerance, vel_tolerance, target_frame='', utc_start='', utc_finish='', plot_style='line', report=True):
    """
    Provides time coverage summary for a given object for a given CK file.
    Several options are available. This function is based on the following
    SPICE API:
    ...
    if not target_frame:
        ...
        for et in et_list:
            ...
            pos_spk1.append(np.sqrt(state[0] * state[0] + state[1] * state[1] + state[2] * state[2]))
            vel_spk1.append(np.sqrt(state[3] * state[3] + state[4] * state[4] + state[5] * state[5]))
        ...
        for et in et_list:
            ...
            pos_spk2.append(np.sqrt(state[0] * state[0] + state[1] * state[1] + state[2] * state[2]))
            vel_spk2.append(np.sqrt(state[3] * state[3] + state[4] * state[4] + state[5] * state[5]))
        ...
        if report:
            ...
            state_report(et_list, pos_spk1, pos_spk2, vel_spk1, vel_spk2, pos_tolerance, vel_tolerance, name=('{}_{}'.format(spk1_filename, spk2_filename)))
...
def read_ik_with_sectors(sensor_name):
    ...
    try:
        ...
        try:
            ...
            return (
             sensnm, sensid, secnum, secsiz, secsis, secfrm, secdir)
...
def hga_angles(sc, time):
    ...
    try:
        ...
        hga_el = np.rad2deg(2.0 * np.arccos(quat[0]))
        cmat = spiceypy.pxform(hga_el_frame, hga_az_frame, time)
        quat = spiceypy.m2q(cmat)
        hga_az = np.rad2deg(2.0 * np.arccos(quat[0]))
...
def solar_aspect_angles(sc, time):
    ...
    try:
        ...
        saa_sa, saa_sa_p, saa_sa_n, saa_sc_x, saa_sc_y, saa_sc_z = (0, 0, 0, 0, 0,
                                                                    0)

    if sa_frame:
        return (
         [
          saa_sa], [saa_sc_x, saa_sc_y, saa_sc_z])
    else:
...
def solar_array_angle(sa_frame, time):
    ...
    try:
        ...
        quat = spiceypy.m2q(cmat)
        sa_ang = np.rad2deg(2.0 * np.arccos(quat[0]))
    except:
        sa_ang = 0
...
def body_distance_to_plane(body_distance, body_plane, time):
    ...
    dist_1 = abs(a1 * pos_2[0] + b1 * pos_2[1] + c1 * pos_2[2] + d1) / np.sqrt(np.square(a1) + np.square(b1) + np.square(c1))
...
def beta_angle(observer, target, time):
    ...
    beta = abs(90.0 - spiceypy.vsep(vec_orbit, vec_tar_sun) * spiceypy.dpr())
    return beta


def ck_coverage_timeline(metakernel, sc, notebook=True, html_file_name='test', plot_width=975, plot_height=500):
    if notebook:
        output_notebook()
        plot_width = 975
        plot_height = 300
    ...
    for kernel in kernels:
        try:
            cov = cov_ck_ker((path + kernel), sc_frame, support_ker=metakernel, time_format='TDB')
        except:
            pass
    ...
    source = ColumnDataSource(data=dict(start_dt=start_dt, finsh_dt=finsh_dt,
      ck_kernels=ck_kernels))
    p = figure(y_range=ck_kernels, plot_height=plot_height, plot_width=plot_width, title='CK Kernels Coverage')
    p.hbar(y=ck_kernels, height=0.2, left=start_dt, right=finsh_dt, color='red')
    labels = LabelSet(x='start_dt', y='ck_kernels', text='ck_kernels', level='glyph', x_offset=5,
      y_offset=(-5),
      source=source)
    p.xaxis.formatter = DatetimeTickFormatter(seconds=['%Y-%m-%d %H:%M:%S'], minsec=[
     '%Y-%m-%d %H:%M:%S'],
      minutes=[
     '%Y-%m-%d %H:%M:%S'],
      hourmin=[
...
def spk_coverage_timeline(metakernel, sc, notebook=True, html_file_name='test', plot_width=975, plot_height=500):
    if notebook:
        output_notebook()
        plot_width = 975
        plot_height = 300
    ...
    for kernel in kernels:
        try:
            cov = cov_spk_ker((path + kernel), (sc.upper()), support_ker=metakernel, time_format='TDB')
        except:
            pass
    ...
    p = figure(y_range=spk_kernels, plot_height=plot_height, plot_width=plot_width, title='SPK Kernels Coverage')
    p.hbar(y=spk_kernels, height=0.2, left=start_dt, right=finsh_dt, color='red')
    labels = LabelSet(x='start_dt', y='spk_kernels', text='spk_kernels', level='glyph', x_offset=5,
      y_offset=(-5),
      source=source)
    p.xaxis.formatter = DatetimeTickFormatter(seconds=['%Y-%m-%d %H:%M:%S'], minsec=[
     '%Y-%m-%d %H:%M:%S'],
