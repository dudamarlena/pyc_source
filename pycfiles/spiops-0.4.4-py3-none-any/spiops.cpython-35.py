...
# Compiled at: 2018-08-27 10:35:14
# Size of source mod 2**32: 72405 bytes
import math, numpy as np, spiceypy as cspice, logging, os, numpy as np
            ...
            if input_processing:
                ...
                if filter_flag:
                    if sclk_fraction == sclk_fraction_prev:
                        ...
                        if len(row_prev) <= 5 and sclk_fraction == sclk_initial:
    ...
    while spk_count >= 0:
        ...
        for id in spk_ids:
            if id == object_id:
                ...
                if global_boundary:
                    et_boundaries_list.append(time.cov_int(object_cov=object_cov, object_id=object_id, kernel=spk_kernel[0], global_boundary=True, time_format='TDB', report=False))
...
def cov_spk_ker(spk, object=False, time_format='TDB', support_ker='', report=False, unload=False):
    """
    ...
    for id in object_id:
        if id in spk_ids:
            ...
            cov = time.cov_int(object_cov=object_cov, object_id=id, kernel=spk, time_format=time_format, report=report)
        else:
...
def cov_ck_ker(ck, object, support_ker=list(), time_format='UTC', report=False, unload=False):
    """
    ...
    if object_id in ck_ids:
        ...
        object_cov = cspice.ckcov(ck=ck, idcode=object_id, needav=False, level='INTERVAL', tol=0.0, timsys='TDB', cover=object_cov)
    else:
        print('{} with ID {} is not present in {}.'.format(object, object_id, ck))
        return
    if time_format == 'SPICE':
...
def fk_body_ifj2000(mission, body, pck, body_spk, frame_id, report=False, unload=False, file=True):
    ...
    if file:
        ...
        with open('{}_{}_IF_J2000.tf'.format(mission, body), 'w+') as (f):
            ...
            f.write('                                        {}\n'.format(r3 * cspice.dpr()))
            f.write('                                        {}\n'.format(r2 * cspice.dpr()))
            f.write('                                        {}\n'.format(r1 * cspice.dpr()))
            f.write('                                     )\n')
        ...
        return '{}_IF->J2000 (3-2-3): {} - {} - {}'.format(body, r3 * cspice.dpr(), r2 * cspice.dpr(), r1 * cspice.dpr())
    if unload:
        cspice.unload(pck)
...
def ckdiff_euler(mk, ck1, ck2, spacecraft_frame, target_frame, resolution, tolerance, utc_start='', utc_finish='', plot_style='line', report=True, notebook=False):
    ...
    plot(et_list, [eul1_ck1, eul1_ck2], yaxis_name=['Euler Angle 1 CK1',
     'Euler Angle 1 CK2'], title='Euler Angle 1 {}'.format(eul1_name), format=plot_style, notebook=notebook)
    plot(et_list, [eul2_ck1, eul2_ck2], yaxis_name=['Euler Angle 2 CK1',
     'Euler Angle 2 CK2'], title='Euler Angle 2 {}'.format(eul2_name), format=plot_style, notebook=notebook)
    plot(et_list, [eul3_ck1, eul3_ck2], yaxis_name=['Euler Angle 3 CK1',
     'Euler Angle 3 CK2'], title='Euler Angle 3 {}'.format(eul3_name), format=plot_style, notebook=notebook)
    if report:
...
def ckdiff(ck1, ck2, spacecraft_frame, target_frame, resolution, tolerance, utc_start='', utc_finish='', mk='', output='boresight', boresight=[0, 0, 1], plot_style='line', report=True, notebook=False):
    """
    Provides time coverage summary for a given object for a given CK file.
    Several options are available. This function is based on the following
    SPICE API:
    ...
    if output == 'euler_angles':
        plot(et_list, [eul1_ck1, eul1_ck2], yaxis_name=['Degrees', 'Degrees'], title='Euler Angle 1 {}'.format(title_name), format=plot_style, notebook=notebook)
        plot(et_list, [eul2_ck1, eul2_ck2], yaxis_name=['Degrees', 'Degrees'], title='Euler Angle 2 {}'.format(title_name), format=plot_style, notebook=notebook)
        plot(et_list, [eul3_ck1, eul3_ck2], yaxis_name=['Degrees', 'Degrees'], title='Euler Angle 3 {}'.format(title_name), format=plot_style, notebook=notebook)
    else:
        plot(et_list, [bsight_ck1, bsight_ck2], yaxis_name=['Degrees', 'Degrees'], title='+Z Axis Angle Difference {}'.format(title_name), format=plot_style, notebook=notebook)
    if report:
...
def ckdiff_error(ck1, ck2, spacecraft_frame, target_frame, resolution, tolerance, mk='', utc_start='', utc_finish='', output='', boresight=[0, 0, 1], plot_style='line', report=True, notebook=False):
    """
    Provides time coverage summary for a given object for a given CK file.
    Several options are available. This function is based on the following
    SPICE API:
    ...
    eul3_name = '{}_{}'.format(ck1_filename, ck2_filename)
    if output == 'euler_angles':
        plot(et_list, [eul1_diff, eul2_diff, eul3_diff], yaxis_name=['Degrees', 'Degrees', 'Degrees'], title='Euler Angle Differences', format=plot_style, notebook=notebook)
    else:
        if output == 'boresight':
            plot(et_list, bsight_diff, yaxis_name='Degrees', title='Boresight Angle Difference', format=plot_style, notebook=notebook)
        else:
            plot(et_list, angle_diff, yaxis_name='Milidegrees', title='Atittude Error', format=plot_style, notebook=notebook)
    if report:
        eul_angle_report(et_list, eul1_ck1, eul1_ck2, 1, tolerance, name=eul1_name)
        eul_angle_report(et_list, eul2_ck1, eul2_ck2, 2, tolerance, name=eul2_name)
        eul_angle_report(et_list, eul3_ck1, eul3_ck2, 3, tolerance, name=eul3_name)
...
def ckplot(ck1, spacecraft_frame, target_frame, resolution, mk='', utc_start='', utc_finish='', notebook=False, plot_style='circle'):
    ...
    plot(et_list, [eul1, eul2, eul3], yaxis_name=[
     'Euler Angle 1', 'Euler Angle 2', 'Euler Angle 3'], title='Euler Angles for {}'.format(ck1.split('/')[(-1)]), notebook=notebook, format=plot_style)
...
def spkdiff(mk, spk1, spk2, spacecraft, target, resolution, pos_tolerance, vel_tolerance, target_frame='', utc_start='', utc_finish='', plot_style='line', report=True):
    ...
    plot(et_list, [pos_spk1, pos_spk2], yaxis_name=['Position SPK1',
     'Position SPK2'], title='Position of {} w.r.t {} ({})'.format(spacecraft, target, target_frame), format=plot_style)
    cspice.unload(spk2)
    if report:
