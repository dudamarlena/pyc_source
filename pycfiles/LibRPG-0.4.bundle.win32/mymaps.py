...
# [GCC 8.4.0]
# Embedded file name: D:\dev\pygame\librpg\librpg\test\worldtest\mymaps.py
# Compiled at: 2009-10-02 02:32:28
...
class SavePoint(ScenarioMapObject):
    ...
    def activate(self, party_avatar, direction):
        self.map.schedule_message(MessageDialog('You game will be saved to %s.' % SAVE_FILE, block_movement=True))
        self.map.save_world(SAVE_FILE)
        self.map.schedule_message(MessageDialog('Game saved.', block_movement=True))


class Chest(MapObject):

    def __init__(self, closed=True):
        MapObject.__init__(self, MapObject.OBSTACLE, image_file=charset_path('chest.png'), image_index=0, basic_animation=[
         [
          0]])
        if closed:
            self.facing = UP
...
class Map1(WorldMap):
    ...
    def __init__(self):
        WorldMap.__init__(self, 'worldtest/map1.map', LOWER_TILESET, UPPER_TILESET)

    def initialize(self, local_state, global_state):
        self.add_area(RelativeTeleportArea(x_offset=-8, map_id=2), RectangleArea((9,
                                                                                  0), (9,
                                                                                       9)))
...
class Map2(WorldMap):
    ...
    def __init__(self):
        WorldMap.__init__(self, 'worldtest/map2.map', LOWER_TILESET, UPPER_TILESET)

    def initialize(self, local_state, global_state):
        self.add_area(RelativeTeleportArea(x_offset=+8, map_id=1), RectangleArea((0,
                                                                                  0), (0,
                                                                                       9)))
        self.add_area(RelativeTeleportArea(x_offset=-8, map_id=3), RectangleArea((9,
                                                                                  0), (9,
                                                                                       9)))
        if local_state is not None:
            self.chest = Chest(local_state['chest_closed'])
...
class Map3(WorldMap):
    ...
    def __init__(self):
        WorldMap.__init__(self, 'worldtest/map3.map', LOWER_TILESET, UPPER_TILESET)

    def initialize(self, local_state, global_state):
        self.add_object(GameOverBarrel(self), Position(6, 4))
        self.add_area(RelativeTeleportArea(x_offset=+8, map_id=2), RectangleArea((0,
                                                                                  0), (0,
                                                                                       9)))
        self.add_area(RelativeTeleportArea(y_offset=+8), RectangleArea((0, 0), (9,
                                                                                0)))
        self.add_area(RelativeTeleportArea(y_offset=-8), RectangleArea((0, 9), (9,
                                                                                9)))
