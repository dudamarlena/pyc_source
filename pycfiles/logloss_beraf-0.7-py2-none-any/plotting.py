...
# [GCC 8.4.0]
# Embedded file name: /Users/dezzan/Projects/ProstateProject/Methylome/logloss-beraf/build/lib/logloss_beraf/visualisation/plotting.py
# Compiled at: 2017-08-11 05:48:44
...
import copy, itertools, seaborn, logging
...
def plot_pca_two_group(group1, group2, title, group1_annotation, group2_annotation, outfile=None):
    ...
    group1_pca_source = ColumnDataSource(data=dict(x=X_c.values[0:group1.shape[1]], y=Y_c.values[0:group1.shape[1]], pid=group1.columns))
    group2_pca_source = ColumnDataSource(data=dict(x=X_c.values[group1.shape[1]:group1.shape[1] + group2.shape[1]], y=Y_c.values[group1.shape[1]:group1.shape[1] + group2.shape[1]], pid=group2.columns))
    p.scatter('x', 'y', source=group1_pca_source, color='red', fill_alpha=0.2, size=10, legend=group1_annotation)
    p.scatter('x', 'y', source=group2_pca_source, color='green', fill_alpha=0.2, size=10, legend=group2_annotation)
...
def plot_pca_by_annotation(df, annotation, clinical_column, sample_name_column, outfile=None):
    ...
    for group1, group2 in itertools.combinations(groups, 2):
        ...
        plot_pca_two_group(clin_group1[tdf_columns].transpose(), clin_group2[tdf_columns].transpose(), clinical_column, str(group1), str(group2), outfile=outfile)
...
def plot_pca_several_group(t_group, title, annotation, clinical_column=None, sample_column=None):
    ...
    if clinical_column is not None:
        for clinical_type in annotation[clinical_column].unique().tolist():
            ...
            current_group_samples_index = [ i for i, val in enumerate(t_group.columns.tolist()) if val in current_group.columns
                                          ]
            logger.debug('Current group: ', clinical_type)
            logger.debug(current_group)
            logger.debug(current_group_samples_index)
            group_pca_source = ColumnDataSource(data=dict(x=X_c.loc[current_group_samples_index].values, y=Y_c.loc[current_group_samples_index].values, pid=current_group.columns))
            p.scatter('x', 'y', source=group_pca_source, fill_alpha=0.2, size=10, legend=clinical_type, color=colors[color_index])
        ...
        t_group_pca_source = ColumnDataSource(data=dict(x=X_c, y=Y_c, pid=t_group.columns))
        p.scatter('x', 'y', source=t_group_pca_source, color='red', fill_alpha=0.2, size=10)
    p.legend.orientation = 'vertical'
    hover = p.select(dict(type=HoverTool))
    hover.tooltips = OrderedDict([
     ('Participant ID', '@pid')])
