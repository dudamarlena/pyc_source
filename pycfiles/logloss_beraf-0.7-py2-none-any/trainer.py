...
# [GCC 8.4.0]
# Embedded file name: /Users/dezzan/Projects/ProstateProject/Methylome/logloss-beraf/build/lib/logloss_beraf/model_ops/trainer.py
# Compiled at: 2017-08-12 06:44:26
import copy, logging, os, matplotlib.pyplot as plt, numpy as np, pandas
...
from logloss_beraf.utils.constants import PREFILTER_PCA_PLOT_NAME, POSTFILTER_PCA_PLOT_NAME, FEATURE_IMPORTANCE_PLOT_NAME, FEATURE_COLUMN, FEATURE_IMPORTANCE_COLUMN, TRAINED_MODEL_NAME
...
class LLBModelTrainer(object):
    ...
    def __init__(self, threads=1, max_num_of_features=20, cv_method='SKFold', class_weights='balanced', final_clf_estimators_num=3000, intermediate_clf_estimators_num=750, logloss_estimates=50, min_beta_threshold=0.2, rr_iterations=1500, correlation_threshold=0.85, output_folder=None):
        """
        :param threads:
        :type threads: int
    ...
    def _run_randomized_regression(self, feature_df, annotation, clinical_column, sample_fraction=0.75):
        ...
        clf = RandomizedLogisticRegression(n_resampling=self.rr_iterations, sample_fraction=sample_fraction, n_jobs=self.threads).fit(feature_df, annotation[clinical_column])
    ...
    def _train_clf(self, X, y, n_estimators=10):
        ...
        if self.cv_method is not None:
            ...
            scores = cross_validation.cross_val_score(clf, X, y, cv=cv_algo, scoring='neg_log_loss', n_jobs=self.threads, verbose=1)
        clf.fit(X, y)
    ...
    def _describe_and_filter_regions(self, basic_region_df, annotation, clinical_column, sample_name_column):
        ...
        plot_pca_by_annotation(basic_region_df, annotation, clinical_column, sample_name_column, outfile=os.path.join(self.output_folder, PREFILTER_PCA_PLOT_NAME))
        selected_features, model = self._run_randomized_regression(basic_region_df, annotation, clinical_column)
        plot_pca_by_annotation(basic_region_df[selected_features], annotation, clinical_column, sample_name_column, outfile=os.path.join(self.output_folder, POSTFILTER_PCA_PLOT_NAME))
        return (
    ...
    def _apply_feature_imp_thresh(self, features, feature_imp, thresh):
        return [ feature[0] for feature in zip(features.values, feature_imp) if feature[1] > thresh
               ]
    ...
    def train(self, train_regions, anndf, sample_class_column, sample_name_column):
        ...
        :return:
        """
        selected_regions, clf = self._describe_and_filter_regions(train_regions, anndf, sample_class_column, sample_name_column)
        first_clf, mean, std = self._train_clf(train_regions[selected_regions.values], anndf[sample_class_column], n_estimators=self.final_clf_estimators_num)
        feature_importances = pandas.DataFrame.from_records(zip(selected_regions.values, first_clf.feature_importances_), columns=[
         FEATURE_COLUMN, FEATURE_IMPORTANCE_COLUMN])
        self.plot_fi_distribution(feature_importances)
        feature_importances = feature_importances[(abs(feature_importances[FEATURE_IMPORTANCE_COLUMN]) > 0)]
        corr_matrix = train_regions[feature_importances[FEATURE_COLUMN]].corr().applymap(lambda x: 1 if abs(x) >= self.correlation_threshold else 0)
        logloss_df_cols = [
         'thresh', 'mean', 'std', 'len']
        logloss_di = pandas.DataFrame(columns=logloss_df_cols)
        for thresh in np.arange(feature_importances[FEATURE_IMPORTANCE_COLUMN].quantile(0.8), feature_importances[FEATURE_IMPORTANCE_COLUMN].max(), (feature_importances[FEATURE_IMPORTANCE_COLUMN].max() - feature_importances[FEATURE_IMPORTANCE_COLUMN].min()) / self.logloss_estimates):
            selected_features = self._apply_feature_imp_thresh(selected_regions, first_clf.feature_importances_, thresh)
            if len(selected_features) < 2:
                continue
            logger.info('Estimating %d features on feature importance threshold %f', len(selected_features), thresh)
            clf, mean, std = self._train_clf(train_regions[selected_features], anndf[sample_class_column], n_estimators=self.intermediate_clf_estimators_num)
            logloss_di = logloss_di.append(pandas.Series([thresh, mean, std, len(selected_features)], index=logloss_df_cols), ignore_index=True)
        ...
        logger.info(thresh)
        selected_features = self._apply_feature_imp_thresh(selected_regions, first_clf.feature_importances_, thresh['thresh'])
        logger.info('Trainig final model...')
        clf, mean, std = self._train_clf(train_regions[selected_features], anndf[sample_class_column], n_estimators=self.final_clf_estimators_num)
        logger.info(('Selected features: {0}').format(selected_features))
        pickle.dump(clf, open(os.path.join(self.output_folder, TRAINED_MODEL_NAME), 'w'))
        return (
