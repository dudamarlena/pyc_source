...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/synapseclient/core/upload/multipart_upload.py
...
"""
import hashlib, json, math, mimetypes, os, requests, time, warnings, ctypes
...
def _start_multipart_upload(syn, filename, md5, fileSize, partSize, contentType, preview=True, storageLocationId=None, forceRestart=False):
    ...
    """
    upload_request = {'contentMD5Hex':md5, 
     'fileName':filename, 
    ...
    return DictObject(**syn.restPOST(uri=('/file/multipart?forceRestart=%s' % forceRestart), body=(json.dumps(upload_request)),
...
def _get_presigned_urls(syn, uploadId, parts_to_upload):
    ...
    presigned_url_batch = syn.restPOST(uri, body=(json.dumps(presigned_url_request)), endpoint=(syn.fileHandleEndpoint))
...
def multipart_upload_file(syn, filepath, filename=None, contentType=None, storageLocationId=None, **kwargs):
    ...
    syn.logger.debug('Initiating multi-part upload for file: [{path}] size={size} md5={md5}, contentType={contentType}'.format(path=filepath,
      size=fileSize,
      md5=md5,
      contentType=contentType))

    def get_chunk_function(n, partSize):
        return get_file_chunk(filepath, n, partSize)

    status = _multipart_upload(syn, filename, contentType, get_chunk_function=get_chunk_function, 
     md5=md5, 
     fileSize=fileSize, 
...
def multipart_upload_string(syn, text, filename=None, contentType=None, storageLocationId=None, **kwargs):
    ...
    def get_chunk_function(n, partSize):
        return get_data_chunk(data, n, partSize)
...
def _upload_chunk(part, completed, status, syn, filename, get_chunk_function, fileSize, partSize, t0, expired, bytes_already_uploaded=0):
    ...
    try:
        ...
        add_part_response = _add_part(syn, uploadId=(status.uploadId), partNumber=partNumber,
        ...
        if isinstance(ex1, SynapseHTTPError):
            if ex1.response.status_code == 403:
    ...
    try:
        while retries < MAX_RETRIES:
            ...
            def chunk_upload(part):
                return _upload_chunk(part, completed=completed, status=status, syn=syn,
                  filename=filename,
            ...
            if completed.value >= fileSize:
                try:
                    syn.logger.debug('attempting to finalize multipart upload because completed.value >= filesize ({completed} >= {size})'.format(completed=(completed.value),
                      size=fileSize))
