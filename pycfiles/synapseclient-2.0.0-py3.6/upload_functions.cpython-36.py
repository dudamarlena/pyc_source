...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/synapseclient/core/upload/upload_functions.py
# Compiled at: 2020-03-23 17:17:03
# Size of source mod 2**32: 7113 bytes
import os, urllib.parse as urllib_parse
...
def upload_file_handle(syn, parent_entity, path, synapseStore=True, md5=None, file_size=None, mimetype=None):
    ...
    if path is None:
        ...
        if not synapseStore:
            ...
            if upload_destination_type == concrete_types.SYNAPSE_S3_UPLOAD_DESTINATION or upload_destination_type == concrete_types.EXTERNAL_S3_UPLOAD_DESTINATION:
            ...
            if upload_destination_type == concrete_types.EXTERNAL_UPLOAD_DESTINATION:
                ...
                if upload_destination_type == concrete_types.EXTERNAL_OBJECT_STORE_UPLOAD_DESTINATION:
                    syn.logger.info('\n%s\n%s\nUploading to endpoint: [%s] bucket: [%s]\n%s\n' % (
                     '#' * 50, location.get('banner', ''), location.get('endpointUrl'), location.get('bucket'),
                     '#' * 50))
                    return upload_client_auth_s3(syn, expanded_upload_path, (location['bucket']), (location['endpointUrl']), (location['keyPrefixUUID']),
                      (location['storageLocationId']), mimetype=mimetype)
    ...
    if is_url(url):
        ...
        if parsed_url.scheme == 'file':
            if os.path.isfile(parsed_url.path):
                ...
                if md5 is not None:
                    if md5 != actual_md5:
                        raise SynapseMd5MismatchError('The specified md5 [%s] does not match the calculated md5 [%s] for local file [%s]', md5, actual_md5, parsed_url.path)
                md5 = actual_md5
...
def upload_external_file_handle_sftp(syn, file_path, sftp_url, mimetype=None):
    ...
    file_handle = syn._createExternalFileHandle(uploaded_url, mimetype=mimetype, md5=(md5_for_file(file_path).hexdigest()),
