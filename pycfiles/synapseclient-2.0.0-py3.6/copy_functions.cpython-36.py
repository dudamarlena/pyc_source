...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/synapseutils/copy_functions.py
# Compiled at: 2020-03-23 17:17:03
# Size of source mod 2**32: 33408 bytes
...
def copyFileHandles(syn, fileHandles, associateObjectTypes, associateObjectIds, newContentTypes=None, newFileNames=None):
    ...
    """
    if not (len(fileHandles) == len(associateObjectTypes) == len(associateObjectIds) and (newContentTypes is None or len(newContentTypes) == len(associateObjectIds)) and (newFileNames is None or len(newFileNames) == len(associateObjectIds))):
    ...
    master_copy_results_list = []
    for batch_file_handles_ids, batch_assoc_obj_types, batch_assoc_obj_ids, batch_con_type, batch_file_name in _batch_iterator_generator([file_handle_ids, associateObjectTypes, associateObjectIds,
     newContentTypes, newFileNames], MAX_FILE_HANDLE_PER_COPY_REQUEST):
        batch_copy_results = _copy_file_handles_batch(syn, batch_file_handles_ids, batch_assoc_obj_types, batch_assoc_obj_ids, batch_con_type, batch_file_name)
...
def _copy_file_handles_batch(self, file_handle_ids, obj_types, obj_ids, new_con_types, new_file_names):
    ...
    :return:                        List of batch filehandle copy results, can include failureCodes: UNAUTHORIZED and
                                    NOT_FOUND
    """
    copy_file_handle_request = _create_batch_file_handle_copy_request(file_handle_ids, obj_types, obj_ids, new_con_types, new_file_names)
    copied_file_handles = self.restPOST('/filehandles/copy', body=(json.dumps(copy_file_handle_request)), endpoint=(self.fileHandleEndpoint))
...
def _create_batch_file_handle_copy_request(file_handle_ids, obj_types, obj_ids, new_con_types, new_file_names):
    ...
    for file_handle_id, obj_type, obj_id, new_con_type, new_file_name in itertools.zip_longest(file_handle_ids, obj_types, obj_ids, new_con_types, new_file_names):
        curr_dict = {'originalFile':{'fileHandleId':file_handle_id, 
...
def _copy_cached_file_handles(cache, copiedFileHandles):
    for copy_result in copiedFileHandles:
        if copy_result.get('failureCode') is None:
...
def copy(syn, entity, destinationId, skipCopyWikiPage=False, skipCopyAnnotations=False, **kwargs):
    ...
    if not skipCopyWikiPage:
        for oldEnt in mapping:
            copyWiki(syn, oldEnt, (mapping[oldEnt]), entitySubPageId=entitySubPageId, destinationSubPageId=destinationSubPageId,
...
def _copyRecursive(syn, entity, destinationId, mapping=None, skipCopyAnnotations=False, **kwargs):
    ...
    if isinstance(ent, Project) or isinstance(ent, Folder):
            ...
            if isinstance(ent, Folder):
                ...
                if isinstance(ent, File):
                    if 'file' not in excludeTypes:
                        copiedId = _copyFile(syn, (ent.id), destinationId, version=version, updateExisting=updateExisting, setProvenance=setProvenance,
                          skipCopyAnnotations=skipCopyAnnotations)
            if isinstance(ent, Link):
                if 'link' not in excludeTypes:
                    copiedId = _copyLink(syn, (ent.id), destinationId, updateExisting=updateExisting)
            if isinstance(ent, Schema):
...
def _copyFile(syn, entity, destinationId, version=None, updateExisting=False, setProvenance='traceback', skipCopyAnnotations=False):
    ...
    existingEntity = updateExisting or syn.findEntityId((ent.name), parent=destinationId)
    if existingEntity is not None:
        ...
        if setProvenance == 'traceback':
            ...
            if setProvenance == 'existing':
                ...
                if setProvenance is None or setProvenance.lower() == 'none':
                    act = None
                else:
                    raise ValueError('setProvenance must be one of None, existing, or traceback')
        bundle = syn._getEntityBundle((ent.id), version=(ent.versionNumber), bitFlags=2049)
    ...
    if destinationSubPageId:
        ...
        for wikiHeader in oldWikiHeaders:
            ...
            if not wiki.get('attachmentFileHandleIds'):
                ...
                nopreviews = [attach['fileHandle'] for attach in results if not attach['fileHandle']['isPreview']]
            ...
            if hasattr(wikiHeader, 'parentId'):
                newWikiPage = Wiki(owner=newOwn, title=(wiki.get('title', '')), markdown=(wiki.markdown), fileHandles=new_file_handles,
                  parentWikiId=(wikiIdMap[wiki.parentWikiId]))
                newWikiPage = syn.store(newWikiPage)
            elif destinationSubPageId is not None:
                if newWikiPage is not None:
