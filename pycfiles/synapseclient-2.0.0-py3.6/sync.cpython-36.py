...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/synapseutils/sync.py
# Compiled at: 2020-03-23 17:17:03
# Size of source mod 2**32: 18166 bytes
import csv, errno
...
def _extract_file_entity_metadata(syn, allFiles):
    ...
    for entity in allFiles:
        row = {'parent':entity['parentId'], 
         'path':entity.get('path'),  'name':entity.name,  'synapseStore':entity.synapseStore, 
...
def _get_file_entity_provenance_dict(syn, entity):
    ...
    try:
        ...
        return {'used':';'.join(prov._getUsedStringList()),  'executed':';'.join(prov._getExecutedStringList()), 
...
def _sortAndFixProvenance(syn, df):
        ...
        if item is None:
            ...
            if os.path.isfile(item_path_normalized):
                ...
                if not utils.is_url(item):
                    if utils.is_synapse_id(item) is None:
    ...
    for path, row in df.iterrows():
        allRefs = []
        if 'used' in row:
            used = row['used'].split(';') if row['used'].strip() != '' else []
