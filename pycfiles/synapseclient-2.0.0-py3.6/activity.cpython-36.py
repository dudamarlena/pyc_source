...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/synapseclient/activity.py
# Compiled at: 2020-03-23 17:17:03
# Size of source mod 2**32: 13200 bytes
"""
**********
Provenance
**********
...
def is_used_entity(x):
    ...
    if not isinstance(x, collections.Mapping) or 'reference' not in x or 'targetId' not in x['reference']:
        return False
    else:
...
def _get_any_bad_args(badargs, dictionary):
    ...
    return list(illegal for illegal in badargs if illegal in dictionary if dictionary[illegal] is not None)
        ...
        if is_used_entity(target):
            ...
            if is_synapse_entity(target):
                ...
                if url:
                    ...
                    _raise_incorrect_used_usage(badargs, 'URL')
                    resource = {'url':url, 
                     'name':name if name else target,  'concreteType':'org.sagebionetworks.repo.model.provenance.UsedURL'}
                else:
                    if is_url(target):
                        ...
                        if isinstance(target, str):
                            ...
                            if len(vals) == 2:
                                if targetVersion:
                                    if int(targetVersion) != int(vals[1]):
