...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/synapseclient/annotations.py
# Compiled at: 2020-03-23 17:17:03
# Size of source mod 2**32: 12390 bytes
"""
***********
Annotations
***********
...
def is_synapse_annotations(annotations):
    ...
    keys = [
     'id', 'etag', 'creationDate', 'uri', 'stringAnnotations', 'longAnnotations', 'doubleAnnotations',
        ...
        for key, value in annotations.items():
            if key in ('id', 'etag', 'blobAnnotations', 'creationDate', 'uri'):
                synapseAnnos[key] = value
                         ...
                         'dateAnnotations') and isinstance(value, collections.Mapping):
                ...
                if all(isinstance(elem, str) for elem in elements):
                    ...
                    if all(isinstance(elem, float) for elem in elements):
                        synapseAnnos.setdefault('doubleAnnotations', {})[key] = elements
                    else:
                        if all(is_date(elem) for elem in elements):
...
def is_submission_status_annotations(annotations):
    ...
    keys = [
     'objectId', 'scopeId', 'stringAnnos', 'longAnnos', 'doubleAnnos']
    if not isinstance(annotations, collections.Mapping):
        return False
    else:
        return all([key in keys for key in annotations.keys()])
    ...
    if is_submission_status_annotations(annotations):
        ...
        for key, value in annotations.items():
            if key in ('objectId', 'scopeId', 'stringAnnos', 'longAnnos', 'doubleAnnos'):
                ...
                synapseAnnos.setdefault('stringAnnos', []).append({'key':key, 
                 'value':str(value).lower(),  'isPrivate':is_private})
            elif isinstance(value, int):
                synapseAnnos.setdefault('longAnnos', []).append({'key':key, 
                 'value':value,  'isPrivate':is_private})
            elif isinstance(value, float):
                synapseAnnos.setdefault('doubleAnnos', []).append({'key':key, 
                 'value':value,  'isPrivate':is_private})
            else:
                if isinstance(value, str):
                    ...
                    if is_date(value):
                        synapseAnnos.setdefault('longAnnos', []).append({'key':key, 
                         'value':to_unix_epoch_time(value),  'isPrivate':is_private})
                    else:
                        synapseAnnos.setdefault('stringAnnos', []).append({'key':key, 
                         'value':str(value),  'isPrivate':is_private})
...
def set_privacy(annotations, key, is_private=True, value_types=['longAnnos', 'doubleAnnos', 'stringAnnos']):
    ...
    See the `Annotations documentation     <http://docs.synapse.org/rest/org/sagebionetworks/repo/model/annotation/Annotations.html>`_ and the docs regarding
