...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\seeed_python_ircamera.py
# Compiled at: 2019-12-16 01:39:51
# Size of source mod 2**32: 11042 bytes
import sys, threading
from serial import Serial
from PyQt5.QtWidgets import QApplication, QGraphicsView, QGraphicsScene, QGraphicsPixmapItem, QGraphicsTextItem, QGraphicsEllipseItem, QGraphicsLineItem, QGraphicsBlurEffect
        ...
        while True:
            ...
            for i in range(0, 768):
                ...
                if i < len(hetData) and isDigital(hetData[i]):
                    ...
                    rightPointIndex = i + 1
                    if belowPointIndex < 768 and curCol != 0:
...
class painter(QGraphicsView):
    ...
    def __init__(self):
        ...
        self.ellipseItem = QGraphicsEllipseItem(0, 0, self.ellipseRadius * 2, self.ellipseRadius * 2)
        self.horLineItem = QGraphicsLineItem(0, 0, self.anchorLineSize, 0)
        self.verLineItem = QGraphicsLineItem(0, 0, 0, self.anchorLineSize)
        self.ellipseItem.setPos(centerX - self.ellipseRadius, centerY - self.ellipseRadius)
    ...
    def draw(self):
        ...
        p.fillRect(0, 0, self.width, self.height + self.textLineHeight, QBrush(QColor(Qt.black)))
        color = QColor()
        for yIndex in range(int(self.height / self.pixelSize)):
            for xIndex in range(int(self.width / self.pixelSize)):
                ...
                p.fillRect(xIndex * self.pixelSize, yIndex * self.pixelSize, self.pixelSize, self.pixelSize, QBrush(color))
                index = index + 1
