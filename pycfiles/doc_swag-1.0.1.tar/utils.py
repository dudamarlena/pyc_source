...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-dgUSur/flasgger/flasgger/utils.py
    ...
    def resolve_path(function, filepath):
        if not filepath.startswith('/'):
            if not hasattr(function, 'root_path'):
                function.root_path = os.path.dirname(function.__globals__['__file__'])
...
def validate(data, schema_id, filepath, root=None):
    ...
    definitions = {}
    main_def = {}
    raw_definitions = _extract_definitions(params, endpoint=endpoint, verb=verb)
