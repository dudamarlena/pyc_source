...
# [GCC 8.4.0]
# Embedded file name: /home/administrator/envs/ums/local/lib/python2.7/site-packages/delhivery_flasgger/base.py
# Compiled at: 2017-08-03 11:58:24
"""
What's the big idea?
        ...
        for item in alist:
            ...
            if schema is not None:
                ...
                if properties is not None:
                    defs += _extract_definitions(properties.values(), level + 1, endpoint, verb)
...
class SpecsView(MethodView):
    ...
    def get(self):
        ...
        specs = [ {'url': url_for(('.').join((base_endpoint, spec['endpoint']))), 'title': spec.get('title'), 'version': spec.get('version'), 'endpoint': spec.get('endpoint')} for spec in self.config.get('specs', [])
                ]
        return jsonify({'specs': specs, 'title': self.config.get('title', 'Flasgger')})


def is_valid_dispatch_view(endpoint):
    klass = endpoint.__dict__.get('view_class', None)
    return klass and hasattr(klass, 'dispatch_request') and hasattr(endpoint, 'methods')
...
class OutputView(MethodView):
    ...
    def get(self):
        data = {'swagger': self.config.get('swagger_version', '2.0'), 
           'info': {'version': self.spec.get('version', '0.0.0'), 
        ...
        for rule in self.get_url_mappings(self.spec.get('rule_filter')):
            ...
            for verb, method in methods.items():
                ...
                summary, description, swag = _parse_docstring(method, self.process_doc, endpoint=rule.endpoint, verb=verb)
                if swag is not None:
                    defs = swag.get('definitions', [])
                    defs = _extract_definitions(defs, endpoint=rule.endpoint, verb=verb)
                    params = swag.get('parameters', [])
                    defs += _extract_definitions(params, endpoint=rule.endpoint, verb=verb)
                    responses = swag.get('responses', {})
                    responses = {str(key):value for key, value in responses.items()}
                    if responses is not None:
                        defs = defs + _extract_definitions(responses.values(), endpoint=rule.endpoint, verb=verb)
...
class Swagger(object):
    ...
    def register_views(self, app):
        blueprint = Blueprint(self.config.get('endpoint', 'swagger'), __name__, url_prefix=self.config.get('url_prefix', None), subdomain=self.config.get('subdomain', None), template_folder=self.config.get('template_folder', 'templates'), static_folder=self.config.get('static_folder', 'static'), static_url_path=self.config.get('static_url_path', None))
        for spec in self.config['specs']:
            self.endpoints.append(spec['endpoint'])
            blueprint.add_url_rule(spec['route'], spec['endpoint'], view_func=OutputView().as_view(spec['endpoint'], view_args=dict(app=app, config=self.config, spec=spec, sanitizer=self.sanitizer, template=self.template)))

        blueprint.add_url_rule(self.config.get('specs_route', '/specs'), 'specs', view_func=SpecsView().as_view('specs', view_args=dict(config=self.config)))
