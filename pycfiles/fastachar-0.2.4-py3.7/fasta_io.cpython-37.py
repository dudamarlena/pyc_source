...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fastachar/fasta_io.py
# Compiled at: 2020-05-08 08:01:24
# Size of source mod 2**32: 26983 bytes
from collections import defaultdict
from itertools import zip_longest
import os, re, sys, xlwt
from .fasta_logic import Sequence, State
OK = 0
ERROR_FILE_NOT_FOUND = 1
ERROR_FILE_INVALID = 2
ERROR_IO = 3
ERROR_NO_CASE_DATA = 4
ERROR_UNEQUAL_SEQS = 5
ERROR_CASE = 7
ERROR_UNKNOWN = 15
...
class Alignment(object):
    ...
    def set_fasta_hdr_fmt(self, header_format='{ID}[_ ]{SPECIES}', IDregex='[A-Za-z0-9_]+[0-9\\.]+[A-Za-z0-9]*', SPECIESregex='[A-Za-z_]+'):
        """
    ...
    def generate_regex_dict(self, header_format, IDregex, SPECIESregex):
        ...
        if '{ID}' not in header_format:
            header_format += '{ID}'
            IDregex = ''
        pattern_dict = dict(ID=IDregex, SPECIES=SPECIESregex, HEADER=header_format, SEP=(header_format.replace('{ID}', '').replace('{SPECIES}', '')))
        regex_dict = dict(header=(re.compile((header_format.format)(**pattern_dict))), ID_first=(header_format.index('ID') < header_format.index('SPECIES')))
    ...
    def load(self, fn):
        ...
        if not error:
            if not self.are_sequences_of_equal_lengths(sequences):
    ...
    def parse_hdr(self, hdr, **kwds):
        ...
        **kwds
        ...
        if regex_dict['header'].match(s) is None:
            raise ValueError('Parsing error of header.\nHeader: {header}\nFormat: {hformat}\nPattern: {pattern}\n'.format(header=(hdr.strip()), hformat=(pattern_dict['HEADER']),
              pattern=(regex_dict['header'].pattern)))
        elif regex_dict['ID_first']:
        ...
        IDstring, species = regex or s.split(regexSep.group())
        if not regex_dict['ID_first']:
    ...
    def select_sequences(self, regex, invert=False, exclude=None):
        ...
        if not exclude:
            ...
            if invert:
                group = [i for i in self.sequences if c.match(i.species) if x.match(i.species)]
            else:
                group = [i for i in self.sequences if c.match(i.species) if not x.match(i.species)]
        return group
...
class ReportXLS(object):
    ...
    def report_mdcs_summary(self, set_A, set_B, mdcs, method):
        ...
        for j, mdc in enumerate(mdcs):
            position = mdc[0]
            self.sheet.write(n + j, 1, '%d' % (position + 1))
            for i, sp in enumerate(spA):
                state = A[sp][position]
                self.sheet.write(n + j, 2 + i, '%s' % state.state)

            for i, sp in enumerate(spB):
                state = B[sp][position]
                self.sheet.write(n + j, lenA + 2 + i, '%s' % state.state)
