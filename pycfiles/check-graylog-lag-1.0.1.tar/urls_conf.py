...
# Embedded file name: /usr/local/lib/python2.7/site-packages/check_docking/utility/urls_conf.py
# Compiled at: 2015-02-05 08:42:47
__doc__ = '从指定urls.py文件中读取REST API地址, 生成url数据相关文件.\n\n    仅针对django项目, 其它项目需要修改适配标识, 与request.get_full_path()相比, 缺少斜杠.\n    用于对REST API的客户端参数传入验证; 用于对某项接口的数据组装.\n'
                        ...
                        for line in lines:
                            try:
                                ...
                                if line.startswith(prefix):
                                    url = line[len(prefix):line.index(suffix)]
