...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fsdb/fsdb.py
# Compiled at: 2016-03-25 11:09:21
from __future__ import unicode_literals
import os, sys, errno, stat, unicodedata, logging
from . import config
...
class Fsdb(object):
    ...
    def __init__(self, fsdbRoot, depth=None, hash_alg=None, fmode=None, dmode=None):
        ...
        Args:
            ...
            fmode  -- string reppresenting the mask (octal)               to use for files creation (default: "660")
            dmode  -- string reppresenting the mask (octal)               to use for folders creation (default depends on fmode)
        """
        self.logger = logging.getLogger(__name__)
        fsdbRoot = os.path.expanduser(fsdbRoot)
        fsdbRoot = os.path.expandvars(fsdbRoot)
        fsdbRoot = os.path.normpath(fsdbRoot)
        fsdbRoot = os.path.abspath(fsdbRoot)
        if sys.version_info[0] == 3 or isinstance(fsdbRoot, unicode):
            fsdbRoot = unicodedata.normalize(b'NFC', fsdbRoot)
        configPath = os.path.join(fsdbRoot, Fsdb.CONFIG_FILE)
        conf = config.get_defaults()
        if Fsdb.config_exists(fsdbRoot):
            self.logger.debug(b'Fsdb config file found. Runtime parameters will be ignored. [' + configPath + b']')
        ...
        self.logger.debug(b'Fsdb initialized successfully: ' + self.__str__())
        return
    ...
    def _calc_digest(self, origin):
        ...
        """
        if hasattr(origin, b'read') and hasattr(origin, b'seek'):
            pos = origin.tell()
            digest = hashtools.calc_digest(origin, algorithm=self._conf[b'hash_alg'])
            origin.seek(pos)
        else:
            digest = hashtools.calc_file_digest(origin, algorithm=self._conf[b'hash_alg'])
        return digest
    ...
    def _copy_content(self, origin, dstPath):
        ...
        """
        if hasattr(origin, b'read'):
            copy_content(origin, dstPath, self.BLOCK_SIZE, self._conf[b'fmode'])
        elif os.path.isfile(origin):
            with open(origin, b'rb') as (f):
                copy_content(f, dstPath, self.BLOCK_SIZE, self._conf[b'fmode'])
        else:
            raise ValueError(b'Could not copy content, `origin` should be a path or a readable object')

    def _create_empty_file(self, path):
        oldmask = os.umask(0)
        try:
            fd = os.open(path, os.O_CREAT | os.O_WRONLY, self._conf[b'fmode'])
            os.close(fd)
        finally:
            os.umask(oldmask)
    ...
    def _makedirs(self, path):
        ...
        try:
            oldmask = os.umask(0)
            os.makedirs(path, self._conf[b'dmode'])
            os.umask(oldmask)
        except OSError as e:
            if e.errno == errno.EACCES:
                raise Exception((b'not sufficent permissions to write on fsdb folder: "{0}"').format(path))
            elif e.errno == errno.EEXIST:
                fstat = os.stat(path)
                if not stat.S_ISDIR(fstat.st_mode):
                    raise Exception((b'fsdb folder already exists but it is not a regular folder: "{0}"').format(path))
    ...
    def add(self, origin):
        ...
        if self.exists(digest):
            self.logger.debug((b'Added File: [{0}] ( Already exists. Skipping transfer)').format(digest))
            return digest
        ...
        self.logger.debug((b'Added file: "{0}" [{1}]').format(digest, absPath))
        return digest
    ...
    def remove(self, digest):
        ...
        while tmpPath != self.fsdbRoot:
            if os.path.islink(tmpPath):
                raise Exception((b'fsdb found a link in db tree: "{0}"').format(tmpPath))
            if len(os.listdir(tmpPath)) > 0:
                break
            os.rmdir(tmpPath)
            tmpPath = os.path.dirname(tmpPath)

        self.logger.debug((b'Removed file: "{0}" [{1}]').format(absPath, digest))
    ...
    def exists(self, digest):
        ...
        if not isinstance(digest, string_types):
            raise TypeError(b'digest must be a string')
    ...
    def get_file_path(self, digest):
        ...
        """
        relPath = Fsdb.generate_tree_path(digest, self._conf[b'depth'])
    ...
    def check(self, digest):
        ...
        if self._calc_digest(path) != digest:
            self.logger.warning((b"found corrupted file: '{0}'").format(path))
            return False
        return True
    ...
    def __iter__(self, overPath=False):
        ...
        for dirpath, dirnames, filenames in os.walk(self.fsdbRoot):
            ...
            if rel_dirpath.count(os.sep) + 1 != self._conf[b'depth']:
                continue
            for f in filenames:
                if overPath:
                    yield os.path.join(self.fsdbRoot, rel_dirpath, f)
                else:
                    yield (rel_dirpath + f).replace(os.sep, b'')

    def __str__(self):
        return b'{root: ' + self.fsdbRoot + b', depth: ' + str(self._conf[b'depth']) + b', hash_alg: ' + self._conf[b'hash_alg'] + b', fmode: ' + str(oct(self._conf[b'fmode'])) + b', dmode: ' + str(oct(self._conf[b'dmode'])) + b'}'
    ...
    def __getitem__(self, digest):
        ...
        if not self.exists(digest):
            raise KeyError((b"no stored file found for '{0}'").format(digest))
        return open(self.get_file_path(digest), b'rb')
    ...
    def generate_tree_path(fileDigest, depth):
        ...
        """
        if depth < 0:
            raise Exception(b'depth level can not be negative')
        if os.path.split(fileDigest)[1] != fileDigest:
            raise Exception(b'fileDigest cannot contain path separator')
        min = 2 ** (depth + 1) - 1
        if len(fileDigest) < min:
            raise Exception(b'fileDigest too short for the given depth')
        path = b''
        index = 0
    ...
    def config_exists(fsdbRoot):
        ...
        try:
            ...
            if e.errno == errno.EACCES:
                raise Exception((b'not sufficent permissions to stat fsdb config file: "{0}"').format(path))
            else:
