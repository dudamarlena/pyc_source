...
# [GCC 8.4.0]
# Embedded file name: C:\Users\User\Documents\GitHub\fourMs\MGT-python\musicalgestures\_cropvideo.py
# Compiled at: 2020-04-26 14:42:42
...
def find_motion_box(grayimage, width, height, motion_box_margin):
    ...
    for j in range(width):
        ...
        the_box[np.clip(te - margin, 0, height - 1),
         np.clip(le - margin, 0, width - 1):np.clip(re + margin, 0, width - 1)] = 1
        the_box[
         np.clip(te - margin, 0, height - 1):np.clip(be + margin, 0, height - 1), np.clip(le - margin, 0, width - 1)] = 1
        the_box[np.clip(be + margin, 0, height - 1),
         np.clip(le - margin, 0, width - 1):np.clip(re + margin, 0, width - 1)] = 1
        the_box[
         np.clip(te - margin, 0, height - 1):np.clip(be + margin, 0, height - 1), np.clip(re + margin, 0, width - 1)] = 1
        return (
         the_box, x_start, x_stop, y_start, y_stop)
    ...
    while vid2findbox.isOpened():
        ...
        if ret == True:
            ...
            motion_frame = filter_frame(motion_frame,
              'Regular', thresh=motion_box_thresh, kernel_size=5)
...
# not in loop:
#	break
#      L.  79       200  BREAK_LOOP          204  'to 204'