...
# [GCC 8.4.0]
# Embedded file name: C:\Users\User\Documents\GitHub\fourMs\MGT-python\mgmodule\_input_test.py
...
class InputError(Error):
    __doc__ = 'Exception raised for errors in the input.\n\n    Attributes\n    ----------\n    - message : str\n\n        Explanation of the error.\n    '
        ...
        if filtertype.lower() not in ('regular', 'binary', 'blob'):
            ...
            if blur.lower() not in ('average', 'none'):
                ...
                if not isinstance(thresh, (float, int)):
                    ...
                    raise InputError(msg)
                msg = isinstance(starttime, (float, int)) or 'Please specify a starttime as a float.'
                raise InputError(msg)
            msg = isinstance(endtime, (float, int)) or 'Please specify a endtime as a float.'
            raise InputError(msg)
        msg = isinstance(skip, int) or 'Please specify a skip as an integer of frames you wish to skip (Max = N frames).'
        raise InputError(msg)
    else:
