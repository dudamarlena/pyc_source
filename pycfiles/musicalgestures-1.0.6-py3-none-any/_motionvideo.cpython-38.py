...
# [GCC 8.4.0]
# Embedded file name: C:\Users\User\Documents\GitHub\fourMs\MGT-python\musicalgestures\_motionvideo.py
# Compiled at: 2020-04-26 14:43:44
# Size of source mod 2**32: 13629 bytes
...
def mg_motionvideo(self, filtertype='Regular', thresh=0.05, blur='None', kernel_size=5, inverted_motionvideo=False, inverted_motiongram=False, unit='seconds', equalize_motiongram=True, save_plot=True, save_data=True, data_format='csv', save_motiongrams=True, save_video=True):
    """
    ...
    if save_plot | save_data | save_motiongrams | save_video:
        ...
        if save_video:
            ...
            out = cv2.VideoWriter(self.of + '_motion' + self.fex, fourcc, self.fps, (self.width, self.height))
        elif save_motiongrams:
            ...
            if save_data | save_plot:
                ...
                com = np.array([])
            ii = 0
            pgbar_text = 'Rendering motion' + ', '.join(np.array(['-video', '-grams', '-plots', '-data'])[np.array([save_video, save_motiongrams, save_plot, save_data])]) + ':'
        ...
        if save_data:
            save_txt(self.of, time, com, qom, self.width, self.height, data_format)
        if save_plot:
...
def save_txt(of, time, com, qom, width, height, data_format):
    ...
    def save_single_file(of, time, com, qom, width, height, data_format):
        ...
        df = pd.DataFrame({'Time':time,  'Qom':qom,  'ComX':com.transpose()[0] / width, 
         'ComY':com.transpose()[1] / height})
        if data_format == 'tsv':
            with open(of + '_motion.tsv', 'wb') as (f):
                f.write(b'Time\tQom\tComX\tComY\n')
                np.savetxt(f, (df.values), delimiter='\t', fmt=[
                 '%d', '%d', '%.15f', '%.15f'])
        else:
            if data_format == 'csv':
                ...
                if data_format == 'txt':
                    ...
                    if data_format not in ('tsv', 'csv', 'txt'):
    ...
    if type(data_format) == str:
        ...
        if type(data_format) == list:
            if all([item.lower() in ('csv', 'tsv', 'txt') for item in data_format]):
                ...
                [save_single_file(of, time, com, qom, width, height, item) for item in data_format]
            else:
