...
# [GCC 8.4.0]
# Embedded file name: /Users/pythonicninja/PycharmProjects/sufler/sufler/base.py
# Compiled at: 2018-03-23 10:42:55
# Size of source mod 2**32: 4663 bytes
import logging, os, re, subprocess, yaml
...
def get_files_autocomplete(already_typed):
    ...
    """
    if os.path.exists(already_typed):
        if os.path.isfile(already_typed):
            return [
             already_typed]
    path = '/'.join(already_typed.split('/')[:-1])
    try:
        res = list(map(lambda file: file + '/' if os.path.isdir(file) else file, os.listdir('{0}/{1}'.format(os.getcwd(), path))))
    except OSError:
        res = [
         already_typed]
        ...
        for i, argument in enumerate(rest_arguments):
            ...
            for key in current_keys:
                ...
                if key.startswith('<File'):
                    ...
                    root.update({file_matching:rest_of_tree for file_matching in files_matching})
                if key.startswith('<Exec'):
                    try:
                        autocomplete_from_command = subprocess.check_output((key.replace('<Exec>', '')),
                    ...
                    root.update({item:rest_of_tree for item in autocomplete_from_command.split('\n')})
                if key.startswith('<Regex'):
                    if next_argument:
                        pattern = re.compile(key.replace('<Regex>', ''))
                        rest_of_tree = root.pop(key)
                        if re.search(pattern, next_argument):
                            root.update({next_argument: rest_of_tree})
                    if key.startswith('<Run'):
                        end_command = key.replace('<Run>', '')
                        print('&>/dev/null |  sufler run \\"{end_command}\\"'.format(end_command=end_command))
                        root = {}
