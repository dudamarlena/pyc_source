...
# [GCC 8.4.0]
# Embedded file name: /Users/pythonicninja/PycharmProjects/sufler/sufler/cli.py
# Compiled at: 2018-03-29 02:35:58
# Size of source mod 2**32: 14630 bytes
import logging, os, shutil, subprocess, sys, zipfile, click, click_log, requests, yaml
...
COMMAND_FOR_SHELL = {'bash':'\ncomplete -F _completer -o default {0}', 
 'zsh':'\ncomplete -F _completer -o default {0}', 
 'fish':'\ncomplete --command {0} --arguments \'({1} "{2}/backends/fish/fish.py" (commandline -cp))\' -f'}
CONFIG_DATA = {'repos': [
           {'url': 'https://github.com/limebrains/sufler-completions/archive/master.zip'}]}

class BaseShell(object):
    __doc__ = ' Template class of shells for install\n\n    '
    ...
    def get_install_path(self):
        ...
        user_path = str(input('Path to shell completions directory not found. Please enter own path to {0} completions:\n'.format(self.shell_name)))
        if user_path:
...
class Bash(BashZshInstallCommandsMixin, BaseShell):
    ...
    def initialize(self):
        ...
        with open(SUFLER_BASE_PATH + '/backends/bash/completer', 'r') as (f):
            completer_content = f.read()
        completer_content = completer_content.format(python_executable=(sys.executable),
...
class Zsh(BashZshInstallCommandsMixin, BaseShell):
    ...
    def initialize(self):
        ...
        completer_content = completer_content.format(bash_content=bash_completer_content.format(python_executable=(sys.executable),
          python_script_path=(bash_backend_path + '/bash.py')))
        logger.debug('Write completer content to file ' + self.install_file_path)
        with open(str(self.install_file_path), 'w') as (f):
            f.write(completer_content)
        logger.debug('Add completer path to startup script ' + self.install_file_path)
...
class Fish(BaseShell):
    ...
    def install(self, commands):
        ...
        current_installed_commands = [file for file in os.listdir(self.install_path) if file.endswith('.fish')]
        not_installed_commands = commands_not_installed(commands, current_installed_commands)
        for command in not_installed_commands:
            logger.debug('Create fish script for ' + command)
            command_completer_file = '{0}{1}.fish'.format(self.install_path, command)
            logger.debug('Write script file in ' + command_completer_file)
            with open(command_completer_file, 'w') as (f):
                f.write(COMMAND_FOR_SHELL[self.shell_name].format(command, sys.executable, SUFLER_BASE_PATH))
    ...
    def install(self, commands):
        ...
        if not os.path.exists(self.install_file_path):
            ...
            if not os.path.exists(autoloader_path):
                logger.debug('Make folder for startup script ' + autoloader_path)
                command = 'mkdir {0}'.format(autoloader_path)
                subprocess.check_output(command, shell=True)
            command = 'echo ". {0}" >> {1}'.format(completer_script_path, str(self.install_file_path))
            subprocess.check_output(command, shell=True)
        completer_file = '{0}/backends/powershell/completer'.format(SUFLER_BASE_PATH)
...
def commands_not_installed(commands, completer_content):
    ...
    """
    return [command for command in commands if command not in completer_content]
...
def get_commands(name):
    ...
    completions_path_files = [file for file in os.listdir(completions_path) if file.endswith('.yml')]
    logger.debug('Completion files from sufler home ' + ', '.join(completions_path_files))
    if name:
        if os.path.isfile('{0}{1}.yml'.format(completions_path, name)):
            return [
             name]
    return [str(command).split('/')[(-1)][:-4] for command in completions_path_files]
...
def install_completion_files():
    ...
    completions_not_installed = set(zip_completions_files).difference(sufler_completions_files)
    logger.debug('Copy files')
    for file in completions_not_installed:
        logger.debug('Install completion file ' + file)
        shutil.copyfile('{0}/{1}'.format(zip_completions_path, file), os.path.expanduser('~/.sufler/completions/' + file))
