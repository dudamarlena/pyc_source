...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/Yass/yass/cli.py
# Compiled at: 2017-03-28 06:53:44
...
def publish(endpoint, purge_files, rebuild_manifest, skip_upload):
    ...
    if target == 's3':
        p = publisher.S3Website(sitename=sitename, aws_access_key_id=endpoint.get('aws_access_key_id'), aws_secret_access_key=endpoint.get('aws_secret_access_key'), region=endpoint.get('aws_region'))
...
def setup_dns(endpoint):
    ...
    endpoint = yass.config.get('hosting.%s' % target)
    if not endpoint:
        raise ValueError('%s endpoint is missing in the hosting config' % target.upper())
    if target == 's3':
        p = publisher.S3Website(sitename=sitename, aws_access_key_id=endpoint.get('aws_access_key_id'), aws_secret_access_key=endpoint.get('aws_secret_access_key'), region=endpoint.get('aws_region'))
