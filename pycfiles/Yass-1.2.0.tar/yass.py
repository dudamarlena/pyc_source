...
# Compiled at: 2017-03-28 08:27:30
"""
~ Yass ~
"""
...
class Yass(object):
    ...
    def _init_jinja(self, global_context={}):
        ...
        self.tpl_env = jinja2.Environment(loader=loader, extensions=[
    ...
    def _get_dest_file_and_url(self, filepath, page_meta={}):
        ...
        fname = slugify(slug) if slug else filename.replace('.html', '').replace('.md', '').replace('.jade', '')
    ...
    def publish(self, target='S3', sitename=None, purge_files=True):
        ...
        if target.upper() == 'S3':
            p = publisher.S3Website(sitename=sitename or self.config.get('sitename'), aws_access_key_id=endpoint.get('aws_access_key_id'), aws_secret_access_key=endpoint.get('aws_secret_access_key'), region=endpoint.get('aws_region'))
