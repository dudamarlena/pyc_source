...
# [GCC 8.4.0]
# Embedded file name: /Users/me/projects/keystone.git/tmp/keystone.git/llvm/utils/llvm-build/llvmbuild/componentinfo.py
...
class LibraryComponentInfo(ComponentInfo):
    ...
    def parse_items(items):
        ...
        kwargs['add_to_library_groups'] = items.get_list('add_to_library_groups')
    ...
    def __init__(self, subpath, name, dependencies, parent, library_name, required_libraries, add_to_library_groups, installed):
    ...
    def get_llvmbuild_fragment(self):
        ...
        if self.add_to_library_groups:
            result += 'add_to_library_groups = %s\n' % (' ').join(self.add_to_library_groups)
...
class OptionalLibraryComponentInfo(LibraryComponentInfo):
    ...
    def __init__(self, subpath, name, dependencies, parent, library_name, required_libraries, add_to_library_groups, installed):
        LibraryComponentInfo.__init__(self, subpath, name, dependencies, parent, library_name, required_libraries, add_to_library_groups, installed)
...
class LibraryGroupComponentInfo(ComponentInfo):
    ...
    def __init__(self, subpath, name, parent, required_libraries=[], add_to_library_groups=[]):
        ComponentInfo.__init__(self, subpath, name, [], parent)
        self.required_libraries = list(required_libraries)
        self.add_to_library_groups = list(add_to_library_groups)
...
class TargetGroupComponentInfo(ComponentInfo):
    ...
    def parse(subpath, items):
        ...
        kwargs['has_asmprinter'] = items.get_optional_bool('has_asmprinter', False)
        kwargs['has_asmparser'] = items.get_optional_bool('has_asmparser', False)
        kwargs['has_disassembler'] = items.get_optional_bool('has_disassembler', False)
        return TargetGroupComponentInfo(subpath, **kwargs)

    def __init__(self, subpath, name, parent, required_libraries=[], add_to_library_groups=[], has_jit=False, has_asmprinter=False, has_asmparser=False, has_disassembler=False):
        ComponentInfo.__init__(self, subpath, name, [], parent)
        self.required_libraries = list(required_libraries)
        self.add_to_library_groups = list(add_to_library_groups)
    ...
    def get_llvmbuild_fragment(self):
        ...
        if self.add_to_library_groups:
            result += 'add_to_library_groups = %s\n' % (' ').join(self.add_to_library_groups)
        for bool_key in ('has_asmparser', 'has_asmprinter', 'has_disassembler', 'has_jit'):
...
class ToolComponentInfo(ComponentInfo):
    ...
    def __init__(self, subpath, name, dependencies, parent, required_libraries):
        ComponentInfo.__init__(self, subpath, name, dependencies, parent)
        self.required_libraries = list(required_libraries)
...
_component_type_map = dict((t.type_name, t) for t in (GroupComponentInfo,
...
def _read_components_from_parser(parser, path, subpath):
    for section in parser.sections():
        ...
        try:
            info = type_class.parse(subpath, IniFormatParser(parser.items(section)))
        except TypeError:
