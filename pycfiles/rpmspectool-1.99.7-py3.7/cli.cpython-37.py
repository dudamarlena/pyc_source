...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/rpmspectool/cli.py
# Compiled at: 2019-12-10 10:25:49
# Size of source mod 2**32: 7769 bytes
import argcomplete, argparse, atexit, logging
...
class CLI(object):
    ...
    def get_arg_parser(self, args=None):
        ...
        action_parser.add_argument('--define',
        ...
        action_parser.add_argument('specfile',
          type=(argparse.FileType('rb')), help=(_('The RPM spec file to read')))
        get_cmd = commands.add_parser('get',
          parents=[action_parser], help=(_('Download files')))
        get_cmd.add_argument('--insecure', action='store', default=False)
        get_cmd.add_argument('--force',
          '-f', action='store_true', default=False)
        get_cmd.add_argument('--dry-run',
          '--dryrun', '-n', action='store_true', default=False)
        ...
        commands.add_parser('list',
          parents=[action_parser], help=(_('List files')))
        version_cmd = commands.add_parser('version',
          help=(_('Show rpmspectool version')))
    ...
    def main(self):
        ...
        if not getattr(args, 'cmd'):
            ...
            if args.cmd == 'version':
                print('{prog} {version}'.format(prog=(sys.argv[0]),
                  version=version))
            else:
                parsed_spec_path = os.path.join(self.tmpdir, 'rpmspectool-' + os.path.basename(self.args.specfile.name))
                ...
                sources, patches = self.filter_sources_patches(args, specfile_res['sources'], specfile_res['patches'])
                if args.cmd == 'list':
                        ...
                        for what in (sources, patches):
                            for i in sorted(what):
                                ...
                                if is_url(url):
                                    try:
                                        download(url,
                                          where=where, dry_run=(args.dry_run), insecure=(args.insecure),
                                          force=(args.force))
                                        ...
                                        try:
                                            log_error('{}: {}'.format(e.args[1], getattr(e, 'filename2', e.filename)))
                                            return 1
                                        finally:
                                            e = None
                                            del e
