...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/autocommit.py
# Compiled at: 2020-01-31 11:05:37
...
import dulwich, numpy as np
commit_styles = [
 'All files in one commit', 'Every file in a single commit']
all_files = '*ALL'
...
class GitUiOpts:
    ...
    def get_staged(self):
        ...
        for type_file in ('delete', 'add', 'modify'):
            for filepath in staged[type_file]:
                self.staged.append({'type':type_file, 
    ...
    def commit_file(self):
        ...
        for file_to_commit in self.staged:
            commit_title = '{} {}'.format(file_to_commit['type'], file_to_commit['path'].split('/')[(-1)])
            self.repo.do_commit((commit_title.encode()),
              committer=(self.committer.encode()))
            print(commit_title)
    ...
    def push_once(self):
        ...
        porcelain.push((self.repo),
          remote_location=remote_url, refspecs='master')
        self.staged = []
        ...
        while is_pushed is False:
            try:
                try:
                    ...
                    self.remote_url_credentials = '//{0}:{1}@'.format(username, password).join(self.remote_url.split('//'))
                    self.push_once()
    ...
    def select_input(self, keyword, message, choices):
        questions = [
         inquirer.List(keyword, message=message,
           choices=choices)]
...
def main():
    ...
    while True:
        ...
        if find_executable('git') is not None:
            ...
            ap.committer = ap.simple_input(content='Who is the author ?\n eg: "PABlond <pierre-alexis.blond@live.fr>"')
        ap.get_unstaged()
        if len(ap.unstaged) == 0:
            sys.exit()
        file_to_commit = ap.select_input(keyword='file',
          message='What file do you want to commit?',
          choices=([all_files] + ap.unstaged))
        if file_to_commit == all_files:
            commit_style = ap.select_input(keyword='style',
              message='How do you want to commit unstaged files?',
              choices=commit_styles)
            if commit_style == commit_styles[0]:
                title = ap.simple_input(content='Choose a title for your commit : ')
