...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyglet/font/ttf.py
...
class TruetypeInfo():
    ...
    _name_id_lookup = {'copyright': 0, 
       ...
       'compatible-name': 18, 
       'sample': 19}
    _platform_id_lookup = {'unicode': 0, 
       'macintosh': 1, 
       'iso': 2, 
       'microsoft': 3, 
       'custom': 4}
    _microsoft_encoding_lookup = {1: 'utf_16_be', 
       2: 'shift_jis', 
       4: 'big5', 
       6: 'johab', 
       10: 'utf_16_be'}
    ...
    def __init__(self, filename):
        ...
        if hasattr(mmap, 'MAP_SHARED'):
            self._data = mmap.mmap(self._fileno, len, mmap.MAP_SHARED, mmap.PROT_READ)
        else:
    ...
    def get_character_advances(self):
        ...
        for i in range(len(ga)):
            if i in gmap and gmap[i] not in self._character_advances:
    ...
    def _add_kernings_format0(self, kernings, offset):
        ...
        for pair in kerning_pairs:
            if (
             pair.left, pair.right) in kernings:
    ...
    def _get_character_map_format4(self, offset):
        ...
        start_count = self._read_array('>%dH' % seg_count, offset + header.size + array_size + 2)
        id_delta = self._read_array('>%dh' % seg_count, offset + header.size + array_size + 2 + array_size)
        id_range_offset_address = offset + header.size + array_size + 2 + array_size + array_size
        id_range_offset = self._read_array('>%dH' % seg_count, id_range_offset_address)
        character_map = {}
...
def _read_table(*entries):
    ...
    class _table_class:
        ...
        def __repr__(self):
            s = '{' + (', ').join([ '%s = %s' % (name, value) for (name, value) in self.pairs ]) + '}'
            return s
...
_read_offset_table = _read_table('scalertype:I', 'num_tables:H', 'search_range:H', 'entry_selector:H', 'range_shift:H')
_read_table_directory_entry = _read_table('tag:4s', 'check_sum:I', 'offset:I', 'length:I')
_read_head_table = _read_table('version:i', 'font_revision:i', 'check_sum_adjustment:L', 'magic_number:L', 'flags:H', 'units_per_em:H', 'created:Q', 'modified:Q', 'x_min:h', 'y_min:h', 'x_max:h', 'y_max:h', 'mac_style:H', 'lowest_rec_p_pEM:H', 'font_direction_hint:h', 'index_to_loc_format:h', 'glyph_data_format:h')
...
_read_kern_subtable_header.horizontal_mask = 1
_read_kern_subtable_header.minimum_mask = 2
_read_kern_subtable_header.perpendicular_mask = 4
_read_kern_subtable_header.override_mask = 5
_read_kern_subtable_header.format_mask = 240
_read_kern_subtable_format0 = _read_table('n_pairs:H', 'search_range:H', 'entry_selector:H', 'range_shift:H')
_read_kern_subtable_format0Pair = _read_table('left:H', 'right:H', 'value:h')
_read_cmap_header = _read_table('version:H', 'num_tables:H')
_read_cmap_encoding_record = _read_table('platform_id:H', 'encoding_id:H', 'offset:L')
_read_cmap_format_header = _read_table('format:H', 'length:H')
_read_cmap_format4Header = _read_table('format:H', 'length:H', 'language:H', 'seg_count_x2:H', 'search_range:H', 'entry_selector:H', 'range_shift:H')
_read_horizontal_header = _read_table('version:i', 'Advance:h', 'Descender:h', 'LineGap:h', 'advance_width_max:H', 'min_left_side_bearing:h', 'min_right_side_bearing:h', 'x_max_extent:h', 'caret_slope_rise:h', 'caret_slope_run:h', 'caret_offset:h', 'reserved1:h', 'reserved2:h', 'reserved3:h', 'reserved4:h', 'metric_data_format:h', 'number_of_h_metrics:H')
_read_long_hor_metric = _read_table('advance_width:H', 'lsb:h')
_read_naming_table = _read_table('format:H', 'count:H', 'string_offset:H')
_read_name_record = _read_table('platform_id:H', 'encoding_id:H', 'language_id:H', 'name_id:H', 'length:H', 'offset:H')