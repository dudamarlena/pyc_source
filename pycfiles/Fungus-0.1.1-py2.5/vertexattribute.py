...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyglet/graphics/vertexattribute.py
...
def create_attribute(format):
    ...
    if generic_index:
        ...
        if attr_class._fixed_count:
            assert count == attr_class._fixed_count, 'Attributes named "%s" must have count of %d' % (
...
class EdgeFlagAttribute(AbstractAttribute):
    ...
    def __init__(self, gl_type):
        assert gl_type in (GL_BYTE, GL_UNSIGNED_BYTE, GL_BOOL), 'Edge flag attribute must have boolean type'
...
class NormalAttribute(AbstractAttribute):
    ...
    def __init__(self, gl_type):
        assert gl_type in (GL_BYTE, GL_SHORT, GL_INT, GL_FLOAT, GL_DOUBLE), 'Normal attribute must have signed type'
...
class TexCoordAttribute(AbstractAttribute):
    ...
    def __init__(self, count, gl_type):
        assert gl_type in (GL_SHORT, GL_INT, GL_INT, GL_FLOAT, GL_DOUBLE), 'Texture coord attribute must have non-byte signed type'
...
class VertexAttribute(AbstractAttribute):
    ...
    def __init__(self, count, gl_type):
        ...
        assert gl_type in (GL_SHORT, GL_INT, GL_INT, GL_FLOAT, GL_DOUBLE), 'Vertex attribute must have signed type larger than byte'
