...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/collective/kss/flygui/browser/foldercontents.py
...
class Table(BaseTable):
    ...
    def __init__(self, context, request, base_url, view_url, items, show_sort_column=False, buttons=[]):
        BaseTable.__init__(self, request, base_url, view_url, items, show_sort_column, buttons)
        self.context = context
        self.portal_state = getMultiAdapter((self.context, self.request), name='plone_portal_state')
        self.portal = self.portal_state.portal()
        self.portal_url = self.portal.absolute_url()
        self.sort_on = request.get('sort_on', FolderContentsTable.default_sort_on)
...
class FolderContentsTable(foldercontents.FolderContentsTable):
    ...
    def __init__(self, context, request, contentFilter={}):
        ...
        self.table = Table(self.context, request, url, view_url, self.items, show_sort_column=self.show_sort_column, buttons=self.buttons)
    ...
    def items(self):
        ...
        for (i, obj) in enumerate(contentsMethod(self.contentFilter)):
            ...
            results.append(dict(url=url, id=obj.getId, quoted_id=urllib.quote_plus(obj.getId), path=path, title_or_id=obj.pretty_title_or_id(), description=obj.Description, obj_type=obj_type, size=obj.getObjSize, modified=modified, icon=icon.html_tag(), type_class=type_class, wf_state=review_state, state_title=portal_workflow.getTitleForStateOnType(review_state, obj_type), state_class=state_class, is_browser_default=is_browser_default, folderish=obj.is_folderish, relative_url=relative_url, view_url=view_url, is_expired=self.context.isExpired(obj), brain=obj, uid=obj.UID, item_manager=ItemManager(obj, self.request)))
