...
# [GCC 8.4.0]
# Embedded file name: /home/luke/PycharmProjects/untitled/NearBeach/forms_special_fields.py
    ...
    def _render(self, name, value, attrs=None, choices=()):
        ...
        for idx, row in enumerate(customer_results):
            if row.organisation_id:
                output = output + '<tr><td><input type="checkbox" id="id_customers_%s" name="customers" value="%s"></td>' % (
                 str(idx), str(row.customer_id)) + '<td>%s</td>' % row.customer_first_name + '<td>%s</td>' % row.customer_last_name + '<td>%s</td>' % str(row.organisation_id) + '</tr>'
            else:
                output = output + '<tr><td><input type="checkbox" id="id_customers_%s" name="customers" value="%s"></td>' % (
                 str(row.customer_id), str(row.customer_id)) + '<td>%s</td>' % row.customer_first_name + '<td>%s</td>' % row.customer_last_name + '<td>%s</td>' % str(row.organisation_id) + '</tr>'
        else:
            output = output + '</table>'
            return output
...
class ConnectOrganisationSelect(forms.SelectMultiple):
    ...
    def _render(self, name, value, attrs=None, choices=()):
        ...
        for idx, row in enumerate(organisation_results):
            output = output + '<tr><td><input type="checkbox" id="id_organisations_%s" name="organisations" value="%s"></td>' % (
             str(row.organisation_id), str(row.organisation_id)) + '<td>%s</td>' % row.organisation_name + '<td>%s</td>' % row.organisation_website + '<td>%s</td>' % str(row.organisation_email) + '</tr>'
        else:
            output = output + '</table>'
            return output
