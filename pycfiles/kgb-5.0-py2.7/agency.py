...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/kgb/agency.py
...
class SpyAgency(object):
    ...
    def unspy(self, func):
        ...
        try:
            ...
            raise ValueError(b'Function %r has not been spied on.' % func)
    ...
    def assertHasSpy(self, spy):
        ...
        """
        if not hasattr(spy, b'spy') and not isinstance(spy, FunctionSpy):
            self.fail(b'%s has not been spied on.' % self._format_spy_or_call(spy))
    ...
    def assertSpyCalled(self, spy):
        ...
        if not spy.called:
            self.fail(b'%s was not called.' % self._format_spy_or_call(spy))
    ...
    def assertSpyNotCalled(self, spy):
        ...
        if spy.called:
            ...
            if call_count == 1:
                msg = b'%s was called 1 time:' % self._format_spy_or_call(spy)
            else:
                msg = b'%s was called %d times:' % (
    ...
    def assertSpyCallCount(self, spy, count):
        ...
        if call_count != count:
            if call_count == 1:
                msg = b'%s was called %d time, not %d.'
            else:
                msg = b'%s was called %d times, not %d.'
            self.fail(msg % (self._format_spy_or_call(spy), call_count,
             count))

    def assertSpyCalledWith(self, spy_or_call, *expected_args, **expected_kwargs):
        ...
        if not spy_or_call.called_with(*expected_args, **expected_kwargs):
            if isinstance(spy_or_call, SpyCall):
                self.fail(b'This call to %s was not passed args=%s, kwargs=%s.\n\nIt was called with:\n\n%s' % (
                ...
                self.fail(b'No call to %s was passed args=%s, kwargs=%s.\n\nThe following calls were recorded:\n\n%s' % (
                 self._format_spy_or_call(spy_or_call),
                 safe_repr(expected_args),
                 safe_repr(expected_kwargs),
                 self._format_spy_calls(spy_or_call, self._format_spy_call_args)))
    ...
    def assertSpyLastCalledWith(self, spy, *expected_args, **expected_kwargs):
        ...
        if not spy.last_called_with(*expected_args, **expected_kwargs):
            self.fail(b'The last call to %s was not passed args=%s, kwargs=%s.\n\nIt was last called with:\n\n%s' % (
    ...
    def assertSpyReturned(self, spy_or_call, return_value):
        ...
        if not spy_or_call.returned(return_value):
            if isinstance(spy_or_call, SpyCall):
                self.fail(b'This call to %s did not return %s.\n\nIt returned:\n\n%s' % (
                 self._format_spy_or_call(spy_or_call),
                 safe_repr(return_value),
                 self._format_spy_call_returned(spy_or_call)))
            else:
                self.fail(b'No call to %s returned %s.\n\nThe following values have been returned:\n\n%s' % (
                 self._format_spy_or_call(spy_or_call),
                 safe_repr(return_value),
                 self._format_spy_calls(spy_or_call, self._format_spy_call_returned)))
    ...
    def assertSpyLastReturned(self, spy, return_value):
        ...
        if not spy.last_returned(return_value):
            self.fail(b'The last call to %s did not return %s.\n\nIt last returned:\n\n%s' % (
             self._format_spy_or_call(spy),
             safe_repr(return_value),
        ...
        if not spy_or_call.raised(exception_cls):
            if isinstance(spy_or_call, SpyCall):
                if spy_or_call.exception is not None:
                    self.fail(b'This call to %s did not raise %s. It raised %s.' % (
                     self._format_spy_or_call(spy_or_call),
                     exception_cls.__name__,
                     self._format_spy_call_raised(spy_or_call)))
                else:
                    self.fail(b'This call to %s did not raise an exception.' % self._format_spy_or_call(spy_or_call))
            else:
                has_raised = any(call.exception is not None for call in spy_or_call.calls)
                if has_raised:
                    self.fail(b'No call to %s raised %s.\n\nThe following exceptions have been raised:\n\n%s' % (
                     self._format_spy_or_call(spy_or_call),
                     exception_cls.__name__,
                     self._format_spy_calls(spy_or_call, self._format_spy_call_raised)))
                else:
                    self.fail(b'No call to %s raised an exception.' % self._format_spy_or_call(spy_or_call))
        return
    ...
    def assertSpyLastRaised(self, spy, exception_cls):
        ...
        if not spy.last_raised(exception_cls):
            if spy.last_call.exception is not None:
                self.fail(b'The last call to %s did not raise %s. It last raised %s.' % (
                 self._format_spy_or_call(spy),
                 exception_cls.__name__,
                 self._format_spy_call_raised(spy.last_call)))
            else:
                self.fail(b'The last call to %s did not raise an exception.' % self._format_spy_or_call(spy))
        return
    ...
    def assertSpyRaisedMessage(self, spy_or_call, exception_cls, message):
        ...
        if not spy_or_call.raised_with_message(exception_cls, message):
            if isinstance(spy_or_call, SpyCall):
                if spy_or_call.exception is not None:
                    self.fail(b'This call to %s did not raise %s with message %r.\n\nIt raised:\n\n%s' % (
                     self._format_spy_or_call(spy_or_call),
                     exception_cls.__name__,
                     message,
                     self._format_spy_call_raised_with_message(spy_or_call)))
                else:
                    self.fail(b'This call to %s did not raise an exception.' % self._format_spy_or_call(spy_or_call))
            else:
                has_raised = any(call.exception is not None for call in spy_or_call.calls)
                if has_raised:
                    self.fail(b'No call to %s raised %s with message %r.\n\nThe following exceptions have been raised:\n\n%s' % (
                     self._format_spy_or_call(spy_or_call),
                     exception_cls.__name__,
                     message,
                     self._format_spy_calls(spy_or_call, self._format_spy_call_raised_with_message)))
                else:
                    self.fail(b'No call to %s raised an exception.' % self._format_spy_or_call(spy_or_call))
        return
    ...
    def assertSpyLastRaisedMessage(self, spy, exception_cls, message):
        ...
        if not spy.last_raised_with_message(exception_cls, message):
            if spy.last_call.exception is not None:
                self.fail(b'The last call to %s did not raise %s with message %r.\n\nIt last raised:\n\n%s' % (
                 self._format_spy_or_call(spy),
                 exception_cls.__name__,
                 message,
                 self._format_spy_call_raised_with_message(spy.last_call)))
            else:
                self.fail(b'The last call to %s did not raise an exception.' % self._format_spy_or_call(spy))
        return
    ...
    def _format_spy_calls(self, spy, formatter):
        ...
        """
        return (b'\n\n').join(b'Call %d:\n%s' % (i, formatter(call, indent=2)) for i, call in enumerate(spy.calls))
    ...
    def _format_spy_call_args(self, call, indent=0):
        ...
        """
        return b'%s\n%s' % (
         self._format_spy_lines(call.args, prefix=b'args=', indent=indent),
         self._format_spy_lines(call.kwargs, prefix=b'kwargs=', indent=indent))
    ...
    def _format_spy_call_returned(self, call, indent=0):
        ...
        """
        return self._format_spy_lines(call.return_value, indent=indent)
    ...
    def _format_spy_call_raised(self, call, indent=0):
        ...
        """
        return self._format_spy_lines(call.exception.__class__.__name__, indent=indent, format_data=False)
    ...
    def _format_spy_call_raised_with_message(self, call, indent=0):
        ...
        """
        return b'%s\n%s' % (
         self._format_spy_lines(call.exception.__class__.__name__, prefix=b'exception=', indent=indent, format_data=False),
         self._format_spy_lines(str(call.exception), prefix=b'message=', indent=indent))

    def _format_spy_lines(self, data, prefix=b'', indent=0, format_data=True):
        ...
        lines = [b'%s%s%s' % (indent_str, prefix, data_lines[0])]
        if len(data_lines) > 1:
            indent_str = b' ' * (indent + len(prefix))
            lines += [ b'%s%s' % (indent_str, line) for line in data_lines[1:]
                     ]
        return (b'\n').join(lines)