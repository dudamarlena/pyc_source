...
class BaseFunctionSig(object):
    ...
    def is_compatible_with(self, other_sig):
        ...
        if compat_args_name and compat_kwargs_name:
            return True
        if source_args_name and not compat_args_name or source_kwargs_name and not compat_kwargs_name:
            return False
        ...
        if len(source_args) != len(compat_args) and (len(source_args) < len(compat_args) and not source_args_name and not compat_kwargs.issuperset(source_args) or len(source_args) > len(compat_args) and not compat_args_name):
            return False
        if not compat_all_args.issuperset(self.kwarg_names) and not compat_kwargs_name:
            return False
        return True
    ...
    def format_forward_call_args(self):
        ...
        result = [ _format_arg(arg_name) for arg_name in self.arg_names
                 ] + [ b'%s=%s' % (arg_name, _format_arg(arg_name)) for arg_name in self.kwarg_names
                     ]
        if self.args_param_name:
            result.append(b'*%s' % _format_arg(self.args_param_name))
        if self.kwargs_param_name:
            result.append(b'**%s' % _format_arg(self.kwargs_param_name))
    ...
    def finalize_state(self):
        ...
        if self.owner is None:
            ...
            try:
                self.defined_func = get_defined_attr_value(self.owner, self.func_name)
        ...
        if not isinstance(self.defined_func, (types.FunctionType,
         ...
         classmethod,
         staticmethod)):
            if hasattr(self.defined_func, b'__get__'):
                self.has_getter = True
            if hasattr(self.defined_func, b'__set__'):
...
class FunctionSigPy2(BaseFunctionSig):
    ...
    """
    FUNC_CLOSURE_ATTR = b'func_closure'
    FUNC_CODE_ATTR = b'func_code'
    FUNC_DEFAULTS_ATTR = b'func_defaults'
    FUNC_GLOBALS_ATTR = b'func_globals'
    FUNC_NAME_ATTR = b'func_name'
    METHOD_SELF_ATTR = b'im_self'
    ...
    def __init__(self, func, owner=_UNSET_ARG):
        ...
        """
        super(FunctionSigPy2, self).__init__(func=func, owner=owner)
        ...
        if inspect.ismethod(func):
            ...
            if method_owner is None:
                ...
                if owner is _UNSET_ARG:
                    logger.warning(b'Unbound method owners can easily be determined on Python 2.x, but not on 3.x. Please pass owner= to spy_on() to set a specific owner for %r.', func)
            else:
            ...
            if owner_func is not None and (owner_func is func or owner_func.func_code is func.func_code):
    ...
    def format_arg_spec(self):
        ...
        Returns:
            ...
            A string representing an argument list for a function definition.
        """
        return inspect.formatargspec(args=self.all_arg_names, varargs=self.args_param_name, varkw=self.kwargs_param_name, defaults=self._defaults, formatvalue=lambda value: b'=_UNSET_ARG')[1:-1]
...
class FunctionSigPy3(BaseFunctionSig):
    ...
    through Python 3.8.
    """
    FUNC_CLOSURE_ATTR = b'__closure__'
    FUNC_CODE_ATTR = b'__code__'
    FUNC_DEFAULTS_ATTR = b'__defaults__'
    FUNC_GLOBALS_ATTR = b'__globals__'
    FUNC_NAME_ATTR = b'__name__'
    METHOD_SELF_ATTR = b'__self__'
    ...
    def __init__(self, func, owner=_UNSET_ARG):
        ...
        """
        super(FunctionSigPy3, self).__init__(func=func, owner=owner)
        if not hasattr(inspect, b'_signature_from_callable'):
            raise InternalKGBError(b'Python %s.%s does not have inspect._signature_from_callable, which is needed in order to generate a Signature from a function.' % sys.version_info[:2])
        func_name = self.func_name
        if inspect.ismethod(func):
            self.func_type = self.TYPE_BOUND_METHOD
            self.owner = func.__self__
        else:
            if b'.' in func.__qualname__:
                if owner is not _UNSET_ARG:
                    ...
                    if owner is _UNSET_ARG or inspect.isclass(owner):
                        ...
                        self.func_type = self.TYPE_BOUND_METHOD
                elif b'<locals>' in func.__qualname__:
                    self.func_type = self.TYPE_FUNCTION
                else:
                    real_func = self.real_func
                    method_owner = inspect.getmodule(real_func)
                    for part in real_func.__qualname__.split(b'.')[:-1]:
                        try:
                    ...
                    if method_owner is not None:
                        self.func_type = self.TYPE_UNBOUND_METHOD
                        self.owner = method_owner
                    logger.warning(b'Determined the owner of %r to be %r, but it may be wrong. Please pass owner= to spy_on() to set a specific owner.', func, self.owner)
            sig = inspect._signature_from_callable(func, follow_wrapper_chains=False, skip_bound_arg=False, sigcls=inspect.Signature)
            all_args = []
            args = []
            kwargs = []
    ...
    def format_forward_call_arg(self, arg_name):
        ...
        Returns:
            ...
            The string used to format the argument call.
        """
        return b'_kgb_l["%(arg)s"] if "%(arg)s" in _kgb_l else %(arg)s' % {b'arg': arg_name}
    ...
    def format_arg_spec(self):
        ...
        for orig_param in self._sig.parameters.values():
            ...
            if orig_param.kind is orig_param.POSITIONAL_OR_KEYWORD and default is not orig_param.empty:
                default = _UNSET_ARG
            parameters.append(inspect.Parameter(name=orig_param.name, kind=orig_param.kind, default=default))
...
if sys.version_info[0] == 2:
    ...
    raise Exception(b'Unsupported Python version')