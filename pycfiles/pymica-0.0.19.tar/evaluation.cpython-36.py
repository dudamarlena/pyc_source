...
# Embedded file name: build/bdist.linux-x86_64/egg/pymic/util/evaluation.py
# Compiled at: 2020-04-15 21:57:06
# Size of source mod 2**32: 12542 bytes
from __future__ import absolute_import, print_function
import os, sys, math, random, configparser, numpy as np
...
def binary_hausdorff95(s, g, spacing=None):
    ...
    dis1 = hausdorff95_from_one_surface_to_another(point_list_s, point_list_g, spacing)
    dis2 = hausdorff95_from_one_surface_to_another(point_list_g, point_list_s, spacing)
...
def average_surface_distance(point_list_s, point_list_g, spacing):
    ...
    elif not (len(point_list_s[0]) == len(point_list_g[0]) and len(point_list_s[0]) == len(spacing)):
        raise AssertionError
    for ps in point_list_s:
        ps_nearest = 10000000000.0
        for pg in point_list_g:
            dd = spacing[0] * (ps[0] - pg[0])
            dh = spacing[1] * (ps[1] - pg[1])
            temp_dis_square = dd * dd + dh * dh
...
def evaluation(config_file):
    ...
    for s_folder in s_folder_list:
        ...
        for i in range(len(patient_names)):
            ...
            s_volume = s_dict['data_array']
            s_spacing = s_dict['spacing']
            g_volume = g_dict['data_array']
            g_spacing = g_dict['spacing']
            if ground_truth_label_convert_source is not None:
                if ground_truth_label_convert_target is not None:
                    g_volume = convert_label(g_volume, ground_truth_label_convert_source, ground_truth_label_convert_target)
            if segmentation_label_convert_source is not None:
                if segmentation_label_convert_target is not None:
                    s_volume = convert_label(s_volume, segmentation_label_convert_source, segmentation_label_convert_target)
            ...
            temp_score = get_evaluation_score(s_volume_sub > 0, g_volume_sub > 0, g_spacing, metric)
