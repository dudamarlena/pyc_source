...
# Embedded file name: build/bdist.linux-x86_64/egg/pymic/train_infer/infer_func.py
...
def volume_infer(image, net, device, class_num, mini_batch_size=None, mini_patch_inshape=None, mini_patch_outshape=None, stride=None, output_num=1):
    """
    Obtain net(image)
    ...
    if mini_patch_inshape is None:
        ...
        outputs = volume_infer_by_patch(image, net, device, class_num, mini_batch_size, mini_patch_inshape, mini_patch_outshape, stride, output_num)
    return outputs


def volume_infer_by_patch(image, net, device, class_num, mini_batch_size, mini_patch_inshape, mini_patch_outshape, stride, output_num):
    """
        Test one image with sub regions along x, y, z axis
    """
    image = image.cpu().numpy()
    img_full_shape = image.shape
    if not img_full_shape[0] == 1:
        ...
        margin = [mini_patch_inshape[i] - mini_patch_outshape[i] for i in range(img_dim)]
        assert min(margin) >= 0
        if max(margin) > 0:
    ...
    for d in range(0, padD, stride[0]):
        ...
        for h in range(0, padH, stride[1]):
            ...
            for w in range(0, padW, stride[2]):
                ...
                sub_image = crop_ND_volume_with_bounding_box(image, crop_start_full, crop_end_full)
                sub_image_patches.append(sub_image)

    out_shape = [
     img_full_shape[0], class_num] + list(image_pad.shape[2:])
    ...
    for mini_batch_idx in range(max_mini_batch):
        ...
        out_mini_batch = net(data_mini_batch)
        ...
        for batch_idx in range(batch_start_idx, batch_end_idx):
            ...
            for i in range(output_num):
                out_list[i] = set_ND_volume_roi_with_bounding_box_range(out_list[i], crop_start, crop_end, out_mini_batch[i][(batch_idx - batch_start_idx)])

            temp_mask = np.zeros_like(out_mask)
            temp_mask = set_ND_volume_roi_with_bounding_box_range(temp_mask, crop_start, crop_end, mask_mini_batch[(batch_idx - batch_start_idx)])
