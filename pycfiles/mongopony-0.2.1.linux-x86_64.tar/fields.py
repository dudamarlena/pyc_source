...
# [GCC 8.4.0]
# Embedded file name: /home/vagrant/envs/conversocial/lib/python2.7/site-packages/mongopony/fields.py
# Compiled at: 2015-09-09 07:05:45
...
class ListField(ComplexField):
    ...
    def to_object(self, values):
        if self.mapper:
            values = [ self.mapper.dict_to_object(value, None) for value in values ]
        return values
...
class DictField(ComplexField):
    ...
    def to_dict(self, instance, field_name):
        values = getattr(instance, field_name)
        values = {k:self.mapper.object_to_dict(v) for k, v in values.iteritems()}
        return values