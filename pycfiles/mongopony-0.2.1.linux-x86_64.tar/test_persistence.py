...
# [GCC 8.4.0]
# Embedded file name: /home/vagrant/envs/conversocial/lib/python2.7/site-packages/mongopony/tests/test_persistence.py
# Compiled at: 2015-09-09 05:10:36
...
class ClassFieldMapper(ClassFieldDelegator):
    name_to_mapper = {'Athlete': AthleteMapper, 
       'Person': PeopleMapper}
...
class TestPeristence(ConnectionMixin, TestCase):
    ...
    def test_write_athlete(self):
        person = AthleteMapper.create(first_name='Colin', last_name='Howe', age=28)
        People.persist(self.db, person)
        query_plan = People.prepare_query(self.db)
        query_plan.filter({'first_name': 'Colin'})
        person, = query_plan.as_list()
