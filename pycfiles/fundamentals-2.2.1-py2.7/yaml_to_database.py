...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fundamentals/mysql/yaml_to_database.py
# Compiled at: 2020-05-01 09:55:21
"""
...
import sys, os, yaml, re, glob, docopt
...
def main(arguments=None):
    ...
    if os.path.isfile(pathToYaml):
        ...
        yaml2db.add_yaml_file_content_to_database(filepath=pathToYaml, deleteFile=deleteFlag)
        basename = os.path.basename(pathToYaml)
        print('Content of %(basename)s added to database' % locals())
    else:
        from fundamentals.mysql import yaml_to_database
        yaml2db = yaml_to_database(log=log, settings=settings, dbConn=dbConn, pathToInputDir=pathToYaml, deleteFiles=deleteFlag)
        yaml2db.ingest()
...
class yaml_to_database(object):
    ...
    **Usage**
    ...
    def __init__(self, log, dbConn, pathToInputDir=False, settings=False, deleteFiles=False):
        self.log = log
        ...
        **Usage**
        ...
        for d in os.listdir(self.pathToInputDir):
            if os.path.isfile(os.path.join(self.pathToInputDir, d)) and 'yaml' in d.lower():
                self.add_yaml_file_content_to_database(filepath=os.path.join(self.pathToInputDir, d), deleteFile=self.deleteFiles)
    ...
    def add_yaml_file_content_to_database(self, filepath, deleteFile=False):
        ...
        **Usage**
        ...
        import codecs, requests, requests.packages.urllib3
        ...
        if 'table' not in yamlContent:
            self.log.warning('A table value is need in the yaml content to indicate which database table to add the content to: %(filepath)s' % locals())
            return
        else:
            ...
            if 'url' in yamlContent:
                ...
                uniqueKeyList = []
            for t in dbTables:
                convert_dictionary_to_mysql_table(dbConn=self.dbConn, log=self.log, dictionary=yamlContent, dbTableName=t, uniqueKeyList=uniqueKeyList, dateModified=True, returnInsertOnly=False, replace=True)
