...
# Compiled at: 2020-05-13 06:21:58
"""
...
def directory_script_runner(log, pathToScriptDirectory, dbConn=False, waitForResult=True, successRule=None, failureRule=None, loginPath=False, databaseName=False, force=True):
    ...
    **Usage**
    ...
    if dbConn:
        for k, v in list(scriptList.items()):
            ...
            if exception:
                log.error('The script %(scriptname)s failed with the following exception: "%(exception)s"' % locals())
                rule = failureRule
                print exception
            else:
                rule = successRule
            _process_script_file(pathToScriptDirectory=pathToScriptDirectory, scriptname=scriptname, rule=rule, log=log)
    ...
    **Usage:**
    ...
    if rule == None or rule == False:
        ...
        try:
            ...
            log.error('could not rename file %s to %s - failed with this error: %s ' % (scriptPath, moveTo, str(e)))
