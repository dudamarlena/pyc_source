...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/myproxy/ws/client/test/test_client.py
# Compiled at: 2012-09-05 07:27:50
...
class WSClientTestCase(unittest.TestCase):
    ...
    config_filepath = os.path.join(here_dir, 'test_myproxywebservice_client.cfg')
    ...
    def test02_logon_with_ssl_client_authn(self):
        ...
        myproxy_client = MyProxyWSClient()
        ssl_ctx = make_ssl_context(cert_file=client_cert_filepath, key_file=client_key_filepath, ca_dir=test_ca_dir, verify_peer=True, url=server_url, method=SSL.SSLv3_METHOD)
        res = myproxy_client.logon(username, password, server_url, ssl_ctx=ssl_ctx)
        self.assert_(res)
        pem_out = res.read()
        cert = crypto.load_certificate(crypto.FILETYPE_PEM, pem_out)
        subj = cert.get_subject()
