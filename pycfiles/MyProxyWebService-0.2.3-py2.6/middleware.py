...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/myproxy/ws/server/wsgi/middleware.py
# Compiled at: 2012-08-22 09:47:00
...
import httplib, socket, base64, traceback
from webob import Request
from OpenSSL import crypto
from myproxy.client import MyProxyClient, MyProxyClientError
from myproxy.server.wsgi.middleware import MyProxyClientMiddlewareBase, MyProxyClientMiddleware
...
class MyProxyLogonWSMiddleware(MyProxyClientMiddleware):
    ...
    DEFAULT_LOGON_FUNC_ENV_KEYNAME = 'myproxy.server.wsgi.middleware.MyProxyClientMiddleware.logon'
    CERT_REQ_POST_PARAM_KEYNAME = 'certificate_request'
    __slots__ = ('__logonFuncEnvironKeyName', )
    ...
    def parseConfig(self, prefix=PARAM_PREFIX, myProxyClientPrefix=None, **app_conf):
        ...
        dictionary
        """
        super(MyProxyLogonWSMiddleware, self).parseConfig(prefix=prefix, myProxyClientPrefix=myProxyClientPrefix, **app_conf)
        logonFuncEnvKeyOptName = prefix + self.__class__.LOGON_FUNC_ENV_KEYNAME_OPTNAME
        self.logonFuncEnvironKeyName = app_conf.get(logonFuncEnvKeyOptName, self.__class__.DEFAULT_LOGON_FUNC_ENV_KEYNAME)
    ...
    def logonFuncEnvironKeyName(self, value):
        ...
        if not isinstance(value, basestring):
            raise TypeError('Expecting string type for "logonFuncEnvironKeyName"; got %r type' % type(value))
    ...
    def __call__(self, environ, start_response):
        ...
        return super(MyProxyLogonWSMiddleware, self).__call__(environ, start_response)
    ...
    def myProxyLogon(self):
        ...
        def _myProxylogon(environ, start_response, username, password):
            ...
            if requestMethod != 'POST':
                ...
                raise HttpBasicAuthResponseException(response, httplib.METHOD_NOT_ALLOWED)
            ...
            if pem_cert_req is None:
                response = 'No %r form variable set in POST message' % cert_req_key
                log.error(response)
                raise HttpBasicAuthResponseException(response, httplib.BAD_REQUEST)
            log.debug('cert req = %r', pem_cert_req)
            try:
                cert_req = crypto.load_certificate_request(crypto.FILETYPE_PEM, pem_cert_req)
            except crypto.Error, e:
                log.error('Error loading input certificate request: %r', pem_cert_req)
                raise HttpBasicAuthResponseException('Error loading input certificate request', httplib.BAD_REQUEST)

            asn1CertReq = crypto.dump_certificate_request(crypto.FILETYPE_ASN1, cert_req)
            try:
                credentials = self.myProxyClient.logon(username, password, certReq=asn1CertReq)
                ...
                raise HttpBasicAuthResponseException(str(e), httplib.UNAUTHORIZED)
            except socket.error, e:
                raise MyProxyLogonWSMiddlewareError('Socket error with MyProxy server %r: %s' % (
                 self.myProxyClient.hostname, e))
            except Exception, e:
                log.error('MyProxyClient.logon raised an unknown exception calling %r: %s', self.myProxyClient.hostname, traceback.format_exc())
                raise
...
class MyProxyGetTrustRootsMiddleware(MyProxyClientMiddlewareBase):
    ...
    @classmethod
    def filter_app_factory(cls, app, global_conf, prefix=PARAM_PREFIX, **app_conf):
    ...
    def parseConfig(self, prefix=PARAM_PREFIX, **app_conf):
        ...
        self.clientEnvironKeyName = app_conf.get(clientEnvKeyOptName, self.__class__.DEFAULT_CLIENT_ENV_KEYNAME)
    ...
    def _setPath(self, value):
        ...
        """
        if not isinstance(value, basestring):
            raise TypeError('Expecting string type for "path"; got %r' % type(value))
        self.__path = value

    path = property(fget=_getPath, fset=_setPath, doc='environ SCRIPT_NAME path which invokes the getTrustRoots method on this middleware')
    ...
    def __call__(self, environ, start_response):
        ...
        if not isinstance(myProxyClient, MyProxyClient):
            raise TypeError('Expecting %r type for "myProxyClient" environ[%r] attribute got %r' % (
             MyProxyClient,
    ...
    def _getTrustRoots(cls, myProxyClient):
        ...
        try:
            ...
            response = ('\n').join([ '%s=%s' % (k, base64.b64encode(v)) for (k, v) in trustRoots.items()
                                   ])
            return response
        except MyProxyClientError, e:
            log.error('MyProxyClient.getTrustRoots raised an MyProxyClientError exception calling %r: %s', myProxyClient.hostname, traceback.format_exc())
            raise
        except socket.error, e:
            raise MyProxyGetTrustRootsMiddlewareError('Socket error with MyProxy server %r: %s' % (
             myProxyClient.hostname,
             e))
        except Exception, e:
            log.error('MyProxyClient.getTrustRoots raised an unknown exception calling %r: %s', myProxyClient.hostname, traceback.format_exc())
            raise