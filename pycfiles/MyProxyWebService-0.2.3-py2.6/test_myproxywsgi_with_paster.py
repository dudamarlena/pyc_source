...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/myproxy/ws/test/test_myproxywsgi_with_paster.py
# Compiled at: 2012-08-21 11:46:09
...
import subprocess, unittest, socket, logging
logging.basicConfig(level=logging.DEBUG)
from OpenSSL import SSL, crypto
from myproxy.ws.test import test_ca_dir, logon_shell_script_path, get_trustroots_shell_script_path
...
class MyProxyLogonAppWithPasterTestCase(unittest.TestCase):
    ...
    def __init__(self, *arg, **kw):
        ...
        self.addService(cfgFilePath=self.__class__.INI_FILEPATH, port=self.__class__.SERVICE_PORTNUM, withSSL=True, withLoggingConfig=False)
    ...
    def test01GetTrustRootsScriptWithBootstrap(self):
        ...
        try:
            proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env={self.__class__.CA_ENV_VARNAME: self.__class__.tmp_ca_dir})
        except OSError, e:
            self.failIf(e.errno == 13, 'Check that the %r script is set with execute permissions' % self.__class__.GET_TRUSTROOTS_SCRIPT_CMD)
            raise

        (stdoutdata, stderrdata) = proc.communicate()
        self.failIf(len(stderrdata) > 0, 'An error message was returned: %s' % stderrdata)
    ...
    def test02LogonScript(self):
        ...
        try:
            p2 = subprocess.Popen(cmd, stdin=p1.stdout, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env={self.__class__.CA_ENV_VARNAME: self.__class__.tmp_ca_dir})
        except OSError, e:
            self.failIf(e.errno == 13, 'Check that the %r script is set with execute permissions' % self.__class__.LOGON_SCRIPT_CMD)
            raise
    ...
    def addService(self, *arg, **kw):
        ...
        if withSSL:
            certFilePath = path.join(self.__class__.THIS_DIR, self.__class__.SSLCERT_FILEPATH)
            priKeyFilePath = path.join(self.__class__.THIS_DIR, self.__class__.SSLKEY_FILEPATH)
    ...
    def __del__(self):
        ...
        for fileName in listdir(self.__class__.CA_DIR):
            if fileName not in self.__class__.RESERVED_CA_DIR_FILENAMES and fileName[0] != '.':
