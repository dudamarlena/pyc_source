...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/myproxy/ws/server/wsgi/httpbasicauth.py
# Compiled at: 2012-08-22 09:10:30
...
import traceback, re, httplib, base64
...
class HttpBasicAuthMiddleware(object):
    ...
    @type __app: function
    """
    AUTHN_FUNC_ENV_KEYNAME = 'myproxy.server.wsgi.httpbasicauth.HttpBasicAuthMiddleware.authenticate'
    ...
    __slots__ = ('__rePathMatchList', '__authnFuncEnvironKeyName', '__realm', '__app')
    ...
    @classmethod
    def filter_app_factory(cls, app, global_conf, prefix=PARAM_PREFIX, **local_conf):
    ...
    def parseConfig(self, prefix='', **app_conf):
        ...
        dictionary
        """
        rePathMatchListOptName = prefix + HttpBasicAuthMiddleware.RE_PATH_MATCH_LIST_OPTNAME
        rePathMatchListVal = app_conf.pop(rePathMatchListOptName, '')
        self.rePathMatchList = [ re.compile(i) for i in rePathMatchListVal.split()
                               ]
        paramName = prefix + HttpBasicAuthMiddleware.AUTHN_FUNC_ENV_KEYNAME_OPTNAME
        self.authnFuncEnvironKeyName = app_conf.get(paramName, HttpBasicAuthMiddleware.AUTHN_FUNC_ENV_KEYNAME)
    ...
    def _setAuthnFuncEnvironKeyName(self, value):
        ...
        if not isinstance(value, basestring):
            raise TypeError('Expecting string type for "authnFuncEnvironKeyName"; got %r type' % type(value))
        self.__authnFuncEnvironKeyName = value

    authnFuncEnvironKeyName = property(fget=_getAuthnFuncEnvironKeyName, fset=_setAuthnFuncEnvironKeyName, doc='key name in environ for the custom authentication function used by this class')
    ...
    def _setRePathMatchList(self, value):
        ...
        if not isinstance(value, (list, tuple)):
            raise TypeError('Expecting list or tuple type for "rePathMatchList"; got %r' % type(value))
        self.__rePathMatchList = value

    rePathMatchList = property(fget=_getRePathMatchList, fset=_setRePathMatchList, doc='List of regular expressions determine the URI paths intercepted by this middleware')
    ...
    def _setRealm(self, value):
        ...
        """
        if not isinstance(value, basestring):
            raise TypeError('Expecting string type for "realm"; got %r' % type(value))
        self.__realm = value

    realm = property(fget=_getRealm, fset=_setRealm, doc='HTTP Authentication realm to set in responses')
    ...
    def _parseCredentials(self, environ):
        ...
        if basicAuthHdr is None:
            log.debug('No %r setting in environ: skipping HTTP Basic Auth', HttpBasicAuthMiddleware.AUTHZ_ENV_KEYNAME)
            return (None, None)
        else:
            (method, encodedCreds) = basicAuthHdr.split(None, 1)
            if method.lower() != HttpBasicAuthMiddleware.AUTHN_SCHEME_HDR_FIELDNAME_LOWER:
                log.debug('Auth method is %r not %r: skipping request', method, HttpBasicAuthMiddleware.AUTHN_SCHEME_HDR_FIELDNAME)
                return (None, None)
    ...
    def __call__(self, environ, start_response):
        ...
        if not self._pathMatch(environ):
            ...
            def start_response_wrapper(status, headers):
                ...
                if statusCode == httplib.UNAUTHORIZED:
                    ...
                    for (name, val) in headers:
                        if name.lower() == self.__class__.AUTHENTICATE_HDR_FIELDNAME_LOWER:
            ...
            if username is None:
                ...
                return self.setErrorResponse(start_response_wrapper, msg='No username set\n')
            authenticateFunc = environ.get(self.authnFuncEnvironKeyName)
            if authenticateFunc is None:
                raise HttpBasicAuthMiddlewareConfigError('No authentication function set in environ')
            try:
                response = authenticateFunc(environ, start_response_wrapper, username, password)
                if response is None:
                    return self.__app(environ, start_response_wrapper)
                return response
            except HttpBasicAuthResponseException, e:
                log.error('Client authentication raised an exception: %s', traceback.format_exc())
                return self.setErrorResponse(start_response_wrapper, msg=e.response, code=e.code)

            return

    @classmethod
    def setErrorResponse(cls, start_response, msg=None, code=httplib.UNAUTHORIZED, contentType=None):
