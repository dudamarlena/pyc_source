...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/myproxy/ws/test/test_myproxywsgi.py
# Compiled at: 2012-08-21 12:10:18
...
class MyProxyPasteDeployTestCaseBase(unittest.TestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        wsgiapp = loadapp('config:' + self.__class__.INI_FILENAME, relative_to=here_dir)
...
class MyProxyLogonAppTestCase(MyProxyPasteDeployTestCaseBase):
    ...
    def _createRequestCreds(self):
        ...
        pemCertReq = crypto.dump_certificate_request(crypto.FILETYPE_PEM, certReq)
        return (keyPair, pemCertReq)

    def test01Logon(self):
        username = self.cfg.get('MyProxyLogonAppTestCase.test01Logon', 'username')
        try:
            password = self.cfg.get('MyProxyLogonAppTestCase.test01Logon', 'password')
        ...
        postData = {MyProxyLogonWSMiddleware.CERT_REQ_POST_PARAM_KEYNAME: certReq}
        response = self.app.post('/logon', postData, headers=headers, status=200)
        print response
        self.assert_(response)
