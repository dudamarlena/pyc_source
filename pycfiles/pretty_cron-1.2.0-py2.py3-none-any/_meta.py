...
# Embedded file name: build/bdist.linux-x86_64/egg/pretty_bad_protocol/_meta.py
# Compiled at: 2018-07-25 20:19:59
__doc__ = 'Meta and base classes for hiding internal functions, and controlling\nattribute creation and handling.\n'
from __future__ import absolute_import
import atexit, codecs, encodings, locale, os, platform, re, shlex, subprocess, sys, threading
try:
    import psutil
except ImportError:
    psutil = None
...
class GPGBase(object):
    ...
    _result_map = {'crypt': _parsers.Crypt, 'delete': _parsers.DeleteResult, 
    ...
    def __init__(self, binary=None, home=None, keyring=None, secring=None, use_agent=False, default_preference_list=None, ignore_homedir_permissions=False, verbose=False, options=None):
        """Create a ``GPGBase``.
        ...
        **homedir** , and the paths to its **binary** and **keyring** .
        ...
        try:
            if not self.binary:
                raise AssertionError('Could not find binary %s' % binary)
                if _util._py3k:
                    assert isinstance(verbose, (bool, str, int)), "'verbose' must be boolean, string, or 0 <= n <= 9"
                else:
                    assert isinstance(verbose, (bool, str, int, unicode)), "'verbose' must be boolean, string, unicode, or 0 <= n <= 9"
                assert isinstance(use_agent, bool), "'use_agent' must be boolean"
                assert self.options is not None and isinstance(self.options, list), 'options not list'
    ...
    def __remove_path__(self, prog=None, at_exit=True):
        ...
        program_base = self.use_agent or os.path.dirname(prog)
        ...
        if not not os.environ.has_key('PATH'):
            raise AssertionError('OS env kept $PATH anyway!')
    ...
    def _check_sane_and_get_gpg_version(self):
        ...
        if proc.returncode:
            ...
            try:
                ...
                log.error('Could neither invoke nor terminate a gpg process... Are you sure you specified the corrent (and full) path to the gpg binary?')

        version_line = result.data.partition(':version:')[2].decode()
        if not version_line:
    ...
    def _make_args(self, args, passphrase=False):
        ...
        if self.verbose:
            ...
            if isinstance(self.verbose, str) or isinstance(self.verbose, int) and self.verbose >= 1:
    ...
    def _open_subprocess(self, args=None, passphrase=False):
        ...
        if platform.system() == 'Windows':
            ...
            expand_shell = False
        environment = {'LANGUAGE': os.environ.get('LANGUAGE') or 'en', 
        ...
        return subprocess.Popen(cmd, shell=expand_shell, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=environment)

    def _read_response(self, stream, result):
        u"""Reads all the stderr output from GPG, taking notice only of lines
    ...
    def _collect_output(self, process, result, writer=None, stdin=None):
        ...
        rr = threading.Thread(target=self._read_response, args=(
         stderr, result))
        rr.setDaemon(True)
    ...
    def _encrypt(self, data, recipients, default_key=None, passphrase=None, armor=True, encrypt=True, symmetric=False, always_trust=True, output=None, throw_keyids=False, hidden_recipients=None, cipher_algo='AES256', digest_algo='SHA512', compress_algo='ZLIB'):
