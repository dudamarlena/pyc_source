...
# Embedded file name: build/bdist.linux-x86_64/egg/pretty_bad_protocol/_util.py
# Compiled at: 2018-08-01 17:04:27
__doc__ = 'Extra utilities for python-gnupg.'
from __future__ import absolute_import
from datetime import datetime
from socket import gethostname
from time import localtime
from time import mktime
import codecs, encodings, os, threading, random, re, string, sys
_STREAMLIKE_TYPES = []
try:
    import io
...
_here = os.path.join(os.getcwd(), 'pretty_bad_protocol')
...
def author_info(name, contact=None, public_key=None):
    u"""Easy object-oriented representation of contributor info.
...
def _copy_data(instream, outstream):
    ...
    sent = 0
    while True:
        if _py3k and isinstance(instream, str) or not _py3k and isinstance(instream, basestring):
            data = instream[:1024]
...
def _threaded_copy_data(instream, outstream):
    ...
    :param file outstream: The file descriptor of a tmpfile to write to.
    """
    copy_thread = threading.Thread(target=_copy_data, args=(
     instream, outstream))
