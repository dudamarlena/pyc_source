...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\bip_utils\bech32.py
# Compiled at: 2020-04-16 14:20:02
# Size of source mod 2**32: 8073 bytes
from .bech32_ex import Bech32ChecksumError, Bech32FormatError
from . import utils

class Bech32Const:
    __doc__ = ' Class container for Bech32 constants. '
    CHARSET = 'qpzry9x8gf2tvdw0s3jn54khce6mua7l'
    SEPARATOR = '1'


class Bech32Utils:
    __doc__ = ' Class container for Bech32 utility functions. '
...
class Bech32Encoder:
    __doc__ = ' Bech32 encoder class. It provides methods for encoding to Bech32 format. '
    ...
    def EncodeAddr(hrp, wit_ver, wit_prog):
        ...
        Returns:
            str: Encoded address
        """
        return Bech32Encoder._Bech32Encoder__Encode(hrp, [wit_ver] + Bech32Utils.ConvertToBits(wit_prog, 8, 5))
    ...
    def __Encode(hrp, data):
        ...
        Returns:
            str: Encoded data
        """
        combined = data + Bech32Encoder._Bech32Encoder__CreateChecksum(hrp, data)
...
class Bech32Decoder:
    __doc__ = ' Bech32 decoder class. It provides methods for decoding Bech32 format. '
    ...
    def DecodeAddr(hrp, addr):
        ...
        """
        hrpgot, data = Bech32Decoder._Bech32Decoder__Decode(addr)
        if hrpgot != hrp:
        ...
        if data[0] == 0:
            if len(decoded) != 20:
                if len(decoded) != 32:
                    raise Bech32FormatError('Invalid segwit address')
        return (
         data[0], utils.ListToBytes(decoded))
    ...
    def __Decode(bech_str):
        ...
        """
        if not any((ord(x) < 33 or ord(x) > 126 for x in bech_str)):
            if bech_str.lower() != bech_str:
                if bech_str.upper() != bech_str:
            ...
            assert Bech32Decoder._Bech32Decoder__VerifyChecksum(hrp, data), 'Invalid checksum'
        return (
         hrp, data[:-6])
