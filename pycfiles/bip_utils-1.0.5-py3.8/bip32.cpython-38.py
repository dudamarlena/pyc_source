...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\bip_utils\bip32.py
# Compiled at: 2020-04-18 14:02:12
# Size of source mod 2**32: 16319 bytes
import ecdsa
...
class Bip32Const:
    __doc__ = ' Class container for BIP32 constants. '
...
class Bip32:
    __doc__ = ' BIP32 class. It allows master key generation and children keys derivation in according to BIP-0032.\n    BIP-0032 specifications: https://github.com/bitcoin/bips/blob/master/bip-0032.mediawiki\n    '
    ...
    def __init__(self, secret, chain, depth=0, index=0, fprint=Bip32Const.MASTER_FINGERPRINT, is_public=False, key_net_ver=Bip32Conf.KEY_NET_VER.Main()):
    ...
    def ChildKey(self, index):
        ...
        if not self.m_is_public:
            return self._Bip32__CkdPriv(index)
        return self._Bip32__CkdPub(index)
    ...
    def __CkdPriv(self, index):
        ...
        i_l, i_r = self._Bip32__HmacHalves(data)
        ...
        return Bip32(secret=secret, chain=i_r,
    ...
    def __CkdPub(self, index):
        ...
        try:
            ...
            return Bip32(secret=k_i, chain=i_r,
              depth=(self.m_depth + 1),
              index=index,
              fprint=(self.FingerPrint()),
              is_public=True,
