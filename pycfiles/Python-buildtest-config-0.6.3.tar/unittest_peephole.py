...
# [GCC 8.4.0]
# Embedded file name: unittest_peephole.py
# Compiled at: 2017-06-09 15:37:41
"""Tests for the astroid AST peephole optimizer."""
import ast, textwrap, unittest, astroid
...
class PeepholeOptimizer(unittest.TestCase):
    ...
    def _get_binops(code):
        ...
        return [ node.value for node in module.body if isinstance(node, ast.Expr)
               ]
    ...
    def test_optimisation_disabled(self):
        try:
            ...
            self.assertIsInstance(module.body[0].value.left.left, astroid.Const)
        finally:
            MANAGER.optimize_ast = True
