...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/iw/recipe/backup/archive.py
...
def archive_folder(location, archive, target_folder, exclude_folders, include_folders, fs_location):
...
def archive_buildout(args):
    """Archive a buildout."""
    (location, format, target_folder, log_file, exclude_folders, include_folders, fs_location, options) = args
    ...
    archive_folder(location, archive_name, target_folder, exclude_folders, include_folders, fs_location)
...
def restore_buildout(args):
    """De-archive buildout"""
    (target_folder, archives_folder, log_file, fs_location, archive_before_backup, prompt_before_restore) = args
    if len(sys.argv) != 2:
        _log('Usage: %s archive_name' % sys.argv[0])
        sys.exit(0)
    if prompt_before_restore:
        res = _get_input('Are you sure you want to restore ? Every data will be lost ! (y/N) ')
        if not res:
            sys.exit(1)
    if archive_before_backup:
        _log('Archiving current folder before restoring')
        args = [target_folder, '%(name)s-%(year)s-%(month)s-%(day)s-%(hour)s-%(minute)s', archives_folder, log_file, [], [], fs_location, {'name': 'before-restore'}]
