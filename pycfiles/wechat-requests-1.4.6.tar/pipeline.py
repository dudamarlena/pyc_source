    ...
    def handle(self, raw_message, **kwargs):
        ...
        for handler in self.handlers:
            try:
                ...
                raise MessageProcessException(handle_error.__str__(), handler=handler, raw_message=context.raw_message)
            else:
    ...
    def _pre_process(self, message, context):
        for _pre_process_handler in self.pre_process_handlers:
            try:
                ...
                raise MessageProcessException(handle_error.__str__(), handler=_pre_process_handler, raw_message=context.raw_message)
    ...
    def _post_process(self, message, context):
        for _post_process_handler in self.post_process_handlers:
            try:
                ...
                raise MessageProcessException(handle_error.__str__(), handler=_post_process_handler, raw_message=context.raw_message)
