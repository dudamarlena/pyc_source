...
class MpApi(Api):
    ...
    def _retry(self, result, method, url, params_dict, **kwargs):
        ...
        if result.is_failed and result.errcode in AUTH_EXPIRED_CODES:
            ...
            return self._execute_request(method, url, params_dict=params, **kwargs)
        else:
            return result
    ...
    def _update_auth(self, response):
        if self._auth_update_callback is None:
            ...
            try:
                ...
                raise RequestException(('Update auth failed use {}').format(self._auth_update_callback.__name__), auth_update_error, response=response)
            else:
