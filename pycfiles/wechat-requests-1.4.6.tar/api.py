...
# [GCC 8.4.0]
# Embedded file name: f:\pipeline\wechat\src\wechat\api.py
# Compiled at: 2018-05-03 05:44:15
import re, logging
...
from .settings import DEFAULT_HEADERS, TIMEOUT, RETRYS, RETRY_BACKOFF_FACTOR, RETRY_STATUS_FORCELIST
log = logging.getLogger(__name__)

def _build_retry_session(session=None):
    session = session or requests.Session()
    retry = Retry(total=RETRYS, read=RETRYS, connect=RETRYS, backoff_factor=RETRY_BACKOFF_FACTOR, status_forcelist=RETRY_STATUS_FORCELIST)
    adapter = HTTPAdapter()
...
class Api(object):
    ...
    def __init__(self, root_path='/', headers=DEFAULT_HEADERS, timeout=TIMEOUT, **kwargs):
    ...
    def request(self, method, api_path, params_dict=None, **kwargs):
        ...
        result = self._execute_request(method, url, params_dict=params_dict, **kwargs)
        if result.is_failed:
            retry_result = self._retry(result, method, url, params_dict, **kwargs)
            if retry_result is None:
                log.warn(('{} retry result is None').format(self.__class__.__name__))
                retry_result = result
            return retry_result
        return result
        return
    ...
    def get(self, api_path, timeout=None, **params):
        return self.request('GET', api_path, params_dict=params, timeout=timeout or self._timeout)

    def post(self, api_path, data=None, json=None, **params):
        return self.request('POST', api_path, data=data, json=json, **params)
    ...
    def _prepare_api_url(self, api_path):
        ...
        if '://' in api_path:
            return api_path
        uri = ('{}/{}').format(self._base_url, api_path)
        return re.sub('(?<!:)//[/]?', '/', uri)

    def _execute_request(self, method, url, params_dict=None, **kwargs):
        response = self._session.request(method, url, params=params_dict, **kwargs)
