...
# [GCC 8.4.0]
# Embedded file name: f:\pipeline\wechat\src\wechat\auth.py
# Compiled at: 2018-05-14 06:22:22
import re
from .compat import url_quote, bytes
from .api import Api
from . import settings
__all__ = [
 'get_mp_access_token', 'web_auth']
...
class MpOuthApi(Api):
    ...
    def _prepare_param_dict(self, params_dict):
        complete_params = {'appid': self._appid, 
           'secret': self._secret}
...
_web_auth_api = Api()
_web_auth_api._base_url = 'https://api.weixin.qq.com/sns/'
...
class WebAuth(object):
    ...
    @staticmethod
    def build_authgrant_url(appid, redirect_uri, state=None, scope=SCOPE_SNSAPI_USERINFO):
        ...
        if redirect_uri.find(':') > 0:
            redirect_uri = url_quote(redirect_uri, '')
        return ('https://{}/connect/oauth2/authorize?appid={}&redirect_uri={}&response_type=code&scope={}&state={}#wechat_redirect').format(settings.OAUTH_HOST, appid, redirect_uri, scope, state)

    @classmethod
    def get_access_token(cls, appid, secret, code):
        return _web_auth_api.get('oauth2/access_token', appid=appid, secret=secret, code=code, grant_type='authorization_code')

    @classmethod
    def refresh_access_token(cls, appid, refresh_token):
        return _web_auth_api.get('oauth2/refresh_token', appid=appid, refresh_token=refresh_token, grant_type='refresh_token')

    @classmethod
    def get_user_info(cls, openid, access_token, lang='zh_CN'):
        return _web_auth_api.get('userinfo', openid=openid, access_token=access_token, lang=lang)
