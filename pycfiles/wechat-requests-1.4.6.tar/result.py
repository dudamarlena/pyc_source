...
def _build_from_xml(xml_text, response):
    ...
    if 'return_code' in _json:
        if _json['return_code'] != RequestResult._XML_SUCCESS_CODE:
...
def build_from(from_x, response=None):
    ...
    if isinstance(from_x, str):
        from_x = from_x.strip()
    if from_x.startswith('{') and from_x.endswith('}'):
        try:
            return _build_from_json_str(from_x, response)
        except JSONDecodeError as json_error:
            pass

    if from_x.startswith('<xml') and from_x.endswith('</xml>'):
        try:
