...
# [GCC 8.4.0]
# Embedded file name: f:\pipeline\wechat\src\wechat\message\crypto.py
# Compiled at: 2018-05-16 08:55:05
# Size of source mod 2**32: 7462 bytes
"""
微信开放平台的公众号消息加密解密实现
...
"""
import base64, string, random, hashlib, time, struct
from Crypto.Cipher import AES
import socket
from bs4 import BeautifulSoup
from wechat.utils import serialize_dict_to_xml
from wechat.compat import unicode, str, is_py3
from .exceptions import SignatureError, InvalidAESKeyError, EncryptError, ReceiveMsgFormatError, InvalidSignature, DecryptError, InvalidAppid
__all__ = [
 'build_message_crypto_for']
...
class PKCS7Encoder(object):
    ...
    def encode(text):
        ...
        amount_to_pad = PKCS7Encoder._BLODK_SIZE - text_length % PKCS7Encoder._BLODK_SIZE
        if amount_to_pad == 0:
...
class MsgCrypt(object):
    ...
    def encrypt(self, msg, nonce=None, timestamp=None):
        ...
        if isinstance(msg, unicode):
            ...
            encryped_msg = self._MsgCrypt__encrypt(msg)
            if nonce is None:
        ...
        return serialize_dict_to_xml(Encrypt=encryped_msg,
    ...
    def decrypt(self, receive_str, signature, timestamp, nonce):
        ...
        try:
            ...
            raise ReceiveMsgFormatError('can not parse:{}'.format(receive_str))
        else:
            if type(timestamp) == int:
                timestamp = str(timestamp)
            signature_check = _sign(self.token, timestamp, nonce, encryped_msg)
            if signature != signature_check:
                raise InvalidSignature('{} != {}'.format(signature_check, signature))
            return self._MsgCrypt__decrypt(encryped_msg)
    ...
    def __encrypt(self, text):
        ...
        """
        _bytes = bytes(bytearray(self._create_random_str(), self._ENCODING) + struct.pack('I', socket.htonl(len(text))) + text + bytearray(self.appid, self._ENCODING))
    ...
    def _create_random_str(self):
        random_chars = random.sample(string.ascii_letters + string.digits, self._RANDOM_STR_LEN)
