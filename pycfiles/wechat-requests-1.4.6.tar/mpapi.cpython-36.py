...
# [GCC 8.4.0]
# Embedded file name: f:\pipeline\wechat\src\wechat\mpapi.py
# Compiled at: 2018-05-11 07:59:45
...
class MpApi(Api):
    ...
    def __init__(self, mp_access_token, auth_update_callback=None, **kwargs):
        ...
        self._MpApi__options = copy(kwargs)
    ...
    def _retry(self, result, method, url, params_dict, **kwargs):
        ...
        """
        if result.is_failed:
            if result.errcode in AUTH_EXPIRED_CODES:
 ...
 method,
 url, params_dict=params, **kwargs)
        else:
            return result

    def group(self, **kwargs):
        cp_options = copy(self._MpApi__options)
    ...
    def _update_auth(self, response):
        if self._auth_update_callback is None:
            ...
            try:
                ...
                raise RequestException(('Update auth failed use {}'.format(self._auth_update_callback.__name__)),
                  auth_update_error,
                  response=response)
