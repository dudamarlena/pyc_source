...
def _build_reply_msg_bytes(receive_message, **kwrags):
    unicode_msg = serialize_dict_to_xml(ToUserName=receive_message.FromUserName, FromUserName=receive_message.ToUserName, CreateTime=int(time.time()), **kwrags)
...
class XMLMessageBuilder(object):
    ...
    def build_reply_textmsg_for(message, content):
        ...
        return _build_reply_msg_bytes(message, MsgType='text', Content=content)
