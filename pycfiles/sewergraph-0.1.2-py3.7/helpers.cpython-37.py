...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\sewergraph\helpers.py
...
import networkx as nx, os, sys, subprocess, pandas as pd, uuid
...
def get_node_values(G, nodes, parameters):
    ...
    upstream_vals = [G.node[n][p] for n in nodes if p in G.node[n] for p in parameters]
    return upstream_vals


def subset(df, lb=None, ub=None, param='capacity_fraction', sum_param='length'):
    """
...
def rename_duplicates(series):
    ...
    for dup in series.get_duplicates():
        cols[series.get_loc(dup)] = [dup + '.' + str(d_idx) if d_idx != 0 else dup for d_idx in range(series.get_loc(dup).sum())]
...
def transform_projection(G, to_crs='epsg:4326'):
    ...
    try:
        ...
        raise ImportError('pyproj and shapely modules needed. get them here: ', 'https://pypi.org/project/pyproj/, https://pypi.org/project/Shapely/')

    st_plane = pyproj.Proj((G.graph['crs']), preserve_units=True)
    wgs = pyproj.Proj(init=to_crs)
