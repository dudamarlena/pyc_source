...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\sewergraph\plot.py
...
def capacity_peak_comparison_plt(df, name='Sewers', title='Peak Flow vs Sewer Capacity'):
    ...
    scatter = Scatter(x=(df.peakQ.round(3).tolist()),
      y=(df.capacity.round(3).tolist()),
      text=(df.LABEL.tolist()),
      mode='markers',
      name=name)
    one_one = Scatter(x=(np.linspace(0.001, maxv)),
      y=(np.linspace(0.001, maxv)),
      text=(df.LABEL.tolist()),
      name='One to One')
    layout = Layout(xaxis=dict(type='log',
      autorange=True,
      title='Peak Flow (cfs)',
      range=[
     0.1, maxv]),
...
def cdf_go(net, parameter='capacity_fraction', cumu_param='length', normal=1, units='', df=None, name=None):
    """
    ...
    text = ['<br>'.join(x) + units for x in zip(df.LABEL.astype(str).tolist(), df.cumu_converted.round(1).astype(str).tolist())]
    graph_obj = Scatter(x=(df[parameter]),
...
def cumulative_distribution(df, name='Capacity Surplus', title='Sewer Capacity Surplus By Length', parameter='capacity_fraction', cumu_param='length'):
    ...
    cap['cumu_miles'] = cap.cumulated_param
    cum_cap = Scatter(x=(cap[parameter]),
