...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\sewergraph\save_load.py
...
def to_swmm5_dataframes(G):
    ...
    for dup in conduits.index.get_duplicates():
        cols[conduits.index.get_loc(dup)] = [dup + '.' + str(d_idx) if d_idx != 0 else dup for d_idx in range(conduits.index.get_loc(dup).sum())]
...
def graph_from_shp(pth='test_processed_01', idcol='facilityid', crs={'init': 'epsg:4326'}):
    """
    ...
    try:
        ...
        raise ImportError('shapely module needed. get it here: https://pypi.org/project/Shapely/')
...
def gdf_from_graph(G, return_type='edges'):
    ...
    try:
        ...
        raise ImportError('GeoPandas module needed. Download with conda: conda install geopandas')
