...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\sewergraph\resolve_data.py
# Compiled at: 2019-07-31 17:17:24
# Size of source mod 2**32: 14090 bytes
...
def preprocess_data(G):
    for u, v, d in G.edges(data=True):
        ...
        h = G[u][v]['height']
        w = G[u][v]['width']
        if not geom not in ('BOX', 'CIR', 'EGG'):
            if sum([_f for _f in [diam, h, w] if _f]) == 0:
                d['pipeshape'] = None
        elif geom == 'CIR':
            if diam == 0:
                if h > 0:
                    if w > 0:
                        ...
                        d['inferred_geom'] = 'Y'
        if h == 0 and w == 0 and geom in ('EGG', 'BOX'):
            d['pipeshape'] = infer_from_dimensions(diam, h, w)
            d['inferred_geom'] = 'Y'
...
def dfs_nodes_attributes_iter(G, source=None, attribute=None, upstream=True, null_val=None):
    """
    ...
    for start in nodes:
        if start in visited:
            ...
            if upstream:
                stack = [
                 (
                  iter(G.pred[start]), start)]
            else:
                stack = [
                 (
                  iter(G.succ[start]), start)]
        while stack:
...
def extend_elevation_data(G, data_key='ELEVATIONI', null_val=0):
    ...
    for n in list(reversed(list(topo_sorted_nodes))):
        invert = None
        if data_key in G1.node[n]:
            if G1.node[n][data_key] != null_val:
