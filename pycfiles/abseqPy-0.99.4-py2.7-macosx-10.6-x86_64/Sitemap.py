...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/TAMO/Sitemap.py
...
def main():
    ...
    for opt, value in opts:
        if opt == '-f':
            ...
            motifs.extend(MotifTools.txt2motifs(value))
        elif opt == '-n':
            motifnums = [ int(x) for x in value.split(',') ]
        elif opt == '-L':
            labels = list(value)
        elif opt == '-t':
            thresh = float(value)
        elif opt == '-a':
            ambigs.extend(value.split(','))
        elif opt == '-S':
            scale = float(value)
