...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/TAMO/DataSources/GO.py
...
"""
import sys, re, os, math, time, pickle, TAMO.paths
...
def load_GO():
    ...
    for line in lines:
        ...
        if anno.aspect != 'P':
            continue
        if anno.desc in BAD_CATEGORIES:
            continue
        if anno.isnot:
            continue
        if not _orfs_by_go.has_key(anno.desc):
            _orfs_by_go[anno.desc] = []
        _orfs_by_go[anno.desc].append(anno.orf)
        if not _gos_by_orf.has_key(anno.orf):
            _gos_by_orf[anno.orf] = []
        _gos_by_orf[anno.orf].append(anno)

    for orflist in _orfs_by_go.values():
        size = len(orflist)
        if not _by_size.has_key(size):
            _by_size[size] = 0.0
        _by_size[size] = _by_size[size] + 1.0


def gene2annotations(gene):
    return annotations(SGD.gene2orf(gene))
...
def geneprobelistmatch(gene, probelist):
    if 'Yeast6kArray' not in dir():
        from TAMO.DataSources import Yeast6kArray
