...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/TAMO/GetDataFiles.py
# Compiled at: 2019-04-23 02:08:32
import re, os, urllib, sys, time, getopt, zipfile, gzip, TAMO.paths
GLOBALS = {}
...
def unzip(root, file, FCN=None):
    ...
    for filename in filenames:
        ...
        if path and not os.path.exists(root + path):
...
def dashes(numblocks, blocksize, totsize):
    ...
    if current_k > total_k:
        current_k = total_k
    if numblocks == 0:
        print '(Total: %8dk) : ' % total_k,
    else:
        sys.stdout.write('\x08\x08\x08\x08\x08\x08\x08\x08\x08')
...
def usage(txt=''):
    if txt:
        print 'Error: %s' % txt
    print 'Usage: %s [--all  | --<data/code source>]' % re.sub('^.*/', '', sys.argv[0])
    print ''
...
def parse_opts():
    ...
    for opt, value in opts:
        if opt in dDATA:
            GLOBALS['functions'].append('%sData' % opt.replace('--', ''))
        if opt in dCODE:
            GLOBALS['functions'].append('%sCode' % opt.replace('--', ''))
        if opt == '--all':
            GLOBALS['all'] = 1
