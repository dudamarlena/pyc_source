...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/TAMO/MDconvert/tamo2table.py
...
def tablemotifs(_motifs):
    ...
    for m in motifs:
        ...
        if getarg('GIF'):
            s = s + '<td><img src="%s"></td>' % m.gifname
        else:
            s = s + '<td>%s</td>' % m.oneletter
        try:
...
def usage(txt=''):
    if txt:
        print 'Error: %s' % txt
    print 'Usage: %s -m motiffile ' % re.sub('^.*/', '', sys.argv[0])
    sys.exit(1)
