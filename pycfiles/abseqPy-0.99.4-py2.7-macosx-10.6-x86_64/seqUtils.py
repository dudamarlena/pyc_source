...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/abseqPy/IgRepAuxiliary/seqUtils.py
...
import matplotlib
matplotlib.use('agg')
import gc, sys, pickle, random, bisect, matplotlib.pyplot as plt, os
...
def generateMotif(sequences, name, alphabet, filename, align=False, transSeq=False, protein=False, weights=None, outDir=None, threads=2, stream=None):
    """
    ...
    if exists(filename):
        ...
        if transSeq:
            ...
            seqs = sequences
        if len(seqs) > 1 * 100000 or weights is not None and sum(weights) > 1 * 100000:
            random.seed(1986)
            seqs = weightedSample(seqs, weights, int(1 * 100000))
            if align:
        ...
        m = motifs.create(alignedSeq, alphabet)
...
@requires('TAMO')
def generateMotifs(seqGroups, align, outputPrefix, transSeq=False, extendAlphabet=False, clusterMotifs=False, protein=False, threads=2, stream=None):
    ...
    for group in groups:
        ...
        m = generateMotif(seqs, group, alphabet, filename, align, transSeq, protein, outDir=logosFolder, threads=threads, stream=stream)
        if m is None:
            return
        motifSeqs = m.instances
        pwm = m.counts.normalize(pseudocounts=None)
        ...
        if clusterMotifs and len(motifSeqs) > 10:
            motif = Motif(map(lambda x: str(x), motifSeqs), backgroundD={'A': 0.6, 'C': 0.4, 'G': 0.4, 'T': 0.6}, id=group)
...
def generateMotifLogo(m, filename, outdir='.', dna=True, stream=None):
    ...
    weblogo = ShortOpts(exe=WEBLOGO, f=quote(tmpFile), o=quote(filename), A='dna' if dna else 'protein', F='png', n=200, D='fasta', s='medium', c='classic' if dna else 'auto', X='NO', Y='NO').append('--errorbars NO --fineprint CSL --resolution 600')
    weblogo()
    os.remove(tmpFile)
