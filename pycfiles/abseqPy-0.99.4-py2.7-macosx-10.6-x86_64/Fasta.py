...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/TAMO/seq/seq/Fasta.py
...
def keys(filename, key_func=None):
    ...
    FID.seek(0)
    if not re.search('\\.fa|\\.fsa|\\.fasta', filename) or first != '>':
...
def find(name, pathhint=None):
    ...
    if re.search('\\.fsa$', name):
        if exists(name):
            return name
        if pathhint and exists(pathhint + tail + '.fsa'):
        ...
        if exists(smroot + '.fsa'):
            return smroot + '.fsa'
        if pathhint and exists(pathhint + smroot + '.fsa'):
