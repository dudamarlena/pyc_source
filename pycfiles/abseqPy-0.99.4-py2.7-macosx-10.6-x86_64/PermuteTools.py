...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/TAMO/util/util/PermuteTools.py
...
def uniq_syl_pairs(width):
    ...
    for left in syls:
        for right in syls:
            word = left + right
            rc = RC(word)
            if memo.has_key(word) or memo.has_key(rc):
                continue
            memo[word] = 1
...
def dimer_words(width):
    ...
    for syl in syls:
        for af in [syl, RC(syl)]:
            word = syl + af
            if memo.has_key(word) or memo.has_key(RC(word)):
                continue
            memo[word] = 1
            tups.append((syl, af))
