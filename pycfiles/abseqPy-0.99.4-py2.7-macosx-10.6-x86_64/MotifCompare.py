...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/TAMO/Clustering/MotifCompare.py
...
def ENMrange(self, other, Srange, Orange):
    ...
    """
    if not self.__dict__.has_key('P') or not self.P:
        self._computeP()
    if not other.__dict__.has_key('P') or not other.P:
        other._computeP()
    sw = self.width
    ow = other.width
...
def probOvlpBinomial(A, B, thresh=0.7, verbose=None):
    ...
    if matchNarrow == 0:
        p = 1.0
        return p
    if not Narrow.__dict__.has_key('probNarrow'):
        Narrow.probNarrow = {}
...
def probOvlp(A, B, thresh=0.7, verbose=None):
    ...
    p = Arith.hypgeomsummore(countWide, numtotal, countNarrow * fudgefactor, countBoth)
    return p
...
def estimate_frequency(motif, k, samples=100000, thresh=0.7):
    ...
    for i in range(40):
        ...
        f = totalcount / total
        d = math.fabs(f - estimate) / (estimate + 1e-08)
        estimate = f
        if d < 0.0001:
            break
        if i > 2 and totalcount > 100:
            break
...
def negcommonbitsrange(self, other, Srange, Orange, PENALTY=0):
    ...
    for si, oi in zip(Srange, Orange):
        if DIAGNOSTIC:
            print '%s %s' % (self.oneletter[si], other.oneletter[oi]),
        S = self.P[si]
        O = other.P[oi]
        colbits = 0.0
        for L in ACGT:
            Sbits = O[L] * LOG(S[L] / Q[L]) / LOG(2.0)
            Obits = S[L] * LOG(O[L] / Q[L]) / LOG(2.0)
            colbits += max(PENALTY, min(Sbits, Obits))
            if DIAGNOSTIC:
                print '%s %3.1f|%3.1f %5.2f|%5.2f  ' % (
                 L, S[L], O[L], Sbits, Obits),

        commonbits += max(0, colbits)
        if DIAGNOSTIC:
            print '%5.3f' % max(0, colbits), colbits
    ...
    if DIAGNOSTIC:
        print ' ', other.oneletter[min(Orange):max(Orange) + 1], '%4.2f' % commonbits
    if DIAGNOSTIC:
        print
    return -commonbits
...
def showdiffXvert(motif, seq, OVLP_FCN=None, DIFF_FCN=None):
    ...
    if not OVLP_FCN:
        OVLP_FCN = lambda A, B: min(min(A.width, B.width) - 1, 7)
    bg = motif.background
    ...
    if rcflag:
        ...
        m = other
    print 'MSOdiff:  %8.4f %s%s%s' % (diff, '               ', motif.oneletter, ' ' * (30 - motif.width))
...
def averagemotifs(motifs, ovlp=2, template=None, DFUNC=negcommonbitsrange, VERBOSE=1, prop=''):
    ...
    if VERBOSE:
        for m in pmotifs:
            ...
            if m.__dict__.has_key('key'):
                print m.key,
            if prop and m.__dict__.has_key(prop):
                print m.__dict__[prop],
            print
