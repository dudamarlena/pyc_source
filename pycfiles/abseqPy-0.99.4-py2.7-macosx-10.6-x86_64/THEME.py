...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/TAMO/MD/THEME.py
...
class THEME():
    ...
    def run_CV(self, models):
        ...
        for trial in range(trials):
            ...
            fg_group_size = len(fg) / self.cv_level
            bg_group_size = len(bg) / self.cv_level
            fg_groups = {}
            bg_groups = {}
            temp_fg = []
            temp_bg = []
            ...
            for k in range(num_models):
                ...
                if self.randomize:
                    ...
                    if trial > 0:
                        try:
                            std = math.sqrt((sum_sq - sum_mean * sum_mean / (trial + 1)) / trial)
                        except:
                            std = 0.0
                    ...
                    if self.family != '':
                        self.best_motif[k].family = self.family
    ...
    def train_classifier(self, model_num, fg_groups, bg_groups, N, trial):
        ...
        if self.refine:
            if self.beta > 0.0:
                ...
                betas = [
                 0.05 / 0.95, 0.1 / 0.9, 0.5, 1.0, 2.0, 1000000000000000000]
        else:
            betas = [
             1000000000000000000]
        for beta in betas:
            ...
            for i in range(self.cv_level):
                ...
                c_vals = [
                 1e-10, 0.0001, 0.001, 0.01, 0.05, 0.1, 1.0, 10.0, 100.0]
                cv_errs = []
                for c in c_vals:
    ...
    def train_final(self, model, fg, bg, N, beta):
        ...
        return (
         final_motif, best_classifier, fn)
    ...
    def SMOTE(self, data, N, k):
        ...
        for i in range(len(data_list)):
            ...
            distances = []
            for j in range(k):
                distances.append((10000.0, 10000.0))
