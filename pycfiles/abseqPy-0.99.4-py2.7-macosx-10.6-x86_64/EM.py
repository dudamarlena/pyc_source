...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/TAMO/MD/MD/EM.py
...
"""
import sys, re, os, math, time, time, MDsupport, TAMO.paths
...
class MotifCandidate():
    ...
    def __init__(self, wmers=''):
        ...
        self.Q = {'A': 0.31, 'C': 0.19, 'G': 0.19, 'T': 0.31}
        self.last_bgprob = 0
        self.deltaMAP = 0.0
    ...
    def _plogp_new(self, wmers):
        ...
        for key in toAscii.keys():
            sumsD[key] = Numeric.sum(Numeric.equal(AsciiSeq, toAscii[key])).astype(type(1.0))
            f = sumsD[key] / nwmers + 1e-10
    ...
    def check_and_update(self, wmer, count, verbose=''):
        ...
        if _MAP > self.MAP + self.deltaMAP:
            ...
            if verbose:
                print 'Adding %d copies of %s: O:%f N:%f (delta:%f)' % (
    ...
    def purge(self, verbose=''):
        ...
        for wmer_omit in wmersD.keys():
            ...
            if _MAP > self.MAP:
                if verbose:
                    print 'Purging %d copies of %s: O:%f N:%f (delta:%f)' % (
...
class EM():
    ...
    def __init__(self, seed_seqs, all_seqs, width=6, verbose=''):
        ...
        self.bgprob = {'A': 0.31, 'C': 0.19, 'G': 0.19, 'T': 0.31}
        self.beta = 0.001
        self.deltamin = 0.001
        self.probes = []
        self.method = 'ZOOPS'
    ...
    def report(self):
        ...
        if theMarkovBackground:
            print '# Using High-order (%d-th)  Markov Background model' % (theMarkovBackground.highestorder - 1)
        if self.gapflank > 0:
    ...
    def seed_models(self):
        ...
        for nmer, count in self.goodwmersT:
            M = MotifTools.Motif(None, self.bgprob)
    ...
    def EM_Cstart(self):
        ...
        if verbose:
            ...
            sys.stdout.flush()
        self.seed_models()
        if not self.param.has_key('gamma'):
            self.param['gamma'] = 0.2
        ...
        _time2 = time.time()
        timings['Probes'] = _time2 - _time
        _time = _time2
        if verbose:
            print 'Optimizing candidates by EM.'
        if verbose:
            sys.stdout.flush()
        c_logZ_sets = {}
        for Model, i in zip(self.models, range(len(self.models))):
            ...
            if not c_logZ_sets.has_key(width):
                c_logZs_set = []
                if verbose:
                    print '#%s   |%s|' % (' ' * 28, '-' * len(self.seqs))
                if verbose:
                    sys.stdout.flush()
                if verbose:
                    print 'Computing background (width %2d)  ' % width,
            ...
            _time2 = time.time()
            timings['C EM'] = timings['C EM'] + _time2 - _time
            _time = _time2
            if newModel == None:
                continue
            ...
            print 'Seed %2d: %s  -->  %s  mask:%9.5f  infoMask:%9.5f d:%9.5f' % (
             ...
             MotifTools.infomaskdiff(newModel, Model),
             Model - newModel)
            if Model.seedtxt:
                newModel.seedtxt = Model.seedtxt
            if Model.source:
            ...
            _time2 = time.time()
            timings['Post'] = timings['Post'] + _time2 - _time
            _time = _time2
    ...
    def best_by_Z(self, Wmers_by_seq):
        ...
        for Wmer_set in Wmers_by_seq:
            ...
            if 0 and self.verbose:
                print 'Best: %s %f' % (bestwmer, math.log(bestZ))
...
class MarkovBackground():
    ...
    def permute(self, letters, depth, seqs=[
 ''], curdepth=0):
        newseqs = []
        for seq in seqs:
...
def permute(depth, letters=[
 'A', 'C', 'G', 'T'], seqs=[''], curdepth=0):
    newseqs = []
    for seq in seqs:
        for letter in letters:
            newseqs.append(seq + letter)
