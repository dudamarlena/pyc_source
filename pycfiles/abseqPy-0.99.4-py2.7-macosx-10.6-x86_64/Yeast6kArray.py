...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/TAMO/DataSources/Yeast6kArray.py
...
TAMO.paths.CHECK([_orf2probefile, _EFprobe2orffile, _EForf2probefile, _probeposfile, _pcrfile, _biasfile], 'Whitehead')
...
def _load_EFfiles():
    FID = open(_EForf2probefile, 'r')
    o2plines = FID.readlines()
    FID.close()
    FID = open(_EFprobe2orffile, 'r')
    p2olines = FID.readlines()
    FID.close()
...
def _load_Xfiles():
    FID = open(_Xorf2probefile, 'r')
    lines = FID.readlines()
    FID.close()
    for line in lines[1:]:
        toks = line.split(',')
        orf, probe, category = toks[0], toks[1], re.sub('"', '', toks[7])
        if not _ORF2PROBES.has_key(orf):
            _ORF2PROBES[orf] = []
        _ORF2PROBES[orf].append(probe)
        if not _PROBE2ORFS.has_key(probe):
            _PROBE2ORFS[probe] = []
        _PROBE2ORFS[probe].append(orf)
        if not _MAPTYPE.has_key(orf):
            _MAPTYPE[orf] = {}
...
def orf2gene(orf):
    return _result(_ORF2GENE, orf)


def orf2probe(orf):
    return _result(_ORF2PROBE, orf)


def probe2orf(probe):
    return _result(_PROBE2ORF, probe)


def gene2orf(gene):
    return _result(_GENE2ORF, gene)


def orf2func(orf):
    return _result(_ORF2FUNC, orf)


def probe2orfs(probe):
    return _result(_PROBE2ORFS, probe, [])


def orf2probes(orf):
    return _result(_ORF2PROBES, orf, [])


def probe2genes(probe):
    return [ orf2gene(x) for x in probe2orfs(probe) ]


def probe2funcs(probe):
    return [ orf2func(x) for x in probe2orfs(probe) ]


def gene2probes(gene):
    return orf2probes(gene2orf(gene))


def probe2func(probe):
    return orf2func(probe2orf(probe))


def gene2func(gene):
    return orf2func(gene2orf(gene))


def gene2probe(gene):
    return orf2probe(gene2orf(gene))


def probe2gene(probe):
    return orf2gene(probe2orf(probe))
...
def stringent_filter(ids, bias_cutoff=10):
    result = []
    for id in ids:
        if pcr_bad(id) or len(spot_pos(id)) > 2:
            continue
        result.append(id)
