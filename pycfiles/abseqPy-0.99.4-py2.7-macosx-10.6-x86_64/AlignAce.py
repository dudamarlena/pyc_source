...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/TAMO/MD/MD/AlignAce.py
...
"""
import sys, re, os, math, tempfile, random, TAMO.paths
...
class MetaAce:
    ...
    def __init__(self, fastafile='', width=10, iterations=5, gcback=0.38):
        self.results = []
        self.seed = int(random.random() * 1000000000.0)
        self.gcback = gcback
    ...
    def flatten(self, thresh=0.5):
            ...
            for i in range(len(ans)):
                ...
                if len(m) == len(other):
                    ...
                    if Df <= thresh or Dr <= thresh:
                        match = 1
                        if m.church == other.church:
                            if m.pvalue < other.pvalue:
                                ans[i] = m
                        elif m.church < other.church:
                            ans[i] = m
                if match:
                    break
        ...
        ans.sort(lambda x, y: cmp(x.church, y.church))
        return ans
...
def flatten(motifs, thresh=0.5, prop='church', secondary_prop='pvalue'):
    ...
    for m in motifs:
        ...
        for i in range(len(ans)):
            ...
            if len(m) == len(other):
                ...
                if Df <= thresh or Dr <= thresh:
                    match = 1
                    if m.__dict__[prop] == other.__dict__[prop]:
                        if m.__dict__[secondary_prop] < other.__dict__[secondary_prop]:
                            ans[i] = m
                    elif m.__dict__[prop] < other.__dict__[prop]:
                        ans[i] = m
            if match:
                break
    ...
    ans.sort(lambda x, y, p1=prop, p2=secondary_prop: cmp(x.__dict__[p1], y.__dict__[p1]) or cmp(x.__dict__[p2], y.__dict__[p2]))
    return ans
