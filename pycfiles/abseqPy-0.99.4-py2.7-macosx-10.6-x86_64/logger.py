...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/abseqPy/logger.py
...
_BANNER = {'all': 'Running the complete QC pipeline', 
...
def formattedTitle(task, defaultTitle=False):
    ...
    string = '-' * 100 + '\n'
    string += '|' + ' ' * 98 + '|\n'
    string += '|' + ' ' * ((98 - len(title)) / 2) + title + ' ' * ((98 - len(title)) / 2 + (98 - len(title)) % 2) + '|\n'
    string += '|' + ' ' * 98 + '|\n'
    string += '-' * 100 + '\n'
    return string