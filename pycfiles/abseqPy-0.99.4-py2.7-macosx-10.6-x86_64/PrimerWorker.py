...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/abseqPy/IgRepAuxiliary/PrimerWorker.py
...
class PrimerWorker(Process):
    ...
    def __init__(self, procCounter, fr4cut, trim5end, trim3end, actualQstart, end5, end3, end5offset, tasks, exitQueue, resultsQueue, stream=None):
        ...
        while True:
            ...
            try:
                ...
                for record, qsRec in zip(nextTask[0], nextTask[1]):
                    ...
                    recs.append(_matchClosestPrimer(qsRec, record, self.actualQstart, self.trim5end, self.trim3end, self.end5offset, self.fr4cut, self.maxPrimer5Length, self.maxPrimer3Length, self.primer5sequences, self.primer3sequences))

                self.resultsQueue.put(recs)
                self.procCounter.increment(len(recs))
            except Exception as e:
                printto(self.stream, 'An error as occurred while processing ' + self.name + (' with error {}').format(str(e)), LEVEL.EXCEPT)
...
def _matchClosestPrimer(qsRec, record, actualQstart, trim5end, trim3end, end5offset, fr4cut, maxPrimer5Length, maxPrimer3Length, primer5seqs, primer3seqs):
    ...
    if primer5seqs:
        ...
        try:
            qsRec['5endPrimer'], qsRec['5endMismatchIndex'], qsRec['5endIndelIndex'], _, _ = findBestMatchedPattern(primer, primer5seqs)
        except Exception as e:
            unexpected5 += 1
    ...
    if primer3seqs:
        ...
        try:
            qsRec['3endPrimer'], qsRec['3endMismatchIndex'], qsRec['3endIndelIndex'], _, _ = findBestMatchedPattern(primer, primer3seqs)
        except Exception as e:
            unexpected3 += 1

    return (
     qsRec, unexpected5, unexpected3)
...
def _parsePrimerFile(primerFile, stream=None):
    if primerFile:
        ...
        if len(set(primerLengths)) != 1:
            printto(stream, ('WARNING: Provided primer file {} has primers with different length. Analysis assumes uniform primer length').format(primerFile), LEVEL.WARN)
