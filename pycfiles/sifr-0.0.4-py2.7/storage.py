...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/sifr/storage.py
# Compiled at: 2015-06-15 17:57:11
from abc import abstractmethod, ABCMeta
import threading, time, six
from sifr.hll import HLLCounter
try:
    from collections import Counter
except ImportError:
    from .backports.counter import Counter
...
class RedisStorage(Storage):
    ...
    def track(self, span, identifier):
        with self.redis.pipeline(transaction=False) as (pipeline):
            ...
            pipeline.expire(span.key + ':t', int(span.expiry) - int(time.time()))
            pipeline.execute()
    ...
    def incr_unique(self, span, identifier):
        with self.redis.pipeline(transaction=False) as (pipeline):
            ...
            pipeline.expire(span.key + ':u', int(span.expiry) - int(time.time()))
            pipeline.execute()
    ...
    def incr(self, span, amount=1):
        with self.redis.pipeline(transaction=False) as (pipeline):
            ...
            if span.expiry is not None:
                pipeline.expire(span.key + ':c', int(span.expiry) - int(time.time()))
            pipeline.execute()
        return
