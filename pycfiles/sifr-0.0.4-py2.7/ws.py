...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/sifr/ws.py
# Compiled at: 2015-06-08 21:23:52
import os
...
def count(path):
    ...
    return jsonify(dict(zip([ k.at.isoformat() for k in time_spans ], [ current_app.sifr.count(k) for k in time_spans ])))
...
def card(path):
    ...
    time_spans = get_time_spans(start, end, keys, [span])
    return jsonify(dict(zip([ k.at.isoformat() for k in time_spans ], [ current_app.sifr.cardinality(k) for k in time_spans ])))
