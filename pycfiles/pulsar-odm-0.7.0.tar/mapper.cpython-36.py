...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/quantmind/pulsar-odm/odm/mapper.py
# Compiled at: 2017-11-24 06:00:10
# Size of source mod 2**32: 15013 bytes
import logging, sys
...
from sqlalchemy.ext.declarative.api import declarative_base, declared_attr, _as_declarative, _add_attribute
...
class OdmMeta(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        if not abstract:
            if not isinstance(klass, DeclarativeMeta):
...
class Mapper:
    ...
    def __init__(self, binds):
        ...
        self._bases = {}
        self._base_declarative = declarative_base(name='OdmBase', metaclass=DeclarativeMeta)
        self.binds = {}
        self.is_green = False
    ...
    def register(self, model, **attr):
        ...
        if not isinstance(model, Table):
            ...
            keys = ('%s.%s' % (label, table.key), label, None) if label else (None, )
            for key in keys:
    ...
    def database_create(self, database, **params):
        ...
        for key, engine in self.keys_engines():
            if hasattr(database, '__call__'):
                ...
                raise AssertionError('Cannot create a database, no db name given')
    ...
    def database_drop(self, database=None, **params):
        ...
        for engine in self.engines():
            if hasattr(database, '__call__'):
                ...
                raise AssertionError('Cannot drop database, no db name given')
    ...
    def _create_model(self, model):
        ...
        if isinstance(model, meta):
            raise ImproperlyConfigured('Cannot register declarative classes only mixins allowed')
        else:
            ...
            if create:
                event.listen(self.metadata, 'after_create', DDL(create.format({'name': name})))
                drop = getattr(model, '__drop_sql__', None)
                if not drop:
                    logger.warning('Model %s has create statement but not drop. To mute this warning add a __drop_sql__ statement in the model class', name)
                else:
                    event.listen(self.metadata, 'before_drop', DDL(drop.format({'name': name})))
        return (
         model, name)
