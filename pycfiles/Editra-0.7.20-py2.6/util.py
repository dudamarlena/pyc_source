...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/util.py
...
import os, sys, mimetypes, encodings, codecs, urllib2, wx, ed_glob, ed_event, ed_crypt, dev_tool, syntax.syntax, syntax.synglob as synglob, ebmlib
_ = wx.GetTranslation
...
class DropTargetFT(wx.PyDropTarget):
    ...
    def __init__(self, window, textcallback=None, filecallback=None):
        ...
        self.window = window
        self._data = dict(data=None, fdata=None, tdata=None, tcallb=textcallback, fcallb=filecallback)
        self._tmp = None
        self._lastp = None
        self.InitObjects()
        return
    ...
    def CreateDragString(self, txt):
        ...
        try:
            mdc = wx.MemoryDC(wx.EmptyBitmap(longest[0] + 5, longest[1] * len(txt), 32))
    ...
    def OnData(self, x_cord, y_cord, drag_result):
        ...
        try:
            ...
            wx.PostEvent(self.window.GetTopLevelParent(), ed_event.StatusEvent(ed_event.edEVT_STATUS, -1, _('Unable to accept dropped file or text')))
            data = False
...
def GetResources(resource):
    ...
    if os.path.exists(rec_dir):
        rec_lst = [ rec.title() for rec in os.listdir(rec_dir) if os.path.isdir(rec_dir + rec) if rec[0] != '.' ]
        return rec_lst
    else:
        return -1


def GetResourceFiles(resource, trim=True, get_all=False, suffix=None, title=True):
    ...
    if not os.path.exists(rec_dir):
        ...
        for rec in recs:
            if os.path.isfile(rec_dir + rec) or get_all and os.path.isfile(rec_dir2 + rec):
...
def GetProxyOpener(proxy_set):
    ...
    proxy_info['passwd'] = ed_crypt.Decrypt(proxy_info['passwd'], proxy_info['pid'])
    Log('[util][info] Formatted proxy request: %s' % (auth_str.replace('%(passwd)s', '****') % proxy_info))
...
class IntValidator(wx.PyValidator):
    ...
    def OnChar(self, event):
        ...
        if key < wx.WXK_SPACE or key == wx.WXK_DELETE or key > 255 or chr(key) in '0123456789':
            event.Skip()
            return
