...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/eclib/choicedlg.py
...
class ChoiceDialog(ecbasewin.ECBaseDlg):
    ...
    def __init__(self, parent, id=wx.ID_ANY, msg='', title='', choices=None, default='', pos=wx.DefaultPosition, size=wx.DefaultSize, style=0, name=ChoiceDialogNameStr):
        ...
        """
        super(ChoiceDialog, self).__init__(parent, id, title, style=wx.CAPTION, pos=pos, size=size, name=name)
        panel = ChoicePanel(self, msg=msg, choices=choices, default=default, style=style)
        self.SetPanel(panel)
        self.SetInitialSize()
...
class ChoicePanel(wx.Panel):
    ...
    def __init__(self, parent, msg='', choices=list(), default='', style=wx.OK | wx.CANCEL):
        """Create the panel
        @param parent: Parent Window
