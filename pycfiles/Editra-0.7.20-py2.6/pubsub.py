...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/extern/pubsub.py
# Compiled at: 2011-10-01 11:27:10
r"""
...
class _WeakMethod():
    ...
    def __eq__(self, method2):
        ...
        if not isinstance(method2, _WeakMethod):
            ...
            return self.fun is method2.fun and self.objRef() is method2.objRef() and self.objRef() is not None
...
class _TopicTreeNode():
    ...
    def createSubtopic(self, subtopic, topicPath):
        ...
        return self.__subtopics.setdefault(subtopic, _TopicTreeNode(topicPath, self.__onDeadListenerWeakCB))
...
class _TopicTreeRoot(_TopicTreeNode):
    ...
    def __init__(self):
        ...
        _TopicTreeNode.__init__(self, (ALL_TOPICS,), _getWeakRef(self.__rootNotifyDead))
    ...
    def getTopics(self, listener):
        ...
        return [ weakNode().getPathname() for weakNode in weakNodes if weakNode() is not None
               ]
...
class PublisherClass():
    ...
    def subscribe(self, listener, topic=ALL_TOPICS):
        ...
        if topic is None:
            raise TypeError, 'Topic must be either a word, tuple of words, or getStrAllTopics()'
    ...
    def validate(self, listener):
        ...
        if not callable(listener):
            raise TypeError, 'Listener ' + `listener` + ' must be a function, bound method or instance.'
        elif ismethod(listener) and not _isbound(listener):
            raise TypeError, 'Listener ' + `listener` + ' is a method but it is unbound!'
        (min, d) = _paramMinCount(listener)
        if min > 1:
            raise TypeError, 'Listener ' + `listener` + " can't require more than one parameter!"
        if min <= 0 and d == 0:
    ...
    def sendMessage(self, topic=ALL_TOPICS, data=None, onTopicNeverCreated=None, context=None):
        ...
        self.__deliveryCount += self.__topicTree.sendMessage(aTopic, message, onTopicNeverCreated)
...
class Message():
    ...
    Data = property(lambda self: self.data, lambda self, data: setattr(self, 'data', data))
    Context = property(lambda self: self.context, lambda self, ctx: setattr(self, 'context', value))
...
def test():
    ...
    def testParam():
        ...
        def testFunc21(a, b, c=1):
            pass

        def testFuncA(*args):
            pass

        def testFuncAK(*args, **kwds):
            pass
    ...
    def testSubscribe():
        ...
        publisher.unsubscribe(lisnr1, 'booboo')
        assert publisher.getAssociatedTopics(lisnr1) == [(topic1,), topic2]
        assert publisher.getAssociatedTopics(lisnr2.notify) == [topic3]
        assert publisher.getAssociatedTopics(lisnr3) == [topic5]
