...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/plugdlg.py
...
import os, sys, re, urllib2, wx, wx.lib.delayedresult as delayedresult, ed_glob, plugin, ed_event, ed_msg, util, ed_txt
from profiler import Profile_Get, Profile_Set
import ed_basewin, eclib
CONFIG_PG = 0
DOWNLOAD_PG = 1
INSTALL_PG = 2
...
class PluginDialog(wx.Frame):
    ...
    def __init__(self, parent, id=wx.ID_ANY, title='', size=wx.DefaultSize):
        super(PluginDialog, self).__init__(parent, title=title, size=size, style=wx.DEFAULT_FRAME_STYLE)
        ...
        if len(pmgr.GetIncompatible()):
            self._nb.AddPage(ConfigPanel(self._nb, style=wx.NO_BORDER, mode=MODE_ERROR), _('Errors'), img_id=IMG_ERROR)
    ...
    def __InitImageList(self):
        ...
        bmp = wx.ArtProvider.GetBitmap(str(ed_glob.ID_PREF), wx.ART_TOOLBAR, (32, 32))
        self._imglst.append(bmp)
    ...
    def OnClose(self, evt):
        ...
        if self._dl_pg.IsDownloading():
            dlg = wx.MessageDialog(self, _('Downloads are incomplete'), _('Do you wish to exit?'), style=wx.YES_NO | wx.ICON_EXCLAMATION | wx.CENTER)
            result = dlg.ShowModal()
            dlg.Destroy()
            if result == wx.ID_NO:
                return
        if self._cfg_pg.ConfigChanged():
            wx.MessageBox(_('You must restart Editra before your changes will take full affect.'), _('Configuration Changes Made'), wx.ICON_INFORMATION | wx.OK)
    ...
    def OnPageChanging(self, evt):
        ...
        self.SetTitle(self.GetTitle().split(' | ')[0] + ' | ' + self._nb.GetPageText(cur_pg))
        if cur_pg == CONFIG_PG:
            self._cfg_pg.PopulateCtrl()
            self.SetStatusText(_('Changes will take affect once the program has been restarted'), 0)
...
class DownloadPanel(ed_basewin.EdBaseCtrlBox):
    ...
    EGG_PATTERN = re.compile('(?P<name>[^-]+)( -(?P<ver>[^-]+) (-py(?P<pyver>[^-]+) (-(?P<plat>.+))? )? )?', re.VERBOSE | re.IGNORECASE).match
        ...
        if len(plugins) < 2:
            ...
            for meta in plugins:
                ...
                if len(data) < 4:
                    continue
                tmpdat = PluginData()
                set_map = {'author': tmpdat.SetAuthor, 'version': tmpdat.SetVersion, 
                   'name': tmpdat.SetName, 
    ...
    def OnButton(self, evt):
        ...
        if e_id == self.ID_DOWNLOAD:
            ...
            for egg in range(len(urls)):
                ...
                delayedresult.startWorker(self._ResultCatcher, _DownloadPlugin, wargs=urls[egg], jobID=egg)
    ...
    try:
        try:
            if Profile_Get('USE_PROXY', default=False):
                proxy_set = Profile_Get('PROXY_SETTINGS', default=dict(uname='', url='', port='80', passwd=''))
...
class InstallPanel(ed_basewin.EdBaseCtrlBox):
    ...
    def __init__(self, parent):
        ...
        toolt = wx.ToolTip(_('To add a new item drag and drop the plugin file into the list.\n\nTo remove an item select it and hit Delete or Backspace.'))
        self._install = wx.ListBox(self, wx.ID_ANY, style=wx.LB_SORT | wx.BORDER_NONE)
        self._install.SetToolTip(toolt)
        self._install.SetDropTarget(util.DropTargetFT(self._install, None, self.OnDrop))
    ...
    def _Install(self):
        ...
        if not len(self._install.GetItems()):
            ...
            dlg = wx.MessageDialog(self, _('Failed to install %d plugins') % self._install.GetCount(), _('Installation Error'), style=wx.OK | wx.CENTER | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
        return
...
class PBPluginItem(eclib.PanelBoxItemBase):
    ...
    def __init__(self, parent, mod, pdata, bmp=None, enabled=False):
        ...
        self._uninstall = eclib.PlateButton(self, label=_('Uninstall'), bmp=bmp, style=eclib.PB_STYLE_NOBG)
        self._uninstall.Unbind(wx.EVT_ERASE_BACKGROUND)
        bmp = wx.ArtProvider.GetBitmap(str(ed_glob.ID_PREF), wx.ART_MENU)
        self._config = eclib.PlateButton(self, label=_('Configure'), bmp=bmp, style=eclib.PB_STYLE_NOBG)
        ...
        if ipath:
            ...
            util.Log('[pluginmgr][warn] cant find plugin path for %s' % self._pdata.GetName())
            self._uninstall.Show(False)
        ...
        if wx.Platform == '__WXMAC__':
            for ctrl in (self._desc, self._auth, self._enabled, self._config, self._uninstall):
    ...
    def OnConfigButton(self, evt):
        ...
        if self._module is not None:
            ...
            event = ed_event.NotificationEvent(ed_event.edEVT_NOTIFY, ed_glob.ID_PREF, cfg_obj)
...
class PluginData(plugin.PluginData):
    ...
    def __init__(self, name='', descript='', author='', ver='', url=''):
