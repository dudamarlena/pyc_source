...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/extern/aui/aui_switcherdialog.py
...
class SwitcherItems(object):
    ...
    def PaintItems(self, dc, win):
        ...
        if self.GetItemFont().IsOk():
            ...
            groupFont = wx.Font(standardFont.GetPointSize(), standardFont.GetFamily(), standardFont.GetStyle(), wx.BOLD, standardFont.GetUnderlined(), standardFont.GetFaceName())
        ...
        for i in xrange(len(self._items)):
            ...
            if not item.GetIsGroup():
                if item.GetBitmap().IsOk() and item.GetBitmap().GetWidth() <= 16 and item.GetBitmap().GetHeight() <= 16:
                    x -= textMarginX
...
class MultiColumnListCtrl(wx.PyControl):
    ...
    def __init__(self, parent, aui_manager, id=wx.ID_ANY, pos=wx.DefaultPosition, size=wx.DefaultSize, style=0, validator=wx.DefaultValidator, name='MultiColumnListCtrl'):
    ...
    def OnKey(self, event):
        ...
        if keyCode in [wx.WXK_ESCAPE, wx.WXK_RETURN]:
            ...
            topLevel.GetEventHandler().ProcessEvent(closeEvent)
            return
        if keyCode in [wx.WXK_TAB, self.GetExtraNavigationKey()]:
            if event.ShiftDown():
...
class SwitcherDialog(wx.Dialog):
    ...
    def __init__(self, items, parent, aui_manager, id=wx.ID_ANY, title=_('Pane Switcher'), pos=wx.DefaultPosition, size=wx.DefaultSize, style=wx.STAY_ON_TOP | wx.DIALOG_NO_PARENT | wx.BORDER_SIMPLE):
        """ Default class constructor. """
        ...
        self._listCtrl = MultiColumnListCtrl(self, aui_manager, style=wx.WANTS_CHARS | wx.NO_BORDER)
