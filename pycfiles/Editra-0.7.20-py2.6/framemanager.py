...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/extern/aui/framemanager.py
...
import wx, time, types, warnings, auibar, auibook, tabmdi, dockart, tabart
...
class AuiPaneInfo(object):
    ...
    buttonCustom2 = 2147483648
    buttonCustom3 = 4294967296
    savedHiddenState = 8589934592
    actionPane = 17179869184
    wasMaximized = 34359738368
    needsRestore = 68719476736
    ...
    def IsSnappable(self):
        ...
        return self.IsTopSnappable() or self.IsBottomSnappable() or self.IsLeftSnappable() or self.IsRightSnappable()
    ...
    def DefaultPane(self):
        ...
        state = self.state
        state |= self.optionTopDockable | self.optionBottomDockable | self.optionLeftDockable | self.optionRightDockable | self.optionNotebookDockable | self.optionFloatable | self.optionMovable | self.optionResizable | self.optionCaption | self.optionPaneBorder | self.buttonClose
        self.state = state
        return self
...
class AuiDockingGuide(wx.Frame):
    ...
    def __init__(self, parent, id=wx.ID_ANY, title='', pos=wx.DefaultPosition, size=wx.DefaultSize, style=wx.FRAME_TOOL_WINDOW | wx.STAY_ON_TOP | wx.FRAME_NO_TASKBAR | wx.NO_BORDER, name='AuiDockingGuide'):
        """
...
class AuiDockingGuideWindow(wx.Window):
    ...
    def DrawIcon(self, dc):
        ...
        if self._direction == wx.TOP:
            ...
            point = rect.GetTopLeft()
            length = rect.width
        dc.GradientFillLinear(rect, colourIconDockingPart1, colourIconDockingPart2, self._direction)
        dc.SetPen(wx.Pen(colourIconBorder))
        if self._direction == wx.CENTER:
...
class AuiSingleDockingGuide(AuiDockingGuide):
    ...
    def __init__(self, parent, direction=0):
        ...
        """
        self._direction = direction
        style = wx.FRAME_TOOL_WINDOW | wx.STAY_ON_TOP | wx.FRAME_NO_TASKBAR | wx.NO_BORDER
...
class AuiCenterDockingGuide(AuiDockingGuide):
    ...
    def __init__(self, parent):
        ...
        :param `parent`: the :class:`AuiManager` parent.
        """
        AuiDockingGuide.__init__(self, parent, style=wx.FRAME_TOOL_WINDOW | wx.STAY_ON_TOP | wx.FRAME_NO_TASKBAR | wx.NO_BORDER | wx.FRAME_SHAPED, name='auiCenterDockTarget')
        self.Hide()
    ...
    def CreateShapesWithStyle(self):
        ...
        if not self._useAero:
            ...
            tld = [
             wx.Point(rectTop.x, rectTop.y + rectTop.height - 8),
             wx.Point(rectLeft.x + rectLeft.width - 8, rectLeft.y),
             rectTop.GetBottomLeft()]
            bld = [
             wx.Point(rectLeft.x + rectLeft.width - 8, rectLeft.y + rectLeft.height),
             wx.Point(rectBottom.x, rectBottom.y + 8),
             rectBottom.GetTopLeft()]
            trd = [
             wx.Point(rectTop.x + rectTop.width, rectTop.y + rectTop.height - 8),
             wx.Point(rectRight.x + 8, rectRight.y),
             rectRight.GetTopLeft()]
            brd = [
             wx.Point(rectRight.x + 8, rectRight.y + rectRight.height),
            ...
            self._allAeroBmps = [
             aero_dock_pane_left.GetBitmap(), aero_dock_pane_top.GetBitmap(),
            ...
            region = wx.RegionFromBitmap(self._aeroBmp)
            self._allAeroBmps = [
             whidbey_dock_pane_left.GetBitmap(), whidbey_dock_pane_top.GetBitmap(),
    ...
    def HitTest(self, x, y):
        ...
        if not self._useAero:
            ...
            constants = [
             wx.LEFT, wx.UP, wx.RIGHT, wx.DOWN, wx.CENTER]
            lenRects = len(self._aeroRects)
            for (indx, rect) in enumerate(self._aeroRects):
...
class AuiDockingHintWindow(wx.Frame):
    ...
    def __init__(self, parent, id=wx.ID_ANY, title='', pos=wx.DefaultPosition, size=wx.Size(1, 1), style=wx.FRAME_TOOL_WINDOW | wx.FRAME_FLOAT_ON_PARENT | wx.FRAME_NO_TASKBAR | wx.NO_BORDER | wx.FRAME_SHAPED, name='auiHintWindow'):
        """
        Default class constructor. Used internally, do not call it in your code!
    ...
    def MakeVenetianBlinds(self):
        ...
        for y in xrange(size.y):
            j = (y & 8 and [1] or [0])[0] | (y & 4 and [2] or [0])[0] | (y & 2 and [4] or [0])[0] | (y & 1 and [8] or [0])[0]
            if 16 * j + 8 < amount:
...
class AuiFloatingFrame(wx.MiniFrame):
    ...
    def __init__(self, parent, owner_mgr, pane=None, id=wx.ID_ANY, title='', style=wx.FRAME_TOOL_WINDOW | wx.FRAME_FLOAT_ON_PARENT | wx.FRAME_NO_TASKBAR | wx.CLIP_CHILDREN):
        """
        Default class constructor. Used internally, do not call it in your code!
        ...
        wx.MiniFrame.__init__(self, parent, id, title, pos=pane.floating_pos, size=pane.floating_size, style=style, name='auiFloatingFrame')
    ...
    def SetPaneWindow(self, pane):
        ...
        contained_pane.Dock().Center().Show().CaptionVisible(False).PaneBorder(False).Layer(0).Row(0).Position(0)
        ...
        if pane_best_size.IsFullySpecified() and (pane_best_size.x < pane_min_size.x or pane_best_size.y < pane_min_size.y):
            pane_min_size = pane_best_size
            self._pane_window.SetMinSize(pane_min_size)
        cur_max_size = self.GetMaxSize()
        if cur_max_size.IsFullySpecified() and (cur_max_size.x < pane_min_size.x or cur_max_size.y < pane_min_size.y):
        ...
        button_size = art_provider.GetMetric(AUI_DOCKART_PANE_BUTTON_SIZE) + 4 * art_provider.GetMetric(AUI_DOCKART_PANE_BORDER_SIZE)
        ...
        self._fly_step = abs(pane.floating_size.y - (caption_size + 2 * wx.SystemSettings.GetMetric(wx.SYS_EDGE_Y))) / 10
    ...
    def OnMoveEvent(self, event):
        ...
        if vert_dist >= horiz_dist:
            if win_rect.y < self._last3_rect.y:
                ...
                dir = wx.SOUTH
        elif win_rect.x < self._last3_rect.x:
            dir = wx.WEST
        else:
            dir = wx.EAST
        self._last3_rect = wx.Rect(*self._last2_rect)
    ...
    def OnFlyTimer(self, event):
        ...
        elif current_size.y + self._fly_step >= floating_size.y:
...
def GetMaxRow(panes, dock_direction, dock_layer):
    ...
    max_row = 0
    for pane in panes:
        if pane.dock_direction == dock_direction and pane.dock_layer == dock_layer and pane.dock_row > max_row:
...
def DoInsertDockRow(panes, dock_direction, dock_layer, dock_row):
    ...
    """
    for pane in panes:
        if not pane.IsFloating() and pane.dock_direction == dock_direction and pane.dock_layer == dock_layer and pane.dock_row >= dock_row:
            pane.dock_row += 1
...
def DoInsertPane(panes, dock_direction, dock_layer, dock_row, dock_pos):
    ...
    for ii in xrange(len(panes)):
        pane = panes[ii]
        if not pane.IsFloating() and pane.dock_direction == dock_direction and pane.dock_layer == dock_layer and pane.dock_row == dock_row and pane.dock_pos >= dock_pos:
...
def FindDocks(docks, dock_direction, dock_layer=-1, dock_row=-1, reverse=False):
    ...
    :param integer `dock_row`: the :class:`AuiDockInfo` row to analyze;
    """
    matchDocks = [ (d.dock_layer, d.dock_row, d.dock_direction, d) for d in docks if dock_direction == -1 or dock_direction == d.dock_direction if dock_layer == -1 or dock_layer == d.dock_layer if dock_row == -1 or dock_row == d.dock_row
                 ]
...
def CheckEdgeDrop(window, docks, pt):
    ...
    if screenPt.y >= frameRect.GetTop() and screenPt.y < frameRect.GetBottom():
        if pt.x < auiLayerInsertOffset and pt.x > auiLayerInsertOffset - auiLayerInsertPixels:
            return wx.LEFT
        if pt.x >= clientSize.x - auiLayerInsertOffset and pt.x < clientSize.x - auiLayerInsertOffset + auiLayerInsertPixels:
            return wx.RIGHT
    if screenPt.x >= frameRect.GetLeft() and screenPt.x < frameRect.GetRight():
        if pt.y < auiLayerInsertOffset and pt.y > auiLayerInsertOffset - auiLayerInsertPixels:
            return wx.TOP
        if pt.y >= clientSize.y - auiLayerInsertOffset and pt.y < clientSize.y - auiLayerInsertOffset + auiLayerInsertPixels:
            return wx.BOTTOM
    return -1
...
def IsDifferentDockingPosition(pane1, pane2):
    ...
    """
    return pane1.IsFloating() != pane2.IsFloating() or pane1.dock_direction != pane2.dock_direction or pane1.dock_layer != pane2.dock_layer or pane1.dock_row != pane2.dock_row
...
class AuiManager(wx.EvtHandler):
    ...
    **Layer** - A layer is akin to an onion. Layer 0 is the very center of the managed pane.
    ...
    def __init__(self, managed_window=None, agwFlags=None):
        ...
        self._autoNBStyle = AUI_NB_DEFAULT_STYLE | AUI_NB_BOTTOM | AUI_NB_SUB_NOTEBOOK | AUI_NB_TAB_EXTERNAL_MOVE
    ...
    def SetManagedWindow(self, managed_window):
        ...
        if isinstance(self._frame, wx.MDIParentFrame):
            ...
            self.AddPane(client_window, AuiPaneInfo().Name('mdiclient').CenterPane().PaneBorder(False))
    ...
    def CreateGuideWindows(self):
        ...
        self._guides.append(AuiDockingGuideInfo().Left().Host(AuiSingleDockingGuide(self._frame, wx.LEFT)))
        self._guides.append(AuiDockingGuideInfo().Top().Host(AuiSingleDockingGuide(self._frame, wx.TOP)))
        self._guides.append(AuiDockingGuideInfo().Right().Host(AuiSingleDockingGuide(self._frame, wx.RIGHT)))
        self._guides.append(AuiDockingGuideInfo().Bottom().Host(AuiSingleDockingGuide(self._frame, wx.BOTTOM)))
        self._guides.append(AuiDockingGuideInfo().Centre().Host(AuiCenterDockingGuide(self._frame)))
    ...
    def InsertPane(self, window, pane_info, insert_level=AUI_INSERT_PANE):
        ...
        if insert_level == AUI_INSERT_PANE:
            self._panes = DoInsertPane(self._panes, pane_info.dock_direction, pane_info.dock_layer, pane_info.dock_row, pane_info.dock_pos)
        elif insert_level == AUI_INSERT_ROW:
            self._panes = DoInsertDockRow(self._panes, pane_info.dock_direction, pane_info.dock_layer, pane_info.dock_row)
        elif insert_level == AUI_INSERT_DOCK:
            self._panes = DoInsertDockLayer(self._panes, pane_info.dock_direction, pane_info.dock_layer)
    ...
    def LoadPaneInfo(self, pane_part, pane):
        ...
        """
        pane_part = pane_part.replace('\\|', '\x07')
        pane_part = pane_part.replace('\\;', '\x08')
        ...
        pane.name = pane.name.replace('\x07', '|')
        pane.name = pane.name.replace('\x08', ';')
        pane.caption = pane.caption.replace('\x07', '|')
        pane.caption = pane.caption.replace('\x08', ';')
        pane_part = pane_part.replace('\x07', '|')
        pane_part = pane_part.replace('\x08', ';')
        return pane
    ...
    def LayoutAddPane(self, cont, dock, pane, uiparts, spacer_only):
        ...
        if pane.HasBorder():
            sizer_item = cont.Add(horz_pane_sizer, pane_proportion, wx.EXPAND | wx.ALL, pane_border_size)
            part = AuiDockUIPart()
    ...
    def LayoutAddDock(self, cont, dock, uiparts, spacer_only):
        ...
        if not self._has_maximized and not dock.fixed and dock.dock_direction in [AUI_DOCK_BOTTOM, AUI_DOCK_RIGHT]:
        ...
        if dock.fixed:
            ...
            if not self._has_maximized and not dock.fixed and dock.dock_direction in [AUI_DOCK_TOP, AUI_DOCK_LEFT]:
                sizer_item = cont.Add((sash_size, sash_size), 0, wx.EXPAND)
                part = AuiDockUIPart()
                part.type = AuiDockUIPart.typeDockSizer
                part.dock = dock
            ...
            if p.IsShown():
                ...
                if arr:
                    ...
                    if p.HasFlag(p.needsRestore) and not p.HasFlag(p.wasMaximized):
                        ...
                        sizes = [ d.rect[2:][isHor] for d in docks if d.IsOk() if d.IsHorizontal() == isHor if not d.toolbar if d.dock_direction != AUI_DOCK_CENTER
                                ]
        ...
        for (ii, dock) in enumerate(docks):
            ...
            for pane in dock.panes:
                if pane.min_size != wx.Size(-1, -1):
                    ...
                    elif pane.min_size.x > dock_min_size:
    ...
    def GetDockSizeConstraint(self):
        ...
        """
        return (
         self._dock_constraint_x, self._dock_constraint_y)
        ...
        for ii in xrange(pane_count):
            ...
            if p.IsFloating():
                if pFrame is None:
                    ...
                    if self._action in [actionDragFloatingPane, actionDragToolbarPane] and self._agwFlags & AUI_MGR_TRANSPARENT_DRAG:
                    ...
                    if pFrame.GetPosition() != p.floating_pos or pFrame.GetSize() != p.floating_size:
                        pFrame.SetDimensions(p.floating_pos.x, p.floating_pos.y, p.floating_size.x, p.floating_size.y, wx.SIZE_USE_EXISTING)
    ...
    def UpdateNotebook(self):
        ...
        for (nb, notebook) in enumerate(self._notebooks):
            if notebook.GetPageCount() == 1:
                ...
                sorted_attributes = [ self.GetAttributes(tup[1]) for tup in sorted_pnp
                                    ]
    ...
    def GetPanePart(self, wnd):
        ...
        """
        for part in self._uiparts:
            if part.type == AuiDockUIPart.typePaneBorder and part.pane and part.pane.window == wnd:
                return part

        for part in self._uiparts:
            if part.type == AuiDockUIPart.typePane and part.pane and part.pane.window == wnd:
                return part
    ...
    def GetDockPixelOffset(self, test):
        ...
        sizer.Destroy()
        for dock in docks:
            if test.dock_direction == dock.dock_direction and test.dock_layer == dock.dock_layer and test.dock_row == dock.dock_row:
                if dock.IsVertical():
                    return dock.rect.y
                else:
                    return dock.rect.x
    ...
    def CheckMovableSizer(self, part):
        ...
        """
        if part.type == AuiDockUIPart.typeDockSizer and part.dock and len(part.dock.panes) == 1 and part.dock.panes[0].IsFixed():
            return False
        if part.pane:
    ...
    def DoDropToolbar(self, docks, panes, target, pt, offset):
        ...
        if dropDir != -1:
            if dropDir == wx.LEFT:
                drop.Dock().Left().Layer(auiToolBarLayer).Row(0).Position(pt.y - self.GetDockPixelOffset(drop) - offset.y)
            elif dropDir == wx.RIGHT:
                drop.Dock().Right().Layer(auiToolBarLayer).Row(0).Position(pt.y - self.GetDockPixelOffset(drop) - offset.y)
            elif dropDir == wx.TOP:
                drop.Dock().Top().Layer(auiToolBarLayer).Row(0).Position(pt.x - self.GetDockPixelOffset(drop) - offset.x)
            elif dropDir == wx.BOTTOM:
                drop.Dock().Bottom().Layer(auiToolBarLayer).Row(0).Position(pt.x - self.GetDockPixelOffset(drop) - offset.x)
            if not target.IsFloating() and safeRect.Contains(pt) and target.dock_direction != drop.dock_direction:
                return (False, target)
            return self.ProcessDockResult(target, drop)
        ...
        if not dock.fixed or dock.dock_direction == AUI_DOCK_CENTER:
            if self._agwFlags & AUI_MGR_ALLOW_FLOATING and drop.IsFloatable() or dock.dock_direction not in [AUI_DOCK_CENTER, AUI_DOCK_NONE]:
                if drop.IsFloatable():
                    drop.Float()
            return self.ProcessDockResult(target, drop)
        dockDropOffset = 0
        ...
        drop.Dock().Direction(dock.dock_direction).Layer(dock.dock_layer).Row(dock.dock_row).Position(dockDropOffset)
        if pt.y <= dock.rect.GetTop() + 2 and dock.IsHorizontal() or pt.x <= dock.rect.GetLeft() + 2 and dock.IsVertical():
        ...
        if pt.y >= dock.rect.GetBottom() - 2 and dock.IsHorizontal() or pt.x >= dock.rect.GetRight() - 2 and dock.IsVertical():
            if dock.dock_direction in [AUI_DOCK_TOP, AUI_DOCK_LEFT]:
                panes = DoInsertDockRow(panes, dock.dock_direction, dock.dock_layer, dock.dock_row + 1)
                drop.dock_row = dock.dock_row + 1
            else:
    ...
    def DoDropNonFloatingPane(self, docks, panes, target, pt):
        ...
        if direction == AUI_DOCK_TOP:
            ...
            if pt.y > part.rect.y + part.rect.height - auiInsertRowPixels and pt.y <= part.rect.y + part.rect.height:
                insert_dock_row = True
        elif direction == AUI_DOCK_LEFT:
            if pt.x >= part.rect.x and pt.x < part.rect.x + auiInsertRowPixels:
                insert_dock_row = True
        elif direction == AUI_DOCK_RIGHT:
            if pt.x > part.rect.x + part.rect.width - auiInsertRowPixels and pt.x <= part.rect.x + part.rect.width:
                insert_dock_row = True
        elif direction == AUI_DOCK_CENTER:
        ...
        if insert_dock_row:
            ...
            drop.Dock().Direction(insert_dir).Layer(insert_layer).Row(insert_row).Position(0)
            return self.ProcessDockResult(target, drop)
        ...
        if offset > size / 2:
            ...
            panes = DoInsertPane(panes, part.pane.dock_direction, part.pane.dock_layer, part.pane.dock_row, part.pane.dock_pos + 1)
        drop.Dock().Direction(part.dock.dock_direction).Layer(part.dock.dock_layer).Row(part.dock.dock_row).Position(drop_position)
        return self.ProcessDockResult(target, drop)
    ...
    def DoDropLayer(self, docks, target, dock_direction):
        ...
        if dock_direction == AUI_DOCK_LEFT:
            drop.Dock().Left()
            drop_new_layer = max(max(GetMaxLayer(docks, AUI_DOCK_LEFT), GetMaxLayer(docks, AUI_DOCK_BOTTOM)), GetMaxLayer(docks, AUI_DOCK_TOP)) + 1
        elif dock_direction == AUI_DOCK_TOP:
            drop.Dock().Top()
            drop_new_layer = max(max(GetMaxLayer(docks, AUI_DOCK_TOP), GetMaxLayer(docks, AUI_DOCK_LEFT)), GetMaxLayer(docks, AUI_DOCK_RIGHT)) + 1
        elif dock_direction == AUI_DOCK_RIGHT:
            drop.Dock().Right()
            drop_new_layer = max(max(GetMaxLayer(docks, AUI_DOCK_RIGHT), GetMaxLayer(docks, AUI_DOCK_TOP)), GetMaxLayer(docks, AUI_DOCK_BOTTOM)) + 1
        elif dock_direction == AUI_DOCK_BOTTOM:
            drop.Dock().Bottom()
            drop_new_layer = max(max(GetMaxLayer(docks, AUI_DOCK_BOTTOM), GetMaxLayer(docks, AUI_DOCK_LEFT)), GetMaxLayer(docks, AUI_DOCK_RIGHT)) + 1
        else:
            return (
             False, target)
    ...
    def ShowHint(self, rect):
        ...
        if self._hint_fadeamt != self._hint_fademax:
    ...
    def IsPaneButtonVisible(self, part):
        ...
        for temp_part in self._uiparts:
            if temp_part.pane == part.pane and temp_part.type == AuiDockUIPart.typeCaption:
    ...
    def DrawPaneButton(self, dc, part, pt):
        ...
        self._art.DrawPaneButton(dc, self._frame, part.button.button_id, state, part.rect, part.pane)
    ...
    def CalculateHintRect(self, pane_window, pt, offset):
        ...
        for part in uiparts:
            if part.pane and part.pane.name == sought:
                rect.Union(wx.RectPS(part.sizer_item.GetPosition(), part.sizer_item.GetSize()))
    ...
    def OnRender(self, event):
        ...
        dc = event.GetDC()
        for part in self._uiparts:
            if part.sizer_item and (not part.sizer_item.IsWindow() and not part.sizer_item.IsSpacer() and not part.sizer_item.IsSizer() or not part.sizer_item.IsShown()):
                continue
            ptype = part.type
    ...
    def OnCaptionDoubleClicked(self, pane_window):
        ...
        if not paneInfo.IsOk():
            raise Exception('Pane window not found')
        if not paneInfo.IsFloatable() or not paneInfo.IsDockable() or self._agwFlags & AUI_MGR_ALLOW_FLOATING == 0:
            return
        else:
    ...
    def OnSize(self, event):
        ...
        if self._frame:
            ...
            if wx.Platform == '__WXMAC__':
                ...
                self.Repaint()
            if isinstance(self._frame, wx.MDIParentFrame) or isinstance(self._frame, tabmdi.AuiMDIClientWindow) or isinstance(self._frame, tabmdi.AuiMDIParentFrame):
                return
        if not skipped:
            event.Skip()
        self.OnMove(None)
    ...
    def UpdateButtonOnScreen(self, button_ui_part, event):
        ...
        if hit_test.pane:
            self._art.DrawPaneButton(cdc, self._frame, button_ui_part.button.button_id, state, button_ui_part.rect, hit_test.pane)
    ...
    def OnLeftDown(self, event):
        ...
        if not part:
            ...
            if part.type in [AuiDockUIPart.typeDockSizer, AuiDockUIPart.typePaneSizer]:
                ...
                self._action_offset = wx.Point(event.GetX() - part.rect.x, event.GetY() - part.rect.y)
                rect = wx.RectPS(self._frame.ClientToScreen(part.rect.GetPosition()), part.rect.GetSize())
    ...
    def OnMotion_ClickCaption(self, event):
        ...
        if not self._action_pane:
            return
        if not (abs(clientPt.x - self._action_start.x) > drag_x_threshold or abs(clientPt.y - self._action_start.y) > drag_y_threshold):
            return
    ...
    def CheckPaneMove(self, pane):
        ...
        if self._last_rect.IsEmpty():
            self._last_rect = wx.Rect(*win_rect)
            return False
        if abs(win_rect.x - self._last_rect.x) > 10 or abs(win_rect.y - self._last_rect.y) > 10:
            self._last_rect = wx.Rect(*win_rect)
            return False
        return True
    ...
    def OnMotion_DragFloatingPane(self, eventOrPt):
        ...
        if pane.frame:
            if diff.x != 0 or diff.y != 0:
                if wx.Platform == '__WXMSW__' and self._agwFlags & AUI_MGR_TRANSPARENT_DRAG == 0:
        ...
        for paneInfo in self._panes:
            ...
            if paneInfo.IsNotebookPage():
                ...
                if not notebookRoot or not notebookRoot.IsDocked():
                    continue
            rc = paneInfo.window.GetScreenRect()
            if rc.Contains(screenPt) and (rc.height < 20 or rc.width < 20):
                pass
            else:
                return
    ...
    def OnMotion_Other(self, event):
        ...
        """
        part = self.HitTest(*event.GetPosition())
        if part and part.type == AuiDockUIPart.typePaneButton and self.IsPaneButtonVisible(part):
    ...
    def OnPaneButton(self, event):
        ...
        if event.button == AUI_BUTTON_CLOSE:
            ...
            if rootManager != self:
                ...
                self.ProcessMgrEvent(e)
                check = e.GetVeto() or self.GetPane(pane.window)
                if check.IsOk():
                    self.ClosePane(pane)
                self.Update()
        ...
        elif event.button == AUI_BUTTON_MAXIMIZE_RESTORE:
            if not pane.IsMaximized():
                e = AuiManagerEvent(wxEVT_AUI_PANE_MAXIMIZE)
                e.SetManager(self)
                e.SetPane(event.pane)
                self.ProcessMgrEvent(e)
                e.GetVeto() or self.MaximizePane(pane)
                self.Update()
        elif event.button == AUI_BUTTON_MAXIMIZE_RESTORE:
            if pane.IsMaximized():
                ...
                e.SetPane(event.pane)
                self.ProcessMgrEvent(e)
                e.GetVeto() or self.RestorePane(pane)
                self.Update()
    ...
    def MinimizePane(self, paneInfo, mgrUpdate=True):
        ...
        """
        if paneInfo.IsToolbar() or paneInfo.IsMinimized() and mgrUpdate:
            return
        else:
            ...
            if paneInfo.icon:
                ...
                minimize_toolbar.AddSimpleTool(ID_RESTORE_FRAME, paneInfo.caption, restore_bitmap, _('Restore %s') % paneInfo.caption, target=target)
                ...
                if posMask != AUI_MINIMIZE_POS_TOOLBAR:
                    if dockDirection == AUI_DOCK_TOP:
                        self.AddPane(minimize_toolbar, AuiPaneInfo().Name(toolpanelname).Caption(paneInfo.caption).ToolbarPane().Top().BottomDockable(False).LeftDockable(False).RightDockable(False).DestroyOnClose())
                    elif dockDirection == AUI_DOCK_BOTTOM:
                        self.AddPane(minimize_toolbar, AuiPaneInfo().Name(toolpanelname).Caption(paneInfo.caption).ToolbarPane().Bottom().TopDockable(False).LeftDockable(False).RightDockable(False).DestroyOnClose())
                    elif dockDirection == AUI_DOCK_LEFT:
                        self.AddPane(minimize_toolbar, AuiPaneInfo().Name(toolpanelname).Caption(paneInfo.caption).ToolbarPane().Left().TopDockable(False).BottomDockable(False).RightDockable(False).DestroyOnClose())
                    elif dockDirection in [AUI_DOCK_RIGHT, AUI_DOCK_CENTER]:
                        self.AddPane(minimize_toolbar, AuiPaneInfo().Name(toolpanelname).Caption(paneInfo.caption).ToolbarPane().Right().TopDockable(False).LeftDockable(False).BottomDockable(False).DestroyOnClose())
    ...
    def CreateNotebookBase(self, panes, paneInfo):
        ...
        baseInfo.SetDockPos(paneInfo).NotebookControl(nbid).CloseButton(False).SetNameFromNotebookId().NotebookDockable(False).Floatable(paneInfo.IsFloatable())
    ...
    def SlideIn(self, event):
        ...
        self._sliding_frame = wx.MiniFrame(None, -1, title=_('Pane Preview'), style=wx.FRAME_TOOL_WINDOW | wx.STAY_ON_TOP | wx.FRAME_NO_TASKBAR | wx.CAPTION)
...
class AuiManager_DCP(AuiManager):
    ...
    def Update(self):
        ...
        haveCenterPane = any(pane != dummyCenterPane and pane.dock_direction == AUI_DOCK_CENTER and not pane.IsFloating() and pane.IsShown() for pane in self.GetAllPanes())
        if haveCenterPane:
            if self.hasDummyPane:
