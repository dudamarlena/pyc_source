...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/plugin.py
...
import os, sys, shutil, wx, ed_glob
...
class PluginMeta(type):
    ...
    def __new__(mcs, name, bases, d):
        ...
        init = d.get('__init__')
        if not init:
            for init in [ b.__init__._original for b in new_obj.mro() if issubclass(b, Plugin) if '__init__' in b.__dict__ ]:
                break
...
class PluginManager(object):
    ...
    def CreateEnvironment(self, path):
        ...
        if pkg_resources != None:
            path = [ EncodeString(pname, sys.getfilesystemencoding()) for pname in path ]
            try:
    ...
    def LoadPluginConfig(self):
        ...
        """
        config = dict()
        reader = util.GetFileReader(os.path.join(ed_glob.CONFIG['CONFIG_DIR'], PLUGIN_CONFIG))
        if reader == -1:
    ...
    def WritePluginConfig(self):
        ...
        """
        writer = util.GetFileWriter(os.path.join(ed_glob.CONFIG['CONFIG_DIR'], PLUGIN_CONFIG))
        if writer == -1:
