...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/extern/aui/auibar.py
...
class AuiDefaultToolBarArt(object):
    ...
    def __init__(self):
        ...
        self._disabled_button_dropdown_bmp = BitmapFromBits(button_dropdown_bits, 5, 3, wx.Colour(128, 128, 128))
    ...
    def DrawDropDownButton(self, dc, wnd, item, rect):
        ...
        if horizontal:
            ...
            dc.DrawBitmap(wx.BitmapFromImage(dropbmp.ConvertToImage().Rotate90(item.GetOrientation() == AUI_TBTOOL_VERT_CLOCKWISE)), dropbmp_x, dropbmp_y, True)
        dc.SetTextForeground(wx.BLACK)
        if item.GetState() & AUI_BUTTON_STATE_DISABLED:
            dc.SetTextForeground(DISABLED_TEXT_COLOUR)
        if self._agwFlags & AUI_TB_TEXT and item.GetLabel() != '':
    ...
    def GetElementSize(self, element_id):
        ...
        if element_id == AUI_TBART_GRIPPER_SIZE:
            return self._gripper_size
        if element_id == AUI_TBART_OVERFLOW_SIZE:
        ...
        for item in items:
            if item.GetKind() not in [ITEM_SEPARATOR, ITEM_SPACER, ITEM_CONTROL]:
                ...
                if kind in [ITEM_CHECK, ITEM_RADIO]:
                    ...
                    m.Check(state)
                items_added += 1
            elif items_added > 0 and item.GetKind() == ITEM_SEPARATOR:
...
class AuiToolBar(wx.PyControl):
    ...
    def __init__(self, parent, id=wx.ID_ANY, pos=wx.DefaultPosition, size=wx.DefaultSize, style=0, agwStyle=AUI_TB_DEFAULT_STYLE):
        """
        Default class constructor.
        ...
        if not dc.IsOk():
            ...
            for (i, item) in enumerate(self._items):
                ...
                if kind == ITEM_LABEL:
                    ...
                    vert_sizer.AddStretchSpacer(1)
                    if self._agwStyle & AUI_TB_TEXT and self._tool_text_orientation == AUI_TBTOOL_TEXT_BOTTOM and item.GetLabel() != '':
    ...
    def OnSize(self, event):
        ...
        if x >= y and self._absolute_min_size.x > x or y > x and self._absolute_min_size.y > y:
            ...
            if self._originalStyle & AUI_TB_OVERFLOW and not self._custom_overflow_append and not self._custom_overflow_prepend:
    ...
    def OnPaint(self, event):
        ...
        for item in self._items:
            ...
            if horizontal and item_rect.x + item_rect.width >= last_extent or not horizontal and item_rect.y + item_rect.height >= last_extent:
                break
    ...
    def OnLeftDown(self, event):
        ...
        if self._action_item:
            ...
            if self._action_item.dropdown:
                if self._action_item.orientation == AUI_TBTOOL_HORIZONTAL and mouse_x >= rect.x + rect.width - BUTTON_DROPDOWN_WIDTH - 1 and mouse_x < rect.x + rect.width or self._action_item.orientation != AUI_TBTOOL_HORIZONTAL and mouse_y >= rect.y + rect.height - BUTTON_DROPDOWN_WIDTH - 1 and mouse_y < rect.y + rect.height:
    ...
    def OnRightDown(self, event):
        ...
        if self._overflow_sizer_item:
            dropdown_size = self._art.GetElementSize(AUI_TBART_OVERFLOW_SIZE)
            if dropdown_size > 0 and event.GetX() > cli_rect.width - dropdown_size and event.GetY() >= 0 and event.GetY() < cli_rect.height and self._art:
                return
        self._action_pos = wx.Point(*event.GetPosition())
        self._action_item = self.FindToolForPosition(*event.GetPosition())
        if self._action_item:
    ...
    def OnMotion(self, event):
        ...
        :param `event`: a :class:`MouseEvent` event to be processed.        
        """
        if not self._dragging and self._action_item != None and self._action_pos != wx.Point(-1, -1) and abs(event.GetX() - self._action_pos.x) + abs(event.GetY() - self._action_pos.y) > 5:
            self.SetToolTipString('')
            self._dragging = True
