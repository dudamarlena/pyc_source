...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/eclib/elistctrl.py
...
__all__ = [
 'EBaseListCtrl', 'ECheckListCtrl', 'EEditListCtrl',
 'EToggleEditListCtrl']
import wx, wx.lib.mixins.listctrl as listmix, elistmix

class EBaseListCtrl(elistmix.ListRowHighlighter, listmix.ListCtrlAutoWidthMixin, wx.ListCtrl):
    """Base listctrl class that provides automatic row highlighting"""

    def __init__(self, parent, _id=wx.ID_ANY, pos=wx.DefaultPosition, size=wx.DefaultSize, style=wx.LC_REPORT, validator=wx.DefaultValidator, name='EListCtrl'):
        wx.ListCtrl.__init__(self, parent, _id, pos, size, style, validator, name)
    ...
    def GetSelections(self):
        ...
        """
        items = [ idx for idx in range(self.GetItemCount()) if self.IsSelected(idx)
                ]
        return items
...
class ECheckListCtrl(listmix.CheckListCtrlMixin, EBaseListCtrl):
...
class EEditListCtrl(listmix.TextEditMixin, EBaseListCtrl):
...
class EToggleEditListCtrl(listmix.CheckListCtrlMixin, listmix.TextEditMixin, EBaseListCtrl):
