...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/profiler.py
...
_DEFAULTS = {'ALPHA': 255, 
   ...
   'MAXIMIZED': False, 
   'MODE': 'CODE', 
   'MYPROFILE': 'default.ppb', 
   'OPEN_NW': False, 
   'PRINT_MODE': PRINT_BLACK_WHITE, 
   'PROXY_SETTINGS': dict(), 
   'REPORTER': True, 
   'SAVE_POS': True, 
   'SAVE_SESSION': False, 
   ...
   'SHOW_LN': True, 
   'SHOW_WS': False, 
   'SPELLCHECK': dict(auto=False, dict='en_US', epath=None), 
   'STATBAR': True, 
   'SYNTAX': True, 
   'SYNTHEME': 'Default', 
   'TABICONS': True, 
    ...
    def Load(self, path):
        ...
        if os.path.exists(path):
            try:
                ...
                fhandle.close()
            except (IOError, SystemError, OSError, cPickle.UnpicklingError, EOFError), msg:
...
def CalcVersionValue(ver_str='0.0.0'):
    ...
    """
    ver_str = ('').join([ char for char in ver_str if char.isdigit() or char == '.'
                        ])
...
def UpdateProfileLoader():
    ...
    if not os.path.exists(prof_name):
        prof_name = os.path.join(CONFIG['CONFIG_DIR'], os.path.basename(prof_name))
