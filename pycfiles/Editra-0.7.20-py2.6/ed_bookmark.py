...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/ed_bookmark.py
# Compiled at: 2011-10-01 11:27:11
"""
Bookmark manager
...
import os, re, wx, ed_msg, iface, plugin
from profiler import Profile_Get, Profile_Set
import ed_glob, util, eclib, ebmlib, ed_basewin
...
class EdBookmarks(plugin.Plugin):
    ...
    def GetMenuEntry(menu):
        ...
        """
        item = wx.MenuItem(menu, ed_glob.ID_BOOKMARK_MGR, _('Bookmarks'), _('View all bookmarks'))
        bmp = wx.ArtProvider.GetBitmap(str(ed_glob.ID_ADD_BM), wx.ART_MENU)
        item.SetBitmap(bmp)
        return item
...
class BookmarkWindow(ed_basewin.EdBaseCtrlBox):
    ...
    def __init__(self, parent):
        ...
        self._delbtn = self.AddPlateButton(_('Delete'), ed_glob.ID_DELETE, wx.ALIGN_RIGHT)
        self._delbtn.ToolTip = wx.ToolTip(_('Delete Bookmark'))
        self._delallbtn = self.AddPlateButton(_('Delete All'), ed_glob.ID_DELETE_ALL, wx.ALIGN_RIGHT)
    ...
    def GotoBookmark(self, mark):
        ...
        if mw:
            ...
            use_handle = True
            if not buf:
                nb.OpenPage(ebmlib.GetPathName(mark.Filename), ebmlib.GetFileName(mark.Filename))
...
class BookmarkList(eclib.EBaseListCtrl):
    ...
    def __init__(self, parent):
        super(BookmarkList, self).__init__(parent, style=wx.LC_REPORT | wx.LC_EDIT_LABELS | wx.LC_VIRTUAL)
        ...
        self.setResizeColumn(BookmarkList.FILE_NAME + 1)
