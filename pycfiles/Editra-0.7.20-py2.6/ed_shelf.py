...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/ed_shelf.py
...
import re, wx, ed_menu, ed_glob
from profiler import Profile_Get
import ed_msg, plugin, iface, ed_fmgr
from extern import aui
import ed_book, ebmlib
...
class Shelf(plugin.Plugin):
    ...
    def GetUiHandlers(self, delegate):
        ...
        """
        handlers = [ (item.GetId(), delegate.UpdateShelfMenuUI) for item in self.observers
                   ]
        return handlers
    ...
    def Init(self, parent):
        ...
        mgr.AddPane(wrapper, ed_fmgr.EdPaneInfo().Name(Shelf.SHELF_NAME).Caption(_('Shelf')).Bottom().Layer(0).CloseButton(True).MaximizeButton(True).BestSize(wx.Size(500, 250)))
        ...
        view.InsertMenu(pos + 1, ed_glob.ID_SHELF, _('Shelf'), menu, _('Put an item on the Shelf'))
        ...
        for observer in self.observers:
            if not observer.IsInstalled() and hasattr(observer, 'InstallComponents'):
...
class EdShelfBook(ed_book.EdBaseBook):
    ...
    def __init__(self, parent):
        style = aui.AUI_NB_BOTTOM | aui.AUI_NB_TAB_SPLIT | aui.AUI_NB_SCROLL_BUTTONS | aui.AUI_NB_CLOSE_ON_ACTIVE_TAB | aui.AUI_NB_TAB_MOVE | aui.AUI_NB_DRAW_DND_TAB
    ...
    def OnTabRightUp(self, evt):
        ...
        if self.MenuCallback:
            ...
            tab_menu.Append(EdShelfBook.ID_CLOSE_LIKE_TABS, _("Close All '%s'") % s_lbl)
            self._menu.AddHandler(EdShelfBook.ID_CLOSE_LIKE_TABS, lambda tab, evt: self.CloseAll(s_lbl))
            tab_menu.AppendSeparator()
            shelf_menu = self.MenuCallback()
            tab_menu.AppendMenu(EdShelfBook.ID_SHELF_SUBMENU, _('Open'), shelf_menu)
    ...
    MainWindow = property(lambda self: self._mw, lambda self, mw: setattr(self, '_mw', mw))
    BitmapCallbacks = property(lambda self: self._name2idx)
    MenuCallback = property(lambda self: self._mcback, lambda self, funct: setattr(self, '_mcback', funct))
    ...
    def AddItem(self, item, name, bmp=wx.NullBitmap):
        ...
        """
        self.AddPage(item, '%s - %d' % (name, self._open.get(name, 0)), select=True)
...
class EdShelfDelegate(object):
    ...
    def GetItemStack(self):
        ...
        if self._shelf is not None:
            for page in range(self._shelf.GetPageCount()):
                rval.append(re.sub(PGNUM_PAT, '', self._shelf.GetPageText(page), 1))
    ...
    def GetShelfObjectMenu(self):
        ...
        for item in menu_items:
            ...
            shortcut = ''
            if combo < 10:
                shortcut = '\tCtrl+Alt+' + unicode(combo)
    ...
    def GetMenu(self):
        ...
        menu.Insert(0, ed_glob.ID_SHOW_SHELF, _('Show Shelf') + ed_menu.EdMenuBar.keybinder.GetBinding(ed_glob.ID_SHOW_SHELF), _('Show the Shelf'))
        return menu
    ...
    def PutItemOnShelf(self, shelfid):
        ...
        if item is None:
            ...
            name = item.GetName()
            if self._shelf.ItemIsOnShelf(name) and not item.AllowMultiple() or self._shelf is None:
                return
