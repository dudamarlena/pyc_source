...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/extern/aui/tabart.py
...
class AuiDefaultTabArt(object):
    ...
    def DrawTab(self, dc, wnd, page, in_rect, close_button_state, paint_control=False):
        ...
        (tab_size, x_extent) = self.GetTabSize(dc, wnd, page.caption, page.bitmap, page.active, close_button_state, control)
        ...
        if pagebitmap.IsOk():
            ...
            dc.DrawBitmap(pagebitmap, bitmap_offset, drawn_tab_yoff + drawn_tab_height / 2 - pagebitmap.GetHeight() / 2, True)
            text_offset = bitmap_offset + pagebitmap.GetWidth()
            text_offset += 3
        elif agwFlags & AUI_NB_CLOSE_ON_TAB_LEFT == 0 or not close_button_width:
            text_offset = tab_x + 8
        ...
        if close_button_state != AUI_BUTTON_STATE_HIDDEN:
            ...
            if agwFlags & AUI_NB_CLOSE_ON_TAB_LEFT:
                rect = wx.Rect(tab_x + 4, tab_y + (tab_height - bmp.GetHeight()) / 2 - shift, close_button_width, tab_height)
            else:
                rect = wx.Rect(tab_x + tab_width - close_button_width - 1, tab_y + (tab_height - bmp.GetHeight()) / 2 - shift, close_button_width, tab_height)
    ...
    def DrawButton(self, dc, wnd, in_rect, button, orientation):
        ...
        elif button_state & AUI_BUTTON_STATE_DISABLED:
        ...
        if orientation == wx.LEFT:
            ...
            rect = wx.Rect(in_rect.x + in_rect.width - bmp.GetWidth(), (in_rect.y + in_rect.height) / 2 - bmp.GetHeight() / 2, bmp.GetWidth(), bmp.GetHeight())
    ...
    def DrawFocusRectangle(self, dc, page, wnd, draw_text, text_offset, bitmap_offset, drawn_tab_yoff, drawn_tab_height, textx, texty):
        ...
        if page.active and wx.Window.FindFocus() == wnd:
            focusRectText = wx.Rect(text_offset, drawn_tab_yoff + drawn_tab_height / 2 - texty / 2, textx, texty)
            if page.bitmap.IsOk():
                focusRectBitmap = wx.Rect(bitmap_offset, drawn_tab_yoff + drawn_tab_height / 2 - page.bitmap.GetHeight() / 2, page.bitmap.GetWidth(), page.bitmap.GetHeight())
    ...
    def GetBestTabCtrlSize(self, wnd, pages, required_bmp_size):
        ...
        if required_bmp_size.IsFullySpecified():
            measure_bmp = wx.EmptyBitmap(required_bmp_size.x, required_bmp_size.y)
        max_y = 0
        for page in pages:
            if measure_bmp.IsOk():
                bmp = measure_bmp
    ...
    def ShowDropDown(self, wnd, pages, active_idx):
        ...
        if self.GetAGWFlags() & AUI_NB_CLOSE_BUTTON:
            longest += 16
        pt = wx.Point(cli_rect.x + cli_rect.GetWidth() - longest, cli_rect.y + cli_rect.height)
...
class AuiSimpleTabArt(object):
    ...
    def DrawTab(self, dc, wnd, page, in_rect, close_button_state, paint_control=False):
        ...
        if page.active and wx.Window.FindFocus() == wnd and agwFlags & AUI_NB_NO_TAB_FOCUS == 0:
            focusRect = wx.Rect(text_offset, (tab_y + tab_height) / 2 - texty / 2 + 1, selected_textx, selected_texty)
            focusRect.Inflate(2, 2)
        out_button_rect = wx.Rect()
        if close_button_state != AUI_BUTTON_STATE_HIDDEN:
            if page.active:
                ...
                bmp = self._disabled_close_bmp
            if agwFlags & AUI_NB_CLOSE_ON_TAB_LEFT:
                rect = wx.Rect(tab_x + tab_height - 2, tab_y + tab_height / 2 - bmp.GetHeight() / 2 + 1, close_button_width, tab_height - 1)
            else:
                rect = wx.Rect(tab_x + tab_width - close_button_width - 1, tab_y + tab_height / 2 - bmp.GetHeight() / 2 + 1, close_button_width, tab_height - 1)
    ...
    def GetBestTabCtrlSize(self, wnd, pages, required_bmp_size):
        ...
        dc = wx.ClientDC(wnd)
        dc.SetFont(self._measuring_font)
        (s, x_extent) = self.GetTabSize(dc, wnd, 'ABCDEFGHIj', wx.NullBitmap, True, AUI_BUTTON_STATE_HIDDEN, None)
        max_y = s[1]
        for page in pages:
            if page.control:
                (controlW, controlH) = page.control.GetSize()
...
class VC71TabArt(AuiDefaultTabArt):
    ...
    def DrawTab(self, dc, wnd, page, in_rect, close_button_state, paint_control=False):
        ...
        if pagebitmap.IsOk():
            ...
            if agwFlags & AUI_NB_CLOSE_ON_TAB_LEFT and close_button_width:
                bitmap_offset += close_button_width - 5
            dc.DrawBitmap(pagebitmap, bitmap_offset, drawn_tab_yoff + drawn_tab_height / 2 - pagebitmap.GetHeight() / 2 + shift, True)
            text_offset = bitmap_offset + pagebitmap.GetWidth()
            text_offset += 3
        elif agwFlags & AUI_NB_CLOSE_ON_TAB_LEFT == 0 or not close_button_width:
            text_offset = tab_x + 8
        ...
        out_button_rect = wx.Rect()
        if agwFlags & AUI_NB_NO_TAB_FOCUS == 0:
            self.DrawFocusRectangle(dc, page, wnd, draw_text, offset_focus, bitmap_offset, drawn_tab_yoff + shift, drawn_tab_height + shift, rectx, recty)
        if close_button_state != AUI_BUTTON_STATE_HIDDEN:
            close_button_width = self._active_close_bmp.GetWidth()
            bmp = self._disabled_close_bmp
            if close_button_state == AUI_BUTTON_STATE_HOVER:
                ...
                bmp = self._pressed_close_bmp
            if agwFlags & AUI_NB_CLOSE_ON_TAB_LEFT:
                rect = wx.Rect(tab_x + 4, drawn_tab_yoff + drawn_tab_height / 2 - bmp.GetHeight() / 2 + shift, close_button_width, tab_height)
            else:
                rect = wx.Rect(tab_x + tab_width - close_button_width - 3, drawn_tab_yoff + drawn_tab_height / 2 - bmp.GetHeight() / 2 + shift, close_button_width, tab_height)
            rect = IndentPressedBitmap(rect, close_button_state)
            dc.DrawBitmap(bmp, rect.x, rect.y, True)
            out_button_rect = rect
        out_tab_rect = wx.Rect(tab_x, tab_y, tab_width, tab_height)
...
class FF2TabArt(AuiDefaultTabArt):
    ...
    def GetTabSize(self, dc, wnd, caption, bitmap, active, close_button_state, control):
        ...
        :param Window `control`: a :class:`Window` instance inside a tab (or ``None``).
        """
        (tab_size, x_extent) = AuiDefaultTabArt.GetTabSize(self, dc, wnd, caption, bitmap, active, close_button_state, control)
    ...
    def DrawTab(self, dc, wnd, page, in_rect, close_button_state, paint_control=False):
        ...
        dc.DrawLabel(draw_text, wx.Rect(text_offset, ypos, rectx, recty))
        if agwFlags & AUI_NB_NO_TAB_FOCUS == 0:
            self.DrawFocusRectangle(dc, page, wnd, draw_text, offset_focus, bitmap_offset, drawn_tab_yoff + shift, drawn_tab_height, rectx, recty)
        out_button_rect = wx.Rect()
        if close_button_state != AUI_BUTTON_STATE_HIDDEN:
            close_button_width = self._active_close_bmp.GetWidth()
            bmp = self._disabled_close_bmp
            if close_button_state == AUI_BUTTON_STATE_HOVER:
                ...
                bmp = self._pressed_close_bmp
            if agwFlags & AUI_NB_CLOSE_ON_TAB_LEFT:
                rect = wx.Rect(tab_x + 5, drawn_tab_yoff + drawn_tab_height / 2 - bmp.GetHeight() / 2 + shift, close_button_width, tab_height)
            else:
                rect = wx.Rect(tab_x + tab_width - close_button_width - 3, drawn_tab_yoff + drawn_tab_height / 2 - bmp.GetHeight() / 2 + shift, close_button_width, tab_height)
            rect = IndentPressedBitmap(rect, close_button_state)
            dc.DrawBitmap(bmp, rect.x, rect.y, True)
...
class VC8TabArt(AuiDefaultTabArt):
    ...
    def DrawTab(self, dc, wnd, page, in_rect, close_button_state, paint_control=False):
        ...
        if close_button_state != AUI_BUTTON_STATE_HIDDEN:
            ...
            if agwFlags & AUI_NB_CLOSE_ON_TAB_LEFT:
                rect = wx.Rect(tab_x + 20, drawn_tab_yoff + drawn_tab_height / 2 - bmp.GetHeight() / 2 + shift, close_button_width, tab_height)
            else:
                rect = wx.Rect(xpos, drawn_tab_yoff + drawn_tab_height / 2 - bmp.GetHeight() / 2 + shift, close_button_width, tab_height)
            rect = IndentPressedBitmap(rect, close_button_state)
            dc.DrawBitmap(bmp, rect.x, rect.y, True)
            out_button_rect = rect
...
class ChromeTabArt(AuiDefaultTabArt):
    ...
    def SetBitmaps(self, mirror):
        ...
        """
        bmps = [
         tab_active_left.GetBitmap(), tab_active_center.GetBitmap(),
    ...
    def DrawTab(self, dc, wnd, page, in_rect, close_button_state, paint_control=False):
        ...
        if close_button_state != AUI_BUTTON_STATE_HIDDEN:
            ...
            if close_button_state == AUI_BUTTON_STATE_HOVER:
                ...
                bmp = self._pressed_close_bmp
            if agwFlags & AUI_NB_CLOSE_ON_TAB_LEFT:
                rect = wx.Rect(tab_x + leftw - 2, drawn_tab_yoff + drawn_tab_height / 2 - bmp.GetHeight() / 2 + 1, close_button_width, tab_height)
            else:
                rect = wx.Rect(tab_x + tab_width - close_button_width - rightw + 2, drawn_tab_yoff + drawn_tab_height / 2 - bmp.GetHeight() / 2 + 1, close_button_width, tab_height)
            if agwFlags & AUI_NB_BOTTOM:
                rect.y -= 1
            rect = IndentPressedBitmap(rect, close_button_state)
            dc.DrawBitmap(bmp, rect.x, rect.y, True)
