...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/syntax/_d.py
...
D_KEYWORDS = (0, 'abstract alias align asm assert auto body break case cast catch cent class continue debug default delegate delete deprecated do else enum export extern false final finally for foreach foreach_reverse function goto if import in inout interface invariant is lazy mixin module new null out override package pragma private protected public return scope short struct super switch synchronized template this throw true try union unittest version void while with')
D_TYPES = (1, 'bool byte cdouble cfloat char const creal dchar double float idouble ifloat ireal int real long static typeof typedef typeid ubyte ucent uint ulong ushort volatile wchar')
...
if wx.VERSION >= (2, 9, 0, 0, ''):
    SYNTAX_ITEMS = [
     (
      stc.STC_D_CHARACTER, 'char_style'),
     (
     ...
     (
      stc.STC_D_STRINGB, 'string_style'),
     (
      stc.STC_D_STRINGEOL, 'stringeol_style'),
     (
      stc.STC_D_STRINGR, 'string_style'),
     (
      stc.STC_D_TYPEDEF, 'default_style'),
     (
     ...
     (
      stc.STC_D_WORD5, 'default_style'),
     (
      stc.STC_D_WORD6, 'default_style'),
     (
      stc.STC_D_WORD7, 'default_style')]
else:
    SYNTAX_ITEMS = [
     (
...
class SyntaxData(syndata.SyntaxDataBase):
    ...
    def GetKeywords(self):
        ...
        return [
         D_KEYWORDS, D_TYPES, DOC_KEYWORDS]
