...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/updater.py
# Compiled at: 2011-01-26 08:27:05
"""
...
import os, sys, stat, re, urllib2, threading, wx, wx.lib.delayedresult as delayedresult, ed_glob, ed_event
...
class UpdateService(object):
    ...
    def GetCurrentVersionStr(self):
        ...
        if len(found):
            ...
            util.Log('[updater][warn] UpdateService.GetCurrentVersionStr Failed to get version info.')
    ...
    def GetUpdateFiles(self, dl_to=wx.GetHomeDir()):
        ...
        if CalcVersionValue(ed_glob.VERSION) < CalcVersionValue(current):
            ...
            try:
                ...
                self._abort = False
                if os.path.exists(dl_to) and os.stat(dl_to)[stat.ST_SIZE] == fsize:
                    return True
                return False
...
class UpdateThread(threading.Thread):
    ...
    def run(self):
        ...
        if result.replace('.', '').isdigit():
            ...
            isupdate = False
        evt = ed_event.NotificationEvent(ed_event.edEVT_NOTIFY, self.id, (isupdate, result))
...
class UpdateProgress(wx.Gauge, UpdateService):
    ...
    def __init__(self, parent, id_, range_=100, style=wx.GA_HORIZONTAL | wx.GA_PROGRESSBAR):
...
class DownloadDialog(wx.Frame):
    ...
    def __init__(self, parent, id_, title, style=wx.DEFAULT_DIALOG_STYLE | wx.MINIMIZE_BOX):
        ...
        dl_loc = wx.StaticText(panel, wx.ID_ANY, _('Downloading To: %s') % floc)
        ...
        mdc = wx.MemoryDC(bmp)
        tmp_bmp = wx.Image(ed_glob.CONFIG['SYSPIX_DIR'] + 'editra.png', wx.BITMAP_TYPE_PNG)
    ...
    def OnUpdate(self, evt):
        ...
        if e_id == self.ID_TIMER:
            ...
            if self._progress.IsDownloading():
                self.SetStatusText(_('Rate: %.2f Kb/s') % speed, self.SB_DOWNLOADED)
            else:
