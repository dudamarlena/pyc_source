...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/ed_basestc.py
# Compiled at: 2012-07-15 11:14:29
"""
...
import wx, wx.stc, ed_glob, ed_style, eclib, ebmlib, ed_msg, ed_txt
from syntax import syntax
from syntax import synglob
import autocomp
from extern import vertedit
from profiler import Profile_Get
import plugin, iface, util, ed_marker
MARK_MARGIN = 0
NUM_MARGIN = 1
FOLD_MARGIN = 2
...
class EditraBaseStc(wx.stc.StyledTextCtrl, ed_style.StyleMgr):
    ...
    def __init__(self, parent, id_=wx.ID_ANY, pos=wx.DefaultPosition, size=wx.DefaultSize, style=0):
        ...
        self._code = dict(compsvc=autocomp.AutoCompService.GetCompleter(self), synmgr=syntax.SyntaxMgr(ed_glob.CONFIG['CACHE_DIR']), keywords=[
         ' '], comment=list(), clexer=None, indenter=None, lang_id=0)
        ...
        if sel[0] == sel[1]:
            ...
            if cpos <= cipos:
                ...
                if len(text) > column and text[column].isspace():
                    ...
                    while end < len(text) and text[end].isspace() and text[end] not in '\r\n':
                        end += 1
        ...
        if self.GetLexer() == wx.stc.STC_LEX_NULL:
            ...
            if interp != wx.EmptyString:
                ...
                if len(interp) and interp[(-1)][0] != '-':
                    ...
                    interp = ''
                ex_map = {'python': 'py', 'wish': 'tcl', 'ruby': 'rb', 'bash': 'sh', 
                   'csh': 'csh', 'perl': 'pl', 'ksh': 'ksh', 
                   'php': 'php', 'booi': 'boo', 'pike': 'pike'}
                self.ConfigureLexer(ex_map.get(interp, interp))
        self.Colourise(0, -1)

    def FireModified(self):
        """Fire a modified event"""
        self.OnChanged(wx.stc.StyledTextEvent(wx.stc.wxEVT_STC_CHANGE, self.GetId()))
    ...
    def InitCompleter(self):
        ...
        if completer is not None:
            ...
            extend = Profile_Get('AUTO_COMP_EX')
    ...
    def RegisterImages(self):
        ...
        images = [
         (
          autocomp.TYPE_FUNCTION, ed_glob.ID_FUNCT_TYPE),
         (
    ...
    def SetKeyWords(self, kw_lst):
        ...
        for keyw in kw_lst:
            if len(keyw) != 2:
                continue
            elif not isinstance(keyw[0], int) or not isinstance(keyw[1], basestring):
                continue
            else:
                kwlist += keyw[1]
    ...
    def SetLexer(self, lexer):
        ...
        elif self._code['clexer'] is not None:
    ...
    def SetProperties(self, prop_lst):
        ...
        for prop in prop_lst:
            if len(prop) != 2:
                continue
            elif not isinstance(prop[0], basestring) or not isinstance(prop[1], basestring):
                continue
            else:
