...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/extern/dexml/fields.py
...
class Value(Field):
    ...
    def render_children(self, obj, val, nsmap):
        if val is not None:
            if val is not self.default and self.tagname:
...
class Dict(Field):
    ...
    for dict items, for example:
      ...
      xml = '<MyModel><dict><MyObject name="obj1">'            <attr>val1</attr></MyObject></dict></MyModel>'
