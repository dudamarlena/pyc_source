...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/style_editor.py
...
import os, glob, wx, ed_glob
from profiler import Profile_Get, Profile_Set
import ed_basestc
from ed_style import StyleItem
import util, syntax.syntax, eclib, ebmlib, ed_basewin
_ = wx.GetTranslation
...
SETTINGS_IDS = [
 ID_FORE_COLOR, ID_BACK_COLOR, ID_BOLD, ID_ITALIC,
...
class StyleEditor(ed_basewin.EdBaseDialog):
    ...
    def __init__(self, parent, id_=wx.ID_ANY, title=_('Style Editor'), style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER):
        super(StyleEditor, self).__init__(parent, id_, title, style=style)
        self.LOG = wx.GetApp().GetLog()
        self._panel = StyleEditorBox(self)
    ...
    def OnOk(self, evt):
        ...
        if modtype == MOD_CHANGE_PRESENT:
            msg = _('Some styles have been changed would you like to save before exiting?')
        elif modtype == MOD_DOESNT_EXIST:
            msg = _("The new style sheet '%s' has not been saved would you like to save before exiting?") % self._panel.StyleTheme
        if msg is not None:
            dlg = wx.MessageDialog(self, msg, _('Save Styles'), style=wx.YES_NO | wx.YES_DEFAULT | wx.CANCEL | wx.ICON_INFORMATION)
            dlg.CenterOnParent()
...
class StyleEditorBox(ed_basewin.EdBaseCtrlBox):
    ...
    def __init__(self, parent):
        ...
        self._style_ch = wx.Choice(ctrlbar, ed_glob.ID_PREF_SYNTHEME, choices=sorted(ss_lst))
        bmp = wx.ArtProvider.GetBitmap(str(ed_glob.ID_ADD), wx.ART_MENU)
        if not bmp.IsOk():
            bmp = None
        self._addbtn = eclib.PlateButton(ctrlbar, label=_('New'), bmp=bmp, style=eclib.PB_STYLE_NOBG)
        bmp = wx.ArtProvider.GetBitmap(str(ed_glob.ID_REMOVE), wx.ART_MENU)
        if not bmp.IsOk():
            bmp = None
        self._delbtn = eclib.PlateButton(ctrlbar, label=_('Remove'), bmp=bmp, style=eclib.PB_STYLE_NOBG)
        ...
        self.Bind(wx.EVT_UPDATE_UI, lambda evt: evt.Enable(not self.IsSystemStyleSheet()), self._delbtn)
        return

    def __setStyleTheme(self, theme):
        self._prevTheme = theme
        self._style_ch.SetStringSelection(theme)

    StyleTheme = property(lambda self: self._style_ch.GetStringSelection(), lambda self, val: self.__setStyleTheme(val))
    SyntaxSheets = property(lambda self: self._style_ch.GetItems(), lambda self, val: self._style_ch.SetItems(sorted(val)))
    ...
    def SaveStyleSheet(self, sheetname=None):
        ...
        if self.WriteStyleSheet(sheet_path):
            ...
            if sheet_path.startswith(ed_glob.CONFIG['STYLES_DIR']) or sheet_path.startswith(ed_glob.CONFIG['SYS_STYLES_DIR']):
                UpdateBufferStyles(sheet)
            rval = True
        return rval
    ...
    def SheetExistOnDisk(self, sheet_name):
        ...
        """
        path = self.GetStyleSheetPath(sheet_name)
        syspath = self.GetStyleSheetPath(sheet_name, True)
    ...
    def OnButton(self, evt):
        ...
        if e_obj is self._addbtn:
            fname = wx.GetTextFromUser(_('Enter style sheet name'), _('New Style Sheet'), parent=self)
            if fname:
                if fname.lower() in [ name.lower() for name in self.SyntaxSheets ]:
                    wx.MessageBox(_('The style %s already exists. Please choose a different name.') % fname, style=wx.OK | wx.CENTER | wx.ICON_INFORMATION)
                else:
            ...
            try:
                ...
                wx.MessageBox(_('Failed to delete style sheet:\nError:\n%s') % msg, style=wx.OK | wx.CENTER | wx.ICON_ERROR)
            else:
                self.RefreshStyleSheets()
                self.StyleTheme = 'Default'
    ...
    def OnThemeChoice(self, evt):
        ...
        if modtype == MOD_CHANGE_PRESENT:
            ...
            msg = msg % oldTheme
        if msg is not None:
            dlg = wx.MessageDialog(self, msg, _('Save Styles'), style=wx.YES_NO | wx.YES_DEFAULT | wx.ICON_INFORMATION)
            dlg.CenterOnParent()
            result = dlg.ShowModal()
            dlg.Destroy()
            if result == wx.YES:
...
class StyleEditorPanel(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        self.prebuff = self.preview.GetPreviewBuffer()
    ...
    StyleTags = property(lambda self: self._settings.TagList.GetItems(), lambda self, val: self._settings.TagList.SetItems(sorted(val)))
    ...
    def GenerateStyleSheet(self):
        ...
        dvals = (';\n\t\t').join([ item.replace(',', ' ') for item in ditem.GetAsList()
                                 ]) + ';'
    ...
    def UpdateSettingsPane(self, syntax_data):
        ...
        val_map = {ID_FORE_COLOR: syntax_data.GetFore(), ID_BACK_COLOR: syntax_data.GetBack(), 
    ...
    def UpdateStyleSet(self, id_):
        ...
        if isinstance(ctrl, wx.CheckBox):
            ...
            return False
        val_map = {ID_FONT: 'face', ID_FONT_SIZE: 'size', 
           ID_BOLD: 'bold', 
           ID_EOL: 'eol', 
           ID_ITALIC: 'italic', 
           ID_ULINE: 'underline', 
...
class SettingsPanel(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        self._tag_list = wx.ListBox(self, size=(-1, 150), style=wx.LB_SINGLE)
        self.__DoLayout()
    ...
    def __DoLayout(self):
        ...
        hsizer.Add(wx.StaticLine(self, size=(-1, 2), style=wx.LI_VERTICAL), 0, wx.ALIGN_CENTER_HORIZONTAL | wx.EXPAND | wx.TOP | wx.BOTTOM, 5)
...
class PreviewPanel(ed_basewin.EdBaseCtrlBox):
    ...
    def __init__(self, parent):
        ...
        self.LOG = wx.GetApp().GetLog()
        self.preview = ed_basestc.EditraBaseStc(self, size=(-1, 200), style=wx.SUNKEN_BORDER)
    ...
    def __DoLayout(self):
        ...
        lexer_lst = wx.Choice(cbar, ed_glob.ID_LEXER, choices=syntax.GetLexerList())
