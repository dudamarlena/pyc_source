...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/autocomp/completer.py
...
__all__ = [
 'TYPE_FUNCTION', 'TYPE_METHOD', 'TYPE_CLASS', 'TYPE_ATTRIBUTE',
 'TYPE_VARIABLE', 'TYPE_ELEMENT', 'TYPE_PROPERTY', 'TYPE_UNKNOWN',
 'BaseCompleter', 'Symbol', 'CreateSymbols']
import wx
(TYPE_FUNCTION, TYPE_METHOD, TYPE_CLASS, TYPE_ATTRIBUTE, TYPE_PROPERTY, TYPE_VARIABLE, TYPE_ELEMENT, TYPE_UNKNOWN) = range(1, 9)
...
class Symbol(object):
    ...
    def __str__(self):
        if self.Type != TYPE_UNKNOWN:
            return ('?').join([self.Name, unicode(self.Type)])
        else:
            return self.Name
    ...
    Name = property(lambda self: self._name, lambda self, n: setattr(self, '_name', n))
    Type = property(lambda self: self._type, lambda self, t: setattr(self, '_type', t))
