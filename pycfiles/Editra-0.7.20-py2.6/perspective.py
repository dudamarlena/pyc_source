...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/perspective.py
...
import os, wx, wx.aui as aui, util, ed_menu, ed_fmgr
...
class PerspectiveManager(object):
    ...
    def __init__(self, base):
        ...
        self._mgr = ed_fmgr.EdFrameManager(flags=aui.AUI_MGR_DEFAULT | aui.AUI_MGR_TRANSPARENT_DRAG | hint | aui.AUI_MGR_ALLOW_ACTIVE_PANE)
        self._mgr.SetManagedWindow(self)
        self._ids = list()
        self._base = os.path.join(base, DATA_FILE)
        self._viewset = dict()
        self.LoadPerspectives()
        self._menu = ed_menu.EdMenu()
        self._currview = Profile_Get('DEFAULT_VIEW')
        self._menu.Append(ID_SAVE_PERSPECTIVE, _('Save Current View'), _('Save the current window layout'))
        self._menu.Append(ID_DELETE_PERSPECTIVE, _('Delete Saved View'))
        self._menu.AppendSeparator()
        self._menu.Append(ID_AUTO_PERSPECTIVE, _('Automatic'), _('Automatically save/use window state from last session'), wx.ITEM_CHECK)
        self._menu.AppendSeparator()
    ...
    def AddPerspectiveMenuEntry(self, name):
        ...
        self._menu.InsertAlpha(per_id, name, _('Change view to "%s"') % name, kind=wx.ITEM_CHECK, after=ID_AUTO_PERSPECTIVE)
        return True
    ...
    def LoadPerspectives(self):
        ...
        if reader == -1:
            util.Log('[perspective][err] Failed to get ' + 'file reader for %s' % self._base)
            return 0
        try:
    ...
    def OnPerspectiveMenu(self, evt):
        ...
        if e_id == ID_SAVE_PERSPECTIVE:
            ...
            if e_id == ID_DELETE_PERSPECTIVE:
                views = [ view for view in self._viewset.keys() if view != AUTO_PERSPECTIVE ]
                name = wx.GetSingleChoice(_('Perspective to Delete'), _('Delete Perspective'), views)
                if name:
