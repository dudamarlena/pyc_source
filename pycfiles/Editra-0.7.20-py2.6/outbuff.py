...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/eclib/outbuff.py
...
__all__ = [
 'OutputBuffer', 'OutputBufferEvent', 'ProcessBufferMixin',
...
import os, sys, time, errno, signal, threading, types, subprocess, wx, wx.stc
if subprocess.mswindows:
    import msvcrt, ctypes
else:
    import shlex, select, fcntl
...
OPB_STYLE_MAX = 3
OPB_ALL_STYLES = (
 wx.stc.STC_STYLE_DEFAULT, wx.stc.STC_STYLE_CONTROLCHAR,
...
class OutputBufferEvent(wx.PyCommandEvent):
    ...
    Value = property(lambda self: self.GetValue(), lambda self, v: setattr(self, '_value', v))
    ErrorMessage = property(lambda self: self.GetErrorMessage(), lambda self, msg: self.SetErrorMessage(msg))
...
class OutputBuffer(wx.stc.StyledTextCtrl):
    ...
    def __init__(self, parent, id=wx.ID_ANY, pos=wx.DefaultPosition, size=wx.DefaultSize, style=wx.BORDER_SUNKEN, name=OUTPUTBUFF_NAME_STR):
        super(OutputBuffer, self).__init__(parent, id, pos, size, style, name)
        self._mutex = threading.Lock()
        self._updating = threading.Condition(self._mutex)
        self._updates = list()
        self._timer = wx.Timer(self)
        self._line_buffer = -1
        self._colors = dict(defaultb=(255, 255, 255), defaultf=(0, 0, 0), errorb=(255,
                                                                                  255,
                                                                                  255), errorf=(255,
                                                                                                0,
                                                                                                0), infob=(255,
    ...
    def FlushBuffer(self):
        ...
        if '\x00' in txt:
            self.AddStyledText(('\x00').join(txt.encode('utf-8')) + '\x00')
        else:
            self.AppendText(txt)
    ...
    def __SetupStyles(self, font=None):
        ...
        if font is None:
            if wx.Platform == '__WXMAC__':
                ...
                fsize = 10
            font = wx.Font(fsize, wx.FONTFAMILY_MODERN, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL)
        style = (
         font.GetFaceName(), font.GetPointSize(), '#FFFFFF')
        wx.stc.StyledTextCtrl.SetFont(self, font)
        self.StyleSetSpec(OPB_STYLE_DEFAULT, 'face:%s,size:%d,fore:#000000,back:%s' % style)
        self.StyleSetSpec(OPB_STYLE_INFO, 'face:%s,size:%d,fore:#0000FF,back:%s' % style)
        self.StyleSetSpec(OPB_STYLE_WARN, 'face:%s,size:%d,fore:#FF0000,back:%s' % style)
        self.StyleSetSpec(OPB_STYLE_ERROR, 'face:%s,size:%d,fore:#FF0000,back:%s' % style)
        self.StyleSetHotSpot(OPB_STYLE_ERROR, True)
        self.StyleSetSpec(wx.stc.STC_STYLE_DEFAULT, 'face:%s,size:%d,fore:#000000,back:%s' % style)
        self.StyleSetSpec(wx.stc.STC_STYLE_CONTROLCHAR, 'face:%s,size:%d,fore:#000000,back:%s' % style)
        self.Colourise(0, -1)
        return
...
class ProcessThreadBase(threading.Thread):
    ...
    def __init__(self, parent):
        ...
        self._proc = None
        self._parent = parent
        self._sig_abort = signal.SIGTERM
        self._last_cmd = ''
        return

    LastCommand = property(lambda self: self._last_cmd, lambda self, val: setattr(self, '_last_cmd', val))
    ...
    def __DoOneRead(self):
        ...
        if subprocess.mswindows:
            ...
            try:
                ...
                avail = ctypes.c_long()
                ctypes.windll.kernel32.PeekNamedPipe(handle, None, 0, 0, ctypes.byref(avail), None)
                if avail.value > 0:
    ...
    def run(self):
        ...
        try:
            ...
            if self.Parent:
                err = OutputBufferEvent(edEVT_PROCESS_ERROR, self.Parent.GetId(), OPB_ERROR_INVALID_COMMAND)
                err.SetErrorMessage(msg)

        if self.Parent:
            evt = OutputBufferEvent(edEVT_PROCESS_START, self.Parent.GetId(), self.LastCommand)
            wx.PostEvent(self.Parent, evt)
...
class ProcessThread(ProcessThreadBase):
    ...
    def __init__(self, parent, command, fname='', args=list(), cwd=None, env=dict(), use_shell=True):
    ...
    def DoPopen(self):
        ...
        if use_shell and subprocess.mswindows:
            ...
            suinfo = None
        proc = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=use_shell, cwd=self._cwd, env=self._env, startupinfo=suinfo)
