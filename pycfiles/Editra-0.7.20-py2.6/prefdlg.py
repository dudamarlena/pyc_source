...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/prefdlg.py
...
import wx, wx.lib.mixins.listctrl as listmix, encodings, os, sys, ed_glob, profiler
from profiler import Profile_Get, Profile_Set
import ed_i18n, ed_event, ed_crypt, updater, util, syntax.syntax, ed_msg, ed_txt, eclib, ed_menu, extern.stcspellcheck as stcspellcheck
...
class PreferencesDialog(wx.Frame):
    ...
    def __init__(self, parent, id_=wx.ID_ANY, style=wx.DEFAULT_DIALOG_STYLE | wx.TAB_TRAVERSAL):
        ...
        """
        super(PreferencesDialog, self).__init__(parent, id_, _('Preferences - Editra'), style=style)
...
class PrefTools(eclib.SegmentBook):
    ...
    def __init__(self, parent):
        ...
        """
        super(PrefTools, self).__init__(parent, wx.ID_ANY, style=eclib.SEGBOOK_STYLE_LABELS | eclib.SEGBOOK_STYLE_NO_DIVIDERS)
        self._imglst = list()
        self.__InitImgList()
        self.AddPage(GeneralPanel(self), _('General'), img_id=self.GENERAL_PG)
        self.AddPage(AppearancePanel(self), _('Appearance'), img_id=self.APPEAR_PG)
        self.AddPage(DocumentPanel(self), _('Document'), img_id=self.DOC_PG)
        self.AddPage(NetworkPanel(self), _('Network'), img_id=self.UPDATE_PG)
        self.AddPage(AdvancedPanel(self), _('Advanced'), img_id=self.ADV_PG)
...
class GeneralPanel(wx.Panel, PreferencesPanelBase):
    ...
    def __init__(self, parent, style=wx.BORDER_SUNKEN):
        ...
        self.SetToolTipString(_('Changes made in this dialog are saved in your current profile. Some Items such as Language require the program to be restarted before taking effect.'))
        self._DoLayout()
        self._layout_done = True
...
class GeneralStartupPanel(wx.Panel):
    ...
    def _DoLayout(self):
        ...
        msizer.AddMany([
         (wx.StaticText(self, label=_('Editor Mode') + ': '),
          0, wx.ALIGN_CENTER_VERTICAL), ((5, 5), 0),
         (
          ExChoice(self, ed_glob.ID_PREF_MODE, choices=[
           'CODE', 'DEBUG', 'VERBOSE DEBUG'], default=Profile_Get('MODE')),
          0, wx.ALIGN_CENTER_VERTICAL)])
        psizer = wx.BoxSizer(wx.HORIZONTAL)
        psizer.AddMany([
         (wx.StaticText(self, label=_('Printer Mode') + ': '),
          0, wx.ALIGN_CENTER_VERTICAL), ((5, 5), 0),
         (
          ExChoice(self, ed_glob.ID_PRINT_MODE, choices=GetPrintModeStrings(), default=Profile_Get('PRINT_MODE')),
          0, wx.ALIGN_CENTER_VERTICAL)])
        reporter_cb = wx.CheckBox(self, ed_glob.ID_REPORTER, _('Disable Error Reporter'))
        ...
        if isadmin:
            chk_update = wx.CheckBox(self, ed_glob.ID_PREF_CHKUPDATE, _('Check for updates on startup'))
        ...
        lsizer.AddMany([
         (wx.StaticText(self, label=_('Language') + ': '),
          0, wx.ALIGN_CENTER_VERTICAL), ((5, 5), 0),
         (
          ed_i18n.LangListCombo(self, ed_glob.ID_PREF_LANG, Profile_Get('LANG')),
          0, wx.ALIGN_CENTER_VERTICAL)])
        sizer = wx.FlexGridSizer(11, 2, 5, 5)
        sizer.AddMany([((10, 10), 0), ((10, 10), 0),
         (
          wx.StaticText(self, label=_('Startup Settings') + ': '),
...
class GeneralFilePanel(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        self._DoLayout()
        self.Bind(wx.EVT_CHECKBOX, self.OnAutoBkup, id=ed_glob.ID_PREF_AUTOBKUP)
        self.Bind(wx.EVT_CHECKBOX, self.OnCustomBackupPath, id=ID_PREF_BKUP_LBL)
        self.Bind(wx.EVT_DIRPICKER_CHANGED, self.OnDirChange, id=ID_PREF_BKUP_PATH)
        self.Bind(wx.EVT_FILEPICKER_CHANGED, self.OnFileChange, id=ID_PREF_ENCHANT_PATH)
    ...
    def _DoLayout(self):
        ...
        fhsizer.AddMany([
         (
          wx.StaticText(self, label=_('File History Length') + ': '),
          0, wx.ALIGN_CENTER_VERTICAL), ((5, 5), 0),
         (
          ExChoice(self, ed_glob.ID_PREF_FHIST, choices=[ str(val) for val in range(1, 10) ], default=Profile_Get('FHIST_LVL', 'str')),
          0, wx.ALIGN_CENTER_VERTICAL)])
        d_encoding = Profile_Get('ENCODING', 'str', default=ed_txt.DEFAULT_ENCODING)
        if d_encoding is None:
            d_encoding = 'utf-8'
            Profile_Set('ENCODING', d_encoding)
        d_encoding = encodings.normalize_encoding(d_encoding)
        enc_ch = ExChoice(self, ed_glob.ID_PREF_ENCODING, choices=util.GetAllEncodings(), default=d_encoding)
        enc_ch.SetToolTipString(_('Encoding to try when auto detection fails'))
        enc_sz = wx.BoxSizer(wx.HORIZONTAL)
        enc_sz.AddMany([
         (wx.StaticText(self, label=_('Preferred Encoding') + ':'),
          0, wx.ALIGN_CENTER_VERTICAL), ((5, 5), ), (enc_ch, 1)])
        win_cb = wx.CheckBox(self, ed_glob.ID_NEW_WINDOW, _('Open files in new windows by default'))
        win_cb.SetValue(Profile_Get('OPEN_NW'))
        pos_cb = wx.CheckBox(self, ed_glob.ID_PREF_SPOS, _('Remember File Position'))
        pos_cb.SetValue(Profile_Get('SAVE_POS'))
        chkmod_cb = wx.CheckBox(self, ed_glob.ID_PREF_CHKMOD, _('Check if on disk file has been modified by others'))
        chkmod_val = Profile_Get('CHECKMOD', default=True)
        chkmod_cb.SetValue(chkmod_val)
        autorl_cb = wx.CheckBox(self, ed_glob.ID_PREF_AUTO_RELOAD, _('Automatically reload files when changes are detected on disk'))
        autorl_cb.SetValue(Profile_Get('AUTO_RELOAD', default=False) and chkmod_val)
        autorl_cb.Enable(chkmod_val)
        eolwarn_cb = wx.CheckBox(self, ed_glob.ID_PREF_WARN_EOL, _('Warn when mixed eol characters are detected'))
        ...
        autobk_cb = wx.CheckBox(self, ed_glob.ID_PREF_AUTOBKUP, _('Automatically Backup Files'))
        ...
        bkup_path_lbl = wx.CheckBox(self, ID_PREF_BKUP_LBL, label=_('Backup Path:'))
        bkup_path_lbl.SetValue(bool(bdir))
        bkup_path = wx.DirPickerCtrl(self, ID_PREF_BKUP_PATH, path=bdir, style=wx.DIRP_CHANGE_DIR | wx.DIRP_USE_TEXTCTRL)
        bkup_path.SetToolTipString(_('Used to set a custom backup path. If not specified the backup will be put in the same directory as the file.'))
        ...
        sprefs = Profile_Get('SPELLCHECK', default=dict())
        auto_cb = wx.CheckBox(self, id=ID_PREF_AUTO_SPELL, label=_('Check spelling while typing'))
        ...
        if not prefpath:
            prefpath = ''
        libpicker = wx.FilePickerCtrl(self, ID_PREF_ENCHANT_PATH, path=prefpath, message=_('Path to libenchant'))
        ...
        if path != cpath:
            try:
                ...
                if ok:
                    ...
                    wx.MessageBox(_('Failed to load Enchant'), _('Library Error'), wx.OK | wx.ICON_ERROR)
...
class DocGenPanel(wx.Panel):
    ...
    def _DoLayout(self):
        ...
        tabsz.AddMany([
         (wx.StaticText(self, label=_('Tab Width') + ': '),
          0, wx.ALIGN_CENTER_VERTICAL), ((5, 5), 0),
         (
          ExChoice(self, ed_glob.ID_PREF_TABW, choices=[
           '2', '3', '4', '5', '6', '7', '8', '9', '10'], default=Profile_Get('TABWIDTH', 'str')), 0,
          wx.ALIGN_CENTER_VERTICAL)])
        indentsz = wx.BoxSizer(wx.HORIZONTAL)
        indentsz.AddMany([
         (wx.StaticText(self, label=_('Indent Width') + ': '),
          0, wx.ALIGN_CENTER_VERTICAL), ((5, 5), 0),
         (
          ExChoice(self, ed_glob.ID_PREF_INDENTW, choices=[
           '2', '3', '4', '5', '6', '7', '8', '9', '10'], default=Profile_Get('INDENTWIDTH', 'str')), 0,
          wx.ALIGN_CENTER_VERTICAL)])
        at_cb = wx.CheckBox(self, ed_glob.ID_PREF_AUTOTRIM, _('Automatically trim whitespace on save'))
        at_cb.SetValue(Profile_Get('AUTO_TRIM_WS', 'bool', False))
        ut_cb = wx.CheckBox(self, ed_glob.ID_PREF_TABS, _('Use Tabs Instead of Spaces'))
        ut_cb.SetValue(Profile_Get('USETABS', 'bool', False))
        bsu_cb = wx.CheckBox(self, ed_glob.ID_PREF_UNINDENT, _('Backspace Unindents'))
        bsu_cb.SetValue(Profile_Get('BSUNINDENT', 'bool', True))
        eolsz = wx.BoxSizer(wx.HORIZONTAL)
        eolmode = ExChoice(self, ed_glob.ID_EOL_MODE, choices=[
         _('Old Macintosh (\\r)'), _('Unix (\\n)'),
         _('Windows (\\r\\n)')])
        eolmode.SetSelection(Profile_Get('EOL_MODE'))
        eolsz.AddMany([
         (
          wx.StaticText(self, label=_('Default EOL Mode') + ': '),
          0, wx.ALIGN_CENTER_VERTICAL), ((5, 5), 0),
         (
        ...
        vs_cb = wx.CheckBox(self, ed_glob.ID_PREF_VIRT_SPACE, _('View Virtual Space After Last Line'))
        ...
        curw_ch = ExChoice(self, ed_glob.ID_PREF_CARET_WIDTH, choices=[
         '1', '2', '3', '4'], default=Profile_Get('CARETWIDTH', default=1) - 1)
        cursw_sz.AddMany([(curlbl, 0, wx.ALIGN_CENTER_VERTICAL),
         (
          curw_ch, 0, wx.LEFT, 5)])
        fnt = Profile_Get('FONT1', 'font', wx.Font(10, wx.FONTFAMILY_MODERN, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL))
        ...
        fnt = Profile_Get('FONT2', 'font', wx.Font(10, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL))
        fpick2 = eclib.PyFontPicker(self, DocGenPanel.ID_FONT_PICKER2, fnt)
        fpick2.SetToolTipString(_('Sets a secondary font used for special regions when syntax highlighting is in use'))
...
class DocCodePanel(wx.Panel):
    ...
    def _DoLayout(self):
        ...
        """
        dlex_ch = ExChoice(self, ed_glob.ID_PREF_DLEXER, choices=syntax.GetLexerList(), default=Profile_Get('DEFAULT_LEX', default='Plain Text'))
        dlex_ch.SetToolTipString(_('Default highlighing for new documents'))
        dlex_sz = wx.BoxSizer(wx.HORIZONTAL)
        dlex_sz.AddMany([
         (wx.StaticText(self, label=_('Default Lexer') + ': '),
          0, wx.ALIGN_CENTER_VERTICAL), ((3, 3), ),
         (
          dlex_ch, 0, wx.ALIGN_CENTER_VERTICAL)])
        vis_lbl = wx.StaticText(self, label=_('Visual Helpers') + ': ')
        br_cb = wx.CheckBox(self, ed_glob.ID_BRACKETHL, _('Bracket Highlighting'))
        ...
        edge_sp = wx.SpinCtrl(self, ed_glob.ID_PREF_EDGE, Profile_Get('EDGE', 'str'), min=0, max=160)
        ...
        hlcaret_cb = wx.CheckBox(self, ed_glob.ID_HLCARET_LINE, _('Highlight Caret Line'))
        hlcaret_cb.SetValue(Profile_Get('HLCARETLINE'))
        ind_cb = wx.CheckBox(self, ed_glob.ID_INDENT_GUIDES, _('Indentation Guides'))
        ...
        compex_cb = wx.CheckBox(self, ed_glob.ID_PREF_AUTOCOMPEX, _('Extended Auto-Comp'))
        compex_cb.SetValue(Profile_Get('AUTO_COMP_EX'))
        compex_cb.Enable(comp_cb.GetValue())
        compex_cb.SetToolTipString(_('Warning suggestions will include context insensitive results'))
        ...
        vi_ncb = wx.CheckBox(self, ed_glob.ID_VI_NORMAL_DEFAULT, _('Start in Normal Mode'))
...
class DocSyntaxPanel(wx.Panel):
    ...
    def _DoLayout(self):
        ...
        syntheme = ExChoice(self, ed_glob.ID_PREF_SYNTHEME, choices=sorted(ss_lst), default=Profile_Get('SYNTHEME', 'str'))
...
class AppearancePanel(wx.Panel, PreferencesPanelBase):
    ...
    def _DoLayout(self):
        ...
        iconsz.AddMany([
         (wx.StaticText(self, label=_('Icon Theme') + ': '), 0,
          wx.ALIGN_CENTER_VERTICAL), ((5, 5), 0),
         (
          ExChoice(self, ed_glob.ID_PREF_ICON, icons, Profile_Get('ICONS', 'str').title()), 0)])
        tbiconsz = wx.BoxSizer(wx.HORIZONTAL)
        tbiconsz.AddMany([
         (wx.StaticText(self, label=_('Toolbar Icon Size') + ': '),
          0, wx.ALIGN_CENTER_VERTICAL), ((5, 5), 0),
         (
          ExChoice(self, ed_glob.ID_PREF_ICONSZ, [
           '16', '24', '32'], str(Profile_Get('ICON_SZ', 'size_tuple')[0])), 0)])
        tabicon_cb = wx.CheckBox(self, ed_glob.ID_PREF_TABICON, _('Show Icons on Tabs'))
        ...
        perspec_sz.AddMany([
         (
          wx.StaticText(self, label=_('Default Perspective') + ': '),
          0, wx.ALIGN_CENTER_VERTICAL), ((5, 5), 0),
         (
          ExChoice(self, ed_glob.ID_PERSPECTIVES, pchoices, Profile_Get('DEFAULT_VIEW')), 0)])
        ws_cb = wx.CheckBox(self, ed_glob.ID_PREF_WSIZE, _('Remember Window Size on Exit'))
        ws_cb.SetValue(Profile_Get('SET_WSIZE'))
        wp_cb = wx.CheckBox(self, ed_glob.ID_PREF_WPOS, _('Remember Window Position on Exit'))
        ...
        sizer.AddMany([((10, 10), 0), ((10, 10), 0),
         ...
         (
          wx.Slider(self, ed_glob.ID_TRANSPARENCY, Profile_Get('ALPHA'), 100, 255, style=wx.SL_HORIZONTAL | wx.SL_AUTOTICKS | wx.SL_LABELS), 0, wx.EXPAND),
         ((10, 10), 0), ((10, 10), 0),
         (
...
class NetConfigPage(wx.Panel):
    ...
    def _DoLayout(self):
        ...
        msizer = wx.BoxSizer(wx.VERTICAL)
        sboxsz = wx.StaticBoxSizer(wx.StaticBox(self, label=_('Proxy Settings')), wx.VERTICAL)
        ...
        pass_txt = wx.TextCtrl(self, ID_PASSWORD, ed_crypt.Decrypt(proxy_val.get('passwd', ''), proxy_val.get('pid', '')), style=wx.TE_PASSWORD)
        ...
        if wx.Platform == '__WXMAC__':
            for lbl in (use_proxy, url_txt, port_sep, port_txt, url_lbl, usr_lbl, usr_txt, pass_lbl, pass_txt, apply_b):
    ...
    def OnApply(self, evt):
        ...
        if evt.GetId() == wx.ID_APPLY:
            key_map = {ID_USERNAME: 'uname', ID_URL: 'url', ID_PORT: 'port', ID_PASSWORD: 'passwd'}
    ...
    def EnableControls(self, enable=True):
        ...
        for child in self.GetChildren():
            if isinstance(child, wx.StaticText) or isinstance(child, wx.TextCtrl) or isinstance(child, wx.Button):
                child.Enable(enable)
...
class UpdatePage(wx.Panel):
    ...
    def _DoLayout(self):
        ...
        upd_bsz.Add(wx.StaticText(self, ID_UPDATE_MSG, _('Status Unknown')), 0, wx.ALIGN_CENTER_HORIZONTAL)
        upd_bsz.Layout()
        ...
        bsizer.Add(wx.Button(self, ID_CHECK_UPDATE, _('Check')), 0, wx.ALIGN_LEFT)
    ...
    def OnButton(self, evt):
        ...
        if e_id == ID_CHECK_UPDATE:
            ...
            dl_dlg = updater.DownloadDialog(None, ed_glob.ID_DOWNLOAD_DLG, _('Downloading Update'))
...
class KeyBindingPanel(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        self.Bind(wx.EVT_UPDATE_UI, lambda evt: evt.Enable(self.Dirty), id=wx.ID_APPLY)
    ...
    def _DoLayout(self):
        ...
        milbls = [ item[1] for item in self.menumap.get(menus.GetStringSelection())
                 ]
        lvsizer.AddMany([
         (wx.StaticText(self, label=_('Menu') + ':'),
          0, wx.ALIGN_LEFT), spacer,
         (
          menus, 0, wx.ALIGN_LEFT | wx.EXPAND), spacer,
         (
          wx.ListBox(self, ID_MENU_ITEMS, choices=sorted(milbls), style=wx.SIMPLE_BORDER), 1, wx.EXPAND),
         spacer])
        rvsizer = wx.BoxSizer(wx.VERTICAL)
        rvsizer.AddMany([
         (wx.StaticText(self, label=_('Modifier 1') + ':'),
          0, wx.ALIGN_LEFT), spacer,
         (
        ...
        line_sz2.AddMany([(5, 1),
         ...
         (5, 1)])
        bsizer = wx.BoxSizer(wx.HORIZONTAL)
        bsizer.AddMany([spacer,
         (
          wx.Button(self, wx.ID_REVERT, _('Revert to Default')), 0, wx.ALIGN_LEFT),
         (
          (-1, 5), 1, wx.EXPAND),
         (
    ...
    def _UpdateBinding(self):
        ...
        binding = self.FindWindowById(ID_BINDING)
    ...
    def _UpdateKeyDisplay(self, keys):
        ...
        """
        mod1 = self.FindWindowById(ID_MOD1)
        mod1.SetItems(MODIFIERS)
        mod2 = self.FindWindowById(ID_MOD2)
        mod2.Enable()
        key = self.FindWindowById(ID_KEYS)
        if keys is None:
            keys = ''
    ...
    Dirty = property(lambda self: self._dirty, lambda self, bDirty: setattr(self, '_dirty', bDirty))
    ...
    def OnButton(self, evt):
        ...
        if e_id == wx.ID_NEW:
            dlg = wx.TextEntryDialog(self, _('New Profile'), _('Enter the name of the new key profile'))
            dlg.ShowModal()
            val = dlg.GetValue()
            if len(val):
            ...
            ed_msg.PostMessage(ed_msg.EDMSG_MENU_LOADPROFILE, Profile_Get('KEY_PROFILE', default=None))
...
class ExtListCtrl(eclib.EEditListCtrl):
    ...
    def __init__(self, parent):
        ...
        self.InsertColumn(ExtListCtrl.EXT_COL, _('Extensions (space separated, no dots)'))
    ...
    def LoadList(self):
        ...
        for key in sorted(self._extreg.keys()):
            ...
            self.SetStringItem(index, ExtListCtrl.EXT_COL, '  %s' % (' ').join(self._extreg[key]))
    ...
    def UpdateExtensions(self):
        ...
        for row in range(self.GetItemCount()):
            ...
            self.SetStringItem(row, ExtListCtrl.EXT_COL, '  ' + (' ').join(self._extreg[ftype]))
