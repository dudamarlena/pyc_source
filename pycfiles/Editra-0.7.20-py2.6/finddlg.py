...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/eclib/finddlg.py
...
__all__ = [
 'FindBox', 'FindEvent', 'FindPanel', 'FindReplaceDlg',
...
AFR_NO_COUNT = 4096
AFR_NO_ALL_BTN = 8192
AFR_SIMPLE = AFR_NOLOOKIN | AFR_NOOPTIONS | AFR_NOUPDOWN | AFR_NO_COUNT | AFR_NO_ALL_BTN
...
_EVENT_MAP = {wx.ID_FIND: edEVT_FIND, wx.ID_REPLACE: edEVT_REPLACE, 
...
class MiniFindReplaceDlg(wx.MiniFrame, FindReplaceDlgBase):
    ...
    def __init__(self, parent, fdata, title, style=AFR_STYLE_FINDDIALOG):
        ...
        """
        wx.MiniFrame.__init__(self, parent, wx.ID_ANY, '', style=wx.DEFAULT_DIALOG_STYLE)
...
class FindBox(ctrlbox.ControlBox):
    ...
    def __init__(self, parent, fdata, id=wx.ID_ANY, pos=wx.DefaultPosition, size=wx.DefaultSize, style=AFR_STYLE_FINDDIALOG, name=FindBoxName):
        """Create the container box
        @param fdata: wx.FindReplaceData

        """
        super(FindBox, self).__init__(parent, id, pos, size, wx.TAB_TRAVERSAL | wx.NO_BORDER, name)
        ...
        self.find = platebtn.PlateButton(ctrlbar, label=_('Find'), bmp=bmp, style=platebtn.PB_STYLE_NOBG)
        bmp = wx.ArtProvider.GetBitmap(wx.ART_FIND_AND_REPLACE, wx.ART_MENU)
        self.replace = platebtn.PlateButton(ctrlbar, label=_('Replace'), bmp=bmp, style=platebtn.PB_STYLE_NOBG)
...
class FindPanel(wx.Panel):
    ...
    def __init__(self, parent, fdata, id=wx.ID_ANY, pos=wx.DefaultPosition, size=wx.DefaultSize, style=AFR_STYLE_FINDDIALOG, name=FindPanelName):
        """Create the panel
        @param fdata: wx.FindReplaceData

        """
        super(FindPanel, self).__init__(parent, id, pos, size, wx.TAB_TRAVERSAL | wx.NO_BORDER, name)
        self._mode = style
        ...
        self.Bind(wx.EVT_BUTTON, lambda evt: self.FireEvent(evt.GetId()) or evt.Skip())
    ...
    def _ShowButtons(self, find=True):
        ...
        if find:
            show = [
             wx.ID_FIND, ID_COUNT, ID_FIND_ALL, 'fspacer']
            hide = [wx.ID_REPLACE, ID_REPLACE_ALL, 'frspacer']
        else:
            show = [
             wx.ID_REPLACE, ID_REPLACE_ALL, 'frspacer']
    ...
    def OnOption(self, evt):
        ...
        """
        fmap = {ID_MATCH_CASE: AFR_MATCHCASE, ID_WHOLE_WORD: AFR_WHOLEWORD, 
           ID_REGEX: AFR_REGEX, 
    ...
    def OnUpdateUI(self, evt):
        ...
        """
        if evt.Id in (wx.ID_FIND, ID_COUNT, wx.ID_REPLACE) and self._lookin.GetSelection() > LOCATION_CURRENT_DOC:
            evt.Enable(False)
        else:
