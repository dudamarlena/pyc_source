...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/eclib/infodlg.py
...
__all__ = [
 'FileInfoDlg', 'CalcSize', 'GetFileType']
import os, time, stat, mimetypes, wx
_ = wx.GetTranslation
PERM_MAP = {'0': '---', '1': '--x', '2': '-w-', '3': '-wx', '4': 'r--', 
   '5': 'r-x', '6': 'rw-', '7': 'rwx'}
...
class FileInfoDlg(wx.MiniFrame):
    ...
    def __init__(self, parent, fname='', ftype=None, bmp=wx.NullBitmap):
        ...
        super(FileInfoDlg, self).__init__(parent, title='%s  %s' % (self._fname, _('Info')), style=wx.DEFAULT_DIALOG_STYLE)
        self._file = fname
        self._ftype = ftype
        ...
        try:
            ...
            self._fstat = dict(mtime=time.asctime(time.localtime(fstat[stat.ST_MTIME])), ctime=time.asctime(time.localtime(fstat[stat.ST_CTIME])), size=CalcSize(fstat[stat.ST_SIZE]), perm=permstr)
