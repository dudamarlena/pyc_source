...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/syntax/_asm68k.py
...
ASM_CPU_INST = (0, 'andi and as b beq bg b bl bne bge bpl bchg bclr bfchg bfclr bfexts bfextu bfffo bfins bfset bftst bkpt bra bset bsr btst callm cas2 cas chk2 chk clr cmpa cmpi cmpm cmp dbcc dbcs dbvc dbvs dbeq dbf dbt dbge dbgt dbhi dbmi dble dbls dblt dbne dbpl dbra eori eor exg extb ext illegaljmp jsr lea link ls mm movea movec movem movep moveq moves move nbcd negx neg nop not ori or pack pea reset ro rox rt sbcd seq sne spl swap tas trap tst unlk unpk abcd')
ASM_MATH_INST = (1, 'adda addi addq addx add div mul suba subi subq subx sub tdiv')
ASM_REGISTER = (2, 'a0 a1 a2 a3 a4 a5 a6 a7 d0 d1 d2 d3 d4 d5 d6 d7 pc sr ccr sp usp ssp vbr sfc sfcr dfc dfcr msp isp zpc cacr caar za0 za1 za2 za3 za4 za5 za6 za7 zd0 zd1 zd2 zd3 zd4 zd5 zd6 zd7 crp srp tc ac0 ac1 acusr tt0 tt1 mmusr dtt0 dtt1 itt0 itt1 urp cal val scc crp srp drp tc ac psr pcsr bac0 bac1 bac2 bac3 bac4 bac5 bac6 bac7 bad0 bad1 bad2 bad3 bad4 bad5 bad6 bad7 fp0 fp1 fp2 fp3 fp4 fp5 fp6 fp7 control status iaddr fpcr fpsr fpiar ')
ASM_DIRECTIVES = (3, 'ALIGN CHIP COMLINE COMMON DC DCB DS END EQU FEQU FAIL FOPT IDNT LLEN MASK2 NAME NOOBJ OFFSET OPT ORG PLEN REG RESTORE SAVE SECT SECTION SET SPC TTL XCOM XDEF XREF')
ASM56K_CPU_INST = (0, 'adc addl addr and andi asl asr bchg bclr bra brclr brset bsclr bset bsr bsset btst bcc bcs bec beq bes bge bgt blc ble bls blt bmi bne bnr bpl bnn brkcc brkcs brkec brkeq brkes brkge brkgt brklc brkle brkls brklt brkmi brkne brknr brkpl brknn bscc bscs bsec bseq bses bsge bsgt bslc bsle bsls bslt bsmi bsne bsnr bspl bsnn clb clr cmp cmpm cmpu debug dmac do forever dor enddo eor extract extractu illegal inc insert jclr jmp jsclr jset jsr jsset jcc jcs jec jeq jes jge jgt jlc jle jls jlt jmi jne jnr jpl jnn jscc jscs jsec jseq jses jsge jsgt jslc jsle jsls jslt jsmi jsne jsnr jspl jsnn lra lsl lsr lua mac maci macr macri max maxm merge move movem movec movep mpy mpyi mpyr mpyri nop norm normf not or ori pflush pflushun pfree plock plockr punlock punlockr rep reset rnd rol ror rti rts sbc stop tcc tfr trap trapcc trapcs trapec trapeq trapes trapge trapgt traplc traple trapls traplt trapmi trapne trapnr trappl trapnn tst vsl wait')
ASM56K_MATH_INST = (1, 'abs add dec div neg sub subl subr')
ASM56K_REGISTERS = (2, 'pc mr ccr sr eom com omr sz sc vba la lc sp ssh ssl ss a a2 a1 a0 b b2 b1 b0 x x0 x1 y y0 y1 r0 r1 r2 r3 r4 r5 r6 r7 m0 m1 m2 m3 m4 m5 m6 m7 n0 n1 n2 n3 n4 n5 n6 n7 ')
ASM56K_DIRECTIVES = (3, 'org equ page tabs list nolist if endif else opt title macro endm dup dupa dupc dupf baddr bsb bsc bsm dc dcb ds dsm dsr buffer endbuf section endsec global local xdef xref mode ')
SYNTAX_ITEMS = [
 (
...
class SyntaxData(syndata.SyntaxDataBase):
    ...
    def GetKeywords(self):
        ...
        if self.LangId == synglob.ID_LANG_68K:
            ...
            return [
             ASM56K_CPU_INST, ASM56K_MATH_INST,
