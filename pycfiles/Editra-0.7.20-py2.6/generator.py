...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/generator.py
...
import wx, wx.stc, time, ed_glob, ed_menu
from ed_style import StyleItem
import util, plugin, ebmlib, eclib
_ = wx.GetTranslation
...
class Generator(plugin.Plugin):
    ...
    def InstallMenu(self, menu):
        ...
        menu.AppendMenu(ed_glob.ID_GENERATOR, _('Generator'), genmenu, _('Generate Code and Documents'))
...
class Html(plugin.Plugin):
    ...
    def GenerateHead(self):
        ...
        return '<head>\n<title>%s</title>\n<meta name="Generator" content="Editra/%s">\n<meta http-equiv="content-type" content="text/html; charset=utf-8">\n</head>' % (
         ebmlib.GetFileName(self.stc.GetFileName()),
         ed_glob.VERSION)
    ...
    def GetMenuEntry(self, menu):
        ...
        """
        return wx.MenuItem(menu, self._id, _('Generate %s') % 'HTML', _('Generate a %s version of the current document') % 'HTML')
...
class LaTeX(plugin.Plugin):
    ...
    def GenDoc(self):
        ...
        tmp = self.TransformText(self._stc.GetTextRange(parse_pos, parse_pos + 1))
        ...
        for parse_pos in xrange(last_pos + 1):
            ...
            if curr_id != last_id or tmp[(-1)] == '\n':
                ...
                if tag == 'operator_style' or tag == 'default_style' and tmp_tex.isspace() and len(tmp_tex) <= 2:
                    tex.append(tmp_tex)
                else:
                    if '\\\\\n' in tmp_tex:
    ...
    def GetMenuEntry(self, menu):
        ...
        """
        return wx.MenuItem(menu, self._id, _('Generate %s') % 'LaTeX', _('Generate an %s version of the current document') % 'LaTeX')
    ...
    def RegisterStyleCmd(self, cmd_name, s_item):
        ...
        if 'bold' in unicode(s_item):
            fore = bold_tmp % fore
        if 'underline' in unicode(s_item):
            fore = uline_tmp % fore
        if 'italic' in unicode(s_item):
            fore = ital_tmp % fore
...
class Rtf(plugin.Plugin):
    ...
    def GetMenuEntry(self, menu):
        ...
        """
        return wx.MenuItem(menu, self._id, _('Generate %s') % 'RTF', _('Generate a %s version of the current document') % 'RTF')
