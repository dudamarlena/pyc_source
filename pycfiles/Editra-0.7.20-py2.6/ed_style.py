...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/ed_style.py
...
import os, re, wx, ed_glob, util
from profiler import Profile_Get, Profile_Set
import eclib, ebmlib
STY_ATTRIBUTES = (
 'face', 'fore', 'back', 'size', 'modifiers')
STY_EX_ATTRIBUTES = ('eol', 'bold', 'italic', 'underline')
...
class StyleItem(object):
    ...
    def __init__(self, fore='', back='', face='', size='', ex=None):
    ...
    def __unicode__(self):
        ...
        style_str = list()
        if self.fore:
            style_str.append('fore:%s' % self.fore)
        if self.back:
            style_str.append('back:%s' % self.back)
        if self.face:
            style_str.append('face:%s' % self.face)
        if self.size:
            style_str.append('size:%s' % unicode(self.size))
        if len(self._exattr):
            style_str.append('modifiers:' + (',').join(self._exattr))
        style_str = (',').join(style_str)
        return style_str.rstrip(',')

    def Clone(self):
        """Make and return a copy of this object"""
        nitem = StyleItem(self.fore, self.back, self.face, self.size, self._exattr)
        if self.null:
            nitem.Nullify()
        return nitem
    ...
    def GetAsList(self):
        ...
        if len(self._exattr):
            retval.append('modifiers:' + (',').join(self._exattr))
        return retval
...
class StyleMgr(object):
    ...
    def BlankStyleDictionary(self):
        ...
        for key in DEF_STYLE_DICT.keys():
            if key in ('select_style', ):
                ...
                sty_dict[key] = StyleItem('#000000', '#FFFFFF', '%(primary)s', '%(size)d')
    ...
    def GetFontDictionary(self, default=True):
        ...
        if hasattr(self, 'fonts') and not default:
            ...
            if font is not None:
                ...
                mfont = wx.Font(10, wx.FONTFAMILY_MODERN, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL)
            ...
            faces = {self.FONT_PRIMARY: primary, 
    ...
    def GetStyleFont(self, primary=True):
        ...
        """
        if primary:
            font = wx.FFont(self.fonts[self.FONT_SIZE], wx.DEFAULT, face=self.fonts[self.FONT_PRIMARY])
        else:
            font = wx.FFont(self.fonts[self.FONT_SIZE2], wx.DEFAULT, face=self.fonts[self.FONT_SECONDARY])
        return font
    ...
    def GetStyleSheet(sheet_name=None):
        ...
        style = ebmlib.AddFileExtension(style, '.ess').lower()
        for sheet in util.GetResourceFiles('styles', trim=False, get_all=True, title=False):
    ...
    def LoadStyleSheet(self, style_sheet, force=False):
        ...
        """
        if isinstance(style_sheet, basestring) and os.path.exists(style_sheet) and (force or style_sheet not in StyleMgr.STYLES or style_sheet != self.style_set):
            ...
            if style_sheet not in StyleMgr.STYLES:
    ...
    def ParseStyleData(self, style_data):
        ...
        """
        style_data = RE_ESS_COMMENT.sub('', style_data)
        style_data = style_data.replace('\r\n', '').replace('\n', '')
        style_data = style_data.replace('\t', '')
        style_tree = [ style.split('{') for style in style_data.split('}') ]
        if len(style_tree) and len(style_tree[(-1)]) and not style_tree[(-1)][0]:
            style_tree.pop()
        ttree = list(style_tree)
        for branch in ttree:
            if len(branch) != 2:
                self.LOG('[ed_style][err] There was an error parsing the syntax data from ' + self.style_set)
                self.LOG('[ed_style][err] Missing a { or } in Def: ' + repr(branch[0]))
                ttree.remove(branch)
                continue
            tmp2 = [ leaf.strip().split(':') for leaf in branch[1].strip().split(';')
                   ]
        ...
        for branch in style_tree:
            value = list()
            tag = branch[0].replace(' ', '')
            for leaf in branch[1]:
                leaf = [ part.strip() for part in leaf ]
                if len(leaf) != 2:
                    self.LOG('[ed_style][err] Missing a : or ; in the declaration of %s' % tag)
                elif leaf[0] not in STY_ATTRIBUTES:
                    self.LOG('[ed_style][warn] Unknown style attribute: %s, In declaration of %s' % (
                     leaf[0], tag))
                else:
                    value.append(leaf)
        ...
        for style_def in style_dict:
            if not style_def[0][0].isalpha():
                self.LOG('[ed_style][err] The style def %s is not a valid name' % style_def[0])
            else:
                style_str = ''
                for attrib in style_dict[style_def]:
                    values = [ val for val in attrib[1].split() if val != ''
                             ]
                    v1ok = v2ok = False
                    n_values = len(values)
                    if n_values and attrib[0] in 'fore back' and RE_HEX_STR.match(values[0]):
                        v1ok = True
                    elif n_values:
                        if attrib[0] == 'size' and (RE_ESS_SCALAR.match(values[0]) or values[0].isdigit()):
                            v1ok = True
                        else:
                            self.LOG('[ed_style][warn] Bad value in %s the value %s is invalid.' % (
                             attrib[0], values[0]))
                    ...
                    if len(values) > 1:
                        for value in values[1:]:
                            if value not in STY_EX_ATTRIBUTES:
                                self.LOG('[ed_style][warn] Unknown extra ' + "attribute '" + values[1] + "' in attribute: " + attrib[0])
                                break
                            else:
                                v2ok = True
    ...
    def SetStyles(self, name, style_dict, nomerge=False):
        ...
        if nomerge:
            ...
            if isinstance(style_dict, dict):
                ...
                self.LOG('[ed_style][err] SetStyles expects a dictionary of StyleItems')
                return False
            return
    ...
    def UpdateBaseStyles(self):
        ...
        self.StyleSetSpec(wx.stc.STC_STYLE_INDENTGUIDE, self.GetStyleByName('guide_style'))
...
DEF_STYLE_DICT = {'brace_good': StyleItem('#FFFFFF', '#0000FF', ex=['bold']), 'brace_bad': StyleItem(back='#FF0000', ex=['bold']), 
   'calltip': StyleItem('#404040', '#FFFFB8'), 
   'caret_line': StyleItem(back='#D8F8FF'), 
   'ctrl_char': StyleItem(), 
   'line_num': StyleItem(back='#C0C0C0', face='%(secondary)s', size='%(size3)d'), 
   'array_style': StyleItem('#EE8B02', face='%(secondary)s', ex=[
                 'bold']), 
   'btick_style': StyleItem('#8959F6', size='%(size)d', ex=['bold']), 
   'default_style': StyleItem('#000000', '#F6F6F6', '%(primary)s', '%(size)d'), 
   ...
   'decor_style': StyleItem('#BA0EEA', face='%(secondary)s', ex=['italic']), 
   'directive_style': StyleItem('#0000FF', face='%(secondary)s', ex=[
                     'bold']), 
   'dockey_style': StyleItem('#0000FF'), 
   'edge_style': StyleItem(), 
   'error_style': StyleItem('#DD0101', face='%(secondary)s', ex=[
                 'bold']), 
   'foldmargin_style': StyleItem(back='#D1D1D1'), 
   'funct_style': StyleItem('#008B8B', ex=['italic']), 
   'global_style': StyleItem('#007F7F', face='%(secondary)s', ex=[
                  'bold']), 
   'guide_style': StyleItem('#838383'), 
   'here_style': StyleItem('#CA61CA', face='%(secondary)s', ex=[
                'bold']), 
