...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/ebmlib/osutil.py
...
__all__ = [
 ...
 'RemovableDrive']
import wx, ctypes, signal, collections
HASWIN32 = False
...
def GetWindowsDriveType(dname):
    ...
    try:
        dtypes = [
         None, None, RemovableDrive, FixedDrive, RemoteDrive, CDROMDrive, RamDiskDrive]
...
def InstallTermHandler(callback, *args, **kwargs):
    ...
    if wx.Platform == '__WXMSW__':
        if HASWIN32:
            win32api.SetConsoleCtrlHandler(lambda dummy: callback(*args, **kwargs), True)
        else:
            installed = False
    else:
        signal.signal(signal.SIGTERM, lambda signum, frame: callback(*args, **kwargs))
    return installed