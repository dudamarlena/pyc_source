...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/extern/aui/dockart.py
...
class AuiDefaultDockArt(object):
    ...
    **Button Identifiers**
    ...
    def GetMetric(self, id):
        ...
        if id == AUI_DOCKART_CAPTION_SIZE:
            return self._caption_size
        if id == AUI_DOCKART_GRIPPER_SIZE:
            return self._gripper_size
        if id == AUI_DOCKART_PANE_BORDER_SIZE:
            return self._border_size
        if id == AUI_DOCKART_PANE_BUTTON_SIZE:
            return self._button_size
        if id == AUI_DOCKART_GRADIENT_TYPE:
            return self._gradient_type
        if id == AUI_DOCKART_DRAW_SASH_GRIP:
    ...
    def GetColor(self, id):
        ...
        if id == AUI_DOCKART_BACKGROUND_GRADIENT_COLOUR:
            return self._background_gradient_colour
        if id == AUI_DOCKART_SASH_COLOUR:
            return self._sash_brush.GetColour()
        if id == AUI_DOCKART_INACTIVE_CAPTION_COLOUR:
            return self._inactive_caption_colour
        if id == AUI_DOCKART_INACTIVE_CAPTION_GRADIENT_COLOUR:
            return self._inactive_caption_gradient_colour
        if id == AUI_DOCKART_INACTIVE_CAPTION_TEXT_COLOUR:
            return self._inactive_caption_text_colour
        if id == AUI_DOCKART_ACTIVE_CAPTION_COLOUR:
            return self._active_caption_colour
        if id == AUI_DOCKART_ACTIVE_CAPTION_GRADIENT_COLOUR:
            return self._active_caption_gradient_colour
        if id == AUI_DOCKART_ACTIVE_CAPTION_TEXT_COLOUR:
            return self._active_caption_text_colour
        if id == AUI_DOCKART_BORDER_COLOUR:
            return self._border_pen.GetColour()
        if id == AUI_DOCKART_GRIPPER_COLOUR:
            return self._gripper_brush.GetColour()
        if id == AUI_DOCKART_HINT_WINDOW_COLOUR:
    ...
    def DrawBackground(self, dc, window, orient, rect):
        ...
        if wx.Platform == '__WXMAC__':
            ...
            dc.DrawRectangle(rect.x, rect.y, rect.width, rect.height)
        DrawGradientRectangle(dc, rect, self._background_brush.GetColour(), self._background_gradient_colour, AUI_GRADIENT_HORIZONTAL, rect.x, 700)
    ...
    def DrawBorder(self, dc, window, rect, pane):
        ...
        if pane.IsToolbar():
            for ii in xrange(0, border_width):
                ...
                dc.SetPen(self._border_pen)
                dc.DrawLine(drect.x, drect.y + drect.height - 1, drect.x + drect.width, drect.y + drect.height - 1)
                dc.DrawLine(drect.x + drect.width - 1, drect.y, drect.x + drect.width - 1, drect.y + drect.height)
                drect.Deflate(1, 1)
    ...
    def DrawCaptionBackground(self, dc, rect, pane):
        ...
        if self._gradient_type == AUI_GRADIENT_NONE:
            ...
            if active:
                if wx.Platform == '__WXMAC__':
                    DrawGradientRectangle(dc, rect, self._active_caption_colour, self._active_caption_gradient_colour, gradient_type)
                else:
                    DrawGradientRectangle(dc, rect, self._active_caption_gradient_colour, self._active_caption_colour, gradient_type)
            elif wx.Platform == '__WXMAC__':
                DrawGradientRectangle(dc, rect, self._inactive_caption_gradient_colour, self._inactive_caption_colour, gradient_type)
            else:
                DrawGradientRectangle(dc, rect, self._inactive_caption_colour, self._inactive_caption_gradient_colour, gradient_type)
...
if _ctypes:
    ...
    class RECT(ctypes.Structure):
        ...
        _fields_ = [
         (
          'left', ctypes.c_ulong), ('top', ctypes.c_ulong), ('right', ctypes.c_ulong), ('bottom', ctypes.c_ulong)]
...
class ModernDockArt(AuiDefaultDockArt):
    ...
    def DrawPaneButton(self, dc, window, button, button_state, rect, pane):
        ...
        if self.usingTheme:
            ...
            elif button_state == AUI_BUTTON_STATE_NORMAL:
                state = 5
            elif button_state == AUI_BUTTON_STATE_HOVER:
                state = 6
            elif button_state == AUI_BUTTON_STATE_PRESSED:
