...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/eclib/platebtn.py
...
class PlateButton(wx.PyControl):
    ...
    def __init__(self, parent, id=wx.ID_ANY, label='', bmp=None, pos=wx.DefaultPosition, size=wx.DefaultSize, style=PB_STYLE_DEFAULT, name=wx.ButtonNameStr):
        ...
        """
        super(PlateButton, self).__init__(parent, id, pos, size, wx.BORDER_NONE | wx.TRANSPARENT_WINDOW, name=name)
        ...
        if bmp is not None:
            ...
            img = img.ConvertToGreyscale(0.795, 0.073, 0.026)
        ...
        self.Bind(wx.EVT_ENTER_WINDOW, lambda evt: self._SetState(PLATE_HIGHLIGHT))
        self.Bind(wx.EVT_LEAVE_WINDOW, lambda evt: wx.CallLater(80, self.__LeaveWindow))
    ...
    def __DrawDropArrow(self, gc, xpos, ypos):
        ...
        if self._menu is not None or self._style & PB_STYLE_DROPARROW:
            ...
            tripoints = [
             (
              xpos, ypos), (xpos + 6, ypos), (xpos + 3, ypos + 5)]
            brush_b = gc.GetBrush()
            pen_b = gc.GetPen()
    ...
    def __DrawHighlight(self, gc, width, height):
        ...
        if self._style & PB_STYLE_GRADIENT:
            ...
            brush = rgc.CreateLinearGradientBrush(0, 1, 0, height, color, AdjustAlpha(color, 55))
            rgc.SetBrush(brush)
        else:
    ...
    def __InitColors(self):
        ...
        colors = dict(default=True, hlight=color, press=pcolor, htxt=BestLabelColour(self.GetForegroundColour()))
        return colors
    ...
    BitmapDisabled = property(lambda self: self.GetBitmapDisabled(), lambda self, bmp: self.SetBitmapDisabled(bmp))
    BitmapLabel = property(lambda self: self.GetBitmapLabel(), lambda self, bmp: self.SetBitmap(bmp))
    BitmapFocus = BitmapLabel
    BitmapHover = BitmapLabel
    BitmapSelected = BitmapLabel
    LabelText = property(lambda self: self.GetLabel(), lambda self, lbl: self.SetLabel(lbl))
