...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/ed_toolbar.py
# Compiled at: 2011-07-23 10:34:06
"""
...
import wx, ed_glob, ed_msg
from profiler import Profile_Get
_ = wx.GetTranslation
TOOL_ID = [
 ed_glob.ID_NEW, ed_glob.ID_OPEN, ed_glob.ID_SAVE, ed_glob.ID_PRINT,
...
class EdToolBar(wx.ToolBar):
    ...
    def _PopulateTools(self):
        ...
        self.AddSimpleTool(ed_glob.ID_PRINT, _('Print'), _('Print Current File'))
        self.AddSeparator()
        self.AddSimpleTool(ed_glob.ID_UNDO, _('Undo'), _('Undo Last Action'))
        self.AddSimpleTool(ed_glob.ID_REDO, _('Redo'), _('Redo Last Undo'))
        self.AddSeparator()
        self.AddSimpleTool(ed_glob.ID_CUT, _('Cut'), _('Cut Selected Text from File'))
        self.AddSimpleTool(ed_glob.ID_COPY, _('Copy'), _('Copy Selected Text to Clipboard'))
        self.AddSimpleTool(ed_glob.ID_PASTE, _('Paste'), _('Paste Text from Clipboard to File'))
        self.AddSeparator()
        self.AddSimpleTool(ed_glob.ID_FIND, _('Find'), _('Find Text'))
        self.AddSimpleTool(ed_glob.ID_FIND_REPLACE, _('Find/Replace'), _('Find and Replace Text'))
        self.AddSeparator()
