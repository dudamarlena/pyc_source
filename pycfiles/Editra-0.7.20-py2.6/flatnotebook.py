...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/extern/flatnotebook.py
# Compiled at: 2009-08-17 16:03:46
"""
...
import wx, random, math, weakref, cPickle
...
class FNBRenderer():
    ...
    def GetDropArrowButtonPos(self, pageContainer):
        ...
        if style & FNB_NO_X_BUTTON and style & FNB_NO_NAV_BUTTONS:
            ...
            if style & FNB_NO_X_BUTTON or style & FNB_NO_NAV_BUTTONS:
                return clientWidth - 36
    ...
    def GetButtonsAreaLength(self, pageContainer):
        ...
        if style & FNB_NO_NAV_BUTTONS and style & FNB_NO_X_BUTTON and not style & FNB_DROPDOWN_TABS_LIST:
            return 0
        if style & FNB_NO_NAV_BUTTONS and not style & FNB_NO_X_BUTTON and not style & FNB_DROPDOWN_TABS_LIST:
            return 22
...
class FNBRendererDefault(FNBRenderer):
    ...
    def DrawTab(self, pageContainer, dc, posx, tabIdx, tabWidth, tabHeight, btnStatus):
        ...
        if hasImage:
            ...
            pc._ImageList.Draw(pc._pagesInfoVec[tabIdx].GetImageIndex(), dc, posx + imageXOffset, imageYCoord, wx.IMAGELIST_DRAW_TRANSPARENT, True)
...
class FlatNotebook(wx.PyPanel):
    ...
    def __init__(self, parent, id=wx.ID_ANY, pos=wx.DefaultPosition, size=wx.DefaultSize, style=0, name='FlatNotebook'):
        """
        Default class constructor.
        ...
        if event.IsWindowChange():
            if len(self._windows) == 0:
                ...
                if self.HasFlag(FNB_SMART_TABS):
                    self._popupWin = self._popupWin or TabNavigatorWindow(self, self._naviIcon)
...
class PageContainer(wx.Panel):
    ...
    def __init__(self, parent, id=wx.ID_ANY, pos=wx.DefaultPosition, size=wx.DefaultSize, style=0):
        ...
        tabHeight = height + FNB_HEIGHT_SPACER
        wx.Panel.__init__(self, parent, id, pos, wx.Size(size.x, tabHeight), style | wx.NO_BORDER | wx.NO_FULL_REPAINT_ON_RESIZE | wx.WANTS_CHARS)
        self._pDropTarget = FNBDropTarget(self)
        self.SetDropTarget(self._pDropTarget)
    ...
    def OnLeftDown(self, event):
        ...
        if self._nLeftClickZone == FNB_DROP_DOWN_ARROW:
            ...
            if self._iActivePage != tabIdx:
                if len(self._pagesInfoVec) > tabIdx and self._pagesInfoVec[tabIdx].GetEnabled():
                    self.FireEvent(tabIdx)
    ...
    def OnDropTarget(self, x, y, nTabPage, wnd_oldContainer):
        """ Handles the drop action from a DND operation. """
        if len(wnd_oldContainer._pagesInfoVec) > nTabPage and not wnd_oldContainer._pagesInfoVec[nTabPage].GetEnabled():
            return wx.DragCancel
        self._isdragging = True
    ...
    def MoveTabPage(self, nMove, nMoveTo):
        ...
        if nMove == nMoveTo:
            return
        if nMoveTo < len(self._pParent._windows):
            nMoveTo = nMoveTo + 1
    ...
    def PopupTabsMenu(self):
        ...
        if has_bmp:
            longest += 20
        if not self.HasFlag(FNB_NO_X_BUTTON) or not self.HasFlag(FNB_NO_NAV_BUTTONS):
            longest += 16
        rect = self.GetClientRect()
        self.PopupMenu(popupMenu, wx.Point(rect.x + rect.GetWidth() - longest, rect.y + rect.height))
