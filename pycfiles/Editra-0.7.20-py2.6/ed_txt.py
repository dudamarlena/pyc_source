...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/ed_txt.py
# Compiled at: 2012-07-15 11:05:06
"""
...
import sys, re, time, wx, codecs, encodings as enclib, locale
from StringIO import StringIO
from util import Log
from profiler import Profile_Get
import ed_msg, ebmlib, ed_thread
...
BOM = {'utf-8': codecs.BOM_UTF8, 'utf-16': codecs.BOM, 
...
class EdFile(ebmlib.FileObjectImpl):
    ...
    def _HandleRawBytes(self, bytes_value):
        ...
        self._raw = True
        return ('\x00').join(bytes_value) + '\x00'
    ...
    def DecodeText(self):
        ...
        if not self._raw and '\x00' in ustr:
            Log('[ed_txt][info] DecodeText - joining nul terminators')
            ustr = ('\x00').join(bytes_value) + '\x00'
            self._raw = True
        if self._raw:
    ...
    def DetectEncoding(self):
        ...
        if self.encoding != None:
            msg = '[ed_txt][info] DetectEncoding, skipping do to user set encoding: %s' % self.encoding
            Log(msg)
            return
        else:
    ...
    def Read(self, chunk=512):
        ...
        if self.DoOpen('rb'):
            ...
            if self.encoding is None:
                self.encoding = Profile_Get('ENCODING', default=DEFAULT_ENCODING)
                Log('[ed_txt][warn] Failed to detect encoding falling back to default: %s' % self.encoding)
            self._ResetBuffer()
            self._raw = False
