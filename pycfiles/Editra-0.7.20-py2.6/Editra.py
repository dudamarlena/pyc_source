...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/Editra.py
# Compiled at: 2012-07-15 10:26:57
"""
...
import codecs, base64, locale, getopt, shutil, wx
try:
...
import ed_glob, ed_i18n, profiler, util, dev_tool, ed_main, ed_art, ed_txt, ed_event, updater, plugin, ed_ipc, ed_session, ebmlib
...
class Editra(wx.App, events.AppEventHandlerMixin):
    ...
    def OnInit(self):
        ...
        if not d_enc:
            ...
            if profiler.Profile_Get('APPSPLASH'):
                ...
                self.splash = wx.SplashScreen(splash_img, wx.SPLASH_CENTRE_ON_PARENT | wx.SPLASH_NO_TIMEOUT, 0, None, wx.ID_ANY)
                self.splash.Show()
            return True
    ...
    def DestroySplash(self):
        ...
        if getattr(self, 'splash', None) is not None and isinstance(self.splash, wx.SplashScreen):
            self.splash.Destroy()
            self.splash = None
        return
    ...
    def OpenNewWindow(self, fname='', caller=None):
        ...
        """
        frame = ed_main.MainWindow(None, wx.ID_ANY, profiler.Profile_Get('WSIZE'), ed_glob.PROG_NAME)
        if caller:
    ...
    def OnNotify(self, evt):
        ...
        """
        e_val = evt.GetValue()
        if evt.GetId() == ID_UPDATE_CHECK and isinstance(e_val, tuple) and e_val[0]:
            self.DestroySplash()
            mdlg = wx.MessageDialog(self.GetActiveWindow(), _('An updated version of Editra is available\nWould you like to download Editra %s now?') % e_val[1], _('Update Available'), wx.YES_NO | wx.YES_DEFAULT | wx.CENTER | wx.ICON_INFORMATION)
            if mdlg.ShowModal() == wx.ID_YES:
                dl_dlg = updater.DownloadDialog(None, wx.ID_ANY, _('Downloading Update'))
        ...
        if name in self._windows:
            ...
            if not len(self._windows):
                ...
                if name == repr(cur_top):
                    ...
                    found or self._log('[app][info] No more top windows exiting app')
                    self.UnLock()
                    self.Exit()
            else:
    ...
    def WindowCanLock(self, winname):
        ...
        if winname in self._windows:
            ...
            self._log('[app][warn] the window %s has not been registered' % winname)
            return False
...
def InitConfig():
    ...
    if util.HasConfigDir() and os.path.exists(ed_glob.CONFIG['PROFILE_DIR']):
        if profiler.ProfileIsCurrent():
            ...
            if mconfig is None:
                mconfig = [
                 synglob.LANG_C, synglob.LANG_CPP,
            ...
            elif sess and os.path.sep in sess:
...
def UpgradeOldInstall():
    ...
    if os.path.exists(old_cdir):
        if base.lower().rstrip(os.sep) != old_cdir.lower().rstrip(os.sep):
...
def _Main(opts, args):
    ...
    if editra_app.GetProfileUpdated():
        ...
        wx.MessageBox(_('Your profile has been updated to the latest version') + '\n' + _('Please check the preferences dialog to verify your preferences'), _('Profile Updated'))
