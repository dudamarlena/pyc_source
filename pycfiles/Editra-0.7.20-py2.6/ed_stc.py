...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/ed_stc.py
# Compiled at: 2013-01-05 16:27:14
"""
...
__svnid__ = '$Id: ed_stc.py 73349 2013-01-05 21:26:19Z CJP $'
__revision__ = '$Revision: 73349 $'
import os, wx, wx.stc, ed_event, ed_glob
from profiler import Profile_Get as _PGET
from syntax import syntax
import util, ed_basestc, ed_marker, ed_msg, ed_mdlg, ed_txt
from ed_keyh import KeyHandler, ViKeyHandler
import eclib, ebmlib, ed_thread
_ = wx.GetTranslation
...
def jumpaction(func):
    ...
    def WrapJump(*args, **kwargs):
        ...
        try:
            ...
            mdata = dict(fname=fname, prepos=pos, preline=line, lnum=cline, pos=cpos)
...
class EditraStc(ed_basestc.EditraBaseStc):
    ...
    def __init__(self, parent, id_, pos=wx.DefaultPosition, size=wx.DefaultSize, style=0, use_dt=True):
        ...
        self.CmdKeyAssign(ord('+'), wx.stc.STC_SCMOD_CTRL | wx.stc.STC_SCMOD_SHIFT, wx.stc.STC_CMD_ZOOMIN)
        ...
        self._config = dict(autocomp=_PGET('AUTO_COMP'), autoindent=_PGET('AUTO_INDENT'), brackethl=_PGET('BRACKETHL'), folding=_PGET('CODE_FOLD'), highlight=_PGET('SYNTAX'), autobkup=_PGET('AUTOBACKUP'))
        self.Configure()
    ...
    def EnableAutoBackup(self, enable):
        ...
        if enable:
            if not self._bktimer.IsRunning():
                self._bktimer.Start(30000)
        elif self._bktimer.IsRunning():
            self._bktimer.Stop()
    ...
    def OnBackupTimer(self, evt):
        ...
        if self.IsLoading() or ebmlib.GetFileSize(fname) > 5242880:
            ...
            if not self._backup_done and (not bkupmgr.HasBackup(fname) or bkupmgr.IsBackupNewer(fname)):
                ...
                def BackupJob(fobj, text):
                    ...
                    nevt = ed_event.StatusEvent(ed_event.edEVT_STATUS, idval, msg, ed_glob.SB_INFO)
    ...
    def OnKeyDown(self, evt):
        ...
        if self.key_handler.PreProcessKey(k_code, ctrl_down, cmd_down, shift_down, alt_down):
            return
        if wx.Platform == '__WXMAC__' and self._MacHandleKey(k_code, shift_down, alt_down, ctrl_down, cmd_down):
            pass
    ...
    def OnChar(self, evt):
        ...
        if self.key_handler.ProcessKey(key_code, evt.ControlDown(), evt.CmdDown(), evt.ShiftDown(), evt.AltDown()):
            pass
        else:
            if not self._config['autocomp'] or not cmpl.ShouldCheck(cpos):
                evt.Skip()
                return
            if key_code in cmpl.GetAutoCompKeys():
                self.HidePopups()
    ...
    def OnUserListSel(self, evt):
        ...
        mdata = dict(ltype=evt.GetListType(), text=evt.GetText(), stc=self)
        ed_msg.PostMessage(ed_msg.EDMSG_UI_STC_USERLIST_SEL, mdata, context=self.TopLevelParent.Id)
        evt.Skip()
    ...
    def OnDwellStart(self, evt):
        ...
        if not brect.Contains(mpoint) or not self.IsShown() or not self.TopLevelParent.IsActive():
            return
        else:
    ...
    def OnMarginClick(self, evt):
        ...
        if margin_num == ed_basestc.FOLD_MARGIN:
            if evt.GetShift() and (evt.GetControl() or wx.Platform == '__WXMAC__' and evt.GetAlt()):
                self.FoldAll()
            else:
                ...
                if level & wx.stc.STC_FOLDLEVELHEADERFLAG:
                    ...
                    elif evt.GetControl() or wx.Platform == '__WXMAC__' and evt.GetAlt():
    ...
    def FoldAll(self):
        ...
        while line_num < line_count:
            ...
            if level & wx.stc.STC_FOLDLEVELHEADERFLAG and level & wx.stc.STC_FOLDLEVELNUMBERMASK == wx.stc.STC_FOLDLEVELBASE:
                if expanding:
    ...
    def ControlDispatch(self, evt):
        ...
        e_map = {ed_glob.ID_COPY: self.Copy, ed_glob.ID_CUT: self.Cut, ed_glob.ID_PASTE: self.Paste, 
           ed_glob.ID_UNDO: self.Undo, ed_glob.ID_REDO: self.Redo, 
           ed_glob.ID_INDENT: self.Tab, ed_glob.ID_REVERT_FILE: self.RevertToSaved, 
        ...
        e_idmap = {ed_glob.ID_ZOOM_OUT: self.DoZoom, ed_glob.ID_ZOOM_IN: self.DoZoom, 
    ...
    def CheckEOL(self):
        ...
        if mixed or diff:
            if mixed:
                ...
                dlg = ed_mdlg.EdFormatEOLDlg(self.TopLevelParent, msg, _('Format EOL?'), eol_map.get(eol, self.GetEOLMode()))
    ...
    def ConvertLineMode(self, mode_id):
        ...
        """
        eol_map = {ed_glob.ID_EOL_MAC: wx.stc.STC_EOL_CR, ed_glob.ID_EOL_UNIX: wx.stc.STC_EOL_LF, 
    ...
    def GetEOLModeId(self):
        ...
        """
        eol_map = {wx.stc.STC_EOL_CR: ed_glob.ID_EOL_MAC, wx.stc.STC_EOL_LF: ed_glob.ID_EOL_UNIX, 
    ...
    def SetEOLMode(self, mode):
        ...
        """
        mode_map = {EDSTC_EOL_CR: wx.stc.STC_EOL_CR, EDSTC_EOL_LF: wx.stc.STC_EOL_LF, 
    ...
    def StartRecord(self):
        ...
        """
        self.recording = True
        evt = ed_event.StatusEvent(ed_event.edEVT_STATUS, self.GetId(), _('Recording Macro') + '...', ed_glob.SB_INFO)
    ...
    def StopRecord(self):
        ...
        evt = ed_event.StatusEvent(ed_event.edEVT_STATUS, self.GetId(), _('Recording Finished'), ed_glob.SB_INFO)
        wx.PostEvent(self.TopLevelParent, evt)
        self._BuildMacro()
        ...
        if os.path.exists(cfile):
            try:
                ...
                if txt is not None:
                    ...
                    return (
                     False, _('Failed to reload: %s') % cfile)
                ...
                ed_msg.PostMessage(ed_msg.EDMSG_FILE_OPENED, self.GetFileName(), context)
                return (True, '')
        else:
    ...
    def SaveFile(self, path):
        ...
        try:
            ...
            if self.File.IsReadOnly():
                wx.MessageBox(_('File is Read Only and cannot be saved'), _('Read Only'), style=wx.OK | wx.CENTER | wx.ICON_WARNING)
                return True
                self.File.IsRawBytes() or self.File.Write(self.GetText())
            else:
        ...
        wx.CallAfter(ed_msg.PostMessage, ed_msg.EDMSG_FILE_SAVED, (
         path, self.GetLangId()), tlw_id)
        return result
