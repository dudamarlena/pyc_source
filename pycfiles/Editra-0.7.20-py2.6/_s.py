...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/syntax/_s.py
...
(STC_S_DEFAULT, STC_S_COMMENT, STC_S_NUMBER, STC_S_STRING, STC_S_STRINGEOL, STC_S_OPERATOR, STC_S_KEYWORD) = range(7)
KEYWORDS = 'all array break call else exp for function if length library list match max mean min while return try NULL NA TRUE FALSE'
...
if wx.VERSION >= (2, 9, 0, 0, ''):
    SYNTAX_ITEMS = [
     ...
     (
      stc.STC_R_INFIX, 'default_style'),
     (
      stc.STC_R_INFIXEOL, 'default_style'),
     (
     ...
     (
      stc.STC_R_STRING2, 'char_style')]
else:
    SYNTAX_ITEMS = [
     (
      STC_S_DEFAULT, 'default_style'),
     (
...
TOKEN_MAP = {Token.Literal.String: STC_S_STRING, Token.Comment: STC_S_COMMENT, 
