...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/extern/vertedit.py
...
class VertEdit(object):
    ...
    def __init__(self, editor, blockBackColour='light blue', markerNumber=1):
    ...
    def SetBlockColor(self, color):
        ...
        """
        self.e.MarkerDefine(self.marker, stc.STC_MARK_BACKGROUND, background=color)
    ...
    def startMode(self, singleLine=False):
        ...
        self.lines = frozenset(range(min(self.origLine, self.curLine), max(self.origLine, self.curLine) + 1))
        self.jitter = None
        if singleLine:
