...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/eclib/ctrlbox.py
# Compiled at: 2012-04-11 20:28:33
"""
...
class ControlBox(wx.PyPanel):
    ...
    def __init__(self, parent, id=wx.ID_ANY, pos=wx.DefaultPosition, size=wx.DefaultSize, style=wx.TAB_TRAVERSAL | wx.NO_BORDER, name=CTRLBOX_NAME_STR):
    ...
    def SetControlBar(self, ctrlbar, pos=wx.TOP):
        ...
        if tbar is None and pos in (wx.TOP, wx.LEFT):
            ...
            if tbar is None and pos in (wx.BOTTOM, wx.RIGHT):
                sizer.Add(ctrlbar, 0, wx.EXPAND)
            else:
                sizer.Replace(tbar, ctrlbar)
                try:
...
class ControlBar(wx.PyPanel):
    ...
    def __init__(self, parent, id=wx.ID_ANY, pos=wx.DefaultPosition, size=wx.DefaultSize, style=CTRLBAR_STYLE_DEFAULT, name=CTRLBAR_NAME_STR):
        super(ControlBar, self).__init__(parent, id, pos, size, wx.TAB_TRAVERSAL | wx.NO_BORDER, name)
    ...
    def DoPaintBackground(self, dc, rect, color, color2):
        ...
        if self._style & CTRLBAR_STYLE_GRADIENT:
            ...
            if gc is None:
                return
                grad = self.IsVerticalMode() or gc.CreateLinearGradientBrush(rect.x, rect.y, rect.x, rect.x + rect.height, color2, color)
            else:
                grad = gc.CreateLinearGradientBrush(rect.x, rect.y, rect.x + rect.width, rect.y, color2, color)
...
class _SegmentButton(object):
    ...
    Id = property(lambda self: self._id, lambda self, id_: setattr(self, '_id', id_))
    Bitmap = property(lambda self: self._bmp, lambda self, bmp: setattr(self, '_bmp', bmp))
    Label = property(lambda self: self._lbl, lambda self, label: setattr(self, '_lbl', label))
    LabelSize = property(lambda self: self._lbl_size, lambda self, size: setattr(self, '_lbl_size', size))
    Rect = property(lambda self: self._rect, lambda self, rect: setattr(self, '_rect', rect))
    Selected = property(lambda self: self._selected, lambda self, sel: setattr(self, '_selected', sel))
    XState = property(lambda self: self._x_state, lambda self, state: setattr(self, '_x_state', state))
    XButton = property(lambda self: self._x_button, lambda self, rect: setattr(self, '_x_button', rect))
    BX1 = property(lambda self: self._bx1, lambda self, x1: setattr(self, '_bx1', x1))
    BX2 = property(lambda self: self._bx2, lambda self, x2: setattr(self, '_bx2', x2))
    Options = property(lambda self: self._opts, lambda self, opt: setattr(self, '_opts', opt))
...
class SegmentBar(ControlBar):
    ...
    def __init__(self, parent, id=wx.ID_ANY, pos=wx.DefaultPosition, size=wx.DefaultSize, style=CTRLBAR_STYLE_DEFAULT, name=SEGBAR_NAME_STR):
    ...
    def DoDrawButton(self, dc, pos, bidx, selected=False, draw_label=False):
        ...
        if self.SegmentHasCloseButton(bidx):
            if not bVertical:
                ...
                brect = wx.Rect(0, button.BX1, self.Rect.Width, button.BX2 - (pos - 1))
    ...
    def DoGetBestSize(self):
        ...
        if self.IsVerticalMode():
            ...
            height = mheight
        size = wx.Size(width + SegmentBar.HPAD * 2, height + SegmentBar.VPAD * 2)
    ...
    def OnMouseMove(self, evt):
        ...
        if index == -1:
            ...
            if button.XState != x_state:
                ...
                brect = self.IsVerticalMode() or wx.Rect(button.BX1, 0, button.BX2 - (button.BX1 - 2), crect.Height)
            else:
                brect = wx.Rect(button.BX1, 0, crect.Width, button.BX2 - (button.BX1 - 2))
    ...
    def OnTipTimer(self, evt):
        ...
        if index != -1:
            ...
            if button.Label:
                ...
                rect.x = x
                rect.y = y
                wx.TipWindow(self, button.Label, rectBound=rect)
    ...
    def OnPaint(self, evt):
        ...
        for (idx, button) in enumerate(self._buttons):
            npos = self.DoDrawButton(gc, npos, idx, self._selected == idx, use_labels)
    ...
    def SegmentHasCloseButton(self, index):
        ...
        """
        button = self._buttons[index]
        if button.Options & SEGBTN_OPT_CLOSEBTNL or button.Options & SEGBTN_OPT_CLOSEBTNR:
            return True
        return False
