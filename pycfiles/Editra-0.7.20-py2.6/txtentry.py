...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/eclib/txtentry.py
...
class CommandEntryBase(wx.SearchCtrl):
    ...
    def __init__(self, parent, id=wx.ID_ANY, value='', pos=wx.DefaultPosition, size=wx.DefaultSize, style=0, validator=wx.DefaultValidator, name='CommandEntryBase'):
        clone = None
        if validator != wx.DefaultValidator:
            clone = validator.Clone()
        super(CommandEntryBase, self).__init__(parent, id, value, pos, size, style, validator, name)
        self._txtctrl = None
    ...
    EnterCallback = property(lambda self: self._enterhook, lambda self, cback: setattr(self, '_enterhook', cback))
