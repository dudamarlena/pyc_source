...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/eclib/segmentbk.py
...
__all__ = [
 'SegmentBook', 'SegmentBookEvent', 'SEGBOOK_STYLE_DEFAULT',
...
SEGBOOK_STYLE_TOP = 4
SEGBOOK_STYLE_BOTTOM = 8
SEGBOOK_STYLE_LEFT = 16
SEGBOOK_STYLE_RIGHT = 32
...
class SegmentBook(ctrlbox.ControlBox):
    ...
    def __init__(self, parent, id=wx.ID_ANY, pos=wx.DefaultPosition, size=wx.DefaultSize, style=SEGBOOK_STYLE_DEFAULT, name=SEGBOOK_NAME_STR):
        """Initialize the SegmentBook"""
        super(SegmentBook, self).__init__(parent, id, pos, size, wx.TAB_TRAVERSAL | wx.NO_BORDER, name)
        self._pages = list()
        self._imglist = None
    ...
    def _DoPageChange(self, psel, csel):
        ...
        """
        event = SegmentBookEvent(edEVT_SB_PAGE_CHANGING, self.GetId(), csel, psel)
    ...
    def _OnRightDown(self, evt):
        ...
        if where in (ctrlbox.SEGMENT_HT_SEG, ctrlbox.SEGMENT_HT_X_BTN):
            if where == ctrlbox.SEGMENT_HT_SEG:
                ...
                if changed:
                    event = SegmentBookEvent(edEVT_SB_PAGE_CONTEXT_MENU, self.GetId())
