...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/ed_cmdbar.py
...
import os, sys, re, wx, util, ed_glob, ed_search, ed_event, ed_msg, ebmlib, eclib, ed_basewin
from profiler import Profile_Get
_ = wx.GetTranslation
from extern.embeddedimage import PyEmbeddedImage
XButton = PyEmbeddedImage('')
...
class CommandBarBase(ed_basewin.EdBaseCtrlBar, ebmlib.FactoryMixin):
    """Base class for control bars"""

    def __init__(self, parent):
        super(CommandBarBase, self).__init__(parent, style=eclib.CTRLBAR_STYLE_GRADIENT)
        ...
        self.ctrl = None
        self.close_b = eclib.PlateButton(self, ID_CLOSE_BUTTON, bmp=XButton.GetBitmap(), style=eclib.PB_STYLE_NOBG)
    ...
    MainControl = property(lambda self: self.ctrl, lambda self, ctrl: self.SetControl(ctrl))
                ...
                if len(to_menu):
                    ...
                    for item in to_menu:
                        if not item.GetLabel():
                            continue
                        self._menu.Append(item.Id, item.GetLabel(), kind=wx.ITEM_CHECK)
    ...
    def IsCustomizable(self, ctrl):
        ...
        ok = ok and (isinstance(ctrl, wx.CheckBox) or isinstance(ctrl, eclib.PlateButton))
        return ok
...
class SearchBar(CommandBarBase):
    ...
    def __init__(self, parent):
        ...
        self.SetControl(ed_search.EdSearchCtrl(self, wx.ID_ANY, menulen=5, size=(180,
                                                                                 -1)))
        ...
        fa_btn = eclib.PlateButton(self, ID_FIND_ALL, _('Find All'), t_bmp, style=eclib.PB_STYLE_NOBG, name='FindAllBtn')
...
class GotoLineBar(CommandBarBase):
    ...
    def __init__(self, parent):
        ...
        self.SetControl(LineCtrl(self, wx.ID_ANY, self._parent.nb.GetCurrentCtrl, size=(100,
                                                                                        -1)))
        self.EnableMenu(False)
...
class CommandExecuter(eclib.CommandEntryBase):
    ...
    def __init__(self, parent, id_, size=wx.DefaultSize):
        ...
        super(CommandExecuter, self).__init__(parent, id_, size=size, style=wx.TE_PROCESS_ENTER | wx.WANTS_CHARS)
    ...
    def ExecuteCommand(self, cmd_str):
        ...
        if cmd in ('x', 'ZZ'):
            cmd = 'wq'
        if cmd.startswith('w'):
            frame.OnSave(wx.MenuEvent(wx.wxEVT_COMMAND_MENU_SELECTED, ed_glob.ID_SAVE))
    ...
    def GetHistCommand(self, pre=True):
        ...
        if pre:
            if self._history['index'] < len(self._history['cmds']) - 1 and self._history['index'] < 25:
                self._history['index'] += 1
    ...
    def GoBuffer(self, cmd):
        ...
        for x in range(min(count, numpage)):
            ...
            if cpage == 0 and cmd == 'N' or cpage + 1 == numpage and cmd == 'n':
                break
    ...
    def GetPaths(self, path, files=False):
        ...
        if not os.access(head, os.R_OK):
            ...
            return list()
        try:
            candidates = [ os.path.basename(p) for p in os.listdir(head) if p.startswith(tail)
                         ]
            candidates = [ append_slash(os.path.join(os.path.dirname(path), cand)) for cand in candidates
                         ]
            if not files:
    ...
    def Quit(self):
        ...
        """
        wx.PostEvent(self.GetTopLevelParent(), wx.CloseEvent(wx.wxEVT_CLOSE_WINDOW))
...
class LineCtrl(eclib.CommandEntryBase):
    ...
    def __init__(self, parent, id_, get_doc, size=wx.DefaultSize):
        ...
        """
        super(LineCtrl, self).__init__(parent, id_, '', size=size, style=wx.TE_PROCESS_ENTER, validator=util.IntValidator(0, 65535))
        self._last = 0
        self.GetDoc = get_doc
...
class PopupList(wx.MiniFrame, PopupListBase):
    ...
    def __init__(self, parent, choices=list(), pos=wx.DefaultPosition):
        ...
        self._list = wx.ListBox(self, choices=choices, style=wx.LC_REPORT | wx.LC_SINGLE_SEL | wx.LC_NO_HEADER | wx.NO_BORDER)
    ...
    def __PostEvent(self):
        ...
        evt = ed_event.NotificationEvent(ed_event.edEVT_NOTIFY, self.GetId(), val, self._list)
