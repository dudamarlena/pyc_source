...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/eclib/colorsetter.py
...
__all__ = [
 'ColorSetter', 'ColorSetterEvent',
 'EVT_COLORSETTER', 'csEVT_COLORSETTER']
import wx, wx.lib.colourselect as csel
...
class ColorSetter(wx.Panel):
    ...
    def __init__(self, parent, id_, color=wx.NullColour):
        ...
        self._txt = wx.TextCtrl(self, value=self._label, style=wx.TE_CENTER, validator=HexValidator())
    ...
    def OnValidateTxt(self, evt):
        ...
        if code in (wx.WXK_DELETE, wx.WXK_BACK, wx.WXK_LEFT,
         wx.WXK_RIGHT, wx.WXK_TAB) or evt.CmdDown():
            ...
            return
        key = unichr(code)
        if key.isdigit() and evt.ShiftDown() or evt.AltDown() or evt.MetaDown():
            return
        if key in HEX_CHARS and (len(self._txt.GetValue().lstrip('#')) < 6 or self._txt.GetStringSelection()):
            evt.Skip()
...
class HexValidator(wx.PyValidator):
    ...
    def OnChar(self, event):
        ...
        if event.CmdDown() or key < wx.WXK_SPACE or key == wx.WXK_DELETE or key > 255 or chr(key) in HEX_CHARS[:-1]:
            event.Skip()
            return
