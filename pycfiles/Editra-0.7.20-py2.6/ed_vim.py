...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Editra/src/ed_vim.py
# Compiled at: 2012-04-11 20:28:34
"""
...
class EditraCommander(object):
    ...
    def _GetSelectionRange(self):
        ...
        """
        (start, end) = minmax(self.stc.GetSelectionStart(), self.stc.GetSelectionEnd())
        return (start, end)
    ...
    def _GetSelectedLines(self):
        ...
        (start, end) = self._GetSelectionRange()
        start_line, end_line = self.stc.LineFromPosition(start), self.stc.LineFromPosition(end - 1) + 1
...
def InsertMode(editor, repeat, cmd):
    ...
    cmd_map = {'i': lambda : None, 
       'I': editor.GotoIndentStart, 
       'A': editor.GotoLineEnd, 
       'a': editor.MoveForward, 
       'o': editor.OpenLine, 
       'O': editor.OpenLineUp}
    func = cmd_map[cmd]
    func()
    editor.SetLastChangeCommand(func)
    editor.SetInsertRepeat(repeat)
    editor.InsertMode()
...
@vim_parser('hjkl\r \x08ļĺ', is_motion=True)
def Arrows(editor, repeat, cmd):
    """Basic arrow movement in vim.
    @see: vim_parser

    """
    cmd_map = {'h': editor.MoveLeft, 
       'j': editor.MoveDown, 
       'k': editor.MoveUp, 
       'l': editor.MoveRight, 
       '\r': editor.MoveDown, 
    ...
    if cmd in cmd_map:
        cmd_map[cmd](repeat)
    elif cmd == 'ļ' and not editor.IsAtLineEnd():
        editor.MoveRight()
    elif cmd == 'ĺ' and not editor.IsAtLineStart():
        editor.MoveLeft()
...
def Words(editor, repeat, cmd):
    ...
    """
    cmd_map = {'w': editor.NextWord, 
       'e': editor.WordEnd, 
       'b': editor.BackWord, 
       'W': editor.NextWordBig, 
       'E': editor.WordEndBig, 
       'B': editor.BackWordBig, 
       '[': editor.BackWordPart, 
       ']': editor.WordPartEnd}
    cmd_map.get(cmd, lambda x: None)(repeat)


@vim_parser('$^0', is_motion=True)
def Line(editor, repeat, cmd):
    """Motions to beginning/end of a line.
    @see: vim_parser

    """
    cmd_map = {'0': editor.GotoLineStart, '^': editor.GotoIndentStart, 
       '$': editor.GotoLineEnd}
    cmd_map.get(cmd, lambda : None)()


@vim_parser('{}', is_motion=True)
def Para(editor, repeat, cmd):
    """Paragraph motions.
    @see: vim_parser

    """
    cmd_map = {'{': editor.ParaUp, '}': editor.ParaDown}
    cmd_map.get(cmd, lambda x: None)(repeat)
...
def FindIdent(editor, repeat, cmd):
    ...
    """
    cmd_map = {'#': editor.PrevIdent, '*': editor.NextIdent}
    cmd_map.get(cmd, lambda x: None)(repeat)
...
def Change(editor, repeat, cmd):
    ...
    cmd_map = {'y': editor.YankSelection, 
       'd': editor.DeleteSelection, 
       'c': editor.ChangeSelection, 
       '<': editor.DedentSelection, 
       '>': editor.IndentSelection}
    cmd_map[cmd]()
    restore_x = cmd in ('y', '<', '>') or cmd == 'd' and line_motion
    editor.PopColumn(restore_x)
    if cmd == 'c':
        pass
    else:
        editor.SetLastInsertedText('')
        editor.NormalMode()
    if not pre_selected:
...
@vim_parser('fFtT', is_motion=True)
def FindChar(editor, repeat, cmd):
    ...
    if len(cmd) == 1:
        return NeedMore
    (cmd, char) = cmd
    cmd_map = {'f': editor.FindNextChar, 
       'F': editor.FindPrevChar, 
       't': editor.FindTillNextChar, 
       'T': editor.FindTillPrevChar}
    if cmd in cmd_map:
...
@vim_parser('HLMGgz', is_motion=True, generic_repeat=False)
