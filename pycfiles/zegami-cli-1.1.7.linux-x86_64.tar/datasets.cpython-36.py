...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/zeg/datasets.py
...
from . import config, http, sql
MIMES = {'.tab':'text/tab-separated-values', 
...
def get(log, session, args):
    """Get a data set."""
    url = '{}datasets/{}'.format(http.get_api_url(args.url, args.project), args.id)
...
def update_from_dict(log, session, configuration):
    """Update a data set."""
    url_url = '{}imagesets/{}/image_url'.format(http.get_api_url(configuration['url'], configuration['project']), configuration['id'])
    replace_url = '{}datasets/{}/'.format(http.get_api_url(configuration['url'], configuration['project']), configuration['id'])
    log.debug('POST: {}'.format(url_url))
    if 'file_config' in configuration:
        file_path, extension, file_mime = _file_type_update(log, configuration['file_config'])
    else:
        if 'sql_config' in configuration:
            file_path, extension, file_mime = _sql_type_update(log, configuration['sql_config'])
    ...
    with open(file_path) as (f):
        ...
        current['source']['upload'] = {'name': file_name}
...
def delete(log, args):
    """Delete an data set."""
    log('dataset id: {highlight}{id}{reset}', highlight=(Fore.GREEN),
      id=(args.id),
...
def _sql_type_update(log, sql_config):
    ...
    return (
     name, mime_type, MIMES[mime_type])
...
def _newest_ctime(entry):
    return (
     -entry.stat().st_ctime, entry.name)


def _get_most_recent_file(path):
    """Get the most recent file in a directory."""
    allowed_ext = tuple(MIMES.keys())
    files_iter = (entry for entry in os.scandir(path) if entry.is_file() if entry.name.lower().endswith(allowed_ext))
