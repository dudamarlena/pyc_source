...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/zeg/imagesets.py
# Compiled at: 2020-03-20 12:32:01
# Size of source mod 2**32: 8423 bytes
"""Collection commands."""
import concurrent.futures, os, uuid
from colorama import Fore, Style
from tqdm import tqdm
from . import config, azure_blobs, http
...
def get(log, session, args):
    """Get an image set."""
    log('imageset id: {highlight}{id}{reset}', highlight=(Fore.GREEN),
      id=(args.id),
...
def _update_file_imageset(log, session, configuration):
    create_url = '{}imagesets/{}/image_url'.format(http.get_api_url(configuration['url'], configuration['project']), configuration['id'])
    complete_url = '{}imagesets/{}/images'.format(http.get_api_url(configuration['url'], configuration['project']), configuration['id'])
    ...
    with concurrent.futures.ThreadPoolExecutor(http.CONCURRENCY) as (executor):
        ...
        futures = [executor.submit(_upload_image, path, session, create_url, complete_url, log) for path in paths]
...
def _update_join_dataset(log, configuration, dataset_id, dataset_column, session, collection_id):
    dataset_create_url = '{}datasets/'.format(http.get_api_url(configuration['url'], configuration['project']))
    log.debug('POST: {}'.format(dataset_create_url))
    join_data = {'name':'join dataset', 
     'source':{'imageset_id':configuration['id'], 
      'dataset_id':dataset_id, 
      'imageset_to_dataset':{'dataset_column': dataset_column}}}
    log.debug('POST (join dataset): {}'.format(dataset_create_url))
    join_response = http.post_json(session, dataset_create_url, join_data)
    collection_url = '{}collections/{}'.format(http.get_api_url(configuration['url'], configuration['project']), collection_id)
    ...
    dz_json_join_url = '{}datasets/{}'.format(http.get_api_url(configuration['url'], configuration['project']), collection['dz_json_dataset_id'])
    log.debug('GET (dz json): {}'.format(dz_json_join_url))
    dz_json_join = http.get(session, dz_json_join_url)
    for_create = {'name':dz_json_join['dataset']['name'], 
...
def check_can_update(ims_type, ims):
    ...
    try:
        ...
        if len(ims.get('images', [])) != 0:
            raise ValueError('Chosen imageset already has images, cannot change type')
...
def update_from_dict(log, session, configuration):
    ...
    ims_url = '{}imagesets/{}'.format(http.get_api_url(configuration['url'], configuration['project']), ims_id)
    ...
    if ims_type == 'url':
        ...
        if ims_type == 'file':
            ...
            if ims_type == 'azure_storage_container':
                if os.environ.get('AZURE_STORAGE_CONNECTION_STRING', None) is None:
                    log.error('The AZURE_STORAGE_CONNECTION_STRING environment variable must be set in order to create an azure storage collection')
...
def _update_to_url_imageset(session, configuration, ims_url):
    keys = [
     'dataset_column', 'url_template']
    url_conf = {key:configuration.get(key) for key in keys if key in configuration}
    ims = {'name':'Imageset created by CLI', 
     'source':{'dataset_id':configuration['dataset_id'], 
      'transfer':{'url': url_conf}}}
...
def _resolve_paths(paths):
    ...
    for path in paths:
        if os.path.isdir(path):
            resolved.extend(entry.path for entry in os.scandir(path) if entry.is_file() if entry.name.lower().endswith(allowed_ext))
        else:
            if os.path.isfile(path) and path.lower().endswith(allowed_ext):
...
def _upload_image(path, session, create_url, complete_url, log):
    ...
    with open(path, 'rb') as (f):
        info = {'image': {'blob_id':str(uuid.uuid4()), 
                   'name':file_name, 
