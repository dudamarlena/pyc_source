...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/bigdoorkit/client.py
# Compiled at: 2010-09-16 19:25:53
import restkit, hashlib
try:
    import json
...
class Client(object):
    ...
    def _flatten_params(self, params):
        ...
        return ('').join([ '%s%s' % (k, params[k]) for k in sorted(params.keys()) if k not in ('sig',
                                                                                               'format')
                         ])
    ...
    def _sign_request(self, method, url, params=None, payload=None):
        ...
        if is_postish and 'time' not in payload:
            payload['time'] = params['time']
        if is_postish and 'token' not in payload:
            payload['token'] = self.generate_token()
        if method == 'delete' and 'delete_token' not in params:
