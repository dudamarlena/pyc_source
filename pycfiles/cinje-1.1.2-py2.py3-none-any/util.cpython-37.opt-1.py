...
def chunk(line, mapping={None:'text', 
 '${':'escape',  '#{':'bless',  '&{':'args',  '%{':'format',  '@{':'json'}):
...
class Line(object):
    __doc__ = 'A rich description for a line of input, allowing for annotation.'
...
class Lines(object):
    __doc__ = 'Iterate input lines of source, with the ability to push lines back.'
    __slots__ = [
     'Line', 'source', 'buffer']
...
class Context(object):
    __doc__ = 'The processing context for translating cinje source into Python source.\n\t\n\tThis is the primary entry point for translation.\n\t'
    ...
    def stream(self):
        ...
        for line in self.input:
            ...
            for line in handler(self):
                if root:
                    mapping.appendleft(line.number or mapping[0])
