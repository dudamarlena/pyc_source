...
# [GCC 8.4.0]
# Embedded file name: /Users/elijah/Development/django-persistent-file-widget/persistent_widget/widgets.py
# Compiled at: 2012-05-25 14:15:09
...
from persistent_widget.utils import get_media_relative_path, get_media_url, make_tmp_media_file
import os, mimetypes
        ...
        if value:
            try:
                ...
                if not hasattr(value, 'url'):
                    ...
                    upload = value
                data = self.get_context_data(name=name, value=value, upload=upload, exists=self.exists)
        ...
        if files is not None and name in files:
            ...
            if existing:
                ...
                with open(path, 'r+b') as (file):
                    contents = file.read()
                    size = file.tell()
                uploaded_file = InMemoryUploadedFile(file=ContentFile(contents), field_name=name, name=existing, content_type=mimetypes.guess_type(path)[0], size=size, charset=mimetypes.guess_type(path)[1])
...
class PersistentImageWidget(PersistentFileWidget):
    ...
    def get_context_data(self, **data):
        ...
        try:
            ...
            upload = data['upload']
            thumb = get_thumbnail(upload, '100x100', crop='center', quality=99)
            thumb_url = get_media_url(thumb.name) if thumb.url.startswith('http:') or thumb.url.startswith('https:') else get_media_url(thumb.url)
