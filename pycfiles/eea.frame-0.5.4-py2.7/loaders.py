...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/frame/loaders.py
...
class Loader(BaseLoader):
    ...
    def _process_resp(self, html):
        substitutions = [
         ...
         ('<!-- block_messages -->', '{% block action_buttons %}{% endblock %}{% block messages %}{% endblock %}'),
         ('<!-- block_content -->', '{% block zope_content %}{% endblock %}'),
         ('<!-- block_head -->', '{% block head %}{% endblock %}'),
         ('src="misc_/', 'src="/misc_/')]
    ...
    def load_template_source(self, template_name, template_dirs=None):
        ...
        if request and getattr(settings, 'FRAME_URL', None) and template_name == 'frame.html':
