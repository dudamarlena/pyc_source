...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/frame/middleware.py
...
class UserMiddleware(MiddlewareMixin):
    ...
    def _fetch_data(self):
        ...
        try:
            resp = requests.get(settings.FRAME_URL, cookies=forwarded_cookies, verify=verify)
            data = resp.json()
    ...
    def process_request(self, request):
        if getattr(settings, 'FRAME_URL', None):
            ...
            if resp_json:
                ...
                request.language = get_current_language(resp_json['frame_html']) or getattr(settings, 'DEFAULT_LANGUAGE', None)
                if request.user_id:
                    request.META['REMOTE_USER'] = {'user_id': request.user_id, 'user_roles': request.user_roles, 
