...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/prompt_tool_kit/layout/menus.py
# Compiled at: 2019-08-15 23:53:38
# Size of source mod 2**32: 19431 bytes
from __future__ import unicode_literals
from six.moves import zip_longest, range
from prompt_tool_kit.filters import HasCompletions, IsDone, Condition, to_cli_filter
...
class CompletionsMenuControl(UIControl):
    ...
    def create_content(self, cli, width, height):
        ...
        if complete_state:
            ...
            return UIContent(get_line=get_line, cursor_position=Point(x=0, y=(index or 0)),
...
def _trim_text(text, max_width):
    ...
    if width > max_width:
        if len(text) == width:
            ...
            trimmed_text += '...'
            return (
             trimmed_text, get_cwidth(trimmed_text))
    else:
        return (
         text, width)
...
class CompletionsMenu(ConditionalContainer):
    ...
    def __init__(self, max_height=None, scroll_offset=0, extra_filter=True, display_arrows=False):
        ...
        super(CompletionsMenu, self).__init__(content=Window(content=(CompletionsMenuControl()),
          width=LayoutDimension(min=8),
          height=LayoutDimension(min=1, max=max_height),
          scroll_offsets=ScrollOffsets(top=scroll_offset, bottom=scroll_offset),
          right_margins=[
         ScrollbarMargin(display_arrows=display_arrows)],
...
class MultiColumnCompletionsMenu(HSplit):
    __doc__ = '\n    Container that displays the completions in several columns.\n    When `show_meta` (a :class:`~prompt_tool_kit.filters.CLIFilter`) evaluates\n    to True, it shows the meta information at the bottom.\n    '
    ...
    def __init__(self, min_rows=3, suggested_max_column_width=30, show_meta=True, extra_filter=True):
        ...
        completions_window = ConditionalContainer(content=Window(content=MultiColumnCompletionMenuControl(min_rows=min_rows,
          suggested_max_column_width=suggested_max_column_width),
          width=LayoutDimension(min=8),
          height=LayoutDimension(min=1)),
          filter=full_filter)
        meta_window = ConditionalContainer(content=Window(content=(_SelectedCompletionMetaControl())),
    ...
    def _get_tokens(self, cli):
        ...
        if state:
            if state.current_completion:
                if state.current_completion.display_meta:
                    return [
                     (
                      token, ' %s ' % state.current_completion.display_meta)]
        return []