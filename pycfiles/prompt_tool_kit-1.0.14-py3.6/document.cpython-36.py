...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/prompt_tool_kit/document.py
# Compiled at: 2019-08-15 23:53:39
# Size of source mod 2**32: 35977 bytes
"""
The `Document` that implements all the text operations/querying.
"""
from __future__ import unicode_literals
import bisect, re, six, string, weakref
...
_text_to_document_cache = weakref.WeakValueDictionary()
...
class Document(object):
    ...
    def find_boundaries_of_current_word(self, WORD=False, include_leading_whitespace=False, include_trailing_whitespace=False):
        ...
        def get_regex(include_whitespace):
            return {(False, False): _FIND_CURRENT_WORD_RE, 
    ...
    def paste_clipboard_data(self, data, paste_mode=PasteMode.EMACS, count=1):
        ...
        if data.type == SelectionType.CHARACTERS:
            ...
            if data.type == SelectionType.BLOCK:
                lines = self.lines[:]
    ...
    def insert_after(self, text):
        ...
        """
        return Document(text=(self.text + text),
    ...
    def insert_before(self, text):
        ...
        if selection_state:
            selection_state = SelectionState(original_cursor_position=(selection_state.original_cursor_position + len(text)),
              type=(selection_state.type))
        return Document(text=(text + self.text),
