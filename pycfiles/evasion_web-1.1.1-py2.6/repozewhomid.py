...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\evasion\web\config\repozewhomid.py
...
def add_auth(app, sitename, sessionname, sessionsecret, password_file, groups_file, permissions_file):
    ...
    form = FriendlyFormPlugin(login_url, login_handler, post_login_url, logout_handler, post_logout_url, login_counter_name=login_counter_name, rememberer_name='cookie')
    cookie = AuthTktCookiePlugin(cookie_secret, cookie_name)
    identifiers = [
     (
      'main_identifier', form), ('basicauth', basicauth), ('cookie', cookie)]
    challengers = [
     (
      'form', form), ('basicauth', basicauth)]
    authenticators = [
     (
      'htpasswd', passauth_and_metadata)]
    mdproviders = [
     (
      'simplemeta', passauth_and_metadata)]
    groups = {'all_groups': INIGroupAdapter(groups)}
    permissions = {'all_perms': INIPermissionsAdapter(permissions)}
    app_with_auth = setup_auth(app=app, group_adapters=groups, permission_adapters=permissions, identifiers=identifiers, authenticators=authenticators, challengers=challengers, mdproviders=mdproviders, log_level=logging.DEBUG)
