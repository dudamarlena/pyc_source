...
# [GCC 8.4.0]
# Embedded file name: e:\nmdev\src\branches\loyalty\evasion-web\evasion\web\scripts\main.py
# Compiled at: 2010-07-30 06:54:13
"""
...
"""
import os, sys, os.path, logging, threading, logging.config
...
class Run(object):
    ...
    def __init__(self, ini_file=None, nologsetup=False):
        ...
        m = dict(state=self.cp.get('Messenger', 'state', 'off'), host=self.cp.get('Messenger', 'host', '127.0.0.1'), port=self.cp.get('Messenger', 'port', 61613), username=self.cp.get('Messenger', 'username', ''), password=self.cp.get('Messenger', 'password', ''), channel=self.cp.get('Messenger', 'channel', 'evasion'))
    ...
    def getConfig(self, cp, section, expected_use_value):
        ...
        if use != expected_use_value:
            msg = "unexpected value for 'use=' in section '%s': expected '%s', found '%s'"
    ...
    def setUpStomp(self):
        ...
        stomp_cfg = dict(host=self.cp.get('Messenger', 'host'), port=int(self.cp.get('Messenger', 'port')), username=self.cp.get('Messenger', 'username'), password=self.cp.get('Messenger', 'password'), channel=self.cp.get('Messenger', 'channel'))
...
def main():
    parser = OptionParser()
    parser.add_option('--config', action='store', dest='config_filename', default=None, help='This evasion.web configuration file used at run time.')
    parser.add_option('--setup-app', action='store_true', dest='setupapp', default=False, help='This does a paster setup-app with the --config option.')
