...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/python/settings.py
# Compiled at: 2020-05-13 19:28:40
# Size of source mod 2**32: 12675 bytes
"""
Module that contains different classes to handle settings
"""
from __future__ import print_function, division, absolute_import
import os, sys, ast, time, configparser
...
class JSONSettings(FileSettings, object):
    ...
    def set_directory(self, directory, filename='settings.json'):
        self.directory = directory
        old = filename.endswith('.json') or path.join_path(directory, filename)
        if path.is_file(old):
            self.file_path = old
            self._read()
            return
...
class INISettings(object):
    ...
    def get_section(self):
        ...
        """
        if self._section:
            return self._section[(len(self._section) - 1)].upper()
    ...
    def show_in_explorer(self):
        ...
        """
        import subprocess
        subprocess.Popen('explorer /select, {0}'.format(self._file))
