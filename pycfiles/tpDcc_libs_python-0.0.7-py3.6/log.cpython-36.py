...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/python/log.py
# Compiled at: 2020-05-13 19:28:40
# Size of source mod 2**32: 12795 bytes
    ...
    def log_file(self, file_name, formatter=None, mode='a', max_bytes=0, backup_count=0, encoding=None, log_level=None, enable_stderr_logger=True):
        ...
        if formatter is None:
            if self._formatter is None:
                formatter = logging.Formatter(fmt=(Logger._LOGGER_DEFAULT_FORMAT), datefmt=(Logger._LOGGER_DEFAULT_DATE_FORMAT))
        if file_name:
            rotating_file_handler = RotatingFileHandler(file_name, mode=mode, maxBytes=max_bytes, backupCount=backup_count,
              encoding=encoding)
            setattr(rotating_file_handler, Logger._LOGGER_IDENTIFIER, True)
            if log_level:
