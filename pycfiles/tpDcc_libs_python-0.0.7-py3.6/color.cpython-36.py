...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/python/color.py
# Compiled at: 2020-05-13 19:28:40
# Size of source mod 2**32: 13035 bytes
"""
...
def convert_single_srgb_to_linear(color_value):
    ...
    if color_value <= 0.04045:
        return color_value * 0.07739938080495357
    else:
...
def convert_single_linear_to_srgb(color_value):
    ...
    if color_value <= 0.0031308:
        ...
        return (1 + a) * pow(color_value, 0.4166666666666667) - a
...
def hex_to_rgba(hex_str):
    ...
    if hex_str.startswith('#'):
        ...
        if len(hex_str) == 8:
            return (
             int(hex_str[2:4], 16), int(hex_str[4:6], 16), int(hex_str[6:8], 16), int(hex_str[0:2], 16))
        if len(hex_str) == 6:
            return (
             int(hex_str[0:2], 16), int(hex_str[2:4], 16), int(hex_str[4:6], 16), 255)
        if len(hex_str) == 3:
            return (
             int(hex_str[0:1] * 2, 16), int(hex_str[1:2] * 2, 16), int(hex_str[2:3] * 2, 16), 255)
...
def hsl_color_offset_int(rgb_color, hue_offset=0, saturation_offset=0, lightness_offset=0):
    ...
    rgb_color = hsl_color_offset_float(rgb_color,
      hue_offset=hue_offset, saturation_offset=saturation_offset, lightness_offset=lightness_offset)
...
class RGBRotate(object):
    ...
    def set_hue_rotation(self, degrees):
        ...
        self.matrix[0][1] = 0.3333333333333333 * (1.0 - cos_a) - math.sqrt(0.3333333333333333) * sin_a
        self.matrix[0][2] = 0.3333333333333333 * (1.0 - cos_a) + math.sqrt(0.3333333333333333) * sin_a
        self.matrix[1][0] = 0.3333333333333333 * (1.0 - cos_a) + math.sqrt(0.3333333333333333) * sin_a
        self.matrix[1][1] = cos_a + 0.3333333333333333 * (1.0 - cos_a)
        self.matrix[1][2] = 0.3333333333333333 * (1.0 - cos_a) - math.sqrt(0.3333333333333333) * sin_a
        self.matrix[2][0] = 0.3333333333333333 * (1.0 - cos_a) - math.sqrt(0.3333333333333333) * sin_a
        self.matrix[2][1] = 0.3333333333333333 * (1.0 - cos_a) + math.sqrt(0.3333333333333333) * sin_a
        self.matrix[2][2] = cos_a + 0.3333333333333333 * (1.0 - cos_a)
