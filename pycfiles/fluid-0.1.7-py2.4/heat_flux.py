...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/fluid/interaction/heat_flux.py
...
def noon_solar_altitude(DOY, Lat):
    ...
    k = N.pi / 180.0
    t = 2 * N.pi * DOY / 365.0
    d = 0.397 + 3.63 * N.sin(t) - 22.98 * N.cos(t) + 0.04 * N.sin(2 * t) - 0.388 * N.cos(2 * t) + 0.075 * N.sin(3 * t) - 0.16 * N.cos(3 * t)
    dl = k * d
    ll = k * Lat
...
def short_wave_clear_sky(Lat, DOY):
    ...
    """
    LatTheta = Lat / 180.0 * N.pi
    phi = (DOY - 21) * 360.0 / 365.0
    phi = phi / 180.0 * N.pi
    Qcs = Lat * 0
    for (L, LT, P, n) in zip(Lat, LatTheta, phi, range(len(Lat))):
        if L > -20 and L <= 40:
            ...
            B1 = -3.27 + 108.7 * N.sin(LT)
            A2 = -0.64 + 7.8 * N.sin(2 * (LT - N.pi / 4))
            B2 = -0.5 + 14.42 * N.cos(2 * (LT - N.pi / 36))
            Qcs[n] = A0 + A1 * N.cos(P) + B1 * N.sin(P) + A2 * N.cos(2 * P) + B2 * N.sin(2 * P)
        elif L > 40 and L < 60:
            A0 = 342.61 - 1.97 * L - 0.018 * L ** 2
            A1 = 52.08 - 5.86 * L + 0.043 * L ** 2
            B1 = -4.8 + 2.46 * L - 0.017 * L ** 2
...
def cloud_cover(Lat, FracQ, NSA):
    ...
    C = C * N.greater_equal(C, 0.3) * N.less_equal(C, 1) + N.greater(C, 1)
    return C
...
def long_wave_radiation(Ts, Ta, ea, C, Lat, method='Clark'):
    ...
    if method == 'Bunker':
        ...
        a = a + (N.greater_equal(abs(Lat), 75) & N.less(abs(Lat), 85)) * 0.84
        a = a + (N.greater_equal(abs(Lat), 65) & N.less(abs(Lat), 75)) * 0.8
        a = a + (N.greater_equal(abs(Lat), 55) & N.less(abs(Lat), 65)) * 0.76
        a = a + (N.greater_equal(abs(Lat), 45) & N.less(abs(Lat), 55)) * 0.72
        a = a + (N.greater_equal(abs(Lat), 35) & N.less(abs(Lat), 45)) * 0.68
        a = a + (N.greater_equal(abs(Lat), 25) & N.less(abs(Lat), 35)) * 0.63
        a = a + (N.greater_equal(abs(Lat), 15) & N.less(abs(Lat), 25)) * 0.59
        a = a + (N.greater_equal(abs(Lat), 7.5) & N.less(abs(Lat), 15)) * 0.52
        a = a + N.less(abs(Lat), 7.5) * 0.5
        CF = 1 - a * C
        Qlw = 0.022 * (_emiss_lw * _sigma * Ta ** 4 * (11.7 - 0.23 * ea) * CF) + 4 * _emiss_lw * _sigma * Ta ** 3 * (Ts - Ta)
    elif method == 'Clark':
        a = N.zeros(N.shape(Lat), N.Float)
        a = a + (N.greater_equal(abs(Lat), 45) & N.less(abs(Lat), 55)) * 0.73
        a = a + (N.greater_equal(abs(Lat), 35) & N.less(abs(Lat), 45)) * 0.69
        a = a + (N.greater_equal(abs(Lat), 25) & N.less(abs(Lat), 35)) * 0.64
        a = a + (N.greater_equal(abs(Lat), 15) & N.less(abs(Lat), 25)) * 0.6
        a = a + (N.greater_equal(abs(Lat), 7.5) & N.less(abs(Lat), 15)) * 0.56
        a = a + (N.greater_equal(abs(Lat), 2.5) & N.less(abs(Lat), 7.5)) * 0.53
        a = a + N.less(abs(Lat), 2.5) * 0.51
        CF = 1 - a * C ** 2
        Qlw = -_emiss_lw * _sigma * Ts ** 4 * (0.39 - 0.05 * ea ** 0.5) * CF - 4 * _emiss_lw * _sigma * Ts ** 3 * (Ts - Ta)
    else:
        return
    return Qlw
