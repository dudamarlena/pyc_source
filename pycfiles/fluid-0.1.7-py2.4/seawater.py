...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/fluid/ocean/seawater.py
# Compiled at: 2006-12-04 09:19:16
"""Seawater routines."""
import numpy as N

def set_Sal(R, t, p):
    a = N.array([0.008, -0.1692, 25.3851, 14.0941, -7.0261, 2.7081])
    r_t = set_r_t(t)
    R_p = set_R_p(R, p, t)
...
def _sal(R_t, t):
    ...
    S = a[0] + a[1] * R_t ** (1 / 2) + a[2] * R_t + a[3] * R_t ** (3 / 2) + a[4] * R_t ** 2 + a[5] * R_t ** (5 / 2) + DeltaS
    return S


def _RT35(XT):
    c = N.array([0.6766097, 0.0200564, 0.0001104259, -6.9698e-07, 1.0031e-09])
    RT35 = (((1.0031e-09 * XT + c[3]) * XT + c[2]) * XT + c[1]) * XT + c[0]
    return RT35


def _c(XP):
    e = N.array([0, 2.07e-05, -6.37e-10, 3.989e-15])
    c = ((e[3] * XP + e[2]) * XP + e[1]) * XP
    return c


def _b(XT):
    d = N.array([0, 0.03426, 0.0004464, 0.4215, -0.003107])
    b = (d[2] * XT + d[1]) * XT + 1.0
    return b
...
def Sal2R(S, t, p):
    ...
    for i in range(20):
        ...
        SI_new = a[1] * R_t ** (1 / 2) + 2 * a[2] * R_t + 3 * a[3] * R_t ** (3 / 2) + 4 * a[4] * R_t ** 2 + 5 * a[5] * R_t ** (5 / 2) + DDeltaS
        R_t = R_t + (S - SI) / SI_new
        SI = _sal(R_t, t)
...
def set_DeltaS(t, R_t):
    ...
    k = 0.0162
    DeltaS = (t - 15) / (1 + k * (t - 15)) * (b[0] + b[1] * R_t ** (1 / 2) + b[2] * R_t + b[3] * R_t ** (3 / 2) + b[4] * R_t ** 2 + b[5] * R_t ** (5 / 2))
    return DeltaS
...
def set_DDeltaS(t, R_t):
    b = N.array([0.0005, -0.0056, -0.0066, -0.0375, 0.0636, -0.0144])
    k = 0.0162
    DeltaS = (t - 15) / (1 + k * (t - 15)) * (b[1] + 2 * b[2] * R_t + 3 * b[3] * R_t ** (3 / 2) + 4 * b[4] * R_t ** 2 + 5 * b[5] * R_t ** (5 / 2))
    return DeltaS
...
def set_R_p(R, t, p):
    ...
    e = N.array([0, 2.07e-05, -6.37e-10, 3.989e-15])
    d = N.array([0, 0.03426, 0.0004464, 0.4215, -0.003107])
    R_p = 1 + p * (e[1] + e[2] * p + e[3] * p ** 2) / (1 + d[1] * t + d[2] * t ** 2 + (d[3] + d[4] * t) * R)
    return R_p
...
def O2_saturation(s, K):
    ...
    """
    a = [
     -173.4292, 249.6339, 143.3483, -21.8492]
    b = [-0.033096, 0.014259, -0.0017]
    lnC = a[0] + a[1] * (100 / K) + a[2] * N.log(K / 100) + a[3] * (K / 100) + s * (b[0] + b[1] * (K / 100) + b[2] * (K / 100) ** 2)
    c = N.exp(lnC)
    return c
...
def _atg(s, t, p):
    ...
    """
    a = [
     3.5803e-05, 8.5258e-06, -6.836e-08, 6.6228e-10]
    ...
    atg = a[0] + (a[1] + (a[2] + a[3] * t) * t) * t + (b[0] + b[1] * t) * (s - 35) + (c[0] + (c[1] + (c[2] + c[3] * t) * t) * t + (d[0] + d[1] * t) * (s - 35)) * p + (e[0] + (e[1] + e[2] * t) * t) * p ** 2
    return atg
...
def _dens0(s, t):
    ...
    """
    b = [
     0.824493, -0.0040899, 7.6438e-05, -8.2467e-07, 5.3875e-09]
    c = [-0.00572466, 0.00010227, -1.6546e-06]
    d = [0.00048314]
    rho0 = _smow(t) + (b[0] + b[1] * t + b[2] * t ** 2 + b[3] * t ** 3 + b[4] * t ** 4) * s + (c[0] + c[1] * t + c[2] * t ** 2) * s ** (3.0 / 2) + d[0] * s ** 2
    return rho0
...
def _seck(s, t, p):
    ...
    p = p / 10.0
    h = [
     3.239908, 0.00143713, 0.000116092, -5.77905e-07]
    aw = h[0] + (h[1] + (h[2] + h[3] * t) * t) * t
    k = [
     8.50935e-05, -6.12293e-06, 5.2787e-08]
    bw = k[0] + (k[1] + k[2] * t) * t
    e = [
     19652.21, 148.4206, -2.327105, 0.01360477, -5.155288e-05]
    kw = e[0] + (e[1] + (e[2] + (e[3] + e[4] * t) * t) * t) * t
    i = [
     0.0022838, -1.0981e-05, -1.6078e-06]
    j = [0.000191075]
    ...
    f = [
     54.6746, -0.603459, 0.0109987, -6.167e-05]
    g = [0.07944, 0.016483, -0.00053009]
    k0 = kw + (f[0] + (f[1] + (f[2] + f[3] * t) * t) * t + (g[0] + (g[1] + g[2] * t) * t) * N.sqrt(s)) * s
    k = k0 + (a + b * p) * p
    return k
...
def _smow(t):
    ...
    """
    a = [
     999.842594, 0.06793952, -0.00909529, 0.0001001685, -1.120083e-06, 6.536332e-09]
...
def specific_volume_anomaly(s, t, p):
    ...
    """
    delta = 1.0 / dens(s, t, p) - 1.0 / dens(35.0, 0.0, p)
    return delta
