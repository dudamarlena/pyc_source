...
# [GCC 8.4.0]
# Embedded file name: /Users/vshekar/anaconda3/lib/python3.7/site-packages/recommender/mutual_information.py
# Compiled at: 2019-06-17 15:53:50
...
DESCRIPTORS_TO_REMOVE = [
 'compound_0_amount_grams', 'compound_1_amount_grams',
...
class Pdf:
    ...
    def __init__(self, group: dict) -> None:
        ...
        self.success_descriptor_normal, self.failure_descriptor_normal = self.calc_joint_pdfs()
    ...
    def calc_pdf(self) -> Any:
        ...
        if self.success_count <= 1 or len(self.group['result']) - self.success_count <= 1:
            return
        self.success_mean = self.success_count / float(len(self.group['result']))
        self.fail_mean = 1 - self.success_mean
        self.descriptor_mean = np.mean((self.group['descriptors']),
          axis=0)
        self.descriptor_cov = np.cov(self.group['descriptors'].T)
        try:
            descriptor_normal = multivariate_normal(mean=(self.descriptor_mean),
              cov=(self.descriptor_cov),
              allow_singular=True)
    ...
    def calc_joint_pdfs(self):
        ...
        if self.success_count <= 1 or len(self.group['result']) - self.success_count <= 1:
            return (None, None)
        success_indices = [idx for idx, val in enumerate(self.group['result']) if val == True]
        success_descriptors = np.array([self.group['descriptors'][i] for i in success_indices])
        try:
            success_descriptor_normal = multivariate_normal(mean=np.mean(success_descriptors, axis=0), cov=(np.cov(success_descriptors.T)),
              allow_singular=True)
        except Exception as e:
            try:
                raise e
        ...
        failure_indices = [idx for idx, val in enumerate(self.group['result']) if val == False]
        failure_descriptors = np.array([self.group['descriptors'][i] for i in failure_indices])
        try:
            failure_descriptor_normal = multivariate_normal(mean=np.mean(failure_descriptors, axis=0), cov=(np.cov(failure_descriptors.T)),
              allow_singular=True)
        except Exception as e:
            try:
                raise e
        ...
        return (
         success_descriptor_normal, failure_descriptor_normal)
    ...
    def row_mutual_information(self, row: list) -> float:
        ...
        if feature:
            ...
            if joint != 0:
                mutual_information += joint * math.log(joint / (self.success_mean * feature), 2)
            joint = self.prob_joint(row, FAILURE)
            if joint > 1e-300:
                mutual_information += joint * math.log(joint / (self.fail_mean * feature), 2)
        return mutual_information
...
class MutualInformation:
    ...
    def __init__(self, dataset: pd.DataFrame, result_column_label: str, compound_column_labels: list, descriptors_to_keep: list) -> None:
        """
    ...
    def get_delta_mutual_info(self, candidate: np.array) -> float:
        ...
        if best_compounds:
            delta_mutual_info = self.groups[best_compounds]['pdf'].delta_mi(candidate)
...
if __name__ == '__main__':
    ...
    print([mi.get_delta_mutual_info(row[:268].astype(float)) for row in dataset_new])