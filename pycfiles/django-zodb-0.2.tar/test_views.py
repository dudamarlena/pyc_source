...
# [GCC 8.4.0]
# Embedded file name: /Users/osantana/Work/django-zodb/django-zodb/tests/test_views.py
...
class ViewsTests(TestCase):
    ...
    def test_traverse_result(self):
        ...
        _eq(tr(ROOT, './foo'), root=ROOT, ctx=ROOT['foo'], mn='', sp=(), tr=('foo', ))
        _eq(tr(ROOT, '/foo'), root=ROOT, ctx=ROOT['foo'], mn='', sp=(), tr=('foo', ))
        _eq(tr(ROOT, '/foo/bar'), root=ROOT, ctx=ROOT['foo']['bar'], mn='', sp=(), tr=('foo',
                                                                                       'bar'))
        _eq(tr(ROOT, '/foo/bar/baz'), root=ROOT, ctx=ROOT['foo']['bar'], mn='baz', sp=(), tr=('foo',
                                                                                              'bar',
                                                                                              'baz'))
        _eq(tr(ROOT, '/foo/bar/baz/subpath/1/2'), root=ROOT, ctx=ROOT['foo']['bar'], mn='baz', sp=('subpath',
                                                                                                   '1',
                                                                                                   '2'), tr=('foo',
                                                                                                             'bar',
                                                                                                             'baz'))
        _eq(tr(ROOT, '/@@foo/bar/baz/'), root=ROOT, ctx=ROOT, mn='foo', sp=('bar',
                                                                            'baz'), tr=('@@foo', ))
        _eq(tr(ROOT, '/foo/@@bar/baz/'), root=ROOT, ctx=ROOT['foo'], mn='bar', sp=('baz', ), tr=('foo',
                                                                                                 '@@bar'))
        _eq(tr(ROOT, '/foo/bar/@@baz/subpath/1/2'), root=ROOT, ctx=ROOT['foo']['bar'], mn='baz', sp=('subpath',
                                                                                                     '1',
                                                                                                     '2'), tr=('foo',
        ...
        _eq(tr(ROOT, '/foo/bar/baz/@@subpath/1/2'), root=ROOT, ctx=ROOT['foo']['bar'], mn='baz', sp=('@@subpath',
                                                                                                     ...
                                                                                                     '2'), tr=('foo',
                                                                                                               'bar',
                                                                                                               'baz'))
        _eq(tr(ROOT, '/foo/qux/'), root=ROOT, ctx=ROOT['foo']['qux'], mn='', sp=(), tr=('foo',
                                                                                        'qux'))
        _eq(tr(ROOT, '/foo/qux/quxx'), root=ROOT, ctx=ROOT['foo']['qux']['quxx'], mn='', sp=(), tr=('foo',
                                                                                                    'qux',
                                                                                                    'quxx'))
        _eq(tr(ROOT, '/foo/qux/quxx/quxxx'), root=ROOT, ctx=ROOT['foo']['qux']['quxx'], mn='quxxx', sp=(), tr=('foo',
                                                                                                               'qux',
                                                                                                               'quxx',
                                                                                                               'quxxx'))
        _eq(tr(ROOT, '/foo/qux/quxx/quxxx/sub'), root=ROOT, ctx=ROOT['foo']['qux']['quxx'], mn='quxxx', sp=('sub', ), tr=('foo',
                                                                                                                          'qux',
                                                                                                                          'quxx',
                                                                                                                          'quxxx'))
    ...
    def test_view_results(self):
        ...
        self.assertEquals(t.method_name, '')
        t['bla'] = 'ble'
