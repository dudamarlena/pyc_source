...
# [GCC 8.4.0]
# Embedded file name: /Users/osantana/Work/django-zodb/django-zodb/tests/test_storage.py
# Compiled at: 2010-11-26 08:17:57
import os
...
class StorageTests(TestCase):
    ...
    def test_disabled_reason(self):
        ...
        del factories.disabled['disabled']
    ...
    def test_zeo_storage_with_host(self):
        ...
        self.assertEquals(self._fake_factories(uri), {'storage': {'addr': ('localhost', 8090), 
    ...
    def test_zeo_storage_with_sock(self):
        ...
        self.assertEquals(self._fake_factories(uri), {'storage': {'addr': '/tmp/zeo.zdsock', 
                       'blob_dir': '/tmp/blobdir', 
                       'wait_timeout': 1, 
                       'wait': True}})
    ...
    def test_mysql_storage(self):
        if not self._enabled('mysql'):
            return
        uri = 'mysql://test_user:test_pass@test_host?compress=1&create=false&read_only=true&blob_dir=/tmp&cache_servers=cache1,cache2,cache3#test_dbname'
        self.assertEquals(self._fake_factories(uri), {'adapter': {'user': 'test_user', 
                       'passwd': 'test_pass', 
                       'host': 'test_host', 
                       'db': 'test_dbname', 
                       'compress': True}, 
    ...
    def test_postgresql_storage(self):
        if not self._enabled('postgresql'):
            return
        uri = 'postgresql://test_user:test_pass@test_host?create=false#test_dbname'
        self.assertEquals(self._fake_factories(uri), {'adapter': {'dsn': 'dbname=test_dbname host=test_host password=test_pass user=test_user'}, 
