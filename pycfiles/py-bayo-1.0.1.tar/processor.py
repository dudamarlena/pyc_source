...
# Embedded file name: /Library/Python/2.5/site-packages/bankpassweb/satchmo_payment/processor.py
# Compiled at: 2008-01-28 10:42:20
import logging
...
class PaymentProcessor(object):
    ...
    def __init__(self, settings):
        ...
        if settings.LIVE.value:
            init = {'env': settings.CONNECTION.value, 'crn': settings.CRN.value, 
               'start_secret': settings.TRANKEY.value, 
               'result_secret': settings.RESULTKEY.value}
        else:
            init = {'env': settings.CONNECTION_TEST.value, 'crn': settings.CRN_TEST.value, 
    ...
    def prepare_data(self, data):
        """
                ...
                """
        amount = str(int(data.balance * 10 ** self.settings.SHIFT_DIGITS.value))
        self.transaction_data = {'first_name': data.contact.first_name, 
    ...
    def _get_start_url(self):
        ...
        data.update({'urldone': rurl('SUCCESS_RETURN_ADDRESS'), 
