...
# Embedded file name: /Library/Python/2.5/site-packages/bankpassweb/apirequests.py
# Compiled at: 2007-12-03 16:38:28
...
class RequestFactory(object):
    ...
    def __getattr__(self, attr, default=None):
        klass = general_parser(Request, attr, '__name__', builder=None)
...
class Request(DataTree):
    """
        ...
        classes.
        """
    macfields = ('OPERAZIONE', 'TIMESTAMP', 'IDNEGOZIO', 'OPERATORE', 'REQREFNUM')
    reqfields = []
    optfields = []
    kwmappings = {'req_id': 'REQREFNUM', 'trans_id': 'IDTRANS', 'order_id': 'NUMORD', 
       'amount': 'IMPORTO', 'cont_type': 'TCONTAB', 'former_req_id': 'REQREFNUMORIG', 
       'start': 'DATAINIZIO', 'end': 'DATAFINE', 
       'currency': 'VALUTA', 'close_order': 'FINEORDINE', 
       'filter': 'FILTRO'}
    ...
    def add_if_missing(self, fieldtag, value):
        """
                ...
                already specified.
                """
        if fieldtag not in self.params:
    ...
    def __prepare_for_req(self, fieldname, val):
        ...
        try:
            k = None
            k = BPWDataType.parse(fieldname, attr='fieldname', builder=None)
    ...
    def __init__(self, engine=None, params=None, xml=None, blankfields=[], **kwargs):
        """
        ...
        for arg in kwargs:
            try:
                ...
                raise TypeError('%s parameter unknown to Request classes' % arg)

            if not self._has_field(fieldname):
                raise TypeError('%s got an unexpected keyword parameter %s' % (
                 self.__class__.__name__,
                 arg))
            assert fieldname not in self.params, '%s is already here -- why?' % fieldname
            self.params[fieldname] = self.__prepare_for_req(fieldname, kwargs[arg])

        self.add_if_missing('OPERAZIONE', self.opname)
        from datetime import datetime
        self.add_if_missing('TIMESTAMP', datetime.now().isoformat()[:23])
        self.add_if_missing('IDNEGOZIO', self.engine.crn)
        self.add_if_missing('OPERATORE', self.engine.operator)
        from uuid import uuid4
        self.add_if_missing('REQREFNUM', ('0' * 32 + str(int(uuid4().int % 1e+32)))[-32:])
    ...
    def __set_if_present(self, tag, attr, valtype=None):
        ...
        if valtype:
            try:
                ...
                raise 'Parse method not available for object %s, type %s' % (
                 val, valtype)
    ...
    def __init_from_xml(self, xml):
        ...
        from datatypes import Amount, Currency, AcctType, AuthFilterType
        mappings = (
         ('IDtrans', 'trans_id'),
         (
        ...
        for el in self.xml:
            try:
                ...
                raise BPWException('Tag %s was not parsed in receipt for request %s' % (
                 el._name,
                 self.__class__.__name__))
    ...
    def _qs(self):
        """
                ...
                querystring.
                """
        self.params['MAC'] = self.engine.compute_mac(self.params, self.reqfields, self.optfields, api=True, secret=self.engine.result_secret)
        return ('&').join([ ('=').join((par, str(self.params[par]))) for par in self.params
                          ])
...
class Authorize(Request):
    ...
    macfields = Request.macfields + ('IDTRANS', 'NUMORD', 'IMPORTO', 'VALUTA', 'TCONTAB',
                                     'FINEORDINE')
    reqfields = macfields

    @classmethod
    def parse(cls, xml, **kwargs):
        req = cls(xml.IDtrans, xml.Importo, xml.Valuta, cont_type=xml.Tcontab, close_order=xml.FineOrdine)
    ...
    def __init__(self, trans_id, order_id, amount, currency, **kwargs):
        ...
        if 'cont_type' not in kwargs:
            kwargs['cont_type'] = 'I'
        if 'close_order' not in kwargs:
...
class ReqTransOrderAmount(Request):
    """
        ...
        """
    macfields = Request.macfields + ('IDTRANS', 'NUMORD', 'IMPORTO', 'VALUTA')
    reqfields = macfields
