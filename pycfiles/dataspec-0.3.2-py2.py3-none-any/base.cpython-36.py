...
# Compiled at: 2020-05-07 12:24:41
# Size of source mod 2**32: 51556 bytes
...
class DictSpec(Spec):
    tag: Tag
    _keyspecs = attr.ib(factory=dict)
    _keyspecs: Mapping[(Hashable, _KeySpec)]
    conformer = None
    conformer: Optional[Conformer]
    ...
    def validate(self, d) -> Iterator[ErrorDetails]:
        try:
            for k, keyspec in self._keyspecs.items():
                if keyspec.is_optional:
                    if k in d:
                        yield from _enrich_errors(keyspec.spec.validate(d[k]), self.tag, k)
                else:
                    if k in d:
                        yield from _enrich_errors(keyspec.spec.validate(d[k]), self.tag, k)
                    else:
    ...
    def merge(cls, tag: Optional[Tag], *specs: 'DictSpec', conformer: Optional[Conformer]=None) -> Spec:
        ...
        for spec in specs:
            for k, keyspec in spec._keyspecs.items():
                map_pred[(OptionalKey(k) if keyspec.is_optional else k)].append(keyspec.spec)
