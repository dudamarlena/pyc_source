...
# [GCC 8.4.0]
# Embedded file name: /Users/chrisrink/Projects/dataspec/src/dataspec/factories.py
# Compiled at: 2020-05-11 10:29:55
# Size of source mod 2**32: 64013 bytes
import re, sys, threading, uuid
from datetime import date, datetime, time
from email.headerregistry import Address as EmailAddress
from functools import partial
from typing import AbstractSet, Any, Callable, Iterator, List, Mapping, MutableMapping, Optional, Pattern, Set, Tuple, Type, TypeVar, Union, cast
from urllib.parse import ParseResult, parse_qs, urlparse
import attr
from dataspec.base import INVALID, Conformer, ErrorDetails, Invalid, ObjectSpec, ObjectSpecKey, OptionalKey, PredicateSpec, Spec, SpecPredicate, Tag, ValidatorFn, ValidatorSpec, any_spec, compose_conformers, make_spec, pred_to_validator, tag_maybe

def blankable_spec(tag_or_pred: Union[(Tag, SpecPredicate)], *preds: SpecPredicate, conformer: Optional[Conformer]=None):
    """
    ...
    if len(preds) != 1:
        raise ValueError("Must provide exactly one Spec predicate for 'blankable' Specs")
    ...
    def blank_or_pred(e):
        ...
        if spec_errors:
            ...
            yield ErrorDetails(message=f"Value '{e}' is not blank",
              pred=blank_or_pred,
              value=e)
    ...
    return ValidatorSpec((tag or 'blankable'),
      blank_or_pred,
      conformer=(compose_conformers(conform_blankable, conformer)))


def bool_spec(tag: Tag='bool', allowed_values: Optional[Set[bool]]=None, conformer: Optional[Conformer]=None) -> Spec:
    """
    ...
    if not allowed_values is None:
        if not all(isinstance(e, bool) for e in allowed_values):
    ...
    if allowed_values is not None:
        ...
        @pred_to_validator(f"Value '{{value}}' not in {allowed_values}",
          complement=True)
        def is_allowed_bool_type(v):
            return v in allowed_values

        validators.append(is_allowed_bool_type)
    return (ValidatorSpec.from_validators)(tag, *validators, **{'conformer': conformer})


def bytes_spec(tag: Tag='bytes', type_: Tuple[(Union[(Type[bytes], Type[bytearray])], ...)]=(
 bytes, bytearray), length: Optional[int]=None, minlength: Optional[int]=None, maxlength: Optional[int]=None, regex: Union[(Pattern, bytes, None)]=None, conformer: Optional[Conformer]=None) -> Spec:
    """
    ...
    if length is not None:
        if not isinstance(length, int):
            ...
            if minlength is not None or maxlength is not None:
                raise ValueError('Cannot define a byte spec with exact length and minlength or maxlength')

        @pred_to_validator(f"Bytes length does not equal {length}", convert_value=len)
        def bytestr_is_exactly_len(v):
            return len(v) != length
    ...
    if minlength is not None:
        ...
        @pred_to_validator(f"Bytes '{{value}}' does not meet minimum length {minlength}")
        def bytestr_has_min_length(s):
    ...
    if regex is not None:
        ...
        @pred_to_validator("Bytes '{value}' does match regex '{regex}'",
          complement=True, regex=regex)
        def bytes_matches_regex(s):
...
def default_spec(tag_or_pred: Union[(Tag, SpecPredicate)], *preds: SpecPredicate, default: Any=None, conformer: Optional[Conformer]=None) -> Spec:
    """
    ...
    return any_spec((tag or 'default'),
      (preds[0]),
...
def _make_datetime_spec_factory(type_: Union[(Type[datetime], Type[date], Type[time])]) -> Callable[(..., Spec)]:
    """
    Factory function for generating datetime type spec factories.

    Yep.
    """
    aware_docstring_blurb = 'If ``is_aware`` is :py:obj:`True` , the resulting Spec will validate that input\n    values are timezone aware. If ``is_aware`` is :py:obj:`False` , the resulting Spec\n    will validate that inpute values are naive. If unspecified, the resulting Spec\n    will not consider whether the input value is naive or aware.' if type_ is not date else 'If ``is_aware`` is specified, a :py:exc:`TypeError` will be raised as\n    :py:class:`datetime.date` values cannot be aware or naive.'
    format_docstring_blurb = f"If the\n    :py:class:`datetime.datetime` object parsed from the ``format_`` string contains\n    a portion not available in :py:class:`datetime.{type_.__name__}` , then the\n    validator will emit an error at runtime." if type_ is not datetime else ''
    ...
    if type_ is date:
        ...
        if type_ is time:
            ...
            def strptime(s: str, fmt: str) -> time:
                parsed = datetime.strptime(s, fmt)
    ...
    def _datetime_spec_factory(tag=type_.__name__, format_=None, before=None, after=None, is_aware=None, conformer=None):

        @pred_to_validator(f"Value '{{value}}' is not {type_}", complement=True)
        def is_datetime_type(v):
        ...
        if after is not None:
            if before is not None:
                if after < before:
                    raise ValueError("Date spec 'after' criteria must be after'before' criteria if both specified")
        if is_aware is not None:
            if type_ is datetime:

                @pred_to_validator("Datetime '{value}' is not aware",
                  complement=is_aware)
        ...
        if format_ is not None:
            ...
            def conform_datetime_str(s):
                try:
                    return strptime(s, format_)
                except (TypeError, ValueError):
                    return INVALID

            def validate_datetime_str(s):
                try:
                    dt = strptime(s, format_)
                except TypeError as e:
                    yield ErrorDetails(message=f"String contains invalid portion for type: {e}",
                      pred=validate_datetime_str,
                      value=s)
                except ValueError:
                    yield ErrorDetails(message=f"String does not contain a date which can be parsed as '{format_}'",
                      pred=validate_datetime_str,
                      value=s)
                else:
...
try:
    ...
    def datetime_str_spec(tag: Tag='datetime_str', iso_only: bool=False, before: Optional[datetime]=None, after: Optional[datetime]=None, is_aware: Optional[bool]=None, conformer: Optional[Conformer]=None) -> Spec:
        """
        ...
        def str_contains_datetime(s):
            try:
                parsed_dt = parse_date_str(s)
            except (OverflowError, ValueError):
                yield ErrorDetails(message=f"String '{s}' does not contain a datetime",
                  pred=str_contains_datetime,
                  value=s)
            else:
                yield from dt_spec.validate(parsed_dt)

        return ValidatorSpec.from_validators(tag,
          is_str,
          str_contains_datetime,
          conformer=(compose_conformers(cast(Conformer, parse_date_str), conformer)))


def dict_tag_spec(tag_or_pred: Union[(Tag, SpecPredicate)], *preds: SpecPredicate, conformer: Optional[Conformer]=None) -> Spec:
    """
    ...
    tag, preds = tag_maybe(tag_or_pred, *preds)
    if len(preds) != 1:
        raise ValueError(f"Dict specs must specify exactly one spec predicate, not {len(preds)}")
    pred = preds[0]
    ...
    return make_spec(filter(None, (tag,)), *({k:make_spec(_unwrap_opt_key(k), v) for k, v in pred.items()},), **{'conformer': conformer})


_IGNORE_OBJ_PARAM = object()
_EMAIL_RESULT_FIELDS = frozenset({'username', 'domain'})

def _obj_attr_validator(object_name: str, attr: str, exact_attr: Any, regex_attr: Any, in_attr: Any, exact_attr_ignore: Any=_IGNORE_OBJ_PARAM, regex_attr_ignore: Any=_IGNORE_OBJ_PARAM, in_attr_ignore: Any=_IGNORE_OBJ_PARAM, disallowed_attrs_regex: Optional[AbstractSet[str]]=None) -> Optional[ValidatorFn]:
    """
    ...
    if exact_attr is not exact_attr_ignore:
        ...
        @pred_to_validator(f"{object_name} attribute '{attr}' value '{{value}}' is not '{exact_attr}'",
          complement=True,
          convert_value=get_obj_attr)
        def obj_attr_equals(v):
            return get_obj_attr(v) == exact_attr

        return obj_attr_equals
    if regex_attr is not regex_attr_ignore:
        if disallowed_attrs_regex is not None:
            if attr in disallowed_attrs_regex:
                raise ValueError(f"Cannot define regex spec for {object_name} attribute '{attr}'")
        if not isinstance(regex_attr, str):
            raise TypeError(f"{object_name} attribute '{attr}_regex' must be a string value")
        pattern = re.compile(regex_attr)

        @pred_to_validator(f"{object_name} attribute '{attr}' value '{{value}}' does not match regex '{regex_attr}'",
          complement=True,
          convert_value=get_obj_attr)
        def obj_attr_matches_regex(v):
            return bool(re.fullmatch(pattern, get_obj_attr(v)))

        return obj_attr_matches_regex
    else:
        if in_attr is not in_attr_ignore:
            if not isinstance(in_attr, (frozenset, set)):
                raise TypeError(f"{object_name} attribute '{attr}_in' must be set or frozenset")

            @pred_to_validator(f"{object_name} attribute '{attr}' value '{{value}}' not in {in_attr}",
              complement=True,
              convert_value=get_obj_attr)
            def obj_attr_is_allowed_value(v):
...
def email_spec(tag: Tag='email', conformer: Optional[Conformer]=None, **kwargs) -> Spec:
    """
    ...
    for email_attr in _EMAIL_RESULT_FIELDS:
        ...
        if sum(int(v is not _IGNORE_OBJ_PARAM) for v in [in_attr, regex_attr, exact_attr]) > 1:
            raise ValueError(f"Email specs may only specify one of {email_attr}, {email_attr}_in, and {email_attr}_regex for any Email attribute")
        attr_validator = _obj_attr_validator('Email', email_attr, exact_attr, regex_attr, in_attr)
    ...
    def str_contains_email(s):
        try:
            addr = EmailAddress(addr_spec=s)
        except (TypeError, ValueError) as e:
            yield ErrorDetails(message=f"String '{s}' does not contain a valid email address: {e}",
    ...
    return ValidatorSpec.from_validators(tag,
      is_str, str_contains_email, conformer=conformer)


def nilable_spec(tag_or_pred: Union[(Tag, SpecPredicate)], *preds: SpecPredicate, conformer: Optional[Conformer]=None) -> Spec:
    """
    ...
    def nil_or_pred(e):
        ...
        if spec_errors:
            yield from spec_errors
            yield ErrorDetails(message=f"Value '{e}' is not None",
              pred=nil_or_pred,
              value=e)

    return ValidatorSpec((tag or 'nilable'),
      nil_or_pred,
      conformer=(compose_conformers(spec.conformer, conformer)))


def num_spec(tag: Tag='num', type_: Union[(Type, Tuple[(Type, ...)])]=(
 float, int), min_: Union[(complex, float, int, None)]=None, max_: Union[(complex, float, int, None)]=None, conformer: Optional[Conformer]=None) -> Spec:
    """
...
def obj_spec(tag_or_pred: Union[(Tag, SpecPredicate)], *preds: SpecPredicate, conformer: Optional[Conformer]=None) -> Spec:
    """
    ...
    return ObjectSpec.from_val((tag or 'object'),
...
try:
    ...
    def conform_phonenumber(s: str, region: Optional[str]=None) -> Union[(Invalid, str)]:
    ...
    def phonenumber_spec(tag: Tag='phonenumber_str', region: Optional[str]=None, is_possible: bool=True, is_valid: bool=True, conformer: Optional[Conformer]=None) -> Spec:
        """
        ...
        if region is not None:
            ...
            @pred_to_validator(f"Parsed telephone number regions ({{value}}) does not match expected region {region}",
              complement=True,
              convert_value=(lambda p: ', '.join(phonenumbers.region_codes_for_country_code(p.country_code))))
            def validate_phonenumber_region(p):
        ...
        if is_possible:
            ...
            @pred_to_validator("Parsed telephone number '{value}' is not possible",
              complement=True)
        ...
        if is_valid:
            ...
            @pred_to_validator("Parsed telephone number '{value}' is not valid",
              complement=True)
        ...
        def str_contains_phonenumber(s):
            try:
                p = phonenumbers.parse(s, region=region)
            except phonenumbers.NumberParseException:
                yield ErrorDetails(message=f"String '{s}' does not contain a telephone number",
...
def register_str_format(tag: Tag, conformer: Optional[Conformer]=None) -> Callable[([ValidatorFn], ValidatorFn)]:
    """
...
def _str_is_uuid(s: str) -> Iterator[ErrorDetails]:
    try:
        ...
        yield ErrorDetails(message='String does not contain UUID',
          pred=_str_is_uuid,
          value=s)
...
if sys.version_info >= (3, 7):
    ...
    def _str_is_iso_date(s: str) -> Iterator[ErrorDetails]:
        try:
            ...
            yield ErrorDetails(message='String does not contain ISO formatted date',
    ...
    def _str_is_iso_datetime(s: str) -> Iterator[ErrorDetails]:
        try:
            ...
            yield ErrorDetails(message='String does not contain ISO formatted datetime',
    ...
    def _str_is_iso_time(s: str) -> Iterator[ErrorDetails]:
        try:
            ...
            yield ErrorDetails(message='String does not contain ISO formatted time',
              pred=_str_is_iso_time,
              value=s)


else:
    _ISO_DATE_REGEX = re.compile('(\\d{4})-(\\d{2})-(\\d{2})')
...
def str_spec(tag: Tag='str', length: Optional[int]=None, minlength: Optional[int]=None, maxlength: Optional[int]=None, regex: Union[(Pattern, str, None)]=None, format_: Optional[str]=None, conform_format: Optional[str]=None, conformer: Optional[Conformer]=None) -> Spec:
    """
    ...
    if length is not None:
        if not isinstance(length, int):
            ...
            if minlength is not None or maxlength is not None:
                raise ValueError('Cannot define a string spec with exact length and minlength or maxlength')

        @pred_to_validator(f"String length does not equal {length}", convert_value=len)
        def str_is_exactly_len(v):
            return len(v) != length
    ...
    if minlength is not None:
        ...
        @pred_to_validator(f"String '{{value}}' does not meet minimum length {minlength}")
        def str_has_min_length(s):
            return len(s) < minlength
    ...
    if regex is not None:
        if format_ is None:
            if conform_format is None:
                _pattern = regex if isinstance(regex, Pattern) else re.compile(regex)

                @pred_to_validator("String '{value}' does not match regex '{regex}'",
                  complement=True,
                  regex=regex)
                def str_matches_regex(s):
                    return bool(_pattern.fullmatch(s))

                validators.append(str_matches_regex)
    if regex is None and format_ is not None and conform_format is None:
        with _STR_FORMAT_LOCK:
        ...
        if sum(int(v is not None) for v in [regex, format_, conform_format]) > 1:
            raise ValueError('Cannot define a spec with more than one of: regex, format, conforming format')
    return (ValidatorSpec.from_validators)(tag, *validators, **{'conformer': conformer})
...
def url_str_spec(tag: Tag='url_str', query: Optional[SpecPredicate]=None, conformer: Optional[Conformer]=None, **kwargs) -> Spec:
    """
    ...
    for url_attr in _URL_RESULT_FIELDS:
        ...
        if sum(int(v is not _IGNORE_OBJ_PARAM) for v in [in_attr, regex_attr, exact_attr]) > 1:
            raise ValueError(f"URL specs may only specify one of {url_attr}, {url_attr}_in, and {url_attr}_regex for any URL attribute")
        attr_validator = _obj_attr_validator('URL',
          url_attr,
          exact_attr,
          regex_attr,
          in_attr,
    ...
    def validate_url(s):
        try:
            ...
            yield ErrorDetails(message=f"String does not contain a valid URL: {e}",
              pred=validate_url,
              value=s)
        else:
...
def uuid_spec(tag: Tag='uuid', versions: Optional[Set[int]]=None, conformer: Optional[Conformer]=None) -> Spec:
    """
    ...
    if versions is not None:
        ...
        @pred_to_validator(f"UUID '{{value}}' is not in versions {versions}",
          complement=True)
        def uuid_is_version(v):
            return v.version in versions
