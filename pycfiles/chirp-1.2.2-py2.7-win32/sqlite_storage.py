...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\chirp\compare\sqlite_storage.py
    ...
    def _load_signals(self, signal_dir):
        ...
        with con:
            ...
            if not self.restrict:
                con.executemany('INSERT OR IGNORE INTO %s (filename) VALUES (?)' % self.signal_table, ((os.path.splitext(os.path.split(x)[1])[0],) for x in files))
            cursor = con.execute('SELECT id, filename FROM %s ORDER BY id' % self.signal_table)
            self.signals = [ (i, os.path.join(signal_dir, f + self.file_pattern)) for i, f in cursor.fetchall() if os.path.join(signal_dir, f + self.file_pattern) in files
                           ]

    def _create_target_table(self, con, values):
        sql_columns = (',\n').join('`%s` %s' % (name, sqlite_type(val)) for name, val in zip(self.compare_stat_fields, values[2:]))
    ...
    def store_results(self):
        ...
        with con:
            try:
                ...
                if self.symmetric and result[0] != result[1]:
                    con.execute(sql2, result)
                while 1:
                    result = yield
                    con.execute(sql1, result)
                    if self.symmetric and result[0] != result[1]:
