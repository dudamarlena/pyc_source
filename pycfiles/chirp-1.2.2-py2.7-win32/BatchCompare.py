...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\chirp\gui\BatchCompare.py
...
class CompareThread(threading.Thread):
    ...
    def run(self):
        ...
        data = ccompare.load_data(self.storager, self.comparator, self.mgr, self.consumer, nworkers=self.nworkers, cout=sys.stderr)
        self.consumer()
        self.storager.output_signals()
        self.consumer = StagedBatchConsumer(self.parent, 'Comparing signals...')
        nq = ccompare.run_comparisons(self.storager, self.comparator, data, self.mgr, self.consumer, nworkers=self.nworkers, cout=sys.stderr)
        if nq > 0:
            self.consumer()
...
class BatchCompare(wx.Frame):
    _inactive_controls = ('filedir', 'config', 'nworkers', 'method', 'storage', 'storageloc',
                          'storage_pick', 'restrict', 'skipcompl', 'btn_start')

    def __init__(self, parent=None):
        wx.Frame.__init__(self, parent, title='Batch Comparison', size=(400, 400), style=wx.MINIMIZE_BOX | wx.CAPTION | wx.CLOSE_BOX | wx.NO_FULL_REPAINT_ON_RESIZE)
    ...
    def create_main_panel(self):
        ...
        self.filedir = wx.DirPickerCtrl(mainPanel, -1, message='Select a directory', style=wx.DIRP_DIR_MUST_EXIST | wx.DIRP_USE_TEXTCTRL)
    ...
    def on_pick_storage(self, event):
        fdlg = wx.FileDialog(self, 'Select storage location', wildcard='All files (*.*)|*.*', style=wx.FD_SAVE)
        val = fdlg.ShowModal()
        if val == wx.ID_OK:
