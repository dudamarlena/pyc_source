...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\chirp\compare\file_storage.py
...
class file_storage(_base_storage):
    ...
    def _load_signals(self, signal_dir):
        ...
        self.signals = [ (i, f) for i, f in enumerate(iglob(os.path.join(signal_dir, '*' + self.file_pattern)))
                       ]
