...
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/CIDAN/LSSC/functions/temporal_correlation.py
# Compiled at: 2020-04-15 19:04:27
...
def calculate_temporal_correlation(dataset):
    shape = dataset.shape
    default_val = 0
    dataset_flat = array.from_array(dataset.transpose([1, 2, 0]).reshape([shape[1] * shape[2], -1]))
    ...
    del shift_right
    shift_left = np.dstack([dataset[:, :, :-1], np.full((shape[0], shape[1], 1), default_val)]).transpose([1, 2, 0]).reshape([shape[1] * shape[2], -1])
...
def test_func(default_val, dataset_flat, shape, dataset):
    shift_up = array.hstack([array.zeros((shape[0], 1, shape[2])), dataset[:, :-1, :]]).transpose([
