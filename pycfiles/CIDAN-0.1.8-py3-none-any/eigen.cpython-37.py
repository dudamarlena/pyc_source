...
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/CIDAN/LSSC/functions/eigen.py
...
def gen_eigen_vectors(*, K: np.ndarray, num_eig: int, spatial_box_num: int, temporal_box_num: int) -> np.ndarray:
    ...
    eig_values, eig_vectors_scaled = linalg.eigsh(P_transformed,
      num_eig, which='LM', return_eigenvectors=True)
    eig_vectors = np.flip((D_neg_sqrt * eig_vectors_scaled),
      axis=1)
    return eig_vectors


@delayed
def save_eigen_vectors(*, e_vectors, spatial_box_num: int, time_box_num: int, save_dir: str):
    eigen_dir = os.path.join(save_dir, 'eigen_vectors')
    if not os.path.isdir(eigen_dir):
        os.mkdir(eigen_dir)
    pickle_save(e_vectors, name=('eigen_vectors_box_{}_{}.pickle'.format(spatial_box_num, time_box_num)), output_directory=eigen_dir)
    return e_vectors
...
def load_eigen_vectors(*, spatial_box_num: int, time_box_num: int, save_dir: str):
    ...
    vector = pickle_load(name=name, output_directory=eigen_dir)
...
def create_embeding_norm_multiple(*, spatial_box_list, save_dir, num_time_steps):
    ...
    for e_vectors, spatial_box in zip(e_vectors_list, spatial_box_list):
        ...
        e_vectors_sum_rescaled = e_vectors_sum * (9.0 / e_vectors_sum.max())
        e_vectors_shaped = np.reshape(e_vectors_sum, spatial_box.shape[1:])
        eigen_images.append(e_vectors_shaped)

    image = combine_images(spatial_box_list, eigen_images)
    img = Image.fromarray(image * (3.0 / image.max()) * 255).convert('L')
