...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/socli/socli.py
# Compiled at: 2019-08-22 06:14:18
"""
# Stack overflow CLI
# Created by
...
"""
import getopt, os, sys, urllib, colorama, requests, urwid
from bs4 import BeautifulSoup
import random, re, urllib3
DEBUG = False
soqurl = 'http://stackoverflow.com/search?q='
...
def helpman():
    ...
    print ' ' + bold('--interactive or -i') + ' : To search in stack overflow and display the matching results. You can chose and browse any of the result interactively'
    ...
    print ' ' + bold('--tag or -t') + ' : To search a query by tag on stack overflow.  Visit http://stackoverflow.com/tags to see the list of all tags.\n   eg:- socli --tag javascript,node.js --query foo bar: Displays the search result of the query "foo bar" in stack overflow\'s javascript and node.js tags.'
    print ' ' + bold('--new or -n') + ' : Opens the stack overflow new questions page in your default browser. You can create a new question using it.'
    print ' ' + bold('--user or -u') + ' : Displays information about the user provided as the next argument(optional). If no argument is provided it will ask the user to enter a default username. Now the user can run the command without the argument.\n   eg:- socli -u: Prompts and saves your username. Now you can just run soci -u to see the stats \n        socli -u 22656: Displays info about user ID 22656'
    print ' ' + bold('--del or -d') + ' : Deletes the configuration file generated by socli -u command.'
    print ' ' + bold('--api or -a') + ' : Sets a custom API key for socli'
    print ' ' + bold('--sosearch or -s') + ' : SoCLI uses google search by default. Override this and use default stack overflow search.'
    print_header('\n\n< Search Query >:')
    print '\n Query to search on Stack overflow'
    print "\nIf no commands are specified then socli will search the stack overflow and simply displays the first search result's most voted answer."
...
def get_question_stats_and_answer(url):
    ...
    answers = [ s.get_text() for s in soup.find_all('div', class_='post-text') ][1:]
    if len(answers) == 0:
        answers.append('No answers for this question ...')
    return (
     question_title, question_desc, question_stats, answers)
...
def socli_interactive(query):
    ...
    class QuestionPage(urwid.WidgetWrap):
        ...
        def __init__(self, data):
            ...
            answer_frame = urwid.Frame(header=urwid.Pile([
             HEADER,
             QuestionText(question_title, question_desc, question_stats),
             urwid.Divider('-')]), body=self.answer_text, footer=urwid.Pile([
             QuestionURL(question_url),
             UnicodeText('↑: previous answer, ↓: next answer, o: open in browser, ←: back')]))
    ...
    class AnswerText(urwid.WidgetWrap):
        ...
        def set_answer(self):
            ...
            """
            self.content = [
             ('less-important', 'Answer: ')] + self.answers[self.index].split('\n')
    ...
    class QuestionText(UnicodeText):
        ...
        def __init__(self, title, description, stats):
            text = [
             'Question: ', ('title', title), description, ('metadata', stats)]
    ...
    class SelectQuestionPage(urwid.WidgetWrap):
        ...
        def __init__(self, questions):
            ...
            frame = urwid.Frame(header=header, body=urwid.Filler(self.questions_box, height=('relative',
                                                                                             100), valign='top'), footer=footer)
...
def fixGoogleURL(url):
    ...
    if 'http' not in url[:4]:
        url = 'https://' + url
    if not bool(re.search('/questions/[0-9]+', url)):
        return None
    else:
        return url
...
def captchacheck(url):
    ...
    :return:
    """
    if re.search('.com/nocaptcha', url):
                ...
                if opt in ('-u', '--user'):
                    ...
                    if len(rem) == 1:
                        try:
                            ...
                            print_warning('Wrong syntax. User ID must be an integer. Follow the instructions on this page to get your User ID: http://meta.stackexchange.com/a/111130\n')
                            helpman()
                            exit(1)
                        ...
                        try:
                            ...
                            del_datafile()
                            print_warning('Error in parsing the data file, it will be now deleted. Please rerun the socli -u command.')
                            exit(1)
                        except FileNotFoundError:
