...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyage/solutions/forams/environment.py
    ...
    def _initialize_grid(self):
        ...
        for i in range(self.size):
            for j in range(self.size):
                grid[i][j].neighbours.extend([ grid[x][y] for x in range(max(0, i - 1), min(self.size, i + 2)) for y in range(max(0, j - 1), min(self.size, j + 2)) if x != i or j != j
                                             ])
