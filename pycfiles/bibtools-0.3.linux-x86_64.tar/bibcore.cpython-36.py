...
# [GCC 8.4.0]
# Embedded file name: /a/lib/python3.6/site-packages/bibtools/bibcore.py
# Compiled at: 2017-04-03 10:08:28
...
import re, sys
from .util import *
__all__ = 'parse_name encode_name normalize_surname sniff_url classify_pub_ref doi_to_maybe_bibcode autolearn_pub print_generic_listing parse_search'.split()
...
def normalize_surname(name):
    ...
    return re.sub('\\.\\.+', '.', re.sub('[^a-z]+', '.', name.lower()))


_arxiv_re_1 = re.compile('^\\d\\d[01]\\d\\.\\d+')
_arxiv_re_2 = re.compile('^[a-z-]+/\\d+')
_bibcode_re = re.compile('^\\d\\d\\d\\d[a-zA-Z0-9&]+')
_doi_re = re.compile('^10\\.\\d+/.*')
_fasy_re = re.compile('.*\\.(\\d+|\\*)$')
...
def sniff_url(url):
    ...
    if url.startswith(p):
        return (
         'doi', urlunquote(url[len(p):]))
    p = 'http://adsabs.harvard.edu/abs/'
    if url.startswith(p):
        return (
         'bibcode', urlunquote(url[len(p):]))
    ...
    if url.startswith(p):
        ...
        if url.startswith(p):
            ...
            return (
             'bibcode', urlunquote(url[len(p):]))
        for p in ('http://arxiv.org/abs/', 'https://arxiv.org/abs/', 'http://arxiv.org/pdf/',
                  'https://arxiv.org/pdf/'):
            if url.startswith(p):
                return (
                 'arxiv', urlunquote(url[len(p):]))
