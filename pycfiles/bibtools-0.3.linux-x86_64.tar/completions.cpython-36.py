...
# [GCC 8.4.0]
# Embedded file name: /a/lib/python3.6/site-packages/bibtools/completions.py
# Compiled at: 2017-04-12 13:38:07
...
def print_dois(app, partial):
    for tup in app.db.execute('SELECT doi FROM pubs WHERE doi LIKE ?', (
     partial + '%',)):
        print(tup[0])


def print_bibcodes(app, partial):
    for tup in app.db.execute('SELECT bibcode FROM pubs WHERE bibcode LIKE ?', (
     partial + '%',)):
        print(tup[0])
...
def print_nfasys(app, partial, is_multi):
    if is_multi:
        for tup in app.db.execute('SELECT DISTINCT nfas FROM pubs WHERE nfas LIKE ?', (partial + '%',)):
            print(tup[0] + '.*')

    for tup in app.db.execute('SELECT DISTINCT nfas, year FROM pubs WHERE nfas LIKE ?', (partial + '%',)):
        print(tup[0] + '.' + str(tup[1]))

    if '.' in partial:
        nfas = partial.rsplit('.', 1)[0]
        need_print_star = is_multi
        for tup in app.db.execute('SELECT DISTINCT year FROM pubs WHERE nfas == ?', (
         nfas,)):
            if need_print_star:
                print(nfas + '.*')
...
def print_arxivs(app, partial):
    for tup in app.db.execute('SELECT arxiv FROM pubs WHERE arxiv LIKE ?', (
     partial + '%',)):
        print(tup[0])


def print_nicknames(app, partial):
    for tup in app.db.execute('SELECT nickname FROM nicknames WHERE nickname LIKE ?', (
     partial + '%',)):
        print(tup[0])
...
def complete_group(app, tool, args):
    if not len(args) or not len(args[0]):
        for tup in app.db.execute('SELECT DISTINCT name FROM publists WHERE name LIKE "user_%"'):
            print(tup[0][5:])

        return
    partial = args[0]
    for tup in app.db.execute('SELECT DISTINCT name FROM publists WHERE name LIKE ?', (
     'user_' + partial + '%',)):
        print(tup[0][5:])