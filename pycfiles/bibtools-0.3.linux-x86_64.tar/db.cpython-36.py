...
# [GCC 8.4.0]
# Embedded file name: /a/lib/python3.6/site-packages/bibtools/db.py
# Compiled at: 2017-03-31 15:02:49
# Size of source mod 2**32: 9328 bytes
"""
The main database.
"""
...
PubRow = collections.namedtuple('PubRow', 'id abstract arxiv bibcode doi keep nfas refdata title year'.split())
AuthorNameRow = collections.namedtuple('AuthorNameRow', [
 'name'])
AuthorRow = collections.namedtuple('AuthorRow', 'type pubid idx authid'.split())
HistoryRow = collections.namedtuple('HistoryRow', 'date pubid action'.split())
NicknameRow = collections.namedtuple('NicknameRow', 'nickname pubid'.split())
PdfRow = collections.namedtuple('PdfRow', 'sha1 pubid'.split())
authtypes = {'author':0, 
 'editor':1}
...
class BibDB(sqlite3.Connection):
    ...
    def get_pub_authors(self, pubid, authtype='author'):
        authtype = authtypes[authtype]
        return (parse_name(a[0]) for a in self.execute('SELECT name FROM authors AS au, author_names AS an WHERE au.type == ? AND au.authid == an.oid   AND au.pubid == ? ORDER BY idx', (
         authtype, pubid)))
    ...
    def get_pub_fas(self, pubid):
        ...
        the 'pubs' table."""
        for t in self.execute('SELECT name FROM authors AS au, author_names AS an WHERE au.type == ? AND au.authid == an.oid   AND au.pubid == ? AND idx == 0', (
         authtypes['author'], pubid)):
            return parse_name(t[0])[1]

    def choose_pub_nickname(self, pubid):
        n = list(self.execute('SELECT nickname FROM nicknames WHERE pubid == ? ORDER BY length(nickname) ASC LIMIT 1', (
         pubid,)))
        if not len(n):
            return
        else:
    ...
    def _fill_pub(self, info, pubid):
        ...
        if 'abstract' in info:
            ...
            if pubid is not None:
                c.execute('UPDATE pubs SET abstract=?, arxiv=?, bibcode=?,   doi=?, keep=?, nfas=?, refdata=?, title=?, year=? WHERE id == ?', row[1:] + (pubid,))
            else:
    ...
    def jsonify_pub(self, pubid):
        ...
        info['doi'] = pub.doi
        info['year'] = pub.year
        info['nicknames'] = list(self.execute('SELECT nickname FROM nicknames WHERE pubid == ?', (
         pubid,)))
