...
# [GCC 8.4.0]
# Embedded file name: /a/lib/python3.6/site-packages/bibtools/__init__.py
# Compiled at: 2017-04-12 13:41:01
# Size of source mod 2**32: 7649 bytes
"""
...
class BibApp(object):
            ...
            if kind == 'doi':
                ...
                if kind == 'bibcode':
                    ...
                    if kind == 'arxiv':
                        ...
                        if kind == 'nickname':
                            q = self.db.pub_fquery('SELECT p.* FROM pubs AS p, nicknames AS n WHERE p.id == n.pubid AND n.nickname = ?', text)
                            ...
                            if kind == 'lastlisting':
                                try:
                                    ...
                                    raise PubLocateError('pub names starting with %% should be followed by positive numbers, but got "%s"', text)

                                q = self.db.pub_fquery('SELECT p.* FROM pubs AS p, publists AS l WHERE p.id == l.pubid AND l.name = ? AND l.idx = ?', 'last_listing', idx)
    ...
    def try_get_pdf(self, pub):
        ...
        sha1 = try_fetch_pdf((self.proxy), temppath, arxiv=(pub.arxiv), bibcode=(pub.bibcode),
          doi=(pub.doi))
        if sha1 is None:
            return
        else:
    ...
    def rsync_backup(self):
        ...
        try:
            subprocess.check_call(fullargs, close_fds=True,
              shell=False,
              cwd=(bibpath()))
        except Exception:
