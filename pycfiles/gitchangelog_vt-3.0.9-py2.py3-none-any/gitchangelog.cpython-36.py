...
# [GCC 8.4.0]
# Embedded file name: C:\Users\grodriguez\Downloads\gitchangelog\src\gitchangelog\gitchangelog.py
# Compiled at: 2020-03-10 09:38:38
# Size of source mod 2**32: 68809 bytes
from __future__ import print_function
from __future__ import absolute_import
import locale, re, os, os.path, sys, glob, textwrap, datetime, collections, traceback, contextlib, itertools, errno, importlib
from subprocess import Popen, PIPE
from numpy import unicode
sys.path.append('gitchangelog')
try:
    import parameters_to_use
except ImportError:
    from gitchangelog import parameters_to_use

try:
    import command_cmd_lines
except ImportError:
    from gitchangelog import command_cmd_lines

try:
    from modules import vt_logic
except ImportError:
    from gitchangelog.modules import vt_logic

try:
    from modules import default_methods
except ImportError:
    from gitchangelog.modules import default_methods
...
if WIN32:
    if not PY3:
        import ctypes, subprocess, _subprocess
        ...
        """
            si = STARTUPINFOW(dwFlags=(startup_info.dwFlags),
            ...
            wenv = None
            if env is not None:
                env = unicode('').join([unicode('%s=%s\x00') % (k, v) for k, v in env.items()]) + unicode('\x00')
                wenv = c_wchar * len(env)()
                wenv.value = env
            pi = PROCESS_INFORMATION()
            creation_flags |= CREATE_UNICODE_ENVIRONMENT
            if CreateProcessW(executable, args, None, None, inherit_handles, creation_flags, wenv, cwd, byref(si), byref(pi)):
                return (
        ...
        class Popen(subprocess.Popen):
            __doc__ = 'This superseeds Popen and corrects a bug in cPython 2.7 implem'
            ...
            def _execute_child(self, args, executable, preexec_fn, close_fds, cwd, env, universal_newlines, startupinfo, creationflags, shell, to_close, p2cread, p2cwrite, c2pread, c2pwrite, errread, errwrite):
                ...
                super(Popen, self)._execute_child(args, executable, preexec_fn, close_fds, cwd, env, universal_newlines, startupinfo, creationflags, False, to_close, p2cread, p2cwrite, c2pread, c2pwrite, errread, errwrite)


        _subprocess.CreateProcess = CreateProcess
else:
    usage_msg = '\n  %(exname)s {-h|--help}\n  %(exname)s {-v|--version}\n  %(exname)s [--debug|-d] [REVLIST]\n  %(exname)s {-c|--clean}\n  %(exname)s {-t|--title}\n  %(exname)s {-s|--show}\n  %(exname)s {-u|--url}\n  %(exname)s {-f|--file}\n  %(exname)s {-tm|--template}\n  %(exname)s {-m|--module}'
    description_msg = 'Run this command in a git repository to output a formatted changelog\n'
    epilog_msg = "%(exname)s uses a config file to filter meaningful commit or do some\nformatting in commit messages thanks to a config file.\n\nConfig file location will be resolved in this order:\n  - in shell environment variable GITCHANGELOG_CONFIG_FILENAME\n  - in git configuration: ``git config gitchangelog.rc-path``\n  - as '.%(exname)s.rc' in the root of the current git repository\n\n"
    ...
    def format_last_exception(prefix='  | '):
        r"""Format the last exception for display it in tests.
    ...
    """
        ...
        return '\n'.join('\n'.join(textwrap.wrap(paragraph.strip())) for paragraph in regexp.split(text)).strip()
    ...
    for _label in ('Indent', 'Wrap', 'ReSub', 'noop', 'final_dot', 'ucfirst', 'strip',
                   'SetIfEmpty'):
    ...
    def file_get_contents(filename):
        ...
        if not PY3:
            ...
            out = re.sub('^(\\s*#.*\\s*)coding[:=]\\s*([-\\w.]+\\s*;?\\s*)', '\\1', out, re.DOTALL)
        return out
    ...
    def _file_regex_match(filename, pattern, **kw):
        ...
        if match is None:
            ...
            raise ValueError("Regex %s did not match any substring in '%s'." % (
             pattern, filename))
        return match
    ...
    def FileFirstRegexMatch(filename, pattern):
        ...
        def _call():
            ...
            if dct:
                if 'rev' not in dct:
                    warn("Named pattern used, but no one are named 'rev'. Using full match.")
    ...
    class Proc(Popen):
        ...
        def __init__(self, command, env=None, encoding=_preferred_encoding):
            super(Proc, self).__init__(command,
    ...
    def cmd(command, env=None, shell=True):
        p = Popen(command, shell=shell, stdin=PIPE,
          stdout=PIPE,
          stderr=PIPE,
          close_fds=(PLT_CFG['close_fds']),
          env=env,
    ...
    class GitCommit(SubGitObjectMixin):
        ...
        def __getattr__(self, label):
            ...
            if missing_attrs:
                ...
                try:
                    ...
                    if DEBUG:
                        raise
                    raise ValueError("Given commit identifier %r doesn't exists" % self.identifier)
            ...
            if match is not None:
                ...
                for match in re.finditer(REGEX_RFC822_KEY_VALUE, postfix):
                    ...
                    try:
                        ...
                        setattr(self, 'trailer_%s' % key, prev_value + [value] if isinstance(prev_value, list) else [
                         prev_value, value])
        ...
        def author_names(self):
            return [re.sub('^([^<]+)<[^>]+>\\s*$', '\\1', author).strip() for author in self.authors]

        @property
        def authors(self):
            co_authors = getattr(self, 'trailer_co_authored_by', [])
            co_authors = co_authors if isinstance(co_authors, list) else [
             co_authors]
        ...
        def tagger_date_timestamp(self):
            ...
            tagger_date_utc = self.git.for_each_ref(('refs/tags/%s' % self.identifier),
    ...
    class GitRepos(object):
        ...
        def __init__(self, path):
            ...
            try:
                ...
                if DEBUG:
                    raise
                raise EnvironmentError('Required ``git`` command not found or broken in $PATH. (calling ``git version`` failed.)')
            ...
            try:
                ...
                if DEBUG:
                    raise
                raise EnvironmentError('Not in a git repository. (calling ``git remote`` failed.)')
        ...
        def log(self, includes=[
 'HEAD'], excludes=[], include_merge=True, encoding=_preferred_encoding):
    ...
    """
        try:
            ...
            stderr("Error parsing git config: %s. Won't be able to read 'template-path' if defined." % str(e))
            template_path = None
        ...
        if template_path:
            ...
            path_file = os.getcwd()
            path_label = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'templates', label)
        ...
        if len(templates) > 0:
            ...
            msg += '\n - ' + '\n - '.join(os.path.basename(f).split('.')[0] for f in templates)
...
def FileRegexSubst(filename, pattern, replace, flags=0):
    replace = re.sub('\\\\([0-9+])', '\\\\g<\\1>', replace)

    def _wrapped(content):
        src = file_get_contents(filename)
        src = re.sub(pattern,
          (replace.replace('\\o', ''.join(content).replace('\\', '\\\\'))),
          src,
          flags=flags)
        if not PY3:
...
def versions_data_iter(repository, revlist=None, ignore_regexps=[], section_regexps=[
 (None, '')], tag_filter_regexp='\\d+\\.\\d+(\\.\\d+)?', include_merge=True, body_process=lambda x: x, subject_process=lambda x: x, log_encoding=DEFAULT_GIT_LOG_ENCODING, warn=warn, jira_url='', clean=None, title='', show=None, url='', file='', template='', module=''):
    ...
    module = module or []
    versions_done = {}
    excludes = [rev[1:] for rev in repository.git.rev_parse(['--rev-only'] + revlist + ['--']).split('\n') if rev.startswith('^')] if revlist else []
    ...
    if revlist:
        ...
        if module:
            try:
                module_import = import_from('modules', module)
            except Exception:
                try:
                    module_import = import_from('gitchangelog.modules', module)
                except Exception:
                    print("¡Ups! You are trying to access a module that doesn't exist. Please verify your request.")
                    exit()

        else:
            module_import = vt_logic
        var_to_show = []
        if revlist:
    ...
    for idx, tag in enumerate(tags):
        current_version = {'date':tag.tagger_date if tag.has_annotated_tag else tag.date, 
         ...
         'commit':tag, 
         'solicited_requests':validation_request_filter(module_import, var_to_show)}
        sections = collections.defaultdict(list)
        commits = repository.log(includes=[
         min(tag, max_rev)],
          excludes=(tags[idx + 1:] + excludes),
          include_merge=include_merge,
          encoding=log_encoding)
        list_tgs = []
        if repository.git.tag():
            if revlist:
                log = repository.git.log([' '.join(revlist)] + ['--simplify-by-decoration'] + [
                 '--format=format: %H-%D'])
                split_tgs = log.split('\n')
            else:
                log = repository.git.log(['--simplify-by-decoration'] + [
                 '--format=format: %H-%D'])
                split_tgs = log.split('\n')
            for item in split_tgs:
                if item.find('tag:') != -1:
                    list_tgs.append(item.split('tag:'))

        for commit in commits:
            tgs = False
            for i in list_tgs:
                commit_tgs = i[0][:-1].lstrip()
                if commit_tgs.find(commit.sha1) != -1:
                    tgs_split = i[1].split(',')
                    tgs = tgs_split[0]

            commit_filter = [commit.subject]
            if any(re.search(pattern, commit.subject) is not None for pattern in ignore_regexps):
                pass
            else:
                matched_section = first_matching(section_regexps, commit.subject)
                template_data = {'id':commit.sha1_short, 
                 'date':commit.date, 
                 ...
                 'jira_url':jira_url, 
                 'tags':tgs}
                if 'send_parser' in dir(module_import):
                    final_data_dictionary = module_import.send_parser(commit, repository, jira_url, template_data)
                else:
                    final_data_dictionary = default_methods.send_parser(commit, repository, jira_url, template_data)
                if clean:
                    for item in commit_filter:
                        find_commit = True
                        for i in clean:
                        ...
                        if find_commit:
                            sections[matched_section].append(final_data_dictionary)
        ...
        current_version['sections'] = [{'label':k,  'commits':sections[k]} for k in section_order if k in sections]
        if len(current_version['sections']) != 0:
            yield current_version
        versions_done[tag] = current_version


def validation_request_filter(module_import, var_to_show):
    if 'see_request_filter' in dir(module_import):
        return module_import.see_request_filter(var_to_show)
    else:
        return default_methods.see_request_filter(var_to_show)
...
def changelog(title, output_engine=mustache, unreleased_version_label='unreleased', warn=warn, **kwargs):
...
def obsolete_replace_regexps(config):
    ...
    if 'replace_regexps' in config:
        for pattern, replace in config['replace_regexps'].items():
            config['subject_process'] = ReSub(pattern, replace) | config.get('subject_process', ucfirst | final_dot)
...
def obsolete_body_split_regexp(config):
    ...
    if 'body_split_regex' in config:
        config['body_process'] = Wrap(config['body_split_regex']) | config.get('body_process', noop)
    ...
    if opts.revlist:
        ...
        if revs:
            ...
            if not isinstance(revs, list):
                die("Invalid type for 'revs' in config file. A 'list' type is required, and a %r was given." % type(revs).__name__)
            revs = [eval_if_callable(rev) for rev in revs]
        else:
            revs = []
    for rev in revs:
        if not isinstance(rev, basestring):
            die("Invalid type for revision in revs list from config file. 'str' type is required, and a %r was given." % type(rev).__name__)
        try:
...
def get_log_encoding(repository, config):
    ...
    if log_encoding is None:
        try:
            ...
            warn("Error parsing git config: %s. Couldn't check if 'i18n.logOuputEncoding' was set." % str(e))
...
def safe_print(content):
    ...
    try:
        ...
        if e.errno == 0:
            ...
            if WIN32 and e.errno == 22 or not WIN32 and e.errno == errno.EPIPE:
                if PY3:
                    try:
                        sys.stdout.close()
...
def main():
    global DEBUG
    reference_config = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'gitchangelog.rc.reference')
    ...
    opts = command_cmd_lines.parse_cmd_line(usage=(i(usage_msg)), description=(i(description_msg)),
      epilog=(i(epilog_msg)),
      exname=basename,
      version=__version__)
    ...
    try:
        ...
        stderr("Error parsing git config: %s. Won't be able to read 'rc-path' if defined." % str(e))
        gc_rc = None
    ...
    for enforce_file_existence, fun in [
     ...
     (
      False,
      lambda : os.path.join(repository.toplevel, '.%s.rc' % basename) if not repository.bare else None)]:
        changelogrc = fun()
        if changelogrc:
    ...
    clean = parameters_to_use.parameter_to_clean(opts)
    title = parameters_to_use.change_title(opts)
    show = parameters_to_use.parameter_to_show(opts)
    url = parameters_to_use.change_jira_url(opts, jira_url=(config['url']))
    file = parameters_to_use.give_file_name(opts)
    module = parameters_to_use.module_implementation(opts)
    config['unreleased_version_label'] = eval_if_callable(config['unreleased_version_label'])
    manage_obsolete_options(config)
    if opts.template:
        parameters_to_use.change_template(opts, config)
    try:
        content = changelog(repository=repository,
          revlist=revlist,
          clean=clean,
          title=title,
          show=show,
          url=url,
          file=file,
          template=(parameters_to_use.change_template(opts, config)),
          module=module,
        ...
        if DEBUG:
            ...
            message = '%s' % e
            err(message)
            stderr('  (set %s environment variable, or use ``--debug`` to see full traceback)' % (
             debug_varname,))
        exit(255)
