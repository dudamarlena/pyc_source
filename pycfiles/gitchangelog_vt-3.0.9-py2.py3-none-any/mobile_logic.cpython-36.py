...
# [GCC 8.4.0]
# Embedded file name: C:\Users\grodriguez\Downloads\gitchangelog\src\gitchangelog\modules\mobile_logic.py
# Compiled at: 2020-03-10 09:44:49
# Size of source mod 2**32: 4832 bytes
import re, pystache, dateutil.parser
...
def change_date_format(commit):
    date = commit.date
    final_date = dateutil.parser.parse(date)
    final_date_format = final_date.strftime('%Y/%m/%d')
    return final_date_format


def send_parser(commit, repository, jira_url, template_data):
    commit_message(commit, jira_url)
    template_data['date'] = change_date_format(commit)
