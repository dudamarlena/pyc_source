...
# [GCC 8.4.0]
# Embedded file name: /opt/adagios/adagios/../adagios/context_processors.py
# Compiled at: 2018-05-16 10:07:32
import pynag.Model, os, getpass
...
import pynag.Model.EventHandlers, pynag.Parsers
from pynag.Parsers import Livestatus
import adagios, adagios.status.utils
...
def get_unhandled_problems(request):
    ...
    try:
        livestatus = adagios.status.utils.livestatus(request)
        num_problems = livestatus.query('GET services', 'Filter: state != 0', 'Filter: acknowledged = 0', 'Filter: host_acknowledged = 0', 'Filter: scheduled_downtime_depth = 0', 'Filter: host_scheduled_downtime_depth = 0', 'Stats: state != 0', 'Stats: host_state != 0', columns=False)
        results['num_problems'] = num_problems[0] + num_problems[1]
        results['num_unhandled_problems'] = num_problems[0] + num_problems[1]
        result = livestatus.query('GET services', 'Stats: state != 0', 'Stats: state != 0', 'Stats: acknowledged = 0', 'Stats: scheduled_downtime_depth = 0', 'Stats: host_state = 0', 'StatsAnd: 4', columns=False)
        num_service_problems_all = result[0]
        num_service_problems_unhandled = result[1]
        result = livestatus.query('GET hosts', 'Stats: state != 0', 'Stats: state != 0', 'Stats: acknowledged = 0', 'Stats: scheduled_downtime_depth = 0', 'Stats: host_state = 1', 'StatsAnd: 4', columns=False)
...
def check_destination_directory(request):
    ...
    if not dest_dir_was_found:
        add_notification(level='warning', notification_id='dest_dir', message=_('Destination for new objects (%s) is not defined in nagios.cfg') % dest)
    elif not os.path.isdir(dest):
        add_notification(level='warning', notification_id='dest_dir', message=_('Destination directory for new objects (%s) is not found. Please create it.') % dest)
    else:
...
def get_user_preferences(request):
    ...
    def theme_to_themepath(theme):
        return os.path.join(settings.THEMES_FOLDER, theme, settings.THEME_ENTRY_POINT)
...
def get_all_nonworking_backends(request):
    ...
    b = [ x for x in get_all_backends(request)['backends'] if not Livestatus(x).test(raise_error=False)
        ]
