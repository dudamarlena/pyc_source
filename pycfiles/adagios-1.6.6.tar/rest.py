...
# [GCC 8.4.0]
# Embedded file name: /opt/adagios/adagios/../adagios/status/rest.py
...
"""
import time, pynag.Control.Command, pynag.Model, pynag.Utils, adagios.status.utils, pynag.Parsers, collections
...
def acknowledge_many(hostlist, servicelist, sticky=1, notify=1, persistent=0, author='adagios', comment='acknowledged by Adagios'):
    ...
    for i in items:
        acknowledge(host_name=i[0], service_description=i[1], sticky=sticky, notify=notify, persistent=persistent, author=author, comment=comment)
...
def acknowledge(host_name, service_description=None, sticky=1, notify=1, persistent=0, author='adagios', comment='acknowledged by Adagios'):
    ...
    if service_description in (None, '', '', '_HOST_'):
        pynag.Control.Command.acknowledge_host_problem(host_name=host_name, sticky=sticky, notify=notify, persistent=persistent, author=author, comment=comment)
    else:
        pynag.Control.Command.acknowledge_svc_problem(host_name=host_name, service_description=service_description, sticky=sticky, notify=notify, persistent=persistent, author=author, comment=comment)
    return
...
def downtime_many(hostlist, servicelist, hostgrouplist, start_time=None, end_time=None, fixed=1, trigger_id=0, duration=7200, author='adagios', comment='Downtime scheduled by adagios', all_services_on_host=False, hostgroup_name=None):
    ...
    for i in items:
        ...
        hostgroup_name = i[2]
        downtime(host_name=host_name, service_description=service_description, start_time=start_time, end_time=end_time, fixed=fixed, trigger_id=trigger_id, duration=duration, author=author, comment=comment, all_services_on_host=all_services_on_host, hostgroup_name=hostgroup_name)
...
def downtime(host_name=None, service_description=None, start_time=None, end_time=None, fixed=1, trigger_id=0, duration=7200, author='adagios', comment='Downtime scheduled by adagios', all_services_on_host=False, hostgroup_name=None):
    ...
    if hostgroup_name:
        result1 = (
         pynag.Control.Command.schedule_hostgroup_host_downtime(hostgroup_name=hostgroup_name, start_time=start_time, end_time=end_time, fixed=fixed, trigger_id=trigger_id, duration=duration, author=author, comment=comment),)
        result2 = pynag.Control.Command.schedule_hostgroup_svc_downtime(hostgroup_name=hostgroup_name, start_time=start_time, end_time=end_time, fixed=fixed, trigger_id=trigger_id, duration=duration, author=author, comment=comment)
        return (
         result1, result2)
    else:
        if all_services_on_host:
            result1 = (
             pynag.Control.Command.schedule_host_svc_downtime(host_name=host_name, start_time=start_time, end_time=end_time, fixed=fixed, trigger_id=trigger_id, duration=duration, author=author, comment=comment),)
            result2 = pynag.Control.Command.schedule_host_downtime(host_name=host_name, start_time=start_time, end_time=end_time, fixed=fixed, trigger_id=trigger_id, duration=duration, author=author, comment=comment)
            return (
             result1, result2)
        else:
            if service_description in (None, '', '', '_HOST_'):
                return pynag.Control.Command.schedule_host_downtime(host_name=host_name, start_time=start_time, end_time=end_time, fixed=fixed, trigger_id=trigger_id, duration=duration, author=author, comment=comment)
            return pynag.Control.Command.schedule_svc_downtime(host_name=host_name, service_description=service_description, start_time=start_time, end_time=end_time, fixed=fixed, trigger_id=trigger_id, duration=duration, author=author, comment=comment)
...
def reschedule(request, host_name=None, service_description=None, check_time=None, wait=0, hostlist='', servicelist=''):
    ...
    if service_description in (None, '', '', '_HOST_', 'undefined'):
        service_description = ''
        ...
        if wait == '1':
            ...
            livestatus.query('GET hosts', 'WaitObject: %s ' % host_name, 'WaitCondition: last_check > %s' % check_time, 'WaitTrigger: check', 'Filter: host_name = %s' % host_name)
    else:
        ...
        if wait == '1':
            livestatus = adagios.status.utils.livestatus(request)
            livestatus.query('GET services', 'WaitObject: %s %s' % (
             host_name, service_description), 'WaitCondition: last_check > %s' % check_time, 'WaitTrigger: check', 'Filter: host_name = %s' % host_name)
    return 'ok'
...
def delete_comment(comment_id, object_type=None, host_name=None, service_description=None):
    ...
    if not host_name:
        pass
    if object_type == 'host' or service_description in (None, '', '', '_HOST_'):
...
def state_history(request, start_time=None, end_time=None, object_type=None, host_name=None, service_description=None, hostgroup_name=None):
    ...
    log_entries = adagios.status.utils.get_state_history(request, start_time=start_time, end_time=end_time, host_name=host_name, service_description=service_description)
...
def metrics(request, **kwargs):
    ...
    for service in services:
        ...
        for metric in metrics:
            metric_dict = {'host_name': service['host_name'], 'service_description': service['description'], 
               'state': service['state'], 
...
def metric_names(request, **kwargs):
    ...
    result = {'services that match filter': len(services), 'filter': kwargs, 
