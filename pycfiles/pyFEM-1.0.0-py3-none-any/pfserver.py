...
# Embedded file name: pyfeld/pfserver.py
...
import argparse, curses, html.parser, json, logging, mimetypes, re, requests, telnetlib, threading, time
list_of_actions = []
...
class SubscriptionHandler:
    ...
    def create_new_subscription(self, local_ip, port, net_location, service, udn):
        ...
        headers = {'Host': net_location, 'Callback': callback_url, 
           'NT': 'upnp:event', 
...
def tellme_search_pathes():
    return [
     'album', 'my music', 'artist', 'composer', 'all']
...
def search(origin, where, name):
    if name == '':
        ...
        return (
         '[]', ("Sorry! Couldn't find {0} in {1}").format(name, origin))
...
def search_and_play(origin, where, name, room):
    ...
    if len(real_json) > 0:
        if real_json[0]['class'] in playAbleItems():
            ...
            return (
             jsonResult, ('Es spielt jetzt {0}').format(real_json[0]['title']))
            ...
            if len(real_json) > 0:
                if real_json[0]['class'] in playAbleItems():
                    ...
                    return (
                     jsonResult, ('Es spielt jetzt  {0}').format(real_json[0]['title']))
    return (
     '[]', ('Folgendes konnte Raumfeld nicht finden: {0} in {1}').format(name, origin))
...
def songs_list(what, onebased_index):
    ...
    if what in ('mostplayed', 'lieblings musik'):
...
def play(what, onebased_index, room_name):
    ...
    return (
     '[]', ("Sorry! Couldn't play {0} {1} in room {2}").format(what, onebased_index, room_name))
...
def play_at(time_point, what, index):
    ...
    return (
     '[]', ('Ok! Um {} spiele ich dann {} {}, aber bitte ... nicht erschrecken!').format(time_point, what, int(index) + 1))
...
def handle_room(room_name):
    ...
    return (
     json.dumps(room_dict), textresult)
...
def call_forwarder(host, port):
    ...
    while running:
        ...
        try:
            ...
            tn.read_until('login:')
            tn.write('#id scjurgen\n')
            tn.read_until('password:')
            tn.write('#pwd bogus\n')
            tn.read_until('ok')
            while True:
                print 'waiting for data'
                received_data = tn.read_until('\n')
