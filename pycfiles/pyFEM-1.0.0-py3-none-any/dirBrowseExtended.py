# uncompyle6 version 3.6.7
# Python bytecode 2.7 (62211)
# Decompiled from: Python 3.8.2 (tags/v3.8.2:7b3ab59, Feb 25 2020, 23:03:10) [MSC v.1916 64 bit (AMD64)]
# Embedded file name: pyfeld/dirBrowseExtended.py
# Compiled at: 2017-11-23 08:41:51
from __future__ import unicode_literals
import json, subprocess

def rf_command_line():
    return b'pyfeld --json browse'


class DirLevel:

    def __init__(self, path, friendly_name, items):
        self.path = path
        self.friendly_name = friendly_name
        self.items = items


class DirBrowseExtended:

    def __init__(self):
        dNull = DirLevel(b'0', b'services', self.retrieve(b'0'))
        self.path = b'0'
        self.pathes = [b'0']
        self.dirs = [dNull]
        self.depth = 0
        self.retrieve(self.pathes[self.depth])

    def get_current_path(self):
        return self.path

    def enter(self, index):
        self.path = self.dirs[self.depth].items[index][b'idPath']
        items = self.retrieve(self.path)
        new_dir = DirLevel(self.path, self.dirs[self.depth].items[index][b'title'], items)
        self.depth += 1
        if len(self.dirs) <= self.depth:
            self.dirs.append(new_dir)
        else:
            self.dirs[self.depth] = new_dir

    def enter_by_friendly_name(self, name):
        pass

    def leave(self):
        if self.depth != 0:
            self.depth -= 1
        self.path = self.dirs[self.depth].path

    def retrieve(self, path):
        command = rf_command_line()
        if type(path).__name__ == b'bytes':
            command += b' "' + path.decode(b'utf-8') + b'"'
        else:
            command += b' "' + path + b'"'
        try:
            process = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        except Exception as e:
            return 0

        jsonFile = b''
        while True:
            nextline = process.stdout.readline()
            if len(nextline) == 0 and process.poll() != None:
                break
            jsonFile += nextline.decode(b'utf-8')

        return json.loads(jsonFile)

    def get_friendly_path_name(self, separator=b' -> '):
        s = b''
        for i in range(1, self.depth + 1):
            s += self.dirs[i].friendly_name + separator

        return s[:-len(separator)]

    def get_friendly_name(self, index):
        return self.dirs[self.depth].items[index][b'title']

    def get_path_for_index(self, index):
        return self.dirs[self.depth].items[index][b'idPath']

    def get_type(self, index):
        return self.dirs[self.depth].items[index][b'class']

    def get_albumarturi(self, index):
        return self.dirs[self.depth].items[index][b'albumarturi']

    def get_album(self, index):
        return self.dirs[self.depth].items[index][b'album']

    def get_artist(self, index):
        return self.dirs[self.depth].items[index][b'artist']

    def get_class(self, index):
        return self.dirs[self.depth].items[index][b'class']

    def get_resourceName(self, index):
        return self.dirs[self.depth].items[index][b'resSourceName']

    def get_resourceType(self, index):
        return self.dirs[self.depth].items[index][b'resSourceType']

    def max_entries_on_level(self):
        return len(self.dirs[self.depth].items)


if __name__ == b'__main__':
    db = DirBrowseExtended()
    db.enter(1)
    db.enter(1)
    print str(db.dirs[2].items[0])
    for item in db.dirs[2].items:
        print (
         item[b'title'].encode(b'utf-8'),
         item[b'class'].encode(b'utf-8'),
         item[b'resSourceName'].encode(b'utf-8'),
         item[b'album'].encode(b'utf-8'),
         item[b'albumarturi'].encode(b'utf-8'),
         item[b'artist'].encode(b'utf-8'))