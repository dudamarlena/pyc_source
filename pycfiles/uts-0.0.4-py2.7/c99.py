...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-intel/egg/uts/c99.py
# Compiled at: 2016-10-31 00:48:55
...
class C99:
    ...
    def segment(self, document):
        ...
        if len(document) < 3:
            return [
             1] + [ 0 for _ in range(len(document) - 1) ]
        n = len(document)
        ...
        for i in range(n):
            for j in range(i, n):
                if i == 0:
                    ...
                    self.sm[i][j] = prefix_sm[j][j] - prefix_sm[(i - 1)][j] - prefix_sm[j][(i - 1)] + prefix_sm[(i - 1)][(i - 1)]
