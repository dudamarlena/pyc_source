...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/twapi_connection/__init__.py
# Compiled at: 2016-11-04 09:36:30
# Size of source mod 2**32: 6975 bytes
try:
...
class Connection:
    ...
    def send_get_request(self, url, query_string_args=None):
        ...
        :return: Decoded version of the ``JSON`` the remote put in                 the body of the response.
    ...
    def send_post_request(self, url, body_deserialization=None):
        ...
        :param str url: The URL or URL path to the endpoint
        :param dict body_deserialization: The request's body message             deserialized

        :return: Decoded version of the ``JSON`` the remote put in                 the body of the response.
        """
        return self._send_request('POST', url, body_deserialization=body_deserialization)
    ...
    def send_put_request(self, url, body_deserialization):
        ...
        :return: Decoded version of the ``JSON`` the remote put in                 the body of the response.
        """
        return self._send_request('PUT', url, body_deserialization=body_deserialization)
    ...
    def _send_request(self, method, url, query_string_args=None, body_deserialization=None):
        if url.startswith(self._api_url):
            url = url
        else:
            url = self._api_url + url
        query_string_args = query_string_args or {}
        request_headers = {'content-type': 'application/json'} if body_deserialization else {}
        if body_deserialization:
            request_body_serialization = json_serialize(body_deserialization)
        else:
            request_body_serialization = None
        response = self._session.request(method, url, params=query_string_args, auth=self._authentication_handler, data=request_body_serialization, headers=request_headers, timeout=self._timeout)
    ...
    def _require_deserializable_response_body(cls, response):
        if response.status_code in (HTTPStatus.OK, HTTPStatus.NO_CONTENT):
            ...
            exception_message = 'Unsupported response status {}'.format(response.status_code)
    ...
    def _require_json_response(response):
        ...
        if content_type != 'application/json':
            exception_message = 'Unsupported response content type {}'.format(content_type)
            raise UnsupportedResponseError(exception_message)
