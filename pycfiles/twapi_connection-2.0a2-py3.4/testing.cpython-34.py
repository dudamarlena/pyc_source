...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/twapi_connection/testing.py
# Compiled at: 2016-10-24 08:57:26
# Size of source mod 2**32: 5274 bytes
from pyrecord import Record
APICall = Record.create_type('APICall', 'url', 'http_method', 'query_string_args', 'request_body_deserialization', query_string_args=None, request_body_deserialization=None)
...
class MockConnection(object):
    __doc__ = 'Mock representation of a :class:`~twapi.Connection`'
    ...
    def send_post_request(self, url, body_deserialization=None):
        return self._call_remote_method(url, 'POST', request_body_deserialization=body_deserialization)

    def send_put_request(self, url, body_deserialization):
        return self._call_remote_method(url, 'PUT', request_body_deserialization=body_deserialization)

    def send_delete_request(self, url):
        return self._call_remote_method(url, 'DELETE')

    def _call_remote_method(self, url, http_method, query_string_args=None, request_body_deserialization=None):
        self._require_enough_api_calls(url)
        expected_api_call = self._expected_api_calls[self._request_count]
        _assert_request_matches_api_call(expected_api_call, url, http_method, query_string_args, request_body_deserialization)
    ...
    def _require_enough_api_calls(self, url):
        ...
        error_message = 'Not enough API calls for new requests (requested {!r})'.format(url)
...
def _assert_request_matches_api_call(api_call, url, http_method, query_string_args, request_body_deserialization):
