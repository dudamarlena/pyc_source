...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\PyAndor\_Shamrock.py
# Compiled at: 2018-01-18 05:28:16
import numpy as np
from ctypes import WinDLL, c_int, c_float, c_char_p, byref, POINTER
...
class ShamRockErrors():
    ...
    ERROR_CODE = {20201: COMMUNICATION_ERROR, 
       20202: SUCCESS, 
       20266: P1INVALID, 
       20267: P2INVALID, 
       20268: P3INVALID, 
...
class ShamRockController():
    ...
    def __init__(self):
        ...
        self.lock = RLock()
        self.dll = WinDLL('ShamrockCIF.dll')
        self.dll.ShamrockInitialize.argtypes = [
         c_char_p]
        ...
        self.dll.ShamrockSetGrating.argtypes = [
         c_int, c_int]
        ...
        self.dll.ShamrockSetWavelength.argtypes = [
         c_int, c_float]
        ...
        self.dll.ShamrockSetAutoSlitWidth.argtypes = [
         c_int, c_int, c_float]
        ...
        self.dll.ShamrockSetShutter.argtypes = [
         c_int, c_int]
        ...
        self.dll.ShamrockSetFilter.argtypes = [
         c_int, c_int]
        ...
        self.dll.ShamrockSetPixelWidth.argtypes = [
         c_int, c_float]
    ...
    def GetFunctionReturnDescription(self, error):
        errorC = c_int(error)
        res = c_char_p(' ' * ShamRockController.ERRORLENGTH)
        with self.lock:
...
class ShamRock():
    ...
    def EepromGetOpticalParams(self):
        ...
        focalTilt = c_float()
        with self.lock:
            error = self.dll.ShamrockEepromGetOpticalParams(self.curId, byref(focalLength), byref(angularDeviation), byref(focalTilt))
        ShamRockErrors.ProcessErrorCode(error)
    ...
    def GetGratingInfo(self, grating):
        ...
        offset = c_int()
        with self.lock:
            error = self.dll.ShamrockGetGratingInfo(self.curId, grating, byref(lines), blaze, byref(home), byref(offset))
        ShamRockErrors.ProcessErrorCode(error)
        self.logger.debug('GetGratingInfo %s' % str((lines.value, blaze.value, home.value, offset.value)))
    ...
    def GetWavelengthLimits(self, grating):
        ...
        wlmax = c_float()
        with self.lock:
            error = self.dll.ShamrockGetWavelengthLimits(self.curId, grating, byref(wlmin), byref(wlmax))
        ShamRockErrors.ProcessErrorCode(error)
    ...
    def GetAutoSlitCoefficients(self, index):
        ...
        y2 = c_int()
        with self.lock:
            error = self.dll.ShamrockGetAutoSlitCoefficients(self.curId, index, byref(x1), byref(y1), byref(x2), byref(y2))
        ShamRockErrors.ProcessErrorCode(error)
    ...
    def GetWavelengths(self):
        return 1e-09 * self.GetCalibration()
