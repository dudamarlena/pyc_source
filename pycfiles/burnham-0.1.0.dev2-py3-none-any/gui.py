...
# Embedded file name: build/bdist.linux-x86_64/egg/burneronfire/gui.py
# Compiled at: 2009-12-21 08:37:41
import os, logging, threading, operator, pygtk, gtk
...
class BurnerOnFireGTK(object):
    ...
    IS_STARTED = False
    MAP_MEDIA_TO_QUANTIFIER = {'cd': 172, 
       'dvd': 1351.7}
    ...
    def __init__(self, args):
        ...
        self.window.set_title('BurnerOnFire v%s' % __version__)
    ...
    def stop_wrapper(self):
        ...
        if filter(lambda t: isinstance(t, Worker), threading.enumerate()):
            mdialog = gtk.MessageDialog(self.window, gtk.DIALOG_MODAL, gtk.MESSAGE_WARNING, gtk.BUTTONS_CLOSE, 'Some burners are still active.\nWindow will close, ' + 'but burners will continue to process.')
            mdialog.run()
            mdialog.destroy()
        self.wrapper.stop()
    ...
    def signal_choose_isoimg(self, widget):
        ...
        dialog = gtk.FileChooserDialog('Choose ISO image to burn', buttons=(
    ...
    def start_burning(self, widget):
        ...
        if self.filename is None:
            mdialog = gtk.MessageDialog(self.window, gtk.DIALOG_DESTROY_WITH_PARENT, gtk.MESSAGE_WARNING, gtk.BUTTONS_CLOSE, 'You must first choose .ISO file to burn!')
            mdialog.run()
            mdialog.destroy()
            return
        else:
            ...
            bof = BurnerOnFire(self.args + [
             ...
             'speed=%s' % self.speed_entry.child.get_text().strip().split('x', 1)[0],
             self.filename], gui_app=self, limit=int(self.limit_entry.get_text().strip()))
    ...
    def open_about_dialog(self, widget):
        ...
        ad.set_copyright('Domen Kožar')
        ad.set_license(open(os.path.join(DATA_DIR, 'LICENSE.txt')).read())
        ad.set_authors(['Domen Kožar', 'Jožko Škrablin'])
        ad.set_artists(['Peter Čuhalev'])
        ad.set_logo(gtk.gdk.pixbuf_new_from_file(os.path.join(ICONS_DIR, 'burneronfire_logo.png')))
        ad.run()
        ad.destroy()
...
class StatusWindow(gtk.Window):
    ...
    def handle_dbus_method(self, udi, method, error_message):
        ...
        try:
            ...
            self.display_error_message('%s:\n %s' % (error_message, e.message))

    def display_error_message(self, message):
        mdialog = gtk.MessageDialog(self, gtk.DIALOG_MODAL, gtk.MESSAGE_ERROR, gtk.BUTTONS_OK, message)
        mdialog.run()
        mdialog.destroy()
    ...
    def alter_burned_discs(self, worker):
        ...
        if burned_discs == worker.limit:
            md = gtk.MessageDialog(self, gtk.DIALOG_MODAL, gtk.MESSAGE_INFO, gtk.BUTTONS_CLOSE, '%d of %d discs burned. You may now quit BurnerOnFire.' % (
