...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyogp/lib/base/message/tests/test_packetdata.py
...
$/LicenseInfo$
"""
import unittest, doctest, pprint, binascii, re, datetime
...
class TestPacketDecode(unittest.TestCase):
    ...
    def test_agent_data_update(self):
        ...
        assert packet.blocks['AgentData'][0].vars['LastName'].data == 'Kraft', 'Wrong last name for agent update'
        assert packet.blocks['AgentData'][0].vars['FirstName'].data == 'JB', 'Wrong first name for agent update'
        return
