...
# Compiled at: 2020-03-29 10:08:19
# Size of source mod 2**32: 14833 bytes
"""
aehostd.pam - PAM authentication, authorisation and session handling
"""
from __future__ import absolute_import
import time, hashlib, logging, socket, ldap0
...
class PAMRequest(req.Request):
    ...
    def _session_tracking_control(self):
        ...
        """
        if 'rhost' in self._params and self._params['rhost']:
            rhost = self._params['rhost']
        else:
            peer_env = self._get_peer_env(names=('SSH_CLIENT', ))
            self._log(logging.DEBUG, 'Retrieved peer env vars: %r', peer_env)
            rhost = peer_env.get('SSH_CLIENT', '').split(' ')[0]
        return SessionTrackingControl(rhost, '%s::%s' % (socket.getfqdn(), self._params['service']), SESSION_TRACKING_FORMAT_OID_USERNAME, self._params.get('ruser', self._params['username']))
        ...
        if CFG.pam_authc_cache_ttl > 0:
            ...
            try:
                ...
                if etime >= time.time():
                    ...
                    return None
                self._log(logging.DEBUG, 'Cached PAM authc result expired => remove it')
