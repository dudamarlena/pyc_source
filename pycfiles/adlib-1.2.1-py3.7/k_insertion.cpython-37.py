...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/adlib/adversaries/k_insertion.py
# Compiled at: 2018-07-17 12:56:28
# Size of source mod 2**32: 23479 bytes
from adlib.adversaries.adversary import Adversary
from adlib.utils.common import fuzz_matrix
from data_reader.binary_input import Instance
from data_reader.real_input import RealFeatureVector
import math, multiprocessing as mp, numpy as np, time
...
class KInsertion(Adversary):
    ...
    def __init__(self, learner, poison_instance, alpha=1e-08, beta=0.1, decay=-1, eta=0.9, max_iter=125, number_to_add=10, verbose=False):
        """
    ...
    def attack(self, instances) -> List[Instance]:
        ...
        for k in range(self.number_to_add):
            ...
            while not iteration == 0:
                if not fv_dist > self.alpha or iteration < self.max_iter:
                    print('Iteration: ', iteration, ' - FV distance: ', fv_dist, ' - gradient norm: ',
                      grad_norm, ' - UV norm: ', uv_norm, ' - beta: ',
                      (self.beta), sep='')
                    begin = time.time()
                    ...
                    if self.verbose:
                        print('\nGradient:\n', gradient, sep='')
                    update_vector = self.eta * old_update_vector + (1 - self.eta) * gradient
                    ...
                    if self.verbose:
                        print('\nFeature vector:\n', (self.x), '\n', sep='')
                        print('Max gradient value:', np.max(gradient), '- Min', 'gradient value:', np.min(gradient))
                        print('Max UV value:', np.max(update_vector), '- Min', 'UV value:', np.min(update_vector))
                        print('Max FV value:', np.max(self.x), '- Min FV value:', np.min(self.x))
                        print('Label:', self.y, '\n')
                    self._generate_inst()
            ...
            print('Iteration: FINAL - FV distance: ', fv_dist, ' - alpha: ', (self.alpha),
              ' - beta: ', (self.beta), sep='')
    ...
    def _generate_x_y_and_inst(self):
        ...
        self.x += abs(np.random.normal(0, 1e-05, len(self.x)))
    ...
    def _calc_grad_helper(self, i):
        ...
        for j in self.learner.model.learner.support_:
            vector.append(self._Q((self.instances[j]), (self.inst), derivative=True, k=i))
        ...
        for j in range(len(self.orig_instances)):
            if j in self.learner.model.learner.support_:
                ...
                partial_z_i_partial_x_k = partial_z_s_partial_x_k[s_v_indices.index(j)]
        ...
        if len(self.instances) in self.learner.model.learner.support_:
            current += self._Q(self.instances[(-1)], self.inst) * partial_z_s_partial_x_k[(-1)]
    ...
    def _solve_matrix(self):
        ...
        """
        learner = self.learner.model.learner
        size = learner.n_support_[0] + learner.n_support_[1] + 1
        matrix = np.full((size, size), 0.0)
        if len(self.instances) - 1 not in learner.support_:
            if self.learner.predict(self.inst) != self.inst.get_label():
                z_c = learner.C
            else:
                return (
                 0.0, matrix)
        ...
        for i in range(size - 1):
            values = list(map(lambda idx: self._Q(self.instances[learner.support_[i]], self.instances[learner.support_[idx]]), range(size - 1)))
            q_s.append(values)
    ...
    def _kernel_derivative_linear(self, fv_1: np.ndarray, fv_2: np.ndarray, k: int):
        """
        ...
        if len(fv_1) != len(fv_2) or k < 0 or k >= len(fv_1):
            raise ValueError('Feature vectors need to have same length and k must be a valid index.')
        return fv_1[k]
    ...
    def _kernel_poly(self, fv_1: np.ndarray, fv_2: np.ndarray):
        ...
        if len(fv_1) != len(fv_2):
            raise ValueError('Feature vectors need to have same length.')
        return (self.learner.gamma * fv_1.dot(fv_2) + self.learner.coef0) ** self.learner.degree

    def _kernel_derivative_poly(self, fv_1: np.ndarray, fv_2: np.ndarray, k: int):
        """
        Returns the value of the derivative of the specified kernel function
        with fv_2 being the variable (i.e. K(x_i, x_c), finding gradient
        evaluated at x_c
        ...
        if len(fv_1) != len(fv_2) or k < 0 or k >= len(fv_1):
            raise ValueError('Feature vectors need to have same length and k must be a valid index.')
        return fv_1[k] * self.learner.degree * self.learner.gamma * (self.learner.gamma * fv_1.dot(fv_2) + self.learner.coef0) ** (self.learner.degree - 1)
    ...
    def _kernel_derivative_rbf(self, fv_1: np.ndarray, fv_2: np.ndarray, k: int):
        """
        Returns the value of the derivative of the specified kernel function
        with fv_2 being the variable (i.e. K(x_i, x_c), finding gradient
        evaluated at x_c
        ...
        if len(fv_1) != len(fv_2) or k < 0 or k >= len(fv_1):
            raise ValueError('Feature vectors need to have same length and k must be a valid index.')
        return self._kernel_rbf(fv_1, fv_2) * 2 * self.learner.gamma * (fv_1[k] - fv_2[k])
    ...
    def _kernel_derivative_sigmoid(self, fv_1: np.ndarray, fv_2: np.ndarray, k: int):
        """
        Returns the value of the derivative of the specified kernel function
        with fv_2 being the variable (i.e. K(x_i, x_c), finding gradient
        evaluated at x_c
    ...
    def _get_kernel(self):
        ...
        if self.learner.model.learner.kernel == 'poly':
            return self._kernel_poly
        if self.learner.model.learner.kernel == 'rbf':
            return self._kernel_rbf
        if self.learner.model.learner.kernel == 'sigmoid':
