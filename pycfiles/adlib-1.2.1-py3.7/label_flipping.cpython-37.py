...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/adlib/adversaries/label_flipping.py
# Compiled at: 2018-07-26 12:42:47
# Size of source mod 2**32: 9209 bytes
from adlib.adversaries.adversary import Adversary
from adlib.utils.common import get_fvs_and_labels, logistic_loss
from data_reader.binary_input import Instance
import cvxpy as cvx, numpy as np
...
class LabelFlipping(Adversary):
    ...
    def __init__(self, learner, cost: List[float], total_cost: float, gamma=0.1, alpha=5e-07, max_iter=10, verbose=False):
        """
    ...
    def attack(self, instances) -> List[Instance]:
        ...
        half_n, n, orig_loss, feature_vectors, labels, cost = self._calculate_constants(instances)
        ...
        while iteration < self.max_iter and q_dist > self.alpha:
            ...
            self._minimize_w_epsilon(instances, n, orig_loss, feature_vectors, labels)
    ...
    def _calculate_constants(self, instances: List[Instance]):
        ...
        orig_loss = logistic_loss(fvs, self.learner, np.array(labels[:len(labels) // 2]))
        orig_loss = np.concatenate([orig_loss, orig_loss])
        cost = np.concatenate([np.full(half_n, 0), np.array(self.cost)])
        return (
         half_n, n, orig_loss, feature_vectors, labels, cost)

    def _minimize_w_epsilon(self, instances, n, orig_loss, feature_vectors, labels):
        """
