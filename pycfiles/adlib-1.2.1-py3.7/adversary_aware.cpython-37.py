...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/adlib/learners/adversary_aware.py
...
class AdversaryAware(object):
    ...
    def get_params(self):
        return {'attacker':self.attacker,  'Uc':self.Uc, 
         'Ua':self.Ua}
    ...
    def compute_c_x(self, instance):
        ...
        p_x_negative = class_prior[0] * np.exp(self.posterior_proba(instance))[(0,
                                                                                0)]
    ...
    def compute_p_a_prime(self, instance):
        ...
        return p_x_ + self.i_x(instance) * np.exp(self.posterior_proba(instance))[(0,
                                                                                   1)]

    def i_x(self, instance):
        if self.learner.predict(instance) == -1:
            return 1
        if self.attacker.w_feature_difference(instance, self.attacker.a(instance)) >= self.Ua[0][1] - self.Ua[1][1]:
            return 1
        return 0
    ...
    def posterior_proba(self, x):
        ...
        for i in range(np.size(self.learner.model.learner.classes_)):
            n_ij = np.sum(np.log(1 / np.sqrt(2.0 * np.pi * self.learner.model.learner.sigma_[i, :] ** 2)))
            n_ij -= 0.5 * np.sum((X - self.learner.model.learner.theta_[i, :]) ** 2 / self.learner.model.learner.sigma_[i, :], 1)
