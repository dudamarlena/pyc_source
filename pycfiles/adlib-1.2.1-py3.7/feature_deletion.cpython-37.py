...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/adlib/learners/feature_deletion.py
...
class FeatureDeletion(Learner):
    ...
    def __init__(self, training_instances=None, params=None):
        ...
        self.num_features = 0
        self.hinge_loss_multiplier = 0.5
        self.max_feature_deletion = 30
        self.bias = 0
    ...
    def get_available_params(self) -> Dict:
        params = {'hinge_loss_multiplier':self.hinge_loss_multiplier,  'max_feature_deletion':self.max_feature_deletion}
        return params
    ...
    def train(self):
        ...
        w = Variable(self.num_features + 1)
        ...
        constraints = [
         t >= K * z + sum_entries(v, axis=1), v >= 0]
        constraints.extend([z[i] + v[i, :] >= y[i] * mul_elemwise(X_prime[i], w).T for i in range(num_instances)])
    ...
    def predict(self, instances):
        ...
        if isinstance(instances, List):
            ...
            if type(instances) == Instance:
                predictions = np.sign(self.predict_instance(instances.get_feature_vector().get_csr_matrix().toarray()))
            else:
                predictions = []
