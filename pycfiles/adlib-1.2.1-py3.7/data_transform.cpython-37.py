...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/adlib/adversaries/datatransform/data_transform.py
...
class DataTransform(Adversary):
    ...
    def __init__(self, beta=0.1, dataset='./data_reader/data/raw/data-transform/house-processed.csv', epsilon=0.001, eta=0.5, initialization='randflip', lambd=1, logdir='./results', logind=0, model='ridge', multiproc=False, numinit=1, objective=1, optimizey=False, partct=4, poisct=75, rounding=False, seed=123, sigma=1.0, testct=500, trainct=300, validct=250, visualize=False):
    ...
    def attack(self, instances):
        ...
        colmap = {}
        trainfile, testfile, validfile, resfile, newlogdir = open_logging_files(args.logdir, args.model, args.logind, args)
        trainx, trainy, testx, testy, poisx, poisy, validx, validy = sample_dataset(x, y, args.trainct, args.poisct, args.testct, args.validct, args.seed)
        for i in range(len(testy)):
            testfile.write(','.join([str(val) for val in [testy[i]] + testx[i].tolist()[0]]) + '\n')

        testfile.close()
        for i in range(len(validy)):
            validfile.write(','.join([str(val) for val in [
        ...
        types = {'linreg':LinRegGDPoisoner,  'lasso':LassoGDPoisoner, 
         'enet':ENetGDPoisoner, 
        ...
        genpoiser = types[args.model](trainx, trainy, testx, testy, validx, validy, args.eta, args.beta, args.sigma, args.epsilon, args.multiproc, trainfile, resfile, args.objective, args.optimizey, colmap)
        ...
        poiser = types[args.model](trainx, trainy, testx, testy, validx, validy, args.eta, args.beta, args.sigma, args.epsilon, args.multiproc, trainfile, resfile, args.objective, args.optimizey, colmap)
        for i in range(args.partct + 1):
            ...
            towrite = [
             numsamples, -1, None, None, err[0], err[1],
             (timeend - timestart).total_seconds()]
            resfile.write(','.join([str(val) for val in towrite]) + '\n')
            trainfile.write('\n')
            for j in range(numsamples):
                trainfile.write(','.join([str(val) for val in [
                 poisresy[j]] + poisres[j].tolist()[0]]) + '\n')

            if args.rounding:
                towrite = [
                 numsamples, 'r', None, None, rounderr[0],
                 rounderr[1],
                 (timeend - timestart).total_seconds()]
                resfile.write(','.join([str(val) for val in towrite]) + '\n')
                trainfile.write('\nround\n')
