...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/adlib/adversaries/restrained_attack.py
...
class Restrained(Adversary):
    ...
    def get_available_params(self) -> Dict:
        params = {'f_attack':self.f_attack,  'binary':self.binary, 
    ...
    def set_innocuous_target(self, train_instances, learner, type):
        ...
        """
        if type == 'random':
            self.innocuous_target = next((x for x in train_instances if x.get_label() == -1), None)
        else:
            if type == 'centroid':
    ...
    def transform(self, instance: Instance):
        ...
        if self.binary:
            ...
            for i in range(0, self.num_features):
                delta_ij = self.innocuous_target.get_feature_vector().get_feature(i) - instance.get_feature_vector().get_feature(i)
                if delta_ij != 0:
                    if self.binary:
            ...
            for i in range(0, self.num_features):
                ...
                if abs(xij) + abs(target) == 0:
                    ...
                    bound = self.discount_factor * (1 - self.f_attack * (abs(target - xij) / (abs(xij) + abs(target)))) * abs(target - xij)
