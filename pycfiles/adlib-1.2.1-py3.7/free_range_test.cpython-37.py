...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/adlib/tests/adversaries/free_range_test.py
...
def test_transform_instance(free_range, freerange_learner, training_data, testing_data):
    ...
    for i in range(0, free_range.num_features):
        delta = free_range.innocuous_target.get_feature_vector().get_feature(i) - sample_.get_feature_vector().get_feature(i)
        assert delta == 0
...
def test_transform_instance_low(free_range, freerange_learner, training_data, testing_data):
    ...
    equal = True
    for i in range(0, free_range.num_features):
        delta = free_range.innocuous_target.get_feature_vector().get_feature(i) - sample.get_feature_vector().get_feature(i)
        if delta != 0:
            equal = False
