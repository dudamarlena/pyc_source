...
# Embedded file name: /Users/mx/Dropbox (MIT)/Science/Code/allesfitter/allesfitter/transit_search/injection_recovery.py
# Compiled at: 2020-02-17 11:12:07
# Size of source mod 2**32: 17906 bytes
...
def make_model(time, flux, flux_err, epoch, period, rplanet, R_star=1.0, M_star=1.0, show_plot=False, save_plot=False, save_csv=False):
    """
    Inputs:
    -------
    time : array of float
    ...
    """
    a = (G / (4 * np.pi ** 2) * (period * u.d) ** 2 * M_star * u.Msun) ** 0.3333333333333333
    a = a.to(u.AU)
...
def to_do_or_not_to_do_that_is_the_question(ex, period, rplanet):
    ...
    for i in range(len(ex)):
        if np.abs(period - ex['inj_period'][i]) < 1e-06 and np.abs(rplanet - ex['inj_rplanet'][i]) < 1e-06:
            return False

    return True


def inject_and_tls_search(time, flux, flux_err, periods, rplanets, logfname, SNR_threshold=5.0, known_transits=None, R_star=1.0, R_star_min=0.13, R_star_max=3.5, M_star=1.0, M_star_min=0.1, M_star_max=1.0, show_plot=False, save_plot=False):
    """
    Inputs:
    -------
    time : array of flaot
    ...
    print('\n', flush=True)
    for period, rplanet in tqdm((itertools.product(periods, rplanets)), total=(len(periods) * len(rplanets))):
        if to_do_or_not_to_do_that_is_the_question(ex, period, rplanet):
            ...
            flux2 = make_model(time, flux, flux_err, epoch, period, rplanet, R_star=R_star,
              M_star=M_star,
              show_plot=show_plot,
              save_plot=save_plot)
            results_all = tls_search(time, flux2, flux_err, SNR_threshold=SNR_threshold,
            ...
            if len(results_all) > 0:
                for r in results_all:
                    with open(logfname, 'a') as (f):
                        f.write(format(period, '.5f') + ',' + format(rplanet, '.5f') + ',' + format(r.period, '.5f') + ',' + format(r.depth, '.5f') + ',' + format(r.duration, '.5f') + ',' + format(r.SDE, '.5f') + ',' + format(r.snr, '.5f') + '\n')

            else:
                with open(logfname, 'a') as (f):
                    f.write(format(period, '.5f') + ',' + format(rplanet, '.5f') + ',' + 'nan' + ',' + 'nan' + ',' + 'nan' + ',' + 'nan' + ',' + 'nan' + '\n')
        else:
...
def inject_and_tls_search_by_tic(tic_id, periods, rplanets, logfname, SNR_threshold=5.0, known_transits=None, show_plot=False, save_plot=False):
    """
    Inputs:
    -------
    tic_id : str or int
    ...
    inject_and_tls_search(time, flux, flux_err, periods, rplanets, logfname, SNR_threshold=SNR_threshold,
      known_transits=known_transits,
      R_star=R_star,
