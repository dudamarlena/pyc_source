...
# Embedded file name: /Users/mx/Dropbox (MIT)/Science/Code/allesfitter/allesfitter/exoworlds_rdx/lightcurves/lightcurve_tools.py
# Compiled at: 2019-05-28 16:47:33
...
def phase_fold(time, flux, P, Tprim, dt=0.02, ferr_type='medsig', ferr_style='std', sigmaclip=False):
    ...
    return (
     phase, phaseflux, phaseflux_err, N, phi)
...
def phase_fold_matrix(time, flux_matrix, P, Tprim, dt=0.02, ferr_type='medsig', ferr_style='std', sigmaclip=False):
    ...
    return (
     phase, phasefluxmatrix, phasefluxmatrix_err, N, phi)


def plot_phase_folded_lightcurve(time, flux, period, epoch, ax=None, xlim=[-0.25, 0.75], dt=0.02, ferr=None, ferr_type='medsig', ferr_style='sem', sigmaclip=False, normalize=True, title='', period_factor=1.0):
    if ax is None:
        fig, ax = plt.subplots()
    else:
        fig = None
...
def plot_phase_folded_lightcurve_dic(ax, dic, obj_id=None, ferr_type='medsig', ferr_style='std', sigmaclip=False, period_factor=1.0):
    if not isinstance(dic['OBJ_ID'], basestring):
        ind = np.where(dic['OBJ_ID'] == obj_id)[0]
        if 'SYSREM_FLUX3_ERR' in dic:
            ferr = dic['SYSREM_FLUX3_ERR'][ind]
        else:
            ferr = None
        ...
        if 'SYSREM_FLUX3_ERR' in dic:
            ferr = dic['SYSREM_FLUX3_ERR']
        else:
            ferr = None
        ind_tr, ind_tr_half, ind_tr_double, ind_out, ind_out_per_tr, tmid = index_transits.index_transits(dic)
...
def plot_binned_lightcurve(ax, time, f, ferr=np.nan, bin_time=0, show_transit_regions=False, period=None, epoch=None, width=None, obj_id=None, normalize=True, title='', exposure=12.0, debug=False):
    if bin_time != 0:
        start = timeit.default_timer()
        bin_width = 1.0 * bin_time / exposure
        time, f, ferr = binning1D_per_night(time, f, bin_width, timegap=(TODO * 3600), setting='median', normalize=normalize)
        stop = timeit.default_timer()
        if debug:
            print('Binning succesfully created in', stop - start, 's.')
    else:
        start = timeit.default_timer()
        ax.plot(time, f, '.', color='grey', rasterized=True)
        ax.set_title(title)
        ax.set_ylabel('Flux')
        ax.set_xlabel('HJD')
        ...
        stop = timeit.default_timer()
        if debug:
            print('Scatter succesfully created in', stop - start, 's.')
        start = timeit.default_timer()
        ...
        stop = timeit.default_timer()
        if debug:
            print('Lines succesfully created in', stop - start, 's.')
        start = timeit.default_timer()
        if show_transit_regions == True:
            if period is not None:
                if epoch is not None and width is not None:
                    T_ingress = epoch - width / 2.0
                    T_egress = epoch + width / 2.0
                    j = 0
                    while T_egress < time[(-1)]:
                        ax.axvspan(T_ingress, T_egress, facecolor='g', alpha=0.5, zorder=(-1))
                        j += 1
                        T_ingress = j * period + (epoch - width / 2.0)
                        T_egress = j * period + (epoch + width / 2.0)

    stop = timeit.default_timer()
    if debug:
        print('Greens succesfully created in', stop - start, 's.')
...
def plot_binned_lightcurve_dic(ax, dic, obj_id=None, bin_time=0, normalize=True, show_transit_regions=False, debug=False):
    if not isinstance(dic['OBJ_ID'], basestring):
        ...
        if 'SYSREM_FLUX3_ERR' in dic:
            ...
            ferr = np.nan
        plot_binned_lightcurve(ax, (dic['HJD'][ind] / 86400.0), (dic['SYSREM_FLUX3'][ind]), ferr=ferr, bin_time=bin_time, normalize=normalize, title=(dic['FIELDNAME'] + ', ' + dic['OBJ_ID'][ind]))
    else:
        if 'SYSREM_FLUX3_ERR' in dic:
            ferr = dic['SYSREM_FLUX3_ERR']
        else:
        ...
        if normalize == True:
            dic['SYSREM_FLUX3'] /= np.nanmedian(dic['SYSREM_FLUX3'][ind_out])
        plot_binned_lightcurve(ax, (dic['HJD'] / 86400.0), (dic['SYSREM_FLUX3']), ferr=ferr, bin_time=bin_time, show_transit_regions=show_transit_regions, period=(dic['PERIOD'] / 86400.0), epoch=(dic['EPOCH'] / 86400.0), width=(dic['WIDTH'] / 86400.0), normalize=True, title=(dic['FIELDNAME'] + ', ' + dic['OBJ_ID']), debug=debug)