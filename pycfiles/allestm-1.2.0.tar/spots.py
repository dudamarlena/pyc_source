...
# Embedded file name: /Users/mx/Dropbox (MIT)/Science/Code/allesfitter/spots.py
# Compiled at: 2019-01-15 14:51:44
        ...
        if config.BASEMENT.settings[('host_N_spots_' + inst)] > 0:
            if mode == 'default':
                ...
                for i_sample, sample in tqdm(enumerate(posterior_samples)):
                    ...
                    model_xx = allesfitter.computer.calculate_model(params, inst, 'flux', xx=xx)
                    baseline_xx = allesfitter.computer.calculate_baseline(params, inst, 'flux', xx=xx)
                    if i_sample == 0:
                        if mode == 'default':
                            ...
                            ax1 = axplot_data(ax1, np.concatenate((config.BASEMENT.data[inst]['time'], config.BASEMENT.data[inst]['time'] + 1, config.BASEMENT.data[inst]['time'] + 2)), np.concatenate((config.BASEMENT.data[inst]['flux'], config.BASEMENT.data[inst]['flux'], config.BASEMENT.data[inst]['flux'])), flux_err=None)
                            ax2 = axplot_residuals(ax2, np.concatenate((config.BASEMENT.data[inst]['time'], config.BASEMENT.data[inst]['time'] + 1, config.BASEMENT.data[inst]['time'] + 2)), np.concatenate((config.BASEMENT.data[inst]['flux'] - model - baseline, config.BASEMENT.data[inst]['flux'] - model - baseline, config.BASEMENT.data[inst]['flux'] - model - baseline)), res_err=None)
                            ax3 = axplot_spots_2d(ax3, spots)
...
def plot_spots(spots, command='show'):
    ...
    for i in range(N_spots):
        ...
        theta = np.random.rand(N_rand) * 2.0 * np.pi
        lonv = lon + r * np.cos(theta)
        latv = lat + r * np.sin(theta)
        lon = lonv / 180.0 * np.pi
        lat = latv / 180.0 * np.pi
        x = radius * np.cos(lon) * np.cos(lat)
        y = radius * np.sin(lon) * np.cos(lat)
        z = radius * np.sin(lat)
