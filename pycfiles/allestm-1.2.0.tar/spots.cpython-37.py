...
# Embedded file name: /Users/mx/Dropbox (MIT)/Science/Code/allesfitter/allesfitter/spots.py
# Compiled at: 2020-03-09 20:29:31
# Size of source mod 2**32: 18997 bytes
...
def convert_many_points_to_an_area(lons, lats, sizes, brightnesses):
    ...
    return (
     lon_list, lat_list, brightness_list)
...
def plot_spots_new(datadir):
    ...
    for inst in alles.BASEMENT.settings['inst_phot']:
        ...
        plt.tight_layout()
        plt.xticks(ticks=(np.deg2rad([-150, -120, -90, -60, -30, 0, 30, 60, 90, 120, 150, 180])), labels=['', '', '$270^\\circ$', '', '', '$0^\\circ$', '', '', '$90^\\circ$', '', '', '$180^\\circ$'])
        plt.yticks(ticks=(np.deg2rad([-90, -60, -30, 0, 30, 60, 90])), labels=['', '$-60^\\circ$', '$-30^\\circ$', '0', '$30^\\circ$', '$60^\\circ$', ''])
            ...
            for i_sample, sample in tqdm(enumerate(posterior_samples)):
                ...
                if i_sample == 0:
                    if mode == 'default':
                        ...
                        ax1 = axplot_data(ax1, (np.concatenate((config.BASEMENT.data[inst]['time'], config.BASEMENT.data[inst]['time'] + 1, config.BASEMENT.data[inst]['time'] + 2))),
                          (np.concatenate((config.BASEMENT.data[inst]['flux'], config.BASEMENT.data[inst]['flux'], config.BASEMENT.data[inst]['flux']))),
                          flux_err=None)
                        ax2 = axplot_residuals(ax2, (np.concatenate((config.BASEMENT.data[inst]['time'], config.BASEMENT.data[inst]['time'] + 1, config.BASEMENT.data[inst]['time'] + 2))),
        ...
        for companion in config.BASEMENT.settings['companions_all']:
            for inst in config.BASEMENT.settings['inst_all']:
                if config.BASEMENT.settings[(companion + '_N_spots_' + inst)] > 0:
                    spots = [[params[(companion + '_spot_' + str(i) + '_long_' + inst)] for i in range(1, config.BASEMENT.settings[(companion + '_N_spots_' + inst)] + 1)],
