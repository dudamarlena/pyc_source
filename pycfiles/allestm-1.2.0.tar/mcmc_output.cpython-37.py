...
# Compiled at: 2020-03-29 11:32:26
...
def draw_mcmc_posterior_samples_at_maximum_likelihood(sampler, as_type='1d_array'):
    log_prob = sampler.get_log_prob(flat=True, discard=(int(1.0 * config.BASEMENT.settings['mcmc_burn_steps'] / config.BASEMENT.settings['mcmc_thin_by'])))
    posterior_samples = sampler.get_chain(flat=True, discard=(int(1.0 * config.BASEMENT.settings['mcmc_burn_steps'] / config.BASEMENT.settings['mcmc_thin_by'])))
...
def plot_MCMC_chains(sampler):
    ...
    axes[0].set(title='lnprob', xlabel='steps', rasterized=True, ylim=[
     mini, maxi])
...
def plot_MCMC_corner(sampler):
    ...
    for companion in config.BASEMENT.settings['companions_all']:
        if companion + '_epoch' in config.BASEMENT.fitkeys:
            ...
            samples[:, ind] -= int(params_median[(companion + '_epoch')])
...
def print_autocorr(sampler):
    ...
    N_evaluation_samples = int(1.0 * config.BASEMENT.settings['mcmc_nwalkers'] * (config.BASEMENT.settings['mcmc_total_steps'] - config.BASEMENT.settings['mcmc_burn_steps']) / config.BASEMENT.settings['mcmc_thin_by'])
...
def get_mcmc_posterior_samples(datadir, Nsamples=None, QL=False, as_type='dic'):
    ...
    return draw_mcmc_posterior_samples(reader, Nsamples=Nsamples, as_type=as_type)