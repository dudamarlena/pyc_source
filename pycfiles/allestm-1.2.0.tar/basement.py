...
# Compiled at: 2019-01-23 21:06:16
...
class Basement:
    ...
    def __init__(self, datadir):
        ...
        for inst in self.settings['inst_phot']:
            key = 'flux'
            if (self.settings[('baseline_' + key + '_' + inst)] == 'sample_GP') & (self.settings[('error_' + key + '_' + inst)] != 'sample'):
    ...
    def load_settings(self):
        ...
        self.settings['companions_all'] = list(np.unique(self.settings['companions_phot'] + self.settings['companions_rv']))
        self.settings['inst_all'] = list(unique(self.settings['inst_phot'] + self.settings['inst_rv']))
        ...
        for companion in self.settings['companions_all']:
            ...
            elif len(self.settings[('inst_for_' + companion + '_epoch')]):
        ...
        if 'multiprocess_cores' not in self.settings.keys():
            ...
            if self.settings['multiprocess_cores'] == 'all':
                self.settings['multiprocess_cores'] = cpu_count() - 1
            else:
                ...
                if 'fast_fit_width' in self.settings.keys() and len(self.settings['fast_fit_width']):
                    ...
                    self.settings['fast_fit_width'] = 0.3333333333333333
                ...
                for inst in self.settings['inst_all']:
                    ...
                    if 't_exp_' + inst in self.settings and 't_exp_n_int_' + inst in self.settings and len(self.settings[('t_exp_n_int_' + inst)]):
    ...
    def load_params(self):
        ...
        self.allkeys = buf['name']
        self.labels = buf['label']
        self.units = buf['unit']
        if 'truth' in buf.dtype.names:
            self.truths = buf['truth']
        else:
        ...
        for companion in self.settings['companions_all']:
            for inst in self.settings['inst_all']:
                ...
                if self.params[(companion + '_sbratio_' + inst)] > 0 and self.params[(companion + '_geom_albedo_' + inst)] == 0:
                    self.params[companion + '_geom_albedo_' + inst] = 1e-15
    ...
    def load_data(self):
        ...
        for inst in self.settings['inst_phot']:
            ...
            self.data[inst] = {'time': time, 'flux': flux, 
    ...
    def change_epoch(self):
        for companion in self.settings['companions_all']:
            ...
            first_epoch = 1.0 * self.params[(companion + '_epoch')]
            ...
            epoch_shift = int(np.round((end - start) / 2.0 / period)) * period
            epoch_for_fit = first_epoch + epoch_shift
            self.params[companion + '_epoch'] = 1.0 * epoch_for_fit
            ind_epoch_fitkeys = np.where(self.fitkeys == companion + '_epoch')[0]
            if len(ind_epoch_fitkeys):
                ind_epoch_fitkeys = ind_epoch_fitkeys[0]
                buf = 1.0 * self.theta_0[ind_epoch_fitkeys]
                self.theta_0[ind_epoch_fitkeys] = 1.0 * epoch_for_fit
                ...
                if self.bounds[ind_epoch_fitkeys][0] == 'uniform':
                    ...
                    self.bounds[ind_epoch_fitkeys][1] = epoch_for_fit - lower
                    self.bounds[ind_epoch_fitkeys][2] = epoch_for_fit + upper
                elif self.bounds[ind_epoch_fitkeys][0] == 'normal':
                    mean = 1.0 * self.theta_0[ind_epoch_fitkeys]
                    std = 1.0 * self.bounds[ind_epoch_fitkeys][2]
                    ...
                    upper = self.bounds[ind_epoch_fitkeys][2] - buf
                    mean = 1.0 * self.theta_0[ind_epoch_fitkeys]
                    std = 1.0 * self.bounds[ind_epoch_fitkeys][4]
                    self.bounds[ind_epoch_fitkeys][1] = epoch_for_fit - lower
                    self.bounds[ind_epoch_fitkeys][2] = epoch_for_fit + upper
