...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/logsnada/handler.py
# Compiled at: 2014-01-14 22:13:47
"""
...
class LoggerNameBased(logging.FileHandler):
    ...
    def __init__(self, fh_class, filename, dir_prefix=None, mode='a', encoding=None):
        """
        ...
        fh_class = isinstance(fh_class, basestring) and BaseConfigurator({}).resolve(fh_class) or fh_class
    ...
    def _prepare_path(cls, logger_name, path_prefix, filename):
        ...
        @rtype: str
        """
        full_path = os.path.join(path_prefix + logger_name.rsplit('.', 1)[(-1)], filename)
    ...
    def _create_handler(self, logger_name):
        filename = self._prepare_path(logger_name, self.__dir_prefix, self.__filename)
