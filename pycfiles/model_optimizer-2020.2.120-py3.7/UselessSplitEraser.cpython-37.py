...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/UselessSplitEraser.py
...
class UselessSplitEraser(MiddleReplacementPattern):
    ...
    def pattern(self):
        return dict(nodes=[
         (
          'split', {'op':'Split',  'num_splits':1})],
          edges=[])

    def replace_pattern(self, graph: Graph, match: dict):
        node = match['split']
        name = node.soft_get('name', node.id)
        assert node.soft_get('input_port', 0) == 0, 'Internal attribute `input_port` was not resolved on front phase, broken Split {}'.format(name)
