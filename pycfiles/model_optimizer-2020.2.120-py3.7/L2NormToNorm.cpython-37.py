...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/L2NormToNorm.py
...
class L2NormToNorm(MiddleReplacementPattern):
    ...
    def pattern(self):
        return dict(nodes=[
         ...
         (
          'rsqrt_pow',
          dict(kind='data', value=(lambda x:           if x is not None:
np.all(x == -0.5) # Avoid dead code: False))),
         (
         ...
         (
          'square_pow',
          dict(kind='data', value=(lambda x:           if x is not None:
np.all(x == 2) # Avoid dead code: False))),
         (
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        if y is None:
            ...
            normalize_node = NormalizeOp(graph, {'name':output_name,  'eps':y,  'across_spatial':0, 
             'channel_shared':0}).create_node()
            rename_node(normalize_node, output_name)
            weights_node = Const(graph, {'value': np.ones(shape=(int64_array([match['input'].shape[(-1)]])), dtype=(match['input'].data_type))}).create_node()
