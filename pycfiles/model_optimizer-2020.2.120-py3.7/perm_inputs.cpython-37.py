...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/graph/perm_inputs.py
# Compiled at: 2020-05-01 08:37:22
# Size of source mod 2**32: 8319 bytes
"""
...
def get_node_with_permutation(node: Node, port_info: str):
    ...
    port = int(port)
    if node_type == 'input':
        return node.in_node(port)
    return node.out_node(port)
...
def axis(op_node: Node, port_info: str, input_port: int):
    ...
    assert permutation_data_node.has_and_set('permutation'), 'Data node "{}" does not have permutation for node {}, port_info "{}".'.format(permutation_data_node.id, op_node.id, port_info)
...
def order(op_node: Node, port_info: str, input_port: int):
    ...
    axis_const = Const(graph, {'value': int64_array(0)}).create_node_with_data()
    gather = Gather(graph, {'name':op_node.name + '/OrderGather_1',  'need_shape_inference':True}).create_node_with_data([data_node, const, axis_const])
    ...
    gather_1 = Gather(graph, {'name':op_node.name + '/OrderGather_2',  'need_shape_inference':True}).create_node_with_data([const_1, gather, axis_const_1])
    attrs = graph.get_edge_data((data_node.id), (op_node.id), key=0).copy()
...
def shape(op_node: Node, port_info: str, input_port: int):
    ...
    const = Const(graph, {'value':permutation.perm,  'need_shape_inference':True}).create_node_with_data()
    axis_const = Const(graph, {'value': int64_array(0)}).create_node_with_data()
    gather = Gather(graph, {'name':op_node.name + '/ShapeGather',  'need_shape_inference':True}).create_node_with_data([data_node, const, axis_const])
    attrs = graph.get_edge_data((data_node.id), (op_node.id), key=0).copy()
    (graph.add_edge)((gather.id), (op_node.id), **attrs)
    graph.remove_edge(data_node.id, op_node.id)
...
class PermuteInputs:
    ...
    input_permutes = {'axis':common_inv_permutation, 
    ...
    def set_input_permutation(self, node1, node2, port_info, permutation_rule):
        """
        ...
        """
        assert permutation_rule in self.input_permutes, 'No `{}` permutation rule in {}'.format(permutation_rule, __class__.__name__)
        nx.set_edge_attributes(G=(node1.graph), values={(
 node1.id, node2.id, 0): (self.input_permutes[permutation_rule],
                                     port_info)},
