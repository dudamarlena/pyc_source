...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/analysis/inputs.py
...
class InputsAnalysis(AnalyzeAction):
    ...
    def fifo_queue_analysis(cls, graph: Graph, inputs_desc: dict):
        ...
        for fifo_queue in graph.get_op_nodes(op='FIFOQueueV2'):
            ...
            if queue_deque.op in ('QueueDequeueMany', 'QueueDequeueManyV2', 'QueueDequeueUpTo',
                                  'QueueDequeueUpToV2'):
                ...
                for port_ind in range(len(queue_deque.out_nodes())):
                    inputs_desc['{}:{}'.format(queue_deque.id, port_ind)] = {'shape':fifo_queue.shapes[port_ind].tolist(),  'value':None, 
    ...
    def ignore_mxnet_softmax_inputs(cls, graph: Graph):
        ...
        inputs_to_ignore = set()
        softmax_nodes = []
        [softmax_nodes.extend(graph.get_op_nodes(op=op)) for op in ('SoftMax', 'SoftmaxActivation',
                                                                    'SoftmaxOutput')]
        ...
        if len(op_nodes):
            ...
            for iter_get_next in op_nodes:
                for port in iter_get_next.out_nodes().keys():
                    inputs_desc['{}:{}'.format(iter_get_next.soft_get('name', iter_get_next.id), port)] = {'shape':iter_get_next.shapes[port].tolist(), 
                     'value':None, 
    ...
    def analyze(self, graph: Graph):
        ...
        for input in placeholders_with_default:
            inputs_desc[input.name] = {'shape':input.soft_get('shape', None), 
             'data_type':input.soft_get('data_type', None), 
             'value':input.in_node(0).value if 0 in input.in_nodes() and input.in_node(0).has_valid('value') else None}
