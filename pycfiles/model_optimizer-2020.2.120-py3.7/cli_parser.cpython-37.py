...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/utils/cli_parser.py
# Compiled at: 2020-05-01 08:37:22
# Size of source mod 2**32: 55510 bytes
"""
...
"""
import argparse, ast, logging as log, os, re, sys
...
class DeprecatedTensorflowOffloadFeatureAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        msg = 'Use of deprecated cli option {} detected. Option use in the following releases will be fatal.'.format(option_string)
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        for name in names.split(','):
            if name != '' and not os.path.exists(name):
                raise Error('The value for command line parameter "{}" must be existing file/directory,  but "{}" does not exist.'.format(self.dest, name))
...
def get_common_cli_parser(parser: argparse.ArgumentParser=None):
    ...
    common_group.add_argument('--input_model', '-w', '-m', help='Tensorflow*: a file with a pre-trained model  (binary or text .pb file after freezing).\n Caffe*: a model proto file with model weights',
      action=CanonicalizePathCheckExistenceAction,
      type=readable_file)
    common_group.add_argument('--model_name', '-n', help='Model_name parameter passed to the final create_ir transform. This parameter is used to name a network in a generated IR and output .xml/.bin files.')
    common_group.add_argument('--output_dir', '-o', help='Directory that stores the generated IR. By default, it is the directory from where the Model Optimizer is launched.',
      default=(get_absolute_path('.')),
      action=CanonicalizePathAction,
      type=writable_dir)
    common_group.add_argument('--input_shape', help='Input shape(s) that should be fed to an input node(s) of the model. Shape is defined as a comma-separated list of integer numbers enclosed in parentheses or square brackets, for example [1,3,227,227] or (1,227,227,3), where the order of dimensions depends on the framework input layout of the model. For example, [N,C,H,W] is used for Caffe* models and [N,H,W,C] for TensorFlow* models. Model Optimizer performs necessary transformations to convert the shape to the layout required by Inference Engine (N,C,H,W). The shape should not contain undefined dimensions (? or -1) and should fit the dimensions defined in the input operation of the graph. If there are multiple inputs in the model, --input_shape should contain definition of shape for each input separated by a comma, for example: [1,3,227,227],[2,4] for a model with two inputs with 4D and 2D shapes. Alternatively, you can specify shapes with the --input option.')
    common_group.add_argument('--scale', '-s', type=float,
      help='All input values coming from original network inputs will be divided by this value. When a list of inputs is overridden by the --input parameter, this scale is not applied for any input that does not match with the original input of the model.')
    ...
    common_group.add_argument('--scale_values', help='Scale values to be used for the input image per channel. Values are provided in the (R,G,B) or [R,G,B] format. Can be defined for desired input of the model, for example: "--scale_values data[255,255,255],info[255,255,255]". The exact meaning and order of channels depend on how the original model was trained.',
      default=())
    common_group.add_argument('--data_type', help='Data type for all intermediate tensors and weights. If original model is in FP32 and --data_type=FP16 is specified, all model weights and biases are quantized to FP16.',
      choices=[
     'FP16', 'FP32', 'half', 'float'],
      default='float')
    common_group.add_argument('--disable_fusing', help='Turn off fusing of linear operations to Convolution',
      action=DeprecatedStoreTrue)
    common_group.add_argument('--disable_resnet_optimization', help='Turn off resnet optimization',
      action='store_true')
    common_group.add_argument('--finegrain_fusing', help="Regex for layers/operations that won't be fused. Example: --finegrain_fusing Convolution1,.*Scale.*")
    common_group.add_argument('--disable_gfusing', help='Turn off fusing of grouped convolutions',
      action=DeprecatedStoreTrue)
    common_group.add_argument('--enable_concat_optimization', help='Turn on concat optimization',
      action='store_true')
    common_group.add_argument('--move_to_preprocess', help='Move mean values to IR preprocess section',
      action='store_true')
    common_group.add_argument('--extensions', help='Directory or a comma separated list of directories with extensions. To disable all extensions including those that are placed at the default location, pass an empty string.',
      default=(import_extensions.default_path()),
      action=CanonicalizePathCheckExistenceAction,
      type=readable_dirs_or_empty)
    common_group.add_argument('--batch', '-b', type=check_positive,
      default=None,
    ...
    common_group.add_argument('--silent', help='Prevent any output messages except those that correspond to log level equals ERROR, that can be set with the following option: --log_level. By default, log level is already ERROR. ',
      action='store_true',
      default=False)
    common_group.add_argument('--freeze_placeholder_with_value', help='Replaces input layer with constant node with provided value, for example: "node_name->True". It will be DEPRECATED in future releases. Use --input option to specify a value for freezing.',
      default=None)
    common_group.add_argument('--generate_deprecated_IR_V7', help='Force to generate old deprecated IR V7 with layers from old IR specification.',
      action=DeprecatedStoreTrue,
      default=False)
    common_group.add_argument('--keep_shape_ops', help='[ Experimental feature ] Enables `Shape` operation with all children keeping. This feature makes model reshapable in Inference Engine',
      action='store_true',
      default=False)
    common_group.add_argument('--disable_weights_compression', help='Disable compression and store weights with original precision',
      action='store_true',
      default=False)
    common_group.add_argument('--progress', help='Enables model conversion progress display',
      action='store_true',
      default=False)
    common_group.add_argument('--stream_output', help='Switches model conversion progress display to a multiline mode',
      action='store_true',
      default=False)
    common_group.add_argument('--transformations_config', help='Use the configuration file with transformations description.',
      action=CanonicalizePathCheckExistenceAction)
    return parser
...
def get_common_cli_options(model_name):
    ...
    d['output_dir'] = ['- Path for generated IR',
     lambda x:      if x != '.':
x # Avoid dead code: os.getcwd()]
    d['model_name'] = [
     '- IR output name',
     lambda x:      if x:
x # Avoid dead code: model_name]
    d['log_level'] = '- Log level'
    d['batch'] = ['- Batch',
     lambda x:      if x:
x # Avoid dead code: 'Not specified, inherited from the model']
    d['input'] = [
     '- Input layers',
     lambda x:      if x:
x # Avoid dead code: 'Not specified, inherited from the model']
...
x # Avoid dead code: 'Not specified, inherited from the model']
    d['mean_values'] = [
     '- Mean values',
     lambda x:      if x:
x # Avoid dead code: 'Not specified']
    d['scale_values'] = [
     '- Scale values',
     lambda x:      if x:
x # Avoid dead code: 'Not specified']
...
x # Avoid dead code: 'Not specified']
    d['data_type'] = [
     '- Precision of IR',
     lambda x:      if x == 'float':
'FP32' # Avoid dead code:      if x == 'half':
'FP16' # Avoid dead code: x]
    d['disable_fusing'] = [
     '- Enable fusing', lambda x: not x]
...
def get_caffe_cli_options():
    d = {'input_proto':[
      '- Path to the Input prototxt', lambda x: x], 
     'caffe_parser_path':[
      '- Path to Python Caffe* parser generated from caffe.proto', lambda x: x], 
     'mean_file':[
      '- Path to a mean file',
      lambda x:       if x:
x # Avoid dead code: 'Not specified'], 
     'mean_file_offsets':[
      '- Offsets for a mean file',
      lambda x:       if x:
x # Avoid dead code: 'Not specified'], 
     'k':'- Path to CustomLayersMapping.xml', 
     'disable_resnet_optimization':[
      '- Enable resnet optimization', lambda x: not x]}
    return OrderedDict(sorted((d.items()), key=(lambda t: t[0])))


def get_tf_cli_options():
    d = {'input_model_is_text':'- Input model in text protobuf format', 
     ...
     'tensorflow_object_detection_api_pipeline_config':'- Use configuration file used to generate the model with Object Detection API', 
...
def get_mxnet_cli_options():
    d = {'input_symbol':'- Deploy-ready symbol file', 
...
def get_kaldi_cli_options():
    d = {'counts':'- A file name with full path to the counts file', 
...
def get_caffe_cli_parser(parser: argparse.ArgumentParser=None):
    ...
    caffe_group.add_argument('--input_proto', '-d', help='Deploy-ready prototxt file that contains a topology structure and layer attributes',
      type=str,
      action=CanonicalizePathCheckExistenceAction)
    caffe_group.add_argument('--caffe_parser_path', help='Path to Python Caffe* parser generated from caffe.proto',
      type=str,
      default=(os.path.join(os.path.dirname(sys.argv[0]), 'mo', 'front', 'caffe', 'proto')),
      action=CanonicalizePathCheckExistenceAction)
    caffe_group.add_argument('-k', help='Path to CustomLayersMapping.xml to register custom layers',
      type=str,
      default=(os.path.join(os.path.dirname(sys.argv[0]), 'extensions', 'front', 'caffe', 'CustomLayersMapping.xml')),
      action=CanonicalizePathCheckExistenceAction)
    caffe_group.add_argument('--mean_file', '-mf', help='Mean image to be used for the input. Should be a binaryproto file',
      default=None,
      action=CanonicalizePathCheckExistenceAction)
    caffe_group.add_argument('--mean_file_offsets', '-mo', help='Mean image offsets to be used for the input binaryproto file. When the mean image is bigger than the expected input, it is cropped. By default, centers of the input image and the mean image are the same and the mean image is cropped by dimensions of the input image. The format to pass this option is the following: "-mo (x,y)". In this case, the mean file is cropped by dimensions of the input image with offset (x,y) from the upper left corner of the mean image',
      default=None)
    caffe_group.add_argument('--disable_omitting_optional', help='Disable omitting optional attributes to be used for custom layers. Use this option if you want to transfer all attributes of a custom layer to IR. Default behavior is to transfer the attributes with default values and the attributes defined by the user to IR.',
      action='store_true',
      default=False)
    caffe_group.add_argument('--enable_flattening_nested_params', help='Enable flattening optional params to be used for custom layers. Use this option if you want to transfer attributes of a custom layer to IR with flattened nested parameters. Default behavior is to transfer the attributes without flattening nested parameters.',
      action='store_true',
      default=False)
    return parser
...
def get_tf_cli_parser(parser: argparse.ArgumentParser=None):
    ...
    tf_group.add_argument('--input_model_is_text', help='TensorFlow*: treat the input model file as a text protobuf format. If not specified, the Model Optimizer treats it as a binary file by default.',
      action='store_true')
    tf_group.add_argument('--input_checkpoint', type=str, default=None, help='TensorFlow*: variables file to load.', action=CanonicalizePathCheckExistenceAction)
    tf_group.add_argument('--input_meta_graph', help='Tensorflow*: a file with a meta-graph of the model before freezing',
      action=CanonicalizePathCheckExistenceAction,
      type=readable_file)
    tf_group.add_argument('--saved_model_dir', default=None, help='TensorFlow*: directory representing non frozen model',
      action=CanonicalizePathCheckExistenceAction,
      type=readable_dirs)
    tf_group.add_argument('--saved_model_tags', type=str, default=None, help="Group of tag(s) of the MetaGraphDef to load, in string format, separated by ','. For tag-set contains multiple tags, all tags must be passed in.")
    tf_group.add_argument('--tensorflow_custom_operations_config_update', help='TensorFlow*: update the configuration file with node name patterns with input/output nodes information.',
      action=CanonicalizePathCheckExistenceAction)
    tf_group.add_argument('--tensorflow_use_custom_operations_config', help='Use the configuration file with custom operation description.',
      action=DeprecatedCanonicalizePathCheckExistenceAction)
    tf_group.add_argument('--tensorflow_object_detection_api_pipeline_config', help='TensorFlow*: path to the pipeline configuration file used to generate model created with help of Object Detection API.',
      action=CanonicalizePathCheckExistenceAction)
    tf_group.add_argument('--tensorboard_logdir', help='TensorFlow*: dump the input graph to a given directory that should be used with TensorBoard.',
      default=None,
      action=CanonicalizePathCheckExistenceAction)
    tf_group.add_argument('--tensorflow_custom_layer_libraries', help='TensorFlow*: comma separated list of shared libraries with TensorFlow* custom operations implementation.',
      default=None,
      action=CanonicalizePathCheckExistenceAction)
    tf_group.add_argument('--disable_nhwc_to_nchw', help='Disables default translation from NHWC to NCHW',
      action='store_true')
    return parser
...
def get_mxnet_cli_parser(parser: argparse.ArgumentParser=None):
    ...
    mx_group.add_argument('--input_symbol', help='Symbol file (for example, model-symbol.json) that contains a topology structure and layer attributes',
      type=str,
      action=CanonicalizePathCheckExistenceAction)
    mx_group.add_argument('--nd_prefix_name', help='Prefix name for args.nd and argx.nd files.',
      default=None)
    mx_group.add_argument('--pretrained_model_name', help='Name of a pretrained MXNet model without extension and epoch number. This model will be merged with args.nd and argx.nd files',
      default=None)
    mx_group.add_argument('--save_params_from_nd', action='store_true',
      help='Enable saving built parameters file from .nd files')
    mx_group.add_argument('--legacy_mxnet_model', action='store_true',
    ...
    mx_group.add_argument('--enable_ssd_gluoncv', action='store_true',
...
def get_kaldi_cli_parser(parser: argparse.ArgumentParser=None):
    ...
    kaldi_group.add_argument('--counts', help='Path to the counts file',
      default=None,
      action=CanonicalizePathCheckExistenceAction)
    kaldi_group.add_argument('--remove_output_softmax', help='Removes the SoftMax layer that is the output layer',
      action='store_true',
      default=False)
    kaldi_group.add_argument('--remove_memory', help='Removes the Memory layer and use additional inputs outputs instead',
      action='store_true',
      default=False)
    return parser
...
def get_all_cli_parser():
    ...
    """
    parser = argparse.ArgumentParser(usage='%(prog)s [options]')
    parser.add_argument('--framework', help='Name of the framework used to train the input model.',
      type=str,
      choices=[
...
def get_data_type_from_input_value(input_value: str):
    ...
    """
    data_type_match = re.match('.*\\{(.*)\\}.*', input_value)
    if data_type_match is not None:
        return destination_type_to_np_data_type(data_type_match.group(1))
...
def get_shape_from_input_value(input_value: str):
    ...
    shape = re.findall('[(\\[]([0-9  -]*)[)\\]]', input_value)
    if len(shape) == 0:
        shape = None
    else:
        if len(shape) == 1:
            ...
            raise Error('Wrong syntax to specify shape. Use --input "node_name[shape]->value"')
    return shape
...
def get_value_from_input_value(input_value: str):
    ...
    if len(parts) == 2:
        ...
        value = isinstance(value, list) or ast.literal_eval(value)
    else:
        if len(parts) > 2:
            raise Error('Wrong syntax to specify value. Use --input "node_name[shape]->value"')
    return value
    ...
    if shape is not None:
        if value is not None:
            if np.prod(shape) != value_size:
                raise Error("The shape '{}' of the input node '{}' does not correspond to the number of elements '{}' in the value: {}".format(shape, node_name, value_size, value))
    return (
     node_name, shape, value, data_type)
...
def get_freeze_placeholder_values(argv_input: str, argv_freeze_placeholder_with_value: str):
    ...
    if argv_freeze_placeholder_with_value is not None:
        for plh_with_value in argv_freeze_placeholder_with_value.split(','):
            ...
            if len(plh_with_value) != 2:
                raise Error('Wrong replacement syntax. Use --freeze_placeholder_with_value "node1_name->value1,node2_name->value2"')
            node_name = plh_with_value[0]
            value = plh_with_value[1]
            if node_name in placeholder_values:
                if placeholder_values[node_name] != value:
                    raise Error("Overriding replacement value of the placeholder with name '{}': old value = {}, new value = {}.".format(node_name, placeholder_values[node_name], value))
            if '[' in value.strip(' '):
                value = value.replace('[', '').replace(']', '').split(' ')
    ...
    return (
     placeholder_values, input_node_names)
...
def get_placeholder_shapes(argv_input: str, argv_input_shape: str, argv_batch=None):
    ...
    if argv_input_shape:
        if argv_batch:
            raise Error('Both --input_shape and --batch were provided. Please provide only one of them. ' + refer_to_faq_msg(56))
        else:
            ...
            if argv_input_shape:
                ...
                elif argv_batch and are_shapes_specified_through_input:
                    raise Error('Shapes are specified using both --input and --batch command-line parameters, but only one parameter is allowed.')
                if are_shapes_specified_through_input:
                    return (
                     placeholder_shapes, placeholder_data_types)
                shapes = list()
                inputs = list()
                placeholder_shapes = None
                first_digit_reg = '([0-9 ]+|-1)'
                next_digits_reg = '(,{})*'.format(first_digit_reg)
                tuple_reg = '((\\({}{}\\))|(\\[{}{}\\]))'.format(first_digit_reg, next_digits_reg, first_digit_reg, next_digits_reg)
                if argv_input_shape:
                    full_reg = '^{}(\\s*,\\s*{})*$|^$'.format(tuple_reg, tuple_reg)
                    if not re.match(full_reg, argv_input_shape):
                        raise Error('Input shape "{}" cannot be parsed. ' + refer_to_faq_msg(57), argv_input_shape)
                    shapes = re.findall('[(\\[]([0-9, -]+)[)\\]]', argv_input_shape)
                if argv_input:
        ...
        if not argv_input or len(shapes) == len(inputs) or len(shapes) == 0:
            inputs = list(map(lambda x: x.split('->')[0], inputs))
            placeholder_shapes = dict(zip_longest(inputs, map(lambda x:             if x:
np.fromstring(x, dtype=(np.int64), sep=',') # Avoid dead code: None, shapes)))
        else:
            if argv_input:
...
def parse_tuple_pairs(argv_values: str):
    ...
    while 1:
        tuples_matches = re.findall('[(\\[]([0-9., -]+)[)\\]]', data_str, re.IGNORECASE)
        if not tuples_matches:
            raise Error('Mean/scale values should be in format: data(1,2,3),info(2,3,4) or just plain set of them without naming any inputs: (1,2,3),(2,3,4). ' + refer_to_faq_msg(101), argv_values)
        ...
        if not input_name:
            ...
            for i in range(0, len(matches)):
                if re.search(words_reg, matches[i]) is not None:
                    raise Error('Mean/scale values should either contain names of input layers: data(1,2,3),info(2,3,4) or just plain set of them without naming any inputs: (1,2,3),(2,3,4).' + refer_to_faq_msg(101), argv_values)
...
def get_tuple_values(argv_values: str or tuple, num_exp_values: int=3, t=float or int):
    ...
    Returns:
        tuple of values
    """
    digit_reg = '(-?[0-9. ]+)' if t == float else '(-?[0-9 ]+)'
    assert num_exp_values > 1, 'Can not parse tuple of size 1'
    content = '{0}\\s*,{1}\\s*{0}'.format(digit_reg, (digit_reg + ',') * (num_exp_values - 2))
    tuple_reg = '((\\({0}\\))|(\\[{0}\\]))'.format(content)
    if isinstance(argv_values, tuple):
        if not len(argv_values):
            return argv_values
    if not (len(argv_values) and re.match(tuple_reg, argv_values)):
        raise Error('Values "{}" cannot be parsed. ' + refer_to_faq_msg(59), argv_values)
    mean_values_matches = re.findall('[(\\[]([0-9., -]+)[)\\]]', argv_values)
    for mean in mean_values_matches:
        if len(mean.split(',')) != num_exp_values:
            raise Error('{} channels are expected for given values. ' + refer_to_faq_msg(60), num_exp_values)
...
def get_mean_scale_dictionary(mean_values, scale_values, argv_input: str):
    ...
    if argv_input:
        ...
        elif type(mean_values) is dict and type(scale_values) is dict:
            if not mean_values:
                if not scale_values:
                    return res
            for inp in inputs:
                ...
                if inp in mean_values or inp in scale_values:
                    res.update({inp: {'mean':mean_values[inp] if inp in mean_values else None, 
                           'scale':scale_values[inp] if inp in scale_values else None}})
        ...
        if inputs:
            if mean_values:
                if scale_values:
                    if len(inputs) != len(mean_values):
                        raise Error('Numbers of inputs and mean values do not match. ' + refer_to_faq_msg(61))
                    if len(inputs) != len(scale_values):
                        raise Error('Numbers of inputs and scale values do not match. ' + refer_to_faq_msg(62))
                    data = list(zip(mean_values, scale_values))
                    for i in range(len(data)):
                        res.update({inputs[i]: {'mean':data[i][0], 
                                     'scale':data[i][1]}})
            ...
            if mean_values:
                ...
                for i in range(len(data)):
                    res.update({inputs[i]: {'mean':data[i], 
                                 'scale':None}})
...
def get_model_name(path_input_model: str) -> str:
    ...
    if parsed_name.startswith('.') or len(parsed_name) == 0:
        return 'model'
    return parsed_name
