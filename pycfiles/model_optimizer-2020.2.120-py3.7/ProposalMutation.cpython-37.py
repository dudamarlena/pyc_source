...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/back/ProposalMutation.py
...
import extensions.back.StridedSliceMasksNormalizer as StridedSliceMasksNormalizer
...
class ProposalMutation(BackReplacementPattern):
    ...
    def run_before(self):
        return [
         ReshapeMutation, StridedSliceMasksNormalizer]
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        if np.array_equal(im_info_shape, [1, 6]):
            log.error(('The model contains Proposal layer "{}" with input of shape [1, 6]. Inference Engine implementation of the Proposal layer uses only 4 first values (indices 0, 1, 2 and 3). Elements with indices 4 and 5 will be ignored.'.format(node.soft_get('name', node.id))),
            ...
            cropped_im_info = StridedSlice(graph, {'name':'cropped_im_info',  'begin_mask':int64_array([1, 1]), 
