...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/reverse_tensor_iterator.py
...
class ReverseTensorIteratorLSTM(MiddleReplacementPattern):
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        if not direct_reverse.seq_axis == inverse_reverse.seq_axis:
            raise AssertionError
        else:
            if not (direct_reverse.batch_axis is None and inverse_reverse.batch_axis is None):
                assert direct_reverse.batch_axis == inverse_reverse.batch_axis
            return self.is_fusable_reverse_sequence(direct_reverse) and self.is_fusable_reverse_sequence(inverse_reverse) or None
        for port_map in [ti.input_port_map, ti.output_port_map]:
            for port in port_map:
                if 'axis' in port:
                    if port['axis'] is not None and 'external_port_id' in port:
                        assert port['axis'] == direct_reverse.seq_axis, 'axis == {} != {} == direct_reverse.seq_dim'.format(port['axis'], direct_reverse.seq_axis)
