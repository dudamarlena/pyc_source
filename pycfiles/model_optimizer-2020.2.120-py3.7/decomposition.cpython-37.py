...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/middle/passes/fusing/decomposition.py
        ...
        if node.has_valid('op'):
            if node.op in ('FusedBatchNorm', 'FusedBatchNormV2', 'FusedBatchNormV3',
                ...
                if any([node.in_port(i).data.get_value() is None for i in range(1, len(node.in_ports()))]):
                    log.warning('Cannot translate FusedBatchNorm {} node with non-constant weights'.format(node.name if node.has_valid('name') else '<UNKNOWN>'))
                    continue
                ...
                eps = node.eps
                if node.has_valid('fix_gamma'):
                    if node.fix_gamma:
                        const.data.get_value().fill(1.0)
            can_be_fused = False if not node.soft_get('can_be_fused') else True
            scale = 1.0 / np.sqrt(variance.data.get_value() + eps)
...
def _fused_batch_norm_decomposition(graph: Graph, tinput: Port, toutput: Port, gamma: Port, beta: Port, mean: np.ndarray, variance: np.ndarray, can_be_fused=True):
    """
...
def convert_scale_shift_to_mul_add(graph: Graph):
    ...
    for node in nodes:
        ...
        has_biases = True
        has_weights = True
        if (shift_port is None or shift_port.data.get_value()) is not None:
            if all([x == 0 for x in shift_port.data.get_value()]):
                has_biases = False
        if not scale_port is None:
            if scale_port.data.get_value() is not None:
                if all([x == 1 for x in scale_port.data.get_value()]):
                    has_weights = False
            else:
                ...
                def broadcast_with_reshape(port):
                    ...
                    reshape = create_op_node_with_second_input(graph, Reshape, reshape_dims, dict(name=(port.node.name + '/Broadcast_')))
