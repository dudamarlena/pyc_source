...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/ops/ReduceOps.py
...
def reduce_infer(node: Node):
    ...
    assert len(connected_in_ports) == 2, '{} node `{}` should have 2 input ports, where 0-input is data input and 1-input represent `reduction_indices`'.format(node.op, node.id)
    in_data = node.in_port(0).data
    in_shape = in_data.get_shape()
    axis = node.in_port(1).data.get_value()
    if axis.size == 1:
        if axis.item() is None:
...
class ReduceOp(Op):
    ...
    def __init__(self, graph, attrs):
        super().__init__(graph, {'op':self.op, 
         ...
         'in_ports_count':2, 
         'out_ports_count':1, 
         'force_precision_in_ports':{1: 'int64' if graph.graph['cmd_params'].generate_experimental_IR_V10 else 'int32'}}, attrs)
        if not isinstance(self.attrs['keep_dims'], int):
            assert isinstance(self.attrs['keep_dims'], bool)
