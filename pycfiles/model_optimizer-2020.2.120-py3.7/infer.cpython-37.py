...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/middle/passes/infer.py
# Compiled at: 2020-05-01 08:37:22
# Size of source mod 2**32: 14578 bytes
"""
...
def control_flow_infer(graph: Graph, node_name: str):
    ...
    def mark_executability(node_id, is_executable):
        if is_executable:
            if not graph.node[node_id]['executable']:
                return
        graph.node[node_id]['executable'] = is_executable

    in_edges_with_data = graph.in_edges(node_name, data=True)
    in_df_edges_with_data = [attrs['control_flow_edge'] or (u, v, attrs) for u, v, attrs in in_edges_with_data if not 'control_flow_edge' not in attrs]
    in_cf_edges_with_data = [(u, v, attrs) for u, v, attrs in in_edges_with_data if 'control_flow_edge' in attrs if attrs['control_flow_edge']]
    is_executable_df = all([graph.node[u]['executable'] for u, _, attrs in in_df_edges_with_data] if len(in_df_edges_with_data) else [True])
    is_executable_cf = all([graph.node[u]['executable'] for u, _, attrs in in_cf_edges_with_data] if len(in_cf_edges_with_data) else [True])
...
def partial_infer(graph: Graph, start_node: str=None):
    ...
    nx.set_node_attributes(G=graph, name='executable', values={n:True for n in graph.get_nodes_with_attributes(kind='data')})
    for n in nodes:
        try:
            node = Node(graph, n)
            node_name = node.soft_get('name')
            if node.has('is_partial_inferred'):
                if (node.is_partial_inferred or node.has)('infer') and node.infer is not None:
                    if debug_logger:
                        log.debug('--------------------')
                    ...
                    for out_port, out_node in out_nodes.items():
                        ...
                        if not out_node.has_valid('shape'):
                            ...
                            is_fully_defined_shape(out_node.shape) or log.error('Shape {} is not fully defined for output {} of "{}". Use --input_shape with positive integers to override model input shapes.'.format(out_node.shape, out_port, node_name))
                            not_all_output_shapes = True

                    if not_all_output_shapes:
                        raise Error('Not all output shapes were inferred or fully defined for node "{}". ' + refer_to_faq_msg(40), node_name)
                    else:
                        if node.kind != 'data':
                            raise Error('There is no registered "infer" function for node "{}" with op = "{}". Please implement this function in the extensions. ' + refer_to_faq_msg(37), node_name, node.soft_get('op'))
            ...
            try:
                ...
                debug_messages = '\n'.join(['Layer "' + node_name + '": ' + node_attrs['debug_message'] for node_name, node_attrs in graph.nodes(data=True) if 'debug_message' in node_attrs])
                if debug_messages != '':
                    log.error('')
                    log.error('Other possible failure reasons are listed below:')
                    log.error(debug_messages)
                elif not debug_logger:
                ...
                raise Error('Stopped shape/value propagation at "{}" node. '.format(node.soft_get('name')) + refer_to_faq_msg(38)) from err
            finally:
                err = None
                del err
...
def type_infer(graph: Graph):
    ...
    for n in nodes:
        ...
        if node.kind == 'op':
            ...
            log.debug('Type infer for node {}: {}'.format(node_name, [port.get_data_type() for port in node.out_ports().values()]))
