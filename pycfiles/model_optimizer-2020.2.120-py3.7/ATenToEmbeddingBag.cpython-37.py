...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/ATenToEmbeddingBag.py
...
class AtenToEmbeddingBag(FrontReplacementPattern):
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for node in graph.get_op_nodes(op='ATen', operator='embedding_bag'):
            ...
            embedding_bag = EmbeddingBag(graph, {'name':node_name,  'mode':node.mode,  'scale_grad_by_freq':node.scale_grad_by_freq}).create_node()
