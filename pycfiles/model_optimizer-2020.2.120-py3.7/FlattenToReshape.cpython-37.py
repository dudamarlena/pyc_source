...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/tf/FlattenToReshape.py
...
class FlattenToReshapeableReshape(FrontReplacementSubgraph):
    ...
    def replace_sub_graph(graph: Graph, match: dict):
        ...
        if not (const_node.has_valid('value') and is_value_is_constant(const_node.value, -1)):
            log.debug('The pattern does not correspond to flatten. The second reshape dimension is not -1. It is {}'.format(const_node.soft_get('value')))
            return
        if len(pack_node.in_nodes()) != 2:
            log.debug('The pattern does not correspond to flatten. The "Pack" operation produces tensor with 3 items but should produce just 2.')
            return
        expected_values = [
         0, 1, 1]
        for ind in range(3):
            if strided_slice_node.in_node(ind + 1).has_valid('value'):
                is_value_is_constant(strided_slice_node.in_node(ind + 1).value, expected_values[ind]) or log.debug('The pattern does not correspond to flatten because of the input with index {}. The value is "{}".'.format(ind, strided_slice_node.soft_get('value')))
                return
