...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/LSTMRNNSequenceToTensorIterator.py
...
class LSTMToTensorIterator(MiddleReplacementPattern):
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        inputs = [Op._create_data_node(body, lstm.name + '/inport/' + str(inp), {'shape':lstm.in_node(inp).shape.copy(),  'value':lstm.in_node(inp).value.copy() if lstm.in_node(inp).value is not None and inp in (1,
        ...
        squeeze_dim_data = Const(body, {'name':lstm.name + '/input_squeeze_dim',  'value':[
          lstm.sequence_dim]}).create_node_with_data()
        inputs[0] = input_squeeze.create_node_with_data([inputs[0], squeeze_dim_data], edge_attrs=[
         {'internal_port_id': 0}])
        outputs = [Op._create_data_node(body, lstm.name + '/outport/' + str(out), {'shape': lstm.out_node(out).shape.copy() if out in lstm.out_nodes() else lstm.in_node(4).shape.copy()}) for out in (0,
                                                                                                                                                                                                       1)]
        for out in outputs:
        ...
        unsqueeze_dim_data = Const(body, {'name':lstm.name + '/output_unsqueeze_dim',  'value':[
          lstm.sequence_dim]}).create_node_with_data()
        lstm_cell_op = LSTMCell(body, dict(hidden_size=(lstm.hidden_size), activations=(lstm.activations),
          ...
          internal_layer_id=1))
        lstm_cell_node = lstm_cell_op.create_node_with_data(inputs, data_nodes=outputs, edge_attrs=[{}, {'internal_port_id': 1},
        ...
        if len(lstm.out_nodes()) == 3:
            output_port_map.extend([
             ...
             {'external_port_id':5, 
              'internal_layer_id':1, 
              'internal_port_id':5}])
        ti_op = TensorIterator(graph, {'name':lstm.name + '/TensorIterator', 
         'body':body, 
         'in_ports_count':3, 
         ...
         'back_edges':[
          ...
          {'from_layer':1, 
           ...
           'to_layer':1, 
           'to_port':2}]})
        assert sorted(lstm.out_nodes().keys()) == list(range(len(lstm.out_nodes()))), 'There are gaps in output ports of LSTMSequence operation. Node {}'.format(lstm.id)
        outs = ti_op.create_node_with_data([lstm.in_node(i) for i in (0, 4, 5)], data_nodes=[lstm.out_node(i) for i in range(len(lstm.out_nodes()))],
          edge_attrs=[
