...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/onnx/mask_rcnn_conversion.py
...
def insert_do(graph: Graph, replacement_descriptions):
    ...
    num_classes = 81
    box_regressions_node = create_op_node_with_second_input(graph, Reshape, int64_array([-1, 4 * num_classes]), dict(name='box_regressions'), Node(graph, '2773'))
    class_predicitons_node = Node(graph, '2774')
    im_info_node = Parameter(graph, {'name':'im_info',  'shape':int64_array([1, 3])}).create_node()
    do_node = ExperimentalDetectronDetectionOutput(graph, {'name':'DetectionOutput',  'class_agnostic_box_regression':0, 
     'deltas_weights':np.array([10.0, 10.0, 5.0, 5.0]), 
     'max_delta_log_wh':replacement_descriptions['max_delta_log_wh'], 
     'nms_threshold':replacement_descriptions['nms_threshold'], 
     'score_threshold':replacement_descriptions['score_threshold'], 
     'num_classes':num_classes, 
     'max_detections_per_image':replacement_descriptions['max_detections_per_image'], 
     'post_nms_count':replacement_descriptions['post_nms_count']}).create_node()
    ...
    do_output_ports = [
     do_node.out_port(0), do_node.out_port(1), do_node.out_port(2)]
...
def insert_ExperimentalDetectronROIFeatureExtractor1(graph: Graph):
    ...
    fpn_roi_align = ExperimentalDetectronROIFeatureExtractor(graph, {'name':'ROIFeatureExtractor_1',  'distribute_rois_between_levels':1, 
     'image_id':0, 
     'output_size':14, 
...
def insert_ExperimentalDetectronROIFeatureExtractor2(graph: Graph):
    ...
    input_fpn_head_nodes = [Node(graph, node_id) for node_id in input_fpn_heads]
    fpn_roi_align = ExperimentalDetectronROIFeatureExtractor(graph, {'name':'ROIFeatureExtractor_2',  'distribute_rois_between_levels':1, 
     'image_id':0, 
     'output_size':7, 
     'preserve_rois_order':1, 
     'pyramid_scales':int64_array([
