...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/GRURNNSequenceToTensorIterator.py
# Compiled at: 2020-05-01 08:37:21
# Size of source mod 2**32: 9473 bytes
"""
...
class GRUAndRNNToTensorIterator(MiddleReplacementPattern):
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        if match['rnn_layer']['op'] == 'LSTM':
            ...
            inputs = [Op._create_data_node(body, rnn_layer.name + '/inport/' + str(inp), {'shape':rnn_layer.in_node(inp).shape.copy(),  'value':rnn_layer.in_node(inp).value.copy() if rnn_layer.in_node(inp).value is not None and inp in (1,
                                                                                            2) else None}) for inp in (0,
                                                                                                                       4,
                                                                                                                       1,
                                                                                                                       2)]
            ...
            input_squeeze_dim = Const(body, dict(name=(rnn_layer.name + '/input_squeeze_dim'), value=(rnn_layer.sequence_dim))).create_node_with_data()
            inputs[0] = input_squeeze.create_node_with_data([inputs[0], input_squeeze_dim], edge_attrs=[
             {'internal_port_id': 0}])
            outputs = [Op._create_data_node(body, rnn_layer.name + '/outport/' + str(out), {'shape': rnn_layer.out_node(out).shape.copy() if out in rnn_layer.out_nodes() else None}) for out in (0, )]
            for out in outputs:
                add_opoutput(body, out.id, 0, False)

            outputs[0].shape = np.delete(outputs[0].shape.copy(), rnn_layer.sequence_dim)
            output_unsqueeze_dim = Const(body, dict(name=(rnn_layer.name + '/output_unsqueeze_dim'), value=(rnn_layer.sequence_dim))).create_node_with_data()
            output_unsqueeze = Unsqueeze(body, dict(name=(rnn_layer.name + '/output_unsqueeze/'), internal_layer_id=2))
            additional_attrs = dict(activations=(rnn_layer.activations), activation_alpha=(rnn_layer.activation_alpha),
              activation_beta=(rnn_layer.activation_beta),
              clip=(rnn_layer.clip))
            if rnn_layer.op == 'GRU':
                additional_attrs['linear_before_reset'] = rnn_layer.linear_before_reset
            rnn_cell_op = self.get_rnn_cell(rnn_layer['op'])(body, dict(hidden_size=rnn_layer.hidden_size, name=rnn_layer.name + '/{}Cell'.format(rnn_layer.op), **additional_attrs, **{'internal_layer_id': 1}))
            gru_cell = rnn_cell_op.create_node_with_data(inputs, data_nodes=outputs, edge_attrs=[{}, {'internal_port_id': 1},
             {'internal_port_id': 2}, {'bin': 'weights'},
             {'bin': 'biases'}])
            gru_cell.in_node().out_edge(0)['internal_port_id'] = 4
            ...
            if not rnn_layer.direction in ('forward', 'reverse'):
            ...
            if len(rnn_layer.out_nodes()) == 2:
                output_port_map.extend([
                 {'external_port_id':4, 
                  'internal_layer_id':1, 
                  'internal_port_id':4}])
            ti_op = TensorIterator(graph, {'name':rnn_layer.name + '/TensorIterator', 
             'body':body, 
             'in_ports_count':4, 
             ...
             'back_edges':[
              {'from_layer':1, 
               ...
               'to_layer':1, 
               'to_port':1}]})
            assert sorted(rnn_layer.out_nodes().keys()) == list(range(len(rnn_layer.out_nodes()))), 'There are gaps in output ports of GRUSequence operation. Node {}'.format(rnn_layer.id)
            outs = ti_op.create_node_with_data([rnn_layer.in_node(i) for i in (0, 4)], data_nodes=[rnn_layer.out_node(i) for i in range(len(rnn_layer.out_nodes()))],
              edge_attrs=[
             {'external_port_id': 0}, {'external_port_id': 1}])
            outs = isinstance(outs, list) or list([outs])
