...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/front/caffe/collect_attributes.py
...
def append_unique(attrs, new_attr, value):
    if new_attr in attrs:
        log.error('The parameter {} overwrites already existing value. '.format(new_attr) + 'This happens due to flattening nested parameters. ' + 'Use enable_flattening_nested_params to flatten nesting')
    return {new_attr: value}


def append_unique_enum(attrs: dict, descriptor, value):
    enum_name = '{}.{}'.format(descriptor.enum_type.full_name.rsplit('.', 1)[0], descriptor.enum_type.values[value].name)
...
def collect_optional_attributes(obj, prefix: str='', disable_omitting_optional: bool=False, enable_flattening_nested_params: bool=False):
    """
    ...
    for descriptor in obj.DESCRIPTOR.fields:
        ...
        if descriptor.has_default_value or disable_omitting_optional or descriptor.name in fields:
            if descriptor.type == descriptor.TYPE_MESSAGE:
                attrs.update(collect_optional_attributes(value, prefix=name,
...
def collect_attributes(obj, prefix: str='', disable_omitting_optional: bool=False, enable_flattening_nested_params: bool=False):
    """
