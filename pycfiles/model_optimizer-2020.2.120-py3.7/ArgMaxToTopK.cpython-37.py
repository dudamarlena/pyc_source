...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/ArgMaxToTopK.py
...
class ArgMaxToTopK(MiddleReplacementPattern):
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        topk_node = TopK(graph, {'axis':axis,  'mode':'max',  'sort':'index',  'remove_values_output':node.has_and_set('remove_values_output')}).create_node()
        ...
        if node.has_and_set('out_max_val'):
            ...
            topk_node.out_port(0).connect(concat_node.in_port(1))
            topk_node.out_port(1).connect(concat_node.in_port(0))
            node.out_port(0).disconnected() or node.out_port(0).get_connection().set_source(concat_node.out_port(1))
        else:
            if not node.out_port(0).disconnected():
                node.out_port(0).get_connection().set_source(topk_node.out_port(1))
            topk_node.in_port(1).connect(Const(graph, {'name':node.soft_get('name') + '/TopK',  'value':node.top_k}).create_node().out_port(0))
