...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/back/CropToStridedSlice.py
# Compiled at: 2020-05-01 08:37:19
# Size of source mod 2**32: 5196 bytes
"""
...
"""
import numpy as np
import extensions.back.ForceStrictPrecision as ForceStrictPrecision
...
class CropToStridedSlice(BackReplacementPattern):
    ...
    def replace_pattern(self, graph: Graph, match: [str, Node]):
        ...
        if len(node.in_nodes()) == 2 and node.has_valid('offset'):
            ...
            if node.has_valid('dim') and node.has_valid('offset'):
                ...
                if node.has_valid('crop_begin') and node.has_valid('crop_end'):
                    ...
                    shape = Shape(graph, {'name': node.name + '/shape_of_crop'}).create_node()
                    const = Const(graph, {'value': -1 * self.mask_normalizer(shape_rank, node.axis, node.crop_end)}).create_node()
                    end = Add(graph, {'name': node.name + '/end'}).create_node()
        ...
        ss = StridedSlice(graph, {'name':'Crop_',  'begin_mask':begin_mask,  'end_mask':end_mask,  'new_axis_mask':np.array([0]),  'shrink_axis_mask':np.array([0]), 
        ...
        ss['force_precision_in_ports'] = {1:'int64', 
         2:'int64',  3:'int64'}