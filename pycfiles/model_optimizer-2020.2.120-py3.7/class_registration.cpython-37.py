...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/utils/class_registration.py
...
"""
import logging as log, networkx as nx, os
...
def _check_unique_ids():
    ...
    for class_type, classes_set in _registered_classes_dict.items():
        for cls in classes_set:
            replacers = [c for c in cls.registered_cls if not hasattr(c, 'op')] + [c for op, c in cls.registered_ops.items() if c]
...
def _update(cls, registered_list: list, registered_dict: dict, key: str, enabled_transforms: list, disabled_transforms: list):
    ...
    for c in cls.__subclasses__():
        if hasattr(c, 'id'):
            if c.id in enabled_transforms:
                setattr(c, 'enabled', True)
            elif hasattr(c, 'id') and c.id in disabled_transforms:
                setattr(c, 'enabled', False)
            if c not in registered_list:
                if hasattr(cls, 'excluded_classes'):
                    if c in cls.excluded_classes:
                        continue
                else:
                    ...
                    if hasattr(c, key):
                        if getattr(c, key) is not None:
                            ...
                            if k.lower() in new_keys_lower:
                                raise Error('Attempt to register of custom name {} for the second time as class {}. Note that custom names are case-insensitive. ' + refer_to_faq_msg(55), k, c)
                            else:
...
class DependencyGraph(Graph):
    ...
    def dump_graph_for_graphviz(self, node_attrs: list=[], edge_attrs: list=[], nodes_to_dump: list=None, save_to_svg=False, highlight_nodes: list=None):
        ...
        for src_node_name, dst_node_name, attrs in self.edges(data=True):
            ...
            if not src_node_name not in nodes_to_dump:
                if dst_node_name not in nodes_to_dump:
                    continue
                src_node = self.node[src_node_name]
                dst_node = self.node[dst_node_name]
                src_node_string = str(src_node_name) + '\\n'.join([str(key) + '=' + str(src_node.get(key, 'None')) for key in node_attrs if key in src_node])
                dst_node_string = str(dst_node_name) + '\\n'.join([str(key) + '=' + str(dst_node.get(key, 'None')) for key in node_attrs if key in dst_node])
    ...
    def cycle_check(self):
        try:
            ...
            try:
                ...
                raise Error('There is(are) cyclic dependency(ies) between replacers. One of the cycles is the following: {}', ' -> '.join([str(node) for node in list(cycles)[0]])) from exception
            finally:
                exception = None
                del exception
    ...
    def repeated_cls_names_check(self):
        ...
        for transform_class in self.node:
            ...
            assert transform_name not in name_to_class_map, 'Transform name `{}` is not unique: at least {} and {} exist'.format(transform_name, transform_class, name_to_class_map[transform_name])
    ...
    def determined_sort(self):
        ...
        try:
            ...
            try:
                raise InternalError('Internal DependencyGraph determined_sort function behaves unexpectedly: cycle found') from e
            finally:
                e = None
                del e

        assert nx_order == order, 'Internal DependencyGraph determined_sort function behaves unexpectedly: nx_order != order'
        return order
...
def get_replacers_order(transform_types: list):
    ...
    for class_type, classes_set in _registered_classes_dict.items():
        if class_type in transform_types:
            for cls in classes_set:
                cur_cls_replacers = [c for c in cls.registered_cls if not hasattr(c, 'op')] + [c for op, c in cls.registered_ops.items() if c]
...
def apply_transform(graph: Graph, replacer_cls, **kwargs):
    ...
    if hasattr(replacer, 'enabled'):
        if not replacer.enabled:
            log.info('Skip replacer {} (enabled = False)'.format(replacer_cls))
            return
    if hasattr(replacer, 'graph_condition'):
        if not all([condition(graph) for condition in replacer.graph_condition]):
    ...
    try:
        ...
        try:
            raise Error('Exception occurred during running replacer "{}" ({}): {}'.format(replacement_id, replacer_cls, str(err).replace('[REPLACEMENT_ID]', replacement_id))) from err
        finally:
            err = None
            del err
        ...
        try:
            raise Exception('Exception occurred during running replacer "{} ({})": {}'.format(replacement_id, replacer_cls, str(err).replace('[REPLACEMENT_ID]', replacement_id))) from err
        finally:
            err = None
            del err
