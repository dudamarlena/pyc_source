...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/NasNet.py
# Compiled at: 2020-05-01 08:37:21
# Size of source mod 2**32: 6439 bytes
"""
...
class NasNet(MiddleReplacementPattern):
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        if not np.array_equal(begin, int64_array([0, 1, 1, 0])):
            ...
            return
        if not (np.array_equal(sslice_node.end_mask, int64_array([0, 0, 0, 0])) and np.array_equal(sslice_node.begin_mask, int64_array([
         0, 1, 1, 0]))):
        ...
        conv_node = Convolution(graph, dict(name=conv_name, stride=(int64_array([1, 1, 1, 1])),
          ...
          output_feature_channel=0)).create_node()
        weights_const_node = Const(graph, dict(name=conv_weights_name, value=conv_weights, shape=(int64_array(conv_weights.shape)))).create_node()
        crop_node = Crop(graph, dict(name=(sslice_node.name + '/Crop_'), axis=(int64_array([1, 2])), dim=(int64_array([output_shape[1] - 1, output_shape[2] - 1])),
