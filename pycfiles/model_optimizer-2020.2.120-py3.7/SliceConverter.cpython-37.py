...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/SliceConverter.py
...
class ConvertSlice(MiddleReplacementPattern):
    ...
    def convert_onnx_slice_opset10(node: Node):
        ...
        ss = StridedSlice(graph, dict(new_axis_mask=np.zeros((len(output_shape)), dtype=(np.int32)), shrink_axis_mask=np.zeros((len(output_shape)), dtype=(np.int32)),
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        if len(node.in_nodes()) >= 3:
            if node.has_valid('format'):
                if node['format'] == 'onnx':
                    self.convert_onnx_slice_opset10(node)
                    return
        else:
            return node.has_valid('start') and node.has_valid('end') or None
        begin = node.start
        end = node.end
        ...
        slice_node_name = node.soft_get('name', node.id)
        if dims == 1 or dims == 0:
            ss = StridedSlice(graph, dict(new_axis_mask=np.zeros((len(output_data.shape)), dtype=(np.int32)), shrink_axis_mask=np.zeros((len(output_data.shape)), dtype=(np.int32)),
