...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/AddMeanScaleValues.py
...
class AddMeanScaleValues(MiddleReplacementPattern):
    ...
    def apply_scale(graph: Graph, input_node: Node, node_mean_scale_values: dict):
        if 'scale' in node_mean_scale_values:
    ...
    def apply_mean_value(graph: Graph, input_node: Node, node_mean_scale_values: dict):
        if 'mean' in node_mean_scale_values:
    ...
    def find_and_replace_pattern(self, graph: Graph):
        ...
        if not isinstance(values, dict):
            ...
            for idx, key in enumerate(input_nodes.keys()):
                values.update({input_nodes[key]['name']: {'mean':data[idx][0], 
                                            'scale':data[idx][1]}})
        ...
        for node_name in values:
            ...
            if node_id not in input_nodes:
                ...
                for placeholder in input_nodes.values():
                    ...
                    if placeholder.has('initial_node_name'):
                        if placeholder.initial_node_name == node_name and (port is None or placeholder_port == port):
                            pass
                        new_node_id = placeholder.id
                        break

                if new_node_id is None:
                    raise Error("Input with name {} wasn't found!".format(node_name) + refer_to_faq_msg(83))
                node_id = new_node_id
