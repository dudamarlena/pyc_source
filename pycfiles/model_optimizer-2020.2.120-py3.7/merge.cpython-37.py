...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/ops/merge.py
...
class Merge(Op):
    ...
    def merge_infer(node: Node):
        ...
        if len(inferred_nodes) < len(node.in_nodes()):
            ...
            inferred_and_executable = [n for n in node.in_nodes().values() if n['is_partial_inferred'] if 'executable' in n if n['executable']]
    ...
    def control_flow_infer(node: Node, is_executable: bool, mark_executability: callable):
        ...
        is_executable = any([d.has_and_set('executable') for i, d in in_data_nodes.items()] if len(in_data_nodes) else [False])
