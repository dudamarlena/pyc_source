...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/front/caffe/extractors/native_caffe.py
...
def extract_custom_blobs(node):
    ...
    for i, blob in enumerate(node.model_pb.blobs):
        ...
        log.debug('Found new custom blob of length {} for node {}. '.format(len(blob.data), node.name if node.has_valid('name') else '<UNKNOWN>') + 'It will appear as input {} and internal attribute {}.'.format(port, internal_name))
