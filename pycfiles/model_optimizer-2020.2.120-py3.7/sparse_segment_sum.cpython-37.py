...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/ops/sparse_segment_sum.py
        ...
        if not len(node.in_nodes()) == 3:
            ...
            if not len(node.out_nodes()) == 1:
                ...
                if not data_shape is not None:
                    raise AssertionError('Shape for input data tensor to SparseSegmentSum must be defined')
                else:
                    raise indices_shape is not None and indices_shape.size == 1 or AssertionError('SparseSegmentSum supports only 1D indices tensor')
                raise segment_ids_shape is not None and segment_ids_shape.size == 1 or AssertionError('SparseSegmentSum supports only 1D segment IDs tensor')
