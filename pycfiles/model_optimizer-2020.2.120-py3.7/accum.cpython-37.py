...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/ops/accum.py
...
class AccumOp(Op):
    ...
    def accum_infer(node: Node):
        ...
        if node.have_reference:
            ...
            top_height_ = node.in_node(num_inputs - 1).shape[2]
            top_width_ = node.in_node(num_inputs - 1).shape[3]
            height_ = top_height_
            width_ = top_width_
        else:
            max_height = -1
            ...
            if not total_channels >= 1:
                raise AssertionError('Accumulated layers must have some channels in total')
            elif node.size_divisible_by:
            ...
            if top_height_ > max_height and top_width_ > max_width:
                height_ = top_height_
                width_ = top_width_
            else:
                height_ = max_height
