...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/front/tf/partial_infer/tf.py
# Compiled at: 2020-05-01 08:37:22
# Size of source mod 2**32: 12333 bytes
"""
...
def tf_native_tf_node_infer(node: Node):
    ...
    nodes_to_extract = node_incoming_neighbourhood(node.graph, node.id, 10) + node_outcoming_neighbourhood(node.graph, node.id, 1)
    ...
    lists_to_init = [
     'input_nodes_names', 'output_tensors_names', 'nodes_order', 'internal_output_node_name',
     'real_input_dims']
...
def generate_feed_dict(graph: tf_v1.Graph, node: Node):
    ...
    for in_data_node_name, edge_attrs in node.get_inputs():
        if 'control_flow_edge' in edge_attrs:
            if edge_attrs['control_flow_edge']:
                continue
        value = node.in_node(edge_attrs['in']).value
        if value is None:
            all_constants = False
            placeholder_pb = node['pbs'][edge_attrs['placeholder_name']]
            value = np.ones(shape=(tf_tensor_shape(placeholder_pb.attr['shape'].shape)), dtype=(tf_dtype_extractor(placeholder_pb.attr['dtype'].type)))
        feed_dict[graph.get_tensor_by_name(edge_attrs['placeholder_name'] + ':0')] = value

    return (
     all_constants, feed_dict)
...
def tf_subgraph_infer(node: Node):
    ...
    for out_port, tensor_value in output_tensors.items():
        ...
        log.debug("Inferred shape of the output tensor with index '{}' of the node '{}': '{}'".format(str(out_port), node.name, out_node.shape))
        if all_constants:
            out_node.value = tensor_value


def add_node_def_to_subgraph(subgraph_node: Node, node_def: tf_v1.NodeDef, name: str=None, position: int=0, is_input: bool=False):
    """
...
def add_placeholders_to_subgraph(node: Node):
    ...
    for index, (in_data_node, edge_attrs) in enumerate(node.get_sorted_inputs()):
        if 'control_flow_edge' in edge_attrs:
            if edge_attrs['control_flow_edge']:
                continue
            elif 'internal_input_node_name' in edge_attrs.keys():
        ...
        if hasattr(pb, 'input'):
            for ind in range(len(pb.input)):
                if pb.input[ind] == old_input_tensor_name or pb.input[ind] == old_input_tensor_name_without_port:
                    ...
                    log.debug("Replacing input '{}' of the node '{}' with placeholder '{}'".format(ind, pb.name, new_input_name))
