...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/middle/pattern_match.py
...
import networkx.algorithms as ism
...
def apply_pattern(graph: Graph, nodes: list, edges: list, action: callable, node_attrs: list=None, edge_attrs: list=None):
    """
    ...
    if not all_edges_in_nodes([node[0] for node in nodes], edges):
        log.warning('Incorrect pattern attributes: not all nodes from edges are in nodes. Please, mention all nodes you need in pattern in nodes attribute. ')
    matches = []
...
def find_pattern_matches(graph: Graph, nodes: list, edges: list, node_attrs: list=None, edge_attrs: list=None):
    """
