...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/back/TileNormalizer.py
# Compiled at: 2020-05-01 08:37:19
# Size of source mod 2**32: 9137 bytes
"""
...
class TileInputAlignment(BackReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        if input_shape.size == tiles.size:
            return
        if input_shape.size < tiles.size:
            unsqueeze = create_op_node_with_second_input(graph, Unsqueeze, int64_array(list(range(tiles.size - input_shape.size))), {'name':name + '/input_alignment', 
             'override_output_shape':True})
            node.in_port(0).get_source().connect(unsqueeze.in_port(0))
            node.in_port(0).get_connection().set_source(unsqueeze.out_port(0))
        else:
            const = Const(graph, {'name':name + '/tile_alignment_const',  'value':np.ones([input_shape.size - tiles.size], dtype=np.int64)}).create_node()
...
class Tile3DReshaper(BackReplacementPattern):
    ...
    def run_before(self):
        return [
         ReshapeMutation, TileVersionDowngrader]
