...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/ops/convolution.py
...
from mo.front.common.partial_infer.utils import int64_array, float_array, mark_input_bins, assign_dims_to_weights, tf_window_op_pad_infer
...
','.join(map(str, get_backend_pad(node.pad, node.spatial_dims, 1))) # Avoid dead code: None),
             (
              'output_padding',
              lambda node:               if node.has_valid('output_padding'):
','.join(map(str, node.output_padding[node.spatial_dims])) # Avoid dead code: None),
             'pad_value',
             'mode']
        return [
         'auto_pad',
    ...
    def infer(node: Node):
        ...
        if input_shape is None:
            ...
            if node.has_valid('reshape_kernel') and node.reshape_kernel:
                if not (node.has_valid('output') and node.has_valid('channel_dims') and node.has_valid('group') and node.has_valid('kernel_spatial')):
        ...
        if not node.has_valid('kernel_spatial_idx'):
            ...
            if (node.has_valid('output') or node.type) in ('Convolution', 'Deconvolution',
                                                           'DeformableConvolution',
                                                           'BinaryConvolution'):
                node['output'] = kernel_shape[node.output_feature_channel]
            else:
                raise Error('Convolution infer function was called for a node {} with unsupported type {}', node.soft_get('name'), node.type)
        ...
        if node.has_valid('output_padding'):
            if len(input_shape) > len(node['output_padding']):
            ...
            if node.has_valid('auto_pad'):
                node['pad_spatial_shape'], node['output_spatial_shape'] = node.has_valid('output_spatial_shape') or tf_window_op_pad_infer(input_spatial_shape, kernel_extent, stride_spatial_shape, node.auto_pad, node.type == 'Deconvolution')
        ...
        if node.type in ('Convolution', 'BinaryConvolution'):
            float_spatial = Convolution.calc_convolution(input_spatial_shape, stride_spatial_shape, pad_spatial_shape, kernel_extent)
            ...
            if node.type == 'Deconvolution':
                ...
                if output_padding is not None:
                    if any(output_padding):
                        ...
                        float_spatial = Convolution.calc_deconvolution(node, input_spatial_shape, pad_spatial_shape, kernel_extent)
                        node['output_spatial_shape'] = int64_array(float_spatial)
                    elif node.type == 'DeformableConvolution':
                        node['output_spatial_shape'] = int64_array([node.in_node(1).shape[2:4]])
                elif not 'Unsupported layer type "{}"'.format(node.type):
                ...
                assign_dims_to_weights(node.in_node(weights_index), node.kernel_spatial_idx, node.input_feature_channel, node.output_feature_channel, len(kernel_shape))
                ...
                PermuteAttrs.set_permutation(node.in_node(weights_index), node, node.get_weights_permute if node.has_valid('get_weights_permute') else None)