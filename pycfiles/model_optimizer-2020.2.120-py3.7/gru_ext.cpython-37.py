...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/onnx/gru_ext.py
...
"""
import numpy as np
import extensions.ops.GRU as GRU
...
class GRUFrontExtractor(FrontExtractorOp):
    ...
    def extract(cls, node):
        ...
        activation_beta = onnx_attr(node, 'activation_beta', 'floats', default=None,
          dst_type=(lambda x: np.array(x, dtype=(np.float32))))
        activations = onnx_attr(node, 'activations', 'strings', default=None, dst_type=(lambda x: list(map(lambda s: s.decode(encoding='utf-8').lower(), list(x)))))
