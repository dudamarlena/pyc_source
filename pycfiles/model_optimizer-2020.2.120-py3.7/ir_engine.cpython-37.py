...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/utils/ir_engine/ir_engine.py
# Compiled at: 2020-05-01 08:37:22
# Size of source mod 2**32: 17830 bytes
"""
...
"""
import hashlib, logging as log, os, sys
...
import mo.utils.ir_engine.compare_graphs as compare_graphs
...
class IREngine(object):
    ...
    def __load_xml(self):
        ...
        for child in xml_root:
            if child.tag == 'layers':
                for layer in child:
                    layer_id, layer_attrs = self._IREngine__load_layer(layer)
                    xml_layers.update({layer_id: layer_attrs})

            elif child.tag == 'edges':
                for edge in child:
                    xml_edges.append(Edge(edge.attrib['from-layer'], int(edge.attrib['from-port']), edge.attrib['to-layer'], int(edge.attrib['to-port'])))

            elif child.tag == 'statistics':
                layers = child.findall('layer')
                for layer in layers:
                    statistics[layer.find('name').text] = {'min':layer.find('min').text, 
        ...
        self.graph.graph['cmd_params'] = Namespace(**self.meta_data)
        if len(statistics):
        ...
        for node in nodes:
            ...
            for port in self.graph.node[node]['ports']:
                ...
                (self.graph.add_node)(data, kind='data', shape=self.graph.node[node]['ports'][port], value=None)
    ...
    def __load_bin_hashes(self):
        graph = self.graph
        bin_hash_map = {name:blob_map.item(0) for name, blob_map in dict(np.load((self.path_to_bin), allow_pickle=True)).items()}
        for node in graph.nodes():
            for w in ('weights', 'biases', 'custom'):
                if w in graph.node[node]:
                    assert Node(graph, node).has_valid('name')
                    node_name = Node(graph, node).name
                    if not (node_name in bin_hash_map and w in bin_hash_map[node_name]):
                        raise AssertionError
                    graph.node[node]['hashes'] = bin_hash_map[node_name][w]

    def __load_ir(self):
        self._IREngine__load_xml()
        if not self.path_to_bin:
            return
        elif self.path_to_bin.endswith('.bin.hashes.npz'):
            self._IREngine__load_bin_hashes()
        else:
            self._IREngine__load_bin()
    ...
    def __load_layer(self, layer):
        ...
        for attr in layer:
            if attr.tag == 'data':
                new_attrs = self._IREngine__normalize_attrs(attr.attrib)
                if layer.attrib['type'] == 'Const':
                    if not ('offset' in new_attrs and 'size' in new_attrs):
                        raise AssertionError('Incorrect attributes for Const layer, {} instead of {}!'.format(new_attrs.keys(), ['offset', 'size']))
                    new_attrs.update(self._IREngine__prepare_bin_attrs(layer, 0, 'custom', new_attrs['offset'], new_attrs['size'], layer[1][0].attrib['precision']))
                ...
                if attr.tag == 'blobs':
                    ...
                    for blob_attr in attr:
                        layer_attrs.update(self._IREngine__prepare_bin_attrs(layer, in_port, blob_attr.tag, blob_attr.attrib['offset'], blob_attr.attrib['size'], blob_attr.attrib.get('precision', None)))
                        in_port += 1

        return (
         layer_id, layer_attrs)
    ...
    def __prepare_bin_attrs(xml_layer, in_port, tag, offset, size, precision):
        ...
        precision_map = {'FP32':(4, np.float32),  'FP16':(
          2, np.float16), 
         'I64':(
          8, np.int64), 
         'I32':(
    ...
    def __normalize_attrs(attrs: dict):
        ...
        for attr, value in attrs.items():
            ...
            n_value = []
            for val in value:
                if IREngine._IREngine__isint(val):
                    n_value.append(int(val))
                elif IREngine._IREngine__isfloat(val):
    ...
    def compare(self, ref_net):
        if not isinstance(ref_net, IREngine):
            ir_input = self._IREngine__find_input(self.graph)[0]
            ref_input = self._IREngine__find_input(ref_net)[0]
            ref_graph = ref_net
        else:
            ir_input = self.input_node or self._IREngine__find_input(self.graph)[0]
            ref_input = ref_net.input_node or ref_net._IREngine__find_input(ref_net.graph)[0]
            ref_graph = ref_net.graph
        result, stderr = compare_graphs(graph=(self.graph), graph_ref=ref_graph, last_node=(ir_input.id), last_node_ref=(ref_input.id),
          check_op_attrs=True)
    ...
    def get_inputs(self):
        input_nodes = self._IREngine__find_input(self.graph)
