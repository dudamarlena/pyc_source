...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/back/fuse_sub_div_min.py
...
class Negate(BackReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: [str, Node]):
        ...
        mul_POS_port = None
        if mul.in_port(0).data.get_value() is not None:
            if np.all(mul.in_port(0).data.get_value() == -1):
                mul_POS_port = mul.in_port(1)
        if mul.in_port(1).data.get_value() is not None:
            if np.all(mul.in_port(1).data.get_value() == -1):
...
class EltwisesToSubtract(BackReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: [str, Node]):
        ...
        minuend_port = add.in_port(0).get_source() if add.in_port(1).get_source().node.id == neg.id else add.in_port(1).get_source()
...
class EltwisesToDiv(BackReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: [str, Node]):
        ...
        const = match['const']
        name = mul.soft_get('name', mul.id)
        devidend_port = mul.in_port(0).get_source() if mul.in_port(1).get_source().node.id == pow.id else mul.in_port(1).get_source()
        divider_port = pow.in_port(0).get_source() if pow.in_port(1).get_source().node.id == const.id else pow.in_port(1).get_source()
