...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/ops/fakequantize.py
...
class FakeQuantize(Op):
    ...
    def infer(node: Node):
        if not len(node.in_nodes()) == 5:
            ...
            assert all([broadcastable(inputs[i].shape, inputs[0].shape) for i in range(1, 5)]), 'Not all shapes from FakeQuantize inputs can be broadcasted to input[0] for node {}'.format(node.soft_get('name'))
            node.out_node().shape = x.shape.copy()
            if all([node.in_node(i).has_valid('value') for i in range(5)]):
                x, input_low, input_high, output_low, output_high = [np.array((np.broadcast_to(node.value, x.value.shape)), dtype=(np.float32)) for node in inputs]
                ...
                def middle_part(x, input_low, input_high, output_low, output_high):
                    return round_half_up((x - input_low) / (input_high - input_low) * (node.levels - 1)) / (node.levels - 1) * (output_high - output_low) + output_low

                output = np.zeros_like(x)
                output[middle_mask] = middle_part(x[middle_mask], input_low[middle_mask], input_high[middle_mask], output_low[middle_mask], output_high[middle_mask])
