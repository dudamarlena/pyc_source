...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/tf/InterpolateTransposes.py
...
class InterpolateTranspose(FrontReplacementFromConfigFileGeneral):
    ...
    pattern_nodes = [
     ...
     (
      'transpose_1_order',
      {'kind':'op',  'op':'Const',  'value':lambda x: x is not None and np.array_equal(x, int64_array([0, 2, 3, 1]))}),
     (
      'transpose_2', {'kind':'op',  'op':'Transpose'}),
     (
      'transpose_2_order',
      {'kind':'op',  'op':'Const',  'value':lambda x: x is not None and np.array_equal(x, int64_array([0, 3, 1, 2]))})]
    pattern_edges = [
     (
