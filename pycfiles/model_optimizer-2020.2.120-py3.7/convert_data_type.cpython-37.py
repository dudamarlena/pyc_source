...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/middle/passes/convert_data_type.py
...
from mo.front.extractor import get_new_placeholder_name
...
def data_type_str_to_np(data_type_str: str):
    if data_type_str in SUPPORTED_DATA_TYPES:
        return SUPPORTED_DATA_TYPES[data_type_str][0]


def data_type_str_to_precision(data_type_str: str):
    if data_type_str in SUPPORTED_DATA_TYPES:
        return SUPPORTED_DATA_TYPES[data_type_str][1]


def data_type_str_to_destination_type(data_type_str: str):
    if data_type_str in SUPPORTED_DATA_TYPES:
        return SUPPORTED_DATA_TYPES[data_type_str][2]
...
def convert_blob(blob: np.ndarray, dst_type: type):
    ...
    if dst_type in (np.int32, np.int64, np.uint8, np.int8):
        if not np.array_equal(blob, converted_blob):
            raise Error('The conversion of blob with value "{}" to dst_type "{}" results in rounding'.format(blob, dst_type))
    ...
    return (
     converted_blob, finite_match_count, zero_match_count)
...
def convert_node_blobs(graph: Graph, node: Node, data_type: type):
    ...
    if any(('bin' in d for _, __, d in out_edges)):
        ...
        if blob.dtype != data_type:
            ...
            log.debug('Blob was converted to {} while dumping to the bin file. This blob is an input for {} nodes.'.format(data_type, consumers))
            if finite_match_count:
                log.error(('{} elements of {} were clipped to infinity while converting a blob for node [{}] to {}. ' + refer_to_faq_msg(76)).format(finite_match_count, blob.size, consumers, data_type))
            if zero_match_count:
                log.warning(('{} elements of {} were clipped to zero while converting a blob for node [{}] to {}. ' + refer_to_faq_msg(77)).format(zero_match_count, blob.size, consumers, data_type))
            node.value = new_blob
            if len(node.out_nodes()) == 1:
                if node.out_node(0).op == 'Const':
...
def convert_parameters_data_type(graph: Graph, data_type_str: str):
    ...
    for input in inputs:
        ...
        if user_defined_data_types is not None:
...
def convert_blobs(graph: Graph, data_type_str: str):
    for node in graph.get_data_nodes():
        if node.value is not None:
            try:
                if node.value.dtype in [np.float32, np.float64, np.float16]:
                    if not node.has_and_set('correct_data_type'):
