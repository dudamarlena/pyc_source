...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/ops/transpose.py
    ...
    def infer(node):
        ...
        if node.has_and_set('reverse_order'):
            if len(connected_ports) == 1:
                if not 0 in in_ports:
                    raise AssertionError('Cannot infer `{}` due to both order and reverse_order was set'.format(node.soft_get('name')))
                order = np.arange(len(input_shape))[::-1]
            else:
                pass
        if len(connected_ports) == 2:
            if not (0 in in_ports and 1 in in_ports):
                raise AssertionError('{} node `{}` should have 2 input ports, where 0-input is a data input and 1-input represents Transpose `order`'.format(node.op, node.id))
