...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/tf/elementwise_ext.py
...
"""
from extensions.ops.elementwise import Add, Mul, Sub, Div, Maximum, Minimum, Pow, LogicalAnd, LogicalOr, Equal, GreaterEqual, Greater, Less, LessEqual, NotEqual, FloorMod, BiasAdd, SquaredDifference
...
class BiasAddExtractor(FrontExtractorOp):
    ...
    @classmethod
    def extract(cls, node):
        BiasAdd.update_node_stat(node, {'data_type':tf_dtype_extractor(node.pb.attr['T'].type),  'data_format':node.pb.attr['data_format'].s.decode()})
        return cls.enabled
