...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/back/TopKNormalizer.py
# Compiled at: 2020-05-01 08:37:19
# Size of source mod 2**32: 2766 bytes
"""
...
"""
import extensions.back.Reshape0DToSqueeze as Reshape0DToSqueeze
...
class TopKNormalizer(BackReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        reshape = create_op_node_with_second_input(graph, Reshape, int64_array([]) if is_scalar else int64_array([1]), {'override_output_shape': True})
        node.in_port(1).get_connection().insert_node(reshape)
        if node.out_port(0).disconnected():
            output = Result(graph, {'name':node.name + '/Result_port_0/',  'remove_from_xml':node.has_and_set('remove_values_output')}).create_node()
