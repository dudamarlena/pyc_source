...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/tf/UnpackPackReverseInputChannels.py
...
class UnpackPackReverseInputChannels(FrontReplacementSubgraph):
    ...
    def run_before(self):
        return [
         AttributedSplitToSplit, Pack, SqueezeAxis]
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        reverse_channels = ReverseChannels(graph, {'name':pack.soft_get('name', pack.id) + '/ReverseChannels', 
         'axis':int64_array(axis), 
         'order':int64_array([2, 0, 1])}).create_node()
