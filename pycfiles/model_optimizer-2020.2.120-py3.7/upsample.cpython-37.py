...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/ops/upsample.py
        ...
        if not len(layout) == 4:
            ...
            if input_shape is None:
                ...
                if len(node.in_nodes()) == 1:
                    ...
                    if not (node.has('width_scale') is not None and node.has('height_scale') is not None):
                        raise AssertionError
                    ...
                    node.out_node().shape = shape_for_layout(layout, batch=(input_shape[get_batch_dim(layout, 4)]),
                      ...
                      height=out_height,
                      width=out_width)
            elif not node.in_node(1).value is not None:
                raise AssertionError
