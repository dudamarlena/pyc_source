...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/back/InterpolateToInterpOrResample.py
...
"""
import extensions.back.ShapeOfToShape as ShapeOfToShape
...
from mo.utils.shape import node_to_get_batch_value, node_to_get_features_dimension_value, new_shape_node_from_shape_nodes
...
class InterpolateToInterpOrResample(BackReplacementPattern):
    ...
    force_clean_up = True
    type_map = {'linear':'caffe.ResampleParameter.LINEAR', 
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        if not (1 in node.in_ports() and node.in_port(1).disconnected() or node.in_port(1).data.get_value() is not None):
            raise AssertionError('Interpolate node {} is corrupted: no 1-port input found')
        mode = node.mode
        if not mode in ('linear', 'nearest', 'cubic', 'area'):
            raise AssertionError
        else:
            in_shape = node.in_port(0).data.get_shape()
            if in_shape is not None:
                raise len(in_shape) in (4, 5) or AssertionError
            else:
                out_shape = node.out_port(0).data.get_shape()
                raise out_shape is not None and len(out_shape) in (4, 5) or AssertionError
            in_height, in_width = in_shape[2], in_shape[3]
            out_height, out_width = out_shape[2], out_shape[3]
            factor = factor_update(None if not node.has_valid('factor') else node.factor, [
             float(out_height) / in_height, float(out_width) / in_width], [
             in_height, in_width], [
             out_height, out_width], node.soft_get('name'))
            update_attrs = {'width':out_width, 
             'height':out_height, 
             'factor':factor}
            if not (node.has_valid('shrink_factor') and node.has_valid('zoom_factor')):
                if factor is None:
                    del update_attrs['factor']
                    if node.has('factor'):
                        del node['factor']
                if not node.has_valid('shrink_factor') or node.shrink_factor != 1 or node.has_valid('zoom_factor') and node.zoom_factor != 1 or 'factor' in update_attrs:
                    if not node.has_valid('width') or node.width == 0:
                        if not node.has_valid('height') or node.height == 0:
        ...
        if mode in ('nearest', 'cubic', 'area') or node.has_and_set('convert_to_resample'):
            ...
            if node.pads_begin == 0:
                if not node.pads_end == 0:
                    raise AssertionError
        ...
        if mode == 'linear':
            ...
            update_attrs.update({'pad_beg':node.pads_begin, 
