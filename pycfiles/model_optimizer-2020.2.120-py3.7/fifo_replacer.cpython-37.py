...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/tf/fifo_replacer.py
...
class FIFOQueue(FrontReplacementSubgraph):
    ...
    def replace_sub_graph(graph: Graph, match: dict, **kwargs):
        ...
        if true_placeholder_shape.ndim == 1:
            if len(true_placeholder_shape) > 1:
                log.warning("Placeholder '{}' got non 0-dimensional shape {} in FIFOQueue pattern. Placeholder will have the same shape after folding the pattern instead of {} shape which is original for the network.".format(match['placeholder'].id, true_placeholder_shape, placeholder_shape))
        ...
        placeholder = Parameter(graph, {'name':placeholder_name,  'shape':placeholder_shape,  'data_type':placeholder_data_type}).create_node()
        graph.create_edge(placeholder, match['image_batch'])
        log.info('FIFOQueueV2 pattern was detected. New shape of placeholder {} is {}. Use -b to set batch size if needed'.format(placeholder.id, placeholder['shape']))
