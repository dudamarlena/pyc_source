...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/ReplacePNorm.py
...
class ReplacePNormNodePattern(MiddleReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        reshape_node = create_op_node_with_second_input(graph, Reshape, int64_array([shape[0], shape[1] / node.group, node.group]), {'name': node.id + '_reshape'})
        reshape_node.in_port(0).connect(power_node.out_port(0))
        reducesum_node = create_op_node_with_second_input(graph, ReduceSum, int64_array([2]), {'name':node.id + '_sum', 
         'keep_dims':False})
