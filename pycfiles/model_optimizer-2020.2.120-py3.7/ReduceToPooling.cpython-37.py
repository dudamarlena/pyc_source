...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/back/ReduceToPooling.py
...
import extensions.back.AvgPool as AvgPool
import extensions.back.MaxPool as MaxPool
...
class ReduceMerge(BackReplacementPattern):
    ...
    def fuse_reduces(first_reduce, second_reduce):
        ...
        new_axes = Concat(graph, {'name':second_reduce_name + '/Axes',  'axis':int64_array(0),  'in_ports_count':2,  'override_output_shape':True}).create_node()
        ...
        log.debug('{0} nodes {1} and {2} were fused to a single {2} node with updated axes input'.format(reduce_type, first_reduce_name, second_reduce_name))

    def find_and_replace_pattern(self, graph: Graph):
        rsorted_nodes = graph.pseudo_topological_sort(reverse=True)
        for reduce_type in reduce_map.keys():
            reduces_of_type = [n for n in rsorted_nodes if n.id in graph if n.soft_get('type') == reduce_type]
...
class ReduceReplacer(BackReplacementPattern):
    ...
    def run_before(self):
        ...
        return [
         ReshapeMutation, MaxPool, AvgPool, ScalarNormalize]
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        if reduce_type not in self.pool_method_map:
            log.error('Reduce type {} is not included in pool_method_map. Please update pool_method_map with new key {}'.format(reduce_type, reduce_type))
            return
        ...
        if end_dim == 1:
            ...
            reshape_dim_const_data = Const(graph, {'name':node.id + '/Reshape/Dim',  'value':reshape_shape}).create_node_with_data()
            final_reshape_op = Reshape(graph, {'name': node.id + '/FinalReshape'})
            final_reshape_dim_const_data = Const(graph, {'name':node.id + '/FinalReshape/Dim',  'value':output_shape}).create_node_with_data()
            pooling_op = Pooling(graph, dict(name=(node.id + '/Pool'), window=pool_window,
        ...
        if reduce_type == 'ReduceSum':
            output_data.in_node().insert_node_with_data_after(output_data, AttributedPower, {'name':node.name + '/Mul', 
...
class ReduceLogicalReplacer(BackReplacementPattern):
    ...
    def pattern(self):
        return dict(nodes=[
         (
          'reduce',
          dict(kind='op', type=(lambda node_type: node_type is not None and node_type.startswith('ReduceLogical'))))],
          edges=[])
