...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/ChangePlaceholderTypes.py
...
class ChangePlaceholderTypes(FrontReplacementPattern):
    ...
    def is_node_casts_to_float_or_shapeof(node: Node):
        return node.soft_get('type') == 'Convert' and node.soft_get('dst_type') == np.float32 or node.soft_get('type') == 'ShapeOf'
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for op in graph.get_op_nodes(type='Parameter'):
            ...
            if op.soft_get('data_type') == np.int64:
                ...
                log.error(('Convert data type of Parameter "{}" to int32'.format(op.soft_get('name', op.id))), extra={'is_warning': True})
