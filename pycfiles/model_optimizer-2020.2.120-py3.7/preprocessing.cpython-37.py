...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/preprocessing.py
...
"""
import extensions.middle.MulAddToSS as MulAddToSS
    ...
    def find_and_replace_pattern(self, graph: Graph):
        ...
        try:
            if argv.mean_file and len(original_shapes) == 1:
                mf = loader.parse_mean(argv.mean_file, original_shapes[input_names[0]], argv.mean_file_offsets, caffe_pb2)
            else:
                if argv.mean_file:
                    raise Error('Mean file for topologies with multiple inputs is not supported. ' + refer_to_faq_msg(9))
        except ValueError as e:
            try:
                raise Error('Cannot load or process mean file: value error {}. ' + refer_to_faq_msg(10), str(e)) from e
            finally:
                e = None
                del e
