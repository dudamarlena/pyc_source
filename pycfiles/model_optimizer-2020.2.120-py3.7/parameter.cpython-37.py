...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/ops/parameter.py
...
class Parameter(Op):
    ...
    def supported_attrs(self):
        if self.ir_version == 10:
            return [('shape', lambda node: ','.join([str(i) for i in node.shape])),
             (
    ...
    def infer(node):
        ...
        assert node.has_valid('shape'), 'Parameter node {} should have `shape` attribute. Please use cli options to set model input shape'.format(name)
