...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/back/ShuffleChannelPatternOptimization.py
...
class ShuffleChannelPatternOptimization(BackReplacementPattern):
    ...
    def feature_dim_splitted(short_shape, long_shape):
        return all([short_shape[i] == long_shape[i] for i in range(len(short_shape) - 1)]) and short_shape[(-1)] == long_shape[(-1)] * long_shape[(-2)]

    @staticmethod
    def replace_pattern(graph: Graph, match: dict):
        reshape_5d = match['reshape_start']
        if not ShuffleChannelPatternOptimization.feature_dim_splitted(short_shape=(reshape_5d.in_port(0).data.get_shape()),
          long_shape=(reshape_5d.out_port(0).data.get_shape())):
            return
        else:
            reshape_4d = match['reshape_end']
            return ShuffleChannelPatternOptimization.feature_dim_splitted(short_shape=(reshape_4d.out_port(0).data.get_shape()),
              long_shape=(reshape_4d.in_port(0).data.get_shape())) or None
        start = match['t_start']
        end = match['t_end']
