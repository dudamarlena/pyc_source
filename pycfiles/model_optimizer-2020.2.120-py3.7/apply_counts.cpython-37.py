...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/kaldi/apply_counts.py
...
def apply_biases_to_last_layer(graph, counts):
    ...
    Case 1:
        weights ---        biases  ---    some layer  ---> AffineTransform ---> SoftMax

    Then, counts are applied to biases of Affine Transform:

        weights             ---        (biases - counts)   ---    some layer              ---> AffineTransform ---> SoftMax

    Case 2:
        weights ---        biases  ---    some layer  ---> AffineTransform

    Just takes the last layer and updates biases:

        weights             ---        (biases - counts)   ---    some layer              ---> AffineTransform
...
def read_counts_file(file_path):
    ...
    if len(file_content) > 1:
        raise Error('Expect counts file to be one-line file. ' + refer_to_faq_msg(90))
    counts_line = file_content[0].strip().replace('[', '').replace(']', '')
    try:
        counts = np.fromstring(counts_line, dtype=float, sep=' ')
    except TypeError:
        raise Error('Expect counts file to contain list of floats.' + refer_to_faq_msg(90))
...
class ApplyCountsFilePattern(FrontReplacementSubgraph):
    ...
    def find_and_replace_pattern(self, graph: Graph):
        try:
            ...
            try:
                raise Error('Model Optimizer is not able to read counts file {}'.format(graph.graph['cmd_params'].counts) + refer_to_faq_msg(92)) from e
            finally:
                e = None
                del e
