...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/back/ScalarConstNormalize.py
...
class ScalarNormalize(BackReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        if node.value.ndim == 0:
            reshape = create_op_node_with_second_input(graph, Reshape, int64_array([1]), {'name': node.id + '/Dims'})
            ...
            if node.has_and_set('type') and node.type in rules:
                for port in rules[node.type]:
                    if port in node.in_ports():
                        src_node = node.in_port(port).disconnected() or node.in_port(port).get_connection().get_source().node
                        if src_node is not None and src_node.has_and_set('type') and src_node.type == 'Const' and src_node.value.ndim == 0:
                            log.info('Converting constant node "{}" from 0D to 1D'.format(src_node.soft_get('name')))
                            reshape = create_op_node_with_second_input(graph, Reshape, int64_array([1]), {'name': src_node.id + '/Dims'})
        ...
        for node in graph.get_op_nodes():
            if node.soft_get('type') in rules:
                for port in rules[node.type]:
                    if port in node.in_ports():
                        src_node = node.in_port(port).disconnected() or node.in_port(port).get_connection().get_source().node
                        reshape = create_op_node_with_second_input(graph, Reshape, int64_array([1]), {'name':src_node.id + '/1D', 
