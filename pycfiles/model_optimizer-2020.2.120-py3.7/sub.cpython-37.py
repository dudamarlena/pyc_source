...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/sub.py
...
class Sub(FrontReplacementPattern):
    ...
    def sub_to_add_replacement(sub: Node):
        if sub.in_port(0).data.get_value() is not None:
            if sub.in_port(1).data.get_value() is not None:
                return
        graph = sub.graph
