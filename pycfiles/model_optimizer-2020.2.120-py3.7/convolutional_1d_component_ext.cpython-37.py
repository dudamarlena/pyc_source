...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/front/kaldi/extractors/convolutional_1d_component_ext.py
...
class ConvolutionalComponentFrontExtractor(FrontExtractorOp):
    ...
    def extract(cls, node: Node) -> bool:
        ...
        if token != '<FilterParams>':
            raise Error('Can not load token {} from Kaldi model'.format(token) + refer_to_faq_msg(94))
        collect_until_whitespace(pb)
        weights, weights_shape = read_binary_matrix(pb)
        collect_until_whitespace(pb)
        biases = read_binary_vector(pb)
        if (patch_stride - kernel) % stride != 0:
            raise Error('Kernel size and stride does not correspond to `patch_stride` attribute of Convolution layer. ' + refer_to_faq_msg(93))
        output = biases.shape[0]
        if weights_shape[0] != output:
            raise Error('Weights shape does not correspond to the `output` attribute of Convolution layer. ' + refer_to_faq_msg(93))
        mapping_rule = {'output':output, 
