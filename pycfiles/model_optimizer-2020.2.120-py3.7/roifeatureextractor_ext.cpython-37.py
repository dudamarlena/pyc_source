...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/onnx/roifeatureextractor_ext.py
...
class ExperimentalDetectronROIFeatureExtractorFrontExtractor(FrontExtractorOp):
    ...
    def extract(cls, node):
        attrs = dict(output_size=(onnx_attr(node, 'output_size', 'i', 7)), sampling_ratio=(onnx_attr(node, 'sampling_ratio', 'i', 2)),
          ...
          pyramid_scales=np.array((onnx_attr(node, 'pyramid_scales', 'ints', [4, 8, 16, 32, 64])), dtype=(np.int64)))
