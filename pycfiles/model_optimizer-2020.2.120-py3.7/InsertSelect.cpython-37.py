...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/InsertSelect.py
# Compiled at: 2020-05-01 08:37:21
# Size of source mod 2**32: 8112 bytes
"""
...
class AddSelectBeforeMemoryNodePattern(MiddleReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        try:
            ...
            existing_counters = find_pattern_matches(graph, nodes=[
             ('mem_in',
              dict(op='Memory', index=1, shape=(int64_array([context_len])))),
             (
              'mem_in_data', dict()),
             (
              'crop_mem_in',
              dict(op='Crop', axis=(int64_array([1])), offset=(int64_array([1])),
             ...
             (
              'mem_out',
              dict(op='Memory', index=0, shape=(int64_array([context_len])))),
             (
              'crop_out',
              dict(op='Crop', axis=(int64_array([1])), offset=(int64_array([0])),
                dim=(int64_array([1])))),
             (
              'crop_out_data', dict()),
             (
              ...
              edges=[
             ('mem_in', 'mem_in_data'), ('mem_in_data', 'crop_mem_in'),
            ...
            if counter_match is not None:
                ...
                mem_out = Memory(graph, {'name':'iteration_number',  'size':2,  'index':1, 
                 'id':'iteration_' + node.name,  'shape':int64_array([context_len]), 
                 'dst_type':np.int32}).create_node()
                cut_first = Crop(graph, {'name':'cut_first',  'axis':int64_array([1]),  'offset':int64_array([1]), 
                 'dim':int64_array([context_len - 1])}).create_node()
                ...
                mem_in = Memory(graph, {'name':'iteration_number_out',  'size':2,  'index':0, 
                 'id':'iteration_' + node.name,  'shape':int64_array([context_len])}).create_node()
                mem_in.in_port(0).connect(concat.out_port(0))
                res = Result(graph, {}).create_node()
                mem_in.out_port(0).connect(res.in_port(0))
                cut_last = Crop(graph, {'name':'cut_last',  'axis':int64_array([1]),  'offset':int64_array([0]), 
