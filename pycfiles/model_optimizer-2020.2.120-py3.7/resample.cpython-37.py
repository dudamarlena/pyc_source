...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/ops/resample.py
# Compiled at: 2020-05-01 08:37:21
# Size of source mod 2**32: 3711 bytes
"""
...
class ResampleOp(Op):
    ...
    def resample_infer(node: Node):
        ...
        if not len(layout) == 4:
            ...
            if node.has('fw'):
                if node.fw == 'tf':
                    ...
                    if not dst_shape is None:
                        if len(input_shape) != 4 or len(dst_shape) != 2:
                            log.error('Node {} with op {} cannot be converted to Resample layer because there is no enough info about src/dst shapes: src_shape = {}, dst_shape = {}'.format(node.name, node.op, input_shape, dst_shape))
                            node.type = None
                            return
        ...
        node.factor = factor_update(node.factor, [
         float(out_height) / in_height, float(out_width) / in_width], [
         in_height, in_width], [
         out_height, out_width], node.soft_get('name'))
        node.out_node().shape = shape_for_layout(layout, batch=(input_shape[get_batch_dim(layout, 4)]),
