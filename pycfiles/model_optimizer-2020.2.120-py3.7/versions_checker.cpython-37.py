...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/utils/versions_checker.py
# Compiled at: 2020-05-05 09:24:36
...
"""
import logging as log, os, re, sys
...
def parse_versions_list(required_fw_versions, version_list):
    ...
    if line == '':
        ...
        splited_versions_by_conditions = re.split('==|>=|<=|>|<', line)
        ...
        if len(splited_versions_by_conditions) == 0:
            ...
            splited_required_versions = re.split(',', line)
...
def check_requirements(framework=None):
    ...
    for name, key, required_version in requirements_list:
        try:
            ...
            try:
                log.error('Error happened while importing {} module. It may happen due to unsatisfied requirements of that module. Please run requirements installation script once more.\nDetails on module importing failure: {}'.format(name, e))
