...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/ConvToBinaryConv.py
...
class ConvToBinaryConv(MiddleReplacementPattern):
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        if not match['operator'].has('multiplication_transparent_ports'):
            ...
            assert len(port) >= 1
            if len(port) > 1:
                log.debug('BinarizeWeightsM1P1 cannot apply transformation for data {} because it consumed more than once'.format(match['quantized'].name))
                return
            assert len(port) == 1
            port = port[0]
            ...
            if not output_low.has_valid('value'):
                if not output_high.has_valid('value'):
                    return
        ...
        weights_consistent = np.all(np.isclose(weights, weights_rounded)) and set(np.unique(weights_rounded)).issubset({-1, 1})
        if weights_consistent and np.all(np.isclose(output_low, 0)) and np.all(np.isclose(output_high, 1)):
            ...
            if weights_consistent and np.all(np.isclose(output_low, -1)) and np.all(np.isclose(output_high, 1)):
                pass
            else:
                log.debug('ConvToBinaryConv: cannot apply transformation because input range is neither in [0, +1] nor in [-1, +1].')
                return
