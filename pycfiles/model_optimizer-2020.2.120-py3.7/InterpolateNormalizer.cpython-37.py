...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/InterpolateNormalizer.py
...
"""
import inspect, logging as log, numpy as np
...
class InterpolateNormalizer(FrontReplacementOp):
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        if 1 not in node.in_ports() or node.in_port(1).disconnected():
            if node.has_valid('factor'):
                factor = node.has_valid('width') or node.has_valid('height') or Const(graph, {'value': np.array(node.factor)}).create_node()
                ...
                ss = StridedSlice(graph, {'name':node.name + '/ss_0_port',  'begin_mask':np.array([1]),  'end_mask':np.array([0]), 
                ...
                if node.soft_get('shrink_factor') != 1 and node.soft_get('zoom_factor') == 1:
                    ...
                    if shrink_factor < 1:
                        ...
                        return
                    const = Const(graph, {'name':node.name + '/pre_shrink_sub_const',  'value':np.array(-1)}).create_node()
                    ...
                    const = Const(graph, {'value':np.array(1 / shrink_factor),  'name':node.name + 'shrink_factor_div_const'}).create_node()
                    ...
                    const = Const(graph, {'name':node.name + '/shrink_factor_add_one_const',  'value':np.array(1)}).create_node()
                    ...
                    if zoom_factor < 1:
                        ...
                        return
                    node['debug_message'] = 'Interpolate layer replacer may be wrong, please, try to update it in the file (extensions/front/InterpolateNormalizer.py at the line {}).'.format(inspect.currentframe().f_lineno) + refer_to_faq_msg(100)
                    const = Const(graph, {'value':np.array(zoom_factor),  'name':node.name + '/zoom_factor_mul_const'}).create_node()
                    ...
                    const = Const(graph, {'value':np.array(-1),  'name':node.name + 'shrink_zoom_factor_sum_const'}).create_node()
