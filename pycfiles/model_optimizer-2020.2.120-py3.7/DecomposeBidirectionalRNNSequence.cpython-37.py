...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/DecomposeBidirectionalRNNSequence.py
...
class DecomposeBidirectionalRNNSequence(MiddleReplacementPattern):
    ...
    def run_after(self):
        import extensions.middle.MXNetRNNSequenceNormalize as MXNetRNNSequenceNormalize
        import extensions.middle.ONNXRNNSequenceNormalize as ONNXRNNSequenceNormalize
        return [
         ONNXRNNSequenceNormalize, MXNetRNNSequenceNormalize]
    ...
    def split_data(self, data: Node):
        ...
        output_data = [Op._create_data_node((data.graph), name=(data.name + '/SplittedBiLSTM/{}'.format(['forward', 'reverse'][i]))) for i in (0,
                                                                                                                                               1)]
        split_op = Split(data.graph, dict(name=(data.name + '/DecomposedBiLSTM_0'), num_splits=2))
        axis_const = Const(data.graph, {'name':data.name + '/DecomposedBiLSTM_0' + '/Split_axis',  'value':np.int64(0)}).create_node_with_data()
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        new_init_cells = self.split_data(bidirectional_cell.in_node(6)) if 6 in bidirectional_cell.in_nodes() else (None,
                                                                                                                    None)
        ...
        outputs = self.split_bidirectional(bidirectional_cell, new_init_hiddens, new_init_cells, splitted_W, splitted_R, splitted_B)
    ...
    def split_bidirectional(self, bidirectional_cell: Node, new_init_hiddens: list, new_init_cells: list, splitted_W: tuple, splitted_R: tuple, splitted_B: tuple):
        """
        ...
        for i in (0, 1):
            ...
            output_data = Op._create_data_node((bidirectional_cell.graph),
            ...
            output_hidden = Op._create_data_node((bidirectional_cell.graph),
            ...
            if bidirectional_cell.op == 'LSTM':
                output_cell = Op._create_data_node((bidirectional_cell.graph),
            ...
            all_outputs.append(op.create_node_with_data(inputs=[
    ...
    def concat_outputs(bi_rnn, forward_outputs, reverse_outputs, final_outputs):
        ...
        concat_ops = [
         Concat(bi_rnn.graph, {'name':bi_rnn.name + '/FinalConcat/Data', 
          'axis':1, 
          'in_ports_count':2}),
         Concat(bi_rnn.graph, {'name':bi_rnn.name + '/FinalConcat/HiddenState', 
          'axis':0, 
          'in_ports_count':2}),
         Concat(bi_rnn.graph, {'name':bi_rnn.name + '/FinalConcat/CellState', 
          'axis':0, 
          'in_ports_count':2})]
