...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/mxnet/slice_replacers.py
...
class SliceFrontReplacer(FrontReplacementOp):
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        node = match['op']
        strided_slice_node = StridedSlice(graph, dict(name=(node.id + '/strided_slice_'), shrink_axis_mask=(np.array(np.zeros((len(node.crop_begin)), dtype=(np.int64)))),
        ...
        crop_begin_node = Const(graph, dict(value=(node.crop_begin), symbol_dict={'name': node.id + '/crop_begin_const'})).create_node()
        crop_end_node = Const(graph, dict(value=(node.crop_end), symbol_dict={'name': node.id + '/crop_end_const'})).create_node()
        ...
        if len(node.step) > 0:
            stride_node = Const(graph, dict(value=(node.step), symbol_dict={'name': node.id + '/steps_const'})).create_node()
