...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/FakeSplitOutputs.py
...
class AddFakeOutputsToSplit(MiddleReplacementPattern):
    ...
    def pattern():
        return dict(nodes=[
         (
          'op', dict(kind='op', op='Split'))],
          edges=[])

    @staticmethod
    def replace_pattern(graph: Graph, match: dict):
        node = match['op']
        if node.has_valid('out_ports_count'):
            if len(node.out_edges()) < node.out_ports_count:
...
class AddFakeOutputsToVariadicSplit(MiddleReplacementPattern):
    ...
    def pattern():
        return dict(nodes=[
         (
          'op', dict(kind='op', op='VariadicSplit'))],
          edges=[])
