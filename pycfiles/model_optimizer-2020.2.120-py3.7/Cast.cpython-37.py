...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/ops/Cast.py
...
class Cast(Op):
    ...
    def backend_attrs(self):
        if self.ir_version == 10:
            return [
             (
              'destination_type', lambda node: np_data_type_to_destination_type(node.dst_type))]
    ...
    @staticmethod
    def infer(node: Node):
        if not node.has_valid('dst_type'):
            raise AssertionError('Destination type of "Cast" operation should be extracted earlier')
        else:
            ...
            if node.in_node(0).has_valid('value'):
                ...
                if finite_match_count:
                    log.error(("{} elements of {} were clipped to infinity while converting an input blob for node '{}' to {}. " + refer_to_faq_msg(76)).format(finite_match_count, new_blob.size, node.name, dst_type))
                if zero_match_count:
                    log.warning(("{} elements of {} were clipped to zero while converting an input blob for node '{}' to {}. " + refer_to_faq_msg(77)).format(zero_match_count, new_blob.size, node.name, dst_type))