...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/utils/graph.py
...
def backward_bfs_for_operation(start_node: Node, op_names: list):
    ...
    while len(q) != 0:
        ...
        for id in range(in_nodes_size):
            ...
            if pnode.kind == 'op':
                if pnode.has_valid('op'):
                    if pnode.op in op_names:
...
def is_connected_component(graph: Graph, node_names: list):
    ...
    while len(d) != 0:
        ...
        adj_nodes = [src_node for src_node, _ in graph.in_edges(cur_node_name) if src_node in node_names] + [dst_node for _, dst_node in graph.out_edges(cur_node_name) if dst_node in node_names]
...
def sub_graph_between_nodes(graph: Graph, start_nodes: list, end_nodes: list, detect_extra_start_node: callable=None):
    ...
    while len(d) != 0:
        ...
        if cur_node_name not in end_nodes:
        ...
        for src_node_name, _ in graph.in_edges(cur_node_name):
            if cur_node_name not in start_nodes and src_node_name not in visited:
                if detect_extra_start_node is not None:
                    if detect_extra_start_node(Node(graph, cur_node_name)):
    ...
    for end_node in end_nodes:
        if end_node not in forward_visited:
            raise Error('End node "{}" is not reachable from start nodes: {}. '.format(end_node, start_nodes) + refer_to_faq_msg(74))
    ...
    for node_name in sub_graph_nodes:
        if graph.node[node_name].get('op', '') == 'Parameter':
            ...
            raise Error('The matched sub-graph contains network input node "{}". '.format(node_name) + refer_to_faq_msg(75))
...
def invert_sub_graph_between_nodes(graph: Graph, start_nodes: list, end_nodes: list, detect_extra_start_node: callable=None):
    ...
    while len(d) != 0:
        cur_node_name = d.popleft()
        sub_graph_nodes.append(cur_node_name)
        if cur_node_name not in start_nodes and detect_extra_start_node is not None and detect_extra_start_node(Node(graph, cur_node_name)):
            extra_start_nodes.append(cur_node_name)
        elif cur_node_name not in end_nodes:
            for src_node_name, _ in graph.in_edges(cur_node_name):
