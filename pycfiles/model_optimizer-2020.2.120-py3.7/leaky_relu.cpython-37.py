...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/middle/passes/leaky_relu.py
...
def _convert_to_leaky_relu_action(graph: Graph, matches: dict):
    ...
    if len(max_op.in_nodes()) > 2:
        log.debug('Maximum layer ({}) can not participate in conversion to leaky ReLU due to it has more than two inputs ({})'.format(max_op.id, len(max_op.in_nodes())))
        return
    if mul_value_data.has_valid('value'):
        if mul_value_data.value.size != 1:
            log.debug('Mul layer ({}) can not participate in conversion to leaky ReLU due to value {}'.format(mul_op.id, mul_value_data.soft_get('value')))
            return
    value = mul_value_data.value.item(0)
    if len(mul_data.out_nodes()) > 1:
        log.debug('Mul layer({}) can not participate in conversion to leaky ReLU due to it has more than one consumer'.format(mul_op.id))
        return
    ...
    log.debug('Successful conversion from {} {} to ReLU with negative slope (leaky ReLU)'.format(max_op.id, mul_op.id))
