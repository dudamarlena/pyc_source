...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/DilatedConvolution.py
...
class DilatedConvolution1DConverter(MiddleReplacementPattern):
    ...
    def swap_pad_and_unsqueeze(self, pad: Node, unsqueeze: Node):
        ...
        for port_id in (1, 2):
            ...
            new_value_node = Const(pad.graph, {'value':np.insert(current_value, unsqueeze_axis.item(), 0),  'override_output_shape':True}).create_node()
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        conv.dilation[conv.spatial_dims] = [
         1, block_size.item()]
