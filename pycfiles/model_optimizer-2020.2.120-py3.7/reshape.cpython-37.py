...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/ops/reshape.py
...
class Reshape(Op):
    ...
    def infer(node: Node):
        ...
        if not (len(connected_inputs) == 2 and all([i in connected_inputs for i in range(2)])):
            raise AssertionError("Reshape should have 2 connected input ports, but it doesn't for node: `{}`. Ports: {}".format(name, connected_inputs))
        input_shape = node.in_port(0).data.get_shape()
        if not input_shape is not None:
            ...
            assert np.argwhere(new_shape == -1).size <= 1, 'Reshape second input should not have several `-1` values set. Node: {}, reshape second input value {}'.format(name, new_shape)
            num_of_input_elements = np.prod(input_shape)
            num_of_output_elements = 1
            for index, x in enumerate(new_shape):
                if x == 0:
                    if node.has_and_set('special_zero'):
            ...
            assert np.prod(input_shape) == np.prod(output_shape), 'Number of elements in input {} and output {} of reshape node {} mismatch'.format(input_shape, output_shape, name)
