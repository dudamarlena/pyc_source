...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/EmbeddingBagResolver.py
    ...
    def find_and_replace_pattern(self, graph: Graph):
        ...
        for node in graph.get_op_nodes(op='EmbeddingBag'):
            ...
            if indices_shape[0] == offsets_shape[0]:
                gather = create_op_with_const_inputs(graph, Gather, {2: int64_array(0)}, {'name': node.name + '/Emb_Bag/Gather_'})
                ...
                if not node.mode == 0:
                    ...
                    sweightedsum = create_op_with_const_inputs(graph, ExperimentalSparseWeightedSum, {2:dense_shape, 
                     4:default_index}, {'name': node.name + '/WeightedSum'})
                ...
                weights_concat = create_op_with_const_inputs(graph, Concat, {1: default_embeddings}, {'axis':0, 
                 'in_ports_count':2})
        ...
        for i, (node, ind_shape) in enumerate(weighted_sum_nodes):
            if merge_offsets and len(weighted_sum_nodes) > 1:
                if new_offsets is None:
                    ...
                    new_offsets = Parameter(graph, {'name':'Emb_Bag/offsets',  'shape':shape, 
                     'data_type':np.int32}).create_node()
                    log.error(('Pre-process of offsets is needed for generated input "Emb_Bag/offsets" of shape: {}. Refer to the documentation on how to convert the ONNX* DLRM model'.format(shape)),
                      extra={'is_warning': True})
                gather = create_op_with_const_inputs(graph, Gather, {1:int64_array(i),  2:int64_array(0)}, {'name': node.name + '/Gather_'})
                new_offsets.out_port(0).connect(gather.in_port(0))
                gather.out_port(0).connect(node.in_port(0))
            else:
                shape = int64_array([ind_shape, 2])
                new_offsets = Parameter(graph, {'name':'Emb_Bag/offsets{}'.format(i),  'shape':shape, 
                 'data_type':np.int32}).create_node()
                new_offsets.out_port(0).connect(node.in_port(0))
                log.error(('Pre-process of offsets is needed for generated input "Emb_Bag/offsets{}" of shape: {}. Refer to the documentation on how to convert the ONNX* DLRM model'.format(i, shape)),
                  extra={'is_warning': True})