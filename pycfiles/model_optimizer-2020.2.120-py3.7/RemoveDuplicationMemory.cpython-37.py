...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/RemoveDuplicationMemory.py
            ...
            if child['op'] == 'Splice' and child.id != mem.id and set(child['context']).issubset(set(context)):
                ...
                for child_of_child in child.out_port(0).get_destinations():
                    ...
                    if out_transfer['op'] == 'Crop':
                        ...
                        crop_node = Crop(graph, {'name':graph.unique_id(prefix='Splice_crop_'),  'offset':left_cont_out - left_cont * mem_shape[(-1)], 
...
class MergeNeighborSplicePattern(MiddleReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        for child_port in mem_parent.get_destinations():
            child = child_port.node
            if child['op'] == 'Splice' and child.id != mem.id and not child['context'][0] == context[(-1)]:
                ...
                for out_port_rem in rem_node.out_port(0).get_destinations():
                    ...
                    if out_transfer['op'] == 'Crop':
                        ...
                        crop_node = Crop(graph, {'name':graph.unique_id(prefix='Splice_crop_'),  'offset':len(new_context) - len(rem_node.context) * mem_shape[(-1)], 
                ...
                for out_port_rem in new_node.out_port(0).get_destinations():
                    ...
                    out_transfer_shape = out_port_rem.data.get_shape().copy()
                    if out_transfer['op'] != 'Crop':
                        crop_node = Crop(graph, {'name':graph.unique_id(prefix='Splice_crop_'),  'offset':np.array([0]), 
