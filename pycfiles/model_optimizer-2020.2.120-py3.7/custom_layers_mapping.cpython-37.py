...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/front/caffe/custom_layers_mapping.py
            ...
            if child.tag == 'CustomLayer':
                if expected_attribs(child.attrib, ['NativeType', 'hasParam'], fileName):
                    ...
                    if layer in layers_map:
                        ...
                        if has_param == 'true':
                            if expected_attribs(child.attrib, ['protoParamName'], fileName) or has_param == 'false':
                                layers_map[layer] = child.attrib
                        else:
                            log.error('Cannot recognize {} value for hasParam for layer {}. Should be true or false. Skipped.'.format(child.attrib['hasParam'], layer))
            else:
...
special_keys = [
 'id', 'name', 'precision', 'type', 'layer', 'value', 'shape', 'op', 'kind', 'infer']
...
def proto_extractor(pb, model_pb, mapping, disable_omitting_optional, enable_flattening_nested_params):
    ...
    if mapping['hasParam'].lower() != 'true':
        return {}
    try:
        native_attr = collect_attributes((getattr(pb, mapping['protoParamName'])), disable_omitting_optional=disable_omitting_optional,
...
def update_extractors(extractors, layers_map, disable_omitting_optional, enable_flattening_nested_params):
    ...
    for layer, attrs in layers_map.items():
        add_or_override_extractor(extractors, keys, layer, lambda l: node_pb_arg(lambda pb, model_pb: proto_extractor(pb, model_pb, l, disable_omitting_optional, enable_flattening_nested_params))(layers_map[layer]), 'custom layer {} from custom layers mapping xml file'.format(layer))
