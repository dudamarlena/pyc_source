...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/tf/ObjectDetectionAPI.py
# Compiled at: 2020-05-01 08:37:20
# Size of source mod 2**32: 74129 bytes
"""
...
import extensions.front.TransposeOrderNormalizer as TransposeOrderNormalizer
...
import extensions.front.tf.CropAndResizeReplacement as CropAndResizeReplacement
...
from extensions.middle.InsertLayoutPropagationTransposes import mark_as_correct_data_layout, mark_input_as_in_correct_layout, mark_output_as_in_correct_layout
...
from mo.front.tf.graph_utils import add_activation_function_after_node, add_convolution_to_swap_xy_coordinates, squeeze_reshape_and_concat, add_fake_background_loc, create_op_node_with_second_input
...
missing_param_error = 'To convert the model specify path to the pipeline configuration file which was used to generate the model. Please use "--tensorflow_object_detection_api_pipeline_config" option:\n--tensorflow_object_detection_api_pipeline_config "<path_to_pipeline.config>"\nIf you have downloaded the model file from the Object Detection Model zoo repository then this file is located in the archive with frozen model and called "pipeline.config".\nIf you did not use this command line parameter before that means that you are using currently deprecated TensorFlow* Object Detection API models conversion mechanism.'
...
def _value_or_raise(match: SubgraphMatch, pipeline_config: PipelineConfig, key: str):
    ...
    if value is None:
        raise Error('The sub-graph replacer "[REPLACEMENT_ID]" was not able to find the value for key "{}" in the pipeline configuration file specified with the --tensorflow_object_detection_api_pipeline_config command line parameter. Update the sub-graph replacement configuration file specified with the --tensorflow_use_custom_operations_config command line parameter by adding key "{}" with required value to the "custom_attributes" dictionary of the "[REPLACEMENT_ID]" replacer.'.format(key, key))
    return value
...
def _find_ssd_head_node(graph: Graph, ssd_head_index: int, head_type: str):
    ...
    if head_type == 'box':
        possible_node_names = [
         'BoxPredictor_%d/BoxEncodingPredictor/BiasAdd' % ssd_head_index,
         'WeightSharedConvolutionalBoxPredictor/BoxPredictor/BiasAdd' if ssd_head_index == 0 else 'WeightSharedConvolutionalBoxPredictor_%d/BoxPredictor/BiasAdd' % ssd_head_index]
    else:
        if head_type == 'class':
            possible_node_names = [
             'BoxPredictor_%d/ClassPredictor/BiasAdd' % ssd_head_index,
             'WeightSharedConvolutionalBoxPredictor/ClassPredictor/BiasAdd' if ssd_head_index == 0 else 'WeightSharedConvolutionalBoxPredictor_%d/ClassPredictor/BiasAdd' % ssd_head_index]
        else:
...
def _relax_reshape_nodes(graph: Graph, pipeline_config: PipelineConfig):
    ...
    if num_layers is None:
        num_layers = pipeline_config.get_param('multiscale_anchor_generator_max_level') - pipeline_config.get_param('multiscale_anchor_generator_min_level') + 1
...
def _create_prior_boxes_node(graph: Graph, pipeline_config: PipelineConfig):
    ...
    for ssd_head_ind in range(num_layers):
        ...
        if ssd_head_ind == 0 and reduce_boxes_in_lowest_layer:
            widths = [
             0.1, min_scale * sqrt(2.0), min_scale * sqrt(0.5)]
            ...
            prior_box_op = PriorBoxClusteredOp(graph, {'width':np.array(widths),  'height':np.array(heights),  'clip':0, 
             'flip':0,  'variance':variance,  'offset':0.5})
            prior_box_node = prior_box_op.create_node([ssd_head_node, Node(graph, 'image_tensor').out_node(0)], {'name': 'PriorBoxClustered_{}'.format(ssd_head_ind)})
...
def insert_weights_swap_xy_sub_graph(graph: Graph, connection):
    ...
    gather_indices = Const(graph, {'name':swapped_weight.name + '/Indices', 
...
def calculate_placeholder_spatial_shape(graph: Graph, match: SubgraphMatch, pipeline_config: PipelineConfig):
    ...
    if 'preprocessed_image_height' in match.custom_replacement_desc.custom_attributes or 'preprocessed_image_width' in match.custom_replacement_desc.custom_attributes:
        log.error('The "preprocessed_image_height" or "preprocessed_image_width" is specified in the sub-graph replacement configuration file but they are ignored. Please, specify desired input shape using the "--input_shape" command line parameter.', extra={'is_warning': True})
    user_defined_height = None
    user_defined_width = None
    if user_shapes:
        if 'image_tensor' in user_shapes:
            if user_shapes['image_tensor']:
    ...
    if resizer_min_dimension:
        if resizer_max_dimension:
            log.debug('The model resizes image using keep aspect ratio with minimum size {}, maximum size {}.'.format(resizer_min_dimension, resizer_max_dimension))
    if resizer_width:
        if resizer_height:
            if user_defined_height and user_defined_width and not user_defined_width != resizer_width:
                if user_defined_width != resizer_width:
                    log.error(('The model expects that the input image is resized to a fixed shape ({}, {}), but the shape provided with the "--input_shape" command line parameter is different ({}, {}).'.format(resizer_height, resizer_width, user_defined_height, user_defined_width)),
        ...
        if resizer_min_dimension and resizer_max_dimension:
            print('[ WARNING ] Model Optimizer removes pre-processing block of the model which resizes image keeping aspect ratio. The Inference Engine does not support dynamic image size so the Intermediate Representation file is generated with the input image size of a fixed size.')
            if user_defined_height and user_defined_width:
                scaled_height, scaled_width = calculate_shape_keeping_aspect_ratio(user_defined_height, user_defined_width, resizer_min_dimension, resizer_max_dimension)
                if scaled_height != user_defined_height or scaled_width != user_defined_width:
                    log.error(('The model resizes the input image keeping aspect ratio with min dimension {}, max dimension {}. The provided input height {}, width {} is transformed to height {}, width {}.'.format(resizer_min_dimension, resizer_max_dimension, user_defined_height, user_defined_width, scaled_height, scaled_width)),
                      extra={'is_warning': True})
                height = scaled_height
                width = scaled_width
    else:
        height = width = resizer_min_dimension
        print('Specify the "--input_shape" command line parameter to override the default shape which is equal to ({}, {}).'.format(height, width))
...
class ObjectDetectionAPIPreprocessorReplacement(FrontReplacementFromConfigFileSubGraph):
    ...
    def run_before(self):
        return [
         Pack, Sub, TransposeOrderNormalizer, PadTFToPad]

    def nodes_to_remove(self, graph: Graph, match: SubgraphMatch):
        new_nodes_to_remove = match.matched_nodes_names()
        for node_to_keep in ('Preprocessor/sub', 'Preprocessor/sub/y', 'Preprocessor/mul',
                             'Preprocessor/mul/x'):
    ...
    def generate_sub_graph(self, graph: Graph, match: SubgraphMatch):
        ...
        if not sub_node.has('op') or sub_node.op != 'Sub':
            raise Error('The output op of the Preprocessor sub-graph is not of type "Sub". Looks like the topology is not created with TensorFlow Object Detection API.')
        mul_node = None
        if sub_node.in_node(0).has('op'):
            if sub_node.in_node(0).op == 'Mul':
        ...
        if initial_input_node_name not in graph.nodes():
            raise Error('Input node "{}" of the graph is not found. Do not run the Model Optimizer with "--input" command line parameter.'.format(initial_input_node_name))
        else:
            placeholder_node = Node(graph, initial_input_node_name)
            batch_dim = get_batch_dim(layout, 4)
            if argv.batch is None:
            ...
            if to_float_node.has('op'):
                if to_float_node.op != 'Cast':
                    raise Error('The output of the node "{}" is not Cast operation. Cannot apply replacer.'.format(initial_input_node_name))
                if mul_node is None:
                    graph.create_edge(to_float_node, sub_node, 0, 0)
            else:
                graph.create_edge(to_float_node, mul_node, 0, 1)
        print('The Preprocessor block has been removed. Only nodes performing mean value subtraction and scaling (if applicable) are kept.')
        return {}
...
class ObjectDetectionAPIDetectionOutputReplacement(FrontReplacementFromConfigFileSubGraph):
    ...
    def run_before(self):
        return [
         ObjectDetectionAPIMaskRCNNROIPoolingSecondReplacement, SqueezeAxis, Sub, TransposeOrderNormalizer]

    def run_after(self):
        return [
         ObjectDetectionAPIProposalReplacement, CropAndResizeReplacement, FakeQuantWithMinMaxVarsToQuantize]
    ...
    def nodes_to_remove(self, graph: Graph, match: SubgraphMatch):
        ...
        for output in outputs:
            if output in graph.nodes:
                ...
                if len(children) != 1:
                    log.warning('Output {} has {} children. It should have only one output: with op==`Result`'.format(output, len(children)))
    ...
    def generate_sub_graph(self, graph: Graph, match: SubgraphMatch):
        ...
        if argv.tensorflow_object_detection_api_pipeline_config is None:
            ...
            activation_conf_node = add_activation_function_after_node(graph, match.single_input_node(1)[0].in_node(0), activation_function)
            reshape_conf_node = create_op_node_with_second_input(graph, Reshape, int64_array([-1, (num_classes + 1) * max_proposals]), dict(name='do_reshape_conf'), activation_conf_node)
            mark_as_correct_data_layout(reshape_conf_node)
            current_node = skip_nodes_by_condition(match.single_input_node(0)[0].in_node(0), lambda x: x['kind'] == 'op' and x.has_and_set('reinterp_shape'))
            reshape_loc_node = create_op_node_with_second_input(graph, Reshape, int64_array([-1, num_classes, 1, 4]), dict(name='reshape_loc'), current_node)
            ...
            reshape_loc_2d_node = create_op_node_with_second_input(graph, Reshape, int64_array([-1, 4]), dict(name='reshape_locs_2d'), loc_node)
            mark_as_correct_data_layout(reshape_loc_2d_node)
            eltwise_locs_op = Mul(graph, dict())
            eltwise_locs_node = eltwise_locs_op.create_node([reshape_loc_2d_node, variances_const_node], dict(name='scale_locs'))
            ...
            if 'coordinates_swap_method' not in custom_attributes:
                log.error('The ObjectDetectionAPIDetectionOutputReplacement sub-graph replacement configuration file must contain "coordinates_swap_method" in the "custom_attributes" dictionary. Two values are supported: "swap_weights" and "add_convolution". The first one should be used when there is a MatMul or Conv2D node before the "SecondStagePostprocessor" block in the topology. With this solution the weights of the MatMul or Conv2D nodes are permutted, simulating the swap of XY coordinates in the tensor. The second could be used in any other cases but it is worse in terms of performance because it adds the Conv2D node which performs permutting of data. Since the attribute is not defined the second approach is used by default.')
            else:
                coordinates_swap_method = custom_attributes['coordinates_swap_method']
            supported_swap_methods = [
             'swap_weights', 'add_convolution']
            if coordinates_swap_method not in supported_swap_methods:
                raise Error('Unsupported "coordinates_swap_method" defined in the sub-graph replacement configuration file. Supported methods are: {}'.format(', '.join(supported_swap_methods)))
        ...
        reshape_loc_do_dims = Const(graph, {'value':int64_array([-1, (num_classes + 1) * max_proposals * 4]),  'name':reshape_loc_do_node.name + '/Dim'}).create_node()
        reshape_loc_do_dims.out_port(0).connect(reshape_loc_do_node.in_port(1))
        mark_as_correct_data_layout(reshape_loc_do_node)
        proposal_nodes_ids = [node_id for node_id, attrs in graph.nodes(data=True) if 'name' in attrs if attrs['name'] == 'crop_proposals']
        if len(proposal_nodes_ids) != 1:
            raise Error("Found the following nodes '{}' with name 'crop_proposals' but there should be exactly 1. Looks like ObjectDetectionAPIProposalReplacement replacement didn't work.".format(proposal_nodes_ids))
        proposal_node = Node(graph, proposal_nodes_ids[0])
        if 'swap_proposals' in custom_attributes:
            if custom_attributes['swap_proposals']:
                proposal_node = add_convolution_to_swap_xy_coordinates(graph, proposal_node, 4)
        reshape_priors_node = create_op_node_with_second_input(graph, Reshape, int64_array([-1, 1, max_proposals * 4]), dict(name='DetectionOutput_reshape_priors_'), proposal_node)
        ...
        detection_output_node = detection_output_op.create_node([
         reshape_loc_do_node, reshape_conf_node, reshape_priors_node], dict(name=(detection_output_op.attrs['type']), share_location=0, variance_encoded_in_target=1, code_type='caffe.PriorBoxParameter.CENTER_SIZE',
        ...
        print('The graph output nodes "num_detections", "detection_boxes", "detection_classes", "detection_scores" have been replaced with a single layer of type "Detection Output". Refer to IR catalogue in the documentation for information about this layer.')
...
class ObjectDetectionAPIMaskRCNNROIPoolingSecondReplacement(FrontReplacementFromConfigFileSubGraph):
    ...
    def generate_sub_graph(self, graph: Graph, match: SubgraphMatch):
        ...
        detection_output_nodes_ids = [node_id for node_id, attrs in graph.nodes(data=True) if 'name' in attrs if attrs['name'] == 'detection_output']
        if len(detection_output_nodes_ids) != 1:
            raise Error("Found the following nodes '{}' with 'detection_output' but there should be exactly 1.".format(detection_output_nodes_ids))
        ...
        reshape_detection_output_2d_node = create_op_node_with_second_input(graph, Reshape, int64_array([-1, 7]), dict(name='reshape_do_2d'), detection_output_node)
        ...
        crop_batch_op = Crop(graph, dict(axis=(int64_array([3])), offset=(int64_array([0])), dim=(int64_array([1])), nchw_layout=True))
        crop_batch_node = crop_batch_op.create_node([detection_output_node], dict(name='crop_do_batch_ids'))
        crop_coordinates_op = Crop(graph, dict(axis=(int64_array([3])), offset=(int64_array([3])), dim=(int64_array([4])), nchw_layout=True))
        ...
        concat_node = concat_op.create_node([crop_batch_node, crop_coordinates_node], dict(name='batch_and_coords', nchw_layout=True))
        reshape_do_node = create_op_node_with_second_input(graph, Reshape, int64_array([-1, 5]), dict(name='reshape_do'), concat_node)
        mark_as_correct_data_layout(reshape_do_node)
        roi_pooling_op = ROIPooling(graph, dict(method='bilinear', spatial_scale=1, pooled_h=roi_pool_size, pooled_w=roi_pool_size))
        roi_pooling_node = roi_pooling_op.create_node([match.single_input_node(0)[0].in_node(), reshape_do_node], dict(name='ROI_pooling_2'))
...
class ObjectDetectionAPIProposalReplacement(FrontReplacementFromConfigFileSubGraph):
    ...
    def generate_sub_graph(self, graph: Graph, match: SubgraphMatch):
        ...
        reshape_classes_node = create_op_node_with_second_input(graph, Reshape, int64_array([0, anchors_count, 2, -1]), dict(name='predictions/Reshape'))
        ...
        order_const = Const(graph, dict(value=(int64_array([0, 2, 1, 3])), name=(softmax_conf_node.name + '/TransposeOrder'))).create_node()
        permute_reshape_softmax_op = Transpose(graph, dict())
        permute_reshape_softmax_node = permute_reshape_softmax_op.create_node([softmax_conf_node, order_const], dict(name=(softmax_conf_node.name + '/Transpose')))
        ...
        if variance_height != variance_width:
            log.error('The values for variance for height "{}" is not equal to variance for width "{}". The detection results will be inaccurate.'.format(variance_height, variance_width))
        if variance_x != variance_y:
            log.error('The values for variance for x "{}" is not equal to variance for y "{}". The detection results will be inaccurate.'.format(variance_x, variance_y))
        if anchor_generator_height_stride != anchor_generator_width_stride:
            log.error('The values for the anchor generator height stride "{}" is not equal to the anchor generator width stride "{}". The detection results will be inaccurate.'.format(anchor_generator_height_stride, anchor_generator_width_stride))
        if anchor_generator_height != anchor_generator_width:
            log.error('The values for the anchor generator height "{}" is not equal to the anchor generator width stride "{}". The detection results will be inaccurate.'.format(anchor_generator_height, anchor_generator_width))
        proposal_op = ProposalOp(graph, dict(min_size=1, framework='tensorflow',
          ...
          nms_thresh=(_value_or_raise(match, pipeline_config, 'first_stage_nms_iou_threshold'))))
        for key in ('clip_before_nms', 'clip_after_nms'):
            if key in match.custom_replacement_desc.custom_attributes:
        ...
        proposal_node = proposal_op.create_node([reshape_permute_node, anchors_node, input_with_image_size_node], dict(name='proposals'))
        if 'do_not_swap_proposals' in match.custom_replacement_desc.custom_attributes and match.custom_replacement_desc.custom_attributes['do_not_swap_proposals']:
            swapped_proposals_node = proposal_node
        else:
            swapped_proposals_node = add_convolution_to_swap_xy_coordinates(graph, proposal_node, 5)
        proposal_reshape_2d_node = create_op_node_with_second_input(graph, Reshape, int64_array([-1, 5]), dict(name='reshape_swap_proposals_2d'), swapped_proposals_node)
        mark_input_as_in_correct_layout(proposal_reshape_2d_node, 0)
        crop_and_resize_nodes_ids = [node_id for node_id in bfs_search(graph, [match.single_input_node(0)[0].id]) if graph.node[node_id]['op'] == 'CropAndResize']
        assert len(crop_and_resize_nodes_ids) != 0, "Didn't find any CropAndResize nodes in the graph."
        if not ('do_not_swap_proposals' not in match.custom_replacement_desc.custom_attributes or match.custom_replacement_desc.custom_attributes['do_not_swap_proposals']):
        ...
        tf_proposal_reshape_4d_node = create_op_node_with_second_input(graph, Reshape, int64_array([-1, 1, max_proposals, 5]), dict(name='reshape_proposal_4d'), swapped_proposals_node)
        crop_op = Crop(graph, dict(axis=(int64_array([3])), offset=(int64_array([1])), dim=(int64_array([4])), nchw_layout=True))
        ...
        tf_proposals_crop_reshape_3d_node = create_op_node_with_second_input(graph, Reshape, int64_array([0, -1, 4]), dict(name='reshape_crop_3d'), crop_node)
...
class ObjectDetectionAPISSDPostprocessorReplacement(FrontReplacementFromConfigFileSubGraph):
    ...
    def run_after(self):
        return [
         ObjectDetectionAPIPreprocessorReplacement, FakeQuantWithMinMaxVarsToQuantize]

    def run_before(self):
        return [
         Div, StandaloneConstEraser, TransposeOrderNormalizer]

    def output_edges_match(self, graph: Graph, match: SubgraphMatch, new_sub_graph: dict):
        return {match.output_node(0)[0].id: new_sub_graph['detection_output_node'].id}

    def generate_sub_graph(self, graph, match):
        argv = graph.graph['cmd_params']
        if argv.tensorflow_object_detection_api_pipeline_config is None:
            raise Error(missing_param_error)
        else:
            pipeline_config = PipelineConfig(argv.tensorflow_object_detection_api_pipeline_config)
            num_classes = _value_or_raise(match, pipeline_config, 'num_classes')
            expand_dims_node = create_op_node_with_second_input(graph, Reshape, int64_array([0, 1, -1, num_classes + 1]), {'name': 'do_ExpandDims_conf'})
            ...
            reshape_loc_node = create_op_node_with_second_input(graph, Reshape, int64_array([0, -1]), {'name': 'do_reshape_loc'})
            current_node = skip_nodes_by_condition(match.input_nodes(0)[0][0].in_node(0), lambda x: x.op == 'Identity' or x.has_and_set('reinterp_shape'))
            reshape_loc_node.in_port(0).connect(current_node.out_port(0))
            mark_as_correct_data_layout(reshape_loc_node)
            reshape_conf_node = create_op_node_with_second_input(graph, Reshape, int64_array([0, -1]), {'name': 'do_reshape_conf'}, activation_conf_node)
            mark_as_correct_data_layout(reshape_conf_node)
            custom_attributes = match.custom_replacement_desc.custom_attributes
            if not 'disable_prior_boxes_layers_generator' not in custom_attributes:
                if custom_attributes['disable_prior_boxes_layers_generator'] or pipeline_config.get_param('ssd_anchor_generator_num_layers') is not None or pipeline_config.get_param('multiscale_anchor_generator_min_level') is not None:
        ...
        detection_output_node = detection_output_op.create_node([
         reshape_loc_node, reshape_conf_node, priors_node], dict(name=(detection_output_op.attrs['type']), confidence_threshold=(_value_or_raise(match, pipeline_config, 'postprocessing_score_threshold')),
          top_k=(_value_or_raise(match, pipeline_config, 'postprocessing_max_detections_per_class')),
          keep_top_k=(_value_or_raise(match, pipeline_config, 'postprocessing_max_total_detections')),
          nms_threshold=(_value_or_raise(match, pipeline_config, 'postprocessing_iou_threshold'))))
...
class ObjectDetectionAPIOutputReplacement(FrontReplacementFromConfigFileGeneral):
    ...
    def run_before(self):
        return [
         ObjectDetectionAPIPreprocessorReplacement, TransposeOrderNormalizer]

    def transform_graph(self, graph: Graph, replacement_descriptions: dict):
        if graph.graph['cmd_params'].output is not None:
            log.warning('User defined output nodes are specified. Skip the graph cut-off by the ObjectDetectionAPIOutputReplacement.')
            return
        outputs = []
...
class ObjectDetectionAPIPSROIPoolingReplacement(FrontReplacementFromConfigFileSubGraph):
    ...
    def run_after(self):
        return [
         ObjectDetectionAPIProposalReplacement, TransposeOrderNormalizer]
    ...
    def generate_sub_graph(self, graph: Graph, match: SubgraphMatch):
        ...
        if argv.tensorflow_object_detection_api_pipeline_config is None:
            ...
            if crop_height != crop_width:
                raise Error('Different "crop_height" and "crop_width" parameters from the pipeline config are not supported: {} vs {}'.format(crop_height, crop_width))
            if 'reshape_swap_proposals_2d' in graph.nodes():
                ...
                reshape_swap_proposals_node = create_op_node_with_second_input(graph, Reshape, int64_array([-1, 5]), {'name': 'reshape_swap_proposals_2d'}, swap_proposals_node)
                mark_input_as_in_correct_layout(reshape_swap_proposals_node, 0)
        psroipooling_node = PSROIPoolingOp(graph, {'name':input_node.soft_get('name') + '/PSROIPooling',  'output_dim':psroipooling_output_dim, 
         'group_size':crop_width / num_spatial_bins_width, 
         'spatial_bins_x':num_spatial_bins_width, 
         'spatial_bins_y':num_spatial_bins_height, 
         'mode':'bilinear', 
         'spatial_scale':1}).create_node([
         input_node, reshape_swap_proposals_node])
        reduce_node = create_op_node_with_second_input(graph, ReduceMean, int64_array([1, 2]), {'name':'mean', 
         'keep_dims':True}, psroipooling_node)
        output_node = match.output_node(0)[0].out_node()
        if len(output_node.in_ports()) == 2:
            if not output_node.in_port(1).disconnected():
                output_node.in_port(1).disconnect()
