...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/onnx/flattenONNX_to_reshape.py
...
class FlattenONNXToReshape(FrontReplacementSubgraph):
    ...
    def pattern(self):
        return dict(nodes=[('flatten', dict(op='FlattenONNX'))], edges=[])

    def replace_sub_graph(self, graph: Graph, match: dict):
        node = match['flatten']
        name = node.soft_get('name', node.id)
        if not node.has_valid('axis'):
            raise AssertionError("Flatten {} should have `axis` attribute extracted, but it's not".format(name))
        else:
            axis = node.axis
            if axis == 0:
                ...
                if axis == 1:
                    ...
                    first_dims = create_op_node_with_second_input(graph, ReduceProd, int64_array([0]), {'keep_dims': True})
