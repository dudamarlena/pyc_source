...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/front/subgraph_matcher.py
...
class SubgraphMatch(object):
    ...
    def __init__(self, graph: Graph, replacement_desc: CustomReplacementDescriptor, matched_nodes: list, inputs_order: list, outputs_order: list, prefix: str):
        """
    ...
    def single_input_node(self, port: int):
        ...
        if len(input_nodes) != 1:
            raise Error('The amount of input nodes for port "{}" is not equal to 1. '.format(port) + refer_to_faq_msg(33))
    ...
    def node_by_pattern(self, pattern: str):
        ...
        if len(found_names) > 1:
            raise Error('The amount of nodes matched pattern "{}" is more than 1. '.format(pattern) + refer_to_faq_msg(78))
    ...
    def _add_output_node(self, node_name: str, node_port: int, sub_graph_output_port: int):
        if sub_graph_output_port in self._output_nodes_map:
            raise Error('Output node for port "{}" has already been specified. '.format(sub_graph_output_port) + refer_to_faq_msg(34))
...
class SubgraphMatcher(object):
        ...
        for list_nodes in inputs_order:
            for node_name_pattern, port in list_nodes:
                if len(find_object_by_pattern(graph.nodes(), '.*' + node_name_pattern)) == 0:
                    log.info('Node "{} does not exist in the graph". Failed to match sub-graph by scope "{}".'.format(node_name_pattern, self.replacement_desc.id))
                    return
    ...
    def _match_sub_graph_for_points(self, graph: Graph):
        ...
        for node_name in start_points + end_points:
            if node_name not in graph.nodes():
                log.info('Node "{}" does not exist in the graph. Failed to match sub-graph by points "{}".'.format(node_name, self.replacement_desc.id))
                return

        matched_nodes = sub_graph_between_nodes(graph, start_points, end_points)
        return SubgraphMatch(graph, self.replacement_desc, matched_nodes, self.replacement_desc.get_inputs_description(), self.replacement_desc.get_outputs_description(), '')
    ...
    def matched_sub_graph_instances(self, graph: Graph):
        ...
        """
        if self.replacement_desc.match_kind == 'points':
            match = self._match_sub_graph_for_points(graph)
            if match is not None:
                yield match
        elif self.replacement_desc.match_kind == 'scope':
            ...
            raise Error('Unsupported match kind "{}". Match kinds "points" or "scope" are supported only. '.format(self.replacement_desc.match_kind) + refer_to_faq_msg(35))