...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/Deconvolution3rdInputNormalization.py
...
class Deconvolution3rdInputNormalization(MiddleReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: dict):
        node = match['op']
        if not (node.has_port('in', 2) and node.in_port(2).disconnected() or node.has_and_set('shape_input')):
            return
        if node.has_valid('layout'):
            if not node.layout.startswith('NC'):
                if graph.graph['layout'] == 'NCHW':
                    ...
                    gather = Gather(graph, {'name':node.name + '/ShapeGather',  'need_shape_inference':True}).create_node_with_data([data_node, const, axis_const])
