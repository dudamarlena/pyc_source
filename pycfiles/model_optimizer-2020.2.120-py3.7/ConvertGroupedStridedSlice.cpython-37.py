...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/ConvertGroupedStridedSlice.py
...
class ConvertGroupedStridedSlice(MiddleReplacementPattern):
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for input_data in list(graph.get_data_nodes()):
            ...
            out_nodes = [node for node in input_data.out_nodes() if node.op == 'StridedSlice' if node.in_node(0).name == input_data.name]
            if len(out_nodes) < 1:
                continue
            ...
            split.create_node_with_data(inputs=[input_data, axis_const, size_splits_const], data_nodes=final_data_nodes_list)
    ...
    def add_squeeze_for_shrink(graph: Graph, ss_node: Node):
        ...
        if graph.graph['layout'] == 'NHWC':
            if ss_node['shrink_axis_mask'][(-1)] == 1:
                dont_permute = True
        while k < len(shape_out):
            if not i >= len(ss_node['shrink_axis_mask']):
                ss_node['shrink_axis_mask'][i] or ss_shape.append(shape_out[k])
                k = k + 1
            else:
        ...
        squeeze_node = Squeeze(graph, dict(name=(ss_node.name + '/Squeeze_shrink'), nchw_layout=dont_permute,
