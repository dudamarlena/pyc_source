...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/ReplaceSpliceNodePattern.py
...
class ReplaceSpliceNodePattern(MiddleReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        input_memory = Memory(graph, {'name':'prev_splice_memory',  'id':memory_pair_id, 
         'index':1, 
         'size':2, 
         'shape':int64_array([memory_size])}).create_node()
        crop = Crop(graph, {'name':'Splice_Crop',  'axis':int64_array([1]), 
        ...
        concat_node = Concat(graph, {'name':'Splice_Concat',  'in_ports_count':2, 
         'axis':1}).create_node()
        concat_node.in_port(0).connect(crop.out_port(0))
        mem_out = Memory(graph, {'name':'out_splice_memory',  'id':memory_pair_id, 
         'index':0, 
         'size':2, 
         'shape':int64_array([memory_size])}).create_node()
        ...
        if node.const_dim != 0:
            ...
            split = create_op_with_const_inputs(graph, VariadicSplit, {1:int64_array(1),  2:int64_array([memory_element, memory_element_constdim])}, {'name':node.id + '_split_const', 
             'out_ports_count':2})
            split.out_port(0).connect(concat_node.in_port(1))
            memory_pair_id = unique_id('memory_for_const_dim')
            input_memory_const_dim = Memory(graph, {'name':'const_dim_in_memory',  'id':memory_pair_id, 
             'index':1, 
             'size':2, 
             'shape':int64_array([memory_size_constdim])}).create_node()
            crop_const_dim = Crop(graph, {'name':'const_dim_crop',  'axis':int64_array([1]), 
            ...
            concat_node_const_dim = Concat(graph, {'name':'const_dim_concat',  'in_ports_count':2, 
             'axis':1}).create_node()
            concat_node_const_dim.in_port(0).connect(crop_const_dim.out_port(0))
            mem_out_const_dim = Memory(graph, {'name':'const_dim_out_memory',  'id':memory_pair_id, 
             'index':0, 
             'size':2, 
             'shape':int64_array([memory_size_constdim])}).create_node()
            ...
            crop_first = Crop(graph, {'name':'const_dim_crop_first',  'axis':int64_array([1]), 
            ...
            concat_const = Concat(graph, {'name':node.id + '_concat_const',  'axis':1,  'in_ports_count':2}).create_node()
