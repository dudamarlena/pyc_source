...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/tf/RetinaNetFilteredDetectionsReplacement.py
# Compiled at: 2020-05-01 08:37:20
# Size of source mod 2**32: 14525 bytes
"""
...
class RetinaNetFilteredDetectionsReplacement(FrontReplacementFromConfigFileSubGraph):
    ...
    def append_variances(priors_scale_node: Node, variance: list):
        ...
        shape_part_for_tiling = StridedSlice(graph, {'name':name + '/get_-2_dim',  'begin_mask':np.array([1]),  'end_mask':np.array([1]), 
        ...
        shape_concat = Concat(graph, {'name':name + '/shape_for_tiling',  'in_ports_count':2,  'axis':np.array(0)}).create_node()
        ...
        concat = Concat(graph, {'name':name + '/priors_concat', 
         'axis':np.array(0),  'in_ports_count':2}).create_node()
    ...
    def placeholder_scales(self, placeholder: Node):
        ...
        assert shape_value is not None, '[ {} replacer ] Placeholder `{}` should have shape attribute'.format(self.replacement_id, name)
        assert isinstance(shape_value, np.ndarray), '[ {} replacer ] Placeholder `{}` shape attribute should be np.ndarray'.format(self.replacement_id, name)
        assert shape_value.size == 4, '[ {} replacer ] Placeholder `{}` should be 4D. Shape: {}'.format(self.replacement_id, name, shape_value)
        ...
        stride = Const(graph, {'value': np.array([1])}).create_node()
        spatial = StridedSlice(graph, {'name':name + '/get_h_w',  'begin_mask':np.array([1]),  'end_mask':np.array([1]), 
         'new_axis_mask':np.array([0]),  'shrink_axis_mask':np.array([0]), 
         'ellipsis_mask':np.array([0])}).create_node()
        ...
        power = Const(graph, {'value': np.array([-1.0])}).create_node()
    ...
    def generate_sub_graph(self, graph: Graph, match: SubgraphMatch):
        reshape_classes_node = create_op_node_with_second_input(graph, Reshape, int64_array([0, -1]), dict(name='do_reshape_classes'), match.single_input_node(1)[0])
        ...
        priors_node = StridedSlice(graph, {'name':priors_name + '/0_batch_slice',  'begin_mask':np.array([1, 1, 1], dtype=np.int32), 
        ...
        assert len(placeholders) == 1, '{} replacer requires model to have one Placeholder, but current model has {} placeholders'.format(self.replacement_id, len(placeholders))
        ...
        try:
            ...
            raise Error('There is no variance attribute in {} replacement config file `custom_attributes`'.format(self.replacement_id))

        priors = self.append_variances(priors_scale_node, variance)
        split_node = create_op_with_const_inputs(graph, VariadicSplit, {1:int64_array(2),  2:int64_array([1, 1, 1, 1])}, {'out_ports_count': 4}, priors_scale_node)
        priors_width_node = Sub(graph, dict(name=(split_node.name + '/sub_2-0_'))).create_node([
         (
          split_node, 2), (split_node, 0)])
        priors_height_node = Sub(graph, dict(name=(split_node.name + '/sub_3-1_'))).create_node([
         (
          split_node, 3), (split_node, 1)])
        concat_1 = Concat(graph, {'name':'concat_width_height',  'axis':-1, 
         'in_ports_count':2}).create_node([priors_width_node, priors_height_node])
        concat_2 = Concat(graph, {'name':'concat_width_height_width',  'axis':-1, 
         'in_ports_count':2}).create_node([concat_1, priors_width_node])
        concat_width_height_node = Concat(graph, {'name':'concat_priors_width_height',  'axis':-1,  'in_ports_count':2}).create_node([
         concat_2, priors_height_node])
        ...
        reshape_regression_node = create_op_node_with_second_input(graph, Reshape, int64_array([0, -1]), dict(name='reshape_regression'), applied_width_height_regressions_node)
        detection_output_op = DetectionOutput(graph, match.custom_replacement_desc.custom_attributes)
        detection_output_node = detection_output_op.create_node([
         reshape_regression_node, reshape_classes_node, priors], dict(name=(detection_output_op.attrs['type']), clip_after_nms=1, normalized=1, variance_encoded_in_target=0, background_label_id=1000))
