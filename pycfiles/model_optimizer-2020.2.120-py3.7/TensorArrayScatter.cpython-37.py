...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/ops/TensorArrayScatter.py
...
class TensorArrayScatter(Op):
    ...
    def array_infer(node: Node):
        ...
        if ta_node.has_valid('element_shape') and len(ta_node.element_shape) > 0 and not match_shapes(ta_node['element_shape'], value.shape[1:]):
            raise AssertionError('Shapes are not compatible: {} and {}'.format(ta_node['element_shape'], value.shape[1:]))
        else:
