...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/ops/gather.py
...
class Gather(Op):
    ...
    def __init__(self, graph, attrs):
        ...
        assert 'axis' not in self.attrs, 'Use AttributedGather operation instead of Gather to create it with `axis` as a parameter'
    ...
    def infer(node: Node):
        ...
        if not (len(connected_in_ports) == 3 and 0 in connected_in_ports and 1 in connected_in_ports and 2 in connected_in_ports):
            raise AssertionError("Gather should have 3 connected input port, but it doesn't for node: `{}`. Ports: {}".format(name, connected_in_ports))
        ...
        if data_value is not None:
            if indices_value is not None:
                node.out_port(0).data.set_value(np.array((np.take(data_value, int64_array(indices_value), axis)), dtype=(data_value.dtype)))
                return
...
class AttributedGather(Op):
    ...
    def infer(node: Node):
        ...
        if not (len(connected_in_ports) == 2 and 0 in connected_in_ports and 1 in connected_in_ports):
            raise AssertionError("AttributedGather should have 2 connected input port, but it doesn't for node: `{}`. Ports: {}".format(name, connected_in_ports))
