...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/middle/passes/fusing/fuse_linear_ops.py
# Compiled at: 2020-05-01 08:37:22
# Size of source mod 2**32: 11175 bytes
"""
...
from mo.middle.passes.fusing.helpers import backward_bfs, forward_bfs, get_value_in_port, get_tensor_in_port
...
def _fuse_mul(graph: Graph, node: Node, fuse_nodes: list, backward: bool=True):
    ...
    for fuse_node in fuse_nodes:
        if fuse_node.soft_get('can_be_fused') is False:
            ...
            if len(fuse_node.in_ports()) < 2:
                ...
                weights_port.data.has_valid('output_channel_dim') and weights_port.data.has_valid('input_channel_dim') or log.warning('Cannot do fuse_mul for node {} because there is no field ' + 'output_channel_dim and/or input_channel_dim in weights.'.format(fuse_node.soft_get('name')))
                return False
    ...
    for fuse_node in fuse_nodes:
        ...
        w_mul = node.copy_node({'in_ports_count':len(node.in_ports()),  'out_ports_count':len(node.out_ports()),  'can_be_fused':False})
        ...
        if backward:
            if len(fuse_node.in_ports()) == 3 and not fuse_node.in_port(2).disconnected():
                if not fuse_node.has_and_set('shape_input'):
...
def _fuse_add(graph: Graph, node: Node, fuse_nodes: List[Node], backward: bool=True):
    ...
    for fuse_node in fuse_nodes:
        ...
        elif fuse_node.in_port(2).disconnected() or fuse_node.in_port(2).data.get_value() is None:
            if value.size == 1:
...
def fuse_linear_ops(graph: Graph):
    ...
    for node in nodes:
        is_fused = False
        if node.soft_get('op') == 'Mul':
            if get_value_in_port(node) is not None:
                if node.soft_get('can_be_fused') is True:
    ...
    for node in nodes:
        ...
        if hasattr(graph, 'graph') and 'cmd_params' in graph.graph and not graph.graph['cmd_params'].generate_experimental_IR_V10:
            if node.soft_get('op') == 'Add' and get_value_in_port(node) is not None:
                if node.soft_get('can_be_fused') is True:
