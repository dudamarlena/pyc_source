...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/mxnet/mx_reshape_to_reshape.py
...
class MXReshapeToReshape(FrontReplacementSubgraph):
    ...
    def resolve_handlers(self, case):
        return {-2:self.resolve_minus2, 
         -3:self.resolve_minus3, 
         -4:self.resolve_minus4}.get(case, self.resolve_const_shape)
    ...
    def resolve_minus2(self, shape_node, input_index, reshape_index, dims):
        ...
        shape_values_node = get_shape_values_by_range_idxs(shape=shape_node, rank=rank_node, begin=input_index,
          end=(-1),
          include_begin=True,
          include_end=True)
        input_index = None
    ...
    def resolve_minus3(self, shape_node, input_index, reshape_index, dims):
        shape_indexes_node1 = Const(shape_node.graph, dict(name=(shape_node.id + '/ShapeMinus3_index_const1_' + str(input_index)), value=(int64_array([input_index])))).create_node()
        dims_node1 = get_shape_values_by_indices_node(shape_node, shape_indexes_node1)
        shape_indexes_node2 = Const(shape_node.graph, dict(name=(shape_node.id + '/ShapeMinus3_index_const2_' + str(input_index)), value=(int64_array([input_index + 1])))).create_node()
    ...
    def resolve_minus4(self, shape_node, input_index, reshape_index, dims):
        shape_const_node = Const(shape_node.graph, dict(name=(shape_node.id + '/ShapeMinus4_index_const_' + str(input_index)), value=(np.take(dims, [reshape_index + 1, reshape_index + 2])))).create_node()
        input_index = input_index + 2
    ...
    def resolve_const_shape(self, shape_node, input_index, reshape_index, dims):
        dim_const_node = Const(shape_node.graph, dict(name=(shape_node.id + '/DimConst_' + str(reshape_index)), value=[
         dims[reshape_index]])).create_node()
    ...
    def resolve(self, input_index, reshape_index, dims, input_shape_node, output_dims_nodes):
        ...
        input_index, reshape_index, dims, dims_node = resolve_handler(input_shape_node, input_index, reshape_index, dims)
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        concat_node = Concat(shape_node.graph, dict(name=(shape_node.id + '/ConcatMXReshape_'), axis=0, in_ports_count=(len(output_dims_nodes)))).create_node()
