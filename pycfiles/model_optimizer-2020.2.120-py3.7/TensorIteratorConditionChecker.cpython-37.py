...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/TensorIteratorConditionChecker.py
...
class ConditionChecks(MiddleReplacementPattern):
    ...
    def replace_pattern(graph, match: dict):
        ...
        params = ss.in_nodes()
        if not np.all(params[1].in_node().value == 0):
            ...
            if match['minimum_data'].value is None:
                log.warning("TF loop doesn't have a constant upper bound produced by node {}, or ModelOptimizer cannot detect a constant in this case. Loops with a dynamic number of iterations are not supported, so in the resulting IR, generated TensorIterator will have a maximum number of iterations determined by input tensor size: {}".format(match['minimum_data'].soft_get('name'), match['Strided_slice_data'].value))
            else:
                assert match['Strided_slice_data'].value == match['minimum_data'].value, 'Values do not match: {} and {}'.format(match['Strided_slice_data'].value, match['minimum_data'].value)
        ...
        for ta in inputs_and_outputs:
            if ta.has_valid('kind'):
                if ta['kind'] == 'op':
                    if ta['op'] in type_list and not ta.in_node(0).id == ss.id:
                        raise AssertionError
