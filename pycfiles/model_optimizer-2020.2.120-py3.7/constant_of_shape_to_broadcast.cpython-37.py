...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/onnx/constant_of_shape_to_broadcast.py
...
class ConstantOfShapeToBroadcast(FrontReplacementPattern):
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for const_of_shape_node in graph.get_op_nodes(op='ConstantOfShape'):
            ...
            broadcast_node.in_port(0).connect(Const(graph, {'name':broadcast_node.name + '/FillValue',  'value':const_of_shape_node.fill_value}).create_node().out_port(0))
