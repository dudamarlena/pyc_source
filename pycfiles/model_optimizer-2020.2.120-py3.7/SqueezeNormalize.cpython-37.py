...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/SqueezeNormalize.py
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for squeeze_node in graph.get_op_nodes(op='Squeeze'):
            if len(squeeze_node.in_nodes()) == 1 and squeeze_node.has_valid('squeeze_dims'):
                dims_node = Const(graph, {'name':squeeze_node.id + '/Dims',  'value':int64_array(squeeze_node.squeeze_dims)}).create_node()
                ...
                raise Error('The Squeeze layer "{}" should either have 2 inputs or one input and an "squeeze_dims" attribute'.format(squeeze_node.soft_get('name')))