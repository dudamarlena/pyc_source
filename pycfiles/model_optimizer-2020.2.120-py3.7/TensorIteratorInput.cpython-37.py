...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/TensorIteratorInput.py
# Compiled at: 2020-05-01 08:37:21
# Size of source mod 2**32: 11043 bytes
"""
...
import extensions.middle.AddIsCyclicAttribute as AddIsCyclicAttribute
...
class SmartInputMatcher(MiddleReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        if not (match['stack_data']['value'][0] == 0 and match['stack_1_data']['value'][0] == 1 and match['stack_2_data']['value'][0] == 1):
            raise AssertionError
        if not (match['start_data']['value'] == 0 and match['delta_data']['value'] == 1):
            raise AssertionError
        ...
        if 0 in ta_size.in_nodes():
            ...
            if shape['kind'] == 'op':
                if shape['op'] == 'Const':
                    ...
                    log.warning('You network cannot be reshaped since shapes of placeholders is a contants.Please, provide non-constant shapes. ')
        input_node = TensorIteratorInput(graph, dict(axis=0, start=start, stride=None, part_size=None, external_port_id=(str(match['Enter_data'].value)),
          internal_layer_id=(match['TensorArrayRead_data'].id),
          name=(match['TensorArrayRead'].name + '/TensorIteratorInput_')))
        input_node.create_node_with_data(inputs=[ta_size_data, value, match['Condition_data']], data_nodes=[
         match['TensorArrayRead_data']])
        safe_nodes = [
         'TensorArrayRead_data', 'Condition', 'Condition_data']
        nodes_for_remove = []
...
class SimpleInputMatcher(MiddleReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        input_node = TensorIteratorInput(graph, dict(external_port_id=None, internal_layer_id=None,
...
class BackEdgeSimpleInputMatcher(MiddleReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        if not len(match['BackEdge'].in_nodes()) == 3:
            raise AssertionError
        else:
            condition = match['BackEdge'].in_node(2)
            init_input = match['BackEdge'].in_node(0)
            cycle_input = match['BackEdge'].in_node(1)
            if (len(init_input.in_nodes()) == 0 or len(init_input.in_nodes())) == 1 and init_input.has_valid('value'):
                if init_input.in_node(0).soft_get('op') != 'TensorIteratorInput':
                    input_node = TensorIteratorInput(graph, dict(external_port_id=None, internal_layer_id=None,
