...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/utils/logger.py
...
"""
import importlib.util, logging as log, os, re, sys
...
class LvlFormatter(log.Formatter):
    format_dict = {log.DEBUG: '[ %(asctime)s ] [ %(levelname)s ] [ %(module)s:%(lineno)d ]  %(msg)s', 
...
class TagFilter(log.Filter):
    ...
    def filter(self, record: log.LogRecord):
        if record.__dict__['funcName'] == 'load_grammar':
            return False
        if self.regex:
...
def progress_bar(function: callable):
    ...
    def wrapper(*args, **kwargs):
        for arg in ('graph', 'curr_transform_num', 'num_transforms'):
            msg = 'Progress bar decorator is enabled for Model Optimizer transformation applying cycle only. Argument `{}` {}'
