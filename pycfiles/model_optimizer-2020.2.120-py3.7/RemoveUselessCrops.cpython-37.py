...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/RemoveUselessCrops.py
...
class RemoveUselessCropsPattern(MiddleReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        if len(crop_node.out_port(0).get_destinations()) != 1:
            ...
            for in_port in outs:
                out = in_port.node
                if out['op'] == 'Crop' and out['axis'] == axis and len(out.out_port(0).get_destinations()) == 1 and out.out_port(0).get_destination().node == concat_node:
