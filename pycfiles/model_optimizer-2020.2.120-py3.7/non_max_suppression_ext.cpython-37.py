...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/tf/non_max_suppression_ext.py
...
class NonMaxSuppressionV4Extractor(FrontExtractorOp):
    ...
    def extract(cls, node):
        ...
        if not pad_to_max_output_size:
            log.warning('The attribute "pad_to_max_output_size" of node {} is equal to False which is not supported.Forcing it to be equal to True'.format(node.soft_get('name')))
        attrs = {'sort_result_descending':1, 
         'box_encoding':'corner'}
        NonMaxSuppression.update_node_stat(node, attrs)
        return cls.enabled
