...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/tf/mvn_unrolled.py
...
class MVNUnrolled(FrontReplacementSubgraph):
    ...
    @staticmethod
    def replace_sub_graph(graph, match):
        mvn = MVN(graph, dict(name=(match['truediv'].name + '/MVN_'),
    ...
    @staticmethod
    def infer(node: Node):
        if node.in_node(1).has_valid('value'):
            node.in_node(2).has_valid('value') or log.warning('Reduction indices for mean and variance for MVN node {} are not constants'.format(node.name))
            return
        else:
            if not (all(node.in_node(1).value == node.required_reduction_indices) and all(node.in_node(2).value == node.required_reduction_indices)):
                log.warning('Reduction indices for mean {} and variance {} do not match required ones {}'.format(node.in_node(1).value, node.in_node(2).value, node.required_reduction_indices))
                return
            node.in_node(3).has_valid('value') and node.in_node(4).has_valid('value') or log.warning('Power or/and epsilon values for MVN node {} are not constants'.format(node.name))
            return
