...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/ops/pooling.py
        ...
        if not len(node.in_nodes()) == 1:
            ...
            if input_shape is None:
                ...
                if node.has_valid('auto_pad'):
                    node.pad_spatial_shape, node.output_spatial_shape = tf_window_op_pad_infer(input_spatial_shape, window_spatial_shape, stride_spatial, node.auto_pad)
                ...
                if node.soft_get('pooling_convention') == 'full' or node.soft_get('rounding_type') == 'ceil':
                    rounding = np.ceil
                output_spatial_shape = np.array((rounding(np.array((input_spatial_shape + pad_spatial_shape - window_spatial_shape), dtype=(np.float)) / stride_spatial)),
                  dtype=(np.int64)) + 1
                original_pads = np.array([i[1] for i in node.pad_spatial_shape])
                for i in range(len(input_spatial_shape)):
                    if original_pads[i] and (output_spatial_shape[i] - 1) * stride_spatial[i] >= input_spatial_shape[i] + original_pads[i]:
