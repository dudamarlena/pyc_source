...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/GeLUMerger_Erf.py
...
class GeLUMergerErf(FrontReplacementSubgraph):
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        if div.in_port(0).get_source().node.soft_get('type') == 'Const':
            ...
            if div.in_port(1).get_source().node.soft_get('type') == 'Const':
                div_param = div.in_port(1).get_source().node.value
            elif mul.in_port(0).get_source().node.soft_get('type') == 'Const':
                mul_param = mul.in_port(0).get_source().node.value
            else:
                if mul.in_port(1).get_source().node.soft_get('type') == 'Const':
