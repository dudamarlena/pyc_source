...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/onnx/detectionoutput_ext.py
...
class ExperimentalDetectronDetectionOutputFrontExtractor(FrontExtractorOp):
    ...
    def extract(cls, node):
        attrs = dict(class_agnostic_box_regression=(onnx_attr(node, 'class_agnostic_box_regression', 'i', 0)), max_detections_per_image=(onnx_attr(node, 'max_detections_per_image', 'i', 100)),
          ...
          max_delta_log_wh=(onnx_attr(node, 'max_delta_log_wh', 'f', log(62.5))),
          deltas_weights=np.array((onnx_attr(node, 'deltas_weights', 'floats', [10.0, 10.0, 5.0, 5.0])), dtype=(np.float32)))
