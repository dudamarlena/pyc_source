...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/permute_tensor_iterator.py
# Compiled at: 2020-05-01 08:37:21
# Size of source mod 2**32: 9692 bytes
"""
...
class TransposeTensorIteratorLSTM(MiddleReplacementPattern):
    __doc__ = " Fuses Transpose(1,0,2) --> TI --> Transpose(1,0,2) pattern to a single TI with changed axis.\n\n        WARNING This transformation is limited to support of very special case of TI but\n        code doesn't check all the cases.\n    "
    enabled = True

    def run_after(self):
        return [
         TensorIteratorMerge, ONNXRNNSequenceNormalize, LSTMToTensorIterator, SwapAxisMiddleReplacer]
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        if len(list(isomorphisms)) != 1:
            ...
            return direct_order is None or np.array_equal(direct_order, permute_order) or None
            inverse_order = inverse_permute.in_port(1).data.get_value()
            return inverse_order is None or np.array_equal(inverse_order, permute_order) or None
        ...
        for shape_of_dst in [dst for dst in direct_permute_dsts if dst.node.soft_get('type') == 'ShapeOf']:
            ...
            gather = create_op_with_const_inputs(graph, op=Gather, op_attrs={'name': name}, port_value_dict={1:int64_array(permute_order), 
             2:int64_array(0)})
