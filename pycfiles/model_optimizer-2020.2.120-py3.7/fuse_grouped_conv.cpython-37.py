...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/middle/passes/fusing/fuse_grouped_conv.py
...
def concat_convolutions(graph: Graph, start_node: Node, last_node: Node):
    ...
    for id in range(len(conv_nodes)):
        ...
        if conv.out_node().id != last_node.in_node(id).id:
            return False
            np.array_equal(conv.in_node(1).shape, gconv.in_node(1).shape) or log.debug('Grouped convolutions fusion : convolutions have different weights shape')
            return False
    ...
    if has_biases:
        for id in range(len(conv_nodes)):
            ...
            if conv.in_node(2).shape != gconv.in_node(2).shape:
                log.debug('Group convolutions fusion : convolutions have different biases shape {} and {}'.format(conv.in_node(2).shape, gconv.in_node(2).shape))
                return False
    ...
    for conv in conv_nodes[1:]:
        ...
        if has_biases:
            bias_value = np.concatenate((bias_value, conv.in_node(2).value), axis=(-1))
    ...
    log.debug('Start node : {} Last node : {}  Nodes inside : {}'.format(start_node.id, last_node.id, len(start_node.out_nodes())))
    ...
    while 1:
        ...
        for node in graph.pseudo_topological_sort():
            if node.kind == 'op':
                ...
                if len(next_nodes) > 1 and all((_node.soft_get('type') in ('Convolution',
                                                                           'Deconvolution') for _node in next_nodes)):
                    for conv in next_nodes:
                        ...
                        if conv_outputs[0].id != last_layer:
                            is_valid_convolutions = False
