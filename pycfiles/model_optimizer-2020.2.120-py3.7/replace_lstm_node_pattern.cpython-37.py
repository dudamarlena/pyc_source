...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/kaldi/replace_lstm_node_pattern.py
...
class ReplaceLSTMNodePattern(FrontReplacementOp):
    ...
    def run_before(self):
        ...
        import extensions.front.MoveEmbeddedInputsToInputs as MoveEmbeddedInputsToInputs
        return [
    ...
    def replace_op(self, graph: Graph, node: Node):
        ...
        prev_lstm_output = Memory(graph, {'name':'prev_memory_output',  'id':memory_pair_input, 
         'index':1, 
         'size':2, 
         'shape':np.array([node.gifo_r_weights_shape[1]], dtype=np.int64)}).create_node()
        ...
        join_input_prev_state_sum = Add(graph, {'name': 'join_input_eltwise'}).create_node([
         fc_layer_from_prev_state, fc_layer_after_input])
        split_joined_input_axis = Const(graph, {'value': np.int64(1)}).create_node()
        split_joined_input = Split(graph, {'name':'join_input_split',  'num_splits':4}).create_node([
         join_input_prev_state_sum, split_joined_input_axis])
        prev_lstm_state = Memory(graph, {'name':'prev_memory_state',  'id':memory_pair_output, 
         'index':1, 
         'size':2, 
         'shape':np.array([node.input_gate_weights.shape[0]], dtype=np.int64)}).create_node()
        ...
        join_prev_lstm_input_joined_input_sum = Add(graph, {'name': 'join_prev_lstm_input_joined_input_eltwise'}).create_node([
         (
          split_joined_input, 1),
         state_input_scaleshift])
        join_prev_lstm_input_joined_forget_sum = Add(graph, {'name': 'join_prev_lstm_input_joined_forget_sum'}).create_node([
         (
          split_joined_input, 2),
        ...
        join_forget_clamp = Clamp(graph, {'name':'join_forget_clamp',  'max':node.clip_value, 
         'min':-node.clip_value}).create_node([
         join_forget_remember_sum])
        next_lstm_state = Memory(graph, {'name':'next_lstm_state',  'id':memory_pair_output, 
         'index':0, 
         'size':2, 
         'shape':np.array([node.input_gate_weights.shape[0]], dtype=np.int64)}).create_node([
         join_forget_clamp])
        ...
        join_next_lstm_input_joined_input_sum = Add(graph, {'name': 'join_next_lstm_input_joined_input_sum'}).create_node([
         (
          split_joined_input, 3), clamp_scaleshift])
        ...
        next_lstm_output = Memory(graph, {'name':'next_lstm_output',  'id':memory_pair_input, 
         'index':0, 
         'size':2, 
         'shape':np.array([node.gifo_r_weights_shape[1]], dtype=np.int64)}).create_node([
         fc_output])
