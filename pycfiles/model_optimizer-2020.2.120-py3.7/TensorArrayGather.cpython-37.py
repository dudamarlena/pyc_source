...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/ops/TensorArrayGather.py
...
class TensorArrayGather(Op):
    ...
    def array_infer(node: Node):
        if not len(node.in_nodes()) == 3:
            ...
            if ta_node.has_valid('element_shape') and ta_node.element_shape is not None and len(ta_node.element_shape) > 0 and not symm_match_shapes(ta_node['element_shape'], node.element_shape):
                raise AssertionError
            else:
                ta_node['element_shape'] = node.element_shape
            data_shape = ta_node['element_shape']
            if not -1 not in data_shape:
                if data_shape.size == 2:
                    if not (data_shape[0] == -1 and data_shape[1] != -1):
                        raise AssertionError
        assert ta_node.has_valid('size')
        size = ta_node['size']
        assert size > 0
        output_shape = [
         size] + [data_shape[i] for i in range(len(data_shape))]
        output_value = None
