...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/mxnet/ssd_anchor_reshape.py
...
class SsdPatternAnchorReshape(FrontReplacementSubgraph):
    ...
    def run_after(self):
        return [
         MulScalarFrontReplacer, AttributedSplitToSplit]

    def run_before(self):
        return [
         SsdPatternDetectionOutputReplacer]
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        variants_dict = {'mul_scalar1x':0.1, 
         'mul_scalar2x':0.2,  'mul_scalar1y':0.1,  'mul_scalar2y':0.2}
        ...
        variants_slice_like = Crop(graph, dict(axis=(slice_like.axis), offset=(slice_like.offset), dim=(slice_like.dim), axes=(slice_like.axes), symbol_dict={'name': slice_like.id + '/variants_slice_like'})).create_node()
