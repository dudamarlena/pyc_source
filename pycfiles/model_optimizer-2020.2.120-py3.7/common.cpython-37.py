...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/pipeline/common.py
# Compiled at: 2020-05-01 08:37:22
# Size of source mod 2**32: 11717 bytes
"""
...
import extensions.back.RemoveUselessConvert as RemoveUselessConvert
...
def get_fw_tensor_debug_info(node: Node):
    while not node.has_valid('fw_tensor_debug_info'):
        if not node.has_valid('output_sort_order'):
...
def get_sorted_outputs(graph: Graph):
    ...
    if len(outputs_for_sort) != len(set(outputs_for_sort.values())):
        log.warning('There are at least two output nodes with the same key used to sort the outputs. This means that IRs with different order of nodes may be generated between Model Optimizer runs. The dictionary with outputs is: {}'.format(outputs_for_sort))
...
def convert_inputs_of_specific_ops(graph: Graph):
    type_port = {'Broadcast':{1:'int64',  2:'int64'},  'ConvolutionBackpropData':{2: 'int64'}, 
     ...
     'Reshape':{1: 'int64'}, 
     'Squeeze':{1: 'int64'}, 
     'StridedSlice':{1:'int64', 
      2:'int64',  3:'int64'}, 
     'Split':{1: 'int64'}, 
     'Tile':{1: 'int64'}, 
     'Transpose':{1: 'int64'}, 
     'Unsqueeze':{1: 'int64'}, 
     'VariadicSplit':{1:'int64', 
      2:'int64'}}
    for node in graph.get_op_nodes():
        if node.soft_get('type') in type_port:
            ...
            for port_id, precision in ports_to_update.items():
                if port_id in node.in_ports():
                    node.in_port(port_id).disconnected() or log.debug('Converting value for the input port "{}" of op "{}" to "{}".'.format(port_id, node.soft_get('name', node.id), precision))
...
def convert_outputs_of_specific_ops(graph: Graph):
    type_port = {'ShapeOf':{0: 'int32'},  'NonMaxSuppression':{0: 'int32'}}
    for node in graph.get_op_nodes():
        if node.soft_get('type') in type_port:
            ports_to_update = type_port[node.soft_get('type')]
            for port_id, precision in ports_to_update.items():
                if port_id in node.out_ports():
                    log.debug('Insert Convert after op "{}" to type "{}"'.format(node.soft_get('name', node.id), precision))
                    node.out_port(port_id).get_connection().insert_node(Cast(graph, {'dst_type': data_type_str_to_np(precision)}).create_node())


def prepare_emit_ir(graph: Graph, data_type: str, output_dir: str, output_model_name: str, mean_data: [list, None]=None, input_names: list=None, meta_info: dict=None):
    if input_names is None:
        ...
        graph.graph['cmd_params'].generate_deprecated_IR_V7 or for_graph_and_each_sub_graph_recursively(graph, convert_inputs_of_specific_ops)
    ...
    generate_ie_ir(graph=graph, file_name=(os.path.join(output_dir, '{}.xml'.format(output_model_name))),
...
def get_ir_version(argv: argparse.Namespace):
    ...
    if argv.generate_experimental_IR_V10:
        ...
        if argv.generate_deprecated_IR_V2:
            version = 2
        else:
            version = 7
    return version