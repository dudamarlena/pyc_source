...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/MXNetSplitMultiLayers.py
...
class MXNetSplitLayersToRNNSequence(MiddleReplacementPattern):
    ...
    def split_multilayer_cell(self, graph: Graph, match: dict):
        ...
        for l in range(num_layers):
            ...
            layer_params_w = param_w[params_layer_size_count:params_layer_size_count + (params_layer_size - layer_bsize)].copy()
            ...
            params_value_node = Const(rnn_layer.graph, dict(name=(name + '/LayerSplittedParamsLSTM/{}/'.format(l)), value=layer_params)).create_node_with_data()
            if have_hidden:
                layer_hidden_state = hidden_state_value[l * direction:l * direction + direction]
                hidden_state_value_node = Const(rnn_layer.graph, dict(name=(name + '/LayerSplittedHiddenState/{}/'.format(l)), value=layer_hidden_state)).create_node_with_data()
            else:
                hidden_state_value_node = None
            if have_cell:
                layer_cell_state = cell_state_value[l * direction:l * direction + direction]
                cell_state_value_node = Const(rnn_layer.graph, dict(name=(name + '/LayerSplittedCellState/{}/'.format(l)), value=layer_cell_state)).create_node_with_data()
            else:
                cell_state_value_node = None
            if l < num_layers - 1:
                output_data = Op._create_data_node((rnn_layer.graph),
    ...
    def concat_output_states(graph: Graph, match: dict, new_states: list):
        ...
        rnn_layer = match['rnn_layer']
        original_states = [rnn_layer.out_node(i) if i in rnn_layer.out_nodes() else None for i in (1,
                                                                                                   2)]
        concat_ops = [
         Concat(rnn_layer.graph, {'name':rnn_layer.name + '/FinalLayerSplitConcat/HiddenState', 
          'axis':-1}),
         Concat(rnn_layer.graph, {'name':rnn_layer.name + '/FinalLayerSplitConcat/CellState', 
          'axis':-1})]
