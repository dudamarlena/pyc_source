...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/ops/topk.py
...
class TopK(Op):
    ...
    def infer(node):
        ...
        assert len(connected_ports) == 2, 'The number of inputs to the TopK layer name "{}" must be equal to 2.'.format(node.soft_get('name'))
        k = node.in_port(1).data.get_value()
        if k is None:
            raise Error('The value defining number of output elements for layer "{}" is not defined'.format(node.soft_get('name')))
