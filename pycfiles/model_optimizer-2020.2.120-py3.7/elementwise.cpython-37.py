...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/ops/elementwise.py
...
class Elementwise(Op):
    ...
    def __init__(self, graph, attrs):
        super().__init__(graph, {'op':self.op, 
         ...
         'out_ports_count':1, 
         'is_eltwise':True, 
         'stop_value_propagation':False}, attrs)
    ...
    def type_infer(node):
        ...
        if in_type_0 != in_type_1:
            ...
            if in_node_0.op == 'Const':
                ...
                if in_node_1.op == 'Const':
                    convert_const_node_value_type(in_node_1, in_type_0)
                else:
                    log.error('Elementwise operation {} has inputs of different data types: {} and {}'.format(node.soft_get('name'), in_type_0, in_type_1))
...
class Div(Elementwise):
    ...
    op = 'Div'
    op_type = 'Divide'
    operation = staticmethod(lambda a, b:     if both_types_are_integer(a, b):
a // b # Avoid dead code: a / b)
...
class Pow(Elementwise):
    ...
    def type_infer(node):
        in_type_0 = node.in_port(0).get_data_type()
        in_type_1 = node.in_port(1).get_data_type()
        assert in_type_0 == in_type_1, 'Power operation {} has inputs of different data types: {} and {}'.format(node.soft_get('name'), in_type_0, in_type_1)
