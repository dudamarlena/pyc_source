...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/ops/const.py
...
"""
import numpy as np
from mo.middle.passes.convert_data_type import data_type_str_to_np, np_data_type_to_destination_type, precision_to_destination_type
...
class Const(Op):
    ...
    def __init__(self, graph, attrs=None):
        super().__init__(graph, {'type':self.op, 
         ...
         'data_type':None, 
         'out_ports_count':1, 
         'type_infer':self.type_infer}, attrs)
        ...
        if 'force_shape' in self.attrs:
            if self.attrs['force_shape'] is not None:
                self.attrs['shape'] = np.array((self.attrs['force_shape']), dtype=(np.int64))
        self.attrs['data_type'] = self.attrs['value'].dtype
        if 'force_type' in self.attrs:
            if self.attrs['force_type'] is not None:
    ...
    def supported_attrs(self):
        if self.ir_version == 10:
            return ['offset',
             ...
             (
              'element_type',
              lambda node:               if node.has_valid('force_type'):
precision_to_destination_type(node.force_type) # Avoid dead code: np_data_type_to_destination_type(node.value.dtype))]
        return []
    ...
    @staticmethod
    def infer(node):
        output_value = node.value if (isinstance(node.value, np.ndarray) or len(node.shape) == 0) else (np.full(node.shape, node.value))
