...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/utils/ir_engine/compare_graphs.py
...
def compare_graphs(graph: Graph, graph_ref: Graph, last_node: str, last_node_ref=None, check_op_attrs=False):
    ...
    if 'statistics' in graph.graph:
        if 'statistics' in graph_ref.graph:
    ...
    while len(q_ref) != 0:
        if len(q) == 0:
            ...
            if len(node_ref.out_nodes()) != len(node.out_nodes()):
                stderr.append('Current node "{}" and reference node "{}" have different amount of output nodes: {} vs {}'.format(node.id, node_ref.id, len(node.out_nodes()), len(node_ref.out_nodes())))
                continue
            if len(node_ref.in_nodes()) != len(node.in_nodes()):
                stderr.append('Current node "{}" and reference node "{}" have different amount of input nodes: {} vs {}'.format(node.id, node_ref.id, len(node.in_nodes()), len(node_ref.in_nodes())))
                continue
            if node_ref.kind != node.kind:
                stderr.append('Current node "{}" and reference node "{}" have different kind parameter'.format(node.id, node_ref.id))
                return (False, stderr)
            if node_ref.has_valid('can_be_fused'):
                if node_ref.soft_get('can_be_fused') != node.soft_get('can_be_fused'):
                    stderr.append('Current node "{}" and reference node "{}" have different "can_be_fused" parameter {} and {}'.format(node.id, node_ref.id, node.soft_get('can_be_fused'), node_ref.soft_get('can_be_fused')))
            if node_ref.kind == 'op':
                if check_op_attrs:
                    ...
                    for attr in graph_ref.node[node_ref.id]:
                        if graph_ref.node[node_ref.id][attr] is None or attr in ('name',
                                                                                 'id',
                                                                                 '_in_ports',
                                                                                 '_out_ports',
                                                                                 'infer',
                        ...
                        if attr not in graph.node[node.id]:
                            ...
                            continue
                        if attr == 'value':
                            values_are_equal(node.value, node_ref.value) or stderr.append('Current node "{}" with type {} and reference node "{}" with type have different values \n{} \nand \n{}'.format(node.id, cur_node_type, node_ref.id, ref_node_type, node.value, node_ref.value))
                            continue

                        def err_format_string():
                            return 'Current node "{}" with type {} and reference node "{}" with type {} have different attr "{}" : {} and {}'.format(node.id, cur_node_type, node_ref.id, ref_node_type, attr, graph.node[node.id][attr], graph_ref.node[node_ref.id][attr])
                        ...
                        if type(graph_ref.node[node_ref.id][attr]) in [np.ndarray, list]:
                            ...
                            eps = 0.05 if (node.has('precision') and node['precision'] == 'FP16') else 0.0001
                            ...
                            if isinstance(graph.node[node.id][attr], np.ndarray):
                                if not np.array_equal(graph.node[node.id][attr], graph_ref.node[node_ref.id][attr]):
                                    stderr.append(err_format_string())
            ...
            if node_ref.has_valid('shape'):
                if not node.has_valid('shape'):
                    stderr.append('{} has None shape'.format(node.id))
                if node_ref.has_valid('value'):
                    if not node.has_valid('value'):
                        stderr.append('{} has None value'.format(node.id))
            elif node_ref.has_valid('shape'):
                if node_ref.shape is not None:
                    np.array_equal(node_ref.shape, node.shape) or stderr.append('Current node "{}" and reference node "{}" have different shapes {} and {}'.format(node.id, node_ref.id, node.shape, node_ref.shape))
                    continue
            if node_ref.has_valid('value'):
                if node_ref.value is not None:
