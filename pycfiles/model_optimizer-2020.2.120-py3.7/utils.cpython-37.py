...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/utils/utils.py
...
"""
import functools, os, re, warnings, numpy as np

def refer_to_faq_msg(question_num: int):
    return '\n For more information please refer to Model Optimizer FAQ (https://docs.openvinotoolkit.org/latest/_docs_MO_DG_prepare_model_Model_Optimizer_FAQ.html), question #{}. '.format(question_num)
    ...
    def deprecated(func):
        ...
        def deprecation_message(*args, **kwargs):
            ...
            if class_name is not None:
                dep_msg += 'Please use {}.{} method'.format(class_name.__name__ if not isinstance(class_name, str) else class_name, func.__name__ if new_method_name is None else new_method_name)
...
def files_by_pattern(dir: str, pattern: str, files_only=True, add_prefix=False):
    ...
    for file_name in os.listdir(dir):
        if re.match(pattern_compiled, file_name):
            if not files_only or os.path.isfile(os.path.join(dir, file_name)):
...
def get_mo_root_dir():
    ...
    """
    return os.path.normpath(os.path.join(os.path.dirname(os.path.abspath(os.path.realpath(__file__))), os.pardir, os.pardir))


def convert_param_type(node, attr_name: str, type_v1x, type_v7):
    if attr_name in node.attrs():
        if node[attr_name] is not None:
