...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/MXNetRNNSequenceNormalize.py
...
class MXNetRNNSequenceNormalize(MiddleReplacementPattern):
    ...
    def repack_weights(graph: Graph, match: dict):
        ...
        W, R = [x.reshape([direction, 1, rnn_layer.multiplier, rnn_layer.hidden_size, -1]) for x in (W, R)]
    ...
    def check_init_states(graph: Graph, match: dict):
        ...
        if 2 not in rnn_cell.in_nodes():
            h_shape = [
             num_directions, batch_size, rnn_cell.hidden_size]
            h_init = np.full(h_shape, 0, dtype=(np.float32))
            Op.create_and_connect_input_data_node(graph, rnn_cell, {'value':h_init, 
        ...
        if rnn_cell.op == 'LSTM':
            if 3 not in rnn_cell.in_nodes():
                ...
                Op.create_and_connect_input_data_node(graph, rnn_cell, {'value':c_init, 
    ...
    def add_output_reshape(graph: Graph, match: dict):
        ...
        if not lstm.has_num_directions:
            ...
            if lstm.batch_dim == 0:
                mo_shape = np.array([input.shape[lstm.batch_dim], input.shape[lstm.sequence_dim], lstm.hidden_size], dtype=(np.int64))
            else:
                mo_shape = np.array([input.shape[lstm.sequence_dim], input.shape[lstm.batch_dim], lstm.hidden_size], dtype=(np.int64))
        ...
        permute_data = Transpose(graph, dict(name=(lstm.name + '/Transpose_mxnet/'))).create_node_with_data([
         new_data, permute_order])
        reshape = Reshape(graph, dict(name=(lstm.name + '/Reshape_mxnet/')))
        reshape_dim_data = Const(graph, {'name':lstm.name + '/Reshape_mxnet_dim',  'value':mxnet_shape}).create_node_with_data()
