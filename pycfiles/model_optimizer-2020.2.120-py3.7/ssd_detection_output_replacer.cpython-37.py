...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/mxnet/ssd_detection_output_replacer.py
...
class SsdPatternDetectionOutputReplacer(FrontReplacementSubgraph):
    ...
    def reshape_priorboxes(self, concat):
        for i, node in concat.in_nodes().items():
            reshape_node = create_op_node_with_second_input(concat.graph, Reshape, int64_array([0, 2, -1]), dict(name=(concat.name + str(i) + '/PriorBoxReshape_')))
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        ssd_concats = {}
        concat_names = [
         'ssd_concat1', 'ssd_concat0', 'ssd_concat2']
        ...
        detection_output_node = DetectionOutput(graph, dict(name=(graph.unique_id() + '/DetectionOutput_'), top_k=top_k,
          keep_top_k=top_k,
        ...
        reshape_node = create_op_node_with_second_input(graph, Reshape, int64_array([0, -1]), dict(name=(graph.unique_id() + '/DetectionOutput_')))
