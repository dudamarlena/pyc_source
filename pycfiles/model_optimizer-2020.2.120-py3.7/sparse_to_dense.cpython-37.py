...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/ops/sparse_to_dense.py
...
class SparseToDense(Op):
    ...
    def infer(node: Node):
        if not len(node.in_nodes()) == 4:
            assert len(node.in_nodes()) == 3, 'Incorrect number of inputs for {} node'.format(node.id)
        else:
            dense_shape = node.in_port(1).data.get_value()
            assert dense_shape is not None, 'SparseToDense is supported only with constant shape value'
            ...
            if input_indices is not None:
                if input_values is not None and default_value is not None:
