...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/kaldi/replace_lstm_nonlinearity.py
...
class ReplaceLstmNonLinearityPattern(FrontReplacementOp):
    ...
    def replace_op(self, graph: Graph, node: Node):
        ...
        split_node = Split(graph, {'name':graph.unique_id(prefix='Split_lstm_input_'),  'num_splits':5}).create_node()
        ...
        prod_i_c_tanh = Eltwise(graph, {'name':graph.unique_id(prefix='prod_i_c_tanh_'),  'operation':'mul'}).create_node()
        i_sigmoid.out_port(0).connect(prod_i_c_tanh.in_port(0))
        c_tanh.out_port(0).connect(prod_i_c_tanh.in_port(1))
        prod_f_ct_1 = Eltwise(graph, {'name':graph.unique_id(prefix='prod_f_ct_1_'),  'operation':'mul'}).create_node()
        ...
        prod_o_c_t_tanh = Eltwise(graph, {'name':graph.unique_id(prefix='prod_o_c_t_tanh_'),  'operation':'mul'}).create_node()
