...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/tf/transposed_mvn_unrolled.py
# Compiled at: 2020-05-01 08:37:20
# Size of source mod 2**32: 9445 bytes
"""
...
import extensions.front.PowerToEltwises as PowerToEltwises
...
def check_applicability(match: dict) -> bool:
    ...
    if not all(mean_reduction.value == variance_reduction.value):
        log.debug('Reduction indices for mean {} and variance {} do not match.'.format(mean_reduction.value, variance_reduction.value))
        return False
...
class TransposedMVNUnrolled(FrontReplacementSubgraph):
    ...
    def run_before(self):
        ...
        return [
         MVNReplacer, MVNUnrolled, Sub, Div, SquaredDifference, PowerToEltwises]
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        c = c1 * c2
        new_reshape = create_op_node_with_second_input(graph, Reshape, int64_array([0, 0, 0, c1, c2]), dict(name=(div_name + '/first_reshape/MVN_T_')))
        permute_order = int64_array([0, 1, 2, 4, 3])
        first_permute = create_op_node_with_second_input(graph, Transpose, permute_order, dict(name=(div_name + '/first_permute/MVN_T_')), new_reshape)
        add = match['add']
        variance = match['variance']
        eps_port_num = 0 if add.in_port(0).get_connection().get_source().node.id != variance.id else 1
        eps = add.in_port(eps_port_num).get_connection().get_source().node
        mvn_node = MVN(graph, dict(name=(div_name + '/MVN/MVN_T_'), required_reduction_indices=[1, 2, 3], eps=(eps.value))).create_node()
        first_permute.out_port(0).connect(mvn_node.in_port(0))
        second_permute = create_op_node_with_second_input(graph, Transpose, permute_order, dict(name=(div_name + '/second_permute/MVN_T_')), mvn_node)
        ...
        gamma_val = np.reshape(match['gamma_identity'].in_port(0).get_connection().get_source().node.value, int64_array([1, 1, 1, c]))
        new_mul = create_op_node_with_second_input(graph, Mul, gamma_val, dict(name=(match['mul'].name + '/MVN_T_')), new_reshape2)
        beta_val = np.reshape(match['beta_identity'].in_port(0).get_connection().get_source().node.value, int64_array([1, 1, 1, c]))
        new_add2 = create_op_node_with_second_input(graph, Add, beta_val, dict(name=(match['add2'].name + '/MVN_T_')), new_mul)
