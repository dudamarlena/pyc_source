...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/ReplaceMemoryOffsetWithSplice.py
# Compiled at: 2020-05-01 08:37:21
# Size of source mod 2**32: 8222 bytes
"""
...
class ReplaceMemoryOffsetNodePattern(MiddleReplacementPattern):
    ...
    def pattern():
        return dict(nodes=[
         (
          'op', dict(op='MemoryOffset', has_default=False))],
          edges=[])
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        node_name = node.name
        node_t = node.t
        splice = Splice(graph, {'name':node_name,  'id':node_id, 
        ...
        for in_port in outs:
            ...
            if out_['op'] != 'MemoryOffset' and out_['op'] != 'Splice':
                crop_input = Crop(graph, {'name':'Splice_Crop',  'axis':int64_array([1]), 
...
class ReplaceMemoryOffsetWithMemoryNodePattern(MiddleReplacementPattern):
    ...
    def pattern():
        return dict(nodes=[
         (
          'op', dict(op='MemoryOffset', has_default=True))],
          edges=[])
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        if node.t >= 0:
            ...
            node_t = abs(node.t)
            memory_out = Memory(graph, {'name':pair_name,  'id':node_name + pair_name,  'index':1, 
             'size':2,  'shape':np.array([in_shape[1] * node_t])}).create_node()
            if node_t > 1:
                crop_concat = Crop(graph, {'name':'Memory_crop',  'dim':np.array([in_shape[1] * (node_t - 1)]),  'offset':np.array([in_shape[1]]), 
                ...
                memory_in = Memory(graph, {'name':node_name,  'id':node_name + pair_name,  'index':0, 
                 'size':2,  'shape':memory_out.shape}).create_node()
                ...
                crop_out = Crop(graph, {'name':'Memory_crop_out',  'dim':np.array([in_shape[1]]),  'offset':np.array([0]), 
                 'axis':np.array([1])}).create_node()
