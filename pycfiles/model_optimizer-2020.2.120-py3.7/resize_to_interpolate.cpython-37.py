...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/onnx/resize_to_interpolate.py
...
class ResizeToInterpolate2D(FrontReplacementSubgraph):
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        if match['mul_1'].in_node(1).value != match['mul_2'].in_node(1).value:
            ...
            return
        interpolate_node = Interpolate(graph, {'name':resize_node.name + '/Interpolate',  'mode':resize_node.mode, 
         'axes':int64_array([2, 3])}).create_node()
...
class ResizeToInterpolate3D(FrontReplacementSubgraph):
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        resize_node = match['resize']
        if match['mul_1'].in_node(1).value != match['mul_2'].in_node(1).value or match['mul_1'].in_node(1).value != match['mul_3'].in_node(1).value:
            log.info('Pattern matched around resize op {} has different scale values.'.format(resize_node.name))
            return
        interpolate_node = Interpolate(graph, {'name':resize_node.name + '/Interpolate',  'mode':resize_node.mode, 
