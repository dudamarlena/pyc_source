...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/EltwiseInputReshape.py
            ...
            if 4 <= len(out_shape) <= 5:
                ...
                for port, node in eltwise_op_node.in_nodes().items():
                    if len(node.shape) != len(out_shape) and len(node.shape) == 1 and out_features == node.shape[0]:
                        new_shape = shape_for_layout(layout, batch=1, features=out_features, height=1, width=1, depth=(1 if len(out_shape) == 5 else None))
