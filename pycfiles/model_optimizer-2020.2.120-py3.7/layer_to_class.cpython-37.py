...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/utils/ir_reader/layer_to_class.py
...
"""
import logging as log, os, numpy as np
...
def collect_ops(path: str):
    ...
    update_registration(classes=[Op, Activation, Elementwise, LogicalElementwise, ReduceOp], enabled_transforms=[], disabled_transforms=[])
...
def propagate_const_values(op: Node):
    ...
    :return:
    """
    assert op.soft_get('type') == 'Const', 'Wrong operation type, {} instead of Const!'.format(op.soft_get('type'))
...
def groupconv_to_conv(op: Node):
    ...
    :return:
    """
    assert op.soft_get('type') == 'GroupConvolution', 'Wrong operation type, {} instead of GroupConvolution!'.format(op.soft_get('type'))
    ...
    if weights_node.type == 'Const':
        ...
        if weights_node.type == 'Reshape':
            assert weights_node.in_port(0).get_source().data.get_shape() == new_shape, 'Weight shape and calculated shape mismatch in GroupConv node {}.'.format(op.name)
            op.in_port(1).disconnect()
            weights_node.in_port(0).get_source().get_connection().set_destination(op.in_port(1))
        else:
            assert op.in_port(1).get_source().data.get_shape() == new_shape, 'Weight shape and calculated shape mismatch in GroupConv node {}.'.format(op.name)
            op['group'] = group
            op.type = 'Convolution'
...
def backprop_to_deconv(op: Node):
    ...
    :return:
    """
    assert op.soft_get('type') in ('ConvolutionBackpropData', 'GroupConvolutionBackpropData'), 'Wrong operation type, {} instead of ConvolutionBackpropData/GroupConvolutionBackpropData!'.format(op.soft_get('type'))
...
def ti_add_edge_attrs(op: Node):
    ...
    :return:
    """
    assert op.soft_get('type') == 'TensorIterator', 'Wrong operation type, {} instead of TensorIterator!'.format(op.soft_get('type'))
    i = 0
...
preprocessing_op_nodes = {'Const':propagate_const_values, 
...
postprocessing_op_nodes = {'TensorIterator': ti_add_edge_attrs}
