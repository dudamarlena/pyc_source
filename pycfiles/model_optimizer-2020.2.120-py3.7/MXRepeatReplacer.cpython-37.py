...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/mxnet/MXRepeatReplacer.py
...
from mo.utils.shape import get_canonical_axis_index_node, new_shape_node_from_shape_nodes, get_shape_values_by_range_idxs
...
class MXRepeatReplacer(FrontReplacementPattern):
    ...
    def mxrepeat_decomposition(node: Node):
        ...
        unsqueeze_axis = create_op_node_with_second_input(graph,
          Add, (int64_array([1])), {'name': name + '/Unsqueeze/Axis'}, input_node=axis)
        ...
        first_input_shape_part = get_shape_values_by_range_idxs(input_shape,
          input_rank, begin=0, end=(node.axis), include_begin=True, include_end=False)
        original_axis_dim = create_op_with_const_inputs(graph,
          Gather, {2: int64_array(0)}, {'name': name + '/OriginalDim'}, input_node=input_shape)
        ...
        second_input_shape_part = get_shape_values_by_range_idxs(input_shape,
          input_rank, begin=(node.axis), end=(-1), include_begin=False, include_end=True)
