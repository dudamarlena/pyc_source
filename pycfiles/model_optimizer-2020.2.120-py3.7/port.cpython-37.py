...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/graph/port.py
# Compiled at: 2020-05-01 08:37:22
# Size of source mod 2**32: 17729 bytes
"""
...
class Port:
    ...
    def __eq__(self, other):
        return self.__class__ == other.__class__ and self.node.graph == other.node.graph and self.node.id == other.node.id and self.type == other.type and self.idx == other.idx
    ...
    def __setattr__(self, key, value):
        edge = self.node.in_edge((self.idx), control_flow=(self.control_flow)) if self.type == 'in' else self.node.out_edge((self.idx), control_flow=(self.control_flow))
        edge[key] = value

    def __getattr__(self, item):
        edge = self.node.in_edge((self.idx), control_flow=(self.control_flow)) if self.type == 'in' else self.node.out_edge((self.idx), control_flow=(self.control_flow))
        if edge.get(item) is None:
            raise Error('Edge from {}_port {} at node {} has no attribute {}'.format(self.type, self.idx, self.node.name, item))
        return edge[item]
    ...
    def _set_shape(self, shape):
        if self.node.graph.stage == 'front':
            ...
            if self.type == 'in':
                ...
                if not data_node.value is None:
                    assert np.array_equal(data_node.shape, int64_array(shape))
    ...
    def _get_value(self):
        if self.node.graph.stage == 'front':
            return
            if self.type == 'in':
                if self.idx in self.node.in_nodes(control_flow=(self.control_flow)) and self.node.in_node((self.idx), control_flow=(self.control_flow)).has_valid('value'):
                    return self.node.in_node((self.idx), control_flow=(self.control_flow)).value
        elif self.idx in self.node.out_nodes(control_flow=(self.control_flow)):
            if self.node.out_node((self.idx), control_flow=(self.control_flow)).has_valid('value'):
    ...
    def _get_attr(self, item: str):
        if self.node.graph.stage == 'front':
            return
            if self.type == 'in':
                if self.idx in self.node.in_nodes(control_flow=(self.control_flow)) and self.node.in_node((self.idx), control_flow=(self.control_flow)).has_valid(item):
                    return self.node.in_node((self.idx), control_flow=(self.control_flow))[item]
        elif self.idx in self.node.out_nodes(control_flow=(self.control_flow)):
            if self.node.out_node((self.idx), control_flow=(self.control_flow)).has_valid(item):
    ...
    def get_source(self):
        ...
        if self.node.graph.stage == 'front':
            ...
            elif len(producer_ports) != 1:
                if self.node.graph.strict_mode:
                    raise Error('Something bad has happened with graph! Data node "{}" has {} producers'.format(self.node.id, len(producer_ports)))
                else:
                    return
    ...
    def get_destination(self):
        ...
        if len(consumer_ports) > 1:
            raise Error('The number of destinations for {} node at {} port is {}'.format(self.node.name, self.idx, len(consumer_ports)))
    ...
    def get_destinations(self):
        ...
        for n, d in producer_node.get_outputs(edge_attr=({'out': self.idx} if self.node.graph.stage == 'front' else None), control_flow=(self.control_flow)):
            node = Node(self.node.graph, n)
    ...
    def _get_data_type(self):
        ...
        node = self.node
        if self.type == 'out':
            if node.has_valid('_out_port_data_type'):
                if self.idx in node._out_port_data_type:
            ...
            if value_data_type is not None:
                value_data_type = value.dtype if value is not None else None
                log.debug('The precision of the output port {} of node {} is determined from the data node as {}'.format(self.idx, self.node.name, value_data_type))
                return value_data_type
            return
        value = self.data.get_value()
        value_data_type = value.dtype if value is not None else None
        if value_data_type is not None:
            log.debug('The precision of the input port {} of node {} is determined from the data node as {}'.format(self.idx, self.node.name, value_data_type))
        ...
        if value_data_type is not None:
            if source_port_data_type is not None:
                if value_data_type != source_port_data_type:
                    log.warning('Inconsistent data type of the data node and port attribute for port {} of node {}: {} vs {}. Return data type of the data node.'.format(self.idx, self.node.name, value_data_type, source_port_data_type))
        if source_port_data_type is not None:
            return source_port_data_type
        return value_data_type

    def get_data_type(self):
        data_type = self._get_data_type()
        if data_type is None:
            raise Error('The data type for {} port {} of node {} is not defined'.format(self.type, self.idx, self.node.name))
        return data_type
    ...
    def set_data_type(self, data_type, override=False):
        if not self.type == 'out':
            raise AssertionError('The method can be called for output ports only')
        else:
            node = self.node
            if not node.has_valid('_out_port_data_type'):
                node['_out_port_data_type'] = {}
            if self.idx in node._out_port_data_type:
                if data_type != node._out_port_data_type[self.idx] and not override:
                    raise Error('Trying to override data type for output port {} of operation {}: from {} to {}'.format(self.idx, node.name, node._out_port_data_type[self.idx], data_type))
