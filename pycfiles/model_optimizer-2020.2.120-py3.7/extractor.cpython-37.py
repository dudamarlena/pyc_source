...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/front/extractor.py
# Compiled at: 2020-05-01 08:37:21
# Size of source mod 2**32: 66170 bytes
"""
...
def restore_edges(graph: Graph, get_edges: callable):
    ...
    for node in list(graph.nodes()):
        ...
        for u, v, d in edges:
            ...
            if len(undefined):
                raise Error('While creating an edge from "{}" to "{}": node name {} is undefined in the graph. Check correctness of the input model. ', u, v, ' and '.join(undefined) + refer_to_faq_msg(25))
...
def update_attrs(attrs: [
 dict, Node], attr: str, new: [str, list]):
...
def spatial_attr_getter(node: Node, field: str=None, dim: int=None, post: callable=None):
    ...
    """
    if node.has(field):
        if type(node[field]) is np.ndarray:
            if node.has('spatial_dims'):
...
def spatial_getter(name: str, field: str, dim: int, post: callable=lambda x: x):
    ...
    """
    return (
     name, lambda node: spatial_attr_getter(node, field=field, dim=dim, post=post))
...
def attr_getter(node: Node, name: str):
    if node.has(name):
        ...
        if type(node[name]) is not np.ndarray:
            return str(node[name])


def kernel_getter(node: Node, dim: int):
    if node.kind == 'op':
        if node.op in ('Conv2D', 'DepthwiseConv2dNative', 'Deconv2D'):
            if node.has('kernel_spatial'):
                return node.kernel_spatial[dim]
             ...
             [
              (
               ...
               [
                ...
                (
                 'type',
                 lambda node:                  if node.has('filler_type'):
node['filler_type'] # Avoid dead code: None),
                (
                 'value',
                 lambda node:                  if node.has('filler_value'):
node['filler_value'] # Avoid dead code: None),
                (
                 'output',
                 lambda node:                  if node.has('output_shape'):
                     if node.has('channel_dims'):
node.output_shape[node.channel_dims][0] # Avoid dead code: None),
                (
                 'input_nodes_names',
                 lambda node:                  if node.has('input_nodes_names'):
' '.join(node['input_nodes_names']) # Avoid dead code: None),
                (
                 'output_tensors_names',
                 lambda node:                  if node.has('output_tensors_names'):
' '.join(node['output_tensors_names']) # Avoid dead code: None), (
                 'real_input_dims',
                 lambda node:                  if node.has('real_input_dims'):
';'.join([' '.join(map(str, shape)) for shape in node['real_input_dims']]) # Avoid dead code: None),
                (
                 'protobuf',
                 lambda node:                  if node.has('pbs'):
node_defs_to_str(node) # Avoid dead code: None),
                {'custom_attributes': None},
                (
                 'strides',
                 lambda node:                  if node.has_valid('stride'):
','.join(map(str, node['stride'][node.spatial_dims])) # Avoid dead code: None),
                (
                 'kernel',
                 lambda node:                  if node.has_valid('kernel_spatial'):
','.join(map(str, node['kernel_spatial'])) # Avoid dead code: None),
                (
                 'dilations',
...
','.join(map(str, get_backend_pad(node.pad, node.spatial_dims, 0))) # Avoid dead code: None),
                (
                 'pads_end',
                 lambda node:                  if node.has_valid('pad'):
','.join(map(str, get_backend_pad(node.pad, node.spatial_dims, 1))) # Avoid dead code: None), ('scale', lambda node: attr_getter(node, 'scale')),
                'crop_width',
                'crop_height',
                'write_augmented',
                'max_multiplier',
...
node_defs_to_str(node) # Avoid dead code: None),
                ...
                (
                 'kernel-x', lambda node: kernel_getter(node, 1)),
                (
                 'kernel-y', lambda node: kernel_getter(node, 0)),
                spatial_getter('dilation-x', 'dilation', 1),
                spatial_getter('dilation-y', 'dilation', 0),
              ...
              '@ports',
              '@consts'])]}
    ir_version_mapping = {None:ir_v4_attrs, 
     10:ir_v4_attrs, 
     7:ir_v4_attrs, 
     6:ir_v3_attrs, 
     5:ir_v3_attrs, 
...
def create_tensor_nodes(graph: Graph):
    ...
    for node in list(graph.nodes()):
        ...
        graph.add_nodes_from([(uid, add_attrs_props(dict(name=uid, kind='data', shape=None, value=None, data_type=None, infer=None))) for port, uid in out_tensor_dict.items()])
        added_out_ports = set()
            ...
            if attrs['op'] == 'Result':
                ...
                try:
                    ...
                    try:
                        ...
                        raise Error('Unexpected exception happened during extracting attributes for node {}.\nOriginal exception message: {}', new_attrs['name'] if 'name' in new_attrs else '<UNKNOWN>', str(e)) from e
                    finally:
                        e = None
                        del e
                ...
                supported or unsupported.add(Node(graph, node))
...
def extract_port_from_string(node_name: str):
    ...
    if len(parts) > 2:
        raise Error('Please provide only one port number for {}. Expected format is NODE:OUT_PORT or IN_PORT:NODE, where IN_PORT and OUTPUT_PORT are integers'.format(node_name))
    if len(parts) == 1:
        return (
         node_name, None, None)
    ...
    try:
        ...
        try:
            ...
            raise Error('Non integer port number in {}. Expected format is NODE:OUT_PORT or IN_PORT:NODE, where IN_PORT and OUTPUT_PORT are integers'.format(node_name))
...
def input_user_data_repack(graph: Graph, input_user_shapes: [None, list, dict, np.ndarray], freeze_placeholder: dict, input_user_data_types=dict()):
    """
    ...
    if freeze_placeholder is None:
        ...
        for placeholder_name, value in freeze_placeholder.items():
            ...
            if port is None and placeholder_id in placeholders_ids:
                ...
                _freeze_new_placeholder[placeholder_id].append({'direction':direction, 
                 'port':port,  'name':placeholder_name,  'id':new_placeholder_id,  'value':value})
    ...
    if input_user_shapes is None:
        ...
        if isinstance(input_user_shapes, list) or isinstance(input_user_shapes, dict):
        ...
        elif not isinstance(input_user_shapes, np.ndarray):
            raise AssertionError
        elif len(placeholders_ids) == 1:
            ...
            if _freeze_placeholder is not None:
                ...
                if len(placeholders_ids) != 1:
                    raise Error("Original placeholders: '{}'. Freezing was requested for '{}'. --input_shape was provided without --input. Can not deduce which node shape to override".format(', '.join(original_phs), ', '.join(_freeze_placeholder.keys())))
                _input_shapes[placeholders_ids[0]].append({'shape':input_user_shapes,  'port':None})
                [_input_shapes[node_id].append({'shape':None,  'port':None}) for node_id in _freeze_placeholder]
            else:
                raise Error('No or multiple placeholders in the model, but only one shape is provided, cannot set it. ' + refer_to_faq_msg(32))
    for node_id in _freeze_new_placeholder:
        new_phs = _freeze_new_placeholder[node_id]
        if node_id not in _input_shapes:
            raise Error('Shape is not specified for the placeholder with name {} through --input_shape option.'.format(new_phs[0]['name']))
        _ins = _input_shapes[node_id]
        for new_ph in new_phs:
            ...
            if any([_in['shape'] is not None and direction in _in and _in[direction] == port for _in in _ins]):
                ...
                raise Error('Shape is not specified for the placeholder with name {} through --input_shape option.'.format(name))

    return (
     _input_shapes, _freeze_placeholder)
...
def user_data_repack(graph: Graph, input_user_shapes: [None, list, dict, np.array], input_user_data_types: dict, outputs: list, freeze_placeholder: dict):
    """
    :param graph: graph to operate on
    ...
    """
    _input_shapes, _freeze_placeholder = input_user_data_repack(graph, input_user_shapes, freeze_placeholder, input_user_data_types=input_user_data_types)
...
def add_output_ops(graph: Graph, user_defined_outputs: dict, inputs: dict=None):
    sinks = []
    if isinstance(user_defined_outputs, list):
        assert len(user_defined_outputs)
    else:
        graph.remove_nodes_from([node_name for node_name in graph.nodes() if 'op' in graph.node[node_name] if graph.node[node_name]['op'] == 'Result'])
        if user_defined_outputs is None:
            ...
            if len(dead_outputs):
                log.info("Possible outputs: '{!s}' are not input reachable. True outputs are {!s}".format(', '.join([str(d_o) for d_o in dead_outputs]), ', '.join([str(u_o) for u_o in undead_outputs])))
        else:
            for node, values in user_defined_outputs.items():
                if node not in graph.nodes():
                    raise Error('Node "{}" does not exist in the graph. ' + refer_to_faq_msg(26), node)
                for value in values:
                    if 'in' in value:
                        in_edges = list(graph.in_edges(node, data=True))
                        if len(in_edges) - 1 < value['in']:
                            raise Error('Port index {} is out of number of available input ports for node "{}". ' + refer_to_faq_msg(29), value['in'], node)
                        ...
                        if 'out' in value:
                            ...
                            if len(out_edges) - 1 < value['out']:
                                raise Error('Port index {} is out of number of available output ports for node "{}". ' + refer_to_faq_msg(29), value['out'], node)
...
def check_input(graph: Graph, node_name: str):
    ...
    if node['kind'] == 'op':
        if node['op'] == 'Parameter':
            if not len(graph.in_edges(node_name)):
                if not node['is_input']:
                    raise Error('--input parameter was provided. Other inputs are needed for output computation. Provide more inputs or choose another place to cut the net. ' + refer_to_faq_msg(27))
        ...
        if len(parts) > 1:
            if parts[0].isdigit():
                ...
                try:
                    ...
                    try:
                        log.warning('Didn\'t recognize port:node format for "{}" because port is not an integer.'.format(node_id))
                    finally:
                        err = None
                        del err
                ...
                try:
                    ...
                    try:
                        log.warning('Didn\'t recognize node:port format for "{}" because port is not an integer.'.format(node_id))
                    finally:
                        err = None
                        del err
...
def add_input_op_output_port_with_data(graph: Graph, node_id: str, input_op, port: int):
    ...
    if not (data_node.has_valid('kind') and data_node.kind == 'data'):
        raise AssertionError
...
def add_input_op(graph: Graph, node_id: str, port: int=0, data: bool=False, shape=None, data_type=None, is_out_port: bool=False):
    """
    ...
    if data_type is None:
        ...
        input_op = Parameter(graph, dict(shape=shape, data_type=data_type, initial_node_name=node_id, name=(get_new_placeholder_name(node_id, is_out_port, port))))
        edge_attrs = {'in':port, 
         'out':0,  'in_attrs':['in'],  'out_attrs':['out'],  'fw_tensor_debug_info':[
          (
           Node(graph, node_id).soft_get('name'), port)], 
         'data_attrs':[
          'fw_tensor_debug_info']}
        if not data:
            if is_out_port:
                new_input_id = add_input_op_output_port_without_data(graph=graph, node_id=node_id, input_op=input_op, port=port)
            else:
                new_input_id = add_input_op_input_port_without_data(graph=graph, node_id=node_id, input_op=input_op, edge_attrs=edge_attrs)
        elif is_out_port:
            new_input_id = add_input_op_output_port_with_data(graph=graph, node_id=node_id, input_op=input_op, port=port)
        else:
            new_input_id = add_input_op_input_port_with_data(graph=graph, node_id=node_id, input_op=input_op, edge_attrs=edge_attrs)
    return new_input_id


def add_input_ops_helper_before_infer_input_port(graph: Graph, smart_node: Node, port: int, node_id: str, shape: np.array, data_type, inputs: list, edges_to_remove: list):
    n_inputs = len(smart_node.in_nodes())
    if n_inputs > 1:
        if port is None:
            raise Error('Node {} has more than 1 input and input shapes were provided. Try not to provide input shapes or specify input port with port:node notation, where port is an integer. {}'.format(smart_node.soft_get('name'), refer_to_faq_msg(30)))
...
def add_input_ops_helper_after_infer_input_port(graph: Graph, smart_node: Node, port: int, node_id: str, inputs: list, edges_to_remove: list):
    n_inputs = len(smart_node.in_nodes())
    if n_inputs > 1:
        if port is not None:
            if port != 0:
                raise Error('Input port > 0 in --input is not supported if --input_shape is not provided. Node: "{}". Omit port index and all input ports will be replaced by placeholders. Or provide --input_shape. ' + refer_to_faq_msg(31), node_id)
    port = port if port is not None else 0
    in_node = smart_node.in_node(port)
    shape = in_node['shape'] if 'shape' in in_node else None
    if shape is None:
        raise Error('Shape for tensor "{}" is not defined. Can not proceed.' + refer_to_faq_msg(41), in_node.soft_get('name'))
    inputs.append(add_input_op(graph=graph, node_id=node_id, port=port, data=True, shape=(shape.copy()),
      data_type=(in_node.soft_get('data_type', None))))
    edges_to_remove.append((in_node.id, node_id))


def add_input_ops_helper_before_infer_output_port(graph: Graph, port: int, node_id: str, shape: np.array, data_type, inputs: list, edges_to_remove: list):
...
def add_input_ops_helper_after_infer_output_port(graph: Graph, smart_node: Node, port: int, node_id: str, inputs: list, edges_to_remove: list):
    out_node = smart_node.out_node(port)
    shape = out_node['shape'] if 'shape' in out_node else None
    if shape is None:
        raise Error('Shape for tensor "{}" is not defined. Can not proceed.' + refer_to_faq_msg(41), out_node.soft_get('name'))
    inputs.append(add_input_op(graph=graph, node_id=node_id, port=port, data=True, shape=(shape.copy()),
      data_type=(out_node.soft_get('data_type', None)),
      is_out_port=True))
    ...
    if user_defined_inputs is None:
        ...
        for node_id in user_defined_inputs:
            for port_and_shape_info in user_defined_inputs[node_id]:
                if 'added' in port_and_shape_info:
                    if port_and_shape_info['added']:
                        continue
                is_out_port = 'out' in port_and_shape_info
                ...
                if is_out_port:
                    port = port_and_shape_info['out']
                    if port is None:
                        raise Error('Output port for input node {} should be specified, it cannot be None!'.format(node_id))
                    if port is not None:
                        if port not in smart_node.out_nodes():
                            raise Error('Output port index {} is out of number of available output ports for node "{}". ' + refer_to_faq_msg(29), port, node_id)
                else:
                    port = port_and_shape_info['in'] if 'in' in port_and_shape_info else None
                if port is not None:
                    if port not in smart_node.in_nodes():
                        raise Error('Input port index {} is out of number of available input ports for node "{}". ' + refer_to_faq_msg(29), port, node_id)
                if smart_node.op == 'Parameter':
                    if port is not None:
                        raise Error('Parameter node "{}" doesn\'t have input port, but input port {} was provided. ' + refer_to_faq_msg(28), node_id, port)
                    ...
                    if shape is None:
                        ...
                        add_input_ops_helper_before_infer_output_port(graph, port, node_id, shape, data_type, inputs, edges_to_remove)
                    else:
                        add_input_ops_helper_before_infer_input_port(graph, smart_node, port, node_id, shape, data_type, inputs, edges_to_remove)
                else:
                    if is_out_port:
                        add_input_ops_helper_after_infer_output_port(graph, smart_node, port, node_id, inputs, edges_to_remove)
                    else:
                        add_input_ops_helper_after_infer_input_port(graph, smart_node, port, node_id, inputs, edges_to_remove)
                port_and_shape_info['added'] = True
