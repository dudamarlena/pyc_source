...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/TensorIteratorLSTMToLSTMSequence.py
...
class TensorIteratorLSTM(MiddleReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        if len(list(isomorphisms)) != 1:
            raise Error('Unsupported TensorIterator layer {} was found: either its body, ports or edges are not supported by Inference Engine. Only TensorIterator with LSTMCell in a body of strict form is supported. Please modify the original network to meet the requirements.'.format(ti.soft_get('name')))
        body_match = isomorphisms[0]
        if body_match['input_hidden'].has_valid('value') or body_match['input_cell'].has_valid('value'):
            raise Error('Unsupported TensorIterator layer {} was found: initial hidden and/or cell states for LSTMCell are constants. This is not supported. Only TensorIterator with LSTMCell in a body of strict form is supported. Please modify the original network to meet the requirements.'.format(ti.soft_get('name')))