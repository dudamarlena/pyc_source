...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/main.py
# Compiled at: 2020-05-05 09:03:41
# Size of source mod 2**32: 15324 bytes
"""
...
"""
import argparse, datetime, logging as log, os, sys, traceback
from collections import OrderedDict
import numpy as np
from mo.pipeline.unified import unified_pipeline
from extensions.back.SpecialNodesFinalization import RemoveConstOps, CreateConstNodesReplacement, RemoveOutputOps, NormalizeTI
...
from mo.utils.cli_parser import get_placeholder_shapes, get_tuple_values, get_model_name, get_common_cli_options, get_caffe_cli_options, get_tf_cli_options, get_mxnet_cli_options, get_kaldi_cli_options, get_onnx_cli_options, get_mean_scale_dictionary, parse_tuple_pairs, get_freeze_placeholder_values, append_exp_keys_to_namespace, get_meta_info
...
def print_argv(argv: argparse.Namespace, is_caffe: bool, is_tf: bool, is_mxnet: bool, is_kaldi: bool, is_onnx: bool, model_name: str):
    ...
    for key in props:
        ...
        for op, desc in props[key].items():
            if isinstance(desc, list):
                ...
                if op is 'k':
                    default_path = os.path.join(os.path.dirname(sys.argv[0]), 'extensions/front/caffe/CustomLayersMapping.xml')
...
def prepare_ir(argv: argparse.Namespace):
    ...
    if not any([is_tf, is_caffe, is_mxnet, is_kaldi, is_onnx]):
        raise Error('Framework {} is not a valid target. Please use --framework with one from the list: caffe, tf, mxnet, kaldi, onnx. ' + refer_to_faq_msg(15), argv.framework)
    if is_tf and not argv.input_model or argv.saved_model_dir:
        if not argv.input_meta_graph:
            raise Error('Path to input model or saved model dir is required: use --input_model, --saved_model_dir or --input_meta_graph')
        else:
            if is_mxnet and not argv.input_model or argv.input_symbol:
                if not argv.pretrained_model_name:
                    raise Error('Path to input model or input symbol or pretrained_model_name is required: use --input_model or --input_symbol or --pretrained_model_name')
                else:
                    if is_caffe and not argv.input_model:
                        if not argv.input_proto:
                            raise Error('Path to input model or input proto is required: use --input_model or --input_proto')
                        else:
                            if is_kaldi or is_onnx:
    ...
    if argv.model_name:
        ...
        if argv.input_model:
            ...
            if is_tf and argv.saved_model_dir:
                model_name = 'saved_model'
            else:
                if is_tf and argv.input_meta_graph:
                    model_name = get_model_name(argv.input_meta_graph)
                else:
                    if is_mxnet:
                        if argv.input_symbol:
                            ...
                            if is_caffe:
                                if not argv.input_proto:
                                    ...
                                    if not argv.input_proto:
                                        raise Error('Cannot find prototxt file: for Caffe please specify --input_proto - a protobuf file that stores topology and --input_model that stores pretrained weights. ' + refer_to_faq_msg(20))
                            ...
                            if is_caffe and argv.mean_file and argv.mean_values:
                                raise Error('Both --mean_file and mean_values are specified. Specify either mean file or mean values. ' + refer_to_faq_msg(17))
                            else:
                                if is_caffe:
                                    if argv.mean_file:
                                        if argv.mean_file_offsets:
                                            ...
                                            if not all([offset >= 0 for offset in mean_file_offsets]):
                                                raise Error("Negative value specified for --mean_file_offsets option. Please specify positive integer values in format '(x,y)'. " + refer_to_faq_msg(18))
                                            argv.mean_file_offsets = mean_file_offsets
                                if argv.scale and argv.scale_values:
                                    raise Error('Both --scale and --scale_values are defined. Specify either scale factor or scale values per input channels. ' + refer_to_faq_msg(19))
                        if argv.scale:
                            if argv.scale < 1.0:
                                log.error('The scale value is less than 1.0. This is most probably an issue because the scale value specifies floating point value which all input values will be *divided*.', extra={'is_warning': True})
                    else:
                        if argv.input_model:
                            if is_tf:
                                if argv.saved_model_dir:
                                    raise Error('Both --input_model and --saved_model_dir are defined. Specify either input model or saved model directory.')
                        if is_tf and argv.saved_model_tags is not None:
                    ...
                    argv.placeholder_shapes, argv.placeholder_data_types = get_placeholder_shapes(argv.input, argv.input_shape, argv.batch)
    ...
    if not os.path.exists(argv.output_dir):
        try:
            ...
            try:
                raise Error('Failed to create directory {}. Permission denied! ' + refer_to_faq_msg(22), argv.output_dir) from e
            finally:
                e = None
                del e
        ...
        if not os.access(argv.output_dir, os.W_OK):
            raise Error('Output directory {} is not writable for current user. ' + refer_to_faq_msg(22), argv.output_dir)
        ...
        if hasattr(argv, 'extensions') and argv.extensions and argv.extensions != '':
            ...
            extensions = None
        argv.freeze_placeholder_with_value, argv.input = get_freeze_placeholder_values(argv.input, argv.freeze_placeholder_with_value)
    if is_tf:
...
def emit_ir(graph: Graph, argv: argparse.Namespace):
    ...
    prepare_emit_ir(graph=graph, data_type=(graph.graph['cmd_params'].data_type),
