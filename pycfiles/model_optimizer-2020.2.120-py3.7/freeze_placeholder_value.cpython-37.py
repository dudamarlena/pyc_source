...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/freeze_placeholder_value.py
...
class FreezePlaceholderValue(FrontReplacementSubgraph):
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        if ph.name in graph.graph['freeze_placeholder']:
            ...
            new_node = Const(graph).create_node(attrs={'value':value, 
             'data_type':type(value),  'name':name + '/const_placeholder',  'shape':ph.shape})
            graph.erase_node(ph)
            graph.add_edges_from([(new_node.id, v, attrs) for u, v, attrs in out_edges])
            log.info('Placeholder node "{}" was replaced with Const node "{}" with value "{}"'.format(name, new_node.name, value))