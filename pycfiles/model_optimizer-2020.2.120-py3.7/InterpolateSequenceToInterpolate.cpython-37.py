...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/InterpolateSequenceToInterpolate.py
...
def replace_sequence(seq: List[Node], graph: Graph):
    ...
    interp_node = Interpolate(graph, dict(name=(fst_interp_node_name + '/Interpolate_'), axes=axes_of_node,
...
class InterpolateSequenceToInterpolate(MiddleReplacementPattern):
    ...
    def run_before(self):
        import extensions.middle.UpsampleToResample as UpsampleToResample
        return [
         UpsampleToResample]

    def find_and_replace_pattern(self, graph: Graph):
        log.debug('Enabled replacement of a sequence of Interpolate layers with one Interpolate layer.')
        interps = [n for n in graph.pseudo_topological_sort() if n.kind == 'op' if n.op == 'Interpolate']
