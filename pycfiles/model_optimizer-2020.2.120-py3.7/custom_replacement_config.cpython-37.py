...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/utils/custom_replacement_config.py
# Compiled at: 2020-05-01 08:37:22
# Size of source mod 2**32: 19990 bytes
"""
...
class CustomReplacementDescriptor(object):
    ...
    def __init__(self, replacement_id, attrs=None):
        """
        ...
        if attrs is not None:
            super(CustomReplacementDescriptor, self).__setattr__('custom_attributes', attrs.setdefault('custom_attributes', {}))
    ...
    def create_instance(cls, match_kind: str, replacement_id: str, attrs: dict=None):
        ...
        raise Error('No class registered for match kind "{}". Supported match kinds are "{}". '.format(match_kind, list(cls.registered_types.keys())) + refer_to_faq_msg(65))
    ...
    def get_config_file_representation(self):
        result = {'match_kind':self.match_kind, 
         'instances':self.instances,  'inputs':self.inputs, 
         'outputs':self.outputs,  'custom_attributes':self.custom_attributes, 
         'id':self.id}
        if self.has('op'):
...
class CustomReplacementDescriptorPoints(CustomReplacementDescriptor):
    ...
    def get_config_file_representation(self):
        result = {'match_kind':self.match_kind, 
         'instances':self.instances,  'custom_attributes':self.custom_attributes, 
         'id':self.id,  'include_inputs_to_sub_graph':bool(self.include_inputs_to_sub_graph), 
    ...
    def update_custom_replacement_attributes(self, graph: Graph):
        if not self.has('instances'):
            raise Error("No instance(s) is(are) defined for the custom replacement '{}'. ".format(self.replacement_id) + refer_to_faq_msg(66))
        else:
            if not isinstance(self.instances, dict):
                raise Error("The instance must be a single dictionary for the custom replacement with id '{}'. ".format(self.replacement_id) + refer_to_faq_msg(67))
            ...
            for src_node_name, dst_node_name, edge_attrs in graph.edges(data=True):
                ...
                if src_node_name not in matched_nodes:
                    if dst_node_name in matched_nodes:
            ...
            if not self.has('inputs'):
                self._replacement_desc['inputs'] = [[{'node':desc[0],  'port':desc[1]} for desc in inp] for inp in sorted(input_nodes_mapping.values())]
                log.debug('Updated inputs of sub-graph for instance "{}"'.format(self.instances))
            self._replacement_desc['outputs'] = self.has('outputs') or [{'node':node,  'port':port} for node, port in sorted(output_tensors)]
...
class CustomReplacementDescriptorScope(CustomReplacementDescriptor):
    ...
    def update_custom_replacement_attributes(self, graph: Graph):
        if not self.has('instances') or len(self.instances) == 0:
            raise Error("No instances are defined for replacement with id '{}'. ".format(self.replacement_id) + refer_to_faq_msg(68))
        else:
            pattern = self.instances[0]
...
def parse_custom_replacement_config_file(file_name: str):
    ...
    if not os.path.exists(file_name):
        raise Error("Custom replacements configuration file '{}' does not exist. ".format(file_name) + refer_to_faq_msg(69))
    try:
        with open(file_name, 'r') as (f):
            data = json.load(f)
    except Exception as exc:
        try:
            raise Error("Failed to parse custom replacements configuration file '{}': {}. ".format(file_name, exc) + refer_to_faq_msg(70)) from exc
        finally:
            exc = None
            del exc
    ...
    for attrs in data:
        if 'id' not in attrs:
            raise Error('One of the custom replacements in the configuration file "{}" does not contain attribute "id". '.format(file_name) + refer_to_faq_msg(71))
        if 'match_kind' not in attrs:
            raise Error('One of the custom replacements in the configuration file "{}" does not contain attribute "match_kind". Possible values are "points", "scope" and "general". '.format(file_name) + refer_to_faq_msg(71))
    ...
    if len(validation_errors) > 0:
        raise Error("File '{}' validation failed:\n{}. ".format(file_name, '\n'.join(validation_errors)) + refer_to_faq_msg(72))
    return result
...
def generate_pattern_for_node(graph: Graph, sub_graph_pattern: str, node_name: str):
    ...
    if len([node for node in sub_graph_nodes if match(sub_graph_pattern + name_suffix, node)]) == 1:
        return name_suffix
    raise RuntimeError('The pattern that uniquely identifies node "{}" using sub-graph pattern "{}" has not been found'.format(node_name, sub_graph_pattern))