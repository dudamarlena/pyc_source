...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/TensorIteratorOutput.py
...
class SmartOutputMatcher(MiddleReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        output = TensorIteratorOutput(graph, dict(axis=0, start=None, stride=None, part_size=None, external_port_id=(str(match['WriteEnter_data'].value)),
          internal_layer_id=(value.id),
          name=(match['TensorArrayWrite'].name + '/TensorIteratorOutput_')))
        output.create_node_with_data(inputs=[ta_size, value, index], data_nodes=[
         match['TensorArrayGather_data']])
        safe_nodes = [
         'TensorArrayGather_data', 'Condition_data']
        nodes_for_remove = []
...
class SimpleOutputMatcher(MiddleReplacementPattern):
    ...
    def run_before(self):
        ...
        return [
         TensorIteratorMerge, LoopConditionMatcher]
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        index = match['TensorArrayWrite'].in_node(1)
        value = match['TensorArrayWrite'].in_node(2)
        output = TensorIteratorOutput(graph, dict(external_port_id=(str(match['WriteEnter_data'].value)),
          internal_layer_id=(value.id),
          name=(match['TensorArrayWrite'].name + '/TensorIteratorOutput_')))
        output.create_node_with_data(inputs=[value, index], data_nodes=[
         match['TensorArrayRead_data']])
        safe_nodes = [
         'TensorArrayRead_data', 'Condition_data']
        nodes_for_remove = []
        for node in match.keys():
            if node not in safe_nodes:
                nodes_for_remove.append(match[node].id)
