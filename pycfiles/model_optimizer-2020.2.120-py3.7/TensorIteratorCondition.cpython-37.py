...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/TensorIteratorCondition.py
# Compiled at: 2020-05-01 08:37:21
# Size of source mod 2**32: 21283 bytes
"""
...
def make_nodes_1D(nodes: list):
    ...
    """
    for node in nodes:
        if not node.shape is None:
            assert len(node.shape) == 0
...
class LoopConditionMatcher(MiddleReplacementPattern):
    ...
    def check_dynamic_seq_len(graph: Graph, match: dict):
        ...
        shape.
        """
        dynamic_seq_len = match['Enter_1_less_data'].value is None or match['Enter_2_less_data'].value is None or not np.array_equal(match['Enter_1_less_data'].value, match['Enter_2_less_data'].value)
        return dynamic_seq_len

    def replace_pattern(self, graph: Graph, match: dict):
        log.debug('================== ConditionFind ===============')
        init_1 = match['init_1_data'].value
        if not init_1 is not None:
            ...
            condition_attrs = dict(time=dict(init=init_2, step=step_2), iter=dict(init=init_1, step=step_1), name=(match['loop_cond'].name + '/TensorIteratorCondition_'))
            condition = TensorIteratorCondition(graph, attrs=condition_attrs)
            condition_data = condition.create_node_with_data(inputs=[match['Strided_slice_data'], match['minimum_data']], data_nodes=[
             loop_condition, iterator_data])
            safe_nodes = [
             'loop_cond_data', 'Identity_1_data', 'Identity_2_data', 'Strided_slice', 'Strided_slice_data',
            ...
            if 'GreaterEqual' in identity_ops:
                ...
                if dynamic_seq_len:
                    ...
                    iterator_data_nodes = [
                     backedge_data, match['add_2_data'], match['add_2_y_data'], match['add_2_y'],
...
class SimpleConditionMatcher(MiddleReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        condition_attrs = dict(iter=dict(init=init_1, step=step_1), name=(match['loop_cond'].name + '/TensorIteratorCondition_'))
        condition = TensorIteratorCondition(graph, attrs=condition_attrs)
        condition.create_node_with_data(inputs=[match['Strided_slice_data']], data_nodes=[
         match['loop_cond_data'], match['Identity_1_data']])
        safe_nodes = [
         'loop_cond_data', 'Identity_1_data', 'Strided_slice', 'Strided_slice_data']
        nodes_for_remove = []
        for node in match.keys():
            if node not in safe_nodes:
                nodes_for_remove.append(match[node].id)
...
class DynamicDecoderConditionMatcher(MiddleReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        condition = TensorIteratorCondition(graph, attrs=condition_attrs)
        condition.create_node_with_data(inputs=[match['Less_enter'].in_node()], data_nodes=[
         loop_condiiton, iterator_data])
        safe_nodes = [
         'loop_cond_data', 'identity_data', 'TensorIteratorOutput', 'TensorIteratorOutput_1']
        nodes_for_remove = []
        for node in match.keys():
            if node not in safe_nodes:
                nodes_for_remove.append(match[node].id)
