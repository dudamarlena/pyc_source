...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/UpsampleToResample.py
    ...
    def replace_pattern(self, graph: Graph, match: Dict[(str, Node)]):
        ...
        if len(upsample.in_nodes()) == 2:
            if upsample.in_node(1).value is None:
                ...
                assert len(scales) in (4, 5), 'Supported scales rank is 4 or 5, but it is {} for node {}'.format(len(scales), upsample.soft_get('name', upsample.id))
                return math.isclose((scales[0]), 1, rel_tol=1e-05) and math.isclose((scales[1]), 1, rel_tol=1e-05) or None
        ...
        if not math.isclose(height_scale, width_scale, rel_tol=1e-05):
            log.debug('Width and height scales are not equal: {} vs {} for node {}'.format(width_scale, height_scale, upsample.soft_get('name')))
            return
        if depth_scale is not None:
            if not math.isclose(height_scale, depth_scale, rel_tol=1e-05):
                log.debug('Depth and height scales are not equal: {} vs {} for node {}'.format(depth_scale, height_scale, upsample.soft_get('name')))
                return
        if 1 in upsample.in_ports():
            if not upsample.in_port(1).disconnected():
            ...
            ss = StridedSlice(graph, {'name':upsample.name + '/ss_0_port',  'begin_mask':np.array([1]),  'end_mask':np.array([0]), 
             'new_axis_mask':np.array([0]),  'shrink_axis_mask':int64_array([0]), 
