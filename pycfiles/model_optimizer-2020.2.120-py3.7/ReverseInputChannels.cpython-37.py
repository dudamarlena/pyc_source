...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/back/ReverseInputChannels.py
...
class InsertReverseChannels(BackReplacementPattern):
    ...
    def find_and_replace_pattern(self, graph: Graph):
        all_params = [(p.soft_get('name', p.id), p, list(p.out_port(0).data.get_shape())) for p in graph.get_op_nodes(type='Parameter')]
        suitable_params = [(name, p, shape) for name, p, shape in all_params if len(shape) == 4 if shape[1] == 3]
...
class ReverseChannelsPropagationDown(BackReplacementPattern):
    ...
    enabled = False
    propagation_rules = {'Convolution':lambda node, rc: ReverseChannelsPropagationDown.pass_rc_through_conv(node, rc), 
    ...
    def pass_rc_through_conv(node, reverse_channels):
        ...
        if group != 1:
            if group == reverse_channels.order.size:
                ...
                new_order = np.take((np.arange(bottom_channels).reshape((group, multiplier))), indices=(reverse_channels.order),
                  axis=0).flatten()
        ...
        for idx, port in node.in_ports().items():
            if port.get_connection().get_source().node.id == reverse_channels.id:
                ...
                if len(non_one_dims) == 1 and shape[non_one_dims.item()] == reverse_channels.order.size:
                    ...
                    if np.array_equal(before_shape, shape):
...
class ReverseChannelsPropagationUp(BackReplacementPattern):
    ...
    enabled = False
    propagation_rules = {'ScaleShift':lambda node, rc: ReverseChannelsPropagationUp.lift_up_through_eltwise(node, rc), 
...
class DecomposeReverseChannels(BackReplacementPattern):
    ...
    def replace_with_split_concat(node):
        ...
        axis = node.axis
        order = node.order
        split = create_op_with_const_inputs(graph, Split, {1: int64_array(axis)}, {'name':name + '/Split', 
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for reverse_channels in graph.get_op_nodes(op='ReverseChannels'):
            if not reverse_channels.in_port(0).disconnected():
                if reverse_channels.out_port(0).disconnected():
...
class ApplyReverseChannels(BackReplacementPattern):
    ...
    def run_before(self):
        ...
        import extensions.back.GroupedConvWeightsNormalize as GroupedConvWeightsNormalize
        return [
         ParameterToInput, GroupedConvWeightsNormalize, SplitNormalizer]
