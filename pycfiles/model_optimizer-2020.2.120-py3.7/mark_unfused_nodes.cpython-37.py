...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/middle/passes/fusing/mark_unfused_nodes.py
...
def _check_lin_op(node: Node, layout: str):
    ...
    if node.soft_get('op') in lin_ops:
        ...
        if weights_id is None:
            ...
            log.info("[ FUSING ] Node {} wasn't marked as fusable (no weights, probably this is element-wise operation that is not fusable)".format(node.id))
            return
