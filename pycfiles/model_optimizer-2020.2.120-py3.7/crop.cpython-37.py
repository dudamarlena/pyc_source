...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/ops/crop.py
# Compiled at: 2020-05-01 08:37:22
# Size of source mod 2**32: 6856 bytes
"""
...
class Crop(Op):
    ...
    def backend_attrs(self):
        return [
         (
          'axis',
          lambda node:           if not node.has_valid('axis'):
None # Avoid dead code: ','.join(map(str, node.axis))),
         (
          'offset',
          lambda node:           if not node.has_valid('offset'):
None # Avoid dead code: ','.join(map(str, node.offset))),
         ('dim',
          lambda node:           if not node.has_valid('dim'):
None # Avoid dead code: ','.join(map(str, node.dim))),
         ('crop_begin',
          lambda node:           if not node.has_valid('crop_begin'):
None # Avoid dead code: ','.join(map(str, node.crop_begin))),
         (
          'crop_end',
          lambda node:           if not node.has_valid('crop_end'):
None # Avoid dead code: ','.join(map(str, node.crop_end)))]
    ...
    def _one_input_infer(node: Node):
        ...
        if node.has_valid('dim'):
            ...
            if node.has_valid('crop_begin'):
                if node.has_valid('crop_end') and not len(node.crop_begin) != len(node.axis):
                    if len(node.crop_end) != len(node.axis):
    ...
    def _two_inputs_infer(node: Node):
        ...
        if node.in_node(0).has_valid('value'):
            if not node.graph.graph['cmd_params'].enable_ssd_gluoncv:
