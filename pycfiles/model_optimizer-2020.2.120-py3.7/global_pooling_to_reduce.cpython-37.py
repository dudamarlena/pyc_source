...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/global_pooling_to_reduce.py
...
class GlobalPoolingToReduce(FrontReplacementPattern):
    ...
    pool_method_to_reduce_type = {'max':ReduceMax, 
     'avg':ReduceMean}
    ...
    def find_and_replace_pattern(self, graph: Graph):
        ...
        for pooling in global_poolings:
            ...
            assert method in self.pool_method_to_reduce_type, 'Unexpected Global Pooling method `{}` for node `{}`'.format(method, name)
