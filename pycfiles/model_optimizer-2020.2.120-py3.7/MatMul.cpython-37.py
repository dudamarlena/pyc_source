...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/ops/MatMul.py
# Compiled at: 2020-05-01 08:37:21
# Size of source mod 2**32: 11713 bytes
"""
...
class MatMul(Op):
    ...
    def shape_alignment(node: Node):
        ...
        for i, shape in enumerate(input_shapes):
            ...
            assert input_shape.ndim == 1, "MatMul doesn't support scalar inputs. {} input of `{}` node has shape {}".format(i, node_name, input_shape)
            assert input_shape.size >= 1, "MatMul doesn't support inputs with rank lower than 1. {} input of `{}` node has shape {}".format(i, node_name, input_shape)
            if input_shape.size < 2:
                input_shape = np.insert(input_shape, 0, 1)
            if i == 0 and transpose_a or i == 1:
                if transpose_b:
        ...
        assert A_shape.size == B_shape.size, 'Shapes were not aligned by length for MatMul `{}`. Shapes: `{}`'.format(node_name, transformed_shapes)
        ...
        assert np.array_equal(A_shape[:-2], B_shape[:-2]), 'MatMul input shapes are incorrect. BATCH_DIMs are not equal. Node: {}. Aligned shapes: {}'.format(node_name, transformed_shapes)
        return (
         A_shape, B_shape)
    ...
    def infer(node: Node):
        ...
        if not (len(connected_in_ports) == 2 and 0 in connected_in_ports and 1 in connected_in_ports):
            raise AssertionError("MatMul should have 2 connected input ports, but it doesn't for node: `{}`. Ports: {}".format(name, connected_in_ports))
        ...
        assert A_shape[(-1)] == B_shape[(-2)], 'MatMul input shapes are incorrect. COL_INDEX_DIMs are not equal. Node: {}. Shapes: {}'.format(name, [A_shape, B_shape])
...
class FullyConnected(Op):
    ...
    def infer(node: Node):
        ...
        connected_in_ports = {idx:port for idx, port in node.in_ports().items() if not port.disconnected() if not port.disconnected()}
        if not (len(connected_in_ports) >= 2 and 0 in connected_in_ports and 1 in connected_in_ports):
            raise AssertionError("FullyConnected should have 2 connected input ports, but it doesn't for node: `{}`. Ports: {}".format(name, connected_in_ports))
        assert node.has_valid('out-size')
        input_shape = node.in_port(0).data.get_shape()
        weights_shape = node.in_port(1).data.get_shape()
        if not (input_shape is not None and weights_shape is not None):
            raise AssertionError('Incorrect FullyConnected input shapes. Node: {}. Shapes: {}'.format(name, [input_shape, weights_shape]))
        ...
        if 2 in connected_in_ports:
            ...
            if not np.array_equal(bias_shape, [out_size]):
                assert np.array_equal(bias_shape, [1, out_size]), 'Incorrect FullyConnected bias shape `{}` for node {}. `out-size`={}'.format(bias_shape, node, out_size)
