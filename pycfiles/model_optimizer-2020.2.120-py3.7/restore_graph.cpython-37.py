...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/utils/ir_reader/restore_graph.py
...
import extensions.back.PackBinaryWeights as PackBinaryWeights
...
import extensions.back.TopKNormalizer as TopKNormalizer
...
def restore_graph_from_ir(path_to_xml: str, path_to_bin: str=None) -> (Graph, dict):
    ...
    assert ir.graph.graph.get('ir_version') >= 10, 'IR version {} is not supported, please generate actual IR for your model and use it.'.format(ir.graph.graph.get('ir_version'))
    path = get_mo_root_dir()
    collect_ops(path)
    collect_extenders(path)
    new_graph = copy_graph_with_ops(ir.graph)
    return (
     new_graph, copy(ir.meta_data))
