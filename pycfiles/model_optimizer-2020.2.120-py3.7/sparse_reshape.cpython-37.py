...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/ops/sparse_reshape.py
...
class SparseReshape(Op):
    ...
    def infer(node: Node):
        ...
        if not (input_shape_value is not None and new_shape_value is not None):
            raise AssertionError('Values for input shape and new shape must be defined')
        assert np.count_nonzero(new_shape_value == -1) <= 1, 'Value -1 occurs in new shape value more than once'
