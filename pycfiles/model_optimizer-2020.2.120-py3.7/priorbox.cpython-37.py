...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/ops/priorbox.py
...
class PriorBoxOp(Op):
    ...
    def priorbox_infer(node: Node):
        ...
        if node.has_valid('fixed_size'):
            if len(node.fixed_size) > 0:
                num_ratios = len(ar_seen) * len(node.fixed_size)
        if node.has_valid('density'):
            if len(node.density) > 0:
