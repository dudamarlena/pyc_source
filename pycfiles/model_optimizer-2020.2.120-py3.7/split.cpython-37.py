...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/ops/split.py
# Compiled at: 2020-05-01 08:37:21
# Size of source mod 2**32: 11557 bytes
"""
...
class VariadicSplitBase(Op):
    ...
    def infer(node):
        ...
        if not (op is not None and op in ('VariadicSplit', 'AttributedVariadicSplit')):
            raise AssertionError('Unexpected `op`={} attribute for Split-like node {}'.format(op, name))
        num_in_ports = 1 if op == 'AttributedVariadicSplit' else 3 if op == 'VariadicSplit' else None
        assert num_in_ports in (1, 3), 'VariadicSplitBase supports AttributedVariadicSplit with 1 input and VariadicSplit with 3 inputs, but it is {} for {} node {}'.format(num_in_ports, op, name)
        connected_inputs = {idx:port for idx, port in node.in_ports().items() if not port.disconnected() if not port.disconnected()}
        if not (len(connected_inputs) == num_in_ports and all([i in connected_inputs for i in range(num_in_ports)])):
            raise AssertionError("{} should have {} connected input ports, but it doesn't for node: `{}`. Ports: {}".format(op, num_in_ports, name, connected_inputs))
        ...
        split_lengths = node.in_port(2).data.get_value() if op == 'VariadicSplit' else node.soft_get('split_lengths', None)
        ...
        if not undefined_elements.size <= 1:
            raise AssertionError('{} split_lengths=`{}` is a list with output sizes, only one of which could be -1. Node: {}'.format(op, split_lengths, name))
        else:
            input_elements = input_shape[axis]
            if not undefined_elements.size != 0:
                assert input_elements == np.sum(split_lengths), 'The sum of split_lengths=`{}` must match data.shape[axis]=`{}`. Node: {}'.format(split_lengths, input_elements, name)
                assert len(split_lengths) >= len([port for i, port in node.out_ports().items() if not port.disconnected()]), 'Number of split_lengths=`{}` is less than connected output ports. Node: {}'.format(split_lengths, name)
...
class VariadicSplit(VariadicSplitBase):
    ...
    def __init__(self, graph, attrs):
        assert 'axis' not in attrs, 'Please use `AttributedVariadicSplit` instead of `VariadicSplit` operation to create node with `axis` parameter set or keep using VariadicSplit operation, but express axis as a scalar second input of VariadicSplit operation'
        assert 'size_splits' not in attrs, 'Please use `AttributedVariadicSplit` instead of `VariadicSplit` operation to create node with `size_splits` parameter set or keep using VariadicSplit operation, but express size_splits as a 1D third input of VariadicSplit operation'
...
class AttributedVariadicSplit(VariadicSplitBase):
    ...
    def __init__(self, graph, attrs):
        ...
        assert 'size_splits' in attrs, 'AttributedVariadicSplit operation should have `size_splits` parameter set while creation'
...
class SplitBase(Op):
    ...
    def infer(node):
        name = node.soft_get('name', node.id)
        op = node.soft_get('op', None)
        if not (op is not None and op in ('Split', 'AttributedSplit')):
            raise AssertionError('Unexpected `op`={} attribute for Split-like node {}'.format(op, name))
        num_in_ports = 1 if op == 'AttributedSplit' else 2 if op == 'Split' else None
        if not num_in_ports in (1, 2):
            raise AssertionError('SplitBase supports AttributedSplit with 1 input and Split with 2 inputs, but it is {} for {} node {}'.format(num_in_ports, op, name))
        else:
            connected_inputs = {idx:port for idx, port in node.in_ports().items() if not port.disconnected() if not port.disconnected()}
            if len(connected_inputs) == num_in_ports:
                if not all([i in connected_inputs for i in range(num_in_ports)]):
                    raise AssertionError("{} should have {} connected input ports, but it doesn't for node: `{}`. Ports: {}".format(op, num_in_ports, name, connected_inputs))
                input_shape = node.in_port(0).data.get_shape()
                ...
                assert axis is not None, '{} `axis` is unknown for node {}'.format(op, name)
                assert axis.ndim == 0, '{} `axis` should be scalar, but it`s not for node {}'.format(op, name)
                assert input_shape[axis] % num_splits == 0, 'Input shape is not evenly divided by `num_splits` of {} node {}. `input_shape`={}, `axis`={}, `num_splits`={}'.format(op, name, input_shape, axis, num_splits)
                out_shape = input_shape.copy()
                out_shape[axis] = np.int64(input_shape[axis] / num_splits)
                input_value = node.in_port(0).data.get_value()
                output_value = np.split((input_value.copy()), axis=axis, indices_or_sections=num_splits) if input_value is not None else None
...
class Split(SplitBase):
    ...
    def __init__(self, graph, attrs):
        ...
        super().__init__(graph, {'op':self.op, 
         'type':self.op, 
         'infer':self.infer, 
         'in_ports_count':2}, attrs)
        assert 'axis' not in self.attrs, 'Please use `AttributedSplit` instead of `Split` operation to create node with `axis` parameter set or keep using Split operation, but express axis as a scalar second input of Split operation'
