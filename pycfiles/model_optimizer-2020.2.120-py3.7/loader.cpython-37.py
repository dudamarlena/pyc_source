...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/front/tf/loader.py
# Compiled at: 2020-05-01 08:37:22
# Size of source mod 2**32: 13770 bytes
"""
...
import mo.utils.summarize_graph as summarize_graph
...
def freeze_checkpoints(graph_def: tf_v1.GraphDef, checkpoint_dir: str, output_node_names: list):
    ...
    with tf_v1.Session() as (sess):
        ...
        all_variables = [n.name for n in sess.graph.as_graph_def().node if n.op in ('Variable',
                                                                                    'VariableV2')]
        ...
        output_graph_def = tf_v1.graph_util.convert_variables_to_constants(sess, graph_def, output_node_names, variable_names_whitelist=white_list,
    ...
    return (
     output_graph_def, variable_values)
...
def read_file_to_graph_def(graph_def: [tf_v1.GraphDef, tf_v1.MetaGraphDef], graph_file_name: str='', is_binary: bool=True):
    """
    ...
    try:
        ...
        try:
            raise FrameworkError('TensorFlow cannot read the model file: "{}" is incorrect TensorFlow model file. \nThe file should contain one of the following TensorFlow graphs:\n1. frozen graph in text or binary format\n2. inference graph for freezing with checkpoint (--input_checkpoint) in text or binary format\n3. meta graph\n\nMake sure that --input_model_is_text is provided for a model in text format. By default, a model is interpreted in binary format. Framework error details: {}. ' + refer_to_faq_msg(43), graph_file_name, str(e)) from e
        finally:
            e = None
            del e
...
def get_output_node_names_list(graph_def, user_defined_output_node_names_list: list):
    ...
    return user_defined_output_node_names_list
...
def deducing_metagraph_path(meta_graph_file: str):
    ...
    if match is not None:
        ...
        raise Error('\n\nMetaGraph freezing mechanism was enabled. \n{} file does not represent MetaGraph. \n\nModel with MetaGraph consists of 3-4 files:\n1. model_name.meta\n2. model_name.index\n3. model_name.data-00000-of-00001 (digit part may vary)\n4. checkpoint (optional)\n\nTo load this model, simply run:\npython3 mo_tf.py --input_meta_graph model_name.meta'.format(meta_graph_file))
    return meta_graph_file


def load_tf_graph_def(graph_file_name: str='', is_binary: bool=True, checkpoint: str='', model_dir: str='', saved_model_tags: list=[], meta_graph_file: str='', user_output_node_names_list: list=[]):
    graph_def = tf_v1.GraphDef()
    if isinstance(graph_file_name, str):
        if re.match('.*\\.(ckpt|meta)$', graph_file_name):
            print('[ WARNING ] The value for the --input_model command line parameter ends with ".ckpt" or ".meta" extension.\nIt means that the model is not frozen.\nTo load non frozen model to Model Optimizer run:\n\n1. For "*.ckpt" file:\n- if inference graph is in binary format\npython3 mo_tf.py --input_model "path/to/inference_graph.pb" --input_checkpoint "path/to/*.ckpt"\n- if inference graph is in text format\npython3 mo_tf.py --input_model "path/to/inference_graph.pbtxt" --input_model_is_text --input_checkpoint "path/to/*.ckpt"\n\n2. For "*.meta" file:\npython3 mo_tf.py --input_meta_graph "path/to/*.meta"')
    variables_values = {}
    try:
        if graph_file_name and not meta_graph_file:
            if not checkpoint:
                return (
                 read_file_to_graph_def(graph_def, graph_file_name, is_binary), variables_values)
            elif graph_file_name:
                if not meta_graph_file:
                    if checkpoint:
                        ...
                        if os.path.isfile(checkpoint):
                            ...
                            if os.path.isdir(checkpoint):
                                graph_def, variables_values = freeze_checkpoints(graph_def=graph_def, checkpoint_dir=checkpoint, output_node_names=outputs)
                        return (
                         graph_def, variables_values)
            if not graph_file_name:
                if meta_graph_file:
                    ...
                    with tf_v1.Session() as (sess):
                        ...
                        graph_def = tf_v1.graph_util.convert_variables_to_constants(sess, input_meta_graph_def.graph_def, outputs)
