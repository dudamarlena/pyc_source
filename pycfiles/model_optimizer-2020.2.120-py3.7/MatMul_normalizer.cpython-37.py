...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/MatMul_normalizer.py
...
class FullyConnectedDecomposer(FrontReplacementSubgraph):
    ...
    def pattern(self):
        return dict(nodes=[
         (
          'op', dict(kind='op', type='FullyConnected'))],
          edges=[])
    ...
    def replace_sub_graph(self, graph: Graph, match: [dict, SubgraphMatch]):
        ...
        node.insert_op_on_input_port(in_port_idx=1, new_op_class=Reshape, new_op_attrs={'name': name + '/weights_reshape'},
          value=reshape_dim)
        if node.has_and_set('transpose_weights'):
            node.insert_op_on_input_port(in_port_idx=1, new_op_class=Transpose, new_op_attrs={'name': name + '/weights_transpose'},
              value=(int64_array([1, 0])))
        if graph.graph['fw'] in ('caffe', 'mxnet'):
            node.insert_op_on_input_port(in_port_idx=0, new_op_class=Reshape, new_op_attrs={'name': name + '/flatten_fc_input'},
...
class GemmDecomposer(FrontReplacementSubgraph):
    ...
    def replace_sub_graph(self, graph: Graph, match: [dict, SubgraphMatch]):
        ...
        node.in_port(2).get_connection().set_destination(bias_node.in_port(1))
        node.out_port(0).connect(bias_node.in_port(0))
        if node.has_valid('alpha'):
            if not math.isclose(node.alpha, 1):
                bias_node.insert_op_on_input_port(in_port_idx=0, new_op_class=Mul, value=(np.array(node.alpha)), new_op_attrs={'name':name + '/Alpha_', 
                 'can_be_scaleshift':False})
                del node['alpha']
        if node.has_valid('beta'):
            if not math.isclose(node.beta, 1):
                bias_node.insert_op_on_input_port(in_port_idx=1, new_op_class=Mul, value=(np.array(node.beta)), new_op_attrs={'name':name + '/Beta_', 
                 'can_be_scaleshift':False})
                del node['beta']
        MatMul.update_node_stat(node, {'transpose_a':node.has_and_set('transpose_a'), 
