...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/ops/normalize.py
...
class NormalizeOp(Op):
    ...
    def __init__(self, graph, attrs):
        ...
        if 'across_spatial' in self.attrs:
            if isinstance(self.attrs['across_spatial'], str):
                self.attrs['across_spatial'] = int(self.attrs['across_spatial'])
        if 'channel_shared' in self.attrs:
            if isinstance(self.attrs['channel_shared'], str):
