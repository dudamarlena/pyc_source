...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/back/ElementwiseOpsToEltwiseOps.py
# Compiled at: 2020-05-01 08:37:19
# Size of source mod 2**32: 14114 bytes
"""
...
simple_eltwise_types = [
 'Power', 'Add', 'Multiply', 'Maximum', 'LogicalAnd', 'LogicalOr', 'LogicalXor', 'Less',
 'LessEqual',
 'Greater', 'GreaterEqual', 'Equal', 'NotEqual', 'FloorMod']
type_to_operation_map = {'Power':'pow', 
 'Add':'sum', 
 'Multiply':'mul', 
 'Maximum':'max', 
...
class EltwisesWithScalarInputToPower(BackReplacementPattern):
    ...
    def pattern():
        return dict(nodes=[
         (
          'op', {'type': lambda type: type in EltwisesWithScalarInputToPower.eltw_types})],
          edges=[])
...
class MulAddPowerMerge(BackReplacementPattern):
    ...
    def run_after(self):
        return [
         EltwisesWithScalarInputToPower]
    ...
    @staticmethod
    def replace_pattern(graph: Graph, match: [str, Node]):
        consumers = [n for n in match if n not in ('mul', 'add_d') if not check_node_usages_out_of_match(match, n)]
        if consumers:
            log.warning('Power(mul,add) pattern was detected. Non pattern consumers of nodes: "{}" were found. Won\'t replace'.format(', '.join([match[n].id for n in consumers])))
            return
        mul = match['mul']
        add = match['add']
        new_power = AttributedPower(graph, {'name':mul.name + '/fused_power',  'scale':mul.scale,  'shift':add.shift}).create_node()
...
class MulPowPowerMerge(BackReplacementPattern):
    ...
    @staticmethod
    def replace_pattern(graph: Graph, match: [str, Node]):
        consumers = [n for n in match if n not in ('mul', 'pow_d') if not check_node_usages_out_of_match(match, n)]
        if consumers:
            log.warning('Power(mul,pow) pattern was detected. Non pattern consumers of nodes: "{}" were found. Won\'t replace'.format(', '.join([match[n].id for n in consumers])))
            return
        mul = match['mul']
        pow = match['pow']
        new_power = AttributedPower(graph, {'name':mul.name + '/fused_power',  'scale':mul.scale,  'power':pow.power}).create_node()
        source = mul.in_port(0).get_connection().get_source()
        mul.in_port(0).disconnect()
        new_power.in_port(0).connect(source)
...
class AddPowPowerMerge(BackReplacementPattern):
    ...
    @staticmethod
    def replace_pattern(graph: Graph, match: [str, Node]):
        consumers = [n for n in match if n not in ('add', 'pow_d') if not check_node_usages_out_of_match(match, n)]
        if consumers:
            log.warning('Power(add,pow) pattern was detected. Non pattern consumers of nodes: "{}" were found. Won\'t replace'.format(', '.join([match[n].id for n in consumers])))
            return
        add = match['add']
        pow = match['pow']
        new_power = AttributedPower(graph, {'name':add.name + '/fused_power',  'shift':add.shift,  'power':pow.power}).create_node()
...
class MulAddPowPowerMerge(BackReplacementPattern):
    ...
    def run_after(self):
        return [
         MulAddPowerMerge, MulPowPowerMerge, AddPowPowerMerge]
    ...
    @staticmethod
    def replace_pattern(graph: Graph, match: [str, Node]):
        consumers = [n for n in match if n not in ('mul_add', 'pow_d') if not check_node_usages_out_of_match(match, n)]
        if consumers:
            log.warning('Power(mul_add,pow) pattern was detected. Non pattern consumers of nodes: "{}" were found. Won\'t replace'.format(', '.join([match[n].id for n in consumers])))
            return
        mul_add = match['mul_add']
        pow = match['pow']
        new_power = AttributedPower(graph, {'name':mul_add.name + '/fused_power',  'shift':mul_add.shift,  'scale':mul_add.scale, 
         'power':pow.power}).create_node()
        source = mul_add.in_port(0).get_connection().get_source()
        mul_add.in_port(0).disconnect()
        new_power.in_port(0).connect(source)
        pow.out_port(0).get_connection().set_source(new_power.out_port(0))
        log.debug('Power nodes {} and {} were fused to single Power node {}'.format(mul_add.name, pow.name, new_power.name))