...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/ops/non_max_suppression.py
...
class NonMaxSuppression(Op):
    ...
    def __init__(self, graph, attrs):
        mandatory_props = {'type':__class__.op, 
         ...
         'force_precision_in_ports':{2: 'int64' if graph.graph['cmd_params'].generate_experimental_IR_V10 else 'int32'}, 
    ...
    @staticmethod
    def type_infer(node):
        node.out_port(0).set_data_type(np.int64 if node.graph.graph['cmd_params'].generate_experimental_IR_V10 else np.int32)