...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/back/CutMemory.py
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        if node.in_port(0).disconnected():
            ...
            for dest in node.out_port(0).get_destinations():
                new_in = Parameter(graph, {'name':'Parameter_' + str(i) + '_for_' + node_id,  'shape':dest.data.get_shape()}).create_node()
                i += 1
                dest.disconnect()
                new_in.out_port(0).connect(dest)
                log.error(('Add input/output mapped {} -> {} '.format(new_in.name, 'Result_for_' + node_id)), extra={'is_warning': True})
