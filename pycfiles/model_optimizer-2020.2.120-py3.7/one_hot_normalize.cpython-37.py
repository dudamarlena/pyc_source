...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/onnx/one_hot_normalize.py
...
class OneHotNormalize(FrontReplacementSubgraph):
    ...
    def pattern(self):
        return dict(nodes=[('onehot', dict(op='OneHot', split_values=True))], edges=[])

    def replace_sub_graph(self, graph: Graph, match: dict):
        onehot = match['onehot']
        name = onehot.soft_get('name', onehot.id)
        split = create_op_with_const_inputs(graph, Split, {1: np.int64(0)}, {'name':name + '/Split', 
         'num_splits':2,  'squeeze_axis':True})
