...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/front/onnx/priorbox_ext.py
...
class PriorBoxFrontExtractor(FrontExtractorOp):
    ...
    def extract(cls, node):
        ...
        if len(variance) == 0:
            variance = [
             0.1]
        update_attrs = {'aspect_ratio':onnx_attr(node, 'aspect_ratio', 'floats', dst_type=lambda x: np.array(x, dtype=(np.float32))), 
