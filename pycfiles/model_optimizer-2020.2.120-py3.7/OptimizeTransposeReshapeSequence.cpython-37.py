...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/back/OptimizeTransposeReshapeSequence.py
# Compiled at: 2020-05-01 08:37:19
# Size of source mod 2**32: 17914 bytes
"""
...
"""
import logging as log, math, numpy as np
...
def match_shapes(input_shape: np.array, output_shape: np.array):
    ...
    while not in_ind < len(input_shape):
...
def split_dims_indices(input_shape: np.array, match_shape: np.array):
    ...
    while match_ind < len(match_shape):
        if in_ind >= len(input_shape):
            ...
            if in_ind < len(input_shape):
                in_left *= input_shape[in_ind]
        elif in_left > match_shape[match_ind] and not in_left > match_shape[match_ind]:
            if match_shape[match_ind] == 1:
...
class OptimizeTransposeReshapeSequence(BackReplacementPattern):
    ...
    def run_after(self):
        ...
        return [
         FuseTransposesSequence, Tile3DReshaper]
    ...
    def is_node_match_for_optimization(self, node: Node):
        ...
        """
        return node.has_and_set('type') and node.type in ('Transpose', 'Reshape') and not node.has_and_set(self.OPTIMIZED_NODE_FLAG)
        ...
        for start_node in graph.pseudo_topological_sort():
            ...
            if self.is_node_match_for_optimization(start_node):
                ...
                while self.is_node_match_for_optimization(next_node):
                    ...
                    if len(next_nodes) > 1:
                        log.debug('There are two consumers of the node {}. Stop matching sequence.'.format(next_node.soft_get('name')))
                        break
    ...
    @staticmethod
    def optimize_permute_reshape_sequence(graph, nodes):
        log.debug('Running permute-reshape optimization of the following nodes: {}'.format([node.soft_get('name') for node in nodes]))
        ...
        for ind in range(1, len(nodes) - 1):
            ...
            if node.type == 'Reshape':
                np.array_equal(input_shape, output_shape) or log.debug('The Reshape node "{}" is not NOP. Shapes: "{}" vs "{}"'.format(node.soft_get('name'), input_shape, output_shape))
                __class__.make_reshape_nop(node)

    @staticmethod
    def add_leading_and_trailing_reshape(graph, nodes):
        """
        ...
        if nodes[0].type == 'Transpose':
            dummy_reshape_node = create_op_node_with_second_input(graph, Reshape, nodes[0].in_port(0).data.get_shape().copy(), {'name': nodes[0].in_port(0).get_connection().get_source().node.id + '/Reshape'})
            ...
            log.debug('Added Reshape op "{}" in the beginning of the permute-reshape sequence'.format(dummy_reshape_node.soft_get('name')))
        if nodes[(-1)].type == 'Transpose':
            dummy_reshape_node = create_op_node_with_second_input(graph, Reshape, nodes[(-1)].out_port(0).data.get_shape().copy(), {'name': nodes[0].out_port(0).get_connection().get_destination().node.id + '/Reshape'})
            dummy_reshape_node[__class__.OPTIMIZED_NODE_FLAG] = True
            nodes[(-1)].out_port(0).get_connection().insert_node(dummy_reshape_node)
            nodes.append(dummy_reshape_node)
            log.debug('Added Reshape op "{}" in the end of the permute-reshape sequence'.format(dummy_reshape_node.soft_get('name')))
    ...
    def forward_new_reshape_shape(reshape_node: Node, initial_output_shape: np.array):
        ...
        if np.all(output_shape == initial_output_shape):
            log.debug('Initial output and new output shapes match for node "{}". Do nothing'.format(reshape_node.soft_get('name')))
            return
            ...
            if dest_node.type == 'Transpose':
                ...
                assert dest_node.in_port(1).data.get_value() is not None, 'The 1st input value "order" is not set for Transpose node "{}"'.format(dest_node.soft_get('name'))
            ...
            log.debug('Two subsequent reshape nodes: "{}" and "{}". Nothing to optimize'.format(reshape_node.soft_get('name'), dest_node.soft_get('name')))
        else:
            assert False, 'Unsupported type of the node "{}" in the Transpose-Reshape optimization'.format(dest_node.type)
    ...
    def backward_new_reshape_shape(reshape_node: Node, initial_input_shape: np.array):
        ...
        if np.all(input_shape == initial_input_shape):
            log.debug('Initial input and new input shapes match for node "{}". Do nothing'.format(reshape_node.soft_get('name')))
            return
            ...
            if src_node.type == 'Transpose':
                ...
                assert src_node.in_port(1).data.get_value() is not None, 'The 1st input value "order" is not set for Transpose node "{}"'.format(src_node.soft_get('name'))
            ...
            log.debug('Two subsequent reshape nodes: "{}" and "{}". Nothing to optimize'.format(reshape_node.soft_get('name'), src_node.soft_get('name')))
        else:
            assert False, 'Unsupported type of the node "{}" in the Transpose-Reshape optimization'.format(src_node.type)

    @staticmethod
    def make_reshape_nop(reshape_node):
        """
