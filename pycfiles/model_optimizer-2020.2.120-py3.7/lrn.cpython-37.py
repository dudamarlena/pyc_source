...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mo/ops/lrn.py
...
class LRN(Op):
    ...
    def __init__(self, graph, attrs):
        ...
        assert 'region' not in attrs, 'LRN operation should not have `region` parameter set while creation, please use AttributedLRN operation instead or keep using LRN operation with region expressed as second `axis`-input'
        super().__init__(graph, {'type':self.op, 
         'op':self.op, 
         'infer':self.infer, 
         'in_ports_count':2, 
    ...
    def infer(node):
        ...
        if not (len(connected_inputs) == 2 and 0 in connected_inputs and 1 in connected_inputs):
            raise AssertionError('LRN should have 2 connected input ports, but it doesn`t for node: `{}`. Ports: {}'.format(name, connected_inputs))
...
class AttributedLRN(Op):
    ...
    def __init__(self, graph, attrs):
        ...
        assert self.attrs['region'] in ('across', 'same'), 'AttributedLRN operation should have `region` parameter set to `across` or `same`, but it is `{}`'.format(self.attrs['region'])
    ...
    def infer(node):
        name = node.soft_get('name', node.id)
        connected_inputs = {idx:port for idx, port in node.in_ports().items() if not port.disconnected() if not port.disconnected()}
        if not (len(connected_inputs) == 1 and 0 in connected_inputs):
            raise AssertionError('AttributedLRN should have 1 connected input port, but it doesn`t for node: `{}`. Ports: {}'.format(name, connected_inputs))
        input_shape = node.in_port(0).data.get_shape()
        assert input_shape is not None, 'Input shape is unknown for node {}'.format(name)
        node.out_port(0).data.set_shape(input_shape)