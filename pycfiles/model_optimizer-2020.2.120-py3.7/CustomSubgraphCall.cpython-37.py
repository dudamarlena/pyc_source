...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extensions/middle/CustomSubgraphCall.py
# Compiled at: 2020-05-01 08:37:21
# Size of source mod 2**32: 15828 bytes
"""
...
"""
import copy, logging as log, numpy as np
...
class CustomSubgraphCall(MiddleReplacementPattern):
    ...
    def update_placeholder_shape_and_add_transpose(node: Node):
        ...
        for placeholder_name in node['input_nodes_names']:
            dummy_node = tf_v1.constant(value=[[[[1]]]], dtype=(tf_v1.float32), name=('random_dummy_name_' + placeholder_name))
            ...
            if len(cur_shape) == 4:
                ...
                transpose = tf_v1.transpose(dummy_node, nchw_to_nhwc_constant, transpose_name)
    ...
    def add_reshape_before_op_node(graph: Graph, data_node_name: str, op_node_name: str, edge_attrs: dict):
        ...
        graph.add_node(reshape_shape_data_node_name, kind='data', name=reshape_shape_data_node_name, value=new_shape, shape=[
         1])
        reshape_node_name = graph.unique_id('Reshape_')
        graph.add_node(reshape_node_name, kind='op', type='Reshape', name=reshape_node_name, op='Reshape', data_type=(data_node['data_type']))
        ...
        graph.add_node(reshaped_data_node_name, kind='data', name=reshaped_data_node_name, shape=new_shape, value=reshaped_value,
          nchw_layout=True)
    ...
    def add_reshape_after_data_node(graph: Graph, data_node_name: str):
        ...
        if len(data_node.out_nodes()) == 1:
            if data_node.out_node().has('op'):
                if data_node.out_node().op == 'Result':
                    is_out_node = True
        ...
        update_ie_fields(graph.node[reshape_node_name])
        reshape_shape_data_node_name = graph.unique_id('Reshape_shape_')
        graph.add_node(reshape_shape_data_node_name, kind='data', name=reshape_shape_data_node_name, value=(np.array(data_node['shape'])),
          shape=[1])
        reshaped_value = None
        if data_node['value'] is not None:
            reshaped_value = np.array(data_node['value'])
        reshaped_data_node_name = graph.unique_id('reshaped_data_')
        graph.add_node(reshaped_data_node_name, kind='data', name=reshaped_data_node_name, shape=(np.array(data_node['shape'])),
    ...
    def add_reshapes_for_tf_subgraph_calls(graph: Graph):
        ...
        for src_node_name, dst_node_name, edge_attrs in list(graph.edges(data=True)):
            ...
            if dst_node.kind == 'op' and dst_node.has_valid('type') and dst_node.type == 'TFCustomSubgraphCall' and src_node.has_valid('shape') and len(src_node.shape) != 4:
                log.info("There is an data tensor of shape '{}' which goes into '{}' node".format(src_node.shape, dst_node.type))
            ...
            if node['kind'] == 'op' and node.has_and_set('type') and node.type == 'TFCustomSubgraphCall':
                for index, data_node in node.out_nodes().items():
                    ...
                    if real_dims_count != 4:
                        log.info("There is an data tensor of shape '{}' with real dims count '{}' which goes out of '{}' node".format(data_node.shape, real_dims_count, node.name))
    ...
    def add_sub_graph_call_output_tensors_transposes(node: Node):
        ...
        for out_tensor_name in node['output_tensors_names']:
            ...
            if len(output_tensors[int(out_port)].shape) == 4:
