...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/eagle.py
# Compiled at: 2006-07-28 15:12:25
...
def _set_icon_list(gtkwidget, stock_id):
    ...
    if iconset:
        ...
        for s in iconset.get_sizes():
            i = iconset.render_icon(style, gtk.TEXT_DIR_NONE, gtk.STATE_NORMAL, s, gtkwidget, None)
            icons.append(i)
...
class _ExpandRule(object):
    ...
    def __str__(self):
        return 'ExpandRule( expand=%s, fill=%s )' % (self.expand, self.fill)
...
class ExpandPolicy(object):
    ...
    class Policy(object):
        ...
        def __str__(self):
            return '%s( horizontal=%s, vertical=%s )' % (self.__class__.__name__, self.horizontal, self.vertical)
        ...
        for (idx, c) in enumerate(self.children):
            ...
            if len(w) == 1:
                ...
                if self.horizontal:
                    ...
                    col0 = 0
                    col1 = 2
                self.attach(w[0], col0, col1, row0, row1, xoptions=xrm, yoptions=yrm, xpadding=self.padding, ypadding=self.padding)
            elif len(w) == 2:
                if not isinstance(policy, (tuple, list)):
                    policy = (
                     policy, policy)
                xrm = (policy[0].horizontal.__get_gtk_resize_policy__(), policy[1].horizontal.__get_gtk_resize_policy__())
                yrm = (
                 policy[0].vertical.__get_gtk_resize_policy__(), policy[1].vertical.__get_gtk_resize_policy__())
                if self.horizontal:
                    row0 = 0
                    row1 = 1
                    row2 = 1
                    ...
                    col2 = 1
                    col3 = 2
                self.attach(w[0], col0, col1, row0, row1, xoptions=xrm[0], yoptions=yrm[0], xpadding=self.padding, ypadding=self.padding)
                self.attach(w[1], col2, col3, row2, row3, xoptions=xrm[1], yoptions=yrm[1], xpadding=self.padding, ypadding=self.padding)
...
class _Panel(gtk.ScrolledWindow):
    ...
    def __setup_gui__(self):
        ...
        self.set_policy(hscrollbar_policy=self._hscrollbar_policy, vscrollbar_policy=self._vscrollbar_policy)
...
class Image(_EGObject, AutoGenId):
    ...
    def __init__(self, **kargs):
        ...
        if 'filename' in kargs:
            ...
            k = {'data': kargs['data'], 'width': kargs['width'], 'height': kargs['height']}
            if 'depth' in kargs:
                k['depth'] = kargs['depth']
    ...
    def save(self, filename, format=None, **options):
        ...
        for f in self.get_writable_formats():
            if format == f['name'] or format in f['extensions'] or format in f['mime_types']:
                t = f['name']
                break
    ...
    def load_data(self, data, width, height, depth=24, has_alpha=None, rowstride=None):
        ...
        if len(data) < height * rowstride:
            raise ValueError('data must be at least width * height * rowstride long.Values are: data size=%d, required=%d' % (len(data), height * rowstride))
        ...
        self._img = gtk.gdk.pixbuf_new_from_data(data, colorspace, has_alpha, bits_per_sample, width, height, rowstride)
        return
    ...
    def get_data(self):
        ...
        """
        return (
         self.get_width(), self.get_height(), self.get_depth(), self.has_alpha(), self.get_rowstride(), self._img.get_pixels())
...
class _EGDataWidget(_EGWidget):
    ...
    def get_value(self):
        ...
        raise NotImplementedError("%s doesn't implement get_value()" % self.__class__.__name__)

    def set_value(self, value):
        """Set data to this widget."""
        raise NotImplementedError("%s doesn't implement set_value()" % self.__class__.__name__)
...
class AboutDialog(_EGWidget, AutoGenId):
    ...
    def __init__(self, app, title, author=None, description=None, help=None, version=None, license=None, copyright=None):
    ...
    def __setup_gui__(self):
        ...
        self._diag = gtk.Dialog(title='About: %s' % self.title, parent=win, flags=flags, buttons=btns)
...
class HelpDialog(_EGWidget, AutoGenId):
    ...
    def __setup_gui__(self):
        ...
        btns = (gtk.STOCK_CLOSE, gtk.RESPONSE_CLOSE)
        flags = gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT
        self._diag = gtk.Dialog(title='Help: %s' % self.title, parent=win, flags=flags, buttons=btns)
        self._diag.set_border_width(self.border)
        self._diag.set_default_size(self.width, self.height)
        self._diag.set_has_separator(False)
        self._diag.vbox.set_spacing(self.spacing)
...
class FileChooser(_EGWidget, AutoGenId):
    ...
    def __init__(self, app, action, filename=None, title=None, filter=None, multiple=False):
    ...
    def __gen_title__(self):
        t = {self.ACTION_OPEN: 'Open: %s', self.ACTION_SAVE: 'Save: %s', self.ACTION_SELECT_FOLDER: 'Open Folder: %s', self.ACTION_CREATE_FOLDER: 'Create Folder: %s'}
    ...
    def __setup_gui__(self):
        win = self.app.__get_window__()
        a = {self.ACTION_OPEN: gtk.FILE_CHOOSER_ACTION_OPEN, self.ACTION_SAVE: gtk.FILE_CHOOSER_ACTION_SAVE, self.ACTION_SELECT_FOLDER: gtk.FILE_CHOOSER_ACTION_SELECT_FOLDER, self.ACTION_CREATE_FOLDER: gtk.FILE_CHOOSER_ACTION_CREATE_FOLDER}.get(self.action, gtk.FILE_CHOOSER_ACTION_OPEN)
        b = (
         gtk.STOCK_OPEN, gtk.RESPONSE_ACCEPT, gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL)
        self._diag = gtk.FileChooserDialog(title=self.title, parent=win, action=a, buttons=b)
...
class PreferencesDialog(_EGWidget, AutoGenId):
    ...
    def __setup_gui__(self):
        ...
        btns = (gtk.STOCK_CLOSE, gtk.RESPONSE_CLOSE)
        flags = gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT
        self._diag = gtk.Dialog(title='Preferences: %s' % self.app.title, parent=win, flags=flags, buttons=btns)
        ...
        self._sw.set_policy(hscrollbar_policy=gtk.POLICY_AUTOMATIC, vscrollbar_policy=gtk.POLICY_AUTOMATIC)
...
class DebugDialog(_EGObject, AutoGenId):
    ...
    def __setup_gui__(self):
        ...
        self._diag = gtk.Dialog('Application Crashed!', parent=None, flags=gtk.DIALOG_MODAL, buttons=b)
        self._diag.set_border_width(self.border)
        self._diag.set_default_size(self.width, self.height)
        self._diag.set_has_separator(False)
        self._diag.vbox.set_spacing(self.spacing)
    ...
    def __setup_text__(self):
        ...
        self._buf.create_tag('code', foreground='gray25', family='monospace')
        self._buf.create_tag('exc', foreground='#880000', weight=pango.WEIGHT_BOLD)
    ...
    def save_exception(self, exctype, value, tb):
        ...
        filename = '%s-%s-%s.tb' % (progname, os.getuid(), int(time.time()))
...
class _EGWidLabelEntry(_EGDataWidget):
    ...
    def __init__(self, id, persistent, label='', expand_policy=None):
        ...
        if expand_policy is None:
            expand_policy = (
             ExpandPolicy.Fill(), ExpandPolicy.Horizontal())
        elif not isinstance(expand_policy, (list, tuple)):
            expand_policy = (
             expand_policy, expand_policy)
        _EGDataWidget.__init__(self, id, persistent, expand_policy=expand_policy)
        self.__label = label
        self.__setup_gui__()
        return
    ...
    def set_label(self, label):
        if self.__label is None:
            raise ValueError("You cannot change label of widget created without one. Create it with placeholder! (label='')")
        self.__label = label
    ...
    def __str__(self):
        return '%s( id=%r, label=%r, value=%r )' % (self.__class__.__name__, self.id, self.label, self.get_value())
...
class App(_EGObject, AutoGenId):
    ...
    def __init__(self, title, id=None, center=None, left=None, right=None, top=None, bottom=None, preferences=None, window_size=(800, 600), quit_callback=None, data_changed_callback=None, author=None, description=None, help=None, version=None, license=None, copyright=None, statusbar=False):
        """App Constructor.
    ...
    def __setitem__(self, name, value):
        ...
        if w is None:
            ...
            raise TypeError("Could not set value of widget '%s' of type '%s'." % (name, type(w).__name__))
        return

    def get_widget_by_id(self, widget_id):
        """Return referece to widget with provided id or None if not found."""
        if isinstance(widget_id, _EGWidget) and widget_id in self._widgets.itervalues():
            return widget_id
        else:
    ...
    def show_about_dialog(self):
        ...
        diag = AboutDialog(app=self, title=self.title, author=self.author, description=self.description, help=self.help, version=self.version, license=self.license, copyright=self.copyright)
        diag.run()

    def show_help_dialog(self):
        """Show L{HelpDialog} of this App."""
        diag = HelpDialog(app=self, title=self.title, help=self.help)
        diag.run()

    def file_chooser(self, action, filename=None, filter=None, multiple=False):
        ...
        @see: L{FileChooser}
        """
        diag = FileChooser(app=self, action=action, filename=filename, filter=filter, multiple=multiple)
        return diag.run()
    ...
    def __add_widget__(self, widget):
        if widget.id in self._widgets:
            ...
            raise ValueError('Object id "%s" (type: %s) already in application "%s" as type: %s!' % (widget.id, widget.__class__.__name__, self.id, w.__class__.__name__))
        elif widget.app is None:
            ...
            try:
                ...
                raise ValueError('Object "%s" already is in another App: "%s"' % (widget.id, id))
        return
    ...
    def __setup_gui__(self):
        ...
        if self.statusbar:
            ...
            self._top_layout.pack_end(self._statusbar, expand=False, fill=True)
        self._win.show_all()
    ...
    def __setup_gui_preferences__(self):
        self._preferences = PreferencesDialog(self, children=self.preferences)
    ...
    def io_watch(self, file, callback, on_in=False, on_out=False, on_urgent=False, on_error=False, on_hungup=False):
        ...
        def wrap(source, cb_condition):
            ...
            return callback(self, source, on_in=on_in, on_out=on_out, on_urgent=on_urgent, on_error=on_error, on_hungup=on_hungup)
...
class Canvas(_EGWidget):
    ...
    def __init__(self, id, width, height, label='', bgcolor=None, scrollbars=True, callback=None, expand_policy=None):
        """Canvas Constructor.
    ...
    def __setup_gui__(self, width, height):
        ...
        self._sw.set_policy(hscrollbar_policy=policy, vscrollbar_policy=policy)
    ...
    def __setup_connections__(self):
        ...
        def expose_event(widget, event):
            ...
            widget.window.draw_drawable(gc, self._pixmap, x, y, x, y, width, height)
            return False
        ...
        buttons_map = {1: self.MOUSE_BUTTON_1, 2: self.MOUSE_BUTTON_2, 3: self.MOUSE_BUTTON_3, 4: self.MOUSE_BUTTON_4, 5: self.MOUSE_BUTTON_5}
        ...
        self._area.set_events(gtk.gdk.EXPOSURE_MASK | gtk.gdk.LEAVE_NOTIFY_MASK | gtk.gdk.BUTTON_PRESS_MASK | gtk.gdk.BUTTON_RELEASE_MASK | gtk.gdk.POINTER_MOTION_MASK | gtk.gdk.POINTER_MOTION_HINT_MASK)
    ...
    def __configure_gc__(self, fgcolor=None, bgcolor=None, fill=None, line_width=None, line_style=None):
    ...
    def resize(self, width, height):
        ...
        if old is None:
            ...
            (w, h) = old.get_size()
            self._pixmap.draw_drawable(self._fg_gc_normal, old, 0, 0, 0, 0, w, h)
        return

    def draw_image(self, image, x=0, y=0, width=None, height=None, src_x=0, src_y=0):
        ...
        if not isinstance(image, Image):
            raise TypeError('image must be instance of Image class, but %s found!' % type(image).__name__)
        ...
        self._pixmap.draw_pixbuf(self._fg_gc_normal, p, src_x, src_y, x, y, width, height)
        self._area.queue_draw_area(x, y, width, width)
        return

    def draw_text(self, text, x=0, y=0, fgcolor=None, bgcolor=None, font_name=None, font_size=None, font_options=0, font_family=None, width=None, wrap_word=False, alignment=LEFT, justify=True):
        """Draw text on canvas.
        ...
        alignment = {self.LEFT: pango.ALIGN_LEFT, self.CENTER: pango.ALIGN_CENTER, self.RIGHT: pango.ALIGN_RIGHT}.get(alignment, pango.ALIGN_CENTER)
        ...
        self._pixmap.draw_layout(self._fg_gc_normal, x, y, layout, fgcolor, bgcolor)
    ...
    def draw_rectangle(self, x, y, width, height, color=None, size=1, fillcolor=None, filled=False):
        """Draw rectagle.
    ...
    def draw_arc(self, x, y, width, height, start_angle, end_angle, color=None, size=1, fillcolor=None, filled=False):
        """Draw arc on canvas.
        ...
        """
        mult = 180.0 / 3.141592653589793 * 64.0
        start_angle = int(mult * start_angle)
        end_angle = int(mult * end_angle)
        if filled:
            gc = self.__configure_gc__(fgcolor=fillcolor, fill=filled)
            self._pixmap.draw_arc(gc, True, x, y, width, height, start_angle, end_angle)
        if size > 0:
            gc = self.__configure_gc__(fgcolor=color, line_width=size)
            self._pixmap.draw_arc(gc, False, x, y, width, height, start_angle, end_angle)
        else:
            size = 0
        half = size / 2
        self._area.queue_draw_area(x - half, y - half, width + size, height + size)

    def draw_polygon(self, points, color=None, size=1, fillcolor=None, filled=False):
    ...
    def fill(self, color):
        (w, h) = self.get_size()
        self.draw_rectangle(0, 0, w, h, color, size=0, fillcolor=color, filled=True)
    ...
    def get_image(self):
        ...
        img.get_from_drawable(self._pixmap, self._area.get_colormap(), 0, 0, 0, 0, w, h)
    ...
    def __str__(self):
        return '%s( id=%r, width=%r, height=%r, label=%r )' % (self.__class__.__name__, self.id, self.width, self.height, self.label)
...
class _MultiLineEntry(gtk.ScrolledWindow):
    ...
    def __init__(self):
        gtk.ScrolledWindow.__init__(self)
        self.set_policy(hscrollbar_policy=gtk.POLICY_AUTOMATIC, vscrollbar_policy=gtk.POLICY_AUTOMATIC)
...
gobject.signal_new('changed', _MultiLineEntry, gobject.SIGNAL_RUN_LAST, gobject.TYPE_NONE, tuple())
...
class Entry(_EGWidLabelEntry):
    ...
    def __init__(self, id, label='', value='', callback=None, editable=True, persistent=False, multiline=False, expand_policy=None):
        """Entry constructor.
        ...
        _EGWidLabelEntry.__init__(self, id, persistent, label, expand_policy=expand_policy)
        self.__setup_gui__()
        self.__setup_connections__()
        return
    ...
    def __configure_orientation__(self, setting):
        ...
        if self.multiline and self.label and setting == self.ORIENTATION_VERTICAL:
...
class Password(Entry):
    ...
    def __init__(self, id, label='', value='', callback=None, persistent=False, expand_policy=None):
        ...
        @param expand_policy: how this widget should fit space, see
               L{ExpandPolicy.Policy.Rule}.
        """
        Entry.__init__(self, id, label, value, callback, persistent, expand_policy=expand_policy)
...
class Spin(_EGWidLabelEntry):
    ...
    def __init__(self, id, label='', value=None, min=None, max=None, step=None, digits=3, callback=None, persistent=False, expand_policy=None):
        """Spin constructor.
...
class IntSpin(Spin):
    ...
    def __init__(self, id, label='', value=None, min=None, max=None, step=None, callback=None, persistent=False, expand_policy=None):
        ...
        if step is not None:
            step = int(step)
        Spin.__init__(self, id, label, value, min, max, step, 0, callback, persistent, expand_policy=expand_policy)
        return
...
class UIntSpin(IntSpin):
    ...
    def __init__(self, id, label='', value=None, min=0, max=None, step=None, callback=None, persistent=False, expand_policy=None):
...
class Color(_EGWidLabelEntry):
    ...
    def __init__(self, id, label='', color=0, use_alpha=False, callback=None, persistent=False, expand_policy=None):
...
class Font(_EGWidLabelEntry):
    ...
    def __init__(self, id, label='', font='sans 12', callback=None, persistent=False, expand_policy=None):
...
class Selection(_EGWidLabelEntry):
    ...
    def __init__(self, id, label='', options=None, active=None, callback=None, persistent=False, expand_policy=None):
...
class Progress(_EGWidLabelEntry):
    ...
    def __init__(self, id, label='', value=0.0, expand_policy=None):
        ...
        """
        self.value = value
        _EGWidLabelEntry.__init__(self, id, False, label, expand_policy=expand_policy)
    ...
    def set_value(self, value):
        if 1.0 < value <= 100.0:
            ...
            raise ValueError('Progress value of "%s" must be between 0.0 and 1.0!' % self.id)
...
class CheckBox(_EGDataWidget):
    ...
    def __init__(self, id, label='', state=False, callback=None, persistent=False, expand_policy=None):
...
class Group(_EGWidget):
    ...
    def __init__(self, id, label='', children=None, horizontal=False, border=BORDER_ETCHED_IN, expand_policy=None):
        """Group constructor.
    ...
    def __setup_gui__(self):
        self._contents = _Table(id='%s-contents' % self.id, children=self.children, horizontal=self.horizontal)
    ...
    def set_border(self, border):
        if self.__border is None:
            raise ValueError('You cannot change border of widget created without one.')
...
class Tabs(_EGWidget):
    ...
    class Page(_EGWidget, AutoGenId):
        ...
        def __init__(self, id=None, label='', children=None, horizontal=False):
        ...
        def __setup_gui__(self):
            self._wid = _Table(id='%s-contents' % self.id, children=self.children, horizontal=self.horizontal)
            self._widgets = (
             self._wid,)

        def set_label(self, label):
            if label is None:
                raise ValueError("You cannot have 'None' labels for Tabs.Page!")
        ...
        if not children:
            ...
            for (i, widget) in enumerate(children):
                if isinstance(widget, Tabs.Page):
                    ...
                    sys.stderr.write('children #%d (%s) is not instance of Tabs.Page, but %s' % (i, widget, type(widget).__name__))
    ...
    def __setup_gui__(self):
        ...
        for w in self.children:
            w.parent = self
            self._wid.append_page(w._widgets[0], gtk.Label(w.label))
...
class Table(_EGWidget):
    ...
    class CellFormat(object):
        __module__ = __name__
        __slots__ = ('fgcolor', 'bgcolor', 'font', 'bold', 'italic', 'underline', 'strike',
                     'contents')
    ...
    def __init__(self, id, label='', items=None, types=None, headers=None, show_headers=True, editable=False, repositioning=False, expand_columns_indexes=None, hidden_columns_indexes=None, cell_format_func=None, selection_callback=None, data_changed_callback=None, expand_policy=None):
        """Table constructor.
        ...
        if isinstance(expand_columns_indexes, (int, long)):
            ...
            raise ValueError('expand_columns_indexes must be a sequence of integers')
        ...
        if isinstance(hidden_columns_indexes, (int, long)):
            ...
            raise ValueError('hidden_columns_indexes must be a sequence of integers')
    ...
    def __setup_connections_editable__(self):
        ...
        def edit_dialog(data):
            ...
            d = gtk.Dialog(title=title, flags=gtk.DIALOG_MODAL, buttons=buttons)
            ...
            for (i, v) in enumerate(data):
                ...
                t.attach(label, 0, 1, i, i + 1, xoptions=gtk.FILL, xpadding=self.spacing, ypadding=self.spacing)
                t.attach(entry, 1, 2, i, i + 1, xoptions=gtk.EXPAND | gtk.FILL, xpadding=self.spacing, ypadding=self.spacing)
                w[i] = entry
            ...
            sw.set_policy(hscrollbar_policy=gtk.POLICY_NEVER, vscrollbar_policy=gtk.POLICY_NEVER)
            d.show_all()
                ...
                if t == bool:
                    ...
                    try:
                        ...
                        try:
                            ...
                            raise ValueError('Unsuported column (%d) type: %s. Type should provide default_value() static method.' % (i, name))
    ...
    def __create_column_cell_format_func__(self, col, cell_rend):
        ...
        def func(column, cell_renderer, model, itr, col_idx):
            ...
            cf = self.cell_format_func(self.app, self, row_idx, col_idx, value)
            if cf is None:
                cf = Table.CellFormat()
            bgcolor = cf.bgcolor
            if bgcolor is not None:
            ...
            cell_renderer.set_property('cell-background-gdk', bgcolor)
            if isinstance(cell_renderer, gtk.CellRendererText):
                ...
                cell_renderer.set_property('strikethrough', bool(cf.strike))
    ...
    def __setup_table__(self):
        ...
        def edited(cell_render, path, text, col):
            ...
            try:
                ...
                name = t.__name__
                error("Invalid contents for column of type '%s': %s" % (name, text))
            else:
...
class RichText(_EGWidget):
    ...
    class Renderer(gtk.TextView):
        ...
        def __init__(self, link_color='#0000ff', foreground=None, background=None, resource_provider=None):
        ...
        def __create_bullets__(self):
            ...
            if klass.bullet is None:
                ...
                white_gc = pixmap.new_gc(foreground=white, background=black, fill=gtk.gdk.SOLID, line_width=1, line_style=gtk.gdk.LINE_SOLID)
                black_gc = pixmap.new_gc(foreground=black, background=white, fill=gtk.gdk.SOLID, line_width=1, line_style=gtk.gdk.LINE_SOLID)
                pixmap.draw_rectangle(white_gc, True, 0, 0, width, height)
                pixmap.draw_arc(black_gc, True, dx, dy, width - dx * 2, height - dy * 2, 0, 23040)
        ...
        def __setup_connections__(self):
            ...
            if not self.__class__.signal_created:
                gobject.signal_new('follow-link', RichText.Renderer, gobject.SIGNAL_RUN_LAST, gobject.TYPE_NONE, (
            ...
            def visibility_notify_event(text_view, event):
                ...
                (x, y) = text_view.window_to_buffer_coords(gtk.TEXT_WINDOW_WIDGET, wx, wy)
                set_cursor_if_appropriate(text_view, x, y)
                return False
        ...
        def __setup_render__(self):
            ...
            self.tags = {'default': create_tag('default', **k), 'bold': create_tag('bold', weight=pango.WEIGHT_BOLD), 'italic': create_tag('italic', style=pango.STYLE_ITALIC), 'link': create_tag('link', foreground=self.link_color, underline=pango.UNDERLINE_SINGLE), 'h1': create_tag('h1', scale=pango.SCALE_XX_LARGE), 'h2': create_tag('h2', scale=pango.SCALE_X_LARGE), 'h3': create_tag('h3', scale=pango.SCALE_LARGE), 'monospaced': create_tag('monospaced', font='monospace')}
        ...
        def send_literal_data(self, data):
            itr = self.buffer.get_end_iter()
            t = [self.tags['default'], self.tags['monospaced']] + self.font + self.link
            self.buffer.insert_with_tags(itr, data, *t)
        ...
        def start_link(self, url, name=None):
            ...
            t.set_data('href', url)
            if name:
                self.buffer.create_mark(name, self.buffer.get_end_iter(), True)
            self.link = [
             self.tags['link'], t]
    ...
    def __init__(self, id, text='', label=None, link_color='blue', fgcolor=None, bgcolor=None, callback=None, img_provider=None, expand_policy=None):
    ...
    def __setup_gui__(self):
        ...
        self._sw = gtk.ScrolledWindow()
        self._renderer = RichText.Renderer(link_color=self.link_color, foreground=self.fgcolor, background=self.bgcolor, resource_provider=img_provider)
        self._sw.set_border_width(self.padding)
        self._sw.set_shadow_type(gtk.SHADOW_IN)
        if self.label is not None:
            self._frame = gtk.Frame(self.label)
    ...
    def __str__(self):
        return '%s( id=%r, label=%r, link_color=%r, fgcolor=%r, bgcolor=%r )' % (self.__class__.__name__, self.id, self.label, self.link_color, self.fgcolor, self.bgcolor)
...
class Button(_EGWidget):
    ...
    callback = _gen_ro_property('callback')
    stock_items = (
     'about', 'help', 'quit', 'add', 'remove', 'refresh', 'update', 'yes', 'no', 'zoom_100', 'zoom_in', 'zoom_out', 'zoom_fit', 'undo', 'execute', 'stop', 'open', 'save', 'save_as', 'properties', 'preferences', 'print', 'print_preview', 'ok', 'cancel', 'apply', 'close', 'clear', 'convert', 'next', 'back', 'up', 'down', 'font', 'color', 'media:play', 'media:pause', 'media:stop', 'media:previous', 'media:next', 'media:forward', 'media:rewind')
    _gtk_stock_map = {'about': gtk.STOCK_ABOUT, 'help': gtk.STOCK_HELP, 'quit': gtk.STOCK_QUIT, 'add': gtk.STOCK_ADD, 'remove': gtk.STOCK_REMOVE, 'refresh': gtk.STOCK_REFRESH, 'update': gtk.STOCK_REFRESH, 'yes': gtk.STOCK_YES, 'no': gtk.STOCK_NO, 'zoom_100': gtk.STOCK_ZOOM_100, 'zoom_in': gtk.STOCK_ZOOM_IN, 'zoom_out': gtk.STOCK_ZOOM_OUT, 'zoom_fit': gtk.STOCK_ZOOM_FIT, 'undo': gtk.STOCK_UNDO, 'execute': gtk.STOCK_EXECUTE, 'stop': gtk.STOCK_STOP, 'open': gtk.STOCK_OPEN, 'save': gtk.STOCK_SAVE, 'save_as': gtk.STOCK_SAVE_AS, 'properties': gtk.STOCK_PROPERTIES, 'preferences': gtk.STOCK_PREFERENCES, 'print': gtk.STOCK_PRINT, 'print_preview': gtk.STOCK_PRINT_PREVIEW, 'ok': gtk.STOCK_OK, 'cancel': gtk.STOCK_CANCEL, 'apply': gtk.STOCK_APPLY, 'close': gtk.STOCK_CLOSE, 'clear': gtk.STOCK_CLEAR, 'convert': gtk.STOCK_CONVERT, 'next': gtk.STOCK_GO_FORWARD, 'back': gtk.STOCK_GO_BACK, 'up': gtk.STOCK_GO_UP, 'down': gtk.STOCK_GO_DOWN, 'font': gtk.STOCK_SELECT_FONT, 'color': gtk.STOCK_SELECT_COLOR, 'media:play': gtk.STOCK_MEDIA_PLAY, 'media:pause': gtk.STOCK_MEDIA_PAUSE, 'media:stop': gtk.STOCK_MEDIA_STOP, 'media:previous': gtk.STOCK_MEDIA_PREVIOUS, 'media:next': gtk.STOCK_MEDIA_NEXT, 'media:forward': gtk.STOCK_MEDIA_FORWARD, 'media:rewind': gtk.STOCK_MEDIA_REWIND}

    def __init__(self, id, label='', stock=None, callback=None, expand_policy=None):
        ...
        for i in self.stock_items:
            if i not in self._gtk_stock_map:
                print >> sys.stderr, 'Stock item %s missing in implementation map!' % (i,)
...
class AboutButton(Button, AutoGenId):
    ...
    def __init__(self, id=None, expand_policy=None):
        ...
        Button.__init__(self, id or self.__get_id__(), stock='about', callback=show_about, expand_policy=expand_policy)
...
class CloseButton(Button, AutoGenId):
    ...
    def __init__(self, id=None, expand_policy=None):
        ...
        Button.__init__(self, id or self.__get_id__(), stock='close', callback=close, expand_policy=expand_policy)
...
class QuitButton(Button, AutoGenId):
    ...
    def __init__(self, id=None, expand_policy=None):
        ...
        Button.__init__(self, id or self.__get_id__(), stock='quit', callback=c, expand_policy=expand_policy)
...
class HelpButton(Button, AutoGenId):
    ...
    def __init__(self, id=None, expand_policy=None):
        ...
        Button.__init__(self, id or self.__get_id__(), stock='help', callback=c, expand_policy=expand_policy)
...
class OpenFileButton(Button, AutoGenId):
    ...
    def __init__(self, id=None, filter=None, multiple=False, callback=None, expand_policy=None):
        """Constructor.
        ...
        def c(app_id, wid_id):
            f = self.app.file_chooser(FileChooser.ACTION_OPEN, filter=filter, multiple=multiple)
            if f is not None and callback:
                callback(self.app, self, f)
            return

        Button.__init__(self, id or self.__get_id__(), stock='open', callback=c, expand_policy=expand_policy)
...
class SaveFileButton(Button, AutoGenId):
    ...
    def __init__(self, id=None, filename=None, filter=None, callback=None, expand_policy=None):
        """Constructor.
        ...
        def c(app_id, wid_id):
            f = self.app.file_chooser(FileChooser.ACTION_SAVE, filename=filename, filter=filter)
            if f is not None and callback:
                callback(self.app, self, f)
            return

        Button.__init__(self, id or self.__get_id__(), stock='save', callback=c, expand_policy=expand_policy)
...
class PreferencesButton(Button, AutoGenId):
    ...
    def __init__(self, id=None, expand_policy=None):
        ...
        Button.__init__(self, id or self.__get_id__(), stock='preferences', callback=c)
...
class HSeparator(_EGWidget, AutoGenId):
    ...
    def __init__(self, id=None, expand_policy=None):
        """You may not provide id, it will be generated automatically"""
        _EGWidget.__init__(self, id or self.__get_id__(), expand_policy=expand_policy)
...
class Label(_EGDataWidget, AutoGenId):
    ...
    def __init__(self, id=None, label='', halignment=LEFT, valignment=MIDDLE, expand_policy=None):
        """Label constructor.
        ...
        _EGDataWidget.__init__(self, id or self.__get_id__(), False, expand_policy=expand_policy)
        self.label = label
    ...
    def __str__(self):
        return '%s( id=%r, label=%r )' % (self.__class__.__name__, self.id, self.label)


def information(message):
    """Show info message to user."""
    d = gtk.MessageDialog(type=gtk.MESSAGE_INFO, message_format=message, buttons=gtk.BUTTONS_CLOSE)
    d.run()
    d.destroy()
...
def warning(message):
    ...
    d = gtk.MessageDialog(type=gtk.MESSAGE_WARNING, message_format=message, buttons=gtk.BUTTONS_CLOSE)
    d.run()
    d.destroy()
...
def error(message):
    ...
    d = gtk.MessageDialog(type=gtk.MESSAGE_ERROR, message_format=message, buttons=gtk.BUTTONS_CLOSE)
    d.run()
    d.destroy()
...
def yesno(message, yesdefault=False):
    ...
    """
    d = gtk.MessageDialog(type=gtk.MESSAGE_QUESTION, message_format=message, buttons=gtk.BUTTONS_YES_NO)
    if yesdefault:
...
def confirm(message, okdefault=False):
    ...
    """
    d = gtk.MessageDialog(type=gtk.MESSAGE_QUESTION, message_format=message, buttons=gtk.BUTTONS_OK_CANCEL)
    if okdefault:
...
def get_app_by_id(app_id):
    ...
    if app_id is None:
        ...
        try:
            ...
            raise ValueError('Application id "%s" doesn\'t exists!' % app_id)
