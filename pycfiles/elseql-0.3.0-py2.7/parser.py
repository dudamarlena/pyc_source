...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/elseql/parser.py
# Compiled at: 2018-02-06 19:56:37
from __future__ import print_function
from pyparsing import Optional, Word, delimitedList, Group, quotedString, nums, Empty, removeQuotes, Combine, Suppress, CaselessLiteral, oneOf, operatorPrecedence, opAssoc, alphas, alphanums, CaselessKeyword, Forward, ParseBaseException, ParseException, ParseFatalException
...
class BinaryOperator(Operator):
    ...
    def __str__(self):
        if self.name == '=':
            ...
            if self.name in ('<=', 'LTE', 'LE'):
                return '%s:[* TO %s]' % (self.operands[0], self.op(1))
            if self.name in ('>=', 'GTE', 'GE'):
...
class ElseParser(object):
    ...
    realNum = Combine(Optional(arithSign) + (Word(nums) + '.' + Optional(Word(nums)) | '.' + Word(nums)) + Optional(E + Optional(arithSign) + Word(nums))).setParseAction(makeAtomObject(floatValue))
    intNum = Combine(Optional(arithSign) + Word(nums) + Optional(E + Optional('+') + Word(nums))).setParseAction(makeAtomObject(intValue))
    ...
    whereCondition = (columnName + binop + columnRval).setParseAction(makeGroupObject(BinaryOperator)) | (columnName + in_.suppress() + lpar + delimitedList(columnRval) + rpar).setParseAction(makeGroupObject(InOperator)) | (columnName + between.suppress() + columnRval + and_.suppress() + columnRval).setParseAction(makeGroupObject(BetweenOperator)) | (columnName + likeop.suppress() + likeExpr).setParseAction(makeGroupObject(LikeOperator)) | Empty().setParseAction(invalidSyntax)
    boolOperand = whereCondition | boolean
    whereExpression = quotedString.setParseAction(removeQuotes) | operatorPrecedence(boolOperand, [
     (
    ...
    filterExpression = (queryToken.suppress() + whereExpression.setResultsName('query')).setParseAction(makeGroupObject(QueryFilter)) | (existToken.suppress() + columnName).setParseAction(makeGroupObject(ExistFilter)) | (missingToken.suppress() + columnName).setParseAction(makeGroupObject(MissingFilter))
    ...
    selectStmt << selectToken + selectExpr.setResultsName('fields') + Optional(facetToken + facetExpr.setResultsName('facets')) + Optional(scriptToken + scriptExpr.setResultsName('script')) + fromToken + indexName.setResultsName('index') + Optional(whereToken + whereExpression.setResultsName('query')) + Optional(filterToken + filterExpression.setResultsName('filter')) + Optional(orderbyToken + orderList.setResultsName('order')) + Optional(limitToken + Group(Optional(limitoffset + comma) + limitcount).setResultsName('limit')) + Optional(routingToken + routingExpr.setResultsName('routing'))
