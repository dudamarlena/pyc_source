...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/fits_align/quad.py
# Compiled at: 2018-08-20 07:58:14
...
class Quad:
    ...
    def __init__(self, fourstars):
        ...
        if not len(fourstars) == 4:
            ...
            Ai, Bi = tests[maxindex]
            Ci, Di = other[maxindex]
            A = fourstars[Ai]
            B = fourstars[Bi]
            C = fourstars[Ci]
            D = fourstars[Di]
            ...
            if testb:
                if not testa:
                    self.hash = (
                     1.0 - xD, 1.0 - yD, 1.0 - xC, 1.0 - yC)
                    A, B = B, A
                    C, D = D, C
            if testa:
                if testb:
                    self.hash = (
                     1.0 - xC, 1.0 - yC, 1.0 - xD, 1.0 - yD)
                    A, B = B, A
...
def proposecands(uknquadlist, refquadlist, n=5, verbose=True):
    ...
    if len(uknquadlist) == 0 or len(refquadlist) == 0:
        ...
        dists = cdist_np(refhashs, uknhashs)
        uknmindistindexes = np.argmin(dists, axis=0)
        uknmindist = np.min(dists, axis=0)
        ...
        for i in range(min(n, nmax)):
            cand = {'uknquad':uknquadlist[uknbestindexes[i]],  'refquad':refquadlist[uknmindistindexes[uknbestindexes[i]]],  'dist':uknmindist[uknbestindexes[i]]}
