...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/fits_align/star.py
# Compiled at: 2018-11-14 07:41:02
# Size of source mod 2**32: 13479 bytes
"""
...
import sys, os, math, numpy as np, operator, copy, itertools
...
def readsexcat(mycat):
    ...
    if len(mycat) == 0:
        ...
        for i, mc in enumerate(mycat):
            flux = mycat['FLUX'][i]
            fwhm = mycat['FWHM'][i]
            newstar = Star(x=(mycat['X'][i]), y=(mycat['Y'][i]), name=(str(i)), flux=flux, fwhm=(mycat['FWHM'][i]),
              elon=(mycat['ELLIPTICITY'][i]))
...
class SimpleTransform:
    ...
    def getrotation(self):
        ...
        """
        return math.atan2(self.v[1], self.v[0]) * (180.0 / math.pi)
    ...
    def matrixform(self):
        ...
        """
        return (
         np.array([[self.v[0], -self.v[1]], [self.v[1], self.v[0]]]), self.v[2:4])
...
def fitstars(uknstars, refstars, verbose=True):
    ...
    if len(uknstars) < 2:
        ...
        ref = np.hstack(listtoarray(refstars))
        uknlist = []
...
def identify(uknstars, refstars, trans=None, r=5.0, getstars=False):
    ...
    dists = cdist_np(ukn, ref)
    mindists = np.min(dists, axis=1)
    minok = mindists <= r
    minokindexes = np.argwhere(minok).flatten()
