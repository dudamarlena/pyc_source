...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-intel/egg/tornadoblueprint/blueprint.py
# Compiled at: 2018-02-27 23:49:34
"""Tornado Blueprint蓝图的实现。"""
import re, functools, threading, six
from tornado import web
__all__ = [
 'Blueprint', 'wraps', 'url_for']
__version__ = '0.2.8'
...
class BlueprintMeta(type):
    ...
    def __new__(metacls, cls_name, bases, namespace):
        _class = super(BlueprintMeta, metacls).__new__(metacls, cls_name, bases, namespace)
    ...
    def get_plugged_in_blueprints(metacls):
        ...
        for _class in metacls.derived_class:
            blueprints.extend([ x for x in _class.blueprints if x.enabled is True ])
...
_REGEXIES = (
 (
  re.compile('<int:.+>'), '([-+]?\\d+)'),
 (
  re.compile('<float:.+>'), '(\\d*\\.?\\d+)'),
 (
  re.compile('<uuid:.+>'), '((?:[0-9a-fA-F]+-){4}[0-9a-fA-F]+)'))
_HTTPMETHODS = ('GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'HEAD', 'OPTIONS')
_HANDLERS = [ (x, getattr(web.RequestHandler, x.lower())) for x in _HTTPMETHODS ]
...
class Blueprint(object):
    ...
    def _get_handler_name(self, handler):
        if hasattr(handler, '__endpoint__'):
    ...
    def route(self, uri, params=None, name=None, methods=('GET', )):
        ...
        def decorator(handler):
            ...
            self.rules.append((self.prefix + internal_uri, handler, params, rule_name))
    ...
    def get_route_rules(self):
        return (
         self.host, self.rules)


def wraps(app):
    if hasattr(_state, '__app__'):
        return getattr(_state, '__app__')
    assert hasattr(app, 'add_handlers') and hasattr(app.add_handlers, '__call__')
