...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/valtioneuvosto_scraper/scraping.py
...
def parse_cabinet(html):
    ...
    for row in rows:
        try:
            ...
            data = row.findAll('td')
            season = (
             data[3].text, data[0].text, data[1].span.text, data[2].span.text)
...
def parse_governments(html):
    ...
    for row in data:
        ...
        govt = (
         id, name, pmname, party, order, begun, ended, duration, govt_type, infourl)
        govts.append(govt)
