...
# [GCC 8.4.0]
# Embedded file name: /home/travis/virtualenv/python3.6.7/lib/python3.6/site-packages/satella/coding/structures/structures.py
...
class ReprableObject:
    ...
    def __init__(self, *args, **kwargs):
        self._ReprableObject__args = args
        self._ReprableObject__kwargs = kwargs

    def __repr__(self):
        fragments = [
         self.__class__.__name__, '(']
        arguments = []
        for arg in self._ReprableObject__args:
            arguments.append(repr(arg))

        fragments.append(', '.join(arguments))
        kwargs = []
        for k, v in self._ReprableObject__kwargs:
...
class TimeBasedHeap(Heap):
 ...
 T, None, None)]:
        ...
        elif not less is not None:
            raise AssertionError('Default clock source returned None!')
        while self:
