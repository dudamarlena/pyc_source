...
# [GCC 8.4.0]
# Embedded file name: /home/travis/virtualenv/python3.6.7/lib/python3.6/site-packages/satella/coding/metaclasses.py
...
import typing as tp
__all__ = [
 'metaclass_maker', 'wrap_with', 'dont_wrap', 'wrap_property']
...
def wrap_property(getter: tp.Callable[([GetterDefinition], GetterDefinition)]=lambda x: x, setter: tp.Callable[([SetterDefinition], SetterDefinition)]=lambda x: x, deleter: tp.Callable[([DeleterDefinition], DeleterDefinition)]=lambda x: x):
    """
...
def wrap_with(callables: tp.Callable[([tp.Callable], tp.Callable)]=lambda x: x, properties: tp.Callable[([property], property)]=lambda x: x, selector_callables: tp.Callable[([tp.Callable], bool)]=lambda clb: True, selector_properties: tp.Callable[([property], bool)]=lambda clb: True):
    """
    ...
    def WrapAllMethodsWithMetaclass(name, bases, dct):
        ...
        for key, value in dct.items():
            if not hasattr(value, '_dont_wrap'):
                if callable(value):
                    if selector_callables(value):
                        value = callables(value)
                if isinstance(value, property):
                    if selector_properties(value):
