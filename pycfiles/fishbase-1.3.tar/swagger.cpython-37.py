...
# [GCC 8.4.0]
# Embedded file name: /Users/itaa/Documents/MYCode/fish_base/test/../fishbase/swagger/swagger.py
# Compiled at: 2019-12-05 01:38:06
...
_swagger_json = {}
_swagger_config = {}
swagger_blueprint = Blueprint(name='swagger', import_name=__name__, url_prefix='/swagger', template_folder=template_path,
...
class SwaggerHelper:
    ...
    def get_api_method(rule):
        ...
        """
        method_option = [
         'delete', 'put', 'patch', 'post', 'get', 'options', 'head']
    ...
    def consumes_content_types(app, _rule_specs):
        ...
        consumes_content_types = _rule_specs.consumes_content_type or api_consumes_content_types
    ...
    def produces_content_types(app, _rule_specs):
        ...
        produces_content_types = _rule_specs.produces_content_type or api_produces_content_types
        ...
        for consumer in _rule_specs.consumes:
            ...
            if 'properties' in spec:
                for name, prop_spec in spec['properties'].items():
                    route_param = {**prop_spec, **{'required':consumer.required, 
                     'in':consumer.location, 
                     'name':name}}

            else:
                route_param = {**spec, **{'required':consumer.required, 
                 'in':consumer.location, 
                 'name':'body'}}
    ...
    def get_responses(_rule_specs):
        ...
        if len(_rule_specs.response) == 0:
            responses['200'] = {'schema':serialize_schema(_rule_specs.produces.field) if _rule_specs.produces else None, 
             'description':_rule_specs.produces.description if _rule_specs.produces else None}
        for status_code, routefield in _rule_specs.response:
            responses['{}'.format(status_code)] = {'schema':serialize_schema(routefield.field), 
...
def flask_swagger(app):
    ...
    _swagger_json['info'] = {'version':app.config.get('API_VERSION', '1.0.0'), 
     ...
     'license':{'name':app.config.get('API_LICENSE_NAME', None), 
    ...
    _swagger_json['tags'] = [
     SwaggerHelper.app_name(app) or 'Test']
    _swagger_json['paths'] = {}
    for rule in app.url_map._rules:
        _endpoint = rule.endpoint
        _method = SwaggerHelper.get_api_method(rule)
        _view_func = app.view_functions.get(_endpoint)
        _rule_specs = route_specs.get(_view_func)
        if not _rule_specs:
            continue
        body = {_method: {'operationId':_rule_specs.operation, 
    ...
    _swagger_json['definitions'] = {obj.object_name:definition for obj, definition in definitions.values()}
