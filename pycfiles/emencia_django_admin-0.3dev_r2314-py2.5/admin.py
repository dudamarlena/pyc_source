...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/emencia_django_admin/admin.py
...
def delete(request, app_label, model_name):
    ...
    try:
        cl = ChangeList(request, admin_obj.model, admin_obj.list_display, admin_obj.list_display_links, admin_obj.list_filter, admin_obj.date_hierarchy, admin_obj.search_fields, admin_obj.list_select_related, admin_obj.list_per_page, admin_obj)
...
class CSVForm(forms.Form):
    ...
    import_mode = forms.ChoiceField(choices=[('change', _('Add or replace')),
     ...
     (
      'test', _('Test')),
     (
      'delete', _('Delete'))], widget=forms.RadioSelect)
...
def csv_form(request, app_label, model_name):
    ...
    if request.POST:
        ...
        form = CSVForm(initial={'delimiter': ':', 'encoding': 'latin1', 
           'import_mode': 'change', 
           'skip_first': True})
    input = []
    if request.FILES:
    ...
    return render_to_response('admin/import_form.html', {'title': _('CSV Import / Export'), 'fields': fields, 
       'output': output, 
       'input': input, 
       'form': form, 
       'cl': ChangeList(request, admin_obj.model, admin_obj.list_display, admin_obj.list_display_links, admin_obj.list_filter, admin_obj.date_hierarchy, admin_obj.search_fields, admin_obj.list_select_related, admin_obj.list_per_page, admin_obj), 
       'model_name': model.__name__, 
       'app_label': app_label}, context_instance=RequestContext(request))
