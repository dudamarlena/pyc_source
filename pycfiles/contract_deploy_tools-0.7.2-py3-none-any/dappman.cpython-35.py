...
# Embedded file name: build/bdist.linux-x86_64/egg/contractvmd/dappman.py
# Compiled at: 2015-11-06 08:17:45
# Size of source mod 2**32: 9103 bytes
import shutil, json, sys, os, getopt, requests, urllib
        ...
        if opt in ('-h', '--help'):
            ...
            if opt in ('-r', '--remove'):
                ...
                if opt in ('-i', '--install'):
                    ...
                    if manifest['name'].lower() not in conf['dapps']['list']:
                        conf['dapps']['list'].append(manifest['name'].lower())
                    if manifest['name'].lower() not in conf['dapps']['enabled']:
