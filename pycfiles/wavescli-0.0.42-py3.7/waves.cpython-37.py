...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/wavescli/waves.py
# Compiled at: 2020-04-24 09:34:39
# Size of source mod 2**32: 13478 bytes
import tempfile, os, shutil, logging, logging.config
...
app.conf.task_queues = (
 Queue((config.QUEUE_NAME), (Exchange(config.QUEUE_NAME)),
...
class WavesLogger(object):
    ...
    def info(self, message):
        ...
        self._logger.info(message, extra={'execution':execution, 
         'task':task_id})
...
def send_update_execution(identifier, task_id, task_name, task_root_id, task_parent_id, status=None, inputs=None, args=None, results=None, last_error=None, hostname=None):
    """
    ...
    params = (
     ...
     last_error,
     hostname)
    sig_status = app.signature('awebrunner.update_execution',
      args=params,
      kwargs={},
      queue='celery')
...
def task_updated(sender=None, headers=None, body=None, **kwargs):
    ...
    send_update_execution(identifier=identifier,
      task_id=task_id,
      task_name=task_name,
    ...
    if type(inputs) == dict:
        ...
        if type(results) == dict:
            ...
            if type(results) == list:
                inputs = {'items': results}
    return inputs


class WavesBaseTask(BaseTask):
    __doc__ = 'Abstract base class for all tasks in my app.'
    abstract = True
    ...
    def _download_inputs(self):
        ...
        for item in self.inputs_values.keys():
            ...
            if item not in self.auto_download:
                continue
            try:
                local_file = get_file(self.inputs_values[item], self.inputs_dir)
            ...
            try:
                if item.startswith('items.') and 'items' in outputs:
                    for subitem_dict in outputs['items']:
                        ...
                        remote_file = send_file(local_file_path, self.bucket, remote_path)
    ...
    def _update_execution(self, identifier, task_id, inputs=None, params=None, result=None, status=None, last_error=None):
        if not status:
    ...
    def on_failure(self, exc, task_id, args, kwargs, einfo):
        ...
        self._update_execution((kwargs['identifier']),
          task_id, status='FAILURE', last_error=error)
    ...
    def folder_to_zip(self, source_folder, compress_format='zip'):
        ...
        return shutil.make_archive((str(source_folder)),
          compress_format,
    ...
    def on_success(self, retval, task_id, args, kwargs):
        ...
        if not hasattr(self, 'on_success_updated'):
            self._update_execution((kwargs['identifier']),
              (self.request.id), result=results)
