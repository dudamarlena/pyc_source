...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/wavescli/apiclient.py
# Compiled at: 2020-04-24 10:22:57
# Size of source mod 2**32: 5389 bytes
...
class ApiClient(object):
    ...
    def execution(self, identifier=None):
        if not identifier:
            ...
            response = requests.get(url,
              headers=(self._get_headers()))
            return response.status_code == 200 or 'Erro: {}: {}'.format(response, response.content)
    ...
    def create(self, inputs=None):
        if self._execution:
            if not inputs:
                return self.run(workflow_identifier=(self._workflow['identifier']),
                  inputs=(self._inputs))
        if self._workflow:
            if inputs:
                return self.run(workflow_identifier=(self._workflow['identifier']),
    ...
    def publish_businesstask(self, btask_content):
        ...
        identifier = '{}@{}'.format(definition.get('name'), definition.get('version', 'latest'))
    ...
    def run(self, workflow_identifier, inputs):
        ...
        new_execution = {'workflow_identifier':workflow_identifier, 
         'inputs':inputs}
        response = requests.post(url,
    ...
    def _get_headers(self, content_type='application/x-yaml'):
        headers = {'User-Agent':'wavescli/{}'.format(__version__), 
...
class ApiError(RuntimeError):
    ...
    def __init__(self, response):
        message = 'status={} data={}'.format(response.status_code, response.json())
