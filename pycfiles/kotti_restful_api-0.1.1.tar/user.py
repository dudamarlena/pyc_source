...
# [GCC 8.4.0]
# Embedded file name: /home/ubuntu/workspace/kotti_restful_api/kotti_restful_api/views/user.py
# Compiled at: 2016-12-20 13:40:33
"""
Created on 2016-01-13
:author: Alteroo Team (oshane@alteroo.com)
"""
import io, os, re, json, csv, copy, time, deform, urllib, shutil, zipfile, logging, mimetypes, subprocess, transaction
...
class UserViews(BaseView):
    ...
    def get_key_token(self, user):
        ...
        skip_url = ('{}-tst-{}-tst-{}').format(token, user.email, ('s{}w').format(datetime.now().strftime('%d%m%y%H')))
        return ('{0}/@@login?ketoken={1}').format(self.request.application_url, skip_url)

    @view_config(name='login-status', renderer='json', root_only=True)
    ...
    @view_config(name='login', renderer='json', root_only=True, request_method='POST', content_type='application/json')
    def json_login(self):
        return self.login()

    @view_config(name='login', renderer='json', root_only=True, cross_request='restful', request_method='POST')
    ...
    def login(self):
        ...
        if login and password:
            ...
            if principals.validate_password(password, user.password):
                ...
                return httpexc.HTTPFound(location='/@@login-status?from_login=1', headers=headers)
            return {'status': 'failed', 
