...
# [GCC 8.4.0]
# Embedded file name: /Users/mando/neurospaces_project/sspy/source/snapshots/0/build/lib/sspy/__init__.py
...
class SSPy:
    ...
    def __init__(self, name='Untitled', verbose=False, solver_directory=os.path.join(_module_directory, 'plugins', 'solvers'), service_directory=os.path.join(_module_directory, 'plugins', 'services'), input_directory=os.path.join(_module_directory, 'plugins', 'inputs'), output_directory=os.path.join(_module_directory, 'plugins', 'outputs')):
    ...
    def GetVerbosityLevel(self):
        ...
        if self.verbose and self.simulation_verbose:
            ...
            if self.verbose and not self.simulation_verbose:
                return 1
            if not self.verbose and self.simulation_verbose:
                return 1
            return 0
    ...
    def _PrintPlugin(self, p=None, verbose=False):
        if p is None:
            ...
            print '%s' % p.GetName()
            if verbose:
                if p.GetLabel() is not None:
                    print 'label: %s' % p.GetLabel()
                if p.GetVersion() is not None:
                    print 'version: %s' % p.GetVersion()
                if p.GetDescription() is not None:
                    print 'description: %s' % p.GetDescription()
                if p.GetFormat() is not None:
                    print 'format:\n\n%s' % p.GetFormat()
                print ''
            return
    ...
    def _PrintOutputObject(self, o=None, verbose=False):
        if o is None:
            ...
            if verbose:
                ...
                if o.GetFilename() is not None:
                    print 'filename: %s' % o.GetFilename()
                try:
                    if o.format is not None:
                        print 'format: %s' % o.format
                    if o.mode is not None:
                        print 'mode: %d' % o.mode
                    if o.resolution is not None:
                        print 'resolution: %d' % o.resolution
                except Exception, e:
                    print e
                else:
                    try:
    ...
    def Dump(self):
        ...
        """
        try:
            data = yaml.dump(self._schedule_data, allow_unicode=True)
    ...
    def ConnectOutputs(self):
        ...
        for o in self._output_parameters:
            self.ConnectOutputParameter(o['path'], o['parameter'], o['output_name'], o['output_type'])
    ...
    def CreateService(self, name='default_model_container', type=None, arguments=None, verbose=False):
        ...
        """
        service = self._service_registry.CreateService(name, type, arguments)
    ...
    def AddOutput(self, path, parameter, output_name=None, output_type=None):
        ...
        """
        output_data = dict(path=path, parameter=parameter, output_name=output_name, output_type=output_type)
            ...
            if data.has_key('initializers'):
                ...
                for i in initializers:
                    ...
                    for arg in arguments:
                        try:
                            service = self._service_registry.CreateService(service_name, service_type, arg)
                        except Exception, e:
    ...
    def _ParseOutputs(self, output_data, output_parameters=None):
        ...
        if output_parameters is not None:
            for o in output_parameters:
                self.AddOutput(path=o['component_name'], parameter=o['field'], output_type=o['outputclass'])
