...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/ytsub/api.py
...
class _UploadPlaylist:
    ...
    def __init__(self, for_channel, channel_list_response):
        ...
        self.playlist_id = channel_list_response['items'][0]['contentDetails']['relatedPlaylists']['uploads']


def get_user_playlists(youtube, credentials):
    return youtube.channels().list(mine=True, part='contentDetails').execute()['items'][0]['contentDetails']['relatedPlaylists']


def get_watched_ids(youtube, credentials):
    ret = []
    id_watch_history_playlist = get_user_playlists(youtube, credentials)['watchHistory']
    query = youtube.playlistItems().list(playlistId=id_watch_history_playlist, part='id', maxResults=50, fields='items(id,kind),nextPageToken')
...
def get_updated_channels(youtube, credentials):
    ...
    while next_page_token is not None:
        sub_channels_response = youtube.subscriptions().list(mine=True, part='snippet', maxResults=50, order='unread', fields='items/snippet,nextPageToken').execute()
...
def channel_playlists_query(youtube, credentials, channel):
    return batch.Query(youtube.channels().list, {'id': channel.channel_id, 'part': 'contentDetails', 
       'fields': 'items/contentDetails'}, limit=batch.QueryLimitCount(1))
...
def get_videos_in_playlists(youtube, credentials, playlist_list, MAX_VIDS, MAX_AGE):
    ...
    for playlist in playlist_list:
        query = batch.Query(youtube.playlistItems().list, {'playlistId': playlist.playlist_id, 'part': 'snippet'}, limit=(
         batch.QueryLimitCount(MAX_VIDS),
...
def mark_watched(youtube, credentials, vids):
    ...
    for vid in vids:
        youtube.playlistItems().insert(part='snippet', body={'snippet': {'playlistId': history_playlist, 'resourceId': {'videoId': vid, 'kind': 'youtube#video'}}}).execute()