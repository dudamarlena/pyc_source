...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/axon_conabio/management/train.py
...
def train(path, config, project, retrain=False):
    ...
    model_klass = load_object(architecture_name, 'architecture', project=project, config=config)
    dataset_klass = load_object(dataset_name, 'dataset', project=project, config=config)
    loss_klass = load_object(loss_name, 'loss', project=project, config=config)
    train_config = get_config(paths=paths)
    trainer = TFTrainer(train_config, path, retrain=retrain)
    trainer.train(model=model_klass, dataset=dataset_klass, loss=loss_klass, model_kwargs=model_kwargs, dataset_kwargs=dataset_kwargs, loss_kwargs=loss_kwargs)
    return