...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/axon_conabio/utils.py
# Compiled at: 2018-12-10 18:39:29
from contextlib import contextmanager
import os, collections, functools, yaml, logging, tensorflow as tf
logger = logging.getLogger(__name__)
TF_DTYPES = {'float': tf.float32, 
   'float16': tf.float16, 
   'float32': tf.float32, 
   'float64': tf.float64, 
...
def summary_scope(storage):
    ...
    tf.summary.image = collection_scope(tf.summary.image, storage)
    tf.summary.scalar = collection_scope(tf.summary.scalar, storage)
    tf.summary.audio = collection_scope(tf.summary.audio, storage)
    tf.summary.histogram = collection_scope(tf.summary.histogram, storage)
    tf.summary.tensor_summary = collection_scope(tf.summary.tensor_summary, storage)
    yield
...
def summary_aggregation(summaries, prefix=None):
    ...
    for key in summaries:
        ...
        global_arguments = [
         name, aggregated_tensors] + list(global_arguments)
        summary = summary_function(*global_arguments, **global_kwargs)
...
def get_checkpoints(summary_dir, tf_subdir='tensorflow', npy_subdir='numpy'):
    tf_step = None
    npy_step = None
    ...
    npy_ckpts = [ x for x in os.listdir(npy_dir) if x[-4:] == '.npz'
                ]
    if npy_ckpts:
        npy_ckpts = sorted(npy_ckpts, key=lambda x: int(x.split('.')[0].split('_')[(-1)]))
