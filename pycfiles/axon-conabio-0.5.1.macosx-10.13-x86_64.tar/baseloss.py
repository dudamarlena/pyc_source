...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/axon_conabio/losses/baseloss.py
# Compiled at: 2018-12-10 18:39:09
from abc import ABCMeta, abstractmethod
import uuid, six, tensorflow as tf
...
class Loss(object):
    ...
    def build_model_loss(self, model, inputs, labels, num_gpus=1, run_name=None):
        ...
        if num_gpus == 1:
            ...
            for i in range(num_gpus):
                with tf.device(('/device:GPU:{}').format(i)):
                    model_outputs = predict(split_inputs[i], run_name=run_name, sub_run=('sub_run_{}').format(i))
                    loss = self.build_single_loss(model_outputs, split_labels[i])
                    losses.append(loss)
