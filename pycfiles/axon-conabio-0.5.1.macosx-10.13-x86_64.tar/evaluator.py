...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/axon_conabio/evaluator/evaluator.py
# Compiled at: 2018-12-10 18:39:29
import os, sys, logging, json, csv, six
...
class Evaluator(object):
    ...
    def __init__(self, config, path, ckpt=None):
        ...
        try:
            ckpt_type, ckpt_path, ckpt_step = get_model_checkpoint(os.path.basename(path), ckpt=ckpt)
            self._ckpt_type = ckpt_type
            self._ckpt_path = ckpt_path
            self._ckpt_step = ckpt_step
        except (IOError, RuntimeError):
            self.logger.warning('No checkpoint was found', extra={'phase': 'construction'})
    ...
    def _build_inputs(self, model):
        ...
        inputs = {key:tf.placeholder(get_dtype(value), shape=get_shape(value)) for key, value in six.iteritems(input_structure)}
        if len(inputs) == 1:
    ...
    def _save_evaluations(self, evaluations, name=''):
        path = os.path.join(self.evaluations_dir, ('evaluation_{}_step_{}').format(name, self._ckpt_step))
    ...
    def _evaluate_tf(self, model=None, model_kwargs=None, dataset=None, dataset_kwargs=None, metrics=None, name=None):
        ...
        self.logger.info('Building model and dataset', extra={'phase': 'construction'})
        graph = tf.Graph()
        ...
        self.logger.info('Starting session and restoring model', extra={'phase': 'construction'})
        ...
        model_instance.restore(sess, path=self._ckpt_path, mode=self._ckpt_type)
        self.logger.info('Starting evaluation', extra={'phase': 'construction'})
        evaluations = []
        pbar = tqdm()
        try:
            while True:
            ...
            self.logger.info('Iterations over', extra={'phase': 'evaluations'})

        if bool(self.config['evaluations']['save_results']):
            self.logger.info('Saving results', extra={'phase': 'saving'})
            self._save_evaluations(evaluations, name=name)
        pbar.close()
        return evaluations

    def _evaluate_no_tf(self, model=None, dataset=None, metrics=None, dataset_kwargs=None, model_kwargs=None, name=None):
        assert issubclass(model, Model)
        assert issubclass(dataset, Dataset)
        assert isinstance(metrics, (list, tuple))
        for metric in metrics:
        ...
        for id_, inputs, label in tqdm(dataset_instance.iter_test()):
            try:
                ...
                self.logger.warning(str(exc), extra={'phase': 'evaluation'})
    ...
    def evaluate(self, model=None, dataset=None, metrics=None, name=None, dataset_kwargs=None):
        fmt = self.config['evaluations']['results_format']
        filepath = os.path.join(self.evaluations_dir, ('evaluation_{}_step_{}.{}').format(name, self._ckpt_step, fmt))
        ...
        if is_tf:
            self.logger.info('Tensorflow dataset detected.', extra={'phase': 'construction'})
            evaluations = self._evaluate_tf(model=model, dataset=dataset, metrics=metrics, name=name, dataset_kwargs=dataset_kwargs)
        else:
            self.logger.info('Iterable dataset detected.', extra={'phase': 'construction'})
            evaluations = self._evaluate_no_tf(model=model, dataset=dataset, metrics=metrics, name=name, dataset_kwargs=dataset_kwargs)
        return evaluations