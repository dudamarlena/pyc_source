...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/axon_conabio/trainer/tf_trainer.py
# Compiled at: 2018-12-10 18:39:29
import logging, collections, os, shutil, six, sys, tensorflow as tf, numpy as np
...
class TFTrainer(object):
    ...
    def __init__(self, config, path, retrain=False):
        ...
        if not os.path.exists(path):
            os.makedirs(path)
        if retrain:
            summaries_dir = os.path.join(path, self.config['summaries']['summaries_dir'])
            if os.path.exists(summaries_dir):
                shutil.rmtree(summaries_dir)
            npy_checkpoints_dir = os.path.join(path, self.config['checkpoints']['numpy_checkpoints_dir'])
            if os.path.exists(npy_checkpoints_dir):
                shutil.rmtree(npy_checkpoints_dir)
            tf_checkpoints_dir = os.path.join(path, self.config['checkpoints']['tensorflow_checkpoints_dir'])
            if os.path.exists(tf_checkpoints_dir):
                shutil.rmtree(tf_checkpoints_dir)
            train_log_dir = os.path.join(path, self.config['logging']['log_path'])
    ...
    def _get_regularization_loss(self, model):
        ...
        with model.graph.as_default():
            ...
            if l1_loss > 0 or l2_loss > 0:
                variables = [ variable for variable in model.variables.values() if self._filter_regularization_variables(variable)
                            ]
        ...
        with model.graph.as_default():
            ...
            for i in range(num_gpus):
                with tf.device(('/device:GPU:{}').format(i)):
                    gradients = optimizer.compute_gradients(losses[i] / num_gpus)
                    gradients_list += gradients

            if reg_loss != 0:
                gradients_list += optimizer.compute_gradients(reg_loss)
            train_op = optimizer.apply_gradients(gradients_list, global_step=model.global_step)
            total_loss = tf.add_n(losses) / num_gpus + reg_loss
        return (train_op, total_loss, gradients_list)

    def _build_summary_op(self, model, loss, gradients=None, prefix=None, reg_loss=0):
    ...
    def _aggregate_gradients_summaries(self, gradients):
        ...
        for var, grad_list in six.iteritems(aggregated_gradients):
            ...
            if len(grad_list) == 0:
                ...
                summaries.append(tf.summary.histogram(('gradients/{name}').format(name=name), mean))

        return tf.summary.merge(summaries)

    def _save_tensors(self, tensors, step):
        tensor_conf = self.config['tensor_logs']
        directory = os.path.join(self.path, tensor_conf['tensors_dir'])
        if not os.path.exists(directory):
            os.makedirs(directory)
        path = os.path.join(directory, ('step_{}').format(step))
        num_sample = int(tensor_conf['tensors_per_batch'])
        tensor_samples = {key:value[:num_sample] for key, value in six.iteritems(tensors)}
        np.savez(path, **tensor_samples)

    def train(self, model=None, loss=None, dataset=None, model_kwargs=None, loss_kwargs=None, dataset_kwargs=None):
        if not issubclass(model, TFModel):
            ...
            if not issubclass(dataset, Dataset):
                ...
                model_instance = model(graph=graph, keep_prob=keep_prob, **model_kwargs)
                train_loss = loss(graph=graph, **loss_kwargs)
                validation_loss = loss(graph=graph, **loss_kwargs)
                self.logger.info('Building inputs', extra={'phase': 'construction'})
                ...
                with graph.as_default():
                    ...
                    train_input, train_label = dataset_instance.iter_train(batch_size=batch_size, epochs=epochs)
                    if validate:
                        validation_input, validation_label = dataset_instance.iter_validation(batch_size=batch_size, epochs=epochs)
                self.logger.info('Building model and losses', extra={'phase': 'construction'})
                num_gpus = int(self.config['architecture']['num_gpus'])
                train_losses = train_loss.build_model_loss(model_instance, train_input, train_label, num_gpus=num_gpus, run_name='train')
                reg_loss = self._get_regularization_loss(model_instance)
                if validate:
                    validation_losses = validation_loss.build_model_loss(model_instance, validation_input, validation_label, num_gpus=num_gpus, run_name='validation')
                    total_validation_loss = tf.add_n(validation_losses) / len(validation_losses)
                self.logger.info('Building gradients and train operation', extra={'phase': 'construction'})
                train_outputs = self._get_train_op_multiple_gpu(model_instance, train_losses, reg_loss)
                train_op, total_train_loss, gradients = train_outputs
                init_op = model_instance.init_op()
                tensorboard_summaries = bool(self.config['summaries']['tensorboard_summaries'])
                if tensorboard_summaries:
                    train_summary_op = self._build_summary_op(model_instance, train_loss, gradients=gradients, reg_loss=reg_loss, prefix='train')
                    validation_summary_op = self._build_summary_op(model_instance, validation_loss, prefix='validation')
                save_tensors = bool(self.config['tensor_logs']['save_tensors'])
                if save_tensors:
                    tensor_list = self.config['tensor_logs']['tensor_list'].split(',')
                    train_tensors = model_instance.get_tensors(run_name='train')
                    train_tensors = {key:value for key, value in six.iteritems(train_tensors) if key in tensor_list}
                    save_tensors = len(train_tensors) > 0
                self.logger.info('Starting session and initializing variables', extra={'phase': 'construction'})
                ...
                self.logger.info('Setting up checkpoint and summary writers', extra={'phase': 'construction'})
                if tensorboard_summaries:
                    summaries_dir = self.config['summaries']['summaries_dir']
                    path = os.path.join(self.path, summaries_dir)
                    train_writer = tf.summary.FileWriter(os.path.join(path, 'train'))
                    if validate:
                        validation_writer = tf.summary.FileWriter(os.path.join(path, 'validation'))
                tf_ckp_dir = self.config['checkpoints'].get('tensorflow_checkpoints_dir')
                npy_ckp_dir = self.config['checkpoints'].get('numpy_checkpoints_dir')
                tf_checkpoint_dir = os.path.join(self.path, tf_ckp_dir)
                npy_checkpoint_dir = os.path.join(self.path, npy_ckp_dir)
                os.path.exists(tf_checkpoint_dir) or os.makedirs(tf_checkpoint_dir)
            os.path.exists(npy_checkpoint_dir) or os.makedirs(npy_checkpoint_dir)
        ckpt = get_checkpoints(self.path, tf_subdir=tf_ckp_dir, npy_subdir=npy_ckp_dir)
        if ckpt is not None:
            ckpt_type, ckpt_path, ckpt_step = ckpt
            model_instance.restore(sess, path=ckpt_path, mode=ckpt_type)
            self.logger.info(('Restoring model to training step {}').format(ckpt_step), extra={'phase': 'construction'})
        else:
            self.logger.info('No checkpoint was found. Starting anew.', extra={'phase': 'construction'})
        log = bool(self.config['logging']['logging']) or tensorboard_summaries
        npy_ckpts = bool(self.config['checkpoints']['numpy_checkpoints'])
        tf_ckpts = bool(self.config['checkpoints']['tensorflow_checkpoints'])
        train_sum_freq = int(self.config['summaries']['train_summaries_frequency'])
        valid_sum_freq = int(self.config['summaries']['validation_summaries_frequency'])
        save_tensors_freq = int(self.config['tensor_logs']['save_tensors_frequency'])
        checkpoints_freq = int(self.config['checkpoints']['checkpoints_frequency'])
        stop_at_step = int(self.config['feed']['stop_at_step'])
        checkpoints = npy_ckpts or tf_ckpts
        self.logger.info('Starting training loop', extra={'phase': 'construction'})
        step = None
        try:
            while True:
                ...
                if checkpoints:
                    if step % checkpoints_freq == 0:
                        ...
                        if tf_ckpts:
                            model_instance.save(sess, os.path.join(tf_checkpoint_dir, 'ckpt'), global_step=step)
                        if npy_ckpts:
                            model_instance.numpy_save(sess, npy_checkpoint_dir, step=step)
