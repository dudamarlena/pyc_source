...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/axon_conabio/models/tf_model.py
# Compiled at: 2018-12-10 18:39:29
from abc import ABCMeta, abstractmethod
import collections, os, logging, tensorflow as tf, numpy as np, six
...
class TFModel(Model):
    ...
    def __init__(self, graph=None, keep_prob=1):
        ...
        if not self.submodel:
            with self.graph.as_default():
                with tf.variable_scope(('variables/{name}').format(name=self.name), reuse=tf.AUTO_REUSE, auxiliary_name_scope=False):
                    self.global_step = tf.get_variable('global_step', dtype=tf.int64, shape=[], initializer=tf.zeros_initializer, trainable=False)
    ...
    def _add_variables(self, variables):
        if not self.variables_are_set:
            ...
            variable_dict = {parse_name(variable):variable for variable in variables}
    ...
    def get_tensors(self, run_name=None):
        if run_name is None:
            run_name = 'default'
        tensors = {key:self.get_tensor(key, run_name=run_name) for key in self.tensors[run_name]}
        return tensors
    ...
    def predict(self, inputs, run_name=None, sub_run=None):
        ...
        with summary_scope(summaries):
            with self.graph.as_default():
                ...
                with tf.variable_scope(vscope_name, reuse=tf.AUTO_REUSE, auxiliary_name_scope=False) as (scope):
                    ...
                    with tf.name_scope(scope_name):
                        ...
                        variables = scope.trainable_variables() + scope.local_variables() + scope.global_variables()
    ...
    def _register_tensors(self, run_name=None, sub_run=None):
        ...
        for key in self._tmp_tensor_storage:
            self.tensors[run_name][key].append(self._tmp_tensor_storage[key])
    ...
    def get_variable_summaries(self, prefix=None):
        ...
        for var_name in self.variables:
            ...
            summaries.append(tf.summary.histogram(prefix + var_name, variable))
