...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/axon_conabio/metrics/basemetrics.py
...
class Metric(object):
    ...
    def __call__(self, prediction, label, with_prediction_preprocess=True, with_label_preprocess=True, with_evaluation_postprocess=True):
...
class MetricBundle(Metric):
    ...
    def evaluation_postprocess(self, results):
        dictionary_results = {name:result for name, result in zip(self.names, results)}
...
class ThresholdedMetric(Metric):
    ...
    def __call__(self, prediction, label):
        ...
        for threshold in self.thresholds:
            thresholded_prediction = self.process_threshold(proc_prediction, threshold)
            evaluation = super(ThresholdedMetric, self).__call__(thresholded_prediction, proc_label, with_label_preprocess=False, with_prediction_preprocess=False)
