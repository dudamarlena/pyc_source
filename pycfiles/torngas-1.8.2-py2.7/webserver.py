...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/torngas/webserver.py
# Compiled at: 2016-02-16 00:41:00
import warnings, logging, sys, os, tornado.httpserver, tornado.ioloop, tornado.options, tornado.web
...
class Server(object):
    ...
    def _patch_httpserver(self):
        ...
        class TorngasHTTPServer(httpserver.HTTPServer):
            ...
            def __init__(self, request_callback, xheaders=xhs, **kwargs):
                super(TorngasHTTPServer, self).__init__(request_callback, xheaders=xheaders, **kwargs)
    ...
    def _install_application(self, application):
        ...
        if 'default_handler_class' in tornado_conf and isinstance(tornado_conf['default_handler_class'], basestring):
        ...
        self.application = app_class(handlers=self.urls, default_host='', transforms=None, wsgi=False, middlewares=settings.MIDDLEWARE_CLASSES, **tornado_conf)
        return
    ...
    def parse_logger_callback(self):
        ...
        for log in settings.LOGGING:
            ...
            map(lambda h: h.setFormatter(LogFormatter(fmt=log.get('formatter', LogFormatter.DEFAULT_FORMAT), color=settings.DEBUG)), logger.handlers)
    ...
    def define(self, options=options):
        ...
        try:
            ...
            options.define('log_rotate_mode', type=str, default='time', help='The mode of rotating files(time or size)')
        except:
            pass
        ...
        options.define('log_patch', default=True, help='Use ProcessTimedRotatingFileHandler instead of the default TimedRotatingFileHandler.', type=bool)
