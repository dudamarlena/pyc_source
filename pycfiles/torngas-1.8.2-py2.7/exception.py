...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/torngas/mixins/exception.py
# Compiled at: 2016-02-16 00:41:00
import traceback, sys, tornado, httplib, os
...
class UncaughtExceptionMixin(object):
    ...
    def get_error_html(self, status_code, **kwargs):
        ...
        if not self.application.settings.get('debug', False):
            ...
            return '<html><title>%(code)d: %(message)s</title><body>            <h1>%(code)d: %(message)s</h1>%(full_message)s</body></html>' % {'code': status_code, 
            ...
            return render_string('%s%s' % (resource, tmpl_file), get_snippet=get_snippet, exception=exception, traceback=traceback, sys=sys, tornado=tornado, status_code=status_code, os=os, kwargs=kwargs)
            return
