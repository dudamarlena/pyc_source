...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/torngas/urlhelper.py
        ...
        if handlers and isinstance(handlers, (tuple, list)):
            for handler in handlers:
                if handler and isinstance(handler, urlspec):
                    patt = pattern + handler.repr_pattern.lstrip('^').lstrip('//').lstrip('/')
                    urlsp = urlspec(patt, handler.handler_class, kwargs=handler.kwargs, name=new_name % handler.name if handler.name else handler.name)
