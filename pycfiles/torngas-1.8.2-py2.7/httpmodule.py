...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/torngas/httpmodule/httpmodule.py
...
class HttpModuleMiddleware(object):
        ...
        try:
            ...
            def run_method_():
                if method.__name__ == 'begin_response':
                    ...
                    if method.__name__ == 'begin_render':
                        return method(handler, clear, **kwargs)
                    if method.__name__ == 'begin_request':
                        return method(handler, clear)
                    if method.__name__ == 'complete_response':
                        return method(handler, clear)
                    return
    ...
    def _class_wrap(self, handler_class, name, url):
        ...
        wrap_class = type(new_classname, (
         handler_class,), {'__module__': handler_class.__module__})
        return wrap_class
    ...
    def process_init(self, application):
        ...
        def check_baseclass_(cls):
            if BaseHttpModule not in cls.__bases__:
                raise BaseError("http_module '%s' must inherit from the                 Basemodule" % str(cls))
