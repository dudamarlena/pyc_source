...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/smallsettings/tests/factory.py
# Compiled at: 2013-12-16 14:12:11
import sys
...
class FactoryTest(TestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual('settings_modulepath', self.factory.settings_modulepath)

    def test_import(self):
        self.factory.import_module('something')
        self.mocks['import'].assert_called_once_with('main_modulepath.settings_modulepath.something')
    ...
    def test_run_module(self):
        ...
        with patch.object(self.factory, 'import_module') as (import_module):
            ...
            module.make_settings.assert_called_once_with(self.factory.settings, self.factory.paths)
    ...
    def test_run_module_error(self):
        ...
        with patch.object(self.factory, 'import_module') as (import_module):
            ...
            self.assertRaises(ImportError, self.factory.run_module, 'some_name')
            module.make_settings.assert_called_once_with(self.factory.settings, self.factory.paths)
            import_module.assert_called_once_with('some_name')
    ...
    def test_init_data(self):
        ...
        self.assertEqual({'paths': ['path'], 'project_path': ['/one/two']}, self.factory.paths)
        ...
        with patch.object(self.factory, 'init_data') as (init_data):
            with patch.object(self.factory, 'run_module') as (run_module):
                with patch.object(self.factory, 'run_module_without_errors') as (run_module_without_errors):
                    result = self.factory.make_settings({'settings': 1}, {'paths': 'path'}, additional_modules=[('local1', False)])
                    self.assertEqual((
    ...
    def test_make_settings_with_error(self):
        ...
        with patch.object(self.factory, 'init_data') as (init_data):
            with patch.object(self.factory, 'run_module') as (run_module):
                with patch.object(self.factory, 'run_module_without_errors') as (run_module_without_errors):
                    result = self.factory.make_settings({'settings': 1}, {'paths': 'path'}, additional_modules=[('local1', True)])
                    self.assertEqual((
                     self.factory.settings, self.factory.paths), result)
                    init_data.assert_called_once_with({'settings': 1}, {'paths': 'path'})
