...
# [GCC 8.4.0]
# Embedded file name: /Users/herbert/dev/python/sctdev/simpleproject/simpleproject/../../communitytools/sphenecoll/sphene/sphwiki/models.py
...
class WikiSnip(models.Model):
    ...
    changelog = (
     ('2007-04-08 00', 'alter', 'ALTER creator_id DROP NOT NULL'),
     ('2007-04-08 01', 'alter', 'ALTER editor_id DROP NOT NULL'))

    def render(self):
        from sphene.sphwiki import wikimacros
        return mark_safe(sph_markdown(self.body, extra_macros={'attachmentlist': wikimacros.AttachmentListMacro(snip=self), 'attachment': wikimacros.AttachmentMacro(snip=self), 
    ...
    def pdf_generate(self):
        ...
        snip_rendered_body = snip_rendered_body.replace('<img src="%(media_url)s' % {'media_url': settings.STATIC_URL}, '<img src="%s/' % static_filepath)
        import codecs
    ...
    def get_parent(self):
        ...
        while lastslash != -1:
            ...
            try:
                return WikiSnip.objects.get(group=self.group, name__exact=name)
    ...
    def get_snip_path(self):
        ...
        for element in path:
            if lastsnip is not None:
                ...
                snipname = element
            try:
                snip = WikiSnip.objects.get(group=self.group, name__exact=snipname)
            except WikiSnip.DoesNotExist:
                snip = None
    ...
    def get_wiki_preference(self):
        if hasattr(self, '_wiki_preference'):
    ...
    def __has_permission(self, user, pref, permission):
        if permission == None or permission <= -1:
            ...
            if permission == 1 and pref != None and pref.snip != None:
                if pref.snip.group.get_member(user) != None:
                    return True
            return False
...
class WikiSnipChange(models.Model):
    ...
    changelog = (
     ('2007-04-08 00', 'alter', 'ALTER editor_id DROP NOT NULL'),
    ...
    def get_absolute_url(self):
        return (
         'sphene.sphwiki.views.diff', (), {'groupName': self.snip.group.name, 'snipName': self.snip.name, 'changeId': self.id})

    get_absolute_url = sphpermalink(get_absolute_url)

    def get_absolute_editurl(self):
        return sph_reverse('sphwiki_editversion', kwargs={'snipName': self.snip.name, 'versionId': self.id})
...
class WikiAttachment(models.Model):
    ...
    fileupload = models.FileField(ugettext_lazy('fileupload'), upload_to=get_sph_setting('wiki_attachments_upload_to'))
    description = models.TextField(ugettext_lazy('description'), blank=True)

    def get_absolute_editurl(self):
        return (
         'sphene.sphwiki.views.attachmentEdit', (), {'groupName': self.snip.group.name, 'snipName': self.snip.name, 'attachmentId': self.id})
