...
# [GCC 8.4.0]
# Embedded file name: /Users/herbert/dev/python/sctdev/simpleproject/simpleproject/../../communitytools/sphenecoll/sphene/sphboard/forms.py
...
class PostModelForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(self.fields['markup'].widget.choices) == 2:
...
class PostForm(forms.Form):
    ...
    body = forms.CharField(label=_('Body'), widget=forms.Textarea(attrs={'rows': 10, 'cols': 70}), help_text=describe_render_choices())
    markup = forms.CharField(label=_('Markup'), widget=forms.Select(choices=POST_MARKUP_CHOICES))
    captcha = sphutils.CaptchaField(label=_('Captcha'), widget=sphutils.CaptchaWidget, help_text=_('Please enter the result of the above calculation.'))
...
class PostPollForm(forms.Form):
    ...
    answers = forms.CharField(label=_('Answers (1 per line)'), widget=forms.Textarea(attrs={'rows': 5, 'cols': 80}))
    choicesPerUser = forms.IntegerField(label=_('Allowed Choices per User'), help_text=_('Enter how many answers a user can select.'), min_value=1, max_value=100, initial=1)
...
class AnnotateForm(forms.Form):
    body = forms.CharField(label=_('Body'), widget=forms.Textarea(attrs={'rows': 10, 'cols': 80}), help_text=describe_render_choices())
    markup = forms.CharField(label=_('Markup'), widget=forms.Select(choices=POST_MARKUP_CHOICES))
...
class MoveForm(forms.Form):
    ...
    MoveAndAnnotateForm
    """
    category = boardforms.SelectCategoryField(label=_('Category'), help_text=_('Select target category'))
    info_link = forms.BooleanField(label=_('Information link'), initial=True, required=False, help_text=_('If checked then information about moved thread will be left in the current category'))
...
class MoveAndAnnotateForm(MoveForm, AnnotateForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['body'].help_text = string_concat(_('Please describe why this thread had to be moved.'), ' ', self.fields['body'].help_text)


class MovePostForm(AnnotateForm):
    move_all_posts = forms.BooleanField(label=_('Move all post after this post'), initial=False, required=False, help_text=_('If checked then all posts following this post will be also moved'))

    def __init__(self, *args, **kwargs):
        super(MovePostForm, self).__init__(*args, **kwargs)
        del self.fields['hide_post']
        self.fields['body'].help_text = string_concat(_('Please describe why this post had to be moved.'), ' ', self.fields['body'].help_text)