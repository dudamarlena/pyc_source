...
# [GCC 8.4.0]
# Embedded file name: /Users/herbert/dev/python/sctdev/simpleproject/simpleproject/../../communitytools/sphenecoll/sphene/sphboard/templatetags/sphboard_extras.py
...
def sphboard_displayCategories(context, categories, maxDepth=5, level=-1):
    if maxDepth < level:
        return {}
    ret = {'categories': [ c for c in categories if c.get_category_type().is_displayed() ], 'level': level + 1, 
       'maxDepth': maxDepth}
...
@cache_inclusion_tag(register, 'sphene/sphboard/_post_authorinfo.html', cache_key_func=authorinfo_cachekey, cache_time=get_sph_setting('board_authorinfo_cache_timeout'))
...
signals.post_save.connect(clear_authorinfo_cache, sender=User)
signals.post_save.connect(clear_authorinfo_cache_postcount, sender=UserPostCount)

def clear_posts_render_cache(instance, **kwargs):
    for group in Group.objects.all():
        allowed_categories = get_all_viewable_categories(group, instance.user)
        post_list = Post.objects.filter(category__id__in=allowed_categories, author=instance.user)
        for post in post_list:
            post.clear_render_cache()


signals.post_save.connect(clear_posts_render_cache, sender=CommunityUserProfile)
...
class RecursiveCategoryIteratorNode(template.Node):
    ...
    def recursive_render(self, context, parent, depth=0):
        filter = Category.objects.filter(group=get_current_group())
...
def show_active_monitors(context, profile_user):
    ...
    if user == profile_user:
        monitors = Monitor.objects.filter(Q(thread=None) | Q(thread__is_hidden=0), user=profile_user, group=get_current_group())
    else:
        if not has_permission_flag(request.user, 'community_manage_users'):
            raise PermissionDenied()
        monitors = Monitor.objects.filter(Q(thread=None) | Q(thread__is_hidden=0), user=profile_user, group=get_current_group())
    return {'monitors': monitors, 'request': request, 
