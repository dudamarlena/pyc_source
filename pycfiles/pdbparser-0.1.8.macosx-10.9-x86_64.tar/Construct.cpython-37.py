...
# [GCC 8.4.0]
# Embedded file name: /Library/Frameworks/Python.framework/Versions/3.7/lib/python3.7/site-packages/pdbparser/Utilities/Construct.py
# Compiled at: 2019-02-17 15:06:52
# Size of source mod 2**32: 61289 bytes
"""
...
class Construct(object):
    ...
    def initialize_useful_pdbs_variables(self):
        ...
        totalNumberOfAtoms = [self.pdbsNumberofAtomsDict[idx] * self.pdbsNumberOfInsertionDict[idx] for idx in range(len(self.pdbs))]
    ...
    def initialize_grid_variables(self, gridUnitSize=None):
        if gridUnitSize is None:
            self.gridUnitSize = np.array([1.0, 1.0, 1.0])
        else:
            try:
    ...
    def check_pdbs_size(self):
        for pdb in self.pdbs:
            ...
            if minMax[1] - minMax[0] > self.boxSize[0] / 2.0 or minMax[3] - minMax[2] > self.boxSize[1] / 2.0 or minMax[5] - minMax[4] > self.boxSize[2] / 2.0:
    ...
    def set_boxsize_and_insertion_number(self):
        ...
        waterDensity = 1e-24
        if boxSizePriority and densityPriority:
            self.insertionNumber = insertionNumberPriority or self.density * waterDensity * np.prod(self.boxSize) * __avogadroNumber__ / np.array(self.pdbsMolarWeight)
            ...
            if insertionNumberPriority and densityPriority:
                totalMass = boxSizePriority or float(np.sum(np.array(self.insertionNumber) * np.array(self.pdbsMolarWeight)))
                cubicBoxSideLength = (totalMass / waterDensity / __avogadroNumber__ / self.density) ** 0.3333333333333333
                ...
                if boxSizePriority and insertionNumberPriority:
                    self.density = densityPriority or np.sum(np.array(self.insertionNumber) * np.array(self.pdbsMolarWeight)) / np.prod(self.boxSize) / waterDensity / __avogadroNumber__
                else:
        ...
        self.translateToBoxCenterVector = np.array(self.boxCenter).astype(float) - np.array(self.boxSize / 2.0).astype(float)
    ...
    def get_pdb_data(self, pdb, indexes=None, attributes=None):
        ...
        if attributes is None:
            attributes = [
             'record_name', 'serial_number', 'atom_name',
    ...
    def initialize_default_attributes(self):
        ...
        if not hasattr(self, 'pdbsAxis'):
            ...
            if not self.pdbsAxis:
                self.pdbsAxis = [self.__defaults__['pdbsAxis'] for idx in range(len(self.pdbs))]
            else:
                if not isinstance(self.pdbsAxis, (list, tuple)):
                    self.pdbsAxis = [
                     self.pdbsAxis] * len(self.pdbs)
        if not hasattr(self, 'boxSize'):
            ...
            if self.boxSize is None:
                ...
                if not hasattr(self, 'boxCenter'):
                    ...
                    if self.boxCenter is None:
                        self.boxCenter = self.__defaults__['boxCenter']
                    else:
                        self.boxCenter = np.array(self.boxCenter)
                        if not self.boxCenter.shape == (3, ):
                            raise AssertionError
                        elif not hasattr(self, 'periodicBoundaries'):
                            self.periodicBoundaries = self.__defaults__['periodicBoundaries']
                        elif not isinstance(self.periodicBoundaries, bool):
                            raise AssertionError
                        else:
                            ...
                            if not hasattr(self, 'exisitingPdbs'):
                                self.exisitingPdbs = [
                                 self.__defaults__['exisitingPdbs']]
                            else:
                            ...
                            if not hasattr(self, 'restrictions'):
                                self.restrictions = [
                                 self.__defaults__['restrictions']]
                            else:
                                if (isinstance(self.restrictions, (list, tuple)) or self).restrictions:
        ...
        if not hasattr(self, 'bondLength'):
            ...
            if not self.bondLength > 0:
                raise AssertionError
            elif not hasattr(self, 'priorities'):
                self.priorities = {'boxSize':True, 
                 'insertionNumber':False,  'density':True}
            else:
    ...
    def finalize(self):
        ...
        for idx in range(self.recordName.shape[0]):
            thisPDB.records.append({'record_name':self.recordName[idx],  'serial_number':self.serialNumber[idx], 
    ...
    def parse_existing_pdbs(self):
        if not isinstance(self.box3DGrid, np.ndarray):
            ...
            for pdb in self.exisitingPdbs:
                ...
                goodIndexes = np.where(np.prod((coords - self.translateToBoxCenterVector > 0), axis=1) * np.prod((coords - self.translateToBoxCenterVector < self.boxSize), axis=1))[0]
    ...
    def parse_restrictions(self):
        if not self.restrictions:
            return
        elif not isinstance(self.box3DGrid, np.ndarray):
            return
            assert np.issubdtype(self.box3DGrid.dtype, np.dtype(int).type)
    ...
    def get_box3DGrid_slicing(self, posIdx, margin):
        return [
         np.array(range(posIdx[0] - margin, posIdx[0] + margin + 1)).reshape(-1, 1, 1) % self.box3DGrid.shape[0],
    ...
    def is_molrecords_accepted(self, molRecords, definedIndexes, exisitingIndexes, minimumDistanceSquared):
        if not len(definedIndexes):
            if not len(exisitingIndexes):
                return True
        continueLoop = True
    ...
    def solvate(self, pdbs=None, insertionNumber=None, thikness=10, density=1, restrictions=None, priorities=None, recursionLimit=10000):
        ...
        boxCenter = np.array([minMax[1] + minMax[0],
         minMax[3] + minMax[2],
         minMax[5] + minMax[4]]) / 2.0
        self.get_pdb().concatenate(AmorphousSystem(pdbs=pdbs, insertionNumber=insertionNumber,
...
class AmorphousSystem(Construct):
    ...
    def __init__(self, pdbs, insertionNumber=None, boxSize=None, boxCenter=None, density=1, recursionLimit=10000, periodicBoundaries=True, restrictions=None, exisitingPdbs=None, priorities=None, *args, **kwargs):
        self.pdbs = pdbs
                ...
                for molNum in range(insertionDict[pdbIndex]):
                    ...
                    while recursion_limit > 0:
                        ...
                        if self.box3DGrid[(posIdx[0], posIdx[1], posIdx[2])] >= 0:
                            ...
                            if self.box3DGrid[(posIdx[0], posIdx[1], posIdx[2])] == self.restrictedUnitValue:
                                continue
...
class Micelle(Construct):
    ...
    def __init__(self, pdbs, insertionNumber=60, pdbsAxis=None, flipPdbs=False, micelleRadius=None, positionsGeneration=None, *args, **kwargs):
        self.pdbs = pdbs
        self.insertionNumber = insertionNumber
        ...
        elif positionsGeneration is None:
    ...
    def construct(self):
        ...
        if self.positionsGeneration == 'symmetric':
            ...
            if self.positionsGeneration == 'asymmetric':
                moleculeWidth_Z = self.minMax[5] - self.minMax[4] + self.interMolecularMinimumDistance / 2.0
                moleculeWidth_Y = self.minMax[3] - self.minMax[2] + self.interMolecularMinimumDistance / 2.0
...
class Liposome(Construct):
    ...
    def __init__(self, pdbs, innerInsertionNumber, innerRadius=None, outerInsertionNumber=None, pdbsAxis=None, flipPdbs=False, positionsGeneration=None, interlayerDistance=None, *args, **kwargs):
        self.pdbs = pdbs
        self.pdbsAxis = pdbsAxis
        self.flipPdbs = flipPdbs
    ...
    def initialize(self):
        if self.interlayerDistance is None:
            ...
            self.interlayerDistance = interlayerDistance
            if not isinstance(interlayerDistance, (float, int)):
                raise AssertionError
            elif not self.interlayerDistance > 0:
                raise AssertionError
            elif self.positionsGeneration is None:
                self.positionsGeneration = [
                ...
                if isinstance(self.positionsGeneration, (list, tuple)):
                    if len(self.positionsGeneration) == 1:
                        ...
                        if len(positionsGeneration) == 2:
                            assert isinstance(self.positionsGeneration[0], basestring)
                            assert self.positionsGeneration[0].lower() in ('symmetric',
                                                                           'asymmetric')
                            assert isinstance(self.positionsGeneration[1], basestring)
                            assert self.positionsGeneration[1].lower() in ('symmetric',
                                                                           'asymmetric')
    ...
    def construct(self):
        ...
        pdbINNER = Micelle((copy.deepcopy(self.pdbs)), insertionNumber=(self.innerInsertionNumber),
        ...
        pdbOUTER = Micelle((copy.deepcopy(self.pdbs)), insertionNumber=(self.outerInsertionNumber),
          pdbsAxis=(self.pdbsAxis),
...
class Sheet(Construct):
    ...
    def __init__(self, element=None, length=50, width=50, orientation=None, bondLength=1.42, *args, **kwargs):
        ...
        if not orientation:
            ...
            if not self.orientation in ('arm-chair', 'zig-zag', 'armchair', 'zigzag'):
                raise AssertionError
            else:
                self.element = element or 'C'
...
class Nanotube(Sheet):
    ...
    def __init__(self, element=None, length=50, radius=10, orientation=None, bondLength=1.42, *args, **kwargs):
        (super(Nanotube, self).__init__)(args, element=element, length=length, width=2.0 * np.pi * radius, orientation=orientation, bondLength=bondLength, **kwargs)
    ...
    def construct(self):
        ...
        minMax = get_min_max(self.get_pdb().indexes, self.get_pdb())
        translate(self.get_pdb().indexes, self.get_pdb(), [-(minMax[1] + minMax[0]) / 2.0, -(minMax[3] + minMax[2]) / 2.0, -(minMax[5] + minMax[4]) / 2.0])
        return self
...
class MultipleWallNanotube(Construct):
    ...
    def __init__(self, element=None, length=50, radius=10, orientation=None, bondLength=1.42, interlayerDistance=3.4, wallsNumber=1, *args, **kwargs):
    ...
    def construct(self):
        for layerNumber in range(self.wallsNumber):
            cnt = Nanotube(element=(self.element), length=(self.length),
...
class SuperCell(Construct):
    ...
    def __init__(self, pdb, cellAxes, supercellSize, *args, **kwargs):
        self.pdbs = pdb
    ...
    def initialize(self):
        if not isinstance(self.cellAxes, (list, tuple, np.ndarray)):
            raise AssertionError
        else:
