...
# [GCC 8.4.0]
# Embedded file name: /Library/Frameworks/Python.framework/Versions/3.7/lib/python3.7/site-packages/pdbparser/Utilities/BoundaryConditions.py
# Compiled at: 2019-02-16 12:02:14
# Size of source mod 2**32: 42339 bytes
"""
...
class PeriodicBoundaries(InfiniteBoundaries):
    ...
    def set_vectors(self, vectorsArray, index=None):
        ...
        if isinstance(vectorsArray, (list, tuple)):
            ...
            if not isinstance(vectorsArray, (Number, np.ndarray)):
                raise AssertionError('vectors can be either None, number, numpy.ndarray')
            elif isinstance(vectorsArray, Number):
                ...
                if vectorsArray.shape == (1, ) or vectorsArray.shape == (1, 1):
                    ...
                    if vectorsArray.shape == (3, ) or vectorsArray.shape == (3, 1):
                        directBasisVectors = np.array([[vectorsArray[0], 0, 0],
                         [
                          0, vectorsArray[1], 0],
                         [
                        ...
                        if vectorsArray.shape == (9, ) or vectorsArray.shape == (9,
                                                                                 1):
                            directBasisVectors = np.array([[vectorsArray[0], vectorsArray[1], vectorsArray[2]],
                             [
                              vectorsArray[3], vectorsArray[4], vectorsArray[5]],
                             [
                              vectorsArray[6], vectorsArray[7], vectorsArray[8]]],
                              dtype=(np.float))
                        else:
                            if vectorsArray.shape == (3, 3):
    ...
    def get_box_real_center(self, index=-1):
        ...
        """
        return self.box_to_real_array(np.array([0.5, 0.5, 0.5]), index)
    ...
    def real_to_box_array(self, realArray, index=-1):
        ...
        if len(shape) == 2 and not shape[1] == 3:
            raise AssertionError('realArray must have three columns for x, y and z coordinates')
        else:
            if not (len(shape) == 1 and len(realArray) == 3):
                raise AssertionError('if realArray is a point vector it should be of length 3 for x, y and z coordinates')
            realArray = realArray.reshape(-1, 3)
        rbasis = self.get_reciprocal_vectors(index)
        boxArray = np.empty(realArray.shape)
        boxArray[:, 0] = realArray[:, 0] * rbasis[(0, 0)] + realArray[:, 1] * rbasis[(1,
                                                                                      0)] + realArray[:, 2] * rbasis[(2,
                                                                                                                      0)]
        boxArray[:, 1] = realArray[:, 0] * rbasis[(0, 1)] + realArray[:, 1] * rbasis[(1,
                                                                                      1)] + realArray[:, 2] * rbasis[(2,
                                                                                                                      1)]
        boxArray[:, 2] = realArray[:, 0] * rbasis[(0, 2)] + realArray[:, 1] * rbasis[(1,
                                                                                      2)] + realArray[:, 2] * rbasis[(2,
                                                                                                                      2)]
        return boxArray
    ...
    def box_to_real_array(self, boxArray, index=-1):
        ...
        if len(shape) == 2 and not shape[1] == 3:
            raise AssertionError('boxArray must have three columns for x, y and z coordinates')
        else:
            if not (len(shape) == 1 and len(boxArray) == 3):
                raise AssertionError('if boxArray is a point vector it should be of length 3 for x, y and z coordinates')
            boxArray = boxArray.reshape(-1, 3)
        basis = self.get_vectors(index)
        realArray = np.empty(boxArray.shape)
        realArray[:, 0] = boxArray[:, 0] * basis[(0, 0)] + boxArray[:, 1] * basis[(1,
                                                                                   0)] + boxArray[:, 2] * basis[(2,
                                                                                                                 0)]
        realArray[:, 1] = boxArray[:, 0] * basis[(0, 1)] + boxArray[:, 1] * basis[(1,
                                                                                   1)] + boxArray[:, 2] * basis[(2,
                                                                                                                 1)]
        realArray[:, 2] = boxArray[:, 0] * basis[(0, 2)] + boxArray[:, 1] * basis[(1,
                                                                                   2)] + boxArray[:, 2] * basis[(2,
                                                                                                                 2)]
        return realArray
    ...
    def real_distance(self, realVector, realArray, index=-1):
        ...
        """
        difference = self.real_difference(realVector, realArray, index).reshape((-1,
                                                                                 3))
        return np.sqrt(np.add.reduce((difference ** 2), axis=1))
    ...
    def box_to_indexes_histogram(self, boxArray, boxBinLength, index=-1):
        ...
        return np.histogramdd(sample=boxArray, bins=bins, range=[
         [
          0, 1], [0, 1], [0, 1]],
    ...
    def real_to_indexes_histogram(self, realArray, realBinLength=0.5, index=-1):
        ...
        return self.boxToIndexesHistogram(boxArray=boxArray, boxBinLength=boxBinLength,
          index=index)
...
class BoundaryConditions(object):
    ...
    def set_pdb(self, pdb):
        ...
        self._BoundaryConditions__pdb = pdb
        self.initialize_variables()

    def initialize_variables(self):
        self._BoundaryConditions__a = None
        self._BoundaryConditions__b = None
        self._BoundaryConditions__c = None

    def __set_a__(self, a):
        if len(a) == 1:
            self._BoundaryConditions__a = np.array([float(a), 0, 0])
        else:
            if len(a) == 3:
                self._BoundaryConditions__a = np.array([float(a[0]), float(a[1]), float(a[2])])
            else:
    ...
    def __set_b__(self, b):
        if len(b) == 1:
            assert float(b)
            self._BoundaryConditions__b = np.array([0, float(b), 0])
        else:
            if len(b) == 3:
                self._BoundaryConditions__b = np.array([float(b[0]), float(b[1]), float(b[2])])
            else:
    ...
    def __set_c__(self, c):
        if len(c) == 1:
            assert float(c)
            self._BoundaryConditions__c = np.array([0, 0, float(c)])
        else:
            if len(c) == 3:
                self._BoundaryConditions__c = np.array([float(c[0]), float(c[1]), float(c[2])])
            else:
                raise pdbparser.Logger.error("'c' should be a float or a three dimensions vector")

    @property
    def vectors(self):
        return (self._BoundaryConditions__a, self._BoundaryConditions__b, self._BoundaryConditions__c)
    ...
    def calculate_vectors(self, lattice='triclinic', bin=5.0):
        ...
        if lattice not in ('triclinic', 'orthorhombic', 'cubic'):
            ...
            raise
        bin = float(bin)
        x, y, z = get_coordinates(self._BoundaryConditions__pdb.indexes, self._BoundaryConditions__pdb)
        self.__calculate_vector__(x, y, z, bin)
        print('\n\n')
        self.__calculate_vector__(y, x, z, bin)
        print('\n\n')
        self.__calculate_vector__(z, y, x, bin)
        self._BoundaryConditions__pdb.visualize()
...
if __name__ == '__main__':
    ...
    print('box size:', boxSize, ' - boxCenter:', boxSize / 2.0)
    print('================================================\n')
