...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/in_limbo/sandwich_estimators.py
# Compiled at: 2014-11-07 07:49:38
import nipy.modalities.fmri.design_matrix as dm
from nipy.modalities.fmri.experimental_paradigm import EventRelatedParadigm
import nibabel as nb
from nipype.utils import filemanip
import os
from nipype.interfaces.base import BaseInterface, BaseInterfaceInputSpec, traits, File, TraitedSpec, OutputMultiPath, isdefined
import scipy as sp
...
class SandwichEstimator(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        print q
        data_cut = np.array([ nb.load(d).get_data().astype(float) for d in self.inputs.cutted_data
                            ])
        ...
        W = np.zeros((residuals.shape[(-1)],) * 2 + data_cut.shape[1:-1])
        for residual in residuals:
            W += np.einsum('...i, ...j->ij...', residual, residual)

        W /= q - 1
        V = np.zeros((X_cut.shape[(-1)],) * 2 + data_cut.shape[1:-1])
        for i, x in enumerate(X_cut):
            print i
        ...
        V /= q
        print V.shape
        nb.save(nb.Nifti1Image(V, np.identity(4)), os.path.abspath('variance.nii.gz'))
        for i, residual in enumerate(residuals):
            nb.save(nb.Nifti1Image(residual, nb.load(self.inputs.cutted_data[0]).get_affine()), os.path.abspath('residuals_%d.nii.gz' % (i + 1)))
...
class GetVoxelCovariance(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        q = len(residuals)
        W_ij = np.zeros((residuals[0].shape[(-1)],) * 2 + residuals[0].shape[:-1])
        for i, residual in enumerate(residuals):
            print 'residual %d' % i, residual.get_data().sum(),
            W_ij += np.einsum('...i, ...j->ij...', residual.get_data()[comp_index], residual.get_data())
            print W_ij.sum()

        W_ij /= q - 1
        V_ij = np.zeros((design_matrices[0].shape[(-1)],) * 2 + residuals[0].shape[:-1])
        ...
        V_ij /= q
        nb.save(nb.Nifti1Image(V_ij, residuals[0].get_affine()), os.path.abspath('covariance_map_%s.nii.gz' % ('_').join(str(e) for e in self.inputs.comp_voxel)))
        return runtime
...
class Level2WLS(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        dof = len(copes) - 1
        z = sp.stats.norm.ppf(sp.stats.t(dof).cdf(t))
        nb.save(nb.Nifti1Image(t, nb.load(self.inputs.copes[0]).get_affine()), os.path.abspath('t_stats.nii.gz'))
        nb.save(nb.Nifti1Image(z, nb.load(self.inputs.copes[0]).get_affine()), os.path.abspath('z_stats.nii.gz'))
        return runtime
