...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/in_limbo/workflows/sandwich_workflows.py
# Compiled at: 2014-11-12 09:50:27
import nipype.pipeline.engine as pe, nipype.interfaces.utility as util, nipype.interfaces.fsl as fsl, nipype.algorithms.modelgen as model, nipype.interfaces.io as nio
...
import pandas, re, glob, os

def create_level1_sandwich(name='level1_sandwich', base_dir='~/workflow_folders'):
    ...
    inputspec = pe.Node(util.IdentityInterface(fields=['functional_runs',
     'session_info',
     'contrasts', 'TR', 'q']), name='inputspec')
    onsets_cutter = pe.MapNode(OnsetsCutter(), iterfield=[
     'session_info'], name='onsets_cutter')
    ...
    ols_esimator = pe.MapNode(OLSFitter(), iterfield=[
     'X', 'data'], name='ols_esimator')
    workflow.connect(onsets_cutter, 'X', ols_esimator, 'X')
    workflow.connect(inputspec, 'functional_runs', ols_esimator, 'data')
    sandwich_estimator = pe.MapNode(SandwichEstimator(), iterfield=[
     'beta', 'design_matrices', 'cutted_data'], name='sandwich_estimator')
    ...
    contraster = pe.MapNode(SandwichContrast(), iterfield=[
     'beta', 'variance', 'q', 'session_info'], name='contraster')
    ...
    sqrt = pe.MapNode(fsl.maths.UnaryMaths(), name='sqrt', iterfield=[
     'in_file'])
    ...
    split = pe.Node(util.Function(function=split, input_names=[
     'list_in',
     'length_sublists'], output_names=[
     'list_out']), name='split')
...
def create_level2_analysis_sandwich_in_limbo(name='level2_sandwich', base_dir='~/workflow_folders'):
    ...
    copemerge = pe.MapNode(interface=fsl.Merge(dimension='t'), iterfield=[
     'in_files'], name='copemerge')
    varcopemerge = pe.MapNode(interface=fsl.Merge(dimension='t'), iterfield=[
     'in_files'], name='varcopemerge')
    level2model = pe.Node(interface=fsl.L2Model(), name='l2model')
    flameo = pe.MapNode(interface=fsl.FLAMEO(run_mode='flame1'), name='flameo', iterfield=[
     'cope_file', 'var_cope_file'])
    ...
    gendof = pe.Node(util.Function(input_names=['dof_files', 'cope_files'], output_names=[
     'dof_volume'], function=get_dofvolumes), name='gendofvolume')
    ...
    get_comparison_voxel = pe.Node(util.Function(function=argmin, input_names=[
     'image'], output_names=[
     'index']), name='get_comparison_voxel')
    level2.connect(cluster, 'threshold_file', get_comparison_voxel, 'image')
    get_covariance_comp_voxel = pe.MapNode(GetVoxelCovariance(), iterfield=[
     'design_matrices', 'residuals'], name='get_covariance_comp_voxel')
    ...
    get_in_limbo_weights = pe.MapNode(GetInLimboWeights(), iterfield=[
     'varcope',
     'contrast',
     'comp_voxel_covariance'], name='get_in_limbo_weights')
    ...
    get_threshold_node = pe.Node(util.Function(function=get_threshold, input_names=[
     'dof', 'alpha'], output_names=[
     'threshold']), name='get_threshold_node')
    ...
    outputspec = pe.Node(util.IdentityInterface(fields=['z_thresholded',
     'in_limbo',
     'in_limbo_unactivated']), name='outputspec')
