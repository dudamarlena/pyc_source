...
# [GCC 8.4.0]
# Embedded file name: /Users/tintin/workspace/python/python-solidity-parser-antlr/build/lib/solidity_parser/parser.py
# Compiled at: 2019-03-05 16:43:12
# Size of source mod 2**32: 35063 bytes
from antlr4 import *
import solidity_parser.solidity_antlr4.SolidityLexer as SolidityLexer
import solidity_parser.solidity_antlr4.SolidityParser as SolidityParser
import solidity_parser.solidity_antlr4.SolidityVisitor as SolidityVisitor
...
class Node(dict):
    ...
    @staticmethod
    def _get_loc(ctx):
        return {'start':{'line':ctx.start.line, 
          'column':ctx.start.column}, 
         'end':{'line':ctx.stop.line, 
...
class AstVisitor(SolidityVisitor):
    ...
    def visitConstructorDefinition(self, ctx: SolidityParser.ConstructorDefinitionContext):
        ...
        if ctx.modifierList().ExternalKeyword(0):
            ...
            if ctx.modifierList().InternalKeyword(0):
                ...
                if ctx.modifierList().PublicKeyword(0):
                    visibility = 'public'
                else:
                    if ctx.modifierList().PrivateKeyword(0):
                        visibility = 'private'
                    else:
                        visibility = 'default'
        ...
        return Node(ctx=ctx, type='FunctionDefinition',
          name=None,
    ...
    def visitParameterList(self, ctx: SolidityParser.ParameterListContext):
        ...
        return Node(ctx=ctx, type='ParameterList',
          parameters=parameters)
    ...
    def visitParameter(self, ctx: SolidityParser.ParameterContext):
        ...
        return Node(ctx=ctx, type='Parameter',
          name=name,
    ...
    def visitModifierInvocation(self, ctx):
        ...
        if exprList is not None:
            ...
            args = []
        return Node(ctx=ctx, type='ModifierInvocation',
          name=(ctx.identifier().getText()),
          arguments=args)

    def visitElementaryTypeNameExpression(self, ctx):
        return Node(ctx=ctx, type='ElementaryTypeNameExpression',
    ...
    def visitTypeName(self, ctx):
        if len(ctx.children) > 2:
            ...
            return Node(ctx=ctx, type='ArrayTypeName',
              baseTypeName=(self.visit(ctx.getChild(0))),
              length=length)
        if len(ctx.children) == 2:
            return Node(ctx=ctx, type='ElementaryTypeName',
    ...
    def visitFunctionCall(self, ctx):
        ...
        return Node(ctx=ctx, type='FunctionCall',
    ...
    def visitVariableDeclaration(self, ctx):
        ...
        return Node(ctx=ctx, type='VariableDeclaration',
    ...
    def visitWhileStatement(self, ctx):
        return Node(ctx=ctx, type='WhileStatement',
          condition=(self.visit(ctx.expression())),
          body=(self.visit(ctx.statement())))

    def visitDoWhileStatement(self, ctx):
        return Node(ctx=ctx, type='DoWhileStatement',
          condition=(self.visit(ctx.expression())),
          body=(self.visit(ctx.statement())))
    ...
    def visitIfStatement(self, ctx):
        ...
        return Node(ctx=ctx, type='IfStatement',
          condition=(self.visit(ctx.expression())),
          TrueBody=TrueBody,
          FalseBody=FalseBody)

    def visitUserDefinedTypeName(self, ctx):
        return Node(ctx=ctx, type='UserDefinedTypeName',
    ...
    def visitBlock(self, ctx):
        return Node(ctx=ctx, type='Block',
          statements=(self.visit(ctx.statement())))

    def visitExpressionStatement(self, ctx):
        return Node(ctx=ctx, type='ExpressionStatement',
          expression=(self.visit(ctx.expression())))
    ...
    def visitNumberLiteral(self, ctx):
        ...
        return Node(ctx=ctx, type='NumberLiteral',
          number=number,
          subdenomination=subdenomination)

    def visitMapping(self, ctx):
        return Node(ctx=ctx, type='Mapping',
    ...
    def visitModifierDefinition(self, ctx):
        ...
        if ctx.parameterList():
            parameters = self.visit(ctx.parameterList())
        return Node(ctx=ctx, type='ModifierDefinition',
          name=(ctx.identifier().getText()),
          parameters=parameters,
    ...
    def visitExpression(self, ctx):
        ...
        if children_length == 2:
            ...
            if op == 'new':
                return Node(ctx=ctx, type='NewExpression',
                  typeName=(self.visit(ctx.typeName())))
            if op in ('+', '-', '++', '--', '!', '~', 'after', 'delete'):
                return Node(ctx=ctx, type='UnaryOperation',
                  operator=op,
            ...
            if children_length == 3:
                if ctx.getChild(0).getText() == '(':
                    if ctx.getChild(2).getText() == ')':
                        return Node(ctx=ctx, type='TupleExpression',
                          components=[
                ...
                if op == '.':
                    ...
                    return Node(ctx=ctx, type='MemberAccess',
                ...
                binOps = [
                 ...
                 '%=']
                if op in binOps:
                    return Node(ctx=ctx, type='BinaryOperation',
                      operator=op,
        ...
        if ctx.expression():
            ...
            if ctx.InternalKeyword(0):
                ...
                if ctx.PublicKeyword(0):
                    ...
                    if ctx.PrivateKeyword(0):
                        visibility = 'private'
        isDeclaredConst = False
        if ctx.ConstantKeyword(0):
            isDeclaredConst = True
        decl = self._createNode(ctx=ctx,
          ...
          isIndexed=False)
        return Node(ctx=ctx, type='StateVariableDeclaration',
          variables=[
         decl],
    ...
    def visitForStatement(self, ctx):
        ...
        return Node(ctx=ctx, type='ForStatement',
          initExpression=(self.visit(ctx.simpleStatement())),
          conditionExpression=conditionExpression,
          loopExpression=Node(type='ExpressionStatement',
          expression=(self.visit(ctx.expression()))),
          body=(self.visit(ctx.statement())))

    def visitPrimaryExpression(self, ctx):
        if ctx.BooleanLiteral():
            return Node(ctx=ctx, type='BooleanLiteral',
              value=(ctx.BooleanLiteral().getText() == 'True'))
            if ctx.HexLiteral():
                return Node(ctx=ctx, type='HexLiteral',
                  value=(ctx.HexLiteral().getText()))
            if ctx.StringLiteral():
                text = ctx.getText()
                return Node(ctx=ctx, type='StringLiteral',
                  value=(text[1:len(text) - 1]))
        elif len(ctx.children) == 3:
            if ctx.getChild(1).getText() == '[' and ctx.getChild(2).getText() == ']':
                node = self.visit(ctx.getChild(0))
                if node.type == 'Identifier':
                    node = Node(ctx=ctx, type='UserDefinedTypeName',
                      namePath=(node.name))
                else:
                    node = Node(ctx=ctx, type='ElementaryTypeName',
                      name=(ctx.getChild(0).getText()))
                return Node(ctx=ctx, type='ArrayTypeName',
                  baseTypeName=node,
                  length=None)
    ...
    def visitIdentifier(self, ctx):
        return Node(ctx=ctx, type='Identifier',
          name=(ctx.getText()))
    ...
    def visitIdentifierList(self, ctx: SolidityParser.IdentifierListContext):
        ...
        for iden in self._mapCommasToNulls(children):
            if iden == None:
                ...
                result.append(self._createNode(ctx=ctx, type='VariableDeclaration',
                  name=(iden.getText()),
                  isStateVar=False,
                  isIndexed=False,
                  iden=iden))
    ...
    def visitVariableDeclarationStatement(self, ctx):
        ...
        return Node(ctx=ctx, type='VariableDeclarationStatement',
          variables=variables,
    ...
    def visitImportDirective(self, ctx):
        ...
        if impDecLen > 0:
            ...
            if impDecLen == 7:
                ...
                if impDecLen == 5:
                    unitAlias = ctx.getChild(3).getText()
        return Node(ctx=ctx, type='ImportDirective',
          path=(pathString[1:len(pathString) - 1]),
          unitAlias=unitAlias,
          symbolAliases=symbolAliases)

    def visitEventDefinition(self, ctx):
        return Node(ctx=ctx, type='EventDefinition',
          name=(ctx.identifier().getText()),
    ...
    def visitEventParameterList(self, ctx):
        ...
        for paramCtx in ctx.eventParameter():
            ...
            parameters.append(self._createNode(ctx=ctx, type='VariableDeclaration',
              typeName=type,
              name=name,
              isStateVar=False,
    ...
    def visitAssemblyItem(self, ctx):
        ...
        if ctx.StringLiteral():
            ...
            return Node(ctx=ctx, type='StringLiteral',
              value=(text[1:len(text) - 1]))
        if ctx.BreakKeyword():
            return Node(ctx=ctx, type='Break')
        if ctx.ContinueKeyword():
            return Node(ctx=ctx, type='Continue')
        return self.visit(ctx.getChild(0))
    ...
    def visitAssemblyCall(self, ctx):
        ...
        return Node(ctx=ctx, type='AssemblyExpression',
    ...
    def visitAssemblyLiteral(self, ctx):
        ...
        if ctx.DecimalNumber():
            return Node(ctx=ctx, type='DecimalNumber',
              value=(ctx.getText()))
        if ctx.HexNumber():
            return Node(ctx=ctx, type='HexNumber',
              value=(ctx.getText()))
        if ctx.HexLiteral():
            return Node(ctx=ctx, type='HexLiteral',
              value=(ctx.getText()))
    ...
    def visitAssemblySwitch(self, ctx):
        return Node(ctx=ctx, type='AssemblySwitch',
    ...
    def visitAssemblyCase(self, ctx):
        ...
        if ctx.getChild(0).getText() == 'case':
            ...
            node = Node(ctx=ctx, type='AssemblyCase',
    ...
    def visitAssemblyLocalDefinition(self, ctx):
        ...
        return Node(ctx=ctx, type='AssemblyLocalDefinition',
          names=names,
          expression=(self.visit(ctx.assemblyExpression())))
    ...
    def visitAssemblyFunctionDefinition(self, ctx):
        ...
        return Node(ctx=ctx, type='AssemblyFunctionDefinition',
          name=(ctx.identifier().getText()),
    ...
    def visitAssemblyAssignment(self, ctx):
        ...
        if names.identifier():
            ...
            names = self.visit(names.assemblyIdentifierList().identifier())
        return Node(ctx=ctx, type='AssemblyAssignment',
          names=names,
          expression=(self.visit(ctx.assemblyExpression())))

    def visitLabelDefinition(self, ctx):
        return Node(ctx=ctx, type='LabelDefinition',
          name=(ctx.identifier().getText()))

    def visitAssemblyStackAssignment(self, ctx):
        return Node(ctx=ctx, type='AssemblyStackAssignment',
          name=(ctx.identifier().getText()))

    def visitAssemblyFor(self, ctx):
        return Node(ctx=ctx, type='AssemblyFor',
    ...
    def visitContractDefinition(self, ctx):
        ...
        return Node(ctx=ctx, type='ContractDefinition',
          name=(ctx.identifier().getText()),
...
def parse(text, start='sourceUnit', loc=False, strict=False):
    import antlr4.InputStream as InputStream
