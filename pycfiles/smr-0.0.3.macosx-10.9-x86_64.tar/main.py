...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/smr/main.py
# Compiled at: 2014-07-27 13:00:57
from __future__ import absolute_import, division, print_function, unicode_literals
import curses, datetime, os, psutil
...
def worker_stdout_read_thread(output_queue, map_process, abort_event):
    ...
    for line in iter(map_process.stdout.readline, b''):
...
def worker_stderr_read_thread(processed_files_queue, input_queue, map_process, abort_event):
    ...
    if not abort_event.is_set() and not write_file_to_descriptor(input_queue, map_process.stdin):
        abort_event.set()
        return
    for line in iter(map_process.stderr.readline, b''):
        line = line.rstrip()
        splt = line.split(b',', 2)
        if len(splt) != 3:
...
def curses_thread(config, abort_event, map_processes, reduce_processes, window, start_time, bytes_total):
    ...
    while not abort_event.is_set() and sleep_time > 0 and not abort_event.wait(sleep_time):
        ...
        add_str(window, 0, (b'smr v{} - {} - elapsed: {}').format(__version__, datetime.datetime.ctime(now), now - start_time))
        i = 1
        for p in map_pids:
            print_pid(p, window, i, b'smr-map')
            i += 1
...
def run(config):
    ...
    map_args = get_args(b'smr-map', config)
    map_processes = []
    read_workers = []
    ...
    reduce_process = subprocess.Popen(get_args(b'smr-reduce', config), bufsize=0, stdin=subprocess.PIPE, stdout=reduce_stdout, stderr=subprocess.PIPE)
    ...
    for map_process in map_processes:
        if map_process.returncode != 0:
            print((b'map process {} exited with code {}').format(map_process.pid, map_process.returncode))
            print((b'partial results are in {}').format(config.output_filename))
            sys.exit(1)
