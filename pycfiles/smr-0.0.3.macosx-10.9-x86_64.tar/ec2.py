...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/smr/ec2.py
# Compiled at: 2014-08-02 17:21:45
from __future__ import absolute_import, division, print_function, unicode_literals
import boto, boto.ec2
from boto.exception import EC2ResponseError
import curses, datetime, os, paramiko, psutil
from Queue import Queue
import socket, subprocess, sys, threading, time
from .version import __version__
from .config import get_config, configure_job
from .shared import reduce_thread, progress_thread, write_file_to_descriptor, print_pid, get_param, add_message, add_str, ensure_dir_exists, get_args
...
def worker_stderr_read_thread(processed_files_queue, input_queue, chan, ssh, abort_event):
    ...
    for line in iter(stderr.readline, b''):
        ...
        splt = line.split(b',', 2)
        if len(splt) != 3:
            add_message((b'invalid message received from mapper: {}').format(line))
        else:
            file_status, file_size, file_name = splt
            if file_status == b'+':
                processed_files_queue.put((file_name, int(file_size)))
            elif file_status == b'!':
                add_message((b'error processing {}, requeuing...').format(file_name))
                input_queue.put(file_name)
            else:
                add_message((b'invalid status received from mapper: {}').format(file_status))
...
def wait_for_instance(instance):
    ...
    status = None
    print((b'getting status for instance {} ...').format(instance.id))
    ...
    print((b'waiting for instance {} ...').format(instance.id))
    while status == b'pending':
        time.sleep(2)
        status = instance.update()

    if status != b'running':
        print((b'Invalid status when starting instance {}: {}').format(instance.id, status))
        return False
    else:
        print((b'New instance {} started: {}').format(instance.id, instance.ip_address))
        return True
...
def initialize_instance_thread(config, instance, abort_event, ssh_key):
    ...
    print((b'waiting for ssh on instance {} {} ...').format(instance.id, instance.ip_address))
    while True:
        try:
    ...
    sftp.close()
    ssh.close()
    print((b'instance {} successfully initialized').format(instance.id))
...
def run_command(ssh, instance, command):
    ...
    try:
        ...
        stderr = chan.makefile_stderr(b'rb')
        for line in iter(stderr.readline, b''):
            print((b'instance {} stderr: {}').format(instance.id, line.rstrip()))

        exit_code = chan.recv_exit_status()
        if exit_code != 0:
            print((b'instance {} invalid exit code of {}: {}').format(instance.id, command, exit_code))
            ssh.close()
            return False
        print((b'instance {} successfully ran {}').format(instance.id, command))
        return True
    except socket.timeout:
        print((b'instance {} timed out while running {}').format(instance.id, command))
        ssh.close()
        return False
...
def start_worker(config, instance, abort_event, output_queue, processed_files_queue, input_queue, ssh_key):
    ...
    try:
        ...
        print((b'could not ssh to {} {}').format(instance.id, instance.ip_address))
        abort_event.set()
        sys.exit(1)

    chan = ssh.get_transport().open_session()
    chan.exec_command((b' ').join(get_args(b'smr-map', config, config.aws_ec2_remote_config_path)))
...
def curses_thread(config, abort_event, instances, reduce_processes, window, start_time, bytes_total):
    ...
    while not abort_event.is_set() and sleep_time > 0 and not abort_event.wait(sleep_time):
        ...
        add_str(window, 0, (b'smr-ec2 v{} - {} - elapsed: {}').format(__version__, datetime.datetime.ctime(now), now - start_time))
        i = 1
        for instance in instances:
            add_str(window, i, (b'  instance {} {}').format(instance.id, instance.ip_address))
            i += 1
            for _ in xrange(config.workers):
                add_str(window, i, b'    smr-map')
                i += 1

        for p in reduce_pids:
            print_pid(p, window, i, b'smr-reduce')
            i += 1

        add_str(window, i + 1, (b'job progress: {0:%}').format(get_param(b'bytes_processed') / bytes_total))
        add_str(window, i + 2, (b'last file processed: {}').format(get_param(b'last_file_processed')))
        messages = get_param(b'messages')[-10:]
        if len(messages) > 0:
            add_str(window, i + 3, b'last messages:')
            i += 4
            for message in messages:
                add_str(window, i, (b'  {}').format(message))
                i += 1
...
def run_job_on_instances(config, instances, abort_event, output_queue, processed_files_queue, input_queue, ssh_key):
    ...
    for chan, worker in workers:
        ...
        exit_code = chan.recv_exit_status()
        if exit_code != 0:
            sys.stderr.write((b'map process exited with code {}\n').format(exit_code))
...
def run_helper(config, ssh_key, bytes_total, file_names, instances):
    ...
    try:
        ...
        abort_event.set()
        print((b'user aborted. elapsed time: {}').format(str(datetime.datetime.now() - start_time)))
        sys.exit(1)

    if abort_event.is_set():
        sys.stderr.write(b'could not initialize workers\n')
        sys.exit(1)
    print((b'initialized instance(s) in: {}').format(str(datetime.datetime.now() - start_time)))
    start_time = datetime.datetime.now()
    try:
        if not config.output_filename:
            config.output_filename = (b'results/{}.{}.out').format(os.path.basename(config.config), datetime.datetime.now())
        ensure_dir_exists(config.output_filename)
        reduce_stdout = open(config.output_filename, b'w')
        reduce_process = subprocess.Popen(get_args(b'smr-reduce', config, config.config), stdin=subprocess.PIPE, stdout=reduce_stdout, stderr=subprocess.PIPE)
        ...
        if config.output_job_progress:
            curses.endwin()
        print((b'user aborted. elapsed time: {}').format(str(datetime.datetime.now() - start_time)))
        print((b'partial results are in {}').format(config.output_filename))
        sys.exit(1)
    ...
    if reduce_process.returncode != 0:
        print((b'reduce process {} exited with code {}').format(reduce_process.pid, reduce_process.returncode))
        print((b'partial results are in {}').format(config.output_filename))
        sys.exit(1)
    reduce_stdout.close()
    for message in get_param(b'messages'):
        print(message)

    print((b'done. elapsed time: {}').format(str(datetime.datetime.now() - start_time)))
    print((b'results are in {}').format(config.output_filename))


def run(config):
    configure_job(config)
    print(b'getting list of the files to process...')
    bytes_total, file_names = get_uris(config)
    if len(file_names) <= 0:
        sys.stderr.write(b'no files to process\n')
        sys.exit(1)
    print(b'generating a new RSA key...')
    ssh_key = paramiko.RSAKey.generate(bits=RSA_BITS)
    user_data = (b'#!/bin/bash -v\necho "ssh-rsa {public_key} smr" > /home/{user}/.ssh/authorized_keys\n').format(public_key=ssh_key.get_base64(), user=config.aws_ec2_ssh_username)
    ...
    print((b'requesting to start {} instances').format(config.aws_ec2_workers))
    reservation = conn.run_instances(image_id=config.aws_ec2_ami, min_count=config.aws_ec2_workers, max_count=config.aws_ec2_workers, user_data=user_data, instance_type=config.aws_ec2_instance_type, security_groups=config.aws_ec2_security_group, instance_profile_name=config.aws_iam_profile)
    instances = reservation.instances
    try:
        run_helper(config, ssh_key, bytes_total, file_names, instances)
    finally:
        instance_ids = [ instance.id for instance in instances ]
        print((b'terminating all instances: {}').format((b',').join(instance_ids)))
