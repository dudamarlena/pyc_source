...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/shabti/commands.py
# Compiled at: 2010-10-01 22:28:45
"""
...
try:
    ...
    from migrate.versioning.exceptions import KnownError, DatabaseAlreadyControlledError
    use_migrate = True
except ImportError:
    use_migrate = False
...
class SqlCommand(Command):
    ...
    parser.add_option('--echo', action='store_true', dest='echo', help='Print output to STDOUT')
    parser.add_option('--setup', action='store_true', dest='setup', help='Run websetup.py after tables created')
    parser.add_option('--table', dest='table', help='Perform action only on given table')
...
class ModelCommand(Command):
    ...
    default_verbosity = 3
    parser = Command.standard_parser(simulate=True)
    parser.add_option('--no-test', action='store_true', dest='no_test', help="Don't create the test; just the model")
    ...
    def command(self):
        ...
        try:
            ...
            if base_package.lower() == name.lower():
                raise BadCommand('Your model name should not be the same as the package name %r.' % base_package)
            name = name.replace('-', '_')
            validate_name(name)
            ...
            file_op.template_vars.update({'name': model_name, 'version': '0.1', 
               ...
               'sqlalchemy': True, 
               'zip_safe': False})
            file_op.copy_file(template='model.py_tmpl', dest=os.path.join('model', directory), filename=name, template_renderer=paste_script_template_renderer)
            file_op.copy_file(template='form.py_tmpl', dest=os.path.join('model', 'forms', directory), filename=name, template_renderer=paste_script_template_renderer)
            file_op.ensure_dir(os.path.join(base_package, 'fixtures', name))
            if not self.options.no_test:
                file_op.copy_file(template='test_model.py_tmpl', dest=os.path.join('tests', 'unit'), filename='test_' + testname, template_renderer=paste_script_template_renderer)
...
class RunnerCommand(Command):
    ...
    default_verbosity = 3
    parser = Command.standard_parser(simulate=True)
    parser.add_option('--create', action='store_true', dest='create', help='Create a new runner script')
    ...
    def command(self):
        try:
            ...
            if base_package.lower() == name.lower():
                raise BadCommand('Your script name should not be the same as the package name %r.' % base_package)
            name = name.replace('-', '_')
            validate_name(name)
            fullname = os.path.join(directory, name)
            if self.options.create:
                ...
                file_op.copy_file(template='script.py_tmpl', dest=os.path.join('scripts', directory), filename=name, template_renderer=paste_script_template_renderer)
            else:
                try:
                ...
                if mod is None:
                    raise BadCommand('Script %(name)s does not exist. Run paster runner --create %(name)s to create script', dict(name=name))
                mod.run(config_file)
        except BadCommand, e:
            raise BadCommand('An error occurred. %s' % e)
        except:
...
class MigrateCommand(Command):
    ...
    default_verbosity = 3
    parser = Command.standard_parser(simulate=True)
    parser.add_option('--dburi', dest='dburi', help='Database URI string')
    parser.add_option('--repository', dest='repository', help='Repository directory')
    parser.add_option('--version', dest='version', help='Database version')
    parser.add_option('--version_table', dest='version_table', help='Database version table')
    parser.add_option('--preview_py', action='store_true', dest='preview_py', help='Preview Python script before running')
    parser.add_option('--preview_sql', action='store_true', dest='preview_sql', help='Preview SQL before running')
    ...
    def _upgrade_command(self, dburi, repository, version):
        return migrate_api.upgrade(dburi, repository, version, preview_sql=self.options.preview_sql, preview_py=self.options.preview_py)

    def _downgrade_command(self, dburi, repository, version):
        return migrate_api.downgrade(dburi, repository, version, preview_sql=self.options.preview_sql, preview_py=self.options.preview_py)
...
class ScaffoldCommand(Command):
    ...
    default_verbosity = 3
    parser = Command.standard_parser(simulate=True)
    parser.add_option('--no-test', action='store_true', dest='no_test', help="Don't create the test; just the controller + views")
    ...
    def _modify_routes_py(self):
        ...
        if mapping in lines:
            ...
            if not found:
                print '# CUSTOM ROUTES HERE line was not found in the config/routes.py file. You have to maually add the statement\n' + mapping + 'to your routes.py'
            else:
    ...
    def _form_for(self):
        fields = {types.Integer: 'text', types.SmallInteger: 'text', types.String: 'text', 
           types.Unicode: 'text', 
           types.Text: 'textarea', 
    ...
    def _create_list_template(self):
        ...
        text += "{var f = document.createElement('form');f.style.display = 'none';" + " this.parentNode.appendChild(f);f.method = 'POST';f.action = this.href;" + " var m = document.createElement('input'); m.setAttribute('type', 'hidden'); " + "m.setAttribute('name', '_method'); m.setAttribute('value', 'delete'); f.appendChild(m);" + " var s = document.createElement('input'); s.setAttribute('type', 'hidden'); " + 'f.appendChild(s);f.submit(); }; return false;")}'
        text += '</td>\n'
        text += '    </tr>\n'
        text += '    % endfor\n'
        text += '</table>\n'
    ...
    def command(self):
        file_op = FileOp(source_dir=os.path.join(os.path.dirname(__file__), 'templates/scaffolding'))
        ...
        if self.package.lower() == self.pluralname.lower():
            raise BadCommand('Your controller name should not be the same as the package name %r.' % self.package)
        ...
        file_op.template_vars.update({'classname': self.classname, 'pluralclassname': self.pluralclassname, 
           ...
           'editable_columns_list': str([ x for x in self.scaffolded_class._sa_class_manager.mapper.c.keys() if x not in ('id',
                                                                                      'created_at',
                                                                                      'updated_at') ])})
        file_op.copy_file(template='altcontroller.py_tmpl', dest='controllers', filename=self.pluralname.replace('-', '_'), template_renderer=paste_script_template_renderer)
        file_op.ensure_dir(self.package + '/templates/' + self.pluralname)
        for filename in os.listdir(os.path.join(os.path.dirname(__file__), 'templates/scaffolding/templates')):
            print filename
            file_op.copy_file(template='templates/' + filename, filename=filename.split('.')[0] + '.mako', add_py=False, dest='templates/' + self.pluralname + '/', template_renderer=paste_script_template_renderer)

        file_op.ensure_dir(self.package + '/public/css')
        file_op.copy_file(template='scaffold.css_tmpl', dest='public/css', filename='scaffold.css', add_py=False, template_renderer=paste_script_template_renderer)
        if not self.options.no_test:
            file_op.copy_file(template='test_restcontroller.py_tmpl', dest=os.path.join('tests', 'functional'), filename='test_' + self.pluralname, template_renderer=paste_script_template_renderer)
        self._modify_routes_py()
        for action in ['new', 'edit']:
            file_op.ensure_file(self.package + '/templates/' + self.pluralname + '/' + action + '.mako', self._create_form(action))

        file_op.ensure_file(self.package + '/templates/' + self.pluralname + '/show.mako', self._create_show_template())
        file_op.ensure_file(self.package + '/templates/' + self.pluralname + '/list.mako', self._create_list_template())