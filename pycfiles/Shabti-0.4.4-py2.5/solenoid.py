...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/shabti/templates/moinmoin/data/moin/data/plugin/theme/solenoid.py
...
import MoinMoin.i18n, re, os, glob, MoinMoin, StringIO

class Theme(ThemeBase):
    name = 'solenoid'
    _ = lambda x: x
    icons = {'help': ('%(page_help_contents)s', 'moin-help.png', 16, 16), 
       'find': ('%(page_find_page)s', 'moin-search.png', 16, 16), 
       'diff': (
              _('Diffs'), 'moin-diff.png', 20, 16), 
       'info': (
              _('Info'), 'moin-info.png', 16, 16), 
       'edit': (
              _('Edit'), 'moin-edit.png', 16, 16), 
       'unsubscribe': (
                     _('Unsubscribe'), 'moin-unsubscribe.png', 16, 16), 
       'subscribe': (
                   _('Subscribe'), 'moin-subscribe.png', 16, 16), 
       'addquicklink': (
                      _('Add quicklink'), 'moin-addquicklink.png', 16, 16), 
       'delquicklink': (
                      _('Delete quicklink'), 'moin-delquicklink.png', 16, 16), 
       'raw': (
             _('Raw'), 'moin-raw.png', 16, 16), 
       'xml': (
             _('XML'), 'moin-xml.png', 20, 13), 
       'print': (
               _('Print'), 'moin-print.png', 16, 16), 
       'view': (
              _('View'), 'moin-show.png', 16, 16), 
       'home': (
              _('Home'), 'moin-home.png', 16, 16), 
       'up': (
            _('Up'), 'moin-parent.png', 16, 16), 
       'attach': ('%(attach_count)s', 'moin-attach.png', 12, 12), 
       'iconattach': ('Add/Manage files', 'moin-iconattach.png', 16, 16), 
       'rss': (
             _('[RSS]'), 'moin-rss.png', 39, 14), 
       'deleted': (
                 _('[DELETED]'), 'moin-deleted.png', 80, 16), 
       'updated': (
                 _('[UPDATED]'), 'moin-updated.png', 80, 16), 
       'renamed': (
                 _('[RENAMED]'), 'moin-renamed.png', 80, 16), 
       'conflict': (
                  _('[CONFLICT]'), 'moin-conflict.png', 23, 16), 
       'new': (
             _('[NEW]'), 'moin-new.png', 80, 16), 
       'diffrc': (
                _('[DIFF]'), 'moin-diff.png', 20, 16), 
       'bottom': (
                _('[BOTTOM]'), 'moin-bottom.png', 12, 12), 
       'top': (
             _('[TOP]'), 'moin-top.png', 12, 12), 
       ...
       'diff-big': (
                  _('Diffs'), 'moin-diff-big.png', 31, 25), 
       'info-big': (
                  _('Info'), 'moin-info-big.png', 25, 25), 
       'edit-big': (
                  _('Edit'), 'moin-edit-big.png', 25, 25), 
       'unsubscribe-big': (
                         _('Unsubscribe'), 'moin-unsubscribe-big.png', 25, 25), 
       'subscribe-big': (
                       _('Subscribe'), 'moin-subscribe-big.png', 25, 25), 
       'raw-big': (
                 _('Raw'), 'moin-raw-big.png', 25, 25), 
       'print-big': (
                   _('Print'), 'moin-print-big.png', 25, 25), 
       'view-big': (
                  _('View'), 'moin-show-big.png', 25, 25), 
       'delquicklink-big': (
                          _('Delete quicklink'), 'moin-delquicklink-big.png', 25, 25), 
       'addquicklink-big': (
                          _('Add quicklink'), 'moin-addquicklink-big.png', 25, 25), 
       'attach-big': (
                    _('Add/Manage files'), 'moin-iconattach-big.png', 25, 25)}
    del _
    ...
    def header(self, d):
        ...
        parts = [self.emit_custom_html(self.cfg.page_header1),
         ...
         '<div class="%s">' % wrapper_class,
         sidebar,
         '<div class="content"%s>\n' % self.content_lang_attr(),
         "<!--[if lt IE 7]><div class='outdated-ie'><p><b>%s! </b>%s</p></div><![endif]-->" % (
          _('Attention'), _('You are using an outdated version of Internet Explorer which is not supported by this site. Please update your browser.'))]
        return ('').join(parts)
    ...
    def title(self, d):
        ...
        if self.request.action != 'print':
            try:
                screen_title = self.title_with_separators
            except AttributeError:
                screen_title = modernized_theme.title
            else:
                return '<h1>%s%s</h1>' % (modernized_theme.interwiki(d), screen_title(d))
        ...
        for line in lines:
            ...
            if not line.startswith('#') and not line.startswith('<<') and not line == '':
                if re.sub('^\\s*=+\\s+[^=]+\\s+=+\\s*$', '', line) == '':
                    return '<span id="pagelocation">%s: %s</span>' % (_('Page Name'), page_path)
                break

        interwiki_name = '%s: ' % (cfg.interwikiname or 'Wiki')
        prefix = ['', interwiki_name][cfg.show_interwiki]
        return '<h1 id="pagelocation">%s%s</h1>' % (prefix, page_path)
    ...
    def sidebar(self, d, **keywords):
        ...
        request = self.request
        _ = self._
        sidebar = request.getPragma('sidebar') or 'SideBar'
        ...
        return '<div class="sidebar">%s<div id="sidebar-end"></div></div>' % buff.getvalue()
    ...
    def editbar(self, d, **keywords):
        ...
        if 'Discussion' in enabled_items and self.request.getPragma('supplementation-page', suplementation_enabled) in (
         True, 1, 'on', '1'):
        ...
        return '<div class="editbar"> %s </div>' % (' ').join(result)
    ...
    def login_link(self, page):
        _ = self._
        params = wikiutil.quoteWikinameURL(page.page_name) + '?action=login'
        text = _('Locked')
        return wikiutil.link_tag(self.request, params, text, css_class='password')

    def info_link(self, page):
        _ = self._
        params = wikiutil.quoteWikinameURL(page.page_name) + '?action=info'
        text = _('History')
    ...
    def revert_link(self, page):
        ...
        if not (self.request.rev and page.isWritable() and self.request.user.may.revert(page.page_name)):
            return ''
        _ = self._
        params = wikiutil.quoteWikinameURL(page.page_name) + '?action=revert&rev=%d' % self.request.rev
        text = _('Revert')
        return wikiutil.link_tag(self.request, params, text, css_class='revert')

    def attach_link(self, page):
        _ = self._
        params = wikiutil.quoteWikinameURL(page.page_name) + '?action=AttachFile'
        text = _('Attachments')
    ...
    def admin_link(self, page):
        _ = self._
        params = wikiutil.quoteWikinameURL(page.page_name) + '?action=PageActions'
        text = _('All Actions')
    ...
    def gotobar(self, d, **k):
        ...
        if request.cfg.navi_bar:
            for text in request.cfg.navi_bar:
                ...
                if pagename == current:
                    ...
                    cls = 'wikilink'
                items.append('<li class="%s">%s</li>' % (cls, link))
                found[pagename] = 1
        ...
        return '<ul class="gotobar">%s<li class="clear"></li></ul>' % (' ').join(items)

    def html_stylesheets(self, d):

        def ie_version_spec(ie_style):
            return re.compile('msie/.*_ie').sub('IE ', ie_style)

        def ie_style(style, ie_version):
            return 'msie/%s_ie%d' % (style, ie_version)

        def is_ie_style(style):
            return style.startswith('msie')
    ...
    def recentchanges_entry(self, d):
        ...
        if d['comments']:
            ...
            rccomm = ''
        html = '<li><b class="rctime">%s</b> %s %s . . . . <span class="rcauth">%s</span> <i class="rccomm">%s</i></li>' % (
         d['time_html'],
         d['pagelink_html'],
         d['icon_html'],
         (', ').join(d['editors']),
    ...
    def recentchanges_daybreak(self, d):
        return '</ul><h3 class="rcdaybreak">%s</h3><ul>' % d['date']

    def recentchanges_header(self, d):
        return '<div class="recentchanges"%s><ul>' % self.ui_lang_attr()
