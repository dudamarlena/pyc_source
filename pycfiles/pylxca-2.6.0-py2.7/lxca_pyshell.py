...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pylxca/pylxca_cmd/lxca_pyshell.py
# Compiled at: 2019-12-06 06:32:45
"""
@since: 5 Feb 2016
...
def set_interactive():
    ...
    """
    global __version__
    ns = {'connect': connect, 'disconnect': disconnect, 
       'chassis': chassis, 
       'cmms': cmms, 
       'fans': fans, 
       'fanmuxes': fanmuxes, 
...
@example 
    ...
    con = _validate_param(keylist, long_short_key_map, mandatory_options_list, optional_keylist, mutually_exclusive_keys, param_dict, *args, **kwargs)
...
@example 
    ...
    keylist = [
     'con', 'uuid', 'chassis', 'status', 'modify', 'metrics']
...
@example 
    ...
    param_dict = {}
    con = None
    long_short_key_map = {'ip': 'i', 'user': 'u', 'pw': 'p', 'rpw': 'r', 
       'job': 'j', 'force': 'f', 'storedcredential_id': 's'}
...
@example 
    ...
    param_dict = {}
    con = None
    long_short_key_map = {'id': 'i', 'endpoint': 'e', 'restart': 'r', 'type': 't', 
       'name': 'n', 'pattern_update_dict': 'p'}
...
@example
    ...
    param_dict = {}
    con = None
    long_short_key_map = {'id': 'i', 'name': 'n', 'endpoint': 'e', 'restart': 'r', 'powerdown': 'p', 
       'force': 'f'}
...
@example
    ...
    param_dict = {}
    con = None
    long_short_key_map = {'info': 'i', 'job': 'j', 'uuid': 'u', 
       'policy': 'p', 'type': 't'}
    keylist = ['con', 'subcmd', 'info', 'job', 'uuid', 'policy', 'type']
...
@example 
    ...
    param_dict = {}
    con = None
    long_short_key_map = {'key': 'k', 'mt': 'm', 'scope': 's', 
       'fixids': 'f', 'type': 't'}
    keylist = ['con', 'subcmd', 'key', 'mt', 'scope', 'fixids', 'type']
...
rep = updatecomp(con, 'apply', action='apply', mode='immediate', dev_list=dev_json)
    ...
    param_dict = {}
    con = None
    long_short_key_map = {'query': 'q', 'mode': 'm', 'action': 'a', 'cmm': 'c', 'switch': 'w', 'server': 's', 'storage': 't', 
       'dev_list': 'l'}
...
@example 
    ...
    param_dict = {}
    con = None
    long_short_key_map = {'id': 'i', 'uuid': 'u', 'state': 's', 
       'cancel': 'c', 'delete': 'd'}
    keylist = ['con', 'id', 'uuid', 'state', 'cancel', 'delete']
...
@example
    ...
    param_dict = {}
    con = None
    long_short_key_map = {'jobUID': 'j', 'children': 'c', 'action': 'a', 
       'updateList': 'u', 'template': 't'}
...
@example
    ...
    param_dict = {}
    command_name = sys._getframe().f_code.co_name
    long_short_key_map = {'uuid': 'u', 'name': 'n', 'description': 'd', 'type': 't', 'solutionVPD': 's', 'members': 'm', 
       'criteria': 'c'}
    keylist = [
     'con', 'subcmd', 'uuid', 'name', 'description',
...
def _validate_param(keylist, long_short_key_map, mandatory_options_list, optional_keylist, mutually_exclusive_keys, param_dict, *args, **kwargs):
    ...
    if not set(keylist + list(long_short_key_map.values())).issuperset(set(kwargs.keys())):
        LOGGER.error(' Invalid Input args: %s unsupported argument passed' % list(set(kwargs.keys()).difference(set(keylist + long_short_key_map.values()))))
        raise ValueError('Invalid Input Arguments')
    return con
...
@example
    ...
    param_dict = {}
    con = None
    long_short_key_map = {'key': 'k', 'fixids': 'f', 'type': 't', 
       'jobid': 'j'}
    keylist = [
     'con', 'subcmd', 'key', 'fixids', 'type', 'files', 'jobid']
...
@example
    ...
    param_dict = {}
    con = None
    long_short_key_map = {'id': 'i', 'query_solutionGroups': 'q', 'solutionGroups': 's', 
       'targetResources': 't', 'all_rules': 'a'}
    keylist = [
     'con', 'id', 'query_solutionGroups',
...
@example
    ...
    param_dict = {}
    con = None
    long_short_key_map = {'id': 'i', 'user_name': 'u', 'description': 'd', 
       'password': 'p'}
    keylist = [
     'con', 'id', 'user_name',
