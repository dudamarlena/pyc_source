...
class InteractiveCommand(object):
    ...
    def get_argparse_options(self):
        parser = argparse.ArgumentParser(prog=self.get_name(), description=self.get_short_desc(), formatter_class=argparse.RawDescriptionHelpFormatter, epilog=self.get_additional_detail(), add_help=self.get_help_option())
        ...
        if subcmd_list:
            ...
            for subcmd in subcmd_list:
                ...
                parser_internal = sub_parser.add_parser(sub_cmd_name, formatter_class=argparse.RawDescriptionHelpFormatter, help=subcmd['help'], epilog=self._process_epilog(subcmd['additional_detail']))
