...
class lxca_rest(object):
    ...
    def set_managementserver(self, url, session, action, files, jobid, fixids):
        url = url + '/managementServer/updates'
        try:
            if not action == None and action == 'apply':
            ...
            if not action == None and action == 'import':
                ...
                return resp
            if not action == None and action == 'acquire':
                ...
                resp = session.post(url, data=json.dumps(payload), verify=False, timeout=REST_TIMEOUT)
                return resp
            if not action == None and action == 'refresh':
                url = url + '?action=refresh'
                payload = {}
                payload['mts'] = ['lxca']
                resp = session.post(url, data=json.dumps(payload), verify=False, timeout=REST_TIMEOUT)
                return resp
            if not action == None and action == 'delete':
                if fixids:
                    url = url + '/' + fixids
                else:
    ...
    def solution_resourcegroups(self, url, session, uuid, name, desc, type, solutionVPD, members, criteria):
        ...
        try:
            if uuid:
                ...
                param_dict['description'] = desc
                param_dict['type'] = type
                if type == 'solution' and isinstance(solutionVPD, dict) and set(['id', 'machineType', 'model', 'serialNumber', 'manufacturer']).issubset(set(solutionVPD.keys())):
    ...
    def osimage_import(self, url, session, kwargs):
        ...
        try:
            ...
            if 'jobId' in kwargs:
                ...
                m = MultipartEncoder(fields={'name': 'uploadedfile', 'uploadedfile': (os.path.basename(kwargs['file']), open(kwargs['file'], 'rb'), 'application/octet-stream')})
                monitor = MultipartEncoderMonitor(m, callback)
