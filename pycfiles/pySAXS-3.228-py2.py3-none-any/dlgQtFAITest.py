...
# [GCC 8.4.0]
# Embedded file name: C:\Anaconda2\lib\site-packages\pySAXS\guisaxs\qt\dlgQtFAITest.py
# Compiled at: 2018-09-18 10:14:15
...
class FAIDialogTest(QtWidgets.QDialog):
    ...
    def importXML(self, filename=None):
        if filename is not None:
            ...
            self.ui.edit_pixelsize.setText(str(self.fai._xmldirectory['user.PixelSize']))
    ...
    def importPONI(self, filename=None):
        if filename is not None:
            ...
            self.ui.edit_pixelsize.setText(str(out['pixelX'] / 10000))
            self.ui.edit_wavelength.setText(str(g.get_wavelength() * 10000000000.0))
        return
    ...
    def prepare(self):
        ...
        self.pixelsize = float(self.ui.edit_pixelsize.text())
        self.wavelength = float(self.ui.edit_wavelength.text())
        self.fai.set_wavelength(self.wavelength * 1e-09)
        self.fai.setFit2D(self.dd * 10, centerX=self.centerx, centerY=self.centery, tilt=self.tilt, pixelX=self.pixelsize * 10000.0, pixelY=self.pixelsize * 10000.0)
    ...
    def loadSample(self):
        ...
        try:
            ...
            print (
             'error in opening ', imageFilename)
            self.im = None
            self.ui.labelSample.setPixmap(self.pixmapExcl)
    ...
    def click(self, obj=None):
        ...
        if name == 'Close':
            ...
            if filename:
                ...
                self.fai.saveGeometryXML(filename, self.centerx, self.centery, self.dd, 90 + self.tilt, self.pixelsize, self.wavelength, str(self.ui.txt_MaskFilename.text()), self.qdiv)
        return
