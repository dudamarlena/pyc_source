...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/tools/assetslibrary/widgets/assetslibrary.py
# Compiled at: 2020-05-13 19:09:57
# Size of source mod 2**32: 20874 bytes
"""
...
class ArtellaAssetsLibraryWidget(base.BaseWidget, object):
    ...
    def ui(self):
        ...
        self._assets_viewer = artellapipe.AssetsViewer(project=(self._project),
          column_count=2,
          parent=self)
    ...
    def update_cache(self, force=False):
        ...
        for i in range(self._assets_viewer.rowCount()):
            for j in range(self._assets_viewer.columnCount()):
                ...
                asset_file = asset_widget.asset.get_file('rig',
                  status=(defines.ArtellaFileStatus.PUBLISHED), must_exist=False)
                asset_name = asset_widget.asset.get_name()
                if asset_file:
                    if os.path.exists(asset_file):
                        continue
                    if asset_name not in self._cache:
                        self._cache[asset_name] = {'item':item,  'has_label':False, 
                ...
                try:
                    try:
                        ...
                        for file_type, extensions in file_info.items():
                            if not extensions:
                                artellapipe.logger.warning('No Extension defined for File Type "{}" in artellapipe.tools.assetslibrary configuration file!'.format(file_type))
                            else:
                                for extension, operation in extensions.items():
                                    if operation == 'reference':
                                        res = asset_widget.asset.import_file(extension=extension,
                                          file_type=file_type,
                                        ...
                                        res = asset_widget.asset.import_file(extension=extension,
                                          ...
                                          reference=False,
                                          status=(defines.ArtellaFileStatus.PUBLISHED))
                                    if res and self._fit_camera_cbx.isChecked():
                                        try:
                                            ...
                                            artellapipe.logger.warning('Impossible to fit camera view to referenced objects | {}!'.format(exc))
