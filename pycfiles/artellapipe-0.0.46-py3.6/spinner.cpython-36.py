...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/widgets/spinner.py
    ...
    def __init__(self, spinner_type=SpinnerType.Thumb, parent=None):
        ...
        if spinner_type == SpinnerType.Thumb:
            for i in range(self.THUMB_INFO[0]):
                self._spin_icons.append(tpDcc.ResourcesMgr().pixmap(self.THUMB_INFO[1].format(i + 1)))

        else:
            if spinner_type == SpinnerType.Loading:
                for i in range(self.LOADING_INFO[0]):
                    self._spin_icons.append(tpDcc.ResourcesMgr().pixmap(self.LOADING_INFO[1].format(i + 1)))

            else:
                for i in range(self.CIRCLE_INFO[0]):
                    self._spin_icons.append(tpDcc.ResourcesMgr().pixmap(self.CIRCLE_INFO[1].format(i + 1)))
        ...
        self._bg.setStyleSheet('#background {border-radius: 3px;border-style: solid;border-width: 1px;border-color: rgb(32,32,32);}')
