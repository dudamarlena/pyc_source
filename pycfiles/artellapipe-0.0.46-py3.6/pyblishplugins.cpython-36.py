...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/managers/pyblishplugins.py
# Compiled at: 2020-05-13 18:51:06
# Size of source mod 2**32: 5251 bytes
"""
...
import os, logging, inspect, importlib, traceback, pyblish.api, tpDcc
...
class PyblishPluginsManager(object):
    ...
    def set_project(self, project):
        ...
        self._config = tpDcc.ConfigsMgr().get_config(config_name='artellapipe-pyblishplugins',
            ...
            try:
                ...
                for sub_module in loader.walk_packages([pyblish_mod_dir]):
                    ...
                    try:
                        ...
                        msg = 'Impossible to register Pyblish Plugin: {}. Exception raised: {} | {}'.format(qname, exc, traceback.format_exc())
                        LOGGER.warning(msg)
                        continue
                    ...
                    for cname, obj in inspect.getmembers(mod, inspect.isclass):
                        if issubclass(obj, (pyblish.api.InstancePlugin, pyblish.api.ContextPlugin)):
                            ...
                            if plugin_name in self._registered_plugins:
                                LOGGER.info('Already registered a plugin with name: "{}". Overwriting ...'.format(plugin_name))
                            self._registered_plugins[obj.__name__] = {'module':mod,  'path':plugin_path, 
                             'class':obj}
