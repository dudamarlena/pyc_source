...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/managers/tags.py
...
class ArtellaTagsManager(object):
    ...
    def set_project(self, project):
        ...
        self._config = tp.ConfigsMgr().get_config(config_name='artellapipe-tags',
    ...
    def get_tag_data_nodes(self, project, as_tag_nodes=False):
        ...
        for obj in objs:
            valid_tag_data = tp.Dcc.attribute_exists(node=obj,
              attribute_name=(self.TagDefinitions.TAG_TYPE_ATTRIBUTE_NAME))
            if valid_tag_data:
                tag_type = tp.Dcc.get_attribute_value(node=obj,
                  attribute_name=(self.TagDefinitions.TAG_TYPE_ATTRIBUTE_NAME))
                if tag_type:
                    if tag_type == project.tag_type_id:
                        if as_tag_nodes:
                            if artellapipe.TagNode:
    ...
    def get_tag_info_nodes(self, as_tag_nodes=False):
        ...
        objs = tp.Dcc.all_scene_objects()
        for obj in objs:
            valid_tag_info_data = tp.Dcc.attribute_exists(node=obj,
              attribute_name=(self.TagDefinitions.TAG_INFO_ATTRIBUTE_NAME))
            if valid_tag_info_data:
                if as_tag_nodes:
                    if artellapipe.TagNode:
                        tag_info = tp.Dcc.get_attribute_value(node=obj,
                          attribute_name=(self.TagDefinitions.TAG_INFO_ATTRIBUTE_NAME))
