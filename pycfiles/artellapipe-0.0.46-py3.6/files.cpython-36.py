...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/managers/files.py
# Compiled at: 2020-05-13 18:51:06
# Size of source mod 2**32: 20513 bytes
"""
...
import os, locale, logging, inspect, tempfile, importlib, six, tpDcc as tp
...
class ArtellaFilesManager(object):
    ...
    def set_project(self, project):
        ...
        self._config = tp.ConfigsMgr().get_config(config_name='artellapipe-files',
    ...
    def get_file_type_extensions(self, file_type):
        ...
        if file_type:
            return file_type_info.get('extensions', list())
        else:
            return list()
    ...
    def fix_path(self, path_to_fix, clean_path=True):
        ...
        if not project_var:
            ...
            if path_to_fix.startswith('${}/'.format(project_env_var)):
                ...
                if path_to_fix.startswith('${}/'.format(root_prefix)):
    ...
    def unlock_file(self, file_path=None, notify=False, warn_user=True):
        ...
        if not file_path:
            ...
            if warn_user:
                ...
                res = tp.Dcc.confirm_dialog(title='Unlock File',
                  message=msg,
                  button=[
                 'Yes', 'No'],
                  default_button='Yes',
    ...
    def upload_working_version(self, file_path=None, skip_saving=False, notify=False, comment=None, force=False):
        ...
        if not valid_path:
            ...
            if comment:
                ...
                comment = qtutils.get_comment(text_message=('Make New Version ({}) : {}'.format(current_version, short_path)),
                  title='Comment',
            ...
            if comment:
                ...
                if notify:
                    self._project.notify(title='New Working Version',
                      msg=('Version {} for file "{}" uploaded to Artella server successfully!'.format(current_version, file_path)))
                return True
            return False
    ...
    def _check_file_path(self, file_path):
        ...
        if not file_path:
            ...
            if not file_path:
                ...
                return False
            file_path.startswith(self._project.get_path()) or LOGGER.error('File "{}" is not a valid project path because it is not located in Project Folder!'.format(file_path))
            return
        else:
