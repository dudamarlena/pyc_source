...
# Size of source mod 2**32: 31531 bytes
"""
...
class AbstractFile(object):
    ...
    def view_locally(self):
        ...
        if not os.path.isdir(asset_path):
            artellapipe.logger.warning('Impossible to open asset path locally because path for Asset "{}" : "{}" does not exists!'.format(self.get_name(), asset_path))
            return
    ...
    def solve_path(self, file_type, template_dict, template=None, extra_dict=None, version=None, fix_path=False, only_local=False, status=defines.ArtellaFileStatus.WORKING, must_exist=True, next_version=False, check_file_type=True):
        if extra_dict is None:
            ...
            if check_file_type:
                if file_type not in available_files_types:
                    artellapipe.logger.warning('File Type "{}" is not valid! Supported File Types: {}'.format(file_type, available_files_types.keys()))
                    return
            if not defines.ArtellaFileStatus.is_valid(status):
                artellapipe.logger.warning('Given File Artella Sync Status: {} is not valid! Supported Statuses: {}'.format(status, defines.ArtellaFileStatus.supported_statuses()))
                return
                ...
                for k, v in file_type_dict.items():
                    if k not in extra_dict:
                        ...
                        if v and v != extra_dict[k]:
                            artellapipe.logger.info('Updating Template Dict from file type specific value: {} | {} >>> {}'.format(k, extra_dict[k], file_type_dict[k]))
                            extra_dict[k] = v
        ...
        if not template:
            ...
            template or artellapipe.logger.warning('Impossible to retrieve file path because template "{}" is not in configuration file'.format(file_template_name))
            return
        else:
    ...
    def import_file(self, status=None, extension=None, file_type=None, sync=False, reference=False):
        ...
        if not status:
            ...
            if extension not in available_extensions:
                artellapipe.logger.warning('Impossible to reference file with extension "{}". Supported extensions: {}'.format(extension, available_extensions))
                return False
            file_types = artellapipe.FilesMgr().get_file_types_by_extension(extension)
            if not file_types:
                artellapipe.logger.warning('Impossible to reference file by its extension ({}) because no file types are registered!'.format(extension))
                return False
            if len(file_types) > 1:
                if not file_type:
                    artellapipe.logger.warning('Multiple file types found with extension: {}. Do not know which file should imported!'.format(extension))
                    return False
            if len(file_types) == 1:
    ...
    def open_in_artella(self):
        ...
        if not artella_url:
            artellapipe.logger.warning('Impossible to open Artella URL for asset "{}" : "{}"'.format(self.get_name(), artella_url))
            return
