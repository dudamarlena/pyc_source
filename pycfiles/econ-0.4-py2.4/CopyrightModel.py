...
# [GCC 8.4.0]
# Embedded file name: build/bdist.darwin-8.8.0-Power_Macintosh/egg/econ/CopyrightModel.py
# Compiled at: 2007-04-18 06:57:54
import math, econ.log
...
class CopyrightIncomeSummary:
    ...
    def calculate(self):
        ...
        while incomePV > self.incomeThreshold or ii < self.minTimePeriod:
            incomePV = self.incomeModel.getIncome(ii) * self.discountRate.getDiscount(ii + self.firstPaymentDelay)
            total += incomePV
            self.presentValueOfIncome.append(incomePV)
            self.cumulativeTotals.append(total)
            if ii > self.maxTimePeriod:
                logger.warning('Income PV was NOT below threshold after ' + str(self.maxTimePeriod) + ' periods. Income in current period was: ' + incomePV)
                break
            ii += 1
