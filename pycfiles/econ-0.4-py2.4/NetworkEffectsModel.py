...
# [GCC 8.4.0]
# Embedded file name: build/bdist.darwin-8.8.0-Power_Macintosh/egg/econ/NetworkEffectsModel.py
...
class NetworkEffectsModel:
    ...
    def __init__(self, nu_A, nu_B, het_A, het_B, priceFunction_A, priceFunction_B, constantList=[]):
        """
    ...
    def equilibriaFunction(self, tindex):
        ...
        @return: A(tindex)
        """
        return self.networkAdvantage(tindex) + self.heterogeneityDisadvantage(tindex) + self._priceFunction_A(self._price_A) + self._priceFunction_B(self._price_B)
    ...
    def getDemand(self, price, startSearch, endSearch):
        ...
        @param endSearch: [startSearch, endSearch] is an area in which to
            ...
            equilibrium.
        """
        (maximalPrice, boundaryDemand) = self._findMaximalAllowablePrice(startSearch, endSearch)
        logger.debug('Maximal allowable price:' + str(maximalPrice))
        if price > maximalPrice:
            logger.info('getDemand: price (' + str(price) + ') exceeeded maximal price (' + str(maximalPrice) + '). Returning demand of 0')
            return 0
        self._price_A = price
    ...
    def solveMonopolistPriceProblem(self, startSearch, endSearch):
        ...
        """
        (maximalPrice, tmp1) = self._findMaximalAllowablePrice(startSearch, endSearch)
...
u(1-t^{e}) + Heterogeneity
        ...
        networkBPart = (1.0 - equilibriumValue) * self._nu_B(equilibriumValue)
        ...
        logger.debug('Welfare: (netA, netB, hetA, hetB): ' + str((networkAPart, networkBPart, hetAPart, hetBPart)))
        totalWelfare = networkAPart + networkBPart + hetAPart + hetBPart + priceAPart + priceBPart
        return totalWelfare