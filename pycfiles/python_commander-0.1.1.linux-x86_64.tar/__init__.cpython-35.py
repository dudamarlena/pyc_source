...
# [GCC 8.4.0]
# Embedded file name: /home/enthys/workspace/python/python_commander/venv/lib/python3.5/site-packages/python_commander/__init__.py
# Compiled at: 2018-11-01 11:07:12
...
class Commander:
    ...
    def command(cls, command, action, description=''):
        cls.commands.append({'command': command, 'action': action, 'description': description})
        ...
        for list_command in cls.commands:
            if list_command['command'] == command:
                if hasattr(list_command['action'], '__call__'):
                    ...
                    result = check_output(list_command['action'], shell=True).decode('utf-8').rstrip()
    ...
    def list_possible_commands(cls):
        ...
        for command in cls.commands:
            if command['description'] is not '':
                list_commands.append({'command': command['command'], 'description': command['description'], 'action': command['action']})
            else:
                list_commands.append({'command': command['command'], 'action': command['action']})

        chosenAction, index = pick(list_commands, title='Pick a command:', indicator='->')
    ...
    def gather_commands(cls):
        ...
        spec = importlib.util.spec_from_file_location('set_commands', commands_path)
