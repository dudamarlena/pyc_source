    ...
    if is_none_or_empty(package_name):
        ...
        if is_none_or_empty(repo_name):
            ...
            if is_none_or_empty(s3_bucket) is False:
                doc_domain_s3 = 'http://{s3_bucket}.s3.amazonaws.com/{package_name}'.format(s3_bucket=s3_bucket,
            ...
            click.secho("There's author introduction file at {repo_name}/docs/source/author.rst, you should change it to your own introduction.".format(repo_name=repo_name),
              fg='red')
...
@click.option('--supported_py_ver',
  prompt="(optional) Enter python version list your package will support, seperate by comma. For example: '2.7.13, 3.4.6'. All available version are listed here: https://github.com/pyenv/pyenv/tree/master/plugins/python-build/share/python-build",
  default='')
...
@click.option('--maintainer_email',
  ...
  default='')
@click.option('--rtd_name',
  prompt='(optional) If use ReadTheDocs to host your document, please specify the project name (will be your domain prefix).',
  default='')
@click.option('--s3_bucket',
  prompt='(optional) If use AWS S3 to host your document, please specify the bucket name.',
  default='')
@click.option('--doc_service',
...
def _initiate_project(package_name, repo_name, github_username, supported_py_ver, author_name, author_email, maintainer_name, maintainer_email, rtd_name, s3_bucket, doc_service):
    if supported_py_ver is None:
        ...
        initiate_project(package_name=package_name,
          repo_name=repo_name,
          github_username=github_username,
          supported_py_ver=supported_py_ver,
          author_name=author_name,
