...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/pygitrepo-project/pygitrepo/integrate.py
        ...
        for table in div.find_all('table'):
            for td in table.find_all('td', class_='content'):
                for a in td.find_all('a'):
                    try:
                        title = a['title']
                        if title != 'patches' and title == a.text:
...
def validate_pyenv_ver(pyenv_ver):
    if pyenv_versions and pyenv_ver not in pyenv_versions:
        raise ValueError('%s is not a valid pyenv supported python version! Go to %s see supported version list.' % (
    ...
    try:
        ...
        if float(py_ver) < 2.6:
            ...
            if py_ver.startswith('3.7'):
                return 'py37'
            return 'py' + pyenv_ver.replace('.', '')
    except:
        raise ValueError('Cannot find supported tox version for %s!' % pyenv_ver)
