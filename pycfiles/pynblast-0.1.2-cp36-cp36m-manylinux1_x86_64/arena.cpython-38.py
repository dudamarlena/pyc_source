...
# Embedded file name: /home/barnesc/work/code/nblast-rs/nblast-py/pynblast/arena.py
# Compiled at: 2020-04-20 10:06:55
...
class NblastArena:
    ...
    def __init__(self, dist_bins: List[float], dot_bins: List[float], score_mat: np.ndarray):
        """
    ...
    def query_target(self, query_idx: Idx, target_idx: Idx, normalize: bool=False, symmetry: Optional[Symmetry]=None) -> float:
    ...
    def queries_targets(self, query_idxs: List[Idx], target_idxs: List[Idx], normalize: bool=False, symmetry: Optional[Symmetry]=None) -> Dict[(Tuple[(Idx, Idx)], float)]:
        """Query all combinations of some query neurons against some target neurons.

        See the ``query_target`` method for more details.
        """
        return self._impl.queries_targets(query_idxs, target_idxs, bool(normalize), symmetry)
    ...
    def tangents(self, idx, rectify=False) -> np.ndarray:
        ...
        if rectify:
            return rectify_tangents(out, True)
        return out