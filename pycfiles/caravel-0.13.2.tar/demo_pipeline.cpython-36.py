...
# [GCC 8.4.0]
# Embedded file name: /Users/mstolarczyk/Uczelnia/UVA/code/caravel/caravel/examples/caravel_demo/metadata/../pipeline/demo_pipeline.py
# Compiled at: 2019-08-08 08:48:54
...
import os, pypiper, csv
parser = ArgumentParser(description='A pipeline to count the number of lines, file sizes and line lengts distribution.')
parser = pypiper.add_pypiper_args(parser, groups=['pypiper', 'common', 'looper'], args=['output-parent', 'config'], required=[
 'sample-name'])
...
line_length_dist_cmd = "awk '{{print length}}' {input} | uniq > {distr_output}".format(input=input_file, distr_output=distr_output)
