...
# Embedded file name: build/bdist.linux-x86_64/egg/pycovjson/read_netcdf.py
# Compiled at: 2016-11-10 06:15:19
# Size of source mod 2**32: 12747 bytes
import netCDF4
...
class NetCDFReader(object):
    ...
    def is_y(self, var):
        ...
        """
        y_list = [
         'lat', 'latitude', 'LATITUDE', 'Latitude', 'y']
        if self.get_units(var) == 'degrees_north':
            return True
        else:
            if self.get_name(var) in y_list:
                return True
            return False
    ...
    def is_x(self, var):
        ...
        """
        x_list = [
         'lon', 'longitude', 'LONGITUDE', 'Longitude', 'x']
            ...
            if elem in ('lon', 'longitude', 'LONGITUDE', 'Longitude', 'x', 'X'):
                ...
                try:
                    if self.dataset[elem].axis == 'X':
                        ...
                        if self.dataset[elem].standard_name in ('lon', 'longitude',
                                                                'LONGITUDE', 'Longitude',
                                                                'x', 'X'):
                            return self.dataset[elem].values
    ...
    def get_z(self):
        for elem in self.dataset.coords:
            if type(self.dataset[elem]) in ('numpy.datetime64', 'numpy.datetime32',
                                            'datetime.datetime'):
                return self.dataset[elem].values
                try:
