...
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/catalyze/client.py
...
class Session:
    ...
    def sign_in(self, username, password):
        resp = self.session.post(config.baas_host + '/v2/auth/signin', headers={'X-Api-Key': config.api_key, 
           'Accept': 'application/json', 
           'Content-Type': 'application/json'}, data=json.dumps({'username': username, 
           'password': password}))
        if is_ok(resp):
            j = resp.json()
