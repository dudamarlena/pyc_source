...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/icsemantic/catalog/indexes/translation.py
    ...
    def _apply_index(self, request, cid=''):
        ...
        for k in record.keys:
            for lang in user_languages:
                filtered_langs = [ l for l in language if l not in [lang, ''] ]
                key = '%s@%s' % (k, lang)
                thesaurus_results += utils.get_equivalent(portal, key, filtered_langs)
                if thesaurus_results != []:
                    ...
                    query = {'SearchableText': search_text, 'Language': language}
...
def manage_addTranslationIndex(self, id, extra=None, REQUEST=None, RESPONSE=None, URL3=None):
    """Add a fake index"""
    return self.manage_addIndex(id, 'TranslationIndex', extra=extra, REQUEST=REQUEST, RESPONSE=RESPONSE, URL1=URL3)