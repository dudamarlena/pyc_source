...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/icsemantic/catalog/setuphandlers.py
...
def add_indexes(context):
    ...
    if SYNONYM_IDX not in catalog.indexes():
        catalog.manage_addIndex(SYNONYM_IDX, 'SynonymIndex')
    if TRANSLATION_IDX not in catalog.indexes():
        catalog.manage_addIndex(TRANSLATION_IDX, 'TranslationIndex')
    if RELATED_IDX not in catalog.indexes():
...
def setup_site(portal, out):
    ...
    if not sm.queryUtility(IicSemanticManagementAdvancedSearchOptions, name='icsemantic.advancedsearch'):
        if HAS_PLONE3:
            sm.registerUtility(icSemanticManagementAdvancedSearchOptions(), interfaces.IicSemanticManagementAdvancedSearchOptions, 'icsemantic.advancedsearch')
        else:
            sm.registerUtility(IicSemanticManagementAdvancedSearchOptions, icSemanticManagementAdvancedSearchOptions(), 'icsemantic.advancedsearch')
