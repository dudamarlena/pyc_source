...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/tests/test_ps.py
...
def test_doc_examples():
    env = {'ps': ps.PsAuxww(context_wrap(PsAuxww_TEST_DOC)), 
...
def test_ps_auxww_from_auxww():
    ...
    assert keys_in([
     ...
     'TIME', 'COMMAND'], d[0])
    assert d[0] == {'%MEM': '0.0', 
       'TTY': '?', 'VSZ': '193892', 'PID': '1', '%CPU': '0.0', 'START': 'Oct23', 
       'USER': 'root', 'STAT': 'Ss', 'TIME': '1:40', 'RSS': '5000', 'COMMAND': '/usr/lib/systemd/systemd --switched-root --system --deserialize 22', 
       'COMMAND_NAME': 'systemd', 
       'ARGS': '--switched-root --system --deserialize 22'}
    assert d[2]['COMMAND'] == '/usr/lib/systemd/systemd-journald'
    assert d[(-2)] == {'USER': 'user_01', 
       'PID': '2713', '%CPU': '2.4', '%MEM': '19.2', 'VSZ': '12508144', 
       'RSS': '3077224', 'TTY': '?', 'STAT': 'Sl', 'START': 'Nov21', 
       'TIME': '533:33', 'COMMAND': '/usr/lib64/firefox/firefox', 'COMMAND_NAME': 'firefox', 
       'ARGS': ''}
...
def test_ps_ef_from_ef():
    ...
    assert keys_in([
     'UID', 'PID', 'PPID', 'C', 'STIME', 'TTY', 'TIME', 'CMD'], d[0])
    assert d[0] == {'UID': 'root', 
       'TTY': '?', 'PID': '1', 'PPID': '0', 'TIME': '00:00:06', 
       'STIME': '03:53', 'C': '0', 'CMD': '/usr/lib/systemd/systemd --system --deserialize 15', 
       'COMMAND_NAME': 'systemd', 
       'ARGS': '--system --deserialize 15'}
    assert d[4]['CMD'] == '/usr/libexec/docker/rhel-push-plugin'
    assert d[(-5)] == {'TTY': '?', 
       'ARGS': '', 'UID': 'root', 'CMD': '/usr/libexec/docker/rhel-push-plugin', 
       'PID': '1995', 
       'C': '0', 'STIME': '03:54', 'TIME': '00:02:06', 'PPID': '1', 
...
def test_ps_auxww_from_auxcww():
    ...
    assert d[0] == {'%MEM': '0.0', 
       ...
       'COMMAND': 'init', 'COMMAND_NAME': 'init', 'USER': 'root', 'STAT': 'Ss', 'TIME': '0:01', 
       'RSS': '1544', 'ARGS': ''}
...
def test_ps_auxww_from_auxwww():
    ...
    assert d[0] == {'USER': 'root', 
       'PID': '1', '%CPU': '0.0', '%MEM': '0.0', 'VSZ': '21452', 
       'RSS': '1536', 'TTY': '?', 'STAT': 'Ss', 'START': 'Mar09', 
       'TIME': '0:01', 'COMMAND': '/sbin/init', 'COMMAND_NAME': 'init', 
       'ARGS': ''}
...
def test_ps_auxww_from_aux():
    ...
    assert d[0] == {'%MEM': '0.0', 
       'TTY': '?', 'VSZ': '19356', 'PID': '1', '%CPU': '0.0', 'START': 'May31', 
       'COMMAND': '/sbin/init', 'COMMAND_NAME': 'init', 'USER': 'root', 'STAT': 'Ss', 
...
def test_ps_eo():
    ...
    assert p.pid_info['2416'] == {'PID': '2416', 
       'PPID': '1', 'COMMAND': 'auditd', 'COMMAND_NAME': 'auditd', 'ARGS': ''}
    assert p.pid_info['18379'] == {'PID': '18379', 
       'PPID': '18347', 'COMMAND': 'ps', 'COMMAND_NAME': 'ps', 'ARGS': ''}
    assert p.children('18347') == [{'PID': '18379', 'PPID': '18347', 'COMMAND': 'ps', 'COMMAND_NAME': 'ps', 'ARGS': ''}]
