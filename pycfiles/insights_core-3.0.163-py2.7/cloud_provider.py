...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/combiners/cloud_provider.py
...
class CloudProvider(object):
    ...
    def _select_provider(self):
        if any(value for value in self.cp_bios_vendor.values()):
            if self.cp_bios_vendor['aws'] and self.__AWS.vv in self.cp_bios_vendor['aws'].lower():
                return self.__AWS.name
            if self.cp_bios_vendor['google'] and self.__GOOGLE.vv in self.cp_bios_vendor['google'].lower():
                return self.__GOOGLE.name
            if self.cp_bios_vendor['azure'] and self.__AZURE.vv in self.cp_bios_vendor['azure'].lower():
                return self.__AZURE.name
            return
        if any(value for value in self.cp_bios_version.values()):
            if self.cp_bios_version['aws'] and self.__AWS.vv in self.cp_bios_version['aws'].lower():
                return self.__AWS.name
            if self.cp_bios_version['google'] and self.__GOOGLE.vv in self.cp_bios_version['google'].lower():
                return self.__GOOGLE.name
            if self.cp_bios_version['azure'] and self.__AZURE.vv in self.cp_bios_version['azure'].lower():
                return self.__AZURE.name
            return
        else:
            if any(value for value in self.cp_rpms.values()):
                if self.cp_rpms[CloudProvider.AWS]:
                    return self.__AWS.name
                if self.cp_rpms[CloudProvider.GOOGLE]:
                    return self.__GOOGLE.name
                if self.cp_rpms[CloudProvider.AZURE]:
                    return self.__AZURE.name
                return
        ...
        if yrl and hasattr(yrl, 'data'):
            for p in self.__PROVIDERS:
                for yval in yrl.data:
                    prov[p.name].append(yval.get('id').lower()) if p.yum and p.yum in yval.get('id').lower() else prov
    ...
    def _get_cp_bios_vendor(self, dmidcd):
        ...
        if dmidcd and dmidcd.bios:
            for p in self.__PROVIDERS:
                prov[p.name] = dmidcd.bios.get('vendor') if p.vv and p.vv in dmidcd.bios.get('vendor').lower() else ''
    ...
    def _get_cp_bios_version(self, dmidcd):
        ...
        if dmidcd and dmidcd.bios:
            for p in self.__PROVIDERS:
                prov[p.name] = dmidcd.bios.get('version') if p.vv and p.vv in dmidcd.bios.get('version').lower() else ''
    ...
    def _get_cp_from_uuid(self, dmidcd):
        prov = self._provider_init_str()
        if dmidcd and dmidcd.bios:
            prov['aws'] = dmidcd.system_info.get('uuid') if dmidcd.system_info.get('uuid').lower().startswith('ec2') else ''
        return prov

    def _get_cp_from_manuf(self, dmidcd):
        prov = self._provider_init_str()
        if dmidcd and dmidcd.system_info:
            prov[CloudProvider.__ALIBABA.name] = dmidcd.system_info.get('manufacturer') if dmidcd.system_info.get('manufacturer').lower() == CloudProvider.__ALIBABA.manuf else ''
        return prov