...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/ssh.py
...
class SshDConfig(Parser):
    ...
    def parse_content(self, content):
        ...
        for line in get_active_lines(content):
            line_splits = [ s.strip() for s in re.split('[\\s=]+', line, 1) ]
            kw, val = line_splits[0], line_splits[1].strip('"') if len(line_splits) == 2 else ''
