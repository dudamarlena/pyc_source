...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/osa_dispatcher_log.py
...
class OSADispatcherLog(LogFileOutput):
    ...
    _LINE_STR = '^(?P<timestamp>\\d{4}/\\d{2}/\\d{2} \\d{2}:\\d{2}:\\d{2} ' + '[+-]?\\d{2}:\\d{2}) (?P<pid>\\d+) (?P<client_ip>\\S+): ' + '(?P<module>\\w+)/(?P<function>[\\w_.-]+)(?:\\((?P<info>.*)\\))?$'
