...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/client/data_collector.py
# Compiled at: 2020-04-16 14:56:28
"""
Collect all the interesting data for analysis
"""
from __future__ import absolute_import
import os, errno, json, logging, copy, glob, six, shlex
...
class DataCollector(object):
    ...
    def _write_branch_info(self, branch_info):
        ...
        self.archive.add_metadata_to_archive(json.dumps(branch_info), '/branch_info')

    def _write_display_name(self):
        if self.config.display_name:
            logger.debug('Writing display_name to archive...')
            self.archive.add_metadata_to_archive(self.config.display_name, '/display_name')

    def _write_version_info(self):
        logger.debug('Writing version information to archive...')
        version_info = get_version_info()
        self.archive.add_metadata_to_archive(json.dumps(version_info), '/version_info')
    ...
    def _write_blacklist_report(self, blacklist_report):
        ...
        self.archive.add_metadata_to_archive(json.dumps(blacklist_report), '/blacklist_report')
    ...
    def _blacklist_check(self, cmd):
        ...
        return len(set.intersection(set(cmd_parts), constants.command_blacklist)) > 0
    ...
    def _parse_command_spec(self, spec, precmds):
        ...
        if 'pre_command' in spec:
            ...
            try:
                ...
                logger.debug('Pre-command %s not found. Skipping %s...', precmd_alias, spec['command'])
                return []
    ...
    def run_collection(self, conf, rm_conf, branch_info, blacklist_report):
        ...
        for c in conf['commands']:
            if c.get('symbolic_name') == 'hostname':
                self.hostname_path = os.path.join('insights_commands', mangle.mangle_command(c['command']))
    ...
    def done(self, conf, rm_conf):
        ...
        if self.config.obfuscate:
            ...
            clean_opts = CleanOptions(self.config, self.archive.tmp_dir, rm_conf, self.hostname_path)
