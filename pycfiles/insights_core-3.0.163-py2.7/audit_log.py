...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/audit_log.py
...
class AuditLog(LogFileOutput):
    ...
    def _parse_line(self, line):
        ...
        if len(linesp) < 2 or not (linesp[0] and linesp[0].startswith('type=')) or not (linesp[1] and linesp[1].startswith('msg=audit(')):
            return info
        ...
        for index in range(len(linesp) - 1, 1, -1):
            ...
            if len(itemsp) < 2:
                unparsed1 = line.rsplit(linesp[(index + 1)])[0] if index + 1 < len(linesp) else line
