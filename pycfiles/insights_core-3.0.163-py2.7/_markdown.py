...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/formats/_markdown.py
...
class MarkdownFormat(Formatter):
    ...
    def __init__(self, broker, missing=False, tracebacks=False, dropped=False, fail_only=False, stream=sys.stdout):
        ...
        self.responses = {'skip': self.response(label='SKIP', title='Missing Deps: '), 
        ...
        for c in sorted(self.broker.get_by_type(rule), key=dr.get_name):
            ...
            if _type:
                if self.missing and _type == 'skip':
                    print_missing(c, v)
                elif self.fail_only and _type == 'rule' or not self.fail_only and _type != 'skip':
                    printit(c, v)
...
class MarkdownFormatAdapter(FormatterAdapter):
    ...
    def preprocess(self, broker):
        self.formatter = MarkdownFormat(broker, self.missing, self.tracebacks, self.dropped, self.fail_only)
