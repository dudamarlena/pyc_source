...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/lsscsi.py
...
class LsSCSI(CommandParser):
    ...
    * ``Primary-Device-Node`` - the primary device node name
    ...
    def parse_content(self, content):
        ...
        self.data = []
        for line in content:
            col_data = dict((LSSCSI_TABLE_HEADER_ITEMS[i], line[col_index[i]:col_index[(i + 1)]].strip()) for i in range(LEN - 1))
