...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/tests/test_vdsm_log.py
...
def test_vdsm_version_3_log():
    ...
    assert parsed_newlines[0] == {'thread': 'Thread-60', 
       'level': 'DEBUG', 
...
def test_bad_vdsm_log_parsing():
    ...
    assert len(parsed) == 4
    assert parsed[0] == {'thread': 'Thread-11', 
       'level': 'INFO', 
       'asctime': datetime(2015, 5, 3, 0, 1, 15, 853000)}
    assert parsed[1] == {'thread': 'Thread-12', 
       'level': 'INFO', 
       'asctime': datetime(2015, 5, 4, 0, 1, 15, 854000)}
    assert parsed[2] == {'thread': 'Thread-13', 
       'level': 'INFO', 
       'asctime': datetime(2015, 5, 4, 0, 1, 15, 854000), 
       'module': 'logUtils', 
       'line': '47', 
...
def test_vdsm_version_4_log():
    ...
    assert parsed[0] == {'thread': 'vm/27f1a8d4', 
       'level': 'ERROR', 
       'logname': 'virt.vm', 
       'module': 'vm', 
       'asctime': datetime(2017, 4, 18, 13, 56, 28, 96000), 
       'lineno': '617', 
       'message': "(vmId='27f1a8d4-1f81-49e6-bd83-4ecb0a462b54') The vm start process failed"}
    ...
    for line in info_lines:
        if 'Attempted double use of PCI slot' in line['message'] and line['level'] == 'INFO':
            ...
            assert 'Stopping connection' in line['message'] and line['level'] == 'INFO' and line['module'] == 'guestagent'

    vdsm_log = VDSMLog(context_wrap(VDSM_VER_4_LOG_2))
    assert 'libvirtError' in vdsm_log
    lines_with_error = vdsm_log.get('ERROR')
    parsed = list(vdsm_log.parse_lines(lines_with_error))
    assert parsed[0] == {'thread': 'vm/1b8dc1a2', 
       'level': 'ERROR', 
       'logname': 'virt.vm', 
       'module': 'vm', 
       'asctime': datetime(2017, 5, 22, 11, 50, 45, 363000), 
       'lineno': '617', 
       'message': "(vmId='1b8dc1a2-acd3-4a57-9a2b-23f12ae52e38') The vm start process failed"}
    assert 'WARN' in vdsm_log
    lines_with_warn = vdsm_log.get('WARN')
    parsed = list(vdsm_log.parse_lines(lines_with_warn))
    assert parsed[0] == {'asctime': datetime(2017, 5, 22, 11, 50, 46, 80000), 
       'level': 'WARN', 
       'lineno': '352', 
       'message': "(vmId='1b8dc1a2-acd3-4a57-9a2b-23f12ae52e38') trying to set state to Powering down when already Down", 
       'module': 'vm', 
       'logname': 'virt.vm', 
       'thread': 'jsonrpc/1'}
    ...
    assert parsed[15] == {'asctime': datetime(2017, 4, 18, 13, 56, 38, 427000), 
       'level': 'INFO', 
       'lineno': '732', 
       'message': 'Resource namespace 01_img_0c78b4d6-ba00-4d3e-9f9f-65c7d5899d71 already registered', 
       'module': 'sd', 
    ...
    assert parsed[0] == {'asctime': datetime(2017, 4, 18, 13, 56, 38, 374000), 
       'level': 'INFO', 
       'lineno': '54', 
       'message': "Run and protect: getSpmStatus, Return response: {'spm_st': {'spmId': 1, 'spmStatus': 'SPM', 'spmLver': 3L}}", 
       'module': 'logUtils', 
       'logname': 'dispatcher', 
       'thread': 'jsonrpc/2'}
    ...
    lines_with_error = vdsm_log.get('ERROR')
    parsed = list(vdsm_log.parse_lines(lines_with_error))
    assert parsed[0] == {'asctime': datetime(2017, 4, 18, 14, 0, 1, 808000), 
       'level': 'ERROR', 
       'lineno': '304', 
    ...
    assert len(parsed) == 1
    assert parsed[0] == {'asctime': datetime(2018, 1, 31, 9, 20, 2, 658000), 
       'level': 'DEBUG', 
       'lineno': '328', 
