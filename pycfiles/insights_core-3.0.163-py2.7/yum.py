...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/yum.py
...
class YumRepoList(CommandParser):
    ...
    def parse_content(self, content):
        ...
        self.data = parse_fixed_table(content, heading_ignore=[
         'repo id'], header_substitute=[
         ('repo id', 'id     '), ('repo name', 'name     ')], trailing_ignore=trailing_line_prefix, empty_exception=True)
        if not self.data:
            raise SkipException('No repolist.')
    ...
    def rhel_repos(self):
        ...
        """
        return [ i.split('/')[0] for i in self.repos if i.startswith('rhel')
               ]