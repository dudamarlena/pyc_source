...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/core/serde.py
...
"""
import json as ser, logging, os, time, traceback
...
def serialize(obj, root=None):
    ...
    return {'type': dr.get_name(type(obj)), 
...
class Hydration(object):
    ...
    def dehydrate(self, comp, broker):
        ...
        if not self.created:
            fs.ensure_path(self.meta_data, mode=504)
            if self.data:
                fs.ensure_path(self.data, mode=504)
            self.created = True
        c = comp
        doc = None
        try:
            name = dr.get_name(c)
            value = broker.get(c)
            errors = [ t for e in broker.exceptions.get(c, []) for t in broker.tracebacks[e]
                     ]
            doc = {'name': name, 
