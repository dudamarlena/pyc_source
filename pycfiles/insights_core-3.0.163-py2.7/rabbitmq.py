...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/rabbitmq.py
        ...
        for line in content:
            ...
            if len(parts) == 4 and not line.startswith('Error:'):
                if parts[3].lower() in TRUE_FALSE:
                    self.data.append(RabbitMQQueues.QueueInfo(parts[0], int(parts[1]), int(parts[2]), TRUE_FALSE[parts[3].lower()]))
                else:
                    raise ParseException(('auto_delete should be true or false: {0}').format(line))
            else:
...
class RabbitMQEnv(SysconfigOptions):
    ...
    def rmq_erl_tcp_timeout(self):
        pattern_connect_options = compile('inet_default_connect_options.*?\\<<([0-9]*)\\:64')
        pattern_listen_options = compile('inet_default_listen_options.*?\\<<([0-9]*)\\:64')
