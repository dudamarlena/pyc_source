...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/core/spec_factory.py
# Compiled at: 2019-12-13 11:36:28
import itertools, logging, os, re, six, traceback, codecs
...
SAFE_ENV = {'PATH': os.path.pathsep.join([
          '/bin',
          '/usr/bin',
          '/sbin',
          '/usr/sbin',
...
def mangle_command(command, name_max=255):
    ...
    """
    pattern = '[^\\w\\-\\.\\/]+'
    mangledname = re.sub('^/(usr/|)(bin|sbin)/', '', command)
    mangledname = re.sub(pattern, '_', mangledname)
    mangledname = re.sub('/', '.', mangledname).strip(' ._-')
    ...
    def _stream(self):
        ...
        try:
            if self._content:
                ...
                args = self.create_args()
                if args:
                    with streams.connect(env=SAFE_ENV, *args) as (s):
                        yield s
                else:
                    with codecs.open(self.path, 'r', encoding='utf-8', errors='surrogateescape') as (f):
                        yield f
...
class CommandOutputProvider(ContentProvider):
    ...
    def load(self):
        ...
        raw = self.ctx.shell_out(command, split=self.split, keep_rc=self.keep_rc, timeout=self.timeout, env=self.create_env())
        if self.keep_rc:
            self.rc, output = raw
        else:
            output = raw
    ...
    def _stream(self):
        ...
        try:
            if self._content:
                ...
                args = self.create_args()
                with self.ctx.connect(env=self.create_env(), timeout=self.timeout, *args) as (s):
                    yield s
        except StopIteration:
            raise
        except Exception as ex:
    ...
    def write(self, dst):
        ...
        fs.ensure_path(os.path.dirname(dst))
        if args:
            p = Pipeline(timeout=self.timeout, env=self.create_env(), *args)
...
class RegistryPoint(object):
    ...
    def __init__(self, metadata=None, multi_output=False, raw=False, filterable=False):
        self.metadata = metadata
        self.multi_output = multi_output
        self.raw = raw
        self.filterable = filterable
        self.__name__ = self.__class__.__name__
        datasource([], metadata=metadata, multi_output=multi_output, raw=raw, filterable=filterable)(self)
...
class simple_file(object):
    ...
    def __init__(self, path, context=None, deps=[], kind=TextFileProvider, **kwargs):
        ...
        self.__name__ = self.__class__.__name__
        datasource(self.context, raw=self.raw, *deps, **kwargs)(self)
...
class glob_file(object):
    ...
    def __init__(self, patterns, ignore=None, context=None, deps=[], kind=TextFileProvider, max_files=1000, **kwargs):
        ...
        self.__name__ = self.__class__.__name__
        datasource(self.context, multi_output=True, raw=self.raw, *deps, **kwargs)(self)
    ...
    def __call__(self, broker):
        ...
        if results:
            if len(results) > self.max_files:
                raise ContentException(('Number of files returned [{0}] is over the {1} file limit, please refine the specs file pattern to narrow down results').format(len(results), self.max_files))
            return results
...
class simple_command(object):
    ...
    def __call__(self, broker):
        ...
        return CommandOutputProvider(self.cmd, ctx, split=self.split, keep_rc=self.keep_rc, ds=self, timeout=self.timeout, inherit_env=self.inherit_env)
...
class command_with_args(object):
    ...
    def __init__(self, cmd, provider, context=HostContext, deps=None, split=True, keep_rc=False, timeout=None, inherit_env=None, **kwargs):
        ...
        self.__name__ = self.__class__.__name__
        datasource(self.provider, self.context, raw=self.raw, *deps, **kwargs)(self)
        return
...
class foreach_execute(object):
    ...
    def __init__(self, provider, cmd, context=HostContext, deps=[], split=True, keep_rc=False, timeout=None, inherit_env=[], **kwargs):
        ...
        self.inherit_env = inherit_env
        self.__name__ = self.__class__.__name__
        datasource(self.provider, self.context, multi_output=True, raw=self.raw, *deps, **kwargs)(self)
    ...
    def __call__(self, broker):
        ...
        for e in source:
            try:
                the_cmd = self.cmd % e
                cop = CommandOutputProvider(the_cmd, ctx, args=e, split=self.split, keep_rc=self.keep_rc, ds=self, timeout=self.timeout, inherit_env=self.inherit_env)
                result.append(cop)
            except:
                log.debug(traceback.format_exc())
