...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/partitions.py
...
class Partitions(Parser, dict):
    ...
    def parse_content(self, content):
        ...
        self.update(dict((row['name'], row) for row in parse_delimited_table(content, heading_ignore=[
         'major', 'minor'], header_substitute=[
         ('#blocks', 'blocks')]) if 'name' in row))
