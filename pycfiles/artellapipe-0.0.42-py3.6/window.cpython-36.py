...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/widgets/window.py
# Compiled at: 2020-04-17 19:05:38
# Size of source mod 2**32: 8309 bytes
"""
...
class ArtellaWindow(tpDcc.Window, object):
    ...
    def __init__(self, name='Window', title='Window', size=(800, 535), fixed_size=False, parent=None, *args, **kwargs):
        self._project = artellapipe.project
        (super(ArtellaWindow, self).__init__)(args, name=name, title=title, size=size, fixed_size=fixed_size, auto_run=True, frame_less=True, use_style=False, parent=parent, **kwargs)
    ...
    def ui(self):
        ...
        if self._project.is_dev():
            ...
            dev_style = 'background-color: rgb({}, {}, {})'.format(int_colors[0], int_colors[1], int_colors[2], 255)
            self._dragger.setStyleSheet(dev_style)

    def setWindowTitle(self, title):
        if self._project.is_dev():
            if self._project.get_environment() not in title:
    ...
    def _get_title_pixmap(self):
        ...
        if self._project:
            ...
            if not title_pixmap.isNull():
                return title_pixmap
            self._project.logger.warning('{} Project Title Background image not found: {}!'.format(self._project.name.title(), title_background + '.png'))
...
def dock_window(project, window_class, min_width=300):
    ...
    if not tpDcc.is_maya():
        ...
        import maya.cmds as cmds, maya.OpenMayaUI as OpenMayaUI
        try:
        ...
        main_control = cmds.workspaceControl((window_class.name),
          ttc=['AttributeEditor', -1], iw=min_width, mw=True, wp='preferred', label=(window_class.title))
