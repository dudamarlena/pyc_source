...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/managers/menus.py
    ...
    def create_menus(self, package_name, project):
        ...
        if not project:
            ...
            if not main_menu:
                ...
                return False
            menus_config = tpDcc.ConfigsMgr().get_config(config_name='artellapipe-menu',
              package_name=(project.get_clean_name()),
              root_package_name='artellapipe',
              environment=(project.get_environment()))
            if not menus_config:
                artellapipe.logger.warning('Impossible to create menu for project "{}" because menu configuration file not found!'.format(project.get_clean_name()))
                return False
