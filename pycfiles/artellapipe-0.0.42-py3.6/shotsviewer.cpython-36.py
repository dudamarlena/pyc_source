...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/widgets/shotsviewer.py
...
class ShotsViewer(base.BaseWidget, object):
    ...
    def ui(self):
        ...
        self._shots_grid = ShotsGrid(project=(self._project),
...
class ShotsGrid(grid.GridWidget, object):
    ...
    def change_sequence(self, sequence_name=None):
        ...
        if sequence_name != 'All':
            if sequence_name not in all_sequences_names:
                LOGGER.warning('Sequence {} not found for current project {}'.format(sequence_name, self._project.name.title()))
                sequence_name = 'All'
        self.clear()
        new_shots = list()
        for new_shot in reversed(self._shots):
            ...
            elif new_shot.shot.get_sequence() == sequence_name:
                new_shot.setVisible(True)
                new_shots.insert(0, new_shot)
            else:
