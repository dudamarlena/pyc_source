...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/loader.py
# Compiled at: 2020-04-17 19:05:38
# Size of source mod 2**32: 8530 bytes
"""
...
import os, inspect, logging.config, tpDcc as tp
...
def init(do_reload=False, import_libs=True, dev=False):
    ...
    class ArtellaPipe(importer.Importer):
    ...
    artella_importer.import_packages(order=packages_order,
      only_packages=False)
    if do_reload:
...
def register_configs():
    ...
    if artella_configs_path:
        if os.path.isdir(artella_configs_path):
...
def register_tools(project_inst, dev=False, do_reload=False):
    ...
    package_names = [
     'artellapipe', project_inst.get_clean_name()]
    ...
    tp.ToolsMgr().load_package_tools(package_name='artellapipe',
      root_package_name=project_name,
      tools_to_load=tools_to_load,
      config_dict=config_dict,
      dev=dev)
    tp.ToolsMgr().load_package_tools(package_name=project_name,
      tools_to_load=tools_to_load,
      config_dict=config_dict,
      dev=dev)
    ...
    for package_name in package_names:
        tp.ToolsetsMgr().load_registered_toolsets(package_name,
          tools_to_load=tools_to_load, dev=dev, do_reload=do_reload)