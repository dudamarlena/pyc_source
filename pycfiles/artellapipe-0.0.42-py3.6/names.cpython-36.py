...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/managers/names.py
# Compiled at: 2020-04-17 19:05:38
# Size of source mod 2**32: 8467 bytes
"""
...
class ArtellaNamesManager(object):
        ...
        if naming_config:
            if auto_suffix:
                if obj_type not in auto_suffix:
                    if obj_type == parsed_name['node_type']:
                        return True
                    return False
                else:
                    if auto_suffix[obj_type] == parsed_name['node_type']:
                        return True
                    return False
        if obj_type == parsed_name['node_type']:
            return True
    ...
    def solve_node_name_by_type(self, node_names=None, **kwargs):
        ...
        if not tpDcc.is_maya():
            ...
            if not naming_config:
                artellapipe.logger.warning('Impossible to generate name from node because not naming configuration was found!')
                return
            auto_suffix = naming_config.get('auto_suffixes', default=(dict()))
            if not auto_suffix:
                artellapipe.logger.warning('Impossible to launch auto suffix functionality because no auto suffixes are defined!')
                return
            solved_names = dict()
            return_names = list()
            if not node_names:
            ...
            for obj_name in node_names:
                ...
                if obj_uuid in solved_names:
                    artellapipe.logger.warning('Node with name: "{} and UUID "{}" already renamed to "{}"! Skipping ...'.format(obj_name, obj_uuid, solved_names[obj_name]))
                else:
                    ...
                    if not name_lib.has_rule(rule_name):
                        if not name_lib.has_rule('node'):
                            artellapipe.logger.warning('Impossible to rename node "{}" by its type "{}" because rule "{}" it is not defined and callback rule "node" either'.format(obj_name, obj_type, rule_name))
                        else:
                            rule_name = 'node'
                    if rule_name == 'node':
    ...
    def solve_name(self, rule_name, *args, **kwargs):
        ...
        if not rule_name or not name_lib.has_rule(rule_name):
            artellapipe.logger.warning('Impossible to retrieve name because rule name "{}" is not defined!'.format(rule_name))
            return
        else:
