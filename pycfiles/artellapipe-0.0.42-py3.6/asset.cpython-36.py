...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/widgets/asset.py
# Compiled at: 2020-04-17 19:05:38
# Size of source mod 2**32: 10472 bytes
"""
...
class ArtellaAssetWidget(base.BaseWidget, object):
    ...
    def update_thumbnail_icon(self, force=False):
        ...
        try:
            ...
            if thumbnail_path and os.path.isfile(thumbnail_path) and not force:
                ...
                self._thumbnail_icon = tpDcc.ResourcesMgr().icon(artellapipe.AssetsMgr().get_default_asset_thumb())
    ...
    def _fill_sync_action_menu(self, sync_menu):
        ...
        if not sync_menu:
            ...
            for asset_type_name in self.asset.FILES:
                ...
                asset_type_action.triggered.connect(partial(self._on_sync, asset_type_name, defines.ArtellaFileStatus.ALL))
            ...
            if actions_to_add:
                ...
                all_action.triggered.connect(partial(self._on_sync, defines.ArtellaFileStatus.ALL, defines.ArtellaFileStatus.ALL))
    ...
    def _on_sync(self, file_type, sync_type, ask=False):
        ...
        """
        if ask:
            res = qtutils.show_question(None, 'Synchronize "{}" file: {}'.format(self.asset.get_name(), file_type.title()), 'Are you sure you want to sync this asset? This can take some time!')
