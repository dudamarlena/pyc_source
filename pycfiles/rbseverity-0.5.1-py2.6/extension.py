...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-intel/egg/rbseverity/extension.py
# Compiled at: 2014-07-19 05:08:23
...
class SeverityCommentDetailDisplay(CommentDetailDisplayHook):
    ...
    """
    SEVERITY_LABELS = {'major': 'Major', 
       'minor': 'Minor', 
       'info': 'Info'}
    HTML_EMAIL_COMMON_SEVERITY_CSS = 'font-weight: bold;font-size: 9pt;'
    HTML_EMAIL_SPECIFIC_SEVERITY_CSS = {'major': 'color: #AA0000;', 
       'minor': 'color: #CC5500;', 
       'info': 'color: #006600;'}
    ...
    def render_review_comment_detail(self, comment):
        ...
        if not severity:
            return ''
        return '<p class="comment-severity comment-severity-%s">Severity: %s</p>' % (
    ...
    def render_email_comment_detail(self, comment, is_html):
        ...
        if not severity:
            ...
            if is_html:
                specific_css = self.HTML_EMAIL_SPECIFIC_SEVERITY_CSS.get(severity, '')
...
class SeverityExtension(Extension):
    ...
    """
    metadata = {'Name': 'Comment Severity'}
    js_extensions = [
     SeverityJSExtension]
    css_bundles = {'default': {'source_filenames': [
                                      'css/severity.less'], 
                   'apply_to': apply_to_url_names}}
    js_bundles = {'severity-review': {'source_filenames': [
                                              'js/severity.js'], 
                           'apply_to': apply_to_url_names}}
