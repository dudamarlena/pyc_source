...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sci_api_req/providers/NASA/neows_provider.py
# Compiled at: 2019-08-29 15:00:25
...
class NeoWsProvider(ApiProvider):
    ...
    def feed(self, start_date: datetime.date, end_date: datetime.date, detailed=True) -> dict:
        ...
        @self._get_request('feed', start_date=start_date,
          end_date=end_date,
          detailed=detailed)
        def inner(response):
            return response
    ...
    def sentry(self, is_active=True, page=0, size=50) -> dict:
        ...
        @self._get_request('neo/sentry', is_active=(str(is_active)),
          page=(str(page)),
          size=(str(size)))
        def inner(response):
            return response
