...
# [GCC 8.4.0]
# Embedded file name: /home/gan/project/source/testgit/yaha/yaha/ksp_dijkstra.py
# Compiled at: 2013-08-20 21:10:15
    ...
    def remove_edge(self, node_from, node_to, cost=None):
        if self._data[node_from].has_key(node_to):
            if not cost:
                ...
                if self._data[node_from][node_to] == cost:
                    self._data[node_from][node_to] = self.INFINITY
                    return cost
                else:
                    return -1
...
def ksp_yen(graph, node_start, node_end, max_k=3):
    ...
    A = [
     {'cost': distances[node_end], 'path': path(previous, node_start, node_end)}]
    B = []
    if not A[0]['path']:
        return A
