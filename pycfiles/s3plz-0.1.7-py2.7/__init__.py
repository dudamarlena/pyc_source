...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/s3plz/__init__.py
# Compiled at: 2017-05-16 02:21:12
import boto, boto.s3, os, utils, warnings
...
class S3:
    ...
    to and from s3.
    """
    serializers = {'json': utils.to_json, 
       'gz': utils.to_gz, 
       'zip': utils.to_zip, 
       'pickle': utils.to_pickle}
    deserializers = {'json': utils.from_json, 
       'gz': utils.from_gz, 
       'zip': utils.from_zip, 
    ...
    def get_meta(self, filepath, **kw):
        ...
        if k:
            return {'content_type': k.content_type, 
        ...
        if serializer is None:
            ...
            for name in serializer.split('.'):
                try:
                    ...
                    raise NotImplementedError(('{} is not a supported serializer. Try one of: {}').format(name, (',').join(self.serializers.keys())))
        ...
        if serializer is None:
            ...
            for name in reversed(serializer.split('.')):
                try:
                    ...
                    raise NotImplementedError(('{} is not a supported deserializer. Try one of: {}').format(name, (',').join(self.deserializers.keys())))
    ...
    def _connect_to_bucket(self, **kw):
        ...
        kwargs or OS
        """
        key = kw.get('key', os.getenv('AWS_ACCESS_KEY_ID'))
        secret = kw.get('secret', os.getenv('AWS_SECRET_ACCESS_KEY'))
        if not key or not secret:
            raise S3AuthError, 'You must pass in a "key" and "secret" to s3plz.connect() or set "AWS_ACCESS_KEY_ID" and "AWS_SECRET_ACCESS_KEY" as environment variables.'
        try:
            conn = boto.connect_s3(key, secret)
        except Exception as e:
            raise S3AuthError, 'Your supplied credentials were invalid!'

        return conn.get_bucket(self.bucket_name)
        raise ValueError(('Bucket {} Does Not Exist!').format(self.bucket_name))
