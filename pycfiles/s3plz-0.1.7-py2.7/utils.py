...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/s3plz/utils.py
# Compiled at: 2017-05-16 02:21:12
from urlparse import urlparse
from datetime import datetime
import json, gzip, zipfile, uuid, pickle, cStringIO, pytz
...
def parse_s3_bucket(uri, _return_path=False):
    ...
    if components.scheme not in ('s3', 's3n') or '/' not in components.path:
...
def filepath_opts():
    ...
    """
    dt = now(ts=False)
    return {'@second': '%02d' % int(dt.second), 
