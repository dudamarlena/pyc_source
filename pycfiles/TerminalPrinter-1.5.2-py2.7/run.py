...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/printer/run.py
# Compiled at: 2019-11-30 20:35:34
import os, argparse
from version import __url__, __version__
from painter import MESS_FILTERS, FONT_LIST, FONT_DIR, FONT_URL, make_terminal_img, text_drawer, get_img
...
def command(args, parse):
    if args.init:
        ...
        if os.path.exists(pic_path) and os.path.isfile(pic_path):
            print make_terminal_img(get_img(pic_path, gray=args.gray), filter_type=args.filter, width=args.width, height=args.height, dye=args.color, reverse=args.reverse, keep_ratio=args.keep_ratio, gray=args.gray)
        else:
            print '请输入有效图片路径'
    elif args.text:
        print make_terminal_img(text_drawer(args.text, args.font), filter_type=args.filter, width=args.width, height=args.height, dye=args.color, reverse=not args.reverse, keep_ratio=False, gray=True, strip_white=True)
    else:
        parse.print_help()


def parser():
    parse = argparse.ArgumentParser(description='Terminal Printer', formatter_class=argparse.RawTextHelpFormatter, epilog='初次使用，需要初始化字体下载\r\n执行 terminalprint -i 初始化\r\n更多帮助信息请访问: ' + __url__)
