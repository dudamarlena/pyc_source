...
# [GCC 8.4.0]
# Embedded file name: /home/matej/archiv/2013/projekty/gg_scrapper/test/test_functional.py
# Compiled at: 2014-12-12 16:34:33
import logging, io, os.path
try:
...
EXP_URL = 'https://groups.google.com/forum/' + '?_escaped_fragment_=forum/jbrout'
...
ARTICLE_URL = 'https://groups.google.com/d/msg/jbrout' + '/xNwoVmC07KI/OfpRHFscUkwJ'
...
class TestGGScrapperFunctional(unittest.TestCase):
    ...
    def test_collecting_articles(self):
        ...
        topic = gg_scraper.Topic(TOPIC_URL, 'repo version incompatible with ' + 'ubuntu 11.04 ?')
    ...
    def test_get_raw_article(self):
        ...
        with io.open(self.dired('message.eml'), 'r', encoding='utf8') as (exp_f):
            self.assertEqual(self.msg_wo_From(article.collect_message()), exp_f.read())
        return

    def test_py26_unicode_raw_article(self):
        self.maxDiff = None
        URL = 'https://groups.google.com/forum/message/raw?' + 'msg=django-oscar/BbBiMWwolf0/gn-s0sFYEhkJ'
        article = self.msg_wo_From(gg_scraper.Article(URL).collect_message())
        with io.open(self.dired('py26_unicode.eml'), 'r', encoding='utf8') as (exp_f):
            expected = exp_f.read()
