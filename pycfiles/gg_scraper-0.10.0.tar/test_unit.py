...
# [GCC 8.4.0]
# Embedded file name: /home/matej/archiv/2013/projekty/gg_scrapper/test/test_unit.py
# Compiled at: 2014-02-12 19:58:29
import os, tempfile, yaml, sys
try:
...
import gg_scraper
from gg_scraper import Group, Topic, Article
    ...
    def test_create_mbox(self):
        ...
        with open('test/mbox.mbx') as (exp_f):
            with open(mbx_file.name) as (mbx_f):
                self.assertEqual(exp_f.read().strip(), mbx_f.read().strip())
...
class TestDemangle(unittest.TestCase):
    ...
    def test_demangle(self):
        self.maxDiff = None
        gg_scraper.demangle('test/unmangled_address.cnf', 'test/mbox.mbx', 'unmangled.mbx')
        with open('unmangled.mbx') as (obs_mbx_f):
            with open('test/mbox_unmangled.mbx') as (exp_mbx_f):
                self.assertAlmostEqual(len(obs_mbx_f.read()), len(exp_mbx_f.read()), delta=100)
        return
