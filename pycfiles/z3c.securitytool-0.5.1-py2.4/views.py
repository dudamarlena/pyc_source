...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/z3c/securitytool/browser/views.py
...
class PrincipalMatrixView(BrowserView):
    ...
    def update(self):
        ...
        (self.viewMatrix, self.views, self.permissions) = security_checker.getPermissionSettingsForAllViews(ifaces, skin, perm)
        self.path = ('/').join(self.request.get('REQUEST_URI', '').split('/')[2:-1])
        self.sortViews()
    ...
    def handlePermissionSelection(self):
        ...
        sessionPerm = ISession(self.request)[SESSION_KEY].get('selectedPermission', '')
        if formPerm:
            if formPerm == 'None':
                selectedPermission = ''
            else:
        ...
        ISession(self.request)[SESSION_KEY]['selectedPermission'] = selectedPermission
    ...
    def skinTypes(self):
        ...
        for (name, util) in zapi.getUtilitiesFor(IBrowserSkinType, self.context):
            ...
            if self.request.form.has_key('selectedSkin') and self.request.form['selectedSkin'] == name:
                skinNames[name] = True
...
class PrincipalDetailsView(BrowserView):
    ...
    def update(self):
        ...
        self.principalPermissions = principal_security(self.principal, skin=skin)
        self.legend = "<span class='Deny'>Red Bold = Denied Permission</span>,<span class='Allow'> Green Normal = Allowed Permission </span>"
        ...
        for (idx, item) in enumerate(permTree):
            for (uid, value) in item.items():
                if value.has_key('permissions'):
                    self.principalPermissions['permissionTree'][idx][uid]['permissions'].sort()
                    self.principalPermissions['permissionTree'][idx][uid]['parentList'].reverse()

        permTree = self.principalPermissions['roleTree']
        for (idx, item) in enumerate(permTree):
            for (uid, value) in item.items():
                if value.has_key('roles'):
                    self.principalPermissions['roleTree'][idx][uid]['roles'].sort()
                    self.principalPermissions['roleTree'][idx][uid]['parentList'].reverse()
...
class PermissionDetailsView(BrowserView):
    ...
    def update(self):
        ...
        permAdapter = zapi.getMultiAdapter((self.context,), IPermissionDetails)
        self.principalPermissions = permAdapter(self.principal, self.view, self.skin)
        self.legend = "<span class='Deny'>Red Bold = Denied Permission</span>,<span class='Allow'> Green Normal = Allowed Permission </span>"
        self.preparePrincipalPermissions()
