...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/JakobGM/astrality/astrality/tests/conftest.py
...
import shutil, pytest, astrality
...
def module_factory(test_config_directory):
    ...
    def _module_factory(name='test', on_startup=None, on_modified=None, on_exit=None, path=None, module_directory=test_config_directory / 'test_modules' / 'using_all_actions', replacer=lambda x: x, context_store={}) -> Module:
...
def module_manager_factory():
    ...
    def _module_manager_factory(*modules, context=Context()) -> ModuleManager:
...
def action_block_factory(test_config_directory):
    ...
    def _action_block_factory(import_context={}, compile={}, copy={}, run={}, stow={}, symlink={}, directory=test_config_directory, replacer=lambda x: x, context_store=Context()):
        """Return module with given parameters."""
        config = {'import_context':import_context, 
         'compile':compile, 
         'copy':copy, 
         'run':run, 
         'stow':stow, 
         'symlink':symlink}
        return ActionBlock(action_block=config,
          directory=directory,
          replacer=replacer,
          context_store=context_store,
          global_modules_config=GlobalModulesConfig(config={}, config_directory=test_config_directory),
          module_name='test')
...
def patch_xdg_directory_standard(tmpdir, monkeypatch, request):
    ...
    if 'dont_patch_xdg' in request.keywords:
        ...
        monkeypatch.setattr(astrality.xdg.XDG, 'data_home', data_dir)
        yield data_dir
        if data_dir.exists():
            shutil.rmtree(str(data_dir))
...
def patch_astrality_config_home(monkeypatch):
    ...
    monkeypatch.setitem(os.environ, 'ASTRALITY_CONFIG_HOME', str(example_config))