...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/site-packages/ripl/slidelayout.py
...
class SlideLayout:
    ...
    def vertical_layout(self, draw, slide):
        ...
        top = padding
        left = padding
        heading.update(dict(width=width,
          height=height,
          top=(self.padding),
          left=(self.padding)))
        top += height + padding
    ...
    def horizontal_layout(self, draw, slide):
        ...
        for row in rows:
            ...
            available_width = WIDTH - (used_width + (1 + len(items)) * padding)
            if images:
