...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/site-packages/ripl/caption.py
...
class SlideShow:
    ...
    def create_image(self, image_file, caption):
        ...
        if image_file:
            ...
            img = img.resize((int(width // ratio),
             int(height // ratio)), Image.ANTIALIAS)
        else:
    ...
    def add_caption(self, image, caption, colour=None):
        ...
        draw.font = self.font
        draw.font = self.font
        draw.text((width // 10, height // 20), caption, fill=colour)
        return image