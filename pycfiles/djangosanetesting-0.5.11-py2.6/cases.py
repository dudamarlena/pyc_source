...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/djangosanetesting/cases.py
# Compiled at: 2011-05-13 08:02:50
import re, sys, urllib2
from django.template import Context, Template, TemplateSyntaxError
from nose.tools import assert_equals, assert_almost_equals, assert_not_equals, assert_raises, assert_true, assert_false
from nose import SkipTest
from djangosanetesting.utils import twill_patched_go, twill_xpath_go, extract_django_traceback
__all__ = ('UnitTestCase', 'DatabaseTestCase', 'DestructiveDatabaseTestCase', 'HttpTestCase',
           'SeleniumTestCase', 'TemplateTagTestCase')
...
class SaneTestCase(object):
    ...
    def __new__(type, *args, **kwargs):
        ...
        def scrape(t):
            for a in [ at for at in dir(t) if at.startswith('assert') if '_' not in at ]:
                v = getattr(t, a)
                setattr(obj, a, v)
    ...
    * `TemplateSyntaxError' is bundled within this class, so that nothing
    ...
    def render_template(self, template, **kwargs):
        ...
        for load in self.preload:
            loads = ('').join([loads, '{% load ', load, ' %}'])

        template = ('').join([loads, template])
