...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/djangosanetesting/noseplugins.py
# Compiled at: 2011-05-10 05:00:42
"""
...
import djangosanetesting, djangosanetesting.cache
from djangosanetesting.selenium.driver import selenium
from djangosanetesting.utils import get_live_server_path, test_database_exists, DEFAULT_LIVE_SERVER_ADDRESS, DEFAULT_LIVE_SERVER_PORT
...
def flush_cache(test_case=None):
    ...
    if test_case and hasattr(test_case, 'flush_django_cache') and test_case.flush_django_cache or not hasattr(test_case, 'flush_django_cache') and getattr(settings, 'DST_FLUSH_DJANGO_CACHE', False):
...
class AbstractLiveServerPlugin(Plugin):
    ...
    def check_database_multithread_compilant(self):
        from django.conf import settings
        if settings.DATABASE_ENGINE == 'sqlite3' and (not getattr(settings, 'TEST_DATABASE_NAME', False) or settings.TEST_DATABASE_NAME == ':memory:'):
            self.skipped = True
            return False
        return True
    ...
    def startTest(self, test):
        ...
        if not self.server_started and getattr(test_case, 'start_live_server', False):
            if not self.check_database_multithread_compilant():
                return
            self.start_server(address=getattr(settings, 'LIVE_SERVER_ADDRESS', DEFAULT_LIVE_SERVER_ADDRESS), port=int(getattr(settings, 'LIVE_SERVER_PORT', DEFAULT_LIVE_SERVER_PORT)))
...
class DjangoPlugin(Plugin):
    ...
    def options(self, parser, env=os.environ):
        Plugin.options(self, parser, env)
        parser.add_option('', '--persist-test-database', action='store_true', default=env.get(self.env_opt), dest='persist_test_database', help='Do not flush database unless neccessary [%s]' % self.env_opt)
...
class SeleniumPlugin(Plugin):
    ...
    def startTest(self, test):
        ...
        if getattr(test_case, 'selenium_start', False):
            sel = selenium(getattr(settings, 'SELENIUM_HOST', 'localhost'), int(getattr(settings, 'SELENIUM_PORT', 4444)), getattr(settings, 'SELENIUM_BROWSER_COMMAND', '*opera'), getattr(settings, 'SELENIUM_URL_ROOT', get_live_server_path()))
            try:
                sel.start()
...
class SaneTestSelectionPlugin(Plugin):
    ...
    RECOGNIZED_TESTS = [
     'unit', 'database', 'destructivedatabase', 'http', 'selenium']
    score = 150

    def options(self, parser, env=os.environ):
        Plugin.options(self, parser, env)
        parser.add_option('-u', '--select-unittests', action='store_true', default=False, dest='select_unittests', help='Run all unittests')
        parser.add_option('--select-databasetests', action='store_true', default=False, dest='select_databasetests', help='Run all database tests')
        parser.add_option('--select-destructivedatabasetests', action='store_true', default=False, dest='select_destructivedatabasetests', help='Run all destructive database tests')
        parser.add_option('--select-httptests', action='store_true', default=False, dest='select_httptests', help='Run all HTTP tests')
        parser.add_option('--select-seleniumtests', action='store_true', default=False, dest='select_seleniumtests', help='Run all Selenium tests')
