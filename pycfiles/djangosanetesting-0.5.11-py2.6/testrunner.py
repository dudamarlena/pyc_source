...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/djangosanetesting/testrunner.py
...
from djangosanetesting.noseplugins import DjangoPlugin, DjangoLiveServerPlugin, SeleniumPlugin, CherryPyLiveServerPlugin, DjangoTranslationPlugin, ResultPlugin
...
class DstNoseTestSuiteRunner(DjangoTestSuiteRunner):
    ...
    def run_suite(self, nose_argv=None):
        ...
        nose_argv.extend(OPTION_TRANSLATION.get(opt, opt) for opt in sys.argv[1:] if opt.startswith('-') if not any(opt.startswith(d) for d in django_opts))
        if self.verbosity >= 2:
            print (' ').join(nose_argv)
        test_program = nose.core.TestProgram(argv=nose_argv, exit=False, addplugins=plugins)
    ...
    def run_tests(self, test_labels, extra_tests=None):
        ...
        from django.conf import settings
        nose_argv = [
         'nosetests', '--verbosity', str(self.verbosity)] + list(test_labels)
...
def _get_options():
    ...
    return tuple(o for o in options if o.dest not in django_opts if o.action != 'help')
    ...
    if hasattr(settings, 'NOSE_PLUGINS'):
        for plg_path in settings.NOSE_PLUGINS:
            try:
                ...
                raise exceptions.ImproperlyConfigured("%s isn't a Nose plugin module" % plg_path)

            p_mod, p_classname = plg_path[:dot], plg_path[dot + 1:]
            try:
                mod = import_module(p_mod)
            except ImportError, e:
                raise exceptions.ImproperlyConfigured('Error importing Nose plugin module %s: "%s"' % (p_mod, e))

            try:
                p_class = getattr(mod, p_classname)
            except AttributeError:
                raise exceptions.ImproperlyConfigured('Nose plugin module "%s" does not define a "%s" class' % (
                 p_mod, p_classname))
