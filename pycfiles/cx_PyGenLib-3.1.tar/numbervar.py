...
# Embedded file name: /home/lameiro/projects/cx_oracle_on_ctypes/cx_Oracle/numbervar.py
# Compiled at: 2015-09-09 18:56:59
import ctypes
...
class NUMBER(Variable):
    ...
    def lookup_precision_and_scale(environment, param):
        scale = OCIAttrGet(param, oci.OCI_HTYPE_DESCRIBE, oci.sb1, oci.OCI_ATTR_SCALE, environment, 'Cursor_ItemDescription(): scale')
        precision = OCIAttrGet(param, oci.OCI_HTYPE_DESCRIBE, oci.sb2, oci.OCI_ATTR_PRECISION, environment, 'Cursor_ItemDescription(): precision')
        return (
         precision, scale)
...
class BaseNumberVarType(VariableType):
    ...
    def pre_define(self, var, param):
        ...
        if var.type is vt_Float:
            scale = OCIAttrGet(param, oci.OCI_HTYPE_DESCRIBE, oci.sb1, oci.OCI_ATTR_SCALE, var.environment, 'NumberVar_PreDefine(): scale')
            precision = OCIAttrGet(param, oci.OCI_HTYPE_DESCRIBE, oci.sb2, oci.OCI_ATTR_PRECISION, var.environment, 'NumberVar_PreDefine(): precision')
    ...
    def get_value(self, var, pos):
        ...
        if var.type in (vt_NumberAsString, vt_LongInteger):
            ...
            typed_data = self.get_typed_data(var)
            status = oci.OCINumberToText(var.environment.error_handle, byref(typed_data[pos]), var.environment.numberToStringFormatBuffer.cast_ptr, var.environment.numberToStringFormatBuffer.size, None, 0, byref(c_string_length), cast_c_string)
    ...
    def set_value_from_long(self, var, pos, value):
        ...
        typed_data = self.get_typed_data(var)
        status = oci.OCINumberFromText(var.environment.error_handle, text_buffer.cast_ptr, text_buffer.size, var.environment.numberFromStringFormatBuffer.cast_ptr, var.environment.numberFromStringFormatBuffer.size, None, 0, byref(typed_data[pos]))
    ...
    def set_value_from_float(self, var, pos, value):
        ...
        typed_data = self.get_typed_data(var)
        status = oci.OCINumberFromReal(var.environment.error_handle, byref(double_value), ctypes.sizeof(double_value), byref(typed_data[pos]))
    ...
    def set_value_from_decimal(self, var, pos, value):
        ...
        typed_data = self.get_typed_data(var)
        status = oci.OCINumberFromText(var.environment.error_handle, text_buffer.cast_ptr, text_buffer.size, format_buffer.cast_ptr, format_buffer.size, var.environment.nlsNumericCharactersBuffer.cast_ptr, var.environment.nlsNumericCharactersBuffer.size, byref(typed_data[pos]))
    ...
    if not python3_or_better():
        ...
        def set_value_from_integer(self, var, pos, value):
            ...
            typed_data = self.get_typed_data(var)
            status = oci.OCINumberFromInt(var.environment.error_handle, byref(c_integer_value), ctypes.sizeof(c_integer_value), oci.OCI_NUMBER_SIGNED, byref(typed_data[pos]))
