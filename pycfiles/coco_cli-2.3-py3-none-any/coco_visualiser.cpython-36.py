...
# Embedded file name: /home/ashwin/Desktop/Projects/COCO-Assistant/coco_assistant/coco_visualiser.py
# Compiled at: 2019-11-05 13:54:04
# Size of source mod 2**32: 6240 bytes
import os, matplotlib.patches, matplotlib.pyplot as plt, matplotlib.widgets, mpl_toolkits.axes_grid1
from pycocotools.coco import COCO
import skimage.io as io

class ImageSlider(matplotlib.widgets.Slider):

    def __init__(self, ax, label, numpages=10, valinit=0, valfmt='%1d', closedmin=True, closedmax=True, dragging=True, **kwargs):
        self.facecolor = kwargs.get('facecolor', 'w')
        self.activecolor = kwargs.pop('activecolor', 'b')
        self.fontsize = kwargs.pop('fontsize', 10)
        self.numpages = numpages
        self.fig = ax.figure
        (super(ImageSlider, self).__init__)(ax, label, 0, numpages, valinit=valinit, 
         valfmt=valfmt, **kwargs)
        self.poly.set_visible(False)
        self.vline.set_visible(False)
        self.pageRects = []
        for i in range(numpages):
            facecolor = self.activecolor if i == valinit else self.facecolor
            r = matplotlib.patches.Rectangle((float(i) / numpages, 0), (1.0 / numpages), 1, transform=(ax.transAxes),
              facecolor=facecolor)
            ax.add_artist(r)
            self.pageRects.append(r)
            ax.text((float(i) / numpages + 0.5 / numpages), 0.5, (str(i + 1)), ha='center',
              va='center',
        ...
        self.button_back = matplotlib.widgets.Button(bax, label='<-', color=(self.facecolor),
          hovercolor=(self.activecolor))
        self.button_forward = matplotlib.widgets.Button(fax, label='->', color=(self.facecolor),
          hovercolor=(self.activecolor))
