...
# [GCC 8.4.0]
# Embedded file name: /Users/kgulliks/.PythonModules/TelluricFitter/src/TelluricFitter.py
# Compiled at: 2015-06-23 15:29:03
"""
...
import FittingUtilities, warnings, numpy as np
...
import MakeModel, DataStructures
...
class TelluricFitter():
    ...
    def __init__(self, debug=False, debug_level=2):
        ...
        """
        self.parnames = [
         'pressure', 'temperature', 'angle', 'resolution', 'wavestart', 'waveend',
         'h2o', 'co2', 'o3', 'n2o', 'co', 'ch4', 'o2', 'no',
         'so2', 'no2', 'nh3', 'hno3']
        self.const_pars = [795.0, 273.0, 45.0, 50000.0, 2200.0, 2400.0,
         50.0, 368.5, 0.039, 0.32, 0.14, 1.8, 210000.0, 1.1e-19,
         0.0001, 0.0001, 0.0001, 0.00056]
        self.bounds = [ [0.0, np.inf] for par in self.parnames ]
        self.fitting = [False] * len(self.parnames)
        self.normalization = [ 1.0 for par in self.parnames ]
        self.observatory = {'latitude': 30.6, 'altitude': 2.0}
        self.data = None
    ...
    def Fit(self, data=None, resolution_fit_mode='gauss', fit_primary=False, return_resolution=False, adjust_wave='model', continuum_fit_order=7, wavelength_fit_order=3, air_wave=True, fit_source=False, source_fcn=None, source_args=None, source_kwargs=None):
        """
        ...
        if len(fitpars) < 1:
            ...
            optdict = {'eps': 5}
            bfgs_optdict = {'disp': 2, 'pgtol': 1e-08, 'epsilon': 1, 'approx_grad': True}
            slsqp_optdict = {'disp': 2, 'eps': 0.1}
            output = leastsq(self.FitErrorFunction, fitpars, full_output=True, epsfcn=0.1)
            fitpars = output[0]
    ...
    def GenerateModel(self, pars, nofit=False, separate_source=False, return_resolution=False, broaden=False, model=None):
        ...
        for i in range(len(self.parnames)):
            if self.fitting[i]:
                if self.bounds[i][1] < np.inf:
                    self.const_pars[i] = self.bounds[i][0] + (np.sin(pars[fit_idx] * self.normalization[i]) + 1.0) * (self.bounds[i][1] - self.bounds[i][0]) / 2.0
                else:
                    self.const_pars[i] = self.bounds[i][0] - 1.0 + np.sqrt((pars[fit_idx] * self.normalization[i]) ** 2 + 1.0)
                fit_idx += 1
        ...
        wavenum_start = 10000000.0 / waveend
        wavenum_end = 10000000.0 / wavestart
        lat = self.observatory['latitude']
        alt = self.observatory['altitude']
        if model is None:
            model = self.Modeler.MakeModel(pressure, temperature, wavenum_start, wavenum_end, angle, h2o, co2, o3, n2o, co, ch4, o2, no, so2, no2, nh3, hno3, lat=lat, alt=alt, wavegrid=None, resolution=None, vac2air=self.air_wave)
        ...
        if self.adjust_wave == 'data' and shift != 0:
            ...
            if self.adjust_wave == 'model' and shift != 0:
    ...
    def WavelengthErrorFunction(self, shift, data, model):
        ...
        weight = 1000000000.0 * np.ones(data.x.size)
    ...
    def FitWavelength(self, data_original, telluric, tol=0.05, linestrength=0.98, oversampling=4, fitorder=3, numiters=10):
        """
    ...
    def FitWavelengthNew(self, data_original, telluric, fitorder=3, be_safe=True):
        ...
        if be_safe:
            args = (
             data_original, modelfcn, 0.05)
        else:
            args = (
             data_original, modelfcn, 100)
        output = leastsq(self.WavelengthErrorFunctionNew, pars, args=args, full_output=True, xtol=1e-12, ftol=1e-12)
        pars = output[0]
        return (
         partial(self.Poly, pars, np.median(data_original.x), min(data_original.x), max(data_original.x)), 0.0)
    ...
    def FitResolution(self, data, model, resolution=75000.0):
        ...
        try:
            resolution = fminbound(ResolutionFitErrorBrute, self.resolution_bounds[0], self.resolution_bounds[1], xtol=1, args=(
             data, newmodel))
        except ValueError:
    ...
    def Broaden(self, data, model, oversampling=5, m=101, dimension=20, full_output=False):
        ...
        Broadening = np.array(Broadening)[(Ellipsis, 0)]
        ...
        if maxindex > m / 2.0 + m / 10.0 or maxindex < m / 2.0 - m / 10.0:
            ...
            warnings.warn('Warning! SVD Broadening function peaked at the wrong location! See SVD_Error2.log for the broadening function')
            badfit = True
        elif np.mean(Broadening[maxindex - int(m / 10.0):maxindex + int(m / 10.0)]) < 3 * np.mean(Broadening[int(m / 5.0):]):
            outfilename = 'SVD_Error2.log'
            np.savetxt(outfilename, np.transpose((Broadening,)))
            warnings.warn('Warning! SVD Broadening function is not strongly peaked! See SVD_Error2.log for the broadening function')
            badfit = True
        if badfit:
            ...
            if full_output:
                ...
                return (
                 model, [gaussian / gaussian.sum(), xnew])
            return model
        ...
        params = [
         0.0, -Broadening[maxindex], maxindex, 10.0]
