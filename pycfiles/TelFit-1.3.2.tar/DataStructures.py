...
# [GCC 8.4.0]
# Embedded file name: /Users/kgulliks/.PythonModules/TelluricFitter/src/DataStructures.py
# Compiled at: 2015-04-13 11:16:44
"""
...
class xypoint:
    ...
    def __init__(self, size=100, x=None, y=None, cont=None, err=None, array=None):
        ...
        if array != None:
            ...
            if cont is None:
                ...
                self.cont = cont.copy()
            if err is None:
                self.err = np.ones(self.y.size) * 1000000000.0
                self.err[self.y > 0] = np.sqrt(self.y[(self.y > 0)])
            else:
                self.err = err.copy()
                self.err[self.err <= 0] = 1000000000.0
            return
    ...
    def __getitem__(self, index):
        if isinstance(index, slice):
            ...
            if isinstance(index, list) or isinstance(index, tuple) or isinstance(index, np.ndarray):
                x = self.x[index]
                y = self.y[index]
                ...
                return [
                 self.x[index], self.y[index], self.cont[index], self.err[index]]
