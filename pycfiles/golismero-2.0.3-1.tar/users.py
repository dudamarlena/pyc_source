...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/tools/sqlmap/plugins/generic/users.py
...
class Users:
    ...
    def getUsers(self):
        ...
        condition = Backend.isDbms(DBMS.MSSQL) and Backend.isVersionWithin(('2005',
                                                                            '2008'))
                ...
                if retVal:
                    for user, password in filterPairValues(zip(retVal[0][('%s.name' % randStr)], retVal[0][('%s.password' % randStr)])):
                        if user not in kb.data.cachedUsersPasswords:
                            kb.data.cachedUsersPasswords[user] = [
                             password]
                        else:
        ...
        if not kb.data.cachedUsersPasswords and isInferenceAvailable() and not conf.direct:
            ...
            if Backend.isDbms(DBMS.SYBASE):
                ...
                for user in users:
                    ...
                    logger.info(infoMsg)
                    if Backend.isDbms(DBMS.MSSQL) and Backend.isVersionWithin(('2005',
                                                                               '2008')):
    ...
    def getPrivileges(self, query2=False):
        ...
        if not kb.data.cachedUsersPrivileges and any(isTechniqueAvailable(_) for _ in (PAYLOAD.TECHNIQUE.UNION, PAYLOAD.TECHNIQUE.ERROR, PAYLOAD.TECHNIQUE.QUERY)) or conf.direct:
            if Backend.isDbms(DBMS.MYSQL) and not kb.data.has_information_schema:
                ...
                if Backend.isDbms(DBMS.ORACLE) and query2:
                    query = rootQuery.inband.query2
                    condition = rootQuery.inband.condition2
                else:
                    query = rootQuery.inband.query
        ...
        return (
         kb.data.cachedUsersPrivileges, areAdmins)
