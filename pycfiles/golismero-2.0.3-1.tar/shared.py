...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/golismero/api/shared.py
# Compiled at: 2013-08-27 10:25:19
"""
...
def check_value(obj):
    ...
    while stack:
        ...
        if any(typ is x for x in valid_containers):
            stack.extend(obj)
            continue
        if all(obj is not x for x in valid_objects) and all(typ is not x for x in valid_scalars) and all(typ is not x for x in shared_containers):
            t = repr(typ)
            if t.startswith("<type '") and t.endswith("'>"):
                t = "'%s'" % t[7:-2]
            raise TypeError('Type %s cannot be used in shared data containers' % t)
...
class SharedMap(AbstractSharedContainer):
    ...
    def get_many(self, keys):
        ...
        return Config._context.remote_call(MessageCode.MSG_RPC_SHARED_MAP_GET, self.shared_id, keys)
    ...
    def check_all(self, keys):
        ...
        """
        keys = [ encode_key(k) for k in keys ]
        return Config._context.remote_call(MessageCode.MSG_RPC_SHARED_MAP_CHECK_ALL, self.shared_id, keys)
    ...
    def check_any(self, keys):
        ...
        """
        keys = [ encode_key(k) for k in keys ]
        return Config._context.remote_call(MessageCode.MSG_RPC_SHARED_MAP_CHECK_ANY, self.shared_id, keys)
    ...
    def check_each(self, keys):
        ...
        """
        keys = [ encode_key(k) for k in keys ]
        return Config._context.remote_call(MessageCode.MSG_RPC_SHARED_MAP_CHECK_EACH, self.shared_id, keys)
    ...
    def pop_many(self, keys):
        ...
        """
        keys = [ encode_key(k) for k in keys ]
        return Config._context.remote_call(MessageCode.MSG_RPC_SHARED_MAP_POP, self.shared_id, keys)
    ...
    def put_many(self, items):
        ...
        return Config._context.remote_call(MessageCode.MSG_RPC_SHARED_MAP_SWAP, self.shared_id, items)
    ...
    def async_put_many(self, items):
        ...
        Config._context.async_remote_call(MessageCode.MSG_RPC_SHARED_MAP_PUT, self.shared_id, items)
    ...
    def delete_many(self, keys):
        ...
        """
        keys = [ encode_key(k) for k in keys ]
        Config._context.async_remote_call(MessageCode.MSG_RPC_SHARED_MAP_DELETE, self.shared_id, keys)
    ...
    def keys(self):
        ...
        :rtype: tuple( immutable, ... )
        """
        keys = Config._context.remote_call(MessageCode.MSG_RPC_SHARED_MAP_KEYS, self.shared_id)
...
class SharedHeap(AbstractSharedContainer):
    ...
    def check_all(self, values):
        ...
        return Config._context.remote_call(MessageCode.MSG_RPC_SHARED_HEAP_CHECK_ALL, self.shared_id, values)
    ...
    def check_any(self, values):
        ...
        """
        values = [ encode_key(v) for v in values ]
        return Config._context.remote_call(MessageCode.MSG_RPC_SHARED_HEAP_CHECK_ANY, self.shared_id, values)
    ...
    def check_each(self, values):
        ...
        """
        values = [ encode_key(v) for v in values ]
        return Config._context.remote_call(MessageCode.MSG_RPC_SHARED_HEAP_CHECK_EACH, self.shared_id, values)
    ...
    def pop_many(self, maximum):
        ...
        if maximum < 1:
            return ()
        values = Config._context.remote_call(MessageCode.MSG_RPC_SHARED_HEAP_POP, self.shared_id, maximum)
    ...
    def add_many(self, values):
        ...
        """
        values = [ encode_key(v) for v in values ]
        Config._context.async_remote_call(MessageCode.MSG_RPC_SHARED_HEAP_ADD, self.shared_id, values)
    ...
    def remove_many(self, values):
        ...
        """
        values = [ encode_key(v) for v in values ]
        Config._context.async_remote_call(MessageCode.MSG_RPC_SHARED_HEAP_REMOVE, self.shared_id, values)
...
_valid_objects = (
 None, Ellipsis)
_valid_scalars = (
 bool, int, long, float, str, unicode, complex)
_valid_containers = (
 tuple, frozenset)
_shared_containers = (
 SharedMap, SharedHeap)