...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/plugins/report/text.py
...
class TextReport(ReportPlugin):
    ...
    def is_supported(self, output_file):
        return not output_file or output_file == '-' or output_file.lower().endswith('.txt')
    ...
    def __iterate(self, data_type=None, data_subtype=None):
        if Database.count(data_type, data_type) < 100:
            return Database.get_many(Database.keys(data_type=data_type, data_subtype=data_subtype))
    ...
    def __write_report(self):
        ...
        if self.__show_data:
            emails = {e.address:'red' if e.get_links(Data.TYPE_VULNERABILITY) else 'green' for e in self.__iterate(Data.TYPE_RESOURCE, Resource.RESOURCE_EMAIL)}
            if emails:
