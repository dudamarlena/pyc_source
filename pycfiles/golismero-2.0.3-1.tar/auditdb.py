...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/golismero/database/auditdb.py
...
import md5, sqlite3, threading, time, urlparse, warnings
pymongo = None
binary = None
objectid = None
Error = None
...
class BaseAuditDB(BaseDB):
    ...
    def append_log_line(self, text, level, is_error, plugin_id, ack_id, timestamp=None):
        """
        Append a log line.
    ...
    def get_log_lines(self, from_timestamp=None, to_timestamp=None, filter_by_plugin=None, filter_by_data=None, page_num=None, per_page=None):
        """
...
class AuditSQLiteDB(BaseAuditDB):
    ...
    def __init__(self, audit_config):
        ...
        filename = self.__parse_connection_string(audit_config.audit_db, audit_config.audit_name)
        have_file = filename and filename != ':memory:' and path.exists(filename)
            ...
            if have_file:
                ...
                if audit_name:
                    if not audit_config.audit_name:
                        ...
                        raise IOError('Database belongs to another audit:\n\t%r vs. %r' % (
        ...
        if not have_file:
            if not filename:
                filename = ('').join((c if c in '-_~' or c.isalnum() else '_') for c in self.audit_name)
            ...
            if filename != ':memory:':
                ...
                if directory and not path.exists(directory):
                    try:
                        ...
                        warnings.warn('Error creating directory %r: %s' % (
                         directory, str(e)), RuntimeWarning)
    ...
    def __parse_connection_string(audit_db, audit_name=None):
        ...
        if not audit_db or audit_db == ':auto:':
            audit_db = None
            if audit_name:
                audit_db = ('').join((c if c in '-_~' or c.isalnum() else '_') for c in audit_name)
    ...
    def get_config_from_closed_database(cls, audit_db, audit_name=None):
        ...
        try:
            try:
                cursor = db.cursor()
                try:
                    cursor.execute('SELECT audit_config, audit_scope FROM golismero LIMIT 1;')
                    row = cursor.fetchone()
                    if not row:
    ...
    def __create(self, audit_config):
        ...
        :type audit_config: AuditConfig
        """
        self.__cursor.execute("SELECT count(*) FROM sqlite_master WHERE type = 'table' AND name = 'golismero';")
        if self.__cursor.fetchone()[0]:
            self.__cursor.execute('SELECT schema_version, audit_name FROM golismero LIMIT 1;')
            row = self.__cursor.fetchone()
            if not row:
                raise IOError('Broken database!')
            if row[0] != self.SCHEMA_VERSION:
                raise IOError('Incompatible schema version: %s != %s' % (
            ...
            self.__cursor.execute('INSERT INTO golismero VALUES (?, ?, NULL, NULL, ?, NULL);', (
             self.SCHEMA_VERSION,
             self.audit_name,
    ...
    def __get_data_table_and_type(self, data):
        ...
        if data_type == Data.TYPE_INFORMATION:
            ...
            warnings.warn('Received %s object of type TYPE_UNKNOWN' % type(data), RuntimeWarning, stacklevel=3)
            if isinstance(data, Information):
                data.data_type = Data.TYPE_INFORMATION
                table = 'information'
                dtype = data.information_type
                if dtype == Information.INFORMATION_UNKNOWN:
                    warnings.warn('Received %s object of subtype INFORMATION_UNKNOWN' % type(data), RuntimeWarning, stacklevel=3)
            elif isinstance(data, Resource):
                data.data_type = Data.TYPE_RESOURCE
                table = 'resource'
                dtype = data.resource_type
                if dtype == Resource.RESOURCE_UNKNOWN:
                    warnings.warn('Received %s object of subtype RESOURCE_UNKNOWN' % type(data), RuntimeWarning, stacklevel=3)
            elif isinstance(data, Vulnerability):
                data.data_type = Data.TYPE_VULNERABILITY
                table = 'vulnerability'
                dtype = data.vulnerability_type
            else:
                raise NotImplementedError('Unknown data type %r!' % type(data))
        else:
            raise NotImplementedError('Unknown data type %r!' % data_type)
        return (
         table, dtype)

    @transactional
    def get_audit_times(self):
        self.__cursor.execute('SELECT start_time, stop_time FROM golismero LIMIT 1;')
        start_time, stop_time = self.__cursor.fetchone()
        return (start_time, stop_time)

    @transactional
    def set_audit_times(self, start_time, stop_time):
        self.__cursor.execute('UPDATE golismero SET start_time = ?, stop_time = ?;', (
         start_time, stop_time))

    @transactional
    def set_audit_start_time(self, start_time):
        self.__cursor.execute('UPDATE golismero SET start_time = ?;', (
         start_time,))

    @transactional
    def set_audit_stop_time(self, stop_time):
        self.__cursor.execute('UPDATE golismero SET stop_time = ?;', (
         stop_time,))
    ...
    def save_audit_config(self, audit_config):
        if audit_config:
            self.__cursor.execute('UPDATE golismero SET audit_config = ?;', (
             self.encode(audit_config),))
        else:
    ...
    def save_audit_scope(self, audit_scope):
        if audit_scope:
            self.__cursor.execute('UPDATE golismero SET audit_scope = ?;', (
    ...
    def __add_data(self, data):
        ...
        if is_new:
            self.__cursor.execute('INSERT INTO stages (identity) VALUES (?);', (
             identity,))
        return is_new
    ...
    def remove_data(self, identity, data_type=None):
        if data_type is None:
            ...
            if data_type == Data.TYPE_INFORMATION:
                ...
                raise NotImplementedError('Unknown data type %r!' % data_type)
            for table in tables:
                self.__cursor.execute('DELETE FROM %s WHERE identity = ?;' % table, (
                 identity,))
                if self.__cursor.rowcount:
                    self.__cursor.execute('DELETE FROM history WHERE identity = ?;', (
                     identity,))
                    self.__cursor.execute('DELETE FROM stages WHERE identity = ?;', (
                     identity,))
                    return True
    ...
    def get_data_keys(self, data_type=None, data_subtype=None):
        if data_type is None:
            if data_subtype is not None:
                raise NotImplementedError("Can't filter by subtype for all types")
            hashes = set()
    ...
    def __get_data_type(self, identity):
        if type(identity) is not str:
            raise TypeError('Expected string, got %s' % type(identity))
        for table, data_type, subtype_filter in (('information', Data.TYPE_INFORMATION, int),
         (
    ...
    def mark_plugin_finished(self, identity, plugin_id):
        ...
        self.__cursor.execute('INSERT INTO history VALUES (NULL, ?, ?);', (
         plugin_rowid, identity))

    def __get_or_create_plugin_rowid(self, plugin_id):
        self.__cursor.execute('SELECT rowid FROM plugin WHERE name = ? LIMIT 1;', (
         plugin_id,))
        rows = self.__cursor.fetchone()
        if rows:
            plugin_rowid = rows[0]
        else:
            self.__cursor.execute('INSERT INTO plugin VALUES (NULL, ?);', (
             plugin_id,))
    ...
    def __mark_stage_finished(self, identity, stage):
        self.__cursor.execute('SELECT stage FROM stages WHERE identity = ? LIMIT 1;', (
         identity,))
        row = self.__cursor.fetchone()
        if row:
            do_insert = True
    ...
    def clear_stage_mark(self, identity):
        if type(identity) is not str:
            raise TypeError('Expected string, got %s' % type(identity))
        self.__cursor.execute('UPDATE stages SET stage = 0 WHERE identity = ?;', (identity,))
    ...
    def get_past_plugins(self, identity):
        if type(identity) is not str:
            raise TypeError('Expected string, got %s' % type(identity))
        self.__cursor.execute('SELECT plugin.name FROM plugin, history WHERE history.plugin_id = plugin.rowid AND       history.identity = ?;', (
         identity,))
    ...
    def get_pending_data(self, stage):
        if type(stage) is not int:
            raise TypeError('Expected integer, got %s' % type(stage))
        self.__cursor.execute('SELECT identity FROM stages WHERE stage < ?;', (
         stage,))
        rows = self.__cursor.fetchall()
        if rows:
            return {str(x[0]) for x in rows}
    ...
    def append_log_line(self, text, level, is_error, plugin_id, ack_id, timestamp=None):
        ...
        try:
            ...
            self.__cursor.execute('INSERT INTO log VALUES (NULL, ?, ?, ?, ?, ?, ?);', (
    ...
    @transactional
    def get_log_lines(self, from_timestamp=None, to_timestamp=None, filter_by_plugin=None, filter_by_data=None, page_num=None, per_page=None):
        query = 'SELECT plugin.name, log.identity, log.text, log.level, log.is_error, log.timestamp FROM plugin, log WHERE plugin.rowid = log.plugin_id'
        params = []
        if filter_by_plugin:
        ...
        if page_num is not None and page_num >= 0 and per_page is not None and per_page > 0:
    ...
    def __get_mapped_values(self, shared_id, keys):
        values = []
        for key in keys:
            self.__cursor.execute('SELECT value FROM shared_map WHERE shared_id = ? AND key_hash = ? LIMIT 1;', (
    ...
    def has_all_mapped_keys(self, shared_id, keys):
        ...
        result = True
        for key in keys:
            self.__cursor.execute('SELECT COUNT(rowid) FROM shared_map WHERE shared_id = ? AND key_hash = ? LIMIT 1;', (
             shared_id, self.get_hash(key)))
    ...
    def put_mapped_values(self, shared_id, items):
        ...
        for key, value in items:
            self.__cursor.execute('INSERT INTO shared_map VALUES (NULL, ?, ?, ?, ?);', (
             shared_id, self.get_hash(key),
    ...
    def __delete_mapped_values(self, shared_id, keys):
        for key in keys:
            self.__cursor.execute('DELETE FROM shared_map WHERE shared_id = ? AND key_hash = ?;', (
             shared_id, self.get_hash(key)))
    ...
    def get_mapped_keys(self, shared_id):
        if type(shared_id) is not str:
            raise TypeError('Expected str, got %s' % type(shared_id))
        self.__cursor.execute('SELECT key FROM shared_map WHERE shared_id = ?;', (
         shared_id,))
    ...
    def has_all_shared_values(self, shared_id, values):
        ...
        result = True
        for value in values:
            self.__cursor.execute('SELECT COUNT(rowid) FROM shared_heap WHERE shared_id = ? AND value_hash = ? LIMIT 1;', (
    ...
    def pop_shared_values(self, shared_id, maximum):
        ...
        result = ()
        if maximum:
            self.__cursor.execute('SELECT rowid, value FROM shared_heap WHERE shared_id = ?' + ' LIMIT %d;' % maximum if maximum > 0 else ';', (
             shared_id,))
            rows = self.__cursor.fetchall()
            result = tuple(self.decode(value) for _, value in rows)
            for rowid, _ in rows:
                self.__cursor.execute('DELETE FROM shared_heap WHERE shared_id = ? AND rowid = ?;', (
                 shared_id, rowid))
    ...
    def add_shared_values(self, shared_id, values):
        if type(shared_id) is not str:
            raise TypeError('Expected str, got %s' % type(shared_id))
        for value in values:
            self.__cursor.execute('INSERT INTO shared_heap VALUES (NULL, ?, ?, ?);', (
    ...
    def remove_shared_values(self, shared_id, values):
        if type(shared_id) is not str:
            raise TypeError('Expected str, got %s' % type(shared_id))
        for value in values:
            self.__cursor.execute('DELETE FROM shared_heap WHERE shared_id = ? AND value_hash = ?;', (
             shared_id, self.get_hash(value)))
...
class AuditMongoDB(BaseAuditDB):
    ...
    def _connectdb(self, audit_config):
        if self.__replicasetname:
            self.__connection = pymongo.MongoReplicaSetClient(hosts_or_uri=self.__mongoadress + ':' + str(self.__mongoport), replicaSet=self.__replicasetname)
        else:
        ...
        self._c_golismero.insert({'schema_version': self.SCHEMA_VERSION, 
    ...
    def add_data(self, data):
        ...
        if not is_new:
            old_data.merge(data)
            data = old_data
        table.update({'identity': identity}, {'$set': {'data': self.encode(data), 
                    'type': dtype}}, True)
        if is_new:
    ...
    def remove_data(self, identity, data_type=None):
        ...
        if data_type is None:
            tables = (
             self._c_information, self._c_resource, self._c_vulnerability)
        else:
            if data_type == Data.TYPE_INFORMATION:
                tables = (
                 self._c_information,)
    ...
    def get_data_type(self, identity):
        if type(identity) is not str:
            raise TypeError('Expected string, got %s' % type(identity))
        for table, data_type, subtype_filter in ((self._c_information, Data.TYPE_INFORMATION, int),
         (
    ...
    def mark_plugin_finished(self, identity, plugin_id):
        ...
        self._c_history.update({'plugin_id': plugin_id, 'identity': identity}, {'$set': {'plugin_id': plugin_id, 
    ...
    def mark_stage_finished(self, identity, stage):
        ...
        if prev_stage is None:
            prev_stage = 0
        if stage > prev_stage:
            self._c_stages.update({'identity': identity}, {'$set': {'stage': stage}}, True)
        return

    def clear_stage_mark(self, identity):
        if type(identity) is not str:
            raise TypeError('Expected string, got %s' % type(identity))
        self._c_stages.update({'identity': identity}, {'$set': {'stage': 0}}, True)

    def clear_all_stage_marks(self):
        self._c_stages.update({}, {'$set': {'stage': 0}})
    ...
    def put_mapped_values(self, shared_id, items):
        if type(shared_id) is not str:
            raise TypeError('Expected str, got %s' % type(shared_id))
        for key, value in items:
            self._c_shared_map.update({'shared_id': shared_id, 'key_hash': self.get_hash(key)}, {'shared_id': shared_id, 'key_hash': self.get_hash(key), 'key': self.encode(key), 'value': self.encode(value)}, True)
    ...
    def add_shared_values(self, shared_id, values):
        ...
        for value in values:
            ...
            self._c_shared_heap.update({'shared_id': shared_id, 'value_hash': value_hash}, {'shared_id': shared_id, 'value_hash': value_hash, 'value': self.encode(value)}, True)
...
class AuditDB(BaseAuditDB):
    ...
    def get_config_from_closed_database(cls, audit_db, audit_name=None):
        if audit_db == ':memory:':
            raise ValueError('Operation not supported for in-memory database!')
        return AuditSQLiteDB.get_config_from_closed_database(audit_db, audit_name)