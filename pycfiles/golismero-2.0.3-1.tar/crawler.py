...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/tools/xsser/XSSer/crawler.py
...
"""
import sys, cgi, urllib, urllib2, urlparse, pycurl, time, traceback, curlcontrol, threadpool
...
class Crawler(object):
    ...
    def _find_args(self, url):
        ...
        for arg_name in qs:
            key = (
             arg_name, parsed.netloc)
            zipped = zip(*self._found_args[key])
            if not zipped or path not in zipped[0]:
    ...
    def generate_result(self, arg_name, path, url):
        ...
        if attack_url not in self._parent.crawled_urls:
    ...
    def _crawl(self, basepath, path, depth=3, width=0):
        ...
        self.pool.addRequest(self._curl_main, [[path, depth, width, basepath]], self._get_done_dummy, self._get_error)
    ...
    def _check_url(self, basepath, path, href, depth, width):
        ...
        if do_crawling and href not in self._crawled:
