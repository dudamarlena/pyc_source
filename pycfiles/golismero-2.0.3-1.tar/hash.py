...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/tools/sqlmap/lib/utils/hash.py
...
def mysql_old_passwd(password, uppercase=True):
    ...
    retVal = '%08lx%08lx' % (a & 2147483647, c & 2147483647)
    if uppercase:
        return retVal.upper()
    return retVal.lower()
...
def mssql_passwd(password, salt, uppercase=False):
    ...
    unistr = ('').join(map(lambda c: ('%s\x00' if ord(c) < 256 else '%s') % utf8encode(c), password))
...
def oracle_old_passwd(password, username, uppercase=True):
    ...
    'F894844C34402B67'
    """
    IV, pad = ('\x00\x00\x00\x00\x00\x00\x00\x00', '\x00')
...
__functions__ = {HASH.MYSQL: mysql_passwd, 
            ...
            for column in columns:
                ...
                if hashRecognition(value):
                    ...
                    if col_user and i < len(table[col_user]['values']):
                        ...
                        attack_dict['%s%d' % (DUMMY_USER_PREFIX, i)] = [
                         value]
    ...
    for hash_regex in hash_regexes:
            ...
            for hash_ in hashes:
                ...
                if re.match(hash_regex, hash_):
                    ...
                    if hash_regex in (HASH.MYSQL, HASH.MYSQL_OLD, HASH.MD5_GENERIC, HASH.SHA1_GENERIC):
                        ...
                        item = [
                         (
                          user, hash_), {'username': user}]
                    elif hash_regex in HASH.ORACLE:
                        item = [
                         (
                          user, hash_), {'salt': hash_[-20:]}]
                    elif hash_regex in (HASH.MSSQL, HASH.MSSQL_OLD, HASH.MSSQL_NEW):
                        item = [
                         (
                          user, hash_), {'salt': hash_[6:14]}]
                    elif hash_regex in HASH.CRYPT_GENERIC:
                        item = [
                         (
                          user, hash_), {'salt': hash_[0:2]}]
                    elif hash_regex in HASH.WORDPRESS:
                        item = [
                         (
                          user, hash_), {'salt': hash_[4:12], 'count': 1 << ITOA64.index(hash_[3]), 'prefix': hash_[:12]}]
        ...
        if not kb.wordlists:
            while not kb.wordlists:
                ...
                except SqlmapFilePathException as msg:
