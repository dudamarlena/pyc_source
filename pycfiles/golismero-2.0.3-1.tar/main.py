...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/tools/xsser/XSSer/main.py
...
"""
import os, re, sys, datetime, hashlib, time, urllib, cgi, traceback, webbrowser, XSSer.fuzzing, XSSer.fuzzing.vectors, XSSer.fuzzing.DCP, XSSer.fuzzing.DOM, XSSer.fuzzing.HTTPsr, XSSer.fuzzing.heuristic
...
class xsser(EncoderDecoder, XSSerReporter):
    ...
    def get_gtk_directory(self):
        if self._gtkdir:
            return self._gtkdir
        local_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'gtk')
        if os.path.exists(local_path):
            self._gtkdir = local_path
            return self._gtkdir
        if os.path.exists('/usr/share/xsser/gtk'):
    ...
    def get_payloads(self):
        ...
        if options.fuzz:
            ...
            payloads = [{'payload': '">PAYLOAD', 'browser': '[IE7.0|IE6.0|NS8.1-IE] [NS8.1-G|FF2.0] [O9.02]'}]
        else:
        ...
        if self.options.nohead:
            ...
            if str(hc.info()['http-code']) in ('200', '302', '301', '401'):
                if str(hc.info()['http-code']) in ('301', ):
                    ...
                    if str(hc.info()['http-code']) in ('302', ):
                        url = url + '/'
                        payload = ''
                        query_string = ''
    ...
    def attack_url_payload(self, url, payload, query_string):
        ...
        def _cb(request, result):
            self.finish_attack_url_payload(c, request, result, payload, query_string, url, newhash)
    ...
    def finish_attack_url_payload(self, c, request, result, payload, query_string, url, orig_hash):
        self.report('=' * 75)
        ...
        if self.options.isalive <= 0:
            ...
            if self.next_isalive == True:
                ...
                self.errors_isalive = 0
            elif str(self.errors_isalive) >= str(self.options.isalive):
                self.report('---------------------')
        ...
        if c.info()['http-code'] in ('200', '302', '301'):
            if self.options.statistics:
                self.success_connection = self.success_connection + 1
            self._report_attack_success(c, dest_url, payload, query_string, url, orig_hash)
        else:
            self._report_attack_failure(c, dest_url, payload, query_string, url, orig_hash)
        if self.options.alt == None:
            pass
        else:
            self.report('=' * 45)
    ...
    def _report_attack_success(self, curl_handle, dest_url, payload, query_string, orig_url, url_orig_hash):
        """
        report success of an attack
        """
        if orig_url not in self.successfull_urls:
        ...
        if payload['browser'] == '[hashed_precheck_system]' or payload['browser'] == '[Heuristic test]':
            ...
            if payload['browser'] == '[Data Control Protocol Injection]':
            ...
            for attack_type in self._ongoing_attacks:
                ...
                elif hashing in curl_handle.body() and str(curl_handle.info()['http-code']) == '200':
                    if 'PAYLOAD' in payload['payload']:
    ...
    def _report_attack_failure(self, curl_handle, dest_url, payload, attack_vector, orig_url, url_orig_hash):
        """
        ...
        if options.url:
            ...
            if options.readfile:
                ...
                if options.crawling:
                    ...
                    all_crawled = []
                    for url in set(urls):
                        self.report('Crawling', url, options.crawling, options.crawler_width)

                    crawler = Crawler(self, Curl, all_crawled, self.pool)
    ...
    def run_crawl(self, crawler, url, options):
        ...
        crawler.crawl(url, int(options.crawler_width), int(options.crawling), options.crawler_local)
    ...
    def print_results(self):
            ...
            for line in self.hash_found:
                ...
                if line[2] == 'xsr':
                    ...
                    if xsa_vulnerable_host[0]['payload'] == line[4] and xsa_vulnerable_host[0]['target'] == line[6] and self.xsa_founded > 1:
                        ...
                        self.report('[I] Target:', line[6])
                        self.report('[+] Injection:', str(line[6]) + '/' + str(line[4]), '[', Curl.agent, ']')
                    ...
                    if coo_vulnerable_host[0]['payload'] == line[4] and coo_vulnerable_host[0]['target'] == line[6] and self.coo_founded > 1:
                        ...
                        self.report('[I] Target:', line[6])
                        self.report('[+] Injection:', str(line[6]) + '/' + str(line[4]), '[', Curl.cookie, ']')
                    ...
                    self.report('[I] Target:', line[6])
                    self.report('[+] Injection:', str(line[6]) + '/' + str(line[4]), '[', line[5]['payload'], ']')
                    ...
                    if str(line[5]['payload']).strip('XSS') == '\\' or str(line[5]['payload']).strip('XSS') == '/' or str(line[5]['payload']).strip('XSS') == '>' or str(line[5]['payload']).strip('XSS') == '<' or str(line[5]['payload']).strip('XSS') == ';' or str(line[5]['payload']).strip('XSS') == "'" or str(line[5]['payload']).strip('XSS') == '"' or str(line[5]['payload']).strip('XSS') == '=':
                        self.report('[I] Target:', line[6])
                        self.report('[+] Parameter(s):', '[', str(line[5]['payload']).strip('XSS'), ']')
        ...
        if self.options.statistics:
            if self.options.heuristic:
                ...
                self.report('=' * 75)
                self.report('-' * 50)
                self.report('  ', '    <not-filt>', '   <filtered>', '    =', '    ASCII', '   +', '   UNE/HEX', '   +', '   DEC')
                ...
                self.report('; ', '       ', self.heuris_semicolon_founded, '            ', heuris_semicolon_total_founded, '               ', self.heuris_semicolon_notfounded, '            ', self.heuris_une_semicolon_founded, '           ', self.heuris_dec_semicolon_founded)
                ...
                self.report('\\ ', '       ', self.heuris_backslash_founded, '            ', heuris_backslash_total_founded, '               ', self.heuris_backslash_notfounded, '            ', self.heuris_une_backslash_founded, '           ', self.heuris_dec_backslash_founded)
                heuris_slash_total_founded = self.heuris_slash_notfounded + self.heuris_une_slash_founded + self.heuris_dec_slash_founded
                self.report('/ ', '       ', self.heuris_slash_founded, '            ', heuris_slash_total_founded, '               ', self.heuris_slash_notfounded, '            ', self.heuris_une_slash_founded, '           ', self.heuris_dec_slash_founded)
                heuris_minor_total_founded = self.heuris_minor_notfounded + self.heuris_une_minor_founded + self.heuris_dec_minor_founded
                self.report('< ', '       ', self.heuris_minor_founded, '            ', heuris_minor_total_founded, '               ', self.heuris_minor_notfounded, '            ', self.heuris_une_minor_founded, '           ', self.heuris_dec_minor_founded)
                heuris_mayor_total_founded = self.heuris_mayor_notfounded + self.heuris_une_mayor_founded + self.heuris_dec_mayor_founded
                self.report('> ', '       ', self.heuris_mayor_founded, '            ', heuris_mayor_total_founded, '               ', self.heuris_mayor_notfounded, '            ', self.heuris_une_mayor_founded, '           ', self.heuris_dec_mayor_founded)
                ...
                self.report('" ', '       ', self.heuris_doublecolon_founded, '            ', heuris_doublecolon_total_founded, '               ', self.heuris_doublecolon_notfounded, '            ', self.heuris_une_doublecolon_founded, '           ', self.heuris_dec_doublecolon_founded)
                heuris_colon_total_founded = self.heuris_colon_notfounded + self.heuris_une_colon_founded + self.heuris_dec_colon_founded
                self.report("' ", '       ', self.heuris_colon_founded, '            ', heuris_colon_total_founded, '               ', self.heuris_colon_notfounded, '            ', self.heuris_une_colon_founded, '           ', self.heuris_dec_colon_founded)
                heuris_equal_total_founded = self.heuris_equal_notfounded + self.heuris_une_equal_founded + self.heuris_dec_equal_founded
                self.report('= ', '       ', self.heuris_equal_founded, '            ', heuris_equal_total_founded, '               ', self.heuris_equal_notfounded, '            ', self.heuris_une_equal_founded, '           ', self.heuris_dec_equal_founded)
                self.report('-' * 70)
            ...
            if len(self.hash_found) + len(self.hash_notfound) == 0:
                ...
                self.report('-' * 25)
                self.report('200-OK:', self.success_connection, '|', '404:', self.not_connection, '|', '503:', self.forwarded_connection, '|', 'Others:', self.other_connection)
                try:
                ...
                self.report('-' * 25)
                self.report('Checker:', self.check_positives, '|', 'Manual:', self.manual_injection, '|', 'Auto:', self.auto_injection, '|', 'DCP:', self.dcp_injection, '|', 'DOM:', self.dom_injection, '|', 'Induced:', self.httpsr_injection, '|', 'XSR:', self.xsr_injection, '|', 'XSA:', self.xsa_injection, '|', 'COO:', self.coo_injection)
                self.report('-' * 50)
                self.report('Total Injections:', len(self.hash_notfound) + len(self.hash_found))
                self.report('-' * 25)
                self.report('Failed:', len(self.hash_notfound), '|', 'Sucessfull:', len(self.hash_found))
                try:
                    _accur = len(self.hash_found) * 100 / total_injections
                except ZeroDivisionError:
                    _accur = 0
                ...
                self.report('-' * 25)
                self.report('Checker:', self.false_positives, '|', 'Manual:', self.manual_founded, '|', 'Auto:', self.auto_founded, '|', 'DCP:', self.dcp_founded, '|', 'DOM:', self.dom_founded, '|', 'Induced:', self.httpsr_founded, '|', 'XSR:', self.xsr_founded, '|', 'XSA:', self.xsa_founded, '|', 'COO:', self.coo_founded)
                self.report('-' * 50)
                self.report('False positives:', self.false_positives, '|', 'Vulnerables:', total_discovered - self.false_positives)
                self.report('-' * 25)
                mana = 0
