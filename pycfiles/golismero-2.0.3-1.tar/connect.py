...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/tools/sqlmap/lib/request/connect.py
# Compiled at: 2013-12-09 06:41:17
"""
Copyright (c) 2006-2013 sqlmap developers (http://sqlmap.org/)
See the file 'doc/COPYING' for copying permission
"""
import httplib, json, logging, re, socket, string, time, urllib2, urlparse, traceback
...
class Connect(object):
    ...
    def _connReadProxy(conn):
        ...
        if not kb.dnsMode and conn:
            headers = conn.info()
            if headers and (headers.getheader(HTTP_HEADER.CONTENT_ENCODING, '').lower() in ('gzip',
                                                                                            'deflate') or 'text' not in headers.getheader(HTTP_HEADER.CONTENT_TYPE, '').lower()):
                ...
                while True:
                    ...
                    if len(_) == MAX_CONNECTION_CHUNK_SIZE:
                        ...
                        singleTimeWarnMessage(warnMsg)
                        _ = re.sub('(?si)%s.+?%s' % (kb.chars.stop, kb.chars.start), '%s%s%s' % (kb.chars.stop, LARGE_CHUNK_TRIM_MARKER, kb.chars.start), _)
                        retVal += _
                    else:
                        retVal += _
                        break
    ...
    def getPage(**kwargs):
        ...
        if conf.dummy:
            ...
            requestMsg += (any((refreshing, crawling)) or '%s%s') % (_.path or '/', '?%s' % _.query if _.query else '') if 1 else url
                ...
                try:
                    ...
                    if direct_:
                        ...
                        if multipart:
                            ...
                            if not skipRead:
                                page = Connect._connReadProxy(conn) if 1 else None
                                responseHeaders = conn.info()
                                responseHeaders[URI_HTTP_HEADER] = conn.geturl()
                                page = decodePage(page, responseHeaders.get(HTTP_HEADER.CONTENT_ENCODING), responseHeaders.get(HTTP_HEADER.CONTENT_TYPE))
                                return page
                            if any((refreshing, crawling)):
                                pass
                            else:
                                if target:
                            ...
                            if not (post is not None and getRequestHeader(req, HTTP_HEADER.CONTENT_LENGTH)):
                    ...
                    if hasattr(conn, 'redurl'):
                        page = threadData.lastRedirectMsg[1] if skipRead or kb.redirectChoice == REDIRECTION.NO else Connect._connReadProxy(conn) if 1 else None
                        skipLogTraffic = kb.redirectChoice == REDIRECTION.NO
                        code = conn.redcode
                    else:
                        page = (skipRead or Connect._connReadProxy)(conn) if 1 else None
                    ...
                    try:
                        try:
                            page = (skipRead or e.read)() if 1 else None
                    ...
                    if e.code == httplib.UNAUTHORIZED:
                        ...
                        logger.debug(debugMsg)
                except (urllib2.URLError, socket.error, socket.timeout, httplib.BadStatusLine, httplib.IncompleteRead, ProxyError, SqlmapCompressionException) as e:
                            ...
                            if 'timed out' in tbMsg:
                                ...
                                if 'URLError' in tbMsg or 'error' in tbMsg:
                                    ...
                                    if 'forcibly closed' in tbMsg:
                                        logger.critical(warnMsg)
                                        return (None, None, None)
                                if ignoreTimeout and any(_ in tbMsg for _ in ('timed out',
                                                                              'IncompleteRead')):
                                    return (None, None, None)
                            if threadData.retriesCount < conf.retries and not kb.threadException:
                                warnMsg += '. sqlmap is going to retry the request'
                ...
                if place == PLACE.CUSTOM_POST:
                    if kb.postHint in (POST_HINT.SOAP, POST_HINT.XML):
                        ...
                        if payload.startswith('"') and payload.endswith('"'):
                            ...
                            payload = json.dumps(payload)[1:-1]
                    value = agent.replacePayload(value, payload)
                elif place in (PLACE.GET, PLACE.URI, PLACE.COOKIE) and not conf.skipUrlEncode or place in (PLACE.POST,) and urlEncodePost:
                ...
                for name, value in variables.items():
                    if name != '__builtins__' and originals.get(name, '') != value:
                        if isinstance(value, (basestring, int)):
                            found = False
                            value = unicode(value)
                            regex = '((\\A|%s)%s=).+?(%s|\\Z)' % (re.escape(delimiter), name, re.escape(delimiter))
                            ...
                            regex = '((\\A|%s)%s=).+?(%s|\\Z)' % (re.escape(conf.cDel or DEFAULT_COOKIE_DELIMITER), name, re.escape(conf.cDel or DEFAULT_COOKIE_DELIMITER))
