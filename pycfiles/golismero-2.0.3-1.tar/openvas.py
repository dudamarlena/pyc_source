...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/plugins/testing/scan/openvas.py
...
from golismero.api.data.vulnerability.infrastructure.outdated_platform import *
...
class OpenVASPlugin(TestingPlugin):
    ...
    def check_params(self):
        ...
        try:
            ...
            raise RuntimeError('Remote host is running an unsupported version of OpenVAS. Only OpenVAS 6 is currently supported.')
    ...
    def recv_info(self, info):
        if not self.state.check('connection_down'):
            ...
            if m_timeout.lower().strip() in ('inf', 'infinite', 'none'):
                ...
                try:
                    Logger.log_more_verbose('Connecting to OpenVAS server at %s:%d' % (m_host, m_port))
                    m_scanner = VulnscanManager(m_host, m_user, m_password, m_port, m_timeout)
                except VulnscanVersionError:
                    Logger.log_error('Remote host is running an unsupported version of OpenVAS. Only OpenVAS 6 is currently supported.')
            ...
            try:
                try:
                    m_scan_id, m_target_id = m_scanner.launch_scan(target=info.address, profile=m_profile, callback_end=partial(lambda x: x.set(), m_event), callback_progress=OpenVASProgress(self.update_status))
    ...
    def parse_results(openvas_results, ip=None):
        ...
        results = []
        hosts_seen = {}
        OPV_LEVELS_TO_GLM_LEVELS = {'debug': 'informational', 
           'log': 'informational', 
           'low': 'low', 
           'medium': 'middle', 
           'high': 'high'}
        ...
        if not use_openvas_db:
            ...
            try:
                settings = settings(DATABASES={'default': {'ENGINE': 'django.db.backends.sqlite3', 
                               'NAME': '%s' % openvas_db}})
            except RuntimeError:
            ...
            if os.path.exists(openvas_yaml):
                try:
                    CATEGORIES = yaml.load(open(openvas_yaml, 'rU'), Loader=Loader)
                except Exception as e:
                    tb = format_exc()
                    Logger.log_error_verbose('Failed to load categories, reason: %s' % str(e))
                    Logger.log_error_more_verbose(tb)

            else:
                Logger.log_error('Missing OpenVAS categories file: %s' % openvas_yaml)
            for opv in openvas_results:
                try:
                    if not isinstance(opv, OpenVASResult):
                        warn('Expected OpenVASResult, got %r instead' % type(opv), RuntimeWarning)
                        continue
                    host = opv.host
                    if host is None:
                        continue
                    ...
                    if not description:
                        ...
                        if not description:
                            ...
                            if not description:
                                description = None
                    if opv.notes:
                        description += '\n' + ('\n').join(' - ' + note.text for note in opv.notes)
                    references = extract_from_text(description)
                    kwargs = {'level': OPV_LEVELS_TO_GLM_LEVELS[level.lower()], 
                    ...
                    if use_openvas_db and oid:
                        ...
                        if len(oid_spt) > 0:
                            ...
                            try:
                                ...
                                tb = format_exc()
                                Logger.log_error_verbose('Failed to find category %r, reason: %s' % (
                                 l_plugin_id, str(e)))
                                Logger.log_error_more_verbose(tb)
...
class OpenVASImportPlugin(ImportPlugin):
    ...
    def import_results(self, input_file):
        try:
            ...
            fmt = format_exc()
            Logger.log_error('Could not load OpenVAS results from file: %s' % input_file)
            Logger.log_error_verbose(str(e))
            Logger.log_error_more_verbose(fmt)

        if golismero_results:
            data_count = len(golismero_results)
            vuln_count = sum(1 for x in golismero_results if x.is_instance(Vulnerability))
            if vuln_count == 0:
                vuln_msg = ''
            elif vuln_count == 1:
