...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/thirdparty_libs/dns/rdtypes/mxbase.py
# Compiled at: 2013-08-26 10:52:44
"""MX-like base classes."""
import cStringIO, struct, dns.exception, dns.rdata, dns.name
...
class MXBase(dns.rdata.Rdata):
    ...
    def to_digestable(self, origin=None):
        return struct.pack('!H', self.preference) + self.exchange.to_digestable(origin)

    def from_wire(cls, rdclass, rdtype, wire, current, rdlen, origin=None):
        preference, = struct.unpack('!H', wire[current:current + 2])
        current += 2
        rdlen -= 2
        exchange, cused = dns.name.from_wire(wire[:current + rdlen], current)
        if cused != rdlen:
