...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/plugins/testing/scan/nikto.py
...
from golismero.api.external import run_external_tool, find_binary_in_path, find_cygwin_binary_in_path, tempfile, get_tools_folder
...
class NiktoPlugin(TestingPlugin):
                ...
                with tempfile(suffix='.conf') as (tmp_config):
                    with open(config, 'rU') as (src):
                        with open(tmp_config, 'w') as (dst):
                            ...
                            if cookie_dict:
                                cookie = (';').join('"%s=%s"' % x for x in cookie_dict.iteritems())
                                dst.write('STATIC-COOKIE=%s\n' % cookie)
                    args = [
                     '-config', tmp_config] + args
    ...
    def run_nikto(self, info, output_filename, command, args):
        ...
        with ConnectionSlot(info.hostname):
            code = run_external_tool(command, args, cwd=cwd, callback=Logger.log_verbose)
        if code:
...
class NiktoImportPlugin(ImportPlugin):
    ...
    def import_results(self, input_file):
        try:
            results, vuln_count = NiktoPlugin.parse_nikto_results(None, input_file)
            if results:
                Database.async_add_many(results)
        except Exception as e:
            fmt = format_exc()
            Logger.log_error('Could not load Nikto results from file: %s' % input_file)
            Logger.log_error_verbose(str(e))
            Logger.log_error_more_verbose(fmt)

        if results:
            Logger.log('Loaded %d vulnerabilities and %d resources from file: %s' % (
