...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/tools/sqlmap/lib/core/option.py
# Compiled at: 2013-12-09 06:41:17
"""
Copyright (c) 2006-2013 sqlmap developers (http://sqlmap.org/)
See the file 'doc/COPYING' for copying permission
"""
import cookielib, glob, inspect, logging, os, re, socket, string, sys, threading, time, urllib2, urlparse, lib.controller.checks, lib.core.common, lib.core.threads, lib.core.convert, lib.request.connect
...
def _urllib2Opener():
    ...
    handlers = [
     proxyHandler, authHandler, redirectHandler, rangeHandler, httpsHandler]
...
def _feedTargetsDict(reqFile, addedTargetUrls):
    ...
    def _parseWebScarabLog(content):
        ...
        for request in reqResList:
            url = extractRegexResult('URL: (?P<result>.+?)\\n', request, re.I)
            method = extractRegexResult('METHOD: (?P<result>.+?)\\n', request, re.I)
            cookie = extractRegexResult('COOKIE: (?P<result>.+?)\\n', request, re.I)
    ...
    def _parseBurpLog(content):
        ...
        for match in reqResList:
            ...
            for index in xrange(len(lines)):
                ...
                line = line.strip('\r')
                match = (method or re.search)('\\A(%s) (.+) HTTP/[\\d.]+\\Z' % ('|').join(getPublicTypeMembers(HTTPMETHOD, True)), line) if 1 else None
...
def _setBulkMultipleTargets():
    if not conf.bulkFile:
        ...
        for line in getFileItems(conf.bulkFile):
            if re.match('[^ ]+\\?(.+)', line, re.I) or CUSTOM_INJECTION_MARK_CHAR in line:
...
def _setMetasploit():
    ...
    if IS_WIN:
        if not conf.msfPath:
            ...
            conf.msfPath = _('SOFTWARE\\Rapid7\\Metasploit', 'Location')
            if conf.msfPath:
    ...
    if conf.msfPath:
        for path in (conf.msfPath, os.path.join(conf.msfPath, 'bin')):
            if all(os.path.exists(normalizePath(os.path.join(path, _))) for _ in ('',
                                                                                  'msfcli',
                                                                                  'msfconsole',
                                                                                  'msfencode',
                                                                                  'msfpayload')):
    ...
    if not msfEnvPathExists:
        ...
        for envPath in envPaths:
            ...
            if all(os.path.exists(normalizePath(os.path.join(envPath, _))) for _ in ('',
                                                                                     'msfcli',
                                                                                     'msfconsole',
                                                                                     'msfencode',
                                                                                     'msfpayload')):
    ...
    if conf.tamper:
        ...
        for tfile in re.split(PARAMETER_SPLITTING_REGEX, conf.tamper):
            ...
            if not tfile:
                ...
                if os.path.exists(os.path.join(paths.SQLMAP_TAMPER_PATH, tfile if tfile.endswith('.py') else '%s.py' % tfile)):
            ...
            if not hasattr(module, '__priority__'):
                priority = PRIORITY.NORMAL if 1 else module.__priority__
                ...
                errMsg = found or "missing function 'tamper(payload, **kwargs)' "
...
def _setHTTPAuthentication():
    ...
    if not conf.authType and not conf.authCred and not conf.authPrivate:
        return
    if conf.authType and not conf.authCred and not conf.authPrivate:
...
def _cleanupOptions():
    ...
    if conf.optimize:
        setOptimize()
    if conf.data:
        conf.data = re.sub(INJECT_HERE_MARK.replace(' ', '[^A-Za-z]*'), CUSTOM_INJECTION_MARK_CHAR, conf.data, re.I)
    if conf.url:
        conf.url = re.sub(INJECT_HERE_MARK.replace(' ', '[^A-Za-z]*'), CUSTOM_INJECTION_MARK_CHAR, conf.url, re.I)
    if conf.os:
    ...
    if conf.csvDel:
        conf.csvDel = conf.csvDel.decode('string_escape')
...
def _setKnowledgeBaseAttributes(flushAll=True):
    ...
    kb.locks = AttribDict()
    for _ in ('cache', 'count', 'index', 'io', 'limit', 'log', 'redirect', 'request',
              'value'):
...
def _basicOptionValidation():
    ...
    if conf.risk is not None and not (isinstance(conf.risk, int) and conf.risk > 0):
        ...
        raise SqlmapSyntaxException(errMsg)
    if conf.limitStart is not None and isinstance(conf.limitStart, int) and conf.limitStart > 0 and conf.limitStop is not None and isinstance(conf.limitStop, int) and conf.limitStop < conf.limitStart:
        errMsg = "value for option '--start' (limitStart) must be smaller or equal than value for --stop (limitStop) option"
        raise SqlmapSyntaxException(errMsg)
    if conf.firstChar is not None and isinstance(conf.firstChar, int) and conf.firstChar > 0 and conf.lastChar is not None and isinstance(conf.lastChar, int) and conf.lastChar < conf.firstChar:
