...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/golismero/api/text/text_utils.py
...
def char_count(text):
    ...
    :rtype: int
    """
    return sum(1 for _ in finditer('\\w', text))
...
__uncamelcase_re = re.compile('%s|%s|%s' % ('(?<=[A-Z])(?=[A-Z][a-z])', '(?<=[^A-Z])(?=[A-Z])',
                                            '(?<=[A-Za-z])(?=[^A-Za-z])'))
