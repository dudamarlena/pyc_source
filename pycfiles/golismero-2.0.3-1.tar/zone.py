...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/thirdparty_libs/dns/zone.py
# Compiled at: 2013-08-26 10:52:44
"""DNS Zones."""
from __future__ import generators
import sys, re, dns.exception, dns.name, dns.node, dns.rdataclass, dns.rdatatype, dns.rdata, dns.rrset, dns.tokenizer, dns.ttl, dns.grange
...
class Zone(object):
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, Zone):
            return False
        if self.rdclass != other.rdclass or self.origin != other.origin or self.nodes != other.nodes:
            return False
        return True
    ...
    def to_file(self, f, sorted=True, relativize=True, nl=None):
        ...
        try:
            if sorted:
                ...
                names = self.iterkeys()
            for n in names:
                l = self[n].to_text(n, origin=self.origin, relativize=relativize)
                if nl is None:
                    print >> f, l
                else:
                    f.write(l)
            ...
            while 1:
                ...
                if token.is_eof():
                    ...
                    if len(self.saved_state) > 0:
                        self.tok, self.current_origin, self.last_name, self.current_file, self.ttl = self.saved_state.pop(-1)
                        continue
                    break
                elif token.is_eol():
                    continue
...
def from_text(text, origin=None, rdclass=dns.rdataclass.IN, relativize=True, zone_factory=Zone, filename=None, allow_include=False, check_origin=True):
    ...
    reader = _MasterReader(tok, origin, rdclass, relativize, zone_factory, allow_include=allow_include, check_origin=check_origin)
    reader.read()
    return reader.zone
...
def from_file(f, origin=None, rdclass=dns.rdataclass.IN, relativize=True, zone_factory=Zone, filename=None, allow_include=True, check_origin=True):
    ...
    if isinstance(f, str_type):
        ...
        want_close = False
    try:
        z = from_text(f, origin, rdclass, relativize, zone_factory, filename, allow_include, check_origin)
    finally:
        if want_close:
            f.close()
