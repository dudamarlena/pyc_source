...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/tools/sqlmap/lib/techniques/union/test.py
...
def _findUnionCharCount(comment, place, parameter, value, prefix, suffix, where=PAYLOAD.WHERE.ORIGINAL):
    ...
    def _orderByTechnique():
        ...
        def _orderByTest(cols):
            ...
            return not re.search('(warning|error|order by|failed)', page or '', re.I) and comparison(page, headers) or re.search('data types cannot be compared or sorted', page or '', re.I)
    ...
    if not isNullValue(kb.uChar):
        for regex in (kb.uChar, '>\\s*%s\\s*<' % kb.uChar):
        ...
        if content and phrase in content:
            ...
            if where == PAYLOAD.WHERE.ORIGINAL:
                ...
                if not all(_ in content for _ in (phrase, phrase2)):
                    vector = (
                     position, count, comment, prefix, suffix, kb.uChar, PAYLOAD.WHERE.NEGATIVE, kb.unionDuplicates)
