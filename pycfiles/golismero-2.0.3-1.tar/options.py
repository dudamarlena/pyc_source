...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/tools/xsser/XSSer/options.py
...
class XSSerOptions(optparse.OptionParser):
    ...
    def __init__(self, *args):
        optparse.OptionParser.__init__(self, description='Cross Site "Scripter" is an automatic -framework- to detect, exploit and\nreport XSS vulnerabilities in web-based applications.', prog='XSSer.py', version='\nXSSer v1.6 (beta): "Grey Swarm!" - 2011/2012 - (GPLv3.0) -> by psy\n', usage='\n\nxsser [OPTIONS] [-u <url> |-i <file> |-d <dork>] [-g <get> |-p <post> |-c <crawl>] [Request(s)] [Vector(s)] [Bypasser(s)] [Technique(s)] [Final Injection(s)]')
        self.set_defaults(verbose=False, threads=5, retries=1, delay=0, timeout=30, silent=False)
        ...
        group1 = optparse.OptionGroup(self, '*Special Features*', 'You can choose Vector(s) and Bypasser(s) to inject code with this extra special features:')
        ...
        group2 = optparse.OptionGroup(self, '*Select Target(s)*', 'At least one of these options has to be specified to set the source to get target(s) urls from. You need to choose to run XSSer:')
        ...
        group3 = optparse.OptionGroup(self, '*Select type of HTTP/HTTPS Connection(s)*', 'These options can be used to specify which parameter(s) we want to use like payload to inject code.')
        ...
        group4 = optparse.OptionGroup(self, '*Configure Request(s)*', 'These options can be used to specify how to connect to target(s) payload(s). You can choose multiple:')
        ...
        group5 = optparse.OptionGroup(self, '*Checker Systems*', "This options are usefull to know if your target(s) have some filters against XSS attacks, to reduce 'false positive' results and to perform more advanced tests:")
        ...
        group6 = optparse.OptionGroup(self, '*Select Vector(s)*', "These options can be used to specify a XSS vector source code to inject in each payload. Important, if you don't want to try to inject a common XSS vector, used by default. Choose only one option:")
        ...
        group7 = optparse.OptionGroup(self, '*Select Bypasser(s)*', 'These options can be used to encode selected vector(s) to try to bypass possible anti-XSS filters on target(s) code and possible IPS rules, if the target use it. Also, can be combined with other techniques to provide encoding:')
        ...
        group8 = optparse.OptionGroup(self, '*Special Technique(s)*', 'These options can be used to try to inject code using different type of XSS techniques. You can choose multiple:')
        ...
        group9 = optparse.OptionGroup(self, '*Select Final injection(s)*', 'These options can be used to specify the final code to inject in vulnerable target(s). Important, if you want to exploit on-the-wild your discovered vulnerabilities. Choose only one option:')
        ...
        group10 = optparse.OptionGroup(self, '*Special Final injection(s)*', "These options can be used to execute some 'special' injection(s) in vulnerable target(s). You can select multiple and combine with your final code (except with DCP code):")
