...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/golismero/api/net/web_utils.py
...
__user_agents = (
 ...
 'Mozilla/5.0 (compatible; Yahoo! Slurp; http://help.yahoo.com/help/us/ysearch/slurp)Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)',
...
def download(url, callback=None, timeout=10.0, allow_redirects=True, allow_out_of_scope=False):
    ...
    request = HTTP_Request(url=url.url, method=url.method, post_data=url.post_params, referer=url.referer, user_agent=Config.audit_config.user_agent)
    ...
    response = HTTP.make_request(request, callback=temp_callback, timeout=timeout, allow_redirects=allow_redirects, allow_out_of_scope=allow_out_of_scope)
    if response:
...
def detect_auth_method(url):
    ...
    if 'www-authenticate' in r.headers:
        ...
        authobj = re.compile('(?:\\s*www-authenticate\\s*:)?\\s*(\\w*)\\s+realm=[\'"]([^\'"]+)[\'"]', re.IGNORECASE)
...
class ParsedURL(object):
    ...
    **Missing properties are returned as empty strings**, except for the port
    ...
    default_ports = {'http': 80, 
       ...
       'ftp': 21, 
       'mailto': 25}
    __two_dashes_required = ('http', 'https', 'ftp')
    ...
    def to_urllib3(self):
        ...
        return Urllib3_Url(self.__scheme, self.auth, self.__host, self.port, self.__path, self.query, self.__fragment)

    def match_extension(self, extension, directory_allowed=True, double_allowed=True, case_insensitive=True):
        """
    ...
    def fragment(self, fragment):
        if not fragment:
            ...
            if fragment.startswith('#'):
                warn("You don't need to use a leading '#' when setting the fragment, this may be an error!", stacklevel=3)
    ...
    def query(self):
        if not self.__query_params:
            if self.__query is not None:
                return self.__query
            return ''
        return ('&').join('%s=%s' % (quote(k, safe=''), quote(v, safe='')) for k, v in sorted(self.__query_params.iteritems()))

    @query.setter
    def query(self, query):
        if query and query.startswith('?'):
            warn("You don't need to use a leading '?' when setting the query string, this may be an error!", stacklevel=3)
        if not query:
            query_params = {}
        else:
            try:
                query_params = dict(map(unquote_plus, (to_utf8(token) + '=').split('=', 2)[:2]) for token in query.split('&'))
...
class HTMLElement(object):
    ...
    def __init__(self, tag_name, attrs, content):
        ...
        self.__attrs = {to_utf8(k):to_utf8(v) for k, v in attrs.iteritems()}
...
class HTMLParser(object):
    ...
    def __convert_to_HTMLElements(self, data):
        ...
        """
        return [ HTMLElement(x.name.encode('utf-8'), {v[0].encode('utf-8'):v[1].encode('utf-8') for v in x.attrs}, ('').join(str(item) for item in x.contents if item != '\n')) for x in data
               ]
