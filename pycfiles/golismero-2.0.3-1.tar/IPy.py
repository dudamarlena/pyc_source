...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/tools/theHarvester/discovery/IPy.py
...
class IPint():
    ...
    def overlaps(self, item):
        ...
        if item.ip >= self.ip and item.ip < self.ip + self.len():
            ...
            if self.ip >= item.ip and self.ip < item.ip + item.len():
                return -1
            return 0
    ...
    def reverseNames(self):
        ...
        if self._ipversion == 4:
            ...
            if self.len() < 256:
                ...
                for i in range(0, self.len(), 256):
                    ret.append(self[i].reverseName()[2:])

            elif self.len() < 16777216:
                for i in range(0, self.len(), 65536):
                    ret.append(self[i].reverseName()[4:])

            else:
                for i in range(0, self.len(), 16777216):
        ...
        if self._ipversion == 6:
            ...
            return [
             '%s.ip6.int.' % s[first_nibble_index:]]
        raise ValueError, 'only IPv4 and IPv6 supported'
...
def parseAddress(ipstr):
    ...
    if ipstr.find(':') != -1:
        ...
        if len(ipstr) == 32:
            return (
             long(ipstr, 16), 6)
        if ipstr.find('.') != -1 or len(ipstr) < 4 and int(ipstr) < 256:
            bytes = ipstr.split('.')
            if len(bytes) > 4:
...
_BitTable = {'0': '0000', '1': '0001', '2': '0010', '3': '0011', '4': '0100', 
