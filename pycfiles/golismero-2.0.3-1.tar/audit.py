...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/golismero/api/audit.py
...
def get_audit_config(audit_name=None):
    ...
    return Config._context.remote_call(MessageCode.MSG_RPC_AUDIT_CONFIG, audit_name)
...
def get_audit_times(audit_name=None):
    ...
    """
    return Config._context.remote_call(MessageCode.MSG_RPC_AUDIT_TIMES, audit_name)
...
def get_audit_stats(audit_name=None):
    ...
    :rtype: dict(str -> \*)
    """
    return Config._context.remote_call(MessageCode.MSG_RPC_AUDIT_STATS, audit_name)


def get_audit_log_lines(audit_name=None, from_timestamp=None, to_timestamp=None, filter_by_plugin=None, filter_by_data=None, page_num=None, per_page=None):
    """
    ...
    """
    return Config._context.remote_call(MessageCode.MSG_RPC_AUDIT_LOG, audit_name, from_timestamp, to_timestamp, filter_by_plugin, filter_by_data, page_num, per_page)
...
def get_audit_scope(audit_name):
    ...
    return Config._context.remote_call(MessageCode.MSG_RPC_AUDIT_SCOPE, audit_name)
...
def start_audit(audit_config):
    ...
    Config._context.send_msg(message_type=MessageType.MSG_TYPE_CONTROL, message_code=MessageCode.MSG_CONTROL_START_AUDIT, message_info=audit_config, priority=MessagePriority.MSG_PRIORITY_HIGH)
...
def stop_audit(audit_name=None):
    ...
    msg = Message(message_type=MessageType.MSG_TYPE_CONTROL, message_code=MessageCode.MSG_CONTROL_STOP_AUDIT, message_info=True, audit_name=audit_name, priority=MessagePriority.MSG_PRIORITY_HIGH)
...
def cancel_audit(audit_name=None):
    ...
    if not audit_name:
        audit_name = Config.audit_name
    msg = Message(message_type=MessageType.MSG_TYPE_CONTROL, message_code=MessageCode.MSG_CONTROL_STOP_AUDIT, message_info=False, audit_name=audit_name, priority=MessagePriority.MSG_PRIORITY_HIGH)
    Config._context.send_raw_msg(msg)