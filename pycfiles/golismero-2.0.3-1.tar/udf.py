...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/tools/sqlmap/lib/takeover/udf.py
...
class UDF:
    ...
    def udfInjectCustom(self):
        ...
        if not self.udfLocalFile.endswith('.dll') and not self.udfLocalFile.endswith('.so'):
            ...
            if self.udfLocalFile.endswith('.so') and Backend.isOs(OS.WINDOWS):
                errMsg = 'you provided a shared object as shared library, but '
                errMsg += 'the database underlying operating system is Windows'
                raise SqlmapMissingMandatoryOptionException(errMsg)
            else:
                if self.udfLocalFile.endswith('.dll') and Backend.isOs(OS.LINUX):
