...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/tools/sqlmap/lib/request/inject.py
...
def _goInferenceProxy(expression, fromUser=False, batch=False, unpack=True, charsetType=None, firstChar=None, lastChar=None, dump=False):
    ...
    if not unpack:
        ...
        if fromUser and ' FROM ' in expression.upper() and (Backend.getIdentifiedDbms() not in FROM_DUMMY_TABLE or Backend.getIdentifiedDbms() in FROM_DUMMY_TABLE and not expression.upper().endswith(FROM_DUMMY_TABLE[Backend.getIdentifiedDbms()])) and not re.search(SQL_SCALAR_REGEX, expression, re.I):
            ...
            if limitCond:
                test = True
                if (not stopLimit or stopLimit <= 1) and Backend.getIdentifiedDbms() in FROM_DUMMY_TABLE and expression.upper().endswith(FROM_DUMMY_TABLE[Backend.getIdentifiedDbms()]):
                    test = False
            if test:
            ...
            if Backend.getIdentifiedDbms() in FROM_DUMMY_TABLE and expression.upper().startswith('SELECT ') and ' FROM ' not in expression.upper():
...
def getValue(expression, blind=True, union=True, error=True, time=True, fromUser=False, expected=None, batch=False, unpack=True, resumeValue=True, charsetType=None, firstChar=None, lastChar=None, dump=False, suppressOutput=None, expectingNone=False, safeCharEncode=True):
    ...
    try:
        ...
        if conf.direct:
            ...
            found = False
            count = 0
            if query and not re.search('COUNT.*FROM.*\\(.*DISTINCT', query, re.I):
