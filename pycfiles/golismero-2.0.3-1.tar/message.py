...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/thirdparty_libs/dns/message.py
# Compiled at: 2013-08-26 10:52:44
"""DNS Messages"""
import cStringIO, random, struct, sys, time, dns.edns, dns.exception, dns.flags, dns.name, dns.opcode, dns.entropy, dns.rcode, dns.rdata, dns.rdataclass, dns.rdatatype, dns.rrset, dns.renderer, dns.tsig, dns.wiredata
...
class Message(object):
    ...
    def section_number(self, section):
        if section is self.question:
            return 0
        if section is self.answer:
            return 1
        if section is self.authority:
            return 2
        if section is self.additional:
            return 3
    ...
    def to_wire(self, origin=None, max_size=0, **kw):
        ...
        if self.keyname is not None:
            r.add_tsig(self.keyname, self.keyring[self.keyname], self.fudge, self.original_id, self.tsig_error, self.other_data, self.request_mac, self.keyalgorithm)
            self.mac = r.mac
        return r.get_wire()
...
def from_wire(wire, keyring=None, request_mac='', xfr=False, origin=None, tsig_ctx=None, multi=False, first=True, question_only=False, one_rr_per_rrset=False, ignore_trailing=False):
    ...
    m.multi = multi
    m.first = first
    reader = _WireReader(wire, m, question_only, one_rr_per_rrset, ignore_trailing)
    reader.read()
    return m
