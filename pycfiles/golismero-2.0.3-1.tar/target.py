...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/tools/sqlmap/lib/core/target.py
# Compiled at: 2013-12-09 06:41:17
"""
Copyright (c) 2006-2013 sqlmap developers (http://sqlmap.org/)
See the file 'doc/COPYING' for copying permission
"""
import codecs, functools, os, re, tempfile, time, urlparse
            ...
            if not (kb.processUserMarks and CUSTOM_INJECTION_MARK_CHAR in conf.data):
                if re.search(JSON_RECOGNITION_REGEX, conf.data):
                    ...
                    if test and test[0] in ('q', 'Q'):
                        ...
                        conf.data = re.sub('("(?P<name>[^"]+)"\\s*:\\s*"[^"]+)"', functools.partial(process, repl='\\g<1>%s"' % CUSTOM_INJECTION_MARK_CHAR), conf.data)
                        conf.data = re.sub('("(?P<name>[^"]+)"\\s*:\\s*)(-?\\d[\\d\\.]*\\b)', functools.partial(process, repl='\\g<0>%s' % CUSTOM_INJECTION_MARK_CHAR), conf.data)
                    ...
                    if test and test[0] in ('q', 'Q'):
                        ...
                        conf.data = conf.data.replace(CUSTOM_INJECTION_MARK_CHAR, ASTERISK_MARKER)
                        conf.data = re.sub('(<(?P<name>[^>]+)( [^<]*)?>)([^<]+)(</\\2)', functools.partial(process, repl='\\g<1>\\g<4>%s\\g<5>' % CUSTOM_INJECTION_MARK_CHAR), conf.data)
                    ...
                    if test and test[0] in ('q', 'Q'):
                        ...
                        conf.data = conf.data.replace(CUSTOM_INJECTION_MARK_CHAR, ASTERISK_MARKER)
                        conf.data = re.sub('(?si)((Content-Disposition[^\\n]+?name\\s*=\\s*[\\"\'](?P<name>[^\\n]+?)[\\"\']).+?)(((\\r)?\\n)+--)', functools.partial(process, repl='\\g<1>%s\\g<4>' % CUSTOM_INJECTION_MARK_CHAR), conf.data)
                        kb.postHint = POST_HINT.MULTIPART
            if not kb.postHint:
                if CUSTOM_INJECTION_MARK_CHAR in conf.data:
                    pass
                else:
                    place = PLACE.POST
                    ...
                    if paramDict:
                        ...
                        testableParameters = True
            elif CUSTOM_INJECTION_MARK_CHAR not in conf.data:
                conf.parameters[PLACE.POST] = conf.data
        kb.processUserMarks = True if kb.postHint and CUSTOM_INJECTION_MARK_CHAR in conf.data else kb.processUserMarks
        if re.search(URI_INJECTABLE_REGEX, conf.url, re.I) and not any(place in conf.parameters for place in (PLACE.GET, PLACE.POST)) and not kb.postHint and CUSTOM_INJECTION_MARK_CHAR not in (conf.data or ''):
...
def _resumeHashDBValues():
    ...
    for injection in hashDBRetrieve(HASHDB_KEYS.KB_INJECTIONS, True) or []:
        if injection.place in conf.paramDict and injection.parameter in conf.paramDict[injection.place]:
...
def _createTargetDirs():
    ...
    if not os.path.isdir(paths.SQLMAP_OUTPUT_PATH):
        try:
            os.makedirs(paths.SQLMAP_OUTPUT_PATH, 493)
        except OSError as ex:
    ...
    if not os.path.isdir(conf.outputPath):
        try:
            os.makedirs(conf.outputPath, 493)
        except OSError as ex:
            tempDir = tempfile.mkdtemp(prefix='sqlmapoutput')
