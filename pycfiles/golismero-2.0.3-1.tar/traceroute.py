...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/golismero/api/data/information/traceroute.py
...
class Traceroute(Fingerprint):
    ...
    def to_dict(self):
        ...
        d['hops'] = [ h.to_dict() if h is not None else None for h in self.__hops
                    ]
        return d
    ...
    def __str__(self):
        ...
        if self.hops:
            ...
            f = '%%%dd %%15s %%4.2f %%s' % w
            m = '%%%dd             ***     *** ***' % w
            s += ('\n').join((f % (i, h.address, h.rtt, h.hostname) if h is not None else m % i) for i, h in enumerate(self.hops))
        return s
