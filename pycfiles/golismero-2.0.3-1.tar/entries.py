...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/tools/sqlmap/plugins/generic/entries.py
                ...
                try:
                    try:
                        ...
                        for column in colList:
                            ...
                            if _ != column:
                                colString = re.sub('\\b%s\\b' % column, _, colString)
                        ...
                        if any(isTechniqueAvailable(_) for _ in (PAYLOAD.TECHNIQUE.UNION, PAYLOAD.TECHNIQUE.ERROR, PAYLOAD.TECHNIQUE.QUERY)) or conf.direct:
                            ...
                            if Backend.getIdentifiedDbms() in (DBMS.ORACLE, DBMS.DB2):
                                query = rootQuery.inband.query % (colString, (conf.db or tbl.upper)() if 1 else '%s.%s' % (conf.db.upper(), tbl.upper()))
                        ...
                        if not kb.data.dumpedTable and isInferenceAvailable() and not conf.direct:
                            ...
                            logger.info(infoMsg)
                            if Backend.getIdentifiedDbms() in (DBMS.ORACLE, DBMS.DB2):
                                query = rootQuery.blind.count % ((conf.db or tbl.upper)() if 1 else '%s.%s' % (conf.db.upper(), tbl.upper()))
                            else:
                                if Backend.getIdentifiedDbms() in (DBMS.SQLITE, DBMS.ACCESS, DBMS.FIREBIRD):
                                    query = rootQuery.blind.count % tbl
                                else:
                                    if Backend.getIdentifiedDbms() in (DBMS.SYBASE, DBMS.MSSQL):
                                                ...
                                                for column in colList:
                                                    ...
                                                    if Backend.getIdentifiedDbms() in (DBMS.MYSQL, DBMS.PGSQL):
                                                        ...
                                                        query = rootQuery.blind.query % (agent.preprocessField(tbl, column),
                                                         (conf.db or tbl.upper)() if 1 else '%s.%s' % (conf.db.upper(), tbl.upper()),
                                                         index)
                        ...
                        if len(kb.data.dumpedTable) == 0 or entriesCount == 0 and kb.permissionFlag:
                            ...
                            kb.data.dumpedTable['__infos__'] = {'count': entriesCount, 'table': safeSQLIdentificatorNaming(tbl, True), 
