...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/tools/sqlmap/lib/core/dump.py
# Compiled at: 2013-12-09 06:41:17
"""
Copyright (c) 2006-2013 sqlmap developers (http://sqlmap.org/)
See the file 'doc/COPYING' for copying permission
"""
import cgi, codecs, os, re, threading
    ...
    def dbTableValues(self, tableValues):
        ...
        if tableValues is None:
            ...
            if hasattr(conf, 'api'):
                ...
                return
            dumpDbPath = os.path.join(conf.dumpPath, re.sub('[^\\w]', '_', unsafeSQLIdentificatorNaming(db)))
            if conf.dumpFormat == DUMP_FORMAT.SQLITE:
                replication = Replication(os.path.join(conf.dumpPath, '%s.sqlite3' % unsafeSQLIdentificatorNaming(db)))
            else:
                if conf.dumpFormat in (DUMP_FORMAT.CSV, DUMP_FORMAT.HTML):
                    if not os.path.isdir(dumpDbPath):
                        os.makedirs(dumpDbPath, 493)
                ...
                for i in xrange(count):
                    ...
                    for column in columns:
                        if column != '__infos__':
                            ...
                            if len(value) > MIN_BINARY_DISK_DUMP_SIZE and '\\x' in value:
                                try:
                                    mimetype = magic.from_buffer(value, mime=True)
                                    if any(mimetype.startswith(_) for _ in ('application',
                                                                            'image')):
                                        if not os.path.isdir(dumpDbPath):
                                            os.makedirs(dumpDbPath, 493)
