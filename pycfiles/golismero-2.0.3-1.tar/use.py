...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/tools/sqlmap/lib/techniques/union/use.py
    ...
    if retVal is None:
        ...
        def _(regex):
            return reduce(lambda x, y: x if x is not None else y, (
             ...
             extractRegexResult(regex, removeReflectiveValues(listToStrValue(headers.headers if headers else None), payload, True), re.DOTALL | re.IGNORECASE)), None)
...
def unionUse(expression, unpack=True, dump=False):
    ...
    if (kb.injection.data[PAYLOAD.TECHNIQUE.UNION].where == PAYLOAD.WHERE.NEGATIVE or dump and (conf.limitStart or conf.limitStop) or 'LIMIT ' in expression.upper()) and ' FROM ' in expression.upper() and (Backend.getIdentifiedDbms() not in FROM_DUMMY_TABLE or Backend.getIdentifiedDbms() in FROM_DUMMY_TABLE and not expression.upper().endswith(FROM_DUMMY_TABLE[Backend.getIdentifiedDbms()])) and not re.search(SQL_SCALAR_REGEX, expression, re.I) or kb.forcePartialUnion:
