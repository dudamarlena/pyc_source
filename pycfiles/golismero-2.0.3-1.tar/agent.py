...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/tools/sqlmap/lib/core/agent.py
...
class Agent(object):
        ...
        if conf.direct:
            ...
            if place == PLACE.URI:
                ...
                if kb.postHint in (POST_HINT.SOAP, POST_HINT.XML):
                    ...
                    origValue = extractRegexResult('(?s)\\"\\s*:\\s*(?P<result>\\d+\\Z)', origValue) or extractRegexResult('(?s)(?P<result>[^"]+\\Z)', origValue)
                else:
                    origValue = extractRegexResult('(?s)(?P<result>[^\\s<>{}();\'\\"]+\\Z)', origValue) or ''
            ...
            if place in (PLACE.URI, PLACE.CUSTOM_POST, PLACE.CUSTOM_HEADER):
                ...
                retVal = re.sub('(\\A|\\b)%s=%s' % (re.escape(parameter), re.escape(origValue)), '%s=%s' % (parameter, self.addPayloadDelimiters(newValue.replace('\\', '\\\\'))), paramString)
            return retVal
    ...
    def limitCondition(self, expression, dump=False):
        ...
        return (
         expression, limitCond, topLimit, startLimit, stopLimit)
    ...
    def addPayloadDelimiters(self, inpStr):
        ...
        """
        if inpStr:
            return '%s%s%s' % (PAYLOAD_DELIMITER, inpStr, PAYLOAD_DELIMITER)
        return inpStr
    ...
    def removePayloadDelimiters(self, inpStr):
        ...
        """
        if inpStr:
            return inpStr.replace(PAYLOAD_DELIMITER, '')
        return inpStr
    ...
    def replacePayload(self, inpStr, payload):
        ...
        _ = re.escape(PAYLOAD_DELIMITER)
        if inpStr:
            return re.sub('(%s.*?%s)' % (_, _), ('%s%s%s' % (PAYLOAD_DELIMITER, payload, PAYLOAD_DELIMITER)).replace('\\', '\\\\'), inpStr)
        return inpStr
