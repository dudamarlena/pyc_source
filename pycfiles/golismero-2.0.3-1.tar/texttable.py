...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/thirdparty_libs/texttable.py
# Compiled at: 2013-09-16 05:36:56
...
class Texttable():
    ...
    def _draw_line(self, line, isheader=False):
        ...
        for i in range(len(line[0])):
            ...
            for cell, width, align in zip(line, self._width, self._align):
                ...
                for attr in bcolors_public_props():
                    cell_line = cell_line.replace(getattr(bcolors, attr), '').replace(bcolors.ENDC, '')
                    if cell_line.replace(bcolors.ENDC, '') != original_cell.replace(bcolors.ENDC, '') and attr != 'ENDC':
                        if not lost_color:
                            lost_color = attr

                fill = width - len(cell_line)
                try:
                    cell_line = get_color_string(getattr(bcolors, lost_color), cell_line)
    ...
    def _splitit(self, line, isheader):
        ...
        for cell, width in zip(line, self._width):
            ...
            for attr in bcolors_public_props():
                cell = cell.replace(getattr(bcolors, attr), '').replace(bcolors.ENDC, '')
                if cell.replace(bcolors.ENDC, '') != original_cell.replace(bcolors.ENDC, '') and attr != 'ENDC':
                    if not lost_color:
                        lost_color = attr
            ...
            for c in cell.split('\n'):
                ...
                try:
                    array.extend([ get_color_string(getattr(bcolors, lost_color), x) for x in textwrap.wrap(c, width)
                                 ])
                except AttributeError:
...
if __name__ == '__main__':
    ...
    table.add_rows([['text', 'float', 'exp', 'int', 'auto'],
     [
      'abcd', '67', 654, 89, 128.001],
     [
      'efghijk', 67.5434, 0.654, 89.6, 1.28e+22],
     [
      'lmn', 5e-78, 5e-78, 89.4, 1.28e-13],
     [
      'opqrstu', 0.023, 5e+78, 92.0, 12800000000000000000000]])
    print table.draw()