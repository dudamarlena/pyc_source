...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/tools/sqlmap/lib/controller/controller.py
...
def _formatInjection(inj):
    ...
    for stype, sdata in inj.data.items():
        ...
        if stype == PAYLOAD.TECHNIQUE.UNION:
            count = re.sub('(?i)(\\(.+\\))|(\\blimit[^A-Za-z]+)', '', sdata.payload).count(',') + 1
            title = re.sub('\\d+ to \\d+', str(count), title)
...
def _saveToHashDB():
    ...
    for injection in injections:
        key = (
         injection.place, injection.parameter, injection.ptype)
        if key not in _:
            _[key] = injection
        else:
            ...
            try:
                try:
                    ...
                    if PLACE.GET in conf.parameters and not any([conf.data, conf.testParameter]):
                        for parameter in re.findall('([^=]+)=([^%s]+%s?|\\Z)' % (conf.pDel or DEFAULT_GET_POST_DELIMITER, conf.pDel or DEFAULT_GET_POST_DELIMITER), conf.parameters[PLACE.GET]):
                            paramKey = (
                             conf.hostname, conf.path, PLACE.GET, parameter[0])
                        ...
                        paramKey = (
                         conf.hostname, conf.path, None, None)
                        if paramKey not in kb.testedParams:
                            testSqlInj = True
                    ...
                    if (len(kb.injections) == 0 or len(kb.injections) == 1 and kb.injections[0].place is None) and (kb.injection.place is None or kb.injection.parameter is None):
                        ...
                        orderList = (
                         PLACE.CUSTOM_POST, PLACE.CUSTOM_HEADER, PLACE.URI, PLACE.POST, PLACE.GET)
