...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/tools/sqlmap/lib/core/common.py
# Compiled at: 2013-12-09 06:41:17
"""
Copyright (c) 2006-2013 sqlmap developers (http://sqlmap.org/)
See the file 'doc/COPYING' for copying permission
"""
import codecs, contextlib, cookielib, copy, httplib, inspect, logging, ntpath, os, posixpath, random, re, socket, string, sys, tempfile, time, urllib, urlparse, unicodedata
...
class Backend:
    ...
    def setDbms(dbms):
        ...
        if dbms is None:
            ...
            if kb.dbms is not None and kb.dbms != dbms:
    ...
    def setOs(os):
        if os is None:
            ...
            if kb.os is not None and isinstance(os, basestring) and kb.os.lower() != os.lower():
                msg = 'sqlmap previously fingerprinted back-end DBMS '
    ...
    def setOsVersion(version):
        if version is None:
            ...
            if kb.osVersion is None and isinstance(version, basestring):
    ...
    def setOsServicePack(sp):
        if sp is None:
            ...
            if kb.osSP is None and isinstance(sp, int):
                kb.osSP = sp
            return
    ...
    def getErrorParsedDBMSes():
        ...
        3. Sort the fingerprint of the DBMS, lib/controller/handler.py -
           fingerprint phase.
        """
        if kb.get('heuristicTest') == HEURISTIC_TEST.POSITIVE:
            return kb.htmlFp
        return []
...
def paramToDict(place, parameters=None):
    ...
    parameters = re.sub('&(\\w{1,4});', '%s\\g<1>%s' % (PARAMETER_AMP_MARKER, PARAMETER_SEMICOLON_MARKER), parameters)
    ...
    for element in splitParams:
        element = re.sub('%s(.+?)%s' % (PARAMETER_AMP_MARKER, PARAMETER_SEMICOLON_MARKER), '&\\g<1>;', element)
        parts = element.split('=')
        if len(parts) >= 2:
            ...
            if condition:
                ...
                if not conf.multipleTargets:
                    ...
                    if _.strip(DUMMY_SQL_INJECTION_CHARS) != _ or re.search('\\A9{3,}', _) or re.search(DUMMY_USER_INJECTION, _):
...
def getDirs():
    ...
    if kb.absFilePaths:
        ...
        logger.warn(warnMsg)
    webDir = extractRegexResult('//[^/]+?/(?P<result>.*)/', conf.url)
    if webDir:
...
def setColor(message, bold=False):
    retVal = message
    level = extractRegexResult('\\[(?P<result>[A-Z ]+)\\]', message) or kb.get('stickyLevel')
...
def parseTargetUrl():
    ...
    if not conf.url:
        ...
        hostnamePort = (re.search('\\[.+\\]', urlSplit.netloc) or urlSplit.netloc.split)(':') if 1 else filter(None, (re.search('\\[.+\\]', urlSplit.netloc).group(0), re.search('\\](:(?P<port>\\d+))?', urlSplit.netloc).group('port')))
        conf.scheme = (conf.forceSSL or urlSplit.scheme.strip().lower)() if 1 else 'https'
...
def parseFilePaths(page):
    ...
    """
    if page:
        for regex in (' in <b>(?P<result>.*?)</b> on line', '(?:>|\\s)(?P<result>[A-Za-z]:[\\\\/][\\w.\\\\/]*)',
                      '(?:>|\\s)(?P<result>/\\w[/\\w.]+)'):
...
def getCharset(charsetType=None):
    ...
    if charsetType is None:
        ...
        asciiTbl.extend([87, 88])
        asciiTbl.extend(xrange(96, 103))
        asciiTbl.extend([119, 120])
...
def getFilteredPageContent(page, onlyText=True):
    ...
    if isinstance(page, unicode):
        retVal = re.sub('(?si)<script.+?</script>|<!--.+?-->|<style.+?</style>%s' % ('|<[^>]+>|\\t|\\n|\\r' if onlyText else ''), ' ', page)
        while retVal.find('  ') != -1:
...
def getSQLSnippet(dbms, sfile, **variables):
    ...
    if sfile.endswith('.sql') and os.path.exists(sfile):
        ...
        if not sfile.endswith('.sql') and os.path.exists('%s.sql' % sfile):
        ...
        retVal = re.sub('(?s);\\s+', '; ', retVal).strip('\r\n')
        for _ in variables.keys():
            retVal = re.sub('%%%s%%' % _, variables[_], retVal)

        for _ in re.findall('%RANDSTR\\d+%', retVal, re.I):
            retVal = retVal.replace(_, randomStr())

        for _ in re.findall('%RANDINT\\d+%', retVal, re.I):
            retVal = retVal.replace(_, randomInt())

    variables = re.findall("(?<!\\bLIKE ')%(\\w+)%", retVal, re.I)
    if variables:
        ...
        if choice and choice[0].lower() == 'y':
            for var in variables:
                ...
                val = readInput(msg)
                retVal = retVal.replace('%%%s%%' % var, val)
...
def getFileItems(filename, commentPrefix='#', unicode_=True, lowercase=False, unique=False):
    ...
    """
    if not unique:
        retVal = list() if 1 else OrderedDict()
        checkFile(filename)
...
def goGoodSamaritan(prevValue, originalCharset):
    ...
    if kb.partRun in kb.commonOutputs:
        ...
        commonCharset.sort()
        return (
         commonValue, commonPattern, commonCharset, originalCharset)
    else:
        return (
         None, None, None, originalCharset)
        return
...
def getPartRun(alias=True):
    ...
    try:
        ...
        for i in xrange(0, len(stack) - 1):
            for regex in ('self\\.(get[^(]+)\\(\\)', 'conf\\.dbmsHandler\\.([^(]+)\\(\\)'):
    ...
    if alias:
        if isinstance(commonPartsDict.get(retVal), tuple):
            return commonPartsDict[retVal][1]
        return retVal
    else:
        return retVal
        return
...
def adjustTimeDelay(lastQueryDuration, lowerStdLimit):
    ...
    if candidate:
        kb.delayCandidates = [
         candidate] + kb.delayCandidates[:-1]
...
def getLastRequestHTTPError():
    ...
    """
    threadData = getCurrentThreadData()
    if threadData.lastHTTPError:
        return threadData.lastHTTPError[1]
    else:
        return
...
def findMultipartPostBoundary(post):
    ...
    candidates = []
    for match in re.finditer('(?m)^--(.+?)(--)?$', post or ''):
        _ = match.group(1)
        if _ in done:
            continue
        else:
...
def extractTextTagContent(page):
    ...
    [u'Title', u'foobar']
    """
    page = re.sub('(?si)[^\\s>]*%s[^<]*' % REFLECTED_VALUE_MARKER, '', page or '')
...
def parseSqliteTableSchema(value):
    ...
    if value:
        table = {}
        columns = {}
        for match in re.finditer('(\\w+)\\s+(TEXT|NUMERIC|INTEGER|REAL|NONE)\\b', value, re.I):
...
def maskSensitiveData(msg):
    ...
    """
    retVal = msg
    for item in filter(None, map(lambda x: conf.get(x), ('hostname', 'googleDork',
                                                         'authCred', 'proxyCred',
                                                         'tbl', 'db', 'col', 'user',
                                                         'cookie', 'proxy'))):
        regex = SENSITIVE_DATA_REGEX % re.sub('(\\W)', '\\\\\\1', item)
...
def removeReflectiveValues(content, payload, suppressWarning=False):
    ...
    if all([content, payload]) and isinstance(content, unicode) and kb.reflectiveMechanism:
        ...
        regex = _(filterStringValue(payload, '[A-Za-z0-9]', REFLECTED_REPLACEMENT_REGEX.encode('string-escape')))
        if regex != payload:
            if all(part.lower() in content.lower() for part in filter(None, regex.split(REFLECTED_REPLACEMENT_REGEX))[1:]):
                ...
                if regex.startswith(REFLECTED_REPLACEMENT_REGEX):
                    regex = '%s%s' % (REFLECTED_BORDER_REGEX, regex[len(REFLECTED_REPLACEMENT_REGEX):])
                else:
                    regex = '\\b%s' % regex
                if regex.endswith(REFLECTED_REPLACEMENT_REGEX):
                    regex = '%s%s' % (regex[:-len(REFLECTED_REPLACEMENT_REGEX)], REFLECTED_BORDER_REGEX)
                else:
                    regex = '%s\\b' % regex
                retVal = re.sub('(?i)%s' % regex, REFLECTED_VALUE_MARKER, retVal)
                if len(parts) > 2:
                    regex = REFLECTED_REPLACEMENT_REGEX.join(parts[1:])
                    retVal = re.sub('(?i)\\b%s\\b' % regex, REFLECTED_VALUE_MARKER, retVal)
            if retVal != content and not kb.heuristicMode:
                kb.reflectiveCounters[REFLECTIVE_COUNTER.HIT] += 1
                if not suppressWarning:
                    warnMsg = 'reflective value(s) found and filtering out'
                    singleTimeWarnMessage(warnMsg)
                if re.search('FRAME[^>]+src=[^>]*%s' % REFLECTED_VALUE_MARKER, retVal, re.I):
...
def safeSQLIdentificatorNaming(name, isTable=False):
    ...
    if isinstance(name, basestring):
        ...
        if _:
            retVal = re.sub('(?i)\\A%s\\.' % DEFAULT_MSSQL_SCHEMA, '', retVal)
        if retVal.upper() in kb.keywords or (retVal or ' ')[0].isdigit() or not re.match('\\A[A-Za-z0-9_@%s\\$]+\\Z' % ('.' if _ else ''), retVal):
            if Backend.getIdentifiedDbms() in (DBMS.MYSQL, DBMS.ACCESS):
                ...
                retVal = '[%s]' % retVal.strip('[]')
        if _ and DEFAULT_MSSQL_SCHEMA not in retVal and '.' not in re.sub('\\[[^]]+\\]', '', retVal):
...
def isAdminFromPrivileges(privileges):
    ...
    retVal |= Backend.isDbms(DBMS.FIREBIRD) and all(_ in privileges for _ in ('SELECT',
                                                                              'INSERT',
                                                                              'UPDATE',
                                                                              'DELETE',
                                                                              'REFERENCES',
...
def resetCookieJar(cookieJar):
    ...
    if not conf.loadCookies:
        ...
        try:
            ...
            errMsg += "cookies file ('%s')" % re.sub("(cookies) file '[^']+'", '\\g<1>', str(msg))
            raise SqlmapGenericException(errMsg)
...
def prioritySortColumns(columns):
    ...
    return sorted(sorted(columns, key=len), --- This code section failed: ---

 L.3626         0  LOAD_DEREF            0  '_'
                3  LOAD_FAST             0  'x'
               ...
               21  UNARY_NOT        
             22_0  COME_FROM             9  '9'
               22  POP_JUMP_IF_FALSE    29  'to 29'
               25  LOAD_CONST               -1
               28  RETURN_END_IF_LAMBDA
             29_0  COME_FROM            22  '22'
               29  LOAD_DEREF            0  '_'
               32  LOAD_FAST             0  'x'
               35  CALL_FUNCTION_1       1  None
               38  UNARY_NOT        
               39  POP_JUMP_IF_FALSE    58  'to 58'
               42  LOAD_DEREF            0  '_'
               45  LOAD_FAST             1  'y'
             ...
             51_0  COME_FROM            39  '39'
               ...
               54  LOAD_CONST               1
               57  RETURN_END_IF_LAMBDA
             58_0  COME_FROM            51  '51'
               58  LOAD_CONST               0
               61  RETURN_VALUE_LAMBDA
               -1  LAMBDA_MARKER    
