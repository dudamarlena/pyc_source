...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/thirdparty_libs/requests/packages/urllib3/connectionpool.py
...
from .exceptions import ClosedPoolError, EmptyPoolError, HostChangedError, MaxRetryError, SSLError, TimeoutError
    ...
    def urlopen(self, method, url, body=None, headers=None, retries=3, redirect=True, assert_same_host=True, timeout=_Default, pool_timeout=None, release_conn=None, **response_kw):
        ...
        try:
            try:
                ...
                raise TimeoutError(self, url, 'Request timed out. (pool_timeout=%s)' % pool_timeout)
            except SocketTimeout as e:
                raise TimeoutError(self, url, 'Request timed out. (timeout=%s)' % timeout)
        ...
        if not conn:
            log.warn("Retrying (%d attempts remain) after connection broken by '%r': %s" % (
             retries, err, url))
...
class HTTPSConnectionPool(HTTPConnectionPool):
    ...
    def __init__(self, host, port=None, strict=False, timeout=None, maxsize=1, block=False, headers=None, key_file=None, cert_file=None, cert_reqs=None, ca_certs=None, ssl_version=None, assert_hostname=None, assert_fingerprint=None):
    ...
    def _new_conn(self):
        ...
        if not ssl:
            ...
            return HTTPSConnection(host=self.host, port=self.port, strict=self.strict)
        connection = VerifiedHTTPSConnection(host=self.host, port=self.port, strict=self.strict)
        connection.set_cert(key_file=self.key_file, cert_file=self.cert_file, cert_reqs=self.cert_reqs, ca_certs=self.ca_certs, assert_hostname=self.assert_hostname, assert_fingerprint=self.assert_fingerprint)
