...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/tools/sqlmap/lib/controller/checks.py
# Compiled at: 2013-12-09 06:41:17
"""
Copyright (c) 2006-2013 sqlmap developers (http://sqlmap.org/)
See the file 'doc/COPYING' for copying permission
"""
import copy, httplib, re, socket, time
            ...
            try:
                ...
                if stype == PAYLOAD.TECHNIQUE.UNION:
                    ...
                    if test.request.columns == '[COLSTART]-[COLSTOP]':
                        ...
                        continue
                    match = re.search('(\\d+)-(\\d+)', test.request.columns)
                    if injection.data and match:
                        lower, upper = int(match.group(1)), int(match.group(2))
                        for _ in (lower, upper):
                            if _ > 1:
                                unionExtended = True
                                test.request.columns = re.sub('\\b%d\\b' % _, str(2 * _), test.request.columns)
                                title = re.sub('\\b%d\\b' % _, str(2 * _), title)
                                test.title = re.sub('\\b%d\\b' % _, str(2 * _), test.title)
...
def checkFalsePositives(injection):
    ...
    """
    retVal = injection
    if len(injection.data) == 1 and any(map(lambda x: x in injection.data, [PAYLOAD.TECHNIQUE.BOOLEAN, PAYLOAD.TECHNIQUE.TIME, PAYLOAD.TECHNIQUE.STACKED])) or len(injection.data) == 2 and all(map(lambda x: x in injection.data, [PAYLOAD.TECHNIQUE.TIME, PAYLOAD.TECHNIQUE.STACKED])):
        pushValue(kb.injection)
...
def heuristicCheckSqlInjection(place, parameter):
    if kb.nullConnection:
        ...
        kb.heuristicTest = HEURISTIC_TEST.CASTED if casting else (result or HEURISTIC_TEST).NEGATIVE if 1 else HEURISTIC_TEST.POSITIVE
        if casting:
...
def checkWaf():
    ...
    if not conf.checkWaf:
        ...
        if not conf.parameters.get(PLACE.GET):
            conf.parameters[PLACE.GET] = '' if 1 else conf.parameters[PLACE.GET] + '&'
            ...
            if not page and HTTP_HEADER.CONTENT_LENGTH in (headers or {}):
                ...
                if page and len(page) == 1 and HTTP_HEADER.CONTENT_RANGE in (headers or {}):
                    ...
                    if HTTP_HEADER.CONTENT_LENGTH in (headers or {}):
                        ...
                        logger.info(infoMsg)
        except SqlmapConnectionException as errMsg:
