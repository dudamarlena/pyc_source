...
# [GCC 8.4.0]
# Embedded file name: /Users/Dani/Documents/Projects/Golismero_2.0/src_github/thirdparty_libs/dns/query.py
# Compiled at: 2013-08-26 10:52:44
"""Talk to a DNS server."""
from __future__ import generators
import errno, select, socket, struct, sys, time, dns.exception, dns.inet, dns.name, dns.message, dns.rdataclass, dns.rdatatype
...
def udp(q, where, timeout=None, port=53, af=None, source=None, source_port=0, ignore_unexpected=False, one_rr_per_rrset=False):
    ...
    try:
        ...
        while 1:
            ...
            if _addresses_equal(af, from_address, destination) or dns.inet.is_multicast(where) and from_address[1:] == destination[1:]:
                break
...
def xfr(where, zone, rdtype=dns.rdatatype.AXFR, rdclass=dns.rdataclass.IN, timeout=None, port=53, keyring=None, keyname=None, relativize=True, af=None, lifetime=None, source=None, source_port=0, serial=0, use_udp=False, keyalgorithm=dns.tsig.default_algorithm):
    ...
    while not done:
        ...
        r = dns.message.from_wire(wire, keyring=q.keyring, request_mac=q.mac, xfr=True, origin=origin, tsig_ctx=tsig_ctx, multi=True, first=first, one_rr_per_rrset=rdtype == dns.rdatatype.IXFR)
        tsig_ctx = r.tsig_ctx
        first = False
        answer_index = 0
        delete_mode = False
