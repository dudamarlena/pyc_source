...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/test/registry/test_fit_function_generators.py
# Compiled at: 2020-04-04 11:50:41
# Size of source mod 2**32: 5620 bytes
from unittest import TestCase
import numpy as np
from eddington_core import InvalidGeneratorInitialization, fit_function, fit_function_generator, polynom, straight_power, inverse_power, FitFunctionsRegistry
from test.registry.fitting_test_case import linear_case, add_test_case, parabolic_case, polynom_3_case, hyperbolic_case, inverse_power_2_case, straight_power_2_case, straight_power_3_case
...
class FitFunctionGeneratorsBaseTestCase:
    ...
    def assert_raises_unfit_parameters(self, n0, func):
        self.assertRaisesRegex(InvalidGeneratorInitialization, f"^input length should be {self.n}, got {n0}$", func, np.random.random(n0), np.random.random())
    ...
    def test_is_generator(self):
        self.assertTrue((self.generator.is_generator()),
...
class TestPolynomFitFunctionGenerator(TestCase, FitFunctionGeneratorsBaseTestCase):
    ...
    def test_polynom_with_zero_degree_raises_error(self):
        self.assertRaisesRegex(InvalidGeneratorInitialization, '^n must be positive, got 0$', polynom, 0)

    def test_polynom_with_negative_degree_raises_error(self):
        self.assertRaisesRegex(InvalidGeneratorInitialization, '^n must be positive, got -1$', polynom, -1)


class TestStraightPowerFitFunctionGenerator(TestCase, FitFunctionGeneratorsBaseTestCase):
    ...
    def test_straight_power_with_zero_degree_raises_error(self):
        self.assertRaisesRegex(InvalidGeneratorInitialization, '^n must be positive, got 0$', straight_power, 0)

    def test_straight_power_with_negative_degree_raises_error(self):
        self.assertRaisesRegex(InvalidGeneratorInitialization, '^n must be positive, got -1$', straight_power, -1)

    def test_straight_power_with_one_degree_raises_error(self):
        self.assertRaisesRegex(InvalidGeneratorInitialization, '^n cannot be 1. use "linear" fit instead.$', straight_power, 1)
...
class TestInversePowerFitFunctionGenerator(TestCase, FitFunctionGeneratorsBaseTestCase):
    ...
    def test_inverse_power_with_zero_degree_raises_error(self):
        self.assertRaisesRegex(InvalidGeneratorInitialization, '^n must be positive, got 0$', inverse_power, 0)

    def test_inverse_power_with_negative_degree_raises_error(self):
        self.assertRaisesRegex(InvalidGeneratorInitialization, '^n must be positive, got -1$', inverse_power, -1)
