...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/test/registry/test_fit_function_registry.py
# Compiled at: 2020-04-04 11:50:41
# Size of source mod 2**32: 5057 bytes
from unittest import TestCase
from eddington_core import FitFunctionLoadError, fit_function, FitFunctionsRegistry
...
class TestFitFunctionRegistryAddAndRemove(TestCase):
    ...
    def test_syntax_of_one_function(self):
        ...
        self.assertEqual((expected_string[1:]),
          (str(syntax)),
    ...
    def test_add_dummy_function_without_saving(self):
        self.assertFalse((FitFunctionsRegistry.exists(self.func4.name)),
          msg="Function was saved, even though it wasn't supposed to")

    def test_get_non_existing_function(self):
        self.assertRaisesRegex(FitFunctionLoadError, f"^No fit function or generator named {self.func4.name}$", FitFunctionsRegistry.get, self.func4.name)

    def test_load_non_existing_function(self):
        self.assertRaisesRegex(FitFunctionLoadError, f"^No fit function or generator named {self.func4.name}$", FitFunctionsRegistry.load, self.func4.name)

    def test_load_fit_function_with_parameters(self):
        self.assertRaisesRegex(FitFunctionLoadError, f"^{self.func1.name} is not a generator and should not get parameters$", FitFunctionsRegistry.load, self.func1.name, 1, 2)