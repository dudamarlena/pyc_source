...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/lazydiff/vars.py
# Compiled at: 2018-12-11 21:11:30
# Size of source mod 2**32: 10714 bytes
import numpy as np, collections, numbers
np.seterr(all='raise')

class Var:
    __doc__ = '\n    A class for lazydiff autograd scalar variables.\n    '

    def __init__(self, val, seed=np.array(1.0)):
        """
    ...
    def _check_numeric(self, other):
        ...
        if not isinstance(other, numbers.Number):
            if not (isinstance(other, np.ndarray) and np.issubdtype(other.dtype, np.number)):
    ...
    def __neg__(self):
        ...
        """
        result = Var(-self.val)
        result.parents[self] = self.children[result] = -1.0
        return result
    ...
    def __add__(self, other):
        ...
        if isinstance(other, Var):
            ...
            result.parents[self] = self.children[result] = 1.0
            result.parents[other] = other.children[result] = 1.0
            return result
        else:
