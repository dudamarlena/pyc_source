...
# [GCC 8.4.0]
# Embedded file name: /users/garrigaf/Documents/git/darfix/build/lib/darfix/core/componentsMatching.py
# Compiled at: 2020-03-03 09:45:08
# Size of source mod 2**32: 11110 bytes
__authors__ = [
 'J. Garriga']
__license__ = 'MIT'
__date__ = '28/02/2020'
...
class ComponentsMatching:
    ...
    def euclidean_distance(self, X, Y):
        ...
        """
        assert X.shape == Y.shape, 'Images have to have same shape to compute euclidean distance'
        dst = numpy.linalg.norm(X - Y)
        return dst

    def match_components(self, id1=None, id2=None, method=Method.orb_feature_matching, tol=8):
        """
                    ...
                    if component1.descriptor is not None:
                        for j, component2 in enumerate(self.descriptors[id2]):
                            if component2.descriptor is not None:
                                good[(i, j)] = numpy.array(bf.match(component1.descriptor, component2.descriptor))

                best_v = []
                for x, y in sorted(good, key=(lambda match: len(good[match])),
                  reverse=True):
                    if x not in final_matches.keys():
                        if y not in final_matches.values():
                            kp1 = []
                            kp2 = []
    ...
    def draw_matches(self, final_matches, matches, id1=None, id2=None, displayMatches=False):
        """
        ...
        for i, img1 in enumerate(self.components[id1]):
            if i in final_matches:
                ...
                if displayMatches:
                    assert all(((isinstance(match, cv2.DMatch) for match in values) for values in matches.values())), 'Dictionary `matches` has to contain values of type `cv2.DMatch`'
                    img = cv2.drawMatches((self.descriptors[id1][i].image), (self.descriptors[id1][i].keypoints),
