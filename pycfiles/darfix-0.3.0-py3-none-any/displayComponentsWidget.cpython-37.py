...
# [GCC 8.4.0]
# Embedded file name: /users/garrigaf/Documents/git/darfix/build/lib/darfix/gui/displayComponentsWidget.py
# Compiled at: 2020-03-03 09:56:11
...
class DisplayComponentsWidget(qt.QMainWindow):
    ...
    def setComponents(self, components, W, dimensions):
        ...
        for i in range(len(self.W.T)):
            self._plot_rocking_curves.getPlot().addCurve((numpy.arange(len(self.W.T[i]))), (self.W.T[i]),
              legend=(str(i)))
        ...
        if self.dimensions:
            ...
            self._scatter_rocking_curves.setData(self.dimensions[keys[0]].astype(numpy.float), self.dimensions[keys[1]].astype(numpy.float), self.W.T[0])
    ...
    def _saveComp(self):
        ...
        if fileDialog.exec_():
            write_components(fileDialog.selectedFiles()[0], 'entry', self.dimensions, self.W, self.components, 1)
