...
# [GCC 8.4.0]
# Embedded file name: /users/garrigaf/Documents/git/darfix/build/lib/darfix/core/geneticShiftDetection.py
...
class GeneticShiftDetection:
    ...
    def __init__(self, data, optimal_shift):
        assert optimal_shift.shape[1] == len(data), 'Optimal shift        columns must be of same length as data'
        self.data = data
    ...
    def mutate(self, children):
        ...
        for i in range(len(children)):
            ...
            normal = numpy.random.multivariate_normal(self.mean, self.sigma * numpy.eye(2), child.shape[1]).T
    ...
    def fit(self, mean, sigma, n_gens, size):
        ...
        for i in range(n_gens):
            ...
            print(('\rBest score: {:10.4f}, Avg score: {:10.4f}      {:3}%'.format(self.scores_best[(-1)], self.scores_avg[(-1)], int(100 * (i + 1) / n_gens))),
              end='\r')
