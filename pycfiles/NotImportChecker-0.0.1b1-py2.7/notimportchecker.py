...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/notimportchecker.py
# Compiled at: 2018-04-14 19:12:20
import ast, os, sys
...
class SearchImport(ast.NodeVisitor):
    ...
    def visit_ImportFrom(self, stmt):
        ...
        self._imports.setdefault(module_name, {'mod_name': names_dict, 'lineno': stmt.lineno})
        for child in ast.iter_child_nodes(stmt):
            self.generic_visit(child)

    def visit_Import(self, stmt):
        for al in stmt.names:
            self._imports.setdefault(al.name, {'mod_name': {al.name: al.name}, 'lineno': stmt.lineno})
        ...
        for key, value in stmt.items():
            for mod_name in value['mod_name']:
                try:
                    ...
                    self._import_error_list.setdefault(key, {'mod_name': mod_name, 'lineno': value['lineno']})
...
if __name__ == '__main__':
    ...
    for f in files:
        if os.path.dirname(f) == '':
            c = Checker(os.path.join(os.getcwd(), f))
