...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nowandnext/parkup.py
# Compiled at: 2009-05-11 19:02:39
import logging, datetime, optparse
from ftplib import FTP
import ftplib, os, traceback, pkg_resources
...
class archiver():
    ...
    def __init__(self, config, warpfactor):
        ...
        scsettings = (
         self._config.get('simplecast', 'host', 'localhost'),
    ...
    def mkParser(cls):
        ...
        parser.add_option('--configfile', dest='configfilepath', help='The location of the configuration file, default is: %s' % defaultconfigfilelocation, default=defaultconfigfilelocation, type='str')
        parser.add_option('--quiet', '-q', dest='verbose', help='Make the logging less verbose', default=True, action='store_false')
        parser.add_option('--warpfactor', '-w', dest='warpfactor', help='Make time appear to go faster - handy for debugging the script but never for production use.', default=1.0, type='float')
        parser.add_option('--debugmode', '-d', dest='debugmode', help='Crash out on critical failure.', default=False, action='store_true')
        return parser

    MetaDataSettings = {'identifier': 'Wavelength2008-05-27B', 'collection': 'Wavelength Show On Resonance 104.4 FM www.resonancefm.com', 
       'title': 'Wavelength', 
       'subject': ' ', 
       'mediatype': 'etree', 
       'year': '2008', 
