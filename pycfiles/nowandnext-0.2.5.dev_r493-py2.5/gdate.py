...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nowandnext/calendar/gdate.py
...
class gdatetime(datetime.datetime):
    ...
    """
    ISO8601_REGEX = re.compile('(?P<year>[0-9]{4})(-(?P<month>[0-9]{1,2})(-(?P<day>[0-9]{1,2})((?P<separator>.)(?P<hour>[0-9]{2}):(?P<minute>[0-9]{2})(:(?P<second>[0-9]{2})(\\.(?P<fraction>[0-9]+))?)?(?P<timezone>Z|(([-+])([0-9]{2}):([0-9]{2})))?)?)?)?')
    ...
    def fromisoformat(cls, dtisoformat, tz=None):
        ...
        return cls(year=int(timegroups['year']), month=int(timegroups['month']), day=int(timegroups['day']), hour=int(timegroups['hour']), minute=int(timegroups['minute']), second=int(timegroups['second']), tzinfo=tz)
