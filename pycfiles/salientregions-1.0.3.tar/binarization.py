...
# [GCC 8.4.0]
# Embedded file name: /home/dafne/SalientRegions/SalientDetector-python/salientregions/binarization.py
...
class ThresholdBinarizer(Binarizer):
    ...
    def binarize(self, img, visualize=True):
        ...
        if visualize:
            helpers.show_image(binarized, title='Binarized with threshold %i' % self.threshold)
        return binarized
...
class OtsuBinarizer(Binarizer):
    ...
    def binarize(self, img, visualize=True):
        ...
        if len(binarized.shape) > 2:
            binarized = binarized[:, :, 0]
        if visualize:
            helpers.show_image(binarized, title='Binarized with threshold %i' % threshold)
        return binarized
...
class DatadrivenBinarizer(Binarizer):
    ...
    def __init__(self, lam, area_factor_large=0.001, area_factor_verylarge=0.1, weights=(0.33, 0.33, 0.33), offset=80, stepsize=1, connectivity=4):
    ...
    def binarize_withthreshold(self, img, visualize=True, output_scores=False):
        ...
        for t in range(max(t_otsu - self.offset, 0), min(t_otsu + self.offset, 255), self.stepsize):
            _, bint = cv2.threshold(img, t, 255, cv2.THRESH_BINARY)
            nccs, labels, stats, centroids = cv2.connectedComponentsWithStats(bint, connectivity=self.connectivity)
        ...
        scores = self.weights[0] * a_nccs_norm + self.weights[1] * a_nccs_large_norm + self.weights[2] * a_nccs_verylarge_norm
        t_opt = scores.argmax()
        _, binarized = cv2.threshold(img, t_opt, 255, cv2.THRESH_BINARY)
        if visualize:
            fig = plt.figure()
            ...
            plt.show()
            helpers.show_image(binarized, title='Binarized with threshold %i' % t_opt)
        if output_scores:
            all_levels = {'level': np.arange(256), 'a_nccs': a_nccs, 'a_nccs_large': a_nccs_large, 'a_nccs_verylarge': a_nccs_verylarge, 
               'a_nccs_norm': a_nccs_norm, 
               'a_nccs_large_norm': a_nccs_large_norm, 
               'a_nccs_verylarge_norm': a_nccs_verylarge_norm, 
               'scores': scores}
            return (
             t_opt, binarized, all_levels)
        else:
            return (
             t_opt, binarized)
