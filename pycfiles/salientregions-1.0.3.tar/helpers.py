...
# [GCC 8.4.0]
# Embedded file name: /home/dafne/SalientRegions/SalientDetector-python/salientregions/helpers.py
...
import cv2, numpy as np, scipy.io as sio, matplotlib.pyplot as plt, math, six
...
colormap = {'holes': [255, 0, 0], 'islands': [
             0, 255, 255], 
   'indentations': [
                  0, 255, 0], 
   'protrusions': [
...
def visualize_elements(img, regions=None, holes=None, islands=None, indentations=None, protrusions=None, visualize=True, title='salient regions'):
...
def visualize_elements_ellipses(img, features, visualize=True, title='salient regions'):
    """Display the image with the salient regions provided.
    ...
    for region_type in features.keys():
        img_to_show = visualize_ellipses(img_to_show, features[region_type], colormap[region_type], visualize=False)
...
def poly2standard_ellipse(A, B, C):
    ...
    return (
     half_major_axis, half_minor_axis, theta)
...
def binary_mask2ellipse_features_single(binary_mask, connectivity=4, saliency_type=1, min_square=False):
    ...
    _, contours, hierarchy = cv2.findContours(binary_mask2, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
    ...
    return (
     num_regions, features_standard, features_poly)
...
def binary_mask2ellipse_features(regions, connectivity=4, min_square=False):
    ...
    for saltype in regions.keys():
        num_regions_s, features_standard_s, features_poly_s = binary_mask2ellipse_features_single(regions[saltype], connectivity=connectivity, saliency_type=region2int[saltype], min_square=min_square)
...
def load_ellipse_features_from_file(filename):
   ...
   """
    ...
    return (
     total_num_regions, num_regions, features)