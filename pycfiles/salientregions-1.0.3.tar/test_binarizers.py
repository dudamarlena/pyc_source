...
# [GCC 8.4.0]
# Embedded file name: /home/dafne/SalientRegions/SalientDetector-python/tests/test_binarizers.py
...
class DataDrivenBinarizerTester(unittest.TestCase):
    ...
    def setUp(self):
        ...
        testdata_features_path = os.path.normpath(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'features/Gray/'))
        self.image = cv2.imread(os.path.join(testdata_path, 'Gray_scale.png'), cv2.IMREAD_GRAYSCALE)
        self.binarized_true = cv2.imread(os.path.join(testdata_features_path, 'Binarization_data_driven.png'), cv2.IMREAD_GRAYSCALE)
        self.binarizer = sr.DatadrivenBinarizer(lam=24, area_factor_large=0.001, area_factor_verylarge=0.01, weights=(0.33,
                                                                                                                      0.33,
                                                                                                                      0.33), offset=80, stepsize=1, connectivity=8)
...
class ThresholdBinarizerTester(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.binarized_true_175 = cv2.imread(os.path.join(testdata_path, 'Binarized_thresh175.png'), cv2.IMREAD_GRAYSCALE)
        self.threshold175 = sr.ThresholdBinarizer(175)
        self.binarized_true_57 = cv2.imread(os.path.join(testdata_path, 'Binarized_thresh57.png'), cv2.IMREAD_GRAYSCALE)
        self.threshold57 = sr.ThresholdBinarizer(57)
        self.binarized_true_0 = cv2.imread(os.path.join(testdata_path, 'Binarized_thresh0.png'), cv2.IMREAD_GRAYSCALE)
        self.threshold0 = sr.ThresholdBinarizer(0)
        self.binarized_true_255 = cv2.imread(os.path.join(testdata_path, 'Binarized_thresh255.png'), cv2.IMREAD_GRAYSCALE)
