...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\sping\PDF\pdfgen.py
...
import pdfutils, pdfdoc, pdfmetrics, pdfgeom
FILL_EVEN_ODD = 0
FILL_NON_ZERO = 1
PATH_OPS = {(0, 0, FILL_EVEN_ODD): 'n', (0, 0, FILL_NON_ZERO): 'n', 
   (1, 0, FILL_EVEN_ODD): 'S', 
   (1, 0, FILL_NON_ZERO): 'S', 
   (0, 1, FILL_EVEN_ODD): 'f*', 
   (0, 1, FILL_NON_ZERO): 'f', 
...
class Canvas():
    ...
    def rect(self, x, y, width, height, stroke=1, fill=0):
        """draws a rectangle"""
        self._code.append('n %0.4f %0.4f %0.4f %0.4f re ' % (x, y, width, height) + PATH_OPS[(stroke, fill, self._fillMode)])
    ...
    def roundRect(self, x, y, width, height, radius, stroke=1, fill=0):
        ...
        self._code.append('%0.4f %0.4f l' % (x3, y0))
        self._code.append('%0.4f %0.4f %0.4f %0.4f %0.4f %0.4f c' % (
         x4, y0, x5, y1, x5, y2))
        self._code.append('%0.4f %0.4f l' % (x5, y3))
        self._code.append('%0.4f %0.4f %0.4f %0.4f %0.4f %0.4f c' % (
         x5, y4, x4, y5, x3, y5))
        self._code.append('%0.4f %0.4f l' % (x2, y5))
        self._code.append('%0.4f %0.4f %0.4f %0.4f %0.4f %0.4f c' % (
         x1, y5, x0, y4, x0, y3))
        self._code.append('%0.4f %0.4f l' % (x0, y2))
        self._code.append('%0.4f %0.4f %0.4f %0.4f %0.4f %0.4f c' % (
         x0, y1, x1, y0, x2, y0))
        self._code.append('h')
        self._code.append(PATH_OPS[(stroke, fill, self._fillMode)])
...
"""
        ...
        PageTransitionEffects = {'Split': [
                   direction_arg, motion_arg], 
           'Blinds': [
                    dimension_arg], 
           'Box': [
                 motion_arg], 
        ...
        self._pageTransitionString = '/Trans <</D %d /S /%s ' % (duration, effectname) + string.join(args, ' ') + ' >>'
...
class PDFTextObject():
    ...
    def setTextOrigin(self, x, y):
        if self._canvas.bottomup:
            self._code.append('1 0 0 1 %0.4f %0.4f Tm' % (x, y))
        else:
            self._code.append('1 0 0 -1 %0.4f %0.4f Tm' % (x, y))
        self._x = x
        self._y = y
    ...
    def setTextTransform(self, a, b, c, d, e, f):
        ...
        self._code.append('%0.4f %0.4f %0.4f %0.4f %0.4f %0.4f Tm' % (a, b, -c, -d, e, f))
        self._x = e
        self._y = f
