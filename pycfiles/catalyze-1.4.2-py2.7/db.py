...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/catalyze/commands/db.py
...
import os, os.path, sys, requests
    ...
    try:
        ...
        with open(filepath, 'rb') as (file):
            with open(enc_filepath, 'wb') as (tf):
                ...
                contents = file.read()
                contents += '\x00' * (AES.block_size - len(contents) % AES.block_size)
        ...
        with open(enc_filepath, 'rb') as (file):
            ...
            resp = services.initiate_import(session, settings['environmentId'], service_id, upload_url, file, base64.b64encode(binascii.hexlify(key)), base64.b64encode(binascii.hexlify(iv)), wipe_first, options)
            task_id = resp['id']
