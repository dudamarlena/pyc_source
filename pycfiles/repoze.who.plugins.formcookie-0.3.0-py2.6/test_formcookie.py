...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/repoze/who/plugins/test_formcookie.py
...
class FixtureBase(Base):
    ...
    def setUp(self):
        ...
        self.app = TestApp(app, extra_environ={'REMOTE_ADDR': '127.0.0.1', 
           ...
           'repoze.who.plugins': {'cookie': DummyCookieIdentifier({'login': 'chris', 'password': 'password'})}})
...
class TestCookieRedirectingFormPlugin(FixtureBase):
    ...
    def _make_one(self, login_form_path='/loginform', login_handler_path='/login', logout_handler_path='/logout', rememberer_name='cookie', **kw):
        plugin = self._getTargetClass()(login_form_path, login_handler_path, logout_handler_path, rememberer_name, **kw)
        return plugin

    def test_make_redirecting_plugin(self):
        from repoze.who.plugins.formcookie import make_redirecting_plugin
        plugin = make_redirecting_plugin(login_form_path='/form', login_handler_path='/login', logout_handler_path='/logout', rememberer_name='cookie', login_redirect_method='cookie', logout_redirect_method='cookie', username_field='usern', password_field='passw', force_https='y')
    ...
    def test_login_nopass(self):
        ...
        result = self.plugin.identify(environ)
        self.assertEqual(result, None)
        self.assertEqual(environ.get('repoze.who.application').location(), self.plugin.login_form_path)
        return

    def test_login_referer(self):
        self.app.extra_environ['HTTP_REFERER'] = 'http://example.com'
        environ = self.app.post('/login', params={'login': 'chris', 'password': 'password'}).req.environ
        result = self.plugin.identify(environ)
    ...
    def test_login_cookie(self):
        environ = self.app.post('/login', headers=[
         ('Cookie', 'came_from=http://example.com')], params={'login': 'chris', 'password': 'password'}).req.environ
        result = self.plugin.identify(environ)
        self.assertEqual(result, {'login': 'chris', 'password': 'password'})
        app = environ['repoze.who.application']
        self.assertEqual(len(app.headers), 1)
    ...
    def test_login_custom_fields_names(self):
        environ = self.app.post('/login', params={'usern': 'christ', 'passw': 'jesus'}).req.environ
        self.plugin = self._make_one(password_field='passw', username_field='usern')
        result = self.plugin.identify(environ)
    ...
    def test_forget(self):
        ...
        self.assertEqual(result, None)
        self.assertEqual(environ['repoze.who.plugins']['cookie'].forgotten, identity)
        return
    ...
    def test_force_https_sucess(self):
        self.plugin = self._make_one(force_https=True)
        environ = self.app.post('/login', params={'login': 'chris', 'password': 'password'}, extra_environ={'wsgi.url_scheme': 'https'}).req.environ
        result = self.plugin.identify(environ)
    ...
    def test_custom_logout(self):
        ...
        environ = self.app.post('/logout', params={'login': 'chris', 'password': 'password'}).req.environ
        result = self.plugin.identify(environ)
        self.failUnless('performing logout' in environ['repoze.who.logger'].messages)
        self.assertEqual(environ['repoze.who.application'].code, 401)
