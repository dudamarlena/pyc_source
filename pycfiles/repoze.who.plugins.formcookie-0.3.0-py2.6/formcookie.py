...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/repoze/who/plugins/formcookie.py
# Compiled at: 2009-12-21 08:27:27
...
class CookieRedirectingFormPlugin(object):
    ...
    def __init__(self, login_form_path, login_handler_path, logout_handler_path, rememberer_name, default_redirect_path='/', logout_redirect_method='cookie', login_redirect_method='cookie', username_field='login', password_field='password', force_https=False):
        ...
        self.login_redirect_method = login_redirect_method
        self.logout_redirect_method = logout_redirect_method
    ...
    def _extract_came_from(self, environ, use_current=False):
        ...
        try:
            ...
            if not use_current:
                default = resolve_relative_url(self.default_redirect_path, environ)
    ...
    def identify(self, environ):
        ...
        on login:
            ...
            Parse form vars ``login`` & ``password`` and if             successful, return them.
        ...
        if path_info == self.logout_handler_path:
            ...
            if path_info == self.login_handler_path:
                ...
                if self.force_https and environ.get('wsgi.url_scheme') != 'https':
                    environ['repoze.who.application'] = HTTPUnauthorized()
                    log and log.debug('force_https is set, but request was http. Skipping')
                    return
                form = parse_formvars(environ)
                try:
                    credentials = {'login': unicode(form[self.username_field], 'utf-8'), 
                       'password': unicode(form[self.password_field], 'utf-8')}
                except KeyError:
                    log and log.debug('login failed: both login and password form inputs must be given')
                    environ['repoze.who.application'] = HTTPFound(self.login_form_path)
                    return
                else:
    ...
    def remember(self, environ, identity):
        ...
        cookie = SET_COOKIE.tuples('%s=""; Path=/; Expires=Sun, 10-May-1971 11:59:00 GMT;' % 'came_from')
...
def make_redirecting_plugin(login_form_path=None, login_handler_path=None, logout_handler_path=None, rememberer_name=None, default_redirect_path='/', login_redirect_method='cookie', logout_redirect_method='cookie', username_field='login', password_field='password', force_https=False, **kw):
    """
    ...
    if login_form_path is None:
        raise ValueError('login_form_url must be set in configuration')
    ...
    if rememberer_name is None:
        raise ValueError('must include rememberer (name of another IIdentifier plugin)')
    force_https = asbool(force_https)
    if not isinstance(force_https, bool):
        raise ValueError('force_https must be a boolean!')
    return CookieRedirectingFormPlugin(login_form_path, login_handler_path, logout_handler_path, rememberer_name, default_redirect_path, logout_redirect_method, login_redirect_method, username_field, password_field, force_https, **kw)
