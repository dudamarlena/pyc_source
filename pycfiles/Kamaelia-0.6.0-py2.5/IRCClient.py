...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/Protocol/IRC/IRCClient.py
...
class IRC_Client(_Axon.Component.component):
    ...
    """
    Inboxes = {'inbox': 'incoming message strings from the server', 'control': 'shutdown handling', 
       'talk': 'takes tuples to be turned into IRC commands '}
    Outboxes = {'outbox': 'IRC commands to be sent out to the server', 'signal': 'shutdown handling', 
    ...
    def __init__(self):
        ...
        self.done = False
        debugSections = {'IRCClient.main': 0, 'IRCClient.handleInput': 0, 
    ...
    def parseIRCMessage(self, line):
        ...
        try:
            ...
            if msgtype == 'PING':
                ...
                recipient = ''
            return (
             msgtype, sender, recipient, body)
        except IndexError:
            return (
             'CLIENT ERROR', 'client', '', line)
    ...
    def handleInput(self, command_tuple):
        ...
        if mod_command[0] == 'ME':
            assert len(mod_command) > 2 and self.debugger.note('IRCClient.handleInput', 10, str(mod_command))
...
def SimpleIRCClientPrefab(host='irc.freenode.net', port=6667):
    ...
    """
    client = Graphline(irc=IRC_Client(), tcp=TCPClient(host, port), linkages={('self', 'inbox'): ('irc', 'talk'), ('irc', 'outbox'): ('tcp', 'inbox'), 
       ('tcp', 'outbox'): ('irc', 'inbox'), 
       ('irc', 'heard'): ('self', 'outbox')})
    return client


__kamaelia_components_ = (
 IRC_Client,)
...
if __name__ == '__main__':
    ...
    print '/join #kamtest'
    print
    Pipeline(ConsoleReader(), PureTransformer(informat), SimpleIRCClientPrefab(), PureTransformer(outformat), ConsoleEchoer()).run()