...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/Device/DVB/Parse/PrettifyTables.py
            ...
            while self.dataReady('inbox'):
                ...
                try:
                    if pat['table_type'] == 'PAT':
                        output = 'PAT received:\n'
                        output += '    Table ID           : %d\n' % pat['table_id']
                        output += '    Table is valid for : ' + ['NEXT (not valid yet)', 'CURRENT (valid)'][pat['current']] + '\n'
                        if 'NIT_PID' in pat:
            ...
            while self.dataReady('inbox'):
                ...
                try:
                    if nit['table_type'] == 'NIT':
                        output = 'NIT received:\n'
                        output += '    Table ID           : %d\n' % nit['table_id']
                        output += '    Table is valid for : ' + ['NEXT (not valid yet)', 'CURRENT (valid)'][nit['current']] + '\n'
                        output += '    Actual or Other n/w: %s\n' % nit['actual_other']
                        output += '    Network ID         : %s\n' % nit['network_id']
                        output += '    Network descriptors:\n'
                        output += formatDescriptors('    Network Descriptors:', '        ', nit['descriptors'])
                        for ts in nit['transport_streams']:
                            ...
                            output += formatDescriptors('        Transport Stream Descriptors:', '            ', ts['descriptors'])
            ...
            while self.dataReady('inbox'):
                ...
                try:
                    if pmt['table_type'] == 'PMT':
                        output = 'PMT received:\n'
                        output += '    Table ID           : %d\n' % pmt['table_id']
                        output += '    Table is valid for : ' + ['NEXT (not valid yet)', 'CURRENT (valid)'][pmt['current']] + '\n'
                        output += '    Services:\n'
                        for (service_id, service) in pmt['services'].items():
                            output += '        Service id : %d\n' % service_id
                            output += '        Program Clock Reference in PID : %d\n' % service['pcr_pid']
                            output += formatDescriptors('        Service Descriptors:', '            ', service['descriptors'])
                            output += '        Streams in service:\n'
                            for stream in service['streams']:
                                output += '            Type : %d\n' % stream['type']
                                output += '                PID  : %d\n' % stream['pid']
                                output += formatDescriptors('                Stream Descriptors:', '                    ', stream['descriptors'])
            ...
            while self.dataReady('inbox'):
                ...
                try:
                    if sdt['table_type'] == 'SDT':
                        output = 'SDT received:\n'
                        output += '    Table ID           : %d\n' % sdt['table_id']
                        output += '    Table is valid for : ' + ['NEXT (not valid yet)', 'CURRENT (valid)'][sdt['current']] + '\n'
                        output += '    Actual or Other n/w: %s\n' % sdt['actual_other']
                        ...
                        for (service_id, service) in sdt['services'].items():
                            output += '        Service id : %d\n' % service_id
                            output += '            EIT present_following? : ' + iif(service['eit_present_following'], 'YES', 'NO') + '\n'
                            output += '            EIT schedule?          : ' + iif(service['eit_schedule'], 'YES', 'NO') + '\n'
                            output += '            Running status         : %d (%s)\n' % (
                             service['running_status'], str(_running_status[service['running_status']]))
                            output += '            Scrambled?             : ' + iif(service['free_CA_mode'], 'YES', 'NO') + '\n'
                            output += formatDescriptors('            Service descriptors:', '                ', service['descriptors'])
            ...
            while self.dataReady('inbox'):
                ...
                try:
                    if eit['table_type'] == 'EIT':
                        output = 'EIT received:\n'
                        output += '    Table ID                      : %d\n' % eit['table_id']
                        output += '    Table is valid for            : ' + ['NEXT (not valid yet)', 'CURRENT (valid)'][eit['current']] + '\n'
                        output += '    Actual or Other n/w           : %s\n' % eit['actual_other']
                        ...
                        for event in eit['events']:
                            ...
                            output += formatDescriptors('            Event descriptors:', '                ', event['descriptors'])
