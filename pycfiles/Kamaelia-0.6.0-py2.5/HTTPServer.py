...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/Protocol/HTTP/HTTPServer.py
...
MapStatusCodeToText = {'100': '100 Continue', 
   '200': '200 OK', 
   '302': '302 Found', 
...
class HTTPShutdownLogicHandling(component):
    Inboxes = {'inbox': 'unused - this is a pure signalling component', 
    ...
    Outboxes = {'outbox': 'unused - this is a pure signalling component', 
...
def HTTPServer(createRequestHandler, **argd):
    ...
    """
    return Graphline(PARSER=HTTPParser(**argd), HANDLER=HTTPRequestHandler(createRequestHandler), CORELOGIC=HTTPShutdownLogicHandling(), linkages={('self', 'inbox'): ('PARSER', 'inbox'), 
...
if __name__ == '__main__':
    ...
    SimpleServer(protocol=createhttpserver, port=8082, socketOptions=(
     socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)).run()