...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/Internet/UDP_ng.py
...
class BasicPeer(Axon.Component.component):
    ...
    """
    Inboxes = {'inbox': 'Data to sent to the socket', 'control': 'Recieve shutdown messages', 
       'readReady': 'Notify that there is incoming data ready on the socket', 
       'writeReady': 'Notify that the socket is ready to send'}
    Outboxes = {'outbox': '(data,(host,port)) tuples for each packet received', 'signal': 'Signals receiver is shutting down', 
    ...
    def safeSend(self, data, target):
        ...
        try:
            ...
            if errorno == errno.AGAIN or errorno == errno.EWOULDBLOCK:
                self.send(newWriter(self, ((self, 'writeReady'), self.sock)), '_selectorSignal')
    ...
    def main(self):
        ...
        while 1:
            ...
            if shutdownOnEmptyBuffer:
                if not self.dataReady('inbox') and len(self.sendBuffer[0]) == 0:
            ...
            if not shutdownOnEmptyBuffer:
                if not self.anyReady() or not (self.sending and len(self.sendBuffer[0]) != 0):
                    self.pause()
            yield 1
...
__kamaelia_components__ = (
 UDPSender, UDPReceiver, SimplePeer,
