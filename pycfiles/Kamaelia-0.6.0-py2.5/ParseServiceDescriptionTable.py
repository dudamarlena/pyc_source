...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/Device/DVB/Parse/ParseServiceDescriptionTable.py
...
def ParseServiceDescriptionTable_ActualTS():
    ...
    """
    return ParseServiceDescriptionTable(acceptTables={66: 'ACTUAL'})
...
def ParseServiceDescriptionTable_OtherTS():
    ...
    """
    return ParseServiceDescriptionTable(acceptTables={70: 'OTHER'})
...
def ParseServiceDescriptionTable_ActualAndOtherTS():
    ...
    """
    return ParseServiceDescriptionTable(acceptTables={66: 'ACTUAL', 70: 'OTHER'})
...
class ParseServiceDescriptionTable(component):
    ...
    """
    Inboxes = {'inbox': 'DVB PSI Packets from a single PID containing SDT table sections', 'control': 'Shutdown signalling'}
    Outboxes = {'outbox': 'Parsed PMT table (only when it changes)', 'signal': 'Shutdown signalling'}

    def __init__(self, acceptTables={66: 'ACTUAL', 70: 'OTHER'}):
    ...
    def parseTable(self, index, sections):
        ...
        msg = {'table_type': 'SDT', 'table_id': table_id, 
        ...
        for (data, section_length) in sections:
            ...
            while i < section_length + 3 - 4:
                ...
                service = {}
                lo = ord(data[(i + 2)])
                service['eit_schedule'] = lo & 2
                service['eit_present_following'] = lo & 1
                hi = ord(data[(i + 3)])
                service['running_status'] = hi >> 5
                service['free_CA_mode'] = hi & 16
                descriptors_length = (hi << 8) + ord(data[(i + 4)]) & 4095
                i = i + 5
...
if __name__ == '__main__':
    ...
    feparams = {'inversion': dvb3.frontend.INVERSION_AUTO, 
       ...
       'code_rate_HP': dvb3.frontend.FEC_3_4, 
       'code_rate_LP': dvb3.frontend.FEC_3_4}
    Pipeline(DVB_Multiplex(505833330.0 / 1000000.0, [SDT_PID], feparams), DVB_Demuxer({SDT_PID: ['outbox']}), ReassemblePSITables(), ParseServiceDescriptionTable_ActualAndOtherTS(), PrettifyServiceDescriptionTable(), ConsoleEchoer()).run()