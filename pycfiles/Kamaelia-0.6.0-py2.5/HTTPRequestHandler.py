...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/Protocol/HTTP/HTTPRequestHandler.py
...
class HTTPRequestHandler(component):
    ...
    Outboxes = {'outbox': 'HTTP responses', 
            ...
            for header in resource.get('headers', []):
                if header[0] == 'Content-Type':
                    if resource.has_key('charset'):
                        header = (
                         header[0], header[1] + '; ' + resource['charset'])
                hl.append(header)
