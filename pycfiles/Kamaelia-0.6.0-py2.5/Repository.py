...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/Support/Data/Repository.py
...
class Scope(object):
    ...
    def __init__(self, type='Module', ASTChildren=None, imports=None, localModules={}, rootScope=None):
        ...
        if ASTChildren is None or ASTChildren == []:
            return
        try:
            ASTChildren = ASTChildren.getChildNodes()
        except:
            pass

        for node in ASTChildren:
            if isinstance(node, ast.From):
                self._parse_From(node)
    ...
    def find(self, name, checkRoot=True):
        ...
        elif checkRoot and self.rootScope != self:
...
class FunctionScope(Scope):
    ...
    def __init__(self, AST, imports, localModules, rootScope):
        super(FunctionScope, self).__init__('Class', None, imports, localModules, rootScope)
        self.ast = AST
        if AST.doc is not None:
            self.doc = AST.doc
        else:
...
class ImportScope(Scope):
    ...
    def __init__(self, importPathName='', imports=None):
        if importPathName == '' and imports == None:
            imports = self
        super(ImportScope, self).__init__('Module', None, imports, [], None)
        self.doc = ''
...
class ModuleDoc(ModuleScope):
    ...
    def scanSubdirs(self, filePath, moduleName):
        ...
        if exists(filename):
            ...
            AST = compiler.parse('')
        subdirs = [ name for name in os.listdir(filePath) if isdir(pathjoin(filePath, name)) if name not in self.ignoreFilenames ]
        sourcefiles = [ name for name in os.listdir(filePath) if name not in subdirs if name[-3:] == '.py' if name not in self.ignoreFilenames ]
        localModules = {}
