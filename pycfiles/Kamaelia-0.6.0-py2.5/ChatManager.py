...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/Protocol/AIM/ChatManager.py
...
class ChatManager(SNACExchanger):
    ...
    """
    Inboxes = {'inbox': 'incoming FLAPs on channel 2', 'errors': 'error messages', 
       'control': 'NOT USED', 
       'talk': 'outgoing messages'}
    Outboxes = {'outbox': 'outgoing FLAPs', 'signal': 'NOT USED', 
       'heard': 'echoes peer messages to this box.'}

    def __init__(self):
        """x.__init__(...) initializes x; see x.__class__.__doc__ for signature"""
        super(ChatManager, self).__init__()
        debugSections = {'ChatManager.main': 5, 'ChatManager.receiveMessage': 5, 
    ...
    def receiveMessage(self, body):
        ...
        if msgchan == 1:
            fragments = readTLVs(parsed[2])
            msgText = fragments[257]
            (charSetID, charSubset) = struct.unpack('!HH', msgText[:4])
            message = msgText[4:]
    ...
    def sendMessage(self, buddyname, text):
        ...
        """
        frag = TLV(1281, struct.pack('!i', 0))
        frag2 = TLV(257, struct.pack('!HH', 0, 0) + text)
        msgCookie = '2671466\x00'
        msgChan = 1
        buddyinfo = Single(len(buddyname)) + buddyname
        body = msgCookie + Double(msgChan) + buddyinfo + TLV(2, frag + frag2)
        self.sendSnac(4, 6, body)
...
if __name__ == '__main__':
    ...
    p = Graphline(chargen=Chargen(), cm=ChatManager(), ce=ConsoleEchoer(), linkages={('chargen', 'outbox'): ('cm', 'inbox'), ('cm', 'heard'): ('ce', 'inbox')})
    p.run()