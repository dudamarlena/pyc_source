...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/Device/DVB/DemuxerService.py
...
if __name__ == '__main__':
        ...
        def main(self):
            while 1:
                for pid in range(0, 10):
                    packet = DVB_RESYNC + chr(0) + chr(pid) + chr(0) + ' ' * 184
                    self.send(packet, 'outbox')
                    yield 1
    ...
    Pipeline(PacketSource(), RegisterService(DemuxerService(), {'MUX1': 'request'})).run()