...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/Internet/TCPServer.py
...
"""
import socket, errno, random, Axon, Kamaelia.IPC as _ki
...
class TCPServer(Axon.Component.component):
   ...
   specified port.
   """
    Inboxes = {'_feedbackFromCSA': 'for feedback from ConnectedSocketAdapter (shutdown messages)', 'newconnection': 'We expected to recieve a message here when a new connection is ready', 
       'control': 'we expect to recieve serverShutdown messages here'}
    Outboxes = {'protocolHandlerSignal': 'For passing on newly created ConnectedSocketAdapter components', 'signal': 'NOT USED', 
    ...
    def makeTCPServerPort(self, suppliedport=None, HOST=None, minrange=2000, maxrange=50000, maxlisten=5):
      ...
      """
        ...
        if suppliedport is None:
            PORT = random.randint(minrange, maxrange)
        else:
            PORT = suppliedport
    ...
    def closeSocket(self, shutdownMessage):
      ...
      """
        ...
        self.send(_ki.shutdownCSA(self, theComponent), 'protocolHandlerSignal')
