...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/UI/OpenGL/Movement.py
...
class PathMover(Axon.Component.component):
    ...
    """
    Inboxes = {'inbox': 'Commands are received here', 
       'control': 'ignored'}
    Outboxes = {'outbox': 'Outbox for sending Control3D commands', 
...
class WheelMover(Axon.AdaptiveCommsComponent.AdaptiveCommsComponent):
    ...
    Outboxes = {'outbox': 'Outbox for sending position updates'}
...
class SimpleMover(Axon.Component.component):
    ...
    def main(self):
        ...
        while 1:
            ...
            y += dy
            z += dz
            if abs(x) > self.borders[0]:
                dx = -dx
            if abs(y) > self.borders[1]:
                dy = -dy
            if abs(z - self.origin[2]) > self.borders[2]:
                dz = -dz
...
__kamaelia_components__ = (
 PathMover, WheelMover, SimpleRotator, SimpleMover, SimpleBuzzer)
