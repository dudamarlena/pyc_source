...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/Device/DVB/Parse/ParseProgramAssociationTable.py
...
class ParseProgramAssociationTable(component):
    ...
    """
    Inboxes = {'inbox': 'DVB PSI Packets containing PAT table sections', 'control': 'Shutdown signalling'}
    Outboxes = {'outbox': 'Parsed PAT table (only when it changes)', 'signal': 'Shutdown signalling'}

    def parseTable(self, table_id, current_next, sections):
        msg = {'table_type': 'PAT', 'table_id': table_id, 
...
if __name__ == '__main__':
    ...
    feparams = {'inversion': dvb3.frontend.INVERSION_AUTO, 
       ...
       'code_rate_HP': dvb3.frontend.FEC_3_4, 
       'code_rate_LP': dvb3.frontend.FEC_3_4}
    Pipeline(DVB_Multiplex(505833330.0 / 1000000.0, [PAT_PID], feparams), DVB_Demuxer({PAT_PID: ['outbox']}), ReassemblePSITables(), ParseProgramAssociationTable(), PrettifyProgramAssociationTable(), ConsoleEchoer()).run()