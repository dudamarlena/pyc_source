...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/Experimental/Chassis.py
...
def Pipeline(*components):
    r"""    Pipeline(\*components) -> new Pipeline component.
...
def Graphline(linkages=None, boxsizes=None, **components):
    r"""    Graphline([linkages][,boxsizes],\*\*components) -> new Graphline component
...
def InboxControlledCarousel(*argv, **argd):
    ...
    - boxsize          -- size limit for "inbox" inbox of the created child component
    """
    return Graphline(CAROUSEL=Carousel(*argv, **argd), linkages={('', 'inbox'): ('CAROUSEL', 'next'), 
...
__kamaelia_prefabs__ = (
 Pipeline, Graphline, Carousel, InboxControlledCarousel)