...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/Chassis/ConnectedServer.py
...
class ServerCore(Axon.AdaptiveCommsComponent.AdaptiveCommsComponent):
    ...
    """
    Inboxes = {'_socketactivity': 'Messages about new and closing connections here', 'control': 'We expect to get serverShutdown messages here'}
    ...
    def mkProtocolHandler(self, **sock_info):
        return self.protocol(peer=sock_info['peer'], peerport=sock_info['peerport'], localip=sock_info['localip'], localport=sock_info['localport'])
    ...
    def handleNewConnection(self, newCSAMessage):
        ...
        self.trackResourceInformation(connectedSocket, [], [
         outboxToShutdownProtocolHandler], protocolHandler)
    ...
    def handleClosedCSA(self, shutdownCSAMessage):
        ...
        self.send(socketShutdown(), resourceOutboxes[0])
        self.send(shutdownMicroprocess(), resourceOutboxes[1])
        self.removeChild(connectedSocket)
        self.removeChild(protocolHandler)
        self.deleteOutbox(resourceOutboxes[0])
        self.deleteOutbox(resourceOutboxes[1])
