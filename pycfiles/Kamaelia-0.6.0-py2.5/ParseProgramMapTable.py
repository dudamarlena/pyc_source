...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/Device/DVB/Parse/ParseProgramMapTable.py
...
class ParseProgramMapTable(component):
    ...
    """
    Inboxes = {'inbox': 'DVB PSI Packets from a single PID containing PMT table sections', 'control': 'Shutdown signalling'}
    ...
    def parseTable(self, table_id, current_next, sections):
        msg = {'table_type': 'PMT', 'table_id': table_id, 
           'current': current_next}
        services = {}
        for (data, section_length) in sections:
            service_id = (ord(data[3]) << 8) + ord(data[4])
            service = {}
            service['pcr_pid'] = (ord(data[8]) << 8) + ord(data[9]) & 8191
            prog_info_length = (ord(data[10]) << 8) + ord(data[11]) & 4095
            i = 12
            ...
            while i < section_length + 3 - 4:
                ...
                stream['pid'] = (ord(data[(i + 1)]) << 8) + ord(data[(i + 2)]) & 8191
                es_info_length = (ord(data[(i + 3)]) << 8) + ord(data[(i + 4)]) & 4095
                i = i + 5
...
if __name__ == '__main__':
    ...
    feparams = {'inversion': dvb3.frontend.INVERSION_AUTO, 
       ...
       'code_rate_LP': dvb3.frontend.FEC_3_4}
    PMT_PID = 4228
    Pipeline(DVB_Multiplex(505833330.0 / 1000000.0, [PMT_PID], feparams), DVB_Demuxer({PMT_PID: ['outbox']}), ReassemblePSITables(), ParseProgramMapTable(), PrettifyProgramMapTable(), ConsoleEchoer()).run()