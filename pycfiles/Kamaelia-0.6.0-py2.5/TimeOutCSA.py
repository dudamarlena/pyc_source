...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/Internet/TimeOutCSA.py
...
class ResettableSender(Axon.ThreadedComponent.threadedcomponent):
    ...
    message = 'NEXT'
    debug = False
    Inboxes = {'inbox': 'Send anything here to reset the timeout countdown.', 
       'control': 'NOT USED'}
    Outboxes = {'outbox': "'NEXT' message sent when the timeout occurs.", 
       'signal': 'NOT USED'}
...
class ActivityMonitor(Axon.Component.component):
    ...
    Please note that this can't wrap adaptive components properly as of yet.
    """
    Inboxes = {'inbox': "Messages to be 'observed'. Will be forwarded on out of the 'outbox' outbox.", 
    ...
    Outboxes = {'outbox': "Forwards any messages received on the 'inbox' inbox", 
...
def InactivityChassis(somecomponent, timeout=2, debug=False):
    ...
    """
    linkages = {('SHUTTERDOWNER', 'outbox'): ('OBJ', 'control'), 
    ...
    return Graphline(OBJ=somecomponent, ACT=ActivityMonitor(), SHUTTERDOWNER=ResettableSender(debug=debug, message=producerFinished(), timeout=timeout), linkages=linkages)
