...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/Device/DVB/Parse/ParseNetworkInformationTable.py
...
def ParseNetworkInformationTable_ActualNetwork():
    ...
    """
    return ParseNetworkInformationTable(acceptTables={64: 'ACTUAL'})
...
def ParseNetworkInformationTable_OtherNetwork():
    ...
    """
    return ParseNetworkInformationTable(acceptTables={65: 'OTHER'})
...
def ParseNetworkInformationTable_ActualAndOtherNetwork():
    ...
    """
    return ParseNetworkInformationTable(acceptTables={64: 'ACTUAL', 65: 'OTHER'})
...
class ParseNetworkInformationTable(component):
    ...
    """
    Inboxes = {'inbox': 'DVB PSI Packets from a single PID containing NIT table sections', 'control': 'Shutdown signalling'}
    Outboxes = {'outbox': 'Parsed NIT (only when it changes)', 'signal': 'Shutdown signalling'}

    def __init__(self, acceptTables={64: 'ACTUAL', 65: 'OTHER'}):
    ...
    def parseTable(self, index, sections):
        ...
        msg = {'table_type': 'NIT', 'table_id': table_id, 
        ...
        for (data, section_length) in sections:
            network_descriptors_length = (ord(data[8]) << 8) + ord(data[9]) & 4095
            i = 10
            ...
            ts_loop_length = (ord(data[i]) << 8) + ord(data[(i + 1)]) & 4095
            i = i + 2
            ...
            while i < ts_loop_end:
                ...
                transport_descriptors_length = (ord(data[(i + 4)]) << 8) + ord(data[(i + 5)]) & 4095
                i = i + 6
    ...
    def main(self):
        ...
        while not self.shutdown():
            while self.dataReady('inbox'):
                ...
                index = (
                 table_id, current_next, network_id)
                crcpass = False
...
if __name__ == '__main__':
    ...
    feparams = {'inversion': dvb3.frontend.INVERSION_AUTO, 
       ...
       'code_rate_HP': dvb3.frontend.FEC_3_4, 
       'code_rate_LP': dvb3.frontend.FEC_3_4}
    Pipeline(DVB_Multiplex(505833330.0 / 1000000.0, [NIT_PID], feparams), DVB_Demuxer({NIT_PID: ['outbox']}), ReassemblePSITables(), ParseNetworkInformationTable_ActualNetwork(), ConsoleEchoer(forwarder=True), PrettifyNetworkInformationTable(), ConsoleEchoer()).run()