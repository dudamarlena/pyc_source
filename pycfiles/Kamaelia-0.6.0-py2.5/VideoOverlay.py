...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/UI/Pygame/VideoOverlay.py
...
class VideoOverlay(component):
    ...
    """
    Inboxes = {'inbox': 'Receives uncompressed video frames', 'control': 'Shutdown signalling'}
    ...
    def newOverlay(self, frame):
        ...
        self.send({'OVERLAYREQUEST': True, 'pixformat': self.pygamepixformat, 
           'yuvservice': (
                        self, 'yuvdata'), 
           'size': (
                  self.size[0] - OVERLAY_FUDGE_OFFSET_FACTOR, self.size[1]), 
           'position': self.position, 
           'yuv': frame['yuv']}, 'displayctrl')
        yield 1
...
if __name__ == '__main__':
    ...
    Pipeline(ReadFileAdaptor('/data/dirac-video/snowboard-jum-352x288x75.yuv', readmode='bitrate', bitrate=18247680), RawYUVFramer(size=(352,
                                                                                                                                         288), pixformat='YUV420_planar'), VideoOverlay()).run()