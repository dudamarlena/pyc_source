...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/UI/OpenGL/OpenGLComponent.py
...
class OpenGLComponent(Axon.AdaptiveCommsComponent.AdaptiveCommsComponent):
    ...
    def main(self):
        self.disprequest = {'OGL_DISPLAYREQUEST': True, 'objectid': id(self), 
           'callback': (
                      self, 'callback'), 
           'events': (
                    self, 'events'), 
    ...
    def applyTransforms(self):
        ...
        if self.oldscaling != self.scaling or self.oldrot != self.rotation or self.oldpos != self.position:
            ...
            transform_update = {'TRANSFORM_UPDATE': True, 'objectid': id(self), 
    ...
    def redraw(self):
        ...
        self.draw()
        glEndList()
        dl_update = {'DISPLAYLIST_UPDATE': True, 'objectid': id(self), 
