...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/Apps/Grey/MailHandler.py
...
class MailHandler(Axon.Component.component):
    ...
    def handleCommand(self, command):
        if len(command) < 1:
            ...
            return
        if command[0] == 'HELO':
            return self.handleHelo(command)
        if command[0] == 'EHLO':
            return self.handleEhlo(command)
        if command[0] == 'MAIL':
            return self.handleMail(command)
        if command[0] == 'RCPT':
            return self.handleRcpt(command)
        if command[0] == 'DATA':
            return self.handleData(command)
        if command[0] == 'QUIT':
            return self.handleQuit(command)
        if command[0] == 'RSET':
            return self.handleRset(command)
        if command[0] == 'NOOP':
            return self.handleNoop(command)
        if command[0] == 'VRFY':
            return self.handleVrfy(command)
        if command[0] == 'HELP':
            return self.handleHelp(command)
        self.netPrint("500 Sorry we don't like broken mailers")
        self.breakConnection = True
