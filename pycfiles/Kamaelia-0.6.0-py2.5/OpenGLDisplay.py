...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/UI/OpenGL/OpenGLDisplay.py
...
class OpenGLDisplay(Axon.AdaptiveCommsComponent.AdaptiveCommsComponent):
    ...
    def calcPow2Size(self, size):
        ...
        pow2size = (
         int(2 ** ceil(log(size[0], 2))), int(2 ** ceil(log(size[1], 2))))
        return pow2size
    ...
    def handleRequest_WRAPPERREQUEST(self, message):
        ...
        try:
            ...
            response = {'texname': self.pygame_texnames[id(surface)], 'texsize': (
                         tex_w, tex_h), 
    ...
    def drawPygameSurfaces(self):
        ...
        for surface in self.pygame_surfaces:
            if id(surface) in self.wrappedsurfaces:
                continue
            ...
            glBegin(GL_QUADS)
            glColor3f(1, 0, 0)
            glTexCoord2f(0.0, 1.0 - tex_h)
            glVertex3f(l, b, -self.farPlaneDist)
            glTexCoord2f(tex_w, 1.0 - tex_h)
            glVertex3f(r, b, -self.farPlaneDist)
            glTexCoord2f(tex_w, 1.0)
            glVertex3f(r, t, -self.farPlaneDist)
            glTexCoord2f(0.0, 1.0)
            glVertex3f(l, t, -self.farPlaneDist)
            glEnd()
    ...
    def updatePygameTexture(self, surface, pow2surface, texname):
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, pow2surface.get_width(), pow2surface.get_height(), 0, GL_RGBA, GL_UNSIGNED_BYTE, textureData)
        glDisable(GL_TEXTURE_2D)
            ...
            try:
                ...
                if eventcomms is not None:
                    ...
                    for event in events:
                        wanted = False
                        try:
                            wanted = self.eventswanted[id(surface)][event.type]
                        except KeyError:
                            pass
                        ...
                        if wanted:
                            if event.type in [pygame.MOUSEMOTION, pygame.MOUSEBUTTONUP, pygame.MOUSEBUTTONDOWN]:
                                ...
                                e.pos = (
                                 pos[0] - surfpos[0], pos[1] - surfpos[1])
                                if event.type == pygame.MOUSEMOTION:
                                    e.rel = event.rel
                                if event.type == pygame.MOUSEMOTION:
                                    e.buttons = event.buttons
