...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/UI/OpenGL/PygameWrapper.py
...
class PygameWrapper(OpenGLComponent):
    ...
    def setup(self):
        ...
        wraprequest = {'WRAPPERREQUEST': True, 'wrapcallback': (
                          self, 'wrapcallback'), 
           'eventrequests': (
                           self, 'eventrequests'), 
...
if __name__ == '__main__':
    ...
    PygameDisplay.setDisplayService(ogl_display[0])
    TICKER = Ticker(size=(150, 150)).activate()
    TICKER_WRAPPER = PygameWrapper(wrap=TICKER, position=(4, 1, -10), rotation=(-20,
                                                                                15,
                                                                                3)).activate()
    MAGNADOODLE = MagnaDoodle(size=(200, 200)).activate()
    MAGNADOODLEWRAPPER = PygameWrapper(wrap=MAGNADOODLE, position=(-2, -2, -10), rotation=(20,
                                                                                           10,
                                                                                           0)).activate()
    READER = ConsoleReader().activate()
    READER.link((READER, 'outbox'), (TICKER, 'inbox'))
