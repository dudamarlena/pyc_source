...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/Support/Protocol/IRC.py
...
def informat(text, defaultChannel='#kamtest'):
    ...
    """
    if text[0] != '/' or text.split()[0] == '/':
        return (
         'PRIVMSG', defaultChannel, text)
    words = text.split()
    tag = words[0]
    ...
    try:
        if tag == 'QUIT' and len(words) >= 2:
            return (
             tag, string.join(words[1:]))
        elif tag in ('PRIVMSG', 'MSG', 'NOTICE', 'KILL', 'TOPIC', 'SQUERY') and len(words) >= 3:
            return (
             tag, words[1], string.join(words[2:]))
        elif tag == 'KICK' and len(words) >= 4:
            return (
             tag, words[1], words[2], string.join(words[3:]))
        elif tag == 'USER':
            return (
             tag, words[1], words[2], words[3], string.join(words[4:]))
        elif tag == 'ME' and len(words) >= 2:
            return (
             tag, defaultChannel, string.join(words[1:]))
        else:
            words[0] = tag
            if tag:
                return words
