...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/Codec/Dirac.py
...
class DiracDecoder(component):
    ...
    """
    Inboxes = {'inbox': 'Strings containing an encoded dirac video stream', 'control': 'for shutdown signalling'}
    Outboxes = {'outbox': 'YUV decoded video frames', 'signal': 'for shutdown/completion signalling'}
...
class DiracEncoder(component):
    ...
    def __init__(self, preset=None, verbose=False, encParams={}, seqParams={}, allParams={}):
        ...
        if _encoder_version == (0, 5, 4):
            self.encoder = EncoderWrapper(preset=preset, bufsize=1048576, verbose=verbose, encParams=allParams, seqParams=allParams)
        else:
            self.encoder = EncoderWrapper(preset=preset, bufsize=1048576, verbose=verbose, allParams=allParams)
