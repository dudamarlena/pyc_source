...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/File/UnixProcess2.py
...
class UnixProcess2(component):
    ...
    """
    Inboxes = {'inbox': 'Binary string data to go to STDIN of the process.', 'control': 'Shutdown signalling'}
    Outboxes = {'outbox': 'Binary string data from STDOUT of the process', 'error': 'Binary string data from STDERR of the process', 
    ...
    def main(self):
        ...
        p = subprocess.Popen(self.command, shell=True, bufsize=self.buffersize, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, close_fds=True)
...
class _ToFileHandle(component):
    ...
    """
    Inboxes = {'inbox': 'Binary string data to be written to the file handle', 'control': 'Shutdown signalling', 
...
class _FromFileHandle(component):
    ...
    Outboxes = {'outbox': 'Binary string data read from the file handle', 'signal': 'Shutdown signalling', 
...
if __name__ == '__main__':
    ...
    if test == 'rate limit output':
        Pipeline(UnixProcess2('cat /dev/zero', 33554432), LineSplit(), Chunk(10), RateLimiter(10), CumulateSize(), ConsoleEchoer(forwarder=True)).run()
    elif test == 'rate limited input':
        ratelimiter = RateLimiter(10)
        ratelimiter.inboxes['inbox'].setSize(None)
        Pipeline(ChargenComponent(), ratelimiter, UnixProcess2('cat -', 32), ConsoleEchoer(forwarder=True)).run()
    elif test == 'reached end of output':
        Pipeline(ChargenComponent(), UnixProcess2('wc', 32), ConsoleEchoer(forwarder=True)).run()
    elif test == 'outpipes':
        try:
            ...
            Graphline(SRC=ChargenComponent(), UXP=UnixProcess2('cat - > /tmp/tmppipe', outpipes={'/tmp/tmppipe': 'output'}), DST=ConsoleEchoer(), linkages={('SRC', 'outbox'): ('UXP', 'inbox'), 
               ('UXP', 'output'): ('DST', 'inbox'), 
               ('SRC', 'signal'): ('UXP', 'control'), 
               ('UXP', 'signal'): ('DST', 'control')}).run()
    elif test == 'inpipes':
        try:
            os.remove('/tmp/tmppipe')
        except OSError:
            ...
            Graphline(SRC=ChargenComponent(), UXP=UnixProcess2('cat /tmp/tmppipe', inpipes={'/tmp/tmppipe': 'input'}), DST=ConsoleEchoer(), linkages={('SRC', 'outbox'): ('UXP', 'input'), 
