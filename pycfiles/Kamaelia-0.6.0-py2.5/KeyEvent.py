...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/UI/Pygame/KeyEvent.py
...
class KeyEvent(Axon.Component.component):
   ...
   """
    ...
    Outboxes = {'outbox': 'NOT USED', 'allkeys': 'Outbox that receives *every* keystroke if enabled', 
    ...
    def main(self):
        ...
        if self.key_events is not None or self.allkeys or self.key_up_events is not None:
            message = {'ADDLISTENEVENT': pygame.KEYDOWN, 'surface': self.display, 'TRACE': 'ME'}
            self.send(message, 'display_signal')
        if self.allkeys or self.key_up_events is not None:
            message = {'ADDLISTENEVENT': pygame.KEYUP, 'surface': self.display, 'TRACE': 'METO'}
            self.send(message, 'display_signal')
        done = False
        while not done:
