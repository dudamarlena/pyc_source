...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/Codec/WAV.py
...
class WAVParser(component):
    ...
    """
    Inboxes = {'inbox': 'Raw WAV file data', 'control': 'Shutdown signalling'}
    Outboxes = {'outbox': 'Binary audio data strings', 'signal': 'Shutdown signalling', 
    ...
    def main(self):
        ...
        if format == 1:
            ...
            self.send({'channels': channels, 'sample_format': audioformat, 
               'sample_rate': sample_rate}, 'all_meta')
        else:
            raise "Can't handle WAV file in anything other than uncompressed format. Format tag found = " + str(format)
        if headerBytesLeft > 0:
            for _ in self.readbytes(headerBytesLeft):
                yield _
        ...
        while size != 0:
            if size > 0:
                for _ in self.readuptobytes(size):
                    yield _

            for _ in self.readuptobytes(32768):
                yield _

            for _ in self.safesend(self.bytesread, 'outbox'):
                yield _
...
class WAVWriter(component):
    ...
    def main(self):
        ...
        try:
            ...
            datachunkheader = 'data' + struct.pack('<L', 0)
...
if __name__ == '__main__':
    ...
    Graphline(READ=RateControlledFileReader('/usr/share/sounds/alsa/Front_Center.wav', readmode='bytes', rate=1000000), PARSE=WAVParser(), ENC=Carousel(lambda meta: WAVWriter(**meta)), WRITE=SimpleFileWriter('test.wav'), linkages={('READ', 'outbox'): (
                          'PARSE', 'inbox'), 
       ('PARSE', 'outbox'): (
                           'ENC', 'inbox'), 
       ('PARSE', 'all_meta'): (
    ...
    Graphline(SRC=RateControlledFileReader('test.wav', readmode='bytes', rate=44100 * 4), WAV=WAVParser(), DST=Carousel(lambda meta: Output(sample_rate=meta['sample_rate'], format=meta['sample_format'], channels=meta['channels'])), linkages={('SRC', 'outbox'): (
                         'WAV', 'inbox'), 
       ('SRC', 'signal'): (
                         'WAV', 'control'), 
       ('WAV', 'outbox'): (
