...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/Protocol/AIM/OSCARClient.py
...
class OSCARProtocol(component):
    ...
    """
    Inboxes = {'inbox': 'receives binary data from the AIM server', 'control': 'shutdown handling', 
       'talk': 'receives messages in the format (channel, FLAP payload)'}
    Outboxes = {'outbox': 'sends binary data to the AIM server.', 'signal': 'shutdown handling', 
    ...
    def __init__(self):
        ...
        self.done = False
        self.buffer = ''
        debugSections = {'OSCARProtocol.main': 0, 'OSCARProtocol.checkBoxes': 0, 
    ...
    def handleinbox(self):
        ...
        while data:
            ...
            if len(data) < 6 + datalen:
                assert self.debugger.note('OSCARProtocol.handleinbox', 1, 'FLAP shorter than specified length! ' + str(datalen) + ' ' + str(len(data)))
    ...
    def sendFLAP(self, data, channel=2):
...
def OSCARClient(server, port):
    ...
    """
    return Graphline(oscar=OSCARProtocol(), tcp=TCPClient(server, port), linkages={('oscar', 'outbox'): ('tcp', 'inbox'), 
...
class SNACExchanger(component):
    ...
    def __init__(self):
        ...
        debugSections = {'SNACExchanger.recvSnac': 0, 'SNACExchanger.sendSnac': 0}
        self.debugger.addDebug(**debugSections)
...
if __name__ == '__main__':
    ...
    class LoginHandler(SNACExchanger):
        ...
        def main(self):
            ...
            reply = self.recv()
            zero = struct.pack('!H', 0)
            request = TLV(1, 'kamaelia1') + TLV(75, zero) + TLV(90, zero)
    ...
    Graphline(osc=OSCARClient('login.oscar.aol.com', 5190), login=LoginHandler(), linkages={('login', 'outbox'): ('osc', 'inbox'), ('osc', 'outbox'): ('login', 'inbox')}).run()