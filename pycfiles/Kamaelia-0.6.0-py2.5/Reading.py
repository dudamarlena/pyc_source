...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/File/Reading.py
...
class PromptedFileReader(component):
    ...
    """
    Inboxes = {'inbox': "requests to 'n' read bytes/lines", 'control': 'for shutdown signalling'}
    Outboxes = {'outbox': 'data output', 'signal': "outputs 'producerFinished' after all data has been read"}
...
def RateControlledFileReader(filename, readmode='bytes', **rateargs):
    ...
    """
    return Graphline(RC=ByteRate_RequestControl(**rateargs), RFA=PromptedFileReader(filename, readmode), linkages={('RC', 'outbox'): ('RFA', 'inbox'), ('RFA', 'outbox'): ('self', 'outbox'), 
...
def FixedRateControlledReusableFileReader(readmode='bytes', **rateargs):
    ...
    - rateargs = arguments for ByteRate_RequestControl component constructor
    """
    return Graphline(RC=ByteRate_RequestControl(**rateargs), CAR=ReusableFileReader(readmode), linkages={('self', 'inbox'): ('CAR', 'next'), 
