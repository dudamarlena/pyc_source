...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/UI/Pygame/EventHandler.py
    ...
    def dispatch(self, event, where):
      ...
      """
        ...
        if event.type == _pygame.ACTIVEEVENT:
            self.active(event.gain, event.state, where)
        if event.type == _pygame.KEYDOWN:
            self.keydown(event.unicode, event.key, event.mod, where)
        if event.type == _pygame.KEYUP:
            self.keyup(event.key, event.mod, where)
        if event.type == _pygame.MOUSEMOTION:
            self.mousemotion(event.pos, event.rel, event.buttons, where)
        if event.type == _pygame.MOUSEBUTTONUP:
            self.mousebuttonup(event.pos, event.button, where)
        if event.type == _pygame.MOUSEBUTTONDOWN:
            self.mousebuttondown(event.pos, event.button, where)
        if event.type == _pygame.JOYAXISMOTION:
            self.joyaxismotion(event.joy, event.axis, event.value, where)
        if event.type == _pygame.JOYBALLMOTION:
            self.joyballmotion(event.joy, event.ball, event.rel, where)
        if event.type == _pygame.JOYHATMOTION:
            self.joyhatmotion(event.joy, event.hat, event.value, where)
        if event.type == _pygame.JOYBUTTONUP:
            self.joybuttonup(event.joy, event.button, where)
        if event.type == _pygame.JOYBUTTONDOWN:
            self.joybuttondown(event.joy, event.button, where)
        if event.type == _pygame.VIDEORESIZE:
            self.videoresize(event.size, where)
        if event.type == _pygame.VIDEOEXPOSE:
            self.videoexpose(where)
        if event.type == _pygame.USEREVENT:
            self.userevent(event.code, where)
