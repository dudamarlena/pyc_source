...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/Util/RateFilter.py
...
class MessageRateLimit(component):
    ...
    """
    Inboxes = {'inbox': 'Incoming items/messages', 'control': 'NOT USED'}
    Outboxes = {'outbox': 'Items/messages limited to specified maximum output rate', 'signal': 'NOT USED'}
...
class ByteRate_RequestControl(component):
    ...
    Outboxes = {'outbox': "requests for 'n' items", 'signal': 'Shutdown signalling'}
    ...
    def __init__(self, rate=100000, chunksize=None, chunkrate=None, allowchunkaggregation=False):
        ...
        if chunksize is None and chunkrate is None:
            chunksize = max(1, rate / 1000)
        if chunksize is not None:
            self.chunksize = chunksize
            chunkrate = float(rate) / float(chunksize)
        elif chunkrate is not None:
...
class VariableByteRate_RequestControl(component):
    ...
    Specify either chunksize or chunkrate, but not both.
    """
    Inboxes = {'inbox': 'New rate', 'control': 'Shutdown signalling'}
    Outboxes = {'outbox': "requests for 'n' items", 'signal': 'Shutdown signalling'}
...
class OnDemandLimit(component):
    ...
    """
    Inboxes = {'inbox': 'Data items to be passed on, on demand.', 'control': 'Shutdown signalling', 
       'slidecontrol': "'NEXT' requests to emit a data item."}
    Outboxes = {'outbox': 'Data items, when requested.', 'signal': 'Shutdown signalling'}
...
__kamaelia_components__ = (
 MessageRateLimit, ByteRate_RequestControl, VariableByteRate_RequestControl, OnDemandLimit)