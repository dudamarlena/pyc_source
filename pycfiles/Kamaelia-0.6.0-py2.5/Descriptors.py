...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/Support/DVB/Descriptors.py
...
def parser_CA_Descriptor(data, i, length, end):
    ...
    (Defined in ISO 13818-1 specification)
    """
    d = {'type': 'CA', 'CA_system_id': (ord(data[(i + 2)]) << 8) + ord(data[(i + 3)]), 
       'pid': (ord(data[(i + 4)]) << 8) + ord(data[(i + 5)]) & 8191, 
...
def parser_ISO_639_Descriptor(data, i, length, end):
    ...
    j = i + 2
    while j < end:
        parts.append({'language_code': data[j:j + 3], 'audio_type': _iso639_audiotypes.get(ord(data[(j + 3)]), ord(data[(j + 3)]))})
        j += 4

    d = {'type': 'ISO_639', 'entries': parts}
    return d
...
def parser_network_name_Descriptor(data, i, length, end):
    ...
    """
    d = {'type': 'network_name', 'network_name': data[i + 2:end]}
    return d
...
def parser_service_Descriptor(data, i, length, end):
    ...
    (Defined in ETSI EN 300 468 specification)
    """
    d = {'type': 'service', 'service_type': _service_types.get(ord(data[(i + 2)]), ord(data[(i + 2)]))}
    length = ord(data[(i + 3)])
    j = i + 4 + length
...
def parser_short_event_Descriptor(data, i, length, end):
    ...
    (Defined in ETSI EN 300 468 specification)
    """
    d = {'type': 'short_event', 'language_code': data[i + 2:i + 5]}
...
def parser_component_Descriptor(data, i, length, end):
    ...
    """
    e = [
     ord(data[(i + 2)]), ord(data[(i + 3)]), ord(data[(i + 4)])]
    e[0] = e[0] & 15
    sctype = _stream_component_mappings.get((e[0], e[1]), (e[0], e[1]))
    d = {'type': 'component', 'stream_content': e[0], 
       'component_type': e[1], 
       'component_tag': e[2], 
       'content,type': sctype, 
...
def parser_stream_identifier_Descriptor(data, i, length, end):
    ...
    (Defined in ETSI EN 300 468 specification)
    """
    d = {'type': 'stream_identifier', 'component_tag': ord(data[(i + 2)])}
    return d
...
def parser_local_time_offset_Descriptor(data, i, length, end):
    ...
    return {'type': 'local_time_offset', 'country': data[i + 2:i + 5], 
       'region': e[0] >> 2, 
       'offset': offset, 
...
def parser_subtitling_Descriptor(data, i, length, end):
    ...
    j = i + 2
    while j < end:
        stype = _stream_component_mappings.get((3, ord(data[(j + 3)])), (ord(data[(j + 3)]), ''))
        parts.append({'language_code': data[j:j + 3], 'subtitling_type': stype, 
           'composition_page_id': (ord(data[(j + 4)]) << 8) + ord(data[(j + 5)]), 
           'ancilliary_page_id': (ord(data[(j + 6)]) << 8) + ord(data[(j + 7)])})
        j += 8

    d = {'type': 'subtitling', 'entries': parts}
    return d
...
def parser_private_data_specifier_Descriptor(data, i, length, end):
    ...
    d = {'type': 'private_data_specifier', 'private_data_specifier': _private_data_specifiers.get(n, n)}
    return d
...
def parser_frequency_list_Descriptor(data, i, length, end):
    ...
    (Defined in ETSI EN 300 468 specification)
    """
    d = {'type': 'frequency_list', 'frequencies': []}
    coding_type = ord(data[(i + 2)]) & 3
    i = i + 3
    while i < end:
        e = [ ord(data[x]) for x in range(i, i + 4) ]
        freq = None
        if coding_type == 1:
            freq = 10000000000 * unBCD(e[0]) + 100000000 * unBCD(e[1]) + 1000000 * unBCD(e[2]) + 10000 * unBCD(e[3])
        elif coding_type == 2:
            freq = 100000000 * unBCD(e[0]) + 1000000 * unBCD(e[1]) + 10000 * unBCD(e[2]) + 100 * unBCD(e[3])
        elif coding_type == 3:
...
def parser_data_broadcast_id_Descriptor(data, i, length, end):
    ...
    """
    d = {'type': 'data_broadcast_id', 'id': (ord(data[(i + 2)]) << 8) + ord(data[(i + 3)]), 
...
def parser_logical_channel_Descriptor(data, i, length, end):
    ...
    while i < end:
        ...
        logical_channel_number = (ord(data[(i + 2)]) << 8) + ord(data[(i + 3)]) & 1023
...
def parse_default_authority_Descriptor(data, i, length, end):
    ...
    (Defined in ETSI TS 102 323 specification)
    """
    return {'type': 'default_authority', 'authority': data[i + 2:end]}
...
def parse_content_identifier_Descriptor(data, i, length, end):
    ...
    return {'type': 'content_identifier', 'crids': crids}


__core_descriptor_parsers = {2: parser_video_stream_Descriptor, 
   3: parser_audio_stream_Descriptor, 
   4: parser_hierarchy_Descriptor, 
   5: parser_registration_Descriptor, 
   6: parser_data_stream_alignment_Descriptor, 
   7: parser_target_background_grid_Descriptor, 
   8: parser_video_window_Descriptor, 
   9: parser_CA_Descriptor, 
   10: parser_ISO_639_Descriptor, 
   11: parser_system_clock_Descriptor, 
   12: parser_multiplex_buffer_utilisation_Descriptor, 
   13: parser_copyright_Descriptor, 
   14: parser_maximum_bitrate_Descriptor, 
   15: parser_private_data_indicator_Descriptor, 
   16: parser_smoothing_buffer_Descriptor, 
   17: parser_STD_Descriptor, 
   18: parser_IBP_Descriptor, 
   36: parser_content_labelling_Descriptor, 
   37: parser_metadata_pointer_Descriptor, 
   38: parser_metadata_Descriptor, 
   64: parser_network_name_Descriptor, 
   65: parser_service_list_Descriptor, 
   66: parser_stuffing_Descriptor, 
   67: parser_satellite_delivery_system_Descriptor, 
   68: parser_cable_delivery_system_Descriptor, 
   69: parser_VBI_data_Descriptor, 
   70: parser_VBI_teletext_Descriptor, 
   71: parser_bouquet_name_Descriptor, 
   72: parser_service_Descriptor, 
   73: parser_country_availability_Descriptor, 
   74: parser_linkage_Descriptor, 
   75: parser_NVOD_reference_Descriptor, 
   76: parser_time_shifted_service_Descriptor, 
   77: parser_short_event_Descriptor, 
   78: parser_extended_event_Descriptor, 
   79: parser_time_shifted_event_Descriptor, 
   80: parser_component_Descriptor, 
   81: parser_mosaic_Descriptor, 
   82: parser_stream_identifier_Descriptor, 
   83: parser_CA_identifier_Descriptor, 
   84: parser_content_Descriptor, 
   85: parser_parental_rating_Descriptor, 
   86: parser_teletext_Descriptor, 
   87: parser_telephone_Descriptor, 
   88: parser_local_time_offset_Descriptor, 
   89: parser_subtitling_Descriptor, 
   90: parser_terrestrial_delivery_system_Descriptor, 
   91: parser_multilingual_network_name_Descriptor, 
   92: parser_multilingual_bouquet_name_Descriptor, 
   93: parser_multilingual_service_name_Descriptor, 
   94: parser_multilingual_component_Descriptor, 
   95: parser_private_data_specifier_Descriptor, 
   96: parser_service_move_Descriptor, 
   97: parser_short_smoothing_buffer_Descriptor, 
   98: parser_frequency_list_Descriptor, 
   99: parser_partial_transport_stream_Descriptor, 
   100: parser_data_broadcast_Descriptor, 
   101: parser_CA_system_Descriptor, 
   102: parser_data_broadcast_id_Descriptor, 
   103: parser_transport_stream_Descriptor, 
   104: parser_DSNG_Descriptor, 
   105: parser_PDC_Descriptor, 
   106: parser_AC3_Descriptor, 
   107: parser_ancillary_data_Descriptor, 
   108: parser_cell_list_Descriptor, 
   109: parser_cell_frequency_link_Descriptor, 
   110: parser_announcement_support_Descriptor, 
   115: parse_default_authority_Descriptor, 
   116: parse_related_content_Descriptor, 
   118: parse_content_identifier_Descriptor, 
   131: parser_logical_channel_Descriptor, 
   132: parser_preferred_name_list_Descriptor, 
   133: parser_preferred_name_identifier_Descriptor, 
   134: parser_service_attribute_Descriptor, 
   135: parser_short_service_name_Descriptor}
__ts102323_override_descriptor_parsers = {64: parser_rar_over_dvb_stream_Descriptor, 
   65: parser_rar_over_ip_Descriptor, 
   66: parser_rnt_scan_Descriptor}

def unBCD(byte):
    return (byte >> 4) * 10 + (byte & 15)


_dvbt_bandwidths = {0: dvb3f.BANDWIDTH_8_MHZ, 
   1: dvb3f.BANDWIDTH_7_MHZ, 
   2: dvb3f.BANDWIDTH_6_MHZ}
_dvbt_constellations = {0: dvb3f.QPSK, 
   1: dvb3f.QAM_16, 
   2: dvb3f.QAM_64}
_dvbt_hierarchy = {0: dvb3f.HIERARCHY_NONE, 
   1: dvb3f.HIERARCHY_1, 
   2: dvb3f.HIERARCHY_2, 
   3: dvb3f.HIERARCHY_4}
_dvbt_code_rate_hp = {0: dvb3f.FEC_1_2, 
   1: dvb3f.FEC_2_3, 
   2: dvb3f.FEC_3_4, 
   3: dvb3f.FEC_5_6, 
   4: dvb3f.FEC_7_8}
_dvbt_code_rate_lp = _dvbt_code_rate_hp
_dvbt_guard_interval = {0: dvb3f.GUARD_INTERVAL_1_32, 
   1: dvb3f.GUARD_INTERVAL_1_16, 
   2: dvb3f.GUARD_INTERVAL_1_8, 
   3: dvb3f.GUARD_INTERVAL_1_4}
_dvbt_transmission_mode = {0: dvb3f.TRANSMISSION_MODE_2K, 
   1: dvb3f.TRANSMISSION_MODE_8K}
_service_types = {1: 'digital television service', 
   2: 'digital radio sound service', 
   3: 'Teletext service', 
   4: 'NVOD reference service', 
   5: 'NVOD time-shifted service', 
   6: 'mosaic service', 
   7: 'PAL coded signal', 
   8: 'SECAM coded signal', 
   9: 'D/D2-MAC', 
   10: 'FM Radio', 
   11: 'NTSC coded signal', 
   12: 'data broadcast service', 
   14: 'RCS Map', 
   15: 'RCS FLS', 
   16: 'DVB MHP service'}
_iso639_audiotypes = {0: '', 
   1: 'CLEAN', 
   2: 'HEARING IMPAIRED', 
   3: 'VISUAL IMPAIRED COMMENTARY'}
_stream_component_mappings = {(1, 1): ('video', '4:3 aspect ratio, 25 Hz'), 
   (1, 2): ('video', '16:9 aspect ratio with pan vectors, 25 Hz'), 
   (1, 3): ('video', '16:9 aspect ratio without pan vectors, 25 Hz'), 
   (1, 4): ('video', '> 16:9 aspect ratio, 25 Hz'), 
   (1, 5): ('video', '4:3 aspect ratio, 30 Hz'), 
   (1, 6): ('video', '16:9 aspect ratio with pan vectors, 30 Hz'), 
   (1, 7): ('video', '16:9 aspect ratio without pan vectors, 30 Hz'), 
   (1, 5): ('video', '> 16:9 aspect ratio, 30 Hz'), 
   (1, 9): ('high definition video', '4:3 aspect ratio, 25 Hz'), 
   (1, 10): ('high definition video', '16:9 aspect ratio with pan vectors, 25 Hz'), 
   (1, 11): ('high definition video', '16:9 aspect ratio without pan vectors, 25 Hz'), 
   (1, 12): ('high definition video', '> 16:9 aspect ratio, 25 Hz'), 
   (1, 13): ('high definition video', '4:3 aspect ratio, 30 Hz'), 
   (1, 14): ('high definition video', '16:9 aspect ratio with pan vectors, 30 Hz'), 
   (1, 15): ('high definition video', '16:9 aspect ratio without pan vec., 30 Hz'), 
   (1, 16): ('high definition video', '> 16:9 aspect ratio, 30 Hz'), 
   (2, 1): ('audio', 'single mono channel'), 
   (2, 2): ('audio', 'dual mono channel'), 
   (2, 3): ('audio', 'stereo (2 channel)'), 
   (2, 4): ('audio', 'multi-lingual, multi-channel'), 
   (2, 5): ('audio', 'surround sound'), 
   (2, 64): ('audio description for the visually impaired', ''), 
   (2, 65): ('audio for the hard of hearing', ''), 
   (3, 1): ('EBU Teletext subtitles', ''), 
   (3, 2): ('associated EBU Teletext', ''), 
   (3, 3): ('VBI data', ''), 
   (3, 16): ('DVB subtitles (normal)', 'with no monitor aspect ratio criticality'), 
   (3, 17): ('DVB subtitles (normal)', 'for display on 4:3 aspect ratio monitor'), 
   (3, 18): ('DVB subtitles (normal)', 'for display on 16:9 aspect ratio monitor'), 
   (3, 19): ('DVB subtitles (normal)', 'for display on 2.21:1 aspect ratio monitor'), 
   (3, 32): ('DVB subtitles (for the hard of hearing)', 'with nomonitor aspect ratio criticality'), 
   (3, 33): ('DVB subtitles (for the hard of hearing)', 'for display on 4:3 aspect ratiomonitor'), 
   (3, 34): ('DVB subtitles (for the hard of hearing)', 'for display on 16:9 aspect ratiomonitor'), 
   (3, 35): ('DVB subtitles (for the hard of hearing)', 'for display on 2.21:1 aspect ratiomonitor')}
_private_data_specifiers = {1: 'SES', 
   2: 'BSkyB 1', 
   3: 'BSkyB 2', 
   4: 'BSkyB 3', 
   190: 'BetaTechnik', 
   ...
   3547765: 'Irdeto', 
   5133388: 'NTL', 
   5451073: 'Scientific Atlanta', 
   1145130585: 'News Datacom (IL) 1', 
   1179796809: 'News Datacom (IL) 1', 
   1396789331: 'Scientific Atlanta'}
_content_types_level_1 = {1: 'Movie/Drama', 
   ...
   9: 'Childrens/Youth Education/Science/Factual', 
   10: 'Leisure hobbies', 
   11: 'Misc', 
   15: 'Drama'}
_content_types_level_2 = {16: 'General', 
   ...
   22: 'Romance', 
   23: 'Serious/ClassicalReligion/Historical', 
   24: 'Adult', 
   32: 'General', 
   ...
   51: 'Talk', 
   64: 'General', 
   65: 'Special Event (Olympics/World cup/...)', 
   66: 'Magazine', 
   67: 'Football/Soccer', 
   68: 'Tennis/Squash', 
   ...
   80: 'General', 
   81: 'Pre-school', 
   82: 'Entertainment (6 to 14 year-olds)', 
   83: 'Entertainment (10 to 16 year-olds)', 
   84: 'Informational/Educational/Schools', 
   85: 'Cartoons/Puppets', 
   96: 'General', 
   97: 'Rock/Pop', 
   ...
   114: 'Fine Arts', 
   115: 'Religion', 
   116: 'Popular Culture/Tradital Arts', 
   117: 'Literature', 
   118: 'Film/Cinema', 
   ...
   179: 'Live Broadcast', 
   240: 'General', 
   241: 'Detective/Thriller', 
   242: 'Adventure/Western/War', 
   243: 'Science Fiction/Fantasy/Horror', 
   244: 'Comedy', 
   245: 'Soap/Melodrama/Folkloric', 
   246: 'Romance', 
   247: 'Serious/ClassicalReligion/Historical', 
   248: 'Adult'}
_content_identifier_content_type = {1: 'instance', 
   2: 'part of series', 
   3: 'recommendation'}