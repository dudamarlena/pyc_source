...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/Device/DVB/Parse/ReassemblePSITables.py
                ...
                if nextCont == None and start_indicator or nextCont == contcount:
                    ...
                    while len(buffer) >= 1:
                        if buffer[0] == chr(255):
                            ...
                            break
                        if len(buffer) >= 3:
                            tlen = (ord(buffer[1]) << 8) + ord(buffer[2]) & 4095
...
class ReassemblePSITablesService(AdaptiveCommsComponent):
    ...
    """
    Inboxes = {'inbox': 'Incoming DVB TS packets', 'control': 'Shutdown signalling', 
    ...
    def main(self):
        while not self.shutdown():
            ...
            while self.dataReady('inbox'):
                ...
                pid = (ord(ts_packet[1]) << 8) + ord(ts_packet[2]) & 8191
                try:
...
if __name__ == '__main__':
    ...
    class Producer(component):
        ...
        def main(self):
            while 1:
                ...
                fakepacket = 'X' + chr(pid >> 8) + chr(pid & 255) + 'Y' * 185
    ...
    Subscriber(28, 1, 2, 3, 4, 5).activate()
    Subscriber(53, 1, 2, 3, 4, 5).activate()
