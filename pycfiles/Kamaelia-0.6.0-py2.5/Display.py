...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/UI/Pygame/Display.py
...
class _PygameEventSource(threadedcomponent):
    ...
    Outboxes = {'outbox': 'Wake up notifications - that there are pygame events waiting', 'signal': 'Not used'}
    ...
    def main(self):
        while 1:
            time.sleep(0.01)
            eventswaiting = pygame.event.peek()
            if eventswaiting:
                try:
...
class PygameDisplay(Axon.AdaptiveCommsComponent.AdaptiveCommsComponent):
   ...
   """
    Inboxes = {'inbox': 'Default inbox, not currently used', 'control': 'NOT USED', 
    ...
    def getDisplayService(tracker=None):
            ...
            if isinstance(message, Axon.Ipc.producerFinished):
                ...
                try:
                    ...
                    if surface is not None:
                        ...
                        if found:
                            (surface, position, callbackcomms, eventcomms, onlymouseinside) = self.surfaces[c]
                            new_position = message.get('position', position)
                            self.surfaces[c] = (surface, new_position, callbackcomms, eventcomms, onlymouseinside)
                except Exception, e:
                ...
                self.overlays.append({'overlay': overlay, 'yuv': yuvdata, 
                   'position': position, 
                   'size': size, 
    ...
    def updateDisplay(self, display):
      ...
      """
        ...
        for (surface, position, callbackcomms, eventcomms, onlymouseinside) in self.surfaces:
            ...
            for (surface, position, callbackcomms, eventcomms, onlymouseinside) in self.surfaces:
                if eventcomms is not None:
                    ...
                    for event in events:
                        wanted = False
                        try:
                            wanted = self.events_wanted[listener][event.type]
                        except KeyError:
                            pass
                        ...
                        if wanted:
                            if event.type in [pygame.MOUSEMOTION, pygame.MOUSEBUTTONUP, pygame.MOUSEBUTTONDOWN]:
                                ...
                                try:
                                    e.pos = (
                                     pos[0] - self.visibility[listener][2][0], pos[1] - self.visibility[listener][2][1])
...
if __name__ == '__main__':
    ...
    for _ in range(6):
        Pipeline(datasource(), TickTock()).activate()
