...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Kamaelia/Protocol/RTP/RTP.py
...
class RTPFramer(component):
    ...
    def main(self):
        ...
        self.timestamp_offset = random.randint(0, 4294967295)
        while not shutdown:
            while self.dataReady('inbox'):
    ...
    def constructPacket(self, content):
        packet = []
        padding = content.get('bytespadding', 0)
        extension = content.get('extension', None)
        csrcs = content.get('csrcs', [])
        ...
        byte = payloadtype & 127
        if marker:
            byte = byte + 128
        packet.append(chr(byte))
        packet.append(struct.pack('>H', self.seqnum))
        self.seqnum = self.seqnum + 1 & 65535
        packet.append(struct.pack('>I', timestamp + self.timestamp_offset & 4294967295))
        packet.append(struct.pack('>I', ssrc & 4294967295))
        for csrc in csrcs:
            packet.append(struct.pack('>I', csrc & 4294967295))

        if extension:
            (ehdr, epayload) = extension
            packet.append(ehdr[0:2])
            packet.append(struct.pack('>H', len(epayload)))
            packet.append(epayload)
        packet.append(payload)
        if padding > 0:
            packet.append('\x00' * (padding - 1) + chr(padding))
        packet = ('').join(packet)
        return packet
