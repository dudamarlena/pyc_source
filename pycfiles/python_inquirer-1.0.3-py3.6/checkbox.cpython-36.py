...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/python_inquirer/prompts/checkbox.py
...
class InquirerControl(TokenListControl):
    ...
    def get_selected_values(self):
        return [c[1] for c in self.choices if not isinstance(c, Separator) if c[1] in self.selected_options]
...
def question(message, **kwargs):
    ...
    if 'default' in kwargs:
        raise ValueError("Checkbox does not implement 'default' use 'checked':True' in choice!")
    ...
    def get_prompt_tokens(cli):
        ...
        if ic.answered:
            ...
            tokens.append((Token.Instruction,
             ' (<up>, <down> to move, <space> to select, <a> to toggle, <i> to invert)'))
        return tokens

    layout = HSplit([
     Window(height=(D.exact(1)), content=TokenListControl(get_prompt_tokens, align_center=False)),
    ...
    def toggle(event):
        pointed_choice = ic.choices[ic.pointer_index][1]
    ...
    def invert(event):
        inverted_selection = [c[1] for c in ic.choices if not isinstance(c, Separator) if c[1] not in ic.selected_options if not c[2]]
    ...
    def move_cursor_down(event):
        ...
        _next()
        while isinstance(ic.choices[ic.pointer_index], Separator) or ic.choices[ic.pointer_index][2]:
            _next()
