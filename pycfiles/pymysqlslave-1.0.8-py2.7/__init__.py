...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pymysqlslave/__init__.py
# Compiled at: 2015-12-25 08:26:35
__version__ = '1.0.8'
...
class MySQLDBSlave(object):
    ...
    def __init__(self, masters, slaves=None, is_auto_allocation=False, reconnect_retry_nums=3, is_reconnect=True):
        ...
        if all([all_masters, all_slaves]):
            ...
            raise MySQLOperationalError('`MySQLDBSlave` instantiationrequires parameters masters or slaves values are not empty')
            ...
            def _wrap(*args, **kwargs):
                ...
                for i in xrange(retry + 1):
                    try:
                        ...
                        _logger.debug((('Retry:{} with_reconnect:{}').format(i + 1, f_val), 'mysqldb reconnect', e))
                ...
                raise MySQLOperationalError(('mysqldb_reconnect:{} *retry:{}*. But MySQL server has gone away').format(f_val, retry))
