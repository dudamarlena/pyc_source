...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/xasm/assemble.py
# Compiled at: 2020-04-17 19:02:38
...
class Assembler(object):
    ...
    def code_init(self, python_version=None):
        if self.python_version is None and python_version:
            self.python_version = python_version
        self.code = xdis.codetype.to_portable(co_argcount=0, co_posonlyargcount=0, co_kwonlyargcount=0, co_nlocals=0, co_stacksize=10, co_flags=0, co_code=[], co_consts=[], co_names=[], co_varnames=[], co_filename='unknown', co_name='unknown', co_firstlineno=1, co_lnotab={}, co_freevars=[], co_cellvars=[])
    ...
    while i < len(lines):
        ...
        if line.startswith('#'):
            if line.startswith('# Python bytecode '):
                ...
                while i < len(lines):
                    ...
                    if match:
                        ...
                        assert index == count, "Constant index {%d} found on line {%d}doesn't match expected constant index {%d}." % (
                         index, i, count)
                        expr = match.group(2)
            ...
            if match:
                ...
                continue
            if not bytecode_seen:
                raise AssertionError('File needs to start out with: # Python bytecode <version>')
                ...
                if opname in asm.opc.opname:
                    ...
                    if inst.opcode in asm.opc.JUMP_OPS:
                        is_int(operand) or backpatch_inst.add(inst)
        ...
        if xdis.op_has_argument(inst.opcode, asm.opc):
            ...
            if asm.opc.version < 3.6:
                if inst.opcode == asm.opc.EXTENDED_ARG:
                    ...
                    extended_value = 0
                bcode += arg_tup
            elif inst.opcode == asm.opc.EXTENDED_ARG:
