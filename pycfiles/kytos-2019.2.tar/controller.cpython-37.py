...
# [GCC 8.4.0]
# Embedded file name: /Users/humberto/src/kytos/kytos-asyncio/kytos/core/controller.py
# Compiled at: 2019-12-13 14:08:03
# Size of source mod 2**32: 32809 bytes
...
.. code-block:: python3
    ...
    controller.start()
"""
import asyncio, atexit, json, logging, os, re, sys, threading
...
class Controller:
    ...
    def __init__(self, options=None, loop=None):
        ...
        self.api_server = APIServer(__name__, self.options.listen, self.options.api_port, self.napps_manager, self.options.napps)
        self.auth = Auth(self)
    ...
    def loggers():
        ...
        """
        return [logging.getLogger(name) for name in logging.root.manager.loggerDict if 'kytos' in name]
    ...
    def create_pidfile(self):
        ...
        try:
            ...
            try:
                ...
                os.kill(old_pid, 0)
                error_msg = 'PID file {} exists. Delete it if Kytos is not running. Aborting.'
    ...
    def start_controller(self):
        ...
        self.server = KytosServer((self.options.listen,
         int(self.options.port)), KytosServerProtocol, self, self.options.protocol_name)
        ...
        def _stop_loop(_):
            ...
            self.log.debug('%s threads before loop.stop: %s', len(threads), threads)
            loop.stop()
        ...
        async def _run_api_server_thread(executor):
            ...
            log.debug('completed: %d, pending: %d', len(completed), len(pending))
    ...
    def _register_endpoints(self):
        ...
        self.api_server.register_core_endpoint('config/', self.configuration_endpoint)
        self.api_server.register_core_endpoint('metadata/', Controller.metadata_endpoint)
        self.api_server.register_core_endpoint('reload/<username>/<napp_name>/', self.rest_reload_napp)
        self.api_server.register_core_endpoint('reload/all', self.rest_reload_all_napps)
    ...
    def stop_controller(self, graceful=True):
        ...
        threads = threading.enumerate()
        self.log.debug('%s threads before threadpool shutdown: %s', len(threads), threads)
    ...
    async def msg_out_event_handler(self):
        ...
        while True:
            ...
            if destination:
                packet = destination.state == ConnectionState.FINISHED or message.pack()
                destination.send(packet)
                self.log.debug('Connection %s: OUT OFP, version: %s, type: %s, xid: %s - %s', destination.id, message.header.version, message.header.message_type, message.header.xid, packet.hex())
        ...
        if vlan_pool.get(dpid):
            ...
            for intf_num, port_list in vlan_pool[dpid].items():
                if not switch.interfaces.get(intf_num):
                    ...
                    intf = Interface(name=intf_num, port_number=intf_num, switch=switch)
    ...
    def load_napp(self, username, napp_name):
        ...
        """
        if (
         username, napp_name) in self.napps:
        ...
        try:
            ...
            try:
                self.log.error("Error loading NApp '%s/%s': %s", username, napp_name, err)
                return
            finally:
                err = None
                del err
        ...
        try:
            ...
            self.log.critical('NApp initialization failed: %s/%s', username,
              napp_name, exc_info=True)
            return
        else:
    ...
    def load_napps(self):
        ...
        for napp in self.napps_manager.get_enabled_napps():
            try:
                ...
                try:
                    self.log.error('Could not load NApp %s: %s', napp.id, exception)
                finally:
                    exception = None
                    del exception
    ...
    def unload_napps(self):
        ...
        for username, napp_name in list(self.napps.keys()):
    ...
    def reload_napp_module(self, username, napp_name, napp_file):
        ...
        try:
            ...
            try:
                self.log.error("Error reloading NApp '%s/%s': %s", username, napp_name, err)
                raise
            finally:
                err = None
                del err
    ...
    def reload_napp(self, username, napp_name):
        ...
        try:
            ...
            self.log.info("NApp '%s/%s' successfully reloaded", username, napp_name)
