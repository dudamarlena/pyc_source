...
# [GCC 8.4.0]
# Embedded file name: /Users/humberto/src/kytos/kytos-asyncio/kytos/core/napps/base.py
# Compiled at: 2019-06-06 13:59:39
# Size of source mod 2**32: 9700 bytes
"""Kytos Napps Module."""
import json, os, re, sys, tarfile, urllib
...
class NApp:
    __doc__ = 'Class to represent a NApp.'

    def __init__(self, username=None, name=None, version=None, repository=None, meta=False):
    ...
    def create_from_uri(cls, uri):
        ...
        if not match:
            ...
            return cls(username=(match.groups()[4]), name=(match.groups()[5]),
    ...
    def _extract(filename):
        ...
        """
        random_string = '{:0d}'.format(randint(0, 1000000))
...
class KytosNApp(Thread, metaclass=ABCMeta):
    __doc__ = 'Base class for any KytosNApp to be developed.'
    ...
    def __init__(self, controller, **kwargs):
        ...
        self._KytosNApp__event = Event()
        self._KytosNApp__interval = -1
        self.setup()
        handler_methods = [getattr(self, method_name) for method_name in dir(self) if method_name[0] != '_' if callable(getattr(self, method_name)) if hasattr(getattr(self, method_name), 'events')]
    ...
    def execute_as_loop(self, interval):
        ...
        """
        self._KytosNApp__interval = interval
    ...
    def run(self):
        ...
        self.execute()
        while self._KytosNApp__interval > 0 and not self._KytosNApp__event.is_set():
            self._KytosNApp__event.wait(self._KytosNApp__interval)
            self.execute()
    ...
    def _shutdown_handler(self, event):
        ...
        """
        if not self._KytosNApp__event.is_set():
            self._KytosNApp__event.set()
            self.shutdown()
