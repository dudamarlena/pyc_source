...
# [GCC 8.4.0]
# Embedded file name: /Users/humberto/src/kytos/kytos-asyncio/kytos/core/api_server.py
# Compiled at: 2019-12-09 19:26:29
# Size of source mod 2**32: 18409 bytes
"""Module used to handle a API Server."""
import json, logging, os, shutil, sys, warnings, zipfile
...
class APIServer:
    __doc__ = 'Api server used to provide Kytos Controller routes.'
    DEFAULT_METHODS = ('GET', )
    _NAPP_PREFIX = '/api/{napp.username}/{napp.name}/'
    _CORE_PREFIX = '/api/kytos/core/'

    def __init__(self, app_name, listen='0.0.0.0', port=8181, napps_manager=None, napps_dir=None):
        ...
        self.listen = listen
        self.port = port
        self.app = Flask(app_name, root_path=(self.flask_dir), static_folder='dist',
    ...
    def register_rest_endpoint(self, url, function, methods):
        ...
        warnings.warn('From now on, use @rest decorator.', DeprecationWarning, stacklevel=2)
        if url.startswith('/'):
            url = url[1:]
    ...
    def start_api(self):
        ...
        self.register_core_endpoint('web/update/<version>/', (self.update_web_ui),
          methods=[
         'POST'])
        self.register_core_endpoint('web/update/', (self.update_web_ui),
          methods=[
         'POST'])
    ...
    def _register_web_ui(self):
        ...
        self.app.add_url_rule('/ui/<username>/<napp_name>/<path:filename>', self.static_web_ui.__name__, self.static_web_ui)
        self.app.add_url_rule('/ui/<path:section_name>', self.get_ui_components.__name__, self.get_ui_components)

    @staticmethod
    def status_api():
        """Display kytos status using the route ``/kytos/status/``."""
        return (
         '{"response": "running"}', HTTPStatus.CREATED.value)
    ...
    def shutdown_api(self):
        ...
        self.server.stop()
        return (
         'Server shutting down...', HTTPStatus.OK.value)
    ...
    def _get_decorated_functions(napp):
        ...
        for name in dir(napp):
            pub_attr = name.startswith('_') or getattr(napp, name)
    ...
    def _start_endpoint(self, rule, function, **options):
        ...
        self.log.info('Started %s - %s', rule, ', '.join(options.get('methods', self.DEFAULT_METHODS)))
    ...
    def register_core_napp_services(self):
        ...
        """
        self.register_core_endpoint('napps/<username>/<napp_name>/enable', self._enable_napp)
        self.register_core_endpoint('napps/<username>/<napp_name>/disable', self._disable_napp)
        self.register_core_endpoint('napps/<username>/<napp_name>/install', self._install_napp)
        self.register_core_endpoint('napps/<username>/<napp_name>/uninstall', self._uninstall_napp)
        self.register_core_endpoint('napps_enabled', self._list_enabled_napps)
        self.register_core_endpoint('napps_installed', self._list_installed_napps)
        self.register_core_endpoint('napps/<username>/<napp_name>/metadata/<key>', self._get_napp_metadata)
    ...
    def _enable_napp(self, username, napp_name):
        ...
        if not self.napps_manager.is_installed(username, napp_name):
            ...
            return self.napps_manager.is_enabled(username, napp_name) or (
             '{"response": "error"}',
    ...
    def _install_napp(self, username, napp_name):
        if self.napps_manager.is_installed(username, napp_name):
            return (
             '{"response": "installed"}', HTTPStatus.OK.value)
        else:
            napp = '{}/{}'.format(username, napp_name)
            return self.napps_manager.install(napp, enable=True) or (
             '{"response": "error"}',
             HTTPStatus.INTERNAL_SERVER_ERROR.value)
        ...
        if self.napps_manager.is_installed(username, napp_name):
            if not self.napps_manager.uninstall(username, napp_name):
                return ('{"response": "error"}',
                 HTTPStatus.INTERNAL_SERVER_ERROR.value)
        return (
         '{"response": "uninstalled"}', HTTPStatus.OK.value)

    def _list_enabled_napps(self):
        """Sorted list of (username, napp_name) of enabled napps."""
        serialized_dict = json.dumps((self.napps_manager.get_enabled_napps()),
          default=(lambda a: [
         a.username, a.name]))
        return (
         '{"napps": %s}' % serialized_dict, HTTPStatus.OK.value)

    def _list_installed_napps(self):
        """Sorted list of (username, napp_name) of installed napps."""
        serialized_dict = json.dumps((self.napps_manager.get_installed_napps()),
          default=(lambda a: [
         a.username, a.name]))
        return (
         '{"napps": %s}' % serialized_dict, HTTPStatus.OK.value)
    ...
    def _get_napp_metadata(self, username, napp_name, key):
        ...
        if not self.napps_manager.is_installed(username, napp_name):
            return (
             'NApp is not installed.', HTTPStatus.BAD_REQUEST.value)
        if key not in valid_keys:
            return (
             'Invalid key.', HTTPStatus.BAD_REQUEST.value)
        data = self.napps_manager.get_napp_metadata(username, napp_name, key)
        serialized_dict = json.dumps({key: data})
        return (
         '%s' % serialized_dict, HTTPStatus.OK.value)