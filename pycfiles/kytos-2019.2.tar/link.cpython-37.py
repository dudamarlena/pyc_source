...
# [GCC 8.4.0]
# Embedded file name: /Users/humberto/src/kytos/kytos-asyncio/kytos/core/link.py
...
class Link(GenericEntity):
    __doc__ = 'Define a link between two Endpoints.'
    ...
    def is_enabled(self):
        ...
        """
        return self._enabled and self.endpoint_a.is_enabled() and self.endpoint_b.is_enabled()
    ...
    def is_active(self):
        ...
        """
        return self._active and self.endpoint_a.is_active() and self.endpoint_b.is_active()
    ...
    def available_tags(self):
        ...
        """
        return [tag for tag in self.endpoint_a.available_tags if tag in self.endpoint_b.available_tags]
    ...
    def is_tag_available(self, tag):
        ...
        return self.endpoint_a.is_tag_available(tag) and self.endpoint_b.is_tag_available(tag)
        ...
        for tag in available_tags_a:
            if tag not in available_tags_b:
                ...
                if not self.endpoint_a.use_tag(tag):
                    continue
                self.endpoint_b.use_tag(tag) or self.endpoint_a.make_tag_available(tag)
                continue
            return tag
