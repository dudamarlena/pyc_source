...
# [GCC 8.4.0]
# Embedded file name: /Users/humberto/src/kytos/kytos-asyncio/kytos/core/logs.py
...
class LogManager:
    __doc__ = 'Manage handlers for all loggers.'
    ...
    def load_config_file(cls, config_file, debug='False'):
        ...
        if Path(config_file).exists():
            ...
            LOG.warning('Log config file "%s" does not exist. Using default Python logging configuration.', config_file)
    ...
    def _use_config_file(cls, config_file):
        ...
        try:
            ...
            LOG.info('Logging config file "%s" loaded successfully.', config_file)
        except OSError:
    ...
    def _catch_config_file_exception(cls, config_file):
        ...
        if 'handler_syslog' in cls._PARSER:
            LOG.warning('Failed to load "%s". Trying to disable syslog handler.', config_file)
            cls._PARSER.remove_section('handler_syslog')
            cls._use_config_file(config_file)
        else:
            LOG.warning('Failed to load "%s". Using default Python logging configuration.', config_file)
    ...
    def add_handler(cls, handler):
        ...
        if cls._PARSER.has_section(cls._DEFAULT_FMT):
            ...
            fmt = Formatter(fmt_conf.get('format', None), fmt_conf.get('datefmt', None))
    ...
    def filter_session_disconnected(record):
        ...
        return not (record.name == 'werkzeug' and record.levelname == 'ERROR' and record.args and isinstance(record.args[0], str) and record.args[0].endswith(msg_end))
...
def _detect_napp_id():
    ...
    for frame in inspect.stack():
        match = frame.filename == __file__ or NAPP_ID_RE.match(frame.filename)
        if match:
