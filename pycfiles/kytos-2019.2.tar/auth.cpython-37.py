...
# [GCC 8.4.0]
# Embedded file name: /Users/humberto/src/kytos/kytos-asyncio/kytos/core/auth.py
# Compiled at: 2019-12-20 12:11:09
# Size of source mod 2**32: 11517 bytes
"""Module with main classes related to Authentication."""
import datetime, getpass, hashlib, logging, time
...
def authenticated(func):
    ...
    def wrapper(*args, **kwargs):
        try:
         ...
         jwt.exceptions.DecodeError) as exc:
            try:
...
class Auth:
    __doc__ = 'Module used to provide Kytos authentication routes.'
    ...
    def register_core_auth_services(self):
        ...
        update users.
        """
        self.controller.api_server.register_core_endpoint('auth/login/', self._authenticate_user)
        self.controller.api_server.register_core_endpoint('auth/users/', self._list_users)
        self.controller.api_server.register_core_endpoint('auth/users/<uid>', self._list_user)
        self.controller.api_server.register_core_endpoint('auth/users/',
          (self._create_user), methods=['POST'])
        self.controller.api_server.register_core_endpoint('auth/users/<uid>',
          (self._delete_user), methods=['DELETE'])
        self.controller.api_server.register_core_endpoint('auth/users/<uid>',
          (self._update_user), methods=['PATCH'])
    ...
    def _authenticate_user(self):
        ...
        try:
            ...
            if user.get('password') != hashlib.sha512(password).hexdigest():
                raise KeyError
            time_exp = datetime.datetime.utcnow() + datetime.timedelta(minutes=TOKEN_EXPIRATION_MINUTES)
    ...
    def _find_user(self, uid):
        ...
        def _find_user_callback(_event, box, error):
            nonlocal response
            if not box:
                response = {'answer':f"User with uid {uid} not found",  'code':HTTPStatus.NOT_FOUND.value}
            else:
                if error:
                    response = {'answer':'User data cannot be shown',  'code':HTTPStatus.INTERNAL_SERVER_ERROR.value}
                else:
                    response = {'answer':{'data': box.data},  'code':HTTPStatus.OK.value}
    ...
    def _list_users(self):
        ...
        def _list_users_callback(_event, boxes, error):
            nonlocal response
            if error:
                response = {'answer':'Users cannot be listed',  'code':HTTPStatus.INTERNAL_SERVER_ERROR.value}
            else:
                response = {'answer':{'users': boxes},  'code':HTTPStatus.OK.value}
    ...
    def _create_user(self):
        ...
        def _create_user_callback(_event, box, error):
            nonlocal response
            if not box:
                response = {'answer':'User already exists',  'code':HTTPStatus.CONFLICT.value}
            else:
                if error:
                    response = {'answer':'User has not been created',  'code':HTTPStatus.INTERNAL_SERVER_ERROR.value}
                else:
                    response = {'answer':'User successfully created',  'code':HTTPStatus.OK.value}
    ...
    def _delete_user(self, uid):
        ...
        def _delete_user_callback(_event, box, error):
            ...
            if not box:
                ...
                if error:
                    response = {'answer':'User has not been deleted',  'code':HTTPStatus.INTERNAL_SERVER_ERROR.value}
                else:
                    response = {'answer':'User successfully deleted',  'code':HTTPStatus.OK.value}
    ...
    def _update_user(self, uid):
        ...
        def _update_user_callback(_event, box, error):
            ...
            if not box:
                ...
                if error:
                    response = {'answer':'User has not been updated',  'code':HTTPStatus.INTERNAL_SERVER_ERROR.value}
                else:
                    response = {'answer':'User successfully updated',  'code':HTTPStatus.OK.value}
        ...
        content = {'namespace':self.namespace,  'box_id':uid, 
         'data':data, 
