...
def make_lemma_to_forms():
    for root in get_data(DATA_DIRECTORY, parser):
        if root is not None:
            ...
            lemma_to_normalized = {lemma:{token['norm'] for token in tokens if token['lemma'] == lemma} for lemma in lemmata_set}
...
def save_lemma_to_forms(lemma_to_forms):
    ...
    words = sorted(words)
    with codecs.open('lemma_to_forms.csv', 'w', encoding='utf-8') as (f):
...
def save_lemmatizer(lemmatizers):
    ...
    words = sorted(words)
    with codecs.open('lemmatizer.csv', 'w', encoding='utf-8') as (f):
