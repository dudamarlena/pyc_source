...
"""
import codecs, os, collections, pickle
...
def make_norm_to_pos_tagger():
    for root in get_data(DATA_DIRECTORY, parser):
        if root is not None:
            ...
            norm_set = set(norm)
            pos_to_norm = {norm:{token['pos'] for token in tokens if token['norm'] == norm} for norm in norm_set}
            yield pos_to_norm
...
def make_lemma_to_pos_tagger():
    for root in get_data(DATA_DIRECTORY, parser):
        if root is not None:
            ...
            lemmata_to_pos = {lemma:{token['pos'] for token in tokens if token['lemma'] == lemma} for lemma in lemmata_set}
...
def make_pos_tagger_to_lemma():
    for root in get_data(DATA_DIRECTORY, parser):
        if root is not None:
            ...
            pos_tags = extract_by_tag('pos', tokens)
            pos_set = set(pos_tags)
            pos_to_lemmata = {pos:{token['lemma'] for token in tokens if token['pos'] == pos} for pos in pos_set}
...
def save_norm_to_pos_tagger(norm_to_pos_taggers):
    ...
    words = sorted(words)
    with codecs.open('norm_pos.csv', 'w', encoding='utf-8') as (f):
...
def save_lemmata(lemmatas):
    ...
    words = sorted(words)
    with codecs.open('lemmata_pos.csv', 'w', encoding='utf-8') as (f):
...
def save_pos_tagger(pos_taggers):
    ...
    poss = sorted(poss)
    with codecs.open('pos_lemmata.csv', 'w', encoding='utf-8') as (f):
