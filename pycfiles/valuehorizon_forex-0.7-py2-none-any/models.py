...
# [GCC 8.4.0]
# Embedded file name: /home/kevin/Development/customapps/valuehorizon-forex/forex/models.py
# Compiled at: 2016-06-02 16:56:19
...
class Currency(models.Model):
    ...
    def __unicode__(self):
        return '%s, %s' % (unicode(self.name), unicode(self.symbol))
...
class CurrencyPriceManager(Manager):
    ...
    def generate_dataframe(self, symbols=None, date_index=None, price_type='mid'):
        ...
        currency_price_data = CurrencyPrice.objects.filter(currency__symbol__in=symbols, date__gte=date_index[0], date__lte=date_index[(-1)]).values_list('date', 'currency__symbol', 'ask_price', 'bid_price')
        try:
...
class CurrencyPrice(models.Model):
    ...
    ask_price = models.DecimalField(max_digits=20, decimal_places=PRICE_PRECISION, validators=[
     MinValueValidator(Decimal('0.00'))])
    bid_price = models.DecimalField(max_digits=20, decimal_places=PRICE_PRECISION, validators=[
     MinValueValidator(Decimal('0.00'))])
    date_modified = models.DateTimeField(null=True, blank=True, editable=False, auto_now=True)
    date_created = models.DateTimeField(null=True, blank=True, editable=False, auto_now_add=True)
    ...
    def __unicode__(self):
        return '%s, %s' % (unicode(self.currency),
         unicode(self.date))
