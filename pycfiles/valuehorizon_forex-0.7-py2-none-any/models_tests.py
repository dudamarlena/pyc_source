...
# [GCC 8.4.0]
# Embedded file name: /home/kevin/Development/customapps/valuehorizon-forex/forex/tests/models_tests.py
# Compiled at: 2016-06-02 17:10:36
...
class CurrencyModelTests(TestCase):
    ...
    def setUp(self):
        ...
        CurrencyPrice.objects.create(currency=test_curr1, date=date(2015, 1, 3), ask_price=6, bid_price=5)
        CurrencyPrice.objects.create(currency=test_curr1, date=date(2015, 1, 5), ask_price=8, bid_price=7)

    def field_tests(self):
        required_fields = [
         'id', 'name', 'symbol', 'ascii_symbol', 'num_code', 'digits', 'description', 'date_created', 'date_modified']
...
class CurrencyPriceModelTests(TestCase):
    ...
    def field_tests(self):
        required_fields = [
         'id', 'currency', 'date', 'ask_price', 'bid_price', 'date_created', 'date_modified']
    ...
    def test_spread(self):
        ...
        price.bid_price = 3
        price.save()
        self.assertEqual(price.spread, 1)
...
class CurrencyPriceDataFrame(TestCase):
    ...
    def test_correct_rate_input(self):
        self.assertRaisesMessage(ValueError, "Incorrect price_type (*BAD*) must be on of 'ask', 'bid' or 'mid'", CurrencyPrice.objects.generate_dataframe, symbols=None, date_index=None, price_type='*BAD*')
        return
...
class ComputeReturnTests(TestCase):
    ...
    def test_bad_input_rate(self):
        test_curr1 = Currency.objects.get(symbol='TEST1')
        self.assertRaisesMessage(ValueError, "Unknown rate type (*BAD*)- must be 'MID', 'ASK' or 'BID'", test_curr1.compute_return, date(2015, 1, 2), date(2015, 1, 5), rate='*BAD*')

    def test_bad_input_dates(self):
        test_curr1 = Currency.objects.get(symbol='TEST1')
        self.assertRaisesMessage(ValueError, 'End date must be on or after start date', test_curr1.compute_return, date(2015, 1, 10), date(2015, 1, 5), rate='MID')

    def test_computation(self):
        test_curr1 = Currency.objects.get(symbol='TEST1')
        self.assertEqual(test_curr1.compute_return(start_date=date(2015, 1, 1), end_date=date(2015, 1, 15), rate='MID'), 7.0 / 3 - 1)
        self.assertEqual(test_curr1.compute_return(start_date=date(2015, 1, 1), end_date=date(2015, 1, 15)), 7.0 / 3 - 1)
...
class ConvertCurrencyTests(TestCase):
    ...
    def setUp(self):
        ...
        Currency.objects.create(name='Test Dollar2', symbol='TEST2')
        test_curr2 = Currency.objects.get(symbol='TEST2')
        CurrencyPrice.objects.create(currency=test_curr2, date=date(2015, 1, 1), ask_price=8, bid_price=6)
        Currency.objects.create(name='Test Dollar3', symbol='TEST3')
        test_curr2 = Currency.objects.get(symbol='TEST3')
        CurrencyPrice.objects.create(currency=test_curr2, date=date(2016, 1, 1), ask_price=10, bid_price=9)
