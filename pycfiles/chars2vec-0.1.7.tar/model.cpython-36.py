...
# [GCC 8.4.0]
# Embedded file name: /Users/vladimir/PycharmProjects/chars2vec/chars2vec/model.py
# Compiled at: 2019-03-14 19:06:03
# Size of source mod 2**32: 8758 bytes
import numpy as np, pickle, keras, os
...
class Chars2Vec:
    ...
    def fit(self, word_pairs, targets, max_epochs, patience, validation_split, batch_size):
        """
        Fits model.
        ...
        if not isinstance(word_pairs, list):
            if not isinstance(word_pairs, np.ndarray):
        ...
        self.model.fit([x_1_pad_seq, x_2_pad_seq], targets, batch_size=batch_size,
          epochs=max_epochs,
...
def train_model(emb_dim, X_train, y_train, model_chars, max_epochs=200, patience=10, validation_split=0.05, batch_size=64):
    """
    ...
    if not isinstance(X_train, list):
        ...
        if not isinstance(model_chars, list):
            if not isinstance(model_chars, np.ndarray):
