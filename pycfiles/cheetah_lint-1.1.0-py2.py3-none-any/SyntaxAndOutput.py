...
# Embedded file name: build/lib.macosx-10.13-x86_64-2.7/Cheetah/Tests/SyntaxAndOutput.py
# Compiled at: 2019-09-22 10:12:27
__doc__ = '\nSyntax and Output tests.\n\nTODO\n- #finally\n- #filter\n- #errorCatcher\n- #echo\n- #silent\n'
import os, os.path, sys, unittest, warnings
...
defaultTestNameSpace = {'aStr': 'blarg', 
   'anInt': 1, 
   'aFloat': 1.5, 
   'aList': [
           'item0', 'item1', 'item2'], 
   ...
   'list': [{'index': 0, 'numOne': 1, 'numTwo': 2}, {'index': 1, 'numOne': 1, 'numTwo': 2}], 'nameList': [
              ('john', 'doe'), ('jane', 'smith')], 
   'letterList': [
                'a', 'b', 'c'], 
...
class OutputTest(unittest.TestCase):
    ...
    def verify(self, input, expectedOutput, inputEncoding=None, outputEncoding=None, convertEOLs=Unspecified):
        ...
        if self._useNewStyleCompilation:
            ...
            templateClass = Template.compile(source=input, compilerSettings=self._getCompilerSettings(), keepRefToGeneratedCode=True, **extraKwArgs)
            ...
            self.template = templateObj = Template(input, searchList=self.searchList(), compilerSettings=self._getCompilerSettings())
            moduleCode = templateObj._CHEETAH_generatedModuleCode
        if self.DEBUGLEV >= 1:
            print 'Module: %s' % templateObj.__module__
            print moduleCode.encode('ascii', 'backslashreplace').decode('ascii')
        try:
    ...
    def _outputMismatchReport(self, output, expectedOutput):
        if self._debugEOLReplacement and self._EOLreplacement:
            ...
            return self.report % {'template': self._input, 'expected': expectedOutput, 
               'actual': output, 
               'end': '(end)'}
...
class EmptyTemplate(OutputTest):
    ...
    def test1(self):
        ...
        warnings.filterwarnings('error', 'You supplied an empty string for the source!', UserWarning)
        try:
            self.verify('', '')
        except UserWarning:
            pass
...
class Backslashes(OutputTest):
    ...
    def setUp(self):
        ...
        fp.write('\\ #LogFormat "%h %l %u %t \\"%r\\" %>s %b"' + '\n\n\n\n\n\n\n')
        fp.flush()
        fp.close()
    ...
    def test1(self):
        r""" a single \ using rawstrings"""
        self.verify('\\ ', '\\ ')

    def test2(self):
        r""" a single \ using rawstrings and lots of lines"""
        self.verify('\\ \n\n\n\n\n\n\n\n\n', '\\ \n\n\n\n\n\n\n\n\n')

    def test3(self):
        r""" a single \ without using rawstrings"""
        self.verify('\\ \\ ', '\\ \\ ')

    def test4(self):
        """ single line from an apache conf file"""
        self.verify('#LogFormat "%h %l %u %t \\"%r\\" %>s %b"', '#LogFormat "%h %l %u %t \\"%r\\" %>s %b"')

    def test5(self):
        r""" single line from an apache conf file with many NEWLINES
        ...
        self.verify('#LogFormat "%h %l %u %t \\"%r\\" %>s %b"' + '\n\n\n\n\n\n\n', '#LogFormat "%h %l %u %t \\"%r\\" %>s %b"' + '\n\n\n\n\n\n\n')

    def test6(self):
        """ test backslash handling in an included file"""
        self.verify('#include "backslashes.txt"', '\\ #LogFormat "%h %l %u %t \\"%r\\" %>s %b"' + '\n\n\n\n\n\n\n')

    def test7(self):
        r""" a single \ without using rawstrings plus many NEWLINES"""
        self.verify('\\ \\ \n\n\n\n\n\n\n\n\n', '\\ \\ \n\n\n\n\n\n\n\n\n')
...
class NonTokens(OutputTest):
    ...
    def test3(self):
        """escapted comments"""
        self.verify('  \\##escaped comment  ', '  ##escaped comment  ')
...
class EncodingDirective(OutputTest):
    ...
    def test3(self):
        ...
        self.verify(source, 'ሴ', outputEncoding='utf8')
    ...
    def test4(self):
        ...
        self.verify(source, 'á\x88´')

    def test5(self):
        """basic #encoding """
        self.verify(b'#encoding latin-1\nAndr\x82', 'Andr\x82')

    def test6(self):
        """Using #encoding on the second line"""
        source = '### Comments on the first line\n#encoding utf-8\nሴ'
        if not PY2:
            source = source.decode('utf-8')
        self.verify(source, 'ሴ', outputEncoding='utf8')
...
class Placeholders_Calls(OutputTest):
    ...
    def test13(self):
        ...
        self.verify("$aFunc('aoeu'*2)", 'aoeuaoeu')

    def test14(self):
        """func placeholder - with (int*int)"""
        self.verify('$aFunc(2*2)', '4')

    def test15(self):
        """func placeholder - with (int*float)"""
        self.verify('$aFunc(2*2.0)', '4.0')

    def test16(self):
        r"""func placeholder - with (int\n*\nfloat)"""
        self.verify('$aFunc(2\n*\n2.0)', '4.0', convertEOLs=False)

    def test17(self):
        """func placeholder - with ($arg=float)"""
        self.verify('$aFunc($arg=4.0)', '4.0')
    ...
    def test19(self):
        ...
        self.verify('$aFunc($arg=$aMeth($arg=$aFunc(1)))', '1')
    ...
    def test25(self):
        ...
        self.verify('$*15*(aFunc(  $arg = $aMeth( $arg = $aFunc( 1 ) ) ) )', '1')
...
class NameMapper(OutputTest):
    ...
    def test1(self):
        """autocalling"""
        self.verify('$aFunc! $aFunc().', 'Scooby! Scooby.')

    def test2(self):
        """nested autocalling"""
        self.verify('$aFunc($aFunc).', 'Scooby.')

    def test3(self):
        """list subscription"""
        self.verify('$aList[0]', 'item0')
    ...
    def test5(self):
        ...
        self.verify('$aList[:2][0]', 'item0')

    def test6(self):
        """dictionary access - NameMapper style"""
        self.verify('$aDict.one', 'item1')
    ...
    def test8(self):
        ...
        self.verify('$aDict.one.upper', 'ITEM1')
    ...
    def test14(self):
        ...
        self.verify('$aDict.nestedDict.two.upper', 'NESTEDITEM2')
    ...
    def test16(self):
        ...
        then slice.
        """
        self.verify('$aDict.nestedDict.two.upper[:4]', 'NEST')

    def test17(self):
        """nested dictionary access - Python style using a soft-coded key"""
        self.verify("$aDict[$anObj.meth('nestedDict')].two", 'nestedItem2')

    def test18(self):
        """object method access"""
        self.verify('$anObj.meth1', 'doo')

    def test19(self):
        """object method access, followed by complex slice"""
        self.verify('$anObj.meth1[0: ((4//4*2)*2)//$anObj.meth1(2) ]', 'do')
    ...
    def test21(self):
        ...
        for."""
        self.verify('$anObj.methWithPercentSignDefaultArg', '110%')
...
class CacheDirective(OutputTest):
    ...
    def test6(self):
        ...
        self.verify('#cache_foo', '#cache_foo')
        self.verify('#cached', '#cached')
...
class RawDirective(OutputTest):
    ...
    def test6(self):
        ...
        self.verify('#raw: This escape should be preserved: \\$unexpanded So should this one: \\#blah The string "\\012" should not disappear.', 'This escape should be preserved: \\$unexpanded So should this one: \\#blah The string "\\012" should not disappear.')
...
class ForDirective(OutputTest):
    ...
    def test6(self):
        ...
        self.verify('#for $i in range(5)#$i#end for#', '01234')
    ...
    def test8(self):
        ...
        self.verify('  #for $i in range($aFunc(5))#$i#end for#  ', '  01234  ')
    ...
    def test13(self):
        """single line #for"""
        self.verify('#for $i in range($aFunc(5)): $i', '01234')

    def test14(self):
        """single line #for with 1 extra leading space"""
        self.verify('#for $i in range($aFunc(5)):  $i', ' 0 1 2 3 4')
...
class RepeatDirective(OutputTest):
    ...
    def test6(self):
        """single-line #repeat"""
        self.verify('#repeat $numTwo: 1', '11')
        self.verify('#repeat $numTwo: 1\n' * 2, '1\n1\n1\n1\n')
        self.verify('#repeat 3:  \n1\n#end repeat', '1\n1\n1\n')
...
class DefDirective(OutputTest):
    ...
    def test11(self):
        ...
        self.verify('#def testMeth: aoeuaoeu\n- $testMeth -', '- aoeuaoeu -')

    def test12(self):
        """single line #def with extra WS and nested $placeholders"""
        self.verify('#def testMeth: $anInt $aFunc(1234)\n- $testMeth -', '- 1 1234 -')

    def test13(self):
        """single line #def escaped $placeholders"""
        self.verify('#def testMeth: \\$aFunc(\\$anInt)\n- $testMeth -', '- $aFunc($anInt) -')
...
class BlockDirective(OutputTest):
    ...
    def test6(self):
        """single line #block """
        self.verify('#block testMeth: This is my block', 'This is my block')
    ...
    def test8(self):
        ...
        self.verify('#block testMeth: \\$aFunc($anInt)', '$aFunc(1)')
    ...
    def test13(self):
        ...
        self.verify("#block $testMeth($arg='This is my block'): $arg", 'This is my block')

    def test14(self):
        """single line #block with None for content"""
        self.verify('#block $testMeth: $None\ntest $testMeth-', 'test -')

    def test15(self):
        """single line #block with nothing for content"""
        self.verify('#block $testMeth: \nfoo\n#end block\ntest $testMeth-', 'foo\ntest foo\n-')
...
class IncludeDirective(OutputTest):
    ...
    def test2(self):
        ...
        self.verify('#include raw source=$blockToBeParsed', '$numOne $numTwo')

    def test3(self):
        """#include raw of 'parseTest.txt'"""
        self.verify("#include raw 'parseTest.txt'", '$numOne $numTwo')

    def test4(self):
        """#include raw of $includeFileName"""
        self.verify('#include raw $includeFileName', '$numOne $numTwo')
    ...
    def test6(self):
        ...
        self.verify("       #include raw source='This is my $Source '*2      ", 'This is my $Source This is my $Source ')

    def test7(self):
        """#include of $blockToBeParsed"""
        self.verify('#include source=$blockToBeParsed', '1 2')
...
class SetDirective(OutputTest):
    ...
    def test4(self):
        ...
        self.verify('#set global $aSetVar = 1234\n#include source=$includeBlock2', '1 2 1234')
...
class IfDirective(OutputTest):
    ...
    def test1(self):
        ...
        self.verify('#if 1 ##comment \n$aStr\n#end if\n', 'blarg\n')
        self.verify('#if 1##for i in range(10)#$i#end for##end if', '0123456789')
        self.verify('#if 1: #for i in range(10)#$i#end for', '0123456789')
        self.verify('#if 1: #for i in range(10):$i', '0123456789')
    ...
    def test13(self):
        ...
        self.verify('#if $emptyString# $anInt#else#$anInt - $anInt#end if', '1 - 1')

    def test14(self):
        """single-line #if: simple"""
        self.verify("#if $emptyString then 'true' else 'false'", 'false')

    def test15(self):
        """single-line #if: more complex"""
        self.verify("#if $anInt then 'true' else 'false'", 'true')

    def test16(self):
        """single-line #if: with the words 'else' and 'then' in the output """
        self.verify("#if ($anInt and not $emptyString==''' else ''') then $str('then') else 'else'", 'then')
...
class SuperDirective(OutputTest):
    ...
    def test1(self):
        ...
        expected = 'this is base foo          This is child foo\nthis is base foo          super-1234\n super-99'
...
class FilterDirective(OutputTest):
    ...
    def test9(self):
        ...
        """
        self.verify('#filter Strip\n$strip2#end filter', 'strippable whitespace')
    ...
    def test11(self):
        ...
        """
        self.verify('#filter StripSqueeze\n$strip3#end filter', 'strippable whitespace 1 2 3')
...
class VarExists(OutputTest):
    ...
    def test1(self):
        """$varExists('$anInt')
        """
        self.verify("$varExists('$anInt')", repr(True))
    ...
    def test3(self):
        """$varExists('$anInt')
        """
        self.verify("$varExists('$bogus')", repr(False))
...
class GetVar(OutputTest):
    ...
    def test3(self):
        """$self.getVar('anInt')
        """
        self.verify("$self.getVar('anInt')", '1')

    def test4(self):
        """$getVar('bogus', 1234)
        """
        self.verify("$getVar('bogus',  1234)", '1234')
...
class WhitespaceAfterDirectiveTokens(OutputTest):
    ...
    def test1(self):
        self.verify('# for i in range(10): $i', '0123456789')
        self.verify('# for i in range(10)\n$i# end for', '0123456789')
        self.verify('# for i in range(10)#$i#end for', '0123456789')
...
class DefmacroDirective(OutputTest):
    ...
    def test1(self):
        self.verify('#defmacro inc: #set @src +=1\n#set i = 1\n#inc: $i\n$i', '2')
        self.verify('#defmacro test\n#for i in range(10): @src\n#end defmacro\n#test: $i-foo#slurp\n#for i in range(3): $i', '0-foo1-foo2-foo3-foo4-foo5-foo6-foo7-foo8-foo9-foo012')
        self.verify('#defmacro test\n#for i in range(10): @src\n#end defmacro\n#test: $i-foo\n#for i in range(3): $i', '0-foo\n1-foo\n2-foo\n3-foo\n4-foo\n5-foo\n6-foo\n7-foo\n8-foo\n9-foo\n012')
        self.verify('#defmacro test: #for i in range(10): @src\n#test: $i-foo#slurp\n-#for i in range(3): $i', '0-foo1-foo2-foo3-foo4-foo5-foo6-foo7-foo8-foo9-foo-012')
        self.verify('#defmacro test##for i in range(10): @src#end defmacro##slurp\n#test: $i-foo#slurp\n-#for i in range(3): $i', '0-foo1-foo2-foo3-foo4-foo5-foo6-foo7-foo8-foo9-foo-012')
        self.verify('#defmacro testFoo: nothing\n#defmacro test(foo=1234): #for i in range(10): @src\n#test foo=234: $i-foo#slurp\n-#for i in range(3): $i', '0-foo1-foo2-foo3-foo4-foo5-foo6-foo7-foo8-foo9-foo-012')
        self.verify("#defmacro testFoo: nothing\n#defmacro test(foo=1234): #for i in range(10): @src@foo\n#test foo='-foo'#$i#end test#-#for i in range(3): $i", '0-foo1-foo2-foo3-foo4-foo5-foo6-foo7-foo8-foo9-foo-012')
        self.verify("#defmacro testFoo: nothing\n#defmacro test(foo=1234): #for i in range(10): @src.strip()@foo\n#test foo='-foo': $i\n-#for i in range(3): $i", '0-foo1-foo2-foo3-foo4-foo5-foo6-foo7-foo8-foo9-foo-012')

    def test2(self):
        self.verify('#aMacro: foo', 'blarg')
...
def install_eols():
    ...
    for klass in klasses:
        ...
        if hasattr(klass, 'convertEOLs') and klass.convertEOLs:
            win32Src = "class %(name)s_Win32EOL(%(name)s): _EOLreplacement = '\\r\\n'" % locals()
            macSrc = "class %(name)s_MacEOL(%(name)s): _EOLreplacement = '\\r'" % locals()
            exec win32Src in globals()
            exec macSrc in globals()
        src = 'class %(name)s_DiffBaseClass(%(name)s): ' % locals()
