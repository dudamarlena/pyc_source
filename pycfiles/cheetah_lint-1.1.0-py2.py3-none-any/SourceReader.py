...
# Embedded file name: build/lib.macosx-10.13-x86_64-2.7/Cheetah/SourceReader.py
# Compiled at: 2019-09-22 10:12:27
__doc__ = "SourceReader class for Cheetah's Parser and CodeGenerator\n"
import re
...
class SourceReader(object):
    ...
    def checkPos(self, pos):
        if not pos <= self._breakPoint:
            raise Error('pos (' + str(pos) + ") is invalid: beyond the stream's end (" + str(self._breakPoint - 1) + ')')
        elif not pos >= 0:
    ...
    def setBreakPoint(self, pos):
        if pos > self._srcLen:
            raise Error('New breakpoint (' + str(pos) + ") is invalid: beyond the end of stream's source string (" + str(self._srcLen) + ')')
        elif not pos >= 0:
    ...
    def gotoBookmark(self, name):
        ...
        if not self.validPos(pos):
            raise Error('Invalid bookmark (' + name + ', ' + str(pos) + ') is invalid: pos is out of range')
        self._pos = pos
    ...
    def matchWhiteSpace(self, WSchars=' \x0c\t'):
        return not self.atEnd() and self.peek() in WSchars

    def getWhiteSpace(self, max=None, WSchars=' \x0c\t'):
    ...
    def matchNonWhiteSpace(self, WSchars=' \x0c\t\n\r'):
        return self.atEnd() or self.peek() not in WSchars

    def getNonWhiteSpace(self, WSchars=' \x0c\t\n\r'):
