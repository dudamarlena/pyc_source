...
# Embedded file name: build/lib.macosx-10.13-x86_64-2.7/Cheetah/SettingsManager.py
# Compiled at: 2019-09-22 10:12:27
import os.path, copy as copyModule
try:
...
complexNumberRE = re.compile('[\\(]*' + Number + '[ \\t]*\\+[ \\t]*' + Number + '[\\)]*')
...
class _SettingsCollector(object):
    ...
    def readSettingsFromPySrcStr(self, theString):
        ...
        globalsDict = {'True': 1 == 1, 'False': 0 == 1}
        ...
        for sect, subDict in list(newSettings.items()):
            for key, val in list(subDict.items()):
                ...
                if key.lower() == 'importsettings':
                    ...
                    subDict.update(mergeNestedDictionaries(subDict, importedSettings))
...
class SettingsManager(_SettingsCollector):
    ...
    def updateSettingsFromPySrcStr(self, theString, merge=True):
        ...
        self.updateSettings(newSettings, merge=newSettings.get('mergeSettings', merge))
