...
# Embedded file name: build/lib.macosx-10.13-x86_64-2.7/Cheetah/FileUtils.py
...
def _escapeRegexChars(txt, escapeRE=re.compile('([\\$\\^\\*\\+\\.\\?\\{\\}\\[\\]\\(\\)\\|\\\\])')):
    return escapeRE.sub('\\\\\\1', txt)
...
class FileFinder:
    ...
    def __init__(self, rootPath, globPatterns=('*', ), ignoreBasenames=('CVS', '.svn'), ignoreDirs=()):
    ...
    def walkDirTree(self, dir='.', listdir=os.listdir, isdir=os.path.isdir, join=os.path.join):
    ...
    def filterDir(self, baseName, fullPath):
        ...
        return not (baseName in self._ignoreBasenames or fullPath in self._ignoreDirs)
...
class _GenSubberFunc:
    ...
    backrefRE = re.compile('\\\\([1-9][0-9]*)')
    groupRE = re.compile('\\\\g<([a-zA-Z_][a-zA-Z_]*)>')
...
class FindAndReplace:
    ...
    def __init__(self, files, patternOrRE, replacement, recordResults=True):
        ...
        if os.popen3('pgrep')[2].read().startswith('Usage:'):
            ...
            tmpFile.write('#')
            if not os.popen3('pgrep "' + pattern + '" ' + tmpFile.name)[2].read():
                self._usePgrep = True
            tmpFile.close()
        self._run()
    ...
    def _subDispatcher(self, match):
        if self._recordResults:
            if self._currFile not in self._results:
                ...
                res = self._results[self._currFile]
            res['count'] += 1
            res['matches'].append({'contents': match.group(), 'start': match.start(), 
               'end': match.end()})
...
class SourceFileStats:
    ...
    def summary(self):
        ...
        stats = {'codeLines': codeLines, 'blankLines': blankLines, 
    ...
    def getFileStats(self, fileName):
        ...
        stats = {'codeLines': codeLines, 'blankLines': blankLines, 'commentLines': commentLines, 
           'totalLines': totalLines}
        return stats