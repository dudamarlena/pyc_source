...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/docbook2sla/tests/test_syncronize.py
# Compiled at: 2008-04-13 10:46:30
...
class SyncronizeScribusDocument01TestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.outputfn = self.d2s.syncronize('tests/data/test_syncronize/01_docbook.xml', 'tests/data/test_syncronize/01_scribus.sla')
...
class SyncronizeScribusDocument02TestCase(unittest.TestCase):
    ...
    def setUp(self):
        self.d2s = DocBook2Sla()
        self.outputfn = self.d2s.syncronize('tests/data/test_syncronize/02_docbook.xml', 'tests/data/test_syncronize/02_scribus.sla')
        self.output = open(self.outputfn, 'r').read()
        self.outputtree = etree.XML(self.output)
...
class SyncronizeScribusDocument03TestCase(unittest.TestCase):
    ...
    def setUp(self):
        self.d2s = DocBook2Sla()
        self.outputfn = self.d2s.syncronize('tests/data/test_syncronize/03_docbook.xml', 'tests/data/test_syncronize/03_scribus.sla')
        self.output = open(self.outputfn, 'r').read()
        self.outputtree = etree.XML(self.output)
