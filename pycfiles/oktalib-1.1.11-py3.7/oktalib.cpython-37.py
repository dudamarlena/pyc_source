...
# [GCC 8.4.0]
# Embedded file name: /Users/ghawker/code/python/oktalib/oktalib/oktalib.py
# Compiled at: 2020-01-13 05:24:47
# Size of source mod 2**32: 13051 bytes
"""
...
"""
import logging, json, backoff
from requests import Session
from .oktalibexceptions import AuthFailed, InvalidGroup, InvalidApplication, ApiLimitReached
from .entities import Group, User, Application
...
class Okta:
    __doc__ = 'Models the api of okta.'
    ...
    def get_group_type_by_name(self, name, group_type='OKTA_GROUP'):
        ...
        """
        group = next((group for group in self.search_groups_by_name(name) if group.type == group_type), None)
        return group
    ...
    def get_group_by_name(self, name):
        ...
        """
        return next((group for group in self.search_groups_by_name(name) if group.name == name), None)
    ...
    @backoff.on_exception((backoff.expo), ApiLimitReached,
      max_time=60)
    ...
    def _get_next_link(response):
        ...
        if links:
            link_text = next((link for link in links.split(',') if 'next' in link), None)
            if link_text:
    ...
    def create_user(self, first_name, last_name, email, login, password=None, enabled=True):
    ...
    def get_application_by_label(self, label):
        ...
        """
        app = next((app for app in self.applications if app.label.lower() == label.lower()), None)
        return app
