...
# [GCC 8.4.0]
# Embedded file name: /Users/ghawker/code/python/oktalib/oktalib/entities/entities.py
# Compiled at: 2020-01-10 08:33:33
# Size of source mod 2**32: 32138 bytes
"""
...
"""
import json, logging
from oktalib.oktalibexceptions import InvalidApplication, InvalidUser, InvalidGroup
...
class Group(Entity):
    __doc__ = 'Models the group object of okta.'
    ...
    def name(self, value):
        ...
        if not response.ok:
            self._logger.error('Setting name failed. Response :{}'.format(response.text))
        else:
            self._update()
    ...
    def description(self, value):
        ...
        response = self._okta.session.put(url, data=(json.dumps(payload)))
        if not response.ok:
            self._logger.error('Setting description failed. Response :{}'.format(response.text))
        else:
            self._update()
    ...
    def users(self):
        ...
        return [User(self._okta, data) for data in self._okta._get_paginated_url(url)]
    ...
    def applications(self):
        ...
        return [Application(self._okta, data) for data in self._okta._get_paginated_url(url)]
    ...
    def add_user_by_login(self, login):
        ...
        """
        user = next((user for user in self._okta.users if user.login.lower() == login.lower()), None)
        if not user:
            raise InvalidUser(login)
        url = '{api}/groups/{id_}/users/{user_id}'.format(api=(self._okta.api), id_=(self.id),
          user_id=(user.id))
        response = self._okta.session.put(url)
        if not response.ok:
            self._logger.error('Adding user failed Response :{}'.format(response.text))
        return response.ok
    ...
    def remove_user_by_login(self, login):
        ...
        response = self._okta.session.delete(url)
        if not response.ok:
            self._logger.error('Removing user failed Response :{}'.format(response.text))
        return response.ok
...
class Application(Entity):
    __doc__ = 'Models the apps in okta.'
    ...
    def groups(self):
        ...
        return [Group(self._okta, data) for data in self._okta._get_paginated_url(url)]
    ...
    def get_associated_saml_roles(self):
        ...
        """
        url = '{api}/internal/apps/{app_id_}/types'.format(api=(self._okta.api), app_id_=(self.id))
    ...
    def add_group_by_id(self, group_id):
        ...
        """
        url = '{api}/apps/{id_}/groups/{group_id}'.format(api=(self._okta.api), id_=(self.id),
          group_id=group_id)
        response = self._okta.session.put(url)
        if not response.ok:
            self._logger.error('Adding group failed Response :{}'.format(response.text))
        return response.ok
    ...
    def remove_group_by_id(self, group_id):
        ...
        response = self._okta.session.delete(url)
        if not response.ok:
            self._logger.error('Removing group failed Response :{}'.format(response.text))
        return response.ok
    ...
    def assign_group_to_saml_user_roles(self, group_id, role, saml_roles):
        ...
        """
        url = '{api}/apps/{app_id_}/groups/{group_id_}'.format(api=(self._okta.api), app_id_=(self.id),
          group_id_=group_id)
        payload = {'id':group_id,  'profile':{'role':role,  'samlRoles':saml_roles}}
        response = self._okta.session.put(url, json=payload)
        if not response.ok:
            self._logger.error('Assigning group to the application failed Response :{}'.format(response.text))
        return response.ok


class User(Entity):
    __doc__ = 'Models the user object of okta.'
    ...
    def activate(self):
        ...
        """
        url = '{api}/users/{id_}/lifecycle/activate?sendEmail=false'.format(api=(self._okta.api),
          id_=(self.id))
    ...
    def deactivate(self):
        ...
        """
        url = '{api}/users/{id_}/lifecycle/deactivate'.format(api=(self._okta.api), id_=(self.id))
    ...
    def unlock(self):
        ...
        """
        url = '{api}/users/{id_}/lifecycle/unlock'.format(api=(self._okta.api), id_=(self.id))
    ...
    def expire_password(self):
        ...
        """
        url = '{api}/users/{id_}/lifecycle/expire_password'.format(api=(self._okta.api), id_=(self.id))
    ...
    def reset_password(self):
        ...
        """
        url = '{api}/users/{id_}/lifecycle/reset_password??sendEmail=false'.format(api=(self._okta.api),
          id_=(self.id))
    ...
    def set_temporary_password(self):
        ...
        """
        url = '{api}/users/{id_}/lifecycle/expire_password?tempPassword=true'.format(api=(self._okta.api),
          id_=(self.id))
        response = self._okta.session.post(url)
        if not response.ok:
    ...
    def suspend(self):
        ...
        """
        url = '{api}/users/{id_}/lifecycle/suspend'.format(api=(self._okta.api), id_=(self.id))
    ...
    def unsuspend(self):
        ...
        """
        url = '{api}/users/{id_}/lifecycle/unsuspend'.format(api=(self._okta.api), id_=(self.id))
    ...
    def update_password(self, old_password, new_password):
        ...
        """
        url = '{api}/users/{id_}/credentials/change_password'.format(api=(self._okta.api), id_=(self.id))
        payload = {'oldPassword':{'value': old_password},  'newPassword':{'value': new_password}}
    ...
    def update_security_question(self, password, question, answer):
        ...
        """
        url = '{api}/users/{id_}/credentials/change_recovery_question'.format(api=(self._okta.api), id_=(self.id))
        payload = {'password':{'value': password},  'recovery_question':{'question':question, 
          'answer':answer}}
        response = self._okta.session.post(url, data=(json.dumps(payload)))
        if not response.ok:
            self._logger.error(response.text)
