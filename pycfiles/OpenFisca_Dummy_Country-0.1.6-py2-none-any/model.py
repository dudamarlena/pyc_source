...
# [GCC 8.4.0]
# Embedded file name: /Users/fpagnoux/dev/openfisca/openfisca-dummy-country/openfisca_dummy_country/model/model.py
# Compiled at: 2017-05-03 12:52:29
...
class city_code(Variable):
    ...
    entity = Famille
    label = 'Code INSEE de la commune de résidence de la famille'
    definition_period = ETERNITY
...
class a_charge_fiscale(Variable):
    column = BoolCol
    entity = Individu
    label = "La personne n'est pas fiscalement indépendante"
    definition_period = MONTH
...
class rmi(DatedVariable):
    column = FloatCol
    entity = Individu
    label = 'RMI (remplacé par le RSA en 2010)'
    definition_period = MONTH
...
class contribution_sociale(DatedVariable):
    column = FloatCol
    entity = Individu
    label = 'Contribution payée sur le salaire'
    definition_period = YEAR
...
class fixed_tax(Variable):
    column = FloatCol
    entity = Individu
    label = 'Former tax used to be paid by every adult'
    definition_period = YEAR
    start_date = date(1980, 1, 1)
    stop_date = date(1989, 12, 31)

    def function(individu, period, legislation):
        return individu('age') >= 7200


class api(DatedVariable):
    column = FloatCol
    entity = Famille
    label = 'Allocation pour Parent Isolé'
    definition_period = MONTH
