...
# [GCC 8.4.0]
# Embedded file name: microdrop\gui\dmf_device_view.py
...
import gtk, gobject, cairo, numpy as np
...
class ElectrodeContextMenu(SlaveView):
    ...
    """
    builder_path = base_path().joinpath('gui', 'glade', 'dmf_device_view_context_menu.glade')
            ...
            try:
                ...
                if channels and max(channels) >= len(state):
                    for i in range(len(app.protocol)):
                        ...
                        options.state_of_channels = np.concatenate([options.state_of_channels,
                         np.zeros(max(channels) - len(options.state_of_channels) + 1, int)])
                        if i != app.protocol.current_step_number:
                            emit_signal('on_step_options_changed', [
                             self.model.controller.name, i], interface=IPlugin)

                self.last_electrode_clicked.channels = channels
                emit_signal('on_step_options_changed', [
                 self.model.controller.name,
                 app.protocol.current_step_number], interface=IPlugin)
    ...
    def on_edit_electrode_area__activate(self, widget, data=None):
        ...
        area = text_entry_dialog('Area of electrode in mm<span rise="5000" font_size="smaller">2</span>:', str(area), 'Edit electrode area')
        if area:
            if is_float(area):
                app.dmf_device.scale = float(area) / self.last_electrode_clicked.area()
            else:
    ...
    def popup(self, state_of_channels, electrode, button, time, register_enabled=True):
...
class DmfDeviceView(GtkVideoView):
    ...
    def update_draw_queue(self):
        if self.window_xid and self._proxy:
            if self.controller.video_enabled:
                overlay_opacity = self.overlay_opacity / 100.0
            else:
                overlay_opacity = 1.0
    ...
    def get_draw_queue(self, width, height, alpha=1.0):
        ...
        if app.dmf_device:
            ...
            self.svg_space = CartesianSpace(device_width, device_height, offset=(
             x, y))
            padding = 20
            if width / device_width < height / device_height:
                ...
                drawing_y = padding
            self.drawing_space = CartesianSpace(drawing_width, drawing_height, offset=(
             drawing_x, drawing_y))
            scale = np.array(self.drawing_space.dims) / np.array(self.svg_space.dims)
    ...
    def draw_electrode(self, electrode, cr, color=None):
        ...
        cr.save()
        if color is None:
            color = [ v / 255.0 for v in p.color ]
        if len(color) < 4:
            color += [1.0] * (len(color) - 4)
    ...
    def _initialize_video(self, device, caps_str, bitrate=None, record_path=None):
        ...
        self._proxy.create(device, caps_str, bitrate=bitrate, record_path=record_path, draw_queue=draw_queue, with_scale=True, with_warp=True)
    ...
    def get_clicked_electrode(self, event):
        ...
        if self.svg_space and self.drawing_space:
            normalized_coords = self.drawing_space.normalized_coords(*event.get_coords())
            shape = app.dmf_device.body_group.space.point_query_first(self.svg_space.translate_normalized(*normalized_coords))
            if shape:
    ...
    def on_electrode_click(self, electrode, event):
        ...
        if event.button == 1:
            ...
            self.popup.popup(state, electrode, event.button, event.time, register_enabled=self.controller.video_enabled)
        return True
    ...
    def _on_register_frame_grabbed(self, cv_img):
        ...
        def do_device_registration():
            ...
            results = dialog.run()
            if results:
                array = np.fromstring(results.tostring(), dtype='float32', count=results.width * results.height)
...
class DeviceRegistrationDialog(RegistrationDialog):
    ...
    def get_glade_path(self):
        assert False
        return base_path().joinpath('opencv_helpers', 'glade', 'registration_demo.glade')
