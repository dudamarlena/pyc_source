...
# [GCC 8.4.0]
# Embedded file name: microdrop\plugin_manager.py
# Compiled at: 2016-08-22 17:18:32
"""
...
import logging, re, os, platform, subprocess
from path_helpers import path
import task_scheduler, yaml
from run_exe import run_exe
from interfaces import Plugin, IPlugin, PluginGlobals, ExtensionPoint, IWaveformGenerator, ILoggingPlugin, IVideoPlugin, SingletonPlugin, implements
...
def post_install(install_path):
    ...
    if platform.system() in ('Linux', 'Darwin'):
        ...
        for ext in ('exe', 'bat'):
            ...
            if on_install_path.isfile():
                run_exe(on_install_path.name, '"%s"' % sys.executable, try_admin=True, working_dir=hooks_path)
                break
...
def get_service_instance_by_package_name(name, env='microdrop.managed'):
    e = PluginGlobals.env(env)
    plugins = [ p for i, p in enumerate(e.services) if name == get_plugin_package_name(p.__class__.__module__)
              ]
    if plugins:
        return plugins[0]
...
def get_plugin_package_name(class_name):
    match = re.search('plugins\\.(?P<name>.*)', class_name)
    if match is None:
        logging.error('Could not determine package name from: %s' % class_name)
        return
    else:
    ...
    if schedule_requests:
        ...
        for request in [ r for name, requests in schedule_requests.items() for r in requests ]:
            try:
                ...
                logging.info('[PluginManager] emit_signal(%s) could not add schedule request %s' % (
                 function, request))
                continue
                ...
                with closing(StringIO()) as (message):
                    if hasattr(observer, 'name'):
                        if interface == ILoggingPlugin:
                            continue
                        print >> message, '%s plugin crashed processing %s signal.' % (
