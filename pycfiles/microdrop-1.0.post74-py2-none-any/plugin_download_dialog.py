...
# [GCC 8.4.0]
# Embedded file name: microdrop\gui\plugin_download_dialog.py
...
class PluginDownloadDialog(object):
    ...
    def __init__(self):
        builder = gtk.Builder()
        builder.add_from_file(glade_path().joinpath('plugin_download_dialog.glade'))
    ...
    def update(self):
        ...
        available = set(p.available_packages(app_version={'major': 1, 'minor': 0, 
           'micro': 0}))
        installed = set([ p.get_plugin_package_name() for p in self.controller.plugins
                        ])
    ...
    def run(self):
        ...
        try:
            ...
            logging.error('Could not connect to plugin repository at: %s' % app.get_app_value('server_url'))
            return gtk.RESPONSE_CANCEL
