...
# [GCC 8.4.0]
# Embedded file name: c:\users\christian\documents\github\microdrop-1.0\microdrop\config.py
# Compiled at: 2016-08-24 19:55:34
"""
...
class Config(object):
    ...
    def _validate(self):
        ...
        if results != True:
            ...
            for section_list, key, _ in flatten_errors(self.data, results):
                if key is not None:
                    logger.error('The "%s" key in the section "%s" failed validation' % (
    ...
    def _init_data_dir(self):
        ...
        if 'data_dir' not in self.data:
            ...
            warnings.warn('Using default Microdrop user data path: %s' % default_data_dir)
        if not path(self['data_dir']).isdir():
            warnings.warn('Microdrop user data directory does not exist.')
            path(self['data_dir']).makedirs_p()
            warnings.warn('Created Microdrop user data directory: %s' % self['data_dir'])

    def _init_plugins_dir(self):
        if self.data['plugins']['directory'] is None:
            self.data['plugins']['directory'] = path(self['data_dir']).joinpath('plugins')
