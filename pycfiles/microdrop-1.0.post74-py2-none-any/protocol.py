...
# [GCC 8.4.0]
# Embedded file name: microdrop\protocol.py
...
import yaml
from plugin_manager import emit_signal, IPlugin, ExtensionPoint, get_service_names
import logging
...
class Protocol():
            ...
            for k, v in out[i].plugin_data.items():
                if k in enabled_plugins:
                    try:
                        ...
                        v = v.replace('!!python/object:gui.dmf_device_controller.', '!!python/object:microdrop.gui.dmf_device_controller.')
                        out[i].plugin_data[k] = yaml.load(v)

        logger.debug('[Protocol].load() loaded in %f s.' % (time.time() - start_time))
        return out
    ...
    def next_step(self):
        if self.current_step_number == len(self.steps) - 1:
            self.insert_step(self.current_step_number, self.current_step().copy())
            self.next_step()
        else:
        ...
        with closing(StringIO()) as (sio):
            for plugin_name, fields in self.plugin_fields.iteritems():
                ...
                if hasattr(service, 'get_step_value'):
                    print >> sio, '[ProtocolController] plugin.name=%s field_values=' % plugin_name,
