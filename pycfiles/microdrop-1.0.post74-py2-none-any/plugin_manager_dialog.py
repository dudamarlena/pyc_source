...
# [GCC 8.4.0]
# Embedded file name: microdrop\gui\plugin_manager_dialog.py
...
class PluginManagerDialog(object):
    ...
    def __init__(self):
        builder = gtk.Builder()
        builder.add_from_file(glade_path().joinpath('plugin_manager_dialog.glade'))
    ...
    def run(self):
        ...
        for p in self.controller.plugins:
            ...
            elif package_name in app.config['plugins']['enabled']:
    ...
    def on_button_install_clicked(self, *args, **kwargs):
        response = open_filechooser('Select plugin file', action=gtk.FILE_CHOOSER_ACTION_OPEN, patterns=[
         '*.tar.gz', '*.tgz', '*.zip'])
        if response is None:
            return True
        else:
