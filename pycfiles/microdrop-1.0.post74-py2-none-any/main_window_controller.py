...
# [GCC 8.4.0]
# Embedded file name: microdrop\gui\main_window_controller.py
...
"""
import pdb, os, webbrowser, pkg_resources, gtk
...
from ..plugin_manager import IPlugin, SingletonPlugin, implements, PluginGlobals, ScheduleRequest, ILoggingPlugin, emit_signal, get_service_instance_by_name
...
class MainWindowController(SingletonPlugin):
    ...
    def on_plugin_enable(self):
        ...
        self.view.set_icon_from_file(pkg_resources.resource_filename('microdrop', 'microdrop.ico'))
        ...
        app.signals['on_menu_experiment_logs_activate'] = self.on_menu_experiment_logs_activate
        app.signals['on_window_destroy'] = self.on_destroy
        app.signals['on_window_delete_event'] = self.on_delete_event
        app.signals['on_checkbutton_realtime_mode_button_press_event'] = self.on_realtime_mode_toggled
        ...
        if app.config.data.get('advanced_ui', False):
            ...
            def activate_debugger():
                try:
                    plugin = get_service_instance_by_name('wheelerlab.dmf_control_board')
            ...
            self.debug_menu_item.connect('activate', lambda *args: activate_debugger())
            self.ipython_menu_item.connect('activate', lambda *args: IPython.embed())
    ...
    def on_menu_manage_plugins_activate(self, widget, data=None):
        service = get_service_instance_by_name('microdrop.gui.plugin_manager_controller', env='microdrop')
    ...
    def error(self, message, title='Error'):
        dialog = gtk.MessageDialog(self.view, gtk.DIALOG_DESTROY_WITH_PARENT, gtk.MESSAGE_ERROR, gtk.BUTTONS_CLOSE, message)
    ...
    def warning(self, message, title='Warning'):
        dialog = gtk.MessageDialog(self.view, gtk.DIALOG_DESTROY_WITH_PARENT, gtk.MESSAGE_WARNING, gtk.BUTTONS_CLOSE, message)
        dialog.set_title(title)
        result = dialog.run()
        dialog.destroy()
        return result
    ...
    def question(self, message, title=''):
        dialog = gtk.MessageDialog(self.view, gtk.DIALOG_DESTROY_WITH_PARENT, gtk.MESSAGE_QUESTION, gtk.BUTTONS_YES_NO, message)
        dialog.set_title(title)
        result = dialog.run()
        dialog.destroy()
        return result
    ...
    def info(self, message, title=''):
        dialog = gtk.MessageDialog(self.view, gtk.DIALOG_DESTROY_WITH_PARENT, gtk.MESSAGE_INFO, gtk.BUTTONS_CLOSE, message)
        dialog.set_title(title)
        result = dialog.run()
        dialog.destroy()
        return result
    ...
    def on_protocol_changed(self):
        app = get_app()
        self.update_protocol_name_label(modified=app.protocol_controller.modified)
    ...
    def on_dmf_device_swapped(self, old_dmf_device, dmf_device):
        ...
        self.update_device_name_label(dmf_device, modified=get_app().dmf_device_controller.modified)

    def on_dmf_device_changed(self):
        self.update_device_name_label(modified=get_app().dmf_device_controller.modified)
