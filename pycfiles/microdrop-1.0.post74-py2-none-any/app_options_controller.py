...
# [GCC 8.4.0]
# Embedded file name: microdrop\gui\app_options_controller.py
...
from ..plugin_manager import IPlugin, SingletonPlugin, implements, ExtensionPoint, emit_signal
...
class AppOptionsController:
    ...
    def __init__(self):
        app = get_app()
        builder = gtk.Builder()
        builder.add_from_file(glade_path().joinpath('app_options_dialog.glade'))
    ...
    def run(self):
        ...
        for name, form in self.forms.iteritems():
            if form is None:
                ...
                schema_entries = [ f for f in form.field_schema if f.properties.get('show_in_gui', True) ]
        ...
        for form_name, form in self.forms.iteritems():
            ...
            for field in fields:
                ...
                form_field.label_widget.set_text(re.sub('_', ' ', field).title())
