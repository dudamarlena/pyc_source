...
# [GCC 8.4.0]
# Embedded file name: microdrop\gui\protocol_controller.py
...
"""
import os, logging, shutil, gtk
from textbuffer_with_undo import UndoableBuffer
from microdrop_utility import is_float, is_int, FutureVersionError
from microdrop_utility.gui import yesno, contains_pointer, register_shortcuts, textentry_validate, text_entry_dialog
from ..protocol import Protocol
from ..plugin_manager import ExtensionPoint, IPlugin, SingletonPlugin, implements, PluginGlobals, ScheduleRequest, emit_signal, get_service_class, get_service_instance, get_service_instance_by_name, get_observers, get_service_names
...
class ProtocolController(SingletonPlugin):
    ...
    def _register_shortcuts(self):
        ...
        shortcuts = {'<Control>r': self.on_run_protocol, '<Control>s': lambda *args: self.save_protocol(), 
        ...
        register_shortcuts(view, shortcuts, disabled_widgets=[
         self.textentry_notes])
        notes_shortcuts = {'<Control>z': self.textentry_notes.get_buffer().undo, 
           '<Control>y': self.textentry_notes.get_buffer().redo}
        register_shortcuts(view, notes_shortcuts, enabled_widgets=[
         self.textentry_notes])
    ...
    def load_protocol(self, filename):
        ...
        if p:
            ...
            if missing_plugins:
                ...
                result = yesno('Some data in the protocol "%s" requires plugins that are not currently installed:\n\t%s\nThis data will be ignored unless you install and enable these plugins. Would youlike to permanently clear this data from the protocol?' % (
                 p.name,
    ...
    def on_plugin_enable(self):
        ...
        self.textentry_protocol_repeats = self.builder.get_object('textentry_protocol_repeats')
        ...
        app.signals['on_textentry_protocol_repeats_focus_out_event'] = self.on_textentry_protocol_repeats_focus_out
        app.signals['on_textentry_protocol_repeats_key_press_event'] = self.on_textentry_protocol_repeats_key_press
    ...
    def on_load_protocol(self, widget=None, data=None):
        app = get_app()
        self.save_check()
        dialog = gtk.FileChooserDialog(title='Load protocol', action=gtk.FILE_CHOOSER_ACTION_OPEN, buttons=(
         gtk.STOCK_CANCEL,
         gtk.RESPONSE_CANCEL,
         gtk.STOCK_OPEN,
         gtk.RESPONSE_OK))
        dialog.set_default_response(gtk.RESPONSE_OK)
        dialog.set_current_folder(os.path.join(app.get_device_directory(), app.dmf_device.name, 'protocols'))
        response = dialog.run()
    ...
    def on_protocol_repeats_changed(self):
        app = get_app()
        if app.protocol:
            app.protocol.n_repeats = textentry_validate(self.textentry_protocol_repeats, app.protocol.n_repeats, int)
        ...
        if app.dmf_device.name:
            if save_as or rename or app.protocol.name is None:
                ...
                if name is None:
                    name = ''
            if name:
                path = os.path.join(app.get_device_directory(), app.dmf_device.name, 'protocols')
    ...
    def run_protocol(self):
        app = get_app()
        app.running = True
        self.button_run_protocol.set_image(self.builder.get_object('image_pause'))
    ...
    def pause_protocol(self):
        ...
        app.running = False
        app.protocol.current_step_attempt = 0
        self.button_run_protocol.set_image(self.builder.get_object('image_play'))
        emit_signal('on_protocol_pause')

    def run_step(self):
        app = get_app()
        if app.protocol and app.dmf_device and (app.realtime_mode or app.running):
            if app.experiment_log:
                app.experiment_log.add_step(app.protocol.current_step_number, app.protocol.current_step_attempt)
            self.waiting_for = get_observers('on_step_run', IPlugin).keys()
            logging.info('[ProcolController.run_step]: waiting for %s' % (', ').join(self.waiting_for))
            emit_signal('on_step_run')

    def on_step_complete(self, plugin_name, return_value=None):
        app = get_app()
        logging.info('[ProcolController].on_step_complete: %s finished' % plugin_name)
        ...
        if len(self.waiting_for):
            logging.debug('[ProcolController].on_step_complete: still waiting for %s' % (', ').join(self.waiting_for))
        elif app.running:
            if self.repeat_step:
    ...
    def _get_dmf_control_fields(self, step_number):
        ...
        dmf_plugin_name = step.plugin_name_lookup('wheelerlab.dmf_control_board_', re_pattern=True)
    ...
    def set_app_values(self, values_dict):
        logging.debug('[ProtocolController] set_app_values(): values_dict=%s', values_dict)
    ...
    def on_step_swapped(self, original_step_number, step_number):
        logging.debug('[ProtocolController.on_step_swapped] original_step_number=%s, step_number=%s', original_step_number, step_number)
        self._update_labels()
        self.run_step()
    ...
    def get_schedule_requests(self, function_name):
        ...
        if function_name == 'on_dmf_device_swapped':
            return [
             ScheduleRequest('microdrop.app', self.name)]
        if function_name == 'on_protocol_swapped':
            return [
             ScheduleRequest('microdrop.app', self.name)]
        return []
