...
# [GCC 8.4.0]
# Embedded file name: microdrop\experiment_log.py
...
class ExperimentLog:
        ...
        for i in range(len(out.data)):
            for plugin_name, plugin_data in out.data[i].items():
                try:
                    ...
                    logger.debug('Not a valid pickle string (plugin: %s). %s.' % (
                     plugin_name, e))
                    try:
                        out.data[i][plugin_name] = yaml.load(plugin_data)
                    except Exception as e:
                        logger.error("Couldn't load experiment log data for plugin: %s. %s." % (
                         plugin_name, e))

        logger.debug('[ExperimentLog].load() loaded in %f s.' % (time.time() - start_time))
        ...
        if version < Version(0, 1, 0):
            ...
            for step_data in self.data:
                if 'control board hardware version' in step_data.keys():
                    plugin_name = 'wheelerlab.dmf_control_board_' + step_data['control board hardware version']

            for i in range(len(self.data)):
                new_data.append({})
                for k, v in self.data[i].items():
                    if plugin_name and (k == 'FeedbackResults' or k == 'SweepFrequencyResults' or k == 'SweepVoltageResults'):
                        try:
    ...
    def add_step(self, step_number, attempt=0):
        self.data.append({'core': {'step': step_number, 'time': time.time() - self.start_time(), 
                    'attempt': attempt}})
