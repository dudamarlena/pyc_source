...
# [GCC 8.4.0]
# Embedded file name: microdrop\gui\plugin_manager_controller.py
# Compiled at: 2016-08-22 17:18:16
"""
...
"""
import os, subprocess, sys, platform, logging
from shutil import ignore_patterns
from zipfile import ZipFile
import tarfile, tempfile, gtk
...
from ..plugin_manager import IPlugin, implements, SingletonPlugin, PluginGlobals, get_service_instance, get_plugin_package_name, enable as enable_service, disable as disable_service
...
class PluginController(object):
    ...
    def __init__(self, controller, name):
        ...
        self.button_uninstall.connect('clicked', self.on_button_uninstall_clicked, None)
        self.button_update = gtk.Button('Update')
        self.button_update.connect('clicked', self.on_button_update_clicked, None)
        ...
        self.box.pack_end(self.button_update, expand=False, fill=False, padding=5)
        self.box.pack_end(self.button_uninstall, expand=False, fill=False, padding=5)
    ...
    def on_button_uninstall_clicked(self, widget, data=None):
        ...
        if response == gtk.RESPONSE_YES:
            ...
            if plugin_path.isdir():
                ...
                app.main_window_controller.info('%s plugin successfully removed.' % package_name, 'Uninstall plugin')
    ...
    def on_button_update_clicked(self, widget, data=None):
        ...
        try:
            ...
            logging.warning('Could not connect to plugin server: %s', app.get_app_value('server_url'))
        except (JSONRPCException, JSONDecodeException):
            logging.warning('Plugin %s not available on plugin server' % app.get_app_value('server_url'))
            return True
    ...
    def get_plugin_path(self, packge_name=None):
        ...
        app = get_app()
        return path(app.config.data['plugins']['directory']).joinpath(packge_name)
...
class PluginManagerController(SingletonPlugin):
    ...
    def update_plugin(self, plugin_controller, verbose=False, force=False):
        ...
        latest_version = Version(**p.latest_version(package_name, app_version={'major': 1, 'minor': 0, 
           'micro': 0}))
        if plugin_controller.version.tags != latest_version.tags:
            if yesno('The currently installed plugin (%s-%s) is from a different branch and may not be compatible with this version of Microdrop. Would you like to download a compatible version?' % (
             plugin_name,
             plugin_controller.version)) == gtk.RESPONSE_YES:
    ...
    def download_and_install_plugin(self, package_name, force=False):
        ...
        try:
            ...
            server_url = app.get_app_value('server_url')
            p = PluginRepository(server_url)
            p.download_latest(package_name, temp_dir, app_version={'major': 1, 'minor': 0, 
               'micro': 0})
    ...
    def update(self):
        ...
        for name in plugin_names:
            ...
            if p.get_plugin_path().abspath() in self.requested_deletions or not p.get_plugin_path().isdir():
                continue
            self.plugins.append(p)

        app = get_app()
        requested_deletion_path = path(app.config.data['plugins']['directory']).joinpath('requested_deletions.yml')
        requested_deletion_path.write_bytes(yaml.dump([ p.abspath() for p in self.requested_deletions
                                                      ]))
        rename_queue_path = path(app.config.data['plugins']['directory']).joinpath('rename_queue.yml')
        rename_queue_path.write_bytes(yaml.dump([ (p1.abspath(), p2.abspath()) for p1, p2 in self.rename_queue
                                                ]))
        post_install_queue_path = path(app.config.data['plugins']['directory']).joinpath('post_install_queue.yml')
        post_install_queue_path.write_bytes(yaml.dump([ p.abspath() for p in self.post_install_queue
                                                      ]))
    ...
    def update_all_plugins(self, force=False):
        ...
        for p in self.plugins:
            ...
            try:
                ...
                logging.info('Could not connect to plugin repository at: %s' % app.get_app_value('server_url'))
    ...
    def install_plugin(self, plugin_root, install_path):
        ...
        plugin_root.copytree(install_path, symlinks=True, ignore=ignore_patterns('*.pyc'))
    ...
    def post_uninstall(self, uninstall_path):
        ...
        if platform.system() in ('Linux', 'Darwin'):
            ...
            hooks_path = uninstall_path.joinpath('hooks', system_name).abspath()
    ...
    def verify_and_install_new_plugin(self, plugin_root, force=False):
        ...
        if plugin_metadata is None:
            ...
            app = get_app()
            installed_plugin_path = path(app.config.data['plugins']['directory']).joinpath(plugin_metadata.package_name)
            ...
            if installed_metadata:
                ...
                if installed_metadata.version.tags == plugin_metadata.version.tags and installed_metadata.version >= plugin_metadata.version:
                    message = 'Plugin %s is up-to-date (version %s).  Skipping installation.' % (
                     installed_metadata.plugin_name,
                     installed_metadata.version)
                    logging.info(message)
                    return
                message = 'A newer version (%s) of the %s plugin is available (current version=%s).' % (
                ...
                if force:
                    try:
                        ...
                        while installed_plugin_path.exists():
                            installed_plugin_path = path('%s%d' % (installed_plugin_path, count))
                ...
                logging.info('%s is not currently installed' % plugin_metadata.plugin_name)
            ...
            app.main_window_controller.info('%s plugin installed successfully.' % plugin_metadata.plugin_name, 'Install plugin')
            return True