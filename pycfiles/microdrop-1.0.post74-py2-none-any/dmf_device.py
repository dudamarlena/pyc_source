...
# [GCC 8.4.0]
# Embedded file name: microdrop\dmf_device.py
...
import numpy as np, pandas as pd, pint, svgwrite, yaml
...
class DmfDevice():
    ...
    def load_svg(cls, svg_path):
        ...
        if warning_list:
            logger.warning('The following paths could not be parsed properly and have been ignored:\n%s' % ('\n').join([ str(w.message) for w in warning_list ]))
        for p in path_group.paths.values():
            if p.color is None:
                p.color = (0, 0, 255)
            for loop in p.loops:
                d = sqrt((loop.verts[0][0] - loop.verts[(-1)][0]) ** 2 + (loop.verts[0][1] - loop.verts[(-1)][1]) ** 2)
                device_diag = sqrt(path_group.get_bounding_box()[2] ** 2 + path_group.get_bounding_box()[3] ** 2)
                if d / device_diag < 0.001:
                    loop.verts.pop()
    ...
    def load(cls, filename):
        ...
        out._upgrade()
        out.init_body_group()
        logger.debug('[DmfDevice].load() loaded in %f s.' % (time.time() - start_time))
        return out
    ...
    def to_svg(self):
        ...
        for i, e in self.electrodes.iteritems():
            ...
            p = Polygon([ (x - minx, y - miny) for x, y in e.path.loops[0].verts
                        ], fill=color, id=('electrode%d' % i), debug=False, **kwargs)
            dwg.add(p)
