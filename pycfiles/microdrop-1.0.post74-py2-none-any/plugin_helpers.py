...
# [GCC 8.4.0]
# Embedded file name: microdrop\plugin_helpers.py
...
PluginMetaData = namedtuple('PluginMetaData', 'package_name plugin_name version')
...
def get_plugin_info(plugin_root):
    ...
    if not properties.isfile():
        ...
        plugin_metadata = PluginMetaData.from_dict(yaml.load(properties.bytes()))
...
class AppDataController(object):
    ...
    def on_plugin_enable(self):
        ...
        if self.name not in app.config.data:
    ...
    def get_app_value(self, key):
        ...
        return values_dict.get(key, self.AppFields.field_schema_mapping[key].default)
        ...
        if not values_dict:
            ...
            for k in values_dict.keys():
                if k not in self.AppFields.field_schema_mapping.keys():
                    logger.error('Invalid key (%s) in configuration file section: [%s].' % (
                     k, self.name))
                    values_dict.pop(k)
            ...
            values = dict([ (k, v.value) for k, v in elements.iteritems() if v.value is not None
                          ])
            app = get_app()
...
class StepOptionsController(object):
    ...
    def get_default_step_options(self):
        if self.get_step_form_class() is None:
            ...
            return dict([ (k, v.value) for k, v in self.StepFields.from_defaults().iteritems()
                        ])
    ...
    def get_step_value(self, name, step_number=None):
        ...
        if options:
            if name not in options and name in self.StepFields.field_schema_mapping:
    ...
    def set_step_values(self, values_dict, step_number=None):
        ...
        logger.debug('[StepOptionsController] set_step[%d]_values(): values_dict=%s' % (
         step_number, values_dict))
        validate_dict = dict([ (k, v) for k, v in values_dict.iteritems() if k in self.StepFields.field_schema_mapping
                             ])
        ...
        if values:
            ...
            emit_signal('on_step_options_changed', [self.name, step_number], interface=IPlugin)
        return
