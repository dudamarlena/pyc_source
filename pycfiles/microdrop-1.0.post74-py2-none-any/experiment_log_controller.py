...
# [GCC 8.4.0]
# Embedded file name: microdrop\gui\experiment_log_controller.py
...
from microdrop_utility.gui import combobox_set_model_from_list, combobox_get_active_text, textview_get_text
from ..experiment_log import ExperimentLog
from ..plugin_manager import IPlugin, SingletonPlugin, implements, PluginGlobals, emit_signal, ScheduleRequest, get_service_names, get_service_instance_by_name
    ...
    def popup(self, event):
        for child in self.menu_popup.get_children():
            if child.get_visible():
                self.menu_popup.popup(None, None, None, event.button, event.time, None)
                break
...
class ExperimentLogController(SingletonPlugin, AppDataController):
    ...
    @property
    def AppFields(self):
        return Form.of(Directory.named('notebook_directory').using(default='', optional=True))
    ...
    def apply_notebook_dir(self, notebook_directory):
        ...
        if self.previous_notebook_dir and notebook_directory == self.previous_notebook_dir:
            return False
        ...
        if self.previous_notebook_dir:
            ...
            skeleton_dir = path(pkg_resources.resource_filename('microdrop', 'static'))
    ...
    def update(self):
        ...
        if not app.experiment_log:
            ...
            try:
                ...
                self.results = self.Results(ExperimentLog.load(log), Protocol.load(protocol), DmfDevice.load(dmf_device))
                ...
                self.builder.get_object('label_software_version').set_text(label)
                label = 'Device: '
                ...
                self.builder.get_object('label_device').set_text(label)
                ...
                self.builder.get_object('label_protocol').set_text(label)
                label = 'Control board: '
                ...
                self.builder.get_object('label_control_board').set_text(label)
                ...
                self.builder.get_object('label_plugins').set_text(label)
                ...
                self.builder.get_object('label_experiment_time').set_text(label)
                label = ''
                ...
                self.builder.get_object('textview_notes').get_buffer().set_text(label)
                ...
                for d in self.results.log.data:
                    if 'step' in d['core'].keys() and 'time' in d['core'].keys():
                        if d['core']['step'] < len(self.results.protocol):
                            ...
                            dmf_plugin_name = step.plugin_name_lookup('wheelerlab.dmf_control_board', re_pattern=True)
    ...
    def save(self):
        ...
        if all([hasattr(app, 'experiment_log'),
         app.experiment_log is not None, [ x for x in app.experiment_log.get('step') if x is not None ]]):
            data = {'software version': app.version}
            data['device name'] = app.dmf_device.name
            data['protocol name'] = app.protocol.name
            data['notes'] = textview_get_text(app.protocol_controller.builder.get_object('textview_notes'))
            plugin_versions = {}
    ...
    def on_button_load_device_clicked(self, widget, data=None):
        app = get_app()
        filename = path(os.path.join(app.experiment_log.directory, str(self.results.log.experiment_id), 'device'))
        try:
    ...
    def on_button_load_protocol_clicked(self, widget, data=None):
        app = get_app()
        filename = path(os.path.join(app.experiment_log.directory, str(self.results.log.experiment_id), 'protocol'))
    ...
    def on_textview_notes_focus_out_event(self, widget, data=None):
        ...
        self.results.log.data[(-1)]['core']['notes'] = textview_get_text(self.builder.get_object('textview_notes'))
        filename = os.path.join(self.results.log.directory, str(self.results.log.experiment_id), 'data')
    ...
    def on_dmf_device_swapped(self, old_dmf_device, dmf_device):
        ...
        if dmf_device and dmf_device.name:
            device_path = os.path.join(app.get_device_directory(), dmf_device.name, 'logs')
    ...
    def _add_list_column(self, title, columnId, format_string=None):
        ...
        if format_string:
            column.set_cell_data_func(cell, self._cell_renderer_format, format_string)
