...
# [GCC 8.4.0]
# Embedded file name: microdrop\gui\protocol_grid_controller.py
...
import gtk
from pygtkhelpers.ui.objectlist.combined_fields import CombinedFields, CombinedRow, RowFields
from microdrop_utility.gui import register_shortcuts, get_accel_group
from microdrop.plugin_manager import ExtensionPoint, IPlugin, SingletonPlugin, implements, PluginGlobals, ScheduleRequest, emit_signal
from microdrop.app_context import get_app

class ProtocolGridView(CombinedFields):

    def __init__(self, forms, enabled_attrs, *args, **kwargs):
        super(ProtocolGridView, self).__init__(forms, enabled_attrs, *args, **kwargs)
    ...
    def _on_step_options_changed(self, form_name, step_number):
        ...
        if hasattr(service, 'get_step_values'):
            ...
            logging.debug('[ProtocolGridView] _on_step_options_changed(): plugin_name=%s step_number=%s attrs=%s' % (
    ...
    def _get_popup_menu(self, item, column_title, value, row_ids, menu_items=None):
        ...
        menu_items += [(None, None)]
        return super(ProtocolGridView, self)._get_popup_menu(item, column_title, value, row_ids, menu_items)
    ...
    def on_selection_changed(self, grid_view):
        if self.selected_ids:
            logging.debug('[ProtocolGridView].on_selection_changed: selected_ids=%s', self.selected_ids)
            app = get_app()
            logging.debug('\tcurrent_step_number=%d', app.protocol.current_step_number)
...
class ProtocolGridController(SingletonPlugin):
    ...
    def update_grid(self, protocol=None):
        ...
        if protocol is None:
            ...
            forms = dict([ (k, f) for k, f in emit_signal('get_step_form_class').iteritems() if f is not None
                         ])
            steps = protocol.steps
            logging.debug('[ProtocolGridController]   forms=%s steps=%s', forms, steps)
            if self.enabled_fields is None:
                self._enabled_fields = dict([ (form_name, set(form.field_schema_mapping.keys())) for form_name, form in forms.items()
                                            ])
            ...
            if self.widget:
                ...
                self.accel_group = self._create_accel_group(app.main_window_controller.view)
                app.main_window_controller.view.add_accel_group(self.accel_group)
            else:
                self.accel_group = None
            self.widget.connect('editing-started', lambda *args: app.main_window_controller.disable_keyboard_shortcuts())
            self.widget.connect('editing-done', lambda *args: app.main_window_controller.enable_keyboard_shortcuts())
            return
    ...
    def _create_accel_group(self, widget):
        ...
        shortcuts = {'<Control>c': self.widget.copy_rows, 
           ...
           '<Control><Shift>i': FocusWrapper(self, lambda : app.protocol.insert_step())}
        return get_accel_group(widget, shortcuts, enabled_widgets=[
         self.widget])
    ...
    def on_step_swapped(self, original_step_number, step_number):
        logging.debug('[ProtocolGridController] on_step_swapped[%d->%d]', original_step_number, step_number)
        if self.widget:
            self.widget.select_row(get_app().protocol.current_step_number)

    def on_step_removed(self, step_number, step):
        logging.debug('[ProtocolGridController] on_step_removed[%d]', step_number)
        self.update_grid()
