...
class Story(PolymorphicModel):
    ...
    def api_url(self):
        return reverse(self.api_detail_name, kwargs={'pk': self.pk})
    ...
    def create(cls, author, **kwargs):
        with reversion.create_revision():
            created_object = cls.objects.create(author=author, **kwargs)
            reversion.set_user(author)
            reversion.set_comment('Created {}'.format(cls.typename))
    ...
    def update(self, user, data):
        with reversion.create_revision():
            ...
            reversion.set_comment('Edited {} through web'.format(self.typename))
    ...
    def difference_tables_between_revisions(self, older, newer):
        ...
        for key, diffs in deltas.items():
            differences[key] = bootstrap_diffs(str(diffs['old']).split('\n'), str(diffs['new']).split('\n'))
