...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fab_addon_geoalchemy/fields.py
# Compiled at: 2018-10-02 00:17:38
...
class PointField(GeometryField):
    ...
    def _getpoint(self, lat, lon):
        if lat is unset_value or lon is unset_value or lat is None or lon is None or lat == '' or lon == '':
            return
        point = 'SRID={srid};POINT({lon} {lat})'.format(lat=lat, lon=lon, srid=self.srid)
        log.debug('returning point for {lat}, {lon}: {point}'.format(lat=lat, lon=lon, point=point))
        return point
    ...
    def process(self, formdata, data=unset_value, obj=None):
        ...
        self.process_errors = []
        log.debug('Processing field {} with data: {}, obj: {} and formdata: {}'.format(self.name, data, obj, formdata))
        if data is unset_value:
            ...
            if formdata is not None:
                if lonname in formdata and latname in formdata:
                    ...
                    self.raw_data = [
                     self._getpoint(formdata.get(latname), formdata.get(lonname))]
