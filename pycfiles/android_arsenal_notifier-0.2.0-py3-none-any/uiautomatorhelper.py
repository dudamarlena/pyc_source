...
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/com/dtmilano/android/uiautomator/uiautomatorhelper.py
...
__version__ = '15.8.1'
import json, os, platform, re, subprocess, sys, threading
try:
    import requests
...
class UiAutomatorHelper():
    ...
    def findObject(self, **kwargs):
        ...
        if r['status'] == 'OK':
            if DEBUG:
                print >> sys.stderr, 'UiAutomatorHelper: findObject: returning', int(r['oid'])
    ...
    def getText(self, uiObject=None):
        ...
        if r['status'] == 'OK':
            if DEBUG:
                print >> sys.stderr, 'UiAutomatorHelper: getText: returning', r['text']
            return r['text']
        else:
            raise RuntimeError('Error: ' + response)
            return
    ...
    def uiScrollable(self, path, params=None):
        ...
        if r['status'] == 'OK':
            if 'oid' in r:
                if DEBUG:
                    print >> sys.stderr, 'UiAutomatorHelper: uiScrollable: returning', int(r['oid'])
                return (int(r['oid']), r)
            else:
                return r
...
class UiScrollable():
    ...
    def getChildByDescription(self, uiSelector, description, allowScrollSearch):
        oid, response = self.uiAutomatorHelper.uiScrollable(str(self.oid) + '/getChildByDescription', {'uiSelector': uiSelector, 'contentDescription': description, 
