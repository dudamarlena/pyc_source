...
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/com/dtmilano/android/adb/adbclient.py
...
import subprocess, threading, unicodedata
...
import string, datetime, struct, cStringIO as StringIO, socket, time, re, os, platform
from com.dtmilano.android.window import Window
from com.dtmilano.android.common import _nd, _nh, _ns, obtainPxPy, obtainVxVy, obtainVwVh, profileStart, profileEnd
...
class WifiManager():
    ...
    def getWifiState(self):
        ...
        if result:
            ...
            if self.WIFI_IS_DISABLED_RE.match(state):
...
class AdbClient():
    ...
    def checkVersion(self, ignoreversioncheck=False, reconnect=True):
        ...
        VALID_ADB_VERSIONS = [
         '00040029', '00040028', '00040027', '00040024', '00040023', '00040020', '0004001f']
        if version not in VALID_ADB_VERSIONS and not ignoreversioncheck:
    ...
    def startActivity(self, component=None, flags=None, uri=None):
        ...
        out = self.shell(cmd)
        if re.search("(Error type)|(Error: )|(Cannot find 'App')", out, re.IGNORECASE | re.MULTILINE):
    ...
    def takeSnapshot(self, reconnect=False):
        ...
        if USE_ADB_FRAMEBUFFER_METHOD:
            ...
            received = self.__receive(4 + 48)
            version, bpp, size, width, height, roffset, rlen, boffset, blen, goffset, glen, aoffset, alen = struct.unpack('<' + 'LLLLLLLLLLLLL', received)
            if version == 2:
                received += self.__receive(4)
                version, bpp, colorspace, size, width, height, roffset, rlen, boffset, blen, goffset, glen, aoffset, alen = struct.unpack('<' + 'LLLLLLLLLLLLLL', received)
            if DEBUG:
                if version == 1:
            ...
            if bpp == 16:
                ...
                mode = argMode
            self.__send('\x00', checkok=False, reconnect=False)
            if DEBUG:
    ...
    def substituteDeviceTemplate(self, template):
        ...
        if osName.startswith('Windows'):
            timestamp.replace(':', '_')
        _map = {'serialno': serialno, 'focusedwindowname': focusedWindowName, 
           'timestamp': timestamp}
