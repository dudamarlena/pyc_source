...
# Embedded file name: build/bdist.linux-x86_64/egg/commonslib/dateutil.py
...
def offset_time(date_time):
    ...
    if dis < 2:
        ...
        distance = str(int(dis / onesecond)) + '秒前'
    elif dis / onehour < 1:
        distance = str(int(dis / oneminute)) + '分钟前'
    elif dis / oneday < 1:
        distance = str(int(dis / onehour)) + '小时前'
    elif dis / oneweek < 1:
        distance = str(int(dis / oneday)) + '天前'
    elif dis / onemonth < 1:
        distance = str(int(dis / oneweek)) + '周前'
    elif dis / onemonth < 12:
        distance = str(int(dis / onemonth)) + '个月前'
    else:
        distance = time.strftime('Y年m月d日', date_time)
    return distance
...
if __name__ == '__main__':
    ...
    print timestamp_to_string(string_to_timestamp('2014-12-12', '%Y-%m-%d') + 86400000, '%Y-%m-%d %H:%M:%S')