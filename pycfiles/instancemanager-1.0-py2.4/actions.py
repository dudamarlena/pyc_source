...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/instancemanager/actions.py
...
import actionutils, config, logging, os, os.path, shutil, sys, tempfile, urllib2, base64
...
class Action:
    ...
    def addOption(self, optionparser):
        optionparser.add_option(self.option, self.alternativeOption, help=self.name, action=self.kindOfAction, default=False)
...
class ArgumentAction(Action):
    ...
    def addOption(self, optionparser):
        optionparser.add_option(self.option, self.alternativeOption, help=self.name, metavar=self.metavar, action=self.kindOfAction)
...
class CopyDatafsAction(Action):
    ...
    def run(self, options=None):
        ...
        if not os.path.exists(source):
            log.info('No prepared Data.fs found at %s, skipping this step.', source)
            return
        else:
    ...
    def run(self, options=None):
        ...
        for source in self.conf.sources(instanceDir):
            try:
                ...
                log.warn('IOError %r when adding %s. Please fix this.', str(e), source.sourceName)

        actionutils.setUpWorkingenv(self.conf, instanceDir)
        log.info('Rebuilt the %s/Products/ and %s/lib/python directories from scratch.', instanceDir, instanceDir)
        for instanceDir in instanceDirs[1:]:
            ...
            log.info('Rebuilt the %s/Products/ directory from scratch.', instanceDir)
            ...
            log.info('Rebuilt the %s/lib/python/ directory from scratch.', instanceDir)
    ...
    def run(self, options=None):
        ...
        for source in self.conf.sources(instanceDirs[0]):
            try:
                ...
                log.warn('IOError %r when updating %s. Please fix this.', str(e), source.sourceName)
    ...
    def run(self, options=None):
        ...
        if self.conf.symlinkableConfiguration():
            ...
            if not os.path.exists(src):
                log.critical('%s does not exist, so I cannot symlink it. Exiting.', src)
                sys.exit(1)
            target = os.path.join(self.conf.configDir, '%s.py' % project)
            if os.path.exists(target):
                log.warn('%s already exists, not overriding it.', target)
            else:
                log.info('Linking config file %s to %s.', src, target)
                os.symlink(src, target)
        localFile = os.path.join(self.conf.configDir, 'local-%s.py' % project)
        if self.conf.contentForLocal:
            if os.path.exists(localFile):
                log.warning('%s already exists, not overriding it.', localFile)
            else:
        ...
        print "'instancemanager %s fresh' should set you up completely." % project
        if self.conf.contentForLocal:
            print 'Look at %s and customize it to your need, first.' % localFile
        print
...
class TestAction(ArgumentAction):
    ...
    def testCommand(self, product):
        ...
        if product == 'MAIN':
            products = actionutils.adaptListForDevelopment(self.conf, self.conf.configData['main_products'])
        else:
            products = [
             product]
...
class QuickreinstallAction(Action):
    ...
    def run(self, options=None):
        ...
        arguments.append(reinstallScript)
        arguments.append(ploneSiteName)
        arguments.append(adminUser)
        arguments += actionutils.adaptListForDevelopment(self.conf, self.conf.configData['main_products'])
        arguments += actionutils.adaptListForDevelopment(self.conf, self.conf.configData['generic_setup_profiles'])
...
class UninstallAction(ArgumentAction):
    ...
    def run(self, options=None):
        ...
        arguments = []
        arguments.append('run')
        arguments.append(uninstallScript)
        arguments.append(ploneSiteName)
        arguments.append(adminUser)
    ...
    def run(self, options=None):
        ...
        for instanceDir in instanceDirs:
            ...
            if os.path.exists(instanceDir):
                ...
                log.debug("Instance dir %s didn't exist.", instanceDir)
        ...
        project = self.conf.configData['project']
        imConfig = os.path.join(self.conf.configDir, '%s.py' % project)
        if os.path.exists(imConfig):
            shutil.os.remove(imConfig)
            log.info('Removed im config file %s.', imConfig)
        else:
            log.debug('Im config file %s did not exist.', imConfig)
        localFile = os.path.join(self.conf.configDir, 'local-%s.py' % project)
        if os.path.exists(localFile):
...
class PackAction(Action):
    ...
    def run(self, options=None):
        ...
        if not self.conf.configData['use_zeo']:
            ...
            auth = base64.encodestring('%(user)s:%(password)s' % self.conf.configData)
            req.add_header('Authorization', 'Basic %s' % auth)
            req.add_data('POST')
            log.info('Calling Database Pack, keeping only changes of last %(pack_days)s day(s)...' % self.conf.configData)
            try:
                ...
                log.info('Database Pack called with result: %s: %s', info.code, info.msg)
        else:
            packScript = self.scriptLocation()
            arguments = []
            arguments.append('run')
            arguments.append(packScript)
            arguments += days
            command = (' ').join(arguments)
            actionutils.runZopectl(self.conf, command)
...
class ChangeOwnershipAction(Action):
    ...
    def run(self, options=None):
        ...
        arguments = []
        arguments.append('run')
        arguments.append(changeownScript)
        ploneSiteName = self.conf.configData['plone_site_name']
        arguments.append(ploneSiteName)
...
class BackupZopeAction(Action):
    ...
    def run(self, options=None):
        ...
        actionutils.backup(conf=self.conf, sourceDatafs=source, targetDir=target, full=False)
...
class RepozoAction(ArgumentAction):
    ...
    option = '--repozo'
    choices = ['sync', 'snapshot', 'restoresnapshot', 'full']

    def addOption(self, optionparser):
        optionparser.add_option(self.option, self.alternativeOption, help=self.name, action=self.kindOfAction, default=False, choices=self.choices)
    ...
    def full(self, options=None):
        target = self.conf.backupBaseDir()
        source = self.conf.databasePath()
        actionutils.backup(conf=self.conf, sourceDatafs=source, targetDir=target, full=True)
    ...
    def restoresnapshot(self, options=None):
        ...
        actionutils.restore(conf=self.conf, sourceDir=sourceDir)
...
class RestoreDateZopeAction(ArgumentAction):
    ...
    def run(self, options=None):
        ...
        actionutils.restore(conf=self.conf, sourceDir=sourceDir, fromTime=fromTime)
...
class RewriteruleAction(Action):
    ...
    def run(self, options=None):
        ...
        sn = '%{SERVER_NAME}'
        output = config.APACHE_TEMPLATE % {'plonesite': ploneSiteName, 'zopeport': zopePort, 'sn': sn}
        print output
...
class ActivateWorkingenvAction(Action):
    ...
    def run(self, options=None):
        if not self.conf.configData['use_workingenv']:
            log.error('--activate is part of workingenv. You have not enabled that (use_workingenv = True).')
            return
        instanceDir = self.conf.instanceDir()
        activateCommand = os.path.join(instanceDir, 'bin', 'activate')
        if not os.path.exists(activateCommand):
            log.error("Workingenv's 'activate' doesn't exist. Did you run --upgradezope to install it?")
            return
        log.debug("Printing out '%s' for --activate.", activateCommand)
...
class IpythonAction(Action):
    ...
    def run(self, options=None):
        ...
        softwarehome = os.path.join(self.conf.zopeDir(), 'lib', 'python')
...
def getActions():
    ...
    actions = [ moduleContents[key] for key in moduleContents.keys() if key.endswith('Action') ]
    actions = [ action for action in actions if action.option ]
