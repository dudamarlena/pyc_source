...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/instancemanager/actionutils.py
...
import config, logging, os, os.path, re, shutil, sys, time, utils, workingenv
...
def runZopectl(conf, command):
    ...
    for instanceDir in instanceDirs:
        zopectlCommand = os.path.join(instanceDir, 'bin', 'zopectl')
...
def runZeoctl(conf, command):
    if not conf.configData['use_zeo']:
        return
    zeoctlCommand = os.path.join(conf.zeoDir(), 'bin', 'zeoctl')
    ...
    if os.path.exists(zeoctlCommand):
        ...
        if command == 'status':
            ...
            log.info('Sleeping %s seconds to make sure.', zeoTimeout)
...
def windowsService(conf, command, options=''):
    zServer = os.path.join(conf.instanceDir(), 'bin', 'zopeservice.py')
    zServerPath = '"%s"' % zServer
    if 'install' in command:
        options = '--startup auto'
    commandLine = (' ').join([conf.configData['python'], zServerPath, options, command])
...
def logInfoAboutCtl(what='zope', conf=None, command=None, instanceDir=None):
    ...
    log.info('%s: %s on port %s.', what, command, port)
...
def setUpWorkingenv(conf, instanceDir):
    if conf.configData['use_workingenv']:
        ...
        try:
            workingenv.main(['--site-packages', '--home', '--no-extra', target])
        except URLError, e:
            log.error("Workingenv could not be installed because of an URLError (%s). Most common cause: you don't have a network connection at the moment.", e)
...
def createInstance(conf, purge=False):
    ...
    mkInstance = os.path.join(conf.zopeDir(), 'bin', command)
    if conf.configData['is_windows']:
    ...
    for instanceDir in instanceDirs:
        ...
        params = '-u "%s:%s" -d %s' % (conf.configData['user'], conf.configData['password'], instancePath)
    ...
    if useZeo:
        ...
        mkInstance = os.path.join(conf.zopeDir(), 'bin', command)
        params = '%s %s' % (zeoDir, conf.zeoPort())
        command = (' ').join([python, mkInstance, params])
        (child_stdout, child_stdin, child_stderr) = os.popen3(command)
        error = child_stderr.readlines()
        if error:
            ...
            sys.exit(('').join(error))
        log.info("Created zeo instance in '%s'.", conf.zeoDir())
...
def handleZopeConf(conf, instanceDir, useZeo=False):
    ...
    for segmentId in config.CONFSNIPPETS.keys():
        openTagPatterns.append(re.compile('\\s*(#?)\\s*<(%s)\\s*>\\s*(\\S*)' % segmentId))
        closeTagPatterns.append(re.compile('\\s*(#?)\\s*</(%s)>\\s*(\\S*)' % segmentId))
    ...
    for (idx, line) in enumerate(oldFile):
        ...
        for pattern in openTagPatterns:
            ...
            if text and not text.startswith('#'):
                continue
            log.debug('Opening tag pattern %r matched: %r.', matchName, line)
            trappedLines = []
            isTrapping = True
            lineSaved = True
            startTag = matchName
        ...
        for pattern in closeTagPatterns:
            ...
            if not matchName == startTag:
                log.debug("The close tag (%s) doesn't match the opening tag (%s): %r.", startTag, matchName, line)
                continue
            text = reMatch.groups()[2]
            isCommented = reMatch.groups()[0] == '#'
            if text and not text.startswith('#'):
                continue
            log.debug('Closing tag pattern %r matched: %r.', matchName, line)
            ...
            if conf.configData[checkThis]:
                log.debug('The section %s can be included.', matchName)
            else:
                log.debug('The section %s wil be excluded.', matchName)
                disableThisSegment = True
            if disableThisSegment:
                ...
                for thisLine in trappedLines:
                    newLine = thisLine
                    for (searchThis, replacement) in config.CONFSNIPPETS[matchName].items():
                        if searchThis in thisLine:
                            newLine = replacement % conf.configData
                            if newLine[(-1)] != '\n':
                                newLine += '\n'
                            log.debug('Replaced %r with %r.', thisLine, newLine)
                            break
        ...
        if lineSaved:
            ...
            continue
        if conf.configData['is_windows']:
            conf.configData['zopeConfigChanges']['products $PRODUCTS'] = '# edited by ' + 'instance manager:\nproducts $INSTANCE/Products\n'
    ...
    if useZeo:
        ...
        newFile.write(snippet % {'zeoport': conf.zeoPort(), 'zeo_client_line': zeo_client_line})
...
def backup(conf=None, sourceDatafs=None, targetDir=None, full=False):
    if not os.path.exists(targetDir):
        log.info('%s does not exist, creating it now.', targetDir)
    ...
    log.info('Backing up database file: %s to %s', sourceDatafs, targetDir)
    log.debug('Command used: %s', command)
    os.system(command)
    log.debug('Trying to clean up old backups.')
    filenames = os.listdir(targetDir)
    log.debug('Looked up filenames in the target dir: %s found. %r.', len(filenames), filenames)
    ...
    log.debug('Filtered out full backups (*.fs): %r.', [ f[0] for f in fullbackups ])
    if len(fullbackups) > num_backups and num_backups != 0:
        ...
        fullbackups.reverse()
        log.debug('Full backups, sorted by date, newest first: %r.', [ f[0] for f in fullbackups ])
        ...
        log.debug('The oldest backup we get to keep is from %s.', last_date_to_keep)
        ...
        log.info('Removed old backups, the latest %s full backups have been kept.', str(num_backups))
    else:
        log.debug('Not removing backups.')
        if len(fullbackups) <= num_backups:
            log.debug('Reason: #backups (%s) <= than max (%s).', len(fullbackups), num_backups)
        if num_backups == 0:
            log.debug('Reason: max # of backups is 0, so that is a sign to us to not remove backups.')


def restore(conf=None, sourceDir=None, fromTime=None):
    useZeo = conf.configData['use_zeo']
    if useZeo:
    ...
    os.environ.update(env)
    command = (' ').join([python, repozo] + arguments)
    log.info('Restoring database file %s from %s.', targetDatafs, sourceDir)
    log.debug('Command used: %s', command)
    os.system(command)
...
def adaptListForDevelopment(config, itemList, simple=True):
    ...
    if config.configData['development_machine']:
        ...
        safeItems = [ item for item in itemList if not isPureDevelopmentItem(item) ]
    if simple:
        simpleList = []
