...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/instancemanager/configuration.py
# Compiled at: 2007-12-17 05:32:50
"""Provides Configuration class that reads the configuration files.
"""
import config, imp, logging, os, os.path, shutil, sources, sys, tempfile, types, urllib, utils
from config import DEBUGCONFIGCHANGES
log = logging.getLogger('configuration')
renamedAttrs = {'tgz_sources': 'archive_sources', 'tgz_basedir_template': 'archive_basedir_template', 'tgzbundle_sources': 'archivebundle_sources', 'tgzbundle_basedir_template': 'archivebundle_basedir_template'}
...
class Configuration(object):
    ...
    def _checkConfigDir(self):
        ...
        self.configDir = os.path.join(userDir, config.CONFIGDIR)
        ...
        skeletonFiles = [ f for f in os.listdir(skeletonDir) if f.endswith('.txt') or f.endswith('.py') ]
        log.debug('Found the following skeleton files: %r.', skeletonFiles)
        for skeletonFile in skeletonFiles:
            ...
            if not os.path.exists(target):
                ...
                log.debug("Copying of skeleton file '%s' not needed.", target)
    ...
    def _loadConfigData(self, project=None):
       ...
       The variables imported from these files should be placed in
        ...
        siteDefaults = self._loadUserConfigModule('SiteDefaults', absolutePath=config.SITECONFIG)
        if siteDefaults:
        ...
        if userDefaults:
            ...
            log.critical('User defaults could not be read from your config directory.')
            sys.exit(1)
        ...
        if project:
            ...
            if projectDefaults:
                ...
                log.warn('Project config file for %s not found in your config directory.', project)
            secretProjectDefaults = self._loadUserConfigModule(config.SECRET_PREFIX + project)
            if secretProjectDefaults:
                self._collectAttributes(secretProjectDefaults)
            else:
                log.debug('No secret project config file.')
        else:
            log.debug('No project config file. We could be bootstrapping.')
    ...
    def _loadUserConfigModule(self, name, absolutePath=None):
        ...
        if absolutePath:
            ...
            log.debug("Trying to load config file at '%s'.", moduleFile)
        else:
    ...
    def _zopeConfigReplacements(self):
        ...
        if listenIpAddress:
            ...
            log.debug('Added desired ip-address change to list of config changes.')
    ...
    def zeoClientPort(self, dir):
        ...
        """
        ports = [ client['port'] for client in self.zeoClients() if client['dir'] == dir ]
        if len(ports) == 0:
    ...
    def product_sources(self, instanceDir):
        ...
        """
        sourceListNames = [ s for s in self.configData.keys() if s.endswith('_sources') if not s.startswith('config') ]
    ...
    def config_sources(self, instanceDir):
        ...
        """
        sourceListNames = [ s for s in self.configData.keys() if s.endswith('_sources') if s.startswith('config') ]
        return self._createWrappedSourcesList(instanceDir, sourceListNames)
    ...
    def _createWrappedSourcesList(self, instanceDir, names):
        ...
        for sourceListName in names:
            ...
            log.debug('Looking for %s candidates: %r.', klassName, sourceList)
            for source in sourceList:
                wrappedSource = klass(sourceConfig=source, config=self, instanceDir=instanceDir)
...
class BootstrapConfiguration(Configuration):
    ...
    def _loadBootstrapData(self, filename):
        bootstrapData = self._loadUserConfigModule('bootstrap', absolutePath=filename)
        if bootstrapData:
    ...
    def configSource(self, instanceDir):
        ...
        """
        imsource = sources.ConfigSource(sourceConfig=self.configData['config_sources'][0], config=self, instanceDir=instanceDir)
        return imsource
    ...
    def symlinkableConfiguration(self):
        ...
        if not config:
            return
        return os.path.join(self.configBaseDir(), config)
