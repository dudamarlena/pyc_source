...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/instancemanager/sources.py
...
"""
import logging, os, os.path, shutil, sys, tempfile, utils, urllib
try:
    import pysvn
    USE_PYSVN = True
except:
...
def sourceNameFromUrl(url):
    ...
    """
    log.debug('Extracting source name from string %r by checking if it is a url and thus needs special treatment.', url)
    parts = url.split('/')
    if len(parts) < 2:
    ...
    for indicator in svnIndicators:
        if url.startswith(indicator):
            log.debug('String starts with %r, possible url.', indicator)
            weRecognizeIt = True
    ...
    for indicator in zipIndicators:
        if url.endswith(indicator):
            result = parts[(-1)]
            log.debug('Possible zipfile (it ends with %r), returning the last part: %r.', indicator, result)
            return result

    log.debug("We checked, it doesn't seem to be a zipfile. Continuing.")
    if parts[(-1)] == 'trunk':
        result = parts[(-2)]
        log.debug("It ends with '/trunk', returning part before that: %r.", result)
        return result
    if parts[(-2)] in ['tags', 'branches']:
        result = parts[(-3)]
        lastPart = ('/').join(parts[-2:])
        log.debug('It ends with %r, so returning part before that: %r.', lastPart, result)
        return result
    return parts[(-1)]
...
class BaseSource(object):
    ...
    def __init__(self, sourceConfig, config, instanceDir):
        if type(sourceConfig) == type(dict()):
            ...
            self.deleteExisting = sourceConfig.get('deleteExisting', True)
            if self.sourceName is None and self.url is not None:
                self.sourceName = sourceNameFromUrl(self.url)
            if self.sourceName is None:
                log.critical('Cannot find a "source" key nor a "url" key, so I cannot deduct a source name. (%r).', sourceConfig)
                sys.exit(1)
        ...
        if type(sourceConfig) == type(''):
            ...
            if self.sourceName != sourceConfig:
                log.debug("We could extract a url from the sourceconfig. Now we'll set the url with the original value.")
                self.url = sourceConfig
                log.debug('Source name set to %r, url to %r.', self.sourceName, self.url)
        ...
        if not self.sourceName:
            log.error("Failed to add %s source: '%s'.", self.name, sourceConfig)
        self.config = config
        self.sourceConfig = sourceConfig
        log.debug("New %s source: '%s' - develop: %s, drop: %s, internalBundles: %s", self.name, self.sourceName, self.develop, self.drop, self.internalBundles)
    ...
    def _canSafelyBeInstalled(self):
        ...
        if self.develop == False:
            ...
            return True
        log.debug('Product %s: development product on non-development machine, so NOT safe for install.', self.sourceName)
        return False
    ...
    def _removeTarget(self, target):
        ...
        if target.lower().endswith('.txt'):
            log.debug('%s is a .txt file, silently removing it without warning.')
        else:
            log.info("'%s' exists, removing it before using %s from %s.", target, self.name, self.sourceName)
    ...
    def prepareInstall(self, target):
        ...
        if not os.path.exists(target):
            log.debug("%s doesn't exist, go ahead with an install.", target)
            return True
        if self.deleteExisting:
            self._removeTarget(target)
            log.debug('Target %s removed, go ahead with an install.', target)
            return True
        log.debug("Target %s exists and we're not supposed to delete it, stop the install of this one.", target)
        return False
    ...
    def updateSource(self, manifest=None):
        ...
        if not os.path.exists(source):
            if self.url:
                log.warn('Source %s not found, attempting download.', source)
                self.attemptDownload()
            else:
                log.critical('Source %s not found, also no url where to download it.', source)
                sys.exit(1)
        else:
            self._updateSource()
    ...
    def _buildManifest(self, nestedSource, target):
        ...
        item = (
         self.name, self._source(), nestedSource, target, version)
        trace.append(item)
...
class SymlinkSource(BaseSource):
    ...
    def _updateSource(self):
        if not self.url:
            log.info('No svn/cvs url set for source %s. Attempting an update anyway, it might be part of a bundle, for instance. Errors are ok here.', self.sourceName)
        target = self._source()
        versionoption = self.version and '-r %s' % self.version or ''
        if USE_PYSVN and not self.usecvs:
            client = pysvn.Client()
            try:
                revisionnumber = self.version and pysvn.Revision(pysvn.opt_revision_kind.number, self.version) or pysvn.Revision(pysvn.opt_revision_kind.head)
                ...
                if revision and revision[0] != -1:
                    log.info('%s at revision %r', target, revision[0].number)
                else:
                    log.warn('Something is wrong with your working copy of %s', target)
    ...
    def attemptDownload(self):
        if not self.url:
            log.info('Set url for symlink for source %s.' % self.sourceName)
            return
        target = self._source()
        ...
        if self.usecvs:
            ...
            command = 'cvs -z3 -d %s co %s -d %s %s' % (cvsroot, versionoption, relativetmp, cvsmodule)
            log.debug('commandline: %s', command)
            os.system(command)
            os.rename(relativetmp, target)
            message = 'Checked out %s from %s ' % (cvsmodule, cvsroot)
            message += versionoption and 'version %s.' % self.version or '.'
            log.info(message)
        else:
...
class SymlinkbundleSource(SymlinkSource):
    ...
    def _addProduct(self):
        ...
        includedFiles = [ f for f in includedFiles if not f.startswith('.') ]
        log.debug('Found this in the symlink bundle dir: %r.', includedFiles)
        for includedFile in includedFiles:
            if includedFile.endswith('.OLD'):
                log.warn('Svn .OLD file, not linking it: %s.', includedFile)
                continue
            if includedFile in self.drop:
                log.warn('file %s in droplist, not linking it.', includedFile)
                continue
    ...
    def moveDirs(self, tempDir):
        try:
            ...
            if self.subPath:
                ...
                log.debug("Appended sub-path '%s' to the extration location.", self.subPath)
            ...
            if extractedDirectory in self.drop:
                log.warn('Directory %s in droplist for %s, not installing it.' % (extractedDirectory, self.sourceName))
                return
            if self.prepareInstall(target):
                log.debug('Moving from %s to %s', extractedDirectory, target)
                shutil.move(extractedDirectory, target)
                if self.userdefinedProductName:
                    os.rename(target, os.path.join(self.productDir, self.userdefinedProductName))
                self._buildManifest(None, target)
        except IndexError:
...
class ArchivebundleSource(ArchiveSource):
    ...
    def moveDirs(self, tempDir):
        ...
        if len(extractedDirectories) == 1:
            log.debug('One nice neat directory extracted, moving its contents.')
            bundleDir = os.path.join(tempDir, extractedDirectories[0])
            os.chdir(bundleDir)
        ...
        for extractedDirectory in extractedDirectories:
            ...
            if extractedDirectory in self.internalBundles:
                ...
                if extractedDirectory == 'lib':
                    ...
                    if localDirs == ['python']:
                        extractedDirectory = os.path.join(extractedDirectory, 'python')
                        log.info('internal lib/python dir found')
                    else:
                        log.warn('Expected python in lib, but found %s', localDirs)
                log.info('Moving the contents of %s to %s.', extractedDirectory, base_target)
                localDirs = os.listdir(extractedDirectory)
                for localDir in localDirs:
                    if '%s/%s' % (extractedDirectory, localDir) in self.drop:
                        log.warn('Directory %s in droplist for %s, not installing it.' % (localDir, self.sourceName))
                        continue
                    target = os.path.join(base_target, localDir)
                    if self.prepareInstall(target):
                        source = os.path.join(extractedDirectory, localDir)
