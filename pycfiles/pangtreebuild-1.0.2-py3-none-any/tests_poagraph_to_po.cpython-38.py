...
# [GCC 8.4.0]
# Embedded file name: /home/paulina/pangtree/pangtreebuild/tests/tests_serialization/tests_poagraph_to_po.py
# Compiled at: 2019-12-30 15:48:57
# Size of source mod 2**32: 6964 bytes
import unittest
...
class PoagraphToPOTests(unittest.TestCase):
    ...
    def test_1_typical_poagraph(self):
        ...
        poagraph_sequences = {msa.SequenceID('seq0'): graph.Sequence(msa.SequenceID('seq0'), [
         ...
         msa.SequenceID('seq2'): graph.Sequence(msa.SequenceID('seq2'), [
                                  graph.SeqPath([*map(nid, [3, 4, 6, 7, 10, 12, 14, 17])])], graph.SequenceMetadata({'group': '1'})), 
         
         msa.SequenceID('seq3'): graph.Sequence(msa.SequenceID('seq3'), [
                                  graph.SeqPath([*map(nid, [11, 13, 14, 15])])], graph.SequenceMetadata({'group': '1'}))}
    ...
    def test_2_consensuses_and_empty_sequences(self):
        ...
        poagraph_sequences = {msa.SequenceID('seq0'): graph.Sequence(msa.SequenceID('seq0'), [
         ...
         msa.SequenceID('seq2'): graph.Sequence(msa.SequenceID('seq2'), [], graph.SequenceMetadata({'group': '1'})), 
         
         msa.SequenceID('seq3'): graph.Sequence(msa.SequenceID('seq3'), [], graph.SequenceMetadata({'group': '1'})), 
         
         msa.SequenceID('CONSENS0'): graph.Sequence(msa.SequenceID('CONSENS0'), [
                                      graph.SeqPath([*map(nid, [0, 3, 4, 5, 7, 8])])], None), 
         
         msa.SequenceID('CONSENS1'): graph.Sequence(msa.SequenceID('CONSENS1'), [
                                      graph.SeqPath([*map(nid, [1, 2, 4, 5, 6, 8])])], None)}
        poagraph = graph.Poagraph(poagraph_nodes, poagraph_sequences)
        actual_po_content = po.poagraph_to_PangenomePO(poagraph)
        expected_po_content = pathtools.get_file_content(expected_po_content_path)
        self.assertEqual(expected_po_content, actual_po_content)
