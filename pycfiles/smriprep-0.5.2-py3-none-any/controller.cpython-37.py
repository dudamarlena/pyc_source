...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-pti7pv2_/pip/pip/_vendor/cachecontrol/controller.py
# Compiled at: 2020-02-14 17:24:43
# Size of source mod 2**32: 14149 bytes
"""
The httplib2 algorithms ported for use with requests.
"""
import logging, re, calendar, time
...
class CacheController(object):
    __doc__ = 'An interface to see if request should cached or not.\n    '
    ...
    def cached_request(self, request):
        ...
        if 'max-age' in cc:
            if cc['max-age'] == 0:
                ...
                resp or logger.warning('Cache entry deserialization failed, entry ignored')
                return False
            if resp.status == 301:
    ...
    def cache_response(self, request, response, body=None, status_codes=None):
        ...
        if body is not None:
            if 'content-length' in response_headers:
                if response_headers['content-length'].isdigit():
                    if int(response_headers['content-length']) != len(body):
                        return
