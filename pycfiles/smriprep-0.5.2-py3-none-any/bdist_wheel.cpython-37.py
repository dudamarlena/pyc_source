...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-pti7pv2_/wheel/wheel/bdist_wheel.py
...
class bdist_wheel(Command):
    ...
    def run(self):
        ...
        self.keep_temp or logger.info('removing %s', self.bdist_dir)
        if not self.dry_run:
    ...
    def egg2dist(self, egginfo_path, distinfo_path):
        ...
        def adios(p):
            ...
            if os.path.exists(p) and (os.path.islink(p) or os.path.isdir)(p):
                shutil.rmtree(p)
            else:
                if os.path.exists(p):
                    os.unlink(p)
