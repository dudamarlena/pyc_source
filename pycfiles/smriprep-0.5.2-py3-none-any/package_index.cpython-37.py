...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-pti7pv2_/setuptools/setuptools/package_index.py
# Compiled at: 2020-02-14 17:24:43
# Size of source mod 2**32: 40614 bytes
...
class PackageIndex(Environment):
        ...
        if url in self.scanned_urls:
            ...
            if dists:
                ...
                if not dists:
                    ...
                    base = f.url
                    page = f.read()
                    if (isinstance(page, str) or isinstance)(f, urllib.error.HTTPError):
                        charset = 'latin-1'
                    else:
    ...
    def scan_egg_links(self, search_path):
        ...
        egg_links = ((path, entry) for path in dirs for entry in os.listdir(path))
        ...
        def find(req, env=None):
            ...
            for dist in env[req.key]:
                if dist.precedence == DEVELOP_DIST:
                    if (develop_ok or dist) not in skipped:
        ...
        try:
            ...
            try:
                if warning:
                    ...
                    raise DistutilsError('%s returned a bad status line. The server might be down, %s' % (
                     url, v.line))
            finally:
                v = None
                del v
