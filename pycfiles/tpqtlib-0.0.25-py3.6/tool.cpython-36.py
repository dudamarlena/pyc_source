...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpQtLib/core/tool.py
# Compiled at: 2020-01-16 21:52:29
# Size of source mod 2**32: 11013 bytes
"""
...
class DockTitleBar(QWidget, object):
    ...
    def mouseDoubleClickEvent(self, event):
        if event.pos().x() <= self._title_label.width():
            if self._renamable:
                self._start_edit()
        else:
...
def create_tool_instance(tool_class, already_registered_tools=None):
    ...
    if is_singleton and tool_class.NAME in [t.NAME for t in already_registered_tools]:
        ...
        if 'any' not in supported_softwares:
            if tp.Dcc.get_name() not in supported_softwares:
                tpQtLib.logger.warning('Tool {} is not suppported in current software: "{}"'.format(tool_class.NAME, tp.Dcc.get_name()))
                return
        return tool_class()