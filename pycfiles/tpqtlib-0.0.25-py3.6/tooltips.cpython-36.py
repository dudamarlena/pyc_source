...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpQtLib/widgets/tooltips.py
# Compiled at: 2020-01-16 21:52:29
# Size of source mod 2**32: 12173 bytes
"""
...
class ExpandedTooltipPopup(dialog.Dialog, object):
    ...
    def __init__(self, widget, width=450, height=50, icon_size=40, parent=None, show_on_initialize=False, stylesheet='', popup_release=Qt.Key_Control, show_at_cursor=True):
        ...
        super(ExpandedTooltipPopup, self).__init__(width=width,
          height=height,
    ...
    def _apply_style(self, text):
        ...
        text = text.replace('class="highlight"', 'style="color: rgb{}; font-weight: bold" '.format(self._theme_color))
...
class ToolTipWidget(QWidget, object):
    ...
    def show_below(self, rect, content, parent_window=None):
        ...
        if not parent_window:
            parent_window = tp.Dcc.get_main_window().windowHandle()
        self._add_widget(content)
        margin_size = QSize(2 * content.style().pixelMetric(QStyle.PM_DefaultTopLevelMargin), 2 * content.style().pixelMetric(QStyle.PM_DefaultTopLevelMargin))
    ...
    def _center_below(self, rect, screen):
        ...
        if not has_room_above:
            if not has_room_below:
                if not has_room_to_left:
                    if not has_room_to_right:
                        return QPoint()
            x = 0
            y = 0
