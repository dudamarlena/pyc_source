...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpQtLib/widgets/expandables.py
# Compiled at: 2020-01-16 21:52:29
# Size of source mod 2**32: 30038 bytes
"""
...
class ExpanderItem(QGroupBox, object):
        ...
        if source:
            if source != self:
                if source.parent() == self.parent():
                    if isinstance(source, ExpanderItem):
    ...
    def mousePressEvent(self, event):
        if self.dragDropMode():
            if event.button() == Qt.LeftButton:
                if self.dragDropRect().contains(event.pos()):
    ...
    def paintEvent(self, event):
        ...
        if self._rolloutStyle == ExpanderStyles.Rounded:
            ...
            self._ExpanderItem__drawTriangle(painter, x, y)
        ...
        if self._rolloutStyle == ExpanderStyles.Maya:
            ...
            if self._rolloutStyle == ExpanderStyles.Boxed:
                if self.isCollapsed():
