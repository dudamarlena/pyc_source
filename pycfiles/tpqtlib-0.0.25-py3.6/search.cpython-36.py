...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpQtLib/widgets/search.py
...
class SearchFindWidget(QWidget, object):
    ...
    def __init__(self, parent=None):
        ...
        self._search_line.setMinimumSize(max(self._search_line.minimumSizeHint().width(), self._clear_button_padded_width() + self._search_button_padded_width()), max(self._search_line.minimumSizeHint().height(), max(self._clear_button_padded_width(), self._search_button_padded_width())))
    ...
    def _popup_menu(self):
        if self._search_menu:
            ...
            y = rect.bottom() if self._search_btn.mapToGlobal(QPoint(0, rect.bottom())).y() + size_hint.height() <= screen_rect.height() else rect.top() - size_hint.height()
