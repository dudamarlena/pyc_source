...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpQtLib/widgets/drop.py
# Compiled at: 2020-01-16 21:52:29
# Size of source mod 2**32: 12908 bytes
"""
...
class DropArea(object):
    ...
    CenterDropArea = 5
    AllAreas = [
     TopDropArea, RightDropArea, BottomDropArea, LeftDropArea, CenterDropArea]
...
class DropOverlay(QFrame, object):
    ...
    def create_drop_indicator_pixmap(palette, size, drop_area):
        ...
        if drop_area == DropArea.TopDropArea:
            ...
            if drop_area == DropArea.RightDropArea:
                ...
                if drop_area == DropArea.BottomDropArea:
                    ...
                    if drop_area == DropArea.LeftDropArea:
            ...
            if drop_area == DropArea.RightDropArea:
                ...
                if drop_area == DropArea.BottomDropArea:
                    ...
                    if drop_area == DropArea.LeftDropArea:
                        ...
                        if drop_area == DropArea.CenterDropArea:
                            r = self.rect()
        if not r.isNull():
            painter.fillRect(r, QBrush(area_color, Qt.Dense4Pattern))
            painter.setBrush(QBrush(area_color))
    ...
    def show_drop_overlay(self, target, target_area_rect=None):
        if target_area_rect is None:
            ...
            if self._target == target:
                if self._target_rect == target_area_rect:
                    return
            self.hide_drop_overlay()
...
class DropOverlayCross(QWidget, object):
    ...
    def grid_pos_for_area(drop_area):
        ...
        if drop_area == DropArea.TopDropArea:
            ...
            if drop_area == DropArea.RightDropArea:
                return (
                 QPoint(1, 2), Qt.AlignLeft | Qt.AlignVCenter)
            else:
                if drop_area == DropArea.BottomDropArea:
                    return (
                     QPoint(2, 1), Qt.AlignHCenter | Qt.AlignTop)
                if drop_area == DropArea.LeftDropArea:
                    return (
                     QPoint(1, 0), Qt.AlignRight | Qt.AlignVCenter)
                if drop_area == DropArea.CenterDropArea:
                    return (
                     QPoint(1, 1), Qt.AlignCenter)
            return (
             QPoint(), int())
    ...
    def reset(self):
        all_areas = [
         DropArea.TopDropArea, DropArea.RightDropArea, DropArea.BottomDropArea, DropArea.LeftDropArea, DropArea.CenterDropArea]
