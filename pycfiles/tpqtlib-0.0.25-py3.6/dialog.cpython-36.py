...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpQtLib/core/dialog.py
# Compiled at: 2020-01-16 21:52:29
# Size of source mod 2**32: 34814 bytes
"""
...
class ColorDialog(Dialog, object):
    ...
    maya_colors = [
     (
      0.467, 0.467, 0.467), (0.0, 0.0, 0.0), (0.247, 0.247, 0.247), (0.498, 0.498, 0.498), (0.608, 0, 0.157), (0, 0.016, 0.373), (0, 0, 1), (0, 0.275, 0.094), (0.145, 0, 0.263), (0.78, 0, 0.78), (0.537, 0.278, 0.2), (0.243, 0.133, 0.122), (0.6, 0.145, 0), (1, 0, 0), (0, 1, 0), (0, 0.255, 0.6), (1, 1, 1), (1, 1, 0), (0.388, 0.863, 1), (0.263, 1, 0.635), (1, 0.686, 0.686), (0.89, 0.675, 0.475), (1, 1, 0.384), (0, 0.6, 0.325), (0.627, 0.412, 0.188), (0.62, 0.627, 0.188), (0.408, 0.627, 0.188), (0.188, 0.627, 0.365), (0.188, 0.627, 0.627), (0.188, 0.404, 0.627), (0.435, 0.188, 0.627), (0.627, 0.188, 0.404)]
    ...
    def _on_set_color(self, color_index):
        if tp.Dcc.get_name() == tp.Dccs.Maya:
            if tp.Dcc.get_version() <= 2016:
                self.color_dialog.setCurrentColor(QColor.fromRgb(self.maya_colors[color_index][0] * 255, self.maya_colors[color_index][1] * 255, self.maya_colors[color_index][2] * 255))
        else:
...
class OpenFileDialog(BaseFileFolderDialog, object):
    ...
    def __init__(self, name='OpenFile', multi=False, title='Open File', size=(200, 125), fixed_size=False, frame_less=True, hide_title=False, parent=None, use_app_browser=False):
        if parent is None:
            parent = tp.Dcc.get_main_window()
        super(OpenFileDialog, self).__init__(name=name,
          title=title,
          size=size,
    ...
    def open_app_browser(self):
        if tp.Dcc.get_name() == tp.Dccs.Maya:
            ...
            sel_file = cmds.fileDialog2(caption=(self.windowTitle()),
              fileMode=1,
...
class SaveFileDialog(BaseFileFolderDialog, object):
    ...
    def __init__(self, name='SaveFile', title='Save File', size=(200, 125), fixed_size=False, frame_less=True, hide_title=False, parent=None, use_app_browser=False):
        if parent is None:
            parent = tp.Dcc.get_main_window()
        super(SaveFileDialog, self).__init__(name=name,
          title=title,
          size=size,
          fixed_size=fixed_size,
          frame_less=frame_less,
...
class SelectFolderDialog(BaseFileFolderDialog, object):
    ...
    def __init__(self, name='SelectFolder', title='Select Folder', size=(200, 125), fixed_size=False, frame_less=True, hide_title=False, use_app_browser=False, parent=None, **kwargs):
        if parent is None:
            parent = tp.Dcc.get_main_window()
        (super(SelectFolderDialog, self).__init__)(name=name, 
         title=title, size=size, fixed_size=fixed_size, frame_less=frame_less, hide_title=hide_title, use_app_browser=use_app_browser, parent=parent, **kwargs)
    ...
    def open_app_browser(self):
        if tp.Dcc.get_name() == tp.Dccs.Maya:
            import maya.cmds as cmds
            sel_folder = cmds.fileDialog2(caption=(self.windowTitle()),
              fileMode=3,
              fileFilter=(self.filters),
              dialogStyle=2)
        else:
        ...
        if sel_folder:
            ...
            result = [
             sel_folder, os.path.dirname(sel_folder), [os.path.basename(sel_folder)]]
            return result[0]
