...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/yakonfig/toplevel.py
...
import collections, contextlib, copy, logging, sys
...
def add_arguments(parser):
    ...
    """
    parser.add_argument('--config', '-c', metavar='FILE', help='read configuration from FILE')
    parser.add_argument('--dump-config', metavar='WHAT', nargs='?', help='dump out configuration then stop (default, effective, full)')
...
def parse_args(parser, modules, args=None):
    ...
    try:
        ...
        set_default_config(modules, params=vars(namespace), validate=not do_dump_config)
        if do_dump_config:
            if namespace.dump_config == 'full':
                ...
                to_dump = diff_config(assemble_default_config(modules), get_global_config())
...
def set_default_config(modules, params=None, yaml=None, filename=None, config=None, validate=True):
...
def defaulted_config(modules, params=None, yaml=None, filename=None, config=None, validate=True):
    ...
    """
    with _temporary_config():
        set_default_config(modules, params=params, yaml=yaml, filename=filename, config=config, validate=validate)
...
def check_toplevel_config(what, who):
    ...
    if config_name not in config:
        raise ConfigurationError(('{0} requires top-level configuration for {1}').format(who, config_name))
...
def _recurse_config(parent_config, modules, f, prefix=''):
    ...
    for module in modules:
        ...
        if config_name is None:
            raise ProgrammerError(('{0!r} must provide a config_name').format(module))
        new_name = prefix + config_name
        f(parent_config, config_name, new_name, module)
        try:
            _recurse_config((parent_config or {}).get(config_name, None), getattr(module, 'sub_modules', []), f, new_name + '.')
        except:
...
def create_config_tree(config, modules, prefix=''):
    ...
    def work_in(parent_config, config_name, prefix, module):
        if config_name not in parent_config:
            ...
            raise ConfigurationError(('{0} must be an object configuration').format(prefix))
...
def _walk_config(config, modules, f, prefix=''):
    ...
    def work_in(parent_config, config_name, prefix, module):
        if config_name not in parent_config:
            raise ProgrammerError(('{0} not present in configuration').format(prefix))
