...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/yakonfig/tests/test_factory.py
# Compiled at: 2015-07-07 22:00:14
from __future__ import absolute_import, division, print_function
import pytest, yakonfig
...
def configurable_both(abc, xyz, a=1, b=2, c=3):
    return dict(configurable_services(abc, xyz), **configurable_defaults(a=a, b=b, c=c))
...
def test_discover_defaults():
    ...
    assert conf == {'name': 'configurable_defaults', 
       'required': [], 'defaults': {'a': 1, 'b': 2, 'c': 3}}


def test_discover_services():
    conf = AutoConfigured.inspect_obj(configurable_services)
    assert conf == {'name': 'configurable_services', 
       'required': [
                  'abc', 'xyz'], 
       'defaults': {}}
...
def test_discover_both():
    ...
    assert conf == {'name': 'configurable_both', 
       'required': [
                  'abc', 'xyz'], 
...
def test_from_obj_config_param():
    ...
    proxy = AutoConfigured.from_obj(configurable_config_param, any_configurable=True)
    assert isinstance(proxy, AutoConfigured)


def test_discover_legacy():
    conf = AutoConfigured.inspect_obj(configurable_legacy)
    assert conf == {'name': 'configurable_legacy', 
       'required': [
                  'config'], 
       'defaults': {}}
...
def test_factory_class_with_config_name():
    ...
    with yakonfig.defaulted_config([factory], config={}) as (config):
        ...
        assert config['SimpleAutoFactory']['configurable_alt_name']['key'] == 'value'
...
def test_factory_class_config_param_defaults():
    ...
    with yakonfig.defaulted_config([factory], config={}) as (config):
        ...
        instantiated = factory.create(configurable_config_param, config={'k': 'v'})
...
def test_factory_class_config_param():
    ...
    with yakonfig.defaulted_config([factory], config=config) as (config):
        ...
        instantiated = factory.create(configurable_config_param)
        assert instantiated.config == {'k': 'v'}
        instantiated = factory.create(configurable_config_param, config={'x': 'y'})
