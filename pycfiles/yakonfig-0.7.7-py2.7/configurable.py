...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/yakonfig/configurable.py
...
def check_subconfig(config, name, sub):
    ...
    if not isinstance(subconfig, collections.Mapping):
        raise ProgrammerError(('configuration for {0} in {1} must be a mapping').format(subname, name))
