...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/yakonfig/cmd.py
...
import argparse
from cmd import Cmd
import logging, pdb, shlex

class ArgParseCmd(Cmd):
    r'''Base class for :mod:`argparse`-based interactive :mod:`cmd` sessions.
    ...
    * ``do_command()`` takes a single argument, which is the parsed
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('action', help='action to run', nargs='?', choices=list(commands))
        parser.add_argument('arguments', help='arguments specific to ACTION', nargs=argparse.REMAINDER)
    ...
    def runcmd(self, cmd, args):
        ...
        if dof is None:
            ...
            if argf is not None:
                parser = argparse.ArgumentParser(prog=cmd, description=getattr(dof, '__doc__', None))
                argf(parser)
    ...
    def args_help(self, parser):
        parser.add_argument('command', nargs='?', help='print help on this command')
    ...
    def do_help(self, args):
        ...
        if args.command:
            ...
            if f:
                parser = argparse.ArgumentParser(prog=args.command, description=docstr)
                f(parser)
