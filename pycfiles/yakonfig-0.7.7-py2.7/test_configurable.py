...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/yakonfig/tests/test_configurable.py
...
import yakonfig, yakonfig.tests.configurable_module, yakonfig.toplevel
...
class ConfigurableArgs(yakonfig.Configurable):
    ...
    def check_config(self, config, name=''):
        if len(config['k']) != 1:
            raise yakonfig.ConfigurationError(("{0} 'k' wrong length").format(name))
...
def test_fill_in():
    c = {'configurable': {}}
    yakonfig.toplevel.fill_in_arguments(c, [ConfigurableLike], {'key': 'value'})
    assert sorted(iterkeys(c)) == ['configurable']
    cc = c['configurable']
...
def test_fill_in_replaces():
    ...
    yakonfig.toplevel.fill_in_arguments(c, [ConfigurableLike], {'key': 'new'})
    assert sorted(iterkeys(c)) == ['configurable']
    cc = c['configurable']
    assert sorted(iterkeys(cc)) == ['key']
    assert cc['key'] == 'new'
...
def test_default_file(request):
    yakonfig.set_default_config([
     ConfigurableArgs()], filename=str(request.fspath.dirpath('argconfig.yaml')))
    try:
...
def test_prog_yaml():
    the_yaml = '\nconfigurable:\n    key: yaml\n'
    yakonfig.set_default_config([ConfigurableLike], params={'key': 'value'}, yaml=the_yaml)
    try:
        c = yakonfig.get_global_config()
        assert sorted(iterkeys(c)) == ['configurable']
        cc = c['configurable']
...
def test_dont_validate():
    yakonfig.set_default_config([ConfigurableArgs()], params={'key': 'longer than one char'}, validate=False)
    try:
        c = yakonfig.get_global_config()
        assert sorted(iterkeys(c)) == ['config']
        cc = c['config']
...
def test_two_level_args():
    yakonfig.set_default_config([ConfigurableTop], {'aaa': 'a', 'bbb': 'b', 'zzz': 'z'})
    try:
        c = yakonfig.get_global_config()
        assert sorted(iterkeys(c)) == ['top']
        c = c['top']
...
def test_cli_file(request):
    ...
    yakonfig.parse_args(parser, [
     yakonfig, ConfigurableArgs()], args=[
     '-c', yaml])
    try:
        c = yakonfig.get_global_config()
        assert 'config' in c
...
def test_check_dependent():
    with yakonfig.defaulted_config([ConfigurableArgs(), ConfigurableLike], {'key': 'value'}) as (config):
...
def test_check_toplevel():
    ...
    with yakonfig.defaulted_config([ConfigurableArgs(), Dependent], {'key': 'k'}) as (config):
        assert sorted(iterkeys(config)) == ['config', 'dependent']
        assert config['config']['k'] == 'k'
    with pytest.raises(yakonfig.ConfigurationError):
        with yakonfig.defaulted_config([ConfigurableArgs(), Dependent], {'key': 'key'}):
            pass
...
def test_proxy_two_level():
    with yakonfig.defaulted_config([
     yakonfig.ProxyConfigurable(ConfigurableTop)], {'aaa': 'a', 'bbb': 'b', 'zzz': 'z'}):
        c = yakonfig.get_global_config()
        assert sorted(iterkeys(c)) == ['top']
        c = c['top']
        assert sorted(iterkeys(c)) == ['aaa', 'bottom']
...
def test_normalize():
    ...
    with yakonfig.defaulted_config([Normalized], yaml='\nnormalized:\n  k: zoom!\n'):
