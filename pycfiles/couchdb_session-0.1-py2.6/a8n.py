...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/couchdbsession/a8n.py
# Compiled at: 2010-02-26 07:34:24
"""
Known limitations:
    * Not thread safe.
"""
import datetime, itertools, types, UserDict
...
class Recorder(object):
    ...
    def create(self, path, value):
        action = {'action': 'create', 'path': self._path + [path], 
           'value': value}
    ...
    def edit(self, path, value, was):
        ...
        if create_action is not None:
            ...
            if edit_action is not None:
                ...
                return
            action = {'action': 'edit', 'path': self._path + [path], 
               'value': value, 
               'was': was}
    ...
    def remove(self, path, was):
        ...
        if create_action is not None:
            ...
            action = {'action': 'remove', 'path': self._path + [path], 
               'was': was}
            self._tracker.append(action)
            return
    ...
    def _remove_nested_actions(self, path):
        ...
        to_delete = (i for (i, change) in enumerate(self._tracker._changes) if change['path'][:full_path_len] == full_path if len(change['path']) > full_path_len)
...
def _make_list_adjuster(adjustment, start, end=None):
    ...
    def adjuster(path):
        if (start is None or start <= path) and (end is None or path < end):
