...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/couchdbsession/tests/test_a8n.py
# Compiled at: 2010-02-26 07:34:24
...
class TestImmutableTracking(unittest.TestCase):
    ...
    def test_types(self):
        tracker = a8n.Tracker()
        for obj in [None, True, False, 'string', 'unicode', 1, 1, 1.0,
...
class TestDictTracking(unittest.TestCase):
    ...
    def test_del_item_add_same(self):
        ...
        del obj['foo']
        obj['foo'] = 'bar'
        assert list(tracker) == [{'action': 'remove', 'path': ['foo'], 'was': 'foo'}, {'action': 'create', 'path': ['foo'], 'value': 'bar'}]
    ...
    def test_replace_nested_with_actions(sel):
        ...
        obj['nested']['b'] = 2
        del obj['nested']['c']
        assert list(tracker) == [{'action': 'edit', 'path': ['nested', 'a'], 'value': 1, 'was': 0}, {'action': 'create', 'path': ['nested', 'b'], 'value': 2}, {'action': 'remove', 'path': ['nested', 'c'], 'was': 2}]
        obj['nested'] = {}
...
class TestListTracking(unittest.TestCase):
    ...
    def test_extend(self):
        ...
        obj = tracker.track([])
        obj.extend(['foo', 'bar'])
        assert list(tracker) == [{'action': 'create', 'path': [0], 'value': 'foo'}, {'action': 'create', 'path': [1], 'value': 'bar'}]
    ...
    def test_replace_nested_with_actions(sel):
        ...
        obj[0]['b'] = 2
        del obj[0]['c']
        assert list(tracker) == [{'action': 'edit', 'path': [0, 'a'], 'value': 1, 'was': 0}, {'action': 'create', 'path': [0, 'b'], 'value': 2}, {'action': 'remove', 'path': [0, 'c'], 'was': 2}]
        obj[0] = {}
    ...
    def test_iter(self):
        ...
        assert list(tracker) == [{'action': 'create', 'path': [0, 'foo'], 'value': 'bar'}, {'action': 'create', 'path': [1, 'foo'], 'value': 'bar'}]

    def test_sort_forward(self):
        tests = [
         (
          [
           3, 1, 2], [{'action': 'edit', 'path': [0], 'value': 1, 'was': 3}, {'action': 'edit', 'path': [1], 'value': 2, 'was': 1}, {'action': 'edit', 'path': [2], 'value': 3, 'was': 2}]),
         (
          [
           1, 3, 2], [{'action': 'edit', 'path': [1], 'value': 2, 'was': 3}, {'action': 'edit', 'path': [2], 'value': 3, 'was': 2}]),
         (
          [
           1], [])]
    ...
    def test_sort_reverse(self):
        tests = [
         (
          [
           3, 1, 2], [{'action': 'edit', 'path': [1], 'value': 2, 'was': 1}, {'action': 'edit', 'path': [2], 'value': 1, 'was': 2}]),
         (
          [
           1, 3, 2], [{'action': 'edit', 'path': [0], 'value': 3, 'was': 1}, {'action': 'edit', 'path': [1], 'value': 2, 'was': 3}, {'action': 'edit', 'path': [2], 'value': 1, 'was': 2}]),
         (
          [
           1], [])]
        for (l, actions) in tests:
...
class TestChangingPaths(unittest.TestCase):
    ...
    def test_delitem(self):
        ...
        del obj[0]
        assert list(tracker) == [{'action': 'create', 'path': [1, 'a'], 'value': 'a'}, {'action': 'remove', 'path': [0], 'was': {}}]
        a_dict['b'] = 'b'
        assert list(tracker) == [{'action': 'create', 'path': [1, 'a'], 'value': 'a'}, {'action': 'remove', 'path': [0], 'was': {}}, {'action': 'create', 'path': [0, 'b'], 'value': 'b'}]
    ...
    def test_insert(self):
        ...
        obj.insert(0, {})
        a_dict['foo'] = 'bar'
        assert list(tracker) == [{'action': 'create', 'path': [0], 'value': {}}, {'action': 'create', 'path': [2, 'foo'], 'value': 'bar'}]
    ...
    def test_pop(self):
        ...
        obj.pop(0)
        a_dict['foo'] = 'bar'
        assert list(tracker) == [{'action': 'remove', 'path': [0], 'was': {}}, {'action': 'create', 'path': [0, 'foo'], 'value': 'bar'}]
    ...
    def test_remove(self):
        ...
        obj.remove({'a': 1})
        a_dict['b'] = 'b'
        assert list(tracker) == [{'action': 'remove', 'path': [0], 'was': {'a': 1}}, {'action': 'edit', 'path': [0, 'b'], 'value': 'b', 'was': 2}]
