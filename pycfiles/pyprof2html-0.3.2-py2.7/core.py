...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/pyprof2html/core.py
# Compiled at: 2016-10-19 12:59:46
"""pyprof2html - Profile data convert to HTML.
"""
import codecs, hashlib, os, re, time
try:
...
def mapping_table(target, nall):
    levelmap = {0: '#ffffff', 1: '#66ff99', 
       2: '#ffcc33', 
       3: '#ff6666', 
       4: '#ff3333'}
    levels = [
     nall * 0.01, nall * 0.03, nall * 0.08, nall * 0.12, nall * 0.2]
...
def convert_unit(float_number):
    ...
    elif 0.001 > float_number >= 1e-06:
        float_number = '%3.1lf [us]' % (float_number * 1000000)
    elif 1e-06 > float_number >= 1e-09:
...
class Converter(object):
            ...
            if not sources.count(filename) or len(profs) == i + 1:
                if info != {}:
                    if self.outputtype == 'html':
                        ...
                        self._printhtml_source(sources[filecnt], info, outputfile)
                        outputfile.close()
                    else:
    ...
    def _analyze_profline(self):
        ...
        for i in self.profline:
            ...
            if i[1] not in profset:
                profset[i[1]] = [
                 float(int(i[2]) / 1000000.0), i[2]]
            else:
                profset[i[1]][0] += float(int(i[2]) / 1000000.0)
                profset[i[1]][1] += i[2]
    ...
    def _collect_callers_data(self, source, call_dict):
        ...
        for cnt, func in enumerate(clist):
            ...
            tempdata.append({'dst_function': dst_function, 'callers_link': callers_link, 
               'ncalls': nc, 
               'ncallslevel': ncl, 
               'tottime': '%8.4f' % tt, 
               'tottimelevel': ttl, 
    ...
    def _analyzed_prof(self):
        ...
        self.prof.stream = open(os.devnull, 'w')
        ...
        profdata = {'totaltime': '%8.4lf' % self.prof.total_tt, 'totalcalls': self.prof.total_calls, 
           'data': datalist}
        return profdata
    ...
    def _hookhtml(self):
        ...
        self.outfile.write(self.tmpl.render(title=titletext, proftime=self.proftime, reporttime=self.reporttime, profdata=profdata, profiledata_count=self.profiledata_count, thisname=self.output_htmlfile, version=pyprof2html.__version__, profile_datatype=self.profile_datatype))
        self.outfile.close()
        filepath = '%s/%s' % (self.output_dir, self.callers_htmlfile)
        self.outfile = open(filepath, 'w')
        render_data = dict(title=titletext, proftime=self.proftime, reporttime=self.reporttime, profile_datatype=self.profile_datatype, callers_data=self.callers_data, profdata=profdata, profiledata_count=self.profiledata_count, thisname=self.callers_htmlfile, version=pyprof2html.__version__)
        self.outfile.write(self.callers_tmpl.render(render_data))
        self.outfile.close()

    def printout(self, filetype='html', output_directory='html', output_htmlfile='index.html', functions_number=20):
