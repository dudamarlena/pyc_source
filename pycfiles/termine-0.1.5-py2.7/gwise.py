...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/termine/gwise.py
# Compiled at: 2012-07-18 10:09:59
...
THISDIR = os.path.abspath(sys.argv[0])
...
class Groupwise():
    ...
    def __login__(self):
        ...
        while invalid:
            ...
            resp = self.client.service.loginRequest(self.pt, self.lang, self.app, self.userid, self.system)
    ...
    def getAppointments(self, window=None):
        ...
        windowmap = {'today': gwfilter.today, 'tomorrow': gwfilter.tomorrow, 
        ...
        if window.lower() not in windowmap.keys():
            ...
            try:
                items = self.getCalendarItems(windowmap.get(window, windowmap[self.defaultwindow])())
            except GWLoginException as e:
                logger.warn('session login failed: %s' % str(e))
                self.__login__()
                raise GWInitException, 'session login failed - retrying...'
    ...
    def getCalendarItems(self, filtergroup):
        ...
        if int(resp.status.code) == 59910:
            logger.warn('invalid session string')
            raise GWSessionException(resp.status.description)
        Filter = self.client.factory.create('ns2:Filter')
        Filter.element = filtergroup
        items = self.client.service.getItemsRequest(resp.folder.id, 'default', Filter, None)
        return items
    ...
    def getBusyTimeRequestId(self, users, s=datetime.utcnow(), e=None):
        ...
        resp = self.client.service.startFreeBusySessionRequest(users=free_busy_list, startDate=s_str, endDate=e_str)
        if int(resp.status.code) == 0:
            ...
            if int(resp.status.code) == 0:
                ...
                for user in users:
                    if hasattr(user, 'displayName'):
                        ...
                        dn = ''
                    u[user.email] = {'email': user.email, 'uuid': user.uuid, 'displayName': dn, 'blocks': user.blocks}
    ...
    def getUserList(self, users):
        ...
        Filter = self.client.factory.create('ns2:Filter')
        Filter.element = FilterGroup
        resp = self.client.service.getItemsRequest(self.addressbookid, None, Filter, None)
        displayName = ''
        if int(resp.status.code) == 0:
            if len(resp.items) == 0:
                return {}
