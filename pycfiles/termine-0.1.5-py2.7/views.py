...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/termine/views.py
# Compiled at: 2012-07-14 04:42:16
...
class GWView:
    ...
    def __appointmentStatus__(self, s, f):
       ...
       @returns MAGENTA (appointment is over)
        ...
        if f < current_localtime:
            return MAGENTA
        if s > current_localtime:
            return BLUE
        if s < current_localtime and f > current_localtime:
            return RED
    ...
    def render(self):
        ...
        for item in self.itemlist:
            ...
            with_id = ''
            if self.id:
                with_id = '%s\t' % self.__readableId__(item['id']).encode('utf-8')
...
class GWAppointmentView:
    ...
    def __soap2string__(self, soapobj):
        item = soapobj.item
        t = {'message': 'This appointment does not contain a message/description', 'place': 'This appointment does not contain a place'}
        t['acceptLevel'] = item.acceptLevel
    ...
    def render(self):
        ...
        print '%s %s' % (self.bold('Subject:'), textwrap.fill(s, initial_indent='', subsequent_indent='         '))
        print self.bold('From:') + '    %s' % textwrap.fill('%s - %s' % (self.d['startDate'].strftime(dtime), self.d['endDate'].strftime(dtime)), initial_indent='', subsequent_indent='         ')
        print self.bold('Place:') + '   %s' % textwrap.fill(p, initial_indent='', subsequent_indent='         ')
        print self.bold('Message:') + ' %s' % textwrap.fill(m.encode('utf-8'), initial_indent='', subsequent_indent='         ')
