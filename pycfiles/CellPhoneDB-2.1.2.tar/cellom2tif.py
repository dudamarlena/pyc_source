...
# Embedded file name: /Users/nuneziglesiasj/anaconda/envs/bioformats/lib/python2.7/site-packages/cellom2tif/cellom2tif.py
# Compiled at: 2014-08-29 05:39:04
...
def read_image(filelike):
    ...
    if not VM_STARTED:
        start()
    if VM_KILLED:
        raise RuntimeError('The Java Virtual Machine has already been killed, and cannot be restarted. See the python-javabridge documentation for more information. You must restart your program and try again.')
...
def main():
    parser = argparse.ArgumentParser(description='Convert a bunch of Cellomics .C01 files to TIFFs.')
    ...
    parser.add_argument('-E', '--error-file', metavar='FILENAME', help='Log problem filenames to the given filename.')
    parser.add_argument('-m', '--ignore-masks', action='store_true', help='Ignore files ending in "o1.C01".')
    parser.add_argument('-v', '--verbose', action='store_true', help='Print out runtime information.')
    args = parser.parse_args()
    paths = os.walk(args.root_path)
    for path, dirs, files in paths:
        convert_files(path.replace(args.root_path, args.out_path, 1), path, files, args.ignore_masks, args.verbose)
