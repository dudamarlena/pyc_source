...
def imsave(filename, data, photometric=None, planarconfig=None, resolution=None, description=None, software='tifffile.py', byteorder=None, bigtiff=False):
    ...
    if not photometric in (None, 'minisblack', 'miniswhite', 'rgb'):
        ...
        if not bigtiff and data.size * data.dtype.itemsize < 2040 * 1048576:
            bigtiff = False
            offset_size = 4
            tag_size = 12
            numtag_format = 'H'
        ...
        samplesperpixel = 1
        extrasamples = 0
        if photometric is None and data.ndim > 2 and (shape[(-3)] in (3, 4) or shape[(-1)] in (3,
                                                                                               4)):
            photometric = 'rgb'
        else:
 ...
 0]):
    ...
    if description:
        ...
        tag('photometric', 'H', 1, {'miniswhite': 0, 'minisblack': 1, 'rgb': 2}[photometric])
...
class TiffFile(object):
    ...
    def series(self):
        ...
        if self.is_ome:
            ...
            series = [
             Record(axes=('').join(dims.get(i[0].strip().upper(), 'Q') for i in mmhd if i[1] > 1), shape=tuple(int(i[1]) for i in mmhd if i[1] > 1), pages=self.pages, dtype=numpy.dtype(self.pages[0].dtype))]
        elif self.is_lsm:
            ...
            axes = ('').join(axes)
            series = [
             Record(pages=self.pages, shape=shape, axes=axes, dtype=numpy.dtype(self.pages[0].dtype))]
        elif self.is_nih:
            series = [
             Record(pages=self.pages, shape=(
              len(self.pages),) + self.pages[0].shape, axes='I' + self.pages[0].axes, dtype=numpy.dtype(self.pages[0].dtype))]
        elif self.pages[0].is_shaped:
            shape = self.pages[0].tags['image_description'].value[7:-1]
            shape = tuple(int(i) for i in shape.split(','))
            series = [
             Record(pages=self.pages, shape=shape, axes='Q' * len(shape), dtype=numpy.dtype(self.pages[0].dtype))]
        if not series:
            shapes = []
            pages = {}
    ...
    def __str__(self):
        ...
        result = [
         ...
         format_size(self._fsize), {'<': 'little endian', '>': 'big endian'}[self.byteorder]]
        if self.is_bigtiff:
...
class TiffPage(object):
    ...
    def _process_tags(self):
        ...
        if self.is_imagej:
            ...
            try:
                adict.update(imagej_meta_data(tags['imagej_meta_data'].value, tags['imagej_byte_counts'].value, self.parent.byteorder))
            except Exception:
                pass
        ...
        if self.is_stk:
            ...
            if self.planar_configuration == 'contig':
                self._shape = (
                 planes, 1, self.image_length, self.image_width,
                ...
                self.axes = 'PSYX'
            if self.is_palette and self.color_map.shape[1] >= 2 ** self.bits_per_sample:
                self.shape = (
                 3, planes, self.image_length, self.image_width)
                self.axes = 'CPYX'
            else:
    ...
    def asarray(self, squeeze=True, colormapped=True, rgbonly=True):
        ...
        if 'ycbcr_subsampling' in self.tags and self.tags['ycbcr_subsampling'].value not in (1, (1, 1)):
        ...
        if not shape:
            ...
            if not self.is_tiled and (self.is_stk or not self.compression and bits_per_sample in (8,
                                                                                                  16,
                                                                                                  32,
    ...
    def __str__(self):
        ...
        s = (', ').join(s for s in (
         ...
         (',').join(t[3:] for t in ('is_stk', 'is_lsm', 'is_nih', 'is_ome', 'is_imagej', 'is_fluoview',
                           'is_mdgel', 'is_mediacy', 'is_reduced', 'is_tiled') if getattr(self, t))) if s)
    ...
    def is_imagej(self):
        ...
        return 'image_description' in self.tags and self.tags['image_description'].value.startswith('ImageJ=')
...
def read_cz_lsm_info(fh, byteorder, dtype, count):
    ...
    result = numpy.rec.fromfile(fh, CZ_LSM_INFO, 1, byteorder=byteorder)[0]
...
def read_cz_lsm_scan_info(fh, byteorder):
    ...
    if 268435456 != struct.unpack(byteorder + 'I', fh.read(4))[0]:
...
@_replace_by('_tifffile.decodepackbits', warn=False)
...
@_replace_by('_tifffile.decodelzw', warn=False)
...
@_replace_by('_tifffile.unpackints', warn=False)
...
TIFF_TAGS = {254: (
   ...
   347: (
       'jpeg_tables', None, None, None, None), 
   530: (
...
def main(argv=None):
    ...
    if float(sys.version[0:3]) < 2.6:
        ...
        parser = optparse.OptionParser(usage='usage: %prog [options] path', description=search_doc('\n\n([^|]*?)\n\n', ''), version='%%prog %s' % search_doc(':Version: (.*)', 'Unknown'))
