...
# Embedded file name: build/bdist.linux-x86_64/egg/pysnmp/proto/secmod/rfc2576.py
...
class SnmpV1SecurityModel(base.AbstractSecurityModel):
    ...
    def _sec2com(self, snmpEngine, securityName, contextEngineId, contextName):
        ...
        if self.__paramsBranchId != snmpTargetParamsSecurityName.branchVersionId:
            ...
            while True:
                ...
                try:
                    ...
                    debug.logger & debug.flagSM and debug.logger('_sec2com: table entries %r/%r hashing failed' % (nextMibNode.syntax, mibNode.syntax))
                    continue
        ...
        if self.__securityBranchId != snmpCommunityName.branchVersionId:
            (snmpCommunitySecurityName, snmpCommunityContextEngineId, snmpCommunityContextName) = snmpEngine.msgAndPduDsp.mibInstrumController.mibBuilder.importSymbols('SNMP-COMMUNITY-MIB', 'snmpCommunitySecurityName', 'snmpCommunityContextEngineID', 'snmpCommunityContextName')
            self.__securityMap = {}
            ...
            while True:
                ...
                try:
                    ...
                    debug.logger & debug.flagSM and debug.logger('_sec2com: table entries %r/%r/%r hashing failed' % (_securityName, _contextEngineId, _contextName))
                    continue
    ...
    def _com2sec(self, snmpEngine, communityName, transportInformation):
        ...
        if self.__transportBranchId != snmpTargetAddrTAddress.branchVersionId:
            (SnmpTagValue, snmpTargetAddrTDomain, snmpTargetAddrTagList) = snmpEngine.msgAndPduDsp.mibInstrumController.mibBuilder.importSymbols('SNMP-TARGET-MIB', 'SnmpTagValue', 'snmpTargetAddrTDomain', 'snmpTargetAddrTagList')
            ...
            while True:
                ...
                try:
                    if targetAddrTagList:
                        self.__transportToTagMap[targetAddr].update([ SnmpTagValue(x) for x in targetAddrTagList.asOctets().split() ])
                    else:
                        self.__transportToTagMap[targetAddr].add(self.__emptyTag)
                except PyAsn1Error:
                    debug.logger & debug.flagSM and debug.logger('_com2sec: table entries %r/%r hashing failed' % (targetAddr, targetAddrTagList))
                    continue
        ...
        if self.__paramsBranchId != snmpTargetParamsSecurityName.branchVersionId:
            ...
            while True:
                ...
                try:
                    ...
                    debug.logger & debug.flagSM and debug.logger('_com2sec: table entries %r/%r hashing failed' % (nextMibNode.syntax, mibNode.syntax))
                    continue
        ...
        if self.__communityBranchId != snmpCommunityName.branchVersionId:
            (snmpCommunitySecurityName, snmpCommunityContextEngineId, snmpCommunityContextName, snmpCommunityTransportTag) = snmpEngine.msgAndPduDsp.mibInstrumController.mibBuilder.importSymbols('SNMP-COMMUNITY-MIB', 'snmpCommunitySecurityName', 'snmpCommunityContextEngineID', 'snmpCommunityContextName', 'snmpCommunityTransportTag')
            ...
            while True:
                ...
                _tagAndCommunity = (
                 transportTag, nextMibNode.syntax)
                try:
                    if _tagAndCommunity not in self.__tagAndCommunityToSecurityMap:
                        self.__tagAndCommunityToSecurityMap[_tagAndCommunity] = set()
                    self.__tagAndCommunityToSecurityMap[_tagAndCommunity].add((securityName, contextEngineId, contextName))
                    ...
                    debug.logger & debug.flagSM and debug.logger('_com2sec: table entries %r/%r hashing failed' % (_tagAndCommunity, nextMibNode.syntax))
                    continue
        ...
        if communityName in self.__communityToTagMap:
            ...
            if candidateSecurityNames:
                candidateSecurityNames.sort(key=lambda x, m=self.__nameToModelMap, v=self.securityModelID: (
                 not int(x[0] in m and v in m[x[0]]), str(x[0])))
                chosenSecurityName = candidateSecurityNames[0]
    ...
    def generateRequestMsg(self, snmpEngine, messageProcessingModel, globalData, maxMessageSize, securityModel, securityEngineId, securityName, securityLevel, scopedPDU):
        ...
        msg.getComponentByPosition(2).setComponentByType(pdu.tagSet, pdu, verifyConstraints=False, matchTags=False, matchConstraints=False)
        debug.logger & debug.flagMP and debug.logger('generateRequestMsg: %s' % (msg.prettyPrint(),))
        try:
            return (
             securityParameters, encoder.encode(msg))
        except PyAsn1Error:
    ...
    def generateResponseMsg(self, snmpEngine, messageProcessingModel, globalData, maxMessageSize, securityModel, securityEngineID, securityName, securityLevel, scopedPDU, securityStateReference):
        ...
        try:
            return (
             communityName, encoder.encode(msg))
        except PyAsn1Error:
    ...
    def processIncomingMsg(self, snmpEngine, messageProcessingModel, maxMessageSize, securityParameters, securityModel, securityLevel, wholeMsg, msg):
        ...
        snmpEngine.observer.storeExecutionContext(snmpEngine, 'rfc2576.processIncomingMsg:writable', scope)
        snmpEngine.observer.clearExecutionContext(snmpEngine, 'rfc2576.processIncomingMsg:writable')
        try:
            (securityName, contextEngineId, contextName) = self._com2sec(snmpEngine, scope.get('communityName', communityName), scope.get('transportInformation', transportInformation))
            ...
            raise error.StatusInformation(errorIndication=errind.unknownCommunityName, communityName=communityName)

        (snmpEngineID,) = snmpEngine.msgAndPduDsp.mibInstrumController.mibBuilder.importSymbols('__SNMP-FRAMEWORK-MIB', 'snmpEngineID')
        securityEngineID = snmpEngineID.syntax
        snmpEngine.observer.storeExecutionContext(snmpEngine, 'rfc2576.processIncomingMsg', dict(transportInformation=transportInformation, securityEngineId=securityEngineID, securityName=securityName, communityName=communityName, contextEngineId=contextEngineId, contextName=contextName))
        ...
        scopedPDU = (
         contextEngineId, contextName, msg.getComponentByPosition(2).getComponent())
        ...
        return (
         securityEngineID, securityName, scopedPDU, maxSizeResponseScopedPDU, securityStateReference)
