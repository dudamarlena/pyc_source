...
# Embedded file name: build/bdist.linux-x86_64/egg/pysnmp/hlapi/lcd.py
...
__all__ = [
 'CommandGeneratorLcdConfigurator', 'NotificationOriginatorLcdConfigurator']

class AbstractLcdConfigurator(object):
    __module__ = __name__
    nextID = nextid.Integer(4294967295)
    cacheKeys = []
    ...
    def configure(self, snmpEngine, authData, transportTarget, contextName, **options):
        ...
        if isinstance(authData, CommunityData):
            if authData.communityIndex not in cache['auth']:
                config.addV1System(snmpEngine, authData.communityIndex, authData.communityName, authData.contextEngineId, authData.contextName, authData.tag, authData.securityName)
                cache['auth'][authData.communityIndex] = authData
        elif isinstance(authData, UsmUserData):
            authDataKey = (
             authData.userName, authData.securityEngineId)
        ...
        if paramsKey in cache['parm']:
            ...
            config.addTargetParams(snmpEngine, paramsName, authData.securityName, authData.securityLevel, authData.mpModel)
        ...
        if transportTarget.transportDomain in cache['tran']:
            ...
            cache['tran'][transportTarget.transportDomain] = (
             transport, 1)
        transportKey = (paramsName, transportTarget.transportDomain, transportTarget.transportAddr, transportTarget.timeout, transportTarget.retries, transportTarget.tagList, transportTarget.iface)
        if transportKey in cache['addr']:
            ...
            config.addTargetAddr(snmpEngine, addrName, transportTarget.transportDomain, transportTarget.transportAddr, paramsName, transportTarget.timeout * 100, transportTarget.retries, transportTarget.tagList)
            cache['addr'][transportKey] = (
             addrName, 1)
        return (
         addrName, paramsName)
...
class NotificationOriginatorLcdConfigurator(AbstractLcdConfigurator):
    ...
    def configure(self, snmpEngine, authData, transportTarget, notifyType, contextName, **options):
        ...
        if not transportTarget.tagList:
            transportTarget.tagList = str(hash((authData.securityName, transportTarget.transportAddr)))
        if isinstance(authData, CommunityData) and not authData.tag:
            authData.tag = transportTarget.tagList.split()[0]
        (addrName, paramsName) = self._cmdGenLcdCfg.configure(snmpEngine, authData, transportTarget, contextName, **options)
        ...
        for tag in tagList:
            ...
            if notifyNameKey in cache['name']:
                ...
                cache['name'][notifyNameKey] = (
                 notifyName, paramsName, 1)

        authDataKey = (
         authData.securityName, authData.securityModel, authData.securityLevel, contextName)
        if authDataKey in cache['auth']:
            (authDataX, subTree, useCount) = cache['auth'][authDataKey]
            cache['auth'][authDataKey] = (authDataX, subTree, useCount + 1)
        else:
            subTree = (1, 3, 6)
            config.addTrapUser(snmpEngine, authData.securityModel, authData.securityName, authData.securityLevel, subTree, contextName=contextName)
    ...
    def unconfigure(self, snmpEngine, authData=None, contextName=null, **options):
        ...
        (addrNames, paramsNames) = self._cmdGenLcdCfg.unconfigure(snmpEngine, authData, contextName, **options)
        ...
        for ((notifyName, paramsName, useCount), notifyNameKey) in notifyAndParamsNames:
            ...
            if useCount:
                cache['name'][notifyNameKey] = (
                 notifyName, paramsName, useCount)
            else:
        ...
        for authDataKey in authDataKeys:
            ...
            useCount -= 1
            if useCount:
                cache['auth'][authDataKey] = (
                 authDataX, subTree, useCount)
            else:
                config.delTrapUser(snmpEngine, authDataX.securityModel, authDataX.securityName, authDataX.securityLevel, subTree)
                del cache['auth'][authDataKey]