...
# Embedded file name: build/bdist.linux-x86_64/egg/pysnmp/smi/mibs/SNMPv2-TC.py
# Compiled at: 2019-08-18 17:24:05
import sys, inspect, string
...
class TextualConvention(object):
    ...
    def prettyOut(self, value):
        ...
        if self.displayHint and (self.__integer.isSuperTypeOf(self, matchConstraints=False) and not self.getNamedValues() or self.__unsigned32.isSuperTypeOf(self, matchConstraints=False) or self.__timeticks.isSuperTypeOf(self, matchConstraints=False)):
            _ = lambda t, f=0: (t, f)
            ...
            if displayHintType == 'x':
                ...
                outputValue = ['B']
                while runningValue:
                    outputValue.insert(0, '%d' % (runningValue & 1))
                    runningValue >>= 1

                return ('').join(outputValue)
            else:
                raise SmiError('Unsupported numeric type spec "%s" at %s' % (displayHintType, self.__class__.__name__))
                    ...
                    if displayFormat == 'a':
                        ...
                        while numberString:
                            ...
                            try:
                                ...
                                raise SmiError('Display format eval failure: %s: %s' % (numberString, sys.exc_info()[1]))
    ...
    def prettyIn(self, value):
        ...
        if self.displayHint and (self.__integer.isSuperTypeOf(self, matchConstraints=False) and self.getNamedValues() or self.__unsigned32.isSuperTypeOf(self, matchConstraints=False) or self.__timeticks.isSuperTypeOf(self, matchConstraints=False)):
            value = str(value)
            _ = lambda t, f=0: (t, f)
            (displayHintType, decimalPrecision) = _(*self.displayHint.split('-'))
            ...
            numDigits = {'x': octets.str2octs(string.hexdigits), 'o': octets.str2octs(string.octdigits), 'd': octets.str2octs(string.digits)}
            ...
            while runningValue and displayHint:
                if displayHint[0] == '*':
                    raise SmiError('Can\'t parse "*" in DISPLAY-HINT (%s)' % self.__class__.__name__)
                octetLength = ''
                while displayHint and displayHint[0] in string.digits:
                    octetLength += displayHint[0]
                    displayHint = displayHint[1:]
                ...
                if displayFormat in ('a', 't'):
                    ...
                    try:
                        ...
                        raise SmiError('Display format eval failure: %s: %s' % (runningValue[:guessedOctetLength], sys.exc_info()[1]))
                    else:
                        num_as_bytes = []
                        if num:
                            while num:
                                num_as_bytes.append(num & 255)
                                num >>= 8
...
class RowStatus(TextualConvention, Integer):
    ...
    namedValues = NamedValues(('notExists', 0), ('active', 1), ('notInService', 2), ('notReady', 3), (
     'createAndGo', 4), ('createAndWait', 5), ('destroy', 6))
    (stNotExists, stActive, stNotInService, stNotReady, stCreateAndGo, stCreateAndWait, stDestroy) = list(range(7))
    stateMatrix = {(stCreateAndGo, stNotExists): (RowCreationWanted, stActive), (stCreateAndGo, stNotReady): (InconsistentValueError, stNotReady), (stCreateAndGo, stNotInService): (InconsistentValueError, stNotInService), (stCreateAndGo, stActive): (InconsistentValueError, stActive), (stCreateAndWait, stNotExists): (RowCreationWanted, stActive), (stCreateAndWait, stNotReady): (InconsistentValueError, stNotReady), (stCreateAndWait, stNotInService): (InconsistentValueError, stNotInService), (stCreateAndWait, stActive): (InconsistentValueError, stActive), (stActive, stNotExists): (InconsistentValueError, stNotExists), (stActive, stNotReady): (InconsistentValueError, stNotReady), (stActive, stNotInService): (None, stActive), (stActive, stActive): (None, stActive), (stNotInService, stNotExists): (InconsistentValueError, stNotExists), (stNotInService, stNotReady): (InconsistentValueError, stNotReady), (stNotInService, stNotInService): (None, stNotInService), (stNotInService, stActive): (None, stActive), (stDestroy, stNotExists): (RowDestructionWanted, stNotExists), (stDestroy, stNotReady): (RowDestructionWanted, stNotExists), (stDestroy, stNotInService): (RowDestructionWanted, stNotExists), (stDestroy, stActive): (RowDestructionWanted, stNotExists), (stNotExists, stNotExists): (None, stNotExists)}

    def setValue(self, value):
        if value is None:
            value = univ.noValue
        value = self.clone(value)
        (excValue, newState) = self.stateMatrix.get((value.hasValue() and value or self.stNotExists, self.hasValue() and self or self.stNotExists), (
        ...
        if excValue is not None:
            excValue = excValue(msg='Exception at row state transition from %r to %r yields state %r and exception' % (self, value, newState), syntax=newState)
            raise excValue
        return newState
