...
# Embedded file name: build/bdist.linux-x86_64/egg/pysnmp/proto/api/v1.py
...
class VarBindAPI(object):
    ...
    def getOIDVal(varBind):
        return (
         varBind[0], varBind[1].getComponent(1))


apiVarBind = VarBindAPI()
getNextRequestID = nextid.Integer(16777215)
...
class PDUAPI(object):
    ...
    def setDefaults(self, pdu):
        ...
        pdu.setComponentByPosition(1, self._errorStatus, verifyConstraints=False, matchTags=False, matchConstraints=False)
        pdu.setComponentByPosition(2, self._errorIndex, verifyConstraints=False, matchTags=False, matchConstraints=False)
    ...
    def getErrorIndex(pdu, muteErrors=False):
        ...
        if errorIndex > len(pdu[3]):
            ...
            raise error.ProtocolError('Error index out of range: %s > %s' % (errorIndex, len(pdu[3])))
        return errorIndex
