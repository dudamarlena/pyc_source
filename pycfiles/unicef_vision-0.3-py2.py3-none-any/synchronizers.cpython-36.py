...
# [GCC 8.4.0]
# Embedded file name: /home/jojo/workspace/vision/src/unicef_vision/synchronizers.py
# Compiled at: 2019-02-05 16:24:31
# Size of source mod 2**32: 9259 bytes
...
class DataSynchronizer:
    ...
    def logger_parameters(self):
        return {'handler_name':self.__class__.__name__, 
    ...
    def sync(self):
        ...
        loader_kwargs.update({kwarg_name:getattr(self, kwarg_name) for kwarg_name in self.LOADER_EXTRA_KWARGS})
...
class VisionDataSynchronizer(DataSynchronizer):
    ...
    def _get_kwargs(self):
        return {'business_area_code':self.business_area_code, 
...
class MultiModelDataSynchronizer(VisionDataSynchronizer):
    ...
    def _get_field_value(self, field_name, field_json_code, json_item, model):
        if field_json_code in self.DATE_FIELDS:
            ...
            if field_name in self.MODEL_MAPPING.keys():
                ...
                if isinstance(related_model, types.FunctionType):
                    ...
                    result = (related_model.objects.get)(**{reversed_dict[field_json_code]: json_item.get(field_json_code, None)})
            else:
                ...
                value_handler = self.FIELD_HANDLERS.get({y:x for x, y in self.MODEL_MAPPING.items()}.get(model), {}).get(field_name, None)
                if value_handler:
    ...
    def _process_record(self, json_item):
        try:
            for model_name, model in self.MODEL_MAPPING.items():
                mapped_item = dict([(field_name, self._get_field_value(field_name, field_json_code, json_item, model)) for field_name, field_json_code in self.MAPPING[model_name].items()])
                kwargs = dict([(field_name, value) for field_name, value in mapped_item.items() if model._meta.get_field(field_name).unique])
                if not kwargs:
                ...
                defaults = dict([(field_name, value) for field_name, value in mapped_item.items() if field_name not in kwargs.keys()])
