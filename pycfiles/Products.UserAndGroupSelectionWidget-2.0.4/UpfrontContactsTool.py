...
# Embedded file name: build/bdist.linux-i686/egg/Products/UpfrontContacts/UpfrontContactsTool.py
...
from Products.UpfrontContacts.organisation_schema import schema as organisation_schema
...
class UpfrontContactsTool(UniqueObject, SimpleItem):
    ...
    security.declareProtected(permissions.View, 'uploadCSVFile')
    ...
    def uploadCSVFile(self, REQUEST=None):
        ...
        if REQUEST:
            ...
            self.REQUEST.RESPONSE.redirect('%s/uc_import_contacts_format' % url)
        return
    ...
    security.declareProtected(permissions.View, 'csv_parser')
    ...
    security.declareProtected(permissions.View, 'contactTypeFieldNames')
        ...
        for field in fields:
            if field.type in ('string', 'text', 'date', 'boolean', 'integer', 'datetime'):
                ...
                for key in field.subfields:
                    field_names.append('%s.%s' % (field.getName(), key))
    ...
    security.declareProtected(permissions.View, 'importCSV')

    def importCSV(self, delimiter='comma', quote_char='double_quote', coding='utf-8', import_type=None, link_via_title=0, REQUEST=None):
                ...
                for v in val:
                    if v < row_len and row[v]:
                        if field_type == 'boolean':
                            elem = row[v]
                            b = type(elem) is type('string') and elem.lower() in ('true',
                                                                                  'yes',
                                                                                  't',
                                                                                  'y') and '1' or elem.lower() in ('false',
                                                                                                                   'no',
                                                                                                                   'f',
                                                                                                                   'n') and ''
                            if b is False:
        ...
        if len(errors) > 0:
            container = self.unrestrictedTraverse(self.REQUEST.get('current_path'))
        ...
        if link_via_title:
            org_titles = {}
            for org in self.contentValues(filter={'portal_type': 'Organisation'}):
                org_titles.setdefault(org.Title(), []).append(org)

            person_titles = {}
            for person in self.contentValues(filter={'portal_type': 'Person'}):
    ...
    def exportCSV(self, contacts=None, fields=None, delimiter='comma', quote_char='double_quote', coding='latin-1', export_type='Person'):
        """
        ...
        container = self.unrestrictedTraverse(self.REQUEST.get('current_path'))
        if contacts is None:
            contacts = container.listFolderContents(contentFilter={'portal_type': export_type})
