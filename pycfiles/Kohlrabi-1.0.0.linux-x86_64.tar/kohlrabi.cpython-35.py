...
# [GCC 8.4.0]
# Embedded file name: /media/storage/eyes/virtualenv/kohlrabi/lib/python3.5/site-packages/libkohlrabi/kohlrabi.py
# Compiled at: 2016-04-01 11:40:01
# Size of source mod 2**32: 7975 bytes
"""
Kohlrabi object class.
"""
import asyncio, logging, os, random, aioredis, msgpack
...
class Kohlrabi(object):
    ...
    def __init__(self, redis_ip: str='127.0.0.1',
                 redis_port: int=6379, side: int=0,
                 thread_count: int=2):
        """
    ...
    def __del__(self):
        self._loop.set_exception_handler(lambda *args**args: True)
    ...
    def begin(self):
        ...
        logger.info('Kohlrabi registered as worker {}.'.format(self._loop.run_until_complete(self._get_num_servers_registered())))
        try:
            try:
    ...
    def apply_task(self, task: ClientTaskBase, *args, **kwargs):
        ...
        """
        ack_id = random.randint(1000000000, 10000000000)
