...
# [GCC 8.4.0]
# Embedded file name: /home/stas/workspace/partners/flask_restframework/flask_restframework/tests/test_flask_app.py
# Compiled at: 2017-07-14 09:11:18
# Size of source mod 2**32: 25271 bytes
import builtins as @py_builtins, _pytest.assertion.rewrite as @pytest_ar, base64, datetime, json, unittest
...
class SimpleFlaskAppTest(unittest.TestCase):
    ...
    def test_validation_mongoengine_will_work_with_model_serializer(self):
        ...
        class Doc(db.Document):
            value = db.StringField(validation=RegexpValidator('\\d+', message='Bad value').for_mongoengine())
    ...
    def test_resource_decorator(self):
        ...
        self.assertEqual(json.loads(resp.data.decode('utf-8')), {'field': ['Field is required']})
    ...
    def testSimpleResourceAndRouter(self):
        ...
        self.assertSetEqual(set(Resource.get_allowed_methods()), {'get', 'post', 'put', 'patch', 'delete'})
    ...
    def testModelResource(self):
        ...
        ins = Model.objects.create(base=Base.objects.create(title='1'), f1='1', f2=True, f3='1', embedded={'value': '123'}, listf=[{'value': '234'}], dictf={'key': 'value'})
        Model.objects.create(base=Base.objects.create(title='2'), f1='2', f2=True, f3='2', embedded={'value': '123'}, listf=[{'value': '234'}])
        ...
        for i in range(10):
            Model.objects.create(base=Base.objects.create(title='1'), f1='1', f2=True, f3='2')
        ...
        data = self._parse(resp.data)
        self.assertEqual(len(data['results']), 2)
        resp = self.client.put('/test/{}'.format(ins.id), data=json.dumps({'f3': 'OLALA'}), headers={'Content-Type': 'application/json'})
    ...
    def testMongoEngineForeignKeyField(self):
        ...
        self.app.add_url_rule('/api', view_func=R.as_view('test2'), methods=[
         'GET', 'POST'])
        resp = self.client.post('/api', data=json.dumps({}), headers={'Content-Type': 'application/json'})
        self.assertEqual(resp.status_code, 400)
    ...
    def testSerialization(self):
        ...
        self.assertEqual(list(map(lambda i: i['value'], data)), [
         '1', '2'])
    ...
    def testModelSerialization(self):
        ...
        data = Serializer(QuerysetWrapper.from_queryset(Col.objects.all())).serialize()
        for item in data:
...
class TestMiddlewares(SimpleFlaskAppTest):
    ...
    def test_authorization(self):
        ...
        self.assertEqual(resp.status_code, 401)
        resp = self.client.get('/test_basic?k=v', headers={'Authorization': b'Basic ' + base64.b64encode(b'admin:1')})
        self.assertEqual(resp.status_code, 200)
...
class TestModelResources(SimpleFlaskAppTest):
    ...
    def test_cant_update_doc_id(self):
        ...
        serializer = self.Serializer(data, context={'instance': instance})
    ...
    def test_reference_field_serialization(self):
        ...
        Ref.objects.delete()
        Doc.objects.delete()
        r1 = Ref.objects.create(**dict(value='1'))
        r2 = Ref.objects.create(**dict(value='2'))
        d = Doc.objects.create(**dict(ref=r1, ref_list=[
         r1, r2]))
    ...
    def test_update_embedded_doc(self):
        doc = self.BaseDoc.objects.create(inner={'value': '1'}, req_field='1')
        serializer = self.Serializer(dict(inner={'value': '2'}, req_field='1'))
        ...
        serializer = self.Serializer(dict(inner={}, req_field='1'))
    ...
    def test_patch_several_fields(self):
        ...
        resp = self.client.patch('/test_resource/{}'.format(id), data=json.dumps({'string': 'other string', 
         'bool': True}), content_type='application/json')
        self.assertEqual(resp.status_code, 200, resp.data)
        data = self._parse(resp.data)
        self.assertEqual(data['string'], 'other string')
        self.assertEqual(data['bool'], True)
        self.assertEqual(data['inner'], {'value': '1'})
        self.assertEqual(data['req_field'], 'required')
        resp = self.client.patch('/test_resource/{}'.format(id), data=json.dumps({'string': 'other string', 
         'bool': 'Very Bad!', 
         'integer_field': 'Not an int!'}), content_type='application/json')
        self.assertEqual(resp.status_code, 400)
        data = self._parse(resp.data)
        self.assertEqual(data['bool'], ['Boolean is required'])
    ...
    def test_unique_embedded(self):
        ...
        Col.objects.delete()
        Col.objects.create(value={'ru': 'ru', 
         'en': 'en'})
        ...
        s = S(data=dict(value=dict(ru='ru', en='en')))
        self.assertEqual(s.validate(), False)
        self.assertTrue('value' in s.errors)
        s = S(data=dict(value=dict(ru='ru1', en='en')))
    ...
    def test_unique_validation(self):
        ...
        instance = UniqueCol.objects.create(value='1', dt_value=datetime.datetime(2000, 1, 1), int_value=1)
        s = SimpleSerializer(data=dict(value='1'))
        self.assertEqual(s.validate(), False)
        self.assertEqual(s.errors, {'value': ['Trying to save duplicate value 1']})
        s = Serialzier(data=dict(value='1', dt_value='2000-01-01 00:00:00', int_value=1))
        self.assertEqual(s.validate(), False)
        self.assertEqual(s.errors, {'dt_value': [
                      'Trying to save duplicate value 2000-01-01 00:00:00'], 
         'int_value': [
                       'Trying to save duplicate value 1'], 
         'value': [
                   'Trying to save duplicate value 1']})
        ...
        resp = self.client.put('/test_unique_validation/{}'.format(instance.id), data=json.dumps({'value': '1', 
         'dt_value': '2000-01-01 00:00:00', 
         'int_value': 2}), content_type='application/json')
        self.assertEqual(resp.status_code, 200, resp.data)
