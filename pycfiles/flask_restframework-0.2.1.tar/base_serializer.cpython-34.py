...
# Size of source mod 2**32: 13083 bytes
...
class BaseSerializer:
    ...
    def from_queryset(cls, qs, context=None):
        ...
        :return:
        """
        return cls(QuerysetWrapper.from_queryset(qs), context)
    ...
    def from_instance(cls, item, context=None):
        ...
        :return:
        """
        return cls(InstanceWrapper.from_instance(item), context)
    ...
    def serialize(self):
        ...
        if isinstance(data, QuerysetWrapper):
            output = [self._serialize_item(item) for item in data.get_data()]
        else:
