...
def test_embedded_inner_serialization(main_record):
    ...
    if not @py_assert2:
        @py_format10 = @pytest_ar._call_reprcompare(('==', ), (@py_assert2,), ('%(py1)s == %(py9)s\n{%(py9)s = %(py3)s(embedded_inner=%(py5)s, embedded_list_inner=%(py7)s)\n}', ), (@py_assert0, @py_assert8)) % {'py5': @pytest_ar._saferepr(@py_assert4),  'py1': @pytest_ar._saferepr(@py_assert0),  'py9': @pytest_ar._saferepr(@py_assert8),  'py3': @pytest_ar._saferepr(dict) if 'dict' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(dict) else 'dict',  'py7': @pytest_ar._saferepr(@py_assert6)}
...
def test_reference_serialization(main_record):
    ...
    if not @py_assert2:
        @py_format11 = @pytest_ar._call_reprcompare(('==',), (@py_assert2,), ('%(py1)s == %(py10)s\n{%(py10)s = %(py3)s(%(py8)s\n{%(py8)s = %(py6)s\n{%(py6)s = %(py4)s.related_inner\n}.id\n})\n}',), (@py_assert0, @py_assert9)) % {'py1': @pytest_ar._saferepr(@py_assert0),  'py8': @pytest_ar._saferepr(@py_assert7),  'py4': @pytest_ar._saferepr(main_record) if 'main_record' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(main_record) else 'main_record',  'py3': @pytest_ar._saferepr(str) if 'str' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(str) else 'str',  'py6': @pytest_ar._saferepr(@py_assert5),  'py10': @pytest_ar._saferepr(@py_assert9)}
    ...
    if not @py_assert2:
        @py_format14 = @pytest_ar._call_reprcompare(('==',), (@py_assert2,), ('%(py1)s == %(py13)s\n{%(py13)s = %(py3)s(%(py11)s\n{%(py11)s = %(py4)s(%(py6)s, %(py9)s\n{%(py9)s = %(py7)s.related_list_inner\n})\n})\n}',), (@py_assert0, @py_assert12)) % {'py1': @pytest_ar._saferepr(@py_assert0),  'py4': @pytest_ar._saferepr(map) if 'map' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(map) else 'map',  'py3': @pytest_ar._saferepr(list) if 'list' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(list) else 'list',  'py13': @pytest_ar._saferepr(@py_assert12),  'py6': @pytest_ar._saferepr(@py_assert5),  'py11': @pytest_ar._saferepr(@py_assert10),  'py9': @pytest_ar._saferepr(@py_assert8),  'py7': @pytest_ar._saferepr(main_record) if 'main_record' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(main_record) else 'main_record'}
