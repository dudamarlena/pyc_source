...
# Compiled at: 2017-07-14 09:15:36
...
class MongoEngineFieldWrapper(BaseFieldWrapper):
    ...
    def get_serializer_field(self, key):
        ...
        validators = []
        if hasattr(self.field, 'validation'):
...
class SqlAlchemyFieldWrapper(BaseFieldWrapper):
    ...
    def get_serializer_field(self, key):
        ...
        return serializerFieldCls(blank=self.field.nullable, required=not self.field.nullable, read_only=self.field.primary_key)
...
class SqlAlchemyModelWrapper(BaseModelWrapper):
    ...
    def get_fields(self):
        return {key:SqlAlchemyFieldWrapper(value) for key, value in self.modelClass.__table__.columns.items()}