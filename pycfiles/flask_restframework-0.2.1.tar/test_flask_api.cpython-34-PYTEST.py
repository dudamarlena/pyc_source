...
def test_ignore_excluded_fields():
    ...
    if not @py_assert16:
        @py_format19 = @pytest_ar._call_reprcompare(('==',), (@py_assert16,), ('%(py15)s\n{%(py15)s = %(py0)s(%(py13)s\n{%(py13)s = %(py11)s\n{%(py11)s = %(py9)s\n{%(py9)s = %(py7)s\n{%(py7)s = %(py5)s\n{%(py5)s = %(py1)s(%(py3)s)\n}.get_fields\n}()\n}.keys\n}()\n})\n} == %(py18)s',), (@py_assert14, @py_assert17)) % {'py15': @pytest_ar._saferepr(@py_assert14),  'py1': @pytest_ar._saferepr(S) if 'S' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(S) else 'S',  'py9': @pytest_ar._saferepr(@py_assert8),  'py3': @pytest_ar._saferepr(@py_assert2),  'py13': @pytest_ar._saferepr(@py_assert12),  'py18': @pytest_ar._saferepr(@py_assert17),  'py5': @pytest_ar._saferepr(@py_assert4),  'py11': @pytest_ar._saferepr(@py_assert10),  'py0': @pytest_ar._saferepr(sorted) if 'sorted' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(sorted) else 'sorted',  'py7': @pytest_ar._saferepr(@py_assert6)}
