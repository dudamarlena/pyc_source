...
# [GCC 8.4.0]
# Embedded file name: build/bdist.freebsd-7.0-RELEASE-amd64/egg/sizedupe/sizedupe.py
# Compiled at: 2008-06-24 14:11:47
"""
...
import bisect, bz2, colorsys, glob, hashlib, heapq, math, logging, os, optparse, stat, sys, threading, time
from collections import defaultdict
from logging import debug, info, warning
HASHCLASS = hashlib.md5
MINIMUM_SIZE = 512
CHUNK_SIZE = 1048576
ROUND_ROBIN_SIZE = 1048576
...
LABEL_GAP = 0.5 * INCH
PAGESIZE = (8.5, 11)
logging.basicConfig(level=logging.DEBUG, format='%(asctime)s\t%(levelname)s\t%(message)s', filename='size_and_dupe.log', filemode='a')
debug('*** Loaded consolidated_dupe module')
try:
    import reportlab, reportlab.pdfgen, reportlab.pdfgen.canvas
...
class DirectoryGraphMixin(FileGraphMixin):
    ...
    def children_and_heights(self, y0=START_Y, recursive=False):
        ...
        for child in self.children:
            ...
            yield (
             child, cur_height, new_height, child.depth)
...
class Color(tuple):
    ...
    def hlsrotate(self, angle, circle=1.0):
        return self.huerotate(angle, tofunc=self.to_hls, fromfunc=self.from_hls, circle=circle)

    def hsvrotate(self, angle, circle=1.0):
        return self.huerotate(angle, tofunc=self.to_hsv, fromfunc=self.from_hsv, circle=circle)
    ...
    def generatePDF(self, filename):
        ...
        for (y0, node, depth) in labels:
            if node.leaf:
                ...
                sizetext = ''
            self.pdf_draw_text(pdf, label_x, y0, MIN_LABEL, node.fullname.replace('&', '&amp;') + ' ' + sizetext)
...
def main():
    ...
    parser.add_option('-p', '--size-pdf', action='callback', callback=pdf_option_callback, dest='size_pdf', default=PDF_AVAILABLE, help='Produce graphical size report as a PDF file.')
    parser.add_option('-s', '--size-text', action='store_true', dest='size_text', default=True, help='Produce tab-delimited size report as a text file.')
    parser.add_option('-d', '--dupe-text', action='store_true', dest='dupe_text', default=False, help='Produce duplicate file report as a python text file.')
    parser.add_option('-t', '--threaded', action='store_true', dest='threaded', default=False, help='Scan directory arguments in parallel.')
    parser.add_option('-m', '--max-parallel', action='store', dest='max_walkers', type='int', default=5, help='Maximum number of directories to scan in parallel.')
    parser.add_option('-z', '--bzip2', action='store_true', dest='do_compress', default=False, help="Write size report files as bzip2'ed text files.")
    parser.add_option('-f', '--file-prefix', action='store', dest='file_prefix', default='sizereport_', help='Set the filename prefix for the produced files.')
