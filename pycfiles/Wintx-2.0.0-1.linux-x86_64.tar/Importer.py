...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.6/site-packages/wintx/interfaces/Importer.py
...
class Importer(Interface):
    ...
    def __init__(self, dataset_name, filter_dict={}, config_file=None):
        ...
        except WintxMalformedQueryError, err:
    ...
    def __checkString__(self, key, value):
        ...
        if type(value) is not type('') and type(value) is not type(''):
    ...
    def __checkInt__(self, key, value, type_date=False):
        ...
        if type_date:
            if type(value) is not type(datetime.now()):
                raise WintxImportError('Invalid value type for %s.' % key)
        elif type(value) is not type(1) and type(value) is not type(1.1):
            raise WintxImportError('Invalid value type for %s.' % key)
        if key not in self.filter_dict:
            return True
        else:
    ...
    def addRecord(self, latitude, longitude, leveltype, level, varname, time_value, value):
        ...
        if not self.checkLatitude(latitude) or not self.checkLongitude(longitude) or not self.checkLevel(level) or not self.checkLevelType(leveltype) or not self.checkVariableName(varname) or not self.checkTime(time_value):
            return
        else:
