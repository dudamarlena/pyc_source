...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.6/site-packages/wintx/__init__.py
...
from errors import *
__all__ = [
 'Fastener', 'Wintx', 'errors', 'interface']
...
def getWintxDict():
  ...
  """
    ...
    return {'latitude': 0.0, 'longitude': 0.0, 'time': datetime.now(), 'datatype': '', 
       'varname': '', 'leveltype': '', 'level': 0, 'value': None}
...
def getWintxIndexesDict():
  ...
  """
    from datetime import datetime
    return {'latitude': 0.0, 'longitude': 0.0, 'vertical': 0, 'time': datetime.now(), 'datatype': '', 
       'varname': '', 'level': 0, 'leveltype': ''}
...
def checkQueryDict(query, restricted_columns=None):
  ...
  """
    ...
    if restricted_columns is None:
        restricted_columns = []
    elif type(restricted_columns) is not type([]) and type(restricted_columns) is not type(()):
    ...
    for column in query:
        ...
        if column == 'latitude' or column == 'longitude' or column == 'level':
            if type(query[column]) is not type(1) and type(query[column]) is not type(1.1) and type(query[column]) is not type({}):
            ...
            if type(query[column]) is type({}):
                for comparison in query[column]:
                    ...
                    if type(query[column][comparison]) is not type(1.1) and type(query[column][comparison]) is not type(1):
            ...
            if type(query[column]) is not type('') and type(query[column]) is not type('') and type(query[column]) is not type([]):
            ...
            if type(query[column]) is type([]):
                for parameter in query[column]:
                    if type(parameter) is not type('') and type(parameter) is not type(''):
                        raise WintxMalformedQueryError("Invalid query comparison operand '%s' provided for parameter '%s'." % (query[column], column))

        elif column == 'time':
            if type(query[column]) is not type(datetime.now()) and type(query[column]) is not type({}):
                raise WintxMalformedQueryError("Invalid query comparison operation(s) provided for parameter '%s'." % column)
            if type(query[column]) is type({}):
                for comparison in query[column]:
                    if comparison not in comparison_operands:
...
def checkSortList(sort_list, restricted_columns=None):
  ...
  """
    ...
    for col_set in sort_list:
        ...
        if type(col_set[0]) is not type('') and type(col_set[0]) is not type(''):
            raise WintxMalformedSortError('Sort column name is not a string')
        if type(col_set[1]) is not type('') and type(col_set[1]) is not type(''):
...
def checkRecordDict(record):
    ...
    for key in record:
        if key == 'varname':
            if type(record[key]) is not type('') and type(record[key]) is not type(''):
