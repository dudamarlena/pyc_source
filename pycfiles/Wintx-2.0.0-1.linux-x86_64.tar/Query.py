...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.6/site-packages/wintx/interfaces/Query.py
...
class Query(Interface):
    ...
    """
        for geo_set in polygon:
            ...
            if type(geo_set[0]) is not type(1) and type(geo_set[0]) is not type(1.0) and type(geo_set[1]) is not type(1) and type(geo_set[1]) is not type(1.0):
