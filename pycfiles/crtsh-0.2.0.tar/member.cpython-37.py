...
# Embedded file name: /Users/aarontraas/workspace/personal/Clash-Royale-Clan-Tools/crtools/models/member.py
# Compiled at: 2019-09-17 16:19:41
...
def process_member_events(config, events):
    ...
    for event in events:
        if event['date'] == 0:
            continue
        processed_events.append({'date':datetime.fromtimestamp(event['date']).strftime('%x'), 
         'timestamp':event['date'], 
         'message':{'join':config['strings']['memberEventJoinedClan'], 
          'role change':config['strings']['memberEventRoleChange'].format(event['role']), 
          'quit':config['strings']['memberEventExitClan']}[event['event']]})
...
class ProcessedClanMember(ClanMember):
    ...
    def enrich_member_with_history(self, config, historical_member, days_from_donation_reset=0, now=datetime.utcnow()):
        ...
        self.events = process_member_events(config, historical_member['events'])
    ...
    def calc_special_status(self, config):
        ...
        if self.safe:
            if self.days_inactive >= config['activity']['threshold_warn']:
                self.vacation = True
    ...
    def calc_derived_member_stats(self, config, clan, warlog, current_war, days_from_donation_reset):
        ...
        self.leadership = self.role == 'leader' or 
