...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pywurfl/wurflprocessor.py
# Compiled at: 2011-01-06 14:58:23
"""
...
__copyright__ = 'Copyright 2004-2011, Armand Lynch'
__license__ = 'LGPL'
...
class DeviceHandler(object):
    ...
    def __init__(self, device):
        ...
        if 'actual_device_root' in device.attrib and device.attrib['actual_device_root'].lower() == 'true':
...
class WurflProcessor(object):
    ...
    def __init__(self, wurflxml, device_handler=None, options=None):
        ...
        if wurflxml.endswith('.gz'):
            ...
            zfile = ZipFile(wurflxml)
            file_handle = StringIO(zfile.read(zfile.namelist()[0]))
        else:
            file_handle = open(wurflxml, 'rb')
...
def main(processor_class, device_handler_class, option_parser):
    ...
    if args:
        ...
        sys.exit(1)
    wurfl = processor_class(wurflxml, device_handler=device_handler_class, options=options.__dict__)
    wurfl.process()
