...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pywurfl/ql.py
# Compiled at: 2011-01-06 14:58:07
"""
...
"""
import re, operator
from pyparsing import CaselessKeyword, Forward, Group, ParseException, QuotedString, StringEnd, Suppress, Word, ZeroOrMore, alphanums, alphas, nums, oneOf, delimitedList
...
def define_language():
    ...
    select_statement = (select_token + select_type)('select')
    expr = Forward()
    ...
    attribute_call = (attribute + Suppress('(') + method_args + Suppress(')'))('attribute_call')
    ...
    any_statement = (any_token + Suppress('(') + any_expr_list + Suppress(')') + binop + expr('rexpr'))('any_statement')
    all_token = CaselessKeyword('all')
    all_expr_list = expr_list('all_expr_list')
    all_statement = (all_token + Suppress('(') + all_expr_list + Suppress(')') + binop + expr('rexpr'))('all_statement')
...
def get_operators():
    ...
    return {'=': operator.eq, '!=': operator.ne, '<': operator.lt, '>': operator.gt, 
       '>=': operator.ge, '<=': operator.le, 'and': and_, 
       'or': or_}


ops = get_operators()

def expr_test_func(lexpr, op, rexpr):
    """
    ...
    def expr_tester(devobj):
        lvalue = _evaluate(devobj, lexpr)
        rvalue = _evaluate(devobj, rexpr)
        return ops[op](lvalue.py_value, rvalue.py_value)

    return expr_tester


def _evaluate(devobj, expression):
    """
    Evaluate an expression with respect to a device object
    """
    value = None
    ...
    if 'capability' in expression.keys():
        ...
        try:
            ...
            raise QueryLanguageError("Invalid capability '%s'" % capability)
        else:
            if isinstance(py_value, bool):
                value = TypeBool(py_value)
            elif isinstance(py_value, int):
                value = TypeNum(py_value)
            elif isinstance(py_value, basestring):
                value = TypeStr(py_value)
            else:
                raise QueryLanguageError("Unknown type '%s'" % py_value.__class__)
    else:
    ...
    for attribute in expression.attribute_concat:
        ...
        if 'attribute_call' in attribute.keys():
            ...
            for method_arg in attribute.attribute_call.method_args:
                ...
                try:
                    method_arg_value = _evaluate(devobj, method_arg.expr)
                except AttributeError, e:
...
def reduce_statement(exp):
    ...
    if exp.any_statement:
        for expr in exp.any_statement.any_expr_list:
            funcs.append(expr_test_func(expr, exp.operator, exp.rexpr))
        ...
        func = ops['and']
    return reduce_funcs(func, funcs)
...
def test_generator(ql_result):
    ...
    while where_test:
        if where_test.any_statement or where_test.all_statement:
            ...
            func = expr_test_func(where_test.lexpr, where_test.operator, where_test.rexpr)
...
def QL(devices):
    ...
    def query(qstr, instance=True):
        ...
        if isinstance(qstr, str):
            raise UnicodeError('query must be a unicode string')
        qstr = qstr.replace('\n', ' ').replace('\r', ' ') + '*'
        try:
            qres = language.parseString(qstr)
            tester = test_generator(qres)
            if qres.select.type == 'ua':
                return (x.devua for x in devices.devids.itervalues() if tester(x))
            if qres.select.type == 'id':
                return (x.devid for x in devices.devids.itervalues() if tester(x))
            if instance:
                return (x() for x in devices.devids.itervalues() if tester(x))
            return (x for x in devices.devids.itervalues() if tester(x))
