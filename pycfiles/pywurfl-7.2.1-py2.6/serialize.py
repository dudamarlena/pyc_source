...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pywurfl/serialize.py
# Compiled at: 2011-01-06 14:58:13
"""
...
class Serialize(object):
    ...
    def _walk(self, device, devices):
        ...
        groups = {}
        for cap in (c for c in device.__dict__ if c not in self.common_caps if not c.startswith('_')):
            value = self._get_value(getattr(device, cap))
            try:
                groups[self._find_group(cap)].append((cap, value))
            except (AttributeError, KeyError):
                groups[self._find_group(cap)] = [
                 (
                  cap, value)]
