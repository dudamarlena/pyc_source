...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/crwy/utils/scrapy_plugs/pipelines.py
...
class MysqlSavePipeline(object):
    ...
    def __init__(self, db_name=None, db_host=None, db_port=None, db_username=None, db_password=None, db_charset=None, db_cursorclass=None):
    ...
    def from_crawler(cls, crawler):
        ...
        return cls(db_name=db_name, db_host=db_host, db_port=db_port, db_username=db_username, db_password=db_password, db_charset=db_charset, db_cursorclass=db_cursorclass)
    ...
    def open_spider(self, spider):
        self.mysql_handle = MysqlHandle(host=self.db_host, port=self.db_port, user=self.db_username, password=self.db_password, db=self.db_name, charset=self.db_charset, cursorclass=self.db_cursorclass)
...
class SqlalchemySavePipeline(object):
    ...
    def open_spider(self, spider):
        self.sqlalchemy_handle = SqlalchemyHandle(db_url=self.db_url, echo=self.echo)
