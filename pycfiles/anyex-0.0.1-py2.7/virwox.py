...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/anyex/virwox.py
...
class virwox(Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(virwox, self).describe(), {'id': 'virwox', 
           'name': 'VirWoX', 
           'countries': [
                       'AT', 'EU'], 
           'rateLimit': 1000, 
           'has': {'CORS': True}, 
           'urls': {'logo': 'https://user-images.githubusercontent.com/1294454/27766894-6da9d360-5eea-11e7-90aa-41f2711b7405.jpg', 
                    'api': {'public': 'http://api.virwox.com/api/json.php', 
    ...
    def fetch_market_price(self, symbol, params={}):
        self.load_markets()
        response = self.publicPostGetBestPrices(self.extend({'symbols': [
                     symbol]}, params))
        result = response['result']
        return {'bid': self.safe_float(result[0], 'bestBuyPrice'), 
    ...
    def fetch_ticker(self, symbol, params={}):
        ...
        start = end - 86400000
        response = self.publicGetGetTradedPriceVolume(self.extend({'instrument': symbol, 
    ...
    def fetch_trades(self, symbol, since=None, limit=None, params={}):
        ...
        response = self.publicGetGetRawTradeData(self.extend({'instrument': symbol, 
           'timespan': 3600}, params))
        result = response['result']
        trades = result['data']
    ...
    def create_order(self, symbol, type, side, amount, price=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        order = {'instrument': market['symbol'], 
    ...
    def cancel_order(self, id, symbol=None, params={}):
        return self.privatePostCancelOrder(self.extend({'orderID': id}, params))
    ...
    def sign(self, path, api='public', method='GET', params={}, headers=None, body=None):
        ...
        nonce = self.nonce()
        if method == 'GET':
            url += '?' + self.urlencode(self.extend({'method': path, 
               'id': nonce}, auth, params))
        else:
            headers = {'Content-Type': 'application/json'}
            body = self.json({'method': path, 
