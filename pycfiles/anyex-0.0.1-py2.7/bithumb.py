...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/anyex/bithumb.py
...
class bithumb(Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(bithumb, self).describe(), {'id': 'bithumb', 
           'name': 'Bithumb', 
           'countries': 'KR', 
           'rateLimit': 500, 
           'has': {'CORS': True, 
                   'fetchTickers': True, 
                   'withdraw': True}, 
           'urls': {'logo': 'https://user-images.githubusercontent.com/1294454/30597177-ea800172-9d5e-11e7-804c-b9d4fa9b56b0.jpg', 
                    'api': {'public': 'https://api.bithumb.com/public', 
    ...
    def fetch_balance(self, params={}):
        self.load_markets()
        response = self.privatePostInfoBalance(self.extend({'currency': 'ALL'}, params))
    ...
    def fetch_ticker(self, symbol, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.publicGetTickerCurrency(self.extend({'currency': market['base']}, params))
    ...
    def fetch_trades(self, symbol, since=None, limit=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.publicGetRecentTransactionsCurrency(self.extend({'currency': market['base'], 
           'count': 100}, params))
    ...
    def create_order(self, symbol, type, side, amount, price=None, params={}):
        ...
        if type == 'limit':
            request = {'order_currency': market['id'], 'Payment_currency': market['quote'], 
               'units': amount, 
               'price': price, 
               'type': 'bid' if side == 'buy' else 'ask'}
            method += 'Place'
        elif type == 'market':
            request = {'currency': market['id'], 'units': amount}
    ...
    def cancel_order(self, id, symbol=None, params={}):
        ...
        return self.privatePostTradeCancel({'order_id': id, 
           'type': params['side'], 
    ...
    def sign(self, path, api='public', method='GET', params={}, headers=None, body=None):
        ...
        if api == 'public':
            ...
            body = self.urlencode(self.extend({'endpoint': endpoint}, query))
            nonce = str(self.nonce())
            auth = endpoint + '\x00' + body + '\x00' + nonce
