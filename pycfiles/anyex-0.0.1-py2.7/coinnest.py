...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/anyex/coinnest.py
# Compiled at: 2018-04-27 06:36:00
...
class coinnest(Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(coinnest, self).describe(), {'id': 'coinnest', 
           'name': 'coinnest', 
           'countries': 'KR', 
           'rateLimit': 1000, 
           'has': {'fetchOpenOrders': True}, 
           'urls': {'logo': 'https://user-images.githubusercontent.com/1294454/38065728-7289ff5c-330d-11e8-9cc1-cf0cbcb606bc.jpg', 
                    'api': {'public': 'https://api.coinnest.co.kr/api', 
    ...
    def fetch_ticker(self, symbol, params={}):
        ...
        ticker = self.publicGetPubTicker(self.extend({'coin': market['baseId']}, params))
        return self.parse_ticker(ticker, market)

    def fetch_order_book(self, symbol, limit=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        orderbook = self.publicGetPubDepth(self.extend({'coin': market['baseId']}, params))
    ...
    def fetch_trades(self, symbol, since=None, limit=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        trades = self.publicGetPubTrades(self.extend({'coin': market['baseId']}, params))
    ...
    def fetch_balance(self, params={}):
        ...
        for i in range(0, len(balancKeys)):
            ...
            if currency not in list(result.keys()):
                result[currency] = {'free': 0.0, 'used': 0.0, 
                   'total': 0.0}
            type = type == 'used' if 1 else 'free'
            result[currency][type] = float(response[key])
            otherType = type == 'free' if 1 else 'used'
    ...
    def create_order(self, symbol, type, side, amount, price=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.privatePostTradeAdd(self.extend({'coin': market['baseId'], 
           'type': side, 
           'number': amount, 
           'price': price}, params))
    ...
    def cancel_order(self, id, symbol=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.privatePostTradeCancel(self.extend({'id': id, 
           'coin': market['baseId']}, params))
        return response

    def fetch_order(self, id, symbol=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        order = self.privatePostTradeFetchtrust(self.extend({'id': id, 
           'coin': market['baseId']}, params))
    ...
    def fetch_open_orders(self, symbol=None, since=None, limit=None, params={}):
        return self.fetch_orders(symbol, since, limit, self.extend({'type': '1'}, params))
    ...
    def sign(self, path, api='public', method='GET', params={}, headers=None, body=None):
        ...
        if api == 'public':
            ...
            body = self.urlencode(self.extend(params, {'key': self.apiKey, 
               'nonce': self.nonce()}))
    ...
    def request(self, path, api='public', method='GET', params={}, headers=None, body=None):
        ...
        if not response or response == 'nil' or status:
            ErrorClass = self.safe_value({'100': DDoSProtection, 
               '101': DDoSProtection, 
               '104': AuthenticationError, 
               '105': AuthenticationError, 
               '106': DDoSProtection}, status, ExchangeError)
