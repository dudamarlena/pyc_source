...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/anyex/bitbank.py
...
class bitbank(Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(bitbank, self).describe(), {'id': 'bitbank', 
           'name': 'bitbank', 
           'countries': 'JP', 
           'version': 'v1', 
           'has': {'fetchOHLCV': True, 
           ...
           'timeframes': {'1m': '1min', 
                          ...
                          '1d': '1day', 
                          '1w': '1week'}, 
           'urls': {'logo': 'https://user-images.githubusercontent.com/1294454/37808081-b87f2d9c-2e59-11e8-894d-c1900b7584fe.jpg', 
                    'api': {'public': 'https://public.bitbank.cc', 
           ...
           'markets': {'BCH/BTC': {'id': 'bcc_btc', 'symbol': 'BCH/BTC', 'base': 'BCH', 'quote': 'BTC', 'baseId': 'bcc', 'quoteId': 'btc'}, 'BCH/JPY': {'id': 'bcc_jpy', 'symbol': 'BCH/JPY', 'base': 'BCH', 'quote': 'JPY', 'baseId': 'bcc', 'quoteId': 'jpy'}, 'MONA/BTC': {'id': 'mona_btc', 'symbol': 'MONA/BTC', 'base': 'MONA', 'quote': 'BTC', 'baseId': 'mona', 'quoteId': 'btc'}, 'MONA/JPY': {'id': 'mona_jpy', 'symbol': 'MONA/JPY', 'base': 'MONA', 'quote': 'JPY', 'baseId': 'mona', 'quoteId': 'jpy'}, 'ETH/BTC': {'id': 'eth_btc', 'symbol': 'ETH/BTC', 'base': 'ETH', 'quote': 'BTC', 'baseId': 'eth', 'quoteId': 'btc'}, 'LTC/BTC': {'id': 'ltc_btc', 'symbol': 'LTC/BTC', 'base': 'LTC', 'quote': 'BTC', 'baseId': 'ltc', 'quoteId': 'btc'}, 'XRP/JPY': {'id': 'xrp_jpy', 'symbol': 'XRP/JPY', 'base': 'XRP', 'quote': 'JPY', 'baseId': 'xrp', 'quoteId': 'jpy'}, 'BTC/JPY': {'id': 'btc_jpy', 'symbol': 'BTC/JPY', 'base': 'BTC', 'quote': 'JPY', 'baseId': 'btc', 'quoteId': 'jpy'}}, 'fees': {'trading': {'maker': 0.0, 
                                'taker': 0.0}, 
           ...
           'precision': {'price': 8, 
                         'amount': 8}, 
           'exceptions': {'20001': AuthenticationError, 
    ...
    def fetch_order_book(self, symbol, limit=None, params={}):
        self.load_markets()
        response = self.publicGetPairDepth(self.extend({'pair': self.market_id(symbol)}, params))
    ...
    def parse_trade(self, trade, market=None):
        ...
        if 'fee_amount_quote' in trade:
            fee = {'currency': market['quote'], 'cost': self.safe_float(trade, 'fee_amount_quote')}
    ...
    def fetch_trades(self, symbol, since=None, limit=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        trades = self.publicGetPairTransactions(self.extend({'pair': market['id']}, params))
    ...
    def fetch_ohlcv(self, symbol, timeframe='5m', since=None, limit=None, params={}):
        ...
        date = self.ymd(date)
        date = date.split('-')
        response = self.publicGetPairCandlestickCandleTypeYYYYMMDD(self.extend({'pair': market['id'], 
           'candle-type': self.timeframes[timeframe], 
           'YYYYMMDD': ('').join(date)}, params))
    ...
    def fetch_balance(self, params={}):
        ...
        for i in range(0, len(balances)):
            ...
            account = {'free': float(balance['free_amount']), 'used': float(balance['locked_amount']), 
    ...
    def parse_order(self, order, market=None):
        ...
        if status == 'FULLY_FILLED':
            ...
            status = 'open'
        return {'id': self.safe_string(order, 'order_id'), 'datetime': self.iso8601(timestamp), 
           'timestamp': timestamp, 
    ...
    def create_order(self, symbol, type, side, amount, price=None, params={}):
        ...
        request = {'pair': market['id'], 'amount': self.amount_to_string(symbol, amount), 
    ...
    def cancel_order(self, id, symbol=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.privatePostUserSpotCancelOrder(self.extend({'order_id': id, 
           'pair': market['id']}, params))
        return response['data']

    def fetch_order(self, id, symbol=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.privateGetUserSpotOrder(self.extend({'order_id': id, 
           'pair': market['id']}, params))
    ...
    def fetch_deposit_address(self, code, params={}):
        ...
        response = self.privateGetUserWithdrawalAccount(self.extend({'asset': currency['id']}, params))
    ...
    def withdraw(self, code, amount, address, tag=None, params={}):
        if 'uuid' not in list(params.keys()):
            raise ExchangeError(self.id + ' uuid is required for withdrawal')
        self.load_markets()
        currency = self.currency(code)
        response = self.privatePostUserRequestWithdrawal(self.extend({'asset': currency['id'], 
    ...
    def request(self, path, api='public', method='GET', params={}, headers=None, body=None):
        ...
        if not success or not data:
            errorMessages = {'10000': 'URL does not exist', '10001': 'A system error occurred. Please contact support', 
