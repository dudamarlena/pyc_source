...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/anyex/coinegg.py
...
class coinegg(Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(coinegg, self).describe(), {'id': 'coinegg', 
           'name': 'CoinEgg', 
           'countries': [
                       'CN', 'UK'], 
           'has': {'fetchOrder': True, 
                   ...
                   'fetchMyTrades': True, 
                   'fetchTickers': True}, 
           'urls': {'logo': 'https://user-images.githubusercontent.com/1294454/36770310-adfa764e-1c5a-11e8-8e09-449daac3d2fb.jpg', 
                    'api': {'web': 'https://www.coinegg.com/coin', 
           ...
           'api': {'web': {'get': [
                   ...
                   'private': {'post': [
                                      ...
                                      'trade_view/{quote}',
                                      'trade_list/{quote}']}}, 
           'fees': {'trading': {'maker': 0.1 / 100, 
                                'taker': 0.1 / 100}, 
                    'funding': {'withdraw': {'BTC': 0.008, 
                                             'BCH': 0.002, 
                                             'LTC': 0.001, 
                                             'ETH': 0.01, 
                                             'ETC': 0.01, 
                                             ...
                                             'MTC': '1%', 
                                             'KTC': 0}}}, 
           'exceptions': {'103': AuthenticationError, 
                          ...
                          '203': OrderNotFound, 
                          '402': DDoSProtection}, 
           'errorMessages': {'100': 'Required parameters can not be empty', 
    ...
    def fetch_markets(self):
        ...
        for b in range(0, len(quoteIds)):
            quoteId = quoteIds[b]
            bases = self.webGetQuoteAllcoin({'quote': quoteId})
            if bases is None:
    ...
    def fetch_ticker(self, symbol, params={}):
        ...
        ticker = self.publicGetTickerQuote(self.extend({'coin': market['baseId'], 
           'quote': market['quoteId']}, params))
    ...
    def fetch_tickers(self, symbols=None, params={}):
        ...
        for b in range(0, len(quoteIds)):
            quoteId = quoteIds[b]
            tickers = self.webGetQuoteAllcoin({'quote': quoteId})
            ...
            for i in range(0, len(baseIds)):
                ...
                if id in self.markets_by_id:
                    ...
                    symbol = market['symbol']
                    result[symbol] = self.parse_ticker({'high': ticker[4], 
                       'low': ticker[5], 
                       'buy': ticker[2], 
                       'sell': ticker[3], 
                       'last': ticker[1], 
    ...
    def fetch_order_book(self, symbol, limit=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        orderbook = self.publicGetDepthQuote(self.extend({'coin': market['baseId'], 
           'quote': market['quoteId']}, params))
    ...
    def fetch_trades(self, symbol, since=None, limit=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        trades = self.publicGetOrdersQuote(self.extend({'coin': market['baseId'], 
           'quote': market['quoteId']}, params))
    ...
    def fetch_balance(self, params={}):
        ...
        for i in range(0, len(keys)):
            ...
            if code not in list(result.keys()):
                result[code] = {'free': None, 'used': None, 
                   'total': None}
    ...
    def parse_order(self, order, market=None):
        ...
        return {'id': self.safe_string(order, 'id'), 
           'datetime': self.iso8601(timestamp), 
           'timestamp': timestamp, 
    ...
    def create_order(self, symbol, type, side, amount, price=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.privatePostTradeAddQuote(self.extend({'coin': market['baseId'], 
           'quote': market['quoteId'], 
           'type': side, 
           'amount': amount, 
           'price': price}, params))
        id = str(response['id'])
        order = self.parse_order({'id': id, 
    ...
    def cancel_order(self, id, symbol=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.privatePostTradeCancelQuote(self.extend({'id': id, 
           'coin': market['baseId'], 
           'quote': market['quoteId']}, params))
        return response
    ...
    def fetch_order(self, id, symbol=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.privatePostTradeViewQuote(self.extend({'id': id, 
           'coin': market['baseId'], 
           'quote': market['quoteId']}, params))
    ...
    def fetch_open_orders(self, symbol=None, since=None, limit=None, params={}):
        result = self.fetch_orders(symbol, since, limit, self.extend({'type': 'open'}, params))
        return result
    ...
    def sign(self, path, api='public', method='GET', params={}, headers=None, body=None):
        ...
        if api == 'public' or api == 'web':
            ...
            query = self.urlencode(self.extend({'key': self.apiKey, 
               'nonce': self.nonce()}, query))
