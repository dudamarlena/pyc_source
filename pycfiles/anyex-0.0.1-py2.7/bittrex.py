...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/anyex/bittrex.py
...
class bittrex(Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(bittrex, self).describe(), {'id': 'bittrex', 
           'name': 'Bittrex', 
           'countries': 'US', 
           'version': 'v1.1', 
           'rateLimit': 1500, 
           ...
           'has': {'CORS': True, 
                   ...
                   'fetchTickers': True, 
                   'withdraw': True}, 
           'timeframes': {'1m': 'oneMin', 
                          '5m': 'fiveMin', 
                          '30m': 'thirtyMin', 
                          '1h': 'hour', 
                          '1d': 'day'}, 
           'urls': {'logo': 'https://user-images.githubusercontent.com/1294454/27766352-cf0b3c26-5ed5-11e7-82b7-f3826b7a97d8.jpg', 
                    'api': {'public': 'https://bittrex.com/api', 
           ...
           'fees': {'trading': {'tierBased': False, 
                    ...
                    'funding': {'tierBased': False, 
                                ...
                                'deposit': {'BTC': 0, 
                                            ...
                                            'DASH': 0, 
                                            'POT': 0}}}, 
           'exceptions': {'Call to Cancel was throttled. Try again in 60 seconds.': DDoSProtection, 
    ...
    def fetch_markets(self):
        ...
        for i in range(0, len(response['result'])):
            ...
            result.append({'id': id, 
               ...
               'info': market, 
               'precision': precision, 
               'limits': {'amount': {'min': market['MinTradeSize'], 
                                     'max': None}, 
    ...
    def fetch_order_book(self, symbol, limit=None, params={}):
        self.load_markets()
        response = self.publicGetOrderbook(self.extend({'market': self.market_id(symbol), 
           'type': 'both'}, params))
        orderbook = response['result']
        if 'type' in params:
            if params['type'] == 'buy':
                orderbook = {'buy': response['result'], 'sell': []}
            elif params['type'] == 'sell':
                orderbook = {'buy': [], 'sell': response['result']}
    ...
    def fetch_ticker(self, symbol, params={}):
        ...
        response = self.publicGetMarketsummary(self.extend({'market': market['id']}, params))
    ...
    def fetch_trades(self, symbol, since=None, limit=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.publicGetMarkethistory(self.extend({'market': market['id']}, params))
    ...
    def fetch_ohlcv(self, symbol, timeframe='1m', since=None, limit=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        request = {'tickInterval': self.timeframes[timeframe], 
    ...
    def parse_order(self, order, market=None):
        ...
        if commission:
            fee = {'cost': float(order[commission])}
            if market:
    ...
    def fetch_deposit_address(self, code, params={}):
        ...
        response = self.accountGetDepositaddress(self.extend({'currency': currency['id']}, params))
    ...
    def sign(self, path, api='public', method='GET', params={}, headers=None, body=None):
        ...
        if api == 'public':
            ...
            if api == 'account' and path != 'withdraw' or path == 'openorders':
                url += method.lower()
            url += path + '?' + self.urlencode(self.extend({'nonce': nonce, 
               'apikey': self.apiKey}, params))
