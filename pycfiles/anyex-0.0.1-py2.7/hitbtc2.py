...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/anyex/hitbtc2.py
# Compiled at: 2018-04-27 06:35:48
...
class hitbtc2(hitbtc):
    ...
    def describe(self):
        return self.deep_extend(super(hitbtc2, self).describe(), {'id': 'hitbtc2', 
           'name': 'HitBTC v2', 
           'countries': 'UK', 
           'rateLimit': 1500, 
           'version': '2', 
           ...
           'timeframes': {'1m': 'M1', 
                          ...
                          '1w': 'D7', 
                          '1M': '1M'}, 
           'urls': {'logo': 'https://user-images.githubusercontent.com/1294454/27766555-8eaec20e-5edc-11e7-9c5b-6dc69fc42f5e.jpg', 
    ...
    def fetch_markets(self):
        ...
        for i in range(0, len(markets)):
            ...
            result.append(self.extend(self.fees['trading'], {'info': market, 
               'id': id, 
               'symbol': symbol, 
               'base': base, 
               'quote': quote, 
    ...
    def fetch_ticker(self, symbol, params={}):
        self.load_markets()
        market = self.market(symbol)
        ticker = self.publicGetTickerSymbol(self.extend({'symbol': market['id']}, params))
    ...
    def fetch_trades(self, symbol, since=None, limit=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.publicGetTradesSymbol(self.extend({'symbol': market['id']}, params))
    ...
    def cancel_order(self, id, symbol=None, params={}):
        self.load_markets()
        return self.privateDeleteOrderClientOrderId(self.extend({'clientOrderId': id}, params))
    ...
    def fetch_order(self, id, symbol=None, params={}):
        self.load_markets()
        response = self.privateGetHistoryOrder(self.extend({'clientOrderId': id}, params))
    ...
    def fetch_open_order(self, id, symbol=None, params={}):
        self.load_markets()
        response = self.privateGetOrderClientOrderId(self.extend({'clientOrderId': id}, params))
    ...
    def fetch_order_trades(self, id, symbol=None, since=None, limit=None, params={}):
        ...
        if symbol is not None:
            market = self.market(symbol)
        response = self.privateGetHistoryOrderIdTrades(self.extend({'id': id}, params))
        numOrders = len(response)
        if numOrders > 0:
            return self.parse_trades(response, market, since, limit)
        else:
    ...
    def create_deposit_address(self, code, params={}):
        ...
        response = self.privatePostAccountCryptoAddressCurrency({'currency': currency['id']})
    ...
    def fetch_deposit_address(self, code, params={}):
        self.load_markets()
        currency = self.currency(code)
        response = self.privateGetAccountCryptoAddressCurrency({'currency': currency['id']})
        address = response['address']
        self.check_address(address)
        tag = self.safe_string(response, 'paymentId')
        return {'currency': currency, 
