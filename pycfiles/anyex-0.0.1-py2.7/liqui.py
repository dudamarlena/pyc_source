...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/anyex/liqui.py
# Compiled at: 2018-04-27 06:35:40
...
class liqui(Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(liqui, self).describe(), {'id': 'liqui', 
           'name': 'Liqui', 
           'countries': 'UA', 
           'rateLimit': 3000, 
           'version': '3', 
           ...
           'has': {'CORS': False, 
                   ...
                   'fetchMyTrades': True, 
                   'withdraw': True}, 
           'urls': {'logo': 'https://user-images.githubusercontent.com/1294454/27982022-75aea828-63a0-11e7-9511-ca584a8edd74.jpg', 
                    'api': {'public': 'https://api.liqui.io/api', 
           ...
           'fees': {'trading': {'maker': 0.001, 
                    ...
                    'funding': {'tierBased': False, 
                                'percentage': False, 
                                'withdraw': {}, 'deposit': {}}}, 
           'commonCurrencies': {'DSH': 'DASH'}, 
           'exceptions': {'803': InvalidOrder, 
                          '804': InvalidOrder, 
                          '805': InvalidOrder, 
                          '806': InvalidOrder, 
                          '807': InvalidOrder, 
    ...
    def fetch_markets(self):
        ...
        for p in range(0, len(keys)):
            ...
            symbol = base + '/' + quote
            precision = {'amount': self.safe_integer(market, 'decimal_places'), 
               'price': self.safe_integer(market, 'decimal_places')}
            amountLimits = {'min': self.safe_float(market, 'min_amount'), 
               'max': self.safe_float(market, 'max_amount')}
            priceLimits = {'min': self.safe_float(market, 'min_price'), 
               'max': self.safe_float(market, 'max_price')}
            costLimits = {'min': self.safe_float(market, 'min_total')}
    ...
    def fetch_markets_from_cache(self):
        ...
        for i in range(0, len(markets)):
            ...
            symbol = base + '/' + quote
            precision = {'amount': self.safe_integer(market, 'AmountPoint'), 
               'price': self.safe_integer(market, 'PricePoint')}
            amountLimits = {'min': self.safe_float(market, 'MinAmount'), 
               'max': self.safe_float(market, 'MaxAmount')}
            priceLimits = {'min': self.safe_float(market, 'MinPrice'), 
               'max': self.safe_float(market, 'MaxPrice')}
            costLimits = {'min': self.safe_float(market, 'MinTotal')}
            limits = {'amount': amountLimits, 
               'price': priceLimits, 
               'cost': costLimits}
    ...
    def fetch_currencies_from_cache(self, params={}):
        ...
        for i in range(0, len(currencies)):
            ...
            result[code] = {'id': id, 
               ...
               'precision': precision, 
               'limits': {'amount': {'min': currency['DepositMinAmount'], 
    ...
    def fetch_balance_from_web(self, params={}):
        self.load_markets()
        if 'currenciesByNumericId' not in list(self.options.keys()):
    ...
    def fetch_balance(self, params={}):
        ...
        for c in range(0, len(currencies)):
            ...
            if balances['open_orders'] == 0:
                ...
                used = 0.0
            account = {'free': funds[currency], 'used': used, 
               'total': total}
    ...
    def fetch_order_books(self, symbols=None, params={}):
        ...
        if not symbols:
            ...
            ids = ('-').join(ids)
        response = self.publicGetDepthPair(self.extend({'pair': ids}, params))
        result = {}
    ...
    def fetch_tickers(self, symbols=None, params={}):
        ...
        if not symbols:
            ...
            ids = self.market_ids(symbols)
            ids = ('-').join(ids)
        tickers = self.publicGetTickerPair(self.extend({'pair': ids}, params))
        result = {}
    ...
    def fetch_order(self, id, symbol=None, params={}):
        self.load_markets()
        response = self.privatePostOrderInfo(self.extend({'order_id': int(id)}, params))
        id = str(id)
    ...
    def update_cached_orders(self, openOrders, symbol):
        ...
        for k in range(0, len(cachedOrderIds)):
            ...
            result.append(order)
            if id not in list(openOrdersIndexedById.keys()):
    ...
    def withdraw(self, currency, amount, address, tag=None, params={}):
        ...
        self.load_markets()
        response = self.privatePostWithdrawCoin(self.extend({'coinName': currency, 
           'amount': float(amount), 
    ...
    def sign(self, path, api='public', method='GET', params={}, headers=None, body=None):
        ...
        if api == 'private':
            ...
            nonce = self.nonce()
            body = self.urlencode(self.extend({'nonce': nonce, 
               'method': path}, query))
            ...
            if 'success' in response:
                ...
                if not success:
                    ...
                    if message == 'invalid api key':
                        ...
                        raise AuthenticationError(feedback)
                    elif message.find('invalid parameter') >= 0:
