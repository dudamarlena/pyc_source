...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/anyex/gateio.py
...
class gateio(Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(gateio, self).describe(), {'id': 'gateio', 
           'name': 'Gate.io', 
           'countries': 'CN', 
           'version': '2', 
           'rateLimit': 1000, 
           ...
           'urls': {'logo': 'https://user-images.githubusercontent.com/1294454/31784029-0313c702-b509-11e7-9ccc-bc0da6a0e435.jpg', 
                    'api': {'public': 'https://data.gate.io/api', 
    ...
    def fetch_markets(self):
        ...
        for i in range(0, len(markets)):
            ...
            symbol = base + '/' + quote
            precision = {'amount': details['decimal_places'], 
               'price': details['decimal_places']}
            amountLimits = {'min': details['min_amount'], 
               'max': None}
            priceLimits = {'min': None, 
               'max': None}
    ...
    def fetch_order_book(self, symbol, limit=None, params={}):
        self.load_markets()
        orderbook = self.publicGetOrderBookId(self.extend({'id': self.market_id(symbol)}, params))
    ...
    def fetch_ticker(self, symbol, params={}):
        ...
        ticker = self.publicGetTickerId(self.extend({'id': market['id']}, params))
        return self.parse_ticker(ticker, market)

    def parse_trade(self, trade, market):
        timestamp = self.parse8601(trade['date']) - 28800000
        return {'id': trade['tradeID'], 
           'info': trade, 
           'timestamp': timestamp, 
           'datetime': self.iso8601(timestamp), 
           'symbol': market['symbol'], 
    ...
    def fetch_trades(self, symbol, since=None, limit=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.publicGetTradeHistoryId(self.extend({'id': market['id']}, params))
    ...
    def query_deposit_address(self, method, currency, params={}):
        ...
        response = getattr(self, method)(self.extend({'currency': currency}, params))
        address = None
        if 'addr' in response:
    ...
    def withdraw(self, currency, amount, address, tag=None, params={}):
        ...
        self.load_markets()
        response = self.privatePostWithdraw(self.extend({'currency': currency.lower(), 
           'amount': amount, 
