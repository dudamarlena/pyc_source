...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/anyex/gdax.py
# Compiled at: 2018-04-27 06:35:51
...
class gdax(Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(gdax, self).describe(), {'id': 'gdax', 
           'name': 'GDAX', 
           'countries': 'US', 
           'rateLimit': 1000, 
           ...
           'timeframes': {'1m': 60, 
                          ...
                          '6h': 21600, 
                          '1d': 86400}, 
           'urls': {'test': 'https://api-public.sandbox.gdax.com', 
    ...
    def fetch_markets(self):
        ...
        for p in range(0, len(markets)):
            ...
            symbol = base + '/' + quote
            priceLimits = {'min': self.safe_float(market, 'quote_increment'), 
               'max': None}
            precision = {'amount': 8, 
            ...
            result.append(self.extend(self.fees['trading'], {'id': id, 
               ...
               'quote': quote, 
               'precision': precision, 
               'limits': {'amount': {'min': self.safe_float(market, 'base_min_size'), 
                                     'max': self.safe_float(market, 'base_max_size')}, 
                          'price': priceLimits, 
                          'cost': {'min': self.safe_float(market, 'min_market_funds'), 
    ...
    def fetch_order_book(self, symbol, limit=None, params={}):
        self.load_markets()
        orderbook = self.publicGetProductsIdBook(self.extend({'id': self.market_id(symbol), 
           'level': 2}, params))
    ...
    def fetch_ticker(self, symbol, params={}):
        ...
        request = self.extend({'id': market['id']}, params)
    ...
    def fetch_trades(self, symbol, since=None, limit=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.publicGetProductsIdTrades(self.extend({'id': market['id']}, params))
        return self.parse_trades(response, market, since, limit)
    ...
    def parse_order(self, order, market=None):
        ...
        fee = {'cost': self.safe_float(order, 'fill_fees'), 
           'currency': None, 
           'rate': None}
        if market:
            symbol = market['symbol']
        return {'id': order['id'], 'info': order, 
           'timestamp': timestamp, 
           'datetime': self.iso8601(timestamp), 
    ...
    def fetch_order(self, id, symbol=None, params={}):
        self.load_markets()
        response = self.privateGetOrdersId(self.extend({'id': id}, params))
    ...
    def create_order(self, symbol, type, side, amount, price=None, params={}):
        self.load_markets()
        order = {'product_id': self.market_id(symbol), 
           'side': side, 
           'size': amount, 
           'type': type}
        if type == 'limit':
    ...
    def deposit(self, currency, amount, address, params={}):
        ...
        return {'info': response, 'id': response['id']}
    ...
    def sign(self, path, api='public', method='GET', params={}, headers=None, body=None):
        ...
        if api == 'private':
            ...
            headers = {'CB-ACCESS-KEY': self.apiKey, 
