...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/anyex/bitso.py
# Compiled at: 2018-04-27 06:36:16
...
class bitso(Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(bitso, self).describe(), {'id': 'bitso', 
           'name': 'Bitso', 
           'countries': 'MX', 
           'rateLimit': 2000, 
           'version': 'v3', 
           ...
           'urls': {'logo': 'https://user-images.githubusercontent.com/1294454/27766335-715ce7aa-5ed5-11e7-88a8-173a27bb30fe.jpg', 
           ...
           'api': {'public': {'get': [
                   ...
                   'private': {'get': [
                               ...
                               'delete': [
                                        'orders/{oid}',
                                        'orders/all']}}, 
           'exceptions': {'0201': AuthenticationError, 
                          '104': InvalidNonce}})
    ...
    def fetch_markets(self):
        ...
        for i in range(0, len(markets['payload'])):
            ...
            limits = {'amount': {'min': float(market['minimum_amount']), 
                          'max': float(market['maximum_amount'])}, 
               'price': {'min': float(market['minimum_price']), 
                         'max': float(market['maximum_price'])}, 
               'cost': {'min': float(market['minimum_value']), 
                        'max': float(market['maximum_value'])}}
            precision = {'amount': self.precision_from_string(market['minimum_amount']), 
    ...
    def fetch_order_book(self, symbol, limit=None, params={}):
        self.load_markets()
        response = self.publicGetOrderBook(self.extend({'book': self.market_id(symbol)}, params))
    ...
    def fetch_ticker(self, symbol, params={}):
        self.load_markets()
        response = self.publicGetTicker(self.extend({'book': self.market_id(symbol)}, params))
    ...
    def fetch_trades(self, symbol, since=None, limit=None, params={}):
        ...
        response = self.publicGetTrades(self.extend({'book': market['id']}, params))
    ...
    def fetch_my_trades(self, symbol=None, since=None, limit=25, params={}):
        ...
        if markerInParams:
            params = self.extend(params, {'marker': int(params['marker'])})
    ...
    def create_order(self, symbol, type, side, amount, price=None, params={}):
        self.load_markets()
        order = {'book': self.market_id(symbol), 
           'side': side, 
           'type': type, 
