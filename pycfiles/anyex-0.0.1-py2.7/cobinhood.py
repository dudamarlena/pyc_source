...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/anyex/cobinhood.py
...
class cobinhood(Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(cobinhood, self).describe(), {'id': 'cobinhood', 
           'name': 'COBINHOOD', 
           'countries': 'TW', 
           'rateLimit': 1000 / 10, 
           ...
           'timeframes': {'1m': '1m', 
                          ...
                          '14d': '14D', 
                          '1M': '1M'}, 
           'urls': {'logo': 'https://user-images.githubusercontent.com/1294454/35755576-dee02e5c-0878-11e8-989f-1595d80ba47f.jpg', 
                    'api': {'web': 'https://api.cobinhood.com/v1', 
    ...
    def fetch_currencies(self, params={}):
        ...
        for i in range(0, len(currencies)):
            ...
            result[code] = {'id': id, 
               ...
               'limits': {'amount': {'min': minUnit, 
                                     'max': None}, 
                          'price': {'min': minUnit, 
                                    'max': None}, 
                          'deposit': {'min': minUnit, 
                                      'max': None}, 
                          'withdraw': {'min': minUnit, 
                                       'max': None}}, 
               'funding': {'withdraw': {'fee': float(currency['withdrawal_fee'])}, 
                           'deposit': {'fee': float(currency['deposit_fee'])}}, 
               'info': currency}
    ...
    def fetch_markets(self):
        ...
        for i in range(0, len(markets)):
            ...
            result.append({'id': id, 
               ...
               'active': active, 
               'precision': precision, 
               'limits': {'amount': {'min': float(market['base_min_size']), 
    ...
    def fetch_ticker(self, symbol, params={}):
        ...
        response = self.publicGetMarketTickersTradingPairId(self.extend({'trading_pair_id': market['id']}, params))
    ...
    def fetch_order_book(self, symbol, limit=None, params={}):
        self.load_markets()
        request = {'trading_pair_id': self.market_id(symbol)}
    ...
    def parse_trade(self, trade, market=None):
        ...
        side = trade['maker_side'] == 'sell' if 1 else 'buy'
        return {'info': trade, 
           'timestamp': timestamp, 
           'datetime': self.iso8601(timestamp), 
           'symbol': symbol, 
    ...
    def fetch_trades(self, symbol, since=None, limit=50, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.publicGetMarketTradesTradingPairId(self.extend({'trading_pair_id': market['id'], 
           'limit': limit}, params))
    ...
    def fetch_balance(self, params={}):
        ...
        for i in range(0, len(balances)):
            ...
            account = {'used': float(balance['on_order']), 'total': float(balance['total'])}
    ...
    def cancel_order(self, id, symbol=None, params={}):
        response = self.privateDeleteTradingOrdersOrderId(self.extend({'order_id': id}, params))
        return response

    def fetch_order(self, id, symbol=None, params={}):
        self.load_markets()
        response = self.privateGetTradingOrdersOrderId(self.extend({'order_id': str(id)}, params))
    ...
    def fetch_order_trades(self, id, symbol=None, params={}):
        self.load_markets()
        response = self.privateGetTradingOrdersOrderIdTrades(self.extend({'order_id': id}, params))
    ...
    def create_deposit_address(self, code, params={}):
        ...
        response = self.privatePostWalletDepositAddresses({'currency': currency['id']})
    ...
    def fetch_deposit_address(self, code, params={}):
        self.load_markets()
        currency = self.currency(code)
        response = self.privateGetWalletDepositAddresses(self.extend({'currency': currency['id']}, params))
    ...
    def withdraw(self, code, amount, address, params={}):
        self.load_markets()
        currency = self.currency(code)
        response = self.privatePostWalletWithdrawals(self.extend({'currency': currency['id'], 
           'amount': amount, 
           'address': address}, params))
        return {'id': response['result']['withdrawal_id'], 
           'info': response}
