...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/anyex/poloniex.py
# Compiled at: 2018-04-27 06:35:34
...
class poloniex(Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(poloniex, self).describe(), {'id': 'poloniex', 
           'name': 'Poloniex', 
           'countries': 'US', 
           'rateLimit': 1000, 
           ...
           'timeframes': {'5m': 300, 
                          ...
                          '4h': 14400, 
                          '1d': 86400}, 
           'urls': {'logo': 'https://user-images.githubusercontent.com/1294454/27766817-e9456312-5ee6-11e7-9b3c-b628ca5626a5.jpg', 
                    'api': {'public': 'https://poloniex.com/public', 
    ...
    def fetch_ohlcv(self, symbol, timeframe='5m', since=None, limit=None, params={}):
        ...
        if not since:
            since = 0
        request = {'currencyPair': market['id'], 'period': self.timeframes[timeframe], 
    ...
    def fetch_markets(self):
        ...
        for p in range(0, len(keys)):
            ...
            symbol = base + '/' + quote
            result.append(self.extend(self.fees['trading'], {'id': id, 
               'symbol': symbol, 
               'base': base, 
               'quote': quote, 
               'active': True, 
    ...
    def fetch_balance(self, params={}):
        self.load_markets()
        balances = self.privatePostReturnCompleteBalances(self.extend({'account': 'all'}, params))
    ...
    def parse_open_orders(self, orders, market, result=[]):
        for i in range(0, len(orders)):
            order = orders[i]
            extended = self.extend(order, {'status': 'open', 
               'type': 'limit', 
               'side': order['type'], 
               'price': order['rate']})
    ...
    def fetch_orders(self, symbol=None, since=None, limit=None, params={}):
        ...
        if symbol:
            market = self.market(symbol)
        pair = market['id'] if market else 'all'
        response = self.privatePostReturnOpenOrders(self.extend({'currencyPair': pair}))
        openOrders = []
        if market:
    ...
    def create_order(self, symbol, type, side, amount, price=None, params={}):
        ...
        price = float(price)
        amount = float(amount)
        response = getattr(self, method)(self.extend({'currencyPair': market['id'], 
           'rate': self.price_to_precision(symbol, price), 
           'amount': self.amount_to_precision(symbol, amount)}, params))
        timestamp = self.milliseconds()
        order = self.parse_order(self.extend({'timestamp': timestamp, 
           'status': 'open', 
           'type': type, 
           'side': side, 
           'price': price, 
           'amount': amount}, response), market)
        id = order['id']
        self.orders[id] = order
    ...
    def edit_order(self, id, symbol, type, side, amount, price=None, params={}):
        ...
        if id in self.orders:
            ...
            self.orders[newid] = self.extend(self.orders[id], {'id': newid, 
               'price': price, 
               'status': 'open'})
            if amount is not None:
    ...
    def cancel_order(self, id, symbol=None, params={}):
        ...
        response = None
        try:
            response = self.privatePostCancelOrder(self.extend({'orderNumber': id}, params))
    ...
    def fetch_order_status(self, id, symbol=None):
        ...
        if id in list(indexed.keys()):
            return 'open'
        return 'closed'

    def fetch_order_trades(self, id, symbol=None, params={}):
        self.load_markets()
        trades = self.privatePostReturnOrderTrades(self.extend({'orderNumber': id}, params))
        return self.parse_trades(trades)

    def create_deposit_address(self, code, params={}):
        currency = self.currency(code)
        response = self.privatePostGenerateNewAddress({'currency': currency['id']})
        address = None
