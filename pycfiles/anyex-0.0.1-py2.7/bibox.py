...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/anyex/bibox.py
# Compiled at: 2018-04-27 06:36:23
...
class bibox(Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(bibox, self).describe(), {'id': 'bibox', 
           'name': 'Bibox', 
           'countries': [
                       'CN', 'US', 'KR'], 
           'version': 'v1', 
           ...
           'timeframes': {'1m': '1min', 
                          ...
                          '1d': 'day', 
                          '1w': 'week'}, 
           'urls': {'logo': 'https://user-images.githubusercontent.com/1294454/34902611-2be8bf1a-f830-11e7-91a2-11b2f292e750.jpg', 
           ...
           'fees': {'trading': {'tierBased': False, 
                    ...
                    'funding': {'tierBased': False, 
                                'percentage': False, 
                                'withdraw': {}, 'deposit': {}}}, 
           'exceptions': {'2015': AuthenticationError, 
                          '2033': OrderNotFound, 
                          '2067': InvalidOrder, 
                          '2068': InvalidOrder, 
                          ...
                          '4003': DDoSProtection}, 
           'commonCurrencies': {'KEY': 'Bihu'}})

    def fetch_markets(self, params={}):
        response = self.publicGetMdata(self.extend({'cmd': 'marketAll'}, params))
    ...
    def fetch_ticker(self, symbol, params={}):
        ...
        response = self.publicGetMdata(self.extend({'cmd': 'ticker', 
    ...
    def parse_trade(self, trade, market=None):
        ...
        fee = None
        if 'fee' in trade:
            fee = {'cost': self.safe_float(trade, 'fee'), 'currency': None}
        return {'id': self.safe_string(trade, 'id'), 
           'info': trade, 
           'timestamp': timestamp, 
           'datetime': self.iso8601(timestamp), 
           'symbol': symbol, 
    ...
    def fetch_trades(self, symbol, since=None, limit=None, params={}):
        ...
        response = self.publicGetMdata(self.extend({'cmd': 'deals', 
           'pair': market['id'], 
           'size': size}, params))
    ...
    def fetch_ohlcv(self, symbol, timeframe='1m', since=None, limit=1000, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.publicGetMdata(self.extend({'cmd': 'kline', 
           'pair': market['id'], 
    ...
    def fetch_currencies(self, params={}):
        response = self.privatePostTransfer({'cmd': 'transfer/coinList', 
           'body': {}})
    ...
    def fetch_balance(self, params={}):
        self.load_markets()
        response = self.privatePostTransfer({'cmd': 'transfer/assets', 
           'body': self.extend({'select': 1}, params)})
    ...
    def create_order(self, symbol, type, side, amount, price=None, params={}):
        ...
        response = self.privatePostOrderpending({'cmd': 'orderpending/trade', 
           'body': self.extend({'pair': market['id'], 
                    'account_type': 0, 
    ...
    def cancel_order(self, id, symbol=None, params={}):
        response = self.privatePostOrderpending({'cmd': 'orderpending/cancelTrade', 
           'body': self.extend({'orders_id': id}, params)})
        return response
    ...
    def parse_order(self, order, market=None):
        ...
        result = {'info': order, 'id': self.safe_string(order, 'id'), 
           'timestamp': timestamp, 
           'datetime': self.iso8601(timestamp), 
    ...
    def fetch_open_orders(self, symbol=None, since=None, limit=None, params={}):
        ...
        if symbol is not None:
            ...
            pair = market['id']
        size = limit if limit else 200
        response = self.privatePostOrderpending({'cmd': 'orderpending/orderPendingList', 
           'body': self.extend({'pair': pair, 
                    'account_type': 0, 
                    'page': 1, 
                    'size': size}, params)})
    ...
    def fetch_closed_orders(self, symbol=None, since=None, limit=200, params={}):
        ...
        self.load_markets()
        market = self.market(symbol)
        response = self.privatePostOrderpending({'cmd': 'orderpending/pendingHistoryList', 
           'body': self.extend({'pair': market['id'], 
                    'account_type': 0, 
                    'page': 1, 
                    'size': limit}, params)})
    ...
    def fetch_my_trades(self, symbol=None, since=None, limit=None, params={}):
        ...
        market = self.market(symbol)
        size = limit if limit else 200
        response = self.privatePostOrderpending({'cmd': 'orderpending/orderHistoryList', 
           'body': self.extend({'pair': market['id'], 
                    'account_type': 0, 
                    'page': 1, 
                    'size': size}, params)})
    ...
    def fetch_deposit_address(self, code, params={}):
        ...
        response = self.privatePostTransfer({'cmd': 'transfer/transferIn', 
           'body': self.extend({'coin_symbol': currency['id']}, params)})
    ...
    def fetch_funding_fees(self, codes=None, params={}):
        ...
        for i in range(0, len(codes)):
            code = codes[i]
            currency = self.currency(code)
            response = self.privatePostTransfer({'cmd': 'transfer/transferOutInfo', 
               'body': self.extend({'coin_symbol': currency['id']}, params)})
            info[code] = response
            withdrawFees[code] = response['result']['withdraw_fee']

        return {'info': info, 'withdraw': withdrawFees, 
           'deposit': {}}
