...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/anyex/cex.py
...
class cex(Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(cex, self).describe(), {'id': 'cex', 
           'name': 'CEX.IO', 
           'countries': [
                       'GB', 'EU', 'CY', 'RU'], 
           'rateLimit': 1500, 
           ...
           'timeframes': {'1m': '1m'}, 
           'urls': {'logo': 'https://user-images.githubusercontent.com/1294454/27766442-8ddc33b0-5ed8-11e7-8b98-f786aef0f3c9.jpg', 
    ...
    def fetch_markets(self):
        ...
        for p in range(0, len(markets['data']['pairs'])):
            ...
            result.append({'id': id, 
               ...
               'precision': {'price': self.precision_from_string(market['minPrice']), 
                             'amount': -1 * math.log10(market['minLotSize'])}, 
               'limits': {'amount': {'min': market['minLotSize'], 
                                     'max': market['maxLotSize']}, 
                          'price': {'min': float(market['minPrice']), 
                                    'max': float(market['maxPrice'])}, 
                          'cost': {'min': market['minLotSizeS2'], 
                                   'max': None}}})
    ...
    def fetch_balance(self, params={}):
        ...
        for i in range(0, len(currencies)):
            ...
            if currency in balances:
                account = {'free': self.safe_float(balances[currency], 'available', 0.0), 'used': self.safe_float(balances[currency], 'orders', 0.0), 
                   'total': 0.0}
    ...
    def fetch_order_book(self, symbol, limit=None, params={}):
        self.load_markets()
        orderbook = self.publicGetOrderBookPair(self.extend({'pair': self.market_id(symbol)}, params))
    ...
    def fetch_tickers(self, symbols=None, params={}):
        ...
        response = self.publicGetTickersCurrencies(self.extend({'currencies': ('/').join(currencies)}, params))
    ...
    def fetch_ticker(self, symbol, params={}):
        self.load_markets()
        market = self.market(symbol)
        ticker = self.publicGetTickerPair(self.extend({'pair': market['id']}, params))
    ...
    def fetch_trades(self, symbol, since=None, limit=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.publicGetTradeHistoryPair(self.extend({'pair': market['id']}, params))
    ...
    def parse_order(self, order, market=None):
        ...
        if market:
            ...
            if baseFee in list(order.keys()) or baseTakerFee in list(order.keys()):
                ...
                fee = {'currency': market['base'], 'rate': feeRate, 
                   'cost': baseFeeCost}
                ...
                fee = {'currency': market['quote'], 'rate': feeRate, 
                   'cost': quoteFeeCost}
        if not cost:
            cost = price * filled
        return {'id': order['id'], 'datetime': self.iso8601(timestamp), 
           'timestamp': timestamp, 
    ...
    def fetch_order(self, id, symbol=None, params={}):
        self.load_markets()
        response = self.privatePostGetOrder(self.extend({'id': str(id)}, params))
