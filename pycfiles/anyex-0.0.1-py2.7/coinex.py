...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/anyex/coinex.py
# Compiled at: 2018-04-27 06:36:03
...
class coinex(Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(coinex, self).describe(), {'id': 'coinex', 
           'name': 'CoinEx', 
           'version': 'v1', 
           'countries': 'CN', 
           'rateLimit': 1000, 
           ...
           'timeframes': {'1m': '1min', 
                          ...
                          '3d': '3day', 
                          '1w': '1week'}, 
           'urls': {'logo': 'https://user-images.githubusercontent.com/1294454/38046312-0b450aac-32c8-11e8-99ab-bc6b136b6cc7.jpg', 
                    'api': {'public': 'https://api.coinex.com', 
    ...
    def fetch_markets(self):
        ...
        for i in range(0, len(keys)):
            ...
            symbol = base + '/' + quote
            precision = {'amount': market['sell_asset_type_places'], 
            ...
            result.append({'id': id, 
               ...
               'info': market, 
               'precision': precision, 
               'limits': {'amount': {'min': float(market['least_amount']), 
                                     'max': None}, 
                          'price': {'min': float(market['merge'][(numMergeLevels - 1)]), 
                                    'max': None}}})
    ...
    def fetch_ticker(self, symbol, params={}):
        ...
        response = self.publicGetMarketTicker(self.extend({'market': market['id']}, params))
    ...
    def fetch_order_book(self, symbol, params={}):
        self.load_markets()
        response = self.publicGetMarketDepth(self.extend({'market': self.market_id(symbol), 
           'merge': '0.00000001'}, params))
    ...
    def fetch_trades(self, symbol, since=None, limit=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.publicGetMarketDeals(self.extend({'market': market['id']}, params))
    ...
    def fetch_ohlcv(self, symbol, timeframe='5m', since=None, limit=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.publicGetMarketKline(self.extend({'market': market['id'], 
           'type': self.timeframes[timeframe]}, params))
    ...
    def parse_order(self, order, market=None):
        ...
        if status == 'done':
            ...
            status = 'open'
        return {'id': self.safe_string(order, 'id'), 'datetime': self.iso8601(timestamp), 
           'timestamp': timestamp, 
    ...
    def cancel_order(self, id, symbol=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.privateDeleteOrderPending(self.extend({'id': id, 
           'market': market['id']}, params))
        return self.parse_order(response['data'], market)

    def fetch_order(self, id, symbol=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.privateGetOrder(self.extend({'id': id, 
           'market': market['id']}, params))
        return self.parse_order(response['data'], market)
    ...
    def fetch_my_trades(self, symbol=None, since=None, limit=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.privateGetOrderUserDeals(self.extend({'market': market['id'], 
           'page': 1, 
           'limit': 100}, params))
    ...
    def request(self, path, api='public', method='GET', params={}, headers=None, body=None):
        ...
        if code != '0' or not data:
            responseCodes = {'24': AuthenticationError, '25': AuthenticationError, 
