...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/anyex/binance.py
# Compiled at: 2018-04-27 06:36:22
...
class binance(Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(binance, self).describe(), {'id': 'binance', 
           'name': 'Binance', 
           'countries': 'JP', 
           'rateLimit': 500, 
           ...
           'fees': {'trading': {'tierBased': False, 
                    ...
                    'funding': {'tierBased': False, 
                                ...
                                'withdraw': {'ADA': 1.0, 
                                             ...
                                             'ZRX': 5.7}, 
                                'deposit': {}}}, 
           'commonCurrencies': {'YOYO': 'YOYOW', 
                                'BCC': 'BCH', 
                                'NANO': 'XRB'}, 
           'options': {'warnOnFetchOpenOrdersWithoutSymbol': True, 
                       'recvWindow': 5000, 
                       'timeDifference': 0, 
                       'adjustForTimeDifference': False}, 
           'exceptions': {'-1013': InvalidOrder, 
                          '-1021': InvalidNonce, 
                          '-1100': InvalidOrder, 
    ...
    def fetch_markets(self):
        ...
        for i in range(0, len(markets)):
            ...
            precision = {'base': market['baseAssetPrecision'], 
            ...
            if 'PRICE_FILTER' in filters:
                ...
                entry['limits']['price'] = {'min': float(filter['minPrice']), 
            ...
            if 'LOT_SIZE' in filters:
                ...
                entry['limits']['amount'] = {'min': float(filter['minQty']), 
    ...
    def fetch_balance(self, params={}):
        ...
        for i in range(0, len(balances)):
            ...
            account = {'free': float(balance['free']), 'used': float(balance['locked']), 
               'total': 0.0}
    ...
    def fetch_ticker(self, symbol, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.publicGetTicker24hr(self.extend({'symbol': market['id']}, params))
    ...
    def parse_trade(self, trade, market=None):
        ...
        if 'commission' in trade:
            fee = {'cost': float(trade['commission']), 'currency': self.common_currency_code(trade['commissionAsset'])}
        return {'info': trade, 
           'timestamp': timestamp, 
    ...
    def parse_order_status(self, status):
        statuses = {'NEW': 'open', 
           ...
           'FILLED': 'closed', 
           'CANCELED': 'canceled'}
        if status in list(statuses.keys()):
    ...
    def create_order(self, symbol, type, side, amount, price=None, params={}):
        ...
        order = {'symbol': market['id'], 'quantity': self.amount_to_string(symbol, amount), 
           'type': type.upper(), 
           'side': side.upper()}
        if type == 'limit':
            order = self.extend(order, {'price': self.price_to_precision(symbol, price), 
               'timeInForce': 'GTC'})
    ...
    def cancel_order(self, id, symbol=None, params={}):
        ...
        self.load_markets()
        market = self.market(symbol)
        response = self.privateDeleteOrder(self.extend({'symbol': market['id'], 
           'orderId': int(id)}, params))
        return response
    ...
    def fetch_deposit_address(self, code, params={}):
        ...
        response = self.wapiGetDepositAddress(self.extend({'asset': currency['id']}, params))
    ...
    def fetch_funding_fees(self, codes=None, params={}):
        ...
        for i in range(0, len(codes)):
            code = codes[i]
            currency = self.currency(code)
            response = self.wapiGetWithdrawFee({'asset': currency['id']})
            withdrawFees[code] = self.safe_float(response, 'withdrawFee')
            info[code] = response

        return {'withdraw': withdrawFees, 'deposit': {}, 'info': info}
    ...
    def sign(self, path, api='public', method='GET', params={}, headers=None, body=None):
        ...
        if path == 'userDataStream':
            ...
            query = self.urlencode(self.extend({'timestamp': self.nonce(), 
