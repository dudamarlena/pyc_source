...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/anyex/braziliex.py
# Compiled at: 2018-04-27 06:36:12
...
class braziliex(Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(braziliex, self).describe(), {'id': 'braziliex', 
           'name': 'Braziliex', 
           'countries': 'BR', 
           'rateLimit': 1000, 
           ...
           'urls': {'logo': 'https://user-images.githubusercontent.com/1294454/34703593-c4498674-f504-11e7-8d14-ff8e44fb78c1.jpg', 
    ...
    def fetch_currencies(self, params={}):
        ...
        for i in range(0, len(ids)):
            ...
            result[code] = {'id': id, 'code': code, 
               ...
               'limits': {'amount': {'min': currency['minAmountTrade'], 
                                     'max': math.pow(10, precision)}, 
                          'price': {'min': math.pow(10, -precision), 
                                    'max': math.pow(10, precision)}, 
                          'cost': {'min': None, 
                                   'max': None}, 
                          'withdraw': {'min': currency['MinWithdrawal'], 
                                       'max': math.pow(10, precision)}, 
                          'deposit': {'min': currency['minDeposit'], 
                                      'max': None}}, 
               'info': currency}
    ...
    def fetch_ticker(self, symbol, params={}):
        ...
        ticker = self.publicGetTickerMarket(self.extend({'market': market['id']}, params))
        ticker = {'date': self.milliseconds(), 
           'ticker': ticker}
    ...
    def fetch_order_book(self, symbol, limit=None, params={}):
        self.load_markets()
        orderbook = self.publicGetOrderbookMarket(self.extend({'market': self.market_id(symbol)}, params))
    ...
    def fetch_trades(self, symbol, since=None, limit=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        trades = self.publicGetTradehistoryMarket(self.extend({'market': market['id']}, params))
    ...
    def parse_order(self, order, market=None):
        ...
        if 'info' in info:
            info = order['info']
        return {'id': order['order_number'], 'datetime': self.iso8601(timestamp), 
           'timestamp': timestamp, 
    ...
    def create_order(self, symbol, type, side, amount, price=None, params={}):
        ...
        response = getattr(self, method)(self.extend({'market': market['id'], 
           'price': price, 
        ...
        order = self.parse_order({'timestamp': self.milliseconds(), 
    ...
    def cancel_order(self, id, symbol=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        result = self.privatePostCancelOrder(self.extend({'order_number': id, 
    ...
    def fetch_open_orders(self, symbol=None, since=None, limit=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        orders = self.privatePostOpenOrders(self.extend({'market': market['id']}, params))
        return self.parse_orders(orders['order_open'], market, since, limit)

    def fetch_my_trades(self, symbol=None, since=None, limit=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        trades = self.privatePostTradeHistory(self.extend({'market': market['id']}, params))
        return self.parse_trades(trades['trade_history'], market, since, limit)

    def fetch_deposit_address(self, code, params={}):
        self.load_markets()
        currency = self.currency(code)
        response = self.privatePostDepositAddress(self.extend({'currency': currency['id']}, params))
    ...
    def sign(self, path, api='public', method='GET', params={}, headers=None, body=None):
        ...
        if api == 'public':
            ...
            query = self.extend({'command': path, 
