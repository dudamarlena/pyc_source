...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/odf/element.py
...
def _range_seq_to_re(range_seq):
    return re.compile(('[{}]').format(('').join(('{}-{}').format(re.escape(unichr(lo)), re.escape(unichr(hi))) for lo, hi in range_seq)), flags=re.UNICODE)
...
class Element(Node):
    ...
    def getAttrNS(self, namespace, localpart):
        ...
        assert (
         ...
         type(namespace) == type(''), type(''), type('')) or (
         type(namespace), type(namespace),
         type(namespace) == type(''), type(''), type(''))
        return result
    ...
    def write_open_tag(self, level, f):
        ...
        if level == 0:
            for namespace, prefix in self.namespaces.items():
                f.write(' xmlns:' + prefix + '="' + _sanitize(str(namespace)) + '"')

        for qname in self.attributes.keys():
            prefix = self.get_nsprefix(qname[0])
            f.write(' ' + _sanitize(str(prefix + ':' + qname[1])) + '=' + _quoteattr(unicode(self.attributes[qname])))
    ...
    def toXml(self, level, f):
        ...
        for qname in self.attributes.keys():
            prefix = self.get_nsprefix(qname[0])
            f.write(' ' + _sanitize(unicode(prefix + ':' + qname[1])) + '=' + _quoteattr(unicode(self.attributes[qname])))
