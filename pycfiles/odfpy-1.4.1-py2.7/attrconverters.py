...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/odf/attrconverters.py
# Compiled at: 2020-01-18 11:47:38
import sys, os.path
sys.path.append(os.path.dirname(__file__))
from odf.namespaces import *
import re, types
pattern_color = re.compile('#[0-9a-fA-F]{6}')
pattern_vector3D = re.compile('\\([ ]*-?([0-9]+(\\.[0-9]*)?|\\.[0-9]+)([ ]+-?([0-9]+(\\.[0-9]*)?|\\.[0-9]+)){2}[ ]*\\)')
...
def cnv_color(attribute, arg, element):
    u""" A RGB color in conformance with §5.9.11 of [XSL], that is a RGB color in notation “#rrggbb”, where
...
def cnv_configtype(attribute, arg, element):
    if str(arg) not in ('boolean', 'short', 'int', 'long', 'double', 'string', 'datetime',
                        'base64Binary'):
...
def cnv_family(attribute, arg, element):
    """ A style family """
    if str(arg) not in ('text', 'paragraph', 'section', 'ruby', 'table', 'table-column',
...
def cnv_formula(attribute, arg, element):
    u""" A string containing a formula. Formulas do not have a predefined syntax, but the string should
...
pattern_language = re.compile('[a-zA-Z]{1,8}(-[a-zA-Z0-9]{1,8})*')
...
def cnv_legend_position(attribute, arg, element):
    if str(arg) not in ('start', 'end', 'top', 'bottom', 'top-start', 'bottom-start',
                        'top-end', 'bottom-end'):
        raise ValueError("'%s' not allowed" % str(arg))
    return str(arg)


pattern_length = re.compile('-?([0-9]+(\\.[0-9]*)?|\\.[0-9]+)((cm)|(mm)|(in)|(pt)|(pc)|(px))')

def cnv_length(attribute, arg, element):
    u""" A (positive or negative) physical length, consisting of magnitude and unit, in conformance with the
...
pattern_namespacedToken = re.compile('[0-9a-zA-Z_]+:[0-9a-zA-Z._\\-]+')
...
pattern_percent = re.compile('-?([0-9]+(\\.[0-9]*)?|\\.[0-9]+)%')
...
pattern_points = re.compile('-?[0-9]+,-?[0-9]+([ ]+-?[0-9]+,-?[0-9]+)*')
...
pattern_viewbox = re.compile('-?[0-9]+([ ]+-?[0-9]+){3}$')
...
attrconverters = {((ANIMNS, 'audio-level'), None): cnv_double, 
   ...
   ((CHARTNS, 'angle-offset'), None): cnv_angle, 
   ((CHARTNS, 'automatic-content'), None): cnv_boolean, 
   ((CHARTNS, 'auto-position'), None): cnv_boolean, 
   ((CHARTNS, 'auto-size'), None): cnv_boolean, 
   ((CHARTNS, 'axis-label-position'), None): cnv_string, 
   ((CHARTNS, 'axis-position'), None): cnv_string, 
   ...
   ((CHARTNS, 'display-equation'), None): cnv_boolean, 
   ((CHARTNS, 'display-label'), None): cnv_boolean, 
   ((CHARTNS, 'display-r-square'), None): cnv_boolean, 
   ...
   ((CHARTNS, 'error-lower-range'), None): cnv_string, 
   ((CHARTNS, 'error-upper-indicator'), None): cnv_boolean, 
   ((CHARTNS, 'error-upper-limit'), None): cnv_string, 
   ((CHARTNS, 'error-upper-range'), None): cnv_string, 
   ((CHARTNS, 'gap-width'), None): cnv_string, 
   ((CHARTNS, 'group-bars-per-axis'), None): cnv_boolean, 
   ((CHARTNS, 'hole-size'), None): cnv_percent, 
   ((CHARTNS, 'include-hidden-cells'), None): cnv_boolean, 
   ...
   ((CHARTNS, 'label-position'), None): cnv_string, 
   ((CHARTNS, 'label-position-negative'), None): cnv_string, 
   ...
   ((CHARTNS, 'reverse-direction'), None): cnv_boolean, 
   ((CHARTNS, 'right-angled-axes'), None): cnv_boolean, 
   ...
   ((CHARTNS, 'sort-by-x-values'), None): cnv_boolean, 
   ...
   ((CHARTNS, 'tick-mark-position'), None): cnv_string, 
   ...
   ((CHARTNS, 'treat-empty-cells'), None): cnv_string, 
   ...
   ((DRAWNS, 'draw-aspect'), None): cnv_draw_aspect, 
   ((DRAWNS, 'end-angle'), None): cnv_angle, 
   ...
   ((DRAWNS, 'fit-to-size'), None): cnv_string, 
   ...
   ((DRAWNS, 'start-angle'), None): cnv_angle, 
   ...
   ((FORMNS, 'linked-cell'), None): cnv_string, 
   ((FORMNS, 'list-linkage-type'), None): cnv_list_linkage_type, 
   ...
   ((FORMNS, 'repeat'), None): cnv_boolean, 
   ((FORMNS, 'selected'), None): cnv_boolean, 
   ((FORMNS, 'size'), None): cnv_nonNegativeInteger, 
   ((FORMNS, 'source-cell-range'), None): cnv_string, 
   ((FORMNS, 'spin-button'), None): cnv_boolean, 
   ...
   ((GRDDLNS, 'transformation'), None): cnv_string, 
   ((LOEXTNS, 'contextual-spacing'), None): cnv_boolean, 
   ((LOEXTNS, 'scale-to-X'), None): cnv_string, 
   ((LOEXTNS, 'scale-to-Y'), None): cnv_string, 
   ...
   ((MANIFESTNS, 'preferred-view-mode'), None): cnv_string, 
   ((MANIFESTNS, 'salt'), None): cnv_string, 
   ((MANIFESTNS, 'size'), None): cnv_nonNegativeInteger, 
   ((MANIFESTNS, 'version'), None): cnv_string, 
   ...
   ((NUMBERNS, 'rfc-language-tag'), None): cnv_language, 
   ((NUMBERNS, 'script'), None): cnv_token, 
   ...
   ((STYLENS, 'join-border'), None): cnv_boolean, 
   ...
   ((STYLENS, 'layout-grid-base-width'), None): cnv_length, 
   ...
   ((STYLENS, 'layout-grid-snap-to'), None): cnv_boolean, 
   ((STYLENS, 'layout-grid-standard-mode'), None): cnv_boolean, 
   ...
   ((STYLENS, 'list-level'), None): cnv_positiveInteger, 
   ...
   ((STYLENS, 'percentage-data-style-name'), None): cnv_StyleNameRef, 
   ...
   ((STYLENS, 'protect'), (STYLENS, 'section-properties')): cnv_boolean, 
   ((STYLENS, 'protect'), (STYLENS, 'graphic-properties')): cnv_string, 
   ...
   ((STYLENS, 'rfc-language-tag'), None): cnv_language, 
   ((STYLENS, 'rfc-language-tag-asian'), None): cnv_language, 
   ((STYLENS, 'rfc-language-tag-complex'), None): cnv_language, 
   ...
   ((STYLENS, 'script-asian'), None): cnv_string, 
   ((STYLENS, 'script-complex'), None): cnv_string, 
   ...
   ((STYLENS, 'text-overline-color'), None): cnv_string, 
   ((STYLENS, 'text-overline-mode'), None): cnv_string, 
   ((STYLENS, 'text-overline-style'), None): cnv_string, 
   ((STYLENS, 'text-overline-type'), None): cnv_string, 
   ((STYLENS, 'text-overline-width'), None): cnv_string, 
   ...
   ((SVGNS, 'stroke-linecap'), None): cnv_stroke_linecap, 
   ...
   ((TABLENS, 'embedded-number-behavior'), None): cnv_string, 
   ...
   ((TABLENS, 'first-row-end-column'), None): cnv_rowOrCol, 
   ((TABLENS, 'first-row-start-column'), None): cnv_rowOrCol, 
   ...
   ((TABLENS, 'last-row-end-column'), None): cnv_rowOrCol, 
   ((TABLENS, 'last-row-spanned'), None): cnv_positiveInteger, 
   ((TABLENS, 'last-row-start-column'), None): cnv_rowOrCol, 
   ...
   ((TABLENS, 'paragraph-style-name'), None): cnv_StyleNameRef, 
   ...
   ((TABLENS, 'protection-key-digest-algorithm'), None): cnv_anyURI, 
   ((TABLENS, 'query-name'), None): cnv_string, 
   ((TABLENS, 'range-usable-as'), None): cnv_string, 
   ((TABLENS, 'rfc-language-tag'), None): cnv_language, 
   ...
   ((TABLENS, 'script'), None): cnv_string, 
   ...
   ((TABLENS, 'target-range-address'), None): cnv_string, 
   ((TABLENS, 'template-name'), None): cnv_string, 
   ((TABLENS, 'title'), None): cnv_string, 
   ((TABLENS, 'track-changes'), None): cnv_boolean, 
   ((TABLENS, 'type'), None): cnv_string, 
   ((TABLENS, 'use-banding-columns-styles'), None): cnv_boolean, 
   ((TABLENS, 'use-banding-rows-styles'), None): cnv_boolean, 
   ((TABLENS, 'use-first-column-styles'), None): cnv_boolean, 
   ((TABLENS, 'use-first-row-styles'), None): cnv_boolean, 
   ((TABLENS, 'use-labels'), None): cnv_string, 
   ((TABLENS, 'use-last-column-styles'), None): cnv_boolean, 
   ((TABLENS, 'use-last-row-styles'), None): cnv_boolean, 
   ((TABLENS, 'use-regular-expressions'), None): cnv_boolean, 
   ((TABLENS, 'use-wildcards'), None): cnv_boolean, 
   ...
   ((TEXTNS, 'continue-list'), None): cnv_IDREF, 
   ...
   ((TEXTNS, 'label-followed-by'), None): cnv_string, 
   ((TEXTNS, 'level'), None): cnv_positiveInteger, 
   ((TEXTNS, 'line-break'), None): cnv_boolean, 
   ((TEXTNS, 'line-number'), None): cnv_string, 
   ((TEXTNS, 'list-id'), None): cnv_NCName, 
   ((TEXTNS, 'list-level-position-and-space-mode'), None): cnv_string, 
   ((TEXTNS, 'list-tab-stop-position'), None): cnv_length, 
   ...
   ((TEXTNS, 'protection-key-digest-algorithm'), None): cnv_anyURI, 
   ...
   ((TEXTNS, 'style-override'), None): cnv_StyleNameRef, 
   ...
   ((XHTMLNS, 'about'), None): cnv_anyURI, 
   ((XHTMLNS, 'content'), None): cnv_string, 
   ((XHTMLNS, 'datatype'), None): cnv_anyURI, 
   ((XHTMLNS, 'property'), None): cnv_anyURI, 
   ((XLINKNS, 'actuate'), None): cnv_string, 
   ((XLINKNS, 'href'), None): cnv_anyURI, 
   ((XLINKNS, 'show'), None): cnv_xlinkshow, 
   ((XLINKNS, 'title'), None): cnv_string, 
   ((XLINKNS, 'type'), None): cnv_xlinktype, 
   ((XMLNS, 'id'), None): cnv_NCName}
