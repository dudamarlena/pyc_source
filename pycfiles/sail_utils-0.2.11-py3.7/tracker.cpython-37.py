...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\sail_utils\cv\head\deep_sort\tracker.py
# Compiled at: 2020-04-22 06:53:59
...
from sail_utils.cv.head.deep_sort.config import MAX_AGE, MAX_IOU_DISTANCE, N_INIT
...
def _convert_detection_struct(from_struct: list) -> list:
    ...
    for each in from_struct:
        ...
        to_struct.append(Detection(np.array([xmin, ymin, xmax - xmin, ymax - ymin], dtype=(np.float64)), confidence, np.zeros(5, dtype=(np.float64)) + 1.0))
...
class Tracker:
    ...
    def __init__(self, metric, max_iou_distance=MAX_IOU_DISTANCE, max_age=MAX_AGE, n_init=N_INIT):
        self.metric = metric
    ...
    def result(self, time_stamp):
        ...
        for track in self.tracks:
            if track.is_confirmed():
                ...
                track_result.append(dict(id=track_id,
                  location=[
    ...
    def _match(self, detections):
        ...
        iou_track_candidates = unconfirmed_tracks + [k for k in unmatched_tracks_a if self.tracks[k].time_since_update <= self.max_age]
        unmatched_tracks_a = [k for k in unmatched_tracks_a if self.tracks[k].time_since_update > self.max_age]
