...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/boto/mashups/server.py
# Compiled at: 2015-11-24 05:02:18
# Size of source mod 2**32: 14068 bytes
"""
...
class Server(Model):
    ...
    hostname = property(getHostname, setReadOnly, None, 'The public DNS name of the server')

    def getPrivateHostname(self):
        if self.instance:
            return self.instance.private_dns_name

    private_hostname = property(getPrivateHostname, setReadOnly, None, 'The private DNS name of the server')

    def getLaunchTime(self):
        if self.instance:
            return self.instance.launch_time

    launch_time = property(getLaunchTime, setReadOnly, None, 'The time the Server was started')

    def getConsoleOutput(self):
        if self.instance:
            return self.instance.get_console_output()

    console_output = property(getConsoleOutput, setReadOnly, None, 'Retrieve the console output for server')
    ...
    groups = property(getGroups, setReadOnly, None, 'The Security Groups controlling access to this server')
    ...
    config = property(getConfig, setConfig, None, 'The instance data for this server')
    ...
    def start(self):
        ...
        cfg = cfg.getvalue()
        r = ami.run(min_count=1, max_count=1, key_name=self.key_name, security_groups=groups, instance_type=self.instance_type, placement=self.zone, user_data=cfg)
        i = r.instances[0]
