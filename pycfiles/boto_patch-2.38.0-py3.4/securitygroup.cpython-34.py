...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/boto/ec2/securitygroup.py
# Compiled at: 2015-11-24 05:02:18
# Size of source mod 2**32: 14687 bytes
"""
...
class SecurityGroup(TaggedEC2Object):
                                ...
                                if name == 'return':
                                    if value == 'false':
                                        ...
                                        if value == 'true':
                                            ...
                                            raise Exception('Unexpected value of status %s for group %s' % (
                                             value,
                                             self.name))
                                else:
    ...
    def delete(self, dry_run=False):
        if self.vpc_id:
            return self.connection.delete_security_group(group_id=self.id, dry_run=dry_run)
        else:
    ...
    def add_rule(self, ip_protocol, from_port, to_port, src_group_name, src_group_owner_id, cidr_ip, src_group_group_id, dry_run=False):
        ...
        rule.add_grant(src_group_name, src_group_owner_id, cidr_ip, src_group_group_id, dry_run=dry_run)
    ...
    def authorize(self, ip_protocol=None, from_port=None, to_port=None, cidr_ip=None, src_group=None, dry_run=False):
        ...
        status = self.connection.authorize_security_group(group_name, src_group_name, src_group_owner_id, ip_protocol, from_port, to_port, cidr_ip, group_id, src_group_group_id, dry_run=dry_run)
        if status:
            if not isinstance(cidr_ip, list):
                cidr_ip = [
                 cidr_ip]
            for single_cidr_ip in cidr_ip:
                self.add_rule(ip_protocol, from_port, to_port, src_group_name, src_group_owner_id, single_cidr_ip, src_group_group_id, dry_run=dry_run)

        return status

    def revoke(self, ip_protocol=None, from_port=None, to_port=None, cidr_ip=None, src_group=None, dry_run=False):
        group_name = None
        if not self.vpc_id:
            group_name = self.name
        group_id = None
        ...
        if src_group:
            ...
            if not self.vpc_id:
                ...
                if hasattr(src_group, 'group_id'):
                    ...
                    src_group_group_id = src_group.id
        status = self.connection.revoke_security_group(group_name, src_group_name, src_group_owner_id, ip_protocol, from_port, to_port, cidr_ip, group_id, src_group_group_id, dry_run=dry_run)
        if status:
            self.remove_rule(ip_protocol, from_port, to_port, src_group_name, src_group_owner_id, cidr_ip, src_group_group_id, dry_run=dry_run)
        return status
    ...
    def copy_to_region(self, region, name=None, dry_run=False):
        ...
        sg = rconn.create_security_group(name or self.name, self.description, dry_run=dry_run)
        source_groups = []
        for rule in self.rules:
            for grant in rule.grants:
