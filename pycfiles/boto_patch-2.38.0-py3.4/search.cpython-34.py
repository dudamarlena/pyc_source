...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/boto/cloudsearch2/search.py
...
class Query(object):
    ...
    def __init__(self, q=None, parser=None, fq=None, expr=None, return_fields=None, size=10, start=0, sort=None, facet=None, highlight=None, partial=None, options=None):
        self.q = q
        self.parser = parser
        self.fq = fq
        self.expr = expr or {}
...
class SearchConnection(object):
    ...
    def build_query(self, q=None, parser=None, fq=None, rank=None, return_fields=None, size=10, start=0, facet=None, highlight=None, sort=None, partial=None, options=None):
        ...
        if self.sign_request:
            ...
            if 'messages' in data and 'error' in data:
                for m in data['messages']:
                    if m['severity'] == 'fatal':
                        raise SearchServiceException('Error processing search %s => %s' % (
                         params, m['message']), query)
                        continue

            elif 'error' in data:
                raise SearchServiceException('Unknown error processing search %s' % json.dumps(data), query)
        data['query'] = query
