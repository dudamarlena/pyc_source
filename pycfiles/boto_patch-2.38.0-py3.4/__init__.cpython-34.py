...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/boto/__init__.py
# Compiled at: 2015-11-24 05:02:18
# Size of source mod 2**32: 41904 bytes
from boto.pyami.config import Config, BotoConfigLocations
from boto.storage_uri import BucketStorageUri, FileStorageUri
import boto.plugin, datetime, os, platform, re, sys, logging, logging.config
...
UserAgent = 'Boto/%s Python/%s %s/%s' % (
 ...
 platform.release())
config = Config()
BUCKET_NAME_RE = re.compile('^[a-zA-Z0-9][a-zA-Z0-9\\._-]{1,253}[a-zA-Z0-9]$')
TOO_LONG_DNS_NAME_COMP = re.compile('[-_a-z0-9]{64}')
GENERATION_RE = re.compile('(?P<versionless_uri_str>.+)#(?P<generation>[0-9]+)$')
...
def storage_uri(uri_str, default_scheme='file', debug=0, validate=True, bucket_storage_uri_class=BucketStorageUri, suppress_consec_slashes=True, is_latest=False):
    ...
    * filename (which could be a Unix path like /a/b/c or a Windows path like
    ...
    if scheme == 'file':
        ...
        object_name = ''
        if validate:
            if bucket_name and (not BUCKET_NAME_RE.match(bucket_name) or TOO_LONG_DNS_NAME_COMP.search(bucket_name)):
