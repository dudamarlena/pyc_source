...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/boto/sdb/domain.py
# Compiled at: 2015-11-24 05:02:18
# Size of source mod 2**32: 14351 bytes
...
class DomainMetaData(object):
    ...
    def endElement(self, name, value, connection):
        if name == 'ItemCount':
            ...
            if name == 'ItemNamesSizeBytes':
                self.item_names_size = int(value)
            else:
                if name == 'AttributeNameCount':
                    self.attr_name_count = int(value)
                else:
                    if name == 'AttributeNamesSizeBytes':
                        self.attr_names_size = int(value)
                    else:
                        if name == 'AttributeValueCount':
                            self.attr_value_count = int(value)
                        else:
                            if name == 'AttributeValuesSizeBytes':
