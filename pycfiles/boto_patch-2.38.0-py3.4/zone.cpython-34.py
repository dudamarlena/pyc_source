...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/boto/route53/zone.py
# Compiled at: 2015-11-24 05:02:18
# Size of source mod 2**32: 16231 bytes
default_ttl = 60
import copy
...
class Zone(object):
    ...
    def _new_record(self, changes, resource_type, name, value, ttl, identifier, comment=''):
        ...
        change = changes.add_change('CREATE', name, resource_type, ttl, identifier=identifier, weight=weight, region=region)
    ...
    def update_record(self, old_record, new_value, new_ttl=None, new_identifier=None, comment=''):
        ...
        self._new_record(changes, record.type, record.name, new_value, new_ttl, new_identifier, comment)
        return Status(self.route53connection, self._commit(changes))
    ...
    def add_cname(self, name, value, ttl=None, identifier=None, comment=''):
        ...
        return self.add_record(resource_type='CNAME', name=name, value=value, ttl=ttl, identifier=identifier, comment=comment)
    ...
    def find_records(self, name, type, desired=1, all=False, identifier=None):
        ...
        if weight is not None:
            results = [r for r in results if r.weight == weight and r.identifier == identifier[0]]
        if region is not None:
            results = [r for r in results if r.region == region and r.identifier == identifier[0]]
        if not all and len(results) > desired:
            message = 'Search: name %s type %s' % (name, type)
            message += '\nFound: '
            message += ', '.join(['%s %s %s' % (r.name, r.type, r.to_print()) for r in results])
