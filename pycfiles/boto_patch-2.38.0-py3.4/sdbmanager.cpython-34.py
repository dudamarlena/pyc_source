...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/boto/sdb/db/manager/sdbmanager.py
...
class SDBConverter(object):
    ...
    def __init__(self, manager):
        ...
        self.type_map = {bool: (self.encode_bool, self.decode_bool),  int: (
               self.encode_int, self.decode_int), 
         float: (
                 self.encode_float, self.decode_float), 
         self.model_class: (
         ...
         date: (
                self.encode_date, self.decode_date), 
         time: (
                self.encode_time, self.decode_time), 
         Blob: (
                self.encode_blob, self.decode_blob), 
         str: (
               self.encode_string, self.decode_string)}
        if six.PY2:
            self.type_map[long] = (
             self.encode_long, self.decode_long)
    ...
    def encode_prop(self, prop, value):
        if isinstance(prop, ListProperty):
            ...
            if isinstance(prop, MapProperty):
...
class SDBManager(object):
    ...
    def save_object(self, obj, expected_value=None):
        ...
        attrs = {'__type__': obj.__class__.__name__,  '__module__': obj.__class__.__module__, 
