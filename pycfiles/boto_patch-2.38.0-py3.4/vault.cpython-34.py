...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/boto/glacier/vault.py
# Compiled at: 2015-11-24 05:02:18
# Size of source mod 2**32: 17601 bytes
import codecs
...
MAXIMUM_ARCHIVE_SIZE = 40000 * _GIGABYTE
MAXIMUM_NUMBER_OF_PARTS = 10000

class Vault(object):
    DefaultPartSize = DEFAULT_PART_SIZE
    SingleOperationThreshold = 100 * _MEGABYTE
    ResponseDataElements = (('VaultName', 'name', None), ('VaultARN', 'arn', None),
                            ('CreationDate', 'creation_date', None), ('LastInventoryDate', 'last_inventory_date', None),
                            ('SizeInBytes', 'size', 0), ('NumberOfArchives', 'number_of_archives', 0))
    ...
    def create_archive_from_file(self, filename=None, file_obj=None, description=None, upload_id_callback=None):
        ...
        if not file_obj:
            ...
            try:
                ...
                raise UploadArchiveError('File size of %s bytes exceeds 40,000 GB archive limit of Glacier.')

            file_obj = open(filename, 'rb')
        writer = self.create_archive_writer(description=description, part_size=part_size)
    ...
    def resume_archive_from_file(self, upload_id, filename=None, file_obj=None):
        ...
        for part_desc in part_list_response['Parts']:
            part_index = self._range_string_to_part_index(part_desc['RangeInBytes'], part_size)
