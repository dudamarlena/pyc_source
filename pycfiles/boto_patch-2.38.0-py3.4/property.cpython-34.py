...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/boto/sdb/db/property.py
# Compiled at: 2015-11-24 05:02:18
# Size of source mod 2**32: 25086 bytes
import datetime
from boto.sdb.db.key import Key
from boto.utils import Password
from boto.sdb.db.query import Query
import re, boto, boto.s3.key
...
class Property(object):
    ...
    def __set__(self, obj, value):
        ...
        try:
            if obj._loaded:
                if hasattr(obj, 'on_set_%s' % self.name):
...
class TextProperty(Property):
    ...
    def __init__(self, verbose_name=None, name=None, default='', required=False, validator=None, choices=None, unique=False, max_length=None):
        super(TextProperty, self).__init__(verbose_name, name, default, required, validator, choices, unique)
...
class ReferenceProperty(Property):
    ...
    def __set__(self, obj, value):
        ...
        if value is not None:
            if obj.id == value or hasattr(value, 'id') and obj.id == value.id:
...
class _ReverseReferenceProperty(Property):
    ...
    def __init__(self, model, prop, name):
        self._ReverseReferenceProperty__model = model
        self._ReverseReferenceProperty__property = prop
    ...
    def __get__(self, model_instance, model_class):
        ...
        if model_instance is not None:
            query = Query(self._ReverseReferenceProperty__model)
            if isinstance(self._ReverseReferenceProperty__property, list):
                props = []
                for prop in self._ReverseReferenceProperty__property:
                    props.append('%s =' % prop)

                return query.filter(props, model_instance)
            else:
                return query.filter(self._ReverseReferenceProperty__property + ' =', model_instance)
        else:
            return self
...
class ListProperty(Property):
    ...
    def validate(self, value):
        ...
        if self.item_type in six.integer_types:
            ...
            if self.item_type in six.string_types:
