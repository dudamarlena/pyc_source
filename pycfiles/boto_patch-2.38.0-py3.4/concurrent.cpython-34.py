...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/boto/glacier/concurrent.py
# Compiled at: 2015-11-24 05:02:18
# Size of source mod 2**32: 17241 bytes
import os, math, threading, hashlib, time, logging
from boto.compat import Queue
import binascii
from boto.glacier.utils import DEFAULT_PART_SIZE, minimum_part_size, chunk_hashes, tree_hash, bytes_to_hex
from boto.glacier.exceptions import UploadArchiveError, DownloadArchiveError, TreeHashDoesNotMatchError
...
class ConcurrentDownloader(ConcurrentTransferer):
    ...
    def _wait_for_download_threads(self, filename, result_queue, total_parts):
        ...
        if self._job.sha256_treehash != final_hash:
            self._shutdown_threads()
            raise TreeHashDoesNotMatchError('Tree hash for entire archive does not match, expected: %s, got: %s' % (
...
class DownloadWorkerThread(TransferThread):
    ...
    def _download_chunk(self, work):
        ...
        if response['TreeHash'] != actual_hash:
            raise TreeHashDoesNotMatchError('Tree hash for part number %s does not match, expected: %s, got: %s' % (
             part_number, response['TreeHash'],
             actual_hash))
        return (
