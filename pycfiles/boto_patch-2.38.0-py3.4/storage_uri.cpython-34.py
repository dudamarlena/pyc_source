...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/boto/storage_uri.py
# Compiled at: 2015-11-24 05:02:18
# Size of source mod 2**32: 39204 bytes
import boto, os, sys, textwrap
...
class StorageUri(object):
    ...
    def check_response(self, resp, level, uri):
        if resp is None:
            raise InvalidUriError('\n'.join(textwrap.wrap('Attempt to get %s for "%s" failed. This can happen if the URI refers to a non-existent object or if you meant to operate on a directory (e.g., leaving off -R option on gsutil cp, mv, or ls of a bucket)' % (
             level, uri), 80)))

    def _check_bucket_uri(self, function_name):
        if issubclass(type(self), BucketStorageUri):
    ...
    def connect(self, access_key_id=None, secret_access_key=None, **kwargs):
        ...
        if hasattr(self, 'suppress_consec_slashes'):
            if 'suppress_consec_slashes' not in connection_args:
...
class BucketStorageUri(StorageUri):
    ...
    def delete_key(self, validate=False, headers=None, version_id=None, mfa_token=None):
        ...
        if self.get_provider().name == 'aws':
            ...
            return bucket.delete_key(self.object_name, headers, version_id, mfa_token)
        if self.get_provider().name == 'google':
