...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/boto/ec2/image.py
...
class Image(TaggedEC2Object):
                        ...
                        if name == 'isPublic':
                            if value == 'false':
                                ...
                                if value == 'true':
                                    ...
                                    raise Exception('Unexpected value of isPublic %s for image %s' % (
                                     value,
                                     self.id))
                        else:
                                                                ...
                                                                if name == 'rootDeviceName':
                                                                    ...
                                                                    if name == 'virtualizationType':
                                                                        ...
                                                                        if name == 'hypervisor':
                                                                            ...
                                                                            if name == 'instanceLifecycle':
                                                                                self.instance_lifecycle = value
                                                                            else:
                                                                                if name == 'sriovNetSupport':
    ...
    def deregister(self, delete_snapshot=False, dry_run=False):
        return self.connection.deregister_image(self.id, delete_snapshot, dry_run=dry_run)

    def get_launch_permissions(self, dry_run=False):
        img_attrs = self.connection.get_image_attribute(self.id, 'launchPermission', dry_run=dry_run)
    ...
    def remove_launch_permissions(self, user_ids=None, group_names=None, dry_run=False):
        return self.connection.modify_image_attribute(self.id, 'launchPermission', 'remove', user_ids, group_names, dry_run=dry_run)
