...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/boto/rds/dbinstance.py
...
class DBInstance(object):
    ...
    def startElement(self, name, attrs, connection):
        if name == 'Endpoint':
            ...
            self.parameter_groups = ResultSet([
             ('DBParameterGroup',
              ParameterGroup)])
            return self.parameter_groups
        if name == 'DBSecurityGroups':
            self.security_groups = ResultSet([
             ('DBSecurityGroup',
              DBSecurityGroup)])
            return self.security_groups
        if name == 'VpcSecurityGroups':
        ...
        if name == 'ReadReplicaDBInstanceIdentifiers':
                    ...
                    if name == 'Engine':
                        ...
                        if name == 'DBInstanceStatus':
                            ...
                            if name == 'AllocatedStorage':
                                ...
                                if name == 'AutoMinorVersionUpgrade':
                                    self.auto_minor_version_upgrade = value.lower() == 'true'
                                else:
                                    if name == 'DBInstanceClass':
                                                ...
                                                if name == 'Address':
                                                    ...
                                                    if name == 'Endpoint':
                                                        ...
                                                        if name == 'AvailabilityZone':
                                                            ...
                                                            if name == 'BackupRetentionPeriod':
                                                                self.backup_retention_period = int(value)
                                                            else:
                                                                if name == 'LatestRestorableTime':
                                                                    self.latest_restorable_time = value
                                                                else:
                                                                    if name == 'PreferredMaintenanceWindow':
                                                                        self.preferred_maintenance_window = value
                                                                    else:
                                                                        if name == 'PreferredBackupWindow':
