...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/boto/dynamodb2/table.py
# Compiled at: 2015-11-24 05:02:18
# Size of source mod 2**32: 61591 bytes
import boto
from boto.dynamodb2 import exceptions
from boto.dynamodb2.fields import HashKey, RangeKey, AllIndex, KeysOnlyIndex, IncludeIndex, GlobalAllIndex, GlobalKeysOnlyIndex, GlobalIncludeIndex
from boto.dynamodb2.items import Item
from boto.dynamodb2.layer1 import DynamoDBConnection
from boto.dynamodb2.results import ResultSet, BatchGetResultSet
from boto.dynamodb2.types import NonBooleanDynamizer, Dynamizer, FILTER_OPERATORS, QUERY_OPERATORS, STRING
...
class Table(object):
    ...
    max_batch_get = 100
    _PROJECTION_TYPE_TO_INDEX = dict(global_indexes=dict(ALL=GlobalAllIndex, KEYS_ONLY=GlobalKeysOnlyIndex, INCLUDE=GlobalIncludeIndex), local_indexes=dict(ALL=AllIndex, KEYS_ONLY=KeysOnlyIndex, INCLUDE=IncludeIndex))
    ...
    @classmethod
    def create(cls, table_name, schema, throughput=None, indexes=None, global_indexes=None, connection=None):
        """
        ...
        **IMPORTANT** - If the table already exists in DynamoDB, additional
        ...
        raw_throughput = {'ReadCapacityUnits': int(table.throughput['read']), 
         'WriteCapacityUnits': int(table.throughput['write'])}
        kwargs = {}
        kwarg_map = {'indexes': 'local_secondary_indexes', 
        ...
        table.connection.create_table(table_name=table.table_name, attribute_definitions=attr_defs, key_schema=raw_schema, provisioned_throughput=raw_throughput, **kwargs)
        return table
    ...
    def _introspect_all_indexes(self, raw_indexes, map_indexes_projection):
        ...
        for field in raw_indexes:
            ...
            if field['Projection']['ProjectionType'] == 'ALL':
                ...
                if field['Projection']['ProjectionType'] == 'KEYS_ONLY':
                    index_klass = map_indexes_projection.get('KEYS_ONLY')
                else:
                    if field['Projection']['ProjectionType'] == 'INCLUDE':
    ...
    def update(self, throughput=None, global_indexes=None):
        ...
        if throughput:
            self.throughput = throughput
            data = {'ReadCapacityUnits': int(self.throughput['read']), 
             'WriteCapacityUnits': int(self.throughput['write'])}
        gsi_data = None
        if global_indexes:
            gsi_data = []
            for gsi_name, gsi_throughput in global_indexes.items():
                gsi_data.append({'Update': {'IndexName': gsi_name, 
                            'ProvisionedThroughput': {'ReadCapacityUnits': int(gsi_throughput['read']), 
                                                      'WriteCapacityUnits': int(gsi_throughput['write'])}}})

        if throughput or global_indexes:
            self.connection.update_table(self.table_name, provisioned_throughput=data, global_secondary_index_updates=gsi_data)
            return True
        else:
            msg = 'You need to provide either the throughput or the global_indexes to update method'
            boto.log.error(msg)
            return False
    ...
    def delete_global_secondary_index(self, global_index_name):
        ...
        if global_index_name:
            gsi_data = [
             {'Delete': {'IndexName': global_index_name}}]
    ...
    def get_item(self, consistent=False, attributes=None, **kwargs):
        ...
        item_data = self.connection.get_item(self.table_name, raw_key, attributes_to_get=attributes, consistent_read=consistent)
    ...
    def delete_item(self, expected=None, conditional_operator=None, **kwargs):
        ...
        **IMPORTANT** - Be careful when using this method, there is no undo.
        ...
        raw_key = self._encode_keys(kwargs)
        try:
            self.connection.delete_item(self.table_name, raw_key, expected=expected, conditional_operator=conditional_operator)
    ...
    def _build_filters(self, filter_kwargs, using=QUERY_OPERATORS):
        ...
        for field_and_op, value in filter_kwargs.items():
            ...
            try:
                ...
                raise exceptions.UnknownFilterTypeError("Operator '%s' from '%s' is not recognized." % (
                 field_bits[(-1)],
                 field_and_op))
    ...
    def query(self, limit=None, index=None, reverse=False, consistent=False, attributes=None, max_page_size=None, **filter_kwargs):
        ...
        **WARNING:** This method is provided **strictly** for
        ...
        """
        reverse = not reverse
        return self.query_2(limit=limit, index=index, reverse=reverse, consistent=consistent, attributes=attributes, max_page_size=max_page_size, **filter_kwargs)
    ...
    def query_count(self, index=None, consistent=False, conditional_operator=None, query_filter=None, scan_index_forward=True, limit=None, exclusive_start_key=None, **filter_kwargs):
        ...
        while 1:
            raw_results = self.connection.query(self.table_name, index_name=index, consistent_read=consistent, select='COUNT', key_conditions=key_conditions, query_filter=built_query_filter, conditional_operator=conditional_operator, limit=limit, scan_index_forward=scan_index_forward, exclusive_start_key=last_evaluated_key)
    ...
    def _query(self, limit=None, index=None, reverse=False, consistent=False, exclusive_start_key=None, select=None, attributes_to_get=None, query_filter=None, conditional_operator=None, **filter_kwargs):
        ...
        kwargs['query_filter'] = self._build_filters(query_filter, using=FILTER_OPERATORS)
