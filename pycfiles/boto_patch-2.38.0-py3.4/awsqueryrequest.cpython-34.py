...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/boto/roboto/awsqueryrequest.py
# Compiled at: 2015-11-24 05:02:18
# Size of source mod 2**32: 18579 bytes
import sys, os, boto, optparse, copy, boto.exception, boto.roboto.awsqueryservice, bdb, traceback
try:
...
class AWSQueryRequest(object):
    ...
    Filters = []
    Response = {}
    CLITypeMap = {'string': 'string',  'integer': 'int', 
     'int': 'int', 
     'enum': 'choice', 
     'datetime': 'string', 
     'dateTime': 'string', 
        ...
        for i, filter in enumerate(self.Filters):
            ...
            if name in filters:
                ...
                for j, value in enumerate(boto.utils.mklist(filters[name])):
                    Encoder.encode(filter, self.request_params, value, 'Filter.%d.Value.%d' % (i + 1, j + 1))
    ...
    def process_standard_options(self, options, args, d):
        if hasattr(options, 'help_filters'):
        ...
        if options.version:
            print('version x.xx')
            exit(0)
        sys.excepthook = boto_except_hook(options.debugger, options.debug)
    ...
    def build_cli_parser(self):
        self.parser = optparse.OptionParser(description=self.Description, usage=self.get_usage())
        ...
        for param in self.Params:
            ...
            if ptype or action == 'store_true':
                if param.short_name:
                    self.parser.add_option(param.optparse_short_name, param.optparse_long_name, action=action, type=ptype, choices=param.choices, help=param.doc)
                else:
                    if param.long_name:
                        self.parser.add_option(param.optparse_long_name, action=action, type=ptype, choices=param.choices, help=param.doc)
                    else:
                        continue
    ...
    def do_cli(self):
        ...
        self.args.update(d)
        if hasattr(self.cli_options, 'filter'):
            if self.cli_options.filter:
                d = {}
