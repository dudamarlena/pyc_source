...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/boto/cloudfront/distribution.py
# Compiled at: 2015-11-24 05:02:18
# Size of source mod 2**32: 31285 bytes
...
class Distribution(object):
    ...
    def __init__(self, connection=None, config=None, domain_name='', id='', last_modified_time=None, status=''):
        self.connection = connection
        self.config = config
        self.domain_name = domain_name
        self.id = id
        ...
        if name == 'Id':
            ...
            if name == 'LastModifiedTime':
                ...
                if name == 'Status':
                    ...
                    if name == 'InProgressInvalidationBatches':
    ...
    def update(self, enabled=None, cnames=None, comment=None):
        ...
        """
        new_config = DistributionConfig(self.connection, self.config.origin, self.config.enabled, self.config.caller_reference, self.config.cnames, self.config.comment, self.config.trusted_signers, self.config.default_root_object)
...
class StreamingDistribution(Distribution):
    ...
    def update(self, enabled=None, cnames=None, comment=None):
        ...
        """
        new_config = StreamingDistributionConfig(self.connection, self.config.origin, self.config.enabled, self.config.caller_reference, self.config.cnames, self.config.comment, self.config.trusted_signers)
        if enabled is not None:
            new_config.enabled = enabled
        if cnames is not None:
            new_config.cnames = cnames
