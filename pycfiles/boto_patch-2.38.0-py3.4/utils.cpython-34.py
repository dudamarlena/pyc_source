...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/boto/utils.py
# Compiled at: 2015-11-24 05:02:18
# Size of source mod 2**32: 36002 bytes
"""
Some handy utility functions used by several classes.
"""
import subprocess, time, logging.handlers, boto, boto.provider, tempfile, random, smtplib, datetime, re, email.mime.multipart, email.mime.base, email.mime.text, email.utils, email.encoders, gzip, threading, locale
...
qsa_of_interest = [
 'acl', 'cors', 'defaultObjectAcl', 'location', 'logging',
...
def unquote_v(nv):
    if len(nv) == 1:
        ...
        return (
         nv[0], urllib.parse.unquote(nv[1]))
        ...
        if key in self._leaves:
            ...
            for i in range(0, self._num_retries):
                try:
                    ...
                    boto.log.debug("encountered '%s' exception: %s" % (
                ...
                boto.log.error("encountered '%s' exception: %s" % (
...
def guess_mime_type(content, deftype):
    ...
    :return: <description>
    """
    starts_with_mappings = {'#include': 'text/x-include-url', 
     '#!': 'text/x-shellscript', 
