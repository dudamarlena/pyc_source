...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/boto/swf/layer2.py
...
DEFAULT_CREDENTIALS = {'aws_access_key_id': None, 
 'aws_secret_access_key': None}

def set_default_credentials(aws_access_key_id, aws_secret_access_key):
    """Set default credentials."""
    DEFAULT_CREDENTIALS.update({'aws_access_key_id': aws_access_key_id, 
...
class Domain(SWFBase):
    __doc__ = 'Simple Workflow Domain.'
    description = None
    retention = 30
    ...
    def activities(self, status='REGISTERED', **kwargs):
        ...
        for act_args in act_types['typeInfos']:
            ...
            act_args.update({'aws_access_key_id': self.aws_access_key_id, 
    ...
    def workflows(self, status='REGISTERED', **kwargs):
        ...
        for wf_args in wf_types['typeInfos']:
            ...
            wf_args.update({'aws_access_key_id': self.aws_access_key_id, 
             'aws_secret_access_key': self.aws_secret_access_key, 
             'domain': self.name, 
             'region': self.region})
    ...
    def executions(self, closed=False, **kwargs):
        ...
        if closed:
            ...
            if 'oldest_date' not in kwargs:
                kwargs['oldest_date'] = time.time() - 86400
        ...
        for exe_args in executions['executionInfos']:
            ...
            exe_args.update({'aws_access_key_id': self.aws_access_key_id, 
             'aws_secret_access_key': self.aws_secret_access_key, 
             'domain': self.name, 
             'region': self.region})
...
class ActivityWorker(Actor):
    __doc__ = 'Base class for SimpleWorkflow activity workers.'
...
class Decider(Actor):
    __doc__ = 'Base class for SimpleWorkflow deciders.'
...
class WorkflowType(SWFBase):
    __doc__ = 'A versioned workflow type.'
    version = None
    task_list = None
    ...
    def register(self, **kwargs):
        ...
        args = {'default_execution_start_to_close_timeout': '3600', 
    ...
    def start(self, **kwargs):
        ...
        run_id = self._swf.start_workflow_execution(self.domain, workflow_id, self.name, self.version, **kwargs)['runId']
        return WorkflowExecution(name=self.name, version=self.version, runId=run_id, domain=self.domain, workflowId=workflow_id, aws_access_key_id=self.aws_access_key_id, aws_secret_access_key=self.aws_secret_access_key)


class WorkflowExecution(SWFBase):
    __doc__ = 'An instance of a workflow.'
    workflowId = None
    runId = None
...
class ActivityType(SWFBase):
    __doc__ = 'A versioned activity type.'
    version = None
    ...
    def register(self, **kwargs):
        ...
        args = {'default_task_heartbeat_timeout': '600', 
