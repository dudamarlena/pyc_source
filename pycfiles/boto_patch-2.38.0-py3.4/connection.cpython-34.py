...
class AWSAuthConnection(object):
    ...
    def new_http_connection(self, host, port, is_secure):
        ...
        if self.use_proxy:
            if not is_secure and not self.skip_proxy(host):
                host = self.proxy
    ...
    def build_base_http_request(self, method, path, auth_path, params=None, headers=None, data='', host=None):
        ...
        if self.host_header:
            if not boto.utils.find_matching_headers('host', headers):
        ...
        if self.use_proxy:
            ...
            if self.proxy_user:
                if self.proxy_pass and not self.is_secure:
