...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/udatetime/_pure.py
# Compiled at: 2018-02-14 07:13:23
...
def _timestamp_to_date_time(timestamp, tzinfo):
    ...
    frac = (t_full - timestamp) * 1000000.0
...
def _get_local_utc_offset():
    ts = time()
    return (dt_datetime.fromtimestamp(ts) - dt_datetime.utcfromtimestamp(ts)).total_seconds() / 60
...
def from_rfc3339_string(rfc3339_string):
    ...
    if 't' not in rfc3339_string:
        raise ValueError("Invalid RFC3339 string. Missing 'T' date/time separator.")
    ...
    if len(_time) > 8:
        if _time[8] == '.':
            ...
            offset = 0
            if len(_time[9:]):
                raise ValueError('Invalid RFC3339 string. Remaining data after time zone.')
        else:
            _time = _time[8:]
    else:
        offset = 0
    if offset is None and (len(_time) == 0 or _time[0] == 'z'):
        ...
        if negative:
            offset = offset * -1
    return dt_datetime(year, month, day, hour, minute, second, usec, TZFixedOffset(offset))
