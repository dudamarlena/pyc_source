...
# [GCC 8.4.0]
# Embedded file name: /Users/smidth/GitHub/django-commentator/commentator/templatetags/commentator_tags.py
# Compiled at: 2015-03-18 05:59:19
...
def _line(context, node, tree, timestamp):
    ...
    rendered = render_to_string('commentator/comment_body.html', {'obj': node, 'comments': tree, 'new_comment': new_comment}, context)
...
def _subtree(context, node, timestamp):
    ...
    for subnode in node.children():
        tree += _subtree(context, subnode, timestamp)
...
def comment_wrapper(context, obj):
    ...
    thread, __ = Thread.objects.get_or_create(object_pk=str(obj.pk), content_type=contenttype)
    timestamp = timezone.now()
    if request.user.is_authenticated():
        threadview, __ = ThreadView.objects.get_or_create(user=request.user, thread=thread)
    ...
    for root_node in comments:
        tree += _subtree(context, root_node, timestamp)

    tree = '<ol class="comment-list" id="comments">%s</ol>\n' % tree
    return {'comments': tree, 'obj': obj, 'thread': thread, 'form': CommentForm(), 
       'request': request}
...
@register.simple_tag
def commentator_js():
    config = '<script type="text/javascript">CommentatorConfig=%s</script>' % json.dumps(COMMENTATOR_CONFIG, ensure_ascii=False)
    script_html = '<script type="text/javascript" src="%s"></script>'
    js_file = urljoin(get_static_url(), 'commentator/js/default.js')
    editor_file = urljoin(get_static_url(), 'commentator/editor/jquery.markitup.js')
...
class GravatarUrlNode(template.Node):
    ...
    def render(self, context):
        ...
        params = dict(s=self.size or getattr(settings, 'GRAVATAR_SIZE', 40), r=getattr(settings, 'GRAVATAR_RATING', 'pg'))
