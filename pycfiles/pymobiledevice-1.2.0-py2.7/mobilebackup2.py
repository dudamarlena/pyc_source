...
# Compiled at: 2019-03-03 17:11:09
import os, datetime, plistlib, logging
...
class MobileBackup2(MobileBackup):
    ...
    def start(self):
        ...
        if not os.path.isdir(self.backupPath):
            os.makedirs(self.backupPath, 493)
    ...
    def mobilebackup2_send_request(self, request, target, source, options={}):
        d = {'TargetIdentifier': target, 'SourceIdentifier': source, 
           'Options': options}
    ...
    def mobilebackup2_send_status_response(self, status_code, status1='___EmptyParameterString___', status2={}):
        a = [
         'DLMessageStatusResponse', status_code, status1, status2]
    ...
    def mb2_handle_send_file(self, filename, errplist):
        ...
        if data != None:
            ...
            if PY3:
                msg = ('').join([CODE_FILE_DATA.to_bytes(1, 'little'), data])
            else:
    ...
    def work_loop(self):
        while True:
            ...
            if not msg:
                break
            assert msg[0] in ('DLMessageDownloadFiles', 'DLContentsOfDirectory', 'DLMessageCreateDirectory',
                              'DLMessageUploadFiles', 'DLMessageMoveFiles', 'DLMessageMoveItems',
                              'DLMessageRemoveFiles', 'DLMessageRemoveItems', 'DLMessageCopyItem',
                              'DLMessageProcessMessage', 'DLMessageGetFreeDiskSpace',
                              'DLMessageDisconnect')
    ...
    def create_status_plist(self, fullBackup=True):
        statusDict = {'UUID': str(uuid4()).upper(), 'BackupState': 'new', 
    ...
    def create_info_plist(self):
        ...
        info = {'BuildVersion': device_info.get('BuildVersion') or '', 'DeviceName': device_info.get('DeviceName') or '', 
        ...
        iTunesFiles = [
         'ApertureAlbumPrefs', 'IC-Info.sidb', 'IC-Info.sidv', 'PhotosFolderAlbums',
    ...
    def restore(self, options={'RestoreSystemFiles': True, 'RestoreShouldReboot': True, 
   'RestorePreserveCameraRoll': True, 
   'RemoveItemsNotRestored': False, 
   'RestoreDontCopyBackup': True, 
   'RestorePreserveSettings': True}, password=None):
...
if __name__ == '__main__':
    ...
    parser.add_option('-b', '--backup', dest='backup', action='store_true', default=False, help='Backup device')
    parser.add_option('-r', '--restore', dest='restore', action='store_true', default=False, help='Restore device')
    parser.add_option('-i', '--info', dest='info', action='store_true', default=False, help='Show backup info')
    parser.add_option('-l', '--list', dest='list', action='store_true', default=False, help='Show backup info')
    parser.add_option('-p', '--path', dest='path', action='store', default=False, help='path to backup/restore to')
