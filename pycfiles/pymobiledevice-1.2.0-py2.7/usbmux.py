...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/pymobiledevice/usbmux/usbmux.py
...
class BinaryProtocol(object):
    ...
    def _unpack(self, resp, payload):
        ...
        if resp == self.TYPE_DEVICE_REMOVE:
...
class PlistProtocol(BinaryProtocol):
    ...
    def sendpacket(self, req, tag, payload={}):
        ...
        if isinstance(req, int):
            req = [
             self.TYPE_CONNECT, self.TYPE_LISTEN][(req - 2)]
...
class MuxConnection(object):
    ...
    def connect(self, device, port):
        ret = self._exchange(self.proto.TYPE_CONNECT, {'DeviceID': device.devid, 'PortNumber': port << 8 & 65280 | port >> 8})
        if ret != 0:
