...
# Embedded file name: /home/nivekuil/code/amp/python3/amp/mpv.py
# Compiled at: 2016-02-10 10:50:06
from ctypes import *
...
class MpvFormat(c_int):
    ...
    def __repr__(self):
        return [
         'NONE', 'STRING', 'OSD_STRING', 'FLAG', 'INT64', 'DOUBLE', 'NODE', 'NODE_ARRAY', 'NODE_MAP'][self.value]
...
class MpvEventID(c_int):
    ...
    CHAPTER_CHANGE = 23
    ANY = (
     SHUTDOWN, LOG_MESSAGE, GET_PROPERTY_REPLY, SET_PROPERTY_REPLY, COMMAND_REPLY, START_FILE, END_FILE,
...
class MpvEvent(Structure):
    ...
    def as_dict(self):
        dtype = {MpvEventID.END_FILE: MpvEventEndFile, MpvEventID.PROPERTY_CHANGE: MpvEventProperty, 
           ...
           MpvEventID.CLIENT_MESSAGE: MpvEventClientMessage}.get(self.event_id.value, None)
        return {'event_id': self.event_id.value, 'error': self.error, 
...
class MpvEventProperty(Structure):
    ...
    def as_dict(self):
        if self.format.value == MpvFormat.STRING:
            ...
            return {'name': self.name.decode(), 'format': self.format, 
               'data': self.data, 
               'value': proptype(cast(self.data, POINTER(c_char_p)).contents.value.decode())}
        else:
            return {'name': self.name.decode(), 'format': self.format, 'data': self.data}
            return
...
class ynbool:
    ...
    def __init__(self, val=False):
        self.val = bool(val and val not in ('no', 'no'))
...
def _ensure_encoding(possibly_bytes):
    if type(possibly_bytes) is bytes:
        return possibly_bytes.decode()
    return possibly_bytes
...
class MPV:
    ...
    def __init__(self, **kwargs):
        ...
        _mpv_set_option_string(self.handle, 'audio-display', 'no')
        istr = --- This code section failed: ---

 L. 299         0  LOAD_GLOBAL           0  'type'
                3  LOAD_FAST             0  'o'
                6  CALL_FUNCTION_1       1  None
                9  LOAD_GLOBAL           1  'bool'
               12  COMPARE_OP            8  is
               15  POP_JUMP_IF_FALSE    34  'to 34'
               18  LOAD_FAST             0  'o'
               21  POP_JUMP_IF_FALSE    30  'to 30'
               24  LOAD_CONST               'yes'
               27  JUMP_ABSOLUTE        43  'to 43'
               30  LOAD_CONST               'no'
               33  RETURN_VALUE_LAMBDA
             34_0  COME_FROM            15  '15'
               34  LOAD_GLOBAL           2  'str'
               37  LOAD_FAST             0  'o'
               40  CALL_FUNCTION_1       1  None
               43  RETURN_VALUE_LAMBDA
               -1  LAMBDA_MARKER    
...
Parse error at or near `None' instruction at offset -1
    ...
    def command(self, name, *args):
        ...
        args = [
         name.encode()] + [ str(arg).encode() for arg in args if arg is not None ] + [None]
