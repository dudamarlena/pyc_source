...
# [GCC 8.4.0]
# Embedded file name: D:\Projects\A_MSU_Hopfield_Model\VS\SingleCellProcessing\bin\fast_tsne.py
# Compiled at: 2019-03-28 09:48:09
# Size of source mod 2**32: 10011 bytes
import os, subprocess, struct, numpy as np

def fast_tsne(X, theta=0.5, perplexity=30, map_dims=2, max_iter=1000, stop_early_exag_iter=250, K=-1, sigma=-1, nbody_algo='FFT', knn_algo='annoy', mom_switch_iter=250, momentum=0.5, final_momentum=0.8, learning_rate=200, early_exag_coeff=12, no_momentum_during_exag=False, n_trees=50, search_k=None, start_late_exag_iter=-1, late_exag_coeff=-1, nterms=3, intervals_per_integer=1, min_num_intervals=50, seed=-1, initialization=None, load_affinities=None, perplexity_list=None, df=1, return_loss=False, nthreads=None):
    ...
    subprocess.call([
     os.path.dirname(os.path.realpath(__file__)) + '/bin/fast_tsne', version_number, 'data.dat', 'result.dat', '{}'.format(nthreads)])
