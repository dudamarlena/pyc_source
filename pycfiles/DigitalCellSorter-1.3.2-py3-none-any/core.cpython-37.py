...
# [GCC 8.4.0]
# Embedded file name: D:\Projects\A_DCS\VS\DigitalCellSorter\DigitalCellSorter\core.py
# Compiled at: 2020-05-06 16:59:09
# Size of source mod 2**32: 123127 bytes
...
"""
import os, platform, copy, multiprocessing, warnings, numpy as np, pandas as pd, scipy.stats, scipy.signal, scipy.linalg, scipy.cluster.hierarchy, scipy.spatial.distance
...
class DigitalCellSorter(VisualizationFunctions):
    ...
    def fileHDFpath(self, value):
        warnings.warn("Direct change of HDF file path is not allowed. Modify 'saveDir' in order to change 'fileHDFpath'", UserWarning)
    ...
    def df_expr(self, value):
        if value is None:
            ...
            warnings.warn("Direct change of 'df_expr' is not allowed. Use function 'prepare' in order to set 'df_expr' or, alternatively, initialize class instance with 'df_expr'. 'df_expr' can be removed by assigning None to it", UserWarning)
        ...
        if type(obj) is pd.Series:
            ...
            if type(obj) is pd.DataFrame:
                ...
                if len(obj.columns.names) > 1:
                    if 'cell' not in obj.columns.names:
                        print('Columns level "cell" not found. Returning None', flush=True)
                        return
                    else:
                        obj.columns.names = [
                         'cell']
                elif 'batch' not in obj.columns.names:
                ...
                if 'cell' in columns:
                    if 'gene' in columns:
                        if 'expr' in columns:
                            ...
                            if 'cell' not in df_expr.columns:
    ...
    def normalize(self, median=None):
        ...
        MIN = np.min(self._df_expr.values[(self._df_expr.values > 0.0)])
        if MIN <= 0.0:
            raise ValueError
        self._df_expr = self._df_expr.replace(0.0, MIN)
        self._df_expr = np.log2(self._df_expr)
        self._df_expr -= np.min(self._df_expr.values)
        self._df_expr = self._df_expr[(self._df_expr.sum(axis=1) > 0)]
        print(('Removed all-zero genes. Data size: %s genes, %s cells' % self._df_expr.shape), flush=True)
        if self.sigmaOverMeanSigma is not None:
    ...
    def project(self, PCAonly=False, do_fast_tsne=True):
        ...
        print('Explained variance:', (np.round(np.sum(_PCA.explained_variance_ratio_) * 100.0, 2)), '%', flush=True)
        ...
        if (PCAonly or self.layout) == 'TSNE':
            ...
            if self.layout == 'UMAP':
                ...
                if self.layout == 'PCA':
                    ...
                    return (
                     X_pca, _PCA.components_, X_projection2)
                    return (
                     X_pca, _PCA.components_)
    ...
    def cluster(self):
        ...
        if type(clusteringFunction) is dict:
            import pynndescent, networkx as nx, community
            try:
            ...
            if self.doFineClustering:
                ...
                for cluster in clusters:
                    ...
                    subData = subData[(subData.sum(axis=1) > 0.0)]
    ...
    def annotate(self, mapNonexpressedCelltypes=True):
        ...
        if 'pDCS' in methodsToUse and 'ratio' in methodsToUse and 'Hopfield' in methodsToUse:
            ...
            annotationResults[2] = np.power(annotationResults[2], 0.3333333333333333)
            annotationResults[2][annotationResults[2] < self.thresholdForUnknown] = 0.0
        else:
            if 'pDCS' in methodsToUse and 'ratio' in methodsToUse and 'Hopfield' not in methodsToUse:
                annotationResults = annotationResults_pDCS
                annotationResults[2] *= annotationResults_ratio[2]
                annotationResults[2] = np.power(annotationResults[2], 0.5)
                annotationResults[2][annotationResults[2] < self.thresholdForUnknown] = 0.0
            else:
                if 'pDCS' in methodsToUse and 'ratio' not in methodsToUse and 'Hopfield' in methodsToUse:
                    annotationResults = annotationResults_pDCS
                    annotationResults[2] *= annotationResults_Hopfield[2]
                    annotationResults[2] = np.power(annotationResults[2], 0.5)
                    annotationResults[2][annotationResults[2] < self.thresholdForUnknown] = 0.0
                else:
                    if 'pDCS' not in methodsToUse and 'ratio' in methodsToUse and 'Hopfield' in methodsToUse:
                        annotationResults = annotationResults_ratio
                        annotationResults[2] *= annotationResults_Hopfield[2]
                        annotationResults[2] = np.power(annotationResults[2], 0.5)
                        annotationResults[2][annotationResults[2] < self.thresholdForUnknown] = 0.0
                    else:
                        if 'pDCS' in methodsToUse and 'ratio' not in methodsToUse and 'Hopfield' not in methodsToUse:
                            annotationResults = annotationResults_pDCS
                        else:
                            if 'pDCS' not in methodsToUse and 'ratio' in methodsToUse and 'Hopfield' not in methodsToUse:
                                annotationResults = annotationResults_ratio
                            else:
                                if 'pDCS' not in methodsToUse and 'ratio' not in methodsToUse and 'Hopfield' in methodsToUse:
    ...
    def process(self, dataIsNormalized=False, cleanData=True):
        ...
        if not dataIsNormalized:
            ...
            dataIsNormalized or self.normalize()
    ...
    def getExprOfGene(self, gene, analyzeBy='cluster'):
        ...
        if not os.path.isfile(fileName):
            ...
            df_markers_expr = df_markers_expr.reindex(columns, axis=1).fillna(0.0)
            if analyzeBy == 'celltype':
                ...
                df_markers_expr.columns = pd.MultiIndex.from_arrays([columns.get_level_values('batch'),
                 ...
                 columns.get_level_values(analyzeBy).str.split(' #', expand=True).get_level_values(0)],
                  names=['batch', 'cell', 'celltype'])
            else:
    ...
    def getExprOfCells(self, cells):
        ...
        index = pd.MultiIndex.from_arrays([self._df_expr.columns.get_level_values('batch'),
         self._df_expr.columns.get_level_values('cell')],
          names=['batch', 'cell'])
    ...
    def getIndexOfGoodQualityCells(self, QCplotsSubDir='QC_plots', **kwargs):
        ...
        """
        df_QC = pd.read_hdf((self.fileHDFpath), key='QC')
        if self.excludedFromQC is not None:
        ...
        mask = (df_QC['count_depth'] > cutoff_count_depth) & (df_QC['number_of_genes'] > cutoff_number_of_genes) & (df_QC['fraction_of_mitochondrialGenes'] < cutoff_fraction_of_mitochondrialGenes)
    ...
    def getQualityControlCutoff(self, se, cutoff, mito=False, MakeHistogramPlot=True, **kwargs):
        ...
        if mito:
            ...
            hist_of_subset = scipy.stats.rv_histogram(np.histogram(subset, bins=100, range=(0,
                                                                                            0.3)))
            ...
            y2 = smoothed[wh]
            cutoff = min(median + 3.0 * std, x1 - y1 * (x2 - x1) / (y2 - y1))
        else:
    ...
    def getCountsDataframe(self, se1, se2, tagForMissing='N/A'):
        ...
        moveTag = lambda df:         if tagForMissing in df.index:
pd.concat([df.iloc[np.where(df.index != tagForMissing)[0]], df.iloc[np.where(df.index == tagForMissing)[0]]], axis=0, sort=False) # Avoid dead code: df
    ...
    def getNewMarkerGenes(self, cluster=None, top=100, zScoreCutoff=None, removeUnknown=False, **kwargs):
        ...
        if zScoreCutoff is None:
            ...
            if cluster is not None:
                ...
                return {'genes':clusterGenes.index.values.tolist(), 
                 'zscore':clusterGenes.values.tolist()}
        ...
        df_new_marker_genes[df_new_marker_genes < 0.0] = 0.0
        ...
        df_new_marker_list = df_new_marker_list.fillna(0.0).sort_index()
        df_new_marker_list[df_new_marker_list > 0.0] = 1.0
        ...
        writer.save()
        df_marker_cell_type = pd.read_excel((os.path.join(self.saveDir, self.dataName + '_annotation.xlsx')), sheet_name='Marker cell type weight matrix',
          index_col=0).T
    ...
    def calculateV(cls, args):
        ...
        df_Z = 1.0 * (df_Z > cutoff)
        df_V = pd.Series(df_M.dot(df_Z).stack().fillna(0.0))
        if removeLowQCscores:
            minimumFraction = 0.1
            df_Q = (1.0 * (df_M > 0.0)).dot(1.0 * (df_Z > 0.0)) < (minimumFraction * (1.0 * (df_Z > 0.0)).sum(axis=0)[None, :]).astype(int)
            df_V[pd.Series(df_Q.stack().fillna(0.0))] = 0.0
        if giveSignificant:
            return (
             df_V, df_Z)
        return df_V
    ...
    def annotateWith_pDCS_Scheme(self, df_markers_expr, df_marker_cell_type):
        ...
        df_positive[df_marker_cell_type < 0.0] = 0.0
        df_negative[df_marker_cell_type > 0.0] = 0.0
        df_positive /= (df_positive > 0.0).sum(axis=0).fillna(1.0).replace(0.0, 1.0).replace(0, 1.0)
        df_negative /= (df_negative < 0.0).sum(axis=0).fillna(1.0).replace(0.0, 1.0).replace(0, 1.0)
        ...
        df_markers_expr_copy.columns = pd.MultiIndex.from_arrays([df_markers_expr_copy.columns.get_level_values(0),
         ...
         uindex],
          names=(df_markers_expr_copy.columns.names))
        ...
        random_df_V.index = pd.MultiIndex.from_arrays([random_df_V.index.get_level_values(0),
         pd.Series(random_df_V.index.get_level_values(1)).replace(dict(zip(range(len(uclusters)), uclusters))).values],
          names=[
         random_df_V.index.names[0], 'cluster'])
        random_df_V = random_df_V.replace(0.0, np.nan)
        ...
        sigma = pd.Series(data=np.nanstd((random_df_V.values), axis=1), index=(random_df_V.index)).replace(0.0, np.inf).unstack()
        mean = pd.Series(data=np.nanmean((random_df_V.values), axis=1), index=(random_df_V.index)).unstack()
        df_L = (df_V - mean) / sigma
        df_L[df_L < self.thresholdForUnknown_pDCS] = 0.0
        df_L[df_V < 0.0] = 0.0
        return (
         df_marker_cell_type, df_markers_expr, df_L, df_V, dict_expressed_markers, df_null_distributions)
    ...
    def annotateWith_ratio_Scheme(self, df_markers_expr, df_marker_cell_type):
        ...
        df_marker_cell_type_full[df_marker_cell_type_full < 0.0] = 0.0
        df_marker_cell_type_full /= (df_marker_cell_type_full > 0.0).sum(axis=0).fillna(1.0).replace(0.0, 1.0)
        df_L = df_V / df_marker_cell_type_full.sum(axis=1).values[:, None]
        df_L[df_L < self.thresholdForUnknown_ratio] = 0.0
        df_L[df_V < 0.0] = 0.0
        return (
         df_marker_cell_type, df_markers_expr, df_L, df_V, dict_expressed_markers, None)
    ...
    def annotateWith_Hopfield_Scheme(self, df_markers_expr, df_marker_cell_type):
        ...
        np.random.seed(0)
        df_marker_cell_type[df_marker_cell_type > 0.0] = 1.0
        df_marker_cell_type[df_marker_cell_type < 0.0] = 0.0
        ...
        df_marker_cell_type = df_marker_cell_type.loc[(np.abs(df_marker_cell_type).sum(axis=1) > 0)]
        df_marker_cell_type[(df_marker_cell_type > 0.0)] /= (df_marker_cell_type[(df_marker_cell_type > 0.0)] > 0).sum(axis=1).replace(0, 1.0).values[:, None]
        df_marker_cell_type[(df_marker_cell_type > 0.0)] /= (df_marker_cell_type[(df_marker_cell_type > 0.0)] > 0).sum(axis=0).replace(0, 1.0).values
        df_marker_cell_type[(df_marker_cell_type < 0.0)] /= (df_marker_cell_type[(df_marker_cell_type < 0.0)] < 0).sum(axis=1).replace(0, 1.0).values[:, None]
        df_marker_cell_type[(df_marker_cell_type < 0.0)] /= (df_marker_cell_type[(df_marker_cell_type < 0.0)] < 0).sum(axis=0).replace(0, 1.0).values
        print(df_marker_cell_type.shape)
        ...
        for i in range(self.nSamples_Hopfield):
            ...
            hop = self.propagateHopfield(sigma=(df_sig.values * 2.0 - 1.0), xi=(df_marker_cell_type.values),
              typesNames=(df_marker_cell_type.columns.values),
              underlyingNetwork=underlyingNetwork,
              clustersNames=(df_sig.columns.values),
              id=i,
              T=(self.HopfieldTemperature),
              path=(os.path.join(self.saveDir, 'HopfieldTrajectories')))
            res = np.argmax(hop, axis=0)
            res[np.where(np.max(hop, axis=0) < 1e-12)] = len(df_marker_cell_type.columns)
            listOfSeries.append(res)

        print(flush=True)
        r = np.vstack(listOfSeries)
        df_L = pd.DataFrame(index=(list(range(len(df_marker_cell_type.columns) + 1))), columns=(df_sig.columns)).fillna(0.0)
        for i in range(len(df_sig.columns)):
            w = np.unique((r[:, i]), return_counts=True)
            for j in range(len(w[0])):
                df_L.iloc[(w[0][j], i)] = w[1][j] / len(r)

        df_L[df_L < self.thresholdForUnknown_Hopfield] = 0.0
        ...
        return (
         df_marker_cell_type.T, df_markers_expr, df_L, df_L.copy(), dict_expressed_markers)
    ...
    def recordAnnotationResults(self, df_marker_cell_type, df_markers_expr, df_L, df_V, dict_expressed_markers, df_null_distributions=None):
        ...
        T[(df_L.values <= 0.0).all(axis=0)] = self.nameForUnknown
        T = pd.Index(T) + ' #0'
        ...
        supporting_markers = [' // '.join(np.intersect1d(all_supp_markers[df_V.loc['Predicted cell type'].loc[cluster].split(' #')[0]], df_V.loc['All markers'].loc[cluster].split(' // ')).tolist()) for cluster in df_V.columns]
        contradicting_markers = [' // '.join(np.intersect1d(all_cont_markers[df_V.loc['Predicted cell type'].loc[cluster].split(' #')[0]], df_V.loc['All markers'].loc[cluster].split(' // ')).tolist()) for cluster in df_V.columns]
        ...
        if df_null_distributions is not None:
    ...
    def propagateHopfield(cls, sigma=None, xi=None, T=0.2, tmax=200, fractionToUpdate=0.5, mode=4, meshSamplingRate=100, underlyingNetwork=None, typesNames=None, clustersNames=None, printInfo=False, recordTrajectories=True, id=None, printSwitchingFraction=False, path=None):
        ...
        if xi is None:
            ...
            if mode == 1:
                ...
                A = Q_inv.dot(xi.T)
                J = xi.dot(A)
            if underlyingNetwork is not None:
            ...
            if recordTrajectories:
                ...
                if id == 0 or id is None:
                    ...
                    for i in range(xi.shape[1]):
                        for j in range(25):
                            st = A[i]
                            temp.append(np.array([st * ((np.random.rand(st.shape[0]) > 0.025 * j) * 2.0 - 1.0) for i in range(meshSamplingRate)]))
            ...
            initial[np.where(np.max((A.dot(sigma)), axis=0) < 1e-12)] = -1
            if printInfo:
                sIn = sigma.copy()
            for t in range(tmax):
                h = J.dot(sigma)
                ...
                sigmaNew = (1.0 / (1.0 + np.exp(-2.0 * h / T)) > (np.random.rand)(*sigma.shape)) * 2.0 - 1.0
                if mode == 4:
                    sigmaNew[sigmaNew - sigma < -1.0] = 0.0
                f = 1.0 if t >= 100 else max(0.05, float(t) / 100.0)
            ...
            if printInfo:
                print('In positive:', ((sIn > 0) * 1.0).sum(axis=0))
                print('Out positive:', ((sigma > 0) * 1.0).sum(axis=0))
                print('Positive common in-out:', ((sIn > 0.0) * (sigma > 0.0) * 1.0).sum(axis=0))
            final = np.argmax((A.dot(sigma)), axis=0)
            final[np.where(np.max((A.dot(sigma)), axis=0) < 1e-12)] = -1
            if printSwitchingFraction:
                print((np.round(100.0 * (final != initial).sum() / len(initial), 2).astype(int)), end=' ', flush=True)
            if not recordTrajectories or id == 0 or id is None:
    ...
    def getSubnetworkOfPCN(self, subnetworkGenes, min_shared_first_targets=30):
        ...
        subnetwork = 1.0 * (targetsOverlaps >= min_shared_first_targets)
        print(subnetwork.shape)
        subnetwork = subnetwork.reindex(subnetworkGenes, axis=0).reindex(subnetworkGenes, axis=1).fillna(0.0)
        print(subnetwork.shape)
        con = subnetwork.sum().sum()
        tot = (len(subnetwork) - 1) * len(subnetwork)
        print('\nConnections in subnetwork: %s, out of %s possible (%s%%)\n' % (con, tot, np.round(100.0 * con / tot, 0)))
        return subnetwork
    ...
    def readMarkerFile(self, mergeFunction='mean', mergeCutoff=0.25):
        ...
        """
        df_marker_cell_type = pd.read_excel((self.geneListFileName), index_col=0, header=[0, 1]).replace(np.nan, 0.0).replace(0, 0.0)
        ...
        df_marker_cell_type = df_marker_cell_type.groupby(level='CellTypeGrouped', axis=1).agg(mergeFunction).fillna(0.0)
        where_positive = df_marker_cell_type >= mergeCutoff
        where_negative = df_marker_cell_type <= -mergeCutoff
        df_marker_cell_type *= 0.0
        df_marker_cell_type[where_positive] = 1.0
        df_marker_cell_type[where_negative] = -1.0
        df_marker_cell_type = df_marker_cell_type.loc[(np.abs(df_marker_cell_type).sum(axis=1) > 0)]
        ...
        def norm(s_input):
            ...
            s.iloc[np.where(s > 0.0)] /= pos_sum if pos_sum > 0.0 else 1.0
            s.iloc[np.where(s < 0.0)] /= neg_sum if neg_sum > 0.0 else 1.0
            return s
    ...
    def loadAnnotatedLabels(self, detailed=False, includeLowQC=True, infoType='label'):
        ...
        if self.KeyInFile('df_markers_expr', self.fileHDFpath):
            ...
            se = detailed or se.str.split(' #', expand=True)[0]
        else:
        ...
        if includeLowQC:
            if self.KeyInFile('df_projection_pre_QC', self.fileHDFpath):
                ...
                if self.KeyInFile('df_projection', self.fileHDFpath):
    ...
    def loadExpressionData(self):
        ...
        if self.KeyInFile('df_expr', self.fileHDFpath):
            ...
            self._df_expr = pd.read_hdf((self.fileHDFpath), key='df_expr').unstack(level=(-1), fill_value=0.0).T
    ...
    def prepareMarkers(self, expressedGenes=None, createColormapForCelltypes=True):
        ...
        df_marker_cell_type = df_marker_cell_type[df_marker_cell_type.columns[((df_marker_cell_type > 0.0).sum(axis=0) >= self.minimumNumberOfMarkersPerCelltype)]]
    ...
    def calculateQCmeasures(self):
        ...
        df_QC = pd.concat([self._df_expr.sum(axis=0),
         ...
         (self._df_expr.loc[self._df_expr.index.intersection(self.mitochondrialGenes)] > 0).sum(axis=0) / (self._df_expr > 0).sum(axis=0)],
          axis=1, sort=False)
        df_QC.columns = [
         'count_depth', 'number_of_genes', 'fraction_of_mitochondrialGenes']
    ...
    def batchEffectCorrection(self, method='COMBAT'):
        ...
        if method == 'COMBAT':
            ...
            if len(np.unique(patients)) == 1:
                ...
                self._df_expr[where_zeros] = 0.0
                print('Setting negative values to zeros', flush=True)
                self._df_expr[self._df_expr < 0.0] = 0.0
        else:
