...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.4/dist-packages/quickcut/__init__.py
# Compiled at: 2015-09-13 16:55:53
# Size of source mod 2**32: 10075 bytes
import os, subprocess, sys, shutil, pkg_resources, datetime as dt
...
def duration_str(h_m_s_start: [
 int, int, int], h_m_s_stop: [int, int, int]):
...
class Main(QtWidgets.QWidget):
    ...
    def doit_controller(self, *args, **kw):
        ...
        self.ok_btn.setEnabled((ok(self.video_pick) or ok(self.subtitle_pick)) and ok(self.start) and ok(self.stop) and ok(self.save_pick))
    ...
    def do_it(self):
        ...
        if video_ret == 0:
            ...
            if vid_out and os.path.isfile(vid_out):
                ...
                if sbt_out and os.path.isfile(sbt_out):
                    f = sbt_out
                else:
                    msg = ''
    ...
    def cut_subtitle(self):
        ...
        if os.path.isfile(sbt_in):
            ...
            subs = pysrt.open(sbt_in, error_handling=pysrt.ERROR_LOG)
            print('Decoded {} with {} items'.format(sbt_out, len(subs)))
            part = subs.slice(starts_after={'hours': h1,  'minutes': m1,  'seconds': s1}, ends_before={'hours': h2,  'minutes': m2,  'seconds': s2})
