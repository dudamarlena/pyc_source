...
# Embedded file name: /Users/grey/.pyenv/versions/cm/lib/python2.7/site-packages/cloudmesh_client/shell/plugins/SecgroupCommand.py
...
class SecgroupCommand(PluginCommand, CloudPluginCommand):
    ...
    def _delete(self, arg):
        if arg.cloud is None:
            ...
            if result is not None:
                Console.ok(('Security Group={GROUP} in cloud={cloud} deleted successfully.').format(**arg))
            else:
                Console.error(('Failed to delete Security Group={GROUP} in cloud={cloud}').format(**arg))
        return
    ...
    def do_secgroup(self, args, arguments):
        ...
        if arguments['list']:
            ...
            try:
                SecGroup.add_rule_to_db(name=arg.RULE, group=arg.GROUP, from_port=arg.FROMPORT, to_port=arg.TOPORT, protocol=arg.PROTOCOL, cidr=arg.CIDR)
            except:
                Console.error('Problem adding security group to db')

        elif arguments['delete']:
            if arg['RULE'] is not None:
                SecGroup.delete_rule_from_db(group=arg['GROUP'], name=arg['RULE'])
            else:
                self._delete(arg)
