...
# Embedded file name: /Users/grey/.pyenv/versions/cm/lib/python2.7/site-packages/cloudmesh_client/deployer/ansible/playbook.py
# Compiled at: 2017-04-23 10:30:41
import json, operator, tempfile, yaml
...
class AnsiblePlaybook(object):
    ...
    def __init__(self, inventory, roles=None, path=None, username=None, become=False, become_user=None, forks=None, modifyKnownHosts=True, extra_vars=None, subprocess_kwargs=None):
        ...
        playbook = [ {'name': role.name, 'hosts': role.hosts, 'become': role.become, 'roles': [dict(role=role.path, **role.variables)]} for role in roles
                   ]
    ...
    def __call__(self):
        cmd_builder_monoid = [
         [
          'ansible-playbook'],
         ['--inventory', self._inventory_file.name] if self._inventory else [],
         ...
         [
          self._modifyKnownHosts or '--ssh-extra-args', '-o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no'] if 1 else [][('--extra-vars', json.dumps(self._extra_vars))] if self._extra_vars else [],
         [
