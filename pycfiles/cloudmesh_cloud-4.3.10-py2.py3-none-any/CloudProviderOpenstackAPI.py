...
# Embedded file name: /Users/grey/.pyenv/versions/cm/lib/python2.7/site-packages/cloudmesh_client/cloud/iaas/provider/openstack/CloudProviderOpenstackAPI.py
...
class CloudProviderOpenstackAPI(CloudProviderBase):
    ...
    def list_clouds(self):
        ...
        for i, cloud in enumerate(clouds):
            if cloud in CloudProviderOpenstackAPI.cloud_pwd:
                cloud_dict[i] = {'cloud': cloud, 'status': CloudProviderOpenstackAPI.cloud_pwd[cloud]['status']}
            else:
                cloud_dict[i] = {'cloud': cloud, 'status': 'Logged Out'}
    ...
    def initialize(self, cloudname, user=None):
        ...
        if os_password.lower() in ('readline', 'read', 'tbd'):
            if cloudname in CloudProviderOpenstackAPI.cloud_pwd and 'pwd' in CloudProviderOpenstackAPI.cloud_pwd[cloudname]:
                os_password = CloudProviderOpenstackAPI.cloud_pwd[cloudname]['pwd']
            else:
                os_password = getpass.getpass(prompt=prompt)
        elif os_password.lower() == 'env':
        ...
        CloudProviderOpenstackAPI.cloud_pwd[cloudname]['status'] = 'Active'
        if 'v2.0' == ksversion:
            kws = dict(username=credentials['OS_USERNAME'], api_key=os_password, project_id=credentials['OS_TENANT_NAME'], auth_url=credentials['OS_AUTH_URL'])
            if cert:
                kws['cacert'] = cert
            ...
            sess = session.Session(auth=self._ksv3_auth(credentials), verify=cert)
        ...
        for id in groups:
            ...
            for rule in group['rules']:
                if rule['ip_protocol'] is not None:
                    element = {'fromPort': rule['from_port'], 
                       'toPort': rule['to_port'], 
                       'group': group['name'], 
    ...
    def create_secgroup(self, secgroup_name):
        secgroup = self.provider.security_groups.create(name=secgroup_name, description=('Security group {}').format(secgroup_name))
        return secgroup

    def add_secgroup_rule(self, **kwargs):
        rule_id = self.provider.security_group_rules.create(kwargs['uuid'], ip_protocol=kwargs['protocol'], from_port=kwargs['from_port'], to_port=kwargs['to_port'], cidr=kwargs['cidr'])
        return rule_id
    ...
    def delete_secgroup(self, name):
        ...
        if groups is not None:
            ...
            print ('Could not find security group [{}] in cloud [{}]').format(name, self.cloud)
        return ret
    ...
    def boot_vm(self, name, group=None, image=None, flavor=None, cloud=None, key=None, secgroup=None, meta=None, nics=None, **kwargs):
        ...
        if cloud is None:
            ...
            if nics is None or len(nics) == 0 or len(nics) == 1 and nics[0]['net-id'] is None:
                netnames = [
                 ('{0}-net').format(self.tenant), 'int-net']
            ...
            id = None
            try:
                server = self.provider.servers.create(name, image_id, flavor_id, meta=meta, key_name=key, security_groups=secgroup, nics=nics)
                id = server.__dict__['id']
                return id
            except Exception as e:
                if 'Invalid key_name provided.' in str(e):
                    Console.error('Invalid key provided. Is the key loaded, the default key set properly and uploaded to the cloud?', traceflag=False)
                else:
    ...
    def get_vm(self, **kwargs):
        ...
        if self.isUuid(vm_name):
            ...
            return self.provider.servers.find(name=vm_name, scope='first')._info
