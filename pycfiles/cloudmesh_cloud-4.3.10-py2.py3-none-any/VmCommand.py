...
# Embedded file name: /Users/grey/.pyenv/versions/cm/lib/python2.7/site-packages/cloudmesh_client/shell/plugins/VmCommand.py
# Compiled at: 2017-04-23 10:30:41
from __future__ import print_function
import json, os, getpass, socket, pyaml, time
    ...
    def do_vm(self, args, arguments):
        ...
        if arguments['boot']:
            ...
            for index in range(0, arg.count):
                vm_details = dotdict({'cloud': arg.cloud, 
                ...
                try:
                    if arg.dryrun:
                        ...
                        if vm_id is not None:
                            Group.add(name=vm_details.group, species='vm', member=vm_details.name, category=vm_details.cloud)
                        msg = 'info. OK.'
                        Console.ok(msg)
                except Exception as e:
            ...
            try:
                ...
                msg = 'info. OK.'
                Console.ok(msg)
                ValueError('default command not implemented properly. Upon first install the defaults should be read from yaml.')
            except Exception as e:
            ...
            try:
                ...
                pprint(test)
                print(Printer.write(test, order=[
                 'id',
                 'name',
                 'status',
                 'ip',
                 'ping',
                 'login'], output='table', sort_keys=True))
            ...
            if arguments['delete']:
                ...
                return ''
            if arguments['ip'] and arguments['assign']:
                ...
                for name in names:
                    ...
                    if vm.floating_ip is None:
                        ...
                        try:
                            floating_ip = Network.find_assign_floating_ip(cloudname=cloud, instance_id=name)
                            Vm.refresh(cloud=cloud)
                            if floating_ip is not None:
                                print(('Floating IP assigned to {:} is {:}').format(name, floating_ip))
                                msg = 'info. OK.'
                                Console.ok(msg)
                        except Exception as e:
                ...
                if chameleon:
                    ...
                    data = dotdict({'name': arguments['NAME'] or Default.vm, 
                       'username': arg.username, 
                       'cloud': arg.cloud, 
                       'command': arg.command})
                    ...
                    try:
                        ...
                        for cloud in clouds:
                            ...
                            if len(result) > 0:
                                ...
                                print(Printer.write(result, order=order, output=_format))
                            else:
                ...
                try:
                    ...
                    if oldnames is None or newnames is None:
                        ...
                        Console.error('The number of VMs to be renamed is wrong', traceflat=False)
                    else:
                        for i in range(0, len(oldnames)):
                            ...
                            if arguments['--dryrun']:
                                ...
                                Vm.rename(cloud=cloud, oldname=oldname, newname=newname, force=force)
