...
# Embedded file name: /Users/grey/.pyenv/versions/cm/lib/python2.7/site-packages/cloudmesh_client/shell/plugins/HadoopCommand.py
...
class Command(object):
    ...
    def start(self, count=3, addons=None, flavor=None, image=None, username=None):
        ...
        cluster = clustercmd.create(count=count, username=username, flavor=flavor, image=image)
        stackcmd = StackCommand()
        addons = addons or []
        playbooks = ['play-hadoop.yml'] + [ os.path.join('addons', name) for name in addons ]
        stackcmd.init(username=username, playbooks=playbooks, ips=clustercmd.get('floating_ip', cluster=cluster))
        try:
            stackcmd.deploy()
...
class HadoopCommand(PluginCommand, CloudPluginCommand):
    ...
    def do_hadoop(self, arg, arguments):
        ...
        cmd = Command()
        if arguments.start:
            cmd.start(count=arguments.COUNT or 3, addons=arguments.ADDON, flavor=arguments['--flavor'], image=arguments['--image'], username=arguments['--username'])
        elif arguments.list:
            cmd.list()
