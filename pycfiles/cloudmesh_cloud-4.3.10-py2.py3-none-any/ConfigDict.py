...
# Embedded file name: /Users/grey/.pyenv/versions/cm/lib/python2.7/site-packages/cloudmesh_client/common/ConfigDict.py
...
def dprint(OD, mode='dict', s='', indent='    ', level=0):
    ...
    for i, k in enumerate(OD.keys()):
        if type(OD[k]) in [dict, OrderedDict]:
            level += 1
            s += (level - 1) * indent + kv_tpl % (k,
             ST + dprint(OD[k], mode=mode, indent=indent, level=level) + (level - 1) * indent + END)
            level -= 1
        else:
...
class ConfigDict(object):
    ...
    def __init__(self, filename, load_order=None, verbose=False, etc=False, reload=False):
        """
    ...
    def write(self, filename=None, output='dict', attribute_indent=4):
        ...
        f = os.open(location, os.O_CREAT | os.O_TRUNC | os.O_WRONLY, stat.S_IRUSR | stat.S_IWUSR)
        if output == 'json':
            os.write(f, self.json())
        elif output in ('yml', 'yaml'):
            os.write(f, ordered_dump(OrderedDict(self), Dumper=yaml.SafeDumper, default_flow_style=False, indent=attribute_indent))
        elif output == 'print':
    ...
    def getUser(cls, cloud):
        try:
            ...
            raise ValueError(('getUser for this cloud type not yet supported: {}').format(cloud))
