...
# Embedded file name: /Users/grey/.pyenv/versions/cm/lib/python2.7/site-packages/cloudmesh_client/cloud/secgroup.py
...
class SecGroup(ListResource):
    ...
    def add_rule_to_db(cls, group=None, name=None, from_port=None, to_port=None, protocol=None, cidr=None):
        ...
        try:
            rule = {'category': 'general', 'kind': 'secgrouprule', 
               'group': group, 
               'name': name, 
               'protocol': protocol, 
               'fromPort': from_port, 
    ...
    @classmethod
    def list(cls, group=None, name=None, category='general', output='table', scope='all'):
        """
        ...
        :return:
        """
        query = dotdict({'kind': 'secgrouprule', 
           'scope': 'all'})
    ...
    def list_rules(cls, group=None, output='table'):
        ...
        try:
            ...
            return Printer.write(rules, order=[
             'user',
             'group',
             'category',
             'name',
        ...
        if cloud in LibcloudDict.Libcloud_category_list:
            ...
            for asecgroup in secgroups:
                if asecgroup.name == secgroup_name:
                    ...
                    rule_exists = False
                    for arule in rules:
                        if arule['from_port'] == 22 and arule['to_port'] == 22 and arule['ip_protocol'] == 'tcp' and arule['ip_range'] == {'cidr': '0.0.0.0/0'}:
                            rule_exists = True
                            break

                    if not rule_exists:
                        cloud_provider.security_group_rules.create(asecgroup.id, ip_protocol='tcp', from_port=22, to_port=22, cidr='0.0.0.0/0')
                    ret = True
                    break
    ...
    def add_rule(cls, cloud, secgroup_uuid, from_port, to_port, protocol, cidr):
        try:
            cloud_provider = CloudProvider(cloud).provider
            args = {'uuid': secgroup_uuid, 
               'protocol': protocol, 
               'from_port': from_port, 
               'to_port': to_port, 
               'cidr': cidr}
            rule_id = cloud_provider.add_secgroup_rule(**args)
            rule = {'kind': 'secgrouprule', 'uuid': str(rule_id), 
               'category': cloud, 
               'fromPort': from_port, 
               'toPort': to_port, 
               'protocol': protocol, 
               'cidr': cidr}
            Console.ok(('Added rule {category} {uuid} {fromPort} {toPort} {protocol} {cidr}').format(**rule))
        except Exception as ex:
    ...
    def reset_defaults(cls):
        ...
        SecGroup.add_rule_to_db(group=secgroup, name='icmp', from_port='-1', to_port='-1', protocol='icmp', cidr='0.0.0.0/0')

    @classmethod
    def delete(cls, category='general', group=None, name=None):
        if category == 'general':
    ...
    def delete_rule(cls, cloud, secgroup, from_port, to_port, protocol, cidr):
        try:
            args = {'group': secgroup['uuid'], 'fromPort': from_port, 
               'toPort': to_port, 
               'protocol': protocol, 
               'cidr': cidr}
            rule = cls.cm.find(kind='secgrouprule', output='object', scope='first', **args)
            if rule is not None:
                cloud_provider = CloudProvider(cloud).provider
                cloud_provider.delete_secgroup_rule(rule.uuid)
                cls.cm.delete(rule)
                return ('Rule [{fromPort} | {toPort} | {protocol} | {cidr}] deleted').format(**args)
            return
        except Exception as ex:
    ...
    def delete_all_rules(cls, secgroup):
        try:
            args = {'group': secgroup['uuid']}
            rules = cls.cm.find(kind='secgrouprule', output='object', **args)
            if rules is not None:
                for rule in rules:
                    cls.cm.delete(rule)
                    Console.ok(('Rule [{fromPort} | {toPort} | {protocol} | {cidr}] deleted').format(**rule))
    ...
    def add_rule_cloud(cls, cloud, groupname, rulename):
        ret = None
        provider = CloudProvider(cloud).provider
        db_rule = cls.cm.find(kind='secgrouprule', category='general', group=groupname, name=rulename, scope='first', output='dict')
        kwargs = {}
    ...
    def get_rule_cloud(cls, cloud, groupname, rulename):
        ...
        for rule in rules:
            if 'cidr' in rule['ip_range']:
                if db_rule['fromPort'] == str(rule['from_port']) and db_rule['toPort'] == str(rule['to_port']) and db_rule['protocol'] == rule['ip_protocol'] and db_rule['cidr'] == rule['ip_range']['cidr']:
                    ruleid = rule['id']
