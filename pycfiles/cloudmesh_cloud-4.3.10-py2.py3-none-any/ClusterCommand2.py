...
# Embedded file name: /Users/grey/.pyenv/versions/cm/lib/python2.7/site-packages/cloudmesh_client/shell/plugins/ClusterCommand2.py
...
from cloudmesh_client.exc import ClusterNameClashException, NoActiveClusterException, UnrecoverableErrorException
...
class Command(object):
    ...
    def create(self, clustername=None, cloud=None, count=1, username=None, image=None, flavor=None, key=None, secgroup=None, assignFloatingIP=True, activate=True):
        """Create a cluster.
        ...
        try:
            cluster = Cluster(name=clustername, count=count, cloud=cloud, username=username, image=image, flavor=flavor, key=key, secgroup=secgroup, assignFloatingIP=assignFloatingIP)
...
class Cluster2Command(PluginCommand, CloudPluginCommand):
    ...
    def do_cluster2(self, args, arguments):
        ...
        cmd = Command()
        if arguments.create:
            cmd.create(clustername=arguments['--name'], count=arguments['--count'] or 1, cloud=arguments['--cloud'] or Default.cloud, username=arguments['--username'], image=arguments['--image'] or Default.image, flavor=arguments['--flavor'] or Default.flavor, key=arguments['--key'] or Default.key, secgroup=arguments['--secgroup'] or Default.secgroup, assignFloatingIP=not arguments['--no-floating-ip'])
        elif arguments.list:
            try:
            ...
            cluster = db.select(Cluster, name=arguments.CLUSTER).one() if arguments.CLUSTER else None
            nodes = cmd.nodes(cluster=cluster)
            for node in nodes:
                print(node.name)

        elif arguments.delete:
            cmd.delete(arguments.NAME, force=arguments['--force'], all=arguments['--all'])
        elif arguments['get']:
            ...
            cmd.inventory(cluster=arguments.NAME, format=arguments['--format'], path=arguments['--path'])
        return


if __name__ == '__main__':
    cmd = Cluster2Command()
    cmd.create(clustername='test3', image='CC-Ubuntu14.04', flavor='m1.medium', key='gambit', count=3)