...
# Embedded file name: /Users/grey/.pyenv/versions/cm/lib/python2.7/site-packages/cloudmesh_client/cloud/iaas/provider/azure/CloudProviderAzureAPI.py
...
class CloudProviderAzureAPI(CloudProviderBase):
    ...
    def add_certificate(self, service_name, certificate_path):
        ...
        with open(certificate_path, 'rb') as (bfile):
            ...
            cert_format = 'pfx'
            cert_password = ''
            cert_res = self.provider.add_service_certificate(service_name=service_name, data=cert_data, certificate_format=cert_format, password=cert_password)
            print(cert_res)
    ...
    def _create_storage_account(self):
        ...
        label = name + 'label'
        result = self.provider.create_storage_account(service_name=name, description=description, label=label, location=self.get_location())
    ...
    def boot_vm(self, name, group=None, image=None, flavor=None, cloud=None, cert_thumbprint=None, pub_key_path=None, cert_path=None, pfx_path=None, secgroup=None, meta=None, nics=None, **kwargs):
        """
        ...
        try:
            self.provider.create_hosted_service(service_name=name, label=name, location=location)
        except:
        ...
        if storage_name is None:
            ...
            storage_name = self._get_storage_name()
        media_link = ('https://{0}.blob.core.windows.net/vhds/{1}.vhd').format(storage_name, name)
        ...
        try:
            vm_create_result = self.provider.create_virtual_machine_deployment(service_name=name, deployment_name=name, deployment_slot='production', label=name, role_name=name, system_config=linux_config, os_virtual_hard_disk=os_hd, network_config=network, role_size=flavor)
    ...
    def delete_vm(self, name, group=None, force=None):
        ...
        :return:
        """
        self.provider.delete_deployment(service_name=name, deployment_name=name)
        import time
        time.sleep(10)
