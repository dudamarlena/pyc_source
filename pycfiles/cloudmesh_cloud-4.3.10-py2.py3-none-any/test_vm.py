...
# Embedded file name: /Users/grey/.pyenv/versions/cm/lib/python2.7/site-packages/tests/cm_cloud/test_vm.py
...
class Test_vm:
    data = dotdict({'wrong_cloud': 'no_cloud', 
       'cloud': Default.cloud, 
       'group': 'test', 
    ...
    def test_001(self):
        ...
        self.load_key()
        command = 'cm vm boot --name={vm} --cloud={cloud} --image={image}' + ' --flavor={flavor} --group={group}'
        result = self.run(command)
        assert 'OK.' in result
