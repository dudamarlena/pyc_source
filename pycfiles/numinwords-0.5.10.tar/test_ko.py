...
# [GCC 8.4.0]
# Embedded file name: /Users/mmonroy/Desktop/data/num2words/tests/test_ko.py
# Compiled at: 2020-04-17 01:13:28
from __future__ import division, print_function, unicode_literals
from unittest import TestCase
from numinwords import numinwords

def n2k(*args, **kwargs):
    return numinwords(lang=b'ko', *args, **kwargs)


class numinwordsKOTest(TestCase):
    ...
    def test_year(self):
        ...
        for num, out in cases:
            self.assertEqual(n2k(num, to=b'year'), out)
    ...
    def test_currency(self):
        ...
        for num, out in cases_krw:
            self.assertEqual(n2k(num, to=b'currency'), out)

        for num, out in cases_usd:
            self.assertEqual(n2k(num, to=b'currency', currency=b'USD'), out)

        for num, out in cases_jpy:
            self.assertEqual(n2k(num, to=b'currency', currency=b'JPY'), out)

        with self.assertRaises(ValueError):
            n2k(190.55, to=b'currency')
        with self.assertRaises(NotImplementedError):
            n2k(4, to=b'currency', currency=b'EUR')

    def test_ordinal(self):
        cases = [(1, '첫 번째'), (101, '백 한 번째'), (2, '두 번째'), (5, '다섯 번째'),
         (10, '열 번째'), (25, '스물다섯 번째'), (137, '백 서른일곱 번째')]
        for num, out in cases:
            self.assertEqual(n2k(num, to=b'ordinal'), out)

    def test_ordinal_num(self):
        cases = [(1, '1 번째'), (101, '101 번째'), (25, '25 번째')]
        for num, out in cases:
            self.assertEqual(n2k(num, to=b'ordinal_num'), out)