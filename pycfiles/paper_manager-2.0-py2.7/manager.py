...
# [GCC 8.4.0]
# Embedded file name: /home/cer/Project/pycharm/paper_manager/paper_manager/manager.py
# Compiled at: 2018-04-14 06:06:08
from __future__ import print_function
import os
from paper_manager.color import Colored, colors
from paper_manager.repository import Repository
import pickle as pkl, sqlite3
...
class Manager:
    ...
    def select_repository(self):
        ...
        while True:
            if len(list(rep_dict.keys())) != 0:
                print('Choose one of the repositories below or add a new repository. (first repository as default, if a new repository is needed, input a new name)')
            ...
            if rep_name in rep_dict:
                ...
                print("It's a new repository name, input the path of this repository.(current dir as default, re-input repository name input 'back')")
                rep_path = input().strip()
                if rep_path.strip() == 'back':
                    continue
                if rep_path == '':
                    rep_path = '.'
                print("Input support suffix of this repository, seperate by space, lisk(pdf mobi). (pdf as default, re-input repository name input 'back')")
    ...
    def create_a_new_table_for_repository(self, rep_name):
        sql_create = ('create table if not exists {} ( paper_name varchar(100) , importance integer, urgency integer, tags varchar(100), path varchar(100), read varchar(10), date TEXT, id integer primary key autoincrement)').format(rep_name)
    ...
    def insert_one(self, paper_name, paper_im, paper_ug, paper_tags, read):
        self.cursor.execute(('INSERT INTO {} (paper_name, importance, urgency, tags, path, read, date) VALUES (?, ?, ?, ?, ?, ?, ?)').format(self.cur_rep.name), (paper_name, paper_im,
         paper_ug, paper_tags,
    ...
    def update_one(self, paper_name, paper_im, paper_ug, paper_tags, read):
        self.cursor.execute(('UPDATE {} SET importance=?, urgency=?, tags=?, path=?, read=?, date=? WHERE paper_name=?').format(self.cur_rep.name), (
    ...
    def prettify_one(self, rec):
        one_row = [
         self.color.blue_yellow(str(rec[7])),
    ...
    def recommend_papers(self):
        ...
        rec_papers = self.cursor.execute(("SELECT * FROM {} WHERE importance!='' AND urgency!='' AND read='n' ORDER BY urgency DESC , importance DESC LIMIT 5 ").format(self.cur_rep.name)).fetchall()
        if len(rec_papers) > 0:
    ...
    def query_by_tags(self, tags_s):
        ...
        if len(tags) > 0:
            for tag in tags:
                recs = self.cursor.execute(("select * from {} where tags like '%{}%'").format(self.cur_rep.name, tag)).fetchall()
                if len(recs) > 0:
                    res_set = set()
                    for rec in recs:
                        res_set.add(rec)
    ...
    def get_on_paper_info_from_user(self):
        paper_importance = input(self.color.red('Please input the importance of this paper (from 1 to 5, 3 as default):')).strip()
        if paper_importance == '':
            paper_importance = '3'
        paper_urgency = input(self.color.yellow('Please input the urgency of this paper (from 1 to 5, 3 as default):')).strip()
        if paper_urgency == '':
            paper_urgency = '3'
        paper_tags = input(self.color.blue('Please input the tags of this paper (split by space):')).strip()
        read = input(self.color.magenta('Is this paper has been read? (y/n, n as default): ')).strip()
        if read == '':
            read = 'n'
        print()
