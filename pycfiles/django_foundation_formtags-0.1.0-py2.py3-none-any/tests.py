...
# [GCC 8.4.0]
# Embedded file name: /Users/cclarke/Dropbox/Development/django-foundation-formtags/foundation_formtags/tests/tests.py
# Compiled at: 2016-09-25 19:50:31
...
class TestFoundationform(unittest.TestCase):
    ...
    def filter_or_tag_test(self, template, context, contains):
        tpl = engines['django'].from_string('{% load foundation_formtags %}' + template)
    ...
    def test_as_foundation_filter_complex(self):
        data = {'char_field': 'Tests', 'password_field': 'password', 'choice_field': 'Option 1', 
           'boolean_field': 'True'}
    ...
    def test_render_field_tag_simple(self):
        data = {'text': 'Tests'}
        simple_form = SimpleForm(data)
        template = '<form class="form" action="" method="POST">                         {% csrf_token %}                         {% render_field form.text %}                     </form>'
        context = {'form': simple_form}
        contains = 'id="id_text"'
        self.filter_or_tag_test(template, context, contains)
    ...
    def test_render_field_tag_complex(self):
        data = {'char_field': 'Tests', 'password_field': 'password', 'choice_field': 'Option 1', 
           'boolean_field': 'True'}
        complex_form = ComplexForm(data)
        template = '<form class="form" action="" method="POST">                         {% csrf_token %}                         {% render_field form.char_field %}                         {% render_field form.password_field %}                     </form>'
        context = {'form': complex_form}
