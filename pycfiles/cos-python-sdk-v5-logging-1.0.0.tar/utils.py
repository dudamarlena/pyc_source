...
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/COS/utils.py
# Compiled at: 2015-06-11 16:01:01
__doc__ = ' Tools for reading 1D spectra extracted from the CalCOS pipeline,\nfor use in co-addition, and for reading and writing the line spread functions.\n\n'
...
__all__ = [
 'read_x1d', 'read_lsf', 'limiting_equivalent_width',
 'significance_level']
datapath = os.path.split(os.path.abspath(__file__))[0] + '/'
dw_orig = dict(G130M=0.00997, G160M=0.01223, G140L=0.083, G185M=0.034, G225M=0.034, G285M=0.04, G230L=0.39)
c_kms = c.to(km / s)
cache_lsf = {}
...
def read_x1d(filename):
    ...
    for i, segment in enumerate(data['SEGMENT']):
        ...
        x1dspec[segment] = COSx1dSpectrum(data[i]['WAVELENGTH'], data[i]['FLUX'], data[i]['ERROR'], data[i]['DQ'], data[i]['GROSS'], data[i]['BACKGROUND'], data[i]['NET'], this_header, meta)
...
def cross_correlate(s1, s2, ishift=0, width=15, s1_start=None, s1_end=None):
    ...
    """
    approx = np.round(ishift)
    ns = len(s1)
    if s1_start is None:
        s1_start = 0
    if s1_end is None:
        s1_end = ns - 1
    s2_start = s1_start - approx + width / 2 if s1_start - approx + width / 2 > 0 else 0
    s2_end = s1_end - approx - width / 2 if s1_end - approx - width / 2 < ns - 1 else ns - 1
    nt = s2_end - s2_start + 1
    if nt < 1.0:
        raise ValueError('cross correlation region too small, or width/ishift too large')
    template2 = s2[s2_start:s2_end + 1]
    corr = np.zeros(width)
    ...
    if k == 0 or k == width - 1.0:
        print 'WARNING: correlation maximum on edge of search area, returning zero offset'
        offset = 0
        return (
         offset, corr)
    else:
...
def scale_factor(wavelength1, flux1, error1, wavelength2, flux2, error2):
    ...
    good1 = (error1 > 0) & ~np.isnan(flux1) & (wmin < wavelength1) & (wavelength1 < wmax)
    good2 = (error2 > 0) & ~np.isnan(flux2) & (wmin < wavelength2) & (wavelength2 < wmax)
    ...
    if not median1 > 0 or not median2 > 0:
        print (
         'Bad medians: ', str(median1), str(median2),
...
class LSF(object):
    ...
    def __init__(self, grating):
        name = 'NUV' if grating in ('G185M', 'G225M', 'G285M', 'G230L') else grating
    ...
    def interpolate(self, dw):
        ...
        if key in cache_lsf.keys():
            ...
            new_lsf = []
            for w in wavs:
                new_lsf.append(interp1d(self.dispersion, self.lsf[w], kind='cubic')(new_disp))
...
def limiting_equivalent_width(significance, wavelength, b, snpix, dispersion=None, smoothing=1):
