...
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/COS/spectrum.py
# Compiled at: 2015-05-23 06:17:39
__doc__ = ' Methods for spectra extracted from the CalCOS pipeline\n\n'
...
class COSx1dSpectrum(object):
    ...
    def __init__(self, wavelength, flux, error, data_quality, gross, background, net, header, meta):
        if wavelength.ndim != 1 or wavelength.shape != flux.shape:
            raise ValueError('wavelength and flux need to be one-dimensional arrays with the same shape')
        flags = dict(data_quality=data_quality, weights=np.ones_like(wavelength))
    ...
    def smooth_background(self, smoothing_length, check=False):
        ...
        while decision != 'y':
            ...
            smoothed_background[~mask] = smooth(self.background.value[(~mask)], smoothing_length)
            if len(smoothed_background[(~mask)]) != 0:
                smoothed_background = np.interp(self.wavelength, self.wavelength[(~mask)], smoothed_background[(~mask)])
            ...
            if check:
                ...
                pl.plot(self.wavelength.value, self.background.value, color='k', label='unsmoothed')
                pl.plot(self.wavelength.value, smoothed_background, color='r', label='smoothed')
                ...
                if decision == 'y':
                    ...
                    smoothing_length = int(raw_input('Enter new smoothing length : '))
            else:
                decision = 'y'
                self.background = smoothed_background / s

    def interpolate(self, wavelength, kind='nearest', bounds_error=False, fill_value=0):
        ...
        """
        flux = interp1d(self.wavelength, self.flux, kind=kind, bounds_error=bounds_error, fill_value=fill_value)(wavelength)
        error = interp1d(self.wavelength, self.error, kind=kind, bounds_error=bounds_error, fill_value=fill_value)(wavelength)
        gross = interp1d(self.wavelength, self.gross, kind=kind, bounds_error=bounds_error, fill_value=fill_value)(wavelength)
        background = interp1d(self.wavelength, self.background, kind=kind, bounds_error=bounds_error, fill_value=fill_value)(wavelength)
        net = interp1d(self.wavelength, self.net, kind=kind, bounds_error=bounds_error, fill_value=fill_value)(wavelength)
        data_quality = np.zeros_like(wavelength)
        return self.__class__(wavelength, flux, error, data_quality, gross, background, net, self.header, self.meta)
    ...
    def write(self, filename, overwrite=False):
        ...
        t = Table([wavelength, flux, error, gross, background, net, dq], meta=self.meta)
