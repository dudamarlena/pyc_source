...
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/COS/reduction.py
# Compiled at: 2015-07-03 08:51:23
__doc__ = ' For reducing raw COS data using the CalCOS pipeline.\n\n'
...
class RawData(object):
    ...
    def __init__(self, data_path='./', calibration_path='./'):
        ...
        if len(self.asn) == 0:
            raise AssertionError('No raw data products in the specified directory!')
        ...
        self.extraction = np.unique([ ('{0}/{1}').format(calibration_path, header['XTRACTAB'].split('$')[1]) for header in rawtag_headers
                                    ])
        ...
        self.info = Table([
         targname, rootname, detector, grating, cenwave, fppos, exptime], names=[
         'TARGNAME', 'ROOTNAME', 'DETECTOR', 'GRATING', 'CENWAVE',
         'FP-POS', 'EXPTIME'])
    ...
    def set_extraction_windows(self, G130M, G160M, G140L, NUV):
        ...
        for fh in self.extraction:
            ...
            g130m = (table['OPT_ELEM'] == 'G130M') & (table['APERTURE'] == 'PSA')
            g160m = (table['OPT_ELEM'] == 'G160M') & (table['APERTURE'] == 'PSA')
            g140l = (table['OPT_ELEM'] == 'G140L') & (table['APERTURE'] == 'PSA')
            nuv = (table['OPT_ELEM'] == 'G230L') & (table['APERTURE'] == 'PSA')
    ...
    def set_background_smoothing(self, background_smoothing_length):
        ...
        for fh in self.extraction:
            hdulist = fits.open(fh, mode='update')
            table = hdulist[1].data
            table['BWIDTH'][table['APERTURE'] == 'PSA'] = background_smoothing_length
            hdulist.flush()
            hdulist.close()
...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(description='Run the COS pipeline CalCOS using the data and parameters in a configuration file to be specified. To dump a default configuration file: runcalcos -d')
    ...
    g130m = (
     cfg['EXTRACTION'].pop('G130M_height'),
