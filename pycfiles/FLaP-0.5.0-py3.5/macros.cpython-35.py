...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\flap\latex\macros.py
# Compiled at: 2017-01-10 07:30:50
# Size of source mod 2**32: 14823 bytes
import re
...
class Macro:
    ...
    def __eq__(self, other):
        if not isinstance(other, Macro):
            return False
        return self._name == other._name and self._signature == other._signature and self._body == other._body
    ...
    def __repr__(self):
        ...
        return '\\def' + self._name + signature + body


class Begin(Macro):
    __doc__ = '\n    A LaTeX environment such as \x08egin{center} \\end{center}.\n    '
    ...
    def _execute(self, parser, invocation):
        ...
        if environment == 'verbatim':
            return parser._create.as_list('\\begin{verbatim}') + parser.capture_until_text('\\end{verbatim}')
        else:
            if environment == 'overpic':
...
class DocumentClass(Macro):
    ...
    def _execute(self, parser, invocation):
        ...
        if class_name == 'subfiles':
            parser.capture_until_text('\\begin{document}')
            document = parser.capture_until_text('\\end{document}')
...
class Def(Macro):
    ...
    def _execute(self, parser, invocation):
        macro = Macro(self._flap, ''.join(map(str, invocation.argument('name'))), invocation.argument('signature'), invocation.argument('body'))
        parser.define(macro)
        return []
...
class Include(TexFileInclusion):
    ...
    def _execute(self, parser, invocation):
        ...
        if self._flap.shall_include(link):
            ...
            return result + parser._create.as_list('\\clearpage')
        return []
...
class IncludeOnly(Macro):
    ...
    def __init__(self, flap):
        super().__init__(flap, '\\includeonly', None, None)
...
class Bibliography(UpdateLink):
    __doc__ = '\n    Intercept the `\x08ibliography` directive\n    '
...
class BibliographyStyle(UpdateLink):
    __doc__ = '\n    Intercept the `\x08ibliographystyle` directive\n    '
...
class GraphicsPath(Macro):
    ...
    def __init__(self, flap):
        super().__init__(flap, '\\graphicspath', None, None)
