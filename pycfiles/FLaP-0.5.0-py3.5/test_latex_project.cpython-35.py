...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\tests\test_latex_project.py
# Compiled at: 2016-11-09 09:07:56
# Size of source mod 2**32: 10351 bytes
...
class TexFileTest(TestCase):
    ...
    def test_does_not_equals_tex_file_with_a_different_path(self):
        self.assertNotEqual(TexFile('dir/' + self.path, self.content), self.tex_file)

    def test_does_not_equals_tex_file_with_a_different_content(self):
        self.assertNotEqual(TexFile(self.path, self.content + 'blablabla'), self.tex_file)
...
class LatexProjectTests(TestCase):
    ...
    def test_differ_when_file_content_differ(self):
        self.assertNotEqual(LatexProject(TexFile('main.tex', 'THIS IS DIFFERENT!')), self.tex)

    def test_differ_when_file_path_differ(self):
        self.assertNotEqual(LatexProject(TexFile('a/different/path.tex', 'blabla')), self.tex)
    ...
    def test_difference_with_a_project_with_an_missing_file(self):
        try:
            ...
            self.assertEqual(LatexProject.MISSING_FILE.format(file='main.tex'), str(error))
    ...
    def test_difference_with_a_project_with_an_extra_file(self):
        try:
            ...
            self.assertEqual(LatexProject.UNEXPECTED_FILE.format(file=extra_file.path), str(error))
    ...
    def test_difference_with_a_project_whose_file_content_differs(self):
        try:
            ...
            self.assertEqual(LatexProject.CONTENT_MISMATCH.format(file='main.tex', expected=content, actual='blabla'), str(error))
...
class LatexProjectGenerationTests(TestCase):
    ...
    def test_setup_a_two_files_project(self):
        self._do_test_setup(LatexProject(TexFile('main.tex', 'blabla'), TexFile('result.tex', 'Some results')))

    def test_setup_a_project_with_subdirectories(self):
        self._do_test_setup(LatexProject(TexFile('main.tex', 'blabla'), TexFile('sections/introduction.tex', 'introduction'), TexFile('sections/conclusions.tex', 'conclusions'), TexFile('images/results.pdf', 'PDF')))
...
class LatexProjectExtractionTests(TestCase):
    ...
    def test_extracting_a_two_files_project(self):
        self._do_test_with_files(('main.tex', 'content'), ('result.tex', 'Here are some results'))

    def test_extracting_files_in_a_subdirectory(self):
        self._do_test_with_files(('main.tex', 'content'), ('test/result.tex', 'Here are some results'))

    def test_extracting_a_complete_project(self):
        self._do_test_with_files(('main.tex', 'blablabla'), ('sections/introduction.tex',
                                                             'Here are some results'), ('sections/development.tex',
                                                                                        'Some more details'), ('sections/conclusions.tex',
                                                                                                               'Some more hindsight'), ('images/results.pdf',
                                                                                                                                        'PDF CONTENT'), ('images/sources/results.svg',
                                                                                                                                                         'SVG CODE'), ('article.bib',
                                                                                                                                                                       'The bibliography'))
