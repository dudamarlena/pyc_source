...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\tests\latex_project.py
# Compiled at: 2016-12-19 09:19:30
...
class LatexProject:
    ...
    def _verify_different_files(self, expectation):
        for path, file in self.files.items():
            if not (path in expectation.files and file == expectation.files[path]):
                raise AssertionError(self.CONTENT_MISMATCH.format(file=path, expected=expectation.files[path].content, actual=file.content))

    CONTENT_MISMATCH = "Content mismatch for {file}\nExpected:\n'{expected}'\nbut found:\n'{actual}'"

    def _verify_missing_files(self, other):
        for path in self.files:
            if not path in other.files:
                raise AssertionError(self.MISSING_FILE.format(file=path))
    ...
    def _verify_extraneous_files(self, other):
        for path in other.files:
            if not path in self.files:
                raise AssertionError(self.UNEXPECTED_FILE.format(file=path))
...
class Fragment:
    ...
    def __eq__(self, other):
        if not isinstance(other, Fragment):
            return False
        return self._file_name == other._file_name and self._line == other._line and self._column == other._column and self._code == other._code
...
class FlapTestCase:
    ...
    def __eq__(self, other):
        if not isinstance(other, FlapTestCase):
            return False
        return self._name == other._name and self._project == other._project and self._expected == other._expected and self._is_skipped == other._is_skipped and self._output == other._output