...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\flap\util\releasing.py
# Compiled at: 2016-11-01 04:08:53
...
class Version:
    ...
    def update_source_code(version):
        ...
        new_content = re.sub('__version__\\s*=\\s*\\"\\d+\\.\\d+\\.\\d+\\"', replacement, content)
    ...
    def __eq__(self, other):
        return other and isinstance(other, Version) and other.micro == self.micro and other.has_major(self.major) and other.has_minor(self.minor)
...
class SourceControl:
    ...
    def tag(self, version):
        command = [
         'git.exe', 'tag', '-a', 'v' + str(version), '-m', '"Version %s"' % str(version)]
        subprocess.call(command, env=self.environment, shell=True)
...
class Release(Command):
    ...
    user_options = [
     ('type=', None, 'The type of release (micro, minor or major')]
