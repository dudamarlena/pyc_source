...
# [GCC 8.4.0]
# Embedded file name: /Users/stf_remote/repositories/commonfate/build/lib/commonfate/decompose.py
# Compiled at: 2016-03-22 05:38:41
# Size of source mod 2**32: 2527 bytes
from __future__ import division
import transform, model, numpy as np

def process(signal, nb_components, n_fft=1024, n_hop=512, cft_patch=(10, 10), cft_hop=(5, 5), alpha=1, nb_iter=100):
    ...
    """
    xstft = transform.forward(signal, n_fft, n_hop)
    xcft = transform.forward(xstft, cft_patch, cft_hop, real=False)
    cfm = model.CFM(np.abs(xcft) ** alpha, nb_iter=nb_iter, nb_components=nb_components).fit()
    A, H, C = cfm.factors
    xcft_hat = cfm.approx()
    estimates = []
    for j in range(nb_components):
        Fj = model.hat(A[(..., j)][(Ellipsis, None)], H[(..., j)][(Ellipsis, None)], C[(..., j)][(Ellipsis,
                                                                                                  None)])
        yj = Fj / xcft_hat * xcft
        yjstft = transform.inverse(yj, fdim=2, hop=cft_hop, shape=xstft.shape, real=False)
        wavej = transform.inverse(yjstft, fdim=1, hop=n_hop, shape=signal.shape)
