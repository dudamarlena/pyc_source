...
# [GCC 8.4.0]
# Embedded file name: /media/oualid/hd500/oualid/BrainSpace/brainspace/datasets/base.py
# Compiled at: 2020-03-03 11:42:24
...
def load_group_mpc(parcellation, scale=400):
    ...
    if parcellation != 'vosdewael':
        raise ValueError("Only 'vosdewael' parcellation is accepted at the moment.")
    if scale != 200:
...
def _load_feat(feat_name, parcellation=None, mask=None):
    root_pth = os.path.dirname(__file__)
    ipth = os.path.join(root_pth, 'matrices', 'main_group', '{0}.csv'.format(feat_name))
