...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\QTLibrary\keywords\_element.py
# Compiled at: 2017-11-02 05:15:30
import datetime
...
class _ElementKeywords(KeywordGroup):
    ...
    def gen_idcard(self, idcard='', maxAge=55, minAge=21):
        ...
        if idlen == 17:
            ...
            ic = choice(province.keys()) + self._gen_nums(4) + self._gen_birthday(int(maxAge), int(minAge)) + self._gen_nums(3)
        ic = ic[0:17]
    ...
    def _Unicode(self):
        val = random.randint(19968, 40895)
        return chr(val)

    def _gen_name(self):
        li_name = [
         '赵', '钱', '孙', '李', '周', '吴', '郑', '王', '冯', '陈', '褚', '蒋',
    ...
    def _hex(self):
        head = random.randint(176, 207)
        body = random.randint(10, 15)
        if body == 15:
            tail = random.randint(0, 14)
        else:
