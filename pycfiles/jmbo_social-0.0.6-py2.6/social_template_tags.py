...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/social/templatetags/social_template_tags.py
...
def filter_permitted_fields(parser, token):
    ...
    if len(check_bits) < 4:
        raise TemplateSyntaxError("'%s' takes at least three arguments (object to filter, object owner and requesting user)" % bits[0])
    obj = bits[1]
    owner = bits[2]
...
def facebook_share_url(parser, token):
    ...
    check_bits = bits[:-2] if 'as' in bits else bits
    if len(check_bits) < 2:
        raise TemplateSyntaxError("'%s' takes at least 1 argument (path to share)" % bits[0])
    path = bits[1]
    as_var = None
    bits = bits[2:]
    if len(bits) >= 2 and bits[(-2)] == 'as':
...
def twitter_share_url(parser, token):
    ...
    check_bits = bits[:-2] if 'as' in bits else bits
    if len(check_bits) < 2:
        raise TemplateSyntaxError("'%s' takes at least 1 argument (path to share, twitter message)" % bits[0])
    path = bits[1]
    message = bits[2]
    as_var = None
    bits = bits[3:]
...
class TwitterShareURLNode(template.Node):
    ...
    def render(self, context):
        ...
        twitter_share_url = 'http://twitter.com/home?%s' % urlencode({'status': tweet, 
