...
# [GCC 8.4.0]
# Embedded file name: /home/winkidney/.virtualenvs/side-project/lib/python2.7/site-packages/cmdtree/shortcuts.py
# Compiled at: 2016-12-10 06:49:46
...
def _mk_group(name, help=None, path_prefix=None):
    ...
    def wrapper(func):
        if isinstance(func, Group):
            raise ValueError(('You can not register group `{name}` more than once').format(name=name))
        _name = name
        _func = func
...
def _mk_cmd(name, help=None, path_prefix=None):
    ...
    def wrapper(func):
        if isinstance(func, Cmd):
            raise ValueError(('You can not register a command more than once: {0}').format(func))
        _func = func
        if isinstance(func, CmdProxy):
            _func = func.func
        _name = name
        ...
        _cmd = Cmd(_func, _name, parser, help=help, full_path=full_path)
        apply2parser(func, parser)
        return _cmd
...
class CmdMeta(object):
    __slots__ = ('full_path', 'name', 'parser')
...
def option(name, help=None, is_flag=False, default=None, type=None):
    ...
    def wrapper(func):
        if isinstance(func, (Group, Cmd, CmdProxy)):
            parser = func.meta.parser
            parser.option(name, help=help, is_flag=is_flag, default=default, type=type)
            return func
        else:
            meta_cmd = CmdProxy(func)
            parser = meta_cmd.meta.parser
