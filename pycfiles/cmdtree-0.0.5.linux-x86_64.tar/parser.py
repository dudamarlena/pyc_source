...
# [GCC 8.4.0]
# Embedded file name: /home/winkidney/.virtualenvs/side-project/lib/python2.7/site-packages/cmdtree/parser.py
# Compiled at: 2016-09-22 05:45:19
...
class AParser(ArgumentParser):
    ...
    def add_cmd(self, name, help=None, func=None):
        ...
        if self.subparsers is None:
            self.subparsers = self.add_subparsers(title='sub-commands', help=help or 'sub-commands')
    ...
    def run(self, args=None, namespace=None):
        ...
        if _func:
            ...
            raise ValueError(('No function binding for args `{args}`').format(args=args))
            return
    ...
    def argument(self, name, help=None, type=None):
        kwargs = {'help': help}
        if name.startswith('-'):
            raise ValueError(('positional argument [{0}] can not contains `-` in').format(name))
        if type is not None:
            kwargs.update(type())
