...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/benchline/save_line_counts.py
# Compiled at: 2014-03-27 16:18:31
"""
...
"""
import os, csv, datetime, subprocess, peewee, benchline.args, benchline.command
...
def gather_line_counts(project_name):
    write_exclude_list()
    output_lines = benchline.command.output('cloc --exclude-list-file=exclude-list --exclude-dir=.idea --csv --by-file .').split(os.linesep)
    delete_exclude_list()
...
def main():
    ...
    parser.add_option('--db-file', action='store_true', default=default_db_file, help='sqlite database file to save to.  default=%s' % default_db_file)
    parser.add_option('-d', '--project-dir', help='Count the lines in all the projects in this directory.  It goes one level deep only.')
