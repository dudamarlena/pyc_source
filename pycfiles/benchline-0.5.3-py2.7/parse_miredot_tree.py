...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/benchline/parse_miredot_tree.py
# Compiled at: 2014-08-22 16:33:17
"""
...
def rest_method_factory(file_name, items):
    ...
    return map(one_rest_method, items[1:])
...
def parse_tree(lines):
    ...
    for line in lines:
        if line.startswith('-'):
            ...
            map(current_class.methods.append, rest_method_factory(current_class.file_name, split_method_line(line, saved_path_prefix)))
        else:
...
def name_from_rest_method(rest_method):
    ...
    part2 = ('').join(map(lambda x: upper_first_letter(camel_case(x)), filter(lambda x: x not in ('/', ''), [rest_method.class_name, rest_method.path])))
    result = mapping[rest_method.http_method] + part2
    if not rest_method.params:
        return result
    return result + 'By' + ('And').join(map(upper_first_letter, rest_method.params))
...
def lower_first_letter(some_str):
    ...
    """
    if some_str:
        return some_str[:1].lower() + some_str[1:]
    return ''
...
def upper_first_letter(some_str):
    ...
    """
    if some_str:
        return some_str[:1].upper() + some_str[1:]
    return ''
...
def rest_class_to_ws_file_contents(rest_class):
    ...
    return template.safe_substitute(class_name=rest_class.file_name, functions=('\n\n').join(map(generate_method_code, rest_class.methods)))
