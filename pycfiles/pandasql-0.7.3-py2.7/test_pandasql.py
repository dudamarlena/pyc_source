...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/pandasql/tests/test_pandasql.py
# Compiled at: 2016-02-01 12:11:24
import pandas as pd
from pandasql import PandaSQL, PandaSQLException, sqldf, load_meat
import string, pytest, pandas.util.testing as pdtest
...
def test_select_legacy(db_uri):
    df = pd.DataFrame({'letter_pos': range(len(string.ascii_letters)), 
...
def test_join(pdsql):
    df = pd.DataFrame({'letter_pos': range(len(string.ascii_letters)), 
       'l2': list(string.ascii_letters)})
    df2 = pd.DataFrame({'letter_pos': range(len(string.ascii_letters)), 
...
def test_name_index(pdsql):
    df = pd.DataFrame({'index': range(len(string.ascii_letters)), 
...
@pytest.mark.parametrize('db_flavor', ['postgres'])
