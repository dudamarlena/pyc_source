...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/pandasql/sqldf.py
# Compiled at: 2016-04-20 17:48:26
import inspect
...
__all__ = [
 'PandaSQL', 'PandaSQLException', 'sqldf']
...
def extract_table_names(query):
    ...
    tables = [ tbl for block in tables_blocks for tbl in re.findall('\\w+', block)
             ]
    return set(tables)
...
def write_table(df, tablename, conn):
    ...
    with catch_warnings():
        ...
        to_sql(df, name=tablename, con=conn, index=not any(name is None for name in df.index.names))
