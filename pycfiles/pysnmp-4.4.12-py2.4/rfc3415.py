...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pysnmp/proto/acmod/rfc3415.py
...
class Vacm(object):
    ...
    def _addAccessEntry(self, groupName, contextPrefix, securityModel, securityLevel, prefixMatch, readView, writeView, notifyView):
        if not groupName:
            return
        ...
        for (viewType, viewName) in (('read', readView), ('write', writeView), ('notify', notifyView)):
            try:
            ...
            for (context, models) in names.items():
                ...
                for (model, levels) in models.items():
                    for (level, viewName) in levels.items():
                        rating = (
                         securityModel == model, match == 1, len(context), level)
    ...
    def isAccessAllowed(self, snmpEngine, securityModel, securityName, securityLevel, viewType, contextName, variableName):
        ...
        (vacmContextName,) = mibInstrumController.mibBuilder.importSymbols('SNMP-VIEW-BASED-ACM-MIB', 'vacmContextName')
        ...
        (vacmGroupName,) = mibInstrumController.mibBuilder.importSymbols('SNMP-VIEW-BASED-ACM-MIB', 'vacmGroupName')
        ...
        if self._viewTreeBranchId != vacmViewTreeFamilyViewName.branchVersionId:
            ...
            while True:
                ...
                subtree = vacmViewTreeFamilySubtree.getNode(vacmViewTreeFamilySubtree.name + instId).syntax
                ...
                ignoredSubOids = [ i * 8 + j for (i, octet) in enumerate(mask) for (j, bit) in enumerate(powerOfTwo) if not bit & octet and i * 8 + j < maskLength ]
                if ignoredSubOids:
