...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pysnmp/smi/rfc1902.py
...
class ObjectIdentity(object):
    ...
    def getMibSymbol(self):
        ...
        if self.__state & self.stClean:
            return (
             self.__modName, self.__symName, self.__indices)
        else:
    ...
    def resolveWithMib(self, mibViewController):
        ...
        * :py:class:`~pysnmp.smi.view.MibViewController` looks up the
        ...
        if self.__asn1SourcesToAdd is None:
            ...
            addMibCompiler(mibViewController.mibBuilder, sources=self.__asn1SourcesToAdd, searchers=self.__asn1SourcesOptions.get('searchers'), borrowers=self.__asn1SourcesOptions.get('borrowers'), destination=self.__asn1SourcesOptions.get('destination'), ifAvailable=self.__asn1SourcesOptions.get('ifAvailable'), ifNotAdded=self.__asn1SourcesOptions.get('ifNotAdded'))
            self.__asn1SourcesToAdd = self.__asn1SourcesOptions = None
            ...
            if isinstance(mibNode, MibTableColumn):
                ...
                if self.__args[2:]:
                    try:
                        ...
                        raise SmiError('Instance index %r to OID conversion failure at object %r: %s' % (self.__args[2:], mibNode.getLabel(), sys.exc_info()[1]))
    ...
    def prettyPrint(self):
        if self.__state & self.stClean:
            ...
            return '%s::%s%s%s' % (self.__modName, self.__symName, self.__indices and '.' or '', ('.').join([ x.isSuperTypeOf(s, matchConstraints=False) and '"%s"' % x.prettyPrint() or x.prettyPrint() for x in self.__indices ]))
        else:
            raise SmiError('%s object not fully initialized' % self.__class__.__name__)
    ...
    def __getattr__(self, attr):
        if self.__state & self.stClean:
            if attr in ('asTuple', 'clone', 'subtype', 'isPrefixOf', 'isSameTypeWith',
                        'isSuperTypeOf', 'getTagSet', 'getEffectiveTagSet', 'getTagMap',
                        'tagSet', 'index'):
...
class ObjectType(object):
    ...
    def resolveWithMib(self, mibViewController, ignoreErrors=True):
        ...
        if not isinstance(self.__args[0].getMibNode(), (
         MibScalar, MibTableColumn)):
            ...
            self.__state |= self.stClean
            return self
        if isinstance(self.__args[1], (rfc1905.UnSpecified, rfc1905.NoSuchObject, rfc1905.NoSuchInstance, rfc1905.EndOfMibView)):
            self.__state |= self.stClean
            return self
        try:
            self.__args[1] = self.__args[0].getMibNode().getSyntax().clone(self.__args[1])
        except PyAsn1Error:
            err = 'MIB object %r having type %r failed to cast value %r: %s' % (self.__args[0].prettyPrint(), self.__args[0].getMibNode().getSyntax().__class__.__name__, self.__args[1], sys.exc_info()[1])
    ...
    def prettyPrint(self):
        if self.__state & self.stClean:
            return '%s = %s' % (self.__args[0].prettyPrint(), self.__args[1].prettyPrint())
        else:
            raise SmiError('%s object not fully initialized' % self.__class__.__name__)
...
class NotificationType(object):
    ...
    def resolveWithMib(self, mibViewController, ignoreErrors=True):
        ...
        * `objects` dictionary is queried for each MIB variable OID,
        ...
        self.__varBinds.append(ObjectType(ObjectIdentity(v2c.apiTrapPDU.snmpTrapOID), self.__objectIdentity).resolveWithMib(mibViewController, ignoreErrors))
        ...
        if isinstance(mibNode, SmiNotificationType):
            for notificationObject in mibNode.getObjects():
                objectIdentity = ObjectIdentity(*(notificationObject + self.__instanceIndex)).resolveWithMib(mibViewController, ignoreErrors)
                self.__varBinds.append(ObjectType(objectIdentity, self.__objects.get(notificationObject, rfc1905.unSpecified)).resolveWithMib(mibViewController, ignoreErrors))
