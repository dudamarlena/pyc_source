...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pysnmp/smi/builder.py
# Compiled at: 2019-08-18 17:24:05
import os, sys, struct, marshal, time, traceback
try:
    import importlib
    try:
    ...
    def read(self, f):
        ...
        for pycSfx in BYTECODE_SUFFIXES:
            try:
                ...
                if ENOENT == -1 or why.errno == ENOENT:
                    debug.logger & debug.flagBld and debug.logger('file %s access error: %s' % (f + pycSfx, why))
                else:
                ...
                if PY_MAGIC_NUMBER == pycData[:4]:
                    ...
                    pycData = pycData[4:]
                    debug.logger & debug.flagBld and debug.logger('file %s mtime %d' % (pycPath, pycTime))
                    break
                else:
        ...
        for pySfx in SOURCE_SUFFIXES:
            try:
                ...
                why = sys.exc_info()[1]
                if ENOENT == -1 or why.errno == ENOENT:
                    debug.logger & debug.flagBld and debug.logger('file %s access error: %s' % (f + pySfx, why))
                else:
...
class ZipMibSource(__AbstractMibSource):
    ...
    def _parseDosTime(dosdate, dostime):
        t = ((dosdate >> 9 & 127) + 1980, dosdate >> 5 & 15, dosdate & 31, dostime >> 11 & 31, dostime >> 5 & 63, (dostime & 31) * 2, -1, -1, -1)
...
class DirMibSource(__AbstractMibSource):
    ...
    def _listdir(self):
        try:
            ...
            why = sys.exc_info()
            debug.logger & debug.flagBld and debug.logger('listdir() failed for %s: %s' % (self._srcName, why[1]))
            return ()
    ...
    def _getData(self, f, mode):
        ...
        try:
            if f in os.listdir(self._srcName):
                p = os.path.join(self._srcName, f)
                fp = open(p, mode)
                data = fp.read()
                fp.close()
...
class MibBuilder(object):
    ...
    def getMibPath(self):
        ...
        for mibSource in self.getMibSources():
            if isinstance(mibSource, DirMibSource):
                ...
                raise error.MibLoadError('MIB source is not a plain directory: %s' % (mibSource,))
    ...
    def loadModule(self, modName, **userCtx):
        ...
        for mibSource in self.__mibSources:
            ...
            try:
                ...
                raise error.MibLoadError("MIB module '%s' load error: %s" % (modPath, traceback.format_exception(*sys.exc_info())))
        ...
        if modName not in self.__modSeen:
            raise error.MibNotFoundError('MIB file "%s" not found in search path (%s)' % (modName and modName + '.py[co]', (', ').join([ str(x) for x in self.__mibSources ])))
        return self
    ...
    def loadModules(self, *modNames, **userCtx):
        ...
        if not modNames:
            raise error.MibNotFoundError('No MIB module to load at %s' % (self,))
    ...
    def unloadModules(self, *modNames):
        ...
        for modName in modNames:
            if modName not in self.mibSymbols:
                raise error.MibNotFoundError('No module %s at %s' % (modName, self))
    ...
    def importSymbols(self, modName, *symNames, **userCtx):
        ...
        for symName in symNames:
            ...
            if modName not in self.mibSymbols:
                raise error.MibNotFoundError('No module %s loaded at %s' % (modName, self))
            if symName not in self.mibSymbols[modName]:
                raise error.SmiError('No symbol %s::%s at %s' % (modName, symName, self))
    ...
    def exportSymbols(self, modName, *anonymousSyms, **namedSyms):
        ...
        for (symName, symObj) in namedSyms.items():
            if symName in mibSymbols:
                raise error.SmiError('Symbol %s already exported at %s' % (symName, modName))
