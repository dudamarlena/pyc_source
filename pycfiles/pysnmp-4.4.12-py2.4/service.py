...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pysnmp/proto/secmod/rfc3414/service.py
# Compiled at: 2019-09-24 12:27:38
import time, sys
...
class SnmpUSMSecurityModel(AbstractSecurityModel):
    __module__ = __name__
    securityModelID = 3
    authServices = {hmacmd5.HmacMd5.serviceID: hmacmd5.HmacMd5(), hmacsha.HmacSha.serviceID: hmacsha.HmacSha(), hmacsha2.HmacSha2.sha224ServiceID: hmacsha2.HmacSha2(hmacsha2.HmacSha2.sha224ServiceID), hmacsha2.HmacSha2.sha256ServiceID: hmacsha2.HmacSha2(hmacsha2.HmacSha2.sha256ServiceID), hmacsha2.HmacSha2.sha384ServiceID: hmacsha2.HmacSha2(hmacsha2.HmacSha2.sha384ServiceID), hmacsha2.HmacSha2.sha512ServiceID: hmacsha2.HmacSha2(hmacsha2.HmacSha2.sha512ServiceID), noauth.NoAuth.serviceID: noauth.NoAuth()}
    privServices = {des.Des.serviceID: des.Des(), des3.Des3.serviceID: des3.Des3(), aes.Aes.serviceID: aes.Aes(), aes192.AesBlumenthal192.serviceID: aes192.AesBlumenthal192(), aes256.AesBlumenthal256.serviceID: aes256.AesBlumenthal256(), aes192.Aes192.serviceID: aes192.Aes192(), aes256.Aes256.serviceID: aes256.Aes256(), nopriv.NoPriv.serviceID: nopriv.NoPriv()}
    ...
    def __getUserInfo(mibInstrumController, securityEngineID, userName):
        (usmUserEntry,) = mibInstrumController.mibBuilder.importSymbols('SNMP-USER-BASED-SM-MIB', 'usmUserEntry')
        ...
        (pysnmpUsmKeyEntry,) = mibInstrumController.mibBuilder.importSymbols('PYSNMP-USM-MIB', 'pysnmpUsmKeyEntry')
    ...
    def __cloneUserInfo(self, mibInstrumController, securityEngineID, userName):
        (snmpEngineID,) = mibInstrumController.mibBuilder.importSymbols('__SNMP-FRAMEWORK-MIB', 'snmpEngineID')
        (usmUserEntry,) = mibInstrumController.mibBuilder.importSymbols('SNMP-USER-BASED-SM-MIB', 'usmUserEntry')
        ...
        return (
         usmUserName.syntax, usmUserSecurityName.syntax, usmUserAuthProtocol.syntax, pysnmpUsmKeyAuthLocalized.syntax, usmUserPrivProtocol.syntax, pysnmpUsmKeyPrivLocalized.syntax)

    def __generateRequestOrResponseMsg(self, snmpEngine, messageProcessingModel, globalData, maxMessageSize, securityModel, securityEngineID, securityName, securityLevel, scopedPDU, securityStateReference):
        mibBuilder = snmpEngine.msgAndPduDsp.mibInstrumController.mibBuilder
        ...
        if securityStateReference is not None:
            ...
            try:
                try:
                    (usmUserName, usmUserSecurityName, usmUserAuthProtocol, usmUserAuthKeyLocalized, usmUserPrivProtocol, usmUserPrivKeyLocalized) = self.__getUserInfo(snmpEngine.msgAndPduDsp.mibInstrumController, securityEngineID, self.__sec2usr(snmpEngine, securityName, securityEngineID))
                except NoSuchInstanceError:
                    (usmUserName, usmUserSecurityName, usmUserAuthProtocol, usmUserAuthKeyLocalized, usmUserPrivProtocol, usmUserPrivKeyLocalized) = self.__getUserInfo(snmpEngine.msgAndPduDsp.mibInstrumController, self.wildcardSecurityEngineId, self.__sec2usr(snmpEngine, securityName, self.wildcardSecurityEngineId))

                debug.logger & debug.flagSM and debug.logger('__generateRequestOrResponseMsg: found USM user entry usmUserName "%s" usmUserSecurityName "%s" usmUserAuthProtocol "%s" usmUserAuthKeyLocalized "%s" usmUserPrivProtocol "%s" usmUserPrivKeyLocalized "%s" by securityEngineID "%s" and  securityName "%s"' % (usmUserName, usmUserSecurityName, usmUserAuthProtocol, usmUserAuthKeyLocalized and usmUserAuthKeyLocalized.prettyPrint(), usmUserPrivProtocol, usmUserPrivKeyLocalized and usmUserPrivKeyLocalized.prettyPrint(), securityEngineID.prettyPrint(), securityName))
            except NoSuchInstanceError:
                ...
                if not reportUnknownName:
                    try:
                        (usmUserName, usmUserSecurityName, usmUserAuthProtocol, usmUserAuthKeyLocalized, usmUserPrivProtocol, usmUserPrivKeyLocalized) = self.__cloneUserInfo(snmpEngine.msgAndPduDsp.mibInstrumController, securityEngineID, self.__sec2usr(snmpEngine, securityName))
                        debug.logger & debug.flagSM and debug.logger('__generateRequestOrResponseMsg: cloned USM user entry usmUserName "%s" usmUserSecurityName "%s" usmUserAuthProtocol "%s" usmUserAuthKeyLocalized "%s" usmUserPrivProtocol "%s" usmUserPrivKeyLocalized "%s" for securityEngineID "%s" and  securityName "%s"' % (usmUserName, usmUserSecurityName, usmUserAuthProtocol, usmUserAuthKeyLocalized and usmUserAuthKeyLocalized.prettyPrint(), usmUserPrivProtocol, usmUserPrivKeyLocalized and usmUserPrivKeyLocalized.prettyPrint(), securityEngineID.prettyPrint(), securityName))
                    except NoSuchInstanceError:
                        debug.logger & debug.flagSM and debug.logger('__generateRequestOrResponseMsg: failed to clone USM user for securityEngineID "%s" securityName "%s"' % (securityEngineID, securityName))
        ...
        if securityStateReference is None and securityLevel in (2, 3):
            if securityEngineID in self.__timeline:
                (snmpEngineBoots, snmpEngineTime, latestReceivedEngineTime, latestUpdateTimestamp) = self.__timeline[securityEngineID]
        ...
        elif securityStateReference is not None:
        ...
        if securityLevel == 3:
            ...
            try:
                ...
                (encryptedData, privParameters) = privHandler.encryptData(usmUserPrivKeyLocalized, (
        ...
        return (
         msg.getComponentByPosition(2), authenticatedWholeMsg)
    ...
    def generateRequestMsg(self, snmpEngine, messageProcessingModel, globalData, maxMessageSize, securityModel, securityEngineID, securityName, securityLevel, scopedPDU):
        return self.__generateRequestOrResponseMsg(snmpEngine, messageProcessingModel, globalData, maxMessageSize, securityModel, securityEngineID, securityName, securityLevel, scopedPDU, None)
    ...
    def generateResponseMsg(self, snmpEngine, messageProcessingModel, globalData, maxMessageSize, securityModel, securityEngineID, securityName, securityLevel, scopedPDU, securityStateReference):
        return self.__generateRequestOrResponseMsg(snmpEngine, messageProcessingModel, globalData, maxMessageSize, securityModel, securityEngineID, securityName, securityLevel, scopedPDU, securityStateReference)
    ...
    def processIncomingMsg(self, snmpEngine, messageProcessingModel, maxMessageSize, securityParameters, securityModel, securityLevel, wholeMsg, msg):
        ...
        (securityParameters, rest) = decoder.decode(securityParameters, asn1Spec=self.__securityParametersSpec)
        ...
        contextName = null
        snmpEngineID = mibBuilder.importSymbols('__SNMP-FRAMEWORK-MIB', 'snmpEngineID')[0].syntax
        if msgAuthoritativeEngineId != snmpEngineID and msgAuthoritativeEngineId not in self.__timeline:
            if msgAuthoritativeEngineId:
                if 4 < len(msgAuthoritativeEngineId) < 33:
                    debug.logger & debug.flagSM and debug.logger('processIncomingMsg: non-synchronized securityEngineID %r' % (msgAuthoritativeEngineId,))
                else:
            ...
            if msgUserName:
                try:
                    (usmUserName, usmUserSecurityName, usmUserAuthProtocol, usmUserAuthKeyLocalized, usmUserPrivProtocol, usmUserPrivKeyLocalized) = self.__getUserInfo(snmpEngine.msgAndPduDsp.mibInstrumController, msgAuthoritativeEngineId, msgUserName)
                    debug.logger & debug.flagSM and debug.logger('processIncomingMsg: read user info from LCD')
                except NoSuchInstanceError:
                    try:
                        (usmUserName, usmUserSecurityName, usmUserAuthProtocol, usmUserAuthKeyLocalized, usmUserPrivProtocol, usmUserPrivKeyLocalized) = self.__getUserInfo(snmpEngine.msgAndPduDsp.mibInstrumController, self.wildcardSecurityEngineId, msgUserName)
                        debug.logger & debug.flagSM and debug.logger('processIncomingMsg: read wildcard user info from LCD')
                    except NoSuchInstanceError:
            ...
            if securityLevel == 3 or securityLevel == 2:
                ...
                try:
                    ...
                    self.__timeline[msgAuthoritativeEngineId] = (
                     securityParameters.getComponentByPosition(1), securityParameters.getComponentByPosition(2), securityParameters.getComponentByPosition(2), int(time.time()))
            ...
            if securityLevel == 3 or securityLevel == 2:
                if msgAuthoritativeEngineId == snmpEngineID:
                    ...
                    (snmpEngineBoots, snmpEngineTime, latestReceivedEngineTime, latestUpdateTimestamp) = self.__timeline[msgAuthoritativeEngineId]
                ...
                if msgAuthoritativeEngineId == snmpEngineID and (snmpEngineBoots == 2147483647 or snmpEngineBoots != msgAuthoritativeEngineBoots or abs(idleTime + int(snmpEngineTime) - int(msgAuthoritativeEngineTime)) > 150):
                ...
                if msgAuthoritativeEngineBoots > snmpEngineBoots or msgAuthoritativeEngineBoots == snmpEngineBoots and msgAuthoritativeEngineTime > latestReceivedEngineTime:
                    self.__timeline[msgAuthoritativeEngineId] = (
                     msgAuthoritativeEngineBoots, msgAuthoritativeEngineTime, msgAuthoritativeEngineTime, int(time.time()))
                    timerResolution = snmpEngine.transportDispatcher is None and 1.0 or snmpEngine.transportDispatcher.getTimerResolution()
                    expireAt = int(self.__expirationTimer + 300 / timerResolution)
                    if expireAt not in self.__timelineExpQueue:
                        self.__timelineExpQueue[expireAt] = []
                ...
                if snmpEngineBoots == 2147483647 or msgAuthoritativeEngineBoots < snmpEngineBoots or msgAuthoritativeEngineBoots == snmpEngineBoots and abs(idleTime + int(snmpEngineTime) - int(msgAuthoritativeEngineTime)) > 150:
        ...
        if securityLevel == 3:
            ...
            if encryptedPDU is None:
                raise error.StatusInformation(errorIndication=errind.decryptionError, msgUserName=msgUserName)
            try:
                decryptedData = privHandler.decryptData(usmUserPrivKeyLocalized, (
                 securityParameters.getComponentByPosition(1), securityParameters.getComponentByPosition(2), securityParameters.getComponentByPosition(5)), encryptedPDU)
        ...
        return (
         msgAuthoritativeEngineId, securityName, scopedPDU, maxSizeResponseScopedPDU, securityStateReference)
