...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pysnmp/smi/mibs/SNMPv2-SMI.py
...
class NotificationType(MibNode):
    ...
    def asn1Print(self):
        return 'NOTIFICATION-TYPE\n  OBJECTS { %s }\n  STATUS %s\n  DESCRIPTION "%s"\n  REFERENCE "%s"\n' % ((', ').join([ x for x in self.getObjects() ]), self.getStatus(), self.getDescription(), self.getReference())
...
class ObjectType(MibNode):
    ...
    def __repr__(self):
        return '%s(%r, %r)' % (self.__class__.__name__, self.name, self.syntax)
...
class MibTree(ObjectType):
    ...
    def readTest(self, name, val, idx, acInfo):
        ...
        if name == self.name:
            if acFun:
                if self.maxAccess not in ('readonly', 'readwrite', 'readcreate') or acFun(name, self.syntax, idx, 'read', acCtx):
    ...
    def writeTest(self, name, val, idx, acInfo):
        ...
        if name == self.name:
            if acFun:
                if self.maxAccess not in ('readwrite', 'readcreate') or acFun(name, self.syntax, idx, 'write', acCtx):
...
class MibTableColumn(MibScalar):
    ...
    def createTest(self, name, val, idx, acInfo):
        ...
        if name == self.name:
            raise error.NoAccessError(idx=idx, name=name)
        if acFun:
            if val is not None and self.maxAccess != 'readcreate' or acFun(name, self.syntax, idx, 'write', acCtx):
        ...
        if val is None and name in self.__createdInstances:
            return
        self.__createdInstances[name] = self.protoInstance(self.name, name[len(self.name):], self.syntax.clone())
...
class MibTableRow(MibTree):
    ...
    def setFromName(self, obj, value, impliedFlag=None, parentIndices=None):
        ...
        if baseTag == self.__intBaseTag:
            ...
            if impliedFlag:
                ...
                return (
                 obj.clone(value[1:value[0] + 1]), value[value[0] + 1:])
    ...
    def registerAugmentions(self, *names):
        for (modName, symName) in names:
            if (
             modName, symName) in self.augmentingRows:
                raise error.SmiError('Row %s already augmented by %s::%s' % (self.name, modName, symName))
    ...
    def __manageColumns(self, action, excludeName, nameSuffix, val, idx, acInfo):
        ...
        for (name, var) in self._vars.items():
            ...
            if name in indexVals:
                getattr(var, action)(name + nameSuffix, indexVals[name], idx, (None,
                                                                               None))
            else:
    ...
    def __delegate(self, subAction, name, val, idx, acInfo):
        ...
        try:
            ...
            self.__manageColumns('create' + subAction, name[:len(self.name) + 1], name[len(self.name) + 1:], None, idx, acInfo)
