...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pysnmp/entity/rfc3413/cmdrsp.py
...
class CommandResponderBase(object):
    ...
    def __init__(self, snmpEngine, snmpContext):
        snmpEngine.msgAndPduDsp.registerContextEngineId(snmpContext.contextEngineId, self.pduTypes, self.processPdu)
    ...
    def close(self, snmpEngine):
        snmpEngine.msgAndPduDsp.unregisterContextEngineId(self.snmpContext.contextEngineId, self.pduTypes)
        self.snmpContext = self.__pendingReqs = None
        return

    def sendVarBinds(self, snmpEngine, stateReference, errorStatus, errorIndex, varBinds):
        (messageProcessingModel, securityModel, securityName, securityLevel, contextEngineId, contextName, pduVersion, PDU, origPdu, maxSizeResponseScopedPDU, statusInformation) = self.__pendingReqs[stateReference]
    ...
    def sendPdu(self, snmpEngine, stateReference, PDU):
        (messageProcessingModel, securityModel, securityName, securityLevel, contextEngineId, contextName, pduVersion, _, origPdu, maxSizeResponseScopedPDU, statusInformation) = self.__pendingReqs[stateReference]
        if messageProcessingModel == 0:
            PDU = rfc2576.v2ToV1(PDU, origPdu)
        try:
            snmpEngine.msgAndPduDsp.returnResponsePdu(snmpEngine, messageProcessingModel, securityModel, securityName, securityLevel, contextEngineId, contextName, pduVersion, PDU, maxSizeResponseScopedPDU, stateReference, statusInformation)
    ...
    def processPdu(self, snmpEngine, messageProcessingModel, securityModel, securityName, securityLevel, contextEngineId, contextName, pduVersion, PDU, maxSizeResponseScopedPDU, stateReference):
        ...
        self.__pendingReqs[stateReference] = (
         messageProcessingModel, securityModel, securityName, securityLevel, contextEngineId, contextName, pduVersion, rspPDU, origPdu, maxSizeResponseScopedPDU, statusInformation)
    ...
    def __verifyAccess(self, name, syntax, idx, viewType, acCtx):
        ...
        (securityModel, securityName, securityLevel, contextName, pduType) = (execCtx['securityModel'], execCtx['securityName'], execCtx['securityLevel'], execCtx['contextName'], execCtx['pdu'].getTagSet())
        try:
            snmpEngine.accessControlModel[self.acmID].isAccessAllowed(snmpEngine, securityModel, securityName, securityLevel, viewType, contextName, name)
        except error.StatusInformation:
            ...
            if errorIndication == errind.noSuchView or errorIndication == errind.noAccessEntry or errorIndication == errind.noGroupName:
                ...
                raise pysnmp.smi.error.GenError(name=name, idx=idx, oid=snmpUnknownContexts.name, val=snmpUnknownContexts.syntax)
            elif errorIndication == errind.notInView:
                return 1
            else:
                raise error.ProtocolError('Unknown ACM error %s' % errorIndication)
        else:
            if securityModel == 1 and syntax is not None and self._counter64Type == syntax.getTagSet() and self._getNextRequestType == pduType:
...
class GetCommandResponder(CommandResponderBase):
    ...
    def handleMgmtOperation(self, snmpEngine, stateReference, contextName, PDU, acInfo):
        ...
        self.sendVarBinds(snmpEngine, stateReference, 0, 0, mgmtFun(v2c.apiPDU.getVarBinds(PDU), (acFun, acCtx)))
        self.releaseStateInformation(stateReference)
...
class SetCommandResponder(CommandResponderBase):
    ...
    def handleMgmtOperation(self, snmpEngine, stateReference, contextName, PDU, acInfo):
        ...
        try:
            self.sendVarBinds(snmpEngine, stateReference, 0, 0, mgmtFun(v2c.apiPDU.getVarBinds(PDU), (
             acFun, acCtx)))
            self.releaseStateInformation(stateReference)
