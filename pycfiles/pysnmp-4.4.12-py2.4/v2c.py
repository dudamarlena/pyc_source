...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pysnmp/proto/api/v2c.py
...
class PDUAPI(v1.PDUAPI):
    ...
    def getVarBindTable(self, reqPDU, rspPDU):
        return [
         apiPDU.getVarBinds(rspPDU)]

    def setEndOfMibError(self, pdu, errorIndex):
        varBindList = self.getVarBindList(pdu)
        varBindList[(errorIndex - 1)].setComponentByPosition(1, rfc1905.endOfMibView, verifyConstraints=False, matchTags=False, matchConstraints=False)

    def setNoSuchInstanceError(self, pdu, errorIndex):
        varBindList = self.getVarBindList(pdu)
        varBindList[(errorIndex - 1)].setComponentByPosition(1, rfc1905.noSuchInstance, verifyConstraints=False, matchTags=False, matchConstraints=False)
...
class BulkPDUAPI(PDUAPI):
    ...
    def setDefaults(self, pdu):
        ...
        pdu.setComponentByPosition(0, getNextRequestID(), verifyConstraints=False, matchTags=False, matchConstraints=False)
        pdu.setComponentByPosition(1, self._nonRepeaters, verifyConstraints=False, matchTags=False, matchConstraints=False)
        pdu.setComponentByPosition(2, self._maxRepetitions, verifyConstraints=False, matchTags=False, matchConstraints=False)
...
class TrapPDUAPI(v1.PDUAPI):
    ...
    def setDefaults(self, pdu):
        ...
        varBinds = [(self.sysUpTime, self._zeroTime), (self.snmpTrapOID, self._genTrap)]
