...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pysnmp/smi/view.py
...
class MibViewController(object):
    ...
    def indexMib(self):
        ...
        for modName in [''] + modNames:
            self.__mibSymbolsIdx[modName] = mibMod = {'oidToLabelIdx': OidOrderedDict(), 'labelToOidIdx': {}, 'varToNameIdx': {}, 'typeToModIdx': OrderedDict(), 'oidToModIdx': {}}
            if not modName:
                globMibMod = mibMod
                continue
            for (n, v) in self.mibBuilder.mibSymbols[modName].items():
                if n == self.mibBuilder.moduleID:
                    continue
                if isinstance(v, classTypes):
                    if n in mibMod['typeToModIdx']:
                        raise error.SmiError('Duplicate SMI type %s::%s, has %s' % (modName, n, mibMod['typeToModIdx'][n]))
                    ...
                    if n in mibMod['varToNameIdx']:
                        raise error.SmiError('Duplicate MIB variable %s::%s has %s' % (modName, n, mibMod['varToNameIdx'][n]))
                    ...
                    raise error.SmiError('Unexpected object %s::%s' % (modName, n))
    ...
    def getNextModuleName(self, modName):
        ...
        try:
            ...
            raise error.SmiError('No module next to %s at %s' % (modName, self))
    ...
    def __getOidLabel(self, nodeName, oidToLabelIdx, labelToOidIdx):
        ...
        if nodeName in labelToOidIdx:
            return (
             labelToOidIdx[nodeName], nodeName, ())
        if nodeName in oidToLabelIdx:
            return (
             nodeName, oidToLabelIdx[nodeName], ())
        if len(nodeName) < 2:
            return (
             nodeName, nodeName, ())
        ...
        if resLabel in labelToOidIdx:
            return (
             labelToOidIdx[resLabel], resLabel, ())
        resOid = oid + suffix
        if resOid in oidToLabelIdx:
            return (
             resOid, oidToLabelIdx[resOid], ())
        return (
         oid, label, suffix)
    ...
    def getNodeNameByOid(self, nodeName, modName=''):
        ...
        (oid, label, suffix) = self.__getOidLabel(nodeName, mibMod['oidToLabelIdx'], mibMod['labelToOidIdx'])
        if oid == label:
            raise error.NoSuchObjectError(str="Can't resolve node name %s::%s at %s" % (modName, nodeName, self))
    ...
    def getNodeNameByDesc(self, nodeName, modName=''):
        ...
        if nodeName in mibMod['varToNameIdx']:
            ...
            raise error.NoSuchObjectError(str='No such symbol %s::%s at %s' % (modName, nodeName, self))
    ...
    def getOrderedNodeName(self, index, modName=''):
        ...
        if not mibMod['oidToLabelIdx']:
            raise error.NoSuchObjectError(str='No variables at MIB module %s at %s' % (modName, self))
        try:
            (oid, label) = mibMod['oidToLabelIdx'].items()[index]
        except KeyError:
            raise error.NoSuchObjectError(str='No symbol at position %s in MIB module %s at %s' % (index, modName, self))
    ...
    def getNextNodeName(self, nodeName, modName=''):
        ...
        try:
            return self.getNodeName(self.__mibSymbolsIdx[modName]['oidToLabelIdx'].nextKey(oid) + suffix, modName)
        except KeyError:
            raise error.NoSuchObjectError(str='No name next to %s::%s at %s' % (modName, nodeName, self))

    def getParentNodeName(self, nodeName, modName=''):
        (oid, label, suffix) = self.getNodeName(nodeName, modName)
        if len(oid) < 2:
            raise error.NoSuchObjectError(str='No parent name for %s::%s at %s' % (modName, nodeName, self))
        return (
    ...
    def getTypeName(self, typeName, modName=''):
        ...
        if typeName in mibMod['typeToModIdx']:
            ...
            raise error.NoSuchObjectError(str='No such type %s::%s at %s' % (modName, typeName, self))
        return (
         m, typeName)
    ...
    def getOrderedTypeName(self, index, modName=''):
        ...
        if not mibMod['typeToModIdx']:
            raise error.NoSuchObjectError(str='No types at MIB module %s at %s' % (modName, self))
    ...
    def getNextType(self, typeName, modName=''):
        ...
        try:
            ...
            raise error.NoSuchObjectError(str='No type next to %s::%s at %s' % (modName, typeName, self))