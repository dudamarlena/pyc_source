...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pysnmp/hlapi/twisted/cmdgen.py
...
__all__ = [
 'getCmd', 'nextCmd', 'setCmd', 'bulkCmd', 'isEndOfMib']
...
def getCmd(snmpEngine, authData, transportTarget, contextData, *varBinds, **options):
    ...
    * varBinds (tuple) : A sequence of
    ...
    deferred = Deferred()
    cmdgen.GetCommandGenerator().sendVarBinds(snmpEngine, addrName, contextData.contextEngineId, contextData.contextName, vbProcessor.makeVarBinds(snmpEngine, varBinds), __cbFun, (
...
def setCmd(snmpEngine, authData, transportTarget, contextData, *varBinds, **options):
    ...
    * varBinds (tuple) : A sequence of
    ...
    (addrName, paramsName) = lcd.configure(snmpEngine, authData, transportTarget, contextData.contextName)
    deferred = Deferred()
    cmdgen.SetCommandGenerator().sendVarBinds(snmpEngine, addrName, contextData.contextEngineId, contextData.contextName, vbProcessor.makeVarBinds(snmpEngine, varBinds), __cbFun, (
     options.get('lookupMib', True), deferred))
    return deferred
...
def nextCmd(snmpEngine, authData, transportTarget, contextData, *varBinds, **options):
    ...
    * varBinds (tuple) :
    ...
    def __cbFun(snmpEngine, sendRequestHandle, errorIndication, errorStatus, errorIndex, varBindTable, cbCtx):
        (lookupMib, deferred) = cbCtx
        if options.get('ignoreNonIncreasingOid', False) and errorIndication and isinstance(errorIndication, errind.OidNotIncreasing):
            errorIndication = None
        if errorIndication:
            deferred.errback(Failure(errorIndication))
        else:
            try:
                varBindsUnmade = [ vbProcessor.unmakeVarBinds(snmpEngine, varBindTableRow, lookupMib)
    ...
    (addrName, paramsName) = lcd.configure(snmpEngine, authData, transportTarget, contextData.contextName)
    deferred = Deferred()
    cmdgen.NextCommandGenerator().sendVarBinds(snmpEngine, addrName, contextData.contextEngineId, contextData.contextName, vbProcessor.makeVarBinds(snmpEngine, varBinds), __cbFun, (
     options.get('lookupMib', True), deferred))
    return deferred
...
def bulkCmd(snmpEngine, authData, transportTarget, contextData, nonRepeaters, maxRepetitions, *varBinds, **options):
    ...
    * varBindTable (tuple):
    ...
    (addrName, paramsName) = lcd.configure(snmpEngine, authData, transportTarget, contextData.contextName)
    deferred = Deferred()
    cmdgen.BulkCommandGenerator().sendVarBinds(snmpEngine, addrName, contextData.contextEngineId, contextData.contextName, nonRepeaters, maxRepetitions, vbProcessor.makeVarBinds(snmpEngine, varBinds), __cbFun, (
     options.get('lookupMib', True), deferred))
    return deferred