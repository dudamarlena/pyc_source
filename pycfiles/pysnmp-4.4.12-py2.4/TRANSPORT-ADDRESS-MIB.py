...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pysnmp/smi/mibs/TRANSPORT-ADDRESS-MIB.py
    ...
    if sys.platform != 'win32':
        ...
        def inet_pton(address_family, ip_string):
            if address_family == socket.AF_INET:
                ...
                raise socket.error('Unknown address family %s' % (address_family,))
            ...
            if spaces == 1:
                ...
                if ip_string.startswith('::'):
                    ...
                    raise socket.error('Invalid IPv6 address: "%s"' % (ip_string,))
            elif spaces == 3:
                if ip_string != '::':
                    raise socket.error('Invalid IPv6 address: "%s"' % (ip_string,))
                return '\x00' * 16
                ...
                raise socket.error('Invalid IPv6 address: "%s"' % (ip_string,))
            groups = [ t for t in [ int(t, 16) for t in groups ] if t & 65535 == t ]
            if len(groups) != 8:
                raise socket.error('Invalid IPv6 address: "%s"' % (ip_string,))
        ...
        def inet_ntop(address_family, packed_ip):
            if address_family == socket.AF_INET:
                ...
                raise socket.error('Unknown address family %s' % (address_family,))
            if len(packed_ip) != 16:
                raise socket.error('incorrect address length: %s' % len(packed_ip))
...
class TransportAddressType(TextualConvention, Integer32):
    ...
    namedValues = NamedValues(('unknown', 0), ('udpIpv4', 1), ('udpIpv6', 2), ('udpIpv4z',
                                                                               3), ('udpIpv6z',
                                                                                    4), ('tcpIpv4',
                                                                                         5), ('tcpIpv6',
                                                                                              6), ('tcpIpv4z',
...
class TransportAddressIPv4(TextualConvention, OctetString):
    ...
    def prettyIn(self, value):
        if isinstance(value, tuple):
            value = inet_pton(socket.AF_INET, value[0]) + int2oct(value[1] >> 8 & 255) + int2oct(value[1] & 255)
        return OctetString.prettyIn(self, value)

    def __asSocketAddress(self):
        if not hasattr(self, '__tuple_value'):
            v = self.asOctets()
            self.__tuple_value = (inet_ntop(socket.AF_INET, v[:4]), oct2int(v[4]) << 8 | oct2int(v[5]))
...
class TransportAddressIPv6(TextualConvention, OctetString):
    ...
    def prettyIn(self, value):
        ...
        if isinstance(value, tuple):
            value = inet_pton(socket.AF_INET6, value[0]) + int2oct(value[1] >> 8 & 255) + int2oct(value[1] & 255)
        return OctetString.prettyIn(self, value)
    ...
    def __asSocketAddress(self):
        if not hasattr(self, '__tuple_value'):
            if not has_ipv6:
                raise error.PySnmpError('IPv6 not supported by platform')
            v = self.asOctets()
            self.__tuple_value = (inet_ntop(socket.AF_INET6, v[:16]), oct2int(v[16]) << 8 | oct2int(v[17]), 0, 0)
        return self.__tuple_value
