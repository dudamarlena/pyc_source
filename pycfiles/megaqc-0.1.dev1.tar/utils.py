...
# [GCC 8.4.0]
# Embedded file name: /Users/ewels/GitHub/MegaQC/megaqc/api/utils.py
...
import copy, json, os, plotly.offline as py, plotly.figure_factory as ff, plotly.graph_objs as go, random, string, sys
...
def store_report_data(user, report_data, uploaded_file):
    ...
    upload_row = Upload(status='NOT TREATED', path=os.path.join(current_app.config['UPLOAD_FOLDER'], file_name), message='File has been created, loading in MegaQC is queued.', user_id=user.user_id)
    upload_row.save()
...
def handle_report_data(user, report_data):
    ...
    if db.session.query(Report).filter(Report.report_hash == report_hash).first():
        return (
         False, 'Report already uploaded')
    try:
        report_created_at = datetime.strptime(report_data['config_creation_date'], '%Y-%m-%d, %H:%M')
    except:
        report_created_at = datetime.now()

    new_report = Report(report_hash=report_hash, user_id=user.user_id, created_at=report_created_at)
    new_report.save()
    current_app.logger.info(('Created new report {} from {}').format(new_report.report_id, user.email))
    report_id = new_report.report_id
    user_report_meta = ReportMeta(report_meta_key='username', report_meta_value=user.username, report_id=report_id)
    ...
    for key in report_data:
        if key.startswith('config') and not isinstance(report_data[key], list) and not isinstance(report_data[key], dict) and report_data[key]:
            new_meta_cnt += 1
            new_meta = ReportMeta(report_meta_key=key, report_meta_value=report_data[key], report_id=report_id)
            new_meta.save()
    ...
    for s_key in report_data.get('report_saved_raw_data', {}):
        ...
        for s_name in report_data['report_saved_raw_data'][s_key]:
            ...
            for d_key in report_data['report_saved_raw_data'][s_key][s_name]:
                key_type = db.session.query(SampleDataType).filter(SampleDataType.data_id == d_key).first()
                if not key_type:
                    key_type = SampleDataType(data_key=('{}__{}').format(section, d_key), data_section=section, data_id=d_key)
                    key_type.save()
                type_id = key_type.sample_data_type_id
                value = report_data['report_saved_raw_data'][s_key][s_name][d_key]
                new_data = SampleData(report_id=report_id, sample_data_type_id=type_id, sample_id=sample_id, value=str(value))
                new_data.save()
    ...
    for plot in report_data.get('report_plot_data'):
        if 'mqc_hcplot_' in plot:
            continue
        if report_data['report_plot_data'][plot]['plot_type'] not in ('bar_graph',
                                                                      'xy_line'):
            continue
        ...
        for dst_idx, dataset in enumerate(report_data['report_plot_data'][plot]['datasets']):
            ...
            plot_config = db.session.query(PlotConfig).filter(PlotConfig.config_type == report_data['report_plot_data'][plot]['plot_type'], PlotConfig.config_name == plot, PlotConfig.config_dataset == dataset_name).first()
            if not plot_config:
                plot_config = PlotConfig(config_type=report_data['report_plot_data'][plot]['plot_type'], config_name=plot, config_dataset=dataset_name, data=config)
                plot_config.save()
                new_plotcfg_cnt += 1
            ...
            if report_data['report_plot_data'][plot]['plot_type'] == 'bar_graph':
                for sub_dict in dataset:
                    ...
                    data = json.dumps({x:y for x, y in list(sub_dict.items()) if x != 'data' if x != 'data'})
                    if not existing_category:
                        existing_category = PlotCategory(report_id=report_id, config_id=config_id, category_name=data_key, data=data)
                    ...
                    for sa_idx, actual_data in enumerate(sub_dict['data']):
                        ...
                        new_dataset_row = PlotData(report_id=report_id, config_id=config_id, sample_id=sample_id, plot_category_id=existing_category.plot_category_id, data=json.dumps(actual_data))
                ...
                for sub_dict in dataset:
                    ...
                    for sa_idx, actual_data in enumerate(sub_dict['data']):
                        ...
                        new_dataset_row = PlotData(report_id=report_id, config_id=config_id, sample_id=sample_id, plot_category_id=category_id, data=json.dumps(sub_dict['data']))
...
def generate_report_plot(plot_type, sample_names):
    ...
    if rows[0][0].config_type == 'bar_graph':
        ...
        for idx, d in enumerate(series):
            my_trace = go.Bar(y=samples, x=[ plot_data[d][x] for x in samples ], name=d, text=row[3].sample_name, orientation='h', visible=config.get('cpswitch_c_active', True), marker=dict(color=colors[(idx % len(colors))], line=dict(color=colors[(idx % len(colors))], width=3)), hoverinfo='text+x')
            plots.append(my_trace)

        for idx, d in enumerate(series):
            my_trace = go.Bar(y=samples, x=[ plot_data_perc[d][x] for x in samples ], name=d, text=row[3].sample_name, orientation='h', visible=not config.get('cpswitch_c_active', True), marker=dict(color=colors[(idx % len(colors))], line=dict(color=colors[(idx % len(colors))], width=3)), hoverinfo='text+x')
            plots.append(my_trace)

        updated_layout = config_translate('bar_graph', config, len(series), go.Layout(barmode='stack', hovermode='closest', height=500, margin=dict(t=80, b=80, l=100, r=40)))
        fig = go.Figure(data=plots, layout=updated_layout)
        plot_div = py.plot(fig, output_type='div', show_link=False, config=dict(modeBarButtonsToRemove=[
         'sendDataToCloud',
         'resetScale2d',
        ...
        return plot_div
    if rows[0][0].config_type == 'xy_line':
        plots = []
        config = json.loads(rows[(-1)][0].data)
        for idx, row in enumerate(rows):
            ...
            plots.append(go.Scatter(y=ys, x=xs, name=row[3].sample_name, text=row[3].sample_name, mode='lines', marker=dict(color=line_color, line=dict(color=line_color, width=1)), hoverinfo='text'))

        updated_layout = config_translate('xy_line', config, len(rows), go.Layout(xaxis={'type': 'category'}, showlegend=False, hovermode='closest', height=500, margin=dict(t=80, b=80, l=80, r=40)))
        fig = go.Figure(data=plots, layout=updated_layout)
        plot_div = py.plot(fig, output_type='div', show_link=False, config=dict(modeBarButtonsToRemove=[
         'sendDataToCloud',
         'resetScale2d',
...
def config_translate(plot_type, config, series_nb, plotly_layout=go.Layout()):
    ...
    if 'ymin' in config and 'ymax' in config:
        my_range = [
         float(config['ymin']), float(config['ymax'])]
    ...
    if plot_type == 'bar_graph':
        ...
        updatemenus = list([
         dict(buttons=list([
          dict(args=[{'visible': [ i < series_nb for i in range(series_nb * 2) ]}], label='Count', method='restyle'),
          dict(args=[{'visible': [ i >= series_nb for i in range(series_nb * 2) ]}], label='Percentage', method='restyle')]), direction='left', active=0 if config.get('cpswitch_c_active', True) else 1, showactive=True, type='buttons', x=-0.1, xanchor='left', y=1.15, yanchor='top')])
        ...
        if 'xPlotBands' in config:
            ...
            for band in config['xPlotBands']:
                shape = {'type': 'rect', 'yref': 'paper', 'x0': band['from'], 
                   'y0': 0, 
                   'x1': band['to'], 
                   'y1': 1, 
        ...
        if 'yPlotBands' in config:
            ...
            for band in config['yPlotBands']:
                shape = {'type': 'rect', 'xref': 'paper', 'x0': 0, 
                   'y0': band['from'], 
                   'x1': 1, 
                   'y1': band['to'], 
                   'line': {'color': band['color'], 
...
def get_sample_metadata_fields(filters=None):
    ...
    for row in sample_metadata_query.all():
        ...
        sample_fields.append({'key': row[0], 
           'section': row[1], 
           'type_id': row[2], 
...
def get_plot_types(user, filters=None):
    ...
    for row in pt_query.all():
        if row[3] == 'xy_line':
            plot_type_obj = {'name': ('{} -- {}').format(row[0], row[1]), 'nicename': json.loads(row[2]).get('title', row[0].replace('_', ' ')), 
               'plot_id': row[0], 
               'plot_ds_name': row[1], 
               'type': 'linegraph', 
               'favourite': (
                           row[0], row[1]) in fav_plot_types}
        elif row[3] == 'bar_graph':
            plot_type_obj = {'name': ('{} -- {}').format(row[0], row[1]), 'nicename': ('{0} ({1})').format(json.loads(row[2]).get('title', row[0].replace('_', ' ')), row[1]), 
               'type': 'bargraph', 
               'favourite': (
                           row[0], row[1]) in fav_plot_types}
...
def aggregate_new_parameters(user, filters=None, short=True):
    ...
    if not short:
        ...
        return (
         len(samples), report_field_keys, sample_fields, plot_types)
    else:
        return (
         len(samples), plot_types)
    ...
    for filter_group in filters:
        ...
        for filter_idx, one_filter in enumerate(filter_group):
            ...
            if one_filter['type'] == 'daterange':
                ...
                if one_filter['type'] == 'date':
                    params = [
                     datetime.strptime(one_filter['value'], '%Y-%m-%d')]
                    cmps = [one_filter['cmp']]
                else:
                    if one_filter['type'] == 'timedelta':
                        params = [
                         datetime.now() - timedelta(int(one_filter['value']))]
...
def get_plot_favourites(user):
    ...
    favourite_list_query = db.session.query(PlotFavourite.plot_favourite_id, PlotFavourite.user_id, PlotFavourite.title, PlotFavourite.description, PlotFavourite.plot_type, PlotFavourite.data, PlotFavourite.created_at).filter_by(user_id=user.user_id).order_by(PlotFavourite.created_at.desc())
    ret_data = []
    for row in favourite_list_query.all():
        ret_data.append(dict(plot_favourite_id=row.plot_favourite_id, title=row.title, description=row.description, plot_type=row.plot_type, data=json.loads(row.data), created_at=row.created_at))
...
def get_favourite_plot_data(user, favourite_id):
    ...
    fp_row = db.session.query(PlotFavourite.plot_favourite_id, PlotFavourite.user_id, PlotFavourite.title, PlotFavourite.description, PlotFavourite.plot_type, PlotFavourite.data, PlotFavourite.created_at).filter_by(plot_favourite_id=favourite_id).first()
    plot_type = fp_row[4]
    ...
    return {'user_id': fp_row[1], 'title': fp_row[2], 
...
def save_plot_favourite_data(user, plot_type, data, title, description=None):
    ...
    new_plot_favourite = PlotFavourite(user_id=user.user_id, title=title, description=description, plot_type=plot_type, data=json.dumps(data))
...
def get_dashboards(user):
    ...
    dashboard_list_query = db.session.query(Dashboard.dashboard_id, Dashboard.user_id, Dashboard.title, Dashboard.data, Dashboard.is_public, Dashboard.modified_at, Dashboard.created_at).filter_by(user_id=user.user_id).order_by(Dashboard.created_at.desc())
    ret_data = []
    for row in dashboard_list_query.all():
        ret_data.append(dict(dashboard_id=row.dashboard_id, user_id=row.user_id, title=row.title, data=json.loads(row.data), is_public=row.is_public, modified_at=row.modified_at, created_at=row.created_at))
...
def get_dashboard_data(user, dashboard_id):
    ...
    row = db.session.query(Dashboard.dashboard_id, Dashboard.user_id, Dashboard.title, Dashboard.data, Dashboard.is_public, Dashboard.modified_at, Dashboard.created_at).filter(or_(Dashboard.user_id == user.user_id, Dashboard.is_public == True), Dashboard.dashboard_id == dashboard_id).first()
    if row is None:
        return
    else:
        parsed_data = json.loads(row.data)
        max_height = max([ d['y'] + d['height'] for d in parsed_data ])
        return dict(dashboard_id=row.dashboard_id, user_id=row.user_id, title=row.title, data=parsed_data, max_height=max_height, is_public=row.is_public, modified_at=row.modified_at, created_at=row.created_at)


def save_dashboard_data(user, title, data, is_public=False, dashboard_id=None):
    """ Save a dashboard """
    new_dashboard = Dashboard(user_id=user.user_id, title=title, data=json.dumps(data), is_public=is_public)
    new_dashboard.save()
...
def generate_distribution_plot(plot_data, nbins=20, ptype='boxplot'):
    ...
    for dtype in sorted(dtypes):
        ...
        if ptype == 'boxplot':
            ...
            figs.append(go.Box(y=pdata, name=('{} ({})').format(dtype, len(pdata)), boxpoints='all', jitter=0.5, pointpos=0, line=dict(width=0), whiskerwidth=0, fillcolor='rgba(0, 0, 0, 0)'))
        elif ptype == 'hist':
            figs.append(go.Histogram(x=pdata, opacity=0.75, nbinsx=nbins, name=('{} ({})').format(dtype, len(pdata))))
        elif ptype == 'violin':
            if len(figs) == 0:
                figs = {}
    ...
    layout = {}
    if ptype == 'hist':
        layout = go.Layout(barmode='overlay', showlegend=True, xaxis=dict(title='Data'), yaxis=dict(title='Sample Count'))
    if ptype == 'violin':
        figure = ff.create_violin(figs)
    else:
        figure = go.Figure(data=figs, layout=layout)
    plot_div = py.plot(figure, output_type='div', show_link=False, config=dict(modeBarButtonsToRemove=[
     'sendDataToCloud',
     'resetScale2d',
     'hoverClosestCartesian',
     'hoverCompareCartesian',
...
def generate_trend_plot(plot_data):
    ...
    for field in sorted(plot_data.keys()):
        if ptype == 'line':
            ...
            figs.append(go.Scatter(x=[ x['time'] for x in plot_data[field] ], y=yvals, mode='markers', text=[ x['name'] for x in plot_data[field] ], name=('{} ({})').format(field, yvalcount), hoverinfo='text+x+y'))
        else:
            return ('Error - unrecognised plot type: {}').format(ptype)

    plot_div = py.plot(go.Figure(data=figs, layout=go.Layout(hovermode='closest')), output_type='div', show_link=False, config=dict(modeBarButtonsToRemove=[
     'sendDataToCloud',
     'resetScale2d',
     'hoverClosestCartesian',
     'hoverCompareCartesian',
...
def generate_comparison_plot(plot_data, data_keys, field_names=None, pointsize=10, joinmarkers=False):
    ...
    if not all([ x == None for x in plot_col ]):
        ...
        annotations.append(go.Annotation(text=field_names['col'], x=1.02, y=0.5, textangle=-90, xref='paper', yref='paper', showarrow=False))
    ...
    if all([ x == None for x in plot_z ]):
        fig = go.Scatter(x=plot_x, y=plot_y, mode='lines+markers' if joinmarkers else 'markers', marker=markers, text=plot_names)
    else:
        markers.update({'opacity': 0.8})
        fig = go.Scatter3d(x=plot_x, y=plot_y, z=plot_z, mode='markers', marker=markers, text=plot_names)
        plot_height = 800
    layout = go.Layout(title=ptitle, xaxis=dict(title=field_names['x']), yaxis=dict(title=field_names['y']), scene=dict(xaxis=dict(title=field_names['x']), yaxis=dict(title=field_names['y']), zaxis=dict(title=field_names['z'])), annotations=annotations, height=plot_height)
    plot_div = py.plot(go.Figure(data=[fig], layout=layout), output_type='div', show_link=False, config=dict(modeBarButtonsToRemove=[
     'sendDataToCloud',
     'resetScale2d',
     'hoverClosestCartesian',
     'hoverCompareCartesian',
...
def get_reports_data(count=False, user_id=None, filters=None):
    ...
    if count:
        ...
        for report in reports:
            ret = {'report_id': report[0].report_id, 'report_hash': report[0].report_hash, 
               'upload_date': report[0].created_at, 
               'username': report[1]}
            report_md_query = db.session.query(ReportMeta).filter(ReportMeta.report_id == report[0].report_id)
...
def get_queued_uploads(count=False, filter_cats=None):
    ...
    if count:
        ...
        uploads_query = db.session.query(Upload).filter(Upload.status.in_(filter_cats)).order_by(Upload.created_at.desc())
        ...
        for upload in uploads:
            ...
            if upload.status == 'TREATED':
                ...
                status_class = 'danger'
            ret_data.append({'upload_id': upload.upload_id, 
