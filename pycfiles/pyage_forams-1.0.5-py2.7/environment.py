...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyage_forams/solutions/environment.py
...
class Environment2d(AbstractEnvironment):
    ...
    def _initialize_grid(self):
        grid = [ [ Cell(randint(0, 4) if random() < self.initial_algae_probability else 0) for _ in range(self.size[1]) ] for _ in range(self.size[0])
               ]
        for i in range(self.size[0]):
            for j in range(self.size[1]):
                grid[i][j]._neighbours.extend([ grid[x][y] for x in range(max(0, i - 1), min(self.size[0], i + 2)) for y in range(max(0, j - 1), min(self.size[1], j + 2)) if x != i or y != j
                                              ])
...
class Environment3d(AbstractEnvironment):
    ...
    def _initialize_grid(self):
        grid = [ [ [ Cell(randint(1, 4) if random() < self.initial_algae_probability else 0) for _ in range(self.size[2]) ] for _ in range(self.size[1]) ] for _ in range(self.size[0])
               ]
        for i in range(self.size[0]):
            for j in range(self.size[1]):
                for k in range(self.size[2]):
                    grid[i][j][k].depth = self.size[0] - k - 1
                    grid[i][j][k]._neighbours.extend([ grid[x][y][z] for x in range(max(0, i - 1), min(self.size[0], i + 2)) for y in range(max(0, j - 1), min(self.size[1], j + 2)) for z in range(max(0, k - 1), min(self.size[2], k + 2)) if x != i or y != j or z != k
                                                     ])
