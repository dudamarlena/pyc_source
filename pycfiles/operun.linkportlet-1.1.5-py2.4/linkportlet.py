...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/operun/linkportlet/portlets/linkportlet.py
...
class ILinkPortlet(IPortletDataProvider):
    __module__ = __name__
    name = schema.TextLine(title=_('Title'), description=_('The title of the portlet. Will be displayed as header.'), required=True)
    links = schema.Tuple(title=_('Link List'), description=_('Links to be shown in the portlet.'), required=True, value_type=schema.Choice(vocabulary='operun.linkportlet.vocabularies.Links'))
...
class Assignment(base.Assignment):
    ...
    def __init__(self, name='Link Portlet', links=None):
        self.name = name
        self.links = links
...
class Renderer(base.Renderer):
    ...
    def _linkList(self):
        ...
        results = catalog(portal_type='operun Link', UID=self.data.links)
        if len(results) > 0:
            return results
        return []
            ...
            if brain.getRemoteUrl is not None:
                ...
                if compiled_regexp.match(remoteurl):
                    ...
                    if remoteurl is None:
                        continue
            link_list.append({'title': brain.Title.decode('latin-1').encode('latin-1'), 'url': remoteurl, 'desc': brain.Description.decode('latin-1').encode('latin-1'), 'target': brain.getLinkTarget == 'new' and '_blank' or ''})
...
class AddForm(base.AddForm):
    ...
    label = _('Add Link Portlet')
    description = _('This portlet displays a selection of Links.')

    def create(self, data):
        return Assignment(name=data.get('name', 'Link Portlet'), links=data.get('links', None))
