...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/pycopula/copula.py
# Compiled at: 2018-11-08 17:34:28
# Size of source mod 2**32: 25818 bytes
...
class Copula:
    ...
    def correlations(self, X):
        ...
        return (
         self.kendall, self.pearson, self.spearman)
    ...
    def cdf(self, x):
        ...
        if self.name == 'indep':
            return np.prod(x)
        if self.name == 'frechet_up':
            return min(x)
        if self.name == 'frechet_down':
            return max(sum(x) - self.dim + 1.0, 0)
    ...
    def pdf(self, x):
        ...
        if self.name in ('frechet_down', 'frechet_up'):
    ...
    def concentration_up(self, x):
        ...
        return (1.0 - 2 * x + self.cdf([x, x])) / (1.0 - x)
    ...
    def pdf_param(self, x, theta):
        ...
        if self.family == 'clayton':
            ...
            def claytonInvertnDerivative(t, theta, order):
                ...
                return -theta ** (order - 1) * product * (1.0 + theta * t) ** (-1.0 / theta - order)
            ...
            if self.dim == 2:
                ...
                def gumbelInvertDerivative(t, theta, order):
                    return 1.0 / theta ** 2 * t ** (1.0 / theta - 2.0) * (theta + t ** (1.0 / theta) - 1.0) * np.exp(-t ** (1.0 / theta))
                ...
                if self.family == 'frank':
                    if self.dim == 2:
                        for i in range(self.dim):
                            prod *= theta / (1.0 - np.exp(theta * x[i]))
                            sumInvert += self.generator(x[i], theta)
                    ...
                    if self.dim == 2:
                        for i in range(self.dim):
                            prod *= -theta * (1.0 - x[i]) ** (theta - 1.0) / (1.0 - (1.0 - x[i]) ** theta)
                            sumInvert += self.generator(x[i], theta)

                        def joeInvertDerivative(t, theta, order):
                            return 1.0 / theta ** 2 * (1.0 - np.exp(-t)) ** (1.0 / theta) * (theta * np.exp(t) - 1.0) / (np.exp(t) - 1.0) ** 2

                        invertNDerivative = joeInvertDerivative
        if self.family == 'amh' and self.dim == 2:
            for i in range(self.dim):
                prod *= (theta - 1.0) / (x[i] * (1.0 - theta * (1.0 - x[i])))
                sumInvert += self.generator(x[i], theta)

            def amhInvertDerivative(t, theta, order):
                return (1.0 - theta) * np.exp(t) * (theta + np.exp(t)) / (np.exp(t) - theta) ** 3
    ...
    def fit(self, X, method='cmle', verbose=False, theta_bounds=None, **kwargs):
        ...
        if method == 'moments':
            if self.kendall == None:
                self.correlations(X)
            if self.family == 'clayton':
                self.parameter = 2.0 * self.kendall / (1.0 - self.kendall)
            else:
                if self.family == 'gumbel':
                    self.parameter = 1.0 / (1.0 - self.kendall)
                else:
                    if self.family == 'frank':
        ...
        if bounds == None:
            ...
            res = estimation.cmle(log_likelihood, theta_start=theta_start, theta_bounds=bounds, optimize_method=kwargs.get('optimize_method', 'Brent'), bounded_optimize_method=kwargs.get('bounded_optimize_method', 'SLSQP'), is_scalar=is_scalar)
            self.parameter = res['x'] if is_scalar else res['x'][0]
        else:
            if method in ('mle', 'ifm'):
                if 'marginals' not in kwargs:
                    raise ValueError('Marginals distribution are required for MLE.')
                if 'hyper_param' not in kwargs:
    ...
    def pdf_param(self, x, R):
        ...
        if len(np.asarray(R).shape) <= 1:
            ...
            if self.dim == 2:
                ...
                RInv = 1.0 / RDet * np.asarray([[cov[1][1], -cov[0][1]], [-cov[0][1], cov[0][0]]])
        else:
        ...
        if method == 'cmle':
            ...
            def log_likelihood(rho):
                ...
                if self.dim == 2:
                    ...
                    RInv = 1.0 / RDet * np.asarray([[S[(1, 1)], -rho], [-rho, S[(0, 0)]]])
                else:
    ...
    def fit(self, X, method='cmle', df_fixed=False, verbose=True, **kwargs):
        ...
        if method == 'cmle':
            ...
            if not df_fixed:
                x_start = [
                 1.0] + x_start
            res = estimation.cmle(log_likelihood, theta_start=x_start, theta_bounds=None, optimize_method=kwargs.get('optimize_method', 'Nelder-Mead'), bounded_optimize_method=kwargs.get('bounded_optimize_method', 'SLSQP'))
            fitted_params = res['x']
        self.R = np.identity(self.dim)
        if df_fixed:
            nu = self.df
