...
# [GCC 8.4.0]
# Embedded file name: /Users/rcabanas/GoogleDrive/UAL/inferpy/repo/InferPy/inferpy/data/loaders.py
...
class DataLoader:
    __doc__ = ' This class defines the basic functionality of any DataLoader '
...
class CsvLoader(DataLoader):
    ...
    def __init__(self, path, var_dict=None, has_header=None, force_eager=False):
        ...
        self._map_batch_fn = self._CsvLoader__build_map_batch_fn(var_dict)
    ...
    def to_tfdataset(self, batch_size=None):
        if batch_size == None:
            ...
            if self.has_header:
                ...
                col_args = {'column_names':[
                  ''] + self._colnames, 
                 'select_columns':list(range(1, len(self._colnames) + 1))}
        return (tf.data.experimental.make_csv_dataset)(self._path, batch_size=batch_size, sloppy=True, 
         shuffle=self.shuffle_buffer_size > 1, shuffle_buffer_size=self.shuffle_buffer_size, **col_args)

    def to_dict(self):
        return dict(get_session().run(self.map_batch_fn(self.to_tfdataset().make_one_shot_iterator().get_next())))
...
class SampleDictLoader(DataLoader):
    ...
    def to_tfdataset(self, batch_size=None):
        if batch_size == None:
            batch_size = self.size
        return tf.data.Dataset.from_tensor_slices(self.sample_dict).shuffle(self.shuffle_buffer_size).batch(batch_size).repeat()
...
def build_data_loader(data):
    ...
    if isinstance(data, dict):
        ...
        if isinstance(data, SampleDictLoader):
            data_loader = data
        else:
            if isinstance(data, CsvLoader):
