...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pyhacc/BalanceDock.py
...
class BalanceSheetAccountBalances(AccountBalanceList):
    ...
    def query(self):
        self.qsub = Query((Splits.account_id.label('account_id'), func.sum(Splits.sum).label('sum'))).join(Accounts).join(AccountTypes).join(Transactions).filter(AccountTypes.balance_sheet == True).filter(AccountTypes.retained_earnings == False).group_by(Splits.account_id).subquery()
        self.pre_q = Query((Accounts.id, Accounts.name, AccountTypes.name.label('type'), self.qsub.c.sum.label('Balance'))).join((
         self.qsub, self.qsub.c.account_id == Accounts.id)).join(AccountTypes).filter(AccountTypes.balance_sheet == True).filter(AccountTypes.retained_earnings == False).filter(self.qsub.c.sum != decimal.Decimal()).order_by(AccountTypes.sort, Accounts.name)
        return self.pre_q
...
class ProfitAndLossAccountBalances(AccountBalanceList):
    ...
    def query(self):
        ...
        tranCriteria = expr.and_(Transactions.date >= datetime.datetime(year, 1, 1), Transactions.date <= datetime.datetime(year, 12, 31))
        self.qsub = Query((Splits.account_id.label('account_id'), func.sum(Splits.sum).label('sum'))).join(Accounts).join(AccountTypes).join(Transactions).filter(tranCriteria).filter(AccountTypes.balance_sheet == False).group_by(Splits.account_id).subquery()
        self.pre_q = Query((Accounts.id, Accounts.name, AccountTypes.name.label('type'), self.qsub.c.sum.label('Balance'))).join((
         self.qsub, self.qsub.c.account_id == Accounts.id)).join(AccountTypes).filter(AccountTypes.balance_sheet == False).filter(self.qsub.c.sum != decimal.Decimal()).order_by(AccountTypes.sort, Accounts.name)
        return self.pre_q