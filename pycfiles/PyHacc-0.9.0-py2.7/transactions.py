...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pyhacc/transactions.py
# Compiled at: 2014-02-05 08:16:29
...
class TransactionEditor(BoundDialog):
    ...
    def __init__(self, parent, row=None, Session=None, row_id=None, flush=True):
        ...
        self.splits_model = ClassTableModel(Splits, ('account', 'debit', 'credit',
                                                     'jrn_name'), readonly=False)
...
def TransactionsCalendar(Session):
    ...
    def load(obj):
        ...
        trans = s.query(Transactions).filter(expr.and_(Transactions.date >= day0, Transactions.date <= day1)).all()
        ...
        c.setEventList(trans, startDate=lambda x: x.date, endDate=lambda x: x.date, text=lambda x: x.payee if x.payee != '' else x.memo, bkColor=lambda x: QtGui.QColor(0, 0, 224))
        s.close()
