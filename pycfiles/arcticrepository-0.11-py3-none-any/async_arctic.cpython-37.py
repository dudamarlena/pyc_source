...
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/arctic/asynchronous/async_arctic.py
# Compiled at: 2019-02-28 12:42:09
# Size of source mod 2**32: 10271 bytes
import logging, time
...
class AsyncArctic(LazySingletonTasksCoordinator):
    ...
    def _verify_request(store, is_modifier, **kwargs):
        ...
        return (
         library_name, symbol, kind, callback, mongo_retry)
    ...
    def await_termination(self, timeout=None):
        while self.total_pending_requests() > 0:
            AsyncArctic.wait_requests((self.requests_by_id.values() + self.deferred_requests), do_raise=False,
              timeout=timeout)
    ...
    def wait_requests(requests, do_raise=False, timeout=None):
        if not AsyncArctic._wait_until_scheduled(requests, timeout):
            raise AsyncArcticException('Timed-out while waiting for request to be scheduled')
        while requests:
            all((r.is_completed for r in requests)) or AsyncArctic.wait_tasks((tuple((r.future for r in requests if not r.is_completed if r.future is not None))), timeout=timeout,
    ...
    def wait_any_request(requests, do_raise=False, timeout=None):
        if not AsyncArctic._wait_until_scheduled(requests, timeout):
            raise AsyncArcticException('Timed-out while waiting for request to be scheduled')
        while requests:
            any((r.is_completed for r in requests)) or AsyncArctic.wait_tasks((tuple((r.future for r in requests if not r.is_completed if r.future is not None))), timeout=timeout,
    ...
    def raise_first_errored(requests):
        errored = tuple((r for r in requests if r.is_completed if r.exception is not None))
        if errored:
            raise errored[0].exception

    @staticmethod
    def filter_errored(requests):
        return tuple((r for r in requests if r.is_completed if r.exception is not None))
