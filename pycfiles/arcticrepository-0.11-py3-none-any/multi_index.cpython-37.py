...
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/arctic/multi_index.py
...
import numpy as np, pandas as pd, six
...
def groupby_asof(df, as_of=None, dt_col='sample_dt', asof_col='observed_dt'):
    ...
    if as_of:
        if as_of.tzinfo is None:
            if df.index.get_level_values(asof_col).tz is not None:
                as_of = as_of.replace(tzinfo=(mktz()))
    return fancy_group_by(df, grouping_level=dt_col,
...
def multi_index_insert_row(df, index_row, values_row):
    ...
    df = pd.concat((df, row))
    if df.index.lexsort_depth == len(index_row):
        if df.index[(-2)] < df.index[(-1)]:
            return df
