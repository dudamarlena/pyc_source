...
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/arctic/asynchronous/_workers_pool.py
...
class LazySingletonTasksCoordinator(ABC):
    ...
    def wait_tasks(futures, timeout=None, return_when=ALL_COMPLETED, raise_exceptions=True):
        ...
        if raise_exceptions:
            [f.result() for f in done if not f.cancelled() if f.exception() is not None]
        ...
        try:
            ...
            try:
                if kill_switch_ev is not None:
                    kill_switch_ev.set()
                    LazySingletonTasksCoordinator.wait_tasks(futures, return_when=ALL_COMPLETED, raise_exceptions=False,
                      timeout=timeout)
                raise e
            finally:
                e = None
    ...
    def submit_task(self, is_looping, fun, *args, **kwargs):
        ...
        with type(self)._POOL_LOCK:
            ...
            self.alive_tasks = {k:v for k, v in iteritems(self.alive_tasks) if not v[0].done() if not v[0].done()}
            self.alive_tasks[new_id] = (
             new_future, shutdown_flag)
    ...
    def await_termination(self, timeout=None):
        ...
        LazySingletonTasksCoordinator.wait_tasks([v[0] for v in itervalues(self.alive_tasks)],
          timeout=timeout,
