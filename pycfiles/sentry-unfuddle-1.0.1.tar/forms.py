...
# [GCC 8.4.0]
# Embedded file name: /var/sentry-unfuddle/sentry_unfuddle/forms.py
# Compiled at: 2015-12-06 14:38:33
import logging
...
class UnfuddleOptionsForm(forms.Form):
    instance_url = forms.CharField(label=_('Unfuddle Instance URL'), widget=forms.TextInput(attrs={'class': 'span6', 'placeholder': 'e.g. "https://yourorg.unfuddle.com"'}), help_text=_('It must be visible to the Sentry server'), required=True)
    username = forms.CharField(label=_('Username'), widget=forms.TextInput(attrs={'class': 'span6'}), help_text=_('Ensure the Unfuddle user has admin perm. on the project'), required=True)
    password = forms.CharField(label=_('Password'), widget=forms.PasswordInput(attrs={'class': 'span6'}), help_text=_('Only enter a value if you wish to change it'), required=False)
    default_project_id = forms.ChoiceField(label=_('Linked Project'), required=True)
    ...
    def clean(self):
        ...
        if sut_response.status_code == 403 or sut_response.status_code == 401:
            ...
            raise ValidationError('Unable to connect to Unfuddle: %s, if you have tried and failed multiple times you may have to enter a CAPTCHA in Unfuddle to re-enable API logins.' % sut_response.status_code)
...
class UnfuddleIssueForm(forms.Form):
    ...
    assignee_id = forms.ChoiceField(label=_('Assignee'), required=True)
    milestone_id = forms.ChoiceField(label=_('Milestone'), required=True)
    priority = forms.ChoiceField(label=_('Priority'), required=True)
    title = forms.CharField(label=_('Issue Summary'), widget=forms.TextInput(attrs={'class': 'span6'}), required=True)
    description = forms.CharField(widget=forms.Textarea(attrs={'class': 'span6'}), required=True)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(milestones) is 0:
            ...
            self.errors['__all__'] = [
             'Error in Unfuddle configuration, no milestones found for user %s.' % unfuddle_client.username]
            return
        kwargs['initial'] = initial
        super(UnfuddleIssueForm, self).__init__(*args, **kwargs)
        self.fields['project_id'].initial = project_id
        self.fields['assignee_id'].choices = self.make_choices([ {'id': u['id'], 'name': ('{0} {1}').format(u['first_name'], u['last_name'])} for u in users ])
        self.fields['milestone_id'].choices = self.make_choices([ {'id': m['id'], 'name': m['title']} for m in milestones ])
