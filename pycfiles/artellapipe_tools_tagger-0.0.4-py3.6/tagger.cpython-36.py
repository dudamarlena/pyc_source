...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/tools/tagger/widgets/tagger.py
# Compiled at: 2020-03-13 14:17:16
...
class ArtellaTaggerWidget(artellapipe.ToolWidget):
    ...
    def create_new_tag_data_node_for_current_selection(self, asset_type=None):
        ...
        if not current_selection or current_selection == artellapipe.TagsMgr().TagDefinitions.SCENE_SELECTION_NAME or not tp.Dcc.object_exists(current_selection):
            ...
            if not artellapipe.TagsMgr().current_selection_has_metadata_node():
                if current_selection != artellapipe.TagsMgr().TagDefinitions.SCENE_SELECTION_NAME:
                    new_tag_data_node = tp.Dcc.create_node(node_type='network',
                      node_name=(artellapipe.TagsMgr().TagDefinitions.TAG_DATA_NODE_NAME))
                    ...
                    new_tag_data_node = tp.Dcc.create_node(node_type='network',
                      node_name=(artellapipe.TagsMgr().TagDefinitions.TAG_DATA_SCENE_NAME))
    ...
    def _create_editors(self):
        ...
        for available_editor in available_editors:
            if available_editor in loaded_editors:
                LOGGER.warning('Editor "{}" of type "{}" is already loaded. Skipping ...'.format(editor_class, editor_type))
            else:
    ...
    def _fill_new_tag_data_node(self, tag_data_node, current_selection):
        ...
        :param tag_data_node:
        """
        tp.Dcc.add_string_attribute(node=tag_data_node,
          attribute_name=(artellapipe.TagsMgr().TagDefinitions.TAG_TYPE_ATTRIBUTE_NAME))
        tp.Dcc.set_string_attribute_value(node=tag_data_node,
          attribute_name=(artellapipe.TagsMgr().TagDefinitions.TAG_TYPE_ATTRIBUTE_NAME),
          attribute_value=(self._project.tag_type_id))
        tp.Dcc.unkeyable_attribute(node=tag_data_node,
          attribute_name=(artellapipe.TagsMgr().TagDefinitions.TAG_TYPE_ATTRIBUTE_NAME))
        tp.Dcc.hide_attribute(node=tag_data_node,
          attribute_name=(artellapipe.TagsMgr().TagDefinitions.TAG_TYPE_ATTRIBUTE_NAME))
        tp.Dcc.lock_attribute(node=tag_data_node,
          attribute_name=(artellapipe.TagsMgr().TagDefinitions.TAG_TYPE_ATTRIBUTE_NAME))
        tp.Dcc.add_message_attribute(node=tag_data_node,
          attribute_name=(artellapipe.TagsMgr().TagDefinitions.NODE_ATTRIBUTE_NAME))
        if current_selection:
            if not tp.Dcc.attribute_exists(node=current_selection,
              attribute_name=(artellapipe.TagsMgr().TagDefinitions.TAG_DATA_ATTRIBUTE_NAME)):
                tp.Dcc.add_message_attribute(node=current_selection,
                  attribute_name=(artellapipe.TagsMgr().TagDefinitions.TAG_DATA_ATTRIBUTE_NAME))
            tp.Dcc.unlock_attribute(node=current_selection,
              attribute_name=(artellapipe.TagsMgr().TagDefinitions.TAG_DATA_ATTRIBUTE_NAME))
            tp.Dcc.unlock_attribute(node=tag_data_node,
              attribute_name=(artellapipe.TagsMgr().TagDefinitions.NODE_ATTRIBUTE_NAME))
            tp.Dcc.connect_attribute(tag_data_node, artellapipe.TagsMgr().TagDefinitions.NODE_ATTRIBUTE_NAME, current_selection, artellapipe.TagsMgr().TagDefinitions.TAG_DATA_ATTRIBUTE_NAME)
            tp.Dcc.lock_attribute(node=current_selection,
              attribute_name=(artellapipe.TagsMgr().TagDefinitions.TAG_DATA_ATTRIBUTE_NAME))
            tp.Dcc.lock_attribute(node=tag_data_node,
              attribute_name=(artellapipe.TagsMgr().TagDefinitions.NODE_ATTRIBUTE_NAME))
            tp.Dcc.select_object(current_selection)
