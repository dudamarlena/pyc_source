...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/thyme/model/fvcom.py
# Compiled at: 2020-04-06 09:21:33
# Size of source mod 2**32: 18759 bytes
"""
...
class FVCOMIndexFile(model.ModelIndexFile):
    ...
    def init_nc(self, model_file, target_cellsize_meters, ofs_model, shoreline_shp=None, subset_grid_shp=None, subset_grid_field_name=None):
        ...
        var_siglay_centroid = self.nc_file.createVariable('siglay_centroid', 'f4', ('siglay',
                                                                                    'nele'), fill_value=FILLVALUE)
...
class FVCOMFile(model.ModelFile):
    ...
    def sigma_to_centroid(self, vertical_coordinates):
        ...
        return (
         siglay_centroid, self.var_lat_centroid, self.var_lon_centroid, self.num_nele, self.num_siglay)
    ...
    def uv_to_regular_grid(self, model_index, time_index, target_depth, interp_method=interp.INTERP_METHOD_SCIPY):
        ...
        h_centroid, zeta_centroid = node_to_centroid(self.var_zeta, self.var_h, self.var_lon_nodal, self.var_lat_nodal, self.var_lon_centroid, self.var_lat_centroid, time_index)
        siglay_centroid = model_index.nc_file.variables['siglay_centroid'][:, :]
        u_target_depth, v_target_depth = vertical_interpolation(self.var_u, self.var_v, h_centroid, zeta_centroid, siglay_centroid, self.num_nele, self.num_siglay, time_index, target_depth)
        return interp.interpolate_to_regular_grid((u_target_depth, v_target_depth), (self.var_lon_centroid),
          (self.var_lat_centroid), (model_index.var_x),
    ...
    def output_native_grid(self, time_index, target_depth):
        ...
        u_target_depth, v_target_depth = vertical_interpolation(self.var_u, self.var_v, h_centroid, zeta_centroid, siglay_centroid[0], self.num_nele, self.num_siglay, time_index, target_depth)
        return (
         u_target_depth, v_target_depth, self.var_lat_centroid, self.var_lon_centroid)
