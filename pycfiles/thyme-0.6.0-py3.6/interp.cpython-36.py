...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/thyme/util/interp.py
...
def interpolate_to_regular_grid(values, x_in, y_in, x_out, y_out, interp_method=INTERP_METHOD_SCIPY):
    ...
    if interp_method == INTERP_METHOD_GDAL:
...
def gdal_interpolate_to_regular_grid(values, x_in, y_in, x_out, y_out):
    ...
    calc_values = []
    for n in range(len(values)):
        calc_values.append(gdal.Grid(('.tif'.format(field_name(n + 1))), ds, format='MEM', width=(len(x_out)),
          height=(len(y_out)),
