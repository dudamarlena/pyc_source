...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\PyUSMC\_PyUSMC.py
# Compiled at: 2018-01-18 03:49:35
...
"""
import time
from ctypes import WinDLL, c_int, c_float, c_char_p, POINTER, c_char, Structure, wintypes, create_string_buffer, c_size_t
__all__ = [
 'StepperMotorController', 'RotationalStage']
...
class _SettingsBase(Structure):
    ...
    def Get(self, variable):
        ...
        """
        allowedKeys, _ = zip(*self._fields_)
        if variable not in allowedKeys:
...
class StepperMotorController:
    ...
    def __init__(self):
        self.motors = []
        self._dll = WinDLL('USMCDLL.dll')
        self._dll.USMC_Init.argtypes = [
         USMC_Devices]
        self._dll.USMC_Init.restype = wintypes.DWORD
        self._dll.USMC_GetState.argtypes = [
         wintypes.DWORD, USMC_State]
        self._dll.USMC_GetState.restype = wintypes.DWORD
        self._dll.USMC_SaveParametersToFlash.argtypes = [
         wintypes.DWORD]
        self._dll.USMC_SaveParametersToFlash.restype = wintypes.DWORD
        self._dll.USMC_SetCurrentPosition.argtypes = [
         wintypes.DWORD, c_int]
        self._dll.USMC_SetCurrentPosition.restype = wintypes.DWORD
        self._dll.USMC_GetMode.argtypes = [
         wintypes.DWORD, USMC_Mode]
        self._dll.USMC_GetMode.restype = wintypes.DWORD
        self._dll.USMC_SetMode.argtypes = [
         wintypes.DWORD, USMC_Mode]
        self._dll.USMC_SetMode.restype = wintypes.DWORD
        self._dll.USMC_GetParameters.argtypes = [
         wintypes.DWORD, USMC_Parameters]
        self._dll.USMC_GetParameters.restype = wintypes.DWORD
        self._dll.USMC_SetParameters.argtypes = [
         wintypes.DWORD, USMC_Parameters]
        self._dll.USMC_SetParameters.restype = wintypes.DWORD
        self._dll.USMC_GetStartParameters.argtypes = [
         wintypes.DWORD, USMC_StartParameters]
        self._dll.USMC_GetStartParameters.restype = wintypes.DWORD
        self._dll.USMC_Start.argtypes = [
         wintypes.DWORD, c_int, POINTER(c_float), USMC_StartParameters]
        self._dll.USMC_Start.restype = wintypes.DWORD
        self._dll.USMC_Stop.argtypes = [
         wintypes.DWORD]
        self._dll.USMC_Stop.restype = wintypes.DWORD
        self._dll.USMC_GetLastErr.argtypes = [
         c_char_p, c_size_t]
        self._dll.USMC_Close.argtypes = []
        self._dll.USMC_Close.restype = wintypes.DWORD
        self._dll.USMC_GetEncoderState.argtypes = [
         wintypes.DWORD, USMC_EncoderState]
...
class Motor:
    ...
    def Start(self, destPos, speed=None):
        ...
        errCode = self._dll.USMC_Start(self.index, self.position.ToUSMCPos(destPos), speed, self.startParameters)
        self._controller.ProcessErrorCode(errCode)
        return
