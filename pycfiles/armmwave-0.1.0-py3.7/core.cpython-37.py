...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/armmwave/core.py
# Compiled at: 2019-11-15 15:45:08
# Size of source mod 2**32: 13897 bytes
"""
...
def rt_amp(index, delta, theta, pol):
    ...
    m_prime = make_2x2(1.0, 0.0, 0.0, 1.0, dtype=complex)
    for i in range(1, len(index) - 1):
        m_prime = np.dot(m_prime, m_mat[i])

    C_m = make_2x2(1.0, (r_amp[(0, 1)]), (r_amp[(0, 1)]), 1.0, dtype=complex)
...
def make_m_matrix(index, t_matrix, r_matrix, delta):
    ...
    for i in range(1, len(index) - 1):
        C_m = make_2x2((np.exp(complex(-0.0, -1.0) * delta[i])), 0.0, 0.0, (np.exp(complex(0.0, 1.0) * delta[i])), dtype=complex)
        r_m = make_2x2(1.0, (r_matrix[(i, i + 1)]), (r_matrix[(i, i + 1)]), 1.0, dtype=complex)
...
def wavenumber(freq, index, tand):
    ...
    """
    k = 2 * np.pi * (freq / 300000000.0) * index * np.sqrt(1 + complex(0.0, 1.0) * tand)
    return k
...
def alpha2imagn(freq, a, b, n):
    ...
    """
    nu = freq / 30000000000.0
    alpha = 2 * a * nu ** b
    imagn = 30000000000.0 * alpha / (4 * np.pi * n * freq)
    return imagn
...
def main(params):
    ...
    results = {'frequency':freq, 
     'transmittance':ts,  'reflectance':rs}
    return results