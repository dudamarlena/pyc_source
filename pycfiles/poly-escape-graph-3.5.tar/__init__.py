...
# Embedded file name: build/bdist.linux-i686/egg/poly/__init__.py
# Compiled at: 2007-03-25 20:29:14
__version__ = '$Revision: 1.43 $'
import cPickle, errno, exceptions, itertools, logging, os, popen2, shutil, sys, tempfile, time, warnings, optbuild
...
REMOTE_COPY_TRIES = 4
__all__ = [
 'jobid', 'jobname,jobindex',
 'jobindex_end,firstjob',
 'lastjob,headnode,localfile',
 'local', 'file_hostname,copy',
 'makedirs,gettempdir',
 'mkdtemp', 'NamedTemporaryFile', 'TemporaryFile,superglobal,chunk',
 'substitute',
...
def copy(*filenames):
    ...
    """
    (remote_copy_args, remote) = zip(*map(_remote_copy_arg, map(os.path.abspath, filenames)))
    if True in remote:
...
def chunk(collection):
    ...
    if _already_chunked:
        warnings.warn('poly.chunk() called earlier in this process -- refusing to chunk further', RuntimeWarning, stacklevel=2)
        return collection
...
def _init_localfile_cmds(default):
    ...
    return _try_each(KeyError, _get_env_poly_localfile_cmds, _get_default)
...
def _init_groupindex():
    ...
    return (
     groupindex, groupindex_end)


_init_logging()
headnode = _try_each(KeyError, _environgetter('POLY_STORAGE_HOST'), lsf._init_headnode, standalone._init_headnode)
(groupindex, groupindex_end) = _init_groupindex()
(jobindex, jobindex_end, firstjob, lastjob, _chunk) = _try_each(KeyError, lsf._init_job_array, standalone._init_job_array)
(jobid, jobname, local, gettempdir, NamedTemporaryFile, TemporaryFile, _localfile_cmds) = _try_each(KeyError, lsf._init_job, standalone._init_job)
_superglobals = _try_each(KeyError, _init_superglobal_load_true, _init_superglobal_load_false)
(firstjob, lastjob, _write_superglobals) = _try_each(KeyError, _init_superglobal_save_true, _init_superglobal_save_false)
localfilename = local
