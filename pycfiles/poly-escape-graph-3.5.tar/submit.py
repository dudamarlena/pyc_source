...
# Embedded file name: build/bdist.linux-i686/egg/poly/submit.py
# Compiled at: 2006-09-08 05:35:20
from __future__ import division
__version__ = '$Revision: 1.8 $'
import errno, os, subprocess, sys
from path import path
from autolog import autolog
import optbuild, poly, poly.watch
_log = autolog()
POLYJOB_CMD = 'polyjob'
...
def bsub_polycat(jobid, jobname, job_dirpath):
    return BSUB_PROGRAM.getoutput('polycat', job_dirpath, options.stdout_filename, options.stderr_filename, q='small', w='ended(%s)' % jobid, J=make_polycat_jobname(jobname))
...
def get_destination_host():
    return poly._try_each(KeyError, poly._environgetter('POLY_STORAGE_HOST'), poly._environgetter('HOSTNAME'), get_current_host)
...
def get_destination_dirpath():
    try:
        ...
        if len(destination_split) != 2:
            _log.die('Environment variable POLY_DESTINATION must contain only one colon')
    ...
    for dirname in ['hostinfo', 'stdout', 'stderr', 'tar']:
        ...
        try:
            ...
            if err.errno == errno.EEXIST:
                _log.die('destination store %s already exists; use %s --remove to remove it', dirpath, sys.argv[0])
            else:
                _log.die("can't create destination store %s: %s", dirpath, err.strerror)
...
def make_superglobals(program_cmdline, job_dirpath):
    ...
    if returncode:
        _log.die('%s failed with return code %s', program_cmdline[0], returncode)
...
def parse_options(args):
    ...
    usage = '%prog [POLYSUB-OPTION]... -J JOBNAME [BSUB-OPTION]... -- COMMAND [COMMAND-ARG]...'
    version = '%%prog %s' % __version__
    parser = OptionParser(usage=usage, version=version)
    parser.add_option('', '--output', dest='stdout_filename', default=None, help='write standard output to FILE', metavar='FILE')
    parser.add_option('', '--error', dest='stderr_filename', default=None, help='write standard error to FILE', metavar='FILE')
    parser.add_option('', '--superglobal', action='store_true', help='generate superglobals interactively before submitting batch job array')
    parser.add_option('', '--remove', action='store_true', help='remove existing desination files before running')
    parser.add_option('', '--no-makedirs', action='store_true', help='do not make job directories')
