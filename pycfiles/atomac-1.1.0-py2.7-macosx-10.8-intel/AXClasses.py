...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/atomac/AXClasses.py
# Compiled at: 2013-02-12 15:31:40
import fnmatch, AppKit, Quartz, time
from PyObjCTools import AppHelper
from collections import deque
from . import _a11y
import AXKeyboard, AXCallbacks, AXKeyCodeConstants
   ...
   """
    ...
    def launchAppByBundleId(bundleID):
      ...
      """
        ...
        ws = AppKit.NSWorkspace.sharedWorkspace()
        r = ws.launchAppWithBundleIdentifier_options_additionalEventParamDescriptor_launchIdentifier_(bundleID, NSWorkspaceLaunchAllowingClassicStartup, AppKit.NSAppleEventDescriptor.nullDescriptor(), None)
        if r[0] == False:
    ...
    def _queueMouseButton(self, coord, mouseButton, modFlags, clickCount=1):
        """ Private method to handle generic mouse button clicking
          ...
          Returns: None
      """
        mouseButtons = {Quartz.kCGMouseButtonLeft: 'LeftMouse', 
        ...
        buttonDown = Quartz.CGEventCreateMouseEvent(None, eventButtonDown, coord, mouseButton)
        Quartz.CGEventSetFlags(buttonDown, modFlags)
        Quartz.CGEventSetIntegerValueField(buttonDown, Quartz.kCGMouseEventClickState, int(clickCount))
        buttonUp = Quartz.CGEventCreateMouseEvent(None, eventButtonUp, coord, mouseButton)
        Quartz.CGEventSetFlags(buttonUp, modFlags)
        Quartz.CGEventSetIntegerValueField(buttonUp, Quartz.kCGMouseEventClickState, int(clickCount))
      ...
      """
        ...
        if 'callback' in kwargs:
            ...
            callbackArgs = (
             retelem,)
            callbackKwargs = kwargs
        return self._setNotification(timeout, notification, callback, callbackArgs, callbackKwargs)
   ...
   """
    ...
    def waitForWindowToDisappear(self, winName, timeout=10):
      ...
      """
        ...
        return self.waitFor(timeout, 'AXUIElementDestroyed', callback=callback, args=args, AXRole='AXWindow', AXTitle=winName)
    ...
    def waitForValueToChange(self, timeout=10):
      ...
      """
        callback = AXCallbacks.returnElemCallback
        retelem = None
        return self.waitFor(timeout, 'AXValueChanged', callback=callback, args=(
         retelem,))
    ...
    def waitForFocusToChange(self, newFocusedElem, timeout=10):
        """ Convenience method to wait for focused element to change (to new
          ...
          Returns: Boolean
      """
        return self.waitFor(timeout, 'AXFocusedUIElementChanged', AXRole=newFocusedElem.AXRole, AXPosition=newFocusedElem.AXPosition)
