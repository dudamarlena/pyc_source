...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/atomac/ldtpd/text.py
# Compiled at: 2013-02-13 13:37:18
"""Text class."""
import re, fnmatch, atomac.Clipboard as Clipboard
...
class Text(Utils):
    ...
    def inserttext(self, window_name, object_name, position, data):
        ...
        if position > size:
            position = size
        object_handle.AXValue = existing_data[:position] + data + existing_data[position:]
        return 1
    ...
    def verifysettext(self, window_name, object_name, text):
        ...
        """
        try:
            return int(re.match(fnmatch.translate(text), self.gettextvalue(window_name, object_name)))
        except:
            return 0
