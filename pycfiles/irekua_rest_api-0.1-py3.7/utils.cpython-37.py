...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/irekua_rest_api/tests/utils.py
...
def create_or_get_user(is_superuser=None, is_developer=None, is_curator=None, is_model=None):
    user, _ = User.objects.get_or_create(username=(uuid4()),
      defaults=dict(password=(uuid4()),
...
class Actions:
    ...
    ALL_ACTIONS = [
     ...
     PARTIAL_UPDATE,
     DESTROY]
    METHOD_MAPPING = {LIST: 'get', 
     CREATE: 'post', 
     RETRIEVE: 'get', 
     UPDATE: 'put', 
     PARTIAL_UPDATE: 'patch', 
     DESTROY: 'delete'}
    STATUS_CODE_MAPPING = {LIST: status.HTTP_200_OK, 
...
class BaseTestCase(object):
    VIEW_NAME_MAPPING = {Actions.LIST: 'list', 
     Actions.CREATE: 'list', 
    ...
    def setUp(self):
        self.admin_user = create_or_get_user(is_superuser=True,
          is_developer=False,
          is_curator=False,
          is_model=False)
        self.regular_user = create_or_get_user(is_superuser=False,
          is_developer=False,
          is_curator=False,
          is_model=False)
        self.model_user = create_or_get_user(is_superuser=False,
          is_developer=False,
          is_curator=False,
          is_model=True)
        self.developer_user = create_or_get_user(is_superuser=False,
          is_developer=True,
          is_curator=False,
          is_model=False)
        self.curator_user = create_or_get_user(is_superuser=False,
          is_developer=False,
          is_curator=True,
          is_model=False)
        ...
        self.collection_user = create_or_get_user(is_superuser=False,
          is_developer=False,
          is_curator=False,
          is_model=False)
        self.collection.add_user((self.collection_user),
          role=(self.role),
          metadata={})
    ...
    def change_user(self, usertype):
        if usertype is Users.NON_AUTHENTICATED:
            ...
            return
        mapping = {Users.USER: self.regular_user, 
    ...
    def get_url_name(self, action, view_name=None):
        ...
        url_name = base_url.format(class_name=class_name,
          view_name=view_name)
        return url_name
