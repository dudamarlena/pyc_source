...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/irekua_rest_api/views/items/items.py
...
class ItemViewSet(mixins.UpdateModelMixin, mixins.RetrieveModelMixin, mixins.DestroyModelMixin, mixins.ListModelMixin, utils.CustomViewSetMixin, GenericViewSet):
    queryset = models.Item.objects.all()
    filterset_class = filters.items.Filter
    search_fields = filters.items.search_fields
    serializer_mapping = utils.SerializerMapping.from_module(serializers.items.items).extend(annotations=(serializers.annotations.annotations.ListSerializer),
    ...
    permission_mapping = utils.PermissionMapping({utils.Actions.UPDATE: [
     ...
     utils.Actions.RETRIEVE: [
                              IsAuthenticated,
                              permissions.IsCreator | permissions.HasViewPermission | permissions.IsCollectionAdmin | permissions.IsCollectionTypeAdmin | permissions.ItemIsOpenToView | IsSpecialUser], 
     ...
     'annotations': [
                     IsAuthenticated,
                     permissions.IsCreator | permissions.HasViewAnnotationsPermission | permissions.IsCollectionAdmin | permissions.IsCollectionTypeAdmin | permissions.ItemIsOpenToViewAnnotations | IsSpecialUser], 
     ...
     'tag_item': [
                  IsAuthenticated,
                  permissions.IsCreator | IsCurator | IsAdmin], 
     ...
     'download': [
                  IsAuthenticated,
                  permissions.IsCreator | permissions.HasDownloadPermission | permissions.IsCollectionAdmin | permissions.IsCollectionTypeAdmin | permissions.ItemIsOpenToDownload | IsSpecialUser], 
     ...
     'add_secondary_item': [
                            IsAuthenticated,
                            permissions.IsCreator | IsSpecialUser]},
    ...
    def get_queryset(self):
        ...
        if self.action == 'annotations':
            item_id = self.kwargs['pk']
            return models.Annotation.objects.filter(item=item_id)
        if self.action == 'secondary_items':
            item_id = self.kwargs['pk']
            return models.SecondaryItem.objects.filter(item=item_id)
    ...
    def upload(self, request, pk=None):
        ...
        if item.item_file.name != '':
            ...
            return serializer.is_valid() or Response('Invalid file', status=(status.HTTP_400_BAD_REQUEST))
        serializer.save()
    ...
    def get_list_queryset(self):
        try:
            ...
            is_special_user = user.is_superuser | user.is_curator | user.is_model | user.is_developer
            if is_special_user:
                return self.get_full_queryset()
            return self.get_normal_queryset(user)

    def get_normal_queryset(self, user):
        is_open = Q(licence__is_active=False) | Q(licence__licence_type__can_view=True)
        is_owner = Q(created_by=(user.pk))
        perm = Permission.objects.get(codename='view_collection_items')
        collections_with_permission = models.CollectionUser.objects.filter(user=(user.pk),
          role__in=(perm.role_set.all())).values('collection')
        is_in_allowed_collection = Q(sampling_event_device__sampling_event__collection__in=collections_with_permission)
        filter_query = is_open | is_owner | is_in_allowed_collection
        queryset = models.Item.objects.filter(filter_query)
        return queryset
