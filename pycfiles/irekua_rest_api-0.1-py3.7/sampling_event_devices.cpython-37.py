...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/irekua_rest_api/views/sampling_events/sampling_event_devices.py
...
class SamplingEventDeviceViewSet(mixins.UpdateModelMixin, mixins.RetrieveModelMixin, mixins.DestroyModelMixin, utils.CustomViewSetMixin, GenericViewSet):
    queryset = models.SamplingEventDevice.objects.all()
    filterset_class = filters.sampling_event_devices.Filter
    search_fields = filters.sampling_event_devices.search_fields
    serializer_mapping = utils.SerializerMapping.from_module(serializers.sampling_events.devices).extend(items=(serializers.items.items.ListSerializer),
    ...
    def get_queryset(self):
        if self.action == 'items':
            ...
            return models.Item.objects.filter(sampling_event_device=object_id)
