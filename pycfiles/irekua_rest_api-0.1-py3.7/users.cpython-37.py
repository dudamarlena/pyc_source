...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/irekua_rest_api/views/users/users.py
# Compiled at: 2019-10-28 01:58:06
# Size of source mod 2**32: 7348 bytes
...
class UserViewSet(mixins.ListModelMixin, mixins.UpdateModelMixin, mixins.CreateModelMixin, mixins.RetrieveModelMixin, utils.CustomViewSetMixin, GenericViewSet):
    queryset = models.User.objects.all()
    filterset_class = filters.users.Filter
    search_fields = filters.users.search_fields
    permission_mapping = utils.PermissionMapping({utils.Actions.CREATE: IsUnauthenticated, 
     utils.Actions.UPDATE: [
                            IsAuthenticated,
                            permissions.IsSelf | IsAdmin]},
      default=IsAuthenticated)
    serializer_mapping = utils.SerializerMapping.from_module(serializers.users.users).extend(items=(serializers.items.items.ListSerializer),
    ...
    def get_queryset(self):
        ...
        if self.action == 'items' or self.action == 'item_locations':
            ...
            return models.Item.objects.filter(created_by=user_id)
        if self.action == 'devices':
            user_id = self.kwargs['pk']
            return models.PhysicalDevice.objects.filter(owner=user_id)
        if self.action == 'sites' or self.action == 'site_locations':
            user_id = self.kwargs['pk']
            return models.Site.objects.filter(created_by=user_id)
