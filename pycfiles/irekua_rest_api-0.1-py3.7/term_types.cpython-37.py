...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/irekua_rest_api/views/object_types/term_types.py
...
class TermTypeViewSet(utils.CustomViewSetMixin, ModelViewSet):
    queryset = models.TermType.objects.all()
    search_fields = filters.term_types.search_fields
    filterset_class = filters.term_types.Filter
    serializer_mapping = utils.SerializerMapping.from_module(serializers.object_types.terms).extend(terms=(serializers.terms.terms.ListSerializer),
    ...
    permission_mapping = utils.PermissionMapping({utils.Actions.DESTROY: [IsAuthenticated, IsAdmin], 
    ...
    def get_queryset(self):
        if self.action == 'entailments':
            return models.Entailment.objects.all()
        if self.action == 'entailment_types':
            return models.EntailmentType.objects.all()
        if self.action == 'terms':
            term_type_id = self.kwargs['pk']
            return models.Term.objects.filter(term_type=term_type_id)
        if self.action == 'synonyms':
            term_type_id = self.kwargs['pk']
            return models.Synonym.objects.filter(source__term_type=term_type_id)
        if self.action == 'suggestions':
            term_type_id = self.kwargs['pk']
            return models.TermSuggestion.objects.filter(term_type=term_type_id)
        if self.action == 'synonym_suggestions':
            term_type_id = self.kwargs['pk']
            return models.SynonymSuggestion.objects.filter(source__term_type=term_type_id)
