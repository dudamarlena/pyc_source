...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/irekua_rest_api/tests/test_physical_devices.py
...
class PhysicalDevicesTestCase(BaseTestCase, APITestCase):
    ...
    def setUp(self):
        ...
        self.random_user = User.objects.create(username=(str(uuid4())),
          password=(str(uuid4())))
        device_type = DeviceType.objects.create(name=(str(uuid4())),
          description='Random device type')
        device_brand = DeviceBrand.objects.create(name=(str(uuid4())))
        self.device = Device.objects.create(device_type=device_type,
          brand=device_brand,
          model=(str(uuid4())),
