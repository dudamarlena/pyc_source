...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/irekua_rest_api/views/data_collections/data_collections.py
# Compiled at: 2019-10-28 01:58:06
# Size of source mod 2**32: 14647 bytes
...
class CollectionViewSet(utils.CustomViewSetMixin, ModelViewSet):
    ...
    serializer_mapping = utils.SerializerMapping.from_module(serializers.data_collections.data_collections).extend(metacollections=(serializers.data_collections.metacollections.ListSerializer),
    ...
    permission_mapping = utils.PermissionMapping({utils.Actions.UPDATE: [
                            IsAuthenticated,
                            permissions.HasUpdatePermission | permissions.IsCollectionAdmin | permissions.IsCollectionTypeAdmin | IsAdmin], 
     ...
     'licences': [
                  IsAuthenticated,
                  permissions.IsCollectionUser | IsSpecialUser], 
     
     'add_licence': [
                     IsAuthenticated,
                     permissions.HasAddLicencePermission | permissions.IsCollectionAdmin], 
     
     'devices': [
                 IsAuthenticated,
                 permissions.IsCollectionUser | permissions.IsCollectionTypeAdmin | IsSpecialUser], 
     
     'add_device': [
                    IsAuthenticated,
                    permissions.HasAddDevicePermission | permissions.IsCollectionAdmin | IsAdmin], 
     ...
     'add_site': [
                  IsAuthenticated,
                  permissions.HasAddSitePermission | permissions.IsCollectionAdmin | permissions.IsCollectionTypeAdmin | IsAdmin], 
     ...
     'add_user': [
                  IsAuthenticated,
                  permissions.HasAddUserPermission | permissions.IsCollectionAdmin | permissions.IsCollectionTypeAdmin | IsAdmin], 
     ...
     'add_sampling_event': [
                            IsAuthenticated,
                            permissions.HasAddItemPermission | permissions.IsCollectionAdmin | permissions.IsCollectionTypeAdmin | IsAdmin], 
    ...
    def get_queryset(self):
        if self.action == 'metacollections':
            return models.MetaCollection.objects.all()
        if self.action == 'licences':
            collection_id = self.kwargs['pk']
            return models.Licence.objects.filter(collection=collection_id)
        if self.action == 'devices':
            collection_id = self.kwargs['pk']
            return models.CollectionDevice.objects.filter(collection=collection_id)
        if self.action == 'sites':
            collection_id = self.kwargs['pk']
            return models.CollectionSite.objects.filter(collection=collection_id)
        if self.action == 'users':
            collection_id = self.kwargs['pk']
            return models.CollectionUser.objects.filter(collection=collection_id)
        if self.action == 'sampling_events':
            collection_id = self.kwargs['pk']
            return models.SamplingEvent.objects.filter(collection=collection_id)
        if self.action == 'types':
            return models.CollectionType.objects.all()
        if self.action == 'licence_types':
            return models.LicenceType.objects.all()
        if self.action == 'items':
            collection_id = self.kwargs['pk']
            return models.Item.objects.filter(sampling_event_device__sampling_event__collection=collection_id)
    ...
    def site_locations(self, request, pk=None):
        ...
        serializer = serializers.sites.CollectionSiteLocationSerializer(queryset,
          many=True,
          read_only=True)
    ...
    def sampling_event_locations(self, request, pk=None):
        queryset = self.filter_queryset(self.get_queryset())
        serializer = serializers.sites.SamplingEventLocationSerializer(queryset,
          many=True,
          read_only=True)
        return Response(serializer.data)