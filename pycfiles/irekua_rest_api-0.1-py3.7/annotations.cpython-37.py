...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/irekua_rest_api/views/annotations/annotations.py
...
class AnnotationViewSet(mixins.UpdateModelMixin, mixins.RetrieveModelMixin, mixins.DestroyModelMixin, mixins.ListModelMixin, utils.CustomViewSetMixin, GenericViewSet):
    ...
    serializer_mapping = utils.SerializerMapping.from_module(serializers.annotations.annotations).extend(vote=(serializers.annotations.votes.CreateSerializer),
    ...
    permission_mapping = utils.PermissionMapping({utils.Actions.UPDATE: [
                            IsAuthenticated,
                            permissions.IsCreator | permissions.HasUpdatePermission | IsCurator | IsAdmin], 
     ...
     'vote': [
              IsAuthenticated,
              permissions.IsCreator | permissions.HasVotePermission | IsCurator | IsAdmin], 
    ...
    def get_list_queryset(self):
        ...
        return open_queryset.union(collection_type_queryset, collection_admin_queryset, view_pemission_queryset)
