...
# Embedded file name: build/bdist.linux-i686/egg/pycrud/scripts/receive.py
# Compiled at: 2008-06-20 03:40:59
__doc__ = 'receive.py - script that processes messages received thru kannel\n\nAUTHOR: Emanuel Gardaya Calso\n\n'
import urllib
from base import *
import message_filter, message_response
recipient = NUMBER

def save(sender, msg):
    entry = model.Message(recipient=recipient, sender=sender, message=msg, folder=INBOX)
...
def reply(sender, msg):
    entry = model.Message(folder=3, message=msg, sender=recipient, recipient=sender)
    model.Session.save(entry)
    model.Session.commit()
...
def main():
    ...
    sender = urllib.unquote_plus(argv[1])
    msg = urllib.unquote_plus(argv[2])
    msg = msg.replace('�', ' ').replace('û', ' ').replace('ü', ' ')
