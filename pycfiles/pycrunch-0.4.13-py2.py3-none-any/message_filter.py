...
# Embedded file name: build/bdist.linux-i686/egg/pycrud/scripts/message_filter.py
# Compiled at: 2008-06-20 03:40:59
__doc__ = 'filter_message.py - tool to filter a given message entry\n\nAUTHOR: Emanuel Gardaya Calso\n\n'
import os, commands, urllib
from base import *

def go_thru_filters(entry):
    print 'go_thru_filters'
...
def reply(received, message):
    ...
    msg_len = len(message)
    if msg_len == 0:
        send(sender, recipient, 'Got no response.')
    elif msg_len <= 160:
...
def send(sender, recipient, message):
    entry = model.Message(folder=OUTBOX, message=message, sender=sender, recipient=recipient)
