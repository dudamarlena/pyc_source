...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mk2/events/server.py
# Compiled at: 2013-08-16 22:15:55
import re
from . import Event, get_timestamp
output_exp = re.compile('(\\d{4}-\\d{2}-\\d{2} |)(\\d{2}:\\d{2}:\\d{2}) \\[([A-Z]+)\\] (?:%s)?(.*)' % ('|').join(re.escape(x) for x in ('[Minecraft] ',
                                                                                                                                        '[Minecraft-Server] ')))
