...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mk2/plugins/script.py
# Compiled at: 2013-08-16 22:15:55
import re, os.path, pwd
...
class ScriptEntry(object):
    ...
    def __init__(self, plugin, line):
        ...
        if line.startswith('@'):
            self.type = 'event'
            event_name, command = re.match('^@([^\\s]+)\\s+(.+)$', line).groups()
    ...
    def parse_time(self, time_spec):
        ...
        for spec_i, bound_i in zip(time_spec, time_bounds):
            n, top, skip = re.match('^(\\d{1,2}|\\*)(?:-(\\d{1,2}))?(?:/(\\d{1,2}))?$', spec_i).groups()
            if n == '*':
                if top:
    ...
    def step(self):
        ...
        for r, t in zip(self.ranges, time):
            if t not in range(r.min, r.max + 1, r.skip):
                return
...
class Script(Plugin):
    ...
    def setup(self):
        ...
        for script in self.scripts:
            if script.type == 'time':
                self.delayed_task(lambda a: self.repeating_task(self.step, 60, now=True), max(0, 60 - localtime().tm_sec) % 60 + 1)
                break
