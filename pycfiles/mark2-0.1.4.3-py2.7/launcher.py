...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mk2/launcher.py
# Compiled at: 2013-08-16 22:15:55
import re, os, sys, glob, stat, time, errno, subprocess, getpass, pwd, tempfile
from . import manager
...
class CommandTySelective(CommandTyStateful):
    options_spec = (
     (
      'name', ('-n', '--name'), 'NAME', 'create or select a server with this name'),)
...
class CommandHelp(Command):
    ...
    def run(self):
        if self.value is None:
            print help_text.format(usage=usage_text, commands=self.columns([ (c.name, c.value_spec, c.__doc__) for c in commands ]))
        elif self.value in commands_d:
            cls = commands_d[self.value]
            print help_sub_text.format(subcommand=self.value, doc=cls.__doc__, value_spec=cls.value_spec)
...
class CommandStart(CommandTyTerminal):
    ...
    def check_ownership(self):
        ...
        if d_user != m_user:
            e = "server directory is owned by '{d_user}', but mark2 is running as '{m_user}'. " + 'please start mark2 as `sudo -u {d_user} mark2 start ...`'
...
class CommandConfig(Command):
    ...
    options_spec = (
     (
      'ask', ('-a', '--ask'), '', 'Ask before starting an editor'),)
    name = 'config'

    def check_executable(self, cmd):
        return subprocess.call([
         'type', cmd], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE) == 0
...
commands = (
 CommandHelp, CommandStart, CommandList, CommandAttach, CommandStop, CommandKill, CommandSend, CommandJarList, CommandJarGet, CommandConfig)
