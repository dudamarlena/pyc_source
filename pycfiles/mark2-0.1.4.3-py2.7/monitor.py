...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mk2/plugins/monitor.py
...
class Check(object):
    ...
    def step(self):
        ...
        if self.timeout and self.time == self.timeout:
            ...
            self.dispatch(ServerEvent(cause='server/error/' + self.event[0], data='REBOOTING SERVER: ' + self.event[1].format(timeout=timeout), priority=1))
            ...
            self.dispatch(ServerEvent(cause='server/warning/' + self.event[0], data='WARNING: ' + self.event[1].format(timeout=time), priority=1))
        elif self.timeout:
            self.console(('{0} -- auto restart in {1} minutes').format(self.warning, self.timeout - self.time))
        else:
            self.console(self.warning)
...
class Monitor(Plugin):
    ...
    def setup(self):
        ...
        if self.crash_enabled:
            do_step = True
            self.checks['crash'] = Check(self, name='crash', timeout=self.crash_timeout, warn=self.crash_warn, message='server has crashed', warning='server might have crashed', event=('hang',
                                                                                                                                                                                         "server didn't respond for {timeout}"), stop_reason='crashed')
        if self.ping_enabled:
            self.register(self.handle_ping, StatPlayerCount)
            do_step = True
            self.checks['ping'] = Check(self, name='ping', timeout=self.ping_timeout, warn=self.ping_warn, message='server is not accepting connections', warning='server might have stopped accepting connections', event=('ping',
                                                                                                                                                                                                                            "server didn't respond for {timeout}"), stop_reason='not accepting connections')
        if self.pcount_enabled:
            self.register(self.handle_pcount, StatPlayerCount)
            do_step = True
            self.checks['pcount'] = Check(self, name='pcount', timeout=self.pcount_timeout, warn=self.pcount_warn, message='server has had 0 players for {timeout}, something is wrong', warning='server has 0 players, might be inaccessible', event=('player-count',
                                                                                                                                                                                                                                                       'server had 0 players for {timeout}'), stop_reason='zero players')
    ...
    def step(self, *a):
        ...
        if self.crash_enabled:
            self.register(self.handle_crash_ok, ServerOutput, pattern='Unknown command.*', track=False)
            self.send('')
    ...
    def handle_oom(self, event):
        ...
        self.dispatch(ServerEvent(cause='server/error/oom', data='server ran out of memory', priority=1))
