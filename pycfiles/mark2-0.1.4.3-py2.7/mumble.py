...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mk2/plugins/mumble.py
...
class MumbleProtocol(DatagramProtocol):
    ...
    def ping(self, *a):
        self.transport.write('\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00', addr=(self.host, self.port))

    def datagramReceived(self, data, (host, port)):
        self.buff += data
        if len(self.buff) < 24:
            return
        if not self.buff.startswith('\x00\x01\x02\x03' + '\x00\x00\x00\x00\x00\x00\x00\x00'):
