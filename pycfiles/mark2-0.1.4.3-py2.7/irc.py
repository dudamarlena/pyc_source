...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mk2/plugins/irc.py
...
class SASLPlain(object):
    ...
    def __init__(self, username, password):
        self.response = ('{0}\x00{0}\x00{1}').format(username, password)

    def is_valid(self):
        return self.response != '\x00\x00'
...
class IRCBot(irc.IRCClient):
    ...
    def privmsg(self, user, channel, msg):
        ...
        if p.irc_players_enabled and msg.lower() == p.irc_command_prefix + 'players':
            self.say(self.channel, p.irc_players_format.format(players=(', ').join(map(self.mangle_username, p.players))))
            ...
            elif command.lower() in p.irc_command_allow.lower().split(',') or p.irc_command_allow == '*':
                p.send((' ').join(argv))
        else:
...
class IRC(Plugin):
    ...
    def setup(self):
        ...
        if self.ssl:
            if have_ssl:
                cf = Mark2ClientContextFactory(self, cert=self.certificate, fingerprint=self.server_fingerprint)
        ...
        if self.game_server_message_enabled and not (self.irc_chat_enabled and self.irc_chat_command.startswith('say ')):
            register(ServerOutput, self.game_server_message_format, pattern='\\[(?:Server|SERVER)\\] (?P<message>.+)')
        if self.game_me_enabled:
            register(ServerOutput, self.game_me_format, pattern='\\* (?P<username>[A-Za-z0-9_]{1,16}) (?P<message>.+)')
