...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mk2/services/ping.py
...
class Ping(Plugin):
    ...
    def server_started(self, event):
        ...
        pattern = '\\s*(?:/{0}:\\d+ lost connection|Reached end of stream for /{0})'
        self.event_id = self.parent.events.register(lambda ev: Event.EAT, ServerOutput, pattern=pattern.format(self.host))
