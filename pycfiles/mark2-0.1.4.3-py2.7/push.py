...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mk2/plugins/push.py
...
class Endpoint(object):
    ...
    def __str__(self):
        return ('<{0} {1} causes={2} priority={3}>').format(self.__class__.__name__, self.url, self.causes, self.priority)
...
class HTTPEndpoint(Endpoint):
    ...
    def push(self, event):
        self.setup(event)
        defer = getPage(self.endpoint, method=self.method, postdata=urlencode(self.postdata), headers={'Content-type': 'application/x-www-form-urlencoded'})
        self.wait(defer)
...
class NMAEndpoint(HTTPEndpoint):
    ...
    def __init__(self, plugin, url):
        self.postdata = {'apikey': url, 
           'application': ('mark2: {0}').format(plugin.parent.server_name)}

    def setup(self, event):
        self.postdata.update(priority=event.priority, event=event.friendly, description=event.data)
...
class PushoverEndpoint(HTTPEndpoint):
    ...
    def __init__(self, plugin, url):
        ...
        self.postdata = {'user': url, 'token': plugin.pushover_token}

    def setup(self, event):
        self.postdata.update(priority=max(-1, event.priority), title=event.friendly, message=event.data)
        if self.device:
...
class SMTPEndpoint(Endpoint):
    ...
    def __init__(self, plugin, url):
        self.smtp_host, self.smtp_user, self.smtp_password = plugin.email_smtp_server, plugin.email_smtp_user, plugin.email_smtp_password
    ...
    def sendEmail(self, from_, from_name, to, msg_, subject=''):
        ...
        def send(host, user=None, pw=None, require_security=False):
            ...
            d = Deferred()
            factory = smtp.ESMTPSenderFactory(user, pw, from_, to, msgfile, d, requireAuthentication=user is not None, requireTransportSecurity=require_security)
