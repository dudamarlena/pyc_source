...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mk2/properties.py
# Compiled at: 2013-08-16 22:15:55
import os, re, shlex, zipfile
...
class Lang(Properties):
    ...
    def get_deaths(self):
        ...
        for k, v in self.get_by_prefix('death.'):
            if v not in seen:
                seen.append(v)
                regex = reduce(lambda a, r: a.replace(*r), ((('\\%{0}\\$s').format(i + 1), ('(?P<{0}>[A-Za-z0-9]{{1,32}})').format(x)) for i, x in enumerate(('username',
                                                                                                                                                              'killer',
                                                                                                                                                              'weapon'))), re.escape(v))
                format = reduce(lambda a, r: a.replace(*r), ((('%{0}$s').format(i + 1), ('{{{0}}}').format(x)) for i, x in enumerate(('username',
                                                                                                                                      'killer',
                                                                                                                                      'weapon'))), v)
