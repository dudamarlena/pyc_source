...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mk2/user_client.py
# Compiled at: 2013-08-16 22:15:55
import getpass, glob, json, os
from string import Template
from twisted.internet import reactor
from twisted.internet.protocol import ClientFactory, ProcessProtocol
from twisted.internet.task import LoopingCall
from twisted.protocols.basic import LineReceiver
import properties, psutil, re, sys, urwid
...
class PMenuWrap(urwid.WidgetPlaceholder):
    ...
    def next(self, widget, result):
        acc = self._pmenu_acc
        acc.append(result)
        if self._pmenu_stage == 1 and not (result in ('kick', 'ban') and len(self._pmenu_lists[2][1]) > 0) or self._pmenu_stage == 2:
...
class UI:
    ...
    def main(self):
        self.loop = urwid.MainLoop(self.g_main, self.palette, input_filter=self.filter_input, event_loop=urwid.TwistedEventLoop())
        self.loop.run()
    ...
    def set_servers(self, servers, current=None):
        ...
        del contents[0:len(contents)]
        sep = '⇉ ' if urwid.supports_unicode() else ':'
        contents.append((urwid.AttrMap(urwid.Text(' mark2 %s' % sep), 'mark2'), self.g_servers.options('pack')))
        contents.extend(new)
