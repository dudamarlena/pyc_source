...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mk2/manager.py
...
class Manager(object):
    ...
    def really_start(self):
        ...
        if self.jar_file is None:
            self.jar_file = process.find_jar(self.config['mark2.jar_path'].split(';'), self.jar_file)
            if self.jar_file is None:
        ...
        if self.properties is None:
            ...
            self.services = plugins.PluginManager(self, search_path='services', name='service', get_config=self.get_service_config)
            ...
            self.plugins = plugins.PluginManager(self, search_path='plugins', name='plugin', get_config=self.get_plugin_config, require_config=True)
            self.load_plugins()
    ...
    def start_logging(self):
        ...
        if log_rotate == 'daily':
            ...
            log_obj = logfile.LogFile('%s.log' % self.server_name, self.shared_path, rotateLength=log_size if log_rotate == 'size' else None, maxRotatedFiles=log_limit if log_limit != '' else None)
        else:
    ...
    def load_config(self):
        self.config = properties.load(properties.Mark2Properties, open_resource('resources/mark2.default.properties'), find_config('mark2.properties'), 'mark2.properties')
    ...
    def handle_server_output(self, event):
        self.events.dispatch(events.Console(source='server', line=event.line, time=event.time, level=event.level, data=event.data))
