...
# [GCC 8.4.0]
# Embedded file name: /home/grzegorz/eclipse-workspacePython/pplus/doc/sphinxext/sphinxcontrib/spelling.py
# Compiled at: 2013-06-07 13:26:36
"""Spelling checker extension for Sphinx.
"""
import codecs, collections, imp, itertools, os, re, textwrap, tempfile, xmlrpclib
...
import enchant
from enchant.tokenize import get_tokenizer, tokenize, Filter, EmailFilter, WikiWordFilter, unit_tokenize, wrap_tokenizer
...
class SpellingDirective(rst.Directive):
    ...
    def run(self):
        ...
        if good_words:
            ...
            env.spelling_document_filters[env.docname].append(IgnoreWordsFilterFactory(good_words))
        return []
...
class AcronymFilter(Filter):
    ...
    def _skip(self, word):
        return word == word.upper() or word[(-1)].lower() == 's' and word[:-1] == word[:-1].upper()
    ...
    def check(self, text):
        ...
        for word, pos in self.tokenizer(text):
            ...
            if correct:
                continue
            yield (
             word, self.dictionary.suggest(word) if self.suggest else [])
...
class SpellingBuilder(Builder):
    ...
    def init(self):
        ...
        self.checker = SpellingChecker(lang=self.config.spelling_lang, suggest=self.config.spelling_show_suggestions, word_list_filename=project_words, filters=filters)
