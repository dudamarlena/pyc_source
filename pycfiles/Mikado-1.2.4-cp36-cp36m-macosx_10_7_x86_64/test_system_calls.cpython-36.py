...
# [GCC 8.4.0]
# Embedded file name: /Users/venturil/workspace/mikado/Mikado/tests/test_system_calls.py
# Compiled at: 2018-05-23 17:14:36
# Size of source mod 2**32: 39789 bytes
import csv, glob, gzip, itertools, logging, os, random, sys, tempfile, unittest, pkg_resources, pyfaidx, yaml, Mikado.daijin, Mikado.subprograms.configure
...
class PrepareCheck(unittest.TestCase):
    ...
    def test_prepare_trinity_gff(self):
        ...
        for test_file in ('trinity.gff3', 'trinity.match_matchpart.gff3', 'trinity.cDNA_match.gff3',
            ...
            with self.subTest(test_file=test_file):
                self.conf['prepare']['files']['gff'] = [pkg_resources.resource_filename('Mikado.tests', test_file)]
                prepare.prepare(args, self.logger)
                fa = pyfaidx.Fasta(os.path.join(self.conf['prepare']['files']['output_dir'], 'mikado_prepared.fasta'))
    ...
    def test_prepare_trinity_and_cufflinks(self):
        ...
        self.conf['prepare']['files']['gff'].append(pkg_resources.resource_filename('Mikado.tests', 'cufflinks.gtf'))
        ...
        for test_file in ('trinity.gff3', 'trinity.match_matchpart.gff3', 'trinity.cDNA_match.gff3',
            ...
            with self.subTest(test_file=test_file):
                ...
                self.conf['prepare']['files']['out'] = 'mikado_prepared.gtf'
                prepare.prepare(args, self.logger)
                self.assertTrue(os.path.exists(os.path.join(self.conf['prepare']['files']['output_dir'], 'mikado_prepared.fasta')))
                self.assertGreater(os.stat(os.path.join(self.conf['prepare']['files']['output_dir'], 'mikado_prepared.fasta')).st_size, 0)
                fa = pyfaidx.Fasta(os.path.join(self.conf['prepare']['files']['output_dir'], 'mikado_prepared.fasta'))
                res = dict((_, len(fa[_])) for _ in fa.keys())
                fa.close()
...
class CompareCheck(unittest.TestCase):
    __doc__ = 'Test to check that compare interacts correctly with match, match_part, cDNA_match'

    def tearDown(self):
        logging.shutdown()
        for suff in ('log', 'refmap', 'tmap', 'stats'):
            [os.remove(_) for _ in glob.glob(os.path.join(tempfile.gettempdir(), 'compare_*.{}'.format(suff)))]
        ...
        for ref in files:
            with self.subTest(ref=ref):
                ...
                with pkg_resources.resource_stream('Mikado.tests', ref) as (ref_handle):
                    with open(temp_ref, 'wb') as (out_handle):
    ...
    def test_compare_trinity(self):
        ...
        for ref, pred in itertools.permutations(files, 2):
            with self.subTest(ref=ref, pred=pred):
                ...
                namespace.log = os.path.join(tempfile.gettempdir(), 'compare_{}_{}.log'.format(files.index(ref), files.index(pred)))
                namespace.out = os.path.join(tempfile.gettempdir(), 'compare_{}_{}'.format(files.index(ref), files.index(pred)))
                compare(namespace)
...
class StatCheck(unittest.TestCase):
    __doc__ = 'This unit test takes care of verifying that statistics are generated correctly when\n    considering four different inputs. Output will be checked against a standard file.'
    ...
    def test_stat(self):
        ...
        for filename in files:
            with self.subTest(filename=filename):
                ...
                with open(os.path.join(tempfile.gettempdir(), '{}.txt'.format(os.path.basename(filename))), 'w') as (out):
                    namespace.out = out
...
class ConfigureCheck(unittest.TestCase):
    __doc__ = 'Test for creating configuration files'
    __genomefile__ = None
    ...
    @unittest.skipUnless(sys.version_info.minor > 4, 'Due to a bug in JSONSCHEMA, Daijin configure fails with Python versions lower than 3.5.')
    def test_daijin_config(self):
        ...
        namespace.threads = 1
        namespace.scoring = random.choice(pkg_resources.resource_listdir('Mikado.configuration', 'scoring_files'))
        out = os.path.join(tempfile.gettempdir(), 'configuration.yaml')
...
class PickTest(unittest.TestCase):
    __doc__ = 'This unit test will check that pick functions correctly.'
    ...
    def test_single_proc(self):
        ...
        json_conf['pick']['files']['input'] = pkg_resources.resource_filename('Mikado.tests', 'mikado_prepared.gtf')
        ...
        with self.assertRaises(SystemExit):
            with self.assertLogs('main_logger', 'INFO'):
                pick_caller()
        ...
        with to_gff(os.path.join(tempfile.gettempdir(), 'mikado.monoproc.loci.gff3')) as (inp_gff):
            lines = [_ for _ in inp_gff if _.header is not True]
        ...
        for num in (1, 2):
            with self.subTest(num=num):
                ...
                with open(json_file, 'wt') as (json_handle):
                    Mikado.subprograms.configure.print_config(yaml.dump(json_conf, default_flow_style=False), json_handle)
    ...
    def test_different_scoring(self):
        ...
        json_conf['db_settings']['db'] = pkg_resources.resource_filename('Mikado.tests', 'mikado.db')
        json_conf['log_settings']['log_level'] = 'WARNING'
        self.assertEqual(os.path.basename(json_conf['pick']['scoring_file']), 'plants.yaml')
        json_file = os.path.join(tempfile.gettempdir(), 'mikado.yaml')
        with open(json_file, 'wt') as (json_handle):
            Mikado.subprograms.configure.print_config(yaml.dump(json_conf, default_flow_style=False), json_handle)
        sys.argv = ['mikado', 'pick', '--json-conf', json_file]
        ...
        with open(os.path.join(json_conf['pick']['files']['output_dir'], 'mikado.test_diff.loci.scores.tsv')) as (tsv):
            ...
            score_header = [_ for _ in reader.fieldnames if _ not in ('tid', 'alias',
                                                                      'parent', 'score',
                                                                      'source_score')]
        ...
        for purging in (False, True):
            with self.subTest(purging=purging):
                ...
                with self.assertRaises(SystemExit):
                    with self.assertLogs('main_logger', 'INFO'):
                        pick_caller()
                with to_gff(os.path.join(tempfile.gettempdir(), json_conf['pick']['files']['loci_out'])) as (gff):
                    lines = [line for line in gff if line.header is False]
                self.assertGreater(len(lines), 0)
                self.assertTrue(any([_ for _ in lines if _.attributes.get('alias', '') == 'foo2.1']))
                if purging is True:
                    self.assertFalse(any([_ for _ in lines if _.attributes.get('alias', '') in ('foo1.2',
                                                                                                'foo1.1')]))
                else:
                    found_line = [_ for _ in lines if _.attributes.get('alias', '') in ('foo1.2',
                                                                                        'foo1.1')]
