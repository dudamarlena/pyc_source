...
# [GCC 8.4.0]
# Embedded file name: /Users/venturil/workspace/mikado/Mikado/tests/transcript_tester_negative.py
# Compiled at: 2018-05-23 17:14:36
# Size of source mod 2**32: 26481 bytes
"""
Unit test for a transcript on the negative strand.
"""
import unittest, operator, re, logging, Mikado, Mikado.parsers, Mikado.loci
...
class TranscriptTesterNegative(unittest.TestCase):
    ...
    def test_print(self):
        ...
        self.assertEqual(real_printed, str(self.tr), diff)
    ...
    def test_cds(self):
        self.assertEqual(sorted(self.tr.combined_cds), sorted(self.tr.selected_cds))
    ...
    def test_utr_metrics(self):
        ...
        self.assertEqual(self.tr.five_utr_length, 71)
        self.assertEqual(self.tr.three_utr_length, 814)
        self.assertEqual(self.tr.selected_start_distance_from_tss, 71, self.tr.selected_end_distance_from_tes)
        self.assertEqual(self.tr.selected_end_distance_from_tes, 814, self.tr.selected_end_distance_from_tes)
        self.assertEqual(self.tr.selected_end_distance_from_junction, 478, self.tr.selected_cds_end)
        self.assertEqual(self.tr.end_distance_from_junction, self.tr.selected_end_distance_from_junction)

    def test_introns(self):
        introns = {
         (8465, 8570), (8326, 8416), (7988, 8235),
        ...
        cds_introns = {
         ...
         (7836, 7941), (7650, 7761), (7451, 7563),
         (7233, 7383), (7070, 7156)}
        self.assertEqual(self.tr.combined_cds_introns, cds_introns, self.tr.combined_cds_introns)
        selected_cds_introns = {
         (8465, 8570), (8326, 8416), (7988, 8235),
         (7836, 7941), (7650, 7761), (7451, 7563),
         (7233, 7383), (7070, 7156)}
        self.assertEqual(self.tr.selected_cds_introns, selected_cds_introns, self.tr.selected_cds_introns)
    ...
    def test_remove_utr(self):
        ...
        combined_cds = [(6915, 7069),
         ...
         (8417, 8464),
         (8571, 8666)]
        self.assertEqual(self.tr.combined_cds, combined_cds, self.tr.combined_cds)
    ...
    def test_lengths(self):
        ...
        self.assertAlmostEqual((self.tr.combined_cds_fraction), 0.45471349353049906, delta=0.01)
        self.assertAlmostEqual((self.tr.selected_cds_fraction), 0.45471349353049906, delta=0.01)
    ...
    def testDoubleOrf(self):
        ...
        self.assertEqual((new_transcripts[0].cdna_length), 624, msg=('{0}-{1}{2}'.format(new_transcripts[0].start, new_transcripts[0].end, new_transcripts[0].strand)))
...
class WheatTest(unittest.TestCase):
    ...
    def test_split(self):
        self.tr.load_orfs([self.bed1, self.bed2])
        new_transcripts = sorted([_ for _ in self.tr.split_by_cds()], key=(operator.attrgetter('start', 'end')))
        self.assertEqual(len(new_transcripts), 2)
