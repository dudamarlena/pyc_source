...
# [GCC 8.4.0]
# Embedded file name: /Users/venturil/workspace/mikado/Mikado/tests/transcript_tester_positive.py
# Compiled at: 2018-05-23 17:14:36
# Size of source mod 2**32: 51496 bytes
"""
Unit test for a transcript on the positive strand.
"""
import unittest, re, copy, Mikado.parsers, Mikado.exceptions, Mikado.loci
...
class MonoBaseTester(unittest.TestCase):
    ...
    def test_print_without_cds(self):
        ...
        self.assertEqual(self.tr.format('gtf', with_cds=False), real_printed_gtf)
...
class DrosoTester(unittest.TestCase):
    ...
    def setUp(self):
        ...
        ref_lines = [Mikado.parsers.GTF.GtfLine(line) for line in filter(lambda x: x != '', ref_gtf.split('\n'))]
        ...
        self.ref.finalize()
        pred_lines = [Mikado.parsers.GTF.GtfLine(line) for line in filter(lambda x: x != '', pred_gtf.split('\n'))]
...
class TranscriptTesterPositive(unittest.TestCase):
    ...
    def test_introns(self):
        introns = {
         (626781, 626841), (626881, 626962), (627060, 627136),
        ...
        introns = {
         (626881, 626962), (627060, 627136), (627194, 627311),
         (627398, 627487), (627560, 627695), (627750, 627839),
         (627916, 628043), (628106, 628181), (628242, 628464)}
        self.assertEqual(self.tr.combined_cds_introns, introns, (
        ...
        cds_introns = {
         ...
         (627398, 627487), (627560, 627695), (627750, 627839),
         (627916, 628043), (628106, 628181), (628242, 628464)}
        self.assertEqual(self.tr.selected_cds_introns, cds_introns, self.tr.selected_cds_introns)
    ...
    def test_utr_metrics(self):
        ...
        self.assertEqual(self.tr.five_utr_length, 175)
        self.assertEqual(self.tr.three_utr_length, 214)
        self.assertEqual(self.tr.selected_start_distance_from_tss, 175, self.tr.selected_end_distance_from_tes)
        self.assertEqual(self.tr.selected_start_distance_from_tss, self.tr.start_distance_from_tss)
        self.assertEqual(self.tr.selected_end_distance_from_tes, 214, self.tr.selected_end_distance_from_tes)
        self.assertEqual(self.tr.selected_end_distance_from_tes, self.tr.end_distance_from_tes)
        self.assertEqual(self.tr.selected_end_distance_from_junction, 107)
    ...
    def test_with_no_gff_utr(self):
        ...
        tr_gff_lines = [Mikado.parsers.GFF.GffLine(line) for line in tr_lines]
        transcript = Mikado.loci.Transcript((tr_gff_lines[0]), logger=logger)
    ...
    def test_raises_invalid(self):
        self.tr.finalized = False
        self.tr.strand = None
        _TranscriptTesterPositive__current = self.tr.deepcopy()
    ...
    def test_lengths(self):
        ...
        self.assertAlmostEqual((self.tr.combined_cds_fraction), 0.6333647502356268, delta=0.01)
        self.assertAlmostEqual((self.tr.selected_cds_fraction), 0.6333647502356268, delta=0.01)
    ...
    def testDoubleOrf(self):
        ...
        candidates = [
         first_orf, second_orf, third_orf]
        logger = create_null_logger('null')
...
class AugustusTester(unittest.TestCase):
    ...
    def test_truncated(self):
        ...
        with self.assertLogs('augustus', level='WARNING') as (cm_out):
            transcript.finalize()
            self.assertTrue(any('The transcript TRIAE4565_1AL_Aug_0021880.1 has coordinates 1:2785' in _ for _ in cm_out.output))
        self.assertTrue(transcript.is_coding)
    ...
    def test_three_truncated(self):
        ...
        with self.assertLogs('augustus', level='WARNING') as (cm_out):
            transcript.finalize()
            self.assertTrue(any('The transcript TRIAE4565_1AL_Aug_0024630.1 has coordinates 204336:224434' in _ for _ in cm_out.output))
        self.assertTrue(transcript.is_coding)
    ...
    def test_invalid_three_truncated(self):
        ...
        with self.assertLogs('augustus', level='WARNING') as (cm_out):
            transcript.finalize()
            self.assertTrue(any('The transcript TRIAE4565_1AL_Aug_0024630.1 has coordinates 204336:225434' in _ for _ in cm_out.output))
    ...
    def test_valid_three_truncated(self):
        ...
        with self.assertLogs('augustus', level='DEBUG') as (cm_out):
            transcript.finalize()
        self.assertTrue(transcript.is_coding)
        self.assertEqual(560, transcript.selected_cds_length, sum(_[1][1] - _[1][0] + 1 for _ in transcript.selected_internal_orf if _[0] == 'CDS'))
