...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/industry11_090319.py
# Compiled at: 2019-03-14 13:36:26
# Size of source mod 2**32: 4398 bytes
"""
...
import pandas as pnd, os, re, xlrd, glob
    ...
    def check(self):
        for segment in self.industry_list:
            ...
            for each in lsd:
                ...
                if len(each) > 10:
                    name = each[:-10]
                    stringtoadd = re.sub(' +', ' ', name)
                temp_lsd.append(stringtoadd)

            new_lsd = list(set(temp_lsd))
