...
# Embedded file name: /home/mareike/work/app/pyrap-dev/python3/pyrap/ptypes.py
# Compiled at: 2018-02-09 05:38:43
...
class Event(object):
    ...
    def notify(self, *args, **kwargs):
        ...
        for listener in self._listeners:
            self._notify(listener, *args, **kwargs)
...
class Var(object):
    ...
    def set(self, v, caller=None):
        ...
        if v != self.value:
            ...
            if dirty:
                self.on_dirty.notify(self, caller)
...
class Percent(Dim):
    ...
    def of(self, v):
        if type(v) is float:
            return self.value * v / 100.0
        if type(v) is int:
            return int(round(self.value * v / 100.0))
        if isinstance(v, str):
            v = parse_value(v)
        if isinstance(v, Pixels):
            return Pixels(int(round(v.value * self._value / 100.0)))
        return type(v)((v.value if isinstance(v, Dim) else v) * self._value / 100.0)
...
class Color(object):
    ...
    def __init__(self, html=None, rgb=None, hsv=None, fct=None, alpha=None):
        ...
        if fct is not None:
            if fct.name == 'rgb':
                ...
                raise Exception('Unknown color value function: %s' % str(fct))
        if html is not None:
            if html in Color.names:
                html = Color.names[html]
            if html.startswith('#'):
                if len(html) < 5:
                    html += html[1:]
                self._r, self._g, self._b = int(html[1:3], base=16) / 255.0, int(html[3:5], base=16) / 255.0, int(html[5:7], base=16) / 255.0
                self._a = float(int(html[7:9], base=16)) / 255.0 if len(html) > 7 else 1.0
            else:
                raise Exception('Illegal color value: %s' % html)
        elif rgb is not None:
            self._r, self._g, self._b = rgb[:3]
            self._a = rgb[3] if len(rgb) > 3 else 1.0
        elif hsv is not None:
            self._r, self._g, self._b = hsv_to_rgb(*hsv[:3])
            self._a = hsv[3] if len(hsv) > 3 else 1.0
        else:
            raise Exception('Illegal color value: %s' % (' ').join(map(str, (html, rgb, hsv, fct, alpha))))
    ...
    def rgbi(self):
        return (
         self.redi, self.greeni, self.bluei)
    ...
    @property
    def htmla(self):
        return self.html + '%.2x' % int(round(self._a * 255.0))
...
def parse_value(v, default=None):
    if isinstance(v, str):
        ...
        if v.endswith('%'):
            return Percent(v)
        if v.startswith('#') or default is Color and v in Color.names:
            return Color(v)
    else:
        if default is None or v is None or type(v) is default:
            return v
        return default(v)
    return
...
def toint(v):
    ...
    if type(v) is int:
        return v
    if type(v) is list:
        return [ toint(e) for e in v ]
    if type(v) is tuple:
        return tuple([ toint(e) for e in v ])
    int(v)
...
class SVG(object):
    ...
    def size(self):
        return (
         px(self._width), px(self._height))
...
if __name__ == '__main__':
    ...
    def getdirty(v, d, c):
        print (
         repr(v), 'is now', {True: 'dirty', False: 'clean'}[d], 'by', c)
