...
# Embedded file name: /home/mareike/work/app/pyrap-dev/python3/pyrap/utils.py
# Compiled at: 2017-11-28 07:44:07
...
class BiMap:
    ...
    def __getitem__(self, s):
        if type(s) is slice:
            if not s.start and s.stop:
                return self._bwd[s.stop]
            if s.start and not s.stop:
                return self._fwd[s.start]
            if s.start and s.stop:
                self._fwd[s.start] = s.stop
                self._bwd[s.stop] = s.start
            else:
                raise AttributeError('Slice argument for BiMap cannot be empty')
        else:
            return self._fwd[s]
...
if __name__ == '__main__':
    print pparti(10, [0.2, 0.2, 0.1, 0.5])