...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tympeg/scripts/file_name_cleaner.py
# Compiled at: 2017-10-18 14:51:13
# Size of source mod 2**32: 9520 bytes
from os import path, rename
from .. import list_files

def clean_file_names(dir_path, primary_scrub_list, secondary_scrub_list, prefix_scrub_list, ext_list=[], test=False, auto_confirm=False):
    ...
    def capitalize_code(s, capitalize_error, no_hyp):
        ...
        if len(splits) == 2 and s.find(' ') < 0:
            ...
            if len(splits) == 1:
                if s not in no_hyp:
                    no_hyp.append(s)
            elif s not in capitalize_error:
    ...
    if len(ext_list) < 1:
        ext_list = [
         '.mp4', '.mkv', '.avi', '.m4v', '.wmv', '.webm', '.flv', '.mov', '.mpg', '.mpeg', '.ogg', '.ogv']
    else:
        all_files = []
        ...
        if confirm == 'n':
            return
        if confirm == 'y':
            save_log(path.join(dir_path, log_file_name), file_names, scrubbed_file_names, max_len, cant_hyphen, cant_capitalize)
