...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tympeg/scripts/convert_sub_dirs.py
# Compiled at: 2018-03-13 17:58:16
# Size of source mod 2**32: 10031 bytes
...
qualities_LQ = {'threshold':[
 ...
 'default':[
  23, (96, s)]}
qualities_HQ_high_min = {'threshold':[
  0, 0.08, 0.11], 
 'video':[
  23, 21, 20], 
 'audio':[
...
def convert_folder_x265(dir_path, qualities, speed, autodelete=False, log=True):
    ...
    for media in files_to_convert:
        ...
        media_size = media.file_size / 1000000
        now = datetime.datetime.now()
        ...
        if not audio:
            ...
            lo.pl('\nCompleted file {0} of {1} at {2} in {3:,.2f} min'.format(count, total_files, now.strftime('%I:%M:%S %p'), minutes))
    ...
    if autodelete:
        try:
            ...
            print("{} could not be removed. Most likely because a file wasn't converted because it already exists in the parent directory and the original file is present for your review. An input file could be smaller than an output file, in whichcase deletion is not done")
...
def decide_quality(qualities, media_object):
    ...
    if bits_pixel <= 0:
        print('Unable to calculate bits per pixel, defaulting to: crf = {}, audio = {}k, channels = {}'.format(crf, audio_bitrate, audio_channels))
    for x in range(0, n):
