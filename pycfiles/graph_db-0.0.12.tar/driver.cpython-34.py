...
# [GCC 8.4.0]
# Embedded file name: /home/asistente04/diggi3/components/graph_db/graph_db/driver/orientdb/driver.py
# Compiled at: 2016-04-07 16:15:03
...
class DBDriver(types.BaseDBDriver):
    ...
    def connect(self):
        if self._connected:
            return
        try:
            url = self._DBDriver__url('connect', self._settings['name'])
    ...
    def selectDB(self):
        if not self._connected:
            self.connect()
        try:
            url = self._DBDriver__url('database', self._settings['name'])
            self._debugOut('GET %s' % url)
            response = requests.get(url, auth=self._auth)
            self._debugIn('[%s]' % response.status_code, response.text)
            if response.status_code == 401:
    ...
    def query(self, sql, *args, **kwargs):
        ...
        url = self._DBDriver__url('command', self._settings['name'], 'sql')
        self._debugOut('GET %s' % url)
        self._debugOut('POST %s\n--> %s' % (url, sql))
        try:
            response = requests.post(url, auth=self._auth, params={'format': 'rid,class,fetchPlan:*:%d' % depth}, data=sql)
