...
# [GCC 8.4.0]
# Embedded file name: /ClusterShell/CLI/Clush.py
# Compiled at: 2019-12-07 15:34:33
# Size of source mod 2**32: 43917 bytes
"""
...
import random, resource, signal, sys, time, threading
try:
    raw_input
except NameError:
    raw_input = input
...
class UpdatePromptException(Exception):
    __doc__ = 'Exception used by the signal handler'


class StdInputHandler(EventHandler):
    __doc__ = 'Standard input event handler class.'
...
class OutputHandler(EventHandler):
    __doc__ = 'Base class for clush output handlers.'
    ...
    def runtimer_init(self, task, ntotal=0):
        ...
        self._runtimer = task.timer(1.33, thandler, interval=0.3333333333333333, autoclose=True)
...
class DirectOutputHandler(OutputHandler):
    __doc__ = 'Direct output event handler class.'
    ...
    def ev_hup(self, worker, node, rc):
        if rc > 0:
            ...
            if self._display.maxrc:
                verb = VERB_STD
            self._display.vprint_err(verb, 'clush: %s: exited with exit code %d' % (
             node, rc))

    def ev_close(self, worker, timedout):
        if timedout:
            nodeset = NodeSet._fromlist1(worker.iter_keys_timeout())
            self._display.vprint_err(VERB_QUIET, 'clush: %s: command timeout' % nodeset)
        self.update_prompt(worker)


class DirectProgressOutputHandler(DirectOutputHandler):
    __doc__ = 'Direct output event handler class with progress support.'
...
class CopyOutputHandler(DirectProgressOutputHandler):
    __doc__ = 'Copy output event handler.'
...
class GatherOutputHandler(OutputHandler):
    __doc__ = 'Gathered output event handler class (clush -b).'
    ...
    def ev_close(self, worker, timedout):
        self._runtimer_finalize(worker)
        nodesetify = lambda v: (
         v[0], NodeSet._fromlist1(v[1]))
        cleaned = False
        for _rc, nodelist in sorted(worker.iter_retcodes()):
            ns_remain = NodeSet._fromlist1(nodelist)
            for buf, nodeset in sorted((map(nodesetify, worker.iter_buffers(nodelist))),
    ...
    def _close_common(self, worker):
        ...
        for rc, nodelist in worker.iter_retcodes():
            if rc != 0:
                ...
                if self._display.verbosity > VERB_QUIET:
                    if len(ns) > 1:
        ...
        if worker.num_timeout() > 0:
            self._display.vprint_err(verbexit, 'clush: %s: command timeout' % NodeSet._fromlist1(worker.iter_keys_timeout()))


class SortedOutputHandler(GatherOutputHandler):
    __doc__ = 'Sorted by node output event handler class (clush -L).'
...
class LiveGatherOutputHandler(GatherOutputHandler):
    __doc__ = 'Live line-gathered output event handler class (-bL).'
    ...
    def _live_line(self, worker):
        while self._mtreeq and len(self._mtreeq[0]) == len(self._nodes):
            ...
            for buf, nodeset in sorted((map(nodesetify, mtree.walk())), key=bufnodeset_cmpkey):
                self._display.print_gather(nodeset, buf)
...
class RunTimer(EventHandler):
    __doc__ = 'Running progress timer event handler'
...
def ttyloop(task, nodeset, timeout, display, remote, trytree):
    ...
    while task.default('USER_running') or interactive and cmd.lower() != 'quit':
        try:
            ...
            elif task.default('USER_interactive') and not task.default('USER_running'):
                if ns_info:
            ...
            if display.gather:
                ...
                for buf, nodeset in sorted((map(nodesetify, task.iter_buffers())), key=bufnodeset_cmpkey):
                    if not print_warn:
                        print_warn = True
                        display.vprint_err(VERB_STD, 'Warning: Caught keyboard interrupt!')
                ...
                for rc, nodelist in task.iter_retcodes():
                    ...
                    if rc != 0:
                        nsdisp = ns = NodeSet._fromlist1(nodelist)
                        if display.verbosity >= VERB_QUIET:
                            if len(ns) > 1:
                                nsdisp = '%s (%d)' % (ns, len(ns))
                ...
                if task.num_timeout() > 0:
                    display.vprint_err(verbexit, 'clush: %s: command timeout' % NodeSet._fromlist1(task.iter_keys_timeout()))
            raise kbe
        ...
        if task.default('USER_running'):
            ...
            if not gws:
                display.vprint_err(VERB_QUIET, 'clush: in progress(%d): %s%s' % (
            ...
            for gw, (chan, metaworkers) in task.gateways.items():
                ...
                if act_targets:
                    display.vprint_err(VERB_QUIET, 'clush: [tree] in progress(%d) on %s: %s' % (
...
def run_command(task, cmd, ns, timeout, display, remote, trytree):
    ...
    if display.gather or display.line_mode:
        if ns is not None:
            if display.gather:
                if display.line_mode:
                    handler = LiveGatherOutputHandler(display, ns)
            elif not display.gather:
                if display.line_mode:
                    handler = SortedOutputHandler(display)
            else:
                handler = GatherOutputHandler(display)
            if display.verbosity in (VERB_STD, VERB_VERB) or display.progress and display.verbosity > VERB_QUIET:
                handler.runtimer_init(task, len(ns))
        else:
            if display.progress and display.verbosity > VERB_QUIET:
                handler = DirectProgressOutputHandler(display)
                handler.runtimer_init(task, len(ns))
    else:
        handler = DirectOutputHandler(display)
    worker = task.shell(cmd, nodes=ns, handler=handler, timeout=timeout, remote=remote,
      tree=trytree)
    if ns is None:
...
def run_copy(task, sources, dest, ns, timeout, preserve_flag, display):
    ...
    for source in sources:
        if not exists(source):
            ...
            clush_exit(1, task)
        task.copy(source, dest, ns, handler=copyhandler, timeout=timeout, preserve=preserve_flag)
...
def run_rcopy(task, sources, dest, ns, timeout, preserve_flag, display):
    ...
    if not exists(dest):
        ...
        clush_exit(1, task)
    if not isdir(dest):
        display.vprint_err(VERB_QUIET, 'ERROR: destination "%s" is not a directory' % dest)
        clush_exit(1, task)
    copyhandler = CopyOutputHandler(display, True)
    if display.verbosity == VERB_STD or display.verbosity == VERB_VERB:
        copyhandler.runtimer_init(task, len(ns) * len(sources))
    for source in sources:
        task.rcopy(source, dest, ns, handler=copyhandler, timeout=timeout, stderr=True,
...
def clush_excepthook(extype, exp, traceback):
    ...
    try:
        ...
        if uncomp_nodes:
            print(('Keyboard interrupt (%s did not complete).' % uncomp_nodes), file=(sys.stderr))
        else:
...
def main():
    ...
    parser.add_option('-n', '--nostdin', action='store_true', dest='nostdin', help="don't watch for possible input from stdin")
    ...
    for opt, nodelist in (('w', wnodelist), ('x', xnodelist)):
        for nodes in nodelist:
            if len(nodes) == 1 and exists(str(nodes)):
                display.vprint_err(VERB_STD, "Warning: using '-%s %s' and local path '%s' exists, was it expanded by the shell?" % (
                 opt, nodes, nodes))
    ...
    for opt_hostfile in options.hostfile:
        try:
            ...
            display.vprint_err(VERB_DEBUG, 'Using nodeset %s from hostfile %s' % (
    ...
    if options.group:
        ...
        for grp in grp_nodeset:
            ...
            display.vprint(VERB_DEBUG, 'Adding nodes from option -g %s: %s' % (grp, addingrp))
    ...
    if options.exgroup:
        ...
        for grp in grp_nodeset:
            ...
            display.vprint(VERB_DEBUG, 'Excluding nodes from option -X %s: %s' % (grp, removingrp))
    ...
    if options.pick:
        if options.pick < len(nodeset_base):
    ...
    stdin_isafgtty = sys.stdin.isatty() and os.tcgetpgrp(sys.stdin.fileno()) == os.getpgrp()
    if interactive:
        if not stdin_isafgtty:
            interactive = False
    ...
    if not options.nostdin:
        stdout_isafgtty = sys.stdout.isatty() and os.tcgetpgrp(sys.stdout.fileno()) == os.getpgrp()
    ...
    task.set_default('USER_stdin_worker', not (sys.stdin.isatty() or options.nostdin or user_interaction))
    display.vprint(VERB_DEBUG, 'Create STDIN worker: %s' % task.default('USER_stdin_worker'))
    task.set_info('debug', config.verbosity >= VERB_DEBUG)
            ...
            if options.topofile:
                ...
                if config.verbosity >= VERB_VERB:
                    ...
                    if options.grooming_delay:
                        if config.verbosity >= VERB_VERB:
                            msg = Display.COLOR_RESULT_FMT % ('Grooming delay: %f' % options.grooming_delay)
                        ...
                        if config.ssh_user:
                            ...
                            if config.ssh_options:
                                ...
                                if options.copy or options.rcopy:
                                    if not args:
                        ...
                        display.vprint(VERB_DEBUG, 'clush: nodeset=%s fanout=%d [timeout conn=%.1f cmd=%.1f] %s' % (
                         nodeset_base, config.fanout,
                         config.connect_timeout,
                         config.command_timeout,
                         op))
                        if not task.default('USER_interactive'):
                            if display.verbosity >= VERB_DEBUG:
                                if task.topology:
                                    print(Display.COLOR_RESULT_FMT % '-' * 15)
                                    print((Display.COLOR_RESULT_FMT % task.topology), end='')
                                    print(Display.COLOR_RESULT_FMT % '-' * 15)
                            if options.copy:
                                run_copy(task, args, options.dest_path, nodeset_base, timeout, options.preserve_flag, display)
                            else:
                                if options.rcopy:
                                    run_rcopy(task, args, options.dest_path, nodeset_base, timeout, options.preserve_flag, display)
                                else:
                                    run_command(task, ' '.join(args), nodeset_base, timeout, display, options.remote != 'no', options.worker is None)
                if user_interaction:
                    ttyloop(task, nodeset_base, timeout, display, options.remote != 'no', options.worker is None)
