...
# [GCC 8.4.0]
# Embedded file name: /ClusterShell/Topology.py
...
class TopologyError(Exception):
    __doc__ = 'topology parser error to report invalid configurations or parsing\n    errors\n    '


class TopologyNodeGroup(object):
    __doc__ = 'Base element for in-memory representation of the propagation tree.\n    Contains a nodeset, with parent-children relationships with other\n    instances.\n    '
    ...
    def add_child(self, child):
        ...
        its parent
        """
        if not isinstance(child, TopologyNodeGroup):
...
class TopologyTree(object):
    __doc__ = 'represent a simplified network topology as a tree of machines to use to\n    connect to other ones\n    '

    class TreeIterator(object):
        __doc__ = 'efficient tool for tree-traversal'
    ...
    def inner_node_count(self):
        ...
        return sum(len(group.nodeset) for group in self.groups if group.children_len() > 0)
...
class TopologyRoute(object):
    __doc__ = 'A single route between two nodesets'
...
class TopologyRoutingTable(object):
    __doc__ = 'This class provides a convenient way to store and manage topology\n    routes\n    '
    ...
    def add_route(self, route):
        ...
        if self._introduce_convergent_paths(route):
            raise TopologyError('Convergent path detected! Cannot add route %s' % str(route))
...
class TopologyGraph(object):
    __doc__ = 'represent a complete network topology by storing every "can reach"\n    relations between nodes.\n    '
    ...
    def add_route(self, src_ns, dst_ns):
        ...
        """
        if not isinstance(src_ns, NodeSet):
            raise AssertionError
        elif not isinstance(dst_ns, NodeSet):
            raise AssertionError
    ...
    def __str__(self):
        ...
        res = '<TopologyGraph>\n'
        res += '\n'.join(['%s: %s' % (str(k), str(v)) for k, v in self._nodegroups.items()])
        return res
...
class TopologyParser(configparser.ConfigParser):
    __doc__ = 'This class offers a way to interpret network topologies supplied under\n    the form :\n\n    # Comment\n    <these machines> : <can reach these ones>\n    '
