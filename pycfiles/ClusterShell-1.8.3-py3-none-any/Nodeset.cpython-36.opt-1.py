...
# [GCC 8.4.0]
# Embedded file name: /ClusterShell/NodeSet.py
# Compiled at: 2019-12-07 15:34:33
# Size of source mod 2**32: 61936 bytes
"""
...
_DEF_RESOLVER_STD_GROUP = NodeUtils.GroupResolverConfig(DEF_GROUPS_CONFIGS, ILLEGAL_GROUP_CHARS)
...
class NodeSetException(Exception):
    __doc__ = 'Base NodeSet exception class.'


class NodeSetError(NodeSetException):
    __doc__ = 'Raised when an error is encountered.'


class NodeSetParseError(NodeSetError):
    __doc__ = 'Raised when NodeSet parsing cannot be done properly.'
...
class NodeSetParseRangeError(NodeSetParseError):
    __doc__ = 'Raised when bad range is encountered during NodeSet parsing.'
...
class NodeSetExternalError(NodeSetError):
    __doc__ = 'Raised when an external error is encountered.'
...
class NodeSetBase(object):
    ...
    def __init__(self, pattern=None, rangeset=None, copy_rangeset=True, autostep=None, fold_axis=None):
        ...
        if self.fold_axis is None:
            if DEFAULTS.fold_axis:
                self.fold_axis = DEFAULTS.fold_axis
        if pattern:
        ...
        for pat, rset in sorted(self._patterns.items()):
            if rset:
                ...
                if rset.dim() == 1:
                    ...
                    for idx in rset:
                        yield (
                         pat, (idx,), (padding,), autostep)

                else:
                    for args, padding in rset.iter_padding():
                        yield (
                         pat, args, padding, autostep)
        ...
        for pat, ivec, pads, _ in self._iter():
            if ivec is not None:
                if len(ivec) == 1:
                    ...
                    yield pat % tuple(['%0*d' % (pad or 0, i) for pad, i in zip(pads, ivec)])
            else:
                yield pat % ()
    ...
    def _iter_nd_pat(self, pat, rset):
        ...
        try:
            ...
            if self.fold_axis is None:
                ...
                fold_axis = [int(x) % dimcnt for x in self.fold_axis if -dimcnt <= int(x) < dimcnt]
        ...
        try:
            for pat, rset in sorted(self._patterns.items()):
                if not rset:
                    ...
                    if rset.dim() == 1:
                        if self.fold_axis is None or list(x for x in self.fold_axis if -1 <= int(x) < 1):
                            rgs = str(rset)
                            cnt = len(rset)
                            if cnt > 1:
                                rgs = '[%s]' % rgs
            ...
            raise NodeSetParseError(pat, 'Internal error: node pattern and ranges mismatch')
                ...
                if index.step is None:
                    ...
                    if index.step < 0:
                        if index.start is not None or index.stop is not None:
                            raise IndexError('illegal start and stop when negative step is used')
                        ...
                        sl_step = index.step
        if not isinstance(sl_start, int) or not isinstance(sl_stop, int) or not isinstance(sl_step, int):
    ...
    def symmetric_difference_update(self, other):
        ...
        for pat, brangeset in other._patterns.items():
            rangeset = self._patterns.get(pat)
            if not rangeset and pat not in self._patterns:
...
class ParsingEngine(object):
        ...
        for opc, pat, rgnd in self._scan_string(nsstr, autostep):
            if self.group_resolver and pat[0] == '@':
                ...
                for nodegroup in NodeSetBase(pat, rgnd):
                    ns_str_ext, ns_nsp_ext = self.parse_group_string(nodegroup, namespace)
                    if ns_str_ext:
                        ns_group.update(self.parse_string(ns_str_ext, autostep, ns_nsp_ext))

                getattr(nodeset, opc)(ns_group)
            else:
                if self.group_resolver and self.node_wc and ('*' in pat or '?' in pat):
                    ...
                    if alln_cache is None:
                        ...
                        try:
                            ...
                            for res in self.all_nodes(namespace):
                                nsb.update(self.parse_string(res, autostep, namespace))
    ...
    def parse_string_single(self, nsstr, autostep):
        ...
        pat, rangesets = self._scan_string_single(_strip_escape(nsstr), autostep)
        if len(rangesets) > 1:
    ...
    def all_nodes(self, namespace=None):
        ...
        try:
            ...
            try:
                ...
                msg = 'Not enough working methods (all or map + list) to get all nodes'
        ...
        for pfx, idx in pfx_nd:
            if idx:
                ...
                if int(idx) != 0:
                    ...
                    idxint = 0
                if idxint > 1e+100:
                    raise NodeSetParseRangeError(RangeSetParseError(idx, 'invalid rangeset index'))
                pat += '%s%%s' % pfx
    ...
    def _scan_string(self, nsstr, autostep):
        ...
        while nsstr:
            ...
            if bracket_idx >= 0 and (op_idx > bracket_idx or op_idx < 0):
                ...
                while bracket_idx >= 0 and (op_idx > bracket_idx or op_idx < 0):
                    ...
                    if pfxlen > 0:
                        ...
                        if pfx:
                            ...
                            rsets += pfxrvec
                    bracket_idx = sfx.find(self.BRACKET_OPEN, bracket_idx - pfxlen)
                ...
                if not nsstr:
                    msg = "missing nodeset operand with '%s' operator" % next_op_code
    ...
    def _amend_leading_digits(self, outer, inner):
        ...
        if outerstriplen < outerlen:
            ...
            inner = ','.join('-'.join(outerdigits + bound for bound in elem.split('-')) for elem in (str(subrng) for subrng in RangeSet(inner).contiguous()))
        return (
         outerstrip, inner)
    ...
    def _amend_trailing_digits(self, outer, inner):
        ...
        if outerstriplen < outerlen:
            ...
            inner = ','.join(shiftstep(s, outlen) for s in ('-'.join(bound + outerdigits for bound in elem.split('-')) for elem in inner.split(',')))
        return (
         outerstrip, inner)
...
class NodeSet(NodeSetBase):
    ...
    def __init__(self, nodes=None, autostep=None, resolver=None, fold_axis=None):
    ...
    def _groups2(self, groupsource=None, autostep=None):
        ...
        groups_info = {}
        allgroups = {}
        if self._resolver.has_node_groups(groupsource) and (not allgrplist or len(allgrplist) >= len(self)):
            pass
        else:
            if not allgrplist:
                return groups_info
                ...
                try:
                    for grp in allgrplist:
                        ...
                        allgroups[grp] = NodeSet((','.join(nodelist)), resolver=(self._resolver))
    ...
    def groups(self, groupsource=None, noprefix=False):
        ...
        for grp, (_, nsb) in groups.items():
            if groupsource:
                ...
                key = '@' + grp
            result[key] = (
             NodeSet(nsb, resolver=(self._resolver)),
             self.intersection(nsb))

        return result

    def regroup(self, groupsource=None, autostep=None, overlap=False, noprefix=False):
        ...
        if not groups:
            ...
            for _, grp in sorted(fulls, key=(lambda x: (
             -x[0], x[1]))):
                if not overlap:
                    if groups[grp][1] not in rest:
                        continue
                    elif groupsource and not noprefix:
...
def set_std_group_resolver_config(groupsconf, illegal_chars=None):
    ...
    if groupsconf:
        ...
        group_resolver = NodeUtils.GroupResolverConfig(groupsconf, illegal_chars)
