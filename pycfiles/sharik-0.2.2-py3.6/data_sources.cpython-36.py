...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sharik/data_sources.py
...
class InlineDataSource(DataSource):
    ...
    def provide_files(self) -> Iterable[Tuple[(str, ContentSupplier)]]:
        return tuple((file_name, partial(_id, buffer)) for file_name, buffer in self.collection_of_files)
...
def _gen_files(starting_path: str, is_recursive: bool) -> Generator[(Tuple[(str, str)], None, None)]:
    if isfile(starting_path):
        ...
        yield (
         split(starting_path)[1], starting_path)
    else:
        if isdir(starting_path):
            if is_recursive:
                source = (join(dir_path, file_name) for dir_path, _ignore, file_names in walk(starting_path) for file_name in file_names)
            else:
                source = (full_path for full_path in (join(starting_path, file_name) for file_name in listdir(starting_path)) if isfile(full_path))
...
class FileDataSource(DataSource):
    ...
    def _create_predicate(self) -> Callable[([str], bool)]:
        ...
        def _predicate(tested):
            is_whitelist_match = whitelist_pattern is None or whitelist_pattern.search(tested) is not None
            is_blacklist_non_match = blacklist_pattern is None or not blacklist_pattern.search(tested)
    ...
    def provide_files(self) -> Iterable[Tuple[(str, ContentSupplier)]]:
        ...
        return tuple((archive_path, partial(_read_file, full_path)) for archive_path, full_path in _gen_files(self.scan_root, self.is_recursive) if predicate(full_path))