...
class Query(models.Model):
    ...
    connection = models.CharField(blank=True, null=True, max_length=128, help_text='Name of DB connection (as specified in settings) to use for this query. Will use EXPLORER_DEFAULT_CONNECTION if left blank')
...
class QueryResult(object):
    ...
    def _get_numerics(self):
        if hasattr(self.connection.Database, 'NUMBER'):
            return [ix for ix, c in enumerate(self._description) if hasattr(c, 'type_code') if c.type_code in self.connection.Database.NUMBER.values]
        else:
            if self.data:
                d = self.data[0]
                return [ix for ix, _ in enumerate(self._description) if not isinstance(d[ix], six.string_types) if six.text_type(d[ix]).isnumeric()]
            return []
