...
# [GCC 8.4.0]
# Embedded file name: /Users/muhammadelias/grove_core/django-sql-explorer/explorer/views.py
# Compiled at: 2019-07-02 17:32:38
import re, six
...
from explorer.utils import url_get_rows, url_get_query_id, url_get_log_id, url_get_params, safe_login_prompt, fmt_sql, allowed_query_pks, url_get_show, url_get_fullscreen
...
class ExplorerContextMixin(object):
    ...
    def gen_ctx(self):
        return {'can_view': app_settings.EXPLORER_PERMISSION_VIEW(self.request.user), 'can_change': app_settings.EXPLORER_PERMISSION_CHANGE(self.request.user)}
...
class PermissionRequiredMixin(object):
    ...
    def get_permission_required(self):
        if self.permission_required is None:
            raise ImproperlyConfigured(('{0} is missing the permission_required attribute. Define {0}.permission_required, or override {0}.get_permission_required().').format(self.__class__.__name__))
    ...
    def handle_no_permission(self, request):
        if django.VERSION >= (1, 11):
            return SafeLoginView.as_view(extra_context={'title': 'Log in', REDIRECT_FIELD_NAME: request.get_full_path()})(request)
...
class ListQueryView(PermissionRequiredMixin, ExplorerContextMixin, ListView):
    ...
    def recently_viewed(self):
        qll = QueryLog.objects.filter(run_by_user=self.request.user, query_id__isnull=False).order_by('-run_at').select_related('query')
        ret = []
        tracker = []
        for ql in qll:
    ...
    def _build_queries_and_headers(self):
        ...
        for q in self.object_list:
            ...
            if headers[header] > 1 and header not in rendered_headers:
                dict_list.append({'title': header, 'is_header': True, 
                   'is_in_category': False, 
                   'collapse_target': collapse_target, 
                   'count': headers[header]})
                rendered_headers.append(header)
            model_dict.update({'is_in_category': headers[header] > 1, 'collapse_target': collapse_target, 
...
class PlayQueryView(PermissionRequiredMixin, ExplorerContextMixin, View):
    ...
    def post(self, request):
        ...
        error = (passes_blacklist or MSG_FAILED_BLACKLIST) % (', ').join(failing_words) if 1 else None
    ...
    def render_with_sql(self, request, query, run_query=True, error=None):
        ...
        return self.render_template('explorer/%s.html' % template, query_viewmodel(request.user, query, title='Playground', run_query=run_query, error=error, rows=rows, form=form))
...
class QueryView(PermissionRequiredMixin, ExplorerContextMixin, View):
    ...
    def post(self, request, query_id):
        if not app_settings.EXPLORER_PERMISSION_CHANGE(request.user):
            ...
            vm = query_viewmodel(request.user, query, form=form, run_query=show, rows=url_get_rows(request), message='Query saved.' if success else None)
...
def query_viewmodel(user, query, title=None, form=None, message=None, run_query=True, error=None, rows=app_settings.EXPLORER_DEFAULT_ROWS):
    ...
    ret = {'tasks_enabled': app_settings.ENABLE_TASKS, 
