...
# [GCC 8.4.0]
# Embedded file name: /Users/muhammadelias/grove_core/django-sql-explorer/explorer/models.py
...
from explorer.utils import passes_blacklist, swap_params, extract_params, shared_dict_update, get_s3_bucket, get_params_for_url, get_valid_connection
MSG_FAILED_BLACKLIST = b'Query failed the SQL blacklist: %s'
...
class Query(models.Model):
    ...
    snapshot = models.BooleanField(default=False, help_text=b'Include in snapshot task (if enabled)')
    connection = models.CharField(blank=True, null=True, max_length=128, help_text=b'Name of DB connection (as specified in settings) to use for this query. Will use EXPLORER_DEFAULT_CONNECTION if left blank')

    def __init__(self, *args, **kwargs):
        self.params = kwargs.get(b'params')
        kwargs.pop(b'params', None)
    ...
    def avg_duration(self):
        return self.querylog_set.aggregate(models.Avg(b'duration'))[b'duration__avg']
    ...
    def get_absolute_url(self):
        return reverse(b'query_detail', kwargs={b'query_id': self.id})
    ...
    def snapshots(self):
        if app_settings.ENABLE_TASKS:
            b = get_s3_bucket()
            keys = b.list(prefix=b'query-%s/snap-' % self.id)
            keys_s = sorted(keys, key=lambda k: k.last_modified)
            return [ SnapShot(k.generate_url(expires_in=0, query_auth=False), k.last_modified) for k in keys_s
                   ]
...
class QueryResult(object):
    ...
    def _get_headers(self):
        if self._description:
            return [ ColumnHeader(d[0]) for d in self._description ]
        return [ColumnHeader(b'--')]

    def _get_numerics(self):
        if hasattr(self.connection.Database, b'NUMBER'):
            return [ ix for ix, c in enumerate(self._description) if hasattr(c, b'type_code') and c.type_code in self.connection.Database.NUMBER.values ]
        if self.data:
            d = self.data[0]
    ...
    def process(self):
        ...
        self.process_rows()
        logger.info(b'Explorer Query Processing took %sms.' % ((time() - start_time) * 1000))
...
class ColumnSummary(object):
    ...
    def __init__(self, header, col):
        self._header = header
        self._stats = [
         ColumnStat(b'Sum', sum),
         ColumnStat(b'Avg', lambda x: float(sum(x)) / float(len(x))),
         ColumnStat(b'Min', min),
         ColumnStat(b'Max', max),
         ColumnStat(b'NUL', lambda x: int(sum(map(lambda y: 1 if y is None else 0, x))), 0, True)]
