...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/ezpyinline.py
# Compiled at: 2007-02-08 05:46:57
'''ezpyinline
...
'''
import os, string, re, sys, os.path, shutil
...
class c_util:
    ...
    c_doublequote = '(?:"(?:\\\\.|[^"\\\\])*")'
    c_singlequote = "(?:\\'(?:\\\\.|[^\\'\\\\])*\\')"
    c_comment = re.compile('(%s|%s)|(?:%s|%s|%s)' % (c_doublequote, c_singlequote, c_cppcomment, c_simplecomment, c_directive))
    const = re.compile('\\s*const\\s*')
    star = re.compile('\\s*\\*\\s*')
    _c_pandn = '((?:(?:[\\w*]+)\\s+)+\\**)(\\w+)'
    c_pandm = re.compile(_c_pandn)
    _c_function = _c_pandn + '\\s*\\(([^\\)]*)\\)'
    c_function_def = re.compile('(?:%s|%s)|(%s)' % (c_doublequote, c_singlequote, _c_function + '\\s*(?:\\{|;)'))
    ...
    def findFunctionDefs(code):
        ...
        for match in c_util.c_function_def.findall(code):
            if match[0]:
                functionDefs.append({'return_type': c_util.trimWhite(match[1]), 'name': c_util.trimWhite(match[2]), 'rawparams': c_util.trimWhite(match[3])})
...
class Builder:
    __module__ = __name__
    ptStringMap = {'unsigned': 'i', 'unsigned int': 'i', 'int': 'i', 'long': 'l', 'float': 'f', 'double': 'd', 'char': 'c', 'short': 'h', 'char*': 's', 'PyObject*': 'O'}
    ...
    def _import(self):
        ...
        build_platlib = os.path.join(self._buildDir, 'build', 'lib' + plat_specifier)
        sys.path.append(build_platlib)
        import imp
        (fp, pathname, description) = imp.find_module(self._moduleName)
        try:
            module = imp.load_module(self._moduleName, fp, pathname, description)
        finally:
            if fp:
                fp.close()
        if self._options.has_key('targetmodule'):
            ...
            for method in self._methods:
                setattr(self._options.get('targetmodule'), method['name'], getattr(module, method['name']))
    ...
    def _parseParams(self, params):
        ...
        if self._void.match(params) or self._blank.match(params) or params == '':
            return []
    ...
    def _parseType(self, typeString):
        ...
        type['text'] = c_util.trimWhite(c_util.star.sub('', typeString) + '*' * type['pointers'])
        return type
    ...
    def _writeModule(self):
        ...
        try:
            srcFile = open(os.path.join(self._buildDir, self._srcFileName), 'w')
        except IOError, e:
        ...
        table = string.join(map(lambda (x): '{"%s", %s, METH_VARARGS}' % (x['name'], x['hashname']), self._methods), ',\n  ')
        srcFile.write(table + ',\n  ')
    ...
    def _writeMethodBody(self, srcFile, method):
        ...
        if method['params']:
            ...
            ptArgs = string.join(map(lambda (x): '&%s' % x['name'], method['params']), ', ')
            srcFile.write('  if(!PyArg_ParseTuple(args, "%s", %s))\n' % (ptString, ptArgs))
            srcFile.write('    return NULL;\n')
        retvalString = ''
        if method['return_type'] != 'void':
            retvalString = '_retval = '
        srcFile.write('  %s%s(%s);\n' % (retvalString, method['name'],
         string.join(map(lambda (x): '%s' % x['name'], method['params']), ', ')))
        if method['return_type'] == 'void':
            ...
            try:
                ...
                raise BuildError("Can't handle return type '%s' in function '%s'" % (method['return_type'], method['name']))
    ...
    def _compile(self):
        ...
        ext = Extension(self._moduleName, [
         self._srcFileName], library_dirs=self._options.get('library_dirs'), libraries=self._options.get('libraries'), define_macros=self._options.get('define_macros'), undef_macros=self._options.get('undef_macros'))
        try:
            setup(name=self._moduleName, version=self._moduleVersion, ext_modules=[ext], script_args=['-q', 'build'] + (self._options.get('distutils_args') or []), script_name='C.py', package_dir=self._buildDir)
        except SystemExit, e:
            raise BuildError(e)
    ...
    def _buildPTString(self, params):
        ...
        for param in params:
            if self.ptStringMap.has_key(param['type']):
                ...
                raise BuildError("Cannot map argument type '%s' for argument '%s'" % (param['type'], param['name']))
