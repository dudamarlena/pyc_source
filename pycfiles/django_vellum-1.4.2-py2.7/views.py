...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/vellum/views.py
# Compiled at: 2012-09-15 16:55:11
...
class PostView(DateDetailView):
    ...
    def get_object(self):
        ...
        if not self.request.user.is_superuser and object.status != 2:
            raise Http404
        if self.request.META.get('REMOTE_ADDR') not in blog_settings.BLOG_INTERNALIPS:
...
def search(request, template_name='vellum/post_search.html', search_fields=[
 'title', 'body', 'tags__name', 'categories__title']):
    """
