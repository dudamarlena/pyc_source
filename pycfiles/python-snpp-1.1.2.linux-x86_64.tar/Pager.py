...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.6/dist-packages/Pager.py
# Compiled at: 2010-05-28 18:46:03
from snpplib import *
...
class Pager:
    ...
    def __init__(self, recips=[], message=None, host=SNPP_HOST, port=SNPP_PORT, debuglevel=0, server=None):
        ...
        self.recip_cmds = {'alert': self.server.alert, 'level': self.server.level, 'time': self.hold_until, 
           'coverage': self.server.coverage}
        self.mess_cmds = {'callerid': self.server.callerid, 'subject': self.server.subject}

    def add_recipient(self, recip):
        if type(recip) == type([]):
            self.recips[(len(self.recips)):] = recip
        else:
            self.recips[(len(self.recips)):] = [
             recip]
    ...
    def _get_parm(self, recip, parm):
        if recip and hasattr(recip, parm) and getattr(recip, parm):
            ...
            if hasattr(self.message, parm) and getattr(self.message, parm):
...
class Message(MRBase):
    ...
    def add_response(self, response):
        ...
        if type(response) == type([]):
            ...
            self.responses[(len(self.responses)):] = [
             response]
