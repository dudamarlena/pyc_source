...
# [GCC 8.4.0]
# Embedded file name: /Users/jmcarp/miniconda/envs/guardian/lib/python2.7/site-packages/guardrail/ext/peewee.py
# Compiled at: 2015-04-02 00:49:10
...
class PeeweePermissionManager(models.BasePermissionManager):
    ...
    def _add_permission(self, agent, target, schema, permission):
        try:
            return schema.create(agent=agent, target=target, permission=permission)
...
def _reference_column(schema, **kwargs):
    return pw.ForeignKeyField(schema, on_update='CASCADE', on_delete='CASCADE', **kwargs)
...
class PeeweePermissionSchemaFactory(models.BasePermissionSchemaFactory):
    ...
    def _make_schema_meta(self, agent, target):
        return type('Meta', (
         object,), dict(db_table=self._make_table_name(agent, target), indexes=(
         (
          ('agent', 'target', 'permission'), True),)))

    def _make_schema_dict(self, agent, target):
        return dict(Meta=self._make_schema_meta(agent, target), id=pw.PrimaryKeyField(), agent=_reference_column(agent, null=False, index=True), target=_reference_column(target, null=False, index=True), permission=pw.CharField(null=False, index=True))
...
class PeeweeLoader(models.BaseLoader):
    ...
    def __call__(self, *args, **kwargs):
        return self.schema.select().where(self.column == kwargs.get(self.kwarg)).first()