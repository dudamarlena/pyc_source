...
# [GCC 8.4.0]
# Embedded file name: /Users/jmcarp/miniconda/envs/guardian/lib/python2.7/site-packages/guardrail/ext/pony.py
# Compiled at: 2015-04-02 00:47:09
...
class PonyPermissionManager(models.BasePermissionManager):
    ...
    def _add_permission(self, agent, target, schema, permission):
        try:
            return schema(agent=agent, target=target, permission=permission)
...
class PonyPermissionSchemaFactory(models.BasePermissionSchemaFactory):
    ...
    def _make_schema_dict(self, agent, target):
        return dict(_table_=self._make_table_name(agent, target), _indexes_=[
         pn.core.Index('agent', 'target', 'permission', is_pk=False, is_unique=True)], id=pn.PrimaryKey(int, auto=True), agent=pn.Required(agent, index=True), target=pn.Required(target, index=True), permission=pn.Required(str, 255, index=True))
...
class PonyLoader(models.BaseLoader):
    ...
    def __call__(self, *args, **kwargs):
        ...
        return pn.select(row for row in self.schema).filter(**query).first()