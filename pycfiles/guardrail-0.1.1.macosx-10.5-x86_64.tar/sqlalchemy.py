...
# [GCC 8.4.0]
# Embedded file name: /Users/jmcarp/miniconda/envs/guardian/lib/python2.7/site-packages/guardrail/ext/sqlalchemy.py
# Compiled at: 2015-04-02 00:47:03
...
class SqlalchemyPermissionManager(models.BasePermissionManager):
    ...
    def _get_permissions(self, agent, target, schema, Agent=None, Target=None, custom=None):
        query = self.session.query(schema.permission)
        query = self._build_query(query, agent, target, schema, Agent=Agent, Target=Target, custom=custom)
        return {each.permission for each in query}

    def _has_permission(self, agent, target, schema, permission, Agent=None, Target=None, custom=None):
        query = self.session.query(schema.permission)
    ...
    def _add_permission(self, agent, target, schema, permission):
        row = schema(agent=agent, target=target, permission=permission)
...
def _reference_column(schema, **kwargs):
    return sa.Column(sa.Integer, sa.ForeignKey(_get_primary_column(schema), onupdate='CASCADE', ondelete='CASCADE'), **kwargs)
...
class SqlalchemyPermissionSchemaFactory(models.BasePermissionSchemaFactory):
    ...
    def _update_parents(self, agent, target, schema):
        ...
        relation = sa.orm.relationship(target, secondary=schema.__table__, backref=backref, passive_deletes=self.cascade, passive_updates=self.cascade)
        setattr(agent, attr, relation)

    def _make_schema_dict(self, agent, target):
        return dict(__tablename__=self._make_table_name(agent, target), __table_args__=(
         sa.UniqueConstraint('agent_id', 'target_id', 'permission'),), id=sa.Column(sa.Integer, primary_key=True), agent_id=_reference_column(agent, nullable=False, index=True), agent=sa.orm.relationship(agent), target_id=_reference_column(target, nullable=False, index=True), target=sa.orm.relationship(target), permission=sa.Column(sa.String, nullable=False, index=True))
...
class SqlalchemyLoader(models.BaseLoader):
    ...
    def __call__(self, *args, **kwargs):
        return self.session.query(self.schema).filter(self.column == kwargs.get(self.kwarg)).first()