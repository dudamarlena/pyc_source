...
# [GCC 8.4.0]
# Embedded file name: /Users/jmcarp/miniconda/envs/guardian/lib/python2.7/site-packages/guardrail/ext/django/models.py
# Compiled at: 2015-04-02 01:00:49
...
class DjangoPermissionManager(models.BasePermissionManager):
    ...
    def _add_permission(self, agent, target, schema, permission):
        try:
            return schema.objects.create(agent=agent, target=target, permission=permission)
...
class DjangoPermissionSchemaFactory(models.BasePermissionSchemaFactory):
    ...
    def _make_schema_meta(self, agent, target):
        return type('Meta', (
         object,), dict(db_table=self._make_table_name(agent, target), unique_together=(('agent', 'target', 'permission'), )))

    def _make_schema_dict(self, agent, target):
        return dict(Meta=self._make_schema_meta(agent, target), __module__=__name__, id=db.models.AutoField(primary_key=True), agent=db.models.ForeignKey(agent, null=False, db_index=True), target=db.models.ForeignKey(target, null=False, db_index=True), permission=db.models.CharField(max_length=255, null=False, db_index=True))
