...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nsq2kafka/__init__.py
# Compiled at: 2016-08-04 15:11:04
from kafka import KafkaProducer
import nsq, json, signal, logging, tornado.ioloop
...
class NSQ2Kafka(object):
    ...
    def __init__(self, **kwargs):
        nsq_args = {k[4:]:v for k, v in kwargs.items() if k[:3] == 'nsq' and v is not None if k[:3] == 'nsq' and v is not None}
        kafka_args = {k[6:]:v for k, v in kwargs.items() if k[:4] == 'kafka' and v is not None if k[:4] == 'kafka' and v is not None}
