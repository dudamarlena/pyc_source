...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xicam\gui\widgets\linearworkfloweditor.py
...
class WorkflowWidget(QWidget):
    ...
    def __init__(self, workflowview):
...
class LinearWorkflowView(QTableView):
    ...
    def selectionChanged(self, selected=None, deselected=None):
        if self.selectedIndexes():
            if self.selectedIndexes()[0].row() < self.model().rowCount():
