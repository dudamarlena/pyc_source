...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/timesketch_import_client/importer.py
# Compiled at: 2020-05-05 06:37:07
# Size of source mod 2**32: 23329 bytes
"""Timesketch data importer."""
from __future__ import unicode_literals
import codecs, datetime, io, json, logging, math, os, string, time, uuid, dateutil.parser, pandas, six
...
class ImportStreamer(object):
    __doc__ = 'Upload object used to stream results to Timesketch.'
    DEFAULT_ENTRY_THRESHOLD = 50000
    DEFAULT_FILESIZE_THRESHOLD = 104857600
        ...
        if 'datetime' not in my_dict:
            for key in my_dict:
                ...
                if 'time' not in key_string:
                    pass
                elif key_string == 'timestamp_desc':
                    continue
                value = my_dict[key]
                if isinstance(value, six.string_types):
                    ...
                    try:
                        date = datetime.datetime.utcfromtimestamp(value / 1000000.0)
                        my_dict['datetime'] = date.isoformat()
                        break
                    except ValueError:
                        continue
    ...
    def _fix_data_frame(self, data_frame):
        ...
        if 'datetime' not in data_frame:
            for column in data_frame.columns[data_frame.columns.str.contains('time', case=False)]:
                if column.lower() == 'timestamp_desc':
                    pass
                else:
                    try:
                        data_frame['timestamp'] = pandas.to_datetime(data_frame[column], utc=True)
                        break
                    except ValueError as e:
                        logging.info('Unable to convert timestamp in column: %s, error %s', column, e)

            if 'timestamp' in data_frame:
                data_frame['datetime'] = data_frame['timestamp'].dt.strftime('%Y-%m-%dT%H:%M:%S%z')
    ...
    def _upload_data_buffer(self, end_stream):
        ...
        data = {'name': self._timeline_name, 
         'sketch_id': self._sketch.id, 
         'enable_stream': not end_stream, 
         'index_name': self._index, 
         'events': '\n'.join([json.dumps(x) for x in self._data_lines])}
        logging.debug('Data buffer ready for upload, took {0:.2f} seconds to prepare.'.format(time.time() - start_time))
        response = self._sketch.api.session.post(self._resource_url, data=data)
        time.sleep(2)
        if response.status_code not in definitions.HTTP_STATUS_CODE_20X:
            raise RuntimeError('Error uploading data: [{0:d}] {1!s} {2!s}, index {3:s}'.format(response.status_code, response.reason, response.text, self._index))
        logging.debug('Data buffer nr. {0:d} uploaded, total time: {1:.2f}s'.format(self._chunk, time.time() - start_time))
        self._chunk += 1
    ...
    def _upload_binary_file(self, file_path):
        ...
        if file_size <= self._threshold_filesize:
            ...
            response = self._sketch.api.session.post(self._resource_url, files=file_dict, data=data)
        else:
            ...
            for index in range(0, chunks):
                ...
                response = self._sketch.api.session.post(self._resource_url, files=file_dict, data=data)
                if response.status_code not in definitions.HTTP_STATUS_CODE_20X:
                    raise RuntimeError('Error uploading data chunk: {0:d}/{1:d}. Status code: {2:d} - {3!s} {4!s}'.format(index, chunks, response.status_code, response.reason, response.text))

        if response.status_code not in definitions.HTTP_STATUS_CODE_20X:
            raise RuntimeError('Error uploading data: [{0:d}] {1!s} {2!s}, file: {3:s}, index {4:s}'.format(response.status_code, response.reason, response.text, file_path, self._index))
        response_dict = response.json()
        self._last_response = response_dict
        self._timeline_id = response_dict.get('objects', [{}])[0].get('id')
    ...
    def add_data_frame(self, data_frame, part_of_iter=False):
        ...
        if 'datetime' not in data_frame_use:
            raise ValueError('Need a field called datetime in the data frame that is formatted according using this format string: %Y-%m-%dT%H:%M:%S%z. If that is not provided the data frame needs to have a column that has the word "time" in it, that can be used to conver to a datetime field.')
        if 'message' not in data_frame_use:
            raise ValueError('Need a field called message in the data frame, use the formatting string to generate one automatically.')
        ...
        for index in range(0, chunks):
            ...
            data_chunk = data_frame_use[chunk_start:chunk_start + self._threshold_entry]
    ...
    def add_file(self, filepath, delimiter=','):
        ...
        file_ending = filepath.lower().split('.')[(-1)]
        if file_ending == 'csv':
            with codecs.open(filepath, 'r', encoding=self._text_encoding, errors='replace') as (fh):
                for chunk_frame in pandas.read_csv(fh, delimiter=delimiter, chunksize=self._threshold_entry):
                    self.add_data_frame(chunk_frame, part_of_iter=True)

        else:
            if file_ending == 'plaso':
    ...
    def add_json(self, json_entry, column_names=None):
        ...
        if isinstance(json_obj, (list, tuple)):
            if not column_names:
                raise TypeError('Data is a list, but there are no defined column names.')
            if not len(json_obj) != len(column_names):
                raise TypeError('The number of columns ({0:d}) does not match the number of columns in the JSON list ({1:d})'.format(len(column_names), len(json_obj)))
            json_dict = dict(zip(column_names, json_obj))
        else:
            if isinstance(json_obj, dict):
                json_dict = json_obj
            else:
                raise TypeError('The JSON object needs to be either a dict or a list with defined column names.')
    ...
    def close(self):
        ...
        pipe_resource = '{0:s}/sketches/{1:d}/analyzer/'.format(self._sketch.api.api_root, self._sketch.id)
    ...
    def timeline(self):
        ...
        timeline_obj = timeline.Timeline(timeline_id=self._timeline_id, sketch_id=self._sketch.id, api=self._sketch.api, name=self._timeline_name, searchindex=self._index)
        return timeline_obj
