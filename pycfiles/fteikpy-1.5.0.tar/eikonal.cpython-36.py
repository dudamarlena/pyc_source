...
# [GCC 8.4.0]
# Embedded file name: ..\fteikpy\eikonal.py
# Compiled at: 2018-02-24 10:43:03
...
class Eikonal:
    ...
    def __init__(self, velocity_model, grid_size, n_sweep=1, zmin=0.0, xmin=0.0, ymin=0.0):
        if not isinstance(velocity_model, np.ndarray):
            if velocity_model.ndim not in (2, 3):
            ...
            if self._n_dim == 3:
                raise isinstance(ymin, (int, float)) or ValueError('ymin must be an integer or float')
        else:
            self._ymin = ymin
    ...
    def rescale(self, new_shape):
        ...
        new_shape : list or ndarray
            New shape.
        """
        if not isinstance(new_shape, (list, tuple, np.ndarray)) or len(new_shape) != self._n_dim or not np.all([isinstance(n, int) for n in new_shape]):
    ...
    def solve(self, sources, dtype='float32', n_threads=1):
        ...
        if not isinstance(sources, (list, tuple, np.ndarray)):
            ...
            if self._n_dim == 2:
                ...
                grid = fteik2d.solve((1.0 / self._velocity_model), (src_shift[:, 0]), (src_shift[:, 1]), dz,
                  dx, (self._n_sweep), n_threads=n_threads)
                tt = [TTGrid(grid=np.array(g, dtype=dtype), source=s, grid_size=(self._grid_size), zmin=(self._zmin), xmin=(self._xmin)) for g, s in zip(grid, src)]
            elif self._n_dim == 3:
                ...
                grid = fteik3d.solve((1.0 / self._velocity_model), (src_shift[:, 0]), (src_shift[:, 1]), (src_shift[:, 2]), dz,
                  dx, dy, (self._n_sweep), n_threads=n_threads)
                tt = [TTGrid(grid=np.array(g, dtype=dtype), source=s, grid_size=(self._grid_size), zmin=(self._zmin), xmin=(self._xmin), ymin=(self._ymin)) for g, s in zip(grid, src)]
        if isinstance(sources, (list, tuple)) or sources.ndim == 1:
            return tt[0]
        else:
            return tt
    ...
    def plot(self, n_levels=200, axes=None, figsize=(10, 4), cont_kws={}):
        ...
        if not isinstance(n_levels, int) or n_levels < 1:
            ...
            if axes is not None:
                ...
                raise isinstance(cont_kws, dict) or ValueError('cont_kws must be a dictionary')
            if self._n_dim == 2:
                if axes is None:
