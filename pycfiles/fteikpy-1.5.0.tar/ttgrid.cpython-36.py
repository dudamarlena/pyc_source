...
# [GCC 8.4.0]
# Embedded file name: ..\fteikpy\ttgrid.py
# Compiled at: 2018-02-19 18:12:16
# Size of source mod 2**32: 14725 bytes
"""
...
class TTGrid:
    ...
    def __init__(self, grid=None, grid_size=None, source=None, zmin=0.0, xmin=0.0, ymin=0.0):
        if grid is not None:
            if not isinstance(grid, np.ndarray):
                if grid.ndim not in (2, 3):
                    raise ValueError('grid must be a 2-D or 3-D ndarray')
                else:
                    if grid is not None and np.any(grid < 0.0):
                ...
                if grid_size is not None and np.any(np.array(grid_size) <= 0.0):
    ...
    def get(self, zq, xq, yq=None, check=True):
        ...
        if check:
            if not isinstance(zq, (int, float)):
            ...
            elif not isinstance(xq, (int, float)):
                ...
                if yq is not None:
                    ...
                    raise self._yaxis[0] <= yq <= self._yaxis[(-1)] or ValueError('yq out of bounds')
        if self._n_dim == 2:
            tq = fteik2d.interp2(self._zaxis, self._xaxis, self._grid, zq, xq)
        else:
            if self._n_dim == 3:
                tq = fteik3d.interp3(self._zaxis, self._xaxis, self._yaxis, self._grid, zq, xq, yq)
        return tq

    def raytracer(self, receivers, ray_step=1.0, max_ray=10000, n_threads=1):
        """
        ...
        if self._n_dim == 3:
            ...
            if self._n_dim == 2:
                ...
                if isinstance(receivers, (list, tuple)) or receivers.ndim == 1:
                    ...
                    ray, npts = fteik2d.ray2d(self._grid, zsrc, xsrc, zrcv, xrcv, dz, dx, ray_step, max_ray)
                    return Ray(z=(ray[:npts, 0] + self._zmin), x=(ray[:npts, 1] + self._xmin))
                else:
                    for z, x in zip(zrcv, xrcv):
                        self._check_2d(z, x)

                    rays, npts = fteik2d.raytracer2d((self._grid), zsrc, xsrc, zrcv, xrcv, dz,
                      dx, ray_step, max_ray, n_threads=n_threads)
    ...
    def load(self, filename):
        ...
        with open(filename, 'rb') as (f):
            tmp = pickle.load(f)
        if tmp.n_dim == 2:
            self.__init__(grid=(tmp.grid), grid_size=(tmp.grid_size),
              source=(tmp.source),
              zmin=(tmp.zmin),
              xmin=(tmp.xmin))
        elif tmp.n_dim == 3:
