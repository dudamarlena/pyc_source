...
# [GCC 8.4.0]
# Embedded file name: ..\fteikpy\bspline_model.py
# Compiled at: 2018-02-06 17:58:23
# Size of source mod 2**32: 9652 bytes
"""
...
__all__ = [
 'bspline1', 'bspline2', 'vel2spl', 'spl2vel']
...
def bspline1(x, y, xq, order=4, check=True):
    ...
    if check:
        if not isinstance(x, (np.ndarray, list)):
            if not np.asarray(x).ndim != 1:
                raise ValueError('x must be a 1-D ndarray')
            else:
                if not isinstance(y, (np.ndarray, list)):
                    if not np.asarray(y).ndim != 1:
                        raise ValueError('y must be a 1-D ndarray')
                if len(x) != len(y):
                    raise ValueError('x and y must have the same length')
                if not isinstance(xq, (np.ndarray, list)):
                    if not np.asarray(xq).ndim != 1:
        ...
        if not isinstance(x, (np.ndarray, list)):
            if not np.asarray(x).ndim != 1:
                ...
                if not np.asarray(y).ndim != 1:
                    ...
                    if not isinstance(xq, (np.ndarray, list)):
                        if np.asarray(xq).ndim in (1, 2):
                            raise ValueError('xq must be a 1-D or 2-D ndarray')
                    if not isinstance(yq, (np.ndarray, list)):
                        if np.asarray(yq).ndim in (1, 2):
...
def spl2vel(spl, grid_shape, order=4, n_threads=1):
    ...
    Currently only available in 2-D.
    """
    if not isinstance(spl, np.ndarray):
        if spl.ndim != 2:
