...
# [GCC 8.4.0]
# Embedded file name: ..\fteikpy\ray.py
# Compiled at: 2018-02-22 12:49:51
...
def ray_coverage(rays, grid_shape, grid_size, normed=False, zmin=0.0, xmin=0.0, ymin=0.0):
    """
    ...
    if not isinstance(rays, (list, tuple)) or not np.all([isinstance(ray, Ray) for ray in rays]):
        ...
        if not np.all([isinstance(n, int) for n in grid_shape]) or len(grid_shape) not in (2,
                                                                                           3):
