...
# Embedded file name: /css_html_js_minify/css_minifier.py
# Compiled at: 2018-04-14 09:27:13
# Size of source mod 2**32: 11827 bytes
__doc__ = 'CSS Minifier functions for CSS-HTML-JS-Minify.'
import re, itertools
...
def _prioritify(line_of_css, css_props_text_as_list):
    ...
    return (
     priority_integer, group_integer)
...
def _props_grouper(props, pgs):
    ...
    if not props:
        ...
        props_by_groups = map(lambda item: sorted(item,
          key=(lambda item: item[0][0])), props_by_groups)
        props = []
...
def sort_properties(css_unsorted_string):
    ...
    """
    css_pgs = _compile_props(CSS_PROPS_TEXT, grouped=False)
...
def remove_comments(css):
    ...
    while comment_start >= 0:
        ...
        if comment_end < 0:
            css = preserve or css[:comment_start]
            break
        else:
...
def condense_zero_units(css):
    ...
    return re.sub('([\\s:])(0)(px|em|%|in|q|ch|cm|mm|pc|pt|ex|rem|s|ms|deg|grad|rad|turn|vw|vh|vmin|vmax|fr)', '\\1\\2', css)
...
def condense_hex_colors(css):
    ...
    regex = re.compile('([^\\"\'=\\s])(\\s*)#([0-9a-f])([0-9a-f])([0-9a-f])([0-9a-f])([0-9a-f])([0-9a-f])', re.I | re.S)
    match = regex.search(css)
    while match:
...
def condense_std_named_colors(css):
    ...
    for color_name, color_hexa in iter(tuple({':aqua;':':#0ff;', 
...
def unquote_selectors(css):
    ...
    return re.compile('([a-zA-Z]+)="([a-zA-Z0-9-_\\.]+)"]').sub('\\1=\\2]', css)
