...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/src/p_util/old_util.py
# Compiled at: 2020-04-07 15:16:31
# Size of source mod 2**32: 5834 bytes
import numpy as np
...
def getAverages(dataLocation, method='mean', map='flat'):
    ...
    channelMap = [
     [
      5, 4, 6, 3, 7, 2, 8, 1], [13, 12, 14, 11, 15, 10, 16, 9], [21, 20, 22, 19, 23, 18, 24, 17], [29, 28, 30, 27, 31, 26, 32, 25]]
...
def plotAverageSignal(dataLocation):
    ...
    summatedSignal = (summatedSignal / len(signals)) ** 0.5
    plt.plot(np.arange(len(summatedSignal)), summatedSignal)


def plotAverages(dataLocation, method='std'):
    channelMap = [5, 4, 6, 3, 7, 2, 8, 1, 13, 12, 14, 11, 15, 10, 16, 9, 21, 20, 22, 19, 23, 18, 24, 17, 29, 28, 30, 27, 31, 26, 32, 25]
...
def plotFiltered(dataLocation, r=[0.1, 200]):
    file = pyopenephys.File(dataLocation)
    experiments = file.experiments
    experiment = experiments[0]
    recordings = experiment.recordings
...
def plotHilbert(dataLocation, r=[
 0.1, 200]):
    fs, signals = getSignals(dataLocation)
    fc = (r[0], r[1])
    for x in range(32):
