...
# [GCC 8.4.0]
# Embedded file name: /home/mafju/current/icm/removing_madis_from_code/avroknife/avroknife/test/command_line_runner.py
...
class RunResult:
    ...
    def __str__(self):
        return ('stdout: {}\n\ninput_dir={}\noutput_dir={}').format(self.__stdout, self.__input_dir, self.__output_dir)
...
class CommandLineRunner:
    ...
    def __init__(self, program_path, local_input_dir, enforce_local=False):
        ...
        self.__local_tmp_dir = self.__initialize_tmp_dirs(self.__local_fs, local_input_dir)
        if not self.__enforce_local:
            self.__hdfs = HDFS()
            self.__hdfs_tmp_dir = self.__initialize_tmp_dirs(self.__hdfs, local_input_dir)
        self.__is_closed = False

    @staticmethod
    def __initialize_tmp_dirs(fs, local_input_dir):
        dir_ = fs.create_temporary_dir()
        fs.copy_from_local_dir(local_input_dir, fs.join_path([dir_, CommandLineRunner.__input_subdir]))
        fs.create_dir(fs.join_path([dir_, CommandLineRunner.__outputs_subdir]))
        return dir_

    def run(self, args_string, is_input_local, is_output_local, discard_stderr=False):
        """
        ...
        if self.__enforce_local:
            if not (is_input_local and is_output_local):
                raise Exception(('is_input_local={}, is_output_local={}, while the enforce_local mode allows running the program only on local file system ').format(is_input_local, is_output_local))
        local_out_dir = self.__local_fs.create_temporary_dir(self.__local_fs.join_path([self.__local_tmp_dir, self.__outputs_subdir]))
        hdfs_out_dir = None
        if not is_output_local:
            hdfs_out_dir = self.__hdfs.create_temporary_dir(self.__hdfs.join_path([self.__hdfs_tmp_dir, self.__outputs_subdir]))
        args_replaced = self.__replace_args(args_string, is_input_local, is_output_local, local_out_dir, hdfs_out_dir)
        ...
        return RunResult(stdout, os.path.join(self.__local_tmp_dir, self.__input_subdir), local_out_dir)
    ...
    def run_raw(self, args_string, discard_stderr=False):
        ...
        Returns:
            stdout string
        """
        return self.__system(self.__program_path + ' ' + args_string, discard_stderr)

    def __replace_args(self, args_string, is_input_local, is_output_local, local_out_dir, hdfs_out_dir):
        text = args_string
        if is_input_local:
            text = self.__replace(text, self.__input_file_prefix, self.__local_path_prefix, self.__local_fs.join_path, [
             self.__local_tmp_dir, self.__input_subdir])
        else:
            text = self.__replace(text, self.__input_file_prefix, self.__hdfs_path_prefix, self.__hdfs.join_path, [
             self.__hdfs_tmp_dir, self.__input_subdir])
        if is_output_local:
            text = self.__replace(text, self.__output_file_prefix, self.__local_path_prefix, self.__local_fs.join_path, [
             local_out_dir])
        else:
            text = self.__replace(text, self.__output_file_prefix, self.__hdfs_path_prefix, self.__hdfs.join_path, [
             hdfs_out_dir])
        return text

    @staticmethod
    def __replace(text, placeholder_prefix, path_prefix, path_joiner_function, dir_name_elements):
        """Replace placeholders with paths to files"""
        replaced = Tools.replace(text, placeholder_prefix, lambda s: path_prefix + path_joiner_function(dir_name_elements + [s]))
        return replaced
