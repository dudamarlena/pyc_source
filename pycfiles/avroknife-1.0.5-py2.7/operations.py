...
# [GCC 8.4.0]
# Embedded file name: /home/mafju/current/icm/removing_madis_from_code/avroknife/avroknife/operations.py
...
def extract(data_store, record_selector, value_field, name_field=None, create_dirs=None, output_dir_path=None):
...
def __prepare_dir(output_dir):
    if output_dir.exists():
        if not output_dir.is_dir():
            error(("File with name '{}' already exists. Unable to create a directory with the same name").format(output_dir))
            raise Exception()
    else:
...
def __prepare_file(output_file, name_field):
    if output_file.exists():
        error_string = ("The file with name '{}' already exists.").format(output_file)
        if name_field is not None:
            error_string = error_string + (" This is probably because the selected values of given field '{}' are not unique").format(name_field)
        error(error_string)
        raise Exception()
    return output_file
...
def copy(data_store, record_selector, output_dir_path):
    ...
    with output_dir_path.append('content.avro').open('w') as (output):
        with DataFileWriter(output, DatumWriter(), data_store.get_schema()) as (writer):
