...
# [GCC 8.4.0]
# Embedded file name: /home/dev/dev/django-rest-framework-features/rest_framework_features/schema.py
# Compiled at: 2019-10-22 18:39:05
# Size of source mod 2**32: 4525 bytes
import re
...
_schema_cache = None
_humanize_pattern = re.compile('([a-z]|[0-9]+[a-z]?|[A-Z]?)([A-Z0-9])')

def view(*groups, **features):

    def decorator(view_class):
        if not features:
            raise ImproperlyConfigured('Invalid view decorator specified for %s. At least one http method kwargs is required' % (
             view_class,))
...
def get_schema(use_cache=True):
    ...
    for url, http_method_name, view_func in end_points:
        ...
        *groups, feature_name = description
        ...
        verbose_name = _humanize_pattern.sub('\\1 \\2', feature_name).lower()
        feature_def = dict(name=feature_name,
          url=url,
...
def render_schema(*args, **kwargs):
    ...
    return render_to_string(args, **kwargs, **{'context': context})
...
__all__ = ('REGISTRY_ATTR_NAME', 'view', 'get_schema', 'render_schema', 'render_json_schema',
           'render_locale_js_schema', 'render_locale_py_schema')