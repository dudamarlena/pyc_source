...
# [GCC 8.4.0]
# Embedded file name: /Users/Dave/Dropbox/github_projects/dryxPython/dryxPython/htmlframework/typography.py
# Compiled at: 2013-09-20 11:30:22
"""
_dryxTBS_elements
=============================
:Summary:
...
def p(content='', lead=False, textAlign=False, color=False, navBar=False, onPhone=True, onTablet=True, onDesktop=True):
...
def emphasizeText(style='em', text=''):
...
def abbr(abbreviation='', fullWord=''):
...
def address(name=False, addressLine1=False, addressLine2=False, addressLine3=False, phone=False, email=False, twitterHandle=False):
...
def blockquote(content='', source=False, pullRight=False):
...
def ul(itemList=[], unstyled=False, inline=False, dropDownMenu=False, navStyle=False, navPull=False, navDirection='horizontal', breadcrumb=False, pager=False, thumbnails=False, mediaList=False):
...
def li(content='', span=False, disabled=False, submenuTitle=False, divider=False, navStyle=False, navDropDown=False, pager=False):
...
def a(content='', href=False, tableIndex=False, triggerStyle=False):
    ...
    **Return:**
    ...
    dropdownCaret = ''
    falseList = [
     href, triggerClass, triggerStyle, tableIndex, dropdownCaret]
    for i in range(len(falseList)):
        if not falseList[i]:
            falseList[i] = ''
...
def descriptionLists(orderedDictionary={}, sideBySide=False):
...
def heroUnit(headline='', tagline='', buttonStyle='primary', buttonText='', buttonHref='#'):
...
def pageHeader(headline='', tagline=''):
