...
# [GCC 8.4.0]
# Embedded file name: /Users/breno/Envs/djangoplus/lib/python3.7/site-packages/djangoplus/docs/usecase.py
# Compiled at: 2019-06-02 08:39:53
# Size of source mod 2**32: 29621 bytes
...
import djangoplus.db.models as model_fields
from djangoplus.ui.components.forms import factory, fields as form_fields
from djangoplus.utils import get_fieldsets
from djangoplus.utils.metadata import get_metadata, find_action, find_model_by_verbose_name, find_model_by_add_label, get_field, find_model_by_verbose_name_plural, find_subset_by_title, get_parameters_names
...
class UseCase(object):
    ...
    def __init__(self, name):
        ...
        if name.startswith(_('Access')):
            ...
            if name.startswith(_('List')):
                self._list(name)
            else:
                if name.startswith(_('Register')) and ' como ' in name:
                    self._signup(name)
                else:
                    if name.startswith(_('Register')) and _('in') not in name:
                        self._register(name)
                    else:
                        if name.startswith(_('Add ')):
                            self._add(name)
                        else:
                            self._execute(name)
    ...
    def _find(self, model, registering=False):
        ...
        if dashboard:
            if not registering:
                ...
                return True
        if not CACHE['LAST_AUTHENTICATED_ROLE'] or True in list_shortcut or CACHE['LAST_AUTHENTICATED_ROLE'] in list_shortcut:
    ...
    def _view(self, model, recursive=False):
        ...
        if list_shortcut or list_menu:
            if self._find(model):
                ...
                self._test_function_code.append("        self.click_icon('{}')".format('Visualizar'))
        else:
        ...
        if not accessible:
            raise ValueError('There is no way to access the model "{}" from "", please do one of the following things:\n i) Add the "list_menu" meta-attribute to the model\n ii) Add the "list_shortcut" meta-attribute to the model\n iii) Add the "composition" field attribute to one of foreignkey fields of the model if it exists and the the relation on the foreignkey model'.format(verbose_name))
    ...
    def _list(self, action):
        ...
        if ':' in action:
            ...
            for meta_data in ('can_admin', 'can_admin_by_role', 'can_admin_by_unit',
                              'can_admin_by_organization', 'can_view', 'can_view_by_role',
                              'can_view_by_unit', 'can_view_by_organization'):
    ...
    def _register(self, action):
        ...
        for field in get_metadata(model, 'get_fields'):
            if isinstance(field, model_fields.ForeignKey):
                isinstance(field, model_fields.OneToOneField) or isinstance(field, model_fields.OneToManyField) or required_data.append(field.verbose_name.lower())
        ...
        for meta_data in ('can_admin', 'can_admin_by_role', 'can_admin_by_unit', 'can_admin_by_organization',
                          'can_add', 'can_add_by_role', 'can_add_by_unit', 'can_add_by_organization'):
            for actor in get_metadata(model, meta_data, iterable=True):
                if actor:
                    self.actors.append(actor)
                        ...
                        if field_name in form.base_fields:
                                ...
                                if hasattr(form_field, 'queryset'):
                                    ...
                                    if scopes:
                                        ...
                                        for role_model in CACHE['ROLE_MODELS']:
                                            if CACHE['ROLE_MODELS'][role_model]['name'] == self.actors[0] and CACHE['ROLE_MODELS'][role_model]['scope'] not in scopes:
                                                ignore_field = True
                                                break
                            ...
                            if meta_class:
                                if hasattr(meta_class.model, field_name):
                                    ...
                                    if hasattr(model_field, 'example'):
                                        if model_field.example:
                            ...
                            isinstance(form_field, form_fields.BooleanField) or isinstance(form_field, form_fields.NullBooleanField) or self._test_function_code.append("        self.enter('{}', '{}')".format(form_field.label, value))
    ...
    def _execute_action(self, action):
        ...
        if CACHE['LAST_AUTHENTICATED_USERNAME']:
            ...
            if hasattr(func, '_action'):
                ...
                self.actors or self.actors.append(_('Superuser'))
            else:
                if hasattr(func, '_view_action'):
    ...
    def as_dict(self):
        return dict(name=(self.name), description=(self.description), actors=(self.actors), business_rules=(self.business_rules),
          pre_conditions=(self.pre_conditions),
          post_condition=(self.post_condition),
          scenario=(self.get_interactions_as_string()))