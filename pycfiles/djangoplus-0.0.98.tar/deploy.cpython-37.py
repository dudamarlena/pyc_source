...
# [GCC 8.4.0]
# Embedded file name: /Users/breno/Envs/djangoplus/lib/python3.7/site-packages/djangoplus/admin/management/commands/deploy.py
# Compiled at: 2019-04-07 22:32:11
# Size of source mod 2**32: 29509 bytes
        ...
        if 'help' not in options:
            if settings.DIGITAL_OCEAN_TOKEN:
                ...
                if options.get('create') or settings.DIGITAL_OCEAN_SERVER:
                    ...
                    print('Please, set the DIGITAL_OCEAN_SERVER variable in settings.py or execute the command with --create parameter to create a new droplet.')
            else:
...
def _setup_nginx_file():
    ...
    if exists(file_path):
        ...
        if checked_domain:
            if checked_domain not in file_content:
                content = []
                for line in file_content.split('\n'):
                    if 'server_name ' in line:
...
def _setup_postgres():
    ...
    if not exists(file_path):
        ...
        run('echo "local   all             postgres                                trust\\nhost    all             postgres        127.0.0.1/32            trust\\nhost    all             postgres        ::1/128                 trust" > {}'.format(file_path))
...
def _check_droplet():
    ...
    if 'droplets' in response:
        for droplet in response['droplets']:
            ...
            if not droplet['name'] == project_name:
                if ip_address == settings.DIGITAL_OCEAN_SERVER:
