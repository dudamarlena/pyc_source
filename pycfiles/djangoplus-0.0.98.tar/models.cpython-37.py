...
# [GCC 8.4.0]
# Embedded file name: /Users/breno/.virtualenvs/djangoplus/lib/python3.7/site-packages/djangoplus/admin/models.py
# Compiled at: 2020-03-22 00:35:36
# Size of source mod 2**32: 35200 bytes
import datetime, os, sys, json, uuid, binascii, operator
...
from django.contrib.auth.models import AbstractBaseUser, PermissionsMixin, UserManager as DjangoUserManager, Group, Permission
...
class Log(models.Model):
    ...
    OPERATION_CHOICES = [
     [
      ADD, _('Add')], [EDIT, _('Edit')], [DELETE, _('Delete')]]
    ...
    fieldsets = (
     (
      _('General Data'),
      {'fields': (('content_type', 'operation'), ('user', 'date'), ('object_id', 'object_description'),
 'get_tags')}),
     (
    ...
    def get_action_description(self):
        return (
         _('added'), _('edited'), _('deleted'))[(self.operation - 1)]
    ...
    def get_tags(self):
        ...
        for log_index in self.logindex_set.all():
            date = datetime.datetime(log_index.log.date.year, log_index.log.date.month, log_index.log.date.day, log_index.log.date.hour, log_index.log.date.minute, log_index.log.date.second)
            qs = Log.objects.filter(content_type=(log_index.content_type),
              user=(log_index.log.user),
...
class LogIndex(models.Model):
    ...
    def get_tags(self):
        data = []
        date = datetime.datetime(self.log.date.year, self.log.date.month, self.log.date.day, self.log.date.hour, self.log.date.minute, self.log.date.second)
        qs = Log.objects.filter(content_type=(self.content_type), user=(self.log.user), date__startswith=date, object_id=(self.object_id))
        for log in qs:
            for attr, old, new in json.loads(log.content):
                data.append('{} : {} >> {}'.format(attr, old, new))
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    photo = models.ImageField(upload_to='profiles', null=True, blank=True, default='/static/images/user.png',
        ...
        for lookup in get_metadata(model, 'list_lookups', (), iterable=True):
            ...
            if hasattr(field.remote_field.model, 'organization_ptr') or hasattr(field.remote_field.model, 'unit_ptr'):
                ...
                for subclass in field.remote_field.model.__subclasses__():
                    ...
                    if role_username:
                        role_lookups[get_metadata(subclass, 'verbose_name')] = '{}__{}__{}'.format(lookup, subclass.__name__.lower(), role_username)

        if hasattr(model, 'organization_ptr'):
            if 'id' not in organization_lookups:
                organization_lookups.append('id')
        if hasattr(model, 'unit_ptr'):
            if 'id' not in unit_lookups:
                unit_lookups.append('id')
        if get_metadata(model, 'role_username'):
            if 'id' not in role_lookups:
                ...
                if field.remote_field.model:
                    ...
                    if field.remote_field.model in CACHE['ABSTRACT_ROLE_MODELS']:
                        for to in CACHE['ABSTRACT_ROLE_MODELS'][field.remote_field.model]:
                            role_lookups[get_metadata(to, 'verbose_name')] = '{}__{}__{}'.format(field.name, to.__name__.lower(), CACHE['ROLE_MODELS'][to])

                    if hasattr(field.remote_field.model, 'unit_ptr_id'):
                        if field.name not in unit_lookups:
            ...
            if CACHE['UNIT_MODEL']:
                for field in get_metadata(CACHE['ORGANIZATION_MODEL'], 'fields'):
                    if field.remote_field and hasattr(field.remote_field.model, 'unit_ptr'):
                        ...
                        if unit_lookup not in unit_lookups:
                            if not hasattr(model, 'unit_ptr'):
            ...
            if CACHE['ORGANIZATION_MODEL']:
                for field in get_metadata(CACHE['UNIT_MODEL'], 'fields'):
                    if field.remote_field:
                        ...
                        if organization_lookup not in organization_lookups:
                            hasattr(model, 'organization_ptr') or organization_lookups.append(organization_lookup)

        return (
         organization_lookups, unit_lookups, role_lookups)
    ...
    def get_permission_mapping(self, model, obj=None):
        ...
        if CACHE['UNIT_MODEL']:
            if unit_organization_field_name:
                unit_organization_lookup = 'scope__unit__{}__{}'.format(CACHE['UNIT_MODEL'].__name__.lower(), unit_organization_field_name)
        scope_queryset = self.role_set.filter(active=True).values_list('group__name', 'scope__organization', 'scope__unit', unit_organization_lookup)
            ...
            for group_name in groups:
                ...
                if obj:
                    if type(obj) in CACHE['PERMISSIONS_BY_SCOPE']:
                        ...
                        can_view_by_organization = group_name in CACHE['PERMISSIONS_BY_SCOPE'][type(obj)].get('add_by_organization', [])
                if (can_view or can_view_by_role or can_view_by_unit or can_view_by_organization) is False:
                    ...
                    can_view_by_organization = group_name in CACHE['PERMISSIONS_BY_SCOPE'][model].get('view_by_organization', [])
                if can_view:
                    ...
                    can_edit_by_organization = group_name in CACHE['PERMISSIONS_BY_SCOPE'][model].get('edit_by_organization', [])
                if can_edit:
                    ...
                    can_delete_by_organization = group_name in CACHE['PERMISSIONS_BY_SCOPE'][model].get('delete_by_organization', [])
                if can_delete:
                    ...
                    if can_delete_by_unit:
                        if unit_ids:
                            for unit_lookup in unit_lookups:
                                lookups['delete_lookups'].append((unit_lookup, unit_ids))

                    if can_delete_by_organization:
                        if organization_ids:
                            for organization_lookup in organization_lookups:
                    ...
                    for actions_dict in (CACHE['INSTANCE_ACTIONS'], CACHE['QUERYSET_ACTIONS']):
                        for category in actions_dict.get(model, ()):
                            for key in list(actions_dict[model][category].keys()):
                                ...
                                can_execute = group_name in CACHE['PERMISSIONS_BY_SCOPE'][model].get('{}'.format(view_name), [])
                                can_execute_by_role = group_name in CACHE['PERMISSIONS_BY_SCOPE'][model].get('{}_by_role'.format(view_name), [])
                                can_execute_by_unit = group_name in CACHE['PERMISSIONS_BY_SCOPE'][model].get('{}_by_unit'.format(view_name), [])
                                can_execute_by_organization = group_name in CACHE['PERMISSIONS_BY_SCOPE'][model].get('{}_by_organization'.format(view_name), [])
                                if can_execute:
                                    ...
                                    if can_execute_by_unit:
                                        if unit_ids:
                                            for unit_lookup in unit_lookups:
                                                execute_lookups.append((unit_lookup, unit_ids))

                                if can_execute_by_organization:
                                    if organization_ids:
                                        for organization_lookup in organization_lookups:
        ...
        for codename in ('view', 'list'):
            if model in CACHE['PERMISSIONS_BY_SCOPE']:
                ...
                if CACHE['PERMISSIONS_BY_SCOPE'][model].get('{}_by_organization'.format(codename)) and not organization_lookups:
                    if not unit_lookups:
...
class Settings(models.Model):
    ...
    logo_pdf = models.ImageField((_('PDF Logo')), upload_to='config', help_text=(_('No-background image')), null=True,
      blank=True,
      default='')
