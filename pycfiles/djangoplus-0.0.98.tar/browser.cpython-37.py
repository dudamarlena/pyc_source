...
# [GCC 8.4.0]
# Embedded file name: /Users/breno/Envs/djangoplus/lib/python3.7/site-packages/djangoplus/tools/browser.py
# Compiled at: 2019-03-28 07:43:50
# Size of source mod 2**32: 8270 bytes
import time, datetime, traceback, djangoplus
...
class Browser(webdriver.Firefox):
    ...
    def __init__(self, server_url, options=None, verbose=True, slowly=False, maximize=True, headless=False):
        if not options:
            ...
            if not headless:
                if djangoplus.test.CACHE['HEADLESS']:
    ...
    def back(self, seconds=None):
        if seconds:
            self.wait(seconds)
        if not (self.current_url and self.current_url.endswith('/admin/')):
            self.open('/admin/')
