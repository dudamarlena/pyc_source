...
# [GCC 8.4.0]
# Embedded file name: /Users/breno/Envs/djangoplus/lib/python3.7/site-packages/djangoplus/tools/video.py
# Compiled at: 2019-02-27 17:25:51
# Size of source mod 2**32: 9638 bytes
import re, platform, os, signal
...
class VideoRecorder(object):
    ...
    def start(self):
        if 'darwin' in platform.system().lower():
            ...
            size = re.findall('\\d+x\\d+', [line.decode('utf-8').strip().split()[0] for line in sizes if '*' in line.decode('utf-8')][0])[0]
...
class VideoUploader(object):
    ...
    def __init__(self):
        ...
        client_config = {'installed': {'client_id':VideoUploader.CLIENT_ID, 
                       'project_id':VideoUploader.PROJECT_ID, 
                       'auth_uri':VideoUploader.AUTH_URI,  'token_uri':VideoUploader.TOKEN_URI, 
    ...
    def upload_videos(self, tuples, force=False):
        ...
        for file_path, title in tuples:
            if title not in uploaded_titles or force:
                ...
                body = dict(snippet=dict(title=title, description='', tags=tags, categoryId='22'),
                  status=dict(privacyStatus='public'))
                media_body = MediaFileUpload(file_path, chunksize=(-1), resumable=True)
                request = self.service.videos().insert(part=(','.join(body.keys())),
                  body=body,
