...
# [GCC 8.4.0]
# Embedded file name: /Users/breno/Envs/djangoplus/lib/python3.7/site-packages/djangoplus/ui/components/forms/factory.py
# Compiled at: 2019-04-13 11:50:58
# Size of source mod 2**32: 15717 bytes
import django.apps as apps
from django.conf import settings
from djangoplus.ui.components import forms
from django.contrib.staticfiles import finders
import django.utils.translation as _
from djangoplus.utils.metadata import get_metadata, find_field_by_name, list_related_objects, get_fiendly_name, getattr2, get_parameters_names
...
def get_js(model):
    ...
    if finders.find(file_path):
        return (
         '/static/js/{}.js'.format(model_name),)
    return ()
...
def get_register_form(request, obj):
    ...
    if hasattr(obj, 'get_parent_field'):
        ...
        if parent_field and not obj.pk:
            if parent_field.name in form.fields:
...
def get_action_form(request, obj, action):
    ...
    if action_input:
        if type(action_input) in [str, str]:
            ...
            form_cls = Form
    elif issubclass(form_cls, forms.ModelForm):
        for key in list(initial.keys()):
            if hasattr(obj, key) and obj.pk and getattr(obj, key):
                del initial[key]
    ...
    if action_display:
        for lookup in action_display:
            ...
            form.fields[lookup] = forms.CharField(label=label,
              initial=value,
              required=False,
    ...
    verbose_name = obj.pk or get_metadata(obj.__class__, 'verbose_name')
    form.fields['instance'] = forms.ModelChoiceField((type(obj).objects.all()), label=verbose_name)
    if form.fieldsets:
        form.fieldsets = (
         (
          verbose_name, {'fields': ('instance', )}),) + form.fieldsets
    return form
