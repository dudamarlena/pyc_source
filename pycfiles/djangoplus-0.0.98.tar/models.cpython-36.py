...
# [GCC 8.4.0]
# Embedded file name: /home/breno/Envs/djangoplus/lib/python3.6/site-packages/djangoplus/admin/models.py
# Compiled at: 2018-09-24 08:48:06
# Size of source mod 2**32: 30396 bytes
...
class Log(models.Model):
    ...
    OPERATION_CHOICES = [
     [
      ADD, 'Cadastro'], [EDIT, 'Edição'], [DELETE, 'Exclusão']]
    ...
    def get_organization(self):
        ...
        if organization_subclass:
            for field in get_metadata(type(self), 'fields'):
                if field.remote_field:
                    if field.remote_field.model == organization_subclass:
    ...
    def get_permission_mapping(self, model, obj=None):
                    ...
                    if field.remote_field.model in loader.role_models:
                        ...
                        if field.remote_field.model in loader.abstract_role_models:
                            for to in loader.abstract_role_models[field.remote_field.model]:
                                role_lookups[get_metadata(to, 'verbose_name')] = '{}__{}__{}'.format(field.name, to.__name__.lower(), loader.role_models[to])
        ...
        if loader.unit_model:
            if unit_organization_field_name:
                unit_organization_lookup = 'scope__unit__{}__{}'.format(loader.unit_model.__name__.lower(), unit_organization_field_name)
        ...
        for codename in ('view', 'list'):
            if model in loader.permissions_by_scope:
                if loader.permissions_by_scope[model].get('{}_by_unit'.format(codename)):
                    if not unit_lookups:
                        raise Exception('A "lookup" meta-attribute must point to a Unit model in {}'.format(model))
                if loader.permissions_by_scope[model].get('{}_by_organization'.format(codename)):
                    if not organization_lookups:
                        if not unit_lookups:
...
class Settings(models.Model):
    ...
    logo_pdf = models.ImageField('Logotipo para PDF', upload_to='config', help_text='Imagem sem fundo transparente', null=True,
      blank=True,
      default='')
