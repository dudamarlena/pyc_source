...
# [GCC 8.4.0]
# Embedded file name: /home/breno/Envs/djangoplus/lib/python3.6/site-packages/djangoplus/utils/metadata.py
...
def list_related_objects(cls):
    return [f for f in cls._meta.get_fields() if f.one_to_many or f.one_to_one if f.auto_created]
...
def get_fiendly_name(model_or_field, lookup, as_tuple=False):
    ...
    while tokens:
        ...
        if pronoun:
            if l:
                l.append(pronoun)
        if hasattr(field_or_function, 'remote_field'):
            if field_or_function.remote_field:
                if field_or_function.remote_field.model:
...
def getattr_rec(obj, args):
    ...
    try:
        ...
        if field:
            ...
            if hasattr(field, 'display') and field.display not in (True, False, None):
