...
# [GCC 8.4.0]
# Embedded file name: /home/breno/Envs/djangoplus/lib/python3.6/site-packages/djangoplus/ui/components/navigation/dropdown.py
# Compiled at: 2018-09-24 08:48:06
# Size of source mod 2**32: 6320 bytes
import copy
        ...
        if inline:
            if self.model in loader.add_inline_actions:
                for add_inline_action in loader.add_inline_actions[self.model]:
                    if (add_inline_action['subset'] is True or add_inline_action['subset'] == subset_name) and permissions.check_group_or_permission(self.request, add_inline_action['can_execute']):
                        self.add_action(add_inline_action['title'], add_inline_action['url'], 'popup', 'fa fa-plus')
        ...
        for action_category in loader.actions[self.model]:
            for view_name in loader.actions[self.model][action_category]:
                ...
                if not permissions.check_group_or_permission(self.request, action_can_execute):
                    pass
                elif not check_condition(action_condition, obj):
                    pass
                else:
                    if is_action_view:
