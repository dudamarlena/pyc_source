...
# [GCC 8.4.0]
# Embedded file name: /Users/breno/Envs/djangoplus/lib/python3.7/site-packages/djangoplus/ui/components/forms/fields.py
# Compiled at: 2019-09-30 07:28:06
...
class CpfField(CharField):
    ...
    default_error_messages = {'invalid':'Número de CPF inválido.', 
     'max_digits':'O CPF deve conter 11 dígitos', 
     'digits_only':'O CPF deve conter apenas dígitos'}

    def __init__(self, *args, **kwargs):
        (super(CpfField, self).__init__)(args, min_length=11, **kwargs)
...
class CnpjField(CharField):
    ...
    default_error_messages = {'invalid':'Número de CNPJ inválido.', 
...
class CepField(forms.fields.RegexField):
    ...
    default_error_messages = {'invalid': 'O CEP deve estar no formato XX.XXX-XXX.'}

    def __init__(self, *args, **kwargs):
        (super(CepField, self).__init__)('^\\d{2}\\.\\d{3}-\\d{3}$', *args, **kwargs)
...
class CarPlateField(CharField):
    ...
    def __init__(self, *args, **kwargs):
        (super(CarPlateField, self).__init__)(args, min_length=8, **kwargs)
...
class MercosulCarPlateField(CharField):
    ...
    def __init__(self, *args, **kwargs):
        (super(MercosulCarPlateField, self).__init__)(args, min_length=8, **kwargs)


class PhoneField(CharField):
    PHONE_DIGITS_RE = re.compile('^(\\d{2})[-\\.]?(\\d{4,5})[-\\.]?(\\d{4})$')
    widget = widgets.PhoneWidget
    default_error_messages = {'invalid': 'O telefone deve estar no formato (XX) XXXX-XXXX.'}
...
class DateRangeField(forms.fields.MultiValueField):
 ...
 DateField(required=False), DateField(required=False)), sum_end_date=False, *args, **kwargs):
...
class CreditCardField(CharField):
    ...
    def __init__(self, *args, **kwargs):
        (super(CreditCardField, self).__init__)(args, min_length=19, **kwargs)
