...
# Embedded file name: build/bdist.linux-i686/egg/pyjsdoc.py
# Compiled at: 2008-10-23 18:29:18
...
try:
    ...
    try:
        ...
        def encode_json(val):
            raise ImportError('Either cjson or simplejson is required for JSON encoding')
...
def is_js_file(filename):
    ...
    """
    return filename.endswith('.js') and '.pack' not in filename and '.min' not in filename
...
def get_doc_comments(text):
    ...
    return [ make_pair(match) for match in re.finditer('/\\*\\*(.*?)\\*/', text, re.DOTALL)
           ]
...
def parse_comments_for_file(filename):
    ...
    interactive use.
    """
    return [ parse_comment(strip_stars(comment), next_line) for (comment, next_line) in get_doc_comments(read_file(filename))
           ]
...
class CodeBaseDoc(dict):
    ...
    def _module_index(self, attr):
        return dict(((obj.name, obj) for module in self.values() for obj in getattr(module, attr)))
    ...
    def save_docs(self, files=None, output_dir=None):
        ...
        if output_dir:
            try:
                ...
                try:
                    ...
                    save_file(os.path.join(output_dir, 'jsdoc.css'), pkg_resources.resource_string(__name__, 'static/jsdoc.css'))
                    ...
                    save_file('%s/index.html' % output_dir, build_html_page('Module index', self.to_html()))
        else:
            output_dir = '.'
        if files is None:
            files = self.keys()
        for filename in files:
            try:
                doc = self[filename]
                save_file('%s/%s.html' % (output_dir, trim_js_ext(doc.name)), build_html_page(doc.name, doc.to_html(self)))
            except KeyError:
...
class FileDoc(object):
    ...
    def __init__(self, file_name, file_text):
        ...
        for method in self.methods:
            try:
                ...
                warn('member %s of %s is not a class', method.member, method.name)
            except KeyError:
                pass
    ...
    def _filtered_iter(self, pred):
        return (self.comments[name] for name in self.order if pred(self.comments[name]))
    ...
    def to_html(self, codebase):
        ...
        vars = [
         ...
         (
          'functions',
          ('\n').join((fn.to_html(codebase) for fn in visible(self.functions)))),
         (
...
class ModuleDoc(CommentDoc):
    ...
    def to_html(self, codebase):
        ...
        def build_dependency(val):
            return (', ').join(('<a href = "%s.html">%s</a>' % (trim_js_ext(name), name) for name in val))
        ...
        html += build_line('all_dependencies', lambda val: len(val) > 1, build_dependency)
...
class FunctionDoc(CommentDoc):
    ...
    def params(self):
        ...
        if self.get('guessed_params') is None:
            ...
            return [ param_dict.get(name) or ParamDoc('{} ' + name) for name in self.get('guessed_params')
                   ]
        return
    ...
    def exceptions(self):
        ...
        return [ make_param(text) for text in self.get_as_list('throws') + self.get_as_list('exception')
               ]
    ...
    def to_dict(self):
        ...
        vars.update({'name': self.name, 
           'params': [ param.to_dict() for param in self.params ], 'options': [ option.to_dict() for option in self.options ], 'exceptions': [ exc.to_dict() for exc in self.exceptions ], 'return_val': self.return_val.to_dict(), 
...
class ClassDoc(CommentDoc):
    ...
    def to_html(self, codebase):
        ...
        return ('<a name = "%s" />\n<div class = "jsclass">\n' + '<h3>%s</h3>\n%s\n<h4>Methods</h4>\n%s</div>') % (
         self.name, self.name,
         htmlize_paragraphs(codebase.translate_links(self.doc, self)) + codebase.build_see_html(self.see, 'h4', self),
...
class CyclicDependency(Exception):
    ...
    def __str__(self):
        return 'The following dependencies result in a cycle: ' + (', ').join(self.values)
...
def topological_sort(dependencies, start_nodes):
    ...
    leftover_nodes = [ node for node in dependencies.keys() if in_degree(node) > 0
                     ]
    if leftover_nodes:
