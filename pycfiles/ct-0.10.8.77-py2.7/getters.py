...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cantools/db/sql/getters.py
# Compiled at: 2020-04-02 03:56:13
import json
...
def get_page(modelName, limit, offset, order='index', filters={}, session=session, count=False, exporter='export'):
    ...
    if '.' in order:
        ...
        if joinz or not get_model(attr):
            desc = False
            if mod.startswith('-'):
                desc = True
                mod = mod[1:]
...
def get_multi(b64keys, session=session):
    ...
    for k in keys:
        mod = k['model']
        if mod not in ents:
            ents[mod] = {'model': modelsubs[mod], 'indices': []}
