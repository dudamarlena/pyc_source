...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/hydro/connector_factory.py
...
class ConnectionPool(Base):
    ...
    def get_connector_class(self, conn_conf):
        ...
        connector_classes = filter(lambda x: issubclass(x, ConnectorBase) and x.__name__ not in ('ConnectorBase', 'DBBaseConnector'), classes)
...
class ConnectorHandler(Base):
    ...
    def get_connection_id(cls, conn_conf):
        ...
        """
        attr = ('source_type', 'connection_string', 'db_name', 'db_user', 'db_password') if not conn_conf.get('dsn') else 'dsn'
