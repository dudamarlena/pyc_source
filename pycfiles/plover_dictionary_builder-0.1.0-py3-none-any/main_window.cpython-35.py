...
# Embedded file name: build/bdist.linux-x86_64/egg/plover_cat/main_window.py
# Compiled at: 2017-10-18 07:28:28
...
class PloverCATWindow(QMainWindow, Ui_PloverCAT):
    ...
    def update(self, project, change_position=False):
        ...
        if change_position is not False:
    ...
    def move_cursor(self):
        stroke_number = self.project.stroke_number_from_position(self.textEdit.textCursor().position())
    ...
    def open_file(self, file_type):
        ...
        if file_type == 'json':
            name = 'Transcript'
            extension = 'json'
        selected_file = QFileDialog.getOpenFileName(self, _('Open ' + name), os.getcwd(), _(name + '(*.' + extension + ')'))
        file_name = selected_file[0]
        if file_type == 'raw':
    ...
    def export(self):
        selected_file = QFileDialog.getSaveFileName(self, _('Export Transcript'), os.path.splitext(self.project.file_name)[0] + '.txt', _('Transcript (*.txt)'))
        file_name = selected_file[0]
        if file_name != '':
