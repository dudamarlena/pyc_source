...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/heatshrink/streams.py
# Compiled at: 2016-11-14 05:16:14
import errno, io, os
...
class _DecompressReader(io.RawIOBase):
    ...
    def seek(self, offset, whence=io.SEEK_SET):
        if whence == io.SEEK_SET:
            ...
            if offset < 0:
                msg = '[Error {code}] {msg}'
                raise IOError(msg.format(code=errno.EINVAL, msg=os.strerror(errno.EINVAL)))
            if offset < self._pos:
                self._rewind()
            else:
                offset -= self._pos
