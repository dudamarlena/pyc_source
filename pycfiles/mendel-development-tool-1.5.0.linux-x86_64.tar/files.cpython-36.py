...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/mdt/files.py
...
class InstallCommand(command.NetworkCommand):
    ...
    def runWithClient(self, client, args):
        ...
        client.close()
        print()
        channel = client.shellExec(('sudo /usr/sbin/mdt-install-package {0}; rm -f {0}'.format(remote_filename)),
          allocPty=True)
...
class PushCommand(command.NetworkCommand):
    ...
    def pushDir(self, sftp, dir, destination):
        ...
        for path, subdirs, files in os.walk(dir):
            ...
            for name in files:
                self.pushFile(sftp, os.path.join(path, name), remote_path)
