...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/mdt/console.py
# Compiled at: 2019-09-10 14:32:03
# Size of source mod 2**32: 8891 bytes
"""
...
"""
import os, platform, queue, select, socket, sys, threading
...
class PosixConsole:
    ...
    def _socketSendQueueLevel(self):
        ...
        if self.uname == 'Linux':
            return struct.unpack('I', fcntl.ioctl(sock.fileno(), SIOCOUTQ, '\x00\x00\x00\x00'))[0]
        else:
        ...
        try:
            ...
            while 1:
                read, write, exception = select.select([self.channel,
                 self.inputfile], [], [], KEEP_ALIVE_SECONDS + 1)
