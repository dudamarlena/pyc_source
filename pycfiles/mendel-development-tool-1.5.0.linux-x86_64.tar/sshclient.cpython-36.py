...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/mdt/sshclient.py
# Compiled at: 2019-09-25 17:19:38
...
"""
import http.client, os, socket, time, fnmatch, paramiko
...
class SshClient:
    ...
    def _pushKeyViaDefaultLogin(self):
        try:
            try:
                ...
                self.client.exec_command('echo {0} >>$HOME/.ssh/authorized_keys'.format(key_line))
        finally:
            self.client.close()
    ...
    def maybeGenerateSshKeys(self):
        if not self.keystore.key():
            ...
            self.keystore.generateKey() or print('Unable to generate private key.')
            return False
        else:
            return True
