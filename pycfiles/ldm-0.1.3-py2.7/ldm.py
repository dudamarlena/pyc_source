...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ldm.py
# Compiled at: 2018-09-05 04:57:29
import os, dlib
from skimage import io
import numpy as np, cv2, time as tm
...
class LDM:
    ...
    def face_rec(self, img1, img2, threshold=0.8):
        ...
        result_dict = {'scorel': scorel, 'face1l': face1l, 'face2l': face2l, 
           'index1l': index1l, 
           'incex2l': index2l}
        return result_dict
    ...
    def face_rec_ms(self, img1, img2, threshold=0.8):
        ...
        result_dict = {'scorel': scorel, 'face1l': [], 'face2l': [], 'index1l': [], 'incex2l': []}
        return result_dict
    ...
    def has_same_person(self, img1, img2, threshold=0.8, savename='tmp.jpg'):
        ...
        cv2.putText(img_a, 'similarity:%.2f' % max_score, (10, 30), cv2.FONT_HERSHEY_COMPLEX, 1, (0,
                                                                                                  255,
                                                                                                  0), 2)
        print savename
    ...
    def compare2dir(self, imagedir1, imagedir2, max_compare_num=30, score_threshold=0.8):
        ...
        score_ave = score_ave / (float(compare_num) + 1e-07)
        t2 = tm.time()
        return (float(sameper_num) / (float(compare_num) + 1e-07), score_ave, compare_num, imagedir1.split('/')[(-1)], imagedir2.split('/')[(-1)], t2 - t1, 'http://clt.management.vipkid.com.cn/operation/classroom/classroom/')
