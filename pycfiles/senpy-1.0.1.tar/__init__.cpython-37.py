...
# [GCC 8.4.0]
# Embedded file name: /home/j/git/lab.gsi/senpy/senpy/senpy/plugins/__init__.py
# Compiled at: 2019-07-07 13:28:45
# Size of source mod 2**32: 29815 bytes
from future import standard_library
standard_library.install_aliases()
from future.utils import with_metaclass
from functools import partial
import os.path, os, re, pickle, logging, pprint, inspect, sys, subprocess, importlib, yaml, threading, multiprocessing, pkg_resources
    ...
    def __new__(mcs, name, bases, attrs, **kwargs):
        ...
        if 'description' not in attrs:
            ...
            if doc:
                ...
                logger.warning('Plugin {} does not have a description. Please, add a short summary to help other developers'.format(name))
        else:
            cls = super(PluginMeta, mcs).__new__(mcs, name, bases, attrs)
            if alias in mcs._classes:
                if os.environ.get('SENPY_TESTING', ''):
                    raise Exception('The type of plugin {} already exists. Please, choose a different name'.format(name))
                else:
...
class Plugin(with_metaclass(PluginMeta, models.Plugin)):
    ...
    _terse_keys = [
     'name', '@id', '@type', 'author', 'description',
    ...
    def process(self, request, activity, **kwargs):
        ...
        """
        newentries = list(self.process_entries(request.entries, activity))
    ...
    def process_entry(self, entry, activity):
        ...
        """
        raise NotImplementedError('You need to implement process, process_entries or process_entry in your plugin')

    def test(self, test_cases=None):
        if not test_cases:
            if not hasattr(self, 'test_cases'):
                raise AttributeError('Plugin {} [{}] does not have any defined test cases'.format(self.id, inspect.getfile(self.__class__)))
    ...
    def test_case(self, case, mock=testing.MOCK_REQUESTS):
        ...
        try:
            ...
            parameters = api.parse_params(given_parameters, self.extra_params)
            request.entries = [
             entry]
...
class Conversion(Plugin):
    ...
    def process(self, response, parameters, plugins=None, **kwargs):
        ...
        for entry in response.entries:
            newentries.append(self.convert_entry(entry, parameters, plugins))

        response.entries = newentries
        return response

    def convert_entry(self, entry, parameters, conversions_applied):
        raise NotImplementedError('You should implement a way to convert each entry, or a custom process method')
...
class SentimentPlugin(Analyser, Evaluable, models.SentimentPlugin):
    ...
    def normalize(self, value, minValue, maxValue):
        nv = minValue + (value - self.minPolarityValue) * (self.maxPolarityValue - self.minPolarityValue) / (maxValue - minValue)
        return nv
    ...
    def evaluate_func(self, X, activity=None):
        if activity is None:
            parameters = api.parse_params({}, self.extra_params)
            activity = self.activity(parameters)
        entries = []
        for feat in X:
...
class EmotionConversion(Conversion):
    ...
    def can_convert(self, fromModel, toModel):
        ...
        for pair in self.onyx__doesConversion:
            if not pair['onyx:conversionTo'] == toModel or fromModel is None or pair['onyx:conversionFrom'] == fromModel:
                return True
...
class TextBox(Box):
    __doc__ = 'A black box plugin that takes only text as input'
...
class SentimentBox(TextBox, SentimentPlugin):
    ...
    def to_entry(self, features, entry, activity, **kwargs):
        if len(features) != len(self.classes):
            raise models.Error('The number of features ({}) does not match the classes (plugin.classes ({})'.format(len(features), len(self.classes)))
...
class EmotionBox(TextBox, EmotionPlugin):
    ...
    def to_entry(self, features, entry, activity, **kwargs):
        ...
        if len(features) != len(self.EMOTIONS):
            raise Exception('The number of classes in the plugin and the number of features do not match')
...
class MappingMixin(object):
    ...
    def to_entry(self, features, entry, activity):
        ...
        for i, feat in enumerate(features):
            features[i] = self.mappings.get(feat, self.mappings.get('default', feat))

        return super(MappingMixin, self).to_entry(features=features, entry=entry,
          activity=activity)
...
class ShelfMixin(object):
    ...
    @property
    def sh--- This code section failed: ---

 L. 600         0  LOAD_GLOBAL              hasattr
                2  LOAD_FAST                'self'
                4  LOAD_STR                 '_sh'
               ...
               14  LOAD_CONST               None
               16  COMPARE_OP               is
               18  POP_JUMP_IF_FALSE   146  'to 146'
             20_0  COME_FROM             8  '8'

 L. 601        20  BUILD_MAP_0           0 
               22  LOAD_FAST                'self'
               24  STORE_ATTR               _sh

 L. 602        26  LOAD_GLOBAL              os
               28  LOAD_ATTR                path
               30  LOAD_METHOD              isfile
               32  LOAD_FAST                'self'
               34  LOAD_ATTR                shelf_file
               36  CALL_METHOD_1         1  '1 positional argument'
               38  POP_JUMP_IF_FALSE   146  'to 146'

 L. 603        40  SETUP_EXCEPT         84  'to 84'

 L. 604        42  LOAD_FAST                'self'
               44  LOAD_METHOD              open
               46  LOAD_FAST                'self'
               48  LOAD_ATTR                shelf_file
               50  LOAD_STR                 'rb'
               52  CALL_METHOD_2         2  '2 positional arguments'
               54  SETUP_WITH           74  'to 74'
               56  STORE_FAST               'p'

 L. 605        58  LOAD_GLOBAL              pickle
               60  LOAD_METHOD              load
               62  LOAD_FAST                'p'
               ...
               70  POP_BLOCK        
               72  LOAD_CONST               None
             74_0  COME_FROM_WITH       54  '54'
               74  WITH_CLEANUP_START
               76  WITH_CLEANUP_FINISH
               78  END_FINALLY      
               80  POP_BLOCK        
               82  JUMP_FORWARD        146  'to 146'
             84_0  COME_FROM_EXCEPT     40  '40'

 L. 606        84  DUP_TOP          
               86  LOAD_GLOBAL              IndexError
               88  LOAD_GLOBAL              EOFError
               90  LOAD_GLOBAL              pickle
               92  LOAD_ATTR                UnpicklingError
               94  BUILD_TUPLE_3         3 
 ...
 L. 607       106  LOAD_FAST                'self'
              108  LOAD_ATTR                log
              110  LOAD_METHOD              warning
              112  LOAD_STR                 'Corrupted shelf file: {}'
              114  LOAD_METHOD              format

 L. 608       116  LOAD_FAST                'self'
              118  LOAD_ATTR                shelf_file
              120  CALL_METHOD_1         1  '1 positional argument'
              122  CALL_METHOD_1         1  '1 positional argument'
              124  POP_TOP          

 L. 609       126  LOAD_FAST                'self'
              128  LOAD_METHOD              get
              130  LOAD_STR                 'force_shelf'
              132  LOAD_CONST               False
              134  CALL_METHOD_2         2  '2 positional arguments'
              136  POP_JUMP_IF_TRUE    140  'to 140'

 L. 610       138  RAISE_VARARGS_0       0  'reraise'
            140_0  COME_FROM           136  '136'
              140  POP_EXCEPT       
              142  JUMP_FORWARD        146  'to 146'
            144_0  COME_FROM            98  '98'
              144  END_FINALLY      
            146_0  COME_FROM           142  '142'
            146_1  COME_FROM            82  '82'
            146_2  COME_FROM            38  '38'
            146_3  COME_FROM            18  '18'

 L. 611       146  LOAD_FAST                'self'
              148  LOAD_ATTR                _sh
              150  RETURN_VALUE     
               -1  RETURN_LAST      

Parse error at or near `RETURN_VALUE' instruction at offset 150
    ...
    @property
    def shelf_file(self):
        if not (hasattr(self, '_shelf_file') and self._shelf_file):
...
is_plugin_file = re.compile('.*\\.senpy$|senpy_[a-zA-Z0-9_]+\\.py$|^(?!test_)[a-zA-Z0-9_]+_plugin.py$')
...
def _instances_in_module(module):
    for _, obj in inspect.getmembers(module):
        if isinstance(obj, Plugin) and inspect.getmodule(obj) == module:
            logger.debug('Found plugin instance: {}@{}'.format(obj, inspect.getmodule(obj)))
            yield obj


def _from_module_name(module, root, info=None, **kwargs):
    module = load_module(module, root)
    for plugin in _from_loaded_module(module=module, 
     root=root, info=info, **kwargs):
        yield plugin
...
class ScikitWrapper(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X=None, y=None):
        if self.plugin is not None:
            if not self.plugin.is_activated:
