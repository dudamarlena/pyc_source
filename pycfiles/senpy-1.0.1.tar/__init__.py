...
# Compiled at: 2019-03-27 14:07:39
...
class PluginMeta(models.BaseMeta):
    ...
    def __new__(mcs, name, bases, attrs, **kwargs):
        ...
        print ('Adding new plugin class', name, bases, attrs, plugin_type)
        attrs['name'] = alias
...
class Plugin(with_metaclass(PluginMeta, models.Plugin)):
    ...
    """
    _terse_keys = [
     'name', '@id', '@type', 'author', 'description', 'extra_params', 'is_activated', 'url', 'version']
    ...
    def process(self, request, analysis, **kwargs):
        """
        An implemented plugin should override this method.
        Here, we assume that a process_entries method exists.
        """
        newentries = list(self.process_entries(request.entries, analysis))
        request.entries = newentries
        return request

    def process_entries(self, entries, activity):
        for entry in entries:
            self.log.debug(('Processing entry with plugin {}: {}').format(self, entry))
    ...
    def test_case(self, case, mock=testing.MOCK_REQUESTS):
        ...
        try:
            ...
            request.entries = [
             entry]
            activity = self.analysis(parameters)
...
class SentimentPlugin(Analyser, models.SentimentPlugin):
    """
...
class EmotionConversion(Conversion):
    ...
    def can_convert(self, fromModel, toModel):
        ...
        for pair in self.onyx__doesConversion:
            if pair['onyx:conversionTo'] == toModel and (fromModel is None or pair['onyx:conversionFrom'] == fromModel):
                return True
...
class MappingMixin(object):
    ...
    def output(self, output, entry, params):
        output = self.mappings.get(output, self.mappings.get('default', output))
        return super(MappingMixin, self).output(output=output, entry=entry, params=params)
...
def install_deps(*plugins):
    ...
    if requirements:
        ...
        process = subprocess.Popen(pip_args, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def evaluate(plugins, datasets, **kwargs):
    ev = gsitk_compat.Eval(tuples=None, datasets=datasets, pipelines=[ plugin.as_pipe() for plugin in plugins ])
    ev.evaluate()
    results = ev.results
