...
# [GCC 8.4.0]
# Embedded file name: /home/david/source/lyrics/lyrics/apps/console/keys.py
...
import app
curses_mapping = {'KEY_NPAGE': '<PageDown>', 
   'KEY_PPAGE': '<PageUp>', 
   'KEY_DOWN': '<Down>', 
   'KEY_UP': '<Up>', 
   'KEY_BACKSPACE': '<BS>', 
...
def _search_write(char):
    ...
    state.search = state.search[:state.search_cursor] + char + state.search[state.search_cursor:]
...
def search_backspace():
    if state.search:
        ...
        state.search = state.search[:state.search_cursor - 1] + state.search[state.search_cursor:]
        state.search_cursor -= 1
        _search_update()
    else:
        search_cancel()
...
def search_down():
    state.search_list[state.search_index] = state.search
    state.search_index = min(state.search_index + 1, len(state.search_list) - 1)
