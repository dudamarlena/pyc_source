...
# [GCC 8.4.0]
# Embedded file name: /home/david/source/lyrics/lyrics/apps/console/playlist.py
# Compiled at: 2013-02-07 06:16:20
import os
...
class Song(object):
    ...
    def search(self, string):
        search = string.lower()
        return search in self.album.lower() or search in self.song.lower() or search in self.artist.lower() or search in self.path.lower()
...
class Playlist(object):
    ...
    def sort(self):
        sort_func = lambda song: (
         not song.artist, song.artist, song.song)
        self.songs = sorted(self.songs, key=sort_func)

    def search(self, string):
        return Playlist([ s for s in self.songs if s.search(string) ], selected=self.selected, parent=self)
    ...
    def from_path(cls, paths, recursive=True):
        ...
        def is_sound(path):
            formats = ('.mp3', '.wav', '.ogg', '.wmv', 'm3u')
            return path[-4:].lower() in formats or path[-5:].lower() == '.flac'
