...
# [GCC 8.4.0]
# Embedded file name: /home/david/source/lyrics/lyrics/apps/console/app.py
# Compiled at: 2013-02-11 17:26:06
""" the curses implementation """
import curses
from threading import Lock
import textwrap, color, keys
...
class NavigableWindow(Window):
    ...
    def move_cursor(self, x, y):
        ...
        debug.debug('cursor', state.current_window, self.cursor_at, y, self.view_at)
        ...
        if self.cursor_at < self.view_at + self.scroll_off:
            ...
            self.view_at = min(num_lines - self.real_height, self.cursor_at - self.real_height + 1 + self.scroll_off)
    ...
    def visible_in_window(self):
        ...
        return (self.view_at,
         min(self.view_at + self.real_height, self.get_num_lines()))
...
class StatusLine(Window):
    ...
    def draw(self):
        self.win_curses.bkgd(' ', color.BG_GREEN)
        r = 'repeat' if state.repeat else 'solo' if state.repeat_solo else 'no-repeat'
