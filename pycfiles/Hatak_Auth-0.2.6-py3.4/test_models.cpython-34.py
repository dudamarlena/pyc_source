...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/haplugin/auth/tests/test_models.py
# Compiled at: 2014-09-30 16:26:20
# Size of source mod 2**32: 4991 bytes
...
class UserTestCase(ModelTestCase):
    ...
    def test_add_permission(self):
        ...
        self.mocks['Permission'].get_or_create.assert_called_once_with(self.db, group='first', name='second')
        self.assertEqual([
         self.mocks['Permission'].get_or_create.return_value], self.model.permissions)
    ...
    def test_has_access_to_route(self):
        ...
        self.registry['route'].routes = {'myroute': 'ctrl'}
        self.model.registry = self.registry
        result = self.model.has_access_to_route('myroute')
        self.assertEqual(self.mocks['has_access_to_controller'].return_value, result)
...
class NotLoggedUserTestCase(ModelTestCase):
    ...
    def test_add_permission(self):
        ...
        self.assertRaises(NotImplementedError, self.model.add_permission, ('base',
                                                                           'view'))

    def test_set_password(self):
        """FakeUser can not change his password."""
        self.assertRaises(NotImplementedError, self.model.set_password, ('password', ))

    def test_validate_password(self):
        """FakeUser can not check his password."""
        self.assertRaises(NotImplementedError, self.model.validate_password, ('password', ))
