...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.4/dist-packages/dustmaker/Tile.py
# Compiled at: 2015-10-28 20:12:15
# Size of source mod 2**32: 9907 bytes
from enum import IntEnum
import math

class TileSpriteSet(IntEnum):
    __doc__ = " Used to describe what set of tiles a tile's sprite comes from. "
    none_0 = 0
    mansion = 1
    forest = 2
    city = 3
...
class Tile:
    __doc__ = ' Represents a single tile in a Dustforce level.  Positional information\n      is stored within Map.\n  '
    ...
    """
        result = self.tile_data[(2 + side * 2)] + (self.tile_data[(2 + side * 2 + 1)] << 8)
        if val is not None:
            self.tile_data[2 + side * 2] = val & 255
    ...
    """
        result = TileSpriteSet(self.tile_data[10] & 15)
        if val is not None:
            self.tile_data[10] = self.tile_data[10] & 240 | val
        return result
    ...
    """
        result = (self.tile_data[10] & 240) >> 4
        if val is not None:
            self.tile_data[10] = self.tile_data[10] & 15 | val << 4
        return result
    ...
    def edge_filth_sprite(self, side, sprite_set=None, spikes=None):
        ind = side // 2
        shft = 4 * (side % 2)
        result = self.dust_data[ind] >> shft & 15
        if sprite_set is not None:
            if spikes is not None:
                val = sprite_set | (8 if spikes else 0)
                self.dust_data[ind] &= 240 >> shft
                self.dust_data[ind] |= val << shft
        return (
         TileSpriteSet(result & 7), result & 8 != 0)

    def edge_filth_cap(self, side, val=None):
        shft = 2 * side
        result = self.dust_data[10] >> shft & 3
        if val is not None:
            self.dust_data[10] &= ~(3 << shft)
    ...
    def transform(self, mat):
        ...
        if flipped:
            if self.shape == TileShape.FULL:
                ...
                if self.shape <= TileShape.SMALL_8:
        ...
        if self.shape == TileShape.FULL:
            ...
            if self.shape <= TileShape.SMALL_4:
