...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.4/dist-packages/dustmaker/Tilter.py
# Compiled at: 2015-10-28 16:37:46
...
def _draw_shape(draw, shape, x, y, mat):
    ...
    if shape == TileShape.FULL:
        ...
        if shape == TileShape.BIG_1:
            coords.append((x, y))
            coords.append((x + 1, y + 0.5))
            coords.append((x + 1, y + 1))
            coords.append((x, y + 1))
        else:
            if shape == TileShape.SMALL_1:
                coords.append((x, y + 0.5))
                coords.append((x + 1, y + 1))
                coords.append((x, y + 1))
            else:
                if shape == TileShape.BIG_5:
                    coords.append((x + 1, y))
                    coords.append((x, y + 0.5))
                    coords.append((x, y + 1))
                    coords.append((x + 1, y + 1))
                else:
                    if shape == TileShape.SMALL_5:
                        coords.append((x + 1, y + 0.5))
                        coords.append((x, y + 1))
                        coords.append((x + 1, y + 1))
    ...
    nmat = [
     ...
     [
      0, 0, 1]]
    nmat[0][2] = mat[0][0] * x + mat[0][1] * y + mat[0][2] - (nmat[0][0] * (x + 0) + nmat[0][1] * (y + 1))
    nmat[1][2] = mat[1][0] * x + mat[1][1] * y + mat[1][2] - (nmat[1][0] * (x + 0) + nmat[1][1] * (y + 1))
