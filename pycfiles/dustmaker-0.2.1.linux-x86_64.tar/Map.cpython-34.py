...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.4/dist-packages/dustmaker/Map.py
# Compiled at: 2015-10-28 19:53:53
...
class Map:
    ...
    """
        ix = int(x)
        iy = int(y)
        self.tiles = {(c[0], c[1] + ix, c[2] + iy):tile for c, tile in self.tiles.items()}
        self.props = {id:(p[0], p[1] + x, p[2] + y, p[3]) for id, p in self.props.items()}
        if hasattr(self, 'backdrop'):
            pos = self.start_position()
            self.start_position((pos[0] + x, pos[1] + y))
            self.entities = {id:(p[0] + x, p[1] + y, p[2]) for id, p in self.entities.items()}
    ...
    """
        ...
        self.props = {prop_remap[id]:self.props[id] for id in self.props.keys()}
        if hasattr(self, 'backdrop'):
            entity_remap = {}
            for id in self.entities.keys():
                entity_remap[id] = self._next_id()

            self.entities = {entity_remap[id]:self.entities[id] for id in self.entities.keys()}
    ...
    """
        self.tiles = {(layer, mat[0][2] + x * mat[0][0] + y * mat[0][1] + min(0, mat[0][0]) + min(0, mat[0][1]), mat[1][2] + x * mat[1][0] + y * mat[1][1] + min(0, mat[1][0]) + min(0, mat[1][1])):tile for (layer, x, y), tile in self.tiles.items()}
        self.props = {id:(layer, mat[0][2] + x * mat[0][0] + y * mat[0][1], mat[1][2] + x * mat[1][0] + y * mat[1][1], prop) for id, (layer, x, y, prop) in self.props.items()}
        ...
        if hasattr(self, 'entities'):
            self.entities = {id:(mat[0][2] + x * mat[0][0] + y * mat[0][1], mat[1][2] + x * mat[1][0] + y * mat[1][1], entity) for id, (x, y, entity) in self.entities.items()}
            for x, y, entity in self.entities.values():
