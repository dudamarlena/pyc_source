...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.4/dist-packages/dustmaker/MapWriter.py
# Compiled at: 2015-10-16 16:36:49
# Size of source mod 2**32: 9490 bytes
from .Map import Map
...
def _write_segment(base_writer, seg_x, seg_y, segment):
    ...
    if tiles:
        ...
        for layer, tilelayer in tiles:
            ...
            for x, y, tile in sorted(tilelayer, key=lambda x: (
             x[1], x[0])):
                if layer == 19:
                    if tile.has_filth():
                ...
                if layer == 19:
                    ...
                    for side in TileSide:
                        ...
                        if edge[0] != TileSpriteSet.none_0:
                            if not edge[1]:
                                dust_filth += 1
                        if tile.edge_bits(side) & 3 == 3 and not tile.edge_filth_sprite(side)[1]:
                            tile_surface += 1
                            continue

                writer.write(5, x)
                writer.write(5, y)
                writer.write(8, tile.shape | 128)
...
def _segment_get(segment_map, x, y):
    sx = int(floor(x / 16)) & 15
    sy = int(floor(y / 16)) & 15
    if (sx, sy) in segment_map:
        return segment_map[(sx, sy)]
    seg = dict(tiles=[[] for x in range(21)], entities=[], props=[])
...
def _region_get(region_map, x, y):
    ...
    reg = dict(segments={}, backdrop=dict(present=False, tiles=[[] for x in range(21)], entities=[], props=[]))
...
def _compute_region_map(map):
    ...
    for coord, tile in map.tiles.items():
        ...
        seg['tiles'][layer].append((x & 15, y & 15, tile))
...
def write_map(map):
  ...
  """
    ...
    writer.write(16, 44)
    writer.write(32, 12 + len(writer_header.bytes()) + len(writer_front.bytes()) + len(writer_back.bytes()))
