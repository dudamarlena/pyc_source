...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/notekeras/optimizers/warmup_v2.py
# Compiled at: 2019-12-03 07:30:22
# Size of source mod 2**32: 7922 bytes
import tensorflow as tf
from tensorflow.python import ops, math_ops, state_ops, control_flow_ops
from tensorflow.python.keras.optimizer_v2.optimizer_v2 import OptimizerV2
import notekeras.backend as backend
__all__ = [
 'AdamWarmup']

class AdamWarmup(OptimizerV2):
    __doc__ = 'Adam optimizer with warmup.'

    def __init__(self, decay_steps, warmup_steps, min_lr=0.0, learning_rate=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-07, weight_decay=0.0, weight_decay_pattern=None, amsgrad=False, name='Adam', **kwargs):
    ...
    def _resource_apply_dense(self, grad, var):
        ...
        lr_t = tf.where(local_step <= warmup_steps, lr_t * (local_step / warmup_steps), min_lr + (lr_t - min_lr) * (1.0 - tf.minimum(local_step, decay_steps) / decay_steps))
        lr_t = lr_t * math_ops.sqrt(1 - beta_2_power) / (1 - beta_1_power)
        m_t = state_ops.assign(m, (beta_1_t * m + (1.0 - beta_1_t) * grad),
          use_locking=(self._use_locking))
        v_t = state_ops.assign(v, (beta_2_t * v + (1.0 - beta_2_t) * math_ops.square(grad)),
          use_locking=(self._use_locking))
        if self.amsgrad:
    ...
    def get_config(self):
        ...
        config.update({'decay_steps':self._serialize_hyperparameter('decay_steps'), 
