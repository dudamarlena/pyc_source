...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/notekeras/layer/transformer5.py
# Compiled at: 2020-01-19 04:17:30
# Size of source mod 2**32: 33441 bytes
import numpy as np
...
__all__ = [
 'WrapCodeComponent', 'EncoderComponent', 'DecoderComponent', 'EncoderListComponent', 'DecoderListComponent']
...
class WrapCodeComponent(Model):
    ...
    def __init__(self, name, head_num, hidden_dim, attention_activation=None, feed_forward_activation='relu', history_only=False, dropout_rate=0.0, trainable=True, use_adapter=False, adapter_units=None, adapter_activation='relu', is_list=False, use_attention=True):
    ...
    def _build(self):
        if self.use_attention:
            self.attention_layer = MultiHeadAttention(name=('%s-Attention' % self.name), head_num=(self.head_num),
              activation=(self.attention_activation),
              history_only=(self.history_only),
              trainable=(self.trainable))
        else:
            self.attention_layer = FeedForward(name=('%s-FeedForward' % self.name), units=(self.hidden_dim),
        ...
        if self.use_adapter:
            self.adapter = FeedForward(units=(self.adapter_units), activation=(self.adapter_activation),
    ...
    def get_config(self):
        config = {'layers':[],  'dropout_rate':self.dropout_rate, 
         'trainable':self.trainable, 
         'use_adapter':self.use_adapter, 
         'adapter_units':self.adapter_units}
        for layer in self.layers:
            config['layers'].append({'class_name':layer.__class__.__name__, 
...
class EncoderComponent(Model):
    ...
    def __init__(self, name, head_num, hidden_dim, attention_activation=None, feed_forward_activation='relu', dropout_rate=0.0, trainable=True, use_adapter=False, adapter_units=None, adapter_activation='relu'):
    ...
    def _build(self):
        self.attention_layer2 = WrapCodeComponent(name=('%s-MultiHeadSelfAttention' % self.name), head_num=(self.head_num),
          ...
          use_attention=True)
        self.feed_forward_layer2 = WrapCodeComponent(name=('%s-FeedForward' % self.name), head_num=(self.head_num),
          hidden_dim=(self.hidden_dim),
          attention_activation=(self.attention_activation),
          feed_forward_activation=(self.feed_forward_activation),
          history_only=False,
...
class DecoderComponent(Model):
    ...
    def _build(self):
        self.self_attention_layer2 = WrapCodeComponent(name=('%s-SelfAttention' % self.name), head_num=(self.head_num),
          hidden_dim=(self.hidden_dim),
          attention_activation=(self.attention_activation),
          feed_forward_activation=(self.feed_forward_activation),
          history_only=True,
          ...
          adapter_activation=(self.adapter_activation),
          use_attention=True)
        self.query_attention_layer2 = WrapCodeComponent(name=('%s-QueryAttention' % self.name), head_num=(self.head_num),
          hidden_dim=(self.hidden_dim),
          attention_activation=(self.attention_activation),
          feed_forward_activation=(self.feed_forward_activation),
          history_only=False,
...
class EncoderListComponent(Model):
    ...
    def __init__(self, encoder_num, head_num, hidden_dim, name='Encoder', attention_activation=None, feed_forward_activation='relu', dropout_rate=0.0, trainable=True, use_adapter=False, adapter_units=None, adapter_activation='relu'):
        """编码层 Get encoders.
    ...
    def _build(self):
        for i in range(self.encoder_num):
            self.layers.append(EncoderComponent(name=('{}-{}'.format(self.name, i + 1)), head_num=(self.head_num),
              hidden_dim=(self.hidden_dim),
              attention_activation=(self.attention_activation),
              feed_forward_activation=(self.feed_forward_activation),
              dropout_rate=(self.dropout_rate),
...
class DecoderListComponent(Model):
    ...
    def __init__(self, decoder_num, head_num, hidden_dim, name='Decoder', attention_activation=None, feed_forward_activation='relu', dropout_rate=0.0, trainable=True, use_adapter=False, adapter_units=None, adapter_activation='relu'):
        """Get decoders.
    ...
    def _build(self):
        for i in range(self.decoder_num):
            self.layers.append(DecoderComponent(name=('{}-{}'.format(self.name, i + 1)), head_num=(self.head_num),
              hidden_dim=(self.hidden_dim),
              attention_activation=(self.attention_activation),
              feed_forward_activation=(self.feed_forward_activation),
              dropout_rate=(self.dropout_rate),
...
class TransformerModel(Model):
    ...
    def __init__(self, token_num, embed_dim, encoder_num, decoder_num, head_num, hidden_dim, name='transformer', attention_activation=None, feed_forward_activation='relu', dropout_rate=0.0, use_same_embed=True, embed_weights=None, embed_trainable=None, trainable=True, use_adapter=False, adapter_units=None, adapter_activation='relu', **kwargs):
    ...
    def _build(self):
        if not isinstance(self.token_num, list):
            ...
            if not isinstance(self.embed_weights, list):
                self.embed_weights = [
                 self.embed_weights, self.embed_weights]
            ...
            if not isinstance(self.embed_trainable, list):
                self.embed_trainable = [
                 self.embed_trainable, self.embed_trainable]
            ...
            if self.use_same_embed:
                encoder_embed_layer = decoder_embed_layer = EmbeddingRet(input_dim=encoder_token_num, output_dim=(self.embed_dim),
                  mask_zero=True,
                  weights=encoder_embed_weights,
                  trainable=encoder_embed_trainable,
                  name='Token-Embedding')
            else:
                encoder_embed_layer = EmbeddingRet(input_dim=encoder_token_num, output_dim=(self.embed_dim),
                  mask_zero=True,
                  weights=encoder_embed_weights,
                  trainable=encoder_embed_trainable,
                  name='Encoder-Token-Embedding')
            decoder_embed_layer = EmbeddingRet(input_dim=decoder_token_num, output_dim=(self.embed_dim),
              mask_zero=True,
        ...
        encoded_layer = EncoderListComponent(encoder_num=(self.decoder_num), head_num=(self.head_num),
          hidden_dim=(self.hidden_dim),
          attention_activation=(self.attention_activation),
          feed_forward_activation=(self.feed_forward_activation),
          dropout_rate=(self.dropout_rate),
          ...
          use_adapter=(self.use_adapter),
          adapter_units=(self.adapter_units),
          adapter_activation=(self.adapter_activation))(encoder_embed)
        ...
        decoded_layer = DecoderListComponent(decoder_num=(self.encoder_num), head_num=(self.head_num),
          hidden_dim=(self.hidden_dim),
          attention_activation=(self.attention_activation),
          feed_forward_activation=(self.feed_forward_activation),
          dropout_rate=(self.dropout_rate),
    ...
    def decode(self, tokens, start_token, end_token, pad_token, top_k=1, temperature=1.0, max_len=10000, max_repeat=10, max_repeat_block=10):
        ...
        while len(list(filter(lambda x: x is None, outputs))) > 0:
            ...
            for i in range(len(predicts)):
                ...
                if not ((last_token == end_token or max_len) is not None and output_len >= max_len):
                    if self._get_max_suffix_repeat_times(decoder_inputs, max_repeat * max_repeat_block) >= max_repeat:
                        pass
