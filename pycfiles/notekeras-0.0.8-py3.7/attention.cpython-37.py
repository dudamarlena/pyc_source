...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/notekeras/layer/attention.py
# Compiled at: 2020-03-10 04:48:49
# Size of source mod 2**32: 28356 bytes
import notekeras.backend as K
from notekeras.backend import keras
Layer = keras.layers.Layer

class ScaledDotProductAttention(Layer):
    __doc__ = '\n    注意力模型主要包含三个输入 即Q(queries), K(keys), V(values)。按照下面这个公式计算的\n\n    \text{Attention}(Q, K, V) = \text{softmax}(\x0crac{Q K^T}{\\sqrt{d_k}}) V\n\n    详见论文: https://arxiv.org/pdf/1706.03762.pdf\n    '
...
class SeqSelfAttention(Layer):
    ...
    def __init__(self, units=32, attention_width=None, attention_type=ATTENTION_TYPE_ADD, return_attention=False, history_only=False, kernel_initializer='glorot_normal', bias_initializer='zeros', kernel_regularizer=None, bias_regularizer=None, kernel_constraint=None, bias_constraint=None, use_additive_bias=True, use_attention_bias=True, attention_activation=None, attention_regularizer_weight=0.0, **kwargs):
        ...
        if history_only:
            if attention_width is None:
                self.attention_width = int(1000000000.0)
        else:
            ...
            if attention_type == SeqSelfAttention.ATTENTION_TYPE_ADD:
                ...
                if attention_type == SeqSelfAttention.ATTENTION_TYPE_MUL:
                    self.Wa, self.ba = (None, None)
                else:
    ...
    def build(self, input_shape):
        if self.attention_type == SeqSelfAttention.ATTENTION_TYPE_ADD:
            ...
            if self.attention_type == SeqSelfAttention.ATTENTION_TYPE_MUL:
                self._build_multiplicative_attention(input_shape)
        super(SeqSelfAttention, self).build(input_shape)

    def _build_additive_attention(self, input_shape):
        feature_dim = int(input_shape[2])
        self.Wt = self.add_weight(shape=(feature_dim, self.units), name=('{}_Add_Wt'.format(self.name)),
          initializer=(self.kernel_initializer),
          regularizer=(self.kernel_regularizer),
          constraint=(self.kernel_constraint))
        self.Wx = self.add_weight(shape=(feature_dim, self.units), name=('{}_Add_Wx'.format(self.name)),
          initializer=(self.kernel_initializer),
          regularizer=(self.kernel_regularizer),
          constraint=(self.kernel_constraint))
        if self.use_additive_bias:
            self.bh = self.add_weight(shape=(self.units,), name=('{}_Add_bh'.format(self.name)),
              initializer=(self.bias_initializer),
              regularizer=(self.bias_regularizer),
              constraint=(self.bias_constraint))
        self.Wa = self.add_weight(shape=(self.units, 1), name=('{}_Add_Wa'.format(self.name)),
          initializer=(self.kernel_initializer),
          regularizer=(self.kernel_regularizer),
          constraint=(self.kernel_constraint))
        if self.use_attention_bias:
            self.ba = self.add_weight(shape=(1, ), name=('{}_Add_ba'.format(self.name)),
              initializer=(self.bias_initializer),
              regularizer=(self.bias_regularizer),
              constraint=(self.bias_constraint))
    ...
    def _build_multiplicative_attention(self, input_shape):
        feature_dim = int(input_shape[2])
        self.Wa = self.add_weight(shape=(feature_dim, feature_dim), name=('{}_Mul_Wa'.format(self.name)),
          initializer=(self.kernel_initializer),
          regularizer=(self.kernel_regularizer),
          constraint=(self.kernel_constraint))
        if self.use_attention_bias:
            self.ba = self.add_weight(shape=(1, ), name=('{}_Mul_ba'.format(self.name)),
              initializer=(self.bias_initializer),
              regularizer=(self.bias_regularizer),
              constraint=(self.bias_constraint))
    ...
    def call(self, inputs, mask=None, **kwargs):
        ...
        if mask is not None:
            ...
            e = K.permute_dimensions(K.permute_dimensions(e * mask, (0, 2, 1)) * mask, (0,
                                                                                        2,
                                                                                        1))
    ...
    def _call_multiplicative_emission(self, inputs):
        e = K.batch_dot(K.dot(inputs, self.Wa), K.permute_dimensions(inputs, (0, 2,
                                                                              1)))
        if self.use_attention_bias:
            e += self.ba[0]
        return e
    ...
    def _attention_regularizer(self, attention):
        ...
        return self.attention_regularizer_weight * K.sum(K.square(K.batch_dot(attention, K.permute_dimensions(attention, (0,
                                                                                                                          2,
                                                                                                                          1))) - eye)) / batch_size
...
class SeqWeightedAttention(Layer):
    ...
    def build(self, input_shape):
        self.W = self.add_weight(shape=(int(input_shape[2]), 1), name=('{}_W'.format(self.name)),
          initializer=(keras.initializers.get('uniform')))
        if self.use_bias:
            self.b = self.add_weight(shape=(1, ), name=('{}_b'.format(self.name)),
    ...
    def compute_output_shape(self, input_shape):
        ...
        if self.return_attention:
            return [
             (
              input_shape[0], output_len), (input_shape[0], input_shape[1])]
        return (
...
class MultiHeadAttention(Layer):
    __doc__ = 'Multi-head attention layer.\n\n    See: https://arxiv.org/pdf/1706.03762.pdf\n    '

    def __init__(self, head_num, activation='relu', use_bias=True, kernel_initializer='glorot_normal', bias_initializer='zeros', kernel_regularizer=None, bias_regularizer=None, kernel_constraint=None, bias_constraint=None, history_only=False, **kwargs):
        """Initialize the layer.
    ...
    def call(self, inputs, mask=None):
        ...
        if self.activation is not None:
            ...
            k = self.activation(k)
            v = self.activation(v)
        y = ScaledDotProductAttention(history_only=(self.history_only), name=('%s-Attention' % self.name))(inputs=[
        ...
        if self.activation is not None:
            y = self.activation(y)
        input_shape = [
         K.int_shape(q), K.int_shape(k), K.int_shape(v)]
