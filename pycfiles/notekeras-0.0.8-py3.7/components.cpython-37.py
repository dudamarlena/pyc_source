...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/notekeras/component/components.py
# Compiled at: 2020-01-19 04:16:22
...
class WideDeepComponent(Layer):
    ...
    def __init__(self, name, deep_shape=None, deep_activation='relu'):
    ...
    def get_config(self):
        config = {'deep_shape':self.deep_shape, 
...
class DeepFM(Layer):
    ...
    def __init__(self, name='DeepFM', deep_shape=None, deep_activation='relu'):
    ...
    def _build(self):
        for shape in self.deep_shape:
            dense = keras.layers.Dense(shape, activation=(self.deep_activation), name=('{}-deep-dense{}'.format(self.name, len(self.deep_dense))))
            self.deep_dense.append(dense)
    ...
    def __call__(self, inputs, **kwargs):
        ...
        summed_features_emb = self.self_sum1(emb)
        summed_features_emb_square = self.multiple1([summed_features_emb, summed_features_emb])
        squared_features_emb = self.multiple2([emb, emb])
        squared_sum_features_emb = self.self_sum2(squared_features_emb)
        sub = self.subtract([summed_features_emb_square, squared_sum_features_emb])
        sub = keras.layers.Lambda(lambda x: x * 0.5)(sub)
        y_second_order = sub
        y_deep = MaskFlatten()(emb)
        for dense in self.deep_dense:
