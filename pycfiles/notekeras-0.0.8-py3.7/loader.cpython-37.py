...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/notekeras/model/loader.py
# Compiled at: 2020-01-13 07:36:14
# Size of source mod 2**32: 11486 bytes
...
class PreTrainedList(object):
    __test__ = PreTrainedInfo('https://github.com/CyberZHG/keras-bert/archive/master.zip', 'keras-bert-master', 'keras-bert')
    ...
    chinese_wwm_base = PreTrainedInfo('https://storage.googleapis.com/hfl-rc/chinese-bert/chinese_wwm_L-12_H-768_A-12.zip', 'publish', 'chinese_wwm_L-12_H-768_A-12')
...
def build_model_from_config(config_file, training=False, trainable=None, output_layer_num=1, seq_len=int(1000000000.0), **kwargs):
    ...
    if trainable is None:
        trainable = training
    model = (bert.get_model)(token_num=config['vocab_size'], pos_num=config['max_position_embeddings'], 
     seq_len=seq_len, 
     ...
     training=training, 
     trainable=trainable, 
     output_layer_num=output_layer_num, **kwargs)
    if not training:
...
def load_trained_model_from_checkpoint(config_file, checkpoint_file, training=False, trainable=None, output_layer_num=1, seq_len=int(1000000000.0), **kwargs):
    """Load trained official model from checkpoint.
    ...
    :return: model
    """
    model, config = build_model_from_config(config_file, training=training, trainable=trainable, output_layer_num=output_layer_num, 
