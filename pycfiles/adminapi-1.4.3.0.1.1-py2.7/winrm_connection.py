...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cloud_utils/net_utils/winrm_connection.py
# Compiled at: 2018-01-31 14:44:08
import copy, re, socket, sys, StringIO, time, traceback
...
class Winrm_Connection:
    ...
    def __init__(self, hostname, username, password, port=5985, protocol='http', transport='plaintext', default_command_timeout=600, url=None, debug_method=None, verbose=True):
    ...
    def reset_shell(self, timeout=None, retries=5, verbose=False):
        retry = 0
        tb = ''
        e = None
        self.close_shell()
        ...
        while retry < retries:
            ...
            try:
                ...
                self.debug('Failed to open shell on attempt#:' + str(retry) + '/' + str(retries) + ', err:' + str(wte))
                if retry < retries:
                    time.sleep(5)
    ...
    def cmd(self, command, console_mode_stdin=True, skip_cmd_shell=False, timeout=None, verbose=None):
        ...
        try:
            try:
                self.command_id = self.winproto.run_command(self.shell_id, command, arguments=arguments, console_mode_stdin=console_mode_stdin, skip_cmd_shell=skip_cmd_shell)
        ...
        if errmsg:
            if re.search('timed out', errmsg, re.IGNORECASE):
                raise CommandTimeoutException('ERROR: Timed out after:' + str(self.winproto.transport.timeout) + ', Cmd:"' + str(orig_cmd))
            else:
    ...
    def get_timed_command_output(self, shell_id, command_id, active_timeout=0):
        ...
        while not command_done:
            ...
            stdout, stderr, return_code, command_done = self.winproto._raw_get_command_output(shell_id, command_id)
            stdout_buffer.append(stdout)
            stderr_buffer.append(stderr)

        return (
         ('').join(stdout_buffer), ('').join(stderr_buffer), return_code)
    ...
    def sys(self, command, include_stderr=False, listformat=True, carriage_return=False, timeout=None, code=None, verbose=None):
        ...
        if code is not None and output['statuscode'] != code:
            raise CommandExitCodeException('Cmd:' + str(command) + ' failed with status code:' + str(output['statuscode']) + '\n, stdout:' + str(output['stdout']) + '\n, stderr:' + str(output['stderr']))
        ret = output['stdout']
        if ret:
    ...
    def get_traceback(cls):
        ...
        try:
            ...
            traceback.print_exception(file=out, *sys.exc_info())
            out.seek(0)
            buf = out.read()
        except Exception as e:
