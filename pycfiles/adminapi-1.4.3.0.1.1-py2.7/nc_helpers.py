...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cloud_admin/hosts/helpers/nc_helpers.py
# Compiled at: 2018-01-31 14:44:08
import os
from prettytable import PrettyTable
import re, stat, time, types
from xml.dom.minidom import parseString
from cloud_utils.net_utils.sshconnection import CommandExitCodeException, CommandTimeoutException, SshCbReturn
...
class NodeControllerHelpers(EucaMachineHelpers):
    ...
    def get_last_capacity_status(self):
        ...
        if str(self.eucahost.eucalyptus_conf.LOGLEVEL).lower() not in ('debug', 'trace'):
            self.log.debug(('Cant fetch capacity status from node with loglevel: DEBUG < "{0}"').format(self.eucahost.eucalyptus_conf.LOGLEVEL))
            return ret
        else:
            euca_path = self.eucahost.eucalyptus_home or ''
            nclog = os.path.join(euca_path, 'var/log/eucalyptus/nc.log')
            try:
                out = self.sys(('tac {0} | grep -m1 "returning status="').format(nclog), code=0, listformat=False)
                if out:
    ...
    def get_vm_availability(self):
        ...
        if not av:
            ...
            for key, value in av.iteritems():
                if value is None:
                    self.log.warn(('Cant get vm availability due to None value: {0}={1}').format(key, value))
                    return ret
    ...
    def show_availability_for_node(self, printmethod=None, print_table=True):
        ...
        main_pt = PrettyTable([
         markup('("{0}"\'s VM AVAILABILITY @ {1})').format(self.eucahost.hostname, cap.get('timestamp'))])
        main_pt.border = 0
        main_pt.align = 'l'
    ...
    def get_local_nc_service_state(self):
        ...
        if self.ssh:
            try:
                ...
                self.debug(('Could not get service state from node:"{0}", err:"{1}"').format(self.hostname), str(E))
        ...
        if self.eucahost:
            ...
            if len(output) > 1:
                ...
                for line in output[2:]:
                    ...
                    instance_list.append({keys[0]: domain_line[0], keys[1]: domain_line[1], 
                       keys[2]: domain_line[2]})

        return instance_list

    def tail_instance_console(self, instance, max_lines=None, timeout=30, idle_timeout=30, print_method=None):
        """
        ...
        try:
            self.eucahost.cmd('tail -F ' + str(console_path), verbose=False, cb=self.remote_tail_monitor_cb, cbargs=[
             instance,
             max_lines,
             lines_read,
             start_time,
    ...
    def remote_tail_monitor_cb(self, buf, instance_id, max_lines, lines_read, start_time, timeout, print_method, prefix, idle_timeout):
    ...
    def get_instance_multipath_dev_info_for_instance_block_dev(self, instance, ebs_block_dev, verbose=False):
        ...
        mpath_dev_info = self.eucahost.sys('multipath -ll ' + str(mpath_dev) + " | sed 's/[[:cntrl:]]//g' ", verbose=verbose, code=0)
    ...
    def get_instance_multipath_dev_for_instance_block_dev(self, instance, ebs_block_dev, verbose=False):
        ...
        sym_links = self.eucahost.sys('udevadm info --name ' + str(dm_dev) + ' --query symlink', verbose=verbose, code=0)[0]
    ...
    def get_instance_xml_text(self, instance_id):
        if not isinstance(instance_id, types.StringTypes):
            instance_id = instance_id.id
        return self.eucahost.sys('virsh dumpxml ' + str(instance_id), listformat=False, verbose=False, code=0)
    ...
    def enable_vnc_for_new_instances(self, listen='0.0.0.0', autoport='yes', keymap='en-us', port='-1', path='/etc/eucalyptus/libvirt.xsl'):
        ...
        for line in current_xsl.splitlines():
            if re.search("graphics\\s+type='vnc'", line):
                ...
                if m:
                    space = m.group()
                line = ("{0}<graphics type='vnc' port='{1}' autoport='{2}' keymap='{3}' listen='{4}'/>").format(space, port, autoport, keymap, listen)
            new_xsl += str(line) + '\n'
