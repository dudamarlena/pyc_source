...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cloud_admin/systemconnection.py
# Compiled at: 2018-01-31 14:44:08
import array, copy, logging
from prettytable import PrettyTable
import re, socket, struct, fcntl, threading
...
class SystemConnection(ServiceConnection):
    ...
    def __init__(self, hostname, username='root', password=None, keypath=None, domain=None, proxy_hostname=None, proxy_username=None, proxy_password=None, proxy_keypath=None, config_yml=None, config_qa=None, credpath=None, aws_access_key=None, aws_secret_key=None, log_level='INFO', boto_debug_level=0, euca_user='admin', euca_account='eucalyptus', https=True):
        self.machine_connect_kwargs = {'hostname': hostname, 
           'username': username, 
           'password': password, 
           'keypath': keypath, 
        ...
        self.creds = AutoCreds(credpath=self._credpath, domain=domain, aws_access_key=aws_access_key, aws_secret_key=aws_secret_key, aws_account_name=euca_account, aws_user_name=euca_user, https=https, logger=self.log, **self.machine_connect_kwargs)
        self.properties_connection = PropertiesConnection(hostname=hostname, aws_secret_key=self.creds.aws_secret_key, aws_access_key=self.creds.aws_access_key, logger=self.log, boto_debug_level=boto_debug_level)
        super(SystemConnection, self).__init__(hostname=hostname, aws_secret_key=self.creds.aws_secret_key, aws_access_key=self.creds.aws_access_key, logger=self.log, boto_debug_level=boto_debug_level)
        return
    ...
    def set_loglevel(self, level, parent=False):
        ...
        if not isinstance(level, int) and not isinstance(level, basestring):
            raise ValueError(('set_loglevel. Level must be of type int or string, got: "{0}/{1}"').format(level, type(level)))
    ...
    def _get_all_local_ip_interfaces(self):
        ...
        outbytes = struct.unpack('iL', fcntl.ioctl(s.fileno(), 35090, struct.pack('iL', bytes, names.buffer_info()[0])))[0]
        namestr = names.tostring()
        interfaces = []
        for i in range(0, outbytes, 40):
            name = namestr[i:i + 16].split('\x00', 1)[0]
    ...
    def get_hosts_for_node_controllers(self, partition=None, instanceid=None):
        if instanceid is not None and not isinstance(instanceid, basestring):
            raise ValueError(('Instance id not of string type, got:"{0}"/"{1}"').format(instanceid, type(instanceid)))
        ...
        if not partition and not instanceid:
            ...
            if instanceid:
                try:
                    ...
                    self.log.error(('{0}\nFailed to find instance:"{1}" on system').format(get_traceback(), instanceid))
                    return []
    ...
    def show_hosts(self, hosts=None, partition=None, service_type=None, serv_columns=None, update=True, print_method=None, print_table=True, save_file=None):
        ...
        if hosts is None:
            ...
            if isinstance(hosts, list):
                ...
                if isinstance(hosts, EucaHost):
                    eucahosts[hosts.hostname] = hosts
                if not isinstance(eucahosts, dict):
                    raise ValueError(('show_machine_mappings requires dict example: {"host ip":[host objs]}, got:"{0}/{1}"').format(eucahosts, type(eucahosts)))
                try:
                    sorted_ips = sorted(list(eucahosts), key=lambda ip: struct.unpack('!L', socket.inet_aton(ip))[0])
                ...
                def add_host(hostip, host, self=self):
                    ...
                    with hostlock:
                        pt.add_row([markup('HOST:') + markup(hostip, [1, 94]),
                         markup('EUCALYPTUS SERVICES:') + markup(('[ {0} ]').format((' ').join(str(x) for x in host.euca_service_codes)), [
                          1, 34])])
    ...
    def upgrade_cloud(self, network_mode=None, ccs=None, ncs=None, clcs=None, scs=None, ufs=None, ws=None, gpgcheck=False, yum_arg_list=None, dry_run=False, rerun=False):
        if rerun:
            ...
            if network_mode not in known_net_modes:
                raise ValueError(('Unknown network mode:{0}, known types {1}').format(network_mode, (', ').join(known_net_modes)))
            ...
            upgrade_dict = {'network_mode': network_mode, 'ccs': ccs, 'ncs': ncs, 'clcs': clcs, 
               'scs': scs, 'ufs': ufs, 'ws': ws, 'gpgcheck': gpgcheck, 'yum_arg_list': yum_arg_list}
            if dry_run:
                return upgrade_dict
            ...
            try:
                try:
                    ...
                    self.log.error('Upgrade failed. The upgrade params are found in self._upgrade_dict.These can be used via the "rerun" argument to rerun this upgradeusing the same environment/machines')
                    raise
    ...
    def modify_cloud_network_config_json(self, net_dict, property_name='cloud.network.network_configuration'):
