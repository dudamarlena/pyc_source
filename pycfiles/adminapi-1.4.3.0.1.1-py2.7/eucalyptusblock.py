...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cloud_admin/cloudview/eucalyptusblock.py
...
def get_values_from_hosts(hostdict, host_method_name=None, host_attr_chain=[], **host_method_kwargs):
    """
    ...
    if not host_method_name and not host_attr_chain or host_method_name and host_attr_chain:
        raise ValueError(('Must provide either host_method_name "or" host_attr, got:{0},{1}').format(host_method_name, host_attr_chain))
    ...
    def get_val_from_host(ip, host, lookup=lookup):
        ...
        with hostlock:
            debug(markup(('Got value for host: "{0}.{1}" = "{2}"').format(ip, lookup, value), [1, 94]), host)
...
class EucalyptusBlock(ConfigBlock):
    ...
    def build_active_config(self, do_topology=True, do_node_config=True, do_props=True, do_repo_urls=True, do_network=True, do_service_images=True, do_euca_conf_gen=True, do_cc_config=True, do_yum_options=True):
        if do_topology:
    ...
    def discover_repo_urls(self):
        ...
        """
        repo_map = {'eucalyptus-repo': ['eucalyptus_repo_file', 'baseurl'], 'eucalyptus-enterprise-repo': [
                                        'eucalyptus_enterprise_repo_file', 'baseurl'], 
           'euca2ools-repo': [
                            'euca2ools_repo_file', 'baseurl']}
    ...
    def discover_yum_options(self):
        ...
        """
        opt_map = {'yum-options': ['eucalyptus_repo_file', 'gpgcheck'], 'yum-options': [
                         'eucalyptus_enterprise_repo_file', 'gpgcheck'], 
           'yum-options': [
                         'euca2ools_repo_file', 'gpgcheck']}
        opt_dict = {}
    ...
    def discover_euca_conf_general(self):
        ...
        """
        config_map = {'cloud-opts': 'CLOUD_OPTS', 'eucalyptus_conf': 'LOG_LEVEL', 
           'home-directory': 'EUCALYPTUS', 
           'user': 'EUCA_USER'}
        for localname, confname in config_map.iteritems():
            value = get_values_from_hosts(hostdict=self._connection.eucahosts, host_attr_chain=[
             'eucalyptus_conf', ('{0}').format(confname)])
            if value:
                setattr(self, localname, value)
...
class NodeControllerBlock(ConfigBlock):
    ...
    def build_active_config(self):
        confmap = {'max-cores': ['eucalyptus_conf', 'MAX_CORES'], 'cachesize': [
                       'eucalyptus_conf', 'NC_CACHE_SIZE'], 
           'service-path': [
                          'eucalyptus_conf', 'NC_SERVICE'], 
           'port': [
                  'eucalyptus_conf', 'NC_PORT'], 
           'work-size': [
                       'eucalyptus_conf', 'NC_WORK_SIZE'], 
           'hypervisor': [
                        'eucalyptus_conf', 'HYPERVISOR'], 
           'work-size': [
                       'eucalyptus_conf', 'NC_WORK_SIZE'], 
           'instance-path': [
                           'eucalyptus_conf', 'INSTANCE_PATH']}
        for localname, confname in confmap.iteritems():
            value = get_values_from_hosts(hostdict=self._connection.eucahosts, host_attr_chain=confname)
            if value:
                setattr(self, localname, value)
...
class ClusterControllerBlock(ConfigBlock):
    ...
    def build_active_config(self):
        confmap = {'port': ['eucalyptus_conf', 'CC_PORT'], 'scheduling-policy': [
                               'eucalyptus_conf', 'SCHEDPOLICY']}
        for localname, confname in confmap.iteritems():
            value = get_values_from_hosts(hostdict=self._connection.eucahosts, host_attr_chain=confname)
            if value:
                setattr(self, localname, value)
...
class NetworkConfigBlock(ConfigBlock):
    ...
    def build_active_config(self):
        interface_map = {'private-interface': 'VNET_PRIVINTERFACE', 'public-interface': 'VNET_PUBINTERFACE', 
