...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cloud_utils/file_utils/eucarc.py
...
class Eucarc(object):
    ...
    def __init__(self, filepath=None, string=None, sshconnection=None, keysdir=None, logger=None, loglevel='INFO'):
        """
    ...
    def keys_dir(self, value):
        ...
        if self._unparsed_lines:
            try:
                ...
                self.log.debug(('Ignoring errors while attemping to resolve conf attrs with keys dir:{0}').format(E))

    def _from_ini_file(self, file_obj, user_string=None, region=None, all_conf=False):
        """
        ...
        for key, value in ret_dict.iteritems():
            try:
                ...
                self.log.error(('{0}\nFailed to set attr:{1} to value:{2}. Error:{3}').format(get_traceback(), key, value, E))
        ...
        if string:
            if not isinstance(string, list):
                if not isinstance(string, basestring):
                    raise TypeError(('"_from_string" expected string(basestring) type, got:{0}:{1}').format(string, type(string)))
                string = str(string)
            ...
            for line in lines:
                if line:
                    match = re.search('^\\s*export\\s+(\\w+)=\\s*(\\S+)$', line) or re.search('^\\s*(\\S+)\\s*=\\s*(\\S+)$', line)
                    if not match:
                        message += line + '\n'
                    else:
                        key = match.group(1)
                        ...
                        if not (key and value):
                            raise ValueError(('Fix me! Could not find key=value, in this line:"{0}"').format(line))
                        key = key.lower().replace('-', '_')
                        self.__setattr__(key, value)
                        new_dict[key] = value
    ...
    def _from_filepath(self, filepath=None, sshconnection=None, keysdir=None, is_ini=None, user_string=None, region=None):
        """
        ...
        if sshconnection:
            ...
            if not remotepath:
                raise ValueError(('Remote File not found on host:"{0}" at path(s):"{1}", or "{2}"').format(sshconnection.host, filepath, os.path.join(filepath, 'eucarc')))
            keysdir = urljoin(sftppath, keysdir)
            self._keysdir = keysdir
            if is_ini:
                return self._from_ini_file(file_obj=sshconnection.sftp.open(remotepath), keysdir=keysdir)
            ...
            if not os.path.isfile(filepath):
                ...
                if not os.path.isfile(filepath):
                    raise ValueError(('File not found at path(s):"{0}", or "{1}"').format(orig_file_path, filepath))
            f = open(filepath)
            if is_ini:
                return self._from_ini_file(file_obj=f, keysdir=keysdir)
            with f:
                string = f.read()
    ...
    def show(self, search=None, print_method=None, print_table=True):
        ...
        for key, value in attrs.iteritems():
            if value is None or isinstance(value, basestring):
                if not search or match_op(re.search(str(search), key) or value and re.search(str(search), value)):
    ...
    def get_eucarc_attrs(self, excludes=[
 '^_'], show_empty=False):
        ret_dict = {}
