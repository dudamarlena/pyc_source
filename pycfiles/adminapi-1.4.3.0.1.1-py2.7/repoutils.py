...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cloud_utils/system_utils/repoutils.py
...
class Yum(PackageManager):
    ...
    def get_url_for_package(self, package_name):
        ...
        for line in out:
            ...
            if match:
                line = match.group(1)
                self.repo_url_cache[package_name] = {'updated': time.time(), 'url': line}
                return line

        self.machine.log.error(markup(('URL not found for local package:"{0}"').format(package_name), markups=[1, 31]))
    ...
    def get_repo_file_by_baseurl(self, url, repopath=None, cleanurl=True, filters=None):
        ...
        try:
            ...
            self.machine.log.error(markup(('Could not find repo for url:"{0}", err:"{1}"').format(url, str(CE)), markups=[1, 31]))
            return
    ...
    def get_repo_file(self, filepath, filters=None):
        ...
        try:
            ...
            self.machine.log.error(markup(('Failed to read repo_file at:"{0}", err:"{1}"').format(filepath, str(CE)), markups=[1, 31]))
            return
        ...
        if not values:
            self.machine.log.error(markup(('No values parsed from:"{0}"').format(filepath), markups=[1, 31]))
            return
        else:
            if not filters:
...
class Apt(PackageManager):
    ...
    def upgrade(self, package=None):
        if package is None:
            package = ''
        return self.machine.sys('export DEBIAN_FRONTEND=noninteractive; apt-get dist-upgrade %s %s ' % (
