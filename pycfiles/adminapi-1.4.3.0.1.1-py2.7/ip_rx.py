...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cloud_utils/net_utils/ip_rx.py
...
import re, socket, sys, traceback, json, struct, time
...
def remote_receiver(ssh, src_addrs=None, proto=17, dst_addrs=None, port=None, count=30, bind=False, timeout=15, cb=None, cbargs=None, verbose_level=DEBUG):
    """
        ...
        for addr, y in packets.iteritems():
            for d, p in y.iteritems():
                ...
                for pn, cnt in p.iteritems():
                    dest_packets[d][pn] = (dest_packets.get(d, None) and dest_packets[d].get(pn, None) or 0) + cnt
...
def get_option_parser():
    parser = OptionParser()
    parser.add_option('-n', '--testname', dest='testname', default=None, help='Name used to identify test results', metavar='TESTNAME')
    parser.add_option('-p', '--dst-ports', dest='destports', default='', help='Comma separated list of Destination Ports to filter on, example:8773', metavar='PORT')
    parser.add_option('-r', '--src-ports', dest='srcports', default='', help='Comma separated list of Source Ports to filter on', metavar='PORT')
    parser.add_option('-o', '--proto', dest='proto', type='int', default=17, help="Protocol type, examples: 6 for TCP, 17 for UDP.\nDefault: 132 for 'sctp'", metavar='PROTO')
    parser.add_option('-t', '--timeout', dest='timeout', type='int', default=None, help="Amount of time to collect packets'", metavar='COUNT')
    parser.add_option('-c', '--count', dest='count', type='int', default=0, help="Max packet count before exiting'", metavar='COUNT')
    parser.add_option('-s', '--src-addrs', dest='srcaddrs', default='', help='Comma delimited list of src ip addresses used to filter', metavar='ADDRS')
    parser.add_option('-d', '--dst-addrs', dest='dstaddrs', default='', help='Comma delimited list of dst ip addresses used to filter, example: 228.7.7.3', metavar='ADDRS')
    parser.add_option('--bind', dest='bind', action='store_true', default=False, help='Flag to enable port binding, default:false')
    parser.add_option('-v', '--verbose', dest='verbose', type='int', default=INFO, help='Verbose level, 0=quiet, 1=info, 2=debug, 3=trace. Default=1')
    parser.add_option('-a', '--addr', dest='addr', default='', help="Local addr to bind to, default is '' or 'listen on all'", metavar='HOST')
    parser.add_option('-f', '--file', dest='resultsfile', default='', help='File Path to save results to', metavar='ADDRS')
    return parser
...
class IPHdr(object):
    ...
    def print_me(self, verbose=INFO):
        ...
        """
        debug(('IP ver:{0}, HDR LEN:{1}, TTL:{2}, PROTO:{3}, SRC ADDR:{4}, DST ADDR:{5}').format(self.version, self.header_len, self.ttl, self.protocol, self.src_addr, self.dst_addr), level=verbose)
...
if __name__ == '__main__':
    ...
    if options.testname is None:
        options.testname = ('PROTO:{0}, SRCADDRS:{1}, DSTADDRS:{2}, PORTS:{3}').format(PROTO, SRCADDRS, DSTADDRS, DSTPORTS)
    results = {'packets': {}, 'protocol': PROTO, 
       'elapsed': None, 
       'count': None, 
    ...
    line = '--------------------------------------------------------------------------------'
    start = time.time()
    debug(('{0} For Protocol:{1}/{2}').format(START_MESSAGE, get_proto_name(PROTO), PROTO), level=INFO)
    if PROTO in (6, 'tcp'):
        ...
        try:
            try:
                ...
                if 'not permitted' in SE.strerror:
                    try:
                        results['error'] += ('ERROR: This may need additional permission(s) to run? root, sudo, etc? Err:"{0}"\n').format(SE)
                    except:
                        pass
            ...
            if socktype == socket.SOCK_STREAM:
                ...
                try:
                    ...
                    sys.stderr.write(('# ERROR sock.accept() timed out. Last knonw sock timeout:{0}').format(time_remaining))
            ...
            while not done:
                ...
                if COUNT == 0 or pkts < COUNT:
                    ...
                    if not socktype == socket.SOCK_STREAM:
                        if DSTPORTS:
                            srcport, dstport = struct.unpack('!HH', data[iphdr.header_len:iphdr.header_len + 4])
                            ...
                            srcport = 'unknown'
                            dstport = 'unknown'
                    if (not SRCADDRS or iphdr.src_addr in SRCADDRS) and (not DSTADDRS or iphdr.dst_addr in DSTADDRS):
