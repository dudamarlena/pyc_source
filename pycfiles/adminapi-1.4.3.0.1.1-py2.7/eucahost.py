...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cloud_admin/hosts/eucahost.py
...
class EucaHost(Machine):
    ...
    def summary_string(self):
        ...
        """
        try:
            return ('{0} {1} {2} {3} {4} {5}').format(self.hostname, self.distro, self.distro_ver, self.arch, self.partitions, ('[{0}]').format((' ').join(str(x) for x in self.euca_service_codes)))
    ...
    def eucalyptus_repo_file(self):
        ...
        if not self._eucalyptus_repo_file:
            self._eucalyptus_repo_file = self.package_manager.get_repo_file_by_baseurl(url=self.get_eucalyptus_repo_url())
    ...
    def eucalyptus_enterprise_repo_file(self):
        ...
        if not self._eucalyptus_enterprise_repo_file:
            self._eucalyptus_enterprise_repo_file = self.package_manager.get_repo_file_by_baseurl(url=self.get_eucalyptus_enterprise_repo_url())
    ...
    def euca2ools_repo_file(self):
        ...
        if not self._euca2ools_repo_file:
            self._euca2ools_repo_file = self.package_manager.get_repo_file_by_baseurl(url=self.get_euca2ools_repo_url())
    ...
    def get_available_eucalyptus_packages(self, searchstring='eucalyptus'):
        cmd = ('yum search eucalyptus -q  2> /dev/null | grep -e "{0}" | grep -iv error | awk \'{print $1}\'').format(searchstring)
        ...
        if basepaths is None:
            ...
            for path in basepaths:
                try:
                    ...
                    out = self.sys(('cat {0}').format(eucalyptus_conf_path), code=0, verbose=verbose)
                    if verbose:
