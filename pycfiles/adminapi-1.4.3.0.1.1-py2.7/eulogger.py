...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cloud_utils/log_utils/eulogger.py
...
class Eulogger(logging.Logger):
    ...
    def __init__(self, identifier, parent_logger_name='eulogger', stdout_level='debug', stdout_format=None, logfile='', logfile_level='INFO', file_format=None, show_init=False):
        """
        ...
        """
        if show_init:
            print ('-----------------------------------------------\nparent_logger_name:{0}\neulogger init:\nidentifier:{1}\nstdout_level:{2}\nstdout_format:{3}\nlogfile:{4}\nlogfile_level:{5}\nfile_format:{6}\n\n-----------------------------------------------').format(parent_logger_name, identifier, stdout_level, stdout_format, logfile, logfile_level, file_format)
        if isinstance(stdout_level, basestring):
            self.stdout_level = getattr(logging, stdout_level.upper(), logging.DEBUG)
        else:
            if isinstance(stdout_level, int):
            ...
            self._default_format = stdout_format or logging.Formatter('[%(asctime)s][%(levelname)s]%(message)s')
