...
def check_claim(irc, channel, sender, chanobj=None):
    ...
    success = not relay or irc.name == relay[0] or not db[relay]['claim'] or irc.name in db[relay]['claim'] or any([mode in sender_modes for mode in frozenset({'o', 'y', 'q', 'a', 'h'})]) and not irc.is_privileged_service(sender) or irc.is_internal_client(sender) or irc.is_internal_server(sender)
    if not success:
        with claim_bounce_cache_lock:
            if irc.name not in claim_bounce_cache:
                    ...
                    if modechar in irc.prefixmodes:
                        ...
                        if arg:
                            for extban_name, extban_prefix in irc.extbans_acting.items():
                                if name in frozenset({'banexception', 'ban'}):
