...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pylinkirc/utils.py
# Compiled at: 2020-04-11 03:31:40
# Size of source mod 2**32: 35852 bytes
"""
...
framework.
"""
import argparse, collections, functools, importlib, ipaddress, os, re, string
...
NORMALIZEWHITESPACE_RE = re.compile('\\s+')
...
def _reset_module_dirs():
    ...
    """
    plugins.__path__ = [
     plugins.__path__[0]] + [expandpath(path) for path in conf.conf['pylink'].get('plugin_dirs', [])]
...
class ServiceBot:
    ...
    def join(self, irc, channels, ignore_empty=None):
        ...
        if irc.has_cap('visible-state-only'):
            ...
            if ignore_empty is None:
                ignore_empty = not irc.serverdata.get('join_empty_channels', conf.conf['pylink'].get('join_empty_channels', False))
        ...
        for channel in channels:
            if channel in irc.channels and uid in irc.channels[channel].users:
                if channel in persistent_channels:
                    log.debug('(%s/%s) Not parting %r because it is registered as a dynamic channel: %r', irc.name, self.name, channel, persistent_channels)
                    continue
    ...
    def call_cmd(self, irc, source, text, called_in=None):
        ...
        if cmd not in self.commands:
            show_unknown_cmds = irc.serverdata.get('%s_show_unknown_commands' % self.name, conf.conf.get(self.name, {}).get('show_unknown_commands', conf.conf['pylink'].get('show_unknown_commands', True)))
            if cmd:
                if show_unknown_cmds:
        ...
        for func in self.commands[cmd]:
            try:
                ...
                log.warning('(%s) Denying access to command %r for %s; msg: %s', irc.name, cmd, irc.get_hostmask(source), e)
    ...
    def add_persistent_channel(self, irc, namespace, channel, try_join=True):
        ...
        if try_join:
            if irc.has_cap('can-manage-bot-channels'):
    ...
    def get_persistent_channels(self, irc, namespace=None):
        ...
        if namespace:
            ...
            log.debug('(%s/%s) get_persistent_channels: adding channels %r from namespace %r (single)', irc.name, self.name, chanlist, namespace)
            channels |= chanlist
        else:
            for dch_namespace, dch_data in self.dynamic_channels.items():
                chanlist = dch_data.get(irc.name, set())
                log.debug('(%s/%s) get_persistent_channels: adding channels %r from namespace %r', irc.name, self.name, chanlist, dch_namespace)
                channels |= chanlist
    ...
    def clear_persistent_channels(self, irc, namespace, try_part=True, part_reason=''):
        ...
        if irc is not None:
            if irc.name in dch_data:
                ...
                log.debug('(%s/%s) Clearing persistent channels %r from namespace %r', irc.name, self.name, chanlist, namespace)
            ...
            for netname, chanlist in dch_data.items():
                log.debug('(%s/%s) Globally clearing persistent channels %r from namespace %r', netname, self.name, chanlist, namespace)
                if try_part and netname in world.networkobjects:
                    self.part((world.networkobjects[netname]), chanlist, reason=part_reason)
    ...
    def listcommands(self, irc, source, args):
        ...
        cmds = sorted(cmd for cmd in self.commands.keys() if '\x01' not in cmd if cmd not in self.alias_cmds)
...
def register_service(name, *args, **kwargs):
    ...
    if name in world.services:
        ...
        if name != 'pylink':
            if not conf.conf.get(name, {}).get('spawn_service', conf.conf['pylink'].get('spawn_services', True)):
...
def wrap_arguments(prefix, args, length, separator=' ', max_args_per_line=0):
    ...
    args = list(args)
    while args:
        assert len(prefix + args[0]) <= length, 'wrap_arguments: Argument %r is too long for the given length %s' % (args[0], length)
...
_strip_color_regex = re.compile('\\x03(\\d{1,2}(,\\d{1,2})?)?')
...
_subrange_re = re.compile('(?P<start>(\\d+))-(?P<end>(\\d+))')
    ...
    for subrange in ranges:
        ...
        if match:
            ...
            if end <= start:
                ...
                if 0 in (end, start):
                    raise ValueError('Got range index 0 in range string %r, this function is one-indexed' % rangestr)
            ...
            if subrange in string.digits:
                index = int(subrange)
                if index == 0:
                    raise ValueError('Got index 0 in range string %r, this function is one-indexed' % rangestr)
...
_duration_re = re.compile('^((?P<week>\\d+)w)?((?P<day>\\d+)d)?((?P<hour>\\d+)h)?((?P<minute>\\d+)m)?((?P<second>\\d+)s)?$')
