...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pylinkirc/plugins/opercmds.py
# Compiled at: 2020-04-11 03:31:40
# Size of source mod 2**32: 22226 bytes
"""
...
def massban(irc, source, args, use_regex=False):
    ...
    if args.channel not in irc.channels:
        ...
        return
    if 'relay' in world.plugins:
        if not world.plugins['relay'].check_claim(irc, args.channel, source):
            if not args.force:
    ...
    for uid in userlist_func((args.banmask), channel=(args.channel)):
        if irc.is_oper(uid):
            ...
            try:
                ...
                log.exception('(%s) Failed to send process massban hook; some bans may have not been sent to plugins / relay networks!', irc.name)
            ...
            try:
                irc.call_hooks([irc.pseudoclient.uid, 'OPERCMDS_MASSKICK',
                 {'channel':args.channel, 
                  'target':uid,  'text':reason,  'parse_as':'KICK'}])
            except:
                log.exception('(%s) Failed to send process massban hook; some kicks may have not been sent to plugins / relay networks!', irc.name)

        results += 1
    else:
        irc.reply('Banned %s users on %r.' % (results, args.channel))
        log.info('(%s) Ran massban%s for %s on %s (%s user(s) removed)', irc.name, 're' if use_regex else '', irc.get_hostmask(source), args.channel, results)
...
def masskill(irc, source, args, use_regex=False):
    ...
    for uid in userlist_func(args.banmask):
            ...
            for channel in userobj.channels.copy():
                if args.force_kb or relay.check_claim(irc, channel, source):
                    ...
                    try:
                        ...
                        log.exception('(%s) Failed to send process massban hook; some kickbans may have not been sent to plugins / relay networks!', irc.name)

                elif uid not in seen_users:
                    killed += 1
                else:
                    irc.reply("Not kicking \x02%s\x02 from \x02%s\x02 because you don't have CLAIM access. If this is another network's channel, ask someone to op you or use the --force-kb option." % (
            ...
            if args.akill:
                ...
                try:
                    irc.call_hooks([irc.pseudoclient.uid, 'OPERCMDS_MASSKILL',
                     {'target':uid, 
                      'parse_as':'KILL',  'userdata':userobj,  'text':reason}])
                except:
                    log.exception('(%s) Failed to send process massban hook; some kickbans may have not been sent to plugins / relay networks!', irc.name)
        ...
        log.info('(%s) Ran masskill%s for %s (%s/%s user(s) removed)', irc.name, 're' if use_regex else '', irc.get_hostmask(source), killed, results)
...
def kick(irc, source, args):
    ...
    try:
        ...
        irc.reply('Done.')
        irc.call_hooks([sender, 'OPERCMDS_KICK',
         {'channel':channel,  'target':targetu,  'text':reason, 
          'parse_as':'KICK'}])
...
def kill(irc, source, args):
    ...
    try:
        ...
        irc.reply('Done.')
        irc.call_hooks([source, 'OPERCMDS_KILL',
         {'target':targetu,  'text':reason,  'userdata':userdata, 
          'parse_as':'KILL'}])
...
def topic(irc, source, args):
    ...
    try:
        ...
        irc.call_hooks([irc.pseudoclient.uid, 'OPERCMDS_TOPIC',
         {'channel':channel, 
          'text':topic,  'setter':source,  'parse_as':'TOPIC'}])
