...
# [GCC 8.4.0]
# Embedded file name: d:\Projects\python\EasyVision\EasyVision\processors\mptransform.py
# Compiled at: 2019-02-15 05:40:02
...
"""
import cv2, numpy as np, multiprocessing as mp, multiprocessing.connection
from .base import *
...
multiprocessing.connection.BUFSIZE = 67108864
...
class MultiProcessing(ProcessorBase, mp.Process):
    ...
    def setup(self):
        if not not self._running.value:
            raise AssertionError
            self.start()
            raise (self._run_event.wait(self._timeout) or TimeoutError)()
        ...
        try:
            for frame in self._vision:
                ...
                if self._freerun:
                    ...
                    while not self._cap_event.wait(0.1):
                        self._remote_call_handle()
                        if not self._running.value:
                            break
