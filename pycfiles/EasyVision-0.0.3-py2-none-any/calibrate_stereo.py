...
# [GCC 8.4.0]
# Embedded file name: d:\Projects\python\EasyVision\EasyVision\bin\calibrate_stereo.py
# Compiled at: 2019-02-06 09:37:48
...
def main():
    ...
    parser.add_argument('-t', '--test', const=True, default=False, action='store_const', help='Test camera calibration file')
    parser.add_argument('-d', '--disparity', const=True, default=False, action='store_const', help='Calculate Disparity Map')
    ...
    builder = Builder(Builder(VideoCapture, Args(left, width=size[0], height=size[1], fps=int(args.fps)), ImageTransform, Args(ocl=args.test), CalibratedCamera, Args(None if camera_model is None else camera_model.left)), Builder(VideoCapture, Args(right, width=size[0], height=size[1], fps=int(args.fps)), ImageTransform, Args(ocl=args.test), CalibratedCamera, Args(None if camera_model is None else camera_model.right)), CalibratedStereoCamera, Args(camera_model, max_samples=args.N, grid_shape=grid, calculate_disparity=args.disparity, display_results=True))
    if args.test:
