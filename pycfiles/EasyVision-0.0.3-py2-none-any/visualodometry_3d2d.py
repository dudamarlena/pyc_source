...
# [GCC 8.4.0]
# Embedded file name: d:\Projects\python\EasyVision\EasyVision\engine\visualodometry_3d2d.py
# Compiled at: 2019-02-07 17:19:30
...
class VisualOdometry3D2DEngine(FeatureMatchingMixin, OdometryBase):
    ...
    def __init__(self, vision, _map=None, feature_type=None, pose=None, num_features=3000, min_matches=30, distance_thresh=None, reproj_thresh=None, reproj_error=None, *args, **kwargs):
    ...
    def compute(self, absolute_scale=1.0):
        ...
        if not frame:
            ...
            if len(self._images) == 3 and self._images[(-3)][1] is not None:
                matches = self._match_features(self._images[(-3)][1].descriptors, self._images[(-1)][0].descriptors, self._feature_type, self._ratio, self._distance_thresh / 3, self._min_matches)
                ...
                for iteration in range(10):
                    ret, r, t, inliers = cv2.solvePnPRansac(points_3d, points_2d, self._camera.matrix, None, _r, _t, use_rt, reprojectionError=self._reproj_error, confidence=0.999 - 0.1 * iteration)
                    projected_2d, _ = cv2.projectPoints(points_3d, r, t, self.camera.matrix, None)
                    reproj_error_inliers = sum(p.dot(p) ** 0.5 for i, p in enumerate(a - b[0] for a, b in zip(points_2d, projected_2d)) if i in inliers) / len(inliers)
                ...
                if ret:
                    if self._pose:
                        self._pose = self._pose._replace(timestamp=frame.timestamp, translation=self._pose.translation + absolute_scale * self._pose.rotation.dot(t), rotation=R.dot(self._pose.rotation), features=Features(points_2d, descriptors, points_3d))
                    else:
    ...
    def _calculate_3d(self, featuresA, featuresB, scale):
        ...
        mask = [ 0.5 < p[0] ** 2 + p[1] ** 2 < 40000 for p in ((a.pt[0] - b.pt[0], a.pt[1] - b.pt[1]) for a, b in zip(kpsA, kpsB)) ]
