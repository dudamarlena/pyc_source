...
# [GCC 8.4.0]
# Embedded file name: d:\Projects\python\EasyVision\EasyVision\processors\blobextractor.py
# Compiled at: 2019-02-06 10:58:10
...
class Blobs(namedtuple('Features', ['points', 'descriptors'])):
    ...
    @property
    def keypoints(self):
        return [ cv2.KeyPoint(x=pt.pt[0], y=pt.pt[1], _size=pt.size, _angle=pt.angle, _response=pt.response, _octave=pt.octave, _class_id=pt.class_id) for pt in self.points
               ]
...
class BlobExtraction(ProcessorBase):
    ...
    def __init__(self, vision, histogram, blur_size=(30, 30), channels=(0, 1), ranges=(0, 180, 0, 256), area=(
 100, 160000), min_circularity=None, min_convexity=None, min_inertia=None, *args, **kwargs):
        ...
        """
        vision = (isinstance(vision, HistogramBackprojection) or HistogramBackprojection)(vision, histogram, channels=channels, ranges=ranges) if 1 else vision
    ...
    def _draw_keypoints(self, image, keypoints):
        ...
        img = cv2.drawKeypoints(image, keypoints, np.array([]), color=(0, 0, 255), flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
