...
# [GCC 8.4.0]
# Embedded file name: d:\Projects\python\EasyVision\EasyVision\processorstackbuilder.py
# Compiled at: 2019-02-07 17:36:06
...
class Args(object):
    ...
    def fromdict(d, classes=None):
        ...
        return Args(*tuple(Args._retrieve_object(classes, d['objects'], arg) for arg in d['args']), **{arg:Args._retrieve_object(classes, d['objects'], value) for arg, value in d['kwargs'].items()})
    ...
    def convert_classes(classes):
        ...
        if isinstance(classes, dict):
            return classes
        if classes:
            return {cls.__name__:cls for cls in classes if hasattr(cls, 'todict') and hasattr(cls, 'fromdict') if hasattr(cls, 'todict') and hasattr(cls, 'fromdict')}
        return {}
