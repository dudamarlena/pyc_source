...
# [GCC 8.4.0]
# Embedded file name: d:\Projects\python\EasyVision\EasyVision\processors\calibratedstereocamera.py
# Compiled at: 2019-02-11 11:29:03
...
class StereoCamera(namedtuple('StereoCamera', 'left right R T E F Q')):
    ...
    def from_parameters(size, M1, d1, R1, P1, M2, d2, R2, P2, R, T, E, F, Q):
        ...
        :return: StereoCamera
        """
        return StereoCamera(PinholeCamera(size, M1, d1, R1, P1), PinholeCamera(size, M2, d2, R2, P2), R, T, E, F, Q)
...
class CameraPairProxy(VisionBase):
    ...
    def get_source(self, source):
        return (
         self._left.get_source(source), self._right.get_source(source))

    def __getattr__(self, name):
        return (
         getattr(self._left, name), getattr(self._right, name))
...
class CalibratedStereoCamera(ProcessorBase):
    ...
    def __init__(self, left, right, camera=None, calculate_disparity=False, num_disparities=255, block_size=15, grid_shape=(9, 6), square_size=20, max_samples=20, frame_delay=1, *args, **kwargs):
        ...
        if not isinstance(left, ProcessorBase) and not isinstance(left, PyroCapture) or not isinstance(right, ProcessorBase) and not isinstance(right, PyroCapture) or left.get_source('CalibratedCamera') is None or right.get_source('CalibratedCamera') is None:
        ...
        if not calibrate:
            ...
            self._camera = None
            self._stereocalib_criteria = (cv2.TERM_CRITERIA_MAX_ITER + cv2.TERM_CRITERIA_EPS, 100, 1e-05)
            self._flags = 0
    ...
    def process(self, image):
        if self._calibrate:
            ...
            if self.display_results:
                ...
                cv2.putText(img, ('Samples added: {}/{}').format(self.calibration_samples, self._max_samples), (20,
                                                                                                                11), cv2.FONT_HERSHEY_PLAIN, 1, (0,
                                                                                                                                                 255,
                                                                                                                                                 0), 1, 8)
    ...
    def _finish_calibration(self, objpoints, imgpoints_l, imgpoints_r, shape):
        ...
        ret, M1, d1, M2, d2, R, T, E, F = cv2.stereoCalibrate(objpoints, imgpoints_l, imgpoints_r, left_camera.matrix, left_camera.distortion, right_camera.matrix, right_camera.distortion, shape, criteria=self._stereocalib_criteria, flags=self._flags)
        R1, R2, P1, P2, Q, vb1, vb2 = cv2.stereoRectify(M1, d1, M2, d2, shape, R, T, flags=cv2.CALIB_ZERO_DISPARITY)
