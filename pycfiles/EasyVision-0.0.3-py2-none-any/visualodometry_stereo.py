...
# [GCC 8.4.0]
# Embedded file name: d:\Projects\python\EasyVision\EasyVision\engine\visualodometry_stereo.py
# Compiled at: 2019-02-07 17:19:34
...
class VisualOdometryStereoEngine(FeatureMatchingMixin, OdometryBase):
    ...
    def __init__(self, vision, _map=None, feature_type=None, pose=None, num_features=None, nlevels=None, ratio=None, distance_thresh=None, reproj_thresh=None, reproj_error=None, min_dZ=None, max_dZ=None, max_dY=None, max_dX=None, *args, **kwargs):
        ...
        _vision = (feature_extractor_provided or FeatureExtraction)(vision, feature_type=feature_type, **defaults) if 1 else vision
        ...
        if not frame:
            ...
            if self._last_3dfeatures is not None:
                ...
                for iteration in range(10):
                    ret, r, t, inliers = cv2.solvePnPRansac(last_points_3d, new_points_2d, self._camera.left.matrix, None, _r, _t, use_rt, reprojectionError=self._reproj_error, confidence=0.999 - 0.1 * iteration, iterationsCount=100)
                    projected_2d, _ = cv2.projectPoints(last_points_3d, r, t, self._camera.left.matrix, None)
                    reproj_error_inliers = sum(p.dot(p) ** 0.5 for i, p in enumerate(a - b[0] for a, b in zip(new_points_2d, projected_2d)) if i in inliers) / len(inliers)
                    reproj_error = sum(p.dot(p) ** 0.5 for p in (a - b[0] for a, b in zip(new_points_2d, projected_2d))) / len(new_points_2d)
                    if reproj_error_inliers < self._reproj_error:
                        R, _ = cv2.Rodrigues(r * -1)
                        t *= -1
                        dZ = sum(i[0] ** 2 for i in t) ** 0.5
                        break
                    else:
                        print (
                         'failed to find inliers', reproj_error_inliers, '<', self._reproj_error)
                else:
                    ret = False
                ...
                if ret:
                    ...
                    if self._pose:
                        self._pose = self._pose._replace(timestamp=frame.timestamp, translation=self._pose.translation + self._pose.rotation.dot(t), rotation=R.dot(self._pose.rotation), features=Features(new_points_2d, new_descriptors, new_points_3d_inliers))
                    else:
                ...
                if self.debug:
                    ...
                    for a, b in zip(last_points_3d, new_points_3d):
                        cv2.line(self.features, (
                         ...
                         300 + int(300 * b[0] / scale), 600 - int(600 * b[2] / scale)), (0,
                                                                                         255,
                                                                                         0))

                    text = 'Number of last 3d features: %i' % len(self._last_3dfeatures[2])
                    cv2.putText(self.features, text, (20, 10), cv2.FONT_HERSHEY_PLAIN, 1, (255,
                                                                                           255,
                                                                                           255), 1, 8)
                    text = 'Number of new 3d features: %i' % len(stereo_features[2])
                    cv2.putText(self.features, text, (20, 25), cv2.FONT_HERSHEY_PLAIN, 1, (255,
                                                                                           255,
                                                                                           255), 1, 8)
                    text = 'Number of matching features: %i' % len(new_points_2d)
                    cv2.putText(self.features, text, (20, 40), cv2.FONT_HERSHEY_PLAIN, 1, (255,
                                                                                           255,
                                                                                           255), 1, 8)
                    text = 'Number of inliers: %i' % len(inliers)
                    cv2.putText(self.features, text, (20, 55), cv2.FONT_HERSHEY_PLAIN, 1, (255,
                                                                                           255,
                                                                                           255), 1, 8)
                    MIN = min(a[2] - b[2] for a, b in zip(last_points_3d, new_points_3d))
                    MAX = max(a[2] - b[2] for a, b in zip(last_points_3d, new_points_3d))
                    text = 'dZ min: %f' % MIN
                    cv2.putText(self.features, text, (20, 70), cv2.FONT_HERSHEY_PLAIN, 1, (255,
                                                                                           255,
                                                                                           255), 1, 8)
                    text = 'dZ max: %f' % MAX
                    cv2.putText(self.features, text, (20, 85), cv2.FONT_HERSHEY_PLAIN, 1, (255,
                                                                                           255,
                                                                                           255), 1, 8)
                    text = 'dZ pos: %f' % dZ
                    cv2.putText(self.features, text, (20, 100), cv2.FONT_HERSHEY_PLAIN, 1, (255,
                                                                                            255,
                                                                                            255), 1, 8)
                    text = 'reproj error inliers: %f' % reproj_error_inliers
                    cv2.putText(self.features, text, (300, 85), cv2.FONT_HERSHEY_PLAIN, 1, (255,
                                                                                            255,
                                                                                            255), 1, 8)
                    text = 'reproj error total  : %f' % reproj_error
                    cv2.putText(self.features, text, (300, 100), cv2.FONT_HERSHEY_PLAIN, 1, (255,
                                                                                             255,
                                                                                             255), 1, 8)
                    ...
                    for p in projected_2d:
                        cv2.circle(img, (int(p[0][0]), int(p[0][1]) + h), 3, (0, 0,
                                                                              255))
    ...
    def _match_stereo(self, last_features, new_features):
        ...
        """
        matches = self._match_features(last_features[3], new_features[3], self._feature_type, self._ratio, self._distance_thresh / 3, self._min_matches)
        if matches is None:
            return
        else:
            last_points_3d = [ last_features[2][m.queryIdx] for m in matches ]
            new_points_3d = [ new_features[2][m.trainIdx] for m in matches ]
            dZ = 3 * sum(i[0] ** 2 for i in self._last_pose.translation) ** 0.5 if self._last_pose else self._dZ
            ...
            return (
             last_points_2d, last_points_3d, last_descriptors, new_points_2d, new_points_3d, new_descriptors, last_points_2d_right, new_points_2d_right)
    ...
    def capabilities(self):
        return EngineCapability((
         ...
         Frame, Pose), {'feature_type': ('FREAK', 'SURF', 'SIFT', 'ORB', 'KAZE', 'AKAZE')})
