...
# [GCC 8.4.0]
# Embedded file name: d:\Projects\python\EasyVision\EasyVision\models\object.py
# Compiled at: 2019-02-06 12:30:07
...
class ObjectModel(ModelBase):
    ...
    def create_from_processed_image(name, image, display_results=False, **kwargs):
        ...
        if not image.feature_type:
            ...
            if outline is None:
                return
            if display_results:
                img = cv2.drawKeypoints(thumb, features.keypoints, np.array([]), color=(0,
                                                                                        0,
                                                                                        255), flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
    ...
    def _match_features(self, image, view, matcher, display_results=False, **kwargs):
        ...
        while len(ptsA) >= self._min_matches:
            ...
            if H is None:
                return results
            if sum(inliers) < self._min_matches or sum(inliers) < len(descriptorsB) * 0.01:
                return results
    ...
    def _draw(self, view_matches):
        ...
        for index, match in enumerate(view_matches):
            ...
            params = dict(singlePointColor=None, matchColor=(0, 255, 0), flags=2)
            res = cv2.drawMatches(match.image.image, match.image.features.keypoints, match.view.image, match.view.features.keypoints, match.matches, None, **params)
