...
# [GCC 8.4.0]
# Embedded file name: d:\Projects\python\EasyVision\EasyVision\engine\visualodometry_2d.py
# Compiled at: 2019-02-07 17:19:26
...
class VisualOdometry2DEngine(FeatureMatchingMixin, OdometryBase):
    ...
    def __init__(self, vision, _map=None, feature_type=None, pose=None, num_features=6000, min_features=1000, min_matches=30, distance_thresh=None, ratio=0.7, reproj_thresh=None, debug=False, display_results=False, *args, **kwargs):
        ...
        if feature_type == 'ORB':
            ...
            self._reproj_thresh = 0.5
            self._distance_thresh = 100
        self._feature_type = feature_type
        _vision = (feature_extractor_provided or FeatureExtraction)(vision, feature_type=feature_type, extract=self._extract, **defaults) if 1 else vision
        self._min_features = min_features
        self._lk_params = dict(winSize=(21, 21), criteria=(
    ...
    def _compute_match(self, timestamp, current_image, absolute_scale):
        ...
        E, mask = cv2.findEssentialMat(current, last, focal=self._camera.focal_point[0], pp=self._camera.center, method=cv2.RANSAC, prob=0.999, threshold=self._reproj_thresh)
        ...
        if ret:
            if self._map is not None:
                ...
                features = None
            if self._pose:
                self._pose = self._pose._replace(timestamp=timestamp, translation=self._pose.translation + absolute_scale * self._pose.rotation.dot(t), rotation=R.dot(self._pose.rotation), features=features)
            else:
    ...
    def _compute_track(self, timestamp, current_image, absolute_scale):
        ...
        if not self._last_image:
            ...
            if self.debug:
                ...
                cv2.imshow(self.name, img)
            E, mask = cv2.findEssentialMat(cur_kps, self._last_kps, focal=self._camera.focal_point[0], pp=self._camera.center, method=cv2.RANSAC, prob=0.999, threshold=self._reproj_thresh)
            last_kps = np.float32([ kp for m, kp in zip(mask, self._last_kps) if m ])
            cur_kps = np.float32([ kp for m, kp in zip(mask, cur_kps) if m ])
            _, R, t, mask = cv2.recoverPose(E, cur_kps, last_kps, focal=self._camera.focal_point[0], pp=self._camera.center)
            if self._pose:
                self._pose = self._pose._replace(timestamp=timestamp, translation=self._pose.translation + absolute_scale * self._pose.rotation.dot(t), rotation=R.dot(self._pose.rotation))
            else:
    ...
    def capabilities(self):
        return EngineCapability((
         ...
         Frame, Pose), {'feature_type': ('FREAK', 'SURF', 'SIFT', 'ORB', 'KAZE', 'AKAZE', 'FAST', 'GFTT')})
    ...
    def _track_features(self, image_ref, image_cur, px_ref):
        ...
        """
        kp2, st, err = cv2.calcOpticalFlowPyrLK(image_ref, image_cur, px_ref, None, **self._lk_params)
    ...
    def _match_features(self, featuresA, featuresB):
        ...
        mask = [ 0.5 < p[0] ** 2 + p[1] ** 2 < 40000 for p in ((a[0] - b[0], a[1] - b[1]) for a, b in zip(ptsA, ptsB)) ]
