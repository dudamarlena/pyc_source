...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\hsync\dir.py
# Compiled at: 2017-11-01 10:06:21
import hashlib, json, locale, os, re, sys, time, requests
...
class File:
    ...
    def from_dict(dict_data):
        return File(path=dict_data['path'], md5=dict_data['md5'], access_time=int(dict_data['access_time']), mod_time=int(dict_data['mod_time']))
...
class LocalDir(Dir):
    ...
    def post(self, index, file_data):
        ...
        with open(file_data.path, 'rb') as (fp):
            requests.post(self.server_list[index].get_request_url(), data=file_data.__dict__, files={'file_body': fp})
