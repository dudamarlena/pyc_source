...
# [GCC 8.4.0]
# Embedded file name: /Users/ctyfoxylos/personal/python/ynabintegrationslib/ynabintegrationslib/ynabintegrationslib.py
# Compiled at: 2019-12-09 07:25:38
...
class Service:
    __doc__ = 'Models a service to retrieve transactions and upload them to YNAB.'
    ...
    def get_transactions_for_budget(self, budget_name=None):
        ...
        if len(self.budgets) > 1:
            ...
            if budget_name is None:
                ...
                budget = next((budget for budget in self.budgets if budget.name.lower() == budget_name.lower()), None)
            return budget or []
        return [YnabServerTransaction(transaction, transaction.account) for transaction in budget.transactions]
    ...
    def get_contract_by_name(self, name):
        ...
        """
        return next((contract for contract in self.contracts if contract.name.lower() == name.lower()), None)
    ...
    def get_account_by_name(self, name):
        ...
        """
        return next((account for account in self.accounts if account.ynab_account_name.lower() == name.lower()), None)
    ...
    def get_transactions_for_ynab_account(self, account_name):
        ...
        if not account:
            return []
        return [YnabServerTransaction(transaction, transaction.account) for transaction in account.transactions]
    ...
    def register_account(self, contract_name, budget_name, ynab_account_name, account_id=None):
        ...
        if not ynab_contract:
            ...
            return False
        try:
            account_wrapper = getattr(importlib.import_module('ynabintegrationslib.adapters'), f"{ynab_contract.bank}{ynab_contract.type}")
            account = ynab_contract.contract.get_account(account_id)
            self._accounts.append(account_wrapper(account, self._ynab, budget_name, ynab_account_name))
            return True
        except Exception:
    ...
    def upload_all_missing_transactions(self, budget_name=None):
        ...
        for account in self.accounts:
            transactions = []
            self._logger.debug('Trying to retrieve all transactions until "%s" for account "%s"', first_transaction.date, account.ynab_account.name)
    ...
    def upload_transactions(self, transactions):
        ...
        return all([self._ynab.upload_transactions(budget_id, payloads) for budget_id, payloads in budgets.items()])