...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.7/site-packages/javatools/change.py
# Compiled at: 2018-10-18 16:07:18
# Size of source mod 2**32: 14910 bytes
"""
...
__all__ = ('squash', 'collect_by_typename', 'collect_by_type', 'iterate_by_type', 'yield_sorted_by_type',
           'Change', 'Addition', 'Removal', 'GenericChange', 'SuperChange', 'SquashedChange',
           'SquashedAddition', 'SquashedRemoval')
...
class Change(object):
    ...
    def get_description(self):
        return self.description or self.label + (' changed' if self.is_change() else ' unchanged')
    ...
    def simplify(self, options=None):
        ...
        its children if any.
        """
        simple = {'class':type(self).__name__, 
...
class SuperChange(GenericChange):
    ...
    def is_ignored(self, options):
        ...
        for change in changes:
            if change.is_change():
                return change.is_ignored(options) or False
    ...
    def simplify(self, options=None):
        ...
        for s in self.collect():
            if s.is_change():
                if not show_ignored:
                    s.is_ignored(options) or subs.append(s)
                elif show_unchanged:
                    subs.append(s)
