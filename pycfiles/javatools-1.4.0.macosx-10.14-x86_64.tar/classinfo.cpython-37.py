...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.7/site-packages/javatools/classinfo.py
...
__all__ = ('SHOW_HEADER', 'SHOW_PUBLIC', 'SHOW_PACKAGE', 'SHOW_PRIVATE', 'main', 'cli',
           'add_classinfo_optgroup', 'cli_class_provides', 'cli_class_requires',
           'cli_json_class', 'cli_print_class', 'cli_print_classinfo', 'cli_simplify_classinfo',
           'cli_simplify_field', 'cli_simplify_fields', 'cli_simplify_method', 'cli_simplify_methods')
SHOW_HEADER = 0
SHOW_PUBLIC = 1
SHOW_PACKAGE = 3
SHOW_PRIVATE = 7
    ...
    if options.indent:
        ...
        if lvt:
            ...
            for o, l, n, d, i in lvt:
                line = (
                 str(o), str(l), str(i), cval(n), cval(d))
                print('   %s' % '\t'.join(line))
        ...
        if lvtt:
            ...
            for o, l, n, s, i in lvtt:
                line = (
                 str(o), str(l), str(i), cval(n), cval(s))
                print('   %s' % '\t'.join(line))
...
def add_classinfo_optgroup(parser):
    ...
    g.add_argument('--class-provides', dest='class_provides', action='store_true',
      default=False,
      help='API provides information at the class level')
    g.add_argument('--class-requires', dest='class_requires', action='store_true',
      default=False,
      help='API requires information at the class level')
    g.add_argument('--api-ignore', dest='api_ignore', action='append',
      default=(list()),
      help='globs of packages to not print in provides or requires modes')
    g.add_argument('--header', dest='show', action='store_const',
      default=SHOW_PUBLIC,
      const=SHOW_HEADER,
      help='show just the class header, no members')
    g.add_argument('--public', dest='show', action='store_const',
      const=SHOW_PUBLIC,
      help='show only public members')
    g.add_argument('--package', dest='show', action='store_const',
      const=SHOW_PACKAGE,
      help='show public and protected members')
    g.add_argument('--private', dest='show', action='store_const',
      const=SHOW_PRIVATE,
      help='show all members')
    g.add_argument('-l', dest='lines', action='store_true', help='show the line number table')
    g.add_argument('-o', dest='locals', action='store_true', help='show the local variable tables')
    g.add_argument('-c', dest='disassemble', action='store_true', help='disassemble method code')
    g.add_argument('-s', dest='sigs', action='store_true', help='show internal type signatures')
    g.add_argument('-p', dest='constpool', action='store_true', help='show the constants pool')
    g.add_argument('--verbose', dest='verbose', action='store_true', help='sets -locsp options and shows stack bounds')


def create_optparser(progname):
    parser = ArgumentParser(prog=progname)
    parser.add_argument('classfile', nargs='+', help='Java class file(s) to inspect')
