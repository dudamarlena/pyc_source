...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.7/site-packages/javatools/__init__.py
# Compiled at: 2019-06-26 15:00:45
# Size of source mod 2**32: 61394 bytes
"""
...
__all__ = ('JavaClassInfo', 'JavaConstantPool', 'JavaMemberInfo', 'JavaCodeInfo', 'JavaExceptionInfo',
           'JavaInnerClassInfo', 'JavaAnnotation', 'NoPoolException', 'Unimplemented',
           'ClassUnpackException', 'platform_from_version', 'is_class', 'is_class_file',
           'unpack_class', 'unpack_classfile', 'CONST_Utf8', 'CONST_Integer', 'CONST_Float',
           'CONST_Long', 'CONST_Double', 'CONST_Class', 'CONST_String', 'CONST_Fieldref',
           'CONST_Methodref', 'CONST_InterfaceMethodref', 'CONST_NameAndType', 'CONST_ModuleId',
           'CONST_MethodHandle', 'CONST_MethodType', 'CONST_InvokeDynamic', 'CONST_Module',
           'CONST_Package', 'ACC_PUBLIC', 'ACC_PRIVATE', 'ACC_PROTECTED', 'ACC_STATIC',
           'ACC_FINAL', 'ACC_SYNCHRONIZED', 'ACC_SUPER', 'ACC_VOLATILE', 'ACC_BRIDGE',
           'ACC_TRANSIENT', 'ACC_VARARGS', 'ACC_NATIVE', 'ACC_INTERFACE', 'ACC_ABSTRACT',
           'ACC_STRICT', 'ACC_SYNTHETIC', 'ACC_ANNOTATION', 'ACC_ENUM', 'ACC_MODULE')
JAVA_CLASS_MAGIC = (202, 254, 186, 190)
_BUFFERING = 16384
CONST_Utf8 = 1
CONST_Integer = 3
CONST_Float = 4
...
class JavaConstantPool(object):
    ...
    def __eq__(self, other):
        return isinstance(other, JavaConstantPool) and self.consts == other.consts
    ...
    def deref_const(self, index):
        ...
        if t in (CONST_Utf8, CONST_Integer, CONST_Float,
         CONST_Long, CONST_Double):
            return v
        if t in (CONST_Class, CONST_String, CONST_MethodType):
            return self.deref_const(v)
        if t in (CONST_Fieldref, CONST_Methodref,
...
class JavaClassInfo(object):
    ...
    def unpack(self, unpacker, magic=None):
        ...
        self.fields = tuple(uobjs(JavaMemberInfo, (self.cpool),
          is_method=False))
        self.methods = tuple(uobjs(JavaMemberInfo, (self.cpool),
          is_method=True))
    ...
    def get_method(self, name, arg_types=()):
        ...
        for m in self.get_methods_by_name(name):
            if m.is_bridge() or m.get_arg_type_descriptors() == arg_types:
                return m
...
class JavaMemberInfo(object):
    ...
    def is_synthetic(self):
        ...
        """
        return self.access_flags & ACC_SYNTHETIC or bool(self.get_attribute('Synthetic'))
    ...
    def get_parameter_annotations(self):
        ...
        """
        return self._get_annotations('parameter_annotations', 'RuntimeVisibleParameterAnnotations',
          for_params=True)
    ...
    def get_invisible_parameter_annotations(self):
        ...
        """
        return self._get_annotations('invisible_parameter_annotations', 'RuntimeInvisibleParameterAnnotations',
          for_params=True)
        ...
        for codelet in self.disassemble():
            ...
            if cur_line == abs_line:
                ...
                if cur_line is not None:
                    yield (
                     cur_line, cur_line - lnt_offset, current)
                cur_line = abs_line
                current = [codelet]
...
class JavaExceptionInfo(object):
    ...
    def __eq__(self, other):
        return isinstance(other, JavaExceptionInfo) and self.info() == other.info()
...
class JavaAnnotation(dict):
    ...
    def __eq__(self, other):
        ...
        for key, lval in self.items():
            rval = other[key]
            if not _annotation_val_eq(lval[0], lval[1], self.cpool, rval[0], rval[1], other.cpool):
                return False
...
def _annotation_val_eq(left_tag, left_data, left_cpool, right_tag, right_data, right_cpool):
    ...
    if left_tag == 'e':
        return lconst(left_data[0]) == rconst(right_data[0]) and lconst(left_data[1]) == rconst(right_data[1])
    if left_tag == 'c':
        return lconst(left_data) == rconst(right_data)
    if left_tag == '@':
    ...
    if left_tag == '[':
        ...
        for index in range(0, len(left_data)):
            ld = left_data[index]
            rd = right_data[index]
            if not _annotation_val_eq(ld[0], ld[1], left_cpool, rd[0], rd[1], right_cpool):
                return False
...
_platforms = (((45, 0), (45, 3), '1.0.2'), ((45, 4), (45, 65535), '1.1'), ((46, 0), (46, 65535), '1.2'),
              ((47, 0), (47, 65535), '1.3'), ((48, 0), (48, 65535), '1.4'), ((49, 0), (49, 65535), '1.5'),
              ((50, 0), (50, 65535), '1.6'), ((51, 0), (51, 65535), '1.7'), ((52, 0), (52, 65535), '1.8'))
...
def _unpack_const_item(unpacker):
    ...
    if typecode == CONST_Utf8:
        ...
        if typecode == CONST_Integer:
            val, = unpacker.unpack('>i')
        else:
            if typecode == CONST_Float:
                val, = unpacker.unpack('>f')
            else:
                if typecode == CONST_Long:
                    val, = unpacker.unpack('>q')
                else:
                    if typecode == CONST_Double:
                        val, = unpacker.unpack('>d')
                    else:
                        if typecode in (CONST_Class, CONST_String, CONST_MethodType,
                         CONST_Module, CONST_Package):
                            val, = unpacker.unpack_struct(_H)
                        else:
                            if typecode in (CONST_Fieldref, CONST_Methodref,
                             CONST_InterfaceMethodref, CONST_NameAndType,
                             CONST_ModuleId, CONST_InvokeDynamic):
                                val = unpacker.unpack_struct(_HH)
                            else:
                                if typecode == CONST_MethodHandle:
                                    val = unpacker.unpack_struct(_BH)
                                else:
                                    if typecode == CONST_MethodType:
                                        val = unpacker.unpack_struct(_H)
                                    else:
            ...
            if typecode == CONST_Float:
                ...
                if typecode == CONST_Long:
                    ...
                    if typecode == CONST_Double:
                        ...
                        if typecode == CONST_Class:
                            typestr = 'class'
                            val = '#%i' % val
                        else:
                            if typecode == CONST_String:
                                typestr = 'String'
                                val = '#%i' % val
                            else:
                                if typecode == CONST_Fieldref:
                                    typestr = 'Field'
                                    val = '#%i.#%i' % val
                                else:
                                    if typecode == CONST_Methodref:
                                        typestr = 'Method'
                                        val = '#%i.#%i' % val
                                    else:
                                        if typecode == CONST_InterfaceMethodref:
                                            typestr = 'InterfaceMethod'
                                            val = '#%i.#%i' % val
                                        else:
                                            if typecode == CONST_NameAndType:
                                                typestr = 'NameAndType'
                                                val = '#%i:#%i' % val
                                            else:
                                                if typecode == CONST_ModuleId:
                                                    typestr = 'ModuleId'
                                                    val = '#%i@#%i' % val
                                                else:
                                                    if typecode == CONST_MethodHandle:
                                                        ...
                                                        if typecode == CONST_MethodType:
                                                            ...
                                                            if typecode == CONST_InvokeDynamic:
                                                                typestr = 'InvokeDynamic'
                                                                val = repr(val)
                                                            else:
                                                                if typecode == CONST_Module:
                                                                    typestr = 'Module'
                                                                else:
                                                                    if typecode == CONST_Package:
                                                                        typestr = 'Package'
                                                                    else:
                                                                        raise Unimplemented('unknown constant type %r' % typecode)
    return (
