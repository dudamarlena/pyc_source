...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.7/site-packages/javatools/cheetah/classdiff_MethodAdded.py
...
class classdiff_MethodAdded(change_Change):
    ...
    def details(self, **KWS):
        ...
        _filter = self._CHEETAH__currentFilter
        write('\n<!-- START BLOCK: details -->\n<div class="details">\n<div class="lrdata">\n')
        _v = VFFSL(SL, 'method_table', False)(VFN(VFFSL(SL, 'change', True), 'get_rdata', False)())
        if _v is not None:
            write(_filter(_v, rawExpr='$method_table($change.get_rdata())'))
        write('\n</div>\n</div>\n')
        write('\n<!-- END BLOCK: details -->\n')
        return _dummyTrans and trans.response().getvalue() or ''
...
if not hasattr(classdiff_MethodAdded, '_initCheetahAttributes'):
    templateAPIClass = getattr(classdiff_MethodAdded, '_CHEETAH_templateClass', Template)
