...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/poker/tests.py
# Compiled at: 2014-04-27 22:28:57
import cpoker, poker_lite, poker, utils

def assert_close(a, b, error=0.001):
    assert not abs(a - b) > error


def test_full_enumeration():
    f = lambda *hands: cpoker.full_enumeration([ utils.to_cards(h) for h in hands ])
    map(assert_close, f('3s 2c', '5c 2h'), [0.398847108925, 0.6011528910749999])
    map(assert_close, f('8h Kh', '2c 5h'), [0.665651659986, 0.33434834001400004])
    map(assert_close, f('2s 3s', '5s Js'), [0.329603271382, 0.6703967286179999])
    map(assert_close, f('9c Jc', 'Ts Td'), [0.327120651473, 0.6728793485269999])
    map(assert_close, f('9d Qh', '3s Ac'), [0.426625762715, 0.5733742372849999])
    map(assert_close, f('9c Ac', 'Ah Kh'), [0.288065962586, 0.711934037414])
    map(assert_close, f('8c 4s', 'Qh 9d'), [0.333585332978, 0.6664146670219999])
    map(assert_close, f('Jc 2s', '7s 2d'), [0.730345779721, 0.269654220279])
    map(assert_close, f('Ah 3c', 'Qs 4d'), [0.609246080135, 0.39075391986499997])
    map(assert_close, f('5c 3h', 'Qh 4h'), [0.376223789701, 0.623776210299])
     ...
     (
      [
       'Ad 6s', 'Js 2d', 'Jd 8s', 'Qs Jc', '8c 8d'], '6d Th 7h 8s 9h', [3])]:
        assert cpoker.multi_holdem([ utils.to_cards(h) for h in hands ], utils.to_cards(board)) == poker.multi_holdem([ utils.to_cards(h) for h in hands ], utils.to_cards(board)) == result
