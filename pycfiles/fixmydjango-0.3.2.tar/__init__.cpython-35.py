...
# [GCC 8.4.0]
# Embedded file name: /home/filipeximenes/Projects/fixmydjango-lib/fixmydjango/__init__.py
# Compiled at: 2017-09-13 09:21:44
# Size of source mod 2**32: 5526 bytes
from __future__ import print_function, unicode_literals
import traceback, django
from django.views import debug
from django.utils.http import urlencode
from django.conf import settings
from django.template import Template, Context
from termcolor import colored
from .sanitize_tb import is_django_exception, split_and_strip_tb_lines, extract_traceback_info, clean_traceback, sanitize_traceback, clean_exception_type
...
class ExceptionReporterPatch(original_ExceptionReporter):
    ...
    def _get_fix_my_django_submission_url(self, tb_info, sanitized_tb):
        ...
        return '{url}?{query}'.format(url=url, query=urlencode({'exception_type': clean_exception_type(tb_info['parsed_traceback']['exc_type']), 
    ...
    def get_traceback_data(self):
        ...
        try:
            ...
            if is_django_exception(split_and_strip_tb_lines(tb)):
                ...
                submission_url = self._get_fix_my_django_submission_url(tb_info=tb_info, sanitized_tb=sanitized_tb)
                response = search_exceptions(exception_type=clean_exception_type(tb_info['parsed_traceback']['exc_type']), raised_by=tb_info['raised_by'], raised_by_line=tb_info['parsed_traceback']['frames'][(-1)]['lineno'])
                if response['error_post_list']:
                    url = response['list_url']
                    message = Template(FIX_MY_DJANGO_MESSAGE).render(Context({'url': url, 
                     'errors': self.best_matching_version(response), 
                     'submission_url': submission_url}))
                    plain_message = FIX_MY_DJANGO_MESSAGE_PLAIN.format(url=url)
                else:
                    message = FIX_MY_DJANGO_MESSAGE_TO_ADMIN.format(submission_url=submission_url)
                    plain_message = FIX_MY_DJANGO_MESSAGE_TO_ADMIN_PLAIN.format(submission_url=submission_url)
