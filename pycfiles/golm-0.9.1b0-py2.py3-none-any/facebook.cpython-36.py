...
# [GCC 8.4.0]
# Embedded file name: /Users/prihodad/Documents/projects/visitor/golm/golm/core/interfaces/facebook.py
# Compiled at: 2018-04-15 15:28:05
# Size of source mod 2**32: 13435 bytes
...
class FacebookInterface:
    ...
    def get_page_token(page_id):
        ...
        if 'FB_PAGE_TOKEN' in settings.GOLEM_CONFIG:
    ...
    def load_profile(user_id, page_id, cache=True):
        ...
        if not cache or not db.exists(key):
            ...
            if not res.status_code == requests.codes.ok:
                ...
                print(res)
                return {}
            db.set(key, (json.dumps(res.json())), ex=1209600)
    ...
    def post_message(session: ChatSession, response):
        ...
        if isinstance(response, SenderActionMessage):
            ...
            response_dict = {'sender_action':response.action, 
             'recipient':{'id': fbid}}
        else:
    ...
    def _do_post(request_mode, response_dict, page_id):
        ...
        r = requests.post(post_message_url, headers={'Content-Type': 'application/json'},
    ...
    def to_setting(response):
        if isinstance(response, GreetingSetting):
            return {'greeting':{'text': response.message},  'setting_type':'greeting'}
        else:
            if isinstance(response, GetStartedSetting):
                return {'call_to_actions':[{'payload': json.dumps((response.payload), default=json_serialize)}],  'setting_type':'call_to_actions', 
                 'thread_state':'new_thread'}
            if isinstance(response, MenuSetting):
                return {'call_to_actions':[FacebookInterface.to_setting(element) for element in response.elements[:10]],  'setting_type':'call_to_actions', 
                 'thread_state':'existing_thread'}
            if isinstance(response, MenuElement):
                r = {'title':response.title,  'type':response.type}
                if response.payload:
    ...
    def to_message(response):
        if isinstance(response, TextMessage):
            ...
            if isinstance(response, GenericTemplateMessage):
                ...
                if isinstance(response, AttachmentMessage):
                    return {'attachment': {'type':response.attachment_type, 
                                    'payload':{'url': response.url}}}
                else:
                    if isinstance(response, GenericTemplateElement):
                        message = {'title':response.title,  'image_url':response.image_url, 
                ...
                if isinstance(response, Button):
                    return response.to_response()
            if isinstance(response, ListTemplate):
                return response.to_response()
        raise ValueError('Error: Invalid message type: {}: {}'.format(type(response), response))
                    ...
                    if 'quick_reply' in raw_message['message']:
                        ...
                        if payload:
                            payload['_message_text'] = [
                             {'value': raw_message['message']['text']}]
                            return {'entities':payload, 
                             'type':'postback'}
    ...
    def parse_sticker(sticker_id):
        if sticker_id in (369239383222810, 369239343222814, 369239263222822):
            ...
            return {'entities':{'sticker_id':sticker_id, 
              '_message_text':None}, 
             'type':'message'}

    @staticmethod
    def parse_attachments(attachments):
        entities = {'intent':[],  'current_location':[],  'attachment':[],  '_message_text':[
          {'value': None}]}
