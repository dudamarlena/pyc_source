...
# [GCC 8.4.0]
# Embedded file name: /Users/prihodad/Documents/projects/visitor/golm/golm/core/interfaces/microsoft.py
# Compiled at: 2018-04-15 15:28:28
...
class MicrosoftInterface:
    ...
    def get_auth_token():
        ...
        if redis.exists('ms_token'):
            ...
            payload = 'grant_type=client_credentials' + '&client_id=' + settings.GOLEM_CONFIG.get('MS_BOT_ID') + '&client_secret=' + settings.GOLEM_CONFIG.get('MS_BOT_TOKEN') + '&scope=' + 'https://api.botframework.com/.default'
    ...
    def processing_start(uid, chat_id):
        ...
        chat_id = str(chat_id)
        payload = {'type':'typing', 
         'from':{'id': MicrosoftInterface.get_bot_id(chat_id)}, 
         'conversation':{'id': chat_id}}
        url = MicrosoftInterface.get_base_url(chat_id) + 'conversations/' + chat_id + '/activities'
        headers = {'Authorization': 'Bearer ' + MicrosoftInterface.get_auth_token()}
        response = requests.post(url, data=(json.dumps(payload)), headers=headers)
    ...
    def accept_request(body, num_tries=1) -> bool:
        if body['type'] == 'message':
            ...
            get_redis().set('chat_message_id:{}'.format(chat_id), body['id'])
