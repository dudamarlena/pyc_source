...
# [GCC 8.4.0]
# Embedded file name: /Users/prihodad/Documents/projects/visitor/golm/golm/core/nlp/tools/toktok.py
...
class ToktokTokenizer:
    ...
    CURRENCY_SYM = str('$¢£¤¥֏؋৲৳৻૱௹฿៛₠₡₢₣₤₥₦₧₨₩₪₫€₭₮₯₰₱₲₳₴₵₶₷₸₹₺꠸﷼﹩＄￠￡￥￦')
    OPEN_PUNCT_RE = (
     re.compile('([{}])'.format(OPEN_PUNCT)), '\\1 ')
    CLOSE_PUNCT_RE = (
     re.compile('([{}])'.format(CLOSE_PUNCT)), '\\1 ')
    CURRENCY_SYM_RE = (
     re.compile('([{}])'.format(CURRENCY_SYM)), '\\1 ')
    URL_FOE_1 = (
     re.compile(':(?!//)'), ' : ')
