...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/ccb/Transaction.py
...
class Transaction(Base):
    SPECIALATTRS = [
     'TRANSACTION_TYPE', 'JIVE', 'ACCOUNT_ID', 'CATEGORY_ID']
    ...
    def commit(self):
        if self.transaction_type == 'transfer':
            try:
                ...
                self.attributes += ['from_account_id', 'to_account_id']
            except:
                raise TransactionError('"from_account_id" and "to_account_id" are required for transaction transfers')
