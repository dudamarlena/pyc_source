...
# Embedded file name: build/bdist.linux-x86_64/egg/pysickle/pysickle.py
# Compiled at: 2014-05-21 02:56:38
...
import sys, getopt, subprocess, threading, os, shutil, matplotlib
matplotlib.use('Agg')
...
def checkMode(mode):
    avail = [
     'sites', 'gaps', 'ugaps', 'insertions', 'uinsertions', 'uinsertionsgaps', 'custom']
...
def schoenify(fasta=None, max_iter=None, finaldir=None, tmpdir=None, msa_tool=None, mode=None, logging=None, gap_penalty=None, unique_gap_penalty=None, insertion_penalty=None, unique_insertion_penalty=None, mismatch_penalty=None, match_reward=None):
    if not fasta:
        ...
        iterTab = []
        headerTab = [
         'matches', 'matchesWithGaps', 'mismatches', ' nogap', 'gaps', 'length', 'iteration', 'numSeq']
        alignmentstats = []
        ...
        ax.legend(handles, labels, bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.0)
        ax = plt.subplot(3, 1, 2)
        plt.plot(arr[:, 6], arr[:, 7])
        ax.set_ylabel('count')
        ax.legend(['numSeq'], bbox_to_anchor=(1.05, 0.3), loc=2, borderaxespad=0.0)
        ...
        try:
            try:
                ...
                ax.legend(['(length-gaps)*numSeq'], bbox_to_anchor=(1.05, 0.3), loc=2, borderaxespad=0.0)
