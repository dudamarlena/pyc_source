...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/dhcpkit_looking_glass/models.py
# Compiled at: 2016-07-18 16:55:02
...
class Client(models.Model):
    ...
    duid = models.CharField(_('DUID'), max_length=258)
...
class Transaction(models.Model):
    ...
    def __str__(self):
        return '{client} -> {server} @ {ts:%Y-%m-%d %H:%H:%S.%f}'.format(client=self.client, server=self.server, ts=self.request_ts)
