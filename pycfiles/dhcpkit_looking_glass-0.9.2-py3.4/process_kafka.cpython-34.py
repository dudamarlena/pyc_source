...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/dhcpkit_looking_glass/management/commands/process_kafka.py
# Compiled at: 2016-08-05 10:20:37
# Size of source mod 2**32: 10949 bytes
"""
Process to read Kafka messages and save a summary to the database
"""
import codecs, datetime, json, logging, re, signal, socket, time
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: ArgumentParser):
        ...
        """
        parser.add_argument('-b', '--brokers', nargs='+', metavar='HOST:PORT', default=['localhost:9092'], help='The Kafka brokers to bootstrap from')
        parser.add_argument('-s', '--source-address', help='Source address to use when connecting to Kafka')
        parser.add_argument('-t', '--topic', default='dhcpkit.messages', help='The topic to subscribe to')
        parser.add_argument('-g', '--consumer-group', default='dhcpkit_looking_glass', help='The consumer group we belong to, for progress tracking')
        parser.add_argument('--from-beginning', action='store_true', help='Start processing messages from the beginning instead of continuing')
        parser.add_argument('--skip-backlog', action='store_true', help='Ignore old messages instead of continuing where we left off')
    ...
    def handle(self, *args, **options):
        ...
        while not stopping:
            try:
                kafka = pykafka.KafkaClient(hosts=','.join(map(_fix_broker, options['brokers'])), source_address=options['source_address'] or '')
                kafka_topic = kafka.topics[options['topic'].encode('ascii')]
                kafka_consumer = kafka_topic.get_balanced_consumer(consumer_group=options['consumer_group'].encode('ascii'), auto_commit_enable=True, auto_offset_reset=OffsetType.LATEST if options['skip_backlog'] else OffsetType.EARLIEST, reset_offset_on_start=options['from_beginning'] or options['skip_backlog'], consumer_timeout_ms=1000)
                while not stopping:
    ...
    def get_transaction_info(message: DHCPKafkaMessage) -> Dict[(str, object)]:
        ...
        if remote_id_obj:
            try:
                ...
                remote_id = '{}:0x{}'.format(remote_id_obj.enterprise_number, codecs.encode(remote_id_obj.remote_id, 'hex').decode('ascii'))
    ...
    def process_message(self, message: DHCPKafkaMessage):
        ...
        client, created = Client.objects.get_or_create(duid=info['duid'], interface_id=info['interface_id'], remote_id=info['remote_id'])
        if created:
        ...
        request = json.dumps(message.message_in, cls=JSONProtocolElementEncoder) if message.message_in else ''
        response_ts = datetime.datetime.utcfromtimestamp(message.timestamp_out).replace(tzinfo=pytz.utc)
        response = json.dumps(message.message_out, cls=JSONProtocolElementEncoder) if message.message_out else ''
        Transaction.objects.update_or_create(client=client, server=server, request_ts=request_ts, defaults={'request_type': info['request_type'], 
         'request': request, 
