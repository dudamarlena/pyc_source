...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/leekspin/generator.py
...
import base64, hashlib, logging, random, re, sys, os, traceback
try:
...
def generateDescriptors(bridge=True, withoutTAP=False, withoutNTOR=False):
    ...
    extrainfoDoc = extrainfo.generateExtraInfo(nick, fingerprintSmooshed, timestamp, ipv4, port, bridge=bridge)
    extrainfoDigestBinary, extrainfoDigest, extrainfoDigestPKCS1 = crypto.digestDescriptorContent(extrainfoDoc)
    extrainfoDesc = crypto.signDescriptorContent(extrainfoDoc, secretSigningKey, digest=extrainfoDigestPKCS1)
    serverDoc = server.generateServerDescriptor(nick, fingerprintSpacey, timestamp, ipv4, ipv6, port, vers, protocols, uptime, bandwidth, extrainfoDigest, onionKeyLine, signingKeyLine, publicNTORKey, bridge=bridge)
    serverDigestBinary, serverDigest, serverDigestPKCS1 = crypto.digestDescriptorContent(serverDoc)
    if bridge:
        serverDoc = '@purpose bridge\n' + serverDoc
    serverDesc = crypto.signDescriptorContent(serverDoc, secretSigningKey, digest=serverDigestPKCS1)
    netstatusDesc = netstatus.generateBridgeNetstatus(nick, fingerprintBinary, serverDigestBinary, timestamp, ipv4, port, ipv6=ipv6, bandwidth_line=bandwidth)
    return (
     extrainfoDesc, serverDesc, netstatusDesc)


def generateHSDesc(replica):
    u"""Generate a ``@type rendezvous-service-descriptor`` for an Hidden
    Service.
    ...
    secretPermanentKey, publicPermanentKey, permanentKeyLine = rendezvous.generatePermanentKey()
    secretSigningKey, publicSigningKey, signingKeyLine = crypto.generateSigningKey()
    permanentID = rendezvous.generatePermanentID(publicPermanentKey)
    descCookie = rendezvous.createDescriptorCookie()
    descCookieB64 = base64.b64encode(descCookie)
    ...
    secretIDPart, secretIDLine = rendezvous.calculateSecretIDPart(permanentID, currentTime, descCookieB64, replica)
    introductionPoints = rendezvous.generateIntroPoints(descCookieB64)
    rendServiceLine = rendezvous.generateRendServiceLine(permanentID, secretIDPart, replica)
    d = []
    ...
    descriptor = crypto.signDescriptorContent(document, secretSigningKey, token=const.TOKEN_HS_SIGNATURE)
...
def createHiddenServiceDescriptors(count, replicas=2):
    ...
    try:
        ...
        descriptorFiles = {'rendezvous-service-descriptors': ('\n').join(rendDescriptors)}
...
def createRelayOrBridgeDescriptors(count, bridge=True, **kwargs):
    ...
    try:
        try:
            netstatus_consensus.append('flag-thresholds stable-uptime=613624 stable-mtbf=2488616 fast-speed=15000 guard-wfu=98.000% guard-tk=691200 guard-bw-inc-exits=55000 guard-bw-exc-exits=55000 enough-mtbf=1 ignoring-advertised-bws=0\n')
            for i in xrange(int(count)):
                try:
                    extrainfo, server, netstatus = generateDescriptors(bridge=bridge, withoutTAP=withoutTAP, withoutNTOR=withoutNTOR)
                except Exception as error:
        ...
        if bridge:
            descriptorFiles = {'networkstatus-bridges': ('').join(netstatus_consensus), 'bridge-descriptors': ('').join(server_descriptors)}
        else:
            descriptorFiles = {'cached-consensus': ('').join(netstatus_consensus), 
...
def create(count, descriptorType=None, withoutTAP=False, withoutNTOR=False):
    ...
    if descriptorType in ('bridge', 'relay'):
        ...
        createRelayOrBridgeDescriptors(count, bridge=bridge, withoutTAP=withoutTAP, withoutNTOR=withoutNTOR)
