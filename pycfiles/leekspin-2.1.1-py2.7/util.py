...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/leekspin/util.py
...
import argparse, ipaddr, random, sys, time
from leekspin import __version__
from leekspin import __package__

def getArgParser():
    """Get our :class:`~argparse.ArgumentParser`."""
    version_ = (b'-').join([__package__, __version__.rsplit(b'_', 1)[0]])
    parser = argparse.ArgumentParser(prog=__package__, add_help=True)
    parser.prog = __package__
    parser.version = version_
    parser.description = b'Generate a signed set of network-status, '
    parser.description += b'extra-info, and server descriptor documents '
    parser.description += b'for mock Tor relays or bridges.'
    otherargs = parser.add_mutually_exclusive_group()
    infoargs = otherargs.add_mutually_exclusive_group()
    infoargs.add_argument(b'-v', b'--verbose', action=b'store_true', help=b'print information to stdout')
    infoargs.add_argument(b'-q', b'--quiet', action=b'store_true', help=b"don't print anything")
    versionargs = parser.add_mutually_exclusive_group()
    versionargs.add_argument(b'--version', action=b'store_true', help=b'print leekspin version and exit')
    descgroup = parser.add_argument_group()
    descgroup.title = b'descriptor types'
    descgroup.argument_default = b'--bridge'
    descgroup1 = descgroup.add_mutually_exclusive_group()
    descgroup1.add_argument(b'-r', b'--relay', action=b'store_true', help=b'generate relay descriptors')
    descgroup1.add_argument(b'-b', b'--bridge', action=b'store_true', help=b'generate bridge descriptors')
    descgroup1.add_argument(b'-hs', b'--hidden-service', action=b'store_true', help=b'generate HS rendezvous descriptors')
    descgroup1.set_defaults(relay=False, bridge=True, hidden_service=False)
    descgroup2 = parser.add_argument_group()
    descgroup2.title = b'descriptor variations'
    descgroup2.add_argument(b'-xt', b'--without-tap', action=b'store_true', help=b'generate descriptors without TAP support, e.g. without RSA keys')
    descgroup2.add_argument(b'-xn', b'--without-ntor', action=b'store_true', help=b'generate descriptors without ntor support, e.g. without Ed25519 keys')
    descgroup2.set_defaults(without_tap=False, without_ntor=False)
    group = parser.add_argument_group()
    group.title = b'required arguments'
    group.add_argument(b'-n', b'--descriptors', default=0, help=b'generate <n> descriptor sets', type=int)
    return parser
...
def _checkIPValidity(ip):
    ...
    """
    if ip.is_link_local or ip.is_loopback or ip.is_multicast or ip.is_private or ip.is_unspecified or ip.version == 6 and ip.is_site_local or ip.version == 4 and ip.is_reserved:
        return False
    return True
...
def getHexString(size):
    ...
    for _ in xrange(size):
        hexstr += random.choice(b'ABCDEF0123456789')
...
def makeTimeStamp(now=None, fmt=None, variation=False, period=None):
    ...
    fmt = fmt if fmt else b'%Y-%m-%d %H:%M:%S'
    if variation:
        then = 1
...
def writeDescToFile(filename, descriptors):
    ...
    descript = descriptors.encode(encoding, b'replace')
    try:
        with open(filename, b'wb', encoding=encoding, errors=b'replace') as (fh):
            fh.write(descript)
            fh.flush()
    except (IOError, OSError) as err:
        print(b"Failure while attempting to write descriptors to file '%s': %s" % (
