...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/leekspin/netstatus.py
...
def generateBridgeNetstatus(nickname, idkey_digest, server_desc_digest, timestamp, ipv4, orport, ipv6=None, dirport=None, flags=b'Fast Guard Running Stable Valid', bandwidth_line=None):
    ...
    idb64 = str(idkey_b64).strip().rstrip(b'==')
    server_b64 = binascii.b2a_base64(server_desc_digest)
    srvb64 = str(server_b64).strip().rstrip(b'==')
    if bandwidth_line is not None:
        bw = int(bandwidth_line.split()[(-1)]) / 1024
