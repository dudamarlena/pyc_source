...
# [GCC 8.4.0]
# Embedded file name: /home/swails/src/ParmEd/parmed/geometry.py
# Compiled at: 2018-02-27 09:47:24
# Size of source mod 2**32: 12872 bytes
"""
                ...
                if u.is_quantity(alpha):
                    ...
                    if u.is_quantity(beta):
                        ...
                        if alpha <= 2 * pi:
                            if beta <= 2 * pi:
                                if gamma <= 2 * pi:
                                    warnings.warn('Strange unit cell vector angles detected. They appear to be in radians...')
                        alpha *= DEG_TO_RAD
                        beta *= DEG_TO_RAD
                        gamma *= DEG_TO_RAD
                        av = [
...
def box_vectors_to_lengths_and_angles(a, b, c):
    ...
    return (
     (
      la, lb, lc) * u.angstroms, (alpha, beta, gamma) * u.degrees)
...
STANDARD_BOND_LENGTHS_SQUARED.update({(1, 1): (0.74 + _OFFSET) ** 2, 
 (6, 6): (1.54 + _OFFSET) ** 2, 
 (7, 7): (1.45 + _OFFSET) ** 2, 
 (8, 8): (1.48 + _OFFSET) ** 2, 
 ...
 (7, 8): (1.4 + _OFFSET) ** 2, (8, 7): (1.4 + _OFFSET) ** 2, 
