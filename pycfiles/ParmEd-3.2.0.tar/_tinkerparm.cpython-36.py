...
# [GCC 8.4.0]
# Embedded file name: /home/swails/src/ParmEd/parmed/amber/_tinkerparm.py
# Compiled at: 2017-03-17 13:53:52
# Size of source mod 2**32: 57077 bytes
"""
...
from ..topologyobjects import Angle, AngleType, Bond, BondType, ChiralFrame, Dihedral, DihedralType, MultipoleFrame, NonbondedException, OutOfPlaneBend, OutOfPlaneBendType, PiTorsion, StretchBend, StretchBendType, TorsionTorsion, TorsionTorsionType, TrigonalAngle, UreyBradley
...
class AmoebaParm(AmberParm):
    ...
    def _load_bond_info(self):
        ...
        if 'AMOEBA_REGULAR_BOND_LIST' not in self.parm_data:
            return
        data = self.parm_data
        del self.bonds[:]
        del self.bond_types[:]
        for k, req in zip(data['AMOEBA_REGULAR_BOND_FORCE_CONSTANT'], data['AMOEBA_REGULAR_BOND_EQUIL_VALUE']):
        ...
        if len(self.bond_types.coeffs) != self.bond_types.degree + 1:
            raise AmberError('Bond degree (%d) does not make sense with %d coefficients' % (
             self.bond_types.degree,
             len(self.bond_types.coeffs)))
        it = iter(self.parm_data['AMOEBA_REGULAR_BOND_LIST'])
        for i, j, k in zip(it, it, it):
            self.bonds.append(Bond(self.atoms[(i - 1)], self.atoms[(j - 1)], self.bond_types[(k - 1)]))

    def _load_angle_info(self):
        """ Load the regular AMOEBA angles, if they exist """
        if 'AMOEBA_REGULAR_ANGLE_LIST' not in self.parm_data:
            return
        data = self.parm_data
        del self.angles[:]
        del self.angle_types[:]
        for k, eq in zip(data['AMOEBA_REGULAR_ANGLE_FORCE_CONSTANT'], data['AMOEBA_REGULAR_ANGLE_EQUIL_VALUE']):
        ...
        if len(self.angle_types.coeffs) != self.angle_types.degree + 1:
            raise AmberError('Angle degree (%d) does not make sense with %d coefficients' % (
             self.angle_types.degree,
             len(self.angle_types.coeffs)))
        it = iter(data['AMOEBA_REGULAR_ANGLE_LIST'])
        for i, j, k, l in zip(it, it, it, it):
            self.angles.append(Angle(self.atoms[(i - 1)], self.atoms[(j - 1)], self.atoms[(k - 1)], self.angle_types[(l - 1)]))

    def _load_urey_bradley_info(self):
        """ Loads the AMOEBA Urey-Bradley terms, if they exist """
        if 'AMOEBA_UREY_BRADLEY_BOND_LIST' not in self.parm_data:
            return
        data = self.parm_data
        del self.urey_bradleys[:]
        del self.urey_bradley_types[:]
        for k, eq in zip(data['AMOEBA_UREY_BRADLEY_BOND_FORCE_CONSTANT'], data['AMOEBA_UREY_BRADLEY_BOND_EQUIL_VALUE']):
            self.urey_bradley_types.append(BondType(k, eq, self.urey_bradley_types))

        self.urey_bradley_types.degree = degree = data['AMOEBA_UREY_BRADLEY_BOND_FTAB_DEGREE'][0]
        self.urey_bradley_types.coeffs = coeffs = data['AMOEBA_UREY_BRADLEY_BOND_FTAB_COEFFS'][:]
        if len(coeffs) != degree + 1:
            raise AmberError('Urey-Bradley degree (%d) does not make sense with %d coefficients' % (
             degree, len(coeffs)))
    ...
    def _load_trigonal_angle_info(self):
        ...
        if 'AMOEBA_TRIGONAL_ANGLE_LIST' not in self.parm_data:
            return
        data = self.parm_data
        del self.trigonal_angles[:]
        del self.trigonal_angle_types[:]
        for k, eq in zip(data['AMOEBA_TRIGONAL_ANGLE_FORCE_CONSTANT'], data['AMOEBA_TRIGONAL_ANGLE_EQUIL_VALUE']):
            self.trigonal_angle_types.append(AngleType(k, eq, self.trigonal_angle_types))

        self.trigonal_angle_types.degree = degree = data['AMOEBA_TRIGONAL_ANGLE_FTAB_DEGREE'][0]
        self.trigonal_angle_types.coeffs = coeffs = data['AMOEBA_TRIGONAL_ANGLE_FTAB_COEFFS'][:]
        if len(coeffs) != degree + 1:
            raise AmberError('Trigonal Angle degree (%d) does not make sense with %d coefficients' % (
             degree, len(coeffs)))
        it = iter(data['AMOEBA_TRIGONAL_ANGLE_LIST'])
        for i, j, k, l, m in zip(it, it, it, it, it):
            self.trigonal_angles.append(TrigonalAngle(self.atoms[(i - 1)], self.atoms[(j - 1)], self.atoms[(k - 1)], self.atoms[(l - 1)], self.trigonal_angle_types[(m - 1)]))

    def _load_oopbend_info(self):
        """ Loads the AMOEBA out-of-plane bending terms, if they exist """
        if 'AMOEBA_OPBEND_ANGLE_LIST' not in self.parm_data:
            return
        data = self.parm_data
        del self.out_of_plane_bends[:]
        del self.out_of_plane_bend_types[:]
        for k in data['AMOEBA_OPBEND_ANGLE_FORCE_CONSTANT']:
            self.out_of_plane_bend_types.append(OutOfPlaneBendType(k, self.out_of_plane_bend_types))

        self.out_of_plane_bend_types.degree = degree = data['AMOEBA_OPBEND_ANGLE_FTAB_DEGREE'][0]
        self.out_of_plane_bend_types.coeffs = coeffs = data['AMOEBA_OPBEND_ANGLE_FTAB_COEFFS'][:]
        if len(coeffs) != degree + 1:
            raise AmberError('OOP-bend angle degree (%d) does not make sense with %d coefficients.' % (
             degree, len(coeffs)))
        it = iter(data['AMOEBA_OPBEND_ANGLE_LIST'])
        for i, j, k, l, m in zip(it, it, it, it, it):
            self.out_of_plane_bends.append(OutOfPlaneBend(self.atoms[(i - 1)], self.atoms[(j - 1)], self.atoms[(k - 1)], self.atoms[(l - 1)], self.out_of_plane_bend_types[(m - 1)]))

    def _load_dihedral_info(self):
        """ Loads the AMOEBA regular torsion terms, if they exist """
        if 'AMOEBA_TORSION_LIST' not in self.parm_data:
            return
        data = self.parm_data
        del self.dihedrals[:]
        del self.dihedral_types[:]
        for k, per, phase in zip(data['AMOEBA_TORSION_FORCE_CONSTANT'], data['AMOEBA_TORSION_PERIODICITY'], data['AMOEBA_TORSION_PHASE']):
            self.dihedral_types.append(DihedralType(k, per, (phase * RAD_TO_DEG), list=(self.dihedral_types)))

        it = iter(data['AMOEBA_TORSION_LIST'])
        for i, j, k, l, m in zip(it, it, it, it, it):
            self.dihedrals.append(Dihedral((self.atoms[(i - 1)]), (self.atoms[(j - 1)]), (self.atoms[(k - 1)]), (self.atoms[(l - 1)]),
              type=(self.dihedral_types[(m - 1)])))

    def _load_pitorsion_info(self):
        """ Loads the AMOEBA pi-torsion terms, if they exist """
        if 'AMOEBA_PI_TORSION_LIST' not in self.parm_data:
            return
        data = self.parm_data
        del self.pi_torsions[:]
        del self.pi_torsion_types[:]
        for k, per, phase in zip(data['AMOEBA_PI_TORSION_FORCE_CONSTANT'], data['AMOEBA_PI_TORSION_PERIODICITY'], data['AMOEBA_PI_TORSION_PHASE']):
            self.pi_torsion_types.append(DihedralType(k, per, (phase * RAD_TO_DEG), list=(self.pi_torsion_types)))

        it = iter(data['AMOEBA_PI_TORSION_LIST'])
        for i, j, k, l, m, n, o in zip(it, it, it, it, it, it, it):
            self.pi_torsions.append(PiTorsion(self.atoms[(i - 1)], self.atoms[(j - 1)], self.atoms[(k - 1)], self.atoms[(l - 1)], self.atoms[(m - 1)], self.atoms[(n - 1)], self.pi_torsion_types[(o - 1)]))

    def _load_stretch_bend_info(self):
        """ Loads the AMOEBA stretch-bend terms, if they exist """
        if 'AMOEBA_STRETCH_BEND_LIST' not in self.parm_data:
            return
        data = self.parm_data
        del self.stretch_bends[:]
        del self.stretch_bend_types[:]
        if 'AMOEBA_STRETCH_BEND_FORCE_CONSTANT' in data:
            for a, b, c, d in zip(data['AMOEBA_STRETCH_BEND_FORCE_CONSTANT'], data['AMOEBA_STRETCH_BEND_BOND1_EQUIL_VALUE'], data['AMOEBA_STRETCH_BEND_BOND2_EQUIL_VALUE'], data['AMOEBA_STRETCH_BEND_ANGLE_EQUIL_VALUE']):
                self.stretch_bend_types.append(StretchBendType(a, a, b, c, d, list=(self.stretch_bend_types)))

        else:
            if 'AMOEBA_STRETCH_BEND_FORCE_CONSTANT_1' in data:
                for a, b, c, d, e in zip(data['AMOEBA_STRETCH_BEND_FORCE_CONSTANT_1'], data['AMOEBA_STRETCH_BEND_FORCE_CONSTANT_2'], data['AMOEBA_STRETCH_BEND_BOND1_EQUIL_VALUE'], data['AMOEBA_STRETCH_BEND_BOND2_EQUIL_VALUE'], data['AMOEBA_STRETCH_BEND_ANGLE_EQUIL_VALUE']):
                    self.stretch_bend_types.append(StretchBendType(a, b, c, d, e, list=(self.stretch_bend_types)))

        it = iter(data['AMOEBA_STRETCH_BEND_LIST'])
        for i, j, k, l in zip(it, it, it, it):
            self.stretch_bends.append(StretchBend(self.atoms[(i - 1)], self.atoms[(j - 1)], self.atoms[(k - 1)], self.stretch_bend_types[(l - 1)]))

    def _load_torsion_torsion_info(self):
        """ Loads the AMOEBA coupled torsion-torsion terms, if they exist """
        if 'AMOEBA_TORSION_TORSION_LIST' not in self.parm_data:
            return
        ...
        for i in range(ntypes):
            ...
            self.torsion_torsion_types.append(TorsionTorsionType(dims, ang1, ang2, f, dfda1, dfda2,
              d2fda1da2, list=(self.torsion_torsion_types)))

        it = iter(data['AMOEBA_TORSION_TORSION_LIST'])
        for i, j, k, l, m, n in zip(it, it, it, it, it, it):
            self.torsion_torsions.append(TorsionTorsion(self.atoms[(i - 1)], self.atoms[(j - 1)], self.atoms[(k - 1)], self.atoms[(l - 1)], self.atoms[(m - 1)], self.torsion_torsion_types[(n - 1)]))
    ...
    def _load_frame_info(self):
        ...
        if 'AMOEBA_CHIRAL_FRAME_LIST' in data:
            ...
            for i, j, k in zip(it, it, it):
                self.chiral_frames.append(ChiralFrame(self.atoms[(i - 1)], self.atoms[(j - 1)], k))

        del self.multipole_frames[:]
        if 'AMOEBA_FRAME_DEF_LIST' in data:
            it = iter(data['AMOEBA_FRAME_DEF_LIST'])
            for i, j, k, l, m in zip(it, it, it, it, it):
                self.multipole_frames.append(MultipoleFrame(self.atoms[(i - 1)], j, k, l, m))
    ...
    def _load_exception_info(self):
        ...
        del self.adjusts[:]
        data = self.parm_data
        for a, b, c, d, e in zip(data['AMOEBA_ADJUST_VDW_WEIGHTS_LIST'], data['AMOEBA_ADJUST_MPOLE_WEIGHTS_LIST'], data['AMOEBA_ADJUST_DIRECT_WEIGHTS_LIST'], data['AMOEBA_ADJUST_POLAR_WEIGHTS_LIST'], data['AMOEBA_ADJUST_MUTUAL_WEIGHTS_LIST']):
            self.adjust_types.append(NonbondedExceptionType(a, b, c, d, e, list=(self.adjust_types)))

        it = iter(data['AMOEBA_ADJUST_LIST'])
        for i, j, k in zip(it, it, it):
            self.adjusts.append(NonbondedException(self.atoms[(i - 1)], self.atoms[(j - 1)], self.adjust_types[(k - 1)]))
    ...
    def _xfer_atom_info(self):
        ...
        data['CHARGE'] = [0.0 for a in self.atoms]
    ...
    def _xfer_stretch_bend_info(self):
        ...
        data['AMOEBA_STRETCH_BEND_FORCE_CONSTANT'] = [strbnd.k1 for strbnd in self.stretch_bend_types]
        data['AMOEBA_STRETCH_BEND_BOND1_EQUIL_VALUE'] = [strbnd.req1 for strbnd in self.stretch_bend_types]
        data['AMOEBA_STRETCH_BEND_BOND2_EQUIL_VALUE'] = [strbnd.req2 for strbnd in self.stretch_bend_types]
        data['AMOEBA_STRETCH_BEND_ANGLE_EQUIL_VALUE'] = [strbnd.theteq for strbnd in self.stretch_bend_types]
    ...
    def _xfer_torsion_torsion_info(self):
        ...
        if len(self.torsion_torsions) == 0:
            delete_flags = set(flag for flag in self.flag_list if flag.startswith('AMOEBA_TORSION_TORSION'))
        ...
        delete_flags = set(flag for flag in self.flag_list if flag.startswith('AMOEBA_TORSION_TORSION_TORTOR_TABLE'))
        for flag in delete_flags:
            self.delete_flag(flag)

        for i, tt in enumerate(self.torsion_torsion_types):
            tblsize = [
             'dimension = (%d,%d)' % tt.dims]
            prefix = 'AMOEBA_TORSION_TORSION_TORTOR_TABLE_%02d_' % (i + 1)
            self.add_flag((prefix + 'DIMS'), '2I8', data=(list(tt.dims)), comments=[
             'dimension = (2)'],
              after=after)
            self.add_flag((prefix + 'ANGLE1'), '5E16.8', data=(tt.ang1[:]), comments=[
             'dimension = (%d)' % tt.dims[0]],
              after=(prefix + 'DIMS'))
            self.add_flag((prefix + 'ANGLE2'), '5E16.8', data=(tt.ang2[:]), comments=[
             'dimension = (%d)' % tt.dims[1]],
              after=(prefix + 'ANGLE1'))
            self.add_flag((prefix + 'FUNC'), '5E16.8', data=(tt.f.data[:]), comments=(tblsize[:]),
              after=(prefix + 'ANGLE2'))
        ...
        data['AMOEBA_TORSION_TORSION_NUM_LIST'] = [
         len(self.torsion_torsions)]
    ...
    def _xfer_frame_info(self):
        ...
        if len(self.chiral_frames) > 0:
            data['AMOEBA_CHIRAL_FRAME_NUM_LIST'] = [
             len(self.chiral_frames)]
        ...
        if len(self.multipole_frames) > 0:
            data['AMOEBA_FRAME_DEF_NUM_LIST'] = [
             len(self.multipole_frames)]
...
class BeemanRestart(AmberFormat):
    ...
    def coordinates(self, value):
        ...
        if value.shape != (3 * self.natom,):
            raise ValueError('Require %d-length sequence for coordinates' % (3 * self.natom))
    ...
    def velocities(self, value):
        value = np.asarray(value).flatten()
        if value.shape != (3 * self.natom,):
            raise ValueError('Require %d-length sequence for velocities' % (3 * self.natom))
        else:
            if 'ATOMIC_VELOCITIES_LIST' not in self.flag_list:
    ...
    def accelerations(self):
        try:
            return np.array(self.parm_data['ATOMIC_ACCELERATIONS_LIST']).reshape((
             1, self.natom, 3))
        except KeyError:
    ...
    def accelerations(self, value):
        value = np.asarray(value).flatten()
        if value.shape != (3 * self.natom,):
            raise ValueError('Require %d-length sequence for accelerations' % 3 * self.natom)
        else:
            if 'ATOMIC_ACCELERATIONS_LIST' not in self.flag_list:
                self.add_flag('ATOMIC_ACCELERATIONS_NUM_LIST', 'i8', data=[
                 self.natom])
    ...
    def old_accelerations(self):
        try:
            return np.array(self.parm_data['OLD_ATOMIC_ACCELERATIONS_LIST']).reshape((
             1, self.natom, 3))
        except KeyError:
    ...
    def old_accelerations(self, value):
        ...
        if value.shape != (3 * self.natom,):
            ...
            if 'OLD_ATOMIC_ACCELERATIONS_LIST' not in self.flag_list:
                self.add_flag('OLD_ATOMIC_ACCELERATIONS_NUM_LIST', 'i8', data=[
                 self.natom])
