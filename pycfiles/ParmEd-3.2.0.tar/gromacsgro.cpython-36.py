...
# [GCC 8.4.0]
# Embedded file name: /home/swails/src/ParmEd/parmed/gromacs/gromacsgro.py
# Compiled at: 2019-03-19 14:47:46
# Size of source mod 2**32: 14288 bytes
"""
...
from parmed.geometry import box_vectors_to_lengths_and_angles, box_lengths_and_angles_to_vectors, reduce_box_vectors
...
class _AtomLineParser(object):
    __doc__ = ' Parses atom lines from GRO files '
    ...
    def read(self, line):
        ...
        if atomic_number == 0:
            ...
            atom = Atom(atomic_number=atomic_number, name=atomname, number=atnum,
              mass=mass)
        if self._digits is None:
            self._pdeci = line.index('.', 20)
            self._ndeci = line.index('.', self._pdeci + 1)
            self._digits = self._ndeci - self._pdeci
        atom.xx, atom.xy, atom.xz = (float(line[20 + i * self._digits:20 + (i + 1) * self._digits]) * 10 for i in range(3))
        wbeg = 20 + self._digits * 3
        wend = wbeg + self._digits
        if line[wbeg:wend].strip():
            atom.vx, atom.vy, atom.vz = (float(line[wbeg + i * self._digits:wend + i * self._digits]) * 10 for i in range(3))
        return (atom, resname, resnum)


@add_metaclass(FileFormatType)
class GromacsGroFile(object):
    __doc__ = ' Parses and writes Gromacs GRO files '
        ...
        try:
            ...
            for i, line in enumerate(fileobj):
                ...
                try:
                    ...
                    raise GromacsError('Could not parse the atom record of GRO file %s' % filename)

                struct.add_atom(atom, resname, resnum)
            else:
                line = fileobj.readline()
                if i + 1 != natom:
                    raise GromacsError('Truncated GRO file. Found %d of %d atoms' % (
                     i + 1, natom))
            ...
            if line.strip():
                try:
                    ...
                    raise GromacsError('Could not understand box line of GRO file %s' % filename)
        ...
        if isinstance(dest, string_types):
            ...
            if not hasattr(dest, 'write'):
                ...
                if combine != 'all':
                    ...
                    for molid in range(n_mols):
                        ...
                        new_molecule = set()
                        last_found_atom = None
                        for residue in molecule.residues:
                            resid += 1
                            for atom in residue.atoms:
                                for original_atom in unused_atoms:
                                    if atom.type == original_atom.type:
                                        if atom.name == original_atom.name:
                                            if atom.residue.name == original_atom.residue.name:
                                                if last_found_atom is not None:
                                                    if original_atom.idx != last_found_atom.idx + 1:
                                                        if len(new_molecule.intersection(original_atom.bond_partners)) == 0:
                                                            continue
                                                atid += 1
                                                _write_atom_line(original_atom, atid % 100000, resid % 100000, has_vels, dest, precision)
            ...
            if struct.box is not None:
                ...
                if all([abs(x - 90) < TINY for x in struct.box[3:]]):
                    dest.write('%10.5f%10.5f%10.5f' % (a[0] / 10, b[1] / 10, c[2] / 10))
                else:
                ...
                if struct.atoms:
                    ...
                    dest.write('%10.5f%10.5f%10.5f' % (diff[0], diff[1], diff[2]))
                    dest.write('\n')
        if own_handle:
            dest.close()