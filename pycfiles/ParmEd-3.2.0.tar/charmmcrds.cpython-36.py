...
# [GCC 8.4.0]
# Embedded file name: /home/swails/src/ParmEd/parmed/charmm/charmmcrds.py
# Compiled at: 2019-02-23 13:42:47
# Size of source mod 2**32: 13012 bytes
"""
...
class CharmmCrdFile(object):
    ...
    def write(struct, dest):
        ...
        for i, atom in enumerate(struct.atoms):
            ...
            dest.write('%10d%10d  %-8s  %-8s%20.10f%20.10f%20.10f  %-8s  %-8s%20.10f\n' % (
...
class CharmmRstFile(object):
                ...
                if len(self.header[row].strip()) != 0:
                    ...
                    if line[0][0:5] == 'NATOM' or line[0][0:6] == '!NATOM':
                        try:
                            ...
                            self.npriv = int(line[1])
                            self.nstep = int(line[2])
                            self.nsavc = int(line[3])
                            self.nsavv = int(line[4])
                            self.jhstrt = int(line[5])
                            break
                        except (ValueError, IndexError):
    ...
    def scan(self, handle, str, r=0):
        ...
        while scanning:
            ...
            if not line:
                raise CharmmError('Premature end of file')
            if len(line.strip()) != 0 and line.strip().split()[0][0:len(str)] == str:
                scanning = False
    ...
    def _get_formatted_crds(self, crdfile, crds):
        for row in range(self.natom):
            ...
            if len(line) < 3 * CHARMLEN:
                raise CharmmError('Less than 3 coordinates present in coordinate row or coords may be truncated.')
