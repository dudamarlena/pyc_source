...
# [GCC 8.4.0]
# Embedded file name: /home/swails/src/ParmEd/parmed/formats/psf.py
# Compiled at: 2017-09-04 22:17:03
# Size of source mod 2**32: 11772 bytes
"""
...
class PSFFile(object):
    ...
    def write(struct, dest, vmd=False):
        ...
        if isinstance(dest, string_types):
            ...
            if ext:
                atmfmt1 = '%10d %-8s %-8i %-8s %-8s %6d %10.6f %13.4f           '
                atmfmt2 = '%10d %-8s %-8i %-8s %-8s %-6s %10.6f %13.4f           '
                intfmt = '%10d'
            else:
                atmfmt1 = '%8d %-4s %-4i %-4s %-4s %4d %10.6f %13.4f           '
                atmfmt2 = '%8d %-4s %-4i %-4s %-4s %-4s %10.6f %13.4f           '
                intfmt = '%8d'
        dest.write('PSF CHEQ ')
        if hasattr(struct, 'flags'):
            ...
            for dih in struct.dihedrals:
                if dih.improper:
                    continue
                a1, a2, a3, a4 = (
                 dih.atom1, dih.atom2, dih.atom3, dih.atom4)
                if not (a1, a2, a3, a4) in torsions:
                    if (a4, a3, a2, a1) in torsions:
                        continue
                    nnormal += 1
            ...
            def improp_gen(struct):
                for imp in struct.impropers:
                    yield (
                     imp.atom1, imp.atom2, imp.atom3, imp.atom4)
            ...
            if struct.groups:
                ...
                typ = 1 if abs(sum(a.charge for a in struct.atoms)) < 0.0001 else 2
