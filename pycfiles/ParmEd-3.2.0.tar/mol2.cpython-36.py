...
# [GCC 8.4.0]
# Embedded file name: /home/swails/src/ParmEd/parmed/formats/mol2.py
# Compiled at: 2017-02-11 08:13:06
# Size of source mod 2**32: 28531 bytes
"""
...
class Mol2File(object):
    __doc__ = ' Class to read and write TRIPOS Mol2 files '
                    ...
                    if line.startswith('#'):
                        ...
                        if not line.strip():
                            ...
                            if line.startswith('@<TRIPOS>'):
                                ...
                                if section == 'MOLECULE':
                                    if restemp.atoms or rescont:
                                        if structure:
                                            raise Mol2Error('Cannot convert MOL2 with multiple @<MOLECULE>s to a Structure')
                                        if not multires_structure:
                                            if mol_info[0]:
                    ...
                    if section == 'ATOM':
                        ...
                        if order in Mol2File.BOND_ORDER_MAP:
                            ...
                            try:
                                ...
                                warnings.warn('Mol2 bond order not recognized: %s' % order, ParameterWarning)
                                order = 1.0
                        ...
                        if atom1.residue is not atom2.residue:
                            if atom1.residue.idx == len(rescont):
                                ...
                                if atom1.residue.idx < len(rescont):
                                    ...
                                    if not atom.residue.idx <= len(rescont):
                                        raise AssertionError('Bad bond!')
                                    else:
                        ...
                        if len(box) != 6:
                            raise ValueError('%d box dimensions found; needed 6' % len(box))
                            ...
                            for a in words[3:]:
                                if a == '0':
                                    ...
                                    for atom in res:
                                        ...
                                        raise Mol2Error('Residue connection atom %s not found in residue %d' % (
                                         a, residx))
        ...
        try:
                ...
                for i, res in enumerate(struct):
                    ...
                    if i < len(struct) - 1:
                        if res.tail is not None:
                            if struct[(i + 1)].head is not None:
            ...
            for i, res in enumerate(residues):
                ...
                if isinstance(res, ResidueTemplate):
                    if i != len(residues) - 1:
                        if res.tail is not None:
                            if residues[(i + 1)].head is not None:
                                intresbonds += 1
                    if i != 0:
                        if res.head is not None:
                            if residues[(i - 1)].tail is not None:
                                intresbonds += 1
                else:
                    for atom in res:
            ...
            if mol3:
                ...
                for i, res in enumerate(residues):
                    if isinstance(res, ResidueTemplate):
                        con = [
                         res.head, res.tail, None, None, None, None]
