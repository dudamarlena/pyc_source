...
# [GCC 8.4.0]
# Embedded file name: /home/swails/src/ParmEd/parmed/openmm/topsystem.py
...
from ..topologyobjects import Angle, AngleType, Atom, AtomType, Bond, BondType, Cmap, CmapType, Dihedral, DihedralType, ExtraPoint, Improper, ImproperType, NonbondedException, NonbondedExceptionType, RBTorsionType, UreyBradley
...
def load_topology(topology, system=None, xyz=None, box=None):
        ...
        for r in c.residues():
            ...
            for a in r.atoms():
                if a.element is None:
                    ...
                    atom = Atom(atomic_number=(a.element.atomic_number), name=(a.name),
                      mass=(a.element.mass),
                      type=atype)
    ...
    if system is None:
        ...
        elif not isinstance(system, mm.System):
                ...
                if isinstance(force, mm.HarmonicAngleForce):
                    ...
                    if isinstance(force, mm.PeriodicTorsionForce):
                        ...
                        if isinstance(force, mm.RBTorsionForce):
                            ...
                            if isinstance(force, mm.CustomTorsionForce):
                                ...
                                if isinstance(force, mm.CMAPTorsionForce):
                                    ...
                                    if isinstance(force, mm.NonbondedForce):
                                        ...
                                        if isinstance(force, ignored_forces):
                                            continue
                                        else:
                                            struct.unknown_functional = True
...
def _process_urey_bradley(struct, force):
    ...
    if not struct.angles:
        warnings.warn('Adding what seems to be Urey-Bradley terms before Angles. This is unexpected, but the parameters will all be present in one form or another.', OpenMMWarning)
    typemap = dict()
    for ii in range(force.getNumBonds()):
        i, j, req, k = force.getBondParameters(ii)
        ai, aj = struct.atoms[i], struct.atoms[j]
        key = (req._value, k._value)
        if struct.angles:
            if ai not in aj.angle_partners:
                warnings.warn('Adding what seems to be Urey-Bradley terms, but atoms %d and %d do not appear to be angled to each other. Parameters will all be present, but may not be in expected places.' % (
                 ai.idx, aj.idx), OpenMMWarning)
        if key in typemap:
...
def _process_nonbonded(struct, force):
    ...
    for i in range(force.getNumParticles()):
        ...
        rmin = sig.value_in_unit(u.angstroms) * 1.122462048309373 / 2
    ...
    for ii in range(force.getNumExceptions()):
        ...
        nbtype = NonbondedExceptionType(sig * 1.122462048309373, eps, chgscale)
