...
# [GCC 8.4.0]
# Embedded file name: /home/swails/src/ParmEd/parmed/amber/netcdffiles.py
# Compiled at: 2018-07-13 08:57:48
# Size of source mod 2**32: 29500 bytes
"""
...
class NetCDFRestart(object):
    __doc__ = ' Class to read or write NetCDF restart files '
    ...
    def __init__(self, fname, mode='r'):
        ...
        if mode.startswith('w'):
            ...
            if mode.startswith('w'):
                warnings.warn('Could not find netCDF4 module. Falling back on scipy implementation, which can significantly slow down simulations if used as a reporter')
            self._ncfile = NetCDFFile(fname, mode, mmap=False)

    @classmethod
    def open_new(cls, fname, natom, box, vels, title='', remd=None, temp=None, remd_dimtypes=None):
        """
                ...
                if temp is None:
                    ...
                    if remd[0] in 'mM':
                        ...
                        if remd_dimtypes is None:
                            raise ValueError('remd_dimtypes must be given for multi-D REMD, and must have the same length.')
                        for dt in remd_dimtypes:
                            if dt not in (1, 3):
                                raise ValueError('remd_dimtypes only supports dimension types 1 and 3 currently')
    ...
    def velocities(self, stuff):
        self._ncfile.variables['velocities'][:] = np.reshape(stuff, (self.atom, 3)) / self.velocity_scale
        self.flush()
...
class NetCDFTraj(object):
    ...
    @classmethod
    def open_new(cls, fname, natom, box, crds=True, vels=False, frcs=False, remd=None, remd_dimension=None, title=''):
        """
        Opens a new NetCDF file and sets the attributes
    ...
    def add_coordinates(self, stuff):
        ...
        self._ncfile.variables['coordinates'][self._last_crd_frame] = np.reshape(stuff, (self.atom, 3))
    ...
    def add_velocities(self, stuff):
        ...
        self._ncfile.variables['velocities'][self._last_vel_frame] = np.reshape(stuff, (self.atom, 3)) / self.velocity_scale
    ...
    def add_forces(self, stuff):
        ...
        self._ncfile.variables['forces'][self._last_frc_frame] = np.reshape(stuff, (self.atom, 3))
    ...
    def add_remd_indices(self, stuff):
        ...
        """
        self._ncfile.variables['remd_indices'][self._last_remd_frame] = np.asarray(stuff, dtype='i')
