...
# [GCC 8.4.0]
# Embedded file name: /home/swails/src/ParmEd/parmed/amber/asciicrd.py
# Compiled at: 2017-03-01 21:14:20
# Size of source mod 2**32: 24550 bytes
"""
...
class AmberAsciiRestart(_AmberAsciiCoordinateFile):
    ...
    def __init__(self, fname, mode='r', natom=0, hasbox=None, title=None, time=0.0):
        ...
        super(AmberAsciiRestart, self).__init__(fname, natom, hasbox, mode, title)
            ...
            if len(lines) == int(ceil(self.natom / 2.0) + 3):
                ...
                if len(lines) == int(2 * ceil(self.natom / 2.0) + 2):
                    ...
                    if len(lines) == int(2 * ceil(self.natom / 2.0) + 3):
                        ...
                        raise RuntimeError('Badly formatted restart file. Has %d lines for %d atoms.' % (
                         len(lines), self.natom))
    ...
    def cell_lengths(self):
        if self._status == 'new':
            if not hasattr(self, '_cell_lengths'):
                raise RuntimeError('Cell lengths not yet available')
            return self.hasbox or None
        else:
    ...
    def cell_angles(self):
        if self._status == 'new':
            if not hasattr(self, '_cell_angles'):
    ...
    def box(self):
        ...
        if self._status == 'new':
            if not (hasattr(self, '_cell_lengths') and hasattr(self, '_cell_angles')):
...
class AmberMdcrd(_AmberAsciiCoordinateFile):
    ...
    def _parse(self):
        ...
        try:
            ...
            _warnings.warn('Unexpected EOF in parsing mdcrd. natom and/or hasbox are likely wrong', RuntimeWarning)
