...
# [GCC 8.4.0]
# Embedded file name: /home/swails/src/ParmEd/parmed/amber/amberformat.py
# Compiled at: 2019-03-31 15:13:01
# Size of source mod 2**32: 38261 bytes
"""
...
from parmed.constants import NATOM, NTYPES, NBONH, NTHETH, NPHIH, NEXT, NRES, NBONA, NTHETA, NPHIA, NUMBND, NUMANG, NPTRA, NATYP, NPHB, IFBOX, IFCAP, AMBER_ELECTROSTATIC, CHARMM_ELECTROSTATIC
...
class FortranFormat(object):
    ...
    strre = re.compile('(\\d+)?a(\\d+)$', re.I)
    intre = re.compile('(\\d+)?i(\\d+)$', re.I)
    floatre = re.compile('(\\d+)?[edf](\\d+)\\.(\\d+)$', re.I)
    floatre2 = re.compile('(\\d+)?\\([edf](\\d+)\\.(\\d+)\\)$', re.I)
    ...
    def __init__(self, format_string, strip_strings=True):
        ...
        if FortranFormat.strre.match(format_string):
            ...
            if FortranFormat.intre.match(format_string):
                self.type = int
                ...
                if FortranFormat.floatre.match(format_string):
                    self.type = float
                    ...
                    if FortranFormat.floatre2.match(format_string):
                        self.type = float
    ...
    def __eq__(self, other):
        return self.format == other.format and self.strip_strings == other.strip_strings
...
class AmberFormat(object):
    ...
    def rdparm(self, fname, slow=False):
        ...
        try:
            ...
            if hasattr(fname, 'read') or slow or fname.startswith('http://') or fname.startswith('https://') or fname.startswith('ftp://') or fname.endswith('.bz2') or fname.endswith('.gz'):
                return self.rdparm_slow(fname)
            try:
        ...
        def read_integer(line_idx, lines, num_items):
            ...
            while i < num_items:
                ...
                try:
                    ...
                    raise ValueError('Error parsing line %d, token %d [%s]: Problem during integer read.' % (
                     line_idx, idx,
        ...
        def read_float(line_idx, lines, num_items):
            ...
            while i < num_items:
                ...
                try:
                    ...
                    raise ValueError('Error parsing line %d, token %d [%s]: Problem during floating point read.' % (
                     line_idx, idx,
    ...
    def set_version(self):
        ...
        self.version = '%%VERSION  VERSION_STAMP = V0001.000  DATE = %02d/%02d/%02d  %02d:%02d:%02d' % (
    ...
    def add_flag(self, flag_name, flag_format, data=None, num_items=-1, comments=None, after=None):
        """
        ...
        if flag_name in self.parm_data:
            ...
            if after is not None:
                after = after.upper()
                if after not in self.flag_list:
            ...
            if data is not None:
                ...
                if num_items < 0:
                    raise AmberError('If you do not supply prmtop data, num_items must be non-negative!')
    ...
    def __getstate__(self):
        return dict(parm_data=(self.parm_data), flag_list=(self.flag_list), formats=(self.formats),
          parm_comments=(self.parm_comments),
          charge_flag=(self.charge_flag),
