...
# [GCC 8.4.0]
# Embedded file name: /home/swails/src/ParmEd/parmed/formats/pqr.py
# Compiled at: 2017-02-11 08:13:06
# Size of source mod 2**32: 14432 bytes
"""
...
class PQRFile(object):
    ...
    def id_format(filename):
        ...
        with closing(genopen(filename, 'r')) as (f):
            for line in f:
                ...
                if not words:
                    continue
                elif words[0] in ('CRYST1', 'END', 'END', 'HEADER', 'NUMMDL', 'MASTER',
                                  'AUTHOR', 'CAVEAT', 'COMPND', 'EXPDTA', 'MDLTYP',
                                  'KEYWDS', 'OBSLTE', 'SOURCE', 'SPLIT', 'SPRSDE',
                                  'TITLE ', 'ANISOU', 'CISPEP', 'CONECT', 'DBREF ',
                                  'HELIX ', 'HET', 'LINK', 'MODRES', 'REVDAT', 'SEQADV',
                                  'SHEET', 'SSBOND', 'FORMUL', 'HETNAM', 'HETSYN',
                                  'SEQRES', 'SITE', 'ENDMDL', 'MODEL', 'JRNL', 'REMARK',
                                  'TER', 'USER'):
                    continue
                    ...
                    if words[0] in ('ATOM', 'HETATM'):
                        ...
                        return words[1].isdigit() or False
                    else:
                        if words[2].isdigit():
                            return False
                        else:
                            if words[3].isdigit():
                                ...
                                return words[(4 + offset)].isdigit() or False
                        try:
            ...
            for line in fileobj:
                ...
                if words[0] in ('ATOM', 'HETATM'):
                    ...
                    if len(words) == 10:
                        ...
                        elem = element_by_name(nam)
                        atomic_number = AtomicNum[elem]
                        mass = Mass[elem]
                        if nam in ('EP', 'LP'):
                            atom = ExtraPoint(atomic_number=atomic_number, name=nam, charge=chg,
                              mass=mass,
                              number=num,
                              solvent_radius=rad)
                        else:
                            atom = Atom(atomic_number=atomic_number, name=nam, charge=chg,
                              mass=mass,
                              number=num,
                              solvent_radius=rad)
    ...
    @staticmethod
    def write(struct, dest, renumber=True, coordinates=None, standard_resnames=False):
        ...
        if not hasattr(dest, 'write'):
            ...
            atomrec = 'ATOM  %5d %-3s  %-3s %1s %3d    %7.3f %7.3f %7.3f %8.4f %8.4f\n'
