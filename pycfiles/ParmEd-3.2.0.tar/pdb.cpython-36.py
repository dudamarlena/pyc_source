...
# [GCC 8.4.0]
# Embedded file name: /home/swails/src/ParmEd/parmed/formats/pdb.py
# Compiled at: 2019-03-30 13:59:06
# Size of source mod 2**32: 80199 bytes
"""
        ...
        if old_atom.residue.name != resname:
            ...
            if old_atom.residue.number != resid:
                ...
                if old_atom.residue.segid != segid.strip():
                    return False
            if old_atom.residue.insertion_code != inscode.strip():
                return False
        return True
...
def _standardize_resname(resname):
    ...
    try:
        return (
         AminoAcidResidue.get(resname, abbronly=True).abbr, False)
    except KeyError:
        try:
            return (
             RNAResidue.get(resname).abbr, False)
        except KeyError:
            try:
                return (
                 DNAResidue.get(resname).abbr, False)
            except KeyError:
...
class PDBFile(object):
    __doc__ = ' Standard PDB file format parser and writer '
    AtomLookupKey = namedtuple('AtomLookupKey', ('name', 'number', 'residue_name',
                                                 'residue_number', 'chain', 'insertion_code',
                                                 'segment_id', 'alternate_location'))
    ...
    def id_format(filename):
        ...
        try:
            for line in fileobject:
                if line[:6] in frozenset({'HETNAM', 'ENDMDL', 'MODEL ', 'OBSLTE', 'CONECT', 'TITLE ', 'AUTHOR', 'TER   ', 'SITE  ', 'REVDAT', 'CISPEP', 'REMARK', 'MDLTYP', 'TER', 'SEQRES', 'JRNL  ', 'END', 'SHEET ', 'NUMMDL', 'SPRSDE', 'END   ', 'HET   ', 'FORMUL', 'SSBOND', 'SOURCE', 'HELIX ', 'HET', 'ANISOU', 'SEQADV', 'MODRES', 'DBREF ', 'CRYST1', 'DBREF2', 'SPLIT ', 'HEADER', 'HETSYN', 'EXPDTA', 'LINKR ', 'LINK  ', 'MASTER', 'COMPND', 'DBREF1', 'KEYWDS', 'DBREF', 'CAVEAT'}):
                    continue
                else:
                    if line[:6] == 'USER  ':
                        if line[6:9] == 'MOD':
                            continue
                if line[:5] in ('ORIGX', 'SCALE', 'MTRIX'):
    ...
    _relatere = re.compile('RELATED ID: *(\\w+) *RELATED DB: *(\\w+)', re.I)
    ...
    def parse(cls, filename, skip_bonds=False):
        ...
        try:
            ...
            raise PDBError('Coordinate shape mismatch. Probably caused by different atom counts in some of the PDB models')
    ...
    def _parse_open_file(self, fileobj):
        method_dispatch = {'REMARK':self._parse_remarks, 
    ...
    def _parse_remarks(self, line):
        ...
        if line[6:10] == ' 290':
            ...
            if line[6:10] == ' 900':
                ...
                if not line[11:22].strip() or self.struct.resolution is not None or line[23:38] == 'NOT APPLICABLE.':
                    return
    ...
    def _parse_atom_parts(cls, line):
        ...
        parts.update(dict(x=(float(line[30:38])),
          y=(float(line[38:46])),
          z=(float(line[46:54])),
    ...
    def _determine_residue_number(self, residue_number, line):
        ...
        if last_residue_number >= 9999 and residue_number == '1000' + '0' * extended_by and line[(26 + extended_by)] == '0':
            ...
            if last_residue_number == 9999 and residue_number != '9999':
                self._residue_indices_overflow = True
                self._last_residue_number_label = residue_number
                return last_residue_number + 1
            self._last_residue_number_label = residue_number
    ...
    def _parse_anisou_record(self, line):
        ...
        try:
            ...
            warnings.warn('Problem parsing anisotropic factors from ANISOU record from: ' + line, PDBWarning)
        else:
            anisou = np.array([u11 / 10000.0, u22 / 10000.0, u33 / 10000.0, u12 / 10000.0, u13 / 10000.0, u23 / 10000.0])
    ...
    def _determine_atom_number(self, atom_number):
        if self._last_atom is not None:
            self._atom_indices_overflow = self._atom_indices_overflow or self._last_atom.number >= 99999
    ...
    def _make_atom_key_from_parts(cls, atom_parts, all_parts=False):
        ...
        return cls.AtomLookupKey(name=(atom_parts['name']),
    ...
    def _parse_atom_record(self, line):
        ...
        atom = AtomClass(atomic_number=(atom_parts['atomic_number']), name=(atom_parts['name']), charge=(atom_parts['charge']),
          mass=(atom_parts['mass']),
          occupancy=(atom_parts['occupancy']),
          bfactor=(atom_parts['bfactor']),
          altloc=(atom_parts['alternate_location']),
          number=atom_number)
        ...
        if current_atom is not None and atom_parts['alternate_location'] in _ascii_letters_set and not self._atom_indices_overflow and not self._residue_indices_overflow:
            if self._current_model_number == 1:
                ...
                return
        elif current_atom not in self._model1_atoms_in_structure:
            return
        else:
            ...
            if self._current_model_number == 1:
                ...
                self._atom_map_from_atom_number[atom_number] = atom
                self.struct.add_atom(atom, atom_parts['residue_name'], residue_number, atom_parts['chain'], atom_parts['insertion_code'], atom_parts['segment_id'])
        ...
        if atom_from_first_model.residue.name != atom_parts['residue_name'] or atom_from_first_model.name != atom_parts['name']:
    ...
    def _new_model(self, line):
        if self._current_model_number == 1:
            if len(self.struct.atoms) == 0:
                return
        if self._model_open:
            warnings.warn('%s begun before last model ended. Assuming it is ending' % line.strip(), PDBWarning)
    ...
    def _parse_connect_record(self, line):
        ...
        if origin_index is None or index_1 is None:
            warnings.warn('Bad CONECT record -- not enough atom indexes in line: %s' % line, PDBWarning)
            return
        ...
        for partner in (atom_1, atom_2, atom_3, atom_4):
            ...
            if not partner is None:
                if partner in origin_atom.bond_partners:
                    pass
                else:
    ...
    def _assign_anisou_to_atoms(self):
        ...
        for key, (anisou_tensor, line) in iteritems(self._anisou_records):
            try:
                ...
                warnings.warn('Could not find atom belonging to anisou tensor with key %s. Line: %s' % (
                 key, line), PDBWarning)
    ...
    @staticmethod
    def write(struct, dest, renumber=True, coordinates=None, altlocs='all', write_anisou=False, charmm=False, use_hetatoms=True, standard_resnames=False, increase_tercount=True, write_links=False):
        ...
        if renumber:
            if write_links:
                raise ValueError('write_links requires renumber=False AND original (not implied) numbers to be assigned')
        if altlocs.lower() == 'all'[:len(altlocs)]:
            altlocs = 'all'
        else:
            if altlocs.lower() == 'first'[:len(altlocs)]:
                altlocs = 'first'
            else:
                if altlocs.lower() == 'occupancy'[:len(altlocs)]:
                    altlocs = 'occupancy'
                else:
                    raise ValueError("Illegal value of occupancy [%s]; expected 'all', 'first', or 'occupancy'" % altlocs)
            own_handle = False
            if not hasattr(dest, 'write'):
                dest = genopen(dest, 'w')
                own_handle = True
            if charmm:
                atomrec = 'ATOM  %5d %-4s%1s%-4s%1s%4d%1s   %8.3f%8.3f%8.3f%6.2f%6.2f      %-4s%2s%-2s\n'
                anisourec = 'ANISOU%5d %-4s%1s%-4s%1s%4d%1s %7d%7d%7d%7d%7d%7d      %2s%-2s\n'
                terrec = 'TER   %5d      %-4s%1s%4d\n'
                reslen = 4
            else:
                atomrec = 'ATOM  %5d %-4s%1s%-3s %1s%4d%1s   %8.3f%8.3f%8.3f%6.2f%6.2f      %-4s%2s%-2s\n'
                anisourec = 'ANISOU%5d %-4s%1s%-3s %1s%4d%1s %7d%7d%7d%7d%7d%7d      %2s%-2s\n'
                terrec = 'TER   %5d      %-3s %1s%4d\n'
                reslen = 3
        linkrec = 'LINK        %-4s%1s%-3s %1s%4d%1s               %-4s%1s%-3s %1s%4d%1s  %6s %6s %5.2f\n'
        ...
        if struct.symmetry is not None:
            ...
            for index, arr in enumerate(struct.symmetry.data):
                arr_list = [
                 1 + index % 3, 1 + index // 3] + arr.tolist()
        ...
        if coordinates is not None:
            ...
            if altlocs == 'all':
                ...
                def print_atoms(atom, coords):
                    return (
                     atom, atom.other_locations, coords[atom.idx])
                ...
                if altlocs == 'first':
                    ...
                    def print_atoms(atom, coords):
                        return (
                         atom, dict(), coords[atom.idx])

                else:
                    if altlocs == 'occupancy':
            ...
            for model, coord in enumerate(coords):
                ...
                for res in struct.residues:
                    ...
                    for atom in atoms:
                        ...
                        if write_anisou:
                            if pa.anisou is not None:
                                anisou = [int(ani * 10000.0) for ani in pa.anisou]
                        ...
                        for key in sorted(others.keys()):
                            ...
                            if write_anisou and oatom.anisou is not None:
                                anisou = [int(ani * 10000.0) for ani in oatom.anisou]
...
class CIFFile(object):
    __doc__ = ' Standard PDBx/mmCIF file format parser and writer '
    ...
    def download(pdb_id, timeout=10, saveto=None):
        ...
        if not isinstance(pdb_id, string_types) or len(pdb_id) != 4:
            ...
            try:
                try:
                    ftp.retrbinary('RETR /pub/pdb/data/structures/divided/mmCIF/%s/%s.cif.gz' % (
                     pdb_id[1:3], pdb_id), fileobj.write)
                except ftplib.all_errors as err:
                    raise IOError('Could not retrieve PDB ID %s; %s' % (pdb_id, err))
    ...
    def parse(filename, skip_bonds=False):
        ...
        for cont in data:
            ...
            for i in range(atoms.getRowCount()):
                ...
                if _compare_atoms(last_atom, atom, resname, resnum, chain, '', inscode):
                    if altloc:
                        ...
                        try:
                            for i in range(anisou.getRowCount()):
                                ...
                                if inscode in '?.':
                                    inscode = ''
                                key = (
                                 resnum, resname, inscode, chain, atnum,
                                 altloc, atname)
    ...
    @staticmethod
    def write(struct, dest, renumber=True, coordinates=None, altlocs='all', write_anisou=False, standard_resnames=False):
        """
...
def _format_atom_name_for_pdb(atom):
    if len(atom.name) < 4:
        if len(Element[atom.atomic_number]) != 2:
            return ' %-3s' % atom.name
    return atom.name[:4]
