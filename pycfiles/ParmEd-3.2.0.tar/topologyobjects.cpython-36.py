...
# [GCC 8.4.0]
# Embedded file name: /home/swails/src/ParmEd/parmed/topologyobjects.py
# Compiled at: 2019-03-25 16:13:20
# Size of source mod 2**32: 188967 bytes
"""
...
__all__ = [
 'Angle', 'AngleType', 'Atom', 'AtomList', 'Bond', 'BondType', 'ChiralFrame', 'Cmap',
...
class _FourAtomTerm(object):
    ...
    def __init__(self, atom1, atom2, atom3, atom4):
        if atom1 is atom2 or atom1 is atom3 or atom1 is atom4 or atom2 is atom3 or atom2 is atom4 or atom3 is atom4:
            raise MoleculeError('4-atom term cannot have duplicate atoms! Atoms are: %s %s %s %s' % (
             atom1, atom2,
             atom3, atom4))
        self.atom1 = atom1
        self.atom2 = atom2
...
class Atom(_ListItem):
    ...
    def __init__(self, list=None, atomic_number=0, name='', type='', charge=None, mass=0.0, nb_idx=0, solvent_radius=0.0, screen=0.0, tree='BLA', join=0.0, irotat=0.0, occupancy=0.0, bfactor=0.0, altloc='', number=-1, rmin=None, epsilon=None, rmin14=None, epsilon14=None):
        self.list = list
        self._idx = -1
    ...
    @classmethod
    def _copy(cls, item):
        new = cls(atomic_number=(item.atomic_number), name=(item.name), type=(item.type), charge=(item.charge),
          mass=(item.mass),
          nb_idx=(item.nb_idx),
          solvent_radius=(item.solvent_radius),
          screen=(item.screen),
          tree=(item.tree),
          join=(item.join),
          irotat=(item.irotat),
          occupancy=(item.occupancy),
          bfactor=(item.bfactor),
          altloc=(item.altloc))
    ...
    def sigma(self):
        ...
        return self.rmin * 0.8908987181403393 * 2

    @sigma.setter
    def sigma(self, value):
        self._rmin = _strip_units(value, unit=(u.angstroms)) * 1.122462048309373 / 2
    ...
    def sigma_14(self):
        ...
        if self._rmin14 is None:
            if self.atom_type is UnassignedAtomType or self.atom_type.rmin_14 is None:
                return self.sigma
            return self.atom_type.rmin_14 * 0.8908987181403393 * 2
        else:
            return self._rmin14 * 0.8908987181403393 * 2

    @sigma_14.setter
    def sigma_14(self, value):
        self._rmin14 = _strip_units(value, u.angstroms) * 1.122462048309373 / 2
    ...
    def bond_to(self, other):
        ...
        if isinstance(other, ExtraPoint):
            ...
            if isinstance(self, ExtraPoint):
                other.children.append(self)
        if self is other:
            raise MoleculeError('Cannot bond atom to itself! Atoms are: %s %s' % (
             self, other))
    ...
    def angle_to(self, other):
        ...
        """
        if self is other:
            raise MoleculeError('Cannot angle an atom with itself! Atoms are: %s %s' % (
             self, other))
    ...
    def dihedral_to(self, other):
        ...
        """
        if self is other:
            raise MoleculeError('Cannot dihedral an atom with itself! Atoms are: %s %s' % (
             self, other))
    ...
    def tortor_to(self, other):
        ...
        """
        if self is other:
            raise MoleculeError('Cannot coupled-dihedral atom to itself Atoms are: %s %s' % (
             self, other))
    ...
    def exclude(self, other):
        ...
        """
        if self is other:
            raise MoleculeError('Cannot exclude an atom from itself! Atoms are: %s %s' % (
             self, other))
    ...
    def prune_exclusions(self):
        ...
        the exclusion list.
        """
        excludes = set(self._exclusion_partners) - set(self._tortor_partners) - set(self._dihedral_partners) - set(self._angle_partners) - set(self._bond_partners)
    ...
    def __getstate__(self):
        retval = dict(name=(self.name), type=(self.type), atom_type=(self.atom_type), _charge=(self._charge),
          mass=(self.mass),
          nb_idx=(self.nb_idx),
          ...
          _rmin14=(self._rmin14),
          _epsilon14=(self._epsilon14),
        ...
        if self._frame_type is not None:
            ...
            if len(self.parent.bonds) == 3:
                ...
                try:
                    ...
                    elif self.parent.element == 8 and other_atom.element == 1 and other_atom2.element == 1:
...
class ThreeParticleExtraPointFrame(object):
    ...
    def get_atoms(self):
        ...
        if b2.atom1 is self.ep.parent:
            ...
            oatom2 = b2.atom1
        return (
         self.ep.parent, oatom1, oatom2)

    @staticmethod
    def from_weights--- This code section failed: ---

 L.1394         0  LOAD_FAST                'a1'
                2  LOAD_FAST                'parent'
                4  LOAD_ATTR                bond_partners
                6  COMPARE_OP               not-in
                8  POP_JUMP_IF_TRUE     20  'to 20'
               10  LOAD_FAST                'a2'
               12  LOAD_FAST                'parent'
               14  LOAD_ATTR                bond_partners
               16  COMPARE_OP               not-in
             18_0  COME_FROM             8  '8'
               18  POP_JUMP_IF_FALSE    28  'to 28'

 L.1395        20  LOAD_GLOBAL              ValueError
               22  LOAD_STR                 'Parent atom not bound to other 2 atoms in frame'
               24  CALL_FUNCTION_1       1  '1 positional argument'
               26  RAISE_VARARGS_1       1  'exception'
             28_0  COME_FROM            18  '18'

 L.1396        28  LOAD_FAST                'a2'
               30  LOAD_FAST                'a1'
               32  LOAD_ATTR                angle_partners
               34  COMPARE_OP               not-in
               36  POP_JUMP_IF_FALSE    56  'to 56'
               38  LOAD_FAST                'a2'
               40  LOAD_FAST                'a1'
               42  LOAD_ATTR                bond_partners
               44  COMPARE_OP               not-in
               46  POP_JUMP_IF_FALSE    56  'to 56'

 L.1397        48  LOAD_GLOBAL              ValueError
               50  LOAD_STR                 'No geometry defined between 2 non-parent atoms'
               52  CALL_FUNCTION_1       1  '1 positional argument'
               54  RAISE_VARARGS_1       1  'exception'
             56_0  COME_FROM            46  '46'
             56_1  COME_FROM            36  '36'

 L.1398        56  LOAD_FAST                'w1'
               58  LOAD_FAST                'w2'
               60  COMPARE_OP               !=
               62  POP_JUMP_IF_FALSE    72  'to 72'

 L.1399        64  LOAD_GLOBAL              ValueError
               66  LOAD_STR                 'Currently only equal weights are supported'
               68  CALL_FUNCTION_1       1  '1 positional argument'
               70  RAISE_VARARGS_1       1  'exception'
             72_0  COME_FROM            62  '62'

 L.1401        72  LOAD_FAST                'dp1'
               74  LOAD_CONST               None
               76  COMPARE_OP               is
 ...
 L.1402        88  SETUP_LOOP          172  'to 172'
               90  LOAD_FAST                'parent'
               92  LOAD_ATTR                bonds
               94  GET_ITER         
               96  FOR_ITER            170  'to 170'
               98  STORE_FAST               'bond'

 L.1403       100  LOAD_FAST                'a1'
              102  LOAD_FAST                'bond'
              104  COMPARE_OP               in
              106  POP_JUMP_IF_FALSE   134  'to 134'

 L.1404       108  LOAD_FAST                'bond'
              110  LOAD_ATTR                type
              112  LOAD_CONST               None
              114  COMPARE_OP               is
              116  POP_JUMP_IF_FALSE   126  'to 126'

 L.1405       118  LOAD_GLOBAL              ParameterError
              120  LOAD_STR                 'Could not determine virtual site geometry'
              122  CALL_FUNCTION_1       1  '1 positional argument'
              124  RAISE_VARARGS_1       1  'exception'
            126_0  COME_FROM           116  '116'

 L.1406       126  LOAD_FAST                'bond'
              128  LOAD_ATTR                type
              130  LOAD_ATTR                req
              132  STORE_FAST               'dp1'
            134_0  COME_FROM           106  '106'

 L.1407       134  LOAD_FAST                'a2'
              136  LOAD_FAST                'bond'
              138  COMPARE_OP               in
              140  POP_JUMP_IF_FALSE    96  'to 96'

 L.1408       142  LOAD_FAST                'bond'
              144  LOAD_ATTR                type
              146  LOAD_CONST               None
              148  COMPARE_OP               is
              150  POP_JUMP_IF_FALSE   160  'to 160'

 L.1409       152  LOAD_GLOBAL              ParameterError
              154  LOAD_STR                 'Could not determine virtual site geometry'
              156  CALL_FUNCTION_1       1  '1 positional argument'
              158  RAISE_VARARGS_1       1  'exception'
            160_0  COME_FROM           150  '150'

 L.1410       160  LOAD_FAST                'bond'
              162  LOAD_ATTR                type
              164  LOAD_ATTR                req
              166  STORE_FAST               'dp2'
              168  JUMP_BACK            96  'to 96'
              170  POP_BLOCK        
            172_0  COME_FROM_LOOP       88  '88'
            172_1  COME_FROM            86  '86'

 L.1411       172  LOAD_FAST                'theteq'
              174  LOAD_CONST               None
              176  COMPARE_OP               is
              178  JUMP_IF_FALSE_OR_POP   186  'to 186'
              180  LOAD_FAST                'd12'
              182  LOAD_CONST               None
              184  COMPARE_OP               is
            186_0  COME_FROM           178  '178'
              186  POP_JUMP_IF_FALSE   368  'to 368'

 L.1412       190  LOAD_FAST                'a2'
              192  LOAD_FAST                'a1'
              194  LOAD_ATTR                angle_partners
              196  COMPARE_OP               not-in
              198  POP_JUMP_IF_FALSE   298  'to 298'

 L.1413       202  SETUP_LOOP          254  'to 254'
              204  LOAD_FAST                'a1'
              206  LOAD_ATTR                bonds
              208  GET_ITER         
 ...
 L.1414       214  LOAD_FAST                'a2'
              216  LOAD_FAST                'bond'
              218  COMPARE_OP               not-in
              220  POP_JUMP_IF_FALSE   224  'to 224'

 L.1414       222  CONTINUE            210  'to 210'
              224  ELSE                     '250'

 L.1415       224  LOAD_FAST                'bond'
              226  LOAD_ATTR                type
              228  LOAD_CONST               None
              230  COMPARE_OP               is
              232  POP_JUMP_IF_FALSE   242  'to 242'

 L.1416       234  LOAD_GLOBAL              ParameterError
              236  LOAD_STR                 'Could not determine virtual site geometry'
              238  CALL_FUNCTION_1       1  '1 positional argument'
              240  RAISE_VARARGS_1       1  'exception'
            242_0  COME_FROM           232  '232'

 L.1417       242  LOAD_FAST                'bond'
              244  LOAD_ATTR                type
              246  LOAD_ATTR                req
              248  STORE_FAST               'd12'
              250  JUMP_BACK           210  'to 210'
              252  POP_BLOCK        
            254_0  COME_FROM_LOOP      202  '202'

 L.1419       254  LOAD_GLOBAL              math
              256  LOAD_ATTR                acos
              258  LOAD_FAST                'dp1'
              260  LOAD_FAST                'dp1'
              262  BINARY_MULTIPLY  
              ...
              294  STORE_FAST               'theteq'
              296  JUMP_FORWARD        366  'to 366'
              298  ELSE                     '366'

 L.1421       298  SETUP_LOOP          430  'to 430'
              300  LOAD_FAST                'a1'
              302  LOAD_ATTR                angles
              304  GET_ITER         
              306  FOR_ITER            350  'to 350'
              308  STORE_FAST               'ang'

 L.1422       310  LOAD_FAST                'a2'
              312  LOAD_FAST                'ang'
              314  COMPARE_OP               in
              316  POP_JUMP_IF_FALSE   306  'to 306'
              320  LOAD_FAST                'a2'
              322  LOAD_FAST                'ang'
              324  LOAD_ATTR                atom2
              326  COMPARE_OP               is-not
              328  POP_JUMP_IF_FALSE   306  'to 306'

 L.1423       332  LOAD_FAST                'ang'
              334  LOAD_ATTR                type
              336  LOAD_ATTR                theteq
              338  LOAD_GLOBAL              DEG_TO_RAD
              340  BINARY_MULTIPLY  
              342  STORE_FAST               'theteq'

 L.1424       344  BREAK_LOOP       
              346  JUMP_BACK           306  'to 306'
              350  POP_BLOCK        

 L.1426       352  LOAD_CONST               False
              354  POP_JUMP_IF_TRUE    430  'to 430'
              358  LOAD_GLOBAL              AssertionError
              360  LOAD_STR                 'Could not find matching angle'
              362  CALL_FUNCTION_1       1  '1 positional argument'
              364  RAISE_VARARGS_1       1  'exception'
            366_0  COME_FROM_LOOP      298  '298'
            366_1  COME_FROM           296  '296'
              366  JUMP_FORWARD        430  'to 430'
              368  ELSE                     '430'

 L.1427       368  LOAD_FAST                'theteq'
              370  LOAD_CONST               None
              372  COMPARE_OP               is
              374  POP_JUMP_IF_FALSE   422  'to 422'

 L.1428       378  LOAD_GLOBAL              math
              380  LOAD_ATTR                acos
              382  LOAD_FAST                'dp1'
              384  LOAD_FAST                'dp1'
              386  BINARY_MULTIPLY  
              ...
              410  LOAD_FAST                'dp2'
              412  BINARY_MULTIPLY  
              414  BINARY_TRUE_DIVIDE
              416  CALL_FUNCTION_1       1  '1 positional argument'
              418  STORE_FAST               'theteq'
              420  JUMP_FORWARD        430  'to 430'
              422  ELSE                     '430'

 L.1430       422  LOAD_FAST                'theteq'
              424  LOAD_GLOBAL              DEG_TO_RAD
              426  INPLACE_MULTIPLY 
              428  STORE_FAST               'theteq'
            430_0  COME_FROM           420  '420'
            430_1  COME_FROM           366  '366'
            430_2  COME_FROM           354  '354'

 L.1432       430  LOAD_GLOBAL              abs
              432  LOAD_FAST                'dp1'
              434  LOAD_FAST                'dp2'
              436  BINARY_SUBTRACT  
              438  CALL_FUNCTION_1       1  '1 positional argument'
              440  LOAD_GLOBAL              TINY
              442  COMPARE_OP               >
              444  POP_JUMP_IF_FALSE   456  'to 456'

 L.1433       448  LOAD_GLOBAL              ValueError
              450  LOAD_STR                 'Cannot deal with asymmetry in EP frame'
              452  CALL_FUNCTION_1       1  '1 positional argument'
              454  RAISE_VARARGS_1       1  'exception'
            456_0  COME_FROM           444  '444'

 L.1435       456  LOAD_GLOBAL              abs
              458  LOAD_FAST                'w1'
              460  LOAD_CONST               2
              462  BINARY_MULTIPLY  
              464  LOAD_GLOBAL              math
              466  LOAD_ATTR                cos
              468  LOAD_FAST                'theteq'
              470  LOAD_CONST               0.5
              472  BINARY_MULTIPLY  
    ...
    def get_weights(self):
        ...
        if found:
            ...
            if a1 not in a2.bond_partners:
                raise RuntimeError('EP frame definition incomplete for 3-point virtual site... cannot determine distance between particles 2 and 3')
            req23 = None
        ...
        if req23 is None:
            raise RuntimeError('Cannot determine 2-3 particle distance in three-site virtual site frame')
        req12 = b1.type.req
        req13 = b2.type.req
...
class OutOfPlaneExtraPointFrame(object):
    ...
    def get_atoms(self):
        ...
        """
        try:
            b1, b2 = [bond for bond in self.ep.parent.bonds if not isinstance(bond.atom1, ExtraPoint) if not isinstance(bond.atom2, ExtraPoint)]
        except (ValueError, TypeError):
            raise RuntimeError('Unsupported bonding pattern in EP frame')
    ...
    def get_weights(self):
        ...
        if len(regbonds) != 2:
            raise ValueError('EP parent bond pattern inconsistent with an out-of-plane, 3-point virtual site frame')
        if mybond is None:
        ...
        try:
            v12 = (
             a2.xx - a1.xx, a2.xy - a1.xy, a2.xz - a1.xz)
...
class Bond(object):
    ...
    def __init__(self, atom1, atom2, type=None, order=1.0):
        ...
        if isinstance(atom1, ExtraPoint):
            if isinstance(atom2, ExtraPoint):
    ...
    def uenergy(self):
        ...
        ene = self.energy()
        if ene is not None:
            return ene * u.kilocalories_per_mole
...
class Angle(object):
    ...
    def __init__(self, atom1, atom2, atom3, type=None):
        if atom1 is atom2 or atom1 is atom3 or atom2 is atom3:
            raise MoleculeError('Cannot angle atom to itself! Atoms are: %s %s %s' % (
             atom1, atom2, atom3))
        self.atom1 = atom1
        self.atom2 = atom2
        self.atom3 = atom3
    ...
    def __contains__(self, obj):
        ...
        if isinstance(obj, Atom):
            ...
            return self.atom1 in obj and self.atom2 in obj or self.atom2 in obj and self.atom3 in obj
...
class Dihedral(_FourAtomTerm):
    ...
    def funct(self):
        if self._funct is None:
            if self.type is not None:
                if isinstance(self.type, DihedralTypeList):
                    return 9
            if self.type is not None:
                if isinstance(self.type, RBTorsionType):
                    return 3
            return 1
        else:
            return self._funct
    ...
    def __contains__(self, thing):
        ...
        if isinstance(thing, Atom):
            ...
            if self.improper:
                return self.atom1 in thing and self.atom3 in thing or self.atom2 in thing and self.atom3 in thing or self.atom4 in thing and self.atom3 in thing
            return self.atom1 in thing and self.atom2 in thing or self.atom2 in thing and self.atom3 in thing or self.atom3 in thing and self.atom4 in thing
    ...
    def same_atoms(self, thing):
        ...
        if isinstance(thing, Dihedral):
            return self.atom1 is thing.atom1 and self.atom2 is thing.atom2 and self.atom3 is thing.atom3 and self.atom4 is thing.atom4 or self.atom1 is thing.atom4 and self.atom2 is thing.atom3 and self.atom4 is thing.atom1
        else:
            thing = list(thing)
            if len(thing) != 4:
                raise TypeError('comparative %s has %d elements! Expect 4.' % (
                 type(thing).__name__, len(thing)))
            return self.atom1.idx == thing[0] and self.atom2.idx == thing[1] and self.atom3.idx == thing[2] and self.atom4.idx == thing[3] or self.atom1.idx == thing[3] and self.atom2.idx == thing[2] and self.atom3.idx == thing[1] and self.atom4.idx == thing[0]
    ...
    def energy(self):
        ...
        if isinstance(self.type, DihedralTypeList):
            ...
            e *= 0.5
            return e
        raise NotImplementedError('Only DihedralType and DihedralTypeList are supported for energy calculations')
...
class DihedralType(_ParameterType, _ListItem):
    ...
    @_exception_to_notimplemented
    def __eq__(self, other):
        return abs(self.phi_k - other.phi_k) < TINY and self.per == other.per and abs(self.phase - other.phase) < TINY and abs(self.scee - other.scee) < TINY and abs(self.scnb - other.scnb) < TINY

    def __repr__(self):
        retstr = [
         '<%s; phi_k=%.3f, per=%d, phase=%.3f, ' % (
...
class RBTorsionType(_ParameterType, _ListItem):
    ...
    @_exception_to_notimplemented
    def __eq__(self, other):
        return abs(self.c0 - other.c0) < TINY and abs(self.c1 - other.c1) < TINY and abs(self.c2 - other.c2) < TINY and abs(self.c3 - other.c3) < TINY and abs(self.c4 - other.c4) < TINY and abs(self.c5 - other.c5) < TINY

    def __copy__(self):
        return RBTorsionType(self.c0, self.c1, self.c2, self.c3, self.c4, self.c5, self.scee, self.scnb)

    def __repr__(self):
        return '<RBTorsionType; c0=%.3f; c1=%.3f; c2=%.3f; c3=%.3f; c4=%.3f; c5=%.3f; scee=%s; scnb=%s>' % (
...
class DihedralTypeList(list, _ListItem):
    ...
    def from_rbtorsion(cls, rbtorsion):
        ...
        c5 = rbtorsion.c5
        phi = 0 * u.degrees
        fc0 = (4 * c0 + 4 * c1 + 4 * c3 - c4 + 4 * c5) / 8.0
        ...
        for i, existing in enumerate(self):
            if existing.per == other.per:
                if existing == other:
                    return
                if override:
                    warnings.warn('Overriding DihedralType in DihedralTypeList with same periodicity', ParameterWarning)
                    self[i] = other
                else:
                    raise ParameterError('Cannot add two DihedralType instances with the same periodicity to the same DihedralTypeList')
...
class UreyBradley(Bond):
    ...
    def __contains__(self, thing):
        ...
        if isinstance(thing, Atom):
            ...
            if thing.atom1 not in self:
                if thing.atom2 not in self:
                    return False
                end1 = thing.atom2
                cent = thing.atom1
            else:
...
class Improper(_FourAtomTerm):
    ...
    def __contains__(self, thing):
        ...
        if isinstance(thing, Atom):
            ...
            return self.atom1 in thing and self.atom2 in thing or self.atom1 in thing and self.atom3 in thing or self.atom1 in thing and self.atom4 in thing
...
class ImproperType(_ParameterType, _ListItem):
    ...
    @_exception_to_notimplemented
    def __eq__(self, other):
        return abs(self.psi_k - other.psi_k) < TINY and abs(self.psi_eq - other.psi_eq) < TINY
...
class Cmap(object):
        ...
        for i in range(len(atmlist)):
            for j in range(i + 1, len(atmlist)):
                if atmlist[i] is atmlist[j]:
                    raise MoleculeError('Cannot cmap atom to itself! Atoms are %s %s' % (
                     atmlist[i],
                     atmlist[j]))
    ...
    def extended(cls, atom1, atom2, atom3, atom4, atom5, atom6, atom7, atom8, type=None):
        ...
        if atom2 is not atom5 or atom3 is not atom6 or atom7 is not atom4:
            raise NotImplementedError('Only consecutive coupled-torsions are supported by CMAPs currently')
    ...
    def __contains__(self, thing):
        ...
        """
        if isinstance(thing, Atom):
            return thing is self.atom1 or thing is self.atom2 or thing is self.atom3 or thing is self.atom4 or thing is self.atom5
        else:
            return self.atom1 in thing and self.atom2 in thing or self.atom2 in thing and self.atom3 in thing or self.atom3 in thing and self.atom4 in thing or self.atom4 in thing and self.atom5 in thing
    ...
    def same_atoms(self, thing):
        ...
        if isinstance(thing, Cmap):
            return self.atom1 is thing.atom1 and self.atom2 is thing.atom2 and self.atom3 is thing.atom3 and self.atom4 is thing.atom4 and self.atom5 is thing.atom5 or self.atom1 is thing.atom5 and self.atom2 is thing.atom4 and self.atom3 is thing.atom3 and self.atom4 is thing.atom2 and self.atom5 is thing.atom1
        else:
            thing = list(thing)
            if len(thing) != 5:
                raise MoleculeError('CMAP can compare to 5 elements, not %d' % len(thing))
            return self.atom1.idx == thing[0] and self.atom2.idx == thing[1] and self.atom3.idx == thing[2] and self.atom4.idx == thing[3] and self.atom5.idx == thing[4] or self.atom1.idx == thing[4] and self.atom2.idx == thing[3] and self.atom3.idx == thing[2] and self.atom4.idx == thing[1] and self.atom5.idx == thing[0]
...
class CmapType(_ParameterType, _ListItem):
    ...
    @_exception_to_notimplemented
    def __eq__(self, other):
        return self.resolution == other.resolution and all(abs(i - j) < TINY for i, j in zip(self.grid, other.grid))
...
class TrigonalAngle(_FourAtomTerm):
    ...
    def __contains__(self, thing):
        if isinstance(thing, Atom):
            ...
            return self.atom1 in thing and self.atom2 in thing or self.atom2 in thing and self.atom3 in thing or self.atom2 in thing and self.atom4 in thing
...
class PiTorsion(object):
    ...
    def __contains__(self, thing):
        if isinstance(thing, Atom):
            return thing is self.atom1 or thing is self.atom2 or thing is self.atom3 or thing is self.atom4 or thing is self.atom5 or thing is self.atom6
        else:
            return self.atom2 in thing and self.atom3 in thing or self.atom1 in thing and self.atom3 in thing or self.atom3 in thing and self.atom4 in thing or self.atom4 in thing and self.atom5 in thing or self.atom4 in thing and self.atom6 in thing
...
class StretchBend(object):
    ...
    def __contains__(self, thing):
        if isinstance(thing, Atom):
            ...
            return self.atom1 in thing and self.atom2 in thing or self.atom2 in thing and self.atom3 in thing
...
class StretchBendType(_ParameterType, _ListItem):
    ...
    @_exception_to_notimplemented
    def __eq__(self, other):
        return abs(self.k1 - other.k1) < TINY and abs(self.k2 - other.k2) < TINY and abs(self.req1 - other.req1) < TINY and abs(self.req2 - other.req2) < TINY and abs(self.theteq - other.theteq) < TINY
...
class _TorTorTable(object):
    ...
    def __init__(self, ang1, ang2, data):
        if len(data) != len(ang1) * len(ang2):
            raise TypeError('Coupled torsion parameter size mismatch. %dx%d grid expects %d elements (got %d)' % (
             len(ang1),
...
class TorsionTorsionType(_ParameterType, _ListItem):
    ...
    def __init__(self, dims, ang1, ang2, f, dfda1=None, dfda2=None, d2fda1da2=None, list=None):
        _ParameterType.__init__(self)
        if len(dims) != 2:
    ...
    def __eq__(self, other):
        if self.dims != other.dims:
            ...
            if self.ang1 != other.ang1:
                return False
            if self.ang2 != other.ang2:
                return False
            return self.f == other.f and self.dfda1 == other.dfda1 and self.dfda2 == other.dfda2 and self.d2fda1da2 == other.d2fda1da2
...
class Residue(_ListItem):
    ...
    def __init__(self, name, number=-1, chain='', insertion_code='', segid='', list=None):
        self.name = name.strip()
        self.number = number
...
class ResidueList(TrackedList):
    __doc__ = ' Array of `Residue` instances '
    ...
    def add_atom(self, atom, resname, resnum, chain='', inscode='', segid=''):
        ...
        try:
            ...
            if last.number != resnum or last.name != resname.strip() or last.chain != chain.strip() or last.segid != segid.strip() or last.insertion_code != inscode.strip():
                new_res = Residue(resname, resnum, chain, inscode, segid,
                  list=self)
                new_res.add_atom(atom)
                self.append(new_res)
            else:
        ...
        for i, type in enumerate(atom_type_list):
            for key in type.nbfix:
                ...
                try:
                    ...
                    obj = (
                     otype._idx, rmin, eps, rmin14, eps14)
...
class NonbondedException(object):
    ...
    def __repr__(self):
        retstr = [
         '<%s; %r and %r' % (type(self).__name__, self.atom1,
          self.atom2)]
        if self.type is not None:
...
class NonbondedExceptionType(_ParameterType, _ListItem):
    ...
    @property
    def sigma(self):
        return self.rmin * 0.8908987181403393

    @sigma.setter
    def sigma(self, value):
        self.rmin = value * 1.122462048309373
    ...
    @_exception_to_notimplemented
    def __eq__(self, other):
        return abs(self.rmin - other.rmin) < TINY and abs(self.epsilon - other.epsilon) < TINY and abs(self.chgscale - other.chgscale) < TINY
...
class AmoebaNonbondedExceptionType(NonbondedExceptionType):
    ...
    def __init__(self, vdw_weight, multipole_weight, direct_weight, polar_weight, mutual_weight, list=None):
    ...
    @_exception_to_notimplemented
    def __eq__(self, other):
        return abs(self.vdw_weight - other.vdw_weight) < TINY and abs(self.multipole_weight - other.multipole_weight) < TINY and abs(self.direct_weight - other.direct_weight) < TINY and abs(self.polar_weight - other.polar_weight) < TINY and abs(self.mutual_weight - other.mutual_weight) < TINY

    def __copy__(self):
        return AmoebaNonbondedExceptionType(self.vdw_weight, self.multipole_weight, self.direct_weight, self.polar_weight, self.mutual_weight)
...
class AtomType(object):
    ...
    def __eq__(self, other):
        ...
        if isinstance(other, AtomType):
            ...
            if not (has_all and not has_none):
                if not (has_none and not has_all):
                    raise AssertionError('Should have all or none at this point')
            return has_all or True
        else:
            if self.charge is None or other.charge is None:
                if self.charge is not other.charge:
                    return False
                else:
                    if abs(self.charge - other.charge) > TINY:
                        return True
                    return abs(self.epsilon - other.epsilon) < TINY and abs(self.rmin - other.rmin) < TINY and abs(self.epsilon_14 - other.epsilon_14) < TINY and abs(self.rmin_14 - other.rmin_14) < TINY and self.nbfix == other.nbfix
            else:
    ...
    @sigma.setter
    def sigma(self, value):
        self.rmin = value * 1.122462048309373 / 2
    ...
    def sigma_14(self):
        """ Sigma is Rmin / 2^(1/6) """
        return self.rmin_14 * 0.8908987181403393 * 2

    @sigma_14.setter
    def sigma_14(self, value):
        self.rmin_14 = value * 1.122462048309373 / 2
    ...
    def __copy__(self):
        cp = AtomType((self.name), (self.number), (self.mass), (self.atomic_number), bond_type=(self._bond_type),
          charge=(self.charge))
...
class _UnassignedAtomType(object):
    ...
    def __eq__(self, other):
        return isinstance(other, _UnassignedAtomType)
...
NoUreyBradley = BondType(0.0, 0.0)