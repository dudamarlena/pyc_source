...
# [GCC 8.4.0]
# Embedded file name: /home/swails/src/ParmEd/parmed/utils/netcdf.py
# Compiled at: 2018-07-20 20:33:33
# Size of source mod 2**32: 39498 bytes
"""
...
__all__ = [
 'netcdf_file']
import sys, warnings, weakref
from operator import mul
from collections import OrderedDict
import mmap as mm, numpy as np
...
class netcdf_file(object):
        ...
        if mode not in 'rwa':
            ...
            if hasattr(filename, 'seek'):
                ...
                if mmap is None:
                    ...
                    if not hasattr(filename, 'fileno'):
    ...
    def createVariable(self, name, type, dimensions):
        ...
        self.variables[name] = netcdf_variable(data,
          typecode, size, shape, dimensions, maskandscale=(self.maskandscale))
    ...
    def _write_var_data(self, name):
        ...
        if not var.isrec:
            ...
            for rec in var.data:
                if not rec.shape:
                    if rec.dtype.byteorder == '<' or rec.dtype.byteorder == '=' and LITTLE_ENDIAN:
                        rec = rec.byteswap()
    ...
    def _write_att_values(self, values):
        ...
        if not values.shape:
            if values.dtype.byteorder == '<' or values.dtype.byteorder == '=' and LITTLE_ENDIAN:
        ...
        for var in range(count):
            ...
            if shape and shape[0] is None:
                ...
                if self.use_mmap:
                    ...
                    self.fp.seek(pos)
            self.variables[name] = netcdf_variable(data,
              typecode, size, shape, dimensions, attributes, maskandscale=(self.maskandscale))
    ...
    def _read_var(self):
        ...
        dtype_ = '>%s' % typecode
        return (
         name, dimensions, shape, attributes, typecode, size, dtype_, begin, vsize)
...
class netcdf_variable(object):
    ...
    def _get_encoded_fill_value(self):
        ...
        if '_FillValue' in self._attributes:
            fill_value = np.array((self._attributes['_FillValue']), dtype=(self.data.dtype)).tostring()
    ...
    def _get_missing_value(self):
        ...
        if '_FillValue' in self._attributes:
            ...
            if 'missing_value' in self._attributes:
