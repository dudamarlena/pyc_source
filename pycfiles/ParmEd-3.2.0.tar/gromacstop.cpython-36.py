...
# [GCC 8.4.0]
# Embedded file name: /home/swails/src/ParmEd/parmed/gromacs/gromacstop.py
# Compiled at: 2019-03-25 16:13:20
# Size of source mod 2**32: 101229 bytes
"""
...
_sectionre = re.compile('\\[ (\\w+) \\]\\s*$')

class _Defaults(object):
    __doc__ = ' Global properties of force fields as implemented in GROMACS '
    ...
    def __repr__(self):
        return '<_Defaults: nbfunc=%d, comb-rule=%d, gen-pairs="%s", fudgeLJ=%g, fudgeQQ=%g>' % (
         self.nbfunc, self.comb_rule,
    ...
    def __eq__(self, other):
        return self.nbfunc == other.nbfunc and self.comb_rule == other.comb_rule and self.gen_pairs == other.gen_pairs and self.fudgeLJ == other.fudgeLJ and self.fudgeQQ == other.fudgeQQ
...
class GromacsTopologyFile(Structure):
                ...
                if not line.strip():
                    ...
                    if line.startswith('#'):
                        ...
                        return sec in ('atoms', 'atomtypes', 'defaults', 'moleculetype',
                                       ...
                                       'cmaps', 'molecules', 'exclusions', 'nonbond_params',
                                       'position_restraints')
    ...
    def __init__(self, fname=None, defines=None, parametrize=True, xyz=None, box=None):
        ...
        self.defaults = _Defaults(gen_pairs='yes')
            ...
            if xyz is not None:
                if isinstance(xyz, string_types):
                    ...
                    if not hasattr(f, 'coordinates') or f.coordinates is None:
                        raise TypeError('File %s does not have coordinates' % xyz)
    ...
    def read(self, fname, defines=None, parametrize=True):
                ...
                if line[0] == '[':
                    ...
                    if current_section == 'moleculetype':
                        ...
                        if molname in molecules:
                            raise GromacsError('Duplicate definition of molecule %s' % molname)
                        molecule = Structure()
                            ...
                            if current_section == 'bonds':
                                ...
                                if current_section == 'pairs':
                                    ...
                                    if current_section == 'angles':
                                        ang, ub, angt, ubt = self._parse_angles(line, angle_types, ub_types, molecule.atoms)
                                        ...
                                        if current_section == 'dihedrals':
                                            self._parse_dihedrals(line, dihedral_types, proper_multiterm_dihedrals, molecule)
                                        else:
                                            if current_section == 'cmap':
                                                ...
                                                if current_section == 'system':
                                                    ...
                                                    if current_section == 'defaults':
                                                        ...
                                                        if words[0] != '1':
                                                            warnings.warn('Unsupported nonbonded type; unknown functional', GromacsWarning)
                                                        ...
                                                        if current_section == 'molecules':
                                                            ...
                                                            if current_section == 'settles':
                                                                ...
                                                                if current_section in ('virtual_sites3',
                                                                                       'dummies3'):
                                                                    try:
                                                                        b, bt = self._parse_vsites3(line, molecule.atoms, params)
                                                                    except KeyError:
                                                                        raise GromacsError('Cannot determine vsite geometry without parameter types')
                                                                    ...
                                                                    if current_section == 'exclusions':
                                                                        ...
                                                                        if current_section == 'atomtypes':
                                                                            ...
                                                                            if current_section == 'nonbond_params':
                                                                                ...
                                                                                params.nbfix_types[(a1, a2)] = (eps, sig * 1.122462048309373)
                                                                                params.nbfix_types[(a2, a1)] = (eps, sig * 1.122462048309373)
                                                                                params.atom_types[a1].add_nbfix(a2, sig * 1.122462048309373, eps)
                                                                                params.atom_types[a2].add_nbfix(a1, sig * 1.122462048309373, eps)
                                                                            else:
        ...
        for molname, num in structure_contents:
            if molname not in molecules:
                raise GromacsError('Structure contains %s molecules, but no template defined' % molname)
            else:
                molecule, nrexcl = molecules[molname]
                if nrexcl < 3:
                    if _any_atoms_farther_than(molecule, nrexcl):
                        warnings.warn('nrexcl %d not currently supported' % nrexcl, GromacsWarning)
                if nrexcl > 3:
                    if _any_atoms_farther_than(molecule, 3):
                        warnings.warn('nrexcl %d not currently supported' % nrexcl, GromacsWarning)
                if num == 0:
                    warnings.warn('Detected addition of 0 %s molecules in topology file' % molname, GromacsWarning)
                if num == 1:
    ...
    def _parse_atoms(self, line, params):
        ...
        if attype is not None:
            ...
            if atomic_number == 0:
                ...
                atom = Atom(atomic_number=atomic_number, name=(words[4]), type=(words[1]),
                  charge=charge,
                  mass=mass)
        return (
         atom, words[3], int(words[2]))
    ...
    def _parse_bonds(self, line, bond_types, atoms):
        ...
        funct = int(words[2])
        if funct != 1:
            warnings.warn('bond funct != 1; unknown functional', GromacsWarning)
            self.unknown_functional = True
        ...
        if len(words) >= 5:
            if funct == 1:
                ...
                if (req, k) in bond_types:
                    ...
                    bond_type = BondType(k * u.kilojoule_per_mole / u.nanometer ** 2 / 2, req * u.nanometer)
    ...
    def _parse_pairs(self, line, exc_types, atoms):
        ...
        funct = int(words[2])
        if funct != 1:
            warnings.warn('pairs funct != 1; unknown functional', GromacsWarning)
            self.unknown_functional = True
        ...
        if funct == 1:
            if len(words) >= 5:
                sig = float(words[3]) * 1.122462048309373
                eps = float(words[4])
                if (sig, eps) in exc_types:
                    nbe.type = exc_types[(sig, eps)]
                else:
                    nbet = NonbondedExceptionType(sig * u.nanometers, eps * u.kilojoules_per_mole, self.defaults.fudgeQQ)
    ...
    def _parse_angles(self, line, angle_types, ub_types, atoms):
        ...
        funct = int(words[3])
        if funct not in (1, 5):
            warnings.warn('angles funct != 1 or 5; unknown functional', GromacsWarning)
            self.unknown_functional = True
        angt = ub = ubt = None
        ...
        if funct == 1 and len(words) >= 6 or funct == 5 and len(words) >= 8:
            ...
            if (theteq, k) in angle_types:
                ...
                angt = AngleType(k * u.kilojoule_per_mole / u.radian ** 2 / 2, theteq * u.degree)
        ...
        if funct == 5:
            if len(words) >= 8:
                ...
                if ubk > 0:
                    if (
                     ubreq, ubk) in ub_types:
                        ub.type = ub_types[(ubreq, ubk)]
                    else:
                        ubt = BondType(ubk * u.kilojoule_per_mole / u.nanometer ** 2 / 2, ubreq * u.nanometer)
    ...
    def _parse_dihedrals(self, line, dihedral_types, PMD, molecule):
        ...
        if funct in (1, 4) or funct == 9 and len(words) < 8:
            dih, diht = self._process_normal_dihedral(words, molecule.atoms, i, j, k, l, dihedral_types, funct == 4)
            ...
            if funct == 2:
                dih, impt = self._process_improper(words, i, j, k, l, molecule.atoms, dihedral_types)
                ...
                if funct == 3:
                    dih, rbt = self._process_rbtorsion(words, i, j, k, l, molecule.atoms, dihedral_types)
                    ...
                    if funct == 9:
                        ...
                        warnings.warn('torsions funct != 1, 2, 3, 4, 9; unknown functional', GromacsWarning)
                        dih = Dihedral(molecule.atoms[i], molecule.atoms[j], molecule.atoms[k], molecule.atoms[l])
                        molecule.dihedrals.append(dih)
                        self.unknown_functional = True
        if dih is not None:
            dih.funct = funct
    ...
    def _parse_cmaps(self, line, atoms):
        ...
        funct = int(words[5])
        if funct != 1:
            warnings.warn('cmap funct != 1; unknown functional', GromacsWarning)
            self.unknown_functional = True
    ...
    def _parse_settles(self, line, atoms):
        ...
        nm = u.nanometers
        bt_oh = BondType(500000.0 * u.kilojoules_per_mole / nm ** 2, doh * nm)
        bt_hh = BondType(500000.0 * u.kilojoules_per_mole / nm ** 2, dhh * nm)
        return (
         [Bond(oxy, hyd1, bt_oh), Bond(oxy, hyd2, bt_oh),
    ...
    def _parse_vsites3(self, line, all_atoms, params):
        ...
        for bond in parent.bonds:
            if atoms[1] in bond:
                key = (
                 _gettype(parent), _gettype(atoms[1]))
                kws['dp1'] = (bond.type or params.bond_types[key]).req
            if atoms[2] in bond:
                key = (
                 _gettype(bond.atom1), _gettype(bond.atom2))
        ...
        for angle in parent.angles:
            if parent is not angle.atom2:
                ...
                if not atoms[0] not in angle:
                    if atoms[1] not in angle:
                        pass
                    else:
                        key = (
                         _gettype(angle.atom1), _gettype(angle.atom2),
    ...
    def _parse_atomtypes(self, line):
        ...
        attype = words[0]
        if len(words[3]) == 1:
            if words[3] in letters:
                atnum = -1
                sigidx = 4
                massidx = 1
                bond_type = None
        if len(words[5]) == 1 and words[5] in letters:
            sigidx = 6
            massidx = 3
            atnum = int(words[2])
            bond_type = words[1]
        ...
        typ.set_lj_params(eps, sig * 1.122462048309373 / 2)
        return (attype, typ)
    ...
    def _parse_angletypes(self, line):
        ...
        if words[3] == '5':
            ...
            if cub == 0:
                ...
                ub = BondType(cub, ub0)
        return (
         words[0], words[1], words[2], AngleType(k, theta), ub)
        ...
        if words[si] == '2':
            ...
            if words[si] == '3':
                ...
                if dtype == 'normal':
                    ...
                    per = int(words[(si + 3)])
                    ptype = DihedralType(phi_k, per, phase, scee=(1 / self.defaults.fudgeQQ),
                    ...
                    if dtype == 'improper':
                        ...
                        ptype = RBTorsionType(c0, c1, c2, c3, c4, c5, scee=(1 / self.defaults.fudgeQQ),
                          scnb=(1 / self.defaults.fudgeLJ))
        return (
         (
          a1, a2, a3, a4), dtype, ptype, replace)
    ...
    def _process_normal_dihedral(self, words, atoms, i, j, k, l, dihedral_types, imp):
        ...
        if len(words) >= 8:
            ...
            if (phase, phi_k, per) in dihedral_types:
                ...
                diht = DihedralType((phi_k * u.kilojoule_per_mole), per,
                  (phase * u.degrees), scee=(1 / self.defaults.fudgeQQ),
                  scnb=(1 / self.defaults.fudgeLJ))
    ...
    def _process_dihedral_series(self, words, dihtype=None):
        phase, phi_k, per = (float(x) for x in words[5:8])
        dt = DihedralType((phi_k * u.kilojoule_per_mole), per,
          (phase * u.degrees), scee=(1 / self.defaults.fudgeQQ),
          scnb=(1 / self.defaults.fudgeLJ))
    ...
    def _process_improper(self, words, i, j, k, l, atoms, dihedral_types):
        ...
        if len(words) >= 7:
            ...
            if (psieq, k) in dihedral_types:
                ...
                impt = ImproperType(k * u.kilojoule_per_mole / u.radian ** 2 / 2, psieq * u.degree)
    ...
    def _process_rbtorsion(self, words, i, j, k, l, atoms, dihedral_types):
        ...
        if len(words) >= 11:
            ...
            if (c0, c1, c2, c3, c4, c5) in dihedral_types:
                ...
                rbt = RBTorsionType((c0 * kjpm), (c1 * kjpm), (c2 * kjpm), (c3 * kjpm),
                  (c4 * kjpm), (c5 * kjpm), scee=(1 / self.defaults.fudgeQQ),
                  scnb=(1 / self.defaults.fudgeLJ))
        ...
        *after* `read`
        ...
        if self.parameterset is None:
            ...
            for pair in self.adjusts:
                ...
                if pair.type is not None:
                    ...
                    key = (
                     _gettype(pair.atom1), _gettype(pair.atom2))
                    if key in params.pair_types:
                        pair.type = params.pair_types[key]
                        pair.type.used = True
                    else:
                        if self.defaults.gen_pairs == 'yes':
                            ...
                            pairtype = NonbondedExceptionType((sig * 1.122462048309373), eps, (self.defaults.fudgeQQ),
                ...
                for bpi in bond.atom1.bond_partners:
                    for bpj in bond.atom2.bond_partners:
                        if len(set([bpi, bond.atom1, bond.atom2, bpj])) < 4:
                            continue
                        if not bpi in bpj.bond_partners:
                            if bpi in bpj.angle_partners:
                                continue
                            else:
                                if bpi > bpj:
                                    true_14.add((bpj, bpi))
            ...
            if len(true_14 - gmx_pair) > 0:
                ...
                warnings.warn("%i 1-4 pairs were missing from the [ pairs ] section and were set to zero; make sure you know what you're doing!" % num_zero_14, GromacsWarning)
            if len(gmx_pair - true_14) > 0:
                warnings.warn("The [ pairs ] section contains %i exceptions that aren't 1-4 pairs; make sure you know what you're doing!" % len(gmx_pair - true_14), GromacsWarning)
        ...
        for t in self.dihedrals:
            if t.type is not None:
                continue
            key = (
             _gettype(t.atom1), _gettype(t.atom2), _gettype(t.atom3),
             _gettype(t.atom4))
            if not t.improper:
                wckey = (
                 'X', _gettype(t.atom2), _gettype(t.atom3), 'X')
                ...
                if key in params.dihedral_types:
                    ...
                    if wckey1 in params.dihedral_types:
                        t.type = params.dihedral_types[wckey1]
                        t.type.used = True
                    else:
                        if wckey2 in params.dihedral_types:
                            t.type = params.dihedral_types[wckey2]
                            t.type.used = True
                        else:
                            if wckey in params.dihedral_types:
        ...
        for t in self.rb_torsions:
            if t.type is not None:
                ...
                if key in params.rb_torsion_types:
                    ...
                    if wckey1 in params.rb_torsion_types:
                        t.type = params.rb_torsion_types[wckey1]
                        t.type.used = True
                    else:
                        if wckey2 in params.rb_torsion_types:
                            t.type = params.rb_torsion_types[wckey2]
                            t.type.used = True
                        else:
                            if wckey in params.rb_torsion_types:
        ...
        for c in self.cmaps:
            if c.type is not None:
                ...
                key = (
                 _gettype(c.atom1), _gettype(c.atom2), _gettype(c.atom3),
    ...
    def from_structure(cls, struct, copy=False):
        ...
        if copy:
            ...
            if struct.trigonal_angles or struct.out_of_plane_bends or struct.pi_torsions or struct.stretch_bends or struct.torsion_torsions or struct.chiral_frames or struct.multipole_frames:
                raise TypeError('GromacsTopologyFile does not support Amoeba FF')
            if hasattr(struct, 'defaults'):
                if isinstance(struct.defaults, _Defaults):
        ...
        gmxtop.parameterset = ParameterSet.from_structure(struct, allow_unequal_duplicates=True)
        return gmxtop
    ...
    def write(self, dest, combine=None, parameters='inline', molfile=None, itp=False):
        ...
        if itp:
            ...
            if molfile is None:
                ...
                if isinstance(molfile, string_types):
        ...
        try:
            ...
            if not itp:
                ...
                if any(typ._bond_type is not None for key, typ in iteritems(params.atom_types)):
                    print_bond_types = True
                else:
                    print_bond_types = False
                if all(typ.atomic_number != -1 for key, typ in iteritems(params.atom_types)):
                    print_atnum = True
                else:
                    print_atnum = False
                parfile.write('; name    ')
                ...
                if self.has_NBFIX():
                    ...
                    for key, val in typemap.items():
                        if key[0] in types_in_system and key[1] in types_in_system:
                            ...
                            eps *= eps_conversion
                            sig *= 0.1
                            dest.write('{0} {1} 1 {2} {3}\n'.format(key[0], key[1], sig / 1.122462048309373, eps))
                ...
                if params.cmap_types:
                    ...
                    for key, param in iteritems(params.cmap_types):
                        if key in used_keys:
                            ...
                            used_keys.add(key)
                            used_keys.add(tuple(reversed(key)))
                            parfile.write('%-6s %-6s %-6s %-6s %-6s   1   %4d %4d' % (
                             key[0], key[1], key[2],
            ...
            if combine is None:
                ...
                if not itp:
                    ...
                    while i < total_mols:
                        for j, (molecule, lst) in enumerate(molecules):
                            ...
                            raise AssertionError('Could not find molecule %d in list' % i)
                ...
                for cl in combine_lists:
                    ...
                    for i, mol in enumerate(mols_in_mol):
                        if i == 0:
                            pass
                        elif not (id(mol) in counts and counts[id(mol)] > 0):
                ...
                for i in range(nmols):
                    ii = i + add
                    if cmc < len(combined_molecules):
                        if combined_molecules[cmc][1] == ii:
                            new_molecules.append([combined_molecules[cmc][0],
                             set([i])])
                            add += combined_molecules[cmc][2] - 1
                            cmc += 1
                    if len(counts[id(moleculedict[ii])]) == 0:
    ...
    def _write_molecule(struct, dest, title, params, writeparams):
        ...
        for residue in struct.residues:
            ...
            for atom in residue:
                runchg += atom.charge
                dest.write('%5d %10s %6d %6s %6s %6d %10.8f %10.6f   ; qtot %.6f\n' % (
        ...
        if struct.bonds:
            conv = (u.kilocalorie_per_mole / u.angstrom ** 2).conversion_factor_to(u.kilojoule_per_mole / u.nanometer ** 2) * 2
            if settle:
            ...
            for bond in struct.bonds:
                if not isinstance(bond.atom1, ExtraPoint):
        ...
        if struct.adjusts:
            ...
            for adjust in struct.adjusts:
                key = (
                 _gettype(adjust.atom1), _gettype(adjust.atom2))
                dest.write('%7d %6d %5d' % (adjust.atom1.idx + 1, adjust.atom2.idx + 1, adjust.funct))
                if struct.defaults.gen_pairs == 'no':
                    if writeparams or key not in params.pair_types or adjust.type != params.pair_types[key]:
                        if adjust.type is not None:
            ...
            if struct.dihedrals:
                ...
                for dihed in struct.dihedrals:
                    if not dihed.ignore_end:
                        if dihed.improper:
                            continue
                        a1, a2 = dihed.atom1, dihed.atom4
                        if not a1 in a2.bond_partners:
                            if a1 in a2.angle_partners:
                                continue
                ...
                dest.write('\n')
        if struct.angles:
            conv = (u.kilocalorie_per_mole / u.radian ** 2).conversion_factor_to(u.kilojoule_per_mole / u.radian ** 2) * 2
            conv2 = (u.kilocalorie_per_mole / u.angstrom ** 2).conversion_factor_to(u.kilojoule_per_mole / u.nanometer ** 2) * 2
            dest.write('[ angles ]\n')
            ...
            for angle in struct.angles:
                ...
                if angle.type is None:
                    ...
                    continue
                key = (
                 _gettype(angle.atom1), _gettype(angle.atom2), _gettype(angle.atom3))
        ...
        if struct.dihedrals:
            ...
            dest.write((';%6s %6s %6s %6s %5s' + ' %10s' * 6) % ('ai', 'aj', 'ak',
                                                                 'al', 'funct', 'c0',
                                                                 'c1', 'c2', 'c3',
                                                                 'c4', 'c5'))
            dest.write('\n')
            ...
            for dihed in struct.dihedrals:
                ...
                key = (
                 _gettype(dihed.atom1), _gettype(dihed.atom2),
        ...
        if struct.rb_torsions:
            ...
            for dihed in struct.rb_torsions:
                ...
                key = (
                 _gettype(dihed.atom1), _gettype(dihed.atom2),
                 _gettype(dihed.atom3), _gettype(dihed.atom4))
                if writeparams or key not in params.rb_torsion_types or params.rb_torsion_types[key] != dihed.type:
        ...
        if struct.impropers:
            dest.write('[ dihedrals ]\n')
            dest.write(';%6s %6s %6s %6s %5s %10s %10s %10s %10s' % ('ai', 'aj', 'ak',
                                                                     'al', 'funct',
                                                                     'c0', 'c1',
                                                                     'c2', 'c3'))
            dest.write('\n')
        ...
        if EPs:
            ...
            for ftype in ftypes:
                if ftype is TwoParticleExtraPointFrame:
                    ...
                    if ftype in (ThreeParticleExtraPointFrame,
                     OutOfPlaneExtraPointFrame):
                        dest.write('[ virtual_sites3 ]\n')
                        dest.write('; Site  from                   funct\n')
                        for EP in EPs:
                            if isinstance(EP.frame_type, ThreeParticleExtraPointFrame):
                                ...
                                dest.write('%-5d %-4d %-4d %-4d %-4d   %.6f  %.6f\n' % (
                                 ...
                                 1, w1, w2))
                            elif isinstance(EP.frame_type, OutOfPlaneExtraPointFrame):
                                a1, a2, a3 = EP.frame_type.get_atoms()
...
def _diff_diheds(dt1, dt2):
    ...
    """
    if type(dt1) is type(dt2):
        if dt1 == dt2:
            return False
    if isinstance(dt2, DihedralTypeList):
        if isinstance(dt1, DihedralType):
            if len(dt2) == 1:
                if dt2[0] == dt1:
                    return False
    return True
