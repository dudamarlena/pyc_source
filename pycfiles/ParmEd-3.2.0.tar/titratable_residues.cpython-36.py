...
# [GCC 8.4.0]
# Embedded file name: /home/swails/src/ParmEd/parmed/amber/titratable_residues.py
# Compiled at: 2018-11-20 11:47:27
# Size of source mod 2**32: 60816 bytes
"""
...
titratable_residues = [
 'AS4', 'GL4', 'CYS', 'TYR', 'HIP', 'LYS', 'DAP', 'DCP',
...
class _ReferenceEnergy(object):
    __doc__ = ' Reference energies for various solvent models '
    LN_TO_LOG = log(10.0)
    KB = 0.00199
    TEMP = 300.0
...
class _LineBuffer(object):
    __doc__ = ' Buffer to add lines to the cpin file '
    CHARS_PER_LINE = 80
...
class TitratableResidue(object):
    ...
    def __str__(self):
        if self.typ == 'ph':
            ...
            for i, atom in enumerate(self.atom_list):
                ret_str += '%8s' % atom + ''.join(['%12.4f' % state.charges[i] for state in self.states]) + '\n'
            ...
            if self.typ == 'redox' or self.typ == 'phredox':
                ret_str += '%8s' % 'Elec Cnt' + ''.join(['%12d' % state.eleccnt for state in self.states]) + '\n'
                ret_str += '-' * (8 + 12 * len(self.states)) + '\n'
    ...
    def add_states(self, charges, refenes, refenes_old=None, protcnts=None, pka_corrs=None, eleccnts=None, eo_corrs=None):
        ...
        if len(charges) != len(refenes) or len(charges) != len(refenes_old) and refenes_old or len(charges) != len(protcnts) and protcnts or len(charges) != len(pka_corrs) and pka_corrs or len(charges) != len(eleccnts) and eleccnts or len(charges) != len(eo_corrs) and eo_corrs:
    ...
    def cpin_pointers(self, first_atom):
        ...
        return {'FIRST_ATOM':first_atom,  'FIRST_CHARGE':self.first_charge, 
    ...
    def check(self):
        ...
        for i in range(1, len(sum_charges)):
            ...
            if abs(charge_diff - diff) >= 0.0001:
                warnings.warn('Inconsistencies detected in charge definitions in %s' % self.resname, AmberWarning)
        ...
        if notset != len(self.states) - 1:
            if valid:
                warnings.warn('Not enough states are pKa-adjusted in %s' % self.resname)


class TitratableResidueList(list):
    __doc__ = ' List of all titratable residues '
    ...
    def set_states(self, statelist):
        ...
        if len(statelist) != len(self):
            warnings.warn('Number of states (%d) does not equal number of residues (%d). Using default initial states.' % (
             len(statelist), len(self)), AmberWarning)
            return
        for i, state in enumerate(statelist):
            if state < 0 or state >= len(self[i].states):
                raise AmberError('Bad state choice (%d). Minimum is 0, maximum is %d' % (
                 state, len(self[i].states)))
        ...
        while nswaps > 0:
            ...
            for i in range(len(self) - 1):
                if self.first_atoms[i] > self.first_atoms[(i + 1)]:
                    nswaps += 1
                    self.first_atoms[i], self.first_atoms[i + 1] = self.first_atoms[(i + 1)], self.first_atoms[i]
                    self[i], self[i + 1] = self[(i + 1)], self[i]
                    self.residue_nums[i], self.residue_nums[i + 1] = self.residue_nums[(i + 1)], self.residue_nums[i]
    ...
    def write_cpin(self, output, igb=2, intdiel=1.0, oldfmt=False, typ='ph', coions=False):
        ...
        for i, energy in enumerate(energies):
            if energy is None:
                raise AmberError("%d'th reference energy not known for igb = %d" % (
                 i, igb))
...
refene2 = _ReferenceEnergy(igb1=21.4298008, igb2=26.8894581, igb5=26.5980488, igb7=23.4181107,
  igb8=26.3448911)
refene2.solvent_energies(igb2=33.2613028, igb5=32.064349, igb7=28.26235, igb8=31.286037)
refene2.dielc2_energies(igb2=12.676908, igb5=13.084913)
refene2.dielc2.solvent_energies()
refene2_old = _ReferenceEnergy(igb1=21.4298008, igb2=26.8894581, igb5=26.5980488, igb7=23.4181107,
  igb8=26.3448911)
refene2_old.solvent_energies(igb2=33.2613028, igb5=32.064349, igb7=28.26235, igb8=31.286037)
...
AS4 = TitratableResidue('AS4', ['N', 'H', 'CA', 'HA', 'CB', 'HB2', 'HB3', 'CG',
 'OD1', 'OD2', 'HD21', 'C', 'O', 'HD22', 'HD11', 'HD12'],
  pka=4.0,
  typ='ph')
AS4.add_state(protcnt=0, refene=refene1, refene_old=refene1, pka_corr=0.0, charges=[
 -0.4157, 0.2719, 0.0341, 0.0864, -0.1783, -0.0122,
 -0.0122, 0.7994, -0.8014, -0.8014, 0.0, 0.5973, -0.5679, 0.0, 0.0,
 0.0])
AS4.add_state(protcnt=1, refene=refene2, refene_old=refene2_old, pka_corr=4.0, charges=[
 -0.4157, 0.2719, 0.0341, 0.0864, -0.0316, 0.0488, 0.0488,
...
refene1.dielc2_energies(igb1=0, igb2=0, igb5=0, igb7=0, igb8=0)
refene1.dielc2.solvent_energies(igb1=0, igb2=0, igb5=0, igb7=0, igb8=0)
refene2 = _ReferenceEnergy(igb1=3.89691326, igb2=8.4057785, igb5=8.0855764, igb7=5.305949,
  igb8=8.3591335)
refene2.solvent_energies(igb2=15.20019319, igb5=13.533409, igb7=10.682953, igb8=13.24241)
refene2.dielc2_energies(igb2=3.455596, igb5=3.95727)
refene2.dielc2.solvent_energies()
refene2_old = _ReferenceEnergy(igb1=3.89691326, igb2=8.4057785, igb5=8.0855764, igb7=5.305949,
  igb8=8.3591335)
refene2_old.solvent_energies(igb2=15.20019319, igb5=13.533409, igb7=10.682953, igb8=13.24241)
refene2_old.dielc2_energies(igb2=3.455596, igb5=3.95727)
refene2_old.dielc2.solvent_energies()
...
GL4 = TitratableResidue('GL4', ['N', 'H', 'CA', 'HA', 'CB', 'HB2', 'HB3', 'CG',
 ...
 'HE22', 'HE11', 'HE12'],
  pka=4.4, typ='ph')
GL4.add_state(protcnt=0, refene=refene1, refene_old=refene1, pka_corr=0.0, charges=[
 -0.4157, 0.2719, 0.0145, 0.0779, -0.0398, -0.0173,
 -0.0173, 0.0136, -0.0425, -0.0425, 0.8054, -0.8188, -0.8188, 0.0,
 0.5973, -0.5679, 0.0, 0.0, 0.0])
GL4.add_state(protcnt=1, refene=refene2, refene_old=refene2_old, pka_corr=4.4, charges=[
 -0.4157, 0.2719, 0.0145, 0.0779, -0.0071, 0.0256, 0.0256,
 -0.0174, 0.043, 0.043, 0.6801, -0.5838, -0.6511, 0.4641, 0.5973,
 -0.5679, 0.0, 0.0, 0.0])
GL4.add_state(protcnt=1, refene=refene2, refene_old=refene2_old, pka_corr=4.4, charges=[
 -0.4157, 0.2719, 0.0145, 0.0779, -0.0071, 0.0256, 0.0256,
 -0.0174, 0.043, 0.043, 0.6801, -0.5838, -0.6511, 0.0, 0.5973,
 -0.5679, 0.4641, 0.0, 0.0])
GL4.add_state(protcnt=1, refene=refene2, refene_old=refene2_old, pka_corr=4.4, charges=[
 -0.4157, 0.2719, 0.0145, 0.0779, -0.0071, 0.0256, 0.0256,
 -0.0174, 0.043, 0.043, 0.6801, -0.6511, -0.5838, 0.0, 0.5973,
...
refene2.dielc2_energies(igb2=(-32.16752), igb5=(-31.751177))
refene2.dielc2.solvent_energies()
...
refene2_old.dielc2_energies(igb2=(-32.16752), igb5=(-31.751177))
refene2_old.dielc2.solvent_energies()
...
TYR = TitratableResidue('TYR', ['N', 'H', 'CA', 'HA', 'CB', 'HB2', 'HB3', 'CG',
 ...
 'HE2', 'CD2', 'HD2', 'C', 'O'],
  pka=9.6, typ='ph')
TYR.add_state(protcnt=1, refene=refene1, refene_old=refene1, pka_corr=9.6, charges=[
 ...
 -0.5679])
TYR.add_state(protcnt=0, refene=refene2, refene_old=refene2_old, pka_corr=0.0, charges=[
 -0.4157, 0.2719, -0.0014, 0.0876, -0.0858, 0.019,
 0.019, -0.213, -0.103, 0.132, -0.498, 0.132, 0.777,
...
refene1.dielc2_energies(igb1=0, igb2=0, igb5=0, igb7=0, igb8=0)
refene1.dielc2.solvent_energies(igb1=0, igb2=0, igb5=0, igb7=0, igb8=0)
refene2 = _ReferenceEnergy(igb1=(-4.208863), igb2=(-2.84183), igb5=(-2.86001), igb7=(-1.741947),
  igb8=(-3.4))
refene2.solvent_energies(igb2=(-2.77641), igb5=(-2.90517))
refene2.dielc2_energies(igb2=(-1.62811), igb5=(-1.691093))
refene2.dielc2.solvent_energies()
refene2_old = _ReferenceEnergy(igb1=(-4.208863), igb2=(-2.84183), igb5=(-2.86001), igb7=(-1.741947),
  igb8=(-3.4))
refene2_old.solvent_energies(igb2=(-2.77641), igb5=(-2.90517))
refene2_old.dielc2_energies(igb2=(-1.62811), igb5=(-1.691093))
refene2_old.dielc2.solvent_energies()
refene2_old.set_pKa(6.5, deprotonated=True)
refene3 = _ReferenceEnergy(igb1=(-8.230643), igb2=(-6.58793), igb5=(-6.70726), igb7=(-5.118453),
  igb8=(-6.319))
refene3.solvent_energies(igb2=(-6.48363), igb5=(-6.82684))
refene3.dielc2_energies(igb2=(-3.4442), igb5=(-3.070113))
refene3.dielc2.solvent_energies()
refene3_old = _ReferenceEnergy(igb1=(-8.230643), igb2=(-6.58793), igb5=(-6.70726), igb7=(-5.118453),
  igb8=(-6.319))
refene3_old.solvent_energies(igb2=(-6.48363), igb5=(-6.82684))
refene3_old.dielc2_energies(igb2=(-3.4442), igb5=(-3.070113))
...
HIP = TitratableResidue('HIP', ['N', 'H', 'CA', 'HA', 'CB', 'HB2', 'HB3', 'CG',
 ...
 'C', 'O'],
  pka=6.6, typ='ph')
HIP.add_state(protcnt=2, refene=refene1, refene_old=refene1, pka_corr=7.1, charges=[
 -0.3479, 0.2747, -0.1354, 0.1212, -0.0414, 0.081,
 0.081, -0.0012, -0.1513, 0.3866, -0.017, 0.2681, -0.1718,
 0.3911, -0.1141, 0.2317, 0.7341, -0.5894])
HIP.add_state(protcnt=1, refene=refene2, refene_old=refene2_old, pka_corr=0.6, charges=[
 -0.3479, 0.2747, -0.1354, 0.1212, -0.111, 0.0402,
 0.0402, -0.0266, -0.3811, 0.3649, 0.2057, 0.1392, -0.5727, 0.0,
 0.1292, 0.1147, 0.7341, -0.5894])
HIP.add_state(protcnt=1, refene=refene3, refene_old=refene3_old, pka_corr=0.0, charges=[
 -0.3479, 0.2747, -0.1354, 0.1212, -0.1012, 0.0367,
...
LYS = TitratableResidue('LYS', ['N', 'H', 'CA', 'HA', 'CB', 'HB2', 'HB3', 'CG',
 ...
 'NZ', 'HZ1', 'HZ2', 'HZ3', 'C', 'O'],
  pka=10.4, typ='ph')
LYS.add_state(protcnt=3, refene=refene1, refene_old=refene1_old, pka_corr=10.4, charges=[
 -0.3479, 0.2747, -0.24, 0.1426, -0.0094, 0.0362,
 0.0362, 0.0187, 0.0103, 0.0103, -0.0479, 0.0621, 0.0621, -0.0143,
 0.1135, 0.1135, -0.3854, 0.34, 0.34, 0.34, 0.7341, -0.5894])
LYS.add_state(protcnt=2, refene=refene2, refene_old=refene2, pka_corr=0.0, charges=[
 -0.3479, 0.2747, -0.24, 0.1426, -0.10961, 0.034,
 0.034, 0.06612, 0.01041, 0.01041, -0.03768, 0.01155, 0.01155,
...
refene1.dielc2.solvent_energies(igb2=38.48917)
...
refene1_old.dielc2.solvent_energies(igb2=38.48917)
...
CYS = TitratableResidue('CYS', ['N', 'H', 'CA', 'HA', 'CB', 'HB2', 'HB3', 'SG',
 'HG', 'C', 'O'],
  pka=8.5, typ='ph')
CYS.add_state(protcnt=1, refene=refene1, refene_old=refene1_old, pka_corr=8.5, charges=[
 -0.4157, 0.2719, 0.0213, 0.1124, -0.1231, 0.1112, 0.1112,
 -0.3119, 0.1933, 0.5973, -0.5679])
CYS.add_state(protcnt=0, refene=refene2, refene_old=refene2, pka_corr=0.0, charges=[
 -0.4157, 0.2719, 0.0213, 0.1124, -0.3593, 0.1122, 0.1122,
...
DAP = TitratableResidue('DAP', ['P', 'O1P', 'O2P', "O5'", "C5'", "H5'1", "H5'2",
 ...
 'H1'],
  pka=3.9, typ='ph')
DAP.add_state(protcnt=1, refene=refene1, refene_old=refene1_old, pka_corr=0.0, charges=[
 1.1659, -0.7761, -0.7761, -0.4954, -0.0069, 0.0754,
 ...
 0.0718, -0.5232, 0.0])
DAP.add_state(protcnt=2, refene=refene2, refene_old=refene2, pka_corr=3.9, charges=[
 1.1659, -0.7761, -0.7761, -0.4954, -0.0069, 0.0754,
...
DCP = TitratableResidue('DCP', ['P', 'O1P', 'O2P', "O5'", "C5'", "H5'1", "H5'2",
 ...
 "C3'", "H3'", "C2'", "H2'1", "H2'2", "O3'", 'H3'],
  pka=4.3,
  typ='ph')
DCP.add_state(protcnt=1, refene=refene1, refene_old=refene1_old, pka_corr=0.0, charges=[
 1.1659, -0.7761, -0.7761, -0.4954, -0.0069, 0.0754,
 ...
 -0.5232, 0.0])
DCP.add_state(protcnt=2, refene=refene2, refene_old=refene2, pka_corr=4.3, charges=[
 1.1659, -0.7761, -0.7761, -0.4954, -0.0069, 0.0754,
...
DG = TitratableResidue('DG', ['P', 'O1P', 'O2P', "O5'", "C5'", "H5'1", "H5'2",
 ...
 "O3'"],
  pka=9.2, typ='ph')
DG.add_state(protcnt=1, refene=refene1, refene_old=refene1, pka_corr=9.2, charges=[
 1.1659, -0.7761, -0.7761, -0.4954, -0.0069, 0.0754,
...
DG.add_state(protcnt=0, refene=refene2, refene_old=refene2_old, pka_corr=0.0, charges=[
 1.1659, -0.7761, -0.7761, -0.4954, -0.0069, 0.0754,
...
refene2.dielc2_energies(igb2=(-28.08573), igb5=(-27.29829))
refene2.dielc2.solvent_energies()
refene2_old = _ReferenceEnergy(igb2=(-56.7729), igb5=(-56.7729))
refene2_old.solvent_energies(igb2=(-28.429391))
refene2_old.dielc2_energies(igb2=(-28.08573), igb5=(-27.29829))
refene2_old.dielc2.solvent_energies()
...
DT = TitratableResidue('DT', ['P', 'O1P', 'O2P', "O5'", "C5'", "H5'1", "H5'2",
 ...
 'C2', 'O2', "C3'", "H3'", "C2'", "H2'1", "H2'2", "O3'"],
  pka=9.7,
  typ='ph')
DT.add_state(protcnt=1, refene=refene1, refene_old=refene1, pka_corr=9.7, charges=[
 1.1659, -0.7761, -0.7761, -0.4954, -0.0069, 0.0754,
 ...
 0.0718, -0.5232])
DT.add_state(protcnt=0, refene=refene2, refene_old=refene2_old, pka_corr=0.0, charges=[
 1.1659, -0.7761, -0.7761, -0.4954, -0.0069, 0.0754,
...
AP = TitratableResidue('AP', ['P', 'O1P', 'O2P', "O5'", "C5'", "H5'1", "H5'2",
 ...
 "O3'", 'H1'],
  pka=3.9, typ='ph')
AP.add_state(protcnt=0, refene=refene1, refene_old=refene1, pka_corr=0.0, charges=[
 1.1662, -0.776, -0.776, -0.4989, 0.0558, 0.0679, 0.0679,
 ...
 0.4186, -0.5246, 0.0])
AP.add_state(protcnt=1, refene=refene2, refene_old=refene2_old, pka_corr=3.5, charges=[
 1.1662, -0.776, -0.776, -0.4989, 0.0558, 0.0679, 0.0679,
...
refene2.dielc2_energies(igb2=18.483513, igb5=19.01639)
refene2.dielc2.solvent_energies()
...
refene2_old.dielc2_energies(igb2=18.483513, igb5=19.01639)
refene2_old.dielc2.solvent_energies()
...
CP = TitratableResidue('CP', ['P', 'O1P', 'O2P', "O5'", "C5'", "H5'1", "H5'2",
 ...
 "C3'", "H3'", "C2'", "H2'1", "O2'", "HO'2", "O3'", 'H3'],
  pka=4.3,
  typ='ph')
CP.add_state(protcnt=1, refene=refene1, refene_old=refene1, pka_corr=0.0, charges=[
 1.1662, -0.776, -0.776, -0.4989, 0.0558, 0.0679, 0.0679,
 ...
 0.0])
CP.add_state(protcnt=2, refene=refene2, refene_old=refene2_old, pka_corr=4.2, charges=[
 1.1662, -0.776, -0.776, -0.4989, 0.0558, 0.0679, 0.0679,
...
refene2.dielc2_energies(igb2=(-47.41098), igb5=(-47.008233))
...
refene2_old.dielc2_energies(igb2=(-47.41098), igb5=(-47.008233))
...
G = TitratableResidue('G', ['P', 'O1P', 'O2P', "O5'", "C5'", "H5'1", "H5'2",
 ...
 "O3'"],
  pka=9.2, typ='ph')
G.add_state(protcnt=1, refene=refene1, refene_old=refene1, pka_corr=9.2, charges=[
 1.1662, -0.776, -0.776, -0.4989, 0.0558, 0.0679, 0.0679,
...
G.add_state(protcnt=0, refene=refene2, refene_old=refene2_old, pka_corr=0.0, charges=[
 1.1662, -0.776, -0.776, -0.4989, 0.0558, 0.0679, 0.0679,
...
refene2.dielc2_energies(igb2=(-67.27069), igb5=(-66.60533))
refene2.dielc2.solvent_energies()
...
refene2_old.dielc2_energies(igb2=(-67.27069), igb5=(-66.60533))
refene2_old.dielc2.solvent_energies()
refene2_old.set_pKa(9.2, deprotonated=True)
U = TitratableResidue('U', ['P', 'O1P', 'O2P', "O5'", "C5'", "H5'1", "H5'2",
 ...
 "C2'", "H2'1", "O2'", "HO'2", "O3'"],
  pka=9.3, typ='ph')
U.add_state(protcnt=1, refene=refene1, refene_old=refene1, pka_corr=9.2, charges=[
 1.1662, -0.776, -0.776, -0.4989, 0.0558, 0.0679, 0.0679,
...
U.add_state(protcnt=0, refene=refene2, refene_old=refene2_old, pka_corr=0.0, charges=[
 1.1662, -0.776, -0.776, -0.4989, 0.0558, 0.0679, 0.0679,
...
refene1.dielc2_energies(igb1=0, igb2=0, igb5=0, igb7=0, igb8=0)
refene1.dielc2.solvent_energies(igb1=0, igb2=0, igb5=0, igb7=0, igb8=0)
refene2 = _ReferenceEnergy(igb2=(-15.493731), igb5=(-16.349152), igb7=(-16.509509), igb8=(-22.025653))
refene2.solvent_energies(igb2=(-15.20927), igb5=(-15.840853), igb7=(-15.495868))
...
HEH = TitratableResidue('HEH', ['FE', 'NA', 'C1A', 'C2A', 'C3A', 'CMA', 'HMA1', 'HMA2', 'HMA3', 'C4A',
 ...
 'HD22'],
  eo=(-0.203),
  typ='redox')
HEH.add_state(eleccnt=2, refene=refene1, eo_corr=0.0, charges=[
 0.666, -0.153, -0.0956, 0.1274, 0.1624, -0.26, 0.0743, 0.0743, 0.0743,
 -0.0766, -0.0586, 0.13, -0.0206, -0.256, 0.1394, -0.224, 0.0663, 0.0663,
 0.0663, 0.0734, -0.0935, 0.1765, -0.4035, 0.1375, 0.1375, 0.1375, 0.0504,
 -0.1806, 0.143, 0.0364, -0.239, 0.1784, -0.2325, 0.0635, 0.0635, 0.0635,
 0.0084, -0.057, 0.213, -0.409, 0.1357, 0.1357, 0.1357, -0.0266, -0.0576,
 0.136, -0.1156, -0.113, 0.1604, -0.2575, 0.0752, 0.0752, 0.0752, 0.1444,
 -0.1126, 0.0104, 0.109, -0.3349, 0.1297, 0.1297, -0.176, -0.0803, 0.0269,
 0.0269, 0.199, -0.293, 0.365, 0.012, 0.118, -0.04, -0.202, 0.179,
 -0.3349, 0.1297, 0.1297, -0.176, -0.0803, 0.0269, 0.0269, 0.199, -0.293,
 0.365, 0.012, 0.118, -0.04, -0.202, 0.179])
HEH.add_state(eleccnt=3, refene=refene2, eo_corr=(-0.203), charges=[
 0.48, -0.1337, -0.1455, 0.1285, 0.1325, -0.2545, 0.0608, 0.0608, 0.0608,
 -0.0865, -0.0815, 0.122, -0.0425, -0.249, 0.1605, -0.1935, 0.0422, 0.0422,
 0.0422, -0.0025, -0.039, 0.172, -0.4255, 0.1348, 0.1348, 0.1348, 0.0405,
 -0.1725, 0.132, -0.0525, -0.198, 0.2125, -0.1985, 0.0405, 0.0405, 0.0405,
 -0.0355, -0.0195, 0.1915, -0.434, 0.132, 0.132, 0.132, -0.0275, -0.0805,
 0.129, -0.1275, -0.102, 0.1335, -0.2525, 0.0615, 0.0615, 0.0615, 0.1415,
 -0.1575, 0.0275, 0.095, -0.3343, 0.13, 0.13, -0.2315, -0.0967, 0.0187,
 0.0187, 0.203, -0.345, 0.355, 0.011, 0.109, -0.027, -0.217, 0.175,
 -0.3343, 0.13, 0.13, -0.2315, -0.0967, 0.0187, 0.0187, 0.203, -0.345,
 0.355, 0.011, 0.109, -0.027, -0.217, 0.175])
HEH.check()
refene1 = _ReferenceEnergy(igb1=0, igb2=0, igb5=0, igb7=0, igb8=0)
refene1.solvent_energies(igb1=0, igb2=0, igb5=0, igb7=0, igb8=0)
refene1.dielc2_energies(igb1=0, igb2=0, igb5=0, igb7=0, igb8=0)
refene1.dielc2.solvent_energies(igb1=0, igb2=0, igb5=0, igb7=0, igb8=0)
refene2 = _ReferenceEnergy(igb2=10.356928, igb5=9.943308, igb7=7.020632, igb8=5.259028)
refene2.solvent_energies(igb2=16.751825, igb5=15.661934, igb7=12.906876, igb8=15.024553)
refene2.dielc2_energies()
refene2.dielc2.solvent_energies()
refene2_old = _ReferenceEnergy(igb2=10.356928, igb5=9.943308, igb7=7.020632, igb8=5.259028)
refene2_old.solvent_energies(igb2=16.751825, igb5=15.661934, igb7=12.906876, igb8=15.024553)
...
PRN = TitratableResidue('PRN', [
 ...
 'O1', 'O2', 'H11', 'H12', 'H21', 'H22'],
  pka=4.85, typ='ph')
PRN.add_state(protcnt=0, refene=refene1, refene_old=refene1, pka_corr=0.0, charges=[
 -0.0508, -0.0173,
 -0.0173, 0.0026, -0.0425, -0.0425, 0.8054, -0.8188, -0.8188, 0.0,
 0.0, 0.0, 0.0])
PRN.add_state(protcnt=1, refene=refene2, refene_old=refene2_old, pka_corr=4.85, charges=[
 -0.0181, 0.0256, 0.0256,
 -0.0284, 0.043, 0.043, 0.6801, -0.6511, -0.5838, 0.4641,
 0.0, 0.0, 0.0])
PRN.add_state(protcnt=1, refene=refene2, refene_old=refene2_old, pka_corr=4.85, charges=[
 -0.0181, 0.0256, 0.0256,
 -0.0284, 0.043, 0.043, 0.6801, -0.6511, -0.5838, 0.0,
 0.4641, 0.0, 0.0])
PRN.add_state(protcnt=1, refene=refene2, refene_old=refene2_old, pka_corr=4.85, charges=[
 -0.0181, 0.0256, 0.0256,
 -0.0284, 0.043, 0.043, 0.6801, -0.5838, -0.6511, 0.0,
 0.0, 0.4641, 0.0])
PRN.add_state(protcnt=1, refene=refene2, refene_old=refene2_old, pka_corr=4.85, charges=[
 -0.0181, 0.0256, 0.0256,
 -0.0284, 0.043, 0.043, 0.6801, -0.5838, -0.6511, 0.0,
...
refene1.dielc2_energies(igb2=0, igb5=0, igb8=0)
refene1.dielc2.solvent_energies(igb1=0, igb2=0, igb5=0, igb7=0, igb8=0)
refene2 = _ReferenceEnergy(igb2=5.409084)
refene2.solvent_energies(igb2=5.210075)
refene2.dielc2_energies()
refene2.dielc2.solvent_energies()
refene3 = _ReferenceEnergy(igb2=(-52.293859))
refene3.solvent_energies(igb2=(-52.185005))
refene3.dielc2_energies()
refene3.dielc2.solvent_energies()
refene4 = _ReferenceEnergy(igb2=24.166408)
refene4.solvent_energies(igb2=24.224913)
...
TYX = TitratableResidue('TYX', ['N', 'H', 'CA', 'HA', 'CB', 'HB2', 'HB3', 'CG',
 ...
 'HE2', 'CD2', 'HD2', 'C', 'O'],
  typ='phredox')
TYX.add_state(protcnt=1, eleccnt=1, refene=refene1, pka_corr=9.6, eo_corr=1.4, charges=[
 -0.4157, 0.2719, -0.0014, 0.0876, -0.1163, 0.0548,
 0.0548, -0.0139, -0.1142, 0.1615, -0.341, 0.1911,
 0.4198, -0.5278, 0.3621, -0.341, 0.1911, -0.1142,
 0.1615, 0.5973, -0.5679])
TYX.add_state(protcnt=1, eleccnt=0, refene=refene2, pka_corr=(-2.0), eo_corr=0.0, charges=[
 -0.4157, 0.2719, -0.0014, 0.0876, -0.2911, 0.1123,
 0.1123, 0.4479, -0.1923, 0.2177, -0.1736, 0.2109,
 0.556, -0.4376, 0.4031, -0.1736, 0.2109, -0.1923,
 0.2177, 0.5973, -0.5679])
TYX.add_state(protcnt=0, eleccnt=1, refene=refene3, pka_corr=0.0, eo_corr=0.71, charges=[
 -0.4157, 0.2719, -0.0014, 0.0876, -0.05, 0.03,
 0.03, -0.1786, -0.1545, 0.1418, -0.4793, 0.1318,
 0.7197, -0.8026, 0.0, -0.4793, 0.1318, -0.1545,
 0.1418, 0.5974, -0.5678])
TYX.add_state(protcnt=0, eleccnt=0, refene=refene4, pka_corr=0.0, eo_corr=0.0, charges=[
 -0.4157, 0.2719, -0.0014, 0.0876, -0.1941, 0.0931,
 0.0931, 0.0538, -0.1216, 0.1629, -0.3224, 0.1643,
 0.6679, -0.4519, 0.0, -0.3224, 0.1643, -0.1216,
