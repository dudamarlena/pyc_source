...
# [GCC 8.4.0]
# Embedded file name: /home/swails/src/ParmEd/parmed/tinker/parameterfile.py
# Compiled at: 2019-03-25 16:13:20
# Size of source mod 2**32: 20658 bytes
"""
...
class BookmarkedFile(object):
    __doc__ = ' Allows setting a bookmark and rewinding to that bookmark '
...
class _ParamType(object):
    __doc__ = ' All parameter types. This caches the list of parameters for easy access '
    TypeList = dict()
    _param_type = ''
...
class _DihedralType(_ParamType):
    ...
    def __init__(self, idx1, idx2, idx3, idx4, *args):
        ...
        if idx2 < idx3 or idx2 == idx3 and (idx1 < idx4 or idx1 == idx4):
            ...
            if idx2 > idx3 or idx2 == idx3 and idx1 > idx4:
...
class _Atom(object):
    __doc__ = ' An atom in a parameter set '
    ...
    def _atomic_number(self):
        return self.type.atomic_number

    def _element(self):
        return self.type.atomic_number

    def _valence(self):
        return self.type.valence
    ...
    def _reduction(self):
        return self.type.reduction

    def _blocked(self):
        raise NotImplementedError('Cannot set this attribute')
...
class AmoebaParameterSet(object):
    ...
    atomre = re.compile('atom *(\\d+) *(\\d+) *([A-Za-z\\-\\+\\*0-9]+) *"(.+)" *(\\d+) *(\\d+\\.\\d+) *(\\d+)', re.I)
    anglere = re.compile('angle([ 345fF])')

    def __init__(self, fname=None):
        self.atoms = dict()
        self.atom_types = _AtomType.TypeList
        self.bonds = _BondType.TypeList
        self.angles = _AngleType.TypeList
    ...
    def load_parameter_file(self, fname):
        ...
        while line.lstrip()[:5].lower() == 'atom ':
            ...
            self.atoms[int(num)] = _Atom(typenum, name, descrip, anum, mass, val)
            line = f.readline().replace('\t', ' ')
        ...
        while line.lstrip()[:9] == 'polarize ' and line:
            ...
            index = int(words[1])
            try:
                self.atoms[index].set_polarizability(words[2], words[3], words[4:])
            except IndexError:
