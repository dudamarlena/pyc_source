...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyupnptools/upnp_soap.py
# Compiled at: 2018-09-02 07:20:38
from .upnp import *
...
class UPnPSoapRequest(OrderedDict):
    ...
    def __str__(self):
        ...
        ns_table = {'s': 'http://schemas.xmlsoap.org/soap/envelope/', 
           'u': self.service_type}
        ...
        action = ET.SubElement(body, ('{{{}}}{}').format(ns_table['u'], self.action_name))
...
class UPnPSoapResponse(OrderedDict):
    ...
    def __str__(self):
        ...
        envelope = ET.Element(('{{{}}}Envelope').format(ns_table['s']))
        body = ET.SubElement(envelope, ('{{{}}}Body').format(ns_table['s']))
        action = ET.SubElement(body, ('{{{}}}{}Response').format(ns_table['u'], self.action_name))
        for k, v in self.items():
            elem = ET.SubElement(action, k)
            elem.text = v
