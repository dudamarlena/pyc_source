...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyupnptools/upnp_control_point.py
# Compiled at: 2018-09-02 07:50:49
from .upnp import *
...
class UPnPControlPoint:
    ...
    def subscribe(self, device, service):
        ...
        headers = {'NT': 'upnp:event', 
           'CALLBACK': ('<http://{}:{}{}>').format(server_addr[0], server_addr[1], cb_path), 
           'TIMEOUT': 'Second-1800'}
    ...
    def renew_subscription(self, subscription):
        url = urljoin(subscription.device.base_url, subscription.service.eventSubURL())
    ...
    def _resolve_expired_subscriptions(self):
        sids = [ subscription.sid for subscription in self._subscriptions.values() if subscription.is_expired()
               ]
        for sid in sids:
