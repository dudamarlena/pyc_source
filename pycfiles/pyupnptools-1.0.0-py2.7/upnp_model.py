...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyupnptools/upnp_model.py
# Compiled at: 2018-09-02 19:35:50
import time
    ...
    def read_xml_node(device_node):
        ...
        for child in device_node:
            ...
            if tagname == 'deviceList':
                device.children = [ UPnPDevice.read_xml_node(device_node) for device_node in child if get_tagname(device_node) == 'device'
                                  ]
            elif tagname == 'serviceList':
                device.services = [ UPnPService.read_xml_node(service_node) for service_node in child if get_tagname(service_node) == 'service'
                                  ]
            elif not list(child):
                value = child.text
    ...
    def read_xml_node(node):
        ...
        for child in node:
            if get_tagname(child) == 'name':
                ...
                action.arguments = [ UPnPActionArgument.read_xml_node(argument_node) for argument_node in child if get_tagname(argument_node) == 'argument'
                                   ]
    ...
    def read(xml):
        ...
        for node in root:
            ...
            if tagname == 'specVersion':
                ...
                scpd.actions = [ UPnPAction.read_xml_node(action_node) for action_node in node if get_tagname(action_node) == 'action' ]
            elif tagname == 'serviceStateTable':
                scpd.state_variables = [ UPnPStateVariable.read_xml_node(state_variable_node) for state_variable_node in node if get_tagname(state_variable_node) == 'stateVariable'
                                       ]
