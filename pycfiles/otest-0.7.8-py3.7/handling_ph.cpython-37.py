...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/otest/aus/handling_ph.py
# Compiled at: 2018-06-21 10:44:22
# Size of source mod 2**32: 6915 bytes
import logging, os, cherrypy
...
class WebIh(InfoHandling):
    ...
    def __init__(self, conf=None, flow_state=None, profile_handler=None, cache=None, session=None, base_url='', pre_html=None, **kwargs):
        (InfoHandling.__init__)(self, flow_state=flow_state, profile_handler=profile_handler, 
         cache=cache, 
    ...
    def flow_list(self):
        ...
        _msg = self.pre_html['flow_list.html'].format(display_profile=(display_profile(self.session.profile)),
          op_choice=(op_choice(self.base_url, self.flow_state.display_info(_tests))),
          legends=(legends()),
          version=(self.session.tool_version))
        return _msg

    def profile_edit(self):
        _msg = self.pre_html['profile.html'].format(profile_form=(profile_form(self.session.profile)),
          version=(self.session.tool_version),
          base=(self.base_url))
        return _msg
    ...
    def test_info(self, testid):
        try:
            ...
            _sess = self.session
            _l = [
             'You have not run this test during this test session.<br>']
            _su = safe_url(_sess.iss)
            path0 = 'log/{}/{}/{}/{}'.format(_su, _sess.tag, _sess.profile, testid)
            if _su.endswith('2F'):
                path1 = 'log/{}/{}/{}/{}'.format(_su[:-2], _sess.tag, _sess.profile, testid)
            else:
                path1 = 'log/{}2F/{}/{}/{}'.format(_su, _sess.tag, _sess.profile, testid)
            ...
            return self.pre_html['sorry.html'].format(error=('\n'.join(_l)),
              link=('<a href="{}">link</a>'.format(self.base_url)))
        else:
            _events = info['events']
            _msg = self.pre_html['testinfo.html'].format(profile_output=profile_output((info['profile_info']),
              version=(self.session.tool_version)),
    ...
    def _display(self, root, issuer, profile):
        ...
        item.sort()
        _msg = self.pre_html['logs.html'].format(display_log=display_log(logs=item, base=self.base_url, **argv),
          version=(self.session.tool_version))
        return _msg
    ...
    def display_log(self, root, issuer='', profile='', testid=''):
        ...
        :return:
        """
        logger.info("display_log root: '{root}' issuer: '{iss}', profile: '{prof}' testid: '{tid}'".format(root=root,
          iss=issuer,
          prof=profile,
          tid=testid))
        if testid:
    ...
    def sorry_response(self, homepage, err):
        _msg = self.pre_html['sorry.html'].format(error=(str(err)),
