...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/otest/aus/request.py
# Compiled at: 2019-01-04 03:35:19
# Size of source mod 2**32: 13880 bytes
import copy, inspect, logging, sys
...
class Request(Operation):
    ...
    def expected_error_response(self, response):
        if isinstance(response, Response):
            ...
            self.conv.events.store(EV_PROTOCOL_RESPONSE, response, sender=(self.__class__.__name__))
            if response['error'] not in self.expect_error['error']:
                raise Break('Wrong error, got {} expected {}'.format(response['error'], self.expect_error['error']))
            try:
...
class SyncRequest(Request):
    ...
    def do_request(self, client, url, body, ht_args):
        response = (client.http_request)(url, method=self.method, data=body, **ht_args)
        self.conv.events.store(EV_HTTP_RESPONSE, response, sender=(self.__class__.__name__))
        return response
    ...
    def handle_response(self, r, csi):
        ...
        if 300 < r.status_code < 400:
            resp = self.conv.entity.parse_response((self.response),
              info=(r.headers['location']), sformat='urlencoded',
              state=state)
        else:
            if r.status_code == 200:
                if 'response_mode' in csi and csi['response_mode'] == 'form_post':
                    ...
                    if r.is_redirect or r.is_permanent_redirect:
                        ...
                        resp = self.conv.entity.parse_response((self.response),
                          info=(r.text), sformat='json',
                          state=state)
                _ent = self.conv.entity
                if isinstance(resp, AccessTokenResponse):
                    if 'id_token' in resp:
                        if isinstance(resp['id_token'], IdToken):
                            pass
                        else:
                            resp.verify(keyjar=(_ent.keyjar), client_id=(_ent.client_id), iss=(_ent.provider_info['issuer']))
                    else:
                        resp.verify(keyjar=(_ent.keyjar), client_id=(_ent.client_id), iss=(_ent.provider_info['issuer']))
                    ...
                    if r.headers['content-type'] == 'application/json':
                        ...
                        resp = ErrorResponse(error='service_error', error_description=(r.text))
                else:
                    resp = r
                ...
                try:
                    try:
                        ...
                        if _id_token.jwe_header:
                            self.conv.events.store('JWE header', _id_token.jwe_header)
                        if 'kid' not in _id_token.jws_header and not _id_token.jws_header['alg'] == 'HS256':
                            keys = self.conv.entity.keyjar.keys_by_alg_and_usage(issuer=(_id_token['iss']),
                            ...
                            try:
                                if self.req_args['nonce'] != _id_token['nonce']:
                                    raise ParameterError('invalid nonce! {} != {}'.format(self.req_args['nonce'], _id_token['nonce']))
                            except KeyError:
                                pass

                            if not same_issuer(self.conv.info['issuer'], _id_token['iss']):
                                raise IssuerMismatch(' {} != {}'.format(self.conv.info['issuer'], _id_token['iss']))
 ...
 self.request, method=self.method, request_args=self.req_args, target=_client.provider_info['issuer'], **self.op_args)
        try:
        ...
        self.catch_exception_and_error((self.handle_response), r=http_response, csi=csi)
...
class AsyncRequest(Request):
    ...
    def run(self):
        _client = self.conv.entity
        url, body, ht_args, csi = (_client.request_info)(
 self.request, method=self.method, request_args=self.req_args, lax=True, **self.op_args)
        self.csi = csi
        self.conv.events.store(EV_REDIRECT_URL, url, sender=(self.__class__.__name__))
    ...
    def parse_response(self, path, inut, message_factory, response=None):
        ...
        _conv = self.conv
        if self.csi is None:
            url, body, ht_args, csi = (_conv.entity.request_info)(self.request, method=self.method, request_args=self.req_args, **self.op_args)
            self.csi = csi
        else:
            try:
            ...
            if response_mode == 'form_post':
                ...
                if response_where in ('url', ''):
                    info = response
                    _ctype = 'dict'
                else:
                    if response_where == 'fragment':
                        try:
            ...
            ev_index = _conv.events.store(EV_RESPONSE, info, sender=(self.__class__.__name__))
            resp_cls = message_factory(self.response_cls)
            try:
                response = _conv.entity.parse_response(resp_cls,
                  info, _ctype, (self.csi['state']),
                  keyjar=(_conv.entity.keyjar))
            except ResponseError as err:
                try:
                    _conv.events.store(EV_FAULT, State((_conv.test_id), ERROR, message=err,
            ...
            _conv.events.store(EV_PROTOCOL_RESPONSE, response, ref=ev_index, sender=(self.__class__.__name__))
...
def factory(name):
    for fname, obj in inspect.getmembers(sys.modules[__name__]):
        if inspect.isclass(obj) and name == fname:
            return obj
