...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/otest/events.py
# Compiled at: 2019-03-14 08:04:16
...
class Base(object):
    ...
    def to_str(self):
        ...
        return json.dumps(_res, sort_keys=True, indent=4, separators=(',', ': '))
...
class Event(object):
    ...
    def __init__(self, timestamp=0, typ='', data=None, ref='', sub='', sender='', direction=0, **kwargs):
...
class Events(object):
    ...
    def store(self, typ, data, ref='', sub='', sender='', direction=0, **kwargs):
        index = time.time()
        typ = typ.lower()
        if typ == EV_HTTP_RESPONSE:
            data = HTTPResponse(data)
        self.events.append(Event((int(index)), typ, data, ref, sub, sender, direction, **kwargs))
        return index
    ...
    def __contains__(self, event):
        ...
        for ev in self.events:
            if event.timestamp == ev.timestamp and event == ev:
                return True
        ...
        if form == 'list':
            ...
            for ev in self.events:
                if isinstance(ev.data, Message):
                    _data = json.dumps((ev.data.to_dict()), sort_keys=True, indent=4,
                      separators=(',', ': '))
                else:
                    _data = ev.data
                text.append('<tr><td>{time}</td><td>{typ}</td><td>{data}}</td></tr>'.format(time=(ev.timestamp),
                  typ=(ev.typ),
                  data=_data))
...
def message_to_str(event):
    return [
     ...
     json.dumps((event.data.to_dict()), sort_keys=True, indent=4, separators=(',', ': '))]


TO_STR = {EV_FUNCTION: funtion_to_str, 
...
def layout(start, event):
    elem = [
     '{}'.format(round(event.timestamp - start, 3))]
    if event.direction:
...
def row(start, event):
    if event.typ == EV_OPERATION:
        _row = ['<tr class="info"><td>{}</td>'.format(round(event.timestamp - start, 3))]
    else:
        _row = [
         '<tr><td>{}</td>'.format(round(event.timestamp - start, 3))]
    try:
        p = TO_STR[event.typ](event)
        _row.append('<td>{}</td><td><pre><code>{}</pre></code></td>'.format(p[0], ' '.join(p[1:])))
    except KeyError:
