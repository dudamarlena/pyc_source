...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/otest/tool.py
# Compiled at: 2017-03-30 13:05:14
...
def get_redirect_uris(cinfo):
    ...
    if isinstance(res, (''.__class__, ''.__class__)):
        return [
         res]
    return res
...
class Tester(object):
    ...
    def __init__(self, inut, sh, profile='', flows=None, check_factory=None, msg_factory=None, cache=None, make_entity=None, map_prof=None, com_handler=None, response_cls=None, client_factory=None, **kwargs):
        self.inut = inut
        self.sh = sh
        self.conv = None
    ...
    def match_profile(self, test_id):
        ...
        return self.map_prof(self.profile.split('.'), _spec['profile'].split('.'))

    def setup(self, test_id, **kw_args):
        redirs = get_redirect_uris(kw_args['client_info'])
        _flow = self.flows[test_id]
        _cli, _cli_info = (self.client_factory.make_client)(**kw_args['client_info'])
        self.conv = Conversation(_flow, _cli, (kw_args['msg_factory']), callback_uris=redirs)
    ...
    def run_flow(self, test_id, index=0, profiles=None, **kwargs):
        ...
        for item in self.conv.sequence[index:]:
            ...
            try:
                _oper = cls(conv=(self.conv), inut=(self.inut), sh=(self.sh), profile=(self.profile),
                  test_id=test_id,
                  funcs=funcs,
        ...
        if isinstance(_oper, Done):
            self.conv.events.store(EV_CONDITION, (State('Done', OK)), sender='run_flow')
        tinfo = self.store_result(res)
        return tinfo['state']