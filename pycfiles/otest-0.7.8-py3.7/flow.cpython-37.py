...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/otest/flow.py
# Compiled at: 2018-06-18 11:05:01
# Size of source mod 2**32: 10760 bytes
...
class RPFlow(Flow):
    ...
    def __init__(self, fdir, profile_handler, cls_factories, func_factory, use=''):
...
class FlowState(RPFlow):
    ...
    def __init__(self, fdir, profile_handler, cls_factories, func_factory, display_order, use=''):
        RPFlow.__init__(self, fdir, profile_handler, cls_factories, func_factory,
          use=use)
        self.test_info = {}
    ...
    def store_test_info(self, tester):
        _conv = tester.conv
        tinfo = self._test_info(_conv.test_id, _conv.events, _conv.index, tester.sh)
        self.test_info[_conv.test_id] = tinfo
        return tinfo

    def _test_info(self, test_id, events, index, session):
        _info = {'test_id':test_id, 
         'events':events, 
         'index':index, 
