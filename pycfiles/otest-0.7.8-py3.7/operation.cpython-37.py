...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/otest/operation.py
# Compiled at: 2019-04-10 02:44:57
# Size of source mod 2**32: 11070 bytes
import copy, inspect, json, logging, time, sys, cherrypy
...
def print_result(resp):
    try:
        ...
        txt = json.dumps((resp.to_dict()), sort_keys=True, indent=2, separators=(',',
                                                                                 ': '))
...
class Operation(object):
    ...
    def __init__(self, conv, inut, sh, test_id='', conf=None, funcs=None, check_factory=None, cache=None, profile='', tool_conf=None, **kwargs):
        self.conv = conv
        self.inut = inut
        self.sh = sh
        self.funcs = funcs or {}
    ...
    def catch_exception_and_error(self, func, **kwargs):
        ...
        res = None
        try:
            self.conv.events.store(EV_FUNCTION, {'name':func.__name__, 
             'kwargs':kwargs})
            res = func(**kwargs)
        except Exception as err:
            try:
                self.conv.events.store(EV_EXCEPTION, '{}:{}'.format(err.__class__.__name__, err))
                if not self.expect_exception:
                    raise
                else:
                    if not err.__class__.__name__ == self.expect_exception:
                        raise
                    else:
                        self.conv.events.store(EV_EVENT, 'got expected exception {}'.format(self.expect_exception))
            finally:
                err = None
                del err
            ...
            if self.expect_exception:
                self.conv.events.store(EV_EVENT, 'Expected exception did not occur')
                raise Exception("Expected exception '{}'.".format(self.expect_exception))
        if self.expect_error:
            try:
                assert isinstance(res, ErrorResponse)
            except AssertionError:
                logger.info('Expected error not received: {}'.format(res.to_dict()))
                self.conv.events.store(EV_EVENT, 'Expected error not received')
            else:
                if res['error'] not in self.expect_error['error']:
                    self.conv.events.store(EV_EVENT, 'Expected error not received: got {}'.format(res['error']))
                    logger.info('Expected error not received: {}'.format(res.to_dict()))
                else:
                    self.conv.events.store(EV_EVENT, 'Got expected error')
                    logger.info('Got expected error: {}'.format(res.to_dict()))
                try:
            ...
            if isinstance(res, ErrorResponse):
                logger.info('Unexpected error response: {}'.format(res.to_dict()))
...
class Note(Notice):
    ...
    def args(self):
        return {'next':link('{}continue?path={}&index={}'.format(self.conv.entity.base_url, self.test_id, self.sh['index'])), 
