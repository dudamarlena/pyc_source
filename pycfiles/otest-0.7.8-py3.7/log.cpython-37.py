...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/otest/log.py
...
class WebLog(Log):
    ...
    def store_test_info(self, session, profile_info=None):
        _info = {'trace':session['conv'].trace, 
        ...
        if profile_info:
            ...
            try:
                _info['profile_info'] = self.profile_info(session, session['testid'])
            except KeyError:
                pass
    ...
    def _display(self, root, issuer, profile):
        ...
        item.sort()
        argv['logs'] = item
        return Response(mako_template='logs.mako', template_lookup=(self.lookup),
          headers=[])
    ...
    def display_log(self, root, issuer='', profile='', test_id=''):
        ...
        if test_id:
            path = os.path.join(root, issuer, profile, test_id).replace(':', '%3A')
        ...
        if err_type == 0:
            ...
            if 'conv' in session:
                if err:
                    ...
                    session['conv'].test_output.append({'id':'-', 
                     'status':err_type,  'message':'%s' % err})
                else:
                    session['conv'].events.store(EV_CONDITION,
                      State(err_type, status=ERROR, message=('Error in %s' % where)),
                      sender='log_fault')
    ...
    def sorry_response(self, homepage, err):
        resp = Response(mako_template='sorry.mako', template_lookup=(self.lookup),
          headers=[])
