...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/otest/rp/endpoints.py
...
def authorization(environ, events):
    _op = environ['oic.op']
    return wsgi_wrapper(environ, _op.authorization_endpoint, events)


def userinfo(environ, events):
    _op = environ['oic.op']
    return wsgi_wrapper(environ, _op.userinfo_endpoint, events)


def clientinfo(environ, events):
    _op = environ['oic.op']
    return wsgi_wrapper(environ, _op.client_info_endpoint, events)


def revocation(environ, events):
    _op = environ['oic.op']
    return wsgi_wrapper(environ, _op.revocation_endpoint, events)
...
def op_info(environ, events):
    _op = environ['oic.op']
    logger.info('op_info')
    return wsgi_wrapper(environ, _op.providerinfo_endpoint, events)
...
def webfinger(environ, events):
    ...
    try:
        if query['rel'] != [OIC_ISSUER]:
            events.store(EV_CONDITION, State('webfinger_parameters', ERROR, message=('parameter rel wrong value: {}'.format(query['rel']))))
            return BadRequest('Parameter value error')
        resource = query['resource'][0]
    except KeyError as err:
        try:
            events.store(EV_CONDITION, State('webfinger_parameters', ERROR, message=('parameter {} missing'.format(err))))
