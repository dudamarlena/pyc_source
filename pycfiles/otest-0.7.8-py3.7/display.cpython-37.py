...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/otest/rp/display.py
...
def print_message(message_as_dict):
    return adjust_lines(json.dumps(message_as_dict, sort_keys=True, indent=2, separators=(',',
                                                                                          ': ')))


BG_COLOR = {OK: 'background-color:lightgreen;', 
...
def do_condition(event):
    return [
     '<li style="{}">{}</li>'.format(BG_COLOR[event.data.status], event.data.test_id)]
...
FUNC_MAP = {EV_CONDITION: do_condition, 
 EV_FAULT: do_fault, 
