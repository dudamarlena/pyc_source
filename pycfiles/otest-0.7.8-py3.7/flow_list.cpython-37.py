...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/otest/aus/preproc/flow_list.py
# Compiled at: 2018-02-04 14:56:51
...
def op_choice(base, flows):
    ...
    """
    _grp = '_'
    color = ['<button type="button" class="btn btn-default" title="Black"><span class="glyphicon glyphicon-play"></span></button>',
     ...
     '<button type="button" class="btn btn-default" title="QuestionMark"><span class="glyphicon glyphicon-question-sign"></span></button>',
     '<button type="button" class="btn btn-default" title="Grey"><span class="glyphicon glyphicon-record"></span></button>']
    line = [
    ...
    for grp, state, desc, tid in flows:
        ...
        if not grp == _grp:
            ...
            line.append('<tr id="{}">{}</tr>'.format(b, _item))
        elif state:
            _info = '<a href=\'{}test_info/{}\' class="btn btn-info" role="button"><span class="glyphicon glyphicon-info-sign"></span></a>'.format(base, tid)
        else:
            _info = ''
        _stat = "<a href='{}{}'>{}</a>".format(base, tid, color[state])
        line.append('<tr><td>{}</td><td style="vertical-align: middle;">({})</td><td style="vertical-align: middle;">{}</td><td>{}</td></tr>'.format(_stat, tid, desc, _info))
...
def legends():
    element = [
     '<table class="table table-condensed" id="legends">']
