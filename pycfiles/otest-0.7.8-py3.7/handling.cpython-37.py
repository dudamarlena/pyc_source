...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/otest/handling.py
# Compiled at: 2018-06-21 09:04:22
...
class InfoHandling(object):
    ...
    def __init__(self, flow_state, desc=None, profile_handler=None, cache=None, session=None, **kwargs):
    ...
    def get_err_type(self, test_id):
        errt = WARNING
        try:
            if self.session.profile.split('.')[0] in self.flow_state[test_id]['MTI']:
                errt = ERROR
        except KeyError:
            pass
    ...
    def log_fault(self, session, err, where, err_type=0):
        ...
        elif 'conv' in session:
            if err:
                session['conv'].events.store(EV_CONDITION, State('Fault', status=ERROR, name=err_type,
                  message=('{}'.format(err))))
            else:
                session['conv'].events.store(EV_CONDITION, State('Fault',
                  status=ERROR, name=err_type,
                  message=('Error in %s' % where)))


class ClIh(InfoHandling):

    def __init__(self, flows=None, profile='', desc='', profile_handler=None, cache=None, session=None, **kwargs):
        (InfoHandling.__init__)(self, flows, profile=profile, desc=desc, profile_handler=profile_handler, 
         cache=cache, 
