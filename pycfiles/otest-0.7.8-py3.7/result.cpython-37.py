...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/otest/result.py
...
class Result(object):
    ...
    def op_based(self, test_id, tag=''):
        ...
        if not tag:
            tag = _sess.tag
        return safe_path(_iss, self.logfile_extension, tag, _sess.profile, test_id)

    def rp_based(self, test_id, tag=''):
        _sess = self.session
        return safe_path(_sess['test_conf']['start_page'], self.logfile_extension, _sess.profile, test_id)
    ...
    def write_info(self, tinfo, test_id='', file_name=None, tag=''):
        if not test_id:
            ...
            if 'conv' not in self.session:
                ...
                output = [
                 'Test tool version: {}'.format(self.session.tool_version)]
                if _pi:
                    _keys = list(_pi.keys())
                    _keys.sort()
    ...
    def _open_file(self, file_name, mode='w'):
        try:
            ...
            try:
                ...
                try:
                    logger.error("Couldn't dump to log file {} reason: {}").format(file_name, err)
                    raise
                finally:
                    err = None
                    del err
