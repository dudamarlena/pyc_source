...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/otest/func.py
...
def check_endpoint(oper, args):
    try:
        ...
        oper.conv.events.store(EV_CONDITION, State('check_endpoint', status=ERROR, message=('{} not in provider configuration'.format(args))))
        oper.skip = True
...
def factory(name):
    for fname, obj in inspect.getmembers(sys.modules[__name__]):
        if inspect.isfunction(obj) and fname == name:
            return obj