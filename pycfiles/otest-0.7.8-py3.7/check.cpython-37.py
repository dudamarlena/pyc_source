...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/otest/check.py
# Compiled at: 2017-09-11 07:21:16
# Size of source mod 2**32: 7523 bytes
"""
...
import json, inspect, traceback, sys
...
NOT_APPLICABLE = 7
STATUSCODE = [
 'INFORMATION', 'OK', 'WARNING', 'ERROR', 'CRITICAL',
...
class TestResult(object):
    ...
    def __init__(self, test_id, status=OK, name='', mti=False, message='', **kwargs):
    ...
    def __str__(self):
        if self.status:
            return '{}: status={}, message={}'.format(self.test_id, STATUSCODE[self.status], self.message)
...
class State(object):
    ...
    def __init__(self, test_id, status, name='', mti=False, message='', context='', **kwargs):
        self.test_id = test_id
        self.status = status
        self.name = name
        self.mti = mti
    ...
    def __str__(self):
        _info = {'ctx':self.context, 
         'id':self.test_id,  'stat':STATUSCODE[self.status], 
         'msg':self.message}
        if self.status != OK:
            if self.context:
                txt = ('{ctx}:{id}: status={stat}, message={msg}'.format)(**_info)
            else:
...
class Check(object):
    ...
    def response(self, **kwargs):
        try:
            name = ' '.join([str(s).strip() for s in self.__doc__.strip().split('\n')])
