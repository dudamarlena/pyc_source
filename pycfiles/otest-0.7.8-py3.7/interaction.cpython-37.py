...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/otest/interaction.py
# Compiled at: 2016-12-07 12:35:36
...
class Interaction(object):
    ...
    def pick_interaction(self, response, base):
        ...
        for interaction in self.interactions:
            ...
            for attr, val in list(interaction['matches'].items()):
                ...
                elif attr == 'content' and unic and val in unic:
                    _match += 1
    ...
    def redirect(self, orig_response, url_regex, **kwargs):
        ...
        if not no_of_matches == 1:
            raise InteractionNeeded('Expected single match but found %d', no_of_matches)
        url = matches[0]
        return self.httpc.send(url, 'GET')
...
class Action(object):
    ...
    def __call__(self, tester, location, response, features, **kwargs):
        ...
        if _conv.events:
            _conv.events.store(EV_FUNCTION, {'name':function.__name__, 
             'kwargs':_args})
