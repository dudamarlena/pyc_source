...
# [GCC 8.4.0]
# Embedded file name: C:/Users/Snochacz/Documents/GitHub/mapel\mapel\voting\objects.py
# Compiled at: 2020-04-21 06:38:10
# Size of source mod 2**32: 7364 bytes
""" This module contains all the objects """
import os, math

class Model:
    __doc__ = ' Abstract model of elections '

    def __init__(self, exp_name):
        self.exp_name = exp_name
        self.num_voters, self.num_candidates, self.num_families, self.families = self.import_controllers(exp_name)
    ...
    def import_controllers(exp_name):
        ...
        for i in range(num_families):
            ...
            if line[0][0] == '#':
                ...
                show = False
            families.append(Family(name=(str(line[1])), special=(float(line[2])), size=(int(line[0])),
              label=(str(line[5])),
              color=(str(line[3].replace(' ', ''))),
              alpha=(float(line[4])),
              show=show))
...
class Model_xd(Model):
    __doc__ = ' Multi-dimensional model of elections '
    ...
    def import_distances(exp_name, metric):
        ...
        return (
         num_points, num_distances, hist_data)


class Model_2d(Model):
    __doc__ = ' Two-dimensional model of elections '
...
class Family:
    __doc__ = ' Family of elections '

    def __init__(self, name='none', special=0.0, size=0, label='none', color='black', alpha=1.0, show=True):
        self.name = name
