...
# Compiled at: 2020-04-21 09:26:45
...
def print_2d(exp_name, num_winners=0, mask=False, angle=0, reverse=False, update=False, values='default', coloring='purple', num_elections=800, winners_order='positionwise_approx_cc', main_order='', metric='positionwise', saveas='map_2d'):
    """ Print the two-dimensional embedding of multi-dimensional map of the elections """
    model = obj.Model_2d(exp_name, num_winners=num_winners, num_elections=num_elections, winners_order=winners_order,
            ...
            if values != 'default':
                ...
                for k in range(model.num_families):
                    for _ in range(model.families[k].size):
                        ...
                        if coloring == 'intervals':
                            ...
                            shade = 1
                        ax.scatter((model.points[model.main_order[ctr]][0]), (model.points[model.main_order[ctr]][1]), label=values,
                          color=color,
                          alpha=shade,
                          s=9)
                        ctr += 1
                ...
                for k in range(model.num_families):
                    if model.families[k].show:
                        ax.scatter((model.points_by_families[k][0]), (model.points_by_families[k][1]), color=(model.families[k].color),
                          label=(model.families[k].label),
                          alpha=(model.families[k].alpha),
                          s=9)
...
def print_matrix(exp_name, scale=1.0, metric='positionwise', saveas='matrix'):
...
def prepare_approx_cc_order(exp_name, metric='positionwise'):
    ...
    for i in range(num_elections):
        ...
        src = os.path.join(src, 'experiments', str(exp_name), 'elections', 'soc_original', 'tmp_' + str(target) + '.soc')
        dst = os.path.join(os.path.abspath(os.path.dirname(__file__)), '..')
        dst = os.path.join(dst, 'experiments', str(exp_name), 'elections', 'soc_' + str(metric) + '_approx_cc', 'tmp_' + str(i) + '.soc')
        copyfile(src, dst)
...
def print_param_vs_distance(exp_name, values='', scale='none', metric='positionwise', saveas='correlation'):
    ...
    for k in range(model.num_families):
        ...
        ax.scatter((distances[left:right]), (times[left:right]), color=(model.families[k].color),
          label=(model.families[k].label),
          alpha=(model.families[k].alpha),
          s=9)
        left = right
