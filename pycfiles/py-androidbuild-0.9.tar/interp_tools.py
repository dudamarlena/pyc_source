...
# Embedded file name: /home/rdussurget/.virtualenvs/compile.octant.UBU64/lib/python2.7/site-packages/altimetry/tools/interp_tools.py
# Compiled at: 2016-03-23 12:35:00
import numpy as np, threading, Queue, bisect, operator, datetime
try:
...
def interp1d(x, Z, xout, spline=False, kind='linear', fill_value=np.NaN, **kwargs):
    ...
    if linear:
        try:
            ...
            Zout = np.repeat(np.NaN, nx)

    else:
        tck = scipy.interpolate.splrep(x, Z, s=0)
        try:
            Zout = scipy.interpolate.splev(xout, tck, der=0, **kwargs)
        except RuntimeError:
            Zout = np.repeat(np.NaN, nx)
...
def interp2d2d(x, y, Z, xout, yout, split_factor=1, **kwargs):
    ...
    for i in tsort:
        if i == tsort[0]:
            ...
            outmat = np.ma.concatenate((outmat, dum[i]), 0) if isinstance(outmat, np.ma.masked_array) else np.concatenate((outmat, dum[i]), 0)
...
def _interp2d2d(x, y, Z, xout, yout, **kwargs):
    ...
    try:
        ...
        Zout = np.ones((xout.size, yout.size)) * np.NaN

    Zout = Zout.reshape((xout.size, yout.size))
    if isinstance(Z, np.ma.masked_array):
        try:
            maskout = scipy.interpolate.griddata(points, gmask, xi, **kwargs).astype(bool)
        except RuntimeError:
            Zout = np.ones((xout.size, yout.size)) * np.NaN
...
def interp3d(x, y, t, Z, xout, yout, tout, **kwargs):
    ...
    nt = t.size
    order = (
     Z.shape.index(nx), Z.shape.index(ny), Z.shape.index(nt))
    Z = Z.transpose(order)
    N = xout.size
    for enum in enumerate(zip(*(xout, yout, tout))):
        ...
        if ntid == 0:
            ...
            res = (b - a) / (t[idt[1]] - t[idt[0]]) * (time - t[idt[0]]) + a
        else:
            res = np.NaN
        if enum[0] == 0:
            Zout = res
        else:
            Zout = np.append(Zout, res)
...
def interp3d_core(x, y, t, Z, xout, yout, tout, **kwargs):
    ...
    gy = np.reshape(np.repeat(y, x.size * t.size), (y.size, x.size, t.size)).transpose((1,
                                                                                        0,
                                                                                        2))
    gt = np.reshape(np.repeat(t, x.size * y.size), (t.size, x.size, y.size)).transpose((1,
                                                                                        2,
                                                                                        0))
    gz = Z.flatten()
