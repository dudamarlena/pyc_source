...
# Embedded file name: /home/rdussurget/.virtualenvs/compile.octant.UBU64/lib/python2.7/site-packages/altimetry/data/hydro.py
# Compiled at: 2016-03-23 12:35:00
import os, glob, inspect, fnmatch, datetime
from warnings import warn
import numpy as np, matplotlib.pyplot as plt
from netCDF4 import Dataset as ncfile
import copy, tempfile
from altimetry.tools.nctools import load_ncVar, load_ncVar_v2, nc as ncobj, dimStr, attrStr, ncStr, varStr
try:
...
class hydro_data(object):
    ...
    def __init__(self, file_pattern, limit=None, verbose=1, round=True, zero_2pi=True, output_is_dict=True, flatten=False, **kwargs):
        ...
        if isinstance(file_pattern, str):
            ...
            ls = (isinstance(file_pattern, list) or file_pattern.tolist)() if 1 else file_pattern
        ...
        if isinstance(file_pattern, str):
            ...
            if len(file_pattern) > 0:
                self.dirname = os.path.dirname(os.path.abspath(file_pattern[0]))
            ...
            for i in np.arange(len(self.fid_list)):
                ...
                if filename != enum[1][i]:
                    self.filelist[i] = os.path.basename(filename)
                res = self.read(filename, output_is_dict=output_is_dict, **kwargs)
                self.update_dataset(res, flatten=flatten)
                self.check_variables()

        if not self.limit_set:
            self.limit = self.extension(round=round)
        if self.count == 0:
            self.warning(1, 'Empty object!')
        return
    ...
    def check_variables(self):
        ...
        for enum in enumerate(infos):
            ...
            varShape = np.shape(self.__dict__.get(enum[1][0]))[::-1]
            dimSize = None
            if hasattr(self.__dict__.get(enum[1][0]), '_dimensions'):
                ...
                if isinstance(self.__dict__.get(enum[1][0]), dict):
                ...
                if masked:
                    if self.__dict__.get(enum[1][0]).mask.size != self.__dict__.get(enum[1][0]).data.size:
                        raise np.ma.core.MaskError('Mask length is not consistent with data')
                ...
                for n, sh in enumerate(varShape):
                    if sh > dimSize[n]:
                        ...
                        self.__dict__[enum[1][0]] = np.ma.masked_array(np.append(self.__dict__[enum[1][0]].data, np.repeat(self.__dict__[enum[1][0]].fill_value if hasattr(self.__dict__[enum[1][0]], 'fill_value') else np.NaN, dimSize[n] - varSize)), mask=np.append(self.__dict__[enum[1][0]].mask, np.ones(dimSize[n] - varSize, dtype='bool')))
    ...
    def create_Variable(self, name, value, dimensions, toCreate=None, createDim=None, extend=True):
        ...
        if toCreate is None:
            toCreate = np.sum(self.par_list == name) == 0
        ...
        if createDim.any() & toCreate:
            ...
            if updateDim:
                for k in dimensions.keys():
                    dimensions.update({k: self._dimensions[k]})
    ...
    def updated_copy(self, flag, deep=True):
        ...
        for p in set(self.__dict__.keys()).difference(emptyObj.__dict__.keys()):
            ...
            try:
                ...
                for a in attr.keys():
                    setattr(emptyObj.__dict__[p], a, attr[a])
    ...
    def slice(self, param, range, surf=False):
        ...
        if np.size(range) == 2:
            if not surf:
                fg = (self.__dict__[param] >= np.min(range)) & (self.__dict__[param] < np.max(range))
            else:
                fg = (self.__dict__[(param + '_surf')] >= np.min(range)) & (self.__dict__[(param + '_surf')] < np.max(range))
        elif np.size(range) == 1:
            if not surf:
                fg = self.__dict__[param] == range
            else:
                fg = self.__dict__[(param + '_surf')] == range
        else:
            self.Error('Range array must have 1 or 2 elements max')
        return fg
    ...
    def time_slice(self, timerange, surf=False):
        ...
        if isinstance(timerange[0], str):
            trange = cnes_convert(timerange)[0]
        else:
            trange = timerange
    ...
    def update_with_slice(self, flag):
        ...
        for par in par_list:
            ...
            if hasattr(self.__dict__[par], '_dimensions'):
                self.__dict__[par]._dimensions['time'] = self._dimensions['time']
    ...
    def extension(self, flag=None, round=True):
        ...
        """
        if flag is None:
            limit = [self.lat.min(), self.lon.min(), self.lat.max(), self.lon.max()] if self.__dict__.has_key('lat') and self.__dict__.has_key('lon') else [
             -90, 0, 90, 360]
        else:
            limit = [self.lat.compress(flag).min(), self.lon.compress(flag).min(), self.lat.compress(flag).max(), self.lon.compress(flag).max()] if self.__dict__.has_key('lat') and self.__dict__.has_key('lon') else [
             -90, 0, 90, 360]
        if round:
    ...
    def get_id_list(self, flag=None):
        if flag is None:
            return np.unique(self.id)
        else:
            return np.unique(self.id.compress(flag))
            return
    ...
    def get_timestats(self, flag, par_list=None, full=True, bins=None):
        ...
        if par_list is None:
            ...
            if not isinstance(par_list, np.ma.masked_array):
                par_list = np.ma.array(par_list, mask=np.zeros(len(par_list), dtype=bool))
            ...
            if bins is None and trange is not None:
                bins = (max(trange[1]) - min(trange[1])).days + 1
            else:
                bins = ()
            ...
            for p in avail_par:
                ...
                t_edges = t_edges[:-1] + (t_edges[1] - t_edges[0]) / 2.0
                par_stats.update({p: {'nb': N, 'mean': H / N, 
                       't_axis': t_edges}})

        if full:
            out = (par_list, valid, per_valid, fname, trange, extent, N, avail_par, avail_par_per, par_stats)
        else:
            out = par_stats
        return out
    ...
    def get_stats(self, flag, par_list=None, full=True):
        ...
        if par_list is None:
            ...
            for p in avail_par:
                var = self.__dict__[p].compress(flag).compressed()
                par_stats.update({p: {'nb': N, 'mean': np.mean(var), 
    ...
    def get_platform_stats(self, id, functype='overall'):
        ...
        elif functype == 'time' or functype == 'temporall':
            get_stats = self.get_timestats
        else:
            get_stats = self.get_stats
        par_list, valid, per_valid, fname, trange, extent, N, avail_par, avail_par_per, par_stats = get_stats(self.id == id)
        return (
         fname, trange, extent, N, avail_par, avail_par_per, par_stats)
    ...
    def map(self, flag=None, fname=None, zoom=False, pmap=None, show=True, bathy=False, **kwargs):
        ...
        """
        if zoom:
            limit = self.extension(flag)
        else:
            limit = self.limit
        if pmap is None:
            pmap = plot_map(0, 0, 0, limit=limit, bathy=bathy)
    ...
    def summary(self, all=False, fig=None, col='.k', legend=None, bathy=False, functype='overall', **kwargs):
        ...
        self.message(0, '\tLoaded from : ' + self.dirname)
    ...
    def plot_track(self, pmap, flag=None, col='.k', endpoint='*r', endpoint_size=None, title=None, fontsize=8, textcolor='b', ms=5, linewidth=1, **kwargs):
        ...
        if self.__dict__.has_key('lon_surf'):
            if flag is None:
                flag = np.ones(self.lon_surf.size, dtype='bool')
            ...
            if flag is None:
                flag = np.ones(self.lon.size, dtype='bool')
        ...
        for j in id_list:
            ...
            if dumvalid > 0:
                if isinstance(col, str):
                    p = pmap.plot(dumlon, dumlat, col, ms=ms, linewidth=linewidth, **kwargs)
                else:
                    p = pmap.scatter(dumlon, dumlat, col.compress(dumflag)[sort], s=ms, linewidth=linewidth, **kwargs)
    ...
    def plot_track_old(self, *args, **kwargs):
        ...
        if len(args) == 3:
            ...
            scatter = False
        if scatter:
            obj_plot.scatter(lon, lat, var, **kwargs)
        else:
            obj_plot.plot(lon, lat, **kwargs)

    def plot_transect(self, x, z, var, xrange=None, zrange=None, vmin=None, vmax=None, xstep=1, zstep=10, s=10, edgecolor='none', **kwargs):
        """
    ...
    def read_ArgoNC(self, filename, params=None, force=False, dephrange=None, timerange=None, **kwargs):
        ...
        for param in par_list:
            dumVar = self.load_ncVar(param, N_PROF=ind, **kwargs)
    ...
    def ncstruct(self, **kwargs):
        ...
        if (np.array(par_list) == 'sat').sum():
            par_list.pop(par_list.index('sat'))
        ...
        for d in varlist:
            ...
            attr = attrStr(keys=attributes, values=[ self.__getattribute__(d).__getattribute__(a) for a in attributes ])
            data = self.__getattribute__(d)
            outStr[d] = varStr(dimensions=curDim if curDim is not None else self._dimensions, attributes=attr, data=data)
    ...
    def write_nc(self, filename, clobber=False, **kwargs):
        ...
        ncalti = self.ncstruct()
        return obj.write(ncalti, filename, clobber=clobber, **kwargs)
...
class buoy_data(hydro_data):
    ...
    def read(self, filename, **kwargs):
        ...
        if extension == '.nc':
            ...
            if extension == '.dat':
                outStr = self.read_txt(filename)
            else:
                if extension == '.asc':
                    kwread = kwargs.get('lon_name', {})
                    kwread = kwargs.get('lat_name', {})
                    outStr = self.read_asc(filename, **kwread)
                else:
                    self.Error('Unknown formatting')
                ...
                datalen = [ np.size(outStr[key]) for key in outStr.keys() ]
                ...
                twoD_flag = []
                for key in outStr.keys():
                    twoD_flag.append(len(np.shape(outStr[key]['data'])) == 2) if isinstance(outStr[key], dict) else twoD_flag.append(len(np.shape(outStr[key])) == 2)
                ...
                for key in twoD_vars:
                    if isinstance(outStr[key], dict):
                        outStr[key]['data'] = outStr[key]['data'][:, outStr[key]['data'].mask.argmin(1)].diagonal()
                    else:
                        outStr[key] = outStr[key][:, outStr[key].mask.argmin(1)].diagonal()

            for key in outStr.keys():
                if isinstance(outStr[key], dict):
                    outStr[key]['data'] = outStr[key]['data'].flatten()
                else:
                    outStr[key] = outStr[key].flatten()
    ...
    def read_txt(self, filename):
        ...
        temp = np.ma.masked_equal(temp, 9999.0)
        u = np.ma.masked_array(data[:, 1])
        u = np.ma.masked_equal(u, 9999.0)
        v = np.ma.masked_array(data[:, 2])
        v = np.ma.masked_equal(v, 9999.0)
    ...
    def read_asc(self, filename, lon_name='LON_FILTRE', lat_name='LAT_FILTRE'):
        ...
        outStr.update({'N_LEVELS': {'_dimensions': {'_ndims': 1, 'N_PROF': sz}, 'data': np.ma.array(np.repeat(25.0, sz), mask=mask.copy())}})
        outStr.update({'id': {'_dimensions': {'_ndims': 1, 'N_PROF': sz}, 'data': np.ma.masked_array(data[:, 0], mask=mask.copy())}})
        outStr.update({'date': {'_dimensions': {'_ndims': 1, 'N_PROF': sz}, 'data': np.ma.masked_array(data[:, 1] / 86400.0, mask=mask.copy())}})
        ...
        mask = outStr['lon']['data'].mask.copy() | outStr['lat']['data'].mask.copy() | outStr['date']['data'].mask.copy()
        outStr['date']['data'].mask = mask.copy()
        for k in par_list:
            if outStr[k].has_key('data'):
                outStr[k]['data'].mask = mask.copy()
...
class argo_trajectory(hydro_data):
    ...
    def read(self, filename, **kwargs):
        ...
        if extension == '.nc':
            ...
            twoD_vars = np.array(outStr.keys()).compress(twoD_flag)
            for key in twoD_vars:
                outStr.update({key: outStr[key][:, outStr[key].mask.argmin(1)].diagonal()})

            for key in outStr.keys():
                outStr.update({key: outStr[key].flatten()})
...
class glider_data(hydro_data):
    ...
    def read(self, filename, **kwargs):
        ...
        dimStr = outStr.pop('_dimensions')
        if dimStr['_ndims'] != 2:
            self.Error(('Variables with {0} dimensions can not be used as glider profiles (2 dimensions)').format(dimStr['_ndims']))
        if not dimStr.has_key('N_LEVELS') or not dimStr.has_key('N_PROF'):
            self.Error(('Data structure must contain dimensions N_LEVELS and N_PROF (only contains {0})').format(dimStr.keys()[1:]))
        nblevels = dimStr['N_LEVELS']
        nbprofiles = dimStr['N_PROF']
        ...
        for key in oneD_vars:
            if isinstance(outStr[key], dict):
                outStr[key]['data'] = np.reshape(np.repeat(outStr[key]['data'], nblevels), [nbprofiles, nblevels])
            else:
                outStr[key] = np.reshape(np.repeat(outStr[key], nblevels), [nbprofiles, nblevels])

        surfStr = self.read_ArgoNC(filename, N_LEVELS=0)
        ...
        for key in surfStr.keys():
            if surfStr[key].has_key('_dimensions'):
                ...
                if surfStr[key]['_dimensions'].has_key('N_PROF'):
                    surfStr[key]['_dimensions'].update({'nbprofiles': surfStr[key]['_dimensions'].pop('N_PROF')})

        for key in surfStr.keys():
            if isinstance(surfStr[key], dict):
                surfStr[key]['data'] = surfStr[key]['data'].flatten()
            else:
                surfStr[key] = surfStr[key].flatten()
...
class TSG_data(hydro_data):
    ...
    def read(self, filename):
        ...
        if extension == '.nc':
            ...
            outStr.update({'temp': outStr.pop('tem2')})
            return outStr
        if extension == '.dat':
    ...
    def read_txt(self, filename):
        ...
        temp = np.ma.masked_greater(temp, 99.0)
        psal = np.ma.masked_array(data[:, 4])
        psal = np.ma.masked_less(psal, 35.0)
        fluo = np.ma.masked_array(data[:, 5])
        fluo = np.ma.masked_where((fluo == 0.0) | (fluo >= 99.0), fluo)
...
class CTD_data(hydro_data):
    ...
    def read_asc(self, filename, **kwargs):
        ...
        return {'_dimensions': {'_ndims': ndims, 'nbpoints': sz[0]}, 'lon': lon, 'lat': lat, 'date': date, 'id': id, 'depth': depth, 'pres': pres, 'temp': temp, 'psal': psal, 
           'fluoro': fluoro, 'dens': dens, 'potemp': potemp, 'cond': cond, 'obs1': obs1, 'obs2': obs2, 'svCM': svCM, 'descend_rate': descend_rate, 'flag': flag}
...
def read_cnv(self, filename, **kwargs):
    ...
    pres = np.ma.masked_greater(pres, 99999.0)
    temp = np.ma.masked_greater(temp, 99.0)
    cond = np.ma.masked_greater(cond, 99.0)
    reclen = len(pres)
    id = np.repeat(('{0}').format(kwargs['stationid']), reclen)
