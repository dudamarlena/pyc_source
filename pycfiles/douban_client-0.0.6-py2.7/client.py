...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-intel/egg/douban_client/client.py
...
class DoubanClient(DoubanAPI):
    ...
    def __init__(self, key, secret, redirect='', scope=''):
        ...
        self.scope = scope
        self.client = Client(key, secret, site=self.API_HOST, authorize_url=self.AUTHORIZE_URL, token_url=self.TOKEN_URL)
    ...
    def auth_with_password(self, username, password, **opt):
        self.access_token = self.client.password.get_token(username=username, password=password, redirect_uri=self.redirect_uri, **opt)
