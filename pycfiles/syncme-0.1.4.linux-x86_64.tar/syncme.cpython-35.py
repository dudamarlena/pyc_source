...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/syncme.py
# Compiled at: 2018-06-07 12:12:49
# Size of source mod 2**32: 19947 bytes
import logging, os, subprocess as sp, getpass, argparse
...
def push(**kwargs):
    ...
    """
    return_code = rsync(source_path=kwargs.get('local_path'), dest_path=kwargs.get('remote_path'), dest_host=kwargs.get('host'), dest_user=kwargs.get('user'), tags=kwargs.get('tags', []), recursive=kwargs.get('recursive', False))
    return return_code
...
def pull(**kwargs):
    ...
    args:
        ...
        tags: list of str tags(options) added to rsync command
    """
    return_code = rsync(dest_path=kwargs.get('local_path'), source_path=kwargs.get('remote_path'), source_host=kwargs.get('host'), source_user=kwargs.get('user'), tags=kwargs.get('tags', []), recursive=kwargs.get('recursive', False))
    return return_code
...
def rsync(**kwargs):
    ...
    if kwargs.get('source_host', None) is None:
        cmd = [
         RSYNC, '{0}'.format(kwargs['source_path']),
         '{0}@{1}:{2}'.format(kwargs['dest_user'], kwargs['dest_host'], kwargs['dest_path'])]
    else:
        if kwargs.get('dest_host', None) is None:
            cmd = [
             RSYNC,
             '{0}@{1}:{2}'.format(kwargs['source_user'], kwargs['source_host'], kwargs['source_path']),
...
def push_sync(config, sync_name='all', host_name='all'):
    ...
    for sync in syncs:
        if host_name == 'all':
            ...
            remote_hosts = [x for x in sync['hosts'] if x['name'] == host_name]
        for host in remote_hosts:
            logger.info('Push %s to %s:', sync['name'], host['name'])
            for local_path, remote_path in zip(sync['paths'], host['paths']):
                if local_path is not None:
                    return_code = push(local_path=local_path, remote_path=remote_path, host=host['address'], user=host['user'], tags=sync['tags'], recursive=sync['recursive'])
                    if return_code != 0:
        ...
        for host in remote_hosts:
            ...
            for local_path, remote_path in zip(sync['paths'], host['paths']):
                if local_path is not None:
                    return_code = push(local_path=local_path, remote_path=remote_path, host=host['address'], user=host['user'], tags=sync['tags'], recursive=sync['recursive'])
                    if return_code != 0:
                        logger.debug('Failed to transfer path %s to local system', remote_path)
                        failed_hosts.append((host['name'], local_path))
