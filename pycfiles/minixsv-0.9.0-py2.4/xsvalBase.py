...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\minixsv\xsvalBase.py
# Compiled at: 2008-08-08 10:45:26
import string, copy
from minixsv import *
...
class XsValBase:
    ...
    def validate(self, inputTree, xsdTreeList):
        ...
        if wxsdLookupDict == {}:
            wxsdLookupDict = {'ElementDict': {}, 'TypeDict': {}, 'GroupDict': {}, 'AttrGroupDict': {}, 'AttributeDict': {}, 'IdentityConstrDict': {}}
            self._importWellknownSchemas(wxsdLookupDict)
        self.xsdLookupDict = {'ElementDict': wxsdLookupDict['ElementDict'].copy(), 'TypeDict': wxsdLookupDict['TypeDict'].copy(), 'GroupDict': wxsdLookupDict['GroupDict'].copy(), 'AttrGroupDict': wxsdLookupDict['AttrGroupDict'].copy(), 'AttributeDict': wxsdLookupDict['AttributeDict'].copy(), 'IdentityConstrDict': wxsdLookupDict['IdentityConstrDict'].copy()}
        ...
        if self.xsdElementDict.has_key(inputRootNsName):
            ...
            self._raiseError('Used root tag %s not found in schema file(s)!' % repr(inputRootNsName), self.inputRoot)
        return
    ...
    def _expandIncludes(self, baseTree, tree, includeDict, lookupDict):
        ...
        for includeNode in rootNode.getChildrenNS(namespaceURI, 'include'):
            ...
            self._includeSchemaFile(baseTree, tree, includeNode, expNamespace, includeUrl, includeNode.getBaseUrl(), includeDict, lookupDict, adaptTargetNamespace=1)
    ...
    def _expandRedefines(self, baseTree, tree, includeDict, lookupDict):
        ...
        for redefineNode in rootNode.getChildrenNS(namespaceURI, 'redefine'):
            ...
            expNamespace = rootNode.getAttributeOrDefault('targetNamespace', None)
            self._includeSchemaFile(baseTree, tree, redefineNode, expNamespace, redefineUrl, redefineNode.getBaseUrl(), includeDict, lookupDict, adaptTargetNamespace=1)
    ...
    def _includeSchemaFile(self, baseTree, tree, nextSibling, expNamespace, includeUrl, baseUrl, includeDict, lookupDict, adaptTargetNamespace=0):
        if includeUrl == None:
        ...
        if includeUrl not in ('C:\\Program Files\\Python24\\Lib\\site-packages\\minixsv\\xml.xsd',
                              'C:\\Program Files\\Python24\\Lib\\site-packages\\minixsv\\XMLSchema.xsd',
                              'C:\\Program Files\\Python24\\Lib\\site-packages\\minixsv\\XMLSchema-instance.xsd'):
    ...
    def _parseIncludeSchemaFile(self, baseTree, tree, nextSibling, includeUrl, baseUrl):
        try:
            ...
            try:
                ...
                if e.errno in (2, 'socket error', 'url error'):
    ...
    def _updateLookupTables(self, rootNode, lookupDict):
        schemaTagDict = {'element': 'ElementDict', 'complexType': 'TypeDict', 'simpleType': 'TypeDict', 'group': 'GroupDict', 'attributeGroup': 'AttrGroupDict', 'attribute': 'AttributeDict'}
    ...
    def _checkComplexTypeTag(self, xsdParentNode, xsdNode, inputNode, inputChildIndex, usedAsBaseType=None):
        ...
        return (
         inputChildIndex, baseTypeAttributes)
        ...
        if inputNode.getAttribute((XSI_NAMESPACE, 'nil')) == 'true':
            ...
            if usedAsBaseType in (None, 'extension'):
                ...
                for validChildTag in validChildTags:
                    if validChildTag.getLocalName() not in ('attribute', 'attributeGroup',
                                                            'anyAttribute'):
                ...
                if usedAsBaseType == None and inputChildIndex < len(childTags):
                    ...
                    self._addError('Unexpected or invalid child tag %s found in tag %s!' % (repr(childNsName), repr(inputNsName)), childTags[inputChildIndex])
    ...
    def _checkList(self, elementMethod, xsdNode, inputParentNode, inputNodeList, currIndex):
        ...
        if occurs == 0 and minOccurs > 0:
            ...
            raise TagException('minOccurs (%d) of child tag %s in tag %s not available (only %d)!' % (minOccurs, repr(expInputTagName), repr(inputParentNode.getTagName()), occurs), inputParentNode, 1)
        return currIndex
    ...
    def _checkElementTag(self, xsdNode, inputParentNode, inputNodeList, currIndex):
        if xsdNode.hasAttribute('ref'):
            ...
            if xsdNode.getFirstChild() == None:
                if not xsdNode.hasAttribute('type') and not inputNode.hasAttribute((XSI_NAMESPACE, 'type')):
                ...
                typeNsName = inputNode.hasAttribute((XSI_NAMESPACE, 'type')) or xsdNode.getQNameAttribute('type')
            else:
    ...
    def _checkSimpleType(self, xsdNode, xsdTypeAttr, inputNode, attrName, attrValue, fixedValue, checkAttribute=0, checkId=1):
        ...
        if checkAttribute == 0 and inputNode.hasAttribute((XSI_NAMESPACE, 'nil')):
            if inputNode[(XSI_NAMESPACE, 'nil')] == 'true' and collapseString(inputNode.getElementValue()) != '':
        ...
        try:
            ...
            elif xsdNode.getFirstChildNS(self.xsdNsURI, 'complexType') != None and xsdNode.getFirstChildNS(self.xsdNsURI, 'complexType').getAttribute('mixed') == 'false':
    ...
    def _checkAttributeTag(self, qAttrName, xsdAttrNode, xsdAttrRefNode, inputNode, inputAttrDict):
        ...
        if use == None:
            use = xsdAttrRefNode.getAttributeOrDefault('use', 'optional')
        ...
        if inputAttrDict.has_key(qAttrName):
            ...
            if xsdAttrRefNode.hasAttribute('default'):
                if not (inputNode.getNsName() == (XSD_NAMESPACE, 'element') and inputNode.hasAttribute('ref') and xsdAttrRefNode.getAttribute('name') == 'nillable'):
    ...
    def _checkWildcardElement(self, xsdNode, inputNode, inputNamespace):
        ...
        if processContents == 'skip':
            pass
        elif inputNode.hasAttribute((XSI_NAMESPACE, 'type')):
            typeNsName = inputNode.getQNameAttribute((XSI_NAMESPACE, 'type'))
            if not self.xsdTypeDict.has_key(typeNsName):
                self._addError('Unknown overloaded type %s!' % repr(typeNsName), inputNode, 0)
            else:
                    ...
                    if baseTypes != None:
                        if baseTypes[0] in ((XSD_NAMESPACE, 'anyType'), (XSD_NAMESPACE, 'anySimpleType')):
                            ...
                            if overloadedType != (None, None):
                                baseTypes = [
                                 inputChild.getQNameAttribute((XSI_NAMESPACE, 'type'))]
                    else:
    ...
    def _getXPath(self, node, xPathNode, identityConstraint=None):
        ...
        return (
         childList, attrNodeList, attrName)
