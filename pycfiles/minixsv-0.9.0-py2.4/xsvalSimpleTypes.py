...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\minixsv\xsvalSimpleTypes.py
# Compiled at: 2008-08-08 10:45:18
...
class XsSimpleTypeVal:
    ...
    def checkSimpleType(self, inputNode, attrName, typeName, attributeValue, returnDict, idCheck):
        ...
        if _suppBaseTypeDict.has_key(typeName):
            ...
            elif typedefNode.getNsName() == (XSD_NAMESPACE, 'complexType') and typedefNode.getFirstChild().getNsName() == (XSD_NAMESPACE, 'simpleContent'):
    ...
    def _checkRestrictionTag(self, inputNode, xsdElement, attrName, attributeValue, returnDict, idCheck):
        ...
        if totalDigitsNode != None:
            ...
            elif len(digits) > string.atoi(totalDigitsValue):
                raise SimpleTypeError('Total number of digits > %s for %s (%s)' % (repr(totalDigitsValue), repr(attrName), repr(attributeValue)))
...
def _checkDateTimeType(inputNode, simpleType, attributeValue, returnDict):
    ...
    try:
        if offset != None:
            ...
            tz = None
        dtObj = datetime.datetime(int(date[0:4]), int(date[5:7]), int(date[8:10]), int(time[0:2]), int(time[3:5]), int(time[6:8]), 0, tz)
    except ValueError, errstr:
        raise BaseTypeError('is invalid (%s)!' % errstr)
...
class TimezoneFixedOffset(datetime.tzinfo):
    ...
    def __init__(self, offset):
        if offset == 'Z':
            ...
            self.__offset = datetime.timedelta(hours=int(offset[0:3]), minutes=int(offset[0] + offset[4:5]))
...
_suppBaseTypeDict = {(XSD_NAMESPACE, 'anySimpleType'): _checkAnySimpleType, (XSD_NAMESPACE, 'string'): _checkStringType, (XSD_NAMESPACE, 'anyURI'): _checkAnyUriType, (XSD_NAMESPACE, 'decimal'): _checkDecimalType, (XSD_NAMESPACE, 'integer'): _checkIntegerType, (XSD_NAMESPACE, 'float'): _checkFloatType, (XSD_NAMESPACE, 'double'): _checkDoubleType, (XSD_NAMESPACE, 'hexBinary'): _checkHexBinaryType, (XSD_NAMESPACE, 'base64Binary'): _checkBase64BinaryType, (XSD_NAMESPACE, 'boolean'): _checkBooleanType, (XSD_NAMESPACE, 'QName'): _checkQNameType, (XSD_NAMESPACE, 'NOTATION'): _checkQNameType, (XSD_NAMESPACE, 'duration'): _checkDurationType, (XSD_NAMESPACE, 'dateTime'): _checkDateTimeType, (XSD_NAMESPACE, 'date'): _checkDateType, (XSD_NAMESPACE, 'time'): _checkTimeType, (XSD_NAMESPACE, 'gYearMonth'): _checkYearMonth, (XSD_NAMESPACE, 'gMonthDay'): _checkMonthDay, (XSD_NAMESPACE, 'gYear'): _checkYear, (XSD_NAMESPACE, 'gMonth'): _checkMonth, (XSD_NAMESPACE, 'gDay'): _checkDay}