...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\genxmlif\xmlifApi.py
# Compiled at: 2008-08-08 10:48:48
...
class XmlTreeWrapper:
    ...
    def cloneTree(self):
        ...
        treeWrapperCopy = self.__class__(self.xmlIf, self.__tree.xmlIfExtCloneTree(rootElementWrapperCopy.element), self.__useCaching)
...
class XmlElementWrapper:
    ...
    def cloneNode(self, deep, cloneCallback=None):
        ...
        if cloneCallback:
            cloneCallback(elementWrapperCopy)
        if deep:
    ...
    def printNode(self, indent='', deep=0, prettyPrint=0, attrMaxLengthDict={}, printElementValue=1, encoding=None):
        ...
        """
        patternXmlTagShort = '%(indent)s<%(qName)s%(attributeString)s/>%(tailText)s%(lf)s'
        patternXmlTagLong = '%(indent)s<%(qName)s%(attributeString)s>%(elementValueString)s%(lf)s%(subTreeString)s%(indent)s</%(qName)s>%(tailText)s%(lf)s'
        subTreeStringList = []
        tailText = ''
        addIndent = ''
        lf = ''
    ...
    def getChildren(self, tagFilter=None):
        ...
        if tagFilter in (None, '*', (None, '*')):
            ...
            children = filter(lambda child: child.xmlIfExtElementWrapper.getNamespaceURI() == tagFilter[0], self.element.xmlIfExtGetChildren())
        else:
    ...
    def getElementsByTagName(self, tagFilter=None):
        ...
        if tagFilter in (None, '*', (None, '*'), (None, None)):
            ...
            descendants = filter(lambda desc: desc.xmlIfExtElementWrapper.getNamespaceURI() == tagFilter[0], self.element.xmlIfExtGetElementsByTagName())
        else:
            nsNameFilter = NsNameTupleFactory(tagFilter)
    ...
    def getIterator(self, tagFilter=None):
        ...
        if tagFilter in (None, '*', (None, '*'), (None, None)):
            ...
            matchingElements = filter(lambda desc: desc.xmlIfExtElementWrapper.getNamespaceURI() == tagFilter[0], self.element.xmlIfExtGetIterator())
        else:
            nsNameFilter = NsNameTupleFactory(tagFilter)
    ...
    def getLocation(self, end=0, fullpath=0):
        ...
        """
        lineMethod = (
         self.getStartLineNumber, self.getEndLineNumber)
            ...
            for (prefix, namespaceURI) in self.getCurrentNamespaces():
                if qNamePrefix == prefix:
                    if prefix != EMPTY_PREFIX or useDefaultNs:
                        nsName = (
                         namespaceURI, qNameLocalName)
                        break
            else:
                if qNamePrefix == None:
                    nsName = (
                     EMPTY_NAMESPACE, qNameLocalName)
                else:
    ...
    def getXPathList(self, xPath, namespaceRef=None, useDefaultNs=1, attrIgnoreList=[]):
        ...
        return (
         completeChildList, attrNodeList, attrNsNameFirst)
    ...
    def __createElement(self, tupleOrLocalName, attributeDict):
        ...
        """
        childElementWrapper = self.treeWrapper.createElement(tupleOrLocalName, attributeDict, self.curNs[:])
