...
# [GCC 8.4.0]
# Embedded file name: C:\Users\shu\PycharmProjects\py34\cazipcode-project\cazipcode\pkg\superjson\_superjson.py
# Compiled at: 2017-07-13 17:14:25
# Size of source mod 2**32: 20687 bytes
import io, os, json, time, zlib, base64, shutil, inspect
    ...
    def __new__(cls, name, bases, attrs):
        ...
        for base in inspect.getmro(klass):
            for attr, value in base.__dict__.items():
                dumper_warning_message = WARN_MSG.format(attr=attr, method_type='dumper', obj_or_dct='obj', dump_or_load='dump')
                loader_warning_message = WARN_MSG.format(attr=attr, method_type='loader', obj_or_dct='dct', dump_or_load='load')
                if attr.startswith('dump_'):
                    try:
                        if is_dumper_method(value):
                            class_name = get_class_name_from_dumper_loader_method(value)
...
def is_compressed_json_file(abspath):
    ...
    if ext in ('.json', '.js'):
        ...
        if ext == '.gz':
            ...
            raise ValueError("'%s' is not a valid json file. extension has to be '.json' or '.js' for uncompressed, '.gz' for compressed." % abspath)
    return is_compressed


@add_metaclass(Meta)
class SuperJson(object):
    __doc__ = 'A extensable json encoder/decoder. You can easily custom converter for \n    any types.\n    '
    _dumpers = dict()
    _loaders = dict()
    ...
    def dumps(self, obj, indent=None, sort_keys=None, pretty=False, float_precision=None, ensure_ascii=True, compress=False, **kwargs):
        ...
        if float_precision is None:
            ...
            json.encoder.FLOAT_REPR = lambda x: format(x, '.%sf' % float_precision)
        s = json.dumps(self._json_convert(obj), indent=indent, sort_keys=sort_keys, ensure_ascii=ensure_ascii, **kwargs)
        if compress:
            s = compresslib.compress(s, return_type='str')
        return s

    def loads(self, s, object_hook=None, decompress=False, ignore_comments=False, **kwargs):
        ...
        obj = json.loads(s, object_hook=object_hook, object_pairs_hook=None, **kwargs)
        return obj

    def dump(self, obj, abspath, indent=None, sort_keys=None, pretty=False, float_precision=None, ensure_ascii=True, overwrite=False, verbose=True, **kwargs):
        ...
        if os.path.exists(abspath):
            if not overwrite:
                prt_console('    Stop! File exists and overwrite is not allowed', verbose)
                return
        st = time.clock()
        s = self.dumps(obj, indent=indent, sort_keys=sort_keys, pretty=pretty, float_precision=float_precision, ensure_ascii=ensure_ascii, compress=False, **kwargs)
        with open(abspath, 'wb') as (f):
            if is_compressed:
                f.write(compresslib.compress(s, return_type='bytes'))
            else:
                f.write(s.encode('utf-8'))
        prt_console('    Complete! Elapse %.6f sec.' % (time.clock() - st), verbose)
        return s

    def safe_dump(self, obj, abspath, indent=None, sort_keys=None, pretty=False, float_precision=None, ensure_ascii=True, verbose=True, **kwargs):
        ...
        **中文文档**
        ...
        s = self.dump(obj, abspath_temp, indent=indent, sort_keys=sort_keys, pretty=pretty, float_precision=float_precision, ensure_ascii=ensure_ascii, overwrite=True, verbose=verbose, **kwargs)
        shutil.move(abspath_temp, abspath)
        return s

    def load(self, abspath, object_hook=None, ignore_comments=False, verbose=True, **kwargs):
        ...
        obj = self.loads(s, object_hook=object_hook, decompress=False, ignore_comments=ignore_comments)
        prt_console('    Complete! Elapse %.6f sec.' % (time.clock() - st), verbose)
        return obj
    ...
    def dump_OrderedDict(self, obj, class_name='collections.OrderedDict'):
        return {'$' + class_name: [(key, self._json_convert(value)) for key, value in iteritems(obj)]}
...
if __name__ == '__main__':
    ...
    def test_numpy():
        data = {'ndarray_int': np.array([[1, 2], [3, 4]]), 
        ...
        data1 = superjson.loads(s)
        for key in data:
            if not np.array_equal(data[key], data1[key]):
    ...
    def test_pandas():
        ...
        """
        data = {'series': pd.Series([('a', datetime(2000, 1, 1)),
                    (
                     'b', datetime(2010, 1, 1))])}
    ...
    def test_extend():
        ...
        class MySuperJson(SuperJson):
            ...
            def dump_Address(self, obj, class_name='sfm.nameddict.Base'):
                key = '$' + class_name
                return {key: {'street': obj.street,  'city': obj.city, 
                       'state': obj.state, 
                       'zipcode': obj.zipcode}}
