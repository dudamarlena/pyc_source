...
# [GCC 8.4.0]
# Embedded file name: /media/sf_www/mezzanine/personal/heroku/o/personal/portfolio/views.py
...
def portfolio_post_list(request, tag=None, year=None, month=None, username=None, category=None, template='portfolio/portfolio_post_list.html', extra_context=None):
    """
    ...
    if category is not None:
        ...
        templates.append('portfolio/portfolio_post_list_%s.html' % str(category.slug))
    author = None
    if username is not None:
        author = get_object_or_404(User, username=username)
        portfolio_posts = portfolio_posts.filter(user=author)
        templates.append('portfolio/portfolio_post_list_%s.html' % username)
    prefetch = ('categories', 'keywords__keyword')
    portfolio_posts = portfolio_posts.select_related('user').prefetch_related(*prefetch)
    portfolio_posts = paginate(portfolio_posts, request.GET.get('page', 1), settings.BLOG_POST_PER_PAGE, settings.MAX_PAGING_LINKS)
    context = {'portfolio_posts': portfolio_posts,  'year': year,  'month': month,  'tag': tag, 
...
def portfolio_post_detail(request, slug, year=None, month=None, day=None, template='portfolio/portfolio_post_detail.html', extra_context=None):
    """. Custom templates are checked for using the name
    ``portfolio/portfolio_post_detail_XXX.html`` where ``XXX`` is the portfolio
    posts's slug.
    """
    portfolio_posts = PortfolioPost.objects.published(for_user=request.user).select_related()
    ...
    context.update(extra_context or {})
    templates = ['portfolio/portfolio_post_detail_%s.html' % str(slug), template]
    return TemplateResponse(request, templates, context)