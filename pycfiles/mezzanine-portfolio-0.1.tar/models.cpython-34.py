...
# [GCC 8.4.0]
# Embedded file name: /media/sf_www/mezzanine/personal/heroku/personal/portfolio/models.py
...
class PortfolioPost(Displayable, Ownable, RichText, AdminThumbMixin):
    __doc__ = '\n    A Portfolio post.\n    '
    categories = models.ManyToManyField('PortfolioCategory', verbose_name=_('Categories'), blank=True, related_name='portfolioposts')
    rating = RatingField(verbose_name=_('Rating'))
    featured_image = FileField(verbose_name=_('Featured Image'), upload_to=upload_to('portfolio.PortfolioPost.featured_image', 'portfolio'), format='Image', max_length=255, null=True, blank=True)
...
class PortfolioImage(Orderable):
    ...
    file = FileField(_('File'), max_length=200, format='Image', upload_to=upload_to('portfolio.PortfolioImage.file', 'portfolio'))
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.id:
            if not self.description:
                ...
                name = ''.join([s.upper() if i == 0 or name[(i - 1)] == ' ' else s for i, s in enumerate(name)])
