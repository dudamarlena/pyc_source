...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/BioClients/chembl/FetchByID.py
# Compiled at: 2020-04-06 17:08:49
...
"""
import sys, os, argparse, logging, csv
import chembl_webresource_client.new_client as new_client
NCHUNK = 50
act_tags_selected = [
 'activity_id', 'assay_chembl_id', 'assay_type', 'src_id',
...
def TID2Targetcomponents(tids, args, fout):
    t_tags = [
     'target_chembl_id', 'target_type', 'organism', 'species_group_flag', 'tax_id']
...
def DID2Documents(dids, args, fout):
    d_tags = [
     'document_chembl_id', 'doc_type', 'src_id', 'pubmed_id',
    ...
    if args.ifile:
        ...
        if args.ids:
            ...
            if args.op == 'cid2Activity':
                ...
                if args.op == 'inchikey2Mol':
                    InchiKey2Molecule(ids, args, fout)
                else:
                    if args.op == 'tid2Targetcomponents':
                        TID2Targetcomponents(ids, args, fout)
                    else:
                        if args.op == 'did2Documents':
