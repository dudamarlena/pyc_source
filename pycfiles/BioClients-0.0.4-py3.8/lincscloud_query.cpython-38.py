...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/BioClients/lincs/lincscloud_query.py
...
if __name__ == '__main__':
    ...
    if args.ifile:
        fin = open(args.ifile)
        if not fin:
            parser.error('failed to open input file: %s' % args.ifile)
        ids = []
        while True:
            line = fin.readline()
            if not line:
        ...
        if args.ids:
            ...
            if not fout:
                parser.error('cannot open outfile: %s' % args.ofile)
            else:
                fout = sys.stdout
            t0 = time.time()
            if args.op == 'geneinfo':
...
# not in loop:
#	break
#      L.  77   156_158  BREAK_LOOP          410  'to 410'