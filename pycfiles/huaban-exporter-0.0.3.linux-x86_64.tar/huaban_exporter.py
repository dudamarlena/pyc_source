...
# [GCC 8.4.0]
# Embedded file name: /home/winkidney/.virtualenvs/side-project/lib/python2.7/site-packages/huaban_exporter.py
# Compiled at: 2017-02-27 05:46:55
import json, logging, os, Queue, random, string
...
def do_request(method, *args, **kwargs):
    ...
    response = getattr(requests, method)(timeout=(2, 10), *args, **kwargs)
    if is_json:
        response.json()
        if _DEBUG:
...
class User(object):
    ...
    def __init__(self, user_url):
        ...
        self.further_url_tpl = urljoin(self.base_url, '?{random_str}&max={board_id}&limit=10&wfl=1')
        self.username = None
    ...
    def _fetch_further(self, prev_boards):
        ...
        further_url = self.further_url_tpl.format(random_str=_random_string(8), board_id=max_id)
        resp = do_request('get', further_url, headers=XHR_HEADERS)
        content = resp.json()
...
class Board(object):
    ...
    def __init__(self, board_url_or_id):
        ...
        if 'http' in board_url_or_id:
            ...
            self.base_url = ('http://huaban.com/boards/{board_id}/').format(board_id=board_url_or_id)
        self.further_pin_url_tpl = urljoin(self.base_url, '?{random_string}&max={pin_id}&limit=20&wfl=1')
        self.pin_count = None
        self.title = None
    ...
    def _fetch_further(self, prev_pins):
        ...
        further_url = self.further_pin_url_tpl.format(pin_id=max_id, random_string=_random_string(8))
        resp = do_request('get', further_url, headers=XHR_HEADERS)
        content = resp.json()
    ...
    def as_dict(self):
        return {'pins': self._pins, 'title': self.title, 
...
class Pin(object):
    ...
    def __init__(self, pin_meta, dir_to_save):
        ...
        filename = ('{title}.{ext}').format(title=pin_meta['pin_id'], ext=pin_meta['ext'])
        self.file_to_save = os.path.join(dir_to_save, filename)
...
class HuaBan(object):
    ...
    def as_dict(self):
        ...
        meta['boards'] = [ board.as_dict() for board in self._boards ]
        return meta
...
class Downloader(object):
    ...
    def __init__(self, user_url, workers=5):
        ...
        self.workers = tuple(Worker(self.queue, self.download_one) for x in xrange(workers))
        return
