...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/ipecac/tools/scan.py
...
def detect_comments(root=''):
    ...
    """
    files = []
    for file in iglob(f"{root}**/*.py", recursive=True):
        if os.path.isfile(file) and not [i for i in IGNORED_DIRS if i in file] and '@api' in open(file).read():
            files.append(file)
