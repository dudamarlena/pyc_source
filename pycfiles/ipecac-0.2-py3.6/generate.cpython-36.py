...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/ipecac/tools/generate.py
...
def read_definition_base(title=None, server=None, root='', get_input=True):
    ...
    if os.path.exists(json_base):
        if os.path.exists(yaml_base):
            error_msg = 'You have two base files. One is JSON and one is YAML. Please remove one in order for ipecac to work correctly.'
