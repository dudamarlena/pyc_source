...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/test/test_generate.py
...
def test_merge_endpoints():
    ...
    expected = {'/alerts':{'get': {'description': 'hmm'}}, 
     '/incidents/':{'delete':{'description': 'cool'}, 
...
def test_create_definition_base_from_scratch():
    ...
    actual = generate.create_definition_base(title='Pavement Hotdogs', server='http://example.com')
    assert expected == actual
...
def test_read_definition_base_scratch():
    expected = {'openapi':'3.0.0', 
     ...
     'servers':[
      {'url':'http://example.com', 
       'description':'The production server for Pavement Hotdogs'}]}
    actual = generate.read_definition_base(title='Pavement Hotdogs', server='http://example.com',
      get_input=False)
    assert expected == actual
...
def test_read_definition_from_json():
    ...
    shutil.copyfile('fixtures/base.json', f"{base}/base.json")
...
def test_read_definition_from_yml():
    ...
    base = utils.generate_sandbox()
    shutil.copyfile('fixtures/base.yml', f"{base}/base.yml")
    actual = generate.read_definition_base(root=f"{base}/")
    assert actual == expected
    shutil.rmtree(base)
...
def test_create_definition():
    ...
    base = utils.generate_sandbox()
    shutil.copyfile('fixtures/base.yml', f"{base}/base.yml")
    shutil.copyfile('fixtures/full_comment.py', f"{base}/full_comment.py")
