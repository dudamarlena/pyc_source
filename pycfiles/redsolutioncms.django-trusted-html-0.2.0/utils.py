...
# [GCC 8.4.0]
# Embedded file name: d:\Dropbox\vk\django-trusted-html\trustedhtml\utils.py
# Compiled at: 2014-04-29 06:05:21
"""Utils used by classes"""
import re
SGML_CHARACTER_ENTITIES = {'nbsp': 160, 
   'iexcl': 161, 
   'cent': 162, 
   'pound': 163, 
   'curren': 164, 
   ...
   'hearts': 9829, 
   'diams': 9830}
SGML_CHARACTER_RE = re.compile('(?P<string>&(#(?P<dec>[0-9]+)|#x(?P<hex>[0-9A-Fa-f]+)|(?P<name>[a-zA-Z][a-zA-Z0-9]*));?)')
...
STYLE_COMMENT_RE = re.compile('\\/\\*[^*]*\\*+([^/][^*]*\\*+)*\\/')
STYLE_COMMENT_REPL = ''
STYLE_CHARACTER_RE = re.compile('(?P<string>\\\\(?P<hex>[0-9a-fA-F]s{1,6})[ \\t\\r\\n\\f]?)')
