...
# [GCC 8.4.0]
# Embedded file name: d:\Dropbox\vk\django-trusted-html\trustedhtml\models.py
...
def log(sender, rule, value, source, **kwargs):
    if source != value:
        Log.objects.create(valid='exception' not in kwargs, source=source, result=value, sender=unicode(sender), rule=unicode(rule.__dict__))
    return value
