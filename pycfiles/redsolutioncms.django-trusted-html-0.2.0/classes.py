...
# [GCC 8.4.0]
# Embedded file name: D:\Dropbox\vk\django-trusted-html\trustedhtml\classes.py
# Compiled at: 2014-04-30 03:56:36
import re
...
class Rule(object):
    ...
    def __init__(self, allow_empty=True, default=None, invalid=False, element_exception=False, data=None):
        """
    ...
    def validate(self, value, path=None):
        ...
        try:
            ...
            except ElementException as exception:
                raise exception
            except TrustedException as exception:
            ...
            results = rule_done.send(sender=self.__class__, rule=self, path=path, value=value, source=source)
            for receiver, response in results:
                value = response

        except TrustedException as exception:
            rule_exception.send(sender=self.__class__, rule=self, path=path, value=value, source=source, exception=exception)
            raise exception
...
class String(Rule):
    ...
    def lower_list(self, values):
        ...
        if values is None or values is False or values is True:
            return values
        return [ self.lower_string(value) for value in values
               ]
...
class RegExp(String):
    ...
    def __init__(self, regexp, flags=0, expand='\\1', **kwargs):
        """
...
class Uri(String):
    ...
    def __init__(self, type=LINK, allow_sites=None, allow_schemes=None, cut_sites=None, cut_schemes=None, verify_sites=None, verify_schemes=None, verify_local=None, local_sites=None, local_schemes=None, **kwargs):
        """
    ...
    def core(self, value, path):
        ...
        if self.inlist(scheme, self.cut_schemes) and self.inlist(authority, self.cut_sites) and (scheme is not None or authority is not None):
            scheme = None
            authority = None
...
class Sequence(String):
    ...
    def __init__(self, rule, regexp='\\s+', flags=0, min_split=0, max_split=0, join_string=' ', prepend_string='', append_string='', **kwargs):
        """
...
class Style(Sequence, Validator):
    ...
    def __init__(self, rules, **kwargs):
        ...
        (or tag attribute) and value is corresponding rule.
        """
        Sequence.__init__(self, rule=None, regexp='\\s*;\\s*', join_string='; ', append_string=';', **kwargs)
    ...
    def sequence(self, values, path):
        ...
        return [ '%s: %s' % (property_name, property_value) for property_name, property_value in properties
               ]
...
class Element(Rule, Validator):
    ...
    def __init__(self, rules=None, contents=None, empty_element=False, remove_element=False, optional_start=False, optional_end=False, save_content=True, **kwargs):
        """
        ``rules`` is dictionary in witch key is name of property
        (or tag attribute) and value is corresponding rule.
    ...
    def preprocess(self, value, path):
        """Do it."""
        value = [ (attribute_name, get_cdata(attribute_value)) for attribute_name, attribute_value in value
                ]
        return value
...
class Html(String):
    ...
    def __init__(self, rules, fix_number=2, prepare_number=2, root_tags=None, allow_empty=True, **kwargs):
        """
        ``rules`` is dictionary in witch key is name of property
        (or tag attribute) and value is corresponding rule.
    ...
    def join(self, soup):
        ...
        while index < len(soup.contents) - 1:
            if not isinstance(soup.contents[(index + 1)], Tag) and not isinstance(soup.contents[index], Tag):
    ...
    def need_wrap(self, content, for_next):
        if isinstance(content, Tag):
            if content.name in self.root_tags:
                return False
        elif not for_next and (content.string == '' or content.string == ' ' or content.string == self.NBSP_CHAR):
            return False
        return True
    ...
    def fix(self, value, path):
        soup = BeautifulSoup(value, markupMassage=self.MARKUP_MASSAGE, convertEntities=BeautifulSoup.ALL_ENTITIES)
