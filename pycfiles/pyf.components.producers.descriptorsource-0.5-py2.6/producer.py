...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyf/components/producers/descriptorsource/producer.py
# Compiled at: 2010-07-19 08:14:25
"""
...
class DescriptorSource(Producer):
    ...
    def launch(self, progression_callback=None, message_callback=None, params=None):
        ...
        if params and 'data' in params:
            ...
            elif self.get_config_key('decriptor_filename'):
                descriptor = get_descriptor(self.get_config_key('decriptor_filename'), self.get_config_key('source_encoding'))
            elif self.get_config_key('decriptor'):
                descriptor = Descriptor(ET.fromstring(self.get_config_key('decriptor')), self.get_config_key('source_encoding'), buffersize=self.get_config_key('buffer_size', 16384))
            if params and 'source' in params:
                ...
                source = codecs.open(source_filename, 'r', encoding=source_encoding)
            record_count = None
