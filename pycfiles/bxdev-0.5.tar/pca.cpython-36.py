...
# Embedded file name: build/bdist.linux-x86_64/egg/bxa/sherpa/background/pca.py
# Compiled at: 2020-01-30 05:05:11
# Size of source mod 2**32: 15958 bytes
from __future__ import print_function
import numpy, json, logging, warnings, os
...
def pca_cut(U, s, V, mean, ncomponents=20):
    return (
     U[:, :ncomponents], s[:ncomponents], V[:, :ncomponents], mean)
...
def get_unit_response(i):
    ...
    unit_arf.specresp = 0.0 * unit_arf.specresp + 1.0
...
class PCAModel(ArithmeticModel):
    ...
    def __init__(self, modelname, data):
        ...
        self.ilo = data['ilo']
        self.ihi = data['ihi']
        p0 = Parameter(modelname=modelname, name='lognorm', val=1, min=(-5), max=20, hard_min=(-100),
          hard_max=100)
        pars = [p0]
        for i in range(len(self.s)):
            pi = Parameter(modelname=modelname, name=('PC%d' % (i + 1)), val=0,
              min=(-20),
              max=20,
              hard_min=(-1e+300),
              hard_max=1e+300)
...
class GaussModel(ArithmeticModel):
    ...
    def __init__(self, modelname):
        self.LineE = Parameter(modelname=modelname, name='LineE', val=1, min=0, max=1e+38)
        self.Sigma = Parameter(modelname=modelname, name='Sigma', val=1, min=0, max=1e+38)
        self.norm = Parameter(modelname=modelname, name='norm', val=1, min=0, max=1e+38)
...
class PCAFitter(object):
    ...
    def decompose(self):
        ...
        y = numpy.log10(cts * 1.0 / ncts + 1.0)
        z = (y - mean) * V
    ...
    def calc_bkg_stat(self):
        ss = [s for s in get_stat_info() if self.id in s.ids if s.bkg_ids is not None if len(s.bkg_ids) > 0]
        if len(ss) != 1:
    ...
    def fit(self):
        ...
        last_aic, last_final, last_nparams, last_val = (
         aic, final, nparams, val)
