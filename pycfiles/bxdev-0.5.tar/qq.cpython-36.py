...
# Embedded file name: build/bdist.linux-x86_64/egg/bxa/xspec/qq.py
# Compiled at: 2020-01-28 12:31:59
# Size of source mod 2**32: 5073 bytes
...
def ADstat(data, model):
    ...
    if not (modelc > 0).all():
        raise AssertionError(['ADstat has zero cumulative denominator', modelc])
    elif not (maxmodelc - modelc > 0).all():
        raise AssertionError(['ADstat has zero=1-1 cumulative denominator', maxmodelc - modelc])
    ad = ((modelc - datac) ** 2 / (modelc * (maxmodelc - modelc)) * model).sum()
    return ad


def qq_plot(bins, data, model, markers=[
 0.2, 1, 2, 5, 10], unit='', annotate=True):
    """
    ...
    if not numpy.isfinite(data).all():
        ...
        if annotate:
            ...
            stats = dict(ks=(KSstat(data, model)),
...
def qq(analyzer, markers=5, annotate=True):
    ...
    if not hasattr(markers, '__len__'):
        ...
        decimals = int(-numpy.log10(bins[(-1)] - bins[0] + 0.0001))
        markers = numpy.linspace(bins[0], bins[(-1)], nmarkers + 2)[1:-1]
        markers = set(numpy.round(markers, decimals=decimals))
    qq_plot(bins=bins, data=(data * width * 2), model=(model * width * 2), markers=markers,
      annotate=annotate,
      unit=(Plot.xAxis))
