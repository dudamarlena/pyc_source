...
# Embedded file name: build/bdist.linux-x86_64/egg/bxa/xspec/sinning.py
...
import matplotlib.pyplot as plt, scipy.special, scipy.stats, sys, os
from . import gof
import tqdm
...
def binning(outputfiles_basename, bins, widths, data, models):
    ...
    for icomponent in range(models.shape[1]):
        ...
        for i, counts_predicted in enumerate(tqdm.tqdm(component)):
            ...
            curgof = -numpy.log10(numpy.min([stats[(stats[:, 0] == n)][:, 2].min() * (stats[:, 0] == n).sum() for n in sorted(set(stats[:, 0]))]) + 1e-300)
    ...
    for (xloi, xhii, ydatai), gofpi in zip(grouped_data, data_gofp):
        ...
        marked_binned.append(dict(x=((xloi + xhii) / 2.0), xerr=((-xloi + xhii) / 2.0), y=y,
          yerr=[
         [
          modelrange_high - y], [y - modelrange_low]],
          color=c))
        ymin = min(ymin, modelrange_low)
        ymax = max(ymax, modelrange_high)

    return dict(marked_binned=marked_binned, gof_avg=gof_avg,
      gof_total=gof_total,
      stats=stats,
      xlim=(
     xlo[0], xhi[(-1)]),
