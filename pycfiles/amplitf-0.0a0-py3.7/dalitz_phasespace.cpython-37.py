...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/amplitf/phasespace/dalitz_phasespace.py
# Compiled at: 2020-04-23 07:11:02
# Size of source mod 2**32: 15053 bytes
...
class DalitzPhaseSpace:
    ...
    def rectangular_grid_sample(self, size_ab, size_bc, space_to_sample='DP'):
        ...
        if space_to_sample == 'linDP':
            vab = (mgrid[0:size_ab, 0:size_bc][0] * (math.sqrt(self.maxab) - math.sqrt(self.minab)) / float(size_ab) + math.sqrt(self.minab)) ** 2.0
            vbc = (mgrid[0:size_ab, 0:size_bc][1] * (math.sqrt(self.maxbc) - math.sqrt(self.minbc)) / float(size_bc) + math.sqrt(self.minbc)) ** 2.0
            v = [vab.reshape(size).astype('d'), vbc.reshape(size).astype('d')]
            dlz = tf.stack(v, axis=1)
        else:
            if space_to_sample == 'sqDP':
                x = np.linspace(self.min_mprimeac, self.max_mprimeac, size_ab)
                y = np.linspace(self.min_thprimeac, self.max_thprimeac, size_bc)
                xnew = x[((x > self.min_mprimeac) & (x < self.max_mprimeac) & (y > self.min_thprimeac) & (y < self.max_thprimeac))]
                ynew = y[((x > self.min_mprimeac) & (x < self.max_mprimeac) & (y > self.min_thprimeac) & (y < self.max_thprimeac))]
                mprimeac, thprimeac = np.meshgrid(xnew, ynew)
                dlz = self.from_square_dalitz_plot(mprimeac.flatten().astype('d'), thprimeac.flatten().astype('d'))
            else:
                vab = mgrid[0:size_ab, 0:size_bc][0] * (self.maxab - self.minab) / float(size_ab) + self.minab
                vbc = mgrid[0:size_ab, 0:size_bc][1] * (self.maxbc - self.minbc) / float(size_bc) + self.minbc
                v = [vab.reshape(size).astype('d'), vbc.reshape(size).astype('d')]
                dlz = tf.stack(v, axis=1)
    ...
    def m_prime_ac(self, sample):
        ...
        return atfi.acos(2 * (mac - math.sqrt(self.minac)) / (math.sqrt(self.maxac) - math.sqrt(self.minac)) - 1.0) / math.pi
    ...
    def m_prime_ab(self, sample):
        ...
        return atfi.acos(2 * (mab - math.sqrt(self.minab)) / (math.sqrt(self.maxab) - math.sqrt(self.minab)) - 1.0) / math.pi
    ...
    def from_square_dalitz_plot(self, mprimeac, thprimeac):
        ...
        """
        m2AC = 0.25 * (self.maxac ** 0.5 * atfi.cos(math.pi * mprimeac) + self.maxac ** 0.5 - self.minac ** 0.5 * atfi.cos(math.pi * mprimeac) + self.minac ** 0.5) ** 2
        m2AB = 0.5 * (-m2AC ** 2 + m2AC * self.ma ** 2 + m2AC * self.mb ** 2 + m2AC * self.mc ** 2 + m2AC * self.md ** 2 - m2AC * atfi.sqrt((m2AC * (m2AC - 2.0 * self.ma ** 2 - 2.0 * self.mc ** 2) + self.ma ** 4 - 2.0 * self.ma ** 2 * self.mc ** 2 + self.mc ** 4) / m2AC) * atfi.sqrt((m2AC * (m2AC - 2.0 * self.mb ** 2 - 2.0 * self.md ** 2) + self.mb ** 4 - 2.0 * self.mb ** 2 * self.md ** 2 + self.md ** 4) / m2AC) * atfi.cos(math.pi * thprimeac) - self.ma ** 2 * self.mb ** 2 + self.ma ** 2 * self.md ** 2 + self.mb ** 2 * self.mc ** 2 - self.mc ** 2 * self.md ** 2) / m2AC
    ...
    def square_dalitz_plot_jacobian(self, sample):
        ...
        mAC = atfi.const(0.5) * diff_AC * (Const(1.0) + atfi.cos(atfi.pi() * mPrime)) + tf.cast(atfi.sqrt(self.minac), atfi.fptype())
        mACSq = mAC * mAC
        eAcmsAC = atfi.const(0.5) * (mACSq - tf.cast(self.mc2, atfi.fptype()) + tf.cast(self.ma2, atfi.fptype())) / mAC
        eBcmsAC = atfi.const(0.5) * (tf.cast(self.md, atfi.fptype()) ** 2.0 - mACSq - tf.cast(self.mb2, atfi.fptype())) / mAC
    ...
    def invariant_mass_jacobian(self, sample):
        ...
        """
        return atfi.const(4.0) * atfi.sqrt(self.m2ab(sample)) * atfi.sqrt(self.m2bc(sample))
    ...
    def m_prime_bc(self, sample):
        ...
        return atfi.acos(2 * (mbc - math.sqrt(self.minbc)) / (math.sqrt(self.maxbc) - math.sqrt(self.minbc)) - 1.0) / math.pi
    ...
    def final_state_momenta(self, m2ab, m2bc):
        ...
        p4a = atfk.lorentz_vector(atfk.vector(atfi.zeros(p_a), atfi.zeros(p_a), p_a), atfi.sqrt(p_a ** 2 + self.ma2))
        p4b = atfk.lorentz_vector(atfk.vector(p_b * atfi.sqrt(1.0 - cos_theta_b ** 2), atfi.zeros(p_b), -p_b * cos_theta_b), atfi.sqrt(p_b ** 2 + self.mb2))
        p4c = atfk.lorentz_vector(atfk.vector(-p_c * atfi.sqrt(1.0 - cos_theta_c ** 2), atfi.zeros(p_c), -p_c * cos_theta_c), atfi.sqrt(p_c ** 2 + self.mc2))
        return (p4a, p4b, p4c)
