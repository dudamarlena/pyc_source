...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/amplitf/kinematics.py
# Compiled at: 2020-04-24 08:45:30
# Size of source mod 2**32: 28810 bytes
import sys, operator, tensorflow as tf, numpy as np, math, itertools
...
def eta(vector):
    ...
    """
    return -atfi.log(pt(vector) / 2.0 / z_component(vector))
...
def rotate(v, angle, axis):
    ...
    if angle != atfi.zeros(angle):
        ...
        if ll == atfi.zeros(ll):
            ...
            vy = y_component(v)
            vz = z_component(v)
            _vx = (ca + (1 - ca) * dx * dx) * vx + ((1 - ca) * dx * dy - sa * dz) * vy + ((1 - ca) * dx * dz + sa * dy) * vz
            _vy = ((1 - ca) * dy * dx + sa * dz) * vx + (ca + (1 - ca) * dy * dy) * vy + ((1 - ca) * dy * dz - sa * dx) * vz
            _vz = ((1 - ca) * dz * dx - sa * dy) * vx + ((1 - ca) * dz * dy + sa * dx) * vy + (ca + (1 - ca) * dz * dz) * vz
...
def project_lorentz_vector(p, axes):
    ...
    p1 = lorentz_vector(vector(scalar_product(x1, p0), scalar_product(y1, p0), scalar_product(z1, p0)), time_component(p))
    return p1
...
def spherical_angles(pb):
    ...
    """
    z1 = unit_vector(spatial_components(pb))
    theta = atfi.acos(z_component(z1))
    phi = atfi.atan2(y_component(pb), x_component(pb))
    return (theta, phi)
...
def nested_helicity_angles(pdecays):
    ...
    angles = ()
    if len(pdecays) != 2:
        sys.exit('ERROR in nested_helicity_angles: lenght of the input list is different from 2')
...
def change_axes(ps, newaxes):
    ...
    for p in ps:
        ...
        py = y_component(p)
        pz = z_component(p)
        pout.append(lorentz_vector(vector(px * x_component(xnew) + py * y_component(xnew) + pz * z_component(xnew), px * x_component(ynew) + py * y_component(ynew) + pz * z_component(ynew), px * x_component(znew) + py * y_component(znew) + pz * z_component(znew)), time_component(p)))
...
def axes_after_rotation(pb, oldaxes=None):
    ...
    a0 = z0 - z1 * scalar(sp)
    x1 = tf.where(scalar(tf.equal(sp, 1.0)), x0, -unit_vector(a0))
    y1 = vector_product(z1, x1)
    return (x1, y1, z1)
...
def axes_before_rotation(pb):
    ...
    z1 = unit_vector(spatial_components(pb))
    eb = time_component(pb)
    z0 = vector(atfi.zeros(eb), atfi.zeros(eb), atfi.ones(eb))
    x0 = vector(atfi.ones(eb), atfi.zeros(eb), atfi.zeros(eb))
    sp = scalar_product(z1, z0)
    a0 = z0 - z1 * scalar(sp)
...
def helicity_angles_3body(pa, pb, pc):
    ...
    pa_prime = lorentz_vector(rotate_euler(spatial_components(pa), -phi_r, atfi.pi() - theta_r, phi_r), time_component(pa))
    pb_prime = lorentz_vector(rotate_euler(spatial_components(pb), -phi_r, atfi.pi() - theta_r, phi_r), time_component(pb))
...
def cos_helicity_angle(p1, p2):
    ...
    """
    p12 = lorentz_vector(spatial_components(p1) + spatial_components(p2), time_component(p1) + time_component(p2))
    pcm1 = boost_to_rest(p1, p12)
    cosHel = scalar_product(unit_vector(spatial_components(pcm1)), unit_vector(spatial_components(p12)))
    return cosHel
...
def wigner_capital_d(phi, theta, psi, j, m1, m2):
    ...
    return atfi.exp(-i * atfi.cast_complex(m1 / 2.0 * phi)) * atfi.cast_complex(wigner_small_d(theta, j, m1, m2)) * atfi.exp(-i * atfi.cast_complex(m2 / 2.0 * psi))
...
def wigner_small_d(theta, j, m1, m2):
    ...
    import sympy.physics.quantum.spin as Wigner
...
def spin_rotation_angle(pa, pb, pc, bachelor=2):
    ...
    """
    if bachelor == 2:
        return atfi.const(0.0)
    pboost = lorentz_vector(-spatial_components(pb) / scalar(time_component(pb)), time_component(pb))
    if bachelor == 0:
...
def helicity_amplitude_3body(thetaR, phiR, thetaA, phiA, spinD, spinR, mu, lambdaR, lambdaA, lambdaB, lambdaC):
    ...
    ph = (mu - lambda1) / 2.0 * phiR + (lambdaR - lambda2) / 2.0 * phiA
    d_terms = wigner_small_d(thetaR, spinD, mu, lambda1) * wigner_small_d(thetaA, spinR, lambdaR, lambda2)
...
def helicity_couplings_from_ls(ja, jb, jc, lb, lc, bls):
    ...
    for ls, b in bls.items():
        l = ls[0]
        s = ls[1]
        coeff = math.sqrt((l + 1) / (ja + 1)) * atfi.clebsch(jb, lb, jc, -lc, s, lb - lc) * atfi.clebsch(l, 0, s, lb - lc, ja, lb - lc)
        if coeff:
            a += atfi.complex(atfi.const(float(coeff)), atfi.const(0.0)) * b
...
def zemach_tensor(m2ab, m2ac, m2bc, m2d, m2a, m2b, m2c, spin):
    ...
    if spin == 1:
        z = atfi.complex(m2ac - m2bc + (m2d - m2c) * (m2b - m2a) / m2ab, atfi.const(0.0))
    if spin == 2:
        z = atfi.complex((m2bc - m2ac + (m2d - m2c) * (m2a - m2b) / m2ab) ** 2 - 0.3333333333333333 * (m2ab - 2.0 * (m2d + m2c) + (m2d - m2c) ** 2 / m2ab) * (m2ab - 2.0 * (m2a + m2b) + (m2a - m2b) ** 2 / m2ab), atfi.const(0.0))
    return z
...
def complex_two_body_momentum(md, ma, mb):
    ...
    """
    return atfi.sqrt(atfi.complex((md ** 2 - (ma + mb) ** 2) * (md ** 2 - (ma - mb) ** 2) / (4 * md ** 2), atfi.const(0.0)))