...
# [GCC 8.4.0]
# Embedded file name: /Users/jordansuchow/Dropbox/Berkeley/Projects/Current/Wallace/wallace/recruiters.py
...
class PsiTurkRecruiter(Recruiter):
    ...
    def __init__(self):
        ...
        self.aws_secret_access_key = os.getenv('aws_secret_access_key', self.config.get('AWS Access', 'aws_secret_access_key'))
    ...
    def open_recruitment(self, n=1):
        ...
        psiturk_access_key_id = os.getenv('psiturk_access_key_id', self.config.get('psiTurk Access', 'psiturk_access_key_id'))
        psiturk_secret_access_id = os.getenv('psiturk_secret_access_id', self.config.get('psiTurk Access', 'psiturk_secret_access_id'))
        ...
        self.amt_services = MTurkServices(self.aws_access_key_id, self.aws_secret_access_key, self.config.getboolean('Shell Parameters', 'launch_in_sandbox_mode'))
        self.shell = PsiturkNetworkShell(self.config, self.amt_services, aws_rds_services, web_services, self.server, self.config.getboolean('Shell Parameters', 'launch_in_sandbox_mode'))
        try:
            participants = Participant.query.all()
            assert participants
        except Exception:
            self.shell.hit_create(n, self.config.get('HIT Configuration', 'base_payment'), self.config.get('HIT Configuration', 'duration'))
        else:
    ...
    def recruit_participants(self, n=1):
        ...
        if auto_recruit:
            ...
            mturkparams = dict(aws_access_key_id=self.aws_access_key_id, aws_secret_access_key=self.aws_secret_access_key, host=host)
