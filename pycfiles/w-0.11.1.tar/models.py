...
# [GCC 8.4.0]
# Embedded file name: /Users/jordansuchow/Dropbox/Berkeley/Projects/Current/Wallace/wallace/models.py
# Compiled at: 2016-08-04 19:15:29
...
class Participant(Base, SharedMixin):
    ...
    bonus = Column(Float)
    status = Column(Enum('working', 'submitted', 'approved', 'rejected', 'returned', 'abandoned', 'did_not_attend', 'bad_data', 'missing_notification', name='participant_status'), nullable=False, default='working', index=True)
...
class Network(Base, SharedMixin):
    ...
    def nodes(self, type=None, failed=False, participant_id=None):
        ...
        if participant_id is not None:
            if failed == 'all':
                return type.query.filter_by(network_id=self.id, participant_id=participant_id).all()
            else:
                return type.query.filter_by(network_id=self.id, participant_id=participant_id, failed=failed).all()
    ...
    def transmissions(self, status='all', failed=False):
        ...
        if status == 'all':
            ...
            if failed == 'all':
                ...
                return Transmission.query.filter_by(network_id=self.id, status=status, failed=failed).all()
...
class Node(Base, SharedMixin):
    ...
    def vectors(self, direction='all', failed=False):
        ...
        if failed == 'all':
            ...
            if direction == 'incoming':
                return Vector.query.filter_by(destination_id=self.id, failed=failed).all()
            if direction == 'outgoing':
    ...
    def is_connected(self, whom, direction='to', failed=None):
        ...
        connected = []
        if direction == 'to':
            vectors = Vector.query.with_entities(Vector.destination_id).filter_by(origin_id=self.id, failed=False).all()
            destinations = set([ v.destination_id for v in vectors ])
            for w in whom_ids:
                connected.append(w in destinations)

        elif direction == 'from':
            vectors = Vector.query.with_entities(Vector.origin_id).filter_by(destination_id=self.id, failed=False).all()
    ...
    def received_infos(self, type=None, failed=None):
        ...
        if not issubclass(type, Info):
            raise TypeError(('Cannot get infos of type {} as it is not a valid type.').format(type))
        transmissions = Transmission.query.with_entities(Transmission.info_id).filter_by(destination_id=self.id, status='received', failed=False).all()
    ...
    def transmissions(self, direction='outgoing', status='all', failed=False):
        ...
        if direction == 'all':
            if status == 'all':
                return Transmission.query.filter(and_(Transmission.failed == False, or_(Transmission.destination_id == self.id, Transmission.origin_id == self.id))).all()
            else:
                return Transmission.query.filter(and_(Transmission.failed == False, Transmission.status == status, or_(Transmission.destination_id == self.id, Transmission.origin_id == self.id))).all()

        if direction == 'incoming':
            if status == 'all':
                return Transmission.query.filter_by(failed=False, destination_id=self.id).all()
            else:
                return Transmission.query.filter(and_(Transmission.failed == False, Transmission.destination_id == self.id, Transmission.status == status)).all()

        if direction == 'outgoing':
            if status == 'all':
                return Transmission.query.filter_by(failed=False, origin_id=self.id).all()
            else:
                return Transmission.query.filter(and_(Transmission.failed == False, Transmission.origin_id == self.id, Transmission.status == status)).all()
    ...
    def transmit(self, what=None, to_whom=None):
        ...
        for w in what:
            for tw in to_whom:
                try:
                    vector = [ v for v in vectors if v.destination_id == tw.id
                             ][0]
                except:
...
class Vector(Base, SharedMixin):
    ...
    origin = relationship(Node, foreign_keys=[origin_id], backref='all_outgoing_vectors')
    destination_id = Column(Integer, ForeignKey('node.id'), index=True)
    destination = relationship(Node, foreign_keys=[destination_id], backref='all_incoming_vectors')
    network_id = Column(Integer, ForeignKey('network.id'), index=True)
    ...
    def transmissions(self, status='all'):
        ...
        if status == 'all':
            ...
            return Transmission.query.filter_by(vector_id=self.id, status=status, failed=False).all()
...
class Info(Base, SharedMixin):
    ...
    def transmissions(self, status='all'):
        ...
        if status == 'all':
            ...
            return Transmission.query.filterby(info_id=self.id, status=status, failed=False).all()
...
class Transmission(Base, SharedMixin):
    ...
    origin_id = Column(Integer, ForeignKey('node.id'), index=True)
    origin = relationship(Node, foreign_keys=[origin_id], backref='all_outgoing_transmissions')
    destination_id = Column(Integer, ForeignKey('node.id'), index=True)
    destination = relationship(Node, foreign_keys=[destination_id], backref='all_incoming_transmissions')
    network_id = Column(Integer, ForeignKey('network.id'), index=True)
    network = relationship(Network, backref='networks_transmissions')
    receive_time = Column(DateTime, default=None)
    status = Column(Enum('pending', 'received', name='transmission_status'), nullable=False, default='pending', index=True)
...
class Transformation(Base, SharedMixin):
    ...
    info_in = relationship(Info, foreign_keys=[info_in_id], backref='transformation_applied_to')
    info_out_id = Column(Integer, ForeignKey('info.id'), index=True)
    info_out = relationship(Info, foreign_keys=[info_out_id], backref='transformation_whence')
    ...
    def __init__(self, info_in, info_out):
        ...
        if info_in.origin_id != info_out.origin_id and info_in.id not in [ t.info_id for t in info_out.origin.transmissions(direction='incoming', status='received')
                                                                         ]:
