...
# [GCC 8.4.0]
# Embedded file name: /Users/jordansuchow/Dropbox/Berkeley/Projects/Current/Wallace/wallace/networks.py
...
class DiscreteGenerational(Network):
    ...
    def add_node(self, node):
        ...
        if curr_generation == 0:
            ...
            prev_agents = Node.query.filter_by(failed=False, network_id=self.id, generation=curr_generation - 1).all()
    ...
    def add_node(self, node):
        ...
        if len(nodes) <= self.m0:
            ...
            for idx_newvector in xrange(self.m):
                these_nodes = [ n for n in nodes if n.id != node.id and not n.is_connected(direction='either', whom=node)
                              ]
...
class SequentialMicrosociety(Network):
    ...
    def add_node(self, node):
        ...
        nodes = sorted(self.nodes(), key=attrgetter('creation_time'), reverse=True)
        other_nodes = [ n for n in nodes if n.id != node.id ]
