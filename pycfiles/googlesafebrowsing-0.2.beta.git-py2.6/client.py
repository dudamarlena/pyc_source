...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/googlesafebrowsing/client.py
# Compiled at: 2010-12-12 06:17:14
"""Google Safe Browsing protocol version 2.2 client."""
import datetime, logging, os, sys, tempfile, threading, datastore, expression, hashprefix_trie, server, util
...
def FullHashIsCurrent(list_entry, sbl, now):
  ...
  """
    ...
    return sbl.UpdateTime() is not None and now - sbl.UpdateTime() < Client.UPDATED_MAX or updated is not None and now - updated < Client.UPDATED_MAX
            ...
            for (addchunknum, hashes_set) in addchunknum_map.iteritems():
                for fullhash in hashes_set:
                    for expr in prefix_matches.GetPrefixMatches(fullhash):
                        if sbl.AddFullHash(fullhash, addchunknum, ghresp.timestamp) and sbl in check_sbls and expr.HashValue() == fullhash:
                            if debug_info:
...
class Client(object):
  ...
  """
    ...
    def __init__(self, ds, apikey, hp=('safebrowsing.clients.google.com', 80), ssl_hp=('sb-ssl.google.com', 443), base_path='/safebrowsing', use_mac=True, size_limit=None, force_delay=None, pre_update_hook=lambda cl: None, post_update_hook=lambda cl: None, gethash_server=None, update_lists=False, sb_server=None, sb_lists=None):
        ...
        if not sb_server:
            self._server = server.Server(hp, ssl_hp, base_path, clientkey=clientkey, wrkey=wrkey, apikey=apikey, gethash_server=gethash_server)
        else:
        ...
        self._thr = threading.Thread(target=self._PollForData, args=(
    ...
    """
        ...
        while True:
            try:
                ...
                logging.info('Finished update number %d, next delay: %d', num_updates, requested_delay)
                if updates_done:
            ...
            logging.debug('requested_delay: %d, force_delay: %s', requested_delay, self._force_delay)
    ...
    """
        ...
        download = self._server.Download(self._sbls.values(), size_limit_bytes=self._size_limit)
        logging.debug('Minimum delay: %d', download.min_delay_sec)
        if download.rekey:
            self._Rekey()
            return (
             download.min_delay_sec, False)
        if download.reset:
            self._sbls.clear()
            return (
             download.min_delay_sec, False)
        ...
        return (
         download.min_delay_sec, updates_done)
...
class UrlChecker(object):
    ...
    """
        ...
        for sbl in cl.Lists().itervalues():
            logging.debug('%s: %d prefixes, %s', sbl.Name(), sbl.NumPrefixes(), sbl.DownloadRequest())
...
def PrintUsage(argv):
    print >> sys.stderr, 'Usage: %s <APIKey> [check <URL> <URL> ...]\nVisit "http://code.google.com/apis/safebrowsing/key_signup.html" to obtain an APIKey' % (
     argv[0],)
...
def CheckForUrl(apikey, urls):
    ...
    cl = Client(ds, apikey, post_update_hook=checker.Updated)
