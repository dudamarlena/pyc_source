...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/googlesafebrowsing/server.py
# Compiled at: 2010-12-12 08:29:33
"""Encapsulates interaction with the safebrowsing servers."""
import sblist, util, base64, datetime, hmac, httplib, itertools, logging, re
try:
    from hashlib import sha1 as sha
except ImportError:
    import sha

import socket, StringIO, struct, sys, urllib2
...
class Server(object):
  ...
  """
    ...
    MAC = re.compile('m:(.+)')
    NEXT = re.compile('n:(\\d+)')
    PLEASEREKEY = re.compile('e:pleaserekey')
    PLEASERESET = re.compile('r:pleasereset')
    LISTRESP = re.compile('i:(.+)')
    URLRESP = re.compile('u:(.+)')
    ADDORSUB = re.compile('([as]):(\\d+):(\\d+):(\\d+)')
    ADDDELRESP = re.compile('ad:(.+)')
    SUBDELRESP = re.compile('sd:(.+)')
    FULLHASHLEN = 32

    def __init__(self, hp, ssl_hp, base_path, clientkey=None, wrkey=None, apikey=None, timeout=20, gethash_server=None, url_request_function=UrllibRequest):
    ...
    """
        ...
        req_lines.append('')
        linereader = LineReader(self._MakeRequest(Server.DOWNLOADS, postdata=('\n').join(req_lines), use_apikey=True))
        ...
        while linereader.ReadLine() != '':
            ...
            if m:
                dlresp.listops[active_sbl.Name()].append(AddDel(active_sbl, Server._GetSequence(m.group(1))))
                continue
            m = Server.SUBDELRESP.match(line)
            if m:
                dlresp.listops[active_sbl.Name()].append(SubDel(active_sbl, Server._GetSequence(m.group(1))))
                continue
    ...
    """
        ...
        if len(prefixes) == 0:
            ...
            try:
                resp = self._MakeRequest(Server.GETHASH, postdata='%d:%d\n%s' % (prefix_length, prefix_length * len(prefixes),
                 ('').join(prefixes)), use_apikey=True, hp=(
                 self._gethash_host, self._gethash_port))
            ...
            for line in resp:
                ...
                while not datareader.End():
                    ghresp.listmap.setdefault(listname, {}).setdefault(addchunk, set()).add(datareader.Read(Server.FULLHASHLEN))
    ...
    def _MakeRequest(self, path, postdata=None, hp=None, use_wrkey=True, use_apikey=False, extra_params='', protocol='http'):
        if hp is None:
            hp = (
    ...
    """
        resp = self._MakeRequest(Server.NEWKEY, hp=(
         self._ssl_host, self._ssl_port), protocol='https')
        clientkey = None
        wrkey = None
        for line in resp:
            split = line.split(':')
    ...
    """
        ...
        try:
            ...
            raise ServerError('Redirect to "%s" failed: %s' % (url, e), original_error=e)
    ...
    def __str__(self):
        ...
        for (listname, chunk_set) in listmap:
            ...
            for (chunknum, prefixes) in sorted(chunk_set.items(), cmp=cmp_first):
                listmap_str += '\t\t%d: %s\n' % (
                 chunknum, (', ').join([ util.Bin2Hex(pre) for pre in prefixes ]))
    ...
    def Apply(self):
        if self._sbl.GotAddChunk(self._chunknum):
            if not len(self._prefixes):
                ...
                return
        assert len(self._prefixes), 'AddChunk objects should only be created for non-empty chunks'
    ...
    def Apply(self):
        if self._sbl.GotSubChunk(self._chunknum):
            if not len(self._prefixes):
                ...
                logging.debug('Recieved duplicate chunk, ignoring')
        assert len(self._prefixes), 'SubChunk objects should only be created for non-empty chunks'
