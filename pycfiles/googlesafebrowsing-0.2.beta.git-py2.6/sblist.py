...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/googlesafebrowsing/sblist.py
# Compiled at: 2010-12-12 06:19:10
"""List objects represent a Google safe browsing blacklist."""
import hashprefix_trie, util, logging, urlparse
...
class List(object):
  ...
  """
    ...
    """
        ...
        if sub_entry:
            logging.debug('Presubbed: %s:%d:%s', self.Name(), addchunknum, util.Bin2Hex(prefix))
            ...
            if prefix in chunknum_prefixes:
                logging.warning('Prefix %s already added from add chunk %d. Ignoring', util.Bin2Hex(prefix), addchunknum)
                return False
    ...
    """
        ...
        if addchunknum in self._chunknum_map:
            ...
            if add_entry is not None:
                ...
                logging.warning('Unable to remove missing prefix:%s sub:%d add:%s', util.Bin2Hex(prefix), subchunknum, addchunknum)
                return False
            return True
        else:
    ...
    """
        if self.DeleteAddChunk(addchunknum):
            logging.debug('Removing data that was associated with add chunk %d' % addchunknum)
