...
# [GCC 8.4.0]
# Embedded file name: /Users/eetu/envs/cmsplugin-articles-ai/project/cmsplugin_articles_ai/models/articles.py
...
__all__ = ('Article', 'ArticleAttachment', 'ArticleQuerySet')
...
def _get_tag_pks(tags):
    ...
    if hasattr(tags, 'values_list'):
        ...
        if all(isinstance(item, int) for item in tags):
            return tags
...
class ArticleQuerySet(models.QuerySet):
    ...
    def tag_filter(self, filter_mode, tags):
        ...
        if filter_mode == TagFilterMode.ALL:
...
class Article(PublisherModel):
    ...
    slug = models.SlugField(max_length=200, verbose_name=_('URL slug'), db_index=True)
    language = LanguageField(verbose_name=_('language'), default=settings.LANGUAGE_CODE, blank=True, help_text=_('Leave this empty if you want the article to be shown regardless of any language filters.'))
    published_from = models.DateTimeField(_('published from'), null=True, blank=True)
    published_until = models.DateTimeField(_('published until'), null=True, blank=True)
    highlight = models.BooleanField(_('highlight'), default=False, help_text=_('Highlight as important.'))
    author = models.ForeignKey(settings.AUTH_USER_MODEL, verbose_name=_('author'), on_delete=models.PROTECT)
    category = models.ForeignKey(Category, verbose_name=_('category'), related_name='articles', null=True, blank=True, on_delete=models.SET_NULL)
    tags = models.ManyToManyField(Tag, verbose_name=_('tags'), related_name='articles', blank=True)
    main_image = FilerImageField(verbose_name=_('main image'), related_name='+', null=True, blank=True, on_delete=models.SET_NULL)
    lead_paragraph = HTMLField(verbose_name=_('lead paragraph'), blank=True)
    main_content = HTMLField(verbose_name=_('content'))
    created_at = models.DateTimeField(_('creation time'), auto_now_add=True, editable=False)
    modified_at = models.DateTimeField(_('last modified'), auto_now=True, editable=False)
...
class ArticleAttachment(models.Model):
    ...
    attachment_file = FilerFileField(verbose_name=_('file'), on_delete=models.SET(''))
    article = models.ForeignKey(Article, verbose_name=_('article'), related_name='attachments')
