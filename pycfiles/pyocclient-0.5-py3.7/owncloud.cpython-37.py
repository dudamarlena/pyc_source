...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/owncloud/owncloud.py
# Compiled at: 2020-01-31 10:27:27
# Size of source mod 2**32: 65726 bytes
...
class ShareInfo(object):
    __doc__ = 'Share information'

    def __init__(self, share_info):
        self.share_info = {}
        del_attrs = [
         'item_type', 'item_source', 'file_source', 'parent',
         'storage', 'mail_send']
    ...
    def get_share_time(self):
        ...
        """
        return datetime.datetime.fromtimestamp(self._get_int('stime'))
...
class PublicShare(ShareInfo):
    __doc__ = 'Public share information'
...
class FileInfo(object):
    ...
    def get_last_modified(self):
        ...
        :rtype: datetime object
        """
        return datetime.datetime.strptime(self.attributes['{DAV:}getlastmodified'], self._DATE_FORMAT)
...
class Client(object):
    ...
    def get_file_contents(self, path):
        ...
        res = self._session.get(self._webdav_url + parse.quote(self._encode_string(path)))
    ...
    def get_file(self, remote_path, local_file=None):
        ...
        res = self._session.get((self._webdav_url + parse.quote(self._encode_string(remote_path))),
          stream=True)
        if res.status_code == 200:
    ...
    def get_directory_as_zip(self, remote_path, local_file):
        ...
        """
        remote_path = self._normalize_path(remote_path)
        url = self.url + 'index.php/apps/files/ajax/download.php?dir=' + parse.quote(remote_path)
    ...
    def put_file(self, remote_path, local_source_file, **kwargs):
        ...
        res = self._make_dav_request('PUT',
          remote_path,
          data=file_handle,
          headers=headers)
        file_handle.close()
    ...
    def put_directory(self, target_path, local_directory, **kwargs):
        r"""Upload a directory with all its contents
    ...
    def _put_file_chunked(self, remote_path, local_source_file, **kwargs):
        r"""Uploads a file using chunks. If the file is smaller than
        ...
        :raises: HTTPResponseError in case an HTTP error status was returned
        """
        chunk_size = kwargs.get('chunk_size', 10485760)
        result = True
        ...
        if kwargs.get('keep_mtime', True):
            headers['X-OC-MTIME'] = str(int(stat_result.st_mtime))
        if size == 0:
            return self._make_dav_request('PUT',
              remote_path,
              data='',
              headers=headers)
        ...
        for chunk_index in range(0, int(chunk_count)):
            ...
            if not self._make_dav_request('PUT',
              chunk_name,
              data=data,
              headers=headers):
                result = False
    ...
    def list_open_remote_share(self):
        ...
        :raises: HTTPResponseError in case an HTTP error status was returned
        """
        res = self._make_ocs_request('GET', self.OCS_SERVICE_SHARE, 'remote_shares/pending')
        if res.status_code == 200:
    ...
    def accept_remote_share(self, share_id):
        ...
        if not isinstance(share_id, int):
            return False
        res = self._make_ocs_request('POST', self.OCS_SERVICE_SHARE, 'remote_shares/pending/' + str(share_id))
        if res.status_code == 200:
            return res
        raise HTTPResponseError(res)
    ...
    def decline_remote_share(self, share_id):
        ...
        if not isinstance(share_id, int):
            return False
        res = self._make_ocs_request('DELETE', self.OCS_SERVICE_SHARE, 'remote_shares/pending/' + str(share_id))
        if res.status_code == 200:
            return res
        raise HTTPResponseError(res)
    ...
    def delete_share(self, share_id):
        ...
        if not isinstance(share_id, int):
            return False
        res = self._make_ocs_request('DELETE', self.OCS_SERVICE_SHARE, 'shares/' + str(share_id))
        if res.status_code == 200:
            return res
        raise HTTPResponseError(res)
    ...
    def update_share(self, share_id, **kwargs):
        ...
        if isinstance(perms, int):
            if perms > self.OCS_PERMISSION_ALL:
                perms = None
        else:
            if not perms:
                if not password:
                    if not public_upload is not None:
                        return False
            return isinstance(share_id, int) or False
        data = {}
        if perms:
        ...
        if public_upload is not None:
            if isinstance(public_upload, bool):
                data['publicUpload'] = str(public_upload).lower()
        res = self._make_ocs_request('PUT',
    ...
    def move(self, remote_path_source, remote_path_target):
        ...
        :raises: HTTPResponseError in case an HTTP error status was returned
        """
        return self._webdav_move_copy(remote_path_source, remote_path_target, 'MOVE')
    ...
    def copy(self, remote_path_source, remote_path_target):
        ...
        :raises: HTTPResponseError in case an HTTP error status was returned
        """
        return self._webdav_move_copy(remote_path_source, remote_path_target, 'COPY')
    ...
    def share_file_with_link(self, path, **kwargs):
        ...
        path = self._normalize_path(path)
        post_data = {'shareType':self.OCS_SHARE_TYPE_LINK, 
        ...
        res = self._make_ocs_request('POST',
          (self.OCS_SERVICE_SHARE),
          'shares',
          data=post_data)
        if res.status_code == 200:
            ...
            return ShareInfo({'id':data_el.find('id').text, 
             'path':path, 
    ...
    def get_share(self, share_id):
        ...
        """
        return share_id is None or isinstance(share_id, int) or None
    ...
    def delete_user(self, user_name):
        ...
        """
        res = self._make_ocs_request('DELETE', self.OCS_SERVICE_CLOUD, 'users/' + user_name)
        if res.status_code == 200:
            return True
        raise HTTPResponseError(res)
    ...
    def get_user_groups(self, user_name):
        ...
        """
        res = self._make_ocs_request('GET', self.OCS_SERVICE_CLOUD, 'users/' + user_name + '/groups')
        if res.status_code == 200:
            tree = ET.fromstring(res.content)
            self._check_ocs_status(tree, [100])
    ...
    def get_user(self, user_name):
        ...
        :raises: ResponseError in case an HTTP error status was returned
        """
        res = self._make_ocs_request('GET',
          (self.OCS_SERVICE_CLOUD),
          ('users/' + parse.quote(user_name)),
          data={})
        tree = ET.fromstring(res.content)
    ...
    def remove_user_from_group(self, user_name, group_name):
        ...
        """
        res = self._make_ocs_request('DELETE',
          (self.OCS_SERVICE_CLOUD),
          ('users/' + user_name + '/groups'),
          data={'groupid': group_name})
        if res.status_code == 200:
    ...
    def get_user_subadmin_groups(self, user_name):
        ...
        """
        res = self._make_ocs_request('GET', self.OCS_SERVICE_CLOUD, 'users/' + user_name + '/subadmins')
        if res.status_code == 200:
            tree = ET.fromstring(res.content)
            self._check_ocs_status(tree, [100])
    ...
    def share_file_with_user--- This code section failed: ---

 L.1257         0  LOAD_FAST                'kwargs'
                2  LOAD_METHOD              get
                4  LOAD_STR                 'remote_user'
                6  LOAD_CONST               False
                8  CALL_METHOD_2         2  '2 positional arguments'
               10  STORE_FAST               'remote_user'

 L.1258        12  LOAD_FAST                'kwargs'
               14  LOAD_METHOD              get
               16  LOAD_STR                 'perms'
               18  LOAD_FAST                'self'
               20  LOAD_ATTR                OCS_PERMISSION_READ
               22  CALL_METHOD_2         2  '2 positional arguments'
               24  STORE_FAST               'perms'

 L.1259        26  LOAD_GLOBAL              isinstance
               28  LOAD_FAST                'perms'
               30  LOAD_GLOBAL              int
               32  CALL_FUNCTION_2       2  '2 positional arguments'
               34  POP_JUMP_IF_FALSE    66  'to 66'
               36  LOAD_FAST                'perms'
               38  LOAD_FAST                'self'
               40  LOAD_ATTR                OCS_PERMISSION_ALL
               42  COMPARE_OP               >
               44  POP_JUMP_IF_TRUE     66  'to 66'

 L.1260        46  LOAD_GLOBAL              isinstance
               48  LOAD_FAST                'user'
               50  LOAD_GLOBAL              six
               52  LOAD_ATTR                string_types
 ...
 L.1261        66  LOAD_CONST               False
               68  RETURN_VALUE     
             70_0  COME_FROM            64  '64'

 L.1263        70  LOAD_FAST                'remote_user'
 ...
 L.1265        92  LOAD_FAST                'self'
               94  LOAD_METHOD              _normalize_path
               96  LOAD_FAST                'path'
               98  CALL_METHOD_1         1  '1 positional argument'
              100  STORE_FAST               'path'

 L.1267       102  LOAD_FAST                'remote_user'
              104  POP_JUMP_IF_FALSE   112  'to 112'
              106  LOAD_FAST                'self'
              108  LOAD_ATTR                OCS_SHARE_TYPE_REMOTE
              110  JUMP_FORWARD        116  'to 116'
            112_0  COME_FROM           104  '104'

 L.1268       112  LOAD_FAST                'self'
              114  LOAD_ATTR                OCS_SHARE_TYPE_USER
            116_0  COME_FROM           110  '110'

 L.1269       116  LOAD_FAST                'user'

 L.1270       118  LOAD_FAST                'self'
              120  LOAD_METHOD              _encode_string
              122  LOAD_FAST                'path'
              124  CALL_METHOD_1         1  '1 positional argument'

 L.1271       126  LOAD_FAST                'perms'
              128  LOAD_CONST               ('shareType', 'shareWith', 'path', 'permissions')
              130  BUILD_CONST_KEY_MAP_4     4 
              132  STORE_FAST               'post_data'

 L.1274       134  LOAD_FAST                'self'
              136  LOAD_ATTR                _make_ocs_request

 L.1275       138  LOAD_STR                 'POST'

 L.1276       140  LOAD_FAST                'self'
              142  LOAD_ATTR                OCS_SERVICE_SHARE

 L.1277       144  LOAD_STR                 'shares'

 L.1278       146  LOAD_FAST                'post_data'
              148  LOAD_CONST               ('data',)
              150  CALL_FUNCTION_KW_4     4  '4 total positional and keyword args'
              152  STORE_FAST               'res'

 L.1281       154  LOAD_FAST                'self'
              156  LOAD_ATTR                _debug
              158  POP_JUMP_IF_FALSE   180  'to 180'

 L.1282       160  LOAD_GLOBAL              print
              162  LOAD_STR                 'OCS share_file request for file %s with permissions %i returned: %i'

 L.1283       164  LOAD_FAST                'path'
              166  LOAD_FAST                'perms'
              168  LOAD_FAST                'res'
              170  LOAD_ATTR                status_code
              172  BUILD_TUPLE_3         3 
              174  BINARY_MODULO    
 ...
 L.1284       180  LOAD_FAST                'res'
              182  LOAD_ATTR                status_code
              184  LOAD_CONST               200
              186  COMPARE_OP               ==
              188  POP_JUMP_IF_FALSE   246  'to 246'

 L.1285       190  LOAD_GLOBAL              ET
              192  LOAD_METHOD              fromstring
              194  LOAD_FAST                'res'
              196  LOAD_ATTR                content
              198  CALL_METHOD_1         1  '1 positional argument'
              200  STORE_FAST               'tree'

 L.1286       202  LOAD_FAST                'self'
              204  LOAD_METHOD              _check_ocs_status
              206  LOAD_FAST                'tree'
              208  CALL_METHOD_1         1  '1 positional argument'
              210  POP_TOP          

 L.1287       212  LOAD_FAST                'tree'
              214  LOAD_METHOD              find
              216  LOAD_STR                 'data'
              218  CALL_METHOD_1         1  '1 positional argument'
              220  STORE_FAST               'data_el'

 L.1288       222  LOAD_GLOBAL              ShareInfo

 L.1290       224  LOAD_FAST                'data_el'
              226  LOAD_METHOD              find
              228  LOAD_STR                 'id'
              230  CALL_METHOD_1         1  '1 positional argument'
              232  LOAD_ATTR                text

 L.1291       234  LOAD_FAST                'path'

 L.1292       236  LOAD_FAST                'perms'
              238  LOAD_CONST               ('id', 'path', 'permissions')
              240  BUILD_CONST_KEY_MAP_3     3 
              242  CALL_FUNCTION_1       1  '1 positional argument'
              244  RETURN_VALUE     
            246_0  COME_FROM           188  '188'

 L.1295       246  LOAD_GLOBAL              HTTPResponseError
              248  LOAD_FAST                'res'
...
Parse error at or near `CALL_FUNCTION_1' instruction at offset 250
    ...
    def delete_group(self, group_name):
        ...
        """
        res = self._make_ocs_request('DELETE', self.OCS_SERVICE_CLOUD, 'groups/' + group_name)
        if res.status_code == 200:
            return True
        raise HTTPResponseError(res)
    ...
    def get_groups(self):
        ...
        """
        res = self._make_ocs_request('GET', self.OCS_SERVICE_CLOUD, 'groups')
        if res.status_code == 200:
            tree = ET.fromstring(res.content)
    ...
    def get_group_members(self, group_name):
        ...
        """
        res = self._make_ocs_request('GET', self.OCS_SERVICE_CLOUD, 'groups/' + group_name)
        if res.status_code == 200:
            tree = ET.fromstring(res.content)
            self._check_ocs_status(tree, [100])
    ...
    def group_exists(self, group_name):
        ...
        """
        res = self._make_ocs_request('GET', self.OCS_SERVICE_CLOUD, 'groups?search=' + group_name)
        if res.status_code == 200:
            tree = ET.fromstring(res.content)
    ...
    def share_file_with_group--- This code section failed: ---

 L.1424         0  LOAD_FAST                'kwargs'
                2  LOAD_METHOD              get
                4  LOAD_STR                 'perms'
                6  LOAD_FAST                'self'
                8  LOAD_ATTR                OCS_PERMISSION_READ
               10  CALL_METHOD_2         2  '2 positional arguments'
               12  STORE_FAST               'perms'

 L.1425        14  LOAD_GLOBAL              isinstance
               16  LOAD_FAST                'perms'
               18  LOAD_GLOBAL              int
               ...
               24  LOAD_FAST                'perms'
               26  LOAD_FAST                'self'
               28  LOAD_ATTR                OCS_PERMISSION_ALL
               30  COMPARE_OP               >
               32  POP_JUMP_IF_TRUE     54  'to 54'

 L.1426        34  LOAD_GLOBAL              isinstance
               36  LOAD_FAST                'group'
               38  LOAD_GLOBAL              six
               40  LOAD_ATTR                string_types
             ...
             54_2  COME_FROM            22  '22'

 L.1427        54  LOAD_CONST               False
               56  RETURN_VALUE     
             58_0  COME_FROM            52  '52'

 L.1429        58  LOAD_FAST                'self'
               60  LOAD_METHOD              _normalize_path
               62  LOAD_FAST                'path'
               64  CALL_METHOD_1         1  '1 positional argument'
               66  STORE_FAST               'path'

 L.1430        68  LOAD_FAST                'self'
               70  LOAD_ATTR                OCS_SHARE_TYPE_GROUP

 L.1431        72  LOAD_FAST                'group'

 L.1432        74  LOAD_FAST                'path'

 L.1433        76  LOAD_FAST                'perms'
               78  LOAD_CONST               ('shareType', 'shareWith', 'path', 'permissions')
               80  BUILD_CONST_KEY_MAP_4     4 
               82  STORE_FAST               'post_data'

 L.1435        84  LOAD_FAST                'self'
               86  LOAD_ATTR                _make_ocs_request

 L.1436        88  LOAD_STR                 'POST'

 L.1437        90  LOAD_FAST                'self'
               92  LOAD_ATTR                OCS_SERVICE_SHARE

 L.1438        94  LOAD_STR                 'shares'

 L.1439        96  LOAD_FAST                'post_data'
               98  LOAD_CONST               ('data',)
              100  CALL_FUNCTION_KW_4     4  '4 total positional and keyword args'
              102  STORE_FAST               'res'

 L.1441       104  LOAD_FAST                'res'
              106  LOAD_ATTR                status_code
              108  LOAD_CONST               200
              110  COMPARE_OP               ==
              112  POP_JUMP_IF_FALSE   170  'to 170'

 L.1442       114  LOAD_GLOBAL              ET
              116  LOAD_METHOD              fromstring
              118  LOAD_FAST                'res'
              120  LOAD_ATTR                content
              122  CALL_METHOD_1         1  '1 positional argument'
              124  STORE_FAST               'tree'

 L.1443       126  LOAD_FAST                'self'
              128  LOAD_METHOD              _check_ocs_status
              130  LOAD_FAST                'tree'
              132  CALL_METHOD_1         1  '1 positional argument'
              134  POP_TOP          

 L.1444       136  LOAD_FAST                'tree'
              138  LOAD_METHOD              find
              140  LOAD_STR                 'data'
              142  CALL_METHOD_1         1  '1 positional argument'
              144  STORE_FAST               'data_el'

 L.1445       146  LOAD_GLOBAL              ShareInfo

 L.1447       148  LOAD_FAST                'data_el'
              150  LOAD_METHOD              find
              152  LOAD_STR                 'id'
              154  CALL_METHOD_1         1  '1 positional argument'
              156  LOAD_ATTR                text

 L.1448       158  LOAD_FAST                'path'

 L.1449       160  LOAD_FAST                'perms'
              162  LOAD_CONST               ('id', 'path', 'permissions')
              164  BUILD_CONST_KEY_MAP_3     3 
              166  CALL_FUNCTION_1       1  '1 positional argument'
              168  RETURN_VALUE     
            170_0  COME_FROM           112  '112'

 L.1452       170  LOAD_GLOBAL              HTTPResponseError
              172  LOAD_FAST                'res'
              174  CALL_FUNCTION_1       1  '1 positional argument'
              176  RAISE_VARARGS_1       1  'exception instance'

Parse error at or near `CALL_FUNCTION_1' instruction at offset 174
    ...
    def get_config(self):
        ...
        """
        path = 'config'
        res = self._make_ocs_request('GET', '', path)
        if res.status_code == 200:
            tree = ET.fromstring(res.content)
            self._check_ocs_status(tree)
            values = []
            ...
            if element is not None:
                keys = [
                 'version', 'website', 'host', 'contact', 'ssl']
                for key in keys:
    ...
    def set_attribute(self, app, key, value):
        ...
        :raises: HTTPResponseError in case an HTTP error status was returned
        """
        path = 'setattribute/' + parse.quote(app, '') + '/' + parse.quote(self._encode_string(key), '')
        res = self._make_ocs_request('POST',
    ...
    def delete_attribute(self, app, key):
        ...
        :raises: HTTPResponseError in case an HTTP error status was returned
        """
        path = 'deleteattribute/' + parse.quote(app, '') + '/' + parse.quote(self._encode_string(key), '')
        res = self._make_ocs_request('POST', self.OCS_SERVICE_PRIVATEDATA, path)
        if res.status_code == 200:
            tree = ET.fromstring(res.content)
            self._check_ocs_status(tree)
            return True
    ...
    def _check_ocs_status(tree, accepted_codes=[100]):
        ...
        if code_el is not None:
            if int(code_el.text) not in accepted_codes:
                r = requests.Response()
    ...
    def make_ocs_request(self, method, service, action, **kwargs):
        r"""Makes a OCS API request and analyses the response
    ...
    def _make_dav_request(self, method, path, **kwargs):
        ...
        res = (self._session.request)(
         method, 
         (self._webdav_url + parse.quote(self._encode_string(path))), **kwargs)
        if self._debug:
    ...
    def _parse_dav_element(self, dav_response):
        ...
        """
        href = parse.unquote(self._strip_dav_path(dav_response.find('{DAV:}href').text))
        if six.PY2:
    ...
    def _webdav_move_copy(self, remote_path_source, remote_path_target, operation):
        ...
        if operation != 'MOVE':
            ...
            remote_path_target = remote_path_target[0] == '/' or '/' + remote_path_target
        remote_path_source = self._normalize_path(remote_path_source)
        headers = {'Destination': self._webdav_url + parse.quote(self._encode_string(remote_path_target))}
        return self._make_dav_request(operation,
          remote_path_source,
          headers=headers)
    ...
    def _get_shareinfo(self, data_el):
        ...
        :returns: instance of ShareInfo class
        """
        return data_el is None or isinstance(data_el, ET.Element) or None
