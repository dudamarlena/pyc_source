    ...
    if has_extension(filename, '.py'):
        for dir in dirs:
            if dir in ('test', 'tests'):
                return 'test'
            if dir in ('doc', 'docs', 'demo', 'example', 'examples'):
                return 'demo'

        if filename.lower().startswith('test_') or os.path.splitext(filename)[0].lower().endswith('_test'):
            return 'test'
        return 'module'
...
def get_package_name_and_type(package, known_extensions):
    ...
    for package_type in known_extensions:
        if package.endswith('.' + package_type):
            return (
             package[:package.rfind('.' + package_type)], package_type)
...
def camel2underscore(name):
    ...
    name = re.sub('([A-Z]+)', capitalize, name)
    return re.sub('([A-Z])', underscore, name)
    ...
    def __init__(cls, name, bases, dict):
        ...
        if 'compute_with' in dict:
            ...
            def _timed_compute_with(self, cheesecake):
                ...
                self.cheesecake.log.debug('Index %s computed in %.2f seconds.' % (
...
class Index(object):
    ...
    def compute(self):
        r"""Compute index value and return it.
    ...
    def _print_info_one(self):
        ...
        print '%s  (%s)' % (
         pad_msg(index_class_to_name(self.name), self.value),
         self.details)
...
class FilesIndex(Index):
    ...
    def get_not_used(self, files_rules):
        ...
        """
        return filter(lambda rule: rule not in self._used_rules, files_rules)
    ...
    def match_filename(self, name, rule):
        ...
        def equal(x, y):
            ...
            if x_root in [y_root.lower(), y_root.upper(), y_root.capitalize()] and x_ext in [y_ext.lower(), y_ext.upper()]:
                return True
            return False
...
class IndexUnitTested(Index):
    ...
    def _is_test_method(self, method):
        nose_methods = [
         'setup', 'setup_package', 'setup_module', 'setUp',
...
class IndexPyLint(Index):
    ...
    def compute(self, files_list, package_dir, pylint_max_execution_time):
        ...
        files_to_lint = filter(lambda name: not name.endswith('__init__.py'), get_files_of_type(files_list, 'module'))
        ...
        for filenames in generate_arguments(files_to_lint, max_arguments_length - len(self._pylint_args())):
            ...
            rc, output = run_cmd('%s %s --persistent=n %s' % (
             pylint_location,
             filenames,
             self._pylint_args()), max_timeout=pylint_max_execution_time)
            if rc & 1 or rc >= 32:
                if output == 'Time exceeded':
                    self.cheesecake.log.debug('pylint exceeded maximum execution time of %d seconds and was terminated.' % pylint_max_execution_time)
                    raise OSError
                self.cheesecake.log.debug('encountered an error (%d):\n***\n%s\n***\n' % (
                 rc, output))
                error_count += 1
            else:
                s = re.search('Your code has been rated at (-?\\d+\\.\\d+)/10', output)
                if s:
    ...
    def _pylint_args(cls):
        try:
            ...
            return (' ').join(map(lambda x: '--%s=%s' % (disable_option, x), cls.disabled_messages))
        except ImportError:
            return ''
...
class IndexPEP8(Index):
    ...
    def compute(self, files_list, package_dir):
        ...
        self.add_info("pep8.py found %d error types; we're scoring %d per error type" % (
        ...
        self.add_info("pep8.py found %d warning types; we're scoring %d per warning type" % (
...
class Cheesecake(object):
    ...
    def __init__(self, keep_log=False, lite=False, logfile=None, name='', path='', pylint_max_execution_time=None, quiet=False, sandbox=None, static_only=False, url='', verbose=False, with_pep8=False):
    ...
    def get_pkg_from_pypi(self):
        ...
        try:
            ...
            url = 'http://www.mems-exchange.org/software/durus/Durus-3.1.tar.gz'
            msg = 'setuptools is not installed and is required for downloading a package by name\nYou can download and process a package by its full URL via the -u or --url option\nExample: python cheesecake.py --url=%s' % url
            self.raise_exception(msg)
        ...
        def fetch_package(mode):
            ...
            if mode == 'pypi_source':
                ...
                source = False
            try:
                output = pkgindex.fetch(Requirement.parse(self.name), self.sandbox, force_scan=True, source=source)
                return (
                 True, output)
    ...
    def unpack_pkg(self):
        ...
        if self.unpack_dir is None:
            self.raise_exception('Could not unpack package %s ... exiting' % self.sandbox_pkg_file)
        self.unpacked = True
    ...
    def install_pkg(self):
        ...
        if self.package_type == 'egg':
            ...
            environment = {'PYTHONPATH': '%(sandbox)s/lib/python2.3/site-packages/:%(sandbox)s/lib/python2.4/site-packages/' % {'sandbox': self.sandbox_install_dir}, 
               'PATH': os.getenv('PATH')}
            rc, output = run_cmd('easy_install --no-deps --prefix %s %s' % (
             self.sandbox_install_dir,
             self.sandbox_pkg_file), environment)
        else:
            ...
            cwd = os.getcwd()
            os.chdir(package_dir)
            rc, output = run_cmd('python setup.py install --root=%s' % self.sandbox_install_dir)
            os.chdir(cwd)
        if rc:
            ...
            self.log('Installation into %s successful.' % self.sandbox_install_dir)
            self.installed = True
...
def process_cmdline_args():
    ...
    parser.add_argument('-n', '--name', dest='name', default='', help='package name (will be retrieved via setuptools utilities, if present)')
    parser.add_argument('-p', '--path', dest='path', default='', help='path of tar.gz/zip package on local file system')
    parser.add_argument('-u', '--url', dest='url', default='', help='package URL')
    parser.add_argument('-q', '--quiet', action='store_true', dest='quiet', default=False, help='only print Cheesecake index value (default=False)')
    parser.add_argument('-v', '--verbose', action='store_true', dest='verbose', default=False, help='verbose output (default=False)')
    parser.add_argument('--lite', action='store_true', dest='lite', default=False, help="don't run time-consuming tests (default=False)")
    parser.add_argument('-t', '--static', action='store_true', dest='static', default=False, help="don't run any code from the package being tested (default=False)")
    parser.add_argument('--with-pep8', action='store_true', dest='with_pep8', default=False, help='check pep8 conformance')
    parser.add_argument('-l', '--logfile', dest='logfile', default=None, help='file to log all cheesecake messages')
    parser.add_argument('-s', '--sandbox', dest='sandbox', default=None, help='directory where package will be unpacked (default is to use random directory inside %s)' % tempfile.gettempdir())
    parser.add_argument('--keep-log', action='store_true', dest='keep_log', default=False, help="don't remove log file even if run was successful")
    parser.add_argument('--pylint-max-execution-time', action='store', dest='pylint_max_execution_time', default=120, help='maximum time (in seconds) you allow pylint process to run (default=120)')
    parser.add_argument('-V', '--version', action='store_true', dest='version', default=False, help='Output cheesecake version and exit')
...
def main():
    ...
    if not name and not url and not path:
        ...
        sys.exit(1)
    try:
        c = Cheesecake(keep_log=keep_log, lite=lite, logfile=logfile, name=name, path=path, pylint_max_execution_time=pylint_max_execution_time, quiet=quiet, sandbox=sandbox, static_only=static_only, url=url, verbose=verbose, with_pep8=with_pep8)
        c.compute_cheesecake_index()
        c.cleanup()
    except CheesecakeError as e:
        print str(e)
