...
# Embedded file name: build/bdist.linux-x86_64/egg/cheesecake/pep8.py
...
import os, sys, re, time, inspect, tokenize
...
indent_match = re.compile('([ \\t]*)').match
raise_comma_match = re.compile('raise\\s+\\w+\\s*(,)').match
operators = ('\n+  -  *  /  %  ^  &  |  =  <  >  >>  <<\n+= -= *= /= %= ^= &= |= == <= >= >>= <<=\n!= <> :\nin is or not and\n').split()
arguments = None
args = None
...
def whitespace_before_parameters(logical_line, tokens):
    ...
    for index in range(1, len(tokens)):
        ...
        if token_type == tokenize.OP and text in '([' and start != prev_end and prev_type == tokenize.NAME and (index < 2 or tokens[(index - 2)][1] != 'class') and not iskeyword(prev_text):
...
class Checker:
    ...
    def __init__(self, filename):
        ...
        arguments.counters['physical lines'] = arguments.counters.get('physical lines', 0) + len(self.lines)
    ...
    def check_logical(self):
        ...
        """
        arguments.counters['logical lines'] = arguments.counters.get('logical lines', 0) + 1
                ...
                if type(offset) is tuple:
                    ...
                    for token_offset, token in self.mapping:
                        if offset >= token_offset:
                            ...
                            original_offset = token[2][1] + offset - token_offset

                self.report_error(original_number, original_offset, text, check)
...
def input_dir(dirname):
    ...
    for root, dirs, files in os.walk(dirname):
        ...
        arguments.counters['directories'] = arguments.counters.get('directories', 0) + 1
        dirs.sort()
        for subdir in dirs:
            if excluded(subdir):
                dirs.remove(subdir)
...
def process_arguments(arglist=None):
    ...
    parser.add_argument('-v', '--verbose', default=0, action='count', help='print status messages, or debug with -vv')
    parser.add_argument('-q', '--quiet', default=0, action='count', help='report only file names, or nothing with -qq')
    parser.add_argument('--exclude', metavar='patterns', default=default_exclude, help='skip matches (default %s)' % default_exclude)
    parser.add_argument('--filename', metavar='patterns', help='only check matching files (e.g. *.py)')
    parser.add_argument('--ignore', metavar='errors', default='', help='skip errors and warnings (e.g. E4,W)')
    parser.add_argument('--repeat', action='store_true', help='show all occurrences of the same error')
    parser.add_argument('--show-source', action='store_true', help='show source code for each error')
    parser.add_argument('--show-pep8', action='store_true', help='show text of PEP 8 for each error')
    parser.add_argument('--statistics', action='store_true', help='count errors and warnings')
    parser.add_argument('--benchmark', action='store_true', help='measure processing speed')
    parser.add_argument('--testsuite', metavar='dir', help='run regression tests from dir')
    parser.add_argument('--doctest', action='store_true', help='run doctest on myself')
