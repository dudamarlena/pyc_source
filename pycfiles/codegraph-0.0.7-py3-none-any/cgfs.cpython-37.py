...
class AssignmentSettingsFile(CachedSpecialFile[t.Dict[(str, str)]]):
    ...
    def parse(self, settings: bytes) -> t.Dict[(str, str)]:
        ...
        for line in settings.split('\n'):
            if not line:
                continue
            key, val = [v.decode('utf8').strip() for v in line.split('=', 1)]
    ...
    def handle_conn(self, conn: socket.socket) -> None:
        ...
        with self.cgfs._lock:
            ...
            if op not in self.ops:
                conn.send('{"ok": false, "error": "unkown op"}')
            del data_dict['op']
            payload = data_dict
            try:
                ...
                logger.debug(traceback.format_exc())
                conn.send('{"ok": false, "error": "Unkown error"}')
...
class CGFS(LoggingMixIn, Operations):
    ...
    def __init__(self, latest_only: bool, socketfile: str, mountpoint: str, tmpdir: str, fixed: bool=False, rubric_append_only: bool=True, assigned_only: bool=False) -> None:
        ...
        with self._lock:
            ...
            self.files.insert(SpecialFile('.cg-mode', 'FIXED\n' if self.fixed else 'NOT_FIXED\n'))
            self.load_courses()
        logger.info('Mounted')
    ...
    def load_submissions(self, assignment: Directory) -> None:
        ...
        for sub in submissions:
            ...
            if sub['user'].get('group') is not None:
                ...
                sub_dir.insert(SpecialFile('.cg-group-members', '\n'.join(members) + '\n'))
