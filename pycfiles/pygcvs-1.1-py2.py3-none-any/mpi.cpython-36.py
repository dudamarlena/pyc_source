...
# Embedded file name: /home/okhin/git/orage.io/pygcrypt/env/lib/python3.6/site-packages/pygcrypt/gctypes/mpi.py
# Compiled at: 2016-09-09 10:00:29
...
class MPI(object):
    ...
    def to_bytes(self):
        ...
        if self.opaque:
            ...
            fmt = getattr(lib, 'GCRYMPI_FMT_' + self.fmt)
...
class MPIint(MPI):
    ...
    def __init__(self, value=0, fmt='USG', flags=[]):
        """
    ...
    def value(self):
        ...
        out = ffi.new('unsigned char [{}]'.format(size[0]), '')
...
class MPIopaque(MPI):
    ...
    def __init__(self, value='', fmt='USG', flags=[]):
        """
        Allocate a new MPI object and initialize it to 0 and initially allocate
        enough memory for a number of at least size bits. This pre-allocation in only
        a small performance issue and not actually necessary because libgcrypt automatically
