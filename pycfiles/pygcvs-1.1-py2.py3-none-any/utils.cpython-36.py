...
# Embedded file name: /home/okhin/git/orage.io/pygcrypt/env/lib/python3.6/site-packages/pygcrypt/utils.py
# Compiled at: 2017-03-12 11:42:23
# Size of source mod 2**32: 9517 bytes
...
def scan(in_bytes, mpi_fmt):
    ...
    fmt = getattr(lib, 'GCRYMPI_FMT_' + mpi_fmt)
...
def create_keys(algo='rsa', nbits=4096, **kwargs):
    """
    ...
    private = key.Key(keySexp['private-key'])
    public = key.Key(keySexp['public-key'])
    return (
     private, public)


def key_derive(passphrase, salt, keysize=128, algo='ITERSALTED_S2K', subalgo='sha256', count=1024):
    """
    ...
    if isinstance(passphrase, str):
        ...
        if not isinstance(passphrase, bytes):
            ...
            algo = getattr(lib, 'GCRY_KDF_' + algo.upper())
            subalgo_int = getattr(lib, 'GCRY_MD_' + subalgo.upper())
            if subalgo_int == 0:
                raise Exception('Sub-Algorithm name {} is unknown'.format(subalgo.decode()))
        key = ffi.new('char [{}]'.format(keysize))
        keysize = ffi.cast('size_t', keysize / 8)
        error = ffi.cast('gcry_error_t', 0)
