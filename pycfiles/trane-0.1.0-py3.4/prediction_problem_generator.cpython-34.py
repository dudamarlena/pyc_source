...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/trane/core/prediction_problem_generator.py
# Compiled at: 2018-04-05 20:01:41
# Size of source mod 2**32: 3579 bytes
import json
...
class PredictionProblemGenerator:
    ...
    def generator(self, dataframe):
        ...
        for ops in iter_over_ops():
            aggregation_op_name, transformation_op_name, row_op_name, filter_op_name = ops
            ...
            is_valid_prediction_problem, filter_column_order_of_types, label_generating_column_order_of_types = prediction_problem.is_valid_prediction_problem(self.table_meta, self.filter_column, self.label_generating_column)
            if not is_valid_prediction_problem:
                continue
            prediction_problem.generate_and_set_hyper_parameters(dataframe, self.label_generating_column, self.filter_column, self.hyper_parameter_memo_table)
            logging.debug('Prediction Problem Generated: {} \n'.format(prediction_problem))
            yield prediction_problem

    def ensure_valid_inputs(self):
        assert self.table_meta.get_type(self.entity_id_column) in [
         TableMeta.TYPE_IDENTIFIER, TableMeta.TYPE_TEXT,
         TableMeta.TYPE_CATEGORY]
        assert self.table_meta.get_type(self.label_generating_column) in [
         TableMeta.TYPE_FLOAT, TableMeta.TYPE_INTEGER]
        assert self.table_meta.get_type(self.time_column) in [
