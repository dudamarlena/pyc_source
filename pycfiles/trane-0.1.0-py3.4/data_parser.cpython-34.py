...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/trane/utils/data_parser.py
...
__all__ = [
 'df_group_by_entity_id', 'csv_to_df', 'parse_data']
...
def parse_data(dataframe, table_meta):
    ...
    for column in columns:
        if table_meta.get_type(column) == TM.TYPE_TIME:
            dataframe[column] = dataframe[column].apply(lambda x: datetime.strptime(x, table_meta.get_property(column, 'format')))
            continue
