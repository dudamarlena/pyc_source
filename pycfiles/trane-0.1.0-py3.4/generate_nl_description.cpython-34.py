...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/trane/utils/generate_nl_description.py
...
def generate_nl_description(problems, meta, entity_id_column, label_generating_column, time_column, cutoff_strategy):
    ...
    def description(prob):
        return 'For each {col}, predict{dataop_des}{filter_des}{cutoff_des}.'.format(col=entity_id_column, dataop_des=dataop_description(prob), filter_des=filter_description(prob), cutoff_des=cutoff_description(prob))
    ...
    def dataop_description(prob):
        ...
        def rowop_description():
            row_op_str_dict = {GreaterRowOp: 'greater than', 
             EqRowOp: 'equal to', 
            ...
            if type(row_op) in row_op_str_dict:
                return ' {col} is {op} {threshold}'.format(col=row_op.column_name, op=row_op_str_dict[type(row_op)], threshold=row_op.hyper_parameter_settings['threshold'])
        ...
        def aggop_description():
            agg_op_str_dict = {FirstAggregationOp: 'the first', 
        ...
        if type(agg_op) == CountAggregationOp:
            ...
            return '{agg_op}{trans_op}{row_op}'.format(agg_op=aggop_description(), trans_op=transop_description(), row_op=rowop_description())
    ...
    def filter_description(prob):
        filter_op_str_dict = {GreaterFilterOp: 'greater than', 
         EqFilterOp: 'equal to', 
        ...
        if isinstance(filter_op, AllFilterOp):
            return ''
        return ', with {col} {op} {threshold}'.format(col=filter_op.column_name, op=filter_op_str_dict[type(filter_op)], threshold=filter_op.hyper_parameter_settings['threshold'])
