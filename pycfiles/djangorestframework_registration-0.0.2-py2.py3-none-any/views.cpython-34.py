...
# [GCC 8.4.0]
# Embedded file name: /home/eugene/Workspace/django-rest-framework-registration/rest_framework_registration/views.py
# Compiled at: 2016-04-03 11:10:51
...
REGISTRATION_SALT = getattr(settings, 'REGISTRATION_SALT', 'rest_framework_registration')
User = get_user_model()
...
class RegistrationView(drf_generics.CreateAPIView):
    ...
    def get_activation_key(self, user):
        return signing.dumps(getattr(user, User.USERNAME_FIELD), salt=REGISTRATION_SALT)

    def send_activation_email(self, user):
        ctx = {'activation_key': self.get_activation_key(user), 
...
class ActivationView(dj_generic.TemplateView):
    ...
    def validate_key(self, activation_key):
        try:
            username = signing.loads(activation_key, salt=REGISTRATION_SALT, max_age=ACCOUNT_ACTIVATION_DAYS * 86400)
