...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/ln/kb.py
...
class Number(object):
    ...
    def __str__(self):
        try:
            ...
            args = self.a1 is not None and self.a2 is not None and '(a1=%s,a2=%s)' % (self.a1, self.a2) or ''
    ...
    def __init__(cls, classname, bases, newdict):
        for base in bases:
            if base is not object:
                KB.tell('is(s=%s,c=%s)' % (classname.lower(), base.__name__.lower()))
...
class Name(object):
    ...
    def __repr__(self):
        return 'isa(e=%s,c=%s)' % (self.name, self.__class__.__name__.lower())
    ...
    def __init__(cls, classname, bases, newdict):
        if classname != 'Verb':
            ...
            if mods:
                mods = '(%s)' % mods
                cons = ';\n%s' % cons
            KB.tell('isa(e=prop(s=X1,v=%s%s,t=X2),c=fact)\n-->\nisa(e=X1,c=%s)%s' % (cls.__name__.lower(), mods, cls.subject.__name__.lower(), cons))
...
class Prop(object):
    ...
    def __repr__(self):
        return 'isa(e=prop(s=%s,v=%s,t=%s),c=fact)' % (str(self.s), str(self.v), str(self.t))
