...
class BaseConfigParser(object):
    ...
    def parse_drop_keep_rename(self, config, dumped_tree, data_store):
        ...
        if 'rename' in config.keys():
            if br_in in config['rename']:
                branches_to_keep.append((datatype, br_in))
...
class BaseMaker(metaclass=abc.ABCMeta):
 ...
 L. 443         0  LOAD_CONST               0
                2  LOAD_CONST               ('NestedYAMLLoader',)
                4  IMPORT_NAME_ATTR         pyBabyMaker.io.NestedYAMLLoader
                6  IMPORT_FROM              NestedYAMLLoader
                8  STORE_FAST               'NestedYAMLLoader'
               10  POP_TOP          

 L. 444        12  LOAD_GLOBAL              open
               14  LOAD_FAST                'yaml_filename'
               16  CALL_FUNCTION_1       1  ''
               18  SETUP_WITH           46  'to 46'
               20  STORE_FAST               'f'

 L. 445        22  LOAD_GLOBAL              yaml
               24  LOAD_METHOD              load
               26  LOAD_FAST                'f'
               28  LOAD_FAST                'NestedYAMLLoader'
               30  CALL_METHOD_2         2  ''
               32  POP_BLOCK        
               34  ROT_TWO          
               36  BEGIN_FINALLY    
