...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/google/apputils/humanize.py
...
DIGIT_SPLITTER = re.compile('\\d+|\\D+').findall
SPECIAL_PLURALS = {'index': 'indices', 
   'matrix': 'matrices', 
   'vertex': 'vertices'}
...
def DecimalPrefix(quantity, unit, precision=1, min_scale=0, max_scale=None):
  ...
  Returns:
    ...
    and the unit.
  """
    return _Prefix(quantity, unit, precision, DecimalScale, min_scale=min_scale, max_scale=max_scale)
...
def _Prefix(quantity, unit, precision, scale_callable, **args):
  ...
  """
    ...
    if scaled_unit:
        separator = ' '
    print_pattern = '%%.%df%%s%%s' % max(0, precision - int(math.log(abs(scaled_quantity), 10)) - 1)
    return print_pattern % (scaled_quantity, separator, scaled_unit)


DECIMAL_PREFIXES = ('y', 'z', 'a', 'f', 'p', 'n', 'µ', 'm', '', 'k', 'M', 'G', 'T',
                    'P', 'E', 'Z', 'Y')
DECIMAL_MIN_SCALE = -8
DECIMAL_MAX_SCALE = 8
...
def DecimalScale(quantity, unit, min_scale=0, max_scale=None):
  ...
  """
    ...
    powers = DECIMAL_PREFIXES[min_scale - DECIMAL_MIN_SCALE:max_scale - DECIMAL_MIN_SCALE + 1]
...
def BinaryScale(quantity, unit):
  ...
  Returns:
    ...
    units (string).
  """
    return _Scale(quantity, unit, 1024, ('Ki', 'Mi', 'Gi', 'Ti', 'Pi', 'Ei', 'Zi',
                                         'Yi'))
...
def _Scale(quantity, unit, multiplier, prefixes=None, min_scale=None):
  ...
  """
    if not prefixes or not quantity or math.isnan(quantity) or quantity in [float('inf'), float('-inf')]:
