...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/google/apputils/run_script_module.py
...
def RunScriptModule(module):
  ...
  """
    ...
    if not os.access(main_filename, os.R_OK):
        raise AssertionError('Cannot exec() %r: file not readable.' % main_filename)
        args = [
         main_filename] + args
        if debug_binary:
            debugger_args = debugger.split()
            program = debugger_args[0]
            program = os.path.isabs(program) or FindEnv(program)
        python_path = sys.executable
        command_vec = [python_path]
        if debug_script:
            command_vec.extend(GetPdbArgs(python_path))
        args = [
         program] + debugger_args[1:] + command_vec + args
    else:
        if debug_script:
            args = [
             sys.executable] + GetPdbArgs(program) + args
        else:
