...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/google/apputils/appcommands.py
...
class Cmd(object):
  ...
  """
    ...
    def __init__(self, name, flag_values, command_aliases=None, all_commands_help=None, help_full=None):
    ...
    """
        ...
        def ReplacementAppUsage(shorthelp=0, writeto_stdout=1, detailed_error=None, exitcode=None):
            AppcommandsUsage(shorthelp, writeto_stdout, detailed_error, exitcode=exitcode, show_cmd=self._command_name, show_global_flags=True)
    ...
    """
        if type(cmd_names) is list and len(cmd_names) > 1 and self._all_commands_help is not None:
            ...
            if self._help_full is not None:
...
def AddCmdFunc(command_name, cmd_func, command_aliases=None, all_commands_help=None):
  ...
  """
    _AddCmdInstance(command_name, _FunctionalCmd(command_name, flags.FlagValues(), cmd_func, command_aliases=command_aliases, all_commands_help=all_commands_help), command_aliases=command_aliases)
