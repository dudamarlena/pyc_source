...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/google/apputils/basetest.py
...
import collections, difflib, getpass, itertools, json, os, re, signal, subprocess, sys, tempfile, types, unittest, urlparse
try:
    import faulthandler
except ImportError:
    faulthandler = None
...
flags.DEFINE_integer('test_random_seed', _GetDefaultTestRandomSeed(), 'Random seed for testing. Some test frameworks may change the default value of this flag between runs, so it is not appropriate for seeding probabilistic tests.', allow_override=1)
flags.DEFINE_string('test_srcdir', os.environ.get('TEST_SRCDIR', ''), 'Root of directory tree where source files live', allow_override=1)
flags.DEFINE_string('test_tmpdir', _GetDefaultTestTmpdir(), 'Directory for temporary testing files', allow_override=1)
...
class TestCase(unittest.TestCase):
    ...
    """
        ...
        try:
            ...
            self.fail('For whole: len(%s) is not supported, it appears to be type: %s' % (
             whole, type(whole)))

        assert prefix_len <= whole_len, 'Prefix length (%d) is longer than whole length (%d).' % (
    ...
    Args:
      ...
      msg: The message to be printed if the test fails.
    """
        if isinstance(expected_seq, basestring) or isinstance(actual_seq, basestring):
            self.fail('Passing a string to assertSameElements is usually a bug. Did you mean to use assertEqual?\nExpected: %s\nActual: %s' % (
    ...
    """
        ...
        if not re.search(regex, actual_str, re.MULTILINE):
            self.fail(message or '"%s" does not contain any of these regexes: %s.' % (
             actual_str, regexes))
    ...
    """
        ...
        self.assertEqual(ret_code, 0, 'Running command\n%s failed with error code %s and message\n%s' % (
    ...
    """
        ...
        if isinstance(regexes[0], unicode):
            regexes = [ regex.encode('utf-8') for regex in regexes ]
        command_string = GetCommandString(command)
        self.assertNotEqual(ret_code, 0, 'The following command succeeded while expected to fail:\n%s' % _QuoteLongString(command_string))
        self.assertRegexMatch(err, regexes, message='Running command\n%s failed with error code %s and message\n%s which matches no regex in %s' % (
         _QuoteLongString(command_string),
         ret_code,
         _QuoteLongString(err),
    ...
    def assertRaisesWithPredicateMatch(self, expected_exception, predicate, callable_obj=None, *args, **kwargs):
    ...
    """
        ...
        def Check(err):
            self.assert_(predicate(err), '%r does not match predicate %r' % (err, predicate))
    ...
    def assertRaisesWithRegexpMatch(self, expected_exception, expected_regexp, callable_obj=None, *args, **kwargs):
    ...
    """
        ...
        def CheckOrder(small, big):
            ...
            self.assertFalse(big <= small, '%r unexpectedly less than or equal to %r' % (
             big, small))
            self.assertGreater(big, small)
            self.assertFalse(small > big, '%r unexpectedly greater than %r' % (small, big))
            self.assertGreaterEqual(big, small)
            self.assertFalse(small >= big, '%r unexpectedly greater than or equal to %r' % (
             small, big))
        ...
        def CheckEqual(a, b):
            ...
            self.assertEqual(hash(a), hash(b), 'hash %d of %r unexpectedly not equal to hash %d of %r' % (
             hash(a), a, hash(b), b))
    ...
    """
        ...
        def Repr(dikt):
            ...
            entries = sorted((safe_repr(k), safe_repr(v)) for k, v in dikt.iteritems())
            return '{%s}' % (', ').join('%s: %s' % pair for pair in entries)

        message = [
         '%s != %s%s' % (Repr(a), Repr(b), ' (%s)' % msg if msg else '')]
        ...
        if unexpected:
            message.append('Unexpected, but present entries:\n%s' % ('').join('%s: %s\n' % (safe_repr(k), safe_repr(v)) for k, v in unexpected))
        if different:
            message.append('repr() of differing entries:\n%s' % ('').join('%s: %s != %s\n' % (safe_repr(k), safe_repr(a_value), safe_repr(b_value)) for k, a_value, b_value in different))
        if missing:
            message.append('Missing entries:\n%s' % ('').join('%s: %s\n' % (safe_repr(k), safe_repr(v)) for k, v in missing))
    ...
    def assertUrlEqual(self, a, b):
        ...
        self.assertEqual(sorted(parsed_a.params.split(';')), sorted(parsed_b.params.split(';')))
        self.assertDictEqual(urlparse.parse_qs(parsed_a.query, keep_blank_values=True), urlparse.parse_qs(parsed_b.query, keep_blank_values=True))
...
def CaptureTestStdout(outfile='', expected_output_filepath=None):
  ...
  """
    ...
    if expected_output_filepath is not None:
        return _DiffingTestOutputContext(lambda : DiffTestStdout(expected_output_filepath))
    else:
        return
...
def CaptureTestStderr(outfile='', expected_output_filepath=None):
  ...
  """
    ...
    if expected_output_filepath is not None:
        return _DiffingTestOutputContext(lambda : DiffTestStderr(expected_output_filepath))
    else:
        return
...
def _WalkStructureForProblems(a, b, aname, bname, problem_list):
    ...
    if type(a) != type(b) and not (isinstance(a, _INT_TYPES) and isinstance(b, _INT_TYPES)):
    ...
    if isinstance(a, collections.Mapping):
        for k in a:
            if k in b:
                _WalkStructureForProblems(a[k], b[k], '%s[%r]' % (aname, k), '%s[%r]' % (bname, k), problem_list)
            else:
        ...
        for i in xrange(minlen):
            _WalkStructureForProblems(a[i], b[i], '%s[%d]' % (aname, i), '%s[%d]' % (bname, i), problem_list)
    ...
    try:
        with open(lhs, 'r') as (lhs_f):
            with open(rhs, 'r') as (rhs_f):
                diff_text = ('').join(difflib.unified_diff(lhs_f.readlines(), rhs_f.readlines()))
        if not diff_text:
            return True
        raise OutputDifferedError('\nComparing %s and %s\nTest output differed from golden file:\n%s' % (
         lhs, rhs, diff_text))
...
def DiffTestStrings(data1, data2):
    """Diff two strings."""
    diff_text = ('').join(difflib.unified_diff(data1.splitlines(True), data2.splitlines(True)))
    if not diff_text:
        return
