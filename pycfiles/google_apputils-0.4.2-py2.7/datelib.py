...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/google/apputils/datelib.py
...
"""
import calendar, copy, datetime, re, sys, time, types, warnings, dateutil.parser, pytz
...
def UTCMicrosToDatetime(micros, tz=None):
  ...
  """
    dt = datetime.datetime.utcfromtimestamp(micros / _MICROSECONDS_PER_SECOND_F)
    if tz is not None:
        dt = tz.fromutc(dt)
    return dt
...
MAXIMUM_PYTHON_TIMESTAMP = Timestamp(9999, 12, 31, 23, 59, 59, 999999, UTC)
MAXIMUM_MICROSECOND_TIMESTAMP = 2147483648 * _MICROSECONDS_PER_SECOND - 1
