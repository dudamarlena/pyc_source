...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/google/apputils/stopwatch.py
  ...
  """
    ...
    """
        if timer not in self.timers:
            raise RuntimeError('Tried to stop timer that was never started: %s' % timer)
    ...
    """
        ...
        if verbose:
            names = all_names
        results = [ (name, self.timervalue(name, now=now), self.counters[name]) for name in names
                  ]
        if verbose:
