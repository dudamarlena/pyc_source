...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/seeta_dataset/record_type.py
...
class RecordType:
    ...
    def dump(self):
        ...
        if self._basicType == BasicType.Map:
            ...
            if self._basicType == BasicType.List:
    ...
    def encode(self, data, buffer):
        if self._basicType == BasicType.Map:
            ...
            if self._basicType == BasicType.List:
                ...
                if self._basicType == BasicType.String:
                    assert isinstance(data, str), 'but get: {}'.format(data)
                    b = data.encode()
                    buffer.extend(b)
                    return len(b)
                else:
                    if self._basicType == BasicType.Int:
                        assert isinstance(data, int), 'but get: {}'.format(data)
                        b = struct.pack('<Q', data)
                        buffer.extend(b)
                        return len(b)
                    if self._basicType == BasicType.Float:
                        assert isinstance(data, float), 'but get: {}'.format(data)
                        b = struct.pack('<d', data)
                        buffer.extend(b)
                        return len(b)
                    assert self._basicType == BasicType.ByteArray and isinstance(data, bytes) or isinstance(data, bytearray), 'but get: {}'.format(data)
                buffer.extend(data)
                return len(data)
    ...
    def decode(self, data):
        if self._basicType == BasicType.Map:
            ...
            if self._basicType == BasicType.ByteArray:
                return data.tobytes()
        raise Exception('Should not run here')
