...
def test_special_headers(simple_app):
    event = make_event(headers={'Content-Type':[
      'text/plain'], 
     'Host':[
      'example.com'], 
     'X-Forwarded-For':[
...
def test_full_event(simple_app):
    ...
    if not @py_assert2:
        @py_format4 = @pytest_ar._call_reprcompare(('==', ), (@py_assert2,), ('%(py1)s == %(py3)s', ), (@py_assert0, event)) % {'py1':@pytest_ar._saferepr(@py_assert0),  'py3':@pytest_ar._saferepr(event) if 'event' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(event) else 'event'}
        @py_format6 = 'assert %(py5)s' % {'py5': @py_format4}
        raise AssertionError(@pytest_ar._format_explanation(@py_format6))
    @py_assert0 = @py_assert2 = None