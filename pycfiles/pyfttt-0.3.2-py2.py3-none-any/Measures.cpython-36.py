...
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyFTS/benchmarks/Measures.py
# Compiled at: 2019-01-28 12:38:06
# Size of source mod 2**32: 13876 bytes
...
def winkler_score(tau, target, forecast):
    ...
    if forecast[0] < target:
        if target < forecast[1]:
            return delta
    if forecast[0] > target:
        return delta + 2 * (forecast[0] - target) / tau
    if forecast[1] < target:
