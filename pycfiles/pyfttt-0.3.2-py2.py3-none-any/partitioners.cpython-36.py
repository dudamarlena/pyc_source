...
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyFTS/models/nonstationary/partitioners.py
# Compiled at: 2018-08-06 09:10:52
...
class PolynomialNonStationaryPartitioner(partitioner.Partitioner):
    ...
    def __init__(self, data, part, **kwargs):
        super(PolynomialNonStationaryPartitioner, self).__init__(name=(part.name), data=data, npart=(part.partitions), func=(part.membership_function),
          names=(part.setnames),
          prefix=(part.prefix),
        ...
        for ct, key in enumerate(self.ordered_sets):
            ...
            tmp = (common.FuzzySet)(set.name, set.mf, set.parameters, location=perturbation.polynomial, 
...
class SimpleNonStationaryPartitioner(partitioner.Partitioner):
    ...
    def __init__(self, data, part, **kwargs):
        super(SimpleNonStationaryPartitioner, self).__init__(name=(part.name), data=data, npart=(part.partitions), func=(part.membership_function),
          names=(part.setnames),
          prefix=(part.prefix),
          transformation=(part.transformation),
          indexer=(part.indexer))
...
def simplenonstationary_gridpartitioner_builder(data, npart, transformation):
    ...
    fs = partitioners.SimpleNonStationaryPartitioner(data, tmp_fs, location=(perturbation.polynomial),
      location_params=[
     1, 0],
      location_roots=0,
      width=(perturbation.polynomial),
