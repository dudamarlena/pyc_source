...
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyFTS/data/artificial.py
# Compiled at: 2019-02-21 12:14:23
# Size of source mod 2**32: 12486 bytes
...
class SignalEmulator(object):
    ...
    def stationary_gaussian(self, mu, sigma, **kwargs):
        ...
        parameters = {'mu':mu, 
         'sigma':sigma}
        self.components.append({'dist':'gaussian',  'type':'constant',  'parameters':parameters, 
         'args':kwargs})
        return self
    ...
    def incremental_gaussian(self, mu, sigma, **kwargs):
        ...
        parameters = {'mu':mu, 
         'sigma':sigma}
        self.components.append({'dist':'gaussian',  'type':'incremental',  'parameters':parameters, 
         'args':kwargs})
        return self
    ...
    def periodic_gaussian(self, type, period, mu_min, sigma_min, mu_max, sigma_max, **kwargs):
        ...
        parameters = {'type':type, 
         'period':period,  'mu_min':mu_min, 
         'sigma_min':sigma_min,  'mu_max':mu_max,  'sigma_max':sigma_max}
        self.components.append({'dist':'gaussian',  'type':'periodic',  'parameters':parameters, 
         'args':kwargs})
        return self
    ...
    def blip(self, **kwargs):
        ...
        """
        parameters = {}
        self.components.append({'dist':'blip',  'type':'blip',  'parameters':parameters, 
         'args':kwargs})
        return self
    ...
    def run(self):
        ...
        for ct, component in enumerate(self.components):
            ...
            if component['type'] == 'constant':
                tmp = generate_gaussian_linear((parameters['mu']), (parameters['sigma']), 0, 0, it=it,
                  num=num,
                  vmin=vmin,
                  vmax=vmax)
            else:
                if component['type'] == 'incremental':
                    tmp = generate_gaussian_linear(0, 0, (parameters['mu']), (parameters['sigma']), it=num,
                      num=1,
                      vmin=vmin,
                      vmax=vmax)
                else:
                    if component['type'] == 'periodic':
                        ...
                        if parameters['type'] == 'sinoidal':
                            tmp = generate_sinoidal_periodic_gaussian(period, mu_min, sigma_min, mu_max, sigma_max, it=num,
                              num=1,
                              vmin=vmin,
                              vmax=vmax)
                        else:
                            tmp = generate_linear_periodic_gaussian(period, mu_min, sigma_min, mu_max, sigma_max, it=num,
                              num=1,
                              vmin=vmin,
                              vmax=vmax)
                    else:
                        if component['type'] == 'blip':
                            ...
                            tmp = [_mx] if np.random.rand() >= 0.5 else [-_mn]
            last_num = num
            last_it = it
