...
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyFTS/hyperparam/GridSearch.py
# Compiled at: 2019-03-15 11:47:54
...
def cluster_method(individual, dataset, **kwargs):
    ...
    return {'parameters':individual, 
     'rmse':np.nanmean(errors),  'size':np.nanmean(size)}
    ...
    for ct, job in enumerate(jobs):
        ...
        if job.status == dispy.DispyJob.Finished and result is not None:
            ...
            for metric in metrics.keys():
                ...
                record = (
                 datasetname, 'GridSearch', 'WHOFTS', None, param['mf'],
            ...
            if order > 1:
                for lag2 in lags:
                    ...
                    if order > 2:
                        ...
                        individuals.append(dict_individual(mf, partitioner, partitions, order, _lags2, alpha_cut))
