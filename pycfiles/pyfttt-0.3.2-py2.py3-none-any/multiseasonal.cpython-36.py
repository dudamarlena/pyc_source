...
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyFTS/models/ensemble/multiseasonal.py
# Compiled at: 2018-11-01 09:21:45
...
class SeasonalEnsembleFTS(ensemble.EnsembleFTS):
    ...
    def train(self, data, **kwargs):
        ...
        results = Parallel(n_jobs=num_cores)(delayed(train_individual_model)(deepcopy(pool[m]['pt']), data, deepcopy(pool[m]['ix'])) for m in pool.keys())
        for tmp in results:
    ...
    def forecast_distribution(self, data, **kwargs):
        ...
        for k in data.index:
            ...
            dist = (ProbabilityDistribution.ProbabilityDistribution)(smooth, uod=uod, data=tmp, name=name, **kwargs)
            ret.append(dist)
