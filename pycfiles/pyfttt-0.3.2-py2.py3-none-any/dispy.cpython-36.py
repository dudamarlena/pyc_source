...
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyFTS/distributed/dispy.py
# Compiled at: 2019-03-15 11:43:03
...
def distributed_train(model, train_method, nodes, fts_method, data, num_batches=10, train_parameters={}, **kwargs):
    import dispy, dispy.httpd, datetime
    batch_save = kwargs.get('batch_save', False)
    ...
    for job in jobs:
        ...
        if job.status == dispy.DispyJob.Finished and tmp is not None:
            model.merge(tmp)
            if batch_save:
                if job.id % batch_save_interval == 0:
