...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/rbtools/api/transport/sync.py
...
class SyncTransport(Transport):
    ...
    def __init__(self, url, cookie_file=None, username=None, password=None, api_token=None, agent=None, session=None, disable_proxy=False, auth_callback=None, otp_token_callback=None, verify_ssl=True, allow_caching=True, cache_location=None, in_memory_cache=False, save_cookies=True, ext_auth_cookies=None, *args, **kwargs):
    ...
    def get_path(self, path, *args, **kwargs):
        ...
        if path.startswith(b'/'):
            path = path[1:]
        return self._execute_request(HttpRequest(self.server.url + path, query_args=kwargs))
    ...
    def _execute_request(self, request):
        ...
        logging.debug(b'Making HTTP %s request to %s', request.method, request.url)
        rsp = self.server.make_request(request)
        info = rsp.info()
        mime_type = info[b'Content-Type']
        item_content_type = info.get(b'Item-Content-Type', None)
        if request.method == b'DELETE':
            return
        else:
            payload = rsp.read()
            payload = decode_response(payload, mime_type)
            return create_resource(self, payload, request.url, mime_type=mime_type, item_mime_type=item_content_type)
            return
    ...
    def enable_cache(self):
        ...
        if self.allow_caching:
            self.server.enable_cache(cache_location=self.cache_location, in_memory=self.in_memory_cache)

    def __repr__(self):
        return b'<%s(url=%r, cookie_file=%r, agent=%r)>' % (
