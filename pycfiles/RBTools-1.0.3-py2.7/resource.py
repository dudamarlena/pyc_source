...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/rbtools/api/resource.py
...
RESOURCE_MAP = {}
LINKS_TOK = b'links'
LINK_KEYS = set([b'href', b'method', b'title'])
_EXCLUDE_ATTRS = [LINKS_TOK, b'stat']
_EXTRA_DATA_PREFIX = b'extra_data__'
...
def _preprocess_fields(fields):
    ...
    for name, value in six.iteritems(fields):
        if name.startswith(_EXTRA_DATA_PREFIX):
            name = b'extra_data.%s' % name[len(_EXTRA_DATA_PREFIX):]
        yield (name, value)
...
def _create(resource, data=None, query_args={}, *args, **kwargs):
    ...
    """
    request = HttpRequest(resource._links[b'create'][b'href'], method=b'POST', query_args=query_args)
    if data is None:
        data = {}
    kwargs.update(data)
...
def _delete(resource, *args, **kwargs):
    ...
    return HttpRequest(resource._links[b'delete'][b'href'], method=b'DELETE', query_args=kwargs)


@request_method_decorator
def _get_self(resource, *args, **kwargs):
    """Generate a request for a resource's 'self' link."""
    return HttpRequest(resource._links[b'self'][b'href'], query_args=kwargs)
...
def _update(resource, data=None, query_args={}, *args, **kwargs):
    ...
    ``extra_data.field_name``.
    """
    request = HttpRequest(resource._links[b'update'][b'href'], method=b'PUT', query_args=query_args)
    if data is None:
        data = {}
    kwargs.update(data)
    for name, value in _preprocess_fields(kwargs):
...
class Resource(object):
    ...
    def __init__(self, transport, payload, url, token=None, **kwargs):
        ...
        if LINKS_TOK in self._payload:
            ...
            if token and isinstance(self._payload[token], dict) and LINKS_TOK in self._payload[token]:
            ...
            for link, method in six.iteritems(SPECIAL_LINKS):
                if link in self._links and method[1]:
                    setattr(self, method[0], lambda resource=self, meth=method[1], **kwargs: meth(resource, **kwargs))

            for link, body in six.iteritems(self._links):
                if link not in SPECIAL_LINKS:
                    setattr(self, b'get_%s' % link, lambda resource=self, url=body[b'href'], **kwargs: self._get_url(url, **kwargs))

    def _wrap_field(self, field):
        if isinstance(field, dict):
            dict_keys = set(field.keys())
            if b'href' in dict_keys and len(dict_keys.difference(LINK_KEYS)) == 0:
...
class ResourceDictField(object):
    ...
    def __getattr__(self, name):
        ...
        raise AttributeError(b'This dictionary resource for %s does not have an attribute "%s".' % (
    ...
    def iteritems(self):
        for key, value in six.iteritems(self._fields):
            yield (
             key, self._resource._wrap_field(value))

    def __repr__(self):
        return b'%s(resource=%r, fields=%r)' % (
...
class ResourceLinkField(ResourceDictField):
    ...
    def get(self, **query_args):
        return HttpRequest(self._fields[b'href'], query_args=query_args)
...
class ResourceListField(list):
    ...
    def __repr__(self):
        return b'%s(resource=%r, list_field=%s)' % (
         self.__class__.__name__,
         self._resource,
...
class ItemResource(Resource):
    ...
    def __init__(self, transport, payload, url, token=None, **kwargs):
        super(ItemResource, self).__init__(transport, payload, url, token=token, **kwargs)
        self._fields = {}
    ...
    def __getattr__(self, name):
        ...
        raise AttributeError(b'This %s does not have an attribute "%s".' % (
    ...
    def iteritems(self):
        for key, value in six.iteritems(self._fields):
            yield (
             key, self._wrap_field(value))

    def __repr__(self):
        return b'%s(transport=%r, payload=%r, url=%r, token=%r)' % (
         self.__class__.__name__,
         self._transport,
         self._payload,
         self._url,
...
class CountResource(ItemResource):
    ...
    def __init__(self, transport, payload, url, **kwargs):
        super(CountResource, self).__init__(transport, payload, url, token=None)
        return
    ...
    def get_self(self, **kwargs):
        ...
        """
        kwargs.update({b'counts_only': False})
...
class ListResource(Resource):
    ...
    def __init__(self, transport, payload, url, token=None, item_mime_type=None, **kwargs):
        super(ListResource, self).__init__(transport, payload, url, token=token, **kwargs)
        ...
        self.total_results = payload[b'total_results']
    ...
    def __getitem__(self, key):
        ...
        try:
            url = payload[b'links'][b'self'][b'href']
        except KeyError:
            url = b''
    ...
    def get_next(self, **kwargs):
        if b'next' not in self._links:
            raise StopIteration()
        return HttpRequest(self._links[b'next'][b'href'], query_args=kwargs)

    @request_method_decorator
    def get_prev(self, **kwargs):
        if b'prev' not in self._links:
            raise StopIteration()
        return HttpRequest(self._links[b'prev'][b'href'], query_args=kwargs)

    @request_method_decorator
    def get_item(self, pk, **kwargs):
        """Retrieve the item resource with the corresponding primary key."""
        return HttpRequest(urljoin(self._url, b'%s/' % pk), query_args=kwargs)
    ...
    def __repr__(self):
        return b'%s(transport=%r, payload=%r, url=%r, token=%r, item_mime_type=%r)' % (
         self.__class__.__name__,
         self._transport,
         self._payload,
         self._url,
...
@resource_mimetype(b'application/vnd.reviewboard.org.root')
class RootResource(ItemResource):
    ...
    _excluded_attrs = [
     b'uri_templates']
    _TEMPLATE_PARAM_RE = re.compile(b'\\{(?P<key>[A-Za-z_0-9]*)\\}')

    def __init__(self, transport, payload, url, **kwargs):
        super(RootResource, self).__init__(transport, payload, url, token=None)
        for name, url in six.iteritems(payload[b'uri_templates']):
            attr_name = b'get_%s' % name
            if not hasattr(self, attr_name):
                setattr(self, attr_name, lambda resource=self, url=url, **kwargs: self._get_template_request(url, **kwargs))

        server_version = payload.get(b'product', {}).get(b'package_version')
        if server_version is not None and parse_version(server_version) >= parse_version(MINIMUM_VERSION):
    ...
    def _get_template_request(self, url_template, values={}, **kwargs):
        ...
        def get_template_value(m):
            try:
                return str(kwargs.pop(m.group(b'key'), None) or values[m.group(b'key')])
            except KeyError:
                raise ValueError(b'Template was not provided a value for "%s"' % m.group(b'key'))
...
class DiffUploaderMixin(object):
    ...
    def prepare_upload_diff_request(self, diff, parent_diff=None, base_dir=None, base_commit_id=None, **kwargs):
        ...
        request.add_file(b'path', b'diff', diff)
        if parent_diff:
            request.add_file(b'parent_diff_path', b'parent_diff', parent_diff)
        if base_dir:
            request.add_field(b'basedir', base_dir)
        if base_commit_id:
            request.add_field(b'base_commit_id', base_commit_id)
        return request


@resource_mimetype(b'application/vnd.reviewboard.org.diffs')
class DiffListResource(DiffUploaderMixin, ListResource):
    ...
    def upload_diff(self, diff, parent_diff=None, base_dir=None, base_commit_id=None, **kwargs):
        ...
        diff output.
        """
        return self.prepare_upload_diff_request(diff, parent_diff=parent_diff, base_dir=base_dir, base_commit_id=base_commit_id, **kwargs)


@resource_mimetype(b'application/vnd.reviewboard.org.diff')
class DiffResource(ItemResource):
    ...
    def get_patch(self, **kwargs):
        ...
        return HttpRequest(self._url, query_args=kwargs, headers={b'Accept': b'text/x-patch'})


@resource_mimetype(b'application/vnd.reviewboard.org.file')
class FileDiffResource(ItemResource):
    ...
    def get_diff_data(self, **kwargs):
        ...
        return HttpRequest(self._url, query_args=kwargs, headers={b'Accept': b'application/vnd.reviewboard.org.diff.data+json'})


@resource_mimetype(b'application/vnd.reviewboard.org.file-attachments')
@resource_mimetype(b'application/vnd.reviewboard.org.user-file-attachments')
class FileAttachmentListResource(ListResource):
    """The File Attachment List resource specific base class."""

    @request_method_decorator
    def upload_attachment(self, filename, content, caption=None, attachment_history=None, **kwargs):
        ...
        """
        request = HttpRequest(self._url, method=b'POST', query_args=kwargs)
        request.add_file(b'path', filename, content)
        if caption:
            request.add_field(b'caption', caption)
        if attachment_history:
            request.add_field(b'attachment_history', attachment_history)
        return request


@resource_mimetype(b'application/vnd.reviewboard.org.draft-file-attachments')
...
@resource_mimetype(b'application/vnd.reviewboard.org.screenshots')
...
@resource_mimetype(b'application/vnd.reviewboard.org.draft-screenshots')
class DraftScreenshotListResource(ScreenshotListResource):
    """The Draft Screenshot List resource specific base class."""
    pass


@resource_mimetype(b'application/vnd.reviewboard.org.review-request')
class ReviewRequestResource(ItemResource):
    ...
    def absolute_url(self):
        ...
        """
        if b'absolute_url' in self._fields:
            return self._fields[b'absolute_url']
        else:
            base_url = self._url.split(b'/api/')[0]
    ...
    def url(self):
        ...
        """
        return self._fields.get(b'url', b'/r/%s/' % self.id)

    @request_method_decorator
    def submit(self, description=None, changenum=None):
        """Submit a review request"""
        data = {b'status': b'submitted'}
        if description:
            data[b'description'] = description
        if changenum:
            data[b'changenum'] = changenum
    ...
    def build_dependency_graph(self):
        ...
        def get_review_request_resource(resource):
            ...
            if url not in review_requests_by_url:
                review_requests_by_url[url] = resource.get(expand=b'repository')
        ...
        while unvisited:
            ...
            for tail in head.depends_on:
                ...
                if path_exists(graph, tail.id, head.id):
                    raise ValueError(b'Circular dependencies.')
                if repository.id == tail.repository.id and tail.status != b'submitted':
                    graph[head].add(tail)
...
@resource_mimetype(b'application/vnd.reviewboard.org.diff-validation')
class ValidateDiffResource(DiffUploaderMixin, ItemResource):
    ...
    @request_method_decorator
    def validate_diff(self, repository, diff, parent_diff=None, base_dir=None, base_commit_id=None, **kwargs):
        """Validate a diff."""
        request = self.prepare_upload_diff_request(diff, parent_diff=parent_diff, base_dir=base_dir, base_commit_id=base_commit_id, **kwargs)
        request.add_field(b'repository', repository)
        if base_commit_id:
            request.add_field(b'base_commit_id', base_commit_id)
        return request