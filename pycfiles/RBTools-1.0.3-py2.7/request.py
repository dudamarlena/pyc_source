...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/rbtools/api/request.py
...
class HttpRequest(object):
    ...
    def __init__(self, url, method=b'GET', query_args={}, headers={}):
        self.method = method
        self._fields = {}
        self._files = {}
        query_args = dict([ (key.replace(b'_', b'-'), value) for key, value in six.iteritems(query_args)
                          ])
        self.headers = {str(key):str(value) for key, value in six.iteritems(headers)}
    ...
    def encode_multipart_formdata(self):
        ...
        if not (self._fields or self._files):
            ...
            content_type = b'multipart/form-data; boundary=%s' % BOUNDARY.decode(b'utf-8')
            return (
...
class Request(URLRequest):
    ...
    def __init__(self, url, body=b'', headers={}, method=b'PUT'):
        normalized_headers = {str(key):str(value) for key, value in six.iteritems(headers)}
    ...
    def http_request(self, request):
        if not self.used:
            if self.password_mgr.api_token:
                ...
                header = b'Basic %s' % base64.b64encode(raw.encode(b'utf-8')).strip()
                request.add_header(self.AUTH_HEADER, header.decode(b'utf-8'))
                self.used = True
        return request
...
class ReviewBoardHTTPBasicAuthHandler(HTTPBasicAuthHandler):
    ...
    def http_error_auth_reqed(self, authreq, host, req, headers):
        ...
        if otp_header and otp_header.startswith(b'required'):
            try:
                self._otp_token_method = otp_header.split(b';')[1].strip()
            except IndexError:
                logging.error(b'Invalid %s header value: "%s". This header is needed for two-factor authentication to work. Please report this!', self.OTP_TOKEN_HEADER, otp_header)
                return

        return HTTPBasicAuthHandler.http_error_auth_reqed(self, authreq, host, req, headers)
    ...
    def retry_http_basic_auth(self, host, request, realm):
        ...
        Raises:
            urllib2.URLError:
                ...
                :http:`401`, it may be handled by this class again.
        """
        if self._otp_token_attempts > self.MAX_OTP_TOKEN_ATTEMPTS or not self._otp_token_method and self._tried_login:
            return
        ...
        if password is None:
            ...
            if self._otp_token_method:
                otp_token = self.passwd.get_otp_token(request.get_full_url(), self._otp_token_method).encode(b'utf-8')
            else:
                otp_token = None
            raw = b'%s:%s' % (user, password)
            auth = b'Basic %s' % base64.b64encode(raw.encode(b'utf-8')).strip()
            if request.get_header(self.auth_header) == auth and (not otp_token or otp_token == self._last_otp_token):
                return
            request.add_unredirected_header(self.auth_header, auth.decode(b'utf-8'))
            if otp_token:
    ...
    if not cookie_file:
        ...
        if not os.path.isfile(cookie_file) and os.path.isfile(post_review_cookies):
            try:
                ...
                logging.warning(b'There was an error while copying legacy post-review cookies: %s', e)
...
class ReviewBoardServer(object):
    ...
    def __init__(self, url, cookie_file=None, username=None, password=None, api_token=None, agent=None, session=None, disable_proxy=False, auth_callback=None, otp_token_callback=None, verify_ssl=True, save_cookies=True, ext_auth_cookies=None):
        ...
        opener.addheaders = [
         (
          str(b'User-agent'), str(self.agent))]
    ...
    def process_error(self, http_status, data):
        ...
        try:
            ...
            logging.debug(b'Got API Error %d (HTTP code %d): %s', rsp[b'err'][b'code'], http_status, rsp[b'err'][b'msg'])
    ...
    def make_request(self, request):
        ...
        try:
            ...
            rsp = self._urlopen(Request(request.url, body, headers, request.method))
