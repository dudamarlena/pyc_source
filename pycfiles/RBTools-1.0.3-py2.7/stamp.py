...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/rbtools/commands/stamp.py
...
from rbtools.utils.review_request import find_review_request_by_change_id, get_draft_or_current_value, get_revisions, guess_existing_review_request
...
class Stamp(Command):
    ...
    description = b'Adds the review request URL to the commit message.'
    args = b'[revisions]'
    option_list = [
     OptionGroup(name=b'Stamp Options', description=b'Controls the behavior of a stamp, including what review request URL gets stamped.', option_list=[
      Option(b'-r', b'--review-request-id', dest=b'rid', metavar=b'ID', default=None, help=b'Specifies the existing review request ID to be stamped.')]),
    ...
    def no_commit_error(self):
        raise CommandError(b'No existing commit to stamp on.')

    def _ask_review_request_match(self, review_request):
        question = b'Stamp with Review Request #%s: "%s"? ' % (
         review_request.id,
         get_draft_or_current_value(b'summary', review_request))
        return confirm(question)

    def determine_review_request(self, api_client, api_root, repository_info, repository_name, revisions):
        ...
        if repository_info.supports_changesets:
            review_request = find_review_request_by_change_id(api_client, api_root, repository_info, repository_name, revisions)
            if review_request and review_request.id:
                return (review_request.id, review_request.absolute_url)
        logging.debug(b'Attempting to guess review request based on summary and description')
        try:
            review_request = guess_existing_review_request(repository_info, repository_name, api_root, api_client, self.tool, revisions, guess_summary=False, guess_description=False, is_fuzzy_match_func=self._ask_review_request_match, no_commit_error=self.no_commit_error)
        except ValueError as e:
            raise CommandError(six.text_type(e))

        if review_request:
            logging.debug(b'Found review request ID %d', review_request.id)
            return (
             review_request.id, review_request.absolute_url)
        else:
            logging.debug(b'Could not find a matching review request')
            return (None, None)
            return
    ...
    def main(self, *args):
        ...
        if not self.tool.can_amend_commit:
            raise NotImplementedError(b'rbt stamp is not supported with %s.' % self.tool.name)
        try:
        ...
        if self.options.rid:
            ...
            review_request_id, review_request_url = self.determine_review_request(api_client, api_root, repository_info, self.options.repository_name, revisions)
        if not review_request_url:
            raise CommandError(b'Could not determine the existing review request URL to stamp with.')
        stamp_commit_with_review_url(revisions, review_request_url, self.tool)
        print(b'Successfully stamped change with the URL:')
