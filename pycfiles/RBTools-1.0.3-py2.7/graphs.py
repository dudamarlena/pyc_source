...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/rbtools/utils/graphs.py
...
def toposort(graph):
    ...
    heads = set(vertex for vertex, indegree in six.iteritems(indegrees) if indegree == 0)
    while len(heads):
        head = heads.pop()
        result.append(head)
        if head in graph:
    ...
    if any(six.itervalues(indegrees)):
        raise ValueError(b'Graph contains cycles.')
    return result