...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/rbtools/hooks/common.py
...
def get_api(server_url, **kwargs):
    ...
    try:
        ...
        raise HookError(b'Could not reach the Review Board server at %s: %s' % (
         server_url, e))
    except APIError as e:
        raise HookError(b'Unexpected API Error: %s' % e)
...
def execute(command):
    ...
    if process.returncode:
        logging.warning(b'Failed to execute command: %s', command)
        return None
    else:
        return output
...
def initialize_logging():
    ...
    handler.setFormatter(logging.Formatter(b'%(levelname)s: %(message)s'))
...
def get_review_request_id(regex, commit_message):
    ...
    return match and int(match.group(b'id')) or 0
...
def get_review_request(review_request_id, api_root):
    ...
    try:
        ...
        raise HookError(b'Error getting review request: %s' % e)
...
def close_review_request(server_url, username, password, review_request_id, description):
    ...
    if review_request.status == SUBMITTED:
        logging.warning(b'Review request #%s is already %s.', review_request_id, SUBMITTED)
        return
    if description:
...
def get_review_request_approval(server_url, username, password, review_request_id):
    ...
    review_request = get_review_request(review_request_id, api_root)
    return (
     review_request.approved, review_request.approval_failure)