...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/rbtools/commands/land.py
...
from rbtools.utils.commands import build_rbtools_cmd_argv, extract_commit_message
...
class Land(Command):
    ...
    option_list = [
     ...
     Option(b'--recursive', dest=b'recursive', action=b'store_true', default=False, help=b'Recursively fetch patches for review requests that the specified review request depends on. This is equivalent to calling "rbt patch" for each of those review requests.', added_in=b'1.0'),
    ...
    def patch(self, review_request_id):
        ...
        rc, output = execute(patch_command, ignore_errors=True, return_error_code=True)
        if rc:
            raise CommandError(b'Failed to execute "rbt patch":\n%s' % output)
    ...
    def can_land(self, review_request):
        ...
        try:
            try:
                ...
                return b'An error was encountered while executing the land command.'
    ...
    def land(self, destination_branch, review_request, source_branch=None, squash=False, edit=False, delete_branch=True, dry_run=False):
        ...
        if source_branch:
            ...
            if not dry_run:
                try:
                    self.tool.merge(target=source_branch, destination=destination_branch, message=review_commit_message, author=author, squash=squash, run_editor=edit, close_branch=delete_branch)
                except MergeError as e:
                    raise CommandError(six.text_type(e))

        else:
            print(b'Applying patch from review request %s.' % review_request.id)
            if not dry_run:
    ...
    def main(self, branch_name=None, *args):
        ...
        if self.options.push and not self.tool.can_push_upstream:
            raise CommandError(b'--push is not supported for %s repositories.' % self.tool.name)
        ...
        if self.options.rid:
            ...
            try:
                review_request = guess_existing_review_request(repository_info=repository_info, repository_name=self.options.repository_name, api_root=api_root, api_client=api_client, tool=self.tool, revisions=get_revisions(self.tool, self.cmd_args), guess_summary=False, guess_description=False, is_fuzzy_match_func=self._ask_review_request_match)
        ...
        if land_error is not None:
            raise CommandError(b'Cannot land review request %s: %s' % (
             review_request_id, land_error))
        land_kwargs = {b'delete_branch': self.options.delete_branch, 
           b'destination_branch': self.options.destination_branch, 
           b'dry_run': self.options.dry_run, 
           b'edit': self.options.edit, 
           b'squash': self.options.squash}
        if self.options.recursive:
            dependency_graph = review_request.build_dependency_graph()
            dependencies = toposort(dependency_graph)[1:]
            if dependencies:
                print(b'Recursively landing dependencies of review request %s.' % review_request_id)
                for dependency in dependencies:
                    land_error = self.can_land(dependency)
                    if land_error is not None:
                        raise CommandError(b'Aborting recursive land of review request %s.\nReview request %s cannot be landed: %s' % (
                         review_request_id, dependency.id, land_error))

                for dependency in reversed(dependencies):
                    self.land(review_request=dependency, **land_kwargs)

        self.land(review_request=review_request, source_branch=branch_name, **land_kwargs)
        if self.options.push:
            print(b'Pushing branch "%s" upstream' % self.options.destination_branch)
