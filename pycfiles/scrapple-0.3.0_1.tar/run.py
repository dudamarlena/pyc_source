...
# [GCC 8.4.0]
# Embedded file name: /home/enygma/Stuff/Project/Scraping/scrapple/scrapple/commands/run.py
# Compiled at: 2016-09-05 02:28:17
"""
...
class RunCommand(command.Command):
    ...
    def execute_command(self):
        ...
        1. Selects the appropriate :ref:`selector class <implementation-selectors>` through         a dynamic dispatch, with the selector_type argument from the CLI input. 

        #. Iterate through the data section in level-0 of the configuration file.         On each data item, call the extract_content() method from the selector class to         extract the content according to the specified extractor rule. 

        #. If there are multiple levels of the extractor, i.e, if there is a 'next'         attribute in the configuration file, call the traverse_next()         :ref:`utility function <implementation-utils>` and parse through successive levels         of the configuration file.

        #. According to the --output_type argument, the result data is saved in a JSON         document or a CSV document. 
        ...
        try:
            ...
            print(Back.WHITE + Fore.RED + 'Use 0, 1 or 2 for verbosity.' + Back.RESET + Fore.RESET, sep='')
        except IOError:
            print(Back.WHITE + Fore.RED + self.args['<projectname>'], '.json does not ', 'exist. Use ``scrapple genconfig``.' + Back.RESET + Fore.RESET, sep='')
                    ...
                    for table in tables:
                        if table.get('selector', '').strip() != '':
                            ...
                            for th in table_headers:
                                if th not in tabular_data_headers:
