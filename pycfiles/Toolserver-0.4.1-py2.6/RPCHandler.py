...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Toolserver/RPCHandler.py
# Compiled at: 2010-03-01 05:38:51
VERSION = '$Id: RPCHandler.py 308 2004-11-15 21:55:35Z gb $'
import sys, zlib, string, traceback
try:
...
class rpc_handler(base_handler):
    ...
    def __init__(self):
        ...
        logInfo('installing %s handler on %s', self._name, self._match_prefix)

    def debug_input(self, data, request):
        print '********************', 'INPUT %s CODE' % self._name, '********************'
        print data

    def debug_output(self, result):
        print '********************', 'OUTPUT %s CODE' % self._name, '********************'
        print result
    ...
    def match(self, request):
        if request.uri == self._match_prefix or request.uri.startswith(self._match_prefix + '/'):
            return 1
        else:
            return 0
                                ...
                                if len(kwnames):
                                    ...
                                    for k in kwnames:
                                        if k not in argnames:
                                            if config.simplify:
                                                orderedargs.append(self.simplify_value(kw[k]))
                                            else:
                                                orderedargs.append(kw[k])
                                            del kw[k]
                                        elif config.simplify:
                                            kw[k] = self.simplify_value(kw[k])
