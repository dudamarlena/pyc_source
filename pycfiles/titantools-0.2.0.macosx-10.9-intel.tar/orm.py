...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/titantools/orm.py
# Compiled at: 2014-06-27 16:46:15
"""
...
class TiORM:
    ...
    def initialize_table(self, table_name, attrs, indexes=None):
        ...
        if table_name != 'watcher':
            z = attrs.copy()
            z.update({'unixtime': {'type': 'integer', 'nullable': False}})
            attrs = z
    ...
    def __parse_columns(self, table_name, columns):
        ...
        return (
         columns, select_columns, original_columns)

    def select(self, table_name, columns=None, where=None, limit=None, order_by=None):
        """select is your basic selection method"""
        columns, select_columns, original_columns = self.__parse_columns(table_name, columns)
        ...
        for i in results:
            ...
            if 'id' in data:
                del data['id']
            final_data = dict(data.items() + dict(zip(original_columns, i)).items() + {'_table': table_name}.items())
