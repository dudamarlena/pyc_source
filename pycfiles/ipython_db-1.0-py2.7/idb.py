...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-intel/egg/idb.py
# Compiled at: 2014-12-23 09:31:39
import os.path
...
def check_db(func):
    ...
    def deco(*args):
        if args[0]._db is None:
            print '[ERROR]Please make connection: `con = %db_connect xx` or `%use_credentials xx` first!'
            return
        else:
            return func(*args)
...
class SQLDB(Magics):
    ...
    def conn(self, parameter_s):
        ...
        if not uri.scheme:
            params = {'dbtype': 'sqlite', 'filename': os.path.join(os.path.expanduser('~'), 'db.sqlite')}
        elif uri.scheme == 'sqlite':
            params = {'dbtype': 'sqlite', 'filename': uri.path}
        else:
            params = {'username': get_or_none(uri.username), 'password': get_or_none(uri.password), 
