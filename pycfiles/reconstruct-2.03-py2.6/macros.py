...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\reconstruct\macros.py
...
def Padding(length, pattern='\x00', strict=False):
    ...
    * strict - whether or not to raise an exception is the actual padding 
    ...
    """
    return PaddingAdapter(Field(None, length), pattern=pattern, strict=strict)
...
def Flag(name, truth=1, falsehood=0, default=False):
    ...
    * default - the default value to assume, when the value is neither 
    ...
    """
    return SymmetricMapping(Field(name, 1), {True: chr(truth), False: chr(falsehood)}, default=default)
...
def CString(name, terminators='\x00', encoding=None, char_field=Field(None, 1)):
...
def IfThenElse(name, predicate, then_subcon, else_subcon):
    ...
    * else_subcon - the subcon that will be used if the predicate returns False
    """
    return Switch(name, lambda ctx: bool(predicate(ctx)), {True: then_subcon, 
       False: else_subcon})
...
def If(predicate, subcon, elsevalue=None):
    ...
    * elsevalue - the value that will be used should the predicate return False.
    ...
    """
    return IfThenElse(subcon.name, predicate, subcon, Value('elsevalue', lambda ctx: elsevalue))
