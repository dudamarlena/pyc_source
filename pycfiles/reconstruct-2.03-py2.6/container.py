...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\reconstruct\lib\container.py
...
class Container(object):
    ...
    @recursion_lock('<...>')
    def __repr__(self):
        attrs = sorted('%s = %s' % (k, repr(v)) for (k, v) in self.__dict__.iteritems() if not k.startswith('_'))
...
class LazyContainer(object):
    __slots__ = [
     'subcon', 'stream', 'pos', 'context', '_value']
