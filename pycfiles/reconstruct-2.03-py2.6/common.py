...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\reconstruct\text\common.py
...
class QuotedString(Construct):
    ...
    * allow_eof - whether to allow EOF before the closing quote is matched.
    ...
    def __init__(self, name, start_quote='"', end_quote=None, esc_char='\\', encoding=None, allow_eof=False):
        ...
        for ch in obj:
            if ch == self.esc_char:
                ...
                if self.esc_char is None:
                    raise QuotedStringError('found ending quote in data, but no escape char defined', ch)
                else:
                    self.char._build(self.esc_char, stream, context)
...
def FloatNumber(name):
    return TextualFloatAdapter(Sequence(name, GreedyRange(Digit('whole')), Literal('.'), GreedyRange(Digit('frac')), Optional(Sequence('exp', Literal('e'), Optional(CharOf('sign', '+-')), GreedyRange(Digit('value'))))))
...
def Line(name, consume_terminator=True, allow_eof=True):
    ...
    * allow_eof - whether to allow EOF to terminate the string. the default 
      is True. this option is applicable only if consume_terminator is set.
    """
    return StringUpto(name, '\n', consume_terminator=consume_terminator, allow_eof=allow_eof)
...
def Identifier(name, headset=set('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_'), tailset=set('0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_')):
    ...
    * tailset - charset for the tail. default is A-Z, a-z, 0-9 and _
    """
    return IdentifierAdapter(Sequence(name, CharOf('head', headset), OptionalGreedyRange(CharOf('tail', tailset))))