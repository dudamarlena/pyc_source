...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\reconstruct\core.py
...
class FormatField(StaticField):
    ...
    * format - a single format character
    ...
    def __init__(self, name, endianity, format):
        if endianity not in ('>', '<', '='):
            raise ValueError("endianity must be be '=', '<', or '>'", endianity)
        if len(format) != 1:
...
class Switch(Construct):
    ...
    * include_key - whether or not to include the key in the return value
    ...
    __slots__ = [
     'subcons', 'keyfunc', 'cases', 'default', 'include_key']

    def __init__(self, name, keyfunc, cases, default=NoDefault, include_key=False):
        Construct.__init__(self, name)
...
class Restream(Subconstruct):
    ...
    * resizer - a function that takes the size of the subcon and "adjusts"
    ...
    """
    __slots__ = [
     'stream_reader', 'stream_writer', 'resizer']
