...
# Embedded file name: build/bdist.linux-x86_64/egg/archipelcore/utils.py
...
import ConfigParser, socket, struct, inspect, logging, logging.handlers, os, xmpp
...
class TNArchipelLogger:
    ...
    def __log(self, level, msg):
        log = logging.getLogger('archipel')
        msg = '\x1b[33m%s.%s (%s)\x1b[0m::%s' % (self.entity.__class__.__name__, inspect.stack()[2][3], self.entity.jid, msg)
        if level < ARCHIPEL_LOG_LEVEL:
            return
        if level == ARCHIPEL_LOG_DEBUG:
            log.debug(msg)
...
class ColorFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        rec = rec.replace('DEBUG', '\x1b[35mDEBUG   \x1b[0m')
        rec = rec.replace('INFO', '\x1b[32mINFO    \x1b[0m')
        rec = rec.replace('WARNING', '\x1b[33mWARNING \x1b[0m')
        rec = rec.replace('ERROR', '\x1b[31mERROR   \x1b[0m')
        rec = rec.replace('CRITICAL', '\x1b[31mCRITICAL\x1b[0m')
        rec = rec.replace('$whiteColor', '\x1b[37m')
        rec = rec.replace('$noColor', '\x1b[0m')
        return rec
