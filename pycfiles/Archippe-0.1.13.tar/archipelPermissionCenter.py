...
# Embedded file name: build/bdist.linux-x86_64/egg/archipelcore/archipelPermissionCenter.py
...
users_have_permissions = Table('users_have_permissions', Base.metadata, Column('user', String, ForeignKey('users.name')), Column('permission', String, ForeignKey('permissions.name')))
...
class TNArchipelPermissionCenter:
    ...
    def add_admin(self, key, new_account):
        ...
        """
        if new_account not in self.root_admins.values():
