...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/rapport/cli.py
# Compiled at: 2013-07-30 09:01:47
"""
Command-line interface to rapport.
"""
from __future__ import print_function
import argparse, datetime, sys, rapport, rapport.config, rapport.email, rapport.plugin, rapport.report, rapport.timeframe
rapport.config.load()
...
class CLI(object):
    ...
    def email_xdg(self, args):
        report = rapport.report.get_report(args.report)
        rapport.email.xdg_compose(to=rapport.config.get('email', 'to'), subject=report['email.subject.text'], body=report['email.body.text'], cc=rapport.config.get('email', 'cc'), bcc=rapport.config.get('email', 'bcc'))

    def main(self):
        parser = argparse.ArgumentParser(prog='rapport', description='Work report generator for the lazy', epilog='See "rapport help COMMAND" for help on a specific command.', add_help=False, formatter_class=RapportHelpFormatter)
        parser.add_argument('--version', action='version', version=rapport.__version__, help="Shows program's version and exits")
        ...
        parser_edit.add_argument('-t', '--type', default='email', choices=('email',
                                                                           'html'))
        parser_edit.add_argument('-ep', '--email-part', default='body', choices=('body',
                                                                                 'subject'))
