...
# Embedded file name: aibolit/__main__.py
# Compiled at: 2020-04-10 11:54:00
# Size of source mod 2**32: 12810 bytes
__doc__ = "The main entry point. Invoke as `aibolit' or `python -m aibolit'.\n"
...
def predict(input_params):
    ...
    out.backward()
    results = x.grad
    sorted_result = OrderedDict(sorted((dict(zip(features_order, results.numpy().tolist())).items()),
...
def main():
    ...
    try:
        parser = argparse.ArgumentParser(description='Find the pattern which has the largest impact on readability')
        ...
        if args:
            ...
            left_space_variance, right_space_variance, max_left_space_diff, max_right_space_diff = IndentationCounter().value(java_file)
            ...
            code_lines_dict = {'var_middle_number':var_numbers, 
            ...
            input_params = {'var_middle_number':len(var_numbers), 
            ...
            for iter, (key, val) in enumerate(sorted_result.items()):
                if key in patterns_list:
                    pattern = found_pattern or key
                    code_lines = code_lines_dict.get(key)
                    if code_lines and val > 1e-20:
                        found_pattern = True
                        value = val
            ...
            if not code_lines:
                ...
                output_str = 'The largest contribution for {file} is {val} for "{pattern}" pattern'.format(file=java_file,
                  pattern=pattern,
                  val=value)
                print(output_str)
                for line in code_lines:
                    if line:
                        print('Line {}. Low readability due to: {}'.format(line, pattern))
