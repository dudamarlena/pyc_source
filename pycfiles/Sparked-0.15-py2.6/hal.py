...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sparked/hardware/hal.py
...
HAL_SUBSYSTEM_PROPERTIES = {'serial': [
            'originating_device', 'device', 'port', 'type'], 
   'video4linux': [
                 'device', 'version'], 
   'input': [
           'device'], 
   'usb': [
         'product', 'vendor', 'serial', 'vendor_id', 'product_id', 'interface.description']}
...
class HardwareMonitor(service.Service):
    ...
    def startService(self):
        ...
        self.bus.add_signal_receiver(self._halDeviceAdded, dbus_interface=HAL_MANAGER_INTERFACE, signal_name='DeviceAdded')
        self.bus.add_signal_receiver(self._halDeviceRemoved, dbus_interface=HAL_MANAGER_INTERFACE, signal_name='DeviceRemoved')
    ...
    def _halDeviceAdded(self, udi):
        ...
        if 'device' in self.deviceInfo[udi] and self.uniquePath:
            by_id = [ p for p in glob.glob(os.path.join(self.uniquePath, '*')) if os.path.realpath(p) == self.deviceInfo[udi]['device'] ]
            if by_id:
