...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sparked/hardware/inputevent.py
...
EV_FF_STATUS = 23
__all__ = [
 'InputEvent', 'InputEventDevice', 'InputEventProtocol']
...
if _64bit:
    ...
    raise RuntimeError("Couldn't determine architecture, modify " + __file__ + ' to support your system.')
...
class InputEvent(object):
    ...
    def unpack(self, buf):
        if _64bit:
            (self.time, t1, self.nanotime, t3, self.etype, self.ecode, self.evalue) = struct.unpack_from(INPUTEVENT_STRUCT, buf)
        elif _32bit:
            (self.time, self.nanotime, self.etype, self.ecode, self.evalue) = struct.unpack_from(INPUTEVENT_STRUCT, buf)
        return self

    def pack(self):
        if _64bit:
            return struct.pack(INPUTEVENT_STRUCT, self.time, 0, self.nanotime, 0, self.etype, self.ecode, self.evalue)
        if _32bit:
            return struct.pack(INPUTEVENT_STRUCT, self.time, self.nanotime, self.etype, self.ecode, self.evalue)
    ...
    def __eq__(self, other):
        return self.etype == other.etype and self.ecode == other.ecode and self.evalue == other.evalue
