...
# [GCC 8.4.0]
# Embedded file name: /home/prologic/work/mio/mio/object.py
...
class Object(object):
    ...
    def __hash__(self):
        return reduce(xor, map(hash, self.attrs.keys()), 0) ^ hash(self.parent) ^ hash(self.value) ^ reduce(xor, map(hash, self.traits.keys()), 0) ^ reduce(xor, map(hash, self.behaviors.keys()), 0) ^ hash(self.binding)
