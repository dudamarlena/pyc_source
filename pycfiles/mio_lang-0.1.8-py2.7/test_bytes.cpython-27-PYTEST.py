...
# [GCC 8.4.0]
# Embedded file name: /home/prologic/work/mio/tests/types/test_bytes.py
...
def test_bytes_type(mio):
    ...
    @py_assert12 = @py_assert10 is str
    if not @py_assert12:
        @py_format14 = @pytest_ar._call_reprcompare(('is',), (@py_assert12,), ('%(py11)s\n{%(py11)s = %(py0)s(%(py9)s\n{%(py9)s = %(py7)s\n{%(py7)s = %(py3)s\n{%(py3)s = %(py1)s.eval\n}(%(py5)s)\n}.value\n})\n} is %(py13)s',), (@py_assert10, str)) % {'py9': @pytest_ar._saferepr(@py_assert8), 'py11': @pytest_ar._saferepr(@py_assert10), 'py0': @pytest_ar._saferepr(type) if 'type' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(type) else 'type', 'py1': @pytest_ar._saferepr(mio) if 'mio' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(mio) else 'mio', 'py3': @pytest_ar._saferepr(@py_assert2), 'py5': @pytest_ar._saferepr(@py_assert4), 'py7': @pytest_ar._saferepr(@py_assert6), 'py13': @pytest_ar._saferepr(str) if 'str' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(str) else 'str'}
...
def test_init(mio):
    @py_assert1 = mio.eval
    @py_assert3 = 'x = Bytes clone(b"foo")'
    @py_assert5 = @py_assert1(@py_assert3)
    @py_assert8 = 'foo'
    @py_assert7 = @py_assert5 == @py_assert8
    if not @py_assert7:
...
def test_getitem(mio):
    @py_assert1 = mio.eval
    @py_assert3 = 'b"foo" __getitem__(0) == b"f"'
    @py_assert5 = @py_assert1(@py_assert3)
    if not @py_assert5:
...
def test_join(mio):
    @py_assert1 = mio.eval
    @py_assert3 = 'b"," join([b"1", b"2", b"3"]) == b"1,2,3"'
    @py_assert5 = @py_assert1(@py_assert3)
    if not @py_assert5:
        @py_format7 = 'assert %(py6)s\n{%(py6)s = %(py2)s\n{%(py2)s = %(py0)s.eval\n}(%(py4)s)\n}' % {'py0': @pytest_ar._saferepr(mio) if 'mio' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(mio) else 'mio', 'py2': @pytest_ar._saferepr(@py_assert1), 'py4': @pytest_ar._saferepr(@py_assert3), 'py6': @pytest_ar._saferepr(@py_assert5)}
        raise AssertionError(@pytest_ar._format_explanation(@py_format7))
...
def test_find(mio):
    @py_assert1 = mio.eval
    @py_assert3 = 'b"foobar" find(b"foo")'
    @py_assert5 = @py_assert1(@py_assert3)
    @py_assert8 = 0
    @py_assert7 = @py_assert5 == @py_assert8
    if not @py_assert7:
...
def test_find2(mio):
    @py_assert1 = mio.eval
    @py_assert3 = 'b"foobar" find(b"foo", 0, 1)'
    @py_assert5 = @py_assert1(@py_assert3)
    @py_assert8 = -1
    @py_assert7 = @py_assert5 == @py_assert8
    if not @py_assert7:
