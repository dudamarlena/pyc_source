...
# [GCC 8.4.0]
# Embedded file name: /home/prologic/work/mio/mio/state.py
...
class State(object):
    ...
    def __init__(self, args=None, opts=None):
        ...
        self.opts = opts
        self.typemap = {'tomio': {dict: self.toDict, 
                     list: self.toList, 
                     tuple: self.toTuple, 
                     bool: self.toBoolean, 
                     str: 'Bytes', 
    ...
    def find(self, name):
        if 'Types' in self.root and name in self.root['Types']:
            ...
            if 'Core' in self.root and name in self.root['Core']:
                return self.root['Core'][name]
            if 'Traits' in self.root and name in self.root['Traits']:
                return self.root['Traits'][name]
            if 'builtins' in self.root and name in self.root['builtins']:
