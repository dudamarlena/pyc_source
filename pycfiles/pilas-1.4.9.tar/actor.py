...
# [GCC 8.4.0]
# Embedded file name: ./pilasengine/actores/actor.py
# Compiled at: 2016-08-25 20:52:02
import inspect, pilasengine
...
class Actor(Estudiante):
    ...
    def actualizar(self):
        u"""Método de actualización lógico del actor.
    ...
    def dibujar(self, painter):
        u"""Pinta el personaje sobre la ventana.
    ...
    def definir_centro(self, (x, y)):
        u""" Define en que posición estará el centro del Actor.
    ...
    def _interpretar_y_convertir_posicion(self, posicion, maximo_valor):
        if posicion in IZQUIERDA + ARRIBA:
            return 0
        if posicion in CENTRO:
            return int(maximo_valor / 2.0)
        if posicion in DERECHA + ABAJO:
            return maximo_valor
    ...
    def definir_posicion(self, x, y):
        u""" Define la posición del Actor en el mundo.
    ...
    def obtener_posicion(self):
        u""" Obtiene la posición del Actor en el mundo. """
        return (
         self.x, self.y)
    ...
    espejado = property(obtener_espejado, definir_espejado, doc='Indica si se tiene que invertir horizontalmente                         la imagen del actor.')
    ...
    def pre_actualizar(self):
        u"""Actualiza comportamiento y habilidades antes de la actualización.
    ...
    def _agregar_callback(self, grupo_de_callbacks, callback):
        u"""Agrega una función para invocar en una colección.
    ...
    def _ejecutar_callback(self, evento, listado_de_callbacks):
        u""" Ejecuta el listado de métodos asociados a un callback si se
    ...
    izquierda = property(get_izquierda, set_izquierda, doc='Establece el espacio entre la izquierda del actor y el centro de coordenadas del mundo.')
    ...
    derecha = property(get_derecha, set_derecha, doc='Establece el espacio entre la derecha del actor y el centro de coordenadas del mundo.')
    ...
    abajo = property(get_abajo, set_abajo, doc='Establece el espacio entre la parte inferior del actor y el centro de coordenadas del mundo.')
    ...
    arriba = property(get_arriba, set_arriba, doc='Establece el espacio entre la parte superior del actor y el centro de coordenadas del mundo.')

    def colisiona_con_un_punto(self, x, y):
        u"""Determina si un punto colisiona con el area del actor.
    ...
    def distancia_al_punto(self, x, y):
        u"""Determina la distancia desde el centro del actor hasta el punto
        determinado
    ...
    def imitar(self, otro_actor_o_figura, *args, **kwargs):
        u""" Hace que un Actor copie la posición y la rotación de otro Actor o
        Figura fisica.
    ...
    def decir(self, mensaje, autoeliminar=True):
        u"""Emite un mensaje usando un globo similar al de los comics.
        ...
        """
        nuevo_actor = self.pilas.actores.Globo(mensaje, self.x, self.y, autoeliminar=autoeliminar, objetivo=self)
    ...
    def esta_fuera_de_la_pantalla(self):
        u"""Indica si el actor está fuera del area visible de la pantalla.
