...
# [GCC 8.4.0]
# Embedded file name: ./pilasengine/habilidades/disparar.py
...
class Disparar(habilidades.Habilidad):
    ...
    def iniciar(self, receptor, municion='Bala', parametros_municion={}, grupo_enemigos=[], cuando_elimina_enemigo=None, frecuencia_de_disparo=10, angulo_salida_disparo=0, distancia=0, offset_origen_actor=(0, 0), cuando_dispara=None, escala=1, rotacion_disparo=90, control='control.boton'):
        u"""
    ...
    def definir_colision(self, grupo_enemigos, cuando_elimina_enemigo):
        self.grupo_enemigos = grupo_enemigos
        self.pilas.colisiones.agregar(self.proyectiles, self.grupo_enemigos, cuando_elimina_enemigo)
    ...
    def disparar(self):
        ...
        if issubclass(self.municion, Actor):
            if self.municion.__name__ in ('Bala', 'Misil'):
                objeto_a_disparar = self.municion(pilas=self.pilas, x=self.receptor.x + offset_origen_actor_x, y=self.receptor.y + self.offset_origen_actor_y, rotacion=self.receptor.rotacion + -self.rotacion_disparo, angulo_de_movimiento=self.receptor.rotacion + self.angulo_salida_disparo)
                ...
                municiones = self.municion.instanciar(pilas=self.pilas, x=self.receptor.x + offset_origen_actor_x, y=self.receptor.y + self.offset_origen_actor_y, rotacion=self.receptor.rotacion + -self.rotacion_disparo, angulo_de_movimiento=self.receptor.rotacion + self.angulo_salida_disparo)
                ...
                objeto_a_disparar.hacer(self.pilas.comportamientos.Proyectil, velocidad_maxima=9, aceleracion=1, angulo_de_movimiento=angulo_de_movimiento, gravedad=0)
...
class DispararConClick(Disparar):
    ...
    def iniciar(self, receptor, municion='Bala', parametros_municion={}, grupo_enemigos=[], cuando_elimina_enemigo=None, frecuencia_de_disparo=10, angulo_salida_disparo=0, distancia=0, offset_origen_actor=(0, 0), cuando_dispara=None, escala=1, rotacion_disparo=90, control=None):
        Disparar.iniciar(self, receptor=receptor, municion=municion, parametros_municion=parametros_municion, grupo_enemigos=grupo_enemigos, cuando_elimina_enemigo=cuando_elimina_enemigo, frecuencia_de_disparo=frecuencia_de_disparo, angulo_salida_disparo=angulo_salida_disparo, distancia=distancia, offset_origen_actor=offset_origen_actor, cuando_dispara=cuando_dispara, escala=escala, rotacion_disparo=rotacion_disparo, control=control)
