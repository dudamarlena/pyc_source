...
# [GCC 8.4.0]
# Embedded file name: ./pilasengine/__init__.py
# Compiled at: 2016-12-23 13:26:53
import sys, os, datetime, traceback, random, signal, imp, time, colores
from PyQt4 import QtGui
from PyQt4 import QtCore
import configuracion, etiquetas, escenas, imagenes, actores, utils, fondos, depurador, musica, interfaz, sonidos, habilidades, comportamientos, eventos, controles, pad, watcher, plugins, simbolos, datos, fisica, widget
VERSION = '1.4.9'
...
class Pilas(object):
    ...
    def __init__(self, ancho=640, alto=480, titulo='pilas-engine', con_aceleracion=None, capturar_errores=True, habilitar_mensajes_log=False, x=None, y=None, modo_test=False, pantalla_completa=False, cargar_plugins=False):
        u"""Inicializa el area de juego con una configuración inicial."""
        ...
        self.log('Iniciando pilas con los parametros', str({'ancho': ancho, 'alto': alto, 
           'titulo': titulo, 
        ...
        self.widget = None
        self.reiniciar(ancho, alto, titulo, con_aceleracion, habilitar_mensajes_log, x, y, capturar_errores, pantalla_completa)
    ...
    def reiniciar(self, ancho=640, alto=480, titulo='pilas-engine', con_aceleracion=None, habilitar_mensajes_log=False, x=None, y=None, capturar_errores=True, pantalla_completa=False):
        ...
        self.log('Reiniciando pilas con los parametros', str({'ancho': ancho, 'alto': alto, 
           'titulo': titulo, 
           'con_aceleracion': con_aceleracion, 
           'capturar_errores': capturar_errores, 
           'habilitar_mensajes_log': habilitar_mensajes_log, 
        ...
        if con_aceleracion:
            ...
            self.widget = widget.WidgetConAceleracion(self, titulo, ancho, alto, self._capturar_errores)
        else:
            self.log('Creando el widget canvas SIN aceleracion de video')
            self.widget = widget.WidgetSinAceleracion(self, titulo, ancho, alto, self._capturar_errores)
        if self._iniciado_desde_asistente and es_reinicio:
    ...
    def reiniciar_si_cambia(self, archivo):
        u"""Regista un archivo para hacer livecoding.
    ...
    def _reiniciar_pilas_para_livecoding(self):
        u"""Calback que se ejecuta cuando se detecta modificación de un archivo observado."""
    ...
    def _modificar_codigo_para_reiniciar(self, contenido):
        ...
        for x in contenido.split('\n'):
            if 'import ' in x and 'import pilasengine' not in x and 'from ' not in x:
                modulo = x.split(' ')[1]
    ...
    def _eliminar_el_anterior_widget(self):
        u"""Quita de la ventana el widget utilizado anteriorente.
    ...
    def _vincular_el_nuevo_widget(self, parent):
        u"""Comienza a mostrar el nuevo widget en pantalla.
    ...
    def usa_aceleracion(self):
        u"""Informa si está habilitado el modo aceleración de video."""
    ...
    def obtener_coordenada_de_pantalla_relativa(self, x, y):
        u"""Convierte una coordenada pantalla en una coordenada de común.
    ...
    def obtener_coordenada_de_pantalla_absoluta(self, x, y):
        u"""Convierte una coordenada común en una coordenada de pantalla.
    ...
    def obtener_area(self):
        u"""Retorna el tamaño real de la ventana."""
    ...
    def realizar_actualizacion_logica(self):
        u"""Realiza la etapa de actualización lógica."""
    ...
    def simular_actualizacion_logica(self):
        u"""Realiza un TICK o actualización forzada de lógica.
    ...
    def realizar_dibujado(self, painter):
        u"""Realiza la etapa de actualización gráfica."""
        try:
    ...
    def obtener_ruta_al_recurso(self, ruta):
        u"""Busca la ruta a un archivo de recursos.
    ...
    def ejecutar(self):
        u"""Muestra la ventana y mantiene el programa en ejecución."""
    ...
    def obtener_posicion_del_mouse(self):
        return (
         self.widget.mouse_x, self.widget.mouse_y)
    ...
    def azar(self, a, b):
        u"""Retorna un número al azar entre `a` y `b`"""
...
def iniciar(ancho=640, alto=480, titulo='pilas-engine', capturar_errores=True, habilitar_mensajes_log=False, con_aceleracion=None, x=None, y=None, modo_test=False, pantalla_completa=False, cargar_plugins=False):
    u"""
    ...
    :cargar_plugins: Parametro de tipo booleano. Si es True, se cargan todos los plugins que se encuentren dentro del directorio
                     de plugins de pilas.
    """
    pilas = Pilas(ancho=ancho, alto=alto, titulo=titulo, capturar_errores=capturar_errores, x=x, y=y, habilitar_mensajes_log=habilitar_mensajes_log, con_aceleracion=con_aceleracion, pantalla_completa=pantalla_completa, modo_test=modo_test, cargar_plugins=cargar_plugins)
    return pilas
