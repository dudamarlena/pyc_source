...
# [GCC 8.4.0]
# Embedded file name: ./pilasengine/interprete/lanas.py
# Compiled at: 2016-08-25 20:52:02
import sys, inspect, os, code, traceback
os.environ['lanas'] = 'enabled'
from PyQt4.QtGui import QWidget, QDesktopWidget, QPalette, QColor, QTextCursor, QTextEdit, QInputDialog, QApplication, QKeyEvent
from PyQt4.QtCore import Qt, QTimer
from editorbase import editor_base
import lanas_ui, io_lanas
...
class InterpreteLanas(editor_base.EditorBase):
    ...
    def __init__(self, parent, codigo_inicial=None):
        ...
        self.history = []
        self.historyIndex = -1
        self.interpreterLocals = {'raw_input': self.raw_input, 'input': self.input, 
           'sys': sys, 
           'help': self.help, 
           'ayuda': self.help}
        palette = QPalette()
    ...
    def insertar_error(self, mensaje):
        ...
        for m in mensajes:
            ...
            self.insertHtml(" <b style='color: #F00000'> &nbsp; ×</b> # %s <br>" % m.decode('utf-8'))
    ...
    def insertar_mensaje(self, mensaje):
        self.insertPlainText('\n')
        self.insertHtml(" <b style='color: green'>  &nbsp; %s </b><br>" % mensaje)
        self.insertPlainText('\n')
        self._mover_cursor_al_final()
    ...
    def marker(self):
        if self.multiline:
            self.insertHtml("<b style='color: #FF0000'>‥</b> ")
            self.insertPlainText('    ')
        else:
            self.insertHtml("<b style='color: #3583FC'>»</b> ")
    ...
    def keyPressEvent(self, event):
        if event.key() in [Qt.Key_Tab]:
            ...
            if event.modifiers() & Qt.ControlModifier:
                if textCursor.blockNumber() != self.document().blockCount() - 1:
                    if event.key() == Qt.Key_V:
                        ...
                        return
                elif textCursor.positionInBlock() < 2 and event.key() == Qt.Key_V:
                    textCursor = self._mover_cursor_al_final()
                    return
            ...
            if event.key() in [Qt.Key_Return, Qt.Key_Enter]:
                ...
                if self.multiline and (not self.haveLine or self._ha_ingresado_solo_espacios(line)):
                    self.append('')
                    try:
                ...
                if self.haveLine and not self.multiline:
                    self.command = line
                    self.append('')
                    if '=' in line:
    ...
    def _autocompletar_argumentos_si_corresponde(self):
        u"""Muestra un mensaje con la documentación de una función ejecutar.
