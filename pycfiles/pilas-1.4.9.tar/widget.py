...
# [GCC 8.4.0]
# Embedded file name: ./pilasengine/widget.py
...
class WidgetConAceleracion(QGLWidget):
    ...
    def obtener_centro_fisico(self):
        ...
        return (
         self.original_width / 2, self.original_height / 2)
    ...
    def timerEvent(self, event):
        u"""Actualiza la simulación completa.
    ...
    def keyPressEvent(self, event):
        ...
        self.pilas.eventos.pulsa_tecla.emitir(codigo=codigo_de_tecla, es_repeticion=event.isAutoRepeat(), texto=event.text())
    ...
    def keyReleaseEvent(self, event):
        if event.isAutoRepeat():
            return
        codigo_de_tecla = Controles.obtener_codigo_de_tecla_normalizado(event.key())
        self.pilas.eventos.suelta_tecla.emitir(codigo=codigo_de_tecla, es_repeticion=event.isAutoRepeat(), texto=event.text())

    @capturar_errores_decorator
    def mousePressEvent(self, event):
        x, y = self.pilas.obtener_coordenada_de_pantalla_relativa((event.pos().x() - self.window_dx) / self.escala, (event.pos().y() - self.window_dy) / self.escala)
    ...
    def desempotrar(self):
        u""" Desempotra el widget de cualquier widget al que esté empotrado.
    ...
    def empotrar(self):
        u""" Empotra el widget al widget que estaba previamente empotrado
    ...
    def esta_en_modo_pausa(self):
        u"""Informa si el widget está o no en modo pausa."""
        return self.pausa

    def avanzar_un_solo_cuadro(self):
        u"""Avanza un solo cuadro de animación estando en modo pausa."""
        self.pilas.realizar_actualizacion_logica()
