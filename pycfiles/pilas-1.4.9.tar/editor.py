...
# [GCC 8.4.0]
# Embedded file name: ./pilasengine/interprete/editor.py
# Compiled at: 2016-08-25 21:09:54
import codecs, re, sys, os, inspect, tempfile, shutil
from PyQt4.QtCore import Qt, QTimer
from PyQt4.Qt import QFrame, QWidget, QPainter, QSize, QVariant
from PyQt4.QtGui import QTextEdit, QTextCursor, QFileDialog, QIcon, QMessageBox, QShortcut, QInputDialog, QLineEdit, QErrorMessage, QKeySequence, QTextFormat, QColor, QKeyEvent
from PyQt4.QtCore import Qt
from PyQt4 import QtCore
from editorbase import editor_base
import editor_ui, pilasengine
CODIGO_INICIAL = '# coding: utf-8\nimport pilasengine\n\npilas = pilasengine.iniciar()\n\nmono = pilas.actores.Mono()\n\n# Algunas transformaciones:\n# (Pulsá el botón derecho del\n#  mouse sobre alguna de las\n#  sentencias)\n\nmono.x = 0\nmono.y = 0\nmono.escala = 1.0\nmono.rotacion = 0\n\npilas.ejecutar()'
...
class WidgetEditor(QWidget, editor_ui.Ui_Editor):
    ...
    def __init__(self, main=None, interpreter_locals=None, consola_lanas=None, ventana_interprete=None, *args):
        ...
        self.boton_abrir.connect(self.boton_abrir, QtCore.SIGNAL('clicked()'), self.editor.abrir_archivo_con_dialogo)
        self.set_icon(self.boton_guardar, 'iconos/guardar.png')
        self.boton_guardar.connect(self.boton_guardar, QtCore.SIGNAL('clicked()'), self.editor.guardar_contenido_directamente)
        self.set_icon(self.boton_guardar_como, 'iconos/guardar_como.png')
        self.boton_guardar_como.connect(self.boton_guardar_como, QtCore.SIGNAL('clicked()'), self.editor.guardar_contenido_con_dialogo)
        self.set_icon(self.boton_actualizar, 'iconos/actualizar.png')
        self.boton_actualizar.connect(self.boton_actualizar, QtCore.SIGNAL('clicked()'), self.actualizar_el_listado_de_archivos)
        self.set_icon(self.boton_nuevo, 'iconos/nuevo.png')
        self.boton_nuevo.connect(self.boton_nuevo, QtCore.SIGNAL('clicked()'), self.pulsa_boton_crear_archivo_nuevo)
        ...
        self.boton_pausar.connect(self.boton_pausar, QtCore.SIGNAL('clicked()'), self.cuando_pulsa_el_boton_pausar)
        self.set_icon(self.boton_siguiente, 'iconos/siguiente.png')
        self.boton_siguiente.connect(self.boton_siguiente, QtCore.SIGNAL('clicked()'), self.cuando_pulsa_el_boton_siguiente)
    ...
    def _vincular_atajos_de_teclado(self):
        ...
        if sys.platform == 'darwin':
            self.boton_ejecutar.setToolTip('Ejecutar el código actual (F5, ⌘R, ⌘S)')
    ...
    def actualizar_el_listado_de_archivos(self):
        ...
        if self.editor.es_archivo_iniciar_sin_guardar():
            self.selector_archivos.addItem('archivo sin título ...')
        else:
...
class Editor(editor_base.EditorBase):
    ...
    def keyPressEvent(self, event):
        u"""Atiene el evento de pulsación de tecla."""
        ...
        if editor_base.EditorBase.keyPressEvent(self, event):
            ...
            if event.key() == Qt.Key_Return:
                ...
                whitespace = re.match('(\\s*)', unicode(block.text())).group(1)
    ...
    def _borrar_un_grupo_de_espacios(self, event):
        cursor = self.textCursor()
        block = self.document().findBlockByNumber(cursor.blockNumber())
        whitespace = re.match('(.*)', unicode(block.text())).group(1)
    ...
    def abrir_dialogo_cargar_archivo(self):
        return QFileDialog.getOpenFileName(self, 'Abrir Archivo', self.nombre_de_archivo_sugerido, 'Archivos python (*.py)', options=QFileDialog.DontUseNativeDialog)
    ...
    def mensaje_guardar_cambios_abrir(self):
        ...
        titulo = '¿Deseas guardar el contenido antes de abrir un archivo?'
        mensaje = 'El contenido se perdera sino los guardas'
    ...
    def mensaje_guardar_cambios_salir(self):
        ...
        titulo = '¿Deseas guardar el contenido antes de salir?'
        mensaje = 'El contenido se perdera sino los guardas'
        return QMessageBox.question(self, titulo, mensaje, 'Salir sin guardar', 'Guardar', 'Cancelar')
    ...
    def ejecutar(self):
        ...
        modulos_a_recargar = [ x for x in self.interpreterLocals.values() if inspect.ismodule(x) and x.__name__ not in ('pilasengine',
                                                                                                                        'inspect') and 'pilasengine.' not in x.__name__
                             ]
        ...
        try:
            exec contenido in self.interpreterLocals
    ...
    def cuando_selecciona_actor_por_indice(self, indice):
        ...
        exec capturar_actor in self.interpreterLocals
        exec resaltar in self.interpreterLocals
