...
# [GCC 8.4.0]
# Embedded file name: ./pilasengine/utils/pitweener.py
...
class TweenerEquations(object):
    ...
    def OUT_IN_QUAD(self, t, b, c, d):
        if t < d * 0.5:
            return self.OUT_QUAD(t * 2, b, c * 0.5, d)
        return self.IN_QUAD(t * 2 - d, b + c * 0.5, c * 0.5, d)
    ...
    def OUT_IN_CUBIC(self, t, b, c, d):
        if t < d * 0.5:
            return self.OUT_CUBIC(t * 2.0, b, c * 0.5, d)
        return self.IN_CUBIC(t * 2.0 - d, b + c * 0.5, c * 0.5, d)
    ...
    def OUT_ELASTIC(self, t, b, c, d):
        ...
        return a * 2.0 ** (-10.0 * t) * math.sin((t * d - s) * (2.0 * math.pi) / p) + c + b
...
class Tweener(TweenerEquations):
    ...
    def add_tween(self, obj, inicial=None, **kwargs):
        ...
        if 'tween_delay' in kwargs:
            ...
            t_delay = 0
        tw = Tween(obj, t_time, t_type, t_complete_func, t_update_func, t_delay, inicial=inicial, **kwargs)
        if tw:
...
class Tween(object):
    ...
    def __init__(self, obj, tduration, tween_type, complete_function, update_function, delay, inicial, **kwargs):
        """Tween object:
    ...
    def update(self, ptime=None):
        ...
        if not self.complete:
            for prop_name, prop, tweenable in self.t_props:
                setattr(self.target, prop, self.tween(self.delta, tweenable.start_value, tweenable.change, self.duration))

            for func_name, func, tweenable in self.t_funcs:
                func(self.tween(self.delta, tweenable.start_value, tweenable.change, self.duration))
...
if __name__ == '__main__':
    ...
    T = Tweener()
    tst = TweenTestObject()
    mt = T.add_tween(tst, tween_time=2.5, tween_type=T.LINEAR, pos=-200, on_complete_function=tst.complete, on_update_function=tst.update)
    changed = False
    while T.has_tweens():
        T.update()
        time.sleep(0.06)
