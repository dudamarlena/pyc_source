...
# [GCC 8.4.0]
# Embedded file name: /home/erik997/dev/python/dhooks-lite/venv/lib/python3.5/site-packages/dhooks_lite/embed.py
# Compiled at: 2020-03-31 11:00:15
# Size of source mod 2**32: 9646 bytes
import logging, datetime, json
logger = logging.getLogger(__name__)

class _EmbedObject:
    __doc__ = 'base class for all Embed objects'
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, type(self)):
            return False
        return all(self.__dict__[key1] == other.__dict__[key2] for key1, key2 in zip(self.__dict__.keys(), other.__dict__.keys()))
...
class Field(_EmbedObject):
    ...
    def __init__(self, name: str, value: str, inline: bool=True):
        ...
        if len(name) > self.MAX_CHARACTERS_NAME:
            raise ValueError('name can not exceed {} characters'.format(self.MAX_CHARACTERS_NAME))
        value = str(value)
        if len(value) > self.MAX_CHARACTERS_VALUE:
            raise ValueError('value can not exceed {} characters'.format(self.MAX_CHARACTERS_VALUE))
        self._name = name
        self._value = value
        self._inline = inline
...
class Thumbnail(Image):
    __doc__ = 'Thumbnail in an Embed'


class Embed(_EmbedObject):
    __doc__ = 'Embedded content for a message'
    MAX_CHARACTERS = 6000
    MAX_TITLE = 256
    ...
    def __init__(self, description: str=None,
        ...
        if fields:
            if len(fields) > self.MAX_FIELDS:
                raise ValueError('Fields can not exceed {} objects'.format(self.MAX_FIELDS))
            for f in fields:
                if not isinstance(f, Field):
                    raise TypeError('all elements in fields must be a Field')

        if description and len(description) > self.MAX_DESCRIPTION:
            raise ValueError('description exceeds max length of {} characters'.format(self.MAX_DESCRIPTION))
        if title and len(title) > self.MAX_TITLE:
            raise ValueError('title exceeds max length of {} characters'.format(self.MAX_TITLE))
        ...
        if len(d_json) > self.MAX_CHARACTERS:
            raise ValueError('Embed exceeds maximum allowed char size of {} by {}'.format(self.MAX_CHARACTERS, len(d_json) - self.MAX_CHARACTERS))
