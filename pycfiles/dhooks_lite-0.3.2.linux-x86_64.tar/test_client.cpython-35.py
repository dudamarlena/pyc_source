...
# [GCC 8.4.0]
# Embedded file name: /home/erik997/dev/python/dhooks-lite/venv/lib/python3.5/site-packages/tests/test_client.py
# Compiled at: 2020-03-31 14:47:06
# Size of source mod 2**32: 10029 bytes
import logging
...
class TestWebhook(TestCase):
    ...
    @patch(MODULE_PATH + '.requests', auto_spec=True)
    def test_produce_normal_logging_with_http_error(self, mock_requests, mock_logger):
        mock_response = Mock()
        mock_response.headers = {'headers': True}
    ...
    @patch(MODULE_PATH + '.logger.getEffectiveLevel', return_value=logging.INFO)
    @patch(MODULE_PATH + '.requests', auto_spec=True)
    def test_produce_no_logging_when_http_ok(self, mock_requests, mock_logger):
        mock_requests.post.return_value = self.response
        hook = Webhook('abc')
        hook.execute('Hi there')
...
class TestWebhookResponse(TestCase):
    ...
    def setUp(self):
        self.response = WebhookResponse(headers={'Content-Type': 'application/json'}, content={'username': 'Bruce Wayne', 
         'content': 'Checkout this new report'}, status_code=200)
...
class TestWebhookAndEmbed(TestCase):
    ...
    def test_can_send_with_embed_only(self, mock_requests):
        ...
        self.assertDictEqual(json['embeds'][0], {'description': 'Hello, world!', 'type': 'rich'})
    ...
    def test_can_add_multiple_embeds(self, mock_requests):
        ...
        self.assertDictEqual(json['embeds'][0], {'description': 'Hello, world!', 'type': 'rich'})
        self.assertDictEqual(json['embeds'][1], {'description': 'Hello, world! Again!', 'type': 'rich'})