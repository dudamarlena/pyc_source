...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/seaweedfs/client.py
# Compiled at: 2019-11-17 02:38:07
...
class Filer(object):
    ...
    def download(self, filename, path):
        ...
        """
        if filename:
            raise path or ParamNoneError
        else:
            url = urljoin(self.url, self.add_slash_if_needed(path) + filename)
            resp = requests.get(url)
            return resp.ok or (False, None)
        return (
         True,
         {'content_length':resp.headers.get('content-length'), 
          'content_type':resp.headers.get('content-type'),  'content':str(resp.content, encoding='utf-8')})
