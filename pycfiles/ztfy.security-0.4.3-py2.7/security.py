...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ztfy/security/security.py
# Compiled at: 2012-09-24 05:17:02
...
from ztfy.security.interfaces import ILocalRoleManager, ISecurityManager, ISecurityManagerRoleChecker, IBaseRoleEvent, IGrantedRoleEvent, IRevokedRoleEvent
...
class SecurityManagerAdapter(object):
    ...
    def getLocalRoles(self, principal=None):
        ...
        if not principal:
            ...
            return {'allow': set(role[0] for role in roles if role[1].getName() == 'Allow'), 'deny': set(role[0] for role in roles if role[1].getName() == 'Deny')}
    ...
    def getLocalPrincipals(self, role):
        ...
        return {'allow': set(principal[0] for principal in principals if principal[1].getName() == 'Allow'), 'deny': set(principal[0] for principal in principals if principal[1].getName() == 'Deny')}
