...
# [GCC 8.4.0]
# Embedded file name: ./vendor/redis/client.py
# Compiled at: 2019-07-06 06:05:05
# Size of source mod 2**32: 101774 bytes
from __future__ import with_statement
from itertools import chain
import datetime, sys, warnings, time, threading, time as mod_time
from redis._compat import b, basestring, bytes, imap, iteritems, iterkeys, itervalues, izip, long, nativestr, unicode, safe_unicode
...
from redis.exceptions import ConnectionError, DataError, ExecAbortError, NoScriptError, PubSubError, RedisError, ResponseError, TimeoutError, WatchError
SYM_EMPTY = b('')
...
SENTINEL_STATE_TYPES = {'can-failover-its-master':int, 
 'config-epoch':int, 
...
def parse_sentinel_state(item):
    ...
    for name, flag in (('is_master', 'master'), ('is_slave', 'slave'), ('is_sdown', 's_down'),
                       ('is_odown', 'o_down'), ('is_sentinel', 'sentinel'), ('is_disconnected', 'disconnected'),
...
def zset_score_pairs(response, **options):
    ...
    """
    return response and options['withscores'] or response
...
def sort_return_tuples(response, **options):
    ...
    """
    return response and options['groups'] or response
    n = options['groups']
...
class StrictRedis(object):
        ...
        if not connection_pool:
            if charset is not None:
                ...
                if unix_socket_path is not None:
                    kwargs.update({'path':unix_socket_path, 
                    ...
                    if ssl:
                        kwargs.update({'connection_class':SSLConnection, 
    ...
    def bitcount(self, key, start=None, end=None):
        ...
        if start is not None and end is not None:
            ...
            if start is not None and end is None or end is not None:
                if start is None:
    ...
    def mset(self, *args, **kwargs):
        ...
        """
        if args:
            if not (len(args) != 1 or isinstance(args[0], dict)):
    ...
    def sort(self, name, start=None, num=None, by=None, get=None, desc=False, alpha=False, store=None, groups=False):
        ...
        if start is not None:
            if num is None or num is not None:
                if start is None:
    ...
    def sscan_iter(self, name, match=None, count=None):
        ...
        cursor = '0'
        while cursor != 0:
            cursor, data = self.sscan(name, cursor=cursor, match=match,
              count=count)
            for item in data:
                yield item
    ...
    def hscan_iter(self, name, match=None, count=None):
        ...
        cursor = '0'
        while cursor != 0:
            cursor, data = self.hscan(name, cursor=cursor, match=match,
              count=count)
    ...
    def zscan_iter(self, name, match=None, count=None, score_cast_func=float):
        ...
        cursor = '0'
        while cursor != 0:
            cursor, data = self.zscan(name, cursor=cursor, match=match, count=count,
    ...
    def zrangebylex(self, name, min, max, start=None, num=None):
        ...
        range.
        """
        if start is not None and num is None or num is not None:
            if start is None:
                raise RedisError('``start`` and ``num`` must both be specified')
        pieces = [
         'ZRANGEBYLEX', name, min, max]
        if start is not None:
            if num is not None:
    ...
    def zrangebyscore(self, name, min, max, start=None, num=None, withscores=False, score_cast_func=float):
        ...
        `score_cast_func`` a callable used to cast the score return value
        """
        if not (start is not None and num is None):
            if num is not None:
                if start is None:
                    raise RedisError('``start`` and ``num`` must both be specified')
            pieces = [
...
class Redis(StrictRedis):
    ...
    RESPONSE_CALLBACKS = dict_merge(StrictRedis.RESPONSE_CALLBACKS, {'TTL':lambda r: r >= 0 and r or None, 
...
class PubSub(object):
    ...
    def handle_message(self, response, ignore_subscribe_messages=False):
        ...
        if message_type == 'pmessage':
            message = {'type':message_type,  'pattern':response[1], 
             'channel':response[2], 
             'data':response[3]}
        else:
            message = {'type':message_type,  'pattern':None, 
             'channel':response[1], 
             'data':response[2]}
...
class BasePipeline(object):
    ...
    def execute_command(self, *args, **kwargs):
        if self.watching or args[0] == 'WATCH':
    ...
    def load_scripts(self):
        ...
        if not all(exists):
            for s, exist in izip(scripts, exists):
                if not exist:
                    s.sha = immediate('SCRIPT', 'LOAD', (s.script), **{'parse': 'LOAD'})
    ...
    def execute(self, raise_on_error=True):
        ...
        if self.scripts:
            self.load_scripts()
        elif self.transaction or self.explicit_transaction:
    ...
    def script_load_for_pipeline(self, script):
        ...
        if not script.sha:
            script.sha = (self.immediate_execute_command)('SCRIPT', 'LOAD', 
             (script.script), **{'parse': 'LOAD'})
        self.scripts.add(script)


class StrictPipeline(BasePipeline, StrictRedis):
    __doc__ = 'Pipeline for the StrictRedis class'


class Pipeline(BasePipeline, Redis):
    __doc__ = 'Pipeline for the Redis class'


class Script(object):
    __doc__ = 'An executable Lua script object returned by ``register_script``'
