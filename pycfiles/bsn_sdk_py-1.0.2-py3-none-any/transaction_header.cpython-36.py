...
# Embedded file name: build\bdist.win-amd64\egg\trans\transaction_header.py
# Compiled at: 2020-04-23 03:24:10
...
def created_common_common_channelheader(t: TransactionHeader, chaincodeheaderextension: proposal_pb2.ChaincodeHeaderExtension):
...
def created_peer_proposal_proposal(common_common_header: common_pb2.Header, payload: proposal_pb2.ChaincodeProposalPayload):
...
def get_notrust_trans_data(channelID, mspid, chainCode, cert_pub_path, transientData, args, funcName):
    ...
    :param channelID: 应用链名称     channelId       String  是
    :param mspid: 城市MSPID       mspId   String  是
    :param chainCode: 链码Code    chainCode       String  是
    :param cert_pub_path:
    :param transientData: 临时数据 transientData Map<string,string>  否
    :param args: 请求参数   args    String[]        否
    :param funcName: 方法名称       funcName        String  是
    :return:
    """
    ...
    peer_chaincode_chaincodeinput = created_peer_chaincode_chaincodeinput(args_bytes)
    peer_chaincode_chaincodespec = created_peer_chaincode_chaincodespec(chaincodeId, peer_chaincode_chaincodeinput)
    peer_chaincode_chaincodeinvocationspec = created_peer_chaincode_chaincodeinvocationspec(peer_chaincode_chaincodespec)
    peer_proposal_chaincodeproposalpayload = created_peer_proposal_chaincodeproposalpayload(peer_chaincode_chaincodeinvocationspec, transientData)
    ...
    comm_channel_header = created_common_common_channelheader(transaction_header, peer_proposal_chaincodeheaderextension)
...
if __name__ == '__main__':
    ...
    mspid = 'OrgbNodeMSP'
    cert_pub_path = 'E:\\hz_workspace\\study\\bsn_sdk_py\\test\\keystore\\hll3@app0001202004161020152918451_cert.pem'
    peer_proposal_proposal = get_notrust_trans_data(channelID=channelID,
      mspid=mspid,
      cert_pub_path='E:\\hz_workspace\\study\\bsn_sdk_py\\test\\keystore\\hll3@app0001202004161020152918451_cert.pem',
    ...
    with open('E:\\hz_workspace\\study\\bsn_sdk_py\\test\\keystore\\hll3@app0001202004161020152918451_private.pem', 'rb') as (f):
        key_data = f.read()
