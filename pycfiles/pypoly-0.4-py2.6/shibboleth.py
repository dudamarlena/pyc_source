...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pypoly/http/auth/shibboleth.py
# Compiled at: 2011-11-24 06:31:31
"""
"""
import re, pypoly, pypoly.session
...
class Main(Component):
    ...
    def start(self):
        if pypoly.config.get('login.auto', False):
            pypoly.hook.register('http.request.serve.pre', 'plugin.auth.shib.autologin', autologin)
...
class ShibAuth(AuthPlugin):
        ...
        if mapping_file != None and len(mapping_file) > 0:
            try:
                ...
                while True:
                    ...
                    if not sep == ':':
                        continue
                    pypoly.log.debug('Group mapping: %s to %s' % (
                     match,
                     group))
                    self._mapping.append((
                     re.compile('^' + match + '$'), [ g.strip() for g in group.split(',') ]))
    ...
    def _map_groups(self, groups):
        if self._mapping == None:
            ...
            pypoly.log.debug('Mapping groups from %s to %s' % (
             str(groups),
             str(res)))
            return res

    def login(self, username, password):
        name = pypoly.http.request.headers.get(pypoly.config.get('username'), None)
        if name == None or name != username:
            ...
            if pypoly.config.get('group.parse') == True:
                ...
                groups = pypoly.http.request.headers.get(pypoly.config.get('group.header'), None)
                pypoly.log.debug("Get from header '%s': %s" % (
                 pypoly.config.get('group.header'),
                 groups))
                if groups != None:
                    groups = groups.split(pypoly.config.get('group.separator').strip())
                    regex = pypoly.config.get('group.regex').strip()
                    if len(regex) == 0:
                        pypoly.log.debug('No RegEx given. Setting groups: %s' % str(tmp_groups))
                    ...
                    return True
            pypoly.session.set('groups', pypoly.config.get('groups'))
            return True
