...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pypoly/content/webpage/form/input.py
# Compiled at: 2011-09-13 08:13:13
import re
...
class TextInput(Input):
    ...
    def __init__(self, name, **options):
        ...
        msg = {'error_maxLength': _('%(label)s: Enter a value less than %(maxLength)i characters long.'), 
    ...
    def validate(self):
        ...
        if self.raw_value != None:
            if self.max_length != None and len(self.raw_value) > self.max_length:
                errors.append(self._messages['error_maxLength'] % {'max_length': self.max_length, 'label': self.label})
            if self.min_length != None and (self.raw_value == None or len(self.raw_value) < self.min_length):
                errors.append(self._messages['error_minLength'] % {'min_length': self.min_length, 'label': self.label})
...
class CustomInput(Input):
    ...
    def __init__(self, name, **options):
        Input.__init__(self, name, **options)
        msg = {'error_characters': _("%(label)s: Only characters '%(characters)s' are allowed"), 
        ...
        if self.characters != None and self._value != None:
            for char in self._value:
                if char not in self.characters:
                    errors.append(self._messages['error_characters'] % {'label': self.label, 
...
class DateInput(Input):
    ...
    def __init__(self, name, **options):
        Input.__init__(self, name, **options)
        msg = {'error_wrong_format': _('%(label)s: Wronge Format')}
        self._messages.update(msg)
    ...
    def validate(self):
        ...
        if self.required == False and self._value == '':
            ...
            if match:
                try:
                    import datetime
                    self._value = datetime.date(int(match.group('year')), int(match.group('month')), int(match.group('day')))
                except Exception, e:
...
class DecimalInput(Input):
    ...
    def __init__(self, name, **options):
        ...
        msg = {'error_nonumber': _('%(label)s: Please enter a number'), 
           ...
           'error_mindecimal': _('%(label)s: Please enter more decimale. Min Decimals %(minDecimal)s'), 
           'error_maxdecimal': _('%(label)s: Please enter less decimale. Max Decimals %s(maxDecimal)s'), 
        ...
        if self.raw_value != None and len(self.raw_value) > 0:
            try:
                ...
                if self.max_value != None and self._value > self.max_value:
                    errors.append(self._messages['error_tolarge'] % {'label': self.label, 'max_value': self.max_value})
                if self.min_value != None and self._value < self.min_value:
                    errors.append(self._messages['error_tosmall'] % {'label': self.label, 'min_value': self.min_value})
                ...
                if self.min_decimal_places != None and decimal_places < self.min_decimal:
                    errors.append(self._messages['error_mindecimal'] % {'label': self.label, 'min_vecimal': self.min_decimal})
                if self.max_decimal_places != None and decimal_places > self.max_decimal_places:
                    errors.append(self._messages['error_maxdecimal'] % {'label': self.label, 'maxDecimal': self.max_decimal_places})
...
class FileInput(Input):
    ...
    def __init__(self, name, **options):
        ...
        self.file = None
        Input.__init__(self, name, **options)
        msg = {'error_nofile': _('%(label)s: This is not a file')}
        self._messages.update(msg)
        return
    ...
    def validate(self):
        ...
        if self.raw_value != None and type(self.raw_value.__class__) != type(cgi.FieldStorage):
            errors.append(self._messages['error_nofile'] % {'label': self.label})
        elif type(self.raw_value.__class__) == type(cgi.FieldStorage) and self.raw_value.file != None:
                ...
                if self.min_decimal != None or self.max_decimal != None:
                    ...
                    if len(tmp) == 2:
                        if self.min_decimal != None and len(tmp[1]) >= self.min_decimal:
                            errors.append(self._messages['error_mindecimal'] % {'label': self.label, 'minDecimal': self.min_decimal})
                        if self.max_decimal != None and len(tmp[1]) >= self.max_decimal:
                            errors.append(self._messages['error_maxdecimal'] % {'label': self.label, 'maxDecimal': self.max_decimal})
                    else:
...
class NumberInput(Input):
    ...
    def __init__(self, name, **options):
        ...
        self.min_value = None
        Input.__init__(self, name, **options)
        msg = {'error_nonumber': _('%(label)s: Please enter a number'), 'error_tolarge': _('%(label)s: The value is to large. Please enter a value less then %(maxValue)s'), 
           'error_tosmall': _('%(label)s: The value is to small. Please enter a value larger then %(minValue)s')}
        self._messages.update(msg)
        return
        ...
        if self.required == False and self._value == '':
            ...
            if self._value != None and len(self._value) > 0:
                try:
                    ...
                    if self.max_value != None and self._value > self.max_value:
                        errors.append(self._messages['error_tolarge'] % {'label': self.label, 'maxValue': self.max_value})
                    if self.min_value != None and self._value < self.min_value:
                        errors.append(self._messages['error_tosmall'] % {'label': self.label, 'minValue': self.min_value})
                except:
                    errors.append(self._messages['error_nonumber'] % {'label': self.label})
