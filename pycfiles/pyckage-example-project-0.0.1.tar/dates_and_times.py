...
# Embedded file name: /MyWork/Projects/PyCK/pyck/lib/dates_and_times.py
# Compiled at: 2015-07-21 23:14:05
...
class DateTime(Date, Time):
    ...
    def to_native(self):
        ...
        assert False == self.is_partial()
        return datetime(year=self.year, month=self.month, day=self.day, hour=self.hour, minute=self.minute, second=self.second, microsecond=self.microsecond)
    ...
    def range(self):
        ...
        if not self.month:
            return (datetime(year=self.year, month=1, day=1),
             datetime(year=self.year, month=12, day=31, hour=23, minute=59, second=59, microsecond=999999))
        if not self.day:
            ...
            return (
             datetime(self.year, self.month, 1), d)
        if not self.hour:
            return (datetime(year=self.year, month=self.month, day=self.day),
             datetime(year=self.year, month=self.month, day=self.day, hour=23, minute=59, second=59, microsecond=999999))
        if not self.minute:
            return (
             datetime(year=self.year, month=self.month, day=self.day, hour=self.hour),
             datetime(year=self.year, month=self.month, day=self.day, hour=self.hour, minute=59, second=59, microsecond=999999))
        if not self.second:
            return (
             datetime(year=self.year, month=self.month, day=self.day, hour=self.hour, minute=self.minute),
             datetime(year=self.year, month=self.month, day=self.day, hour=self.hour, minute=self.minute, second=59, microsecond=999999))
        if not self.microsecond:
            return (
             datetime(year=self.year, month=self.month, day=self.day, hour=self.hour, minute=self.minute, second=self.second),
             datetime(year=self.year, month=self.month, day=self.day, hour=self.hour, minute=self.minute, second=self.second, microsecond=999999))