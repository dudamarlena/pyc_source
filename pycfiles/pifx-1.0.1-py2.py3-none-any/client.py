...
# [GCC 8.4.0]
# Embedded file name: pifx/client.py
...
class LIFXWebAPIClient:
    ...
    def perform_request(self, method, endpoint, endpoint_args=[], argument_tuples=None, json_body=False, parse_data=True):
        http_endpoint = self._full_http_endpoint(endpoint.format(*endpoint_args))
        data = None
        if argument_tuples is not None:
            data = util.arg_tup_to_dict(argument_tuples)
        if json_body:
            res = self._s.request(method=method, url=http_endpoint, json=data, headers=self.headers)
        else:
            res = self._s.request(method=method, url=http_endpoint, data=data, headers=self.headers)
