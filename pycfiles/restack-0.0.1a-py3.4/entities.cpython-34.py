...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/restack/entities.py
# Compiled at: 2015-02-22 07:21:23
...
class Device(object):
    ...
    def _load_from_response_data(self, response_data):
        fields = ('id', 'name', 'description', 'visibility', 'status', 'url', 'created',
                  'updated')
    ...
    def create_stack(self, name, unit, symbol, stack_type):
        if stack_type not in (Stack.NUMERIC, Stack.ALPHANUMERIC):
            raise RestackError('Stack type must be either `Stack.Numeric` or `Stack.ALPHANUMERIC`')
...
class Stack(object):
    ...
    @name.setter
    def name(self, value):
        match = re.match('[a-zA-Z0-9-_]*$', value)
        if not match:
