...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/psqlparse/nodes.py
...
class FromClause(object):
    ...
    def __init__(self, items):
        ...
        for item in items:
            ...
            self.items.append(_NODE_MAP[key](item[key]) if key in _NODE_MAP else item)
...
class WithClause(object):
    ...
    def __init__(self, obj):
        ...
        self.queries = {cte['ctename']:Statement(cte['ctequery']) for cte in ctes}
    ...
    def __str__(self):
        ...
        if self.recursive:
            s += 'RECURSIVE '
        s += ', '.join(['%s AS (%s)' % (name, query) for name, query in six.iteritems(self.queries)])
        return s
...
class Statement(object):
    ...
    def __init__(self, obj):
        ...
        self.from_clause = FromClause(self._obj.get('fromClause')) if 'fromClause' in self._obj else None
        self.target_list = TargetList(self._obj.get('targetList')) if 'targetList' in self._obj else None
        self.where_clause = WhereClause(self._obj.get('whereClause')) if 'whereClause' in self._obj else None
        self.with_clause = WithClause(self._obj.get('withClause')) if 'withClause' in self._obj else None
