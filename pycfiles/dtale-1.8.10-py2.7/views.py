...
def dtype_formatter(data, dtypes, data_ranges, prev_dtypes=None):
    ...
    def _formatter(col_index, col):
        ...
        s = data[col]
        dtype_data = dict(name=col, dtype=dtype, index=col_index, visible=visible, hasMissing=bool(s.isnull().any()), hasOutliers=False)
...
def outliers(data_id, column):
    try:
        ...
        query = ('(({column} < {lower}) or ({column} > {upper}))').format(column=column, lower=json_float(iqr_lower), upper=json_float(iqr_upper))
