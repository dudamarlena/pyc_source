...
# [GCC 8.4.0]
# Embedded file name: D:\django\hris\adminlte\templatetags\menu.py
# Compiled at: 2017-05-30 03:08:10
...
class _Menu:
    ...
    def add(self, label, link='', icon='', id='', parent=''):
        ...
        if parent != '':
            ...
            self.parents.append({id: {'label': label, 
                  'link': link, 
                  'icon': icon, 
                  'childs': []}})
    ...
    def render(self, context, menus={}):
        ...
        for group in menus:
            ...
            if group[key]['icon'] != '':
                if re.match('\\<([a-z]*)\\b[^\\>]*\\>(.*?)\\<\\/\\1\\>', group[key]['icon']):
                    icon = group[key]['icon']
                else:
            ...
            if app['app_label'] in self.exclude:
                ...
                for model in app['models']:
                    if app['app_label'] + '.' + model['object_name'] in self.exclude:
                        ...
                        if model['object_name'].title() in self.models_icon and self.models_icon[model['object_name'].title()] != '':
                            if re.match('\\<([a-z]*)\\b[^\\>]*\\>(.*?)\\<\\/\\1\\>', self.models_icon[model['object_name'].title()]):
    ...
    def get_model_icon(self, context):
        icon = '<i class="fa fa-circle-o"></i>'
        if context['model']['object_name'].title() in self.models_icon and self.models_icon[context['model']['object_name'].title()] != '':
            if re.match('\\<([a-z]*)\\b[^\\>]*\\>(.*?)\\<\\/\\1\\>', self.models_icon[context['model']['object_name'].title()]):
