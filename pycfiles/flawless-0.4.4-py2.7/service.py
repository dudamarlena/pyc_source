...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/flawless/server/service.py
# Compiled at: 2018-03-20 21:15:42
...
import logging, os, os.path, re, smtplib, subprocess, sys, threading, time, traceback, urllib
try:
    import urllib.parse
...
def code_identifier_equality(self, other):
    if self.filename != other.filename:
        ...
        if self_fragment and other_fragment and self_fragment not in other_fragment and other_fragment not in self_fragment:
            return False
        return True
...
class FlawlessServiceBaseClass(object):
    ...
    def __init__(self, storage_factory=DiskStorage, thread_cls=threading.Thread, open_process_func=subprocess.Popen, smtp_client_cls=smtplib.SMTP, time_func=time.time):
        self.__dict__.update({k:v for k, v in iteritems(locals()) if k != 'self' if k != 'self'})
        self.extract_base_path_patterns = [ re.compile('^.*/%s/?(.*)$' % d) for d in config.report_runtime_package_directory_names
                                          ]
    ...
    def _read_whitelist_configs(self):
        ...
        config_storage.open()
        self.building_blocks = self._parse_whitelist(config_storage['building_blocks'] or api_ttypes.CodeIdentifierList())
        self.third_party_whitelist = self._parse_whitelist(config_storage['third_party_whitelist'] or api_ttypes.CodeIdentifierList())
        self.known_errors = self._parse_whitelist(config_storage['known_errors'] or api_ttypes.KnownErrorList())
        self.ignored_exceptions = set((config_storage['ignored_exceptions'] or api_ttypes.IgnoredExceptionList()).exceptions)
        self.watch_all_errors, self.watch_only_if_blamed = self._parse_watchers_file(config_storage['watch_list'] or api_ttypes.WatchList())
        self.disownerships = self._parse_disownership(config_storage['disownership_list'] or api_ttypes.FileDisownershipList())
    ...
    def _parse_watchers_file(self, watch_list):
        ...
        all_error_tree.set_accumulator(accumulator_intializer=list(), accumulator_func=lambda x, y: x + y if y else x)
        return (
         all_error_tree, blame_only_tree)
    ...
    def _parse_disownership(self, disownership_list):
        ...
        disownership_tree.set_accumulator(accumulator_intializer=list(), accumulator_func=lambda x, y: x + y if y else x)
    ...
    def _convert_epoch_ms(self, cls, epoch_ms=None):
        ...
        return cls.fromtimestamp(epoch_ms / 1000.0)
    ...
    def _blame_line(self, traceback):
        ...
        for stack_line in traceback:
            ...
            if line_type in [api_ttypes.LineType.DEFAULT, api_ttypes.LineType.KNOWN_ERROR]:
        ...
        return (
         key, blamed_entry, email_recipients, False)
    ...
    def _get_line_type(self, line):
        ...
        if not filepath:
            ...
            if not self._matches_filepath_pattern(filepath):
                return api_ttypes.LineType.IGNORED_FILEPATH
            if entry in self.building_blocks[filepath]:
                return api_ttypes.LineType.BUILDING_BLOCK
            if entry in self.known_errors[filepath]:
                return api_ttypes.LineType.KNOWN_ERROR
            if self.raise_exception_pattern.match(line.text):
    ...
    def _format_traceback(self, err_info, append_locals=True, show_full_stack=False, include_err_info=False, linebreak='<br />', spacer='&nbsp;', start_bold='<strong>', end_bold='</strong>', escape_func=cgi.escape):
        request = err_info.last_error_data
        parts = []
        if include_err_info:
            parts.append(('{b}Current Exception Count:{xb} {count}{lb}').format(b=start_bold, xb=end_bold, count=err_info.error_count, lb=linebreak))
            parts.append(('{b}Last Occurred:{xb} {last_occurrence}{lb}').format(b=start_bold, xb=end_bold, last_occurrence=err_info.last_occurrence, lb=linebreak))
            parts.append(('{b}Last Occurrence Logged From:{xb} {hostname}{lb}').format(b=start_bold, xb=end_bold, hostname=request.hostname, lb=linebreak))
        if request.exception_type:
            parts.append(('{b}Exception Type:{xb} {type}{lb}').format(b=start_bold, xb=end_bold, type=request.exception_type, lb=linebreak))
        parts.append(('{b}Traceback (most recent call last):{xb}').format(b=start_bold, xb=end_bold))
        formatted_stack = [ ('{sp}{sp}File "{filename}", line {line}, in {function}{lb}{sp}{sp}{sp}{sp}{code}').format(sp=spacer, lb=linebreak, filename=l.filename, line=l.line_number, function=l.function_name, code=escape_func(l.text)) for l in request.traceback
                          ]
        ...
        frames_to_show = [ l for l in request.traceback if l.frame_locals is not None and (self._get_line_type(l) in types_to_show or show_full_stack)
                         ]
        for l in frames_to_show:
            line_info = ('{sp}{sp}{b}File "{filename}", line {line}, in {function}{xb}').format(sp=spacer, filename=l.filename, line=l.line_number, function=l.function_name, b=start_bold, xb=end_bold)
            local_vals = [ ('{sp}{sp}{sp}{sp}{name}={value}').format(sp=spacer, name=name, value=escape_func(decode(value))) for name, value in sorted(l.frame_locals.items())
                         ]
        ...
        if request.additional_info:
            ...
            parts.append(escape_func(decode(request.additional_info)).replace('\n', linebreak))
        return linebreak.join(parts)


class FlawlessThriftServiceHandler(FlawlessServiceBaseClass):
    """Handler for Thrift server API"""
    VALIDATE_EMAIL_PATTERN = re.compile('^[A-Za-z0-9.%+\\-_]+@(?:(?:[a-zA-Z0-9]+-?)*[a-zA-Z0-9]\\.)+[A-Za-z]{2,4}$')
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.emailed_errors_cache = ExpiringLRUCache(size=1000, expiration_seconds=300)
    ...
    def _sendmail(self, to_addresses, subject, body):
        ...
        if invalid_addresses:
            ...
            self._handle_flawless_issue('Invalid email address found. Not sending to: %s' % (', ').join(invalid_addresses), log_func=log.warning)
        ...
        if request.exception_type and request.exception_type in self.ignored_exceptions:
            ...
            if not key:
                if not was_whitelisted:
                    ...
                    line_types = [ ('%s:%d' % (stack_line.filename, stack_line.line_number), name_map.get(self._get_line_type(stack_line))) for stack_line in request.traceback
                                 ]
            ...
            if key not in self.errors_seen:
                ...
                err_info = api_ttypes.ErrorInfo(error_count=1, developer_email=dev_email or 'unknown', date=mod_time, email_sent=False, last_occurrence=cur_time, is_known_error=bool(known_entry), last_error_data=request)
                self.errors_seen[key] = err_info
                log.info('Error %s caused by %s on %s' % (str(key), dev_email, mod_time))
                if not dev_email:
                    self._handle_flawless_issue('Unable to do blame for %s from %s. You may want to consider setting only_blame_filepaths_matching in your flawless.cfg ' % (
            ...
            if blamed_entry not in self.known_errors[blamed_entry.filename]:
                if not err_info.email_sent and err_info.date >= config.report_only_after_minimum_date and err_info.error_count >= (request.error_threshold or config.report_error_threshold):
                    send_email = True
            else:
                known_entry = self._get_entry(blamed_entry, self.known_errors)
                if known_entry.min_alert_threshold and err_info.error_count >= known_entry.min_alert_threshold and not err_info.email_sent:
                    send_email = True
                if known_entry.alert_every_n_occurrences and err_info.error_count % known_entry.alert_every_n_occurrences == 0:
                    send_email = True
                if known_entry.max_alert_threshold is not None and err_info.error_count > known_entry.max_alert_threshold:
                    send_email = False
            if send_email:
    ...
    def _send_error_email(self, request, err_key, err_info, blamed_entry, known_entry, email_recipients):
        ...
        email_body.append("<br /><br /><a href='%s/view_traceback?%s'>View most recent occurrence</a>" % (
         config.hostname,
         urlencode(dict(filename=err_key.filename, function_name=err_key.function_name, text=err_key.text, line_number=err_key.line_number, timestamp=self._epoch_ms() / 1000))))
        email_body.append("<br /><a href='%s#%s'>View all errors assigned to you</a>" % (config.hostname, dev_email))
        email_body.append("<br /><a href='%s/add_known_error?%s'>Add to whitelist</a>" % (
         config.hostname,
         urlencode(dict(filename=err_key.filename, function_name=err_key.function_name, code_fragment=err_key.text))))
        ...
        try:
            self._sendmail(to_addresses=email_recipients, subject='Error on %s in %s' % (request.hostname, err_key.filename), body=('<br />').join([ s for s in email_body if s ]))
            err_info.email_sent = True
