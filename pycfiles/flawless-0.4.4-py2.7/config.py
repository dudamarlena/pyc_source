...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/flawless/lib/config/config.py
# Compiled at: 2017-12-21 14:20:54
import collections
try:
    import configparser
except ImportError:
...
OPTIONS = [
 ...
 FlawlessOption('hostname', 'localhost', str, 'The host:port to connect to for the flawless backend service. This is used for generating URLs in the emails that are sent out'),
 FlawlessOption('email_domain_name', 'example.com', str, 'Domain name used to send emails'),
 FlawlessOption('ignore_vcs_email_domain', False, bool, 'Version control systems sometimes report bogus email domains, set this ifthe email domain from the version control system should be ignored and ifemail_domain_name should be used instead'),
 FlawlessOption('default_contact', None, str, 'The default email address to contact for issues when Flawless is unable to determine who to email'),
 ...
 FlawlessOption('report_runtime_package_directory_names', 'site-packages', lambda s: [ e.strip() for e in s.split(',') if e.strip() ], 'The site-packages directory name'),
 FlawlessOption('report_only_after_minimum_date', '1970-1-1', str, 'Ignore errors occurring in old code that was last modified prior to this date'),
 FlawlessOption('report_error_threshold', 3, int, 'Number of times an error should occur before generating an alert'),
 FlawlessOption('only_blame_filepaths_matching', '^(?!.*\\.egg).*$', lambda s: [ e.strip() for e in s.split(',') if e.strip() ], 'When assigning blame to a file, you can explicity only blame files whose filepath matches any pattern on this CSV list'),
 FlawlessOption('max_concurrent_git_blames', 10, int, 'The maximum number of git blames that can run at the same time'),
 ...
 FlawlessOption('repo_url', None, str, 'URL of the remote repo, including username and password credentials'),
 FlawlessOption('repo_branch_pattern', None, str, 'Regular expressions to identify release branches'),
 FlawlessOption('flawless_hostports', None, lambda s: [ e.strip() for e in s.split(',') if e.strip() ], 'Host:port of the flawless backend'),
...
RUNTIME_CONFIG = FlawlessConfig(dict((o.name, o.type(o.default) if o.default is not None else None) for o in OPTIONS))
