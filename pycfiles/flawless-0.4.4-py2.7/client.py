...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/flawless/client/client.py
# Compiled at: 2019-03-21 17:55:42
import functools, hashlib, linecache, logging, math, os.path, random, re, socket, sys, time, traceback, warnings
...
def _get_backend_host():
    ...
    if HOSTPORT_INFO:
        return random.choice(HOSTPORT_INFO)
    else:
        return
...
def _wrap_function_with_error_decorator(func, save_current_stack_trace=True, reraise_exception=True, error_threshold=None):
    preceding_stack = []
    ...
    def wrapped_func_with_error_reporting(*args, **kwargs):
        ...
        try:
            ...
            record_error(hostname=hostname, exc_info=(
             type, value, sys_traceback), preceding_stack=preceding_stack, error_threshold=error_threshold)
            if reraise_exception:
