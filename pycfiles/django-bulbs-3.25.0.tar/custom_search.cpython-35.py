...
# [GCC 8.4.0]
# Embedded file name: /webapp/bulbs/content/custom_search.py
...
def custom_search_model(model, query, preview=False, published=False, id_field='id', sort_pinned=True, field_map={}):
    ...
    if published:
        ...
        if pinned_ids and sort_pinned:
            pinned_query = es_query.FunctionScore(boost_mode='multiply', functions=[
    ...
    if values:
        ...
        if operation == 'any':
            ...
            if operation == 'none':
                ...
                raise ValueError("ES conditions must be one of the following values: ['all', 'any', 'none']")
...
def date_range_filter(range_name):
    ...
    filter_days = list(filter(lambda time: time['label'] == range_name, settings.CUSTOM_SEARCH_TIME_PERIODS))
