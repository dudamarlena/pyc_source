...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\typepad2blogger\mt.py
# Compiled at: 2008-05-16 00:19:09
...
class stringdata(MoveableType):
    ...
    def __init__(self, input=None):
        if isinstance(input, basestring):
            ...
            raise TypeError("input must be a 'basestring' type object: " + 'got %s type' % str(type(input))[6:-1])
        ...
        if isinstance(input, basestring):
            ...
            for line in lines:
                if 'EXCERPT' in self.keys():
                    ...
                    continue
                elif line.startswith('TITLE:') or line.startswith('URL:') or line.startswith('IP:') or line.startswith('DATE:') or line.startswith('BLOG NAME:'):
                    (k, v) = line.split(':', 1)
                    self[k] = v.strip()
                else:
                    self['EXCERPT'] = [
        ...
        if isinstance(input, basestring):
            ...
            for line in lines:
                if 'BODY' in self.keys():
                    ...
                    continue
                elif line.startswith('AUTHOR:') or line.startswith('EMAIL:') or line.startswith('URL:') or line.startswith('IP:') or line.startswith('DATE:'):
                    (k, v) = line.split(':', 1)
                    self[k] = v.strip()
                else:
                    self['BODY'] = [
...
class metadata(MoveableType, dict):
    ...
    def __init__(self, input=None):
        ...
        if isinstance(input, basestring):
            lines = input.strip('\n-').replace('\r\n', '\n').split('\n')
            for line in lines:
                if line.startswith('AUTHOR:') or line.startswith('TITLE:') or line.startswith('DATE:') or line.startswith('PRIMARY CATEGORY:') or line.startswith('STATUS:'):
                    (k, v) = line.split(':', 1)
                    self[k] = v.strip()
                ...
                elif line.startswith('ALLOW COMMENTS:') or line.startswith('ALLOW PINGS:') or line.startswith('CONVERT BREAKS:') or line.startswith('NO ENTRY:'):
                    (k, v) = line.split(':', 1)
                    self[k] = bool(v.strip())
                elif line.strip() == '':
                    continue
...
class blog(MoveableType, list):
    ...
    def read(self, input, blocksize=8192):
        ...
        if isinstance(input, basestring):
            ...
            raise TypeError("input requires a 'file' type object or a " + "'basestring' type object: " + 'got %s type' % str(type(input))[6:-1])
        fh.seek(0)
