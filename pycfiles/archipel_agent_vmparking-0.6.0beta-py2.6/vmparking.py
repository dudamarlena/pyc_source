...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/archipelagentvmparking/vmparking.py
# Compiled at: 2013-03-20 13:50:16
import datetime, os, random, shutil, sqlite3, string, xmpp
...
class TNVMParking(TNArchipelPlugin):
    ...
    def __init__(self, configuration, entity, entry_point_group):
        ...
        if isinstance(self.entity, TNArchipelHypervisor):
            registrar_items = [
             {'commands': ['park'], 'parameters': [{'name': 'identifiers', 'description': 'the UUIDs of the VM to park, separated with comas, with no space'}], 'method': self.message_park_hypervisor, 
                'permissions': [
                              'vmparking_park'], 
                'description': 'Park the virtual machine with the given UUIDs'},
             {'commands': ['unpark'], 'parameters': [{'name': 'identifiers', 'description': 'UUIDs of the virtual machines or parking tickets, separated by comas, with no space'}], 'method': self.message_unpark, 
                'permissions': [
                              'vmparking_unpark'], 
                'description': 'Unpark the virtual machine parked with the given identifier'},
             {'commands': ['park list'], 'parameters': [], 'method': self.message_list, 
                'permissions': [
                              'vmparking_list'], 
                'description': 'List all parked virtual machines'}]
        elif isinstance(self.entity, TNArchipelVirtualMachine):
            registrar_items = [
             {'commands': ['park'], 'parameters': [], 'method': self.message_park_vm, 
                'permissions': [
                              'vmparking_park'], 
    ...
    def iq_unpark(self, iq):
        ...
        try:
            ...
            for item in items:
                ...
                autostart = False
                if item.getAttr('start') and item.getAttr('start').lower() in ('yes',
                                                                               'y',
                                                                               'true',
                                                                               '1'):
                    autostart = True
