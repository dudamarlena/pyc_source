...
# Embedded file name: /home/bcsaller/Projects/chimera/build/lib.linux-i686-2.5/chimera/colors.py
...
def parseColorToRGBA(color, masterAlpha=1.0):
    ...
    if isinstance(color, tuple):
        if len(color) == 3:
            color = color + (masterAlpha,)
        if len(color) != 4:
            raise ValueError(color)
        return color
    if color.startswith('#'):
        return colorStringToDoubles(color)
    else:
...
def colorStringToTuple(color):
    ...
    c = int('0x' + color[:6], 16)
    if len(color) == 8:
        a = int('0x' + color[6:8], 16)
    else:
        a = 255
    color = (
     c >> 16, c >> 8 & 255, c & 255, a)
    return color
