...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyculiarity/detect_ts.py
# Compiled at: 2018-08-28 00:38:38
...
def detect_ts(df, max_anoms=0.1, direction='pos', alpha=0.05, only_last=None, threshold=None, e_value=False, longterm=False, piecewise_median_period_weeks=2, plot=False, y_log=False, xlabel='', ylabel='count', title=None, verbose=False):
    """
    ...
    if not isinstance(df, DataFrame):
        ...
        if len(df.columns) != 2 or not df.iloc[:, 1].map(np.isreal).all():
            raise ValueError('data must be a 2 column data.frame, with thefirst column being a set of timestamps, and the second coloumn being numeric values.')
        if df.dtypes[0].type is not np.datetime64 and df.dtypes[0].type is not np.int64:
        ...
        if only_last and only_last not in ('day', 'hr'):
        ...
        for i in range(len(all_data)):
            directions = {'pos': Direction(True, True), 
