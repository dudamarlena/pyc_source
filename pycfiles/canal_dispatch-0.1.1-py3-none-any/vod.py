...
# Embedded file name: build/bdist.macosx-10.5-i386/egg/canalweb/vod.py
# Compiled at: 2009-02-12 16:42:44
__doc__ = 'VOD-related operations.'
import cookielib, datetime, logging, os, re, urllib2
...
class Downloader(object):
    ...
    _SHOW_URL = {'zapping': '/c-infos-documentaires/pid1830-c-zapping.html', 
       'guignoles': '/c-humour/pid1784-c-les-guignols.html', 
       'magzin': '/c-humour/pid1787-c-groland.html'}
    _CONTENT_RE = re.compile('CONTENT_ID.*=\\s*["\\\'](\\d+)')
    _CFG_URL = 'http://www.canalplus.fr/flash/xml/module/embed-video-player/embed-video-player.php?video_id=%(video_id)s&pid=%(pid)d'
    ...
    def _fetch_html(self, show):
        ...
        txdata = None
        txheaders = {'User-Agent': 'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US; rv:1.8.1.7) Gecko/20070914 Firefox/2.0.0.7'}
    ...
    def _video_url(self, data, show, vid):
        ...
        match = re.search('_(\\d{6})_', url.split('/')[(-1)])
        if match:
            info = match.group(1)
            stamp = datetime.datetime(year=2000 + int(info[:2]), month=int(info[2:4]), day=int(info[4:6]))
        else:
            stamp = None
