...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyf/components/producers/webextractor/producer.py
# Compiled at: 2010-10-01 03:33:54
"""
...
import codecs, decimal, logging
from pyf.componentized.configuration.keys import SimpleKey, RepeatedKey, CompoundKey
...
class PyFSpider(BaseSpider):
    ...
    def parse(self, response):
        ...
        for item in items:
            yield Packet(dict([ (key, item.select(getter).extract()) for (key, getter) in self.field_selectors.iteritems()
                              ]))
...
class WebExtractor(Producer):
    ...
    name = 'webextractor'
    configuration = [
     RepeatedKey('start_urls', 'start_url', default=[
      ''], content=SimpleKey('start_url', help_text='ex. "http://www.example.com/listing"')),
     SimpleKey('item_selector', label='Individual item XPath', help_text="ex. '//ul[1]/li'"),
     RepeatedKey('fields', 'field', default=[
      dict(name='', xpath='')], content=CompoundKey('field', text_value='xpath', attributes={'name': 'name'}, fields=[
      InputField('name', label='Attribute', help_text='Target attribute'),
      InputField('xpath', label='Field XPath', help_text='Path to search (ex. "p/*/text()")')])),
     SimpleKey('link_selector', label='Other pages urls xpath', help_text='ex "p[@id=\'links\']/a/@href" (optionnal)'),
     SimpleKey('url_base', label='Base url for links', help_text='ex. "http://wwww.example.com/"'),
     SimpleKey('page_limit', label='Limit to N Pages', default='10')]
    ...
    def DynamicSpider(self):
        return self.__create_spider_class('DynamicSpider', self.get_config_key('start_urls'), self.get_config_key('item_selector'), dict([ (field.get('name'), field.get('xpath')) for field in self.get_config_key('fields')
                                                                                                                                         ]), self.get_config_key('link_selector') or None, self.get_config_key('url_base'))

    def __create_spider_class(self, name, start_urls, item_selector, field_selectors, link_selector, url_base):
        return type.__new__(type, name.title().replace(' ', ''), (
         PyFSpider,), dict(name=name, start_urls=start_urls, item_selector=item_selector, field_selectors=field_selectors, link_selector=link_selector, url_base=url_base))
