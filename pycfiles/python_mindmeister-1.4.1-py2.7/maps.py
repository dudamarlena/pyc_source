...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mindmeister/maps.py
# Compiled at: 2012-05-17 15:36:33
"""
...
def getList(token):
    ...
    """
    rawResult = common.performRequest('rest', token, method='mm.maps.getList')
...
def add(token):
    ...
    """
    rawResult = common.performRequest('rest', token, method='mm.maps.add')
    root = parser.parse(rawResult)
    if root.attrib['stat'] != 'ok':
...
def delete(token, map_id):
    ...
    """
    rawResult = common.performRequest('rest', token, method='mm.maps.delete', map_id=map_id)
    root = parser.parse(rawResult)
    if root.attrib['stat'] != 'ok':
...
def duplicate(token, map_id):
    ...
    """
    rawResult = common.performRequest('rest', token, method='mm.maps.duplicate', map_id=map_id)
    root = parser.parse(rawResult)
    if root.attrib['stat'] != 'ok':
...
def getMap(token, map_id, **args):
    ...
    """
    rawResult = common.performRequest('rest', token, method='mm.maps.getMap', map_id=map_id, **args)
    root = parser.parse(rawResult)
    if root.attrib['stat'] != 'ok':
...
def undo(token, map_id):
    ...
    """
    rawResult = common.performRequest('rest', token, method='mm.maps.undo', map_id=map_id)
    root = parser.parse(rawResult)
    if root.attrib['stat'] != 'ok':
...
def redo(token, map_id):
    ...
    """
    rawResult = common.performRequest('rest', token, method='mm.maps.redo', map_id=map_id)
    root = parser.parse(rawResult)
    if root.attrib['stat'] != 'ok':
...
def insertGeistesblitz(token, title):
    ...
    """
    rawResult = common.performRequest('rest', token, method='mm.maps.insertGeistesblitz', title=title)
    root = parser.parse(rawResult)
    if root.attrib['stat'] != 'ok':
...
def history(token, map_id, **args):
    ...
    """
    rawResult = common.performRequest('rest', token, method='mm.maps.history', map_id=map_id, **args)
    root = parser.parse(rawResult)
    if root.attrib['stat'] != 'ok':
