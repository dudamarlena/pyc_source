...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/zoocli/commands.py
# Compiled at: 2015-06-15 16:40:21
# Size of source mod 2**32: 6758 bytes
import os, re, atexit, tempfile
...
class ZooCommands(Commands):
    ...
    def stat(self, path=None):
        ...
        lines = [
         'Created: {created} by session id: {created_id}',
         ...
         'pzxid: {pzxid}']
        return '\n'.join(lines).format(version=stat.version, aversion=stat.acl_version, cversion=stat.children_version, created=timestamp_to_date(stat.created), created_id=stat.creation_transaction_id, modified=timestamp_to_date(stat.last_modified), modified_id=stat.last_modified_transaction_id, owner=stat.owner_session_id, data_length=stat.data_length, children=stat.children_count, pzxid=stat.pzxid)
    ...
    def getacl(self, path=None):
        ...
        for i, acl in enumerate(current_acl):
            lines.append('{}: {} {} ({})'.format(i, acl.id.scheme, acl.id.id, ', '.join(acl.acl_list)))
    ...
    def find(self, path=None, name_filter=None, mindepth=None, maxdepth=None):
        ...
        def filter_matches(name):
            return not name_filter or re.search('^{}$'.format(name_filter), name)
