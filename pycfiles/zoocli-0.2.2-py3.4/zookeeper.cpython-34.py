...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/zoocli/zookeeper.py
# Compiled at: 2015-05-27 15:39:04
...
class ZooKeeper(object):
    ...
    def create(self, path, data=None, ephemeral=False, sequence=False, makepath=False):
        if data:
            ...
            data = b''
        try:
            self._zookeeper.create(path, value=data, ephemeral=ephemeral, sequence=sequence, makepath=makepath)
        except NoNodeError:
            raise ZooKeeperException('No such node: {}'.format(path))
