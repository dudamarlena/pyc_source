...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/yedis/client.py
# Compiled at: 2018-10-31 18:35:23
from __future__ import with_statement
from itertools import chain
import datetime, sys, warnings, time, threading, time as mod_time, hashlib
...
def parse_georadius_generic(response, **options):
    ...
    return [ list(map(lambda fv: fv[0](fv[1]), zip(f, r)))
...
class StrictRedis(object):
    ...
    def tsadd(self, name, *args):
        ...
        if args:
            if len(args) % 2 != 0:
                raise RedisError('TSADD requires an equal number of values and scores')
            pieces.extend(args)
