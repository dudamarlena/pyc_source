...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/yedis/connection.py
...
from redis._compat import b, xrange, imap, byte_to_chr, unicode, bytes, long, BytesIO, nativestr, basestring, iteritems, LifoQueue, Empty, Full, urlparse, parse_qs, recv, recv_into, select, unquote
...
if HIREDIS_AVAILABLE:
    ...
    if not HIREDIS_SUPPORTS_BYTE_BUFFER or sys.version_info[0] == 2 and sys.version_info[1] < 7:
...
class Connection(object):
    ...
    def pack_command(self, *args):
        ...
        command = args[0]
        if ' ' in command:
            args = tuple([ Token.get_token(s) for s in command.split()
                         ]) + args[1:]
        else:
            args = (
             Token.get_token(command),) + args[1:]
        buff = SYM_EMPTY.join((
...
class ConnectionPool(object):
    ...
    def get_encoder(self):
        ...
        return Encoder(encoding=kwargs.get('encoding', 'utf-8'), encoding_errors=kwargs.get('encoding_errors', 'strict'), decode_responses=kwargs.get('decode_responses', False))
