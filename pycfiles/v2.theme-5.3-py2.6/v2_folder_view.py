...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/v2/theme/browser/v2_folder_view.py
...
class V2FolderView(BrowserView):
    ...
    def isVideo(self, item):
        if hasattr(item, 'getObject'):
            ...
            itemObj = item
        result = IFlowPlayable.providedBy(itemObj)
        if result:
            print 'Item is a video!'
        elif itemObj.id[-3:] == 'mov' or itemObj.id[-3:] == 'avi' or itemObj.id[-3:] == 'mp4' or itemObj.id[-3:] == 'm4v':
            result = True
        return result
    ...
    def audio_only(self, item):
        if hasattr(item, 'getObject'):
            ...
            itemObj = item
        result = IAudio.providedBy(itemObj)
        return result