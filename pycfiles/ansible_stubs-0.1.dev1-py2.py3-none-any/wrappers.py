...
# Embedded file name: build/bdist.macosx-10.13-intel/egg/ansible/module_utils/selvpc_utils/wrappers.py
        ...
        def inner(*args, **kwargs):
            ...
            if result:
                if isinstance(result, dict):
                    if 'added' in result:
                        params.update({make_plural(object_type): [ el._info for el in result['added'] ]})
                    if 'deleted' in result:
                        params.update({make_plural(object_type) + '_deleted': result['deleted']})
                elif isinstance(result, list):
                    params.update({make_plural(object_type): [ el._info for el in result ]})
                else:
