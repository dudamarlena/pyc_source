...
# Embedded file name: build/bdist.macosx-10.13-intel/egg/ansible/module_utils/selvpc_utils/common.py
# Compiled at: 2018-02-16 17:52:51
import ipaddress
...
def _check_quotas_changes(client, after_quotas, project_id):
    ...
    for key in after_quotas:
        for quota in after_quotas[key]:
            item = [ item for item in before_quotas_json[key] if item['region'] == quota['region'] and item['zone'] == quota['zone'] and item['value'] == quota['value']
                   ]
            if not item:
                return True
...
def _check_project_roles(client, roles):
    ...
    try:
        for role in roles:
            if role not in [ r._info for r in client.roles.get_project_roles(role['project_id'])
                           ]:
                to_add.append(role)
...
def compare_existed_and_needed_objects(before, after, force):
    ...
    """
    possible_task = True
    to_create = dict((key, before.get(key)) for key in before if key in after)
    to_delete = {}
    for n_key in after:
        if possible_task:
            if n_key not in before:
                ...
                if after_quantity == before_quantity:
                    ...
                    to_create.pop(n_key)
                    if not force:
                        if down >= after_quantity - active and after_quantity >= active:
                            to_delete.update({n_key: down - (after_quantity - active)})
                        else:
                            possible_task = False
                    else:
                        to_delete.update({n_key: before_quantity - after_quantity})
                else:
...
def check_project_id(func):
    ...
    def inner(*args, **kwargs):
        ...
        if show_list:
            return func(module, cli, project_id, project_name, show_list=show_list, *args[4:])
        if not project_id:
...
def clear_quotas(quotas):
    to_clear = {'quotas': {}}
    for item in quotas:
        to_clear['quotas'].update({item['resource']: {'region': item['region'], 'value': 0}})

    return to_clear


def abort_particle_response_task(module, client, resp, project_id=None, is_quotas=False):
    ...
    res_json = {'error': '207 Multi-status', 'details': resp.get_fail_info()}
