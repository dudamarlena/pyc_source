...
# [GCC 8.4.0]
# Embedded file name: vfclust/vfclust.py
# Compiled at: 2014-07-08 11:31:20
"""
...
import os, re, subprocess, csv, argparse, sys, cPickle as pickle
...
class ParsedResponse():
    ...
    def __init__(self, response_type, letter_or_category, quiet=False, cmudict=None, english_words=None, lemmas=None, names=None, permissible_words=None):
    ...
    def generate_phonetic_representation(self, word):
        ...
        with NamedTemporaryFile() as (temp_file):
            ...
            t2pargs = [
             os.path.abspath(os.path.join(os.path.dirname(__file__), 't2p/t2p')),
             '-transcribe', os.path.join(data_path, 'cmudict.0.7a.tree'),
             temp_file.name]
            temp_file.seek(0)
            output, error = subprocess.Popen(t2pargs, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
            output = output.split()
    ...
    def clean(self):
        ...
        while current_index < len(self.unit_list):
            ...
            if self.type == 'PHONETIC':
                test = word.startswith(self.letter_or_category) and not word.endswith('-') and '_' not in word and word.lower() in self.english_words
        ...
        while current_index < len(self.unit_list) - 1:
            if stemmer.stem(self.unit_list[current_index].text) == stemmer.stem(self.unit_list[(current_index + 1)].text):
...
class VFClustEngine(object):
    ...
    def __init__(self, response_category, response_file_path, target_file_path=None, collection_types=[
 'cluster', 'chain'], similarity_measures=[
 'phone', 'biphone', 'lsa'], clustering_parameter=91, quiet=False, similarity_file=None, threshold=None):
    ...
    """
        self.valid_semantic_categories = [
         'animals']
        ...
        if self.response_format not in ('csv', 'TextGrid'):
            raise VFClustException('Currently, VF-Clust only accepts responses in ' + 'comma-separated (csv) or Praat TextGrid formats. ' + 'Your response format is ' + self.response_format)
        if self.response_format == 'csv':
            ...
            self.raw_response = [ re.sub(' ', '', word) for word in self.raw_response
                                ]
        ...
        self.parsed_response = ParsedResponse(self.type, quiet=self.quiet, letter_or_category=response_category, cmudict=self.cmudict, english_words=self.english_words, lemmas=self.lemmas, names=self.names, permissible_words=self.permissible_words)
        if self.response_format == 'csv':
    ...
    def load_lsa_information(self):
        ...
        if not 49 < int(self.clustering_parameter) < 101:
            raise Exception('Only LSA dimensionalities in the range 50-100' + ' are supported.')
        if not self.quiet:
            print 'Loading LSA term vectors...'
        with open(os.path.join(data_path, self.category + '_' + os.path.join('term_vector_dictionaries', 'term_vectors_dict' + str(self.clustering_parameter) + '_cpickle.dat')), 'rb') as (infile):
    ...
    def get_raw_counts(self):
        ...
        for unit in self.parsed_response:
            ...
            test = False
            if self.type == 'PHONETIC':
                test = word.startswith(self.letter) and 'T_' not in word and 'E_' not in word and '!' not in word and 'FILLEDPAUSE_' not in word and not word.endswith('-') and word.lower() in self.english_words
            elif self.type == 'SEMANTIC':
                test = word in self.permissible_words
            if test:
        ...
        self.measures['COUNT_unique_permissible_words'] = self.measures['COUNT_permissible_words'] - self.measures['COUNT_exact_repetitions'] - self.measures['COUNT_stem_repetitions']
        if not self.quiet:
            print
            print 'Counts:'
    ...
    def compute_similarity_score(self, unit1, unit2):
        ...
        if self.type == 'PHONETIC':
            ...
            if self.current_similarity_measure == 'biphone':
            ...
            if self.current_similarity_measure == 'custom':
                try:
    ...
    def compute_similarity_scores(self):
        ...
        if not self.quiet:
            ...
            table = [('Word 1', 'Word 2', 'Score')] + [ (self.parsed_response[i].text, self.parsed_response[(i + 1)].text, ('{0:.3f}').format(round(self.similarity_scores[i], 2))) for i in range(len(self.parsed_response) - 1)
                                                      ]
            print_table(table)
    ...
    def compute_collections(self):
        ...
        if self.type == 'PHONETIC':
            if self.current_similarity_measure == 'phone':
                phonetic_similarity_thresholds = {'a': 0.222222222222, 'b': 0.3, 'c': 0.2857142857134, 
                   'd': 0.3, 
                   'e': 0.25, 
                   'f': 0.333333333333, 
            ...
            if self.type == 'SEMANTIC':
                if self.current_similarity_measure == 'lsa':
                    if self.category == 'animals':
                        thresholds = {'50': 0.229306542684, '51': 0.225946872032, '52': 0.224032352058, '53': 0.214750475853, '54': 0.210178113675, 
                           '55': 0.209214667474, '56': 0.204037629443, 
                           '57': 0.203801260742, '58': 0.203261303516, 
                           '59': 0.20351336453, '60': 0.19834361416, 
                           '61': 0.19752806853, '62': 0.191322450624, 
                           '63': 0.194312302459, '64': 0.188165419858, 
                           '65': 0.184645454503, '66': 0.184781367314, 
                           '67': 0.178950849271, '68': 0.177441756062, 
                           '69': 0.176398889963, '70': 0.175374032744, 
                           '71': 0.172350911698, '72': 0.171158753965, 
                           '73': 0.172621416351, '74': 0.165803036975, 
                           '75': 0.164168434928, '76': 0.166395146381, 
                           '77': 0.162961462955, '78': 0.161888890545, 
                           '79': 0.160416925579, '80': 0.157132807023, 
                           '81': 0.159653951557, '82': 0.155974588379, 
                           '83': 0.156068321827, '84': 0.149922400199, 
                           '85': 0.151864625954, '86': 0.149766386146, 
                           '87': 0.149423885352, '88': 0.14740916275, 
                           '89': 0.148213369526, '90': 0.141889414227, 
                           '91': 0.140395152983, '92': 0.141251008272, 
                           '93': 0.140135804694, '94': 0.139334834651, 
                           '95': 0.139679588617, '96': 0.135698594642, 
                           '97': 0.135394351192, '98': 0.136194738818, 
                           '99': 0.136671316751, '100': 0.135307208304}
    ...
    def compute_pairwise_similarity_score(self):
        ...
        self.measures['COLLECTION_' + self.current_similarity_measure + '_pairwise_similarity_score_mean'] = get_mean(all_scores) if len(pairs) > 0 else 'na'
    ...
    def compute_collection_measures(self, no_singletons=False):
        ...
        self.measures[prefix + 'size_mean'] = get_mean(collection_sizes_temp) if self.measures[(prefix + 'count')] > 0 else 0
        self.measures[prefix + 'size_max'] = max(collection_sizes_temp) if len(collection_sizes_temp) > 0 else 0
    ...
    def compute_duration_measures(self):
        ...
        if self.response_format == 'TextGrid':
            self.compute_response_vowel_duration('TIMING_')
    ...
    def compute_response_vowel_duration(self, prefix):
        ...
        self.measures[prefix + 'response_vowel_duration_mean'] = get_mean(durations) if len(durations) > 0 else 'na'
        if not self.quiet:
    ...
    def compute_response_continuant_duration(self, prefix):
        ...
        self.measures[prefix + 'response_continuant_duration_mean'] = get_mean(durations) if len(durations) > 0 else 'na'
        if not self.quiet:
    ...
    def compute_between_collection_interval_duration(self, prefix):
        ...
        if not self.quiet:
            ...
            table = [
             (self.current_collection_type + ' 1 (start,end)', 'Interval',
              self.current_collection_type + ' 2 (start,end)')] + [ (str(d1), str(i1), str(d2)) for d1, i1, d2 in zip(durations[:-1], interstices, durations[1:]) ]
            print_table(table)
            print
            print 'Mean ' + self.current_similarity_measure + ' between-' + self.current_collection_type + ' duration',
            print self.measures[(prefix + 'between_collection_interval_duration_mean')]
    ...
    def compute_within_collection_interval_duration(self, prefix):
        ...
        self.measures[prefix + 'within_collection_interval_duration_mean'] = get_mean(interstices) if len(interstices) > 0 else 'na'
        if not self.quiet:
            print 'Mean within-' + self.current_similarity_measure + '-' + self.current_collection_type + ' between-word duration:', self.measures[(prefix + 'within_collection_interval_duration_mean')]
    ...
    def compute_within_collection_vowel_duration(self, prefix, no_singletons=False):
        ...
        self.measures[prefix + 'within_collection_vowel_duration_mean'] = get_mean(durations) if len(durations) > 0 else 'na'
        if not self.quiet:
            if no_singletons:
                print 'Mean within-' + self.current_similarity_measure + '-' + self.current_collection_type + ' vowel duration, excluding singletons:',
                print self.measures[(prefix + 'within_collection_vowel_duration_mean')]
            else:
                print 'Mean within-' + self.current_similarity_measure + '-' + self.current_collection_type + ' vowel duration, including singletons:',
                print self.measures[(prefix + 'within_collection_vowel_duration_mean')]
    ...
    def compute_within_collection_continuant_duration(self, prefix, no_singletons=False):
        ...
        self.measures[prefix + 'within_collection_continuant_duration_mean'] = get_mean(durations) if len(durations) > 0 else 'na'
        if not self.quiet:
            if no_singletons:
                print 'Mean within-' + self.current_similarity_measure + '-' + self.current_collection_type + ' continuant duration, excluding singletons:',
                print self.measures[(prefix + 'within_collection_continuant_duration_mean')]
            else:
                print 'Mean within-' + self.current_similarity_measure + '-' + self.current_collection_type + ' continuant duration, including singletons:',
                print self.measures[(prefix + 'within_collection_continuant_duration_mean')]
    ...
    def print_output(self):
        ...
        if self.target_file:
            with open(self.target_file, 'w') as (outfile):
                header = [
                 'file_id'] + [ self.type + '_' + e for e in self.measures if 'COUNT_' in e ] + [ self.type + '_' + e for e in self.measures if 'COLLECTION_' in e ] + [ self.type + '_' + e for e in self.measures if 'TIMING_' in e ]
                writer = csv.writer(outfile, quoting=csv.QUOTE_MINIMAL)
                writer.writerow(header)
                writer.writerow([self.measures['file_id']] + [ self.measures[('_').join(e.split('_')[1:])] for e in header[1:] ])
...
def get_duration_measures(source_file_path, output_path=None, phonemic=False, semantic=False, quiet=False, similarity_file=None, threshold=None):
    ...
    engine = VFClustEngine(response_category=response_category, response_file_path=args.source_file_path, target_file_path=target_file_path, quiet=args.quiet, similarity_file=args.similarity_file, threshold=args.threshold)
...
def validate_arguments(args):
    ...
    if args.output_path == None:
        ...
        try:
            ...
            print 'Error creating folder for program output. Make sure you have write permissions to the folder you provided. You can change the folder with the -o option.The output directory will be the same as the input directory.'

    if args.semantic:
        args.semantic = args.semantic.lower()
    if args.phonemic:
        args.phonemic = args.phonemic.lower()
    if not (args.semantic or args.phonemic):
        raise VFClustException('You must specify at least one phonemic or semantic test to run using -p or -s, followed by the test type.')
    if args.semantic and args.semantic not in semantic_tests:
        raise VFClustException('Currently only ' + (',').join(semantic_tests) + ' are supported for semantic testing. You provided ' + args.semantic)
    if args.phonemic and args.phonemic not in phonemic_tests:
        raise VFClustException('Currently only ' + (',').join(phonemic_tests) + ' are supported for phonemic testing.  You provided ' + args.phonemic)
...
def main(test=False):
    if test or len(sys.argv) > 1 and sys.argv[1] == 'test':
        ...
        parser.add_argument('source_file_path', help='Full path of textgrid or csv file to parse')
        parser.add_argument('-s', dest='semantic', default=False, action='store', help='Usage: -s animals\n\n                                If included, calculates measures for the given category \n\n                                for the semantic fluency test, i.e. animals, fruits, etc.\n                                ')
        parser.add_argument('-p', dest='phonemic', default=False, action='store', help='Usage: -p f\n\n                                If included, calculates measures for the given category \n\n                                for the phonemic fluency test, i.e. a, f, s, etc.\n                                ')
        parser.add_argument('-o', dest='output_path', default='', help='Where to put output - default is the same directory as the input file working directory.')
        parser.add_argument('-q', dest='quiet', default=False, action='store_true', help='Use to eliminate output (default is print everything to stdout).')
        parser.add_argument('--similarity-file', dest='similarity_file', default=None, help='Usage: --similarity-file /path/to/similarity/file\n\n                        Location of custom word similarity file.\n                        If used, the default "LSA" option is overridden.\n                        You must also include a threshold number with\n                        --threshold X')
        parser.add_argument('--threshold', dest='threshold', default=None, help='Usage: --threshold X, where X is a number.\n                                    A custom threshold is required when including a custom similarity file.')
        args = parser.parse_args()
        get_duration_measures(output_path=args.output_path, phonemic=args.phonemic, semantic=args.semantic, source_file_path=args.source_file_path, quiet=args.quiet, similarity_file=args.similarity_file, threshold=args.threshold)
    return
...
def test_script():
    ...
    results_csv = get_duration_measures(source_file_path=example_csv, output_path=False, phonemic='f')
    results_textgrid = get_duration_measures(source_file_path=example_textgrid, output_path=False, semantic='animals')
    results_textgrid_custom = get_duration_measures(source_file_path=example_textgrid, output_path=False, semantic='animals', similarity_file=os.path.abspath(os.path.join(os.path.dirname(__file__), 'data/similarity/similarity.txt')), threshold=0.5)
