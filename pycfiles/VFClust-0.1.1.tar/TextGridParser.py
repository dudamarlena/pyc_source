...
# [GCC 8.4.0]
# Embedded file name: /Volumes/Data/git/vf-clust/vfclust/TextGridParser.py
# Compiled at: 2014-06-24 13:25:20
import re
...
class TextGrid(object):
    ...
    def __init__(self, textgrid):
        ...
        self.word_intervals = [ interval + '$' for interval in self.word_intervals
                              ]
        self.phone_intervals = textgrid[textgrid.index('item [2]:'):]
        self.phone_intervals = self.phone_intervals[self.phone_intervals.index('intervals [1]'):]
        self.phone_intervals = self.phone_intervals.split('"\n')
        del self.phone_intervals[-1]
        self.phone_intervals = [ interval + '$' for interval in self.phone_intervals
                               ]
    ...
    def parse_phones(self):
        ...
        for i in self.phone_intervals:
            start = float(i[i.index('xmin = ') + 7:i.index('xmin = ') + 12].strip('\t').strip('\n'))
