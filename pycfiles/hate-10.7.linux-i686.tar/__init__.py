...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.6/site-packages/hate/__init__.py
# Compiled at: 2010-09-09 20:28:22
"""
...
"""
import os, sys, vte, gtk.gdk, ConfigParser
...
def getFontSize():
    config = getConfiguration()
    try:
        return int(config.get('main', 'font_size'))
...
def _loadIcon(iconfile):
    return gtk.gdk.pixbuf_new_from_file(os.path.join(os.path.expanduser('~/.hate/images'), iconfile))
...
class Hate(object):
    ...
    def __init__(self, container, titleFunc=nop, exitFunc=nop, command=None):
        term = self.term = vte.Terminal()
        self.exitFunc = exitFunc
        self.command = command
        self.topwidget = tw = gtk.HBox()
        self.term.connect('window-title-changed', self.titleChanged)
        self.term.connect('child-exited', self.exited)
        self.term.connect('increase-font-size', self.increase_font_size)
        self.term.connect('decrease-font-size', self.decrease_font_size)
    ...
    def setFont(self):
        self.term.set_font_from_string('%s %s' % (self.font, self.font_size))

    def increase_font_size(self, *ev):
        self.font_size = self.font_size + 1
        self.setFont()
        return True

    def decrease_font_size(self, *ev):
        self.font_size = max(self.font_size - 1, 4)
        self.setFont()
        return True
    ...
    def doTransparency(self, amount):
        ...
        if self.term.get_screen().is_composited():
            self.term.set_opacity(int(65535 * (1.0 - amount)))
        else:
    ...
    def start(self):
        if self.command:
            self.term.fork_command(self.command[0], self.command)
        else:
    ...
    def geometry_hints(self):
        ...
        return dict(base_width=xpad, base_height=ypad, width_inc=wid, height_inc=hig, min_width=xpad + wid * 4, min_height=ypad + hig * 2)
...
class HateWindow:
    ...
    def __init__(self, command=None):
        ...
        self.hate = Hate(self.vbox, self.changeTitle, self.processEnded, command=command)
        self.win.connect('destroy', self.destroyed)
        self.ico = gtk.gdk.pixbuf_new_from_file(os.path.join(os.path.join(os.path.dirname(__file__)), 'hateicon.svg'))
        self.win.set_icon(self.ico)
        accelGroup = gtk.AccelGroup()
        self.ag = accelGroup = gtk.AccelGroup()
        self.win.add_accel_group(accelGroup)
        accelGroup.connect_group(ord('n'), gtk.gdk.CONTROL_MASK | gtk.gdk.SHIFT_MASK, gtk.ACCEL_LOCKED, self.newterm)
        ...
        self.win.show_all()
        self.hate.start()
        self.win.set_geometry_hints(self.hate.term, **self.hate.geometry_hints())
