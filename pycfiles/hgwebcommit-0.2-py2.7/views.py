...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hgwebcommit/views.py
...
OPERATION_MESSAGE = {'commit': lazy_gettext('Commit'), 
...
def get_choices_ctrl(repo):
    return [ (val, 'M %s' % val) for val in repo.status_modified() ] + [ (val, 'A %s' % val) for val in repo.status_added() ] + [ (val, 'R %s' % val) for val in repo.status_removed() ] + [ (val, '! %s' % val) for val in repo.status_deleted() ]
...
def index():
    ...
    return render_template('index.html', repository=repo, form_unknown=form_unknown, form_ctrl=form_ctrl, form_actions=form_actions, hostname=gethostname(), allow_commit=get_allow_commit())
...
def add_unknown_confirm():
    ...
    if not form.validate():
        ...
        form.files.choices = get_choices_unknown(repo)
        form.validate()
        return render_template('add_unknown_confirm.html', repository=repo, form=form, hostname=gethostname())
...
def submit_confirm():
    ...
    if not form.validate():
        ...
        form.files.choices = get_choices_ctrl(repo)
        form.validate()
        return render_template('submit.html', repository=repo, form=form, message=OPERATION_MESSAGE.get(form.data['operation']), enable_commit_message=form.data['operation'] == 'commit', hostname=gethostname())
