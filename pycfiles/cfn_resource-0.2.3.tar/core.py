...
# Embedded file name: build/bdist.linux-x86_64/egg/cfn_pyplates/core.py
# Compiled at: 2016-03-13 19:12:01
__doc__ = "Core functionality and all required components of a working CFN template.\n\nThese are all available without preamble in a pyplate's global namespace.\n"
import inspect, json, traceback, warnings
...
class CloudFormationTemplate(JSONableDict):
    ...
    def __init__(self, description=None, options=None):
        super(CloudFormationTemplate, self).__init__({'AWSTemplateFormatVersion': aws_template_format_version})
        if description:
...
def _load_pyplate(pyplate, options_mapping=None):
    ...
    exec_namespace = {'options': options_mapping}
