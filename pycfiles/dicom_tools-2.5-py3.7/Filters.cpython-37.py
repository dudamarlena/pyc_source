...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/dicom_tools/pyqtgraph/flowchart/library/Filters.py
# Compiled at: 2018-05-21 04:28:19
# Size of source mod 2**32: 13172 bytes
...
class Downsample(CtrlNode):
    __doc__ = 'Downsample by averaging samples together.'
    nodeName = 'Downsample'
    uiTemplate = [
     (
...
class Subsample(CtrlNode):
    __doc__ = 'Downsample by selecting every Nth sample.'
    nodeName = 'Subsample'
    uiTemplate = [
     (
      'n', 'intSpin', {'min':1,  'max':1000000})]
...
class Bessel(CtrlNode):
    __doc__ = 'Bessel filter. Input data must have time values.'
    nodeName = 'BesselFilter'
    uiTemplate = [
     (
      'band', 'combo', {'values':['lowpass', 'highpass'],  'index':0}),
     (
      'cutoff', 'spin', {'value':1000.0,  'step':1,  'dec':True,  'range':[0.0, None],  'suffix':'Hz',  'siPrefix':True}),
     (
...
class Butterworth(CtrlNode):
    __doc__ = 'Butterworth filter'
    nodeName = 'ButterworthFilter'
    uiTemplate = [
     (
      'band', 'combo', {'values':['lowpass', 'highpass'],  'index':0}),
     (
      'wPass', 'spin', {'value':1000.0,  'step':1,  'dec':True,  'range':[0.0, None],  'suffix':'Hz',  'siPrefix':True}),
     (
      'wStop', 'spin', {'value':2000.0,  'step':1,  'dec':True,  'range':[0.0, None],  'suffix':'Hz',  'siPrefix':True}),
     (
...
class ButterworthNotch(CtrlNode):
    __doc__ = 'Butterworth notch filter'
    nodeName = 'ButterworthNotchFilter'
    uiTemplate = [
     (
      'low_wPass', 'spin', {'value':1000.0,  'step':1,  'dec':True,  'range':[0.0, None],  'suffix':'Hz',  'siPrefix':True}),
     (
      'low_wStop', 'spin', {'value':2000.0,  'step':1,  'dec':True,  'range':[0.0, None],  'suffix':'Hz',  'siPrefix':True}),
     (
     ...
     (
      'high_wPass', 'spin', {'value':3000.0,  'step':1,  'dec':True,  'range':[0.0, None],  'suffix':'Hz',  'siPrefix':True}),
     (
      'high_wStop', 'spin', {'value':4000.0,  'step':1,  'dec':True,  'range':[0.0, None],  'suffix':'Hz',  'siPrefix':True}),
     (
...
class Mean(CtrlNode):
    __doc__ = 'Filters data by taking the mean of a sliding window'
    nodeName = 'MeanFilter'
    uiTemplate = [
     (
      'n', 'intSpin', {'min':1,  'max':1000000})]
...
class Median(CtrlNode):
    __doc__ = 'Filters data by taking the median of a sliding window'
...
class Mode(CtrlNode):
    __doc__ = 'Filters data by taking the mode (histogram-based) of a sliding window'
    nodeName = 'ModeFilter'
    uiTemplate = [
     (
...
class Denoise(CtrlNode):
    __doc__ = 'Removes anomalous spikes from data, replacing with nearby values'
...
class Gaussian(CtrlNode):
    __doc__ = 'Gaussian smoothing filter.'
...
class Derivative(CtrlNode):
    __doc__ = 'Returns the pointwise derivative of the input'
...
class Integral(CtrlNode):
    __doc__ = 'Returns the pointwise integral of the input'
...
class Detrend(CtrlNode):
    __doc__ = 'Removes linear trend from the data'
...
class RemoveBaseline(PlottingCtrlNode):
    __doc__ = 'Remove an arbitrary, graphically defined baseline from the data.'
...
class AdaptiveDetrend(CtrlNode):
    __doc__ = 'Removes baseline from data, ignoring anomalous events'
...
class HistogramDetrend(CtrlNode):
    __doc__ = 'Removes baseline from data by computing mode (from histogram) of beginning and end of data.'
    ...
    def processData(self, data):
        ...
        for i in xrange(1, self.ctrls['harmonics'].value() + 2):
            ...
            for j in range(ind1, ind2 + 1):
                ...
                re = mag * np.cos(phase)
                im = mag * np.sin(phase)
                ft[j] = re + im * complex(0.0, 1.0)
                ft[len(ft) - j] = re - im * complex(0.0, 1.0)
