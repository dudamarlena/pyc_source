...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/dicom_tools/pyqtgraph/debug.py
# Compiled at: 2018-05-21 04:28:19
# Size of source mod 2**32: 39518 bytes
"""
...
def getExc(indent=4, prefix='|  ', skip=1):
    lines = traceback.format_stack()[:-skip] + ['  ---- exception caught ---->\n'] + traceback.format_tb(sys.exc_info()[2]) + (traceback.format_exception_only)(*sys.exc_info()[:2])
    lines2 = []
    for l in lines:
...
def refPathString(chain):
    ...
    while i < len(chain) - 1:
        ...
        if not isinstance(o1, list):
            ...
            if hasattr(o1, '__dict__') and o2 == o1.__dict__:
                i += 1
                if i >= len(chain):
                    s += '.__dict__'
                    continue
...
def objectSize(obj, ignore=None, verbose=False, depth=0, recursive=False):
    ...
    ignoreTypes = [
     'MethodType', 'UnboundMethodType', 'BuiltinMethodType', 'FunctionType', 'BuiltinFunctionType']
...
class ObjTracker(object):
    ...
    def collect(self):
        ...
        for k in objs:
            ...
            typ = type(o)
            oid = id(o)
            if not ObjTracker.isObjVar(o):
                if typ in ignoreTypes:
                    continue
                try:
                    ref = weakref.ref(obj)
                except:
...
def describeObj(obj, depth=4, path=None, ignore=None):
    ...
    if path is None:
        ...
        printed or print('Dead end: ' + refPathString(path))
...
class PrintDetector(object):
    __doc__ = 'Find code locations that print to stdout.'
...
class PeriodicTrace(object):
    ...
    def trace(self, frame, event, arg):
        if threading.current_thread() is self.mainThread:
