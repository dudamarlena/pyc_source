...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/dicom_tools/pyqtgraph/widgets/TableWidget.py
# Compiled at: 2018-05-21 04:28:19
# Size of source mod 2**32: 18258 bytes
...
class TableWidget(QtGui.QTableWidget):
    ...
    def appendData(self, data):
        ...
        try:
            ...
            if not self.verticalHeadersSet:
            ...
            if not self.horizontalHeadersSet:
            ...
            if self._sorting:
                if self.horizontalHeader().sortIndicatorSection() >= self.columnCount():
    ...
    def iteratorFn(self, data):
        if not isinstance(data, list):
            if isinstance(data, tuple):
                return (
                 lambda d: d.__iter__(), None)
            if isinstance(data, dict):
                return (
                 lambda d: iter(d.values()), list(map(asUnicode, data.keys())))
        elif HAVE_METAARRAY:
            if hasattr(data, 'implements') and data.implements('MetaArray'):
                if data.axisHasColumns(0):
                    header = [asUnicode(data.columnName(0, i)) for i in range(data.shape[0])]
                else:
                    if data.axisHasValues(0):
                        header = list(map(asUnicode, data.xvals(0)))
                    else:
                        header = None
                return (
                 self.iterFirstAxis, header)
        if isinstance(data, np.ndarray):
            return (
             self.iterFirstAxis, None)
        if isinstance(data, np.void):
            return (
             self.iterate, list(map(asUnicode, data.dtype.names)))
        if data is None:
            return (None, None)
...
class TableWidgetItem(QtGui.QTableWidgetItem):
    ...
    def __lt__(self, other):
        if self.sortMode == 'index':
            if hasattr(other, 'index'):
                return self.index < other.index
        if self.sortMode == 'value':
