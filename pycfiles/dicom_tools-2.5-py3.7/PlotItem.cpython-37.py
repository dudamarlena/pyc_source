...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/dicom_tools/pyqtgraph/graphicsItems/PlotItem/PlotItem.py
# Compiled at: 2018-05-21 04:28:19
# Size of source mod 2**32: 47463 bytes
"""
        ...
        *axisItems*     Optional dictionary instructing the PlotItem to use pre-constructed items
        ...
        """
            ...
            for m in ('setXRange', 'setYRange', 'setXLink', 'setYLink', 'setAutoPan',
                      'setAutoVisible', 'setRange', 'autoRange', 'viewRect', 'viewRange',
                      'setMouseEnabled', 'setLimits', 'enableAutoRange', 'disableAutoRange',
                      'setAspectLocked', 'invertY', 'invertX', 'register', 'unregister'):
        ...
        """
                    ...
                    for p in list(self.itemMeta.get(c, {}).keys()):
                        if type(p) is tuple:
                            ...
                            if len(matches) == 0:
                                ...
                                if p in self.paramList:
                                    if self.paramList[p] is True:
            ...
            def restoreState(self, state):
                ...
                if 'view' not in state:
                    r = [
                     [
                      float(state['xMinText']), float(state['xMaxText'])], [float(state['yMinText']), float(state['yMaxText'])]]
                    state['view'] = {'autoRange':[
                      state['xAutoRadio'], state['yAutoRadio']], 
                     'linkedViews':[
                      state['xLinkCombo'], state['yLinkCombo']], 
                     'targetRange':r, 
                     'viewRange':r}
            ...
            def widgetGroupInterface(self):
                return (
                 None, PlotItem.saveState, PlotItem.restoreState)
        ...
        """
            ...
            def downsampleMode(self):
                ...
                if self.ctrl.subsampleRadio.isChecked():
                    ...
                    if self.ctrl.meanRadio.isChecked():
                        method = 'mean'
                    else:
                        if self.ctrl.peakRadio.isChecked():
                            method = 'peak'
                return (
                 ds, auto, method)
        ...
        **Arguments:**
        ...
        """
            ...
            def updateButtons(self):
                try:
                    if self._exportOpts is False and self.mouseHovering:
                        self.buttonsHidden or all(self.vb.autoRangeEnabled()) or self.autoBtn.show()
                    else:
                        self.autoBtn.hide()
                except RuntimeError:
                    pass
