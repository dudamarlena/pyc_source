...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/dicom_tools/BrukerMRI.py
# Compiled at: 2018-05-21 04:28:19
# Size of source mod 2**32: 13895 bytes
"""This should at some point be a library with functions to import and
reconstruct Bruker MRI data.

2014, Joerg Doepfert
2018  Carlo Mancini some small modifications
...
class BrukerData:
    __doc__ = 'Class to store and process data of a Bruker MRI Experiment'
    ...
    def __init__(self, path='', ExpNum=0, B0=9.4):
        ...
        self.reco_data_norm = np.array([])
        self.B0 = B0
        self.GyroRatio = 0
        self.ConvFreqsFactor = 0
        self.path = path
        self.ExpNum = ExpNum
    ...
    def GenerateKspace(self):
        ...
        if self.method == {}:
            ...
            if self.method['Method'] == 'jd_UFZ_RAREst':
                ...
                if self.method['Method'] == 'FLASH' or self.method['Method'] == 'mic_flash':
    ...
    def _GenKspace_FLASH(self):
        ...
        NScans = self.acqp['NI'] * self.acqp['NAE'] * self.acqp['NA'] * self.acqp['NR']
        Matrix = self.method['PVM_Matrix']
        kSpace = np.reshape(complexValues, (-1, Matrix[0]), order='F')
    ...
    def _GenKspace_UFZ_RARE(self):
        complexValues = self.raw_fid
        complexValues = RemoveVoidEntries(complexValues, self.acqp['ACQ_size'][0])
        NEchoes = self.method['CEST_Number_Echoes']
        NPoints = self.method['CEST_Number_SatFreqs']
        NScans = self.method['PVM_NRepetitions']
        return np.reshape(complexValues, (NPoints, NEchoes, NScans), order='F')
    ...
    def _Reco_UFZ_RARE(self, **kwargs):
        ...
        if NEchoes == 'opt':
            ...
            MaxIndizes.append(MaxIndizes[0] + 1 - 2 * (Data[(MaxIndizes[0] - 1, 0, 0)] > Data[(MaxIndizes[0] + 1, 0, 0)]))
        ...
        for i in range(0, NScans):
            ...
            KspaceAveraged[:, i] = np.mean((self.k_data[:, RecoEchoes, i]),
              axis=1)
            KspaceAveraged[(KspaceCutoffIdx, i)] = 0

        self.reco_data, _ = FFT_center(KspaceAveraged)
        if self.method['CEST_AcqMode'] == 'On_and_Off_Scan':
            self.reco_data_norm = np.divide(abs(self.reco_data[:, 1::2]), abs(self.reco_data[:, 0::2]))


def ReadFolder(path, verbose=False):
    """Read in a Bruker MRI Experiment a single folder. 
    Returns raw data, processed data, and method and acqp 
    parameters in a dictionary.

    """
    if verbose:
        print('BrukerMRI ReadFolder: input path:', path)
    tmp = path.rpartition('/')
    if verbose:
        print('BrukerMRI ReadFolder: parsed path:', tmp)
    path = tmp[0]
    ExpNum = tmp[2]
    if verbose:
        print('BrukerMRI ReadFolder: reading ExpNum', ExpNum, 'from folder:', path)
    return ReadExperiment(path + '/', ExpNum)
...
def ReadExperiment(path, ExpNum):
    ...
    try:
        ...
        print('BrukerMRI WARNING: method file not found')

    data.acqp = ReadParamFile(path + str(ExpNum) + '/acqp')
    data.reco = ReadParamFile(path + str(ExpNum) + '/pdata/1/reco')
    data.proc_data = ReadProcessedData(path + str(ExpNum) + '/pdata/1/2dseq', data.reco, data.acqp)
    raw_data = ReadRawData(path + str(ExpNum) + '/fid')
    data.raw_fid = raw_data[0::2] + complex(0.0, 1.0) * raw_data[1::2]
    data.GyroRatio = data.acqp['SFO1'] * 2 * np.pi / data.B0 * 1000000
    data.ConvFreqsFactor = 1 / (data.GyroRatio * data.B0 / 1000000 / 2 / np.pi)
    data.path = path
    data.ExpNum = ExpNum
    return data
...
def FFT_center(Kspace, sampling_rate=1, ax=0):
    ...
    n = FT.shape[ax]
    freq_axis = np.fft.fftshift(np.fft.fftfreq(n, 1 / float(sampling_rate)))
    return (
     spectrum, freq_axis)
...
def RemoveVoidEntries(datavector, acqsize0):
    ...
    for i in range(0, len(datavector) / blocksize):
        DelIdx.append(range(i * blocksize + acqsize0 / 2, (i + 1) * blocksize))
...
def ReadProcessedData(filepath, reco, acqp):
    with open(filepath, 'r') as (f):
        ...
        data = data.reshape((reco['RECO_size'][0]), (reco['RECO_size'][1]),
          (-1), order='F')
        if data.ndim == 3:
