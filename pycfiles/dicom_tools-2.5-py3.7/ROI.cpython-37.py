...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/dicom_tools/pyqtgraph/graphicsItems/ROI.py
# Compiled at: 2018-05-21 04:28:19
# Size of source mod 2**32: 90046 bytes
"""
...
class ROI(GraphicsObject):
        ...
        *snap* can be:
        ...
        if self.maxBounds is not None:
            ...
            if self.maxBounds.left() > r.left():
                ...
                if self.maxBounds.right() < r.right():
                    d[0] = self.maxBounds.right() - r.right()
                elif self.maxBounds.top() > r.top():
                    d[1] = self.maxBounds.top() - r.top()
                else:
                    if self.maxBounds.bottom() < r.bottom():
    ...
    def mouseClickEvent(self, ev):
        ...
        if ev.button() == QtCore.Qt.RightButton and self.contextMenuEnabled():
            ...
            if int(ev.button() & self.acceptedMouseButtons()) > 0:
                ev.accept()
                ...
                if h['type'] == 's':
                    ...
                    if h['type'] in ('r', 'rf'):
                        if h['type'] == 'rf':
                            ...
                            return self.rotateAllowed or None
                        try:
    ...
    def getArraySlice(self, data, img, axes=(0, 1), returnSlice=True):
        ...
        """
        dShape = (
         data.shape[axes[0]], data.shape[axes[1]])
        try:
    ...
    def getAffineSliceParams(self, data, img, axes=(0, 1)):
        ...
        vectors = (
         (
          vx.x() * sx, vx.y() * sx), (vy.x() * sy, vy.y() * sy))
        shape = self.state['size']
...
class MultiRectROI(QtGui.QGraphicsObject):
    ...
    def getHandlePositions(self):
        ...
        pos = [
         self.mapFromScene(self.lines[0].getHandles()[0].scenePos())]
        for l in self.lines:
...
class EllipseROI(ROI):
    ...
    def __init__(self, pos, size, **args):
        ...
        self.addScaleHandle([0.8535533905932737, 0.8535533905932737], [0.5, 0.5])
...
class PolyLineROI(ROI):
    ...
    def addSegment(self, h1, h2, index=None):
        ...
        for h in seg.handles:
            ...
            h['item'].setAcceptedMouseButtons(h['item'].acceptedMouseButtons() | QtCore.Qt.LeftButton)
    ...
    def getArrayRegion(self, data, img, axes=(0, 1), returnMappedCoords=False, **kwds):
        ...
        p.drawPath(self.shape())
        p.end()
        mask = fn.imageToArray(im)[:, :, 0].astype(float) / 255.0
        shape = [1] * data.ndim
...
class CrosshairROI(ROI):
    __doc__ = 'A crosshair ROI whose position is at the center of the crosshairs. By default, it is scalable, rotatable and translatable.'
