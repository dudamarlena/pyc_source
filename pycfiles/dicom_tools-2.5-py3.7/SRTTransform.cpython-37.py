...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/dicom_tools/pyqtgraph/SRTTransform.py
...
class SRTTransform(QtGui.QTransform):
    __doc__ = 'Transform that can always be represented as a combination of 3 matrices: scale * rotate * translate\n    This transform has no shear; angles are always preserved.\n    '
    ...
    def __init__(self, init=None):
        ...
        if init is None:
            ...
            if isinstance(init, SRTTransform):
                self._state = {'pos':Point(init._state['pos']),  'scale':Point(init._state['scale']), 
                 'angle':init._state['angle']}
                self.update()
            else:
                if isinstance(init, QtGui.QTransform):
                    self.setFromQTransform(init)
                else:
                    if isinstance(init, QtGui.QMatrix4x4):
    ...
    def setFromQTransform(self, tr):
        ...
        if dp2.angle(dp3) > 0:
            ...
            da = 0
            sy = 1.0
        self._state = {'pos':Point(p1), 
         'scale':Point(dp2.length(), dp3.length() * sy), 
         'angle':np.arctan2(dp2[1], dp2[0]) * 180.0 / np.pi + da}
        self.update()
    ...
    def setFromMatrix4x4(self, m):
        ...
        self._state = {'pos':Point(m.getTranslation()), 
    ...
    def saveState(self):
        ...
        return {'pos':(
          p[0], p[1]), 
         'scale':(s[0], s[1]),  'angle':self._state['angle']}
