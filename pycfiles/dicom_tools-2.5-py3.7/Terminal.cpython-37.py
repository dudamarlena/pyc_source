...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/dicom_tools/pyqtgraph/flowchart/Terminal.py
# Compiled at: 2018-05-21 04:28:19
# Size of source mod 2**32: 23570 bytes
from ..Qt import QtCore, QtGui
import weakref
import graphicsItems.GraphicsObject as GraphicsObject
    ...
    def recolor(self, color=None, recurse=True):
        if color is None:
            if not self.isConnected():
                ...
                if self.isInput():
                    color = self.hasInput() or QtGui.QColor(200, 200, 0)
                else:
                    if self._value is None or eq(self._value, {}):
                        color = QtGui.QColor(255, 255, 255)
                    else:
                        if self.valueIsAcceptable() is None:
                            color = QtGui.QColor(200, 200, 0)
                        else:
                            if self.valueIsAcceptable() is True:
...
class ConnectionItem(GraphicsObject):
    ...
    def updateLine(self):
        ...
        if isinstance(self.target, TerminalGraphicsItem):
            ...
            if isinstance(self.target, QtCore.QPointF):
    ...
    def hoverEvent(self, ev):
        if (ev.isExit() or ev.acceptClicks)(QtCore.Qt.LeftButton):
            self.hovered = True
        else:
            self.hovered = False
        self.update()
