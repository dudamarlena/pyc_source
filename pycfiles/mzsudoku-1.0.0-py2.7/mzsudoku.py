...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\mzsudoku.py
# Compiled at: 2013-12-18 18:33:55
"""
Name: mzsudoku
...
def run(setting, options='tnr'):
    ...
    while True:
        ...
        if failure:
            ...
            break
        if 'a' not in options.lower():
            break
        setting2 = apply_known(setting, marked, hidden_singles)
        if setting == setting2:
            print 'Cannot find algorithmical solution. Try set something with .fill() method.'
            break
...
def user_break(options):
    if 'A' in options:
        print '\n==== next iteration ====\n'
        return False
    return raw_input('press Enter for next iteration (something else + enter to stop)')
...
def p(a, marked_by_N):
    ...
    for i in xrange(81):
        if a[i] == '0':
            ...
            for j in xrange(81):
                if a[j] != '0' and (same_row(i, j) or same_col(i, j) or same_block(i, j)):
...
def n(marked, options, hidden_singles, marked_by_N):
    ...
    while True:
        ...
        for ln in xrange(9):
            ...
            solveN('ln%s' % (ln + 1), block, excluded, used_groups, hidden_groups, block_pos, hidden_singles, to_print, marked, print_result)
        ...
        for cl in xrange(9):
            ...
            solveN('cl%s' % (cl + 1), block, excluded, used_groups, hidden_groups, block_pos, hidden_singles, to_print, marked, print_result)
        ...
        for bl in xrange(9):
            ...
            solveN('bl%s' % (bl + 1), block, excluded, used_groups, hidden_groups, block_pos, hidden_singles, to_print, marked, print_result)

        if print_result and to_print:
            print 'hidden n-combinations'
            print '[as naked <=> where / digits / field position (unknown fields counted)]:'
        ...
        if hidden_singles or naked_count or used_groups >= len(hidden_groups):
            break
        if not used_groups:
            hidden_groups.sort(key=lambda item: item[6])
            print 'no easy solution - can try apply N-groups'
        while used_groups < len(hidden_groups):
            hidden_group = hidden_groups[used_groups]
            removed = set()
            for where1 in hidden_group[2]:
                pos = hidden_group[4][(where1 - 1)]
                for number in hidden_group[5]:
                    if number not in hidden_group[1] and number not in marked[pos]:
                        removed.add(number)
...
def solveN(symbol, block, excluded, used_groups, hidden_groups, block_pos, hidden_singles, to_print, marked, print_result):
    global failure
    included = []
    ...
    for lenN in xrange(len(block) - 1, 0, -1):
        for candidate in itertools.combinations(included, lenN):
            ...
            cnt_naked = len(block) - len(where)
            if len(where) == 1:
                to_append = (
                 candidate, block_pos[(where[0] - 1)])
                if to_append not in hidden_singles:
