...
# [GCC 8.4.0]
# Embedded file name: /Users/mattcaldwell/.virtualenvs/django-smart-proxy/lib/python3.3/site-packages/smart_proxy/recorder.py
...
from .models import SmartProxyRequest, SmartProxyResponse
logger = logging.getLogger(__name__)

class SmartProxyRecorder(object):
    __doc__ = '\n    Facilitates recording and playback of Django HTTP requests and responses.\n    '
    response_types_supported = ('application/javascript', 'application/xml', 'text/css',
                                'text/html', 'text/javascript', 'text/plain', 'text/xml')

    def __init__(self, domain, port):
        super(SmartProxyRecorder, self).__init__()
    ...
    def record_request(self, request):
        ...
        recorded_request, created = SmartProxyRequest.objects.get_or_create(method=request.method, domain=self.domain, port=self.port, path=request.path, querykey=self._get_query_key(request))
    ...
    def record_request_parameters(self, request, recorded_request):
        ...
        for name, values_list in request.GET.lists():
            for value in values_list:
                recorded_request.parameters.create(order=position, name=name, value=value)
                position += 1
    ...
    def record_response(self, recorded_request, response):
        ...
        except SmartProxyResponse.DoesNotExist:
            pass

        content_type = response['Content-Type']
        encoding = content_type.partition('charset=')[(-1)] or 'utf-8'
        SmartProxyResponse.objects.create(request=recorded_request, status=response.status_code, content_type=content_type, content=response.content.decode(encoding))
    ...
    def playback(self, request):
        ...
        """
        try:
            matching_request = SmartProxyRequest.objects.filter(method=request.method, domain=self.domain, port=self.port, path=request.path, querykey=self._get_query_key(request)).latest()
        except SmartProxyRequest.DoesNotExist as e:
            raise SmartProxyRequestNotRecorded('The request made has not been recorded yet. Please run smart_proxy in "record" mode first.')
        ...
        return HttpResponse(response.content.encode(encoding), status=response.status, mimetype=response.content_type)

    def response_supported(self, response):
        return response['Content-Type'].partition(';')[0] in self.response_types_supported
    ...
    def _request_string(self, request):
        ...
        """
        return '%(domain)s:%(port)d%(path)s' % {'domain': self.domain, 
         'port': self.port, 
