...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\zssget\util\log.py
# Compiled at: 2016-07-24 01:54:27
# Size of source mod 2**32: 2682 bytes
import os, sys
IS_ANSI_TERMINAL = os.getenv('TERM') in ('eterm-color', 'linux', 'screen', 'vt100',
                                         'xterm')
RESET = 0
BOLD = 1
UNDERLINE = 4
...
def sprint(text, *colors):
    ...
    if IS_ANSI_TERMINAL and colors:
        return '\x1b[{}m{content}\x1b[{}m'.format(';'.join([str(color) for color in colors]), RESET, content=text)
    return text
...
def print_log(text, *colors):
    ...
    sys.stderr.write(sprint('{}: {}'.format('zss-get', text), *colors) + '\n')
