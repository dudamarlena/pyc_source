...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/testoob/main.py
...
import commandline, testoob.reporting
...
def _main(suite, defaultTest, options, test_names, parser):
    ...
    def conflicting_options(*option_names):
        given_options = [ name for name in option_names if getattr(options, name) is not None ]
        given_options.sort()
        if len(given_options) > 1:
            raise ArgumentsError("The following options can't be specified together: %s" % (', ').join(given_options))
        return
    ...
    conflicting_options('threads', 'processes', 'processes_old', 'processes_pyro', 'list')
    ...
    commandline.parsing.kwargs = {'stop_on_fail': options.stop_on_fail, 'reporters': [], 'extraction_decorators': [], 'fixture_decorators': [], 'interval': options.interval}
    kwargs = commandline.parsing.kwargs
    ...
    kwargs['suites'] = _get_suites(suite, defaultTest, test_names, test_loader=get_test_loader())
    if options.coverage is not None:
        from running import fixture_decorators
        from coverage import Coverage
        cov = Coverage(_coverage_ignore_paths())
        kwargs['fixture_decorators'].append(fixture_decorators.get_coverage_fixture(cov))
        if options.coverage != 'silent':
            testoob.reporting.options.coverage = (
             options.coverage, cov)
    if options.capture is not None:
        from running import fixture_decorators
        kwargs['fixture_decorators'].append(fixture_decorators.get_capture_fixture())
    if options.vassert:
        import asserter
    ...
    if options.timeout is not None:
        ...
        from running import fixture_decorators
        kwargs['fixture_decorators'].append(fixture_decorators.get_alarmed_fixture(options.timeout))
    ...
    if options.timeout_with_threads is not None:
        ...
        from running import fixture_decorators
        kwargs['fixture_decorators'].append(fixture_decorators.get_thread_timingout_fixture(options.timeout_with_threads))
    ...
    if options.debug is not None:
        ...
        def runDebug(test, err_info, flavour, reporter, real_add):
            ...
            print '\nDebugging for %s in test: %s' % (flavour, reporter.getDescription(test))
            if options.rerun_on_fail is not None:
    ...
    def text_run_decorator():
        if options.profiler is not None:
            import profiling
            return profiling.profiling_decorator(options.profiler, options.profdata)
        return lambda x: x
        return
