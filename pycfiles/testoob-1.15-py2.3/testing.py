...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/testoob/testing.py
...
def assert_matches(regex, actual, msg=None, filter=None):
    ...
    if filter is not None:
        actual = filter(actual)
    if re.search(regex, actual, re.DOTALL) is not None:
        return
    if msg is None:
...
def assert_raises(exception_class, callable, *args, **kwargs):
    ...
    try:
        ...
        if expected_args is not None:
            assert_equals(expected_args, e.args, msg='%s raised %s with unexpected args: expected=%r, actual=%r' % (callsig(), e.__class__, expected_args, e.args))
        if expected_regex is not None:
            assert_matches(expected_regex, str(e), msg="%s raised %s, but the regular expression '%s' doesn't match %r" % (callsig(), e.__class__, expected_regex, str(e)))
    except:
        import sys
        ...
        raise TestoobAssertionError('%s not raised' % exc_name(), description='assert_raises failed')

    return


def command_line(args, input=None, expected_output=None, expected_error=None, expected_output_regex=None, expected_error_regex=None, expected_rc=None, rc_predicate=None, skip_check=None):
    """
    ...
    try:
        ...
        e.long_message = ('\n').join(['command_line test failed', '== args: %s' % args, "== equiv. cmdline: '%s'" % (' ').join(args), '== rc: %s' % rc, annotated_err_string('stdout', output), annotated_err_string('stderr', error)])
        raise
