...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/testoob/compatibility/trace.py
...
"""
import linecache, os, re, sys, threading, token, tokenize, types, gc
try:
    import cPickle
    pickle = cPickle
except ImportError:
...
def find_lines_from_code(code, strs):
    ...
    def odd_indexed_items(seq):
        return [ seq[i] for i in xrange(1, len(seq), 2) ]
...
def main(argv=None):
    ...
    try:
        (opts, prog_argv) = getopt.getopt(argv[1:], 'tcrRf:d:msC:lT', [
         'help', 'version', 'trace', 'count', 'report', 'no-report', 'summary', 'file=', 'missing', 'ignore-module=', 'ignore-dir=', 'coverdir=', 'listfuncs', 'trackcalls'])
