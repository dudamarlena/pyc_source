...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/testoob/compatibility/itertools.py
...
__all__ = [
 'chain', 'count', 'cycle', 'dropwhile', 'groupby', 'ifilter', 'ifilterfalse', 'imap', 'islice', 'izip', 'repeat', 'starmap', 'takewhile', 'tee']
    ...
    def next(self):
        try:
            ...
            try:
                ...
                raise TypeError('%s has no next() method' % self._cur_iterable_iter)
...
class cycle:
    ...
    def next(self):
        try:
            ...
            raise TypeError('%s has no next() method' % self._cur_iter)
    ...
    def next(self):
        while self.currkey == self.tgtkey:
            try:
                ...
                raise TypeError('%s has no next() method' % self.it)
...
class imap:
    ...
    def next(self):
        try:
            ...
            raise TypeError('%s has no next() method' % it)
...
class islice:
    ...
    def __init__(self, iterable, *args):
        ...
        if self.step is None:
            self.step = 1
        if self.start < 0 or self.stop is not None and self.stop < 0 or self.step <= 0:
...
def tee(iterable, n=2):
    ...
    if isinstance(iterable, TeeObject):
        return tuple([iterable] + [ TeeObject(tee_data=iterable.tee_data) for i in xrange(n - 1) ])
