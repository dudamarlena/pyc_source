...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/testoob/reporting/textstream.py
...
class BogusWriter(StreamWriter):
    """For testing"""
    __module__ = __name__
...
class TextStreamReporter(BaseReporter):
    ...
    def addError(self, test_info, err_info):
        ...
        self._report_failure('ERROR', 'E', self.writers.failure, test_info, err_info)

    def addFailure(self, test_info, err_info):
        BaseReporter.addFailure(self, test_info, err_info)
        self._report_failure('FAIL', 'F', self.writers.failure, test_info, err_info)
    ...
    def stopTest(self, test_info):
        ...
        if self.time_each_test:
            self.writers.normal.write(' [%.2f seconds]' % self.current_test_total_time)
        if self.showAll:
            self._writeln('')
    ...
    def _printResults(self):
        ...
        self._write('Ran %d test%s in %.3fs' % (self.testsRun, testssuffix, self.total_time))
    ...
    def _print_coverage_statistics(self, coverage):
        ...
        for (filename, stats) in coverage.getstatistics().items():
            print >> self.writers.normal, '%5d   %5d   %3d%%   %s   (%s)' % (stats['lines'], stats['covered'], stats['percent'], modname(filename), filename)

        print >> self.writers.normal, '--------------------------------------'
        print >> self.writers.normal, '%5d   %5d   %3d%%   TOTAL' % (coverage.total_lines(), coverage.total_lines_covered(), coverage.total_coverage_percentage())
    ...
    def _print_coverage(self, coverage):
        ...
        for (filename, stats) in coverage.getstatistics().items():
            print >> self.writers.normal, (module_tmpl + '   %5d   %5d   %3d%%   %s') % (modname(filename), stats['lines'], stats['covered'], stats['percent'], self._get_missing_lines_str(*coverage.coverage[filename].values()))

        print >> self.writers.normal, '-' * maxmodule + '---------------------------------'
        print >> self.writers.normal, (module_tmpl + '   %5d   %5d   %3d%%') % ('TOTAL', coverage.total_lines(), coverage.total_lines_covered(), coverage.total_coverage_percentage())
