...
# Embedded file name: build/bdist.macosx-10.4-x86_64/egg/brownant/pipeline/html.py
...
class ElementTreeProperty(PipelineProperty):
    r"""The element tree built from a text response property. There is an usage
    example::
...
class XPathTextProperty(PipelineProperty):
    ...
    def choice_pick_impl(self):
        ...
        impl = {'join': self.pick_joining, 
           'first': self.pick_first, 
           'keep': self.keep_value}.get(pick_mode)
        if not impl:
