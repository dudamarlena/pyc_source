...
# Embedded file name: build/bdist.macosx-10.4-x86_64/egg/brownant/app.py
...
class Brownant(object):
    ...
    def __init__(self):
        self.url_map = Map(strict_slashes=False, host_matching=True, redirect_defaults=False)
    ...
    def parse_url(self, url_string):
        ...
        url_adapter = self.url_map.bind(server_name=url.hostname, url_scheme=url.scheme, path_info=url.path)
        query_args = url_decode(url.query)
        return (
         url, url_adapter, query_args)
    ...
    def validate_url(self, url):
        ...
        has_path = not len(url.path) or url.path.startswith(b'/')
        if not (has_hostname and has_http_scheme and has_path):
            raise NotSupported(b'invalid url: %s' % repr(url))
        return url
    ...
    def dispatch_url(self, url_string):
        ...
        try:
            ...
            new_url = (b'{0.new_url}?{1}').format(e, url_encode(query_args))
...
class BrownAnt(Brownant):
    ...
    def __init__(self, *args, **kwargs):
        warn(b"The class name 'BrownAnt' has been deprecated. Please use 'Brownant' instead.", DeprecationWarning)
