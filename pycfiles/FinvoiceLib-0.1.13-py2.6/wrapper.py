...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/finvoicelib/wrapper.py
...
class Invoice(object):
    ...
    def set_epi_details(self, doc):
        ...
        if len(epi_details) > 0:
            epi_payment_details = epi_details[0].EpiPaymentInstructionDetailsSet
            if len(epi_payment_details) > 0:
                epi_reference = epi_payment_details[0].EpiRemittanceInfoIdentifierSet
        ...
        if len(invoice_details) > 0:
            ...
            if len(payment_details) > 0:
                ...
                if len(fine_details) > 0:
                    overdue_text = fine_details[0].PaymentOverDueFineFreeTextSet
                    if len(overdue_text) > 0:
                        self.payment_overdue_text = overdue_text[0].text
                    payment_overdue_percent = fine_details[0].PaymentOverDueFinePercentSet
                    if len(payment_overdue_percent) > 0:
                        self.payment_overdue_percent = payment_overdue_percent[0].text
            ...
            if len(total_untaxed) > 0:
                ...
                currency = total_untaxed[0].attributes.get('amountcurrencyidentifier', '')
            ...
            if len(total_taxed) > 0:
                ...
                self.total_taxed_currency = total_taxed[0].attributes.get('amountcurrencyidentifier', '')
            vat_amount = invoice_details[0].InvoiceTotalVatAmountSet
            if len(vat_amount) > 0:
                self.vat_amount = _Decimal(vat_amount[0].text)
                self.vat_amount_currency = vat_amount[0].attributes.get('amountcurrencyidentifier', '')
...
class InvoiceRecipient(object):
    ...
    def __init__(self, doc):
        ...
        if len(invoice_recipient_party_details) > 0:
            postal_address = invoice_recipient_party_details[0].InvoiceRecipientPostalAddressDetailsSet
            if len(postal_address) > 0:
                self.address = InvoiceRecipientAddress(postal_address[0])
            party_identifier = invoice_recipient_party_details[0].InvoiceRecipientPartyIdentifierSet
            if len(party_identifier) > 0:
                self.party_identifier = party_identifier[0].text
            organisation_name = invoice_recipient_party_details[0].InvoiceRecipientOrganisationNameSet
            if len(organisation_name) > 0:
                self.organisation_name = organisation_name[0].text
            tax_code = invoice_recipient_party_details[0].InvoiceRecipientOrganisationTaxCodeSet
            if len(tax_code) > 0:
        ...
        if len(info_details) > 0:
            ...
            if len(invoice_recipient_details) > 0:
                invoice_recipient_einvoice_address = invoice_recipient_details[0].InvoiceRecipientAddressSet
                if len(invoice_recipient_einvoice_address) > 0:
                    self.einvoice_address = invoice_recipient_einvoice_address[0].text
                invoice_recipient_intermediator_address = invoice_recipient_details[0].InvoiceRecipientIntermediatorAddressSet
                if len(invoice_recipient_intermediator_address) > 0:
                    self.intermediator_address = invoice_recipient_intermediator_address[0].text
...
class DeliveryParty(object):
    ...
    def __init__(self, doc):
        ...
        if len(delivery_comm_details) > 0:
            email_address = delivery_comm_details[0].DeliveryEmailaddressIdentifierSet
        ...
        if len(delivery_party_details) > 0:
            postal_address = delivery_party_details[0].DeliveryPostalAddressDetailsSet
            if len(postal_address) > 0:
                self.address = DeliveryAddress(postal_address[0])
            organisation_name = delivery_party_details[0].DeliveryOrganisationNameSet
            if len(organisation_name) > 0:
...
class Payee(object):
    ...
    def __init__(self, doc, envelope=None):
        ...
        if len(org_unit_number) > 0:
            ...
            if not self.business_id and str(self.organisation_unit_number).startswith('0037') and len(self.organisation_unit_number) > 11:
        ...
        if len(details) > 0:
            ...
            if not self.business_id and len(business_id) > 0 and business_id[0].text:
...
class Payer(object):
    ...
    def __init__(self, doc=None, envelope=None):
        ...
        if len(org_unit_number) > 0 and org_unit_number[0].text:
            self.organisation_unit_number = org_unit_number[0].text
            if len(self.business_id) == 0 and self.organisation_unit_number.startswith('0037') and len(self.organisation_unit_number) > 10:
                business_id = self.organisation_unit_number[4:12]
                if BusinessID.is_valid(business_id):
                    self.business_id = business_id
        details = doc.BuyerPartyDetailsSet
        if len(details) > 0:
            self.name = details[0].BuyerOrganisationNameSet.as_string(delim=' ')
            ...
            if len(taxcode) > 0 and taxcode[0].text:
                ...
                if len(self.business_id) == 0:
                    if BusinessID.is_valid(self.organisation_taxcode):
                        self.business_id = str(BusinessID(self.organisation_taxcode))
    ...
    def __init__(self, row):
        ...
        def get_QuantitySetData(set):
            if len(set) > 0:
                ...
                try:
                    ...
                    unit_codes = [
                     set[0].attributes.get('quantityunitcode', '').strip()]
                    for q in set[1:]:
                        unit_code = ('%s %s' % (
                         q.text,
        ...
        if len(unit_price) > 0 and unit_price[0]:
            ...
            if len(_unit_price) > 0:
                ...
                currency = unit_price[0].attributes.get('amountcurrencyidentifier', '')
        ...
        if len(total_taxed) > 0 and total_taxed[0].text:
            ...
            if len(_total_taxed) > 0:
                ...
                currency = total_taxed[0].attributes.get('amountcurrencyidentifier', '')
        ...
        if len(vat_amount) and vat_amount[0].text:
            ...
            if len(_vat_amount) > 0:
                ...
                currency = vat_amount[0].attributes.get('amountcurrencyidentifier', '')
...
class FinvoiceWrapper(object):
    ...
    def check_finvoice_version(self, doc):
        if doc.version != '1.2':
            self.warnings.append(Warning('Finvoice version used is %s. Current implementation was written for version 1.2.'))

    def validate_invoice_totals(self):
        if self.invoice.total_taxed:
            total_taxed = sum([ row.total_taxed for row in self.rows if row.total_taxed != None
                              ])
            if total_taxed != self.invoice.total_taxed:
                description = 'Value of the element InvoiceTotalVatIncludedAmount does not match with the sum of RowAmount elements in the InvoiceRow aggregate.'
