...
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/PyGS/LuminosityFunction.py
# Compiled at: 2013-11-06 04:12:01
...
try:
    from fort.luminosity import lumfunc_denom
    fort_routines = True
except:
...
def absmag_to_lum(absmag):
    ...
    """
    lum = np.log10(3.839 * 100000000000000000000000000) + (4.83 - absmag) / 2.512
    return lum
...
def mag_bounds_optimal(absmag, Np, min_acceptable=1, verbose=True):
    ...
    return (
     final_min_m, final_max_m)


def EEP(absmag, apmag, Np, min_acceptable=1, eps=0.01, max_iter=10, verbose=True):
    """
    ...
    phi[0] = np.histogram(absmag, bins=bin_edges)[0] / deltaM
    Hmat = lumfunc_denom.get_hmatrix(np=Np, ngal=len(absmag), absmagmax=np.asfortranarray(max_absmag), bins=np.asfortranarray(bin_edges), deltam=deltaM)
    difference = 10.0
    l = 0
    while l < max_iter and difference > eps:
        l += 1
        denom = lumfunc_denom.denom(np=Np, ngal=len(absmag), phi=np.asfortranarray(phi[(l - 1)]), deltam=deltaM, hmat=np.asfortranarray(Hmat))
        print 'Iteration ', l
        phi[l] = phi[0] / denom
...
def vmax_method(absmag, appmag, c_dist, redshift, bins=100):
    ...
    weights = 1.0 / max_dist ** 3
    weights = weights / np.sum(weights)
    hist, edges = np.histogram(absmag, bins=bins, weights=weights, density=True, range=(-22,
                                                                                        -16))
    M = (edges[1:] + edges[:-1]) / 2
    return (
     hist, M)
