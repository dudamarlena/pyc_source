...
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/PyGS/Correlations.py
# Compiled at: 2013-10-10 20:39:22
...
import fort.corr as corr, cosmolopy.distance as cd, SelectionFunction as sf
...
def _correlation_function_dr_wrapper(pos1, pos2, end_r, n_r, n_cores):
    ...
    Simply a wrapper for the fortran correlation function routines
    """
    correlation = corr.correlation_dr(pos1=np.asfortranarray(pos1), pos2=np.asfortranarray(pos2), end_r=end_r, nR=n_r, n_cores=n_cores)
    return correlation
...
def _correlation_1d_DR_wrapper(pos1, pos2, end_s, ns, n_cores):
    ...
    pos1 = np.sort(pos1)
    pos2 = np.sort(pos2)
    correlation = corr.correlation_dr_1d(nr=ns, end_r=end_s, n=len(pos1), pos1=np.asfortranarray(pos1), n2=len(pos2), pos2=np.asfortranarray(pos2), n_cores=n_cores)
