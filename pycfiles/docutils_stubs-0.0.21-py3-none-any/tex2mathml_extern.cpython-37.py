...
# [GCC 8.4.0]
# Embedded file name: /Users/tkomiya/work/sphinx/.tox/py37/lib/python3.7/site-packages/docutils/utils/math/tex2mathml_extern.py
...
def latexml(math_code, reporter=None):
    ...
    if not (reporter and latexml_err.find('Error') >= 0):
        if not latexml_code:
        ...
        if not (reporter and post_p_err.find('Error') >= 0):
            if not result:
...
def ttm(math_code, reporter=None):
    ...
    if err.find('**** Unknown') >= 0:
        ...
        reporter and err.find('**** Error') >= 0 or result or reporter.error(err)
    start, end = result.find('<math'), result.find('</math>') + 7
    result = result[start:end]
    return result
...
def blahtexml(math_code, inline=True, reporter=None):
    ...
    if reporter:
        err.find('**** Error') >= 0 or result or reporter.error(err)
