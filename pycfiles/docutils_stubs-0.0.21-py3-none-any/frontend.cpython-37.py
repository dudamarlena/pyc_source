...
# [GCC 8.4.0]
# Embedded file name: /Users/tkomiya/work/sphinx/.tox/py37/lib/python3.7/site-packages/docutils/frontend.py
# Compiled at: 2018-11-25 06:19:18
# Size of source mod 2**32: 36893 bytes
"""
...
import os, os.path, sys, warnings, configparser as CP, codecs, optparse
from optparse import SUPPRESS_HELP
import docutils, docutils.utils, docutils.nodes
from docutils.utils.error_reporting import locale_encoding, SafeString, ErrorOutput, ErrorString
...
def validate_boolean(setting, value, option_parser, config_parser=None, config_section=None):
...
def validate_comma_separated_list(setting, value, option_parser, config_parser=None, config_section=None):
...
def validate_strip_class(setting, value, option_parser, config_parser=None, config_section=None):
...
class OptionParser(optparse.OptionParser, docutils.SettingsSpec):
    ...
    thresholds = {'info':1, 
     'warning':2,  'error':3,  'severe':4,  'none':5}
    booleans = {'1':True, 
     'on':True,  'yes':True,  'true':True,  '0':False, 
     'off':False,  'no':False,  'false':False,  '':False}
    default_error_encoding = getattr(sys.stderr, 'encoding', None) or locale_encoding or 'ascii'
    ...
    settings_spec = (
     ...
     (
      ...
      (
       ...
       [
        '--date', '-d'],
       {'action':'store_const',  'const':'%Y-%m-%d',  'dest':'datestamp'}),
      (
      ...
      (
       ...
       [
        '--no-datestamp'],
       {'action':'store_const',  'const':None,  'dest':'datestamp'}),
      (
      ...
      (
       ...
       [
        '--no-source-link'],
       {'action':'callback', 
        'callback':store_multiple,  'callback_args':('source_link', 'source_url')}),
      (
       'Link from section headers to TOC entries.  (default)',
       [
        '--toc-entry-backlinks'],
       {'dest':'toc_backlinks', 
        'action':'store_const',  'const':'entry',  'default':'entry'}),
      (
      ...
      (
       ...
       {'action':'append', 
        'dest':'strip_elements_with_classes',  'metavar':'<class>', 
      ...
      (
       ...
       [
        '--traceback'],
       {'action':'store_true',  'default':None,  'validator':validate_boolean}),
      (
      ...
      (
       ...
       [
        '--output-encoding', '-o'],
       {'metavar':'<name[:handler]>', 
        'default':'utf-8',  'validator':validate_encoding_and_error_handler}),
      (
       ...
       {'default':'strict', 
        'validator':validate_encoding_error_handler}),
      (
       'Specify text encoding and error handler for error output.  Default: %s:%s.' % (
        default_error_encoding, default_error_encoding_error_handler),
       [
        '--error-encoding', '-e'],
       {'metavar':'<name[:handler]>', 
        'default':default_error_encoding,  'validator':validate_encoding_and_error_handler}),
      (
       'Specify the error handler for unencodable characters in error output.  Default: %s.' % default_error_encoding_error_handler,
       [
      ...
      (
       ...
       [
        '--record-dependencies'],
       {'metavar':'<file>', 
        'validator':validate_dependency_file,  'default':None}),
      (
       'Read configuration settings from <file>, if it exists.',
       [
        '--config'],
       {'metavar':'<file>',  'type':'string',  'action':'callback', 
      ...
      (
       ...
       {'action':'append', 
        'dest':'expose_internals',  'validator':validate_colon_separated_string_list}),
      (
 ...
 self, *args, option_class=Option, add_help_option=None, formatter=optparse.TitledHelpFormatter(width=78), **kwargs)
        if not self.version:
        ...
        for component in components:
            ...
            for i in range(0, len(settings_spec), 3):
                ...
                for help_text, option_strings, kwargs in option_spec:
                    option = (group.add_option)(option_strings, help=help_text, **kwargs)
    ...
    def check_args(self, args):
        ...
        if source:
            if source == destination:
                self.error('Do not specify the same file for both source and destination.  It will clobber the source file.')
        return (
         source, destination)
...
class ConfigParser(CP.RawConfigParser):
    old_settings = {'pep_stylesheet':('pep_html writer', 'stylesheet'), 
        ...
        for key, value in list(options.items()):
            if key in self.old_settings:
                ...
                if not self.has_section(section):
                    ...
                    section = 'general'
                    setting = key
                self.has_option(section, setting) or self.set(section, setting, value)
                ...
                if option.validator:
                    ...
                    try:
                        new_value = option.validator(setting,
                          value, option_parser, config_parser=self,
...
class ConfigDeprecationWarning(DeprecationWarning):
    __doc__ = 'Warning for deprecated configuration file features.'