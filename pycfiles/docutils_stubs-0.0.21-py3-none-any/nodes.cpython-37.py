...
# [GCC 8.4.0]
# Embedded file name: /Users/tkomiya/work/sphinx/.tox/py37/lib/python3.7/site-packages/docutils/nodes.py
# Compiled at: 2018-12-02 05:57:09
# Size of source mod 2**32: 77200 bytes
"""
...
import sys, os, re, warnings, types, unicodedata

class Node(object):
    __doc__ = 'Abstract base class of nodes in a document tree.'
    parent = None
    document = None
    source = None
    line = None
        ...
        * the siblings of the parent (if ascend is true) and their
        ...
        if ascend:
            ...
            if include_self:
                ...
                r = []
                if not include_self or condition is None or condition(self):
                    r.append(self)
            if descend and len(self.children):
                for child in self:
                    r.extend(child.traverse(include_self=True, descend=True, siblings=False,
                      ascend=False,
                      condition=condition))
        ...
        if siblings or ascend:
            ...
            while node.parent:
                ...
                for sibling in node.parent[index + 1:]:
                    r.extend(sibling.traverse(include_self=True, descend=descend,
                      siblings=False,
                      ascend=False,
                      condition=condition))
    ...
    def next_node(self, condition=None, include_self=False, descend=True, siblings=False, ascend=False):
        ...
        """
        iterable = self.traverse(condition=condition, include_self=include_self,
          descend=descend,
          siblings=siblings,
          ascend=ascend)
        try:
...
class Element(Node):
    ...
    def emptytag(self):
        return '<%s/>' % ' '.join([self.tagname] + ['%s="%s"' % (n, v) for n, v in self.attlist()])
        ...
        if isinstance(key, str):
            ...
            if isinstance(key, slice):
                if not key.step in (None, 1):
                    raise AssertionError('cannot handle slice with stride')
        else:
    ...
    def update_all_atts_coercion(self, dict_, replace=True, and_source=False):
        """
        Updates all attributes from node or dictionary `dict_`.
    ...
    def replace_self(self, new):
        ...
        elif isinstance(update, Element):
            update.update_basic_atts(self)
        else:
            for att in self.basic_attributes:
                if self[att]:
                    raise AssertionError('Losing "%s" attribute: %s' % (att, self[att]))
    ...
    def pformat(self, indent='    ', level=0):
        return ''.join(['%s%s\n' % (indent * level, self.starttag())] + [child.pformat(indent, level + 1) for child in self.children])
    ...
    def set_class(self, name):
        ...
        warnings.warn("docutils.nodes.Element.set_class deprecated; append to Element['classes'] list attribute directly", DeprecationWarning,
          stacklevel=2)
        assert ' ' not in name
...
class FixedTextElement(TextElement):
    __doc__ = 'An element which directly contains preformatted text.'
...
class PreBibliographic:
    __doc__ = 'Category of Node which may occur before Bibliographic Nodes.'
...
class Special(Body):
    __doc__ = 'Special internal body elements.'


class Invisible(PreBibliographic):
    __doc__ = "Internal elements that don't appear in output."
...
class Labeled:
    __doc__ = 'Contains a `label` as its first element.'
...
class document(Root, Structural, Element):
        ...
        if explicit:
            if old_explicit:
                ...
                if old_id is not None:
                    ...
                    if 'refuri' in node:
                        refuri = node['refuri']
                        if old_node['names']:
                            if 'refuri' in old_node:
                                if old_node['refuri'] == refuri:
                                    level = 1
                    if level > 1:
                        dupname(old_node, name)
                        self.nameids[name] = None
                msg = self.reporter.system_message(level,
                  ('Duplicate explicit target name: "%s".' % name), backrefs=[
                 id],
                  base_node=node)
                if msgnode != None:
                    msgnode += msg
            ...
            if old_id is not None:
                if not old_explicit:
                    ...
                    dupname(old_node, name)
            dupname(node, name)
        if explicit and not old_explicit:
            if old_id is not None:
                msg = self.reporter.info(('Duplicate implicit target name: "%s".' % name),
                  backrefs=[
                 id],
                  base_node=node)
                if msgnode != None:
    ...
    def note_substitution_def(self, subdef, def_name, msgnode=None):
        ...
        if name in self.substitution_defs:
            msg = self.reporter.error(('Duplicate substitution definition name: "%s".' % name),
              base_node=subdef)
            if msgnode != None:
                msgnode += msg
...
class decoration(Decorative, Element):
    ...
    def get_header(self):
        if not (len(self.children) and isinstance(self.children[0], header)):
            self.insert(0, header())
        return self.children[0]

    def get_footer(self):
        if not (len(self.children) and isinstance(self.children[(-1)], footer)):
            self.append(footer())
...
class pending(Special, Invisible, Element):
    ...
    def copy(self):
        return (self.__class__)((self.transform), (self.details), (self.rawsource), **self.attributes)
...
class NodeVisitor:
    ...
    def dispatch_visit(self, node):
        ...
        self.document.reporter.debug('docutils.nodes.NodeVisitor.dispatch_visit calling %s for %s' % (
    ...
    def dispatch_departure(self, node):
        ...
        self.document.reporter.debug('docutils.nodes.NodeVisitor.dispatch_departure calling %s for %s' % (
         method.__name__, node_name))
        return method(node)
...
_non_id_translate = {248:'o', 
 ...
 589:'r', 
 591:'y'}
_non_id_translate_digraphs = {223:'sz', 
 230:'ae', 
 339:'oe', 
 568:'db', 
 569:'qp'}
