...
# [GCC 8.4.0]
# Embedded file name: /Users/tkomiya/work/sphinx/.tox/py37/lib/python3.7/site-packages/docutils/parsers/rst/roles.py
# Compiled at: 2018-11-25 06:19:18
# Size of source mod 2**32: 14779 bytes
"""
...
def role(role_name, language_module, lineno, reporter):
    ...
    if normname in _roles:
        ...
        if role_name:
            ...
            try:
                ...
                try:
                    msg_text.append('Problem retrieving role entry from language module %r: %s.' % (
                     language_module, error))
                finally:
                    error = None
                    del error
    ...
    if not canonicalname:
        try:
            ...
            msg_text.append('Using English fallback for role "%s".' % role_name)
        except KeyError:
            msg_text.append('Trying "%s" as canonical role name.' % role_name)
...
def set_implicit_options(role_fn):
    ...
    if not hasattr(role_fn, 'options') or role_fn.options is None:
        ...
        if 'class' not in role_fn.options:
            role_fn.options['class'] = directives.class_option
    ...
    def __call__(self, role, rawtext, text, lineno, inliner, options={}, content=[]):
        ...
        if cont:
            if content:
                cont += '\n'
        cont.extend(content)
        return self.base_role(role, rawtext, text, lineno, inliner, options=opts,
          content=cont)
...
def pep_reference_role(role, rawtext, text, lineno, inliner, options={}, content=[]):
    try:
        ...
        msg = inliner.reporter.error(('PEP number must be a number from 0 to 9999; "%s" is invalid.' % text),
          line=lineno)
        ...
        return (
         [
          (nodes.reference)(rawtext, 'PEP ' + utils.unescape(text), refuri=ref, **options)], [])
...
def rfc_reference_role(role, rawtext, text, lineno, inliner, options={}, content=[]):
    try:
        ...
        msg = inliner.reporter.error(('RFC number must be a number greater than or equal to 1; "%s" is invalid.' % text),
          line=lineno)
        prb = inliner.problematic(rawtext, rawtext, msg)
        return ([prb], [msg])
    else:
...
def unimplemented_role(role, rawtext, text, lineno, inliner, attributes={}):
    msg = inliner.reporter.error(('Interpreted text role "%s" not implemented.' % role),
      line=lineno)
    prb = inliner.problematic(rawtext, rawtext, msg)
    return ([prb], [msg])
