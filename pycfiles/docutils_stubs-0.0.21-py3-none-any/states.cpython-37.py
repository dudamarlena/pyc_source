...
# [GCC 8.4.0]
# Embedded file name: /Users/tkomiya/work/sphinx/.tox/py37/lib/python3.7/site-packages/docutils/parsers/rst/states.py
# Compiled at: 2018-11-25 06:19:18
# Size of source mod 2**32: 131353 bytes
"""
...
import docutils.nodes as normalize_name
from docutils.nodes import whitespace_normalize_name
import docutils.parsers.rst
from docutils.parsers.rst import directives, languages, tableparser, roles
import docutils.parsers.rst.languages as _fallback_language_module
...
class Struct:
    __doc__ = 'Stores data attributes for dotted-attribute access.'
...
class RSTStateMachine(StateMachineWS):
    ...
    def run(self, input_lines, document, input_offset=0, match_titles=True, inliner=None):
        ...
        self.memo = Struct(document=document, reporter=(document.reporter),
...
class RSTState(StateWS):
    ...
    def no_match(self, context, transitions):
        ...
        """
        self.reporter.severe('Internal error: no transition pattern match.  State: "%s"; transitions: %s; context: %s; current line: %r.' % (
    ...
    def nested_parse(self, block, input_offset, node, match_titles=False, state_machine_class=None, state_machine_kwargs=None):
        ...
        if state_machine_class is None:
            ...
            if use_default == 2:
                ...
                state_machine.run(block, input_offset, memo=(self.memo), node=node,
    ...
    def title_inconsistent(self, sourcetext, lineno):
        error = self.reporter.severe('Title level inconsistent:',
          (nodes.literal_block('', sourcetext)), line=lineno)
        return error
    ...
    def new_subsection(self, title, lineno, messages):
        ...
        newabsoffset = self.nested_parse((self.state_machine.input_lines[offset:]),
          input_offset=absoffset, node=section_node,
          match_titles=True)
    ...
    def unindent_warning(self, node_name):
        ...
        return self.reporter.warning(('%s ends without a blank line; unexpected unindent.' % node_name),
          line=lineno)
...
class Inliner:
    ...
    def init_customizations(self, settings):
        ...
        parts = (
         ...
         [
          (
           'start', '', self.non_whitespace_after,
           [
            '\\*\\*',
            '\\*(?!\\*)',
            '``',
            ...
            '\\|(?!\\|)']),
          (
           'whole', '', end_string_suffix,
           [
            '(?P<refname>%s)(?P<refend>__?)' % self.simplename,
            (
             'footnotelabel', '\\[', '(?P<fnend>\\]_)',
             [
              '[0-9]+',
              '\\#(%s)?' % self.simplename,
              '\\*',
              '(?P<citationlabel>%s)' % self.simplename])]),
          (
           'backquote',
           '(?P<role>(:%s:)?)' % self.simplename,
        ...
        self.parts = parts
        self.patterns = Struct(initial=(build_regexp(parts)),
          emphasis=(re.compile(self.non_whitespace_escape_before + '(\\*)' + end_string_suffix, re.UNICODE)),
          strong=(re.compile(self.non_whitespace_escape_before + '(\\*\\*)' + end_string_suffix, re.UNICODE)),
          ...
          literal=(re.compile(self.non_whitespace_before + '(``)' + end_string_suffix, re.UNICODE)),
          target=(re.compile(self.non_whitespace_escape_before + '(`)' + end_string_suffix, re.UNICODE)),
          substitution_ref=(re.compile(self.non_whitespace_escape_before + '(\\|_{0,2})' + end_string_suffix, re.UNICODE)),
    ...
    def parse(self, text, lineno, memo, parent):
        ...
        while remaining:
            ...
            if match:
                ...
                method = dispatch[(groups['start'] or groups['backquote'] or groups['refend'] or groups['fnend'])]
    ...
    uri_end_delim = '[>]'
    urilast = '[_~*/=+a-zA-Z0-9]'
    uri_end = '(?:%(urilast)s|%(uric)s(?=%(uri_end_delim)s))' % locals()
    ...
    def inline_obj(self, match, lineno, end_pattern, nodeclass, restore_backslashes=False):
        ...
        if self.quoted_start(match):
            return (
             string[:matchend], [], string[matchend:], [], '')
        endmatch = end_pattern.search(string[matchend:])
        if endmatch:
            if endmatch.start(1):
                text = unescape(endmatch.string[:endmatch.start(1)], restore_backslashes)
                textend = matchend + endmatch.end(1)
                rawsource = unescape(string[matchstart:textend], 1)
                return (string[:matchstart], [nodeclass(rawsource, text)],
                 string[textend:], [], endmatch.group(1))
        msg = self.reporter.warning(('Inline %s start-string without end-string.' % nodeclass.__name__),
          line=lineno)
    ...
    def emphasis(self, match, lineno):
        before, inlines, remaining, sysmessages, endstring = self.inline_obj(match, lineno, self.patterns.emphasis, nodes.emphasis)
        return (before, inlines, remaining, sysmessages)

    def strong(self, match, lineno):
        before, inlines, remaining, sysmessages, endstring = self.inline_obj(match, lineno, self.patterns.strong, nodes.strong)
        return (before, inlines, remaining, sysmessages)
    ...
    def interpreted_or_phrase_ref(self, match, lineno):
        ...
        if role:
            ...
            if self.quoted_start(match):
                ...
                if endmatch and endmatch.start(1):
                    ...
                    if endmatch.group('role'):
                        if role:
                            msg = self.reporter.warning('Multiple roles in interpreted text (both prefix and suffix present; only one allowed).',
                              line=lineno)
                    ...
                    if rawsource[-1:] == '_':
                        if role:
                            msg = self.reporter.warning(('Mismatch: both interpreted text role %s and reference suffix.' % position),
                              line=lineno)
                            text = unescape(string[rolestart:textend], 1)
                            prb = self.problematic(text, text, msg)
                            return (string[:rolestart], [prb], string[textend:], [msg])
            ...
            msg = self.reporter.warning('Inline interpreted text or phrase reference start-string without end-string.',
              line=lineno)
            text = unescape(string[matchstart:matchend], 1)
            prb = self.problematic(text, text, msg)
            return (string[:matchstart], [prb], string[matchend:], [msg])
    ...
    def phrase_ref(self, before, after, rawsource, escaped, text):
        ...
        if match:
            ...
            if aliastext.endswith('_'):
                aliastype = underscore_escaped or self.patterns.uri.match(aliastext) or 'name'
    ...
    def interpreted(self, rawsource, text, role, lineno):
        ...
        msg = self.reporter.error(('Unknown interpreted text role "%s".' % role),
          line=lineno)
        return ([self.problematic(rawsource, rawsource, msg)],
         messages + [msg])

    def literal(self, match, lineno):
        before, inlines, remaining, sysmessages, endstring = self.inline_obj(match,
          lineno, (self.patterns.literal), (nodes.literal), restore_backslashes=True)
        return (before, inlines, remaining, sysmessages)
    ...
    def inline_internal_target(self, match, lineno):
        ...
        if inlines:
            if isinstance(inlines[0], nodes.target):
    ...
    def reference(self, match, lineno, anonymous=False):
        ...
        referencenode = nodes.reference((referencename + match.group('refend')),
          referencename, name=(whitespace_normalize_name(referencename)))
        if anonymous:
    ...
    def standalone_uri(self, match, lineno):
        if not match.group('scheme') or match.group('scheme').lower() in urischemes.schemes:
            ...
            return [
             nodes.reference((unescape(text, 1)), unescaped, refuri=(addscheme + unescaped))]
    ...
    def pep_reference(self, match, lineno):
        ...
        if text.startswith('pep-'):
            ...
            if text.startswith('PEP'):
                ...
                raise MarkupMismatch
        ref = self.document.settings.pep_base_url + self.document.settings.pep_file_url_template % pepnum
        unescaped = unescape(text, 0)
    ...
    def implicit_inline(self, text, lineno):
        ...
        return [
         nodes.Text((unescape(text)), rawsource=(unescape(text, 1)))]
...
class Body(RSTState):
    ...
    enum = Struct()
    enum.formatinfo = {'parens':Struct(prefix='(', suffix=')', start=1, end=-1), 
    ...
    enum.sequencepats = {'arabic':'[0-9]+',  'loweralpha':'[a-z]', 
     'upperalpha':'[A-Z]', 
     'lowerroman':'[ivxlcdm]+', 
     'upperroman':'[IVXLCDM]+'}
    enum.converters = {'arabic':int,  'loweralpha':_loweralpha_to_int, 
     'upperalpha':_upperalpha_to_int, 
     'lowerroman':_lowerroman_to_int, 
     'upperroman':roman.fromRoman}
    enum.sequenceregexps = {}
    for sequence in enum.sequences:
        enum.sequenceregexps[sequence] = re.compile(enum.sequencepats[sequence] + '$', re.UNICODE)
    ...
    pats['longopt'] = '(--|/)%(optname)s([ =]%(optarg)s)?' % pats
    ...
    patterns = {'bullet':'[-+*•‣⁃]( +|$)', 
     'enumerator':'(%(parens)s|%(rparen)s|%(period)s)( +|$)' % pats, 
     'field_marker':':(?![: ])([^:\\\\]|\\\\.)*(?<! ):( +|$)', 
     'option_marker':'%(option)s(, %(option)s)*(  +| ?$)' % pats, 
     'doctest':'>>>( +|$)', 
     'line_block':'\\|( +|$)', 
     ...
     'explicit_markup':'\\.\\.( +|$)', 
     'anonymous':'__( +|$)', 
     'line':'(%(nonalphanum7bit)s)\\1* *$' % pats, 
     'text':''}
    initial_transitions = ('bullet', 'enumerator', 'field_marker', 'option_marker',
                           'doctest', 'line_block', 'grid_table_top', 'simple_table_top',
    ...
    def block_quote(self, indented, line_offset):
        ...
        while indented:
            ...
            while indented:
                indented = indented[0] or indented[1:]
                line_offset += 1
    ...
    def bullet(self, match, context, next_state):
        ...
        bulletlist += i
        offset = self.state_machine.line_offset + 1
        new_line_offset, blank_finish = self.nested_list_parse((self.state_machine.input_lines[offset:]),
    ...
    def enumerator(self, match, context, next_state):
        ...
        if ordinal != 1:
            ...
            msg = self.reporter.info('Enumerated list start value not ordinal-1: "%s" (ordinal %s)' % (
             text, ordinal))
            self.parent += msg
        listitem, blank_finish = self.list_item(match.end())
        enumlist += listitem
        offset = self.state_machine.line_offset + 1
        newline_offset, blank_finish = self.nested_list_parse((self.state_machine.input_lines[offset:]),
          input_offset=(self.state_machine.abs_line_offset() + 1),
          node=enumlist,
                ...
                if sequence.endswith('alpha'):
                    if ordinal > 26:
                        ...
                        if sequence.endswith('roman'):
                            ...
                            raise ParserError('unknown enumerator sequence: "%s"' % sequence)
                elif sequence.startswith('lower'):
                    enumerator = enumerator.lower()
                else:
                    if sequence.startswith('upper'):
    ...
    def parse_option_marker(self, match):
        ...
        for optionstring in optionstrings:
            ...
            if len(firstopt) > 1:
                ...
                if len(tokens[0]) > 2:
                    if not tokens[0].startswith('-') or tokens[0].startswith('--'):
            ...
            if len(tokens) > 1 and tokens[1].startswith('<'):
                ...
                raise MarkupError('wrong number of option tokens (=%s), should be 1 or 2: "%s"' % (
                 len(tokens), optionstring))
    ...
    def line_block(self, match, context, next_state):
        ...
        if not blank_finish:
            ...
            self.goto_line(new_line_offset)
        if not blank_finish:
            self.parent += self.reporter.warning('Line block ends without a blank line.',
              line=(lineno + 1))
        if len(block):
    ...
    def table_top(self, match, context, next_state, isolate_function, parser_class):
        ...
        if not blank_finish:
            msg = self.reporter.warning('Blank line required after table.',
    ...
    def table(self, isolate_function, parser_class):
        ...
        if block:
            try:
                ...
                try:
                    nodelist = self.malformed_table(block, (' '.join(err.args)), offset=(err.offset)) + messages
                finally:
                    err = None
                    del err
    ...
    def isolate_grid_table(self):
        ...
        try:
            ...
            try:
                ...
                messages.append(self.reporter.error('Unexpected indentation.', source=src,
                  line=srcline))
                blank_finish = 0
            finally:
                err = None
        ...
        return (
         block, messages, blank_finish)

    def isolate_simple_table--- This code section failed: ---

 L.1718         0  LOAD_FAST                'self'
                2  LOAD_ATTR                state_machine
                4  LOAD_ATTR                line_offset
                6  STORE_FAST               'start'

 L.1719         8  LOAD_FAST                'self'
               10  LOAD_ATTR                state_machine
               12  LOAD_ATTR                input_lines
               14  STORE_FAST               'lines'

 L.1720        16  LOAD_GLOBAL              len
               18  LOAD_FAST                'lines'
 ...
 L.1721        28  LOAD_GLOBAL              len
               30  LOAD_FAST                'lines'
               32  LOAD_FAST                'start'
               34  BINARY_SUBSCR    
               36  LOAD_METHOD              strip
               38  CALL_METHOD_0         0  '0 positional arguments'
               40  CALL_FUNCTION_1       1  '1 positional argument'
               42  STORE_FAST               'toplen'

 L.1722        44  LOAD_FAST                'self'
               46  LOAD_ATTR                simple_table_border_pat
               48  LOAD_ATTR                match
               50  STORE_FAST               'pattern_match'

 L.1723        52  LOAD_CONST               0
               54  STORE_FAST               'found'

 L.1724        56  LOAD_CONST               None
               58  STORE_FAST               'found_at'

 L.1725        60  LOAD_FAST                'start'
               62  LOAD_CONST               1
               64  BINARY_ADD       
               66  STORE_FAST               'i'

 L.1726     68_70  SETUP_LOOP          358  'to 358'
               72  LOAD_FAST                'i'
               74  LOAD_FAST                'limit'
               76  COMPARE_OP               <=
               78  POP_JUMP_IF_FALSE   248  'to 248'

 L.1727        80  LOAD_FAST                'lines'
               82  LOAD_FAST                'i'
               84  BINARY_SUBSCR    
               86  STORE_FAST               'line'

 L.1728        88  LOAD_FAST                'pattern_match'
               90  LOAD_FAST                'line'
               92  CALL_FUNCTION_1       1  '1 positional argument'
               94  STORE_FAST               'match'

 L.1729        96  LOAD_FAST                'match'
               98  POP_JUMP_IF_FALSE   238  'to 238'

 L.1730       100  LOAD_GLOBAL              len
              102  LOAD_FAST                'line'
              104  LOAD_METHOD              strip
              106  CALL_METHOD_0         0  '0 positional arguments'
              108  CALL_FUNCTION_1       1  '1 positional argument'
              110  LOAD_FAST                'toplen'
              112  COMPARE_OP               !=
              114  POP_JUMP_IF_FALSE   188  'to 188'

 L.1731       116  LOAD_FAST                'self'
              118  LOAD_ATTR                state_machine
              120  LOAD_METHOD              next_line
              122  LOAD_FAST                'i'
              124  LOAD_FAST                'start'
              126  BINARY_SUBTRACT  
 ...
 L.1732       132  LOAD_FAST                'self'
              134  LOAD_METHOD              malformed_table

 L.1733       136  LOAD_FAST                'lines'
              138  LOAD_FAST                'start'
              140  LOAD_FAST                'i'
              142  LOAD_CONST               1
              144  BINARY_ADD       
              146  BUILD_SLICE_2         2 
              148  BINARY_SUBSCR    
              150  LOAD_STR                 'Bottom/header table border does not match top border.'
              152  CALL_METHOD_2         2  '2 positional arguments'
              154  STORE_FAST               'messages'

 L.1735       156  BUILD_LIST_0          0 
              158  LOAD_FAST                'messages'
              160  LOAD_FAST                'i'
              162  LOAD_FAST                'limit'
              164  COMPARE_OP               ==
              166  JUMP_IF_TRUE_OR_POP   184  'to 184'
              168  LOAD_FAST                'lines'
              170  LOAD_FAST                'i'
              172  LOAD_CONST               1
              174  BINARY_ADD       
            ...
            184_0  COME_FROM           166  '166'
              184  BUILD_TUPLE_3         3 
              186  RETURN_VALUE     
            188_0  COME_FROM           114  '114'

 L.1736       188  LOAD_FAST                'found'
              190  LOAD_CONST               1
              192  INPLACE_ADD      
              194  STORE_FAST               'found'

 L.1737       196  LOAD_FAST                'i'
              198  STORE_FAST               'found_at'

 L.1738       200  LOAD_FAST                'found'
              202  LOAD_CONST               2
              204  COMPARE_OP               ==
              ...
              222  BINARY_ADD       
              224  BINARY_SUBSCR    
              226  LOAD_METHOD              strip
              228  CALL_METHOD_0         0  '0 positional arguments'
              230  POP_JUMP_IF_TRUE    238  'to 238'
            232_0  COME_FROM           214  '214'
            232_1  COME_FROM           206  '206'

 L.1739       232  LOAD_FAST                'i'
              234  STORE_FAST               'end'

 L.1740       236  BREAK_LOOP       
            238_0  COME_FROM           230  '230'
            238_1  COME_FROM            98  '98'

 L.1741       238  LOAD_FAST                'i'
              240  LOAD_CONST               1
              242  INPLACE_ADD      
              244  STORE_FAST               'i'
              246  JUMP_BACK            72  'to 72'
            248_0  COME_FROM            78  '78'
              248  POP_BLOCK        

 L.1743       250  LOAD_FAST                'found'
          252_254  POP_JUMP_IF_FALSE   294  'to 294'

 L.1744       256  LOAD_STR                 ' or no blank line after table bottom'
              258  STORE_FAST               'extra'

 L.1745       260  LOAD_FAST                'self'
              262  LOAD_ATTR                state_machine
              264  LOAD_METHOD              next_line
              266  LOAD_FAST                'found_at'
              268  LOAD_FAST                'start'
              270  BINARY_SUBTRACT  
              272  CALL_METHOD_1         1  '1 positional argument'
              274  POP_TOP          

 L.1746       276  LOAD_FAST                'lines'
              278  LOAD_FAST                'start'
              280  LOAD_FAST                'found_at'
              282  LOAD_CONST               1
              284  BINARY_ADD       
              286  BUILD_SLICE_2         2 
              288  BINARY_SUBSCR    
              290  STORE_FAST               'block'
              292  JUMP_FORWARD        330  'to 330'
            294_0  COME_FROM           252  '252'

 L.1748       294  LOAD_STR                 ''
              296  STORE_FAST               'extra'

 L.1749       298  LOAD_FAST                'self'
              300  LOAD_ATTR                state_machine
              302  LOAD_METHOD              next_line
              304  LOAD_FAST                'i'
              306  LOAD_FAST                'start'
              308  BINARY_SUBTRACT  
              310  LOAD_CONST               1
 ...
 L.1750       318  LOAD_FAST                'lines'
              320  LOAD_FAST                'start'
              322  LOAD_CONST               None
              324  BUILD_SLICE_2         2 
              326  BINARY_SUBSCR    
              328  STORE_FAST               'block'
            330_0  COME_FROM           292  '292'

 L.1751       330  LOAD_FAST                'self'
              332  LOAD_METHOD              malformed_table

 L.1752       334  LOAD_FAST                'block'
              336  LOAD_STR                 'No bottom table border found%s.'
              338  LOAD_FAST                'extra'
              340  BINARY_MODULO    
              342  CALL_METHOD_2         2  '2 positional arguments'
              344  STORE_FAST               'messages'

 L.1753       346  BUILD_LIST_0          0 
              348  LOAD_FAST                'messages'
              350  LOAD_FAST                'extra'
              352  UNARY_NOT        
              354  BUILD_TUPLE_3         3 
              356  RETURN_VALUE     
            358_0  COME_FROM_LOOP       68  '68'

 L.1754       358  LOAD_FAST                'self'
              360  LOAD_ATTR                state_machine
              362  LOAD_METHOD              next_line
              364  LOAD_FAST                'end'
              366  LOAD_FAST                'start'
              368  BINARY_SUBTRACT  
 ...
 L.1755       374  LOAD_FAST                'lines'
              ...
              384  BUILD_SLICE_2         2 
              386  BINARY_SUBSCR    
              388  STORE_FAST               'block'

 L.1757       390  LOAD_FAST                'block'
              392  LOAD_METHOD              pad_double_width
              394  LOAD_FAST                'self'
              396  LOAD_ATTR                double_width_pad_char
              398  CALL_METHOD_1         1  '1 positional argument'
              400  POP_TOP          

 L.1758       402  LOAD_FAST                'block'
              404  BUILD_LIST_0          0 
              406  LOAD_FAST                'end'
              408  LOAD_FAST                'limit'
              410  COMPARE_OP               ==
          412_414  JUMP_IF_TRUE_OR_POP   432  'to 432'
              416  LOAD_FAST                'lines'
              418  LOAD_FAST                'end'
              420  LOAD_CONST               1
              422  BINARY_ADD       
...
Parse error at or near `COME_FROM' instruction at offset 238_0
    ...
    def citation(self, match):
        ...
        return (
         [
          citation], blank_finish)

    def hyperlink_target(self, match):
        pattern = self.explicit.patterns.target
        lineno = self.state_machine.abs_line_number()
        block, indent, offset, blank_finish = self.state_machine.get_first_known_indented((match.end()),
          until_blank=True, strip_indent=False)
    ...
    def is_reference(self, reference):
        match = self.explicit.patterns.reference.match(whitespace_normalize_name(reference))
        if not match:
            return
    ...
    def substitution_def(self, match):
        ...
        if not block:
            msg = self.reporter.warning(('Substitution definition "%s" missing contents.' % subname),
        ...
        new_abs_offset, blank_finish = self.nested_list_parse(block,
          input_offset=offset, node=substitution_node, initial_state='SubstitutionDef',
          blank_finish=blank_finish)
        i = 0
        ...
        for node in substitution_node.traverse(nodes.Element):
            if self.disallowed_inside_substitution_definitions(node):
                ...
                msg = self.reporter.error('Substitution definition contains illegal element:',
                  pformat,
                  (nodes.literal_block(blocktext, blocktext)), source=src,
                  line=srcline)
                return ([msg], blank_finish)

        if len(substitution_node) == 0:
            msg = self.reporter.warning(('Substitution definition "%s" empty or invalid.' % subname),
              (nodes.literal_block(blocktext, blocktext)),
              source=src,
              line=srcline)
            return ([msg], blank_finish)
    ...
    def disallowed_inside_substitution_definitions(self, node):
        if not ((node['ids'] or isinstance)(node, nodes.reference) and node.get('anonymous')):
            if not isinstance(node, nodes.footnote_reference) or node.get('auto'):
                return 1
        return 0
    ...
    def run_directive(self, directive, match, type_name, option_presets):
        ...
        for i in range(len(result)):
            assert isinstance(result[i], nodes.Node), 'Directive "%s" returned non-Node object (index %s): %r' % (
    ...
    def parse_directive_block(self, indented, line_offset, directive, option_presets):
        ...
        if indented:
            if not indented[0].strip():
                indented.trim_start()
                line_offset += 1
        else:
            while indented:
                indented[(-1)].strip() or indented.trim_end()

            if indented:
                if not directive.required_arguments:
                    if directive.optional_arguments or option_spec:
        ...
        if arg_block:
            if not directive.required_arguments:
                if not directive.optional_arguments:
                    content = arg_block + indented[i:]
                    content_offset = line_offset
                    arg_block = []
        while content:
            content[0].strip() or content.trim_start()
            content_offset += 1
    ...
    def parse_directive_arguments(self, directive, arg_block):
        ...
        if len(arguments) < required:
            ...
            if len(arguments) > required + optional:
                if directive.final_argument_whitespace:
                    arguments = arg_text.split(None, required + optional - 1)
                else:
                    raise MarkupError('maximum %s argument(s) allowed, %s supplied' % (
    ...
    def parse_extension_options(self, option_spec, datalines):
        ...
        newline_offset, blank_finish = self.nested_list_parse(datalines,
          0, node, initial_state='ExtensionOptions', blank_finish=True)
        ...
        try:
            ...
            try:
                return (
                 0, 'unknown option: "%s"' % detail.args[0])
            finally:
                detail = None
                del detail
            ...
            try:
                return (
                 0, 'invalid option value: %s' % ' '.join(detail.args))
            finally:
                detail = None
                del detail
            ...
            try:
                return (
                 0, 'invalid option data: %s' % ' '.join(detail.args))
            finally:
                detail = None
                del detail
    ...
    def line(self, match, context, next_state):
        ...
        if self.state_machine.match_titles:
            ...
            if len(match.string.strip()) < 4:
                ...
                msg = self.reporter.severe('Unexpected section title or transition.',
                  (nodes.literal_block(blocktext, blocktext)),
                  line=(self.state_machine.abs_line_number()))
                self.parent += msg
                return ([], next_state, [])
...
class BulletList(SpecializedBody):
    __doc__ = 'Second and subsequent bullet_list list_items.'
...
class DefinitionList(SpecializedBody):
    __doc__ = 'Second and subsequent definition_list_items.'

    def text(self, match, context, next_state):
        """Definition lists."""
        return (
         [
          match.string], 'Definition', [])


class EnumeratedList(SpecializedBody):
    __doc__ = 'Second and subsequent enumerated_list list_items.'

    def enumerator--- This code section failed: ---

 L.2527         0  LOAD_FAST                'self'
                2  LOAD_METHOD              parse_enumerator

 L.2528         4  LOAD_FAST                'match'
                6  LOAD_FAST                'self'
                8  LOAD_ATTR                parent
               10  LOAD_STR                 'enumtype'
               12  BINARY_SUBSCR    
               14  CALL_METHOD_2         2  '2 positional arguments'
               16  UNPACK_SEQUENCE_4     4 
               18  STORE_FAST               'format'
               20  STORE_FAST               'sequence'
               22  STORE_FAST               'text'
               24  STORE_FAST               'ordinal'

 L.2529        26  LOAD_FAST                'format'
               28  LOAD_FAST                'self'
               30  LOAD_ATTR                format
               32  COMPARE_OP               !=
               34  POP_JUMP_IF_TRUE     92  'to 92'

 L.2530        36  LOAD_FAST                'sequence'
               38  LOAD_STR                 '#'
               40  COMPARE_OP               !=
               42  POP_JUMP_IF_FALSE    78  'to 78'
               44  LOAD_FAST                'sequence'
               46  LOAD_FAST                'self'
               48  LOAD_ATTR                parent
               50  LOAD_STR                 'enumtype'
               52  BINARY_SUBSCR    
               54  COMPARE_OP               !=
               56  POP_JUMP_IF_TRUE     92  'to 92'

 L.2531        58  LOAD_FAST                'self'
               60  LOAD_ATTR                auto
               62  POP_JUMP_IF_TRUE     92  'to 92'

 L.2532        64  LOAD_FAST                'ordinal'
               66  LOAD_FAST                'self'
               68  LOAD_ATTR                lastordinal
               70  LOAD_CONST               1
               72  BINARY_ADD       
               74  COMPARE_OP               !=
               76  POP_JUMP_IF_TRUE     92  'to 92'
             78_0  COME_FROM            42  '42'

 L.2533        78  LOAD_FAST                'self'
               80  LOAD_METHOD              is_enumerated_list_item
               82  LOAD_FAST                'ordinal'
             ...
             92_2  COME_FROM            56  '56'
             92_3  COME_FROM            34  '34'

 L.2535        92  LOAD_FAST                'self'
               94  LOAD_METHOD              invalid_input
               96  CALL_METHOD_0         0  '0 positional arguments'
               98  POP_TOP          
            100_0  COME_FROM            90  '90'

 L.2536       100  LOAD_FAST                'sequence'
              102  LOAD_STR                 '#'
              104  COMPARE_OP               ==
              106  POP_JUMP_IF_FALSE   114  'to 114'

 L.2537       108  LOAD_CONST               1
              110  LOAD_FAST                'self'
              112  STORE_ATTR               auto
            114_0  COME_FROM           106  '106'

 L.2538       114  LOAD_FAST                'self'
              116  LOAD_METHOD              list_item
              118  LOAD_FAST                'match'
              120  LOAD_METHOD              end
              122  CALL_METHOD_0         0  '0 positional arguments'
              124  CALL_METHOD_1         1  '1 positional argument'
              126  UNPACK_SEQUENCE_2     2 
              128  STORE_FAST               'listitem'
              130  STORE_FAST               'blank_finish'

 L.2539       132  LOAD_FAST                'self'
              134  DUP_TOP          
              136  LOAD_ATTR                parent
              138  LOAD_FAST                'listitem'
              140  INPLACE_ADD      
              142  ROT_TWO          
              144  STORE_ATTR               parent

 L.2540       146  LOAD_FAST                'blank_finish'
              148  LOAD_FAST                'self'
              150  STORE_ATTR               blank_finish

 L.2541       152  LOAD_FAST                'ordinal'
              154  LOAD_FAST                'self'
              156  STORE_ATTR               lastordinal

 L.2542       158  BUILD_LIST_0          0 
              160  LOAD_FAST                'next_state'
              162  BUILD_LIST_0          0 
              164  BUILD_TUPLE_3         3 
              166  RETURN_VALUE     
               -1  RETURN_LAST      
...
class FieldList(SpecializedBody):
    __doc__ = 'Second and subsequent field_list fields.'
...
class OptionList(SpecializedBody):
    __doc__ = 'Second and subsequent option_list option_list_items.'
...
class RFC2822List(SpecializedBody, RFC2822Body):
    __doc__ = 'Second and subsequent RFC2822-style field_list fields.'
...
class LineBlock(SpecializedBody):
    __doc__ = 'Second and subsequent lines of a line_block.'
    blank = SpecializedBody.invalid_input
...
class Explicit(SpecializedBody):
    __doc__ = 'Second and subsequent explicit markup construct.'
...
class SubstitutionDef(Body):
    ...
    patterns = {'embedded_directive':re.compile('(%s)::( +|$)' % Inliner.simplename, re.UNICODE), 
     'text':''}
...
class Text(RSTState):
    ...
    def underline(self, match, context, next_state):
        ...
        if column_width(title) > len(underline):
            if len(underline) < 4:
                ...
                blocktext = context[0] + '\n' + self.state_machine.line
                msg = self.reporter.warning('Title underline too short.', (nodes.literal_block(blocktext, blocktext)),
                  line=lineno)
                messages.append(msg)
        if not self.state_machine.match_titles:
            blocktext = context[0] + '\n' + self.state_machine.line
            src, srcline = self.state_machine.get_source_and_line()
            msg = self.reporter.severe('Unexpected section title.', (nodes.literal_block(blocktext, blocktext)),
              source=src,
              line=srcline)
            self.parent += messages
            self.parent += msg
    ...
    def text(self, match, context, next_state):
        ...
        try:
            ...
            try:
                block, src, srcline = err.args
                msg = self.reporter.error('Unexpected indentation.', source=src,
                  line=srcline)
            finally:
                err = None
                del err
    ...
    def quoted_literal_block(self):
        ...
        new_abs_offset = self.nested_parse((self.state_machine.input_lines[offset:]),
          input_offset=abs_line_offset,
          node=parent_node,
          match_titles=False,
          state_machine_kwargs={'state_classes':(
          QuotedLiteralBlock,), 
    ...
    def definition_list_item(self, termline):
        ...
        itemnode += definition
        if termline[0][-2:] == '::':
            definition += self.reporter.info('Blank line missing before literal block (after the "::")? Interpreted as a definition list item.',
              line=(lineno + 1))
    ...
    def term(self, lines, lineno):
        ...
        term_node.source, term_node.line = self.state_machine.get_source_and_line(lineno)
...
class Definition(SpecializedText):
    __doc__ = 'Second line of potential definition_list_item.'
...
class Line(SpecializedText):
    ...
    def text(self, match, context, next_state):
        ...
        try:
            ...
            if len(overline.rstrip()) < 4:
                ...
                msg = self.reporter.severe('Incomplete section title.',
                  (nodes.literal_block(blocktext, blocktext)),
                  line=lineno)
                self.parent += msg
                return ([], 'Body', [])
        ...
        if not self.transitions['underline'][0].match(underline):
            ...
            if len(overline.rstrip()) < 4:
                ...
                msg = self.reporter.severe('Missing matching underline for section title overline.',
            ...
            if overline != underline:
                ...
                if len(overline.rstrip()) < 4:
                    ...
                    msg = self.reporter.severe('Title overline & underline mismatch.',
                      (nodes.literal_block(source, source)),
                      line=lineno)
                    self.parent += msg
                    return ([], 'Body', [])
                ...
                if column_width(title) > len(overline):
                    ...
                    if len(overline.rstrip()) < 4:
                        ...
                        msg = self.reporter.warning('Title overline too short.',
                          (nodes.literal_block(source, source)),
                          line=lineno)
                        messages.append(msg)
            style = (
             overline[0], underline[0])
            self.eofcheck = 0
    ...
    def underline(self, match, context, next_state):
        ...
        msg = self.reporter.error('Invalid section title or transition marker.',
          (nodes.literal_block(blocktext, blocktext)),
          line=lineno)
        self.parent += msg
        return ([], 'Body', [])
...
class QuotedLiteralBlock(RSTState):
    ...
    patterns = {'initial_quoted':'(%(nonalphanum7bit)s)' % Body.pats, 
     'text':''}
    ...
    def eof(self, context):
        if context:
            ...
            self.parent += self.reporter.warning('Literal block expected; none found.',
              line=(self.state_machine.abs_line_number()))
            self.state_machine.previous_line()
...
state_classes = (
 Body, BulletList, DefinitionList, EnumeratedList, FieldList,
