...
# [GCC 8.4.0]
# Embedded file name: /Users/tkomiya/work/sphinx/.tox/py37/lib/python3.7/site-packages/docutils/utils/math/math2html.py
# Compiled at: 2018-11-25 06:19:18
# Size of source mod 2**32: 178955 bytes
import sys

class Trace(object):
    __doc__ = 'A tracing class'
    debugmode = False
    quietmode = False
    showlinesmode = False
    prefix = None
...
class BibStylesConfig(object):
    __doc__ = 'Configuration class from elyxer.config file'
    abbrvnat = {'@article':'$authors. $title. <i>$journal</i>,{ {$volume:}$pages,} $month $year.{ doi: $doi.}{ URL <a href="$url">$url</a>.}{ $note.}', 
     'cite':'$surname($year)', 
     'default':'$authors. <i>$title</i>. $publisher, $year.{ URL <a href="$url">$url</a>.}{ $note.}'}
    alpha = {'@article':'$authors. $title.{ <i>$journal</i>{, {$volume}{($number)}}{: $pages}{, $year}.}{ <a href="$url">$url</a>.}{ <a href="$filename">$filename</a>.}{ $note.}', 
     'cite':'$Sur$YY', 
     'default':'$authors. $title.{ <i>$journal</i>,} $year.{ <a href="$url">$url</a>.}{ <a href="$filename">$filename</a>.}{ $note.}'}
    authordate2 = {'@article':'$authors. $year. $title. <i>$journal</i>, <b>$volume</b>($number), $pages.{ URL <a href="$url">$url</a>.}{ $note.}', 
    ...
    default = {'@article':'$authors: “$title”, <i>$journal</i>,{ pp. $pages,} $year.{ URL <a href="$url">$url</a>.}{ $note.}', 
    ...
    defaulttags = {'YY':'??', 
     'authors':'',  'surname':''}
    ieeetr = {'@article':'$authors, “$title”, <i>$journal</i>, vol. $volume, no. $number, pp. $pages, $year.{ URL <a href="$url">$url</a>.}{ $note.}', 
     '@book':'$authors, <i>$title</i>. $publisher, $year.{ URL <a href="$url">$url</a>.}{ $note.}', 
     'cite':'$index', 
     'default':'$authors, “$title”. $year.{ URL <a href="$url">$url</a>.}{ $note.}'}
    plain = {'@article':'$authors. $title.{ <i>$journal</i>{, {$volume}{($number)}}{:$pages}{, $year}.}{ URL <a href="$url">$url</a>.}{ $note.}', 
    ...
    vancouver = {'@article':'$authors. $title. <i>$journal</i>, $year{;{<b>$volume</b>}{($number)}{:$pages}}.{ URL: <a href="$url">$url</a>.}{ $note.}', 
...
class ContainerConfig(object):
    __doc__ = 'Configuration class from elyxer.config file'
    endings = {'Align':'\\end_layout', 
     'BarredText':'\\bar',  'BoldText':'\\series', 
     'Cell':'</cell',  'ChangeDeleted':'\\change_unchanged', 
     'ChangeInserted':'\\change_unchanged', 
     'ColorText':'\\color',  'EmphaticText':'\\emph', 
     'Hfill':'\\hfill',  'Inset':'\\end_inset',  'Layout':'\\end_layout', 
     'LyXFooter':'\\end_document',  'LyXHeader':'\\end_header', 
     'Row':'</row',  'ShapedText':'\\shape',  'SizeText':'\\size', 
     'StrikeOut':'\\strikeout',  'TextFamily':'\\family', 
     'VersalitasText':'\\noun'}
    extracttext = {'allowed':[
      'StringContainer', 'Constant', 'FormulaConstant'], 
     'cloned':[
      ''], 
     'extracted':[
    ...
    startendings = {'\\begin_deeper':'\\end_deeper', 
     '\\begin_inset':'\\end_inset',  '\\begin_layout':'\\end_layout'}
    starts = {'':'StringContainer', 
     '#LyX':'BlackBox',  '</lyxtabular':'BlackBox',  '<cell':'Cell', 
     '<column':'Column',  '<row':'Row',  '\\align':'Align', 
     '\\bar':'BarredText',  '\\bar default':'BlackBox', 
     '\\bar no':'BlackBox',  '\\begin_body':'BlackBox', 
     '\\begin_deeper':'DeeperList',  '\\begin_document':'BlackBox', 
     '\\begin_header':'LyXHeader',  '\\begin_inset Argument':'ShortTitle', 
     '\\begin_inset Box':'BoxInset', 
     '\\begin_inset Branch':'Branch',  '\\begin_inset Caption':'Caption', 
     ...
     '\\begin_inset ERT':'ERT', 
     '\\begin_inset Flex':'FlexInset',  '\\begin_inset Flex Chunkref':'NewfangledChunkRef', 
     '\\begin_inset Flex Marginnote':'SideNote', 
     '\\begin_inset Flex Sidenote':'SideNote', 
     '\\begin_inset Flex URL':'FlexURL', 
     '\\begin_inset Float':'Float',  '\\begin_inset FloatList':'ListOf', 
     '\\begin_inset Foot':'Footnote',  '\\begin_inset Formula':'Formula', 
     ...
     '\\begin_inset Newpage':'NewPageInset', 
     '\\begin_inset Note':'Note',  '\\begin_inset OptArg':'ShortTitle', 
     '\\begin_inset Phantom':'PhantomText', 
     '\\begin_inset Quotes':'QuoteContainer', 
     '\\begin_inset Tabular':'Table', 
     '\\begin_inset Text':'InsetText',  '\\begin_inset VSpace':'VerticalSpace', 
     '\\begin_inset Wrap':'Wrap',  '\\begin_inset listings':'Listing', 
     '\\begin_inset script':'ScriptInset', 
     '\\begin_inset space':'Space',  '\\begin_layout':'Layout', 
     '\\begin_layout Abstract':'Abstract',  '\\begin_layout Author':'Author', 
     ...
     '\\begin_layout Itemize':'ListItem', 
     '\\begin_layout List':'List',  '\\begin_layout LyX-Code':'LyXCode', 
     '\\begin_layout Plain':'PlainLayout', 
     '\\begin_layout Standard':'StandardLayout', 
     '\\begin_layout Title':'Title', 
     '\\begin_preamble':'LyXPreamble',  '\\change_deleted':'ChangeDeleted', 
     '\\change_inserted':'ChangeInserted', 
     '\\change_unchanged':'BlackBox', 
     '\\color':'ColorText',  '\\color inherit':'BlackBox', 
     '\\color none':'BlackBox',  '\\emph default':'BlackBox', 
     '\\emph off':'BlackBox',  '\\emph on':'EmphaticText', 
     '\\emph toggle':'EmphaticText',  '\\end_body':'LyXFooter', 
     '\\family':'TextFamily',  '\\family default':'BlackBox', 
     '\\family roman':'BlackBox',  '\\hfill':'Hfill', 
     '\\labelwidthstring':'BlackBox',  '\\lang':'LangLine', 
     '\\length':'InsetLength',  '\\lyxformat':'LyXFormat', 
     '\\lyxline':'LyXLine',  '\\newline':'Newline', 
     '\\newpage':'NewPage',  '\\noindent':'BlackBox', 
     '\\noun default':'BlackBox',  '\\noun off':'BlackBox', 
     '\\noun on':'VersalitasText',  '\\paragraph_spacing':'BlackBox', 
     '\\series bold':'BoldText',  '\\series default':'BlackBox', 
     '\\series medium':'BlackBox',  '\\shape':'ShapedText', 
     '\\shape default':'BlackBox',  '\\shape up':'BlackBox', 
     '\\size':'SizeText',  '\\size normal':'BlackBox', 
     '\\start_of_appendix':'StartAppendix',  '\\strikeout default':'BlackBox', 
     '\\strikeout on':'StrikeOut'}
    string = {'startcommand': '\\'}
    table = {'headers': ['<lyxtabular', '<features']}
...
class EscapeConfig(object):
    ...
    chars = {'\n':'', 
     ' -- ':' — ',  ' --- ':' — ',  "'":'’',  '`':'‘'}
    commands = {'\\InsetSpace \\space{}':'\xa0', 
     '\\InsetSpace \\thinspace{}':'\u2009',  '\\InsetSpace ~':'\xa0', 
     '\\SpecialChar \\-':'',  '\\SpecialChar \\@.':'.', 
     '\\SpecialChar \\ldots{}':'…',  '\\SpecialChar \\menuseparator':'\xa0▷\xa0', 
     '\\SpecialChar \\nobreakdash-':'-', 
     '\\SpecialChar \\slash{}':'/',  '\\SpecialChar \\textcompwordmark{}':'', 
     '\\backslash':'\\'}
    entities = {'&':'&amp;', 
     '<':'&lt;',  '>':'&gt;'}
    html = {'/>': '>'}
...
class FormulaConfig(object):
    __doc__ = 'Configuration class from elyxer.config file'
    alphacommands = {'\\AA':'Å', 
     '\\AE':'Æ',  '\\AmS':'<span class="versalitas">AmS</span>', 
     '\\Angstroem':'Å',  '\\DH':'Ð', 
     '\\Koppa':'Ϟ',  '\\L':'Ł',  '\\Micro':'µ',  '\\O':'Ø',  '\\OE':'Œ', 
     '\\Sampi':'Ϡ',  '\\Stigma':'Ϛ',  '\\TH':'Þ',  '\\aa':'å', 
     '\\ae':'æ',  '\\alpha':'α',  '\\beta':'β',  '\\delta':'δ', 
     '\\dh':'ð',  '\\digamma':'ϝ',  '\\epsilon':'ϵ',  '\\eta':'η', 
     '\\eth':'ð',  '\\gamma':'γ',  '\\i':'ı',  '\\imath':'ı', 
     '\\iota':'ι',  '\\j':'ȷ',  '\\jmath':'ȷ',  '\\kappa':'κ', 
     '\\koppa':'ϟ',  '\\l':'ł',  '\\lambda':'λ',  '\\mu':'μ', 
     '\\nu':'ν',  '\\o':'ø',  '\\oe':'œ',  '\\omega':'ω',  '\\phi':'φ', 
     '\\pi':'π',  '\\psi':'ψ',  '\\rho':'ρ',  '\\sampi':'ϡ', 
     '\\sigma':'σ',  '\\ss':'ß',  '\\stigma':'ϛ',  '\\tau':'τ', 
     '\\tcohm':'Ω',  '\\textcrh':'ħ',  '\\th':'þ',  '\\theta':'θ', 
     '\\upsilon':'υ',  '\\varDelta':'∆',  '\\varGamma':'Γ', 
     '\\varLambda':'Λ',  '\\varOmega':'Ω',  '\\varPhi':'Φ', 
     '\\varPi':'Π',  '\\varPsi':'Ψ',  '\\varSigma':'Σ',  '\\varTheta':'Θ', 
     '\\varUpsilon':'Υ',  '\\varXi':'Ξ',  '\\varbeta':'ϐ', 
     '\\varepsilon':'ε',  '\\varkappa':'ϰ',  '\\varphi':'φ', 
     '\\varpi':'ϖ',  '\\varrho':'ϱ',  '\\varsigma':'ς',  '\\vartheta':'ϑ', 
     '\\xi':'ξ',  '\\zeta':'ζ'}
    array = {'begin':'\\begin', 
     'cellseparator':'&',  'end':'\\end',  'rowseparator':'\\\\'}
    bigbrackets = {'(':[
      '⎛', '⎜', '⎝'], 
     ')':['⎞', '⎟', '⎠'],  '[':['⎡', '⎢', '⎣'],  ']':[
      '⎤', '⎥', '⎦'], 
    ...
    bigsymbols = {'∑':[
      '⎲', '⎳'], 
     '∫':['⌠', '⌡']}
    bracketcommands = {'\\left':'span class="symbol"', 
     '\\left.':'<span class="leftdot"></span>', 
     '\\middle':'span class="symbol"', 
     '\\right':'span class="symbol"',  '\\right.':'<span class="rightdot"></span>'}
    combiningfunctions = {'\\"':'̈', 
     "\\'":'́',  '\\^':'̂',  '\\`':'̀',  '\\acute':'́',  '\\bar':'̄', 
     '\\breve':'̆',  '\\c':'̧',  '\\check':'̌',  '\\dddot':'⃛', 
     '\\ddot':'̈',  '\\dot':'̇',  '\\grave':'̀',  '\\hat':'̂', 
     '\\mathring':'̊',  '\\overleftarrow':'⃖',  '\\overrightarrow':'⃗', 
     '\\r':'̊',  '\\s':'̩',  '\\textcircled':'⃝', 
     '\\textsubring':'̥',  '\\tilde':'̃',  '\\v':'̌', 
     '\\vec':'⃗',  '\\~':'̃'}
    commands = {'\\ ':'\xa0', 
     '\\!':'',  '\\#':'#',  '\\$':'$',  '\\%':'%',  '\\&':'&', 
     '\\,':' ',  '\\:':'\u205f',  '\\;':'\u2004',  '\\AC':'∿',  '\\APLcomment':'⍝', 
     '\\APLdownarrowbox':'⍗',  '\\APLinput':'⍞',  '\\APLinv':'⌹', 
     '\\APLleftarrowbox':'⍇',  '\\APLlog':'⍟',  '\\APLrightarrowbox':'⍈', 
     '\\APLuparrowbox':'⍐',  '\\Box':'□',  '\\Bumpeq':'≎', 
     '\\CIRCLE':'●',  '\\Cap':'⋒',  '\\CapitalDifferentialD':'ⅅ', 
     '\\CheckedBox':'☑',  '\\Circle':'○',  '\\Coloneqq':'⩴', 
     '\\ComplexI':'ⅈ',  '\\ComplexJ':'ⅉ',  '\\Corresponds':'≙', 
     '\\Cup':'⋓',  '\\Delta':'Δ',  '\\Diamond':'◇',  '\\Diamondblack':'◆', 
     '\\Diamonddot':'⟐',  '\\DifferentialD':'ⅆ',  '\\Downarrow':'⇓', 
     '\\EUR':'€',  '\\Euler':'ℇ',  '\\ExponetialE':'ⅇ', 
     '\\Finv':'Ⅎ',  '\\Game':'⅁',  '\\Gamma':'Γ',  '\\Im':'ℑ', 
     '\\Join':'⨝',  '\\LEFTCIRCLE':'◖',  '\\LEFTcircle':'◐',  '\\LHD':'◀', 
     '\\Lambda':'Λ',  '\\Lbag':'⟅',  '\\Leftarrow':'⇐',  '\\Lleftarrow':'⇚', 
     '\\Longleftarrow':'⟸',  '\\Longleftrightarrow':'⟺', 
     '\\Longrightarrow':'⟹',  '\\Lparen':'⦅',  '\\Lsh':'↰', 
     '\\Mapsfrom':'⇐|',  '\\Mapsto':'|⇒',  '\\Omega':'Ω',  '\\P':'¶', 
     '\\Phi':'Φ',  '\\Pi':'Π',  '\\Pr':'Pr',  '\\Psi':'Ψ',  '\\Qoppa':'Ϙ', 
     '\\RHD':'▶',  '\\RIGHTCIRCLE':'◗',  '\\RIGHTcircle':'◑', 
     '\\Rbag':'⟆',  '\\Re':'ℜ',  '\\Rparen':'⦆',  '\\Rrightarrow':'⇛', 
     '\\Rsh':'↱',  '\\S':'§',  '\\Sigma':'Σ',  '\\Square':'☐', 
     '\\Subset':'⋐',  '\\Sun':'☉',  '\\Supset':'⋑',  '\\Theta':'Θ', 
     '\\Uparrow':'⇑',  '\\Updownarrow':'⇕',  '\\Upsilon':'Υ', 
     '\\Vdash':'⊩',  '\\Vert':'∥',  '\\Vvdash':'⊪',  '\\XBox':'☒', 
     '\\Xi':'Ξ',  '\\Yup':'⅄',  '\\\\':'<br/>',  '\\_':'_', 
     '\\aleph':'ℵ',  '\\amalg':'∐',  '\\anchor':'⚓',  '\\angle':'∠', 
     '\\aquarius':'♒',  '\\arccos':'arccos',  '\\arcsin':'arcsin', 
     '\\arctan':'arctan',  '\\arg':'arg',  '\\aries':'♈', 
     '\\arrowbullet':'➢',  '\\ast':'∗',  '\\asymp':'≍',  '\\backepsilon':'∍', 
     '\\backprime':'‵',  '\\backsimeq':'⋍',  '\\backslash':'\\', 
     '\\ballotx':'✗',  '\\barwedge':'⊼',  '\\because':'∵', 
     '\\beth':'ℶ',  '\\between':'≬',  '\\bigcap':'∩',  '\\bigcirc':'○', 
     '\\bigcup':'∪',  '\\bigodot':'⊙',  '\\bigoplus':'⊕', 
     '\\bigotimes':'⊗',  '\\bigsqcup':'⊔',  '\\bigstar':'★', 
     '\\bigtriangledown':'▽',  '\\bigtriangleup':'△',  '\\biguplus':'⊎', 
     '\\bigvee':'∨',  '\\bigwedge':'∧',  '\\biohazard':'☣', 
     '\\blacklozenge':'⧫',  '\\blacksmiley':'☻',  '\\blacksquare':'■', 
     '\\blacktriangle':'▲',  '\\blacktriangledown':'▼', 
     '\\blacktriangleleft':'◂',  '\\blacktriangleright':'▶', 
     '\\blacktriangleup':'▴',  '\\bot':'⊥',  '\\bowtie':'⋈', 
     '\\box':'▫',  '\\boxast':'⧆',  '\\boxbar':'◫',  '\\boxbox':'⧈', 
     '\\boxbslash':'⧅',  '\\boxcircle':'⧇',  '\\boxdot':'⊡', 
     '\\boxminus':'⊟',  '\\boxplus':'⊞',  '\\boxslash':'⧄', 
     '\\boxtimes':'⊠',  '\\bullet':'•',  '\\bumpeq':'≏', 
     '\\cancer':'♋',  '\\cap':'∩',  '\\capricornus':'♑',  '\\cat':'⁀', 
     '\\cdot':'⋅',  '\\cdots':'⋯',  '\\cent':'¢',  '\\centerdot':'∙', 
     '\\checkmark':'✓',  '\\chi':'χ',  '\\circ':'∘',  '\\circeq':'≗', 
     '\\circlearrowleft':'↺',  '\\circlearrowright':'↻',  '\\circledR':'®', 
     '\\circledast':'⊛',  '\\circledbslash':'⦸',  '\\circledcirc':'⊚', 
     '\\circleddash':'⊝',  '\\circledgtr':'⧁',  '\\circledless':'⧀', 
     '\\clubsuit':'♣',  '\\colon':': ',  '\\coloneqq':'≔',  '\\complement':'∁', 
     '\\cong':'≅',  '\\coprod':'∐',  '\\copyright':'©', 
     '\\cos':'cos',  '\\cosh':'cosh',  '\\cot':'cot',  '\\coth':'coth', 
     '\\csc':'csc',  '\\cup':'∪',  '\\curlyvee':'⋎',  '\\curlywedge':'⋏', 
     '\\curvearrowleft':'↶',  '\\curvearrowright':'↷', 
     '\\dag':'†',  '\\dagger':'†',  '\\daleth':'ℸ', 
     '\\dashleftarrow':'⇠',  '\\dashv':'⊣',  '\\ddag':'‡', 
     '\\ddagger':'‡',  '\\ddots':'⋱',  '\\deg':'deg',  '\\det':'det', 
     '\\diagdown':'╲',  '\\diagup':'╱',  '\\diameter':'⌀', 
     '\\diamond':'◇',  '\\diamondsuit':'♦',  '\\dim':'dim', 
     '\\div':'÷',  '\\divideontimes':'⋇',  '\\dotdiv':'∸', 
     '\\doteq':'≐',  '\\doteqdot':'≑',  '\\dotplus':'∔',  '\\dots':'…', 
     '\\doublebarwedge':'⌆',  '\\downarrow':'↓',  '\\downdownarrows':'⇊', 
     '\\downharpoonleft':'⇃',  '\\downharpoonright':'⇂', 
     '\\dsub':'⩤',  '\\earth':'♁',  '\\eighthnote':'♪', 
     '\\ell':'ℓ',  '\\emptyset':'∅',  '\\eqcirc':'≖', 
     '\\eqcolon':'≕',  '\\eqsim':'≂',  '\\euro':'€',  '\\exists':'∃', 
     '\\exp':'exp',  '\\fallingdotseq':'≒',  '\\fcmp':'⨾', 
     '\\female':'♀',  '\\flat':'♭',  '\\forall':'∀',  '\\fourth':'⁗', 
     '\\frown':'⌢',  '\\frownie':'☹',  '\\gcd':'gcd',  '\\gemini':'♊', 
     '\\geq)':'≥',  '\\geqq':'≧',  '\\geqslant':'≥',  '\\gets':'←', 
     '\\gg':'≫',  '\\ggg':'⋙',  '\\gimel':'ℷ',  '\\gneqq':'≩', 
     '\\gnsim':'⋧',  '\\gtrdot':'⋗',  '\\gtreqless':'⋚',  '\\gtreqqless':'⪌', 
     '\\gtrless':'≷',  '\\gtrsim':'≳',  '\\guillemotleft':'«', 
     '\\guillemotright':'»',  '\\hbar':'ℏ',  '\\heartsuit':'♥', 
     '\\hfill':'<span class="hfill"> </span>',  '\\hom':'hom', 
     '\\hookleftarrow':'↩',  '\\hookrightarrow':'↪',  '\\hslash':'ℏ', 
     '\\idotsint':'<span class="bigsymbol">∫⋯∫</span>',  '\\iiint':'<span class="bigsymbol">∭</span>', 
     '\\iint':'<span class="bigsymbol">∬</span>', 
     '\\imath':'ı',  '\\inf':'inf', 
     '\\infty':'∞',  '\\intercal':'⊺',  '\\interleave':'⫴', 
     '\\invamp':'⅋',  '\\invneg':'⌐',  '\\jmath':'ȷ', 
     '\\jupiter':'♃',  '\\ker':'ker',  '\\land':'∧',  '\\landupint':'<span class="bigsymbol">∱</span>', 
     '\\lang':'⟪',  '\\langle':'⟨', 
     '\\lblot':'⦉',  '\\lbrace':'{',  '\\lbrace)':'{',  '\\lbrack':'[', 
     '\\lceil':'⌈',  '\\ldots':'…',  '\\leadsto':'⇝',  '\\leftarrow)':'←', 
     '\\leftarrowtail':'↢',  '\\leftarrowtobar':'⇤',  '\\leftharpoondown':'↽', 
     '\\leftharpoonup':'↼',  '\\leftleftarrows':'⇇', 
     '\\leftleftharpoons':'⥢',  '\\leftmoon':'☾',  '\\leftrightarrow':'↔', 
     '\\leftrightarrows':'⇆',  '\\leftrightharpoons':'⇋', 
     '\\leftthreetimes':'⋋',  '\\leo':'♌',  '\\leq)':'≤', 
     '\\leqq':'≦',  '\\leqslant':'≤',  '\\lessdot':'⋖',  '\\lesseqgtr':'⋛', 
     '\\lesseqqgtr':'⪋',  '\\lessgtr':'≶',  '\\lesssim':'≲', 
     '\\lfloor':'⌊',  '\\lg':'lg',  '\\lgroup':'⟮',  '\\lhd':'⊲', 
     '\\libra':'♎',  '\\lightning':'↯',  '\\limg':'⦇',  '\\liminf':'liminf', 
     '\\limsup':'limsup',  '\\ll':'≪',  '\\llbracket':'⟦', 
     '\\llcorner':'⌞',  '\\lll':'⋘',  '\\ln':'ln',  '\\lneqq':'≨', 
     '\\lnot':'¬',  '\\lnsim':'⋦',  '\\log':'log',  '\\longleftarrow':'⟵', 
     '\\longleftrightarrow':'⟷',  '\\longmapsto':'⟼', 
     '\\longrightarrow':'⟶',  '\\looparrowleft':'↫',  '\\looparrowright':'↬', 
     '\\lor':'∨',  '\\lozenge':'◊',  '\\lrcorner':'⌟', 
     '\\ltimes':'⋉',  '\\lyxlock':'',  '\\male':'♂',  '\\maltese':'✠', 
     '\\mapsfrom':'↤',  '\\mapsto':'↦',  '\\mathcircumflex':'^', 
     '\\max':'max',  '\\measuredangle':'∡',  '\\medbullet':'⚫', 
     '\\medcirc':'⚪',  '\\mercury':'☿',  '\\mho':'℧',  '\\mid':'∣', 
     '\\min':'min',  '\\models':'⊨',  '\\mp':'∓',  '\\multimap':'⊸', 
     '\\nLeftarrow':'⇍',  '\\nLeftrightarrow':'⇎',  '\\nRightarrow':'⇏', 
     '\\nVDash':'⊯',  '\\nabla':'∇',  '\\napprox':'≉', 
     '\\natural':'♮',  '\\ncong':'≇',  '\\nearrow':'↗',  '\\neg':'¬', 
     '\\neg)':'¬',  '\\neptune':'♆',  '\\nequiv':'≢',  '\\newline':'<br/>', 
     ...
     '\\ntrianglelefteq':'⋬',  '\\ntriangleright':'⋫', 
     '\\ntrianglerighteq':'⋭',  '\\nvDash':'⊭',  '\\nvdash':'⊬', 
     '\\nwarrow':'↖',  '\\odot':'⊙',  '\\officialeuro':'€', 
     '\\oiiint':'<span class="bigsymbol">∰</span>',  '\\oiint':'<span class="bigsymbol">∯</span>', 
     ...
     '\\parallel':'∥',  '\\partial':'∂',  '\\pencil':'✎',  '\\perp':'⊥', 
     '\\pisces':'♓',  '\\pitchfork':'⋔',  '\\pluto':'♇',  '\\pm':'±', 
     '\\pointer':'➪',  '\\pointright':'☞',  '\\pounds':'£',  '\\prec':'≺', 
     '\\preccurlyeq':'≼',  '\\preceq':'≼',  '\\precsim':'≾', 
     '\\prime':'′',  '\\prompto':'∝',  '\\qoppa':'ϙ',  '\\qquad':'  ', 
     '\\quad':' ',  '\\quarternote':'♩',  '\\radiation':'☢', 
     '\\rang':'⟫',  '\\rangle':'⟩',  '\\rblot':'⦊',  '\\rbrace':'}', 
     '\\rbrace)':'}',  '\\rbrack':']',  '\\rceil':'⌉',  '\\recycle':'♻', 
     '\\rfloor':'⌋',  '\\rgroup':'⟯',  '\\rhd':'⊳',  '\\rightangle':'∟', 
     '\\rightarrow)':'→',  '\\rightarrowtail':'↣',  '\\rightarrowtobar':'⇥', 
     '\\rightharpoondown':'⇁',  '\\rightharpoonup':'⇀', 
     '\\rightharpooondown':'⇁',  '\\rightharpooonup':'⇀', 
     '\\rightleftarrows':'⇄',  '\\rightleftharpoons':'⇌', 
     '\\rightmoon':'☽',  '\\rightrightarrows':'⇉', 
     '\\rightrightharpoons':'⥤',  '\\rightthreetimes':'⋌', 
     '\\rimg':'⦈',  '\\risingdotseq':'≓',  '\\rrbracket':'⟧', 
     '\\rsub':'⩥',  '\\rtimes':'⋊',  '\\sagittarius':'♐', 
     '\\saturn':'♄',  '\\scorpio':'♏',  '\\searrow':'↘', 
     '\\sec':'sec',  '\\second':'″',  '\\setminus':'∖',  '\\sharp':'♯', 
     '\\simeq':'≃',  '\\sin':'sin',  '\\sinh':'sinh',  '\\sixteenthnote':'♬', 
     '\\skull':'☠',  '\\slash':'∕',  '\\smallsetminus':'∖', 
     '\\smalltriangledown':'▿',  '\\smalltriangleleft':'◃', 
     '\\smalltriangleright':'▹',  '\\smalltriangleup':'▵', 
     '\\smile':'⌣',  '\\smiley':'☺',  '\\spadesuit':'♠', 
     '\\spddot':'¨',  '\\sphat':'',  '\\sphericalangle':'∢', 
     '\\spot':'⦁',  '\\sptilde':'~',  '\\sqcap':'⊓', 
     '\\sqcup':'⊔',  '\\sqsubset':'⊏',  '\\sqsubseteq':'⊑', 
     '\\sqsupset':'⊐',  '\\sqsupseteq':'⊒',  '\\square':'□', 
     '\\sslash':'⫽',  '\\star':'⋆',  '\\steaming':'☕',  '\\subseteqq':'⫅', 
     '\\subsetneqq':'⫋',  '\\succ':'≻',  '\\succcurlyeq':'≽', 
     '\\succeq':'≽',  '\\succnsim':'⋩',  '\\succsim':'≿', 
     '\\sun':'☼',  '\\sup':'sup',  '\\supseteqq':'⫆',  '\\supsetneqq':'⫌', 
     '\\surd':'√',  '\\swarrow':'↙',  '\\swords':'⚔', 
     '\\talloblong':'⫾',  '\\tan':'tan',  '\\tanh':'tanh', 
     '\\taurus':'♉',  '\\textasciicircum':'^',  '\\textasciitilde':'~', 
     '\\textbackslash':'\\',  '\\textcopyright':"©'", 
     '\\textdegree':'°',  '\\textellipsis':'…',  '\\textemdash':'—', 
     '\\textendash':'—',  '\\texteuro':'€',  '\\textgreater':'>', 
     '\\textless':'<',  '\\textordfeminine':'ª',  '\\textordmasculine':'º', 
     '\\textquotedblleft':'“',  '\\textquotedblright':'”', 
     '\\textquoteright':'’',  '\\textregistered':'®', 
     '\\textrightarrow':'→',  '\\textsection':'§', 
     '\\texttrademark':'™',  '\\texttwosuperior':'²', 
     '\\textvisiblespace':' ',  '\\therefore':'∴', 
     '\\third':'‴',  '\\top':'⊤',  '\\triangle':'△',  '\\triangleleft':'⊲', 
     '\\trianglelefteq':'⊴',  '\\triangleq':'≜',  '\\triangleright':'▷', 
     '\\trianglerighteq':'⊵',  '\\twoheadleftarrow':'↞', 
     '\\twoheadrightarrow':'↠',  '\\twonotes':'♫', 
     '\\udot':'⊍',  '\\ulcorner':'⌜',  '\\unlhd':'⊴',  '\\unrhd':'⊵', 
     '\\unrhl':'⊵',  '\\uparrow':'↑',  '\\updownarrow':'↕', 
     '\\upharpoonleft':'↿',  '\\upharpoonright':'↾',  '\\uplus':'⊎', 
     '\\upuparrows':'⇈',  '\\uranus':'♅',  '\\urcorner':'⌝', 
     '\\vDash':'⊨',  '\\varclubsuit':'♧',  '\\vardiamondsuit':'♦', 
     '\\varheartsuit':'♥',  '\\varnothing':'∅',  '\\varspadesuit':'♤', 
     '\\vdash':'⊢',  '\\vdots':'⋮',  '\\vee':'∨',  '\\vee)':'∨', 
     '\\veebar':'⊻',  '\\vert':'∣',  '\\virgo':'♍',  '\\warning':'⚠', 
     '\\wasylozenge':'⌑',  '\\wedge':'∧',  '\\wedge)':'∧', 
     '\\wp':'℘',  '\\wr':'≀',  '\\yen':'¥',  '\\yinyang':'☯', 
     '\\{':'{',  '\\|':'∥',  '\\}':'}'}
    decoratedcommand = {}
    decoratingfunctions = {'\\overleftarrow':'⟵', 
     '\\overrightarrow':'⟶',  '\\widehat':'^'}
    endings = {'bracket':'}', 
     'complex':'\\]',  'endafter':'}',  'endbefore':'\\end{', 
     'squarebracket':']'}
    environments = {'align':[
      'r', 'l'], 
     'eqnarray':['r', 'c', 'l'],  'gathered':[
      'l', 'l']}
    fontfunctions = {'\\boldsymbol':'b', 
     '\\mathbb':'span class="blackboard"',  '\\mathbb{A}':'𝔸', 
     '\\mathbb{B}':'𝔹',  '\\mathbb{C}':'ℂ',  '\\mathbb{D}':'𝔻', 
     '\\mathbb{E}':'𝔼',  '\\mathbb{F}':'𝔽',  '\\mathbb{G}':'𝔾', 
     '\\mathbb{H}':'ℍ',  '\\mathbb{J}':'𝕁',  '\\mathbb{K}':'𝕂', 
     '\\mathbb{L}':'𝕃',  '\\mathbb{N}':'ℕ',  '\\mathbb{O}':'𝕆', 
     '\\mathbb{P}':'ℙ',  '\\mathbb{Q}':'ℚ',  '\\mathbb{R}':'ℝ', 
     '\\mathbb{S}':'𝕊',  '\\mathbb{T}':'𝕋',  '\\mathbb{W}':'𝕎', 
     '\\mathbb{Z}':'ℤ',  '\\mathbf':'b',  '\\mathcal':'span class="scriptfont"', 
     '\\mathcal{B}':'ℬ',  '\\mathcal{E}':'ℰ', 
     '\\mathcal{F}':'ℱ',  '\\mathcal{H}':'ℋ',  '\\mathcal{I}':'ℐ', 
     '\\mathcal{L}':'ℒ',  '\\mathcal{M}':'ℳ',  '\\mathcal{R}':'ℛ', 
     '\\mathfrak':'span class="fraktur"',  '\\mathfrak{C}':'ℭ', 
     '\\mathfrak{F}':'𝔉',  '\\mathfrak{H}':'ℌ',  '\\mathfrak{I}':'ℑ', 
     '\\mathfrak{R}':'ℜ',  '\\mathfrak{Z}':'ℨ',  '\\mathit':'i', 
     '\\mathring{A}':'Å',  '\\mathring{U}':'Ů',  '\\mathring{a}':'å', 
     '\\mathring{u}':'ů',  '\\mathring{w}':'ẘ',  '\\mathring{y}':'ẙ', 
     '\\mathrm':'span class="mathrm"',  '\\mathscr':'span class="scriptfont"', 
     '\\mathscr{B}':'ℬ',  '\\mathscr{E}':'ℰ', 
     '\\mathscr{F}':'ℱ',  '\\mathscr{H}':'ℋ',  '\\mathscr{I}':'ℐ', 
     '\\mathscr{L}':'ℒ',  '\\mathscr{M}':'ℳ',  '\\mathscr{R}':'ℛ', 
     '\\mathsf':'span class="mathsf"',  '\\mathtt':'tt'}
    hybridfunctions = {'\\addcontentsline':[
      '{$p!}{$q!}{$r!}', 'f0{}', 'ignored'], 
     '\\addtocontents':[
      '{$p!}{$q!}', 'f0{}', 'ignored'], 
     '\\backmatter':[
      '', 'f0{}', 'ignored'], 
     '\\binom':[
      '{$1}{$2}', 'f2{(}f0{f1{$1}f1{$2}}f2{)}', 'span class="binom"', 'span class="binomstack"', 'span class="bigsymbol"'], 
     '\\boxed':[
      '{$1}', 'f0{$1}', 'span class="boxed"'], 
     '\\cfrac':[
      '[$p!]{$1}{$2}', 'f0{f3{(}f1{$1}f3{)/(}f2{$2}f3{)}}', 'span class="fullfraction"', 'span class="numerator align-$p"', 'span class="denominator"', 'span class="ignored"'], 
     '\\color':[
      '{$p!}{$1}', 'f0{$1}', 'span style="color: $p;"'], 
     '\\colorbox':[
      '{$p!}{$1}', 'f0{$1}', 'span class="colorbox" style="background: $p;"'], 
     '\\dbinom':[
      '{$1}{$2}', '(f0{f1{f2{$1}}f1{f2{\u2005}}f1{f2{$2}}})', 'span class="binomial"', 'span class="binomrow"', 'span class="binomcell"'], 
     '\\dfrac':[
      '{$1}{$2}', 'f0{f3{(}f1{$1}f3{)/(}f2{$2}f3{)}}', 'span class="fullfraction"', 'span class="numerator"', 'span class="denominator"', 'span class="ignored"'], 
     '\\displaystyle':[
      '{$1}', 'f0{$1}', 'span class="displaystyle"'], 
     '\\fancyfoot':[
      '[$p!]{$q!}', 'f0{}', 'ignored'], 
     '\\fancyhead':[
      '[$p!]{$q!}', 'f0{}', 'ignored'], 
     '\\fbox':[
      '{$1}', 'f0{$1}', 'span class="fbox"'], 
     '\\fboxrule':[
      '{$p!}', 'f0{}', 'ignored'], 
     '\\fboxsep':[
      '{$p!}', 'f0{}', 'ignored'], 
     '\\fcolorbox':[
      '{$p!}{$q!}{$1}', 'f0{$1}', 'span class="boxed" style="border-color: $p; background: $q;"'], 
     '\\frac':[
      '{$1}{$2}', 'f0{f3{(}f1{$1}f3{)/(}f2{$2}f3{)}}', 'span class="fraction"', 'span class="numerator"', 'span class="denominator"', 'span class="ignored"'], 
     '\\framebox':[
      '[$p!][$q!]{$1}', 'f0{$1}', 'span class="framebox align-$q" style="width: $p;"'], 
     '\\frontmatter':[
      '', 'f0{}', 'ignored'], 
     '\\href':[
      '[$o]{$u!}{$t!}', 'f0{$t}', 'a href="$u"'], 
     '\\hspace':[
      '{$p!}', 'f0{ }', 'span class="hspace" style="width: $p;"'], 
     '\\leftroot':[
      '{$p!}', 'f0{ }', 'span class="leftroot" style="width: $p;px"'], 
     '\\mainmatter':[
      '', 'f0{}', 'ignored'], 
     '\\markboth':[
      '{$p!}{$q!}', 'f0{}', 'ignored'], 
     '\\markright':[
      '{$p!}', 'f0{}', 'ignored'], 
     '\\nicefrac':[
      '{$1}{$2}', 'f0{f1{$1}⁄f2{$2}}', 'span class="fraction"', 'sup class="numerator"', 'sub class="denominator"', 'span class="ignored"'], 
     '\\parbox':[
      '[$p!]{$w!}{$1}', 'f0{1}', 'div class="Boxed" style="width: $w;"'], 
     '\\raisebox':[
      '{$p!}{$1}', 'f0{$1.font}', 'span class="raisebox" style="vertical-align: $p;"'], 
     '\\renewenvironment':[
      '{$1!}{$2!}{$3!}', ''], 
     '\\rule':[
      '[$v!]{$w!}{$h!}', 'f0/', 'hr class="line" style="width: $w; height: $h;"'], 
     '\\scriptscriptstyle':[
      '{$1}', 'f0{$1}', 'span class="scriptscriptstyle"'], 
     '\\scriptstyle':[
      '{$1}', 'f0{$1}', 'span class="scriptstyle"'], 
     '\\sqrt':[
      '[$0]{$1}', 'f0{f1{$0}f2{√}f4{(}f3{$1}f4{)}}', 'span class="sqrt"', 'sup class="root"', 'span class="radical"', 'span class="root"', 'span class="ignored"'], 
     '\\stackrel':[
      '{$1}{$2}', 'f0{f1{$1}f2{$2}}', 'span class="stackrel"', 'span class="upstackrel"', 'span class="downstackrel"'], 
     '\\tbinom':[
      '{$1}{$2}', '(f0{f1{f2{$1}}f1{f2{\u2005}}f1{f2{$2}}})', 'span class="binomial"', 'span class="binomrow"', 'span class="binomcell"'], 
     '\\textcolor':[
      '{$p!}{$1}', 'f0{$1}', 'span style="color: $p;"'], 
     '\\textstyle':[
      '{$1}', 'f0{$1}', 'span class="textstyle"'], 
     '\\thispagestyle':[
      '{$p!}', 'f0{}', 'ignored'], 
     '\\unit':[
      '[$0]{$1}', '$0f0{$1.font}', 'span class="unit"'], 
     '\\unitfrac':[
      '[$0]{$1}{$2}', '$0f0{f1{$1.font}⁄f2{$2.font}}', 'span class="fraction"', 'sup class="unit"', 'sub class="unit"'], 
     '\\uproot':[
      '{$p!}', 'f0{ }', 'span class="uproot" style="width: $p;px"'], 
     '\\url':[
      '{$u!}', 'f0{$u}', 'a href="$u"'], 
     '\\vspace':[
      '{$p!}', 'f0{ }', 'span class="vspace" style="height: $p;"']}
    hybridsizes = {'\\binom':'$1+$2', 
     '\\cfrac':'$1+$2',  '\\dbinom':'$1+$2+1',  '\\dfrac':'$1+$2', 
     '\\frac':'$1+$2',  '\\tbinom':'$1+$2+1'}
    labelfunctions = {'\\label': 'a name="#"'}
    limitcommands = {'\\biginterleave':'⫼', 
     '\\bigsqcap':'⨅',  '\\fint':'⨏',  '\\iiiint':'⨌', 
     '\\int':'∫',  '\\intop':'∫',  '\\lim':'lim',  '\\prod':'∏', 
     '\\smallint':'∫',  '\\sqint':'⨖',  '\\sum':'∑',  '\\varointclockwise':'∲', 
     '\\varprod':'⨉',  '\\zcmp':'⨟',  '\\zhide':'⧹', 
     '\\zpipe':'⨠',  '\\zproject':'⨡'}
    misccommands = {'\\limits':'LimitPreviousCommand', 
     '\\newcommand':'MacroDefinition',  '\\renewcommand':'MacroDefinition', 
     '\\setcounter':'SetCounterFunction', 
     '\\tag':'FormulaTag',  '\\tag*':'FormulaTag', 
     '\\today':'TodayCommand'}
    modified = {'\n':'', 
     ' ':'',  '$':'',  '&':'\t',  "'":'’',  '+':'\u205f+\u205f',  ',':',\u205f', 
     '-':'\u205f−\u205f',  '/':'\u205f⁄\u205f',  ':':' : ',  '<':'\u205f&lt;\u205f',  '=':'\u205f=\u205f', 
     '>':'\u205f&gt;\u205f',  '@':'',  '~':''}
    onefunctions = {'\\Big':'span class="bigsymbol"', 
     '\\Bigg':'span class="hugesymbol"',  '\\bar':'span class="bar"', 
     '\\begin{array}':'span class="arraydef"',  '\\big':'span class="symbol"', 
     '\\bigg':'span class="largesymbol"',  '\\bigl':'span class="bigsymbol"', 
     '\\bigr':'span class="bigsymbol"',  '\\centering':'span class="align-center"', 
     ...
     '\\underline':'u',  '\\vphantom':'span class="phantom"'}
    spacedcommands = {'\\Bot':'⫫', 
     '\\Doteq':'≑',  '\\DownArrowBar':'⤓',  '\\DownLeftTeeVector':'⥞', 
     '\\DownLeftVectorBar':'⥖',  '\\DownRightTeeVector':'⥟', 
     '\\DownRightVectorBar':'⥗',  '\\Equal':'⩵', 
     '\\LeftArrowBar':'⇤',  '\\LeftDownTeeVector':'⥡',  '\\LeftDownVectorBar':'⥙', 
     '\\LeftTeeVector':'⥚',  '\\LeftTriangleBar':'⧏', 
     '\\LeftUpTeeVector':'⥠',  '\\LeftUpVectorBar':'⥘', 
     '\\LeftVectorBar':'⥒',  '\\Leftrightarrow':'⇔', 
     '\\Longmapsfrom':'⟽',  '\\Longmapsto':'⟾',  '\\MapsDown':'↧', 
     '\\MapsUp':'↥',  '\\Nearrow':'⇗',  '\\NestedGreaterGreater':'⪢', 
     '\\NestedLessLess':'⪡',  '\\NotGreaterLess':'≹', 
     '\\NotGreaterTilde':'≵',  '\\NotLessTilde':'≴', 
     '\\Nwarrow':'⇖',  '\\Proportion':'∷',  '\\RightArrowBar':'⇥', 
     '\\RightDownTeeVector':'⥝',  '\\RightDownVectorBar':'⥕', 
     '\\RightTeeVector':'⥛',  '\\RightTriangleBar':'⧐', 
     '\\RightUpTeeVector':'⥜',  '\\RightUpVectorBar':'⥔', 
     '\\RightVectorBar':'⥓',  '\\Rightarrow':'⇒', 
     '\\Same':'⩶',  '\\Searrow':'⇘',  '\\Swarrow':'⇙', 
     '\\Top':'⫪',  '\\UpArrowBar':'⤒',  '\\VDash':'⊫',  '\\approx':'≈', 
     '\\approxeq':'≊',  '\\backsim':'∽',  '\\barin':'⋶',  '\\barleftharpoon':'⥫', 
     '\\barrightharpoon':'⥭',  '\\bij':'⤖',  '\\coloneq':'≔', 
     '\\corresponds':'≙',  '\\curlyeqprec':'⋞',  '\\curlyeqsucc':'⋟', 
     '\\dashrightarrow':'⇢',  '\\dlsh':'↲',  '\\downdownharpoons':'⥥', 
     '\\downuparrows':'⇵',  '\\downupharpoons':'⥯', 
     '\\drsh':'↳',  '\\eqslantgtr':'⪖',  '\\eqslantless':'⪕', 
     '\\equiv':'≡',  '\\ffun':'⇻',  '\\finj':'⤕',  '\\ge':'≥', 
     '\\geq':'≥',  '\\ggcurly':'⪼',  '\\gnapprox':'⪊',  '\\gneq':'⪈', 
     '\\gtrapprox':'⪆',  '\\hash':'⋕',  '\\iddots':'⋰',  '\\implies':'\u205f⇒\u205f', 
     '\\in':'∈',  '\\le':'≤',  '\\leftarrow':'←',  '\\leftarrowtriangle':'⇽', 
     '\\leftbarharpoon':'⥪',  '\\leftrightarrowtriangle':'⇿', 
     '\\leftrightharpoon':'⥊',  '\\leftrightharpoondown':'⥐', 
     '\\leftrightharpoonup':'⥎',  '\\leftrightsquigarrow':'↭', 
     '\\leftslice':'⪦',  '\\leftsquigarrow':'⇜', 
     '\\leftupdownharpoon':'⥑',  '\\leq':'≤',  '\\lessapprox':'⪅', 
     '\\llcurly':'⪻',  '\\lnapprox':'⪉',  '\\lneq':'⪇', 
     '\\longmapsfrom':'⟻',  '\\multimapboth':'⧟',  '\\multimapdotbothA':'⊶', 
     '\\multimapdotbothB':'⊷',  '\\multimapinv':'⟜', 
     '\\nVdash':'⊮',  '\\ne':'≠',  '\\neq':'≠',  '\\ngeq':'≱', 
     '\\nleq':'≰',  '\\nni':'∌',  '\\not\\in':'∉',  '\\notasymp':'≭', 
     '\\npreceq':'⋠',  '\\nsqsubseteq':'⋢',  '\\nsqsupseteq':'⋣', 
     '\\nsubset':'⊄',  '\\nsucceq':'⋡',  '\\pfun':'⇸', 
     '\\pinj':'⤔',  '\\precapprox':'⪷',  '\\preceqq':'⪳',  '\\precnapprox':'⪹', 
     '\\precnsim':'⋨',  '\\propto':'∝',  '\\psur':'⤀', 
     '\\rightarrow':'→',  '\\rightarrowtriangle':'⇾',  '\\rightbarharpoon':'⥬', 
     '\\rightleftharpoon':'⥋',  '\\rightslice':'⪧', 
     '\\rightsquigarrow':'⇝',  '\\rightupdownharpoon':'⥏', 
     '\\sim':'~',  '\\strictfi':'⥼',  '\\strictif':'⥽', 
     '\\subset':'⊂',  '\\subseteq':'⊆',  '\\subsetneq':'⊊', 
     '\\succapprox':'⪸',  '\\succeqq':'⪴',  '\\succnapprox':'⪺', 
     '\\supset':'⊃',  '\\supseteq':'⊇',  '\\supsetneq':'⊋', 
     '\\times':'×',  '\\to':'→',  '\\updownarrows':'⇅', 
     '\\updownharpoons':'⥮',  '\\upupharpoons':'⥣',  '\\vartriangleleft':'⊲', 
     '\\vartriangleright':'⊳'}
    starts = {'beginafter':'}', 
     'beginbefore':'\\begin{',  'bracket':'{',  'command':'\\', 
     'comment':'%',  'complex':'\\[',  'simple':'$',  'squarebracket':'[', 
     'unnumbered':'*'}
    symbolfunctions = {'^':'sup', 
     '_':'sub'}
    textfunctions = {'\\mbox':'span class="mbox"', 
     '\\text':'span class="text"',  '\\textbf':'b', 
     '\\textipa':'span class="textipa"',  '\\textit':'i',  '\\textnormal':'span class="textnormal"', 
     '\\textrm':'span class="textrm"', 
     '\\textsc':'span class="versalitas"', 
     '\\textsf':'span class="textsf"', 
     '\\textsl':'i',  '\\texttt':'tt',  '\\textup':'span class="normal"'}
    unmodified = {'characters': ['.', '*', '€', '(', ')', '[', ']', '·', '!', ';', '|', '§', '"']}
    urls = {'googlecharts': 'http://chart.googleapis.com/chart?cht=tx&chl='}


class GeneralConfig(object):
    __doc__ = 'Configuration class from elyxer.config file'
    version = {'date':'2015-02-26', 
     'lyxformat':'413',  'number':'1.2.5'}


class HeaderConfig(object):
    __doc__ = 'Configuration class from elyxer.config file'
    parameters = {'beginpreamble':'\\begin_preamble', 
     'branch':'\\branch',  'documentclass':'\\textclass', 
     'endbranch':'\\end_branch',  'endpreamble':'\\end_preamble', 
     'language':'\\language',  'lstset':'\\lstset', 
     'outputchanges':'\\output_changes',  'paragraphseparation':'\\paragraph_separation', 
     'pdftitle':'\\pdf_title', 
     'secnumdepth':'\\secnumdepth',  'tocdepth':'\\tocdepth'}
    styles = {'article':[
      'article', 'aastex', 'aapaper', 'acmsiggraph', 'sigplanconf', 'achemso', 'amsart', 'apa', 'arab-article', 'armenian-article', 'article-beamer', 'chess', 'dtk', 'elsarticle', 'heb-article', 'IEEEtran', 'iopart', 'kluwer', 'scrarticle-beamer', 'scrartcl', 'extarticle', 'paper', 'mwart', 'revtex4', 'spie', 'svglobal3', 'ltugboat', 'agu-dtd', 'jgrga', 'agums', 'entcs', 'egs', 'ijmpc', 'ijmpd', 'singlecol-new', 'doublecol-new', 'isprs', 'tarticle', 'jsarticle', 'jarticle', 'jss', 'literate-article', 'siamltex', 'cl2emult', 'llncs', 'svglobal', 'svjog', 'svprobth'], 
     'book':[
...
class ImageConfig(object):
    __doc__ = 'Configuration class from elyxer.config file'
    converters = {'imagemagick':'convert[ -density $scale][ -define $format:use-cropbox=true] "$input" "$output"', 
     'inkscape':'inkscape "$input" --export-png="$output"', 
     'lyx':'lyx -C "$input" "$output"'}
    cropboxformats = {'.eps':'ps', 
     '.pdf':'pdf',  '.ps':'ps'}
    formats = {'default':'.png', 
     'vector':['.svg', '.eps']}
...
class NewfangleConfig(object):
    __doc__ = 'Configuration class from elyxer.config file'
    constants = {'chunkref':'chunkref{', 
     'endcommand':'}',  'endmark':'&gt;',  'startcommand':'\\', 
     'startmark':'=&lt;'}
...
class StyleConfig(object):
    __doc__ = 'Configuration class from elyxer.config file'
    hspaces = {'\\enskip{}':'\u2002', 
     '\\hfill{}':'<span class="hfill"> </span>',  '\\hspace*{\\fill}':'\u2003', 
     '\\hspace*{}':'',  '\\hspace{}':'\u2003',  '\\negthinspace{}':'', 
     '\\qquad{}':'\u2003\u2003',  '\\quad{}':'\u2003',  '\\space{}':'\xa0', 
     '\\thinspace{}':'\u2009',  '~':'\xa0'}
    quotes = {'ald':'»', 
     'als':'›',  'ard':'«',  'ars':'‹',  'eld':'&ldquo;',  'els':'&lsquo;', 
     'erd':'&rdquo;',  'ers':'&rsquo;',  'fld':'«',  'fls':'‹', 
     'frd':'»',  'frs':'›',  'gld':'„',  'gls':'‚',  'grd':'“', 
     'grs':'‘',  'pld':'„',  'pls':'‚',  'prd':'”',  'prs':'’', 
     'sld':'”',  'srd':'”'}
    referenceformats = {'eqref':'(@↕)', 
     'formatted':'¶↕',  'nameref':'$↕',  'pageref':'#↕',  'ref':'@↕', 
     'vpageref':'on-page#↕',  'vref':'@on-page#↕'}
    size = {'ignoredtexts': ['col', 'text', 'line', 'page', 'theight', 'pheight']}
    vspaces = {'bigskip':'<div class="bigskip"> </div>', 
...
class TOCConfig(object):
    __doc__ = 'Configuration class from elyxer.config file'
    extractplain = {'allowed':[
      'StringContainer', 'Constant', 'TaggedText', 'Align', 'TextFamily', 'EmphaticText', 'VersalitasText', 'BarredText', 'SizeText', 'ColorText', 'LangLine', 'Formula'], 
     'cloned':[
      ''], 
     'extracted':['']}
    extracttitle = {'allowed':[
      'StringContainer', 'Constant', 'Space'], 
     'cloned':[
      'TextFamily', 'EmphaticText', 'VersalitasText', 'BarredText', 'SizeText', 'ColorText', 'LangLine', 'Formula'], 
     'extracted':[
      'PlainLayout', 'TaggedText', 'Align', 'Caption', 'StandardLayout', 'FlexInset']}
...
class TagConfig(object):
    ...
    family = {'sans':'span class="sans"', 
     'typewriter':'tt'}
    flex = {'CharStyle:Code':'span class="code"', 
     'CharStyle:MenuItem':'span class="menuitem"', 
     'Code':'span class="code"', 
     'MenuItem':'span class="menuitem"',  'Noun':'span class="noun"', 
     'Strong':'span class="strong"'}
    group = {'layouts': ['Quotation', 'Quote']}
    layouts = {'Center':'div', 
     'Chapter':'h?',  'Date':'h2',  'Paragraph':'div',  'Part':'h1', 
     'Quotation':'blockquote',  'Quote':'blockquote',  'Section':'h?', 
     'Subsection':'h?',  'Subsubsection':'h?'}
    listitems = {'Enumerate':'ol', 
     'Itemize':'ul'}
    notes = {'Comment':'', 
     'Greyedout':'span class="greyedout"',  'Note':''}
    script = {'subscript':'sub', 
     'superscript':'sup'}
    shaped = {'italic':'i', 
     'slanted':'i',  'smallcaps':'span class="versalitas"'}


class TranslationConfig(object):
    __doc__ = 'Configuration class from elyxer.config file'
    constants = {'Appendix':'Appendix', 
     'Book':'Book',  'Chapter':'Chapter',  'Paragraph':'Paragraph', 
     'Part':'Part',  'Section':'Section',  'Subsection':'Subsection', 
     'Subsubsection':'Subsubsection',  'abstract':'Abstract', 
     'bibliography':'Bibliography',  'figure':'figure', 
     'float-algorithm':'Algorithm ',  'float-figure':'Figure ', 
     'float-listing':'Listing ',  'float-table':'Table ', 
     'float-tableau':'Tableau ',  'footnotes':'Footnotes', 
     'generated-by':'Document generated by ',  'generated-on':' on ', 
     'index':'Index',  'jsmath-enable':'Please enable JavaScript on your browser.', 
     'jsmath-requires':' requires JavaScript to correctly process the mathematics on this page. ', 
     'jsmath-warning':'Warning: ', 
     'list-algorithm':'List of Algorithms',  'list-figure':'List of Figures', 
     'list-table':'List of Tables',  'list-tableau':'List of Tableaux', 
     'main-page':'Main page',  'next':'Next', 
     'nomenclature':'Nomenclature',  'on-page':' on page ', 
     'prev':'Prev',  'references':'References',  'toc':'Table of Contents', 
     'toc-for':'Contents for ',  'up':'Up'}
    languages = {'american':'en', 
     'british':'en',  'deutsch':'de',  'dutch':'nl',  'english':'en', 
     'french':'fr',  'ngerman':'de',  'russian':'ru',  'spanish':'es'}


class CommandLineParser(object):
    __doc__ = 'A parser for runtime options'
    ...
    def readquoted(self, args, initial):
        ...
        value = initial[1:]
        while len(args) > 0:
            args[0].endswith('"') or args[0].startswith('--') or Trace.error('Appending ' + args[0])
            value += ' ' + args[0]
            del args[0]
...
class Options(object):
    __doc__ = 'A set of runtime options'
    instance = None
    location = None
    nocopy = False
    copyright = False
        ...
        if Options.help:
            ...
            if Options.version:
                ...
                if Options.forceformat:
                    Options.imageformat = Options.imageformat or Options.forceformat
            ...
            if Options.toc:
                Options.tocfor or Trace.error('Option --toc is deprecated; use --tocfor "page" instead')
    ...
    def parsefootnotes(self):
        ...
        if not Options.footnotes:
            ...
            if not Options.endfoot:
                if not Options.marginfoot:
                    if not Options.hoverfoot:
                        Options.hoverfoot = True
        elif not Options.numberfoot:
            if not Options.symbolfoot:
...
class BranchOptions(object):
    __doc__ = 'A set of options for a branch'
...
class Cloner(object):
    __doc__ = 'An object used to clone other objects.'
...
class ContainerExtractor(object):
    __doc__ = 'A class to extract certain containers.'
...
class LoneCommand(Parser):
    __doc__ = 'A parser for just one command line'
...
class TextParser(Parser):
    __doc__ = 'A parser for a command and a bit of text'
    stack = []
...
class ExcludingParser(Parser):
    __doc__ = 'A parser that excludes the final line'
...
class BoundedParser(ExcludingParser):
    __doc__ = 'A parser bound by a final line'
...
class BoundedDummy(Parser):
    __doc__ = 'A bound parser that ignores everything'
...
class ContentsOutput(ContainerOutput):
    __doc__ = 'Outputs the contents converted to HTML'
...
class TaggedOutput(ContentsOutput):
    __doc__ = 'Outputs an HTML tag surrounding the contents.'
    tag = None
    breaklines = False
    empty = False
...
class FilteredOutput(ContentsOutput):
    __doc__ = 'Returns the output in the contents, but filtered:'
...
class StringOutput(ContainerOutput):
    __doc__ = 'Returns a bare string as output'
...
class LineReader(object):
    __doc__ = 'Reads a file line by line'
    ...
    def finished(self):
        ...
        if self.lastline:
            if self.linenumber == self.lastline:
                return True
        if self.mustread:
            self.readline()
...
class LineWriter(object):
    __doc__ = 'Writes a file as a series of lists'
    file = False
...
class Globable(object):
    __doc__ = 'A bit of text which can be globbed (lumped together in bits).\n  Methods current(), skipcurrent(), checkfor() and isout() have to be\n  implemented by subclasses.'
    leavepending = False
...
class EndingList(object):
    __doc__ = 'A list of position endings'
    ...
    def pop(self, pos):
        ...
        if pos.isout():
            ...
            ending or Trace.error('No ending at ' + pos.current())
            return ''
        for each in reversed(self.endings):
            self.endings.remove(each)
            if each == ending:
                return each.ending
                each.optional or Trace.error('Removed non-optional ending ' + each)
    ...
    def findending(self, pos):
        ...
        for index, ending in enumerate(reversed(self.endings)):
            if ending.checkin(pos):
                return ending
                return ending.optional or None
...
class PositionEnding(object):
    __doc__ = 'An ending for a parsing position'
...
class TextPosition(Position):
    __doc__ = 'A parse position based on a raw text.'
...
class FilePosition(Position):
    __doc__ = 'A parse position based on an underlying file.'
...
class Container(object):
    __doc__ = 'A container for text and objects in a lyx file'
    partkey = None
    parent = None
    begin = None
...
class BlackBox(Container):
    __doc__ = 'A container that does not output anything'
...
class LyXFormat(BlackBox):
    __doc__ = 'Read the lyxformat command'
...
class StringContainer(Container):
    __doc__ = 'A container for a single string'
    parsed = None
    ...
    def replacespecial(self, line):
        ...
        if ContainerConfig.string['startcommand'] in replaced:
            if len(replaced) > 1:
                if self.begin:
    ...
    def changeline(self, line):
        ...
        if ContainerConfig.string['startcommand'] not in line:
            return line
...
class TaggedText(Container):
    ...
    def __unicode__(self):
        ...
        if not hasattr(self.output, 'tag'):
            ...
            return self.output.tag or 'Tagged <unknown tag>'
        return 'Tagged <' + self.output.tag + '>'


class DocumentParameters(object):
    __doc__ = 'Global parameters for the document.'
    pdftitle = None
...
class FormulaParser(Parser):
    ...
    def parseformula(self, reader):
        ...
        if begincommand in reader.currentline():
            if beginbracket in reader.currentline():
...
class MacroParser(FormulaParser):
    __doc__ = 'A parser for a formula macro.'
...
class TaggedBit(FormulaBit):
    __doc__ = 'A tagged string in a formula'
...
class FormulaConstant(Constant):
    __doc__ = 'A constant string in a formula'
...
class RawText(FormulaBit):
    __doc__ = 'A bit of text inside a formula'
...
class FormulaSymbol(FormulaBit):
    __doc__ = 'A symbol inside a formula'
...
class FormulaNumber(FormulaBit):
    __doc__ = 'A string of digits in a formula'
...
class Comment(FormulaBit):
    __doc__ = 'A LaTeX comment: % to the end of the line.'
...
class WhiteSpace(FormulaBit):
    __doc__ = 'Some white space inside a formula.'
...
class Bracket(FormulaBit):
    __doc__ = 'A {} bracket inside a formula'
    ...
    def innertext(self, pos):
        ...
        while (pos.finished() or pos.current()) in specialchars:
            self.add(self.factory.parseany(pos))
            if pos.checkskip(' '):
                self.original += ' '
            else:
    ...
    def innerliteral(self, pos):
        ...
        self.literal = ''
        while not pos.finished():
            if not pos.current() == self.ending:
...
class MathsProcessor(object):
    __doc__ = 'A processor for a maths construction inside the FormulaProcessor.'
...
class FormulaProcessor(object):
    __doc__ = 'A processor specifically for formulas.'
    processors = []
...
class Formula(Container):
    ...
    def mathjax(self):
        ...
        if self.header[0] != 'inline':
            tag += ';mode=display'
        self.contents = [
         TaggedText().constant(self.parsed, tag + '"', True)]
...
class FormulaFactory(object):
    __doc__ = 'Construct bits of formula'
    types = [
     FormulaSymbol, RawText, FormulaNumber, Bracket, Comment, WhiteSpace]
    ...
    def instance(self, type):
        ...
        if not (type not in self.instances or self.instances[type]):
    ...
    def clearskipped(self, pos):
        ...
        while not pos.finished():
            return self.skipany(pos) or None
...
import unicodedata, gettext

class Translator(object):
    __doc__ = 'Reads the configuration file and tries to find a translation.'
    instance = None
    ...
    def findtranslation(self):
        ...
        if not DocumentParameters.language:
            ...
            return
        if DocumentParameters.language not in TranslationConfig.languages:
            Trace.error('Unknown language ' + DocumentParameters.language)
            return
        if TranslationConfig.languages[DocumentParameters.language] == 'en':
            return
        langcodes = [
         TranslationConfig.languages[DocumentParameters.language]]
        try:
    ...
    def getuntranslated(self, key):
        ...
        if key not in TranslationConfig.constants:
...
class NumberCounter(object):
    __doc__ = 'A counter for numbers (by default).'
    name = None
    value = None
    mode = None
    master = None
...
class DependentCounter(NumberCounter):
    __doc__ = 'A counter which depends on another one (the master).'
...
class NumberGenerator(object):
    __doc__ = 'A number generator for unique sequences and hierarchical structures. Used in:'
    chaptered = None
    generator = None
    ...
    def isnumbered(self, type):
        ...
        if '*' in type:
            ...
            if self.isroman(type):
                return True
            return self.isinordered(type) or False
    ...
    def getlevel(self, type):
        ...
        if self.isunique(type):
            ...
            self.isinordered(type) or Trace.error('Unknown layout type ' + type)
            return 0
    ...
    def getparttype(self, type):
        ...
        if NumberGenerator.appendix:
            if self.getlevel(type) == 1:
                return 'Appendix'
    ...
    def getdependentcounter(self, type, master):
        ...
        if not (type not in self.counters or self.counters[type].master):
...
class ChapteredGenerator(NumberGenerator):
    __doc__ = 'Generate chaptered numbers, as in Chapter.Number.'
...
class ContainerSize(object):
    __doc__ = 'The size of a container.'
    width = None
    height = None
    maxwidth = None
    maxheight = None
    ...
    def checkvalidheight(self, container):
        ...
        if self.height:
            if self.extractnumber(self.height) == '1':
                if heightspecial == 'totalheight':
                    self.height = None
    ...
    def checkimage(self, width, height):
        ...
        if width:
            ...
            if self.scale:
                ...
                if self.scale:
                    self.height = self.height or self.scalevalue(height)
        else:
            if self.width:
                if not self.height:
                    self.height = 'auto'
...
class QuoteContainer(Container):
    __doc__ = 'A container for a pretty quote'
    ...
    def process(self):
        ...
        if self.type not in StyleConfig.quotes:
            Trace.error('Quote type ' + self.type + ' not found')
            self.html = ['"']
            return
        self.html = [
         StyleConfig.quotes[self.type]]
...
class EmphaticText(TaggedText):
    __doc__ = 'Text with emphatic mode'
...
class ShapedText(TaggedText):
    __doc__ = 'Text shaped (italic, slanted)'

    def process(self):
        self.type = self.header[1]
        if self.type not in TagConfig.shaped:
...
class TextFamily(TaggedText):
    __doc__ = 'A bit of text from elyxer.a different family'

    def process(self):
        """Parse the type of family"""
        self.type = self.header[1]
        if self.type not in TagConfig.family:
...
class BarredText(TaggedText):
    __doc__ = 'Text with a bar somewhere'

    def process(self):
        """Parse the type of bar"""
        self.type = self.header[1]
        if self.type not in TagConfig.barred:
...
class LangLine(TaggedText):
    __doc__ = 'A line with language information'

    def process(self):
        """Only generate a span with lang info when the language is recognized."""
        lang = self.header[1]
        if lang not in TranslationConfig.languages:
            self.output = ContentsOutput()
            return
...
class InsetLength(BlackBox):
    __doc__ = 'A length measure inside an inset.'
...
class Space(Container):
    __doc__ = 'A space of several types'
    ...
    def getlength(self):
        ...
        return len(self.contents) == 0 or isinstance(self.contents[0], InsetLength) or None
        return self.contents[0].length


class VerticalSpace(Container):
    __doc__ = 'An inset that contains a vertical space.'
    ...
    def process(self):
        ...
        if self.type not in StyleConfig.vspaces:
            ...
            return
        self.html = [
         StyleConfig.vspaces[self.type]]


class Align(Container):
    __doc__ = 'Bit of aligned text'
...
class Separator(Container):
    __doc__ = 'A separator string which is not extracted by extracttext().'
...
class StartAppendix(BlackBox):
    __doc__ = 'Mark to start an appendix here.'
...
class Link(Container):
    __doc__ = 'A link to another part of the document'
    anchor = None
    url = None
    type = None
    page = None
...
class LinkOutput(ContainerOutput):
    __doc__ = 'A link pointing to some destination'
...
class Postprocessor(object):
    __doc__ = 'Postprocess a container keeping some context'
    stages = []
    ...
    def postrecursive(self, container):
        ...
        if not hasattr(container, 'contents'):
            ...
            if hasattr(container, 'postprocess'):
                return container.postprocess or None
...
class StageDict(object):
    __doc__ = 'A dictionary of stages corresponding to classes'
    ...
    def getstage(self, element):
        ...
        if element.__class__ not in self.stagedict:
            return
        return self.stagedict[element.__class__]


class Label(Link):
    __doc__ = 'A label to be referenced'
    names = dict()
    lastlayout = None
...
class Reference(Link):
    __doc__ = 'A reference to a label.'
    references = dict()
    key = 'none'
    ...
    def process(self):
        ...
        self.formatcontents()
        if self.key not in Reference.references:
            Reference.references[self.key] = []
        Reference.references[self.key].append(self)

    def formatcontents--- This code section failed: ---

 L.4004         0  LOAD_FAST                'self'
                2  LOAD_METHOD              getparameter
                4  LOAD_STR                 'LatexCommand'
                6  CALL_METHOD_1         1  '1 positional argument'
                8  STORE_FAST               'formatkey'

 L.4005        10  LOAD_FAST                'formatkey'
               12  POP_JUMP_IF_TRUE     18  'to 18'

 L.4006        14  LOAD_STR                 'ref'
               16  STORE_FAST               'formatkey'
             18_0  COME_FROM            12  '12'

 L.4007        18  LOAD_STR                 '↕'
               20  LOAD_FAST                'self'
               22  STORE_ATTR               formatted

 L.4008        24  LOAD_FAST                'formatkey'
               26  LOAD_GLOBAL              StyleConfig
               28  LOAD_ATTR                referenceformats
               30  COMPARE_OP               in
               32  POP_JUMP_IF_FALSE    48  'to 48'

 L.4009        34  LOAD_GLOBAL              StyleConfig
               36  LOAD_ATTR                referenceformats
               38  LOAD_FAST                'formatkey'
               40  BINARY_SUBSCR    
               42  LOAD_FAST                'self'
               44  STORE_ATTR               formatted
               46  JUMP_FORWARD         62  'to 62'
             48_0  COME_FROM            32  '32'

 L.4011        48  LOAD_GLOBAL              Trace
               50  LOAD_METHOD              error
               52  LOAD_STR                 'Unknown reference format '
               54  LOAD_FAST                'formatkey'
               56  BINARY_ADD       
               58  CALL_METHOD_1         1  '1 positional argument'
               60  POP_TOP          
             62_0  COME_FROM            46  '46'

 L.4012        62  LOAD_FAST                'self'
               64  LOAD_METHOD              replace
               66  LOAD_STR                 '↕'
               68  LOAD_FAST                'self'
               70  LOAD_ATTR                direction
               72  CALL_METHOD_2         2  '2 positional arguments'
               74  POP_TOP          

 L.4013        76  LOAD_FAST                'self'
               78  LOAD_METHOD              replace
               80  LOAD_STR                 '#'
               82  LOAD_STR                 '1'
               84  CALL_METHOD_2         2  '2 positional arguments'
               86  POP_TOP          

 L.4014        88  LOAD_FAST                'self'
               90  LOAD_METHOD              replace
               92  LOAD_STR                 'on-page'
               94  LOAD_GLOBAL              Translator
               96  LOAD_METHOD              translate
               98  LOAD_STR                 'on-page'
              100  CALL_METHOD_1         1  '1 positional argument'
              102  CALL_METHOD_2         2  '2 positional arguments'
              104  POP_TOP          

 L.4015       106  LOAD_FAST                'self'
              108  LOAD_ATTR                destination
              110  LOAD_METHOD              findpartkey
              112  CALL_METHOD_0         0  '0 positional arguments'
              114  STORE_FAST               'partkey'

 L.4017       116  LOAD_FAST                'self'
              118  LOAD_METHOD              replace
              120  LOAD_STR                 '@'
              122  LOAD_FAST                'partkey'
              124  JUMP_IF_FALSE_OR_POP   130  'to 130'
              126  LOAD_FAST                'partkey'
              128  LOAD_ATTR                number
            130_0  COME_FROM           124  '124'
              130  CALL_METHOD_2         2  '2 positional arguments'
              132  POP_TOP          

 L.4018       134  LOAD_FAST                'self'
              136  LOAD_METHOD              replace
              138  LOAD_STR                 '¶'
              140  LOAD_FAST                'partkey'
              142  JUMP_IF_FALSE_OR_POP   148  'to 148'
              144  LOAD_FAST                'partkey'
              146  LOAD_ATTR                tocentry
 ...
 L.4019       152  LOAD_STR                 '$'
              154  LOAD_FAST                'self'
              156  LOAD_ATTR                formatted
              158  COMPARE_OP               not-in
              160  POP_JUMP_IF_TRUE    172  'to 172'
              162  LOAD_FAST                'partkey'
              164  POP_JUMP_IF_FALSE   172  'to 172'
              166  LOAD_FAST                'partkey'
              168  LOAD_ATTR                titlecontents
              170  POP_JUMP_IF_TRUE    190  'to 190'
            172_0  COME_FROM           164  '164'
            172_1  COME_FROM           160  '160'

 L.4021       172  LOAD_GLOBAL              Constant
              174  LOAD_FAST                'self'
              176  LOAD_ATTR                formatted
              178  CALL_FUNCTION_1       1  '1 positional argument'
              180  BUILD_LIST_1          1 
              182  LOAD_FAST                'self'
              184  STORE_ATTR               contents

 L.4022       186  LOAD_CONST               None
              188  RETURN_VALUE     
            190_0  COME_FROM           170  '170'

 L.4023       190  LOAD_FAST                'self'
              192  LOAD_ATTR                formatted
              194  LOAD_METHOD              split
              196  LOAD_STR                 '$'
              198  CALL_METHOD_1         1  '1 positional argument'
              200  STORE_FAST               'pieces'

 L.4024       202  LOAD_GLOBAL              Constant
              204  LOAD_FAST                'pieces'
              206  LOAD_CONST               0
              208  BINARY_SUBSCR    
              210  CALL_FUNCTION_1       1  '1 positional argument'
              212  BUILD_LIST_1          1 
              214  LOAD_FAST                'self'
              216  STORE_ATTR               contents

 L.4025       218  SETUP_LOOP          272  'to 272'
              220  LOAD_FAST                'pieces'
              222  LOAD_CONST               1
              224  LOAD_CONST               None
              226  BUILD_SLICE_2         2 
              228  BINARY_SUBSCR    
              ...
              234  STORE_FAST               'piece'

 L.4026       236  LOAD_FAST                'self'
              238  DUP_TOP          
              240  LOAD_ATTR                contents
              242  LOAD_FAST                'partkey'
              244  LOAD_ATTR                titlecontents
              246  INPLACE_ADD      
              248  ROT_TWO          
              250  STORE_ATTR               contents

 L.4027       252  LOAD_FAST                'self'
              254  LOAD_ATTR                contents
              256  LOAD_METHOD              append
              258  LOAD_GLOBAL              Constant
              260  LOAD_FAST                'piece'
              ...
              270  POP_BLOCK        
            272_0  COME_FROM_LOOP      218  '218'

Parse error at or near `POP_BLOCK' instruction at offset 270
...
class FormulaCommand(FormulaBit):
    __doc__ = 'A LaTeX command inside a formula'
    types = []
...
class CommandBit(FormulaCommand):
    __doc__ = 'A formula bit that includes a command'
...
class EmptyCommand(CommandBit):
    __doc__ = 'An empty command (without parameters)'
    commandmap = FormulaConfig.commands

    def parsebit(self, pos):
        """Parse a command without parameters"""
        self.contents = [
         FormulaConstant(self.translated)]


class SpacedCommand(CommandBit):
    __doc__ = 'An empty command which should have math spacing in formulas.'
...
class AlphaCommand(EmptyCommand):
    __doc__ = 'A command without paramters whose result is alphabetical'
...
class OneParamFunction(CommandBit):
    __doc__ = 'A function of one parameter'
...
class SymbolFunction(CommandBit):
    __doc__ = 'Find a function which is represented by a symbol (like _ or ^)'
...
class TextFunction(CommandBit):
    __doc__ = 'A function where parameters are read as text.'
...
class LabelFunction(CommandBit):
    __doc__ = 'A function that acts as a label'
...
class FontFunction(OneParamFunction):
    __doc__ = 'A function of one parameter that changes the font'
...
class BigSymbol(object):
    __doc__ = 'A big symbol generator.'
...
class BigBracket(BigSymbol):
    __doc__ = 'A big bracket generator.'
    ...
    def getcontents(self):
        ...
        return self.size == 1 or self.pieces or self.getsinglebracket()
        rows = []
        ...
        return [
         TaggedBit().complete(rows, 'span class="array"')]

    def getsinglebracket(self):
        """Return the bracket as a single sign."""
        if self.original == '.':
            return [
             TaggedBit().constant('', 'span class="emptydot"')]
        return [
         TaggedBit().constant(self.original, 'span class="symbol"')]


class FormulaEquation(CommandBit):
    __doc__ = 'A simple numbered equation.'
    piece = 'equation'
...
class FormulaCell(FormulaCommand):
    __doc__ = 'An array cell inside a row'
...
class FormulaRow(FormulaCommand):
    __doc__ = 'An array row inside an array'
...
class MultiRowFormula(CommandBit):
    __doc__ = 'A formula with multiple rows.'
...
class FormulaArray(MultiRowFormula):
    __doc__ = 'An array within a formula'
    piece = 'array'
...
class FormulaMatrix(MultiRowFormula):
    __doc__ = 'A matrix (array with center alignment).'
    piece = 'matrix'
...
class EquationEnvironment(MultiRowFormula):
    __doc__ = 'A \\begin{}...\\end equation environment with rows and cells.'
...
class BeginCommand(CommandBit):
    __doc__ = 'A \\begin{}...\\end command and what it entails (array, cases, aligned)'
    commandmap = {FormulaConfig.array['begin']: ''}
    types = [
     FormulaEquation, FormulaArray, FormulaCases, FormulaMatrix]
...
class DecoratingFunction(OneParamFunction):
    __doc__ = 'A function that decorates some bit of text'
...
class LimitCommand(EmptyCommand):
    __doc__ = 'A command which accepts limits above and below, in display mode.'
...
class LimitPreviousCommand(LimitCommand):
    __doc__ = 'A command to limit the previous command.'
    commandmap = None
...
class LimitsProcessor(MathsProcessor):
    __doc__ = 'A processor for limits inside an element.'
    ...
    def checklimits(self, contents, index):
        ...
        if not DocumentParameters.displaymode:
            ...
            if self.checkcommand(contents, index + 1, LimitPreviousCommand):
                ...
                return False
            return isinstance(contents[index], LimitCommand) or False
...
class BracketCommand(OneParamFunction):
    __doc__ = 'A command which defines a bracket.'
...
class BracketProcessor(MathsProcessor):
    __doc__ = 'A processor for bracket commands.'
...
class ParameterDefinition(object):
    __doc__ = 'The definition of a parameter in a hybrid function.'
    parambrackets = [
     ('[', ']'), ('{', '}')]
        ...
        for opening, closing in ParameterDefinition.parambrackets:
            if pos.checkskip(opening):
                if opening == '[':
                    ...
                    if pos.checkskip('!'):
                        self.literal = True
                    pos.checkskip(closing) or Trace.error('Wrong parameter closing ' + pos.skipcurrent())
                    return
                return self
...
class ParameterFunction(CommandBit):
    __doc__ = 'A function with a variable number of parameters defined in a template.'
    ...
    def getliteralvalue(self, name):
        ...
        return param and param.literalvalue or None
...
class HybridFunction(ParameterFunction):
    ...
    def writeparam(self, pos):
        ...
        if name not in self.params:
            ...
            return self.params[name] or None
        if pos.checkskip('.'):
...
class HybridSize(object):
    __doc__ = 'The size associated with a hybrid function.'
...
class HeaderParser(Parser):
    ...
    def parseline(self, reader, contents):
        ...
        if line.startswith(HeaderConfig.parameters['branch']):
            ...
            return
        if line.startswith(HeaderConfig.parameters['lstset']):
            LstParser().parselstset(reader)
            return
        if line.startswith(HeaderConfig.parameters['beginpreamble']):
...
class PreambleParser(Parser):
    __doc__ = 'A parser for the LyX preamble.'
    preamble = []
...
class LstParser(object):
    __doc__ = 'Parse global and local lstparams.'
    globalparams = dict()
...
class MacroDefinition(CommandBit):
    __doc__ = 'A function that defines a new command (a macro).'
    macros = dict()
...
class MacroParameter(FormulaBit):
    __doc__ = 'A parameter from elyxer.a macro.'
...
class MacroFunction(CommandBit):
    __doc__ = 'A function that was defined using a macro.'
...
class FormulaMacro(Formula):
    __doc__ = 'A math macro defined in an inset.'
