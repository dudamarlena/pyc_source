...
# [GCC 8.4.0]
# Embedded file name: /Users/tkomiya/work/sphinx/.tox/py37/lib/python3.7/site-packages/docutils/writers/_html_base.py
# Compiled at: 2018-11-25 06:19:18
# Size of source mod 2**32: 63742 bytes
...
class HTMLTranslator(nodes.NodeVisitor):
    ...
    def is_compactable(self, node):
        if 'compact' in node['classes']:
            ...
            if isinstance(node, (nodes.enumerated_list, nodes.bullet_list)):
                return self.settings.compact_lists or False
    ...
    def depart_colspec(self, node):
        if isinstance(node.next_node(descend=False, siblings=True), nodes.colspec):
            return
        if ('colwidths-auto' in node.parent.parent['classes'] or 'colwidths-auto') in self.settings.table_style:
    ...
    def depart_document(self, node):
        ...
        if self.context:
            raise AssertionError('len(context) = %s' % len(self.context))
    ...
    def visit_image(self, node):
        ...
        if 'scale' in node:
            if PIL:
                if 'width' in node:
                    if 'height' in node or self.settings.file_insertion_enabled:
        ...
        elif (isinstance(node.parent, nodes.TextElement) or isinstance)(node.parent, nodes.reference):
            suffix = isinstance(node.parent.parent, nodes.TextElement) or ''
        else:
            suffix = '\n'
        if 'align' in node:
    ...
    def depart_label(self, node):
        ...
        if self.settings.footnote_backlinks:
            if len(backrefs) > 1:
                backlinks = ['<a href="#%s">%s</a>' % (ref, i + 1) for i, ref in enumerate(backrefs)]
    ...
    def visit_target(self, node):
        if not 'refuri' in node:
            'refid' in node or 'refname' in node or self.body.append(self.starttag(node, 'span', '', CLASS='target'))
