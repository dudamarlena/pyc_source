...
class TestShortEntryFormatter(object):
    ...
    def test_timestamp(self):
        ...
        if not @py_assert1:
            @py_format8 = @pytest_ar._call_reprcompare(('in', ), (@py_assert1,), ('%(py0)s in %(py7)s\n{%(py7)s = %(py4)s\n{%(py4)s = %(py2)s.format\n}(%(py5)s)\n}', ), (expected, @py_assert6)) % {'py0': @pytest_ar._saferepr(expected) if 'expected' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(expected) else 'expected',  'py5': @pytest_ar._saferepr(entry) if 'entry' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(entry) else 'entry',  'py2': @pytest_ar._saferepr(formatter) if 'formatter' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(formatter) else 'formatter',  'py4': @pytest_ar._saferepr(@py_assert3),  'py7': @pytest_ar._saferepr(@py_assert6)}
    ...
    @pytest.mark.parametrize(('entry', 'expected'), [
     ({'MESSAGE': 'message'},
      'localhost ?: message\n'),
     (
      {'_HOSTNAME': 'host',  'MESSAGE': 'message'},
      'host ?: message\n'),
     (
      {'_HOSTNAME': 'host',  '_COMM': 'comm', 
       'MESSAGE': 'message'},
      'host comm: message\n'),
     (
      {'_HOSTNAME': 'host',  '_COMM': 'comm', 
      ...
      'host comm[1]: message\n'),
     (
      {'_HOSTNAME': 'host',  'SYSLOG_IDENTIFIER': 'syslogid', 
       '_COMM': 'comm', 
       'MESSAGE': 'message'},
    ...
    def test_format(self, entry, expected):
        ...
        if not @py_assert4:
            @py_format6 = ('' + 'assert %(py5)s\n{%(py5)s = %(py2)s\n{%(py2)s = %(py0)s.startswith\n}(%(py3)s)\n}') % {'py0': @pytest_ar._saferepr(formatted) if 'formatted' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(formatted) else 'formatted',  'py3': @pytest_ar._saferepr(date) if 'date' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(date) else 'date',  'py2': @pytest_ar._saferepr(@py_assert1),  'py5': @pytest_ar._saferepr(@py_assert4)}
