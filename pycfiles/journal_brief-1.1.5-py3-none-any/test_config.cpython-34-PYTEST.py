...
class TestEntryCounter(object):
    ...
    def test_get_counts(self):
        reader = [
         {'MESSAGE': 'message 1',  'KEY': 'multiple'},
         {'MESSAGE': 'message 1',  'KEY': 'multiple'}, {'MESSAGE': 'message 1'}, {'MESSAGE': 'message 1'},
        ...
        if not @py_assert3:
            @py_format8 = @pytest_ar._call_reprcompare(('==', ), (@py_assert3,), ('%(py2)s\n{%(py2)s = %(py0)s.total_entries\n} == %(py7)s\n{%(py7)s = %(py4)s(%(py5)s)\n}', ), (@py_assert1, @py_assert6)) % {'py0': @pytest_ar._saferepr(counter) if 'counter' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(counter) else 'counter',  'py5': @pytest_ar._saferepr(reader) if 'reader' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(reader) else 'reader',  'py2': @pytest_ar._saferepr(@py_assert1),  'py4': @pytest_ar._saferepr(len) if 'len' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(len) else 'len',  'py7': @pytest_ar._saferepr(@py_assert6)}
        ...
        for exp_field, exp_value, exp_count in expected:
            ...
            if not @py_assert3:
                @py_format5 = @pytest_ar._call_reprcompare(('==', ), (@py_assert3,), ('%(py2)s\n{%(py2)s = %(py0)s.field\n} == %(py4)s', ), (@py_assert1, exp_field)) % {'py0': @pytest_ar._saferepr(count) if 'count' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(count) else 'count',  'py2': @pytest_ar._saferepr(@py_assert1),  'py4': @pytest_ar._saferepr(exp_field) if 'exp_field' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(exp_field) else 'exp_field'}
            ...
            if not @py_assert6:
                @py_format8 = @pytest_ar._call_reprcompare(('==', ), (@py_assert6,), ('%(py5)s\n{%(py5)s = %(py0)s(%(py3)s\n{%(py3)s = %(py1)s.entries\n})\n} == %(py7)s', ), (@py_assert4, exp_count)) % {'py0': @pytest_ar._saferepr(len) if 'len' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(len) else 'len',  'py3': @pytest_ar._saferepr(@py_assert2),  'py5': @pytest_ar._saferepr(@py_assert4),  'py7': @pytest_ar._saferepr(exp_count) if 'exp_count' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(exp_count) else 'exp_count',  'py1': @pytest_ar._saferepr(count) if 'count' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(count) else 'count'}
                @py_format10 = 'assert %(py9)s' % {'py9': @py_format8}
                raise AssertionError(@pytest_ar._format_explanation(@py_format10))
...
class TestDebriefer(object):
    ...
    def test_get_exclusions(self):
        ...
        if not @py_assert1:
            @py_format10 = @pytest_ar._call_reprcompare(('==', ), (@py_assert1,), ('%(py0)s == %(py9)s\n{%(py9)s = %(py5)s\n{%(py5)s = %(py3)s.join\n}(%(py7)s)\n}', ), (formatted, @py_assert8)) % {'py0': @pytest_ar._saferepr(formatted) if 'formatted' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(formatted) else 'formatted',  'py3': @pytest_ar._saferepr(@py_assert2),  'py9': @pytest_ar._saferepr(@py_assert8),  'py5': @pytest_ar._saferepr(@py_assert4),  'py7': @pytest_ar._saferepr(@py_assert6)}
