...
# [GCC 8.4.0]
# Embedded file name: /home/twaugh/devel/journal-brief/build/lib/tests/format/test_login.py
...
class TestLoginEntryFormatter(object):
    ...
    def test_login(self):
        ...
        for user in ['user1', 'user2', 'user1', 'User3']:
            ...
            if not @py_assert6:
                @py_format9 = @pytest_ar._call_reprcompare(('==', ), (@py_assert6,), ('%(py5)s\n{%(py5)s = %(py2)s\n{%(py2)s = %(py0)s.format\n}(%(py3)s)\n} == %(py8)s', ), (@py_assert4, @py_assert7)) % {'py0': @pytest_ar._saferepr(formatter) if 'formatter' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(formatter) else 'formatter',  'py3': @pytest_ar._saferepr(entry) if 'entry' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(entry) else 'entry',  'py2': @pytest_ar._saferepr(@py_assert1),  'py5': @pytest_ar._saferepr(@py_assert4),  'py8': @pytest_ar._saferepr(@py_assert7)}
        ...
        @py_assert10 = [
         '', 'User logins:', '', '    2 x user1', '    1 x user2', '    1 x User3']
        @py_assert9 = @py_assert7 == @py_assert10
        if not @py_assert9:
            @py_format12 = @pytest_ar._call_reprcompare(('==', ), (@py_assert9,), ('%(py8)s\n{%(py8)s = %(py6)s\n{%(py6)s = %(py4)s\n{%(py4)s = %(py2)s\n{%(py2)s = %(py0)s.flush\n}()\n}.splitlines\n}()\n} == %(py11)s', ), (@py_assert7, @py_assert10)) % {'py0': @pytest_ar._saferepr(formatter) if 'formatter' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(formatter) else 'formatter',  'py11': @pytest_ar._saferepr(@py_assert10),  'py4': @pytest_ar._saferepr(@py_assert3),  'py8': @pytest_ar._saferepr(@py_assert7),  'py2': @pytest_ar._saferepr(@py_assert1),  'py6': @pytest_ar._saferepr(@py_assert5)}
