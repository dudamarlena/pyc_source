...
class TestRebootEntryFormatter(object):
    ...
    def test_reboot(self):
        ...
        if not @py_assert7:
            @py_format10 = @pytest_ar._call_reprcompare(('==', ), (@py_assert7,), ('%(py6)s\n{%(py6)s = %(py2)s\n{%(py2)s = %(py0)s.format\n}(%(py4)s)\n} == %(py9)s', ), (@py_assert5, @py_assert8)) % {'py6': @pytest_ar._saferepr(@py_assert5), 'py0': @pytest_ar._saferepr(formatter) if 'formatter' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(formatter) else 'formatter', 'py4': @pytest_ar._saferepr(@py_assert3), 'py9': @pytest_ar._saferepr(@py_assert8), 'py2': @pytest_ar._saferepr(@py_assert1)}
