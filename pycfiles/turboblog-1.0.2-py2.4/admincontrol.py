...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/turboblog/admincontrol.py
# Compiled at: 2007-04-02 19:23:06
"""
...
from turboblog.model import Blog, Settings, hub, Post, Comment, Tag, User, Group, StoredFile
...
from turboblog.forms import file_upload_form, writepost_form, search_text_form, search_date_form, edit_comment_form, submod
import cherrypy
...
def render_submenu(blog_id, active):
    ...
    """
    subelements = [
     'posts', 'comments', 'tags', 'files']
    submenu = Element('ul', id='submenu')
    for subelement in subelements:
        s = SubElement(submenu, 'li', title=_('Manage %s' % subelement.capitalize()))
        a = SubElement(s, 'a', href=tg.url('/blog_admin/manage_%s?bid=%s' % (subelement, blog_id)))
...
def render_adminmenu(blog_id):
    ...
    returns: an ElementTree.Element instance containing the desired HTML code
    """
    subelements = [{'name': 'admin', 'url': tg.url('/admin'), 'text': _('Site Dashboard'), 'popup': _('Lets you manage all the blogs')}, {'name': 'dashboard', 'url': tg.url('/blog_admin?bid=%s' % blog_id), 'text': _('Blog Dashboard'), 'popup': _('Lets you manage your blog details')}, {'name': 'write', 'url': tg.url('/blog_admin/write?bid=%s' % blog_id), 'text': _('Write'), 'popup': _('Write a new post for your blog')}, {'name': 'manage', 'url': tg.url('/blog_admin/manage?bid=%s' % blog_id), 'text': _('Manage'), 'popup': _('Manage posts, comments, tags and files')}, {'name': 'settings', 'url': tg.url('/blog_admin/settings?bid=%s' % blog_id), 'text': _('Settings'), 'popup': _('Change the blog settings such as name and tagline')}]
...
class AdminController(Controller, identity.SecureResource):
    ...
    def user_create(self, *args, **kw):
        ...
        hub.begin()
        u = User(user_name=kw['login'], display_name=kw['fullname'], password=kw['psw'], email_address=kw['email'], avatar=None, about='')
        for g in groups:
    ...
    def blogs(self):
        return dict(defblog=SETTINGS.default_blog, curadmin=SETTINGS.admin)
...
class BlogAdminController(Controller, identity.SecureResource):
    ...
    def kick(self, kw):
        ...
        if not identity.current.user == blog.owner and not identity.current.user == SETTINGS.admin:
    ...
    def save_post(self, bid=None, post_id=None, post_title=None, content=None, publication_state=False, trackback_url=None):
        self.kick({'bid': bid})
        ...
        if edit:
            ...
            p = Post(title=post_title, content=content, author=user, published=publication_state, blog=blog, trackback_urls=trackback_url)
        hub.commit()
        if p.published:
            p.send_trackbacks()
    ...
    def manage_files(self, *args, **kw):
        ...
        adminmenu = render_adminmenu(bid)
        return dict(files=blog.get_files(), bid=bid, blog=blog, submenu=submenu, adminmenu=adminmenu)
    ...
    def manage_comments(self, *args, **kw):
        ...
        if 's' in kw.keys():
            ...
            try:
                ...
                blog_comments = blog.search_comments_bydate(start_date, end_date)
        else:
        ...
        sd_params = dict(values=dict(bid=bid, calendar_lang='en'), options=dict(), form_opts=dict(action=tg.url('/blog_admin/manage_comments?bid=%s' % bid), submit_text=_('Search')))
        st_params = dict(values=dict(bid=bid), options=dict(), form_opts=dict(action=tg.url('/blog_admin/manage_comments?bid=%s' % bid), submit_text=_('Search')))
        return dict(blog=blog, mass=mass, blog_comments=blog_comments, submenu=submenu, adminmenu=adminmenu, submod=submod, search_text=search_text, search_date=search_date, sd_params=sd_params, st_params=st_params)
    ...
    def edit_comment(self, bid, cid):
        ...
        params = dict(values=dict(bid=bid, cid=comment.id, comment_content=comment.content), options=dict(), form_opts=dict(action=tg.url('/blog_admin/save_comment'), submit_text=_('Save'), form_attrs=dict()))
    ...
    def save_comment(self, bid, cid, comment_content):
        ...
        try:
            ...
            payload = '<html><body>%s</body></html>' % _('The comment does not exist')
            return payload

        comment.content = comment_content
        hub.commit()
        html_payload = '<html><body                 onload="javascript:window.top.hidePopWin(true);">'
    ...
    def comment_value(self, cid):
        ...
        """
        comment = Comment.get(cid)
        return dict(comment_id=cid, comment_content=comment.content)
    ...
    def manage_posts(self, *args, **kw):
        ...
        if 's' in kw.keys():
            ...
            try:
                ...
                blog_posts = blog.search_posts_bydate(start_date, end_date)
        else:
            blog_posts = blog.get_posts()
        sd_params = dict(values=dict(bid=bid, calendar_lang='en'), options=dict(), form_opts=dict(action=tg.url('/blog_admin/manage_posts?bid=%s' % bid), submit_text=_('Search')))
        st_params = dict(values=dict(bid=bid), options=dict(), form_opts=dict(action=tg.url('/blog_admin/manage_posts?bid=%s' % bid), submit_text=_('Search')))
        return dict(blog=blog, blog_posts=blog_posts, submenu=submenu, adminmenu=adminmenu, search_text=search_text, search_date=search_date, st_params=st_params, sd_params=sd_params)
    ...
    def settings_reading(self, *args, **kw):
        ...
        if rss_type == 'summary':
            ...
            rss_summary = False
        adminmenu = render_adminmenu(bid)
        return dict(blog=blog, adminmenu=adminmenu, posts_per_page=posts_per_page, posts_per_rss=posts_per_rss, rss_summary=rss_summary)
    ...
    def write(self, *args, **kw):
        ...
        if post:
            params = dict(values=dict(bid=bid, post_id=post.id, post_title=post.title, content=post.content, publication_state=post.published, disabled_fields=[]), options=dict())
        else:
            params = dict(values=dict(bid=bid))
        return dict(writepost_form=writepost_form, params=params, blog=blog, adminmenu=adminmenu)
    ...
    @expose()
    def update_general_settings(self, bid, blogname, blogdescription, admin, theme, submit):
        """
        ...
        blog.theme = theme
        hub.commit()
        tg.flash({'status': 'success', 'msg': _('Settings saved successfully')})
        tg.redirect(tg.url('/blog_admin/settings_general?bid=%s' % bid))

    @expose()
    def update_read_settings(self, bid, posts_per_page, posts_per_rss, rss_use_excerpt, Submit):
        """
        Exposed method that handles forms from the settings management
        for the read TAB
        """
    ...
    def add_file(self, bid):
        ...
        submenu = render_submenu(bid, 'files')
        adminmenu = render_adminmenu(bid)
        params = dict(values=dict(bid=bid), options=dict())
        return dict(bid=bid, blog=blog, adminmenu=adminmenu, submenu=submenu, file_upload_form=file_upload_form, params=params)

    @expose()
    @validate(validators={'file_obj': validators.FieldStorageUploadConverter()})
    def upload_file(self, bid, file_obj):
        ...
        newfile = StoredFile(filename=file_obj.filename, filesize=len(file_data), blog=Blog.get(bid), mimetype=file_obj.type, data=file_data)
        hub.commit()
        tg.redirect('/blog_admin/manage_files?bid=%s' % bid)