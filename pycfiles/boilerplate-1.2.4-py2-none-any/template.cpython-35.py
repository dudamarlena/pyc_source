...
# [GCC 8.4.0]
# Embedded file name: /home/jqb/projects/boilerplate/boilerplate/template.py
# Compiled at: 2019-02-15 10:01:28
...
", {
   ...
   >>>
"""
import re, os, sys, os.path as ospath
...
def process_line(line, context, var_re=VAR_RE):
    if not var_re.findall(line):
        return line
    return ''.join([resolve(part, context, var_re=var_re) for part in var_re.split(line)])
            ...
            if self.directory_should_be_ignored(dirname):
                ...
                for fname in filelist:
                    if self.file_should_be_ignored(fname):
                        ...
                        destination_file_path = ospath.join(destination_dir_path, destination_file_name)
...
class Template(object):
    ...
    def create(self, destination_path, project_name):
        ...
        creator.create(template_dir=config.get_template_absolute_path(), target=destination_path, context=config.get_context(project_name, template_name=self.name))
