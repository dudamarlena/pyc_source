...
class Handler(object):
    ...
    def parse_cmdline(self, argv):
        parser = OptionParser(usage='usage: %prog <template-name> <project-name>', description='Project structure generation tool')
        ...
        for tmpl_place in self.get_templates_places():
            if not ospath.exists(tmpl_place):
                if not self.completion_request():
                    self.stdout.write('WARNING Directory: %s, does not exists, omitting.\n' % tmpl_place)
                continue
    ...
    def handle(self, argv):
        ...
        if options.bash_completion:
            ...
            return 0
        if len(args) == 0:
            self.stderr.write("There's no enough arguments. Exact two args should be passed.\n")
            parser.print_usage(file=self.stderr)
            return 1
        template_name = args[0]
        if len(args) == 1:
            self.stderr.write('Please add project name\n')
            parser.print_usage(file=self.stderr)
            return 1
