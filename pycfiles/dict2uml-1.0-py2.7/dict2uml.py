...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/dict2uml/dict2uml.py
    ...
    for key, value in obj.iteritems():
        ...
        if isinstance(value, list):
            for idx, a in enumerate(value):
                children_str.append(('{}: {}').format(idx, '[object Object]'))
                if isinstance(a, dict):
                    children[idx] = a
                else:
                    children[idx] = {('value [{}]').format(type(a).__name__): a}

            vertices.append({'clsName': key, 
            ...
            for k, v in value.iteritems():
                if isinstance(v, dict):
                    children_str.append(('{}: {}').format(k, '[object Object]'))
                    children[k] = v
        ...
        if children:
            ...
            for o in tmpobj:
                o['clsNameFullReference'] = ('{}.{}').format(key, o['clsNameFullReference'])
                vertices.append(o)

            for child in children:
                if parent is not None:
                    edges.append(('"{}.{}" "1" --> "1" "{}.{}.{}"').format(parent, key, parent, key, child))
                else:
                    edges.append(('"{}" "1" --> "1" "{}.{}"').format(key, key, child))
...
def printClass(cls):
    ...
    s = ''
    s += ('class "{}" as {} {{').format(cls['clsName'], cls['clsNameFullReference']) + '\n'
    s += '\t.. Properties ..\n'
