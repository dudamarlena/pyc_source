...
# [GCC 8.4.0]
# Embedded file name: /Users/joshbraegger/bc/git-crucible/crucible/commands.py
# Compiled at: 2012-03-30 23:27:55
...
class CreateReview(object):
    ...
    def config_parser(parser):
        parser.add_argument('INFILE', type=argparse.FileType('r'), help='A patch-file to add.  Note when using stdin, you must specify all parameters on the command-line')
    ...
    def run(api, args):
        ...
        permaid = api.create_review(*[ args[i] for i in ('title', 'description', 'project',
                                                         'jira_issue', 'moderator') ])
...
class Main(object):
    ...
    def get_config(self):
        parser = argparse.ArgumentParser(description='Submit code reviews to Crucible.', formatter_class=argparse.RawTextHelpFormatter, epilog=self.epilog)
        ...
        for command in self.commands:
            subparser = subparsers.add_parser(command.command, help=command.help, epilog=command.epilog, formatter_class=argparse.RawTextHelpFormatter)
