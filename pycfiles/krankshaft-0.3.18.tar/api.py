...
# [GCC 8.4.0]
# Embedded file name: /Users/daniel_lamotte/sandbox/krankshaft/krankshaft/api.py
# Compiled at: 2014-01-03 16:06:08
from . import util
from .auth import Auth
from .exceptions import Abort, KrankshaftError, InvalidOptions, ResolveError, ValueIssue
from .serializer import Serializer
from .throttle import Throttle
from .util import Annotate
from .valid import Expecter
import functools, inspect, logging, sys, traceback, urlparse
...
class API(object):
    ...
    def __init__(self, name='', debug=False, error=None):
        """
        Options:
    ...
    def __call__(self, view_or_resource=None, register=True, url=None, **opts):
        ...
        """
        return self.wrap(view_or_resource, register=register, url=url, **opts)
    ...
    def abort(self, response_or_request, status=None, **headers):
        ...
        if hasattr(response_or_request, 'method'):
            if status is None:
                raise TypeError('abort() requires a status when passed a response')
            raise self.Abort(self.response(response_or_request, status, **headers))
        else:
            if headers:
                raise self.Error('Cannot pass headers with given a response')
    ...
    def deserialize(self, request, abortable=True):
        ...
        try:
            query = urlparse.parse_qs(request.META.get('QUERY_STRING', ''), keep_blank_values=True)
            query = MultiValueDict(query)
            content_type = request.META.get('CONTENT_TYPE')
            content_length = request.META.get('HTTP_CONTENT_LENGTH', request.META.get('CONTENT_LENGTH', 0))
            try:
            ...
            if content_type and content_length > 0:
                data = self.serializer.deserialize_request(request, content_type)
            else:
                data = {}
            return (
             query, data)
    ...
    def dispatch(self, view, opts, request, *args, **kwargs):
        ...
        try:
            ...
            if opts['methods'] is not None and request.method not in opts['methods'] and request.method.lower() not in opts['methods']:
                return self.response(request, 405, Allow=(', ').join([ method.upper() for method in opts['methods']
                                                                     ]))
            ...
            if auth:
                ...
                throttle = self.Throttle if opts['throttle'] is True else opts['throttle']
                if throttle:
                    throttle = throttle(request, auth)
                    allowed, headers = throttle.allow(suffix=opts['throttle_suffix'])
                    if not allowed:
                        return self.throttled(request, **headers)
            with Annotate(request, {'auth': auth, 
    ...
    def handler500(self, request, exc_info, error=None):
        ...
        log.error('%s, %s: %s', error, exc.__name__, inst, exc_info=exc_info, extra=self.extra())
        data = {'error': error}
        if self.debug:
    ...
    def make_resource_helper(self, klass_to_wrap, opts):
        ...
        class Helper(object):
            ...
            def __call__(self, request, *args, **kwargs):
                view = lambda request, *args, **kwargs: api.route(self.instance, request, args, kwargs)
    ...
    def options_dispatch(self, opts):
        ...
        """
        return util.valid(util.defaults({} if opts is None else opts, self.defaults_dispatch), self.defaults_dispatch.keys())
    ...
    def register(self, view, url=None):
        ...
        """
        if url is not None:
            if not isinstance(url, basestring) and isinstance(url, (list, tuple)) and not 1 <= len(url) <= 4:
                raise self.Error('register called with invalid url param: %r' % (url,))
            if hasattr(view, 'urls'):
                raise self.Error('Will not register a resource to a url if it has an urls attribute')
    ...
    def resolve(self, paths):
        ...
        for path in paths:
            ...
            view_resource = getattr(view, 'resource', None) or getattr(view, 'im_self', None)
            if not view or not view_resource:
                raise self.ResolveError('Unable to find a resource for path: %s' % path)
            ids.append(args[0] if args else kwargs.values()[0])
            if resource is None:
                resource = view_resource
            if resource != view_resource:
                raise self.ResolveError('Multiple resources found for given paths')
    ...
    def route(self, obj, request, args, kwargs):
        ...
        if hasattr(obj, 'route'):
            ...
            if isinstance(obj, dict):
                ...
                avail = {method:getattr(obj, method, None) for method in self.methods}
            method = request.method.lower()
            view = avail.get(method)
            if not view:
                return self.response(request, 405, Allow=(', ').join([ method.upper() for method, view in avail.iteritems() if view
                                                                     ]))
            return view(request, *args, **kwargs)
    ...
    def schema_default(self, view):
        return {'doc': view.__doc__, 
           'endpoint': {}, 'url': ''}

    def schema_view(self, request):
        return self.serialize(request, 200, self.schema)

    def serialize(self, request, status, obj, content_type=None, opts=None, **headers):
        """serialize(request, 200, obj) -> response

        Serialize an status and object to a response given a request.
        """
        opts = opts or {}
        content, content_type = self.serializer.serialize(obj, (content_type or request.META.get('HTTP_ACCEPT')), **opts)
    ...
    def urls(self):
        ...
        for view, url in self.registry:
            if url:
                urlitem = (
                 url, view, None, self.endpoint(view.__name__))
        ...
        if self.name:
            urlpatterns = patterns('', url('^%s/' % self.name, include(urlpatterns)))
        return urlpatterns
    ...
    def urls_local(self):
        ...
        return patterns('', url('^$', self.wrap(self.schema_view), name=self.endpoint('schema')))

    def wrap(self, view_or_resource=None, register=False, url=None, **opts):
        r"""wrap(myview) -> wrapped_view
        ...
        def decorator(view_or_resource):
            if inspect.isclass(view_or_resource):
                ...
                def view(request, *args, **kwargs):
                    return self.dispatch(view_or_resource, opts, request, *args, **kwargs)
