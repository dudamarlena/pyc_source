...
# [GCC 8.4.0]
# Embedded file name: /Users/daniel_lamotte/sandbox/krankshaft/krankshaft/resource.py
# Compiled at: 2014-02-24 14:04:36
...
class DjangoModelResource(object):
    ...
    def __init__(self):
        self.fields = tuple([ field for field in list(self.model._meta.fields) + [ field for field, model in self.model._meta.get_m2m_with_model() ] if self.fields is None or field.name in self.fields
                            ])
        if self.excludes:
            self.fields = tuple([ field for field in self.fields if field.name not in self.excludes
                                ])
        ...
        if self.allowed_methods:
            self.allowed_methods = tuple([ method.lower() for method in self.allowed_methods
                                         ])
        if self.allowed_methods_endpoint:
            self.allowed_methods_endpoint = {endpoint:tuple([ method.lower() for method in methods ]) for endpoint, methods in self.allowed_methods_endpoint.iteritems() if methods if methods}
        if self.version_field and self.version_field_allow_put_list is None:
    ...
    def allowed(self, endpoint, map):
        ...
        tocheck = [
         ...
         self.allowed_methods_endpoint.get(endpoint) if self.allowed_methods_endpoint else None]
        newmap = {}
    ...
    def clean(self, request, data, list=False):
        ...
        try:
            ...
            self.api.abort(self.api.serialize(request, 422, {'error': 'Supplied data was invalid', 
               'invalid': exc.errors}))
    ...
    def clean_id(self, request, id):
        ...
        try:
            ...
            if not request:
                raise
            self.api.abort(self.api.serialize(request, 400, {'error': 'Invalid ID for model %s' % self.model.__name__, 
               'invalid': exc.errors}))

    def check(self, request, instance, clean, abortable=True):
        r"""check(request, instance, clean)
        ...
        while True:
            if instance and instance.pk and self.version_field:
                if self.version_field not in clean:
                    code = 400
                    detail = {'error': 'The "%s" field must be specified' % self.version_field}
                    break
    ...
    def fetch(self, *ids):
        ...
        if set_ids != set_instance_pks:
            ...
            raise self.model.DoesNotExist('Could not find instances for: %s' % (', ').join([ str(id) for id in list(missing) ]))
    ...
    def fetch_list(self, request, query, qs=None):
        ...
        try:
            ...
            self.api.abort(self.api.serialize(request, 403, {'error': 'There are issues with your query', 
               'invalid': exc.errors}))
    ...
    def fetch_set(self, request, idset, qs=None):
        ...
        set_instance_pks = set([ instance.pk for instance in instances ])
        if set_ids != set_instance_pks:
            self.api.abort(self.api.serialize(request, 404, {'error': 'Missing some requested objects', 
    ...
    def load_post(self):
        ...
        """
        self.expected.update({field.name:self.api.expecter.from_field(field) for field in self.fields})
        for field in self.fields:
            is_many = isinstance(field, models.ManyToManyField)
            is_related = is_many or isinstance(field, models.ForeignKey)
            if not is_related:
                continue
            self.expected[field.name] = self.make_related_validator(field, self.expected[field.name], many=is_many)
            ...
            if value is not None and resource:
                try:
                    ...
                    if resolved_resource != resource:
                        raise self.api.ResolveError('Unexpected resource found: %s (expected %s)' % (
                         resolved_resource.name, resource.name))
                except self.api.ResolveError as e:
                    raise ValueError('Unable to resolve related: %s, %s' % (value, str(e)))
    ...
    def routes(self):
        ...
        """
        prefix = '^%s/' % self.name
        list_methods = {'delete': self.delete_list, 
           'get': self.get_list, 
           'post': self.post_list, 
           'put': self.put_list}
        ...
        return (
         ...
         (
          'single', prefix + '(?P<id>[^/]+)/$',
          self.allowed('single', {'delete': self.delete, 
             'get': self.get, 
             'put': self.put}),
          ('id', )),
         (
          'set', prefix + 'set/(?P<idset>[^/](?:[^/]|;)*)/$',
          self.allowed('set', {'delete': self.delete_set, 
             'get': self.get_set, 
             'put': self.put_set}),
          ('idset', )))
    ...
    def schema(self):
        ...
        for endpoint, regex, methods, params in self.routes:
            schema['endpoint'][endpoint] = {'allow': [ method.upper() for method in methods.keys()
                      ], 
               'docs': {method.upper():view.__doc__ for method, view in methods.iteritems()}, 
               'params': params, 
               'url': self.reverse(endpoint, args=[ ':' + param for param in params
                            ])}

        schema['fields'] = {}
        for field in self.fields:
            d = schema['fields'][field.name] = {'help_text': field.help_text if isinstance(field.help_text, basestring) else '', 
               'indexed': self.Query.is_indexed(field), 
               'nullable': field.null, 
               'type': field.__class__.__name__}
            if field.choices:
                d['choices'] = [ (value, display) for value, display in field.choices
                               ]
    ...
    def serialize(self, instance):
        ...
        data = {}
        for field in self.fields:
            if instance._deferred and isinstance(instance.__class__.__dict__.get(field.attname), DeferredAttribute):
                continue
            ...
            if method:
                ...
                data[field.name] = [ self.related(field, related.pk) for related in qs
                                   ]
                data[field.name + '_id'] = [ related.pk for related in qs
                                           ]
            elif isinstance(field, models.ForeignKey):
                data[field.name] = self.related(field, getattr(instance, field.name + '_id'))
                data[field.name + '_id'] = getattr(instance, field.name + '_id')
            elif isinstance(field, models.FileField):
                try:
                    data[field.name] = getattr(instance, field.name).name
                    data[field.name + '_href'] = getattr(instance, field.name).url
                except ValueError:
                    data[field.name] = ''
    ...
    def update(self, instance, clean):
        ...
        for field in self.fields:
            ...
            if method:
                ...
                getattr(instance, field.name).save(file.name, file, save=False)
            else:
        ...
        for field, instances in manytomany.iteritems():
            ...
            if hasattr(instance, '_prefetched_objects_cache') and field.name in instance._prefetched_objects_cache:
                instance._prefetched_objects_cache[field.name] = instances
    ...
    def urls(self):
        ...
        return patterns('', *[ url(regex, self.api.wrap(self.make_view(self.endpoint(endpoint), methods)), name=self.endpoint(endpoint)) for endpoint, regex, methods, params in self.routes
                             ])
    ...
    def get_list(self, request):
        ...
        query, data = self.api.deserialize(request)
        instances, meta = self.fetch_list(request, query)
        return self.api.serialize(request, 200, {'meta': meta, 
           'objects': [ self.serialize(instance) for instance in instances
                    ]})
    ...
    def get_set(self, request, idset):
        ...
        """
        instances = self.fetch_set(request, idset)
        return self.api.serialize(request, 200, {'objects': [ self.serialize(instance) for instance in instances
                    ]})
    ...
    def post_list(self, request):
        ...
        if self.use_location:
            return self.api.response(request, 204, Location=self.reverse_single(instance.pk))
        else:
            return self.api.serialize(request, 200, self.serialize(instance))
    ...
    def put_list(self, request):
        ...
        """
        query, data = self.api.deserialize(request)
        query = self.make_query(query).without('defer', 'limit', 'offset', 'only')
        with atomic():
            instances, meta = self.fetch_list(request, query)
            for instance in instances:
        ...
        if self.use_location:
            idset = (';').join([ str(instance.pk) for instance in instances
                               ])
            return self.api.response(request, 204, Location=self.reverse('set', args=(idset,)))
        else:
            return self.api.serialize(request, 200, {'meta': meta, 
               'objects': [ self.serialize(instance) for instance in instances
                        ]})
    ...
    def put_set(self, request, idset):
        ...
        with atomic():
            ...
            if not all(self.pk_name in obj for obj in clean):
                return self.api.serialize(request, 400, {'error': 'You must supply the primary key with each object'})
            instance_lookup = {instance.pk:instance for instance in instances}
            code_and_details = []
            for obj in clean:
                id = obj[self.pk_name]
                instance = instance_lookup[id]
                code, detail = self.check(request, instance, obj, abortable=False)
                if code:
                    code_and_details.append((code, id, detail))

            if code_and_details:
                codes = list(set([ code for code, id, detail in code_and_details ]))
                has_details = any(detail for code, id, detail in code_and_details)
                if len(codes) == 1:
                    if has_details:
                        self.api.abort(self.api.serialize(request, codes[0], {'invalid': {id:detail for code, id, detail in code_and_details}}))
                    else:
                        self.api.abort(request, codes[0])
                else:
                    self.api.abort(self.api.serialize(request, 400, {'error': 'Mixed error codes', 
                       'invalid': {id:util.defaults({}, detail or {}, {'code': code}) for code, id, detail in code_and_details}}))
            for obj in clean:
                id = obj[self.pk_name]
