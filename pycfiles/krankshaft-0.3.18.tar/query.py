...
# [GCC 8.4.0]
# Embedded file name: /Users/daniel_lamotte/sandbox/krankshaft/krankshaft/query.py
# Compiled at: 2014-03-07 14:45:55
...
class Query(object):
    ...
    def options(self, opts, defaults):
        ...
        """
        return util.valid(util.defaults(opts, defaults), self.defaults.keys())
...
class DjangoQuery(Query):
    ...
    Any = object()
    Indexed = object()
    lookups_all = ('exact', 'iexact', 'contains', 'icontains', 'in', 'gt', 'gte', 'lt',
    ...
    defaults = {'defer_allow_related': False, 
       'filter': Indexed, 
    ...
    def apply(self, query, **opts):
        ...
        for name, value in self.qs.iteritems():
            ...
            if opts['filter'] is not self.Any and opts['filter'] is not self.Indexed and accessor_name not in opts['filter']:
            ...
            if not opts['filter_allow_related'] and len(accessor) > 1:
                ...
                continue
            if lookup and opts['filter_lookups'] is not self.Any and lookup not in opts['filter_lookups']:
            ...
            if lookup == 'in':
                ...
                value = value.split(',')
                if len(value) != 2:
                    errors.append('A range lookup requires exactly 2 params: %s' % name)
                    continue
            elif lookup == 'isnull':
                try:
                    value = valid.bool(value, None)
                except ValueError as exc:
                    errors.append('An isnull lookup requires a truthy/falsey value: %s' % name)
                ...
                if value and isinstance(field, (
                 ...
                 models.NullBooleanField)):
                    try:
                        ...
                        errors.append('Boolean fields require a truthy/falsey value: %s' % name)

            query = query.filter(**{name: value})

        if used_filter and opts['filter'] is self.Indexed and not filter_uses_index:
            errors.append('You are required to use an indexed field in the filter')
        ...
        if defer:
            ...
            for name in defer:
                ...
                if not opts['defer_allow_related'] and len(accessor) > 1:
                    errors.append('Cross relation defer not allowed: %s' % name)
        ...
        if only:
            ...
            for name in only:
                ...
                if not opts['only_allow_related'] and len(accessor) > 1:
                    errors.append('Cross relation only not allowed: %s' % name)
        ...
        if order_by:
            ...
            for name in order_by:
                accessor, lookup = self.parse_field(name[1:] if name.startswith('-') else name)
                accessor_name = ('__').join(accessor)
                field = self.get_field(model, accessor)
                if not ordering_uses_index:
                    ordering_uses_index = self.is_indexed(field)
                if opts['ordering'] is not self.Any and opts['ordering'] is not self.Indexed and accessor_name not in opts['ordering']:
                    errors.append('Unallowed ordering used: %s' % name)
                if not opts['ordering_allow_related'] and len(accessor) > 1:
                    errors.append('Cross relation ordering not allowed: %s' % name)

            if opts['ordering'] is self.Indexed and not ordering_uses_index:
                errors.append('You are required to use an indexed field in the order_by')
            ...
            try:
                ...
                if offset:
                    if opts['max_offset'] and offset > opts['max_offset']:
                        errors.append('Your offset is above the maximum allowed: %s ' % opts['max_offset'])
                        offset = 0
                    elif offset < 0:
                        errors.append('A offset must be positive')
                        offset = 0
                if opts['max_limit'] and limit > opts['max_limit']:
                    errors.append('Your limit is above the maximum allowed: %s' % opts['max_limit'])
                elif limit == 0:
    ...
    def make_meta(self, limit, offset):
        ...
        if offset >= limit:
            meta['previous'] = base + 'limit=%s&offset=%s' % (limit, offset - limit)
        return meta
    ...
    def parse_field(self, name):
        ...
        for part in name.split('__'):
            if part in self.lookups_all:
                if lookup:
                    raise self.Error('Multiple lookups found for field: %s' % name)
                lookup = part
            else:
                parsed.append(part)
