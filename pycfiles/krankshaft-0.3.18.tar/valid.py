...
# [GCC 8.4.0]
# Embedded file name: /Users/daniel_lamotte/sandbox/krankshaft/krankshaft/valid.py
# Compiled at: 2014-01-03 15:57:01
"""
...
class Expecter(object):
    ...
    defaults = {'ignore_extra_keys': False, 
    ...
    def expect(self, expected, data, **opts):
        ...
        if not method:
            raise self.ExpectedIssue('Your expected data structure is unhandled for type: %s' % expected.__class__)
    ...
    def expect_dict(self, expected, data, opts):
        ...
        if not (opts['ignore_extra_keys'] and opts['ignore_missing_keys']) and expected_keys != data_keys:
            extra_keys = data_keys - expected_keys
            missing_keys = expected_keys - data_keys
            if not opts['ignore_extra_keys'] and extra_keys:
                errors.setdefault('__nonkeyerrors__', []).append('Extra keys: %s' % (', ').join(list(extra_keys)))
            if not opts['ignore_missing_keys'] and missing_keys:
                errors.setdefault('__nonkeyerrors__', []).append('Missing keys: %s' % (', ').join(list(missing_keys)))
    ...
    def expect_list(self, expected, data, opts):
        ...
        if len(expected) == 1:
            ...
            errors['__nonindexerrors__'] = ['Expected list of length %s, saw %s' % (
              len(expected),
              len(data))]
        if errors:
            raise self.ValueIssue(errors)
    ...
    def from_field(self, field):
        ...
        if field.__class__ in (
         ...
         models.ManyToManyField):
            ...
            try:
                ...
                raise self.ExpectedIssue('Unable to resolve "%s" to a validator, register your model field with api.expecter.register(Field, valid.validator)' % str(field))
        ...
        if field.primary_key:
            ...
            validator = blank_not(validator)
        if field.choices:
            validator = choice(validator, tuple([ value for value, display in field.choices
                                                ]))
        if hasattr(field, 'max_length') and field.max_length:
            if isinstance(field, models.FileField):
                validator = max_length(validator, field.max_length, accessor=lambda value: len(field.generate_filename(value, value.name)))
            else:
    ...
    def options(self, opts, defaults):
        ...
        """
        return util.valid(util.defaults(self.shortcuts(opts), defaults), self.defaults.keys())
...
def blank_not(validator):
    ...
    def blank_not_validator(value, expect):
        if value == '':
            raise ValueError('%s does not accept blank values' % blank_not_validator.__name__)
        return validator(value, expect)
...
def no_none(validator):
    ...
    def no_none_validator(value, expect):
        if value is None:
            raise ValueError('%s does not accept None' % no_none_validator.__name__)
        return validator(value, expect)
...
def csv(validator, separator=','):
    ...
    def csv_validator(data, expect):
        ...
        if data is not None:
            clean = expect([
             validator], [ member for member in data.split(',') if member ])
            return (',').join([ str(member) for member in clean
                              ])
        return data
...
def list_n_or_more(validator, n):
    ...
    """
    if n < 1:
        raise KrankshaftError('list_n_or_more only accepts values >= 1, not %s' % n)
    ...
    def list_n_or_more_validator(data, expect):
        ...
        if clean is not None and len(clean) < n:
            errors.setdefault('__nonindexerrors__', []).append('Expected list with %s or more elements, saw %s' % (
             n, len(data)))
        if errors:
            raise expect.ValueIssue(errors)
        return clean

    return wraps(list_n_or_more_validator, validator, prefix='list_%s_or_more_' % n)
...
def max_length(validator, n, accessor=None):
    ...
    def max_length_validator(value, expect):
        ...
        if value is not None and accessor(value) > n:
            raise ValueError('The value is greater than max length %s: %s' % (
             n, accessor(value)))
        return value
...
def range(validator, low, high):
    ...
    def range_validator--- This code section failed: ---

 L. 610         0  LOAD_DEREF            1  'validator'
                3  LOAD_FAST             0  'value'
                6  LOAD_FAST             1  'expect'
                9  CALL_FUNCTION_2       2  None
               12  STORE_FAST            0  'value'

 L. 611        15  LOAD_FAST             0  'value'
               18  LOAD_CONST               None
               21  COMPARE_OP            9  is-not
               24  POP_JUMP_IF_FALSE   144  'to 144'

 L. 613        27  LOAD_DEREF            2  'low'
               30  LOAD_CONST               None
               33  COMPARE_OP            9  is-not
               36  POP_JUMP_IF_FALSE    79  'to 79'
               39  LOAD_DEREF            0  'high'
               42  LOAD_CONST               None
               45  COMPARE_OP            9  is-not
             48_0  COME_FROM            36  '36'
               48  POP_JUMP_IF_FALSE    79  'to 79'
               51  LOAD_DEREF            2  'low'
               54  LOAD_FAST             0  'value'
               57  DUP_TOP          
               58  ROT_THREE        
               59  COMPARE_OP            1  <=
               62  JUMP_IF_FALSE_OR_POP    74  'to 74'
               65  LOAD_DEREF            0  'high'
               68  COMPARE_OP            1  <=
               71  JUMP_ABSOLUTE       112  'to 112'
             74_0  COME_FROM            62  '62'
               74  ROT_TWO          
               75  POP_TOP          
               76  JUMP_FORWARD         33  'to 112'

 L. 614        79  LOAD_DEREF            2  'low'
               82  LOAD_CONST               None
               85  COMPARE_OP            9  is-not
               88  POP_JUMP_IF_FALSE   103  'to 103'
               91  LOAD_DEREF            2  'low'
               94  LOAD_FAST             0  'value'
               97  COMPARE_OP            1  <=
              100  JUMP_FORWARD          9  'to 112'

 L. 615       103  LOAD_FAST             0  'value'
              106  LOAD_DEREF            0  'high'
              109  COMPARE_OP            1  <=
            112_0  COME_FROM           100  '100'
            112_1  COME_FROM            76  '76'
              112  UNARY_NOT        
            113_0  COME_FROM            24  '24'
              113  POP_JUMP_IF_FALSE   144  'to 144'

 L. 617       116  LOAD_GLOBAL           1  'ValueError'

 L. 618       119  LOAD_CONST               'The value is not within the range %s <= %s <= %s'

 L. 619       122  LOAD_DEREF            2  'low'
              125  LOAD_FAST             0  'value'
              128  LOAD_DEREF            0  'high'
              131  BUILD_TUPLE_3         3 
              134  BINARY_MODULO    
              135  CALL_FUNCTION_1       1  None
              138  RAISE_VARARGS_1       1  None
              141  JUMP_FORWARD          0  'to 144'
            144_0  COME_FROM           141  '141'

 L. 621       144  LOAD_FAST             0  'value'
              147  RETURN_VALUE     

Parse error at or near `JUMP_ABSOLUTE' instruction at offset 71
...
def bool_or_none(value, expect):
    ...
    value = choice(str_or_none_lower, ('0', 'false', 'no', 'null') + truthy)(value, expect)
    if value is None:
        return
    else:
...
Expecter.field_to_validator = {models.AutoField: int_or_none_range(1, 2147483647), 
