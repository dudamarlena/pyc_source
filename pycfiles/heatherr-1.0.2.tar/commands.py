...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/smn/heatherr/heatherr/groups/commands.py
...
def list(request, match):
    ...
    """
    groups = Group.objects.filter(slackaccount__team_id=request.POST['team_id'])
    person, _ = Person.objects.get_or_create(person_id=request.POST['user_id'])
    if not groups.exists():
        return JsonResponse({'text': 'No groups exist'})
    return 'Groups:\n%s' % (
     ('\n').join([ '- %s%s' % (unicode(group), ' (member)' if person.groups.filter(pk=group.pk).exists() else '') for group in groups
            ]),)


@announce.respond('^create (?P<group_name>[\\w-]+)$')
def create(request, match):
    ...
    group, created = Group.objects.get_or_create(slackaccount=slackaccount, group_name=group_name)
    if created:
...
@announce.respond('^join (?P<group_name>[\\w-]+)$')
...
@announce.respond('^leave (?P<group_name>[\\w-]+)$')
...
@announce.respond('^members (?P<group_name>[\\w-]+)$')
def members(request, match):
    ...
    try:
        ...
        return 'People in %s: %s' % (
         group_name,
         (', ').join([ person.person_name for person in group.person_set.all() ]))
    except Group.DoesNotExist:
        return 'The group %s does not exist.' % (group_name,)


@announce.respond('^announce (?P<group_name>[\\w-]+) (?P<message>.+)$')
def announce(request, match):
    ...
    try:
        ...
        return 'Message from <@%s> to `%s`:\n%s\n%s' % (
         ...
         (' ').join([ '<@%s>' % (member.person_id,) for member in group.person_set.all()
           ]),
         message)
    except Group.DoesNotExist:
        return 'The group %s does not exist.' % (group_name,)