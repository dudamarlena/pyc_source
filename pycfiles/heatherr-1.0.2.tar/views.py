...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/smn/heatherr/heatherr/views.py
...
class NotFoundHandler(object):
    ...
    def handle(self, request):
        return JsonResponse({'text': "Sorry, I don't know what to do with `%s`." % (
...
class CommandRouter(object):
    ...
    def auto_document(self):
        self.registry[self.auto_document_handler].append('^help$')
    ...
    def auto_document_handler(self, request, match):
        ...
        """
        docstrings = ('\n\n').join([ inspect.cleandoc(func.__doc__) for func in self.registry
                                   ])
        if self.command:
    ...
    def noop(self, text):
        return JsonResponse({'text': "Sorry, don't know what to do."})
