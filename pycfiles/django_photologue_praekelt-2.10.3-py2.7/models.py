...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/photologue/models.py
# Compiled at: 2015-04-10 08:21:09
import os, random, shutil, zipfile, utils, unicodedata, logging, re
...
try:
    import Image, ImageFile, ImageFilter, ImageEnhance
except ImportError:
    try:
...
class GalleryUpload(models.Model):
    zip_file = models.FileField(_('images file (.zip)'), upload_to=PHOTOLOGUE_DIR + '/temp', help_text=_('Select a .zip file of images to upload into a new Gallery.'))
    ...
    def process_zipfile(self):
        if os.path.isfile(self.zip_file.path):
            ...
            if self.gallery:
                ...
                gallery = Gallery.objects.create(title=self.title, title_slug=slugify(self.title), description=self.description, is_public=self.is_public, tags=self.tags)
                ...
                if len(data):
                    ...
                    while 1:
                        ...
                        try:
                            ...
                            photo = Photo(title=title, title_slug=slug, caption=self.caption, is_public=self.is_public, tags=self.tags)
...
class ImageModel(models.Model):
    image = models.ImageField(_('image'), max_length=IMAGE_FIELD_MAX_LENGTH, upload_to=get_storage_path, blank=True)
    ...
    def admin_thumbnail(self):
        ...
        if func is None:
            ...
            if hasattr(self, 'get_absolute_url'):
                ...
                if self.image:
                    return '<a href="%s"><img src="%s"></a>' % (self.image.url, func())
                return ''
    ...
    def _get_SIZE_filename(self, size):
        photosize = PhotoSizeCache().sizes.get(size)
        return smart_str(os.path.join(self.cache_path(), self._get_filename_for_size(photosize.name)))
    ...
    def add_accessor_methods(self, *args, **kwargs):
        for size in PhotoSizeCache().sizes.keys():
            ...
            setattr(self, 'get_%s_photosize' % size, curry(self._get_SIZE_photosize, size=size))
            setattr(self, 'get_%s_url' % size, curry(self._get_SIZE_url, size=size))
            setattr(self, 'get_%s_filename' % size, curry(self._get_SIZE_filename, size=size))
    ...
    def resize_image(self, im, photosize):
        ...
        if photosize.crop:
            ...
            if self.crop_from == 'top':
                box = (
                 int(x_diff), 0, int(x_diff + new_width), new_height)
            elif self.crop_from == 'left':
                box = (
                 0, int(y_diff), new_width, int(y_diff + new_height))
            elif self.crop_from == 'bottom':
                box = (
                 int(x_diff), int(yd), int(x_diff + new_width), int(y))
            elif self.crop_from == 'right':
                box = (
                 int(xd), int(y_diff), int(x), int(y_diff + new_height))
            else:
                box = (
                 int(x_diff), int(y_diff), int(x_diff + new_width), int(y_diff + new_height))
    ...
    def create_size(self, photosize):
        if not self.image:
            ...
            if image_model_obj.effect is not None:
                ...
                if photosize.effect is not None:
        ...
        if self.date_taken is None:
            try:
                ...
                if exif_date is not None:
                    ...
                    self.date_taken = datetime(int(year), int(month), int(day), int(hour), int(minute), int(second))
            except:
                pass
