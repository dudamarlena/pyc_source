...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/photologue/tests.py
...
class PLTest(TestCase):
    ...
    def setUp(self):
        ...
        self.pl.image.save(os.path.basename(LANDSCAPE_IMAGE_PATH), ContentFile(open(LANDSCAPE_IMAGE_PATH, 'rb').read()))
        self.pl.save()
...
class PhotoTest(PLTest):
    ...
    def test_new_photo(self):
        ...
        self.assertEqual(os.path.getsize(self.pl.image.path), os.path.getsize(LANDSCAPE_IMAGE_PATH))

    def test_paths(self):
        self.assertEqual(os.path.normpath(str(self.pl.cache_path())).lower(), os.path.normpath(os.path.join(settings.MEDIA_ROOT, PHOTOLOGUE_DIR, 'photos', 'cache')).lower())
        self.assertEqual(self.pl.cache_url(), settings.MEDIA_URL + PHOTOLOGUE_DIR + '/photos/cache')
    ...
    def test_accessor_methods(self):
        ...
        self.assertEquals(self.pl.get_test_size(), Image.open(self.pl.get_test_filename()).size)
        self.assertEquals(self.pl.get_test_url(), self.pl.cache_url() + '/' + self.pl._get_filename_for_size(self.s))
        self.assertEquals(self.pl.get_test_filename(), os.path.join(self.pl.cache_path(), self.pl._get_filename_for_size(self.s)))
...
class ImageModelTest(PLTest):
    ...
    def setUp(self):
        ...
        self.pu.image.save(os.path.basename(UNICODE_IMAGE_PATH), ContentFile(open(UNICODE_IMAGE_PATH, 'rb').read()))
        self.pn = TestPhoto(name='portrait')
        self.pn.image.save(os.path.basename(NONSENSE_IMAGE_PATH), ContentFile(open(NONSENSE_IMAGE_PATH, 'rb').read()))
        self.ps = TestPhoto(name='portrait')
        self.ps.image.save(os.path.basename(SPACED_IMAGE_PATH), ContentFile(open(SPACED_IMAGE_PATH, 'rb').read()))
