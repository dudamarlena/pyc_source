...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/ore/dtmlview/dtmlview.py
# Compiled at: 2008-10-31 09:53:36
"""
...
class DTMLViewFile(object):
    ...
    def get_context(self, instance, request, *args, **kw):
        namespace = dict(request=request, view=instance, context=instance.context, views=ViewMapper(instance.context, request))
        kw.update(namespace)
        return kw
    ...
    def _cook_check(self):
        ...
        self._read_file()
        if self._v_errors:
            logging.error('DocumentTemplateFile: Error in template %s: %s', self.filename, ('\n').join(self._v_errors))
            return
