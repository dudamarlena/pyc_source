...
# [GCC 8.4.0]
# Embedded file name: /Users/MyKings/Documents/github/clocwalk/clocwalk/libs/core/update_mysql.py
# Compiled at: 2019-12-26 21:09:20
# Size of source mod 2**32: 9238 bytes
import datetime, glob, gzip, json, os, re, shutil, time, gevent, requests
...
class Upgrade(object):
    ...
    def __init__(self, proxies=None, upgrade_interval_day='7d', http_timeout=15):
        ...
        interval_type = re.search('(\\d+)(\\w)', upgrade_interval_day)
        if interval_type:
            if interval_type.group(2) in ('d', 'h'):
                if interval_type.group(2) == 'd':
                    self.upgrade_interval = 86400 * int(interval_type.group(1))
                else:
                    if interval_type.group(2) == 'h':
                        self.upgrade_interval = 3600 * int(interval_type.group(1))
                    else:
                        self.upgrade_interval = 604800
    ...
    def cve_upgrade(self):
        ...
        def get_problem_type(info):
            ...
            """
            result = ''
            if 'problemtype_data' in info:
                if info['problemtype_data']:
                    if 'description' in info['problemtype_data'][0]:
                        if info['problemtype_data'][0]['description']:
        ...
        def get_links(info):
            ...
            """
            result = []
            if 'reference_data' in info:
                if info['reference_data']:
            ...
            with open(cve_file, 'rb') as (fp):
                ...
                for _ in json_obj['CVE_Items']:
                    if not _['configurations']['nodes']:
                        ...
                        year_re = re.search('CVE-(\\d+)-\\d+', cve, re.I)
                        ...
                        if 'cpe_match' not in _['configurations']['nodes'][0]:
                            if 'children' in _['configurations']['nodes'][0] and 'cpe_match' in _['configurations']['nodes'][0]['children'][0]:
