...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gentex/comat.py
# Compiled at: 2019-10-04 13:17:54
# Size of source mod 2**32: 20304 bytes
...
try:
    ...
    print('Failed to load _libmakecomat.so.  Compile the library using python setup.py build_ext -i from the package root directory.')
    sys.exit(-1)
...
array_3d_int = np.ctypeslib.ndpointer(dtype=(np.intc), ndim=3, flags='CONTIGUOUS')
array_4d_int = np.ctypeslib.ndpointer(dtype=(np.intc), ndim=4, flags='CONTIGUOUS')
libmakecomat_api = {'makecomat1D':(
  None,
  [
   array_1d_int,
   array_1d_int,
...
def comat(image, mask, coords, levels=255):
    ...
    if dims == 1:
        ...
        assert len(coords) == 1
        _comat.makecomat1D(image, mask, image.shape[0], coords, levels, out)
    if dims == 2:
        assert image.ndim == 2
        assert image.min() >= 0
        assert image.max() < levels
        ...
        coords = np.asarray(coords, dtype=c_int)
        assert len(coords) == 2
        _comat.makecomat2D(image, mask, image.shape[0], image.shape[1], coords, levels, out)
    if dims == 3:
        assert image.ndim == 3
        assert image.min() >= 0
        assert image.max() < levels
        ...
        coords = np.asarray(coords, dtype=c_int)
        assert len(coords) == 3
        _comat.makecomat3D(image, mask, image.shape[0], image.shape[1], image.shape[2], coords, levels, out)
    if dims == 4:
        assert image.ndim == 4
        assert image.min() >= 0
        assert image.max() < levels
        ...
        coords = np.asarray(coords, dtype=c_int)
        assert len(coords) == 4
        _comat.makecomat4D(image, mask, image.shape[0], image.shape[1], image.shape[2], image.shape[3], coords, levels, out)
    return out
...
def comat_2T(image1, mask1, image2, mask2, coords, levels1=255, levels2=255):
    ...
    if dims == 1:
        ...
        coords = np.asarray(coords, dtype=c_int)
        assert len(coords) == 1
        _comat.makecomat1D_2T(image1, mask1, image1.shape[0], image2, mask2, image2.shape[0], coords, levels1, levels2, out)
    if dims == 2:
        ...
        coords = np.asarray(coords, dtype=c_int)
        assert len(coords) == 2
        _comat.makecomat2D_2T(image1, mask1, image1.shape[0], image1.shape[1], image2, mask2, image2.shape[0], image2.shape[1], coords, levels1, levels2, out)
    if dims == 3:
        ...
        coords = np.asarray(coords, dtype=c_int)
        assert len(coords) == 3
        _comat.makecomat3D_2T(image1, mask1, image1.shape[0], image1.shape[1], image1.shape[2], image2, mask2, image2.shape[0], image2.shape[1], image2.shape[2], coords, levels1, levels2, out)
    if dims == 4:
        ...
        coords = np.asarray(coords, dtype=c_int)
        assert len(coords) == 4
        _comat.makecomat4D_2T(image1, mask1, image1.shape[0], image1.shape[1], image1.shape[2], image1.shape[3], image2, mask2, image2.shape[0], image2.shape[1], image2.shape[2], image2.shape[3], coords, levels1, levels2, out)
    return out
