...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cnfformula/cnf.py
# Compiled at: 2019-10-28 05:33:57
# Size of source mod 2**32: 46571 bytes
...
class CNF(object):
    ...
    def add_clause(self, clause, literal_repetitions=False, opposite_literals=False, auto_variables=True, strict=False):
    ...
    def latex(self, export_header=True, extra_text=None, full_document=False):
        r"""Output a LaTeX version of the CNF formula
            ...
            def write_clause(cls, first, full_document):
                ...
                if len(cls) == 0:
                    ...
                    if full_document:
                        ...
                        output.write('\\left( ' + ' \\lor '.join(map_literals(l) for l in cls) + ' \\right)')
    ...
    class unary_mapping(object):
        __doc__ = 'Unary CNF representation of a mapping between two sets.'
        Domain = None
        Range = None
        Pattern = None
        Complete = False
                    ...
                    for a, b in combinations(self.Domain, 2):
                        for i, j in product(self.images(a), self.images(b)):
                            if self.RankRange[i] > self.RankRange[j]:
                                yield [
                                 (
                                  False, self.var_name(a, i)), (False, self.var_name(b, j))]

    class binary_mapping(object):
        __doc__ = 'Binary CNF representation of a mapping between two sets.'
        Domain = None
        Range = None
        Bits = None
        Injective = False
        ...
        def __init__(self, D, R, **kwargs):
            ...
            self.ImageToBits = {}
            self.BitsToImage = {}
            for i, bs in islice(enumerate(product([0, 1], repeat=(self.Bits))), len(self.Range)):
        ...
        def forbid_bitstring(self, i, bs):
            ...
            return [(bs[b] == 0, self.var_name(i, self.Bits - 1 - b)) for b in range(self.Bits)]
        ...
        def clauses(self):
            for i, bs in product(self.Domain, islice(product([0, 1], repeat=(self.Bits)), len(self.Range), None)):
            ...
            if self.NonDecreasing:
                pairs_of_maps = product(combinations(self.Domain, 2), combinations(self.Range, 2))
