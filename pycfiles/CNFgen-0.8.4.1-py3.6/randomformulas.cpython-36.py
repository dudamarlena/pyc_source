...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cnfformula/families/randomformulas.py
...
"""
    for assignment in assignments:
        if any(var in assignment and polarity == assignment[var] for polarity, var in cls):
            continue
        else:
            return False
...
def sample_clauses(k, indices, m, planted_assignments):
    ...
    while len(clauses) < m and t < 10 * m:
        t += 1
        cls = tuple((random.choice([True, False]), 'x_{0}'.format(i)) for i in sorted(random.sample(indices, k)))
...
class RandCmdHelper(object):
    __doc__ = 'Command line helper for random formulas\n    '
    name = 'randkcnf'
