...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cnfformula/transformations/substitutions.py
# Compiled at: 2019-10-28 05:33:57
# Size of source mod 2**32: 21954 bytes
from ..cnf import CNF
...
class BaseSubstitution(CNF):
    __doc__ = 'Apply a substitution to a formula\n    '
    ...
    def __init__(self, cnf, new_variables=None):
        ...
        if not cnf._coherent:
            ...
            for i in range(1, len(varadditional)):
                varadditional[i] = [list(self._compress_clause(cls)) for cls in varadditional[i]]

            for i in range(1, len(substitutions)):
                substitutions[i] = [list(self._compress_clause(cls)) for cls in substitutions[i]]

            for orig_cls in self._orig_cnf._clauses:
                domains = [substitutions[lit] for lit in orig_cls]
                domains = tuple(domains)
                block = [tuple([lit for clause in clause_tuple for lit in clause]) for clause_tuple in product(*domains)]
...
class IfThenElseSubstitution(BaseSubstitution):
    __doc__ = 'Transformed formula: substitutes variable with a three variables\n    if-then-else\n    '
...
class OrSubstitution(BaseSubstitution):
    __doc__ = 'Transformed formula: substitutes variable with a OR\n    '
...
class AllEqualSubstitution(BaseSubstitution):
    __doc__ = "Transformed formula: substitutes variable with 'all equals'\n    "
    ...
    def transform_a_literal(self, polarity, varname):
        ...
        if polarity:
            ...
            return [
             [(False, a) for a in names], [(True, a) for a in names]]


@register_cnf_transformation
class NotAllEqualSubstitution(AllEqualSubstitution):
    __doc__ = "Transformed formula: substitutes variable with 'not all equals'\n    "
...
class XorSubstitution(BaseSubstitution):
    __doc__ = 'Transformed formula: substitutes variable with a XOR\n    '
...
class FormulaLifting(BaseSubstitution):
    __doc__ = 'Formula lifting: Y variable select X values\n    '
...
class ExactlyOneSubstitution(BaseSubstitution):
    __doc__ = 'Transformed formula: exactly one variable is true\n    '

    def __init__(self, cnf, rank):
        self._rank = rank
        super(ExactlyOneSubstitution, self).__init__(cnf)
        self._header = 'Formula transformed by "exactly one"' + ' substitution over {} values\n\n'.format(self._rank) + self._header
    ...
    def transform_a_literal(self, polarity, varname):
        ...
        if polarity:
            ...
            for name in varnames:
                clauses.append([(False, name)] + [(True, other) for other in varnames if other != name])
...
class FlipPolarity(BaseSubstitution):
    __doc__ = 'Flip the polarity of variables\n    '
...
class VariableCompression(BaseSubstitution):
    ...
    def transform_a_literal(self, polarity, varname):
        ...
        if self._function == 'xor':
            ...
            if self._function == 'maj':
                threshold = (len(local_names) + 1) // 2
                if polarity:
