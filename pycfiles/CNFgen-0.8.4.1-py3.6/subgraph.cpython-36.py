...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cnfformula/families/subgraph.py
...
import cnfformula.families, cnfformula.cmdline
...
def SubgraphFormula(graph, templates, symmetric=False):
    ...
    if len(templates) == 0:
        ...
        if len(templates) == 1:
            ...
            if len(selectors) > 1:
                F.header = dedent('                 CNF encoding of the claim that a graph contains one among\n                 a family of {0} possible subgraphs.\n                 '.format(len(templates))) + F.header
            else:
                F.header = dedent('                 CNF encoding of the claim that a graph contains an induced\n                 copy of a subgraph.\n                 '.format(len(templates))) + F.header
            N = graph.order()
            k = max([s.order() for s in templates])
            var_name = lambda i, j: 'S_{{{0},{1}}}'.format(i, j)
            if symmetric:
                mapping = F.unary_mapping((list(range(k))), (list(range(N))), var_name=var_name, functional=True,
                  injective=True,
                  nondecreasing=True)
            else:
                mapping = F.unary_mapping((list(range(k))), (list(range(N))), var_name=var_name, functional=True,
        ...
        for i in range(len(templates)):
            ...
            if symmetric:
                localmaps = product(combinations(list(range(k)), 2), combinations(list(range(N)), 2))
            else:
                localmaps = product(combinations(list(range(k)), 2), permutations(list(range(N)), 2))
...
def BinaryCliqueFormula(G, k):
    ...
    clauses_gen = F.binary_mapping((range(1, k + 1)), (G.nodes()), injective=True,
      nondecreasing=True)
    ...
    for (i1, i2), (v1, v2) in product(combinations(range(1, k + 1), 2), combinations(G.nodes(), 2)):
...
class KCliqueCmdHelper(object):
    __doc__ = 'Command line helper for k-clique formula\n    '
    name = 'kclique'
...
class RWCmdHelper(object):
    __doc__ = 'Command line helper for ramsey graph formula\n    '
    name = 'ramlb'
    ...
    def setup_command_line(parser):
        ...
        - `parser`: parser to load with options.
        """
        parser.add_argument('k', metavar='<k>', type=int, action='store',
          help='size of the clique to be found')
        parser.add_argument('s', metavar='<s>', type=int, action='store',
...
class SubGraphCmdHelper(object):
    __doc__ = 'Command line helper for Graph Isomorphism formula\n    '
    name = 'subgraph'
