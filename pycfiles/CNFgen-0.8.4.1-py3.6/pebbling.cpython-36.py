...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cnfformula/families/pebbling.py
# Compiled at: 2019-11-21 07:56:44
# Size of source mod 2**32: 14043 bytes
...
import cnfformula.families, cnfformula.cmdline, sys
...
def _uniqify_list(seq):
    ...
    """
    seen = set()
    return [x for x in seq if x not in seen if not seen.add(x)]
...
def stone_formula_helper(F, D, mapping):
    ...
    for stone in mapping.range():
        ...
        F.add_variable((color_vn[stone]), description=('Stone ${}$ is red'.format(stone)))
    ...
    for v in vertices:
        for j in mapping.images(v):
            ...
            for stones_tuple in product(*tuple([s for s in mapping.images(v) if s != j] for v in pred)):
                F.add_clause_unsafe([(False, stone_vn[(p, s)]) for p, s in zip(pred, stones_tuple)] + [(False, stone_vn[(v, j)])] + [(False, color_vn[s]) for s in _uniqify_list(stones_tuple)] + [
                 (
                  True, color_vn[j])])
...
def StoneFormula(D, nstones):
    ...
    for v in vertices:
        for j in stones:
            ...
            cnf.add_variable((stone_vn[(v, j)]), description=('Stone ${1}$ on vertex ${0}$'.format(v, j)))

    for j in stones:
        color_vn[j] = 'R_{{{0}}}'.format(j)
        cnf.add_variable((color_vn[j]), description=('Stone ${}$ is red'.format(j)))
    ...
    for v in vertices:
        for j in stones:
            ...
            for stones_tuple in product([s for s in stones if s != j], repeat=(len(pred))):
                cnf.add_clause_unsafe([(False, stone_vn[(p, s)]) for p, s in zip(pred, stones_tuple)] + [(False, stone_vn[(v, j)])] + [(False, color_vn[s]) for s in _uniqify_list(stones_tuple)] + [
                 (
                  True, color_vn[j])])
...
class PebblingCmdHelper:
    __doc__ = 'Command line helper for pebbling formulas\n    '
    name = 'peb'
...
class StoneCmdHelper:
    __doc__ = 'Command line helper for stone formulas\n    '
    name = 'stone'
