...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cnfformula/cmdline.py
# Compiled at: 2019-11-21 10:18:59
# Size of source mod 2**32: 29027 bytes
...
"""
import sys, argparse, networkx, random
...
__all__ = [
 'register_cnfgen_subcommand', 'is_cnfgen_subcommand',
...
def register_cnfgen_subcommand(cls):
    ...
    """
    assert hasattr(cls, 'build_cnf') and hasattr(cls, 'setup_command_line') and hasattr(cls, 'name') and hasattr(cls, 'description')
...
def register_cnf_transformation_subcommand(cls):
    ...
    """
    assert hasattr(cls, 'transform_cnf') and hasattr(cls, 'setup_command_line') and hasattr(cls, 'name') and hasattr(cls, 'description')
...
class GraphHelper(object):
    __doc__ = 'Command Line helper for reading graphs\n    '
...
class DirectedAcyclicGraphHelper(GraphHelper):
    ...
    def setup_command_line(parser, suffix='', required=False):
        ...
        """
        gr = parser.add_argument_group(title=('Input directed acyclic graph (DAG) ' + suffix), description='\n                                     You can either read the input DAG from file according to one of\n                                     the formats, or generate it using one of the constructions included.')
        gr = gr.add_mutually_exclusive_group(required=required)
        gr.add_argument(('--input' + suffix), ('-i' + suffix), type=(argparse.FileType('r')),
          metavar='<input>',
          default='-',
          help="Read the DAG from <input>. Setting '<input>' to '-' is another way\n                        to read from standard input. (default: -) ")
        gr.add_argument(('--tree' + suffix), type=positive_int, action='store', metavar='<height>', help='rooted tree digraph')
        gr.add_argument(('--pyramid' + suffix), type=positive_int, action='store', metavar='<height>', help='pyramid digraph')
        gr = parser.add_argument_group('I/O options')
        gr.add_argument(('--savegraph' + suffix), ('-sg' + suffix), type=(argparse.FileType('w')),
          metavar='<graph_file>',
          default=None,
          help="Save the DAG to <graph_file>.\n                            Setting '<graph_file>' to '-' is\n                            another way to send the DAG to\n                            standard output.  (default:  -)\n                            ")
        gr.add_argument(('--graphformat' + suffix), ('-gf' + suffix), choices=(graph_formats()['dag']),
          default='autodetect',
    ...
    def obtain_graph(args, suffix=''):
        ...
        if getattr(args, 'tree' + suffix) is not None:
            ...
            if getattr(args, 'pyramid' + suffix) is not None:
                ...
                try:
                    print(("INFO: reading directed acyclic graph {} from '{}'".format(suffix, getattr(args, 'input' + suffix).name)), file=(sys.stderr))
                    D = readGraph(getattr(args, 'input' + suffix), 'dag', getattr(args, 'graphformat' + suffix))
        ...
        if getattr(args, 'savegraph' + suffix) is not None:
            writeGraph(D, getattr(args, 'savegraph' + suffix), 'dag', getattr(args, 'graphformat' + suffix))
        return D
...
class SimpleGraphHelper(GraphHelper):
    ...
    def setup_command_line(parser, suffix='', required=False):
        ...
        gr.add_argument(('--gnp' + suffix), nargs=2, action=IntFloat, metavar=('n',
                                                                               'p'), help='random graph according to G(n,p) model (i.e. independent edges)')
        gr.add_argument(('--gnm' + suffix), type=positive_int, nargs=2, action='store', metavar=('n',
                                                                                                 'm'), help='random graph according to G(n,m) model (i.e. m random edges)')
        gr.add_argument(('--gnd' + suffix), type=positive_int, nargs=2, action='store', metavar=('n',
                                                                                                 'd'), help='random d-regular graph according to G(n,d) model (i.e. d random edges per vertex)')
        gr.add_argument(('--grid' + suffix), type=positive_int, nargs='+', action='store', metavar=('d1',
                                                                                                    'd2'),
          help='n-dimensional grid of dimension d1 x d2 x ... ')
        gr.add_argument(('--torus' + suffix), type=positive_int, nargs='+', action='store', metavar=('d1',
                                                                                                     'd2'),
          help='n-dimensional torus grid of dimensions d1 x d2 x ... x dn')
        gr.add_argument(('--complete' + suffix), type=positive_int, action='store', metavar='<N>', help='complete graph on N vertices')
        gr.add_argument(('--empty' + suffix), type=positive_int, action='store', metavar='<N>', help='empty graph on N vertices')
        gr = parser.add_argument_group('Modifications for input graph ' + suffix)
        gr.add_argument(('--plantclique' + suffix), type=positive_int, action='store', metavar='<k>', help='choose k vertices at random and add all edges among them')
        gr.add_argument(('--addedges' + suffix), type=positive_int, action='store', metavar='<k>', help='add k NEW random edges to the graph (applied last)')
        ...
        gr.add_argument(('--graphformat' + suffix), ('-gf' + suffix), choices=(graph_formats()['simple']),
          default='autodetect',
    ...
    def obtain_graph(args, suffix=''):
        ...
        if getattr(args, 'gnd' + suffix) is not None:
            ...
            if getattr(args, 'gnp' + suffix) is not None:
                n, p = getattr(args, 'gnp' + suffix)
                G = networkx.gnp_random_graph(n, p)
            else:
                if getattr(args, 'gnm' + suffix) is not None:
                    n, m = getattr(args, 'gnm' + suffix)
                    G = networkx.gnm_random_graph(n, m)
                else:
                    if getattr(args, 'grid' + suffix) is not None:
                        G = networkx.grid_graph(getattr(args, 'grid' + suffix))
                    else:
                        if getattr(args, 'torus' + suffix) is not None:
                            G = networkx.grid_graph((getattr(args, 'torus' + suffix)), periodic=True)
                        else:
                            if getattr(args, 'complete' + suffix) is not None:
                                G = networkx.complete_graph(getattr(args, 'complete' + suffix))
                            else:
                                if getattr(args, 'empty' + suffix) is not None:
                                    G = networkx.empty_graph(getattr(args, 'empty' + suffix))
                                elif getattr(args, 'graphformat' + suffix) is not None:
                                    try:
                                        print(("INFO: reading simple graph {} from '{}'".format(suffix, getattr(args, 'input' + suffix).name)), file=(sys.stderr))
                                        G = readGraph(getattr(args, 'input' + suffix), 'simple', getattr(args, 'graphformat' + suffix))
                                    except ValueError as e:
                                        print(("ERROR ON '{}'. {}".format(getattr(args, 'input' + suffix).name, e)),
                                          file=(sys.stderr))
                                        exit(-1)
            ...
            if getattr(args, 'savegraph' + suffix) is not None:
                writeGraph(G, getattr(args, 'savegraph' + suffix), 'simple', getattr(args, 'graphformat' + suffix))
        return G
...
class BipartiteGraphHelper(GraphHelper):
    ...
    def setup_command_line(parser, suffix='', required=False):
        ...
        gr.add_argument(('--bp' + suffix), nargs=3, action=IntIntFloat, metavar=('l',
                                                                                 'r',
                                                                                 'p'), help='Random bipartite graph with independent edges')
        gr.add_argument(('--bm' + suffix), type=positive_int, nargs=3, action=BipartiteEdge, metavar=('l',
                                                                                                      'r',
                                                                                                      'm'), help='Bipartite graph with m random edges')
        gr.add_argument(('--bd' + suffix), type=positive_int, nargs=3, action=BipartiteLeft, metavar=('l',
                                                                                                      'r',
                                                                                                      'd'), help='Bipartite graph with d random edges per left vertex')
        gr.add_argument(('--bregular' + suffix), nargs=3, action=BipartiteRegular, metavar=('l',
                                                                                            'r',
                                                                                            'd'), help='Bipartite regular graph, with d random edges per left vertex.')
        gr.add_argument(('--bshift' + suffix), type=positive_int, nargs='*', action=BipartiteShift, metavar='N', help='Args <N> <M> <v1> <v2> ... NxM bipartite. Vertex i connexted to i+v1, i+v2,... (mod M)')
        gr.add_argument(('--bcomplete' + suffix), type=positive_int, nargs=2, action='store', metavar=('l',
                                                                                                       'r'), help='Complete bipartite graph')
        gr = parser.add_argument_group('Modify the graph structure')
        gr.add_argument(('--plantbiclique' + suffix), type=positive_int, nargs=2, action='store', metavar=('l',
                                                                                                           'r'), help='Plant a random (l,r)-bipartite clique')
        gr.add_argument(('--addedges' + suffix), type=positive_int, action='store', metavar='<k>', help='Add k NEW random edges to the graph (applied in the end)')
        gr = parser.add_argument_group('File I/O options', description=' \n                                     Additional option regarding the input and output of the files\n                                     containing the graph structure.\n                                     ')
        gr.add_argument(('--savegraph' + suffix), ('-sg' + suffix), type=(argparse.FileType('w')),
          metavar='<graph_file>',
          default=None,
          help="Save the graph to <graph_file>. Setting '<graph_file>' to '-'sends\n                        the graph to standard output. (default: -) ")
        gr.add_argument(('--graphformat' + suffix), ('-gf' + suffix), choices=(graph_formats()['bipartite']),
          default='autodetect',
          help='Format of the graph file. (default: autodetect)')
                ...
                if getattr(args, 'bd' + suffix) is not None:
                    ...
                    if getattr(args, 'bregular' + suffix) is not None:
                        ...
                        if getattr(args, 'bshift' + suffix) is not None:
                            ...
                            if getattr(args, 'bcomplete' + suffix) is not None:
                                ...
                                try:
                                    print(("INFO: reading bipartite graph {} from '{}'".format(suffix, getattr(args, 'input' + suffix).name)), file=(sys.stderr))
                                    G = readGraph(getattr(args, 'input' + suffix), 'bipartite', getattr(args, 'graphformat' + suffix))
                                except ValueError as e:
                                    print(("ERROR ON '{}'. {}".format(getattr(args, 'input' + suffix).name, e)), file=(sys.stderr))
                                    exit(-1)
                    ...
                    if getattr(args, 'addedges' + suffix) is not None:
                        ...
                        if hasattr(G, 'name'):
                            G.name = '{} with {} new random edges'.format(G.name, k)
            if getattr(args, 'savegraph' + suffix) is not None:
                writeGraph(G, getattr(args, 'savegraph' + suffix), 'bipartite', getattr(args, 'graphformat' + suffix))
        return G