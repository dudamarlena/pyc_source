...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/islatu/tests/test_stitching.py
# Compiled at: 2020-04-22 02:46:11
...
class TestStitcher(unittest.TestCase):
    ...
    def test_normalise_ter(self):
        ...
        """
        y = [
         1000000000.0, 1000000000.0, 1000000000.0, 316000000.0, 129000000.0, 62500000.0, 33700000.0, 20000000.0]
        dy = [10000000.0, 10000000.0, 10000000.0, 3160000.0, 1290000.0, 625000.0, 337000.0, 200000.0]
        x = np.logspace(-2, -0.8, 8)
        reflected_intensity = unumpy.uarray(y, dy)
        exp_y = unumpy.uarray(y, dy) / 1000000000.0
        reflected_intensity = stitching.normalise_ter(x, reflected_intensity)
        assert_almost_equal(unumpy.nominal_values(reflected_intensity), unumpy.nominal_values(exp_y))
        assert_almost_equal(unumpy.std_devs(reflected_intensity), [
    ...
    def test_normalise_ter_a(self):
        ...
        assert_almost_equal(unumpy.nominal_values(reflected_intensity), unumpy.nominal_values(exp_y))
        assert_almost_equal(unumpy.std_devs(reflected_intensity), [
         0.0, 0.0141421, 0.0141421, 0.0044689,
    ...
    def test_normalise_ter_b(self):
        ...
        """
        y = [
         1000000000.0, 129000000.0, 62500000.0, 33700000.0, 20000000.0]
        dy = [10000000.0, 1290000.0, 625000.0, 337000.0, 200000.0]
    ...
    def test_rebin(self):
        ...
        """
        reflected_intensity = unumpy.uarray(np.arange(1, 9, 1), np.arange(1, 9, 1) * 0.1)
