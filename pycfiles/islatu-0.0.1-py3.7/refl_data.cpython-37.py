...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/islatu/refl_data.py
# Compiled at: 2020-04-22 08:36:40
# Size of source mod 2**32: 13400 bytes
"""
...
class Profile:
    ...
    def crop_and_bkg_sub(self, crop_function, bkg_sub_function, crop_kwargs=None, bkg_sub_kwargs=None, progress=True):
        """
    ...
    def q_uncertainty_from_pixel(self, number_of_pixels=None, detector_distance=None, energy=None, pixel_size=0.000172):
        """
...
class Scan:
    ...
    def __init__(self, file_path, parser, q_axis_name='qdcd', theta_axis_name='dcdtheta', energy_name='dcm1energy'):
        self.file_path = file_path
        self.metadata, self.data = parser(self.file_path)
        if q_axis_name is None:
            h = physical_constants['Planck constant in eV s'][0] * 0.001
            c = physical_constants['speed of light in vacuum'][0] * 10000000000.0
            q = self.metadata[energy_name] * 4 * np.pi * unp.sin(self.data[theta_axis_name]) / (h * c)
            self.q = unp.uarray(q, np.zeros(self.data[theta_axis_name].size))
        else:
            self.q = unp.uarray(self.data[q_axis_name], np.zeros(self.data[q_axis_name].size))
        self.data = self._check_files_exist()
        self.theta = unp.uarray(self.data[theta_axis_name], np.zeros(self.data[theta_axis_name].size))
        self.R = unp.uarray(np.zeros(self.data[q_axis_name].size), np.zeros(self.data[q_axis_name].size))
    ...
    def q_uncertainty_from_pixel(self, number_of_pixels=None, detector_distance=None, energy=None, pixel_size=0.000172):
        ...
        if detector_distance is None:
            detector_distance = self.metadata['diff1detdist'][0] * 0.001
        if energy is None:
            energy = self.metadata['dcm1energy'][0]
        offset = np.arctan(pixel_size * 1.96 * number_of_pixels * 0.5 / detector_distance)
        h = physical_constants['Planck constant in eV s'][0] * 0.001
        c = physical_constants['speed of light in vacuum'][0] * 10000000000.0
        q_uncertainty = energy * 4 * np.pi * unp.sin(offset) / (h * c)
        self.q = unp.uarray(unp.nominal_values(self.q), unp.std_devs(self.q) + q_uncertainty)
...
def _get_iterator(q, progress):
    ...
    if progress:
        try:
            ...
            print('For the progress bar, you need to have the tqdm package installed. No progress bar will be shown')
