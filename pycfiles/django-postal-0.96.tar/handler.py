...
# [GCC 8.4.0]
# Embedded file name: /home/mick/src/django-postal/src/postal/handler.py
        ...
        if hasattr(new_mcs, 'model'):
            if already_registered(new_mcs.model, new_mcs.is_anonymous):
                if not getattr(settings, 'PISTON_IGNORE_DUPE_MODELS', False):
                    warnings.warn('Handler already registered for model %s, you may experience inconsistent results.' % new_mcs.model.__name__)
            typemapper[new_mcs] = (
             new_mcs.model, new_mcs.is_anonymous)
        else:
            typemapper[new_mcs] = (
             None, new_mcs.is_anonymous)
