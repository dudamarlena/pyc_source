...
# [GCC 8.4.0]
# Embedded file name: /home/mick/src/django-postal/src/postal/resource.py
# Compiled at: 2015-03-03 03:42:01
"""
Code imported and optimized from django-piston 0.2.3rc1
"""
import decimal, inspect, re, sys
        ...
        def _model(data, fields=None):
            ...
            if handler or fields:
                ...
                if not fields or hasattr(handler, 'fields'):
                    ...
                    if not get_fields:
                        get_fields = set([ f.attname.replace('_id', '', 1) for f in data._meta.fields + data._meta.virtual_fields
                                         ])
                ...
                for f in data._meta.local_fields + data._meta.virtual_fields:
                    if f.serialize and not any([ p in met_fields for p in [f.attname, f.name] ]):
                        ...
                        elif f.attname[:-3] in get_fields:
...
class Resource(object):
    ...
    """
    callmap = {'GET': 'read', 'POST': 'create', 'PUT': 'update', 
       'DELETE': 'delete'}
    ...
    def authenticate(self, request, rm):
        ...
        for authenticator in self.authentication:
            if not authenticator.is_authenticated(request):
                ...
                return (
                 self.handler, self.handler.is_anonymous)
    ...
    def email_exception(self, reporter):
        ...
        message = EmailMessage(settings.EMAIL_SUBJECT_PREFIX + subject, html, settings.SERVER_EMAIL, [ admin[1] for admin in settings.ADMINS ])
    ...
    def error_handler(self, e, request, meth, em_format):
        ...
        if isinstance(e, Http404):
            return rc.NOT_FOUND
        if isinstance(e, HttpStatusCode):
            return e.response
