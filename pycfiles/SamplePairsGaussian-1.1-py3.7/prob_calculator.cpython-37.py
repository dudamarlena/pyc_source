...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/samplePairsGaussian/prob_calculator.py
# Compiled at: 2019-06-29 21:31:52
...
class ProbCalculator:
    ...
    def _compute_probs(self):
        ...
        if self._n < 2:
            self._reset()
            return
        self._dists_idxs_first_particle, self._dists_idxs_second_particle = np.triu_indices((self._n), k=1)
        dr = self._posns[self._dists_idxs_first_particle] - self._posns[self._dists_idxs_second_particle]
        self._dists_squared = np.sum((dr * dr), axis=1)
