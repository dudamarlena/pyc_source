...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/scikits/bvp_solver/Solution.py
# Compiled at: 2011-07-22 15:26:35
import numpy, tools, bvp_solverf, pickle

class Solution:
    """Stores the results of a :func:`solve` operation and provides a mechanism to evaluate valid results.
    """

    def __init__(self, mesh, solution, parameters=None, work=None, iwork=None, yerror=None, successIndicator=None, extended=False):
        """
        """
    ...
    def __call__(self, points, eval_derivative=False):
        ...
        bvp_solverf.bvp.bvp_eval_wrap(eval_derivative=eval_derivative, points=tools.farg(points), node_in=self._solution.shape[0], npar_in=npar, leftbc_in=1, npts_in=len(self._mesh), info_in=self._successIndicator, mxnsub_in=300, x_in=tools.farg(self._mesh), y_in=tools.farg(self._solution), parameters_in=tools.farg(self._parameters), work_in=tools.farg(self._work), iwork_in=tools.farg(self._iwork))
        if eval_derivative:
    ...
    def from_arg_list(bvp_object):
        ...
        """
        new = Solution(mesh=tools.fromf(bvp_object.x), solution=tools.fromf(bvp_object.y), parameters=tools.fromf(bvp_object.parameters), work=tools.fromf(bvp_object.work), iwork=tools.fromf(bvp_object.iwork), yerror=tools.fromf(bvp_object.yerror), successIndicator=tools.fromf(bvp_object.info))
        return new
