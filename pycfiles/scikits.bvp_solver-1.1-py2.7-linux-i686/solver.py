...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/scikits/bvp_solver/solver.py
# Compiled at: 2012-09-26 17:32:48
import bvp_solverf, numpy
from Solution import Solution
import tools

def solve(bvp_problem, solution_guess, initial_mesh=None, parameter_guess=None, max_subintervals=300, singular_term=None, tolerance=1e-06, method=4, trace=0, error_on_fail=True):
    ...
    if isinstance(solution_guess, Solution):
        if not (initial_mesh == None and parameter_guess == None):
        ...
        if not callable(solution_guess):
            ...
            if solution_guess.shape == (bvp_problem.num_ODE,) or solution_guess.shape == () and bvp_problem.num_ODE == 1:
                bvp_solverf.bvp.guess_1_wrap(nparam_in=bvp_problem.num_parameters, leftbc_in=bvp_problem.num_left_boundary_conditions, x_in=tools.farg(initial_mesh), y_in=tools.farg(solution_guess), parameters_in=tools.farg(parameter_guess), mxnsub_in=max_subintervals, node_in=bvp_problem.num_ODE)
                ...
                bvp_solverf.bvp.guess_2_wrap(nparam_in=bvp_problem.num_parameters, leftbc_in=bvp_problem.num_left_boundary_conditions, x_in=tools.farg(initial_mesh), y_in=tools.farg(solution_guess), parameters_in=tools.farg(parameter_guess), mxnsub_in=max_subintervals, node_in=bvp_problem.num_ODE)
                init_solution = Solution.from_arg_list(bvp_solverf.bvp)
        else:
            y_in = numpy.zeros((bvp_problem.num_ODE, 1))
            bvp_solverf.bvp.guess_1_wrap(nparam_in=bvp_problem.num_parameters, leftbc_in=bvp_problem.num_left_boundary_conditions, x_in=tools.farg(initial_mesh), y_in=y_in, parameters_in=tools.farg(parameter_guess), mxnsub_in=max_subintervals, node_in=bvp_problem.num_ODE)
            init_solution = Solution.from_arg_list(bvp_solverf.bvp)
...
def _guess_3_wrap(node_in, nparam_in, leftbc_in, x_in, fcn, parameters_in, mxnsub_in):
    y_in = 0.0
    sol = bvp_solverf.bvp.guess_1_wrap(node_in, nparam_in, leftbc_in, x_in, y_in, parameters_in, mxnsub_in)