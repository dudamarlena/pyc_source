...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/scikits/bvp_solver/ProblemDefinition.py
# Compiled at: 2011-07-22 15:26:35
import numpy, tools, pickle

class ProblemDefinition():
    """Defines a boundary value problem.
    """

    def __init__(self, num_ODE, num_parameters, num_left_boundary_conditions, boundary_points, function, boundary_conditions, function_derivative=None, boundary_conditions_derivative=None):
        """
        Parameters
        ----------
        num_ODE : int
        ...
        self.has_function_derivative = self._function_derivative is not None
        self.has_boundary_conditions_derivative = self._boundary_conditions_derivative is not None
    ...
    def test(self, test_solution):
        ...
        """
        if self._num_parameters > 0:
            tools.argShapeTest(test_solution.parameters, (self._num_parameters,), 'parameter array', 'Should be (num_parameters,)')
        tools.argShapeTest(test_solution.solution, (self._num_ODE, len(test_solution.mesh)), 'solution array', 'Should be (num_ODE,length(mesh))')
        if not self._boundary_points.shape == (2, ):
            raise ValueError('This boundary value problem definition must be given exactly two boundary points, but got: ' + self._boundary_points + ' as the boundary values')
        if self._num_parameters == 0:
            f = self._function(test_solution.mesh[0], test_solution.solution[:, 0])
            tools.argShapeTest(f, (self._num_ODE,), 'function callback return', 'Should be (num_ODE,)')
            bca, bcb = self._boundary_conditions(test_solution.solution[:, 0], test_solution.solution[:, -1])
            tools.argShapeTest(bca, (self._num_left_boundary_conditions,), 'Boundary conditions callback first return', 'Should be (num_left_boundary_conditions,)')
            tools.argShapeTest(bcb, (self._num_ODE + self._num_parameters - self._num_left_boundary_conditions,), 'Boundary conditions callback second return', 'Should be (num_ODE + num_parameters - num_left_boundary_conditions,)')
            if self.has_function_derivative:
                df = self._function_derivative(test_solution.mesh[0], test_solution.solution[:, 0])
                tools.argShapeTest(df, (self._num_ODE, self._num_ODE), 'function derivative callback first return', 'Should be (num_ODE, num_ODE)')
            if self.has_boundary_conditions_derivative:
                dbca, dbcb = self._boundary_conditions_derivative(test_solution.solution[:, 0], test_solution.solution[:, 0])
                tools.argShapeTest(dbca, (self._num_left_boundary_conditions, self._num_ODE), 'Boundary conditions derivative callback first return', 'Should be (num_left_boundary_conditions, num_ODE)')
                tools.argShapeTest(dbcb, (self._num_ODE + self._num_parameters - self._num_left_boundary_conditions, self._num_ODE), 'Boundary conditions derivative callback second return', 'Should be (num_ODE + num_parameters - num_left_boundary_conditions, num_ODE)')
        else:
            f = self._functionp(test_solution.mesh[0], test_solution.solution[:, 0], test_solution.parameters)
            tools.argShapeTest(f, (self._num_ODE,), 'function callback return', 'Should be (num_ODE,)')
            bca, bcb = self._boundary_conditionsp(test_solution.solution[:, 0], test_solution.solution[:, 0], test_solution.parameters)
            tools.argShapeTest(bca, (self._num_left_boundary_conditions,), 'Boundary conditions callback first return', 'Should be (num_left_boundary_conditions,)')
            tools.argShapeTest(bcb, (self._num_ODE + self._num_parameters - self._num_left_boundary_conditions,), 'Boundary conditions callback second return', 'Should be (num_ODE + num_parameters - num_left_boundary_conditions,)')
            if self.has_function_derivative:
                df, dfp = self._function_derivativep(test_solution.mesh[0], test_solution.solution[:, 0], test_solution.parameters)
                tools.argShapeTest(df, (self._num_ODE, self._num_ODE), 'function derivative callback first return', 'Should be (num_ODE, num_ODE)')
                tools.argShapeTest(dfp, (self._num_ODE, self._num_parameters), 'function derivative callback second return', 'Should be (num_ODE ,num_parameters)')
            if self.has_boundary_conditions_derivative:
                dbca, dbcb, dbcap, dbcbp = self._boundary_conditions_derivativep(test_solution.solution[:, 0], test_solution.solution[:, 0], test_solution.parameters)
                tools.argShapeTest(dbca, (self._num_left_boundary_conditions, self._num_ODE), 'boundary conditions derivative callback first return', 'Should be (num_left_boundary_conditions, num_ODE)')
                tools.argShapeTest(dbcb, (self._num_ODE + self._num_parameters - self._num_left_boundary_conditions, self._num_ODE), 'boundary conditions derivative callback second return', 'Should be (num_ODE + num_parameters - num_left_boundary_conditions, num_ODE)')
                tools.argShapeTest(dbcap, (self._num_left_boundary_conditions, self._num_parameters), 'boundary conditions derivative callback third return', 'Should be (num_left_boundary_conditions, num_parameters)')
                tools.argShapeTest(dbcbp, (self._num_ODE + self._num_parameters - self._num_left_boundary_conditions, self._num_parameters), 'boundary conditions derivative callback fourth return', 'Should be (num_ODE + num_parameters - num_left_boundary_conditions, num_ODE, num_parameters)')
        step = 1e-08
        places = 4
        middlePoint = numpy.round(test_solution.mesh.size * 0.61, 0)
        if self.has_function_derivative:
            if self._num_parameters > 0:
            ...
            if self.has_boundary_conditions_derivative:
                ...
                if self._num_parameters > 0:
                    bcA_derivative_calc, bcB_derivative_calc, bcA_param_derivative_calc, bcB_param_derivative_calc = self._boundary_conditions_derivativep(test_solution.solution[:, 0], test_solution.solution[:, endPoint], test_solution.parameters)
                    ...
                    bcA_derivative_calc, bcB_derivative_calc = self._boundary_conditions_derivative(test_solution.solution[:, 0], test_solution.solution[:, endPoint])
                    bcA_derivative_num = numpy.zeros((self._num_left_boundary_conditions, self._num_ODE))
