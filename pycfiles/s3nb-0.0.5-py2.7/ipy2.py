...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/s3nb/ipy2.py
...
class S3NotebookManager(NotebookManager):
    s3_bucket = Unicode('', config=True)
    s3_prefix = Unicode('', config=True)
    ...
    def _s3_key_dir_to_model(self, key):
        ...
        model = {'name': key.name.rsplit(self.s3_key_delimiter, 2)[(-2)], 
           'path': key.name, 
           'last_modified': None, 
           'created': None, 
           'type': 'directory'}
    ...
    def _s3_key_notebook_to_model(self, key, timeformat):
        ...
        model = {'name': key.name.rsplit(self.s3_key_delimiter, 1)[(-1)], 
           'path': key.name, 
           'last_modified': datetime.datetime.strptime(key.last_modified, timeformat).replace(tzinfo=tz.UTC), 
           'created': None, 
           'type': 'notebook'}
    ...
    def __init__(self, **kwargs):
        ...
        config = kwargs['parent'].config[self.__class__.__name__]
    ...
    def get_notebook(self, name, path='', content=True):
        ...
        if content:
            try:
                ...
                raise web.HTTPError(400, 'Unreadable Notebook: %s %s' % (os_path, e))
    ...
    def save_notebook(self, model, name, path=''):
        ...
        if 'content' not in model:
            raise web.HTTPError(400, 'No notebook JSON data provided')
        ...
        try:
            ...
            raise web.HTTPError(400, 'Unexpected Error Writing Notebook: %s %s %s' % (path, name, e))
    ...
    def update_notebook(self, model, name, path=''):
        ...
        if path != new_path or name != new_name:
            ...
            if self.bucket.get_key(dst_key):
                raise web.HTTPError(409, 'Notebook with name already exists: %s' % src_key)
    ...
    def get_checkpoint_name(self, checkpoint_id, name):
        ...
        checkpoint_name = ('{name}--{checkpoint_id}{ext}').format(name=basename, checkpoint_id=checkpoint_id, ext=self.filename_ext)
    ...
    def get_checkpoint_model(self, checkpoint_id, name, path=''):
        ...
        checkpoint_notebook_model = self._s3_key_notebook_to_model(key, timeformat=S3_TIMEFORMAT_GET_KEY)
        checkpoint_model = {'id': checkpoint_id, 
    ...
    def list_checkpoints(self, name, path=''):
        ...
        if key is None:
            ...
            return [
             self.get_checkpoint_model(checkpoint_id, name, path)]
            return