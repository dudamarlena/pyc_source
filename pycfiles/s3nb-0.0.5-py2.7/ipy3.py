...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/s3nb/ipy3.py
# Compiled at: 2015-05-09 06:58:50
import codecs
from collections import namedtuple
import datetime, tempfile, boto
...
class S3ContentsManager(ContentsManager):
    ...
    def _s3_key_dir_to_model(self, key):
        ...
        model = {'name': self._get_key_dir_name(key.name), 
    ...
    def __init__(self, **kwargs):
        ...
        config = self.config[self.__class__.__name__]
        ...
        self.log.debug('initialized base_uri: %s bucket: %s prefix: %s', self.s3_base_uri, self.s3_bucket, self.s3_prefix)
    ...
    def get(self, path, content=True, type=None, format=None):
        ...
        if type == 'notebook' or type is None and path.endswith('.ipynb'):
            ...
            if content:
                try:
                    ...
                    raise web.HTTPError(400, 'Unreadable Notebook: %s %s' % (path, e))
        ...
        if content:
            try:
                ...
                raise web.HTTPError(400, 'Unreadable file: %s %s' % (path, e))
    ...
    def _save_file(self, path, content, format):
        if format != 'text':
            raise web.HTTPError(400, 'Only text files are supported')
        try:
            bcontent = content.encode('utf8')
        except Exception as e:
            raise web.HTTPError(400, ('Encoding error saving {}: {}').format(content, e))
    ...
    def _save_notebook(self, path, nb):
        ...
        try:
            ...
            raise web.HTTPError(400, 'Unexpected Error Writing Notebook: %s %s' % (path, e))
    ...
    def rename(self, old_path, new_path):
        ...
        if self.bucket.get_key(dst_key):
            raise web.HTTPError(409, 'Notebook with name already exists: %s' % dst_key)
