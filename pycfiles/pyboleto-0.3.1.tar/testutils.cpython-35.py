...
# [GCC 8.4.0]
# Embedded file name: /Users/ecereto/src/pyboleto/tests/testutils.py
# Compiled at: 2016-04-11 04:00:06
# Size of source mod 2**32: 6248 bytes
from __future__ import with_statement
import difflib, fnmatch, os, re, sys, subprocess, tempfile, unittest
...
def diff_files(orig, new, verbose=False):
    with open(orig) as (f_orig):
        with open(new) as (f_new):
            return _diff(f_orig.readlines(), f_new.readlines(), short=os.path.basename(orig), verbose=verbose)
...
def diff_pdf_htmls(original_filename, filename):
    for fname in [original_filename, filename]:
        with open(fname) as (f):
            data = f.read()
            data = re.sub('name="date" content="(.*)"', 'name="date" content="%%DATE%%"', data)
            data = re.sub('<pdf2xml[^>]+>', '<pdf2xml>', data)
...
class BoletoTestCase(unittest.TestCase):
    ...
    def _get_expected(self, bank, generated):
        fname = os.path.join(os.path.dirname(pyboleto.__file__), '..', 'tests', 'xml', bank + '-expected.xml')
