...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-ppc/egg/quizdrill/SaDrill.py
# Compiled at: 2007-05-13 18:19:14
import os.path
...
class MissingTagsError(SaDrillError):
    ...
    def __init__(self, file, tags):
        self.file = file
        self.tags = tags
        self.str = ngettext('Error: Missing mandatory tag "%(t)s" in %(f)s.', 'Error: Missing mandatory tags "%(t)s in %(f)s".', len(tags)) % {'t': tags, 'f': file}
...
class WordPairError(SaDrillError):
    ...
    def __init__(self, file, line, type=WRONG_NUMBER_OF_WORDPAIRS):
        ...
        if type == self.WRONG_NUMBER_OF_WORDPAIRS:
            self.str = _('Error: Wrong number of "=" in line %(l)s of file %(f)s.') % {'l': line, 'f': file}
        elif type == self.NON_CLOSING_TAG:
            self.str = _('Error: Tag in line %(l)s of file %(f)s does not declare its end with a ":".') % {'l': line, 'f': file}
...
class SaDrill:
    ...
    def __init__(self, head_tag_dict={}, build_tag_dict={}, mandatory_head_tags=[], mandatory_build_tags=[], mandatory_has_questions=False):
        self.head_tag_dict = {' ': self.on_unknown_head_tag, 'language': self.on_default_head_tag, 'question': self.on_default_head_tag, 'type': self.on_default_head_tag, 'media': self.on_default_head_tag, 'generator': self.on_default_head_tag}
        self.build_tag_dict = {' ': self.on_unknown_build_tag, 'build_to': self.on_default_build_tag, 'builder': self.on_default_build_tag, 'filter': self.on_default_build_tag}
    ...
    def parse(self, drill_file):
        ...
        def _parse_taged_line(type, tag_dict, mandatory_tags, used_mandatory_tags):
            try:
                colon = line.index(':')
            except ValueError:
                raise WordPairError(drill_file, i + 1, WordPairError.NON_CLOSING_TAG)
            else:
                tag = line[1:colon]
                word_pair = [ w.strip() for w in line[colon + 1:].split('=') ]
                if tag in tag_dict:
                    if tag in mandatory_tags and tag not in used_mandatory_tags:
        ...
        for (i, line) in enumerate(f.readlines()):
            ...
            if len(line) > 0:
                ...
                if type == '#':
                    ...
                    _parse_taged_line(type, head_tag_dict, mandatory_head_tags, used_mandatory_head_tags)
                elif type == '[':
                    line = line[1:-1]
                    ...
                    _parse_taged_line(type, build_tag_dict, mandatory_build_tags, used_mandatory_build_tags)
                else:
                    type = ''
        ...
        missing_tags = self.mandatory_head_tags - used_mandatory_head_tags | self.mandatory_build_tags - used_mandatory_build_tags
        if missing_tags != set([]):
