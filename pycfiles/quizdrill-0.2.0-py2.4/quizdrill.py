...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-ppc/egg/quizdrill/quizdrill.py
# Compiled at: 2007-05-12 21:56:35
from SaDrill import SaDrill, SaDrillError
import pygtk
pygtk.require('2.0')
import gobject, gtk, gtk.glade, random, os, os.path
from pkg_resources import resource_filename
import cPickle as pickle, locale, gettext
_ = gettext.gettext
APP = 'quizdrill'
...
class Gui:
    ...
    SHOW_TABS = {'vocabulary': [True, True, True], 'questionnaire': [True, True, True], 'flashcard': [False, False, True], 'all': [True, True, True]}
    break_length = 900000
    snooze_length = 300000
    ...
    def __init__(self):
        ...
        self.main_notbook_tabs = {'multi': [gw('multi_tab_label'), gw('multi_tab_vbox')], 'simple': [gw('simple_tab_label'), gw('simple_tab_vbox')], 'flash': [gw('flash_tab_label'), gw('flash_tab_vbox')]}
        ...
        self.question_topic_labels = [gw('multi_question_topic_label'), gw('simple_question_topic_label'), gw('flash_question_topic_label')]
        self.question_labels = [
         gw('multi_question_label'), gw('simple_question_label'), gw('flash_question_label')]
        self.multi_question_buttons = [
         gw('button11'), gw('button12'), gw('button13'), gw('button14'), gw('button15'), gw('button16'), gw('button17')]
        self.simple_question_button = gw('simple_question_button')
        self.flash_notebook = gw('flash_notebook')
        self.flash_answer_buttons = [gw('flash_answer_button_no'), gw('flash_answer_button_yes')]
        self.flash_answer_label = gw('flash_answer_label')
        self.progressbar1 = gw('progressbar1')
        sb = self.statusbar1 = gw('statusbar1')
        self.statusbar_contextid = {'last_answer': sb.get_context_id('The answer to the last asked question.')}
        for quiz_file in [os.path.expanduser('~/.quizdrill/' + default_quiz), resource_filename(__name__, '../quizzes/'),
         resource_filename(__name__, '../share/quizdrill/' + default_quiz), resource_filename(__name__, '../quizdrill/' + default_quiz), '/usr/share/quizdrill/', '/usr/locale/share/quizdrill/']:
    ...
    def update_gui(self):
        ...
        self.progressbar1.set_fraction(float(self.quiz.answered) / self.quiz.session_length)
        for (button, text) in zip(self.multi_question_buttons, self.quiz.multi_choices):
    ...
    def redisplay_correctly_answered(self, last_question):
        ...
        """
        text = _("'%(answer)s' to '%(question)s' was correct.") % {'answer': last_question[self.quiz.answer_to], 'question': last_question[self.quiz.ask_from]}
    ...
    def switch_quiz(self, quiz_filer=None):
        ...
        self.update_gui()
        if quiz_filer.type not in self.SHOW_TABS:
            print _('Warning: unknown quiz type "%s".') % quiz_filer.type
            type = 'all'
        else:
            type = self.quiz_filer.type
        for (tab, visi) in zip(self.main_notbook_tabs.itervalues(), self.SHOW_TABS[type]):
            for widget in tab:
                if visi:
                    widget.show()
                else:
        ...
        for label in self.question_topic_labels:
            label.set_markup('<b>%s</b>' % quiz_filer.question_topic[self.quiz.ask_from])
        ...
        for (i, title) in enumerate(quiz_filer.data_name):
            tvcolumn = gtk.TreeViewColumn(title, gtk.CellRendererText(), text=i)
    ...
    def start_relax_time(self, break_length=None, minimize=True):
        ...
        self.timer_id = gobject.timeout_add(break_length, self.on_end_relax_time)
        return
    ...
    def on_main_window_window_state_event(self, widget, event):
        ...
        if 'iconified' in event.new_window_state.value_nicks and not self.timer_id:
            self.start_relax_time(self.snooze_length, False)
        elif 'iconified' not in event.new_window_state.value_nicks and self.timer_id:
            gobject.source_remove(self.timer_id)
            self.timer_id = 0
    ...
    def on_open1_activate(self, widget):
        ...
        chooser = gtk.FileChooserDialog('Open Quiz', None, gtk.FILE_CHOOSER_ACTION_OPEN, (gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL, gtk.STOCK_OPEN, gtk.RESPONSE_OK))
        chooser.set_current_folder(os.path.abspath(os.path.dirname(self.quiz_filer.quiz_file_path)))
        response = chooser.run()
        chooser.hide()
        if response == gtk.RESPONSE_OK:
            try:
                self.quiz_filer_list = [
                 Quiz_Filer(chooser.get_filename())]
            except (IOError, SaDrillError), e:
                message = gtk.MessageDialog(type=gtk.MESSAGE_ERROR, buttons=gtk.BUTTONS_OK, message_format=e.str)
                message.run()
                message.destroy()
            else:
                self.switch_quiz(self.quiz_filer_list[0])
    ...
    def on_multi_question_answer_button_clicked(self, widget, data=None):
        ...
        if self.quiz.check(answer):
            ...
            text = _("To '%(quest)s' '%(ans)s' would be the correct answer.") % {'ans': answer, 'quest': self.quiz.get_question_to_answer(answer)}
            self.statusbar1.pop(self.statusbar_contextid['last_answer'])
            self.statusbar1.push(self.statusbar_contextid['last_answer'], text)
    ...
    def on_flash_question_button_clicked(self, widget, date=None):
        self.flash_answer_label.set_text(self.quiz.question[self.quiz.answer_to])
    ...
    def on_flash_answer_button_clicked(self, widget, data=None):
        if isinstance(self.quiz, Weighted_Quiz):
            self.quiz.set_answer_quality(self.flash_answer_buttons.index(widget))
        self.quiz.next()
...
class Quiz_Filer(SaDrill):
    ...
    def __init__(self, quiz_file_path=None):
        tag_dict = {'language': self.on_tag_language, 'question': self.on_tag_question, 'type': self.on_tag_type, 'media': self.on_tag_media, 'generator': self.on_tag_generator}
        SaDrill.__init__(self, head_tag_dict=tag_dict, mandatory_has_questions=True)
        ...
        self.treestore = gtk.TreeStore(gobject.TYPE_STRING, gobject.TYPE_STRING, gobject.TYPE_BOOLEAN)
        if quiz_file_path != None:
            ...
            self.quiz_file_path = resource_filename(__name__, '../quizzes/no-file.drill')
            quizlist = [['', '']]
            score = {'': 0}
    ...
    def _get_score_file(self, quiz_file, type):
        return self.SCORE_PATH + os.path.basename(quiz_file) + '_' + type + '.score'
    ...
    def on_tag_language(self, as_text, word_pair, tag='language', type='!'):
        ...
        self.all_subquizzes = [word_pair[0] + ' → ' + word_pair[1], word_pair[1] + ' → ' + word_pair[0]]

    def on_tag_question(self, as_text, word_pair=[
 '$what'], tag='question', type='!'):
        common = {'$what': _('What is this?'), '$voc_test': _('Please translate:')}
        if word_pair[0] in common:
            word_pair = [
             common[word_pair[0]], common[word_pair[0]]]
...
class Quiz:
    ...
    def __init__(self, quiz_pool, ask_from=0, exam_length=15):
        self.listoners = {'break_time': [], 'question_changed': [], 'direction_changed': []}
        self.quiz_pool = []
        self.answered = 0
...
class Weighted_Quiz(Quiz):
    ...
    def __init__(self, quiz_pool, question_score={}, ask_from=0, exam_length=15):
    ...
    def check(self, solution):
        ...
        if Quiz.check(self, solution):
            self._update_score(self.question[self.ask_from], self.tries == 0)
            return True
        else:
            return False
    ...
    def _update_score(self, word, correct_answered):
        ...
        self.question_score[word] = (self.question_score[word] * 3 + correct_answered) / 4
        self.score_sum += self.question_score[word]
    ...
    def _gen_score_sum(self, quizzes=None):
        ...
        for question in quizzes:
            if question[self.ask_from] in self.question_score:
                ...
                least_score = 0.0
                self.question_score[question[self.ask_from]] = 0.0
...
class Queued_Quiz(Weighted_Quiz):
    ...
    def __init__(self, question_pool, question_score={}, ask_from=0, exam_length=15, bad_score=0.4, min_num_bad_scores=3, min_question_num=20, batch_length=5):
        self.new_quiz_pool = []
    ...
    def _insure_min_quiz_num(self):
        ...
        num_missing = min(len(self.new_quiz_pool), self.min_question_num - len(self.quiz_pool))
        if num_missing > 0:
