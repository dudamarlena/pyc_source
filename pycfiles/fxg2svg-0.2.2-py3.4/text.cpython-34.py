...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fxg2svg/processors/text.py
# Compiled at: 2016-07-26 13:04:39
# Size of source mod 2**32: 4816 bytes
...
def process_font(element, **attrs):
    ...
    if 'fontStyle' in attrs:
        ...
        if get_attribute(element, 'fontStyle'):
        ...
        if 'textAlign' in attrs:
            params['text-anchor'] = {'left': 'start',  'center': 'middle', 
             'right': 'end'}[attrs['textAlign']]
...
class RichTextTagProcessor(DefaultTagProcessor):
    ...
    def on_start(self, target):
        ...
        target.element = etree.SubElement(target.element, 'text', **params)
...
class PTagProcessor(DefaultTagProcessor):
    ...
    def on_start(self, target):
        ...
        target.element = etree.SubElement(target.element, 'tspan', **params)
