...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fxg2svg/processors/fill.py
# Compiled at: 2016-07-06 07:43:17
# Size of source mod 2**32: 3479 bytes
from lxml import etree
import re, uuid, math, logging
...
def get_coordinates(element):
    if element.tag == 'rect':
        ...
        return (
         x, y)
    if element.tag == 'path':
        x, y, *_ = re.findall('\\d+\\.\\d+', element.attrib['d'])
        return (
         float(x), float(y))
    return (0, 0)
...
class RadialGradientTagProcessor(DefaultTagProcessor):
    ...
    def on_start(self, target):
        ...
        rx = scaleX / 2
        ry = scaleY / 2
        target.element = etree.SubElement(target.element, 'radialGradient', cx=str(cx), cy=str(cy), rx=str(rx), ry=str(ry), id=str(uuid.uuid4()))
...
class LinearGradientTagProcessor(DefaultTagProcessor):
    ...
    def on_start(self, target):
        ...
        target.element = etree.SubElement(target.element, 'linearGradient', x1=str(x1), y1=str(y1), x2=str(x2), y2=str(y2), id=str(uuid.uuid4()))
...
class GradientEntryTagProcessor(DefaultTagProcessor):
    ...
    def on_start(self, target):
        target.element = etree.SubElement(target.element, 'stop', offset=target.attrs.get('ratio', '1'), style='stop-color:%s' % target.attrs.get('color', '#000000'))