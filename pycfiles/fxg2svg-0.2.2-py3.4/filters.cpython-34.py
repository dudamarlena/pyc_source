...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fxg2svg/processors/filters.py
# Compiled at: 2016-07-12 14:57:40
...
class PathTagProcessor(DefaultTagProcessor):
    ...
    def on_start(self, target):
        ...
        params = {}
        defs = target.defs
        filter_params = {'id': str(uuid.uuid4()), 
         'width': '250%', 
         'height': '250%', 
         'x': '-50%', 
         'y': '-50%'}
        filter = etree.SubElement(defs, 'filter', **filter_params)
        offset_params = {}
        blur_params = {}
        ct_params = {}
        if 'blurX' in attrs:
        ...
        if len(offset_params.keys()) > 0:
            ...
            etree.SubElement(filter, 'feOffset', **offset_params)
        if len(blur_params.keys()) > 0:
            blur_params['in'] = current_state
            blur_params['result'] = 'blurResult'
            current_state = blur_params['result']
            etree.SubElement(filter, 'feGaussianBlur', **blur_params)
        if 'alpha' in attrs:
            ct = etree.SubElement(filter, 'feComponentTransfer', **{'in': current_state,  'result': 'ctResult'})
            etree.SubElement(ct, 'feFuncA', **{'type': 'linear',  'slope': attrs['alpha']})
            current_state = 'ctResult'
        merge = etree.SubElement(filter, 'feMerge')
        etree.SubElement(merge, 'feMergeNode', **{'in': current_state})
