...
# [GCC 8.4.0]
# Embedded file name: /home/anthony/myui/myui/__init__.py
# Compiled at: 2016-01-11 17:47:14
...
def gen_settings():
    ...
    return dict(login_url=tornado.options.options.login_url, app_title=tornado.options.options.app_title, template_path=tornado.options.options.template_path, static_path=tornado.options.options.static_path, cookie_secret=tornado.options.options.cookie_secret, debug=tornado.options.options.debug, plugin_opts=tornado.options.options.plugin_opts)
...
def main():
    settings = parse_options()
    try:
        settings['ui_modules'] = {'uimodules': import_module(('{0}.uimodules').format(tornado.options.options.plugins))}
    except ImportError:
        pass
