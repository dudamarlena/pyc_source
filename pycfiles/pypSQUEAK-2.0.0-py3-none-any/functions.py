...
# Embedded file name: /home/dswistowski/pso/pypso/src/pypso/functions.py
# Compiled at: 2007-06-30 10:49:57
__doc__ = 'Moduł zawierający optymalizowane funkcje'
import random
...
class ELengthMismatch(Exception):
    u"""Wyjątek podnoszony podczas niezgodności długości wektorów"""
...
class Function(Parameters):
    ...
    def __init__(self, f, min=-5.2, max=5.2):
        ...
        boundary = Boundary(self['n'], min, max)
        self.f = f
...
class Schafferf6(Schaffer):
    ...
    def __call__(self, x):
        xx = x[0] ** 2 + x[1] ** 2
        return 0.5 + (sin(xx ** 0.5) - 0.5) / (1 + 0.001 * xx)
...
if __name__ == '__main__':
    ...
    print elmul(Vector([1.0, 2]), Vector([2, 0.5]))
