...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/policytool/cli.py
# Compiled at: 2019-05-23 11:08:11
from __future__ import print_function
import click
from click import ClickException
from requests_kerberos import HTTPKerberosAuth
import atlas, hive, policycache, tagsync, ranger, rangersync
...
@cli.command('audit_tags', help='A dry run providing audit information about tags. It includes differences between source files and Atlas.')
@click.option('-s', '--srcdir', help='The schema for the generated table', default='src/main/tags')
@click.option('-e', '--environment', help='Destination environment', required=True)
@click.option('-c', '--config', help='Config file', type=click.Path(exists=True))
@click.option('--tabletagfile', help='The source file for table tags file', default='table_tags.csv')
...
@cli.command('policy_cache_sync', help='Reads a policy cache file copied from hive sercer andset all tags in it in Atlas. Useful when you tagstorein Ranger is out of sync with Atlas.')
@click.option('-e', '--environment', help='Destination environment. Used to get Atlas and ranger config.', required=True)
@click.option('-c', '--config', help='Config file', type=click.Path(exists=True))
...
@click.option('--hdfs/--no-hdfs', help='Set tags on hive tables corresponding hdfs directory. No effect if tagfiles are created', default=False)
def policy_cache_sync(environment, config, policycachefile, tabletagfile, columntagfile, hdfs):
    policycache.extract_policy_cache(JSONPropertiesFile(config).get(environment), policycache, tabletagfile, columntagfile, hdfs)
