...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/policytool/hive.py
...
class Client:
    ...
    def _connection(self):
        if not self._conn:
            self._conn = hive.Connection(host=self.host, port=self.port, auth=self.auth, kerberos_service_name=self.service_name)
        return self._conn
    ...
    def get_location(self, database, table=None):
        ...
        if table is not None and table != '*':
            ...
            for key, value, _ in cursor.fetchall():
                if key is not None and key.strip() == 'Location:':
                    return value.strip()
