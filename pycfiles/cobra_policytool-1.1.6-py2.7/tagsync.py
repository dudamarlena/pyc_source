...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/policytool/tagsync.py
    ...
    def get_tables_for_schema_from_atlas(self, schemas):
        ...
        for schema in schemas:
            for table in self.atlas_client.get_tables(schema):
                result[strip_qualified_name(table['attributes']['qualifiedName'])] = {'guid': table['guid'], 
    ...
    def get_columns_for_tables_from_atlas(self, src_tables):
        ...
        for schema_table in src_tables:
            ...
            for table in self.atlas_client.get_columns(schema, table):
                result[strip_qualified_name(table['attributes']['qualifiedName'])] = {'guid': table['guid'], 'tags': set(table['classificationNames'])}
    ...
    def sync_table_storage_tags(self, src_table_tags, clear_not_listed=False):
        ...
        while True:
            try:
                ...
                for s in src_table_tags:
                    self.worklog.update(self._sync_tags_for_one_tables_storage(s['schema'], s['table'], _tags_as_set(s)))
