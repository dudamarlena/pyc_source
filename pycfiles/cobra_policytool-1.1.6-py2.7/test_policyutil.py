...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/test_policyutil.py
...
class TestResourceType(unittest.TestCase):
    ...
    def test_get_resource_type_when_not_providing_policytype_0_must_fail(self):
        ...
        self.assertEqual('PolicyType must be 0 to support option expandHiveResourceToHdfs. Policy: foo', e.exception.message)


class TestExtendTagPolicyWithHdfs(unittest.TestCase):

    def test_read_only_access(self):
        policy_input = {'policyType': 0, 
           'resources': {'tag': {}}, 'policyItems': [
                         {'accesses': [
                                       {'type': 'hive:select', 
                                          'isAllowed': True},
                                       {'type': 'hive:read', 
           ...
           'resources': {'tag': {}}, 'denyPolicyItems': [
                             {'accesses': [
                                ...
                                'users': [
                                        'myuser'], 
                                'delegateAdmin': False}]}
        policy_expected = {'denyPolicyItems': [
                             {'accesses': [
                                           {'isAllowed': True, 
                                              'type': 'hive:insert'},
                                           {'isAllowed': True, 
