...
# [GCC 8.4.0]
# Embedded file name: /home/javed/Work/Dinette/dinette/views.py
# Compiled at: 2013-07-02 04:58:02
...
def index_page(request):
    ...
    now = datetime.now()
    users_online = DinetteUserProfile.objects.filter(last_activity__gte=now - timedelta(seconds=900)).count() + 1
    last_registered_user = User.objects.order_by('-date_joined')[0]
    payload = {'users_online': users_online, 'forums_list': forums, 'totaltopics': totaltopics, 'totalposts': totalposts, 
...
def moderate_topic(request, topic_id, action):
    ...
    if request.user not in topic.category.moderated_by.all():
        raise Http404
