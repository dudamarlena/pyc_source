...
# [GCC 8.4.0]
# Embedded file name: /home/javed/Work/Dinette/dinette/libs/postmarkup.py
...
_re_url = re.compile('((https?):((//)|(\\\\\\\\))+[\\w\\d:#@%/;$()~_?\\+-=\\\\\\.&]*)', re.MULTILINE | re.UNICODE)
...
_re_excerpt = re.compile('\\[".*?\\]+?.*?\\[/".*?\\]+?', re.DOTALL | re.UNICODE)
_re_remove_markup = re.compile('\\[.*?\\]', re.DOTALL | re.UNICODE)
...
def create(include=None, exclude=None, use_pygments=True, **kwargs):
    ...
    add_tag(SearchTag, 'wiki', 'http://en.wikipedia.org/wiki/Special:Search?search=%s', 'wikipedia.com', **kwargs)
    add_tag(SearchTag, 'google', 'http://www.google.com/search?hl=en&q=%s&btnG=Google+Search', 'google.com', **kwargs)
    add_tag(SearchTag, 'dictionary', 'http://dictionary.reference.com/browse/%s', 'dictionary.com', **kwargs)
    add_tag(SearchTag, 'dict', 'http://dictionary.reference.com/browse/%s', 'dictionary.com', **kwargs)
    add_tag(ImgTag, 'img')
...
class LinkTag(TagBase):
    ...
    _re_domain = re.compile('//([a-z0-9-\\.]*)', re.UNICODE)
...
class QuoteTag(TagBase):
    ...
    def render_open(self, parser, node_index):
        if self.params:
            return "<div class='quotebox'><blockquote><em>%s</em><br/>" % PostMarkup.standard_replace(self.params)
        else:
            return "<div class='quotebox'><blockquote>"
...
class PostMarkup(object):
    ...
    _re_end_eq = re.compile('\\]|\\=', re.UNICODE)
    _re_quote_end = re.compile('"|\\]', re.UNICODE)
    ...
    def tokenize(cls, post):
        ...
        post_find = post.find
        while True:
            brace_pos = post_find('[', pos)
            if brace_pos == -1:
                if pos < len(post):
                    yield (
                     TOKEN_TEXT, post[pos:], pos, len(post))
                return
            if brace_pos - pos > 0:
                yield (
                 TOKEN_TEXT, post[pos:brace_pos], pos, brace_pos)
            pos = brace_pos
            end_pos = pos + 1
            ...
            if open_tag_pos != -1 and open_tag_pos < end_pos:
                yield (
                 TOKEN_TEXT, post[pos:open_tag_pos], pos, open_tag_pos)
                end_pos = open_tag_pos
                pos = end_pos
                continue
            if post[end_pos] == ']':
                yield (
                 TOKEN_TAG, post[pos:end_pos + 1], pos, end_pos + 1)
                pos = end_pos + 1
                continue
            if post[end_pos] == '=':
                try:
                    ...
                    if post[end_pos] != '"':
                        ...
                        if post[end_pos] == '"':
                            ...
                            if end_pos == -1:
                                return
                            yield (
                             TOKEN_PTAG, post[pos:end_pos + 1], pos, end_pos + 1)
                        else:
                            yield (
                             TOKEN_TAG, post[pos:end_pos + 1], pos, end_pos)
                    pos = end_pos + 1
                except IndexError:
                    return
    ...
    def insert_paragraphs(self, post_markup):
        ...
        for tag_type, tag_token, start_pos, end_pos in self.tokenize(post_markup):
            if tag_type == TOKEN_TEXT:
                if enclosed_count:
                    ...
                    txt = _re_break_groups.sub('[p]', txt)
                    parts.append(txt)
                continue
            elif tag_type == TOKEN_TAG:
                tag_token = tag_token[1:-1].lstrip()
                if ' ' in tag_token:
                    tag_name = tag_token.split(' ', 1)[0]
                elif '=' in tag_token:
                    tag_name = tag_token.split('=', 1)[0]
                else:
                    tag_name = tag_token
        ...
        new_markup = ('').join(parts)
        return new_markup

    _re_blank_tags = re.compile('\\<(\\w+?)\\>\\s*\\</\\1\\>')
    ...
    def render_to_html(self, post_markup, encoding='ascii', exclude_tags=None, auto_urls=True, paragraphs=False, clean=True, tag_data=None):
        ...
        for tag_type, tag_token, start_pos, end_pos in self.tokenize(post_markup):
            ...
            if not end_tag:
                ...
                if tag.auto_close:
                    ...
                    close_tag(tag)
            elif break_stack and break_stack[(-1)].name == tag_name:
                break_stack.pop()
...
def render_bbcode(bbcode, encoding='ascii', exclude_tags=None, auto_urls=True, paragraphs=False, clean=True, tag_data=None):
    ...
    """
    return _postmarkup(bbcode, encoding, exclude_tags=exclude_tags, auto_urls=auto_urls, paragraphs=paragraphs, clean=clean, tag_data=tag_data)
...
def _tests():
    ...
    tests.append('[img]http://upload.wikimedia.org/wikipedia/commons/6/61/Triops_longicaudatus.jpg[/img]')
...
def _run_unittests():
    ...
    class TestPostmarkup(unittest.TestCase):
        ...
        def testlinks(self):
            ...
            tests = [
             ('[link=http://www.willmcgugan.com]blog1[/link]', '<a href="http://www.willmcgugan.com">blog1</a>'),
...
def _ff_test():
    ...
    text = 'sdl;fk;sdlfks;dflksd;flksdfsdfwerwerwgwegwegwegwegwegegwweggewwegwegwegwettttttttttttttttttttttttttttttttttgggggggggg;slbdfkwelrkwelrkjal;sdfksdl;fksdf;lb'
    REPEAT = 100000
