...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.6/site-packages/wintx/drivers/MySQL/Driver.py
# Compiled at: 2016-03-23 14:50:18
"""MySQL driver for Wintx"""
import hashlib, mysql.connector, wintx
...
class Driver(object):
    ...
    def __init__(self, host='localhost', user='', password='', database='', timeout='3', port=3306, use_ssl=False, ssl_ca=None, ssl_cert=None, ssl_key=None, compress=False):
    ...
    """
        return {'time': wintx.getTime(record[0]), 'latitude': record[1], 'longitude': record[2], 
    ...
    def getLocationCorners(self):
        ...
        results = None
        cursor = self.connection.cursor()
        cursor.execute('SELECT MAX(X(`lat_lon`)) AS `MAX_LAT`, MIN(X(`lat_lon`)) AS `MIN_LAT`, MAX(Y(`lat_lon`)) AS `MAX_LON`, MIN(Y(`lat_lon`)) AS `MIN_LON` FROM `Location`')
        results = cursor.fetchall()
        cursor.close()
        self.connection.rollback()
        lat_max = results[0][0]
    ...
    """
        ...
        try:
            for record in records:
                ...
                try:
                    ...
                    if err.errno == errorcode.ER_DUP_KEY or err.errno == errorcode.ER_DUP_ENTRY or err.errno == errorcode.ER_DUP_KEYNAME or err.errno == errorcode.ER_DUP_ARGUMENT or err.errno == errorcode.ER_DUP_UNIQUE:
        ...
        cursor.close()
        return {'insertedRecords': inserted_records, 'duplicateRecords': duplicate_records, 
           'errors': []}