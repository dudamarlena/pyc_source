...
# [GCC 8.4.0]
# Embedded file name: /media/jks/Root/Dev/python/jkspy/jkspy/modules/words.py
# Compiled at: 2016-02-03 05:32:15
...
class Generator:
    ...
    def randSyllable(self):
        c1_dice = self.random.random_sample() < 0.91
        s1_dice = self.random.random_sample() < 0.05
        v1_dice = self.random.random_sample() < 0.85
        c2_add_dice = self.random.random_sample() < 0.28
        c2_dice = self.random.random_sample() < 0.91
        s2_dice = self.random.random_sample() < 0.03
 ...
 2, 2, 1, 2, 3, 2, 1, 2, 2]):
    ...
    def randDictionary(self, word_list=[
 'apple', 'banana', 'cake', 'dog', 'elephant', 'fruit', 'guava', 'human', 'island', 'joke', 'king', 'love', 'mother', 'nature', 'ocean', 'pie', 'queen', 'random', 'start', 'tree', 'up', 'vine', 'wisdom', 'yellow', 'zoo']):
