...
# [GCC 8.4.0]
# Embedded file name: /Users/stephenmoore/Projects/utility/photons-interactor/photons_interactor/database/models/scene_spec.py
...
class sized_list_spec(sb.Spec):
    ...
    def normalise_filled(self, meta, val):
        ...
        if len(val) != self.length:
            raise BadSpecValue('Expected certain number of parts',
              want=(self.length), got=(len(val)), meta=meta)
        return val
...
def make_spec(storing=True):
    ...
    class Fields(dictobj.Spec):
        ...
        matcher = dictobj.Field((json_string_spec(sb.dictionary_spec(), storing)),
          wrapper=(sb.required))
    ...
    if not storing:
        ...
        class Fields(Fields):
            ...
            def transform_options(self):
                ...
                dct = {'power':power, 
                 'color':self.color,  'duration':self.duration}
                return {k:v for k, v in dct.items() if v is not None if v is not None}

            def colors_from_hsbks(self, hsbks, overrides):
                return [{'hue':overrides.get('hue', h),  'saturation':overrides.get('saturation', s),  'brightness':overrides.get('brightness', b),  'kelvin':overrides.get('kelvin', k)} for h, s, b, k in hsbks]
            ...
            def zone_msgs(self, overrides):
                ...
                while i < len(colors) - 1:
                    ...
                    if colors[i] != color:
                        yield (MultiZoneMessages.SetColorZones)(start_index=start, 
                         end_index=i - 1, **color, **{'duration':duration, 
                         'res_required':False})
                        color = colors[i]
                        start = i
                ...
                color = colors[i]
                yield (MultiZoneMessages.SetColorZones)(start_index=start, 
                 end_index=i, **color, **{'duration':duration,  'res_required':False})
            ...
            def chain_msgs(self, overrides):
                ...
                for i, lst in enumerate(self.chain):
                    ...
                    yield TileMessages.Set64(tile_index=i,
                      length=1,
                      x=0,
                      y=0,
                      width=8,
