...
# [GCC 8.4.0]
# Embedded file name: /home/bruce/GoTonight/restful_poc/Flask-RESTful-DRY/build/lib/flask_dry/model/validation.py
# Compiled at: 2015-04-13 08:57:32
# Size of source mod 2**32: 8171 bytes
"""Column-level validation functions.
...
"""
import re
__all__ = ('validate_min', 'validate_max', 'validate_min_length', 'validate_max_length',
           'strip_value', 'rstrip_value', 'validate_phone_strict', 'validate_phone',
           'validate_email', 'no_html_chars', 'must_not_match')

def validate_min(min):
    """Ensures that the value is at least min.
...
def validate_max(max):
    """Ensures that the value is at least max.
...
def strip_value(key, value):
    """Strips whitespace off of the beginning and end of the user value.
    """
    if value is None:
        return (None, None)
    return (
...
def rstrip_value(key, value):
    """Strips whitespace off of the end of the user value.
    """
    if value is None:
        return (None, None)
    return (
...
def validate_min_length(min_length):
    """Ensures that the value is at least min_length.
    ...
    validate_min_length.info = ('min_length', min_length,
     'Requires minimum of {{min_length}} character{}'.format('' if min_length == 1 else 's'))
    return validate_min_length


def validate_max_length(max_length):
    """Ensures that the value is no longer than max_length.
    ...
    validate_max_length.info = ('max_length', max_length,
     'Exceeds maximum length of {{max_length}} character{}'.format('' if max_length == 1 else 's'))
    return validate_max_length


strict_phone_regex = re.compile('(\\([0-9]{3}\\)|[0-9]{3})[ -]?([0-9]{3})[ -]?([0-9]{4})$')

def validate_phone_strict(key, value, regex=strict_phone_regex):
    """Validates a phone number, not allowing any information after the
    number.
...
phone_regex = re.compile(strict_phone_regex.pattern[:-1] + '( +[a-zA-Z0-9.() -]+)? *$')
...
no_html_regex = re.compile('[<>]')

def no_html_chars(key, value):
    """Ensures that the value has no tag delimiters.
...
def must_not_match(regex, message):
    """Ensures that the value does not match the regex.
