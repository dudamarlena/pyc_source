...
# [GCC 8.4.0]
# Embedded file name: /home/bruce/GoTonight/restful_poc/Flask-RESTful-DRY/build/lib/flask_dry/api/steps.py
# Compiled at: 2015-04-13 12:09:04
# Size of source mod 2**32: 46063 bytes
"""These are the steps to choose from to implement an HTTP method.
...
@step('authorize', 'authorization_requirements,authorization_context,cache_public', 'role,cache_public', milestone=200)
def authorize(context):
    """Does the authorization check against `authorization_context`.
    ...
    context.role = None
    if requirements:
        if current_user.is_authenticated() and current_user.superuser and login_fresh():
            if context.debug:
...
def check_csrf(context):
    if current_user.is_authenticated():
        ...
        if not (current_app.testing and csrf == 'Get Out of Jail Free'):
...
def get_item_versions(context):
    context.last_modified = get_universal(context.item, 'last_modified_timestamp')
    context.etag = get_universal(context.item, 'etag')


@step('check_conditionals', milestone=300)
def no_conditionals(context):
    for header in ('If-Modified-Since', 'If-Unmodified-Since', 'If-Match', 'If-None-Match'):
        if header in request.headers:
            abort(400, message='{}: Conditional header not allowed'.format(header))
            continue


@step('check_conditionals', 'last_modified,etag,headers', milestone=300)
def check_get_conditionals(context):
    """Checks conditional headers for GET.
    ...
    if if_unmodified_since:
        if if_unmodified_since <= datetime.utcnow() and context.last_modified is not None and context.last_modified > if_unmodified_since:
            abort(412)
    if 'If-Match' in request.headers:
        if not request.if_match.contains(str(context.etag)):
            abort(412)
    no_change = False
    changed = False
    if_modified_since = request.if_modified_since
    if if_modified_since:
        if if_modified_since <= datetime.utcnow() and context.last_modified is not None:
...
def check_update_conditionals(context):
    """Checks conditional headers for PUT/DELETE.
    ...
    if last_modified is None:
        if 'If-Unmodified-Since' in request.headers:
            abort(400, message='If-Unmodified-Since header not supported on {}'.format(context.method.upper()))
    elif 'If-Unmodified-Since' not in request.headers:
        abort(400, message='If-Unmodified-Since header required on {}'.format(context.method.upper()))
    if 'If-Match' not in request.headers:
        abort(400, message='If-Match header required on {}'.format(context.method.upper()))
    if last_modified is not None and last_modified > request.if_unmodified_since or not request.if_match.contains(str(context.etag)):
        print('last_modified', last_modified, 'etag', context.etag)
        abort(412)


@parameterize
@if_needed('unpack_json_parameters', 'errors', 'required_parameters,optional_parameters,errors')
def unpack_json_parameters(*required_parameters, context, **optional_parameters):
...
def get_key_columns(context):
    """Creates a mapping from column_name to column_value.

    Remaps the key names in columns_to_keys to column names and looks up each
    key in context for the value.
    """
    context.key_columns = {c:getattr(context, k) for c, k in context.columns_to_keys.items()}
...
def get_auth_item(context):
    ...
    try:
        ...
        abort(500, debug='Multiple responses found for {}: id={}'.format(context.auth_model.__name__, id))
...
def get_item(context):
    ...
    try:
        ...
        abort(500, debug='Multiple responses found for {}'.format(', '.join('{}={}'.format(k, v) for k, v in sorted(context.key_columns.items()))))
    except NoResultFound:
        if len(context.key_columns) == 1:
            abort(context.model.get_status(tuple(context.key_columns.values())[0]))
        else:
            abort(404)
...
def query_list(context):
    model = context.model
    context.query = context.query.filter(*[getattr(model, key) == getattr(context, key) for key in context.keys])
...
def init_errors(context):
    """Initializes the `errors` and `revised` attributes on context.
    """
...
def init_list_errors(context):
    """Initializes the `errors` attribute on context.
    """
...
def get_new_item(context):
    """Places the json input payload into `new_item`.

    Converts datetime columns into datetime objects.
    """
    context.new_item = _convert_datetimes(context.model._dry_column_info, request.json)
...
def get_new_list(context):
    """Places the json input payload into `new_list`.

    Converts datetime columns into datetime objects.
    """
    column_info = context.model._dry_column_info
    context.new_list = [_convert_datetimes(column_info, x) for x in request.json]
...
@if_needed('get_updated_columns', 'new_item,item.1,model,errors', 'updated_columns,errors,changed')
def get_updated_item_columns(context):
    """Populates updated_columns with columns that have changed from new_item.
    ...
    unknown child ids.
    """
    context.updated_columns = make_updated_item(context.model, context.new_item, context.item, context.errors, context.column_auth_for_update())
    context.changed = context.updated_columns._status != 'unchanged'


@if_needed('get_updated_columns', 'new_item,model,errors', 'updated_columns,errors')
def get_insert_item_columns(context):
    """Populates updated_columns with legal columns from new_item.
    ...
    """
    context.updated_columns = make_updated_item(context.model, context.new_item, None, context.errors, context.column_auth_for_insert())


@step('get_updated_columns', 'new_list,output_list.1,model,errors,keys', 'updated_list,errors,changed')
def get_updated_list_columns(context):
    """Figures out what changed between `output_list` and `new_list`.
    ...
    context.updated_list = make_sublist(context.model, context.new_list, context.output_list, context.errors, item_column_auth(context), key, value)
    context.changed = context.updated_list._status != 'unchanged'


@step('abort_no_updates', 'changed,errors,revised', milestone=290)
...
@step('model_validation', 'updated_columns.2,role,now,errors,revised', 'updated_columns.3,errors,revised')
def validate_update(context):
    """Does cross validation checks between the updated_columns and item.

    Also adds system_updated_attributes (e.g., time modified columns).

    Calls validate_update on the model to do this.
    """
    _model_validation(context, context.updated_columns, context.role, context.now, context.errors, context.revised, context.updated_columns._old_item.validate_update)


@step('model_validation', 'updated_columns.2,role,now,errors,revised', 'updated_columns.3,errors,revised')
def validate_insert(context):
    """Does cross validation checks in updated_columns.

    Also adds system_updated_attributes (e.g., time modified columns).

    Calls validate_insert on the model to do this.
    """
    _model_validation(context, context.updated_columns, context.role, context.now, context.errors, context.revised, context.updated_columns._model.validate_insert)


def _model_validation(context, updated_columns, role, now, errors, revised, validation_method):
    validation_method(context, updated_columns, role, now, errors, revised)
    for column, updated_list in updated_columns.sublists():
        revised_list = _model_list_validation(context, updated_list, role, now, errors[column], revised.get(column))
        if any(revised_list):
            revised[column] = revised_list
            continue


@step('model_validation', 'updated_list.2,role,now,errors,revised', 'updated_list.3,errors,revised')
def validate_updates_in_list(context):
    revised_list = _model_list_validation(context, context.updated_list, context.role, context.now, context.errors, context.revised)
    if any(revised_list):
        context.revised = revised_list


def _model_list_validation(context, updated_list, role, now, errors_list, revised_list=None):
    ...
    for updated_item in updated_list.updated_items:
        if updated_item._status in ('updated', 'new'):
            _model_validation(context, updated_item, role, now, errors_list[updated_item._index], revised_list[updated_item._index], update_item._old_item.validate_update if updated_item._status == 'updated' else updated_item._model.validate_insert)
            continue

    return revised_list


@step('validate_columns', 'updated_columns,errors,revised', 'updated_columns.2,errors,revised', milestone=270)
def validate_updated_columns(context):
    """Calls :method:`.base_data.column_info.Column_info.validate` on each
    column.
    """
...
def _validate_columns(updated_columns, errors, revised):
    ...
    for column, value in tuple(updated_columns.user_updated_attributes()):
        new_value, column_errors = model.dry_get_column_info(column).validate(value, updated_columns._status == 'updated')
        if new_value != value:
...
@step('validate_columns', 'updated_list,errors', 'updated_list.2,errors,revised')
def validate_updated_columns_in_list(context):
    """Calls :method:`.base_data.column_info.Column_info.validate` on each
    column.
    """
    context.revised = _validate_columns_in_list(context.updated_list, context.errors)


def _validate_columns_in_list(updated_list, errors_list):
    """Validates the columns in the items within `updated_list`.
    ...
    for updated_item in updated_list.updated_items:
        if updated_item._status in ('updated', 'new'):
            _validate_columns(updated_item, errors_list[updated_item._index], revised_list[updated_item._index])
            continue
...
@step('update_item', 'updated_columns,now,errors,revised', 'item.2,errors,revised,final_last_modified,final_etag')
def update_item(context):
    """Updates `_old_item` in `updated_columns`.

    Calls :method:`.Model.dry_cross_validate` and
    :method:`.Model.check_database_errors`.
    """
    context.final_last_modified, context.final_etag = _update_item(context.updated_columns, context.now, context.errors, context.revised)


def _update_item(updated_item, now, errors, revised):
    """Returns final_last_modified, final_etag.
    """
    ...
    for column, updated_list in updated_item.sublists():
        revised_list = _update_list(updated_list, now, errors[column], revised.get(column))
        if revised_list:
            revised[column] = revised_list
            continue
...
@step('update_columns', 'updated_list,now,errors,revised', 'output_list.2,errors,revised')
def update_list(context):
    context.revised = _update_list(context.updated_list, context.now, context.errors, context.revised)
...
@step('create_item', 'updated_columns,now,errors,revised', 'errors,revised,item,final_last_modified,final_etag')
def create_item(context):
    """Creates `item` from `updated_columns`.
    ...
    """
    context.item, context.final_last_modified, context.final_etag = _create_item(context.updated_columns, context.now, context.errors, context.revised)


def _create_item(new_item, now, errors, revised):
    """Returns (new item, last_modified_timestamp, etag).
    """
    ...
    for column, updated_list in new_item.sublists():
        ...
        for new_child in updated_list.get_by_status('new'):
            ...
            new_child.add_system_updated_attribute(updated_list.parent_link_column, parent_key)
            i = new_child._index
    ...
    return (
     item, getattr(item, 'last_modified_timestamp', None),
     getattr(item, 'etag', None))


@step('delete_item', 'item,now', 'output')
def delete_item(context):
    """Logically deletes an item.
...
def _create_metadata(item, metadata_type):
    ...
    def convert(column_info):
        if isinstance(column_info, dict):
            return dict(type='children', child=_create_metadata(column_info, metadata_type))
        ans = dict(type=column_info.col_type, nullable=column_info.nullable)
        if metadata_type == 'create':
            ans['optional'] = column_info.optional
        ans.update(column_info.validator_info())
        return ans

    return {column:(convert(column_info) if column != '_links' else column_info) for column, column_info in item.items()}
...
def filter_item_columns(context):
    """Filter allowed columns from `item`, creating `output`.
    """
...
def _filter_item(item, context, action_column_auth):
    ...
    def convert(key, value):
        if isinstance(value, (list, tuple)) and value and isinstance(value[0], (dict, Model)):
            sub_action_column_auth = action_column_auth.get_sublist_column_auth(key)
            return [_filter_item(x, context, sub_action_column_auth) for x in value]
        else:
    ...
    return {key:convert(key, get_universal(item, key)) for key in action_column_auth.allowed(item.keys())}


@step('filter_output_columns', 'output_list,column_auth_method', 'output_list.2')
def filter_list_columns(context):
    action_column_auth = getattr(context, context.column_auth_method)()
    context.output_list = [_filter_item(x, context, action_column_auth) for x in context.output_list]
...
def convert_links_for_item(item, model, add_top_self_link=False):
    ...
    if add_top_self_link:
        if len(model._dry_primary_keys) == 1:
    ...
    for column, foreign_model in model.get_links():
        ...
        if link is not None:
            item[column] = {column: item[column],  '_links': link}
            continue

    for column, (foreign_model, _) in model.get_relationships():
        if column in item:
            for x in item[column]:
                convert_links_for_item(x, foreign_model, add_top_self_link=True)
...
def list_to_dict(context):
    """Converts the list in `output_list` into an `output` dict.
    ...
    if output_list:
        ...
        def get_keys(obj):
            """Translates id: {'id': xx, '_links': ...} into xx.
            """
            ans = []
            for key in keys:
...
def to_json(context):
    context.output_bytes = json.dumps(context.output, **json_settings).encode('utf-8')
...
@if_needed('set_final_versions', 'last_modified,etag', 'final_last_modified,final_etag')
def set_final_versions(context):
    """Copies last_modified/etag to final_last_modified/final_etag.
    """
    context.final_last_modified = context.last_modified
    context.final_etag = context.etag


@step('add_version_headers', 'final_last_modified,final_etag,headers', 'headers')
def add_version_headers(context):
    context.headers = _add_version_headers(context.final_last_modified, context.final_etag, context.headers)
...
@step('add_general_links', 'relation_category,authorization_context,output,cache_public', 'output.2,cache_public')
def add_general_links(context):
    rc_links, public = links.get_relation_category_links(context.relation_category, context.authorization_context)
...
def set_known_keys_from_item(context):
    item = context.item
    model = context.model
    context.known_keys = {(model.__name__.lower() + '_id' if col == 'id' else col):getattr(item, col) for col in model._dry_primary_keys}


@step('add_specific_links', 'known_keys,authorization_context,output,cache_public', 'output.2,cache_public')
def add_specific_links(context):
    specific_links, public = links.get_keyed_links(context.known_keys, context.authorization_context)
...
def add_links(output, links):
    """Merges links into output['_links'].
    """
    if links:
        ...
        for rc, relations in links.items():
            ...
            for relation, url in relations.items():
                if relation in _links[rc]:
                    abort(500, debug='duplicate linke: {}.{}'.format(rc, relation))
...
def add_location_header(context):
    """Only used in post to report the location of the newly created item.
    """
    context.headers['Location'] = context.model.item_resource.url_for_item(context.item)
...
def create_response(context):
    if context.output_bytes is not None:
        ...
        context.response = make_response(context.output_bytes, context.status, context.headers)
    else:
        output = context.output
        if not output:
            context.response = make_response('', context.status, context.headers)
        else:
            context.response = (
             output, context.status, context.headers)


def strip_errors(errors):
    """Strip lists of empty dicts from child errors lists.
...
def strip_revised(revised):
    """Strip lists of empty dicts from child revised lists.
