...
# [GCC 8.4.0]
# Embedded file name: /home/bruce/GoTonight/restful_poc/Flask-RESTful-DRY/build/lib/flask_dry/api/updated_items.py
# Compiled at: 2015-04-14 07:41:19
# Size of source mod 2**32: 17773 bytes
import sys
from collections import defaultdict
from itertools import chain

class system_attributes:
    __doc__ = 'Records system updated attributes on an item.\n\n    System updated attributes are set by the system and bypass validation.\n    Examples are last_modified_timestamp and etag.\n    '
...
class new_item(system_attributes):
    ...
    def __getattr__(self, key):
        """This does not provide access to system_updated_attributes.
        """
        if key.startswith('_'):
        ...
        try:
            ...
            raise AttributeError('New {!r} object has no attribute {!r}'.format(self._model.__name__, key)) from None
    ...
    def attr_changed(self, attr):
        return attr in self._user_updated_attributes or attr in self._system_updated_attributes

    def user_updated_attributes(self):
        """Generates key, value pairs for user_updated_attributes.
        """
        return self._user_updated_attributes.items()

    def updated_attributes(self):
        """Generates key, value pairs for all updated attributes.
        """
        return chain(self._user_updated_attributes.items(), self._system_updated_attributes.items())
...
class updated_item(new_item):
    ...
    def __repr__(self):
        keys = ', '.join('{}={}'.format(key, getattr(self._old_item, key)) for key in self._model._dry_primary_keys)
        updated_attrs = tuple(self._user_updated_attributes.keys()) + tuple(key for key, sl in self._sublists.items() if sl._status != 'unchanged')
        return '<updated_item: {}={{{}}} updated {}>'.format(self._model.__name__, keys, updated_attrs)
...
class updated_list:
    ...
    def get_by_status(self, status):
        """Generates updated_items that match `status`.
        """
...
def make_updated_item(model, user_item, old_item, errors, action_column_auth):
    """Returns a new_item or updated_item.
        ...
        if new_column in column_info:
            if old_item is None or new_value != getattr(old_item, new_column):
                if new_column in denied:
                    print('Attempted change to unauthorized column', new_column, 'in', model.__name__, file=sys.stderr)
            ...
            sublists[new_column] = make_sublist(child_model, new_value, () if old_item is None else getattr(old_item, new_column), list_errors, action_column_auth.get_sublist_column_auth(new_column), parent_link_column, key)
...
def make_sublist(model, user_list, old_list, list_errors, item_column_auth, parent_link_column, parent_key=None):
    """Returns an updated_list.
    ...
    old_items = {getattr(item, key_column):item for item in old_list}
    sublist = []
    for i, user_item in enumerate(user_list):
        if key_column in user_item:
            ...
            if item_key not in old_items:
                ...
                continue
            updated_item = make_updated_item(model, user_item, old_items[item_key], list_errors[i], item_column_auth.get_update_column_auth())
            del old_items[item_key]
        else:
            updated_item = make_updated_item(model, user_item, None, list_errors[i], item_column_auth.get_insert_column_auth())
        updated_item._index = i
        if updated_item._status != 'unchanged':
            if parent_key is not None:
                user_parent_key = getattr(updated_item, parent_link_column, None)
                if user_parent_key is None:
                    updated_item.add_system_updated_attribute(parent_link_column, parent_key)
                elif user_parent_key != parent_key:
                    list_errors[i][parent_link_column].append('Incorrect parent id')
            sublist.append(updated_item)

    sublist.extend(deleted_item(model, old_item) for old_item in old_items.values())
