...
# [GCC 8.4.0]
# Embedded file name: /home/bruce/GoTonight/restful_poc/Flask-RESTful-DRY/build/lib/flask_dry/api/item_resource.py
...
def init_self(cls, url_dummies, dummy_path):
    ...
    if len(cls.keys) == 1:
        if len(cls.model._dry_primary_keys) != 1:
            raise AssertionError('Number of primary keys, {}, does not match single url_parameter, {}'.format(len(cls.model._dry_primary_keys), cls.keys))
        cls.columns_to_keys = {cls.model._dry_primary_keys[0]: cls.keys[0]}
    else:
        if sorted(cls.model._dry_primary_keys) != sorted(cls.keys):
            raise AssertionError('Multiple primary keys {} do not match url_parameters {}'.format(sorted(cls.model._dry_primary_keys), sorted(cls.keys)))
...
def init_metadata_update(cls, url_dummies, dummy_path):
    ...
    if pm is not None:
        cls.get_method.authorization_requirements = pm.authorization_requirements
...
class Item_Resource(DRY_Resource):
    ...
    self = attrs(get_method=lookup('get_item_method'), put_method=lookup('put_item_method'), delete_method=lookup('delete_item_method'), post_method=None, init_fn=init_self, provided_attributes=extend('columns_to_keys'))
    self.metadata_update = attrs(url_extension='/metadata', get_method=lookup('get_update_metadata_method'), put_method=None, post_method=None, delete_method=None, init_fn=init_metadata_update)
    collection = attrs(url_extension='', auth_item=None, provided_attributes=extend('auth_item'), get_method=lookup('get_list_method'), post_method=lookup('post_item_method'), put_method=None, delete_method=None, init_fn=init_collection)
    collection.metadata_create = attrs(url_extension='/metadata', get_method=lookup('get_create_metadata_method'), put_method=None, post_method=None, delete_method=None, init_fn=init_metadata_create)
    ...
    def create_url_classes(cls, resource_class, api, url_dummies, dummy_path=(), base_url_class=None):
        if not hasattr(cls.self, 'url_extension'):
            cls.self.url_extension = '/<int:{}_id>'.format(cls.model.__name__.lower())
        super().create_url_classes(resource_class, api, url_dummies, dummy_path, base_url_class)

    @classmethod
    def url_for_item(cls, item):
        self = cls.relations['self']
        return self.url_format.format(**{key:get_universal(item, column) for column, key in self.columns_to_keys.items()})

    @classmethod
    def link_for_key(cls, key):
        """Returns {relation: url}.
        """
        self = cls.relations['self']
