...
# [GCC 8.4.0]
# Embedded file name: /home/bruce/GoTonight/restful_poc/Flask-RESTful-DRY/build/lib/flask_dry/api/links.py
# Compiled at: 2015-04-14 08:38:39
# Size of source mod 2**32: 5569 bytes
...
def register_link(app, keys, relation_category, relation, url_format, authorization_requirements):
    requirements = []
...
def get_relation_category_links(relation_category, auth_context):
    """Returns {relation_category: {relation: url}}, cache_public.
    """
    links, cache_public = _filter(current_app.dry_relation_category_links[relation_category], auth_context)
    if links:
...
def get_keyed_links(keys, auth_context):
    ...
    """
    auth_links, cache_public = _filter(chain.from_iterable(current_app.dry_keyed_links[tuple(sorted(subset))] for subset in powerset(keys.keys())), auth_context)
    return (
     {relation_category:{relation:url_format.format(**keys) for _, relation, url_format in links} for relation_category, links in groupby(sorted(auth_links, key=itemgetter(0)), key=itemgetter(0))},
     cache_public)


def _filter(links, auth_context):
    """Returns authorized links, cache_public.
    """
    auth_links = []
    cache_public = True
