...
# Compiled at: 2018-04-19 18:06:30
"""
...
@author: jrm
"""
import sh, os, re, sys, json, shutil, tarfile, fnmatch, compileall
...
def shprint(cmd, *args, **kwargs):
    ...
    kwargs.update({'_err_to_out': True, 
       '_out_bufsize': 0, 
       '_iter': True})
    print ('{}[INFO]:   -> running  {} {}{}').format(Colors.CYAN, cmd, (' ').join([ a for a in args if not isinstance(a, sh.RunningCommand)
                                                                                  ]), Colors.RESET)
    buf = []
    for c in cmd(*args, **kwargs):
        if debug:
            ...
            write(('{}\r[DEBUG]:       {:<{w}}{}').format(color, msg, Colors.RESET, w=100))
            flush()
            buf = []
        else:
            buf.append(c)
...
class Create(Command):
    ...
    def run(self, args):
        ...
        configure_logger(stream_level='DEBUG' if args.verbose else 'INFO', debug_file=None)
        cookiecutter(template, no_input=args.no_input, overwrite_if_exists=args.overwrite_if_exists)
...
class BuildRecipe(Command):
    ...
    def run(self, args):
        ...
        shprint(self.cli.conda, 'build', args.package, _env=env, *args.args)
...
class MakePipRecipe(Command):
    ...
    def build(self, package, args):
        ...
        for recipe in new:
            ...
            build_script += [ ('{script} --no-compile --install-base=$PREFIX/{prefix} --install-lib=$PREFIX/{prefix}/python/site-packages --install-scripts=$PREFIX/{prefix}/scripts --install-data=$PREFIX/{prefix}/data --install-headers=$PREFIX/{prefix}/include').format(script=script.strip(), prefix=p, **ctx) for p in [
...
class NdkStack(Command):
    ...
    def run(self, args=None):
        ctx = self.ctx
        env = ctx['android']
        ndk_stack = sh.Command(os.path.expanduser(join(env['ndk'], 'ndk-stack')))
...
class NdkBuild(Command):
    ...
    def run(self, args=None):
        ctx = self.ctx
        env = ctx['android']
        ndk_build = sh.Command(os.path.expanduser(join(env['ndk'], 'ndk-build')))
        ...
        for arch in arches:
            cfg = dict(arch=arch, local_arch=ANDROID_ABIS[arch], ndk_build_dir=ndk_build_dir)
            cfg.update(env)
    ...
    def run(self, args=None):
        ...
        if args.target == 'android':
            for arch in env['targets']:
                cfg.update(dict(target=('android/{}').format(arch), local_arch=arch, arch=ANDROID_TARGETS[arch]))
                break
...
class Link(Command):
    ...
    args = set_default([
     (
      'names',
      dict(help='Package name (optional) If not set links all projects.', nargs='*'))])
    package_dir = 'venv'
        ...
        try:
            ...
            if Link.is_settings_linked(settings_gradle, pkg):
                new_settings = [ line for line in settings_gradle.split('\n') if line.strip() not in [
                 ...
                 ("project(':{name}').projectDir = new File(rootProject.projectDir, '../{path}/android/{name}')").format(path=path, name=pkg)]
                               ]
                with open(join('android', 'settings.gradle'), 'w') as (f):
                    f.write(('\n').join(new_settings))
    ...
    def run(self, args=None):
        with cd('android'):
            ...
            if args and args.release:
                shprint(gradlew, 'assembleRelease', _debug=True, *args.extra)
            else:
                shprint(gradlew, 'assembleDebug', _debug=True, *args.extra)
    ...
    def run(self, args=None):
        ...
        with cd('android'):
            ...
            if args and args.release:
                ...
                shprint(gradlew, 'installDebug', _debug=True, *args.extra)
