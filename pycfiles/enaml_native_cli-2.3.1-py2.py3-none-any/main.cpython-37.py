...
# [GCC 8.4.0]
# Embedded file name: /home/jrm/Workspace/enaml-native-cli/enamlnativecli/main.py
# Compiled at: 2019-08-24 09:43:34
# Size of source mod 2**32: 64078 bytes
"""
            ...
            if args.recursive:
                ...
                for pkg in requires:
                    ...
                    if pkg in excluded or pkg in self._built:
                        continue
                    if not args.force:
                        exists('pip-{}'.format(pkg)) or self.build(pkg, args)
...
class Install(Command):
    ...
    args = set_default([
     (
      'args',
      dict(nargs='*', help='Args to pass to conda install. If blank it uses the environment.yml file'))])
    app_dir_required = set_default(False)
