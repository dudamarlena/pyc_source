...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/urlparse3/tests.py
# Compiled at: 2016-12-14 08:52:25
import unittest
...
class TestUrlParse(TestCase):
    ...
    def test_parse_url(self):
        ...
        self.assertEqual(parsed_url.geturl(), 'http://google.com/path/?cardNumber=12345678910')
    ...
    def test_join_url(self):
        ...
        self.assertEqual('http://yandex.ru/mail/?id=123&name=alex#anchor', parsed_url.geturl())
        parsed_url.fragment = 'fragment'
        self.assertEqual('http://yandex.ru/mail/?id=123&name=alex#fragment', parsed_url.geturl())
        url = 'http://yandex.ru/path/?id=1&id=2&id=3&name=alex#anchor'
        parsed_url = urlparse3.parse_url(url)
        self.assertEqual(len(set(parsed_url.query['id'])), 3, 'Missing parameters in query')
        for i in parsed_url.query['id']:
            self.assertIn(i, ['1', '2', '3'])

        parsed_url.query['id'] = [
         '1', '2']
        self.assertEqual('http://yandex.ru/path/?id=1&id=2&name=alex#anchor', parsed_url.geturl())
        url = 'http://yandex.ru'
        parsed_url = urlparse3.parse_url(url)
        parsed_url.path = 'search'
        self.assertEqual('http://yandex.ru/search', parsed_url.geturl())
        url = 'http://yandex.ru'
        parsed_url = urlparse3.parse_url(url)
        self.assertEqual('http://yandex.ru/', parsed_url.geturl())
    ...
    def test_query_parameters_order(self):
        ...
        self.assertEqual(('{0}&action={1}&order={2}').format(url, sort_action, price_order), new_url)
    ...
    def test_rearrage_parameters_order(self):
        ...
        new_url = parsed_url.geturl()
        self.assertEqual(('{0}?action={1}&order={2}&url={3}').format(base_url, sort_action, price_order, url_parameter), new_url)
