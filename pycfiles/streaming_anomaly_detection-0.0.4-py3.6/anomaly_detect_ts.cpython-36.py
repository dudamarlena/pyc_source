...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/anomaly_detection/anomaly_detect_ts.py
# Compiled at: 2019-02-21 08:36:20
# Size of source mod 2**32: 29812 bytes
"""
Description:
...
import datetime, statsmodels.api as sm, logging
...
def _get_data_tuple(raw_data, period_override, resampling=False):
    ...
    if timediff.days > 0:
        ...
        if timediff.seconds / 60 / 60 >= 1:
            granularity = 'hr'
            period = _get_period(24, period_override)
        else:
            if timediff.seconds / 60 >= 1:
                granularity = 'min'
...
def anomaly_detect_ts(raw_data, max_anoms=0.1, direction='pos', alpha=0.05, only_last=None, threshold=None, e_value=False, longterm=False, piecewise_median_period_weeks=2, plot=False, y_log=False, xlabel='', ylabel='count', title='shesd output: ', verbose=False, dropna=False, resampling=False, period_override=None, multithreaded=False):
    if verbose:
        logging.info('Validating input parameters')
    else:
        if not isinstance(raw_data, pd.Series):
            if not isinstance(raw_data, ddf.Series):
                raise AssertionError('Data must be a Pandas or Dask Series')
            else:
                if not raw_data.values.dtype in [int, float]:
                    raise AssertionError('Values of the series must be number')
                else:
                    if not raw_data.index.dtype == np.dtype('datetime64[ns]'):
                        raise AssertionError('Index of the series must be datetime')
                    else:
                        if not (max_anoms <= 0.49 and max_anoms >= 0):
                            raise AssertionError('max_anoms must be non-negative and less than 50% ')
                        else:
        ...
        for series in all_data:
            shesd = _detect_anoms(series, k=max_anoms, alpha=alpha, num_obs_per_period=period, use_decomp=True, use_esd=False,
              direction=direction,
              verbose=verbose,
    ...
    if all_anoms.empty:
        ...
        return {'anoms':pd.Series(), 
         'plot':None}
    else:
        if plot:
            num_days_per_line
            raise Exception('TODO: Unsupported now')
        return {'anoms':all_anoms, 
         'expected':seasonal_plus_trend if e_value else None, 
         'plot':'TODO' if plot else None}


def _detect_anoms(a_series, k=0.49, alpha=0.05, num_obs_per_period=None, use_decomp=True, use_esd=False, direction='pos', verbose=False, multithreaded=False):
    if not num_obs_per_period:
        raise AssertionError('must supply period length for time series decomposition')
    else:
        if not direction in ('pos', 'neg', 'both'):
            raise AssertionError('direction options: pos | neg | both')
        elif not a_series.size >= num_obs_per_period * 2:
            raise AssertionError('Anomaly detection needs at least 2 periods worth of data')
    ...
    return {'anoms':R_idx,  'stl':smoothed}
