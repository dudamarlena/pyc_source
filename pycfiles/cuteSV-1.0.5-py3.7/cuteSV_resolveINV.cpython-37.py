...
def resolution_INV(path, chr, svtype, read_count, max_cluster_bias, sv_size, bam_path, action, hom, het, MaxSize):
    """
        cluster INV
        ************************************************************************
        path:   INV.sigs
    ...
    for line in file:
        ...
        if breakpoint_1_in_read - semi_inv_cluster[(-1)][0] > max_cluster_bias:
            if len(semi_inv_cluster) >= read_count:
                generate_semi_inv_cluster(semi_inv_cluster, chr, svtype, read_count, sv_size, candidate_single_SV, max_cluster_bias, bam_path, action, hom, het, MaxSize)
            semi_inv_cluster = []
...
def generate_semi_inv_cluster(semi_inv_cluster, chr, svtype, read_count, sv_size, candidate_single_SV, max_cluster_bias, bam_path, action, hom, het, MaxSize):
        ...
        if i[1] - last_bp > max_cluster_bias:
            if temp_count >= read_count:
                ...
                if inv_len >= sv_size and max_count_id >= read_count:
                    if inv_len <= MaxSize:
                        if action:
                            DV, DR, GT = call_gt(bam_path, int(breakpoint_1), int(breakpoint_2), chr, list(temp_id.keys()), max_cluster_bias, hom, het)
                        else:
                            DR = '.'
                            GT = './.'
