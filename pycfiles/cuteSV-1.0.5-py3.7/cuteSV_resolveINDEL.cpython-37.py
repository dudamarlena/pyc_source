...
# Size of source mod 2**32: 17619 bytes
import sys, numpy as np
from collections import Counter

def resolution_DEL(path, chr, svtype, read_count, threshold_gloab, max_cluster_bias, threshold_local, minimum_support_reads, bam_path, action, hom, het):
    """
        cluster DEL
        ********************************************************************************************
        path:   DEL.sigs
    ...
    for line in file:
        ...
        if pos - semi_del_cluster[(-1)][0] > max_cluster_bias:
            if len(semi_del_cluster) >= read_count:
                generate_del_cluster(semi_del_cluster, chr, svtype, read_count, threshold_gloab, threshold_local, minimum_support_reads, candidate_single_SV, bam_path, max_cluster_bias, action, hom, het)
            semi_del_cluster = []
...
def generate_del_cluster(semi_del_cluster, chr, svtype, read_count, threshold_gloab, threshold_local, minimum_support_reads, candidate_single_SV, bam_path, max_cluster_bias, action, hom, het):
    """
        generate deletion
        *************************************************************
    ...
    if len(read_tag) < read_count:
        ...
        if alelle_sort[(-1)][2][0] >= minimum_support_reads and float(alelle_sort[(-1)][2][0] * 1.0 / len(read_tag)) >= threshold_local:
            ...
            if action:
                DV, DR, GT = call_gt(bam_path, search_threshold, chr, alelle_sort[(-1)][3], max_cluster_bias, hom, het)
    else:
        DR = '.'
        GT = './.'
    ...
    if len(alelle_sort) > 1 and alelle_sort[(-2)][2][0] >= minimum_support_reads and alelle_sort[(-2)][2][0] + alelle_sort[(-1)][2][0] >= 0.95 * len(read_tag) and alelle_sort[(-2)][2][0] >= 0.3 * len(read_tag):
        ...
        if action:
            DV, DR, GT = call_gt(bam_path, search_threshold, chr, alelle_sort[(-2)][3], max_cluster_bias, hom, het)
        else:
            DR = '.'
            GT = './.'
        candidate_single_SV.append([chr,
...
def resolution_INS(path, chr, svtype, read_count, threshold_gloab, max_cluster_bias, threshold_local, minimum_support_reads, bam_path, action, hom, het):
    """
        cluster INS
        ********************************************************************************************
        path:   INS.sigs
    ...
    for line in file:
        ...
        if pos - semi_ins_cluster[(-1)][0] > max_cluster_bias:
            if len(semi_ins_cluster) >= read_count:
                generate_ins_cluster(semi_ins_cluster, chr, svtype, read_count, threshold_gloab, threshold_local, minimum_support_reads, candidate_single_SV, bam_path, max_cluster_bias, action, hom, het)
            semi_ins_cluster = []
...
def generate_ins_cluster(semi_ins_cluster, chr, svtype, read_count, threshold_gloab, threshold_local, minimum_support_reads, candidate_single_SV, bam_path, max_cluster_bias, action, hom, het):
    """
        generate deletion
        *************************************************************
        threshold_gloab         threshold_local         minimum_support_reads
    ...
    if len(read_tag) < read_count:
        ...
        if alelle_sort[(-1)][2][0] >= minimum_support_reads and float(alelle_sort[(-1)][2][0] * 1.0 / len(read_tag)) >= threshold_local:
            ...
            signalLen_STD = np.std(alelle_sort[(-1)][1])
            if action:
                DV, DR, GT = call_gt(bam_path, int(breakpointStart), chr, alelle_sort[(-1)][3], max_cluster_bias, hom, het)
    else:
        DR = '.'
        GT = './.'
    candidate_single_SV.append([chr,
            ...
            if alelle_sort[(-2)][2][0] + alelle_sort[(-1)][2][0] >= 0.95 * len(read_tag):
                if alelle_sort[(-2)][2][0] >= 0.3 * len(read_tag):
                    ...
                    if signalLen_STD < last_signalLen_STD:
                        if action:
                            DV, DR, GT = call_gt(bam_path, int(breakpointStart), chr, alelle_sort[(-2)][3], max_cluster_bias, hom, het)
                        else:
                            DR = '.'
                            GT = './.'
                        candidate_single_SV.append([chr,
