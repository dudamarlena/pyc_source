...
def resolution_DUP(path, chr, read_count, max_cluster_bias, sv_size, bam_path, action, hom, het, MaxSize):
    ...
    for line in file:
        ...
        if pos_1 - semi_dup_cluster[(-1)][0] > max_cluster_bias:
            if len(semi_dup_cluster) >= read_count:
                generate_dup_cluster(semi_dup_cluster, chr, read_count, max_cluster_bias, sv_size, candidate_single_SV, bam_path, action, hom, het, MaxSize)
            semi_dup_cluster = []
...
def generate_dup_cluster(semi_dup_cluster, chr, read_count, max_cluster_bias, sv_size, candidate_single_SV, bam_path, action, hom, het, MaxSize):
    ...
    for i in predictions:
        if i[2] >= read_count and i[2] >= int(len(support_read) / 3) and i[1][0] - i[0][0] >= sv_size:
            ...
            if i[1][0] - i[0][0] <= MaxSize:
                if action:
                    DV, DR, GT = call_gt(bam_path, int(i[0][0]), int(i[1][0]), chr, support_read, max_cluster_bias, hom, het)
                else:
                    DR = '.'
                    GT = './.'
