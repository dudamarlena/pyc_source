...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/refaction/controllers/website.py
# Compiled at: 2008-09-18 12:59:00
...
class WebsiteController(ListController):
    table = model.Site
    children = dict(application=dict(table=model.SiteApp, columns=[
     'app', 'path'], parent=dict(app=dict(table=model.App, column='name'))), subdomain=dict(table=model.SiteDomain, columns=[
     'subdomain'], parent=dict(subdomain=dict(table=model.Subdomain, column='name'))))
    ...
    def _get_subdomains(self, p):
        ...
        for entry in entries:
            subdomains.append(('.').join([
             entry.name,
             model.Domain.get(entry.domain).name]).lstrip('.'))
