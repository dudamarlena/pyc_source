...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/refaction/lib/base.py
# Compiled at: 2008-09-18 10:45:42
...
import refaction.lib.helpers as h, refaction.model as model
...
class OwnedController(BaseBaseController):
    ...
    def __before__(self, action, **kw):
        ...
        if wsgi['action'] in ('login', 'logout', 'restricted') or wsgi['controller'] in ('index',
                                                                                         'template'):
            return
    ...
    def _get_cookie(self):
        ...
        elif not request.path_info.endswith('restricted') and not request.path_info.endswith('login'):
...
class ListController(ListBaseController, BaseController):
    ...
    def _get_permission(self):
        ...
        owner_info = dict(table=model.User, column='name')
