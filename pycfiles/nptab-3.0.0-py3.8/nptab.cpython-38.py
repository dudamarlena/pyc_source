...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/nptab/nptab.py
# Compiled at: 2020-03-13 18:05:40
...
try:
    ...
    def tabulate(nptab, columns=None, tablefmt=None):
        """Alt tabulate
        """
        outp = ''
        outp += ' '.join(columns) + '\n'
...
try:
    ...
    """
        ...
        Returns:
            self. I like chaining/fluent api's and don't care about being pythonic
        """
                if len(self) == 0 and isinstance(nptab, Nptab):
                    self.__init__((nptab.data), columns=(nptab.columns))
                else:
                    if len(self) == 0:
                        self.__init__(nptab)
                    else:
                        if isinstance(nptab, Nptab):
                            assert set(self.columns) == set(nptab.columns), 'Not the same columns in Nptab: {} {}'.format(self.columns, nptab.columns)
                            for col, dta in zip(nptab.columns, nptab.data):
                                ci = self.columns.index(col)
                                self.data[ci] = np.concatenate([self.data[ci], dta])

                        else:
                            if isinstance(nptab, pd.DataFrame):
                                assert set(self.columns) == set(nptab), 'Not the same columns in Nptab: {} {}'.format(self.columns, nptab.keys())
                                for col, dta in nptab.items():
                                    ci = self.columns.index(col)
                                    self.data[ci] = np.concatenate([self.data[ci], dta])
    ...
    Examples:
        ...
        :mod:`nptab.Nptab.__getitem__` with the same relation between key and
        ...
        Arguments:
            ...
            value (object):
                ...
                element:
                    ...
                    :mod:`nptab.Nptab.dtype` to get the type of the columns.
        ...
        Notes:
            ...
            than the whole column is replaced with the column provided.
                ...
                >>> nptab = Nptab( [ ["John", "Joe", "Jane"], [1.82, 1.65, 2.15],
        ...
        """
                nptab = tabulate([[c[r] for c in self.data] for r in range(min(len(self), self.max_repr_rows))],
                  (self.columns),
                  tablefmt=(self.repr_layout))
                len_str = '\n{} rows'.format(len(self))
                typ_str = ' {}'.format([dt.dtype.descr[0][1] for dt in self.data])
                return nptab + len_str + typ_str
        ...
        """
            ...
            def astype(self, dtypes):
                """Returns a type-converted nptab.

        Converts the nptab according to the provided list of dtypes and returns
        ...
        Examples:
            >>> nptab = Nptab({'a':['b', 'g', 'd'], 'b':list(range(3))})
            >>> nptab.sort('a')
            >>> nptab
             a   |   b
            -----+-----
