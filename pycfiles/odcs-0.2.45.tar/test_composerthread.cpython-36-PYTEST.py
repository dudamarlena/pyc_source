...
# Size of source mod 2**32: 15573 bytes
import builtins as @py_builtins, _pytest.assertion.rewrite as @pytest_ar, os, time
...
class TestComposerThread(ModelsBaseTest):
    ...
    def setUp(self):
        ...
        self.patch_pungi_conf_path = patch.object((odcs.server.conf), 'pungi_conf_path',
    ...
    @patch('odcs.server.utils.execute_cmd')
    @patch('odcs.server.backend._write_repo_file')
    def test_submit_build_module_without_release_not_in_pdc(self, wrf, execute_cmd):
...
class TestComposerThreadLostComposes(ModelsBaseTest):
    ...
    def setUp(self):
        ...
        self.composer = ComposerThread()
        self.patch_generate_new_compose = patch('odcs.server.backend.ComposerThread._generate_new_compose')
