...
# Size of source mod 2**32: 17301 bytes
import builtins as @py_builtins, _pytest.assertion.rewrite as @pytest_ar, json, unittest, mock
...
class TestMakeEndPoint(unittest.TestCase):
    ...
    def test_make_endpoint(self):
        ...
        self.assertEqual('{0}api/{1}/composes/'.format(self.server_url, odcs.api_version), endpoint_url)
        endpoint_url = odcs._make_endpoint('/composes/')
        self.assertEqual('{0}api/{1}/composes/'.format(self.server_url, odcs.api_version), endpoint_url)
...
class TestMakeRequest(unittest.TestCase):
    ...
    def test_with_kerberos_auth(self, HTTPKerberosAuth, requests):
        ...
        self.assertEqual(requests.get.return_value, r)
        requests.get.assert_called_once_with((odcs._make_endpoint(self.resource_path)),
          auth=expected_auth)
    ...
    def test_with_openidc_auth(self, requests):
        ...
        odcs = ODCS((self.server_url), auth_mech=(AuthMech.OpenIDC),
        ...
        requests.post.assert_called_once_with((odcs._make_endpoint(self.resource_path)),
          data=(json.dumps({'id': 1})),
          headers={'Authorization':'Bearer {0}'.format(fake_openidc_token), 
         'Content-Type':'application/json'})

    @patch('odcs.client.odcs.requests')
    def test_with_ssl_auth(self, requests):
        requests.post.return_value.status_code = 200
        odcs = ODCS((self.server_url), auth_mech=(AuthMech.SSL),
          ssl_cert='./ssl.crt',
          ssl_key='./ssl.key')
        r = odcs._make_request('post', (self.resource_path), data={'id': 1})
        self.assertEqual(requests.post.return_value, r)
...
class TestGetCompose(unittest.TestCase):
    ...
    def test_get_compose(self, requests):
        ...
        requests.get.assert_called_once_with('{0}api/{1}/composes/1'.format(self.server_url, self.odcs.api_version))
...
class TestNewCompose(unittest.TestCase):
    ...
    def test_create_a_new_compose(self, requests):
        ...
        new_compose = self.odcs.new_compose('cf-1.0-rhel-5', 'tag',
          packages=[
         'libdnet'],
          sigkeys=[
         '123', '456'],
        ...
        requests.post.assert_called_once_with((self.odcs._make_endpoint('composes/')),
          data=(json.dumps({'source':{'source':'cf-1.0-rhel-5', 
          'type':'tag', 
          'packages':[
           'libdnet'], 
          'sigkeys':[
...
class TestRenewCompose(unittest.TestCase):
    ...
    def test_renew_a_compose(self, requests):
        ...
        requests.patch.assert_called_once_with((self.odcs._make_endpoint('composes/6')),
...
class TestFindComposes(unittest.TestCase):
    ...
    def test_find_composes_without_pagination(self, requests):
        ...
        requests.get.assert_called_once_with((self.odcs._make_endpoint('composes/')),
