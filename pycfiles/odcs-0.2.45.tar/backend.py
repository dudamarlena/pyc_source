...
# Compiled at: 2019-04-03 04:23:10
...
def resolve_compose(compose):
    ...
    if compose.source_type == PungiSourceType.REPO:
        ...
        if not compose.koji_event:
            ...
            elif tag_changed(koji_session, compose.source, LAST_EVENTS_CACHE[compose.source]):
                event_id = int(koji_session.getLastEvent()['id'])
            else:
    ...
    if compose.source_type == PungiSourceType.RAW_CONFIG:
        ...
        for old_compose in composes:
            ...
            if module_defaults_url != old_module_defaults_url:
                ...
                continue
            if compose.koji_event and old_compose.koji_event and compose.koji_event < old_compose.koji_event:
...
def generate_pulp_compose(compose):
    ...
    pulp = Pulp(server_url=conf.pulp_server_url, username=conf.pulp_username, password=conf.pulp_password, compose=compose)
    repofile = ''
...
def generate_pungi_compose(compose):
    ...
    if compose_to_reuse:
        ...
        if compose.source_type == PungiSourceType.RAW_CONFIG:
            ...
            pungi_cfg = PungiConfig(compose.name, '1', compose.source_type, compose.source, packages=packages, sigkeys=compose.sigkeys, results=compose.results, arches=compose.arches.split(' '), multilib_arches=multilib_arches, multilib_method=compose.multilib_method, builds=builds, flags=compose.flags, lookaside_repos=compose.lookaside_repos, modular_koji_tags=compose.modular_koji_tags, module_defaults_url=compose.module_defaults_url)
