...
# Size of source mod 2**32: 22104 bytes
import imp, os, sys
...
def init_config(app):
    ...
    if any(['nosetests' in arg or 'noserunner.py' in arg or 'py.test' in arg or 'pytest.py' in arg for arg in sys.argv]):
        ...
        if 'ODCS_DEVELOPER_ENV' in os.environ:
            if os.environ['ODCS_DEVELOPER_ENV'].lower() in ('1', 'on', 'true', 'y',
                                                            'yes'):
                config_section = 'DevConfiguration'
                from conf import config
                config_module = config
...
class Config(object):
    ...
    _defaults = {'debug':{'type':bool, 
     ...
     'pungi_timeout':{'type':int, 
      'default':3600, 
      'desc':'Time in seconds after which the local pungi-koji is killed and compose is marked as failed'}, 
     'pungi_runroot_enabled':{'type':bool, 
      'default':False, 
      'desc':'When True, ODCS will run Pungi in Koji runroot'}, 
     'pungi_parent_runroot_channel':{'type':str, 
      'default':'', 
      'desc':'Name of Koji channel to run pungi runroot task with.'}, 
     'pungi_parent_runroot_packages':{'type':list, 
      'default':'', 
      'desc':'List of packages to install to pungi Koji runroot task.'}, 
     'pungi_parent_runroot_mounts':{'type':list, 
      'default':'', 
      'desc':'List of mounts to mount in pungi Koji runroot task.'}, 
     'pungi_parent_runroot_weight':{'type':float, 
      'default':3, 
      'desc':'Weight of pungi Koji runroot task.'}, 
     'pungi_parent_runroot_tag':{'type':str, 
      'default':'', 
      'desc':'Name of tag to run pungi Koji runroot task for.'}, 
     'pungi_parent_runroot_arch':{'type':str, 
      'default':'x86_64', 
      'desc':'Architecture to run pungi Koji runroot task for.'}, 
     'pungi_runroot_target_dir':{'type':str, 
      'default':'', 
      'desc':'Path to target dir to which store composes done by pungi in runroot'}, 
     'pungi_runroot_target_dir_url':{'type':str, 
      'default':'', 
      'desc':'Public facing URL to target_dir.'}, 
     'pungi_conf_path':{'type':str, 
      'default':'/etc/odcs/pungi.conf', 
      'desc':'Full path to the pungi.conf jinja2 template.'}, 
     'pungi_runroot_koji_conf_path':{'type':str, 
      'default':'/etc/odcs/runroot_koji.conf', 
      'desc':'Full path to the runroot_koji.conf configuration file. This file holds Koji configuration used in a Pungi parent runroot task if pungi_runroot_enabled is True.'}, 
     'target_dir':{'type':str, 
      'default':'/tmp', 
     ...
     'allowed_clients':{'type':dict, 
      'default':{'groups':{},  'users':{}},  'desc':'Groups and users that are allowed to generate composes.'}, 
     'admins':{'type':dict, 
      'default':{'groups':[],  'users':[]},  'desc':'Admin groups and users.'}, 
     ...
     'auth_openidc_required_scopes':{'type':list, 
      'default':[],  'desc':'Required scopes for submitting request to run new compose.'}, 
     'base_module_names':{'type':set, 
      'default':set(['platform', 'bootstrap']), 
      'desc':'Set of module names which defines the product version (by their stream) of modules depending on them.'}, 
     'messaging_backend':{'type':str, 
      'default':'', 
      'desc':'Messaging backend, fedmsg or umb.'}, 
     'messaging_broker_urls':{'type':list, 
      'default':[],  'desc':'List of messaging broker URLs.'}, 
     ...
     'messaging_topic_prefix':{'type':str, 
      'default':'', 
     ...
     'internal_messaging_topic':{'type':str, 
      'default':'', 
      'desc':'Messaging topic to which internal ODCS-only messages are sent.'}, 
     'oidc_base_namespace':{'type':str, 
      'default':'https://pagure.io/odcs/', 
      'desc':'Base namespace of OIDC scopes.'}, 
     'sigkeys':{'type':list, 
      'default':[],  'desc':'Default list of sigkeys. Any package in a compose must be signed by one of those keys. Can be overriden in a compose request.'}, 
     'pulp_server_url':{'type':str, 
      'default':'', 
      'desc':'Server URL of Pulp.'}, 
     'pulp_username':{'type':str, 
      'default':'', 
      'desc':'Username to login Pulp.'}, 
     'pulp_password':{'type':str, 
      'default':'', 
      'desc':'Password to login Pulp.'}, 
     'koji_config':{'type':str, 
      'default':None, 
      'desc':'Koji config file.'}, 
     'koji_profile':{'type':str, 
      'default':None, 
      'desc':'Koji config profile.'}, 
     'koji_krb_ccache':{'type':str, 
      'default':None, 
      'desc':'Kerberos ccache file to use for Koji auth.'}, 
     'koji_krb_keytab':{'type':str, 
      'default':None, 
      'desc':'Kerberos keytab to use for Koji auth.'}, 
     'koji_krb_principal':{'type':str, 
      'default':None, 
      'desc':'Kerberos principal to use for Koji auth.'}, 
     'koji_tag_cache_cleanup_timeout':{'type':int, 
      'default':30, 
      'desc':'Number of days after which the cached Koji tag data stored in the "koji_tag_cache" directory will be removed.'}, 
     'raw_config_urls':{'type':dict, 
      'default':{},  'desc':'URLs to get the raw Pungi config from for "raw_config" source_type. Key is the name of the raw_config "source", value is the URL in which the %s string is replaced with, "commit_hash" value.'}, 
     'raw_config_wrapper_conf_path':{'type':str, 
      'default':'/etc/odcs/raw_config_wrapper.conf', 
      'desc':'Full path to the raw_config_wrapper.conf configuration file. This file holds Pungi configuration which should import real pungi configuration from raw_config.conf in order to override some variables.'}, 
     'pungi_koji_args':{'type':list, 
      'default':[],  'desc':'Command line arguments used to construct pungi-koji command.'}, 
     'raw_config_pungi_koji_args':{'type':dict, 
      'default':{},  'desc':'Command line argument for raw_config source type, which overwrite arguments listed PUNGI_KOJI_ARGS.'}, 
     'celery_config':{'type':dict, 
      'default':{},  'desc':'Configuration dict to pass to Celery.'}, 
     'celery_broker_url':{'type':str, 
      'default':'', 
      'desc':'Celery broker URL'}, 
     'celery_pulp_composes_queue':{'type':str, 
      'default':'pulp_composes', 
      'desc':'Name of the Celery queue for Pulp composes.'}, 
     'celery_pungi_composes_queue':{'type':str, 
      'default':'pungi_composes', 
      'desc':'Name of the Celery queue for Pungi composes.'}, 
     'celery_cleanup_queue':{'type':str, 
      'default':'cleanup', 
      'desc':'Name of the Celery queue for cleanup task.'}, 
     'celery_router_config':{'type':dict, 
      'default':{'routing_rules':{'odcs.server.celery_tasks.generate_pungi_compose':{'pungi_composes': {'source_type': 3}}, 
        'odcs.server.celery_tasks.generate_pulp_compose':{'pulp_composes': {'source_type': 4}}}, 
       'cleanup_task':'odcs.server.celery_tasks.run_cleanup', 
       'default_queue':'pungi_composes'}, 
      'desc':'Configuration for custom celery router.'}, 
     'runroot_extra_mounts':{'type':list, 
      'default':[],  'desc':'Extra mountpoint directories for odcs-mock-runroot.'}}
        ...
        for role, role_dict in clients.items():
            ...
            for user, user_dict in role_dict.items():
                if type(user_dict) != dict:
                    raise TypeError("allowed_clients['%s']['%s'] is not a dict" % (role, user))
                for key, value in user_dict.items():
                    if type(value) not in [set, list]:
                        raise ValueError("allowed_clients['%s']['%s']['%s'] is not a list" % (
                         role, user, key))
        ...
        for name, url_data in raw_config_urls.items():
            ...
            for key in ('url', 'config_filename'):
                if key not in url_data:
                    raise ValueError("raw_config_urls['%s']['%s'] is not defined." % (
                     name, key))
    ...
    def _setifok_log_backend(self, s):
        if s is None:
            ...
            if s not in logger.supported_log_backends():
