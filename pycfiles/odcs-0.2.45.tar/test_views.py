...
from odcs.common.types import COMPOSE_STATES, COMPOSE_RESULTS, COMPOSE_FLAGS, MULTILIB_METHODS
...
class ViewBaseTest(ModelsBaseTest):
    ...
    def setUp(self):
        ...
        patched_allowed_clients = {'groups': {'composer': {}, 'dev2': {'source_types': [
                                              'module']}}, 
           'users': {'dev': {'arches': [
                                      'ppc64', 's390', 'x86_64']}, 
                     'dev2': {'source_types': [
...
class TestOpenIDCLogin(ViewBaseTest):
    ...
    def setUp(self):
        ...
        self.patch_auth_backend = patch.object(odcs.server.auth.conf, 'auth_backend', new='openidc')
...
class TestViews(ViewBaseTest):
    ...
    def setUp(self):
        ...
        self.oidc_base_namespace = patch.object(conf, 'oidc_base_namespace', new='http://example.com/')
    ...
    def test_submit_invalid_json(self):
        with self.test_request_context(user='dev'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data='{')
            data = json.loads(rv.get_data(as_text=True))
    ...
    def test_submit_build(self):
        with self.test_request_context(user='dev'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'module', 'source': 'testmodule:master'}}))
            data = json.loads(rv.get_data(as_text=True))
    ...
    def test_submit_build_no_packages(self):
        with self.test_request_context(user='dev'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'tag', 'source': 'f26'}, 'flags': [
                       'no_deps']}))
            data = json.loads(rv.get_data(as_text=True))
    ...
    def test_submit_build_nodeps(self):
        with self.test_request_context(user='dev'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'tag', 'source': 'f26', 'packages': ['ed']}, 'flags': [
                       'no_deps']}))
            data = json.loads(rv.get_data(as_text=True))
    ...
    def test_submit_build_boot_iso(self):
        with self.test_request_context(user='dev'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'tag', 'source': 'f26', 'packages': ['ed']}, 'results': [
                         'boot.iso']}))
            data = json.loads(rv.get_data(as_text=True))
        ...
        c = db.session.query(Compose).filter(Compose.id == 3).one()
        self.assertEqual(c.state, COMPOSE_STATES['wait'])
        self.assertEqual(c.results, COMPOSE_RESULTS['boot.iso'] | COMPOSE_RESULTS['repository'])

    def test_submit_build_sigkeys(self):
        with self.test_request_context(user='dev'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'tag', 'source': 'f26', 'packages': ['ed'], 'sigkeys': [
                                    '123', '456']}}))
            data = json.loads(rv.get_data(as_text=True))
    ...
    def test_submit_build_default_sigkeys(self, sigkeys):
        with self.test_request_context(user='dev'):
            sigkeys.return_value = [
             'x', 'y']
            flask.g.oidc_scopes = [
             ('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'tag', 'source': 'f26', 'packages': ['ed']}}))
            data = json.loads(rv.get_data(as_text=True))
    ...
    def test_submit_build_arches(self):
        with self.test_request_context(user='dev'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'tag', 'source': 'f26', 'packages': ['ed']}, 'arches': [
                        'ppc64', 's390']}))
            data = json.loads(rv.get_data(as_text=True))
    ...
    def test_submit_build_multilib_method(self):
        with self.test_request_context(user='dev'):
            ...
            data = json.loads(rv.get_data(as_text=True))
        self.assertEqual(data['multilib_method'], MULTILIB_METHODS['runtime'] | MULTILIB_METHODS['devel'])
        db.session.expire_all()
        c = db.session.query(Compose).filter(Compose.id == 1).one()
        self.assertEqual(c.state, COMPOSE_STATES['wait'])
    ...
    def test_submit_build_modular_koji_tags(self):
        with self.test_request_context(user='dev'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'tag', 'source': 'f26', 'modular_koji_tags': [
                                              'f26-modules']}}))
            data = json.loads(rv.get_data(as_text=True))
    ...
    def test_submit_build_module_defaults_url(self):
        with self.test_request_context(user='dev'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'tag', 'source': 'f26', 'module_defaults_url': 'git://localhost.tld/x.git', 
    ...
    def test_submit_build_module_defaults_url_no_branch(self):
        with self.test_request_context(user='dev'):
            ...
            self.assertEqual(data['status'], 400)
            self.assertEqual(data['error'], 'Bad Request')
            self.assertEqual(data['message'], 'The "module_defaults_url" and "module_defaults_commit" must be used together.')

    def test_submit_build_duplicate_sources(self):
        with self.test_request_context(user='dev'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'module', 'source': 'foo:x foo:x foo:y'}}))
            data = json.loads(rv.get_data(as_text=True))
    ...
    def test_submit_build_extra_builds(self):
        with self.test_request_context(user='dev'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'tag', 'source': 'f26', 'packages': ['ed'], 'builds': [
                                   'foo-1-1', 'bar-1-1']}}))
            data = json.loads(rv.get_data(as_text=True))
    ...
    def test_submit_build_source_type_build(self):
        with self.test_request_context(user='dev'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'build', 'packages': ['ed'], 'builds': [
                                   'foo-1-1', 'bar-1-1']}}))
            data = json.loads(rv.get_data(as_text=True))
        self.assertEqual(data['builds'], 'foo-1-1 bar-1-1')
        db.session.expire_all()
    ...
    def test_submit_build_resurrection_removed(self):
        ...
        db.session.commit()
        with self.test_request_context(user='dev'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'renew-compose')]
            rv = self.client.patch('/api/1/composes/1')
            data = json.loads(rv.get_data(as_text=True))
    ...
    def test_submit_build_not_allowed_source_type(self):
        with self.test_request_context(user='dev'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'repo', 'source': '/path'}}))
            data = json.loads(rv.get_data(as_text=True))
        self.assertEqual(data['message'], 'User dev not allowed to operate with compose with source_types=repo')

    def test_submit_build_unknown_source_type(self):
        with self.test_request_context(user='dev'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'unknown', 'source': '/path'}}))
            data = json.loads(rv.get_data(as_text=True))
    ...
    def test_submit_module_build_wrong_source(self):
        with self.test_request_context(user='dev2'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'module', 'source': 'testmodule:master x'}}))
            data = json.loads(rv.get_data(as_text=True))
        self.assertEqual(data['message'], 'Module definition must be in "n:s", "n:s:v" or "n:s:v:c" format, but got x')

    def test_submit_module_build_base_module_in_source(self):
        with self.test_request_context(user='dev2'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'module', 'source': 'testmodule:master platform:x'}}))
            data = json.loads(rv.get_data(as_text=True))
        self.assertEqual(data['message'], 'ODCS currently cannot create compose with base modules, but platform was requested.')
    ...
    def test_submit_build_per_user_source_type_not_allowed(self):
        with self.test_request_context(user='dev2'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'tag', 'source': '/path', 'packages': [
                                     'foo']}}))
            data = json.loads(rv.get_data(as_text=True))
        self.assertEqual(data['message'], 'User dev2 not allowed to operate with compose with source_types=tag')
    ...
    def test_submit_build_per_group_source_type_not_allowed(self):
        with self.test_request_context(user='unknown', groups=['dev2', 'x']):
            ...
            rv = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'tag', 'source': '/path', 'packages': [
                                     'foo']}}))
            data = json.loads(rv.get_data(as_text=True))
        self.assertEqual(data['message'], 'User unknown not allowed to operate with compose with source_types=tag')
    ...
    def test_query_compose_source_type(self):
        resp = self.client.get('/api/1/composes/?source_type=%d' % PungiSourceType.MODULE)
        evs = json.loads(resp.get_data(as_text=True))['items']
        self.assertEqual(len(evs), 1)
    ...
    def test_query_composes_order_by_id_unknown_key(self):
        ...
        self.assertEqual(data['status'], 400)
        self.assertEqual(data['error'], 'Bad Request')
        self.assertTrue(data['message'].startswith('An invalid order_by key was suplied, allowed keys are'))
    ...
    def test_delete_compose(self):
        with freeze_time(self.initial_datetime) as (frozen_datetime):
            ...
            with self.test_request_context(user='root'):
                flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'delete-compose')]
    ...
    def test_delete_not_allowed_states_compose(self):
        for state in COMPOSE_STATES.keys():
            if state not in ('done', 'failed'):
                ...
                self.assertEqual(data['status'], 400)
                six.assertRegex(self, data['message'], 'Compose \\(id=%s\\) can not be removed, its state need to be in .*.' % new_c.id)
                self.assertEqual(data['error'], 'Bad Request')
    ...
    def test_can_not_create_compose_with_non_composer_user(self):
        with self.test_request_context(user='qa'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            resp = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'module', 'source': 'testmodule:master'}}))
            data = json.loads(resp.get_data(as_text=True))
        self.assertEqual(resp.status, '403 FORBIDDEN')
        self.assertEqual(resp.status_code, 403)
        self.assertEqual(data['status'], 403)
    ...
    def test_can_create_compose_with_user_in_configured_groups(self):
        with self.test_request_context(user='another_user', groups=['composer']):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            resp = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'module', 'source': 'testmodule:rawhide'}}))
        db.session.expire_all()
    ...
    def test_can_delete_compose_with_user_in_configured_groups(self):
        ...
        self.assertEqual(data['status'], 202)
        six.assertRegex(self, data['message'], 'The delete request for compose \\(id=%s\\) has been accepted and will be processed by backend later.' % c3.id)
    ...
    def test_use_seconds_to_live_in_request(self, mock_seconds_to_live, mock_max_seconds_to_live):
        ...
        with self.test_request_context(user='dev'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'module', 'source': 'testmodule:master'}, 'seconds-to-live': 43200}))
            data = json.loads(rv.get_data(as_text=True))
    ...
    def test_use_max_seconds_to_live_in_conf(self, mock_seconds_to_live, mock_max_seconds_to_live):
        ...
        with self.test_request_context(user='dev'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'module', 'source': 'testmodule:master'}, 'seconds-to-live': 604800}))
            data = json.loads(rv.get_data(as_text=True))
        time_submitted = datetime.strptime(data['time_submitted'], '%Y-%m-%dT%H:%M:%SZ')
        time_to_expire = datetime.strptime(data['time_to_expire'], '%Y-%m-%dT%H:%M:%SZ')
...
class TestExtendExpiration(ViewBaseTest):
    ...
    def test_bad_request_if_seconds_to_live_is_invalid(self):
        post_data = json.dumps({'seconds-to-live': '600s'})
        with self.test_request_context():
            rv = self.client.patch(('/api/1/composes/{0}').format(self.c1.id), data=post_data)
            data = json.loads(rv.get_data(as_text=True))
            self.assertEqual(400, data['status'])
            self.assertEqual('Bad Request', data['error'])
            self.assertIn('Invalid seconds-to-live specified in request', data['message'])
    ...
    def test_fail_if_extend_non_existing_compose(self):
        post_data = json.dumps({'seconds-to-live': 600})
        with self.test_request_context():
            flask.g.oidc_scopes = [
    ...
    def test_extend_compose_expiration(self):
        ...
        post_data = json.dumps({'seconds-to-live': expected_seconds_to_live})
        with self.test_request_context():
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'renew-compose')]
...
class TestViewsRawConfig(ViewBaseTest):
    ...
    def test_submit_build_raw_config_too_many_sources(self):
        with self.test_request_context(user='dev2'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'raw_config', 'source': 'pungi_cfg#hash pungi2cfg_hash'}}))
            data = json.loads(rv.get_data(as_text=True))
        self.assertEqual(data['status'], 400)
        self.assertEqual(data['message'], 'Only single source is allowed for "raw_config" source_type')

    def test_submit_build_raw_config_no_hash(self):
        with self.test_request_context(user='dev2'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'raw_config', 'source': 'pungi_cfg'}}))
            data = json.loads(rv.get_data(as_text=True))
        self.assertEqual(data['status'], 400)
        self.assertEqual(data['message'], 'Source must be in "source_name#commit_hash" format for "raw_config" source_type.')
    ...
    def test_submit_build_raw_config_empty_name(self):
        with self.test_request_context(user='dev2'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'raw_config', 'source': '#hash'}}))
            data = json.loads(rv.get_data(as_text=True))
        self.assertEqual(data['status'], 400)
        self.assertEqual(data['message'], 'Source must be in "source_name#commit_hash" format for "raw_config" source_type.')
    ...
    def test_submit_build_raw_config_empty_name_hash(self):
        with self.test_request_context(user='dev2'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'raw_config', 'source': '#'}}))
            data = json.loads(rv.get_data(as_text=True))
        self.assertEqual(data['status'], 400)
        self.assertEqual(data['message'], 'Source must be in "source_name#commit_hash" format for "raw_config" source_type.')
    ...
    def test_submit_build_raw_config_unknown_name(self):
        with self.test_request_context(user='dev2'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'raw_config', 'source': 'pungi_cfg#hash'}}))
            data = json.loads(rv.get_data(as_text=True))
        self.assertEqual(data['status'], 400)
        self.assertEqual(data['message'], 'Source "pungi_cfg" does not exist in server configuration.')

    @patch.object(odcs.server.config.Config, 'raw_config_urls', new={'pungi_cfg': 'http://localhost/pungi.conf#%s'})
    def test_submit_build_raw_config(self):
        with self.test_request_context(user='dev2'):
            flask.g.oidc_scopes = [('{0}{1}').format(conf.oidc_base_namespace, 'new-compose')]
            self.client.post('/api/1/composes/', data=json.dumps({'source': {'type': 'raw_config', 'source': 'pungi_cfg#hash'}}))
        db.session.expire_all()
        c = db.session.query(Compose).filter(Compose.id == 1).one()
        self.assertEqual(c.state, COMPOSE_STATES['wait'])
