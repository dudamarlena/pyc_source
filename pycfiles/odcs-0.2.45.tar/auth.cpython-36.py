...
# Size of source mod 2**32: 11251 bytes
...
def query_ldap_groups(uid):
    ...
    groups = client.search_s((conf.auth_ldap_group_base), (ldap.SCOPE_ONELEVEL),
      attrlist=[
     'cn', 'gidNumber'],
...
def raise_if_source_type_not_allowed(source_type):
    ...
    for group in conf.allowed_clients.get('groups', []):
        ...
        if group_name in flask.g.groups:
    ...
    for user in conf.allowed_clients.get('users', []):
        if type(user) == dict:
            ...
            user_name = user
            source_types = conf.allowed_source_types
        if user_name == flask.g.user.username:
            if source_type in source_types:
                return

    raise Forbidden('User %s not allowed to operate with compose of %s source_type' % (
     flask.g.user.username, source_type))
