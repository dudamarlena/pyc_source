...
# [GCC 8.4.0]
# Embedded file name: /home/hanzz/releases/odcs/server/tests/test_remove_expired_composes_thread.py
# Compiled at: 2018-08-16 00:37:29
...
class TestRemoveExpiredComposesThread(ModelsBaseTest):
    ...
    def test_remove_left_composes(self, remove_compose_dir, glob, isdir):
        ...
        self.thread.do_work()
        self.assertEqual(remove_compose_dir.call_args_list, [
    ...
    @patch('glob.glob')
    @patch('odcs.server.backend.RemoveExpiredComposesThread._remove_compose_dir')
    def test_remove_left_composes_expired_compose(self, remove_compose_dir, glob, isdir):
        isdir.return_value = True
        self._mock_glob(glob, ['latest-odcs-1-1', 'odcs-1-1-2017.n.0'])
        c = db.session.query(Compose).filter(Compose.id == 1).one()
