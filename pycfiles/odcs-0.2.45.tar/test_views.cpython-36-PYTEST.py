...
# Size of source mod 2**32: 45238 bytes
...
class ViewBaseTest(ModelsBaseTest):
    ...
    def setUp(self):
        ...
        patched_allowed_clients = {'groups':[
          'composer', {'dev2': ['module']}], 
         'users':[
          'dev',
          {'dev2': [
                    'module', 'raw_config']}]}
        patched_admins = {'groups':['admin'],  'users':['root']}
        self.patch_allowed_clients = patch.object((odcs.server.auth.conf), 'allowed_clients',
          new=patched_allowed_clients)
        self.patch_admins = patch.object((odcs.server.auth.conf), 'admins',
          new=patched_admins)
    ...
    def test_request_context(self, user=None, groups=None, **kwargs):
        with (app.test_request_context)(**kwargs):
            ...
            if user is not None:
                patch_auth_backend = patch.object((odcs.server.auth.conf), 'auth_backend',
                  new='kerberos')
...
class TestOpenIDCLogin(ViewBaseTest):
    __doc__ = 'Test that OpenIDC login'

    def setUp(self):
        super(TestOpenIDCLogin, self).setUp()
        self.patch_auth_backend = patch.object((odcs.server.auth.conf),
          'auth_backend', new='openidc')
...
class TestHandlingErrors(ViewBaseTest):
    __doc__ = 'Test registered error handlers'
...
class TestViews(ViewBaseTest):
    ...
    def setup_test_data(self):
        self.initial_datetime = datetime(year=2016, month=1, day=1, hour=0,
          minute=0,
          second=0)
    ...
    def test_submit_build_nodeps(self):
        with self.test_request_context(user='dev'):
            flask.g.oidc_scopes = ['{0}{1}'.format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=(json.dumps({'source':{'type':'tag', 
              'source':'f26',  'packages':['ed']}, 
             'flags':[
              'no_deps']})))
            data = json.loads(rv.get_data(as_text=True))
    ...
    def test_submit_build_not_allowed_source_type(self):
        with self.test_request_context(user='dev'):
            ...
            data = json.loads(rv.get_data(as_text=True))
        self.assertEqual(data['message'], 'User dev not allowed to operate with compose of repo source_type')
    ...
    def test_submit_build_per_user_source_type_allowed(self):
        with self.test_request_context(user='dev2'):
            flask.g.oidc_scopes = ['{0}{1}'.format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=(json.dumps({'source': {'type':'module',  'source':'/path'}})))
            data = json.loads(rv.get_data(as_text=True))
        self.assertEqual(data['state_name'], 'wait')
    ...
    def test_submit_build_per_user_source_type_not_allowed(self):
        with self.test_request_context(user='dev2'):
            flask.g.oidc_scopes = ['{0}{1}'.format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=(json.dumps({'source': {'type':'tag',  'source':'/path'}})))
            data = json.loads(rv.get_data(as_text=True))
        self.assertEqual(data['message'], 'User dev2 not allowed to operate with compose of tag source_type')
    ...
    def test_submit_build_per_group_source_type_not_allowed(self):
        with self.test_request_context(user='unknown', groups=['dev2', 'x']):
            ...
            rv = self.client.post('/api/1/composes/', data=(json.dumps({'source': {'type':'tag',  'source':'/path'}})))
            data = json.loads(rv.get_data(as_text=True))
        self.assertEqual(data['message'], 'User unknown not allowed to operate with compose of tag source_type')
    ...
    def test_use_seconds_to_live_in_request(self, mock_seconds_to_live, mock_max_seconds_to_live):
        ...
        with self.test_request_context(user='dev'):
            flask.g.oidc_scopes = ['{0}{1}'.format(conf.oidc_base_namespace, 'new-compose')]
            rv = self.client.post('/api/1/composes/', data=(json.dumps({'source':{'type':'module', 
              'source':'testmodule-master'}, 
             'seconds-to-live':43200})))
            data = json.loads(rv.get_data(as_text=True))
