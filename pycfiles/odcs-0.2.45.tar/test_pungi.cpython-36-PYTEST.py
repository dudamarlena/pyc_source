...
# Size of source mod 2**32: 12765 bytes
import builtins as @py_builtins, _pytest.assertion.rewrite as @pytest_ar, os, shutil, tempfile, unittest, koji
from mock import patch, MagicMock, call
from kobo.conf import PyConfigParser
from odcs.server.pungi import Pungi, PungiConfig, PungiSourceType, COMPOSE_RESULTS
...
class TestPungiConfig(unittest.TestCase):
    ...
    def test_pungi_config_tag(self):
        pungi_cfg = PungiConfig('MBS-512', '1', (PungiSourceType.KOJI_TAG), 'f26',
          packages=['file'], sigkeys='123 456', arches=[
         'ppc64', 's390'])
...
class TestPungi(unittest.TestCase):
    ...
    def test_pungi_run_raw_config(self, execute_cmd):
        ...
        self.download_file.assert_called_once_with('http://localhost/pungi.conf#hash', AnyStringWith('/raw_config.conf'))
...
class TestPungiRunroot(unittest.TestCase):
    ...
    def test_pungi_run_runroot(self):
        ...
        pungi.run(self.compose)
        conf_topdir = os.path.join(conf.target_dir, 'odcs/unique_path')
        self.koji_session.uploadWrapper.assert_any_call((os.path.join(conf_topdir, 'pungi.conf')),
          'odcs/unique_path', callback=None)
        self.koji_session.uploadWrapper.assert_any_call((os.path.join(conf_topdir, 'variants.xml')),
          'odcs/unique_path', callback=None)
        self.koji_session.uploadWrapper.assert_any_call((os.path.join(conf_topdir, 'comps.xml')),
          'odcs/unique_path', callback=None)
        self.koji_session.runroot.assert_called_once_with('f26-build',
          'x86_64', 'cp /mnt/koji/work/odcs/unique_path/* . && cp ./odcs_koji.conf /etc/koji.conf.d/ && pungi-koji --config=./pungi.conf --target-dir=/mnt/koji/compose/odcs --nightly',
          channel='channel',
          mounts=['/mnt/odcs-secrets'],
          packages=['pungi'],
          weight=3.5)
    ...
    def test_pungi_run_runroot_raw_config(self):
        ...
        self.koji_session.uploadWrapper.assert_has_calls([
         call((os.path.join(conf_topdir, 'odcs_koji.conf')), 'odcs/unique_path',
           callback=None),
         call((os.path.join(conf_topdir, 'pungi.conf')), 'odcs/unique_path',
           callback=None),
         call((os.path.join(conf_topdir, 'raw_config.conf')), 'odcs/unique_path',
           callback=None)])
        self.koji_session.runroot.assert_called_once_with('f26-build',
          'x86_64', 'cp /mnt/koji/work/odcs/unique_path/* . && cp ./odcs_koji.conf /etc/koji.conf.d/ && pungi-koji --config=./pungi.conf --target-dir=/mnt/koji/compose/odcs --nightly',
          channel='channel',
