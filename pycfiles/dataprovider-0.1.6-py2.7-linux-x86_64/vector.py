...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dataprovider/vector.py
...
class Vec3d(object):
    ...
    def _o2(self, v, f):
        ...
        if isinstance(v, Vec3d):
            return Vec3d(f(self.x, v.x), f(self.y, v.y), f(self.z, v.z))
        else:
            if hasattr(v, '__getitem__'):
                return Vec3d(f(self.x, v[0]), f(self.y, v[1]), f(self.z, v[2]))
            return Vec3d(f(self.x, v), f(self.y, v), f(self.z, v))

    def _r_o2(self, v, f):
        """Any two-operator operation where the right operand is a Vec3d"""
        if hasattr(v, '__getitem__'):
            return Vec3d(f(v[0], self.x), f(v[1], self.y), f(v[2], self.z))
        else:
            return Vec3d(f(v, self.x), f(v, self.y), f(v, self.z))
    ...
    def get_distance(self, v):
        return math.sqrt((self.x - v[0]) ** 2 + (self.y - v[1]) ** 2 + (self.z - v[2]) ** 2)
    ...
    def cross(self, v):
        return Vec3d(self.y * v[2] - self.z * v[1], self.z * v[0] - self.x * v[2], self.x * v[1] - self.y * v[0])

    def interpolate_to(self, v, range):
        return Vec3d(self.x + (v[0] - self.x) * range, self.y + (v[1] - self.y) * range, self.z + (v[2] - self.z) * range)

    def convert_to_basis(self, x_vector, y_vector, z_vector):
        return Vec3d(self.dot(x_vector) / x_vector.get_length_sqrd(), self.dot(y_vector) / y_vector.get_length_sqrd(), self.dot(z_vector) / z_vector.get_length_sqrd())
