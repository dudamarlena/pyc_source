...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dataprovider/box.py
...
class Box(object):
    ...
    def contains(self, v):
        ...
        x, y, z = Vec3d(v)
        return self._min.x <= x < self._max.x and self._min.y <= y < self._max.y and self._min.z <= z < self._max.z

    def overlaps(self, other):
        """Return true if a box overlaps this box."""
        return self._max.x > other._min.x and self._min.x < other._max.x and self._max.y > other._min.y and self._min.y < other._max.y and self._max.z > other._min.z and self._min.z < other._max.z
    ...
    def expand_by(self, v):
        ...
        self._min -= v
        self._max += v
        assert self._min.x < self._max.x and self._min.y < self._max.y and self._min.z < self._max.z
