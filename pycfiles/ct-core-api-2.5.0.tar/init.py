...
# Embedded file name: build/bdist.linux-x86_64/egg/cantools/scripts/init.py
# Compiled at: 2020-03-23 21:11:40
...
class Builder(object):
    ...
    def __init__(self, pname=None, cantools_path=CTP, web_backend='dez', refresh_symlinks=False):
        ...
        if not os.path.exists(self.ctroot):
            error('failed to establish cantools root', 'We tried: %s' % (self.ctroot,), 'This directory does not contain cantools.', 'You can download cantools at https://github.com/bubbleboy14/cantools', 'Please pass the path to your local copy of cantools via the -c flag.')
    ...
    def _getplug(self, plugin):
        ...
        if hasattr(init, 'model'):
            ...
            self._update(config.init, 'model', [ 'from %s import %s' % (m, (', ').join(k)) for m, k in list(init.model.items())
                                               ])
        if hasattr(init, 'routes'):
            log('adding %s routes to app.yaml' % (len(init.routes),), 3)
            self._update(config.init.yaml, 'core', [ '- url: %s\r\n  %s: %s' % (u, s.endswith('.py') and 'script' or 'static_dir', s) for u, s in list(init.routes.items())
                                                   ])
    ...
    def make_files(self):
        ...
        cp(self.web_backend == 'gae' and '%s\r\n%s' % (config.init.yaml.gae, config.init.yaml.core) % (self.pname,) or config.init.yaml.core, 'app.yaml')
        ...
        for plugin, mod in list(self.plugins.items()):
            if hasattr(mod.init, 'templates'):
                ...
                cp(read(os.path.join(mod.__ct_mod_path__, mod.init.templates)), os.path.join('emails', mod.init.templates))
                ...
                for dname, fnames in list(mod.init.copies.items()):
                    ...
                    if fnames == '*':
                        ...
                        for fname in fnames:
                            ...
                            cp(read(os.path.join(dp, fname)), os.path.join(dname, fname))
        ...
        for mod in list(self.plugins.values()):
            if hasattr(mod.init, 'syms'):
                for dname, fnames in list(mod.init.syms.items()):
                    for fname in fnames:
                        sym(os.path.join(mod.__ct_mod_path__, dname, fname), os.path.join(dname, fname))
...
def parse_and_make():
    ...
    parser.add_option('-p', '--plugins', dest='plugins', default='', help='which plugins would you like to use in your project?')
    parser.add_option('-c', '--cantools_path', dest='cantools_path', default=CTP, help='where is cantools? (default: %s)' % (CTP,))
    parser.add_option('-w', '--web_backend', dest='web_backend', default='dez', help='web backend. options: dez, gae. (default: dez)')
    parser.add_option('-r', '--refresh_symlinks', action='store_true', dest='refresh_symlinks', default=False, help='add symlinks to project and configure version control path exclusion (if desired)')
    parser.add_option('-u', '--update', action='store_true', dest='update', default=False, help='update cantools and all managed plugins')
    parser.add_option('-a', '--admin', action='store_true', dest='admin', default=False, help='compile admin pages [ctdev only]')
    ...
    if options.update:
        ...
        Builder(len(args) and args[0], options.cantools_path, options.web_backend, options.refresh_symlinks)
    log('done! goodbye.')
