...
# Embedded file name: build/bdist.linux-x86_64/egg/cantools/scripts/index.py
# Compiled at: 2019-08-05 00:45:08
...
def go():
    ...
    parser.add_option('-m', '--mode', dest='mode', default='refcount', help="may be: 'refcount' (default - count up all foreignkey references for sort orders and such); 'index' (assign each record a sequential integer index); 'urlsafekeys' (update all key/keylist properties to use urlsafe keys introduced in ct 0.8); 'cleanup' (delete zero-count reference counters). Note regarding 'index' mode: it _must_ happen remotely; it's generally unnecessary unless you're trying to migrate an unindexed database away from gae and need an index/key per record; it should be invoked from _outside_ -- that's right, outside -- of your project's directory (to avoid loading up a bunch of google network tools that may be crappy or cause issues outside of their normal 'dev_appserver' environment)")
    parser.add_option('-d', '--domain', dest='domain', default='localhost', help="('index' mode only) what's the domain of the target server? (default: localhost)")
    parser.add_option('-p', '--port', dest='port', default='8080', help="('index' mode only) what's the port of the target server? (default: 8080)")
    parser.add_option('-s', '--skip', dest='skip', default='', help="skip these tables ('index' mode only) - use '|' as separator, such as 'table1|table2|table3' (default: none)")
    parser.add_option('-i', '--index', dest='index', default=0, help="start with this index ('index' mode only) (default: 0)")
    options, args = parser.parse_args()
    log('mode: %s' % (options.mode,), important=True)
    if options.mode == 'refcount':
        refcount()
    elif options.mode == 'index':
        index(options.domain, int(options.port), options.skip and options.skip.split('|') or [], options.index)
