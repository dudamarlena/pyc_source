...
# Embedded file name: build/bdist.linux-x86_64/egg/cantools/config.py
# Compiled at: 2019-11-20 19:43:16
import json, getpass
...
for key, val in items:
    if key in ('ENCODE', 'DB_ECHO', 'DB_PUBLIC', 'GEO_TEST', 'MEMCACHE_REQUEST', 'MEMCACHE_DB',
               'PUBSUB_ECHO', 'PUBSUB_META', 'PUBSUB_B64', 'SSL_VERIFY', 'ADMIN_MONITOR_LOG',
               'WEB_XORIGIN'):
        val = val == 'True'
    ...
    if key == 'DB':
        ...
        if target in ('pubsub_botnames', 'log_allow', 'geo_user_geonames', 'geo_user_google',
                      'admin_contacts', 'web_rollz', 'admin_monitor_geo', 'build_exclude',
                      'build_include', 'web_whitelist', 'admin_whitelist'):
            val = val.split('|')
            if target == 'web_rollz':
                rollz = {}
                for v in val:
