...
# Embedded file name: /home/erwincoumans/dev/tmp/bullet3/examples/pybullet/gym/pybullet_envs/bullet/minitaur_gym_env.py
# Compiled at: 2020-04-29 23:02:34
# Size of source mod 2**32: 16337 bytes
__doc__ = 'This file implements the gym environment of minitaur.\n\n'
import os, inspect
currentdir = os.path.dirname(os.path.abspath(inspect.getfile(inspect.currentframe())))
parentdir = os.path.dirname(os.path.dirname(currentdir))
os.sys.path.insert(0, parentdir)
import math, time, gym
from gym import spaces
from gym.utils import seeding
import numpy as np, pybullet, pybullet_utils.bullet_client as bc
...
class MinitaurBulletEnv(gym.Env):
    ...
    def reset(self):
        if self._hard_reset:
            ...
            self.minitaur = minitaur.Minitaur(pybullet_client=(self._pybullet_client), urdf_root=(self._urdf_root),
    ...
    """
        ...
        return (
         np.array(self._noisy_observation()), reward, done, {})
    ...
    def render(self, mode='rgb_array', close=False):
        if mode != 'rgb_array':
            ...
            view_matrix = self._pybullet_client.computeViewMatrixFromYawPitchRoll(cameraTargetPosition=base_pos,
              ...
              roll=0,
              upAxisIndex=2)
            proj_matrix = self._pybullet_client.computeProjectionMatrixFOV(fov=60, aspect=(float(RENDER_WIDTH) / RENDER_HEIGHT),
              nearVal=0.1,
              farVal=100.0)
            _, _, px, _, _ = self._pybullet_client.getCameraImage(width=RENDER_WIDTH, height=RENDER_HEIGHT,
    ...
    def _reward(self):
        ...
        energy_reward = np.abs(np.dot(self.minitaur.GetMotorTorques(), self.minitaur.GetMotorVelocities())) * self._time_step
        reward = self._distance_weight * forward_reward - self._energy_weight * energy_reward + self._drift_weight * drift_reward + self._shake_weight * shake_reward
    ...
    def _noisy_observation(self):
        ...
        if self._observation_noise_stdev > 0:
            observation += np.random.normal(scale=(self._observation_noise_stdev), size=(observation.shape)) * self.minitaur.GetObservationUpperBound()
        return observation
