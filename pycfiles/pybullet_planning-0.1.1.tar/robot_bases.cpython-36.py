...
# Embedded file name: /home/erwincoumans/dev/tmp/bullet3/examples/pybullet/gym/pybullet_envs/robot_bases.py
# Compiled at: 2020-04-29 23:02:34
# Size of source mod 2**32: 13851 bytes
import pybullet, gym, gym.spaces, gym.utils, numpy as np, os, inspect
    ...
    def addToScene(self, bullet_client, bodies):
        ...
        for i in range(len(bodies)):
            ...
            for j in range(self._p.getNumJoints(bodies[i])):
                self._p.setJointMotorControl2((bodies[i]), j,
        ...
        return (
         parts, joints, ordered_joints, self.robot_body)
...
class URDFBasedRobot(XmlBasedRobot):
    ...
    def __init__(self, model_urdf, robot_name, action_dim, obs_dim, basePosition=[
 0, 0, 0], baseOrientation=[
 0, 0, 0, 1], fixed_base=False, self_collision=False):
        XmlBasedRobot.__init__(self, robot_name, action_dim, obs_dim, self_collision)
    ...
    def reset(self, bullet_client):
        ...
        if self.self_collision:
            self.parts, self.jdict, self.ordered_joints, self.robot_body = self.addToScene(self._p, self._p.loadURDF((os.path.join(pybullet_data.getDataPath(), self.model_urdf)), basePosition=(self.basePosition),
...
class SDFBasedRobot(XmlBasedRobot):
    ...
    def __init__(self, model_sdf, robot_name, action_dim, obs_dim, basePosition=[
 0, 0, 0], baseOrientation=[
 0, 0, 0, 1], fixed_base=False, self_collision=False):
        XmlBasedRobot.__init__(self, robot_name, action_dim, obs_dim, self_collision)
    ...
    def reset(self, bullet_client):
        self._p = bullet_client
        self.ordered_joints = []
        self.parts, self.jdict, self.ordered_joints, self.robot_body = self.addToScene(self._p, self._p.loadSDF(os.path.join('models_robot', self.model_sdf)))
        self.robot_specific_reset(self._p)
        s = self.calc_state()
        self.potential = self.calc_potential()
        return s
...
class BodyPart:
    ...
    def speed(self):
        if self.bodyPartIndex == -1:
            ...
            (x, y, z), (a, b, c, d), _, _, _, _, (vx, vy, vz), (vr, vp, vy) = self._p.getLinkState((self.bodies[self.bodyIndex]),
              (self.bodyPartIndex), computeLinkVelocity=1)
    ...
    def reset_velocity(self, linearVelocity=[
 0, 0, 0], angularVelocity=[0, 0, 0]):
...
class Joint:
    ...
    def current_relative_position(self):
        ...
        return (
         2 * (pos - pos_mid) / (self.upperLimit - self.lowerLimit), 0.1 * vel)
