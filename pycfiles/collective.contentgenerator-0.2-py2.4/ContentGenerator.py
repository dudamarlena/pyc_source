...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/collective/contentgenerator/ContentGenerator.py
...
class ContentSetup:
    ...
    feeds = ['front_page', 'world', 'uk', 'business', 'uk_politics', 'health', 'education', 'sci/tech', 'technology', 'entertainment', 'talking_point', 'magazine']
    ...
    def customize(self, portal, context, settings={}, logging='log'):
        ...
        if self.checkZEXP(profile, sectioncount):
            self.addZEXPContent(portal, sectioncount=sectioncount, profile=profile)
        else:
            self.leechSomeData(settings=settings)
            self.addContent(portal, sectioncount=sectioncount, foldercount=foldercount, itemcount=settings.get('itemcount', 10), imagecount=settings.get('imagecount', 5), bigimagecount=settings.get('bigimagecount', 5), profile=profile)
            self.exportZEXPContent(portal, sectioncount=sectioncount, profile=profile)
        self.cloneContent(portal, multiply=settings.get('multiply', 2), sectioncount=sectioncount, foldercount=foldercount, profile=profile)
        if logging == 'debug':
            error = ''
    ...
    def leechSomeData(self, settings):
        ...
        try:
            ...
            err = 'Sorry the Content Generator uses crawled data \\\n                    from the internet. \\\n                    A web connection is required when a profile is first used'
            raise Exception, err

        self.somedata = {'shorttexts': [], 'mediumtexts': [], 'longtexts': [], 'urls': [], 'images': [], 'bigimages': []}
        itemtally = 0
        imagetally = 0
            ...
            for n in items:
                ...
                if 'link' in itemdata:
                    ...
                    if 'description' in itemdata:
                        self.somedata['mediumtexts'].append(itemdata['description'])
                    if 'link' in itemdata:
        ...
        for category in self.ftags:
            ...
            for n in items:
                ...
                if len(els) and getattr(els[0], 'firstChild'):
                    ...
                    title = ''
                for link in links:
                    self.crawlImage(imgurl=link.getAttribute('url'), imagekey='images', title=title)

                for link in biglinks:
                    self.crawlImage(imgurl=link.getAttribute('url'), imagekey='bigimages', title=title)
    ...
    def addContent(self, portal, sectioncount=3, foldercount=10, itemcount=10, imagecount=5, bigimagecount=5, profile='intranet'):
        """ Populate the site with folders and content """
        wftool = getToolByName(portal, 'portal_workflow')
        supported_types = ['News Item', 'Document']
        topfolders = [ x for x in portal.contentValues() if x.getId() != 'Members' if x.isPrincipiaFolderish ]
        ...
        for i in range(1, sectioncount + 1):
            ...
            for j in range(1, foldercount + 1):
                ...
                sf.reindexObject()
                types = [ x for x in sf.getAllowedTypes() if x.getId() in supported_types ]
                if not types:
                    continue
                ...
                for k in range(1, imagecount + 1):
                    self.createImage(folder=sf, imagekey='images', title='image' + str(j) + str(k))

                for k in range(1, bigimagecount + 1):
                    self.createImage(folder=sf, imagekey='bigimages', title='image' + str(j) + str(k))
    ...
    def cloneContent(self, portal, multiply=2, sectioncount=3, foldercount=5, profile='intranet'):
