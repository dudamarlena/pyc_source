...
# Embedded file name: build/bdist.macosx-10.5-i386/egg/alchemist/security/role.py
...
class LocalPrincipalRoleMap(object):
    ...
    def getPrincipalsForRole(self, role_id):
        ...
        """
        s = select([prm.c.principal_id, prm.c.setting], and_(prm.c.role_id == role_id, prm.c.object_type == self.object_type, prm.c.object_id == self.oid))
        for o in s.execute():
    ...
    def getRolesForPrincipal(self, principal_id):
        ...
        """
        s = select([prm.c.role_id, prm.c.setting], and_(prm.c.principal_id == principal_id, prm.c.object_type == self.object_type, prm.c.object_id == self.oid))
        for o in s.execute():
            yield (o[0], BooleanAsSetting[o[1]])
    ...
    def getSetting(self, role_id, principal_id):
        ...
        """
        s = select([prm.c.setting], and_(prm.c.principal_id == principal_id, prm.c.role_id == role_id, prm.c.object_type == self.object_type, prm.c.object_id == self.oid))
    ...
    def getPrincipalsAndRoles(self):
        ...
        """
        s = select([prm.c.role_id, prm.c.principal_id, prm.c.setting], and_(prm.c.object_type == self.object_type, prm.c.object_id == self.oid))
        for (role_id, principal_id, setting) in s.execute():
            yield (role_id, principal_id, BooleanAsSetting[setting])

    def assignRoleToPrincipal(self, role_id, principal_id):
        prm.insert(values=dict(role_id=role_id, principal_id=principal_id, object_id=self.oid, object_type=self.object_type)).execute()

    def removeRoleFromPrincipal(self, role_id, principal_id):
        s = select([prm.c.role_id, prm.c.setting], and_(prm.c.principal_id == principal_id, prm.c.object_type == self.object_type, prm.c.role_id == role_id, prm.c.object_id == self.oid))
        if s.execute().fetchone():
            self.unsetRoleForPrincipal(role_id, principal_id)
        prm.insert(values=dict(role_id=role_id, principal_id=principal_id, setting=False, object_id=self.oid, object_type=self.object_type)).execute()

    def unsetRoleForPrincipal(self, role_id, principal_id):
        prm.delete(and_(prm.c.role_id == role_id, prm.c.principal_id == principal_id, prm.c.object_type == self.object_type, prm.c.object_id == self.oid)).execute()
