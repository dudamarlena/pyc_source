...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/waferslim/specs/protocol_spec.py
# Compiled at: 2010-03-10 21:01:51
"""
...
class UnpackBehaviour(object):
    ...
    def require_square_brackets(self):
        ...
        spec.unpack('[hello').should_raise(UnpackingError("'[hello' has no trailing ']'"))
        spec.unpack('hello]').should_raise(UnpackingError("'hello]' has no leading '['"))

    @lancelot.verifiable
    def require_separators(self):
        """ Unpacking a string without separators should raise an error """
        spec = lancelot.Spec(unpack)
        spec.unpack('[000000]').should_raise(UnpackingError("'[000000]' has no ':' separator at pos 7"))
        spec.unpack('[000001:000000]').should_raise(UnpackingError("'[000001:000000]' has no ':' separator at pos 14"))
...
def request_responder_behaviour():
    ...
    spec.should_collaborate_with(request.send(('Slim -- V0.1\n').encode('utf-8')).will_return(2), request.recv(7).will_return(('000009:').encode('utf-8')), request.recv(1024).will_return(('[000000:]').encode('utf-8')), instructions.execute(Type(ExecutionContext), Type(Results)), request.send(('000009:[000000:]').encode('utf-8')).will_return(4), request.recv(7).will_return(('000003:').encode('utf-8')), request.recv(1024).will_return(('bye').encode('utf-8')), and_result=(
     26, 6))
    request = lancelot.MockSpec(name='request')
    instructions = lancelot.MockSpec(name='instructions')
    request_responder = RequestResponder()
    ...
    spec = lancelot.Spec(request_responder)
    spec.respond_to_request(instructions=lambda data: instructions)
    spec.should_collaborate_with(request.send(('Slim -- V0.1\n').encode('utf-8')).will_return(2), request.recv(7).will_return(('000009:').encode('utf-8')), request.recv(1024).will_return(('[000000:]').encode('utf-8')), instructions.execute(Type(ExecutionContext), Type(Results)), request.send(('000009:[000000:]').encode('utf-8')).will_return(4), request.recv(7).will_return(('000009:').encode('utf-8')), request.recv(1024).will_return(('[000000:]').encode('utf-8')), instructions.execute(Type(ExecutionContext), Type(Results)), request.send(('000009:[000000:]').encode('utf-8')).will_return(8), request.recv(7).will_return(('000003:').encode('utf-8')), request.recv(1024).will_return(('bye').encode('utf-8')), and_result=(
     42, 14))
...
def responder_handles_errors():
    ...
    spec = lancelot.Spec(request_responder)
    spec.respond_to_request(instructions=lambda data: instructions)
    unpacking_error = '000109:[000001:000092:' + '[000002:000014:UnpackingError:000053:' + '__EXCEPTION__: message:<<MALFORMED_INSTRUCTION mint>>:]:]'
    spec.should_collaborate_with(request.send(('Slim -- V0.1\n').encode('utf-8')).will_return(2), request.recv(7).will_return(('000009:').encode('utf-8')), request.recv(1024).will_return(('[000000:]').encode('utf-8')), instructions.execute(Anything(), Anything()).will_raise(UnpackingError('mint')), request.send(unpacking_error.encode('utf-8')).will_return(6), request.recv(7).will_return(('000003:').encode('utf-8')), request.recv(1024).will_return(('bye').encode('utf-8')), and_result=(
     26, 8))
