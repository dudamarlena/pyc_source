...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/waferslim/specs/instruction_spec.py
# Compiled at: 2010-02-27 12:48:09
"""
...
from waferslim.instructions import Instruction, Make, Import, Call, CallAndAssign
from waferslim.execution import ParamsConverter
from waferslim.specs.spec_classes import ClassWithNoArgs, ClassWithOneArg, ClassWithTwoArgs, ClassWithSystemUnderTestMethod, ClassWithSystemUnderTestField
...
class BaseInstructionBehaviour(object):
    ...
    def execute_fails(self):
        ...
        spec.execute(execution_context, results).should_collaborate_with(results.failed(instruction, 'INVALID_STATEMENT nonsense'))
...
def make_creates_instance():
    ...
    classes = [
     ClassWithNoArgs, ClassWithOneArg, ClassWithTwoArgs]
    ...
    for i in range(0, 3):
        ...
        spec.execute(execution_context, results).should_collaborate_with(execution_context.get_type(params[1]).will_return(target), execution_context.to_args(params, 2).will_return(args), execution_context.store_instance(name, Type(target)), results.completed(make_instruction))
...
class MakeExceptionBehaviour(object):
    ...
    def handles_wrong_args(self):
        ...
        cause = 'COULD_NOT_INVOKE_CONSTRUCTOR FakeClass ' + 'object.__new__() takes no parameters'
        execution_context = lancelot.MockSpec(name='execution_context')
        results = lancelot.MockSpec(name='results')
        a_class = ClassWithNoArgs
        make_instruction = Make('wrong params', wrong_params)
        spec = lancelot.Spec(make_instruction)
        spec.execute(execution_context, results).should_collaborate_with(execution_context.get_type('FakeClass').will_return(a_class), execution_context.to_args(wrong_params, 2).will_return(wrong_args), results.failed(make_instruction, cause))
    ...
    def handles_bad_type(self):
        ...
        make_instruction = Make('wrong params', wrong_params)
        spec = lancelot.Spec(make_instruction)
        spec.execute(execution_context, results).should_collaborate_with(execution_context.get_type('FakeClass').will_raise(type_error), results.failed(make_instruction, cause))
    ...
    def handles_bad_import(self):
        ...
        make_instruction = Make('wrong params', wrong_params)
        spec = lancelot.Spec(make_instruction)
        spec.execute(execution_context, results).should_collaborate_with(execution_context.get_type('FakeClass').will_raise(import_error), results.failed(make_instruction, cause))
...
def call_invokes_method():
    ...
    for target in methods.keys():
        ...
        spec.execute(execution_context, results).should_collaborate_with(execution_context.get_instance(params[0]).will_return(instance), execution_context.target_for(instance, params[1]).will_return(lambda life: 'meaning'), execution_context.to_args(params, 2).will_return(('life', )), results.completed(call_instruction, 'meaning'))
...
class CallExceptionBehaviour(object):
    ...
    def handles_bad_instance(self):
        ...
        spec = lancelot.Spec(call_instruction)
        spec.execute(execution_context, results).should_collaborate_with(execution_context.get_instance(params[0]).will_return(None), execution_context.get_library_method(params[1]).will_return(None), results.failed(call_instruction, cause))
        return
...
def import_adds_to_pythonpath():
    ...
    spec.execute(execution_context, results).should_collaborate_with(execution_context.add_import_path('/some_path'), results.completed(import_instruction))
    execution_context = lancelot.MockSpec('execution_context')
    results = lancelot.MockSpec('results')
    import_instruction = Import('id', ['c:\\some_path'])
    spec = lancelot.Spec(import_instruction)
    spec.execute(execution_context, results).should_collaborate_with(execution_context.add_import_path('c:\\some_path'), results.completed(import_instruction))
...
def import_adds_to_type_context():
    ...
    spec = lancelot.Spec(import_instruction)
    spec.execute(execution_context, results).should_collaborate_with(execution_context.add_type_prefix('some.module'), results.completed(import_instruction))
...
def call_and_assign_sets_variable():
    ...
    spec.execute(execution_context, results).should_collaborate_with(execution_context.get_instance('list').will_return([]), execution_context.target_for([], '__len__').will_return(lambda : 0), execution_context.to_args(['list', '__len__'], 2).will_return(()), execution_context.store_symbol('symbol', 0), results.completed(call_and_assign, 0))
