...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/waferslim/protocol.py
# Compiled at: 2010-03-08 09:43:15
"""
...
_NUMERIC_BLOCK_LENGTH = len((_NUMERIC_ENCODING % 0).encode(BYTE_ENCODING)) + _SEPARATOR_LENGTH
...
def unpack(packed_string):
    ...
    if isinstance(packed_string, unicode) or isinstance(packed_string, str):
        chunks = []
...
class RequestResponder(object):
    ...
    def respond_to_request(self, instructions=Instructions, execution_context=ExecutionContext, isolate_imports=False, results=Results):
        ...
        (received, sent) = self._message_loop(instructions, context, results)
        return (
         received, sent + ack_bytes)
