...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/waferslim/specs/converter_spec.py
# Compiled at: 2010-03-28 09:06:36
"""
BDD-style Lancelot specifications for the behaviour of the core library classes
"""
from waferslim.converters import register_converter, converter_for, convert_arg, convert_result, Converter, StrConverter, TrueFalseConverter, YesNoConverter, FromConstructorConverter, DateConverter, TimeConverter, DatetimeConverter, IterableConverter, TableTableConstants, DictConverter, to_string, from_string
...
def base_converter_to_string():
    ...
    spec.to_string('I hope sir was not overdoing it').should_be(Type(unicode))
...
def datetime_converter_behaviour():
    ...
    spec.from_string('%s %s' % (date_part, time_part)).should_be(datetime.datetime.combine(DateConverter().from_string(date_part), TimeConverter().from_string(time_part)))
    time_trunc = '21:54:32'
    spec.from_string('%s %s' % (date_part, time_trunc)).should_be(datetime.datetime.combine(DateConverter().from_string(date_part), TimeConverter().from_string(time_trunc)))
...
class ConvertArgBehaviour(object):
    ...
    def returns_callable_to_type(self):
        ...
        spec.__call__('1.99').should_raise(TypeError)
