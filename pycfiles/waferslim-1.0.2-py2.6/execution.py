...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/waferslim/execution.py
# Compiled at: 2010-03-10 21:10:12
"""
...
_INSTRUCTION_TYPES = {'make': Make, 'import': Import, 
   'call': Call, 
    ...
    def execute(self, execution_context, results):
        ...
        for item in self._unpacked_list:
            ...
            try:
                ...
                self._logger.warn('Error executing %s:', instruction, exc_info=1)
...
def pythonic(method_name):
    ...
    return method_name[0].lower() + ('').join([ _underscored_lowercase(char) for char in method_name[1:] ])
...
class ExecutionContext(object):
    ...
    def __init__(self, params_converter=ParamsConverter, isolate_imports=False, logger=logging.getLogger('Execution')):
    ...
    def _import_module(self, fully_qualified_name):
        ...
        if dot_pos == -1:
            ...
            return self._import(fully_qualified_name, fromlist=[
    ...
    def target_for(self, instance, method_name, convert_name=True):
        ...
        try:
            return getattr(instance, convert_name and pythonic(method_name) or method_name)
        except AttributeError:
            return convert_name and self.target_for(instance, method_name, False) or None
    ...
    def warn_polluting_library_methods(self, library):
        ...
        for name in ['execute', 'reset', 'table']:
            if hasattr(library, name) and hasattr(getattr(library, name), '__call__'):
