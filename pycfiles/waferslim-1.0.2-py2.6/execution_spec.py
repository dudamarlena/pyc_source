...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/waferslim/specs/execution_spec.py
# Compiled at: 2010-03-10 21:02:49
"""
...
from waferslim.execution import ExecutionContext, Results, Instructions, instruction_for, ParamsConverter, pythonic
from waferslim.instructions import Make, Import, Call, CallAndAssign, Instruction
...
class ExecutionContextBehaviour(object):
    ...
    def _nonsrc_path(self, execution_context):
        ...
        path = os.path.abspath(slim_path + '%s..%snon-waferslim' % (os.sep, os.sep))
        return path
    ...
    def get_isolated_module_type(self):
        ...
        for (_type, mod) in _types.items():
            ...
            spec.get_type(fully_qualified_name).should_not_be(SameAs(different_type))
            instances = (
    ...
    def get_unisolated_module_type(self):
        ...
        for (_type, mod) in _types.items():
            ...
            other_context.add_import_path(self._nonsrc_path(other_context))
            different_type = other_context.get_type(fully_qualified_name)
            spec.get_type(fully_qualified_name).should_be(SameAs(different_type))
            instances = (
             same_type(), different_type())
            spec = lancelot.Spec(instances[0])
            spec.when(spec.alter_state())
    ...
    def warns_if_library_methods_may_pollute_tests(self):
        ...
        for name in polluting_names:
            ...
            spec = lancelot.Spec(context)
            spec.store_instance('library', library).should_collaborate_with(logger.debug(Anything()), logger.warning(Contain('%s()' % name)))
    ...
    def uses_added_type_context(self):
        ...
        spec.when(spec.add_type_prefix('waferslim.examples.decision_table'))
        spec.then(spec.get_type('ShouldIBuyMilk')).should_not_raise(TypeError)
        ctx.cleanup_imports()
    ...
    def imports_twisted(self):
        ...
        for location in sys.path:
            ...
            if exists(pkg) or exists(pkg + 'c') or exists(pkg + 'o'):
                twisted_found = True
                break

        lancelot.Spec(twisted_found).it().should_be(True)
        context = ExecutionContext(isolate_imports=False)
...
def instruction_for_behaviour():
    ...
    instructions = {'make': Type(Make), 'import': Type(Import), 
       'call': Type(Call), 
...
class InstructionsBehaviour(object):
    ...
    def loops_through_list(self):
        ...
        instructions = Instructions(a_list, lambda item: mock_fn.instruction_for(item))
        spec = lancelot.Spec(instructions)
        ctx = ExecutionContext()
        results = Results()
        spec.execute(ctx, results).should_collaborate_with(mock_fn.instruction_for(a_list[0]).will_return(mock_make), mock_make.execute(ctx, results), mock_fn.instruction_for(a_list[1]).will_return(mock_call), mock_call.execute(ctx, results))
        ctx.cleanup_imports()
    ...
    def handles_execute_exceptions(self):
        ...
        try:
            spec.execute(ctx, results).should_collaborate_with(mock_fn.instruction_for(a_list[0]).will_return(mock_call), mock_call.execute(ctx, results).will_raise(Exception(msg)), results.failed(mock_call, msg, False))
        finally:
    ...
    def handles_stoptest_exceptions(self):
        ...
        logger.setLevel(logging.ERROR)
        try:
            spec.execute(ctx, results).should_collaborate_with(mock_fn.instruction_for(a_list[0]).will_return(mock_call), mock_call.execute(ctx, results).will_raise(StopTestException(msg)), results.failed(mock_call, msg, True))
        finally:
            logger.setLevel(log_level)
...
class ResultsBehaviour(object):
    ...
    def completed_ok(self):
        ...
        spec.completed(instruction).should_collaborate_with(instruction.instruction_id().will_return('a'))
    ...
    def failed(self):
        ...
        spec.should_collaborate_with(instruction.instruction_id().will_return('b'))
    ...
    def completed_with_result(self):
        ...
        instruction = lancelot.MockSpec(name='instruction')
        spec = lancelot.Spec(Results())
        spec.completed(instruction, result=Fake()).should_collaborate_with(instruction.instruction_id().will_return('b'))
    ...
    def completed_without_return_value(self):
        ...
        instruction = lancelot.MockSpec(name='instruction')
        spec = lancelot.Spec(Results())
        spec.completed(instruction, result=None).should_collaborate_with(instruction.instruction_id().will_return('c'))
...
def params_converter_behaviour():
    ...
    execution_context = lancelot.MockSpec('execution_context')
    spec = lancelot.Spec(ParamsConverter(execution_context))
    spec.to_args(['$A', '$b_', 'C$'], 0).should_collaborate_with(execution_context.get_symbol('A').will_return('X'), execution_context.get_symbol('b_').will_return('Y'), and_result=('X',
                                                                                                                                                                                      'Y',
                                                                                                                                                                                      'C$'))
    execution_context = lancelot.MockSpec('execution_context')
    spec = lancelot.Spec(ParamsConverter(execution_context))
    spec.to_args(['$id'], 0).should_collaborate_with(execution_context.get_symbol('id').will_return('20'), and_result=('20', ))
    execution_context = lancelot.MockSpec('execution_context')
    spec = lancelot.Spec(ParamsConverter(execution_context))
    ...
    execution_context = lancelot.MockSpec('execution_context')
    spec = lancelot.Spec(ParamsConverter(execution_context))
    spec.to_args(['$s11$s$s1$s11'], 0).should_collaborate_with(execution_context.get_symbol('s11').will_return('I'), execution_context.get_symbol('s').will_return('R'), execution_context.get_symbol('s1').will_return('O'), execution_context.get_symbol('s11').will_return('N'), and_result=('IRON', ))
    execution_context = lancelot.MockSpec('execution_context')
    long_multiline_expression = '$a(long{     multi-line expression})\n    $and(another{     multi-line expression})'
    spec = lancelot.Spec(ParamsConverter(execution_context))
    spec.to_args([long_multiline_expression], 0).should_collaborate_with(execution_context.get_symbol('a').will_return('$a'), execution_context.get_symbol('and').will_return('$and'), and_result=(
     '%s' % long_multiline_expression,))
...
def pythonic_method_names():
    ...
    names = {'method': 'method', 'aMethod': 'a_method', 
