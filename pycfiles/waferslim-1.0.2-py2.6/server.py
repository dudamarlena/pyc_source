...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/waferslim/server.py
# Compiled at: 2010-02-27 17:45:22
"""
Server classes. 
...
class SlimRequestHandler(SocketServer.BaseRequestHandler, waferslim.protocol.RequestResponder):
...
class WaferSlimServer(SocketServer.ThreadingMixIn, SocketServer.TCPServer):
    ...
    def __init__(self, options):
        ...
        server_address = (
         options.inethost, int(options.port))
        SocketServer.TCPServer.__init__(self, server_address, SlimRequestHandler)
...
def _get_options():
    ...
    parser = OptionParser()
    parser.add_option('-p', '--port', dest='port', metavar='PORT', help='listen on port PORT')
    parser.add_option('-i', '--inethost', dest='inethost', metavar='HOST', default='localhost', help='listen on inet address HOST (default: localhost)')
    parser.add_option('-e', '--encoding', dest='encoding', metavar='ENCODING', default='utf-8', help='byte (de-)encode with ENCODING (default: utf-8)')
    parser.add_option('-v', '--verbose', dest='verbose', default=False, action='store_true', help='log verbose messages at runtime (default: False)')
    parser.add_option('-k', '--keepalive', dest='keepalive', default=False, action='store_true', help='keep alive for multiple requests (default: False)')
    parser.add_option('-l', '--logconf', dest='logconf', metavar='CONFIGFILE', default='', help='use logging configuration from CONFIGFILE')
    parser.add_option('-s', '--syspath', dest='syspath', metavar='SYSPATH', default='', help='add entries from SYSPATH to sys.path')
