...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cuiows/client.py
# Compiled at: 2017-01-22 10:20:49
# Size of source mod 2**32: 12286 bytes
import logging, typing, curio
...
import yarl
from cuiows.wsproto.connection import WSConnection, CLIENT, ConnectionEstablished, ConnectionClosed
...
class WSClient(object):
    ...
    def __init__(self, host: str, port: int=443, path: str='/', use_ssl: bool=True, *, ssl_context: CurioSSLContext=None):
        self.host = host
        self.port = port
        if use_ssl:
    ...
    async def connect(cls, url: str, *args, **kwargs) -> 'WSClient':
        ...
        return await cls.connect_host_port(url.host, url.port, path, use_ssl=ssl is not None, ssl_context=ssl)

    @classmethod
    async def connect_host_port(cls, host: str, port: int=443,
                                path: str='/', *args, **kwargs) -> 'WSClient':
        """
        Opens a connection to a websocket server.
        ...
        connection = await curio.open_connection(host=obb.host, port=obb.port, ssl=obb.ssl_context, server_hostname=server_hostname)
    ...
    def _fail(self, event: ConnectionClosed):
        """
    ...
    def close_now(self, code: int=1000,
                  reason=None):
        """
    ...
    def poll_event(self) -> DataReceived:
        """
    ...
    def poll(self, decode: bool=False, *, encoding: str='utf-8') -> typing.Union[(str, bytes)]:
        """
    ...
    def send(self, data: typing.Union[(str, bytes)], *, encoding: str='utf-8'):
        """
