...
# [GCC 8.4.0]
# Embedded file name: /Users/romainegele/Documents/Argonne/deephyper/build/lib/deephyper/search/nas/baselines/gail/adversary.py
...
class TransitionClassifier(object):
    ...
    def __init__(self, env, hidden_size, entcoeff=0.001, lr_rate=0.001, scope='adversary'):
        self.scope = scope
        ...
        generator_logits = self.build_graph((self.generator_obs_ph),
          (self.generator_acs_ph), reuse=False)
        ...
        self.losses = [
         generator_loss, expert_loss, entropy,
         entropy_loss, generator_acc, expert_acc]
        self.loss_name = ['generator_loss', 'expert_loss',
        ...
        self.lossandgrad = U.function([self.generator_obs_ph, self.generator_acs_ph, self.expert_obs_ph, self.expert_acs_ph], self.losses + [U.flatgrad(self.total_loss, var_list)])

    def build_ph(self):
        self.generator_obs_ph = tf.placeholder((tf.float32),
          ((None, ) + self.observation_shape), name='observations_ph')
        self.generator_acs_ph = tf.placeholder((tf.float32),
          ((None, ) + self.actions_shape), name='actions_ph')
        self.expert_obs_ph = tf.placeholder((tf.float32),
          ((None, ) + self.observation_shape), name='expert_observations_ph')
        self.expert_acs_ph = tf.placeholder((tf.float32),
          ((None, ) + self.actions_shape), name='expert_actions_ph')
