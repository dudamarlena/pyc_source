...
# [GCC 8.4.0]
# Embedded file name: /Users/romainegele/Documents/Argonne/deephyper/build/lib/deephyper/search/nas/baselines/ppo2/runner.py
# Compiled at: 2019-07-10 12:45:57
# Size of source mod 2**32: 4817 bytes
import numpy as np
from deephyper.search.nas.baselines.common.runners import AbstractEnvRunner
...
class Runner(AbstractEnvRunner):
    ...
    def run(self):
        minibatch = {'observations':[],  'actions':[],  'rewards':[],  'values':[],  'dones':[],  'neglogpacs':[]}
        data_type = {'observations':self.obs.dtype, 
         'actions':np.float32, 
         'rewards':np.float32, 
         'values':np.float32, 
         'dones':np.float32, 
        ...
        minibatch['advs'], minibatch['returns'] = self.advantage_and_returns(values=(minibatch['values']), rewards=(minibatch['rewards']),
    ...
    def advantage_and_returns(self, values, rewards, dones, last_values, last_dones, gamma, use_non_episodic_rewards=False):
        """
