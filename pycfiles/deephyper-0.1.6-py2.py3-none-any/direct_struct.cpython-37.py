...
# [GCC 8.4.0]
# Embedded file name: /Users/romainegele/Documents/Argonne/deephyper/build/lib/deephyper/search/nas/model/space/struct/direct_struct.py
# Compiled at: 2019-07-11 14:24:06
# Size of source mod 2**32: 9077 bytes
from collections.abc import Iterable
from functools import reduce
import networkx as nx
from tensorflow import keras
from tensorflow.python.keras.utils.vis_utils import model_to_dot
from deephyper.core.exceptions.nas.struct import InputShapeOfWrongType, NodeAlreadyAdded, StructureHasACycle, WrongSequenceToSetOperations, WrongOutputShape
...
class DirectStructure(NxStructure):
    ...
    def __init__(self, input_shape, output_shape, *args, **kwargs):
        ...
        if type(input_shape) is tuple:
            ...
            if type(input_shape) is list and all(map(lambda x: type(x) is tuple, input_shape)):
                self.input_nodes = list()
                for i in range(len(input_shape)):
                    op = Tensor(keras.layers.Input((input_shape[i]),
                      name=f"input_{i}"))
    ...
    def connect(self, node1, node2):
        ...
        if not isinstance(node1, Node):
            ...
            assert nx.is_directed_acyclic_graph(self.graph), f"the connection between {node1} -> {node2} is creating a cycle in the structure's graph."
    ...
    def denormalize(self, indexes):
        ...
        """
        assert isinstance(indexes, Iterable), 'Wrong argument, "indexes" should be of Iterable.'
