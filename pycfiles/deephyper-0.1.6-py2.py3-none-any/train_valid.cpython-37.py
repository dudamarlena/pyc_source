...
# Size of source mod 2**32: 15603 bytes
import math, time, traceback
from inspect import signature
import numpy as np, tensorflow as tf
from sklearn.metrics import mean_squared_error
from tensorflow import keras
import core.logs.logging as jm
from .... import util
...
class TrainerTrainValid:
    ...
    def __init__(self, config, model):
        ...
        self.learning_rate = self.config_hp.get(a.learning_rate, 0.001)
        ...
        if type(self.config[a.data][a.train_Y]) is not np.ndarray:
            ...
            if type(self.config[a.data][a.valid_Y]) is not np.ndarray:
                ...
                if type(self.config[a.data][a.train_X]) is np.ndarray and type(self.config[a.data][a.valid_X]) is np.ndarray:
                    ...
                    if type(self.config[a.data][a.train_X]) is list and type(self.config[a.data][a.valid_X]) is list:

                        def f(x):
                            return type(x) is np.ndarray

                        if not (all(map(f, self.config[a.data][a.train_X])) and all(map(f, self.config[a.data][a.valid_X]))):
            ...
            assert all(map(lambda x: np.shape(x)[0] == self.valid_size, self.valid_X)), 'All validation inputs data should have same length!'
    ...
    def predict(self, dataset: str='valid', keep_normalize: bool=False) -> tuple:
        ...
        if self.preprocessing_func and not keep_normalize:
            if not self.data_config_type == 'gen':
                if dataset == 'valid':
