...
class Evaluator:
    ...
    def __init__(self, run_function, cache_key=None):
        ...
        if moduleName == '__main__':
            raise RuntimeError(f'Evaluator will not execute function "{run_function.__name__}" because it is in the __main__ module.  Please provide a function imported from an external module!')
    ...
    def dump_evals(self, saved_key=None):
        ...
        if saved_key is None:
            with open('results.json', 'w') as (fp):
                json.dump((self.finished_evals), fp, indent=4, sort_keys=True,
                  cls=Encoder)
        resultsList = []
