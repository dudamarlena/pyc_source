...
# [GCC 8.4.0]
# Embedded file name: /Users/romainegele/Documents/Argonne/deephyper/build/lib/deephyper/search/nas/baselines/ppo2/layers.py
...
def ppo_lstm(num_units=128, layer_norm=False):
    ...
    def network_fn(input, mask, state):
        ...
        if layer_norm:
            h, next_state = lnlstm([
             input],
              [mask[:, None]], state, scope='lnlstm', nh=num_units)
        else:
            h, next_state = lstm([input], [mask[:, None]], state,
              scope='lstm', nh=num_units)
        h = h[0]
        return (h, next_state)
...
def ppo_cnn_lstm(num_units=128, layer_norm=False, **conv_kwargs):
    ...
    def network_fn(input, mask, state):
        ...
        if layer_norm:
            ...
            h, next_state = lstm([h], [mask[:, None]], state,
              scope='lstm', nh=num_units)
        h = h[0]
        return (h, next_state)
