...
# [GCC 8.4.0]
# Embedded file name: /Users/romainegele/Documents/Argonne/deephyper/build/lib/deephyper/search/nas/model/trainer/train_valid.py
# Compiled at: 2019-07-11 14:24:06
# Size of source mod 2**32: 14967 bytes
import math, time, traceback, numpy as np, tensorflow as tf
...
class TrainerTrainValid:
    ...
    def load_data(self):
        ...
        if self.data_config_type == 'gen':
            ...
            if self.data_config_type == 'ndarray':
                ...
                raise RuntimeError(f"Data config is not supported by this Trainer: '{self.data_config_type}'!")
    ...
    def load_data_ndarray(self):
        if type(self.config[a.data][a.train_Y]) is not np.ndarray:
            ...
            if type(self.config[a.data][a.valid_Y]) is not np.ndarray:
            ...
            if type(self.config[a.data][a.train_X]) is np.ndarray and type(self.config[a.data][a.valid_X]) is np.ndarray:
                self.train_X = [
                 self.config[a.data][a.train_X]]
                self.valid_X = [self.config[a.data][a.valid_X]]
            elif type(self.config[a.data][a.train_X]) is list:
                if type(self.config[a.data][a.valid_X]) is list:

                    def f(x):
                        return type(x) is np.ndarray

                    if not all(map(f, self.config[a.data][a.train_X])) or not all(map(f, self.config[a.data][a.valid_X])):
                        raise RuntimeError('all inputs data should be of type np.ndarray !')
            ...
            if not all(map(lambda x: np.shape(x)[0] == self.train_size, self.train_X)):
                raise RuntimeError('All training inputs data should have same length!')
            self.valid_size = np.shape(self.valid_X[0])[0]
            raise all(map(lambda x: np.shape(x)[0] == self.valid_size, self.valid_X)) or RuntimeError('All validation inputs data should have same length!')
    ...
    def set_dataset_train(self):
        if self.data_config_type == 'ndarray':
            self.dataset_train = tf.data.Dataset.from_tensor_slices(({f"input_{i}":tX for i, tX in enumerate(self.train_X)},
             self.train_Y))
        else:
            self.dataset_train = tf.data.Dataset.from_generator((self.train_gen), output_types=(self.data_types),
              output_shapes=(
    ...
    def set_dataset_valid(self):
        if self.data_config_type == 'ndarray':
            self.dataset_valid = tf.data.Dataset.from_tensor_slices(({f"input_{i}":vX for i, vX in enumerate(self.valid_X)},
             self.valid_Y))
        else:
            self.dataset_valid = tf.data.Dataset.from_generator((self.valid_gen), output_types=(self.data_types),
              output_shapes=(
             {f"input_{i}":tf.TensorShape([*self.data_shapes[0][f"input_{i}"]]) for i in range(len(self.data_shapes[0]))},
             tf.TensorShape([*self.data_shapes[1]])))
    ...
    def model_compile(self):
        ...
        decay_rate = self.learning_rate / self.num_epochs if self.num_epochs > 0 else 1.0
    ...
    def predict(self, dataset: str='valid', keep_normalize: bool=False) -> tuple:
        ...
        if not (dataset == 'valid' or dataset == 'train'):
            ...
            if dataset == 'valid':
                y_pred = self.model.predict((self.dataset_valid),
                  steps=(self.valid_steps_per_epoch))
            else:
                y_pred = self.model.predict((self.dataset_train), steps=(self.train_steps_per_epoch))
            if self.preprocessing_func and not keep_normalize and not self.data_config_type == 'gen':
                if dataset == 'valid':
    ...
    def evaluate(self, dataset='train'):
        ...
        if dataset == 'train':
            ...
            return self.model.evaluate((self.dataset_valid), steps=(self.valid_steps_per_epoch))
    ...
    def train(self, num_epochs: int=None, with_pred: bool=False, last_only: bool=False):
        ...
        if num_epochs > 0:
            ...
            if not last_only:
                ...
                history = self.model.fit((self.dataset_train),
                  epochs=num_epochs,
            ...
            if num_epochs < 0:
                raise RuntimeError(f"Trainer: number of epochs should be >= 0: {num_epochs}")
        if with_pred:
