...
# [GCC 8.4.0]
# Embedded file name: /Users/romainegele/Documents/Argonne/deephyper/build/lib/deephyper/search/nas/baselines/common/tests/envs/identity_env.py
...
class DiscreteIdentityEnv(IdentityEnv):
    ...
    def __init__(self, dim, episode_len=None):
...
class MultiDiscreteIdentityEnv(IdentityEnv):
    ...
    def __init__(self, dims, episode_len=None):
        self.action_space = MultiDiscrete(dims)
        super().__init__(episode_len=episode_len)

    def _get_reward(self, actions):
        if all(self.state == actions):
            return 1
        else:
            return 0
...
class BoxIdentityEnv(IdentityEnv):
    ...
    def __init__(self, shape, episode_len=None):
