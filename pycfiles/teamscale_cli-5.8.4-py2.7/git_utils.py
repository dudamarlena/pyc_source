...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/teamscale_precommit_client/git_utils.py
# Compiled at: 2020-04-21 02:31:18
from __future__ import absolute_import
from __future__ import unicode_literals
from __future__ import print_function
import os
from pygit2 import Repository, GIT_STATUS_WT_DELETED, GIT_STATUS_WT_MODIFIED, GIT_STATUS_WT_RENAMED, GIT_STATUS_WT_TYPECHANGE, GIT_STATUS_INDEX_NEW, GIT_STATUS_INDEX_MODIFIED, GIT_STATUS_INDEX_DELETED, discover_repository
from io import open
_STATI_CONSIDERED_FOR_PRECOMMIT = GIT_STATUS_INDEX_NEW | GIT_STATUS_INDEX_MODIFIED | GIT_STATUS_WT_MODIFIED | GIT_STATUS_WT_RENAMED | GIT_STATUS_WT_TYPECHANGE
...
def get_current_branch(path_to_repository):
    ...
    head = repo.lookup_reference(b'HEAD').resolve()
...
def get_current_timestamp(path_to_repository):
    ...
    """
    repo = Repository(path_to_repository)
    head = repo.revparse_single(b'HEAD')
...
def filter_changed_files(changed_files, path_to_repository):
    ...
    for changed_file in changed_files:
        if os.path.getsize(os.path.join(path_to_repository, changed_file)) > 1048576:
            print(b'File too large for precommit analysis. Ignoring: %s' % changed_file)
        else:
...
def get_changed_files_and_content(path_to_repository):
    ...
    return {filename:open(os.path.join(path_to_repository, filename), encoding=b'utf-8').read() for filename in changed_files}
