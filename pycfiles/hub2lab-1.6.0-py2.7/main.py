...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/github2gitlab/main.py
# Compiled at: 2018-12-13 03:56:39
import argparse, git, gitdb, hashlib, json, logging, os, re, requests, six
...
class GitHub2GitLab(object):
    TAG_MERGED = ':MERGED:'
    STATE_EVENT2MERGE_STATE = {'merge': 'merged', 
       'reopen': 'opened', 
       'close': 'closed'}

    def __init__(self, args):
        self.args = args
        self.args.ssh_public_key = os.path.expanduser(self.args.ssh_public_key)
        if not self.args.gitlab_repo:
            self.args.gitlab_repo = self.args.github_repo
        self.args.gitlab_namespace, self.args.gitlab_name = self.args.gitlab_repo.split('/')
        self.args.gitlab_repo = parse.quote_plus(self.args.gitlab_repo)
        self.github = {'url': 'https://api.github.com', 
           'git': 'https://github.com', 
           'repo': self.args.github_repo, 
           'token': self.args.github_token}
        if self.args.branches:
            self.github['branches'] = self.args.branches.split(',')
        self.gitlab = {'git': self.args.gitlab_url.replace('http://', 'git@'), 'host': self.args.gitlab_url, 
    ...
    @staticmethod
    def get_parser():
        parser = argparse.ArgumentParser(description='migrate projects from GitHub to GitLab')
        parser.add_argument('--gitlab-url', help='Gitlab url', required=True)
        parser.add_argument('--gitlab-token', help='Gitlab authentication token', required=True)
        parser.add_argument('--gitlab-repo', help='Gitlab repo (for instance ceph/ceph)')
        parser.add_argument('--github-token', help='GitHub authentication token')
        parser.add_argument('--github-repo', help='GitHub repo (for instance ceph/ceph)', required=True)
        parser.add_argument('--ssh-public-key', default='~/.ssh/id_rsa.pub', help='SSH public key')
        parser.add_argument('--branches', help='comma separated list of git branches to mirror (defaults to all)')
        parser.add_argument('--ignore-closed', action='store_const', const=True, help='ignore pull requests closed and not merged')
        parser.add_argument('--skip-pull-requests', action='store_const', const=True, help='do not mirror PR to MR')
        parser.add_argument('--verbose', action='store_const', const=True, help='enable verbose (debug) logging')
        parser.add_argument('--cache', action='store_const', const=True, help='cache GitHub pull requests list')
        parser.add_argument('--clean', action='store_const', const=True, help='Remove the repo after sync')
        return parser
    ...
    def sh(self, command):
        ...
        proc = subprocess.Popen(args=command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=True, bufsize=1)
        lines = []
        with proc.stdout:
    ...
    def gitlab_create_remote(self, repo):
        url = self.gitlab['git'].replace('https://', ('https://user:{}@').format(self.gitlab['token']))
        repo.create_remote('gitlab', url + ('/' if url.startswith('http') else ':') + self.gitlab['namespace'] + '/' + self.gitlab['name'] + '.git')

    def git_mirror(self):
        name = self.gitlab['name']
        if not os.path.exists(name):
            self.sh('git clone --bare ' + self.github['git'] + '/' + self.github['repo'] + ' ' + name)
        repo = git.Repo(name)
        os.chdir(name)
        if not hasattr(repo.remotes, 'gitlab'):
            self.gitlab_create_remote(repo)
        if 'branches' in self.github:
            branches_ref = (' ').join([ ('+refs/heads/{b}:refs/heads/{b}').format(b=b) for b in self.github['branches']
                                      ])
        else:
            branches_ref = '+refs/heads/*:refs/heads/*'
        self.sh('git fetch --force origin ' + branches_ref + ' +refs/tags/*:refs/tags/*')
        if self.args.skip_pull_requests:
            self.git_mirror_optimize(repo)
        else:
            self.sh('git fetch origin +refs/pull/*:refs/heads/pull/*')
        self.sh('git push --prune --force gitlab ' + branches_ref + ' ' + '+refs/heads/pull/*:refs/heads/pull/* ' + '+refs/tags/*:refs/tags/* ')
        os.chdir('..')
    ...
    def git_mirror_optimize(self, repo):
        ...
        for head in repo.refs:
            ...
            log.debug(action + ' ref ' + known_head_name + '==' + head.commit.hexsha)
            repo.git.update_ref('refs/' + known_head_name, head.commit)
            log.info(action + ' branch ' + 'pull/' + pr + '/merge == ' + merge.hexsha)
    ...
    def add_key(self):
        ...
        if list(filter(lambda key: key['key'] == public_key, keys)):
            ...
            name = 'github2gitlab'
            log.info('add ' + name + ' ssh public key from ' + self.args.ssh_public_key)
            query['title'] = name
            query['key'] = public_key
            result = requests.post(url, query)
            if result.status_code != requests.codes.created:
                log.warn(('Key {} already in GitLab. Possible under a different user. Skipping...').format(self.args.ssh_public_key))
            return public_key
            return
    ...
    def add_project(self):
        ...
        if requests.get(url, params=query).status_code == requests.codes.ok:
            ...
            log.debug('project ' + g['repo'] + ' added: ' + result.text)
            return result.json()
            return
    ...
    def unprotect_branches(self):
        ...
        for branch in r.json():
            if branch['protected']:
                r = requests.put(url + '/' + branch['name'] + '/unprotect', params=query)
                r.raise_for_status()
                unprotected += 1
    ...
    @staticmethod
    def field_equal(pull, pull_field, pull_value, merge, merge_field, merge_value):
        if pull_field == 'state':
            return pull_value == 'open' and merge_value == 'opened' or pull_value == 'closed' and merge_value in ('closed',
                                                                                                                  'merged')
        else:
            if pull_field == 'body':
                ...
                return pull_value[:DESCRIPTION_MAX] == merge_value[:DESCRIPTION_MAX]
            else:
    ...
    @staticmethod
    def field_update(pull, pull_field, pull_value, merge, merge_field, merge_value):
        if pull_value is None:
            pull_value = ''
        if pull_field == 'state':
            if pull_value == 'open':
            ...
            if pull_field == 'body':
                ...
                return (
                 merge_field, pull_value)

            return

    def sync(self):
        pull_f2merge_f = {'state': 'state', 
           'body': 'description', 
           'title': 'title'}
        for number in sorted(self.pull_requests.keys()):
            ...
            if number in self.pull2merge:
                ...
                if self.rev_parse(pull, source_branch) and self.rev_parse(pull, target_branch):
                    data = {'title': pull['title'], 'source_branch': source_branch, 'target_branch': target_branch}
                    if pull['body']:
            ...
            if merge:
                ...
                for pull_field, merge_field in six.iteritems(pull_f2merge_f):
                    if not self.field_equal(pull, pull_field, pull[pull_field], merge, merge_field, merge[merge_field]):
                        key, value = self.field_update(pull, pull_field, pull[pull_field], merge, merge_field, merge[merge_field])
                        updates[key] = value

                if updates:
                    self.update_merge_request(merge, updates)
                else:
                    log.debug('https://github.com/' + self.github['repo'] + '/' + 'pull/' + number + ' == ' + self.gitlab['host'] + '/' + parse.unquote(self.gitlab['repo']) + '/' + 'merge_requests/' + str(merge['iid']))
    ...
    def rev_parse(self, pull, revision):
        ...
        try:
            ...
            log.debug('ignore https://github.com/' + self.github['repo'] + '/pull/' + str(pull['number']) + ' because ' + revision + ' is not a known revision')
            return False
    ...
    def get(self, url, query, cache):
        payloads_file = self.tmpdir + '/' + hashlib.sha1(url.encode('utf-8')).hexdigest() + '.json'
        if not cache or not os.access(payloads_file, 0) or time.time() - os.stat(payloads_file).st_mtime > 86400:
            payloads = []
            next_query = query
            while next_query:
                ...
                for link in result.headers.get('Link', '').split(','):
                    if 'rel="next"' in link:
                        ...
                        if m:
                            ...
                            next_query = query
                            next_query.update(dict(parse.parse_qsl(parsed_url.query)))
    ...
    def get_pull_requests(self):
        ...
        pulls = filter(f, self.get(g['url'] + '/repos/' + g['repo'] + '/pulls', query, self.args.cache))
    ...
    def get_merge_requests(self):
        ...
        g = self.gitlab
        merges = self.get(g['url'] + '/projects/' + g['repo'] + '/merge_requests', {'private_token': g['token'], 'state': 'all'}, cache=False)
    ...
    def create_merge_request(self, query):
        ...
        for key, value in six.iteritems(query):
            ...
            if value.strip().replace('\n', '').replace('\r', '') != merge.get(key).strip().replace('\n', '').replace('\r', ''):
                raise ValueError(url + ' ' + key + ' expected ' + value + ' but is ' + merge.get(key, 'None'))
    ...
    def update_merge_request(self, merge_request, updates):
        ...
        if state_event == 'merge' and result['state'] == 'opened':
    ...
    def put_merge_request(self, merge_request, updates):
        ...
        url = g['url'] + '/projects/' + g['repo'] + '/merge_requests/' + str(merge_request['iid'])
    ...
    def verify_merge_update(self, updates, result):
        ...
        for key, value in six.iteritems(updates):
            ...
            if value.strip() != result_value.strip():
                url = g['host'] + '/' + parse.unquote(g['repo']) + '/' + 'merge_requests/' + str(result['iid'])
                raise ValueError(('{url}: {key} value expected to be {value} but is {result}').format(url=url, key=key, value=value, result=result_value))