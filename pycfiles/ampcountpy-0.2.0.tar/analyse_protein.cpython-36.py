...
# Embedded file name: /home/cw12401/code/work/ampal/src/ampal/analyse_protein.py
# Compiled at: 2018-04-11 08:12:46
# Size of source mod 2**32: 25258 bytes
__doc__ = 'Contains various tools for analysing protein structure.'
from collections import Counter
import warnings, numpy
from .pseudo_atoms import Primitive
from .geometry import angle_between_vectors, dihedral, distance, find_foot, unit_vector, is_acute
from .amino_acids import residue_mwt, water_mass, residue_ext_280, residue_pka, residue_charge, side_chain_dihedrals
...
def sequence_charge(seq, pH=7.4):
    ...
    if 'X' in seq:
        warnings.warn(_nc_warning_str, NoncanonicalWarning)
    adj_protein_charge = sum([partial_charge(aa, pH) * residue_charge[aa] * n for aa, n in Counter(seq).items()])
...
def measure_sidechain_torsion_angles(residue, verbose=True):
    ...
    if aa not in side_chain_dihedrals:
        ...
        for set_atoms in side_chain_dihedrals[aa]:
            ...
            try:
                angle = dihedral(residue[required_for_dihedral[0]]._vector, residue[required_for_dihedral[1]]._vector, residue[required_for_dihedral[2]]._vector, residue[required_for_dihedral[3]]._vector)
                chi_angles.append(angle)
            except KeyError as k:
                print("{0} atom missing from residue {1} {2} - can't assign dihedral".format(k, residue.mol_code, residue.id))
...
def crick_angles(p, reference_axis, tag=True, reference_axis_name='ref_axis'):
    ...
    ref_points = reference_axis.coordinates
    cr_angles = [dihedral(ref_points[i], prim_cas[i], prim_cas[(i + 1)], p_cas[i]) for i in range(len(prim_cas) - 1)]
...
def alpha_angles(p, reference_axis, tag=True, reference_axis_name='ref_axis'):
    ...
    prim_cas = p.primitive.coordinates
    ref_points = reference_axis.coordinates
    alphas = [abs(dihedral(ref_points[(i + 1)], ref_points[i], prim_cas[i], prim_cas[(i + 1)])) for i in range(len(prim_cas) - 1)]
    alphas.append(None)
    if tag:
        p.tags[reference_axis_name] = reference_axis
...
def reference_axis_from_chains(chains):
    ...
    coords = [
     numpy.array(chains[0].primitive.coordinates)]
...
def make_primitive(cas_coords, window_length=3):
    ...
    if len(cas_coords) >= window_length:
        ...
        raise ValueError('A primitive cannot be generated for {0} atoms using a (too large) averaging window_length of {1}.'.format(len(cas_coords), window_length))
    return primitive
...
def make_primitive_smoothed(cas_coords, smoothing_level=2):
    ...
    try:
        ...
        raise ValueError('Smoothing level {0} too high, try reducing the number of rounds or give a longer Chain (curent length = {1}).'.format(smoothing_level, len(cas_coords)))
...
def make_primitive_extrapolate_ends(cas_coords, smoothing_level=2):
    ...
    """
    try:
        smoothed_primitive = make_primitive_smoothed(cas_coords,
          smoothing_level=smoothing_level)
    except ValueError:
        smoothed_primitive = make_primitive_smoothed(cas_coords,
          smoothing_level=(smoothing_level - 1))
    ...
    for ca in cas_coords:
        ...
        closest_indices = sorted([x[0] for x in sorted((enumerate(prim_dists)),
          key=(lambda k: k[1]))[:3]])
