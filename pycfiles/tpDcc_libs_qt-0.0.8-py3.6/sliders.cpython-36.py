...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/qt/widgets/sliders.py
# Compiled at: 2020-04-15 12:12:43
# Size of source mod 2**32: 35408 bytes
"""
        ...
        if event.button() == Qt.MidButton:
            if not self._draggers:
                ...
                if self._is_float:
                    ...
                    self._draggers.move(self.mapToGlobal(QPoint(event.pos().x() - 1, event.pos().y() - (self._draggers.height() - draggers_height / 6))))
...
class HoudiniDoubleSlider(QWidget, object):
    ...
    def __init__(self, parent, slider_type='float', style=0, name=None, slider_range=None, default_value=0.0, dragger_steps=None, main_color=None, *args):
        if slider_range is None:
            ...
            if dragger_steps is None:
                dragger_steps = FLOAT_SLIDER_DRAG_STEPS
            (super(HoudiniDoubleSlider, self).__init__)(*args, **{'parent': parent})
            h = 20
            self._parent = parent
    ...
    def __init__(self, parent, color1=None, color2=None, slider_range=None, dragger_steps=None, main_color=None, *args):
        if color1 is None:
            ...
            if dragger_steps is None:
                dragger_steps = [
                 5.0, 1.0, 0.25]
        (super(GradientSlider, self).__init__)(*args, parent=parent, slider_range=slider_range, dragger_steps=dragger_steps)
        self._parent = parent
...
class ColorSlider(QWidget, object):
    ...
    def __init__(self, parent=None, start_color=None, slider_type='float', alpha=False, height=50, *args):
        (super(ColorSlider, self).__init__)(*args, **{'parent': parent})
        self._parent = parent
        self._type = slider_type
        self._alpha = alpha
        ...
        self._color_btn.setStyleSheet(self._style_str % (
