...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/qt/widgets/attributes.py
# Compiled at: 2020-04-15 12:12:43
# Size of source mod 2**32: 17851 bytes
"""
...
class Float2Editor(BaseEditor, object):
    ...
    def get_value(self):
        return (
         self.value1_line.value, self.value2_line.value)
        ...
        if node_values:
            if len(node_values) > 1:
                ...
                if type(editor_value) in [QPoint, QPointF]:
                    ...
                    if type(editor_value) in [list, tuple]:
...
class Float3Editor(BaseEditor, object):
    ...
    def get_value(self):
        return (
         self.value1_line.value(), self.value2_line.value(), self.value3_line.value())
...
class ColorPicker(BaseEditor, object):
    ...
    def OnSliderChanged(self):
        ...
        if not self._current_value:
            tp.logger.debug('Caching color: (%d, %d, %d)' % (
...
WIDGET_MAPPER = dict(float=[
 FloatEditor, 0.0],
  float2=[
 Float2Editor, [0.0, 0.0]],
  float3=[
 Float3Editor, [0.0, 0.0, 0.0]],
  bool=[
 BoolEditor, False],
  str=[
 StringEditor, ''],
