...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/qt/externals/pysideuic/uiparser.py
# Compiled at: 2020-04-15 12:12:43
# Size of source mod 2**32: 31976 bytes
import sys, logging, os.path, re
try:
...
class UIParser(object):
        ...
        if isinstance(widget, QtGui.QTreeView):
            ...
            if isinstance(widget, QtGui.QTableView):
                ...
                if isinstance(topwidget, QtGui.QToolBox):
                    ...
                    if isinstance(topwidget, QtGui.QTabWidget):
                        icon = self.wprops.getAttribute(elem, 'icon')
                        if icon is not None:
                        ...
                        if isinstance(topwidget, QtGui.QWizard):
                            topwidget.addPage(widget)
                        else:
                            if isinstance(topwidget, QtGui.QStackedWidget):
                                topwidget.addWidget(widget)
                            else:
                                if isinstance(topwidget, (QtGui.QDockWidget, QtGui.QScrollArea)):
                                    ...
                                    if type(widget) == QtGui.QWidget:
                                        ...
                                        if isinstance(widget, QtGui.QToolBar):
                                            ...
                                            if isinstance(widget, QtGui.QMenuBar):
                                                ...
                                                if isinstance(widget, QtGui.QStatusBar):
    ...
    def createSpacer(self, elem):
        ...
        policy = (
         QtGui.QSizePolicy.Minimum, sizeType)
        ...
        spacer = self.factory.createQObject('QSpacerItem', (self.uniqueName('spacerItem')),
          (size_args + policy), is_attribute=False)
        if self.stack.topIsLayout():
            lay = self.stack.peek()
            gp = elem.attrib['grid-position']
            if isinstance(lay, QtGui.QFormLayout):
    ...
    def handleItem(self, elem):
        ...
        if isinstance(w, QtGui.QComboBox):
            ...
            if isinstance(w, QtGui.QListWidget):
                text = self.wprops.getProperty(elem, 'text')
                icon = self.wprops.getProperty(elem, 'icon')
                ...
                if isinstance(w, QtGui.QTreeWidget):
                        ...
                        if c_prop_name == 'text':
                            ...
                            if c_prop:
                                ...
                                if c_prop_name == 'icon':
                                    ...
                                    if c_prop_name == 'flags':
                                        item.setFlags(c_prop)
                                    else:
                                        if c_prop_name == 'checkState':
                                            item.setCheckState(column, c_prop)
                                        else:
                                            if c_prop_name == 'background':
                    ...
                    if isinstance(w, QtGui.QTableWidget):
                        text = self.wprops.getProperty(elem, 'text')
                        icon = self.wprops.getProperty(elem, 'icon')
                        flags = self.wprops.getProperty(elem, 'flags')
                        check_state = self.wprops.getProperty(elem, 'checkState')
    ...
    def createAction(self, elem):
        self.setupObject('QAction', self.currentActionGroup or self.toplevelWidget, elem)
    ...
    widgetTreeItemHandlers = {'widget':createWidget, 
     'addaction':addAction, 
     'layout':createLayout, 
     'spacer':createSpacer, 
     'item':handleItem, 
