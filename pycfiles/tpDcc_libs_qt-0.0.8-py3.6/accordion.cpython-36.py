...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/qt/widgets/accordion.py
# Compiled at: 2020-04-15 12:12:43
# Size of source mod 2**32: 18493 bytes
"""
...
class AccordionItem(QGroupBox, object):
    ...
    def dragEnterEvent(self, event):
        ...
        source = event.source()
        if source != self:
            if source.parent() == self.parent():
                if isinstance(source, AccordionItem):
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            if self.get_drag_drop_rect().contains(event.pos()):
            ...
            if self.expand_collapse_rect().contains(event.pos()):
                self._clicked = True
                event.accept()
        else:
            event.ignore()
    ...
    def paintEvent(self, event):
        ...
        if self.rollout_style == AccordionStyle.ROUNDED:
            ...
            if self.rollout_style == AccordionStyle.SQUARE:
                painter.drawText(x + 33, y + 3, w, 16, Qt.AlignLeft | Qt.AlignTop, self.title())
                self._draw_triangle(painter, x, y)
                pen = QPen(self.palette().color(QPalette.Light))
                pen.setWidthF(0.6)
                ...
                if self.rollout_style == AccordionStyle.MAYA:
                    ...
                    if self.rollout_style == AccordionStyle.BOXED:
