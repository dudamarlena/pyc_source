...
# Embedded file name: build/bdist.linux-x86_64/egg/accountable/accountable.py
# Compiled at: 2016-11-19 19:22:06
...
class Accountable(object):
    ...
    def projects(self):
        ...
        return [(project['id'], project['key'], project['name']) for project in metadata['projects']]
    ...
    def issue_types(self, project_key):
        metadata = self._metadata()
        if project_key:
            project = next(project for project in metadata['projects'] if project['key'] == str(project_key))
    ...
    def checkout(self, issue_key):
        ...
        issue = self.issue()
        issue_data = {'self': issue['self'], 
         'id': issue['id'], 
         'key': issue['key']}
        branch_name = '{}-{}'.format(issue_key, slugify(issue['fields']['summary']))
    ...
    def issue_add_comment(self, body):
        return self.resource.post('issue/{}/comment'.format(self.issue_key), {'body': body})

    def issue_worklog(self):
        return self.resource.get('issue/{}/worklog'.format(self.issue_key))

    def issue_transitions(self):
        return self.resource.get('issue/{}/transitions'.format(self.issue_key))

    def issue_do_transition(self, transition_id):
        return self.resource.post('issue/{}/transitions'.format(self.issue_key), {'transition': {'id': transition_id}})

    def users(self, query):
        payload = self.resource.get('user/search', params={'username': query})
        return payload
    ...
    def _access_field(field, d):
        ...
        raise TypeError('There is an issue with your issue fieldconfiguration.')
...
class Resource(object):
    ...
    def get(self, resource, params={}):
        r = requests.get('{}/{}'.format(self.api_uri, resource), params=params, auth=self.auth)
        return r.json()

    def post(self, resource, payload={}):
        r = requests.post('{}/{}'.format(self.api_uri, resource), auth=self.auth, json=payload)
        try:
            return r.json()
        except ValueError:
            return r