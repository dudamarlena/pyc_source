...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hatenadiary.py
# Compiled at: 2009-07-01 08:59:18
import base64, random, urllib, httplib, re, datetime
...
class HatenaDiary:
    ...
    def post(self, title, text, date=datetime.datetime.now(), isDraft=False):
        ...
        result = self._open('POST', url, entry)
        return self._parse_entry(minidom.parse(result))
    ...
    def _parseRFC3339(self, string):
        value = re.findall('(\\d+)-(\\d+)-(\\d+)T(\\d+):(\\d+):(\\d+)([\\+-])(\\d+):(\\d+)', string)[0]
        if len(value) != 9:
            return None
        else:
        ...
        for node in dom.getElementsByTagName('entry'):
            for link in node.getElementsByTagName('link'):
                if link.getAttribute('rel') == 'edit':
                    ...
                    edit = url
                elif link.getAttribute('rel') == 'alternate':
    ...
    def _open(self, method, url, body=None, header=None):
        header_info = {'X-WSSE': _WSSE(self.userid, self.passwd).get_wsse(), 'User-Agent': "yoshiori's test(yoshiori@gmail.com"}
        if header:
        ...
        if not 200 <= r.status < 300:
            raise Exception('status = ' + str(r.status), 'msg = ' + str(r.msg), 'body = ' + r.read())
        return r
