...
# [GCC 8.4.0]
# Embedded file name: /Users/paulos/.virtualenvs/doid/lib/python3.6/site-packages/doid/tests/test_interfaces.py
# Compiled at: 2018-09-26 00:51:15
...
class TestInterfaces(TestCase):
    ...
    def setUp(self):
        self.data = ListContainer(GenericObject(name=name, city=city, born=born, ampm=ampm) for name, city, born, ampm in DATA)
    ...
    def test_generic_filter__negated(self):
        ...
        result = self.data.filter(~am_filter)
        self.assertEqual(len(result), len([_ for _ in self.data if _.ampm != 'AM']))
    ...
    def test_generic_filter__or(self):
        ...
        self.assertEqual(len(result), len([_ for _ in self.data if _.born.year >= 1960 or _.born.month == 12]))
