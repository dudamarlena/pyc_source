...
# [GCC 8.4.0]
# Embedded file name: /Users/paulvicol/Code/Python/equibel_to_update/equibel/parsers/bcfparser.py
# Compiled at: 2016-05-29 20:48:46
...
import equibel.parsers.FormulaParserSim as FormulaParserSim, equibel.formatters.BCF_Formatter as BCF_Formatter
__all__ = [
 'parse_bcf']
    ...
    for token in tokens:
        if '..' in token:
            ...
            if range_start > range_end:
                raise FormatError(('End of range is smaller than start of range on line {0}: "{1}"').format(line_num, token))
...
def process_atom_line(tokens, graph, line_num):
    ...
    for token in tokens:
        if not re.match('[a-zA-Z][a-zA-Z0-9]*', token):
...
def process_edge_line(tokens, graph, line_num):
    ...
    if len(tokens) != 2:
        raise FormatError(('Expected exactly 2 arguments in edge line on line {0}, got: "{1}"').format(line_num, (', ').join(tokens)))
...
process_line = {'n': process_node_line, 
   'a': process_atom_line, 
   'e': process_edge_line, 
