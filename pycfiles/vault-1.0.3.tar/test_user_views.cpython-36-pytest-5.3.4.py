...
# [GCC 8.4.0]
# Embedded file name: /Users/julio.rama/git/vault-opensource/identity/tests/test_user_views.py
# Compiled at: 2020-03-19 01:55:36
# Size of source mod 2**32: 16267 bytes
...
from identity.views import ListUserView, CreateUserView, UpdateUserView, DeleteUserView, UpdateProjectUserPasswordView
...
class ListUserTest(TestCase):
    ...
    def test_show_user_list(self):
        patch('identity.keystone.Keystone.user_list', Mock(return_value=[FakeResource(1)])).start()
...
class CreateUserTest(TestCase):
    ...
    def setUp(self):
        ...
        self.request.META.update({'SERVER_NAME':'globo.com', 
         'SERVER_PORT':'80'})
        self.request.user.token = FakeToken
        patch('actionlogger.actionlogger.ActionLogger.log', Mock(return_value=None)).start()
        patch('identity.keystone.Keystone._create_keystone_connection').start()
        patch('identity.keystone.Keystone.project_list', Mock(return_value=[FakeResource(1, 'project1')])).start()
        patch('identity.keystone.Keystone.role_list', Mock(return_value=[FakeResource(1, 'role1')])).start()
        patch('keystoneclient.v3.client.Client').start()
    ...
    def test_validating_a_blank_field(self):
        ...
        user.project_id = 1
        patch('identity.keystone.Keystone.user_get', Mock(return_value=user)).start()
        self.request.method = 'POST'
        self.request.META.update({'HTTP_HOST': 'localhost'})
        post = self.request.POST.copy()
        post.update({'name':'',  'enabled':True,  'id':1,  'project':1,  'role':1, 
         'password':'aaa',  'password_confirm':'aaa',  'email':'a@a.net'})
    ...
    def test_validating_email_field(self):
        ...
        self.request.META.update({'HTTP_HOST': 'localhost'})
        post = self.request.POST.copy()
        post.update({'name':'aaa',  'enabled':True,  'id':1,  'project':1,  'role':1, 
         'password':'aaa',  'password_confirm':'aaa',  'email':'a.a.net'})
        self.request.POST = post
        response = self.view(self.request)
        response.render()
    ...
    def test_user_create_method_was_called(self, mock):
        ...
        mock.assert_called_with(name='aaa', enabled=True, project_id=1,
          role_id=1,
          password='aaa',
          email='a@a.net',
          domain=None)
...
class UpdateUserTest(TestCase):
    ...
    def test_user_update_method_was_called(self, mock_user_update):
        patch('identity.keystone.Keystone.project_list', Mock(return_value=[FakeResource(1, 'project1')])).start()
        patch('identity.keystone.Keystone.project_get', Mock(return_value=1)).start()
        user = FakeResource(1, 'user1')
        user.to_dict = lambda : {'name': user.name}
        user.default_project_id = 1
        patch('identity.keystone.Keystone.user_get', Mock(return_value=user)).start()
        ...
        patch('identity.keystone.Keystone.project_get', Mock(return_value=project)).start()
        self.request.method = 'POST'
        post = self.request.POST.copy()
        post.update({'id':1,  'name':'aaa',  'project':1})
        self.request.POST = post
        self.view(self.request)
        mock_user_update.assert_called_with(user, name='aaa', project=(project.id), domain=None,
          enabled=True,
          password=None,
          email=None)
...
class DeleteUserTest(TestCase):
    ...
    def test_user_delete_method_was_called(self, mock_user_delete):
        patch('identity.keystone.Keystone.user_get', Mock(return_value=(FakeResource(1, 'user1')))).start()
...
class UpdateProjectUserPasswordTest(TestCase):
    ...
    def test_user_update_was_called(self, mock_user_update):
        ...
        patch('identity.keystone.Keystone.create_password', Mock(return_value=password)).start()
        self.view(self.request)
