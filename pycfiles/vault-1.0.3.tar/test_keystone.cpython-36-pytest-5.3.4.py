...
# [GCC 8.4.0]
# Embedded file name: /Users/julio.rama/git/vault-opensource/identity/tests/test_keystone.py
...
class TestKeystoneV3(TestCase):
    __doc__ = ' Test keystone version 3 '
    ...
    def setUp(self):
        ...
        self.project = Project('123', {'id':str(uuid4()), 
         'name':'project_test', 
    ...
    def test_keystone_create_user_with_no_role(self, mock_add_user_role):
        ...
        keystone = Keystone(self.request)
        keystone.user_create('name', email='email@email.com', password='password',
          project_id='project_id',
          enabled=True)
        keystone.conn.users.create.assert_called_with('name', domain=None, email='email@email.com',
          enabled=True,
          password='password',
          project='project_id')
    ...
    def test_vault_create_project(self, mock_gp_create, mock_key_pass, mock_key_user):
        ...
        computed = keystone.vault_project_create((self.project.name), (self.group.id), description=(self.project.description))
        self.mock_project_create.assert_called_with((self.project.name), description=(self.project.description),
          enabled=True)
        mock_swiftop_role = self.mock_keystone_conn.return_value.roles.find.return_value.id
        mock_key_user.assert_called_with(name=('u_{}'.format(self.project.name)), password='password',
          project_id=(self.project.id),
          role_id=mock_swiftop_role)
        mock_gp_create.assert_called_with(group_id=(self.group.id), project_id=(self.project.id),
          owner=1)
        self.assertEqual(computed, expected)
    ...
    def test_vault_create_project_forbidden_on_user_create(self, mock_key_user, mock_project_delete, mock_project_create):
        mock_project_create.return_value = Project('123', {'id':self.project.id, 
    ...
    def test_vault_update_project_keystone(self, mock_prj_owner):
        ...
        keystone = Keystone((self.request), project_name='project_name')
        computed = keystone.vault_project_update((self.project.id), (self.project.name),
          (self.group.id),
          description=(self.project.description))
        expected = {'status':True,  'project':self.mock_project_update.return_value}
        self.assertEqual(computed, expected)
...
class TestKeystoneDeleteProject(TestCase):
    ...
    def setUp(self):
        ...
        self.project = Project('321', {'id':str(uuid4()), 
         'name':'project_test_delete', 
         'description':'project test delete description', 
         'enabled':True})
        self.mock_keystone_conn = patch('identity.keystone.Keystone._create_keystone_connection').start()
    ...
    def test_vault_delete_project(self, mock_endpoints, mock_swift_delete, mock_keystone_delete):
