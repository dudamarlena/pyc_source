...
# [GCC 8.4.0]
# Embedded file name: /Users/julio.rama/git/vault-opensource/identity/tests/test_project_views.py
# Compiled at: 2020-03-19 00:13:32
# Size of source mod 2**32: 10663 bytes
...
class ListProjectTest(TestCase):
    ...
    def test_show_project_list(self):
        ...
        patch('identity.views.Keystone.project_list', Mock(return_value=[fake_project])).start()
        render_mock = patch('identity.views.ListProjectView.render_to_response').start()
...
class CreateProjectTest(TestCase):
    ...
    def setUp(self):
        ...
        post.update({'name':'project-teste', 
         'description':'desc', 
         'enabled':True, 
         'group':self.group.id})
    ...
    def test_invove_vault_project_create_when_request_is_valid(self, mock):
        ...
        mock.assert_called_with('project-teste', group_id, created_by='vault',
          description='desc',
...
class CreateProjectSuccessTest(TestCase):
    ...
    def test_render_success_create_page(self, mock_render, mock_get_endpoints):
        os_endpoints = {'adminURL':'https://adminURL/storage', 
         'publicURL':'https://publicURL/storage', 
         'internalURL':'https://internalURL/storage'}
        bs_enpoints = {'adminURL':'https://adminURL/faas', 
         'publicURL':'https://publicURL/faas', 
         'internalURL':'https://internalURL/faas'}
        mock_get_endpoints.return_value = {'object_store':os_endpoints, 
         'block_storage':bs_enpoints}
        project_create_result = {'user_name':'fake_user', 
...
class UpdateProjectTestTest(TestCase):
    ...
    def setUp(self):
        ...
        patch('identity.views.actionlog.log').start()
        patch('identity.views.log').start()
        patch('identity.views.Keystone.project_get', return_value=(FakeResource(1, 'fake_resource'))).start()
        self.mock_get_context_data = patch('identity.views.UpdateProjectView.get_context_data').start()
...
class DeleteProjectTest(TestCase):
    ...
    def test_get_request_invoke_render_with_form_object(self):
        ...
        form object
        """
        mock_render = patch('identity.views.DeleteProjectView.get_context_data').start()
    ...
    def test_post_invalid_form(self):
        mock_delete = patch('identity.keystone.Keystone.vault_project_delete').start()
        mock_render = patch('identity.views.DeleteProjectView.render_to_response').start()
    ...
    def test_delete_project_connect_with_username_and_password_passed_by_form(self, mock_keystone):
        ...
        mock_keystone.asser_called_with((self.request),
          username='teste_user', password='secret')