...
# [GCC 8.4.0]
# Embedded file name: /Users/julio.rama/git/vault-opensource/storage/tests/test_storage.py
# Compiled at: 2020-04-20 15:18:40
# Size of source mod 2**32: 62398 bytes
...
class BaseTestCase(TestCase):
    ...
    def setUp(self):
        ...
        patch('storage.views.main.actionlog', Mock(return_value=None)).start()
...
class TestStorage(BaseTestCase):
    ...
    def test_create_object_status_201(self, mock_requests_put):
        ...
        expected = reverse('objectview', kwargs={'container':fakecontainer,  'prefix':prefix, 
    ...
    def test_delete_container_without_deleting_objects(self, mock_delete_container, mock_action_log, mock_delete_object, mock_get_object):
        fakecontainer = 'fakecontainer'
    ...
    @patch('storage.views.main.actionlog.log')
    @patch('storage.views.main.client.delete_container')
    def test_delete_container_deleting_objects(self, mock_delete_container, mock_action_log, mock_delete_object, mock_get_container):
        fakecontainer = 'fakecontainer'
    ...
    @patch('storage.views.main.actionlog.log')
    @patch('storage.views.main.client.delete_container')
    def test_delete_container_fail_to_get_objects(self, mock_delete_container, mock_action_log, mock_delete_object, mock_get_container):
        fakecontainer = 'fakecontainer'
        mock_get_container.side_effect = client.ClientException('')
        expected = False
    ...
    @patch('storage.views.main.actionlog.log')
    @patch('storage.views.main.client.delete_container')
    def test_delete_container_fail_to_delete_container(self, mock_delete_container, mock_action_log, mock_delete_object, mock_get_container, mock_log_exception):
        fakecontainer = 'fakecontainer'
    ...
    def test_create_pseudofolder_with_prefix(self, mock_put_object):
        ...
        self.request.POST = post
        response = views.create_pseudofolder(self.request, project_name, 'fakecontainer', prefix)
    ...
    def test_view_delete_object_inside_a_container(self, mock_delete_object):
        ...
        project_name = self.request.session.get('project_name')
        response = views.delete_object_view(self.request, project_name, fakecontainer, fakeobject_name)
        msgs = [msg for msg in self.request._messages]
        self.assertEqual(msgs[0].message, _('Object deleted'))
        headers = dict([i for i in response.items()])
        expected = reverse('objectview', kwargs={'container':fakecontainer,  'project':project_name})
        self.assertEqual(headers['Location'], expected)
    ...
    def test_view_delete_object_inside_a_pseudofolder(self, mock_delete_object):
        ...
        self.assertEqual(msgs[0].message, _('Object deleted'))
        headers = dict([i for i in response.items()])
        expected = reverse('objectview', kwargs={'container':fakecontainer,  'prefix':fakepseudofolder, 
         'project':project_name})
        self.assertEqual(headers['Location'], expected)
    ...
    def test_delete_empty_pseudofolder(self, mock_get_container, mock_delete_object):
        ...
        mock_get_container.return_value = [
         'stats', [{'name': fakepseudofolder}]]
        response = views.delete_pseudofolder(self.request, project_name, fakecontainer, fakepseudofolder)
        msgs = [msg for msg in self.request._messages]
        self.assertTrue(mock_delete_object.called)
    ...
    def test_delete_non_empty_pseudofolder(self, mock_get_container, mock_delete_object):
        ...
        mock_get_container.return_value = [
         'stats', fakepseudofolder_content]
    ...
    def test_upload_view_without_temp_key_without_prefix(self, mock_get_account):
        mock_get_account.return_value = fakes.get_account()
        patch('storage.views.main.get_temp_key', Mock(return_value=None)).start()
        prefix = ''
        fakecontainer = 'fakecontainer'
        project_name = self.request.session.get('project_name')
    ...
    def test_download(self, mock_get):
        ...
        project_name = self.request.session.get('project_name')
        mock_get.return_value = fakes.FakeRequestResponse(content=content, headers=headers)
    ...
    def test_metadataview_return_headers_from_container(self, mock_head):
        ...
        project_name = self.request.session.get('project_name')
        mock_head.return_value = fakes.FakeRequestResponse(content='', headers=headers)
    ...
    def test_metadataview_return_headers_from_object(self, mock_head):
        headers = {'content-type': 'fake/object'}
        mock_head.return_value = fakes.FakeRequestResponse(content='', headers=headers)
        response = views.metadataview(self.request, 'fakecontainer', 'fakeobject')
        self.assertIn('fake/object', response.content.decode('UTF-8'))

    @patch('storage.views.main.actionlog.log')
    @patch('storage.views.main.client.post_container')
    @patch('storage.views.main.client.put_container')
    def test_enable_versioning(self, mock_put_container, mock_post_container, mock_actionlog):
    ...
    @patch('storage.views.main.client.post_container')
    @patch('storage.views.main.client.put_container')
    def test_enable_versioning_fail_to_create_container(self, mock_put_container, mock_post_container, mock_actionlog):
        mock_put_container.side_effect = client.ClientException('')
        container = 'fakecontainer'
        computed = views.enable_versioning(self.request, container)
    ...
    @patch('storage.views.main.client.post_container')
    @patch('storage.views.main.client.put_container')
    def test_enable_versioning_fail_to_update_container(self, mock_put_container, mock_post_container, mock_actionlog):
    ...
    def test_disable_versioning(self, mock_head_container, mock_delete_container, mock_post_container, mock_actionlog):
    ...
    @patch('storage.views.main.delete_container')
    @patch('storage.views.main.client.head_container')
    def test_disable_versioning_fail_to_get_container_headers(self, mock_head_container, mock_delete_container, mock_post_container, mock_logging):
    ...
    @patch('storage.views.main.delete_container')
    @patch('storage.views.main.client.head_container')
    def test_disable_versioning_fail_to_update_container_header(self, mock_head_container, mock_delete_container, mock_post_container, mock_logging):
        mock_post_container.side_effect = client.ClientException('')
        version_location = '_version_fakecontainer'
        mock_head_container.return_value = {'x-versions-location': version_location}
        container = 'fakecontainer'
    ...
    @patch('storage.views.main.client.head_container')
    def test_object_versioning_view_versioning_disabled(self, mock_head_container, mock_render):
    ...
    @patch('storage.views.main.client.get_container')
    @patch('storage.views.main.client.head_container')
    def test_object_versioning_view_versioning_enabled(self, mock_head_container, mock_get_container, mock_render):
    ...
    def test_object_versioning_view_enabling_versioning(self, mock_enable):
        ...
        headers = dict([i for i in response.items()])
        expected = reverse('object_versioning', kwargs={'project':project_name, 
         'container':'fakecontainer'})
        self.assertEqual(headers['Location'], expected)
...
class TestStorageAcls(BaseTestCase):
    ...
    def test_edit_acl_list_acls_container_public(self, mock_get_container):
        ...
        """
        mock_get_container.return_value = {'x-container-read':'.r:*', 
         'x-container-write':''}
        project_name = self.request.session.get('project_name')
    ...
    def test_edit_acl_list_acls_container_private(self, mock_get_container):
        ...
        """
        mock_get_container.return_value = {'x-container-read':'', 
         'x-container-write':''}
        project_name = self.request.session.get('project_name')
        response = views.edit_acl(self.request, project_name, 'fakecontainer')
        self.assertEqual(response.status_code, 200)
    ...
    def test_edit_acl_private_container_but_public_for_read_and_write_for_an_user_and_project(self, mock_head_container):
        ...
        if the "Make Public" action is available
        """
        mock_head_container.return_value = {'x-container-read':'projectfake:userfake', 
    ...
    def test_edit_acl_public_container_and_public_for_read_for_more_than_one_user_and_project(self, mock_head_container):
        ...
        """
        mock_head_container.return_value = {'x-container-read':'.r:*,projectfake:userfake', 
    ...
    def test_edit_acl_grant_read_and_write_permission_for_a_project_and_user(self, mock_head_container, mock_post_container):
        mock_head_container.return_value = {'x-container-read':'', 
         'x-container-write':''}
        self.request.method = 'POST'
        post = self.request.POST.copy()
        post.update({'username':'projectfake:userfake', 
         'read':'On', 
         'write':'On'})
        self.request.POST = post
        project_name = self.request.session.get('project_name')
        ...
        expected_arg = {'X-Container-Write':',projectfake:userfake', 
    ...
    def test_edit_acl_make_private(self, mock_head_container, mock_post_container):
        ...
        self.assertTrue(mock_post_container.called)
        name, args, kargs = mock_post_container.mock_calls[0]
        expected_arg = {'X-Container-Write':'projectfake2:userfake2,', 
         'X-Container-Read':'projectfake:userfake,'}
        self.assertEqual(expected_arg, kargs['headers'])
...
class TestStorageCORS(BaseTestCase):
    ...
    def test_define_novo_host_para_regra_de_cors_no_container(self, mock_head_container, mock_post_container):
        mock_head_container.return_value = {'x-container-meta-access-control-allow-origin': ''}
        self.request.method = 'POST'
        self.request.META.update({'HTTP_HOST': 'localhost'})
        post = self.request.POST.copy()
        project_name = self.request.session.get('project_name')
    ...
    def test_remove_host_da_regra_de_cors_do_container(self, mock_head_container, mock_post_container):
        mock_head_container.return_value = {'x-container-meta-access-control-allow-origin': 'globo.com globoi.com'}
        self.request.method = 'GET'
        self.request.META.update({'HTTP_HOST': 'localhost'})
        get = self.request.GET.copy()
        project_name = self.request.session.get('project_name')
