...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mathbench/lab/notebook.py
# Compiled at: 2008-04-09 04:20:52
"""
Inspired by
...
"""
import sys, os, wx, wx.lib.dialogs, wx.py, wx.py.frame as PyFrame, wx.py.dispatcher as dispatcher
...
class LabBook(EditorNotebookFrame):
    ...
    def __init__(self, parent=None, id=-1, title='MathBench', config=None, pos=wx.DefaultPosition, size=(800, 600), style=wx.DEFAULT_FRAME_STYLE | wx.NO_FULL_REPAINT_ON_RESIZE, filename=None, history_dir=None):
    ...
    def __new__(cls, parent=None, id=-1, title='Math Bench', config=None, pos=wx.DefaultPosition, size=(800, 600), style=wx.DEFAULT_FRAME_STYLE | wx.NO_FULL_REPAINT_ON_RESIZE, filename=None, history_dir=None):
        """
    ...
    def _init_notebook(self):
        ...
        dispatcher.connect(receiver=self._newPageOnNotebook, signal='NewPageOnNotebook', sender=self.notebook)
    ...
    def _init_helpbar(self):
        ...
        img = art_prov.GetBitmap(wx.ART_HELP_BOOK, wx.ART_TOOLBAR, tb_img_size)
        ...
        dummylongtext.Hide()
        self.query_widget = wx.TextCtrl(name='query', parent=self.helpbar, size=wx.Size(nice_width, -1))
        ...
        img = art_prov.GetBitmap(wx.ART_DELETE, wx.ART_TOOLBAR, tb_img_size)
        ...
        img = art_prov.GetBitmap(wx.ART_FIND, wx.ART_TOOLBAR, tb_img_size)
    ...
    def _modify_menus(self):
        ...
        m.Append(PyFrame.ID_NEW, '&New \tCtrl+N', 'New file')
        m.Append(PyFrame.ID_OPEN, '&Open... \tCtrl+O', 'Open file')
        m.AppendSeparator()
        m.Append(PyFrame.ID_REVERT, '&Revert \tCtrl+R', 'Revert to last saved version')
        m.Append(PyFrame.ID_CLOSE, '&Close \tCtrl+W', 'Close file')
        m.AppendSeparator()
        m.Append(PyFrame.ID_SAVE, '&Save... \tCtrl+S', 'Save file')
        m.Append(PyFrame.ID_SAVEAS, 'Save &As \tCtrl+Shift+S', 'Save file with new name')
        m.AppendSeparator()
        m.Append(PyFrame.ID_PRINT, '&Print... \tCtrl+P', 'Print file')
        m.AppendSeparator()
        m.Append(ID_HISTORY_TO_SCRIPT, 'Session &To Script', "Create a script from the session's history")
        m.Append(ID_LOAD_OLD_SESSION, 'Recover &Older Session', 'Load (and exectute) the history of a previous session')
        m.AppendSeparator()
        m.Append(ID_EXECUTE, '&Execute \tF9', 'Execute the script in a new shell')
        m.Append(ID_EXECUTE_MAIN, 'Execute in &main shell \tShift+F9', 'Execute the script in the main shell')
        m.Append(PyFrame.ID_NAMESPACE, '&Update Namespace \tCtrl+Shift+N', 'Update namespace for autocompletion and calltips')
        m.AppendSeparator()
        m.Append(PyFrame.ID_EXIT, 'E&xit\tCtrl+Q', 'Exit Program')
        m = self.editMenu = wx.Menu()
        m.Append(PyFrame.ID_UNDO, '&Undo \tCtrl+Z', 'Undo the last action')
        m.Append(PyFrame.ID_REDO, '&Redo \tCtrl+Y', 'Redo the last undone action')
        m.AppendSeparator()
        m.Append(PyFrame.ID_CUT, 'Cu&t \tCtrl+X', 'Cut the selection')
        m.Append(PyFrame.ID_COPY, '&Copy \tCtrl+C', 'Copy the selection')
        m.Append(PyFrame.ID_COPY_PLUS, 'Cop&y Plus \tCtrl+Shift+C', 'Copy the selection - retaining prompts')
        m.Append(PyFrame.ID_PASTE, '&Paste \tCtrl+V', 'Paste from clipboard')
        m.Append(PyFrame.ID_PASTE_PLUS, "Past&e'n'run \tCtrl+Shift+V", 'Paste and run commands')
        m.AppendSeparator()
        m.Append(PyFrame.ID_EMPTYBUFFER, 'E&mpty Buffer...', 'Delete all the contents of the edit buffer')
        m.Append(PyFrame.ID_SELECTALL, 'Select A&ll \tCtrl+A', 'Select all text')
        if wx.Platform == '__WXGTK__':
            m.AppendSeparator()
            m.Append(ID_PREFERENCE, '&Preferences', 'Open the preference panel')
        m = self.autocompMenu = wx.Menu()
        m.Append(PyFrame.ID_AUTOCOMP_SHOW, 'Show &Auto Completion\tCtrl+Shift+A', 'Show auto completion list', wx.ITEM_CHECK)
        m.Append(PyFrame.ID_AUTOCOMP_MAGIC, 'Include &Magic Attributes\tCtrl+Shift+M', 'Include attributes visible to __getattr__ and __setattr__', wx.ITEM_CHECK)
        m.Append(PyFrame.ID_AUTOCOMP_SINGLE, 'Include Single &Underscores\tCtrl+Shift+U', 'Include attibutes prefixed by a single underscore', wx.ITEM_CHECK)
        m.Append(PyFrame.ID_AUTOCOMP_DOUBLE, 'Include &Double Underscores\tCtrl+Shift+D', 'Include attibutes prefixed by a double underscore', wx.ITEM_CHECK)
        m = self.calltipsMenu = wx.Menu()
        m.Append(PyFrame.ID_CALLTIPS_SHOW, 'Show Call &Tips\tCtrl+Shift+T', 'Show call tips with argument signature and docstring', wx.ITEM_CHECK)
        m.Append(PyFrame.ID_CALLTIPS_INSERT, '&Insert Call Tips\tCtrl+Shift+I', '&Insert Call Tips', wx.ITEM_CHECK)
        m = self.viewMenu = wx.Menu()
        m.AppendMenu(PyFrame.ID_AUTOCOMP, '&Auto Completion', self.autocompMenu, 'Auto Completion Options')
        m.AppendMenu(PyFrame.ID_CALLTIPS, '&Call Tips', self.calltipsMenu, 'Call Tip Options')
        m.Append(PyFrame.ID_WRAP, '&Wrap Lines\tCtrl+Shift+W', 'Wrap lines at right edge', wx.ITEM_CHECK)
        ...
        if wx.Platform == '__WXMAC__':
            m.AppendSeparator()
            m.Append(ID_LIBRARY, '&Library desk', 'Show the library desk and its useful pointers')
        ...
        m.Append(PyFrame.ID_FIND, '&Find Text... \tCtrl+F', 'Search for text in the edit buffer')
        m.Append(PyFrame.ID_FINDNEXT, 'Find &Next \tF3', 'Find next/previous instance of the search text')
    ...
    def _checkCurrentBufferSaved(self, title='Savind required'):
        ...
        if not self.bufferHasChanged():
            ...
            d = wx.MessageDialog(self, 'You need to save the script first.\nDo you want to do is now ?', '%s' % title, wx.YES_NO | wx.ICON_WARNING)
            yes_no = d.ShowModal()
            if yes_no != wx.ID_YES:
                return False
    ...
    def bufferPrint(self):
        ...
        if isinstance(self.editor, wx.py.shell.Shell):
            printer.Print(self.editor.GetText(), '*Shell*')
        else:
            printer.Print(self.editor.getText(), self.editor.buffer.name)
    ...
    def OnPreference(self, event):
        ...
        if self.options_frame is None:
            ...
            dispatcher.connect(signal='OptionsChanged', receiver=self._onOptionsChanged, sender=self.options_frame)
            dispatcher.connect(signal='OptionsClosed', receiver=self._onOptionsClosed, sender=self.options_frame)
        else:
    ...
    def OnLoadOldSession(self, event):
        """
                ...
                """
        d = wx.FileDialog(self, 'Please chose a session to load', self.history_dir, wildcard='*.session', style=wx.FD_OPEN)
