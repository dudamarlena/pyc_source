...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mathbench/ext/pluginmanagement_widgets.py
# Compiled at: 2008-04-09 11:15:56
"""
...
import wx.lib.hyperlink as hlk, logging
...
class SimplePluginPanel(wx.Panel):
    ...
    def create(self):
        ...
        if hasattr(self.plugin_info, 'website'):
            hyper1 = hlk.HyperLinkCtrl(self, wx.ID_ANY, 'Website', URL=self.plugin_info.website)
        ...
        if hasattr(self.plugin_info, 'path'):
            ...
            def on_reveal(event):
                """
                                ...
                                """
                dlg = wx.FileDialog(self, message="Plugin's files", defaultDir=self.plugin_info.path, defaultFile='', style=wx.FD_CHANGE_DIR)
                dlg.ShowModal()
                dlg.Destroy()
...
class PluginsConfigPanel(wx.Panel):
    ...
    def applyPreferences(self):
        ...
        self.list.update()
        d = wx.MessageDialog(self, 'Changes have been applied.\n\nPlease restart for them to take effect.', 'Plugin [de]activations', style=wx.OK | wx.ICON_INFORMATION)
        d.ShowModal()
        d.Destroy()
    ...
    def install_a_plugin(self):
        """
                ...
                """
        d = wx.FileDialog(self, message='Select the plugin to install', wildcard='*.%s' % self.plugin_manager.plugin_info_ext, style=wx.FD_OPEN)
        ...
        if ok_cancel == wx.ID_OK and os.path.isfile(os.path.join(current_directory, plugin_info_filename)):
            ...
            if not install_ok:
                d = wx.MessageDialog(self, 'Sorry, the required plugin could not be installed !', 'Plugin installation', style=wx.OK | wx.ICON_ERROR)
                d.ShowModal()
                d.Destroy()
            else:
