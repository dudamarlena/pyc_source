...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyogp/lib/client/inventory.py
...
class InventoryManager(DataManager):
    ...
    def search_inventory(self, folder_list=[], item_id=None, name=None, match_list=[]):
        ...
        for item in search_folders:
            ...
            if isinstance(item, InventoryFolder):
                matches = self.search_inventory_folder(folder_list, item.FolderID, _id=item_id, name=name)
    ...
    def _store_inventory_folder(self, folder_data, destination='inventory'):
        ...
        if len(index) == 1:
            if self.settings.LOG_VERBOSE:
                logger.debug("Replacing a stored inventory folder: %s for agent '%s'" % (folder.FolderID, self.agent.agent_id))
            contents = container[index[0]].inventory
            container[index[0]] = folder
            container[index[0]].inventory = contents
        else:
            if self.settings.LOG_VERBOSE:
                logger.debug('Storing inventory folder %s' % folder.Name)
    ...
    def send_FetchInventory(self, agent_id, session_id, owner_id, inventory_ids):
        ...
        """
        packet = Message('FetchInventory', Block('AgentData', AgentID=agent_id, SessionID=session_id), *[ Block('InventoryData', OwnerID=owner_id, ItemID=item_id) for item_id in inventory_ids ])
    ...
    def handle_inventory_offer(self, packet):
        ...
        ToAgentID = packet['MessageBlock'][0]['ToAgentID']
        BinaryBucket = packet['MessageBlock'][0]['BinaryBucket']
    ...
    def create_new_item(self, folder, name, desc, asset_type, inv_type, wearable_type, next_owner_permission, callback=None):
        """
        ...
        def onUpdateCreateInventoryItem(packet):
            if str(transaction_id) != str(packet['AgentData'][0]['TransactionID']):
                ...
                item = InventoryItem(inv_data['ItemID'], inv_data['FolderID'], inv_data['CreatorID'], inv_data['OwnerID'], inv_data['GroupID'], inv_data['BaseMask'], inv_data['OwnerMask'], inv_data['GroupMask'], inv_data['EveryoneMask'], inv_data['NextOwnerMask'], inv_data['GroupOwned'], inv_data['AssetID'], inv_data['Type'], inv_data['InvType'], inv_data['Flags'], inv_data['SaleType'], inv_data['SalePrice'], inv_data['Name'], inv_data['Description'], inv_data['CreationDate'], inv_data['CRC'])
        ...
        self.send_CreateInventoryItem(self.agent.agent_id, self.agent.session_id, 0, folder.FolderID, transaction_id, next_owner_permission, asset_type, inv_type, wearable_type, name, desc)

    def send_CreateInventoryItem(self, agent_id, session_id, callback_id, folder_id, transaction_id, next_owner_mask, type_, inv_type, wearable_type, name, desc):
        """ sends a CreateInventoryItem message """
        args = [
         Block('AgentData', AgentID=agent_id, SessionID=session_id)]
        args += [Block('InventoryBlock', CallbackID=callback_id, FolderID=folder_id, TransactionID=transaction_id, NextOwnerMask=next_owner_mask, Type=type_, InvType=inv_type, WearableType=wearable_type, Name=name, Description=desc)]
        self.agent.region.enqueue_message(Message('CreateInventoryItem', *args))

    def remove_inventory_item(self, item, folder, folder_list):
        folder.inventory.remove(item)
        self.send_RemoveInventoryItem(self.agent.agent_id, self.agent.session_id, item.ItemID)

    def send_RemoveInventoryItem(self, agent_id, session_id, item_id):
        """ sends a RemoveInventoryItem message """
        args = [
         Block('AgentData', AgentID=agent_id, SessionID=session_id)]
        args += [Block('InventoryData', ItemID=item_id)]
...
class UDP_Inventory(InventoryManager):
    ...
    def sendFetchInventoryDescendentsRequest(self, folder_id=None):
        """ send a request to the grid for folder contents """
        packet = Message('FetchInventoryDescendents', Block('AgentData', AgentID=self.agent.agent_id, SessionID=self.agent.session_id), Block('InventoryData', FolderID=UUID(str(folder_id)), OwnerID=self.agent.agent_id, SortOrder=0, FetchFolders=True, FetchItems=True))
...
class InventoryFolder(object):
    ...
    def send_PurgeInventoryDescendents(self, agent, agent_id, session_id, folder_id):
        ...
        packet = Message('PurgeInventoryDescendents', Block('AgentData', AgentID=agent_id, SessionID=session_id), Block('InventoryData', FolderID=folder_id))
    ...
    def send_MoveInventoryFolder(self, agent, agent_id, session_id, restamp, folder_id, parent_id):
        ...
        packet = Message('MoveInventoryFolder', Block('AgentData', AgentID=agent_id, SessionID=session_id, Stamp=restamp), Block('InventoryData', FolderID=folder_id, ParentID=parent_id))
        agent.region.enqueue_message(packet)
    ...
    def send_RemoveInventoryFolder(self, agent, agent_id, session_id, restamp, folder_id):
        ...
        packet = Message('RemoveInventoryFolder', Block('AgentData', AgentID=agent_id, SessionID=session_id, Stamp=restamp), Block('InventoryData', FolderID=folder_id))
        agent.region.enqueue_message(packet)
...
class InventoryItem(object):
    ...
    def __init__(self, ItemID=None, FolderID=None, CreatorID=None, OwnerID=None, GroupID=None, BaseMask=None, OwnerMask=None, GroupMask=None, EveryoneMask=None, NextOwnerMask=0, GroupOwned=None, AssetID=None, Type=None, InvType=None, Flags=None, SaleType=None, SalePrice=None, Name=None, Description=None, CreationDate=None, CRC=None, LastOwnerID=UUID()):
        ...
        self.ItemID = UUID(str(ItemID))
        self.FolderID = UUID(str(FolderID))
        self.CreatorID = UUID(str(CreatorID))
        self.OwnerID = UUID(str(OwnerID))
    ...
    def give(self, agent, to_agent_id):
        ...
        _Message = self.Name
        _BinaryBucket = struct.pack('>b', self.Type) + self.ItemID.get_bytes()
