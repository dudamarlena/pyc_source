...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyogp/lib/client/agent.py
...
import re, sys, signal, sets, struct
...
class Agent(object):
    ...
    def _parse_login_response(self):
        ...
        if self.grid_type == 'Legacy':
            self.firstname = re.sub('\\"', '', self.login_response['first_name'])
    ...
    def send_AgentDataUpdateRequest(self):
        ...
        packet = Message('AgentDataUpdateRequest', Block('AgentData', AgentID=self.agent_id, SessionID=self.session_id))
        self.region.enqueue_message(packet)

    def say(self, _Message, Type=1, Channel=0):
        ...
        Type: 0 = Whisper
              ...
              2 = Shout
        """
        packet = Message('ChatFromViewer', Block('AgentData', AgentID=self.agent_id, SessionID=self.session_id), Block('ChatData', Message=_Message, Type=Type, Channel=Channel))
        self.region.enqueue_message(packet)

    def instant_message(self, ToAgentID=None, _Message=None, _ID=None):
    ...
    def send_ImprovedInstantMessage(self, AgentID=None, SessionID=None, FromGroup=None, ToAgentID=None, ParentEstateID=None, RegionID=None, Position=None, Offline=None, Dialog=None, _ID=None, Timestamp=None, FromAgentName=None, _Message=None, BinaryBucket=None):
    ...
    def send_RetrieveInstantMessages(self):
        ...
        packet = Message('RetrieveInstantMessages', Block('AgentData', AgentID=self.agent_id, SessionID=self.session_id))
        self.region.enqueue_message(packet())
    ...
    def onChatFromSimulator(self, packet):
        ...
        message = AppEvent('ChatReceived', FromName=FromName, SourceID=SourceID, OwnerID=OwnerID, SourceType=SourceType, ChatType=ChatType, Audible=Audible, Position=Position, Message=_Message)
    ...
    def teleport(self, region_name=None, region_handle=None, region_id=None, landmark_id=None, position=Vector3(X=128, Y=128, Z=128), look_at=Vector3(X=128, Y=128, Z=128)):
        ...
        if landmark_id:
            ...
            packet = Message('TeleportLandmarkRequest', Block('Info', AgentID=self.agent_id, SessionID=self.session_id, LandmarkID=UUID(landmark_id)))
            self.region.enqueue_message(packet)
            return
        ...
        if region_id:
            ...
            packet = Message('TeleportRequest', Block('AgentData', AgentID=self.agent_id, SessionID=self.session_id), Block('Info', RegionID=region_id, Position=position, LookAt=look_at))
            self.region.enqueue_message(packet)
        elif region_handle:
            logger.info('sending TP location request packet')
            packet = Message('TeleportLocationRequest', Block('AgentData', AgentID=self.agent_id, SessionID=self.session_id), Block('Info', RegionHandle=region_handle, Position=position, LookAt=look_at))
            self.region.enqueue_message(packet)
        else:
            logger.info("Target region's handle not known, sending map name request")
            self.map_service.request_handle(region_name, lambda handle: self.teleport(region_handle=handle, position=position, look_at=look_at))
    ...
    def onTeleportFinish(self, packet):
        ...
        self._enable_current_region(region_x=region_x, region_y=region_y, seed_capability=packet['Info'][0]['SeedCapability'], sim_ip=sim_ip, sim_port=packet['Info'][0]['SimPort'])
        return
    ...
    def request_agent_names(self, agent_ids, callback):
        ...
        def _fire_callback(_):
            cbdata = [ (agent_id, self.agent_id_map[agent_id][0], self.agent_id_map[agent_id][1]) for agent_id in agent_ids ]
            callback(cbdata)

        names_to_request = [ agent_id for agent_id in agent_ids if agent_id not in self.agent_id_map ]
        if names_to_request:
    ...
    def send_UUIDNameRequest(self, agent_ids, callback):
        ...
        def onUUIDNameReply(packet):
            ...
            missing = [ agent_id for agent_id in agent_ids if agent_id not in self.agent_id_map ]
            if not missing:
        ...
        packet = Message('UUIDNameRequest', [ Block('UUIDNameBlock', ID=UUID(agent_id)) for agent_id in agent_ids ])
        self.region.enqueue_message(packet)
    ...
    def request_balance(self, callback):
        ...
        def onMoneyBalanceReply(packet):
            ...
            handler.unsubscribe(onMoneyBalanceReply)
            balance = packet['MoneyData'][0]['MoneyBalance']
            callback(balance)

        handler.subscribe(onMoneyBalanceReply)
        logger.info('sending MoneyBalanceRequest')
        packet = Message('MoneyBalanceRequest', Block('AgentData', AgentID=self.agent_id, SessionID=self.session_id), Block('MoneyData', TransactionID=UUID()))
        self.region.enqueue_message(packet)

    def give_money(self, target_id, amount, description='', transaction_type=MoneyTransactionType.Gift, flags=TransactionFlags.Null):
        """Give money to another agent"""
        logger.info('sending MoneyTransferRequest')
        packet = Message('MoneyTransferRequest', Block('AgentData', AgentID=self.agent_id, SessionID=self.session_id), Block('MoneyData', SourceID=self.agent_id, DestID=UUID(target_id), Flags=flags, Amount=amount, AggregatePermNextOwner=0, AggregatePermInventory=0, TransactionType=transaction_type, Description=description))
        self.region.enqueue_message(packet)
