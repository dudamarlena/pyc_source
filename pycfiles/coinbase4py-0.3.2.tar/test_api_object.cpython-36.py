...
class TestNewApiObject(unittest2.TestCase):
    ...
    def test_new_api_object(self):
        ...
        warnings = lambda x: x
        obj = new_api_object(api_client,
          simple_data, response=response, pagination=pagination, warnings=warnings)
    ...
    def test_new_api_object_guesses_based_on_resource_field(self):
        ...
        coinbase.wallet.model._resource_to_model = {'foo': Foo}
        obj = new_api_object(api_client, simple_data)
        self.assertIsInstance(obj, Foo)
    ...
    def test_attr_access(self):
        ...
        if not broken_key not in simple_obj:
            raise AssertionError
        else:
            if not not hasattr(simple_obj, broken_key):
                raise AssertionError
            else:
