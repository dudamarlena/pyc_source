...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/tools/shadersmanager/widgets/shaderexporter.py
# Compiled at: 2020-03-13 14:15:56
...
class ArtellaShaderExporterWidget(base.BaseWidget, object):
    ...
    def export(self, new_version=False, comment=None):
        ...
        if self.do_export.isChecked():
            if self._asset:
                exported_shader = artellapipe.ShadersMgr().export_asset_shaders(asset=(self._asset),
                  shader_swatch=(self._shader_swatch),
                  new_version=new_version,
                  comment=comment,
                  shaders_to_export=[self._name])
            else:
                exported_shader = artellapipe.ShadersMgr().export_shader(shader_name=(self._name),
                  shader_swatch=(self._shader_swatch),
                  new_version=new_version,
                  comment=comment)
...
class ShaderExporter(base.BaseWidget, object):
    ...
    def export_shaders(self, new_version=False, comment=None):
        ...
        if self._shaders_list.count() <= 0:
            ...
            if self._asset:
                if self.export_shader_mapper_cbx.isChecked():
                    self._progress_text.setText('Exporting Shaders Mapping File ... Please wait!')
                    self.repaint()
                    artellapipe.ShadersMgr().export_asset_shaders_mapping((self._asset),
                      comment=comment, new_version=new_version)
            return exported_shaders
    ...
    def publish_shaders(self, comment=None):
        ...
        if not publish_path:
            LOGGER.warning('Impossible to publish shaders. Was not possible to found publish path. Publish shaders manually.')
            return
    ...
    def _open_asset_shaders_file(self):
        ...
        file_path = self._asset.get_file(file_type=shading_file_type,
    ...
    def _on_export_shaders(self):
        ...
        self.repaint()
        try:
            self.export_shaders(new_version=(self.upload_new_version_cbx.isChecked()),
