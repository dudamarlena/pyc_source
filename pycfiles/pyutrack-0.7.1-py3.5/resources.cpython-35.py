...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyutrack/resources.py
...
class Role(object):
    ...
    __create__ = {'url': 'admin/role/%(name)s', 
     'args': ('name', ), 
     'kwargs': {'description': ''}}
    __delete__ = {'url': 'admin/role/%(name)s'}
    __update__ = {'url': 'admin/role/%(name)s', 
     'args': ('description', 'newName')}
    __list__ = {'url': 'admin/role', 'hydrate': True}
    __label__ = '%(name)s'
    __render__ = ('name', 'description')
    __associations__ = {'permissions': {'type': Permission, 
                     'get': {'url': 'admin/role/%(name)s/permission', 
                             'hydrate': False}, 
                     
                     'add': {'url': 'admin/role/%(name)s/permission/%(permission)s', 
                             'key': 'permission', 
                             'method': 'post'}, 
                     
                     'remove': {'url': 'admin/role/%(name)s/permission/%(permission)s', 
                                'key': 'permission'}}}
...
class Group(object):
    ...
    __create__ = {'url': 'admin/group/%(name)s', 
     'args': ('name', ), 
     'kwargs': {'autoJoin': False, 
                'description': ''}}
    __delete__ = {'url': 'admin/group/%(name)s'}
    __update__ = {'url': 'admin/group/%(name)s', 
     'args': ('description', 'autoJoin', 'newName')}
    __list__ = {'url': 'admin/group', 'hydrate': True}
    __render__ = ('name', 'description', 'autoJoin')
    __label__ = '%(name)s'
    __associations__ = {'roles': {'type': Role, 
               'get': {'url': 'admin/group/%(name)s/role', 
                       'hydrate': True}, 
               
               'add': {'url': 'admin/group/%(name)s/role/%(role)s', 
                       'key': 'role', 
                       'method': 'put'}, 
               
               'remove': {'url': 'admin/group/%(name)s/role/%(role)s', 
                          'key': 'role'}}}
...
class User(object):
    ...
    __create__ = {'url': 'admin/user/%(login)s', 
     'args': ('login', 'fullName', 'email', 'password')}
    __delete__ = {'url': 'admin/user/%(login)s'}
    __update__ = {'url': 'admin/user', 
     'kwargs': {'login': '', 
                'fullName': '', 
                'email': '', 
                'password': ''}}
    __list__ = {'url': 'admin/user?q=%(query)s&role=%(role)s&permission=%(permission)s&group=%(group)s', 
    ...
    __associations__ = {'groups': {'type': Group, 
                'get': {'url': 'admin/user/%(login)s/group', 
                        'hydrate': False}, 
                
                'add': {'url': 'admin/user/%(login)s/group/%(group)s', 
                        'key': 'group', 
                        'method': 'post'}, 
                
                'remove': {'url': 'admin/user/%(login)s/group/%(group)s', 
                           'key': 'group'}}, 
     
     'roles': {'type': Role, 
               'get': {'url': 'admin/user/%(login)s/role', 
                       'hydrate': True}}}
...
class Issue(object):
    ...
    __create__ = {'url': 'issue/', 
     'args': ('project', ), 
     'kwargs': {'summary': '', 
                'description': ''}}
    __delete__ = {'url': 'issue/%(id)s'}
    __update__ = {'url': 'issue/%(id)s/', 
     'kwargs': {'summary': '', 
                'description': ''}}
    __list__ = {'url': 'issue?filter=%(filter)s&max=%(max)d', 
     'args': ('filter', ), 
     'kwargs': {'max': 100}, 
    ...
    __label__ = '%(id)s'
    __attributes__ = {'id': 'id', 
     'assignee': 'Assignee/0/value', 
     'reporter': 'reporterName', 
     'updater': 'updaterName', 
     'priority': 'Priority', 
     'status': 'Status'}
    __render__ = ('id', 'summary', 'status', 'assignee', 'reporter', 'updater', 'priority',
                  'link')
    __render_min__ = ('id', 'summary')
    __associations__ = {'issue_links': {'type': IssueLink, 
    ...
    def command(self, command=None, comment=None):
        ...
        :param str command: The youtrack command to execute. See
         ...
         for command grammar.
        """
        url = 'issue/%(id)s/execute' % {'id': self.id}
        self.connection.post(url, {'command': command, 'comment': comment}, parse=False)
        self.get()
...
class Project(object):
    ...
    __create__ = {'url': 'admin/project/%(projectId)s', 
     'args': ('projectName', 'projectLeadLogin'), 
     'kwargs': {'projectId': None, 
                'startingNumber': 1, 
                'description': ''}}
    __delete__ = {'url': 'admin/project/%(projectId)s'}
    __list__ = {'url': 'project/all?verbose=true', 'hydrate': False}
    __associations__ = {'issues': {'type': Issue, 
                'get': {'url': 'issue/byproject/%(projectId)s?filter=%(filter)s&max=%(max)d', 
