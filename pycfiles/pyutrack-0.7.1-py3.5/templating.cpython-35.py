...
class ProxyTemplate(object):
    ...
    def render(self, object):
        return self.compiled_template.render({k:object.attribute(k) for k in self.template_requirements})
...
class BuiltinTemplate(ProxyTemplate):
    ...
    def __init__(self, resource_type):
        ...
        super(BuiltinTemplate, self).__init__(self.header() + self.field_sep.join([self.field_template(field) for field in self.fields]) + self.footer())
...
BUILTIN_TEMPLATES = {'csv': CSVTemplate, 
 'tsv': TSVTemplate, 
 'oneline': CSVTemplate, 
 'tree': TreeTemplate, 
