...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyutrack/cli/list.py
...
def users(ctx, query, group, role, permission, project):
    ...
    list users
    """
    return User.list(ctx.obj.connection, query=query, group=group, role=role, permission=permission, project=project)
