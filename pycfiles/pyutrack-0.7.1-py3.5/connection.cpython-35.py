...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyutrack/connection.py
...
class Connection(object):
    ...
    def __init__(self, base_url=None, credentials=None):
        self._Connection__session = requests.Session()
        self._Connection__session.debug = True
        self._Connection__session.headers.update({'Accept': 'application/json'})
        self._Connection__session_args = {}
        self._Connection__credentials = credentials
        self._Connection__base_url = None
        if self.credentials and self.credentials.cookies:
            self._Connection__session.cookies.update(credentials.cookies)
        self._Connection__api_url = ''
        if base_url:
            self.api_url = base_url

    def login(self, persist_credentials=True):
        try:
            self.post('user/login', {'login': self.credentials.username, 
             'password': self.credentials.password}, False)
            self.credentials.cookies = self._Connection__session.cookies.get_dict()
    ...
    def credentials(self):
        return self._Connection__credentials
    ...
    @property
    def api_url(self):
        return self._Connection__api_url

    @api_url.setter
    def api_url(self, api_url):
        ref = api_url.rstrip('/')
        if ref.endswith('/rest'):
            self._Connection__api_url = ref
        else:
            self._Connection__api_url = '%s/rest' % ref
        self._Connection__base_url = self._Connection__api_url.replace('/rest', '')

    @property
    def base_url(self):
        return self._Connection__base_url

    @fix_auth
    def get(self, path, parse=True):
        return self._Connection__unwrap(self._Connection__session.get('%s/%s' % (self._Connection__api_url, path), **self._Connection__session_args), parse)

    @fix_auth
    def post(self, path, data, parse=True):
        return self._Connection__unwrap(self._Connection__session.post('%s/%s' % (self._Connection__api_url, path), data, **self._Connection__session_args), parse)

    @fix_auth
    def put(self, path, data, parse=True):
        return self._Connection__unwrap(self._Connection__session.put('%s/%s' % (self._Connection__api_url, path), data, **self._Connection__session_args), parse)

    @fix_auth
    def delete(self, path, parse=False):
        return self._Connection__unwrap(self._Connection__session.delete('%s/%s' % (self._Connection__api_url, path), **self._Connection__session_args), parse)
    ...
    def __unwrap(self, response, parse):
        ...
        if response.status_code == 201 and response.headers.get('location'):
            return self._Connection__session.get(response.headers['location']).json()
        else:
            return parse or response.content
        try:
