...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/vcard/vcard.py
# Compiled at: 2010-08-11 07:40:15
...
"""
import codecs, getopt, re, sys, warnings
from vcard_defs import ALL_PROPERTIES, CRLF_CHARS, ID_CHARS, MANDATORY_PROPERTIES, MSG_CONTINUATION_AT_START, MSG_DOT_AT_LINE_START, MSG_EMPTY_VCARD, MSG_INVALID_LINE_SEPARATOR, MSG_INVALID_PARAM_NAME, MSG_INVALID_PROPERTY_NAME, MSG_INVALID_SUBVALUE, MSG_INVALID_VALUE, MSG_MISMATCH_GROUP, MSG_MISSING_GROUP, MSG_MISSING_PARAM_VALUE, MSG_MISSING_PROPERTY, MSG_MISSING_VALUE_STRING, QSAFE_CHARS, SAFE_CHARS, SP_CHAR, VALUE_CHARS, VCARD_LINE_MAX_LENGTH_RAW
import vcard_utils, vcard_validators
...
def unfold_vcard_lines(lines):
    ...
    for index in range(len(lines)):
        ...
        if not line.endswith(CRLF_CHARS):
            raise vcard_validators.VCardFormatError(MSG_INVALID_LINE_SEPARATOR, {'File line': index + 1})
        if len(line) > VCARD_LINE_MAX_LENGTH_RAW:
            warnings.warn('Long line in vCard: %s' % line.encode('utf-8'))
        if line.startswith(' '):
            if index == 0:
                raise vcard_validators.VCardFormatError(MSG_CONTINUATION_AT_START, {'File line': index + 1})
            ...
            property_lines[-1] = property_lines[(-1)][:-len(CRLF_CHARS)] + line[1:]
        else:
...
def get_vcard_group(lines):
    ...
    if group_match is not None:
        ...
        for index in range(len(lines)):
            ...
            if not next_match:
                raise vcard_validators.VCardFormatError(MSG_MISSING_GROUP, {'File line': index + 1})
            if next_match.group(1) != group:
                raise vcard_validators.VCardFormatError(MSG_MISMATCH_GROUP + ': %s != %s' % (
                 next_match.group(1),
                 group), {'File line': index + 1})
...
def get_vcard_property_param_values(values_string):
    ...
    for value in values:
        if not re.match('^[' + SAFE_CHARS + ']+$|^"[' + QSAFE_CHARS + ']+"$', value):
            raise vcard_validators.VCardFormatError(MSG_INVALID_VALUE + ': %s' % value, {})
...
def get_vcard_property_param(param_string):
    ...
    try:
        ...
        raise vcard_validators.VCardFormatError(MSG_MISSING_PARAM_VALUE + ': %s' % str(error), {})

    values = get_vcard_property_param_values(values_string)
    if not re.match('^[' + ID_CHARS + ']+$', param_name):
        raise vcard_validators.VCardFormatError(MSG_INVALID_PARAM_NAME + ': %s' % param_name, {})
...
def get_vcard_property_subvalues(value_string):
    ...
    for subvalue in subvalues:
        if not re.match('^[' + VALUE_CHARS + ']*$', subvalue):
            raise vcard_validators.VCardFormatError(MSG_INVALID_SUBVALUE + ': %s' % subvalue, {})
...
def get_vcard_property(property_line):
    ...
    if len(property_parts) < 2:
        raise vcard_validators.VCardFormatError(MSG_MISSING_VALUE_STRING + ': %s' % property_line, {})
    ...
    if prop['name'].upper() not in ALL_PROPERTIES and not re.match('^X-[' + ID_CHARS + ']+$', prop['name'], re.IGNORECASE):
        raise vcard_validators.VCardFormatError(MSG_INVALID_PROPERTY_NAME + ': %s' % prop['name'], {})
    try:
    ...
    except vcard_validators.VCardFormatError, error:
...
def get_vcard_properties(lines):
    ...
    for mandatory_property in MANDATORY_PROPERTIES:
        if mandatory_property not in [ prop['name'].upper() for prop in properties ]:
            raise vcard_validators.VCardFormatError(MSG_MISSING_PROPERTY + ': %s' % mandatory_property, {'Property': mandatory_property})
...
class VCard:
    ...
    def __init__(self, text, filename=None):
        ...
        if text == '' or text is None:
            raise vcard_validators.VCardFormatError(MSG_EMPTY_VCARD, {'vCard line': 1, 'File line': 1})
        self.text = text
