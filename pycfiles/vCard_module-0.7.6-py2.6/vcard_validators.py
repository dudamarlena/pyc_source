...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/vcard/vcard_validators.py
# Compiled at: 2010-08-11 07:38:26
"""
...
import warnings, isodate
from vcard_defs import DQUOTE_CHAR, ID_CHARS, ESCAPED_CHARS, MSG_INVALID_DATE, MSG_INVALID_LANGUAGE_VALUE, MSG_INVALID_PARAM_NAME, MSG_INVALID_PARAM_VALUE, MSG_INVALID_SUBVALUE, MSG_INVALID_SUBVALUE_COUNT, MSG_INVALID_TEXT_VALUE, MSG_INVALID_TIME, MSG_INVALID_TIME_ZONE, MSG_INVALID_URI, MSG_INVALID_VALUE, MSG_INVALID_VALUE_COUNT, MSG_INVALID_X_NAME, MSG_MISMATCH_PARAM, MSG_MISSING_PARAM, MSG_NON_EMPTY_PARAM, QSAFE_CHARS, SAFE_CHARS, SP_CHAR, WARN_DEFAULT_TYPE_VALUE, WARN_INVALID_EMAIL_TYPE, WARN_MULTIPLE_NAMES

def _show_warning(message, category=UserWarning, filename='', lineno=-1, file=sys.stderr, line=None):
    ...
    def __str__(self):
        ...
        for key in keys:
            if key in self.context:
                message += '\n' + _stringify(key) + ': ' + _stringify(self.context.pop(key))
...
def validate_date(text):
    ...
    String: aaaa-bb-cc
    """
    if re.match('^\\d{4}-?\\d{2}-?\\d{2}$', text) is None:
...
def validate_time_zone(text):
    ...
    String: 01:1
    """
    if not re.match('^(Z|[+-]\\d{2}:?\\d{2})$', text):
...
def validate_time(text):
    ...
    String: Z+01
    """
    time_timezone = re.match('^(\\d{2}:?\\d{2}:?\\d{2}(?:,\\d+)?)(.*)$', text)
...
def validate_language_tag(text):
    ...
    """
    text = text.lower()
    if re.match('^([a-z]{1,8})(-[a-z]{1,8})*$', text) is None:
...
def validate_x_name(text):
    ...
    String: foo
    """
    if re.match('^X-[' + ID_CHARS + ']+$', text) is None:
...
def validate_text_value(text):
    ...
    String: ...
    """
    if re.match('^([' + SAFE_CHARS + ':' + DQUOTE_CHAR + ']|\\' + ESCAPED_CHARS + ')*$', text) is None:
...
def validate_quoted_string(text):
    ...
    String: "ÿÿ"
    """
    if re.match('^' + DQUOTE_CHAR + '[' + QSAFE_CHARS + ']' + DQUOTE_CHAR + '$', text) is None:
        raise VCardFormatError(MSG_INVALID_PARAM_VALUE, {'String': text})
    return
...
def validate_text_parameter(parameter):
    ...
    if param_name == 'VALUE':
        if param_values != set(['ptext']):
            raise VCardFormatError(MSG_INVALID_PARAM_VALUE + ': %s' % param_values, {})
        return
...
def validate_float(text):
    ...
    if re.match('^[+-]?\\d+(\\.\\d+)?$', text) is None:
        raise VCardFormatError(MSG_INVALID_SUBVALUE + ', expected float value: %s' % text, {})
    return
    ...
    try:
        if property_name in ('BEGIN', 'END'):
            if 'parameters' in prop:
                raise VCardFormatError(MSG_NON_EMPTY_PARAM + ': %s' % prop['parameters'], {})
            if len(prop['values']) != 1:
                raise VCardFormatError(MSG_INVALID_VALUE_COUNT + ': %d (expected 1)' % len(prop['values']), {})
            if len(prop['values'][0]) != 1:
                raise VCardFormatError(MSG_INVALID_SUBVALUE_COUNT + ': %d (expected 1)' % len(prop['values'][0]), {})
            if prop['values'][0][0].lower() != 'vcard':
                raise VCardFormatError(MSG_INVALID_VALUE + ': %s (expected "VCARD")' % prop['values'][0][0], {})
            ...
            for (param_name, param_values) in prop['parameters'].items():
                if param_name.upper() == 'VALUE':
                    ...
                    if 'CONTEXT' in prop['parameters']:
                        raise VCardFormatError(MSG_MISMATCH_PARAM + ': %s and %s' % ('VALUE',
                                                                                     'CONTEXT'), {})
                    ...
                    raise VCardFormatError(MSG_INVALID_PARAM_NAME + ': %s' % param_name, {})
        ...
        if property_name == 'FN':
            ...
            if len(prop['values']) != 5:
                raise VCardFormatError(MSG_INVALID_VALUE_COUNT + ': %d (expected 5)' % len(prop['values']), {})
            for names in prop['values']:
                warnings.showwarning = _show_warning
                for name in names:
                    validate_text_value(name)
                    if name.find(SP_CHAR) != -1 and ('').join([ ('').join(names) for names in prop['values']
                                                              ]) != name:
                        warnings.warn(WARN_MULTIPLE_NAMES + ': %s' % name.encode('utf-8'))
            ...
            for (param_name, param_values) in prop['parameters'].items():
                if param_name.upper() == 'ENCODING':
                    if param_values != set(['b']):
                        raise VCardFormatError(MSG_INVALID_PARAM_VALUE + ': %s' % param_values, {})
                    if 'VALUE' in prop['parameters']:
                        raise VCardFormatError(MSG_MISMATCH_PARAM + ': %s and %s' % ('ENCODING',
                                                                                     'VALUE'), {})
                elif param_name.upper() == 'TYPE' and 'ENCODING' not in prop['parameters']:
                    raise VCardFormatError(MSG_MISSING_PARAM + ': ENCODING', {})
            ...
            if len(prop['values']) != 7:
                raise VCardFormatError(MSG_INVALID_VALUE_COUNT + ': %d (expected 7)' % len(prop['values']), {})
            if 'parameters' in prop:
                for (param_name, param_values) in prop['parameters'].items():
                    if param_name.upper() == 'TYPE':
                        for param_subvalue in param_values:
                            if param_subvalue not in ('dom', 'intl', 'postal', 'parcel',
                                                      'home', 'work', 'pref'):
                                raise VCardFormatError(MSG_INVALID_PARAM_VALUE + ': %s' % param_subvalue, {})
                        ...
                        if param_values == set([
                         ...
                         'parcel',
                         'work']):
                            warnings.warn(WARN_DEFAULT_TYPE_VALUE + ': %s' % prop['values'])
                    else:
                        validate_text_parameter(parameter)
            ...
            if 'parameters' in prop:
                for (param_name, param_values) in prop['parameters'].items():
                    if param_name.upper() == 'TYPE':
                        for param_subvalue in param_values:
                            if param_subvalue.lower() not in ('home', 'msg', 'work',
                                                              'pref', 'voice', 'fax',
                                                              'cell', 'video', 'pager',
                                                              'bbs', 'modem', 'car',
                                                              'isdn', 'pcs'):
                        ...
                        if set([ value.lower() for value in param_values ]) == set(['voice']):
                            warnings.warn(WARN_DEFAULT_TYPE_VALUE + ': %s' % prop['values'])
                    else:
                        raise VCardFormatError(MSG_INVALID_PARAM_NAME + ': %s' % param_name, {})
            ...
            if 'parameters' in prop:
                for (param_name, param_values) in prop['parameters'].items():
                    if param_name.upper() == 'TYPE':
                        for param_subvalue in param_values:
                            if param_subvalue.lower() not in ('internet', 'x400', 'pref',
                                                              'dom', 'intl', 'postal',
                                                              'parcel', 'home', 'work'):
                                warnings.warn(WARN_INVALID_EMAIL_TYPE + ': %s' % param_subvalue)

                        if set([ value.lower() for value in param_values ]) == set(['internet']):
                            warnings.warn(WARN_DEFAULT_TYPE_VALUE + ': %s' % prop['values'])
                    else:
                        raise VCardFormatError(MSG_INVALID_PARAM_NAME + ': %s' % param_name, {})
        ...
        if property_name == 'GEO':
            ...
            if len(prop['values']) != 2:
                raise VCardFormatError(MSG_INVALID_VALUE_COUNT + ': %d (expected 2)' % len(prop['values']), {})
            ...
            if 'parameters' in prop:
                for (param_name, param_values) in prop['parameters'].items():
                    if param_name.upper() != 'VALUE':
                        raise VCardFormatError(MSG_INVALID_PARAM_NAME + ': %s' % param_values, {})
                    if param_values != set(['uri']):
                        raise VCardFormatError(MSG_INVALID_PARAM_VALUE + ': %s' % param_values, {})
