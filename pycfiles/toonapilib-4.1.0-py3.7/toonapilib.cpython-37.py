...
# [GCC 8.4.0]
# Embedded file name: /Users/ctyfoxylos/personal/python/toonapilib/toonapilib/toonapilib.py
# Compiled at: 2019-11-30 08:38:34
...
import coloredlogs, backoff
from cachetools import TTLCache, cached
from .configuration import STATES, STATE_CACHING_SECONDS, THERMOSTAT_STATE_CACHING_SECONDS, BURNER_STATES, PROGRAM_STATES
from .helpers import Agreement, Light, PowerUsage, SmartPlug, SmokeDetector, Solar, ThermostatInfo, ThermostatState, Usage, Data
from .toonapilibexceptions import InvalidAuthenticationToken, InvalidDisplayName, InvalidThermostatState, InvalidProgramState, IncompleteStatus, AgreementsRetrievalError
...
class Toon:
    __doc__ = 'Model of the toon smart meter from eneco.'

    def __init__(self, authentication_token, tenant_id='eneco', display_common_name=None):
    ...
    def _get_authenticated_session(self, token, display_common_name):
        ...
        if display_common_name:
            ...
            agreement = next((agreement for agreement in agreements if agreement.display_common_name.lower() == display_common_name.lower()), None)
            return agreement or InvalidDisplayName(display_common_name)
        else:
        ...
        session.headers.update({'X-Common-Name':agreement.display_common_name,  'X-Agreement-ID':agreement.id})
        self._api_url = '{}/toon/v3/{}'.format(self._base_url, agreement.id)
    ...
    def _get_agreements(self, session):
        ...
        try:
            ...
            agreements = [Agreement(agreement.get('agreementId'), agreement.get('agreementIdChecksum'), agreement.get('heatingType'), agreement.get('displayCommonName'), agreement.get('displayHardwareVersion'), agreement.get('displaySoftwareVersion'), agreement.get('isToonSolar'), agreement.get('isToonly')) for agreement in agreements_json]
        except AttributeError:
            if agreements_json.get('fault', {}).get('faultstring', {}) == 'Invalid Access Token':
    ...
    def status(self):
        ...
        if response.status_code == 202:
            self._logger.debug('Response accepted but no data yet, trying one more time...')
        response = self._session.get(url)
        try:
            data = response.json()
        except ValueError:
    ...
    def thermostat_states(self):
        ...
        return [ThermostatState(STATES[state.get('id')], state.get('id'), state.get('tempValue'), state.get('dhw')) for state in states]
    ...
    def _get_endpoint_data(self, endpoint, params=None):
        url = '{base}{endpoint}'.format(base=(self._api_url), endpoint=endpoint)
    ...
    def smokedetectors(self):
        ...
        return [SmokeDetector(smokedetector.get('devUuid'), smokedetector.get('name'), smokedetector.get('lastConnectedChange'), smokedetector.get('connected'), smokedetector.get('batteryLevel'), smokedetector.get('type')) for smokedetector in self.status.get('smokeDetectors', {}).get('device', [])]
    ...
    def get_smokedetector_by_name(self, name):
        ...
        """
        return next((smokedetector for smokedetector in self.smokedetectors if smokedetector.name.lower() == name.lower()), None)

    @property
    def lights(self):
        """:return: A list of light objects."""
        return [Light(self, light.get('name')) for light in self.status.get('deviceStatusInfo', {}).get('device', []) if light.get('rgbColor')]
    ...
    def smartplugs(self):
        ...
        return [SmartPlug(self, plug.get('name')) for plug in self.status.get('deviceStatusInfo', {}).get('device', []) if plug.get('networkHealthState')]
    ...
    def get_smartplug_by_name(self, name):
        ...
        """
        return next((plug for plug in self.smartplugs if plug.name.lower() == name.lower()), None)
    ...
    def gas(self):
        ...
        return Usage(usage.get('avgDayValue'), usage.get('avgValue'), usage.get('dayCost'), usage.get('dayUsage'), usage.get('isSmart'), usage.get('meterReading'), usage.get('value'))

    @property
    def power(self):
        """:return: A power object modeled as a named tuple."""
        power = self._get_status_value('powerUsage')
        return PowerUsage(power.get('avgDayValue'), power.get('avgValue'), power.get('dayCost'), power.get('dayUsage'), power.get('isSmart'), power.get('meterReading'), power.get('value'), power.get('meterReadingLow'), power.get('dayLowUsage'))

    @property
    def solar(self):
        """:return: A solar object modeled as a named tuple."""
        power = self._get_status_value('powerUsage')
        return Solar(power.get('maxSolar'), power.get('valueProduced'), power.get('valueSolar'), power.get('avgProduValue'), power.get('meterReadingLowProdu'), power.get('meterReadingProdu'), power.get('dayCostProduced'))
    ...
    def get_thermostat_state_by_name(self, name):
        ...
        return next((state for state in self.thermostat_states if state.name.lower() == name.lower()), None)
    ...
    def get_thermostat_state_by_id(self, id_):
        ...
        :return: The thermostat state object
        """
        return next((state for state in self.thermostat_states if state.id == id_), None)

    @property
    def burner_on(self):
        """Boolean value of the state of the burner."""
        if int(self.thermostat_info.burner_info):
            return True
        return False
    ...
    def _validate_thermostat_state_name(name):
        if name.lower() not in [value.lower() for value in STATES.values() if value.lower() != 'unknown']:
    ...
    def thermostat_state(self):
        ...
        if not state:
            self._logger.debug('Manually set temperature, no Thermostat State chosen!')
        return state
    ...
    def thermostat_state(self, name):
        ...
        """
        self._validate_thermostat_state_name(name)
        id_ = next((id_ for id_, state in STATES.items() if state.lower() == name.lower()), None)
    ...
    def program_state(self, name):
        ...
        """
        id_ = next((id_ for id_, state in PROGRAM_STATES.items() if state.lower() == name.lower()), None)
        if id_ is None:
