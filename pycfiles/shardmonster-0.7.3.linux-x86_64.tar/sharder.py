...
# [GCC 8.4.0]
# Embedded file name: /home/vagrant/envs/conversocial/lib/python2.7/site-packages/shardmonster/sharder.py
# Compiled at: 2016-11-29 07:48:54
"""
...
"""
import pymongo, sys, threading, time
from datetime import datetime
from shardmonster import api, metadata
from shardmonster.connection import close_thread_connections, get_connection, parse_location
...
def _do_copy(collection_name, shard_key, manager):
    ...
    shard_metadata, = shards_coll.find({'realm': realm['name'], 'shard_key': shard_key})
    ...
    current_collection = _get_collection_from_location_string(current_location, collection_name)
    new_collection = _get_collection_from_location_string(new_location, collection_name)
...
def _get_metadata_for_shard(realm_name, shard_key):
    shards_coll = api._get_shards_coll()
    shard_metadata, = shards_coll.find({'realm': realm_name, 'shard_key': shard_key})
...
def _sync_from_oplog(collection_name, shard_key, oplog_pos):
    ...
    cursor = repl_coll.find({'ts': {'$gte': oplog_pos}}, cursor_type=pymongo.CursorType.TAILABLE, oplog_replay=True)
    ...
    for r in cursor:
        ...
        if r['op'] == 'u':
            ...
            new_collection.update({'_id': oid}, r['o'], w=1)
        elif r['op'] == 'i':
            try:
...
def _delete_source_data(collection_name, shard_key, manager):
    ...
    current_location = shard_metadata['location']
    current_collection = _get_collection_from_location_string(current_location, collection_name)
    cursor = current_collection.find({shard_field: shard_key}, {'_id': 1}, no_cursor_timeout=True)
    try:
        for doc in cursor:
...
class ShardMovementThread(threading.Thread):
    ...
    def __init__(self, collection_name, shard_key, new_location, manager):
    ...
    def run(self):
        try:
            ...
            api.start_migration(self.collection_name, self.shard_key, self.new_location)
            ...
            api.set_shard_to_migration_status(self.collection_name, self.shard_key, metadata.ShardStatus.MIGRATING_SYNC)
            oplog_pos = _sync_from_oplog(self.collection_name, self.shard_key, oplog_pos)
            while time.time() < start_sync_time + api.get_caching_duration():
                time.sleep(0.05)
                oplog_pos = _sync_from_oplog(self.collection_name, self.shard_key, oplog_pos)

            api.set_shard_to_migration_status(self.collection_name, self.shard_key, metadata.ShardStatus.POST_MIGRATION_PAUSED_AT_DESTINATION)
            time.sleep(0.1)
            _sync_from_oplog(self.collection_name, self.shard_key, oplog_pos)
            self.manager.set_phase('delete')
            api.set_shard_to_migration_status(self.collection_name, self.shard_key, metadata.ShardStatus.POST_MIGRATION_DELETE)
            _delete_source_data(self.collection_name, self.shard_key, self.manager)
            api.set_shard_at_rest(self.collection_name, self.shard_key, self.new_location, force=True)
...
class ShardMovementManager(object):
    ...
    def __init__(self, collection_name, shard_key, new_location, delete_throttle=None, insert_throttle=None):
        self.collection_name = collection_name
        self.shard_key = shard_key
        self.new_location = new_location
    ...
    def start_migration(self):
        self._migration_thread = ShardMovementThread(self.collection_name, self.shard_key, self.new_location, manager=self)
        self._migration_thread.start()

    def is_finished(self):
        if self._migration_thread.exception:
            raise Exception('Migration failed %s' % (self._migration_thread.exception,))
...
def _begin_migration(collection_name, shard_key, new_location, delete_throttle=None, insert_throttle=None):
    if metadata.are_migrations_happening():
        raise Exception('Cannot start migration when another migration is in progress')
    manager = ShardMovementManager(collection_name, shard_key, new_location, delete_throttle=delete_throttle, insert_throttle=insert_throttle)
    manager.start_migration()
    return manager


def do_migration(collection_name, shard_key, new_location, delete_throttle=None, insert_throttle=None):
    ...
    """
    manager = _begin_migration(collection_name, shard_key, new_location, delete_throttle=delete_throttle, insert_throttle=insert_throttle)
    return manager