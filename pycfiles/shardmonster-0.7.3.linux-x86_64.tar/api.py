...
# [GCC 8.4.0]
# Embedded file name: /home/vagrant/envs/conversocial/lib/python2.7/site-packages/shardmonster/api.py
# Compiled at: 2016-08-05 04:37:45
from shardmonster.connection import add_cluster, connect_to_controller, _get_cluster_coll, get_cluster_uri, parse_location
from shardmonster.metadata import _get_location_for_shard, _get_realm_coll, _get_realm_by_name, _get_realm_for_collection, _get_shards_coll, ShardStatus, activate_caching, get_caching_duration, realm_changed
...
def create_realm(realm, shard_field, collection_name):
    _get_realm_coll().insert({'name': realm, 
...
def ensure_realm_exists(name, shard_field, collection_name):
    ...
    if cursor.count():
        existing = cursor[0]
        if existing['shard_field'] != shard_field or existing['collection'] != collection_name:
    ...
    if cursor.count():
        existing = cursor[0]
        if existing['shard_field'] != shard_field or existing['name'] != name:
            raise Exception('Realm for collection %s already exists' % collection_name)
        else:
            return
...
def set_shard_at_rest(realm, shard_key, location, force=False):
    ...
    if shards_coll.find(query).count() and not force:
        raise Exception('Shard with key %s has already been placed. Use force=true if you really want to do this' % shard_key)
    shards_coll.update(query, {'$set': {'location': location, 
                'status': ShardStatus.AT_REST}, 
       '$unset': {'new_location': 1}}, upsert=True)
    realm_changed(realm)
...
def set_shard_to_migration_status(realm, shard_key, status):
    ...
    """
    shards_coll = _get_shards_coll()
    shards_coll.update({'realm': realm, 'shard_key': shard_key}, {'$set': {'status': status}})
...
def start_migration(realm_name, shard_key, new_location):
    ...
    shards_coll.update({'realm': realm_name, 'shard_key': shard_key}, {'$set': {'status': ShardStatus.MIGRATING_COPY, 
...
class ShardAwareCollectionProxy(object):
    ...
    def find(self, *args, **kwargs):
        return operations.multishard_find(self.collection_name, with_options=self._with_options, *args, **kwargs)

    def find_one(self, *args, **kwargs):
        return operations.multishard_find_one(self.collection_name, with_options=self._with_options, *args, **kwargs)

    def update(self, *args, **kwargs):
        return operations.multishard_update(self.collection_name, with_options=self._with_options, *args, **kwargs)

    def insert(self, *args, **kwargs):
        return operations.multishard_insert(self.collection_name, with_options=self._with_options, *args, **kwargs)

    def remove(self, *args, **kwargs):
        return operations.multishard_remove(self.collection_name, with_options=self._with_options, *args, **kwargs)

    def save(self, *args, **kwargs):
        return operations.multishard_save(self.collection_name, with_options=self._with_options, *args, **kwargs)
    ...
    def ensure_index(self, *args, **kwargs):
        return operations.multishard_ensure_index(self.collection_name, *args, **kwargs)

    def aggregate(self, *args, **kwargs):
        return operations.multishard_aggregate(self.collection_name, with_options=self._with_options, *args, **kwargs)

    def find_and_modify(self, *args, **kwargs):
        return operations.multishard_find_and_modify(self.collection_name, *args, **kwargs)
