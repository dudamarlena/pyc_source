...
# [GCC 8.4.0]
# Embedded file name: /home/vagrant/envs/conversocial/lib/python2.7/site-packages/shardmonster/tests/test_metadata.py
...
class TestShardMetadataStore(ShardingTestCase):
    ...
    def test_cache_single_shard(self, mock_query):
        expected_shard_metadata = {'status': metadata.ShardStatus.AT_REST, 
           'shard_key': 1}
    ...
    def test_cache_single_shard_in_flux(self, mock_query):
        expected_shard_metadata = {'status': metadata.ShardStatus.MIGRATING_SYNC, 
           'shard_key': 1}
        mock_query.return_value = [expected_shard_metadata.copy()]
        store = metadata.ShardMetadataStore({'realm': 'dummy-realm'})
        actual_shard_metadata = store.get_single_shard_metadata(1)
    ...
    def test_cache_all_shards(self, mock_query):
        expected_metadata_1 = {'status': metadata.ShardStatus.AT_REST, 
           'shard_key': 1}
        expected_metadata_2 = {'status': metadata.ShardStatus.AT_REST, 
           'shard_key': 2}
        mock_query.return_value = [
         expected_metadata_1.copy(), expected_metadata_2.copy()]
        store = metadata.ShardMetadataStore({'realm': 'dummy-realm'})
        actual_metadata = store.get_all_shard_metadata()
        self.assertEquals({1: expected_metadata_1, 2: expected_metadata_2}, actual_metadata)
        self.assertEquals(1, mock_query.call_count)
        actual_metadata = store.get_all_shard_metadata()
        self.assertEquals({1: expected_metadata_1, 2: expected_metadata_2}, actual_metadata)
        self.assertEquals(1, mock_query.call_count)
    ...
    def test_cache_all_shards_in_flux(self, mock_query):
        expected_metadata_1 = {'status': metadata.ShardStatus.MIGRATING_SYNC, 
           'shard_key': 1}
        expected_metadata_2 = {'status': metadata.ShardStatus.AT_REST, 
           'shard_key': 2}
        mock_query.return_value = [
...
class TestLocationMetadata(TestCase):
    ...
    def test_repr(self):
        ...
        self.assertEquals('LocationMetadata(somewhere/banana, contains: [], excludes: [])', repr(meta))
        meta.contains += [1, 2, 3, 4, 5, 6]
        meta.excludes.append(9)
        self.assertEquals('LocationMetadata(somewhere/banana, contains: [1,2,3,4,5...], excludes: [9])', repr(meta))