...
# [GCC 8.4.0]
# Embedded file name: /home/vagrant/envs/conversocial/lib/python2.7/site-packages/shardmonster/metadata.py
# Compiled at: 2016-08-05 04:37:06
import time
from shardmonster.connection import _cluster_uri_cache, _get_cluster_coll, get_controlling_db
...
class ShardMetadataStore(object):
    ...
    def _cache_entry_is_valid(self, shard_key):
        now = time.time()
        return self._in_flux != shard_key and shard_key in self._cache and self._cache[shard_key][1] > now
    ...
    def get_all_shard_metadata(self):
        ...
        return {shard_key:metadata for shard_key, (metadata, _) in self._cache.iteritems()}
    ...
    def _refresh_single_shard_metadata(self, shard_key):
        ...
        if not shards:
            raise Exception('Shard key %s not placed for %s' % (
...
class LocationMetadata(object):
    ...
    def __init__(self, location):
        assert location.count('/') == 1, 'Location must be of the form cluster/db and not %s' % location
...
def _get_realm_for_collection(collection_name):
    ...
    if expiry <= now:
        ...
        try:
            ...
            raise Exception('Realm for collection %s does not exist' % collection_name)
...
def _get_realm_by_name(realm_name):
    ...
    try:
        ...
        raise Exception('Realm named %s does not exist' % realm_name)
