...
# [GCC 8.4.0]
# Embedded file name: /home/vagrant/envs/conversocial/lib/python2.7/site-packages/shardmonster/connection.py
# Compiled at: 2016-11-29 07:48:22
import logging, pymongo, threading, time
logger = logging.getLogger('shardmonster')
CLUSTER_CACHE_LENGTH = 600
_connection_cache = {}
_cluster_uri_cache = {}
...
def get_controlling_db():
    ...
    if not _controlling_db:
        raise Exception('Call connect_to_controller before attempting to get a connection')
...
def ensure_cluster_exists(name, uri):
    ...
    if not cursor.count():
        ...
        existing = cursor[0]
        if existing['uri'] != uri:
            logger.warn('Cluster in database does not match cluster being configured. This is normally OK if clusters are being moved about.')
