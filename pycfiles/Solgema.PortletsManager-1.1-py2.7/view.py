...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/Solgema/PortletsManager/portlets/view.py
# Compiled at: 2014-11-05 08:38:01
from zope.interface import implements
from zope.component import getUtility, getAdapters, getMultiAdapter
from Acquisition import aq_parent, aq_inner
from plone.portlets.interfaces import IPortletManager
from plone.portlets.interfaces import IPortletManagerRenderer
from plone.portlets.interfaces import IPortletAssignmentMapping, IPortletAssignmentSettings
...
class PortletManagerView(BrowserView):
    ...
    def spm_move_portlet_up(self):
        ...
        portlethash = self.request.get('viewname')
        info = unhashPortletInfo(portlethash)
        assignments = assignment_mapping_from_key(self.context, info['manager'], info['category'], info['key'])
    ...
    def spm_stop_portlet(self):
        ...
        assignments = assignment_mapping_from_key(self.context, info['manager'], info['category'], info['key'])
        IPortletPermissionChecker(assignments.__of__(aq_inner(self.context)))()
        assigned = IPortletAssignmentSettings(assignments[info['name']])
    ...
    def spm_allow_portlet(self):
        ...
        li = []
        assigned = IPortletAssignmentSettings(assignments[info['name']])
        if hasattr(assigned, 'stopUrls') and len(getattr(assigned, 'stopUrls', [])) > 0:
            urls = assigned.stopUrls
            for url in urls:
