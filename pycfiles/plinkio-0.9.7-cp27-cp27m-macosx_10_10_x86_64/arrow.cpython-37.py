...
from .gui import *
default_arrow_params = dict(abs_gap_size=9.0, rel_gap_size=0.2,
  no_arrow_size=0,
...
class Arrow:
    ...
    def find_segments(self, crossings, include_overcrossings=False):
        ...
        for c in crossings:
            ...
            if not c.over == self or include_overcrossings or params['include_overcrossings']:
                t = self ^ c.under
                if t:
    ...
    def draw(self, crossings=[], recurse=True, skip_frozen=True):
        ...
        for x0, y0, x1, y1 in segments[:-1]:
            self.lines.append(self.canvas.create_line(x0,
              y0, x1, y1, width=thickness,
              fill=color,
              tags='transformable'))
        ...
        for c in crossings:
            if self == c.under and c.is_virtual:
                self.dots.append(self.canvas.create_oval((c.x - 5),
                  (c.y - 5), (c.x + 5), (c.y + 5), fill='black',
                  outline='black',
