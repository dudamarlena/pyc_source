...
# Embedded file name: /build/plink/build/lib/plink/smooth.py
...
class SmoothArc:
    ...
    def __init__(self, canvas, vertices, color='black', tension1=1.0, tension2=1.0):
        ...
        self.canvas_items = []
        self.spline_knots = K = [
         V[0]] + [0.5 * (V[k] + V[(k + 1)]) for k in range(1, len(V) - 2)] + [
         V[(-1)]]
        self.tangents = [
         V[1] - K[0]] + [V[(k + 1)] - K[k] for k in range(1, len(V) - 2)] + [
         V[(-1)] - V[(-2)]]
    ...
    def tk_draw(self, thickness=4):
        XY = self.bezier()
        self.tk_clear()
        self.canvas_items.append((self.canvas.create_line)(*XY, smooth='raw', width=thickness, fill=self.color, capstyle=Tk_.ROUND, 
    ...
    def tikz_draw(self, file, transform):
        ...
        for i in range(3, len(points) - 3, 3):
            file.write(self.color, '\n          ' + '%s .. controls %s and %s .. ' % tuple(points[i:i + 3]))
    ...
    def _build_curves(self):
        ...
        for polyline, color in self.polylines:
            ...
            for arc in polyline:
                polygon += arc[1:-1]
                if arc[0] == arc[(-1)]:
                    A = SmoothLoop((self.canvas), arc, color, tension1=(self.tension1),
                      tension2=(self.tension2))
                    curves.append(A)
                else:
                    A = SmoothArc((self.canvas), arc, color, tension1=(self.tension1),
                      tension2=(self.tension2))
                    curves.append(A)
...
def save_as_eps(canvas, file_name, colormode='color', width=312.0):
    ...
    canvas.postscript(file=file_name, x=ulx, y=uly, width=(lrx - ulx), height=(lry - uly), colormode=colormode,
      pagewidth=width)
