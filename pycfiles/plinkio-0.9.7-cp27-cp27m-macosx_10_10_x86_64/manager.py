...
# Embedded file name: /build/plink/build/lib/plink/manager.py
...
DT_alphabet = b'_abcdefghijklmnopqrstuvwxyzZYXWVUTSRQPONMLKJIHGFEDCBA'
...
class LinkManager:
    ...
    def _from_string(self, contents):
        lines = [ line for line in contents.split(b'\n') if len(line) > 0 ]
        num_lines = len(lines)
        first_line = lines.pop(0)
        has_virtual_crossings = first_line.startswith(b'% Virtual Link Projection')
        if not (first_line.startswith(b'% Link Projection') or first_line.startswith(b'% Virtual Link Projection')):
            tkMessageBox.showwarning(b'Bad file', b'This is not a SnapPea link projection file')
        else:
            try:
                ...
                tkMessageBox.showwarning(b'Bad file', b'Failed while parsing line %d' % (num_lines - len(lines)))
    ...
    def update_crosspoints(self):
        ...
        self.CrossPoints = [ Vertex(c.x, c.y, self.canvas, style=b'hidden') for c in self.Crossings ]
        return
    ...
    def arrow_components(self, include_isolated_vertices=False, distinguish_closed=False):
        ...
        if include_isolated_vertices:
            for vertex in [ v for v in self.Vertices if v.is_isolated() ]:
                nonclosed.append([
                 Arrow(vertex, vertex, self.canvas, color=vertex.color)])

        def oldest_vertex(component):

            def oldest(arrow):
                return min([ self.Vertices.index(v) for v in [
                 arrow.start, arrow.end] if v
                           ])
    ...
    def crossing_components(self):
        ...
        for vertex in self.Vertices:
            if vertex.is_endpoint():
                raise ValueError(b'All components must be closed.')
    ...
    def SnapPea_KLPProjection(self):
        ...
        for component in components:
            ...
            for n in range(N):
                ...
                this.crossing.KLP[b'sign'] = sign = this.crossing.sign()
                if this.strand == b'X':
                    this.crossing.KLP[b'Xbackward_neighbor'] = id(previous)
                    this.crossing.KLP[b'Xbackward_strand'] = previous.strand
                    this.crossing.KLP[b'Xforward_neighbor'] = id(next)
                    this.crossing.KLP[b'Xforward_strand'] = next.strand
                    this.crossing.KLP[b'Xcomponent'] = this_component
                else:
                    this.crossing.KLP[b'Ybackward_neighbor'] = id(previous)
                    this.crossing.KLP[b'Ybackward_strand'] = previous.strand
                    this.crossing.KLP[b'Yforward_neighbor'] = id(next)
                    this.crossing.KLP[b'Yforward_strand'] = next.strand
                    this.crossing.KLP[b'Ycomponent'] = this_component
        ...
        return (
         num_crossings, num_free_loops, num_components, KLP_crossings)
    ...
    def PD_code(self):
        ...
        ecrossings = [ ec for component in components for ec in component
                     ]
    ...
    def DT_code(self, alpha=False, signed=True, return_sizes=False):
        ...
        if not alpha:
            ...
            prefix_ints = [
             len(self.Crossings), len(sorted_components)]
            prefix_ints += DT_chunks
            if prefix_ints[0] > 26:
                tkMessageBox.showwarning(b'Error', b'Alphabetical DT codes require fewer than 26 crossings.')
                return
            alphacode = (b'').join(tuple([ DT_alphabet[(n >> 1)] for n in even_codes ]))
            prefix = (b'').join(tuple([ DT_alphabet[n] for n in prefix_ints ]))
            if signed:
                alphacode += b'.' + (b'').join([ str(f) for f in flips ])
            result = [
             prefix + alphacode]
        if return_sizes:
        ...
        for component in components:
            ...
            for ec in component:
                ...
                if crossing.comp1 == crossing.comp2 == component:
                    if ec.crossing.sign() == b'RH':
                        m += 1
                    elif ec.crossing.sign() == b'LH':
                        m -= 1
    ...
    def DT_normal(self):
        ...
        code = self.DT_code()
        if code:
            self.write_text((b'DT: %s,  %s' % code).replace(b', ', b','))
    ...
    def Gauss_info(self):
        ...
        if code:
            self.write_text((b'Gauss: %s' % code).replace(b', ', b','))
    ...
    def PD_info(self):
        ...
        code = self.PD_code()
        if code:
            self.write_text((b'PD: %s' % code).replace(b', ', b','))
    ...
    def BB_info(self):
        ...
        if framing:
            self.write_text((b'BB framing:  %s' % framing).replace(b', ', b','))
    ...
    def SnapPea_projection_file(self):
        ...
        result = b''
        result += b'% Virtual Link Projection\n' if has_virtual_crossings else b'% Link Projection\n'
        ...
        for crossing in self.Crossings:
            ...
            is_virtual = b'v' if crossing.is_virtual else b'r'
            result += b'%4s %4.1d %4.1d\n' % (is_virtual, under, over) if has_virtual_crossings else b'%4.1d %4.1d\n' % (under, over)
    ...
    def twister_surface_file(self):
            ...
            for arrow in component:
                ...
                for index, virtual_crossing in enumerate(virtual_crossings):
                    ...
                    this_arrows_crossings.append((
                     arrow ^ other_arrow, index, b'+' if sign else b'-'))
        ...
        curves = list(ascii_lowercase) + [ b'%s%d' % (letter, index) for index in range((len(closed_components) + len(nonclosed_components)) // 26) for letter in ascii_lowercase
                                         ]
        i = 0
        for component in closed_components:
            result += b'annulus,%s,%s,%s#\n' % (
             curves[i], curves[i].swapcase(), (b',').join(component_sequence(component)))
            i += 1

        for component in nonclosed_components:
            result += b'rectangle,%s,%s,%s#\n' % (
             curves[i], curves[i].swapcase(), (b',').join(component_sequence(component)))
            i += 1

        return result

    def save_as_tikz(self, file_name, colormode=b'color', width=282.0):
        ...
        for polyline in polylines:
            for line in polyline[0]:
                points = [ b'(%.2f, %.2f)' % tikz.transform(xy) for xy in line ]
                tikz.write(polyline[1], b'    \\draw ' + (b' -- ').join(points) + b';\n')
    ...
    def unpickle(self, vertices, arrows, crossings, hot=None):
        ...
        for under, over, is_virtual, label in crossings:
            U, O, V, L = (
             self.Arrows[int(under)], self.Arrows[int(over)], bool(is_virtual), str(label))
