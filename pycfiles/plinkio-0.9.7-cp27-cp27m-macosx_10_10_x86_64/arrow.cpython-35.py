...
# Embedded file name: /build/plink/build/lib/plink/arrow.py
# Compiled at: 2019-07-15 20:56:41
# Size of source mod 2**32: 8430 bytes
__doc__ = '\nThis module exports the class Arrow which represents a (directed)\nline segment in a PL link diagram.\n'
from math import sqrt
from .gui import *
default_arrow_params = dict(abs_gap_size=9.0, rel_gap_size=0.2, no_arrow_size=0, double_gap_at_ends=False, include_overcrossings=False)

class Arrow:
    """Arrow"""
    epsilon = 8

    def __init__(self, start, end, canvas=None, style='normal', color='black', other_params=None):
    ...
    def find_segments(self, crossings, include_overcrossings=False):
        ...
        for i in range(len(cross_params) - 1):
            ...
            if params['double_gap_at_ends'] and (i == 0 or i == len(cross_params) - 2):
                rel_gap = 2 * rel_gap
    ...
    def draw(self, crossings=[], recurse=True, skip_frozen=True):
        ...
        for x0, y0, x1, y1 in segments[:-1]:
            self.lines.append(self.canvas.create_line(x0, y0, x1, y1, width=thickness, fill=color, tags='transformable'))
        ...
        self.lines.append(self.canvas.create_line(x0, y0, x1, y1, arrow=arrow, width=thickness, fill=color, tags='transformable'))
        if recurse:
        ...
        for c in crossings:
            if self == c.under and c.is_virtual:
                self.dots.append(self.canvas.create_oval(c.x - 5, c.y - 5, c.x + 5, c.y + 5, fill='black', outline='black', tags=('dot',
                                                                                                                                  'transformable')))
