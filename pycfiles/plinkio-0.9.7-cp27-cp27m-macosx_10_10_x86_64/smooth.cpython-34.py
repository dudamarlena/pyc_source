...
class SmoothArc:
    ...
    def tk_draw(self, thickness=4):
        XY = self.bezier()
        self.tk_clear()
        self.canvas_items.append(self.canvas.create_line(*XY, smooth='raw', width=thickness, fill=self.color, capstyle=Tk_.ROUND, splinesteps=100, tags=('smooth',
                                                                                                                                                         'transformable')))

    def pyx_draw(self, canvas, transform):
        XY = [transform(xy) for xy in self.bezier()]
        arc_parts = [
         pyx.path.moveto(*XY[0])]
...
def save_as_eps(canvas, file_name, colormode='color', width=312.0):
    ...
    canvas.postscript(file=file_name, x=ulx, y=uly, width=lrx - ulx, height=lry - uly, colormode=colormode, pagewidth=width)
