...
class PLinkBase(LinkViewer):
    ...
    def __init__(self, root=None, manifold=None, file_name=None, title='', show_crossing_labels=False):
        ...
        self.infotext = ttk.Entry(self.infoframe, state='readonly', font='Helvetica 16', textvariable=self.infotext_contents)
    ...
    def show_labels(self):
        ...
        for crossing in self.Crossings:
            ...
            self.labels.append(self.canvas.create_text((
             crossing.x - 1, crossing.y - yshift), anchor=Tk_.E, text=str(crossing.label)))
    ...
    def show_DT(self):
        ...
        for crossing in self.Crossings:
            ...
            self.DTlabels.append(self.canvas.create_text((
             crossing.x - 10, crossing.y - yshift), anchor=Tk_.E, text=str(crossing.hit1)))
            self.DTlabels.append(self.canvas.create_text((
             crossing.x + 10, crossing.y - yshift), anchor=Tk_.W, text=str(crossing.hit2) + flip))
    ...
    def save(self):
        savefile = asksaveasfile(parent=self.window, mode='w', title='Save As Snappea Projection File', defaultextension='.lnk', filetypes=[
...
class LinkEditor(PLinkBase):
    ...
    def crossed_arrows(self, arrow, ignore_list=[]):
        ...
        for n, diagram_arrow in enumerate(self.Arrows):
            if not arrow == diagram_arrow:
                if diagram_arrow in ignore_list:
                    continue
