...
# Embedded file name: /build/plink/build/lib/plink/canvasvg.py
# Compiled at: 2019-07-15 20:56:41
# Size of source mod 2**32: 20652 bytes
from __future__ import division
__author__ = 'Wojciech Muła <wojciech_mula@poczta.onet.pl>'
__all__ = [
 'convert', 'SVGdocument', 'saveall', 'PYTHON', 'MODULE', 'NONE',
...
def configure(*flags):
    ...
    for flag in flags:
        if flag == SEGMENT_TO_LINE:
            ...
            if flag == SEGMENT_TO_PATH:
                segment = segment_to_path
            else:
                raise ValueError('Please use one of constants: SEGMENT_TO_LINE, SEGMENT_TO_PATH')
    ...
    for item in items:
        ...
        if itemtype not in supported_item_types:
            ...
            if 'current' in options['tags']:
                ...
                elif not options['state'] in ['normal', DISABLED, 'hidden']:
            ...
            if itemtype == 'line':
                ...
                if itemtype == 'arc' and options['style'] == ARC:
                    options['fill'] = ''
                    options['activefill'] = ''
            ...
            try:
                ...
                if width:
                    ...
                    if state == DISABLED and canvas.itemcget(item, 'disableddash'):
                        ...
                        if state == ACTIVE and canvas.itemcget(item, 'activedash'):
                ...
                if itemtype == 'text':
                    ...
                    element = setattribs(document.createElement('text'), x=x, y=y)
                    elements.append(element)
                    element.appendChild(document.createTextNode(tounicode(canvas.itemcget(item, 'text'))))
...
def SVGdocument():
    ...
    doctype = implementation.createDocumentType('svg', '-//W3C//DTD SVG 1.1//EN', 'http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd')
...
def polyline(document, coords):
    ...
    return setattribs(document.createElement('polyline'), points=' '.join(points))
...
def polygon(document, coords):
    ...
    return setattribs(document.createElement('polygon'), points=' '.join(points))
...
def rectangle(document, coords):
    ...
    return setattribs(element, x=coords[0], y=coords[1], width=coords[2] - coords[0], height=coords[3] - coords[1])
...
def oval(document, coords):
    ...
    x1, y1, x2, y2 = coords
    if x2 - x1 == y2 - y1:
        return setattribs(document.createElement('circle'), cx=(x1 + x2) / 2, cy=(y1 + y2) / 2, r=abs(x2 - x1) / 2)
    else:
        return setattribs(document.createElement('ellipse'), cx=(x1 + x2) / 2, cy=(y1 + y2) / 2, rx=abs(x2 - x1) / 2, ry=abs(y2 - y1) / 2)
    return element
