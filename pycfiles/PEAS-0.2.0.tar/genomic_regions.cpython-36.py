...
# [GCC 8.4.0]
# Embedded file name: /gpfs/data01/glasslab/home/dskola/workspace/PEAS/peas/genomic_regions.py
# Compiled at: 2018-10-19 18:37:41
# Size of source mod 2**32: 14575 bytes
import numpy, pandas
...
def load_and_parse_peak_file(peak_fname, feature_columns):
    ...
    feature_columns = numpy.array(feature_columns) - 1
...
def normalize_features(features, rip_norm=constants.DEFAULT_RIP_NORM, znorm=constants.DEFAULT_ZNORM, log_transform=constants.DEFAULT_LOG_TRANSFORM, pseudocount=constants.DEFAULT_PSEUDOCOUNT):
    """
...
def find_genomic_region_crds_vector(peak_filename, peak_file_format, feature_columns, output_filename='', rip_norm=constants.DEFAULT_RIP_NORM, znorm=constants.DEFAULT_ZNORM, log_transform=constants.DEFAULT_LOG_TRANSFORM, pseudocount=constants.DEFAULT_PSEUDOCOUNT, tail=constants.DEFAULT_TAIL, min_score=constants.DEFAULT_MIN_SCORE, pvalue=constants.DEFAULT_PVALUE_THRESHOLD, fdr=constants.DEFAULT_FDR_THRESHOLD, min_size=constants.DEFAULT_MIN_SIZE, max_size=constants.DEFAULT_MAX_SIZE, alpha=constants.DEFAULT_ALPHA, bins=constants.DEFAULT_BINS):
    ...
    if not min_size >= 2:
        ...
        assert 0 < pvalue <= 1
        assert 0 <= min_score
    features = normalize_features(features=features, rip_norm=rip_norm, znorm=znorm, log_transform=log_transform,
    ...
    elif not len(feature_columns) == 2:
        raise AssertionError
    ...
    for chrom, chrom_annotations in annotations.groupby(annotations.columns[0]):
        ...
        if len(chrom_vector) > min_size + 1:
            ...
            this_chrom_peas = interface.find_peas_vector(input_vector=chrom_vector, min_score=min_score, max_pval=pvalue,
              min_size=min_size,
              max_size=max_size,
...
def find_genomic_region_crds_matrix(peak_filename, peak_file_format, feature_columns, output_filename='', rip_norm=constants.DEFAULT_RIP_NORM, znorm=constants.DEFAULT_ZNORM, log_transform=constants.DEFAULT_LOG_TRANSFORM, pseudocount=constants.DEFAULT_PSEUDOCOUNT, tail=constants.DEFAULT_TAIL, min_score=constants.DEFAULT_MIN_SCORE, pvalue=constants.DEFAULT_PVALUE_THRESHOLD, fdr=constants.DEFAULT_FDR_THRESHOLD, min_size=constants.DEFAULT_MIN_SIZE, max_size=constants.DEFAULT_MAX_SIZE, alpha=constants.DEFAULT_ALPHA, start_diagonal=constants.DEFAULT_START_DIAGONAL, parameter_smoothing_window_size=constants.DEFAULT_PARAMETER_SMOOTHING_WINDOW_SIZE, null_distribution_type=constants.DEFAULT_NULL_DISTRIBUTION, random_seed=None):
    if peak_file_format == 'bed':
        annotations, features = load_and_parse_bed_file(bed_fname=peak_filename, feature_columns=feature_columns)
    else:
        annotations, features = load_and_parse_peak_file(peak_fname=peak_filename, feature_columns=feature_columns)
    ...
    for chrom, chrom_annotations in annotations.groupby(annotations.columns[0]):
        if chrom_annotations.shape[0] > min_size + 1:
            ...
            this_chrom_peas = interface.find_peas_matrix(input_matrix=chrom_corrs, min_score=min_score, max_pval=pvalue,
              min_size=min_size,
              max_size=max_size,
              maximization_target=(constants.DEFAULT_MAXIMIZATION_TARGET),
              tail=tail,
...
def generate_bed_df(regions, annotations):
    ...
    for start, end, size, score, pval in regions:
        ...
        assert annotations.iloc[(start, 0)] == annotations.iloc[(
         end, 0)], "First region peak chromosome {} doesn't match end peak chromosome {}".format(annotations.iloc[(start, 0)], annotations.iloc[(end, 0)])
        assert overall_start < overall_end, 'Invalid region start and end positions: {}, {}'.format(overall_start, overall_end)
    ...
    if region_list:
        ...
        region_df.columns = ('chrom', 'chromStart', 'chromEnd', 'name', 'score', 'strand',
                             'pval')
        region_df.index = region_df['name']
    else:
        region_df = pandas.DataFrame(columns=('chrom', 'chromStart', 'chromEnd', 'name',
                                              'score', 'strand', 'pval'))
    return region_df
...
def write_ucsc_bed_file(filename, bed_df, track_name, description=''):
    ...
    """
    ucsc_formatted_bed_df = bed_df.loc[:,
