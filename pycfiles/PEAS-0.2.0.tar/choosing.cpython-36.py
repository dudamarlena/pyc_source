...
# [GCC 8.4.0]
# Embedded file name: /gpfs/data01/glasslab/home/dskola/workspace/PEAS/peas/choosing.py
...
def stitch_regions(region_coords, region_scores, region_pvals):
    ...
    for region_idx in range(len(region_coords) - 1):
        join = region_coords[(region_idx + 1)][0] - region_coords[region_idx][1] == 1 and region_coords[region_idx][3] * region_coords[(region_idx + 1)][3] > 0
        if join:
            if not in_region:
