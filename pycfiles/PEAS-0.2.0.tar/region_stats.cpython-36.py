...
# [GCC 8.4.0]
# Embedded file name: /gpfs/data01/glasslab/home/dskola/workspace/PEAS/peas/region_stats.py
# Compiled at: 2018-10-20 21:01:24
# Size of source mod 2**32: 12207 bytes
...
def generate_permuted_matrix_scores(matrix, num_shuffles, min_region_size=2, max_region_size=0, start_diagonal=1, matrix_score_func=scoring.compute_mean_table_2d, random_seed=None):
    """
    ...
    integer_seed = int(hash(tuple(numpy.random.get_state()[1])) % 4294967294)
    log_print('setting random seed to {}'.format(integer_seed), 3)
    n = matrix.shape[0]
    if max_region_size == 0:
        max_region_size = n
    sampled_scores = {region_size:numpy.empty((n - (region_size - 1)) * num_shuffles) for region_size in range(min_region_size, max_region_size + 1)}
    ...
    for shuffle_idx in range(num_shuffles):
        ...
        if elapsed_seconds > MIN_REPORTING_TIME:
            ...
            last_time = cur_time
        if constants.USE_C:
            scores = scoring_funcs_cython.compute_mean_table_2d_shuffled(data_matrix=matrix, start_diagonal=start_diagonal,
        ...
        for region_size in range(min_region_size, max_region_size + 1):
            ...
            sampled_scores[region_size][shuffle_idx * len(diag_sample):(shuffle_idx + 1) * len(diag_sample)] = diag_sample

    return sampled_scores


def fit_distributions(sampled_scores, support_ranges, matrix_size, start_diagonal, distribution_class=constants.NULL_DISTRIBUTIONS_BY_NAME[constants.DEFAULT_NULL_DISTRIBUTION], parameter_smoothing_method=constants.DEFAULT_PARAMETER_SMOOTHING_METHOD, parameter_smoothing_window_size=constants.DEFAULT_PARAMETER_SMOOTHING_WINDOW_SIZE):
    """
    Given a matrix of values, returns a dictionary, keyed by region size, of
    ...
    fit_params = fit_distros(sampled_scores, support_ranges=support_ranges,
...
def compute_pscores(region_scores, null_distributions, tail):
    ...
    pval_scores = compute_pscores_matrix(data_matrix=region_scores, distro_dict=null_distributions,
...
def compute_pvalues_matrix(data_matrix, distro_dict, tail='right', diagonal_start=1, diagonal_end=0, fill_value=1.0):
    ...
    for diagonal in range(diagonal_start, diagonal_end):
        diag_indices = my_diag_indices(n, diagonal)
        p_vals[diag_indices] = compute_pvalues_tailed(frozen_distribution=(distro_dict[(diagonal + 1)]), x=(data_matrix[diag_indices]),
          tail=tail)
...
def compute_pscores_matrix(data_matrix, distro_dict, tail='right', diagonal_start=1, diagonal_end=0, fill_value=0.0):
    ...
    for diagonal in range(diagonal_start, diagonal_end):
        diag_indices = my_diag_indices(n, diagonal)
        p_scores[diag_indices] = -compute_log_pvals_tailed(frozen_distribution=(distro_dict[(diagonal + 1)]), x=(data_matrix[diag_indices]),
          tail=tail)
...
def smooth_parameters(param_dict, parameter_smoothing_method=constants.DEFAULT_PARAMETER_SMOOTHING_METHOD, parameter_smoothing_window_size=constants.DEFAULT_PARAMETER_SMOOTHING_WINDOW_SIZE):
    if parameter_smoothing_method == 'savgol':
        if len(param_dict) >= 3:
            if parameter_smoothing_window_size > 0:
                ...
                log_print('smoothing parameters with Savitsky-Golay filter of size {}'.format(parameter_smoothing_window_size), 3)
                param_df = pandas.DataFrame(param_dict).T
                param_array = savgol_filter(param_df, parameter_smoothing_window_size, 1, axis=0)
                param_dict = {region_size:params for region_size, params in zip(sorted(param_dict.keys()), param_array.tolist())}
        else:
            log_print('Too few region sizes to perform parameter smoothing (need at least 3)', 3)
    return param_dict


def fit_distros(shuffled_samples, distribution_class, support_ranges, matrix_size, start_diagonal=1, max_pvalue_cv=constants.DEFAULT_MAX_PVALUE_CV, parameter_smoothing_method=None, parameter_smoothing_window_size=constants.DEFAULT_PARAMETER_SMOOTHING_WINDOW_SIZE, fit_kwargs={}):
    """
    ...
    for region_size in region_sizes:
        ...
        num_unique_samples = compute_expected_unique_samples(total_items=universe_size, num_samples=(len(shuffled_samples[region_size])))
        this_fit_results = (distribution_class.informative_fit)(shuffled_samples[region_size], unique_samples=num_unique_samples, 
         support_range=support_ranges[region_size], 
         max_pvalue_cv=max_pvalue_cv, **fit_kwargs, **{'log_message_indentation': 4})
        fit_params[region_size] = this_fit_results['params']

    return smooth_parameters(fit_params, parameter_smoothing_method=parameter_smoothing_method, parameter_smoothing_window_size=parameter_smoothing_window_size)