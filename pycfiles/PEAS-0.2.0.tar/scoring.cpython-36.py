...
# [GCC 8.4.0]
# Embedded file name: /gpfs/data01/glasslab/home/dskola/workspace/PEAS/peas/scoring.py
...
def compute_sum_table_1d(vector, end_diagonal=0):
    ...
    if n > 1:
        ...
        sum_table[d1_idx] = sum_table[truncate_array_tuple(d0_idx, 1, 0)] + sum_table[truncate_array_tuple(d0_idx, 0, 1)]
        if n > 2:
            for k in range(2, end_diagonal):
                dk_idx = my_diag_indices(n, k)
                dk_prev = my_diag_indices(n, k - 1)
                dk_prevprev = my_diag_indices(n, k - 2)
                sum_table[dk_idx] = sum_table[truncate_array_tuple(dk_prev, 1, 0)] + sum_table[truncate_array_tuple(dk_prev, 0, 1)] - sum_table[truncate_array_tuple(dk_prevprev, 1, 1)]
...
def compute_min_table_1d(vector, end_diagonal=0):
    ...
    if n > 1:
        for k in range(1, end_diagonal):
            dk_idx = my_diag_indices(n, k)
            dk_prev = my_diag_indices(n, k - 1)
            min_table[dk_idx] = numpy.minimum(min_table[truncate_array_tuple(dk_prev, 1, 0)], min_table[truncate_array_tuple(dk_prev, 0, 1)])
...
def compute_max_table_1d(vector, end_diagonal=0):
    ...
    if n > 1:
        for k in range(1, end_diagonal):
            dk_idx = my_diag_indices(n, k)
            dk_prev = my_diag_indices(n, k - 1)
            max_table[dk_idx] = numpy.maximum(max_table[truncate_array_tuple(dk_prev, 1, 0)], max_table[truncate_array_tuple(dk_prev, 0, 1)])
...
def _compute_sum_table_2d_c(data, start_diagonal, end_diagonal):
    return scoring_funcs_cython.compute_sum_table_2d(data_matrix=data, start_diagonal=start_diagonal, end_diagonal=end_diagonal)
...
def _compute_sum_table_2d_py(data, start_diagonal, end_diagonal):
    ...
    if end_diagonal - start_diagonal >= 2:
        ...
        sum_table[d2_idx] = data[d2_idx] + sum_table[truncate_array_tuple(d1_idx, 1, 0)] + sum_table[truncate_array_tuple(d1_idx, 0, 1)]
        if end_diagonal - start_diagonal >= 3:
            for diagonal_idx in range(start_diagonal + 2, end_diagonal):
                ...
                sum_table[dk_idx] = data[dk_idx] + sum_table[truncate_array_tuple(dk_prev, 1, 0)] + sum_table[truncate_array_tuple(dk_prev, 0, 1)] - sum_table[truncate_array_tuple(dk_prevprev, 1, 1)]
    ...
    if not data.shape[0] == data.shape[1]:
        ...
        if end_diagonal - start_diagonal >= 2:
            for diagonal_idx in range(start_diagonal + 1, end_diagonal):
                dk_idx = my_diag_indices(n, diagonal_idx)
                dk_prev = my_diag_indices(n, diagonal_idx - 1)
                min_table[dk_idx] = numpy.minimum(data[dk_idx], numpy.minimum(min_table[truncate_array_tuple(dk_prev, 1, 0)], min_table[truncate_array_tuple(dk_prev, 0, 1)]))
    ...
    if not data.shape[0] == data.shape[1]:
        ...
        if end_diagonal - start_diagonal >= 2:
            for diagonal_idx in range(start_diagonal + 1, end_diagonal):
                dk_idx = my_diag_indices(n, diagonal_idx)
                dk_prev = my_diag_indices(n, diagonal_idx - 1)
                max_table[dk_idx] = numpy.maximum(data[dk_idx], numpy.maximum(max_table[truncate_array_tuple(dk_prev, 1, 0)], max_table[truncate_array_tuple(dk_prev, 0, 1)]))
