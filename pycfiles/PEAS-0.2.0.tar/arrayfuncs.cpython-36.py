...
# [GCC 8.4.0]
# Embedded file name: /gpfs/data01/glasslab/home/dskola/workspace/PEAS/peas/arrayfuncs.py
# Compiled at: 2018-10-19 18:37:41
# Size of source mod 2**32: 6573 bytes
import numpy
...
def compute_vector_trim_points(arr):
    ...
    if nan_elements[0] == 1:
        if len(candidate_start_trim_points) > 0:
        ...
        if nan_elements[(-1)] == 1:
            if len(candidate_end_trim_points) > 0:
...
def compute_matrix_trim_points(arr):
    ...
    if nan_rows[0] == 1:
        if len(row_candidate_start_trim_points) > 0:
        ...
        if nan_rows[(-1)] == 1:
            if len(row_candidate_end_trim_points) > 0:
    ...
    if nan_cols[0] == 1:
        if len(col_candidate_start_trim_points) > 0:
        ...
        if nan_cols[(-1)] == 1:
            if len(col_candidate_end_trim_points) > 0:
