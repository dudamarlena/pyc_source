...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/customcontent/middleware.py
...
class CustomContentMiddleware(object):
    ...
    def process_response(self, request, response):
        ...
        response.content = re.sub('(</head>)', '%s\\1' % head_data, encoded)
        body_data = cc_item(item, 'customitem_set')
        response.content = re.sub('(</body>)', '%s\\1' % body_data, encoded)
        return response