...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/etheno/keyfile.py
...
def _create_v3_keyfile_json(private_key, password, kdf, work_factor=None):
    ...
    if work_factor is None:
        ...
        derived_key = _pbkdf2_hash(password,
          hash_name='sha256',
          salt=salt,
          iterations=work_factor,
          dklen=DKLEN)
        kdfparams = {'c':work_factor, 
         'dklen':DKLEN, 
         'prf':'hmac-sha256', 
         'salt':encode_hex_no_prefix(salt)}
    else:
        if kdf == 'scrypt':
            derived_key = _scrypt_hash(password,
              salt=salt,
              buflen=DKLEN,
              r=SCRYPT_R,
              p=SCRYPT_P,
    ...
    return {'address':remove_0x_prefix(address), 
     'crypto':{'cipher':'aes-128-ctr', 
      'cipherparams':{'iv': encode_hex_no_prefix(int_to_big_endian(iv))}, 
...
def _derive_scrypt_key(crypto, password):
    ...
    derived_scrypt_key = _scrypt_hash(password,
      salt=salt,
      n=n,
      r=r,
      p=p,
...
def _pbkdf2_hash(password, hash_name, salt, iterations, dklen):
    derived_key = hashlib.pbkdf2_hmac(hash_name=hash_name,
      password=password,
      salt=salt,
...
if __name__ == '__main__':
    ...
    print(create_keyfile_json(int_to_bytes(98633124374909015237298838304927890976057357882602286383805396629848713762241), b'etheno'))