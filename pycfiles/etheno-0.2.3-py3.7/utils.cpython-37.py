...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/etheno/utils.py
# Compiled at: 2019-06-27 23:49:12
# Size of source mod 2**32: 3229 bytes
import math, os, socket, tempfile
...
def decode_value(v):
    if isinstance(v, int):
        return v
    if v.startswith('0x') or frozenset(['a', 'b', 'c', 'd', 'e', 'f']) & frozenset(v):
        return int(v, 16)
    return int(v)
