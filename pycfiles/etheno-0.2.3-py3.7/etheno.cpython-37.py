...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/etheno/etheno.py
...
class Etheno(object):
    ...
    def master_client(self, client):
        ...
        self.accounts = list(map(lambda a: int(a, 16), client.post({'id':1, 
         'jsonrpc':'2.0', 
    ...
    def estimate_gas(self, transaction):
        ...
        """
        clients = [
         self.master_client] + self.clients
                ...
                for client in self.clients:
                    try:
                        if hasattr(client, method):
                            ...
                            if isinstance(client, SelfPostingClient):
                                if method == 'eth_getTransactionReceipt':
    ...
    def _create_accounts(self, client):
        for account in self.accounts:
            client.create_account(balance=(int(1e+20)), address=account)
    ...
    def deploy_contract(self, from_address, bytecode, gas=629145, gas_price=None, value=0):
        if gas_price is None:
            ...
            if 'result' in receipt:
                if receipt['result'] and 'contractAddress' in receipt['result'] and receipt['result']['contractAddress']:
