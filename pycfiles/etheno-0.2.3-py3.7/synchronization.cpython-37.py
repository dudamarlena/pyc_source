...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/etheno/synchronization.py
# Compiled at: 2019-06-27 23:49:12
# Size of source mod 2**32: 12006 bytes
        ...
        if self._client == self._client.etheno.master_client:
            ...
            if 'params' in data:
                ...
                if method == 'eth_sendTransaction' or method == 'eth_sendRawTransaction':
                    if ret and 'result' in ret and ret['result'] and self._client.etheno.rpc_client_result and not isinstance(self._client.etheno.rpc_client_result, JSONRPCError):
                        if 'result' in self._client.etheno.rpc_client_result and self._client.etheno.rpc_client_result['result']:
                ...
                elif old_decoded is None:
                    new_decoded is None or self._client.logger.warn('Call to %s returned %s from the master client but %s from this client; ignoring...' % (method, self._client.etheno.rpc_client_result['result'], ret['result']))
        else:
            if method == 'eth_getTransactionReceipt':
                ...
                if 'contractAddress' in ret['result']:
                    if ret['result']['contractAddress']:
                        ...
                        if master_address is not None and our_address is not None:
                            ...
                            master_address is None and our_address is None or self._client.logger.warn('Call to %s returned %s from the master client but %s from this client; ignoring...' % (method, self._client.etheno.rpc_client_result['result']['contractAddress'], ret['result']['contractAddress']))
            return ret
