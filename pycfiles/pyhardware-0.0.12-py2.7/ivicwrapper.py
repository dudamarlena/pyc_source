...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pyivi\ivic\ivicwrapper.py
# Compiled at: 2013-08-28 11:45:15
import ctypes
...
class IviCWrapper(object):
    ...
    def __init__(self, session, address, simulate=False):
        ...
        self.call('InitWithOptions', address, False, False, 'Simulate = ' + str(simulate).lower(), ctypes.byref(self.visession))
        for name, wrapper_class in self._repeated_capabilities.iteritems():
            dic = OrderedDict()
    ...
    def _new_attr(cls, name, id, type_, multi_channel, multi_trace, read_only):
        ...
        """
        c_type = {'ViInt32': ctypes.c_int, 'ViReal64': ctypes.c_double, 
        ...
        def getter(self):
            ...
            if multi_channel:
                ...
                if multi_trace:
                    ...
                    channel_name = ''
                try:
                    self.call('GetAttribute' + type_, self.visession, channel_name, id, ctypes.byref(val))
                except IviCError:
        ...
        if read_only:
            ...
            def setter(self, val):
                ...
                self.call('SetAttribute' + type_, self.visession, channel_name, id, val)
    ...
    def call(self, funcname, *args):
        ...
        arguments
        """
        func = self._session.__getattr__(self._session._name + '_' + funcname)
        return_value = func(*args)
        if return_value:
            error_message = ctypes.create_string_buffer(256)
            self.call('error_message', self.visession, return_value, ctypes.byref(error_message))
