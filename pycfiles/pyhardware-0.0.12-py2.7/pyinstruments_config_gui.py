...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pyhardware\config\gui\pyinstruments_config_gui.py
...
from pyhardware.drivers import VisaDriver, SerialDriver
...
class PyInstrumentsWindow(QtGui.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.setWindowIcon(QtGui.QIcon(os.path.split(__file__)[0] + '/usb.png'))
        QtCore.QObject.connect(self.actionRefresh, QtCore.SIGNAL('triggered()'), self.dev_list.refresh)
        QtCore.QObject.connect(self.actionAutoDetect, QtCore.SIGNAL('triggered()'), self.dev_list.auto_detect)
        QtCore.QObject.connect(self.actionRemoveAll, QtCore.SIGNAL('triggered()'), self.dev_list.remove_all)
        QtCore.QObject.connect(self.actionQueryModels, QtCore.SIGNAL('triggered()'), self.dev_list.query_models)
        self.central = CentralWidgetInstruments(self)
        self.setCentralWidget(self.central)
        self.addDockWidget(QtCore.Qt.DockWidgetArea(QtCore.Qt.LeftDockWidgetArea), self.dev_list)
        self.statusbar.showMessage('running...')
        self.resize(QtCore.QSize(1500, 700))

    def retranslateUi(self, MainWindow):
        """qt's stuffs..."""
        MainWindow.setWindowTitle(QtGui.QApplication.translate('MainWindow', 'pyinstruments GUI', None, QtGui.QApplication.UnicodeUTF8))
        ...
        self.actionQueryModels.setText(QtGui.QApplication.translate('MainWindow', 'query models', None, QtGui.QApplication.UnicodeUTF8))
        return
...
class PyInstrumentsConfigGui(QtGui.QDockWidget):
    ...
    def _refresh(self, full=False):
        ...
        for tag, instr in pic.iteritems():
            ...
            try:
                widgetItem.set_values(tag, instr['address'], instr['model'], instr['simulate'], instr['code'])
    ...
    def contextMenuModels(self, point):
        ...
        def add_model_in_menu(menu, model):
            ...
            model_action.triggered.connect(model_action.change_model_to_mine)
    ...
    def contextMenu(self, point):
        ...
        if len(items) > 0:
            ...
            actionRemoveDevice = QtGui.QAction('remove ' + logical_name + '?', self)
            ...
            actionQueryModel = QtGui.QAction('query model at address ' + address, self)
    ...
    def get_from_gui(self):
        ...
        for itm in self.tree_widget:
            pic[itm.val('logical_name')] = {'address': itm.val('address'), 'model': itm.val('model'), 'simulate': itm.val('simulate'), 
               'code': itm.val('code')}
