...
# [GCC 8.4.0]
# Embedded file name: /home/luke/PycharmProjects/untitled1/NearBeach/views_project_information.py
...
def information_project_customer(request, project_id):
    project_groups_results = object_assignment.objects.filter(is_deleted='FALSE',
      project_id=project.objects.get(project_id=project_id)).values('group_id')
    ...
    if request.method == 'POST':
        ...
        submit_customer = project_customer(project_id=project.objects.get(pk=project_id),
    ...
    new_customers_results = customer.objects.filter(organisation_id=(project_results.organisation_id),
      is_deleted='FALSE').exclude(customer_id__in=(project_customer.objects.filter(is_deleted='FALSE',
      project_id=(project_results.project_id)).values('customer_id')))
    project_customers_results = project_customer.objects.filter(is_deleted='FALSE',
...
def information_project_history(request, project_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            current_user = project_history_results == '' or request.user
            project_id_instance = project.objects.get(pk=project_id)
            data = project_history(project_id=project_id_instance,
            ...
            print(form.errors)
    project_history_results = project_history.objects.filter(project_id=project_id,
      is_deleted='FALSE')
    t = loader.get_template('NearBeach/project_information/project_history.html')
    c = {'project_history_results':project_history_results, 
     'project_id':project_id, 
     'project_permissions':permission_results['project'], 
