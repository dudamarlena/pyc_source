...
# [GCC 8.4.0]
# Embedded file name: /home/luke/PycharmProjects/untitled1/NearBeach/views_requirements.py
# Compiled at: 2020-03-01 01:12:07
# Size of source mod 2**32: 35816 bytes
...
def requirement_customer_information(request, requirement_id):
    ...
    if request.method == 'POST':
        if permission_results['requirement'] > 1:
            ...
            submit_customer = requirement_customer(requirement_id=requirement_id,
              customer_id=customer_id,
              change_user=(request.user))
            submit_customer.save()
    else:
        requirement_customer_results = customer.objects.filter(is_deleted='FALSE',
          customer_id__in=(requirement_customer.objects.filter(is_deleted='FALSE',
        ...
        if requirement_results.organisation:
            new_customers_results = customer.objects.filter(is_deleted='FALSE',
              organisation_id_id=(requirement_results.organisation_id)).exclude(customer_id__in=(requirement_customer.objects.filter(is_deleted='FALSE',
              requirement_id=requirement_id).values('customer_id')))
        else:
            new_customers_results = customer.objects.filter(is_deleted='FALSE',
              organisation_id__isnull=True).exclude(customer_id__in=(requirement_customer.objects.filter(is_deleted='FALSE',
              requirement_id=requirement_id).values('customer_id')))
    t = loader.get_template('NearBeach/requirement_information/requirement_customer_information.html')
    c = {'requirement_customer_results':requirement_customer_results, 
...
def new_requirement(request, location_id='', destination=''):
    ...
    if permission_results['requirement'] < 2:
        return HttpResponseRedirect(reverse('permission_denied'))
    elif request.method == 'POST' and permission_results['requirement'] > 2:
        ...
        if form.is_valid():
            ...
            requirement_save = requirement(requirement_title=requirement_title,
            ...
            for row in requirement_permission:
                submit_permission = object_assignment(requirement_id=requirement_save,
                  group_id=row,
                  change_user=(request.user))
                submit_permission.save()

            if destination == 'opportunity':
                object_assignment_submit = object_assignment(requirement_id=requirement_save,
    ...
    c = {'new_requirement_form':new_requirement_form(), 
...
def new_requirement_item(request, requirement_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            requirement_item_submit = requirement_item(requirement_item_title=(form.cleaned_data['requirement_item_title']),
    ...
    c = {'new_requirement_item_form':new_requirement_item_form(), 
     'requirement_id':requirement_id, 
...
def new_requirement_item_link(request, requirement_item_id, location_id='', destination=''):
    ...
    if request.method == 'POST':
        if location_id == '' or destination == '':
            ...
            requirement_item_link_submit = requirement_item_link(change_user=(request.user),
    ...
    project_results = project.objects.filter(is_deleted='FALSE',
     ...
     'New', 'Open'},
      ...
      username_id=(request.user.id)).values('group_id'))).values('project_id'))).exclude(project_id__in=(requirement_item_link.objects.filter(is_deleted='FALSE',
      project_id__isnull=False,
      requirement_item_id=requirement_item_id).values('project_id')))
    task_results = task.objects.filter(is_deleted='FALSE',
      task_status__in={
     'New', 'Open'},
      task_id__in=(object_assignment.objects.filter(is_deleted='FALSE',
      group_id__in=(user_group.objects.filter(is_deleted='FALSE',
      username_id=(request.user.id)).values('group_id'))).values('task_id'))).exclude(task_id__in=(requirement_item_link.objects.filter(is_deleted='FALSE',
      task_id__isnull=False,
      requirement_item_id=requirement_item_id).values('task_id')))
    t = loader.get_template('NearBeach/requirement_information/new_requirement_link.html')
    c = {'requirement_item_id':requirement_item_id, 
...
def new_requirement_link(request, requirement_id, location_id='', destination=''):
    ...
    if request.method == 'POST':
        if location_id == '' or destination == '':
            ...
            object_assignment_submit = object_assignment(change_user=(request.user),
    ...
    project_results = project.objects.filter(is_deleted='FALSE',
     ...
     'Backlog', 'Blocked', 'In Progress', 'Test/Review'},
      project_id__in=(object_assignment.objects.filter(is_deleted='FALSE',
      group_id__in=(user_group.objects.filter(is_deleted='FALSE',
      username_id=(request.user.id)).values('group_id'))).values('project_id'))).exclude(project_id__in=(object_assignment.objects.filter(is_deleted='FALSE',
      project_id__isnull=False,
      requirement_id=requirement_id).values('project_id')))
    task_results = task.objects.filter(is_deleted='FALSE',
      task_status__in={
     'Backlog', 'Blocked', 'In Progress', 'Test/Review'},
      task_id__in=(object_assignment.objects.filter(is_deleted='FALSE',
      group_id__in=(user_group.objects.filter(is_deleted='FALSE',
      username_id=(request.user.id)).values('group_id'))).values('task_id'))).exclude(task_id__in=(object_assignment.objects.filter(is_deleted='FALSE',
      task_id__isnull=False,
      requirement_id=requirement_id).values('task_id')))
    opportunity_results = opportunity.objects.filter(is_deleted='FALSE',
      ...
      group_id__in=(user_group.objects.filter(is_deleted='FALSE',
      username_id=(request.user.id)).values('group_id'))).values('opportunity_id')),
     ...
     '6',
     '7',
     '8'}).exclude(opportunity_id__in=(object_assignment.objects.filter(is_deleted='FALSE',
      opportunity_id__isnull=False,
      requirement_id=requirement_id).values('opportunity_id')))
    t = loader.get_template('NearBeach/requirement_information/new_requirement_link.html')
...
def requirement_information(request, requirement_id):
    ...
    initial = {'requirement_title':requirement_results.requirement_title, 
...
def requirement_item_information(request, requirement_item_id):
    ...
    if permission_results['requirement'] == 0:
        return HttpResponseRedirect(reverse('permission_denied'))
    elif request.method == 'POST' and permission_results['requirement'] > 1:
    ...
    link_results = object_assignment.objects.filter(is_deleted='FALSE',
      requirement_item_id=requirement_item_id,
      assigned_user__isnull=True,
      group_id__isnull=True)
    initial = {'requirement_item_title':requirement_item_results.requirement_item_title, 
...
def requirement_item_link_remove(request, requirement_item_link_id):
    ...
    if request.method == 'POST':
        ...
        if permission_results['requirement_link'] < 4:
            return HttpResponseRedirect(reverse('permission_denied'))
        requirement_item_link.objects.filter(requirement_item_link_id=requirement_item_link_id).update(is_deleted='TRUE')
...
def requirement_link_list(request, requirement_id):
    ...
    requirement_item_results = requirement_item.objects.filter(is_deleted='FALSE',
      requirement_id=requirement_id)
    requirement_link_results = object_assignment.objects.filter(is_deleted='FALSE',
      requirement_id=requirement_id,
      assigned_user__isnull=True,
      group_id__isnull=True,
    ...
    c = {'requirement_item_results':requirement_item_results, 
...
def requirement_link_remove(request, requirement_link_id):
    ...
    if request.method == 'POST':
        ...
        if permission_results['requirement_link'] < 4:
            return HttpResponseRedirect(reverse('permission_denied'))
        requirement_link.objects.filter(requirement_link_id=requirement_link_id).update(is_deleted='TRUE')
        t = loader.get_template('NearBeach/blank.html')
        c = {}
        return HttpResponse(t.render(c, request))
    return HttpResponseBadRequest('Sorry, can only do this in post')
...
def requirement_readonly(request, requirement_id):
    ...
    c = {'requirement_readonly_form':requirement_readonly_form(initial={'requirement_scope': requirement_results.requirement_scope}), 
     'requirement_results':requirement_results, 
     'requirement_item_results':requirement_item_results, 
     'requirement_id':requirement_id, 
...
def requirement_remove_customer(request, requirement_id, customer_id):
    ...
    if permission_results['requirement'] < 4:
        return HttpResponseRedirect(reverse('permission_denied'))
    if request.method == 'POST':
        update_customer = requirement_customer.objects.filter(is_deleted='FALSE',
          requirement_id=requirement_id,
          customer_id=customer_id)
