...
# [GCC 8.4.0]
# Embedded file name: /Users/ben/dev/FAB/fab_controller/FAB.py
# Compiled at: 2015-03-09 10:48:57
import functools, json, webbrowser
from collections import deque
from datetime import datetime
import os, re, signal, sys, pyfirmata
...
live_pins = {'left': {'high_limit_pin': board.get_pin(('d:{}:i').format(HIGH_LIMIT_PIN.left)), 
   ...
   'right': {'high_limit_pin': board.get_pin(('d:{}:i').format(HIGH_LIMIT_PIN.right)), 
...
gevent.joinall([
 ...
 gevent.spawn(check_pin, live_pins['right']['sensor_pin'])], timeout=15, raise_error=True)
scale_range = lambda x, OldMin, OldMax, NewMin, NewMax: (x - OldMin) * (NewMax - NewMin) / (OldMax - OldMin) + NewMin
...
class Crusher(object):
    ...
    def go_to_top_and_init(self):
        ...
        self.pulse(n=100)
        print (
         'Moving', self.name, 'to top to initialise.')
        self.go_to_top()
    ...
    def pulse(self, n=1):
        ...
        if self.direction is DOWN and self.steps_from_top >= MAX_STEPS - n and not self.at_bottom:
            print (
             self.name, 'too low to step. Now at ', self.steps_from_top, 'Need to step ', n)
            return AT_BOTTOM
        if self.direction is UP:
            if self.steps_from_top <= 100 or self.at_top:
                print (
                 self.name, 'too high to step. Now at ', self.steps_from_top, '. At-top=', self.at_top, '. Need to step ', n)
                if self.at_top:
                    self.steps_from_top = 0
                return AT_TOP
    ...
    def track(self):
        ...
        margin = max([ALLOWABLE_DISCREPANCY, self.target * 0.05])
...
def manual_pulse(data):
    print (
     'Executing manual pulses', str(data))
    crusher = getattr(app, data['hand'])
...
def build_log_entry(app):
    return {'target_L': app.left.target, 
...
def validate_json_program(jsondata):
    try:
        ...
        prog_ints = [ list(map(int, i)) for i in lines ]
        return [ Block(x[0], Pair(x[1], x[2])) for x in prog_ints ]
...
def log_sensors():
    while 1:
        if app.left.target > 0 or app.right.target > 0:
            _log_session_data({'measurement': Pair(app.left.grams(), app.right.grams()), 
...
if __name__ == '__main__':
    ...
    app.left = Crusher(live_pins['left']['sensor_pin'].read(), TWO_KG.left, 'left')
    app.right = Crusher(live_pins['right']['sensor_pin'].read(), TWO_KG.right, 'right')
    gevent.joinall([
