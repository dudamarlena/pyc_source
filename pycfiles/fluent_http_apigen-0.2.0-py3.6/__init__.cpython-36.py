...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/apigen/__init__.py
# Compiled at: 2019-11-27 13:21:44
...
class DefaultLineHandler:
    ...
    def handle(self, line, context):
        return [
         (self.regexp.sub(self.replacement, line).format)(**context).strip()]
...
class EndPointLineHandler(DefaultLineHandler):
    ...
    def handle(self, line, context):
        lines = [
         self.add_anchor((self.regexp.sub(self.replacement, line).format)(**context))]
...
LINE_HANDLERS = [
 JavadocLineHandler(),
 ContextLineHandler('@Prefix\\("([a-zA-Z/]+)".*', 'url_prefix'),
 DefaultLineHandler('@Prefix\\("([a-zA-Z/]+)".*', '# <a name="\\1"></a>\\1'),
 EndPointLineHandler('@(\\w+)\\("?([a-zA-Z/:\\?=]*)"?.*', '## \\1 {url_prefix}\\2')]
TOC_LINE_HANDLERS = [
 ContextLineHandler('@Prefix\\("([a-zA-Z/]+)".*', 'url_prefix'),
 DefaultLineHandler('@Prefix\\("([a-zA-Z/]+)".*', '- [\\1](#\\1)'),
 TocEndPointLineHandler('@(\\w+)\\("?([a-zA-Z/:\\?=]*)"?.*', '- \\1 {url_prefix}\\2')]
