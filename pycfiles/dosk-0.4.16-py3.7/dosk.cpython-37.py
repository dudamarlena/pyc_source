...
# Compiled at: 2020-04-18 21:31:09
# Size of source mod 2**32: 14404 bytes
import argparse, json, asyncio, functools, logging, os, shlex, signal, string, sys
from ruamel.yaml import YAML
import string, random, copy, coloredlogs
...
def parse_args():
    ...
    verbose.add_argument('--verbose', '-v', action='store_const', const=(logging.DEBUG), default=(logging.INFO), help='Print additional information')
    verbose.add_argument('--quiet', '-q', action='store_const', dest='verbose', const=(logging.WARNING), help='Print less information')
...
class AsyncTaskList(TaskList):
    ...
    async def run(self, args=None):
        ...
        await asyncio.wait([task.run() for task in self.tasks],
