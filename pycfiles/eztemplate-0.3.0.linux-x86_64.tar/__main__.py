...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/eztemplate/__main__.py
# Compiled at: 2016-02-23 00:28:12
"""Provide a simple templating system for text files."""
from __future__ import absolute_import
from __future__ import print_function
import argparse, errno, os, os.path, re, sys
from . import engines
...
def parse_args(args=None):
    ...
    parser = argparse.ArgumentParser(prog=__package__, description='Make substitutions in text files.')
    parser.add_argument('-V', '--version', action='version', version='%%(prog)s %s' % (__version__,))
    group = parser.add_argument_group('Engine')
    group.add_argument('-e', '--engine', dest='engine', default='string.Template', help='templating engine', metavar='ENGINE')
    group.add_argument('-t', '--tolerant', action='store_true', dest='tolerant', help="don't fail on missing names")
    group = parser.add_argument_group('Output')
    group.add_argument('-s', '--stdout', action='append_const', dest='outfiles', const=sys.stdout, help='use standard output')
    group.add_argument('-o', '--outfile', action='append', dest='outfiles', help='output file', metavar='FILE')
    group.add_argument('--vary', action='store_true', dest='vary', help='vary output file name according to template')
    group.add_argument('-r', '--read-old', action='store_true', dest='read_old', help='read preexisting output files andhand the respective content to the template')
    group.add_argument('-d', '--delete-empty', action='store_true', dest='delete_empty', help='delete file if output is empty')
    group = parser.add_argument_group('Input')
    group.add_argument('--stdin', action='append_const', dest='infiles', const=sys.stdin, help='use standard input')
    group.add_argument('-i', '--infile', action='append', dest='infiles', help='any number of input files', metavar='FILE')
    group.add_argument('-c', '--concatenate', action='store_true', dest='concatenate', help='concatenate multiple input files into one output')
    group = parser.add_argument_group('Name-value pairs')
    group.add_argument('-a', '--arg', action='append', dest='args', help='any number of name-value pairs', metavar='NAME=VALUE')
    group.add_argument('-p', '--pyarg', action='append', dest='args', type=_PyArg, help='evaluate a python expression', metavar='NAME=EXPRESSION')
    group.add_argument('-n', '--next', action='append_const', dest='args', const='--', help='begin next argument group')
    parser.add_argument(dest='remainder', nargs=argparse.REMAINDER, help='possible input files and name-value pair groups if not already specified through options')
    ...
    if not args.infiles:
        if args.args:
            ...
            last = len(args.remainder) if args.vary or args.concatenate else first + 1
        ...
        args.infiles = [ path if path != '-' else sys.stdin for path in infiles ] if infiles else [sys.stdin]
    if args.args:
        flat_args = args.args
    else:
        ...
        for arg in flat_args:
            if isinstance(arg, _PyArg):
                ...
                name_value = arg.split('=', 1)
                mapping[name_value[0]] = name_value[1] if len(name_value) > 1 else None
...
def make_path_properties(file_or_path, prefix=''):
    ...
    if is_std or not path:
        ...
        numbers = [ int(s) for s in re.findall('\\d+', basename) ]
...
def perform_templating(args):
    ...
    if args.vary:
        it = variable_outfile_iterator(args.outfiles, args.infiles, args.args, engine)
    else:
        it = constant_outfile_iterator(args.outfiles, args.infiles, args.args)
    process_combinations(it, engine, tolerant=args.tolerant, read_old=args.read_old, delete_empty=args.delete_empty)
