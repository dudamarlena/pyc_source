...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pylxca/pylxca_cmd/lxca_icommands.py
# Compiled at: 2020-03-12 01:38:00
# Size of source mod 2**32: 10741 bytes
"""
@since: 15 Sep 2015
...
"""
import sys, getopt, os, json, logging, traceback, argparse
...
class InteractiveCommand(object):
    ...
    def _validate_combination(self, input_dict, valid_dict):
        ...
        for k in input_dict.keys():
            if k not in ('func', 'view', 'subcmd'):
                copy_input_dict[k] = input_dict[k] == None or isinstance(input_dict[k], list) and len(input_dict[k]) == 0 or input_dict[k]
    ...
    def get_argparse_options(self):
        parser = argparse.ArgumentParser(prog=(self.get_name()), description=(self.get_short_desc()), formatter_class=(argparse.RawDescriptionHelpFormatter),
        ...
        if subcmd_list:
            ...
            for subcmd in subcmd_list:
                ...
                parser_internal = sub_parser.add_parser(sub_cmd_name, formatter_class=(argparse.RawDescriptionHelpFormatter),
                  help=(subcmd['help']),
                  epilog=(self._process_epilog(subcmd['additional_detail'])))
