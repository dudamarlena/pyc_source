...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pylxca/pylxca_api/lxca_api.py
# Compiled at: 2020-03-12 01:40:34
# Size of source mod 2**32: 50751 bytes
"""
@since: 21 Oct 2015
...
class lxca_api(with_metaclass(Singleton, object)):
    ...
    def __init__(self):
        ...
        """
        self.con = None
        self.func_dict = {'connect':self.connect,  'disconnect':self.disconnect, 
    ...
    def api(self, object_name, dict_handler=None, con=None):
        try:
            if con == None:
                if self.con:
                    if isinstance(self.con, lxca_connection):
                        con = self.con
    ...
    def do_manage(self, dict_handler=None):
        ...
        resp = lxca_rest().do_manage(self.con.get_url(), self.con.get_session(), ip_addr, user, pw, rpw, force, jobid, storedcredential_id)
        try:
            py_obj = json.loads(resp.text)
            return py_obj
        except AttributeError as ValueError:
    ...
    def do_configpatterns(self, dict_handler=None):
        ...
        if not self.con:
            ...
            if dict_handler:
                ...
                patternname = next((item for item in [dict_handler.get('n'), dict_handler.get('name')] if item is not None), None)
            ...
            if patternname:
                if not patternid:
                    resp = lxca_rest().do_configpatterns(self.con.get_url(), self.con.get_session(), None, None, None, None, None, None)
                    py_obj = json.loads(resp.text)
    ...
    def get_configprofiles(self, dict_handler=None):
        ...
        if dict_handler:
            ...
            profilename = next((item for item in [dict_handler.get('n'), dict_handler.get('name')] if item is not None), None)
            endpoint = next((item for item in [dict_handler.get('e'), dict_handler.get('endpoint')] if item is not None), None)
            restart = next((item for item in [dict_handler.get('r'), dict_handler.get('restart')] if item is not None), None)
            subcmd = next((item for item in [dict_handler.get('subcmd')] if item is not None), None)
            powerdown = next((item for item in [dict_handler.get('p'), dict_handler.get('powerdown')] if item is not None), None)
            resetimm = next((item for item in [dict_handler.get('resetimm')] if item is not None), None)
            resetswitch = next((item for item in [dict_handler.get('resetswitch')] if item is not None), None)
            force = next((item for item in [dict_handler.get('f'), dict_handler.get('force')] if item is not None), None)
        if subcmd == 'rename':
            if profilename:
                resp = lxca_rest().put_configprofiles(self.con.get_url(), self.con.get_session(), profileid, profilename)
        if subcmd == 'activate' and endpoint and restart:
    ...
    def get_set_osimage(self, dict_handler=None):
        ...
        if 'list' in osimages_info:
            ...
            if 'globalsettings' in osimages_info:
                ...
                if 'hostsettings' in osimages_info:
                    if action:
                        if action in ('update', ):
        ...
        if not self.con:
            ...
            if 'list' in subcmd:
                ...
                if 'delete' in subcmd:
                    ...
                    if 'create' in subcmd:
                        if 'dynamic' in type:
                            resp = lxca_rest().dynamic_resourcegroups(self.con.get_url(), self.con.get_session(), uuid, name, desc, type, criteria)
                        else:
                            if 'solution' in type:
                                resp = lxca_rest().solution_resourcegroups(self.con.get_url(), self.con.get_session(), uuid, name, desc, type, solutionVPD, members, criteria)
                            else:
    ...
    def get_set_rules(self, dict_handler=None):
        ...
        if not self.con:
            ...
            if dict_handler:
                id = next((item for item in [dict_handler.get('i'), dict_handler.get('id')] if item is not None), None)
                rule = next((item for item in [dict_handler.get('r'), dict_handler.get('rule')] if item is not None), None)
            if rule:
        ...
        if not self.con:
            ...
            if dict_handler:
                ...
                query_solutionGroups = next((item for item in [
                 dict_handler.get('q'), dict_handler.get('query_solutionGroups')] if item is not None), None)
                solutionGroups = next((item for item in [dict_handler.get('s'),
                 dict_handler.get('solutionGroups')] if item is not None), None)
                targetResources = next((item for item in [dict_handler.get('t'), dict_handler.get('targetResources')] if item is not None), None)
                all_rules = next((item for item in [dict_handler.get('a'), dict_handler.get('all_rules')] if item is not None), None)
            if all_rules or solutionGroups or targetResources:
                resp = lxca_rest().set_compositeResults(self.con.get_url(), self.con.get_session(), solutionGroups, targetResources, all_rules)
            else:
                resp = lxca_rest().get_compositeResults(self.con.get_url(), self.con.get_session(), id, query_solutionGroups)
        py_obj = json.loads(resp.text)
        return py_obj
    ...
    def get_set_storedcredentials(self, dict_handler=None):
        ...
        if dict_handler:
            id = next((item for item in [dict_handler.get('i'), dict_handler.get('id')] if item is not None), None)
            user_name = next((item for item in [dict_handler.get('u'), dict_handler.get('user_name')] if item is not None), None)
            description = next((item for item in [dict_handler.get('d'), dict_handler.get('description')] if item is not None), None)
            password = next((item for item in [dict_handler.get('p'), dict_handler.get('password')] if item is not None), None)
            delete_id = next((item for item in [dict_handler.get('delete_id')] if item is not None), None)
        if delete_id:
            resp = lxca_rest().delete_storedcredentials(self.con.get_url(), self.con.get_session(), delete_id)
            resp = json.loads(resp.text)
        elif id:
            if user_name or password or description:
