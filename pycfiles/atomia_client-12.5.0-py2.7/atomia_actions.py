...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\atomia_client\atomia_actions.py
# Compiled at: 2012-04-24 09:35:35
"""
...
class AtomiaActions(object):
    ...
    def create_service(self, service_name, parent_service, account_number, username=None, password=None):
        username = self.username if username == None else username
        password = self.password if password == None else password
        return self.client.CreateService(username, password, serviceName=service_name, parentService=parent_service, accountName=account_number)
    ...
    def add_service(self, service, parent_service, account_number, resource_request_descriptions=None, username=None, password=None):
        username = self.username if username == None else username
        password = self.password if password == None else password
        return self.client.AddService(username, password, service=service, parentService=parent_service, accountName=account_number, resourceRequestDescriptions=resource_request_descriptions)

    def delete_service(self, service, account_number, username=None, password=None):
        username = self.username if username == None else username
        password = self.password if password == None else password
        return self.client.DeleteService(username, password, service=service, accountName=account_number)

    def modify_service(self, service, account_number, username=None, password=None):
        username = self.username if username == None else username
        password = self.password if password == None else password
        return self.client.ModifyService(username, password, service=service, accountName=account_number)
    ...
    def find_services_by_path_with_paging(self, search_criteria_list, account_number, search_properties=None, sort_by_prop_name='', sort_asc='true', page_number='0', page_size='100', username=None, password=None):
        username = self.username if username == None else username
        password = self.password if password == None else password
        return self.client.FindServicesByPathWithPaging(username, password, searchCriteriaList=search_criteria_list, properties=search_properties, account=account_number, sortByPropName=sort_by_prop_name, sortAsc=sort_asc, pageNumber=page_number, pageSize=page_size)

    def get_service_by_id(self, account_number, service_id, username=None, password=None):
        username = self.username if username == None else username
        password = self.password if password == None else password
        return self.client.GetServiceById(username, password, accountId=account_number, serviceID=service_id)
    ...
    def list_existing_service(self, parent_service, account_number, get_logical_children='true', sort_asc='true', page_number='0', page_size='100', username=None, password=None):
        username = self.username if username == None else username
        password = self.password if password == None else password
        return self.client.ListExistingServices(username, password, parentService=parent_service, getLogicalChildren=get_logical_children, accountId=account_number, sortAsc=sort_asc, pageNumber=page_number, pageSize=page_size)