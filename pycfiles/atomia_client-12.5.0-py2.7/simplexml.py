...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\atomia_client\pysimplesoap_atomia\simplexml.py
...
Date = datetime.date
Time = datetime.time
TYPE_MAP = {str: 'string', unicode: 'string', bool: 'boolean', 
   short: 'short', byte: 'byte', int: 'int', 
   long: 'long', integer: 'integer', float: 'float', 
   double: 'double', Decimal: 'decimal', 
   datetime.datetime: 'dateTime', 
   datetime.date: 'date'}
TYPE_MARSHAL_FN = {datetime.datetime: datetime_m, datetime.date: date_m}
TYPE_UNMARSHAL_FN = {datetime.datetime: datetime_u, datetime.date: date_u, bool: bool_u}
...
class SimpleXMLElement(object):
    ...
    def __init__(self, text=None, elements=None, document=None, namespace=None, prefix=None):
        ...
        if text:
            ...
            self.__elements = [
             self.__document.documentElement]
        else:
    ...
    def add_child(self, name, text=None, ns=True):
        ...
        if not ns or not self.__ns:
            ...
            if DEBUG:
                print 'adding %s ns %s %s' % (name, self.__ns, ns)
            if self.__prefix:
        ...
        return SimpleXMLElement(elements=[
         element], document=self.__document, namespace=self.__ns, prefix=self.__prefix)
    ...
    def __setattr__(self, tag, text):
        ...
        if tag.startswith('_'):
            ...
            if DEBUG:
                print '__setattr__(%s,%s)' % (tag, text)
    ...
    def get_namespace_uri(self, ns):
        ...
        return getattr(self.__document.documentElement.attributes, 'xmlns:%s' % ns, None)
            ...
            if tag is None:
                ...
                if ns and not elements:
                    for ns_uri in isinstance(ns, (tuple, list)) and ns or (ns,):
                        if DEBUG:
                            print 'searching %s by ns=%s' % (tag, ns_uri)
                ...
                if not elements:
                    if DEBUG:
                        print self._element.toxml()
                    if error:
                        raise AttributeError('No elements found')
                    else:
                        return
                return SimpleXMLElement(elements=elements, document=self.__document, namespace=self.__ns, prefix=self.__prefix)
    ...
    def __iter__(self):
        ...
        try:
            for __element in self.__elements:
                yield SimpleXMLElement(elements=[
                 __element], document=self.__document, namespace=self.__ns, prefix=self.__prefix)
