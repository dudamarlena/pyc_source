...
# [GCC 8.4.0]
# Embedded file name: C:\Users\derek_2\Google Drive\nvda-addon-exploded\notepad++\scons-local-2.5.0\SCons\Node\FS.py
...
import fnmatch, os, re, shutil, stat, sys, time, codecs, SCons.Action, SCons.Debug
from SCons.Debug import logInstanceCreation
import SCons.Errors, SCons.Memoize, SCons.Node, SCons.Node.Alias, SCons.Subst, SCons.Util, SCons.Warnings
...
Valid_Duplicates = [
 'hard-soft-copy', 'soft-hard-copy',
 'hard-copy', 'soft-copy', 'copy']
Link_Funcs = []

def set_duplicate(duplicate):
    global Link_Funcs
    link_dict = {'hard': _hardlink_func, 
       'soft': _softlink_func, 
       'copy': _copy_func}
...
def get_DefaultSCCSBuilder():
    ...
    if DefaultSCCSBuilder is None:
        ...
        DefaultSCCSBuilder = SCons.Builder.Builder(action=act, env=None, name='DefaultSCCSBuilder')
...
def get_DefaultRCSBuilder():
    ...
    if DefaultRCSBuilder is None:
        ...
        DefaultRCSBuilder = SCons.Builder.Builder(action=act, env=None, name='DefaultRCSBuilder')
...
class EntryProxy(SCons.Util.Proxy):
    ...
    dictSpecialAttrs = {'base': __get_base_path, 'posix': __get_posix_path, 
...
node_bwcomp = {'abspath': Base.get_abspath, 'labspath': Base.get_labspath, 
...
class Dir(Base):
    ...
    def rel_path(self, other):
        ...
        if self is other:
            ...
            path_elems = [
             '..'] * (len(self._path_elements) - i) + [ n.name for n in other._path_elements[i:] ]
    ...
    def srcdir_find_file(self, filename):
        ...
        for srcdir in self.srcdir_list():
            for rdir in srcdir.get_all_rdirs():
                ...
                if node:
                    result = (
                     File(filename, self, self.fs), srcdir)
                    memo_dict[filename] = result
                    return result
    ...
    def file_on_disk(self, name):
        if self.entry_exists_on_disk(name) or diskcheck_rcs(self, name) or diskcheck_sccs(self, name):
            try:
...
class RootDir(Dir):
    r"""A class for the root directory of a file system.
