...
# [GCC 8.4.0]
# Embedded file name: C:\Users\derek_2\Google Drive\nvda-addon-exploded\notepad++\scons-local-2.5.0\SCons\Tool\install.py
...
import os, re, shutil, stat, SCons.Action, SCons.Tool, SCons.Util
_INSTALLED_FILES = []
...
def add_targets_to_INSTALLED_FILES(target, source, env):
    ...
    global _UNIQUE_INSTALLED_FILES
...
def generate(env):
    ...
    if BaseInstallBuilder is None:
        ...
        if install_sandbox:
            ...
            target_factory = env.fs
        BaseInstallBuilder = SCons.Builder.Builder(action=install_action, target_factory=target_factory.Entry, source_factory=env.fs.Entry, multi=1, emitter=[
         add_targets_to_INSTALLED_FILES], source_scanner=SCons.Scanner.Base({}, name='Install', recursive=False), name='InstallBuilder')
    if BaseVersionedInstallBuilder is None:
        ...
        if install_sandbox:
            ...
            target_factory = env.fs
        BaseVersionedInstallBuilder = SCons.Builder.Builder(action=installVerLib_action, target_factory=target_factory.Entry, source_factory=env.fs.Entry, multi=1, emitter=[
         add_versioned_targets_to_INSTALLED_FILES], name='InstallVersionedBuilder')
