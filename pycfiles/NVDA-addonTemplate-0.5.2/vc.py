...
# [GCC 8.4.0]
# Embedded file name: C:\Users\derek_2\Google Drive\nvda-addon-exploded\notepad++\scons-local-2.5.0\SCons\Tool\MSCommon\vc.py
...
import SCons.compat, SCons.Util, os, platform
from string import digits as string_digits
import SCons.Warnings, common
debug = common.debug
import sdk
...
_ARCH_TO_CANONICAL = {'amd64': 'amd64', 
   'emt64': 'amd64', 
   'i386': 'x86', 
   'i486': 'x86', 
   'i586': 'x86', 
   ...
   'x86_64': 'amd64', 
   'x86_amd64': 'x86_amd64'}
_HOST_TARGET_ARCH_TO_BAT_ARCH = {('x86', 'x86'): 'x86', 
   ('x86', 'amd64'): 'x86_amd64', 
...
_VCVER = [
 '14.0', '14.0Exp', '12.0', '12.0Exp', '11.0', '11.0Exp', '10.0', '10.0Exp', '9.0', '9.0Exp', '8.0', '8.0Exp', '7.1', '7.0', '6.0']
_VCVER_TO_PRODUCT_DIR = {'14.0': [
          (
           SCons.Util.HKEY_LOCAL_MACHINE, 'Microsoft\\VisualStudio\\14.0\\Setup\\VC\\ProductDir')], 
   '14.0Exp': [
             (
              SCons.Util.HKEY_LOCAL_MACHINE, 'Microsoft\\VCExpress\\14.0\\Setup\\VC\\ProductDir')], 
   '12.0': [
          (
           SCons.Util.HKEY_LOCAL_MACHINE, 'Microsoft\\VisualStudio\\12.0\\Setup\\VC\\ProductDir')], 
   '12.0Exp': [
             (
              SCons.Util.HKEY_LOCAL_MACHINE, 'Microsoft\\VCExpress\\12.0\\Setup\\VC\\ProductDir')], 
   '11.0': [
          (
           SCons.Util.HKEY_LOCAL_MACHINE, 'Microsoft\\VisualStudio\\11.0\\Setup\\VC\\ProductDir')], 
   '11.0Exp': [
             (
              SCons.Util.HKEY_LOCAL_MACHINE, 'Microsoft\\VCExpress\\11.0\\Setup\\VC\\ProductDir')], 
   '10.0': [
          (
           SCons.Util.HKEY_LOCAL_MACHINE, 'Microsoft\\VisualStudio\\10.0\\Setup\\VC\\ProductDir')], 
   '10.0Exp': [
             (
              SCons.Util.HKEY_LOCAL_MACHINE, 'Microsoft\\VCExpress\\10.0\\Setup\\VC\\ProductDir')], 
   '9.0': [
         (
          SCons.Util.HKEY_CURRENT_USER, 'Microsoft\\DevDiv\\VCForPython\\9.0\\installdir'),
         (
          SCons.Util.HKEY_LOCAL_MACHINE, 'Microsoft\\VisualStudio\\9.0\\Setup\\VC\\ProductDir')], 
   '9.0Exp': [
            (
             SCons.Util.HKEY_LOCAL_MACHINE, 'Microsoft\\VCExpress\\9.0\\Setup\\VC\\ProductDir')], 
   '8.0': [
         (
          SCons.Util.HKEY_LOCAL_MACHINE, 'Microsoft\\VisualStudio\\8.0\\Setup\\VC\\ProductDir')], 
   '8.0Exp': [
            (
             SCons.Util.HKEY_LOCAL_MACHINE, 'Microsoft\\VCExpress\\8.0\\Setup\\VC\\ProductDir')], 
   '7.1': [
         (
          SCons.Util.HKEY_LOCAL_MACHINE, 'Microsoft\\VisualStudio\\7.1\\Setup\\VC\\ProductDir')], 
   '7.0': [
         (
          SCons.Util.HKEY_LOCAL_MACHINE, 'Microsoft\\VisualStudio\\7.0\\Setup\\VC\\ProductDir')], 
   '6.0': [
         (
          SCons.Util.HKEY_LOCAL_MACHINE, 'Microsoft\\VisualStudio\\6.0\\Setup\\Microsoft Visual C++\\ProductDir')]}
...
def get_default_version(env):
    ...
    if msvs_version and not msvc_version:
        ...
        if msvc_version and msvs_version:
            if not msvc_version == msvs_version:
                SCons.Warnings.warn(SCons.Warnings.VisualVersionMismatch, 'Requested msvc version (%s) and msvs version (%s) do not match: please use MSVC_VERSION only to request a visual studio version, MSVS_VERSION is deprecated' % (
