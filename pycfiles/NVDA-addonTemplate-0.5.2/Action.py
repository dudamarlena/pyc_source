...
# [GCC 8.4.0]
# Embedded file name: C:\Users\derek_2\Google Drive\nvda-addon-exploded\notepad++\scons-local-2.5.0\SCons\Action.py
...
import dis, os, pickle, re, sys, subprocess, SCons.Debug
from SCons.Debug import logInstanceCreation
import SCons.Errors, SCons.Util, SCons.Subst
...
class ActionBase(object):
    ...
    def get_contents(self, target, source, env):
        result = [
         self.get_presig(target, source, env)]
...
class CommandGeneratorAction(ActionBase):
    ...
    def __call__(self, target, source, env, exitstatfunc=_null, presub=_null, show=_null, execute=_null, chdir=_null, executor=None):
        ...
        if act is None:
            raise SCons.Errors.UserError("While building `%s': Cannot deduce file extension from source files: %s" % (
