...
# [GCC 8.4.0]
# Embedded file name: C:\Users\derek_2\Google Drive\nvda-addon-exploded\notepad++\scons-local-2.5.0\SCons\Taskmaster.py
...
import operator, sys, traceback, SCons.Errors, SCons.Node, SCons.Warnings
...
class Task(object):
    ...
    *should* be able to do what it wants by sub-classing this
    ...
    def prepare(self):
        ...
        T = self.tm.trace
        if T:
            T.write(self.trace_message('Task.prepare()', self.node))
    ...
    def execute(self):
        ...
        T = self.tm.trace
        if T:
            T.write(self.trace_message('Task.execute()', self.node))
        try:
            cached_targets = []
    ...
    def make_ready_current(self):
        ...
        T = self.tm.trace
        if T:
            T.write(self.trace_message('Task.make_ready_current()', self.node))
        self.out_of_date = []
    ...
    def postprocess(self):
        ...
        T = self.tm.trace
        if T:
            T.write(self.trace_message('Task.postprocess()', self.node))
        ...
        for t in targets:
            if t.waiting_parents:
                if T:
                    T.write(self.trace_message('Task.postprocess()', t, 'removing'))
        ...
        for p, subtract in parents.items():
            ...
            if T:
                T.write(self.trace_message('Task.postprocess()', p, 'adjusted parent ref count'))
            if p.ref_count == 0:
                self.tm.candidates.append(p)
    ...
    def _find_next_ready_node(self):
        ...
        while 1:
            if True:
                ...
                if node is None:
                    if T:
                        T.write(self.trace_message('No candidate anymore.') + '\n')
                    return
                node = node.disambiguate()
                state = node.get_state()
                if CollectStats:
                    node.attributes.stats = hasattr(node.attributes, 'stats') or Stats()
                ...
                S = None
            if T:
                T.write(self.trace_message('    Considering node %s and its children:' % self.trace_node(node)))
            if state == NODE_NO_STATE:
                ...
                if state > NODE_PENDING:
                    ...
                    if T:
                        T.write(self.trace_message('       already handled (executed)'))
                    continue
                ...
                for child in chain(executor.get_all_prerequisites(), children):
                    ...
                    if T:
                        T.write(self.trace_message('       ' + self.trace_node(child)))
                ...
                if children_not_ready:
                    for child in children_not_ready:
                        ...
                        if T:
                            T.write(self.trace_message('     adjusted ref count: %s, child %s' % (
            ...
            if S:
                S.build = S.build + 1
            if T:
                T.write(self.trace_message('Evaluating %s\n' % self.trace_node(node)))
            return node
