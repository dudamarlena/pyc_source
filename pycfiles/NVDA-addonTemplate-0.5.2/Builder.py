...
# [GCC 8.4.0]
# Embedded file name: C:\Users\derek_2\Google Drive\nvda-addon-exploded\notepad++\scons-local-2.5.0\SCons\Builder.py
...
import collections, SCons.Action, SCons.Debug
from SCons.Debug import logInstanceCreation
from SCons.Errors import InternalError, UserError
import SCons.Executor, SCons.Memoize, SCons.Util, SCons.Warnings
...
class DictCmdGenerator(SCons.Util.Selector):
    ...
    def __call__(self, target, source, env, for_signature):
        if not source:
            ...
            if not ext:
                raise UserError("While building `%s': Cannot deduce file extension from source files: %s" % (
...
misleading_keywords = {'targets': 'target', 
   'sources': 'source'}
