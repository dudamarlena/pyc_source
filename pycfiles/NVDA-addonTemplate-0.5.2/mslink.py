...
# [GCC 8.4.0]
# Embedded file name: C:\Users\derek_2\Google Drive\nvda-addon-exploded\notepad++\scons-local-2.5.0\SCons\Tool\mslink.py
...
"""
__revision__ = 'src/engine/SCons/Tool/mslink.py rel_2.5.0:3543:937e55cd78f7 2016/04/09 11:29:54 bdbaddog'
import os.path, SCons.Action, SCons.Defaults, SCons.Errors, SCons.Platform.win32, SCons.Tool, SCons.Tool.msvc, SCons.Tool.msvs, SCons.Util
from MSCommon import msvc_setup_env_once, msvc_exists

def pdbGenerator(env, target, source, for_signature):
    try:
        return [
         '/PDB:%s' % target[0].attributes.pdb, '/DEBUG']
...
def prog_emitter(target, source, env):
    ...
    if version_num >= 11.0 and env.get('PCH', 0):
        ...
        if pchobj not in extrasources:
            extrasources.append(pchobj)
    return (
     target + extratargets, source + extrasources)
