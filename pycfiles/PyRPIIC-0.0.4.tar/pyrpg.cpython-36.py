...
# Embedded file name: build\bdist.win32\egg\pyrpg.py
# Compiled at: 2017-12-28 21:28:38
# Size of source mod 2**32: 7226 bytes
__doc__ = 'The PyRPG Lib, Written By Nirudium'
__version__ = '1.12r'
import random
...
class Enemy:
    ...
    def __init__(self, **kwargs):
        ...
        self.e_data = 'Level {} {} [HEALTH: {} | ATK: {} | XP: {}] ENCOUNTERED'.format(self.level, self.name, self.health, self.attack, self.xp)

    def __repr__(self):
        return '<Enemy(name={}, health={}, attack={},, level={}) xp={}>'.format(self.name, self.health, self.attack, self.level, self.xp)

    def __str__(self):
        return 'Level {} {} [HEALTH: {} | ATK: {} | XP: {}] ENCOUNTERED'.format(self.level, self.name, self.health, self.attack, self.xp)

    def foe_dec(self, name, level, xp, gold):
        self.name = name
        self.level = level
        self.xp = xp
        self.gold = gold
...
class Player:
    player_name_opts = ('Makar', 'Joker', 'Gumball The SJW Destroyer', 'Inigo The Brave',
                        'Noor The Thot/Furry/SJW Hunter', 'Keemstar The Thot Obliterator',
                        'Hito')
    ...
    def __repr__(self):
        return '<player(name={}, health={}, attack={}, level={})>'.format(self.name, self.health, self.attack, self.level)

    def __str__(self):
        return 'Level {} {} [HEALTH: {} | ATK: {} | Level = {}]'.format(self.level, self.name, self.health, self.attack)
...
class Battle:
    ...
    def __init__(self, **kwargs):
        ...
        self.spell_list = [{'heal':self.magic_damage / 2,  'cost':2}, {'fireball':self.magic_damage,  'cost':4}, {'freeze':self.magic_damage * 2,  'cost':6}]
        self.mana = p.level * 2
        self.name = kwargs.get('name')
        self.gold = kwargs.get('gold')
        self.xp = kwargs.get('xp')
        self.e = Enemy(level=(self.foelvl), name=(self.name), xp=(self.xp), gold=(self.gold)) or random.choice(self.e.namelist)
        p.mana = self.mana
    ...
    def spell_choice(self):
        if self.mana <= 0:
            ...
            choice = input('Type 1 to heal yourself, 2 to throw a fireball at your foe, or type 3 in order to freeze them: ')
            if choice == '1':
                p.health = p.health + self.spell_list[0]['heal']
...
def genfoe(level, **kwargs):
    ...
    health = e.health
    return (
     name, lvl, hp, gold, xp, attack, health)
