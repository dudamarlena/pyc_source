...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/spotsh/packagebuilder.py
# Compiled at: 2011-05-02 12:54:57
import os, re, string, tempfile, tarfile, subprocess, shutil, sys
...
def build_package(name, path):
    cfg = PackageConfig(name, [
     ExistingImageDisk(DISK_TYPE_HD, path)], DISK_TYPE_HD, True, True, True, True, True)
...
def gzip_file_(file_path):
    ...
    try:
        proc = subprocess.Popen('nice gzip --fast -c %s > %s ' % (file_path, gz_path), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
