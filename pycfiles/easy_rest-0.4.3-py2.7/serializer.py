...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/easyapi/serializer.py
# Compiled at: 2014-08-29 22:18:46
import inspect
...
class EmbeddedObjectsField(Field):
    ...
    def __init__(self, model, embedded_def_dict=None):
        ...
        self.related_fields = [ field for field in self.opts.fields if field.serialize and field.rel ]
        ...
        self.possible_embedded_names = set(self.related_fields_names + self.reverse_rel_names + self.embedded_function_names() + self.embedded_property_names())
    ...
    def field_to_native(self, obj, the_field_name):
        ...
        for model_field in self.related_fields:
            ...
            if field_name in embedded_def_dict:
                result[field_name] = serialize_model(related_model, getattr(obj, field_name), embedded_def_dict[field_name])
        ...
        for relation in self.reverse_rels:
            ...
            if to_many:
                result[relation_name] = serialize_queryset(related_model, getattr(obj, relation_name).all(), embedded_def_dict[relation_name])
            else:
                try:
                ...
                result[relation_name] = serialize_model(related_model, instance, embedded_def_dict[relation_name])

        for method_name, method in self.embedded_functions():
            if method_name and method_name in embedded_def_dict:
                embedded_result = method(obj)
                result[method_name] = convert_result(embedded_result, embedded_def_dict[method_name], getattr(method, 'rest_data_type', None), getattr(method, 'rest_many', False))

        for prop_name, descriptor in self.embedded_properties():
            if prop_name and prop_name in embedded_def_dict:
                embedded_result = getattr(obj, prop_name)
                result[prop_name] = convert_result(embedded_result, embedded_def_dict[prop_name], getattr(descriptor, 'rest_data_type', None), getattr(descriptor, 'rest_many', False))
    ...
    def embedded_functions(self):
        return [ (method.rest_embeddable_function, method) for method_name, method in class_functions_with_attr(self.model, 'rest_embeddable_function')
               ]
    ...
    def embedded_properties(self):
        return [ (getattr(descriptor, 'rest_embeddable_property') or descriptor.fget.__name__, descriptor) for prop_name, descriptor in class_properties_with_attr(self.model, 'rest_embeddable_property')
               ]
...
class AutoModelSerializer(ModelSerializer):
    ...
    def __init__(self, instance=None, data=None, files=None, context=None, partial=False, many=None, allow_add_remove=False, embedded_def_dict=None, **kwargs):
        self.embedded_def_dict = embedded_def_dict
        super(AutoModelSerializer, self).__init__(instance, data, files, context, partial, many, allow_add_remove, **kwargs)
    ...
    def get_default_fields(self):
        ...
        related_fields = [ field for field in opts.fields if field.serialize and field.rel ]
        ...
        for p_name, p_val in class_properties_with_attr(cls, 'field_class'):
            field_name = getattr(p_val, 'name', p_name) or p_name
