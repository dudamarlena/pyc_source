...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/easyapi/filters.py
# Compiled at: 2014-07-15 17:49:41
...
class QuerySetFilter(object):
    ...
    def filter_params(self):
        return {key[1:]:value for key, value in self.all_params.iteritems() if key.startswith('@') if key.startswith('@')}

    def filtered_queryset(self):
        queryset = self.queryset
        for filter_key, value in self.filter_params.iteritems():
            queryset = queryset.filter(**{filter_key: self.get_converter(filter_key)(value)})
    ...
    def get_filter(self, filter_key):
        ...
        if len(components) == 1:
            return Filter(key=filter_key, base_converter=self.get_main_field_converter(main_field_name), field_path=[
             main_field_name], condition=[
             FILTER_EXACT])
        if len(components) == 2 and components[1] in self.get_conditions(main_field_name):
            return Filter(key=filter_key, base_converter=self.get_main_field_converter(main_field_name), field_path=[
             main_field_name], condition=components[1])
        model = self.get_model()
        ...
        for index, component in enumerate(components):
            ...
            if is_last and index > 1 and component in FILTER_ALL:
                return Filter(key=filter_key, base_converter=self.get_field_converter(field), field_path=components[:-1], condition=component)
            field = get_model_field(model, component)
            if is_last:
                return Filter(key=filter_key, base_converter=self.get_field_converter(field), field_path=components, condition=[
                 FILTER_EXACT])
