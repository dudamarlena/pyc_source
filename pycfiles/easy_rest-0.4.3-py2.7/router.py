...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/easyapi/router.py
# Compiled at: 2014-07-31 22:01:57
...
INSTANCE_METHOD_ROUTE = Route(url='^{prefix}/{lookup}/{methodname}{trailing_slash}$', mapping={'{httpmethod}': '{methodname}'}, name='{basename}-{methodnamehyphen}', initkwargs={})
MANAGER_METHOD_ROUTE = Route(url='^{prefix}-manager/{methodname}{trailing_slash}$', mapping={'{httpmethod}': '{methodname}'}, name='{basename}-manager-{methodnamehyphen}', initkwargs={})
...
class EasyApiRouter(DefaultRouter):
    ...
    def compute_routes(self, wrapper, method_name, route):
        ...
        if httpmethods:
            if method_name in self.known_actions():
                raise ImproperlyConfigured('Cannot use @rest_method decorator on method "%s" as it is an existing route' % method_name)
            httpmethods = [ method.lower() for method in httpmethods ]
            dynamic_routes.append((httpmethods, method_name))
        instance_routes = []
        for httpmethods, method_name in dynamic_routes:
            initkwargs = route.initkwargs.copy()
            instance_routes.append(Route(url=replace_methodname(route.url, method_name), mapping=dict((httpmethod, method_name) for httpmethod in httpmethods), name=replace_methodname(route.name, method_name), initkwargs=initkwargs))
    ...
    def get_method_map(self, viewset, method_map):
        ...
        for method, action in method_map.items():
            if hasattr(viewset, action) or is_instance_viewset and action in viewset.instance_rest_methods() or is_instance_viewset and action in viewset.manager_rest_methods():
