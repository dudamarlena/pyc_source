...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/easyapi/tests/test_instance_method_property.py
...
def test_instance_method_list_qs_embed(staff_api_client, function):
    ...
    response = staff_api_client.get('/api/company/%d/%s/' % (company.pk, function), data={'_embedded': 'company'})
    response_data = json.loads(response.content)
    assert len(response_data) == 3
    for project_dict in response_data:
        check_project_dict(project_dict)
