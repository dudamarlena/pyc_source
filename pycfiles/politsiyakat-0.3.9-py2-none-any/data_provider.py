...
# [GCC 8.4.0]
# Embedded file name: /home/bhugo/workspace/politsiyakat/politsiyakat/data/data_provider.py
# Compiled at: 2018-04-13 09:47:35
from pyrap.tables import table
import politsiyakat, os
from politsiyakat.data.misc import *
stokes_types = [
 'Undef', 'I', 'Q', 'U', 'V', 'RR', 'RL', 'LR', 'LL', 'XX', 'XY', 'YX', 'YY']
...
class data_provider():
    ...
    def check_ms(cls, **kwargs):
        ...
        try:
            ...
            raise ValueError("check_ms (or any task that calls it) expects a measurement set (key 'msname') as input")

        try:
            nrows_to_read = int(kwargs['nrows_chunk'])
        except:
            raise ValueError("Task check_ms expects num rows per chunk (key 'nrows_chunk')")
        ...
        with table(ms, readonly=True, ack=False) as (t):
            ms_meta['nchunk'] = int(np.ceil(t.nrows() / float(nrows_to_read)))
            flag_shape = t.getcell('FLAG', 0).shape
            ms_meta['nrows'] = t.nrows()
        if len(flag_shape) != 2:
            raise RuntimeError('%s does not support storing spectral flags. Maybe run pyxis ms.prep?' % ms)
        ...
        assert np.alltrue([ spw_nchans[0] == spw_nchans[c] for c in xrange(ms_meta['nspw'])
        ...
        ms_meta['no_baselines'] = ms_meta['nant'] * (ms_meta['nant'] - 1) // 2 + ms_meta['nant']
        with table(ms + '::POLARIZATION', readonly=True, ack=False) as (t):
            ncorr = t.getcol('NUM_CORR')
        assert np.alltrue([ ncorr[0] == ncorr[c] for c in xrange(len(ncorr)) ]), 'for now we can only handle rows that all have the same number correlations'
    ...
    def data(self):
        ...
        if self.__maintable_chunk is not None:
            return {'a1': self.__maintable_chunk['a1'][:self.__rows_read], 'a2': self.__maintable_chunk['a2'][:self.__rows_read], 
    ...
    def flush_flags(self):
        ...
        with table(self.__ms, readonly=False, ack=False) as (t):
            rows_to_write = min(t.nrows() - (self.__ichunk - 1) * self.__nrows_chunk, self.__nrows_chunk)
            t.putcol('FLAG', self.__maintable_chunk['flag'][:rows_to_write], (self.__ichunk - 1) * self.__nrows_chunk, rows_to_write)

    def __len__(self):
        return self.__nchunk

    def __iter__(self):
        if self.__ichunk != self.__nchunk - 1 or len([ v for v in self.__old_read_exclude if v not in self.__read_exclude ]) != 0:
            self.__nxchunk_wkr = None
            self.__dbchunk_wkr = None
        self.__ichunk = 0
        return self
    ...
    def next(self):
        if self.__ichunk < self.__nchunk:
            if self.__nxchunk_wkr is None or self.__dbchunk_wkr is None:
                self.__nxchunk_wkr = politsiyakat.pool.submit_io(politsiyakat.data.data_provider._read_buffer, self.__ms, self.__read_exclude, self.__maintable_chunk, self.__nrows_chunk, self.__ichunk, self.__ms_meta, self.__data_column)
                self.__rows_read = self.__nxchunk_wkr.result()
                self.__old_read_exclude = self.__read_exclude
                self.__dbchunk_wkr = politsiyakat.pool.submit_io(politsiyakat.data.data_provider._read_buffer, self.__ms, self.__read_exclude, self.__double_buffer, self.__nrows_chunk, (self.__ichunk + 1) % self.__nchunk, self.__ms_meta, self.__data_column)
            else:
...
def _read_buffer(ms, read_exclude, buffer, nrows_chunk, ichunk, ms_meta, data_column):
    """ Reads next buffer """
    map_descriptor_to_spw = ms_meta['map_descriptor_to_spw']
    with table(ms, readonly=True, ack=False) as (t):
        nrows_to_read = min(t.nrows() - ichunk * nrows_chunk, nrows_chunk)
        if 'a1' not in read_exclude:
            t.getcolnp('ANTENNA1', buffer['a1'][0:nrows_to_read], ichunk * nrows_chunk, nrows_to_read)
        if 'a2' not in read_exclude:
            t.getcolnp('ANTENNA2', buffer['a2'][0:nrows_to_read], ichunk * nrows_chunk, nrows_to_read)
        if 'a1' not in read_exclude and 'a2' not in read_exclude:
            buffer['baseline'][:nrows_to_read] = baseline_index(buffer['a1'], buffer['a2'], ms_meta['nant'])[:nrows_to_read]
        if 'field' not in read_exclude:
            t.getcolnp('FIELD_ID', buffer['field'][0:nrows_to_read], ichunk * nrows_chunk, nrows_to_read)
        if 'data' not in read_exclude:
            t.getcolnp(data_column, buffer['data'][0:nrows_to_read, :, :], ichunk * nrows_chunk, nrows_to_read)
        if 'flag' not in read_exclude:
            t.getcolnp('FLAG', buffer['flag'][0:nrows_to_read, :, :], ichunk * nrows_chunk, nrows_to_read)
        if 'desc' not in read_exclude:
            t.getcolnp('DATA_DESC_ID', buffer['desc'][0:nrows_to_read], ichunk * nrows_chunk, nrows_to_read)
        if 'spw' not in read_exclude and 'desc' not in read_exclude:
            buffer['spw'][:nrows_to_read] = map_descriptor_to_spw[buffer['desc']][:nrows_to_read]
        if 'scan' not in read_exclude:
            t.getcolnp('SCAN_NUMBER', buffer['scan'][0:nrows_to_read], ichunk * nrows_chunk, nrows_to_read)
        if 'time' not in read_exclude:
            t.getcolnp('TIME', buffer['time'][0:nrows_to_read], ichunk * nrows_chunk, nrows_to_read)
