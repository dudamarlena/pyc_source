...
# [GCC 8.4.0]
# Embedded file name: /home/bhugo/workspace/politsiyakat/politsiyakat/modules/flag_tasks.py
# Compiled at: 2018-04-13 11:40:20
import os, politsiyakat, re
...
class flag_tasks():
    ...
    def flag_phase_drifts(cls, **kwargs):
        ...
        try:
            if not re.match('^[0-9]+(?:,[0-9]+)*$', kwargs['field']):
                raise ValueError('Expect list of field identifiers')
            fields = [ int(f) for f in kwargs['field'].split(',') ]
        except:
            raise ValueError("flag_phase_drifts expects field(s) (key 'field') as input")

        try:
            if not re.match('^[0-9]+(?:,[0-9]+)*$', kwargs['cal_field']):
                raise ValueError('Expect list of field identifiers')
            cal_fields = [ int(f) for f in kwargs['cal_field'].split(',') ]
        except:
            raise ValueError("flag_phase_drifts expects calibrator field(s) (key 'cal_field') as input")

        try:
            nrows_to_read = int(kwargs['nrows_chunk'])
        except:
            raise ValueError("flag_phase_drifts expects number of rows to read per chunk (key 'nrows_chunk') as input")

        try:
            simulate = bool(kwargs['simulate'])
            if simulate:
                politsiyakat.log.warn('Warning: you specified you want to simulate a flagging run. This means I will compute statistics for you and dump some diagnostics but not actually touch your data.')
        except:
            raise ValueError("flag_phase_drifts expects simulate flag (key 'simulate') as input")

        try:
            output_dir = str(kwargs['output_dir'])
        except:
            raise ValueError("flag_phase_drifts expects an output_directory (key 'output_dir') as input")

        try:
            s2s_sigma = float(kwargs['scan_to_scan_threshold'])
        except:
            raise ValueError("flag_phase_drifts expect a scan to scan variation threshold (sigma) (key 'scan_to_scan_threshold') as input")

        try:
            b2g_sigma = float(kwargs['baseline_to_group_threshold'])
        except:
            raise ValueError("flag_phase_drifts expect a sigma threshold for antenna variation from group(key 'baseline_to_group_threshold') as input")

        try:
            dpi = int(kwargs['plot_dpi'])
        except:
            dpi = 300
            politsiyakat.log.warn("Warning: defaulting to plot dpi of 300. Keyword 'plot_dpi' can be used to control this behaviour.")

        try:
            plt_size = float(kwargs['plot_size'])
        except:
            plt_size = 6
            politsiyakat.log.warn("Warning: defaulting to plot size of 6 units. Keyword 'plot_size' can be used to control this behaviour.")
        ...
        for fi, f in enumerate(fields):
            politsiyakat.log.info('\t(%d): %s' % (f, source_names[fi]) + (' (calibrator)' if f in cal_fields else ''))

        source_scan_info = {}
        obs_start = None
        with data_provider(msname=ms, data_column=DATA, nrows_chunk=nrows_to_read) as (dp):
            for chunk_i, data in enumerate(iter(dp)):
                ...
                obs_start = min(obs_start, np.min(data['time'])) if obs_start is not None else np.min(data['time'])
                politsiyakat.log.info('\tProcessing field:')
                for field_i, field_id in enumerate(fields):
                    if field_id not in source_scan_info:
                        source_scan_info[field_id] = {'is_calibrator': field_id in cal_fields, 'scan_list': []}
                    ...
                    for s in source_scans:
                        ...
                        if s not in source_scan_info[field_id]:
                            source_scan_info[field_id][s] = {'scan_start': scan_start, 'scan_end': scan_end, 
                            ...
                            source_scan_info[field_id][s]['scan_start'] = min(source_scan_info[field_id][s]['scan_start'], scan_start)
                            source_scan_info[field_id][s]['scan_end'] = max(source_scan_info[field_id][s]['scan_end'], scan_end)
                        for spw in xrange(nspw):
                            epic_name = 'antenna stats for %s' % source_names[field_i]
                            for a in xrange(nant):
                                politsiyakat.pool.submit_to_epic(epic_name, _wk_per_ant_flag_stats, a, ncorr, nchan, spw, field_id, s, data)

                            for bl in xrange(no_baselines):
                                politsiyakat.pool.submit_to_epic(epic_name, _wk_per_bl_phase_stats, bl, ncorr, nchan, spw, field_id, s, data)

                            res = politsiyakat.pool.collect_epic(epic_name)
                            for r in res:
                                if r[0] == 'antstat':
                                    task, ant, spw, totflagged, totrowcount = r
                                    source_scan_info[field_id][s]['tot_flagged'][ant, spw * nchan:(spw + 1) * nchan, :] += totflagged
                                    source_scan_info[field_id][s]['tot_rowcount'][ant, spw * nchan:(spw + 1) * nchan, :] += totrowcount
                                elif r[0] == 'blstat':
                                    task, bl, spw, blchisq, sph, cph = r
                                    source_scan_info[field_id][s]['chisq_phase'][bl, spw * nchan:(spw + 1) * nchan, :] += blchisq
                                    source_scan_info[field_id][s]['sum_phase'][bl, spw * nchan:(spw + 1) * nchan, :] += sph
                                    source_scan_info[field_id][s]['count_phase'][bl, spw * nchan:(spw + 1) * nchan, :] += cph
            ...
            for field_i, field_id in enumerate(fields):
                ...
                for s in source_scan_info[field_id]['scan_list']:
                    ...
                    politsiyakat.log.info('\t\tScan %d (duration: %0.2f seconds) is %.2f %% flagged' % (
                     s,
                     source_scan_info[field_id][s]['scan_end'] - source_scan_info[field_id][s]['scan_start'],
                     flagged / count * 100.0))
            ...
            for field_i, field_id in enumerate(fields):
                for s in source_scan_info[field_id]['scan_list']:
                    ant_flagged += np.sum(np.sum(source_scan_info[field_id][s]['tot_flagged'], axis=1), axis=1)
                    ant_count += np.sum(np.sum(source_scan_info[field_id][s]['tot_rowcount'], axis=1), axis=1)
            ...
            for field_i, field_id in enumerate(fields):
                ...
                flgs_field_intrabl = np.zeros([len(source_scan_info[field_id]['scan_list']),
                 no_baselines, nchan * nspw, ncorr], dtype=np.bool)
                ...
                for si, s in enumerate(source_scan_info[field_id]['scan_list']):
                    flgs_field_intrabl[si, :, :, :] = np.abs(chisq_stacked[si] - mean_intrabl_phase) > s2s_sigma * std_intrabl_phase

                median_array_phase = np.nanmean(mean_intrabl_phase, axis=0)
                std_array_phase = np.nanstd(mean_intrabl_phase, axis=0)
                for si, s in enumerate(source_scan_info[field_id]['scan_list']):
                    flgs_field_interbl[si, :, :, :] = np.abs(chisq_stacked[si] - median_array_phase) > s2s_sigma * std_array_phase
            ...
            for field_i, field_id in enumerate(fields):
                ...
                for si, s in enumerate(source_scan_info[field_id]['scan_list']):
                    flg = np.nansum(np.nansum(flg_intra_bl[field_id][si], axis=0), axis=0)
            ...
            for field_i, field_id in enumerate(fields):
                ...
                politsiyakat.log.info('\tField %s:' % source_names[field_i])
                for si, s in enumerate(source_scan_info[field_id]['scan_list']):
                    flg = np.nansum(np.nansum(flg_inter_bl[field_id][si], axis=0), axis=0)
                    cnt = float(no_baselines * nchan)
                    politsiyakat.log.info('\t\tScan %s is [%s] %% flagged' % (
                     s, (',').join([ str(v) for v in flg / cnt * 100.0 ])))
            ...
            for sc in apply_grps.keys():
                for field_i, field_id in enumerate(fields):
                    if sc in source_scan_info[field_id]['scan_list']:
                        ...
                        calflgs = np.logical_or(flg_intra_bl[field_id][sc_i], flg_inter_bl[field_id][sc_i])
            ...
            for chunk_i, data in enumerate(iter(dp)):
                ...
                for field_i, field_id in enumerate(source_scan_info.keys()):
                    politsiyakat.log.info('\tSelecting field %s...' % source_names[field_i])
                    has_updated = False
                    for si, s in enumerate(source_scan_info[field_id]['scan_list']):
                        ...
                        for bl in xrange(no_baselines):
                            politsiyakat.pool.submit_to_epic(epic_name, _wk_per_bl_update, bl, field_id, s, si, data, flgs, nspw, nchan)
            ...
            uv_dist = uv_dist_per_baseline(no_baselines, nant, antenna_positions)
            ...
            for field_i, field_id in enumerate(fields):
                ...
                f, axarr = plt.subplots(nxplts, nyplts, dpi=dpi, figsize=(nxplts * plt_size, nyplts * plt_size))
                if not isinstance(axarr, np.ndarray):
                    arplt = np.empty((1, 1), dtype=object)
                    arplt[(0, 0)] = axarr
                    axarr = arplt
                axarr = axarr.reshape([nyplts, nxplts])
                for si, s in enumerate(source_scan_info[field_id]['scan_list']):
                    politsiyakat.log.info('\t\tScan %s...' % s)
                    for c in xrange(ncorr):
                        axarr[(si // nxplts, si % nxplts)].plot(uv_dist[ranked_uv_dist], np.sum(new_flags[field_id][s][ranked_uv_dist, :, c], axis=1), label=ms_meta['ms_feed_names'][c])
            ...
            for field_i, field_id in enumerate(fields):
                for s in source_scan_info[field_id]['scan_list']:
                    obs_end = max(obs_end, source_scan_info[field_id][s]['scan_end'])
                    obs_start = min(obs_start, source_scan_info[field_id][s]['scan_start'])

            heatmaps = np.zeros([len(fields), no_baselines, ncorr, 128, nchan * nspw])
            fph = {}
            for field_i, field_id in enumerate(fields):
                sh = [
                 len(source_scan_info[field_id]['scan_list']), no_baselines, nchan * nspw, ncorr]
                if np.prod(np.array(sh)) == 0:
                    continue
                fph[field_i] = np.zeros(sh)
                scan_mid = np.zeros([len(source_scan_info[field_id]['scan_list'])])
                for si, s in enumerate(source_scan_info[field_id]['scan_list']):
                    fph[field_i][si, :, :, :] = np.divide(source_scan_info[field_id][s]['sum_phase'], source_scan_info[field_id][s]['count_phase'])
                    scan_mid[si] = np.array([
                     0.5 * (source_scan_info[field_id][s]['scan_end'] - source_scan_info[field_id][s]['scan_start']) - obs_start])

                for bl in xrange(no_baselines):
                    politsiyakat.pool.submit_to_epic('waterfall plots', _wkr_bl_corr_regrid, source_names[field_i], field_i, ncorr, nchan, nspw, obs_end, obs_start, scan_mid, fph, antnames, heatmaps, bl)
                ...
                for corr in xrange(ncorr):
                    ...
                    f, axarr = plt.subplots(nyplts, nxplts, dpi=dpi, figsize=(
                     nyplts * plt_size, nxplts * plt_size), sharex=True, sharey=True)
                    scale_min = np.nanmin(heatmaps[field_i, :, corr, :, :])
                    scale_max = np.nanmax(heatmaps[field_i, :, corr, :, :])
                    for bl in xrange(no_baselines):
                        z = heatmaps[field_i, bl, corr, :, :]
                        im = axarr[(bl // nxplts, bl % nxplts)].imshow(z, aspect='auto', extent=[
                         0, nchan * nspw,
                         (obs_end - obs_start) / 3600, 0], vmin=scale_min, vmax=scale_max, cmap='gist_rainbow')
                    ...
                    f.text(0.5, 0.94, 'FIELD: %s, CORR: %s' % (source_names[field_i], ms_meta['ms_feed_names'][corr]), ha='center')
    ...
    def flag_autocorr_drifts(cls, **kwargs):
        ...
        try:
            ...
            raise ValueError("flag_autocorr_drifts expects field(s) (key 'field') as input")
        ...
        try:
            ...
            raise ValueError("flag_autocorr_drifts expects calibrator field(s) (key 'cal_field') as input")

        try:
            nrows_to_read = int(kwargs['nrows_chunk'])
        except:
            raise ValueError("flag_autocorr_drifts expects number of rows to read per chunk (key 'nrows_chunk') as input")
        ...
        try:
            ...
            raise ValueError("flag_autocorr_drifts expects simulate flag (key 'simulate') as input")

        try:
            output_dir = str(kwargs['output_dir'])
        except:
            raise ValueError("flag_autocorr_drifts expects an output_directory (key 'output_dir') as input")

        try:
            s2s_sigma = float(kwargs['scan_to_scan_threshold'])
        except:
            raise ValueError("flag_autocorr_drifts expect a scan to scan variation threshold (sigma) (key 'scan_to_scan_threshold') as input")

        try:
            a2g_sigma = float(kwargs['antenna_to_group_threshold'])
        except:
            raise ValueError("flag_autocorr_drifts expect a sigma threshold for antenna variation from group(key 'antenna_to_group_threshold') as input")
        ...
        with data_provider(msname=ms, data_column=DATA, nrows_chunk=nrows_to_read) as (dp):
                ...
                for field_i, field_id in enumerate(fields):
                    ...
                    for s in source_scans:
                        ...
                        for spw in xrange(nspw):
                            epic_name = 'antenna stats for %s' % source_names[field_i]
                            for a in xrange(nant):
                                politsiyakat.pool.submit_to_epic(epic_name, _wk_per_ant_stats, a, ncorr, nchan, spw, field_id, s, data)

                            res = politsiyakat.pool.collect_epic(epic_name)
                            for r in res:
                                if r[0] == 'antstat':
                                    task, ant, spw, autopow, autocount, totflagged, totrowcount = r
                                    source_scan_info[field_id][s]['tot_autopower'][ant, spw * nchan:(spw + 1) * nchan, :] += autopow
                                    source_scan_info[field_id][s]['tot_autocount'][ant, spw * nchan:(spw + 1) * nchan, :] += autocount
                                    source_scan_info[field_id][s]['tot_flagged'][ant, spw * nchan:(spw + 1) * nchan, :] += totflagged
                                    source_scan_info[field_id][s]['tot_rowcount'][ant, spw * nchan:(spw + 1) * nchan, :] += totrowcount
            ...
            for field_i, field_id in enumerate(fields):
                ...
                scan_amp_flags = np.zeros([len(source_scan_info[field_id]['scan_list']), nant, nchan * nspw, ncorr], np.bool)
                for si, s in enumerate(source_scan_info[field_id]['scan_list']):
                    amp_scans[si] = np.divide(source_scan_info[field_id][s]['tot_autopower'], source_scan_info[field_id][s]['tot_rowcount'])
                ...
                scan_amp_flags_inter = np.zeros([len(source_scan_info[field_id]['scan_list']), nant, nchan * nspw, ncorr], np.bool)
            ...
            politsiyakat.log.info('Resulting inter-antenna scan flagging based on autocorraltion amplitude:')
            for field_i, field_id in enumerate(fields):
                politsiyakat.log.info('\tField %s:' % source_names[field_i])
                for ant in xrange(nant):
                    politsiyakat.log.info('\t\tAntenna %s:' % antnames[ant])
            ...
            for chunk_i, data in enumerate(iter(dp)):
                ...
                for field_i, field_id in enumerate(source_scan_info.keys()):
                    ...
                    for si, s in enumerate(source_scan_info[field_id]['scan_list']):
                        ...
                        for a in xrange(nant):
                            politsiyakat.pool.submit_to_epic(epic_name, _wk_per_ant_update, a, field_id, s, si, data, field_scan_flags_intra, field_scan_flags_inter, nspw, nchan)
            ...
            for field_i, field_id in enumerate(fields):
                sh = [
                 len(source_scan_info[field_id]['scan_list']), nant, nchan * nspw, ncorr]
                if np.prod(np.array(sh)) == 0:
                    continue
                famp[field_i] = np.zeros(sh)
                scan_mid = np.zeros([len(source_scan_info[field_id]['scan_list'])])
                for si, s in enumerate(source_scan_info[field_id]['scan_list']):
                    famp[field_i][si, :, :, :] = np.divide(source_scan_info[field_id][s]['tot_autopower'], source_scan_info[field_id][s]['tot_rowcount'])
                    scan_mid[si] = 0.5 * (source_scan_info[field_id][s]['scan_end'] - source_scan_info[field_id][s]['scan_start']) - obs_start

                for ant in xrange(nant):
                    politsiyakat.pool.submit_to_epic('waterfall plots', _wkr_ant_corr_regrid, source_names[field_i], field_i, ncorr, nchan, nspw, obs_end, obs_start, scan_mid, famp, antnames, heatmaps, ant)
                ...
                for corr in xrange(ncorr):
                    ...
                    for ant in xrange(nant):
                        ...
                        im = axarr[(ant // nxplts, ant % nxplts)].imshow(dbscale, aspect='auto', extent=[
                         0, nchan * nspw,
                         (obs_end - obs_start) / 3600, 0], vmin=scale_min, vmax=scale_max, cmap='gist_rainbow')
                        plt.colorbar(im, ax=axarr[(ant // nxplts, ant % nxplts)])
                        axarr[(ant // nxplts, ant % nxplts)].set_title(antnames[ant])

                    f.text(0.5, 0.94, 'FIELD: %s, CORR: %s (dB-scale)' % (source_names[field_i], ms_meta['ms_feed_names'][corr]), ha='center')
                    f.text(0.5, 0.04, 'channel', ha='center')
                    f.text(0.04, 0.5, 'Time (hrs)', va='center', rotation='vertical')
...
def _wk_per_ant_update(a, field_id, s, si, data, field_scan_flags_intra, field_scan_flags_inter, nspw, nchan):
    nrow_per_chunk = 100
    nchunks = int(np.ceil(data['data'].shape[0] / float(nrow_per_chunk)))
    for n in xrange(nchunks):
        nrows_to_read = min(data['data'].shape[0] - n * nrow_per_chunk, nrow_per_chunk)
        chunk_start = n * nrow_per_chunk
        chunk_end = n * nrow_per_chunk + nrows_to_read
        scan_sel = data['scan'][chunk_start:chunk_end] == s
        ant_sel = np.logical_or(data['a1'][chunk_start:chunk_end] == a, data['a2'][chunk_start:chunk_end] == a)
        ...
        for spwi in xrange(nspw):
            ...
            newf = np.logical_or(field_scan_flags_intra[field_id][si][a][spwi * nchan:nchan * (spwi + 1), :], field_scan_flags_inter[field_id][si][a][spwi * nchan:nchan * (spwi + 1), :])
            data['flag'][np.argwhere(accum_sel)][chunk_start:chunk_end] = np.logical_or(data['flag'][np.argwhere(accum_sel)][chunk_start:chunk_end], newf)


def _wk_per_bl_update(bl, field_id, s, si, data, flgs, nspw, nchan):
    nrow_per_chunk = 100
    nchunks = int(np.ceil(data['data'].shape[0] / float(nrow_per_chunk)))
    for n in xrange(nchunks):
        nrows_to_read = min(data['data'].shape[0] - n * nrow_per_chunk, nrow_per_chunk)
        ...
        for spwi in xrange(nspw):
            spwsel = data['spw'][chunk_start:chunk_end] == spwi
            accum_sel = np.logical_and(accum_sel, spwsel)
            data['flag'][np.argwhere(accum_sel)][chunk_start:chunk_end] = np.logical_or(data['flag'][np.argwhere(accum_sel)][chunk_start:chunk_end], flgs[bl][spwi * nchan:nchan * (spwi + 1), :])


def _wk_per_ant_flag_stats(a, ncorr, nchan, spw, field_id, s, data):
    totflagged = None
    totrowcount = None
    nrow_per_chunk = 100
    nchunks = int(np.ceil(data['data'].shape[0] / float(nrow_per_chunk)))
    for n in xrange(nchunks):
        ...
        antsel = np.logical_or(data['a1'][chunk_start:chunk_end] == a, data['a2'][chunk_start:chunk_end] == a)
        sel = np.tile(np.logical_and(accum_sel, antsel), (
         ncorr, nchan, 1)).T
    ...
    return (
     'antstat', a, spw, totflagged, totrowcount)


def _wk_per_bl_phase_stats(bl, ncorr, nchan, spw, field_id, s, data):
    nbins = 120
    ...
    for n in xrange(nchunks):
        ...
        sel = np.logical_and(accum_sel_tiled, np.logical_not(data['flag'][chunk_start:chunk_end]))
    ...
    return (
     'blstat', bl, spw, blchisq, blsum, blcount)


def _wk_per_ant_stats(a, ncorr, nchan, spw, field_id, s, data):
    autopow = None
    autocount = None
    totflagged = None
    totrowcount = None
    ...
    for n in xrange(nchunks):
        ...
        accum_sel = np.logical_and(scan_sel, spw_sel)
        accum_sel_tiled = np.tile(accum_sel, (ncorr, nchan, 1)).T
        autocorrs_sel = np.logical_and(data['a1'][chunk_start:chunk_end] == data['a2'][chunk_start:chunk_end], data['a1'][chunk_start:chunk_end] == a)
        sel = np.logical_and(accum_sel_tiled, np.logical_and(np.logical_not(data['flag'][chunk_start:chunk_end]), np.tile(autocorrs_sel, (
         ncorr, nchan, 1)).T))
        dn = data['data'][chunk_start:chunk_end].copy()
        dn[np.logical_not(sel)] = np.nan
        d = np.abs(dn)
    ...
    return (
     'antstat', a, spw, autopow, autocount, totflagged, totrowcount)


def _wkr_ant_corr_regrid(field_name, field_i, ncorr, nchan, nspw, obs_end, obs_start, scan_mid, famp, antnames, heatmaps, ant):
    for corr in xrange(ncorr):
        X, Y = np.linspace(0.0, float(obs_end - obs_start) / 3600.0, 512), np.arange(nchan * nspw)
        ...
        heatmap = interp.griddata((xcoords,
         ycoords), famp[field_i][:, ant, :, corr].flatten(), (
         X[:, None], Y[None, :]), method='nearest')
        heatmaps[field_i, ant, corr, :, :] = heatmap

    return


def _wkr_bl_corr_regrid(field_name, field_i, ncorr, nchan, nspw, obs_end, obs_start, scan_mid, fph, antnames, heatmaps, bl):
    for corr in xrange(ncorr):
        X, Y = np.linspace(0.0, float(obs_end - obs_start) / 3600.0, 128), np.arange(nchan * nspw)
        xcoords = np.repeat(scan_mid / 3600.0, nchan * nspw)
        ycoords = np.tile(np.arange(nchan * nspw), [
         1, fph[field_i].shape[0]]).flatten()
        heatmap = interp.griddata((xcoords,
         ycoords), fph[field_i][:, bl, :, corr].flatten(), (
         X[:, None], Y[None, :]), method='nearest')
