...
# Compiled at: 2019-02-06 18:29:07
import os, logging
...
class Layer(object):
    ...
    def _set_data_source(self, new_source):
        ...
        if PRO:
            ...
            self.layer_object.updateConnectionProperties(old_connection_properties, {'dataset': desc.name, 
               'connection_info': {'database': desc.path}, 'workspace_factory': new_factory_type})
        else:
    ...
    def symbology(self, symbology):
        ...
        if PRO:
            ...
            elif isinstance(symbology, arcpy._mp.Layer) or isinstance(symbology, Layer):
            ...
            if self.layer_object.symbologyType != source_data.symbologyType:
                log.warning(('Trying to apply symbology with a renderer of type {} to a layer with renderer of type {} - thismay fail in ArcMap').format(source_data.symbologyType, self.layer_object.symbologyType))
            arcpy.mapping.UpdateLayer(data_frame=self.map.map_object, update_layer=self.layer_object, source_layer=source_data, symbology_only=True)
        return
