...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/django_pds/conf.py
...
class LazySettings(LazyObject):
    ...
    def __repr__(self):
        ...
        return '<django_pds.conf.LazySettings "%(settings_module)s">' % {'settings_module': self._wrapped.SETTINGS_MODULE}
...
class SettingsOverride:
    ...
    def __init__(self, _SettingsOverride__settings):
        self._SettingsOverride__settings = _SettingsOverride__settings
        ...
        for setting in dir(mod):
            if setting.isupper():
                ...
                if setting in list_settings:
                    if not isinstance(setting_value, (list, tuple)):
                        raise ImproperlyConfigured('The %s setting must be a list or a tuple. ' % setting)
                setattr(self._SettingsOverride__settings, setting, setting_value)
