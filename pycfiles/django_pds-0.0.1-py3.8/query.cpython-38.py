...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/django_pds/core/pds/parser/query.py
# Compiled at: 2020-05-02 14:49:51
# Size of source mod 2**32: 5904 bytes
import json, re
from mongoengine import Q
import django_pds.core.helper.xstr as xstr
from .terms import WHERE, SELECT, EQUAL, LT, GT, AND, OR, EMPTY, COMMA, FILTER, PARENTHESIS_LEFT, PARENTHESIS_RIGHT, ORDER_BY, RAW_WHERE
REGEX = "\\(|\\)|\\w+=[\\w+|\\-*|\\d*|\\:*|\\.*|\\ *|\\,*|\\'*|\\%*]*|\\&+|\\@+"
REGEX_STRING_QUERY = [
...
class QueryParser:
    ...
    def validate(self):
        if not self._QueryParser__count_lt_gt():
            raise Exception('Your SQL missing some < / >')
        if not self._QueryParser__count_parenthesis():
            raise Exception('Your SQL missing some ( / )')
    ...
    def making_q(self, tree, _filter, q, start, position=None):
        ...
        if isinstance(subtree[0], list):
            ...
            if sub_q and start == 0:
                ...
                if sub_q and tree[(start - 1)] == OR:
                    q = q.__or__(sub_q)
                else:
                    if sub_q and tree[(start - 1)] == AND:
                        q = q.__and__(sub_q)
        else:
            ...
            for replace_with in REPLACE_DICT:
                ...
                self._QueryParser__raw[keys[0].strip()] = value
    ...
    def deconstruct_deep_where(self, clause):
        if not (clause.endswith(')') and clause.startswith('(')):
            clause = '(' + clause + ')'
    ...
    def parse(self):
        ...
        for keyword in splits:
            ...
            if dictionary.get(WHERE, None):
                ...
                dictionary[RAW_WHERE] = self._QueryParser__raw
            return dictionary