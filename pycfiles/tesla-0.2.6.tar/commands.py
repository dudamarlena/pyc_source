...
# [GCC 8.4.0]
# Embedded file name: /Users/danjac/petprojects/tesla/tesla/commands.py
# Compiled at: 2007-08-29 05:58:04
"""
...
class SqlCommand(Command):
    ...
    def command(self):
        try:
            ...
            metadata.engine.echo = self.options.echo
            try:
...
class ModelCommand(Command):
    ...
    def command(self):
        ...
        try:
            ...
            model_name = pylons.util.class_name_from_module_name(name.split('/')[(-1)])
            ...
            file_op.template_vars.update({'name': model_name, 'fname': os.path.join(directory, name), 
               'package': base_package, 
               'test_model_name': test_model_name})
            file_op.copy_file(template='model.py_tmpl', dest=os.path.join('model', directory), filename=name)
            file_op.copy_file(template='form.py_tmpl', dest=os.path.join('model', 'forms', directory), filename=name)
            file_op.ensure_dir(os.path.join(base_package, 'fixtures', name))
            if not self.options.no_test:
                file_op.copy_file(template='test_model.py_tmpl', dest=os.path.join('tests', 'unit'), filename='test_' + testname)
    ...
    def command(self):
        try:
            ...
            if self.options.create:
                ...
                file_op.copy_file(template='script.py_tmpl', dest=os.path.join('scripts', directory), filename=name)
            else:
                try:
