...
# [GCC 8.4.0]
# Embedded file name: /Users/danjac/petprojects/tesla/tests/test_make_project.py
# Compiled at: 2007-08-27 02:55:35
...
template_path = os.path.join(os.path.dirname(__file__), 'filestotest').replace('\\', '/')
test_environ = os.environ.copy()
test_environ['PASTE_TESTING'] = 'true'
testenv = TestFileEnvironment(os.path.join(os.path.dirname(__file__), 'output').replace('\\', '/'), template_path=template_path, environ=test_environ)
projenv = None
auth_projenv = None
authxp_projenv = None
try:
...
def paster_create(project_name, template):
    res = testenv.run(_get_script_name('paster'), 'create', '--verbose', '--no-interactive', '--template=%s' % template, project_name)
    ...
    testenv.run(_get_script_name('python') + ' setup.py egg_info', cwd=os.path.join(testenv.cwd, project_name).replace('\\', '/'), expect_stderr=True)
    projenv = TestFileEnvironment(os.path.join(testenv.base_path, project_name).replace('\\', '/'), start_clear=False, template_path=template_path, environ=test_environ)
    projenv.environ['PYTHONPATH'] = projenv.environ.get('PYTHONPATH', '') + ':' + projenv.base_path
    return projenv
...
def create_tesla_auth_xp_proj():
    global authxp_projenv
    authxp_projenv = paster_create('AuthXpProjectName', 'tesla_auth_xp')
...
def setup_model():
    ...
    _do_proj_test(projenv, 'ProjectName', {os.path.join(projenv_files, '__init__.py_tmpl'): os.path.join('projectname', 'model', '__init__.py'), os.path.join(projenv_files, 'websetup.py_tmpl'): os.path.join('projectname', 'websetup.py'), 
...
def auth_controllers():
    ...
    _do_proj_test(auth_projenv, 'AuthProjectName', {os.path.join(authprojenv_files, 'auth_controller.py_tmpl'): os.path.join('authprojectname', 'controllers', 'auth.py'), os.path.join(authprojenv_files, 'development.ini_tmpl'): 'development.ini', 
...
def authxp_setup():
    ...
    _do_proj_test(authxp_projenv, 'AuthXpProjectName', {os.path.join(authxpprojenv_files, 'news_controller.py_tmpl'): os.path.join('authxpprojectname', 'controllers', 'news.py'), os.path.join(authxpprojenv_files, 'development.ini_tmpl'): 'development.ini', 
...
def _do_proj_test(projenv, project_name, copydict, emptyfiles=None):
    ...
    res = projenv.run(_get_script_name('nosetests') + ' -d', expect_stderr=True, cwd=os.path.join(testenv.cwd, project_name).replace('\\', '/'))
...
def test_project():
    ...
    yield (create_tesla_auth_xp_proj,)
    yield (make_model,)
    yield (setup_model,)
    yield (open_shell,)
    yield (models,)
