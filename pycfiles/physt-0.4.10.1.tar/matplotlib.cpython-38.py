...
# [GCC 8.4.0]
# Embedded file name: /home/honza/code/my/physt/physt/plotting/matplotlib.py
# Compiled at: 2019-11-08 06:23:04
# Size of source mod 2**32: 34191 bytes
...
def bar(h1: Histogram1D, ax: Axes, *, errors: bool=False, **kwargs):
    ...
    (ax.bar)(h1.bin_left_edges, data, h1.bin_widths, align='edge', label=label, 
     color=colors, linewidth=lw, **kwargs)
    if show_values:
...
def scatter(h1: Histogram1D, ax: Axes, *, errors: bool=False, **kwargs):
    ...
    if errors:
        err_data = get_err_data(h1, cumulative=cumulative, density=density)
        ax.errorbar((h1.bin_centers), data, yerr=err_data, fmt=(kwargs.pop('fmt', 'o')), ecolor=(kwargs.pop('ecolor', 'black')),
          ms=0)
...
def line(h1: Union[(Histogram1D, 'HistogramCollection')], ax: Axes, *, errors: bool=False, **kwargs):
    ...
    if errors:
        err_data = get_err_data(h1, cumulative=cumulative, density=density)
        (ax.errorbar)(h1.bin_centers, data, yerr=err_data, fmt=kwargs.pop('fmt', '-'), 
         ecolor=kwargs.pop('ecolor', 'black'), **kwargs)
    else:
...
def map(h2: Histogram2D, ax: Axes, *, show_zero: bool=True, show_values: bool=False, show_colorbar: bool=True, x=None, y=None, **kwargs):
    ...
    for i in range(len(xpos)):
        ...
        if not data[i] != 0:
            if show_zero:
                if not transformed:
                    rect = (plt.Rectangle)([xpos[i], ypos[i]], dx[i], dy[i], facecolor=bin_color, 
                     edgecolor=kwargs.get('grid_color', cmap(0.5)), 
                     lw=kwargs.get('lw', 0.5), 
                     alpha=alpha, **rect_args)
                ...
                if not text_color:
                    if yiq_y > 0.5:
                        ...
                        text_color = (
                         1.0, 1.0, 1.0,
                         kwargs.get('text_alpha', alpha))
                (ax.text)(tx, ty, text, horizontalalignment='center', verticalalignment='center', 
                 color=text_color, clip_on=True, **rect_args)
        if show_colorbar:
...
def image(h2: Histogram2D, ax: Axes, *, show_colorbar: bool=True, interpolation: str='nearest', **kwargs):
    ...
    for binning in h2._binnings:
        ...
        _apply_xy_lims(ax, h2, data=data, kwargs=kwargs)
        _add_labels(ax, h2, kwargs)
        (ax.imshow)(data.T[::-1, :], cmap=cmap, norm=norm, extent=(
 h2.bins[0][(0, 0)], h2.bins[0][(-1, 1)],
...
def polar_map(hist: Histogram2D, ax: Axes, *, show_zero: bool=True, show_colorbar: bool=True, **kwargs):
    ...
    for i in range(len(rpos)):
        if not data[i] > 0:
            if show_zero:
                bin_color = colors[i]
                bars = (ax.bar)(phipos[i], dr[i], width=dphi[i], bottom=rpos[i], align='edge', color=bin_color, edgecolor=kwargs.get('grid_color', cmap(0.5)), 
                 lw=kwargs.get('lw', 0.5), alpha=alphas[i], **bar_args)
...
def globe_map(hist: Union[(Histogram2D, DirectionalHistogram)], ax: Axes3D, *, show_zero: bool=True, **kwargs):
    ...
    for i in range(hist.shape[0]):
        for j in range(hist.shape[1]):
            if not (show_zero or data[(i, j)]):
                pass
            else:
                x = (
                 xs[(i, j)], xs[(i, j + 1)], xs[(i + 1, j + 1)], xs[(i + 1, j)])
...
@register(2, use_3d=True)
def surface_map(hist, ax: Axes3D, *, show_zero: bool=True, x=lambda x, y: x, y=lambda x, y: y, z=lambda x, y: 0, **kwargs):
...
def pair_bars(first: Histogram1D, second: Histogram2D, *, orientation: str='vertical', kind: str='bar', **kwargs) -> Axes:
    ...
    xlim = kwargs.pop('xlim', (
     min(first.bin_left_edges[0], first.bin_left_edges[0]), max(first.bin_right_edges[(-1)], second.bin_right_edges[(-1)])))
    ...
    if ylim != 'keep':
        if isinstance(ylim, tuple):
                ...
                if h.ndim == 1:
                    if data.size > 0:
                        if data.max() > 0:
                            ylim = (
                             0,
                             max(ylim[1], data.max() + (data.max() - ylim[0]) * 0.1))
                    if yscale == 'log':
                        ylim = (
                         abs(data[(data > 0)].min()) * 0.9, ylim[1] * 1.1)
            elif h.ndim == 2 and h.shape[1] >= 2:
                ylim = (
                 h.get_bin_left_edges(1)[0],
            ...
            if invert_y:
                ylim = ylim[::-1]
            ax.set_ylim(ylim)
        if isinstance(xlim, tuple) if xlim != 'keep' else xlim:
            xlim = ax.get_xlim()
            if h.shape[0] >= 1:
                if h.ndim == 1:
                    xlim = (
                     h.bin_left_edges[0], h.bin_right_edges[(-1)])
                else:
                    if h.ndim == 2:
