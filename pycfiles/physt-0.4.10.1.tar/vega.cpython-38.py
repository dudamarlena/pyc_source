...
# [GCC 8.4.0]
# Embedded file name: /home/honza/code/my/physt/physt/plotting/vega.py
# Compiled at: 2019-11-08 06:23:33
# Size of source mod 2**32: 22206 bytes
...
try:
    ...
    """
        ...
        def wrapper(hist, write_to=None, write_format='auto', display='auto', indent=2, **kwargs):
            ...
            if display is True:
                if not VEGA_IPYTHON_PLUGIN_ENABLED:
    ...
    """
        ...
        if not (write_format == 'html' or write_format) == 'auto' or write_to.endswith('.html'):
            output = HTML_TEMPLATE.replace('{{ title }}', title or 'Histogram').replace('{{ spec }}', spec)
        else:
            if not (write_format == 'json' or write_format) == 'auto' or write_to.endswith('.json'):
                output = spec
            else:
    ...
    """
        if VEGA_IPYTHON_PLUGIN_ENABLED:
            if display:
    ...
    """
        ...
        vega['data'] = [
         {'name':'table', 
          'values':[{'x':lefts[i], 
           'x2':rights[i], 
           'y':data[i]} for i in range(h1.bin_count)]}]
        ...
        for i in range(h3.shape[0]):
            for j in range(h3.shape[1]):
                for k in range(h3.shape[2]):
                    if (show_zero or values[i][j][k]) == 0:
                        pass
                    else:
                        item = {'x':float(x[i]), 
                         'x1':float(x1[i]), 
                ...
                vega['signals'] = [
                 {'name':'k', 
                  'value':h3.shape[2] // 2,  'bind':{'input':'range', 
                   'min':0,  'max':h3.shape[2] - 1,  'step':1, 
    ...
    def _create_figure(kwargs: Mapping[(str, Any)]) -> dict:
        ...
        return {'$schema':'https://vega.github.io/schema/vega/v3.json', 
    ...
    def _create_series_scales(vega: dict):
        vega['scales'].append({'name':'series', 
         'type':'ordinal', 
         'range':'category', 
         'domain':{'data':'table', 
          'field':'c'}})
        vega['scales'].append({'name':'labels', 
         'type':'ordinal', 
         'range':'category', 
         'domain':{'data':'labels', 
          'field':'data'}})
    ...
    def _create_axes(hist: HistogramBase, vega: dict, kwargs: dict):
        ...
        vega['axes'] = [
         {'orient':'bottom', 
          'scale':'xscale',  'title':xlabel},
         {'orient':'left', 
          'scale':'yscale',  'title':ylabel}]
    ...
    def _create_tooltips(hist: Histogram1D, vega: dict, kwargs: dict):
        ...
        if kwargs.pop('tooltips', False):
            ...
            vega['signals'].append({'name':'tooltip', 
             'value':{},  'on':[
            ...
            vega['marks'].append({'type':'text', 
             'encode':{'enter':{'align':{'value': 'center'}, 
              ...
              'update':{'x':{'scale':'xscale', 
                'signal':'(tooltip.x + tooltip.x2) / 2',  'band':0.5}, 
               'y':{'scale':'yscale', 
                'signal':'tooltip.y',  'offset':-2}, 
