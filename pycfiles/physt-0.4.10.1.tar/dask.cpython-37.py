...
# [GCC 8.4.0]
# Embedded file name: /home/honza/code/my/physt/physt/compat/dask.py
# Compiled at: 2020-02-17 15:03:55
...
def _run_dask(name: str, data: 'dask.array.Array', compute: bool, method, func, expand_arg: bool=False):
    ...
    import dask
    if expand_arg:
        graph = dict((('{0}-{1}-{2}'.format(name, data.name, index), (func, *item)) for index, item in enumerate(data.__dask_keys__())))
    else:
        graph = dict((('{0}-{1}-{2}'.format(name, data.name, index), (func, item)) for index, item in enumerate(data.__dask_keys__())))
...
def histogram1d(data, bins=None, *args, **kwargs):
    ...
    return _run_dask(name='dask_adaptive1d',
      data=data,
...
def histogramdd(data, bins=None, *args, **kwargs):
    ...
    import dask
    import dask.array.rechunk as rechunk
    if isinstance(data, (list, tuple)):
        data = dask.array.stack(data, axis=1)
    elif not hasattr(data, 'dask'):
    ...
    return _run_dask(name='dask_adaptive2d',
      data=data,
      compute=(kwargs.pop('compute', True)),
      method=(kwargs.pop('dask_method', 'threaded')),
      func=block_hist,
...
def h3(data, *args, **kwargs):
    ...
    return histogramdd(data, *args, dim=3, **kwargs)