...
# Compiled at: 2020-02-17 14:53:06
# Size of source mod 2**32: 8055 bytes
"""
...
import numpy as np
from physt.bin_utils import find_human_width_24, find_human_width_60, find_human_width_decimal
...
class TimeTickHandler:
    ...
    def deduce_level(cls, h1: Histogram1D, min_: float, max_: float) -> 'TimeTickHandler.LevelType':
        ...
        if ideal_width < 0.8:
            return (
             'sec', find_human_width_decimal(ideal_width))
        if ideal_width < 50:
            return (
             'sec', find_human_width_60(ideal_width))
        if ideal_width < 3000:
            return (
             'min', find_human_width_60(ideal_width / 60))
        if ideal_width < 70000:
            return (
             'hour', find_human_width_24(ideal_width / 3600))
