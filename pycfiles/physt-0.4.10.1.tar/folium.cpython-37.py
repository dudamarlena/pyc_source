...
# [GCC 8.4.0]
# Embedded file name: /home/honza/code/my/physt/physt/plotting/folium.py
...
def _bins_to_json(h2):
    ...
    return {'type':'FeatureCollection', 
     'features':[{'type':'Feature',  'geometry':{'type':'Polygon',  'coordinates':[[[east[j], south[i]], [east[j], north[i]], [west[j], north[i]], [west[j], south[i]], [east[j], south[i]]]]},  'properties':{'count': float(h2.frequencies[(i, j)])}} for i in range(h2.shape[0]) for j in range(h2.shape[1])]}
...
def geo_map(h2, map=None, tiles='stamenterrain', cmap='wk', alpha=0.5, lw=1, fit_bounds=None, layer_name=None):
    ...
    if not map:
        ...
        layer_name = layer_name or h2.name
    ...
    def styling_function(bin):
        ...
        return {'fillColor':color_map(count), 
         'color':'black', 
