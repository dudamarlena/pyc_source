...
# [GCC 8.4.0]
# Embedded file name: /home/honza/code/my/physt/physt/histogram_base.py
# Compiled at: 2019-10-24 07:19:28
# Size of source mod 2**32: 29357 bytes
...
class HistogramBase(abc.ABC):
        ...
        if frequencies is None:
            ...
            if dtype is not None:
                ...
                if np.issubdtype(frequencies.dtype, np.integer):
                    ...
                    if np.issubdtype(frequencies.dtype, np.floating):
    ...
    def _get_axis(self, name_or_index: AxisIdentifier) -> int:
        ...
        if isinstance(name_or_index, int) and not name_or_index < 0:
            if name_or_index >= self.ndim:
            ...
            if isinstance(name_or_index, str):
                if name_or_index not in self.axis_names:
                    ...
                    raise ValueError('No axis with such name: {0}, available names: {1}. In most places, you can also use numbers.'.format(name_or_index, ', '.join(named_axes)))
            ...
            if np.can_cast(self.dtype, value):
                ...
                if np.issubdtype(value, np.integer):
                    if self.dtype.kind == 'f':
                        for array in (
                         self._frequencies, self._errors2):
                            if np.any(array % 1.0):
    ...
    def merge_bins(self, amount: Optional[int]=None, *, min_frequency: Optional[float]=None, axis: Optional[AxisIdentifier]=None, inplace: bool=False) -> 'HistogramBase':
        ...
        if not inplace:
            ...
            if amount is not None:
                ...
                bin_map = [(
                 i, i // amount) for i in range(self.shape[axis])]
            else:
    ...
    def _reshape_data(self, new_size, bin_map, axis=0):
        ...
        self._apply_bin_map(old_frequencies=(self._frequencies),
    ...
    def _apply_bin_map(self, old_frequencies, new_frequencies, old_errors2, new_errors2, bin_map, axis=0):
        ...
        """
        if old_frequencies is not None:
            if old_frequencies.shape[axis] > 0:
    ...
    def _kwargs_from_dict(cls, a_dict: dict) -> dict:
        ...
        kwargs = {'binnings':[BinningBase.from_dict(binning_data) for binning_data in a_dict['binnings']], 
    ...
    def __repr__(self):
        if self.name:
            result = "{0}('{4}', bins={1}, total={2}, dtype={3})".format(self.__class__.__name__, self.shape, self.total, self.dtype, self.name)
        else:
            result = '{0}(bins={1}, total={2}, dtype={3})'.format(self.__class__.__name__, self.shape, self.total, self.dtype)
        return result
    ...
    def __imul__(self, other):
        ...
        if np.issubdtype(self.dtype, np.integer):
            if np.issubdtype(type(other), np.floating):
                self.dtype = float
    ...
    @classmethod
    def _merge_meta_data--- This code section failed: ---

 L. 828         0  LOAD_GLOBAL              set
                2  LOAD_DEREF               'first'
                4  LOAD_ATTR                _meta_data
                6  LOAD_METHOD              keys
                8  CALL_METHOD_0         0  ''
               10  CALL_FUNCTION_1       1  ''
               12  STORE_FAST               'keys'

 L. 829        14  LOAD_FAST                'keys'
               16  LOAD_METHOD              union
               18  LOAD_GLOBAL              set
               20  LOAD_DEREF               'second'
               22  LOAD_ATTR                _meta_data
               24  LOAD_METHOD              keys
               26  CALL_METHOD_0         0  ''
 ...
 L. 830        34  LOAD_CLOSURE             'first'
               36  LOAD_CLOSURE             'second'
               38  BUILD_TUPLE_2         2 
               40  LOAD_DICTCOMP            '<code_object <dictcomp>>'
               42  LOAD_STR                 'HistogramBase._merge_meta_data.<locals>.<dictcomp>'
               44  MAKE_FUNCTION_8          'closure'

 L. 832        46  LOAD_FAST                'keys'
