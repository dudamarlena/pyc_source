...
# [GCC 8.4.0]
# Embedded file name: /home/honza/code/my/physt/physt/histogram_nd.py
# Compiled at: 2019-08-28 04:52:53
# Size of source mod 2**32: 20226 bytes
...
class HistogramND(HistogramBase):
    __doc__ = 'Multi-dimensional histogram data.\n\n    Attributes\n    ----------\n\n    '
    ...
    def __getitem__(self, index):
        ...
        if isinstance(index, tuple):
            if len(index) > self.ndim:
                raise IndexError('Too many indices ({0}) to select from {1}D histogram'.format(len(index), self.ndim))
            if len(index) == self.ndim:
                if all((isinstance(i, int) for i in index)):
                    return (tuple(((self.get_bin_left_edges(i)[j], self.get_bin_right_edges(i)[j]) for i, j in enumerate(index))),
    ...
    def get_bin_widths(self, axis: Optional[AxisIdentifier]=None) -> np.ndarray:
        if axis is not None:
    ...
    def find_bin(self, value, axis: Optional[AxisIdentifier]=None):
        ...
        if axis is not None:
            ...
            if ixbin == 0:
                ...
                if value < self.get_bin_right_edges(axis)[(ixbin - 1)]:
                    return ixbin - 1
                if ixbin == self.shape[axis]:
                    return
                return
    ...
    def _reduce_dimension(self, axes, frequencies, errors2, **kwargs) -> HistogramBase:
        ...
        if len(axes) == 1:
            ...
            return klass(binning=(bins[0]), frequencies=frequencies, errors2=errors2, axis_name=(axis_names[0]),
              name=name)
        if len(axes) == 2:
            klass = kwargs.get('type', Histogram2D)
            return klass(binnings=bins, frequencies=frequencies, errors2=errors2, axis_names=axis_names,
              name=name)
    ...
    def __eq__(self, other: Any):
        ...
        if not isinstance(other, self.__class__):
            ...
            return other.axis_names == self.axis_names or False
        return True


class Histogram2D(HistogramND):
    __doc__ = 'Specialized 2D variant of the general HistogramND class.\n\n    In contrast to general HistogramND, it is plottable.\n    '
    ...
    def numpy_like(self) -> Tuple[(np.ndarray, np.ndarray, np.ndarray)]:
        """Same result as would the numpy.histogram function return."""
        return (
         self.frequencies, self.numpy_bins[0], self.numpy_bins[1])
...
def calculate_frequencies(data, ndim: int, binnings, weights=None, dtype=None) -> Tuple[(np.ndarray, np.ndarray, float)]:
    ...
    if data is not None:
        ...
        elif data.shape[0] != weights.shape[0]:
