...
# [GCC 8.4.0]
# Embedded file name: /home/honza/code/my/physt/physt/plotting/ascii.py
...
try:
    ...
    def map(h2: 'Histogram2D', **kwargs):
        ...
        for i in range(h2.shape[0] - 1, -1, -1):
            line = [xtermcolor.colorize('█', bg=0, rgb=(colors[(i, j)])) for j in range(h2.shape[1])]
            line = '|' + ''.join(line) + '|'
            if i == h2.shape[0] - 1:
        ...
        colorbar = [xtermcolor.colorize('█', bg=0, rgb=(65793 * int(j * 255 / (h2.shape[1] + 2)))) for j in colorbar_range]
