...
class BinningBase:
    ...
    def __init__(self, bins=None, numpy_bins=None, includes_right_edge=False, adaptive=False):
        ...
        if bins is not None:
            ...
            assert is_rising(bins), 'Bins must be in rising order.'
        else:
            if numpy_bins is not None:
    ...
    def is_regular(self, rtol: float=1e-05, atol: float=1e-08) -> bool:
    ...
    def is_consecutive(self, rtol: float=1e-05, atol: float=1e-08) -> bool:
    ...
    def adapt(self, other: 'BinningBase'):
        ...
        if np.array_equal(self.bins, other.bins):
            return (None, None)
            assert self.is_adaptive(), 'Cannot adapt non-adaptive binning.'
        else:
    ...
    def as_fixed_width(self, copy: bool=True) -> 'FixedWidthBinning':
        ...
        if self.bin_count == 0:
            ...
            if (self.bin_count == 1 or self.is_consecutive)():
                if self.is_regular():
    ...
    def apply_bin_map(self, bin_map) -> 'BinningBase':
        ...
        includes_right_edge = self.includes_right_edge and bins[(-1, 1)] == self.bins[(-1,
                                                                                       1)]
...
class NumpyBinning(BinningBase):
    __doc__ = 'Binning schema working as numpy.histogram.\n    '
...
class FixedWidthBinning(BinningBase):
    __doc__ = 'Binning schema with predefined bin width.'
    adaptive_allowed = True

    def __init__(self, bin_width, bin_count=0, bin_times_min=None, min=None, includes_right_edge=False, adaptive=False, bin_shift=None, align=True, **kwargs):
        ...
        if bin_times_min is not None or bin_shift is not None:
            if min is not None:
    ...
    def __repr__(self):
        result = '{0}(bin_width={1}, bin_count={2}, min={3}'.format(self.__class__.__name__, self.bin_width, self.bin_count, self.first_edge)
        if self.is_adaptive():
    ...
    def _force_bin_existence_single(self, value, includes_right_edge=None):
        ...
        if value < self.numpy_bins[0]:
            ...
            if value >= self.numpy_bins[(-1)]:
                ...
                if self.last_edge == value:
                    if not includes_right_edge:
                        add_right += 1
                        self._bin_count += 1
    ...
    def copy(self):
        return FixedWidthBinning(bin_width=(self._bin_width),
...
class ExponentialBinning(BinningBase):
    __doc__ = 'Binning schema with exponentially distributed bins.'
    adaptive_allowed = False
...
def human_binning(data=None, bin_count: Optional[int]=None, *, kind: Optional[str]=None, range: Optional[Tuple[(float, float)]]=None, min_bin_width: Optional[float]=None, max_bin_width: Optional[float]=None, **kwargs) -> FixedWidthBinning:
...
def integer_binning(data=None, **kwargs) -> FixedWidthBinning:
    ...
    return fixed_width_binning(data=data, bin_width=kwargs.pop('bin_width', 1), align=True, 
     bin_shift=0.5, **kwargs)
...
def exponential_binning(data=None, bin_count: Optional[int]=None, *, range=None, **kwargs) -> ExponentialBinning:
    ...
    if bin_count is None:
        ...
        range = (
         np.log10(range[0]), np.log10(range[1]))
    else:
        range = (
         np.log10(data.min()), np.log10(data.max()))
...
def calculate_bins_nd(array, bins=None, *args, **kwargs):
    ...
    if range_:
        kwargs['range'] = range_
    bins = [calculate_bins(array[:, i], bins[i], *(arg[i] for arg in args if arg[i] is not None), **{k:kwarg[i] for k, kwarg in kwargs.items() if kwarg[i] is not None}) for i in range(dim)]
    return bins


binning_methods = {'numpy':numpy_binning, 
 ...
 'human':human_binning}
try:
    import astropy.stats.histogram as _astropy_histogram
    import warnings
...
def ideal_bin_count(data, method: str='default') -> int:
    ...
    if method == 'rice':
        return int(np.ceil(2 * np.power(n, 0.3333333333333333)))


bincount_methods = [
 'default', 'sturges', 'rice', 'sqrt', 'doane']
