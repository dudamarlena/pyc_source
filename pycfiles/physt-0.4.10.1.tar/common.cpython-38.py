...
# [GCC 8.4.0]
# Embedded file name: /home/honza/code/my/physt/physt/plotting/common.py
# Compiled at: 2019-10-30 17:25:46
...
def pop_kwargs_with_prefix--- This code section failed: ---

 L.  86         0  LOAD_CLOSURE             'prefix'
                2  BUILD_TUPLE_1         1 
                4  LOAD_LISTCOMP            '<code_object <listcomp>>'
                6  LOAD_STR                 'pop_kwargs_with_prefix.<locals>.<listcomp>'
                8  MAKE_FUNCTION_8          'closure'
               10  LOAD_DEREF               'kwargs'
               12  GET_ITER         
               14  CALL_FUNCTION_1       1  ''
               16  STORE_FAST               'keys'

 L.  87        18  LOAD_CLOSURE             'kwargs'
               20  LOAD_CLOSURE             'prefix'
               22  BUILD_TUPLE_2         2 
               24  LOAD_DICTCOMP            '<code_object <dictcomp>>'
               26  LOAD_STR                 'pop_kwargs_with_prefix.<locals>.<dictcomp>'
...
class TimeTickHandler:
    __doc__ = 'Callable that creates ticks and labels corresponding to "sane" time values.\n\n    Note: This class is very experimental and subject to change or disappear.\n    '
    ...
    def parse_level(cls, value: Union[(LevelType, float, str, timedelta)]) -> 'TimeTickHandler.LevelType':
        if isinstance(value, tuple):
            ...
            if isinstance(value, timedelta):
                ...
                matchers = (('^(center|edge)s?$', lambda m: (m[1], 0)),
                 (
    ...
    def deduce_level(cls, h1: Histogram1D, min_: float, max_: float) -> 'TimeTickHandler.LevelType':
        ...
        if ideal_width < 0.8:
            return (
             'sec', cls.find_human_width_decimal(ideal_width))
        if ideal_width < 50:
            return (
             'sec', cls.find_human_width_60(ideal_width))
        if ideal_width < 3000:
            return (
             'min', cls.find_human_width_60(ideal_width / 60))
        if ideal_width < 70000:
            return (
             'hour', cls.find_human_width_decimal(ideal_width / 3600))
    ...
    def format_time_ticks(self, ticks: List[float], level: LevelType) -> List[str]:
        ...
        return [(('-' if neg else '+') if sign else '') + format.format(h, m if not include_hours else str(m).zfill(2), s if not include_mins else str(s).zfill(2)) for neg, h, m, s in hms]
