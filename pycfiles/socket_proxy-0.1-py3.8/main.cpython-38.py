...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/socket_proxy/main.py
# Compiled at: 2020-05-03 13:12:57
...
def security_group(parser, server: bool):
    ...
    if server:
        ...
        text_cert.append(a)
        text_key.append(a)
    group.add_argument('--ca',
      metavar='FILE', type=valid_file, help=(' '.join(text_ca)))
    ...
    group.add_argument('--key',
      ...
      required=server,
      help=(' '.join(text_key)))
    group.add_argument('--cipher',
      help='Ciphers to use for the TLS connection.')
    if not server:
        group.add_argument('--no-verify-hostname',
          action='store_true',
          default=False,
    ...
    if server:
        group.add_argument('-l',
         ...
         '', DEFAULT_PORT),
          ...
          help=f"The address to listen on. If host is not given the server will listen for connections from all IPs. If the port is not given the server will listen on port {DEFAULT_PORT}.")
    else:
        group.add_argument('-c',
          ...
          help=f"The address to connect with host[:port]. Required for clients. The default port is {DEFAULT_PORT}.")
        group.add_argument('-d',
          '--dst',
          dest='dst',
          metavar='[host]:port',
          type=(lambda x: parse_address(x, host='localhost')),
          help='Target host and port for the connection. If the host is not given localhost will be used.')
...
def option_group(parser, server: bool):
    ...
    group.add_argument('--ban-time',
      type=int,
      default=60,
      help='Seconds until the number of connects by an IP resets.')
    group.add_argument('--max-clients',
      type=int,
      default=0,
      help="Maximum number of clients able to use the tunnel. This option can be set on the server and client. For the server it's the maximum number of clients per tunnel.")
    group.add_argument('--max-connects',
      type=int,
      default=0,
      help='Maximum number of connects an IP is allowed to do within a certain time span. Disabled if 0. If set on both sites of the tunnel  the lower number is used.')
    if server:
        group.add_argument('--max-tunnels',
          type=int,
          default=0,
...
def run_client(args):
    for arg in ('ca', 'connect', 'dst'):
        if not getattr(args, arg):
            ...
            sys.exit(1)
        cli = TunnelClient(args.connect, *(args.dst), **{'ca':args.ca, 
         'cert':args.cert, 
         'key':args.key, 
...
def run_server(args):
    for arg in ('cert', 'key'):
        if not getattr(args, arg):
            _logger.critical('Missing -%s argument', arg)
            sys.exit(1)
        server = ProxyServer(*args.listen, ca=args.ca, 
         cert=args.cert, 
         key=args.key, 
         bantime=args.ban_time, 
         max_clients=args.max_clients, 
