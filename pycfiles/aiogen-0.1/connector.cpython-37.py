...
# Embedded file name: build/bdist.linux-x86_64/egg/aiogcd/connector/connector.py
...
DATASTORE_URL = 'https://datastore.googleapis.com/v1/projects/{project_id}:{method}'
_MAX_LOOPS = 128

class GcdConnector:

    def __init__(self, project_id, client_id, client_secret, token_file, scopes=DEFAULT_SCOPES):
        self.project_id = project_id
        self._token = Token(client_id, client_secret, token_file, scopes)
        self._run_query_url = DATASTORE_URL.format(project_id=(self.project_id),
          method='runQuery')
        self._commit_url = DATASTORE_URL.format(project_id=(self.project_id),
          method='commit')
        self._lookup_url = DATASTORE_URL.format(project_id=(self.project_id),
          method='lookup')
    ...
    async def commit(self, mutations):
        ...
        """
        data = {'mode':'NON_TRANSACTIONAL', 
         'mutations':mutations}
        async with aiohttp.ClientSession() as session:
            async with session.post((self._commit_url),
              data=(json.dumps(data)),
              headers=(await self._get_headers())) as resp:
                content = await resp.json()
                if resp.status == 200:
                    return tuple(content.get('mutationResults', tuple()))
                raise ValueError('Error while committing to the datastore: {} ({})'.format(content.get('error', 'unknown'), resp.status))
    ...
    async def _run_query(self, data):
        ...
        while True:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post((self._run_query_url),
                  data=(json.dumps(data)),
                  headers=(await self._get_headers())) as resp:
                    content = await resp.json()
                    if resp.status == 200:
                        entity_results = content['batch'].get('entityResults', [])
                        results.extend(entity_results)
                        more_results = content['batch']['moreResults']
                        cursor = content['batch']['endCursor']
                        if more_results in ('NO_MORE_RESULTS', 'MORE_RESULTS_AFTER_LIMIT',
                                            'MORE_RESULTS_AFTER_CURSOR'):
                            break
                        if more_results == 'NOT_FINISHED':
                            continue
                        raise ValueError('Unexpected value for "moreResults": {}'.format(more_results))
                    raise ValueError('Error while query the datastore: {} ({})'.format(content.get('error', 'unknown'), resp.status))
    ...
    async def _get_entities_cursor(self, data):
        ...
        return (
         [Entity(result['entity']) for result in results], cursor)
    ...
    async def get_entities_by_keys(self, keys, missing=None, deferred=None, eventual=False):
        ...
        data = lambda : json.dumps({'readOptions':read_options, 
        ...
        while keys and attempts < _MAX_LOOPS:
            attempts += 1
            async with aiohttp.ClientSession() as session:
                async with session.post((self._lookup_url),
                  data=(data()),
                  headers=(await self._get_headers())) as resp:
                    content = await resp.json()
                    entities.extend((Entity(result['entity']) for result in content.get('found', [])))
                    if missing is not None:
                        missing.extend((result['entity'] for result in content.get('missing', [])))
                    deferred_keys = [Key(result) for result in content.get('deferred', [])]
                    if deferred is not None:
    ...
    async def get_entity_by_key(self, key, missing=None, deferred=None, eventual=False):
        ...
        """
        entity = await self.get_entities_by_keys([key], missing, deferred, eventual)
        if entity:
            return entity[0]
    ...
    async def _commit_entities_or_keys(self, entities_or_keys, method):
        mutations = [{method: entity_or_key.get_dict()} for entity_or_key in entities_or_keys]
        mutations_results = await self.commit(mutations)
        return tuple((self._check_mutation_result(entity_or_key, mutation_result) for entity_or_key, mutation_result in zip(entities_or_keys, mutations_results)))


class GcdServiceAccountConnector(GcdConnector):

    def __init__(self, project_id, service_file, session=None, scopes=None):
        scopes = scopes or 
        self.project_id = project_id
        self._token = ServiceAccountToken(project_id, service_file, scopes, session)
        self._run_query_url = DATASTORE_URL.format(project_id=(self.project_id),
          method='runQuery')
        self._commit_url = DATASTORE_URL.format(project_id=(self.project_id),
          method='commit')
