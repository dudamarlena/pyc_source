...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/fluas/barcodes.py
# Compiled at: 2018-05-04 16:24:07
# Size of source mod 2**32: 13811 bytes
"""
"""
import errno, logging, os, pandas as pd, re, sys
...
def run_fastq_multx(tsv, r1, r2, i1, out_dir, stats_file=None, m=0, d=2, q=0):
    ...
    if not rerun:
        ...
        if not stats_file:
            ...
            cmd = 'fastq-multx -B {tsv} {i1} {r1} {r2} -o n/a -o {dir}/%_R1.fastq -o {dir}/%_R2.fastq -m {m} -d {d} -q {q} > {stats}'.format(tsv=tsv,
              i1=i1,
              r1=r1,
              r2=r2,
              dir=out_dir,
...
def demultiplex_fastq(tsv, r1, i1=None, r2=None, out_dir=None, stats_file=None, dist_plot=None, m=0, d=2, q=0):
    ...
    fastqs = run_fastq_multx(tsv,
      r1, r2, i1, out_dir, stats_file=stats_file, m=m, d=d, q=q)
    if stats_file:
...
def barcode_stats(run_id, fastq, run_barcodes, count_table=None, stat_table=None, all_barcodes=None):
    ...
    if count_table:
        if stat_table:
            if file_exists([count_table, stat_table]):
                return (
                 count_table, stat_table)
    else:
    ...
    df.to_csv((count_table if count_table else sys.stdout),
      index_label='Barcode',
      float_format='%.0f')
    if stat_table:
        total = df.Count.sum()
        ...
        if all_barcodes:
            off_target = df.ix[df.Sample.isnull()].dropna(subset=[
             'Plate Number']).Count.sum()
