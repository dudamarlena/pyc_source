...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/fluas/plots.py
# Compiled at: 2018-05-04 16:38:01
# Size of source mod 2**32: 3500 bytes
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt, os, pandas as pd, seaborn as sns
from .stats import lc

def lorenz_curve(x, out_file, xlabel='Fraction of Samples', ylabel='Fraction of On Target Reads', title='Read Distribution'):
    ...
    plt.plot([0.0, max(L)], color='k', linewidth=3, linestyle='dashed')
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.title(title)
...
def read_joining_summary_plot(a, b, out_file, colors=[
 'r', 'b'], labels=[
 'Unjoined', 'Joined'], xlabel='Read count', ylabel='Density'):
    sns.set(style='ticks', palette='muted', color_codes=True)
    xmax = max([a.max(), b.max()])
    pa = sns.distplot(a,
      hist=False,
      color=(colors[0]),
      kde_kws={'color':colors[0], 
     'lw':3,  'label':labels[0]})
    pa = sns.distplot(b,
      hist=False,
      color=(colors[1]),
      ax=pa,
      kde_kws={'color':colors[1], 
     'lw':3,  'label':labels[1]})
