...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/fluas/joining.py
...
def fastq_join(r1, r2, out_file, p=2, m=240):
    ...
    if os.path.exists(r1) and read_count(r1) == 0:
        ...
        with file_transaction([out_file, joined_read_lengths, joined_read_stats]) as (tx):
            ...
            tmp = tx[0] + '_%.fastq'
            cmd = 'fastq-join -p {p} -m {m} -o {out} -r {lengths} {r1} {r2} > {stats}'.format(p=p,
              m=m,
              out=tmp,
              lengths=(tx[1]),
              r1=r1,
...
def fastq_join_stdout_to_dataframe(fnames):
    ...
    df = pd.DataFrame(d).T
    df[['Total joined', 'Total reads']] = df[['Total joined', 'Total reads']].astype(int)
    df[['Average join len', 'Stdev join len']] = df[[
     'Average join len', 'Stdev join len']].astype(float)
