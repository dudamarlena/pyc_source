...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/fluas/utils.py
# Compiled at: 2018-05-04 16:38:01
# Size of source mod 2**32: 12626 bytes
import functools, logging, os, pandas as pd, re, shutil, subprocess, tempfile, time
...
def check_sample_id(s):
    ...
    sampleid = re.sub('[\\W_]', '-', sampleid).strip('-')
    return sampleid
...
def replace_suffix(to_transform, suffix):
    ...
    if is_sequence(to_transform):
        ...
        return transformed
    if is_string(to_transform):
        base, _ = os.path.splitext(to_transform)
        return base + suffix
    raise ValueError('replace_suffix takes a single filename as a string or a list of filenames to transform.')
...
def append_stem(to_transform, word):
    ...
    raise ValueError('append_stem takes a single filename as a string or a list of filenames to transform.')
...
def touch(fname, mode=438, dir_fd=None, **kwargs):
