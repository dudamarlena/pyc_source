...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/fluas/samplesheet.py
# Compiled at: 2018-05-04 16:38:01
# Size of source mod 2**32: 8104 bytes
import logging, numpy as np, os, pandas as pd, re, string
...
def preprocess_samplesheet(excel_file, out_file=None, cols=None, reverse_complement=False):
    ...
    if out_file is None:
        ...
        column_name_map = {'PLATE NAME(S)':'PRIMER_PLATE_NAME', 
        ...
        if any(df.POC.isnull()):
            ...
            raise ValueError('All rows must contain a POC [CHECK ROWS [%s]]' % ','.join(map(str, offending_rows)))
        if any(df.PRIMER_BARCODE.isnull()):
            offending_rows = [i + 2 for i in df.index[(df.PRIMER_BARCODE.isnull() == True)].tolist()]
            raise ValueError('All rows must contain a PRIMER BARCODE [CHECK ROWS [%s]]' % ','.join(map(str, offending_rows)))
        if any(df.SAMPLE_ID.isnull()):
            offending_rows = [i + 2 for i in df.index[(df.SAMPLE_ID.isnull() == True)].tolist()]
            raise ValueError('All rows must contain a SAMPLE ID [CHECK ROWS [%s]]' % ','.join(map(str, offending_rows)))
        ...
        df['VALID_SAMPLE_ID'] = np.where(df.duplicated(subset='VALID_SAMPLE_ID', keep=False), df['VALID_SAMPLE_ID'] + '-' + df['PRIMER_BARCODE'], df['VALID_SAMPLE_ID'])
        ...
        if cols:
            for label_value_pair in cols.split(','):
                try:
                    ...
                    logger.warning("Ignoring label:value pair [%s] as no ':' was identified as the separator" % label_value_pair)
...
def barcode_tsv_from_samplesheet(csv, out_file=None):
    ...
    df[['VALID_SAMPLE_ID', 'PRIMER_BARCODE']].to_csv(out_file,
...
def contamination_per_plate(csv, out_file):
    ...
    heatmap_by_group(df, out_file, 'TOTAL_CONTAMINATION', 'WELL_ROW', 'WELL_COL', 'PRIMER_PLATE_NUMBER')
    return out_file