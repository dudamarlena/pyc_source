...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/eye_keyboard/calibration.py
# Compiled at: 2015-11-24 20:44:34
...
def _init_display(display_size):
    disp = pygame.display.set_mode(display_size, pygame.FULLSCREEN | pygame.HWSURFACE | pygame.DOUBLEBUF)
...
def _init_buttons(camera_size, display_size):
    ...
    btn_pos = _compute_button_positions(camera_size[0], camera_size[1], display_size[0], display_size[1])
...
def _merge_button_data(btn_img, btn_pos):
    ...
    for bn in btn_img.keys():
        btn_pos[bn] = (
         btn_pos[bn][0] - BUTTON_HEIGHT / 2, btn_pos[bn][1] - BUTTON_WIDTH / 2)
...
def _place_rectangle_at_click_position(blitpos, img_size, stagevars, inp, settings):
    ...
    if mouse_pos[0] > blitpos[0] and mouse_pos[0] < blitpos[0] + img_size[0] and mouse_pos[1] > blitpos[1] and mouse_pos[1] < blitpos[1] + img_size[1]:
