...
# [GCC 8.4.0]
# Embedded file name: /home/ziirish/workspace/burp-ui/burpui/misc/parser/burp1.py
...
"""
import re, os, json, shutil, codecs
...
class Parser(Doc):
    ...
    def __init__(self, backend=None):
        ...
        self.openssl_auth = OSSLAuth(self.server_conf.get('ca_name'), self.openssl_conf, self.server_conf)
        return
    ...
    def _line_is_file_include(line):
        ...
        if not line:
            ...
            return line.startswith('.') or re.match('^#+\\s*\\.', line) is not None
    ...
    def _md5(path):
        ...
        try:
            with open(path, 'rb') as (bfile):
                for chunk in iter(lambda : bfile.read(4096), ''):
    ...
    def _load_conf_clients(self, name=None, in_path=None):
        ...
        for cli in clients:
            ...
            self._parse_conf_recursive(conf, client=True)
    ...
    def _readfile(self, path=None, client=False, insecure=False):
        ...
        if not self._is_secure_path(path) and not insecure:
            return (ret, path, False)
        if (path != self.conf or path != self.confcli) and not path.startswith('/'):
            if client:
    ...
    def _parse_lines(self, data, name=None, mode='srv'):
        ...
        for line in data:
            ...
            if res:
                key = res.group(1)
                val = res.group(2)
                if mode == 'srv' and key == 'directory':
                    self.workingdir = val
                if key == 'clientconfdir':
                    if mode == 'srv':
    ...
    def _is_secure_path(self, path=None):
        ...
        if not any(cond):
            self.logger.warning(('Tried to access non-allowed path: {}').format(path))
            return False
        return True
    ...
    def _list_clients(self, force=False):
        ...
        for cli in os.listdir(self.clientconfdir):
            ...
            if os.path.isfile(full_file) and not cli.startswith('.') and not cli.endswith('~'):
                res.append({'name': cli, 
                   'value': full_file})
    ...
    def _write_key(self, fil, key, data, conf, mode=None):
        if key in self.boolean_srv or key in self.boolean_cli:
            ...
            if key == 'clientconfdir' and mode == 'srv' and val != self.clientconfdir:
                self.clientconfdir = val
                self._refresh_cache(purge=True)
            if key == 'ca_conf' and mode == 'srv' and val != self.server_conf.get(key):
    ...
    def path_expander(self, pattern=None, source=None, client=None):
        ...
        if not pattern:
            ...
            if not pattern.startswith('/'):
                if source and (source.startswith(self.clientconfdir) or source.startswith(self.root)):
            ...
            if not re.search('\\?|\\*|\\[.*\\]', pattern):
                if self._is_secure_path(pattern):
                    return [pattern]
                return []
            return [ x for x in glob(pattern) if os.path.isfile(x) and not x.endswith('~') and not x.endswith('.back') and self._is_secure_path(x) ]
    ...
    def read_client_conf(self, client=None, conf=None):
        ...
        """
        res = {'common': [], 'boolean': [], 'integer': [], 'multi': [], 'includes': [], 'includes_ext': [], 'clients': self._list_clients()}
        ...
        res2['includes'] = [ x for x in parsed.flatten('include', False).keys()
                           ]
    ...
    def store_conf(self, data, conf=None, client=None, mode='srv', insecure=False, source=None):
        ...
        if not mconf:
            ...
            if not self._is_secure_path(mconf) and not insecure:
                return [
                 [
                  NOTIF_ERROR,
                  ('Sorry you are not allowed to access this path: {}').format(mconf)]]
            ...
            if not os.path.isfile(ref) and os.path.isfile(mconf):
                ...
                try:
                    with codecs.open(mconf, 'w', 'utf-8', errors='ignore') as (fil):
                        for line in orig:
                            if self._line_removed(line, data.keys()) and not self._line_is_comment(line) and not self._line_is_file_include(line):
                                fil.write(('#{}\n').format(line))
                                ...
                                if key not in already_multi:
                                    self._write_key(fil, key, data, conf=conffile, mode=mode)
                                if key in getattr(self, ('multi_{}').format(mode)):
            ...
            return [
             [
              NOTIF_OK, 'Configuration successfully saved.']]
    ...
    def cancel_restore(self, name=None):
        ...
        try:
            if os.path.exists(path):
                ...
                return [
                 NOTIF_WARN, 'There is no restoration scheduled for this client']
        except OSError as exp:
            return [
             NOTIF_ERROR, ('Unable to cancel restoration: {}').format(str(exp))]

        return [
         NOTIF_OK, 'Restoration successfully canceled']
    ...
    def server_initiated_restoration(self, name=None, backup=None, files=None, strip=None, force=None, prefix=None, restoreto=None):
        ...
        if self.read_backup(name):
            raise BUIserverException('A backup is already scheduled. Cannot schedule both restore and backup at the same time.')
        ...
        try:
            ...
            return [
             NOTIF_OK, 'Server-initiated restoration successfully scheduled']
        except Exception as exp:
            return [NOTIF_ERROR,
             ('Unable to schedule a server-initiated restoration: {}').format(str(exp))]
    ...
    def cancel_backup(self, name=None):
        ...
        try:
            if os.path.exists(path):
                ...
                return [
                 NOTIF_WARN, 'There is no backup scheduled for this client']
        except OSError as exp:
            return [
             NOTIF_ERROR, ('Unable to cancel backup: {}').format(str(exp))]

        return [
         NOTIF_OK, 'Backup successfully canceled']
    ...
    def server_initiated_backup(self, name=None):
        ...
        if self.read_restore(name):
            raise BUIserverException('A restoration is already scheduled. Cannot schedule both restore and backup at the same time.')
        path = self._get_server_backup_path(name)
        try:
            with open(path, 'w'):
                os.utime(path, None)
            ...
            return [
             NOTIF_ERROR,
             ('Unable to schedule a server-initiated backup: {}').format(str(exp))]
