...
# [GCC 8.4.0]
# Embedded file name: /home/ziirish/workspace/burp-ui/burpui/misc/parser/openssl.py
...
"""
import os, re, codecs, logging, subprocess
...
class OSSLAuth(object):
    ...
    def _get_crt_path(self, client):
        return ('{}.crt').format(os.path.join(self.ossl_conf.values.get('CA_DIR'), client))
    ...
    def revoke_client(self, client, update=True):
        ...
        try:
            ...
            subprocess.check_call([
             ...
             '--revoke',
             serial], stderr=subprocess.STDOUT, stdout=DEVNULL)
            if update:
                subprocess.check_call([
                 self.global_conf.get('ca_burp_ca'),
                 '--name',
                 ...
                 '--dir',
                 self.ossl_conf.values.get('CA_DIR'),
                 '--crl'], stderr=subprocess.STDOUT, stdout=DEVNULL)
                self._load_crl()
...
class OSSLConf(object):
    ...
    def _translate_local(self, temp, pattern, val):
        ...
        return re.sub('\\${?' + pattern + '}?', res, val)

    @staticmethod
    def _is_env(val):
        """Look for 'global' variables (ie. $ENV::HOME)"""
        env = re.compile('\\${?ENV::(\\w+)}?')
        res = env.findall(val)
        if res:
            return res
        return False
    ...
    def _translate_env(self, temp, pattern, val):
        ...
        """
        res = self.env_cache.get(pattern) or os.getenv(pattern) or self.conf.get(pattern) or temp.get(pattern) or ''
        self.env_cache[pattern] = res
        return re.sub('\\${?ENV::' + pattern + '}?', res, val)
