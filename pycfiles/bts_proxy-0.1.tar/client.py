...
# Embedded file name: build/bdist.linux-x86_64/egg/btsync/client.py
...
class Client(object):
    ...
    def __init__(self, **kwargs):
        ...
        assert len(kwargs.keys()) == 0, 'Unrecognized params: %r' % kwargs.keys()
    ...
    def _make_request(self, **kwargs):
        ...
        url = ('http://{host}:{port}/gui/{endpoint}?{qs}').format(host=self._host, port=self._port, endpoint=endpoint, qs=urllib.urlencode(params))
        method = getattr(session if session is not None else self._session, method)
    ...
    def _get_token(self, session):
        response_text = self._make_request(endpoint='token.html', method='post', session=session, with_token=False, is_json=False)
        return response_text.split("<html><div id='token' style='display:none;'>")[1].split('</div></html>')[0]
    ...
    @property
    def os_type(self):
        return self._make_request(params={'action': 'getostype'}).get('os', None)

    @property
    def version(self):
        return self._make_request(params={'action': 'getversion'}).get('version', None)

    @property
    def new_version(self):
        return self._make_request(params={'action': 'checknewversion'}).get('version', None)

    @property
    def sync_folders(self):
        folders = self._make_request(params={'action': 'getsyncfolders'}).get('folders', [])
    ...
    def add_sync_folder(self, name, secret, force=False):
        params = {'action': 'addsyncfolder', 'name': name, 
           'secret': secret}
        if force:
            params['force'] = 1
    ...
    def remove_sync_folder(self, name, secret):
        self._make_request(params={'action': 'removefolder', 
           'name': name, 
           'secret': secret})

    @property
    def settings(self):
        return Settings(**self._make_request(params={'action': 'getsettings'})['settings'])
    ...
    def update_secret(self, name, secret, new_secret):
        self._make_request(params={'action': 'updatesecret', 
           'name': name, 
           'secret': secret, 
           'newsecret': new_secret})

    def generate_invite(self, name, secret, readonly=False):
        invite_action = 'generateinvite' if not readonly else 'generateroinvite'
        response = self._make_request(params={'action': invite_action, 
           'name': name, 
           'secret': secret})
