...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/userprofile/forms.py
# Compiled at: 2012-05-29 05:24:39
from datetime import date
from django import forms
from django.forms import ModelForm
from django.utils.translation import ugettext_lazy as _
from userprofile.models import AbstractAvatarProfile, AbstractProfileBase, AbstractLocationProfile, AbstractContactProfile, AbstractPersonalProfile, AbstractSubscriptionProfile
...
class ProfileForm(forms.Form):
    """
    This is the generic profile form which you can use for all your user
    profile forms. It takes care of setting and saving User and Pofile
    objects for the form. You can provide it with field excludes to build
    specific forms, i.e. avatar change for, subscriptions form etc.
    """
    ...
    dob_year = forms.ChoiceField(label=_('Birth Year'), choices=[
     ('', 'Year')] + [ (i, i) for i in reversed(range(datetime.now().year - 100, datetime.now().year - 4))
                     ], required=False)
    ...
    def _gen_dob(self):
        if 'dob_day' in self.cleaned_data and 'dob_month' in self.cleaned_data:
            if 'dob_year' in self.cleaned_data:
                if [
                 '', '', ''] == [self.cleaned_data['dob_day'],
                 self.cleaned_data['dob_month'], self.cleaned_data['dob_year']]:
                    return
