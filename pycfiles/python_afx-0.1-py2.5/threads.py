...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/afx/threads.py
# Compiled at: 2008-01-20 01:32:13
...
itc_port = 17777 if 'ITC_PORT' not in environ else int(environ['ITC_PORT'])
p = None
    ...
    def start(self):
        ...
        def worker():
            ...
            if environ.get('PYPROF', '') != '':
                try:
                    outpath = environ['PYPROF'] % currentThread().getName()
                except:
                    real_worker()
                else:
                    runctx('real_worker()', locals(), globals(), outpath)
            else:
                real_worker()
        ...
        with self.lock:
            if not self.stopped:
                self.s = s
                self.threads = [ Thread(target=worker).start() for _ in xrange(self.size)
                               ]
            else:
                s.close()
        return
    ...
    def stop(self):
        with self.lock:
            if self.threads is not None:
                [ self.i.put(stop_msg) for _ in xrange(len(self.threads)) ]
