...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/afx/pubsub.py
# Compiled at: 2008-01-18 17:28:57
...
def read_pickle(read, init='', length_thresh=100000):
    with closing(StringIO()) as (sio):
        ...
        if (yield read_until(4)):
            ...
            if length_thresh is not None and length > length_thresh or length <= 0:
                warning('read_pickle', 'got length', length, 'streamlen', streamlen(sio), 'first bytes %x %x %x %x' % tuple(map(ord, lengthstr)))
...
def get_traceback():
    ...
    try:
        ...
        yield af.result(sys.exc_info()[2])
