...
# Embedded file name: cubicweb_localperms/views.py
...
class SecurityManagementView(management.SecurityManagementView):
    ...
    def granted_permission_edit_form(self, entity):
        ...
        form = self._cw.vreg['forms'].select('base', self._cw, entity=newperm, form_buttons=[
         wdgs.SubmitButton()], domid='reqperm%s' % entity.eid, __redirectvid='security', __redirectpath=entity.rest_path())
        ...
        if permnames is not None:
            field = guess_field(cwpermschema, self._cw.vreg.schema.rschema('name'), widget=wdgs.Select({'size': 1}), choices=permnames)
        else:
        ...
        form.append_field(field)
        renderer = self._cw.vreg['formrenderers'].select('htable', self._cw, rset=None, display_progress_div=False)
...
class CWGroupsTable(cwuser.CWGroupsTable):
    ...
    cwuser.CWGroupsTable.column_renderers['local_permissions'] = tableview.EntityTableColRenderer(renderfunc=local_permissions_cell, header=_('local_permissions'))
