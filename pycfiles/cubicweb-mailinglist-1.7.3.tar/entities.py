...
# Embedded file name: cubicweb_localperms/entities.py
# Compiled at: 2019-03-13 06:52:29
__doc__ = "cubicweb-localperms entity's classes"
...
def grant_permission(self, entity, pname, plabel=None):
    ...
    a new one.
    """
    if not self._cw.execute('SET X require_group G WHERE E eid %(e)s, G eid %(g)s, E granted_permission X, X name %(name)s, X label %(label)s', {'e': entity.eid, 'g': self.eid, 'name': pname, 
       'label': plabel}):
        self._cw.create_entity('CWPermission', name=pname, label=plabel, require_group=self, reverse_granted_permission=entity)
