...
# Embedded file name: cubicweb_localperms/ccplugin.py
# Compiled at: 2019-03-13 06:52:19
__doc__ = 'cubicweb-ctl plugin providing the check-localperms command'
...
def read_permission_edges(eschemas):
    ...
    for eschema in eschemas:
        for rqlexpr in eschema.get_rqlexprs('read'):
            ...
            triplets = [ triplet for triplet in triplets if len(triplet) == 3 and triplet[1] != 'has_group_permission'
                       ]
...
class CheckLocalPermsCommand(Command):
    ...
    options = (
     (
      'graph',
      {'short': 'g', 'type': 'string', 'metavar': '<file>', 'default': None, 
    ...
    def run(self, args):
        ...
        for eschema in eschemas:
            if not analyzer.is_root(eschema) and any(edge for edge in perm_edges if edge[1] == eschema) and not any(edge for edge in prop_edges if edge[1] == eschema):
                warn("%s is used in a read permission but isn't reached by propagation", eschema)
        ...
        if self.config.graph:
            ...
            for subj, obj, rtype, package in perm_edges:
                graph.add_edge(str(subj), str(obj), label=rtype, color=get_color(package), arrowhead='normal', style='dashed')
