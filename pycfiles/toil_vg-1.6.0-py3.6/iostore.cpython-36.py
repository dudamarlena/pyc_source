...
# Size of source mod 2**32: 36706 bytes
"""
...
"""
import sys, os, os.path, json, collections, logging, logging.handlers, struct, socket, threading, tarfile, shutil, tempfile, functools, random, time, dateutil, traceback, stat
...
class FileIOStore(IOStore):
    ...
    def read_input_file(self, input_path, local_path):
        ...
        elif not not os.path.exists(local_path):
            raise AssertionError
        real_path = os.path.abspath(os.path.join(self.path_prefix, input_path))
        if not os.path.exists(real_path):
            RealtimeLogger.error("Can't find {} from FileIOStore in {}!".format(input_path, self.path_prefix))
...
class AzureIOStore(IOStore):
    ...
    def __init__(self, account_name, container_name, name_prefix=''):
        ...
        self.name_prefix = name_prefix
        if self.name_prefix != '':
            if not self.name_prefix.endswith('/'):
                self.name_prefix += '/'
    ...
    def list_input_directory(self, input_path, recursive=False, with_times=False):
        ...
        if fake_directory != '':
            if not fake_directory.endswith('/'):
                fake_directory += '/'
        marker = None
        subdirectories = set()
        while 1:
            result = self.connection.list_blobs((self.container_name), prefix=fake_directory,
              marker=marker)
    ...
    def exists(self, path):
        ...
        while 1:
            ...
            result = self.connection.list_blobs((self.container_name), prefix=(self.name_prefix + path),
              marker=marker)
            for blob in result:
