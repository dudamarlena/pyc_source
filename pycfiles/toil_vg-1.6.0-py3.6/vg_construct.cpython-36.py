...
# Size of source mod 2**32: 95669 bytes
"""
vg_construct.py: construct a graph from a vcf and fasta

"""
import argparse, sys, os, os.path, errno, random, subprocess, shutil, itertools, glob, tarfile, doctest, re, json, collections, time, timeit, logging, logging.handlers, struct, socket, threading, string, getpass, pdb, logging, os
...
def construct_subparser(parser):
    ...
    parser.add_argument('--vcf', default=[], nargs='+', help='Variants to make graph from (single vcf or 1/region IN SAME ORDER AS REGIONS (as passed in by --regions or scanned in from --fasta_regions or --regions_file)).  VCFs separated by commas will be merged together and treated as one using bcftools merge (so must be over distinct sample sets).')
    parser.add_argument('--regions', default=[], nargs='+', help='1-based inclusive VCF coordinates in the form of SEQ or SEQ:START-END')
    parser.add_argument('--regions_file', default=None, help='List of regions (replaces --regions). Only first column of each line considered (so .fai acceptable)')
    ...
    parser.add_argument('--regions_regex', default=[], nargs='+', help="Ignore sequence names not fully matching (union of) given regexes when using --fasta_regions or --regions_file (ex: --regions_regex 'chr[1-9,M,X,Y,EBV][0-9]{0,1}' 'chr.*decoy' to keep only chroms and decoys from hs38d1)")
    parser.add_argument('--alt_regions_bed', type=make_url, help='BED file mapping alt regions (cols 1-3) to sequence names (col 4) from the FASTA. Alt regions will be aligned to the graph using vg msga')
    ...
    parser.add_argument('--handle_svs', action='store_true', help='pass --handle-sv to vg construct to parse symbolic SV alts')
    ...
    parser.add_argument('--validate', action='store_true', help='Run vg validate on constructed graphs')
    parser.add_argument('--add_chr_prefix', action='store_true', help='add "chr" prefix to chromosome names if not already present')
    parser.add_argument('--remove_chr_prefix', action='store_true', help='remove "chr" prefix from chromosome names')
    parser.add_argument('--keep_vcfs', action='store_true', help='write the VCFs created to make the filtered and control graphs to the output store')
    ...
    parser.add_argument('--filter_samples', nargs='+', help='Make a graph where all variants private to the the listed smaples are excluded')
    ...
    parser.add_argument('--pre_min_af', type=float, default=None, help='Run minimum allele frequency filter as preprocessing step on each input VCF.  Unlike --min_af, this will be applied before merging and any control graph construction')
    parser.add_argument('--mask_ambiguous', action='store_true', help='Convert IUPAC ambiguous characters in FASTA to Ns')
...
def validate_construct_options(options):
    ...
    """
    require(options.regions or options.fasta_regions or options.regions_file, '--regions or --fasta_regions required')
    require(not options.regions_file or not (options.fasta_regions or options.regions), '--regions_file cannot be used with --regions or --fasta_regions')
    require(not options.regions_regex or options.fasta_regions or options.regions_file, '--regions_regex can only be used with --fasta_regions or --regions_file')
    require(not options.add_chr_prefix or not options.remove_chr_prefix, '--add_chr_prefix cannot be used with --remove_chr_prefix')
    ...
    require('gbwt' not in options.indexes or 'xg' in options.indexes, '--xg_index required with --gbwt_index')
    require('gbwt' not in options.indexes or not options.pangenome and not options.pos_control and not options.neg_control and not options.sample_graph and not options.haplo_sample and not options.min_af or len(options.vcf) >= 1, '--gbwt_index with any graph other than --primary requires --vcf')
...
def run_merge_all_vcfs(job, context, vcf_file_ids_list, vcf_names_list, tbi_file_ids_list):
    ...
    for vcf_file_ids, vcf_names, tbi_file_ids in zip(vcf_file_ids_list, vcf_names_list, tbi_file_ids_list):
        if len(vcf_file_ids) > 1:
            merge_job = job.addChildJobFn(run_merge_vcfs, context, vcf_file_ids, vcf_names, tbi_file_ids, cores=(context.config.construct_cores),
    ...
    return (
     out_vcf_ids_list, out_names_list, out_tbi_ids_list)
...
def run_merge_vcfs(job, context, vcf_file_ids, vcf_names, tbi_file_ids):
    ...
    if len(vcf_file_ids) == 1:
        ...
        with open(os.path.join(work_dir, merged_name), 'wb') as (merged_file):
            cmd = [
             [
              'bcftools', 'merge', '--missing-to-ref', '--force-samples'] + vcf_names]
        ...
        return (
         context.write_intermediate_file(job, os.path.join(work_dir, merged_name)),
         merged_name,
...
def run_mask_ambiguous(job, context, fasta_id, fasta_name):
    ...
    job.fileStore.readGlobalFile(fasta_id, fasta_file, mutable=True)
    fa_mask_cmd = [
     'awk', 'BEGIN{FS=" "}{if(!/>/){ gsub(/[YRWSKMDVHBXyrwskmdvhbx]/,"N"); print }else{print $1}}',
...
def run_scan_fasta_sequence_names(job, context, fasta_id, fasta_name, regions=None, regions_regex=None):
    ...
    grep_output = context.runner.call(job, cmd, work_dir=work_dir, check_output=True,
      tool_name='bgzip')
    seq_names = [] if not regions else regions
    for line in grep_output.decode().split('\n'):
        if len(line) > 1:
            name = line.split()[0]
            if name.startswith('>') and (not regions or name[1:] not in regions) and (not regions_regex or re_fullmatch(regions_regex, name[1:])):
...
def run_fix_chrom_names(job, context, to_ucsc, regions, fasta_ids, fasta_names, vcf_ids_list, vcf_names_list, tbi_ids_list, alt_regions_id):
    """
    ...
    if something_to_rename:
        ...
        for vcf_ids, vcf_names, tbi_ids in zip(vcf_ids_list, vcf_names_list, tbi_ids_list):
            ...
            for vcf_id, vcf_name, tbi_id in zip(vcf_ids, vcf_names, tbi_ids):
                vcf_rename_job = job.addChildJobFn(run_fix_vcf_chrom_names, context, vcf_id, vcf_name, tbi_id, name_map_id, cores=(context.config.construct_cores),
                  memory=(context.config.construct_mem),
                  disk=(context.config.construct_disk))
    ...
    if something_to_rename:
        for fasta_id, fasta_name in zip(fasta_ids, fasta_names):
            ...
            with open(os.path.join(work_dir, out_fasta_name), 'w') as (out_fasta_file):
                with open(os.path.join(work_dir, in_fasta_name)) as (in_fasta_file):
    ...
    if alt_regions_id:
        ...
        with open(alt_regions_path) as (in_regions):
            with open(alt_regions_out_path, 'w') as (out_alt_regions):
...
def run_subtract_alt_regions(job, context, alt_regions_id, regions):
    ...
    return (
     [region for region in regions if region not in alt_regions], list(alt_regions))


def run_generate_input_vcfs(job, context, vcf_ids, vcf_names, tbi_ids, regions, output_name, do_primary=False, do_pan=False, pos_control_sample=None, neg_control_sample=None, sample_graph=None, haplo_sample=None, filter_samples=[], min_afs=[], vcf_subdir=None):
    """
    ...
    if pos_control_sample or neg_control_sample:
        ...
        if not not neg_control_sample:
            if not neg_control_sample == control_sample:
                raise AssertionError
        if not not pos_control_sample:
            if not pos_control_sample == control_sample:
                raise AssertionError
        ...
        for vcf_id, vcf_name, tbi_id in zip(vcf_ids, vcf_names, tbi_ids):
            make_controls = job.addChildJobFn(run_make_control_vcfs, context, vcf_id, vcf_name, tbi_id, control_sample,
    ...
    if sample_graph:
        ...
        for vcf_id, vcf_name, tbi_id in zip(vcf_ids, vcf_names, tbi_ids):
            make_sample = job.addChildJobFn(run_make_control_vcfs, context, vcf_id, vcf_name, tbi_id, sample_graph, pos_only=True,
              vcf_subdir=vcf_subdir,
              cores=(context.config.construct_cores),
              memory=(context.config.construct_mem),
              disk=(context.config.construct_disk))
    ...
    if haplo_sample:
        ...
        for vcf_id, vcf_name, tbi_id in zip(vcf_ids, vcf_names, tbi_ids):
            make_controls = job.addChildJobFn(run_make_control_vcfs, context, vcf_id, vcf_name, tbi_id, haplo_sample, pos_only=True,
              vcf_subdir=vcf_subdir,
              cores=(context.config.construct_cores),
              memory=(context.config.construct_mem),
              disk=(context.config.construct_disk))
    ...
    if filter_samples:
        ...
        filter_vcf_names = []
        for vcf_id, vcf_name, tbi_id in zip(vcf_ids, vcf_names, tbi_ids):
            filter_job = job.addChildJobFn(run_filter_vcf_samples, context, vcf_id, vcf_name, tbi_id, filter_samples,
              vcf_subdir=vcf_subdir,
              cores=(context.config.construct_cores),
              memory=(context.config.construct_mem),
              disk=(context.config.construct_disk))
    ...
    for min_af in min_afs:
        ...
        af_vcf_names = []
        for vcf_id, vcf_name, tbi_id in zip(vcf_ids, vcf_names, tbi_ids):
            af_job = job.addChildJobFn(run_min_allele_filter_vcf_samples, context, vcf_id, vcf_name, tbi_id, min_af,
              vcf_subdir=vcf_subdir,
              cores=(context.config.construct_cores),
              memory=(context.config.construct_mem),
              disk=(context.config.construct_disk))
...
def run_construct_all(job, context, fasta_ids, fasta_names, vcf_inputs, max_node_size, alt_paths, flat_alts, handle_svs, regions, merge_graphs=False, sort_ids=False, join_ids=False, wanted_indexes=set(), haplo_extraction_sample=None, haplotypes=[0, 1], gbwt_prune=False, normalize=False, validate=False, alt_regions_id=None, alt_regions=[]):
    """ 
    ...
    for name, (vcf_ids, vcf_names, tbi_ids, output_name, region_names) in list(vcf_inputs.items()):
        ...
        construct_job = job.addChildJobFn(run_construct_genome_graph, context, fasta_ids, fasta_names, vcf_ids, vcf_names, tbi_ids, max_node_size, 'gbwt' in wanted_indexes or haplo_extraction or alt_paths, flat_alts, handle_svs, regions, region_names, sort_ids, join_ids, name, merge_output_name, normalize and name != 'haplo', validate, alt_regions_id)
            ...
            for vcf_id, tbi_id in zip(vcf_ids, tbi_ids):
                if vcf_id:
                    if tbi_id:
                        ...
                        if not (vcf_id == None and tbi_id == None):
                            raise AssertionError

        index_prev_job = construct_job
        if haplo_extraction:
            haplo_index_job = construct_job.addFollowOnJobFn(run_make_haplo_indexes, context, input_vcf_ids,
              input_tbi_ids, vcf_names,
              joined_vg_ids, joined_vg_names, output_name_base,
              regions, haplo_extraction_sample, intermediate=merge_graphs)
            ...
            if name == 'sample-graph':
                ...
                join_job = sample_job.addFollowOnJobFn(run_join_graphs, context, (sample_job.rv()), False,
                  region_names, name, sample_merge_output_name, cores=(context.config.construct_cores),
                  memory=(context.config.construct_mem),
                  disk=(context.config.construct_disk))
                if len(regions) > 1:
                    if 'xg' in wanted_indexes:
                        wanted = set('xg')
                        construct_job.addFollowOnJobFn(run_indexing, context, joined_vg_ids, joined_vg_names,
                          output_name_base, chroms, [], [], wanted=wanted)
                ...
                haplo_job = haplo_index_job.addFollowOnJobFn(run_make_haplo_graphs, context, joined_vg_ids,
                  joined_vg_names, haplo_xg_ids, output_name_base,
                  regions, haplo_extraction_sample,
                  haplotypes, gbwt_ids, intermediate=merge_graphs)
                for haplotype in haplotypes:
                    haplo_xg_job = haplo_job.addFollowOnJobFn(run_xg_indexing, context, (haplo_job.rv(haplotype)), joined_vg_names,
        ...
        if haplo_extraction:
            ...
            wanted.discard('gbwt')
        indexing_job = index_prev_job.addFollowOnJobFn(run_indexing, context, joined_vg_ids, joined_vg_names,
          output_name_base, chroms, (input_vcf_ids if 'gbwt' in wanted else []),
...
def run_construct_genome_graph(job, context, fasta_ids, fasta_names, vcf_ids, vcf_names, tbi_ids, max_node_size, alt_paths, flat_alts, handle_svs, regions, region_names, sort_ids, join_ids, name, merge_output_name, normalize, validate, alt_regions_id):
    """
    ...
    for i, (region, region_name) in enumerate(zip(regions, region_names)):
        ...
        construct_region_job = child_job.addChildJobFn(run_construct_region_graph, context, fasta_id,
          fasta_name, vcf_id,
          vcf_name, tbi_id, region, region_name, max_node_size,
          alt_paths, flat_alts, handle_svs, is_chrom=(not region or ':' not in region),
          sort_ids=sort_ids,
          normalize=normalize,
          validate=validate,
          cores=(context.config.construct_cores),
          ...
          disk=(context.config.construct_disk))
        if alt_regions_id:
            region_graph_ids.append(construct_region_job.addFollowOnJobFn(run_msga, context, (region_name + '.vg'), (construct_region_job.rv()),
              fasta_id,
              alt_regions_id,
              region,
              normalize=normalize,
    ...
    return child_job.addFollowOnJobFn(run_join_graphs, context, region_graph_ids, join_ids, region_names,
      name, merge_output_name, cores=(context.config.construct_cores),
      memory=(context.config.construct_mem),
...
def run_join_graphs(job, context, region_graph_ids, join_ids, region_names, name, merge_output_name=None):
    ...
    if merge_output_name:
        ...
        assert merge_output_name is not None and merge_output_name not in region_files
        cmd = [
         'vg', 'combine'] + region_files
...
def run_construct_region_graph(job, context, fasta_id, fasta_name, vcf_id, vcf_name, tbi_id, region, region_name, max_node_size, alt_paths, flat_alts, handle_svs, is_chrom=False, sort_ids=True, normalize=False, validate=False):
    """
...
def run_make_control_vcfs(job, context, vcf_id, vcf_name, tbi_id, sample, pos_only=False, vcf_subdir=None, no_filter_if_sample_not_found=False):
    ...
    if not sample is not None:
        ...
        if not found_sample:
            if no_filter_if_sample_not_found:
                return (
                 vcf_id, tbi_id, vcf_id, tbi_id)
...
def run_make_haplo_indexes(job, context, vcf_ids, tbi_ids, vcf_names, vg_ids, vg_names, output_name, regions, sample, intermediate=False):
    """
    ...
    if not sample is not None:
        ...
        if not len(vcf_ids) == 1:
            if not len(vcf_ids) <= len(regions):
                raise AssertionError
    ...
    for i, (vg_id, vg_name, region) in enumerate(zip(vg_ids, vg_names, chroms)):
        ...
        xg_job = job.addChildJobFn(run_xg_indexing, context, [vg_id], [vg_name], xg_name,
          vcf_id, tbi_id, make_gbwt=True,
...
def run_make_haplo_graphs(job, context, vg_ids, vg_names, xg_ids, output_name, regions, sample, haplotypes, gbwt_ids, intermediate=False):
    """
    ...
    for i, (vg_id, vg_name, region, xg_id) in enumerate(zip(vg_ids, vg_names, chroms, xg_ids)):
        if not not gbwt_ids:
            if not len(gbwt_ids) in [1, len(xg_ids)]:
                raise AssertionError
        gbwt_id = None if not gbwt_ids else gbwt_ids[0] if len(gbwt_ids) == 1 else gbwt_ids[i]
        hap_job = job.addChildJobFn(run_make_haplo_thread_graphs, context, vg_id, vg_name, output_name,
          [region], xg_id, sample, haplotypes, gbwt_id, intermediate=intermediate,
          cores=(context.config.construct_cores),
          memory=(context.config.construct_mem),
          disk=(context.config.construct_disk))
...
def run_make_haplo_thread_graphs(job, context, vg_id, vg_name, output_name, chroms, xg_id, sample, haplotypes, gbwt_id, intermediate=False):
    """
    ...
    if gbwt_id:
        ...
        try:
            ...
            RealtimeLogger.warning('No GBWT threads found in {}.  Using reference path for haplotype extraction'.format(os.path.basename(gbwt_path)))
            thread_count = 0
    ...
    for hap in haplotypes:
        ...
        try:
            ...
            if thread_count == 0:
                ...
                cmd = [
                 'vg', 'paths', '-d', '-v', os.path.basename(vg_path)]
...
def run_make_sample_graphs(job, context, vg_ids, vg_names, xg_ids, output_name, regions, sample, gbwt_ids):
    ...
    for i, (vg_id, vg_name, region, xg_id) in enumerate(zip(vg_ids, vg_names, chroms, xg_ids)):
        ...
        hap_job = job.addChildJobFn(run_make_sample_region_graph, context, vg_id, vg_name, output_name,
          region, xg_id, sample, [0, 1], gbwt_id, cores=(context.config.construct_cores),
          memory=(context.config.construct_mem),
          disk=(context.config.construct_disk))
    ...
    with context.get_toil(options.jobStore) as (toil):
        if not toil.options.restart:
            ...
            for vcf_batch in options.vcf:
                ...
                inputTBIFileIDs.append([importer.load((make_url(vcf + '.tbi')), wait_on=(inputVCFFileIDs[(-1)][i])) for i, vcf in enumerate(vcf_batch.split(','))])
            ...
            for i, fasta in enumerate(options.fasta):
                if fasta.endswith('.gz'):
                    inputFastaFileIDs[i] = init_job.addChildJobFn(run_unzip_fasta, context, (inputFastaFileIDs[i]), (os.path.basename(fasta)),
                      disk=(context.config.construct_disk)).rv()
            ...
            if options.mask_ambiguous:
                ...
                for i, (fasta_id, fasta_name) in enumerate(zip(inputFastaFileIDs, inputFastaNames)):
                    inputFastaFileIDs[i] = mask_root.addChildJobFn(run_mask_ambiguous, context, (inputFastaFileIDs[i]), (inputFastaNames[i]), disk=(context.config.construct_disk)).rv(0)
            ...
            if options.pre_min_af:
                ...
                for vcf_ids, vcf_names, tbi_ids in zip(inputVCFFileIDs, inputVCFNames, inputTBIFileIDs):
                    af_vcf_ids, af_tbi_ids = [], []
                    for vcf_id, vcf_name, tbi_id in zip(vcf_ids, vcf_names, tbi_ids):
                        af_job = min_af_job.addChildJobFn(run_min_allele_filter_vcf_samples, context, vcf_id, vcf_name,
                          tbi_id, (options.pre_min_af), cores=(context.config.construct_cores),
                          memory=(context.config.construct_mem),
                          disk=(context.config.construct_disk))
                        af_vcf_ids.append(af_job.rv(0))
                        af_tbi_ids.append(af_job.rv(1))
            ...
            if options.regions_file:
                cur_job = cur_job.addFollowOnJobFn(run_scan_regions_file, context, inputRegionsFileID, regions_regex, memory=(context.config.misc_mem),
                  disk=(context.config.misc_disk))
                regions = cur_job.rv()
            else:
                if options.fasta_regions:
                    cur_job = cur_job.addFollowOnJobFn(run_scan_fasta_sequence_names, context, (inputFastaFileIDs[0]),
                      (inputFastaNames[0]),
                      (options.regions),
                      regions_regex,
                ...
                if options.add_chr_prefix or options.remove_chr_prefix:
                    cur_job = cur_job.addFollowOnJobFn(run_fix_chrom_names, context, (options.add_chr_prefix),
                      regions,
                      inputFastaFileIDs,
                      inputFastaNames,
                      inputVCFFileIDs,
                ...
                if alt_regions_id:
                    cur_job = cur_job.addFollowOnJobFn(run_subtract_alt_regions, context, alt_regions_id, regions)
                    regions, alt_regions = cur_job.rv(0), cur_job.rv(1)
                else:
                    alt_regions = []
            cur_job = cur_job.addFollowOnJobFn(run_merge_all_vcfs, context, inputVCFFileIDs, inputVCFNames, inputTBIFileIDs)
            inputVCFFileIDs = cur_job.rv(0)
            inputVCFNames = cur_job.rv(1)
            inputTBIFileIDs = cur_job.rv(2)
            vcf_job = cur_job.addFollowOnJobFn(run_generate_input_vcfs, context, inputVCFFileIDs,
              inputVCFNames, inputTBIFileIDs, regions,
              (options.out_name),
            ...
            vcf_job.addFollowOnJobFn(run_construct_all, context, inputFastaFileIDs, inputFastaNames,
              (vcf_job.rv()), (options.max_node_size),
              (options.alt_paths or 'alt-gam' in options.indexes), (options.flat_alts),
              (options.handle_svs), regions, merge_graphs=(options.merge_graphs),
              sort_ids=True,
              join_ids=True,
            ...
            if inputBWAFastaID:
                init_job.addFollowOnJobFn(run_bwa_index, context, inputBWAFastaID, copy_fasta=True,
