...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_vg/vg_chunk.py
...
def chunk_subparser(parser):
    ...
    parser.add_argument('--gam', type=make_url, help='GAM to chunk')
    parser.add_argument('--graph', type=make_url, required=True, help='graph or xg index to chunk')
...
def chunk_parse_args(parser, path_components=True):
    ...
    """
    parser.add_argument('--connected_component_chunking', action='store_true', help='split into connected components')
    parser.add_argument('--ref_path_chunking', action='store_true', help='chunk on --ref_paths if specified, all paths otherwise')
    parser.add_argument('--ref_paths', nargs='+', default=[], help='reference paths to call (and chunk) on')
    parser.add_argument('--output_format', choices=['pg', 'hg', 'vg'], default='pg', help='output format [pg]')
    parser.add_argument('--chunk_cores', type=int, help='number of threads used for extracting chunks for calling')
    parser.add_argument('--chunk_mem', type=str, help='memory alotment for extracting chunks for calling')


def validate_chunk_options(options, chunk_optional=False):
    num_opts = [
     options.connected_component_chunking, options.ref_path_chunking].count(True)
    if chunk_optional == False:
        require(num_opts == 1, 'Must specify (exactly) one of --connected_component_chunking, ref_path_chunking')
        if options.ref_paths:
            require(options.ref_path_chunking == True, 'Must specify --ref_path_chunking when using --ref_paths')
    else:
        require(num_opts in (0, 1), 'Must specify at most one of --connected_component_chunking or --ref_path_chunking')
    if options.gam:
        require(options.gam.endswith('.gam'), 'Input GAM file must have .gam extension')


def run_chunking(job, context, graph_id, graph_basename, chunk_paths, connected_component_chunking, output_format, gam_id=None, to_outstore=False):
    ...
    try:
        context.runner.call(job, (['vg', 'chunk'] + gen_opts + input_opts + output_opts), work_dir=work_dir)
...
def chunk_main(context, options):
    ...
    with context.get_toil(options.jobStore) as (toil):
        if not toil.options.restart:
            ...
            importer.wait()
            root_job = Job.wrapJobFn(run_chunking, context, (importer.resolve(inputGraphFileID)),
