...
# Size of source mod 2**32: 27308 bytes
"""
...
def call_subparser(parser):
    ...
    parser.add_argument('--gam', type=make_url, required=True, help='GAM to call')
    parser.add_argument('--graph', type=make_url, required=True, help='graph to augment')
    parser.add_argument('--snarls', type=make_url, help='Path to snarls file')
...
def call_parse_args(parser):
    ...
    parser.add_argument('--sample', type=str, help='sample name for output VCF')
    parser.add_argument('--genotype_vcf', type=make_url, help='genotype the given VCF.  Input graph must contain alt paths from VCF')
    parser.add_argument('--recall', action='store_true', help='do not augment. only recall variants in graph. (on by default with --genotype_vcf)')
    ...
    parser.add_argument('--min_call_support', type=float, help='minimum support to make a call')
    parser.add_argument('--calling_cores', type=int, help='number of threads during the variant calling step')
    parser.add_argument('--calling_mem', type=str, help='memory alotment during the variant calling step')
    parser.add_argument('--gam_chunking', action='store_true', help='split the GAM into chromosome components (in addition to the graph)')


def validate_call_options(options):
    require(not options.vcf_offsets or len(options.vcf_offsets) == len(options.ref_paths), 'Number of --vcf_offsets if specified must be same as number of --ref_paths')
    require(not options.snarls or options.recall or options.genotype_vcf, '--snarls can only be used with --recall or --genotype_vcf')
    require(not options.connected_component_chunking or not options.genotype_vcf, '--only path chunking (not --connected_component_chunking) supported with --genotype_vcf')
    require(not options.connected_component_chunking or not options.ref_path_chunking, '--connected_components cannot be used with --ref_path_chunking')
...
def run_chunked_calling(job, context, graph_id, graph_basename, gam_id, gam_basename, batch_input=None, snarls_id=None, genotype_vcf_id=None, genotype_tbi_id=None, sample=None, augment=False, connected_component_chunking=False, output_format=None, min_augment_coverage=None, expected_coverage=None, min_mapq=None, min_baseq=None, ref_paths=[], ref_path_chunking=True, min_call_support=None, vcf_offsets={}, gam_chunking=False):
    ...
    if sample:
        ...
        if batch_input is None:
            if connected_component_chunking or ref_path_chunking:
                chunk_job = child_job.addChildJobFn(run_chunking, context, graph_id=graph_id,
                ...
                recurse_job = child_job.addFollowOnJobFn(run_chunked_calling, context, graph_id=None,
                  graph_basename=graph_basename,
                  gam_id=gam_id,
                ...
                if augment and os.path.splitext(graph_basename)[1] != '.' + output_format:
                    convert_job = child_job.addChildJobFn(run_convert, context, graph_id=graph_id,
                      graph_basename=graph_basename,
                      output_format=output_format,
    ...
    for chunk_name, chunk_results in list(batch_input.items()):
        ...
        if augment:
            augment_job = calling_root_job.addChildJobFn(run_augmenting, context, graph_id=graph_id,
              graph_basename=graph_basename,
              gam_id=gam_id,
              gam_basename=gam_basename,
              augment_gam=True,
        ...
        if ref_path_chunking:
            ...
            ref_path = ref_paths
        calling_job = calling_root_job.addFollowOnJobFn(run_calling, context, graph_id=graph_id,
          graph_basename=graph_basename,
          gam_id=gam_id,
          gam_basename=gam_basename,
          snarls_id=snarls_id,
    ...
    concat_job = child_job.addFollowOnJobFn(run_concat_vcfs, context, out_name=out_vcf_name,
      vcf_ids=None,
      tbi_ids=None,
...
def run_calling(job, context, graph_id, graph_basename, gam_id, gam_basename, snarls_id=None, genotype_vcf_id=None, genotype_tbi_id=None, sample=None, expected_coverage=None, min_mapq=None, ref_paths=None, min_call_support=None, vcf_offsets=None, to_outstore=True):
    """
        ...
        if ref_paths and len(ref_paths) == 1:
            ...
            with open(sub_vcf_path, 'wb') as (sub_vcf_file):
                context.runner.call(job, ['bcftools', 'view', os.path.basename(genotype_vcf_path), '-r',
                 ref_paths[0], '-O', 'z'],
                  work_dir=work_dir, outfile=sub_vcf_file)
    ...
    pack_cmd = [
     'vg', 'pack', '-x', os.path.basename(graph_path), '-g', os.path.basename(gam_path),
    ...
    out_vcf_path += '.vcf.gz'
    call_cmd = [
     'vg', 'call', os.path.basename(graph_path), '-k', os.path.basename(pack_path), '-t', str(job.cores)]
    if genotype_vcf_id:
...
def run_convert(job, context, graph_id, graph_basename, output_format):
    """ convert a graph """
    work_dir = job.fileStore.getLocalTempDir()
    graph_path = os.path.join(work_dir, graph_basename)
    job.fileStore.readGlobalFile(graph_id, graph_path)
    ...
    with open(out_graph_path, 'wb') as (out_graph_file):
        convert_cmd = [
         'vg', 'convert', os.path.basename(graph_path)]
...
def run_concat_vcfs(job, context, out_name, vcf_ids, tbi_ids=[], write_to_outstore=False, call_timers_lists=[], batch_data=None):
    ...
    if batch_data:
        ...
        command = [
         'bcftools', 'concat'] + [os.path.basename(vcf_path) for vcf_path in vcf_paths]
        command += ['--output', os.path.basename(out_file), '--output-type', 'z']
        context.runner.call(job, command, work_dir=work_dir)
        context.runner.call(job, ['tabix', '--force', '--preset', 'vcf', os.path.basename(out_file)], work_dir=work_dir)
...
def run_filtering(job, context, graph_id, graph_basename, gam_id, gam_basename, filter_opts):
    work_dir = job.fileStore.getLocalTempDir()
    if not filter_opts:
        return gam_id
    else:
        ...
        with open(filter_path, 'wb') as (filter_file):
            context.runner.call(job, (['vg', 'filter', os.path.basename(gam_path)] + filter_opts), work_dir=work_dir,
              outfile=filter_file)
    ...
    with context.get_toil(options.jobStore) as (toil):
        if not toil.options.restart:
            ...
            if context.config.filter_opts:
                root_job = Job.wrapJobFn(run_filtering, context, graph_id=(importer.resolve(inputGraphFileID)),
            ...
            calling_job = Job.wrapJobFn(run_chunked_calling, context, graph_id=(importer.resolve(inputGraphFileID)),
              graph_basename=(os.path.basename(options.graph)),
