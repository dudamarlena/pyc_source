...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_vg/vg_augment.py
# Compiled at: 2020-04-30 13:47:21
# Size of source mod 2**32: 12694 bytes
"""
...
def augment_subparser(parser):
    ...
    parser.add_argument('--gam', type=make_url, required=True, help='GAM to augment')
...
def augment_parse_args(parser, stand_alone=False):
    ...
    """
    if stand_alone:
        parser.add_argument('--augment_gam', action='store_true', help='produce and augmented GAM')
    parser.add_argument('--min_augment_coverage', type=int, help='minimum coverage for breakpoint to be applied')
    parser.add_argument('--expected_coverage', type=int, help='expected coverage.  only affects memory usage.  use if coverage >> 100')
    parser.add_argument('--min_mapq', type=int, help='ignore reads with MAPQ less than this')
    parser.add_argument('--min_baseq', type=int, help='ignore edits with minimum average base quality less than this')
    parser.add_argument('--augment_cores', type=int, help='number of threads during augmentation')
    parser.add_argument('--augment_mem', type=str, help='memory alotment during augmentation')


def run_chunked_augmenting(job, context, graph_id, graph_basename, gam_id, gam_basename, batch_input=None, all_path_components=False, chunk_paths=[], connected_component_chunking=False, output_format=None, augment_gam=False, min_augment_coverage=None, expected_coverage=None, min_mapq=None, min_baseq=None, to_outstore=False):
    """
    ...
    if batch_input is None:
        if all_path_components or connected_component_chunking or len(chunk_paths) > 1:
            ...
            recurse_job = child_job.addFollowOnJobFn(run_chunked_augmenting, context, graph_id=None,
              graph_basename=None,
              gam_id=None,
              gam_basename=None,
    ...
    for chunk_name, chunk_results in list(batch_input.items()):
        augment_job = job.addChildJobFn(run_augmenting, context, graph_id=(chunk_results[0]),
...
def run_augmenting(job, context, graph_id, graph_basename, gam_id, gam_basename, augment_gam=False, min_augment_coverage=None, expected_coverage=None, min_mapq=None, min_baseq=None, to_outstore=False):
    """
    ...
    augment_cmd = [
     'vg', 'augment', os.path.basename(graph_path), os.path.basename(gam_path), '-t', str(job.cores)]
...
def augment_main(context, options):
    ...
    with context.get_toil(options.jobStore) as (toil):
        if not toil.options.restart:
            ...
            importer.wait()
            root_job = Job.wrapJobFn(run_chunked_augmenting, context, graph_id=(importer.resolve(inputGraphFileID)),
