...
# Size of source mod 2**32: 38862 bytes
"""
...
def map_parse_index_args(parser):
    ...
    parser.add_argument('--gcsa_index', type=make_url, help='Path to GCSA index (for map and mpmap)')
    parser.add_argument('--minimizer_index', type=make_url, help='Path to minimizer index (for gaffe)')
    parser.add_argument('--distance_index', type=make_url, help='Path to distance index (for gaffe)')
    ...
    parser.add_argument('--mapper', default='map', choices=['map', 'mpmap', 'gaffe'], help='vg mapper to use')
...
def map_parse_args(parser, stand_alone=False):
    ...
    parser.add_argument('--fq_split_cores', type=int, help='number of threads used to split input FASTQs')
    ...
    parser.add_argument('--interleaved', action='store_true', default=False, help='treat fastq as interleaved read pairs. overrides *_opts')
    ...
    parser.add_argument('--gaffe_opts', type=str, help='arguments for vg gaffe (wrapped in "")')
    ...
    parser.add_argument('--validate', action='store_true', help='run vg validate on ouput GAMs')
    parser.add_argument('--id_ranges', type=make_url, default=None, help='Path to file with node id ranges for each chromosome in BED format.')
...
def validate_map_options(context, options):
    ...
    require(options.mapper == 'mpmap' or options.snarls_index is None, '--snarls_index can only be used with --mapper mpmap')
    if options.mapper == 'mpmap':
        require('-S' in context.config.mpmap_opts or '--single-path-mode' in context.config.mpmap_opts, '-S must be used with mpmap mapper to produce GAM output')
        require(not options.bam_output, '--bam_output not currently supported with mpmap mapper')
...
def run_split_reads_if_needed(job, context, fastq, gam_input_reads, bam_input_reads, reads_file_ids):
    ...
    if not context.config.single_reads_chunk:
        reads_chunk_ids = job.addChildJobFn(run_split_reads, context, fastq, gam_input_reads, bam_input_reads, reads_file_ids,
...
def run_mapping(job, context, fastq, gam_input_reads, bam_input_reads, sample_name, interleaved, mapper, indexes, reads_file_ids=None, reads_chunk_ids=None, bam_output=False, surject=False, gbwt_penalty=None, validate=False):
    """
    ...
    """
    if not bool(fastq) + bool(gam_input_reads) + bool(bam_input_reads) == 1:
        raise AssertionError
    else:
        assert bool(reads_file_ids) + bool(reads_chunk_ids) == 1
        chunk_job = None
        if reads_chunk_ids is None:
            chunk_job = job.addChildJobFn(run_split_reads_if_needed, context, fastq, gam_input_reads, bam_input_reads, reads_file_ids,
              cores=(context.config.misc_cores), memory=(context.config.misc_mem), disk=(context.config.misc_disk))
            reads_chunk_ids = chunk_job.rv()
        align_job = Job.wrapJobFn(run_whole_alignment, context, fastq, gam_input_reads, bam_input_reads, sample_name, interleaved,
          mapper, indexes, reads_chunk_ids, bam_output=bam_output,
          surject=surject,
    ...
    if fastq:
        if len(fastq):
            for fastq_i, reads_file_id in enumerate(reads_file_ids):
                reads_chunk_ids.append(job.addChildJobFn(run_split_fastq, context, fastq, fastq_i, reads_file_id, cores=(context.config.fq_split_cores),
                  memory=(context.config.fq_split_mem),
                  disk=(context.config.fq_split_disk)).rv())

        else:
            assert gam_input_reads and len(reads_file_ids) == 1
            reads_chunk_ids.append(job.addChildJobFn(run_split_gam_reads, context, gam_input_reads, (reads_file_ids[0]), cores=(context.config.fq_split_cores),
              memory=(context.config.fq_split_mem),
              disk=(context.config.fq_split_disk)).rv())
    else:
        if not bam_input_reads:
            raise AssertionError
        elif not len(reads_file_ids) == 1:
            raise AssertionError
        reads_chunk_ids.append(job.addChildJobFn(run_split_bam_reads, context, bam_input_reads, (reads_file_ids[0]), cores=(context.config.fq_split_cores),
          memory=(context.config.fq_split_mem),
          disk=(context.config.fq_split_disk)).rv())
    return reads_chunk_ids
...
def run_split_bam_reads(job, context, bam_input_reads, bam_reads_file_id):
    ...
    cmd = [
     [
      'samtools', 'view', os.path.basename(bam_path)]]
    cmd.append(['split', '-l', str(chunk_lines),
     "--filter=bash -c 'cat <(samtools view -H {}) <(cat -)".format(os.path.basename(bam_path)) + ' | samtools view -O BAM --threads {} -'.format(max(1, int(context.config.fq_split_cores) - 1)) + " > $FILE.bam'", '-', 'bam_reads_chunk.'])
    context.runner.call(job, cmd, work_dir=work_dir)
    bam_chunk_ids = []
    for chunk_name in sorted(os.listdir(work_dir)):
...
def run_whole_alignment(job, context, fastq, gam_input_reads, bam_input_reads, sample_name, interleaved, mapper, indexes, reads_chunk_ids, bam_output=False, surject=False, gbwt_penalty=None, validate=False):
    """
    ...
    for chunk_id, chunk_filename_ids in enumerate(zip(*reads_chunk_ids)):
        chunk_alignment_job = child_job.addChildJobFn(run_chunk_alignment, context, gam_input_reads, bam_input_reads, sample_name,
          interleaved,
          mapper, chunk_filename_ids, chunk_id, indexes,
    ...
    if not bam_output:
        merge_gams_job = child_job.addFollowOnJobFn(run_merge_gams, context, sample_name, (indexes.get('id_ranges')), gam_chunk_file_ids, gam_chunk_running_times,
          cores=(context.config.misc_cores),
          memory=(context.config.misc_mem),
          disk=(context.config.misc_disk))
...
def run_chunk_alignment(job, context, gam_input_reads, bam_input_reads, sample_name, interleaved, mapper, chunk_filename_ids, chunk_id, indexes, bam_output=False, gbwt_penalty=None, always_check_population=True, validate=False):
    """
    ...
    with open(output_file, 'wb') as (alignment_file):
        ...
        if mapper == 'mpmap':
            ...
            if '-S' not in vg_parts:
                if '--single-path-mode' not in vg_parts:
            ...
            if mapper == 'map':
                ...
                if interleaved is True:
                    if '-i' not in vg_parts:
                        if '--interleaved' not in vg_parts:
                            vg_parts += ['-i']
                if interleaved is False:
                    if 'i' in vg_parts:
                        del vg_parts[vg_parts.index('-i')]
                if interleaved is False:
                    if '--interleaved' in vg_parts:
                        del vg_parts[vg_parts.index('--interleaved')]
                if bam_output is True:
                    if '--surject-to' not in vg_parts:
                        vg_parts += ['--surject-to', 'bam']
                if bam_output is False:
                    if '--surject-to' in vg_parts:
                        sidx = vg_parts.index('--surject-to')
                        del vg_parts[sidx]
                        del vg_parts[sidx]
                type_to_option = {'gbwt':'--gbwt-name', 
                 'xg':'-x', 
                 'gcsa':'-g', 
                 'lcp':None, 
                 'distance':'-d', 
                ...
                try:
                    ...
                    if validate:
                        ...
                        context.runner.call(job, ['vg', 'validate', '--xg', os.path.basename(index_files['xg']),
                         '--gam', os.path.basename(output_file)],
                          work_dir=work_dir)
                except:
                    end_time = timeit.default_timer()
    ...
    RealtimeLogger.info('Aligned {}. Process took {} seconds with {} vg-{}'.format(output_file, run_time, 'paired-end' if paired_end else 'single-end', mapper))
...
def run_merge_gams(job, context, sample_name, id_ranges_file_id, gam_chunk_file_ids, gam_chunk_running_times):
    ...
    for i, chr in enumerate(chroms):
        ...
        chr_gam_id = job.addChildJobFn(run_merge_chrom_gam, context, sample_name, chr, shard_ids, cores=(context.config.fq_split_cores),
          memory=(context.config.fq_split_mem),
          disk=(context.config.fq_split_disk)).rv()
...
def map_main(context, options):
    ...
    with context.get_toil(options.jobStore) as (toil):
        if not toil.options.restart:
            ...
            importer.wait()
            root_job = Job.wrapJobFn(run_mapping, context, (options.fastq), (options.gam_input_reads),
              (options.bam_input_reads), (options.sample_name),
              (options.interleaved),
              (options.mapper), (importer.resolve(indexes)), reads_file_ids=(importer.resolve(inputReadsFileIDs)),
