...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_vg/test/test_vg.py
# Compiled at: 2020-04-30 13:47:21
# Size of source mod 2**32: 45737 bytes
import logging, shlex, shutil, subprocess, tempfile, textwrap, filecmp, shutil
from contextlib import closing
from unittest import TestCase, skip
from urllib.parse import urlparse
from uuid import uuid4
import gzip, urllib.request, os, sys, posixpath, pytest
...
class VGCGLTest(TestCase):
    ...
    def test_07_BRCA1_BRCA2_NA12877(self):
        ...
        self._run(self.base_command + [
         ...
         '--vcfeval_baseline', self.baseline, '--vcfeval_fasta', self.chrom_fa])
        self._run(['toil', 'clean', self.jobStoreLocal])
        self._assertOutput('NA12877', (self.local_outstore), f1_threshold=0.7)
        ...
        self._run(['toil-vg', 'vcfeval', self.jobStoreLocal, '--clean', 'never',
         ...
         '--vcfeval_opts', ' --ref-overlap'])
        self._run(['toil', 'clean', self.jobStoreLocal])
        self._assertOutput(None, outstore, f1_threshold=0.7)
    ...
    def _test_09_construct(self, container_override):
        ...
        prev_vg_size = None
        prev_vcf_size = None
        for ext in ('', '_filter', '_minus_HG00096', '_HG00096_sample_withref', '_HG00096_haplo',
                    '_minaf_0.6'):
            ...
            elif not os.path.isfile(vcf_file + '.tbi'):
    ...
    def _assertOutput(self, sample_name, outstore, f1_threshold=0.9):
    ...
    def _assertCallEvalOutput(self, outstore, names, f1_threshold, happy_snp_f1_threshold=-1, happy_indel_f1_threshold=-1):
    ...
    def _assertXREF(self, sample, outstore):
        ...
        with gzip.open(os.path.join(outstore, '{}.vcf.gz'.format(sample)), 'rb') as (vcf_file):
            for line in vcf_file:
                if line.strip() and line.strip()[0] != '#' and len(line.split()) > 4 and line.split()[4] != '.':
                    var_count += 1
                    if 'XREF' in line:
                        xref_count += 1
