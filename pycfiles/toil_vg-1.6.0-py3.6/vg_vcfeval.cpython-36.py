...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_vg/vg_vcfeval.py
# Compiled at: 2020-04-30 13:47:21
# Size of source mod 2**32: 33939 bytes
"""
...
"""
import argparse, sys, os, os.path, random, subprocess, shutil, itertools, glob, json, time, timeit, errno, vcf
...
def vcfeval_subparser(parser):
    ...
    parser.add_argument('--call_vcf', type=make_url, required=True, help='input vcf (must be bgzipped and have .tbi')
...
def vcfeval_parse_args(parser):
    ...
    parser.add_argument('--vcfeval_baseline', type=make_url, help='Path to baseline VCF file for comparison (must be bgzipped and have .tbi)')
    parser.add_argument('--vcfeval_fasta', type=make_url, help='Path to DNA sequence file, required for vcfeval. Maybe be gzipped')
    parser.add_argument('--vcfeval_bed_regions', type=make_url, help='BED file of regions to consider')
    parser.add_argument('--vcfeval_opts', type=str, help='Additional options for vcfeval (wrapped in "")',
      default=None)
    parser.add_argument('--vcfeval_cores', type=int, default=1,
      help='Cores to use for vcfeval')
    parser.add_argument('--vcfeval_score_field', default=None, help='vcf FORMAT field to use for ROC score.  overrides vcfeval_opts')
    parser.add_argument('--vcfeval', action='store_true', help='run rtg vcfeval comparison.  (will be run by default if no other tool specified)')
    parser.add_argument('--happy', action='store_true', help='run hap.py comparison.')
    parser.add_argument('--sveval', action='store_true', help='run bed-based sv comparison.')
    parser.add_argument('--min_sv_len', type=int, default=20, help='minimum length to consider when doing bed sv comparison (using --sveval)')
    parser.add_argument('--max_sv_len', type=int, default=(sys.maxsize), help='maximum length to consider when doing bed sv comparison (using --sveval)')
    parser.add_argument('--sv_region_overlap', type=float, default=1.0, help='sv must overlap bed region (--vcfeval_bed_regions) by this fraction to be considered')
    parser.add_argument('--sv_overlap', type=float, default=0.5, help='minimum overlap coverage required for bed intersection to count as TP')
    parser.add_argument('--ins_max_gap', type=int, default=20, help='maximum distance between insertions to be compared')
    parser.add_argument('--ins_seq_comp', action='store_true', help='compare insertion sequence instead of their size only.')
    parser.add_argument('--del_min_rol', type=float, default=0.1, help='the minimum reciprocal overlap when computing coverage on deletions')
    parser.add_argument('--check_inv', action='store_true', help='try to identify inversions during SV evaluation.')
    parser.add_argument('--genotype_eval', action='store_true', help='genotype evaluation instead of calling evaluation during SV evaluation.')
    parser.add_argument('--normalize', action='store_true', help='normalize both VCFs before SV comparison with bcftools norm (requires --vcfeva_fasta)')
    parser.add_argument('--normalize_baseline', action='store_true', help='normalize the truth VCF before SV comparison with bcftools norm (requires --vcfeva_fasta)')
    parser.add_argument('--normalize_calls', action='store_true', help='normalize the calls VCFs before SV comparison with bcftools norm (requires --vcfeva_fasta)')
    parser.add_argument('--vcfeval_sample', help='extract this sample from calls and truth vcf (if possible) before comparison')


def validate_vcfeval_options(options):
    """ check some options """
    if not (options.vcfeval_baseline and options.vcfeval_baseline.endswith('.vcf.gz')):
        raise AssertionError
    elif not options.call_vcf.endswith('.vcf.gz'):
        raise AssertionError
    else:
        if not not (options.happy or options.vcfeval):
            assert options.vcfeval_fasta
            if not not options.normalize:
                assert options.vcfeval_fasta
        elif not not options.normalize_calls:
            if not options.vcfeval_fasta:
                raise AssertionError
        elif not not options.normalize_baseline:
            assert options.vcfeval_fasta
        if not options.sveval:
            assert options.vcfeval_fasta
...
def sort_vcf(job, drunner, vcf_path, sorted_vcf_path):
    ...
    with open(sorted_vcf_path, 'wb') as (outfile):
        ...
        drunner.call(job, [['bcftools', 'view', '-H', vcf_name],
         [
          'sort', '-k1,1d', '-k2,2n']],
          outfile=outfile, work_dir=vcf_dir)
...
def run_vcfeval_roc_plot(job, context, roc_table_ids, names=[], kind=None, number=0, title=None, show_scores=False, line_width=2, ps_plot=False):
    """
    ...
    out_ids = []
    roc_cmd = [
     'rtg', 'rocplot', '--svg', os.path.basename(out_roc_path)]
...
def run_vcfeval(job, context, sample, vcf_tbi_id_pair, vcfeval_baseline_id, vcfeval_baseline_tbi_id, fasta_path, fasta_id, bed_id, out_name=None, score_field=None):
    """
    ...
    cmd = [
     'rtg', 'vcfeval', '--calls', call_vcf_name,
    ...
    out_summary_id = context.write_output_file(job, (os.path.join(work_dir, out_tag, 'summary.txt')), out_store_path=('{}_summary.txt'.format(out_tag)))
...
def run_happy(job, context, sample, vcf_tbi_id_pair, vcfeval_baseline_id, vcfeval_baseline_tbi_id, fasta_path, fasta_id, bed_id, fasta_idx_id=None, out_name=None):
    """
    ...
    cmd = [
     'hap.py', vcfeval_baseline_name, call_vcf_name,
    ...
    out_summary_id = context.write_output_file(job, (os.path.join(work_dir, out_name + '.summary.csv')), out_store_path=('{}_summary.csv'.format(out_tag)))
    context.runner.call(job, ['tar', 'czf', out_tag + '.tar.gz', out_tag], work_dir=work_dir)
    out_archive_id = context.write_output_file(job, os.path.join(work_dir, out_tag + '.tar.gz'))
    ...
    context.write_output_file(job, f1_path, out_store_path=('{}_f1.txt'.format(out_tag)))
    return {'parsed_summary':happy_results, 
     'summary':out_summary_id, 
     'archive':out_archive_id}


def run_sv_eval(job, context, sample, vcf_tbi_id_pair, vcfeval_baseline_id, vcfeval_baseline_tbi_id, min_sv_len, max_sv_len, sv_overlap, sv_region_overlap, bed_id=None, ins_ref_len=10, del_min_rol=0.1, ins_seq_comp=False, check_inv=False, genotype_eval=False, out_name='', fasta_path=None, fasta_id=None, normalize_baseline=False, normalize_calls=False):
    ...
    if normalize:
        ...
        for vcf_name, norm_name in norm_inputs:
            ...
            sort_vcf(job, context.runner, os.path.join(work_dir, norm_name), os.path.join(work_dir, norm_name + '_sorted.vcf'))
    ...
    sveval_cmd = 'sveval::svevalOl("{}", "{}"'.format(call_vcf_name, vcfeval_baseline_name)
    ...
    context.runner.call(job, ['tar', 'czf', os.path.basename(tar_dir) + '.tar.gz', os.path.basename(tar_dir)], work_dir=work_dir)
    ...
    with context.get_toil(options.jobStore) as (toil):
        if not toil.options.restart:
            ...
            if options.vcfeval_sample:
                call_sample_job = init_job.addChildJobFn(run_make_control_vcfs, context, call_vcf_id, (os.path.basename(options.call_vcf)),
                  call_tbi_id,
                  (options.vcfeval_sample), pos_only=True, no_filter_if_sample_not_found=True,
                ...
                truth_sample_job = init_job.addChildJobFn(run_make_control_vcfs, context, vcfeval_baseline_id, (os.path.basename(options.vcfeval_baseline)),
                  vcfeval_baseline_tbi_id,
                  (options.vcfeval_sample), pos_only=True, no_filter_if_sample_not_found=True,
                  cores=(context.config.vcfeval_cores),
                  memory=(context.config.vcfeval_mem),
                  disk=(context.config.vcfeval_disk))
            ...
            if options.vcfeval:
                vcfeval_job = Job.wrapJobFn(run_vcfeval, context, None, (
                 call_vcf_id, call_tbi_id),
                  vcfeval_baseline_id,
                  vcfeval_baseline_tbi_id, (options.vcfeval_fasta),
                  fasta_id, bed_id, score_field=(options.vcfeval_score_field),
                  cores=(context.config.vcfeval_cores),
                  memory=(context.config.vcfeval_mem),
                  disk=(context.config.vcfeval_disk))
            ...
            if options.happy:
                happy_job = Job.wrapJobFn(run_happy, context, None, (
                 call_vcf_id, call_tbi_id),
                  vcfeval_baseline_id,
                  vcfeval_baseline_tbi_id, (options.vcfeval_fasta),
                  fasta_id, bed_id, cores=(context.config.vcfeval_cores),
                  memory=(context.config.vcfeval_mem),
                  disk=(context.config.vcfeval_disk))
            ...
            if options.sveval:
                sv_job = Job.wrapJobFn(run_sv_eval, context, None, (
                 call_vcf_id, call_tbi_id),
                  vcfeval_baseline_id,
                  vcfeval_baseline_tbi_id, (options.min_sv_len),
                  (options.max_sv_len), (options.sv_overlap),
                  (options.sv_region_overlap), bed_id,
