...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/models/features/basic.py
...
import logging, regex as re, traceback
...
REGEX_EMAIL = re.compile('\\b\\S+@\\S+\\.\\S+\\b', re.IGNORECASE)

def phones(text):
    """Returns list of phone numbers without punctuation."""
    return imap(lambda m: m.group(0).replace('-', ''), REGEX_PHONE.finditer(text))
