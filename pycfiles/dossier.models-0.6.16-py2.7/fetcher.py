...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/models/openquery/fetcher.py
# Compiled at: 2015-07-08 07:34:06
import logging, os, time, dateutil.parser
...
class Fetcher(object):
    ...
    def get(self, url):
        headers = {'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_9_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/38.0.2125.122 Safari/537.36', 
...
class ChunkRoller(object):
    ...
    def close(self):
        if self.o_chunk:
            self.o_chunk.close()
            o_path = os.path.join(self.chunk_dir, '%d-%s.sc.xz' % (len(self.o_chunk), self.o_chunk.md5_hexdigest))
