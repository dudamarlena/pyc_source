...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/models/openquery/google.py
...
"""
import gzip, json, logging, StringIO, sys, time, traceback, urllib, urllib2
...
class Google(object):
    url = 'https://www.googleapis.com/customsearch/v1?key=%(key)s&cx=012511506682437389639:sw8_ywndm-g&q=%(query)s&num=%(num)s&safe=off&start=%(start)s'
    ...
    def web_search(self, query, start=0, limit=100, max_tries=3):
        ...
        """
        tries = 0
        url = self.url % dict(key=self.api_key, query=urllib.quote(query.strip()), num=min(10, limit - start), start=start)
