...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/models/web/routes.py
...
import os.path as path, urllib
from bs4 import BeautifulSoup
import bottle, cbor, json
...
import kvlayer, rejester
from rejester._task_master import AVAILABLE, BLOCKED, PENDING, FINISHED
import yakonfig, operator
app = bottle.Bottle()
...
def v1_folder_report(request, response, kvlclient, store, fid):
    response.headers['Content-Disposition'] = 'attachment; filename="report-%s.xlsx"' % fid
...
def rejester_run_extract(work_unit):
    if 'config' not in work_unit.spec:
        raise rejester.exceptions.ProgrammerError('could not run profile import without global config')
    web_conf = Config()
    unitconf = work_unit.spec['config']
    with yakonfig.defaulted_config([rejester, kvlayer, dblogger, web_conf], config=unitconf):
        fid, sid = cbor.loads(work_unit.key)
        tfidf = web_conf.tfidf
        folders = Folders(web_conf.kvlclient)
        fetcher = Fetcher()
        queries = extract_keyword_queries(web_conf.store, web_conf.label_store, folders, fid, sid)
        ...
        for link, queries in link2queries.items():
            ...
            if fc is not None and any(k.startswith('subtopic|') for k in fc.iterkeys()):
                logger.info('skipping ingest for %r (abs url: %r) because an FC with user data already exists.', cid, link)
                continue
            other_features = {'keywords': StringCounter(list(queries))}
            try:
                fc = create_fc_from_html(link, si.body.raw, encoding=si.body.encoding or 'utf-8', tfidf=tfidf, other_features=other_features)
                logger.info('created FC for %r (abs url: %r)', cid, link)
                web_conf.store.put([(cid, fc)])
            except Exception:
                logger.info('failed ingest on %r (abs url: %r)', cid, link, exc_info=True)
...
def extract_keyword_queries(store, label_store, folders, fid, sid):
    ...
    negative_ids = imap(itemgetter(0), negative_subfolder_ids(label_store, folders, fid, sid))
    negative_fcs = map(itemgetter(1), store.get_many(negative_ids))
    pos_words, neg_words = extract(positive_fcs, negative_fcs, features=[
     'GPE'])
    ...
    query_plus_pred = original_query + ' ' + name_filter(pos_words, query_names)
    logger.info('query 1: + most predictive %s', query_plus_pred)
    queries.append(query_plus_pred)
    query_min_least = original_query + ' -' + name_filter(reversed(pos_words), query_names)
    logger.info('query 2: - least predictive %s', query_min_least)
    queries.append(query_min_least)
    query_min_most_neg = original_query + ' -' + name_filter(neg_words, query_names)
    logger.info('query 3: - most predictive for neg %s', query_min_most_neg)
    queries.append(query_min_most_neg)
    query_plus_least_neg = original_query + ' ' + name_filter(reversed(neg_words), query_names)
...
def name_filter(keywords, names):
    ...
    for key_tuple in keywords:
        if key_tuple[0] not in name_set:
            return '"' + key_tuple[0] + '"'

    return ''


def OLD_v1_folder_extract_post(request, response, kvlclient, store, label_store, fid, sid):
    folders = new_folders(kvlclient, request)
    ids = map(itemgetter(0), folders.items(fid, sid))
    positive_fcs = map(itemgetter(1), store.get_many(ids))
    negative_ids = imap(itemgetter(0), negative_subfolder_ids(label_store, folders, fid, sid))
    ...
    pos_words, neg_words = extract(positive_fcs, negative_fcs, features=[
     'bowNP_sip'])
    return {'postive': dict(pos_words), 'negative': dict(neg_words)}
...
def v1_fc_put(request, response, store, tfidf, cid):
    ...
    if request.headers.get('content-type', '').startswith('text/html'):
        ...
        fc['keywords'] = StringCounter(keywords)
        store.put([(cid, fc)])
        response.status = 201
        return
...
def create_fc_from_html(url, html, encoding='utf-8', tfidf=None, other_features=None):
    ...
    if other_features is None:
        other_features = {}
    other_features.update({'title': StringCounter([title]), 
       'titleBow': StringCounter(title.split())})
