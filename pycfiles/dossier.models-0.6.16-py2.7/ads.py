...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/models/etl/ads.py
# Compiled at: 2015-07-08 07:34:06
from __future__ import absolute_import, division, print_function
from collections import defaultdict
import re, traceback, sys, zlib, cbor, happybase, happybase.hbase.ttypes as ttypes
...
class Ads(ETL):
    ...
    def get_conn(self):
        return happybase.Connection(host=self.host, port=self.port, table_prefix=self.table_prefix)

    def cids_and_fcs(self, mapper, start, end, limit=5):
        return mapper(row_to_content_obj, self.get_artifact_rows(limit=limit, start_key=start, stop_key=end))

    def get_artifact_rows(self, limit=5, start_key=None, stop_key=None):
        while True:
            try:
                scanner = self.get_scanner(limit=limit, start_key=start_key, stop_key=stop_key)
    ...
    def get_scanner(self, limit=5, start_key=None, stop_key=None):
        ...
        return t.scan(row_start=start_key, row_stop=stop_key, limit=limit, batch_size=20)
...
def row_to_content_obj(key_row):
    ...
    try:
        ...
        fc = html_to_fc(response.get('body', ''), url=row.get('url'), timestamp=row.get('timestamp'), other_features=dict(other_bows, **{'artifact_id': artifact_id}))
    except:
        fc = None
...
def unpack_artifact_row(row):
    data = {'url': row['f:url'], 
       ...
       'request': {'method': row['f:request.method'], 
       ...
       'response': {'status': row['f:response.status'], 
                    'server': {'hostname': row['f:response.server.hostname'], 
    ...
    for kk, vv in row.items():
        mm = re.match('^f:index\\.(?P<key>.*)\\.[0-9]+$', kk)
        if mm is not None:
