...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/models/web/run.py
...
import argparse, logging, bottle, dblogger
...
import dossier.web as web, kvlayer, yakonfig
...
def get_application():
    ...
    bottle.debug(True)
    app = web.WebBuilder().set_config(config).enable_cors().inject('tfidf', lambda : config.tfidf).inject('google', lambda : config.google).add_routes(models_app).add_filter('already_labeled', same_subfolder).add_search_engine('similar', similar).add_search_engine('dissimilar', dissimilar).get_app()
    return (
     args, app)
