...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/models/folder.py
# Compiled at: 2015-07-08 07:34:06
from __future__ import absolute_import, division, print_function
from collections import defaultdict
import dossier.web as web, yakonfig
...
class Folders(web.Folders):
    ...
    def items(self, fid, subid, *args, **kwargs):
        ...
        for it in self.list(fid + '/' + subid):
            if '@' in it.name:
                vals.append(tuple(map(lambda s: s.encode('utf-8'), it.name.split('@'))))
            else:
