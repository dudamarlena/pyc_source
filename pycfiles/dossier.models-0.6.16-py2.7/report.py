...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/models/report.py
...
def main():
    ...
    p.add_argument('-o', '--output', required=True, help='path to write Excel workbook file')
    p.add_argument('-u', '--user', default='unknown', help='user name (default=ALL)')
    p.add_argument('folder', help='folder name')
    p.add_argument('subfolder', nargs='?', default=None, help='subfolder name (default=ALL)')
    args = p.parse_args()
    ...
    gen = ReportGenerator(store, folders, args.folder, subfolder_name=args.subfolder, user=args.user)
...
class ReportGenerator(object):
    ...
    def run(self, output):
        ...
        if self.folder_id not in self.folders.folders(self.user):
            print('E: folder not found: %s' % self.folder_name, file=sys.stderr)
            return
        else:
            wb = self.workbook = xlsxwriter.Workbook(output)
            self.formats['title'] = wb.add_format({'font_size': '18', 'bold': True})
            self.formats['default'] = wb.add_format({'align': 'top'})
            self.formats['bold'] = wb.add_format({'bold': True})
            self.formats['header'] = wb.add_format({'bold': True, 
               'align': 'center', 
               'valign': 'top', 
               'font_size': '14', 
               'font_color': '#506050', 
               ...
               'right': 1, 
               'border_color': 'white'})
            self.formats['pre'] = wb.add_format({'font_name': 'Courier', 'valign': 'top'})
            self.formats['link'] = wb.add_format({'valign': 'top', 'font_color': 'blue', 
               'underline': True})
            self.formats['type_text'] = wb.add_format({'font_color': '#BF8645', 
               'valign': 'top', 
               'align': 'center'})
            self.formats['type_image'] = wb.add_format({'font_color': '#84BF45', 
               'valign': 'top', 
               'align': 'center'})
    ...
    def _generate_report_single(self, sid):
        ...
        if sid not in self.folders.subfolders(self.folder_id, self.user):
...
class Item(object):
    ...
    def __init__(self, generator, subtopic):
        ...
        self.content_id, self.subtopic_id, self.meta_url, type, self.data = subtopic[0:5]
    ...
    def generate_to(self, worksheet, row):
        ...
        try:
            if self.data:
                ...
                worksheet.insert_image(row, 4, 'image', {'image_data': image})
                embedded = True
        except:
            pass
...
Item.constructors = {'text': ItemText, 
   'image': ItemImage, 
   'manual': ItemManual}
