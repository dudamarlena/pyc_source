...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/models/features/sip.py
...
def sip_noun_phrases(tfidf, noun_phrases, limit=40):
    ...
    return dict([ (tfidf.id2word[word], count) for word, count in Counter(dict(bow)).most_common(limit)
                ])
...
def noun_phrases(text, included_unnormalized=False):
    ...
    for terms in noun_phrases_as_tokens(text):
        key = ('_').join(map(normalize, terms))
        normalizations[key].append((' ').join(terms))
