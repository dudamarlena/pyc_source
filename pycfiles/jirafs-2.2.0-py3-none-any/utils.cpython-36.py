...
import collections, contextlib, getpass, os, pkg_resources, re, subprocess
...
def stash_local_changes(repo):
    try:
        if repo.version >= 10:
            repo.run_git_command('stash',
              failure_ok=True)
        yield
    finally:
        if repo.version >= 10:
            patch = repo.run_git_command('stash',
              'show', '-p', failure_ok=True)
            if not patch:
                return
            repo.run_git_command('apply',
              '-3', failure_ok=True, stdin=(patch.encode('utf-8')))
            repo.run_git_command('stash',
              'drop', failure_ok=True)
