    ...
    def validate_issue(self, folder):
        ...
        for target, data in links.items():
            ...
            if data[1].get('status') not in valid_types:
                raise exceptions.IssueValidationError(('{status} is not a valid issue link type for {target}; options include the following: {options}').format(status=data[1].get('status'), target=target, options=(', ').join(valid_types.keys())))
    ...
    def main(self, folder, **kwargs):
        ...
        with utils.stash_local_changes(folder):
            ...
            for filename in status['ready']['files']:
                upload = six.BytesIO(folder.get_local_file_at_revision(filename, 'HEAD', binary=True))
                filename, upload = folder.execute_plugin_method_series('alter_file_upload', args=(
                 (
                  filename, upload),), single_response=True)
                ...
                upload.seek(0)
                attachment = folder.jira.add_attachment(folder.ticket_number, upload, filename=filename)
            ...
            for target, data in links.get('issue', {}).items():
                ...
                if orig is None:
                    ...
                    for existing_link in folder.issue.fields.issuelinks:
                        if hasattr(existing_link, 'inwardIssue') and existing_link.inwardIssue.key == target:
            ...
            folder.run_git_command('add', folder.get_path('.jirafs/remote_files.json'), failure_ok=True)
            folder.run_git_command('add', folder.get_path(constants.TICKET_NEW_COMMENT), failure_ok=True)
            folder.run_git_command('commit', '-m', 'Pushed local changes', failure_ok=True)
            ...
            if result.strip():
                ...
                folder.run_git_command('commit', '-m', 'Updating macro patch', failure_ok=True)
            ...
            folder.run_git_command('commit', '-m', 'Pulled remote changes', failure_ok=True, shadow=True)
