...
# Compiled at: 2019-10-22 00:07:49
import argparse, codecs, copy, logging, logging.config, os, subprocess, sys, time, traceback
...
LOGGING = {'version': 1, 
   ...
   'formatters': {'pretty': {'format': '[%(levelname)s %(issue)s] %(message)s'}, 
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Edit Jira issues locally from your filesystem', add_help=False)
    ...
    parser.add_argument('--no-subfolders', action='store_true', default=False)
    parser.add_argument('--traceback', action='store_true', default=False)
    args, extra = parser.parse_known_args()
    logging.config.dictConfig(LOGGING)
    import logging_tree
    logging_tree.print_out()
    ...
    try:
        value = cmd_class.execute_command(extra, jira=jira, path=args.folder, command_name=command_name)
        logger.debug('Command %s(%s) finished in %s seconds', command_name, extra, time.time() - started)
        if value:
            value.echo()
        sys.exit(value.return_code)
    except GitCommandError as e:
        print ('{t.red}Error (code: {code}) while running git command.{t.normal}').format(t=term, code=e.returncode)
        print ''
        ...
        if not getattr(cmd_class, 'TRY_SUBFOLDERS', False):
            print ("{t.red}The command '{cmd}' must be ran from within an issue folder.{t.normal}").format(t=term, cmd=command_name)
            sys.exit(20)
        else:
            ...
            for folder in os.listdir(os.getcwd()):
                ...
                try:
                    ...
                    result = subprocess.call((' ').join([ shlex_quote(a) for a in full_args ]), cwd=full_path, shell=True)
                    if result == 0:
                        count_runs += 1
                except NotTicketFolderException:
                    pass
        ...
        print ('{t.red}Jirafs encountered an error while interacting with your JIRA instance: {t.normal}{t.red}{t.bold}{error}{t.normal}').format(t=term, error=str(e))
        if args.traceback:
            traceback.print_exc()
        sys.exit(70)
    except JiraInteractionFailed as e:
        print ('{t.red}JIRA was unable to satisfy your request: {t.normal}{t.red}{t.bold}{error}{t.normal}').format(t=term, error=str(e))
        if args.traceback:
            traceback.print_exc()
        sys.exit(80)
    except JirafsError as e:
        print ('{t.red}Jirafs encountered an error processing your request: {t.normal}{t.red}{t.bold}{error}{t.normal}').format(t=term, error=str(e))
        if args.traceback:
            traceback.print_exc()
        sys.exit(90)