...
def migration_0005(repo, init=False, **kwargs):
    ...
    for filename in os.listdir(repo_path):
        if filename not in temp_dir:
            filename.endswith('.jira.rst') or shutil.copyfile(os.path.join(repo_path, filename), os.path.join(temp_path, filename))
...
def migration_0010(repo, init=False, **kwargs):
    ...
    * ``plugin_meta`` *must* be tracked, or when we pop stash, 
    ...
    repo.run_git_command('rm',
      '-f', '--cached', '.jirafs/operation.log', failure_ok=True)
...
def migration_0014(repo, init=False, **kwargs):
    ...
    repo.run_git_command('commit',
      '-m', 'Completing migration_0014', failure_ok=True, shadow=True)
    set_repo_version(repo, 14)
