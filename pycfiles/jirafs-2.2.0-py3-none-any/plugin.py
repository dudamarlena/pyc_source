...
class CommandResult(six.text_type):
    ...
    def __add__(self, other):
        ...
        return CommandResult(joined_strings, return_code=return_code, cursor=self.cursor)
...
class Plugin(JirafsPluginBase):
    ...
    def metadata_filename(self):
        return self.ticketfolder.get_metadata_path('plugin_meta', '%s.json' % self.plugin_name)
...
class CommandPlugin(JirafsPluginBase):
    ...
    def execute_command(cls, extra_args, jira, path, command_name, **ckwargs):
        ...
        parser = argparse.ArgumentParser(prog=os.path.basename(sys.argv[0]) + ' ' + command_name, description=cmd.get_description())
        if cmd.auto_instantiate_folder():
            parser.add_argument('--no-migrate', dest='migrate', default=True, action='store_false')
        ...
        if getattr(cls, 'RUN_FOR_SUBTASKS', False):
            for subfolder in folder.subtasks:
                try:
                    cls.execute_command(extra_args, jira, subfolder.path, command_name, **ckwargs)
...
class MacroPlugin(Plugin):
    ...
    def get_matcher(self):
        return re.compile(self.BASE_REGEX.format(tag_name=self.COMPONENT_NAME), re.MULTILINE | re.DOTALL)
    ...
    def process_text_data(self, content):
        ...
        def run_replacement(match_data):
            ...
            return self.execute_macro(data.get('content'), **self.get_attributes(data.get('start', '')))

        try:
            return self.get_matcher().sub(run_replacement, content)
        except Exception as e:
            self.ticketfolder.log('Error encountered while running macro %s: %s', args=(
             self.plugin_name,
...
class BlockElementMacroPlugin(MacroPlugin):
    BASE_REGEX = '^(?P<start>{{{tag_name}[^}}]*}})(?P<content>.*?)(?P<end>{{{tag_name}}})$'


class VoidElementMacroPlugin(MacroPlugin):
    BASE_REGEX = '^(?P<start>{{{tag_name}}})'