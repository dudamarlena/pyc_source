...
# Compiled at: 2019-10-19 17:12:20
import collections, contextlib, getpass, logging, os, pkg_resources, re, subprocess
...
def stash_local_changes(repo):
    try:
        ...
        if repo.version >= 10:
            ...
            if not patch:
                return
            repo.run_git_command('apply', '-3', failure_ok=True, stdin=patch.encode('utf-8'))
...
def get_jira(domain=None, config=None):
    ...
    if not config.has_option(section, 'password'):
        value = get_user_input('JIRA Password (%s):' % login_data['server'], password=True)
...
def get_git_version():
    result = subprocess.check_output([
     'git', '--version'], stderr=subprocess.PIPE).decode('utf8')
...
PostStatusResponse = collections.namedtuple('PostStatusResponse', [
 'new', 'hash'])