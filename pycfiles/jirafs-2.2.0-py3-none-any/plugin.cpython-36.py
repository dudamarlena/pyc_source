...
# Size of source mod 2**32: 11458 bytes
        ...
        for plugin in folder_plugins:
            if not hasattr(plugin, post_method):
                ...
                if post_result is not None:
                    result = cls.get_command_result(post_result,
                      original=result)
...
class MacroPlugin(Plugin):
    ...
    def process_text_data(self, content):
        ...
        try:
            ...
            self.ticketfolder.log('Error encountered while running macro %s: %s',
              args=(
             self.plugin_name,
             e),
              level=(logging.ERROR))
