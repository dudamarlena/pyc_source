...
# [GCC 8.4.0]
# Embedded file name: /home/acoddington/Documents/Projects/jirafs/jirafs/plugin.py
# Compiled at: 2020-01-13 01:02:43
# Size of source mod 2**32: 25803 bytes
from __future__ import print_function
import argparse, codecs, hashlib, json, logging, os, re, sys
from typing import Dict, Iterator, List, Match, Optional, Pattern, Set, Tuple, TYPE_CHECKING, Union
...
class MacroResult(str):
    ...
    def __new__(cls, string=None, generated_filenames=None):
        if string is None:
            string = ''
...
class CommandResult(str):
    ...
    def __new__(cls, string=None, return_code=None, cursor=0, no_format=False, **kwargs):
        if string is None:
            string = ''
        else:
            if string:
                if not string.endswith('\n'):
                    string = string + '\n'
            terminal = Terminal()
            kwargs['t'] = no_format or terminal
            try:
                string = (string.format)(**kwargs)
            except KeyError:
                logger.warning("An error was encountered while attempting to format string; returning the original string unformatted. The caller may want to use the 'no_format' option if the outgoing string includes curly braces.")
    ...
    def add_line(self, the_line: str, no_format: bool=False, **kwargs):
        if not the_line.endswith('\n'):
            the_line = the_line + '\n'
        kwargs['t'] = no_format or self.terminal
        try:
    ...
    def __add__(self, other):
        ...
        if isinstance(other, CommandResult):
            if other.return_code is not None:
                return_code = other.return_code
        return CommandResult(joined_strings,
          return_code=return_code, cursor=(self.cursor), no_format=True)
...
class JirafsPluginBase(object):
    ...
    def validate(self, **kwargs) -> bool:
        if not (self.MIN_VERSION and self.MAX_VERSION):
            raise PluginValidationError('Minimum and maximum version numbers not specified.')
        ...
        if not min_version <= curr_version < max_version:
            raise PluginValidationError("Plugin '%s' is not compatible with version %s of Jirafs; minimum version: %s; maximum version %s." % (
    ...
    def metadata_filename(self) -> str:
        return self.ticketfolder.get_metadata_path('plugin_meta', '%s.json' % self.entrypoint_name)
...
class CommandPlugin(JirafsPluginBase):
    ...
    def execute_command(cls, extra_args, jira, path, command_name, **ckwargs):
        ...
        parser = argparse.ArgumentParser(prog=(os.path.basename(sys.argv[0]) + ' ' + command_name),
          description=(cmd.get_description()))
        if cmd.auto_instantiate_folder():
            parser.add_argument('--no-migrate',
              dest='migrate', default=True, action='store_false')
        ...
        if getattr(cls, 'RUN_FOR_SUBTASKS', False):
            for subfolder in folder.subtasks:
                try:
                    ...
                    try:
                        logger.exception("Exception encountered while running '%s' for ticket subfolder '%s': %s" % (
...
class MacroPlugin(Plugin):
    TAG_NAME = None
    MATCHERS = [
     '<jirafs:(?P<start>{tag_name}[^>]*)>(?P<content>.*?)</jirafs:(?P<end>{tag_name})>',
     '<jirafs:(?P<start>{tag_name}[^/]*)/>']
    ...
    def get_matchers(self) -> List[Pattern]:
        return [re.compile(rex.format(tag_name=(self.tag_name)), re.MULTILINE | re.DOTALL) for rex in self.MATCHERS]
    ...
    def get_processed_macro_data(self, data: str, attrs: JirafsMacroAttributes, config: Dict) -> Union[(MacroResult, str)]:
    ...
    def process_text_data(self, content: str, path: Optional[str]=None) -> str:
        ...
        def run_replacement(match_data):
            ...
            if data.get('end'):
                if 'src' in attrs:
                    raise MacroContentError("Macro cannot use block element form while also specifying the 'src' attribute.  'src' is used for specifying an external file to use as macro content.")
            body = data.get('content')
            if 'src' in attrs:
                with open(os.path.join(self.ticketfolder.path, attrs['src']), 'r') as (inf):
                    body = inf.read()
        ...
        try:
            ...
            try:
                raise MacroContentError('Error encountered while running macro %s: %s' % (self.tag_name, e)) from e
            finally:
                e = None
                del e
    ...
    def execute_macro(self, data: str, attrs: JirafsMacroAttributes, config: Dict) -> Union[(MacroResult, str)]:
        raise NotImplementedError()
    ...
    def generate_tag_from_data_and_attrs(self, data: str, attrs: JirafsMacroAttributes) -> str:
        attrs_string = self._generate_attrs_string(attrs)
        if 'src' in attrs:
            return f"<jirafs:{self.tag_name}{attrs_string} />"
        return f"<jirafs:{self.tag_name}{attrs_string}>{data}</jirafs:{self.tag_name}>"
...
class AutomaticReversalMacroPlugin(MacroPlugin):
    ...
    def _generate_metadata_key(self, data_hash: str, attrs: JirafsMacroAttributes, config: Dict) -> str:
        hashable_attrs = json.dumps(attrs, sort_keys=True).encode('utf-8')
        return hashlib.sha256(''.join([data_hash.encode('utf-8'), hashable_attrs])).hexdigest()

    def store_cache_entry(self, replacement: str, filenames: List[str], attrs: JirafsMacroAttributes, data_hash: str, config: Dict) -> None:
        metadata_key = self._generate_metadata_key(data_hash, attrs, config)
        is_temp = config['generated_path'] != self.ticketfolder.path
        self.metadata.setdefault('reversal_cache', {})[metadata_key] = {'filenames':filenames, 
         'attrs':attrs, 
    ...
    def find_cache_entry(self, data: str, attrs: JirafsMacroAttributes, data_hash: str, config: Dict) -> Dict:
        generated_path = config['generated_path']
        existing_files = os.listdir(generated_path if generated_path else self.ticketfolder.path)
        metadata_key = self._generate_metadata_key(data_hash, attrs, config)
        try:
    ...
    def cleanup_post_process(self) -> None:
        ...
        obsolete_local_files = set()
        obsolete_temp_files = set()
        for key in obsolete_keys:
            ...
            del cache[key]
            logger.debug('%s: deleting cache key %s', self.entrypoint_name, key)

        local_to_delete = (obsolete_local_files | obsolete_temp_files) - active_local_files
        ...
        for filename in local_to_delete:
            if filename in existing_local_files:
                ...
                logger.debug('%s: deleting obsolete local file %s', self.entrypoint_name, filename)
        ...
        for filename in temp_to_delete:
            if filename in existing_files:
                ...
                logger.debug('%s: deleting obsolete temp file %s', self.entrypoint_name, filename)

        self.save()

    def execute_macro_reversal(self, data: str) -> str:
        for replacement, original in self.metadata.get('replacements', {}).items():
            data = data.replace(replacement, self.generate_tag_from_data_and_attrs(original['data'], original['attrs']))
    ...
    def get_processed_macro_data(self, data: str, attrs: JirafsMacroAttributes, config: Dict) -> Union[(MacroResult, str)]:
        ...
        assert replacement
        if replacement:
            self.store_cache_entry(replacement, filenames, attrs, hashed, config)
            self.metadata.setdefault('replacements', {})[replacement] = {'data':data, 
             'attrs':attrs}
        return replacement

    def get_replacement(self, data: str, attrs: JirafsMacroAttributes, config: Dict) -> Tuple[(str, str)]:
        raise NotImplementedError()


class ImageMacroPlugin(AutomaticReversalMacroPlugin):

    def get_extension_and_image_data(self, data: str, attrs: JirafsMacroAttributes) -> Tuple[(str, bytes)]:
        raise NotImplementedError()

    def execute_macro(self, data: str, attrs: JirafsMacroAttributes, config: Dict) -> MacroResult:
        generated_path = config['generated_path']
        hashed = hashlib.sha256(data.encode('utf-8')).hexdigest()
        ...
        return MacroResult(f'!{filename}|alt="jirafs:{self.tag_name}"!',
          generated_filenames=[
         filename])