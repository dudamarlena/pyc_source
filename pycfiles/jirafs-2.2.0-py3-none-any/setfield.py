...
# [GCC 8.4.0]
# Embedded file name: /home/acoddington/Documents/Projects/jirafs/jirafs/commands/setfield.py
...
class Command(CommandPlugin):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--json', help='Interpret value argument as JSON data.', action='store_true', default=False)
    ...
    def main(self, folder, field_name, value, as_json=False):
        ...
        if as_json:
            try:
                ...
                raise JirafsError("Value '%s' could not be decoded as JSON." % (
                 value,))
                ...
                for idx, component in enumerate(dotpath_parts):
                    if idx == last_key:
                        ...
                        if component not in data:
                            raise JirafsError("Key '%s' (of dotpath '%s') could not be found in field '%s'." % (
                             component,
                             key_dotpath,
                             field_name))
