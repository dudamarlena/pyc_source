...
# [GCC 8.4.0]
# Embedded file name: /home/acoddington/Documents/Projects/jirafs/jirafs/ticketfolder.py
# Compiled at: 2020-01-13 00:37:20
# Size of source mod 2**32: 30538 bytes
import codecs, fnmatch, logging, logging.handlers, io, json, os, re, subprocess
...
class TicketFolder(object):
    ...
    def __init__(self, path, jira, migrate=True, quiet=False):
        ...
        self.get_jira = jira
        self._formatter = logging.Formatter(fmt='%(asctime)s\t%(levelname)s\t%(module)s\t%(message)s')
        self._handler = logging.handlers.RotatingFileHandler((self.get_metadata_path(constants.TICKET_OPERATION_LOG)),
          maxBytes=1048576,
          backupCount=2)
        self._handler.setFormatter(self._formatter)
        self._logger = logging.getLogger('.'.join([__name__, self.ticket_number.replace('-', '_')]))
        self._logger.addHandler(self._handler)
        self._logger_adapter = TicketFolderLoggerAdapter(self._logger, {'issue_id': self.ticket_number})
        if not os.path.isdir(self.metadata_dir):
            raise exceptions.NotTicketFolderException('%s is not a synchronizable ticket folder' % path)
    ...
    def subtasks(self):
        if hasattr(self, '_subtasks'):
            ...
            return os.path.exists(subtasks_path) or self._subtasks
        with open(subtasks_path, 'r') as (in_):
            for line in in_:
                ticket_number = line.strip()
                folder = self.__class__(self.get_path(ticket_number), utils.lazy_get_jira())
        ...
        for name, status in config.items(constants.CONFIG_PLUGINS):
            if not utils.convert_to_boolean(status):
                ...
                if name not in installed_plugins:
                    self.log("Plugin '%s' is not available.", (name,))
                    continue
                ...
                try:
                    ...
                    try:
                        self.log("Plugin '%s' did not pass validation; not loading: %s.", (name, e))
                    finally:
                        e = None
                        del e
    ...
    def set_config_value(self, section, key, value):
        with utils.stash_local_changes(self):
            local_config_file = self.get_metadata_path('config')
            config = utils.get_config(additional_configs=[
             local_config_file],
    ...
    def jira_base(self):
        ...
        if not match:
            raise ValueError('Could not infer JIRA server URL from issue URL %s' % (self.issue_url,))
    ...
    def ticket_number(self):
        ...
        if not match:
            raise ValueError('Could not infer ticket number from URL %s' % self.issue_url)
        return match.group(1)
    ...
    def store_cached_issue(self, shadow=True):
        ...
        with io.open(self.get_path('.jirafs/issue.json', shadow=shadow),
          'w', encoding='utf-8') as (out):
            out.write(json.dumps(storable, indent=4, sort_keys=True, ensure_ascii=False))
        ...
        if not hasattr(self, '_cached_issue'):
            try:
                ...
                with io.open(issue_path, 'r', encoding='utf-8') as (_in):
                    ...
                    self._cached_issue = Issue(storable['options'], None, storable['raw'])
            except IOError:
                self.log('Error encountered while loading cached issue!',
                  level=(logging.ERROR))
    ...
    def get_ticket_url(self):
        try:
            with io.open((self.get_metadata_path('issue_url')),
              'r', encoding='utf-8') as (in_):
    ...
    def version(self):
        try:
            with io.open((self.get_metadata_path('version')),
    ...
    def initialize_ticket_folder(cls, ticket_url, path, jira):
        ...
        with io.open((os.path.join(metadata_path, 'issue_url')),
          'w', encoding='utf-8') as (out):
        ...
        instance.log('Ticket folder for issue %s created at %s', (
    ...
    def run_git_command(self, command, *args, **kwargs):
        ...
        if command == 'commit':
            ...
            if handle.returncode != 0:
                if not failure_ok:
                    command = ' '.join(cmd)
                    raise exceptions.GitCommandError(('Error running command `%s`' % command),
                      returncode=(handle.returncode),
                      stdout=result,
                      cmd=command)
            return binary or result.decode('utf-8').strip()
        return result

    def get_local_file_at_revision(self, path, revision, failure_ok=True, binary=False):
        return self.run_git_command('show',
          ('%s:%s' % (revision, path)), failure_ok=failure_ok, binary=binary)
    ...
    def get_ignore_globs(self, which=constants.LOCAL_ONLY_FILE):
        ...
        for field in constants.FILE_FIELDS:
            all_globs.append(constants.TICKET_FILE_FIELD_TEMPLATE.format(field_name=field))
        ...
        try:
            with io.open((self.get_local_path(which)),
              'r', encoding='utf-8') as (local_ign):
                all_globs.extend(get_globs_from_file(local_ign))
        except IOError:
            pass

        try:
            with io.open((os.path.expanduser('~/%s' % which)),
              'r', encoding='utf-8') as (global_ignores):
    ...
    def get_ready_changes(self):
        ready = {'fields':self.get_fields('HEAD') - self.get_fields(self.git_merge_base), 
        ...
        committed_files = set(self.run_git_command('ls-tree', '--name-only', '-r', current_hash).split('\n'))
        merge_base_files = set(self.run_git_command('ls-tree', '--name-only', '-r', self.git_merge_base).split('\n'))
        ready['deleted'] = self.filter_ignored_files((list(merge_base_files - committed_files)),
          (constants.LOCAL_ONLY_FILE),
          (constants.GIT_IGNORE_FILE),
          (constants.GIT_EXCLUDE_FILE),
          allow_nonfile=True)
        changed_files = self.filter_ignored_files(self.run_git_command('diff', '--name-only', '%s..master' % self.git_merge_base).split('\n'), constants.LOCAL_ONLY_FILE)
        ready['files'] = [filename for filename in changed_files if filename not in ready['deleted']]
        return ready

    def get_uncommitted_changes(self):
        uncommitted = {'fields':self.get_fields() - self.get_fields('HEAD'), 
        ...
        uncommitted['files'] = self.filter_ignored_files([filename for filename in new_files + modified_files if filename], constants.LOCAL_ONLY_FILE, constants.GIT_IGNORE_FILE, constants.GIT_EXCLUDE_FILE)
        uncommitted['deleted'] = self.filter_ignored_files([filename for filename in deleted_files if filename],
          (constants.LOCAL_ONLY_FILE),
          (constants.GIT_IGNORE_FILE),
          (constants.GIT_EXCLUDE_FILE),
          allow_nonfile=True)
    ...
    def get_local_uncommitted_changes(self):
        new_files = self.run_git_command('ls-files', '-o', failure_ok=True).split('\n')
        modified_files = self.run_git_command('ls-files', '-m', failure_ok=True).split('\n')
        committable = self.filter_ignored_files([filename for filename in new_files + modified_files if filename], constants.LOCAL_ONLY_FILE)
        uncommitted = self.filter_ignored_files([filename for filename in modified_files + new_files if filename not in committable], constants.GIT_IGNORE_FILE, constants.GIT_EXCLUDE_FILE)
        return {'files': uncommitted}

    def get_remotely_changed(self):
        metadata = self.get_remote_file_metadata(shadow=True)
        assets = []
        attachments = self.filter_ignored_files(getattr(self.issue.fields, 'attachment', []), constants.REMOTE_IGNORE_FILE)
            ...
            for entrypoint_name, status in config.items(constants.CONFIG_PLUGINS):
                if not utils.convert_to_boolean(status):
                    ...
                    if entrypoint_name not in installed_plugins:
                        self.log("Macro plugin '%s' is not available; this is probably because this plugin is not a macro.",
                          (
                         entrypoint_name,),
                          level=(logging.DEBUG))
                        continue
                    ...
                    try:
                        ...
                        try:
                            self.log("Plugin '%s' did not pass validation; not loading: %s.", (
                             entrypoint_name, e))
                        finally:
                            e = None
                            del e
    ...
    def process_macros(self, data, path=None):
        ...
        unprocessed = re.compile('(<jirafs:.*>)', re.MULTILINE | re.DOTALL).findall(data)
        if unprocessed:
    ...
    def get_new_comment(self, clear=False, staged=False, ready=False):
        try:
            with io.open((self.get_local_path(constants.TICKET_NEW_COMMENT)),
              'r+',
              encoding='utf-8') as (c):
            ...
            if not ready:
                if contents == self.get_new_comment(ready=True):
                    contents = ''
            if contents == local_contents:
                if clear:
                    with io.open((self.get_local_path(constants.TICKET_NEW_COMMENT)),
                      'r+',
                      encoding='utf-8') as (c):
            ...
            try:
                for component in key_dotpath.split('.'):
                    if not isinstance(data, dict):
                        raise exceptions.JirafsError("Key '%s' (of dotpath '%s') is not an object in field '%s'." % (
                         component, key_dotpath, field_name))
                    elif component not in data:
                        if 'default' in kwargs:
                            data = kwargs['default']
                        else:
                            raise exceptions.JirafsError(f"Keypath {key_dotpath} does not exist")
                        break
                    else:
                        data = data[component]

            except (ValueError, TypeError):
                raise exceptions.JirafsError("Field '%s' could not be parsed as JSON for retrieving dotpath '%s'." % (
                 field_name, key_dotpath))
    ...
    def is_up_to_date(self):
        ...
        try:
            self.run_git_command('merge-base', '--is-ancestor', jira_commit, master_commit)
    ...
    def status(self):
        ...
        return {'ready':self.get_ready_changes(), 
    ...
    def run_migrations(self, init=False):
        ...
        if init:
            ...
            if not os.path.exists(self.get_metadata_path('git')):
                raise exceptions.JirafsError('{path} is not a valid ticket folder!'.format(path=(self.path)))
            if self.version < constants.CURRENT_REPO_VERSION:
                print('Your ticket folder at {path} is out-of-date and is being automatically updated.'.format(path=(self.path)))
        while self.version < constants.CURRENT_REPO_VERSION:
            migrator = getattr(migrations, 'migration_%s' % str(self.version + 1).zfill(4))
    ...
    def build_ignore_files(self):
        metadata_excludes = [
         ...
         'subtasks',
         'temp-generated']
        with codecs.open(self.get_local_path(constants.GIT_EXCLUDE_FILE), 'w', 'utf-8') as (out):
        ...
        with codecs.open(self.get_metadata_path('combined_ignore'), 'w', 'utf-8') as (out):
            try:
                out.write('# ~/%s\n' % constants.GIT_IGNORE_FILE_PARTIAL)
                with codecs.open(os.path.expanduser('~/%s' % constants.GIT_IGNORE_FILE_PARTIAL), 'r', 'utf-8') as (in_):
                    for line in in_:
                        out.write('%s\n' % line.strip())
            ...
            try:
                ...
                with codecs.open(self.get_path(constants.GIT_IGNORE_FILE_PARTIAL), 'r', 'utf-8') as (in_):
                    for line in in_:
                        out.write('%s\n' % line.strip())
