...
# [GCC 8.4.0]
# Embedded file name: /home/acoddington/Documents/Projects/jirafs/jirafs/utils.py
# Compiled at: 2020-01-13 00:37:20
# Size of source mod 2**32: 8842 bytes
import collections, configparser, contextlib, getpass, logging, os, pkg_resources, re, subprocess
...
def convert_to_boolean(string):
    if string.upper().strip() in ('Y', 'YES', 'ON', 'ENABLED', 'ENABLE', 'TRUE'):
        return True
    if string.upper().strip() in ('N', 'NO', 'OFF', 'DISABLED', 'DISABLE', 'FALSE'):
        return False
...
def stash_local_changes(repo):
    try:
        ...
        if repo.version >= 10:
            ...
            if not patch:
                return
            repo.run_git_command('apply',
              '-3', failure_ok=True, stdin=(patch.encode('utf-8')))
...
def get_installed_commands():
    ...
    for entry_point in pkg_resources.iter_entry_points(group='jirafs_commands'):
        try:
            ...
            logger.warning('Attempted to load entrypoint %s, but an ImportError occurred.', entry_point)
            continue

        if not issubclass(loaded_class, CommandPlugin):
            logger.warning('Loaded entrypoint %s, but loaded class is not a subclass of `jirafs.plugin.CommandPlugin`.', entry_point)
            continue
...
def get_installed_plugins(subclass=Plugin):
    ...
    for entry_point in pkg_resources.iter_entry_points(group='jirafs_plugins'):
        ...
        if not issubclass(loaded_class, Plugin):
            logger.warning('Loaded entrypoint %s, but loaded class is not a subclass of `jirafs.plugin.Plugin`.', entry_point)
            continue
...
def get_default_jira_server(config=None):
    if config is None:
        ...
        value = config.has_option(constants.CONFIG_JIRA, 'server') or get_user_input('Default JIRA URL: ')
        config.set(constants.CONFIG_JIRA, 'server', value)
    with open(os.path.expanduser('~/%s' % constants.GLOBAL_CONFIG), 'w') as (global_config):
...
def get_jira(domain=None, config=None):
    ...
    if config is None:
        ...
        if not config.has_section(constants.CONFIG_MAIN):
            ...
            if config.has_option(constants.CONFIG_MAIN, 'ask_to_save'):
                ask_to_save = convert_to_boolean(config.get(constants.CONFIG_MAIN, 'ask_to_save'))
    ...
    value = config.has_option(section, 'password') or get_user_input(('JIRA Password (%s):' % login_data['server']),
      password=True)
    ...
    elif config.has_option(section, 'verify'):
