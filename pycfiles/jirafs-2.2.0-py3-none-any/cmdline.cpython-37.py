...
import argparse, codecs, copy, logging, logging.config, os, shlex, subprocess, sys, time, traceback
...
from . import utils
from .exceptions import GitCommandError, JiraInteractionFailed, JirafsError, NotTicketFolderException, UnknownMacroError, MacroError
...
LOGGING = {'version':1, 
 ...
 'formatters':{'pretty':{'format': '[%(levelname)s] %(message)s'}, 
...
def main():
    ...
    parser.add_argument('--traceback',
      action='store_true', default=False)
    parser.add_argument('--debugger-port',
      default=58024,
    ...
    if args.debugger:
        try:
            ...
            print(f"{term.magenta}Waiting for debugger connection on port {args.debugger_port}...{term.normal}")
    ...
    try:
        ...
        if not getattr(cmd_class, 'TRY_SUBFOLDERS', False):
            ...
            for folder in os.listdir(os.getcwd()):
                ...
                try:
                    ...
                    result = subprocess.call((' '.join([shlex.quote(a) for a in full_args])),
                      cwd=full_path,
                      shell=True)
                    if result == 0:
                        count_runs += 1
        ...
        try:
            print('{t.red}Jirafs encountered an unknown macro while processing your content: {t.normal}{t.red}{t.bold}{error}{t.normal}'.format(t=term,
              error=(str(e))))
            if args.traceback:
                traceback.print_exc()
            sys.exit(30)
        ...
        try:
            print('{t.red}Jirafs encountered an error while processing a {t.bold}{macro}{t.normal}{t.red} macro: {t.normal}{t.red}{t.bold}{error}{t.normal}'.format(t=term,
