...
# Size of source mod 2**32: 9424 bytes
...
class Command(CommandPlugin):
    ...
    def apply_macros(self, folder):
        ...
        try:
            ...
            folder.log('Error encountered while reversing applied macros: %s',
              args=(
             e.output,),
              level=(logging.WARNING))
        except Exception as e:
            folder.log('Unhandled error encountered while reversing macros: %s',
              args=(
             e.output,),
              level=(logging.ERROR))
            return
    ...
    def main(self, folder, **kwargs):
        ...
        for filename in folder.get_remotely_changed():
            for attachment in folder.issue.fields.attachment:
                if attachment.filename == filename:
                    ...
                    filename, content = folder.execute_plugin_method_series('alter_file_download',
                      args=(
                     (
                      filename, content),),
            ...
            for field in sorted(folder.issue.raw['fields'].keys()):
                ...
                if field in constants.FILE_FIELDS:
                    ...
                    with io.open(file_field_path,
                      'w',
                      encoding='utf-8') as (file_field_file):
        ...
        folder.run_git_command('commit',
          '-m', 'Fetched remote changes', failure_ok=True,
          shadow=True)
