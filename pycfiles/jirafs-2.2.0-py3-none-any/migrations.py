...
# [GCC 8.4.0]
# Embedded file name: /home/acoddington/Documents/Projects/jirafs/jirafs/migrations.py
...
def set_repo_version(repo, version):
    ...
    repo.run_git_command('add', '-f', repo.get_metadata_path('version'), failure_ok=True)
    repo.run_git_command('commit', '-m', 'Upgraded Repository to v%s' % version, failure_ok=True)
...
def migration_0002(repo, **kwargs):
    ...
    subprocess.check_call(('git', 'clone', '-q', '../git', '.'), cwd=repo.get_metadata_path('shadow'), stdout=subprocess.PIPE)
    try:
...
def migration_0003(repo, init=False, **kwargs):
    ...
    storable = {'options': repo.issue._options, 
       'raw': repo.issue.raw}
...
def migration_0005(repo, init=False, **kwargs):
    ...
    if init:
        ...
        return
    repo_path = repo.path
    temp_path = os.path.normpath(os.path.join(repo_path, '../', repo.path.split('/')[(-1)] + '.tmp'))
    repo.clone(repo.issue_url, repo.get_jira, temp_path)
    temp_dir = os.listdir(temp_path)
    for filename in os.listdir(repo_path):
        if filename not in temp_dir and not filename.endswith('.jira.rst'):
            shutil.copyfile(os.path.join(repo_path, filename), os.path.join(temp_path, filename))
...
def migration_0006(repo, init=False, **kwargs):
    ...
    if init:
        ...
        return
    repo.run_git_command('config', '--file=%s' % repo.get_metadata_path('git', 'config'), 'core.excludesfile', '.jirafs/gitignore')
    set_repo_version(repo, 6)
...
def migration_0008(repo, init=False, **kwargs):
    ...
    repo.run_git_command('add', '.jirafs/gitignore')
    ...
    * ``plugin_meta`` *must* be tracked, or when we pop stash, 
    ...
    with open(repo.get_metadata_path('gitignore'), 'w') as (out):
        out.write(('\n').join([
         ...
         '.jirafs/shadow',
         '.jirafs/operation.log']))
    repo.run_git_command('add', '-f', '.jirafs/gitignore')
    try:
        os.mkdir(repo.get_metadata_path('plugin_meta'))
    except OSError:
        pass
    ...
    with open(repo.get_metadata_path('plugin_meta', '.empty'), 'w') as (out):
        out.write('')
    repo.run_git_command('add', '-f', repo.get_metadata_path('plugin_meta', '.empty'))
    repo.run_git_command('rm', '-f', '--cached', '.jirafs/operation.log', failure_ok=True)
...
def migration_0012(repo, init=False, **kwargs):
    ...
    subprocess.check_call(('git', 'remote', 'set-url', 'origin', '../git'), cwd=repo.get_metadata_path('shadow'), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def migration_0014(repo, init=False, **kwargs):
    ...
    if os.path.exists(repo.get_local_path('.jirafs_ignore')):
        shutil.copyfile(repo.get_local_path('.jirafs_ignore'), repo.get_local_path('.jirafs_local'))
        repo.run_git_command('add', '.jirafs_local')
    if os.path.exists(repo.get_metadata_path('gitignore')):
        shutil.copyfile(repo.get_metadata_path('gitignore'), repo.get_local_path('.jirafs_ignore'))
        repo.run_git_command('add', '.jirafs_ignore')
        repo.run_git_command('rm', repo.get_metadata_path('gitignore'))
    repo.run_git_command('config', '--file=%s' % repo.get_metadata_path('git', 'config'), 'core.excludesfile', '.jirafs/combined_ignore')
    ...
    ignored = repo.filter_ignored_files(set(tracked_files) - set(filtered_files), '.jirafs_local')
    for filename in ignored:
        repo.run_git_command('rm', '--cached', filename, failure_ok=True, shadow=True)

    repo.run_git_command('commit', '-m', 'Completing migration_0014', failure_ok=True, shadow=True)
    set_repo_version(repo, 14)
