...
class Command(CommandPlugin):
    ...
    def has_changes(self, section, *keys):
        if not keys:
            keys = [
             'files', 'deleted', 'fields', 'new_comment', 'links']
        for key in keys:
    ...
    def status_text(self, folder, folder_status):
        ...
        if not folder_status['up_to_date']:
            ...
            if self.has_changes(staged):
                printed_changes = True
                result = result.add_line('')
                result = result.add_line('Uncommitted changes: use `jirafs submit` to submit changes to JIRA, or use `jirafs commit` to commit your changes for submission during a later `jirafs push`.')
            ...
            if self.has_changes(local_uncommitted, 'files'):
                ...
                result = self.format_field_changes(local_uncommitted,
                  'cyan', no_upload=True, result=result)
