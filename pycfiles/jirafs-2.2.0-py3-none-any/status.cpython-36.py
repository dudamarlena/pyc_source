...
class Command(CommandPlugin):
    ...
    def status_text(self, folder, folder_status):
        result = CommandResult()
        result = result.add_line('On ticket {ticket} ({url})',
        ...
        if self.has_changes(staged):
            ...
            if self.has_changes(local_uncommitted, 'files'):
                ...
                result = self.format_field_changes(local_uncommitted,
                  'cyan',
                  no_upload=True,
                  result=result)
    ...
    def format_field_changes(self, changes, color, no_upload=False, result=None):
        ...
        for filename in changes.get('files', []):
            result = result.add_line(('\t{t.%s}{filename}{t.normal} {post_message}' % color),
              filename=filename,
              post_message=('(track in repository)' if no_upload else '(upload attachment)'))
            ...
            if new is not None:
                ...
                result = result.add_line(('\t{t.%s}{description}: {link}{t.normal} {post_message}' % (
                 color,)),
                  ...
                  link=link,
                  post_message=(' (changed remote link)' if orig else ' (new remote link)'))
            else:
        ...
        for link, data in changes.get('links', {}).get('issue', {}).items():
            ...
            if new is not None:
                if new.get('status'):
                    ...
                    status = '(Untitled)'
                result = result.add_line(('\t{t.%s}{status}: {link}{t.normal} {post_message}' % color),
                  status=(status.title()),
                  link=link,
                  post_message=('(changed issue link)' if orig else '(new issue link)'))
            else:
                if orig.get('status'):
                    status = orig['status']
                else:
        ...
        for field, value_set in changes.get('fields', {}).items():
            result = result.add_line(('\t{t.%s}{field}{t.normal}' % color),
              field=field)
