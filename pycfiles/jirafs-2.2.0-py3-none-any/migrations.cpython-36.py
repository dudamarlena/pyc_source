...
# Size of source mod 2**32: 13720 bytes
...
def set_repo_version(repo, version):
    ...
    repo.run_git_command('add',
      '-f', (repo.get_metadata_path('version')), failure_ok=True)
    repo.run_git_command('commit',
      '-m', ('Upgraded Repository to v%s' % version), failure_ok=True)
...
def migration_0002(repo, **kwargs):
    ...
    subprocess.check_call(('git', 'clone', '-q', '../git', '.'),
    ...
    repo.run_git_command('commit',
      '--allow-empty', '-m', 'Shadow Created', shadow=True)
...
def migration_0003(repo, init=False, **kwargs):
    ...
    repo.run_git_command('commit',
      '-m', 'Completing migration_0003', shadow=True)
    repo.run_git_command('push', 'origin', 'jira', shadow=True)
...
def migration_0004(repo, **kwargs):
    ...
    repo.run_git_command('commit',
      '-m', 'Completing migration_0004', shadow=True)
    repo.run_git_command('push', 'origin', 'jira', shadow=True)
    repo.run_git_command('merge', 'jira')
...
def migration_0012(repo, init=False, **kwargs):
    ...
    subprocess.check_call(('git', 'remote', 'set-url', 'origin', '../git'),
      cwd=(repo.get_metadata_path('shadow')),
      stdout=(subprocess.PIPE),
...
def migration_0014(repo, init=False, **kwargs):
    ...
    tracked_files = repo.run_git_command('ls-files',
      '-c', failure_ok=True).split('\n')
...
def migration_0016(repo, init=False, **kwargs):
    ...
    repo.run_git_command('commit',
      '-m', 'Completing migration_0015', failure_ok=True)
