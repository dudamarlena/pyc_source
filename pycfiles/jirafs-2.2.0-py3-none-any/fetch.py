...
# Compiled at: 2019-06-01 16:39:22
import io, json, logging, os, textwrap, six
...
class Command(CommandPlugin):
    ...
    def apply_macros(self, folder):
        macro_patch_filename = folder.get_path('.jirafs/macros_applied.patch', shadow=True)
        if not folder.applied_macros_exist(shadow=True):
            return
        try:
            folder.run_git_command('apply', '--3way', '--reverse', macro_patch_filename, shadow=True)
        except GitCommandError as e:
            folder.log('Error encountered while reversing applied macros: %s', args=(
             e.output,), level=logging.WARNING)
        except Exception as e:
            folder.log('Unhandled error encountered while reversing macros: %s', args=(
             e.output,), level=logging.ERROR)
            return

        folder.run_git_command('commit', '-am', 'Reversing applied macros', shadow=True, failure_ok=True)
    ...
    def main(self, folder, **kwargs):
        ...
        for filename in folder.get_remotely_changed():
            for attachment in folder.issue.fields.attachment:
                if attachment.filename == filename:
                    folder.log('Download file "%s"', (
                     attachment.filename,))
                    content = six.BytesIO(attachment.get())
                    filename, content = folder.execute_plugin_method_series('alter_file_download', args=(
                     (
                      filename, content),), single_response=True)
        ...
        with io.open(detail_path, 'w', encoding='utf-8') as (dets):
            for field in sorted(folder.issue.raw['fields'].keys()):
                ...
                if not isinstance(value, six.string_types):
                    value = json.dumps(value, sort_keys=True, indent=4, ensure_ascii=False)
        ...
        with io.open(links_path, 'w', encoding='utf-8') as (links_handle):
            for link in folder.issue.fields.issuelinks:
                ...
                if 'inwardIssue' in link.raw:
                    category = 'inward'
                links_handle.write(six.u('* {status}: {key}\n').format(status=getattr(link.type, category).title(), key=getattr(link, '%sIssue' % category).key))

            for link in folder.jira.remote_links(folder.issue):
                if link.object.title:
                    links_handle.write(six.u('* {title}: {url}\n').format(title=link.object.title, url=link.object.url))
                else:
                    links_handle.write(six.u('* {url}\n').format(title=link.object.title, url=link.object.url))
        ...
        with io.open(comments_filename, 'w', encoding='utf-8') as (comm):
            for comment in folder.issue.fields.comment.comments:
                comm.write(six.text_type('* At %s, %s wrote:\n\n') % (
                 comment.created,
                 comment.author))
                final_lines = []
                ...
                for line in lines:
                    if not line:
                        ...
                        final_lines.extend(textwrap.wrap(line, width=70, expand_tabs=False, replace_whitespace=False, break_long_words=False))
                ...
                for issue in subtasks:
                    ...
                    if not os.path.exists(issue_path):
                        ...
                        args = [
                         ...
                         issue_path]
                        path = folder.path
                        commands[command_name].execute_command(args, jira=jira, path=path, command_name=command_name)

        folder.build_ignore_files()
        folder.run_git_command('add', '-A', shadow=True)
        folder.run_git_command('commit', '-m', 'Fetched remote changes', failure_ok=True, shadow=True)
