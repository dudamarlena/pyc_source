...
# Compiled at: 2019-03-14 00:29:48
import os, re, shutil, subprocess, tempfile, six
...
class Command(CommandPlugin):
    ...
    def clone_from_git_repository(self, url, path, jira):
        ...
        subprocess.check_call([
         ...
         'clone',
         url,
         temp_dir], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        for branch in ['jira', 'master']:
            subprocess.check_call([
             'git',
             'checkout',
             branch], cwd=temp_dir, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        ...
        if not match:
            ...
            raise exceptions.NotTicketFolderException('The git repository at %s is not a Jirafs backup.' % (
             url,))
        if path:
            path = os.path.realpath(path)
        else:
        ...
        subprocess.check_call([
         ...
         'clone',
         path,
         shadow_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        ...
        subprocess.check_call([
         ...
         'set-url',
         'origin',
         '../git'], cwd=shadow_path, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        subprocess.check_call([
         'git',
         'checkout',
         'jira'], cwd=shadow_path, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('path', nargs='*', type=six.text_type)