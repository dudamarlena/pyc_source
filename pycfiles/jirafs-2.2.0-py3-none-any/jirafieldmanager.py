...
import io, json, logging, os, re, six
        ...
        with io.open(folder_path, 'w', encoding='utf-8') as (out):
            for field in sorted(self.keys()):
                if field not in used_fields | requested_fields:
                    out.write(('* {human} ({field}):\n').format(human=self.get_human_name_for_field(field), field=field))
                    field_string = self[field]
                    if field_string is None:
                        field_string = ''
                    else:
                        if not isinstance(field_string, six.string_types):
                            field_string = json.dumps(field_string, sort_keys=True, indent=4, ensure_ascii=False)
