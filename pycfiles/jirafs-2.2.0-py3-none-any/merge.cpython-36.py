...
class Command(CommandPlugin):
    ...
    def main(self, folder, **kwargs):
        with utils.stash_local_changes(folder):
            ...
            jira_fields = JiraFieldManager.create(folder,
              revision='jira')
            master_fields = JiraFieldManager.create(folder,
              revision=original_merge_base)
            for field, values in (jira_fields - master_fields).items():
                folder.log('Field {field} changed: "{fr}" -> "{to}"'.format(field=field,
                  fr=(self.truncate_field_value(values[0])),
                  to=(self.truncate_field_value(values[1]))))

            jira_links = JiraLinkManager.create(folder,
              revision='jira')
            ...
            for category in ('issue', 'remote'):
                ...
                for field, values in values_dict.items():
                    folder.log('Link {field} changed: "{fr}" -> "{to}"'.format(field=field,
                      fr=(values[0]),
                      to=(values[1])))
