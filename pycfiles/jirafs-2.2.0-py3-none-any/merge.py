...
# [GCC 8.4.0]
# Embedded file name: /home/acoddington/Documents/Projects/jirafs/jirafs/commands/merge.py
...
class Command(CommandPlugin):
    ...
    def main(self, folder, **kwargs):
        with utils.stash_local_changes(folder):
            ...
            new_comments = folder.run_git_command('diff', '%s..%s' % (
             original_merge_base,
             final_merge_base), '--', constants.TICKET_COMMENTS).strip()
            if new_comments:
                folder.log('New comment(s) have been posted.')
            jira_fields = JiraFieldManager.create(folder, revision='jira')
            master_fields = JiraFieldManager.create(folder, revision=original_merge_base)
            for field, values in (jira_fields - master_fields).items():
                folder.log(('Field {field} changed: "{fr}" -> "{to}"').format(field=field, fr=self.truncate_field_value(values[0]), to=self.truncate_field_value(values[1])))
            ...
            for category in ('issue', 'remote'):
                ...
                for field, values in values_dict.items():
                    folder.log(('Link {field} changed: "{fr}" -> "{to}"').format(field=field, fr=values[0], to=values[1]))

            if original_merge_base != final_merge_base:
                folder.log("Merged 'jira' into 'master'; merge-base is now %s" % final_merge_base)
            return utils.PostStatusResponse(original_merge_base == final_merge_base, final_merge_base)