...
# Size of source mod 2**32: 8502 bytes
import io
    ...
    def main(self, folder, **kwargs):
        ...
        with utils.stash_local_changes(folder):
            ...
            for filename in status['ready']['deleted']:
                folder.log('Deleting file "%s"', (filename,))
            ...
            for filename in status['ready']['files']:
                upload = io.BytesIO(folder.get_local_file_at_revision(filename, 'HEAD', binary=True))
                folder.log('Uploading file "%s"', (filename,))
                for attachment in folder.issue.fields.attachment:
            ...
            folder.run_git_command('add',
              (folder.get_path('.jirafs/remote_files.json')), failure_ok=True)
            folder.run_git_command('add',
              (folder.get_path(constants.TICKET_NEW_COMMENT)), failure_ok=True)
            ...
            folder.run_git_command('commit',
              '-m', 'Pulled remote changes', failure_ok=True, shadow=True)
