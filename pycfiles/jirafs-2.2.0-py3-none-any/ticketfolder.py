...
class TicketFolder(object):
    ...
    def set_config_value(self, section, key, value):
        with utils.stash_local_changes(self):
            ...
            config = utils.get_config(additional_configs=[
             local_config_file], include_global=False)
    ...
    def store_cached_issue(self, shadow=True):
        storable = {'options': self.issue._options, 
           'raw': self.issue.raw}
        with io.open(self.get_path('.jirafs/issue.json', shadow=shadow), 'w', encoding='utf-8') as (out):
            out.write(six.text_type(json.dumps(storable, indent=4, sort_keys=True, ensure_ascii=False)))
    ...
    def cached_issue(self):
        if not hasattr(self, '_cached_issue'):
            try:
                ...
                self.log('Error encountered while loading cached issue!', level=logging.ERROR)
    ...
    def get_ticket_url(self):
        try:
            with io.open(self.get_metadata_path('issue_url'), 'r', encoding='utf-8') as (in_):
    ...
    def get_remote_file_metadata(self, shadow=True):
        ...
        return self.execute_plugin_method_series('alter_get_remote_file_metadata', args=(
         data,), single_response=True)

    def set_remote_file_metadata(self, data, shadow=True):
        data = self.execute_plugin_method_series('alter_set_remote_file_metadata', args=(
         data,), single_response=True)
        remote_files = self.get_path('.jirafs/remote_files.json', shadow=shadow)
        with io.open(remote_files, 'w', encoding='utf-8') as (out):
            out.write(six.text_type(json.dumps(data, indent=4, sort_keys=True, ensure_ascii=False)))
    ...
    def initialize_ticket_folder(cls, ticket_url, path, jira):
        ...
        subprocess.check_call((
         ...
         '--bare',
         'init',
         os.path.join(metadata_path, 'git')), stdout=subprocess.PIPE)
        subprocess.check_call((
         'git',
         'config',
    ...
    def applied_macros_exist(self, shadow=False):
        macro_patch_filename = self.get_path('.jirafs/macros_applied.patch', shadow=shadow)
    ...
    def run_git_command(self, command, *args, **kwargs):
        ...
        self.log('Executing git command `%s`', (
         (' ').join(cmd),), logging.DEBUG, sublogger='git')
        handle = subprocess.Popen(cmd, cwd=cwd, stdin=subprocess.PIPE, stderr=subprocess.STDOUT, stdout=subprocess.PIPE)
        result, _ = handle.communicate(stdin)
        if handle.returncode != 0 and not failure_ok:
            command = (' ').join(cmd)
            raise exceptions.GitCommandError('Error running command `%s`' % command, returncode=handle.returncode, stdout=result, cmd=command)
        if not binary:
            return result.decode('utf-8').strip()
        return result

    def get_local_file_at_revision(self, path, revision, failure_ok=True, binary=False):
        return self.run_git_command('show', '%s:%s' % (
         revision,
         path), failure_ok=failure_ok, binary=binary)
    ...
    def get_uncommitted_changes(self):
        ...
        uncommitted['files'] = self.filter_ignored_files([ filename for filename in new_files + modified_files if filename
                                                         ], constants.LOCAL_ONLY_FILE, constants.GIT_IGNORE_FILE, constants.GIT_EXCLUDE_FILE)
        return uncommitted
    ...
    def get_local_uncommitted_changes(self):
        ...
        uncommitted = self.filter_ignored_files([ filename for filename in modified_files + new_files if filename not in committable
                                                ], constants.GIT_IGNORE_FILE, constants.GIT_EXCLUDE_FILE)
    ...
    def get_remotely_changed(self):
        ...
        return self.execute_plugin_method_series(name='alter_remotely_changed', args=(
         assets,), single_response=True)
    ...
    def filter_ignored_files(self, files, *which):
        ...
        for list_path in which:
            ...
            for fileish in files:
                ...
                if self.file_matches_globs(filename, ignore_globs):
                    continue
                if not attachment and not os.path.isfile(os.path.join(self.path, filename)):
                    continue
        ...
        return self.execute_plugin_method_series(name='alter_filter_ignored_files', args=(
         assets,), single_response=True)
    ...
    def get_new_comment(self, clear=False, staged=False, ready=True):
        ...
        return self.execute_plugin_method_series(name='alter_new_comment', args=(
         contents,), single_response=True)
    ...
    def status(self):
        ...
        return self.execute_plugin_method_series('alter_status_dict', args=(
         status,), single_response=True)
