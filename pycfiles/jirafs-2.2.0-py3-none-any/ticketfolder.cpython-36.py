...
# Size of source mod 2**32: 31220 bytes
import codecs, datetime, fnmatch, io, inspect, json, logging, os, re, subprocess
...
class TicketFolder(object):
        ...
        if not config.has_section(constants.CONFIG_PLUGINS):
            ...
            for name, status in config.items(constants.CONFIG_PLUGINS):
                ...
                if name not in installed_plugins:
                    self.log("Plugin '%s' is not available.", (
                     name,))
                else:
    ...
    def set_config_value(self, section, key, value):
        with utils.stash_local_changes(self):
            ...
            self.run_git_command('commit',
              '-m', 'Config change', failure_ok=True)

    @property
    def jira_base(self):
        match = re.match('(.*)\\/browse\\/.*', self.issue_url)
        if not match:
            raise ValueError('Could not infer JIRA server URL from issue URL %s' % (
             self.issue_url,))
    ...
    def store_cached_issue(self, shadow=True):
        storable = {'options':self.issue._options, 
         'raw':self.issue.raw}
        with io.open(self.get_path('.jirafs/issue.json', shadow=shadow),
          'w',
          encoding='utf-8') as (out):
            out.write(six.text_type(json.dumps(storable,
              indent=4,
              sort_keys=True,
              ensure_ascii=False)))
    ...
    def get_remote_file_metadata(self, shadow=True):
        remote_files = self.get_path('.jirafs/remote_files.json',
          shadow=shadow)
        try:
        ...
        return self.execute_plugin_method_series('alter_get_remote_file_metadata',
          args=(
         data,),
          single_response=True)

    def set_remote_file_metadata(self, data, shadow=True):
        data = self.execute_plugin_method_series('alter_set_remote_file_metadata',
          args=(
         data,),
          single_response=True)
        remote_files = self.get_path('.jirafs/remote_files.json',
    ...
    def initialize_ticket_folder(cls, ticket_url, path, jira):
        ...
        subprocess.check_call((
         ...
         '--bare',
         'init',
         os.path.join(metadata_path, 'git')),
    ...
    def applied_macros_exist(self, shadow=False):
        macro_patch_filename = self.get_path('.jirafs/macros_applied.patch',
          shadow=shadow)
    ...
    def run_git_command(self, command, *args, **kwargs):
        ...
        cmd.extend(args)
        self.log('Executing git command `%s`',
          (
         ' '.join(cmd),),
          (logging.DEBUG),
          sublogger='git')
    ...
    def get_ignore_globs(self, which=constants.LOCAL_ONLY_FILE):
        ...
        try:
            with io.open((os.path.expanduser('~/%s' % which)),
              'r',
              encoding='utf-8') as (global_ignores):
    ...
    def get_uncommitted_changes(self):
        ...
        new_files = self.run_git_command('ls-files',
          '-o', failure_ok=True).split('\n')
        modified_files = self.run_git_command('ls-files',
          '-m', failure_ok=True).split('\n')
    ...
    def get_remotely_changed(self):
        ...
        return self.execute_plugin_method_series(name='alter_remotely_changed',
          args=(
         assets,),
          single_response=True)
    ...
    def filter_ignored_files(self, files, *which):
        ...
        return self.execute_plugin_method_series(name='alter_filter_ignored_files',
          args=(
         assets,),
          single_response=True)
    ...
    def get_new_comment(self, clear=False, staged=False, ready=True):
        ...
        return self.execute_plugin_method_series(name='alter_new_comment',
          args=(
         contents,),
          single_response=True)
