...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.5/site-packages/bookshelf/api_v2/vagrant.py
# Compiled at: 2016-08-21 18:45:36
# Size of source mod 2**32: 3421 bytes
import re
from fabric.api import sudo, settings, run
from fabric.context_managers import hide
from bookshelf.api_v2.os_helpers import install_ubuntu_development_tools, lsb_release
from bookshelf.api_v2.pkg import apt_add_repository_from_apt_string, apt_install_from_url, apt_install
    ...
    with settings(hide('running', 'stdout')):
        if use_sudo:
            if plugin not in sudo('vagrant plugin list'):
                sudo(cmd)
        elif plugin not in run('vagrant plugin list'):
            run(cmd)
