...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.5/site-packages/bookshelf/tests/api_v2/test_rackspace.py
# Compiled at: 2016-08-21 18:37:21
# Size of source mod 2**32: 11398 bytes
import unittest
from bookshelf.api_v2 import rackspace
import novaclient, vcr, mock
from collections import OrderedDict
import os

class ConnectToRackspaceTests(unittest.TestCase):

    @vcr.use_cassette('fixtures/vcr_ConnectToRackspace.yml', decode_compressed_response=True)
    def test_connect_to_rackspace_returns_connection_object(self, *args, **kwargs):
        conn = rackspace.connect_to_rackspace(os.environ.get('OS_REGION_NAME'), os.environ.get('OS_USERNAME'), os.environ.get('OS_PASSWORD'))
...
class CreateRackspaceImageTests(unittest.TestCase):
    ...
    def test_CreateRackspaceImage(self, *args, **kwargs):
        conn = rackspace.connect_to_rackspace(os.environ.get('OS_REGION_NAME'), os.environ.get('OS_USERNAME'), os.environ.get('OS_PASSWORD'))
        import bookshelf.api_v2.cloud
        bookshelf.api_v2.cloud.is_ssh_available = mock.Mock(return_value=True)
        srv = rackspace.create_server_rackspace(connection=conn, distribution='centos7', disk_name='disk', disk_size='75', ami='CentOS 7 (PVHVM)', region='IAD', key_pair='mykeypair', instance_type='1GB Standard Instance', instance_name='CentOS_7', tags={}, security_groups=None)
        img = rackspace.create_rackspace_image(connection=conn, name='myImage', description='my Image', server_id=srv.id)
...
class CreateServerRackspaceTests(unittest.TestCase):
    ...
    def test_CreateServerRackspace(self, *args, **kwargs):
        conn = rackspace.connect_to_rackspace(os.environ.get('OS_REGION_NAME'), os.environ.get('OS_USERNAME'), os.environ.get('OS_PASSWORD'))
        import bookshelf.api_v2.cloud
        bookshelf.api_v2.cloud.is_ssh_available = mock.Mock(return_value=True)
        srv = rackspace.create_server_rackspace(connection=conn, distribution='centos7', disk_name='disk', disk_size='75', ami='CentOS 7 (PVHVM)', region='IAD', key_pair='mykeypair', instance_type='1GB Standard Instance', instance_name='CentOS_7', tags={}, security_groups=None)
        self.assertIsInstance(srv, novaclient.v2.servers.Server)
...
class DownRackspaceTests(unittest.TestCase):
    ...
    def test_DownRackspace(self, *args, **kwargs):
        conn = rackspace.connect_to_rackspace(os.environ.get('OS_REGION_NAME'), os.environ.get('OS_USERNAME'), os.environ.get('OS_PASSWORD'))
        import bookshelf.api_v2.cloud
        bookshelf.api_v2.cloud.is_ssh_available = mock.Mock(return_value=True)
        srv = rackspace.create_server_rackspace(connection=conn, distribution='centos7', disk_name='disk', disk_size='75', ami='CentOS 7 (PVHVM)', region='IAD', key_pair='mykeypair', instance_type='1GB Standard Instance', instance_name='CentOS_7', tags={}, security_groups=None)
...
class GetIpAddressFromRackspaceServerTests(unittest.TestCase):
    ...
    def test_GetIpAdressFromRackspaceServer(self, *args, **kwargs):
        conn = rackspace.connect_to_rackspace(os.environ.get('OS_REGION_NAME'), os.environ.get('OS_USERNAME'), os.environ.get('OS_PASSWORD'))
        import bookshelf.api_v2.cloud
        bookshelf.api_v2.cloud.is_ssh_available = mock.Mock(return_value=True)
        srv = rackspace.create_server_rackspace(connection=conn, distribution='centos7', disk_name='disk', disk_size='75', ami='CentOS 7 (PVHVM)', region='IAD', key_pair='mykeypair', instance_type='1GB Standard Instance', instance_name='CentOS_7', tags={}, security_groups=None)
        self.assertRegexpMatches(rackspace.get_ip_address_from_rackspace_server(connection=conn, server_id=srv.id), '\\d+\\.\\d+\\.\\d+\\.\\d+')
