...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.5/site-packages/bookshelf/tests/api_v2/docker_based_tests.py
...
def build_docker_image(image, base_image, distribution):
    ...
    if 'ubuntu' in distribution:
        contents = ['FROM ' + base_image,
         ...
         'RUN apt-get update && apt-get install -y sudo openssh-server lsb-release software-properties-common python-pip curl',
    ...
    with open(dockerfile, 'w') as (f):
        f.write('\n'.join(contents))
    local('docker build -f %s -t %s .' % (dockerfile, image), capture=False)
...
def docker_run(image, privileged=False):
    ...
    if privileged:
        flags = '--privileged'
    container = local('docker run %s -d -P %s' % (flags, image), capture=True)
    return container
...
def docker_container_port(container):
    ...
    """
    with settings(quiet()):
        output = local('docker port %s 22' % container, capture=True)
...
def prepare_required_docker_images():
    ...
    ip = local("docker run -it alpine route -n | grep -E '^0.0.0.0' | awk '{ print $2 }'", capture=True)
    ...
    for item in images:
        if item['image'] not in local('docker images', capture=True):
            ...
            build_docker_image(image=item['image'], base_image=item['base_image'], distribution=item['distribution'])