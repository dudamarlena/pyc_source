...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.5/site-packages/bookshelf/api_v3/ec2.py
# Compiled at: 2016-08-21 18:37:21
# Size of source mod 2**32: 11674 bytes
from time import sleep
import boto.ec2
from boto.ec2.blockdevicemapping import BlockDeviceMapping, EBSBlockDeviceType
from pyrsistent import PClass, field, pmap, PMap, pvector, PVector
from zope.interface import implementer, provider
from bookshelf.api_v3.cloud_instance import ICloudInstance, ICloudInstanceFactory, Distribution
...
class EC2State(PClass):
    ...
    distro = field(mandatory=True, factory=Distribution, serializer=lambda _, x: x.value)
...
def _parse_unicode_pmap(d):
    return pmap({unicode(k):unicode(v) for k, v in d.iteritems()})
...
class EC2Configuration(PClass):
    ...
    security_groups = field(type=PVector, mandatory=True, factory=_parse_unicode_pvector)
...
def _connect_to_ec2(region, credentials):
    ...
    """
    conn = boto.ec2.connect_to_region(region, aws_access_key_id=credentials.access_key_id, aws_secret_access_key=credentials.secret_access_key)
    if conn:
        return conn
    else:
...
def _create_server_ec2(connection, region, disk_name, disk_size, ami, key_pair, instance_type, tags={}, security_groups=None, delete_on_termination=True, log=False, wait_for_ssh_available=True):
    """
    ...
    reservation = image.run(1, 1, key_name=key_pair, security_groups=security_groups, block_device_map=bdm, instance_type=instance_type)
...
class EC2Instance:
    ...
    def create_from_config(cls, config, distro, region):
        ...
        connection = _connect_to_ec2(region=region, credentials=parsed_config.credentials)
        instance = _create_server_ec2(connection=connection, region=region, disk_name=parsed_config.disk_name, disk_size=parsed_config.disk_size, ami=parsed_config.ami, key_pair=parsed_config.key_pair, instance_type=parsed_config.instance_type, tags=parsed_config.tags, security_groups=parsed_config.security_groups, delete_on_termination=True, log=False, wait_for_ssh_available=True)
        state = EC2State(instance_id=instance.id, region=region, distro=distro)
        return cls(connection=connection, instance=instance, config=parsed_config, state=state)

    @classmethod
    def create_from_saved_state(cls, config, saved_state, timeout=600):
        parsed_config = EC2Configuration.create(config)
        state = EC2State.create(saved_state)
        connection = _connect_to_ec2(region=state.region, credentials=parsed_config.credentials)
        instance = connection.start_instances(instance_ids=state.instance_id)[0]
        instance.update()
    ...
    def create_image(self, image_name):
        ami = self.connection.create_image(self.state.instance_id, image_name, description=self.config.image_description)
        image_status = self.connection.get_image(ami)
        while image_status.state != 'available' and image_status.state != 'failed':
    ...
    def list_images(self):
        ...
        for image in sorted(images, key=lambda x: x.creationDate):
            log_green('{}\t{:50}\t{}'.format(image.creationDate, image.name, image.id))
    ...
    def destroy(self):
        self.down()
        volumes = self.connection.get_all_volumes(filters={'attachment.instance-id': self.state.instance_id})
        instance = self.connection.terminate_instances(instance_ids=[
         self.state.instance_id])[0]
    ...
    def down(self):
        instance = self.connection.stop_instances(instance_ids=self.state.instance_id)[0]
