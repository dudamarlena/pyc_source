...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.5/site-packages/bookshelf/api_v3/gce.py
# Compiled at: 2016-08-21 18:37:21
# Size of source mod 2**32: 17597 bytes
"""
...
class GCEConfiguration(PClass):
    ...
    credentials_private_key = field(factory=unicode, initial='', mandatory=True)
    credentials_email = field(factory=unicode, initial='', mandatory=True)
...
class GCEInstance(object):
    ...
    def create_from_config(cls, config, distro, region):
        instance_name = '{}-{}'.format(config['instance_name'], unicode(uuid.uuid4()))
        assert len(instance_name) <= 61, 'Instance name too long for GCE'
        state = GCEState(instance_name=instance_name, ip_address='', distro=distro.value, zone=region)
    ...
    def _ensure_instance_running(self, instance_name):
        ...
        """
        try:
            instance_info = self._compute.instances().get(project=self.project, zone=self.zone, instance=instance_name).execute()
            if instance_info['status'] == 'RUNNING':
                pass
            else:
                if instance_info['status'] == 'TERMINATED':
                    self._start_terminated_server(instance_name)
                else:
                    msg = 'Instance {} is in state {}, please start it from the console'.format(instance_name, instance_info['status'])
                    raise Exception(msg)
    ...
    def _start_terminated_server(self, instance_name):
        ...
        operation = self._compute.instances().start(project=self.project, zone=self.zone, instance=instance_name).execute()
    ...
    def _set_instance_networking(self):
        ...
        """
        instance_data = self._compute.instances().get(project=self.project, zone=self.zone, instance=self.state.instance_name).execute()
        ip_address = instance_data['networkInterfaces'][0]['accessConfigs'][0]['natIP']
        self.state = self.state.transform(['ip_address'], ip_address)
        wait_for_ssh(self.state.ip_address)
        log_green('Connected to server with IP address {0}.'.format(ip_address))
    ...
    def create_image(self, image_name):
        ...
        body = {'rawDisk': {}, 
         'name': image_name, 
         'sourceDisk': 'projects/{}/zones/{}/disks/{}'.format(self.project, self.zone, disk_name), 
         
         'description': self.description}
        self._wait_until_done(self._compute.images().insert(project=self.project, body=body).execute())
        return image_name
    ...
    def down(self):
        ...
        self._wait_until_done(self._compute.instances().stop(project=self.project, zone=self.zone, instance=self.state.instance_name).execute())

    def _destroy_instance(self):
        log_yellow('destroying server: {}'.format(self.state.instance_name))
        try:
            self._wait_until_done(self._compute.instances().delete(project=self.project, zone=self.zone, instance=self.state.instance_name).execute())
        except HttpError as e:
            if e.resp.status == 404:
    ...
    def destroy(self):
        ...
        self._destroy_instance()
        try:
            self._wait_until_done(self._compute.disks().delete(project=self.project, zone=self.zone, disk=disk_name).execute())
        except HttpError as e:
            if e.resp.status == 404:
    ...
    def _get_instance_config(self, instance_name, image, disk_name=None):
        """
        ...
        if disk_name:
            disk_config = {'type': 'PERSISTENT', 
             'boot': True, 
             'mode': 'READ_WRITE', 
             'autoDelete': False, 
             'source': 'projects/{}/zones/{}/disks/{}'.format(self.project, self.zone, disk_name)}
        else:
            disk_config = {'type': 'PERSISTENT', 
             'boot': True, 
             'mode': 'READ_WRITE', 
             ...
             'initializeParams': {'sourceImage': image, 
                                  ...
                                  'diskSizeGb': '10'}}
        gce_slave_instance_config = {'name': instance_name, 
         'machineType': 'projects/{}/zones/{}/machineTypes/{}'.format(self.project, self.zone, self.config.machine_type), 
         
         'disks': [disk_config], 
         'networkInterfaces': [
                               {'network': 'projects/%s/global/networks/default' % self.project, 
         ...
         'metadata': {'items': [
                                {'key': 'sshKeys', 
                                 'value': '{}:{}'.format(self.config.username, public_key)}]}, 
    ...
    def startup_instance(self, instance_name, image, disk_name=None):
        ...
        operation = self._compute.instances().insert(project=self.project, zone=self.zone, body=instance_config).execute()
    ...
    def _get_gce_compute(self):
        if self.config.credentials_email and self.config.credentials_private_key:
            credentials = SignedJwtAssertionCredentials(self.config.credentials_email, self.config.credentials_private_key, scope=[
             'https://www.googleapis.com/auth/compute'])
        else:
    ...
    def _get_latest_image(self, base_image_project, image_name_prefix):
        ...
        * centos-cloud, centos-7
        ...
        while not latest_image:
            response = self._compute.images().list(project=base_image_project, maxResults=500, pageToken=page_token, filter='name eq {}.*'.format(image_name_prefix)).execute()
            latest_image = next((image for image in response.get('items', []) if 'deprecated' not in image), None)
    ...
    def get_state(self):
        ...
        from the config.
        """
        data = {'ip_address': self.state.ip_address, 
