...
# [GCC 8.4.0]
# Embedded file name: e:\project\airtestide\airtest\airtest\report\report.py
# Compiled at: 2020-01-13 05:54:32
...
class LogToHtml(object):
    __doc__ = 'Convert log to html display '
    scale = 0.5
    ...
    def _translate_screen(self, step, code):
        if step['tag'] != 'function':
            ...
            screen = {'src':None, 
             'rect':[],  'pos':[],  'vector':[],  'confidence':None}
            ...
            if step['data']['name'] in ('touch', 'assert_exists', 'wait', 'exists'):
                if self.is_pos(step['data'].get('ret')):
                    ...
                    if self.is_pos(step['data']['call_args'].get('v')):
    ...
    def report(self, template_name=HTML_TPL, output_file=None, record_list=None):
        ...
        if not self.script_name:
            ...
            if self.export_dir:
                ...
                output_file = output_file if (output_file and os.path.isabs(output_file)) else (os.path.join(self.script_root, output_file or HTML_FILE))
                if not self.static_root.startswith('http'):
