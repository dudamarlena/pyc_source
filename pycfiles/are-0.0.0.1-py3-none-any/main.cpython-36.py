...
class Command(object):
    ...
    def init_config(self, arguments):
        ...
        params = {}
        if getattr(base_parser, 'project', False):
            if base_parser.project is not None:
                params['path'] = base_parser.project
        if getattr(base_parser, 'conffile', False):
            if base_parser.conffile is not None:
    ...
    def parse_commandline(self):
        ...
        if self.args.command_name == 'deploy':
            if self.args.lambdafunctions:
                if self.args.lambdafunctions is not '_ALL_':
