...
class Trigger(object):
    ...
    def __init__(self, *args, **kwargs):
        self.lambda_function_arn = kwargs[b'lambda_function_arn']
        self.lambda_conf = kwargs[b'lambda_conf']
        self.client = self.set_client()
        self.awslambda = AWSCli(config=self.lambda_conf).get_lambda_client()

    def get_triggers(self):
        trigger_conf = self.lambda_conf.get(b'triggers', {}).get(self.trigget_type)
        if not len(trigger_conf):
            raise ArdyNoTriggerConfError((b'Not exists conf for lambda {} and trigger {}').format(self.lambda_function_arn, self.trigget_type))
        return trigger_conf
    ...
    def lambda_exist_policy(self, function_name, StatementId):
        ...
        policies = json.loads(response[b'Policy'])
        for policy in policies[b'Statement']:
            if policy[b'Sid'] == StatementId:
                return True
