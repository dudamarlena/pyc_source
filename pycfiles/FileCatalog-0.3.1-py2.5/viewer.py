...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/filecatalog/viewer.py
# Compiled at: 2008-05-16 08:54:00
"""
File Catalog Viewer
===================
...
FILE_WILDCARD = ('|').join(('YAML files (*.yaml, *.yml)|*.yaml;*.yml', 'All files (*.*)|*.*'))
...
class MainFrame(wx.Frame):
    ...
    def create_toolbar(self):
        """Create the toolbar."""
        self.toolbar = tb = self.CreateToolBar(wx.NO_BORDER | wx.TB_FLAT | wx.TB_HORZ_TEXT)
        ...
        tb.AddSeparator()
        tb.AddLabelTool(wx.ID_SAVEAS, 'Export as XHTML', get_tb_bitmap(wx.ART_FILE_SAVE_AS))
    ...
    def OnToolOpen(self, event):
        ...
        dlg = wx.FileDialog(self, message='Choose a file to load', defaultDir=os.getcwd(), defaultFile='', wildcard=FILE_WILDCARD, style=wx.OPEN)
        ...
        try:
            ...
            errdlg = wx.MessageDialog(self, str(e), 'Error loading data file', wx.OK | wx.ICON_ERROR)
            errdlg.ShowModal()
            errdlg.Destroy()
    ...
    def OnToolExportXhtml(self, event):
        ...
        """
        wildcard = 'XHTML files (*.html, *.xhtml)|*.html;*.xhtml|' + FILE_WILDCARD
        dlg = wx.FileDialog(self, message='Choose a filename to save to', defaultDir=os.getcwd(), defaultFile='', wildcard=wildcard, style=wx.SAVE | wx.OVERWRITE_PROMPT)
        if dlg.ShowModal() == wx.ID_OK:
        ...
        dlg = wx.MessageDialog(self, 'Done.', 'The tree has been exported as XHTML to "%s".', wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
...
class TreePanel(wx.Panel):
    ...
    def prepare_icons(self):
        ...
        """
        icon_names = ('harddisk', 'report_view', 'folder', 'folder_open', 'normal_file')
        self.image_list = wx.ImageList(*TREE_ICON_SIZE)
        self.bitmap_ids = {}
        for name in icon_names:
            self.bitmap_ids[name] = self.image_list.Add(GetBitmap(getattr(wx, 'ART_%s' % name.upper()), wx.ART_OTHER, TREE_ICON_SIZE))
    ...
    def set_item(self, item, image_key_normal, image_key_expanded=None, data=None):
        ...
        self.tree.SetItemImage(item, self.bitmap_ids[image_key_normal], wx.TreeItemIcon_Normal)
        self.tree.SetItemImage(item, self.bitmap_ids[image_key_expanded], wx.TreeItemIcon_Expanded)
        return
