...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\py_mysql\lib\mysql\connector\django\operations.py
...
class DatabaseOperations(BaseDatabaseOperations):
    ...
    if django.VERSION >= (1, 7):
        integer_field_ranges = dict((BaseDatabaseOperations.integer_field_ranges), PositiveSmallIntegerField=(0,
                                                                                                              4294967295),
        ...
        def date_extract_sql(self, lookup_type, field_name):
            if lookup_type == 'week_day':
                ...
                return 'EXTRACT({0} FROM {1})'.format(lookup_type.upper(), field_name)
        ...
        """
            ...
            try:
                ...
                sql = "CAST(DATE_FORMAT({0}, '{1}') AS DATETIME)".format(field_name, format_str)
            return sql
        ...
        def datetime_extract_sql(self, lookup_type, field_name, tzname):
            ...
            if lookup_type == 'week_day':
                ...
                sql = 'EXTRACT({0} FROM {1})'.format(lookup_type.upper(), field_name)
            return (
             sql, params)
        ...
        def datetime_trunc_sql(self, lookup_type, field_name, tzname):
            ...
            try:
                ...
                format_str = ''.join([f for f in format_[:i]] + [f for f in format_def[i:]])
                sql = "CAST(DATE_FORMAT({0}, '{1}') AS DATETIME)".format(field_name, format_str)
            return (
             sql, params)
        ...
        if django.VERSION >= (1, 8):
            ...
            """
                return (
                 "INTERVAL '%d 0:0:%d:%d' DAY_MICROSECOND" % (
            ...
            def date_interval_sql(self, sql, connector, timedelta):
                """Returns SQL for calculating date/time intervals
            """
                fmt = "({sql} {connector} INTERVAL '{days} 0:0:{secs}:{msecs}' DAY_MICROSECOND)"
                return fmt.format(sql=sql,
                  connector=connector,
                  days=(timedelta.days),
    ...
    def sql_flush(self, style, tables, sequences, allow_cascade=False):
        if tables:
            ...
            for table in tables:
                sql.append('{keyword} {table};'.format(keyword=(style.SQL_KEYWORD('TRUNCATE')),
    ...
    if django.VERSION < (1, 8):
        ...
        def sequence_reset_by_name_sql(self, style, sequences):
            ...
            if self.connection.mysql_version < (5, 0, 13):
                fmt = '{alter} {table} {{tablename}} {auto_inc} {field};'.format(alter=(style.SQL_KEYWORD('ALTER')),
