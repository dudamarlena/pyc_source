...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\py_mysql\lib\mysql\connector\pooling.py
...
CNX_POOL_NAMEREGEX = re.compile('[^a-zA-Z0-9._:\\-*$#]')
...
class PooledMySQLConnection(object):
    ...
    def config(self, **kwargs):
        ...
        raise errors.PoolError('Configuration for pooled connections should be done through the pool itself.')
...
class MySQLConnectionPool(object):
    __doc__ = 'Class defining a pool of MySQL connections'
    ...
    def set_config(self, **kwargs):
        ...
        with CONNECTION_POOL_LOCK:
            try:
                ...
                raise errors.PoolError('Connection configuration not valid: {0}'.format(err))
    ...
    def _queue_connection(self, cnx):
        ...
        """
        if not isinstance(cnx, MySQLConnection):
            raise errors.PoolError('Connection instance not subclass of MySQLConnection.')
        try:
    ...
    def add_connection(self, cnx=None):
        ...
        with CONNECTION_POOL_LOCK:
            if not self._cnx_config:
                raise errors.PoolError('Connection configuration not available')
            else:
                ...
                if not cnx:
                    ...
                    try:
                        if self._reset_session:
                            if self._cnx_config['compress']:
                                if cnx.get_server_version() < (5, 7, 3):
                                    raise errors.NotSupportedError('Pool reset session is not supported with compression for MySQL server version 5.7.2 or earlier.')
                    except KeyError:
                        pass

                    cnx._pool_config_version = self._config_version
                elif not isinstance(cnx, MySQLConnection):
                    raise errors.PoolError('Connection instance not subclass of MySQLConnection.')
