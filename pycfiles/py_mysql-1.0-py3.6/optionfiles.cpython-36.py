...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\py_mysql\lib\mysql\connector\optionfiles.py
# Compiled at: 2017-12-07 02:34:36
# Size of source mod 2**32: 13885 bytes
"""Implements parser to parse MySQL option files.
"""
import codecs, io, os, re
                    ...
                    try:
                        if option == 'socket':
                            ...
                            if option in CNX_FABRIC_ARGS:
                                if option not in fabric_options or fabric_options[option][1] <= value[1]:
                                    fabric_options[option] = value
                                continue
                            if option not in CNX_POOL_ARGS:
                                if option not in ('fabric', 'failover'):
...
class MySQLOptionsParser(SafeConfigParser):
    __doc__ = 'This class implements methods to parse MySQL option files'
    ...
    def __init__(self, files=None, keep_dashes=True):
        ...
        cannot be found.
        """
        self.OPTCRE = re.compile('(?P<option>[^:=\\s][^:=]*)\\s*(?:(?P<vi>[:=])\\s*(?P<value>.*))?$')
        self._options_dict = {}
        if PY2:
        ...
        for file_ in files:
            try:
                if file_ in files[index + 1:]:
                    raise ValueError("Same option file '{0}' occurring more than once in the list".format(file_))
                    ...
                    for line in op_file.readlines():
                        if line.startswith('!includedir'):
                            ...
                            for entry in os.listdir(dir_path):
                                ...
                                if os.path.isfile(entry) and entry.endswith(self.default_extension):
                ...
                raise ValueError("Failed reading file '{0}': {1}".format(file_, str(exc)))

        read_files = self.read(files)
        not_read_files = set(files) - set(read_files)
        if not_read_files:
            raise ValueError('File(s) {0} could not be read.'.format(', '.join(not_read_files)))
            ...
            try:
                ...
                for line in codecs.open(filename, encoding='utf-8'):
                    ...
                    if not self.SECTCRE.match(line) and match_obj:
                        optname, delimiter, optval = match_obj.group('option', 'vi', 'value')
                        if optname:
                            if not optval:
                                if not delimiter:
                                    out_file.write(line + '=\n')
            ...
            try:
                ...
                for group in self._sections.keys():
                    ...
                    for option, value in self._sections[group].items():
                        self._options_dict[group][option] = (
                         value, priority)
    ...
    def get_groups(self, *args):
        ...
        for group in args:
            try:
                for option, value in [(key, value) for key, value in self._options_dict[group].items() if key != '__name__' if not key.startswith('!')]:
    ...
    def get_groups_as_dict_with_priority(self, *args):
        ...
        for group in args:
            try:
                options[group] = dict((key, value) for key, value in self._options_dict[group].items() if key != '__name__' if not key.startswith('!'))
            except KeyError:
                pass
    ...
    def get_groups_as_dict(self, *args):
        ...
        for group in args:
            try:
                options[group] = dict((key, value[0]) for key, value in self._options_dict[group].items() if key != '__name__' if not key.startswith('!'))
            except KeyError:
                pass
