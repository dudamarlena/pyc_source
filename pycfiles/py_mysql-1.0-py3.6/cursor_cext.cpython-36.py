...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\py_mysql\lib\mysql\connector\cursor_cext.py
# Compiled at: 2017-12-07 02:34:36
# Size of source mod 2**32: 26151 bytes
"""Cursor classes using the C Extension
"""
from collections import namedtuple
import re, weakref
from _mysql_connector import MySQLInterfaceError
from .abstracts import MySQLConnectionAbstract, MySQLCursorAbstract, NAMED_TUPLE_CACHE
from .catch23 import PY2, isunicode
from . import errors
from .errorcode import CR_NO_RESULT_SET
from .cursor import RE_PY_PARAM, RE_SQL_INSERT_STMT, RE_SQL_ON_DUPLICATE, RE_SQL_COMMENT, RE_SQL_INSERT_VALUES, RE_SQL_SPLIT_STMTS
...
class CMySQLCursor(MySQLCursorAbstract):
    __doc__ = 'Default cursor for interacting with MySQL using C Extension'
    _raw = False
    _buffered = False
    ...
    def _fetch_warnings(self):
        ...
        try:
            ...
            raise errors.get_mysql_exception(msg=(exc.msg), errno=(exc.errno), sqlstate=(exc.sqlstate))
        except Exception as err:
            raise errors.InterfaceError('Failed getting warnings; {0}'.format(str(err)))
    ...
    def _handle_warnings(self):
        ...
        if self._cnx.get_warnings is True:
    ...
    def _handle_result(self, result):
        ...
        if self._cnx.raise_on_warnings is True:
            if self._warnings:
    ...
    def execute(self, operation, params=(), multi=False):
        ...
        if not operation:
            ...
            if not self._cnx:
                ...
                try:
                    result = self._cnx.cmd_query(stmt, raw=(self._raw), buffered=(self._buffered),
    ...
    def executemany(self, operation, seq_params):
        ...
        if not operation or not seq_params:
            ...
            if not isinstance(seq_params, (list, tuple)):
                raise errors.ProgrammingError('Parameters for query must be list or tuple.')
            ...
            try:
                ...
                raise errors.ProgrammingError('Failed executing the operation; {0}'.format(err))
    ...
    def callproc(self, procname, args=()):
        ...
        if not procname or not isinstance(procname, str):
            ...
            try:
                ...
                result = self._cnx.cmd_query(call, raw=(self._raw), raw_as_string=(self._raw_as_string))
                results = []
    ...
    def fetchmany(self, size=1):
        ...
        if rows:
            ...
            if not self._nextrow:
                if not self._cnx.more_results:
                    self._cnx.free_result()
            rows or self._handle_eof()
            return []
        else:
            self._rowcount += len(rows)
            return rows
...
class CMySQLCursorBuffered(CMySQLCursor):
    __doc__ = 'Cursor using C Extension buffering results'
...
class CMySQLCursorRaw(CMySQLCursor):
    __doc__ = 'Cursor using C Extension return raw results'
    _raw = True


class CMySQLCursorBufferedRaw(CMySQLCursorBuffered):
    __doc__ = 'Cursor using C Extension buffering raw results'
    _raw = True


class CMySQLCursorDict(CMySQLCursor):
    __doc__ = 'Cursor using C Extension returning rows as dictionaries'
    _raw = False
...
class CMySQLCursorBufferedDict(CMySQLCursorBuffered):
    __doc__ = 'Cursor using C Extension buffering and returning rows as dictionaries'
    _raw = False
...
class CMySQLCursorNamedTuple(CMySQLCursor):
    __doc__ = 'Cursor using C Extension returning rows as named tuples'
...
class CMySQLCursorBufferedNamedTuple(CMySQLCursorBuffered):
    __doc__ = 'Cursor using C Extension buffering and returning rows as named tuples'
...
class CMySQLCursorPrepared(CMySQLCursor):
    __doc__ = 'Cursor using Prepare Statement\n    '

    def __init__(self, connection):
        super(CMySQLCursorPrepared, self).__init__(connection)
        raise NotImplementedError('Alternative: Use connection.MySQLCursorPrepared')