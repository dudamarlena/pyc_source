...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\py_mysql\execute_queries.py
# Compiled at: 2017-12-14 01:13:35
# Size of source mod 2**32: 11695 bytes
...
class ExcSql(object):
    ...
    def exc_sql(self):
        ...
        with MySQLDB(self.host, self.dst_db, self.myuser, self.mypass) as (mysqldb):
            ...
            for sql in sqls:
                if mysqldb.is_transacted:
                    if 'COMMIT' in sql.upper():
    ...
    def _confirm_before_commit(self, mysqldb: MySQLDB, dnr: delete_null_row.DeleteNullRow):
        ...
        if ans == 1:
            ...
            if ans == 1:
                ...
                if ans == 2:
                    ...
                    cfm_sqls = self.dnr.read_text_file(('{}'.format(r_dir_path)), encode=r_encoding)
    ...
    def _output_result(self, line: str, dir_path: str, encode: str):
        ...
        if not dir_path == 'コンソール':
            line += '\n'
            self.dnr.write_text_file(line, ('{}'.format(dir_path)), 'a', encode=encode)
        else:
            print(line)
...
def global_entry_point(self):
    ...
    argparser.add_argument('--hostname', metavar='<HOSTUSER>', type=str, required=False, default='127.0.0.1',
      help='接続対象となるホスト/IPを指定.')
    argparser.add_argument('-d', '--database', metavar='<DATABASE>', type=str, required=True,
      help='接続先のデータベース名を指定.')
    argparser.add_argument('-u', '--user', metavar='<USER>', type=str, required=True, help='接続するユーザ.')
    args = argparser.parse_args()
    password = getpass('{} ユーザのパスワード: '.format(args.user))
    excsql = ExcSql(host=(args.hostuser), dst_db=(args.database), myuser=(args.user),
      mypass=password)
    excsql.exc_sql()
