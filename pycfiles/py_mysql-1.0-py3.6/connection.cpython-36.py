...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\py_mysql\lib\mysql\connector\connection.py
# Compiled at: 2017-12-07 02:34:36
# Size of source mod 2**32: 40942 bytes
...
from .constants import ClientFlag, ServerCmd, ServerFlag, flag_is_set, ShutdownType, NET_BUFFER_LENGTH
from . import errors
from .conversion import MySQLConverter
from .cursor import CursorBase, MySQLCursor, MySQLCursorRaw, MySQLCursorBuffered, MySQLCursorBufferedRaw, MySQLCursorPrepared, MySQLCursorDict, MySQLCursorBufferedDict, MySQLCursorNamedTuple, MySQLCursorBufferedNamedTuple
...
class MySQLConnection(MySQLConnectionAbstract):
    __doc__ = 'Connection to a MySQL Server'
    ...
    def _do_handshake(self):
        ...
        try:
            ...
            raise errors.InterfaceError('Failed parsing handshake; {0}'.format(err))

        self._server_version = self._check_server_version(handshake['server_version_original'])
        if not handshake['capabilities'] & ClientFlag.SSL:
            self._client_flags &= ~ClientFlag.SSL
            if self._ssl.get('verify_cert'):
                raise errors.InterfaceError("SSL is required but the server doesn't support it", errno=2026)
        else:
    ...
    def _do_auth(self, username=None, password=None, database=None, client_flags=0, charset=33, ssl_options=None):
        ...
        if client_flags & ClientFlag.SSL:
            ...
            self._socket.switch_to_ssl(ssl_options.get('ca'), ssl_options.get('cert'), ssl_options.get('key'), ssl_options.get('verify_cert') or False, ssl_options.get('cipher'))
            self._ssl_active = True
        packet = self._protocol.make_auth(handshake=(self._handshake),
          username=username,
          password=password,
          database=database,
          charset=charset,
    ...
    def _auth_switch_request(self, username=None, password=None):
        ...
        if packet[4] == 254:
            if len(packet) == 5:
                raise errors.NotSupportedError('Authentication with old (insecure) passwords is not supported. For more information, lookup Password Hashing in the latest MySQL manual')
        if packet[4] == 254:
            new_auth_plugin, auth_data = self._protocol.parse_auth_switch_request(packet)
            auth = get_auth_plugin(new_auth_plugin)(auth_data,
              password=password, ssl_enabled=(self._ssl_active))
    ...
    def _get_connection(self, prtcls=None):
        ...
        if self.unix_socket:
            ...
            conn = MySQLTCPSocket(host=(self.server_host), port=(self.server_port),
    ...
    def _open_connection(self):
        ...
        self._do_handshake()
        self._do_auth(self._user, self._password, self._database, self._client_flags, self._charset_id, self._ssl)
    ...
    def _send_cmd(self, command, argument=None, packet_number=0, packet=None, expect_response=True, compressed_packet_number=0):
        ...
        try:
            self._socket.send(self._protocol.make_command(command, packet or argument), packet_number, compressed_packet_number)
    ...
    def _handle_load_data_infile(self, filename):
        ...
        try:
            ...
            raise errors.InterfaceError("File '{0}' could not be read".format(filename))
    ...
    def _handle_result(self, packet):
        ...
        if packet[4] == 251:
            ...
            for i in range(0, column_count):
                columns[i] = self._protocol.parse_column(self._socket.recv(), self.python_charset)
    ...
    def get_rows(self, count=None, binary=False, columns=None):
        ...
        try:
            if binary:
                rows = self._protocol.read_binary_result(self._socket, columns, count)
            else:
                rows = self._protocol.read_text_result((self._socket), (self._server_version),
                  count=count)
        ...
        if rows[(-1)] is not None:
            row = rows[(-1)]
            self._handle_server_status(row['status_flag'] if 'status_flag' in row else row['server_status'])
            self.unread_result = False
        return rows
    ...
    def cmd_init_db(self, database):
        ...
        Returns a dict()
        """
        return self._handle_ok(self._send_cmd(ServerCmd.INIT_DB, database.encode('utf-8')))
    ...
    def cmd_query(self, query, raw=False, buffered=False, raw_as_string=False):
        ...
        if self._have_next_result:
            raise errors.InterfaceError('Use cmd_query_iter for statements with multiple queries.')
        return result
    ...
    def cmd_process_kill(self, mysql_pid):
        ...
        Returns a dict()
        """
        return self._handle_ok(self._send_cmd(ServerCmd.PROCESS_KILL, int4store(mysql_pid)))
    ...
    def cmd_change_user(self, username='', password='', database='', charset=33):
        ...
        self.handle_unread_result()
        if self._compress:
            raise errors.NotSupportedError('Change user is not supported with compression.')
        packet = self._protocol.make_change_user(handshake=(self._handshake),
          username=username,
          password=password,
          database=database,
          charset=charset,
    ...
    def reset_session(self, user_variables=None, session_variables=None):
        ...
        if not self.is_connected():
            ...
            try:
                ...
                self.cmd_change_user(self._user, self._password, self._database, self._charset_id)
    ...
    def reconnect(self, attempts=1, delay=0):
        ...
        while counter != attempts:
            ...
            try:
                ...
                if counter == attempts:
                    msg = 'Can not reconnect to MySQL after {0} attempt(s): {1}'.format(attempts, str(err))
    ...
    def cursor(self, buffered=None, raw=None, prepared=None, cursor_class=None, dictionary=None, named_tuple=None):
        ...
        if cursor_class is not None:
            if not issubclass(cursor_class, CursorBase):
                raise errors.ProgrammingError('Cursor class needs be to subclass of cursor.CursorBase')
        ...
        try:
            ...
            raise ValueError('Cursor not available with given criteria: ' + ', '.join([args[i] for i in range(5) if cursor_type & 1 << i != 0]))
    ...
    def cmd_stmt_prepare(self, statement):
        ...
        if result['num_params'] > 0:
            for _ in range(0, result['num_params']):
                result['parameters'].append(self._protocol.parse_column(self._socket.recv(), self.python_charset))

            self._handle_eof(self._socket.recv())
        if result['num_columns'] > 0:
            for _ in range(0, result['num_columns']):
                result['columns'].append(self._protocol.parse_column(self._socket.recv(), self.python_charset))
    ...
    def cmd_stmt_execute(self, statement_id, data=(), parameters=(), flags=0):
        ...
        execute_packet = self._protocol.make_stmt_execute(statement_id, data, tuple(parameters), flags, long_data_used, self.charset)
    ...
    def cmd_stmt_close(self, statement_id):
        ...
        anything.
        """
        self._send_cmd((ServerCmd.STMT_CLOSE), (int4store(statement_id)), expect_response=False)
    ...
    def cmd_stmt_send_long_data(self, statement_id, param_id, data):
        ...
        try:
            ...
            while buf:
                ...
                self._send_cmd((ServerCmd.STMT_SEND_LONG_DATA), packet=packet, expect_response=False)
                total_sent += len(buf)
                buf = data.read(chunk_size)
    ...
    def cmd_reset_connection(self):
        ...
        if self._server_version < (5, 7, 3):
            raise errors.NotSupportedError('MySQL version 5.7.2 and earlier does not support COM_RESET_CONNECTION.')
