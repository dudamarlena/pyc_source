...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\py_mysql\lib\mysql\connector\protocol.py
# Compiled at: 2017-12-07 02:34:36
# Size of source mod 2**32: 27841 bytes
"""Implements the MySQL Client/Server protocol
"""
import struct, datetime
from decimal import Decimal
from .constants import FieldFlag, ServerCmd, FieldType, ClientFlag
...
class MySQLProtocol(object):
    __doc__ = 'Implements MySQL client/server protocol\n\n    Create and parses MySQL packets.\n    '
    ...
    def _auth_response(self, client_flags, username, password, database, auth_plugin, auth_data, ssl_enabled):
        ...
        if not password:
            ...
            try:
                ...
                raise errors.InterfaceError('Failed authentication: {0}'.format(str(exc)))
    ...
    def make_auth(self, handshake, username=None, password=None, database=None, charset=33, client_flags=0, max_allowed_packet=1073741824, ssl_enabled=False, auth_plugin=None):
        """Make a MySQL Authentication packet"""
        try:
            auth_data = handshake['auth_data']
            auth_plugin = auth_plugin or handshake['auth_plugin']
        except (TypeError, KeyError) as exc:
            raise errors.ProgrammingError('Handshake misses authentication info ({0})'.format(exc))
        ...
        packet = struct.pack('<IIB{filler}{usrlen}sx'.format(filler=('x' * 23),
          usrlen=(len(username_bytes))), client_flags, max_allowed_packet, charset, username_bytes)
        packet += self._auth_response(client_flags, username, password, database, auth_plugin, auth_data, ssl_enabled)
    ...
    def make_auth_ssl(self, charset=33, client_flags=0, max_allowed_packet=1073741824):
    ...
    def make_change_user(self, handshake, username=None, password=None, database=None, charset=33, client_flags=0, ssl_enabled=False, auth_plugin=None):
        ...
        packet = struct.pack('<B{usrlen}sx'.format(usrlen=(len(username_bytes))), ServerCmd.CHANGE_USER, username_bytes)
        packet += self._auth_response(client_flags, username, password, database, auth_plugin, auth_data, ssl_enabled)
        packet += self._connect_with_db(client_flags, database)
    ...
    def parse_handshake(self, packet):
        ...
        packet, res['server_version_original'] = utils.read_string((packet[5:]),
          end=b'\x00')
        res['server_threadid'], auth_data1, capabilities1, res['charset'], res['server_status'], capabilities2, auth_data_length = struct_unpack('<I8sx2sBH2sBxxxxxxxxxx', packet[0:31])
        ...
        if capabilities & ClientFlag.SECURE_CONNECTION:
            ...
            if capabilities & ClientFlag.PLUGIN_AUTH:
                if b'\x00' not in packet:
                    if res['server_version_original'].startswith('5.5.8'):
    ...
    def parse_ok(self, packet):
        ...
        try:
            ...
            ok_packet['status_flag'], ok_packet['warning_count'] = struct_unpack('<HH', packet[0:4])
            packet = packet[4:]
            if packet:
    ...
    def parse_column(self, packet, charset='utf-8'):
        ...
        packet, _ = utils.read_lc_string(packet)
        try:
            _, _, field_type, flags, _ = struct_unpack('<xHIBHBxx', packet)
        except struct.error:
    ...
    def parse_statistics(self, packet, with_header=True):
        ...
        res = {}
        if with_header:
            pairs = packet[4:].split(b'  ')
        else:
            pairs = packet.split(b'  ')
        for pair in pairs:
            try:
            ...
            try:
                ...
                try:
                    ...
                    raise errors.InterfaceError('{0} ({1}:{2}).'.format(errmsg, lbl, val))
    ...
    def _parse_binary_integer(self, packet, field):
        ...
        if field[1] == FieldType.TINY:
            ...
            if field[1] == FieldType.SHORT:
                format_ = 'h'
                length = 2
            else:
                if field[1] in (FieldType.INT24, FieldType.LONG):
                    format_ = 'i'
                    length = 4
                else:
                    if field[1] == FieldType.LONGLONG:
                        format_ = 'q'
                        length = 8
    ...
    def _parse_binary_timestamp(self, packet, field):
        ...
        value = None
        if length == 4:
            value = datetime.date(year=(struct_unpack('H', packet[1:3])[0]),
              month=(packet[3]),
              day=(packet[4]))
        else:
            if length >= 7:
                ...
                value = datetime.datetime(year=(struct_unpack('H', packet[1:3])[0]),
                  month=(packet[3]),
                  day=(packet[4]),
                  hour=(packet[5]),
                  minute=(packet[6]),
    ...
    def _parse_binary_time(self, packet, field):
        ...
        if data[0] == 1:
            days *= -1
        tmp = datetime.timedelta(days=days, seconds=(data[7]),
          microseconds=mcs,
          minutes=(data[6]),
          hours=(data[5]))
        return (
            ...
            if value <= 255:
                ...
                if value <= 65535:
                    ...
                    if value <= 4294967295:
                        ...
                        format_ = 'Q'
        return (
         struct.pack(format_, value), field_type, flags)
    ...
    def _prepare_binary_timestamp(self, value):
        ...
        if isinstance(value, datetime.datetime):
            packed = packed + utils.int1store(value.hour) + utils.int1store(value.minute) + utils.int1store(value.second)
    ...
    def _prepare_binary_time(self, value):
        ...
        if not isinstance(value, (datetime.timedelta, datetime.time)):
            ...
            if isinstance(value, datetime.timedelta):
                ...
                packed += utils.int4store(abs(value.days)) + utils.int1store(hours) + utils.int1store(mins) + utils.int1store(secs)
                mcs = value.microseconds
            else:
                packed += utils.int4store(0) + utils.int1store(value.hour) + utils.int1store(value.minute) + utils.int1store(value.second)
    ...
    def make_stmt_execute(self, statement_id, data=(), parameters=(), flags=0, long_data_used=None, charset='utf8'):
        ...
        if parameters and data:
            if len(data) != len(parameters):
                raise errors.InterfaceError('Failed executing prepared statement: data values does not match number of parameters')
            for pos, _ in enumerate(parameters):
                value = data[pos]
                flags = 0
                if value is None:
                    null_bitmap[(pos // 8)] |= 1 << pos % 8
                    types.append(utils.int1store(FieldType.NULL) + utils.int1store(flags))
                    continue
                else:
                    if pos in long_data_used:
                        ...
                        if isinstance(value, int):
                            packed, field_type, flags = self._prepare_binary_integer(value)
                            ...
                            if isinstance(value, str):
                                ...
                                if isinstance(value, bytes):
                                    ...
                                    if isinstance(value, Decimal):
                                        values.append(utils.lc_int(len(str(value).encode(charset))) + str(value).encode(charset))
                                        ...
                                        if isinstance(value, float):
                                            ...
                                            if isinstance(value, (datetime.datetime, datetime.date)):
                                                packed, field_type = self._prepare_binary_timestamp(value)
                                                values.append(packed)
                                            else:
                                                if isinstance(value, (datetime.timedelta, datetime.time)):
                                                    packed, field_type = self._prepare_binary_time(value)
                                                    values.append(packed)
                                                else:
                                                    raise errors.ProgrammingError("MySQL binary protocol can not handle '{classname}' objects".format(classname=(value.__class__.__name__)))
                types.append(utils.int1store(field_type) + utils.int1store(flags))

        packet = utils.int4store(statement_id) + utils.int1store(flags) + utils.int4store(iteration_count) + (b'').join([struct.pack('B', bit) for bit in null_bitmap]) + utils.int1store(1)
        for a_type in types:
            packet += a_type
