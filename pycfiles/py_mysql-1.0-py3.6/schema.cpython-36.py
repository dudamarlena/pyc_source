...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\py_mysql\lib\mysql\connector\django\schema.py
    ...
    def add_field(self, model, field):
        ...
        if self.skip_default(field):
            if field.default not in (None, NOT_PROVIDED):
                ...
                self.execute('UPDATE %(table)s SET %(column)s = %%s' % {'table':self.quote_name(model._meta.db_table), 
