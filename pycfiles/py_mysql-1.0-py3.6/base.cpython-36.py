...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\py_mysql\lib\mysql\connector\django\base.py
# Compiled at: 2017-12-07 02:34:36
# Size of source mod 2**32: 20924 bytes
...
try:
    ...
    raise ImproperlyConfigured('Error loading mysql.connector module: {0}'.format(err))
...
try:
    ...
    HAVE_CEXT = True
if version < (1, 1):
    raise ImproperlyConfigured('MySQL Connector/Python v1.1.0 or newer is required; you have %s' % mysql.connector.__version__)
else:
...
def adapt_datetime_with_timezone_support(value):
    if settings.USE_TZ:
        if timezone.is_naive(value):
            warnings.warn('MySQL received a naive datetime (%s) while time zone support is active.' % value, RuntimeWarning)
...
class DjangoMySQLConverter(MySQLConverter):
    __doc__ = 'Custom converter for Django for MySQLConnection'
...
class DjangoCMySQLConverter(MySQLConverterBase):
    __doc__ = 'Custom converter for Django for CMySQLConnection'
...
class CursorWrapper(object):
    ...
    def _execute_wrapper(self, method, query, args):
        ...
        try:
            ...
            six.reraise(utils.ProgrammingError, utils.ProgrammingError(err.msg), sys.exc_info()[2])
        except mysql.connector.IntegrityError as err:
            six.reraise(utils.IntegrityError, utils.IntegrityError(err.msg), sys.exc_info()[2])
        except mysql.connector.OperationalError as err:
            if err.args[0] in self.codes_for_integrityerror:
                six.reraise(utils.IntegrityError, utils.IntegrityError(err.msg), sys.exc_info()[2])
            else:
                six.reraise(utils.DatabaseError, utils.DatabaseError(err.msg), sys.exc_info()[2])
...
class DatabaseWrapper(BaseDatabaseWrapper):
    vendor = 'mysql'
    _data_types = {'AutoField':'integer AUTO_INCREMENT', 
    ...
    operators = {'exact':'= %s', 
     ...
     'istartswith':'LIKE %s', 
     'iendswith':'LIKE %s'}
    pattern_esc = "REPLACE(REPLACE(REPLACE({}, '\\\\', '\\\\\\\\'), '%%', '\\%%'), '_', '\\_')"
    pattern_ops = {'contains':"LIKE BINARY CONCAT('%%', {}, '%%')", 
        ...
        for table_name in table_names:
            ...
            if not primary_key_column_name:
                ...
                for column_name, referenced_table_name, referenced_column_name in key_columns:
                    cursor.execute(ref_query.format(primary_key_column_name, column_name, table_name, referenced_table_name, column_name, referenced_column_name, column_name, referenced_column_name))
                    for bad_row in cursor.fetchall():
                        msg = "The row in table '{0}' with primary key '{1}' has an invalid foreign key: {2}.{3} contains a value '{4}' that does not have a corresponding value in {5}.{6}.".format(table_name, bad_row[0], table_name, column_name, bad_row[1], referenced_table_name, referenced_column_name)
