...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\py_mysql\lib\mysql\connector\cursor.py
# Compiled at: 2017-12-07 02:34:36
# Size of source mod 2**32: 47023 bytes
"""Cursor classes
"""
...
SQL_COMMENT = '\\/\\*.*?\\*\\/'
RE_SQL_COMMENT = re.compile('({0})|(["\'`][^"\'`]*?({0})[^"\'`]*?["\'`])'.format(SQL_COMMENT), re.I | re.M | re.S)
RE_SQL_ON_DUPLICATE = re.compile('\\s*ON\\s+DUPLICATE\\s+KEY(?:[^"\'`]*["\'`][^"\'`]*["\'`])*[^"\'`]*$', re.I | re.M | re.S)
RE_SQL_INSERT_STMT = re.compile('({0}|\\s)*INSERT({0}|\\s)*INTO\\s+[`\'\\"]?.+[`\'\\"]?(?:\\.[`\'\\"]?.+[`\'\\"]?){{0,2}}\\s+VALUES\\s*\\(.+(?:\\s*,.+)*\\)'.format(SQL_COMMENT), re.I | re.M | re.S)
RE_SQL_INSERT_VALUES = re.compile('.*VALUES\\s*(\\(.*\\)).*', re.I | re.M | re.S)
RE_PY_PARAM = re.compile(b'(%s)')
RE_PY_MAPPING_PARAM = re.compile(b'\n    %\n    \\((?P<mapping_key>[^)]+)\\)\n    (?P<conversion_type>[diouxXeEfFgGcrs%])\n    ', re.X)
...
class _ParamSubstitutor(object):
    ...
    def __call__(self, matchobj):
        ...
        try:
            ...
            raise errors.ProgrammingError('Not enough parameters for the SQL statement')
...
class MySQLCursor(CursorBase):
    ...
    def _process_params_dict(self, params):
        ...
        try:
            ...
            raise errors.ProgrammingError('Failed processing pyformat-parameters; %s' % err)
        else:
            return res
    ...
    def _process_params(self, params):
        ...
        try:
            ...
            raise errors.ProgrammingError('Failed processing format-parameters; %s' % err)
        else:
            return tuple(res)
    ...
    def _handle_noresultset(self, res):
        ...
        try:
            ...
            raise errors.ProgrammingError('Failed handling non-resultset; {0}'.format(err))

        self._handle_warnings()
        if self._connection.raise_on_warnings is True:
            if self._warnings:
                raise errors.get_mysql_exception(self._warnings[0][1], self._warnings[0][2])
            ...
            if not self._connection:
                ...
                if params is not None:
                    if isinstance(params, dict):
                        ...
                        if psub.remaining != 0:
                            raise errors.ProgrammingError('Not all parameters were used in the SQL statement')
                self._executed = stmt
                if multi:
                    self._executed_list = []
                ...
                try:
                    ...
                    if self._connection._have_next_result:
                        raise errors.InterfaceError('Use multi=True when executing multiple statements')
                    raise
    ...
    def _batch_insert(self, operation, seq_params):
        ...
        tmp = re.sub(RE_SQL_ON_DUPLICATE, '', re.sub(RE_SQL_COMMENT, remove_comments, operation))
        matches = re.search(RE_SQL_INSERT_VALUES, tmp)
        if not matches:
            raise errors.InterfaceError('Failed rewriting statement for multi-row INSERT. Check SQL syntax.')
        ...
        try:
            ...
            raise errors.InterfaceError('Failed executing the operation; %s' % err)
    ...
    def executemany(self, operation, seq_params):
        ...
        if not operation or not seq_params:
            ...
            try:
                ...
                raise errors.ProgrammingError('Parameters for query must be an Iterable.')
            ...
            try:
                ...
                raise errors.InterfaceError('Failed executing the operation; {0}'.format(err))
            except:
                raise
    ...
    def callproc(self, procname, args=()):
        ...
        if not procname or not isinstance(procname, str):
            ...
            try:
                ...
                if args:
                    for idx, arg in enumerate(args):
                        ...
                        if isinstance(arg, tuple):
                            argtypes.append(' CAST({0} AS {1})'.format(argname, arg[1]))
                ...
                raise errors.InterfaceError('Failed calling stored routine; {0}'.format(err))
    ...
    def _handle_warnings(self):
        ...
        if self._connection.get_warnings is True:
    ...
    def _fetch_row(self):
        ...
        if not self._have_unread_result():
            ...
            if self._nextrow == (None, None):
                row, eof = self._connection.get_row(binary=(self._binary),
                  columns=(self.description))
            else:
                row, eof = self._nextrow
            if row:
                self._nextrow = self._connection.get_row(binary=(self._binary),
                  columns=(self.description))
                eof = self._nextrow[1]
                if eof is not None:
    ...
    def fetchone(self):
        ...
        if row:
            if hasattr(self._connection, 'converter'):
                return self._connection.converter.row_to_python(row, self.description)
            else:
                return row
...
class MySQLCursorBuffered(MySQLCursor):
    __doc__ = 'Cursor which fetches rows within execute()'
        ...
        if self._rows is None:
            ...
            if hasattr(self._connection, 'converter'):
                for row in self._rows[self._next_row:]:
                    res.append(self._connection.converter.row_to_python(row, self.description))
...
class MySQLCursorPrepared(MySQLCursor):
    __doc__ = 'Cursor using MySQL Prepared Statements\n    '
    ...
    def execute(self, operation, params=(), multi=False):
        ...
        if operation is not self._executed:
            ...
            if self._prepared['parameters']:
                if not params:
                    return
            if len(self._prepared['parameters']) != len(params):
                raise errors.ProgrammingError(errno=1210,
                  msg='Incorrect number of arguments executing prepared statement')
        res = self._connection.cmd_stmt_execute((self._prepared['statement_id']),
          data=params,
    ...
    def executemany(self, operation, seq_params):
        ...
        try:
            ...
            raise errors.InterfaceError('Failed executing the operation; {error}'.format(error=err))
        except:
            raise
    ...
    def fetchall(self):
        ...
        while self._have_unread_result():
            if self._cursor_exists:
                self._connection.cmd_stmt_fetch(self._prepared['statement_id'], MAX_RESULTS)
            tmp, eof = self._connection.get_rows(binary=(self._binary),
              columns=(self.description))
            rows.extend(tmp)
            self._handle_eof(eof)
