...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\py_mysql\lib\mysql\connector\abstracts.py
# Compiled at: 2017-12-07 02:34:36
# Size of source mod 2**32: 39952 bytes
...
class MySQLConnectionAbstract(object):
    __doc__ = 'Abstract class for classes connecting to a MySQL server'
    ...
    def _read_option_files(self, config):
        ...
        if 'option_files' in config:
            ...
            if isinstance(config['option_files'], str):
                config['option_files'] = [
                 config['option_files']]
            option_parser = MySQLOptionsParser((list(config['option_files'])), keep_dashes=False)
                ...
                try:
                    for option, value in config_from_file[group].items():
                        try:
                            ...
                            if option not in config_options or config_options[option][1] <= value[1]:
                                config_options[option] = value
                        except KeyError:
                            if group is 'connector_python':
                                raise AttributeError("Unsupported argument '{0}'".format(option))
        ...
        if set_ssl_flag:
            if 'verify_cert' not in self._ssl:
                ...
                if bool('key' in self._ssl) != bool('cert' in self._ssl):
                    raise AttributeError('ssl_key and ssl_cert need to be both specified, or neither.')
                else:
                    if not set(('key', 'cert')) <= set(self._ssl):
                        self._ssl['key'] = None
                        self._ssl['cert'] = None
                    elif (self._ssl['key'] is None) != (self._ssl['cert'] is None):
                        raise AttributeError('ssl_key and ssl_cert need to be both set, or neither.')
    ...
    def _check_server_version(self, server_version):
        ...
        if isinstance(server_version, BYTE_TYPES):
            ...
            if 'fabric' in match.group(4).lower():
                if version < (1, 5):
                    raise errors.InterfaceError("MySQL Fabric '{0}' is not supported".format(server_version))
            elif version < (4, 1):
                raise errors.InterfaceError("MySQL Version '{0}' is not supported.".format(server_version))
        return version
    ...
    def set_client_flags(self, flags):
        ...
        if isinstance(flags, int):
            ...
            if isinstance(flags, (tuple, list)):
                ...
                raise errors.ProgrammingError('set_client_flags expect integer (>0) or set')
    ...
    def set_charset_collation(self, charset=None, collation=None):
        ...
        if charset:
            if isinstance(charset, int):
                self._charset_id, charset_name, collation_name = CharacterSet.get_charset_info(charset)
            else:
        ...
        self._execute_query("SET NAMES '{0}' COLLATE '{1}'".format(charset_name, collation_name))
        try:
        ...
        if self.in_transaction:
            ...
            if isolation_level:
                ...
                if level not in levels:
                    raise ValueError('Unknown isolation level "{0}"'.format(isolation_level))
                self._execute_query('SET TRANSACTION ISOLATION LEVEL {0}'.format(level))
            if readonly is not None:
                if self._server_version < (5, 6, 5):
                    raise ValueError('MySQL server version {0} does not support this feature'.format(self._server_version))
                else:
                    if readonly:
    ...
    def reset_session(self, user_variables=None, session_variables=None):
        ...
        try:
            ...
            if self._compress:
                raise errors.NotSupportedError('Reset session is not supported with compression for MySQL server version 5.7.2 or earlier.')
            else:
