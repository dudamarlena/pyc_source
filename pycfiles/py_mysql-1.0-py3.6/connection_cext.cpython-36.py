...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\py_mysql\lib\mysql\connector\connection_cext.py
# Compiled at: 2017-12-07 02:34:36
# Size of source mod 2**32: 20882 bytes
"""Connection class using the C Extension
"""
from . import errors
from .catch23 import INT_TYPES
from .constants import CharacterSet, FieldFlag, ServerFlag, ShutdownType, ClientFlag
from .abstracts import MySQLConnectionAbstract, MySQLCursorAbstract
from .protocol import MySQLProtocol
HAVE_CMYSQL = False
try:
    import _mysql_connector
    from .cursor_cext import CMySQLCursor, CMySQLCursorRaw, CMySQLCursorBuffered, CMySQLCursorBufferedRaw, CMySQLCursorPrepared, CMySQLCursorDict, CMySQLCursorBufferedDict, CMySQLCursorNamedTuple, CMySQLCursorBufferedNamedTuple
    from _mysql_connector import MySQLInterfaceError
except ImportError as exc:
    raise ImportError('MySQL Connector/Python C Extension not available ({0})'.format(str(exc)))
else:
    HAVE_CMYSQL = True

class CMySQLConnection(MySQLConnectionAbstract):
    __doc__ = 'Class initiating a MySQL Connection using Connector/C'
    ...
    def _do_handshake(self):
        ...
        self._handshake = {'protocol':self._cmysql.get_proto_info(), 
        ...
        self._server_version = self._check_server_version(self._handshake['server_version_original'])
    ...
    def _open_connection(self):
        ...
        if not self._ssl_disabled:
            cnx_kwargs.update({'ssl_ca':self._ssl.get('ca'), 
    ...
    def fetch_eof_columns(self):
        ...
        return {'eof':{'status_flag':self._server_status, 
    ...
    def fetch_eof_status(self):
        ...
        if self._cmysql:
            return {'warning_count':self._cmysql.st_warning_count(),  'field_count':self._cmysql.st_field_count(), 
    ...
    def cmd_query(self, query, raw=False, buffered=False, raw_as_string=False):
        ...
        try:
            ...
            self._cmysql.query(query, raw=raw,
              buffered=buffered,
              raw_as_string=raw_as_string)
        except MySQLInterfaceError as exc:
            raise errors.get_mysql_exception((exc.errno), msg=(exc.msg), sqlstate=(exc.sqlstate))
        except AttributeError:
            ...
            raise errors.OperationalError(errno=2055,
              values=(addr, 'Connection not available.'))
    ...
    def cursor(self, buffered=None, raw=None, prepared=None, cursor_class=None, dictionary=None, named_tuple=None):
        ...
        if cursor_class is not None:
            if not issubclass(cursor_class, MySQLCursorAbstract):
                raise errors.ProgrammingError('Cursor class needs be to subclass of cursor_cext.CMySQLCursor')
