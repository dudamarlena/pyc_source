...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/jinjatag/decorators.py
# Compiled at: 2012-03-28 23:00:44
...
def create_extension_decorator(cls):
    ...
    def outer_dec(name=None, *args, **kwargs):
        ...
        def dec(func):
            ...
            new_cls = type(tag_name, (cls,), {'tags': set([tag_name]), 
...
class BaseTag(Extension):
    ...
    def call_tag_func(self, *args, **kwargs):
        try:
            ...
            raise TypeError(('Failed to satisfy arguments for {0}({1}): provided ({2}).').format(iter(self.tags).next(), (', ').join(arg_list), (', ').join([ repr(arg) for arg in args ] + [ ('{0}={1}').format(k, repr(v)) for k, v in kwargs.items() ])))
...
class simple_block(BaseTag):
    ...
    def parse(self, parser):
        ...
        attrs = self.parse_attrs(parser)
        end_tags = [
         'name:end' + tag.value, 'name:end_' + tag.value]
        body = parser.parse_statements(end_tags, drop_needle=True)
        return [
         nodes.CallBlock(self.call_method('_call_simple_block', args=[attrs]), [], [], body).set_lineno(tag.lineno)]
...
class multibody_block(BaseTag):
    ...
    def parse(self, parser):
        ...
        end_tags = (
         end_tags_in_block, end_tags_outside_block)
        state = OUTSIDE_BLOCK
        attrs = self.parse_attrs(parser)
        ...
        node_list = [ nodes.CallBlock(self.call_method('_subblock', args=[block_name]), [], [], block).set_lineno(lineno) for block_name, block, lineno in blocks
                    ]
