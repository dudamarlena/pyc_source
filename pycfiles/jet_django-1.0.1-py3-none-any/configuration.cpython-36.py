...
# [GCC 8.4.0]
# Embedded file name: /Users/f1nal/Dropbox/python/jet-bridge/src/packages/jet_django/jet_django/configuration.py
# Compiled at: 2019-11-12 12:35:55
...
class JetDjangoConfiguration(Configuration):
    ...
    def get_hidden_model_description(self):
        return [
         '__jet__token', 'jet_django_token', 'django_migrations']
    ...
    def guess_display_field(self, model, fields):
        ...
        regexes = [
         'return\\s+self\\.(\\w+)',
         'return\\s+str\\(self\\.(\\w+)\\)',
         'return\\s+text_type\\(self\\.(\\w+)\\)',
         "return\\su?\\'%[sd]\\'\\s+%\\s+self\\.(\\w+)",
         "return\\s+u?\\'\\{0?\\}\\'\\.format\\(self.(\\w+)\\)"]
    ...
    def serialize_model(self, model):
        ...
        result = {'model':model._meta.db_table, 
    ...
    def serialize_field(self, field):
        result = {'db_column':field.get_attname_column()[1], 
        ...
        if field.default == timezone.now or field.default == datetime.now or getattr(field, 'auto_now', False) or getattr(field, 'auto_now_add', False):
            result['default_type'] = 'datetime_now'
        else:
            if field.default is None or isinstance(field.default, (str, bool, int, float)):
        ...
        if hasattr(field, 'choices'):
            if field.choices:
                if len(field.choices) > 0:
                    ...
                    result['params'] = {'options': list(map(lambda x: {'value':self.serializable(x[0]), 
                                 'name':six.text_type(x[1])}, field.choices))}
        if not field.editable:
            if not field.null:
                if result.get('default_type') is None:
    ...
    def media_url(self, path, request):
        ...
        if not django_settings.MEDIA_URL.startswith('http://'):
            if not django_settings.MEDIA_URL.startswith('https://'):
