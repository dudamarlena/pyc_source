...
# [GCC 8.4.0]
# Embedded file name: /Users/f1nal/Dropbox/python/jet-bridge/src/packages/jet_django/jet_django/route_view.py
...
class BaseRouteView(generic.View):
    ...
    def before_dispatch(self):
        ...
        self.view.request = Request(self.request.method.upper(), self.request.scheme, self.request._get_raw_host(), self.request.path, self.kwargs, self.request.get_full_path(), dict(self.request.GET.lists()), self.request_headers(), self.request.body, dict(self.request.POST.lists()), self.request_files())
    ...
    def write_response(self, response):
        if isinstance(response, RedirectResponse):
            ...
            if isinstance(response, OptionalJSONResponse) and isinstance(response.data, HttpResponseBase):
