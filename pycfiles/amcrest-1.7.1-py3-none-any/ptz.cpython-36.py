...
class Ptz(object):
    ...
    def move_directly(self, channel=1, startpoint_x=None, startpoint_y=None, endpoint_x=None, endpoint_y=None):
        """
        ...
        """
        if startpoint_x is None or startpoint_y is None or endpoint_x is None or endpoint_y is None:
            raise RuntimeError('Required args, start_point_x, start_point_yend_point_x and end_point_y not speficied')
        ret = self.command('ptzBase.cgi?action=moveDirectly&channel={0}&startPoint[0]={1}&startPoint[1]={2}&endPoint[0]={3}&endPoint[1]={4}'.format(channel, startpoint_x, startpoint_y, endpoint_x, endpoint_y))
        return ret.content.decode('utf-8')