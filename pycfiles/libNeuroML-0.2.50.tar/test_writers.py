...
# [GCC 8.4.0]
# Embedded file name: /home/padraig/libNeuroML/neuroml/test/test_writers.py
...
class TestJSONWriter(unittest.TestCase):
    ...
    def setUp(self):
        ...
        transposed_arraymorph = am.ArrayMorphology(vertices=transposed_vertices, connectivity=connectivity)
        bigger_d = d + 0.5
        fatter_vertices = np.array([x, y, z, bigger_d]).T
        fatter_arraymorph = am.ArrayMorphology(vertices=fatter_vertices, connectivity=connectivity)
    ...
    def test_write_to_mongodb_expected(self):
        ...
        db_name = 'test_db_4'
        writer_method = neuroml.writers.JSONWriter.write_to_mongodb
        writer_method(neuroml_document=self.test_doc, db=db_name)
        loader_method = neuroml.loaders.JSONLoader.load_from_mongodb
        doc = loader_method(db=db_name, id=self.test_doc.id, host='localhost')
        ...
        self.assertEqual(connectivity_equal, None)
        self.assertEqual(physical_masks_equal, None)
        self.assertEqual(vertices_equal, None)
        return
