...
# [GCC 8.4.0]
# Embedded file name: /home/padraig/libNeuroML/neuroml/test/test_arraymorph.py
# Compiled at: 2019-11-06 05:47:09
import neuroml.arraymorph as am, neuroml, numpy as np, neuroml.writers as writers, neuroml.loaders as loaders
try:
...
class TestObjectBuiltMorphology(unittest.TestCase):
    ...
    def setUp(self):
        ...
        seg_id = 1
        for i in range(100):
            p = neuroml.Point3DWithDiam(x=parent_segment.distal.x, y=parent_segment.distal.y, z=parent_segment.distal.z, diameter=0.1)
            d = neuroml.Point3DWithDiam(x=parent_segment.distal.x + 10, y=parent_segment.distal.y, z=parent_segment.distal.z, diameter=0.1)
...
class TestArrayMorphology(unittest.TestCase):
    ...
    def setUp(self):
        ...
        physical_mask[2] = 1
        physical_mask[20] = 1
        self.complex_morphology = am.ArrayMorphology(vertices=vertices, connectivity=connectivity, physical_mask=physical_mask, id='test_arraymorph')
        self.valid_vertices = [
         [
          0, 0, 0, 0.1],
         [
         ...
         [
          3, 0, 0, 0.4]]
        self.valid_connectivity = [
         -1, 0, 1, 2]
        self.optimized_morphology = am.ArrayMorphology(vertices=self.valid_vertices, connectivity=self.valid_connectivity, id='test_arraymorph')
        proximal_point = neuroml.Point3DWithDiam(x=0.1, y=0.2, z=0.3, diameter=1.1)
        distal_point = neuroml.Point3DWithDiam(x=0.0, y=0.0, z=0.0, diameter=1.1)
        soma = neuroml.Segment(proximal=proximal_point, distal=distal_point)
    ...
    def test_add_segment_vertices_added(self):
        proximal_point = neuroml.Point3DWithDiam(x=0.1, y=0.2, z=0.3, diameter=0.1)
    ...
    def test_to_root(self):
        new_morphology = am.ArrayMorphology(self.optimized_morphology.vertices, self.optimized_morphology.connectivity)
    ...
    def test_large_arraymorph(self):
        ...
        vertices = np.array([x, y, z, d]).T
        connectivity = range(-1, num_segments)
        big_arraymorph = am.ArrayMorphology(vertices=vertices, connectivity=connectivity)
