...
# [GCC 8.4.0]
# Embedded file name: /home/padraig/libNeuroML/neuroml/test/test_cell.py
    ...
    def test_cell_methods(self):
        ...
        for cell_name in cells:
            ...
            for grp in ['soma_group', ['soma_group', 'basal_dends'], ['dendrite_group'], ['all']]:
                print '-----------------------------'
                print '   Testing %s...' % grp
                segs, cuml, path_prox, path_dist = cell.get_ordered_segments_in_groups(grp, check_parentage=grp == ['all'], include_cumulative_lengths=True, include_path_lengths=True)
                ...
                if grp == ['all']:
                    if not len(segs['all']) == 9:
                        raise AssertionError
                        all_len = cuml['all'][(-1)]
                    assert grp == ['dendrite_group'] and len(segs['dendrite_group']) == 8
