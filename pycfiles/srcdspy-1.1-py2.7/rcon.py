...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/srcdspy/rcon.py
# Compiled at: 2012-12-28 01:37:01
import socket, re
...
class SourceRcon(object):
    ...
    def _send(self, command, message):
        self.req_id += 1
        raw = pack_long(self.req_id) + pack_long(command) + message + ('').join([MESSAGE_TERMINATOR] * 2)
