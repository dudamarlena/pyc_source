...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/Products/GroupUserFolder/tests/testGroupUserFolderAPI.py
...
class TestGroupUserFolderAPI(GRUFTestCase.GRUFTestCase, testInterface.TestInterface):
    ...
    def test_getPureUserNames(self):
        ...
        users = ['manager', 'u1', 'u2', 'u3', 'u4', 'u5', 'u6', 'u7', 'u8', 'u9', 'u10', 'u11']
        un.sort()
        users.sort()
        for u in users:
            self.failUnless(u in un, "Invalid users list: '%s' is not in acl_users." % (u,))
    ...
    def test_getPureUsers(self):
        ...
        """
        users = [
         'manager', 'u1', 'u2', 'u3', 'u4', 'u5', 'u6', 'u7', 'u8', 'u9', 'u10', 'u11']
    ...
    def test_getGroupNames(self):
        ...
        users = ['g1', 'g2', 'g3', 'g4', 'ng1', 'ng2', 'ng3', 'ng4', 'ng5', 'extranet', 'intranet', 'compta']
        un.sort()
        users.sort()
        for u in users:
            self.failUnless(u in un, "Invalid users list: '%s' is not in acl_users." % (u,))
    ...
    def test_getGroupIds(self):
        ...
        users = ['group_g1', 'group_g2', 'group_g3', 'group_g4', 'group_ng1', 'group_ng2', 'group_ng3', 'group_ng4', 'group_ng5', 'group_extranet', 'group_intranet', 'group_compta']
        un.sort()
        users.sort()
        for u in users:
            self.failUnless(u in un, "Invalid users list: '%s' is not in acl_users." % (u,))
    ...
    def test_getGroupById(self):
        ...
        self.failUnless(grp is None)
        self.failUnlessEqual(self.gruf.getGroupById('ZORGLUB'), None)
    ...
    def test_getGroupByName(self):
        ...
        self.failUnlessEqual(self.gruf.getGroupByName('ZORGLUB'), None)
        self.failUnless(self.gruf.getGroupByName('ZORGLUB', default='bla') == 'bla')
        return

    def test_userFolderAddUser(self):
        self.gruf.userFolderAddUser(name='created_user', password='secret', roles=[], groups=[], domains=())
        self.failUnless(self.gruf.getUser('created_user'))
        self.gruf.userFolderAddUser(name='group_test_prefix', password='secret', roles=[], groups=[], domains=())
    ...
    def test_userFolderEditUser(self):
        self.gruf.userFolderEditUser(name='u1', password='secret2', roles=['r1'], groups=['g1'], domains=())
        self.compareRoles(None, 'u1', ['r1'])
        return

    def test_userFolderUpdateUser(self):
        self.gruf.userFolderUpdateUser(name='u5', roles=['r2'])
        self.compareRoles(None, 'u5', ['r1', 'r2'])
        self.compareGroups('u5', ['g2', 'g3'])
        self.gruf.userFolderUpdateUser(name='u6', roles=None)
        self.compareRoles(None, 'u6', ['r1', 'r2'])
        return

    def test_userFolderDelUsers(self):
        self.gruf.userFolderAddUser(name='created_user', password='secret', roles=[], domains=(), groups=[])
    ...
    def test_userFolderAddGroup(self):
        self.gruf.userFolderAddGroup(name='created_group', roles=[], groups=[])
        self.failUnless(self.gruf.getGroup('created_group'))
        self.gruf.userFolderAddGroup(name='group_test_prefix', roles=[], groups=[])
    ...
    def test_userFolderEditGroup(self):
        self.gruf.userFolderAddGroup(name='created_group', roles=[], groups=[])
        self.gruf.userFolderEditGroup(name='created_group', roles=['r1'], groups=['group_g1'])
        self.compareRoles(None, 'created_group', ['r1'])
        self.failUnless('g1' in self.gruf.getGroupByName('created_group').getAllGroupNames(), self.gruf.getGroupByName('created_group').getAllGroupNames())
        self.gruf.userFolderEditGroup(name='created_group', roles=['r1'], groups=['g2'])
        self.failUnless('g2' in self.gruf.getGroupByName('created_group').getAllGroupNames(), self.gruf.getGroupByName('created_group').getAllGroupNames())
        return

    def test_userFolderUpdateGroup(self):
        self.gruf.userFolderAddGroup(name='created_group', roles=[], groups=[])
        self.gruf.userFolderUpdateGroup(name='created_group', roles=['r1'], groups=['group_g1'])
        self.compareRoles(None, 'created_group', ['r1'])
        self.failUnless('g1' in self.gruf.getGroupByName('created_group').getAllGroupNames(), self.gruf.getGroupByName('created_group').getAllGroupNames())
        self.gruf.userFolderUpdateGroup(name='created_group', roles=['r1'], groups=None)
        self.failUnless('g1' in self.gruf.getGroupByName('created_group').getAllGroupNames(), self.gruf.getGroupByName('created_group').getAllGroupNames())
        return
    ...
    def test_userSetPassword(self):
        ...
        try:
            self.gruf.userSetPassword('g1', 'bloub')
        except ValueError:
            pass
        else:
        ...
        try:
            self.gruf.userSetPassword('group_g1', 'bloub')
        except ValueError:
            pass
        else:
            raise 'AssertionError', 'Should raise'
    ...
    def test_userSetGroups(self):
        self.gruf.userFolderAddUser(name='created_user', password='secret', domains=(), roles=(), groups=[])
    ...
    def test_userAddGroup(self):
        self.gruf.userFolderAddUser(name='created_user', password='secret', groups=['g2'], roles=(), domains=())
    ...
    def test_userRemoveGroup(self):
        ...
        """
        self.gruf.userFolderAddUser(name='created_user', password='secret', groups=['g2', 'g1'], domains=(), roles=())
        self.gruf.userRemoveGroup('created_user', 'g1')
        self.compareGroups('created_user', ['g2'])

    def test_searchUsersByAttribute(self):
        self.failUnlessRaises(NotImplementedError, self.gruf.searchUsersByAttribute, 'a', 'b')

    def test_searchUsersByName(self):
        self.failUnlessEqual(self.gruf.searchUsersByName('u3'), [
         'u3'])
        self.failUnlessEqual(self.gruf.searchUsersByName('U3'), [
         'u3'])
    ...
    def test_searchUsersById(self):
        self.failUnlessEqual(self.gruf.searchUsersById('u3'), [
         'u3'])
        self.failUnlessEqual(self.gruf.searchUsersById('U3'), [
         'u3'])
    ...
    def test_searchGroupsByAttribute(self):
        self.failUnlessRaises(NotImplementedError, self.gruf.searchGroupsByAttribute, 'a', 'b')
    ...
    def test_searchGroupsById(self):
        self.failUnlessEqual(self.gruf.searchGroupsById('g5'), [
         'group_ng5'])
        self.failUnlessEqual(self.gruf.searchGroupsById('G5'), [
         'group_ng5'])
    ...
    def test_getUsersOfRole(self):
        should_be = ['group_ng2', 'group_ng3', 'group_ng4', 'group_ng5', 'u9', 'u5', 'u4', 'u7', 'u6', 'u11', 'u10', 'group_g3', 'group_g4']
        should_be.sort()
    ...
    def test_userFolderGetRoles(self):
        ...
        """
        should_be = [
         'Anonymous', 'Authenticated', 'Manager', 'Owner', 'r1', 'r2', 'r3', 'test_role_1_']
        should_be.sort()
    ...
    def test_userFolderAddRole(self):
        ...
        self.failUnless('r9' in self.gruf.userFolderGetRoles())
    ...
    def test_userFolderDelRoles(self):
        ...
        self.failUnless('r9' in self.gruf.getRolesOfUser('u1'))
        self.failUnless('r9' in self.gruf.getRolesOfUser('u4'))
        self.gruf.userFolderDelRoles(['r9'])
        self.failUnless('r9' not in self.gruf.getRolesOfUser('u1'))
        self.failUnless('r9' not in self.gruf.getRolesOfUser('u4'))

    def test_getMemberIds(self):
        should_be = [
         'group_ng2', 'group_ng3', 'group_ng4', 'group_ng5', 'u9', 'u5', 'u4', 'u6', 'u11', 'u10']
        should_be.sort()
    ...
    def test_getGroupMemberIds(self):
        should_be = [
         'group_ng2', 'group_ng3', 'group_ng4', 'group_ng5']
        should_be.sort()
    ...
    def test_getAllLocalRoles(self):
        normal_allowed = {'group_g1': ['r3'], 'u6': ['r2', 'r3'], 'test_user_1_': ['Owner'], 'u3': ['r2', 'r3']}
        blocked_allowed = {'u6': ['r2'], 'test_user_1_': ['Owner'], 'u3': ['r2']}
        ob = self.sublr2
