...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/Products/GroupUserFolder/GroupsTool.py
...
class GroupsTool(UniqueObject, SimpleItem, ActionProviderBase):
    ...
    if hasattr(ActionProviderBase, '__implements__'):
        __implements__ = (
         IGroupsTool, ActionProviderBase.__implements__)
    id = 'portal_groups'
    ...
    manage_options = ({'label': 'Configure', 'action': 'manage_config'},) + ActionProviderBase.manage_options + ({'label': 'Overview', 'action': 'manage_overview'},) + SimpleItem.manage_options
    ...
    def createGrouparea(self, id):
        ...
        if id and self.getGroupWorkspacesCreationFlag():
            if workspaces is None:
                pt.constructContent(type_name=self.getGroupWorkspaceContainerType(), container=parent, id=self.getGroupWorkspacesFolderId())
                workspaces = self.getGroupWorkspacesFolder()
                ...
                portal_catalog.unindexObject(workspaces)
