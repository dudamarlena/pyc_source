...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/nosejs/nosejs.py
# Compiled at: 2009-09-14 11:00:57
...
class RhinoJavaScriptTester(JavaScriptTester):
    ...
    def __init__(self, stream, config):
        ...
        cmd = [
         config.java_bin, '-jar', config.rhino_jar]
    ...
    def load_files(self, files):
        ...
        p = subprocess.Popen(cmd, env={'PATH': os.environ.get('PATH', None)}, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, cwd=self.config.nosejs_work_dir)
...
class SpidermonkeyJavaScriptTester(JavaScriptTester):
    ...
    def load_files(self, files):
        ...
        """
        if len(files) < 2:
            raise ValueError('files %s should contain at least a main program (i.e. main.js) and one other JavaScript to execite')
...
class NoseJS(Plugin):
    ...
    def options(self, parser, env=os.environ):
        ...
        parser.add_option('--with-javascript-only', dest='javascript_only', action='store_true', help='Activates the NoseJS plugin and prevents Nose from running any other tests.  Implies --with-javascript.')
        parser.add_option('--no-javascript-tests', dest='run_javascript_tests', default=True, action='store_false', help="Disables default behavior of looking for JavaScript test files and loading them if they match nose's testMatch (i.e. [tT]est.*\\.js)")
        parser.add_option('--no-javascript-lint', dest='run_javascript_lint', default=True, action='store_false', help='Disables default behavior of looking for JavaScript files and running them through the jsl command line tool to report lint errors and warnings.')
        parser.add_option('--jsl-bin', default='jsl', help='Path to jsl executable.  Default: %default (using $PATH)')
        parser.add_option('--jsl-opt', default=[], action='append', help='Additional command line options to send to the jsl executable.  You can specify this multiple times.')
        parser.add_option('--java-bin', default='java', help='Path to java executable.  Default: %default (using $PATH)')
        parser.add_option('--rhino-jar', help='Path to rhino1_7R1/js.jar (or later release). Download from http://www.mozilla.org/rhino/')
        parser.add_option('--rhino-jar-no-debug', dest='rhino_jar_debug', action='store_false', default=True, help='Do not pass the -debug flag to js.jar.  By default -debug is passed because it shows tracebacks.')
        parser.add_option('--spidermonkey', action='store_true', help='Run tests using the spidermonkey module from Python (this trumps --java-bin, --rhino-jar, and --rhino-no-debug) ')
        parser.add_option('--with-dom', action='store_true', help="Simulate the DOM by loading John Resig's env.js before any other JavaScript files.")
        parser.add_option('--load-js-lib', dest='javascript_libs_to_load', action='append', metavar='JS_FILEPATH', default=[], help='Path to a JavaScript file that should be loaded before the tests. This option can be specified multiple times.')
        parser.add_option('--javascript-dir', dest='javascript_dirs', action='append', metavar='JS_DIRECTORY', default=[], help='Path to where JavaScript tests live.  Must be a subdirectory of where Nose is already looking for tests (i.e. os.getcwd()).  This option can be specified multiple times.')
        possible_res = []
        ...
        parser.add_option('--load-js-resource', dest='resources_to_load', action='append', metavar='RELATIVE_FILEPATH', help='Relative path to internal nosejs resource file that should be loaded before tests (but after DOM initialization).  This option can be specified multiple times.  Possible values: %s' % (', ').join(possible_res))
        runner = get_resource('rhino-testrunner.js')
        if not os.path.exists(runner):
            runner = None
        parser.add_option('--rhino-testrunner', metavar='RHINO_TESTRUNNER', default=runner, help='JavaScript that runs tests in Rhino.  When test files are discovered they will be executed as java -jar rhino1_7R1/js.jar RHINO_TESTRUNNER <discovered_test_file>.  Default: %default')
        parser.add_option('--nosejs-work-dir', help='A path to change into before running any commands')
        return
    ...
    def configure(self, options, config):
        ...
        for dir in options.javascript_dirs:
            ...
            if not absdir.startswith(root):
                raise ValueError('Option --javascript-dir=%s must be somewhere along the working directory path (currently %s)' % (
                 absdir, root))
    ...
    def _configureJsLint(self, options, config):
        ...
        if not found_jsl:
            raise ConfigError('Could not find jsl binary for JavaScript lint ($PATH=%r).  Try setting an explicit path with --jsl-bin or else disable lint with --no-javascript-lint' % os.environ.get('PATH', ''))
    ...
    def _configureJsTesters(self, options, config):
        ...
        libs = [get_resource('__nosejs__.js')]
        if options.with_dom:
    ...
    def loadTestsFromFile(self, filename):
        ...
        if not self.options.run_javascript_lint:
            ...
            yield JsLintTestCase(filename, self.options.jsl_bin, jsl_options=self.options.jsl_opt, stop_on_error=self.config.stopOnError)
    ...
    def report(self, stream):
        if self.options.run_javascript_tests:
            ...
            files = [self.options.rhino_testrunner]
