...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/rbbz/bugzilla.py
# Compiled at: 2014-12-18 10:16:27
...
class Bugzilla(object):
    ...
    def log_in(self, username, password, cookie=False):
        if cookie:
            ...
            try:
                result = self.proxy.User.login({'login': username, 'password': password})
            except xmlrpclib.Fault, e:
                if e.faultCode == 300:
                    logging.error('Login failure for user %s: invalid username or password.' % username)
                    return
                if e.faultCode == 301:
                    logging.error('Login failure for user %s: user is disabled.' % username)
                    return
                raise
    ...
    @xmlrpc_to_bugzilla_errors
    def post_rb_url(self, bug_id, review_id, summary, comment, url, reviewers):
        ...
        for a in attachments:
            ...
            for f in a.get('flags', []):
                if f['name'] != 'review' and f['name'] != 'feedback':
                    continue
                if f['name'] == 'review' and 'requestee' in f and f['requestee'] in reviewers:
                    flags.append({'id': f['id'], 'name': 'review', 'status': '?', 
        ...
        for r in sorted(reviewers):
            flags.append({'name': 'review', 'status': '?', 'requestee': r, 'new': True})
    ...
    def get_rb_attachments(self, bug_id):
        ...
        params = {'ids': [
                 bug_id], 
           'include_fields': [
                            'id', 'data', 'content_type', 'is_obsolete',
                            'flags']}
        return [ a for a in self.proxy.Bug.attachments(params)['bugs'][str(bug_id)] if a['content_type'] == 'text/x-review-board-request'
               ]
