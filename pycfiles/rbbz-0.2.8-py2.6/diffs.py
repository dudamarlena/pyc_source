...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/rbbz/diffs.py
# Compiled at: 2014-12-09 20:39:24
import HTMLParser
from reviewboard.diffviewer.diffutils import get_diff_files, populate_diff_chunks

def get_file_chunks_in_range_custom(context, filediff, interfilediff, first_line, num_lines):
    """
    ...
    if key in context:
        ...
        files = get_diff_files(filediff.diffset, filediff, interdiffset, request=request)
    ...
    for chunk in files[0]['chunks']:
        if 'headers' in chunk['meta'] and (chunk['meta']['headers'][0] or chunk['meta']['headers'][1]):
        ...
        if lines[(-1)][0] >= first_line >= lines[0][0]:
            ...
            new_chunk = {'lines': chunk['lines'][start_index:last_index], 
...
def render_comment_plain(comment, context, is_reply):
    ...
    chunks = list(get_file_chunks_in_range_custom(context, comment.filediff, comment.interfilediff, comment.first_line, comment.num_lines))
    lines = [
     '::: %s' % comment.filediff.dest_file_display]
    if comment.interfilediff:
        lines.append('(Diff revisions %s - %s)' % (
         comment.filediff.diffset.revision,
         comment.interfilediff.diffset.revision))
    else:
        lines.append('(Diff revision %s)' % comment.filediff.diffset.revision)
    for chunk in chunks:
...
def build_plaintext_review(review, url, context):
    ...
    for comment in review.comments.all():
        review_text.append(render_comment_plain(comment, context, review.is_reply()))
