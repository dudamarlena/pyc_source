...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/astrocalc/distances/converter.py
...
class converter(object):
    ...
    **Usage**
    ...
    def distance_to_redshift(self, mpc):
    ...
    def redshift_to_distance(self, z, WM=0.3, WV=0.7, H0=70.0):
        ...
        **Usage**
        ...
        n = 1000
        for i in range(n):
            a = az + old_div((1.0 - az) * (i + 0.5), n)
            adot = math.sqrt(WK + old_div(WM, a) + old_div(WR, math.pow(a, 2)) + WV * math.pow(a, 2))
            DCMR = DCMR + 1.0 / (a * adot)

        DCMR = old_div((1.0 - az) * DCMR, n)
        DCMR_Mpc = old_div(c, H0) * DCMR
        x = math.sqrt(abs(WK)) * DCMR
        if x > 0.1:
            if WK > 0.0:
                ratio = old_div(0.5 * (math.exp(x) - math.exp(-x)), x)
            else:
                ratio = old_div(math.sin(x), x)
        else:
            y = math.pow(x, 2)
            if WK < 0.0:
                y = -y
        ...
        DA = az * ratio * DCMR
        DA_Mpc = old_div(c, H0) * DA
        DA_scale = DA_Mpc / 206.264806
        DL = old_div(DA, math.pow(az, 2))
        DL_Mpc = old_div(c, H0) * DL
        ...
        z = '%0.*f' % (precision, z)
        results = {'dcmr_mpc': float(DCMR_Mpc), 
           'da_mpc': float(DA_Mpc), 
