...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/dxf2x/__init__.py
# Compiled at: 2019-10-15 11:20:04
...
def command_line_options():
    parser = ArgumentParser(prog='dxf2x', description='convert between DXF, TSV, SVG, PXF(Protobuf) formats.',
      epilog='Report bugs to <redraiment@gmail.com>.')
    parser.add_argument('files', metavar='FILE', nargs='+', help='All the files are read and converted in turn.')
    parser.add_argument('-s', '--source', metavar='FORMAT', choices=[
     'dxf', 'tsv', 'svg', 'pxf'],
      help='Specifies the format of the input. Determined by FILE by default.')
    parser.add_argument('-t', '--target', metavar='FORMAT', required=True, choices=[
     'dxf', 'tsv', 'svg', 'pxf'],
      help='Specifies the format of the output.')
    parser.add_argument('-d', '--dir', help='Specifies the directory for output files.')
    parser.add_argument('-q', '--quiet', action='store_true', default=False, help='Disable the progress.')
...
def dxf2x():
    ...
    readers = {'dxf':dxf.read, 
     'svg':svg.read, 
     'tsv':tsv.read, 
     'pxf':proto.read}
    writers = {'dxf':dxf.write, 
     'svg':svg.write, 
     'tsv':tsv.write, 
     'pxf':proto.write}
