...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/dxf2x/proto.py
# Compiled at: 2019-10-15 11:10:30
...
def to_block(block):
    yield {'0':'BLOCK', 
     '8':'1',  '2':block.name,  '70':'0',  '10':'0.0',  '20':'0.0'}
    entities = []
...
create_proto_rules = {'BLOCK':{'type':Block, 
  '2':(
   'name', str)}, 
 'INSERT':{'type':Insert, 
  '2':(
 ...
 'POLYLINE':{'type':Polyline, 
  ...
  '8':(
   'layer',
   lambda layer:    if layer.isdigit():
int(layer) # Avoid dead code: -1), 
  '70':(
   'closed', lambda flag: flag == '1')}, 
 'VERTEX':{'type':Vertex, 
  '10':(
   'x', float), 
  '20':(
 ...
 'TEXT':{'type':Text, 
  ...
  '50':(
   'rotate', float)}}
fill_proto_rules = {'blocks':{'BLOCK': {'collection':'blocks', 
            'children':{'entities': {'POLYLINE':{'collection':'polylines', 
                           'children':{'vertices': {'VERTEX': {'collection': 'vertices'}}}}, 
                          'LINE':{'collection': 'lines'}, 
                          'POINT':{'collection': 'points'}, 
                          'TEXT':{'collection': 'annotations'}}}}}, 
 'entities':{'INSERT':{'collection': 'inserts'}, 
  'TEXT':{'collection': 'attributes'}}}
index = 0

def create(entity):
    global create_proto_rules
    global index
    name = entity['0']
