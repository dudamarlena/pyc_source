...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/dxf2x/svg.py
# Compiled at: 2019-10-15 11:10:32
# Size of source mod 2**32: 9968 bytes
...
def to_block(block):
    yield {'0':'BLOCK', 
     '8':'1',  '2':block.attrib['data-name'],  '70':'0',  '10':'0.0',  '20':'0.0'}
    for entity in list(block):
        if entity.tag in {'polygon', 'polyline'}:
...
class BoundingBox:
    ...
    def merge(self, other):
        ...
        for name, compare in BoundingBox.directions.items():
            if not name in that or name not in this or compare(this[name], that[name]):
                this[name] = that[name]
...
class Shape:
    ...
    def getter(key, f=None):
        if f is not None:
            return lambda entity:             if key in entity:
f(entity[key]) # Avoid dead code: None
        return lambda entity: entity.get(key, None)

    getters = {'layer':getter('8', lambda layer: f"L{layer}"), 
     'rotate':getter('50')}
    rules = {'POLYLINE':{'tag':lambda entity:       if entity['70'] == '1':
'polygon' # Avoid dead code: 'polyline', 
      'attributes':{'class':getters['layer'],  'data-style':getter('6'), 
     ...
     'LINE':{'tag':'line', 
      'attributes':{'class':getters['layer'], 
       'x1':'10', 
       'y1':'20', 
       'x2':'11', 
       'y2':'21', 
