...
# [GCC 8.4.0]
# Embedded file name: /Users/juancomish/miniconda3/lib/python3.7/site-packages/pyehub/energy_hub/input_data.py
# Compiled at: 2019-07-30 13:57:16
# Size of source mod 2**32: 14795 bytes
"""
...
import pandas as pd
from data_formats.request_format import Capacity, Converter, Stream, Storage, TimeSeries, Network_links
from energy_hub.utils import cached_property
T = TypeVar('T')

class InputData:
    __doc__ = 'Provides convenient access to needed data to implement an energy hub\n    model.'
    ...
    def time_series_list(self) -> List[TimeSeries]:
        ...
        return [TimeSeries(time_series) for time_series in self._request['time_series']]

    @cached_property
    def streams(self) -> List[Stream]:
        """The list of streams."""
        return [Stream(stream, self._request) for stream in self._request['streams']]
    ...
    def import_streams(self) -> List[str]:
        ...
        return [stream.name for stream in self.streams if stream.importable]

    @cached_property
    def export_streams(self) -> List[str]:
        """The names of streams that are exportable."""
        return [stream.name for stream in self.streams if stream.exportable]

    @property
    def demands(self) -> Iterable[TimeSeries]:
        """Return the TimeSeries that are demands."""
        return (demand for demand in self.time_series_list if demand.is_demand)

    @property
    def source(self) -> Iterable[TimeSeries]:
        """Return the TimeSeries that are sources."""
        return (source for source in self.time_series_list if source.is_source)
    ...
    def time_series_data(self) -> Dict[(str, Dict[(int, float)])]:
        ...
        by time."""
        return {time_series.name:{t:time_series.data[t] for t in self.time} for time_series in self.time_series_list}
    ...
    def loads(self) -> Dict[(str, Dict[(int, float)])]:
        ...
        time series ID, time)."""
        return {demand.stream:{t:demand.data[t] for t in self.time} for demand in self.demands}
    ...
    def c_matrix(self) -> Dict[(str, Dict[(str, float)])]:
        ...
        """
        c_matrix = pd.DataFrame(0, index=(self.converter_names), columns=(self.stream_names),
          dtype=float)
    ...
    def part_load(self) -> Dict[(str, Dict[(str, float)])]:
        ...
        part_load = defaultdict(defaultdict)
        for tech in self.converters:
    ...
    def converters_capacity(self) -> Dict[(str, float)]:
        ...
        return {tech.name:tech.capacity for tech in self.converters}

    @property
    def stream_timeseries(self) -> Dict[(str, float)]:
        """Return the streams that have an availability timeseries."""
        return {stream.name:stream.timeseries for stream in self.streams if stream.timeseries if stream.timeseries}
    ...
    def linear_cost(self) -> Dict[(str, float)]:
        ...
        return {converter.name:converter.capital_cost for converter in self.converters}

    @cached_property
    def fixed_capital_costs(self) -> Dict[(str, float)]:
        """Return the fixed capital cost for each converter."""
        return {converter.name:converter.fixed_capital_cost for converter in self.converters}
    ...
    def _calculate_npv(self, lifetime: float) -> float:
        ...
        return 1 / (((1 + r) ** lifetime - 1) / (r * (1 + r) ** lifetime))

    @cached_property
    def tech_npv(self) -> Dict[(str, float)]:
        """The net present value of each converter."""
        return {tech.name:round(self._calculate_npv(tech.lifetime), 4) for tech in self.converters}

    @cached_property
    def variable_maintenance_cost(self) -> Dict[(str, float)]:
        """The variable maintenance cost of each converter."""
        return {tech.name:tech.usage_maintenance_cost for tech in self.converters}
    ...
    def fuel_price(self) -> Dict[(str, Union[(float, str)])]:
        ...
        return {stream.name:stream.price for stream in self.streams}

    @cached_property
    def feed_in(self) -> Dict[(str, Union[(float, str)])]:
        """The export price of each output stream."""
        return {stream.name:stream.export_price for stream in self.streams if stream.exportable if stream.exportable}
    ...
    def _get_from_storages(self, attribute: str) -> Dict[(str, T)]:
        ...
        return {storage.name:getattr(storage, attribute) for storage in self.storages}

    @cached_property
    def storage_npv(self) -> Dict[(str, float)]:
        """The net present value of each storage."""
        return {storage.name:self._calculate_npv(storage.lifetime) for storage in self.storages}
    ...
    def _get_from_links(self, attribute: str) -> Dict[(int, T)]:
        return {link.link_id:getattr(link, attribute) for link in self.links}
    ...
    def link_capacity(self) -> Dict[(int, float)]:
        """Return the capacities of the converters."""
        return {link.link_id:link.link_capacity for link in self.links}
