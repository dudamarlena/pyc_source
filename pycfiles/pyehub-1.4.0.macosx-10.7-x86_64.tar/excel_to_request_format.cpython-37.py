...
# [GCC 8.4.0]
# Embedded file name: /Users/juancomish/miniconda3/lib/python3.7/site-packages/pyehub/excel_to_request_format.py
# Compiled at: 2019-07-23 12:49:06
# Size of source mod 2**32: 14671 bytes
"""
...
class FormatUnsupportedError(Exception):
    __doc__ = 'The Excel format is not supported.'
...
class _NewFormatConverter(Converter):
    ...
    def _get_general(self):
        ...
        return {'interest_rate': float(sheet.cell(2, 1).value)}
    ...
    def _get_converters(self):
        ...
        for column in self._get_columns('Converters'):
            name, capacity, fixed_capital_cost, capital_cost, annual_maintenance_cost, usage_maintenance_cost, efficiency, lifetime, output_ratio, min_load, inputs, outputs = column
            if name not in ('#', ''):
                converter = {'name':name.strip(),  'efficiency':float(efficiency)}
                with suppress(ValueError):
                ...
                if inputs:
                    converter['inputs'] = [in_.strip() for in_ in inputs.split(',')]
                if outputs:
                    converter['outputs'] = [output.strip() for output in outputs.split(',')]
    ...
    def _get_storages(self):
        ...
        for column in self._get_columns('Storages'):
            name, stream, capacity, cost, annual_maintenance_cost, fixed_capital_cost, lifetime, charge_efficiency, discharge_efficiency, decay, max_charge, max_discharge, min_state = column
            if name not in ('#', ''):
                storage = {'name':name,  'stream':stream, 
                 'cost':float(cost), 
    ...
    def _get_time_series(self):
        ...
        for column in self._get_columns('Time series'):
            series_id, series_type, stream, node, units, source, *rest = column
            if series_id not in ('#', ''):
                data = rest[3:]
                time_series = {'id':series_id, 
                 'type':series_type, 
                 'stream':stream, 
                 'units':units, 
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Converts an excel file into a EHub Request format.')
