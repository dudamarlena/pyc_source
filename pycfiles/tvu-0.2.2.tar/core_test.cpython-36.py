...
# [GCC 8.4.0]
# Embedded file name: /home/paczesiowa/projects/tvu/tests/core_test.py
# Compiled at: 2017-04-25 15:14:13
...
class TVUTest(TestCase):
    ...
    def multiple_types_test(self):
        ...
        foo('a')
        foo([])
        with self.assertRaises(TypeError, 'x must be int, str or list, not None'):
            foo(None)
    ...
    def unification_test(self):
        ...
        self.assertTrue(isinstance(foo(1.0), float))
        with self.assertRaises(TypeError, 'x must be float or int, not None'):
            foo(None)
    ...
    def validation_test(self):
        ...
        with self.assertRaises(TypeError, 'x must be int, not None'):
            foo(None)
        with self.assertRaises(ValueError, 'x must be positive number, not: 0'):
            foo(0)
        with self.assertRaises(ValueError, 'x must be positive number, not: -1'):
            foo(-1)
    ...
    def enum_test(self):
        ...
        with self.assertRaises(TypeError, 'x must be SampleEnum, not None'):
            foo(None)
    ...
    def unicode_enum_test(self):
        ...
        self.assertEqual(foo(UnicodeEnum.foo), 'bar')
        self.assertEqual(foo('foo'), 'bar')
        err_msg = "x could be UnicodeEnum's variant name, not: " + repr('bar')
        with self.assertRaises(ValueError, err_msg):
            foo('bar')
    ...
    def nullable_complex_test(self):
        ...
        self.assertEqual(foo(1), 1.0)
        self.assertEqual(foo(2), 2.0)
        self.assertEqual(foo(None), 0.0)
        with self.assertRaises(TypeError, "x must be int, float or None, not 'a'"):
            foo('a')
        with self.assertRaises(ValueError, 'x must be None or positive number, not: 0'):
            foo(0)
        with self.assertRaises(ValueError, 'x must be None or positive number, not: 0.0'):
            foo(0.0)
