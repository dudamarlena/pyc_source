...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/customBurndownChart/embed.py
# Compiled at: 2011-09-19 06:42:18
"""
...
class BurndownChartEmbed(Component):
    ...
    __estimation_field = get_estimation_field()

    def __init__(self, *args, **kwargs):
        init_config(self.env, self.__estimation_field)
        if not self.env.config.has_option('components', 'estimationtools.*') or self.env.config.get('components', 'estimationtools.*') != 'enabled':
            self.log.error('TracLShiftBurndownChart(%s)::EstimationTools is not installed/enabled. Component disabled.' % (
    ...
    def _create_burndown_chart(self, args):
        ...
        params = {'milestone': milestone_name, 
           ...
           'width': 1000, 
           'height': 300}
        macro = '[[BurndownChart(%s)]]' % (',').join([ '%s=%s' % (key, str(value)) for key, value in params.iteritems() ])
        chart = HTML(tag.div(format_to_html(self.env, Context.from_request(req), macro), class_=self.__container_div_class)) | Transformer('.//p').unwrap()
        return chart

    def _do_filter_init(self, ms_selector, ms_id, req):
        """ Abstraction of common code for _do_{view,roadmap}_filters. """
        classes = (' or ').join([ "@class='%s'" % c for c in ['info', 'description'] ])
        content = '%(ms_selector)s/child::div[%(classes)s]' % locals()
        chart_transform = Transformer()
        ...
        if dates and has_hours:
            ...
            chart_transform = Transformer(chart_selector).append(tag.td(chart_macro))
    ...
    def _do_view_filter(self, req, method, filename, stream, data):
        ...
        transform = Transformer(content).cut(streambuffer, accumulate=True).buffer().end().select(content).remove().end().select('%(ms_selector)s/h1' % locals()).after(tag.table(tag.tr(tag.td(streambuffer)))).end()
    ...
    def _do_roadmap_filter(self, req, method, filename, stream, data):
        """
            ...
            using .render('html')
        """
        stream = stream | Transformer("//div[@class='milestones']").before(tag.br(clear='all'))
        ...
        current_milestones = zip(xrange(1, len(current_milestones) + 1), current_milestones)
        milestones_to_update = [ (no, name) for no, name in current_milestones if name in target_milestones
                               ]
        stream = HTML(html)
        for index, milestone_name in milestones_to_update:
            ms_selector = "//div[@class='milestone'][%(index)d]" % locals()
            content, chart_transform = self._do_filter_init(ms_selector, milestone_name, req)
            streambuffer = StreamBuffer()
            transform = Transformer(content).cut(streambuffer, accumulate=True).buffer().end().select(content).remove().end().select(ms_selector).append(tag.table(tag.tr(tag.td(streambuffer)))).end()
