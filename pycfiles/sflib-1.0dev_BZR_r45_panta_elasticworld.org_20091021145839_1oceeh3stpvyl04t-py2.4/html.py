...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/sflib/html.py
# Compiled at: 2008-04-30 03:08:15
"""
...
HTML_FULL_TAGS = (
 (
  'html', None, {'INLINE': False}), ('head', None, {'INLINE': False}), ('body', None, {'INLINE': False}), 'title', ('div', None, {'INLINE': False, 'NEWLINE': True}), 'span', 'p', 'b', 'i', 'tt', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'font', 'blockquote', 'ul', 'ol', 'li', 'dl', 'dt', 'dd', 'a', 'map', 'area', ('table', None, {'INLINE': False}), ('tr', None, {'INLINE': False}), ('td', {'colspan': 1, 'rowspan': 1}, {'INLINE': False}), ('th', None, {'INLINE': False}), 'caption', 'frameset', 'frame', 'base', 'noframes', 'form', 'input', 'select', 'option', 'textarea', 'meta', 'style', 'abbr', 'acronym', 'address', 'base', 'bdo', 'big', 'button', 'center', 'cite', 'code', 'col', 'colgroup', 'del', 'dfn', 'em', ('fieldset', None, {'INLINE': False}), 'iframe', 'ins', 'kbd', 'label', ('legend', None, {'INLINE': False}), 'noscript', 'object', 'optgroup', 'pre', 'q', 'samp', 'script', 'small', 'strong', 'sub', 'tbody', 'tfoot', 'thead', 'var')
HTML_EMPTY_TAGS = (
 (
  'br', None, {'NEWLINE': True}), 'hr', 'img', 'link')
...
class HTMLEntity(object):
    ...
    def _html(self, do_indent=False, indent_level=0, start_column=0):
        ...
        if self.NEWLINE:
            nl = True
        s_open = '<%s%s>' % (tag_open, make_html_attrs(attrs))
        s_close = ''
        if tag_close:
            s_close = '<%s>' % tag_close
        contents_s = s_open
        ...
        if not tag_close:
            ...
            if inner_contents_s:
                r = '<%s%s %s />' % (tag_open, make_html_attrs(attrs), unicode(inner_contents_s))
            else:
                r = '<%s%s />' % (tag_open, make_html_attrs(attrs))
            if nl:
                r = r + '\n'
            return (r, nl)
        assert tag_close
...
def _make_html_classes():
    ...
    for tag in HTML_FULL_TAGS:
        ...
        open_tag = tag
        close_tag = '/' + tag
        cls_attrs = {'NAME': tag.lower(), 'DEFAULT_ATTRIBUTES': default_attributes, 'OPENCLOSE_TAGS': (open_tag, close_tag)}
    ...
    for tag in HTML_EMPTY_TAGS:
        ...
        clsname = tag.upper()
        open_tag = tag
        newcls = type(clsname, (HTMLEntity,), {'NAME': tag.lower(), 'DEFAULT_ATTRIBUTES': default_attributes, 'OPENCLOSE_TAGS': (open_tag, '')})
        this_mod.__dict__[clsname] = newcls
        global_dict[clsname] = newcls
