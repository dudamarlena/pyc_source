...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/sflib/fileconf/configparser.py
...
class TokenParser(object):
    ...
    def _perform_var_subst(self, text):
        while True:
            ...
            text = re.sub('\\$\\{' + re.escape(varname) + '\\}', varvalue, text)
...
class ConfigParser(object):
    ...
    def Load(self):
        ...
        """
        import os, sys
        self.cnf = self.cnf or Config(initial={'ENV': os.environ, 'sys': {'executable': sys.executable, 'path': sys.path, 'platform': sys.platform, 'version': sys.version, 'subversion': sys.subversion, 'version_info': sys.version_info}, 'os': {'sep': os.path.sep, 'pathsep': os.path.pathsep, 'pardir': os.path.pardir, 'curdir': os.path.curdir}})
