...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/sflib/aviation.py
...
"""
import math
EARTH_RADIUS_FAI = 6371000.0
EARTH_RADIUS_WGS84_EQUATORIAL = 6378137.0
EARTH_RADIUS_WGS84_POLAR = 6356752.0
...
class Point(object):
    ...
    def distance(self, p2):
        ...
        """
        return math.acos(math.sin(self.lat_r) * math.sin(p2.lat_r) + math.cos(self.lat_r) * math.cos(p2.lat_r) * math.cos(self.lon_r - p2.lon_r))
    ...
    def distance_short(self, p2):
        ...
        The distance is in radians (range [0:1]).
        """
        return 2.0 * math.asin(math.sqrt(math.sin((self.lat_r - p2.lat_r) / 2.0) ** 2.0 + (math.cos(self.lat_r) * math.cos(p2.lat_r) * math.sin((self.lon_r - p2.lon_r) / 2.0)) ** 2.0))
    ...
    def course(self, p2):
        ...
        if math.cos(self.lat_r) < EPS:
            ...
            if math.sin(p2.lon_r - self.lon_r) < 0.0:
                tc = math.acos((math.sin(p2.lat_r) - math.sin(self.lat_r) * math.cos(d)) / (math.sin(d) * math.cos(self.lat_r)))
            else:
