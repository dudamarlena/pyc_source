...
# [GCC 8.4.0]
# Embedded file name: /home/wduss/src/github.com/dalloriam/engel/engel/widgets/base.py
...
class BaseContainer(BaseElement):
    ...
    def add_child(self, child):
        ...
        if self.view:
            if self.view.is_loaded:
                self.view.dispatch({'name':'append', 
                 'html':child.compile(), 
    ...
    def remove_child(self, child):
        ...
        if self.view:
            if self.view.is_loaded:
                self.view.dispatch({'name':'remove', 
                 'selector':'#' + child.id})
    ...
    def replace_child(self, old_child, new_child):
        for i, child in enumerate(self.children):
            if child is old_child:
                ...
                if self.view:
                    if self.view.is_loaded:
                        self.view.dispatch({'name':'replace', 
