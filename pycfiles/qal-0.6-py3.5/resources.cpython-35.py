...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/qal/common/resources.py
# Compiled at: 2016-04-12 13:41:36
# Size of source mod 2**32: 7720 bytes
"""
...
def resource_types():
    ...
    return [
     'CUSTOM', 'FLATFILE', 'MATRIX', 'XPATH', 'RDBMS', 'SPREADSHEET']
...
class Resource(object):
    ...
    def as_json_dict(self):
        ...
        for _curr_data_key, _curr_data_value in sorted(self.__dict__.items()):
            if _curr_data_value is not None and _curr_data_key[0] != '_' and _curr_data_key not in self._excluded_fields:
...
class Resources(Recurse):
    ...
    def __init__(self, _resources_list=None, _external_resources_callback=None, _base_path=None):
        """
    ...
    def parse_json(self, _resources_list=None):
        ...
        for _curr_resource in _resources_list:
            if 'uuid' in _curr_resource:
                ...
                self._debug_print('parse_json: Append resource: ' + str(_new_resource.caption) + ' uuid: ' + str(_new_resource.uuid) + ' type: ' + str(_new_resource.type), 4)
