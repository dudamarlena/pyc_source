...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/qal/common/discover.py
...
def import_error_to_help(_module, _err_obj, _pip_package, _apt_package, _win_package, _import_comment=None):
    ...
    if str(_err_obj) == "No module named '" + str(_module) + "'":
        ...
        _err_msg += [
         'Run sudo pip3 install ' + str(_pip_package) + empty_if_none(' or sudo apt-get install ' + str(_apt_package), _apt_package),
         'Either run pip install ' + str(_pip_package) + empty_if_none(', download and install ' + str(_win_package) + ' or install from source.', _win_package),
         'If available on your platform, run pip' + get_python_versions(_style='Minor') + ' install ' + str(_pip_package) + ' otherwise download and install from source.'][platform_to_int()]
...
def get_python_versions(_style=None):
    ...
    * build : 3.4.4 build 1300
    ...
    if _style.lower() == 'major':
        return str(_major)
    if _style.lower() == 'minor':
        return str(_major) + '.' + str(_minor)
    if _style.lower() == 'release':
