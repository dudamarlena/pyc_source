...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/qal/dataset/custom.py
...
class CustomDataset(object):
    ...
    def _structure_insert_row(self, _row_idx, _row_data, _commit=None, _no_logging=False):
        """Inserts a row at _row_idx in the self.data_table, containing _row_data        Commonly overridden by subclasses"""
        if not _no_logging:
    ...
    def _structure_update_row(self, _row_idx, _row_data, _commit=None, _no_logging=False):
        """Updates the row at _row_idx in the self.data_table with _row_data.        Commonly overridden by subclasses"""
        if not _no_logging:
    ...
    def _structure_delete_row(self, _row_idx, _commit=None, _no_logging=False):
        """Deletes a row at _row_idx from the self.data_table.        Commonly overridden by subclasses"""
        if not _no_logging:
            self.log_delete(_row_idx, self.data_table[_row_idx])
        self.data_table.pop(_row_idx)

    def _structure_init(self, _dataset):
        """Initialize underlying structure before applying data to it.        Overridden by subclasses."""
        pass
    ...
    def _structure_apply_merge(self, _insert, _update, _delete, _sorted_dest, _commit=True):
        ...
        if len(self.data_table) > 0:
            ...
            while _insert_idx > -1:
                self._structure_insert_row(0, _insert[_insert_idx][2], _commit=_commit)
                _insert_idx -= 1
    ...
    def apply_new_data(self, _new_data_table, _key_fields, _insert=None, _update=None, _delete=None, _commit=True):
        ...
        if len(self._structure_key_fields) > 0:
            _deletes, _inserts, _updates, _dest_sorted = compare(_left=_new_data_table, _right=self.data_table, _key_columns=_key_fields, _full=True)
        else:
            _inserts = [[0, 0, _new_data_table[_curr_idx]] for _curr_idx in range(len(_new_data_table) - 1, -1, -1)]
            _deletes = _updates = []
            _dest_sorted = []
        if _delete is None or _delete is False:
            _deletes = None
        if _insert is None or _insert is False:
            _inserts = None
        if _update is None or _update is False:
            _updates = None
        self.data_table = self._structure_apply_merge(_inserts, _updates, _deletes, _dest_sorted, _commit=_commit)
        return (
         self.data_table, _deletes, _inserts, _updates)
