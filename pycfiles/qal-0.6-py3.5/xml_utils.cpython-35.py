...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/qal/common/xml_utils.py
# Compiled at: 2016-04-12 13:41:36
# Size of source mod 2**32: 5632 bytes
"""
...
def xml_get_allowed_value(_node, _type):
    ...
    if value in _type[1] or value == '':
        return value
    if value[0:8].lower() == 'varchar(' and value[8:len(value) - 1].isnumeric() and value[(len(value) - 1)] == ')':
        return value
...
class XMLTranslation(Recurse):
    ...
    def get_root_node(self, _nodename, _xml='', _node=None):
        ...
        elif _node.nodeName != _nodename and _node.nodeName != self._add_own(_nodename):
