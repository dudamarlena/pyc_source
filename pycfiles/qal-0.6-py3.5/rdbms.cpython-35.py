...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/qal/dataset/rdbms.py
# Compiled at: 2016-04-12 13:41:36
# Size of source mod 2**32: 10772 bytes
"""
...
class RDBMSDataset(CustomDataset):
    __doc__ = 'The RDMBS Dataset holds a two-dimensional array of data, typically representing a table in a database.\n    If the data set is not a table but based on a more complex query, data will not be possible to apply to it.'
    _dal = None
    table_name = None
    query = None
    ...
    def _structure_init(self, _dataset):
        ...
        if len(self.field_types) == 0:
            if len(_dataset) == 0:
                raise Exception('RDBMSDataset.structure_init error: The _dataset cannot be empty for field types to be derived from it.')
            try:
                self.field_types = [python_type_to_sql_type(type(_curr_value)) for _curr_value in _dataset[0]]
                _create_table_sql = create_table_skeleton(self.table_name, self.field_names, self.field_types).as_sql(self._dal.db_type)
    ...
    def _rdbms_init_deletes(self):
        ...
        for _field_idx in self._structure_key_fields:
            _new_cond = ParameterCondition(_left=ParameterIdentifier(_identifier=self.field_names[_field_idx]), _right=ParameterParameter(_datatype=self.field_types[_field_idx]), _operator='=', _and_or='AND')
    ...
    def _rdbms_init_inserts(self):
        ...
        self._structure_insert_sql = make_insert_sql_with_parameters(self.table_name, self.field_names, self._dal.db_type, self.field_types)
    ...
    def load(self):
        if self.table_name and self.table_name != '':
            ...
            self.data_table = self._dal.query(select_all_skeleton(self.table_name, self.field_names).as_sql(self._dal.db_type))
