...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/qal/sql/meta_queries.py
...
"""
_table_info_postgresql = "SELECT a.attname AS COLUMN_NAME, t.typname AS DATA_TYPE,       CASE WHEN a.attlen > -1 THEN a.attlen  ELSE a.atttypmod END - 4 AS DATA_LENGTH, NOT a.attnotnull AS NULLABLE FROM pg_class c, pg_attribute a, pg_type t WHERE c.relname = ':TABLENAME'  AND a.attnum > 0  AND a.attrelid = c.oid  AND a.atttypid = t.oid ORDER BY a.attnum;"
_table_info_mysql = "SELECT COLUMN_NAME, DATA_TYPE, IS_NULLABLE AS NULLABLE, CHARACTER_MAXIMUM_LENGTH AS DATA_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=':DATABASENAME' AND TABLE_NAME = ':TABLENAME'"
_table_info_oracle = "SELECT COLUMN_NAME, DATA_TYPE, NULLABLE, DATA_LENGTH FROM user_tab_columns WHERE table_name=':TABLENAME'"
_table_info_db2 = "SELECT NAME AS COLUMN_NAME, COLTYPE AS DATA_TYPE, NULLS AS NULLABLE, LENGTH AS DATA_LENGTH FROM SYSIBM.SYSCOLUMNS WHERE UPPER(TBNAME) = UPPER(':TABLENAME');"
_table_info_sqlserver = "SELECT clmns.name AS [COLUMN_NAME],usrt.name AS [DATA_TYPE],clmns.is_nullable AS [NULLABLE],usrt.max_length AS [DATA_LENGTH]FROM sys.tables AS tbl INNER JOIN sys.all_columns AS clmns ON clmns.object_id=tbl.object_id LEFT OUTER JOIN sys.types AS usrt ON usrt.user_type_id = clmns.user_type_id WHERE (tbl.name=':TABLENAME') ORDER BY clmns.column_id ASC"
table_list_sqlserver = ''
_table_list_postgresql_by_database_name = "SELECT  table_name FROM information_schema.tables WHERE table_type = 'BASE TABLE'     AND table_schema = 'public'     AND table_catalog = ':DATABASE' ORDER BY table_type, table_name "
...
_table_list_db2_by_schema = "select NAME from sysibm.systables where CREATOR  = ':USER' and type = 'T';"
...
def _table_info_sql(_db_type, _table_name, _database_name):
    ...
    _sql = [
     _table_info_mysql, _table_info_postgresql, _table_info_oracle, _table_info_db2, _table_info_sqlserver][_db_type]
...
class MetaQueries(object):
    __doc__ = 'The meta queries class collects methods for gathering meta data about a database.'
