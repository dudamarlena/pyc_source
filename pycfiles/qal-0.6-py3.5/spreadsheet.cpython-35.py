...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/qal/dataset/spreadsheet.py
# Compiled at: 2016-04-12 13:41:36
# Size of source mod 2**32: 12138 bytes
"""
...
class SpreadsheetDataset(CustomDataset):
    __doc__ = 'The Spreadsheet dataset can read data from a spreadsheet, currently only Excel files, and store that data in its.\n    '
    has_header = None
    filename = None
    field_names = None
    sheet_name = None
    ...
    def load_xls(_filename, _sheet_name, _x_offset, _y_offset, _header_offset):
        ...
        try:
            ...
            raise Exception(import_error_to_help(_module='xlrd', _err_obj=_err, _pip_package='xlrd', _apt_package=None, _win_package=None))
    ...
    def load_xlsx(_filename, _sheet_name, _x_offset, _y_offset, _header_offset):
        ...
        try:
            ...
            raise Exception(import_error_to_help(_module='openpyxl', _err_obj=_err, _pip_package='openpyxl', _apt_package=None, _win_package=None))
        ...
        try:
            ...
            if _header_offset > 0:
                _field_names = [_sheet.cell(column=_x, row=1 + _y_offset).value for _x in range(1 + _x_offset, _number_of_columns)]
            else:
            ...
            for _curr_y in range(1 + _y_offset + _header_offset, _number_of_rows):
                _data_table.append([_sheet.cell(column=_x, row=_curr_y).value for _x in range(1 + _x_offset, _number_of_columns)])
    ...
    def load(self):
        ...
        if _extension.lower() in ('.xls', '.xlsx'):
            self.data_table, self.field_names = self.load_xls(_filename=_filename, _sheet_name=self.sheet_name, _x_offset=_x_offset, _y_offset=_y_offset, _header_offset=_header_offset)
        else:
            if _extension.lower() == '.odt':
    ...
    def save_xlsx(self, _filename, _data_table, _number_of_rows, _number_of_columns, _x_offset, _y_offset, _header_offset, _field_names, _sheet_name):
        ...
        try:
            ...
            for _curr_row_idx in range(0, _number_of_rows - 1):
                for _curr_col_idx in range(0, _number_of_columns):
                    _sheet.cell(row=1 + _curr_row_idx + _y_offset + _header_offset, column=1 + _curr_col_idx + _x_offset).value = _data_table[_curr_row_idx][_curr_col_idx]

            _workbook.save(filename=_filename)
        except IOError as e:
            raise Exception('SpreadsheetDataset.save_xlsx: Error saving file:' + str(e))

    def save_xls(self, _filename, _data_table, _number_of_rows, _number_of_columns, _x_offset, _y_offset, _header_offset, _field_names, _sheet_name):
        print('SpreadsheetDataset.save_xls: using xlwt')
        try:
            import xlwt
        except ImportError as _err:
            raise Exception(import_error_to_help(_module='xlwt', _err_obj=_err, _pip_package='xlwt', _apt_package=None, _win_package=None))
        ...
        try:
            ...
            for _curr_row_idx in range(0, _number_of_rows - 1):
                for _curr_col_idx in range(0, _number_of_columns):
                    _sheet.write(_curr_row_idx + _y_offset + _header_offset, _curr_col_idx + _x_offset, label=_data_table[_curr_row_idx][_curr_col_idx])
    ...
    def save(self, _save_as=None):
        ...
        if _extension.lower() == '.xls':
            self.save_xls(_filename, _data_table=self.data_table, _number_of_rows=_number_of_rows, _number_of_columns=_number_of_columns, _x_offset=self.x_offset, _y_offset=self.y_offset, _header_offset=_header_offset, _field_names=self.field_names, _sheet_name=self.sheet_name)
        else:
            if _extension.lower() == '.xlsx':
                self.save_xlsx(_filename, _data_table=self.data_table, _number_of_rows=_number_of_rows, _number_of_columns=_number_of_columns, _x_offset=self.x_offset, _y_offset=self.y_offset, _header_offset=_header_offset, _field_names=self.field_names, _sheet_name=self.sheet_name)
            else:
                if _extension.lower() == '.odt':
                    raise Exception('SpreadsheetDataset.load: Open Document Spreadsheet not implemented yet.')
                else:
