...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/qal/sql/json.py
# Compiled at: 2016-04-12 13:41:36
# Size of source mod 2**32: 15376 bytes
"""
...
from qal.sql.types import sql_property_to_type, and_or, constraint_types, index_types, verbs, set_operator, join_types, in_types, quoting_types, data_source_types
...
class SQLJSON(Recurse):
    ...
    def _get_child_types(self):
        ...
        def make_one_of(_classes):
            return {'type': 'object', 
             'anyOf': [{'properties': {x: {'$ref': '#/definitions/' + x}}} for x in _classes]}
    ...
    def generate_schema(self):
        ...
        _result = {'$schema': 'http://json-schema.org/draft-04/schema#', 
         ...
         'properties': {'statement': {'$ref': '#/definitions/statement'}, 
                        'resources': {'items': {'$ref': 'qal://resources.json#/definitions/Resource'}, 
        ...
        for _curr_class in list_prefixed_classes(globals(), 'parameter'):
            _result['definitions'].update({_curr_class: {'type': 'object', 
        ...
        if _object is not None and _object != '':
            ...
            if hasattr(_object, '__dict__'):
                ...
                for _curr_property_name, _curr_property_value in sorted(_object.__dict__.items()):
                    if _curr_property_name.lower() not in ('row_separator', ) and not hasattr(_curr_property_value, '__call__') and _curr_property_name[0:1] != '_':
                        if isinstance(_curr_property_value, list):
                            _content[_curr_property_name] = self._list_to_dict(_curr_property_value)
                        else:
                            if hasattr(_curr_property_value, 'as_sql'):
                                ...
                                if str(_curr_property_value).isnumeric() and len(_curr_type) > 1:
                                    ...
                                    if _curr_property_value is not None:
                                        _content[_curr_property_name] = str(_curr_property_value)
                                    else:
    ...
    def json_get_allowed_value(_value, _type):
        ...
        if _value in _type[1] or _value == '':
            return _value
        if _value[0:8].lower() == 'varchar(' and _value[8:len(_value) - 1].isnumeric() and _value[(len(_value) - 1)] == ')':
            return _value
    ...
    def _parse_attribute(self, _attribute_name, _attribute_value, _parent_obj):
        ...
        if hasattr(_obj, 'as_sql'):
            ...
            self._debug_print('_parse_class_json: Found matching Parameter class for ' + _attribute_name + ' : ' + _obj_name)
        else:
        ...
        for _curr_item_key, _curr_obj in _obj.__dict__.items():
            if _curr_item_key != 'row_separator' and _curr_item_key in _attribute_value:
                ...
                if isinstance(_curr_obj, list):
                    ...
                    if _curr_type[0].lower() in ('string', 'datatypes', 'boolean',
                                                 'integer', 'float', 'number'):
                        _obj.__dict__[_curr_item_key] = _curr_value
                    else:
                        if _curr_type[0:5] == 'verb_' or _curr_type[0:10] == 'parameter_':
                            raise Exception('_parse_attribute: Strange error happened parsing VERB/PARAMETER.. parent: ' + _parent_obj.__class__.__name__ + 'Class: ' + _attribute_name + ' Currtype: ' + _curr_type)
                        else:
                            if len(_curr_type) > 1 and type(_curr_type[1]) == list:
        ...
        if self._resources and hasattr(_obj, 'resource_uuid') and _obj.resource_uuid is not None and _obj.resource_uuid != '':
            _obj._resource = self._resources.get_resource(_obj.resource_uuid)
            self._debug_print('_parse_attribute: Added resource_uuid for ' + _obj_name + ': ' + _obj.resource_uuid, 1)
