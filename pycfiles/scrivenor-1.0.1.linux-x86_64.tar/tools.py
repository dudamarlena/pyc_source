...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/scrivenor/tools.py
# Compiled at: 2018-11-11 13:13:03
import sys, os
from distutils import util
from datetime import date
import time, glob, xml.etree.ElementTree as ET
...
class PortableDocument(object):
    ...
    def __init__(self, image_filepath, structdoc_filepath, element, endpage, prev_barcode):
        cfg = load_config()
        self.cfg = cfg
        self.image_filepath = image_filepath
        self.structdoc_filepath = structdoc_filepath
        self.stemname = Filesphere(filepath=image_filepath).get_stemname()
        self.events_registry = EventsRegistry(cfg['dbhost'], cfg['dbname'], cfg['dbuser'], cfg['dbpass'], cfg['events_tablename'], cfg['events_primarykey'], cfg['timestamp_format'])
        ...
        self.portable_document_filepath = ('{}/{}-{}.{}').format(self.today_portabledoc_dir, self.stemname, self.startpage, self.cfg['portabledoc_ext'])
        if self.barcode is None:
            ...
            if self.barcode[0] != 'B':
                return
            if self.num > 0:
                self.convert_cmd = ('convert {}[{}-{}] {}').format(self.image_filepath, self.startpage, self.endpage, self.portable_document_filepath)
                if DEBUGGING:
            ...
            mypid = os.getpid()
            this_script_filespec = ('{}/{}').format(os.path.dirname(os.path.realpath(__file__)), __file__)
            ...
            if self.num > 0:
                if self.cfg['ignore_warnings'] or self.cfg['ignore_stderr']:
                    ...
                    if DEBUGGING:
                        ...
                        pp.pprint(result)
                    if result['code'] != 0:
                        Event(self.events_registry, self.me).register_err_fail(('command returned a nonzero value ({}): {}').format(result['code'], self.convert_cmd))
                        return
                    Event(self.events_registry, self.me).register_info_success(('{} + {} -> {}').format(self.image_filepath, self.structdoc_filepath, self.portable_document_filepath))
                    return
                ...
                if DEBUGGING:
                    ...
                    pp.pprint(result)
                if result['code'] != 0:
                    Event(self.events_registry, self.me).register_err_fail(('command returned a nonzero value ({}): {}; {}').format(result['code'], self.convert_cmd, result['stderr']))
                    return
                Event(self.events_registry, self.me).register_info_success(('{} + {} -> {}').format(self.image_filepath, self.structdoc_filepath, self.portable_document_filepath))
            return
...
class Scrivenor(object):
    ...
    def get_list_of_today_image_filepaths(self):
        today = date.today()
        images_todaypath = ('{}/{}*.{}').format(self.cfg['imagefile_dir'], today.strftime('%Y%m%d'), self.cfg['imagefile_ext'])
        if DEBUGGING:
    ...
    def get_list_of_today_structdoc_filepaths(self):
        today = date.today()
        structdoc_todaypath = ('{}/{}*.{}').format(self.cfg['structdoc_dir'], today.strftime('%Y%m%d'), self.cfg['structdoc_ext'])
        if DEBUGGING:
    ...
    def convert_image_filepath_to_structdoc_filepath(self, image_filepath):
        return ('{}/{}.{}').format(self.cfg['structdoc_dir'], Filesphere(filepath=image_filepath).get_stemname(), self.cfg['structdoc_ext'])

    def convert_structdoc_filepath_to_image_filepath(self, structdoc_filepath):
        return ('{}/{}.{}').format(self.cfg['imagefile_dir'], Filesphere(filepath=structdoc_filepath).get_stemname(), self.cfg['imagefile_ext'])
        ...
        for image_filepath in list_of_today_image_filepaths:
            ...
            if self.cfg['ignore_warnings'] or self.cfg['ignore_stderr']:
                ...
                if DEBUGGING:
                    ...
                    pp.pprint(result)
                if result['code'] != 0:
                    Event(self.events_registry, self.me).register_err_fail(('command returned a nonzero value ({}): {}').format(result['code'], cmd))
                    continue
                Event(self.events_registry, self.me).register_info_success(('{} -> {}').format(image_filepath, structdoc_filepath))
                continue
            ...
            if DEBUGGING:
                ...
                pp.pprint(result)
            if result['code'] != 0:
                Event(self.events_registry, self.me).register_err_fail(('command returned a nonzero value ({}): {}; {}').format(result['code'], cmd, result['stderr']))
                continue
            Event(self.events_registry, self.me).register_info_success(('{} -> {}').format(image_filepath, structdoc_filepath))
    ...
    def convert_structured_documents_to_portable_format(self):
        ...
        for structdoc_filepath in list_of_today_structdoc_filepaths:
            ...
            for i, item in enumerate(container):
                ...
                pd = PortableDocument(image_filepath, structdoc_filepath, item, endpage, prev_barcode)
                if DEBUGGING:
                    pp.pprint(pd)
