...
# [GCC 8.4.0]
# Embedded file name: /Volumes/Work/dev/cadasta/django-buckets/buckets/storage.py
# Compiled at: 2017-09-29 05:13:52
# Size of source mod 2**32: 3149 bytes
import os
...
class S3Storage(Storage):
    ...
    def get_boto_ressource(self):
        return boto3.resource('s3', aws_access_key_id=self.access_key, aws_secret_access_key=self.secret_key, region_name=self.region, config=Config(signature_version='s3v4'))

    def _open(self, name, mode='rb'):
        s3 = self.get_boto_ressource()
        f = s3.Object(self.bucket_name, name).get()
        write_path = os.path.join(settings.MEDIA_ROOT, 's3/downloads', name.split('/')[(-1)])
    ...
    def _save(self, name, content):
        ...
        return 'https://s3-{}.amazonaws.com/{}/{}'.format(self.region, self.bucket_name, name)
    ...
    def get_signed_url(self, key):
        ...
        client = boto3.client('s3', aws_access_key_id=self.access_key, aws_secret_access_key=self.secret_key, region_name=self.region, config=Config(signature_version='s3v4'))
