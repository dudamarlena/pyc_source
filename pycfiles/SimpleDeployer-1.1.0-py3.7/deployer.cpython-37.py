...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\deployer\deployer.py
# Compiled at: 2019-04-13 20:32:38
# Size of source mod 2**32: 5843 bytes
from fnmatch import fnmatch
import os, shutil, tempfile
from pathlib import Path
from typing import Dict
import git, subprocess, sys, click, yaml
home = Path.home()
config_path = Path.joinpath(home, '.deployer')
config_file = Path.joinpath(config_path, 'deployer.yaml')

@click.command()
@click.option('--create/--no-create', default=True, help="Should automatically create config file if it doesn't already exist. Default is to automatically create.")
@click.option('-f', '--file', default=config_file, type=click.Path(exists=False, file_okay=True, dir_okay=False, writable=True,
  readable=True,
  resolve_path=True),
...
def run(config: Dict, prompt: bool, verbose=False):
    for definition in config['definitions']:
        ...
        try:
            ...
            print(f"Cloning repo {definition['repository']}" + (f" branch: {definition['branch']}" if 'branch' in definition else '') + (f" to {temp_dir}" if verbose else ''))
