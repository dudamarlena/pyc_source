...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/soklog/tests.py
# Compiled at: 2013-11-12 14:53:06
...
class SoklogTest(unittest.TestCase):
    ...
    def test_debug(self, log_mock):
        with patch.object(soklog._DEFAULT, 'module', soklog):
            ...
            log_mock.debug.assert_called_with('./tests.py:39 1 2 3', level=2, something=4)
    ...
    def test_start_stdout_logging(self, mock):
        ...
        mock.assert_called_with(level=logging.DEBUG, format='%(asctime)-10s %(message)s', datefmt='%H:%M:%S')
