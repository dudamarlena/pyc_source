...
# Embedded file name: build/bdist.linux-x86_64/egg/prometheus_connect/prometheus_connect.py
# Compiled at: 2019-03-20 13:52:55
# Size of source mod 2**32: 11796 bytes
__doc__ = '\nA Class for collection of metrics from a Prometheus Host.\n'
from urllib.parse import urlparse
import bz2, os, sys, json, logging, requests, dateparser
...
class PrometheusConnect:
    ...
    def all_metrics(self):
        ...
        """
        response = requests.get(('{0}/api/v1/label/__name__/values'.format(self.url)), verify=(self.ssl_verification),
          headers=(self.headers))
        if response.status_code == 200:
            self._all_metrics = response.json()['data']
        else:
            raise Exception('HTTP Status Code {} ({})'.format(response.status_code, response.content))
    ...
    def get_current_metric_value(self, metric_name, label_config=None):
        ...
        if label_config:
            ...
            query = metric_name
        response = requests.get(('{0}/api/v1/query'.format(self.url)), params={'query': query},
    ...
    @retry(stop_max_attempt_number=MAX_REQUEST_RETRIES, wait_fixed=CONNECTION_RETRY_WAIT_TIME)
    def get_metric_range_data(self, metric_name, label_config=None, start_time='10m', end_time='now', chunk_size=None, store_locally=False):
        """
        A method to get the current metric value for the specified metric
        and label configuration.
        ...
        if int(end - start) < chunk_seconds:
            ...
            if label_config:
                ...
                query = metric_name
        while start < end:
            response = requests.get(('{0}/api/v1/query'.format(self.url)), params={'query':query + '[' + chunk_size + ']', 
            ...
            if response.status_code == 200:
                ...
                raise Exception('HTTP Status Code {} ({})'.format(response.status_code, response.content))
            if store_locally:
                self._store_metric_values_local(metric_name, json.dumps(response.json()['data']['result']), start + chunk_seconds)
            start += chunk_seconds
