...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/test/test_default_api.py
...
"""
import unittest, os, urllib.request, urllib.parse, urllib.error, time, behavioral_signals_swagger_client
...
class TestDefaultApi(unittest.TestCase):
    __doc__ = 'DefaultApi unit test stubs'
    ...
    def setUp(self):
        ...
        self.api = behavioral_signals_swagger_client.api.default_api.DefaultApi()
    ...
    def test_wav_2ch_dataset_noasr(self):
        ...
        for i in range(num_of_files):
            ...
            print(proc)
            self.assertTrue(isinstance(proc, behavioral_signals_swagger_client.models.process.Process), 'API must return a Process object when posting URL requests')
            self.assertEqual(int(proc.cid), self.cid, 'Created process must have the' + 'same client ID used to post the request ' + str(proc.cid) + ' vs ' + str(self.cid))
            self.assertTrue(isinstance(proc.pid, int), 'Returned process' + 'id must be integer')
        ...
        for pid in pids:
            ...
            self.assertTrue(isinstance(proc, behavioral_signals_swagger_client.models.process.Process), 'API must return a Process object on get process info call')
            self.assertEqual(int(proc.cid), self.cid, 'Created process must have the' + 'same client ID used to post the request ' + str(proc.cid) + ' vs ' + str(self.cid))
            self.assertTrue(isinstance(proc.pid, int), 'Returned process' + 'id must be integer')
        ...
        self.assertTrue((len(pids) - len(completed_pids)) / len(pids) < 0.05, 'At least 95% ' + 'of the submitted Pids must complete to continue')
        print('Will try to get the results now')
        for pid in completed_pids:
            res = self.api.get_process_results(self.cid, pid)
            self.assertTrue(isinstance(res, behavioral_signals_swagger_client.models.result_json.ResultJSON), 'API must return ResultJSON objects' + ' when the get process results method is called')
            res = self.api.get_process_results_frames(self.cid, pid)
            self.assertTrue(isinstance(res, behavioral_signals_swagger_client.models.result_json_frames.ResultJSONFrames), 'API must return ResultJSONFrames objects' + ' when the get process results frames method is called')
    ...
    def test_wav_2ch_dataset(self):
        ...
        for pid in completed_pids:
            ...
            self.assertTrue(isinstance(res, behavioral_signals_swagger_client.models.result_jsonasr.ResultJSONASR), 'API must return ResultJSONASR objects' + ' when the get process results asr method is called')
    ...
    def test_service_status(self):
        ...
        print(repr(status))
        self.assertTrue(isinstance(status, behavioral_signals_swagger_client.models.status.Status), 'Response of the service status endpoint is not of expected class')
