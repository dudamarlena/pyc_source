...
import setuptools.command.build_ext as SetuptoolsBuildExt
...
def generate_build_ext_command(packagename, release):
    ...
    class build_ext(SetuptoolsBuildExt):
        ...
        def _final_class(cls):
            ...
            if uses_cython:
                try:
                    import Cython.Distutils.old_build_ext as base_cls
                except ImportError:
                    import Cython.Distutils as base_cls
        ...
        def run(self):
            ...
            if self.uses_cython:
                if self.uses_cython != cython_version:
                    build_py = self.get_finalized_command('build_py')
                    package_dir = build_py.get_package_dir(packagename)
