...
# [GCC 8.4.0]
# Embedded file name: /Users/rgeda/project/repo/webbpsf/astropy_helpers/astropy_helpers/setup_helpers.py
# Compiled at: 2019-07-20 17:47:20
# Size of source mod 2**32: 26278 bytes
"""
...
import collections, os, re, subprocess, sys, traceback, warnings
from distutils import log
from distutils.errors import DistutilsOptionError, DistutilsModuleError
from distutils.core import Extension
from distutils.core import Command
import distutils.command.sdist as DistutilsSdist
from setuptools import find_packages as _find_packages
from .distutils_helpers import add_command_option, get_compiler_option, get_dummy_distribution, get_distutils_build_option, get_distutils_build_or_install_option
from .version_helpers import get_pkg_version_module
from .utils import walk_skip_hidden, import_file, extends_doc, resolve_name, AstropyDeprecationWarning
...
def adjust_compiler(package):
    ...
    to.
    """
    warnings.warn('Direct use of the adjust_compiler function in setup.py is deprecated and can be removed from your setup.py.  This functionality is now incorporated directly into the build_ext command.', AstropyDeprecationWarning)
...
def add_exclude_packages(excludes):
    if _module_state['excludes_too_late']:
        raise RuntimeError('add_package_excludes must be called before all other setup helper functions in order to properly handle excluded packages')
...
def register_commands(package, version, release, srcdir='.'):
    ...
    elif _module_state['have_sphinx']:
        try:
...
def add_command_hooks(commands, srcdir='.'):
    ...
    """
    hook_re = re.compile('^(pre|post)_(.+)_hook$')
    ...
    for cmd_name, cmd_hooks in hooks.items():
        commands[cmd_name] = generate_hooked_command(cmd_name, dist.get_command_class(cmd_name), cmd_hooks)
...
def generate_hooked_command(cmd_name, cmd_cls, hooks):
    ...
    return type(cmd_name, (cmd_cls, object), {'run':run, 
     'run_command_hooks':run_command_hooks,  'pre_hooks':hooks.get('pre', []), 
    ...
    for modname, hook in hooks:
        if isinstance(hook, str):
            try:
                ...
                try:
                    raise DistutilsModuleError('cannot find hook {0}: {1}'.format(hook, exc))
                finally:
                    exc = None
                    del exc
        ...
        log.info('running {0} from {1} for {2} command'.format(hook_kind.rstrip('s'), modname, cmd_obj.get_command_name()))
        try:
            hook_obj(cmd_obj)
        except Exception:
            log.error('{0} command hook {1} raised an exception: %s\n'.format(hook_obj.__name__, cmd_obj.get_command_name()))
...
def get_package_info(srcdir='.', exclude=()):
    ...
    package_dir = {}
    if exclude:
        warnings.warn('Use of the exclude parameter is no longer supported since it does not work as expected. Use add_exclude_packages instead. Note that it must be called prior to any other calls from setup helpers.', AstropyDeprecationWarning)
    ...
    return {'ext_modules':ext_modules,  'packages':packages, 
...
def pkg_config(packages, default_libraries, executable='pkg-config'):
    ...
    flag_map = {'-I':'include_dirs', 
     '-L':'library_dirs',  '-l':'libraries',  '-D':'define_macros', 
     '-U':'undef_macros'}
...
def add_external_library(library):
    ...
    for command in ('build', 'build_ext', 'install'):
        add_command_option(command, (str('use-system-' + library)), ('Use the system {0} library'.format(library)),
          is_bool=True)
...
def find_packages(where='.', exclude=(), invalidate_cache=False):
    ...
    if not invalidate_cache:
        if _module_state['package_cache'] is not None:
...
class FakeBuildSphinx(Command):
    ...
    def initialize_options(self):
        try:
            raise RuntimeError('Sphinx and its dependencies must be installed for build_docs.')
        except:
            log.error('error: Sphinx and its dependencies must be installed for build_docs.')
            sys.exit(1)