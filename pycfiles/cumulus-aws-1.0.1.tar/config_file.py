...
# Embedded file name: /Users/sebastian/git/skymill/cumulus/cumulus/cumulus_ds/config/config_file.py
# Compiled at: 2014-04-07 10:01:28
__doc__ = ' Manage configuration file parsing '
...
CONF = {'general': {}, 'environments': {}, 'stacks': {}, 'bundles': {}}
GENERAL_OPTIONS = [
 (
  'log-level', False),
 (
...
def configure(args):
    ...
    if args.config:
        ...
        if not conf_file_found:
            raise ConfigurationException(('No configuration file found. Looked for {}').format((', ').join(config_files)))
...
def __get_include_files(config_files):
    ...
    config.read(config_files)
    try:
        return [ ospath.expanduser(c.strip()) for c in config.get('general', 'include').split(',')
               ]
...
def _populate_environments(args, config):
    ...
    if section not in config.sections():
        raise ConfigurationException(('No configuration found for environment {}').format(environment))
            ...
            if option == 'bundles':
                ...
                for item in config.get(section, option).split(','):
                    ...
                    if args.stacks and item not in args.stacks:
                        continue
                    stacks.append(('{}-{}').format(args.environment, item))
                ...
                if args.version:
                    ...
                    CONF['environments'][environment][option] = config.get(section, option)
            else:
                CONF['environments'][environment][option] = config.get(section, option)
        except NoOptionError:
            if required:
                raise ConfigurationException(('Missing required option {}').format(option))
            ...
            try:
                if option == 'log-level':
                    ...
                    if log_level not in ('DEBUG', 'INFO', 'WARNING', 'ERROR'):
                        LOGGER.warning(('Invalid log level "{}". Using default log level.').format(log_level))
                        log_level = 'DEBUG'
    ...
    for section in config.sections():
        if section.startswith('stack: '):
            ...
            if 'stack-name-prefix' in CONF['environments'][args.environment]:
                stack = ('{}-{}').format(CONF['environments'][args.environment]['stack-name-prefix'], stack)
            if 'stack-name-suffix' in CONF['environments'][args.environment]:
                stack = ('{}-{}').format(stack, CONF['environments'][args.environment]['stack-name-suffix'])
            CONF['stacks'][stack] = {}
            for option, required in STACK_OPTIONS:
                try:
                    if option == 'disable-rollback':
                        CONF['stacks'][stack][option] = config.getboolean(section, option)
                    elif option == 'template':
                        CONF['stacks'][stack][option] = ospath.expanduser(config.get(section, option))
                    elif option == 'parameters':
                        try:
                            raw_parameters = config.get(section, option).split('\n')
                            ...
                            raise ConfigurationException(('Error parsing parameters for stack {}').format(stack))

                    elif option == 'tags':
                        try:
                            raw_tags = config.get(section, option).split('\n')
                            if not raw_tags[0]:
                                raw_tags.pop(0)
                            tags = {}
                            for tag in raw_tags:
                            ...
                            raise ConfigurationException(('Error parsing tags for stack {}').format(stack))

                    elif option == 'timeout-in-minutes':
                        CONF['stacks'][stack][option] = config.getint(section, option)
                    else:
                        CONF['stacks'][stack][option] = config.get(section, option)
                except NoOptionError:
                    if required:
                        raise ConfigurationException(('Missing required option {}').format(option))
            ...
            try:
                if args.parameters:
                    if 'parameters' not in CONF['stacks'][stack]:
                    ...
                    if option == 'paths':
                        ...
                        for line in lines:
                            try:
                                ...
                                raise ConfigurationException(('Invalid path-rewrites for bundle {}').format(bundle))

                            if target[0] == '/':
                                target = target[1:]
                            if destination[0] == '/':
                                destination = destination[1:]
                            CONF['bundles'][bundle]['path-rewrites'].append({'target': target.strip(), 
                               'destination': destination.strip()})

                    else:
                        CONF['bundles'][bundle][option] = config.get(section, option)
                except NoOptionError:
                    if option == 'paths' and config.has_option(section, 'pre-built-bundle'):
                        continue
                    if required:
                        raise ConfigurationException(('Missing required option {}').format(option))