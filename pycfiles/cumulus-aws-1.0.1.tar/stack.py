...
# Embedded file name: /Users/sebastian/git/skymill/cumulus/cumulus/cumulus_ds/helpers/stack.py
# Compiled at: 2014-04-17 03:46:39
__doc__ = ' Stack helpers '
...
def delete_stack(stack):
    ...
    :returns: str -- Status code for the stack
    """
    LOGGER.info(('Deleting stack {}').format(stack))
    CONNECTION.delete_stack(stack)
    return _wait_for_stack_complete(stack, filter_type='DELETE')


def ensure_stack(stack_name, parameters, template, tags=None, disable_rollback=False, timeout_in_minutes=None, capabilities=['CAPABILITY_IAM']):
    ...
    """
    LOGGER.info(('Ensuring stack {} with template {}').format(stack_name, template))
    cumulus_parameters = [
     (
    ...
    for parameter in cumulus_parameters + parameters:
        LOGGER.debug(('Adding parameter "{}" with value "{}" to CF template').format(parameter[0], parameter[1]))

    if timeout_in_minutes:
        LOGGER.debug(('Will time out stack creation after {:d} minutes').format(timeout_in_minutes))
    try:
        if stack_exists(stack_name):
            LOGGER.debug(('Updating existing stack to version {}').format(config.get_environment_option('version')))
            if template[0:4] == 'http':
                CONNECTION.update_stack(stack_name, parameters=cumulus_parameters + parameters, template_url=template, disable_rollback=disable_rollback, capabilities=[
                 'CAPABILITY_IAM'], timeout_in_minutes=timeout_in_minutes, tags=tags)
            else:
                CONNECTION.update_stack(stack_name, parameters=cumulus_parameters + parameters, template_body=_get_json_from_template(template), disable_rollback=disable_rollback, capabilities=[
                 'CAPABILITY_IAM'], timeout_in_minutes=timeout_in_minutes, tags=tags)
            stack_status = _wait_for_stack_complete(stack_name, filter_type='UPDATE')
        else:
            LOGGER.debug(('Creating new stack with version {}').format(config.get_environment_option('version')))
            if template[0:4] == 'http':
                CONNECTION.create_stack(stack_name, parameters=cumulus_parameters + parameters, template_url=template, disable_rollback=disable_rollback, capabilities=[
                 'CAPABILITY_IAM'], timeout_in_minutes=timeout_in_minutes, tags=tags)
            else:
                CONNECTION.create_stack(stack_name, parameters=cumulus_parameters + parameters, template_body=_get_json_from_template(template), disable_rollback=disable_rollback, capabilities=[
                 'CAPABILITY_IAM'], timeout_in_minutes=timeout_in_minutes, tags=tags)
            stack_status = _wait_for_stack_complete(stack_name, filter_type='CREATE')
    except IOError as error:
        LOGGER.error(('Error reading template file: {}').format(error))
        return
    except ValueError as error:
        raise InvalidTemplateException(('Malformatted template: {}').format(error))
    except boto.exception.BotoServerError as error:
        if error.error_code == 'ValidationError' and error.error_message == 'No updates are to be performed.':
            LOGGER.warning(('No CloudFormation updates are to be performed for {}').format(stack_name))
            return
        LOGGER.error(('Boto exception {}: {}').format(error.error_code, error.error_message))
        return
...
def _get_stack_outputs(stack_name_or_id):
    ...
    try:
        ...
        LOGGER.debug(('No stack with name or id {} found').format(stack_name_or_id))
        return []
...
def _print_event_log_event(event):
    ...
    if event_status[(len(event_status) - 1)] == 'COMPLETE':
        status = '\x1b[92m' + event.resource_status + '\x1b[0m'
    elif event_status[(len(event_status) - 1)] == 'PROGRESS':
        status = '\x1b[94m' + event.resource_status + '\x1b[0m'
    elif event_status[(len(event_status) - 1)] == 'FAILED':
        status = '\x1b[91m' + event.resource_status + '\x1b[0m'
    else:
        status = event.resource_status
    row = ('{timestamp:<19}').format(timestamp=datetime.strftime(event.timestamp, '%Y-%m-%d %H:%M:%S'))
...
def _print_stack_output(stack_name_or_id):
    ...
    if not outputs:
        ...
        return
    LOGGER.info(('Output data from stack "{}"').format(stack_name_or_id))
    print '--------------------+-----------------------------------------------------------------------------------------------------------------------------'
    print ('{key_title:<19} | {value_title:<45}').format(key_title='Tag', value_title='Value')
    print '--------------------+-----------------------------------------------------------------------------------------------------------------------------'
    for output in outputs:
        print ('{key:<19} | {value:<45}').format(key=output.key, value=output.value)
...
def _wait_for_stack_complete(stack_name, check_interval=5, filter_type=None):
    ...
    :returns: str -- Last status code for the stack
    """
    ...
    while not complete:
        ...
        for event in reversed(CONNECTION.describe_stack_events(stack.stack_id)):
            ...
            if not filter_type:
                log = True
            elif filter_type == 'CREATE' and event_type in ('CREATE', 'ROLLBACK'):
                log = True
            elif filter_type == 'DELETE' and event_type in ('DELETE', 'ROLLBACK'):
                log = True
            elif filter_type == 'UPDATE' and event_type in ('UPDATE', 'ROLLBACK'):
                log = True
            if not log:
                continue
            _print_event_log_event(event)
        ...
        if stack.stack_status in complete_statuses:
            _print_event_log_separator()
            LOGGER.info(('Stack {} - Stack completed with status {}').format(stack.stack_name, stack.stack_status))
            complete = True
