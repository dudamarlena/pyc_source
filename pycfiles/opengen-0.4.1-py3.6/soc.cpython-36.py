...
class SecondOrderCone(Constraint):
    ...
    def __init__(self, a: float=1.0):
        ...
        self._SecondOrderCone__a = a

    @property
    def a(self):
        """Returns the value of parameter `a`"""
        return self._SecondOrderCone__a
    ...
    def distance_squared(self, u):
        ...
        a = self._SecondOrderCone__a
        x = u[0:nu - 1]
        r = u[(nu - 1)]
        eps = 1e-16
        norm_x = fn.norm2(x)
        sq_norm_x = cs.dot(x, x)
        ...
        f = cs.if_else(condition0, 0, cs.if_else(condition1, fun1, cs.if_else(condition2, fun2, fun3, True), True), True)
        return f
