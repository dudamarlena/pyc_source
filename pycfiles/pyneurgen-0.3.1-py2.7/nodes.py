...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyneurgen/nodes.py
...
class Node(ProtoNode):
    ...
    def feed_forward(self):
        ...
        for conn in self.input_connections:
            if conn.lower_node.get_value() is None:
                raise ValueError('Uninitialized node %s' % conn.lower_node.node_no)
    ...
    def adjust_weights(self, learnrate, halt_on_extremes):
        ...
        """
        for conn in self.input_connections:
            conn.add_weight(self._adjust_weight(learnrate, conn.lower_node.activate(), self.error))
...
class CopyNode(Node):
    ...
    def load_source_value(self):
        ...
        self._value = self._value * self._existing_weight + value * self._incoming_weight
    ...
    def source_update_config(self, source_type, incoming_weight, existing_weight):
        ...
        if source_type in ('a', 'v'):
            ...
            raise ValueError("Invalid source type, %s. Valid choices are 'a' or 'v'")
