...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyneurgen/recurrent.py
...
class RecurrentConfig(object):
    ...
    def _apply_config(self, neural_net):
        ...
        for snode in self.get_source_nodes(neural_net):
            ...
            for level in xrange(self.copy_levels):
                ...
                copy_node.source_update_config(self.source_type, self.incoming_weight, self.existing_weight)
                copy_node.set_activation_type(self.activation_type)
                if self.connection_type == 'm':
                    self._fully_connect(copy_node, self.get_upper_nodes(neural_net))
                elif self.connection_type == 's':
                    copy_node.add_input_connection(Connection(copy_node, snode))
                else:
    ...
    def get_upper_nodes(self, neural_net):
        ...
        return [ node for node in layer.nodes if node.node_type != NODE_BIAS
               ]
