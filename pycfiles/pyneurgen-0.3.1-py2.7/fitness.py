...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyneurgen/fitness.py
...
SCALING_TYPES = [
 SCALING_LINEAR, SCALING_TRUNC, SCALING_EXPONENTIAL,
 SCALING_LOG]
...
class FitnessList(list):
    ...
    def best_value(self):
        ...
        if self._fitness_type == MAX:
            return self.max_value()
        if self._fitness_type == CENTER:
    ...
    def stddev(self):
        ...
        return pow(total / (float(len(self)) - 1.0), 0.5)
...
class Selection(object):
    ...
    def _roulette_wheel(scale_list):
        ...
        if round(sum(scale_list), 10) != 1.0:
            raise ValueError('The scaled list received does not total 1.0: %s' % sum(scale_list))
        cumu = [
         0.0]
    ...
    def set_tournament_size(self, tournament_size):
        ...
        if tournament_size is not None:
            if not isinstance(tournament_size, int):
                raise ValueError('Tournament size, %s must be an int.' % tournament_size)
...
class Fitness(Selection):
    ...
    def set_fitness_list(self, fitness_list):
        ...
        if fitness_list.get_fitness_type() == CENTER:
            ...
            for i in xrange(length):
                self._selection_list.append(abs(fitness_list[i][0] - target_value))
    ...
    def _scale_list(self):
        ...
        if seltype == MAX and fit_type == MIN:
            inverse = True
        elif seltype == MIN and fit_type == MAX:
            inverse = True
        elif seltype == MAX and fit_type == CENTER:
            inverse = True
        else:
            inverse = False
        if inverse:
            self._selection_list = [ self._invert(value) for value in self._selection_list ]
    ...
    *    SCALING_LOG = 'logarithmic'
    ...
    def set_scaling_type(self, scaling_type):
        ...
        if scaling_type not in SCALING_TYPES:
            raise ValueError('Invalid scaling type: %s, valid scaling types are %s' % (
    ...
    def _check_minmax(self):
        ...
        if min(self._selection_list) < 0.0 and self._scaling_type == 'logarithmic':
    ...
    def _apply_prop_scaling(self, param):
        ...
        if self._scaling_type == SCALING_LINEAR:
            ...
            if param is None:
                ...
                exponent = param
            self._selection_list = [ pow(item, exponent) for item in self._selection_list
                                   ]
            self._selection_list = self._make_prob_list(self._selection_list)
        elif self._scaling_type == SCALING_LOG:
            self._selection_list = [ math.log(item) for item in self._selection_list ]
            self._selection_list = self._make_prob_list(self._selection_list)
...
class FitnessElites(Fitness):
    ...
    def set_rate(self, rate):
        ...
        if not (isinstance(rate, float) and 0.0 < rate <= 1.0):
            raise ValueError('The rate, %s, should between 0.0 and 1.0' % rate)
        self._rate = rate
...
class FitnessLinearRanking(Fitness):
    ...
    def set_worstfactor(self, worstfactor):
        ...
        if not isinstance(worstfactor, float):
            raise ValueError('Worstfactor must be a float value between 0 and 2.0.')
    ...
    def select(self):
        ...
        prob_list = self._linear_ranking(length, self._worstfactor)
        select_list = [ [sort_list[i][1], prob_list[i]] for i in xrange(length)
                      ]
...
class FitnessTruncationRanking(Fitness):
    ...
    def set_trunc_rate(self, trunc_rate):
        ...
        if not isinstance(trunc_rate, float):
            raise ValueError('Trunc_rate, %s, should between 0.0 and 1.0' % trunc_rate)
...
class ReplacementDeleteWorst(Replacement):
    ...
    def set_replacement_count(self, replacement_count):
        ...
        if not isinstance(replacement_count, int):
            raise ValueError('Replacement count, %s should an int between 0 and %s' % (
             replacement_count, length))
        if not 0 < replacement_count <= length:
            raise ValueError('Replacement count, %s should between 0 and %s' % (
             replacement_count, length))
