...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyneurgen/grammatical_evolution.py
...
logging.basicConfig(format='%(asctime)s %(message)s', filename=DEFAULT_LOG_FILE, level=logging.INFO)
...
class GrammaticalEvolution(object):
    ...
    def __init__(self):
        ...
        """
        self.stopping_criteria = {STOPPING_MAX_GEN: None, 
    ...
    def set_genotype_length(self, start_gene_length, max_gene_length=None):
        ...
        if not isinstance(start_gene_length, long):
            raise ValueError('start_gene_length, %s, must be a long' % start_gene_length)
        if start_gene_length < 0:
            raise ValueError('start_gene_length, %s, must be above 0' % start_gene_length)
        if not isinstance(max_gene_length, long):
            raise ValueError('max_gene_length, %s, must be a long' % max_gene_length)
        if max_gene_length < 0:
            raise ValueError('max_gene_length, %s, must be above 0' % max_gene_length)
    ...
    def get_genotype_length(self):
        ...
        """
        return (
         self._start_gene_length, self._max_gene_length)
...
. The following lines can also hold additional values to accommodate
        ...
        def strip_spaces(key, values):
            ...
            if key.startswith(STATEMENT_FORMAT):
                values = [ value.rstrip() for value in values.split('|') if value ]
            else:
                values = [ value.strip() for value in values.split('|') if value
                         ]
            return values
    ...
    def set_children_per_crossover(self, children_per_crossover):
        ...
        if children_per_crossover not in (1, 2):
            raise ValueError('The children per crossovermust be either 1 or 2.')
    ...
    def set_max_generations(self, generations):
        ...
        if isinstance(generations, int) and generations >= 0:
            ...
            raise ValueError('\n                generations, %s, must be an int 0 or greater' % generations)
    ...
    def run(self, starting_generation=0):
        ...
        logging.info('completed run: generations: %s, best member:%s fitness: %s' % (
         self._generation,
    ...
    def create_genotypes(self):
        ...
        while member_no < self._population_size:
            gene = Genotype(self._start_gene_length, self._max_gene_length, member_no)
    ...
    def _crossover(self, parent1, parent2):
        ...
        child1_binary, child2_binary = self._crossover_function(child1.binary_gene, child2.binary_gene, crosspoint)
    ...
    def _crossover_function(child1_binary, child2_binary, crosspoint):
        ...
        """
        child1_binary, child2_binary = child1_binary[0:crosspoint] + child2_binary[crosspoint:], child2_binary[0:crosspoint] + child1_binary[crosspoint:]
        return (
         child1_binary, child2_binary)
    ...
    def _continue_processing(self):
        ...
        if self.stopping_criteria[STOPPING_FITNESS_LANDSCAPE] is not None:
            status = self.stopping_criteria[STOPPING_FITNESS_LANDSCAPE](self.fitness_list)
            logging.info((' ').join([
             'stopping processing due to',
