...
# [GCC 8.4.0]
# Embedded file name: /Users/ryosukekita/Desktop/PROJECTS/DSVISUALIZER/aefiles/lib/cloudstorage/rest_api.py
...
class _RestApi(object):
  ...
  """
    ...
    def __init__(self, scopes, service_account_id=None, token_maker=None, retry_params=None):
        """Constructor.
    ...
    def __getstate__(self):
        ...
        return {'scopes': self.scopes, 'id': self.service_account_id, 
           'a_maker': None if self.make_token_async == _make_token_async else self.make_token_async, 
           'retry_params': self.retry_params, 
           'expiration_headroom': self.expiration_headroom}

    def __setstate__(self, state):
        """Restore state as part of deserialization/unpickling."""
        self.__init__(state['scopes'], service_account_id=state['id'], token_maker=state['a_maker'], retry_params=state['retry_params'])
    ...
    """
        retry_wrapper = api_utils._RetryWrapper(self.retry_params, retriable_exceptions=api_utils._RETRIABLE_EXCEPTIONS, should_retry=api_utils._should_retry)
        resp = yield retry_wrapper.run(self.urlfetch_async, url=url, method=method, headers=headers, payload=payload, deadline=deadline, callback=callback, follow_redirects=False)
    ...
    """
        ...
        ts = yield _AE_TokenStorage_.get_by_id_async(key, use_cache=True, use_memcache=True, use_datastore=self.retry_params.save_access_token)
        if refresh or ts is None or ts.expires < time.time() + self.expiration_headroom:
            ...
            if timeout > 0:
                yield ts.put_async(memcache_timeout=timeout, use_datastore=self.retry_params.save_access_token, use_cache=True, use_memcache=True)
