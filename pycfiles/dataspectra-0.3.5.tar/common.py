...
# [GCC 8.4.0]
# Embedded file name: /Users/ryosukekita/Desktop/PROJECTS/DSVISUALIZER/aefiles/lib/cloudstorage/common.py
...
_GCS_BUCKET_REGEX_BASE = '[a-z0-9\\.\\-_]{3,63}'
_GCS_BUCKET_REGEX = re.compile(_GCS_BUCKET_REGEX_BASE + '$')
_GCS_BUCKET_PATH_REGEX = re.compile('/' + _GCS_BUCKET_REGEX_BASE + '$')
_GCS_PATH_PREFIX_REGEX = re.compile('/' + _GCS_BUCKET_REGEX_BASE + '.*')
_GCS_FULLPATH_REGEX = re.compile('/' + _GCS_BUCKET_REGEX_BASE + '/.*')
...
class GCSFileStat(object):
    ...
    def __init__(self, filename, st_size, etag, st_ctime, content_type=None, metadata=None, is_dir=False):
        """Initialize.
    ...
    def __repr__(self):
        ...
        return '(filename: %(filename)s, st_size: %(st_size)s, st_ctime: %(st_ctime)s, etag: %(etag)s, content_type: %(content_type)s, metadata: %(metadata)s)' % dict(filename=self.filename, st_size=self.st_size, st_ctime=self.st_ctime, etag=self.etag, content_type=self.content_type, metadata=self.metadata)

    def __cmp__(self, other):
        if not isinstance(other, self.__class__):
            raise ValueError('Argument to cmp must have the same type. Expect %s, got %s', self.__class__.__name__, other.__class__.__name__)
...
def validate_file_path(path):
  ...
  """
    ...
    if not _GCS_FULLPATH_REGEX.match(path):
        raise ValueError('Path should have format /bucket/filename but got %s' % path)
...
def _add_ns(tagname):
    return '{%(ns)s}%(tag)s' % {'ns': CS_XML_NS, 'tag': tagname}
