...
# [GCC 8.4.0]
# Embedded file name: /Users/ryosukekita/Desktop/PROJECTS/DSVISUALIZER/aefiles/lib/cloudstorage/cloudstorage_api.py
...
__all__ = [
 ...
 'stat',
 'compose']
import logging, StringIO, urllib, os, itertools, types, xml.etree.cElementTree as ET
...
def open(filename, mode='r', content_type=None, options=None, read_buffer_size=storage_api.ReadBuffer.DEFAULT_BUFFER_SIZE, retry_params=None, _account_id=None, offset=0):
  ...
  """
    ...
    if mode == 'r':
        ...
        return storage_api.ReadBuffer(api, filename, buffer_size=read_buffer_size, offset=offset)
...
def delete(filename, retry_params=None, _account_id=None):
  ...
  """
    ...
    errors.check_status(status, [204], filename, resp_headers=resp_headers, body=content)
...
def compose(list_of_files, destination_file, files_metadata=None, content_type=None, retry_params=None, _account_id=None):
  ...
  """
    ...
    file_list, _ = _validate_compose_list(destination_file, list_of_files, files_metadata, 32)
...
def _validate_compose_list(destination_file, file_list, files_metadata=None, number_of_files=32):
  ...
  """
    ...
    if list_len > number_of_files:
        raise ValueError('Compose attempted to create composite with too many(%i) components; limit is (%i).' % (
...
class _Bucket(object):
    ...
    def _init(self, api, path, options):
        ...
        self._get_bucket_fut = self._api.get_bucket_async(self._path + '?' + urllib.urlencode(self._options))
    ...
    """
        ...
        while self._get_bucket_fut:
            ...
            errors.check_status(status, [200], self._path, resp_headers=resp_headers, body=content, extras=self._options)
            ...
            next_dir = dirs.next()
            while (max_keys is None or total < max_keys) and not (next_file is None and next_dir is None):
                total += 1
                if next_file is None:
                    ...
                    logging.error('Should never reach. next file is %r. next dir is %r.', next_file, next_dir)
    ...
    """
        for e in root.getiterator(common._T_COMMON_PREFIXES):
            yield common.GCSFileStat(self._path + '/' + e.find(common._T_PREFIX).text, st_size=None, etag=None, st_ctime=None, is_dir=True)
            e.clear()
    ...
    """
        if 'max-keys' in self._options and self._options['max-keys'] <= common._MAX_GET_BUCKET_RESULT:
            ...
            elements = self._find_elements(content, set([common._T_IS_TRUNCATED,
