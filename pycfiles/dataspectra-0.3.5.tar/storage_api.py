...
# [GCC 8.4.0]
# Embedded file name: /Users/ryosukekita/Desktop/PROJECTS/DSVISUALIZER/aefiles/lib/cloudstorage/storage_api.py
# Compiled at: 2017-10-27 18:08:49
"""Python wrappers for the Google Storage RESTful API."""
__all__ = [
 'ReadBuffer',
 'StreamingBuffer']
import collections, os, urlparse
...
def _get_storage_api(retry_params, account_id=None):
  ...
  Returns:
    ...
    to the real GCS.
  """
    api = _StorageApi(_StorageApi.full_control_scope, service_account_id=account_id, retry_params=retry_params)
...
class _StorageApi(rest_api._RestApi):
  ...
  """
    ...
    """
        return (
         super(_StorageApi, self).__getstate__(), {'api_url': self.api_url})
    ...
    """
        ...
        try:
            resp_tuple = yield super(_StorageApi, self).do_request_async(url, method=method, headers=headers, payload=payload, deadline=deadline, callback=callback)
        except urlfetch.DownloadError as e:
            raise errors.TimeoutError('Request to Google Cloud Storage timed out.', e)
...
class ReadBuffer(object):
    ...
    def __init__(self, api, path, buffer_size=DEFAULT_BUFFER_SIZE, max_request_size=MAX_REQUEST_SIZE, offset=0):
        """Constructor.
    ...
    """
        return {'api': self._api, 'path': self._path, 
...
class StreamingBuffer(object):
  ...
  """
    ...
    """
        if not self._maxrequestsize > self._blocksize:
            ...
            errors.check_status(status, [201], path, headers, resp_headers, body=content)
            loc = resp_headers.get('location')
            raise (loc or IOError)('No location header found in 201 response')
    ...
    """
        while finish and self._buffered >= 0 or not finish and self._buffered >= self._blocksize:
            tmp_buffer = []
            tmp_buffer_len = 0
            excess = 0
            while self._buffer:
                ...
                if tmp_buffer_len >= self._maxrequestsize:
                    ...
                    break
                if not finish and tmp_buffer_len % self._blocksize + self._buffered < self._blocksize:
