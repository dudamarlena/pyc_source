...
# Embedded file name: build/bdist.macosx-10.13-intel/egg/cogandmem/experiment.py
# Compiled at: 2018-06-23 18:19:41
...
import sys, os, numpy as np, time, copy, pygame
from pygame.locals import *
import text, score, tetromino, writing, generic
BLACK = (
 0, 0, 0)
WHITE = (255, 255, 255)
LETTERS = (
...
def get_position(p, s, f, top, right, bottom, left):
    ...
    assert 0 <= top + bottom < 1, 'The sum of top and bottom must be between 0 and 1.'
    assert 0 <= left + right < 1, 'The sum of left and right must be between 0 and 1.'
...
class InterTrialStimulus():
    ...
    def __init__(self, duration, fixation='+', font=None, colour=WHITE, background=BLACK, antialiasing=True, location='middle centre', top=0.025, right=0.025, bottom=0.025, left=0.025, final_width=None, final_height=None):
        ...
        assert fixation and 0 <= top + bottom < 1, 'The sum of top and bottom must be between 0 and 1.'
        if not 0 <= left + right < 1:
            raise AssertionError('The sum of left and right must be between 0 and 1.')
            try:
                location = get_position(location, fixation, font, top, right, bottom, left)
                x, y = location
        ...
        fixation_surface, fixation_rect = text.render_string(fixation, font, colour, background, antialiasing)
        try:
...
class RatingScale():
    ...
    def __init__(self, choices, font, horizontal=True, colour=WHITE, background=BLACK, antialiasing=True, size=0.5, numbers=True, characters=None, keys=None):
        ...
        if not (numbers and not keys and 2 <= choices <= 9):
            raise AssertionError('Because numbers is True, choices must be between 2 and 9, inclusive.')
        else:
            if not numbers and not keys:
                assert 2 <= choices <= 26, 'Because numbers is False, choices must be between 2 and 26, inclusive.'
                assert characters and len(set(characters)) == len(characters) == choices, 'The number of characters must be equal to the number of choices.'
                if keys:
                    assert len(set(keys)) == len(keys) == choices, 'The number of keys must be equal to the number of choices.'
            ...
            self.size = size
            if horizontal:
                assert all(font.size(character)[0] < pixels_per_anchor for character in characters), 'At least one of the anchor characters is too wide when rendered with the given font.'
                ...
                for character in characters:
                    character_surf, character_rect = text.render_string(character, font, colour, background, antialiasing)
                ...
                assert all(font.size(character)[1] < pixels_per_anchor for character in characters), 'At least one of the anchor characters is too tall when rendered with the given font.'
    ...
    def get_rating(self, scale_position=BOTTOM, rated_text=None, duration=None, return_after_input=False, text_position=MIDDLE, exclude_top=0.025, exclude_right=0.025, exclude_bottom=0.025, exclude_left=0.025, text_colour=None, text_background=None, text_font=None, text_antialias=True, timer=None, frame_rate=30, exit_keys=(K_ESCAPE,), files=()):
        ...
        if self.horizontal:
            ...
            if scale_position == TOP:
                ...
                scale_rect.top = int(exclude_top * screen_height + pixel_rows // 2 - scale_rect.height // 2)
            else:
                scale_rect.bottom = int(screen_height - exclude_bottom * screen_height - 1)
        else:
            scale_rect.top = int(exclude_top * screen_height + pixel_rows // 2 - scale_rect.height // 2 + 1)
            if scale_position == LEFT:
                scale_rect.left = int(exclude_left * screen_width + 1)
            elif scale_position == MIDDLE:
                scale_rect.left = int(exclude_left * screen_width + pixel_columns // 2 - scale_rect.width // 2)
            else:
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            rated_surf, rated_rect = memory.text.string_to_surface_and_rect(rated_text, text_font, text_colour, text_background, max_width, max_height, antialiasing=text_antialias)

                rated_width, rated_height = isinstance(rated_text, str) or scale_rect.size
                assert rated_width <= pixel_columns, 'rated_text is too wide.'
                assert rated_height <= pixel_rows, 'rated_text is too tall.'
                if scale_position == TOP or scale_position == BOTTOM:
                    assert rated_height < pixel_rows - scale_rect.height, 'rated_text is too tall.'
                elif scale_position == LEFT or scale_position == RIGHT:
                    assert rated_width < pixel_columns - scale_rect.width, 'rated_text is too wide.'
                else:
                    if self.horizontal:
                        if not rated_height < pixel_rows // 2 - scale_rect.height // 2:
                            raise AssertionError('rated_text is too tall.')
                        else:
                            assert rated_width < pixel_columns // 2 - scale_rect.width // 2, 'rated_text is too wide.'
                            if scale_position == TOP or scale_position == BOTTOM:
                    ...
                    if duration:
                        ...
                        response_obtained = return_after_input or False
            while True:
                ...
                for event in pygame.event.get():
                    if event.type == QUIT or event.type == KEYUP and event.key in exit_keys:
                        ...
                        if duration and not return_after_input:
...
defaults to "middle centre".
    ...
    def __init__(self, word, font, colour=WHITE, background=BLACK, antialiasing=True, condition=None, x=None, y=None, response=None, position='middle centre', top_exclude=0.025, right_exclude=0.025, bottom_exclude=0.025, left_exclude=0.025):
        ...
        if x == None and y == None:
            x, y = get_position(position, word, font, top_exclude, right_exclude, bottom_exclude, left_exclude)
        elif x == None or y == None:
            raise TypeError('You cannot set only one of the x and y attributes. Eitherset both or neither.')
        self.x = x
        self.y = y
        return
        ...
        while not response_obtained:
            for event in pygame.event.get():
                if event.type == QUIT or event.type == KEYUP and event.key in quit_keys:
                    ...
                    if show_input:
                        r_surface, r_rect = text.render_string(user_response, self.font, self.colour, self.background, antialiasing=self.antialiasing)
    ...
    def create_copy(self):
        ...
        stimulus_copy = Stimulus(word_copy, font_copy, colour=colour_copy, background=background_copy, antialiasing=antialiasing_copy, condition=condition_copy, x=x_copy, y=y_copy, response=response_copy)
...
class WordPair():
    ...
    def get_surface1(self):
        ...
        return self.font.render(self.word1, self.antialiasing, self.colour, self.background)

    def get_surface2(self):
        """Return a surface for word2."""
        return self.font.render(self.word2, self.antialiasing, self.colour, self.background)
    ...
    def study(self, duration, scale=None, end_after_input=True, ticker=None, frame_rate=30, exit_keys=(K_ESCAPE,), other_files=()):
        ...
        if scale:
            if any(key in exit_keys for key in scale.keys):
                raise ValueError('There cannot be overlap between exit_keys and scale.keys.')
        screen = pygame.display.get_surface()
        ...
        while True:
            for event in pygame.event.get():
                if event.type == QUIT or event.type == KEYUP and event.key in exit_keys:
                    ...
                    if end_after_input:
                        time_studying = generic.seconds_to_milliseconds(time.time() - start_time)
            ...
            for event in pygame.event.get():
                if event.type == QUIT or event.type == KEYUP and event.key in exit_keys:
                    ...
                    if event.type == KEYUP and event.key == K_RETURN:
                        self.response = response
                        return
                    if event.type == KEYUP and event.key == K_BACKSPACE and response:
                        old_rect = response_rect.copy()
                        response = response[:len(response) - 1]
                        screen.fill(self.background, old_rect)
                        response_surf, response_rect = text.render_string(response, self.font, self.colour, self.background, antialiasing=self.antialiasing)
                        response_rect.center = (
                         screen_width // 2, screen_height // 2)
                        response_rect.top = response_top
                        screen.blit(response_surf, response_rect)
                        pygame.display.update(generic.combine_rects((old_rect, response_rect)))
    ...
    def create_copy(self):
        ...
        condition_copy = copy.deepcopy(self.condition)
        response_copy = copy.deepcopy(self.response)
        word_pair_copy = WordPair(word1_copy, word2_copy, font_copy, left_to_right=left_to_right_copy, cue=cue_copy, target=target_copy, colour=colour_copy, background=background_copy, antialiasing=antialiasing_copy, condition=condition_copy, response=response_copy)
...
class Image():
    ...
    def get_keypress(self, response_keys=(
 K_o, K_n), start_time=0, end_time=None, ticker=None, frame_rate=30, quit_keys=(K_ESCAPE,), files=()):
        """
    ...
    def recognition_probe(self, allowed_keys=(
 K_o, K_n), begin_time=0, finish_time=None, c=None, fps=30, exit_keys=(K_ESCAPE,), files=()):
        """
        ...
        Keyword Parameters:
            ...
            files: any files that need to be closed if the program closes;
                defaults to an empty tuple.
        """
        self.response = self.get_keypress(response_keys=allowed_keys, start_time=begin_time, end_time=finish_time, ticker=c, frame_rate=fps, quit_keys=exit_keys, files=files)
    ...
    def rate(self, allowed_keys=NUMBERS, begin_time=0, finish_time=None, c=None, fps=30, exit_keys=(K_ESCAPE,), files=()):
        ...
        Keyword Parameters:
            ...
            files: any files that need to be closed if the program closes;
                defaults to an empty tuple.
        """
        self.rating = self.get_keypress(response_keys=allowed_keys, start_time=begin_time, end_time=finish_time, ticker=c, frame_rate=fps, quit_keys=exit_keys, files=files)
    ...
    def create_copy(self):
        ...
        new_image = Image(new_file_name, new_label, new_background, new_height, new_width, new_condition, new_x, new_y, new_response, new_rating)
        return new_image
...
def single_item_study_phase(stimuli, duration, isi, isi_fixation='+', randomize=False, case='', location='middle centre', locations=None, top_margin=0.025, right_margin=0.025, bottom_margin=0.025, left_margin=0.025, font=None, fonts=None, use_antialiasing=True, stimulus_colour=None, stimulus_colours=None, background_colour=None, background_colours=None, isi_font=None, isi_antialiasing=True, isi_colour=None, isi_background=None, timer=None, fps=30, keys_to_quit=(K_ESCAPE,), data_file=None, other_files=()):
    ...
    if not duration > 0:
        ...
        if not fps > 0:
            raise AssertionError('fps must be positive.')
            try:
                ...
                if not isinstance(current_stim, Stimulus):
                    ...
                    if locations:
                        ...
                        try:
                            ...
                            pixel_x, pixel_y = get_position(current_location, current_stim, current_font, top_margin, right_margin, bottom_margin, left_margin)
                    ...
                    new_stim = Stimulus(current_stim, current_font, colour=current_colour, background=current_background, antialiasing=use_antialiasing, x=pixel_x, y=pixel_y)
                    stimuli[i] = new_stim
            ...
            if isi_colour or stimulus_colour:
                isi_colour = stimulus_colour
            elif stimulus_colours:
                isi_colour = stimulus_colours[0]
            else:
                isi_colour = stimuli[0].colour
        if isi_antialiasing == None:
            isi_antialiasing = use_antialiasing
    for stim in stimuli:
        stim.study(duration, ticker=timer, frame_rate=fps, quit_keys=keys_to_quit, files=other_files)
        if isi:
            interstimulus_interval(isi, fixation=isi_fixation, font=isi_font, fixation_colour=isi_colour, background_colour=isi_background, antialiasing=isi_antialiasing, ticker=timer, frame_rate=fps, quit_keys=keys_to_quit, files=other_files)
...
def free_recall_test(f, line_size=None, antialias=True, text_colour=WHITE, background_colour=BLACK, prompt="Enter words you remember from the study phase. When you can't remember more words, enter q.", verification='Are you finished? Press y for yes and n for no.', finished_string='q', trim_width=0.05, trim_height=0.05, allowed_keys=LETTERS, time_limit=None, time_up_message='Time up.', time_up_message_duration=2000, show_previous_input=False, ticker=None, frame_rate=30, quit_keys=(K_ESCAPE,), data_file=None, close_data_file=True, other_files=()):
    ...
    """
    if not (0 <= trim_width < 1 and 0 <= trim_height < 1):
        raise AssertionError('trim_width and trim_height must be proportions.')
        assert not any(k in allowed_keys for k in (K_RETURN, K_BACKSPACE)), 'Neither return nor backspace may appear in allowed_keys.'
        assert not any(k in quit_keys for k in (K_RETURN, K_BACKSPACE)), 'Neither return nor backspace may appear in quit_keys.'
        assert not any(k in allowed_keys for k in quit_keys), 'There may not be overlap between allowed_keys and quit_keys.'
        ...
        line_size = line_size or f.get_linesize()
    if prompt:
        prompt_surface, prompt_rect = text.string_to_surface_and_rect(prompt, f, text_colour, background_colour, pixel_columns, pixel_rows, line_size=line_size, antialiasing=antialias)
    ...
    if verification:
        verification_surface, verification_rect = text.string_to_surface_and_rect(verification, f, text_colour, background_colour, pixel_columns, pixel_rows, line_size=line_size, antialiasing=antialias)
    ...
    if time_limit and time_up_message and time_up_message_duration:
        time_up_surface, time_up_rect = text.string_to_surface_and_rect(time_up_message, f, text_colour, background_colour, pixel_columns, pixel_rows, line_size=line_size, antialiasing=antialias)
    ...
    if show_previous_input:
        try:
            response_position = (
             left_margin, prompt_rect.bottom + line_size)
        except NameError:
            response_position = (
        ...
        try:
            ...
            response_position = (screen_width // 2 + left_margin, pixel_rows // 2 + top_margin)
        ...
        if show_previous_input:
            ...
            assert max_line_size <= space_for_responses, "There isn't enough room for responses to appear. Try shortening the prompt message or decreasing the font size."
        protocol = []
        response = ''
        if time_limit:
        ...
        while keep_testing:
            for event in pygame.event.get():
                if event.type == QUIT or event.type == KEYUP and event.key in quit_keys:
                    ...
                    response_surface, response_rect = text.render_string(response, f, text_colour, background_colour, antialias)
                    if show_previous_input:
                    ...
                    if response == finished_string:
                        ...
                        if show_previous_input:
                            ...
                            responses_height = text.height_of_strings(on_screen, f, line_size)
                            if space_for_responses - responses - height - line_size < max_line_size:
                                while space_for_responses - responses - height - line_size < max_line_size:
                                    del on_screen[0]
                                    responses_height = text.height_of_strings(on_screen, f, line_size)
                                    new_surface, new_rect = text.render_lines(on_screen, f, text_colour, background_colour, line_size=line_size, use_antialias=antialias)
                                    try:
                                        new_rect.topleft = (
                                         prompt_rect.left,
                                    ...
                                    try:
                                        response_position = (
                                         prompt_rect.left, top_margin + prompt_rect.height + line_size + responses_height)
                                    except NameError:
                                        response_position = (
                                         left_margin, top_margin + responses_height + line_size)

                                    pygame.display.update()

                            else:
                                response_surface, response_rect = text.render_string(protocol[(-1)], f, text_colour, background_colour, antialias)
                                response_rect.topleft = response_position
...
def free_recall(targets, stim_duration, isi_duration, study_instructions, test_instructions, top_margin=0.025, right_margin=0.025, bottom_margin=0.025, left_margin=0.025, response_keys=LETTERS, randomize=True, blocks=1, balance_conditions=True, targets_per_block=(), recall_prompt="Enter words you remember from the study phase. When you can't remember more words, enter q.", recall_verification='Are you finished? Press y for yes and n for no.', finished_string='q', time_for_recall=None, time_up_message='Time up.', time_up_message_duration=2000, previous_responses_stay_on_screen=False, antialiasing=True, stim_case='', stim_font=None, stim_fonts=None, stim_colour=WHITE, stim_colours=None, stim_background=BLACK, stim_backgrounds=None, stim_location='middle centre', stim_locations=None, conditions=None, preliminary_instructions=(), distraction_instructions=None, instruction_font=None, instruction_colour=WHITE, instruction_background=BLACK, instruction_line_size=None, lines_between_instructions_and_continue_message=2, continue_instruction_message='Press the space bar to continue.', continue_instruction_font=None, continue_instruction_line_size=None, continue_keys=(K_SPACE,), backward_keys=(), split_sentences_between_screens=False, sentence_terminators=SENTENCE_TERMINATORS, sentence_terminator_exclusions=(), isi_string='+', isi_font=None, isi_colour=WHITE, isi_background=BLACK, isi_position='middle centre', distractor=True, distraction_duration=120000, tetromino_sound_file=None, clock=None, frame_rate=30, exit_keys=(K_ESCAPE,), study_file=None, study_files=None, distractor_file=None, distractor_files=None, protocol_file=None, protocol_files=None, intrusion_file=None, intrusion_files=None, close_matches_file=None, close_matches_files=None, results_file=None, results_files=None, other_files=()):
        ...
        if not isinstance(target, Stimulus):
            ...
            if stim_locations:
                ...
                try:
                    ...
                    pixel_x, pixel_y = get_position(target_location, target, target_font, top_margin, right_margin, bottom_margin, left_margin)
            ...
            new_target = Stimulus(target, target_font, colour=target_colour, background=target_background, antialiasing=antialiasing, x=pixel_x, y=pixel_y)
    ...
    if blocks == 1:
        ...
        if balance_conditions or targets_per_block:
            if not sum(targets_per_block) == len(targets):
                raise AssertionError('Values in targets_per_block must sum to the length of targets.')
                target_count = 0
                ...
                assert len(targets) % blocks == 0, 'The length of targets does not evenly divide into the specified number of blocks.'
            ...
            if not targets_per_block:
                for key in conditions:
                    assert conditions[key] % blocks == 0, 'The number of targets in each condition must evenly divide by the number of blocks.'
                ...
                for list_length in targets_per_block:
                    assert list_length % len(conditions) == 0, 'List lengths in targets_per_block must evenly divide by the number of conditions.'
                    current_study_list = []
                    for key in conditions:
                        to_delete_indices = []
                        for j in xrange(len(targets_copy)):
                ...
                for instructions in preliminary_instructions:
                    text.display_text_until_keypress(instructions, instruction_font, instruction_colour, instruction_background, proportion_width=1 - left_margin - right_margin, proportion_height=1 - top_margin - bottom_margin, main_line=instruction_line_size, break_sentences=split_sentences_between_screens, sentence_terminators=sentence_terminators, terminator_exceptions=sentence_terminator_exclusions, gap=lines_between_instructions_and_continue_message, bottom_message=continue_instruction_message, bottom_font=continue_instruction_font, bottom_line=continue_instruction_line_size, advance_keys=continue_keys, reverse_keys=backward_keys, quit_keys=exit_keys, ticker=clock, frame_rate=frame_rate, files=other_files)

            for study_list in study_lists:
                text.display_text_until_keypress(study_instructions, instruction_font, instruction_colour, instruction_background, proportion_width=1 - left_margin - right_margin, proportion_height=1 - top_margin - bottom_margin, main_line=instruction_line_size, break_sentences=split_sentences_between_screens, sentence_terminators=sentence_terminators, terminator_exceptions=sentence_terminator_exclusions, gap=lines_between_instructions_and_continue_message, bottom_message=continue_instruction_message, bottom_font=continue_instruction_font, bottom_line=instruction_line_size, advance_keys=continue_keys, reverse_keys=backward_keys, quit_keys=exit_keys, ticker=clock, frame_rate=frame_rate, files=other_files)
                single_item_study_phase(study_list, stim_duration, isi_duration, isi_fixation=isi_string, case=stim_case, use_antialiasing=antialiasing, isi_font=isi_font, isi_antialiasing=antialiasing, isi_colour=isi_colour, isi_background=isi_background, timer=clock, fps=frame_rate, keys_to_quit=exit_keys, other_files=other_files)
                if distractor:
                    text.display_text_until_keypress(distraction_instructions, instruction_font, instruction_colour, instruction_background, proportion_width=1 - left_margin - right_margin, proportion_height=1 - top_margin - bottom_margin, main_line=instruction_line_size, break_sentences=split_sentences_between_screens, sentence_terminators=sentence_terminators, terminator_exceptions=sentence_terminator_exclusions, gap=lines_between_instructions_and_continue_message, bottom_message=continue_instruction_message, bottom_font=continue_instruction_font, bottom_line=continue_instruction_line_size, advance_keys=continue_keys, reverse_keys=backward_keys, quit_keys=exit_keys, ticker=clock, frame_rate=frame_rate, files=other_files)
                    lines_formed, times_lost = tetromino.distractor(distraction_duration / 1000, instruction_font, top_exclude=top_margin, right_exclude=right_margin, bottom_exclude=bottom_margin, left_exclude=left_margin, antialias=antialiasing, text_colour=instruction_colour, c=clock, fps=frame_rate, sound_file=tetromino_sound_file, press_to_quit=exit_keys, files=other_files)
                    lines.append(lines_formed)
                    losses.append(times_lost)
                text.display_text_until_keypress(test_instructions, instruction_font, instruction_colour, instruction_background, proportion_width=1 - left_margin - right_margin, proportion_height=1 - top_margin - bottom_margin, main_line=instruction_line_size, break_sentences=split_sentences_between_screens, sentence_terminators=sentence_terminators, terminator_exceptions=sentence_terminator_exclusions, gap=lines_between_instructions_and_continue_message, bottom_message=continue_instruction_message, bottom_font=continue_instruction_font, bottom_line=continue_instruction_line_size, advance_keys=continue_keys, reverse_keys=backward_keys, quit_keys=exit_keys, ticker=clock, frame_rate=frame_rate, files=other_files)
                recall_responses_i = free_recall_test(instruction_font, line_size=instruction_line_size, antialias=antialiasing, text_colour=instruction_colour, background_colour=instruction_background, prompt=recall_prompt, verification=recall_verification, finished_string=finished_string, trim_width=left_margin + right_margin, trim_height=top_margin + bottom_margin, allowed_keys=response_keys, time_limit=time_for_recall, time_up_message=time_up_message, time_up_message_duration=time_up_message_duration, show_previous_input=previous_responses_stay_on_screen, ticker=clock, frame_rate=frame_rate, quit_keys=exit_keys, other_files=other_files)
    ...
    if study_file:
        ...
        for i in xrange(blocks):
            ...
            writing.study_phase(study_list, study_file, close_when_finished=False)
            study_file.write('\n')
    ...
    if protocol_file:
        ...
        for i in xrange(blocks):
            ...
            if protocol:
                protocol_file.write(('The protocol for Block {:d} follows.\n').format(i + 1))
                writing.list_or_tuple(protocol_file, protocol, c=False)
            else:
                protocol_file.write(('No items were recalled for Block {:d}.\n').format(i + 1))
    ...
    if intrusion_file:
        ...
        for i in xrange(blocks):
            ...
            if current_intrusions:
                intrusion_file.write(('Intrusions for Block {:d} follow.\n').format(i + 1))
                writing.list_or_tuple(intrusion_file, current_intrusions, c=False)
            else:
                intrusion_file.write(('There were no intrusions for Block {:d}.\n').format(i + 1))
    ...
    if close_matches_file:
        ...
        for i in xrange(blocks):
            ...
            if close_ones:
                close_matches_file.write(('Block {:d} close matches follow.\n').format(i + 1))
                ...
                close_matches_file.write(('Block {:d} had no close matches.\n').format(i + 1))
...
def generate_word_pairs(words, illegal_pairs=(), max_attempts=100, relatedness_matrix=None, sep=None, cutoff=0.6):
    ...
    if relatedness_matrix:
        ...
        if len(matrix_[0]) == columns_and_rows - 1:
            ...
            if len(matrix_[0]) != columns_and_rows:
                raise ValueError('Something is wrong with the similarity matrix.')
            assert all(len(row) == columns_and_rows for row in matrix_[1:]), "Something is wrong with the relatedness_matrix file. The number of rows isn't adding up."
            assert len(matrix_) == columns_and_rows, "The number of columns doesn't add up in the relatedness_matrix file."
            matrix_dict = {}
    ...
    while i < max_attempts:
        word_pairs = np.random.choice(words, size=(number_of_pairs, 2), replace=False)
...
def cued_recall_study(stimuli, duration, distance=135, horizontal=True, balance_targets=True, isi_object=None, isi_duration=None, isi_fixation='+', isi_location='middle centre', illegal_pairs=None, similarity_matrix=None, similarity_cutoff=None, matrix_sep=None, randomize=False, scale=None, end_trial_after_scale_input=False, case='', stim_font=None, stim_fonts=None, use_antialiasing=True, stim_colour=None, stim_colours=None, stim_background=None, stim_backgrounds=None, isi_font=None, isi_colour=None, isi_background=None, isi_antialiasing=True, timer=None, fps=30, keys_to_quit=(K_ESCAPE,), data_file=None, other_files=()):
    ...
    if not duration > 0:
        ...
        if not fps > 0:
            ...
            if isinstance(stimuli, (list, tuple)) and isinstance(stimuli[0], str) or isinstance(stimuli, (str, file)):
                return_stimuli = True
                stimuli = generate_word_pairs(stimuli, illegal_pairs, relatedness_matrix=similarity_matrix, sep=matrix_sep, cutoff=similarity_cutoff)
            if isinstance(stimuli[0], (list, tuple)):
                ...
                if case == 'u':
                    ...
                    for i in xrange(len(stimuli)):
                        ...
                        stimuli[i] = WordPair(item1, item2, current_font, apart=distance, left_to_right=horizontal, cue=cue_item, target=target_item, colour=current_colour, background=current_background, antialiasing=use_antialiasing)

            if not isi_object and isi_duration:
                if isi_fixation:
                    if not isi_font:
                        isi_font = stimuli[0].font
                    isi_colour = isi_colour or stimuli[0].colour
            isi_background = isi_background or stimuli[0].background
        isi_object = InterTrialStimulus(isi_duration, isi_fixation, isi_font, isi_colour, isi_background, isi_antialiasing, location=isi_location)
    if scale:
        screen = pygame.display.get_surface()
        try:
            screen_width, screen_height = screen.get_size()
    ...
    for i in xrange(len(stimuli)):
        word_pair = stimuli[i]
        left_over_time = word_pair.study(duration, scale, end_after_input=end_trial_after_scale_input, ticker=timer, frame_rate=fps, exit_keys=keys_to_quit, other_files=other_files)
        if isi_object:
            if scale:
                time_to_add = left_over_time
            elif i == 0:
                time_to_add = 0
            isi_object.present(time_change=time_to_add, clock=timer, frame_rate=fps, exit_keys=keys_to_quit, files=other_files)
...
def cued_recall_test(stimuli, isi_object=None, isi_duration=None, isi_fixation='+', isi_location='middle centre', isi_font=None, isi_colour=None, isi_background=None, isi_antialiasing=True, randomize=False, response_font=None, use_antialiasing=True, timer=None, fps=30, allowed_keys=LETTERS, keys_to_quit=(K_ESCAPE,), data_file=None, other_files=()):
    ...
    Keyword Parameters:
        ...
        other_files: any currently opened files that need to be closed if the
            program is closed.
    """
    if not not any(k in allowed_keys for k in (K_RETURN, K_BACKSPACE)):
        raise AssertionError('Neither return nor backspace may appear in allowed_keys.')
        assert not any(k in keys_to_quit for k in (K_BACKSPACE, K_RETURN)), 'Neither return nor backspace may appear in keys_to_quit.'
        assert not any(k in allowed_keys for k in keys_to_quit), 'There may not be overlap between allowed_keys and keys_to_quit.'
        if not fps > 0:
            raise AssertionError('fps must be positive.')
            if randomize:
                np.random.shuffle(stimuli)
            if not isi_object and isi_duration:
                if isi_fixation:
                    ...
                    isi_colour = isi_colour or stimuli[0].colour
            isi_background = isi_background or stimuli[0].background
        isi_object = InterTrialStimulus(isi_duration, isi_fixation, isi_font, isi_colour, isi_background, isi_antialiasing, isi_location)
    for word_pair in stimuli:
        word_pair.test(allowed_keys, timer, fps, keys_to_quit, other_files)
        try:
            isi_object.present(clock=timer, frame_rate=fps, exit_keys=keys_to_quit, files=other_files)
        except AttributeError:
            pass
...
def rate_images(images, response_keys=NUMBERS, start_after=0, end_after=None, isi=None, ticker=None, frame_rate=30, quit_keys=(K_ESCAPE,), files=()):
    ...
    for i in xrange(len(images)):
        image = images[i]
        image.rate(allowed_keys=response_keys, begin_time=start_after, finish_time=end_after, c=ticker, fps=frame_rate, exit_keys=quit_keys, files=files)
