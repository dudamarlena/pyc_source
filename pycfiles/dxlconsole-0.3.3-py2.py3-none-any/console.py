...
# [GCC 8.4.0]
# Embedded file name: /opendxl/viji/opendxl-console/dxlconsole/console.py
# Compiled at: 2019-06-07 18:26:01
from __future__ import absolute_import
import base64, logging, uuid, threading, pkg_resources, tornado
...
class ConsoleRequestHandler(BaseRequestHandler):
    ...
    def get(self, *args, **kwargs):
        ...
        HTTP GET
        """
        console_html = pkg_resources.resource_string(__name__, 'console.html').decode('utf8')
        ...
        for module in self.application.modules:
            if module.enabled:
                ...
                toolstrip_button = "isc.ToolStripButton.create({                         autoDraw:false,                         ID: '" + button_name + "',                         iconWidth:64,                         iconHeight:64,                         icon: '" + module.get_icon_path + "',                         actionType: 'radio',                         showClippedTitleOnHover: true,                         titleHoverHTML: function() { return '" + module.title + '\'; },                         titleClipped: function() { return true; },                         radioGroup: \'console_module\',                         click: \'console_deck.setCurrentPane("' + module.root_content_name + '")\',                     });'
                console_html += '\n' + toolstrip_button
        ...
        console_html += "isc.Deck.create({autoDraw:false, ID: 'console_deck', panes: [" + module_names + '] });'
        if first_button:
            console_html += first_button + '.select();'
            console_html += "console_deck.setCurrentPane('" + first_pane + "');"
        console_html += "isc.HLayout.create({ width: '100%', height: '100%', " + "members: ['console_toolstrip', 'console_deck'] });"
...
class LoginHandler(RequestHandler):
    ...
    def get(self, *args, **kwargs):
        ...
        if auth_header is not None:
            ...
            if username == self.application.bootstrap_app.username and password == self.application.bootstrap_app.password:
            ...
            console_html = pkg_resources.resource_string(__name__, 'login.html').decode('utf8')
            console_html = console_html.replace('@CONSOLE_NAME@', self.application.bootstrap_app.console_name)
            self.write(console_html)
        return
    ...
    def post(self, *args, **kwargs):
        ...
        if name == self.application.bootstrap_app.username and password == self.application.bootstrap_app.password:
...
class WebConsole(Application):
    ...
    def __init__(self, app):
        ...
        handlers = [
         (
          '/public/(.*)', ConsoleStaticFileRequestHandler, {'path': ''}),
         (
          '/favicon.ico(.*)', ConsoleStaticFileRequestHandler, {'path': 'images/favicon.ico'}),
         (
          '/login', LoginHandler),
         (
         ...
         (
          '/websocket', ConsoleWebSocketHandler, dict(module=self))]
    ...
    def send_web_socket_message(self, client_id, message):
        ...
        """
        with self._web_socket_dict_lock:
            self.io_loop.add_callback(self._web_socket_dict[client_id].write_message, message)
    ...
    def send_all_web_socket_message(self, message):
        ...
        with self._web_socket_dict_lock:
            for key in self._web_socket_dict:
                try:
                    self.io_loop.add_callback(self._web_socket_dict[key].write_message, message)
                except Exception:
                    pass
    ...
    def start(self):
        ...
        http_server = HTTPServer(self, ssl_options={'certfile': client_config.cert_file, 
