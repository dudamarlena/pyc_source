...
# [GCC 8.4.0]
# Embedded file name: /opendxl/viji/opendxl-console/dxlconsole/modules/monitor/services_handler.py
...
class ServiceUpdateHandler(BaseRequestHandler):
    ...
    def get(self, *args, **kwargs):
        ...
        for service_guid in self._module.services:
            ...
            entry = {'itemId': service.get('serviceGuid'), 'itemName': service.get('serviceType'), 
               ...
               'metaData': '<pre><code>' + json.dumps(service.get('metaData'), indent=4, sort_keys=True) + '</code></pre>'}
            response['data'].append(entry)
            response['totalRows'] += 1
            for request_topic in service['requestChannels']:
                entry = {'itemId': service['serviceGuid'] + request_topic, 'itemName': request_topic, 
