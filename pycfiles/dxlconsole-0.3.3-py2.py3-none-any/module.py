...
# [GCC 8.4.0]
# Embedded file name: /opendxl/viji/opendxl-console/dxlconsole/modules/topology/module.py
# Compiled at: 2019-06-07 18:26:01
from __future__ import print_function
from __future__ import absolute_import
import threading
from threading import Thread
import logging, json, tornado, pkg_resources
...
class TopologyModule(Module):
    ...
    def __init__(self, app):
        ...
        """
        super(TopologyModule, self).__init__(app, 'topology', 'Fabric Topology', '/public/images/topology.png', 'topology_layout')
        self.app.dxl_service_client.add_event_callback(TopologyModule.FABRIC_CHANGE_EVENT_TOPIC, _FabricChangeEventCallback(self))
        self._reconnect_refresh_thread = threading.Thread(target=self._refresh_broker_registry)
    ...
    def content(self):
        ...
        """
        return pkg_resources.resource_string(__name__, 'content.html').decode('utf8')
    ...
    def handlers(self):
        ...
        return [
         (
          '/broker_registry_query', BrokerRegistryQueryHandler, dict(module=self))]
    ...
    def update_broker_registry(self):
        ...
        self.app.send_all_web_socket_message('brokersUpdated')
...
class BrokerRegistryQueryHandler(BaseRequestHandler):
    ...
    def get(self, *args, **kwargs):
        ...
        current_broker = query_broker_id if query_broker_id else self._module.current_connected_broker
    ...
    def _build_connected_broker_list(self, current_broker, brokers, connected_broker_list):
        ...
        entry = {'brokerId': broker.get('guid'), 'connectionLimit': str(broker.get('connectionLimit')), 
           ...
           'webSocketPort': str(broker.get('webSocketPort')) if broker.get('webSocketPort') is not None and broker.get('webSocketPort') != 0 else '', 
