...
# [GCC 8.4.0]
# Embedded file name: /opendxl/viji/opendxl-console/dxlconsole/modules/monitor/messages_handler.py
        ...
        if messages:
            for message in messages:
                ...
                if message.message_type == Message.MESSAGE_TYPE_ERROR:
                    ...
                    try:
                        payload = '<pre><code>' + self.escape(MessageUtils.dict_to_json(MessageUtils.json_payload_to_dict(message), True)) + '</pre></code>'
                    except Exception:
                        try:
                ...
                message_type = 'Event' if message.message_type == Message.MESSAGE_TYPE_EVENT else 'Response' if message.message_type == Message.MESSAGE_TYPE_RESPONSE else 'Request' if message.message_type == Message.MESSAGE_TYPE_REQUEST else 'Error Response' if message.message_type == Message.MESSAGE_TYPE_ERROR else 'Unknown'
                message_entry = {'topic': topic, 
                   'received': '', 
                   ...
                   'otherFields': '<pre><code>' + self.escape(MessageUtils.dict_to_json(message.other_fields, True)) + '</pre></code>'}
                response['data'].append(message_entry)

        self._module.clear_messages(client_id)
        logger.debug('Message handler response: %s', json.dumps(response_wrapper))
