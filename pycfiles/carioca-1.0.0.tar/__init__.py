...
# Embedded file name: /home/scott/.virtualenvs/carinata/lib/python2.7/site-packages/carinata/__init__.py
# Compiled at: 2014-08-06 17:34:30
...
class TestGenerator(object):
    ...
    def process_line_match(self, lineno, line_match):
        ...
        if len(self.blocks) != 1 and block.indent <= self.blocks[(-1)].indent:
            self.blocks = [ b for b in self.blocks if b.is_applicable(block.indent) ]
    ...
    def defer_it(self):
        u"""Put an ‘it’ block on a list to be bundled into a test class"""
        self.blocks[(-1)].decorators = self.deferred_decorators[:]
        self.deferred_decorators = []
        self.deferred_its.append(self.blocks[(-1)])

    def process_its(self, args=None):
        u"""Process the ‘it’ blocks given the context in the spec file"""
...
class SuiteGenerator(object):
    ...
    def _get_output(self, indir, infile):
        ...
        outfile = os.path.join(outdir, os.path.relpath(outfile, indir))
...
def parse_args():
    ...
    parser.add_argument('directories', action='append', help='The list of directories in which to search for spec files (always recursive)')
    parser.add_argument('-o', '--output-dir', dest='output_dir', help='The directory in which to create output test files (Created in a temporary directory if this argument is not given)')
    parser.add_argument('-g', '--generate', action='store_true', default=False, help='Only generate test files, do not run them (False by default, so tests will run if this argument is not given)')
    parser.add_argument('-f', '--force', action='store_true', default=False, help='Generate test files regardless of whether original files have changed or not (False by default, so only changed tests are generated)')
    parser.add_argument('-c', '--clean', action='store_true', default=False, help='Clean up files instead of generating them')
...
def main_cmdline():
    ...
    args = parse_args()
    main(args.directories, args.output_dir, args.generate, args.force, args.clean)
