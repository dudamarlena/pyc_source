...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.4/dist-packages/sciroccoclient/http/requestadapter.py
...
class RequestsAdapter:
    ...
    def __init__(self, request_manager, request_manager_response_handler, meta_data_http, content_type_detector):
        self._api_url = None
        self._node_id = None
    ...
    def get_fixed_headers(self):
        return {self.meta_data_http.get_http_header_by_field_name('node_source'): self._node_id, 
...
class RequestManagerContentTypeDetector:
    ...
    def detect_from_body(self, body):
        if self.check_for_binary(body):
            ...
            if self.check_for_object(body):
                return 'application/json'
            if self.check_for_string(body):
                return 'text/plain'
            return self.default_type
