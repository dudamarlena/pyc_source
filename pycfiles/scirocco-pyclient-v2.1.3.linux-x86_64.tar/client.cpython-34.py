...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.4/dist-packages/sciroccoclient/client.py
# Compiled at: 2016-11-22 16:46:46
# Size of source mod 2**32: 3800 bytes
import inspect, time, signal
...
class Client:
    ...
    def on_receive(self, callback, async=False, request_interval=0.5):
        ...
        if len(inspect.signature(callback).parameters) < 2:
            raise SciroccoInvalidOnReceiveCallBackError('Callback must have 2 params, first for ack function, second for received message.')
