...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/dba-project/dba/mongodb/backup/strategy_backup_every_x_seconds_for_y_seconds.py
# Compiled at: 2019-04-16 13:10:13
...
import os, re, uuid, boto3, datetime, subprocess
...
def create_gzip_archive_filename(dbname):
    ...
    return tpl.format(dbname=dbname,
      utcnow_formatted=utcnow_formatted,
      guid=guid)
...
def backup_to_local(dbname):
    ...
    mongodump_cmd = 'mongodump --archive={archive_path} --db {dbname} --gzip'.format(archive_path=archive_path,
      dbname=dbname)
...
def backup_to_local_and_s3(dbname):
    ...
    mongodump_cmd = 'mongodump --archive={archive_path} --db {dbname} --gzip'.format(archive_path=archive_path,
      dbname=dbname)
    subprocess.call(command_to_array(mongodump_cmd))
    archive_s3_uri = '{MONGO_BACKUP_S3_URI}/{dbname}/{archive_filename}'.format(MONGO_BACKUP_S3_URI=MONGO_BACKUP_S3_URI,
      dbname=dbname,
      archive_filename=archive_filename)
    s3_cp_cmd = 'aws s3 cp {archive_path} {archive_s3_uri} --profile {aws_profile}'.format(archive_path=archive_path,
...
def remove_expired_backup(dbname, period_days):
    ...
    s3 = ses.client('s3')
    response = s3.list_objects(Bucket=(Config.MONGO_BACKUP_S3_BUCKET),
