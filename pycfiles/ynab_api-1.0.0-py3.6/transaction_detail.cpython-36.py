...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ynab_api/models/transaction_detail.py
# Compiled at: 2019-11-21 19:08:24
# Size of source mod 2**32: 21662 bytes
"""
    YNAB API Endpoints
...
class TransactionDetail(object):
    ...
    def __init__(self, id=None, date=None, amount=None, memo=None, cleared=None, approved=None, flag_color=None, account_id=None, payee_id=None, category_id=None, transfer_account_id=None, transfer_transaction_id=None, matched_transaction_id=None, import_id=None, deleted=None, account_name=None, payee_name=None, category_name=None, subtransactions=None, local_vars_configuration=None):
    ...
    def cleared(self, cleared):
        ...
        if self.local_vars_configuration.client_side_validation:
            if cleared is None:
                raise ValueError('Invalid value for `cleared`, must not be `None`')
        allowed_values = [
         'cleared', 'uncleared', 'reconciled']
        if self.local_vars_configuration.client_side_validation:
            if cleared not in allowed_values:
                raise ValueError('Invalid value for `cleared` ({0}), must be one of {1}'.format(cleared, allowed_values))
    ...
    def flag_color(self, flag_color):
        ...
        if self.local_vars_configuration.client_side_validation:
            if flag_color not in allowed_values:
                raise ValueError('Invalid value for `flag_color` ({0}), must be one of {1}'.format(flag_color, allowed_values))
    ...
    def subtransactions(self, subtransactions):
        ...
        if self.local_vars_configuration.client_side_validation:
            if subtransactions is None:
                raise ValueError('Invalid value for `subtransactions`, must not be `None`')
