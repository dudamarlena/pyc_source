...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ynab_api/models/category.py
# Compiled at: 2019-11-21 19:08:24
# Size of source mod 2**32: 16962 bytes
"""
    YNAB API Endpoints
    ...
    def category_group_id(self, category_group_id):
        ...
        if self.local_vars_configuration.client_side_validation:
            if category_group_id is None:
                raise ValueError('Invalid value for `category_group_id`, must not be `None`')
    ...
    def goal_type(self, goal_type):
        ...
        if self.local_vars_configuration.client_side_validation:
            if goal_type not in allowed_values:
                raise ValueError('Invalid value for `goal_type` ({0}), must be one of {1}'.format(goal_type, allowed_values))
