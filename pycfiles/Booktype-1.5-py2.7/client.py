...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/redis/client.py
# Compiled at: 2012-02-14 23:34:00
import datetime, errno, socket, threading, time, warnings
...
class ConnectionPool(threading.local):
    ...
    def get_connection(self, host, port, db, password, socket_timeout):
        ...
        if key not in self.connections:
            self.connections[key] = Connection(host, port, db, password, socket_timeout)
    ...
    def read(self, length=None):
        ...
        try:
            ...
            if e.args and e.args[0] == errno.EAGAIN:
                raise ConnectionError('Error while reading from socket: %s' % e.args[1])
...
def list_or_args(command, keys, args):
    ...
    if oldapi:
        warnings.warn(DeprecationWarning('Passing *args to Redis.%s has been deprecated. Pass an iterable to ``keys`` instead' % command))
        keys.extend(args)
    return keys
...
class Redis(threading.local):
    ...
    def __init__(self, host='localhost', port=6379, db=0, password=None, socket_timeout=None, connection_pool=None, charset='utf-8', errors='strict'):
    ...
    def pipeline(self, transaction=True):
        ...
        """
        return Pipeline(self.connection, transaction, self.encoding, self.errors)
    ...
    def _execute_command(self, command_name, command, **options):
        ...
        if self.subscribed and not subscription_command:
            raise RedisError('Cannot issue commands other than SUBSCRIBE and UNSUBSCRIBE while channels are open')
        try:
    ...
    def execute_command(self, *args, **options):
        ...
        return self._execute_command(args[0], ('*%s\r\n%s' % (cmd_count, ('').join(cmds))), **options)
        ...
        if byte == ':':
            ...
            if byte == '*':
                ...
                for i in range(length):
                    try:
                        data.append(self._parse_response(command_name, catch_errors))
    ...
    def get_connection(self, host, port, db, password, socket_timeout):
        ...
        conn = self.connection_pool.get_connection(host, port, db, password, socket_timeout)
    ...
    def select(self, db, host=None, port=None, password=None, socket_timeout=None):
        """
        ...
        if host is None:
            if self.connection is None:
                raise RedisError('A valid hostname or IP address must be specified')
            host = self.connection.host
        if port is None:
            if self.connection is None:
                raise RedisError('A valid port must be specified')
            port = self.connection.port
        self.connection = self.get_connection(host, port, db, password, socket_timeout)
        return
    ...
    def flush(self, all_dbs=False):
        warnings.warn(DeprecationWarning("'flush' has been deprecated. Use Redis.flushdb() or Redis.flushall() instead"))
        if all_dbs:
        ...
        * The following flags have been deprecated *
        ...
        if kwargs:
            if 'preserve' in kwargs:
                warnings.warn(DeprecationWarning("preserve option to 'rename' is deprecated, use Redis.renamenx instead"))
                if kwargs['preserve']:
    ...
    def set(self, name, value, **kwargs):
        ...
        * The following flags have been deprecated *
        ...
        if kwargs:
            if 'getset' in kwargs:
                warnings.warn(DeprecationWarning("getset option to 'set' is deprecated, use Redis.getset() instead"))
                if kwargs['getset']:
                    return self.getset(name, value)
            if 'preserve' in kwargs:
                warnings.warn(DeprecationWarning("preserve option to 'set' is deprecated, use Redis.setnx() instead"))
                if kwargs['preserve']:
    ...
    def pop(self, name, tail=False):
        ...
        * This method has been deprecated,
        ...
        """
        warnings.warn(DeprecationWarning('Redis.pop has been deprecated, use Redis.lpop or Redis.rpop instead'))
        if tail:
    ...
    def push(self, name, value, head=False):
        ...
        * This method has been deprecated,
        ...
        """
        warnings.warn(DeprecationWarning('Redis.push has been deprecated, use Redis.lpush or Redis.rpush instead'))
        if head:
    ...
    def zincr(self, key, member, value=1):
        ...
        warnings.warn(DeprecationWarning('Redis.zincr has been deprecated, use Redis.zincrby instead'))
    ...
    def zinter(self, dest, keys, aggregate=None):
        warnings.warn(DeprecationWarning('Redis.zinter has been deprecated, use Redis.zinterstore instead'))
    ...
    def zunion(self, dest, keys, aggregate=None):
        warnings.warn(DeprecationWarning('Redis.zunion has been deprecated, use Redis.zunionstore instead'))
    ...
    def listen(self):
        ...
        while self.subscribed:
            ...
            if r[0] == 'pmessage':
                msg = {'type': r[0], 'pattern': r[1], 
                   'channel': r[2], 
                   'data': r[3]}
            else:
                msg = {'type': r[0], 'pattern': None, 
                   'channel': r[1], 
                   'data': r[2]}
...
class Pipeline(Redis):
    ...
    def _execute_command(self, command_name, command, **options):
        ...
        if command_name in ('AUTH', 'SELECT'):
            return super(Pipeline, self)._execute_command(command_name, command, **options)
        self.command_stack.append((command_name, command, options))
        return self

    def _execute_transaction(self, commands):
        all_cmds = ('').join([ c for _1, c, _2 in chain((('', 'MULTI\r\n', ''), ), commands, (('', 'EXEC\r\n', ''), ))
                             ])
    ...
    def _execute_pipeline(self, commands):
        ...
        for command_name, _, options in commands:
            data.append(self.parse_response(command_name, catch_errors=True, **options))
