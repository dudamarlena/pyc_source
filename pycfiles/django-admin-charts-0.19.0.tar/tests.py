...
# [GCC 8.4.0]
# Embedded file name: /home/petr/.local/share/virtualenvs/blenderhub_server-izvs0qj4/src/django-admin-charts/admin_tools_stats/tests.py
# Compiled at: 2020-04-02 04:59:48
import datetime
...
class AdminToolsStatsAdminCharts(BaseAuthenticatedClient):
    ...
    def test_admin_dashboard_page(self):
        ...
        self.assertContains(response, '<h2>User graph</h2>', html=True)
        self.assertContains(response, '<h2>User logged in graph</h2>', html=True)
        self.assertContains(response, '<svg style="width:100%;height:300px;"></svg>', html=True)
        self.assertContains(response, '<option value="true">Active</option>', html=True)
        self.assertContains(response, '<option value="false">Inactive</option>', html=True)

    def test_admin_dashboard_page_multi_series(self):
        stats = mommy.make('DashboardStats', date_field_name='date_joined', model_name='User', model_app_name='auth', graph_key='user_graph1')
        criteria = mommy.make('DashboardStatsCriteria', criteria_name='active', dynamic_criteria_field_name='is_active', criteria_dynamic_mapping={'': [
              None, 'All'], 
           'false': [
                   False, 'Inactive'], 
           'true': [
        ...
        response = self.client.get('/admin/')
        self.assertContains(response, '<option class="chart-input" value="">-------</option>', html=True)
        self.assertContains(response, '<option class="chart-input" value="%s" selected="selected">active</option>' % stats.id, html=True)
        return

    def test_admin_dashboard_page_post(self):
        """Test function to check dashboardstatscriteria admin pages"""
        response = self.client.post('/admin/', {'select_box_user_graph': 'true'})
        self.assertContains(response, '<input type="hidden" class="hidden_graph_key" name="graph_key" value="user_graph">', html=True)
        self.assertContains(response, '<option value="true">Active</option>', html=True)
...
class ModelTests(TestCase):
    ...
    def setUp(self):
        self.stats = mommy.make('DashboardStats', date_field_name='date_joined', model_name='User', model_app_name='auth', graph_key='user_graph')
        self.kid_stats = mommy.make('DashboardStats', date_field_name='birthday', model_name='TestKid', model_app_name='demoproject', graph_key='kid_graph')
    ...
    def test_get_multi_series(self):
        ...
        testing_data = {datetime.datetime(2010, 10, 8, 0, 0): {'': 0}, datetime.datetime(2010, 10, 9, 0, 0): {'': 0}, datetime.datetime(2010, 10, 10, 0, 0): {'': 1}, datetime.datetime(2010, 10, 11, 0, 0): {'': 0}, datetime.datetime(2010, 10, 12, 0, 0): {'': 0}}
    ...
    def test_get_multi_series_datetime_tz(self):
        ...
        interval = 'days'
        serie = self.stats.get_multi_time_series({}, time_since, time_until, interval)
        testing_data = {current_tz.localize(datetime.datetime(2010, 10, 9, 0, 0)): {'': 0}, current_tz.localize(datetime.datetime(2010, 10, 10, 0, 0)): {'': 1}, current_tz.localize(datetime.datetime(2010, 10, 11, 0, 0)): {'': 0}}
        self.assertDictEqual(serie, testing_data)
    ...
    def test_get_multi_series_date_tz(self):
        ...
        testing_data = {datetime.date(2010, 10, 9): {'': 0}, datetime.date(2010, 10, 10): {'': 1}, datetime.date(2010, 10, 11): {'': 0}}
        self.assertDictEqual(serie, testing_data)
    ...
    def test_get_multi_series_change_dst(self):
        ...
        mommy.make('TestKid', birthday=datetime.date(2019, 10, 27))
        time_since = datetime.datetime(2019, 10, 26)
        time_until = datetime.datetime(2019, 10, 29)
        interval = 'days'
        serie = self.kid_stats.get_multi_time_series({}, time_since, time_until, interval)
        testing_data = {datetime.date(2010, 10, 9): {'': 0}, datetime.date(2010, 10, 10): {'': 1}, datetime.date(2010, 10, 11): {'': 0}}
        self.assertDictEqual(serie, testing_data)
    ...
    def test_get_multi_series_distinct_count(self):
        ...
        stats = mommy.make('DashboardStats', model_name='User', date_field_name='date_joined', model_app_name='auth', type_operation_field_name='Count', distinct=True, operation_field_name='first_name')
        ...
        testing_data = {datetime.datetime(2010, 10, 8, 0, 0): {'': 0}, datetime.datetime(2010, 10, 9, 0, 0): {'': 0}, datetime.datetime(2010, 10, 10, 0, 0): {'': 2}, datetime.datetime(2010, 10, 11, 0, 0): {'': 0}, datetime.datetime(2010, 10, 12, 0, 0): {'': 0}}
        self.assertDictEqual(serie, testing_data)
    ...
    def test_get_multi_series_distinct_functions(self):
        ...
        for func, result in (
         ...
         ('AvgCountPerInstance', 1)):
            stats = mommy.make('DashboardStats', model_name='TestKid', date_field_name='birthday', model_app_name='demoproject', type_operation_field_name=func, operation_field_name='age')
    ...
    def test_get_multi_series_dynamic_field_name(self):
        ...
        testing_data = {datetime.datetime(2010, 10, 10, 0, 0): OrderedDict((('Active', 0), ('Inactive', 0))), 
    ...
    def test_get_multi_series_dynamic_field_name_old_format(self):
        ...
        """
        criteria = mommy.make('DashboardStatsCriteria', criteria_name='active', dynamic_criteria_field_name='is_active', criteria_dynamic_mapping={'False': 'Inactive', 
           'True': 'Active'})
        m2m = mommy.make('CriteriaToStatsM2M', criteria=criteria, stats=self.stats, use_as='multiple_series')
        mommy.make('User', date_joined=datetime.date(2010, 10, 12), is_active=True)
        mommy.make('User', date_joined=datetime.date(2010, 10, 13), is_active=False)
    ...
    def test_get_multi_series_criteria_without_dynamic_mapping(self):
        ...
        """
        criteria = mommy.make('DashboardStatsCriteria', criteria_name='active', dynamic_criteria_field_name='is_active')
        m2m = mommy.make('CriteriaToStatsM2M', criteria=criteria, stats=self.stats, use_as='multiple_series')
        mommy.make('User', date_joined=datetime.date(2010, 10, 12), is_active=True)
        mommy.make('User', date_joined=datetime.date(2010, 10, 13), is_active=False)
        time_since = datetime.datetime(2010, 10, 10)
        ...
        interval = 'days'
        serie = self.stats.get_multi_time_series({'select_box_multiple_series': m2m.id}, time_since, time_until, interval)
        testing_data = {datetime.datetime(2010, 10, 10, 0, 0): OrderedDict((('True', 0), ('False', 0))), 
    ...
    def test_get_multi_series_criteria_without_dynamic_mapping_choices(self):
        ...
        """
        criteria = mommy.make('DashboardStatsCriteria', criteria_name='name', dynamic_criteria_field_name='last_name')
        m2m = mommy.make('CriteriaToStatsM2M', criteria=criteria, stats=self.stats, use_as='multiple_series')
        ...
        interval = 'days'
        serie = self.stats.get_multi_time_series({'select_box_multiple_series': m2m.id}, time_since, time_until, interval)
        testing_data = {datetime.datetime(2010, 10, 10, 0, 0): OrderedDict((('Bar', 0), ('Foo', 0))), 
    ...
    def test_get_multi_series_criteria_combine(self):
        ...
        """
        criteria = mommy.make('DashboardStatsCriteria', criteria_name='name', dynamic_criteria_field_name='last_name')
        criteria_active = mommy.make('DashboardStatsCriteria', criteria_name='active', dynamic_criteria_field_name='is_active')
        m2m = mommy.make('CriteriaToStatsM2M', criteria=criteria, stats=self.stats, use_as='multiple_series')
    ...
    def test_get_multi_series_fixed_criteria(self):
        ...
        """
        criteria = mommy.make('DashboardStatsCriteria', criteria_name='name', dynamic_criteria_field_name='last_name')
        criteria_active = mommy.make('DashboardStatsCriteria', criteria_name='active', criteria_fix_mapping={'is_active': True})
        m2m = mommy.make('CriteriaToStatsM2M', criteria=criteria, stats=self.stats, use_as='multiple_series')
...
class AdminToolsStatsModel(TestCase):
    ...
    def setUp(self):
        self.dashboard_stats_criteria = DashboardStatsCriteria(criteria_name='call_type', criteria_fix_mapping='', dynamic_criteria_field_name='disposition', criteria_dynamic_mapping={'INVALIDARGS': 'INVALIDARGS', 
           'BUSY': 'BUSY', 
           'TORTURE': 'TORTURE', 
           'ANSWER': 'ANSWER', 
           'DONTCALL': 'DONTCALL', 
        ...
        self.dashboard_stats = mommy.make('DashboardStats', graph_key='user_graph_test', graph_title='User graph', model_app_name='auth', model_name='User', date_field_name='date_joined', is_visible=1)
