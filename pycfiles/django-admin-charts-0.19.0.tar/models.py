...
# [GCC 8.4.0]
# Embedded file name: /home/petr/.local/share/virtualenvs/blenderhub_server-izvs0qj4/src/django-admin-charts/admin_tools_stats/models.py
# Compiled at: 2020-04-02 04:52:31
import logging
...
class DashboardStatsCriteria(models.Model):
    ...
    **Name of DB table**: dash_stats_criteria
    ...
    criteria_dynamic_mapping = jsonfield.fields.JSONField(null=True, blank=True, verbose_name=_('dynamic criteria / value'), help_text=_(mark_safe('a JSON dictionary with records in two following possible formats:<br/>"key_value": "name"<br/>"key": [value, "name"]<br/>use blank key for no filter<br/>Example:<br/><pre>{<br/>  "": [null, "All"],<br/>  "True": [true, "True"],<br/>  "False": [false, "False"]<br/>}</pre><br/>Left blank to exploit all choices of CharField with choices')))
...
class DashboardStats(models.Model):
    ...
    **Name of DB table**: dashboard_stats
    """
    graph_key = models.CharField(unique=True, max_length=90, verbose_name=_('graph identifier'), help_text=_('it needs to be one word unique. ex. auth, mygraph'))
    ...
    date_field_name = models.CharField(max_length=90, verbose_name=_('date field name'), help_text=_('ex. date_joined, invitation__invitation_date'))
    user_field_name = models.CharField(max_length=90, verbose_name=_('user field name'), null=True, blank=True, help_text=_('ex. owner, invitation__owner'))
    operation_field_name = models.CharField(max_length=90, verbose_name=_('Operate field name'), null=True, blank=True, help_text=_('The field you want to aggregate, ex. amount, salaries__total_income'))
    distinct = models.BooleanField(default=False, null=False, blank=True, help_text=_("Note: Distinct is supported only for Count, Sum, Avg and 'Avgerage count per active model instance'.<br/>Django>=3.0 is needed for distinct Sum and Avg."))
    ...
    default_chart_type = models.CharField(max_length=90, verbose_name=_('Default chart type'), null=False, blank=False, choices=chart_types, default='discreteBarChart')
    default_time_period = models.PositiveIntegerField(verbose_name=_('Default period'), help_text=_('Number of days'), null=False, blank=False, default=31)
    default_time_scale = models.CharField(verbose_name=_('Default time scale'), null=False, blank=False, default='days', choices=time_scales, max_length=90)
    y_axis_format = models.CharField(max_length=90, verbose_name=_('Y axis format'), help_text=_("Format of Y axis.<a href='https://github.com/d3/d3-format' target='_blank'>See description of possible values</a>."), null=True, blank=True, default=None)
    ...
    def get_time_series(self, dynamic_criteria, all_criteria, request, time_since, time_until, interval):
        ...
        for dkwargs in dynamic_kwargs:
            ...
            operation = {'AvgCountPerInstance': lambda field_name, distinct, dkwargs: ExpressionWrapper(1.0 * Count(field_name, distinct=distinct, filter=dkwargs) / Count('id', distinct=True, filter=Q(**{field_name + '__isnull': False})), output_field=models.FloatField()), 
    ...
    def get_control_form(self):
        ...
        temp += '</select>'
        temp += 'Since: <input class="chart-input select_box_date_since" type="date" name="time_since" value="%s">' % (now() - timedelta(days=self.default_time_period)).strftime('%Y-%m-%d')
        temp += 'Until: <input class="chart-input select_box_date_since" type="date" name="time_until" value="%s">' % now().strftime('%Y-%m-%d')
...
class CriteriaToStatsM2M(models.Model):
    ...
    criteria = models.ForeignKey(DashboardStatsCriteria, on_delete=models.CASCADE)
    stats = models.ForeignKey(DashboardStats, on_delete=models.CASCADE)
    order = models.PositiveIntegerField(unique=True, null=True, blank=True)
    prefix = models.CharField(max_length=255, verbose_name=_('criteria field prefix'), default='', help_text=_('prefix, that will be added befor all lookup paths of criteria'), blank=True)
    use_as = models.CharField(max_length=90, blank=False, null=False, verbose_name=_('Use dynamic criteria as'), choices=(
     ('chart_filter', 'Chart filter'),
     ('multiple_series', 'Multiple series')), default='chart_filter')
    ...
    def _get_dynamic_choices(self, slef):
        ...
        if field_name:
            if field_name.endswith('__isnull'):
                ...
                choices.update((i, (i, fchoices[i] if i in fchoices else i)) for i in model.objects.values_list(field_name, flat=True).distinct().order_by(field_name))
                return choices
