...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/droneapi/lib/WebClient.py
# Compiled at: 2015-07-24 17:03:38
from webapi_pb2 import *
import sys, socket, time, logging, uuid, threading
...
class GCSHooks(object):
    ...
    def __init__(self, host='api.3drobotics.com', port=5555):
        ...
        self.startTime = long(round(time.time() * 1000000.0))

    def filterMavlink(self, fromInterface, bytes):
        curtime = long(round(time.time() * 1000000.0))
        e = Envelope()
        l = e.mavlink
