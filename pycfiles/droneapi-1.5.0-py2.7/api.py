...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/droneapi/module/api.py
# Compiled at: 2015-08-12 17:19:03
import os, time, threading, traceback, logging, math
from pymavlink import mavutil
from MAVProxy.modules.lib import mp_module
from droneapi.lib.WebClient import *
from droneapi.lib import APIConnection, Vehicle, VehicleMode, Location, Attitude, GPSInfo, Parameters, CommandSequence, APIException, Battery, Rangefinder
...
class MPCommandSequence(CommandSequence):
    ...
    def takeoff(self, alt=None):
        if alt is not None:
            ...
            self.__module.master.mav.command_long_send(self.__module.target_system, self.__module.target_component, mavutil.mavlink.MAV_CMD_NAV_TAKEOFF, 0, 0, 0, 0, 0, 0, 0, altitude)
        return
    ...
    def goto(self, l):
        ...
        self.__module.master.mav.mission_item_send(self.__module.target_system, self.__module.target_component, 0, frame, mavutil.mavlink.MAV_CMD_NAV_WAYPOINT, 2, 0, 0, 0, 0, 0, l.lat, l.lon, l.alt)
...
class APIModule(mp_module.MPModule):
    ...
    def mavlink_packet(self, m):
        ...
        if typ == 'GLOBAL_POSITION_INT':
            self.lat, self.lon = m.lat / 10000000.0, m.lon / 10000000.0
    ...
    def cmd_api(self, args):
        ...
        if args[0] == 'list':
            ...
            g = {'local_connect': self.get_connection, 
               'local_path': os.path.dirname(os.path.abspath(args[1])), 
