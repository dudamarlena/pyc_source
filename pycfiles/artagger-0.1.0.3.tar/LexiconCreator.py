...
# [GCC 8.4.0]
# Embedded file name: /root/app/RDRPOSTagger-master/Utility/LexiconCreator.py
...
def createLexicon(corpusFilePath, fullLexicon):
    if fullLexicon not in ('full', 'short'):
        ...
        for word in wordTagCounter:
            ...
            if fullLexicon == 'full':
                dictionary[word] = tag
            elif len(pairs) == 1 and pairs[0][1] > 1 or len(pairs) > 1:
                dictionary[word] = tag
            if re.search('[0-9]+', word) != None:
                ...
                if isCapital:
                    if tag not in tagCounter_CapitalizedWord:
                        ...
                        tagCounter_CapitalizedWord[tag] += 1
                elif tag not in tagCounter_Alphabet:
