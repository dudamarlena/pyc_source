...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cgcloud/core/commands.py
# Compiled at: 2016-11-22 15:21:45
from __future__ import print_function
import argparse, functools, logging, os, re, sys
...
class RoleCommand(ContextCommand):
    ...
    def __init__(self, application, **kwargs):
        ...
        self.option('role', metavar='ROLE', completer=self.completer, help=heredoc('The name of the role. Use the list-roles command to show\n                     all available roles.'))
    ...
    def run_in_ctx(self, options, ctx):
        ...
        if role is None:
            raise UserError("No such role: '%s'" % options.role)
...
class InstanceCommand(BoxCommand):
    ...
    def run_on_box(self, options, box):
        ...
        box.bind(ordinal=options.ordinal, cluster_name=options.cluster_name, wait_ready=self.wait_ready, instance_id=options.instance_id)
...
class ListCommand(BoxCommand):
    ...
    def __init__(self, application):
        ...
        self.option('--cluster-name', '-c', metavar='NAME', help='Only list boxes belonging to a cluster of the given name.')
...
class UserCommandMixin(Command):
    ...
    def __init__(self, application, **kwargs):
        ...
        self.begin_mutex()
        self.option('--login', '-l', default=None, metavar='USER', dest='user', help=heredoc("Name of user to login as. The default depends on the role,\n                     for most roles the default is the administrative user. Roles that define a\n                     second less privileged application user will default to that user. Can't be\n                     used together with -a, --admin."))
        self.option('--admin', '-a', default=False, action='store_true', help=heredoc("Force logging in as the administrative user. Can't be used\n                     together with -l, --login."))
        self.end_mutex()
        return
    ...
    def _user(box, options):
        if options.admin:
            return box.admin_account()
        return options.user or box.default_account()
...
class ShowCommand(InstanceCommand):
    ...
    def print_dict(self, d, visited, depth):
        for k, v in sorted(d.iteritems(), key=itemgetter(0)):
            k = str(k)
            if k[0:1] != '_' and k != 'connection' and not isinstance(v, EC2Connection):
                sys.stdout.write('\n%s%s: ' % ('\t' * depth, k))
                if isinstance(v, str):
                ...
                elif isinstance(v, BlockDeviceType) or isinstance(v, Group):
...
class TerminateCommand(LifecycleCommand):
    ...
    def __init__(self, application, **kwargs):
        ...
        self.option('--quick', '-Q', default=False, action='store_true', help=heredoc("Exit immediately after termination request has been made,\n                     don't wait until the box is terminated."))
...
class ListImagesCommand(BoxCommand):
    ...
    def run_on_box(self, options, box):
        for ordinal, image in enumerate(box.list_images()):
            print(('{name}\t{ordinal}\t{id}\t{state}').format(ordinal=ordinal, **image.__dict__))
...
class CreationCommand(BoxCommand):
    ...
    def __init__(self, application):
        ...
        def option(o):
            ...
            if not option_name_re.match(k):
                raise ValueError("An option name must start with a letter and contain only letters, digits and underscore. '%s' does not." % o)
            return (
             k, v)

        self.option('--option', '-O', metavar='NAME=VALUE', type=option, action='append', default=[], dest='role_options', help=heredoc('Set a role-specific option for the instance. To see a list\n                     of options for a role, use the list-options command.'))
        self.begin_mutex()
        self.option('--terminate', '-T', default=None, action='store_true', help=heredoc('Terminate the box when setup is complete. The default is to\n                     leave the box running except when errors occur.'))
        self.option('--never-terminate', '-N', default=None, dest='terminate', action='store_false', help=heredoc('Never terminate the box, even after errors. This may be\n                     useful for a post-mortem diagnosis.'))
        self.end_mutex()
        return
    ...
    def preparation_kwargs(self, options, box):
        ...
        return dict(options.role_options, ec2_keypair_globs=map(resolve_me, options.ec2_keypair_names), instance_type=options.instance_type, virtualization_type=options.virtualization_type, vpc_id=options.vpc_id, subnet_id=options.subnet_id, spot_bid=options.spot_bid, spot_launch_group=options.spot_launch_group, spot_auto_zone=options.spot_auto_zone)

    def creation_kwargs(self, options, box):
        return dict(terminate_on_error=options.terminate is not False, spot_timeout=options.spot_timeout, spot_tentative=options.spot_tentative)
    ...
    def log_ssh_hint(self, options):
        ...
        cmd = concat(hint.executable, hint.command, (opt(**option) for option in hint.options), hint.args)
    ...
    def ssh_hint(self, options):
        x = Expando
        return x(executable=os.path.basename(sys.argv[0]), command='ssh', options=[
         x(name='-n', value=options.namespace, default=self.default_namespace),
         x(name='-z', value=options.availability_zone, default=self.default_zone)], args=[
         options.role], object='box')
...
class RegisterKeyCommand(ContextCommand):
    ...
    def __init__(self, application, **kwargs):
        ...
        self.option('ssh_public_key', metavar='KEY_FILE', help=heredoc('Path of file containing the SSH public key to upload to the\n                     EC2 keypair.'))
        self.option('--force', '-F', default=False, action='store_true', help='Overwrite potentially existing EC2 key pair')
        self.option('--keypair', '-k', metavar='NAME', dest='ec2_keypair_name', default='__me__', help=heredoc('The desired name of the EC2 key pair. The name should\n                     associate the key with you in a way that it is obvious to other users in\n                     your organization.  The string __me__ anywhere in the key pair name will be\n                     replaced with the name of the IAM user whose credentials are used to issue\n                     requests to AWS.'))

    def run_in_ctx(self, options, ctx):
        with open(options.ssh_public_key) as (f):
            ssh_public_key = f.read()
        try:
            ctx.register_ssh_pubkey(ec2_keypair_name=ctx.resolve_me(options.ec2_keypair_name, drop_hostname=False), ssh_pubkey=ssh_public_key, force=options.force)
        except ValueError as e:
            raise UserError(cause=e)
...
class ListRolesCommand(Command):
    ...
    def run(self, options):
        print(tabulate((name, (role.__doc__ or '').strip().split('\n')[0].strip()) for name, role in self.application.roles.iteritems()))
        log.info('If you are expecting to see more roles listed above, you may need to set/change the CGCLOUD_PLUGINS environment variable.')
...
class ImageReferenceCommand(Command):
    ...
    ValueError
    """
    ami_id_re = re.compile('^ami-([0-9a-fA-F]{8})$')
...
class DeleteImageCommand(ImageReferenceCommand, BoxCommand):
    ...
    def __init__(self, application):
        ...
        self.begin_mutex()
        self.option('--keep-snapshot', '-K', default=False, action='store_true', help=heredoc("Do not delete the EBS volume snapshot associated with the\n                     given image. This will leave an orphaned snapshot which should be removed at\n                     a later time using the 'cgcloud cleanup' command."))
        self.option('--quick', '-Q', default=False, action='store_true', help=heredoc("Exit immediately after deregistration request has been made,\n                     don't wait until the image is deregistered. Implies --keep-snapshot."))
        self.end_mutex()

    def run_on_box(self, options, box):
        box.delete_image(options.image, wait=not options.quick, delete_snapshot=not options.keep_snapshot)
...
class RecreateCommand(ImageReferenceCommand, CreationCommand):
    ...
    def __init__(self, application):
        ...
        self.option('--quick', '-Q', default=False, action='store_true', help=heredoc("Don't wait for the box to become running or reachable via\n                     SSH. If the agent is disabled in the boot image (this is uncommon,\n                     see the --no-agent option to the 'create' command), no additional SSH\n                     keypairs will be deployed."))

    def preparation_kwargs(self, options, box):
        return dict(super(RecreateCommand, self).preparation_kwargs(options, box), image_ref=options.boot_image)

    def creation_kwargs(self, options, box):
        return dict(super(RecreateCommand, self).creation_kwargs(options, box), wait_ready=not options.quick)
...
class CreateCommand(CreationCommand):
    ...
    def __init__(self, application):
        ...
        self.option('--boot-image', '-i', metavar='AMI_ID', help=heredoc('The AMI ID of the image from which to create the box. This\n                     argument is optional and the default is determined automatically based on\n                     the role. Typically, this option does not need to be used.'))
        self.option('--no-agent', default=False, action='store_true', help=heredoc("Don't install the cghub-cloud-agent package on the box. One\n                     note-worthy effect of using this option this is that the SSH keys will be\n                     installed initially, but not maintained over time."))
        self.option('--create-image', '-I', default=False, action='store_true', help='Create an image of the box as soon as setup completes.')
        self.option('--upgrade', '-U', default=False, action='store_true', help=heredoc("Bring the package repository as well as any installed\n                     packages up to date, i.e. do what on Ubuntu is achieved by doing 'sudo\n                     apt-get update ; sudo apt-get upgrade'."))

    def preparation_kwargs(self, options, box):
        return dict(super(CreateCommand, self).preparation_kwargs(options, box), image_ref=options.boot_image, enable_agent=not options.no_agent)
...
class ResetSecurityCommand(ContextCommand):
    ...
    def run_in_ctx(self, options, ctx):
        message = 'Do you really want to delete all IAM instance profiles, IAM roles and EC2 security groups in namespace %s and its children? Although these resources will be created on-the-fly for newly created boxes, existing boxes will likely be impacted negatively.' % ctx.namespace
