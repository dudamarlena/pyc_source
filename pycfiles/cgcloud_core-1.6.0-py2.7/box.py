...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cgcloud/core/box.py
# Compiled at: 2016-11-22 15:21:45
import datetime, hashlib, socket, subprocess32, threading, time
...
from cgcloud.lib.ec2 import ec2_instance_types, wait_instances_running, inconsistencies_detected, create_spot_instances, create_ondemand_instances, tag_object_persistently
from cgcloud.lib.ec2 import retry_ec2, a_short_time, a_long_time, wait_transition
from cgcloud.lib.util import UserError, camel_to_snake, ec2_keypair_fingerprint, private_to_public_key, mean, std_dev
...
class Box(object):
    ...
    def __setup_security_groups(self, vpc_id=None):
        ...
        try:
            sg = self.ctx.ec2.create_security_group(name=name, vpc_id=vpc_id, description='Security group for box of role %s in namespace %s' % (
            ...
            if e.error_code == 'InvalidGroup.Duplicate':
                ...
                for attempt in retry_ec2(retry_while=inconsistencies_detected, retry_for=600):
                    with attempt:
    ...
    def __get_virtualization_types(self, instance_type, requested_vtype=None):
        ...
        if supported_vtypes:
            ...
            elif requested_vtype in supported_vtypes:
                virtualization_types = [
                 requested_vtype]
            else:
                raise UserError('Virtualization type %s not supported by role %s and instance type %s' % (
                 requested_vtype, self.role(), instance_type))
        else:
            raise RuntimeError('Cannot find any virtualization types supported by both role %s and instance type %s' % (
             self.role(), instance_type))
    ...
    def __get_image(self, virtualization_types, image_ref=None):
        if image_ref is None:
            for virtualization_type in virtualization_types:
                log.info('Looking up default image for role %s and virtualization type %s, ... ', self.role(), virtualization_type)
                try:
    ...
    def prepare(self, ec2_keypair_globs, instance_type=None, image_ref=None, virtualization_type=None, spot_bid=None, spot_launch_group=None, spot_auto_zone=False, vpc_id=None, subnet_id=None, **options):
        """
        ...
        if subnet_id is not None and spot_auto_zone:
            raise UserError('Cannot automatically choose an availability zone for spot instances while placing them in an explicitly defined subnet since the subnet implies a specific availability zone.')
        ...
        if vpc_id is not None and subnet_id is None:
            ...
            subnets = self.ctx.vpc.get_all_subnets(filters={'vpc-id': vpc_id, 'availability-zone': zone})
            if subnets:
                subnet_id = subnets[0].id
            else:
                raise UserError('There is no subnet belonging to VPC %s in availability zone %s. Please create a subnet manually using the VPC console.' % (
                 vpc_id, zone))
        ...
        spec = Expando(instance_type=instance_type, key_name=ec2_keypairs[0].name, placement=zone, security_group_ids=security_group_ids, subnet_id=subnet_id, instance_profile_arn=self.get_instance_profile_arn())
        self._spec_block_device_mapping(spec, image)
        self._spec_spot_market(spec, bid=spot_bid, launch_group=spot_launch_group, auto_zone=spot_auto_zone)
        return spec

    def _spec_spot_market(self, spec, bid, launch_group, auto_zone):
        if bid is not None:
            if not ec2_instance_types[spec.instance_type].spot_availability:
                raise UserError('The instance type %s is not available on the spot market.' % spec.instance_type)
            if auto_zone:
    ...
    def _choose_spot_zone(cls, zones, bid, spot_history):
       ...
       """
        ...
        for zone in zones:
            zone_histories = filter(lambda zone_history: zone_history.availability_zone == zone.name, spot_history)
    ...
    def _check_spot_bid(spot_bid, spot_history):
        ...
        if spot_bid > average * 2:
            log.warn("Your bid $ %f is more than double this instance type's average spot price ($ %f) over the last week", spot_bid, average)
    ...
    def _get_spot_history(self, instance_type):
        ...
        spot_data = self.ctx.ec2.get_spot_price_history(start_time=one_week_ago.isoformat(), instance_type=instance_type, product_description='Linux/UNIX')
        spot_data.sort(key=attrgetter('timestamp'), reverse=True)
        return spot_data

    def create(self, spec, num_instances=1, wait_ready=True, terminate_on_error=True, spot_timeout=None, spot_tentative=False, cluster_ordinal=0, executor=None):
        """
        ...
        try:
            if 'price' in spec:
                ...
                for batch in create_spot_instances(self.ctx.ec2, price, self.image_id, spec, num_instances=num_instances, timeout=spot_timeout, tentative=spot_tentative, tags=tags):
                    adopt(batch)

            else:
                adopt(create_ondemand_instances(self.ctx.ec2, self.image_id, spec, num_instances=num_instances))
            if spot_tentative:
                if not boxes:
                    return boxes
            else:
    ...
    def _batch_wait_ready(self, boxes, executor, callback):
        ...
        if not num_running + num_other == len(boxes):
            raise AssertionError
            raise (num_running or RuntimeError)('None of the instances entered the running state.')
        if num_other:
    ...
    def _get_instance_options(self):
        ...
        """
        options = dict(Name=self.ctx.to_aws_name(self.role()), generation=str(self.generation), cluster_ordinal=str(self.cluster_ordinal), cluster_name=self.cluster_name)
        for option in self.get_role_options():
    ...
    class RoleOption(namedtuple('_RoleOption', 'name type repr help inherited')):
        ...
        def type(self, value):
            try:
                ...
                raise UserError("'%s' is not a valid value for option %s" % (value, self.name))
    ...
    def bind(self, instance=None, instance_id=None, ordinal=None, cluster_name=None, wait_ready=True, verbose=True):
        """
        ...
        if self.instance is None:
            ...
            assert instance is not None and ordinal is None and cluster_name is None and instance_id is None
            name = instance.tags['Name']
            assert self.ctx.contains_aws_name(name)
            if not self.ctx.base_name(self.ctx.from_aws_name(name)) == self.role():
                raise AssertionError
            elif instance_id is not None:
                if not ordinal is None:
                    raise AssertionError
                    try:
                    ...
                    raise (self.ctx.try_contains_aws_name(name) or UserError)("Instance %s with Name tag '%s' is not in namespace %s." % (
                ...
                instance = self.__get_instance_by_ordinal(ordinal=ordinal, cluster_name=cluster_name)
            self.instance = instance
            ...
            if wait_ready:
                ...
                log.info('... bound to %s.', self.instance.id)
        return self
    ...
    def list(self, wait_ready=False, **tags):
        return [ box.bind(instance=instance, wait_ready=wait_ready, verbose=False) for box, instance in izip(concat(self, self.clones()), self.__list_instances(**tags))
               ]
    ...
    def __ordinal_sort_key(self, instance):
        return (
         instance.launch_time, instance.private_ip_address, instance.id)
    ...
    def __get_instance_by_ordinal(self, ordinal=None, cluster_name=None):
        ...
        if not instances:
            raise UserError('No instance performing role %s in namespace %s' % (self.role(), self.ctx.namespace) if cluster_name is None else 'No instance performing role %s in cluster %s and namespace %s' % (
             self.role(), cluster_name, self.ctx.namespace))
        if ordinal is None:
            if len(instances) > 1:
                raise UserError("More than one instance performing role '%s'. Please specify an ordinal, a cluster name or both to disambiguate." % self.role())
            ordinal = 0
        try:
            return instances[ordinal]
        except IndexError:
            raise UserError('No instance performing role %s in namespace %s has ordinal %i' % (self.role(), self.ctx.namespace, ordinal) if cluster_name is None else 'No instance performing role %s in cluster %s and namespace %s has ordinal %i' % (
    ...
    def image(self):
        ...
        image_id = self.ctx.ec2.create_image(instance_id=self.instance_id, name=image_name, block_device_mapping=self._image_block_device_mapping())
        while True:
            try:
    ...
    def stop(self):
        ...
        wait_transition(self.instance, from_states={
         'running', 'stopping'}, to_state='stopped')
    ...
    def terminate(self, wait=True):
        ...
        if self.instance_id is not None:
            ...
            if instance.state != 'terminated':
                ...
                if wait:
                    wait_transition(instance, from_states={
                     'running', 'shutting-down', 'stopped'}, to_state='terminated')
    ...
    def _ssh_client(self):
        ...
        client.connect(hostname=self.ip_address, username=self.admin_account(), timeout=a_short_time)
        return client
    ...
    def _ssh_args(self, user, command):
        ...
        return [
         'ssh', '%s@%s' % (user, self.host_name), '-A'] + command
    ...
    def _propagate_authorized_keys(self, user, group=None):
        ...
        """
        if group is None:
            group = run('getent group $(getent passwd %s | cut -d : -f 4) | cut -d : -f 1' % user)
        args = dict(src_user=self.admin_account(), dst_user=user, dst_group=group)
        sudo(('install -d ~{dst_user}/.ssh -m 755 -o {dst_user} -g {dst_group}').format(**args))
        sudo(('install -t ~{dst_user}/.ssh ~{src_user}/.ssh/authorized_keys -m 644 -o {dst_user} -g {dst_group}').format(**args))
        return

    @classmethod
    def recommended_instance_type(cls):
        if 'hvm' in cls.supported_virtualization_types():
            return 't2.micro'
        return 't1.micro'
    ...
    def list_images(self):
        ...
        images.sort(key=attrgetter('name'))
        return images
    ...
    def get_instance_profile_arn(self):
        ...
        aws_role_name = self.ctx.setup_iam_ec2_role(self._hash_iam_role_name(iam_role_name), policies)
        log.info('Set up instance profile using hashed IAM role name %s, derived from %s.', aws_role_name, iam_role_name)
        ...
        if len(profile.roles) > 1:
            ...
            if profile.roles.member.role_name == aws_role_name:
                return profile_arn
            self.ctx.iam.remove_role_from_instance_profile(aws_instance_profile_name, profile.roles.member.role_name)
    ...
    def __default_virtualization_type(self, instance_type):
        ...
        if family in self.paravirtual_families:
            return 'paravirtual'
        return 'hvm'
        ...
        if wait:
            ...
            while True:
                if self.ctx.ec2.get_image(image_id):
                    log.info('... image still registered, trying again in %is ...' % a_short_time)
                    time.sleep(a_short_time)
                else:
    ...
    def __delete_image_snapshot(self, image, wait=True):
        for root_device in self.possible_root_devices:
            ...
            if root_bdt:
                ...
                for attempt in retry_ec2(retry_for=a_long_time if wait else 0, retry_while=lambda e: e.error_code == 'InvalidSnapshot.InUse'):
                    with attempt:
    ...
    def _provide_generated_keypair(self, ec2_keypair_name, private_key_path, overwrite_local=True, overwrite_ec2=False):
        """
        ...
        if ec2_keypair is None:
            if key_file_exists:
                if overwrite_local:
                    ...
                    raise UserError('Private key already exists on box. Creating a new key pair in EC2 would require overwriting that file')
            ssh_privkey, ssh_pubkey = self.__generate_keypair(ec2_keypair_name, private_key_path)
        elif key_file_exists:
            ssh_privkey, ssh_pubkey = self.__verify_generated_keypair(ec2_keypair, private_key_path)
        elif overwrite_ec2:
            self.ctx.ec2.delete_key_pair(ec2_keypair_name)
            ssh_privkey, ssh_pubkey = self.__generate_keypair(ec2_keypair_name, private_key_path)
        else:
            raise UserError(('The key pair {ec2_keypair.name} is registered in EC2 but the corresponding private key file {private_key_path} does not exist on the instance. In order to create the private key file, the key pair must be created at the same time. Please delete the key pair from EC2 before retrying.').format(**locals()))
        put(local_path=StringIO(ssh_pubkey), remote_path=private_key_path + '.pub')
        return (
         ssh_privkey, ssh_pubkey)
    ...
    def __generate_keypair(self, ec2_keypair_name, private_key_path):
        ...
        assert ec2_keypair.material, "Created key pair but didn't get back private key"
    ...
    def __verify_generated_keypair(self, ec2_keypair, private_key_path):
        ...
        if ec2_keypair.fingerprint != fingerprint:
            raise UserError(("The fingerprint {ec2_keypair.fingerprint} of key pair {ec2_keypair.name} doesn't match the fingerprint {fingerprint} of the private key file currently present on the instance. Please delete the key pair from EC2 before retrying. ").format(**locals()))
        ssh_pubkey = self.ctx.download_ssh_pubkey(ec2_keypair)
        if ssh_pubkey != private_to_public_key(ssh_privkey):
            raise RuntimeError("The private key on the data volume doesn't match the public key in EC2.")
        return (ssh_privkey, ssh_pubkey)
