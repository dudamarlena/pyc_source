...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cgcloud/core/cluster_commands.py
...
from cgcloud.core.commands import RecreateCommand, ContextCommand, SshCommandMixin, RsyncCommandMixin
from cgcloud.lib.util import abreviated_snake_case_class_name, UserError, heredoc, thread_pool, allocate_cluster_ordinals
...
class ClusterTypeCommand(ContextCommand):
    ...
    def __init__(self, application):
        ...
        self.option('--num-threads', metavar='NUM', type=int, default=100, help='The maximum number of tasks to be performed concurrently.')
        self.option('cluster_type', metavar='TYPE', completer=self.completer, help=heredoc("The type of the cluster to be used. The cluster type is\n                     covariant with the role of the leader node. For example, a box performing\n                     the 'foo-leader' role will be part of a cluster of type 'foo'."))

    def completer(self, prefix, **kwargs):
        return [ cluster_type for cluster_type in self.application.cluster_types.iterkeys() if cluster_type.startswith(prefix)
               ]
...
class CreateClusterCommand(ClusterTypeCommand, RecreateCommand):
    ...
    def __init__(self, application):
        ...
        self.cluster = None
        self.option('--cluster-name', '-c', metavar='NAME', help=heredoc('A name for the new cluster. If absent, the instance ID of\n                     the master will be used. Cluster names do not need to be unique, but they\n                     should be in order to avoid user error.'))
        self.option('--num-workers', '-s', metavar='NUM', type=int, default=1, help='The number of workers to launch.')
        self.option('--ebs-volume-size', '-e', metavar='GB', help=heredoc('The size in GB of an EBS volume to be attached to each node\n                     for persistent data. The volume will be mounted at /mnt/persistent.'))
        self.option('--leader-on-demand', '-D', default=False, action='store_true', help=heredoc('Use this option to insure that the leader will be an\n                     on-demand instance, even if --spot-bid is given.'))
        self.option('--share', '-S', metavar='PATH', default=None, dest='share_path', help=heredoc("The path to a local file or directory for distribution to\n                     the cluster. The given file or directory (or the contents of the given\n                     directory, if the path ends in a slash) will be placed in the default user's\n                     ~/shared directory on each node."))
        self.option('--ssh-opts', metavar='OPTS', default=None, help=heredoc('Additional options to pass to ssh when uploading the files\n                     shared via rsync. For more detail refer to cgcloud rsync --help'))
        return

    def preparation_kwargs(self, options, box):
        return dict(super(CreateClusterCommand, self).preparation_kwargs(options, box), cluster_name=options.cluster_name, ebs_volume_size=options.ebs_volume_size)

    def creation_kwargs(self, options, box):
        return dict(super(CreateClusterCommand, self).creation_kwargs(options, box), num_instances=options.num_workers)
    ...
    def option(self, option_name, *args, **kwargs):
        ...
        if option_name == '--instance-type':
            ...
            assert args[0] == '-t'
            kwargs['help'] = kwargs['help'].replace('for the box', 'for the leader')
            _super.option('--leader-instance-type', '-T', dest='instance_type', *args[1:], **kwargs)
    ...
    def run_on_box(self, options, leader):
        ...
        if options.leader_on_demand:
            preparation_kwargs = {k:v for k, v in preparation_kwargs.iteritems() if not k.startswith('spot_') if not k.startswith('spot_')}
        spec = leader.prepare(**preparation_kwargs)
        creation_kwargs = dict(self.creation_kwargs(options, leader), num_instances=1, wait_ready=True)
        ...
        if options.num_workers:
            ...
            preparation_kwargs = dict(self.preparation_kwargs(options, first_worker), leader_instance_id=leader.instance_id, instance_type=options.worker_instance_type)
            spec = first_worker.prepare(**preparation_kwargs)
            with thread_pool(min(options.num_threads, options.num_workers)) as (pool):
                workers = first_worker.create(spec, cluster_ordinal=(leader.cluster_ordinal + 1), executor=pool.apply_async, **self.creation_kwargs(options, first_worker))
        else:
            workers = []
        if options.list:
            self.list([leader])
        ...
        if not workers:
            log.warn("This cluster has no workers. You may ssh into the leader now but you should use 'cgcloud grow-cluster' to add worker instances before doing real work.")
        self.log_ssh_hint(options)

    def run_on_creation(self, leader, options):
        local_path = options.share_path
        if local_path is not None:
            log.info('=== Copying %s%s to ~/shared on leader ===', 'the contents of ' if local_path.endswith('/') else '', local_path)
...
class GrowClusterCommand(ClusterCommand, RecreateCommand):
    ...
    def __init__(self, application):
        ...
        self.cluster = None
        self.option('--num-workers', '-s', metavar='NUM', type=int, default=1, help='The number of workers to add.')
        return
    ...
    def option(self, option_name, *args, **kwargs):
        ...
        if option_name == '--instance-type':
            assert 'dest' not in kwargs
            assert args[0] == '-t'
            kwargs['help'] = kwargs['help'].replace('for the box', 'for the workers')
        _super.option(option_name, *args, **kwargs)
    ...
    def creation_kwargs(self, options, box):
        return dict(super(GrowClusterCommand, self).creation_kwargs(options, box), num_instances=options.num_workers)
    ...
    def run_on_box(self, options, first_worker):
        ...
        leader.bind(cluster_name=options.cluster_name, ordinal=options.ordinal, wait_ready=False)
        log.info('=== Creating workers  ===')
        ...
        assert len(used_cluster_ordinals) == len(workers)
        if not 0 not in used_cluster_ordinals:
            raise AssertionError
            used_cluster_ordinals.add(0)
            cluster_ordinal = allocate_cluster_ordinals(num=options.num_workers, used=used_cluster_ordinals)
            first_worker.unbind()
            spec = first_worker.prepare(leader_instance_id=leader.instance_id, cluster_name=leader.cluster_name, **self.preparation_kwargs(options, first_worker))
            with thread_pool(min(options.num_threads, options.num_workers)) as (pool):
                workers = first_worker.create(spec, cluster_ordinal=cluster_ordinal, executor=pool.apply_async, **self.creation_kwargs(options, first_worker))
            if options.list:
                self.list(workers)
            workers or log.warn('No workers were added to the cluster.')
...
class ApplyClusterCommand(ClusterCommand):
    ...
    def __init__(self, application):
        ...
        self.option('--skip-leader', '-L', default=False, action='store_true', help=heredoc("Don't perform the operation on the leader."))
...
class ClusterLifecycleCommand(ApplyClusterCommand):
    ...
    def run_on_cluster(self, options, ctx, cluster):
        cluster.apply(partial(self.run_on_node, options), cluster_name=options.cluster_name, ordinal=options.ordinal, leader_first=self.leader_first, skip_leader=options.skip_leader, wait_ready=self.wait_ready, pool_size=options.num_threads, operation=self.operation() + '()')
...
class TerminateClusterCommand(ClusterLifecycleCommand):
    ...
    def __init__(self, application):
        ...
        self.option('--quick', '-Q', default=False, action='store_true', help="Exit immediately after termination request has been made, don't wait\n                     until the cluster is terminated.")
...
class SshClusterCommand(SshCommandMixin, ApplyClusterCommand):
    ...
    def __init__(self, application):
        ...
        self.option('--parallel', '-P', default=False, action='store_true', help=heredoc('Run command on the workers in parallel. Note that this\n                     doesn\'t work if SSH or the command itself prompts for input. This will\n                     likely be the case on the first connection attempt when SSH typically\n                     prompts for confirmation of the host key. An insecure work-around is to pass\n                     "-o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no".'))

    def run_on_cluster(self, options, ctx, cluster):
        exit_codes = []
        cluster.apply(partial(self.ssh, options), cluster_name=options.cluster_name, ordinal=options.ordinal, leader_first=True, skip_leader=options.skip_leader, pool_size=options.num_threads if options.parallel else 0, wait_ready=False, callback=exit_codes.append)
...
class RsyncClusterCommand(RsyncCommandMixin, ApplyClusterCommand):
    ...
    def run_on_cluster(self, options, ctx, cluster):
        cluster.apply(partial(self.rsync, options), cluster_name=options.cluster_name, ordinal=options.ordinal, leader_first=True, skip_leader=options.skip_leader, pool_size=options.num_threads, wait_ready=False)