...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cgcloud/core/rc_local_box.py
...
class RcLocalBox(Box):
    ...
    def _register_init_command(self, cmd):
        ...
        self._prepend_remote_shell_script(script=cmd, remote_path=rc_local_path, use_sudo=True, mirror_local_mode=True)
    ...
    env_entry_re = re.compile('^\\s*([^=\\s]+)\\s*=\\s*"?(.*?)"?\\s*$')
    ...
    def toposort2(data):
        ...
        while True:
            ...
            if not ordered:
                break
            yield ordered
            data = {item:dep - ordered for item, dep in data.iteritems() if item not in ordered}
