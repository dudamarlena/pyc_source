...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cgcloud/core/cli.py
...
def plugin_module(plugin):
    ...
    try:
        ...
        raise UserError("Cannot find plugin module '%s'. Running 'pip install %s' may fix this." % (
...
def main(args=None):
    ...
    try:
        ...
        plugins = concat(cgcloud.core, [ plugin_module(plugin) for plugin in plugins.split(':') if plugin ])
...
class CGCloud(Application):
    ...
    def __init__(self, plugins, root_logger=None):
        ...
        self.option('--debug', default=False, action='store_true', help='Write debug log to %s in current directory.' % self.debug_log_file_name)
        self.option('--script', '-s', metavar='PATH', help='The path to a Python script with additional role definitions.')
    ...
    def prepare(self, options):
        ...
        if options.script:
            plugin = imp.load_source(os.path.splitext(os.path.basename(options.script))[0], options.script)
            self._import_plugin_roles(plugin)
