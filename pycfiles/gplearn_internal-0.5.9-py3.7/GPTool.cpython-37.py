...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/gplearn/GPTool.py
# Compiled at: 2020-05-13 00:09:46
# Size of source mod 2**32: 12511 bytes
"""
...
parammathdict = {1:('add', 'sub', 'mul', 'div'), 
 2:'all', 
 3:('add', 'sub', 'mul', 'div', 'hypot', 'sin', 'cos', 'tan'), 
 4:('add', 'sub', 'mul', 'div', 'max', 'min', 'abs'), 
 5:('add', 'sub', 'mul', 'div', 'sqrt', 'log', 'abs', 'neg', 'inv', 'sin', 'cos', 'tan',
 'sigmoid', 'ceil', 'fabs', 'floor', 'trunc', 'cbrt', 'modulox'), 
 6:('sigmoid', 'and', 'or', 'xor', 'abs', 'add', 'mul', 'div', 'sub', 'hypot', 'heaviside'), 
 7:('sigmoid', 'and', 'or', 'xor', 'add', 'sub', 'mul', 'div', 'abs', 'add', 'mul', 'div',
 'sub', 'hypot', 'heaviside'), 
...
def recherche():
    ...
    gp = GP.GP_SymReg(sizepop, numbergen, stpcrit, parammathdict[parammath], warmstart, crossover, subtreemutation, hoistmutation, pointmutation, maxsamples, verboz, parsimonycoefficient, randomstate, njobs, warmreduce, pkldump)
...
def recherche_GC():
    ...
    if ajustother == 'o':
        ...
        low_memory = False
    parammath = int(input('\n' + paramsecho + '\nParametre de calcul a utiliser?: '))
    gp = GC.GP_Classifier(population_size=population_size, generations=generations, tournament_size=tournament_size,
      stopping_criteria=stopping_criteria,
      const_range=const_range,
      init_depth=init_depth,
      init_method=init_method,
      function_set=(parammathdict[parammath]),
...
def verifier(pathofcsv, pathofmodel):
    ...
    yes, total = (0, 0)
    for i in DATA[1:]:
        print(total / len(DATA[1:]) * 1.0 * 100.0, '%')
        if i.count(',') == 0:
            break
    ...
    print('Programme précis à: ' + str(yes / (total * 1.0) * 100.0) + ' % (selon les données)')
