...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/gplearn/GP.py
# Compiled at: 2020-05-12 23:15:24
# Size of source mod 2**32: 11250 bytes
import numpy as np
...
class GP_SymReg(object):
    ...
    def load_csv(self, filepath='test.csv', name_y='y'):
        ...
        for i in DATA[0].keys():
            if 'x' in i and string_to_int(i) > maxmult:
    ...
    def __init__(self, population_size=5000, generations=20, stopping_criteria=0.01, function_set=('add', 'sub', 'mul', 'div'), warm_start=False, p_crossover=0.7, p_subtree_mutation=0.1, p_hoist_mutation=0.05, p_point_mutation=0.1, max_samples=0.9, verbose=1, parsimony_coefficient=0.01, random_state=0, n_jobs=1, warmreduce=False, pkldump=False):
        ...
        if function_set == 'all':
            function_set = ('add', 'sub', 'mul', 'div', 'sqrt', 'log', 'abs', 'neg',
                            'inv', 'max', 'min', 'sin', 'cos', 'tan', 'sigmoid',
                            'ceil', 'fabs', 'floor', 'trunc', 'cbrt', 'hypot', 'modulox',
                            'moyenne2', 'moyenne3', 'moyenne4', 'moyenne5', 'moyenne6',
                            'moyenne7', 'moyenne8', 'moyenne9', 'moyenne10')
        self.est_gp = SymbolicRegressor(population_size=population_size, generations=generations,
    ...
    def learn(self):
        if self.pkldump:
            ...
            if self.warmreduce:
                ...
                print('OK.')
            if self.warm_start:
                with self.pkldump or open(self.filepkl, 'wb') as (f):
 ...
 16, 19], scale=1.0):
    ...
    def approx_stat(self, x):
        a, b = self.predict(X=x, scale=1), self.predict(X=x, scale=0.001)
        return mean([a, b])
    ...
    def make_from_csv_for_corrector(self, filepath='newcorrect.csv'):
        ...
        for x in self.x_.tolist():
            xprime = [
             self.approx_stat(x), self.y_.tolist()[j]]
...
def round_predict(FILEg, FILEh, x=[]):
    ...
    g.load(FILEg)
    h.load(FILEh)
    return round(h.predict([g.predict(x, 1)], 0.001))
