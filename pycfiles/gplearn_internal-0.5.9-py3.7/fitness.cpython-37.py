...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/gplearn/fitness.py
...
def make_fitness(function, greater_is_better, wrap=True):
    ...
    if not isinstance(greater_is_better, bool):
        ...
        assert isinstance(function(np.array([1, 1]), np.array([2, 2]), np.array([1, 1])), numbers.Number), 'function must return a numeric.'
    if wrap:
...
def _weighted_pearson(y, y_pred, w):
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        ...
        corr = np.sum(w * y_pred_demean * y_demean) / np.sum(w) / np.sqrt(np.sum(w * y_pred_demean ** 2) * np.sum(w * y_demean ** 2) / np.sum(w) ** 2)
    if np.isfinite(corr):
        return np.abs(corr)
    return 0.0
