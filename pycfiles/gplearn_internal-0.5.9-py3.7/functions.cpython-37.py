...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/gplearn/functions.py
# Compiled at: 2020-05-12 22:52:31
# Size of source mod 2**32: 11322 bytes
...
def nnnfunc0(x1, x2):
    return (
     np.maximum(np.maximum(x1, x1), x1 / x1), np.maximum(np.maximum(x2, x2), x2 / x2))
...
def make_function(function, name, arity, wrap=True, noarityerror=False):
    ...
    if not noarityerror:
        ...
        if not isinstance(function, np.ufunc):
            if function.__code__.co_argcount != arity:
                raise ValueError('arity %d does not match required number of function arguments of %d.' % (
        ...
        assert np.all(np.isfinite(function(*args))), 'supplied function %s does not have closure against zeros in argument vectors.' % name
    ...
    if wrap:
        return _Function(function=(wrap_non_picklable_objects(function)), name=name,
          arity=arity)
    return _Function(function=function, name=name,
      arity=arity)
...
moyenne = make_function_multiple_arity(function='means', name='moyenne', arity_range=(2,
                                                                                      11))
exec(moyenne)
_function_map = {'add':add2,  'sub':sub2, 
 'mul':mul2, 
 'div':div2, 
 'sqrt':sqrt1, 
 'log':log1, 
