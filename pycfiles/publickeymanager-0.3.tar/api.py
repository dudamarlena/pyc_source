...
# Embedded file name: build/bdist.linux-x86_64/egg/publickey/api.py
# Compiled at: 2015-02-08 01:02:46
...
import requests, publickey.yamldata as yd
...
def challenge_to_connect(client, params, quiet, yamlpath=None):
    try:
        ...
        client.connect(password=password, **params)
    ...
    with closing(paramiko.SSHClient()) as (client):
        ...
        for host in hosts:
            ...
            if filepath:
                ...
                stdout.write("    ==> downloading '%s/%s' members publickeys from github.com.\n" % (
            ...
            with closing(client.open_sftp()) as (sftp):
                stdout.write("    ==> uploading authorized_keys to '%s/%s'.\n" % (
                 host, params['hostname']))
