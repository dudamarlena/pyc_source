...
def circuit_attributes(site_client):
    attr_names = [
     ...
     'vendor']
    attrs = ({'name':a, 
     'resource_name':'Circuit'} for a in attr_names)
...
def device_a(site_client):
    ...
    return site_client.sites(site_client.default_site).devices.post({'hostname': 'foo-bar01'})


@pytest.fixture
def device_z(site_client):
    """ Device for the Z side of the circuit """
    return site_client.sites(site_client.default_site).devices.post({'hostname': 'foo-bar02'})


@pytest.fixture
def interface_a(site_client, device_a, network):
    """ Interface for the A side of the circuit """
    return site_client.sites(site_client.default_site).interfaces.post({'device':device_a['id'], 
     'name':'eth0', 
     'addresses':[
      '10.20.30.2/32']})
...
def interface_z(site_client, device_z, network):
    ...
    return site_client.sites(site_client.default_site).interfaces.post({'device':device_z['id'], 
     'name':'eth0', 
     'addresses':[
      '10.20.30.3/32']})
...
def circuit(site_client, circuit_attributes, interface_a, interface_z):
    ...
    return site_client.sites(site_client.default_site).circuits.post({'name':'test_circuit', 
...
def dangling_circuit(site_client, interface):
    ...
    """
    return site_client.sites(site_client.default_site).circuits.post({'name':'remote_vendor_circuit', 
...
def attributeless_circuit(site_client, interface):
    ...
    return site_client.sites(site_client.default_site).circuits.post({'name':'attributeless_circuit', 
     'endpoint_a':interface['id'], 
     'attributes':{}})