...
# [GCC 8.4.0]
# Embedded file name: /Users/ryanh/src/pynsot/tests/app/test_protocols.py
...
from tests.fixtures import attribute, attributes, client, config, device, interface, network, protocol_type, site, site_client
from tests.fixtures.circuits import circuit, circuit_attributes, interface_a, interface_z, device_a, device_z
from tests.fixtures.protocols import protocol, protocols, protocol_attribute, protocol_attribute2
...
def test_protocols_add(site_client, device_a, interface_a, site, protocol_type):
    ...
    with runner.isolated_filesystem():
        result = runner.run("protocols add -t bgp -D %s -I %s -e 'my new proto'" % (device_id, interface_id))
        ...
        result = runner.run('protocols add -t bgp -D %s -I %s -a %s' % (device_id, interface_id, attributes))
        assert result.exit_code == 0
        assert 'Added protocol!' in result.output
        result = runner.run('protocols list')
        assert result.exit_code == 0
...
def test_protocols_update(site_client, interface_a, device_a, site, circuit, protocol, protocol_attribute):
    ...
    with runner.isolated_filesystem():
        ...
        result = runner.run('protocols update -i %s --add-attributes -a boo=test_attribute' % proto_id)
        assert result.exit_code == 0
        assert 'Updated protocol!' in result.output
        result = runner.run('protocols list')
        assert result.exit_code == 0
        assert 'test_attribute' in result.output
        result = runner.run('protocols update -i %s -a boo=test_attribute' % proto_id)
        assert result.exit_code == 0
        assert 'Updated protocol!' in result.output
        result = runner.run('protocols list')
        assert result.exit_code == 0
        assert 'test_attribute' in result.output
        result = runner.run('protocols update -i %s --delete-attributes -a foo=test_protocol' % proto_id)
        assert result.exit_code == 0
        assert 'Updated protocol!' in result.output
        result = runner.run('protocols list')
        assert result.exit_code == 0
        assert 'test_protocol' not in result.output
        result = runner.run('protocols update -i %s --replace-attributes -a foo=test_replace' % proto_id)
        assert result.exit_code == 0
        assert 'Updated protocol!' in result.output
        result = runner.run('protocols list')
        assert result.exit_code == 0
