...
# [GCC 8.4.0]
# Embedded file name: /data/envs/dfuzz/project/dfuzz/dfuzz/core/loader.py
# Compiled at: 2011-05-17 16:29:36
    ...
    def check_methods(self, cls, str_mod):
        ...
        for method in required_methods:
            if not hasattr(cls, method):
                logging.warning('"FuzzWrapper" class in module "%s" has no method "%s", skipping' % (
                 str_mod, method))
                return False
    ...
    def add_valid_modules(self, str_mods, prior):
        ...
        for mod in mod_strings:
            ...
            if '(' in mod:
                match = re.search('(.*)\\((.*)\\).*', mod)
                if match == None:
                    logging.warning('Bad format for module parameters: %s. Skipping', mod)
                    continue
