...
# [GCC 8.4.0]
# Embedded file name: /Library/Frameworks/Python.framework/Versions/2.7/lib/python2.7/site-packages/static/server.py
# Compiled at: 2014-04-20 17:43:52
import os, re, cgi, json, urllib, logging, importlib, webapp2, jinja2
    ...
    def route(self, controllers):
        ...
        for c in controllers:
            ...
            if c.allow_extensions:
                e = '(?:\\.(.+))?'
            self.add_route(current + e, c)
            if c._supports_model:
                self.add_route(current + '/new', c)
                self.add_route(current + '/([0-9]+)' + e, c)
                self.add_route(current + '/([0-9]+)/edit', c)
...
class ModelController(BaseController):
    ...
    def render_appropriate(self, mode, **params):
        ...
        if mode == 'index':
            ...
            self.response.render((self._name + '/' + mode + '.html'), **params)
