...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/constant2-project/constant2/pkg/inspect_mate/getter.py
# Compiled at: 2018-12-19 11:17:01
"""
    ...
    for attr, value in inspect.getmembers(klass):
        try:
            if tester_func(klass, attr):
                ...
                elif not (attr.startswith('__') or attr.endswith('__')):
                    pairs.append((attr, value))
        except:
            pass
...
get_attributes = functools.partial(_get_members, tester_func=is_attribute, return_builtin=False)
get_attributes.__doc__ = 'Get all class attributes members.'
get_property_methods = functools.partial(_get_members, tester_func=is_property_method, return_builtin=False)
get_property_methods.__doc__ = 'Get all property style attributes members.'
get_regular_methods = functools.partial(_get_members, tester_func=is_regular_method, return_builtin=False)
get_regular_methods.__doc__ = 'Get all non static and class method members'
get_static_methods = functools.partial(_get_members, tester_func=is_static_method, return_builtin=False)
get_static_methods.__doc__ = 'Get all static method attributes members.'
get_class_methods = functools.partial(_get_members, tester_func=is_class_method, return_builtin=False)
...
def get_all_attributes(klass_or_instance):
    ...
    """
    pairs = list()
    for attr, value in inspect.getmembers(klass_or_instance, lambda x: not inspect.isroutine(x)):
...
def get_all_methods(klass_or_instance):
    ...
    """
    pairs = list()
    for attr, value in inspect.getmembers(klass_or_instance, lambda x: inspect.isroutine(x)):
        if not (attr.startswith('__') or attr.endswith('__')):
            pairs.append((attr, value))
