...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/phonolammps/iofile.py
# Compiled at: 2019-09-03 07:24:07
import os, numpy as np
from phonopy.structure.atoms import PhonopyAtoms
from phonolammps.phonopy_link import PhonopyAtomsTinker

def mass_to_symbol(mass, tolerance=0.5):
...
def get_structure_from_poscar(file_name, number_of_dimensions=3):
    ...
    direct_cell = np.array([ data_lines[i].split() for i in range(2, 2 + number_of_dimensions)
                           ], dtype=float)
    ...
    try:
        ...
        if coordinates_type == 'D' or coordinates_type == 'd':
            scaled_positions = np.array([ data_lines[(8 + k)].split()[0:3] for k in range(np.sum(number_of_types))
                                        ], dtype=float)
        else:
            positions = np.array([ data_lines[(8 + k)].split()[0:3] for k in range(np.sum(number_of_types))
                                 ], dtype=float)
        atomic_types = []
        ...
        if coordinates_type == 'D' or coordinates_type == 'd':
            scaled_positions = np.array([ data_lines[(7 + k)].split()[0:3] for k in range(np.sum(number_of_types))
                                        ], dtype=float)
        else:
            positions = np.array([ data_lines[(7 + k)].split()[0:3] for k in range(np.sum(number_of_types))
                                 ], dtype=float)
        atomic_types = []
    ...
    return PhonopyAtoms(symbols=atomic_types, scaled_positions=scaled_positions, cell=direct_cell)
...
def get_structure_from_lammps(command_list, show_log=False):
    ...
    return PhonopyAtoms(positions=positions, masses=masses, symbols=symbols, cell=unitcell)
...
def get_structure_from_txyz(file_name, key_file):
    ...
    unitcell = [
     ...
     [
      c1, c2, c3]]
    return PhonopyAtomsTinker(positions=np.array(coordinates, dtype=float), symbols=atomic_elements, cell=unitcell, connectivity=connectivity, atom_types=atom_types)
...
def generate_tinker_key_file(structure, archive='overwrite', wdv_cutoff=20):
    cell = structure.get_cell()
