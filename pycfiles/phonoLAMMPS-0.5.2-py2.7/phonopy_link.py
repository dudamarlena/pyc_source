...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/phonolammps/phonopy_link.py
...
def get_phonon(structure, NAC=False, setup_forces=True, super_cell_phonon=np.identity(3), primitive_matrix=np.identity(3), symmetrize=True):
    """
    ...
    """
    phonon = Phonopy(structure, super_cell_phonon, primitive_matrix=primitive_matrix, symprec=1e-05, is_symmetry=symmetrize)
    if setup_forces:
        if structure.get_force_constants() is not None:
            ...
            structure.set_force_constants(ForceConstants(phonon.get_force_constants(), supercell=structure.get_force_sets().get_supercell()))
        else:
...
def obtain_phonon_dispersion_bands(structure, bands_ranges, force_constants, supercell, NAC=False, band_resolution=30, band_connection=False, primitive_matrix=np.identity(3)):
    """
    ...
    :return:
    """
    phonon = get_phonon(structure, NAC=NAC, setup_forces=False, super_cell_phonon=supercell, primitive_matrix=primitive_matrix)
...
def standarize_structure(structure):
    ...
    lattice, positions, numbers = standardize_cell(structure, to_primitive=False, no_idealize=False, symprec=1e-05)
