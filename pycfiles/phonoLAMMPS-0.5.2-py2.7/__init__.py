...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/phonolammps/__init__.py
# Compiled at: 2019-09-03 07:24:07
__version__ = '0.5.2'
import numpy as np
...
unit_factors = {'real': 0.04336410389526464, 'metal': 1.0, 
   'si': 624150636.3094, 
   'tinker': 0.043}
    ...
    * get_forces()
    ...
    def get_path_using_seek_path(self):
        ...
        try:
            ...
            return {'ranges': band_ranges, 'labels': [
                        [
                         'GAMMA', '1/2 0 1/2']]}
    ...
    def get_force_constants(self, include_data_set=False):
        ...
        if self._force_constants is None:
            phonon = get_phonon(self._structure, setup_forces=False, super_cell_phonon=self._supercell_matrix, primitive_matrix=self._primitive_matrix, NAC=self._NAC, symmetrize=self._symmetrize)
    ...
    def plot_phonon_dispersion_bands(self):
        ...
        _bands = obtain_phonon_dispersion_bands(self._structure, bands_and_labels['ranges'], force_constants, self._supercell_matrix, primitive_matrix=self._primitive_matrix, band_resolution=30)
        ...
        if 'labels' in bands_and_labels:
            ...
            for i, freq in enumerate(_bands[1]):
                if labels[i][0] == labels[(i - 1)][1]:
                    ...
                    labels_e.append(replace_list(labels[(i - 1)][1]) + '/' + replace_list(labels[i][0]))
...
class Phonolammps(PhonoBase):
    ...
    def __init__(self, lammps_input, supercell_matrix=np.identity(3), primitive_matrix=np.identity(3), displacement_distance=0.01, show_log=False, show_progress=False, use_NAC=False, symmetrize=True):
        """
    ...
    def get_forces(self, cell_with_disp):
        ...
        for i in range(na):
            lmp.command(('set atom {} x {} y {} z {}').format(i + 1, coordinates[(template[i], 0)], coordinates[(template[i], 1)], coordinates[(template[i], 2)]))
...
class PhonoTinker(PhonoBase):
    ...
    def __init__(self, txyz_input_file, key_input_file, force_field_file, supercell_matrix=np.identity(3), primitive_matrix=np.identity(3), displacement_distance=0.01, show_log=False, show_progress=False, use_NAC=False, symmetrize=True):
        """
    ...
    def get_forces(self, cell_with_disp):
        ...
        supercell_wd = rebuild_connectivity_tinker(self._structure, cell_with_disp, self._supercell_matrix)
        ...
        tinker_command = './testgrad ' + tinker_input_file.name + ' ' + self.force_field + ' Y N N ' + ' -k ' + tinker_key_file.name
...
if __name__ == '__main__':
    ...
    phonon = get_phonon(structure, setup_forces=False, super_cell_phonon=[
     [
      2, 0, 0], [0, 2, 0], [0, 0, 2]], NAC=False, symmetrize=True)
    phonon.get_displacement_dataset()
    phonon.generate_displacements(distance=0.0001)
    cells_with_disp = phonon.get_supercells_with_displacements()
    print cells_with_disp[0]
    print generate_VASP_structure(cells_with_disp[0])
    supercell_wd = rebuild_connectivity_tinker(structure, cells_with_disp[0], phonon.get_supercell_matrix())
    ...
    tinker_key_file.close()
    print (
     'filename', tinker_input_file)
    tinker_command = './testgrad ' + tinker_input_file.name + ' ' + force_field + ' Y N N ' + ' -k ' + tinker_key_file.name
    tinker_process = subprocess.Popen(tinker_command, stdin=PIPE, stderr=PIPE, stdout=PIPE, shell=True)
    output, err = tinker_process.communicate()
    tinker_process.wait()
    os.unlink(tinker_input_file.name)
