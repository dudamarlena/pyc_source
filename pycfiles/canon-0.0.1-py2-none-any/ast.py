...
# Embedded file name: build/bdist.linux-x86_64/egg/canoe/config/ast.py
...
class DeclNode(Node):
    ...
    def evaluate(self, env):
        ...
        if self.decl_type == 'filter':
            ...
            raise ValueError("Don't know what to do with '%s'" % self.decl_type)
        return klass(**kwargs)
...
class DerefNode(Node):
    ...
    def evaluate(self, env):
        ...
        raise NameError("can't dereference '%s'. Not defined." % self.identifier)
