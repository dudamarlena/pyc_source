...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/bridge_tk.py
# Compiled at: 2016-11-01 16:16:32
# Size of source mod 2**32: 13019 bytes
import multiprocessing, tkinter
...
class Gui:
    ...
    def __init__(self, conn):
        ...
        w.pack()
        self.table = tkinter.Canvas(w, bd=2, relief='raised', bg='blue', height=60, width=100)
        ...
        scoreboard = self.scoreboard = tkinter.Frame(w, height=140, width=100, bg='red')
    ...
    def show_deal(self, hand, me, dealer):
        ...
        for i in range(1, 8):
            for s in suit_all():
                ss = suit_chr(s)
                bb = tkinter.Button(self.bidwin, text=str(i) + ss[0], fg=ss[1])
        ...
        for b in self.bbutts:
            b.config(command=lambda x=b.bid: self.make_bid(x), state='disabled')
    ...
    def _show_cards(self, hand, where, cbutts, mine):
        cards = {s:set(c for c in hand if c.suit is s) for s in suit_real()}
        suits = dict()
        if mine:
            self.suits = suits
        for i in self.suit_order:
            ...
            ss = suit_chr(i)
            l = tkinter.Label(suits[i], bg='white', fg=ss[1], text=ss[0])
            l.pack(side='left', anchor='w')
            ctemp = sorted(cards[i], reverse=True)
            for j in ctemp:
                rs = rank_chr(j.rank)
                cbutts[i].append(tkinter.Button(suits[i], text=rs, fg=ss[1], width=1, padx=2))
                cbutts[i][(-1)].card = j
    ...
    def show_dum(self, dum_hand, dummy):
        ...
        self.dwin = self.w.create_window(x, y, anchor='nw', window=dhand)
        self.top.after(100, self.check_pipe)
    ...
    def _update_order(self, trump_suit):
        if trump_suit in {Suit.Spades, Suit.NT}:
            ...
            if trump_suit is Suit.Hearts:
                self.suit_order = [
                 Suit.Hearts, Suit.Spades, Suit.Diamonds,
                 Suit.Clubs]
            else:
                if trump_suit is Suit.Diamonds:
                    self.suit_order = [
                     Suit.Diamonds, Suit.Spades, Suit.Hearts,
                     Suit.Clubs]
                else:
                    self.suit_order = [
                     Suit.Clubs, Suit.Hearts, Suit.Spades,
                     Suit.Diamonds]
    ...
    def _show(self, pos, card=None):
        ...
        if card is not None:
            ...
            l.pos = pos
            if pos == 0:
                l.i = self.table.create_window(50, 60, anchor='s', window=l)
        else:
            if pos == 1:
                l.i = self.table.create_window(0, 30, anchor='w', window=l)
            else:
                if pos == 2:
                    l.i = self.table.create_window(50, 0, anchor='n', window=l)
                else:
                    l.i = self.table.create_window(100, 30, anchor='e', window=l)
    ...
    def ask(self, trick, who):
        if who == self.me:
            ...
            cbutts = self.dbutts
        for s in suit_real():
            if trick.lead_suit is None or s is trick.lead_suit or len(cbutts[trick.lead_suit]) == 0:
                for c in cbutts[s]:
                    c.config(command=lambda x=c, y=cbutts: self.choose_card(x, y))
