...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/cluebin/sqldata.py
# Compiled at: 2008-06-27 12:04:19
import datetime, sqlalchemy as sa
from sqlalchemy import orm
from cluebin import paste
metadata = sa.MetaData()
paste_table = sa.Table('pastes', metadata, sa.Column('pasteid', sa.Integer, primary_key=True), sa.Column('author_name', sa.Unicode(40)), sa.Column('language', sa.Unicode(20)), sa.Column('paste', sa.UnicodeText()), sa.Column('date', sa.DateTime()))
...
class SqlPasteDataStore(paste.PasteDataStore):
    DEFAULT_ORDERING = [
     SqlPaste.date.desc(), SqlPaste.pasteid.desc()]
    ...
    def _session_factory(self):
        ...
        if session_factory is None:
            session_factory = orm.sessionmaker(bind=self._engine, autoflush=True, transactional=True)
