...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/lems/parser/LEMS.py
# Compiled at: 2017-12-08 05:05:49
"""
...
def get_nons_tag_from_node(node):
    ...
    if len(bits) == 1:
        ...
        if '/lems/' in bits[0]:
            return bits[1]
        if 'neuroml2' in bits[0]:
            return bits[1]
        if 'rdf' in bits[0]:
            return 'rdf_' + bits[1]
        if 'model-qualifiers' in bits[0]:
            return 'bqmodel_' + bits[1]
        if 'biology-qualifiers' in bits[0]:
...
class LEMSFileParser(LEMSBase):
    ...
    def init_parser(self):
        ...
        self.valid_children['component'] = [
         'component']
        self.valid_children['conditionalderivedvariable'] = [
         'case']
        self.valid_children['regime'] = [
         'oncondition', 'onentry', 'timederivative']
        ...
        self.valid_children['structure'] = ['childinstance',
         ...
         'with',
         'tunnel']
        self.valid_children['foreach'] = [
         'foreach', 'eventconnection']
        self.valid_children['simulation'] = [
         'record', 'eventrecord', 'run',
    ...
    def parse_component_reference(self, node):
        ...
        if 'name' in node.lattrib:
            ...
            self.raise_error('<ComponentReference> must specify a name for the ' + 'reference.')
        if 'type' in node.lattrib:
            type_ = node.lattrib['type']
        else:
            self.raise_error('<ComponentReference> must specify a type for the ' + 'reference.')
    ...
    def parse_derived_parameter(self, node):
        ...
        if 'select' in node.lattrib:
            ...
            select = None
        self.current_component_type.add_derived_parameter(DerivedParameter(name, value, dimension, select))
        return
    ...
    def parse_for_each(self, node):
        ...
        if 'instances' in node.lattrib:
            ...
            self.raise_error('<ForEach> must specify a reference to targetinstances')
    ...
    def parse_kinetic_scheme(self, node):
        ...
        self.current_regime.add_kinetic_scheme(KineticScheme(name, nodes, state_variable, edges, edge_source, edge_target, forward_rate, reverse_rate))
    ...
    def parse_multi_instantiate(self, node):
        ...
        if 'number' in node.lattrib:
            ...
            self.raise_error("Multi instantiation of '{0}' must specify a parameter specifying the number.", component)
    ...
    def parse_index_parameter(self, node):
        ...
        """
        if self.current_component_type == None:
            self.raise_error('IndexParameters can only be defined in ' + 'a component type')
        try:
            name = node.lattrib['name']
        except:
    ...
    def parse_record(self, node):
        ...
        if self.current_simulation == None:
            self.raise_error('<Record> must be only be used inside a ' + 'simulation specification')
    ...
    def parse_state_assignment(self, node):
        ...
        if 'value' in node.lattrib:
            ...
            self.raise_error("State assignment for '{0}' must specify a value.", variable)
    ...
    def parse_time_derivative(self, node):
        ...
        if 'value' in node.lattrib:
            ...
            self.raise_error("Time derivative for '{0}' must specify an expression.", variable)
