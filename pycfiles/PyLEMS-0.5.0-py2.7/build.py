...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/lems/sim/build.py
# Compiled at: 2020-01-30 05:13:04
"""
Simulation builder.
...
class SimulationBuilder(LEMSBase):
    ...
    def build(self):
        ...
        for component_id in self.model.targets:
            if component_id not in self.model.components:
                raise SimBuildError("Unable to find target component '{0}'", component_id)
    ...
    def build_runnable(self, component, parent=None, id_=None):
        ...
        for dpn in derived_parameter_ordering:
            ...
            expression = self.build_expression_from_tree(runnable, None, derived_parameter.expression_tree)
            derived_parameter_code += [
             ('self.{0} = ({1})').format(derived_parameter.name, expression)]
            derived_parameter_code += [
             ('self.{0}_shadow = ({1})').format(derived_parameter.name, expression)]

        suffix = ''
        runnable.add_method('update_derived_parameters' + suffix, ['self'], derived_parameter_code)
        ...
        for child in component.child_components:
            ...
            for children in component.children:
                ...
                elif child_runnable.id == children.name:
        ...
        dynamics = component.dynamics
        self.add_dynamics_2(component, runnable, dynamics, dynamics)
        for regime in dynamics.regimes:
    ...
    def build_structure(self, component, runnable, structure):
        ...
        for mi in structure.multi_instantiates:
            template = self.build_runnable(mi.component, runnable)
            for i in range(mi.number):
                instance = template.copy()
                instance.id = ('{0}__{1}__{2}').format(component.id, template.id, i)
    ...
    def build_foreach(self, component, runnable, foreach, name_mappings={}):
        ...
        for target_runnable in target_array:
            if self.debug:
                print 'Applying contents of for_each to %s, as %s' % (target_runnable.id, foreach.as_)
    ...
    def add_dynamics_1(self, component, runnable, regime, dynamics):
        ...
        for td in regime.time_derivatives:
            if td.variable not in regime.state_variables and td.variable not in dynamics.state_variables:
                raise SimBuildError(('Time derivative for undefined state variable {0} in component {1}').format(td.variable, component.id))
            exp = self.build_expression_from_tree(runnable, regime, td.expression_tree)
            time_step_code += [
             ('self.{0} += dt * ({1})').format(td.variable, exp)]

        runnable.add_method('update_state_variables' + suffix, ['self', 'dt'], time_step_code)
        derived_variable_code = []
        derived_variables_ordering = order_derived_variables(regime)
        for dvn in derived_variables_ordering:
            if dvn in dynamics.derived_variables:
                dv = dynamics.derived_variables[dvn]
                runnable.add_derived_variable(dv.name)
                if dv.value:
                    derived_variable_code += [
                     ('self.{0} = ({1})').format(dv.name, self.build_expression_from_tree(runnable, regime, dv.expression_tree))]
                elif dv.select:
                    if dv.reduce:
                        derived_variable_code += self.build_reduce_code(dv.name, dv.select, dv.reduce)
                    else:
                        derived_variable_code += [
                         ('self.{0} = (self.{1})').format(dv.name, dv.select.replace('/', '.'))]
                else:
                    raise SimBuildError(('Inconsistent derived variable settingsfor {0}').format(dvn))
            elif dvn in dynamics.conditional_derived_variables:
                dv = dynamics.conditional_derived_variables[dvn]
                runnable.add_derived_variable(dv.name)
                derived_variable_code += self.build_conditional_derived_var_code(runnable, regime, dv)
            else:
                raise SimBuildError("Unknown derived variable '{0}' in '{1}'", dvn, runnable.id)

        runnable.add_method('update_derived_variables' + suffix, ['self'], derived_variable_code)
        ...
        for eh in regime.event_handlers:
            if isinstance(eh, OnStart):
                startup_event_handler_code += self.build_event_handler(runnable, regime, eh)
            elif isinstance(eh, OnCondition):
                post_event_handler_code += self.build_event_handler(runnable, regime, eh)
            else:
                if isinstance(eh, OnEntry):
                    on_entry_added = True
                pre_event_handler_code += self.build_event_handler(runnable, regime, eh)
        ...
        runnable.add_method('run_startup_event_handlers' + suffix, ['self'], startup_event_handler_code)
        runnable.add_method('run_preprocessing_event_handlers' + suffix, ['self'], pre_event_handler_code)
        runnable.add_method('run_postprocessing_event_handlers' + suffix, ['self'], post_event_handler_code)
        ...
        for ks in regime.kinetic_schemes:
            ...
            try:
                ...
                for edge in edges:
                    ...
                    ks_code += [
                     ('self.{0}.{2} += dt * (-self.{3}.{4} * self.{0}.{2}_shadow + self.{3}.{5} * self.{1}.{2}_shadow)').format(from_, to, ks.state_variable, edge.id, ks.forward_rate, ks.reverse_rate)]
                    ks_code += [
                     ('self.{1}.{2} += dt * (self.{3}.{4} * self.{0}.{2}_shadow - self.{3}.{5} * self.{1}.{2}_shadow)').format(from_, to, ks.state_variable, edge.id, ks.forward_rate, ks.reverse_rate)]
                ...
                for node in nodes:
                    ks_code += [
                     ('self.{0}.{1}_shadow = self.{0}.{1}').format(node, ks.state_variable)]
    ...
    def process_simulation_specs(self, component, runnable, simulation):
        ...
        for run in simulation.runs:
            ...
            target.configure_time(run.increment, run.total)
        ...
        if tree_node.type == ExprNode.VALUE:
            if tree_node.value[0].isalpha():
                if tree_node.value == 't':
                    ...
                    if tree_node.value in component_type.requirements:
                        var_prefix = 'self'
                        v = tree_node.value
                        r = runnable
                    ...
                    if tree_node.value in dynamics.derived_variables or regime is not None and tree_node.value in regime.derived_variables:
            ...
            if tree_node.type == ExprNode.FUNC1:
                pattern = '({0}({1}))'
    ...
    def build_on_condition(self, runnable, regime, on_condition):
        ...
        on_condition_code = []
        on_condition_code += [
         ('if {0}:').format(self.build_expression_from_tree(runnable, regime, on_condition.expression_tree))]
    ...
    def build_on_event(self, runnable, regime, on_event):
        ...
        on_event_code = []
        if self.debug:
            on_event_code += ['print("Maybe handling something for %s ("+str(id(self))+")")' % runnable.id,
        ...
        on_event_code += [
         ("self.event_in_counters['{0}'] = 0").format(on_event.port)]
    ...
    def build_state_assignment(self, runnable, regime, state_assignment):
        ...
        """
        return [
         ('self.{0} = {1}').format(state_assignment.variable, self.build_expression_from_tree(runnable, regime, state_assignment.expression_tree))]
    ...
    def build_event_out(self, event_out):
        ...
        """
        event_out_code = [
         ('if "{0}" in self.event_out_callbacks:').format(event_out.port),
    ...
    def build_transition(self, transition):
        ...
        """
        return [
         ("self.new_regime = '{0}'").format(transition.regime)]
    ...
    def build_reduce_code(self, result, select, reduce):
        ...
        code = [
         ('self.{0} = {1}').format(result, acc_start)]
    ...
    def build_conditional_derived_var_code(self, runnable, regime, dv):
        ...
        for case in dv.cases:
            if case.condition_expression_tree:
                code += [
                 el + ('if {0}:').format(self.build_expression_from_tree(runnable, regime, case.condition_expression_tree))]
                el = 'el'
                code += [
                 ('    self.{0} = {1}').format(dv.name, self.build_expression_from_tree(runnable, regime, case.value_expression_tree))]
...
def order_derived_parameters(component):
    ...
    if count == 0:
        raise SimBuildError(("Unable to find ordering for derived parameter in component '{0}'").format(component))
    return ordering
...
def order_derived_variables(regime):
    ...
    while count > 0 and dvs != []:
        ...
        for dv1 in dvs:
            ...
            if isinstance(dv, DerivedVariable):
                ...
                for case in dv.cases:
                    for dv2 in dvs:
                        if dv1 != dv2 and (is_var_in_exp_tree(dv2, case.condition_expression_tree) or is_var_in_exp_tree(dv2, case.value_expression_tree)):
                            found = True
    ...
    if count == 0:
        raise SimBuildError(("Unable to find ordering for derived variables in regime '{0}'").format(regime.name))
