...
# Embedded file name: /Users/sanhehu/Documents/GitHub/apipool-project/apipool/stats.py
# Compiled at: 2018-08-21 16:57:28
...
class StatusCollection(object):
    ...
    def get_subclasses(cls):
        return [
         cls.c1_Success, cls.c5_Failed, cls.c9_ReachLimit]
    ...
    def get_mapper_id_to_description(cls):
        return {klass.id:klass.description for klass in cls.get_subclasses()}

    @classmethod
    def get_mapper_description_to_id(cls):
        return {klass.description:klass.id for klass in cls.get_subclasses()}

    @classmethod
    def get_status_list(cls):
        return [Status(id=klass.id, description=klass.description) for klass in cls.get_subclasses()]
...
class StatsCollector(object):
    ...
    def _add_all_status(self):
        Status.smart_insert(self.engine, StatusCollection.get_status_list())
    ...
    def add_event(self, primary_key, status_id):
        event = Event(apikey_id=self._cache_apikey[primary_key], finished_at=datetime.now(), status_id=status_id)
        Event.smart_insert(self.engine, event)

    def query_event_in_recent_n_seconds(self, n_seconds, primary_key=None, status_id=None):
    ...
    def usage_count_in_recent_n_seconds(self, n_seconds, primary_key=None, status_id=None):
        q = self.query_event_in_recent_n_seconds(n_seconds, primary_key=primary_key, status_id=status_id)
        return q.count()

    def usage_count_stats_in_recent_n_seconds(self, n_seconds):
        n_seconds_before = get_n_seconds_before(n_seconds)
        q = self.ses.query(ApiKey.key, func.count(Event.apikey_id)).select_from(Event).join(ApiKey).filter(Event.finished_at >= n_seconds_before).group_by(Event.apikey_id).order_by(ApiKey.key)
