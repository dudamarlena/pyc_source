...
# [GCC 8.4.0]
# Embedded file name: /Users/leland/PycharmProjects/pynndescent/pynndescent/tests/test_pynndescent_.py
# Compiled at: 2019-12-12 19:26:45
# Size of source mod 2**32: 10926 bytes
import os, io, re
...
def test_nn_descent_neighbor_accuracy():
    knn_indices, _ = NNDescent(nn_data,
      'euclidean', {}, 10, random_state=(np.random))._neighbor_graph
    tree = KDTree(nn_data)
    ...
    assert_greater_equal(percent_correct, 0.98, 'NN-descent did not get 99% accuracy on nearest neighbors')


def test_angular_nn_descent_neighbor_accuracy():
    knn_indices, _ = NNDescent(nn_data,
      'cosine', {}, 10, random_state=(np.random))._neighbor_graph
...
def test_sparse_nn_descent_neighbor_accuracy():
    knn_indices, _ = NNDescent(sparse_nn_data,
      'euclidean', n_neighbors=20, random_state=None)._neighbor_graph
    ...
    assert_greater_equal(percent_correct, 0.85, 'Sparse NN-descent did not get 95% accuracy on nearest neighbors')


def test_sparse_angular_nn_descent_neighbor_accuracy():
    knn_indices, _ = NNDescent(sparse_nn_data,
      'cosine', {}, 20, random_state=None)._neighbor_graph
    ...
    percent_correct = num_correct / (sparse_nn_data.shape[0] * 10)
    assert_greater_equal(percent_correct, 0.85, 'Sparse angular NN-descent did not get 98% accuracy on nearest neighbors')
...
def test_nn_descent_query_accuracy():
    ...
    assert_greater_equal(percent_correct, 0.95, 'NN-descent query did not get 95% accuracy on nearest neighbors')


def test_sparse_nn_descent_query_accuracy():
    nnd = NNDescent((sparse_nn_data[200:]),
      'euclidean', n_neighbors=10, random_state=None)
    ...
    percent_correct = num_correct / (true_indices.shape[0] * 10)
    assert_greater_equal(percent_correct, 0.95, 'Sparse NN-descent query did not get 95% accuracy on nearest neighbors')
...
def test_transformer_equivalence():
    ...
    indices_sorted = np.vstack([indices[(i, sort_idx[i])] for i in range(sort_idx.shape[0])])
...
def test_random_state_none():
    knn_indices, _ = NNDescent(nn_data,
      'euclidean', {}, 10, random_state=None)._neighbor_graph
    tree = KDTree(nn_data)
    true_indices = tree.query(nn_data, 10, return_distance=False)
    num_correct = 0.0
    for i in range(nn_data.shape[0]):
...
def test_rp_trees_should_not_stack_overflow_with_duplicate_data():
    ...
    n_neighbors = 10
    knn_indices, _ = NNDescent(data,
      'cosine', {}, n_neighbors, random_state=(np.random), n_trees=20)._neighbor_graph
    for i in range(data.shape[0]):
        assert_equal(len(knn_indices[i]), len(np.unique(knn_indices[i])), 'Duplicate graph_indices in knn graph')
...
def test_deduplicated_data_behaves_normally():
    ...
    assert_greater_equal(proportion_correct, 0.95, 'NN-descent did not get 95% accuracy on nearest neighbors')


def test_output_when_verbose_is_true():
    out = io.StringIO()
    with redirect_stdout(out):
        _ = NNDescent(data=spatial_data,
          metric='euclidean',
          metric_kwds={},
          n_neighbors=4,
...
def test_transformer_output_when_verbose_is_true():
    out = io.StringIO()
    with redirect_stdout(out):
        _ = PyNNDescentTransformer(n_neighbors=4,
          metric='euclidean',
          metric_kwds={},
          random_state=(np.random),
          n_trees=5,
          n_iters=2,
          verbose=True).fit_transform(spatial_data)
    output = out.getvalue()
    assert_true(re.match('^.*5 trees', output, re.DOTALL))
    assert_true(re.match('^.*2 iterations', output, re.DOTALL))
...
def test_transformer_output_when_verbose_is_false():
    ...
    with redirect_stdout(out):
        _ = PyNNDescentTransformer(n_neighbors=4,
          ...
          n_trees=5,
          n_iters=2,
          verbose=False).fit_transform(spatial_data)
    output = out.getvalue().strip()
    assert_equal(len(output), 0)