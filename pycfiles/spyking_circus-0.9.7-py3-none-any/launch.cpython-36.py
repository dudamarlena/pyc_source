...
# Compiled at: 2019-11-21 11:07:38
# Size of source mod 2**32: 19832 bytes
...
def main(argv=None):
    if argv is None:
        ...
        nb_cpu, hostfile, batch, preview, result, extension, output, benchmark, info, second = (
         args.cpu, args.hostfile, args.batch,
            ...
            for subtask, command in subtasks:
                if subtask in steps:
                    if command == 'python':
                        ...
                        if subtask in ('filtering', 'benchmarking'):
                            if not is_writable:
                                if not preview:
                                    if overwrite:
