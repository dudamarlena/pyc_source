...
# [GCC 8.4.0]
# Embedded file name: /home/pierre/github/spyking-circus/build/lib/circus/converting.py
# Compiled at: 2020-04-08 03:56:02
# Size of source mod 2**32: 19728 bytes
from circus.shared.utils import *
import circus.shared.files as io
import os, shutil, numpy as np, warnings
...
def get_rpv(spikes, sampling_rate, duration=0.005):
...
def main(params, nb_cpu, nb_gpu, use_gpu, extension):
    ...
    if export_all:
        if not params.getboolean('fitting', 'collect_all'):
            if comm.rank == 0:
            ...
            for key in list(result['spiketimes'].keys()):
                ...
                if prelabelling:
                    if has_purity:
                        ...
                        if rpv <= rpv_threshold:
                            if numpy.abs(median_amp - 1) < 0.25:
                                labels += [[temp_id, 'good']]
                        if median_amp < 0.5:
                            labels += [[temp_id, 'mua']]
        ...
        def write_pcs(path, params, extension, N_tm, mode=0):
            ...
            if sign_peaks in ('negative', 'both'):
                ...
                comm.Barrier()
                if comm.rank == 0:
                    numpy.save(os.path.join(output_path, 'pc_feature_ind'), pc_features_ind.astype(numpy.uint32))
