...
# Size of source mod 2**32: 22404 bytes
...
class DataFile(object):
    ...
    def get_data(self, idx, chunk_size, padding=(0, 0), nodes=None):
        if self.is_stream:
            ...
            idx -= self._chunks_in_sources[cidx]
            return (
             self._sources[cidx].read_chunk(idx, chunk_size, padding, nodes), self._sources[cidx].t_start + idx * chunk_size)
        else:
            return (
             self.read_chunk(idx, chunk_size, padding, nodes), self.t_start + idx * chunk_size)
