...
# Size of source mod 2**32: 82469 bytes
    ...
    if do_temporal_whitening:
        ...
        if fit_on:
            ...
            if templates is None:
                ...
                if numpy.iterable(n_elec) or square:
...
def view_local_merges(waveforms_data, clusters_data, old_allocation, new_allocation, merge_history, save=False, max_nb_traces=200):
    import matplotlib.colors as clr, matplotlib.pyplot as plt
    marker_size = 10
    ...
    if nb_local_merges > 0:
        ...
        colors = {cluster_nb:clr.to_rgb('C{}'.format(k % 10)) for k, cluster_nb in enumerate(cluster_nbs)}
...
def view_triggers_bis(file_name, mode='random', save=True):
    ...
    """
    import matplotlib.pyplot as plt, matplotlib.gridspec as gridspec, scipy as sp
...
def view_loss_curve(losss, title=None, save=None):
    ...
    fig = pylab.figure()
    ax = fig.gca()
    ax.semilogy((list(range(x_min, x_max + 1))), (losss[1:]), color='blue', linestyle='solid')
