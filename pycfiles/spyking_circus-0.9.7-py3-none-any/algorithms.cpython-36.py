...
# Compiled at: 2019-12-18 06:43:54
# Size of source mod 2**32: 33590 bytes
import os, logging, sys, scipy.optimize, numpy, pylab, scipy.spatial.distance, scipy.stats, shutil, h5py, scipy.linalg, scipy.sparse
...
import scipy.linalg, scipy.sparse, statsmodels.api as sm
...
class DistanceMatrix(object):
    ...
    def get_deltas(self, rho):
        rho_sort_id = numpy.argsort(rho)
        rho_sort_id = rho_sort_id[::-1]
        sort_rho = rho[rho_sort_id]
        ...
        delta[rho == numpy.max(rho)] = numpy.max(delta[numpy.logical_not(numpy.isinf(delta))])
...
def compute_rho(data, update=None, mratio=0.01):
    ...
    if update is None:
        ...
        return (
         numpy.nan_to_num(rho, copy=False), dist_sorted)
...
def find_centroids_and_cluster(dist, rho, delta, alpha=3):
    ...
    nclus = len(auxid)
    centers[auxid] = numpy.arange(nclus) + 1
    if nclus <= 1:
        labels = numpy.ones(npnts)
    else:
        centersx = numpy.where(centers)[0]
        dist2cent = dist.get_rows(centersx)
        labels = numpy.argmin(dist2cent, axis=0) + 1
        _, cluscounts = numpy.unique(labels, return_counts=True)
...
def halo_assign(dist, labels, centers, n_min):
    ...
    sameclus_cent = sameclusmat[centers > 0, :]
    dist2cent = dist.get_rows(numpy.where(centers > 0)[0])
    dist2cluscent = dist2cent * sameclus_cent
    nclusmem = numpy.sum(sameclus_cent, axis=1)
    meandist2cent = numpy.sum(dist2cluscent, axis=1) / nclusmem
...
def merging(groups, merging_method, merging_param, data):
    ...
    def perform_merging(groups, merging_method, merging_param, data):
            ...
            for ic2 in range(ic1 + 1, len(clusters)):
                ...
                if merging_method == 'folding':
                    ...
                    if merging_method == 'distance':
                        ...
                        if merging_method == 'bhatta':
                            try:
                                dist = bhatta_dist(pr_1, pr_2)
                            except Exception:
                                dist = numpy.inf

                    if merging_method == 'nd-bhatta':
                        try:
        ...
        if dmin < thr:
            ...
            merge = (c1, c2)
            return (
             True, groups, merge, dmin)
        else:
            return (
             False, groups, None, None)
...
def slice_clusters(params, result, to_remove=[], to_merge=[], extension='', input_extension='', light=False, method='safe'):
            ...
            if not been_found:
                ...
                for j in all_idx[l + 1:]:
                    ...
                    if is_a1:
                        ...
                        if similarity > cc_merge and local_overlap < cc_merge and k not in mixtures:
                            mixtures += [k]
                            been_found = True
                            break
