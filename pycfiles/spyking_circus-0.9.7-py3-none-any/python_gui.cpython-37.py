...
# [GCC 8.4.0]
# Embedded file name: /home/pierre/github/spyking-circus/build/lib/circus/scripts/python_gui.py
...
import sys, subprocess, pkg_resources, argparse, circus, tempfile, numpy, logging
            ...
            if not os.path.exists(output_path):
                ...
                if file_format in supported_by_phy:
                    if not params.getboolean('data', 'overwrite'):
                        gui_params['dat_path'] = '%s' % params.get('data', 'data_file_no_overwrite')
                    else:
                        if params.get('data', 'stream_mode') == 'multi-files':
                            data_file = params.get_data_file(source=True, has_been_created=False)
                            gui_params['dat_path'] = ['%s' % f for f in data_file.get_file_names()]
                        else:
                            gui_params['dat_path'] = '%s' % params.get('data', 'data_file')
                else:
