...
# [GCC 8.4.0]
# Embedded file name: /home/pierre/github/spyking-circus/build/lib/circus/fitting.py
# Compiled at: 2020-04-16 10:00:35
# Size of source mod 2**32: 32768 bytes
...
def main(params, nb_cpu, nb_gpu, use_gpu):
    ...
    if collect_all:
        ...
        garbage_times_file = None
        garbage_temp_file = None
    if debug:
        ...
        comm.Barrier()
        peak_local_time_steps_debug_file = open((file_out_suff + '.peak_local_time_steps_debug_%d.data' % comm.rank),
          mode='wb')
        comm.Barrier()
        peak_time_steps_debug_file = open((file_out_suff + '.peak_time_steps_debug_%d.data' % comm.rank), mode='wb')
        comm.Barrier()
        peak_scalar_products_debug_file = open((file_out_suff + '.peak_scalar_products_debug_%d.data' % comm.rank),
          mode='wb')
        comm.Barrier()
            ...
            if not is_last:
                if is_last:
                    ...
                    if is_first:
                        ...
                        padding = (
                         -temp_3_shift, temp_3_shift)
        else:
            padding = (0, 0)
        result = {'spiketimes':[],  'amplitudes':[],  'templates':[]}
        result_debug = {'chunk_nbs':[],  'iteration_nbs':[],  'peak_nbs':[],  'peak_local_time_steps':[],  'peak_time_steps':[],  'peak_scalar_products':[],  'peak_solved_flags':[],  'template_nbs':[],  'success_flags':[]}
        ...
        if use_gpu:
            ...
            while numpy.mean(failure) < total_nb_chances:
                ...
                if (a_min <= best_amp_n) & (best_amp_n <= a_max):
                    ...
                    if t_spike >= local_restriction[0]:
                        if t_spike < local_restriction[1]:
