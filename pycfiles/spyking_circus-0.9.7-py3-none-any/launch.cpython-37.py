...
# [GCC 8.4.0]
# Embedded file name: /home/pierre/github/spyking-circus/build/lib/circus/scripts/launch.py
# Compiled at: 2020-04-08 13:35:26
# Size of source mod 2**32: 19918 bytes
import os, sys, argparse, shutil, subprocess, psutil, warnings
with warnings.catch_warnings():
    warnings.filterwarnings('ignore', category=FutureWarning)
    import h5py
import pkg_resources, circus, logging, numpy
...
def main(argv=None):
    if argv is None:
        ...
        parser.add_argument('-m', '--method', default='filtering,whitening,clustering,fitting,merging',
          help=method_help)
        parser.add_argument('-c', '--cpu', type=int, default=(int(psutil.cpu_count() / 2)), help='number of CPU')
        parser.add_argument('-H', '--hostfile', help='hostfile for MPI', default=(pjoin(user_path, 'circus.hosts')))
        parser.add_argument('-b', '--batch', help='datafile is a list of commands to launch, in a batch mode', action='store_true')
        parser.add_argument('-p', '--preview', help='GUI to display the first second filtered with thresholds', action='store_true')
        parser.add_argument('-r', '--result', help='GUI to display the results on top of raw data', action='store_true')
        parser.add_argument('-s', '--second', type=int, default=0, help='If preview mode, begining of the preview [in s]')
        parser.add_argument('-e', '--extension', help='extension to consider for merging, converting and deconverting', default='None')
        parser.add_argument('-o', '--output', help='output file [for generation of synthetic benchmarks]')
        parser.add_argument('-t', '--type', help='benchmark type', choices=[
         'fitting', 'clustering', 'synchrony'])
        if len(argv) == 0:
            parser.print_help()
            sys.exit(0)
        ...
        if not os.path.exists(file_params):
            batch or print(Fore.RED + 'The parameter file %s is not present!' % file_params)
            ...
            if not batch:
                ...
                if not os.path.exists(file_params):
                    ...
                    if use_output_dir:
                        ...
                        os.path.exists(file_out) or os.makedirs(file_out)
                    else:
                        file_out = f_next
        ...
        if tasks_list is not None:
            ...
            if not SHARED_MEMORY:
                ...
                if params.getboolean('detection', 'matched-filter'):
                    if not params.getboolean('clustering', 'smart_search'):
                ...
                if n_edges > 100:
                    if not params.getboolean('clustering', 'compress'):
                                        ...
                                        if subtask in ('filtering', 'benchmarking') and not is_writable:
                                            if not preview:
                                                if overwrite:
                                                    ...
                                                    sys.exit(0)
                                    if subtask in ('filtering', ) and not support_parallel_write:
                                        if args.cpu > 1:
                                            ...
                                            if subtask != 'fitting':
                                                ...
                                                nb_tasks = str(args.cpu)
                                        if subtask == 'benchmarking' and not output is None:
