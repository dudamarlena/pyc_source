...
# [GCC 8.4.0]
# Embedded file name: /home/pierre/github/spyking-circus/build/lib/circus/shared/parser.py
# Compiled at: 2020-04-20 04:52:20
# Size of source mod 2**32: 45742 bytes
"""
parser.py
...
class CircusParser(object):
    ...
    __all_sections__ = [
     'data', 'whitening', 'extracting', 'clustering',
    ...
    def __init__(self, file_name, create_folders=True, **kwargs):
        ...
        if not valid_path:
            ...
            if comm.rank == 0:
                ...
                self.parser.set('data', 'file_out', file_out)
                self.parser.set('data', 'file_out_suff', file_out + self.parser.get('data', 'suffix'))
                self.parser.set('data', 'data_file_noext', f_next)
                try:
        ...
        if self.parser.getboolean('triggers', 'clean_artefact') and not self.parser.get('triggers', 'trig_file') == '':
            if self.parser.get('triggers', 'trig_windows') == '':
                if comm.rank == 0:
            ...
            test = self.parser.get('clustering', 'extraction').lower() in ('median-raw',
                                                                           'mean-raw')
            if not test:
                if comm.rank == 0:
                    print_and_log(['Only 4 extraction modes in [clustering]: median-raw, mean-raw!'], 'error', logger)
                sys.exit(0)
            test = self.parser.get('detection', 'peaks').lower() in ('negative', 'positive',
                                                                     'both')
            if not test:
                if comm.rank == 0:
            ...
            if method in ('folding', 'nd-folding'):
                self.parser.set('clustering', 'merging_param', '1e-9')
            else:
                if method in ('bhatta', 'nd-bhatta'):
        ...
        if not test:
            ...
            if self.parser.get('converting', 'export_pcs').lower() == 'none':
                ...
                if self.parser.get('converting', 'export_pcs').lower() == 'some':
                    self.parser.set('converting', 'export_pcs', 's')
                else:
                    if self.parser.get('converting', 'export_pcs').lower() == 'all':
    ...
    def _update_rate_values(self):
        ...
        if self._N_t is None:
            ...
            self._N_t = int(self.rate * self._N_t * 0.001)
            ...
            self.set('detection', 'spike_width', int(self.rate * spike_width * 0.001))
            jitter = self.getfloat('detection', 'jitter_range')
            jitter_range = int(self.rate * jitter * 0.001)
            ...
            for section in ('clustering', 'whitening', 'extracting'):
                ...
                if safety_time == 'auto':
                    ...
                    safety_time = int(float(safety_time) * self.rate * 0.001)
                    self.set(section, 'safety_time', safety_time)

            refractory = self.getfloat('fitting', 'refractory')
            self.set('fitting', 'refractory', int(refractory * self.rate * 0.001))
    ...
    def _create_data_file(self, data_file, is_empty, params, stream_mode):
        ...
        if N_e > self.nb_channels:
            if comm.rank == 0:
                lines = [
                 'Analyzed %d channels but only %d are recorded' % (N_e, self.nb_channels)]
    ...
    def get_data_file(self, is_empty=False, params=None, source=False, has_been_created=True):
        ...
        if not self.getboolean('data', 'overwrite'):
            source or print_and_log(['Reading first the real data file to get the parameters'], 'debug', logger)
    ...
    def write(self, section, flag, value, preview_path=False):
        ...
        if comm.rank == 0:
            ...
            for count, line in enumerate(lines):
                if idx == 1:
                    if line.strip().replace('[', '').replace(']', '') in self.__all_sections__:
