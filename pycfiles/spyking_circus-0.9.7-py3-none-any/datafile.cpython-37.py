...
# [GCC 8.4.0]
# Embedded file name: /home/pierre/github/spyking-circus/build/lib/circus/files/datafile.py
# Compiled at: 2020-04-20 05:25:22
# Size of source mod 2**32: 22162 bytes
...
def get_offset(data_dtype, dtype_offset):
    if dtype_offset == 'auto':
        if data_dtype in ['uint16', numpy.uint16, '<u2', '>u2']:
            ...
            if data_dtype in ['int16', numpy.int16, '<i2', '>i2']:
                dtype_offset = 0
            else:
                if data_dtype in ['int32', numpy.int32, '<i4', '>i4']:
                    dtype_offset = 0
                else:
                    if data_dtype in ['int64', numpy.int64, '<i8', '>i8']:
                        dtype_offset = 0
                    else:
                        if data_dtype in ['float32', numpy.float32]:
                            dtype_offset = 0
                        else:
                            if data_dtype in ['int8', numpy.int8]:
                                dtype_offset = 0
                            else:
                                if data_dtype in ['uint8', numpy.uint8, '<u1', '>u1']:
                                    dtype_offset = 127
                                else:
                                    if data_dtype in ['float64', numpy.float64]:
                                        dtype_offset = 0
        if comm.rank == 0:
...
class DataFile(object):
    ...
    def __init__(self, file_name, params, is_empty=False, stream_mode=None):
        ...
        if not is_empty:
            ...
            self.is_stream = True
            if stream_mode not in self.is_streamable:
                if self.is_master:
            ...
            self.is_empty or self._fill_from_header(self._read_from_header())
        else:
            self._shape = (0, 0)
    ...
    def read_chunk(self, idx, chunk_size, padding=(0, 0), nodes=None, mode='r'):
        """
    ...
    def _check_extension(self, extension):
        if len(self.extension) > 0:
            if extension not in self.extension + [item.upper() for item in self.extension]:
                if self.is_master:
    ...
    def _display_requirements_(self):
        to_write = [
         'The parameters for %s file format are:' % self.description.upper(), '']
        nb_params = 0
        for key, value in list(self._required_fields.items()):
    ...
    def _scale_data_to_float32(self, data):
        ...
        if self.data_dtype != numpy.float32:
            ...
            if numpy.any(self.gain != 1):
                data *= self.gain
            data = data.flags['C_CONTIGUOUS'] or numpy.ascontiguousarray(data)
        return data
    ...
    def get_snippet(self, global_time, length, nodes=None, mode='r'):
        """
        ...
        return self.get_data(0, chunk_size=length, padding=(local_time, local_time), nodes=nodes, mode=mode)[0]

    def get_data(self, idx, chunk_size, padding=(0, 0), nodes=None, mode='r'):
        if self.is_stream:
            cidx = numpy.searchsorted(self._chunks_in_sources, idx, 'right') - 1
            idx -= self._chunks_in_sources[cidx]
            return (self._sources[cidx].read_chunk(idx, chunk_size, padding, nodes, mode), self._sources[cidx].t_start + idx * chunk_size)
        return (self.read_chunk(idx, chunk_size, padding, nodes, mode), self.t_start + idx * chunk_size)
