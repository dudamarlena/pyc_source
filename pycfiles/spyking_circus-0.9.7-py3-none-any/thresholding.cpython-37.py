...
# [GCC 8.4.0]
# Embedded file name: /home/pierre/github/spyking-circus/build/lib/circus/thresholding.py
# Compiled at: 2020-03-11 06:54:19
# Size of source mod 2**32: 9891 bytes
        ...
        if do_spatial_whitening:
            ...
            if matched_filter:
                if sign_peaks in ('positive', 'both'):
                    ...
                    for i in range(N_e):
                        peaktimes = scipy.signal.find_peaks((filter_chunk[:, i]),
                          height=(matched_tresholds_pos[i]), width=spike_width, distance=dist_peaks, wlen=N_t)[0]
                ...
                if sign_peaks in ('negative', 'both'):
                    ...
                    for i in range(N_e):
                        peaktimes = scipy.signal.find_peaks((filter_chunk[:, i]),
                          height=(matched_tresholds_neg[i]), width=spike_width, distance=dist_peaks, wlen=N_t)[0]
                        local_peaktimes.append(peaktimes)
                        local_elecs.append(i * numpy.ones((len(peaktimes)), dtype='uint32'))
                        local_amps.append(filter_chunk[(peaktimes, i)])
                ...
                for i in range(N_e):
                    if sign_peaks == 'negative':
                        peaktimes = scipy.signal.find_peaks((-local_chunk[:, i]),
                          height=(thresholds[i]), width=spike_width, distance=dist_peaks, wlen=N_t)[0]
                    else:
