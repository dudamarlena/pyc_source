...
# [GCC 8.4.0]
# Embedded file name: /home/pierre/github/spyking-circus/build/lib/circus/files/rhd.py
# Compiled at: 2020-03-11 06:54:20
# Size of source mod 2**32: 13528 bytes
...
def read_header(fid):
    ...
    if magic_number != int('c6912702', 16):
        ...
        freq['dsp_enabled'], freq['actual_dsp_cutoff_frequency'], freq['actual_lower_bandwidth'], freq['actual_upper_bandwidth'], freq['desired_dsp_cutoff_frequency'], freq['desired_lower_bandwidth'], freq['desired_upper_bandwidth'] = struct.unpack('<hffffff', fid.read(26))
        notch_filter_mode, = struct.unpack('<h', fid.read(2))
        header['notch_filter_frequency'] = 0
        if notch_filter_mode == 1:
            header['notch_filter_frequency'] = 50
        else:
            if notch_filter_mode == 2:
    ...
    header['notes'] = {'note1':note1, 
     'note2':note2, 
     'note3':note3}
    header['num_temp_sensor_channels'] = 0
    if not (version['major'] == 1 and version['minor'] >= 1):
        if version['major'] > 1:
...
def get_bytes_per_data_block(header):
    ...
    bytes_per_block = bytes_per_block + 2 * header['num_supply_voltage_channels']
    ...
    if header['num_temp_sensor_channels'] > 0:
        bytes_per_block = bytes_per_block + 2 * header['num_temp_sensor_channels']
