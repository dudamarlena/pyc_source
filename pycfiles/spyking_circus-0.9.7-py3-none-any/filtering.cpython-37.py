...
# [GCC 8.4.0]
# Embedded file name: /home/pierre/github/spyking-circus/build/lib/circus/filtering.py
# Compiled at: 2020-04-20 05:25:22
# Size of source mod 2**32: 18249 bytes
"""
filtering.py
...
def check_if_done(params, flag, logger):
    ...
    if value == 'started':
        ...
        if comm.rank == 0:
            if flag == 'filter_done':
                msg = [
                 'Code was interrupted while filtering', common_sentence, particular_sentence]
            else:
                if flag == 'artefacts_done':
                    msg = [
                     'Code was interrupted while removing artefacts', common_sentence, particular_sentence]
                else:
                    if flag == 'median_done':
                        msg = [
                         'Code was interrupted while removing median', common_sentence, particular_sentence]
                    else:
                        if flag == 'ground_done':
                            msg = [
                             'Code was interrupted while removing ground', common_sentence, particular_sentence]
                        else:
                            msg = [
                             'Code was interrupted']
...
def main(params, nb_cpu, nb_gpu, use_gpu):
    ...
    def filter_file(data_file_in, data_file_out, do_filtering, do_remove_median, do_remove_ground):
        try:
            ...
            cut_off = [cut_off, 0.95 * (params.rate / 2.0)]
        except Exception:
        ...
        b, a = signal.butter(butter_order, np.array(cut_off) / (params.rate / 2.0), 'pass')
        ...
        for count, gidx in enumerate(to_explore):
            ...
            if is_first:
                ...
                padding = (0, 0)
            local_chunk, t_offset = data_file_in.get_data(gidx, chunk_size, padding, mode='r+')
            if do_filtering:
            ...
            if data_file_in != data_file_out:
                if data_file_in.is_first_chunk(gidx, nb_chunks):
    ...
    def compute_artefacts(data_file):
        ...
        if trig_in_ms:
            ...
            artefacts[:, 1] *= numpy.int64(data_file.sampling_rate * 0.001)
            windows[:, 1] *= numpy.int64(data_file.sampling_rate * 0.001)
        else:
            if comm.rank == 0:
            ...
            for count, artefact in enumerate(local_labels):
                ...
                if make_plots not in ('None', ''):
                    save = [
                     plot_path, '%d.%s' % (artefact, make_plots)]
    ...
    def remove_artefacts(data_file, art_dict):
        ...
        if trig_in_ms:
            ...
            if comm.rank == 0:
                to_write = [
                 'Removing artefacts from %d stimuli' % nb_stimuli]
                print_and_log(to_write, 'default', logger)
    ...
    if params.getboolean('data', 'overwrite'):
        ...
        if comm.rank == 0:
            ...
            if comm.rank == 0:
                ...
                if not (has_been_created or filter_done):
                    if median_done or artefacts_done:
                        if comm.rank == 0:
                ...
                if comm.rank == 0 and clean_artefact:
                    params.write('noedits', 'artefacts_done', 'True')
            data_file_in.close()
            params.getboolean('data', 'overwrite') or data_file_out.close()
        comm.Barrier()