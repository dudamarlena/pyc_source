...
# [GCC 8.4.0]
# Embedded file name: /home/pierre/github/spyking-circus/build/lib/circus/whitening.py
# Compiled at: 2019-12-18 09:51:15
# Size of source mod 2**32: 31784 bytes
...
def main(params, nb_cpu, nb_gpu, use_gpu):
    ...
    for gidx in [all_chunks[comm.rank]]:
        ...
        local_borders = (
         template_shift, local_shape - template_shift)
    ...
    if comm.rank == 0:
        ...
        if do_temporal_whitening:
            ...
            temporal_whitening = get_whitening_matrix((all_res_temp.astype(numpy.double)), fudge=0.001)[template_shift].astype(numpy.float32)
    ...
    for gcount, gidx in enumerate(to_explore):
        ...
        local_borders = (
         snippet_duration, local_shape - snippet_duration)
    ...
    if comm.rank == 0:
        ...
        if sign_peaks in ('negative', 'both'):
            ...
            if ratio < 0.4:
                ...
                while ratio < 0.4:
                    to_crop += 1
                    ratio = numpy.sum(dispersion[to_crop:-to_crop] > 1.1) / float(len(dispersion[to_crop:-to_crop]))

                to_crop /= params.data_file.sampling_rate * 0.001
                suggestion = (N_t - 2 * tp_crop) / (params.data_file.sampling_rate * 0.001)
                print_and_log(['Time window N_t in [detection] is too large! Try %g ms' % suggestion], 'info', logger)
                warning_n_t = True
            else:
                if ratio == 1:
