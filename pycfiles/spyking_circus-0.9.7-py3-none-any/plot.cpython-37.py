...
# [GCC 8.4.0]
# Embedded file name: /home/pierre/github/spyking-circus/build/lib/circus/shared/plot.py
# Compiled at: 2020-04-18 02:50:15
# Size of source mod 2**32: 82579 bytes
...
def view_fit(file_name, t_start=0, t_stop=1, n_elec=2, fit_on=True, square=True, templates=None, save=False):
    ...
    if do_temporal_whitening:
        ...
        if fit_on:
            ...
            elif not numpy.iterable(n_elec):
                if square:
            ...
            for count, i in enumerate(idx):
                ...
                if not square or count < n_elec * (n_elec - 1):
                    if not square:
                        if not count != n_elec - 1:
                            x, y = pylab.xticks()
...
def view_clusters(data, rho, delta, centers, halo, injected=None, save=False, alpha=3):
    ...
    ax.scatter((data[(assigned[idx], 0)]),
      (data[(assigned[idx], 1)]), s=marker_size, c=(rho[assigned[idx]]), cmap=my_cmap, linewidths=0)
    ...
    ax.scatter((data[(assigned[idx], 0)]),
      (data[(assigned[idx], 1)]), s=marker_size, c=(numpy.log(1 + delta[assigned[idx]])),
      cmap=my_cmap,
      linewidths=0)
    ax.scatter((data[(centers, 0)]), (data[(centers, 1)]), s=marker_size, c='r', linewidths=0)
    if injected is not None:
...
def view_local_merges(waveforms_data, clusters_data, old_allocation, new_allocation, merge_history, save=False, max_nb_traces=200):
    ...
    if nb_local_merges > 0:
        ...
        for local_merge, cluster_distance in zip(local_merges, cluster_distances):
            ...
            local_merge_groups[cluster_nb_1] = (
             group_1, group_2)
        ...
        if not len(local_merge_groups) == len(local_merge_flat_groups):
            ...
            nb_columns = 2 + max_nb_clusters_per_group
            ...
            colors = {cluster_nb:mcolors.to_rgb('C{}'.format(k % 10)) for k, cluster_nb in enumerate(cluster_nbs)}
            ...
            c = np.array([colors[cluster_nb] for cluster_nb in allocation[is_assigned]])
            ax.scatter(x, y, c=c, cmap=color_map, linewidth=0, s=marker_size)
            nb_clusters = len(np.unique(allocation[is_assigned]))
            ax.annotate(('{:d} clusters'.format(nb_clusters)),
              xy=(0, 0),
            ...
            ax.annotate(('{} (thr.={:f})'.format(merging_method, merging_threshold)),
              xy=(0.0, 0.0),
...
def view_local_merges_backup(waveforms_data, clusters_data, old_allocation, new_allocation, merge_history, save=False, max_nb_traces=200):
    import matplotlib.pyplot as plt
    marker_size = 10
    color_map = plt.get_cmap('jet')
    ...
    for merge_nb in range(0, nb_merges):
        ...
        selection = numpy.logical_or(allocation == cluster_nb_1, allocation == cluster_nb_2)
        selected_nbs = np.where(selection)[0]
        selected_nbs = np.random.permutation(selected_nbs)
        selected_nbs = selected_nbs[0:max_nb_traces]
...
def view_waveforms(file_name, temp_id, n_spikes=2000):
    ...
    for count, t_spike in enumerate(numpy.random.permutation(spikes)[:n_spikes]):
        padding = (
         t_spike - int(N_t - 1) // 2, t_spike - int(N_t - 1) // 2)
...
def view_templates(file_name, temp_id=0, best_elec=None, templates=None):
    ...
    probe = params.probe
    positions = {}
    for i in list(probe['channel_groups'].keys()):
    ...
    if best_elec is None:
        ...
        for count, i in enumerate(range(N_e)):
            ...
            if i == best_elec:
                ...
                if i in inv_nodes[edges[nodes[best_elec]]]:
                    c = 'k'
                else:
                    c = '0.5'
...
def raster_plot(file_name):
    ...
    return (
     numpy.array(times), numpy.array(templates))
        ...
        if 'minimal' == mode:
            ...
            if 'maximal' == mode:
                ...
                for k, ss in enumerate(gs):
                    ...
                    if 0 == k:
                        ax.imshow((mean_spike.T), cmap='seismic', interpolation='nearest', vmin=v_min,
                          vmax=v_max)
                        ax.set_title('mean spike')
                    else:
                        idx = idxs[(k - 1)]
                        ax.imshow((spikes[:, :, idx].T), cmap='seismic', interpolation='nearest', vmin=v_min,
                          vmax=v_max)
                ...
                iqr = q75 - q25
                h = 2.0 * iqr * float(spike_amplitudes.shape[0]) ** (-0.3333333333333333)
        ...
        for mode in ('predict', 'decision_function'):
            ...
            if mode == 'predict':
                ...
                if mode == 'decision_function':
                    ...
                    _ = p.fit(X_raw, y)
                    X_raw_ = p.transform(X_raw)
                    sc = ax.scatter((X_raw_[:, 0]), (X_raw_[:, 1]), c=c, s=5, lw=0.1, cmap='bwr', vmin=vmin,
                      vmax=vmax)
                    cb = fig.colorbar(sc)
                    ax.grid(True)
    ...
    if fpr is not None:
        ...
        if HAVE_RESULT:
            ...
            if scerror is None:
                ...
                thresh = int(params.getint('data', 'sampling_rate') * 2 * 0.001)
                result = load_data(params, 'results')
                ...
                for i in range(templates.shape[1] / 2):
                    d = numpy.corrcoef(templates[:, i].toarray().flatten(), source_temp)[(0,
                                                                                          1)]
