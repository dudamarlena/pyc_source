...
# [GCC 8.4.0]
# Embedded file name: /home/pierre/github/spyking-circus/build/lib/circus/shared/mpi.py
# Compiled at: 2020-03-13 05:34:40
# Size of source mod 2**32: 8137 bytes
import numpy, os, mpi4py, logging, sys, mpi4py
...
def test_mpi_ring(nb_nodes):
    if comm.size != nb_nodes:
        if comm.rank == 0:
            lines = ['The MPI install does not seems to be correct!',
...
def gather_mpi_arguments(hostfile, params):
    ...
    if MPI_VENDOR[0] == 'Open MPI':
        ...
        if MPI_VENDOR[0] == 'Microsoft MPI':
            mpi_args = [
             'mpiexec']
            if os.path.exists(hostfile):
                mpi_args += ['-machinefile', hostfile]
        else:
            if MPI_VENDOR[0] == 'MPICH2':
                mpi_args = [
                 'mpiexec']
                if os.path.exists(hostfile):
                    mpi_args += ['-f', hostfile]
                ...
                print_and_log([
                 '%s may not be yet properly implemented: contact developpers' % MPI_VENDOR[0]], 'error', logger)
                mpi_args = ['mpirun']
                if os.path.exists(hostfile):
                    mpi_args += ['-hostfile', hostfile]
            return mpi_args
