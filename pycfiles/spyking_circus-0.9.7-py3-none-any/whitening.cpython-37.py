...
# Size of source mod 2**32: 33938 bytes
            ...
            if ignore_spikes:
                ...
                for i in range(N_e):
                    peaktimes = scipy.signal.find_peaks((numpy.abs(local_chunk[:, i])),
                      height=(thresholds[i]), width=spike_width, wlen=N_t)[0]
            ...
            for i in range(N_e):
                ...
                if ignore_dead_times:
                    if dead_indices[0] != dead_indices[1]:
                        is_included = numpy.in1d(peaktimes + t_offset, all_dead_times[dead_indices[0]:dead_indices[1]])
            ...
            if len(local_peaktimes) > 0:
                ...
                for midx, peak in zip(argmax_peak, all_idx):
                    ...
                    if groups[elec] < upper_bounds:
                        ...
                        sub_mat = myslice.any() or local_chunk[peak - snippet_duration:peak + snippet_duration + 1, elec]
                        if reject_noise:
