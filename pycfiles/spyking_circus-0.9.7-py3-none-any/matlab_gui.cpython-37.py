...
# [GCC 8.4.0]
# Embedded file name: /home/pierre/github/spyking-circus/build/lib/circus/scripts/matlab_gui.py
# Compiled at: 2020-03-11 06:54:24
# Size of source mod 2**32: 6102 bytes
import os, sys, subprocess, argparse, pkg_resources, circus, tempfile, numpy, logging
...
def main(argv=None):
    if argv is None:
        ...
        parser.add_argument('-e', '--extension', help='extension to consider for visualization', default='')
        if len(argv) == 0:
            parser.print_help()
            sys.exit()
        else:
            ...
            if os.path.exists(params.logfile):
                ...
                def generate_matlab_mapping(probe):
                    ...
                    cfile = h5py.File(t, 'w')
                    to_write = {'positions':positions / 10.0, 
            ...
            if file_format not in supported_by_matlab:
                gui_params = [
                 params.rate, os.path.abspath(file_out_suff), '%s.mat' % extension, mapping, 2, t_start]
        ...
        arguments = ', '.join(["'%s'" % arg if s else '%s' % arg for arg, s in zip(gui_params, is_string)])
