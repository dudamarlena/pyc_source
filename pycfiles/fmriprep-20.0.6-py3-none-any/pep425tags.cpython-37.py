...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-92t6atcz/wheel/wheel/pep425tags.py
# Compiled at: 2020-04-16 14:32:20
# Size of source mod 2**32: 9071 bytes
"""Generate and work with PEP 425 Compatibility Tags."""
import distutils.util, platform, sys, os, sysconfig, warnings
from .macosx_libfile import extract_macosx_min_system_version
try:
    import importlib.machinery as get_all_suffixes
except ImportError:
...
def get_abi_tag():
    ...
    impl = get_abbr_impl()
    if (soabi or impl) in ('cp', 'pp') and hasattr(sys, 'maxunicode'):
        d = ''
        m = ''
        u = ''
...
def calculate_macosx_platform_tag(archive_root, platform_tag):
    ...
    if len(base_version) >= 2:
        ...
        if 'MACOSX_DEPLOYMENT_TARGET' in os.environ:
            deploy_target = tuple([int(x) for x in os.environ['MACOSX_DEPLOYMENT_TARGET'].split('.')])
