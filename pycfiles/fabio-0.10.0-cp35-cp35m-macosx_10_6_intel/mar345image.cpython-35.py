...
# [GCC 8.4.0]
# Embedded file name: /Users/kieffer/workspace/fabio/build/lib.macosx-10.6-intel-3.5/fabio/mar345image.py
# Compiled at: 2020-04-03 09:02:03
# Size of source mod 2**32: 17328 bytes
"""
...
__licence__ = 'MIT'
import struct, time, logging, numpy, fabio
...
class Mar345Image(FabioImage):
    ...
    DEFAULT_EXTENSIONS = [
     'mar2300', 'mar1200', 'mar1600', 'mar2000',
     'mar3450', 'mar3000', 'mar2400', 'mar1800']
    ...
    def _readheader(self, infile=None):
        ...
        self.numhigh = struct.unpack(fs, data[8:12])[0]
        h['NumHigh'] = self.numhigh
        i = struct.unpack(fs, data[12:16])[0]
        if i == 1:
            h['Format'] = 'compressed'
        else:
            if i == 2:
                ...
                h['Format'] = 'compressed'
                logger.warning('image format could not be determined' + '- assuming compressed mar345')
            h['Mode'] = {0: 'Dose', 1: 'Time'}[struct.unpack(fs, data[16:20])[0]]
            self.numpixels = struct.unpack(fs, data[20:24])[0]
            h['NumPixels'] = str(self.numpixels)
            dim2 = self.numpixels // dim1
            self._shape = (dim2, dim1)
            h['PixelLength'] = struct.unpack(fs, data[24:28])[0] / 1000.0
            h['PixelHeight'] = struct.unpack(fs, data[28:32])[0] / 1000.0
            h['Wavelength'] = struct.unpack(fs, data[32:36])[0] / 1000000.0
            h['Distance'] = struct.unpack(fs, data[36:40])[0] / 1000.0
            h['StartPhi'] = struct.unpack(fs, data[40:44])[0] / 1000.0
            h['EndPhi'] = struct.unpack(fs, data[44:48])[0] / 1000.0
            h['StartOmega'] = struct.unpack(fs, data[48:52])[0] / 1000.0
            h['EndOmega'] = struct.unpack(fs, data[52:56])[0] / 1000.0
            h['Chi'] = struct.unpack(fs, data[56:60])[0] / 1000.0
            h['TwoTheta'] = struct.unpack(fs, data[60:64])[0] / 1000.0
            data = f.read(128)
            if b'mar research' not in data:
                logger.warning('the string "mar research" should be in ' + 'bytes 65-76 of the header but was not')
                start = 128
            else:
        ...
        for m in data.splitlines():
            ...
            if n[0] == '':
                ...
                if n[0] in ('PROGRAM', 'DATE', 'SCANNER', 'HIGH', 'MULTIPLIER', 'GAIN',
                            'WAVELENGTH', 'DISTANCE', 'RESOLUTION', 'CHI', 'TWOTHETA',
                            'MODE', 'TIME', 'GENERATOR', 'MONOCHROMATOR', 'REMARK'):
                ...
                if n[0] in 'FORMAT':
                    ...
                    continue
                    if n[0] in ('PIXEL', 'OFFSET', 'PHI', 'OMEGA', 'COUNTS', 'CENTER',
                                'INTENSITY', 'HISTOGRAM', 'COLLIMATOR'):
                        n = m.split()
    ...
    def binary_header(self):
        ...
        binheader[8] = int(float(self.header.get('WAVELENGTH', 1)) * 1000000.0)
        binheader[9] = int(float(self.header.get('DISTANCE', 1)) * 1000.0)
        binheader[10] = int(float(self.header.get('PHI_START', 1)) * 1000.0)
        binheader[11] = int(float(self.header.get('PHI_END', 1)) * 1000.0)
        binheader[12] = int(float(self.header.get('OMEGA_START', 1)) * 1000.0)
        binheader[13] = int(float(self.header.get('OMEGA_END', 1)) * 1000.0)
        binheader[14] = int(float(self.header.get('CHI', 1)) * 1000.0)
        binheader[15] = int(float(self.header.get('TWOTHETA', 1)) * 1000.0)
