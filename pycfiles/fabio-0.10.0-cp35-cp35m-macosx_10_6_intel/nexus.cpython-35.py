...
# [GCC 8.4.0]
# Embedded file name: /Users/kieffer/workspace/fabio/build/lib.macosx-10.6-intel-3.5/fabio/nexus.py
# Compiled at: 2020-04-03 09:02:03
# Size of source mod 2**32: 13796 bytes
...
import logging, numpy, os, sys, time
...
try:
    ...
    class Nexus(object):
        ...
        """
            for grp_name in self.h5:
                if grp_name == name:
                    ...
                    if isinstance(grp, h5py.Group) and 'start_time' in grp and 'NX_class' in grp.attrs and grp.attrs['NX_class'] == 'NXentry':
                        return grp
        ...
        """
            entries = [(grp, from_isotime(self.h5[(grp + '/start_time')][()])) for grp in self.h5 if isinstance(self.h5[grp], h5py.Group) and 'start_time' in self.h5[grp] and 'NX_class' in self.h5[grp].attrs and self.h5[grp].attrs['NX_class'] == 'NXentry']
            if entries:
                entries.sort(key=lambda a: a[1], reverse=True)
                return [self.h5[i[0]] for i in entries]
            else:
                entries = [grp for grp in self.h5 if isinstance(self.h5[grp], h5py.Group) and 'NX_class' in self.h5[grp].attrs and self.h5[grp].attrs['NX_class'] == 'NXentry']
        ...
        def new_entry(self, entry='entry', program_name='pyFAI', title='description of experiment', force_time=None, force_name=False):
            """
        Create a new entry
        ...
        """
            coll = [grp[name] for name in grp if isinstance(grp[name], h5py.Group) and 'NX_class' in grp[name].attrs and grp[name].attrs['NX_class'] == class_type]
            return coll
        ...
        """
            ...
            for grp in self.get_class(grp, class_type):
                result += [grp[name] for name in grp if isinstance(grp[name], h5py.Dataset) and 'signal' in grp[name].attrs]
