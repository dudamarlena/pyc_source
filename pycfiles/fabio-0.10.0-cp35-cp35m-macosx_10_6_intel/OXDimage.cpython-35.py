...
# [GCC 8.4.0]
# Embedded file name: /Users/kieffer/workspace/fabio/build/lib.macosx-10.6-intel-3.5/fabio/OXDimage.py
# Compiled at: 2020-04-03 09:02:03
# Size of source mod 2**32: 27854 bytes
...
class OxdImage(FabioImage):
    ...
    def _readheader(self, infile):
        ...
        if header_version < 4.0:
            ascii_section_size = self.header['Header Size In Bytes'] - (self.header['General Section size in Byte'] + self.header['Special Section size in Byte'] + self.header['KM4 Section size in Byte'] + self.header['Statistic Section in Byte'] + self.header['History Section in Byte'])
        else:
    ...
    def _writeheader(self):
        ...
        if self.header.get('Theta step in deg', None):
            KM.setData(None, 376, numpy.float64, deg2rad(self.header['Theta step in deg']))
            if self.header.get('Theta start in deg', None):
                KM.setData(None, 288, numpy.int32, self.header['Theta start in deg'] / self.header['Theta step in deg'])
            if self.header.get('Theta end in deg', None):
                KM.setData(None, 328, numpy.int32, self.header['Theta end in deg'] / self.header['Theta step in deg'])
            if self.header.get('Theta zero corr. in deg', None):
                KM.setData(None, 516, numpy.int32, self.header['Theta zero corr. in deg'] / self.header['Theta step in deg'])
        if self.header.get('Kappa step in deg', None):
            KM.setData(None, 384, numpy.float64, deg2rad(self.header['Kappa step in deg']))
            if self.header.get('Kappa start in deg', None):
                KM.setData(None, 292, numpy.int32, self.header['Kappa start in deg'] / self.header['Kappa step in deg'])
            if self.header.get('Kappa end in deg', None):
                KM.setData(None, 332, numpy.int32, self.header['Kappa end in deg'] / self.header['Kappa step in deg'])
            if self.header.get('Kappa zero corr. in deg', None):
                KM.setData(None, 520, numpy.int32, self.header['Kappa zero corr. in deg'] / self.header['Kappa step in deg'])
        if self.header.get('Phi step in deg', None):
            KM.setData(None, 392, numpy.float64, deg2rad(self.header['Phi step in deg']))
            if self.header.get('Phi start in deg', None):
                KM.setData(None, 296, numpy.int32, self.header['Phi start in deg'] / self.header['Phi step in deg'])
            if self.header.get('Phi end in deg', None):
                KM.setData(None, 336, numpy.int32, self.header['Phi end in deg'] / self.header['Phi step in deg'])
            if self.header.get('Phi zero corr. in deg', None):
                KM.setData(None, 524, numpy.int32, self.header['Phi zero corr. in deg'] / self.header['Phi step in deg'])
