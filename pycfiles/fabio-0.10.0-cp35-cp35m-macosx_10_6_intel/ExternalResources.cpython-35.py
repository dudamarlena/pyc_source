...
# [GCC 8.4.0]
# Embedded file name: /Users/kieffer/workspace/fabio/build/lib.macosx-10.6-intel-3.5/fabio/utils/ExternalResources.py
# Compiled at: 2020-04-03 09:02:03
# Size of source mod 2**32: 12569 bytes
from fileinput import filename
__authors__ = [
 'Thomas Vincent', 'J. Kieffer']
__license__ = 'MIT'
__date__ = '23/10/2019'
import os, threading, json, logging, tempfile, unittest
...
class ExternalResources(object):
    __doc__ = 'Utility class which allows to download test-data from www.silx.org\n    and manage the temporary data during the tests.\n\n    '
    ...
    def getfile(self, filename):
        ...
        if not os.path.isfile(fullfilename):
            ...
            try:
                data = opener('%s/%s' % (self.url_base, filename), data=None, timeout=self.timeout).read()
            ...
            try:
                ...
                raise IOError('unable to write downloaded                     data to disk at %s' % self.data_home)

            if not os.path.isfile(fullfilename):
                raise RuntimeError('Could not automatically download test images %s!' % filename + 'If you are behind a firewall, please set both environment variable http_proxy and https_proxy.' + 'This works even under windows !' + 'Otherwise please try to download the images manually from' + '%s/%s' % (self.url_base, filename))
    ...
    def getdir(self, dirname):
        ...
        """
        lodn = dirname.lower()
        if lodn.endswith('tar') or lodn.endswith('tgz') or lodn.endswith('tbz2') or lodn.endswith('tar.gz') or lodn.endswith('tar.bz2'):
            import tarfile
    ...
    def get_file_and_repack(self, filename):
        """
        Downloads the requested file and decompress it if needed.
        ...
        if not os.path.isfile(fullimagename_bz2):
            ...
            if not os.path.isfile(fullimagename_bz2):
                raise RuntimeError('Could not automatically download test images %s!' % filename + 'If you are behind a firewall, please set the environment variable http_proxy and https_proxy.' + 'Otherwise please try to download the images manually from' + '%s/%s' % (self.url_base, filename))
        ...
        if not raw_file_exists or not gz_file_exists:
            ...
            if not raw_file_exists:
                try:
                    ...
                    raise IOError('unable to write decompressed                     data to disk at %s' % self.data_home)

            if not gz_file_exists:
                try:
                    gzip.open(fullimagename_gz, 'wb').write(decompressed)
                except IOError:
                    raise IOError('unable to write gzipped                     data to disk at %s' % self.data_home)
